...
typedef u8  TFlowPortId;
typedef u16 TFlowNodeId;
typedef u16 TFlowNodeTypeId;
typedef u32 TFlowGraphId;
typedef i32    TFlowSystemContainerId;
...
{
	eFDT_Any = -1,
	eFDT_Void = drx_variant::get_index<SFlowSystemVoid, TFlowInputDataVariant>::value,
	eFDT_Int = drx_variant::get_index<i32, TFlowInputDataVariant>::value,
	eFDT_Float = drx_variant::get_index<float, TFlowInputDataVariant>::value,
	eFDT_EntityId = drx_variant::get_index<EntityId, TFlowInputDataVariant>::value,
	eFDT_Vec3 = drx_variant::get_index<Vec3, TFlowInputDataVariant>::value,
	eFDT_String = drx_variant::get_index<string, TFlowInputDataVariant>::value,
	eFDT_Bool = drx_variant::get_index<bool, TFlowInputDataVariant>::value,
};

inline EFlowDataTypes FlowNameToType(tukk typeName)
{
...
inline tukk FlowTypeToName(EFlowDataTypes flowDataType)
{
	switch (flowDataType)
	{
	case eFDT_Any:
...
inline tukk FlowTypeToHumanName(EFlowDataTypes flowDataType)
{
	tukk szTypeName = FlowTypeToName(flowDataType);
	if (*szTypeName == '\0')
...
{ \
	...
	FLOWSYSTEM_CONVERTVARIANT_SPECIALIZATION(i32);
...
{ \
	...
	FLOWSYSTEM_CONVERTTOVARIANT_SPECIALIZATION(i32);
...
FLOWSYSTEM_STRING_CONVERSION(i32, "%d");
...
struct SFlowSystemConversion<From, TFlowInputDataVariant>
{
	...
	{
		return drx_variant::ConvertToVariant(from, to);
	}
};
template<>
...
inline bool DefaultInitializedForTag::Initialize<stl::variant_size<TFlowInputDataVariant>::value>(EFlowDataTypes type, TFlowInputDataVariant& var) const
{
	...
	DRX_ASSERT_MESSAGE(var.index() == stl::variant_npos, "Invalid variant index.");
	return false;
}
//! \endcond
...
class TFlowInputData
{
	...
	{
	public:
		MemStatistics(IDrxSizer* pSizer) : m_pSizer(pSizer) {}
...
public:
	...
	static TFlowInputData CreateDefaultInitializedForTag(i32 tag, bool locked = false)
	{
	...
	bool CheckIfForcedConversionOfCurrentValueWithString(const string& valueStr)
	{
		...
		// (eg float to i32 truncates and is accepted)
		...
		if (convertedValueStr.compare(valueStr) != 0)
		{
			...
			case eFDT_Float:
				...
				// intentional fallthrough : float also checks for i32 leading zeros
			case eFDT_Int:
				{
...
template<class T>
ILINE SOutputPortConfig OutputPortConfig(tukk name, tukk description = NULL, tukk humanName = NULL)
{
	SOutputPortConfig result = { name, humanName, description, drx_variant::get_index<T, TFlowInputDataVariant>::value };
	return result;
}

ILINE SOutputPortConfig OutputPortConfig_AnyType(tukk name, tukk description = NULL, tukk humanName = NULL)
{
	SOutputPortConfig result = { name, humanName, description, eFDT_Any };
	return result;
}

ILINE SOutputPortConfig OutputPortConfig_Void(tukk name, tukk description = NULL, tukk humanName = NULL)
{
...
template<class T>
ILINE SInputPortConfig InputPortConfig(tukk name, tukk description = NULL, tukk humanName = NULL, tukk sUIConfig = NULL)
{
...
ILINE SInputPortConfig InputPortConfig(tukk name, const ValueT& value, tukk description = NULL, tukk humanName = NULL, tukk sUIConfig = NULL)
{
	SInputPortConfig result = { name, humanName, description, sUIConfig, TFlowInputData(T(value), true) };
	return result;
}

ILINE SInputPortConfig InputPortConfig_AnyType(tukk name, tukk description = NULL, tukk humanName = NULL, tukk sUIConfig = NULL)
{
	SInputPortConfig result = { name, humanName, description, sUIConfig, TFlowInputData(0, false) };
	return result;
}

ILINE SInputPortConfig InputPortConfig_Void(tukk name, tukk description = NULL, tukk humanName = NULL, tukk sUIConfig = NULL)
{
	SInputPortConfig result = { name, humanName, description, sUIConfig, TFlowInputData(SFlowSystemVoid(), false) };
	return result;
}

ILINE tukk CopyStr(tukk str)
{
	tuk var = new char[strlen(str) + 1];
...
struct SFlowNodeConfig
{
	...
	// Input Config
	void SetInputSize(i32 size)
	{
		pInputPorts = new SInputPortConfig[size + 1];
		memset((uk )pInputPorts, 0, sizeof(SInputPortConfig) * (size + 1));
	}
	void AddInputPort(i32 slot, const SInputPortConfig& cfg)
	{
		memcpy((uk )(&pInputPorts[slot]), (uk )&cfg, sizeof(SInputPortConfig));
	}
	void AddStringInputPort(i32 slot, tukk name, tukk description)
	{
		AddInputPort(slot, InputPortConfig<string>(CopyStr(name), CopyStr(description)));
	}
	void AddVoidInputPort(i32 slot, tukk name, tukk description)
	{
		AddInputPort(slot, InputPortConfig_Void(CopyStr(name), CopyStr(description)));
	}
	void AddIntInputPort(i32 slot, tukk name, tukk description)
	{
		AddInputPort(slot, InputPortConfig<i32>(CopyStr(name), CopyStr(description)));
	}
	void AddBoolInputPort(i32 slot, tukk name, tukk description)
	{
		AddInputPort(slot, InputPortConfig<bool>(CopyStr(name), CopyStr(description)));
	}

	// Output Config
	void SetOutputSize(i32 size)
	{
		pOutputPorts = new SOutputPortConfig[size + 1];
		memset((uk )pOutputPorts, 0, sizeof(SOutputPortConfig) * (size + 1));
	}
	void AddOutputPort(i32 slot, const SOutputPortConfig& cfg)
	{
		memcpy((uk )(&pOutputPorts[slot]), (uk )&cfg, sizeof(SOutputPortConfig));
	}
	void AddStringOutputPort(i32 slot, tukk name, tukk description)
	{
		AddOutputPort(slot, OutputPortConfig<string>(CopyStr(name), CopyStr(description)));
	}
	void AddVoidOutputPort(i32 slot, tukk name, tukk description)
	{
		AddOutputPort(slot, OutputPortConfig_Void(CopyStr(name), CopyStr(description)));
	}
	void AddIntOutputPort(i32 slot, tukk name, tukk description)
	{
		AddOutputPort(slot, OutputPortConfig<i32>(CopyStr(name), CopyStr(description)));
	}
	void AddBoolOutputPort(i32 slot, tukk name, tukk description)
	{
...
struct IFlowNode : public _i_reference_target_t
{
	...
	{
		typedef void (*ActivateOutputCallback)(SActivationInfo *actInfo,i32 nOutputPort, const TFlowInputData& value);

		SActivationInfo(IFlowGraph* pGraph = 0, TFlowNodeId myID = 0, uk pUserData = 0, TFlowInputData* pInputPorts = 0)
		{
			this->pGraph = pGraph;
			this->myID = myID;
		...
		TFlowInputData* GetInputPort(i32 idx) { return &pInputPorts[idx]; }
	};
	...
	virtual bool GetPortGlobalEnum(u32 portId, IEntity* pNodeEntity, tukk szName, string& outGlobalEnum) const { return false; }
...
struct IFlowNodeData
{
	...
	virtual tukk GetNodeName() = 0;
	virtual void        GetConfiguration(SFlowNodeConfig&) = 0;

	//! Gets the amount of ports in the flownode
	virtual i32      GetNumInputPorts() const = 0;
	virtual i32      GetNumOutputPorts() const = 0;
...
struct IFlowGraphHook
{
	...
	virtual bool CreatedNode(TFlowNodeId id, tukk name, TFlowNodeTypeId typeId, IFlowNodePtr pNode) = 0;

	//! This gets called if CreatedNode was called, and then cancelled later
	virtual void                        CancelCreatedNode(TFlowNodeId id, tukk name, TFlowNodeTypeId typeId, IFlowNodePtr pNode) = 0;
...
struct SFlowNodeActivationListener
{
	...
	virtual bool OnFlowNodeActivation(IFlowGraphPtr pFlowgraph, TFlowNodeId srcNode, TFlowPortId srcPort, TFlowNodeId toNode, TFlowPortId toPort, tukk value) = 0;
	// </interfuscator:shuffle>
};
...
struct Wrapper<i32>
{
	explicit Wrapper(i32k& v) : value(v) {}
	i32k& value;
};
template<>
...
struct IFlowSystemTyped
{
	...
	virtual void DoActivatePort(const SFlowAddress, const Wrapper<i32>& value) = 0;
...
struct IFlowGraph : public NFlowSystemUtils::IFlowSystemTyped
{
	...
	virtual void SetGraphEntity(EntityId id, i32 nIndex = 0) = 0;

	//! Retrieves id of the default entity associated with this flow graph.
	virtual EntityId GetGraphEntity(i32 nIndex) const = 0;
	...
	virtual tukk GetDebugName() const = 0;
	//! Set a human readable name for debug purposes
	virtual void SetDebugName(tukk sName) = 0;
	...
	//! Node manipulation.
	//! ##@{.
	virtual SFlowAddress ResolveAddress(tukk addr, bool isOutput) = 0;
	virtual TFlowNodeId  ResolveNode(tukk name) = 0;
	virtual TFlowNodeId  CreateNode(TFlowNodeTypeId typeId, tukk name, uk pUserData = 0) = 0;

	//!< Deprecated.
	virtual TFlowNodeId     CreateNode(tukk typeName, tukk name, uk pUserData = 0) = 0;

	virtual IFlowNodeData*  GetNodeData(TFlowNodeId id) = 0;
	virtual bool            SetNodeName(TFlowNodeId id, tukk sName) = 0;
	virtual tukk     GetNodeName(TFlowNodeId id) = 0;
	virtual TFlowNodeTypeId GetNodeTypeId(TFlowNodeId id) = 0;
	virtual tukk     GetNodeTypeName(TFlowNodeId id) = 0;
	virtual void            RemoveNode(tukk name) = 0;
	...
	virtual bool NotifyFlowNodeActivationListeners(TFlowNodeId srcNode, TFlowPortId srcPort, TFlowNodeId toNode, TFlowPortId toPort, tukk value) = 0;
	...
	//! tukk string to avoid passing string class across dll boundaries
	virtual void                  ActivatePortCString(SFlowAddress output, tukk cstr) = 0;
	...
	virtual bool                  GetActivationInfo(tukk nodeName, IFlowNode::SActivationInfo& actInfo) = 0;
	...
	void ActivateStringOutput(TFlowNodeId nodeId, i32 nPort, const string& value)
	{
		SFlowAddress addr(nodeId, nPort, true);
		ActivatePort(addr, value);
	}
	void ActivateIntOutput(TFlowNodeId nodeId, i32 nPort, i32k& value)
	{
		SFlowAddress addr(nodeId, nPort, true);
		ActivatePort(addr, value);
	}
	void ActivateBoolOutput(TFlowNodeId nodeId, i32 nPort, const bool& value)
	{
		SFlowAddress addr(nodeId, nPort, true);
		ActivatePort(addr, value);
	}
	void ActivateAnyOutput(TFlowNodeId nodeId, i32 nPort)
	{
		SFlowAddress addr(nodeId, nPort, true);
	...
	virtual tukk                    GetGlobalNameForGraphToken(tukk tokenName) const = 0; //! Get the corresponding name for the GTS registry
...
struct IFlowSystemContainer
{
	...
	virtual TFlowInputData GetItem(i32 i) = 0;
	virtual void           Clear() = 0;

	virtual void           RemoveItemAt(i32 i) = 0;
...
struct IFlowSystemEngineModule : public Drx::IDefaultModule
{
	DRXINTERFACE_DECLARE_GUID(IFlowSystemEngineModule, "96b19348-6ad3-427f-9a8f-7764052a5536"_drx_guid);
};
...
struct IFlowSystem
{
	...
	virtual TFlowNodeTypeId RegisterType(tukk typeName, IFlowNodeFactoryPtr factory) = 0;

	//! Unregister previously registered type, keeps internal type ids valid
	virtual bool UnregisterType(tukk typeName) = 0;
	...
	virtual tukk GetTypeName(TFlowNodeTypeId typeId) = 0;

	//! Gets a flow node type ID from a type name.
	virtual TFlowNodeTypeId GetTypeId(tukk typeName) = 0;
	...
	virtual IFlowGraphModuleUpr* GetIModuleUpr() = 0;
...
ILINE bool IsPortActive(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	return pActInfo->pInputPorts[nPort].IsUserFlagSet();
}

ILINE EFlowDataTypes GetPortType(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	return (EFlowDataTypes)pActInfo->pInputPorts[nPort].GetType();
}

ILINE const TFlowInputData& GetPortAny(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	return pActInfo->pInputPorts[nPort];
}

ILINE bool GetPortBool(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	bool result;
	return pActInfo->pInputPorts[nPort].GetValueWithConversion(result) ? result : false;
}

ILINE i32 GetPortInt(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	i32 result;
	return pActInfo->pInputPorts[nPort].GetValueWithConversion(result) ? result : 0;
}

ILINE EntityId GetPortEntityId(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	EntityId result;
	return pActInfo->pInputPorts[nPort].GetValueWithConversion(result) ? result : INVALID_ENTITYID;
}

ILINE float GetPortFloat(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	float result;
	return pActInfo->pInputPorts[nPort].GetValueWithConversion(result) ? result : 0.0f;
}

ILINE Vec3 GetPortVec3(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	Vec3 result;
	return pActInfo->pInputPorts[nPort].GetValueWithConversion(result) ? result : Vec3Constants<float>::fVec3_Zero;
}

ILINE string GetPortString(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	string result;
...
ILINE bool IsBoolPortActive(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
...
template<class T>
ILINE void ActivateOutput(IFlowNode::SActivationInfo* pActInfo, i32 nPort, const T& value)
{
...
ILINE bool IsOutputConnected(IFlowNode::SActivationInfo* pActInfo, i32 nPort)
{
	SFlowAddress addr(pActInfo->myID, nPort, true);
