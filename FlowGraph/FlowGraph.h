...
public:
	...
	virtual void                       SetGraphEntity(EntityId id, i32 nIndex = 0);
	virtual EntityId                   GetGraphEntity(i32 nIndex) const;
	virtual SFlowAddress               ResolveAddress(tukk addr, bool isOutput);
	virtual TFlowNodeId                ResolveNode(tukk name);
	...
	virtual TFlowNodeId                CreateNode(TFlowNodeTypeId typeId, tukk name, uk pUserData = 0);
	virtual TFlowNodeId                CreateNode(tukk typeName, tukk name, uk pUserData = 0);
	virtual bool                       SetNodeName(TFlowNodeId id, tukk sName);
	virtual IFlowNodeData*             GetNodeData(TFlowNodeId id);
	virtual tukk                GetNodeName(TFlowNodeId id);
	virtual TFlowNodeTypeId            GetNodeTypeId(TFlowNodeId id);
	virtual tukk                GetNodeTypeName(TFlowNodeId id);
	virtual void                       RemoveNode(tukk name);
	...
	virtual void                       ActivatePortCString(SFlowAddress output, tukk cstr);
	...
	virtual bool                       GetActivationInfo(tukk nodeName, IFlowNode::SActivationInfo& actInfo);
	...
	virtual tukk                    GetGlobalNameForGraphToken(tukk tokenName) const;
	...
	// get some more stats
	void GetGraphStats(i32& nodeCount, i32& edgeCount);
	...
	bool NotifyFlowNodeActivationListeners(TFlowNodeId srcNode, TFlowPortId srcPort, TFlowNodeId toNode, TFlowPortId toPort, tukk value);
...
private:
	...
	void       FlowLoadError(tukk format, ...) PRINTF_PARAMS(2, 3);
	...
	std::pair<CFlowData*, TFlowNodeId> CreateNodeInt(TFlowNodeTypeId typeId, tukk name, uk pUserData = 0);
	...
	SFlowAddress ResolveAddress(tukk node, tukk port, bool isOutput);
	...
	struct SEdge
	{
		...
		{
			DRX_ASSERT(from.isOutput);
			DRX_ASSERT(!to.isOutput);
		}
...
{
	return DrxStringUtils::toString(value);
}
...
inline string CFlowGraphBase::ToString(const TFlowInputData& value)
{
	...
	case eFDT_Int:
		return ToString(*value.GetPtr<i32>());
	default:
		return "unknown";
	}
}
...
{
	// DRX_PROFILE_FUNCTION_ARG(PROFILE_ACTION, m_debugName); // enable only when needed, it increases FG update total time
	...
	if (addr.isOutput)
	{
		...
		i32k firstEdgeIndex = m_flowData[addr.node].GetOutputFirstEdge(addr.port);
...
public:
	...
	virtual void DoActivatePort(const SFlowAddress address, const NFlowSystemUtils::Wrapper<i32>& value)
	{
		CFlowGraphBase::PerformActivation(address, value.value);
	}
