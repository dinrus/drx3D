...
// -------------------------------------------------------------------------
//  Имя файла:   FlowXmlDataNodes.cpp
//  Описание: Flowgraph nodes to dealing with data in Xml elements
// -------------------------------------------------------------------------
//  История:
...
public:
	...
	virtual bool Execute(SActivationInfo* pActInfo)
	{
		...
		if (GDM->GetXmlDocument(pActInfo->pGraph, &doc) && doc->active)
		{
			...
			float value = 0.0f;
			tukk content = doc->active->getContent();
			if (sscanf(content, "%f", &value))
			{
				// Increment and set back
				char newContent[32];
				drx_sprintf(newContent, "%f", value + amount);
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		{
			ADD_BASE_OUTPUTS(),
			OutputPortConfig<i32>("Count",_HELP("Number of attributes")),
			{ 0 }
		};
