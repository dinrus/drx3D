		...
		case eFE_Activate:
			{
				...
				if (root == NULL)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "FlowGraph: TimeOfDay Loading Node: Could not load tod file %s. Aborting.", pathAndfileName.c_str());
				...
				if (pTimeOfDay == NULL)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "FlowGraph: TimeOfDay Loading Node: Could not obtain ITimeOfDay interface from engine. Aborting.");
					ActivateOutput(pActInfo, EOP_Fail, true);
					return;
				}
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("year",  "Current Year"),
			OutputPortConfig<i32>("month", "Current Month [1..12]"),
			OutputPortConfig<i32>("day",   "Current Day of the Month [1..31]"),
			OutputPortConfig<i32>("hours"),
			OutputPortConfig<i32>("minutes"),
			OutputPortConfig<i32>("seconds"),
			{ 0 }
		};
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("min",      _HELP("Minimal timer output value")),
			InputPortConfig<i32>("max",      _HELP("Maximal timer output value")),
		...
		case eFE_Update:
			{
				...
				{
					m_last = time;
					i32 nMin = GetPortInt(pActInfo, IN_MIN);
					i32 nMax = GetPortInt(pActInfo, IN_MAX);
					m_nCurrentCount++;
...
public:
	...
	CFlowNode_TimedCounter(SActivationInfo* pActInfo) : m_count(0)
	{
	}
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("limit",       _HELP("How many ticks the counter will do before finish")),
			{ 0 }
		};
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<i32>("count",       _HELP("outputs the tick counter value")),
			{ 0 }
		};
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("secs"),
			OutputPortConfig<i32>("msecs"),
		...
		case eFE_Update:
			{
				...
				if (now != m_lasttime)
				{
					u32 msec = (u32)((now - m_basetime).GetMilliSeconds());
					u32 period = (u32)(m_period.GetMilliSeconds());
