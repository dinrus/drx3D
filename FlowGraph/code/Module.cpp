...
bool CFlowGraphModule::PreLoadModule(tukk fileName)
{
	...
	if (!moduleRef)
	{
		DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "Unable to preload Flowgraph Module: %s", PathUtil::GetFileName(fileName).c_str());
		return false;
	}
	...
	if (modulePorts)
	{
		i32 nPorts = modulePorts->getChildCount();
...
bool CFlowGraphModule::LoadModuleGraph(tukk moduleName, tukk fileName)
{
	...
	if (!moduleRef)
	{
		DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "Unable to load Flowgraph Module Graph: %s", moduleName);
		return false;
	}
...
{
	IFlowGraphModuleUpr* pMgr = gEnv->pFlowSystem ? gEnv->pFlowSystem->GetIModuleUpr() : nullptr;
	if (pMgr)
	{
...
void CFlowGraphModule::CreateInstance(EntityId entityId, TModuleInstanceId runningInstanceId)
{
	...
	if (m_pRootGraph)
	{
		...
		pModuleUpr->BroadcastModuleInstanceStarted(this, runningInstanceId);
	}
}
...
void CFlowGraphModule::OnInstanceFinished(EntityId entityId, TModuleInstanceId instanceId, bool bSuccess, const TModuleParams& params)
{
	...
	CFlowGraphModuleUpr* pModuleUpr = static_cast<CFlowSystem*>(gEnv->pFlowSystem)->GetModuleUpr();
	pModuleUpr->BroadcastModuleInstanceFinished(this, instanceId);
}
