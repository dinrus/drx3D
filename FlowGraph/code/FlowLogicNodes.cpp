...
public:
	...
	virtual void Serialize(SActivationInfo*, TSerialize ser)
	{
		...
		for (i32 i = 0; i < NUM_INPUTS; ++i)
		{
			drx_sprintf(name, "m_triggered_%d", i);
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] =
		{
			...
			InputPortConfig<i32>("Limit", 1,                                            _HELP("How many times 'in' triggered values are sent to the output. After this number is reached, the output will not be triggered again (unless a reset is called)"),0, _UICONFIG("v_min=1,v_max=1000000000")),
			{ 0 }
		};
		static const SOutputPortConfig out_config[] = {
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Initialize:
			...
			break;
		case eFE_Activate:
			i32 limit = GetPortInt(pActInfo, INP_Limit);
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig_AnyType("In",  _HELP("Input")),
			InputPortConfig<i32>("outMin", _HELP("Min number of outputs to activate.")),
			InputPortConfig<i32>("outMax", _HELP("Max number of outputs to activate.")),
			{ 0 }
		};
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			...
			{
				i32 minOut = GetPortInt(pActInfo, 1);
				i32 maxOut = GetPortInt(pActInfo, 2);
				...
				i32 n = drx_random(minOut, maxOut);

				// Collect the outputs to use
				static i32 out[NUM_OUTPUTS];
				...
				// Shuffle
				for (i32 i = 0; i < n; i++)
					std::swap(out[i], out[drx_random(0, nout - 1)]);
...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_ConnectOutputPort:
			if (pActInfo->connectPort < NUM_OUTPUTS)
			{
				...
				for (i32 i = 0; i < m_nOutputCount; ++i)
				{
		...
		case eFE_Activate:
			...
			{
				i32 numCandidates = m_nOutputCount - m_nTriggered;
				if (numCandidates <= 0)
					return;
				i32k cand = drx_random(0, numCandidates - 1);
				i32k whichPort = m_nPorts[cand];
				m_nPorts[cand] = m_nPorts[numCandidates - 1];
				m_nPorts[numCandidates - 1] = -1;
				++m_nTriggered;
				assert(whichPort >= 0 && whichPort < NUM_OUTPUTS);
				// DrxLogAlways("CFlowNode_RandomTrigger: Activating %d", whichPort);
				...
				{
					// DrxLogAlways("CFlowNode_RandomTrigger: Done.");
					// Done
	...
	virtual void Serialize(SActivationInfo*, TSerialize ser)
	{
		...
		for (i32 i = 0; i < NUM_OUTPUTS; ++i)
		{
			drx_sprintf(name, "m_nPorts_%d", i);
		...
		if (ser.IsReading())
		{
			...
			for (i32 i = 0; i < NUM_OUTPUTS && !bNeedReset; ++i)
			{
				bool bFound = false;
				for (i32 j = 0; j < NUM_OUTPUTS && !bFound; ++j)
				{
	...
	i32 m_nConnectedPorts[NUM_OUTPUTS];   // array with port-numbers which are connected.
	i32 m_nPorts[NUM_OUTPUTS];            // permutation of m_nConnectedPorts array with m_nOutputCount valid entries
	i32 m_nConnectionCounts[NUM_OUTPUTS]; // number of connections on each port
	i32 m_nTriggered;
	i32 m_nOutputCount;
};
...
public:
		...
		case eFE_Activate:
			{
				...
				if (m_needToCheckConnectedPorts)
				{
					...
					m_numConnectedPorts = 0;
					for (i32 port = 0; port < NUM_OUTPUTS; ++port)
					{
	...
	bool         m_closed;
	u32 m_lastTriggeredPort;
	u32 m_numConnectedPorts;
	i32          m_connectedPorts[NUM_OUTPUTS];
};
