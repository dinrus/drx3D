...
#include <drx3D/FlowGraph/FlowGraph.h>
...
// Debug mapping from NodeIDs (==tuk Name) to internal IDs (index in std::vector)
	...
	, m_graphTokens()
{
	...
	for (i32 i = 0; i < MAX_GRAPH_ENTITIES; i++)
		m_graphEntityId[i] = 0;
...
bool CFlowGraphBase::NotifyFlowNodeActivationListeners(TFlowNodeId srcNode, TFlowPortId srcPort, TFlowNodeId toNode, TFlowPortId toPort, tukk value)
{
	...
	else
	{
		...
		{
			IFlowGraphModule* pModule = gEnv->pFlowSystem->GetIModuleUpr()->GetModule(this);
...
tukk CFlowGraphBase::GetDebugName() const
{
#if !defined(_RELEASE)
...
void CFlowGraphBase::SetDebugName(tukk sName)
{
#if !defined(_RELEASE)
	m_debugName = sName;
#endif
...
{
	tukk typeName = pGraph->GetNodeTypeName(id);
	tukk nodeName = pGraph->GetNodeName(id);
	string human;
	human += typeName;
	human += " Node:";
...
//////////////////////////////////////////////////////////////////////////
void CFlowGraphBase::SetGraphEntity(EntityId id, i32 nIndex)
{
...
//////////////////////////////////////////////////////////////////////////
EntityId CFlowGraphBase::GetGraphEntity(i32 nIndex) const
{
	if (nIndex >= 0 && nIndex < MAX_GRAPH_ENTITIES)
...
//////////////////////////////////////////////////////////////////////////
SFlowAddress CFlowGraphBase::ResolveAddress(tukk addr, bool isOutput)
{
	SFlowAddress flowAddr;
	tukk colonPos = strchr(addr, ':');
	if (colonPos)
	{
		PREFAST_SUPPRESS_WARNING(6255)
		tuk prefix = (tuk)alloca(colonPos - addr + 1);
...
SFlowAddress CFlowGraphBase::ResolveAddress(tukk node, tukk port, bool isOutput)
{
	SFlowAddress flowAddr;
...
TFlowNodeId CFlowGraphBase::ResolveNode(tukk name)
{
...
{
	DRX_ASSERT(ValidateNode(nodeId));
...
bool CFlowGraphBase::ValidateNode(TFlowNodeId id) const
{
...
bool CFlowGraphBase::ValidateLink(SFlowAddress& from, SFlowAddress& to)
{
	...
	{
		tukk type = from.isOutput ? "output" : "input";
...
//////////////////////////////////////////////////////////////////////////
TFlowNodeId CFlowGraphBase::CreateNode(TFlowNodeTypeId typeId, tukk name, uk pUserData)
{
...
//////////////////////////////////////////////////////////////////////////
TFlowNodeId CFlowGraphBase::CreateNode(tukk typeName, tukk name, uk pUserData)
{
...
//////////////////////////////////////////////////////////////////////////
std::pair<CFlowData*, TFlowNodeId> CFlowGraphBase::CreateNodeInt(TFlowNodeTypeId typeId, tukk name, uk pUserData)
{
	...
	if (!pNode)
	{
		tukk typeName = m_pSys->GetTypeName(typeId);
...
void CFlowGraphBase::RemoveNode(tukk name)
{
...
//////////////////////////////////////////////////////////////////////////
bool CFlowGraphBase::SetNodeName(TFlowNodeId id, tukk sName)
{
	if (id < m_flowData.size())
	{
...
//////////////////////////////////////////////////////////////////////////
tukk CFlowGraphBase::GetNodeName(TFlowNodeId id)
{
	if (id < m_flowData.size())
	{
...
//////////////////////////////////////////////////////////////////////////
tukk CFlowGraphBase::GetNodeTypeName(TFlowNodeId id)
{
	if (id < m_flowData.size())
	{
...
{
	DRX_PROFILE_FUNCTION_ARG(PROFILE_ACTION, m_debugName);
...
void CFlowGraphBase::DoUpdate(IFlowNode::EFlowEvent event)
{
	...
	DRX_ASSERT(m_firstFinalActivatingNode == END_OF_MODIFIED_LIST);
	...
	while (m_firstModifiedNode != END_OF_MODIFIED_LIST && numLoops++ < MAX_LOOPS)
	{
		...
		DRX_ASSERT(m_firstActivatingNode == END_OF_MODIFIED_LIST);
	...
	{
#if defined (FLOW_DEBUG_PENDING_UPDATES)
		DrxLogAlways("[flow] CFlowGraphBase::DoUpdate: %s -> Reached maxLoops %d during event %d", m_debugName.c_str(), MAX_LOOPS, event);
#endif
...
tukk CFlowGraphBase::GetGlobalNameForGraphToken(tukk tokenName) const
{
...
void CFlowGraphBase::UnregisterGraphTokens()
{
	...
	{
		tukk globalName = GetGlobalNameForGraphToken(it->name.c_str());
...
void CFlowGraphBase::ResetGraphToken(const IFlowGraph::SGraphToken& token)
{
	...
	tukk globalName = GetGlobalNameForGraphToken(token.name);

	TFlowInputData temp = TFlowInputData::CreateDefaultInitializedForTag((i32)token.type, true);
...
void CFlowGraphBase::SortEdges()
{
	...
	for (std::vector<CFlowData>::iterator iter = m_flowData.begin(); iter != m_flowData.end(); ++iter)
	{
		if (!iter->IsValid())
			continue;
		for (i32 i = 0; i < iter->GetNumOutputs(); i++)
		{
...
void CFlowGraphBase::RemoveNodeFromActivationArray(TFlowNodeId id, TFlowNodeId& front, std::vector<TFlowNodeId>& array)
{
	...
	else
	{
		...
		while (current != id)
		{
			DRX_ASSERT(current != END_OF_MODIFIED_LIST);
			DRX_ASSERT(current != NOT_MODIFIED);
			previous = current;
			current = array[current];
		}
		DRX_ASSERT(previous != NOT_MODIFIED);
		DRX_ASSERT(current == id);
		array[previous] = array[current];
		array[current] = NOT_MODIFIED;
	}

	DRX_ASSERT(array[id] == NOT_MODIFIED);
}
...
void CFlowGraphBase::ActivatePortCString(SFlowAddress addr, tukk cstr)
{
...
bool CFlowGraphBase::SetInputValue(TFlowNodeId node, TFlowPortId port, const TFlowInputData& value)
{
	...
	if(bOk)
	{
		i32 entityOffset = (data.HasEntity() ? 1 : 0);
...
bool CFlowGraphBase::GetActivationInfo(tukk nodeName, IFlowNode::SActivationInfo& actInfo)
{
...
void CFlowGraphBase::FlowLoadError(tukk format, ...)
{
	if (!format)
		return;
	...
	{
		if (m_pAIAction != 0)
			DrxFatalError("[flow] %s : %s", m_pAIAction->GetName(), buffer);
		else
			DrxFatalError("[flow] %s : %s", pEnt ? pEnt->GetName() : "<noname>", buffer);
	}
	else
	{
		if (m_pAIAction != 0)
			DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_ERROR, "[flow] %s : %s", m_pAIAction->GetName(), buffer);
		else
			DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_ERROR, "[flow] %s : %s", pEnt ? pEnt->GetName() : "<noname>", buffer);
	}
}
...
bool CFlowGraphBase::ReadXML(const XmlNodeRef& root)
{
	...
	while (true)
	{
...
#ifdef FLOW_DEBUG_ID_MAPPING
		{
			IEntity* pEnt = gEnv->pEntitySystem->GetEntity(GetGraphEntity(0));
			if (m_pAIAction != 0)
				DrxLogAlways("[flow] %s 0x%p: Loading FG", m_pAIAction->GetName(), this);
			else
				DrxLogAlways("[flow] %s 0x%p: Loading FG", pEnt ? pEnt->GetName() : "<noname>", this);
		}
#endif

		i32 nNodes = nodes->getChildCount();

		i32 nConcreteNodes = 0;

		for (i = 0; i < nNodes; i++)
		{
			XmlNodeRef node = nodes->getChild(i);
			tukk type = node->getAttr(NODE_TYPE_ATTR);
			tukk name = node->getAttr(NODE_NAME_ATTR);
		...
		for (i = 0; i < nNodes; i++)
		{
...
#ifdef FLOW_DEBUG_ID_MAPPING
			{
				IEntity* pEnt = gEnv->pEntitySystem->GetEntity(GetGraphEntity(0));
				if (m_pAIAction != 0)
					DrxLogAlways("[flow] %s : Mapping ID=%s to internal ID=%d", m_pAIAction->GetName(), name, actInfo.myID);
				else
					DrxLogAlways("[flow] %s : Mapping ID=%s to internal ID=%d", pEnt ? pEnt->GetName() : "<noname>", name, actInfo.myID);
			}
#endif
		...
		i32 nEdges = edges->getChildCount();

		i32 nConcreteEdges = 0;
		...
		for (i = 0; i < nEdges; i++)
		{
			...
			else
			{
				...
				if (m_pAIAction != 0)
				{
					DrxLogAlways("[flow] Disabled edge %d for AI action graph '%s' <%s,%s> to <%s,%s>", i, m_pAIAction->GetName(),
					             edge->getAttr("nodeOut"), edge->getAttr("portOut"),
					             edge->getAttr("nodeIn"), edge->getAttr("portIn"));
				}
				else
		...
		if (graphTokens)
		{
			i32 nTokens = graphTokens->getChildCount();
			RemoveGraphTokens();
			m_graphTokens.reserve(nTokens);
			for (i32 j = 0; j < nTokens; ++j)
			{
				XmlString tokenName;
				i32 tokenType;
...
bool CFlowGraphBase::WriteXML(const XmlNodeRef& root)
{
	...
	for (i32 i = 0; i < m_graphTokens.size(); ++i)
	{
...
bool CFlowGraphBase::IsOutputConnected(SFlowAddress addr)
{
	...
	DRX_ASSERT(ValidateAddress(addr));
...
void CFlowGraphBase::Serialize(TSerialize ser)
{
	...
	else
	{
		...
		while (nodeCount--)
		{
			...
			/*
			   ...
			   DrxLogAlways("Map Contents:");
			   while (iter != m_nodeNameToId.end())
			   {
			    DrxLogAlways("Map: %s", iter->first.c_str());
			   ++iter;
			   }
			   }
			 */
	...
	// regular updates
	std::vector<DrxFixedStringT<32>> regularUpdates;
	...
	if (ser.IsReading())
	{
		...
		for (std::vector<DrxFixedStringT<32>>::const_iterator iter = regularUpdates.begin(); iter != regularUpdates.end(); ++iter)
		{
...
{
	DRX_ASSERT(ValidateNode(flowNodeId));
...
//////////////////////////////////////////////////////////////////////////
void CFlowGraphBase::GetGraphStats(i32& nodeCount, i32& edgeCount)
{
...
//////////////////////////////////////////////////////////////////////////
void CFlowGraphBase::GetMemoryUsage(IDrxSizer* s) const
{
	{
...
void CFlowGraph::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
