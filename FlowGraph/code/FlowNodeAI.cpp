...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   FlowNode_AI.cpp
   $Id$
   Описание: place for miscellaneous AI related flow graph nodes
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("Total"),
			OutputPortConfig<i32>("Enemy"),
			{ 0 }
		};
...
private:
	...
	i32        m_localAICount;
	i32        m_localAIEnemyCount;
	static i32 m_currentAICount;
	static i32 m_currentAIEnemyCount;
	static i32 m_lastUpdateframeID;
};

i32 CFlowNode_AIActiveCount::m_currentAICount = -1;
i32 CFlowNode_AIActiveCount::m_currentAIEnemyCount = -1;
i32 CFlowNode_AIActiveCount::m_lastUpdateframeID = -1;
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config) override
	{
		...
		{
			OutputPortConfig<i32>("Count",        _HELP("The number of active AIs in the specified faction")),
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo) override
	{
		...
		case eFE_Update:
			{
				i32 count = CountCurrentlyActiveAIsInSpecifiedFaction();
...
private:
	...
	i32   m_currentCount;        // no. of currently active AIs in the specified faction

	i32 CountCurrentlyActiveAIsInSpecifiedFaction()
	{
		i32 count = 0;
		IActorIteratorPtr pIter = gEnv->pGameFramework->GetIActorSystem()->CreateActorIterator();
		while (IActor* pActor = pIter->Next())
...
private:
	...
	struct SActiveAIStatistics
	{
		...
		ILINE i32 GetActiveAICount() const { return m_currentActiveAIs; }

	private:
		i32 m_lastUpdateFrameId;
		i32 m_currentActiveAIs;
	};
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] =
		{
			...
			InputPortConfig<i32>("MaxActiveAIs", 0,                                                                 _HELP("Maximum number of active AI which is being used as condition for the node outputs")),
		...
		static const SOutputPortConfig out_config[] =
		{
			...
			OutputPortConfig<i32>("CurrentActiveAIs", _HELP("Outputs number of active AIs")),
			{ 0 }
		};
		...
		case eFE_Update:
			{
				...
				else
				{
					...
					i32k activeAICount = s_activeAIStatistics.GetActiveAICount();
...
public:
	...
	{
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("alertness",      "0 - green\n1 - orange\n2 - red"),
		...
		static const SInputPortConfig in_config[] = {
			InputPortConfig<i32>("groupID", _HELP("The group ID of the group to get the alertness from")),
			{ 0 }
		};
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		else if (event == eFE_Update)
		{
			...
			i32 maxAlertness = 0;
			IAISystem* pAISystem = gEnv->pAISystem;
			i32 groupID = GetPortInt(pActInfo, 0);    // group ID
			i32 n = pAISystem->GetGroupCount(groupID, IAISystem::GROUP_ENABLED);
			for (i32 i = 0; i < n; i++)
			{
			...
			if (!m_PlayerWasRecognized)
			{
				...
				if (playerIsRecognized)
				{
					...
					i32k targetVisibleOutputPort = 4;
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("fromID",           0,    _HELP("The group which is to be merged.")),
			InputPortConfig<i32>("toID",             0,    _HELP("The group to merge to.")),
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		{
			i32 fromID = GetPortInt(pActInfo, 1);
			i32 toID = GetPortInt(pActInfo, 2);
			bool enable = GetPortBool(pActInfo, 3);

			// Collect members
			std::vector<IAIObject*> members;
			AutoAIObjectIter it(gEnv->pAISystem->GetAIObjectUpr()->GetFirstAIObject(OBJFILTER_GROUP, fromID));
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("groupID", _HELP("AI Group ID of the entity")),
			{ 0 }
		};
		static const SInputPortConfig in_config[] = {
			InputPortConfig_AnyType("sink", NULL,                                         _HELP("Sync")),
			InputPortConfig<i32>("groupID", _HELP("New group ID to set to the entity.")),
			{ 0 }
		};
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		else if (event == eFE_Update)
		{
			...
			if (pEntity)
			{
				...
				if (pAIActor)
				{
					i32 id = pAIActor->GetParameters().m_nGroup;
					if (m_lastID != id)
					{
						m_lastID = id;
...
public:
	...
	{
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("count", _HELP("The number of agents in a group.")),
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (event == eFE_Initialize)
		{
			...
			IAISystem* pAISystem = gEnv->pAISystem;
			i32 groupID = GetPortInt(pActInfo, 0);      // group ID
			i32 n = pAISystem->GetGroupCount(groupID, true);
...
public:
	...
	virtual IFlowNodePtr Clone(SActivationInfo* pActInfo) { pActInfo->m_pUserData = (uk )(UINT_PTR)m_entityId; return new CFlowNode_AISOHelper(pActInfo); };
	...
	void Compute(IEntity* pEntity)
	{
		...
		SmartObjectHelper* pHelper = gEnv->pAISystem->GetSmartObjectUpr()->GetSmartObjectHelper(className, helperName);
		if (!pHelper)
			return;
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Update:
			...
			if (!actionName.empty())
			{
				...
				i32 id = gEnv->pAISystem->GetSmartObjectUpr()->SmartObjectEvent(actionName, pUser, pObject, NULL, highPriority);
				if (id)
				{
					if (pUser)
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] =
		{
			...
			InputPortConfig<i32>("Reaction",                                                        0,                                                            _HELP("Source faction's reaction to target faction."),
		...
		case eFE_Activate:
			{
				...
				u8 source = gEnv->pAISystem->GetFactionMap().GetFactionID(GetPortString(pActInfo, InFactionSource));
				u8 target = gEnv->pAISystem->GetFactionMap().GetFactionID(GetPortString(pActInfo, InFactionTarget));
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, InSet))
				{
					...
					if (pAi)
					{
						...
						u8 faction = gEnv->pAISystem->GetFactionMap().GetFactionID(szFactionName);
...
void CFlowNode_AISetNavCostFactor::GetConfiguration(SFlowNodeConfig& config)
{
	...
	static const SOutputPortConfig out_config[] = {
		OutputPortConfig<i32>("Done", _HELP("Done")),
		{ 0 }
	};
...
public:
	...
	virtual bool GetPortGlobalEnum(u32 portId, IEntity* pNodeEntity, tukk szName, string& outGlobalEnum) const
	{
		if (EIP_CommName == portId && pNodeEntity)
		{
			...
			if (pAIProxy)
			{
				tukk szCommConfigName = pAIProxy->GetCommunicationConfigName();
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (eFE_Update == event)
		{
			...
			if ((bool)m_playID)
			{
				...
				assert(pCommunicationUpr);

				const bool bIsPlaying = pCommunicationUpr->IsPlaying(m_playID);
				if (!bIsPlaying)
				{
					IEntity* pEntity = pActInfo->pEntity;
	...
	void StartCommunication(SActivationInfo* pActInfo)
	{
		...
		if (pAIProxy)
		{
			...
			request.channelID = pCommunicationUpr->GetChannelID(GetPortString(pActInfo, EIP_ChannelName));
			request.commID = pCommunicationUpr->GetCommunicationID(GetPortString(pActInfo, EIP_CommName));
			request.configID = pCommunicationUpr->GetConfigID(pAIProxy->GetCommunicationConfigName());
			...
			m_playID = pCommunicationUpr->PlayCommunication(request);
			if ((bool)m_playID)
			{
	...
	void StopCommunication(SActivationInfo* pActInfo)
	{
		...
		if ((bool)m_playID)
		{
			if (pCommunicationUpr->IsPlaying(m_playID))
			{
				pCommunicationUpr->StopCommunication(m_playID);
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig outputs[] = {
			OutputPortConfig<i32>("AliveCount",   _HELP("How many of the actors are alive")),
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, IN_Trigger))
				{
					...
					for (i32 i = numAlive; i < NUM_ACTORS; ++i)
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			...
			InputPortConfig<i32>("FilterAI",      0,                                         _HELP("Filter which AIs are used for the alertness value."),                                                _HELP("FilterAI"),    _UICONFIG("enum_int:All=0,Enemies=1,Friends=2,Faction=3")),
...
private:
	...
	EAIFilterType GetFilterType(u8 filterType)
	{
		switch (filterType)
		{
		case 0:
...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			...
			pCommunicationUpr->SetVariableValue(variableName.c_str(), variableValue);
			break;
		}
	}
...
public:
	...
	virtual void         GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("GroupId", _HELP("Id of the group that needs to be notified.")),
			{ 0 }
		};
		static const SOutputPortConfig out_config[] = {
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Initialize:
			break;
		case eFE_Activate:
			i32k groupId = GetPortInt(pActInfo, eIN_GroupId);
			tukk signalName = "OnRequestReinforcementTriggered";
...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo) override
	{
		...
		case eFE_FinalActivate:
			...
			gEnv->pAISystem->GetNavigationSystem()->GetUpdateUpr()->WorldChanged(aabb);
			break;
		}
	}
...
public:
	...
	IAIActor* GetIAIActor(SActivationInfo* pActInfo) const
	{
		...
		if (pActInfo->pEntity)
		{
			...
			if (!pAIActor)
			{
				DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "AI:BehaviorTree: entity %s is not an AI Actor.", pActInfo->pEntity->GetName());
			}

		}
		else
		{
			DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "AI:BehaviorTree: missing entity in FG node.");
		}
