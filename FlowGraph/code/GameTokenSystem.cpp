...
// -------------------------------------------------------------------------
//  Имя файла:   CGameTokenSystem.cpp
//  Version:     v1.00
...
i32 CGameTokenSystem::m_CVarShowDebugInfo = 0;
i32 CGameTokenSystem::m_CVarPosX = 0;
i32 CGameTokenSystem::m_CVarPosY = 0;
i32 CGameTokenSystem::m_CVarNumHistoricLines = DBG_HISTORYSIZE;
...
//////////////////////////////////////////////////////////////////////////
IGameToken* CGameTokenSystem::SetOrCreateToken(tukk sTokenName, const TFlowInputData& defaultValue)
{
	DRX_ASSERT(sTokenName);
	...
	DrxLogAlways("GT 0x%p: DEBUG looking for token '%s'", this, sTokenName);
	while (iter != m_pGameTokensMap->end())
	{
		DrxLogAlways("GT 0x%p: Token key='%s' name='%s' Val='%s'", this, (*iter).first, (*iter).second->GetName(), (*iter).second->GetValueAsString());
		++iter;
	}
#endif
...
void CGameTokenSystem::DeleteToken(IGameToken* pToken)
{
	...
	{
#ifdef DEBUG_GAME_TOKENS
		DrxLogAlways("GameTokenSystemNew::DeleteToken: About to delete Token 0x%p '%s' val=%s", pToken, pToken->GetName(), pToken->GetValueAsString());
#endif
...
//////////////////////////////////////////////////////////////////////////
IGameToken* CGameTokenSystem::FindToken(tukk sTokenName)
{
	if (m_pGameTokensMap)
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenSystem::RenameToken(IGameToken* pToken, tukk sNewName)
{
	DRX_ASSERT(pToken);
	...
	if (it != m_pGameTokensMap->end())
		m_pGameTokensMap->erase(it);
#ifdef DEBUG_GAME_TOKENS
	DrxLogAlways("GameTokenSystemNew::RenameToken: 0x%p '%s' -> '%s'", pCToken, pCToken->m_name, sNewName);
#endif
...
//////////////////////////////////////////////////////////////////////////
CGameToken* CGameTokenSystem::GetToken(tukk sTokenName)
{
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenSystem::RegisterListener(tukk sGameToken, IGameTokenEventListener* pListener, bool bForceCreate, bool bImmediateCallback)
{
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenSystem::UnregisterListener(tukk sGameToken, IGameTokenEventListener* pListener)
{
	CGameToken* pToken = GetToken(sGameToken);
	if (pToken)
	{
...
void CGameTokenSystem::Notify(EGameTokenEvent event, CGameToken* pToken)
{
	...
	if (CGameTokenSystem::m_CVarShowDebugInfo != 0 && pToken)
	{
		...
		if (filterPass)
		{
			i32 numHistoryLines = GetHistoryBufferSize();
			...
			if (CGameTokenSystem::m_CVarShowDebugInfo != 2)
				DrxLog("---GAMETOKEN CHANGE.  '%s' = '%s'", pToken->GetName(), pToken->GetValueAsString());
		}
	}
#endif
}
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenSystem::LoadLibs(tukk sFileSpec)
{
...
bool CGameTokenSystem::LoadLevelLibrary(tukk sFileSpec, bool bEraseLevelTokens)
{
...
void CGameTokenSystem::RemoveLibrary(tukk sPrefix)
{
	if (m_pGameTokensMap)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CGameTokenSystem::_InternalLoadLibrary(tukk filename, tukk tag)
{
	...
	string libName;
	{
		tukk sLibName = root->getAttr("Name");
		if (sLibName == 0)
		{
	...
	i32 numChildren = root->getChildCount();
	for (i32 i = 0; i < numChildren; i++)
	{
		XmlNodeRef node = root->getChild(i);

		tukk sName = node->getAttr("Name");
		tukk sType = node->getAttr("Type");
		tukk sValue = node->getAttr("Value");
		tukk sLocalOnly = node->getAttr("LocalOnly");
		i32 localOnly = atoi(sLocalOnly);
		...
		TFlowInputData initialValue = TFlowInputData::CreateDefaultInitializedForTag((i32)tokenType, true);
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenSystem::SerializeSaveLevelToLevel(tukk* ppGameTokensList, u32 numTokensToSave)
{
	...
	u32 numTokensSaved = 0;
	for (u32 i = 0; i < numTokensToSave; ++i)
	{
		tukk pName = ppGameTokensList[i];
		...
		if (pToken)
		{
			...
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "GameTokenSystem. GameToken %s was not found when trying to serialize (save) level to level", pName ? pName : "<NULL>");
	}
...
void CGameTokenSystem::SerializeReadLevelToLevel()
{
	...
	for (u32 i = 0; i < numTokens; i++)
	{
		...
		if (pToken)
			...
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "GameTokenSystem. GameToken '%s' was not found when trying to serialize (read) level to level", tokenName.c_str());
		ser.EndGroup();
	}
...
void CGameTokenSystem::DumpAllTokens()
{
	...
	while (iter != m_pGameTokensMap->end())
	{
		DrxLogAlways("#%04d [%s]=[%s]", i, (*iter).second->GetName(), (*iter).second->GetValueAsString());
		++iter;
		++i;
	}
}
...
void CGameTokenSystem::DebugDraw()
{
...

	{
		...
		if (numHistoryLines > 0)
		{
			...
			linesDrawn++;
			for (i32 i = 0; i < numHistoryLines; i++)
			{
...
//////////////////////////////////////////////////////////////////////////
tukk CGameTokenSystem::GetTokenDebugString(CGameToken* pToken)
{
	if (!pToken)
...
void CGameTokenSystem::ClearDebugHistory()
{
	...
	for (i32 i = 0; i < DBG_HISTORYSIZE; i++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenSystem::DrawToken(tukk pTokenName, tukk pTokenValue, const CTimeValue& timeChanged, i32 line)
{
	IRenderer* pRenderer = gEnv->pRenderer;
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenSystem::AddTokenToDebugList(tukk pToken)
{
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenSystem::RemoveTokenFromDebugList(tukk pToken)
{
	string tokenName(pToken);
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenSystem::GetMemoryStatistics(IDrxSizer* s)
{
