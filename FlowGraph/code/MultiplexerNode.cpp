...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Initialize:
			{
				i32k mode = GetPortInt(pActInfo, 1);
				...
				i32k port = (GetPortInt(pActInfo, 0) & 0x07) + 2;
...
public:
	...
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig<i32>("Index",    _HELP("Select Output port index from 0 to 7")),
			InputPortConfig<i32>("Mode",     eTM_Always,                                    _HELP("InputOnly: Only [Input] triggers output.\nIndexOnly: Only [Index] triggers output.\nBoth: Both [Input] and [Index] trigger output."),0, _UICONFIG("enum_int:Always=0,InputOnly=1,IndexOnly=2")),
		...
		case eFE_Initialize:
			{
				...
				if (mode == eTM_InputOnly && !bInputActive)
					return;
				i32k port = (GetPortInt(pActInfo, 0) & 0x07);
