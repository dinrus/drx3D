...
// -------------------------------------------------------------------------
//  Имя файла:   FlowInventoryNodes.cpp
//  Version:     v1.00
...
public:
	...
	void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (event == eFE_Activate && IsPortActive(pActInfo, 0))
		{
			...
			tukk pItemClass = itemClass.c_str();
		...
		else if (event == eFE_PrecacheResources)
		{
			...
			if (!itemClass.empty())
			{
				...
				DRX_ASSERT_MESSAGE(pGameRules != NULL, "No game rules active, can not precache resources");
				if (pGameRules)
				{
...
public:
	...
	void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (event == eFE_Activate && IsPortActive(pActInfo, EIP_Trigger))
		{
			...
			{
				if (gEnv->bServer)
					CDrxAction::GetDrxAction()->GetIItemSystem()->GetIEquipmentUpr()->GiveEquipmentPack(pActor, packName.c_str(), addPack, selectPrimary);
			}
		...
		else if (event == eFE_PrecacheResources)
		{
			...
			if (!packName.empty())
			{
				IGameRules* const pGameRules = CDrxAction::GetDrxAction()->GetIGameRulesSystem()->GetCurrentGameRules();
				if (pGameRules)
				{
...
public:
	...
	void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (event == eFE_Activate && IsPortActive(pActInfo, 0))
		{
			CItemSystem* pItemSystem = static_cast<CItemSystem*>(CDrxAction::GetDrxAction()->GetIItemSystem());
