...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Op",     EOP_ADD,                                                                                  _HELP("Operation"),_HELP("Operation"), _UICONFIG("enum_int:Add=0,Sub=1,Mul=2,Div=3")),
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, INP_DoCalc))
				{
					...
					i32 op = GetPortInt(pActInfo, INP_Operation);
					float out = 0;
					switch (op)
					{
					case EOP_ADD:
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("outRounded"),
			{ 0 }
		};
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("AmountOfDecimals"),
			{ 0 }
		};
		static const SOutputPortConfig out_config[] = {
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Initialize:
			...
			i32 decimals = GetPortInt(pActInfo, IN_AMOUNT_OF_DECIMALS);
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig_Void("DoCalc", _HELP("Do the calc and send the result to the output when receiving event on this port")),
			InputPortConfig<i32>("Op",     EOP_ADD,                                                                                  _HELP("Operation"),_HELP("Operation"), _UICONFIG("enum_int:Add=0,Sub=1,Mul=2,Cross=3,Dot=4")),
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("max"),
			{ 0 }
		};
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("count"),
			{ 0 }
		};
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			i32 nMax = GetPortInt(pActInfo, 2);
			i32 nCounter = m_nCounter;
...
public:
	...
	virtual void         GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("portThreshold",  0,                                                          _HELP("When portCount reaches portThreshold, portTrigger is activated")),
			InputPortConfig<i32>("totalThreshold", 0,                                                          _HELP("When totalCount reaches totalThreshold, totalTrigger is activated")),
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("portCount",     _HELP("Total number of ports that have been set since last reset. Only counts each port once. (0 to 16)")),
			OutputPortConfig<i32>("totalCount",    _HELP("Sum of all the times any of the ports have been set, since last reset.")),
	...
	void Serialize(SActivationInfo*, TSerialize ser)
	{
		...
		{
			memset((uk )m_abPorts, 0, sizeof(m_abPorts)); // clear them in case we can't read all values
			std::vector<bool> ab;
			ser.Value("m_abPorts", ab);
...
private:
	...
	void Reset()
	{
		...
		memset((uk )m_abPorts, 0, sizeof(m_abPorts));
	}
};
//////////////////////////////////////////////////////////////////////////
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<i32>("outRounded", _HELP("[out] but rounded to next integer value")),
			{ 0 }
		};
		config.pInputPorts = in_config;
		config.pOutputPorts = out_config;
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig_Void("set", _HELP("Send value to output when receiving event on this port")),
			InputPortConfig<i32>("in",  _HELP("Value to be set on output")),
			{ 0 }
		};
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("out"),
...
public:
	...
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig<i32>("Preset",    _HELP("Preset value")),
			InputPortConfig<i32>("HighLimit", _HELP("Upper limit")),
			InputPortConfig<i32>("LowLimit",  _HELP("Low limit")),
