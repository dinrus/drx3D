...
{
	typedef u32 InternalID;  // InternalID is used to control that only 1 of those nodes is active at any time
	...
	void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			if (IsPortActive(pActInfo, eIP_Set))
			{
				...
				IMaterial* pMat = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(mat.c_str(), false);
				if (pMat)
				{
...
void CFlowNode_SkyMaterialSwitch::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
}
...
void CFlowNode_SkyMaterialSwitch::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_Activate:
		if (IsPortActive(pActInfo, 1))
		{
			...
			IMaterial* pSkyMtl = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(mat, false, false, IMaterialUpr::ELoadingFlagsPreviewMode);
...
public:
		...
		case eFE_Activate:
			...
			if (IsPortActive(pActInfo, eIP_Texture) && (m_Job == nullptr))
			{
				...
				if (AsyncLoad)
				{
					...
					else if (gEnv->pDrxPak->IsFileExist(szNameTex.c_str()))
					{
	...
	virtual void StreamOnComplete(IReadStream* pStream, unsigned nError) override
	{
		...
		i32 texId = 0;
		tukk pNameTex = pStream->GetName();
		if (pNameTex && gEnv->pDrxPak->IsFileExist(pNameTex))
		{
