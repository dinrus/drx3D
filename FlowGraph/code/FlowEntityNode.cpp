...
#include <drx3D/FlowGraph/FlowEntityNode.h>
...
//////////////////////////////////////////////////////////////////////////
void CFlowEntityClass::FreeStr(tukk src)
{
	if (src)
		delete[]src;
}
...
//////////////////////////////////////////////////////////////////////////
tukk CFlowEntityClass::CopyStr(tukk src)
{
	tuk dst = 0;
	if (src)
	{
...
{
	tuk name = strchr((tuk)config.name, '_');
	if (name != 0)
...
{
	i32 nEvents = pEntityClass->GetEventCount();
	for (i32 i = 0; i < nEvents; i++)
	{
		...
		if (event.bOutput)
		{
			...
			case IEntityClass::EVT_INT:
				m_outputs.push_back(OutputPortConfig<i32>(CopyStr(event.name)));
				break;
		...
		else
		{
			...
			case IEntityClass::EVT_BOOL:
				...
				break;
			case IEntityClass::EVT_INT:
				m_inputs.push_back(InputPortConfig<i32>(CopyStr(event.name)));
				MakeHumanName(m_inputs.back());
				break;
			case IEntityClass::EVT_FLOAT:
	...
	case eFE_Activate:
		{
			...
			for (i32 i = 0; i < m_pClass->m_inputs.size() - 1; i++)
			{
...
void CFlowEntityNode::SendEventToEntity(SActivationInfo* pActInfo, IEntity* pEntity)
{
	...
	for (i32 iInput = 0, inputCount = m_pClass->m_inputs.size() - 1; iInput < inputCount; ++iInput)
	{
			...
			case eFDT_Int:
				{
					...
					i32k value = GetPortInt(pActInfo, iInput);
...
void CFlowEntityNode::OnEntityEvent(IEntity* pEntity, const SEntityEvent& event)
{
	...
	case ENTITY_EVENT_SCRIPT_EVENT:
		{
			tukk sEvent = (tukk)event.nParam[0];
			// Find output port.
			for (i32 i = 0; i < m_pClass->m_outputs.size(); i++)
			{
				...
				if (strcmp(sEvent, m_pClass->m_outputs[i].name) == 0)
				{
					...
					{
					case IEntityClass::EVT_INT:
						assert(event.nParam[2] && "Attempt to activate FlowGraph port of type i32 without value");
						m_pGraph->ActivatePort(addr, *(i32*)event.nParam[2]);
						break;
					case IEntityClass::EVT_FLOAT:
...
public:
	...
	virtual IFlowNodePtr Clone(SActivationInfo* pActInfo) { pActInfo->m_pUserData = (uk )(UINT_PTR)m_entityId; return new CFlowNode_EntityPos(pActInfo); };
	virtual void         GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("CoordSys", 1, _HELP("In which coordinate system the values are expressed"), _HELP("CoordSys"), _UICONFIG("enum_int:Parent=0,World=1")),
			{ 0 }
		};
		static const SOutputPortConfig out_config[] = {
		...
		case ENTITY_EVENT_XFORM:
			{
				...
				OutputValues(&actInfo, u32(event.nParam[0]));
			}
			break;
		}
	}
...
public:
	...
	virtual IFlowNodePtr Clone(SActivationInfo* pActInfo) { pActInfo->m_pUserData = (uk )(UINT_PTR)m_entityId; return new CFlowNode_EntityTagpoint(pActInfo); }
...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		for (u32 i = INP_ENT00; i <= INP_ENT15; ++i)
		{
...
public:
	...
	void UtilDraw2DLine(float x1, float y1, float x2, float y2, const ColorF& color, float thickness)
	{
		...
		ColorB col((u8)(color.r * 255.0f), (u8)(color.g * 255.0f), (u8)(color.b * 255.0f), (u8)(color.a * 255.0f));
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Output", 0,     _HELP("OutputType"), _HELP("Output Type"),_UICONFIG("enum_int:Angle(Degrees)=0,Percentage=1")),
	...
	float CalculateOutputValue(i32 outputType, float projectionAngleDegrees, float maxAngleDegrees)
	{
		...
		case eFE_Update:
			{
				...
				{
					i32k w = std::max(IRenderAuxGeom::GetAux()->GetCamera().GetViewSurfaceX(), 1);
					i32k h = std::max(IRenderAuxGeom::GetAux()->GetCamera().GetViewSurfaceZ(), 1);
				...
				i32k outputType = GetPortInt(pActInfo, INP_OUT_TYPE);                                                         // either degrees or percentage
				...
				for (u32 i = INP_FIRST_ENTITY_IDX; i <= INP_LAST_ENTITY_IDX; ++i)
				{
					bool bEntityInRange = false;
					u32k inEntityIndex = i - INP_FIRST_ENTITY_IDX;
					u32k outEntityIndex = OUT_FIRST_ENTITY_IDX + inEntityIndex * 2;
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config) override
	{
		static const SInputPortConfig inputs[] =
		{
			...
			InputPortConfig<i32>("CoordSys", 1, _HELP("In which coordinate system the values are expressed"),_HELP("CoordSys"), _UICONFIG("enum_int:Local=0,World=1")),
			{ 0 }
		};
...
public:
	...
	virtual IFlowNodePtr Clone(SActivationInfo* pActInfo) { pActInfo->m_pUserData = (uk )(UINT_PTR)m_entityId; return new CFlowNode_EntityFaceAt(pActInfo); }
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("FwdDir", eFA_XPlus, _HELP("Axis that will be made to point at the target"), _HELP( "FwdDir" ), "enum_int:X+=0,X-=1,Y+=2,Y-=3,Z+=4,Z-=5"),
...
public:
	...
	virtual IFlowNodePtr Clone(SActivationInfo* pActInfo) { pActInfo->m_pUserData = (uk )(UINT_PTR)m_entityId; return new CFlowNode_EntityId(pActInfo); };
	virtual void         GetConfiguration(SFlowNodeConfig& config)
	{
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("Id", _HELP("Entity ID")),
			{ 0 }
		};
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("parentId", _HELP("Entity ID of the parent entity")),
			{ 0 }
		};
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("ChildIndex", _HELP("Index to be used when getting a child by ID")),
			{0}
		};
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("ChildCount", _HELP("Number of children that this entity has")),
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo *pActInfo)
	{
		if (event == eFE_Activate)
		{
			...
			{
				i32 index = GetPortInt(pActInfo, IN_Index);
...
public:
	...
	virtual IFlowNodePtr Clone(SActivationInfo* pActInfo) { pActInfo->m_pUserData = (uk )(UINT_PTR)m_entityId; return new CFlowNode_EntitySetProperty(pActInfo); };
	virtual void         GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
	...
	SmartScriptTable ResolveScriptTable(IScriptTable* pTable, tukk sKey, bool bPerArchetype, string& outKey)
	{
	...
	void ChangeProperty(SActivationInfo* pActInfo, IEntity* pEntity, IScriptTable* pScriptTable, SmartScriptTable& smartScriptTable, const string& propertyName, tukk propertyValue)
	{
...
public:
	...
	virtual IFlowNodePtr Clone(SActivationInfo* pActInfo) { pActInfo->m_pUserData = (uk )(UINT_PTR)m_entityId; return new CFlowNode_EntityGetProperty(pActInfo); };
	virtual void         GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		if (event == eFE_Activate)
		{
			if (IsPortActive(pActInfo, IN_SMARTNAME) || IsPortActive(pActInfo, IN_ACTIVATE))
			{
				...
				tukk pKey = (selectedParameter.replace(":", ".")).c_str();
				tukk pPropertiesString = bPerArchetype ? "Properties" : "PropertiesInstance";
				...
				if (strcmp(selectedParameter.substr(selectedParameter.find_last_of(".") + 1, 3), "clr") == 0)
				{
					...
					for (i32 i = 0; i < 3; i++)
					{
						...
						drx_strcpy(newKey, pKey);

						if (i == 0) drx_strcat(newKey, ".x");
						if (i == 1) drx_strcat(newKey, ".y");
						if (i == 2) drx_strcat(newKey, ".z");
	...
	static bool ReadScriptValue(IScriptTable* pTable, tukk pPropertiesString, tukk pKey, ScriptAnyValue& value)
	{
...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		{
			tukk entityName = pActInfo->pEntity->GetName();
			...
			case CS_WORLD:
				{
					...
					if (vPos.IsZero())
					{
						DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "BeamEntity Teleported %s to vPos zero.", entityName);
					}
...
public:
	...
	virtual IFlowNodePtr Clone(SActivationInfo* pActInfo) { pActInfo->m_pUserData = (uk )(UINT_PTR)m_entityId; return new CFlowNodeRenderParams(pActInfo); };
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Param", 0, _HELP("Render parameter to set (of type float)") , NULL, _UICONFIG("enum_int:None=0,Opacity=1")),
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		{
			bool success = false;
			i32 paramType = GetPortInt(pActInfo, EIP_ParamFloatName);
...
public:
	...
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig<i32>("CharSlot", 0, _HELP("Character Slot within Entity")),
			...
			InputPortConfig<i32>("SubMtlId", 0, _HELP("Sub Material Id")),
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		i32k charSlot = GetPortInt(pActInfo, EIP_CharSlot);
		...
		IMaterialUpr* pMatMgr = gEnv->p3DEngine->GetMaterialUpr();
		if (bSetMaterial)
		{
...
public:
	...
	ScriptAnyValue FillArgumentFromAnyPort(SActivationInfo* pActInfo, i32 port)
	{
...
{
public:
	CAutoRegFlowNodeWithAllowOverride(tukk sClassName) : CAutoRegFlowNode<T>(sClassName) {}
...
DRX_EXPORT_STATIC_LINK_VARIABLE(g_AutoRegCFlowNode_EntityTagpoint);
