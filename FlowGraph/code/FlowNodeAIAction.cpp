...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   FlowNodeAIAction.cpp
   $Id$
   Описание: place for AI action related flow graph nodes
...
#include <drx3D/FlowGraph/FlowNodeAIAction.h>
...
{
	//		pActInfo->m_pUserData = (uk ) m_entityId;
...
void CFlowNode_AISignal::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig<i32>("iValue",     _HELP("i32 signal data")),
...
void CFlowNode_AIExecute::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig<i32>("maxAlertness",       2,                                                                            _HELP("maximum alertness which allows execution (0, 1 or 2)"),                                      _HELP("MaxAlertness"),_UICONFIG("enum_int:Idle=0,Alerted=1,Combat=2")),
...
void CFlowNode_AIExecute::OnCancelPortActivated(IPipeUser* pPipeUser, SActivationInfo* pActInfo)
{
	if (m_EntityId)
	{
		...
		{
			m_bCancel = true;
			gEnv->pAISystem->GetAIActionUpr()->AbortAIAction(pEntity, m_GoalPipeId);
		}
	}
}
...
{
	gEnv->pAISystem->GetSmartObjectUpr()->SetSmartObjectState(GetEntity(pActInfo), GetPortString(pActInfo, 1));
	ActivateOutput(pActInfo, 0, m_EntityId);
	ActivateOutput(pActInfo, 1, m_EntityId);
	m_bExecuted = false;
	m_bSynchronized = false;
...
void CFlowNode_AIStance::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		InputPortConfig_Void("sink",                                                                                                _HELP("for synchronization only"), _HELP("Sync")),
		InputPortConfig<i32>("stance",                                                                                              4,                                 _HELP("0-prone\n1-crouch\n2-combat\n3-combat alerted\n4-relaxed\n5-stealth"),
...
void CFlowNode_AIStance::DoProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	i32 stance = GetPortInt(pActInfo, 1);
	state.forceWeaponAlertness = false;
	switch (stance)
	{
	case 0:
