...
public:
	...
	void Serialize(SActivationInfo* pActInfo, TSerialize ser)
	{
		...
		if (ser.IsReading())
		{
			...
			{
				IItemSystem* pItemSys = CDrxAction::GetDrxAction()->GetIItemSystem();
				...
				// DrxLog("[flow] CFlowNode_WeaponListener::Serialize() successfully created on '%s'", pItem->GetEntity()->GetName());
			}
			else
			{
				Reset();
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] =
		{
			...
			InputPortConfig<i32>("Ammo",           _HELP("Number of times the listener can be triggered. 0 means infinite"),_HELP("ShootCount")),
			{ 0 }
		};
		static const SOutputPortConfig outputs[] =
		{
			OutputPortConfig_Void("OnShoot",      _HELP("OnShoot")),
			OutputPortConfig<i32>("AmmoLeft",     _HELP("Shoots left"),                        _HELP("ShootsLeft")),
		...
		case eFE_Activate:
			{
				...
				if (IsPortActive(pActInfo, IN_ENABLE))
				{
					...
					//DrxLog("WeaponListener successfully created on %s", pItem->GetEntity()->GetName());
				}
				break;
			}
		}
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_ports[] =
		{
			...
			InputPortConfig<i32>("AmountCount", _HELP("Select amount to add/set"), _HELP("")),
		...
		{
			OutputPortConfig<i32>("MagazineAmmo",  _HELP("")),
			OutputPortConfig<i32>("InventoryAmmo", _HELP("")),
			OutputPortConfig<i32>("TotalAmmo",     _HELP("")),
			{ 0 }
		};
			...
			if (pInventory)
			{
				...
				if (pAmmoClass)
				{
					if (IsPortActive(pActInfo, IN_SET))
					{
						...
						i32k ammoAmount = GetPortInt(pActInfo, IN_AMMOCOUNT);

						pInventory->SetAmmoCount(pAmmoClass, bAdd ? (ammoAmount + pInventory->GetAmmoCount(pAmmoClass)) : (ammoAmount));
					}

					i32 magazineAmmo = 0;
					i32 inventoryAmmo = pInventory->GetAmmoCount(pAmmoClass);
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] =
		{
			...
			InputPortConfig<i32>("NumberOfShots",  _HELP("number of shots to fire (0 = unlimited)")),
	...
	void CalcFiringPosition(SActivationInfo* pActInfo, IWeapon* pWeapon)
	{
		...
		if (acc < 1)
		{
			bool itHits = drx_random(0.0f, 1.0f) < acc;
			if (!itHits)
			{
				...
				float errorAngle = drx_random(MIN_ERROR_ANGLE, MAX_ERROR_ANGLE);

				// 2d angle, in the plane normal to the vector from weapon to target.
				float dirErrorAngle = drx_random(0.0f, 360.0f);
