...
// -------------------------------------------------------------------------
//  Имя файла:   FlowMouseInfo.cpp
//  Version:     v1.00
//  Created:     08/26/2013 - Sascha Hoba
//  Описание: Mouse Info Flownode
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig outputs[] =
		{
			...
			OutputPortConfig<i32>("ScreenX",      _HELP("Screen X coord")),
			OutputPortConfig<i32>("ScreenY",      _HELP("Screen Y coord")),
			OutputPortConfig<i32>("DeltaScreenX", _HELP("delta screen X coord")),
			OutputPortConfig<i32>("DeltaScreenY", _HELP("Delta screen Y coord")),
			{ 0 }
		};
	...
	virtual void OnHardwareMouseEvent(i32 iX, i32 iY, EHARDWAREMOUSEEVENT eHardwareMouseEvent, i32 wheelDelta = 0)
	{
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig outputs[] =
		{
			OutputPortConfig<i32>("MousePressed",  _HELP("The i32 represents the mouse button that was pressed")),
			OutputPortConfig<i32>("MouseReleased", _HELP("The i32 represents the mouse button that was released")),
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			InputPortConfig_Void("Enable",           _HELP("Enable mouse button info")),
			InputPortConfig_Void("Disable",          _HELP("Disable mouse button info")),
			InputPortConfig<i32>("RayType",          0,                                                         _HELP("What should cause a ray hit?"),"All", _UICONFIG("enum_int: All=0, Terrain=1, Rigid=2, Static=3, Water=4, Living=5")),
			InputPortConfig<i32>("EntitiesToIgnore", _HELP("Container with Entities to ignore during raycast")),
			{ 0 }
		};

		static const SOutputPortConfig outputs[] =
		{
			OutputPortConfig<Vec3>("HitPos",       _HELP("The i32 represents the mouse button that was pressed")),
			OutputPortConfig<Vec3>("HitNormal",    _HELP("The i32 represents the mouse button that was released")),
	...
	void StoreRayType()
	{
		i32 ray_type = GetPortInt(&m_actInfo, EIP_RayType);
		switch (ray_type)
		{
		case 0:
			m_rayTypeFilter = ent_all;
	...
	virtual void OnPostUpdate(float fDeltaTime)
	{
		...
		i32 invMouseY = camera.GetViewSurfaceZ() - m_mouseY;
		...
		i32 containerId = GetPortInt(&m_actInfo, EIP_EntitiesToIgnore);
		if (containerId != 0)
		{
...
{
private:
	i32             m_screenX, m_screenY, m_screenX2, m_screenY2;
	SActivationInfo m_actInfo;
	bool            m_get;
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			...
			InputPortConfig<i32>("ContainerId", _HELP("The container to store the entitieIds in, user is responsible for creating/deleting this")),
			InputPortConfig<i32>("ScreenX",     _HELP("Hide the mouse cursor")),
			InputPortConfig<i32>("ScreenY",     _HELP("Hide the mouse cursor")),
			InputPortConfig<i32>("ScreenX2",    _HELP("Hide the mouse cursor")),
			InputPortConfig<i32>("ScreenY2",    _HELP("Hide the mouse cursor")),
			{ 0 }
		};
