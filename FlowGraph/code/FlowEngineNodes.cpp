...
void CSkyBoxLoadingCallback::StreamAsyncOnComplete(IReadStream* pStream, unsigned nError)
{
	...
	static tukk pNameSuffixes[] = { "box_12.dds", "box_34.dds", "box_5.dds" };
	string szNameBump;
	for (u32 i = 0; i < 3; ++i)
	{
		szNameBump = PathUtil::Make(pSkyboxLoadingInfo->sSkyTextureName, pNameSuffixes[i]);
		// check if the texture exists
		if (gEnv->pDrxPak->IsFileExist(szNameBump))
...
void CFlowNode_SkyboxSwitch::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
}
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("ObjectType", 0, _HELP("Object type (render node type)"), NULL,
...
private:
	void SetMaterial(const string& mtlName)
	{
		...
		if (pMtl && m_nObjType > 0)
		{
			...
			u32 nCnt = gEnv->p3DEngine->GetObjectsByTypeInBox((EERType) m_nObjType, aabbPos);
			if (nCnt)
			{
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("x",      "Current top left x position of the viewport."),
			OutputPortConfig<i32>("y",      "Current top left y position of the viewport."),
			OutputPortConfig<i32>("width",  "Current width of the viewport."),
			OutputPortConfig<i32>("height", "Current height of the viewport."),
			{ 0 }
		};
...
public:
	...
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig<i32>("ShadowCachesCascade", 4, _HELP("Sets what cascade we start using cached-shadows from (r_ShadowsCache)")),
			...
			InputPortConfig<i32>("GsmLodsNum", 5, _HELP("Sets the number of Gsm Lods (minimum 1) (e_GsmLodsNum)")),
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eIn_Apply))
				{
					...
					{
						i32k nGsmCache = GetPortInt(pActInfo, eIn_ShadowCachesCascade);
					...
					{
						i32k lods = GetPortInt(pActInfo, eIn_GsmLodsNum);
