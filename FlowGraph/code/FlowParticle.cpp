...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		const bool triggerPort = IsPortActive(pActInfo, i32(EInputs::Activate));
		if (!triggerPort)
			return;

		const string inputPropertyName = GetPortString(pActInfo, i32(EInputs::Name));
...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		const bool valuePort = IsPortActive(pActInfo, i32(EInputs::Value));
		const bool triggerPort = IsPortActive(pActInfo, i32(EInputs::Activate));
		...
		{
		case IParticleAttributes::ET_Integer:
			attributes.SetAsInteger(attributeId, GetPortInt(pActInfo, i32(i32(EInputs::Value))));
			break;
		case IParticleAttributes::ET_Float:
			attributes.SetAsFloat(attributeId, GetPortFloat(pActInfo, i32(i32(EInputs::Value))));
			break;
		case IParticleAttributes::ET_Boolean:
			attributes.SetAsBoolean(attributeId, GetPortBool(pActInfo, i32(i32(EInputs::Value))));
			break;
		case IParticleAttributes::ET_Color:
			attributes.SetAsColor(attributeId, ColorF(GetPortVec3(pActInfo, i32(i32(EInputs::Value)))));
			break;
		}
	}
};
