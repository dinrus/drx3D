...
#include <drx3D/FlowGraph/FlowFrameworkBaseNode.h>

//////////////////////////////////////////////////////////////////////////
static i32k FLOWGRAPH_SHAKE_ID = 24;
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		if (pPresetsHelp[0] == 0)
		{
			drx_strcpy(pPresetsHelp, "Preset input values. When this is used, all parameter inputs are ignored.\n");
			for (i32 i = 0; i < NUM_PRESETS; i++)
			{
				char buf[300];
				drx_sprintf(buf, "-- %s : Angle: (%4.3f,%4.3f,%4.3f)  Shift: (%4.3f,%4.3f,%4.3f )  Freq: %3.1f  Random: %3.1f  Distance: %3.1f  RangeMin: %3.1f RangeMax: %3.1f sustainDuration: %3.1f FadeInDur: %3.1f  FadeOutDur: %3.1f \n",
		...
		{
			drx_strcpy(pPresetsEnumDef, "enum_int:NoPreset=0,");
			for (i32 i = 0; i < NUM_PRESETS; i++)
			{
				char buf[100];
				drx_sprintf(buf, "%s=%1d,", m_Presets[i].pName, i + 1);
				drx_strcat(pPresetsEnumDef, buf);
			}
		}
#endif
		...
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Restrict",          ER_None,                                   _HELP("Restriction"),                                                                                                                              0,      _UICONFIG("enum_int:None=0,NoVehicle=1,InVehicle=2")),
			InputPortConfig<i32>("View",              VT_FirstPerson,                            _HELP("Which view to use. FirstPerson or Current (might be Trackview)."),                                                                          0,      _UICONFIG("enum_int:FirstPerson=0,Current=1")),
...
#ifdef _RELEASE
			InputPortConfig<i32>("Preset",            0,                                         _HELP("Preset input values. When this is used, all parameter inputs are ignored.")),
#else
			InputPortConfig<i32>("Preset",            0,                                         pPresetsHelp,                                                                                                                                      0,      _UICONFIG(pPresetsEnumDef)),
#endif
			{ 0 }
		};
	...
	void Shake(SActivationInfo* pActInfo, bool bUpdateOnly)
	{
		...
		i32 currentFrameId = gEnv->pRenderer->GetFrameID();
	...
	{
		i32 presetIndex = GetPortInt(pActInfo, EIP_Preset);
