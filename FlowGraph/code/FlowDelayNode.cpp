...
void CFlowDelayNode::Serialize(SActivationInfo* pActInfo, TSerialize ser)
{
...
#if 0
		DrxLogAlways("CDelayNode write: current time(ms): %f", curTime.GetMilliSeconds());
		Activations::iterator iter = m_activations.begin();
		while (iter != m_activations.end())
		{
			DrxLogAlways("CDelayNode write: ms=%d  timevalue(ms): %f", (*iter).first, (*iter).second.m_timeout.GetMilliSeconds());
			++iter;
		}
#endif
	}
	else
	{
		...
		if (gEnv->IsEditor())
		{
			...
			Activations::iterator iter = m_activations.begin();
#if 0
			DrxLogAlways("CDelayNode read: current time(ms): %f", curTime.GetMilliSeconds());
			while (iter != m_activations.end())
			{
				DrxLogAlways("CDelayNode read: ms=%d  timevalue(ms): %f", (*iter).first, (*iter).second.m_timeout.GetMilliSeconds());
				++iter;
			}
#endif
		...
		else
		{
			...
			for (iter = activations.begin(); iter != activations.end(); ++iter)
			{
				...
				IGameFramework::TimerID timerId = gEnv->pGameFramework->AddTimer(relTime, false, functor(CFlowDelayNode::OnTimer),
				                                                                       this);
...
void CFlowDelayNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_FinalActivate:
		{
			...
			if (gEnv->IsEditor())
			{
				...
				m_activations[(i32)finishTime.GetMilliSeconds()] = SDelayData(finishTime, GetPortAny(pActInfo, 0));
	...
	case eFE_Update:
		...
		DRX_ASSERT(!m_activations.empty());
		CTimeValue curTime = gEnv->pTimer->GetFrameStartTime();
...
void CFlowDelayNode::OnTimer(uk pUserData, IGameFramework::TimerID ref)
{
	DRX_ASSERT(gEnv->IsEditor() == false);
...
protected:
	...
	{
		return drx_random(GetPortFloat(pActInfo, 1), GetPortFloat(pActInfo, 2));
	}
...
public:
	...
	void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			...
			break;
		case eFE_Update:
			if (m_frameStamp != gEnv->nMainFrameID - 1)
			{
				DrxLogAlways("Frame delay node didn't skip 1 frame - activated frame %d, updated frame %d", m_frameStamp, gEnv->nMainFrameID);
			}
