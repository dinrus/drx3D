...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SInputPortConfig inputs[] =
		{
			ADD_BASE_INPUTS(),
			InputPortConfig<string>("Name","",  _HELP("Name of child node"),                  0, 0),
			InputPortConfig<i32>("Index",  1,   _HELP("Location of child in list (1-based)"), 0, 0),
			{ 0 }
		};
	...
	virtual bool Execute(SActivationInfo* pActInfo)
	{
		...
		if (GDM->GetXmlDocument(pActInfo->pGraph, &doc) && doc->active)
		{
			...
			i32k childIndex = GetPortInt(pActInfo, EIP_Index);
			i32k childCount = doc->active->getChildCount();
			XmlNodeRef ref = NULL;
			for (i32 i = 0, realCount = 0; i < childCount; ++i)
			{
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		{
			ADD_BASE_OUTPUTS(),
			OutputPortConfig<i32>("Count",_HELP("Number of children")),
			{ 0 }
		};
	...
	virtual bool Execute(SActivationInfo* pActInfo)
	{
		...
		if (GDM->GetXmlDocument(pActInfo->pGraph, &doc) && doc->active)
		{
			i32k count = doc->active->getChildCount();
