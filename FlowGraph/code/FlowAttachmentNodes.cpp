...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("CharacterSlot",      0,                                                               _HELP("Host character slot")),
	...
	IAttachment* GetAttachment(IEntity* pEntity)
	{
		...
		if (pEntity && !m_boneName.empty() && m_characterSlot > eCharacterSlot_Invalid)
		{
			...
			{
				if (IAttachmentUpr* pAttachmentUpr = pCharacterInstance->GetIAttachmentUpr())
				{
					pAttachment = pAttachmentUpr->GetInterfaceByName(m_boneName);
				}
			}
		}
		return pAttachment;
