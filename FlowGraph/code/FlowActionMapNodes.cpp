		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, EIP_SET))
				{
					if (pActInfo->pEntity)
					{
						...
						IActionMapUpr* pActionMapMgr = gEnv->pGameFramework->GetIActionMapUpr();
...
public:
	...
	{
		if (event.event == SActionMapEvent::eActionMapUprEvent_DefaultActionEntityChanged)
		{
...
{
	typedef VectorMap<u32, bool> TAnalogInputsPressed;
...
public:
	...
	virtual bool OnInputEvent(const SInputEvent& event)
	{
		...
		if (m_pActionMap && m_pActionMap->Enabled() && (m_filterEntityId == INVALID_ENTITYID || m_filterEntityId == m_pActionMap->GetActionListener()))
		{
			if (m_pAction)
			{
				tukk eventKeyNameCStr = event.keyName.c_str();

				i32k iNumInputData = m_pAction->GetNumActionInputs();
	...
	bool GetAnalogInputPressed(u32 actionCrc) const
	{
	...
	void SetAnalogInputPressed(u32 actionCrc, bool state)
	{
...
//////////////////////////////////////////////////////////////////////
class CFlowNode_ActionMapUpr : public CFlowBaseNode<eNCT_Instanced>
{
	enum EInputs
	{
		EIP_ENABLE = 0,
...
public:
	CFlowNode_ActionMapUpr(SActivationInfo* pActInfo)
		: m_pActInfo(pActInfo)
	{
	}

	~CFlowNode_ActionMapUpr()
	{
	}
	...
	IFlowNodePtr Clone(SActivationInfo* pActInfo)
	{
		return new CFlowNode_ActionMapUpr(pActInfo);
	}
...
public:
	...
	void Serialize(SActivationInfo* pActInfo, TSerialize ser)
	{
		...
		// on saving we ask the ActionMapUpr if the filter is enabled
		...
		{
			bool bWroteIt = false;
			IActionMapUpr* pAMMgr = gEnv->pGameFramework->GetIActionMapUpr();
...
REGISTER_FLOW_NODE("Input:ActionMaps:ActionMapUpr", CFlowNode_ActionMapUpr);
