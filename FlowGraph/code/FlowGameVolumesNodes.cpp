		...
		case eFE_Activate:
			{
				if (pActInfo->pEntity)
				{
					if (IsPortActive(pActInfo, eIP_TestWorldPosition))
					{
						...
						IGameVolumes* pGameVolumesUpr = gEnv->pGameFramework->GetIGameVolumesUpr();
						IGameVolumes::VolumeInfo volumeInfo;
						if (pGameVolumesUpr->GetVolumeInfoForEntity(pActInfo->pEntity->GetId(), &volumeInfo))
						{
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig &config) override
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("PointIndex", 0,   _HELP("Index to the point that is to be fetched.")),
		...
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<i32>   ("PointCount", _HELP("Number of points in the volume.")),
				...
				if (pActInfo->pEntity)
				{
					...
					if (bAnyGetPort)
					{
						...
						if (pGameVolumesUpr->GetVolumeInfoForEntity(pActInfo->pEntity->GetId(), &volumeInfo))
						{
							...
							{
								bSuccess = true;
								ActivateOutput(pActInfo, eOP_PointCount, static_cast<i32>(volumeInfo.verticesCount));
							}

							if (bGetPointByIndex)
							{
								i32 index = GetPortInt(pActInfo, eIP_PointIndex);
								if ((index >= 0) && (index < static_cast<i32>(volumeInfo.verticesCount)))
								{
									bSuccess = true;
							...
							if (bGetLastPoint)
							{
								if (volumeInfo.verticesCount>0)
								{
									...
									u32k index = volumeInfo.verticesCount - 1;
									const Vec3 localPoint = volumeInfo.pVertices[index];
