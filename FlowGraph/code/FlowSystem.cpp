...
#include <drx3D/FlowGraph/FlowLogNode.h>
#include <drx3D/FlowGraph/FlowStartNode.h>
#include <drx3D/FlowGraph/FlowTrackEventNode.h>
#include <drx3D/FlowGraph/FlowDelayNode.h>

#include <drx3D/FlowGraph/FlowScriptedNode.h>
#include <drx3D/FlowGraph/FlowCompositeNode.h>
#include <drx3D/FlowGraph/FlowEntityNode.h>

#include <drx3D/FlowGraph/FlowInspectorDefault.h>
...
TFlowInputData CFlowSystemContainer::GetItem(i32 i)
{
	return m_container[i];
}

void CFlowSystemContainer::RemoveItemAt(i32 i)
{
...
i32 CFlowSystemContainer::GetItemCount() const
{
	return m_container.size();
}

void CFlowSystemContainer::GetMemoryUsage(IDrxSizer* s) const
{
	if (GetItemCount() > 0)
...
void CFlowSystem::PreInit()
{
	...
	m_pModuleUpr = new CFlowGraphModuleUpr();
...
void CFlowSystem::LoadExtensions(string path)
{
	...
	if (handle != -1)
	{
		...
		do
		{
			drx_strcpy(filename, path.c_str());
			drx_strcat(filename, "/");
...
{
	XmlNodeRef node;
	i32 nChildren = nodeParent->getChildCount();
	i32 iChild;
...
TFlowNodeTypeId CFlowSystem::GenerateNodeTypeID()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_ERROR, "CFlowSystem::GenerateNodeTypeID: Reached maximum amount of NodeTypes: Limit=%d", CFlowData::TYPE_MAX_COUNT);
...
TFlowNodeTypeId CFlowSystem::RegisterType(tukk type, IFlowNodeFactoryPtr factory)
{
	...
	else
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "CFlowSystem::RegisterType: Type '%s' Id=%u already registered. Overriding not allowed by node factory.", type, nTypeId);
			return InvalidFlowNodeTypeId;
		}
...
bool CFlowSystem::UnregisterType(tukk typeName)
{
...
void CFlowSystem::Update()
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_ACTION, "CFlowSystem::Update()");
	...
	m_pModuleUpr->RemoveCompletedModuleInstances();
}
...
void CFlowSystem::Shutdown()
{
	...
	if (m_pModuleUpr != nullptr)
	{
		m_pModuleUpr->Shutdown();
		delete m_pModuleUpr;
		m_pModuleUpr = nullptr;
	}
...
//////////////////////////////////////////////////////////////////////////
TFlowGraphId CFlowSystem::RegisterGraph(CFlowGraphBase* pGraph, tukk debugName)
{
...
void CFlowSystem::LoadBlacklistedFlownodeXML()
{
	if (!m_blacklistNode)
	{
		...
		if (gEnv->pDrxPak->IsFileExist(BLACKLIST_FILE_PATH))
		{
...
//////////////////////////////////////////////////////////////////////////
bool CFlowSystem::BlacklistedFlownode(tukk* nodeName)
{
	if (m_blacklistNode)
	{
		i32 childCount = m_blacklistNode->getChildCount();
		tukk attrKey;
		tukk attrValue;
		for (i32 n = 0; n < childCount; ++n)
		{
			...
			{
				//replace name
				i32 numAttr = child->getNumAttributes();
...
//////////////////////////////////////////////////////////////////////////
tukk CFlowSystem::GetTypeName(TFlowNodeTypeId typeId)
{
	assert(typeId < m_typeRegistryVec.size());
	if (typeId < m_typeRegistryVec.size())
...
//////////////////////////////////////////////////////////////////////////
TFlowNodeTypeId CFlowSystem::GetTypeId(tukk typeName)
{
...
//////////////////////////////////////////////////////////////////////////
void CFlowSystem::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
//////////////////////////////////////////////////////////////////////////
IFlowGraphModuleUpr* CFlowSystem::GetIModuleUpr()
{
	return m_pModuleUpr;
}
...
{
	i32 count = m_FlowSystemContainers.size();
	...
	else
	{
		...
		for (i32 i = 0; i < count; i++)
		{
			...
			if (CreateContainer(id))
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_ERROR, "CFlowSystem::Serialize: Could not create or get container with ID from save file - container not restored");
				}
			}
			ser.EndGroup();
		}
