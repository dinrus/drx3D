...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("partIndex",   0,                            _HELP("Index of the part that will receive the impulse (optional, 1-based, 0=unspecified)")),
			InputPortConfig<i32>("CoordSys",    1,                            _HELP("Defines which coordinate system is used for the inputs values"),                        _HELP("CoordSys"), _UICONFIG("enum_int:Parent=0,World=1,Local=2")),
			{ 0 }
		};
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		if (event == eFE_Activate && IsPortActive(pActInfo, IN_ACTIVATE))
		{
			...
			i32 ipart = GetPortInt(pActInfo, IN_PARTINDEX);
			if (ipart > 0)
...
public:
	...
	virtual void GetConfiguration( SFlowNodeConfig& config ) override
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>( "PartId", -1, _HELP("Part ID to get joint info for.") ),
			{0}
		};
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<i32>( "JointID", _HELP("Id of the joint to which the specified part Id is attached on the selected entity.") ),
	...
	virtual void ProcessEvent( EFlowEvent event, SActivationInfo *pActInfo ) override
	{
		if (event == eFE_Activate)
		{
			...
			{
				i32k partId = GetPortInt(pActInfo, eIP_PartId);
				if (bGetJointId)
				{
				...
				if (bGetJointName)
				{
					...
					if (IEntity * pEntity = pActInfo->pEntity)
					{
						if (const ICharacterInstance* pCharacterInstance = pEntity->GetCharacter(0))
						{
							...
							tukk szJointName = defaultSkeleton.GetJointNameByID(partId);
...
{
	static i32 GetObjTypes(SActivationInfo* pActInfo, i32 objTypeInputIndex)
	{
		i32 result = GetPortInt(pActInfo, objTypeInputIndex);
		if ((result&ent_all) == 0)
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Raycast node got invalid flag sum %d for Object Types input", result);
			result = ent_all;
		}
	...
	static tukk GetObjTypesDescription()
	{
	...
	static tukk GetObjTypesUiConfig()
	{
...
public:
	...
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig<i32>("ObjType", ent_all, _HELP(GetObjTypesDescription()),nullptr, _UICONFIG(GetObjTypesUiConfig())),
			{0}
		};
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("ObjType", _HELP("The selected value as an integer.")),
			{0}
		};
		config.pInputPorts = in_config;
		config.pOutputPorts = out_config;
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo *pActInfo) override
	{
		...
		{
			i32k val = GetPortInt(pActInfo, eIP_ObjType);
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("ObjTypes", ent_all, _HELP(GetObjTypesDescription())),
		...
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<i32>("surfacetype",       _HELP("Surface type index at the hit point")),
			OutputPortConfig<i32>("partid", _HELP("Hit part id")),
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (event == eFE_Activate && IsPortActive(pActInfo, GO))
		{
			...
			if (pEntity)
			{
				i32 objTypes = GetObjTypes(pActInfo, OBJ_TYPES);
				ray_hit hit;
				...
				i32 numHits = pWorld->RayWorldIntersection(
				...
				if (numHits)
				{
					...
					ActivateOutput(pActInfo, SURFTYPE, (i32)hit.surface_idx);
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (event == eFE_Activate && IsPortActive(pActInfo, GO))
		{
			...
			if (skipEntityId != INVALID_ENTITYID && skipEntityId != localPlayerId)
			{
				...
				{
					DrxLog("FG Node 'Physics:RayCastCamera': Maximum skipped entities reached: the IgnoredEntity is not being ignored!");
				}
			}
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("EntityCamera", _HELP("")),
			{ 0 }
		};
...
CreateCam:
			...
			sph.r = 0.1f;
			sph.center.zero();
			IGeometry* psph = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::sphere::type, &sph);
			phys_geometry* pGeom = gEnv->pPhysicalWorld->GetGeomUpr()->RegisterGeometry(psph);
			psph->Release();
...
public:
	...
	static i32 OnConstraintBroken(const EventPhysJointBroken* ejb)
	{
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Id",                 1000,                     _HELP("Requested constraint Id; -1 to auto-generate, -2 to auto-generate and remember")),
			...
			InputPortConfig<i32>("PartIdA",            -1,                       _HELP("Part id to attach to; -1 to use default")),
			InputPortConfig<EntityId>("EntityB",       0,                        _HELP("Constrained entity (can be an actor)")),
			InputPortConfig<i32>("PartIdB",            -1,                       _HELP("Part id to attach to; -1 to use default")),
			InputPortConfig<i32>("Type",               _HELP("Constraint type"), _HELP("Type"),                                                                                         _UICONFIG("enum_int:Point=0,RotOnly=1,Line=2,Plane=3")),
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("Id",      _HELP("Constraint Id")),
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		else if (event == eFE_Activate)
		{
			...
			if (IsPortActive(pActInfo, IN_CREATE))
			{
				...
				i32k cflags[] = { 0, constraint_free_position, constraint_line, constraint_plane };
				...
				pe_params_flags pf;
				i32 queued = aac.pBuddy == WORLD_ENTITY ? 0 : aac.pBuddy->SetParams(&pf) - 1, id = pentPhys->Action(&aac, -queued >> 31);
				if (m_id == -2)
					m_id = id;
				...
				if (!is_unused(aac.maxPullForce || !is_unused(aac.maxBendTorque)) && !(aac.flags & constraint_no_tears))
				{
					if (!pRec)
						pRec = new SConstraintRec;
					gEnv->pPhysicalWorld->AddEventClient(EventPhysJointBroken::id, (i32 (*)(const EventPhys*))OnConstraintBroken, 1);
				}
				if (pRec)
				{
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>( "CollisionsPerFrame", 0, _HELP("Set the maximum amount of reported collisions per frame (0 = don't change)") ),
		...
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<i32>("PartIdA",         _HELP("Part id inside the first colliding entity")),
			OutputPortConfig<EntityId>("IdB",        _HELP("Id of the second colliding entity")),
			OutputPortConfig<i32>("PartIdB",         _HELP("Part id inside the second colliding entity")),
	...
	static i32   OnCollision(const EventPhysCollision* pColl)
	{
		...
		for (i32 i = 0; i < 2; i++)
			if (coll[i])
			{
				...
				if (ISurfaceType* pSurf = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr()->GetSurfaceType(pColl->idmat[i]))
					ActivateOutput(pActInfo, OUT_SURFACETYPE_A, string(pSurf->GetName()));
				if (ISurfaceType* pSurf = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr()->GetSurfaceType(pColl->idmat[i ^ 1]))
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		else if (event == eFE_Activate)
		{
			...
			if (IsPortActive(pActInfo, IN_ADD_ID))
			{
				...
				if (g_listeners.find(cl.id) == g_listeners.end() && (pent = gEnv->pEntitySystem->GetEntity(cl.id)) && pent->GetPhysics())
				{
					if (g_listeners.empty())
						gEnv->pPhysicalWorld->AddEventClient(EventPhysCollision::id, (i32 (*)(const EventPhys*))OnCollision, 1);
					pe_simulation_params sp;
			...
			if (IsPortActive(pActInfo, IN_REMOVE_ID) && (iter = g_listeners.find(GetPortEntityId(pActInfo, IN_REMOVE_ID))) != g_listeners.end())
			{
				...
				g_listeners.erase(iter);
				if (g_listeners.empty())
					gEnv->pPhysicalWorld->RemoveEventClient(EventPhysCollision::id, (i32 (*)(const EventPhys*))OnCollision, 1);
			}
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("PhysId", _HELP("Physical entity id")),
			{ 0 }
		};
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Type", _HELP("Type of the parameters to set"),         _HELP("Type"),_UICONFIG("enum_int:\
...
#define VAL(i)                                                                                        \
  ...
  InputPortConfig_AnyType("Value" # i "", _HELP("Sets the value 'Name" # i "' (must have correct type)")),
		...
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig<i32>("Result", _HELP("Param-specific result")),
			{ 0 }
		};
		config.nFlags |= EFLN_TARGET_ENTITY;
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("EntitySlot", 0, _HELP("Entity slot that contains the character")),
			InputPortConfig<i32>("PhysIndex", -1, _HELP("Index of the character physics (-1 - the main skeleton, >= 0 - auxiliary, such as ropes or cloth)")),
			{ 0 }
		};
		static const SOutputPortConfig out_config[] = {
	...
	std::map<i32,IEntity*> g_mapSkels;
};
...
struct STableLogger : IScriptTable
{
	...
	virtual uk GetUserDataValue() { return nullptr; }
	...
	virtual ScriptVarType GetValueType(tukk sKey) { return svtNull; }
	virtual ScriptVarType GetAtType(i32 nIdx) { return svtNull; }
	virtual void SetAtAny(i32 nIndex, const ScriptAnyValue& any) {}
	virtual bool GetAtAny(i32 nIndex, ScriptAnyValue& any) { return false; }
	...
	virtual bool GetValueAny(tukk sKey, ScriptAnyValue& any, bool bChain = false) { m_fields.push_back(string(sKey)); return false; }
};


#define PP(name) ("Physics:Params:" name)
static tukk s_PhysParamNames[] = 
...
{
public:
	CFlowNode_PhysParams2(i32 type, SActivationInfo *pActInfo, SInputPortConfig* inputs, string* params) : m_type(type), m_inputs(inputs), m_params(params)
	{
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (event == eFE_Activate)
		{
			for(i32 i=1; m_inputs[i].name; m_activeMask |= (uint64)IsPortActive(pActInfo,i) << (i-1), i++)
				;
...
{
public:
	static CAutoRegParamsNode s_Params[DRX_ARRAY_COUNT(s_PhysParamNames)];
	i32 GetType() const { return (i32)(this-s_Params); }
	...
	{ 
		...
		if (m_inputs.empty())
		{
			...
			{
				DrxCreateClassInstanceForInterface(drxiidof<IEntityScriptComponent>(), pScriptComponentDummy);
			}
			...
			for(const auto& str : m_table.m_fields)
			{
				...
				char *dst = (tuk)buf.data();
				bool caps = str[0] != 'k';
				for(tukk src=str.c_str(); *src; src++)
				{
					if (*src != '_')
					{
				...
				*dst = 0;
				...
				if (str[0]=='b' && isupper(str[1]) || !strncmp(str,"is_",3))
				{
					...
					input.defaultData = TFlowInputData::CreateDefaultInitialized<i32>(true);
				else if (type != 3 && (
					strstr(str, "heading") || strstr(str, "normal") || strstr(str, "origin") || (subs = (tuk)strstr(str, "end")) && subs[-1] != 'b' || 
...
CAutoRegParamsNode CAutoRegParamsNode::s_Params[DRX_ARRAY_COUNT(s_PhysParamNames)];
