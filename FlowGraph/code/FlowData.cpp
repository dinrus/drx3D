...
CFlowData::CFlowData(IFlowNodePtr pImpl, const string& name, TFlowNodeTypeId typeId)
{
	...
	{
		DRX_ASSERT(config.pInputPorts != NULL);
		m_pInputData[i] = config.pInputPorts[i].defaultData;
	}
	m_pOutputFirstEdge = std::unique_ptr<i32[]>(new i32[m_nOutputs]());
}
	...
	if (config.nFlags & EFLN_TARGET_ENTITY)
	{
		...
		*pInput++ = InputPortConfig<EntityId>("entityId", _HELP("Changes the attached entity dynamically"));
		...
		if (config.pInputPorts)
		{
			...
			{
				DRX_ASSERT(pInput != inputs + MAX_INPUT_PORTS);
...
bool CFlowData::ResolvePort(tukk name, TFlowPortId& port, bool isOutput)
{
	SFlowNodeConfig config;
	DoGetConfiguration(config);
	if (isOutput)
	{
		for (i32 i = 0; i < m_nOutputs; i++)
		{
			tukk sPortName = config.pOutputPorts[i].name;
	...
	else
	{
		for (i32 i = 0; i < m_nInputs; i++)
		{
			tukk sPortName = config.pInputPorts[i].name;
			...
			else if (sPortName)
			{
				...
				tukk sSpecial = strchr(sPortName, '_');
				if (sSpecial)
				...
				if (!stricmp(name, sPortName))
				{
					...
					DrxLogAlways("[flow] CFlowData::ResolvePort: Deprecation warning for port name: '%s' should be '%s'",
					             sPortName, config.pInputPorts[i].name);
					DrxLogAlways("[flow] Solution is to resave flowgraph in editor!");
	#endif
					port = i;
					return true;
				}
...
bool CFlowData::SerializeXML(IFlowNode::SActivationInfo* pActInfo, const XmlNodeRef& node, bool reading)
{
	...
	if (reading)
	{
		...
		{
			for (i32 i = m_hasEntity; i < m_nInputs; i++)
			{
				tukk value = inputsNode->getAttr(config.pInputPorts[i].name);
				if (value[0])
				...
				else
				{
					...
					{
	#ifdef FG_WARN_ABOUT_STRIPPED_PORTNAMES
						DrxLogAlways("[flow] CFlowData::SerializeXML: Deprecation warning for port name: '%s' should be '%s'",
						             sPortName, config.pInputPorts[i].name);
						DrxLogAlways("[flow] Solution is to resave flowgraph in editor!");
	#endif
						SetFromString(m_pInputData[i], value);
		...
		if (config.nFlags & EFLN_TARGET_ENTITY)
		{
			tukk sGraphEntity = node->getAttr("GraphEntity");
			if (*sGraphEntity != 0)
			{
				i32 nIndex = atoi(sGraphEntity);
				if (nIndex == 0)
				{
			...
			else
			{
				...
				else
				{
					//i32 id;
					//node->getAttr("Id",id);
					//tukk nodeClass = node->getAttr("Class");
...
void CFlowData::Serialize(IFlowNode::SActivationInfo* pActInfo, TSerialize ser)
{
	...
	char idName[16] = "in_x";
	for (i32 i = startIndex; i < m_nInputs; i++)
	{
		itoa(i, &idName[3], 10);
