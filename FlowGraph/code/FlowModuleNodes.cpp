...
void CFlowModuleReturnNodeFactory::GetConfiguration(SFlowNodeConfig& config)
{
	...
	if (m_pModule && (gEnv->IsEditing() || m_inputs.empty()))
	{
		...
		for (size_t i = 0; i < customPortCount; ++i)
		{
			...
			if (pPort)
			{
				m_inputs[idx + i].defaultData = TFlowInputData::CreateDefaultInitializedForTag((i32)pPort->type, (pPort->type == eFDT_Any ? false : true)); //lock ports of specific type
...
{
	CFlowGraphModuleUpr* pModuleUpr = static_cast<CFlowSystem*>(gEnv->pFlowSystem)->GetModuleUpr();
	if (!pModuleUpr->GetModule(m_moduleId))
	{
...
void CFlowModuleUserIdNode::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig<i32>("UserID",     0, _HELP("Custom user id (e.g. an entity id)")),
		InputPortConfig<string>("ModuleName", _HELP("Name of the FG Module"), _HELP("Module Name"), _UICONFIG("dt=fgmodules")),
		InputPortConfig<i32>("InstanceID",-1, _HELP("Module instance id")),
		{ 0 }
	};

	static const SOutputPortConfig out_config[] = {
		OutputPortConfig<i32>("ModuleId", _HELP("Module instance id for given user id (returns -1 if not id was found)"), _HELP("InstanceID")),
		{ 0 }
	};
...
void CFlowModuleUserIdNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_Activate:
		if (IsPortActive(pActInfo, eIP_Get))
		{
			...
			i32k userid = GetPortInt(pActInfo, eIP_UserId);

			TInstanceUserIDs::const_iterator it = s_ids.find(moduleName);
			if (it != s_ids.end())
			{
				std::map<i32, TModuleInstanceId>::const_iterator it2 = it->second.find(userid);
		...
		{
			const string& moduleName = GetPortString(pActInfo, eIP_ModuleName);
			i32k instanceId = GetPortInt(pActInfo, eIP_ModuleInstanceId);
			i32k userid = GetPortInt(pActInfo, eIP_UserId);
...
{
	CFlowGraphModuleUpr* pModuleUpr = static_cast<CFlowSystem*>(gEnv->pFlowSystem)->GetModuleUpr();
	pModuleUpr->RegisterListener(this, "CFlowModuleCountListener");
}

CFlowModuleCountListener::~CFlowModuleCountListener()
{
	CFlowGraphModuleUpr* pModuleUpr = static_cast<CFlowSystem*>(gEnv->pFlowSystem)->GetModuleUpr();
	pModuleUpr->UnregisterListener(this);
}
...
void CFlowModuleCountListener::GetConfiguration(SFlowNodeConfig& config)
{
	...
	static const SOutputPortConfig out_config[] = {
		OutputPortConfig<i32>("Count", _HELP("Number of running instances of this module. -1 if the module name is not found")),
		{ 0 }
	};
...
void CFlowModuleCountListener::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_Activate:
		if (IsPortActive(pActInfo, eIP_ModuleName))
		{
			m_sModuleName = GetPortString(pActInfo, eIP_ModuleName);
			CFlowGraphModuleUpr* pModuleUpr = static_cast<CFlowSystem*>(gEnv->pFlowSystem)->GetModuleUpr();
			CFlowGraphModule* pModule = static_cast<CFlowGraphModule*>(pModuleUpr->GetModule(m_sModuleName.c_str()));
			ActivateOutput(pActInfo, eOP_InstanceCount, pModule ? static_cast<i32>(pModule->GetRunningInstancesCount()) : -1);
		}
		break;
	}
}
...
void CFlowModuleCountListener::OnModuleInstanceCreated(IFlowGraphModule* pModule, TModuleInstanceId instanceID)
{
	...
	{
		ActivateOutput(&m_actInfo, eOP_InstanceCount, static_cast<i32>(pModule->GetRunningInstancesCount()));
	}
}
