...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   FlowCustomActionNodes.h
   $Id$
...
#include <drx3D/FlowGraph/FlowCustomActionNodes.h>
...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		if (!pCustomAction) // Not inside a custom graph, must be an instance hack, get associated entity
		{
			...
			if (!pEntity)
			{
				DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "CFlowNode_CustomActionEnd::ProcessEvent: Instance hack must have assigned entity");
				return;
			}
			...
			if (pCustomActionUpr)
			{
				pCustomAction = pCustomActionUpr->GetActiveCustomAction(pEntity);
				if (!pCustomAction)
				{
					DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "CFlowNode_CustomActionEnd::ProcessEvent: Can't find custom action for entity");
					return;
				}
			}
		}
...
CFlowNode_CustomActionControl::~CFlowNode_CustomActionControl()
{
	...
	if (pCustomActionUpr)
	{
		pCustomActionUpr->UnregisterListener(this);
	}
}
...
void CFlowNode_CustomActionControl::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	if (event == eFE_Activate)
	{
		...
		ICustomActionUpr* pCustomActionUpr = gEnv->pGameFramework->GetICustomActionUpr();
		DRX_ASSERT(pCustomActionUpr != NULL);
		if (!pCustomActionUpr)
			return;
		...
		if (IsPortActive(pActInfo, EIP_Start))
		{
			...
			pCustomActionUpr->StartAction(pEntity, customGraphName.c_str(), this);
		...
		else if (IsPortActive(pActInfo, EIP_Succeed))
		{
			...
			pCustomActionUpr->SucceedAction(pEntity, customGraphName.c_str(), this);

			// Output port is activated from event
		}
		else if (IsPortActive(pActInfo, EIP_SucceedWait))
		{
			pCustomActionUpr->SucceedWaitAction(pEntity);

			// Output port is activated from event
		}
		else if (IsPortActive(pActInfo, EIP_SucceedWaitComplete))
		{
			pCustomActionUpr->SucceedWaitCompleteAction(pEntity);
			// Output port is activated from event
		}
		else if (IsPortActive(pActInfo, EIP_Abort))
		{
			pCustomActionUpr->AbortAction(pEntity);

			// Output port is activated from event
		}
		else if (IsPortActive(pActInfo, EIP_EndSuccess))
		{
			ICustomAction* pCustomAction = pCustomActionUpr->GetActiveCustomAction(pEntity);
			if (!pCustomAction)
			{
				return;
			}
