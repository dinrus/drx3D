...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("ValueType",      0,                                           _HELP("Defines if the 'Value' input will be interpreted as speed or as time (duration)."),  _HELP("ValueType"),     _UICONFIG("enum_int:speed=0,time=1")),
			InputPortConfig<float>("Value",        0.f,                                         _HELP("Speed (Degrees/sec) or Time (duration in secs), depending on 'ValueType' input.")),
			InputPortConfig<i32>("CoordSys",       0,                                           _HELP("Destination in Parent relative, World (absolute) or Local relative angles"),         _HELP("CoordSys"),      _UICONFIG("enum_int:Parent=0,World=1,Local=2")),
		...
		case eFE_Update:
			{
				...
				if (curTime >= m_endTime)
				{
					...
					static const float s_Drxsis3HacDeactivateDelay = 1.0f;
					if (curTime >= (m_endTime + s_Drxsis3HacDeactivateDelay))
					{
						pActInfo->pGraph->SetRegularlyUpdated(pActInfo->myID, false);
					}
