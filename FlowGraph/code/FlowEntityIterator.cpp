...
#define ADD_BASE_INPUTS_ENTITY_ITER()                                                                                           \
  ...
  InputPortConfig<i32>("Limit", 0, _HELP("Limit how many entities are returned. Use 0 to get all entities")),                   \
  InputPortConfig<bool>("Immediate", false, _HELP("TRUE to iterate immediately through results, without having to call Next")), \
  InputPortConfig<i32>("Type", 0, _HELP("Type of entity to iterate"), 0, _UICONFIG(ENTITY_TYPE_ENUM))
...
#define ADD_BASE_OUTPUTS_ENTITY_ITER()                                                                                      \
  ...
  OutputPortConfig<i32>("Count", _HELP("Called each time an entity is found, with the current count returned")),            \
...
bool IsValidType(i32 requested, const EEntityType& type)
{
	bool bValid = false;
...
protected:
	...
	void Output()
	{
		if (m_Count > 0)
		{
			i32k outputCount = m_bImmediate ? m_Count : m_Iter + 1;
...
public:
	...
	virtual void GetEntities()
	{
		...
		i32k entityCount = pWorld->GetEntitiesInBox(m_min, m_max, ppList, ent_all);

		for (i32 i = 0; i < entityCount; ++i)
		{
			i32k id = pWorld->GetPhysicalEntityId(ppList[i]);
			if (id > 0)
			{
...
public:
	...
	virtual void GetEntities()
	{
		...
		IPhysicalEntity** ppList = NULL;
		i32k size = gEnv->pPhysicalWorld->GetEntitiesInBox(boxMin, boxMax, ppList, ent_all);
		for (i32 i = 0; i < size; ++i)
		{
			i32k id = gEnv->pPhysicalWorld->GetPhysicalEntityId(ppList[i]);
			if (id > 0)
			{
					...
					case ENTITY_AREA_TYPE_SHAPE:
						{
							...
							i32k count = pAreaProxy->GetPointsCount();
							if (count > 0)
							{
