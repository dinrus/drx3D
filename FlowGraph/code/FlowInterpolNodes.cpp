...
// -------------------------------------------------------------------------
//  Имя файла:   FlowInterpolNodes.cpp
//  Version:     v1.00
...
{
	static tukk min_name;
	static tukk max_name;
...
template<typename T> tukk Limits<T>::min_name = "StartValue";
template<typename T> tukk Limits<T>::max_name = "EndValue";

template<> const float Limits<float>::min_val = 0.0f;
template<> const float Limits<float>::max_val = 1.0f;
template<> i32k Limits<i32>::min_val = 0;
template<> i32k Limits<i32>::max_val = 255;
...
template<> tukk LimitsColor<Vec3>::min_name = "color_StartValue";
template<> tukk LimitsColor<Vec3>::max_name = "color_EndValue";
...
#define BUILD_EASING_UI_ENUMS(enumValue, enumName, ...) \
  temp.Format(",%02d:%s=%d", static_cast<i32>(enumValue), enumName, static_cast<i32>(enumValue)); \
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Easing",                        _HELP("Easing-Function that is applied to the interpolation"),0,                                                       _UICONFIG(Easing::GetDropdownString().c_str())),
			{ 0 }
		};
	...
	bool GetValue(SActivationInfo* pActInfo, i32 nPort, T& value)
	{
...
typedef CFlowInterpolNode<i32>                     IntInterpol;
...
typedef CFlowSmoothNode<i32>   SmoothInt;
