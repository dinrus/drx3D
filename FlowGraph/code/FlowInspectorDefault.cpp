...
namespace
{
static i32k MAX_ROWS = 23;             // max number of one-time-flow lines
static i32k MAX_ROWS_CONT = 12;        // max number of rows for continuous flows
...
tukk GetPortName(IFlowGraph* pGraph, const SFlowAddress addr)
{
...
{
	tukk typeName = pGraph->GetNodeTypeName(addr.node);
	tukk nodeName = pGraph->GetNodeName(addr.node);
	string human;
	human += typeName;
	human += " Node:";
...
CFlowInspectorDefault::NotifyFlow(IFlowGraph* pGraph, const SFlowAddress from, const SFlowAddress to)
{
	...
	drx_sprintf(msg, "0x%p %s [%s:%s] -> [%s:%s] Val=%s",
...
void
CFlowInspectorDefault::DrawLabel(float col, float row, const ColorF& color, float glow, tukk szText, float fScale) const
{
	const float ColumnSize = COL_SIZE;
	const float RowSize = ROW_SIZE;

	DrxFixedStringT<128> msg;
...
/* virtual */ void
CFlowInspectorDefault::DrawRecords(const std::deque<TFlowRecord>& inRecords, i32 inBaseRow, float inMaxAge) const
{
	// records are pushed back, but we want to draw new records on top
	// so we draw from bottom to top, ensuring linear mem access
	i32 row = inRecords.size() - 1 + inBaseRow;
	string val;
	ColorF color;
...
template<class T>
static void AddFlowRecordsTo(const T& cont, IDrxSizer* s)
{
	s->AddObject(cont);
...
void CFlowInspectorDefault::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
