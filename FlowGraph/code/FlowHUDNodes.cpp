	...
	void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Update:
			...
			if (CFlowSystemCVars::Get().fg_DisableHUDText == 0)
			{
				...
				i32 aspectRatioFlag = 0;

				if ((x < 1.f || y < 1.f) && gEnv->pRenderer)
				{
					i32k screenWidth  = std::max(IRenderAuxGeom::GetAux()->GetCamera().GetViewSurfaceX(), 1);
					i32k screenHeight = std::max(IRenderAuxGeom::GetAux()->GetCamera().GetViewSurfaceZ(), 1);
				...
				i32 flags = eDrawText_2D | aspectRatioFlag | eDrawText_FixedSize | (GetPortBool(pActInfo, INP_Centered) ? eDrawText_Center | eDrawText_CenterV : 0);
