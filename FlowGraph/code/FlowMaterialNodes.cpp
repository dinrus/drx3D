...
// -------------------------------------------------------------------------
//  Имя файла:   FlowMaterialNodes.cpp
//  Version:     v1.00
//  Created:     04/11/2013 by Marco H.
//  Описание: flownodes copied and updated from FlowEngineNodes.cpp
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
public:
			...
			if (ser.IsReading())
			{
				if (pEntity)
				{
					...
					else
					{
						...
						{
							IMaterial* pMtl = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(matSavedName.c_str());
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] =
		{
			...
			InputPortConfig<i32>("slot", -1, _HELP("Slot on which to apply the material. -1 if it should be applied to the whole entity.")),
		...
		case eFE_PrecacheResources:
			{
				...
				m_pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(mtlName.c_str());
				break;
			}
	...
	void ChangeMat(SActivationInfo* pActInfo, IEntity* pEntity)
	{
		...
		const string& mtlName = GetPortString(pActInfo, IN_MATERIAL);
		_smart_ptr<IMaterial> pMtl = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(mtlName.c_str());
		if (pMtl != NULL)
		{
	...
	{
		i32k slot = GetPortInt(pActInfo, IN_SLOT);
		if (slot == -1)
		{
			pEntity->SetMaterial(pMtl);
		}
...
public:
	...
	IMaterial* ObtainMaterial(SActivationInfo* pActInfo)
	{
		...
		IMaterial* pMtl = gEnv->p3DEngine->GetMaterialUpr()->FindMaterial(matName);
		if (pMtl == 0)
		{
...
public:
	...
	/// this does not happens in drxsis2, so is good enough for now.
	...
	void Serialize(SActivationInfo* pActInfo, TSerialize ser)
	{
		...
		// we do assume that those inputs never change, which is ok for drxsis2, but of course is not totally right
	...
	bool ReadFloatValueFromMat(IMaterial* pSubMtl, IRenderShaderResources* pRendShaderRes, tukk paramName, float& floatValue)
	{
	...
	bool ReadColorValueFromMat(IMaterial* pSubMtl, IRenderShaderResources* pRendShaderRes, tukk paramName, Vec3& colorValue)
	{
	...
	//////////////////////////////////////////////////////////////////////////
	bool ObtainMaterialPtrs(IEntity* pEntity, i32 slot, i32 subMtlId, bool cloneMaterial, IMaterial*& pSubMtl, IRenderShaderResources*& pRenderShaderRes)
	{
		...
		{
			pMaterial = gEnv->p3DEngine->GetMaterialUpr()->CloneMultiMaterial(pMaterial);
			if (!pMaterial)
			{
...
public:
	...
	void UnApplyMaterial(IEntity* pEntity, i32 slot)
	{
		if (pEntity == 0)
			return;
		IEntityRender* pIEntityRender = pEntity->GetRenderInterface();
	...
	void CloneAndApplyMaterial(IEntity* pEntity, i32 slot)
	{
		if (pEntity == 0)
			return;
		...
		pMtl = gEnv->p3DEngine->GetMaterialUpr()->CloneMultiMaterial(pMtl);
		pIEntityRender->SetSlotMaterial(slot, pMtl);
	}
