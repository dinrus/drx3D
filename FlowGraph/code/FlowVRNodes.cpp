		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, EIP_RecenterPose))
				{
					...
					const IHmdUpr* pHmdUpr = gEnv->pSystem->GetHmdUpr();
					IHmdDevice* pDev = pHmdUpr ? pHmdUpr->GetHmdDevice() : nullptr;
					if (pDev && pHmdUpr->IsStereoSetupOk())
					{
		...
		case eFE_Update:
			{
				...
				const IHmdUpr* pHmdUpr = gEnv->pSystem->GetHmdUpr();
				const IHmdDevice* pDev = pHmdUpr ? pHmdUpr->GetHmdDevice() : nullptr;
				if (pDev)
				{
					const HmdTrackingState& sensorState = pDev->GetLocalTrackingState();
					if (sensorState.CheckStatusFlags(eHmdStatus_IsUsable))
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig<bool>("Enabled", true, _HELP("Enable / disable the node")),
			InputPortConfig<i32>("Index",    true, _HELP("Controller Index")),
			{ 0 }
		};
		static const SOutputPortConfig out_config[] = {
...
public:
	...
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig<i32>("Layer",               0,                          _HELP("VR Quad render layer"),                         _HELP("Layer")),
			InputPortConfig<bool>("Enable",             false,                      _HELP("Enable/Disable sending this layer to the Hmd"), _HELP("Enable")),
			InputPortConfig<i32>("Type",                0,                          _HELP("Free Quad or Locked Quad"),                     _HELP("Type"),   _UICONFIG(RenderLayer::GetQuadLayerType_UICONFIG())),
		...
		case eFE_Activate:
			{
				...
				i32k id = GetPortInt(pActInfo, EIP_Layer);
				...
				{
					i32k type = GetPortInt(pActInfo, EIP_Type);
...
public:
	...
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig<i32>("LayerType",          0,                                                                  _HELP("VR render layer type"), _HELP("LayerType"),   _UICONFIG(RenderLayer::GetLayerType_UICONFIG())),
			InputPortConfig<i32>("QuadLayerId",        0,                                                                  _HELP("VR Quad layer id"),     _HELP("QuadLayerId"), _UICONFIG(RenderLayer::GetQuadLayer_UICONFIG())),
		...
		case eFE_Activate:
			{
				// Get Layer info
				RenderLayer::CProperties* pLayerProps = nullptr;
				i32k layerType = GetPortInt(pActInfo, EIP_LayerType);
				i32k layerId = GetPortInt(pActInfo, EIP_LayerId);
