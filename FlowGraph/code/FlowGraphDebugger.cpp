...
// -------------------------------------------------------------------------
//  Имя файла:   FlowGraphDebugger.cpp
//  Version:     v1.00
...
#include <drx3D/FlowGraph/StdAfx.h>
	...
	#include <drx3D/FlowGraph/FlowGraphDebugger.h>
	#include <drx3D/FlowGraph/FlowData.h>
...
DRXREGISTER_SINGLETON_CLASS(CFlowGraphDebugger)
	...
	if (iter != m_DebugInfo.end())
	{
		...
		if (iterNode != flownodesDebugInfo->end())
		{
			...
			for (std::vector<SBreakPointPortInfo>::const_iterator iterBreakpoint = (*flowPortIDS).begin(); iterBreakpoint != (*flowPortIDS).end(); ++iterBreakpoint)
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "Port %u for Flownode %u is already in debug list [CFlowgraphDebugger::AddBreakPoint]", addr.port, addr.node);
					return false;
				}
			}
...
bool CFlowGraphDebugger::RemoveBreakpoint(IFlowGraphPtr pFlowgraph, const SFlowAddress& addr)
{
	...
	if (iterDebugInfo != m_DebugInfo.end())
	{
		...
		if (iterNode != flownodesDebugInfo->end())
		{
			...
			i32k numBreakpoints = breakPointInfo->inputPorts.size() + breakPointInfo->outputPorts.size();
		...
		{
			DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "Flownode %u is not in debug list [CFlowgraphDebugger::RemoveBreakPoint]", addr.node);
			return false;
		}
	}

	DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "Port %u of Flownode %u is not in debug list [CFlowgraphDebugger::RemoveBreakPoint]", addr.port, addr.node);
	return false;
}

bool CFlowGraphDebugger::RemoveAllBreakpointsForNode(IFlowGraphPtr pFlowgraph, TFlowNodeId nodeID)
{
	DRX_ASSERT(nodeID != InvalidFlowNodeId);
	...
	{
		DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "Flownode %u is not in debug list [CFlowgraphDebugger::RemoveBreakPointsForNode]", nodeID);
		return false;
	}
...
bool CFlowGraphDebugger::PerformActivation(IFlowGraphPtr pFlowgraph, i32 edgeIndex, const SFlowAddress& fromAddr, const SFlowAddress& toAddr, const TFlowInputData& value)
{
	if (m_BreakpointHit && !m_RePerformActivation)
	{
		// The system tries to activate a port after a breakpoint was hit, so cache the activation in a deque.
...
bool CFlowGraphDebugger::RePerformActivation()
{
	...
	else if (m_BreakPoint.type == eBT_Output || m_BreakPoint.type == eBT_Input)
	{
		...
		DRX_ASSERT(m_BreakPoint.flowGraph != 0);
		...
		if (pEdgeIter)
		{
			...
			if (m_BreakPoint.type == eBT_Input)
			{
				...
				if (pData)
				{
					i32 firstEdge = pData->GetOutputFirstEdge(m_BreakPoint.fromAddr.port) + m_BreakPoint.edgeIndex;
			...
			i32 edgeIndex = m_BreakPoint.edgeIndex;
			bool bPerform = true;
...
bool CFlowGraphDebugger::TryActivatePort(SBreakPoint& breakpoint, IFlowEdgeIterator::Edge& edge, bool& bPerform, i32 edgeIndex)
{
...
bool CFlowGraphDebugger::RegisterListener(IFlowGraphDebugListener* pListener, tukk name)
{
...
IFlowGraphPtr CFlowGraphDebugger::GetRootGraph(IFlowGraphPtr pFlowGraph) const
{
	...
	{
		IFlowGraphModule* pModule = gEnv->pFlowSystem->GetIModuleUpr()->GetModule(pFlowGraph);
