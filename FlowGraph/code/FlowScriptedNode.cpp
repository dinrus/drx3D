...
namespace
{
bool GetCategory(tukk catName, u32& outCategory)
{
...
{
public:
	CFlowDataToScriptDataVisitor(IScriptTable* pTable, tukk name) : m_pTable(pTable), m_name(name) {}
...
void CFlowScriptedNode::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
}
...
i32 CFlowScriptedNode::ActivatePort(IFunctionHandler* pH, size_t nOutput, const TFlowInputData& data)
{
...
bool CFlowScriptedNodeFactory::Init(tukk path, tukk nodeName)
{
...
const string& CFlowScriptedNodeFactory::AddString(tukk c_str)
{
	string str = c_str;
	return *m_stringTable.insert(str).first;
}

i32 CFlowScriptedNodeFactory::ActivateFunction(IFunctionHandler* pH, uk pBuffer, i32 nSize)
{
	// TODO: log errors
	IScriptSystem* pSS = pH->GetIScriptSystem();

	DRX_ASSERT(nSize == sizeof(size_t));
...
void CFlowScriptedNodeFactory::GetMemoryUsage(IDrxSizer* s) const
{
...
bool CFlowSimpleScriptedNodeFactory::Init(tukk path, tukk nodeName)
{
	static SInputPortConfig termInput = { 0 };
	static SOutputPortConfig termOutput = { 0 };
	activateFlags = 0;

	if (tukk p = strchr(nodeName, ':'))
		nodeName = p + 1;
	...
	{
		i32 nItems = inputs->Count();
		for (i32 i = 1; i <= nItems; i++)
		{
			...
			if (inputs->GetAt(i, tbl))
			{
				...
				else if (0 == strcmp("i32", type))
					m_inputs.push_back(InputPortConfig<i32>(name.c_str(), dname, NULL, enumStr));
		...
		for (i32 i = 1; i <= nItems; i++)
		{
			...
			if (outputs->GetAt(i, tbl))
			{
				...
				if (0 == strcmp("string", type))
					...
					m_outputs.push_back(OutputPortConfig<i32>(name.c_str(), dname));
				else if (0 == strcmp("float", type))
				...
				{
					GameWarning("Неизвестный тип %s", type);
					return false;
				}
			}
			else
...
const string& CFlowSimpleScriptedNodeFactory::AddString(tukk c_str)
{
	string str = c_str;
	return *m_stringTable.insert(str).first;
}

void CFlowSimpleScriptedNodeFactory::GetMemoryUsage(IDrxSizer* s) const
{
