...
IGameToken* GetGameToken(tukk callingNodeName, IFlowNode::SActivationInfo* pActInfo, i32 tokenPort, bool bForceCreate = false)
{
	...
	if (!pToken && pActInfo->pGraph != nullptr)
	{
		...
		tukk name = pActInfo->pGraph->GetGlobalNameForGraphToken(tokenName.c_str());
...
void WarningIfGameTokenUsedWithWrongType(tukk callingNodeName, IFlowNode::SActivationInfo* pActInfo, tukk tokenName, TFlowInputData& data, const string& valueStr)
{
...
void DryRunAndWarningIfGameTokenUsedWithWrongType(tukk place, IFlowNode::SActivationInfo* pActInfo, i32 tokenPort, i32 valuePort)
{
...
{
	DRX_ASSERT(tokenData.GetType() == valueData.GetType());
...
private:
	...
	static constexpr tukk s_sNodeName = "GameTokenSet";
};
...
private:
	...
	IGameToken* m_pCachedToken;
	static constexpr tukk s_sNodeName = "GameTokenGet";
};
...
private:
	...
	static constexpr tukk s_sNodeName = "GameTokenCheck";
};
...
private:
	...
	static constexpr tukk s_sNodeName = "GameTokenCheckMulti";
};
...
private:
	void TriggerOutputs()
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_ERROR, "[FG] %s: GameToken %s changed its mind from %s to %s.",
				s_sNodeName,
		...
		if (!GetPortBool(&m_actInfo, eIn_TriggerOnlyOnResultChange) || (i32)bEquals != m_prevEqualResult)
		{
...
private:
	...
	IGameToken*     m_pCachedToken;
	TFlowInputData  m_cachedValue;
	i32             m_prevEqualResult;
	static constexpr tukk s_sNodeName = "GameToken (listener)";
};
...
public:
	...
	bool                      not_zero(i32& val)           { return val != 0; }
	...
	void WarnIfWrongOperationType(SActivationInfo* pActInfo)
	{
		...
		i32 opType = GetPortInt(pActInfo, eIN_Type);
		...
		if (tempGT)
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_ERROR, "[FG] %s: Using wrong operation type for GameToken '%s'", s_sNodeName, tempGT->GetName());
			}
		}
	}
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig_Void("Trigger",            _HELP("Trigger this input to actually get the game token value"), _HELP("Trigger")),
			InputPortConfig<string>("gametoken_Token", _HELP("Game token to set"), _HELP("Token")),
			InputPortConfig<i32>("Op",     EOP_SET,    _HELP("Operation token = token OP value"), _HELP("Operation"), _UICONFIG("enum_int:Set=0,Add=1,Sub=2,Mul=3,Div=4")),
			InputPortConfig<i32>("Type",   ET_STRING,  _HELP("Type"), _HELP("Type"), _UICONFIG("enum_int:Bool=0,Int=1,Float=2,Vec3=3,String=4")),
		...
		case eFE_Activate:
			{
				...
				if (IsPortActive(pActInfo, eIN_Trigger))
				{
					...
					if (m_pCachedToken)
					{
						...
						case ET_INT:
							{ Helper<i32> h(tokenData, m_cachedValue); DoOp(op, h); if (h.m_ok) m_pCachedToken->SetValue(TFlowInputData(h.m_value)); }    break;
						case ET_FLOAT:
...
private:
	...
	IGameToken* m_pCachedToken;
	TFlowInputData m_cachedValue;
	static constexpr tukk s_sNodeName = "GameTokenModify";
};
		...
		case eFE_Activate:
			{
				...
				{
					std::vector<tukk > gameTokensList;
					for (u32 i = INP_TOKEN1; i <= INP_TOKEN10; ++i)
					{
						const string& gameTokenName = GetPortString(pActInfo, i);
						if (!gameTokenName.empty())
							gameTokensList.push_back(gameTokenName.c_str());
					}
					tukk* ppGameTokensList = &(gameTokensList[0]);
					GetIGameTokenSystem()->SerializeSaveLevelToLevel(ppGameTokensList, (u32)gameTokensList.size());
				}
				break;
			}
		}
