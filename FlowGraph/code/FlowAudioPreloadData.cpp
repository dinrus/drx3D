...
public:
	...
	void Enable(SActivationInfo* pActInfo, bool bEnable)
	{
		...
		{
			for (u32 i = eIn_PreloadRequestFirst; i <= eIn_PreloadRequestLast; ++i)
			{
				string const& preloadName = GetPortString(pActInfo, static_cast<i32>(i));

				if (!preloadName.empty())
				{
					DrxAudio::PreloadRequestId const preloadRequestId = DrxAudio::StringToId(preloadName.c_str());
