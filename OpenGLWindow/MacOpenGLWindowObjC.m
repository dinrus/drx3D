...
-(void) MakeContext:(i32) openglVersion;
-(void) MakeCurrent;
...
i32 Mac_updateWindow(struct MacOpenGLWindowInternalData* m_internalData)
{
    ...
    do
    {
		...
		{
			i32 modifiers = [event modifierFlags];
		...
		if ([event type] == MyNSKeyUp)
        {
			...
			u32 virtualKeycode = [event keyCode];
		   
			i32 keycode = getAsciiCodeFromVirtualKeycode(virtualKeycode);
			// printf("keycode = %d\n", keycode);
...
{
    i32 closeme = m_internalData->m_myview.GetRequestClose;
...
i32 Mac_fileOpenDialog(tuk filename, i32 maxNameLength)
{
   ...
   if (zUrl)
   {
       ...
       i32 slen = (i32)[myString length];
       if (slen < maxNameLength)
       {
           tukk cfilename=[myString UTF8String];
           //expect file:// at start of URL
           tukk p = strstr(cfilename, "file://");
            if (p==cfilename)
            {
                i32 actualLen = (i32)strlen(cfilename)-7;
