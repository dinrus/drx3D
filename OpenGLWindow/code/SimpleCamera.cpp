...
SimpleCameraInternalData
{
	...
	D3_ATTRIBUTE_ALIGNED16(float)
	m_offsetTransformVR[16];
	b3Vec3 m_cameraTargetPosition;
	float m_cameraDistance;
	b3Vec3 m_cameraUp;
	b3Vec3 m_cameraForward;
	i32 m_cameraUpAxis;
	//the m_cameraPosition is a cached value, recomputed from other values
	b3Vec3 m_cameraPosition;
	float m_yaw;
...
void SimpleCamera::setVRCamera(const float viewMat[16], const float projectionMatrix[16])
{
	...
	b3Matrix3x3 vm;
	vm.setVal(viewMat[0], viewMat[4], viewMat[8],
				viewMat[1], viewMat[5], viewMat[9],
				viewMat[2], viewMat[6], viewMat[10]);

	b3Vec3 vp = b3MakeVector3(viewMat[12], viewMat[13], viewMat[14]);
	b3Transform tr;
	tr.setBasis(vm);
	tr.setOrigin(vp);
...
#endif
static void b3CreateLookAt(const b3Vec3& eye, const b3Vec3& center, const b3Vec3& up, float result[16])
{
...
void SimpleCamera::setCameraUpAxis(i32 upAxis)
{
...
i32 SimpleCamera::getCameraUpAxis() const
{
...
void SimpleCamera::update()
{
	...
	if (m_data->m_cameraForward.length2() < D3_EPSILON)
	{
		m_data->m_cameraForward.setVal(1.f, 0.f, 0.f);
	}
	else
	{
...
{
	m_data->m_cameraTargetPosition.setVal(x, y, z);
	update();
}
...
{
	m_data->m_cameraUp.setVal(x, y, z);
	update();
}
