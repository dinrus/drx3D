...
i32 getSpecialKeyFromVirtualKeycode(i32 virtualKeyCode)
{
	i32 keycode = -1;
...
i32 getAsciiCodeFromVirtualKeycode(i32 virtualKeyCode)
{
	i32 keycode = 0xffffffff;
...
bool Win32Window::isModifierKeyPressed(i32 key)
{
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_KEYUP:
		{
			i32 keycode = getSpecialKeyFromVirtualKeycode(wParam);
			switch (keycode)
			{
				case B3G_ALT:
				{
		...
		case WM_CHAR:
		{
			...
			i32 keycode = getAsciiCodeFromVirtualKeycode(wParam);
			if (keycode < 0)
			{
		...
		{
			i32 zDelta = (short)HIWORD(wParam);
			i32 xPos = LOWORD(lParam);
			i32 yPos = HIWORD(lParam);
...
void Win32Window::setWindowTitle(tukk titleChar)
{
#ifdef _WIN64
...
{
	i32 oglViewportWidth = ci.m_width;
	i32 oglViewportHeight = ci.m_height;
	bool fullscreen = ci.m_fullscreen;
	i32 colorBitsPerPixel = ci.m_colorBitsPerPixel;
	uk windowHandle = ci.m_windowHandle;
	...
	if (!windowHandle)
	{
		...
		i32 windowLeft = (GetSystemMetrics(SM_CXSCREEN) - m_data->m_fullWindowWidth) / 2;
		i32 windowTop = (GetSystemMetrics(SM_CYSCREEN) - m_data->m_fullWindowHeight) / 2;
		...
		GetClientRect(m_data->m_hWnd, &clientRect);
		i32 w = clientRect.right - clientRect.left;
		i32 h = clientRect.bottom - clientRect.top;
...
void Win32Window::switchFullScreen(bool fullscreen, i32 width, i32 height, i32 colorBitsPerPixel)
{
	LONG res;
	DEVMODE dm;
	memset(&dm, 0, sizeof(dm));
	...
	if (fullscreen)
	{
		...
		SetWindowPos(m_data->m_hWnd, NULL, 0, 0, (i32)width, (i32)height,
...
void Win32Window::getMouseCoordinates(i32& x, i32& y)
{
...
void Win32Window::setDebugMessage(i32 x, i32 y, tukk message)
{
}
