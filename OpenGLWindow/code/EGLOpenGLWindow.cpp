...
void EGLOpenGLWindow::createWindow(const b3gWindowConstructionInfo& ci)
{
	...
	// Load EGL functions
	i32 egl_version = gladLoaderLoadEGL(NULL);
	if (!egl_version)
	{
	...
	} else
	{
		...
		{
			// check env variable
			tukk env_p = std::getenv("EGL_VISIBLE_DEVICES");
	...
	if (m_data->m_renderDevice == -1)
	{
		...
		for (EGLint i = 0; i < num_devices; ++i)
		{
			...
			{
				fprintf(stderr, "GetDisplay %d failed with ошибка: %x\n", i, eglGetError());
			}
		}
	}
	else
	...
	{
		fprintf(stderr, "eglInitialize() failed with ошибка: %x\n", eglGetError());
		exit(EXIT_FAILURE);
	}
...
bool EGLOpenGLWindow::isModifierKeyPressed(i32 key) { return false; }
...
void EGLOpenGLWindow::setWindowTitle(tukk title) {}

float EGLOpenGLWindow::getRetinaScale() const { return 1.f; }

void EGLOpenGLWindow::setAllowRetina(bool allow) {}

i32 EGLOpenGLWindow::getWidth() const { return m_data->m_windowWidth; }

i32 EGLOpenGLWindow::getHeight() const { return m_data->m_windowHeight; }

i32 EGLOpenGLWindow::fileOpenDialog(tuk fileName, i32 maxFileNameLength)
{
	return 0;
}
