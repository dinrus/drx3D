...
static void GLFWErrorCallback(i32 error, tukk description)
{
	fprintf(stderr, "Ошибка: %s\n", description);
}

static void GLFWMouseButtonCallback(GLFWwindow* window, i32 button, i32 glfwState, i32)
{
	GLFWOpenGLWindow* wnd = (GLFWOpenGLWindow*)glfwGetWindowUserPointer(window);
	if (wnd && wnd->getMouseButtonCallback())
	{
		i32 state = (glfwState == GLFW_PRESS) ? 1 : 0;
...
static void GLFWSizeCallback(GLFWwindow* window, i32 width, i32 height)
{
	GLFWOpenGLWindow* wnd = (GLFWOpenGLWindow*)glfwGetWindowUserPointer(window);
	{
...
i32 getBulletKeyFromGLFWKeycode(i32 glfwKeyCode)
{
	i32 keycode = -1;
...
void GLFWOpenGLWindow::keyboardCallbackInternal(i32 key, i32 state)
{
	if (getKeyboardCallback())
	{
		//convert keyboard codes from glfw to bullet
		i32 d3code = getBulletKeyFromGLFWKeycode(key);
		i32 d3state = (state == GLFW_RELEASE) ? 0 : 1;
		...
		getKeyboardCallback()(d3code, d3state);
	}
}

void GLFWOpenGLWindow::mouseButtonCallbackInternal(i32 button, i32 state)
{
...
void GLFWOpenGLWindow::resizeInternal(i32 width, i32 height)
{
...
void GLFWOpenGLWindow::createDefaultWindow(i32 width, i32 height, tukk title)
{
...
bool GLFWOpenGLWindow::isModifierKeyPressed(i32 key)
{
	bool result = false;
...
void GLFWOpenGLWindow::setWindowTitle(tukk title)
{
	if (m_data->m_glfwWindow)
	{
...
i32 GLFWOpenGLWindow::getWidth() const
{
	if (m_data->m_glfwWindow)
	{
		glfwGetFramebufferSize(m_data->m_glfwWindow, &m_data->m_width, &m_data->m_height);
	}
	i32 width = m_data->m_width / m_data->m_retinaScaleFactor;
	return width;
}
i32 GLFWOpenGLWindow::getHeight() const
{
	if (m_data->m_glfwWindow)
	{
		glfwGetFramebufferSize(m_data->m_glfwWindow, &m_data->m_width, &m_data->m_height);
...
i32 GLFWOpenGLWindow::fileOpenDialog(tuk fileName, i32 maxFileNameLength)
{
	return 0;
}
