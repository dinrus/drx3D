...
//#define GLAD_GL 1
#include <X/glad/glad/gl.h>

#ifdef GLEW_DYNAMIC_LOAD_ALL_GLX_FUNCTIONS
#include <X/glad/glad/glx.h>
#else
#include <GL/glx.h>
...
#ifdef DYNAMIC_LOAD_X11_FUNCTIONS
typedef i32 (*XErrorHandler) (Display*, XErrorEvent*);
///our X11 function typedefs

typedef i32 (*PFNXFREE)(uk );
typedef XErrorHandler (*PFNXSETERRORHANDLER)(XErrorHandler);
typedef i32 (*PFNXSYNC)(Display* a, Bool b);
typedef Display* (*PFNXOPENDISPLAY)(_Xconst tuk a);
typedef Colormap (*PFNXCREATECOLORMAP)(Display* a, Window b, Visual* c, i32 d);
typedef Window (*PFNXCREATEWINDOW)(Display* a, Window b, i32 c, i32 d, u32 e, u32 f, u32 g, i32 h, u32 i, Visual* j, u64 k, XSetWindowAttributes* l);
typedef i32 (*PFNXMAPWINDOW)(Display*, Window);
typedef i32 (*PFNXSTORENAME)(Display* a, Window b, _Xconst char* c);
typedef i32 (*PFNXCLOSEDISPLAY)(Display* a);
typedef i32 (*PFNXDESTROYWINDOW)(Display* a, Window b);
typedef i32 (*PFNXRAISEWINDOW)(Display* a, Window b);

#if NeedWidePrototypes
typedef KeySym* (*PFNXGETKEYBOARDMAPPING)(Display*, u32, i32, i32*);
typedef KeySym (*PFNXKEYCODETOKEYSYM)(Display* a, u32 b, i32 c);
#else
typedef KeySym* (*PFNXGETKEYBOARDMAPPING)(Display*, KeyCode, i32, i32*);
typedef KeySym (*PFNXKEYCODETOKEYSYM)(Display* a, KeyCode b, i32 c);
#endif
typedef void (*PFNXCONVERTCASE)(KeySym /* sym */, KeySym* /* lower */, KeySym* /* upper */);
typedef i32 (*PFNXPENDING)(Display* a);
typedef i32 (*PFNXNEXTEVENT)(Display* a, XEvent* b);
typedef i32 (*PFNXEVENTSQUEUED)(Display* a, i32 b);
typedef i32 (*PFNXPEEKEVENT)(Display* a, XEvent* b);
typedef KeySym (*PFNXLOOKUPKEYSYM)(XKeyEvent* a, i32 b);
...
struct InternalData2
{
		  ...
		  m_keyboardCallback(0)
	{
		...
		if (!m_x11_library)
		{
			...
			fprintf(stderr, "Ошибка при открытии библиотеки X11 %s: %s\n", X11_LIBRARY, dlerror());
			exit(EXIT_FAILURE);
		}
		...
		if (missingFunc)
		{
			fprintf(stderr, "Ошибка: отсутствует функция XFree в %s, прерывание!\n", X11_LIBRARY);
			exit(EXIT_FAILURE);
		}
		missingFunc = ((m_x11_XSetErrorHandler = (PFNXSETERRORHANDLER)dlsym(m_x11_library, "XSetErrorHandler")) == NULL) | missingFunc;
		if (missingFunc)
		{
			fprintf(stderr, "Ошибка: отсутствует функция XSetErrorHandler в %s, прерывание!\n", X11_LIBRARY);
			exit(EXIT_FAILURE);
		}
		missingFunc = ((m_x11_XSetErrorHandler = (PFNXSETERRORHANDLER)dlsym(m_x11_library, "XSetErrorHandler")) == NULL) | missingFunc;
		if (missingFunc)
...
typedef GLXContext (*glXCreateContextAttribsARBProc)(Display*, GLXFBConfig, GLXContext, Bool, i32k*);

// Helper to check for extension string presence.  Adapted from:
//   http://www.opengl.org/resources/features/OGLextensions/
static bool isExtensionSupported(tukk extList, tukk extension)
{
	tukk start;
	tukk where, terminator;
...
static i32 ctxErrorHandler(Display* dpy, XErrorEvent* ev)
{
...
void X11OpenGLWindow::enableOpenGL()
{
	if (forceOpenGL3)
	{
		...
		tukk glxExts = glXQueryExtensionsString(m_data->m_dpy,
		...
		i32 (*oldHandler)(Display*, XErrorEvent*) =
		...
		{
			printf(
				"glXCreateContextAttribsARB() не найден"
				" ... используется контекст GLX в старом стиле\n");
...
*/
			...
			else
			{
				...
				printf(
					"Не удалось создать контекст GL 3.3"
					" ... используется контекст GLX в старом стиле\n");
				ctx = glXCreateContextAttribsARB(m_data->m_dpy, m_data->m_bestFbc, 0,
												 True, context_attribs);
			}
		...
		if (ctxErrorOccurred || !ctx)
		{
			fprintf(stderr, "Не удалось создать контекст OpenGL\n");
			exit(1);
		}
		...
		{
			printf("Получен косвенный контекст отображения GLX\n");
		}
		else
		{
			printf("Получен прямой контекст отображения GLX\n");
		}

		printf("Контекст сделан текущим\n");
...
void X11OpenGLWindow::createWindow(const b3gWindowConstructionInfo& ci)
{
	...
	{
		fprintf(stderr, "\n\tне удаётся подключиться к X server\n\n");
		exit(EXIT_FAILURE);
	}
	...
	i32 res = gladLoaderLoadGLX(m_data->m_dpy, DefaultScreen(m_data->m_dpy));
	if (!res)
	{
		printf("Ошибка в gladLoadGLX\n");
		exit(0);
	}
	...
	if (forceOpenGL3)
	{
		...
		if (!fbc)
		{
			fprintf(stderr, "Не удалось получить конфигурацию буфера кадров\n");
			exit(1);
		}
///don't use highest samples, it is really slow on some NVIDIA Quadro cards
		...
		if (!m_data->m_win)
		{
			fprintf(stderr, "Не удаётся создать окно\n");
			exit(EXIT_FAILURE);
		}
	...
	else
	{
		...
		if (m_data->m_vi == NULL)
		{
			fprintf(stderr, "\n\tне найдено подходящего визуала\n\n");
			exit(EXIT_FAILURE);
		}
		else
		{
			printf("\n\tвыбран визуал %p\n", (uk )m_data->m_vi->visualid); /* %p creates hexadecimal output like in glxinfo */
		}
...
i32 X11OpenGLWindow::getAsciiCodeFromVirtualKeycode(i32 keycode)
{
	i32 result = 0;

	KeySym key, key_lc, key_uc;

	i32 keysyms_per_keycode_return;
...
bool X11OpenGLWindow::isModifierKeyPressed(i32 key)
{
...
void X11OpenGLWindow::pumpMessage()
{
	...
	while (MyXPending(m_data->m_dpy))
	{
		...
		switch (m_data->m_xev.type)
		{
			case KeyPress:
			{
				i32 keycode = getAsciiCodeFromVirtualKeycode(m_data->m_xev.xkey.keycode);
				switch (keycode)
				{
					case B3G_ALT:
			...
			case ButtonPress:
			{
				...
				i32 xpos = m_data->m_xev.xmotion.x;
				i32 ypos = m_data->m_xev.xmotion.y;
...
void X11OpenGLWindow::setWindowTitle(tukk title)
{
//	m_data->  ->m_win = title;
}
...
i32 X11OpenGLWindow::getWidth() const
{
	if (m_data)
		return m_data->m_glWidth;
	return 0;
}
i32 X11OpenGLWindow::getHeight() const
{
	if (m_data)
...
i32 X11OpenGLWindow::fileOpenDialog(tuk filename, i32 maxNameLength)
{
	i32 len = 0;
	...
	{
		printf("Ошибка: fileOpenDialog no popen output, perhaps install zenity?\n");
	}
