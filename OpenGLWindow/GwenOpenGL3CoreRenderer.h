...
#include <X/Gwen/BaseRender.h>
#include "GLPrimitiveRenderer.h"
#include "../OpenGLWindow/OpenGLInclude.h"

struct sth_stash;
#include "fontstash.h"
#include <X/Gwen/Texture.h>
...
public:
	...
	{
		//		DRX3D_PROFILE("GWEN_DrawFilledRect");
		Translate(rect);
	...
	void RenderWText(Gwen::Font* pFont, Gwen::Point rasterPos, const Gwen::WTxt& text)
	{
		tukk unicodeText = ~text.ToTxt();
		...
		else
		{
			...
			{
				i32 c = unicodeText[pos];
				r.h = m_currentFont->m_CharHeight;
				r.w = m_currentFont->m_CharWidth[c] + extraSpacing;
				Gwen::Rect rect = r;
				Translate(rect);

				m_primitiveRenderer->drawTexturedRect2(rect.x, rect.y + m_yOffset, rect.x +
				rect.w, rect.y + rect.h + m_yOffset, m_currentColor, m_currentFont->m_CharU0[c],
				                          m_currentFont->m_CharV0[c], m_currentFont->m_CharU1[c],
				                                                   m_currentFont->m_CharV1[c]);
	...
	Gwen::Point MeasureWText(Gwen::Font* pFont, const Gwen::WTxt& text)
	{
		...
		else
		{
			...
			while (unicodeText[pos])
			{
				width += m_currentFont->m_CharWidth[(i32)unicodeText[pos]] + extraSpacing;
				pos++;
			}
			Gwen::Point pt;
			i32 fontHeight = m_currentFont->m_CharHeight;
		...
		return Gwen::Renderer::Base::MeasureWText(pFont, text);
	}
	...
	{
		//		DRX3D_PROFILE("DrawTexturedRect");
		Translate(rect);
...
*/
		...
		m_primitiveRenderer->drawTexturedRect(rect.x, rect.y + m_yOffset, rect.x + rect.w,
