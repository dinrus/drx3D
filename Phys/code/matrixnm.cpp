...
i32 matrix_tpl<ftype>::jacobi_transformation(matrix_tpl<ftype> &evec, ftype *eval, ftype prec) const
{
	if (!(flags & mtx_symmetric) || nCols!=nRows) return 0;

	matrix_tpl a(*this);
	i32 n = nRows, p,q,r,iter,pmax,qmax,sz=nRows*nCols;
...
template<class ftype>
i32 matrix_tpl<ftype>::conjugate_gradient(ftype *startx,ftype *rightside, ftype minlen,ftype minel) const
{
...
template<class ftype>
i32 matrix_tpl<ftype>::biconjugate_gradient(ftype *startx,ftype *rightside, ftype minlen,ftype minel) const
{
...
template<class ftype>
i32 matrix_tpl<ftype>::minimum_residual(ftype *startx,ftype *rightside, ftype minlen,ftype minel) const
{
	ftype a,b,r2,r2new,err,denom,maxel;
	i32 i,iter=nRows*3;
	minlen*=minlen;
...
template<class ftype>
i32 matrix_tpl<ftype>::LUdecomposition(ftype *&LUdata,i32 *&LUidx) const
{
	if (nRows!=nCols) return 0;
	i32 i,j,k,imax,alloc=((LUdata==0) | (LUidx==0)<<1);
	ftype aij,bij,maxaij,t;
	if (alloc & 1) LUdata = new ftype[nRows*nRows];
	if (alloc & 2) LUidx = new i32[nRows];
...
template<class ftype>
i32 matrix_tpl<ftype>::solveAx_b(ftype *x,ftype *b, ftype *LUdata,i32 *LUidx) const
{
	i32 LUidx_buf[16],alloc=0;
  if (!LUdata) {
    if (nRows*nRows*2<DRX_ARRAY_COUNT(mtx_pool)) {
      if (mtx_pool_pos+nRows*nRows > DRX_ARRAY_COUNT(mtx_pool))
        mtx_pool_pos = 0;
      LUdata = mtx_pool+mtx_pool_pos; mtx_pool_pos += nRows*nRows;
    }
    if (nRows<=DRX_ARRAY_COUNT(LUidx_buf)) LUidx = LUidx_buf;
...
matrix_tpl<ftype>& matrix_tpl<ftype>::invert()
{
	...
	if (nRows==1)
		...
		if (det==0) goto done;	det = (ftype)1.0/det;
	    ...
	    ftype *LUdata=0,*colmark; i32 *LUidx=0,LUidx_buf[32], alloc=0;
	    if (nRows*nRows*2<DRX_ARRAY_COUNT(mtx_pool)) {
	      if (mtx_pool_pos+nRows*nRows > DRX_ARRAY_COUNT(mtx_pool))
	        mtx_pool_pos = 0;
	      LUdata = mtx_pool+mtx_pool_pos; mtx_pool_pos += nRows*nRows;
	    ...
	    if (!LUdecomposition(LUdata,LUidx)) goto done;

	    if (nRows*2<DRX_ARRAY_COUNT(mtx_pool)) {
	      if (mtx_pool_pos+nRows > DRX_ARRAY_COUNT(mtx_pool))
	        mtx_pool_pos=0;
...
template<class ftype>
ftype matrix_tpl<ftype>::determinant(ftype *LUdata,i32 *LUidx) const
{
	if (nRows!=nCols) return 0;
	i32 i,j; ftype det=0;
	if (nRows==1)
...
template<class ftype>
i32 matrix_tpl<ftype>::LPsimplex(i32 m1,i32 m2, ftype &objfunout,ftype *xout, i32 nvars, ftype e) const
{
	i32 M=nRows-2,N=nCols-1,i,j,imax,jmax,iobjfun=M, res=0,iter=(M+N)*8;
	ftype rpivot,t;
	i32k imask=0x7FFFFFFF;
	i32 *irow=new i32[M],*icol=new i32[N];
	#define a (*this)
...
{
	matrixf mtxf; float f,*pf; i32 *pi;
