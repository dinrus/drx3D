...
void CVoxelBV::GetNodeBV(BV *&pBV,i32 iNode,i32 iCaller)
{
	pBV = &g_BVvox;
...
void CVoxelBV::GetNodeBV(const Matrix33 &Rw,const Vec3 &offsw,float scalew, BV *&pBV, i32 iNode, i32 iCaller)
{
	pBV = &g_BVvox;
	g_BVvox.voxgrid.origin = m_pgrid->origin + Vec3(m_iBBox[0].x*m_pgrid->step.x, m_iBBox[0].y*m_pgrid->step.y, m_iBBox[0].z*m_pgrid->step.z);
...
void CVoxelBV::MarkUsedTriangle(i32 itri, geometry_under_test *pGTest)
{
...
i32 CVoxelBV::GetNodeContents(i32 iNode, BV *pBVCollider,i32 bColliderUsed,i32 bColliderLocal, 
															geometry_under_test *pGTest,geometry_under_test *pGTestOp)
{
	i32 i,icell,nPrims=0,nTris=0,nTrisDst;
	Vec3i iBBox[2],ic;
	indexed_triangle *ptri;
	i32k MAXTESTRIS = 256;
	i32 idxbuf[MAXTESTRIS*2], *plist=idxbuf, *plistDst=idxbuf+MAXTESTRIS;
	intptr_t idmask = ~iszero_mask(m_pMesh->m_pIds);
	char idnull=(char)-1, *ptrnull=&idnull, *pIds = (tuk)((intptr_t)m_pMesh->m_pIds&idmask | (intptr_t)ptrnull&~idmask);
	...
	for(i=0;i<nTris;i++) if (!(bColliderUsed & pGTest->pUsedNodesMap[plist[i]>>5]>>(plist[i]&31) & 1)) {
		ptri = (indexed_triangle*)((tuk)pGTest->primbuf+nPrims*pGTest->szprim);
...
i32 CVoxelBV::PrepareForIntersectionTest(geometry_under_test *pGTest, CGeometry *pCollider,geometry_under_test *pGTestColl)
{
	i32 mapsz = (m_pMesh->m_nTris-1>>5)+1;
