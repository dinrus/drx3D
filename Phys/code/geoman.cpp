...
IGeometry *CGeomUpr::CreatePrimitive(i32 type, const primitive *pprim)
{
	IGeometry *pRes = 0;
	switch (type) {
...
i32 CGeomUpr::AddRefGeometry(phys_geometry *pgeom)
{
...
{ 
	if (DrxInterlockedDecrement(&m_nRefCount)<=0){ 
		if(g_pPhysWorlds[0]) {
			CGeomUpr *pGeoman = (CGeomUpr*)((IPhysicalWorld*)g_pPhysWorlds[0])->GetGeomUpr();
			pGeoman->m_sizeExtGeoms-=GetSizeFast(); 
		}	delete this;
	} 
}

void CGeomUpr::FlushOldGeoms()
{
...
phys_geometry *CGeomUpr::GetFreeGeomSlot()
{
	i32 i,j;
...
IGeometry *CGeomUpr::LoadGeometry(CMemStream &stm, strided_pointer<const Vec3> pVertices, 
																			strided_pointer<unsigned short> pIndices, char *pIds)
{
	i32 itype; stm.Read(itype);
	IGeometry *pGeom;
	switch (itype) {
...
phys_geometry *CGeomUpr::LoadPhysGeometry(CMemStream &stm, strided_pointer<const Vec3> pVertices, 
																							strided_pointer<unsigned short> pIndices, char *pIds)
{
...
/*#if SIZEOF_PTR != 4
	StructUnpack(
		...
		functor(
		  stm,
		  reinterpret_cast<void (CMemStream::*)(uk , u32)>(
			  &CMemStream::ReadRaw)
		),
		offsetof(phys_geometry, pForeignData));
#else
	stm.ReadRaw(pgeom, (tuk)&pgeom->pForeignData-(tuk)pgeom);
	SwapEndian(*pgeom);
#endif*/
	stm.Read(pgs);
	//SwapEndian(pgs);
...
IGeometry *CGeomUpr::CloneGeometry(IGeometry *pGeom)
{
	IGeometry *pClone=0;
...
ITetrLattice *CGeomUpr::CreateTetrLattice(const Vec3 *pt,i32 npt, i32k *pTets,i32 nTets)
{
	return (new CTetrLattice(GetIWorld()))->CreateLattice(pt,npt,pTets,nTets);
}


i32 CGeomUpr::RegisterCrack(IGeometry *pGeom, Vec3 *pVtx, i32 idmat)
{
	i32 i0;
	float edgelen[3];
	Vec3 edge,pt0,n;
...
void CGeomUpr::UnregisterCrack(i32 id)
{
	i32 i;
...
IGeometry *CGeomUpr::GetCrackGeom(const Vec3 *pt,i32 idmat, geom_world_data *pgwd)
{
...
IBreakableGrid2d *CGeomUpr::GenerateBreakableGrid(Vec2 *ptsrc,i32 npt, const Vec2i &nCells, i32 bStaticBorder, i32 seed)
{	
