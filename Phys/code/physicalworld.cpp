...
#include <drx3D/Phys/spheregeom.h>
...
#include <drx3D/Phys/rigidentity.h>
#include <drx3D/Phys/walkingrigidentity.h>
#include <drx3D/Phys/particleentity.h>
#include <drx3D/Phys/livingentity.h>
#include <drx3D/Phys/wheeledvehicleentity.h>
#include <drx3D/Phys/articulatedentity.h>
#include <drx3D/Phys/ropeentity.h>
#include <drx3D/Phys/softentity.h>
#include <drx3D/Phys/tetrlattice.h>
#include <drx3D/Phys/physicalworld.h>
#include <drx3D/Phys/waterman.h>
#include <drx3D/CoreX/Platform/DrxWindows.h>

i32 g_dummyBuf[16];

//  предварительные объявления of memory functions to access static data
namespace TriMeshStaticData{ void GetMemoryUsage( IDrxSizer *pSizer ); }
...
IPhysicalEntity* CPhysicalWorld::CPhysicalEntityIt::Next()
{
  if (IsEnd())
	...
	if (m_pCurEntity->m_next)
		...
		i32 i; for(i=m_pCurEntity->m_iSimClass+1; i<8 && !m_pWorld->m_pTypedEnts[i]; i++);
...
{
	i32 i; for(i=0;i<8 && !m_pWorld->m_pTypedEnts[i];i++);
	m_pCurEntity = i<8 ? m_pWorld->m_pTypedEnts[i] : m_pWorld->m_pHiddenEnts;
}
...
#else
TLS_DEFINE(i32*, g_pidxPhysThread);

void MarkAsPhysThread() {
	static i32 g_ibufPhysThread[2] = { 1,0 };
	static i32 *g_lastPtr = g_ibufPhysThread;
	i32 *ptr = TLS_GET(i32*, g_pidxPhysThread);
	if (ptr!=g_lastPtr) {
...
void MarkAsPhysWorkerThread(i32 *pidx) {
	TLS_SET(g_pidxPhysThread, pidx);
}
#endif


i32 GetEntityProfileType(CPhysicalEntity *pent)
{
	i32 i = pent->GetType();
...
CPhysicalWorld::CPhysicalWorld(ILog *pLog) : m_nWorkerThreads(0)
{
	...
	g_nPhysWorlds = min(g_nPhysWorlds+1,(i32)(DRX_ARRAY_COUNT(g_pPhysWorlds)));
	m_pEventClient = 0;
	if (g_nPhysWorlds==1)
	...
	i32 i; for(i=0; i<EVENT_TYPES_NUM; i++)
...
CPhysicalWorld::~CPhysicalWorld()
{
	...
	//////////////////////////////////////////////////////////////////////////
	for(i32 i=0; i<EVENT_TYPES_NUM; i++) {
		EventClient *pClient,*pNextClient;
		for (i32 bLogged=0;bLogged<2;bLogged++) {
	...
	memset(g_StaticPhysicalEntity.m_pUsedParts = new i32[MAX_PHYS_THREADS+1][16], 0, sizeof(*g_StaticPhysicalEntity.m_pUsedParts));
	i32 i;
...
void CPhysicalWorld::Init()
{
	...
	m_iTimePhysics = 0;
	i32 i; for(i=0;i<8;i++) { m_pTypedEnts[i]=m_pTypedEntsPerm[i]=0; m_updateTimes[i]=0; }
	m_pHiddenEnts = 0;
...
void CPhysicalWorld::InitGThunksPool()
{
	...
	i32k bytes = 32768*sizeof(pe_gridthunk);
	i32k numPages = (bytes+(64*1024-1)) >> 16;
	i32k nStartingThunks = (numPages<<16)/sizeof(pe_gridthunk);
	AllocGThunksPool(nStartingThunks);
}

static pe_gridthunk* AllocateThunks(i32& nNewSize)
{
	// Round up to multiples of 64k
	size_t bytes = sizeof(pe_gridthunk)*nNewSize;
	size_t numPages = (bytes + 64*1024-1) >> 16;
	nNewSize = (numPages<<16)/sizeof(pe_gridthunk);
	uk memory = malloc(numPages<<16);
...
//////////////////////////////////////////////////////////////////////////
void CPhysicalWorld::AllocGThunksPool( i32 nNewSize )
{
	...
	else
	{
		...
		i32 i; for(i=0;i<m_thunkPoolSz-1;i++) gthunks[i].inextOwned = i+1;
...
void CPhysicalWorld::Shutdown(i32 bDeleteGeometries)
{
	WriteLock lock(m_lockStep);

	for(i32 i=m_nWorkerThreads-1;i>=0;i--) {
		m_threads[i]->bStop=1,m_threadStart[i].Set(),m_threadDone[i].Wait();
		delete m_threads[i]; m_threads[i] = NULL;
	}
	i32 i; CPhysicalEntity *pent,*pent_next;
...
IPhysicalEntity *CPhysicalWorld::SetHeightfieldData(const heightfield *phf, i32 *pMatMapping,i32 nMats)
{
	i32 iCaller;
	CPhysicalEntity *pHF;
	if (!phf) {
	...
	for(iCaller=0; iCaller<=MAX_PHYS_THREADS; iCaller++) {
		...
		if (pMatMapping)
			memcpy(pHF->m_parts[0].pMatMapping = new i32[nMats], pMatMapping,
				(pHF->m_parts[0].nMats=nMats)*sizeof(i32));
		else
...
void CPhysicalWorld::SetHeightfieldMatMapping(i32 *pMatMapping, i32 nMats)
{
	if(!pMatMapping)
		return;
	...
	for(i32 iCaller=0; iCaller<=MAX_PHYS_THREADS; iCaller++)
	{
		...
		{
			memcpy(pHF->m_parts[0].pMatMapping, pMatMapping, nMats*sizeof(i32));
			pHF->m_parts[0].nMats = nMats;
		}
	}
}

void SEntityGrid::Setup(i32 axisz, Vec3 org, i32 nx,i32 ny, float stepx,float stepy, i32 _log2PODscale, i32 _bCyclic)
{
	nx = min(1024,nx);
	ny = min(1024,ny);
	iup = axisz;
...
IPhysicalEntity* CPhysicalWorld::SetupEntityGrid(i32 axisz, Vec3 org, i32 nx,i32 ny, float stepx,float stepy, i32 log2PODscale, i32 bCyclic, IPhysicalEntity* pHost, const QuatT& posInHost)
{
	...
	if (pgrid->cells) {
		...
		if (m_pEntsById) for(i=0;i<m_iNextId;i++) if (m_pEntsById[i] && GetGrid(m_pEntsById[i])==pgrid) {
			...
			if (!m_pEntsById[i]->m_pEntBuddy || m_pEntsById[i]->m_pEntBuddy==m_pEntsById[i]) {
				...
				for(i32 j=0;j<pent->m_nParts;j++) if (pent->m_parts[j].pPlaceholder)
...
i32 SEntityGrid::SetParams(pe_params* _params, i32 bThreadSafe)
{
...
i32 SEntityGrid::GetParams(pe_params* _params) const
{
	if (_params->type==pe_params_outer_entity::type_id) {
...
i32 SEntityGrid::Release()
{
	i32 res = DrxInterlockedDecrement(&m_refCount);
	if (res<=0 && m_pWorld)
...
SEntityGrid* CPhysicalWorld::DestroyGrid(SEntityGrid *pgrid)
{
	...
	for(i32 icell=0; icell<=pgrid->size.x*pgrid->size.y; icell++)
		while (i32 ithunk=pgrid->cells[icell]) {
...
void CPhysicalWorld::DeactivateOnDemandGrid()
{
	...
	for(i32 i=0;i<8;i++) for(CPhysicalEntity *pent=m_pTypedEnts[i]; pent; pent=pent->m_next) if (GetGrid(pent)==&grid) {
...
void SEntityGrid::RegisterBBoxInPODGrid(const Vec3 *BBox, IPhysicsStreamer *pStreamer)
{
	...
	for(ix=igx[0];ix<=igx[1];ix++) for(iy=igy[0];iy<=igy[1];iy++) {
		...
		if (!pPODcells[i]) {
			...
			for(i32 j=0;j<64;j++) pPODcells[i][j].zlim.set(1000.0f,-1000.0f);
...
i32 CPhysicalWorld::AddRefEntInPODGrid(IPhysicalEntity *_pent, const Vec3 *BBox)
{
	WriteLock lock(m_lockPODGrid);
	return GetGrid(_pent)->AddRefEntInPODGrid(_pent,BBox);
}
i32 SEntityGrid::AddRefEntInPODGrid(IPhysicalEntity *_pent, const Vec3 *BBox)
{
	CPhysicalEntity *pent = (CPhysicalEntity*)_pent;
	i32 i,ix,iy,nCells=0;
	Vec2i ig[2];
...
void SEntityGrid::GetPODGridCellBBox(i32 ix,i32 iy, Vec3 &center,Vec3 &size) const
{
...
i32 CPhysicalWorld::SetSurfaceParameters(i32 surface_idx, float bounciness,float friction, u32 flags)
{
	if ((u32)surface_idx>=(u32)NSURFACETYPES)
		return 0;
...
i32 CPhysicalWorld::GetSurfaceParameters(i32 surface_idx, float &bounciness,float &friction, u32 &flags)
{
	if ((u32)surface_idx>=(u32)NSURFACETYPES)
		return 0;
...
i32 CPhysicalWorld::SetSurfaceParameters(i32 surface_idx, float bounciness, float friction,
...
i32 CPhysicalWorld::GetSurfaceParameters(i32 surface_idx, float &bounciness,float &friction,
																				 float &damage_reduction, float &ric_angle, float &ric_dam_reduction,
																				 float &ric_vel_reduction, u32 &flags)
{
	if ((u32)surface_idx>=(u32)NSURFACETYPES)
...
IPhysicalEntity* CPhysicalWorld::CreatePhysicalEntity(pe_type type, float lifeTime, pe_params* params, uk pForeignData,i32 iForeignData,
																											i32 id, IPhysicalEntity *pHostPlaceholder, IGeneralMemoryHeap* pHeap)
{
...
//#endif
	...
	if (!m_lockStep && (isPOD || !m_lockTPR) && lifeTime==0) {
		...
		if (++m_nEnts > m_nEntsAlloc-1) {
			i32 nEntsAllocNew = m_nEntsAlloc+4096;
		...
		AllocRequestsQueue(sizeof(i32)*3+sizeof(uk ));
		QueueData(4);	// RepositionEntity opcode
		QueueData((i32)(sizeof(i32)*3+sizeof(uk )));	// size
		QueueData(res);
		QueueData(2);	// flags
	} else {
...
IPhysicalEntity *CPhysicalWorld::CreatePhysicalPlaceholder(pe_type type, pe_params* params, uk pForeignData,i32 iForeignData, i32 id)
{
	i32 i,j,iChunk;
...
i32 CPhysicalWorld::DestroyPhysicalEntity(IPhysicalEntity* _pent,i32 mode,i32 bThreadSafe)
{
	DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );
	...
	if (_pent->GetType()==PE_GRID) {
		...
		if (pgrid!=&m_entgrid && mode==0 && DrxInterlockedDecrement(&pgrid->m_refCount)<=0) {
	...
	if (idx) {
		...
		i32 i,j,iChunk = idx>>PLACEHOLDER_CHUNK_SZLG2;
	...
	if (mode==2) {
		if (pent->m_iSimClass==-1 && pent->m_iPrevSimClass>=0) {
			...
			i32 locked = RepositionEntity(pent);
			AtomicAdd(&m_lockGrid, -locked);
			if (pent->m_pStructure && pent->m_pStructure->defparts)
				for(i32 i=0;i<pent->m_nParts;i++) if (pent->m_pStructure->defparts[i].pSkinInfo && pent->m_pStructure->defparts[i].pSkelEnt)
	...
	if ((u32)pent->m_iPrevSimClass<8u && pent->m_iSimClass>=0) {
...
DEBUG_BREAK;
	...
	if (mode==0) {
		i32 bWasRegistered = !(pent->m_flags & 0x80000000u);
...
i32 CPhysicalWorld::ReserveEntityCount(i32 nNewEnts)
{
...
void CPhysicalWorld::PatchEventsQueue(IPhysicalEntity* pEntity, uk pForeignData, i32 iForeignData)
{
	WriteLock lock(m_lockEventsQueue);
	EventPhys *pEvent;
	EventPhysMono* pMono;
...
i32 CPhysicalWorld::GetFreeEntId()
{
	i32 nPhysEnts=m_nEnts-m_nExtIds, nPhysSlots=m_iNextId-m_lastExtId;
...
i32 CPhysicalWorld::SetPhysicalEntityId(IPhysicalEntity *_pent, i32 id, i32 bReplace, i32 bThreadSafe)
{
	WriteLockCond lock(m_lockEntIdList,bThreadSafe^1);
	CPhysicalPlaceholder *pent = (CPhysicalPlaceholder*)_pent;
	u32 previd = (u32)pent->m_id;
	if (previd<(u32)m_nIdsAlloc) {
...
i32 CPhysicalWorld::GetPhysicalEntityId(IPhysicalEntity *pent)
{
	return pent==WORLD_ENTITY ? -2 : (pent ? ((CPhysicalEntity*)pent)->m_id : -1);
}

IPhysicalEntity* CPhysicalWorld::GetPhysicalEntityById(i32 id)
{
	...
	if (id==-1)
		...
		i32 bNoExpand = id>>30; id &= ~(1<<30);
		if ((u32)id<(u32)m_nIdsAlloc)
...
static inline void swap(CPhysicalEntity **pentlist,float *pmass,i32 *pids, i32 i1,i32 i2) {
	CPhysicalEntity *pent = pentlist[i1]; pentlist[i1] = pentlist[i2]; pentlist[i2] = pent;
	float m = pmass[i1]; pmass[i1] = pmass[i2]; pmass[i2] = m;
	if (pids) {
		i32 id = pids[i1]; pids[i1] = pids[i2]; pids[i2] = id;
	}
}
static void qsort(CPhysicalEntity **pentlist,float *pmass,i32 *pids, i32 ileft,i32 iright)
{
...
#define prev m_next_coll1
	...
	for(i32 iGroup=-1; pent; pent=pent1) {
		pent1 = pent->next;
...
i32 CPhysicalWorld::ReallocTmpEntList(CPhysicalEntity **&pEntList, i32 iCaller, i32 szNew)
{
...
static ILINE i32 getcell_safe(grid& g, i32 ix, i32 iy, i32 iOutOfBounds)
{
	return (iOutOfBounds & get_entities_out_of_bounds)==0 ? (iy*g.stride.y + ix*g.stride.x) : g.getcell_safe(ix, iy);
}

i32k ent_GEA_recursive = 1<<30;

i32 CPhysicalWorld::GetEntitiesAround(const Vec3 &ptmin,const Vec3 &ptmax, CPhysicalEntity **&pList, i32 objtypes,
																			CPhysicalEntity *pPetitioner, i32 szListPrealloc, i32 iCaller)
{
	...
	i32 nout=0,nout0=0,itypePetitioner;

	i32 maskCaller = 1<<iCaller, maskCaller4 = sqr(sqr(maskCaller));
	EventPhysBBoxOverlap event;
	i32 szList = GetTmpEntList(pTmpEntList, iCaller);
	IF ((szListPrealloc | (objtypes & ent_allocate_list))==0, 1) {
		pList = pTmpEntList;
	}

	if (pPetitioner) {
		itypePetitioner = 1<<pPetitioner->m_iSimClass & -iszero((i32)pPetitioner->m_flags&pef_never_affect_triggers);
	...
	i32k itype = itypePetitioner;
	i32k bAreasOnly = iszero(objtypes-ent_areas);
#ifndef _RELEASE
	m_nGEA[iCaller]++;
#endif
	...
	PHYS_FUNC_PROFILER((tukk)((INT_PTR)"GetEntitiesAround(Physics)"&~mask | (INT_PTR)"GetEntitiesAround(External)"&mask));
#endif

	Vec3 gBBox[2]; grid.BBoxToGrid(ptmin,ptmax, gBBox);
	float zdiff = gBBox[1].z-gBBox[0].z, zsum = gBBox[1].z+gBBox[0].z;
	for(i32 i=0;i<2;i++) gBBox[i] = gBBox[i]*Diag33(grid.stepr.x, grid.stepr.y, grid.rzGran);
	i32 igx[2] = { float2int(gBBox[0].x-0.5f), float2int(gBBox[1].x-0.5f) };
	i32 igy[2] = { float2int(gBBox[0].y-0.5f), float2int(gBBox[1].y-0.5f) };

	if (m_entgrid.iscyclic()) {
	}	else
	IF ((m_vars.iOutOfBounds & get_entities_out_of_bounds)==0, 1) {
		i32 mask1, mask2, mask3, mask4, mask5, mask6, mask7, mask8;
	...
	{ ReadLock lock0(m_lockGrid);
		for(i32 ix=igx0;ix<=igx1;ix++) {
			for(i32 iy=igy0;iy<=igy1;iy++) {
				if ((objtypes & (ent_static|ent_no_ondemand_activation))==ent_static)	{
					...
					if (fabs_tpl((zsum-pPODcell->zlim[1]-pPODcell->zlim[0])*zrange) < (zdiff+zrange)*zrange)	{
						DrxInterlockedAdd(&m_lockGrid,-1);
						ReadLock lockPOD(m_lockPODGrid);
						if (pPODcell->lifeTime<=0) {
							DrxInterlockedAdd(&m_lockPODGrid,-1);
				...
				for(i32 ithunk = grid.cells[getcell_safe(grid, ix, iy, m_vars.iOutOfBounds)], iObjTypesValid = objtypes & 1<<m_gthunks[ithunk].iSimClass, ithunk_next = 0; ithunk && iObjTypesValid|bAreasOnly^1;
					ithunk=ithunk_next, iObjTypesValid = objtypes & 1<<m_gthunks[ithunk].iSimClass)
				{
					ithunk_next = m_gthunks[ithunk].inext;
					i32 iz0 = m_gthunks[ithunk].BBoxZ0, iz1 = m_gthunks[ithunk].BBoxZ1;
					iz1 += isneg(iz1-iz0)<<16;
					i32 diz = float2int((gBBox[0].z+gBBox[1].z-iz0-iz1)*(0.5f/65536))*65536;
					if (iObjTypesValid &&
						...
						!(m_gthunks[ithunk].pent->m_bProcessed & maskCaller))
					{
						...
						if (bContact) {
							...
							if ((u32)(pGridEnt->m_iSimClass-5)>1u) {
								m_bGridThunksChanged = 0;
								...
								if (!pGridEnt->m_pEntBuddy || pent->m_pEntBuddy==pGridEnt) {
									...
									bProcessed = iszero(m_bUpdateOnlyFlagged & ((i32)pent->m_flags^pef_update)) | iszero(pent->m_iSimClass);
								...
								} else if ((m_bUpdateOnlyFlagged & ((i32)pent->m_flags^pef_update) & -pent->m_iSimClass>>31)==0) {
									bProcessed = isneg(-(i32)(pent->m_bProcessed & maskCaller));
									AtomicAdd(&pent->m_lockUpdate, bProcessed^1);
									 char *pw=( tuk)&pent->m_lockUpdate+(1+eBigEndian); for(;*pw;); // ReadLock(m_lockUpdate)
									...
									i32 nUsedParts = pent->m_nUsedParts>>iCaller*4 & 15;
									i32 notFull = nUsedParts+1>>4 ^ 1;
	...
	for(i32 i=(objtypes & ent_GEA_recursive);i<nout;i++)	{
		AtomicAdd(&(pTmpEntList[i]->m_pEntBuddy ? pTmpEntList[i]->m_pEntBuddy:pTmpEntList[i])->m_bProcessed,-maskCaller);
		i32 j,nUsedParts = pTmpEntList[i]->m_nUsedParts>>iCaller*4 & 15;
		nUsedParts &= ~-iszero(pTmpEntList[i]->m_iSimClass-5);
		if (nUsedParts==15)	for(j=0;j<pTmpEntList[i]->m_nParts;j++)
			AtomicAdd(&pTmpEntList[i]->m_parts[j].pPlaceholder->m_bProcessed,-(i32)(pTmpEntList[i]->m_parts[j].pPlaceholder->m_bProcessed & maskCaller));
	...
	if (objtypes & ent_sort_by_mass) {
		for(i32 i=0;i<nout;i++) m_pMassList[i] = pTmpEntList[i]->GetMassInv();
		...
		i32 ilast;
		for(i32 i=ilast=nout-1; i>0; i--) if (m_pMassList[i]==0) {
			if (i!=ilast)
	...
	if (szListPrealloc<nout) {
		if (!(objtypes & ent_allocate_list))
			...
			for(i32 i=0;i<nout;i++) pList[i] = pTmpEntList[i];
		}
	}	else
		for(i32 i=nout0;i<nout;i++) pList[i] = pTmpEntList[i];

	if (objtypes & ent_addref_results)
		for(i32 i=0;i<nout;i++) pList[i]->AddRef();
...
void CPhysicalWorld::ProcessIslandSolverResults(i32 i, i32 iter, float groupTimeStep,float Ebefore, i32 nEnts,float fixedDamping, i32 &bAllGroupsFinished,
																								entity_contact **pContacts,i32 nContacts,i32 nBodies, i32 iCaller,int64 iticks0)
{
	i32 i1,j,bGroupFinished,nBrokenParts,nParts0,idCurGroup=m_pGroupIds[i];
	...
	// process deforming (breaking) enities of this group
	{
		...
		for(i1=j=nBrokenParts=0; i1<m_nDeformingEnts; i1++) if (m_pDeformingEnts[i1]->m_iGroup==idCurGroup) {
			if ((nParts0=m_pDeformingEnts[i1]->m_nParts) && m_pDeformingEnts[i1]->m_iSimClass!=7) {
				...
				nBrokenParts += -iszero((i32)m_pDeformingEnts[i1]->m_flags & aef_recorded_physics) & nParts0-m_pDeformingEnts[i1]->m_nParts;
			} else
				m_pDeformingEnts[i1]->m_flags &= ~pef_deforming;
...
i32 CPhysicalWorld::ReadDelayedSolverResults(CMemStream &stm, float &dt,float &Ebefore,i32 &nEnts,float &fixedDamping,
																						 entity_contact **pContacts,RigidBody **pBodies)
{
	i32 iCaller = get_iCaller_int();
	i32 iGroup,nContacts,nBodies;
	...
	stm.Read(nContacts); stm.ReadRaw(pContacts, sizeof(uk )*nContacts);
	stm.Read(nBodies); stm.ReadRaw(pBodies, sizeof(uk )*nBodies);
	return iGroup;
}

void CPhysicalWorld::ProcessNextEntityIsland(float time_interval, i32 ipass, i32 iter, i32 &bAllGroupsFinished, i32 iCaller)
{
	i32 i,j,n,nEnts,nAnimatedObjects,nBodies,bStepValid,bGroupInvisible;
		...
		for(phead=m_pTmpEntList1[i],bGroupInvisible=pef_invisible,maxGroupFriction=100.0f; phead; phead=phead->m_next_coll)	{
			...
			for(j=0,n=phead->GetColliders(pentlist); j<n; j++) if (pentlist[j]->m_iSimClass>1 && pentlist[j]->GetMassInv()<=0) {
				if (ipass) {
					...
					if (pentlist[j]->GetType()==PE_ARTICULATED)	for(i32 ibody=0; ibody<pentlist[j]->m_nParts; ibody++) {
...
void CPhysicalWorld::ProcessNextEngagedIndependentEntity(i32 iCaller)
{
	...
	do {
		...
		do {
			...
			m_threadData[iCaller].bGroupInvisible = isneg(-((i32)pent->m_flags & pef_invisible));
			pent->m_flags &= ~pef_step_requested;
			float dtFull=pent->m_timeIdle, dt;
			for(i32 iter=0; !pent->Step(dt=pent->GetMaxTimeStep(dtFull)) && ++iter<m_vars.nMaxSubsteps && dtFull>0.001f; )
				dtFull -= dt;
			pent->m_bMoved = 0;
			if (pent==pentEnd)
				break;
...
void CPhysicalWorld::ProcessNextLivingEntity(float time_interval, i32 bSkipFlagged, i32 iCaller)
{
...
void CPhysicalWorld::ProcessNextIndependentEntity(float time_interval, i32 bSkipFlagged, i32 iCaller)
{
	CPhysicalEntity *pent,*pentEnd;
	i32 iter;
	do {
...
void CPhysicalWorld::ThreadProc(i32 ithread, SPhysTask *pTask)
{
	if (pTask->bStop)
		return;
...
i32 __curstep = 0; // debug

void CPhysicalWorld::TimeStep(float time_interval, i32 flags)
{
	DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );

	float m,/*m_groupTimeStep,*/time_interval_org = time_interval;
	CPhysicalEntity *pent,*phead,*ptail,**pentlist,*pent_next,*pent1,*pentmax;
	i32 i,i1,j,n,iter,ipass,nGroups,bHeadAdded,bAllGroupsFinished,bSkipFlagged;
	...
	if (m_vars.numThreads!=m_nWorkerThreads+FIRST_WORKER_THREAD) {
		...
		for(i=0;i<m_vars.numThreads-FIRST_WORKER_THREAD;i++) {
			...
			if (!gEnv->pThreadUpr->SpawnThread(m_threads[i], "PhysicsWorkerThread_%u", i)) {
				DrxFatalError("Error spawning \"PhysicsWorkerThread_%u\" thread.", i);
			}
		}
    ...
    WriteLock lock1(m_lockCaller[MAX_PHYS_THREADS]),lock2(m_lockStep);
		...
		if (time_interval>0 && !(flags & ent_flagged_only))
			MarkAsPhysThread();
		phys_geometry *pgeom;
		for(i=0; i<nQueueSlots; i++) for(j=0; (iter=*(i32*)(pQueueSlots[i]+j))!=-1; j+=*(i32*)(pQueueSlots[i]+j+sizeof(i32))) {
			if (iter<0)
				continue;
			pent = *(CPhysicalEntity**)(pQueueSlots[i]+j+sizeof(i32)*2);
			if (!(pent->m_iSimClass==7 || pent->m_iSimClass==5 && pent->m_iDeletionTime==2))	switch (iter) {
				case 0: pent->SetParams((pe_params*)(pQueueSlots[i]+j+sizeof(i32)*2+sizeof(uk )),1); break;
				case 1: pent->Action((pe_action*)(pQueueSlots[i]+j+sizeof(i32)*2+sizeof(uk )),1); break;
				case 2: pent->AddGeometry(pgeom=*(phys_geometry**)(pQueueSlots[i]+j+sizeof(i32)*2+sizeof(uk )),
																	(pe_geomparams*)(pQueueSlots[i]+j+sizeof(i32)*3+sizeof(uk )*2),
																	*(i32*)(pQueueSlots[i]+j+sizeof(i32)*2+sizeof(uk )*2),1);
								AtomicAdd(&pgeom->nRefCount, -1);	break;
				case 3: pent->RemoveGeometry(*(i32*)(pQueueSlots[i]+j+sizeof(i32)*2+sizeof(uk )*2),1); break;
				case 4:
					pent->m_flags &= ~0x80000000u;
					i1 = RepositionEntity(pent,*(i32*)(pQueueSlots[i]+j+sizeof(i32)*2+sizeof(uk )),0,1);
				...
				case 5:
					DestroyPhysicalEntity((IPhysicalEntity*)pent, *(i32*)(pQueueSlots[i]+j+sizeof(i32)*2+sizeof(uk )),1);
			}
			if (iter!=4)
				AtomicAdd(&pent->m_bProcessed,-PENT_QUEUED);
		}
		if (nQueueSlots) {
			m_nQueueSlotsAux=1; m_nQueueSlotSizeAux=0; *(i32*)m_pQueueSlotsAux[0]=-1;
		}
		m_grpProfileData[14].nTicksLast = DrxGetTicks()-timer;
	}
	{
	...
	if (m_vars.bProfileGroups) {
		...
		m_grpProfileData[15].nTicksLast = DrxGetTicks()-timer;
	}
	} // m_lockStep
...
static i32 s_numThunksAllocated = 0;

NO_INLINE void TrackThunkUsageAlloc(i32 thunk)
{
	s_numThunksAllocated++;
	if (DrxGetIMemReplay()->MemStatEnterAlloc())
		DrxGetIMemReplay()->MemStatLeaveAlloc((uk )(thunk + 0xf0000000), sizeof(pe_gridthunk));
}

NO_INLINE void TrackThunkUsageFree(i32 thunk)
{
	s_numThunksAllocated--;
	if (DrxGetIMemReplay()->MemStatEnterFree())
		DrxGetIMemReplay()->MemStatLeaveFree((uk )(thunk + 0xf0000000));
}
...
void CPhysicalWorld::DetachEntityGridThunks(CPhysicalPlaceholder *pobj)
{
	if (pobj->m_iGThunk0) {
		...
		i32 ithunk,ithunk_next,ithunk_last,icell,iprev,inext;
...
#endif
			m_gthunks[inext].iprev = iprev & -(i32)inext>>31;
...
void CPhysicalWorld::SortThunks()
{
	...
	i32 i,j,icell,nthunks=1;
	i32 *new2old=new i32[m_thunkPoolSz], *old2new=new i32[m_thunkPoolSz];
	SEntityGrid *pgrid;
	...
	for(i=0; i<nthunks; i++) {
		if (m_gthunks[i].pent) {
			...
			if (m_gthunks[i].bFirstInCell) {
				...
				icell = Vec2i(max(0,min(pgrid->size.x-1,(i32)m_gthunks[i].iprev&1023)), max(0,min(pgrid->size.y-1,(i32)m_gthunks[i].iprev>>10&1023)))*pgrid->stride;
				if (pgrid->cells[(u32)icell]!=i)
					icell = pgrid->size.x*pgrid->size.y;
				pgrid->cells[(u32)icell] = old2new[i];
			}	else
	...
	for(i=1; i<nthunks; i++) if (m_gthunks[i].pent && !(m_gthunks[i].pent->m_bProcessed & 1<<31)) {
		i32 idxNew = old2new[m_gthunks[i].pent->m_iGThunk0];
...
i32 CPhysicalWorld::ChangeEntitySimClass(CPhysicalEntity *pent, i32 bGridLocked)
{
	...
	if ((u32)(pent->m_iPrevSimClass-0x100)<8u) {
	...
	}
	{
		...
		u32k iSimClass=pent->m_iSimClass, iPrevSimClass=pent->m_iPrevSimClass;
		CPhysicalEntity *pent0=pent,*pent1=pent;
		i32 bPermanent = pent->m_bPermanent;
...

	{
		...
		for(i32 ithunk=pent->m_iGThunk0; ithunk; ithunk=m_gthunks[ithunk].inextOwned)
...
i32 CPhysicalWorld::GetFreeThunk()
{
	if (!m_iFreeGThunk0) {
		...
		i32k increaseThunks = (64*1024)/sizeof(pe_gridthunk);	// Increase in multiples of 64K to avoid fragmentation
		AllocGThunksPool(m_thunkPoolSz+increaseThunks);
	}
	i32 ithunk = m_iFreeGThunk0;
	m_iFreeGThunk0 = m_gthunks[m_iFreeGThunk0].inextOwned;
	return ithunk;
}


i32 CPhysicalWorld::RepositionEntity(CPhysicalPlaceholder *pobj, i32 flags, Vec3 *BBox, i32 bQueued)
{
	SEntityGrid *pgrid = GetGrid(pobj);
	i32 i,j,igx[2],igy[2],igxInner[2],igyInner[2],igz[2],ix,iy,ithunk,ithunk0;
	u32 n;
	if ((u32)pobj->m_iSimClass>=7u) return 0; // entity is frozen
	i32 bGridLocked = 0;
	i32 bBBoxUpdated = 0;
	...
	if (flags&1 && pgrid->cells) {
		...
		for(i=0;i<2;i++) {
			...
			igz[i] = (i32)(gBBox[i].z*pgrid->rzGran)+i;
		}
		if (pobj->m_ig[0].x!=NO_GRID_REG) // if m_igx[0] is NO_GRID_REG, the entity should not be registered in grid at all
			if (igx[0]-pobj->m_ig[0].x | igy[0]-pobj->m_ig[0].y | igx[1]-pobj->m_ig[1].x | igy[1]-pobj->m_ig[1].y | flags&4 | flags>>2&1 ^ pobj->m_bOBBThunks) {
				...
				i32 moveToEnd = -iszero(pobj->m_ig[0].x-GRID_REG_LAST);
				...
				if (pobj->m_iSimClass!=5) {
					if (n==0 || n>(u32)m_vars.nMaxEntityCells) {
						Vec3 pos = (pcurobj->m_BBox[0]+pcurobj->m_BBox[1])*0.5f;
						char buf[256];
						drx_sprintf(buf,"Error: %s @ %.1f,%.1f,%.1f is too large or invalid", !m_pRenderer ? "entity" :
				...
				} else if (n>(u32)m_vars.nMaxAreaCells)
					return -1;
				...
				for(ix=igx[0];ix<=igx[1];ix++) for(iy=igy[0];iy<=igy[1];iy++) {
					...
					i32 ithunkGrid = pgrid->cells[j];

					if (!ithunkGrid || m_gthunks[ithunkGrid].iSimClass-5 & ~moveToEnd) {
						i32& entGridEntry = pgrid->cells[(u32)j];
						...
						for(ithunk0=ithunkGrid; iszero((i32)m_gthunks[m_gthunks[ithunk0].inext].iSimClass-5) | moveToEnd & m_gthunks[ithunk0].inext; ithunk0=m_gthunks[ithunk0].inext);
						pNewGThunk->bFirstInCell = 0;
						pNewGThunk->inext = m_gthunks[ithunk0].inext;
						pNewGThunk->iprev = ithunk0;
						m_gthunks[m_gthunks[ithunk0].inext].iprev = ithunk & -(i32)m_gthunks[ithunk0].inext>>31;
			...
			if (pobj->m_iSimClass > 4) {
				...
				if (SEntityGrid *pgrid1 = GetHostedGrid(pent)) {
					...
					for(ithunk=m_gthunks[pgrid1->m_iGThunk0].inext; ithunk; ithunk=m_gthunks[ithunk].inext)
						...
						else if ((u32)m_gthunks[ithunk].pent->m_iSimClass-1u < 2u) {
							CPhysicalEntity *pent1 = (CPhysicalEntity*)m_gthunks[ithunk].pent;
							i32 nbodies = pent1->GetType()!=PE_ARTICULATED ? 1:pent1->m_nParts-1, nimp=0;
...
i32 CPhysicalWorld::DeformPhysicalEntity(IPhysicalEntity *pient, const Vec3 &ptHit,const Vec3 &dirHit,float r, i32 flags)
{
	...
	i32 i,bEntChanged,bPartChanged;
...
void CPhysicalWorld::ClonePhysGeomInEntity(CPhysicalEntity *pent,i32 i,IGeometry *pNewGeom)
{
	phys_geometry *pgeom;
	...
	{ WriteLock lock(m_lockGeoman);
		*(pgeom = GetFreeGeomSlot()) = *pent->m_parts[i].pPhysGeomProxy;
		...
		if (pgeom->pMatMapping)
			memcpy(pgeom->pMatMapping=new i32[pgeom->nMats], pent->m_parts[i].pPhysGeomProxy->pMatMapping, pgeom->nMats*sizeof(i32));
	}
...
i32 CPhysicalWorld::DeformEntityPart(CPhysicalEntity *pent,i32 i, pe_explosion *pexpl, geom_world_data *gwd,geom_world_data *gwd1, i32 iSource)
{
	IGeometry *pGeom,*pHole;
	CTriMesh *pNewGeom=0;
	EventPhysUpdateMesh epum;
	i32 bCreateConstraint=0;
	epum.pEntity = pent;
	...
	if ((pent->m_parts[i].idmatBreakable>>7 | pexpl->iholeType)!=0 && !(pent->m_parts[i].idmatBreakable & 128<<pexpl->iholeType)
		...
		return 0;
	for(i32 j=0;j<min(pent->m_nParts,5);j++)
...
void CPhysicalWorld::UnmarkEntityAsDeforming(CPhysicalEntity *pent)
{
	if (pent->m_flags & pef_deforming) {
		WriteLock lock(m_lockDeformingEntsList);
		pent->m_flags &= ~pef_deforming;
		i32 i; for(i=m_nDeformingEnts-1; i>=0 && m_pDeformingEnts[i]!=pent;i--);
		if (i>=0)
...
void CPhysicalWorld::SimulateExplosion(pe_explosion *pexpl, IPhysicalEntity **pSkipEnts,i32 nSkipEnts, i32 iTypes, i32 iCaller)
{
	DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );

	CPhysicalEntity **pents;
	i32 nents,nents1,i,j,i1,bBreak,bEntChanged;
	RigidBody *pbody;
	...
	for(i=0;i<nSkipEnts;i++)
		if(pSkipPhysEnts[i]->m_flags & pef_traceable)
			...
			i32 numParts = pSkipPhysEnts[i]->m_nParts;
	...
	for(nents--,m_nExplVictims=0; nents>=0; nents--) {
		...
		{ ReadLock lock0(pents[nents]->m_lockUpdate);
					  ...
					  box_sphere_overlap_check(&bboxPart,&sphExpl))))
			{
				...
				{
					i32 iprim=0,ifeat,ncont,bMultipart;
					Vec3 ptdst[2];
					CBoxGeom boxGeom;
						...
						for(j=0;j<pmd->nIslands;j++) if (((ptdst[0]=gwd.R*pmd->pIslands[j].center*gwd.scale+gwd.offset)-sphExpl.center).len2()<sqr(sphExpl.r*2)) {
							...
							if (!pmd->pMats)
								...
								for(iprim=pmd->pIslands[j].itri,epc.idmat[1]=-1,epc.iPrim[1]=-1; iprim<pmd->nTris && pmd->pMats[iprim]>=0; iprim=pmd->pTri2Island[iprim].inext) {
									i32 idmat = pents[nents]->GetMatId(pmd->pMats[iprim],i);
									i32 bBreakable = -((i32)m_SurfaceFlagsTable[idmat] & sf_manually_breakable)>>31;
...
void CPhysicalWorld::PurgeDeletedEntities()
{
	auto IterateQueue = [this](std::function<void(char *slot,CPhysicalWorld*)> func)	{
		for(i32 i=0; i<m_nQueueSlots; i++)
			for(i32 j=0; *(i32*)(m_pQueueSlots[i]+j)!=-1; j+=*(i32*)(m_pQueueSlots[i]+j+sizeof(i32)))
				func(m_pQueueSlots[i]+j,this);
	};
	{ WriteLock lock1(m_lockQueue);
		IterateQueue([](char *slot,CPhysicalWorld *pWorld) {
			CPhysicalEntity *pent = *(CPhysicalEntity**)(slot+sizeof(i32)*2);
			if (*slot==5)
				pWorld->DestroyPhysicalEntity(pent, *(i32*)(slot+sizeof(i32)*2+sizeof(uk )), 1);
		});
		IterateQueue([](char *slot,CPhysicalWorld*) { if ((*(CPhysicalEntity**)(slot+sizeof(i32)*2))->m_iSimClass==7) *(i32*)slot = -2; });
	}
	for(SEntityGrid *pgrid=m_pDeletedGrids; pgrid; pgrid=DestroyGrid(pgrid));
	m_pDeletedGrids = nullptr;
...
void CPhysicalWorld::DrawPhysicsHelperInformation(IPhysRenderer *pRenderer, i32 iCaller)
{
#ifndef _RELEASE
	i32 entype; CPhysicalEntity *pent=0;
	QuatT offs0 = (m_pRenderer = pRenderer)->SetOffset(m_vars.helperOffset);

	if (m_vars.iDrawHelpers) {
		assert(iCaller<=MAX_PHYS_THREADS);
		i32 i,n=0,nEntListAllocs,nGEA;
	...
	if (m_vars.iDrawHelpers & 8192 && m_cubeMapStatic.N) {
 		...
 		i32 i,ix,iy,cx,cy,cz,nOccRes=m_cubeMapStatic.N;
 		Vec3 pt0,pt1,dir;
 		xscale = 2.0f/nOccRes;
 //		xoffs = 1.0f-xscale;
 		for(i=0;i<6;i++) {
	...
	if (m_vars.iDrawHelpers & 0x100<<6)
		for(SEntityGrid *pgrid=m_entgrid.m_next; pgrid; pgrid=pgrid->m_next)
			for(i32 i=0;i<2;i++) for(i32 ix=0;ix<=pgrid->size[i];ix++) {
	...
	if (m_vars.bLogActiveObjects) {
		...
		if (m_vars.bLogActiveObjects & 1)	{
			...
			for(pent=m_pTypedEnts[2]; pent; pent=pent->m_next) if (pent->GetMassInv()>0) {
				...
				DrxLogAlways("%s @ %7.2f,%7.2f,%7.2f, mass %.2f, v %.1f, w %.1f, #polies %d, id %d",
		...
		if (m_vars.bLogActiveObjects & 2)
			for(pent=m_pTypedEntsPerm[4]; pent; pent=pent->m_next) if (!(pent->m_flags & pef_disabled) && pent->IsAwake()) {
				...
				DrxLogAlways("%s @ %7.2f,%7.2f,%7.2f, %s, id %d",
					m_pRenderer ? m_pRenderer->GetForeignName(pent->m_pForeignData,pent->m_iForeignData,pent->m_iForeignFlags):"",
					pent->m_pos.x,pent->m_pos.y,pent->m_pos.z, type, pent->m_id);
			}
		DrxLogAlways("%d active object(s)",nCount);
		m_vars.bLogActiveObjects = 0;
	}
#endif//_RELEASE
}

void CPhysicalWorld::DrawEntityHelperInformation(IPhysRenderer *pRenderer, i32 entityId, i32 iDrawHelpers)
{
#ifndef _RELEASE
...
i32 CPhysicalWorld::CollideEntityWithBeam(IPhysicalEntity *_pent, Vec3 org,Vec3 dir,float r, ray_hit *phit)
{
	if (!_pent)
		return 0;
	DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );
	...
	for(i32 i=0;i<pent->m_nParts;i++) if (pent->m_parts[i].flags & geom_collides) {
...
i32 CPhysicalWorld::CollideEntityWithPrimitive(IPhysicalEntity *_pent, i32 itype, primitive *pprim, Vec3 dir, ray_hit *phit, intersection_params* pip)
{
...
static inline void swap(CPhysicalEntity **pentlist, i32 i1,i32 i2) {
	CPhysicalEntity *pent = pentlist[i1]; pentlist[i1] = pentlist[i2]; pentlist[i2] = pent;
}
static void qsort(CPhysicalEntity **pentlist, const Vec3 &mask0,const Vec3 &mask1, i32 ileft,i32 iright)
{
	if (ileft>=iright) return;
	i32 i,ilast;
...
float CPhysicalWorld::PrimitiveWorldIntersection(const SPWIParams &pp, WriteLockCond *pLockContactsExp, tukk pNameTag)
{
	i32 i,j,j1,ncont,nents,iActive=0;
	i32 iCaller = get_iCaller();
	...
	if (pp.entTypes & rwi_queue) {
		...
		m_pwiQueue[m_pwiQueueHead].nSkipEnts = min((i32)(sizeof(m_pwiQueue[0].idSkipEnts)/sizeof(m_pwiQueue[0].idSkipEnts[0])),pp.nSkipEnts);
...
i32 CPhysicalWorld::RayTraceEntity(IPhysicalEntity *pient, Vec3 origin,Vec3 dir, ray_hit *pHit, pe_params_pos *pp,
								   u32 geomFlagsAny /*=geom_colltype0|geom_colltype_player*/)
{
	...
	if (((CPhysicalPlaceholder*)pient)->m_iSimClass!=5) {
		...
		auto FillHit = [pHit,pent](const geom_contact& cont, i32 ipart) {
			pHit->dist = cont.t;
			pHit->pCollider = pent;
...
void CPhysicalWorld::GetMemoryStatistics(IDrxSizer *pSizer)
{
	...
	{
#ifndef NOT_USE_DRX_MEMORY_MANAGER
...
/*#if DRX_PLATFORM_WINDOWS
	...
	_IMAGE_DOS_HEADER *pMZ = (_IMAGE_DOS_HEADER*)GetModuleHandle("DinrusXPhys.dll");
	_IMAGE_NT_HEADERS *pPE = (_IMAGE_NT_HEADERS*)((tuk)pMZ+pMZ->e_lfanew);
	IMAGE_SECTION_HEADER *sections = IMAGE_FIRST_SECTION(pPE);
	for(i=0;i<pPE->FileHeader.NumberOfSections;i++) for(j=0;j<DRX_ARRAY_COUNT(sec_ids);j++)
	if (!strncmp((tuk)sections[i].Name, sec_ids[j], min(8,strlen(sec_ids[j])+1))) {
		SIZER_COMPONENT_NAME(pSizer, sec_names[j]);
		pSizer->AddObject((uk )sections[i].VirtualAddress, sections[i].Misc.VirtualSize);
	}
#endif*/
	...
	{ SIZER_COMPONENT_NAME(pSizer,"world queues");
		...
		pSizer->AddObject(m_pQueueSlots, m_nQueueSlots*(sizeof(i32*)+QUEUE_SLOT_SZ));
		pSizer->AddObject(m_pQueueSlotsAux, m_nQueueSlotsAux*(sizeof(i32*)+QUEUE_SLOT_SZ));
	...
	void GetRBMemStats(IDrxSizer *pSizer);
	...
	{	SIZER_COMPONENT_NAME(pSizer,"placeholders");
		...
		pSizer->AddObject(m_pPlaceholderMap, ((size_t)m_nPlaceholderChunks<<PLACEHOLDER_CHUNK_SZLG2-5)*sizeof(i32));
	}
...
void CPhysicalWorld::AddEntityProfileInfo(CPhysicalEntity *pent,i32 nTicks)
{
	if (m_vars.bProfileGroups)
		m_grpProfileData[GetEntityProfileType(pent)].nTicks += nTicks;

	if (m_nProfiledEnts==DRX_ARRAY_COUNT(m_pEntProfileData) &&
	...
	i32 iBound[2]={ -1,m_nProfiledEnts };
	do {
		i = iBound[0]+iBound[1]>>1;
		PREFAST_ASSUME(i>0 && i<DRX_ARRAY_COUNT(m_pEntProfileData));
		iBound[isneg(m_pEntProfileData[i].nTicksStep-nTicks)] = i;
	} while(iBound[1]>iBound[0]+1);
	m_nProfiledEnts = min(m_nProfiledEnts+1, (i32)(DRX_ARRAY_COUNT(m_pEntProfileData)));
...
void CPhysicalWorld::AddFuncProfileInfo(tukk name, i32 nTicks)
{
	WriteLock lock(m_lockFuncProfiler);
	i32 i,iBound[2]={ -1,m_nProfileFunx };
	if (m_nProfileFunx) do {
		i = iBound[0]+iBound[1]>>1;
		iBound[isneg((i32)(name-m_pFuncProfileData[i].pName))] = i;
	}	while(iBound[1]>iBound[0]+1);
...
void CPhysicalWorld::AddEventClient(i32 type, i32 (*func)(const EventPhys*), i32 bLogged, float priority)
{
...
i32 CPhysicalWorld::RemoveEventClient(i32 type, i32 (*func)(const EventPhys*), i32 bLogged)
{
	WriteLock lock(m_lockEventClients);
...
EventPhys *CPhysicalWorld::AllocEvent(i32 id,i32 sz)
{
	if (m_pFreeEvents[id]==0) {
		...
		m_pFreeEvents[id] = (EventPhys*)((tuk)(m_pCurEventChunk+1)+m_szCurEventChunk);
...
u32 CPhysicalWorld::GetPumpLoggedEventsTicks()
{
	u32 ticks = m_nPumpLoggedEventsHits;
	m_nPumpLoggedEventsHits = 0;
	return ticks;
}
...
void CPhysicalWorld::PumpLoggedEvents()
{
	...
	i32 lastPoolHitBlockSize;
	{ WriteLock lock(m_lockEventsQueue);
		pEventFirst = m_pEventFirst;
		m_pEventFirst = m_pEventLast = 0;
		for(i32 i=0; i<EVENT_TYPES_NUM; i++) m_nEvents[i] = 0;
		m_iLastLogPump++;
	}

	u32 bNotZeroStep = m_vars.lastTimeStep==0.f ? 0 : (u32)-1;
	...
	for(pEvent=pEventFirst; pEvent; pEvent=pEvent->next)
					...
					pEvent->idval>EventPhysCollision::id && ((CPhysicalEntity*)((EventPhysMono*)pEvent)->pEntity)->m_iDeletionTime))
		{
			...
			i32 bRWIorPWI = iszero(pEvent->idval-EventPhysRWIResult::id)+iszero(pEvent->idval-EventPhysPWIResult::id);
			if (bRWIorPWI && ((EventPhysRWIResult*)pEvent)->OnEvent)
				...
				i32 bContinue = pClient->OnEvent(pEvent);
				bContinue += bRWIorPWI;
				if (!bContinue)
					break;
			}
	...
	for(pClient=m_pEventClients[EventPhysPostPump::id][1]; pClient; pClient=pClient->next) {
		BLOCK_PROFILER(pClient->ticks);
		i32 bContinue = pClient->OnEvent(NULL);
	}
	...
	for(iEvent=0;iEvent<EVENT_TYPES_NUM;iEvent++)
		for(pClient=m_pEventClients[iEvent][1],iClient=0; pClient; pClient=pClient->next,iClient++) {
			if (pClient->ticks*300 > m_vars.ticksPerSecond) {
				drx_sprintf(pClient->tag, "PhysEventHandler(%d,%d)", iEvent,iClient);
		...
		if (pEventLast) {
			...
			for(pEvent=pEventLast; pEvent; pEvent=pEvent->next)
				if (pEvent->idval==EventPhysRWIResult::id) {
					...
					i32 i,bNoSolidHits = isneg(pEventRWI->pHits[0].dist);
	...
	if (pLastPoolHit) {
		...
		i32 i; for(i=1; i<lastPoolHitBlockSize && pLastPoolHit[-i].next==pLastPoolHit-i+1; i++);
		if (i<lastPoolHitBlockSize)// || pLastPoolHit->next->next!=pLastPoolHit->next+1)
			DrxLog("Error: queued RWI hits pool corrupted");
		else {
	...
	m_nPumpLoggedEventsHits += (u32)(pumpEnd.QuadPart-pumpStart.QuadPart) - (u32)(endYields - startYields);
#endif
}
...
void SerializeGeometries(CPhysicalWorld *pWorld, tukk fname,i32 bSave);
void SerializeWorld(CPhysicalWorld *pWorld, tukk fname,i32 bSave);

i32 CPhysicalWorld::SerializeWorld(tukk fname, i32 bSave)
{
	::SerializeWorld(this,fname,bSave);
	return 1;
}
i32 CPhysicalWorld::SerializeGeometries(tukk fname, i32 bSave)
{
	::SerializeGeometries(this,fname,bSave);
	return 1;
}
#else
i32 CPhysicalWorld::SerializeWorld(tukk fname, i32 bSave) { return 0; }
i32 CPhysicalWorld::SerializeGeometries(tukk fname, i32 bSave) { return 0; }
#endif


i32 CPhysicalWorld::AddExplosionShape(IGeometry *pGeom, float size,i32 idmat, float probability)
{
	i32 i,j,bCreateConstraint=idmat>>16 & 1;
	idmat &= 0xFFFF;
	...
	for(i32 explIdx = i + 1; explIdx <= m_nExpl; ++ explIdx)
	...
	if (i>0 && m_pExpl[i-1].idmat==idmat) {
		...
		i32 jc = m_pExpl[i].iFirstByMat+m_pExpl[i-1].nSameMat;
...
void CPhysicalWorld::RemoveExplosionShape(i32 id)
{
	i32 i,j;
...
#endif
#endif
IGeometry *CPhysicalWorld::GetExplosionShape(float size,i32 idmat, float &scale, i32 &bCreateConstraint)
{
	i32 i,j,mask,ibound[2]={ -1,m_nExpl };
	...
	f = drx_random(0.0f, probabilitySum);
...
i32 CPhysicalWorld::SetWaterUprParams(pe_params *params)
{
...
IPhysicalEntity* CPhysicalWorld::LoadPhysicalEntityPtr(TSerialize ser)
{
	...
	ser.Value("mass", mass);
	ser.EndGroup();
	iSimClass1 = (u32)(iSimClass-1)<2u ? (iSimClass^3):iSimClass;
...
i32 CPhysicalEntity::SetStateFromTypedSnapshot(TSerialize ser, i32 iSnapshotType, i32 flags)
{
	...
	return g_pTypedEnts[min((i32)(DRX_ARRAY_COUNT(g_pTypedEnts)),max(0,iSnapshotType))]->
		SetStateFromSnapshot(ser,flags);
}

void CPhysicalWorld::SerializeGarbageTypedSnapshot( TSerialize ser, i32 iSnapshotType, i32 flags )
{
	static CPhysicalEntity g_entStatic(0);
	static CRigidEntity g_entRigid(0);
	static CWheeledVehicleEntity g_entWheeled(0);
	...
	g_pTypedEnts[min((i32)(DRX_ARRAY_COUNT(g_pTypedEnts)),max(0,iSnapshotType))]->
...
phys_job_info& GetJobProfileInst(i32 ijob) { return g_pPhysWorlds[0]->GetJobProfileInst(ijob); }
...
linkedpt* crop_poly2d(linkedpt* p0, const Vec2* bounds, linkedpt* pbuf)
{
	...
	for(i32 i=0;i<4;i++) {
		...
		i32 inside = (p=p0)->pt[i&1]*sg < b*sg;
		do { pnext = p->next[1];
...
void CPhysicalWorld::RasterizeEntities(const grid3d& grid, uchar *rbuf, i32 objtypes, float massThreshold, const Vec3& offsBBox, const Vec3& sizeBBox, i32 flags)
{
	linkedpt pbuf[10];
	Vec3 bounds = grid.size*Diag33(grid.step), wbounds=Matrix33(grid.Basis.T()).Fabs()*bounds, n,pt;
	CPhysicalEntity **pents;
	Quat qBasis = Quat(grid.Basis);
	i32 iCaller=get_iCaller(), flagsAll=0, flagsAny=-1, ystride=grid.size.x;
	...
	for(i32 ient=GetEntitiesAround(grid.origin+max(Vec3(ZERO),offsBBox-sizeBBox),grid.origin+min(wbounds,offsBBox+sizeBBox),pents,objtypes)-1; ient>=0; ient--)
		if (pents[ient]->GetMassInv()*massThreshold<=1.0f) for(i32 ipart=pents[ient]->GetUsedPartsCount(iCaller)-1; ipart>=0; ipart--) {
			...
			i32 igeom = part.pPhysGeomProxy->pGeom->GetType();

			if (igeom==GEOM_TRIMESH || igeom==GEOM_BOX) {
				const mesh_data *md = (const mesh_data*)part.pPhysGeomProxy->pGeom->GetData();
				i32 nvtx = 3;
				if (igeom==GEOM_BOX) {
					static i32 boxidx[] = { 0,2,3,1, 0,4,6,2, 2,6,7,3, 3,7,5,1, 1,5,4,0, 4,5,7,6 };
				...
				for(i32 itri=0; itri<md->nTris; itri++) if ((n=qPart2Grid.q*md->pNormals[itri]).z>0) {
					...
					i32 iy = float2int(p[0]->pt.y*grid.stepr.y);
					for(p[1]=p[0]; ;iy++) {
						for(i=0; i<2; i++) {
						...
						i32 ix[2] = { float2int(rx[0]*grid.stepr.x), float2int(rx[1]*grid.stepr.x) };
						...
						for(ibuf=(i=ix[0])+iy*ystride; i<ix[1]; i++,ibuf++,zh-=grid.step.x*n.x*rnz)
							rbuf[ibuf] = max((i32)rbuf[ibuf], max(0,min(255,float2int(zh*grid.stepr.z-0.5f))));
					}
					finished:;
				}
			} else {
				...
				for(org.y=((j=ibbox[0].y)+0.5f)*grid.step.y; j<=ibbox[1].y; j++,org.y+=grid.step.y)
					for(ibuf=j*ystride+(i=ibbox[0].x),org.x=(ibbox[0].x+0.5f)*grid.step.x; i<=ibbox[1].x; i++,ibuf++,org.x+=grid.step.x)
						if ((aray.origin=qGrid2Part*org, g_Intersector.Check(igeom,ray::type, pprim,&aray, &inters)) && inters.n.z>0)
							rbuf[ibuf] = max((i32)rbuf[ibuf], max(0,min(255,float2int((org.z-((inters.pt[0]-aray.origin)*dirn))*grid.stepr.z-0.5f))));
			}
		}
}
