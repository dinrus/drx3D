...
{
	phf->stepr.x = 1.0f/phf->step.x; phf->stepr.y = 1.0f/phf->step.y;
	m_hf = *phf;
	m_hf.Basis.SetIdentity();
	m_hf.origin.zero();
	i32 i = (phf->typemask^phf->typemask-1)+1>>1;
	for(m_hf.typepower=0; !(i&1); i>>=1,m_hf.typepower++);

	m_minHeight = -1000; m_maxHeight = 1000;
	...
	m_Tree.m_pUsedTriMap = new u32[(m_nTrisAlloc-1>>5)+1];
...
struct hf_cell_checker {
	...
	i32 check_cell(const Vec2i &icell, i32 &ilastcell) {
		...
		i32 itype = phf->gettype(icell.x,icell.y);
...
i32 CHeightfield::Intersect(IGeometry *pCollider, geom_world_data *pdata1,geom_world_data *pdata2, intersection_params *pparams, geom_contact *&pcontacts)
{
	if (pCollider->GetType()==GEOM_RAY) {
		geometry_under_test GRay;
		bool bKeepPrevContacts;
		i32 bThreadSafe,bActive,iCaller=get_iCaller();
		hf_cell_checker hcc;
		hcc.bNoCull = 2; GRay.iCaller=iCaller;
		if (pparams) {
		...
		if (DrawRayOnGrid(&m_hf, origin,dir, hcc) && hcc.idcontact!=-2) {
			...
			if (((CGeometry*)pCollider)->m_iCollPriority<m_iCollPriority || !pparams)
				...
				for(i32 i=0;i<3;i++) hcc.hftri.pt[i] = pdata1->R*hcc.hftri.pt[i]*pdata1->scale + pdata1->offset;
...
i32 CHeightfield::PrepareForIntersectionTest(geometry_under_test *pGTest, CGeometry *pCollider,geometry_under_test *pGTestColl, bool bKeepPrevContacts)
{
	box abox,aboxext,*pbox; pCollider->GetBVTree()->GetBBox(&abox); 
	if (pGTestColl && pGTestColl->sweepstep>0) {
		ExtrudeBox(&abox, pGTestColl->sweepdir_loc,pGTestColl->sweepstep_loc, &aboxext);
		pbox = &aboxext;
	} else pbox = &abox;
	i32 ix,iy,sx,sy,i,j,itri,icol,irow,res=0,szHeights=0,idx_max=PHYS_MAX_INDICES;
  ...
  float* heights;
	...
	if (m_Tree.m_PatchStart.x-ix|m_Tree.m_PatchStart.y-iy|m_Tree.m_PatchSize.x-sx|m_Tree.m_PatchSize.y-sy) {		
		...
		for(i=ix,maxh=heights[0]=m_hf.getheight(ix,iy);i<=ix+sx;i++) for(j=iy;j<=iy+sy;j++)
		...
		for(itri=0;itri<m_nTris;itri++) if (m_pIds[itri]==(char)-1) 
		for(j=0;j<3;j++) if ((i=m_pTopology[itri].ibuddy[j])>=0) // remove connectivity for 'hole' triangles
  ...
  if (!pGTest || pGTest->bStopIntersection) { 
	...
	if (szHeights>(16<<10)) delete[] heights;
	return res; 
}


inline void CheckPtEdgeDist(const Vec3 *pte, const Vec3 &pt, Vec3 &ptres,i32 &iFeature)
{
...
inline void CheckPtTriDist(const triangle &tri, const Vec3 &pt, Vec3 &ptres,i32 &iFeature)
{
...
i32 CHeightfield::FindClosestPoint(geom_world_data *pgwd, i32 &iPrim,i32 &iFeature, const Vec3 &ptdst0,const Vec3 &ptdst1,
																	 Vec3 *ptres, i32 nMaxIters)
{
	...
	if ((u32)ix>(u32)m_hf.size.x-2 || (u32)iy>(u32)m_hf.size.y-2 || m_hf.gettype(ix,iy)<0) {
...
i32 CHeightfield::PointInsideStatus(const Vec3 &pt) 
{ 
	float xf=pt.x*m_hf.stepr.x, yf=pt.y*m_hf.stepr.y;
	i32 ix=float2int(xf-0.5f), iy = float2int(yf-0.5f);
	if ((u32)ix>(u32)m_hf.size.x-2 || (u32)iy>(u32)m_hf.size.y-2 || m_hf.gettype(ix,iy)<0)
		return 0;
	float h[3];
	xf-=ix; yf-=iy;
...
i32 CHeightfield::DrawToOcclusionCubemap(const geom_world_data *pgwd, i32 iStartPrim,i32 nPrims, i32 iPass, SOcclusionCubeMap* cubeMap)
{
	i32 nTris,ix,iy;
	...
	for(ix=irect[0].x,nTris=0;ix<irect[1].x;ix++) for(iy=irect[0].y;iy<irect[1].y;iy++) 
		if (m_hf.gettype(ix,iy)>=0) {
			...
			tri.pt[0] = tri.pt[2]; tri.pt[2] += pgwd->R*Vec3(pgwd->scale*m_hf.step.x,0,0); 
...
void CHeightfield::GetMemoryStatistics(IDrxSizer *pSizer)
{
