...
public:
	...
	virtual i32 SetParams(pe_params* params,i32 bThreadSafe=1) { return 0; }
	virtual i32 GetParams(pe_params* params) const { return 0; }
	virtual i32 GetStatus(pe_status* status) const { return 0; }
	virtual i32 Action(pe_action* action,i32 bThreadSafe=1) { return 0; }

	virtual i32 AddGeometry(phys_geometry *pgeom, pe_geomparams* params,i32 id=-1,i32 bThreadSafe=1) { return -1; }
	...
	virtual uk GetForeignData(i32 itype=0) const { return 0; }
	virtual i32 GetiForeignData() const { return 0; }

	virtual i32 GetStateSnapshot(class CStream &stm, float time_back=0, i32 flags=0) { return 0; }
	virtual i32 GetStateSnapshot(TSerialize ser, float time_back=0, i32 flags=0) { return 0; }
	virtual i32 SetStateFromSnapshot(class CStream &stm, i32 flags=0) { return 0; }
	virtual i32 SetStateFromSnapshot(TSerialize ser, i32 flags=0) { return 0; }
	virtual i32 SetStateFromTypedSnapshot(TSerialize ser, i32 type, i32 flags/* =0 */) {return 0;}
	virtual i32 PostSetStateFromSnapshot() { return 0; }
	virtual i32 GetStateSnapshotTxt(char *txtbuf,i32 szbuf, float time_back=0) { return 0; }
	...
	virtual i32 Step(float time_interval) { return 0; }
...
float getHeightFunc(float *data,getHeightCallback func, i32 ix,i32 iy) { return func(ix,iy); }
float getHeightData(float *data,getHeightCallback func, i32 ix,i32 iy) { return data[ix+iy]*0.33f; }

i32 CPhysArea::ApplyParams(const Vec3& pt, Vec3& gravity, const Vec3 &vel, pe_params_buoyancy *pbdst, i32 nBuoys, i32 nMaxBuoys, i32 &iMedium0, 
													 IPhysicalEntity *pent) const
{
	i32 bRes=1,bUseBuoy=0,bDiffGrid=0;
	...
	if (m_ptSpline && !m_bUniform) {
		...
		i32 iClosestSeg; float tClosest,mindist;
				...
				if (m_pGeom && m_pGeom->GetType()==GEOM_HEIGHTFIELD || m_pWaterMan && pent!=m_pContainerEnt) {
					if (pent && m_pb.waterPlane.origin.z>-1000.0f) {
						...
						if (true) {//fabs_tpl((org-m_pb.waterPlane.origin)*n)-sz*n.abs() < 1.5f) {
							...
							i32 iyscale=1,vmask=0; float *pdata; Vec3 vel(ZERO),*pvel=&vel;
							float (*getHeight)(float *data,getHeightCallback func, i32 ix,i32 iy);
							if (m_pWaterMan) {
...
i32 CPhysArea::CheckPoint(const Vec3& pttest, float radius) const
{
	Vec3 ptloc = ((pttest-m_offset)*m_rscale)*m_R;
	i32 iClosestSeg; float tClosest;
	if (!is_unused(m_gravity))
		radius = 0.0f;
	...
	if (m_pGeom && !m_pt)
		...
		i32 i,iend,i1,istart[2],ilim[2],bInside=0;
		float fend;
		quotientf y,ymax;
		Vec2 dp;
...
float CPhysArea::FindSplineClosestPt(const Vec3 &ptloc, i32 &iClosestSegRet,float &tClosestRet) const
{
	i32 i,j,iClosestSeg;
...
i32 CPhysArea::FindSplineClosestPt(const Vec3 &org, const Vec3 &dir, Vec3 &ptray, Vec3 &ptspline) const
{
	i32 i,j;
...
i32 CPhysArea::RayTraceInternal(const Vec3 &org, const Vec3 &dir, ray_hit *pHit, pe_params_pos *pp)
{
	geom_world_data gwd;
	geom_contact *pcontacts;
	intersection_params ip;
	i32 ncont,iClosestSeg;
	float tClosest;
	if (pp) {
		quaternionf qrot;
...
void CPhysArea::GetRandomPoints(Array<PosNorm> points, CRndGen& seed, EGeomForm eForm)	const
{
	if (m_pGeom && !m_debugGeomHash) {
		...
		for (auto& ran : points) {
			...
			i32 i = m_Extents[GeomForm_Edges].RandomPart(seed);
...
i32 CPhysArea::SetParams(pe_params *_params, i32 bThreadSafe)
{
	...
	if (_params->type==pe_params_area::type_id) {
		...
		for(i32 i=0;i<sizeof(m_waveSim)/sizeof(float);i++) if (!is_unused(((float*)&params->waveSim)[i])) {
	...
	if (_params->type==pe_params_buoyancy::type_id) {
		...
		if (!is_unused(params->iMedium)) {
			...
			if ((u32)(m_pb.iMedium=params->iMedium)<16) {
...
i32 CPhysArea::GetParams(pe_params *_params) const
{
	if (_params->type==pe_simulation_params::type_id) {
		pe_simulation_params *params = (pe_simulation_params*)_params;
...
i32 CPhysArea::Action(pe_action *_action,i32 bThreadSafe) 
{
...
i32 CPhysArea::GetStatus(pe_status *_status) const
{
...
i32 CPhysArea::OnBBoxOverlap(const EventPhysBBoxOverlap *pEvent)
{
	if (pEvent->iForeignData[1]==PHYS_FOREIGN_ID_PHYS_AREA)	{
		...
		float V=0; for(i32 i=0;i<pent->m_nParts;i++) 
			V += pent->m_parts[i].pPhysGeom->V*cube(pent->m_parts[i].scale)*isneg(-((i32)pent->m_parts[i].flags & geom_floats));
...
void CPhysArea::Update(float dt)
{
	...
	if (m_V>0 && !m_pContainer) {
			...
			if (CGeometry *pGeom = (CGeometry*)((CPhysicalEntity*)rhit.pCollider)->m_parts[rhit.ipart].pPhysGeom->pGeom)
				if (pContainer = (CTriMesh*)pGeom->GetTriMesh()) {
					...
					if (!((CPhysicalPlaceholder*)rhit.pCollider)->m_iSimClass) {
						...
						i32 igeom=0,ngeoms=0,nNewParts,listBuf[MAX_GEOMS*3],*list0=listBuf,*list1=listBuf+MAX_GEOMS; 
						m_nContainerParts=nNewParts=1; m_pContainerParts = new i32[MAX_GEOMS];
						...
						extern i32 g_bBruteforceTriangulation;
						i32 bft=g_bBruteforceTriangulation; g_bBruteforceTriangulation=1;

						do {
							i32 i,j,i1,j1,ngeomsNew=0,*listNew=listBuf+MAX_GEOMS*2;
							if (nNewParts) {
								i32 nents = m_pWorld->GetEntitiesAround(bboxes[m_nContainerParts-1][0],bboxes[m_nContainerParts-1][1],pents,ent_static,(CPhysicalEntity*)this,0,iCaller);
							...
							if (ngeomsNew) {
								...
								i32 *t=list0; list0=list1; list1=t; igeom=0; ngeomsNew=0;
							}
		...
		if (pContainer) { 
			if (m_szCell>0 && m_qtsContainer.s!=1.0f) {
				for(i32 i=0;i<pContainer->m_nVertices;i++) pContainer->m_pVertices[i]*=m_qtsContainer.s;
	...
	if (m_pContainer) {
		...
		if (m_pContainerEnt) {
			for(i32 i=m_pContainerEnt->m_nParts-1;i>0 && m_pContainerEnt->m_parts[i].id!=m_idpartContainer;i--);
		...
		static i32 g_nGeomsAlloc=0;
		static QuatTS *g_qtsGeoms;
		CPhysicalEntity **pents;
		i32 ngeoms, nents=m_pWorld->GetEntitiesAround(m_BBox[0], m_BBox[1], pents, ent_static|ent_sleeping_rigid|ent_rigid, (CPhysicalEntity*)this, 0,iCaller);
		for(i32 i=ngeoms=0;i<nents;i++) {
			float Vent=0;	i32 j1;
			for(i32 j=0;j<pents[i]->GetUsedPartsCount(iCaller);j++) {
		...
		i32 npt = ((CTriMesh*)m_pContainer)->FloodFill(loc*m_offset0,V,loc.q*g,V*m_accuracyV, ptnew,qtBorder,depth,m_bConvexBorder, g_pGeoms,g_qtsGeoms,ngeoms, Vsubmerged,comSubmerged);
		if (!npt)
			return;
		float e = sqr(max(m_pWorld->m_vars.maxContactGap,m_borderPad*0.3f));
		i32 j=1; for(i32 i=1;i<npt;j+=isneg(e-(ptnew[i++]-ptnew[j-1]).GetLength2())) ptnew[j]=ptnew[i];
		npt = j-isneg((ptnew[0]-ptnew[j-1]).GetLength2()-e);
		for(i32 i=j=1;i<npt;i++) {
		...
		if (m_borderPad) {
			...
			for(i32 i=0; i<npt-1; i++,ptprev=ptcur) {
		...
		{ WriteLock lock(m_lockUpdate);
			...
			if (!m_pTrigger) {
				...
				m_pWorld->AddEventClient(EventPhysBBoxOverlap::id,(i32(*)(const EventPhys*))OnBBoxOverlap,0,1000.0f);
			}	else
		...
		if (m_szCell<=0) {
			...
			if (m_pSurface) {
				if (m_npt>m_nptAlloc) {
					ReallocateList(m_pSurface->m_pIndices,0,m_npt*6*(sizeof(i32)/sizeof(index_t)));
					ReallocateList(m_pSurface->m_pNormals,0,m_npt*2);
					ReallocateList(m_pSurface->m_pVertices.data,0,m_nptAlloc=m_npt);
				}
				pIdx=(i32*)m_pSurface->m_pIndices; pt=m_pSurface->m_pVertices.data;
			} else {
				pIdx=new i32[m_npt*6]; pt=new Vec3[m_nptAlloc=m_npt];
			}
			for(i32 i=0;i<m_npt;i++) pt[i]=Vec3(m_pt[i]);
			MARK_UNUSED m_pt[m_npt].x;
			i32 nTris = TriangulatePoly(m_pt,m_npt+1,pIdx,m_npt*2);
			if (!nTris)	{
			...
			if (!m_pSurface) {
				...
				if (sizeof(i32)!=sizeof(index_t))
					for(i32 i=0;i<nTris*3;i++) m_pSurface->m_pIndices[i]=pIdx[i];
				...
				if ((m_pWorld->m_vars.iDrawHelpers & (ent_areas<<8|pe_helper_geometry))==(ent_areas<<8|pe_helper_geometry))
					for(i32 i=0;i<nTris;i++) m_pSurface->RecalcTriNormal(i);
			}
		}
	...
	if (m_szCell>0 && m_pt && !m_pWaterMan) {
		...
		i32 nCells = float2int(dim*(1.0f+m_sizeReserve)/m_szCell)+1|1;
		if (nCells<=1)
			return;
		WriteLock lock(m_pWorld->m_lockWaterMan);
	...
	if (m_pWaterMan) {
		...
		i32 nvtx = sqr(swm.nCells)*sqr(swm.nTiles*2+1);
		if (!m_pSurface) {
			...
			for(i32 i=0;i<swm.nCells-1;i++) for(i32 j=0;j<swm.nCells-1;j++)	{
	...
	if (bAwakeEnv && (m_pWorld->m_idStep&3)==3)	{
		CPhysicalEntity **pents;
		for(i32 i=m_pWorld->GetEntitiesAround(m_BBox[0],m_BBox[1],pents,ent_sleeping_rigid,0,0,iCaller)-1;i>=0;i--)
			for(i32 j=0;j<pents[i]->m_nParts;j++) if (pents[i]->m_parts[j].flags & geom_floats) {
				pents[i]->Awake(); break;
			}
	}
...
void CPhysArea::DrawHelperInformation(IPhysRenderer *pRenderer, i32 flags)
{	
	if (m_bDeleted)
		return;
	//ReadLock lock(m_lockUpdate);
	...
	if (flags & pe_helper_geometry)	{
		...
		if (m_pt) {
			...
			if (flags & pe_helper_collisions && (!m_pGeom || m_debugGeomHash!=hash) && sizeof(i32)==sizeof(index_t)) {
				WriteLock lock(m_lockUpdate);
				if (m_pGeom) m_pGeom->Release();
				Vec3 *pt = new Vec3[m_npt*2];
				Vec3i *pTris = new Vec3i[m_npt*6];
				MARK_UNUSED m_pt[m_npt].x;
				i32 nTris = TriangulatePoly(m_pt,m_npt+1,(i32*)pTris,m_npt*2);
				...
				m_pGeom = m_pWorld->CreateMesh(pt,strided_pointer<ushort>((ushort*)pTris,sizeof(i32)),0,0,(m_npt+nTris)*2,mesh_SingleBB|mesh_shared_vtx|mesh_shared_idx|mesh_no_vtx_merge|mesh_no_filter,0);
...
void CPhysArea::GetMemoryStatistics(IDrxSizer *pSizer) const
{
...
static inline void swap(float *pval,i32 *pidx, i32 i1,i32 i2) {	
	float x=pval[i1]; pval[i1]=pval[i2]; pval[i2]=x;
	i32 i=pidx[i1]; pidx[i1]=pidx[i2]; pidx[i2]=i;
}
static void qsort(float *pval,i32 *pidx, i32 ileft,i32 iright)
{
...
void CPhysArea::ProcessBorder()
{
	if (m_pt) {
		...
		m_idxSort[0] = new i32[m_npt];
		m_idxSort[1] = new i32[m_npt];
		memset(m_pMask = new u32[((m_npt-1>>5)+1)*(MAX_PHYS_THREADS+1)],0,((m_npt-1>>5)+1)*(MAX_PHYS_THREADS+1)*sizeof(i32));
		float *xstart = new float[m_npt];

		for(i32 iend=0; iend<2; iend++)	{
			for(i32 i=0;i<m_npt;i++) {
				m_idxSort[iend][i] = i; 
				i32 j = i+1 & i-m_npt+1>>31;
				i32 k = isneg(m_pt[i].x-m_pt[j].x)^iend;
...
IPhysicalEntity *CPhysicalWorld::AddArea(Vec3 *pt,i32 npt, float zmin,float zmax, const Vec3 &pos,const quaternionf &q,float scale, 
																				 const Vec3 &normal, i32 *pTessIdx,i32 nTessTris,Vec3 *pFlows)
{
	if (npt<=0)
		return 0;
	...
	memset(pArea->m_pMask = new u32[((npt-1>>5)+1)*(MAX_PHYS_THREADS+1)],0,((npt-1>>5)+1)*(MAX_PHYS_THREADS+1)*sizeof(i32));
	...
	if (maxdist>0.05f || pFlows) {
		i32 nTris, *pidx=new i32[npt*3];
		Vec3 *pvtx=new Vec3[npt];
		MARK_UNUSED pArea->m_pt[npt].x;
		if (pTessIdx && nTessTris)
			memcpy(pidx,pTessIdx,(nTris=nTessTris)*3*sizeof(i32));
		else
...
IPhysicalEntity *CPhysicalWorld::AddArea(Vec3 *pt,i32 npt, float r, const Vec3 &pos,const quaternionf &q,float scale)
{
	if (npt<=0)
		return 0;
	WriteLock lock(m_lockAreas);
...
void CPhysicalWorld::RemoveArea(IPhysicalEntity *_pArea)
{
	...
	for(i32 i=0;i<=MAX_PHYS_THREADS;i++)
		m_prevGEAobjtypes[i] = -1;
}

i32 CPhysicalWorld::CheckAreas(const Vec3 &ptc, Vec3 &gravity, pe_params_buoyancy *pb, i32 nMaxBuoys, i32 iMedium, const Vec3 &vel, IPhysicalEntity *pent, i32 iCaller)
{
	if (!m_pGlobalArea)
		return 0;
	CPhysArea *pArea;
	...
	if (m_nBigAreas==m_nAreas) {
		...
		for(i32 i=GetEntitiesAround(bbox[0],bbox[1],pEnts,ent_areas,(CPhysicalEntity*)pent,0,iCaller)-1; i>=0; i--)
...
void CPhysicalWorld::SetWaterMat(i32 imat) 
{ 
	m_matWater=imat; 
