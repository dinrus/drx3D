...
{	
	...
	//CArticulatedEntity
	static_assert(DRX_ARRAY_COUNT(m_Ya_vec) == 2, "Invalid array size!");
	m_Ya_vec[0].Set(0,0,0);
	m_Ya_vec[1].Set(0,0,0);

	static_assert(DRX_ARRAY_COUNT(m_posHist) == 2, "Invalid array size!");
	static_assert(DRX_ARRAY_COUNT(m_qHist) == 2, "Invalid array size!");
...
void CArticulatedEntity::AlertNeighbourhoodND(i32 mode)
{
	if (m_pHost)
		m_pHost->Release();
	m_pHost = 0;
...
i32 CArticulatedEntity::AddGeometry(phys_geometry *pgeom, pe_geomparams* _params,i32 id, i32 bThreadSafe)
{
	if (pgeom && fabs_tpl(_params->mass)+fabs_tpl(_params->density)!=0 && (pgeom->V<=0 || pgeom->Ibody.x<0 || pgeom->Ibody.y<0 || pgeom->Ibody.z<0))	{
		char errmsg[256];
		drx_sprintf(errmsg, "CArticulatedEntity::AddGeometry: (%s at %.1f,%.1f,%.1f) Trying to add bad geometry",
	...
	i32 res,i,j,nPartsAlloc=m_nPartsAlloc,nParts=m_nParts,idx;
	...
	if (m_joints[i].iStartPart+m_joints[i].nParts<idx) {
		...
		for(j=0;j<NMASKBITS && getmask(j)<=m_constraintMask;j++) if (m_constraintMask & getmask(j)) 
			for(i32 iop=0;iop<2;iop++) if (m_pConstraints[j].pent[iop]==this && m_pConstraints[j].ipart[iop]>=idx)
...
void CArticulatedEntity::RemoveGeometry(i32 id, i32 bThreadSafe)
{
	...
	{ 
		...
		if (--m_joints[j=m_infos[i].iJoint].nParts<=0 && !m_joints[j].nChildren) {
			...
			for(i32 ipart=0;ipart<m_nParts;ipart++)
...
void CArticulatedEntity::RecomputeMassDistribution(i32 ipart,i32 bMassChanged)
{
	i32 i,iStart,iEnd;
	float V,M0=m_body.M;
	Vec3 bodypos;
...
i32 CArticulatedEntity::SetParams(pe_params *_params, i32 bThreadSafe)
{
	...
	if (CRigidEntity::SetParams(_params,1)) {
		if (_params->type==pe_params_pos::type_id) {
			...
			{ WriteLock lock(m_lockUpdate);
				if ((prevq.v-m_qrot.v).len2()>0) {
					...
					for(i32 i=0;i<m_nJoints;i++) if (m_joints[i].iParent<0)	{
				...
				if (m_iSimClass > 2)
					m_bPartPosForced |= 2;
				else for(i32 i=0;i<m_nJoints;i++) {
					i32 j = m_joints[i].iStartPart;
		...
		if (_params->type==pe_params_part::type_id)	{
			...
			if (!m_pHost && params->bRecalcBBox && m_body.Minv==0 && m_nParts) {
				...
				i32 nEnts=m_pWorld->GetEntitiesAround(min(m_BBox[0],prevBBox[0])-gap,max(m_BBox[1],prevBBox[1])+gap,pentlist,m_collTypes&(ent_sleeping_rigid|ent_living|ent_independent)|ent_triggers,this);
			...
			if (i<m_nParts) {
				i32 iJoint=m_infos[i].iJoint, i0=m_joints[iJoint].iStartPart;
	...
	if (_params->type==pe_params_joint::type_id) {
		...
		m_bPartPosForced &= ~1;
		i32 i,j,op[2]={-1,-1},nChanges=0,bAnimated=m_bFeatherstone|iszero(m_body.M);
		for(i=0;i<m_nJoints;i++) {
		...
		for(i=0;i<3;i++) {
			...
			if (!is_unused(params->qext[i]) && m_joints[op[1]].iParent==op[0]) {
				...
				i32 sgnDelta = sgnnz(delta); delta = fabs_tpl(delta);
				...
				{	// qext violates limits; adjust the limits
					...
					diff[0] -= (i32)(diff[0]/(2*g_PI))*2*g_PI;
					diff[1] -= (i32)(diff[1]/(2*g_PI))*2*g_PI;
	...
	if (_params->type==pe_params_articulated_body::type_id) {
		...
		if (!is_unused(params->pivot)) {
			...
			for(i32 i=0;i<m_nJoints;i++) m_joints[i].body.pos += offs;
		}
		...
		if (!is_unused(params->nJointsAlloc) && params->nJointsAlloc>m_nJointsAlloc) {
			...
			if (params->nJointsAlloc>m_nPartsAlloc) {
				...
				for(i32 i=m_nParts;i<m_nPartsAlloc;i++)
...
i32 CArticulatedEntity::GetParams(pe_params *_params) const
{
	i32 res = CRigidEntity::GetParams(_params);
	if (res)
		return res;

	if (_params->type==pe_params_joint::type_id) {
		pe_params_joint *params = (pe_params_joint*)_params;
		i32 i; for(i=0;i<m_nJoints && m_joints[i].idbody!=params->op[1];i++);
		if (i>=m_nJoints)
			return 0;
...
i32 CArticulatedEntity::GetStatus(pe_status* _status) const
{
	...
	if (_status->type==pe_status_dynamics::type_id) {
		i32 i; pe_status_dynamics *status = (pe_status_dynamics*)_status;
		ReadLock lock(m_lockJoints);
		if (!is_unused(status->partid))
			for(i=0; i<m_nParts && m_parts[i].id!=status->partid; i++);
		else i = status->ipart;
	...
	i32 res = CRigidEntity::GetStatus(_status);
	if (res) {
...
i32 CArticulatedEntity::Action(pe_action *_action, i32 bThreadSafe)
{
	...
	if (_action->type==pe_action_batch_parts_update::type_id) {
		...
		if (!m_pHost && m_body.Minv==0 && m_nParts && m_iSimClass<4) {
			CPhysicalEntity **pentlist;
			i32 nEnts=m_pWorld->GetEntitiesAround(m_BBox[0],m_BBox[1],pentlist,m_collTypes&(ent_sleeping_rigid|ent_living|ent_independent)|ent_triggers,this);
			for(--nEnts;nEnts>=0;nEnts--) if (pentlist[nEnts]!=this)
				pentlist[nEnts]->Awake();
		}
		return 1;
	...
	if (_action->type==pe_action_impulse::type_id) {
		...
		if ((u32)i>=(u32)m_nParts) {
			if (is_unused(action->point)) 
				...
				if ((u32)i>=(u32)m_nParts) return 0;
			}
		}

		if ((u32)i>=(u32)m_nParts || (u32)(j=m_infos[i].iJoint)>=(u32)m_nJoints || (!action->iSource && m_joints[j].flags & joint_ignore_impulses))
			return 0;
		if (action->iSource==0) {
	...
	if (_action->type==pe_action_reset::type_id) {
		/*{ WriteLock lock(m_lockJoints);
			for(i32 idx=0;idx<m_nJoints;idx++) {
...
i32 CArticulatedEntity::GetPotentialColliders(CPhysicalEntity **&pentlist, float dt)
{
	pentlist = m_pCollEntList; return m_nCollEnts;
}

i32 CArticulatedEntity::CheckSelfCollision(i32 ipart0,i32 ipart1)
{
	return m_joints[m_infos[ipart0].iJoint].selfCollMask & getmask(ipart1);
}

RigidBody *CArticulatedEntity::GetRigidBody(i32 ipart,i32 bModify)
{
	return (u32)ipart<(u32)m_nParts ? &m_joints[m_infos[ipart].iJoint].body : &m_body;
}
RigidBody *CArticulatedEntity::GetRigidBodyData(RigidBody *pbody, i32 ipart) 
{ 
	RigidBody *pbodyInt = GetRigidBody(ipart);
	*pbody = *pbodyInt;
	if (m_iSimClass!=4 || (u32)ipart>=(u32)m_nParts)
		pbody->v += m_velHost;
	else {
...
void CArticulatedEntity::GetLocTransformLerped(i32 ipart, Vec3 &offs, quaternionf &q, float &scale, float k, const CPhysicalPlaceholder *trg) const 
{
...
void CArticulatedEntity::UpdatePosition(i32 bGridLocked)
{
	i32 i;
	WriteLock lock(m_lockUpdate); 
...
i32 CArticulatedEntity::SyncWithHost(i32 bRecalcJoints, float time_interval)
{
	if (m_pHost) {
...
i32 CArticulatedEntity::IsAwake(i32 ipart) const
{
	return (u32)ipart<(u32)m_nParts ? m_joints[m_infos[ipart].iJoint].bAwake : m_bAwake;
}


i32 CArticulatedEntity::GetUnprojAxis(i32 idx, Vec3 &axis)
{
	axis.Set(0,0,0);
	if (!(m_joints[idx].flags & (angle0_limit_reached|angle0_locked|angle0_gimbal_locked)*6)) // y and z axes are not locked
		return 1;
	else {
		i32 i; for(i=2; i>=0 && m_joints[idx].flags & (angle0_limit_reached|angle0_locked|angle0_gimbal_locked)<<i; i--);
		if (i>=0)	{
...
i32 CArticulatedEntity::Step(float time_interval)
{
	i32 i,j,bboxUpdated,bBounced=0;
	Vec3 gravity;
	i32 bWasAwake = m_bAwake;
	...
	if (!m_bFeatherstone)	{
		...
		UpdatePosition(m_pWorld->RepositionEntity(this,1,m_BBoxNew));
		gravity = m_gravity;
		i = PostStepNotify(time_interval,pb,DRX_ARRAY_COUNT(pb),iCaller);
		...
		if (!i)
			m_simTimeAux = 0;
		return UpdateHistory(i | isneg(3-(i32)m_nStepBackCount));
	}
	...
	UpdateJointDyn();
	gravity = m_gravity;
	PostStepNotify(time_interval,pb,DRX_ARRAY_COUNT(pb),iCaller);
	if ((m_gravity-gravity).len2()>0)
		m_gravityLyingMode = m_gravity;

	i32 bDone = UpdateHistory(isneg(m_timeStepFull-m_timeStepPerformed-0.001f));
	return bDone | isneg(m_pWorld->m_bWorldStep-2);
}


void CArticulatedEntity::UpdateJointRotationAxes(i32 idx)
{
...
void CArticulatedEntity::CheckForGimbalLock(i32 idx)
{
	i32 i;
	...
	if (!(m_joints[idx].flags & angle0_locked*5) && sqr(m_joints[idx].rotaxes[0]*m_joints[idx].rotaxes[2])>0.999f*0.999f) {
		...
		{ // if joint is 3dof w/o limits, just rotate its quat0 to avoid gimbal lock
			...
			i = iszero((i32)m_joints[idx].flags & angle0_limit_reached) << 1; // gimbal lock angle that already reached its limit, if this is the case
...
void CArticulatedEntity::SyncBodyWithJoint(i32 idx, i32 flags)
{
...
void CArticulatedEntity::SyncJointWithBody(i32 idx, i32 flags)
{
...
i32 CArticulatedEntity::CalcBodyZa(i32 idx, float time_interval, vectornf &Za_change)
{
	i32 i,j,curidx,nextidx,idir;
...
i32 CArticulatedEntity::CalcBodyIa(i32 idx, matrixf& Ia_change, i32 bIncludeLimits)
{
	i32 i,j,curidx,nextidx;
	Vec3 d,r,pos_parent;
	quaternionf q_parent;
...
i32 CArticulatedEntity::CollectPendingImpulses(i32 idx,i32 &bNotZero,i32 bBounce)
{
	i32 i,j,curidx,newidx,bChildNotZero;
	bNotZero = 0;
...
void CArticulatedEntity::PropagateImpulses(const Vec3 &dv, i32 bLockLimits, i32 bApplyVel, const Vec3 &dw)
{
	i32 idx,i,j,bHitsLimit;
	Vec3 pos_parent;
...
void CArticulatedEntity::CalcBodiesIinv(i32 bLockLimits)
{
	i32 idx,i,nAngles;
...
i32 CArticulatedEntity::StepJoint(i32 idx, float time_interval,i32 &bBounced,i32 bFlying, i32 iCaller)
{
	i32 i,j,idxpivot,ncont,itmax,curidx,sgq,bSelfColl,bFreeFall=isneg(-m_nBodyContacts)^1;
	...
	if (m_bCheckCollisions) {// && m_joints[idx].bAwake) {
			...
			m_joints[idx].body.Minv>m_body.Minv*10.0f) 
		{
			...
			for(i32 ipart=m_joints[idx].iStartPart; ipart<m_joints[idx].iStartPart+m_joints[idx].nParts; ipart++)
...
void CArticulatedEntity::StepFeatherstone(float time_interval, i32 bBounced, Matrix33 &M0host)
{
	i32 i;
	...
	if (m_nColliders && !nPhysColl) {
		...
		entity_contact **pContacts; i32 nContacts=0;
		InvokeContactSolver(time_interval,&m_pWorld->m_vars,Ebefore, pContacts,nContacts);
		for(i=0; i<nContacts; i++) for(i32 j=0;j<2;j++) 
...
void CArticulatedEntity::StepBack(float time_interval)
{
	if (m_simTime>0) {
		...
		for(i32 i=0;i<m_nJoints;i++) {
			...
			for(i32 j=m_joints[i].iStartPart; j<m_joints[i].iStartPart+m_joints[i].nParts; j++) {
...
void CArticulatedEntity::DrawHelperInformation(IPhysRenderer *pRenderer, i32 flags)
{
	if (!pRenderer) return;
	i32 idsel = -1;
	if (flags & 1<<28) {
	...
	if(flags&0x10){
		for(i32 i=0;i<m_nJoints;i++) if ((m_joints[i].idbody | idsel>>31)==idsel) {
			quaternionf q_parent;
			...
			i32 ipart = m_joints[i].iStartPart;
...
i32 CArticulatedEntity::GetStateSnapshot(CStream &stm,float time_back,i32 flags)
{
	stm.WriteNumberInBits(SNAPSHOT_VERSION,4);
	stm.Write((u8)m_nJoints);
	stm.Write(m_pos);
	stm.Write(m_body.v);
	...
	for(i=0;i<m_nParts;i++) {
		...
		stm.WriteBits((u8*)&m_parts[i].q,sizeof(m_parts[i].q)*8);
	}
	return 1;
#endif
	...
	for(i=0;i<m_nJoints;i++) {
		...
		stm.WriteBits((u8*)&m_joints[i].body.q,sizeof(m_joints[i].body.q)*8);
		if (m_joints[i].bQuat0Changed) {
			stm.Write(true);
			stm.WriteBits((u8*)&m_joints[i].quat0, sizeof(m_joints[i].quat0)*8);
...
i32 CArticulatedEntity::SetStateFromSnapshot(CStream &stm, i32 flags)
{
	i32 i=0,j,ver=0; 
	bool bnz;
	Matrix33 R;
...
#ifdef DEBUG_BONES_SYNC
stm.ReadBits((u8*)&m_qrot,sizeof(m_qrot)*8);
for(i=0;i<m_nParts;i++) {
	stm.Read(m_parts[i].pos);
	stm.ReadBits((u8*)&m_parts[i].q,sizeof(m_parts[i].q)*8);
}
#else
		for(i=0;i<m_nJoints;i++) {
			stm.Read(bnz);
			...
			stm.ReadBits((u8*)&m_joints[i].body.q,sizeof(m_joints[i].body.q)*8);
			stm.Read(bnz); if (bnz)	{
				stm.ReadBits((u8*)&m_joints[i].quat0,sizeof(m_joints[i].quat0)*8);
...
i32 CArticulatedEntity::GetStateSnapshot(TSerialize ser, float time_back, i32 flags)
{
...
i32 CArticulatedEntity::SetStateFromSnapshot(TSerialize ser, i32 flags)
{
	if (ser.GetSerializationTarget() == eST_Network)
	{
		pe_action_awake aa;
...
i32 CollectPendingImpulses(strided_pointer<ArticulatedBody> joints, i32 idx)
{
	i32 i,curidx,newidx;
...
{
	for(i32 idx=0,iparent; idx<=joints[0].nChildrenTree; idx++) {
...
void ArticulatedBody::ApplyImpulse(const Vec3& dP, const Vec3& dL, body_helper *bodies, i32 iCaller)
{
	Vec3 dPspat[2] = { dP, dL };
	Pext += dP; Lext += dL;
	...
	if (fs->useTree<0) {
		...
		i32 useTree = 0;
		for(i32 i=0; i<=joints[0].nChildrenTree; i++) if (joints[i].body.bProcessed[iCaller])
			for(i32 j=i; j>0; j=joints[j].iParent) {
				useTree = max(useTree, joints[j].fs->useTree);
				joints[j].fs->useTree = 1;
			}
		if (!useTree)
			for(i32 i=0; i<=joints[0].nChildrenTree; joints[i++].fs->useTree>>=31);
	}

	if (!fs->useTree) {
		vectornf(6,&body.w.x) -= matrixf(6,6,0,fs->Iinv[0])*vectornf(6,&dPspat[0].x);
		i32 ibody = body.bProcessed[iCaller]-1;
		...
		for(i32 i=0,j;i<=joints[0].nChildrenTree;i++) if (j=joints[i].body.bProcessed[iCaller]) {
...
void CArticulatedEntity::OnContactResolved(entity_contact *pcontact, i32 iop, i32 iGroupId)
{
...
void CArticulatedEntity::GetJointTorqueResponseMatrix(i32 idx, Matrix33 &K)
{
...
entity_contact *CArticulatedEntity::CreateConstraintContact(i32 idx)
{
...
void CArticulatedEntity::AssignContactsToJoints()
{
	if (!m_bContactsAssigned) {
		for(i32 i=0;i<m_nJoints;i++) m_joints[i].pContact = 0;
		for(entity_contact *pContact=m_pContactStart; pContact!=CONTACT_END(m_pContactStart); pContact=pContact->next) if (!(pContact->flags & contact_2b_verified)) {
			i32 j = m_infos[pContact->ipart[0]].iJoint;
...
i32 CArticulatedEntity::RegisterContacts(float time_interval,i32 nMaxPlaneContacts)
{
	m_Ejoints = 0;
	i32 flags = 0;
	if (m_bFeatherstone) {
		if (!m_joints[0].fs)
			return 0;
		DisablePreCG();
		i32 useTree = m_joints[0].nPotentialAngles==0 && m_M0inv.IsZero() ? -1:1;
		for(i32 i=0;i<m_nJoints;i++) {
...
__ae_step++;
	...
	for(idx=0;idx<m_nJoints;idx++) {
		...
		if (m_constraintMask&getmask(i) && m_pConstraintInfos[i].bActive && 
				(u32)(m_pConstraints[i].ipart[0]-m_joints[idx].iStartPart) < (u32)m_joints[idx].nParts)
		...
		if (!m_bFeatherstone && (m_bGrounded || m_joints[idx].iParent>=0)) {
			...
			for(i=j=0,ks=kd=0;i<3;i++) if (!(m_joints[idx].flags & (angle0_locked|angle0_gimbal_locked)<<i)) {
				...
				kd = max(kd, m_joints[idx].kdashpot[i]*(1^iszero((i32)m_joints[idx].flags & joint_dashpot_reached))); 
...
i32 CArticulatedEntity::Update(float time_interval, float damping)
{
	if (m_bFeatherstone && m_nJoints>0) {
		...
		if (m_joints[0].fs) {
			...
			if (active) {
				...
				for(i32 i=0;i<m_nJoints;i++) {
					for(i32 j=0;j<m_joints[i].nActiveAngles;j++) 
	...
	i32 i,j,nCollJoints=0,bPosChanged=0, bFloating=m_bFloating|iszero(m_gravity.len2()); 
	...
	for(pContact=m_pContactStart,i=0; pContact!=CONTACT_END(m_pContactStart); pContact=pContact->next) {
		...
		i |= isneg(sqr_signed(pContact->n*m_gravity)+m_gravity.len2()*0.5f) & iszero((i32)pContact->pent[1]->m_iSimClass*2+(i32)((CRigidEntity*)pSafePtr)->m_bAwake-5);
	}
	minEnergy *= 1-i*0.9f;
	...
	if (m_body.M>0) {
		...
		m_nSleepFrames = (m_nSleepFrames&~-(i32)m_bAwake) + (m_bAwake^1);
		m_bAwake |= isneg((i32)m_nSleepFrames-4) & isneg(nCollJoints-3);
...
void CArticulatedEntity::GetMemoryStatistics(IDrxSizer *pSizer) const
{
	...
	for(i32 i=0; i<m_nJoints; i++) if (m_joints[i].fsbuf)
