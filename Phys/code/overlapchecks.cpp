...
{
	for(i32 i=0;i<NPRIMS;i++) for(i32 j=0;j<NPRIMS;j++)
...
i32 default_overlap_check(const primitive*, const primitive *, COverlapChecker*)
{
	return 1;
}

i32 box_box_overlap_check(const box *box1, const box *box2, COverlapChecker *pOverlapper)
{
...
i32 box_heightfield_overlap_check(const box *pbox, const heightfield *phf, COverlapChecker*)
{
	box boxtr;
	Vec3 vtx[4];
	triangle hftri;
	Vec2 sz,ptmin,ptmax;
	i32 ix,iy,ix0,iy0,ix1,iy1;
	float hmax;
...
i32 heightfield_box_overlap_check(const heightfield *phf, const box *pbox, COverlapChecker *pOverlapper) 
{
	return box_heightfield_overlap_check(pbox,phf,pOverlapper);
}

i32 box_voxgrid_overlap_check(const box *pbox, const voxelgrid *pgrid, COverlapChecker*)
{
	i32 i,j,nTris=0,nTrisDst,icell;
	box boxtr;
	Vec3 dim;
	Vec3i ic,iBBox[2];
	i32k MAXTESTRIS = 8;
	i32 idxbuf[(MAXTESTRIS+1)*2], *plist=idxbuf, *plistDst=idxbuf+MAXTESTRIS+1;
	triangle atri;
	...
	for(i=0;i<nTris;i++) {
		for(j=0;j<3;j++) atri.pt[j] = pgrid->pVtx[pgrid->pIndices[plist[i]*3+j]];
...
i32 voxgrid_box_overlap_check(const voxelgrid *pgrid, const box *pbox, COverlapChecker *pOverlapper) 
{
	return box_voxgrid_overlap_check(pbox,pgrid,pOverlapper);
}

i32 box_tri_overlap_check(const box *pbox, const triangle *ptri, COverlapChecker*)
{
	Vec3 pt[3],n;
	float l1,l2,l3,l,c;
	if (pbox->bOriented) {
...
i32 tri_box_overlap_check(const triangle *ptri, const box *pbox, COverlapChecker *pOverlapper)
{
	return box_tri_overlap_check(pbox,ptri,pOverlapper);
}

i32 box_ray_overlap_check(const box *pbox, const ray *pray, COverlapChecker*)
{
	Vec3 l,m,al;
	if (pbox->bOriented) {
...
i32 ray_box_overlap_check(const ray *pray, const box *pbox, COverlapChecker *pOverlapper)
{
	return box_ray_overlap_check(pbox,pray,pOverlapper);
}

i32 box_sphere_overlap_check(const box *pbox, const sphere *psph, COverlapChecker*)
{
	Vec3 center, dist;
	if (pbox->bOriented)
...
i32 sphere_box_overlap_check(const sphere *psph, const box *pbox, COverlapChecker *pOverlapper)
{
...
i32 tri_sphere_overlap_check(const triangle *ptri, const sphere *psph, COverlapChecker*)
{
	i32 bFace;
...
i32 sphere_tri_overlap_check(const sphere *psph, const triangle *ptri, COverlapChecker *pOverlapper)
{
	return tri_sphere_overlap_check(ptri,psph,pOverlapper);
}

i32 sphere_sphere_overlap_check(const sphere *psph1, const sphere *psph2, COverlapChecker*)
{
	return isneg((psph1->center-psph2->center).len2()-sqr(psph1->r+psph2->r));
}

i32 heightfield_sphere_overlap_check(const heightfield *phf, const sphere *psph, COverlapChecker*)
{
	Vec3 center;
	Vec2i irect[2];
	i32 ix,iy,bContact=0;
...
i32 sphere_heightfield_overlap_check(const sphere *psph, const heightfield *phf, COverlapChecker *pOverlapper)
{
