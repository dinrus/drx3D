...
i32 CWheeledVehicleEntity::SetParams(pe_params *_params, i32 bThreadSafe)
{
    ...
    if (!is_unused(params->minBrakingFriction)) m_minBrakingFriction = params->minBrakingFriction;
		...
		if (!is_unused(params->gearRatios)) for(i32 i=0;i<m_nGears;i++) m_gears[i] = params->gearRatios[i];
	...
	if (_params->type==pe_params_wheel::type_id) {
		...
		WriteLock lock(m_lockVehicle);
		i32 iWheel = max(0,min(m_nParts-m_nHullParts-1,params->iWheel));
...
i32 CWheeledVehicleEntity::GetParams(pe_params *_params) const
{
	if (_params->type==pe_params_car::type_id) {
		pe_params_car *params = (pe_params_car*)_params;
    ...
    params->minBrakingFriction = m_minBrakingFriction;
		...
		if (!is_unused(params->gearRatios))
			for(i32 i=0;i<m_nGears;i++) params->gearRatios[i] = m_gears[i];
	...
	if (_params->type==pe_params_wheel::type_id) {
		pe_params_wheel *params = (pe_params_wheel*)_params;
		ReadLock lock(m_lockVehicle);
		if ((u32)params->iWheel>=(u32)(m_nParts-m_nHullParts))
			return 0;
		i32 iWheel = params->iWheel;
...
i32 CWheeledVehicleEntity::Action(pe_action *_action, i32 bThreadSafe)
{
	...
	if (res = CRigidEntity::Action(_action,1)) {
		/*if (_action->type==pe_action_impulse::type_id && ((pe_action_impulse*)_action)->iSource!=1)
			...
			for(i32 i=0;i<m_nParts-m_nHullParts;i++) 
	...
	if (_action->type==pe_action_drive::type_id) {
		...
		if (!is_unused(action->steer) || !is_unused(action->dsteer)) {
			...
			if (m_kSteerToTrack==0) {
				if (m_steer!=0) {	// apply Ackerman steering to front wheels, ackermanOffset = 0.0 -> back wheels fixed, 0.5 -> front+back wheels steer, 1.0 -> front wheels fixed
					...
					i32k numWheels = m_nParts - m_nHullParts;
					float ackermanOffset, maxX=0.f, maxY=-10.f, minY=+10.f, dir=fsgnf(m_steer);
					for (i32 j=0;j<numWheels;j++) if (m_susp[j].iAxle>=0 && m_susp[j].bCanSteer) {
		...
		i32 bPrevHandBrake = m_bHandBrake;
...
i32 CWheeledVehicleEntity::GetStatus(pe_status* _status) const
{
	i32 res;
	if (_status->type==pe_status_pos::type_id) {
		pe_status_pos *status = (pe_status_pos*)_status;
		i32 iwheel,i,iLock=0,flags=status->flags;
		...
		if ((u32)iwheel<(u32)(m_nParts-m_nHullParts)) {
	...
	if (_status->type==pe_status_vehicle::type_id) {
		...
		status->vel = m_body.v;
		i32 i; for(i=status->bWheelContact=0; i<m_nParts-m_nHullParts; i++)
	...
	if (_status->type==pe_status_wheel::type_id) {
		...
		if ((u32)status->iWheel>=(u32)(m_nParts-m_nHullParts))
			return 0;
...
void CWheeledVehicleEntity::RecalcSuspStiffness()
{
	...
	i32k numWheels = m_nParts - m_nHullParts;
	i32 i,j, *idx = (i32*)alloca(numWheels*sizeof(i32)); 
	...
	for(i=0;i<numWheels;i++) {
		...
		for(j=i+1;j<numWheels;j++) if (m_susp[j].iAxle==max(0,(i32)m_susp[i].iAxle)) {
...
i32 CWheeledVehicleEntity::AddGeometry(phys_geometry *pgeom, pe_geomparams *_params, i32 id, i32 bThreadSafe)
{
	if (!pgeom)
		return 0;
	...
	i32 flags0=params->flags, flagsCollider0=params->flagsCollider;
	...
	if (params->bDriving<0) {
		...
		i32 idx = m_nParts-1-m_nHullParts;
		if (idx>=m_suspAlloc) {
...
void CWheeledVehicleEntity::RemoveGeometry(i32 id, i32 bThreadSafe)
{
...
void CWheeledVehicleEntity::AlertNeighbourhoodND(i32 mode) 
{
...
void CWheeledVehicleEntity::ComputeBBox(Vec3 *BBox, i32 flags) 
{ 
...
void CWheeledVehicleEntity::CheckAdditionalGeometry(float time_interval)
{
	...
	i32 i,j,nents,ient,iwheel,ncontacts,icont,bRayCast,bHasContacts,ient1,j1,bHasMatSubst=0;
	i32 bFakeInnerWheels, iwMiny[2]={-1,-1},iwMaxy[2]={-1,-1};
			...
			for(icont=0; icont<ncontacts; icont++) {
				...
				if (fabs_tpl(pcontacts[icont].n*axisy) < m_maxTilt) {
					if (tcur>tmax/*&& (ip.bSweepTest || pcontacts[icont].vel>0)*/)	{
						...
						i32 scMask = (2<<m_susp[iwheel].pent->m_iSimClass)-(m_susp[iwheel].pent->m_id>>31);
						if (bHasMatSubst & scMask) for(ient1=0;ient1<nents;ient1++) for(i32 j2=0;j2<pentlist[ient1]->GetUsedPartsCount(iCaller);j2++)
...
float CWheeledVehicleEntity::ComputeDrivingTorque(float time_interval)
{
	...
	i32 i,iNewGear,nContacts,sg=sgnnz(m_gears[m_iCurGear]);
	...
	iNewGear = max((i32)m_minGear,min((i32)m_maxGear, (i32)iNewGear));
...
i32 g_iwhist=0,g_checksum[NH],g_ncompare=0,g_bstartcompare=0,g_histinit=0,g_iwhist0=-1;
i32 g_forcepedal = 0;*/


void CWheeledVehicleEntity::AddAdditionalImpulses(float time_interval)
{
	i32 i,j,/*idx[NMAXWHEELS],nContacts,*/bAllSlip=1,iDriver[2],bContact[2],iside;//,i1,j1,nfr=0,nfr1,slide[16];
...
//			idx[nContacts++] = i;
	...
	if ((m_kSteerToTrack!=0 && bContact[iside=isneg(m_susp[i].pt.x)] && m_susp[i].bDriving || m_susp[i].bContact) && m_susp[i].iAxle>=0) {
		...
		iDriver[iside] |= i & -(i32)m_susp[i].bDriving;
	...
	/*if (nfr>0) {
		...
		float sign; i32 d,prev_j,bSliding=0;
...
i32 CWheeledVehicleEntity::RegisterContacts(float time_interval,i32 nMaxPlaneContacts)
{
...
void CWheeledVehicleEntity::OnContactResolved(entity_contact *pcontact, i32 iop, i32 iGroupId)
{
...
float CWheeledVehicleEntity::GetMaxTimeStep(float time_interval) 
{
	...
	m_maxAllowedStep *= 1.0f-0.25f*(-((i32)m_flags & pef_invisible)>>31);
...
i32 CWheeledVehicleEntity::HasContactsWith(CPhysicalEntity *pent)
{
	i32 i; 
...
i32 CWheeledVehicleEntity::HasPartContactsWith(CPhysicalEntity *pent, i32 ipart, i32 bGreaterOrEqual)
{
	for(i32 i=0;i<m_nParts-m_nHullParts;i++)
...
i32 CWheeledVehicleEntity::RemoveCollider(CPhysicalEntity *pCollider, bool bRemoveAlways)
{
	for(i32 i=0;i<m_nParts-m_nHullParts;i++) if (m_susp[i].pent==pCollider) {
...
i32 CWheeledVehicleEntity::Update(float time_interval, float damping)
{
	if (m_nContacts)
		m_timeNoContacts = 0;
	else 
		m_timeNoContacts += time_interval;
	i32 i,bWheelContact=0;
	for(i=0;i<m_nParts-m_nHullParts;i++) 
...
i32 CWheeledVehicleEntity::GetStateSnapshot(CStream &stm, float time_back, i32 flags)
{
...
i32 CWheeledVehicleEntity::SetStateFromSnapshot(CStream &stm, i32 flags)
{
	i32 res = CRigidEntity::SetStateFromSnapshot(stm,flags);
	if (res && res!=2) {
		bool bnz; i32 gear;
...
void SWheeledVehicleEntityNetSerialize::Serialize( TSerialize ser, i32 nSusp )
{
	if (ser.GetSerializationTarget()!=eST_Network) {
		ser.BeginGroup("wheels");
		for (i32 i=0; i<nSusp; i++)
...
i32 CWheeledVehicleEntity::GetStateSnapshot(TSerialize ser, float time_back, i32 flags)
{
...
i32 CWheeledVehicleEntity::SetStateFromSnapshot(TSerialize ser, i32 flags)
{
...
void CWheeledVehicleEntity::DrawHelperInformation(IPhysRenderer *pRenderer, i32 flags)
{
	CRigidEntity::DrawHelperInformation(pRenderer, flags);

	if (flags & pe_helper_collisions) {
		for(i32 i=0;i<m_nParts-m_nHullParts;i++) if (m_susp[i].bContact && m_susp[i].iAxle>=0)
			pRenderer->DrawLine(m_susp[i].ptcontact, m_susp[i].ptcontact + m_susp[i].ncontact*m_pWorld->m_vars.maxContactGap*30, m_iSimClass);
	}
}


void CWheeledVehicleEntity::GetMemoryStatistics(IDrxSizer *pSizer) const
{
