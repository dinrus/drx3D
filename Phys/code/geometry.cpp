...
#include <drx3D/Phys/physicalworld.h>
 ...
 i32 *g_pLockIntersect;
class CPhysicalWorld; 
extern CPhysicalWorld* g_pPhysWorlds[]; 


DRX_ALIGN(128) intersData g_idata[MAX_PHYS_THREADS+1];
...
i32 IntersectBVs(geometry_under_test *pGTest, BV *pBV1, BV *pBV2) 
{
  MEMSTREAM_DEBUG_ASSERT(pGTest && pBV1 && pBV2);
	intptr_t mask;
	i32 iNode[2],*pUsedNodesMap[2],res=0,nullmap=0,bNodeUsed[2],*pnullmap=&nullmap;
	mask = iszero_mask(pGTest[0].pUsedNodesMap); // mask = -(pUsedNodeMap==0)
	pUsedNodesMap[0] = (i32*)((intptr_t)pGTest[0].pUsedNodesMap&~mask | (intptr_t)pnullmap&mask);
	iNode[0] = pBV1->iNode&~mask;
	mask = iszero_mask(pGTest[1].pUsedNodesMap); // mask = -(pUsedNodeMap==0)
	pUsedNodesMap[1] = (i32*)((intptr_t)pGTest[1].pUsedNodesMap&~mask | (intptr_t)pnullmap&mask);
  ...
  MEMSTREAM_DEBUG_ASSERT(pGTest[0].pBVtree && pGTest[1].pBVtree);
    ...
    MEMSTREAM_DEBUG_ASSERT(pBV_split2->type>=0 && pBV_split2->type<NPRIMS);
		...
		i32 nPrims1,nPrims2,iClient,i,j;
		char *ptr[2];
		...
		inters.nbordersz = DRX_ARRAY_COUNT(ptborder_loc)-1;
		...
		for(i=0,ptr[0]=(tuk)pGTest[0].primbuf; i<nPrims1; i++,ptr[0]=(ptr[0]+pGTest[0].szprim)) 
			for(j=0,ptr[1]=(tuk)pGTest[1].primbuf; j<nPrims2; j++,ptr[1]=(ptr[1]+pGTest[1].szprim))
...
i32 SweepTestBVs(geometry_under_test *pGTest, BV *pBV1, BV *pBV2) 
{
  MEMSTREAM_DEBUG_ASSERT(pGTest && pBV1 && pBV2);
	if (!g_Overlapper.Check(pBV1->type,pBV2->type, *pBV1,*pBV2))
		return 0;
...
i32 RadiusCheckBVs(radius_check_data *prcd, BV *pBV)
{
	...
	if (prcd->pBVtree->SplitPriority(pBV)>0) {
		...
		i32 res = RadiusCheckBVs(prcd, pBV_split1)+RadiusCheckBVs(prcd, pBV_split2);
		prcd->pBVtree->ReleaseLastBVs(prcd->iCaller);
		return res;
	} else {
		i32 iStartPrim,nPrims;
...
float CGeometry::BuildOcclusionCubemap(geom_world_data *pgwd, i32 iMode, SOcclusionCubeMap* cubemap0, SOcclusionCubeMap* cubemap1, i32 nGrow)
{
...
i32 CGeometry::Intersect(IGeometry *pCollider, geom_world_data *pdata1,geom_world_data *pdata2, intersection_params *pparams, geom_contact *&pcontacts)
{
	//DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );
	geometry_under_test gtest[2];
	geom_world_data *pdata[2] = { pdata1,pdata2 };
	i32 i,j,jmax,mask,nContacts=0,iStartNode[2],bActive=0,iCaller;
	...
	if (!pparams->bNoAreaContacts && g_nAreas<DRX_ARRAY_COUNT(g_AreaBuf) && g_nAreaPt<DRX_ARRAY_COUNT(g_AreaPtBuf)) {
		...
		pcontacts[0].parea->npt = 0; pcontacts[0].parea->nmaxpt = DRX_ARRAY_COUNT(g_AreaPtBuf)-g_nAreaPt;
	} else
		pcontacts[0].parea = 0;
	...
	if (!pparams->bSweepTest) {
		...
		i32 iClient = -(gtest[1].pGeometry->m_iCollPriority-gtest[0].pGeometry->m_iCollPriority>>31);
		if (iClient) for(i=0;i<nContacts;i++) {
			if (pcontacts[i].parea && pcontacts[i].parea->npt) {
				...
				i32 *pprim=pcontacts[i].parea->piPrim[0]; pcontacts[i].parea->piPrim[0]=pcontacts[i].parea->piPrim[1]; pcontacts[i].parea->piPrim[1]=pprim;
				i32 *pfeat=pcontacts[i].parea->piFeature[0]; pcontacts[i].parea->piFeature[0]=pcontacts[i].parea->piFeature[1]; 
			...
			i32 iprim=pcontacts[i].iPrim[0]; pcontacts[i].iPrim[0]=pcontacts[i].iPrim[1]; pcontacts[i].iPrim[1]=iprim;
			i32 ifeature=pcontacts[i].iFeature[0]; pcontacts[i].iFeature[0]=pcontacts[i].iFeature[1]; pcontacts[i].iFeature[1]=ifeature;
		}
...
i32 CGeometry::RegisterIntersection(primitive *pprim1,primitive *pprim2, geometry_under_test *pGTest1,geometry_under_test *pGTest2, 
																		prim_inters *pinters)
{
	unprojection_mode unproj;
	contact contact_cur;
	...
	if (g_nAreas<DRX_ARRAY_COUNT(g_AreaBuf) && g_nAreaPt<DRX_ARRAY_COUNT(g_AreaPtBuf)) {
...
void DrawBBox(IPhysRenderer *pRenderer, i32 idxColor, geom_world_data *gwd, CBVTree *pTree,BBox *pbbox,i32 maxlevel,i32 level, i32 iCaller)
{
...
i32 CPrimitive::Intersect(IGeometry *_pCollider, geom_world_data *pdata1, geom_world_data *pdata2, 
													intersection_params *pparams, geom_contact *&pcontacts)
{
	...
	if (max(g_BrdPtBufPos+8-(i32)(DRX_ARRAY_COUNT(g_BrdPtBuf)), g_nTotContacts-(i32)g_maxContacts)>=0)
		return 0;
	pcontacts = g_Contacts+g_nTotContacts;
	...
	if (!pparams->bNoAreaContacts && g_nAreas<DRX_ARRAY_COUNT(g_AreaBuf) && g_nAreaPt<DRX_ARRAY_COUNT(g_AreaPtBuf)) {
		...
		parea->npt = 0; parea->nmaxpt = DRX_ARRAY_COUNT(g_AreaPtBuf)-g_nAreaPt;
		...
		if (pparams->iUnprojectionMode==0) {
			if (pparams->vrel_min<1E9f) {
				...
				if (!pcontacts->nborderpt && pdata1->w.len2()+pdata2->w.len2()>0) {
					...
					i32 iPrim,iFeature; // dummy parameters
...
i32 SphereCheckBVs(CBVTree *pBVtree, sphere *psph, BV *pBV, i32 iCaller)
{
	...
	if (pBVtree->SplitPriority(pBV)>0) {
		...
		i32 res = SphereCheckBVs(pBVtree,psph,pBV_split1,iCaller);
		if (!res)
...
i32 CGeometry::SphereCheck(const Vec3 &center, float r, i32 iCaller)
{
	ResetGlobalPrimsBuffers(iCaller);
	...
	if (IsAPrimitive()) {
		geom_world_data gwd;
		primitive *pprim;
		i32 itype = PreparePrimitive(&gwd,pprim,iCaller);
...
i32 CGeometry::IntersectLocked(IGeometry *pCollider, geom_world_data *pdata1,geom_world_data *pdata2, intersection_params *pparams, geom_contact *&pcontacts, 
		WriteLockCond &lock,i32 iCaller)
{
	...
	i32 res = Intersect(pCollider, pdata1,pdata2, pparams, pcontacts);
	if (!res) {
...
void TestBV(CBVTree* pBVtree, BV *pBV, i32 iCaller, i32 level, bool& okay, i32 maxDepth)
{
...
i32 SanityCheckTree(CBVTree* pBVtree, i32 maxDepth)
{
	BV* pBV;
	bool okay = true;
	i32 iCaller = get_iCaller(), locked=0;
	if (iCaller==MAX_PHYS_THREADS && (!IsPODThread(g_pPhysWorlds[0]) || !*g_pLockIntersect))
