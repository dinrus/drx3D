...
void CHeightfieldBV::GetNodeBV(BV *&pBV,i32 iNode,i32 iCaller)
{
	pBV = &g_BVhf;
	g_BVhf.hf = *m_phf;
...
void CHeightfieldBV::GetNodeBV(const Matrix33 &Rw,const Vec3 &offsw,float scalew, BV *&pBV, i32 iNode,i32 iCaller)
{
	pBV = &g_BVhf;
	g_BVhf.hf = *m_phf;
...
void CHeightfieldBV::MarkUsedTriangle(i32 itri, geometry_under_test *pGTest)
{
	m_pUsedTriMap[itri>>5] |= 1u<<(itri&31);
}

i32 CHeightfieldBV::GetNodeContents(i32 iNode, BV *pBVCollider,i32 bColliderUsed,i32 bColliderLocal, 
																		geometry_under_test *pGTest,geometry_under_test *pGTestOp)
{
	i32 iStart,nCols,nRows,nPrims,ix,iy,i,j;
	float minz;
	...
	if (!bColliderUsed) {
      ...
      char *primbuf = (tuk)pGTest->primbuf;
