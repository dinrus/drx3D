...
void CAABBTree::SetParams(i32 nMinTrisPerNode, i32 nMaxTrisPerNode, float skipdim, const Matrix33 &Basis, i32 planeOptimisation)
{
...
float CAABBTree::Build(CGeometry *pMeshIn)
{
	...
	for(i=0; i<nTris; i++) {
		i32k i0 = pMesh->m_pIndices[i*3+0];
		i32k i1 = pMesh->m_pIndices[i*3+1];
		i32k i2 = pMesh->m_pIndices[i*3+2];
		...
		const float minX = min(min(v0.x, v1.x), v2.x);
		const float minY = min(min(v0.y, v1.y), v2.y);
		const float minZ = min(min(v0.z, v1.z), v2.z);
		const float maxX = max(max(v0.x, v1.x), v2.x);
		const float maxY = max(max(v0.y, v1.y), v2.y);
		const float maxZ = max(max(v0.z, v1.z), v2.z);
		bbtri[i][0] = Vec3(minX, minY, minZ);
		bbtri[i][1] = Vec3(maxX, maxY, maxZ);
		bbtri[i][2] = (v0+v1+v2)*(1.0/3.0);
		ptmin.x = min(ptmin.x,minX); ptmax.x = max(ptmax.x,maxX);
		ptmin.y = min(ptmin.y,minY); ptmax.y = max(ptmax.y,maxY);
		ptmin.z = min(ptmin.z,minZ); ptmax.z = max(ptmax.z,maxZ);
	}
	...
	m_size.Set(max(m_size.x,mindim), max(m_size.y,mindim), max(m_size.z,mindim));

	if (m_axisSplitMask==0) {	// Enable plane optimisation, turn off smallest dimension for splitting
		i32k minIdx = idxmin3(m_size); i32k minIdx1 = dec_mod3[minIdx], minIdx2 = inc_mod3[minIdx];
	...
	i32 *pNewTri2Node = new i32[(nTris-1>>5-m_nBitsLog)+1];
	memset(pNewTri2Node,0,sizeof(i32)*((nTris-1>>5-m_nBitsLog)+1));
	for(i=0;i<nTris;i++)
...
float CAABBTree::BuildNode(i32 iNode, i32 iTriStart,i32 nTris, Vec3 center,Vec3 size, Vec3 (*bbtri)[3], i32 nDepth)
{
	i32 i,j;
	...
	for(i=iTriStart;i<(iTriStart+nTris);i++) {
		ptmin.x = min_safe(ptmin.x,bbtri[i][0].x); ptmax.x = max_safe(ptmax.x,bbtri[i][1].x);
		ptmin.y = min_safe(ptmin.y,bbtri[i][0].y); ptmax.y = max_safe(ptmax.y,bbtri[i][1].y);
		ptmin.z = min_safe(ptmin.z,bbtri[i][0].z); ptmax.z = max_safe(ptmax.z,bbtri[i][1].z);
	}
	...
	if (size.x>mindim) {
		...
		m_pNodes[iNode].minx = min((i32)m_pNodes[iNode].minx,(i32)m_pNodes[iNode].maxx);
		m_pNodes[iNode].maxx = max((i32)m_pNodes[iNode].minx,(i32)m_pNodes[iNode].maxx);
	...
	if (size.y>mindim) {
		...
		m_pNodes[iNode].miny = min((i32)m_pNodes[iNode].miny,(i32)m_pNodes[iNode].maxy);
		m_pNodes[iNode].maxy = max((i32)m_pNodes[iNode].miny,(i32)m_pNodes[iNode].maxy);
	...
	if (size.z>mindim) {
		...
		m_pNodes[iNode].minz = min((i32)m_pNodes[iNode].minz,(i32)m_pNodes[iNode].maxz);
		m_pNodes[iNode].maxz = max((i32)m_pNodes[iNode].minz,(i32)m_pNodes[iNode].maxz);
...
#if (DRX_PLATFORM_WINDOWS && DRX_PLATFORM_64BIT) || (DRX_PLATFORM_LINUX && DRX_PLATFORM_64BIT) || DRX_PLATFORM_APPLE
	 // compiler bug workaround?
#endif
	i32 iAxis, allowedAxis=-1;
	i32 numtris[3],/*nTrisAx[3],*/iPart,iMode[3]={2,2,2},idx;
	...
	if (m_axisSplitMask>=0) {	// As an optimisation, choose the largest dimension that is not blocked
		i32k i0 = m_axisSplitMask&3, i1 = (m_axisSplitMask>>2)&3;
	...
	for(i=j=iTriStart;i<iTriStart+nTris;i++) {
		...
		if ((bbtri[i][iPart][iAxis] - cx)<0.f) {
			...
			bbtri[i][0] = bbtri[j][0]; bbtri[i][1] = bbtri[j][1]; bbtri[i][2] = bbtri[j][2];
			bbtri[j][0] = tmp0; bbtri[j][1] = tmp1; bbtri[j][2] = tmp2;
...
void CAABBTree::GetNodeBV(BV *&pBV,i32 iNode, i32 iCaller)
{
...
void CAABBTree::GetNodeBV(BV *&pBV, const Vec3 &sweepdir,float sweepstep, i32 iNode, i32 iCaller)
{
...
void CAABBTree::GetNodeBV(const Matrix33 &Rw,const Vec3 &offsw,float scalew, BV *&pBV, i32 iNode, i32 iCaller)
{
	pBV = g_BBoxBuf + min(g_BBoxBufPos++, k_BBoxBufSize-1);
	pBV->type = box::type;
	pBV->iNode = 0;
...
void CAABBTree::GetNodeChildrenBVs(const Matrix33 &Rw,const Vec3 &offsw,float scalew, 
																	 const BV *pBV_parent, BV *&pBV_child1,BV *&pBV_child2, i32 iCaller)
{
	...
	i32k ichild = m_pNodes[bbox_parent->iNode].ichild;
...
void CAABBTree::GetNodeChildrenBVs(const BV *pBV_parent, BV *&pBV_child1,BV *&pBV_child2, i32 iCaller)
{
	BBox * boxBuf = g_BBoxBuf;
	BBox * bbox_parent=(BBox*)pBV_parent;
	BBox * __restrict bbox_child1;
...
void CAABBTree::GetNodeChildrenBVs(const BV *pBV_parent, const Vec3 &sweepdir,float sweepstep, BV *&pBV_child1,BV *&pBV_child2, i32 iCaller)
{
...
void CAABBTree::ReleaseLastBVs(i32 iCaller)
{
	g_BBoxBufPos-=2;
}

void CAABBTree::ReleaseLastSweptBVs(i32 iCaller)
{
	g_BBoxExtBufPos-=2;
}

i32 CAABBTree::GetNodeContents(i32 iNode, BV *pBVCollider,i32 bColliderUsed,i32 bColliderLocal, 
...
void CAABBTree::MarkUsedTriangle(i32 itri, geometry_under_test *pGTest)
{
	if (!pGTest->pUsedNodesMap) return;
	i32 iNode = m_pTri2Node[itri>>5-m_nBitsLog]>>((itri&(1<<5-m_nBitsLog)-1)<<m_nBitsLog) & (1<<(1<<m_nBitsLog))-1;
...
i32 CAABBTree::PrepareForIntersectionTest(geometry_under_test *pGTest, CGeometry *pCollider,geometry_under_test *pGTestColl)
{
	if (m_maxSkipDim<=0 || m_pMesh->m_nTris<=16) {
		...
		i32 mapsz = (m_nNodes-1>>5) + 1;
		if (mapsz<=(i32)(DRX_ARRAY_COUNT(g_UsedNodesMap))-g_UsedNodesMapPos) {
			pGTest->pUsedNodesMap = g_UsedNodesMap+g_UsedNodesMapPos; g_UsedNodesMapPos += mapsz;
		} else {
			pGTest->pUsedNodesMap = new i32[mapsz];
    }
		pGTest->pUsedNodesIdx = g_UsedNodesIdx+g_UsedNodesIdxPos;
		pGTest->nMaxUsedNodes = min(32,(i32)(DRX_ARRAY_COUNT(g_UsedNodesIdx)-g_UsedNodesIdxPos));
...
void CAABBTree::CleanupAfterIntersectionTest(geometry_under_test *pGTest)
{
	if (!pGTest->pUsedNodesMap)
		return;
	if ((u32)(pGTest->pUsedNodesMap-g_UsedNodesMap) > (u32)DRX_ARRAY_COUNT(g_UsedNodesMap)) {
		delete[] pGTest->pUsedNodesMap; return;
	}
	if (pGTest->nUsedNodes < pGTest->nMaxUsedNodes-1) {
		for(i32 i=0;i<=pGTest->nUsedNodes;i++) 
...
void CAABBTree::GetMemoryStatistics(IDrxSizer *pSizer)
{
	...
	if (m_pTri2Node)
		pSizer->AddObject(m_pTri2Node, ((m_pMesh->m_nTris-1>>5-m_nBitsLog)+1)*sizeof(i32));
}
...
void CAABBTree::Load(CMemStream &stm, CGeometry *pGeom)
{
	...
	m_pTri2Node = new i32[(m_pMesh->m_nTris-1>>5-m_nBitsLog)+1];
	assert(m_pMesh); // placed here just to suppress a warning from the code analyzer
	memset(m_pTri2Node,0,sizeof(i32)*((m_pMesh->m_nTris-1>>5-m_nBitsLog)+1));
	for(i=0;i<m_nNodes;i++)
	{
		PREFAST_ASSUME(i > 0 && i < m_nNodes);;
		for(i32 j=0;j<(i32)m_pNodes[i].ntris;j++)
		{
...
i32 CAABBTree::SanityCheck()
{
	i32 iCaller = MAX_PHYS_THREADS;
	i32k maxDepth1 = (DRX_ARRAY_COUNT(g_BBoxBuf)-1)/4;
	i32k maxDepth2 = (DRX_ARRAY_COUNT(g_BBoxExtBuf)-1)/2;
