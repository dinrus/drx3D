...
static inline void swap(i32 *v,uk *p, i32 i1,i32 i2)
{
	i32 ti=v[i1]; v[i1]=v[i2]; v[i2]=ti;
	uk tp=p[i1]; p[i1]=p[i2]; p[i2]=tp;
}
static void qsort(i32 *v,uk *p, i32 left,i32 right)
{
...
static i32 bin_search(i32 *v,i32 n,i32 idx)
{
...
i32 qhull(strided_pointer<Vec3> _pts, i32 npts, index_t*& pTris, qhullmalloc qmalloc)
{
#if DRX_PLATFORM_64BIT
	static ptitem ptbuf[4096];
	static qhtritem trbuf[4096];
	static qhtritem *tmparr_ptr_buf[2048];
	static i32 tmparr_idx_buf[2048];
#else
	static ptitem ptbuf[1024];
	static qhtritem trbuf[1024];
	static qhtritem *tmparr_ptr_buf[512];
	static i32 tmparr_idx_buf[512];
#endif
	static  i32 g_lockQhull;
	i32 iter=0,maxiter=0;
	__qhullcalled++;
	strided_pointer<Vec3mem> pts = strided_pointer<Vec3mem>((Vec3mem*)_pts.data,_pts.iStride);

	ptitem *pt,*ptmax,*ptdeleted,*ptlist = npts>DRX_ARRAY_COUNT(ptbuf) ? new ptitem[npts] : ptbuf;
	qhtritem *tr,*trnext,*trend,*trnew,*trdata=trbuf,*trstart=0,*trlast,*trbest;
	i32 i,j,k,ti,trdatasz=DRX_ARRAY_COUNT(trbuf), bidx[6],n,next_iter,delbuds;
	qhtritem **tmparr_ptr=tmparr_ptr_buf;
	i32 *tmparr_idx=tmparr_idx_buf, tmparr_sz=512;
	...
	/*if (npts<=8) {
		if (!pTris)
			pTris = (index_t*)trbuf;
		i32 nnew,nsplits,itri,ptmask=0;
		Vec3 norm;
...
#define DELETE_TRI(ptri) { \
	...
	tr=trstart; do {
		...
		pt=tr->ptassoc; if (pt) {
			...
			maxdist=-1E37f; do {
				if ((dist = pts[(i32)(pt-ptlist)]*tr->n) > maxdist) 
			...
			tr=trstart; trlast=tr->prev; ti=ptmax-ptlist; maxdist=-1E37f;
			do {
				trnext = tr->next; 
				...
				do {
					...
					if (e_cansee(pts[ti]-tr->pt0,tr->n,-0.001f)) {
						...
						if (delbuds >= 2) {	// delete triangles that have 2+ buddies deleted
							...
							i32 bi,bi0,bi1,nfantris,fandir;
							qhtritem *fantris[64],*tr1;

							for(bi0=0; bi0<2 && !tr->buddy[bi0]->deleted; bi0++); // bi0 - deleted buddy index
...
static inline void swap(edgeitem *plist, i32 i1,i32 i2) {
	i32 ti=plist[i1].idx; plist[i1].idx=plist[i2].idx; plist[i2].idx=ti;
}
static void qsort(edgeitem *plist, i32 left,i32 right)
{
	if (left>=right) return;
	i32 i,last; 
...
i32 qhull2d(ptitem2d *pts,i32 nVtx, edgeitem *edges, i32 nMaxContacts)
{
	intptr_t imask;
	i32 i,j,i0,i1,nEdges;
	Vec2 edge;
	...
	do {
		...
		if (plist) {
			...
			edgeitem *pedge[2]; Vec2 edge[2]; float dist[2];
