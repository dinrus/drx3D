														 ...
														 contact *pcontact, geom_contact_area *parea)
{
	...
	i32 itri,i,j,j1,bContact,bBest,idbest=-1,imask=iFeature2<<8 | iFeature1;
														 ...
														 contact *pcontact, geom_contact_area *parea)
{
	...
	i32 i,j,bContact,bBest,idbest=-1,imask=iFeature2<<8 | iFeature1;
														 ...
														 contact *pcontact, geom_contact_area *parea)
{
	...
	i32 res =  ray_tri_rot_unprojection(pmode,pray,iFeature2,ptri,iFeature1,pcontact,parea);
	if (res) {
														 ...
														 contact *pcontact, geom_contact_area *parea)
{
	...
	i32 i,j,icap,idbest=-1,bContact,bBest;
...
{
	(const_cast<unprojection_mode*>(pmode))->dir.Flip();
	i32 res =  ray_cyl_rot_unprojection(pmode,pray,iFeature2,pcyl,iFeature1,pcontact,parea);
	if (res) {
		pcontact->pt = pcontact->pt.GetRotated(pmode->center,pmode->dir, pcontact->taux,-pcontact->t);
		pcontact->n = pcontact->n.GetRotated(pmode->dir, pcontact->taux,-pcontact->t).Flip();
		i32 iFeature = pcontact->iFeature[0]; pcontact->iFeature[0]=pcontact->iFeature[1]; pcontact->iFeature[1]=iFeature;
...
i32 ray_box_rot_unprojection(unprojection_mode *pmode, const ray *pray,i32 iFeature1, const box *pbox,i32 iFeature2, 
														 contact *pcontact, geom_contact_area *parea)
{
	i32 i,j,ifeat,idir,ix,iy,bContact,bBest,idbest=-1;
...
{
	(const_cast<unprojection_mode*>(pmode))->dir.Flip();
	i32 res =  ray_box_rot_unprojection(pmode,pray,iFeature2,pbox,iFeature1,pcontact,parea);
	if (res) {
		pcontact->pt = pcontact->pt.GetRotated(pmode->center,pmode->dir, pcontact->taux,-pcontact->t);
		pcontact->n = pcontact->n.GetRotated(pmode->dir, pcontact->taux,-pcontact->t).Flip();
		i32 iFeature = pcontact->iFeature[0]; pcontact->iFeature[0]=pcontact->iFeature[1]; pcontact->iFeature[1]=iFeature;
																 ...
																 contact *pcontact, geom_contact_area *parea)
{
	...
	for(i=1; i>=0; i--) {
		...
		for(icap=-1; icap<=1; icap+=2) {
			ccap = center+axis*(hh*icap);
			kcos = ptx[i]*(ptz[i]-ccap)*2; ksin = pty[i]*(ptz[i]-ccap)*2;	k = ptx[i].len2()+(ptz[i]-ccap).len2()-r2;
			a = sqr(ksin)+sqr(kcos); b = ksin*k; c = sqr(k)-sqr(kcos); d = b*b-a*c;
			if (d>=0) {
				d = sqrt_tpl(d); tsin.set(-b-d,a);
				for(j=0;j<2;j++,tsin.x+=d*2) 
...
{
	(const_cast<unprojection_mode*>(pmode))->dir.Flip();
	i32 res =  ray_capsule_rot_unprojection(pmode,pray,iFeature2,pcaps,iFeature1,pcontact,parea);
	if (res) {
		pcontact->pt = pcontact->pt.GetRotated(pmode->center,pmode->dir, pcontact->taux,-pcontact->t);
		pcontact->n = pcontact->n.GetRotated(pmode->dir, pcontact->taux,-pcontact->t).Flip();
		i32 iFeature = pcontact->iFeature[0]; pcontact->iFeature[0]=pcontact->iFeature[1]; pcontact->iFeature[1]=iFeature;
																...
																contact *pcontact, geom_contact_area *parea)
{
	...
	quotient tmax(0,1),tsin,tcos,t;
	i32 i,j,idbest=-1,bContact,bBest;
	polynomial_tpl<real,4> pn;
	...
	for(i=1; i>=0; i--) {
		// ray end - sphere
		kcos = ptx[i]*(ptz[i]-center)*2; ksin = pty[i]*(ptz[i]-center)*2;	k = ptx[i].len2()+(ptz[i]-center).len2()-r2;
		a = sqr(ksin)+sqr(kcos); b = ksin*k; c = sqr(k)-sqr(kcos); d = b*b-a*c;
		if (d>=0) {
			d = sqrt_tpl(d); tsin.set(-b-d,a);
			for(j=0;j<2;j++,tsin.x+=d*2) 
...
{
	(const_cast<unprojection_mode*>(pmode))->dir.Flip();
	i32 res =  ray_sphere_rot_unprojection(pmode,pray,iFeature2,psph,iFeature1,pcontact,parea);
	if (res) {
		pcontact->pt = pcontact->pt.GetRotated(pmode->center,pmode->dir, pcontact->taux,-pcontact->t);
		pcontact->n = pcontact->n.GetRotated(pmode->dir, pcontact->taux,-pcontact->t).Flip();
		i32 iFeature = pcontact->iFeature[0]; pcontact->iFeature[0]=pcontact->iFeature[1]; pcontact->iFeature[1]=iFeature;
