...
inline i32 FrameOwner(const entity_contact &cnt) { return isneg(cnt.pbody[1]->Minv-cnt.pbody[0]->Minv) & cnt.pent[1]->m_iSimClass-3>>31; }

inline void GetContactFrames(const entity_contact &cnt, QuatT* frames, i32 n=2)	{ 
	float scale;
	for (i32 i=0;i<n;i++) 
...
#endif
{	 
	static_assert(DRX_ARRAY_COUNT(m_BBoxNew) == 2, "Invalid array size!");
	m_BBoxNew[0].zero();
	m_BBoxNew[1].zero();
...
void CRigidEntity::AttachContact(entity_contact *pContact, i32 i, CPhysicalEntity *pCollider) 
...
void CRigidEntity::DetachContact(entity_contact *pContact, i32 i,i32 bCheckIfEmpty,i32 bAcquireContactLock) 
{ // detach contact from the contact list
	CPhysicalEntity *pCollider=0; {
	i32 bCollidersLocked = 0;
...
void CRigidEntity::DetachAllContacts()
{
	...
	for(i32 i=m_nColliders-1;i>=0;i--) {
...
void CRigidEntity::DetachPartContacts(i32 ipart,i32 iop0, CPhysicalEntity *pent,i32 iop1, i32 bCheckIfEmpty) 
{
	WriteLock lock(m_lockContacts);
	entity_contact *pContact,*pContactNext;
	for(pContact=m_pContactStart; pContact!=CONTACT_END(m_pContactStart); pContact=pContactNext) {
...
void CRigidEntity::MoveConstrainedObjects(const Vec3 &dpos, const quaternionf &dq)
{
	...
	for(i32 i=0;i<m_nColliders;i++) 
		if ((u32)m_pColliders[i]->m_iSimClass-1u<2u && 
...
i32 CRigidEntity::AddGeometry(phys_geometry *pgeom, pe_geomparams* params,i32 id, i32 bThreadSafe)
{
	if (pgeom && fabs_tpl(params->mass)+fabs_tpl(params->density)!=0 && (pgeom->V<=0 || pgeom->Ibody.x<0 || pgeom->Ibody.y<0 || pgeom->Ibody.z<0))	{
		char errmsg[256];
		drx_sprintf(errmsg,"CRigidEntity::AddGeometry: (%s at %.1f,%.1f,%.1f) Trying to add bad geometry",
	...
	if (req.IsQueued()) {
		...
		if (id<0)
			*((i32*)req.GetQueuedStruct()-1) = id = m_iLastIdx++;
		else 
			m_iLastIdx = max(m_iLastIdx,id+1);
		return id;
	}

	i32 res,nParts=m_nParts;
...
void CRigidEntity::RemoveGeometry(i32 id, i32 bThreadSafe)
{
...
void CRigidEntity::RecomputeMassDistribution(i32 ipart,i32 bMassChanged)
{
	float V;
...
i32 CRigidEntity::SetParams(pe_params *_params, i32 bThreadSafe)
{
	i32 bRecalcBounds = 0;
	...
	if (res = CPhysicalEntity::SetParams(_params,1)) {
		if (_params->type==pe_params_pos::type_id) {
			...
			i32 bRealMove = isneg(min(sqr(0.003f)-(pos0-m_body.pos).len2(), sqr(0.05f)-(q0*!m_body.q).v.len2()));
...
i32 CRigidEntity::GetParams(pe_params *_params) const
{
	if (_params->type==pe_simulation_params::type_id) {
		pe_simulation_params *params = (pe_simulation_params*)_params;
...
i32 CRigidEntity::GetStatus(pe_status *_status) const
{
	...
	if (_status->type==pe_status_netpos::type_id) {
		...
		if (m_pNetStateHistory) {
			...
			i32 numStates = m_pNetStateHistory->GetNumReceivedStates();
			if (numStates > 0) {
...
i32 CRigidEntity::Action(pe_action *_action, i32 bThreadSafe)
{
	...
	if (_action->type==pe_action_add_constraint::type_id) {
		...
		if (!pBuddy || (u32)pBuddy->m_iSimClass>4u)
			return 0;
		i32 i,ipart[2],id,flagsInfo,bBreakable=0,flagsLin=contact_constraint_3dof;
		Vec3 nloc,pt0,pt1;
	...
	if (_action->type==pe_action_register_coll_event::type_id) {
		...
		i32 i; for(i=0;i<m_nParts && m_parts[i].id!=action->partid[0];i++);
...
i32 CRigidEntity::RemoveCollider(CPhysicalEntity *pCollider, bool bRemoveAlways)
{
	INT_PTR plock0=(INT_PTR)&m_lockColliders, plock1=(INT_PTR)&pCollider->m_lockColliders, mask = pCollider->m_id-m_id>>31;
	WriteLock lock(*( i32*)(plock0+(plock1-plock0&mask)));
	WriteLockCond lock1(*( i32*)(plock1+(plock0-plock1&mask)), pCollider!=this);
	pCollider->RemoveColliderNoLock(this,bRemoveAlways);
	return RemoveColliderNoLock(pCollider,bRemoveAlways);
}

i32 CRigidEntity::RemoveColliderNoLock(CPhysicalEntity *pCollider, bool bRemoveAlways)
{
	i32 i; 
	if (!bRemoveAlways) {
		for(i=0;i<m_nColliders && m_pColliders[i]!=pCollider; i++);
...
i32 CRigidEntity::AddCollider(CPhysicalEntity *pCollider)
{
	INT_PTR plock0=(INT_PTR)&m_lockColliders, plock1=(INT_PTR)&pCollider->m_lockColliders, mask = pCollider->m_id-m_id>>31;
	WriteLock lock(*( i32*)(plock0+(plock1-plock0&mask)));
	WriteLockCond lock1(*( i32*)(plock1+(plock0-plock1&mask)), pCollider!=this);
...
i32 CRigidEntity::AddColliderNoLock(CPhysicalEntity *pCollider)
{
	...
	i32 nColliders=m_nColliders, nCollidersAlloc=m_nCollidersAlloc, i=CPhysicalEntity::AddCollider(pCollider);
...
i32 CRigidEntity::HasContactsWith(CPhysicalEntity *pent)
{
	i32 i; for(i=0;i<m_nColliders && m_pColliders[i]!=pent;i++);
	return i==m_nColliders ? 0 : (m_pColliderContacts[i]!=0 || m_pColliderConstraints[i]!=0);
}

i32 CRigidEntity::HasCollisionContactsWith(CPhysicalEntity *pent)
{
	i32 i; for(i=0;i<m_nColliders && m_pColliders[i]!=pent;i++);
	return i==m_nColliders ? 0 : m_pColliderContacts[i]!=0;
}

i32 CRigidEntity::HasConstraintContactsWith(const CPhysicalEntity *pent, i32 flagsIgnore) const
{
	i32 i; for(i=0;i<m_nColliders && m_pColliders[i]!=pent;i++);
	...
	for(i32 j=0; j<NMASKBITS && getmask(j)<=m_pColliderConstraints[i]; j++) 
		if (m_pColliderConstraints[i] & getmask(j) && !(m_pConstraintInfos[j].flags & flagsIgnore))
			return 1;
	return 0;
}

i32 CRigidEntity::HasPartContactsWith(CPhysicalEntity *pent, i32 ipart, i32 bGreaterOrEqual)
{
	ReadLock lockc(m_lockColliders);
...
i32 CRigidEntity::Awake(i32 bAwake,i32 iSource)
{
	if ((u32)m_iSimClass>6u) {
...
void CRigidEntity::AlertNeighbourhoodND(i32 mode) 
{ 
	i32 i,iSimClass=m_iSimClass;
	m_iSimClass = 7; // notifies the others that we are being deleted

	for(i=0;i<NMASKBITS && getmask(i)<=m_constraintMask;i++) 
		if (m_constraintMask & getmask(i) && m_pConstraintInfos[i].pConstraintEnt && (u32)m_pConstraintInfos[i].pConstraintEnt->m_iSimClass<7u) 
...
masktype CRigidEntity::MaskIgnoredColliders(i32 iCaller, i32 bScheduleForStep)
{
...
void CRigidEntity::UnmaskIgnoredColliders(masktype constraint_mask, i32 iCaller)
{
	ReadLock lock(m_lockColliders);
	i32 i;
	...
	for(i=0; i<NMASKBITS && getmask(i)<=constraint_mask; i++) 
		if (constraint_mask & getmask(i) && m_pConstraintInfos[i].flags & constraint_ignore_buddy) 
			AtomicAdd(&m_pConstraints[i].pent[1]->m_bProcessed, -((i32)m_pConstraints[i].pent[1]->m_bProcessed&1<<iCaller));
}

bool CRigidEntity::IgnoreCollisionsWith(const CPhysicalEntity *pent, i32 bCheckConstraints) const
{
	i32 i; 
...
inline i32 wait_for_ent( CPhysicalEntity *pent) 
{ 
...
i32 CRigidEntity::GetPotentialColliders(CPhysicalEntity **&pentlist, float dt)
{
	i32 i,j,nents,bSameGroup;
	...
	if (m_flags & ref_use_simple_solver) for(i=0;i<m_nColliders;i++)
		AtomicAdd(&m_pColliders[i]->m_bProcessed, -((i32)m_pColliders[i]->m_bProcessed&1<<iCaller));
...
void CRigidEntity::ProcessContactEvents(geom_contact* pcontact, i32 i, i32 iCaller) 
{
	EventPhysCollision epc;
...
i32 CRigidEntity::CheckForNewContacts(geom_world_data *pgwd0,intersection_params *pip, i32 &itmax, Vec3 sweep, i32 iStartPart,i32 nParts, i32 *pFlagsAccum)
{
	CPhysicalEntity **pentlist;
	geom_world_data gwd1;
	i32 ient,nents,i,j,icont,ncontacts,ipt,nTotContacts=0,bHasMatSubst=0,ient1,j1,j2,bSquashy,isAwake,flagsAccum=0;
	i32 iCaller = get_iCaller_int();
	RigidBody body(false),*pbody[2];
	...
	i32 bPivotFilled=isneg(pip->ptOutsidePivot[0].x-1E9f);
	pip->bThreadSafe = 1;
	box bbox;
	itmax = -1;
	...
	nents = GetPotentialColliders(pentlist, pip->time_interval*((i32)pip->bSweepTest & ~-iszero((i32)m_flags & ref_small_and_fast)));
	  ...
	  isAwake=IsAwake(i); 
				...
				(isAwake || pentlist[ient]->IsAwake(j)) && (!bCheckBBox || AABB_overlap(BBox,pentlist[ient]->GetPartBBox(j,BBox1,this)))))
		{
			...
			if (pentlist[ient]->m_parts[j].flags & geom_car_wheel && pentlist[ient]->GetMassInv()*5>m_body.Minv)
				continue;
			i32 bSameGroup = iszero(m_iGroup-pentlist[ient]->m_iGroup);
			if (bSameGroup & (i32)pip->bSweepTest)
				continue;
			...
			if (pGeom!=&aray) {
				...
				for(icont=0; icont<ncontacts; icont++) {
					...
					i32 scMask = (2<<pentlist[ient]->m_iSimClass)-(pentlist[ient]->m_id>>31);
					if (bHasMatSubst & scMask) for(ient1=0;ient1<nents;ient1++) for(i32 j3=0;j3<pentlist[ient1]->GetUsedPartsCount(iCaller);j3++)
			...
			bHasMatSubst |= pentlist[ient]->m_parts[j].flagsCollider & -((i32)pentlist[ient]->m_parts[j].flags & geom_mat_substitutor)>>31;
	}
	CollidersNoMore:
...
void CRigidEntity::CleanupAfterContactsCheck(i32 iCaller)
{
	i32 i;
...
i32 CRigidEntity::RemoveContactPoint(CPhysicalEntity *pCollider, const Vec3 &pt, float mindist2)
{
	i32 i;
	for(i=0;i<m_nColliders && m_pColliders[i]!=pCollider;i++);
	if (i<m_nColliders) {
...
entity_contact *CRigidEntity::RegisterContactPoint(i32 idx, const Vec3 &pt, const geom_contact *pcontacts, i32 iPrim0,i32 iFeature0, 
																									 i32 iPrim1,i32 iFeature1, i32 flags, float penetration, i32 iCaller, const Vec3 &nloc)
{
	...
	entity_contact *pContact;
	i32 j;
	i32k bUseSimpleSolver=0;//iszero((i32)m_flags&ref_use_simple_solver)^1;
	RigidBody body(false), *pbody1 = g_CurColliders[idx]->GetRigidBodyTrans(&body,g_CurCollParts[idx][1],this,1,true);
	//i32 iPrimCode = iPrim0|iFeature0<<8|iPrim1<<16|(iFeature1&0x7F)<<24;
	...
	pContact->friction *= iszero((i32)pContact->pent[1]->m_parts[pContact->ipart[1]].flags & geom_car_wheel);
	...
	if (bNoCollResponse) {
		i32 bLastInGroup = flags & contact_last &&		NO_BUFFER_OVERRUN
...
{
	//i32 i,nContacts,bResolveInstantly;
	entity_contact *pContact,*pContactNext;//*pContacts[16];
  
	//bResolveInstantly = /isneg((i32)(DRX_ARRAY_COUNT(pContacts))-nContacts);
	m_bStable = max(m_bStable, (u32)(isneg(2-m_nContacts)*2));
	for(pContact=m_pContactStart; pContact!=CONTACT_END(m_pContactStart); pContact=pContactNext)	{
		pContactNext = pContact->next;
...
i32 CRigidEntity::UpdatePenaltyContact(entity_contact *pContact, float time_interval)//, i32 bResolveInstantly,entity_contact **pContacts,i32 &nContacts)
{
	DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );

	i32 j,bRemoveContact,bCanPull;
	Vec3 dp,n,vrel,dP,r;
	float dpn,dptang2,dPn,dPtang2;
...
i32 CRigidEntity::RegisterConstraint(const Vec3 &pt0,const Vec3 &pt1, i32 ipart0, CPhysicalEntity *pBuddy,i32 ipart1, i32 flags,i32 flagsInfo, CPhysicalEntity *pConstraintEnt)
{
	i32 i;
	...
	if (i>=m_nConstraintsAlloc) {
		...
		i32 nConstraints = m_nConstraintsAlloc;	// cppcheck-suppress memsetClass
	...
	i32k buddyCollider = AddCollider(pBuddy);
...
i32 CRigidEntity::RemoveConstraint(i32 iConstraint)
{
	i32 i;
...
void CRigidEntity::OnContactResolved(entity_contact *pContact, i32 iop, i32 iGroupId)
{
	i32 i,j;
...
void CRigidEntity::UpdateConstraints(float time_interval)
{
	...
	if (!(m_pConstraintInfos[i].flags & constraint_inactive)) {
		i32 bSimple = m_pConstraints[i].pent[1]->m_iSimClass>3 && m_pConstraints[i].pent[1]->GetType()!=PE_ARTICULATED;
		...
		i32 bActive = m_pConstraintInfos[i].bActive;
		...
		if (m_pConstraintInfos[i].flags & constraint_rope) {
			...
			i32 iframe = FrameOwner(m_pConstraints[i]);
			...
			if (m_pConstraints[i].flags & contact_constraint_3dof) {
				...
				{ i32 iside = sgnnz(m_pConstraintInfos[i].limits[0]+m_pConstraintInfos[i].limits[1]-angles.x*2);
...
i32 CRigidEntity::EnforceConstraints(float time_interval)
{
	return 0;
	/*i32 i,i1,bEnforced=0;
	Vec3 pt[2],v0,v1;
	Ang3 ang;
	quaternionf qframe0,qframe1;
...
{
	for(i32 i=0; i<min(2,min(m_nParts,m_iVarPart0)); i++) {
...
void CRigidEntity::UpdatePosition(i32 bGridLocked)
{
	i32 i;
...
void CRigidEntity::CheckContactConflicts(geom_contact *pcontacts, i32 ncontacts, i32 iCaller)
{
	i32 i,j;
...
void CRigidEntity::ProcessCanopyContact(geom_contact *pcontacts, i32 i, float time_interval, i32 iCaller)
{
	i32 j;
	box bbox; 
	Vec3 dP,center,dv;
...
void CRigidEntity::DelayedIntersect(geom_contact *pcontacts, i32 ncontacts, CPhysicalEntity **pColliders, i32 (*iCollParts)[2])
{
	DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );
	PHYS_ENTITY_PROFILER
	i32 i,j;
	i32 iCaller = MAX_PHYS_THREADS;
	for(i=0; i<ncontacts; i++) {
	...
	i32 bCanopy = m_bCanopyContact&1;
	...
	for(i=0;i<ncontacts;i++) if (!(m_parts[g_CurCollParts[i][0]].flags & geom_squashy)) {
		...
		i32 bPrimPrimContact = -((pcontacts[i].iNode[0]&pcontacts[i].iNode[1])>>31), flagsLast=contact_last;
...
i32 __bstop = 0;
extern i32 __curstep;

i32 CRigidEntity::Step(float time_interval)
{
	DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );
  PHYS_ENTITY_PROFILER

	geom_contact *pcontacts;
	i32 i,j,itmax,itmax0,ncontacts,bHasContacts,bNoForce,bSeverePenetration=0,bNoUnproj=0,bWasUnproj=0,nsweeps=0;
	i32k bUseSimpleSolver = 0;//isneg(-((i32)m_flags&ref_use_simple_solver));
	i32 iCaller = get_iCaller_int();
	float r,e;
	quaternionf qrot;
	Vec3 axis,pos;
	...
	bNoUnproj = isneg(4-(i32)m_nFutileUnprojFrames*bNoForce);
	m_bSmallAndFastForced &= m_bAwake;
	m_flags &= ~ref_small_and_fast;
	m_flags |= ref_small_and_fast & -(i32)m_bSmallAndFastForced;
	...
	if (m_bAwake | bUseSimpleSolver) {
		...
		i32 bSlowRot = isneg(m_body.w.len2()-sqr(3.0f));
		...
		for(i=0;i<ncontacts;i++) if (pcontacts[i].t>=0) { // penetration contacts - register points and add additional penalty impulse in solver
			if (!(m_parts[g_CurCollParts[i][0]].flags & geom_squashy)) {
				...
				i32 bPrimPrimContact = -((pcontacts[i].iNode[0]&pcontacts[i].iNode[1])>>31)&-bSlowRot, hasArea=0, flagsLast=contact_last;
				r=0; axis.zero(); 
				if (pcontacts[i].parea && pcontacts[i].parea->npt>=2) {
					ntilt = (axis = pcontacts[i].parea->n1)^pcontacts[i].n;
					pcontacts[i].n = -pcontacts[i].dir;
	...
	i32 nBuoys = PostStepNotify(time_interval,pb,4,iCaller);
	...
	return (m_bHadSeverePenetration^1) | isneg(3-(i32)m_nStepBackCount);//1;
}


i32 CRigidEntity::PostStepNotify(float time_interval,pe_params_buoyancy *pb,i32 nMaxBuoys,i32 iCaller)
{
	Vec3 gravity;
	i32 nBuoys=0;
...
float CRigidEntity::GetMaxTimeStep(float time_interval)
{
	...
	i32 bDegradeQuality = -m_pWorld->m_threadData[iCaller].bGroupInvisible & ~(1-m_nColliders>>31);
	return min(min(m_timeStepFull-m_timeStepPerformed,m_maxAllowedStep*(1-bDegradeQuality*0.5f)),time_interval);
}


void CRigidEntity::ArchiveContact(entity_contact *pContact, float imp, i32 bLastInGroup, float r)
{
			...
			if ((pContact->pent[0]->m_parts[pContact->ipart[0]].flags | pContact->pent[1]->m_parts[pContact->ipart[1]].flags) & geom_no_coll_response) {
				...
				if (m_nEvents==m_nMaxEvents) {	
					if (vrel>m_vcollMin && vrel<1E10f) {
						...
						for(m_vcollMin=1E10f,i=0,m_icollMin=0x1F; i<(i32)m_nEvents; i++) 
...
i32 CRigidEntity::GetContactCount(i32 nMaxPlaneContacts)
{
	if (m_flags & ref_use_simple_solver)
		return 0;

	i32 i,nContacts,nTotContacts;
	entity_contact *pContact;
...
i32 CRigidEntity::RegisterContacts(float time_interval,i32 nMaxPlaneContacts)
{
	DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );

	i32 i,j,bComplexCollider,bStable,nContacts,bUseAreaContact,constrFlags=0;
	...
	m_bStable &= -((i32)m_flags & ref_use_simple_solver)>>31;
	...
	for(i=0;i<m_nColliders;i++) {
		if (!(m_flags & ref_use_simple_solver) && m_pColliderContacts[i]) {
			...
			m_bStable = max(m_bStable,(u32)bStable);
...
void CRigidEntity::StepBack(float time_interval)
{
	...
	if (m_pEventsColl && m_pWorld->m_iLastLogPump==m_iLastLogColl) for(u32 i=0;i<m_nEvents;i++) 
...
i32 CRigidEntity::Update(float time_interval, float damping)
{
	i32 i,j,iCaller=get_iCaller_int();
	float dt,E,E_accum, Emin = m_bFloating && m_nColliders+m_nPrevColliders==0 ? m_EminWater : m_Emin;
	Vec3 L_accum,pt[4];
	coord_block_BBox partCoordTmp[2];
	//m_nStickyContacts = m_nSlidingContacts = 0;
	m_nStepBackCount = (m_nStepBackCount&-(i32)m_bSteppedBack)+m_bSteppedBack;
	if (m_flags & ref_use_simple_solver)
		damping = max(0.9f,damping);
	CapBodyVel();
	m_nCanopyContactsLost += iszero((i32)(m_bCanopyContact & 3)-2);
	m_timeCanopyFallen += 3.0f*iszero((i32)(m_nCanopyContactsLost & 3)-3);
	...
	if (m_flags & pef_log_collisions && m_nMaxEvents>0)	{
		...
		m_vcollMin += (1e10f-m_vcollMin)*(i^1);
		m_iLastLogColl = m_pWorld->m_iLastLogPump;
		for(i=0;i<(i32)m_nEvents;i++) if (m_pEventsColl[i]->pEntContact) {
	...
	for(pContact=m_pContactStart,i=j=0; pContact!=CONTACT_END(m_pContactStart) && j<16*(1-i); pContact=pContact->next,j++) {
		i32 bLiesOnTop = isneg(sqr_signed(pContact->n*m_gravity)+m_gravity.len2()*0.5f);
		INT_PTR mask = -iszero((i32)pContact->pent[1]->m_iSimClass-2);
...
void SRigidEntityNetStateHistory::PushReceivedSequenceDelta(u8 delta) {
	...
	if (receivedSequencesStart == 0 && numReceivedSequences == MAX_SEQUENCE_HISTORY_SNAPSHOTS) {
		...
		for (i32 i=0; i<numReceivedSequences; ++i) {
...
void SRigidEntityNetStateHistory::UpdateSequenceDeltaAverage(u8 delta, i32 sampleCount) {
	if (sampleCount > 1) {
...
i32 CRigidEntity::GetStateSnapshot(CStream &stm, float time_back, i32 flags)
{
	if (flags & ssf_checksum_only) {
		...
		if (m_pWorld->m_vars.bMultiplayer) {
			...
			stm.WriteBits((u8*)&m_qrot,sizeof(m_qrot)*8);
			if (m_body.Minv>0) {
				stm.Write(m_body.P);
				stm.Write(m_body.L);	
			} else {
...
#if USE_IMPROVED_RIGID_ENTITY_SYNCHRONISATION
	ser.Value( "sequenceNumber", sequenceNumber, 'u8');
#endif
}

i32 CRigidEntity::GetStateSnapshot( TSerialize ser, float time_back, i32 flags )
{
	CPhysicalEntity::GetStateSnapshot(ser,time_back,flags);

#if USE_IMPROVED_RIGID_ENTITY_SYNCHRONISATION
	u8 sequenceNumber = m_pWorld ? GetLocalSequenceNumber() : 0;
...
i32 CRigidEntity::WriteContacts(CStream &stm,i32 flags)
{
	i32 i;
	entity_contact *pContact;
	...
	for(i=0; i<m_nColliders; i++) {
		...
		if (m_pColliderContacts[i]) {
			...
			for(;;pContact=pContact->prev) {
				...
				WritePacked(stm, (i32)pContact->iConstraint);
				WritePacked(stm, (i32)pContact->iNormal);
...
i32 CRigidEntity::WriteContacts(TSerialize ser)
{
	i32 n = 0;
...
i32 CRigidEntity::ReadContacts(TSerialize ser)
{
	DetachAllContacts();
	i32 n = 0;
...
i32 CRigidEntity::SetStateFromSnapshot(CStream &stm, i32 flags)
{
	bool bnz;
	i32 ver=0;
	i32 iMiddle,iBound[2]={ m_iLastChecksum+1-NCHECKSUMS,m_iLastChecksum+1 };
	u32 checksum,checksum_hist;
	coord_block_BBox partCoordTmp[2];
	...
	if (!bnz) {
		...
		if (!(flags & ssf_no_update)) {
			...
			m_flags |= ref_checksum_outofsync & ~-iszero((i32)(checksum-checksum_hist));
		...
		if (!(flags & ssf_no_update)) {
			...
			if (m_pWorld->m_vars.bMultiplayer) {
				...
				stm.ReadBits((u8*)&m_qNew,sizeof(m_qrot)*8);
				if (m_body.Minv>0) {
					stm.Read(m_body.P);
					stm.Read(m_body.L);
				} else {
...
void CRigidEntity::SetNetworkAuthority(i32 auth, i32 paused)
{
#if USE_IMPROVED_RIGID_ENTITY_SYNCHRONISATION
	if (auth==1 && !m_hasAuthority) {
		if (m_pNetStateHistory) {
			// Offset the sequence numbers so they match up with the previous owner of this entity
			u8 prevSequenceNum = (u8)GetInterpSequenceNumber();
			u8 newSequenceNum = GetLocalSequenceNumber();
...
#if USE_IMPROVED_RIGID_ENTITY_SYNCHRONISATION
u8 CRigidEntity::GetLocalSequenceNumber() const
{
	const CTimeValue &frameStartTime = gEnv->pTimer->GetFrameStartTime();
	return (u8)((frameStartTime.GetMilliSecondsAsInt64() * m_pWorld->m_vars.netSequenceFrequency) / 1000);
}
...
bool CRigidEntity::GetInterpolatedState(float sequenceNumber, SRigidEntityNetSerialize &interpState)
{
	...
	for (i32 i=m_pNetStateHistory->GetNumReceivedStates()-1; i>=0; --i) {
	...
	interpState.sequenceNumber = (u8)sequenceNumber;
	return true;
}
#endif

i32 CRigidEntity::SetStateFromSnapshot( TSerialize ser, i32 flags )
{
			...
			{
				{
					...
					u8 sequenceDelta = helper.sequenceNumber - GetLocalSequenceNumber();
...
i32 CRigidEntity::ReadContacts(CStream &stm, i32 flags)
{
	i32 i,j,id; bool bnz;
	pe_status_id si;
	i32 idPrevColliders[16],nPrevColliders=0;
	...
	if (!(flags & ssf_no_update)) {
		...
		for(i=0;i<m_nColliders;i++) {
			...
			while(true) {
				...
				i32 iNormal; ReadPacked(stm,iNormal); pContact->iNormal=iNormal;
		...
		for(i=0;i<nPrevColliders;i++)	{
			for(j=0;j<m_nColliders && (i32)(INT_PTR)m_pColliders[j]!=idPrevColliders[i];j++);
			if (j<m_nColliders)
...
i32 CRigidEntity::PostSetStateFromSnapshot()
{
	if (m_bJustLoaded) {
		pe_status_id si;
		i32 i,j;
		...
		for(pContact=m_pContactStart; pContact!=CONTACT_END(m_pContactStart); pContact=pContactNext) {
			...
			if (pContact->pent[1] && (u32)pContact->ipart[0]<(u32)pContact->pent[0]->m_nParts &&
					(u32)pContact->ipart[1]<(u32)pContact->pent[1]->m_nParts) 
			{
...
u32 CRigidEntity::GetStateChecksum()
{
	return //*(u32*)&m_testval;
...
void CRigidEntity::ApplyBuoyancy(float time_interval,const Vec3& gravity,pe_params_buoyancy *pb,i32 nBuoys)
{
	if (m_kwaterDensity==0 || m_body.Minv==0) {
		m_submergedFraction=0; return;
	}
	i32 i,ibuoy,ibuoySplash=-1;
	...
	for(ibuoy=0;ibuoy<nBuoys;ibuoy++) if ((u32)pb[ibuoy].iMedium<2u) {
...
i32 CRigidEntity::CompactContactBlock(entity_contact *pContact,i32 endFlags, float maxPlaneDist, i32 nMaxContacts,i32 &nContacts,
...
i32 CRigidEntity::ExtractConstraintInfo(i32 i, masktype constraintMask, pe_action_add_constraint &aac)
{
	i32 i1,j;
	i32 realent = m_pConstraints[i].pent[1]->m_iSimClass<3 || m_pConstraints[i].pent[1]->GetType()==PE_ARTICULATED;
...
EventPhysJointBroken &CRigidEntity::ReportConstraintBreak(EventPhysJointBroken &epjb, i32 i)
{
	epjb.idJoint=m_pConstraintInfos[i].id; epjb.bJoint=0; MARK_UNUSED epjb.pNewEntity[0],epjb.pNewEntity[1];
	for(i32 iop=0;iop<2;iop++) {
			...
			if (m_pConstraints[i].pent[0]==pentOrig || m_pConstraints[i].pent[1]==pentOrig) {
				...
				if ((ipart = pentOrig->TouchesSphere(m_pConstraints[i].pt[0],m_pConstraintInfos[i].sensorRadius))>=0) {
					for(;i<i1;i++) {
						...
						m_pConstraints[i].pbody[j] = pentOrig->GetRigidBody(ipart,iszero((i32)m_pConstraintInfos[i].flags & constraint_inactive));
					}
				} else {
...
void CRigidEntity::DrawHelperInformation(IPhysRenderer *pRenderer, i32 flags)
{
#if USE_IMPROVED_RIGID_ENTITY_SYNCHRONISATION
...
void CRigidEntity::GetMemoryStatistics(IDrxSizer *pSizer) const
{
	if (GetType()==PE_RIGID)
