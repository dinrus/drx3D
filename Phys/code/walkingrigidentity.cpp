...
i32 CWalkingRigidEntity::AddGeometry(phys_geometry *pgeom, pe_geomparams* params,i32 id,i32 bThreadSafe)
{
	...
	i32 res = CPhysicalEntity::AddGeometry(pgeom,params,id);
	if (res>=0)
		RecomputeMassDistribution();
	return res;
}

void CWalkingRigidEntity::RemoveGeometry(i32 id, i32 bThreadSafe)
{
...
void CWalkingRigidEntity::RecomputeMassDistribution(i32 ipart, i32 bMassChanged)
{
...
i32 CWalkingRigidEntity::SetParams(pe_params *_params, i32 bThreadSafe)
{
	...
	if (_params->type==pe_player_dynamics::type_id) {
		...
		if (!is_unused(params->surface_idx)) {
			for(i32 i=0;i<m_nParts;i++) if (m_parts[i].id==100)
...
i32 CWalkingRigidEntity::GetParams(pe_params *_params) const
{
	if (_params->type==pe_player_dynamics::type_id) {
		pe_player_dynamics *params = (pe_player_dynamics*)_params;
	...
	if (_params->type==pe_params_sensors::type_id) {
		...
		if (!is_unused(params->pOrigins) && params->pOrigins)
			for(i32 i=0;i<m_nLegs;i++) const_cast<Vec3*>(params->pOrigins)[i] = m_legs[i*3];
		if (!is_unused(params->pDirections) && params->pDirections)
			for(i32 i=0;i<m_nLegs;i++) const_cast<Vec3*>(params->pDirections)[i] = m_legs[i*3+1];
		return 1;
	}
	if (_params->type==pe_params_walking_rigid::type_id) {
		pe_params_walking_rigid *params = (pe_params_walking_rigid*)_params;
...
i32 CWalkingRigidEntity::GetStatus(pe_status* _status) const
{
...
i32 CWalkingRigidEntity::Step(float dt) 
{ 
...
bool CWalkingRigidEntity::OnSweepHit(geom_contact &cnt, i32 icnt, float &dt, Vec3 &vel, i32 &nsweeps)
{
	m_pos = m_posNew;
	if (!m_pentGround) {
...
i32 CWalkingRigidEntity::GetPotentialColliders(CPhysicalEntity **&pentlist, float dt)
{
	if (m_nCollEnts<0) {
		for(i32 i=0,j;i<m_nLegs;i++) 
		...
		for(i32 i=0; i<nents; i++) if (pents[i]->GetMassInv()*m_minLegsCollMass <= 1.0f)
			for(i32 j,j1=pents[i]->GetUsedPartsCount(iCaller)-1; j1>=0; j1--)
				if (pents[i]->m_parts[j = pents[i]->GetUsedPart(iCaller,j1)].flags & m_colltypeLegs) {
					...
					geom_contact *pcont;
					if (i32 ncont = part.pPhysGeomProxy->pGeom->Intersect(&aray, gwd,gwd+1, 0, pcont)) 
...
i32 CWalkingRigidEntity::RegisterContacts(float dt, i32 nMaxPlaneContacts)
{
	if (m_pentGround) 
...
void CWalkingRigidEntity::GetMemoryStatistics(IDrxSizer *pSizer) const
{
