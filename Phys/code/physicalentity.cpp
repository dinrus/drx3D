...
i32 *CPhysicalEntity::g_jointidx = 0;
i32 CPhysicalEntity::g_nPartsAlloc=0, CPhysicalEntity::g_nJointsAlloc=0;
	...
	, m_pStructure(nullptr)
{
	...
	static_assert(DRX_ARRAY_COUNT(m_BBox) == 2, "Invalid array size!");
	m_BBox[0].zero();
	m_BBox[1].zero();
	m_iSimClass = 0;
	m_iGThunk0 = 0;
	static_assert(DRX_ARRAY_COUNT(m_ig) == 2, "Invalid array size!");
...
CPhysicalEntity::~CPhysicalEntity()
{
	...
	for(i32 i=0;i<m_nParts;i++) if (m_parts[i].pPhysGeom) {
		if (m_parts[i].pMatMapping && m_parts[i].pMatMapping!=m_parts[i].pPhysGeom->pMatMapping)
			delete[] m_parts[i].pMatMapping;
		m_pWorld->GetGeomUpr()->UnregisterGeometry(m_parts[i].pPhysGeom);
		m_pWorld->GetGeomUpr()->UnregisterGeometry(m_parts[i].pPhysGeomProxy);
	...
	if (m_pStructure) {
		...
		if (m_pStructure->defparts) {
			for(i32 j=0;j<m_nParts;j++) if (m_pStructure->defparts[j].pSkinInfo) {
...
void CPhysicalEntity::GetLocTransform(i32 ipart, Vec3 &offs, quaternionf &q, float &scale, const CPhysicalPlaceholder *trg) const
{
	if ((u32)ipart<(u32)m_nParts) {
...
void CPhysicalEntity::ComputeBBox(Vec3 *BBox, i32 flags)
{
	i32k numParts = m_nParts+(flags & part_added);
	if (numParts) {
		IGeometry *pGeom[3];
		i32 i,j;
...
i32 CPhysicalEntity::SetParams(pe_params *_params, i32 bThreadSafe)
{
	if (_params->type==pe_params_part::type_id) {
		...
		if (!is_unused(params->pPhysGeom) && params->pPhysGeom && params->pPhysGeom->pGeom)
			m_pWorld->GetGeomUpr()->AddRefGeometry(params->pPhysGeom);
		if (!is_unused(params->pPhysGeomProxy) && params->pPhysGeomProxy && params->pPhysGeomProxy->pGeom)
			m_pWorld->GetGeomUpr()->AddRefGeometry(params->pPhysGeomProxy);
	}
	i32 bRecalcBounds = 0;
	...
	if (_params->type==pe_params_pos::type_id) {
		...
		i32 i,j,bPosChanged=0,bBBoxReady=0;
		...
		if (pgridNew != pgridCur) {
			...
			i32 nparts = GetType()==PE_ARTICULATED ? m_nParts : 1;
		...
		if ((scale=get_xqs_from_matrices(params->pMtx3x4,params->pMtx3x3, params->pos,params->q,params->scale)).len2()>3.03f) {
			...
			for(i=0;i<m_nParts;i++) {
				...
				if (m_parts[i].pPhysGeom!=m_parts[i].pPhysGeomProxy) {
					...
					if (BakeScaleIntoGeometry(pgeom,m_pWorld->GetGeomUpr(),scale)) {
						m_pWorld->GetGeomUpr()->UnregisterGeometry(m_parts[i].pPhysGeomProxy);
				...
				if (!(pgeom = (phys_geometry*)m_parts[i].pPhysGeom->pGeom->GetForeignData(DATA_UNSCALED_GEOM)))
					doBake = (pgeom = m_parts[i].pPhysGeom)->nRefCount>1;	// can't store DATA_UNSCALED_GEOM ptr to a  geom
				if (doBake && BakeScaleIntoGeometry(pgeom,m_pWorld->GetGeomUpr(),scale)) {
					if (m_parts[i].pPhysGeom==m_parts[i].pPhysGeomProxy) {
						m_pWorld->GetGeomUpr()->UnregisterGeometry(m_parts[i].pPhysGeomProxy);
						(m_parts[i].pPhysGeomProxy = pgeom)->nRefCount++;
					}
	...
	if (_params->type==pe_params_bbox::type_id) {
		...
		i32 bPosChanged = m_pWorld->RepositionEntity(this,1,BBox);
		{ WriteLock lock(m_lockUpdate);
			WriteBBox(BBox);
			m_pWorld->UnlockGrid(this,-bPosChanged);
		}
	...
	if (_params->type==pe_params_part::type_id) {
		...
		if (is_unused(params->ipart) && is_unused(params->partid)) {
			for(i=0;i<m_nParts;i++) if (is_unused(params->flagsCond) || m_parts[i].flags & params->flagsCond) {
				...
				if (!is_unused(params->pMatMapping)) {
					i32 *pMapping0 = m_parts[i].pMatMapping;
					if ((!pMapping0 || params->nMats!=m_parts[i].nMats || pMapping0==m_parts[i].pPhysGeom->pMatMapping) && params->pMatMapping)
						m_parts[i].pMatMapping = new i32[m_parts[i].nMats = params->nMats];
					else pMapping0 = 0;
					if (params->pMatMapping && params->nMats>0)
						memcpy(m_parts[i].pMatMapping, params->pMatMapping, params->nMats*sizeof(i32));
					else
					{
		...
		if ((scale=get_xqs_from_matrices(params->pMtx3x4,params->pMtx3x3, params->pos,params->q,params->scale)).len2()>3.03f) {
			if (is_unused(params->pPhysGeom))
				m_pWorld->GetGeomUpr()->AddRefGeometry(params->pPhysGeom=m_parts[i].pPhysGeom);
			if (is_unused(params->pPhysGeomProxy))
				m_pWorld->GetGeomUpr()->AddRefGeometry(params->pPhysGeomProxy=m_parts[i].pPhysGeomProxy);
			phys_geometry *pgeom0 = params->pPhysGeom;
			if (BakeScaleIntoGeometry(params->pPhysGeom,m_pWorld->GetGeomUpr(),scale,1))
				m_pWorld->GetGeomUpr()->AddRefGeometry(params->pPhysGeom);
			if (pgeom0!=params->pPhysGeomProxy)	{
				if (BakeScaleIntoGeometry(params->pPhysGeomProxy,m_pWorld->GetGeomUpr(),scale,1))
					m_pWorld->GetGeomUpr()->AddRefGeometry(params->pPhysGeomProxy);
			} else
		...
		if (!is_unused(params->pPhysGeom) && params->pPhysGeom!=m_parts[i].pPhysGeom && params->pPhysGeom && params->pPhysGeom->pGeom) {
			...
			if (m_parts[i].pPhysGeom==m_parts[i].pPhysGeomProxy) {
				m_pWorld->GetGeomUpr()->UnregisterGeometry(m_parts[i].pPhysGeomProxy);
				m_pWorld->GetGeomUpr()->AddRefGeometry(m_parts[i].pPhysGeomProxy = params->pPhysGeom);
			}
			m_pWorld->GetGeomUpr()->UnregisterGeometry(m_parts[i].pPhysGeom);
			...
			if (m_nRefCount && m_iSimClass==0 && m_pWorld->m_vars.lastTimeStep>0.0f) {
				...
				for(i32 j=m_pWorld->GetEntitiesAround(m_BBox[0]-inflator,m_BBox[1]+inflator, pentlist, ent_sleeping_rigid|ent_living|ent_independent)-1; j>=0; j--)
					if (m_iSimClass || pentlist[j]->HasPartContactsWith(this,i))
						pentlist[j]->Awake();
			}
		}
		if (!is_unused(params->pPhysGeom) && params->pPhysGeom && params->pPhysGeom->pGeom)
			m_pWorld->GetGeomUpr()->UnregisterGeometry(params->pPhysGeom);
		...
		if (!is_unused(params->pPhysGeomProxy) && params->pPhysGeomProxy && params->pPhysGeomProxy->pGeom)
			m_pWorld->GetGeomUpr()->UnregisterGeometry(params->pPhysGeomProxy);
		...
		if (!is_unused(params->idParent)) {
			AllocStructureInfo();
			i32 j; for(j=m_nParts-1;j>=0 && m_parts[j].id!=params->idParent;j--);
	...
	if (_params->type==pe_params_foreign_data::type_id) {
		...
		uk pForeignDataOld = m_pForeignData;
	...
	if (_params->type==pe_params_skeleton::type_id) {
		...
		if (!is_unused(params->ipart))
			...
			return 0;
		if ((u32)i>=(u32)m_nParts || !m_pStructure || !m_pStructure->defparts || !m_pStructure->defparts[i].pSkelEnt)
			return 0;
...
i32 CPhysicalEntity::GetParams(pe_params *_params) const
{
	if (_params->type==pe_params_bbox::type_id) {
		pe_params_bbox *params = (pe_params_bbox*)_params;
	...
	if (_params->type==pe_params_part::type_id) {
		...
		} else if ((u32)(i = params->ipart)>=(u32)m_nParts)
			return 0;
	...
	if (_params->type==pe_params_structural_joint::type_id) {
		...
		if ((u32)i>=(u32)m_pStructure->nJoints)
			return 0;
		for(iop=0;iop<2;iop++)
...
i32 CPhysicalEntity::GetStatus(pe_status *_status) const
{
	...
	if (_status->type==pe_status_id::type_id) {
		...
		if ((u32)status->iPrim >= (u32)pgeom->pGeom->GetPrimitiveCount() ||
...
i32 CPhysicalEntity::Action(pe_action *_action, i32 bThreadSafe)
{
	ChangeRequest<pe_action> req(this,m_pWorld,_action,bThreadSafe);
	if (req.IsQueued())	{
		if (_action->type==pe_action_remove_all_parts::type_id) {
			WriteLock lock(m_lockUpdate);
			for(i32 i=0;i<m_nParts;i++) m_parts[i].flags |= geom_removed;
		}
		return 1;
	}
	...
	if (_action->type==pe_action_impulse::type_id) {
		...
		if (!is_unused(action->ipart) || !is_unused(action->partid)) {
			...
			if ((u32)i>=(u32)m_nParts)
				return 0;
			if (!is_unused(action->point))
				i = MapHitPointFromParent(i,action->point);

			float scale = 1.0f+(m_pWorld->m_vars.breakImpulseScale-1.0f)*iszero((i32)m_flags & pef_override_impulse_scale);
			if (m_pStructure) {
				i32 bMarkEntityAsDeforming = 0;
				...
				if (max((m_pStructure->pParts[i].Pext+dP*scale).len2(), (m_pStructure->pParts[i].Lext+dL*scale).len2()*sqr(sz.x+sz.y+sz.z)*0.1f) > sqr(m_parts[i].mass*0.01f)) {
					...
					if (m_pStructure->bHasDirectBreaks && !is_unused(action->point) && action->iSource==0) {
						...
						i32 ijoint=-1; float dist,mindist=1e10f,imp=dP.len2()*sqr(scale);
						for(i32 j=0; j<m_pStructure->nJoints; j++)
	...
	if (_action->type==pe_action_move_parts::type_id) {
		...
		if (pTarget) for(i=m_nParts-1;i>=0;i--)
			if (m_parts[i].id>=action->idStart && m_parts[i].id<=action->idEnd) {
				i32 idnew = action->MapPartId ? action->MapPartId(m_parts[i].id,action->auxData) : m_parts[i].id+action->idOffset;
				if (idnew<0)
					continue;
				...
				if (m_pStructure) for(j=0;j<m_pStructure->nJoints;j++)
					if ((iop=m_pStructure->pJoints[j].ipart[1]==i) || m_pStructure->pJoints[j].ipart[0]==i)	{
						...
						i32 ibuddy = m_pStructure->pJoints[j].ipart[iop^1];
						if (ibuddy>=0 && m_parts[ibuddy].id>=action->idStart && m_parts[ibuddy].id<=action->idEnd) {
							...
							i32 ipart = pTarget->TouchesSphere(pTarget->m_qrot*psj.pt+pTarget->m_pos, m_pStructure->pJoints[i].size*0.5f);
...
void CPhysicalEntity::UpdatePartIdmatBreakable(i32 ipart, i32 nParts)
{
	i32 i,id,flags=0;
	...
	if (m_parts[ipart].pMatMapping) {
		for(i=0;i<(i32)m_parts[ipart].nMats;i++) {
		...
		if ((m_parts[ipart].idmatBreakable>=0 || flags & sf_manually_breakable) && nParts>1) {
			...
			for(i=m_parts[ipart].pPhysGeom->pGeom->GetPrimitiveCount()-1; i>=0; i--) {
				...
				id = m_pWorld->m_SurfaceFlagsTable[m_parts[ipart].pMatMapping[min((i32)m_parts[ipart].nMats-1,id)]];
				m_parts[ipart].idmatBreakable = max(m_parts[ipart].idmatBreakable, (id>>sf_matbreakable_bit)-1);
				flags |= id;
			}
		}
...
i32 CPhysicalEntity::AddGeometry(phys_geometry *pgeom, pe_geomparams* params, i32 id, i32 bThreadSafe)
{
	...
	if (id>=0 && (!(params->flagsCollider & geom_allow_id_duplicates) || params->flags & geom_proxy)) {
		i32 i; for(i=0;i<m_nParts && m_parts[i].id!=id;i++);
		if (i<m_nParts) {
			if (params->flags & geom_proxy) {
				if (pgeom) {
					m_pWorld->GetGeomUpr()->AddRefGeometry(m_parts[i].pPhysGeomProxy = pgeom);
					if (params->bRecalcBBox) {
	...
	get_xqs_from_matrices(params->pMtx3x4,params->pMtx3x3, params->pos,params->q,params->scale, &pgeom,m_pWorld->GetGeomUpr());
	...
	if (m_nParts==m_nPartsAlloc) {
		WriteLock lock(m_lockUpdate);
		i32 nPartsAlloc = m_nPartsAlloc;
		i32 nPartsAllocNew = (nPartsAlloc<2)?nPartsAlloc+1:(nPartsAlloc+4 & ~3); // 0 -> 1 -> 2 -> 4 -> next boundary. 2 is very common in vegetation
		geom *pparts = m_parts;
		...
		for(i32 i=0; i<m_nParts; i++) if (m_parts[i].pNewCoords==(coord_block_BBox*)&pparts[i].pos)
	...
	for(i32 j=0;j<2;j++) m_pWorld->GetGeomUpr()->AddRefGeometry(pgeom);
	...
	if (params->pMatMapping)
		memcpy(m_parts[m_nParts].pMatMapping = new i32[params->nMats], params->pMatMapping, (m_parts[m_nParts].nMats=params->nMats)*sizeof(i32));
	else {
	...
	if (m_parts[m_nParts].idmatBreakable>=0 && m_parts[m_nParts].pPhysGeom->pGeom->GetType()==GEOM_TRIMESH) {
		...
		if (pMesh->m_pTree->GetMaxSkipDim()>0.05f) {
			i32 bNonLockable = isneg(pMesh->m_lockUpdate);
			pMesh->Lock();
            pMesh->RebuildBVTree();
            //pMesh->m_lockUpdate;
            pMesh->Unlock();
...
void CPhysicalEntity::RemoveGeometry(i32 id, i32 bThreadSafe)
{
...
RigidBody *CPhysicalEntity::GetRigidBody(i32 ipart,i32 bWillModify)
{
...
i32 CPhysicalEntity::IsPointInside(Vec3 pt) const
{
	pt = (pt-m_pos)*m_qrot;
	ReadLock lock(m_lockUpdate);
	for(i32 i=0;i<m_nParts;i++) if ((m_parts[i].flags & geom_collides) &&
...
void CPhysicalEntity::AlertNeighbourhoodND(i32 mode)
{
...
i32 CPhysicalEntity::RemoveCollider(CPhysicalEntity *pCollider, bool bRemoveAlways)
{
	PREFAST_ASSUME(pCollider);
	if (m_pColliders && m_iSimClass!=0) {
		i32 i,islot; for(i=0;i<m_nColliders && m_pColliders[i]!=pCollider;i++);
		if (i<m_nColliders) {
...
i32 CPhysicalEntity::AddCollider(CPhysicalEntity *pCollider)
{
	PREFAST_ASSUME(pCollider);
	if (m_iSimClass==0)
		return 1;
...
void CPhysicalEntity::DrawHelperInformation(IPhysRenderer *pRenderer, i32 flags)
{
	//ReadLock lock(m_lockUpdate);
	i32 i,j;
	geom_world_data gwd;
	...
	if (flags & (pe_helper_geometry|pe_helper_lattice)) {
		i32 iLevel = flags>>16 & 0xFFF, mask=0, bTransp=(m_pStructure && (flags & 16 || m_pWorld->m_vars.bLogLatticeTension)) | flags>>29 & 1;
		if (iLevel & 1<<11)
		...
		if (m_pWorld->m_vars.bLogLatticeTension && !m_pStructure && GetMassInv()>0)	{
			char txt[32];
			drx_sprintf(txt, "%.2fkg" ,1.0f/GetMassInv() );
	...
	if ((m_pWorld->m_vars.bLogLatticeTension || flags & 16) && m_pStructure) {
		...
		i32 ipart; index_t idx[]={0,2,1,0,1,3,1,2,3,2,0,3};
		trinfo top[4];
		box bbox;
		...
		for(i=0;i<m_pStructure->nJoints;i++) {
			...
			for(j=0;j<2;j++) if ((u32)(ipart=m_pStructure->pJoints[i].ipart[j])<(u32)m_nParts) {
		...
		if (m_pWorld->m_vars.bLogLatticeTension) {
			...
			if (m_nParts != 0)
			{
				for(i=0;i<m_nParts;i++)	{
					...
					drx_sprintf(txt, "%.2fkg" ,m_parts[i].mass);
			...
			if (m_pStructure->nPrevJoints) {
				...
				for(i=0;i<m_pStructure->nPrevJoints;i++) if (m_pStructure->pJoints[i].tension>0) {
					...
					switch (m_pStructure->pJoints[i].itens) {
						case 0: drx_sprintf(txt, "twist: %.1f/%.1f", tens,maxtens=m_pStructure->pJoints[i].maxTorqueTwist); break;
						case 1: drx_sprintf(txt, "bend: %.1f/%.1f", tens,maxtens=m_pStructure->pJoints[i].maxTorqueBend); break;
						case 2: drx_sprintf(txt, "push: %.1f/%.1f", tens,maxtens=m_pStructure->pJoints[i].maxForcePush); break;
						case 3: drx_sprintf(txt, "pull: %.1f/%.1f", tens,maxtens=m_pStructure->pJoints[i].maxForcePull); break;
						case 4: drx_sprintf(txt, "shift: %.1f/%.1f", tens,maxtens=m_pStructure->pJoints[i].maxForceShift); break;
						default: continue;
					}
	...
	if (m_pWorld->m_vars.bDebugExplosions) {
		...
		if (!m_pStructure || !m_pStructure->Pexpl) {
			...
			drx_sprintf(str, "%.1f/%.1f", m_pStructure->Pexpl[i].len(),m_pStructure->Lexpl[i].len());
			m_parts[i].pPhysGeom->pGeom->GetBBox(&bbox);
...
void CPhysicalEntity::GetMemoryStatistics(IDrxSizer *pSizer) const
{
	...
	for(i32 i=0;i<m_nParts;i++) {
		...
		i32 nMats = m_parts[i].pMatMapping && m_parts[i].pMatMapping!=m_parts[i].pPhysGeom->pMatMapping ? m_parts[i].nMats : 0;
		pSizer->AddObject(m_parts[i].pMatMapping, nMats*sizeof(i32), nMats);
	}
	if(m_pColliders)
	...
	if (m_pStructure) {
		...
		if (m_pStructure->defparts) {
			...
			for(i32 i=0;i<m_nParts;i++) if (m_pStructure->defparts[i].pSkelEnt) {
...
struct SMemSerializer : ISerialize {
	SMemSerializer(i32 size0=256) { buf=new char[size=size0]; pos=0; reading=false; }
	SMemSerializer(char *buf0, bool read=true) { buf=buf0; size=-1; pos=0; reading=read; }
	virtual ~SMemSerializer() { if (size>0) delete[] buf; }
	virtual void ReadStringValue(tukk  name, SSerializeString &curValue, u32 policy=0) {}
	virtual void WriteStringValue(tukk  name, SSerializeString& buffer, u32 policy=0) {}
	virtual void Update( ISerializeUpdateFunction * pUpdate ) {}
	virtual void FlagPartialRead() {}
	virtual void BeginGroup( tukk  szName ) {}
	virtual bool BeginOptionalGroup(tukk szName, bool condition) { return condition; }
...
#define SERIALIZATION_TYPE(T) \
	virtual void Value( tukk  name, T& x, u32 policy=0 ) { \
		if (pos+sizeof(T)>(u32)size) { ReallocateList(buf,size,size+256); size+=256; } \
...
#define SERIALIZATION_TYPE(T) virtual void ValueWithDefault( tukk  name, T& x, const T& defaultValue ) { Value(name,x); }
#include <drx3D/Network/SerializationTypes.h>
#undef SERIALIZATION_TYPE
	virtual void ValueWithDefault( tukk  name, SSerializeString& x, const SSerializeString& defaultValue ) {}
	i32 reading;
	tuk buf;
	i32 pos,size;
};

i32 CPhysicalEntity::GetStateSnapshotTxt(char *txtbuf,i32 szbuf, float time_back)
{
	i32k version = 0;
	SMemSerializer mser;
	if (txtbuf)	{
		for(i32 i=0,j=version,j10; i<4; i++,j=j10)
	...
	return (txtbuf ? bin2ascii((u8*)mser.buf,mser.pos,(u8*)txtbuf) : (mser.pos*4)/3+2)+4;
}

void CPhysicalEntity::SetStateFromSnapshotTxt(tukk txtbuf,i32 szin)
{
	i32 szbuf=szin, version=0;
	for(i32 i=0; i<4; i++)
		version = version*10 + *txtbuf++-'0';
	u8k *txtbuf1 = (u8k*)txtbuf+(*txtbuf=='~' ? 1:0);
	char *buf = new char[szbuf];
	SMemSerializer mser(buf);
	while (!ascii2bin(txtbuf1,szbuf,(u8*)buf)) {
...
void CPhysicalEntity::RemoveBrokenParent(i32 i, i32 nIsles)
{
	...
	for(i32 j=0;j<m_nParts;j++) if (m_pStructure->pParts[j].iParent-1==i)	{
...
i32 CPhysicalEntity::MapHitPointFromParent(i32 iParent, const Vec3 &pt)
{
	if (!m_pStructure || !m_pStructure->pParts)
		return iParent;

	i32 i,j,iprim,iClosest,iter=0;
	...
	do {
		for(i=0,minDist=maxDist*10.0f,iClosest=iParent; i<m_nParts; i++) if (m_parts[i].mass>massCheck)
			for(j=m_pStructure->pParts[i].iParent-1; j>=0; j=m_pStructure->pParts[j].iParent-1) if (j==iParent) {
				...
				i32 bBest = isneg((dist=(ptres[0]-ptres[1]).len2())-minDist);
...
i32 CPhysicalEntity::UpdateStructure(float time_interval, pe_explosion *pexpl, i32 iCaller, Vec3 gravity)
{
	i32 i,j,i0,i1,flags=0,nPlanes,ihead,itail,nIsles,iter,bBroken,nJoints,nJoints0,nMeshSplits=0,iLastIdx=m_iLastIdx;
	...
	pp.q = m_qrot;
	pp.pGridRefEnt = this;
	nPlanes = min(m_nGroundPlanes, (i32)(DRX_ARRAY_COUNT(ground)));
	...
	static  i32 g_lockUpdateStructure = 0;
	...
	{ ReadLock lockUpd(m_lockUpdate);
		...
		if (m_pStructure && m_nParts>0 && m_pStructure->nJoints>0) {
			...
			if (g_nJointsAlloc < m_pStructure->nJoints+1) {
				...
				g_jointidx = new i32[g_nJointsAlloc*2];
			...
			if (m_pWorld->m_vars.breakImpulseScale || m_flags & pef_override_impulse_scale) {
				for(i=0; i<m_nParts; i++) {
					...
					g_parts[i].v += gravity*iszero((i32)m_flags & aef_recorded_physics);
			...
			for(i=m_nParts-1,j=0; i>=0; i--) if (g_parts[i].isle!=0)
				if (nRemoveGeoms+j<DRX_ARRAY_COUNT(idRemoveGeoms))
					j++;
				else
					g_parts[i].isle=0;
			...
			for(j=ihead=0,eprep.idOffs=0,i0=-1; i>=0; i--) {
				...
				if (g_parts[i].isle!=0 && nRemoveGeoms<DRX_ARRAY_COUNT(idRemoveGeoms)) {
					i0 += i-i0 & i0>>31;
			...
			if (j | iter) {
				...
				for(i1=1;i1<=j;i1++) {
					i32 idloc = idRemoveGeoms[nRemoveGeoms-i1]-eprep.idOffs;
				...
				if (m_updStage==0) {
					...
					if (pMeshNew = ((CTriMesh*)m_parts[i].pPhysGeomProxy->pGeom)->SplitIntoIslands(ground,nPlanes,GetType()==PE_RIGID)) {
						...
						if (m_parts[i].pLattice) {
							...
							SIZEOF_ARRAY_OK
							if (j>DRX_ARRAY_COUNT(pChunkMeshesBuf)) {
						...
						for(j=0; pMeshNew; pMeshNew=pMeshNext,j++) {
							...
							if (!m_pStructure) {
								...
								bGeomOverflow |= isneg((i32)(DRX_ARRAY_COUNT(gpAddGeoms))-nAddGeoms-2);
								nAddGeoms += 1-bGeomOverflow;
								//nAddGeoms=min(nAddGeoms+1, DRX_ARRAY_COUNT(gpAddGeoms));
							...
							if (!m_pStructure) {
								...
								i32 partId = m_parts[i].id;	// new foliage proxies will have ids main id+1024*n
									...
									if (pMeshIsland) do {
										...
										if (bBroken) {
											...
											if (pMeshIsland==m_parts[i1].pPhysGeom->pGeom) {
												...
												DrxInterlockedDecrement(( i32*) &pgeom1->nRefCount);
												if (pMeshIslandPrev)
			...
			if (nMeshSplits) {
				...
				for(i=m_nParts-1;i>=0;i--) if (m_parts[i].idmatBreakable<0)	{
					...
					for(pMeshIsland=(CTriMesh*)m_parts[i].pLattice; pMeshIsland; pMeshIsland=(CTriMesh*)pMeshIsland->GetForeignData(-2)) {
						...
						nAddGeoms=min(nAddGeoms+1, (i32)(DRX_ARRAY_COUNT(gpAddGeoms)-1));
					}
					gpAux.bRecalcBBox = 1;
					m_parts[i].pPhysGeom->pGeom->GetBBox(&bbox);
				...
				for(i=m_nParts-1; i>=0; i--)
					if (m_parts[i].flags & geom_removed) {
						...
						nRemoveGeoms = min(nRemoveGeoms+1, (i32)(DRX_ARRAY_COUNT(idRemoveGeoms)-1));
	...
	for(i=0;i<nAddGeoms;i++) {
		i32 id = gpAddGeoms[i].bRecalcBBox | gpAddGeoms[i].bRecalcBBox-2>>31;
		gpAddGeoms[i].bRecalcBBox = 1;
		AddGeometry(pAddGeoms[i], gpAddGeoms+i,id,1);
		DrxInterlockedDecrement(( i32*) &pAddGeoms[i]->nRefCount);
	}
	for(i=0;i<nRemoveGeoms;i++) {
		DrxInterlockedIncrement(( i32*) &pRemoveGeoms[i]->nRefCount);
		RemoveGeometry(idRemoveGeoms[i]);
		DrxInterlockedDecrement(( i32*) &pRemoveGeoms[i]->nRefCount);
	}
...
void CPhysicalEntity::OnContactResolved(entity_contact *pContact, i32 iop, i32 iGroupId)
{
	i32 i;
	if (iop<2 && (u32)pContact->ipart[iop]<(u32)m_nParts &&
			...
			!(pContact->flags & contact_angular))
	{
		...
		if (m_pStructure) {
			...
			if (max(Plen2, Llen2*sqr(sz.x+sz.y+sz.z)*0.1f) >
					sqr(m_parts[i].mass*0.01f)*m_pWorld->m_vars.gravity.len2()*(0.01f+0.99f*iszero((i32)m_flags & aef_recorded_physics)))
			{
...
i32 CPhysicalEntity::GetStateSnapshot(TSerialize ser, float time_back, i32 flags)
{
...
i32 CPhysicalEntity::SetStateFromSnapshot(TSerialize ser, i32 flags)
{
	if (ser.GetSerializationTarget()!=eST_Network)
	{
		bool bVal;
      ...
      psj.bReplaceExisting = true;
			...
			if (str.size()) {
				...
				tukk ptr=str.c_str()+1, *ptr1;
				for(;*ptr;ptr=ptr1+1) {
					i32 id = strtoul(ptr,(tuk*)&ptr1,16);
					for(i=0;i<m_nParts;i++) {
						i32 mask = ~geom_will_be_destroyed | ~-iszero(m_parts[i].id-id);
...
i32 CPhysicalEntity::TouchesSphere(const Vec3 &center, float r)
{
	ReadLock lock(m_lockUpdate);
	i32 i;
...
i32 CPhysicalEntity::CapsulizePart(i32 ipart)
{
	IGeometry *pGeom;
	caps_piece pieceBuf[16],*pieces=pieceBuf;
	pe_geomparams gp;
	i32 i,j,nSlices,nParts=0;
	...
	{ ReadLock lock(m_lockUpdate);
		...
		i32 iax,icnt,nLastParts=0,nNewParts,nPartsAlloc=DRX_ARRAY_COUNT(pieceBuf);
	...
	for(j=0;j<nParts;j++) {
		...
		if (AddGeometry(pgeom, &gp)<0)
			goto exitcp;
		DrxInterlockedDecrement(( i32*) &pgeom->nRefCount);
	}
...
bool CPhysicalEntity::MakeDeformable(i32 ipart, i32 iskel, float r)
{
	if (((CGeometry*)m_parts[iskel].pPhysGeom->pGeom)->IsAPrimitive())
		return false;
	i32 i,j,bPrimitive=((CGeometry*)m_parts[ipart].pPhysGeom->pGeom)->IsAPrimitive();
	geom_world_data gwd[2];
	...
	aap.piVtx = new i32[md[1]->nVertices];
...
void CPhysicalEntity::UpdateDeformablePart(i32 ipart)
{
	i32 i,j;
	mesh_data *md[2];
	Vec3 vtx[3],doffs;
...
CPhysicalPlaceholder *CPhysicalEntity::ReleasePartPlaceholder(i32 i)
{
	...
	int64 timeout = DrxGetTicks()+m_pWorld->m_vars.ticksPerSecond*5;
	while(m_parts[i].pPlaceholder->m_bProcessed & 0xFFFF && DrxGetTicks()<timeout); // make sure no external rwi/geb references it
...
{
	if (!(m_flags & pef_parts_traceable) || (u32)m_iSimClass>2u)	{
		for(i32 i=0;i<m_nParts;i++) if (m_parts[i].pPlaceholder)
			m_pWorld->DestroyPhysicalEntity(ReleasePartPlaceholder(i),0,1);
		m_nUsedParts = 0;
		return;
	}
	if (!m_pUsedParts) {
		if (m_pHeap)
			m_pUsedParts = (i32(*)[16])m_pHeap->Malloc(sizeof(i32) * (MAX_PHYS_THREADS+1) * 16, "Used parts");
		else
			m_pUsedParts = new i32[MAX_PHYS_THREADS+1][16];
...
i32 CPhysicalEntity::GenerateJoints()
{
	i32 i,j,i1,j1,ncont,ihead,itail,nFakeJoints,idGnd=-1,iCaller=get_iCaller();
	CPhysicalEntity **pentlist;
	geom_world_data gwd[2];
	intersection_params ip;
	geom_contact *pcontacts;
	...
	for(i=0;i<m_nParts;i++) if (m_parts[i].mass>0) {
		...
		for(i32 j0=GetUsedPartsCount(iCaller)-1; j0>=0; j0--) {
			...
			for(i1=m_pStructure->nJoints-1; i1>=0; i1--) {
				i32 icode = m_pStructure->pJoints[i1].ipart[1]<<16 | m_pStructure->pJoints[i1].ipart[0];
			...
			if (ncont=m_parts[i].pPhysGeom->pGeom->Intersect(m_parts[j].pPhysGeom->pGeom, gwd,gwd+1, &ip, pcontacts)) {
				...
				for(psj.pt.zero(),psj.n.zero(),i1=0,area=0; i1<ncont; i1++) {
					...
					i32 npt = min(pcontacts[i1].nborderpt, (i32)(DRX_ARRAY_COUNT(pt2d)));
					for(j1=0; j1<npt; j1++)
				...
				i32 icodeMat = GetMatId(m_parts[i].pPhysGeom->pGeom->GetPrimitiveId(0,0),i)<<16 |
											 GetMatId(m_parts[j].pPhysGeom->pGeom->GetPrimitiveId(0,0),j);
				i32 icodeMat1 = icodeMat>>16 | icodeMat<<16;
				for(i1=0,pJoint=&sampleJoint,ihead=5; i1<nFakeJoints; i1++) if (pFakeJoints[i1].id!=joint_impulse) {
					i32 icode = pFakeJoints[i1].ipart[1]<<16 | pFakeJoints[i1].ipart[0];
					if (icode==(i<<16|j) || icode==(j<<16|i)) {
