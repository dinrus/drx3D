...
i32 g_BoolIdBufThread[MAX_PHYS_THREADS+1][4096];
i32 g_BoolGridThread[MAX_PHYS_THREADS+1][4096];
u32 g_BoolHashThread[MAX_PHYS_THREADS+1][8192];
struct inters2d {
	Vec2 pt;
	i32 iedge[2];
};
...
inline i32 line_seg_inters(const Vec2 *seg0, const Vec2 *seg1, Vec2 *ptres)
{
	...
	if (sqr(ds^dp0) < 1E-6f*len2(ds)*len2(dp0)) { // segments are [almost] parallel and touching
		float t[2][2]; const Vec2 *ppt[2][2]; i32 idir;
		t[0][0]=0; t[0][1]=len2(dp0); ppt[0][0]=seg0; ppt[0][1]=seg0+1;
...
inline Vec2i& get_cell(const Vec2 &pt, const Vec2 &rstep, Vec2i &ipt) 
{
	ipt.set(float2int(pt.x*rstep.x-0.5f),float2int(pt.y*rstep.y-0.5f));
	return ipt;
}

inline void get_rect(const Vec2i &ipt0,const Vec2i &ipt1,Vec2i *irect,const Vec2i &isz) 
{
...
inline i32 check_if_inside(i32 iCaller, i32 iobj, Vec2i &ipt,const Vec2i &isz,const Vec2 *ptsrc, const Vec2 &pt) 
{
	i32 bInside,i,bStop=0;
	Vec2 pt0,pt1,dp;
	quotientf ycur;
	if ((u32)ipt.x>=(u32)isz.x || ipt.y<0)
		return 0;
	i32 *g_BoolGrid = ::g_BoolGridThread[iCaller];
	u32 *g_BoolHash = ::g_BoolHashThread[iCaller];
...
static i32 __bforce1=0,__bforce2=0;
i32 boolean2d(booltype type, Vec2 *ptbuf1,i32 npt1, Vec2 *ptbuf2,i32 npt2,i32 bClosed, Vec2 *&ptres,i32 *&pidres)
{
	i32 iCaller = get_iCaller();
	Vec2 *g_BoolPtBuf = ::g_BoolPtBufThread[iCaller];
	i32 *g_BoolIdBuf = ::g_BoolIdBufThread[iCaller];
	i32 *g_BoolGrid = ::g_BoolGridThread[iCaller];
	u32 *g_BoolHash = ::g_BoolHashThread[iCaller];
	inters2d *g_BoolInters = ::g_BoolIntersThread[iCaller];

	Vec2 *ptsrc[2] = { ptbuf1,ptbuf2 };
	i32 npt[2] = { npt1,npt2 };
	Vec2 dp,dp1,ptmin[2],ptmax[2],sz,ptbox[2],ptint[2],pttest,rstep;
	i32 iobj,i,j,idx,nsz,n,npttmp,nptres,ninters=0,istart,ix,iy,bInside,bPrevInside,idx_next,idx_prev,inext,/*inext1,*/iobj1;
	Vec2i ipt0,ipt1,irect[2],isz;
...
//for(i=0;i<max(npt1,npt2);i++) g_BoolIdBuf[i]=0;
	...
	i = (i32)(sizeof(g_BoolGridThread[0])/sizeof(g_BoolGridThread[0][0])-1);
	...
	if (g_BoolGrid[nsz-1]>(i32)(sizeof(g_BoolHashThread[0])/sizeof(g_BoolHashThread[0][0])))
		return 0;
	...
	for(get_cell(ptsrc[iobj][0]-ptbox[0],rstep,ipt0),i=0; i<npt[iobj]; i++) { 
		...
		for(ix=irect[0].x;ix<=irect[1].x;ix++) for(iy=irect[0].y;iy<=irect[1].y;iy++)		NO_BUFFER_OVERRUN
...
//		inext1 = min(inext+1,npt[iobj]-1) & (i+1-npt[iobj]>>31 | ~-bClosed);
		...
		if (bInside | bPrevInside | bClosed) {
			...
			if (nptres>=(i32)(sizeof(g_BoolPtBufThread[0])/sizeof(g_BoolPtBufThread[0][0])))
				return nptres;
		}
		if (bInside | bClosed) {
