...
i32 CSphereGeom::CalcPhysicalProperties(phys_geometry *pgeom)
{ 
	pgeom->pGeom = this;
...
i32 CSphereGeom::PointInsideStatus(const Vec3 &pt)
{
...
static DRX_ALIGN(16) char g_SphIdBuf[1];
i32 CSphereGeom::PrepareForIntersectionTest(geometry_under_test *pGTest, CGeometry *pCollider,geometry_under_test *pGTestColl, bool bKeepPrevContacts)
{
...
i32 CSphereGeom::PreparePrimitive(geom_world_data *pgwd,primitive *&pprim,i32 iCaller)
{
...
i32 CSphereGeom::GetPrimitiveList(i32 iStart,i32 nPrims, i32 typeCollider,primitive *pCollider,i32 bColliderLocal,
...
i32 CSphereGeom::GetUnprojectionCandidates(i32 iop,const contact *pcontact, primitive *&pprim,i32 *&piFeature, geometry_under_test *pGTest)
{
...
i32 CSphereGeom::FindClosestPoint(geom_world_data *pgwd, i32 &iPrim,i32 &iFeature, const Vec3 &ptdst0,const Vec3 &ptdst1,
...
i32 CSphereGeom::UnprojectSphere(Vec3 center,float r,float rsep, contact *pcontact)
{
...
void CSphereGeom::CalculateMediumResistance(const plane *pplane, const geom_world_data *pgwd, Vec3 &dPres,Vec3 &dLres)
{
	...
	lx = max(-r*0.999f,min(r*0.999f,l)); ly = sqrt_tpl(max(0.0f,r*r-lx*lx));
	circ_S = (lx*ly+r*r*(g_PI*0.5f+asin_tpl(lx/r)))*0.5f;
	circ_x = cube(ly)*(-1.0f/3);
	lx = max(-ry*0.999f,min(ry*0.999f,(l-cx)/max(0.001f,fabs_tpl(nv)))); ly = sqrt_tpl(max(0.0f,ry*ry-lx*lx));
...
i32 CSphereGeom::DrawToOcclusionCubemap(const geom_world_data *pgwd, i32 iStartPrim,i32 nPrims, i32 iPass, SOcclusionCubeMap* cubeMap)
{
	...
	axisx = center.GetOrthogonal().normalized(); axisy = (axisx^centerNorm);
...
void CSphereGeom::DrawWireframe(IPhysRenderer *pRenderer, geom_world_data *gwd, i32 iLevel, i32 idxColor)
{
	/*if (iLevel==0) {
		i32 i,ix,iy,iz;
		Vec3 pt0,pt1;
...
void CSphereGeom::GetMemoryStatistics(IDrxSizer *pSizer)
{
