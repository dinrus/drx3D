...
void CLivingEntity::SetGroundCollider(CPhysicalEntity *pCollider, i32 bAcceptStatic)
{
...
i32 CLivingEntity::Awake(i32 bAwake,i32 iSource)
{ 
...
float CLivingEntity::UnprojectionNeeded(const Vec3 &pos,const quaternionf &qrot, float hCollider,float hPivot, 
																				const Vec3 &newdim, i32 bCapsule, Vec3 &dirUnproj, i32 iCaller) const
{
	i32 i,j,nents;
	float t;
		...
		if (pentlist[i]!=this && (!m_pForeignData || pentlist[i]->m_pForeignData!=m_pForeignData) && !pentlist[i]->IgnoreCollisionsWith(this,1)) {
			if (pentlist[i]->GetType()!=PE_LIVING) {
				...
				for(j=0;j<pentlist[i]->m_nParts;j++) if (pentlist[i]->m_parts[j].flags & collider_flags) {
					...
					for(i32 ipart=1;ipart<m_nParts;ipart++) if (m_parts[ipart].flagsCollider & pentlist[i]->m_parts[j].flags) {
...
i32 CLivingEntity::SetParams(pe_params *_params, i32 bThreadSafe)
{
	ChangeRequest<pe_params> req(this,m_pWorld,_params,bThreadSafe);
	if (req.IsQueued()) {
		if (_params->type==pe_player_dimensions::type_id && (u32)m_iSimClass<7u) {
			i32 iter=0,iCaller=get_iCaller();
	...
	if (res = CPhysicalEntity::SetParams(_params,1)) {
		if (_params->type==pe_params_pos::type_id && !(((pe_params_pos*)_params)->bRecalcBounds & 32)) {
			...
			for(i32 i=0;i<3;i++) if (fabs_tpl(m_qrot.v[i])<1e-10f) 
				m_qrot.v[i] = 0;
...
i32 CLivingEntity::GetParams(pe_params *_params) const
{
	i32 res;
...
i32 CLivingEntity::AddGeometry(phys_geometry *pgeom, pe_geomparams* params,i32 id, i32 bThreadSafe)
{
...
void CLivingEntity::RemoveGeometry(i32 id, i32 bThreadSafe)
{
...
i32 CLivingEntity::Action(pe_action* _action, i32 bThreadSafe)
{
...
i32 CLivingEntity::GetStateSnapshot(CStream &stm,float time_back,i32 flags)
{
	...
	i32 iLastGroundColliderPart=m_iLastGroundColliderPart;
...
i32 CLivingEntity::GetStateSnapshot(TSerialize ser, float time_back, i32 flags)
{
 	WriteLock lock0(m_lockUpdate),lock1(m_lockLiving);
...
i32 CLivingEntity::SetStateFromSnapshot(CStream &stm, i32 flags)
{
...
//	i32 bFlying0=m_bFlying;
	...
	u32 imft; stm.ReadNumberInBits(imft,2);
...
i32 CLivingEntity::SetStateFromSnapshot(TSerialize ser, i32 flags)
{
	SLivingEntityNetSerialize helper;
	helper.Serialize( ser );
...
float CLivingEntity::ShootRayDown(le_precomp_entity* pents, i32 nents, le_precomp_part *pparts, const Vec3 &pos,
	Vec3 &nslope, float time_interval, bool bUseRotation,bool bUpdateGroundCollider,bool bIgnoreSmallObjects)
{
	i32 i,j,jbest,ncont,idbest,idbestAux=-1,iPrim=-1,bHasMatSubst=0;
	i32 j1,iCaller=get_iCaller_int();
	Matrix33 R;
	...
	if (bUpdateGroundCollider) {
		...
		if (pentbest) {
			...
			i32 mask = (2<<pentbest->m_iSimClass)-(pentbest->m_id>>31);
...
void CLivingEntity::AddLegsImpulse(const Vec3 &vel, const Vec3 &nslope, bool bInstantChange)
{
	...
	if (m_pLastGroundCollider && m_flags&lef_push_objects &&
			(u32)m_pLastGroundCollider->m_iSimClass-1u<2u && m_pLastGroundCollider->m_flags & pef_pushable_by_players && 
...
void CLivingEntity::RegisterContact(const Vec3 &posSelf, const Vec3& pt,const Vec3& n, CPhysicalEntity *pCollider, i32 ipart,i32 idmat, 
																		float imp, i32 bLegsContact, i32 iPrim, i32 ipartMin)
{
...
void CLivingEntity::ComputeBBox(Vec3 *BBox, i32 flags) 
{ 
...
void CLivingEntity::UpdatePosition(const Vec3 &pos, const Vec3 *BBox, i32 bGridLocked)
{
	...
	if (m_nParts==1)	{
		...
		for(i32 i=0;i<m_nParts;i++) m_parts[i].pNewCoords = (coord_block_BBox*)&m_parts[i].pos;
		ComputeBBox(m_BBox);
	}
...
i32 CLivingEntity::Step(float time_interval)
{
	if (time_interval<=0)
		return 1;
	...
	i32k iCaller = get_iCaller_int();
	i32 i,j,jmin,ipartMin,nents,ncont,bFlying,bWasFlying,bUnprojected,idmat,iPrim, bHasExtraParts=0,
			...
			m_bActiveEnvironment || m_nslope.z<m_slopeSlide || m_velGround.len2()>0))	
	{
		...
		if (m_pWorld->m_vars.iCollisionMode!=0 && m_pWorld->m_vars.bFlyMode) {	//Rich S - is bFlyMode a cheat? Can we disable in release?
			...
			for(i=0; i<nents; i++) {
				...
				for(i32 j1=0; j1<nUsedPartsCount; j1++) {
					...
					ent.nParts+=1-iszero(-j1>>31 & part.partflags & (i32)collider_flags);
				}
			}
			...
			if (m_parts[0].flagsCollider) {
				for(i=nUnproj=0,ncontactSum.zero();i<nents;++i) 
					...
					i32k bHeavy = isneg(minv*0.4f-fMassInv);
					i32k iSimClass = pPrecompEnts[i].iSimClass;
					...
					for(i32 iCyl=0; iCyl<bFastPhys+m_nParts; iCyl++) {
						i32 igwd,flagsCollider;	IGeometry *pCurCyl; 
						if (iCyl<=bFastPhys) {
						...
						for(i32 j1=pPrecompEnts[i].iPartsBegin, bCheckBBox=pPrecompEnts[i].bCheckBBox; j1<pPrecompEnts[i].iPartsEnd; ++j1)
							if (pPrecompParts[j1].partflags & flagsCollider && (!bCheckBBox || AABB_overlap(BBoxInner,pPrecompParts[j1].BBox)))	{
								...
								if (icnt=pCurCyl->Intersect(pPrecompParts[j1].pgeom, gwd+igwd,gwd+1, &ip, pcontacts)) {
									...
									if (iCyl==0 && bHeavy && !(pPrecompParts[j1].partflags & geom_no_coll_response)) {
										i32k bVeryHeavyFlags = (~(-isneg(minv*2-fMassInv))) | pent->m_flags;
										if (m_pWorld->m_bWorldStep==2) { // this means step induced by rigid bodies moving around
											...
											if (bPushOther = iSimClass>0 && minv>0) {// && pent->m_iGroup==m_pWorld->m_iCurGroup) {
												nUnproj = min(nUnproj+1,(i32)(DRX_ARRAY_COUNT(unproj)));
									...
									if (iSimClass==2) {
										...
										i32 bPushOther = (uFlags & lef_push_objects) && (pent->m_flags & pef_pushable_by_players);
										bPushOther |= iszero(iSimClass-2);
										bPushOther &= iszero((i32)(INT_PTR)pent-(i32)(INT_PTR)m_pLastGroundCollider)^1;
										if (bPushOther)	{
			...
			if (movelen>m_size.x*1E-4f && m_parts[0].flagsCollider!=0) {
				...
				do {
					...
					for(i=0; i<nents; ++i) {
						...
						{
							CPhysicalEntity *const pent=pPrecompEnts[i].pent;
							i32 bCheckBBox=pPrecompEnts[i].bCheckBBox;
							for(i32 j1=pPrecompEnts[i].iPartsBegin; j1<pPrecompEnts[i].iPartsEnd; ++j1) {
								u32k uPartFlags = pPrecompParts[j1].partflags;
							...
							if (kd>=0) {
								...
								if (fabs_tpl(gwd[0].v.z)>m_size.z*1E-5f) {
									i32 nSignZ = sgnnz(gwd[0].v.z);	//Could be float?
					...
					if (tmin<=ip.time_interval) {
						...
						if (!bFlying && inrange(ncontact*axis, 0.85f,-0.1f) && (u32)pentmin->m_iSimClass-1u<2u && 
							pentmin->GetMassInv()>m_massinv*0.25f)
							ncontact.z=0, ncontact.normalize();
						i32 bPush = pentmin->m_iSimClass>0 || isneg(min(m_slopeClimb-ncontact*axis, ncontact*axis+m_slopeFall));
						i32 bUnmovable = isneg(-pentmin->m_iSimClass>>31 & ~(-((i32)m_flags & pef_pushable_by_players)>>31));
						bPush &= bUnmovable^1;
						{
							Matrix33 K(ZERO);
							i32 bPushOther = (m_flags & (pentmin->m_iSimClass==3 ? lef_push_players : lef_push_objects)) && 
							...
							bPushOther &= iszero((i32)(INT_PTR)pentmin-(i32)(INT_PTR)m_pLastGroundCollider)^1;
			...
			i32 bGroundContact = isneg(max(pos*axis-m_hPivot-(h+m_groundContactEps), m_slopeFall-nslope*axis));
			if (!bGroundContact)
				ReleaseGroundCollider();
			...
			if (m_pHeadGeom) {
				...
				for(i=0;i<nents;i++) if (pentlist[i]->m_iSimClass==0) {//pentlist[i]->GetType()!=PE_LIVING && pentlist[i]->GetMassInv()*0.4f<m_massinv) {
					for(i32 j1=0;j1<pentlist[i]->GetUsedPartsCount(iCaller);j1++) 
		...
		i32 locked = m_pWorld->RepositionEntity(this,1,BBoxInner);
		pos = coord.pos;
...
float CLivingEntity::CalcEnergy(float time_interval)
{
	...
	for(i32 i=0;i<m_nContacts;i++) // account for extra energy we are going to add by enforcing penertation unprojections
		E += sqr(min(3.0f,(m_pContacts[i].penetration*10.0f)))*m_pContacts[i].pent->GetRigidBody(m_pContacts[i].ipart)->M;
	return E; 
}

i32 CLivingEntity::RegisterContacts(float time_interval,i32 nMaxPlaneContacts)
{
	i32 i,j;
...
i32 CLivingEntity::Update(float time_interval, float damping)
{
	i32 i,j;
...
RigidBody *CLivingEntity::GetRigidBody(i32 ipart,i32 bWillModify)
{
	if (!bWillModify)
...
void CLivingEntity::OnContactResolved(entity_contact *pContact, i32 iop, i32 iGroupId)
{
...
void CLivingEntity::DrawHelperInformation(IPhysRenderer *pRenderer, i32 flags)
{
...
void CLivingEntity::GetMemoryStatistics(IDrxSizer *pSizer) const
{
...
void CLivingEntity::Step_HandleFlying(Vec3 &vel, const Vec3& velGround, i32 bWasFlying, const Vec3& heightAdj, const float kInertia, const float time_interval)
{
...
void CLivingEntity::Step_HandleWasFlying(Vec3& vel, i32& bFlying, const Vec3& axis, i32k bGroundContact)
{
	if (!m_bSwimming) {
