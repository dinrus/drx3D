...
{
	for(i32 i=0;i<NPRIMS;i++) for(i32 j=0;j<8;j++)
...
i32 tri_tri_intersection(const triangle *ptri1, const triangle *ptri2, prim_inters *pinters)
{
	...
	Vec3r axes[3]; axes[2] = ptri1->n^ptri2->n;
	...
	i32 imin,imax,iEdge[2][2], nCross,iNotCross,bCross;
...
i32 tri_box_intersection(const triangle *ptri, const box *pbox, prim_inters *pinters)
{
	...
	i32 i,j,idir,iStart,iStartBest,iEndBest,bOutside[3],bBest,bEnd,bSwitch,iEndCur,bState;
	real sg;
	Vec3i axsg;
	...
	if ((pt[1]-pt[0]^pt[2]-pt[0]).len2() > sqr(pbox->size[inc_mod3[iStart]]*pbox->size[dec_mod3[iStart]])) { 
		i32 nborderpt0 = pinters->nborderpt;
		for(i=0;i<12;i++) {
...
{
	i32 res = tri_box_intersection(ptri,pbox,pinters);
	Vec3 pt; i32 iFeature;
...
i32 tri_cylinder_intersection(const triangle *ptri, const cylinder *pcyl, prim_inters *pinters)
{
	...
	i32 i,j,iStart,iStartBest,iEndBest,bOutside,bEnd,iEndCur,bSwitch,bState,bBest,sg,bInters;//,bVertexContact,bPrevVertexContact;
	r2 = sqr(pcyl->r);
	...
	if (iStartBest<0) { // triangle edges do not intersect cylinder
		...
		for(i=0,pt2d.set(1,0); i<8; i++) {
			...
			float tmp=pt2d.x; pt2d.x=(pt2d.x-pt2d.y)*(sqrt2/2); pt2d.y=(tmp+pt2d.y)*(sqrt2/2); // rotate by 45 degrees
		}
		return 1;
	}
...
{
	i32 res = tri_cylinder_intersection(ptri,pcyl,pinters);
	Vec3 pt; i32 iFeature;
	pt=pinters->pt[0]; pinters->pt[0]=pinters->pt[1]; pinters->pt[1]=pt;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
...
i32 tri_capsule_intersection(const triangle *ptri, const capsule *pcaps, prim_inters *pinters)
{
	...
	real r2,a,b,c,d;
	quotient t[3][2],dist,mindist;
	i32 i,j,iStart,iStartBest,iEndBest,bOutside,bEnd,iEndCur,bSwitch,bState,bBest,sg,bInters;
	r2 = sqr(pcaps->r);
	...
	do {	// find edge-capsule intersections
		dp = ptri->pt[dec_mod3[i]]-ptri->pt[i]; pc = ptri->pt[i]-pcaps->center;
		t[i][0].x = t[i][1].x = -pc*pcaps->axis;
		t[i][0].y = t[i][1].y = dp*pcaps->axis; sg = sgnnz(t[i][0].y); 
		t[i][sg+1>>1].x += pcaps->hh; t[i][sg+1>>1^1].x -= pcaps->hh;
		t[i][0].x*=sg; t[i][0].y*=sg; t[i][1].x*=sg; t[i][1].y*=sg;

		a = dp.len2(); b = dp*(pc+pcaps->axis*(pcaps->hh*sg)); c = (pc+pcaps->axis*(pcaps->hh*sg)).len2()-r2;
		if ((d=b*b-a*c)>=0)	{
			d = sqrt_tpl(d);
			if (quotient(-b+d,a) < t[i][0]) {
				t[i][0].set(-b-d,a); t[i][1].set(-b+d,a);
			} else
				t[i][0] = min(t[i][0],quotient(-b-d,a));
		}
		b = dp*(pc-pcaps->axis*(pcaps->hh*sg)); c = (pc-pcaps->axis*(pcaps->hh*sg)).len2()-r2;
		if ((d=b*b-a*c)>=0)	{
			d = sqrt_tpl(d);
		...
		vec1 = dp^pcaps->axis; vec0 = pc^pcaps->axis;
		a = vec1*vec1; b = vec0*vec1; c = vec0*vec0-r2; d = b*b-a*c;
		if (d>=0 && vec1.len2()>(real)1E-10) {
			d = sqrt_tpl(d); 
			t[i][0] = max(t[i][0],quotient(-b-d,a));
	...
	if (iStartBest<0) { // triangle edges do not intersect capsule
		...
		ptmin = pcaps->center - ptmax; ptmax += pcaps->center; pt = ptmin;
		// check if a line between them intersects triangle
		t[0][0].set((ptri->pt[0]-ptmin)*ptri->n, (ptmax-ptmin)*ptri->n);
		bInters = isneg(fabs_tpl(t[0][0].x*2-t[0][0].y)-t[0][0].y); // t is in (0..1)
		ptmin = ptmin*t[0][0].y + (ptmax-ptmin)*t[0][0].x;
...
i32 capsule_tri_intersection(const capsule *pcaps, const triangle *ptri, prim_inters *pinters)
{
	i32 res = tri_capsule_intersection(ptri,pcaps,pinters);
	Vec3 pt; i32 iFeature;
	pt=pinters->pt[0]; pinters->pt[0]=pinters->pt[1]; pinters->pt[1]=pt;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
...
i32 tri_sphere_intersection(const triangle *ptri, const sphere *psphere, prim_inters *pinters)
{
	...
	real r2,a,b,c,d;
	quotient t[3][2],dist,mindist;
	i32 i,j,iStart,iStartBest,iEndBest,bOutside,bEnd,iEndCur,bState,bBest,bSwitch,bFaceContact=0;
	r2 = sqr(psphere->r);
...
{
	i32 res = tri_sphere_intersection(ptri,psphere,pinters);
	Vec3 pt; i32 iFeature;
	pt=pinters->pt[0]; pinters->pt[0]=pinters->pt[1]; pinters->pt[1]=pt;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
...
i32 ray_tri_intersection(const ray * __restrict pray, const triangle * __restrict ptri, prim_inters * __restrict pinters)
{
...
{
	i32 res = ray_tri_intersection(pray,ptri,pinters);
	i32 iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	pinters->n.Flip();
...
{
	// axes: n2 x n  n1 x n  n = n1 x n2
	Vec3r axes[3]; axes[2] = ptri->n^pplane->n;
	...
	Vec2r pt2d[3];
	i32 sgnx[3],nCross,iNotCross,bCross;
	quotient tmin,tmax;
...
{
	i32 res = tri_plane_intersection(ptri,pplane,pinters);
	Vec3 pt; i32 iFeature;
	pt=pinters->pt[0]; pinters->pt[0]=pinters->pt[1]; pinters->pt[1]=pt;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
...
i32 box_box_intersection(const box *pbox1, const box *pbox2, prim_inters *pinters)
{
	...
	Vec3_tpl<i32> axsg;
	i32 ibox,i,idir,iEdge;
	i32 bBest,sg,bLocInters[2];
	quotient t[2],t1[2];
...
i32 box_cylinder_intersection(const box *pbox, const cylinder *pcyl, prim_inters *pinters)
{
	...
	i32 i,ix,iy,iz,idir,icap,isgz,imask,bNega,bNegad,bInRange,bLocInters[2],sg,bBest;
	quotient t[2],t1[2];
	...
	for(iz=0;iz<3;iz++) if (fabs_tpl(axis[iz])<(real)0.999) {
		...
		for(i=0;i<4;i++) {
			...
			if (sqr(center[iz]+axis[iz]*(hh*icap)-size[iz]*isgz)*n.len2()*fabs_tpl(n[iz]) < sqr(n[iz]*r)*fabs_tpl(n[iz])) {
				...
				imask &= (bNegad&bNega | (bNegad^1)&(bInRange^bNega))<<1 | ((bNegad^1)&(bNega^1) | bNegad&(bInRange^bNega^1));
				if (imask) {
					rx = sqrt_tpl(max((real)0,rx2)); rnz = (real)1.0/n[iz];
					if (imask&1)
...
{
	i32 res = box_cylinder_intersection(pbox,pcyl,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	pinters->n.Flip();
	return res;
...
{
	i32 res = box_sphere_intersection(pbox,psphere,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	pinters->n.Flip();
	return res;
...
i32 box_capsule_intersection(const box *pbox, const capsule *pcaps, prim_inters *pinters)
{
	Vec3r axis,center,n,pt,vec0,vec1,size;
	i32 i,ix,iy,iz,idir,icap,bLocInters[2],sg,bBest;
	quotient t[2],t1[2];
	...
	for(iz=0;iz<3;iz++) {
		...
		for(i=0;i<4;i++) {
			// edge-cap intersections
			pt[ix]=size[ix]*((i<<1&2)-1); pt[iy]=size[iy]*((i&2)-1); pt[iz]=0;
			sg = sgnnz(axis[iz]);	t[0].y=t[1].y = fabs_tpl(axis[iz]);
			t[0].x=t[1].x = ((center-pt)*axis)*sg; t[0].x -= hh; t[1].x += hh;

			kb=(-center[iz]-axis[iz]*(hh*sg)); kc=(pt-center-axis*(hh*sg)).len2()-r*r;
			if ((kd=kb*kb-kc)>=0)	{
				kd = sqrt_tpl(kd);
			...
			pt[iz]=0; kb=(-center[iz]+axis[iz]*(hh*sg)); kc=(pt-center+axis*(hh*sg)).len2()-r*r;
			if ((kd=kb*kb-kc)>=0)	{
				kd = sqrt_tpl(kd);
				if ((inrange((-kb-kd)*t[0].y, t[0].x,t[1].x)^1) & isneg(fabs_tpl(-kb-kd)-size[iz]))
					pt[iz] = -kb-kd, pinters->ptborder[pinters->nborderpt++] = pt*pbox->Basis+pbox->center;
	...
	t[0].set(-hh-r,1); t[1].set(hh+r,1);
	for(iz=bLocInters[0]=bLocInters[1]=0; iz<3; iz++) {
		idir = isneg(axis[iz]); sg = 1-(idir<<1); 
		t1[idir]. set((-size[iz]-center[iz])*sg, fabs_tpl(axis[iz]));
		t1[idir^1].set((size[iz]-center[iz])*sg, fabs_tpl(axis[iz]));
		bLocInters[0] |= (bBest = isneg(t[0]-t1[0]));	// t[0] = max(t[0],t1[0]);
		t[0].x += (t1[0].x-t[0].x)*bBest; t[0].y += (t1[0].y-t[0].y)*bBest;
		bLocInters[1] |= (bBest = isneg(t1[1]-t[1]));	// t[1] = min(t[1],t1[1]);
		t[1].x += (t1[1].x-t[1].x)*bBest; t[1].y += (t1[1].y-t[1].y)*bBest;
	}
	if (t[0]<t[1]) {
		if (bLocInters[0])
			pinters->ptborder[pinters->nborderpt++] = (center+axis*t[0].val())*pbox->Basis + pbox->center;
...
{
	i32 res = box_capsule_intersection(pbox,pcaps,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	pinters->n.Flip();
	return res;
...
{
	i32 res = box_ray_intersection(pbox,pray,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
{
	i32 res = box_plane_intersection(pbox,pplane,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
inline i32 root_inrange(float a,float ad,float b,float hca,float d,float h) {
	i32 sg = sgnnz((-b+d)*ad+hca);
		...
		for(i=-1; i<=1; i+=2) {
			...
			if (fabs_tpl(cosa)<0.003f) {
				...
				if (cube(p)*(1.0f/27)+sqr(q) > 0)	{
					...
					j = 1-sg2d.x*sg2d.y>>1;
					i32 iangle,idx[2] = {0,SINCOSTABSZ};
					do {
					...
					if (fabs_tpl(dc*pcyl[icyl^1]->axis+sina*sgnnz(cosa)*g_sintab[iangle]*sg2d.y*r0*(icyl*2-1)) < pcyl[icyl^1]->hh) {
						bHasInters = 1;
						i32 icenter,idx1,imiddle,iquad,ipass;
...
{
	i32 res = cylinder_sphere_intersection(pcyl,psphere,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
i32 cylinder_capsule_intersection(const cylinder *pcyl, const capsule *pcaps, prim_inters *pinters)
{
	...
	for(i=-1; i<=1; i+=2) {
		...
		if (bOutside) {
			dc -= pcyl->axis*(dc*pcyl->axis); dc -= dc.normalized()*r0;
			dir += dc;
		}
		if (dir.len2()<r1*r1) {
...
{
	i32 res = cylinder_capsule_intersection(pcyl,pcaps,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
{
	i32 sg,bHit,bHit0,bHitCap,bHitSide=0;
	quotient t,tcap;
	Vec3r vec0,vec1;
	real a,b,c,d;
...
{
	i32 res = cylinder_ray_intersection(pcyl,pray,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
{
	i32 res = cylinder_plane_intersection(pcyl,pplane,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
{
	i32 res = sphere_ray_intersection(psphere,pray,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
{
	i32 res = sphere_plane_intersection(psphere,pplane,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
{
	i32 res = ray_plane_intersection(pray,pplane,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
{
	i32 res = capsule_sphere_intersection(pcaps,psphere,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
...
i32 capsule_ray_intersection(const capsule *pcaps, const ray *pray, prim_inters *pinters)
{
	...
	for(icap=0;icap<2;icap++) {
		...
		b = pray->dir*(pray->origin-vec0); c = (pray->origin-vec0).len2()-sqr(pcaps->r);
		...
		if ((d=b*b-a*c)>=0) {
			...
			tmin.x += (t.x-tmin.x)*bHit; tmin.y += (t.y-tmin.y)*bHit;
	...
	vec0 = pray->origin-pcaps->center ^ pcaps->axis; vec1 = pray->dir ^ pcaps->axis;
	a = vec1*vec1; b = vec0*vec1; c = vec0*vec0-sqr(pcaps->r); d = b*b-a*c;
	if (d>=0) {
		d = sqrt_tpl(d); t.set(-b-d, a);
...
{
	i32 res = capsule_ray_intersection(pcaps,pray,pinters), iFeature;
	iFeature=pinters->iFeature[0][0]; pinters->iFeature[0][0]=pinters->iFeature[1][1]; pinters->iFeature[1][1]=iFeature;
	iFeature=pinters->iFeature[0][1]; pinters->iFeature[0][1]=pinters->iFeature[1][0]; pinters->iFeature[1][0]=iFeature;
	return res;
}
