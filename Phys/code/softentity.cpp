...
static quaternionf g_lastqHost(0,0,0,0);
 i32 CPhysicalEntity::g_lockProcessedAux = 0;
...
CSoftEntity::~CSoftEntity()
{
	if (m_vtx) { 
		if (m_pWorld->m_bMassDestruction)
			for(i32 i=0;i<m_nVtx;i++) m_vtx[i].pContactEnt=0;
...
void CSoftEntity::AlertNeighbourhoodND(i32 mode)
{
	for(i32 i=0;i<m_nVtx;i++) if (m_vtx[i].pContactEnt && (!m_vtx[i].bAttached || mode==0)) {
		m_vtx[i].pContactEnt->Release(); m_vtx[i].pContactEnt=0; m_vtx[i].bAttached=0;
	}
	RemoveCore();
}

i32 CSoftEntity::Awake(i32 bAwake, i32 iSource) 
{ 
...
i32 CSoftEntity::AddGeometry(phys_geometry *pgeom, pe_geomparams* params, i32 id, i32 bThreadSafe)
{
	...
	i32 res = CPhysicalEntity::AddGeometry(pgeom,params,id,1);
	WriteLock lock(m_lockUpdate);
	i32 i,j,i0,i1,bDegen,iedge,itri,itri0,ivtx,itrinew,nVtxEdges,(*pInfo)[3];
	...
	if (params->pLattice) {
		...
		i32 *pIdx = new i32[pLattice->m_nTetr*12];
		...
		if (m_flags & se_rigid_core) {
			m_corevtx = new i32[pLattice->m_nVtx];
	...
	if (params->pLattice) {
		...
		i32 *pEdgeMask = new i32[iedge = m_nVtx*(m_nVtx+1)+63>>6];
		memset(pEdgeMask, 0, iedge*sizeof(i32));

		for(i=m_nEdges=0;i<pMesh->m_nTris;i++) for(j=0;j<3;j++) {
			i0 = pMesh->m_pIndices[i*3+j]; i1 = pMesh->m_pIndices[i*3+inc_mod3[j]];
			i32 mask=i1-i0>>31; i0^=i1&mask; i1^=i0&mask; i0^=i1&mask;
			i0 = (i1*(i1+1)>>1)+i0;
		...
		m_pTetrEdges = new i32[pLattice->m_nTetr*6];
		m_pVtxEdges = new i32[m_nEdges*2];
		...
		for(i=iedge=0;i<pLattice->m_nTetr;i++) for(i32 ivtx0=0;ivtx0<3;ivtx0++) for(i32 ivtx1=ivtx0+1;ivtx1<4;ivtx1++,iedge++) {
		...
		m_pTetrQueue = new i32[pLattice->m_nTetr];
	...
	memset(pInfo=new i32[pMesh->m_nTris][3],-1,pMesh->m_nTris*sizeof(pInfo[0]));
	...
	for(j=0;j<3;j++) if (!m_vtx[ivtx=pMesh->m_pIndices[i*3+j]].iSorted) {
		...
		i32 loop=0;	do { // first, trace cw fan until we find an open edge (if any)
...
void CSoftEntity::BakeCurrentPose()
{
	...
	for(i=0; i<m_nVtx; i++) {
		for(j=m_vtx[i].iStartEdge,n.zero(); j<=m_vtx[i].iEndEdge; j++)	{
			i32 j1 = j+1 + (m_vtx[i].iStartEdge-j-1 & ~(j-m_vtx[i].iEndEdge>>31));
...
void CSoftEntity::RemoveGeometry(i32 id, i32 bThreadSafe)
{
...
i32 CSoftEntity::SetParams(pe_params *_params, i32 bThreadSafe)
{
	ChangeRequest<pe_params> req(this,m_pWorld,_params,bThreadSafe);
	if (req.IsQueued())
		return 1;

	i32 res = CPhysicalEntity::SetParams(_params,1);
	if (res) {
	...
	if (_params->type==pe_simulation_params::type_id) {
		...
		if (!is_unused(params->mass) && params->mass>0 && m_nParts>0) {
			i32 i; float dmass=m_parts[0].mass/params->mass, rdmass=params->mass/m_parts[0].mass;
			for(i=0;i<m_nVtx;i++) 
...
i32 CSoftEntity::GetParams(pe_params *_params) const
{
	if (_params->type==pe_simulation_params::type_id) {
		pe_simulation_params *params = (pe_simulation_params*)_params;
...
i32 CSoftEntity::GetStatus(pe_status *_status) const
{
	i32 res;
	...
	if (_status->type==pe_status_softvtx::type_id) {
		...
		if (status->flags & eSSV_LockPos) {
			++m_lockUpdate; for(;*((tuk)&m_lockUpdate+2););
		} 
	...
	if (_status->type==pe_status_dynamics::type_id) {
		...
		if (m_nVtx) {
			...
			for(i32 i=0; i<m_nVtx; status->centerOfMass+=m_vtx[i++].pos);
...
void CSoftEntity::AttachPoints(pe_action_attach_points *action, CPhysicalEntity *pent,i32 ipart, float rvtxmass,float vtxmass, i32 bAttached, const Vec3 &_offs,const quaternionf &_q)
{
	pe_params_rope pr;
	i32 bRope = pent->GetParams(&pr);
	Vec3 offs=_offs; Quat q=_q;
	for(i32 i=0;i<action->nPoints;i++) if (action->piVtx[i]<m_nVtx) {
		...
		if (bAttached & -bRope) {
			...
			for(i32 j=0; j<pr.nSegments; j++) {
				...
				i32 ismin = -isneg(dist.x*mindist.y-mindist.x*dist.y);
...
i32 CSoftEntity::Action(pe_action *_action, i32 bThreadSafe)
{
	...
	if (_action->type==pe_action_impulse::type_id) {
		...
		if (m_nVtx>0 && !is_unused(action->impulse)) {
			...
			if ((u32)i < (u32)pMesh->m_nTris) {
				float rarea,k; i32 idx[3];
				...
				i32 itri; Vec3 w; float wdenom;
				for(i=0; i<pMesh->m_nTris; i++) {
					Vec3 vtx[3],n; for(j=0;j<3;j++) vtx[j]=m_vtx[pMesh->m_pIndices[i*3+j]].pos;
					n = vtx[1]-vtx[0]^vtx[2]-vtx[0];
					i32 bInside = isneg(max(max((pt-vtx[0]^vtx[1]-vtx[0])*n,(pt-vtx[1]^vtx[2]-vtx[1])*n),(pt-vtx[2]^vtx[0]-vtx[2])*n));
	...
	if (_action->type==pe_action_attach_points::type_id) {
		...
		i32 i,ipart=0, bAttached=iszero((intptr_t)pent)^1;
		...
		if (is_unused(action->nPoints) && pent) {
			...
			i32 iCaller=get_iCaller(), nents=pent && action->pEntity!=WORLD_ENTITY ? 1 :
				m_pWorld->GetEntitiesAround(m_BBox[0],m_BBox[1],pents,ent_terrain|ent_static|ent_sleeping_rigid|ent_rigid,0,0,iCaller), npt=0;
			action->piVtx = new i32[m_nVtx];
			for(i32 ient=0;ient<nents;ient++) for(i32 j=0;j<pents[ient]->GetUsedPartsCount(iCaller);j++) {
		...
		i32 ihead,itail,j,i1,imaxLvl=0;
	...
	if (_action->type==pe_action_target_vtx::type_id) {
		...
		if (is_unused(action->points) || !action->points)	{
			...
			for(i32 i=0; i<m_nVtx; i++) if (!m_vtx[i].bAttached) 
				m_vtx[i].ptAttach = (m_vtx[i].pos-action->posHost)*action->qHost;
		} else {
			for(i32 i=0; i<action->nPoints; i++) //if (!m_vtx[i].bAttached) 
				m_vtx[i].ptAttach = (action->points[i]-action->posHost)*action->qHost;
			if (m_maxSafeStep>0) for(i32 i=0;i<m_nEdges;i++) {
	...
	if (_action->type==pe_action_slice::type_id) {
		...
		for(i32 i=0;i<pMesh->m_nTris;i++) pMesh->RecalcTriNormal(i);
		
		primitives::triangle triCut; 
		for(i32 i=0;i<3;i++) triCut.pt[i] = (((action->pt[i]-m_pos)*m_qrot-gp.pos)*gp.q)/gp.scale;
		...
		if (pMesh->Slice(&triCut,0.01f,0.05f)) {
			...
			asp.piVtx = new i32[m_nAttachedVtx];
			for(i32 i=0;i<m_nAttachedVtx;i++) asp.piVtx[i] = m_vtx[i].idx;
			...
			i32 *pVtxEdges=m_pVtxEdges; m_pVtxEdges=0;
			...
			Action(&asp);
			i32 nSimVtx = pMesh->m_nVertices;
			for(i32 i=nSimVtx-1; i>=m_nConnectedVtx; i--)
			...
			for(i32 i=0;i<nVtx;i++) {
				...
				for(i32 j0=vtx[i].iStartEdge;j0<=vtx[i].iEndEdge;j0++) if (edges[pVtxEdges[j0]].ivtx[0]==i)
					for(i32 j1=m_vtx[i].iStartEdge;j1<=m_vtx[i].iEndEdge;j1++) if (edges[pVtxEdges[j0]].ivtx[1]==m_edges[m_pVtxEdges[j1]].ivtx[1])
...
void CSoftEntity::StepInner(float time_interval, i32 bCollMode, check_part *checkParts,i32 nCheckParts, 
														const plane &waterPlane,const Vec3 &waterFlow,float waterDensity,
														const Vec3 &lastposHost, const quaternionf &lastqHost,
                            se_vertex *pvtx)
{
	#define m_vtx pvtx
	i32 i,j,i0,i1,j1,iter,imask;
...
i32 CSoftEntity::Step(float time_interval)
{
	...
	i32 i,j,nEnts=0,i0,i1,nCheckParts=0,bGridLocked=0,nhostPt=0,ijob=-2;
	...
	if ((m_windTimer+=time_interval*4)>1.0f) {
		...
		m_wind1 = w+Vec3(drx_random(0.0f, windage),drx_random(0.0f, windage),drx_random(0.0f, windage));
	}
	...
	{	pentlist[i]->GetLocTransformLerped(j, checkParts[nCheckParts].offset,lastqHost,kr,ktimeBack,this);
		...
		if (box_box_overlap_check(&boxent,&checkParts[nCheckParts].bbox,&Overlapper)) {
			...
			if (m_parts[0].pPhysGeom!=m_parts[0].pPhysGeomProxy && !checkParts[nCheckParts].bPrimitive) {
				...
				checkParts[nCheckParts].nCont = min(checkParts[nCheckParts].nCont, (i32)(sizeof(checkParts[0].contPlane)/sizeof(checkParts[0].contPlane[0])));
	...
	for(i0=m_nAttachedVtx; i0<m_nConnectedVtx; i0++) if (m_vtx[i=m_vtx[i0].idx].pContactEnt) 
		if (!(m_vtx[i].pContactEnt->m_bProcessed & 1<<iCaller) ||
				 (u32)(j = m_vtx[i].pContactEnt->GetCheckPart(m_vtx[i].iContactPart))>=(u32)nCheckParts)
		{
	...
	for(i0=i1=0,center.zero(); i0<m_nAttachedVtx; i0++) {
		...
		if (min(m_maxSafeStep,m_timeStepPerformed+0.001f-m_timeStepFull)>0) 
			for(j=m_vtx[i].iStartEdge; j<=m_vtx[i].iEndEdge; j++) if (m_edges[m_pVtxEdges[j]].len>=0) {
				i32 i2 = m_edges[m_pVtxEdges[j]].ivtx[0]+m_edges[m_pVtxEdges[j]].ivtx[1]-i;
	...
	bGridLocked = m_pWorld->RepositionEntity(this,3-iszero((i32)m_flags & pef_traceable),BBox);
...
i32 CSoftEntity::RayTrace(SRayTraceRes& rtr)
{
	if (m_nVtx>0) {
		CTriMesh *pMesh = (CTriMesh*)m_parts[0].pPhysGeom->pGeom;
		prim_inters inters;
...
i32 CSoftEntity::GetStateSnapshot(CStream &stm,float time_back,i32 flags)
{
	stm.WriteNumberInBits(SNAPSHOT_VERSION,4);
	stm.WriteBits((u8*)&m_qrot0,sizeof(m_qrot0)*8);
	stm.Write(m_bAwake!=0);
	return 1;
}

i32 CSoftEntity::SetStateFromSnapshot(CStream &stm, i32 flags)
{
	i32 ver=0; 
	bool bnz;
	...
	if (!(flags & ssf_no_update)) {
		pe_params_pos pp;
		stm.ReadBits((u8*)&pp.q,sizeof(pp.q)*8);
		SetParams(&pp);
		stm.Read(bnz);
		m_bAwake = bnz? 1:0;
	} else
...
i32 CSoftEntity::GetStateSnapshot(TSerialize ser, float time_back, i32 flags)
{
...
i32 CSoftEntity::SetStateFromSnapshot(TSerialize ser, i32 flags)
{
	if (m_flags & sef_skeleton || flags & 16) {
...
void CSoftEntity::DrawHelperInformation(IPhysRenderer *pRenderer, i32 flags)
{
...
void CSoftEntity::GetMemoryStatistics(IDrxSizer *pSizer) const
{
	if (GetType()==PE_SOFT)
