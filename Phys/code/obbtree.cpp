...
void COBBTree::SetParams(i32 nMinTrisPerNode, i32 nMaxTrisPerNode, float skipdim)
{
...
float COBBTree::Build(CGeometry *pMesh)
{
	...
	memset(m_pMapVtxUsed = new i32[(m_pMesh->m_nVertices-1>>5)+1], 0, (m_pMesh->m_nVertices-1>>3)+1);
...
float COBBTree::BuildNode(i32 iNode, i32 iTriStart,i32 nTris, i32 nDepth)
{
	i32 nHullTris,i,j,nPts,iStart=1<<30,iEnd=-1,idx0=iTriStart*3,nidx=nTris*3;
	index_t *pHullTris=0;
	...
	for(i=0; i<nidx; i++) {
		...
		iStart = min(iStart, (i32)m_pMesh->m_pIndices[idx0+i]);
		iEnd = max(iEnd, (i32)m_pMesh->m_pIndices[idx0+i]);
	}
...
#endif
	i32 iAxis;
	i32 numtris[3],/*nTrisAx[3],*/iPart,iMode[3],idx;
...
void COBBTree::GetNodeBV(BV *&pBV,i32 iNode, i32 iCaller)
{
...
void COBBTree::GetNodeBV(BV *&pBV, const Vec3 &sweepdir,float sweepstep, i32 iNode, i32 iCaller)
{
	iNode = min(iNode, m_nNodes-1);
	pBV = g_BBoxBuf + min(g_BBoxBufPos++, k_BBoxBufSize-1);
	pBV->type = box::type;
...
void COBBTree::GetNodeBV(const Matrix33 &Rw,const Vec3 &offsw,float scalew, BV *&pBV, i32 iNode, i32 iCaller)
{
	iNode = min(iNode, m_nNodes-1);
	pBV = g_BBoxBuf + min(g_BBoxBufPos++, k_BBoxBufSize-1);
	pBV->type = box::type;
...
void COBBTree::GetNodeChildrenBVs(const BV *pBV_parent, BV *&pBV_child1,BV *&pBV_child2, i32 iCaller)
{
	BBox *bbox_parent=(BBox*)pBV_parent, *&bbox_child1=(BBox*&)pBV_child1, *&bbox_child2=(BBox*&)pBV_child2;
	bbox_child1 = g_BBoxBuf + min(g_BBoxBufPos++, k_BBoxBufSize-1);
	bbox_child2 = g_BBoxBuf + min(g_BBoxBufPos++, k_BBoxBufSize-1);
...
void COBBTree::GetNodeChildrenBVs(const BV *pBV_parent, const Vec3 &sweepdir,float sweepstep, BV *&pBV_child1,BV *&pBV_child2, i32 iCaller)
{
	BBox *bbox_parent=(BBox*)pBV_parent, *&bbox_child1=(BBox*&)pBV_child1, *&bbox_child2=(BBox*&)pBV_child2;
	bbox_child1 = g_BBoxBuf + min(g_BBoxBufPos++, k_BBoxBufSize-1);
	bbox_child2 = g_BBoxBuf + min(g_BBoxBufPos++, k_BBoxBufSize-1);
...
void COBBTree::ReleaseLastBVs(i32 iCaller)
{
	g_BBoxBufPos-=2;
}

void COBBTree::ReleaseLastSweptBVs(i32 iCaller)
{
	g_BBoxBufPos-=2;
}

i32 COBBTree::GetNodeContents(i32 iNode, BV *pBVCollider,i32 bColliderUsed,i32 bColliderLocal, geometry_under_test *pGTest,
...
void COBBTree::MarkUsedTriangle(i32 itri, geometry_under_test *pGTest)
{
	if (!pGTest->pUsedNodesMap) return;
	i32 iNode = m_pTri2Node[itri];
...
i32 COBBTree::PrepareForIntersectionTest(geometry_under_test *pGTest, CGeometry *pCollider,geometry_under_test *pGTestColl)
{
	if (m_maxSkipDim<=0) {
		...
		IF (mapsz<=(i32)(DRX_ARRAY_COUNT(g_UsedNodesMap))-g_UsedNodesMapPos,1) {
...
void COBBTree::CleanupAfterIntersectionTest(geometry_under_test *pGTest)
{
	if (pGTest->pUsedNodesMap){
		IF ((u32)(pGTest->pUsedNodesMap-g_UsedNodesMap) > (u32)DRX_ARRAY_COUNT(g_UsedNodesMap), 0) 
		{
...
void COBBTree::GetMemoryStatistics(IDrxSizer *pSizer)
{
...
void COBBTree::Load(CMemStream &stm, CGeometry *pGeom)
{
	...
	m_pTri2Node = new index_t[m_pMesh->m_nTris];
	for(i32 i=0;i<m_nNodes;i++) for(i32 j=0;j<m_pNodes[i].ntris;j++)
...
i32 COBBTree::SanityCheck()
{
	i32 iCaller = MAX_PHYS_THREADS;
	i32k bufLength = DRX_ARRAY_COUNT(g_BBoxBuf);
