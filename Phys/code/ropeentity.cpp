...
CRopeEntity::~CRopeEntity()
{
	...
	delete[] m_attach;
	m_segs = 0; m_nSegs = -1;
	for(i32 i=0;i<2;i++) if (m_pTiedTo[i] && !m_pWorld->m_bMassDestruction)  m_pTiedTo[i]->Release();
}

void CRopeEntity::AlertNeighbourhoodND(i32 mode)
{
	if (!mode) {
		pe_params_rope pr;
		pr.pEntTiedTo[0] = pr.pEntTiedTo[1] = 0;
		SetParams(&pr);
	} else if (mode==2) for (i32 i=0;i<2;i++) if (m_pTiedTo[i] && m_pTiedTo[i]->GetType()==PE_ROPE)
...
i32 CRopeEntity::Awake(i32 bAwake,i32 iSource)
{ 
...
i32 CRopeEntity::SetParams(pe_params *_params, i32 bThreadSafe)
{
	...
	if (res = CPhysicalEntity::SetParams(_params,1)) {
		...
		if (m_nSegs>0 && !(m_flags & (rope_collides|rope_collides_with_terrain)) && (flags0 & (rope_collides|rope_collides_with_terrain)))
			for(i32 i=0;i<=m_nSegs;i++) m_segs[i].pContactEnt = 0;

		RecalcBBox();

		if ((m_flags^flags0) & rope_collides)
			(m_collTypes &= ~(ent_static|ent_sleeping_rigid|ent_rigid|ent_living)) |= (ent_static|ent_sleeping_rigid|ent_rigid|ent_living) & -((i32)m_flags & rope_collides)>>31;
		...
		if (m_segs) for(i=0;i<2;i++) if (!is_unused(params->pEntTiedTo[i])) {
			...
			if (m_pTiedTo[i]!=pPrevTied && pPrevTied)	{
				...
				if (!(m_flags & pef_disabled)) {
					if ((u32)pPrevTied->m_iSimClass<7u)
						pPrevTied->Awake();
					if (m_pTiedTo[i^1] && (u32)m_pTiedTo[i^1]->m_iSimClass<7u)
						m_pTiedTo[i^1]->Awake();
					diff++;
				}
			}
...
i32 CRopeEntity::GetParams(pe_params *_params) const
{
	i32 res;
	...
	if (_params->type==pe_params_rope::type_id) {
		...
		for(i32 i=0;i<2;i++) if (params->pEntTiedTo[i] = m_pTiedTo[i]) {
...
i32 CRopeEntity::GetStatus(pe_status *_status) const
{
	...
	if (_status->type==pe_status_rope::type_id) {
		...
		{ ReadLock lock(m_lockUpdate);
			...
			i32 i;
			i32 *nCollCount[2] = { &(status->nCollStat=0), &(status->nCollDyn=0) };
			...
			for(i=0;i<m_nSegs;i++) if (pContactEnt = *( CPhysicalEntity**)&m_segs[i].pContactEnt)
		...
		if (m_flags & rope_subdivide_segs) {
			...
			status->nVtx = m_nVtx0;
			if (status->pVtx) for(i32 i=0;i<m_nVtx0;i++)
...
void CRopeEntity::EnforceConstraints(float seglen, const quaternionf& qtv,const Vec3& offstv,float scaletv, i32 bTargetPoseActive, float dt)
{
	i32 i,iDir,iStart,iEnd,bHasContacts;
	...
	if (!m_nAttach && (!m_pTiedTo[0] || !m_pTiedTo[1])) {
		...
		if (m_flags & rope_subdivide_segs && m_vtx) {
			...
			for(i=1;i<m_nSegs;i++) m_segs[i].kdP = iszero(m_segs[i].iVtx0+1-m_segs[i+1].iVtx0) & (i32)is_unused(m_vtx[m_segs[i].iVtx0].ncontact);
		...
		i32 iDir0 = (iDir = m_pTiedTo[1] && !m_pTiedTo[0] ? 1:-1);
		do {
			iDir = -iDir;
			iStart = m_nSegs & iDir>>31;
			iEnd = m_nSegs & -iDir>>31;
...
i32 CRopeEntity::Action(pe_action *_action, i32 bThreadSafe)
{
	if (_action->type==pe_action_notify::type_id) {
		...
		if (action->iCode==pe_action_notify::ParentChange) {
			i32 i; Vec3 pos; quaternionf q; float scale;
	...
	if (_action->type==pe_action_target_vtx::type_id) {
		...
		if (!is_unused(action->points) && action->nPoints==m_nSegs+1) {
			i32 bFar = (m_segs[0].pt-action->points[0]).len2()>sqr(m_length*5);
			for(i=0;i<=m_nSegs;i++)	{
			...
			i32 iParent = (m_flags & rope_target_vtx_rel1)!=0;
			Vec3 offsParent;
			quaternionf qParent;
			float scaleParent;
	...
	if (_action->type==pe_action_attach_points::type_id) {
		...
		if (!action->nPoints) {
			...
			for(i=0;i<action->nPoints;i++) if ((u32)action->piVtx[i]>m_nSegs)
				return 0;
	...
	if (_action->type==pe_action_slice::type_id) {
		...
		if (i==0)
			...
			if (i<m_nSegs) {
				...
				if (m_nVtx) {
					i32 nVtx = m_nVtx-m_segs[i].iVtx0+1;
...
RigidBody *CRopeEntity::GetRigidBodyData(RigidBody *pbody, i32 ipart)
{ 
...
void CRopeEntity::GetLocTransform(i32 ipart, Vec3 &offs, quaternionf &q, float &scale, const CPhysicalPlaceholder *trg)	const
{
	scale = 1.0f;
	...
	/*if (m_flags & (rope_target_vtx_rel0 | rope_target_vtx_rel1)) {
		i32 i = (m_flags & rope_target_vtx_rel1)!=0;
		Vec3 offsParent;
		quaternionf qParent;
		float scaleParent;
		if (m_pTiedTo[i]) {
...
void CRopeEntity::FillVtxContactData(rope_vtx *pvtx,i32 iseg, SRopeCheckPart &cp, geom_contact *pcontact)
{
...
void CRopeEntity::StepSubdivided(float time_interval, SRopeCheckPart *checkParts,i32 nCheckParts, float seglen)
{
	i32 i,j,iStart,iEnd,i1,j1,iter,iMoveEnd[2],ncont,bIncomplete[2],bStrainRechecked=0,bRopeChanged;
...
{
	i32 i,j,iseg,jseg,imin=-1,imax=-1;//,nVtx,nSegs,nExtraSegs,nExtraSegsCur;
	...
	for(i=iseg=jseg=0; i<m_nVtx-1; i=j) {
		...
		i32 i0=max(i+1,m_segs[iseg].iVtx0), i1=min(j-1,m_segs[jseg].iVtx0), imid=-1;
		for(i32 ivtx=i0; ivtx<=i1; ivtx++,end0+=seglen) {
	...
	for(i=iseg=nExtraSegs=0; i<m_nVtx-1; i=j,iseg=jseg) {
		...
		i32 nsegs = float2int(((m_vtx[j].pt-m_vtx[i].pt).len()-(m_segs[iseg].pt-m_vtx[i].pt).len()-(m_segs[jseg].pt-m_vtx[j].pt).len())*rseglen);
	...
	for(i=iseg=jseg=nVtx=nSegs=0; i<m_nVtx-1; i=j,iseg=jseg,len-=curlen,nExtraSegs-=nExtraSegsCur) {
		...
		float end0=(m_segs[iseg].pt-m_vtx[i].pt).len(), end1=(m_segs[jseg].pt-m_vtx[j].pt).len();
		i32 nsegs = float2int((curlen-end0-end1)*rseglen)-(nExtraSegsCur = len>0 ? float2int(nExtraSegs*curlen/len) : 0) | jseg-iseg>>31;
		seglen = (curlen-end0-end1)/max(1,nsegs);
		AllocSubVtx(); (m_vtx1[nVtx++] = m_vtx[i]).dir = dir;
		i32 j1 = iszero(m_segs[iseg].iVtx0-i); end0 += (seglen-end0)*j1;
...
float CRopeEntity::Solver(float time_interval, float seglen)
{
	...
	if (m_bHasContacts+m_nAttach || m_flags & rope_subdivide_segs) {
		DRX_PROFILE_REGION(PROFILE_PHYSICS, "Rope solver MC");
		i32 bBounced; iter=m_maxIters;
		...
		DRX_PROFILE_REGION(PROFILE_PHYSICS, "Rope solver CG");
...
void CRopeEntity::ApplyStiffness(float time_interval, i32 bTargetPoseActive, const quaternionf &qtv,const Vec3 &offstv,float scaletv)
{
	i32 i;
...
void CRopeEntity::CheckCollisions(i32 iDir, SRopeCheckPart *checkParts,i32 nCheckParts, float seglen,float rseglen, const Vec3& hingeAxis)
{
	i32 i,j,iseg,iStart,iEnd,iend,hinge=hingeAxis.len2()>0;
...
i32 CRopeEntity::Step(float time_interval)
{
	...
	i32 i,j,k,iDir,iEnd,iter,bTargetPoseActive=m_bTargetPoseActive,bGridLocked=0,bHasContacts=0,nCheckParts=0;
	i32 collTypes = m_collTypes;
	...
	for(i=j=0;i<2;i++) 
		if (m_pTiedTo[i] && (m_pTiedTo[i]->m_iDeletionTime>0 || 
				(u32)m_pTiedTo[i]->m_iSimClass-1u<2u && m_iTiedPart[i]>=m_pTiedTo[i]->m_nParts))
		j |= 1<<i;
	if (j) {
		pe_params_rope pr;
	...
	if ((m_windTimer+=time_interval*4)>1.0f) {
		...
		m_wind1 = dir+Vec3(drx_random(0.0f, a)-a*0.5f,drx_random(0.0f, a)-a*0.5f,drx_random(0.0f, a)-a*0.5f);
	}
	...
	if (collTypes & ent_all | m_flags & rope_collides_with_attachment) {
		DRX_PROFILE_REGION(PROFILE_PHYSICS, "Rope collision");
		i32 objtypes;
		CPhysicalEntity **pentlist,*pentbuf[2];
		i32 iseg,nEnts,iend,ippbv=0,nPrecompPartBVs=0;
		box boxrope,boxpart;
		...
		i32k nMaxTris = m_pWorld->m_vars.maxRopeColliderSize;
		...
		for (k=0,nPrecompPartBVs=m_pWorld->m_threadData[iCaller].nTmpPartBVs; k<nPrecompPartBVs; ++k) { 
			...
			if (box_box_overlap_check(&boxrope,&partBVList[k].partbox,&Overlapper)) {
				...
				if (++nCheckParts==DRX_ARRAY_COUNT(checkParts))
					goto enoughgeoms;
			}
			boxrope.bOriented++;
		} enoughgeoms:
...
i32 CRopeEntity::RegisterContacts(float time_interval,i32 nMaxPlaneContacts)
{
	m_energy = 0;
	if (!m_bStrained || !m_pTiedTo[0] || !m_pTiedTo[1] || m_penaltyScale==0)
		return 1;
	i32 i,j,iStart,iEnd,jStart,sg,nUniqueBodies,ivtx0,ivtx1,icnt0,icnt1;
	...
	for(iStart=j=0; iStart<m_nVtx-1; iStart=iEnd) {
		...
		pContact->Pspare = 0;
		pContact->pBounceCount = (i32*)(m_vtxSolver+jStart);
...
i32 CRopeEntity::Update(float time_interval, float damping)
{
...
i32 CRopeEntity::RayTrace(SRayTraceRes& rtr)
{
...
void CRopeEntity::ApplyVolumetricPressure(const Vec3 &epicenter, float kr, float rmin)
{
	if (m_nSegs>0 && m_maxForce>0 && m_pTiedTo[0] && m_pTiedTo[1]) {
		...
		float dP;
		Vec3 r;
		i32 bThreadSafe = -(get_iCaller()-MAX_PHYS_THREADS>>31);
...
i32 CRopeEntity::GetStateSnapshot(CStream &stm,float time_back,i32 flags)
{
	ReadLock lock(m_lockUpdate);
	stm.WriteNumberInBits(SNAPSHOT_VERSION,4);
	WritePacked(stm,m_nSegs);

	if (m_nSegs>0) for(i32 i=0; i<=m_nSegs; i++) {
...
i32 CRopeEntity::SetStateFromSnapshot(CStream &stm, i32 flags)
{
	i32 i,ver=0; 
	bool bnz;
...
i32 CRopeEntity::GetStateSnapshot(TSerialize ser, float time_back, i32 flags)
{
	i32 i;
	bool bAwake;
...
i32 CRopeEntity::SetStateFromSnapshot(TSerialize ser, i32 flags)
{
	i32 i;
	bool bAwake;
	i32 nSegs;
...
void CRopeEntity::DrawHelperInformation(IPhysRenderer *pRenderer, i32 flags)
{
	...
	if (flags & pe_helper_geometry) {
		...
		if (flags & 16) {
			...
			for(i=0;i<nSegs;i++) {
				...
				i32 iend = i-1>>31 | (i+1-m_nSegs>>31)+1;
...
void CRopeEntity::GetMemoryStatistics(IDrxSizer *pSizer) const
{
	if (GetType()==PE_ROPE)
...
i32 CRopeEntity::GetVertices(strided_pointer<Vec3>& vtx) const
{
	if (m_flags & rope_subdivide_segs && m_nVtx > 0) {
		vtx = strided_pointer<Vec3>(&m_vtx[0].pt0, sizeof(rope_vtx));
		return m_nVtx0;
...
void CRopeEntity::GetRandomPoints(Array<PosNorm> points, CRndGen& seed, EGeomForm eForm) const
{
	...
	strided_pointer<Vec3> vtx;
	i32 nVerts = GetVertices(vtx);

	for (auto& ran : points) {
		Vec3 dir;
		if (eForm == GeomForm_Vertices) {
			i32 i = seed.GetRandom(0, nVerts - 1);
			ran.vPos = vtx[i];
			dir = (vtx[min(i+1,nVerts-1)] - vtx[max(i-1,0)]).normalized();
		}
		else {
			i32 i = ext.RandomPart(seed);
