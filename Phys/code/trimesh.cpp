...
#include <drx3D/Phys/overlapchecks.h>
#include <drx3D/Phys/intersectionchecks.h>
#include <drx3D/Phys/unprojectionchecks.h>
#include <drx3D/Phys/bvtree.h>
#include <drx3D/Phys/aabbtree.h>
#include <drx3D/Phys/obbtree.h>
...
InitTriMeshGlobals::InitTriMeshGlobals() {
  for(i32 iCaller=0; iCaller<=MAX_PHYS_THREADS; iCaller++) {
	...
	, m_voxgrid(0)
{
	...
	static_assert(DRX_ARRAY_COUNT(m_bConvex) == DRX_ARRAY_COUNT(m_ConvexityTolerance), "Invalid array size!");
	for(i32 i=0;i<DRX_ARRAY_COUNT(m_bConvex);i++)	{
		m_bConvex[i]=0;
...
inline void swap(i32 *v, i32 i1,i32 i2)
{	i32 ti=v[i1]; v[i1]=v[i2]; v[i2]=ti; }

i32 bin_search(i32 *v,index_t *idx,i32 n,i32 refidx)
{
	i32 left=0,right=n,m;
	do {
		m = left+right>>1;
...
template<class ftype> void qsort(i32 *v,strided_pointer<ftype> pkey, i32 left,i32 right)
{
	i32 i, j, mi;
	ftype m;
...
void qsort(i32 *v, i32 left,i32 right)
{
	i32 i,j, mi;
	i32 m;
...
CTriMesh* CTriMesh::CreateTriMesh(strided_pointer<const Vec3> pVertices,strided_pointer<unsigned short> pIndices,char *pIds,i32 *pForeignIdx,
																	i32 nTris, i32 flags, i32 nMinTrisPerNode,i32 nMaxTrisPerNode, float favorAABB)
{
	__gtrimesh++;
	i32 i;
	if (sizeof(index_t)<4)
	...
	m_nVertices = 0;
	for(i=0;i<m_nTris;i++)
		m_nVertices = max(m_nVertices, (i32)max(max(m_pIndices[i*3],m_pIndices[i*3+1]),m_pIndices[i*3+2]));
	m_nVertices++;
	...
	if (!(flags & mesh_shared_foreign_idx) && pForeignIdx)
		memcpy(m_pForeignIdx = new i32[m_nTris], pForeignIdx, m_nTris*sizeof(i32));
	else if ((flags & mesh_shared_foreign_idx) && !pForeignIdx)	{
		m_pForeignIdx = new i32[m_nTris]; m_flags &= ~mesh_shared_foreign_idx;
		for(i=0;i<m_nTris;i++) m_pForeignIdx[i] = i;
	}	else
		m_pForeignIdx = pForeignIdx;

	i32 *vsort[3],j,k,*pList[4],nList[4];
	struct vtxinfo {
		i32 id;
		Vec3i isorted;
	};
	...
	if (!(flags & mesh_no_vtx_merge)) {
		...
		for(i=0;i<3;i++) vsort[i] = new i32[max(m_nTris,m_nVertices)];
		for(i=0;i<4;i++) pList[i] = new i32[m_nVertices];
		...
		if (!(flags & mesh_no_filter)) {
			...
			{
				j--;
				for(k=0;k<3;k++) { i32 itmp=m_pIndices[j*3+k]; m_pIndices[j*3+k]=m_pIndices[i*3+k]; m_pIndices[i*3+k]=itmp; }
				if (m_pIds) { char stmp=m_pIds[j]; m_pIds[j]=m_pIds[i]; m_pIds[i]=stmp; }
				if (m_pForeignIdx) { i32 itmp=m_pForeignIdx[j]; m_pForeignIdx[j]=m_pForeignIdx[i]; m_pForeignIdx[i]=itmp; }
			}	else i++;
			m_nTris = i;
		}
	} else
	...
	if (flags & mesh_SingleBB) {
		...
		if (flags & mesh_AABB_rotated) {
			if (nTrees>0) {
				...
				if (m_pForeignIdx) memcpy(pTreeFIdx[nTrees-1] = new i32[m_nTris], m_pForeignIdx, sizeof(m_pForeignIdx[0])*m_nTris);
			}
			CAABBTree *pTree = new CAABBTree;
...
CTriMesh *CTriMesh::Clone(CTriMesh *src, i32 flags)
{
	m_lockUpdate = 0;
	...
	if (!(flags & mesh_shared_vtx)) {
		m_pVertices = strided_pointer<Vec3>(new Vec3[m_nVertices]);
		for(i32 i=0;i<m_nVertices;i++) m_pVertices[i] = src->m_pVertices[i];
	...
	if (!(flags & mesh_shared_foreign_idx) && src->m_pForeignIdx)
		memcpy(m_pForeignIdx = new i32[m_nTris], src->m_pForeignIdx,m_nTris*sizeof(m_pForeignIdx[0]));
	...
	if (src->m_pVtxMap)
		memcpy(m_pVtxMap = new i32[m_nVertices], src->m_pVtxMap,m_nVertices*sizeof(m_pVtxMap[0]));
...
i32 CTriMesh::CalculateTopology(index_t *pIndices, i32 bCheckOnly)
{
	i32 i,j,i1,j1,nTris,itri,*pTris,*pVtxTris,*pEdgeTris,nBadEdges[2]={0,0};
	Vec3 edge,v0,v1;
	float elen2;
	quotientf sina,minsina;
	trinfo dummy,*pTopology = !bCheckOnly ? m_pTopology : &dummy;
	strided_pointer<Vec3mem> pVtxMem = strided_pointer<Vec3mem>((Vec3mem*)m_pVertices.data,m_pVertices.iStride);
	memset(pTris = new i32[m_nVertices+1], 0,(m_nVertices+1)*sizeof(i32)); // for each used vtx, points to the corresponding tri list start
	pVtxTris = new i32[m_nTris*4]; // holds tri lists for each used vtx
...
void CTriMesh::RebuildBVTree(CBVTree *pRefTree)
{
	...
	i32 nMinTrisPerNode=2,nMaxTrisPerNode=4,i,j,jnext,*pIdxOld2New;
	Vec3 n,nnext,BBox[2];
	trinfo t,tnext;
	box bbox;
	CBVTree *pTree = pRefTree ? pRefTree : m_pTree;
	pIdxOld2New = new i32[m_nTris*2];
	...
	for(i=0;i<DRX_ARRAY_COUNT(m_bConvex);m_ConvexityTolerance[i++]=-1);
...
i32 CTriMesh::BuildIslandMap()
{
	...
	m_nIslands = 0;
	if (m_nTris>0) {
		i32 i,iFreeTri,itri,imask,*pTris,nTris,nTrisAlloc;
		Vec3 vtx[3];
...
CTriMesh *CTriMesh::SplitIntoIslands(plane *pGround,i32 nPlanes, i32 bOriginallyMobile)
{
	...
	i32 i,j,imask,isle,idx,itri,ivtx,nRemovedTri=0,*pTriMap,*pVtxMap;
	...
	memset(pVtxMap = new i32[m_nVertices], -1, m_nVertices*sizeof(i32));
	memset(pTriMap = new i32[m_nTris], -1, m_nTris*sizeof(i32));
	...
	if (m_pForeignIdx && nRemovedTri>0) {
		...
		if (j) {
			...
			AddRef(); AddRef();
			pmu->pRemovedTri = new i32[(pmu->nRemovedTri=j)+1];
			for(i=j=0; i<m_nTris; i++) {
...
void CTriMesh::Empty()
{
	...
	if (m_nTris>0) {
		...
		pmu->pRemovedTri = new i32[(pmu->nRemovedTri = m_nTris)+1];
		pmu->pMesh[0]=pmu->pMesh[1] = this;
		AddRef(); AddRef();
		for(i32 i=0;i<m_nTris;i++)
...
void CTriMesh::CollapseTriangleToLine(i32 itri,i32 ivtx, i32 *pTriMap, bop_meshupdate *pmu)
{
	// welds the ivtx+1'th vertex of itri to its ivtx'th vertex
	if (itri>=0 && pTriMap[itri]!=-2) {
		i32 i,j,iprev,iedge;
		i32 ibuddy[3] = { m_pTopology[itri].ibuddy[dec_mod3[ivtx]],m_pTopology[itri].ibuddy[inc_mod3[ivtx]],m_pTopology[itri].ibuddy[ivtx] };
...
i32 CTriMesh::FilterMesh(float minlen,float minangle,i32 bLogUpdates)
{
	if (m_flags & mesh_no_filter)
		return 0;
	i32 i,j,iedge,ibuddy,ibuddy1,*pTriMap,nChanges,iter=0;
	i32 idxmin,idxmax,imask,ivtx[3],*pVtxMap=m_pVtxMap;
	Vec3 vtx[4];
	float len0,len1,minlen2=sqr(minlen),minangle2=sqr(minangle);
	bop_meshupdate mu;
	memset(pTriMap = new i32[m_nTris], -1, m_nTris*sizeof(i32));
	if (!pVtxMap)	{
		pVtxMap = new i32[m_nVertices];
	...
	if (imask!=1) {
		...
		memset(pTriMap, -1, m_nTris*sizeof(i32));
		CalculateTopology(m_pIndices);
	}
...
template<class dtype> void ReallocateListCompact(dtype *&plist, i32 sznew)
{
...
void CTriMesh::CompactTriangleList(i32 *pTriMap, bool bNoRealloc)
{
	i32 i,j,itri,idx,imask;
...
float CTriMesh::GetExtent(EGeomForm eForm) const
{
	...
	if (!ext) {
		i32 nParts = TriMeshPartCount(eForm, m_nTris*3);
		ext.ReserveParts(nParts);
		for (i32 i=0; i<nParts ; i++) {
			i32 nIndices[3];
			Vec3 aVec[3];
			for (i32 v = TriIndices(nIndices, i, eForm)-1; v >= 0; v--)
...
void CTriMesh::GetRandomPoints(Array<PosNorm> points, CRndGen& seed, EGeomForm eForm) const
{
	...
	else
	{
		...
		for (auto& ran : points) {
			i32 nPart = extent.RandomPart(seed);

			i32 aIndices[3];
			PosNorm aRan[3];
			for (i32 v = TriIndices(aIndices, nPart, eForm) - 1; v >= 0; v--)
...
float CTriMesh::GetVolume()
{
	if (is_unused(m_V)) {
		m_V = 0;
		for(i32 itri=0; itri<m_nTris; itri++) {
			Vec3 vtx[3]; for(i32 i=0;i<3;i++) vtx[i] = m_pVertices[m_pIndices[itri*3+i]];
...
i32 CTriMesh::IsConvex(float tolerance)
{
	i32 idx,i,j;
	for(idx=0; idx<DRX_ARRAY_COUNT(m_bConvex)-1 && m_ConvexityTolerance[idx]>=0 && m_ConvexityTolerance[idx]!=tolerance; idx++);
...
i32 CTriMesh::CalcPhysicalProperties(phys_geometry *pgeom)
{
	pgeom->pGeom = this;
...
i32 CTriMesh::PrepareForIntersectionTest(geometry_under_test *pGTest, CGeometry *pCollider,geometry_under_test *pGTestColl, bool bKeepPrevContacts)
{
	i32k iCaller = pGTest->iCaller;
	...
	i32 nNodeTris = pTree->MaxPrimsInNode();

	IF (nNodeTris<=(i32)(DRX_ARRAY_COUNT(g_IdxTriBuf))-g_IdxTriBufPos, 1) {
	...
	IF (m_nMaxVertexValency<=(i32)(DRX_ARRAY_COUNT(g_IdxTriBuf))-g_IdxTriBufPos, 1) {
	...
	IF (m_nMaxVertexValency<=(i32)(DRX_ARRAY_COUNT(g_iFeatureBuf))-g_iFeatureBufPos, 1) {
		pGTest->iFeature_buf = g_iFeatureBuf+g_iFeatureBufPos; g_iFeatureBufPos+=m_nMaxVertexValency;
	}
	else
		pGTest->iFeature_buf = new i32[m_nMaxVertexValency];

	i32 szbuf = max(nNodeTris,m_nMaxVertexValency);
	IF (szbuf<=(i32)(DRX_ARRAY_COUNT(g_IdBuf))-g_IdBufPos, 1) {
	...
	IF (m_nMaxVertexValency<=(i32)(DRX_ARRAY_COUNT(g_SurfaceDescBuf))-g_SurfaceDescBufPos, 1) {
	...
	IF (m_nMaxVertexValency<=(i32)(DRX_ARRAY_COUNT(g_EdgeDescBuf))-g_EdgeDescBufPos, 1) {
...
void CTriMesh::CleanupAfterIntersectionTest(geometry_under_test *pGTest)
{
	...
	i32k iCaller = pGTest->iCaller;
	if ((u32)((indexed_triangle*)pGTest->primbuf-g_IdxTriBuf) > (u32)(DRX_ARRAY_COUNT(g_IdxTriBuf)))
		delete[] pGTest->primbuf;
	if ((u32)((indexed_triangle*)pGTest->primbuf1-g_IdxTriBuf) > (u32)(DRX_ARRAY_COUNT(g_IdxTriBuf)))
		delete[] pGTest->primbuf1;
	if ((u32)(pGTest->iFeature_buf-g_iFeatureBuf) > (u32)(DRX_ARRAY_COUNT(g_iFeatureBuf)))
		delete[] pGTest->iFeature_buf;
	if ((u32)(pGTest->idbuf-g_IdBuf) > (u32)(DRX_ARRAY_COUNT(g_IdBuf)))
		delete[] pGTest->idbuf;
	if ((u32)(pGTest->surfaces-g_SurfaceDescBuf) > (u32)(DRX_ARRAY_COUNT(g_SurfaceDescBuf)))
		delete[] pGTest->surfaces;
	if ((u32)(pGTest->edges-g_EdgeDescBuf) > (u32)(DRX_ARRAY_COUNT(g_EdgeDescBuf)))
		delete[] pGTest->edges;
}


i32 CTriMesh::GetFeature(i32 iPrim,i32 iFeature, Vec3 *pt)
{
	i32 npt=0;
...
void CTriMesh::PrepareTriangle(i32 itri,triangle *ptri, const geometry_under_test *pGTest)
{
	i32 idx = itri*3;
...
i32 CTriMesh::PreparePrimitive(geom_world_data *pgwd, primitive *&pprim, i32 iCaller)
{
...
i32 CTriMesh::TraceTriangleInters(i32 iop, primitive *pprims[], i32 idx_buddy,i32 type_buddy, prim_inters *pinters,
																	geometry_under_test *pGTest, border_trace *pborder)
{
	indexed_triangle *ptri = (indexed_triangle*)pprims[iop];
	i32 itri,itri0=ptri->idx,itri_end,itri_prev,itri_cur,iedge,itypes[2],iter;
	Vec3 dir0,dir1;
...
i32 CTriMesh::GetUnprojectionCandidates(i32 iop,const contact *pcontact, primitive *&pprim,i32 *&piFeature, geometry_under_test *pGTest)
{
	i32 i,itri,iFeature = pcontact->iFeature[iop];
	indexed_triangle *ptri = (indexed_triangle*)pGTest->primbuf1;
	intptr_t idmask = ~iszero_mask(m_pIds);
	char idnull=(char)-1, *ptrnull=&idnull, *pIds = (tuk)((intptr_t)m_pIds&idmask | (intptr_t)ptrnull&~idmask);
	...
	switch (iFeature & 0x60) {
		case 0x00: { // vertex - return triangle fan around it (except the previous contacting triangle)
				i32 itri_prev,itri0,iedge,ntris=0;
...
i32 CTriMesh::GetPrimitiveList(i32 iStart,i32 nPrims, i32 typeCollider,primitive *pCollider,i32 bColliderLocal,
															 geometry_under_test *pGTest,geometry_under_test *pGTestOp, primitive *pRes,char *pResId)
{
	intptr_t mask;
	i32 i,j,iEnd=min(m_nTris,iStart+nPrims),bContact,bPivotCulling;
	char *pIds,noid=(char)-2,*pnoid=&noid;
	mask = ~iszero_mask(m_pIds);
	intptr_t inv_mask = ~mask;
	pIds = (tuk)((intptr_t)m_pIds&mask | (intptr_t)pnoid&inv_mask);
...
i32 CTriMesh::RegisterIntersection(primitive *pprim1,primitive *pprim2, geometry_under_test *pGTest1,geometry_under_test *pGTest2,
																	 prim_inters *pinters)
{
	//DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );
  ...
  pprims[0] = &tri; pprims[1] = pprim2;
	i32 iFeature_dummy = -1;
  i32 *piFeature[2];
  piFeature[0] = piFeature[1] = &iFeature_dummy;
	i32 idx_prim[2] = { ((indexed_triangle*)pprim1)->idx, -1 };
	i32 bNoUnprojection=0,bSurfaceSurfaceContact=0,bSurfaceEdgeContact=0,bUseLSNormal=0;
	i32 i,j,res=0,ipt,ibest,jbest,iop,nprims1,nprims2,nSmallSteps;
	i32 indexed_triangle::*pidxoffs=0;
	i32k iCaller = pGTest1->iCaller;
	float len,maxlen;
	real t,tmax;
	border_trace border;
	...
	border.szbuf = min(100+pGTest1->pParams->bExactBorder*2900, (i32)(DRX_ARRAY_COUNT(g_BrdPtBuf)) - g_BrdPtBufPos-16);
	if (border.szbuf<0)
		return 0;
	...
	do {
		...
		for(nSmallSteps=0; contact_best.t>0 && nSmallSteps<3; ) {
			...
			for(i=0,ptr[0]=(tuk)pprims[0]; i<nprims1; i++,ptr[0]=ptr[0]+pGTest[0]->szprim)
				for(j=0,ptr[1]=(tuk)pprims[1]; j<nprims2; j++,ptr[1]=ptr[1]+pGTest[1]->szprim)
	...
	if (bNoUnprojection) {
		...
		if (bSurfaceSurfaceContact || bSurfaceEdgeContact) {
			...
			i32 *idbuf1[2],*idbuf2[2],*idbuf,id,idmask,bEdgeEdge;
			i32 npt1,npt2,npt;
			G(PolyPtBufPos) = 0;
	...
	if (*pGTest[0]->pnContacts>=pGTest[0]->nMaxContacts || pGTest[0]->pParams->bStopAtFirstTri || pGTest[0]->pParams->bBothConvex)
		...
		if (!pGTest[0]->pParams->bNoAreaContacts &&
				g_nAreas<DRX_ARRAY_COUNT(g_AreaBuf) && g_nAreaPt<DRX_ARRAY_COUNT(g_AreaPtBuf))
		{
			...
			pres->parea->npt = 0; pres->parea->nmaxpt = DRX_ARRAY_COUNT(g_AreaPtBuf)-g_nAreaPt;
...
i32 CTriMesh::GetNeighbouringEdgeId(i32 vtxid, i32 ivtx)
{
	i32 iedge,itri,itrinew,itri0,iter=50;
...
i32 CTriMesh::PreparePolygon(coord_plane *psurface, i32 iPrim,i32 iFeature, geometry_under_test *pGTest, Vec2 *&ptbuf,
														 i32 *&pVtxIdBuf,i32 *&pEdgeIdBuf)
{
	i32 *pUsedVtxMap,UsedVtxIdx[16],nUsedVtx, *pUsedTriMap,UsedTriIdx[16],nUsedTri;
	i32 i,ihead,itail,ntri,nvtx,itri,itri_parent,ivtx0,ivtx,itri_new,iedge,iedge_open,ivtx0_new,ivtx_start,ivtx_end,ivtx_end1,
	...
	if ((m_nVertices-1>>5)+1<DRX_ARRAY_COUNT(g_UsedVtxMap))
		pUsedVtxMap = g_UsedVtxMap;
	else memset(pUsedVtxMap = new i32[(m_nVertices-1>>5)+1], 0, ((size_t)m_nVertices-1>>5)+1<<2);
	if ((m_nTris-1>>5)+1<DRX_ARRAY_COUNT(g_UsedTriMap))
		pUsedTriMap = g_UsedTriMap;
	else memset(pUsedTriMap = new i32[(m_nTris-1>>5)+1], 0, ((size_t)m_nTris-1>>5)+1<<2);
	nUsedVtx = nUsedTri = 0;

	#define ENQUEUE_TRI \
	if (itri_new>=0 && !(pUsedTriMap[itri_new>>5] & 1<<(itri_new&31)) && m_pNormals[itri_new]*n0>1.0f-2E-4f) {	\
		ihead = ihead+1&(i32)(DRX_ARRAY_COUNT(g_TriQueue))-1; \
		ntri = min(ntri+1,(i32)(DRX_ARRAY_COUNT(g_TriQueue))); \
	...
	while(ntri>0) {
		...
		ivtx_head = ivtx0 = g_TriQueue[itail].ivtx0; itail = itail+1&DRX_ARRAY_COUNT(g_TriQueue)-1; ntri--;
		...
		if (nSides==0) { // add new vertex to the list if triangle does not touch neighbours
			if (nvtx==DRX_ARRAY_COUNT(g_VtxList))
				break;
	...
	do {
		...
		if ((u32)(g_VtxList[ivtx_head].id>>8)>=(u32)m_nTris)
			break;
	...
	} while(ivtx_head!=ivtx_start && nvtx<DRX_ARRAY_COUNT(g_VtxList));
...
i32 CTriMesh::PreparePolyline(coord_plane *psurface, i32 iPrim,i32 iFeature, geometry_under_test *pGTest, Vec2 *&ptbuf,
															i32 *&pVtxIdBuf,i32 *&pEdgeIdBuf)
{
	i32 *pUsedVtxMap,UsedVtxIdx[16],nUsedVtx=0;
	i32 nvtx,iedge,itri,itri_prev,ivtx,itri0,iorder,i,iprev,iter=0;
	Vec3 n0,edge,pt;
	...
	pVtxIdBuf = g_PolyVtxIdBuf + g_PolyPtBufPos;
	pEdgeIdBuf = g_PolyEdgeIdBuf + g_PolyPtBufPos;
	for(nvtx=0,ivtx=iprev; ivtx>=0 && nvtx<DRX_ARRAY_COUNT(g_VtxList); ivtx=g_VtxList[ivtx].ibuddy[1],nvtx++) {
...
i32 CTriMesh::FindClosestPoint(geom_world_data *pgwd, i32 &iPrim,i32 &iFeature, const Vec3 &ptdst0,const Vec3 &ptdst1,
															 Vec3 *ptres, i32 nMaxIters)
{
	Vec3 pt[3],n,nprev,edge,dp,seg,cross,ptdst,ptseg[3];
	i32 i,itri,ivtx,iedge,bEdgeOut[4],bOut,itri0,inewtri,itrimax,bBest,iter;
	float t,tmax,dist[3],ptdstw=1.0f;
	i32 iPrim_hist[4]={-1,-1,-1,-1},iFeature_hist[4]={-1,-1,-1,-1},ihist=0,bLooped;
	i32 bLine = isneg(1E-4f-(seg=ptdst1-ptdst0).len2());
	ptseg[0] = ptdst0; ptseg[1] = ptdst1;
	ptres[1] = ptdst0;
	ptres[0].Set(1E10f,1E10f,1E10f);
	vector2_w_enforcer<float> wdiv(ptres[1],ptres[0],ptdstw);

	if ((u32)iPrim<(u32)m_nTris && (iFeature&0x1F)<3) do {
...
i32 CTriMesh::PointInsideStatusMesh(const Vec3 &pt, indexed_triangle *pHitTri)
{
	i32 res=0;
	float mindist=1E20f;
	triangle atri;
...
void CTriMesh::PrepareForRayTest(float raylen)
{
	...
	if (m_nHashPlanes==0) {
		...
		float rcellsize=0;
		Vec3i isz;
		i32 i,iPlane,b2Planes,bForce=isneg(raylen);
		raylen *= 1-2*bForce;
...
void CTriMesh::HashTrianglesToPlane(const coord_plane &hashplane, const Vec2 &hashsize, grid &hashgrid,index_t *&pHashGrid,index_t *&pHashDataRet,
																		float rcellsize, float pad, const index_t* pIndices, i32 nTris, i32 maxhash)
{
	float maxsz,tsx,tex,tsy,tey,ts,te;
	Vec2 sz,pt[3],edge[3],step,rstep,ptc,sg;
	Vec2i isz,irect[2],ipt;
	i32 i,itri,ipass,bFilled,imax,ix,iy,idx;
...
i32 CTriMesh::Intersect(IGeometry *pCollider, geom_world_data *pdata1,geom_world_data *pdata2, intersection_params *pparams, geom_contact *&pcontacts)
{
	...
	if (pCollider->GetType()==GEOM_RAY && m_nHashPlanes>0) {
		...
		i32 nTris[3],iPlane,iListPlane,iListRes,iListTmp,ix,iy,iCell,bNoBackoff;
		Vec2 pt[2];
		Vec2i ipt[2],irect[2];
		...
		prim_inters inters;
		unprojection_mode unproj;
		i32 i,j,i1,jmax,nSmallSteps,iEdge,bActive,bThreadSafe,bThreadSafeMesh,overflowed=0,valid=0, nContacts=0;
		i32 iCaller = get_iCaller();
		intptr_t idmask = ~iszero_mask(m_pIds);
		char idnull=(char)-1, *pidnull=&idnull, *pIds=(tuk)((intptr_t)m_pIds&idmask|(intptr_t)pidnull&~idmask);
		...
		if ((u32)pdata1->iStartNode-1u<(u32)m_nTris) {
...
float CTriMesh::CalculateBuoyancy(const plane *pplane, const geom_world_data *pgwd, Vec3 &massCenter)
{
	...
	i32 itri,i,j,imask,nAbove,iLow,iHigh,sign[3],nPieces;
...
i32 CTriMesh::DrawToOcclusionCubemap(const geom_world_data *pgwd, i32 iStartPrim,i32 nPrims, i32 iPass, SOcclusionCubeMap* cubeMap)
{
	Vec3 pt[3];
	for(i32 i=iStartPrim;i<iStartPrim+nPrims;i++) {
...
namespace {
	template<class T> struct SplitArray {
		...
		T *ptr[2];
		i32 sz[2];
		T& operator[](i32 idx) { return ptr[idx>>27][idx & ~(1<<27)]; }
		T MapSafe(i32 idx) {
			i32 iarr=idx>>27, islot=idx & ~(1<<27);
			i32 bValid = -inrange(islot, -1,sz[iarr]);
			i32 ival = ptr[iarr][islot & bValid] | ~bValid;
			return idx & (ival>>31) | ival & ~(ival>>31);
		}
	};
}

void CTriMesh::RemapForeignIdx(i32 *pCurForeignIdx, i32 *pNewForeignIdx, i32 nTris)
{
	if (m_pForeignIdx) {
		i32 i,iop;
		SplitArray<i32> pIdxMap;
		bop_meshupdate *pmu;

		for(i=0;i<nTris;i++)
			pIdxMap.sz[pCurForeignIdx[i]>>27] = max(pIdxMap.sz[pCurForeignIdx[i]>>27], pCurForeignIdx[i]&~BOP_NEWIDX0);
		++pIdxMap.sz[0]; ++pIdxMap.sz[1];
		pIdxMap.ptr[1] = (pIdxMap.ptr[0] = new i32[pIdxMap.sz[0]+pIdxMap.sz[1]])+pIdxMap.sz[0];
...
void CTriMesh::AppendVertices(Vec3 *pVtx,i32 *pVtxMap, i32 nVtx)
{
	if (nVtx>0) {
		if (!ExpandVtxList()) {
...
float CTriMesh::GetIslandDisk(i32 matid, const Vec3 &ptref, Vec3 &center,Vec3 &normal,float &peakDist)
{
	if (!m_pIds)
		return 0;
	ReadLock lock(m_lockUpdate);
	i32k szq=256;
	i32 i,j,itri,itri1=-1,ihead,itail,queue[szq];
...
void CTriMesh::DrawWireframe(IPhysRenderer *pRenderer, geom_world_data *gwd, i32 iLevel, i32 idxColor)
{
	i32 i,ix,iy;
...
void CTriMesh::GetMemoryStatistics(IDrxSizer *pSizer)
{
	if (GetType()==GEOM_TRIMESH)
		pSizer->AddObject(this, sizeof(CTriMesh));
	m_pTree->GetMemoryStatistics(pSizer);
	i32 i, ivtx0; for(i=0,ivtx0=m_nVertices;i<m_nTris*3;i++) ivtx0=min(ivtx0,(i32)m_pIndices[i]);
...
static i32 *g_pVtxUsed=0;
static i32 *g_pMapBuf=0,g_nVtxAlloc=0;

static inline void ExpandGlobalLoadState(i32 nVerts)
{
	if (g_nVtxAlloc>0)
		delete[] g_pVtxBuf,delete[] g_pMapBuf, delete[] g_pVtxUsed;
	g_pVtxBuf = new Vec3[g_nVtxAlloc = (nVerts-1 & ~511)+512];
	g_pVtxUsed = new i32[g_nVtxAlloc];
	g_pMapBuf = new i32[g_nVtxAlloc];
}
...
void CTriMesh::Load(CMemStream &stm, strided_pointer<const Vec3> pVertices, strided_pointer<unsigned short> pIndices, char *pIds)
{
	...
	stm.Read(bVtxMap);
	if (bVtxMap) {
		m_pVtxMap = (m_flags & mesh_shared_vtx) ? new i32[m_nVertices] : g_pMapBuf;
		for(i=0;i<m_nVertices;i++)
			m_pVtxMap[i] = stm.Read<u16>();
	}
	...
	if (bForeignIdx && !(m_flags & mesh_full_serialization)) {
		...
		m_pForeignIdx = new i32[m_nTris];
		for(i=0;i<m_nTris;i++) {
			m_pForeignIdx[i] = stm.Read<u16>();
		...
		if (!(m_flags & mesh_shared_vtx)) {
			...
			m_pVertices = strided_pointer<Vec3>((Vec3*)pVertices.data,pVertices.iStride);
		for(i=0;i<m_nTris*3;i++) { m_pIndices[i] = stm.Read<u16>(); }
		stm.Read(bIds);
		if (bIds) {
	...
	if (!(m_flags & mesh_shared_vtx)) {
		if (m_pVtxMap) for(i=0,m_ivtx0=m_nVertices;i<m_nTris*3;i++)
			m_ivtx0 = min(m_ivtx0, min((i32)m_pIndices[i],(i32)m_pVtxMap[m_pIndices[i]]));
		else for(i=0,m_ivtx0=m_nVertices;i<m_nTris*3;i++)
			m_ivtx0 = min(m_ivtx0, (i32)m_pIndices[i]);
		if (m_ivtx0*4>m_nVertices) {
			...
			if (m_pVtxMap) {
				memcpy(m_pVtxMap=new i32[m_nVertices-m_ivtx0], g_pMapBuf+m_ivtx0, (m_nVertices-m_ivtx0)*sizeof(m_pVtxMap[0]));
	...
	if (m_pVtxMap==g_pMapBuf)
		memcpy(m_pVtxMap=new i32[m_nVertices], g_pMapBuf, m_nVertices*sizeof(i32));
}
...
i32 CTriMesh::ExpandVtxList()
{
	if (m_ivtx0>0) {
		...
		for(i=0;i<m_nTris*3;i++) m_pIndices[i] += m_ivtx0;
		if (m_pVtxMap) {
			memcpy((m_pVtxMap = new i32[m_nVertices+m_ivtx0])+m_ivtx0, pmap, m_nVertices*sizeof(i32));
...
i32 CTriMesh::UnprojectSphere(Vec3 center,float r,float rsep,contact *pcontact)
{
	ReadLock lock(m_lockUpdate);
	i32 i,iedge=0,itri,itriNext,nOutside,idx[3],bInside,icell;
	triangle atri;

	if ((u32)(itri=pcontact->iFeature[0])<(u32)m_nTris) {
		for(i=0;i<3;i++)
	...
	PrepareForRayTest(0);
	if (!m_pUsedTriMap)
		m_pUsedTriMap = new u32[m_nTris+31>>5];
...
namespace TriMeshStaticData
{
	void GetMemoryUsage(IDrxSizer *pSizer)
	{
		...
		pSizer->AddObject(g_pMapBuf, sizeof(i32)*g_nVtxAlloc);
	}
}
...
template<bool rngchk> CTriMesh::voxgrid_tpl<rngchk> CTriMesh::Voxelize(quaternionf q, float celldim, const index_t* pIndices, i32 nTris, i32 pad)
{
	i32 i,ix,iy,iz,idmask=-1;
	...
	for(i=0;i<nTris;i++) if (pIds[i&idmask]>=0) {
		Vec3 vtx[3]; for(i32 j=0;j<3;j++) vtx[j] = m_pVertices[pIndices[i*3+j]]*q-c;
	...
	vox_iterate(grid,ix,iy,iz) {
		i32 &vox = grid(ix,iy,iz); vox = min(1,max(0,vox));
	}
	return grid;
}
...
#include <drx3D/Phys/boxgeom.h>
template<bool rngchk> void CTriMesh::voxgrid_tpl<rngchk>::DrawHelpers(IPhysRenderer *pRenderer, const Vec3& pos,const quaternionf& qrot,float scale, i32 flags)
{
	i32 ix,iy,iz,bitmax,v;
	...
	union {	float f; uint i; } u = { (float) bitVis };
...
quotientf EdgeDistSq(const CTriMesh &mesh, i32 istart,i32 iend,i32 ivtx)
{
...
i32 crop_polygon_with_bound(const Vec2 *ptsrc,i32k *idedgesrc,i32 nsrc, const Vec2 &sz,i32 icode,i32 iedge, Vec2 *ptdst,i32 *idedgedst);
...
i32 CTriMesh::Boxify(primitives::box *pboxes,i32 nMaxBoxes, const SBoxificationParams &params)
{
	...
	i32k szQueue = 256;
	struct {
		i32 itri,itriBase,iedgeBase,bSmall;
		void set(i32 _itri,i32 _itriBase,i32 _iedgeBase,i32 _bSmall) { itri=_itri;itriBase=_itriBase;iedgeBase=_iedgeBase;bSmall=_bSmall; }
	}	queue[szQueue];
	i32 itriBig,itri,itri1,itail,ihead,nPatches=0,ivtxMin,ivtxMax;
	...
	for(itriBig=m_nTris-1; itriBig>=0; itriBig--) {
		...
		do { // grow patches around large triangles
			...
			i32 bSmall = queue[itail].bSmall, bSmall1;
			if (!bSmall) {
			...
			i32 itriBase = queue[itail].itriBase & bSmall | itri & ~bSmall;
			i32 iedgeBase = queue[itail].iedgeBase & bSmall;
			for(i=0;i<3;i++) {
		...
		if (nbest.len2()) {	// find the optimal 2d box for the patch
			...
			i32 nhull = qhull2d(pt2d,j,edges);
	...
	for(i=nPatches-1;i>=0;i--) {
		...
		for(iz=iz0=iz1=float2int(((patches[j].center-voxpatch.center)*voxpatch.q).z*rcelldim-0.5f); iz>=-voxpatch.sz.z; iz--) {
			for(ix=ibbox[0].x,nFilled=nReused=0;ix<ibbox[1].x;ix++) for(iy=ibbox[0].y;iy<ibbox[1].y;iy++)	{
				i32 &vox=voxpatch(ix,iy,iz); nFilled+=vox&1; nReused+=vox>>1;
		...
		for(ix=ibbox[0].x;ix<ibbox[1].x;ix++) for(iy=ibbox[0].y;iy<ibbox[1].y;iy++)	for(iz=iz0;iz>=iz1;iz--) {
			...
			for(i32 ioffs=0;ioffs<4;ioffs++) { // apply the 'used' markers back to main (oversample x4 to avoid gaps due to aliasing)
	...
	vox_iterate(voxmain,ix,iy,iz) if ((voxmain(ix,iy,iz)&7)==1) {
		...
		do { // locate islands of 'unused' voxel; build semi-optimal boxes around them
			...
			for(d=Vec3(1); d.z<4; i=++d.x>>2,d.x-=3&-i,i=(d.y+=i)>>2,d.y-=3&-i,d.z+=i) {
				i32 &buddy = voxmain(ivox1=ivox+d-Vec3i(2)); j += iszero((buddy&7)-3);
				if ((buddy&7)==1)
					buddy|=4, queueVox[ihead=min(ihead+1,(i32)(DRX_ARRAY_COUNT(queueVox))-1)]=ivox1, ci+=ivox1;
			}
		} while(ihead!=itail);
...
static i32 ineighb[27*4] = {-1};
static Vec3i idx2dir[27],idx2dir1[19];
static i32 dirIsAxis;
static float inv[27] = {0};

template<class itype> inline i32 getNextBit(itype& mask)
{
	if (!mask)
		return -1;
	i32 bit = ilog2((mask^mask-1)+1)-1;
	mask &= ~((itype)1<<bit);
	return bit;
}

inline i32 check_islands(u32 mask, Vec4i *centers=0)
{
	i32 queue[32],ihead,itail,nislands=0;
	for(; mask; nislands++) {
		for(queue[0]=getNextBit(mask),itail=0,ihead=1; itail!=ihead; )
			for(i32 idx=queue[itail++],i=0,j;i<4;i++) if (mask & 1<<(j=ineighb[idx*4+i]))
				mask &= ~(1<<(queue[ihead++]=j));
		if (centers) {
			Vec3i c(0); for(i32 i=0;i<ihead;i++) c+=idx2dir[queue[i]];
...
inline i32 packdir(const Vec3i& idir) { return (idir.z&31)<<10 | (idir.y&31)<<5 | (idir.x&31); }
inline Vec3i unpackdir(i32 i,i32 bit) { i=(u32)i>>bit & ~(bit>>31); return Vec3i((i<<32-5)>>32-5, (i<<32-10)>>32-5, (i<<32-15)>>32-5); }

struct SNbList {
	i32 mask;
	i32 count,count1;
};
template<bool rngchk> inline SNbList packnb(CTriMesh::voxgrid_tpl<rngchk>& vox, const Vec3i& ic,i32 ibit,i32 ibitDir)
{
	SNbList res; res.mask=res.count=res.count1=0;
	Vec3i dir0 = unpackdir(vox(ic),ibitDir);
	for(i32 i=0,j,j0,v; i<27; res.count+=j, res.count1+=j0 & dirIsAxis>>i++) {
		Vec3i dir1 = unpackdir(v=vox(ic+idx2dir[i]),ibitDir);
		res.mask |= (j = (j0=v>>ibit & 1) & ~(dir0*dir1>>31))<<i;
	}
	return res;
}
template<bool rngchk> inline i32 packnb(CTriMesh::voxgrid_tpl<rngchk>& vox, const Vec3i& ic,i32 ibit)
{
	i32 mask=0;
	for(i32 i=0;i<27;i++)
...
inline void count_neighbors(CTriMesh::voxgrid_tpl<false>& vox, i32 bitSrc, i32 bitDst, i32 auxAND=-1)
{	// count neighbors in principal directions using running sums
	Vec3i ic,dc;
	auxAND &= ~(7<<bitDst);
	for(i32 ix=0,mask=1; ix<3; ix++,mask=7,bitSrc=bitDst) {
		i32 iy=inc_mod3[ix], iz=dec_mod3[ix], sum,v0,v1,auxAdd=ix>>1;
...
inline i32 dilatable(CTriMesh::voxgrid_tpl<false>& vox, const Vec3i& ic)
{
	if ((vox(ic) & 2+8)!=2)
		return 0;
...
inline i32 is_line(const SNbList& nb, i32 maxIsleSz, Vec3i *idir=0)
{
	static Vec4i isle[19];
	i32 state0=nb.mask>>13&1, count=nb.count-state0, mask=nb.mask&~(1<<13);
	if (count<2)
		return 0;
	else if (count==2) {
		i32 i=getNextBit(mask), j=ilog2(mask);
		if (idir)	{
...
inline Vec3i trace_line(CTriMesh::voxgrid_tpl<false>& vox, const Vec3i& ic, Vec3i indir, i32 ibit)
{
	i32 imax = 13;
	for(i32 i=0;i<27;i++) if (idx2dir[i]*indir>idx2dir[imax]*indir && vox(ic+idx2dir[i]) & 1<<ibit)
		imax = i;
...
{
	i32 ncnt=0;
	for(i32 i=0;i<6;i++) { // count filled face neighbors
...
template<class t_prim> inline i32 cell_inside(CTriMesh::voxgrid_tpl<false>&, const t_prim&, const Vec3i&);
template<> inline i32 cell_inside(CTriMesh::voxgrid_tpl<false> &vox, const box& box, const Vec3i& ipt)
{
	Vec3 ptloc = (box.Basis*((Vec3(ipt)+Vec3(0.5f))*vox.celldim-box.center)).abs()-box.size;
	return isneg(max(max(ptloc.x,ptloc.y),ptloc.z));
}
template<> inline i32 cell_inside(CTriMesh::voxgrid_tpl<false> &vox, const cylinder& cyl, const Vec3i& ipt)
{
...
template<> inline i32 cell_inside(CTriMesh::voxgrid_tpl<false> &vox, const capsule& caps, const Vec3i& ipt)
{
...
{
	Vec3i ipt;
	for(i32 ipass=0;ipass<3;ipass++)
		for(i32 iz=0; iz<3; iz++)	{	// trace lines in xyz; blend inside/outside state along the length; average between xyz, fill if >0.5
			...
			for(ipt[ix]=BBox[0][ix];ipt[ix]<=BBox[1][ix];ipt[ix]++) for(ipt[iy]=BBox[0][iy];ipt[iy]<=BBox[1][iy];ipt[iy]++) {
				i32 bounds[2] = { 1<<29 }; float val[3];
				for(ipt[iz]=BBox[0][iz];ipt[iz]<=BBox[1][iz];ipt[iz]++) {
					i32 i = cell_inside(vox,prim,ipt);
				...
				if (bounds[0]==1<<29)
					continue;
				if (!ipass) {
					for(i32 i=0;i<2;i++) { ipt[iz]=bounds[i]; val[i]=(vox(ipt)&1 & isneg(fabs_tpl(bounds[i])-(vox.sz[iz]-2-(bounds[i]>>31))))*1024; }
	...
	i32 thresh = float2int(1024*3*fillThresh);
	voxiter_BBox if (cell_inside(vox,prim,ipt)) {
		i32& v=vox(ipt), fill=isneg(thresh-(v>>19));
		v = fill | v & ~(1|-1<<19);
	}
}

Vec2i CheckPrim(CTriMesh::voxgrid_tpl<false>& vox, i32 itype, const primitive *prim, float &V, float inflate=0, float fillThresh=-1)
{
	...
	switch (itype) {
		case box::type:	{
			...
			if (fillThresh<0)	voxiter_BBox {
				i32 i = cell_inside(vox,box,ipt); ncells += Vec2i(i & vox(ipt) & 1,i);
			}	else
		...
		case cylinder::type: {
			...
			if (fillThresh<0) voxiter_BBox {
				i32 i = cell_inside(vox,cyl,ipt); ncells += Vec2i(i & vox(ipt) & 1,i);
			}	else
		...
		case capsule::type: {
			...
			if (fillThresh<0) voxiter_BBox {
				i32 i = cell_inside(vox,caps,ipt); ncells += Vec2i(i & vox(ipt) & 1,i);
			}	else
...
inline bool MergeLine(SVoxLine& ldst, const SVoxLine& lsrc)
{
	...
	i32 len[]={ (i32) dir[0].len2(), (i32) dir[1].len2() }, i=isneg(len[0]-len[1]), closed[]={ ldst.closed,lsrc.closed };
			...
			((org[i^1]-org[i]^dir[i]).len2() <= len[i]*4 || (org[i^1]+dir[i^1]-org[i]^dir[i]).len2() <= len[i]*4)) // distance is <=2 voxels
	{
		...
		for(i32 iendDst=ldst.closed=0;iendDst<2;iendDst++) for(i32 isrc=0;isrc<2;isrc++) for(i32 iendSrc=0;iendSrc<2;iendSrc++)
...
#include <drx3D/Phys/cylindergeom.h>
#include <drx3D/Phys/capsulegeom.h>

i32 CTriMesh::Proxify(IGeometry **&pOutGeoms, SProxifyParams *pparams)
{
	SProxifyParams params;
	if (pparams) params = *pparams;
	i32 isle,nTris,nIslands, i,j;
	Vec3i ic;
	index_t *pTris=0;
	i32 *nbcnt=0,*nbidx=0,nbcntAlloc=0,nbidxAlloc=0,nGeoms=0;
	Vec3r *norm[2] = {0,0};
	Vec3 *vtx=0, *pNormals=0;
	i32 *pVtxMap = params.convexHull ? new i32[m_nVertices+(m_nVertices>>5)+1] : 0;
	i32k MAXPRIMS = 256;
	...
	auto GetIsland = [this,&pTris,&pNormals](i32 isle,i32 nTris=0)
	{
		for(i32 i=m_pIslands[isle].itri; i!=0x7fff; i=m_pTri2Island[i].inext) {
	...
	{
		memset(pVtxMap+m_nVertices,0,((m_nVertices>>5)+1)*sizeof(i32));
		i32 imin=m_nVertices,imax=-1,i,j;
		for(i32 isle=getNextBit(islandMap); isle>=0; isle=getNextBit(islandMap))
			for(i=m_pIslands[isle].itri; i!=0x7fff; i=m_pTri2Island[i].inext) for(j=0;j<3;j++) {
				i32 ivtx = m_pIndices[i*3+j];
		...
		i32 nTris = qhull(m_pVertices,j,pTris);
	...
	if (params.closeHoles) {
		...
		i32 *pHoleVtx = new i32[m_nVertices], *pHoleTris = new i32[m_nVertices*6];
		i32 nTrisAlloc=m_nTris, nTris00=m_nTris;
		...
		for(isle=getNextBit(mask); isle>=0; isle=getNextBit(mask)) {
			i32 nTris0=m_nTris, itriLast;
			for(i=m_pIslands[isle].itri; i!=0x7fff; i=m_pTri2Island[i].inext)
				for(i32 ivtx=(itriLast=i,0);ivtx<3;ivtx++) if (m_pTopology[i].ibuddy[ivtx]<0) {
					Vec2i mark(-1,0); i32 count=10,nHoleVtx=0;
					for(i32 iedge=ivtx,itri=i,itri1;(itri-mark.x|iedge-mark.y);) if ((itri1=m_pTopology[itri].ibuddy[inc_mod3[iedge]])>=0)	{
					...
					i32 nHoleTris = TriangulatePoly(pHoleVtx2d,nHoleVtx+1,pHoleTris,m_nVertices*6);
					...
					for(j=0;j<nHoleTris;j++) {
						for(i32 ivtx=0;ivtx<3;ivtx++) m_pIndices[(m_nTris+j)*3+ivtx] = pHoleVtx[pHoleTris[j*3+2-ivtx]];
	...
	for(isle=getNextBit(params.islandMap); isle>=0 && nGeoms<params.maxGeoms; isle=getNextBit(params.islandMap)) {
		...
		i32 ihead=0,itail;
		auto build_surface = [&](i32 bit,i32 bitIdx,i32 bitDir,bool useCorners)
		{
			Vec3i *pidx2dir; i32 ndirs,j0,has0;
			if (useCorners)
		...
		if (params.findPrimLines | params.findPrimSurfaces) {
			...
			for(;i<ihead;i++)	if (is_line(packnb(vox,queue[i],1,4),3)) {
 			...
 			for(i32 ilayer=itail=0,ihead1; itail<ihead; ilayer++) {
		...
		if (params.findPrimSurfaces) vox_iterate1(vox,ic)
			if ((vox(ic)&(2|8))==2 && inrange((nb=packnb(vox,ic,1,4)).count,4,21) && nb.count1<6 && check_islands(nb.mask^(1<<27)-1)==2 && nsurf<MAXPRIMS) {	// find surface skeletons
				...
				if (ihead >= params.minSurfCells) {
					...
					for(i32 iter=sqrt(ihead)*0.5f*params.surfPrimIters; iter>0; iter--,swap(norm[1],norm[0]))
					...
					for(i=0; i<ihead && nsurf<MAXPRIMS; i++) {
						...
						if (j==nbcnt[i+1] && norm[0][i].len2()>sqr(params.surfMinNormLen)) { // find local maxima of normal lengths - they are centers of box/cylider candidates
							i32 is; Vec3 n = norm[0][i].normalized();
		...
		auto CheckOppo = [&](i32 start,i32 len, i32 minlen, float maxdiff, const Vec3& c,const Vec3& axis)
		...
		{
			geom_contact *pcont;
			for(i32 icont=Intersect(&gray,0,0,0,pcont)-1; icont>=0;icont--) if (pcont[icont].id[0]>=0 && pcont[icont].n*gray.m_dirn>0)
				return pcont+icont;
			return 0;
		};

		for(i32 isurf=0; isurf<nsurf; isurf++) { // surface (box or cylinder) candidates
			...
			for(i=j=0,r0/=nhits; i<NRAYS; i++) {
				...
				i32 inrng = isneg(fabs(ri-r0)-r0*0.2f);
				r += ri*inrng; j += inrng;
			}
			cylinder cyl;
		...
		for(i32 iline=0; iline<nline && nGeoms<params.maxGeoms; iline++) if (!(params.skipPrimMask & 1ull<<iline+nsurf)) { // line (capsule or cylinder) condidates
		...
		if (params.findMeshes) vox_iterate1(vox,ic) if ((vox(ic)&3)==1 && is_surface(vox,ic)) {
			...
			for(i=0;i<ihead;i++) { // for each surface voxel, refine vertex/normal from the original mesh
				...
				i32 nCellTris=0,offsEnd=params.surfRefineWithMesh ? 14:0;
				for(i32 offs=0,icell[2]; offs<offsEnd; offs+=2) { // collect triangles	in the vicinity
					...
					i32 ntris = intersect_lists(pHashData[0]+pHashGrid[0][icell[0]],pHashGrid[0][icell[0]+1]-pHashGrid[0][icell[0]],
				...
				struct closest_point {
					closest_point(quotientf d2,const Vec3& pt,i32 i0,i32 i1=-1) : dist2(d2),pt(pt),ivtx0(i0),ivtx1(i1+(i0+1&i1>>31)) {}
					quotientf dist2; Vec3 pt; i32 ivtx0,ivtx1;
				} ptbest = closest_point(quotientf(sqr(vox.celldim),1),Vec3(0),-2);
				for(i32 itri=0,ivtx; itri<nCellTris; itri++) { // find the closest point on the mesh
				...
				if (ptbest.ivtx0!=-2) {
					if (ptbest.ivtx0>=0)	{ // if closest pt is edge or vertex, average normal from all involved tris
						for(i32 itri=(norm[0][i].zero(),0);itri<nCellTris;itri++) {
							for(i32 idx=j=0;idx<3;idx++) {
								i32 ivtx = pTris[pCellTris[itri]*3+idx];
			...
			for(i32 iter=params.surfMeshIters; iter>0; iter--,swap(norm[1],norm[0]))
				for(i=0;i<ihead;i++) { // average normals from neighbors
			...
			if (params.storeVox) {
				...
				memcpy(vox.surf.nbcnt=new i32[ihead+1], nbcnt, (ihead+1)*sizeof(i32));
				memcpy(vox.surf.nbidx=new i32[nbcnt[ihead]], nbidx, nbcnt[ihead]*sizeof(i32));
				vox.bitVis = 6;
			}

			for(i=0;i<ihead;i++) // mark voxels with local minima normals as vertices
				for(float minmax=1-(i32)params.surfMaxAndMinNorms*2; minmax<=1; minmax+=2) {
			...
			i32 *voridx=(i32*)norm[1], *vqueue=voridx+ihead, *vtxidx=vqueue+ihead, *edges=vtxidx+ihead, vhead,vtail,nvtx,nedges,nTris=0;
			Vec3_tpl<unsigned short> *pOutTris = (Vec3_tpl<unsigned short>*)(edges+ihead);
			memset(voridx,-1,ihead*sizeof(i32));
			...
			for(i=0,memset(nbidx,-1,nbcnt[nvtx]*sizeof(nbidx[0]));i<nedges;i++)	{ // register only unique vtxi-vtxj edges
				i32 v0=edges[i]>>16, v1=edges[i]&0xffff;
			...
			for(i=0;i<nvtx;i++) for(j=nbcnt[i];j<nbcnt[i+1]-1 && nbidx[j]>=0;j++)
				for(i32 j1=j+1;j1<nbcnt[i+1] && nbidx[j1]>=0;j1++) { // create a triangle if any 2 connections of vtxi are connected to each other
					i32 v0=min(nbidx[j],nbidx[j1]), v1=nbidx[j]+nbidx[j1]-v0;
					for(i32 j2=nbcnt[v0]; j2<nbcnt[v0+1] && nbidx[j2]>=0; j2++) if (nbidx[j2]==v1) {
						i32 flip = -isneg((vtx[v0]-vtx[i]^vtx[v1]-vtx[i])*(norm[0][vtxidx[i]]+norm[0][vtxidx[v0]]+norm[0][vtxidx[v1]]));
			...
			if (nvtx && nTris) {
				...
				pGeoms[nGeoms++] = pMesh->CreateTriMesh(vtx,&pOutTris[0].x,(tuk)pOutTris,0,nTris,(nTris<11 ? mesh_SingleBB : mesh_AABB|mesh_AABB_rotated|mesh_OBB)|mesh_multicontact1);
				if (nGeoms>=params.maxGeoms)
					break;
			}
		}
...
float	CTriMesh::ComputeVesselVolume(const plane *pplane, IGeometry **pFloaters,QuatTS *qtsFloaters,i32 nFloaters, i32 ihash, index_t *pIndices,Vec3 *pNormals, float dim, float &Vsum,Vec3 &comSum)
{
	Vec3 massCenter;
	geom_world_data gwd;
	prim_inters inters;
	...
	for(i32 i=0; i<nFloaters; i++) {
		...
		for(ic.x=max(0,min(g.size.x,ibbox[0].x)); ic.x<=max(-1,min(g.size.x-1,ibbox[1].x)); ic.x++)
			for(ic.y=max(0,min(g.size.y,ibbox[0].y)); ic.y<=max(-1,min(g.size.y-1,ibbox[1].y)); ic.y++)
				for(i32 j=m_pHashGrid[ihash][icell=g.stride*ic]; j<m_pHashGrid[ihash][icell+1]; j++)
		...
		if (!bOutside) {
			...
			for(i32 j=m_pHashGrid[ihash][icell=g.stride*ic]; j<m_pHashGrid[ihash][icell+1]; j++)
				if (!m_pTri2Island[itri=m_pHashData[ihash][j]].bFree) {
					...
					for(i32 i1=0;i1<3;i1++) tri.pt[i1]=m_pVertices[pIndices[itri*3+i1]];
...
i32 CTriMesh::FloodFill(const Vec3& org, float &Vtrg, const Vec3& gdir, float e, Vec2* &ptborder,QuatT &qtBorder,float &depth,i32 bConvexBorder,
...
#define GetVesselVolume ComputeVesselVolume(&waterPlane,pFloaters,qtsFloaters,nFloaters,ihash,pIndices,pNormals,dim,Vsum,comSum)
	...
	i32 i,ihash,j,nTris=m_nTris,ibrd0,*pVtxVal,*pBrdNext,*pBrdPrev,*pVtx2Tri,*pTriMap;
	...
	memset(pVtxVal=new i32[m_nVertices+1],0,(m_nVertices+1)*sizeof(i32));
	memset(pBrdNext=new i32[m_nVertices],-1,m_nVertices*sizeof(i32));
	pBrdPrev=new i32[m_nVertices]; pVtx2Tri=new i32[m_nTris*3];
	memset(pTriMap=new i32[m_nTris+1],-1,(m_nTris+1)*sizeof(i32)); ++pTriMap;
	...
	i32 ivtxBott,ivtxBott0,nTris0,nNewTris;
	float V[2],Vsum;
	Vec3 comSum;
	ivtxBott = pIndices[pcont[ncont-1].iPrim[0]*3];
	do {
		for(i=pVtxVal[ivtxBott0=ivtxBott]; i<pVtxVal[ivtxBott0+1]; i++) {
			j=pVtx2Tri[i]; i32 ivtxBott1 = pIndices[j*3+idxmax3(Vec3(m_pVertices[pIndices[j*3]]*gdir,m_pVertices[pIndices[j*3+1]]*gdir,m_pVertices[pIndices[j*3+2]]*gdir))];
