...
CParticleEntity::~CParticleEntity()
{
	if (m_nParts>0 && m_pWorld)
		m_pWorld->GetGeomUpr()->UnregisterGeometry(m_parts[0].pPhysGeom);
...
i32 CParticleEntity::SetParams(pe_params *_params, i32 bThreadSafe)
{
...
i32 CParticleEntity::GetParams(pe_params *_params) const
{
...
i32 CParticleEntity::GetStateSnapshot(CStream &stm, float time_back, i32 flags)
{
	...
	/*if (m_qspin.v.len2()<0.01*0.01) stm.Write(false);
		...
		for(i32 i=0;i<3;i++) stm.Write((unsigned short)float2int((angles[i]+pi)*(65535.0/2/pi)));
	}
...
i32 CParticleEntity::SetStateFromSnapshot(CStream &stm, i32 flags)
{
	bool bnz;	i32 ver=0;
	...
	/*stm.Read(bnz); if (bnz) {
		unsigned short tmp; i32 i; Vec3 axis(zero);
...
i32 CParticleEntity::GetStateSnapshot(TSerialize ser, float time_back, i32 flags)
{
...
i32 CParticleEntity::SetStateFromSnapshot(TSerialize ser, i32 flags)
{
	if (ser.GetSerializationTarget()==eST_Network)
	{
		if (ser.BeginOptionalGroup("ifawake", IsAwake()!=0))
...
i32 CParticleEntity::GetStatus(pe_status* _status) const
{
...
i32 CParticleEntity::Action(pe_action* _action, i32 bThreadSafe)
{
...
i32 CParticleEntity::Awake(i32 bAwake,i32 iSource) 
{ 
	if (bAwake)
		m_bForceAwake = 1; 
	else {
...
i32 CParticleEntity::IsAwake(i32 ipart) const
{
	ReadLock lock(m_lockParticle);
	if (!m_pWorld)
		return false;
...
/*i32 CheckPointInside(const Vec3& pt)
{
	i32 i,j,nents;
...
i32 CParticleEntity::DoStep(float time_interval, i32 iCaller)
{
  ...
  ray_hit (&hits)[8] = _hits[get_iCaller()];
	...
	i32 i,j,nhits,bHit,flags,bHasIgnore=(m_pColliderToIgnore!=0);
  ...
  if (m_timeSurplus==0)
	...
	if (IsAwake()) {
		...
		m_bRecentCollisions = max(0,(i32)m_bRecentCollisions-1);
		...
		if (iCaller<MAX_PHYS_THREADS && m_pWorld->m_bWorldStep==2 && m_iPierceability<=sf_max_pierceable) {
			...
			sp.pGridRefEnt = this;
			Vec3 posFixed;
			i32 nents = m_pWorld->GetEntitiesAround(pos0-Vec3(m_dim,m_dim,m_dim),pos0+Vec3(m_dim,m_dim,m_dim),pentlist,ent_rigid,0,0,iCaller);
		...
		if (!bHit && m_iPierceability<=sf_max_pierceable) {
			...
			nhits = m_pWorld->RayWorldIntersection(
				rp.Init(pos0,pos-pos0+heading0*m_dim, m_collTypes|ent_water,
					...
					pIgnoredColliders+1-bHasIgnore,1+bHasIgnore, 0,0,0, portals,DRX_ARRAY_COUNT(portals)-1),
				"RayWorldIntersection(PhysParticles)", iCaller);
		...
		for(i=0; i<nhits; i++) {	// register all hits in history 
			...
			if (hits[j].pCollider && ((CPhysicalPlaceholder*)hits[j].pCollider)->m_iSimClass!=5) {
				...
				i32 ipart = max(0,min(hits[j].ipart, pCollider->m_nParts-1));
				...
				{	Vec3 vrel = vel0-event.vloc[1];
					...
					if (j>0)
						kmul = 0.5f*(1-(1.0f/15)*(i32)((m_pWorld->m_SurfaceFlagsTable[min(NSURFACETYPES-1,max(0,(i32)hits[j].surface_idx))] & sf_pierceable_mask)-m_iPierceability));
...
goto doretest; }*/
		...
		if (iCaller<MAX_PHYS_THREADS) {
			...
			for(i=0; portals[i]; i++)	{
				...
				i32 isExit = 1-isneg(-(((CPhysicalPlaceholder*)portals[i])->m_iForeignFlags & TRIGGER_PORTAL_INV));
				i32 inside = insideWhat->GetStatus(&scp);
				if (inside ^ isExit)
		...
		if ((m_nStepCount|(i32)(char)m_areaCheckPeriod>>31)==0) {
		...
		m_nStepCount -= 1+((i32)(char)m_nStepCount>>31);
...
i32 CParticleEntity::RayTrace(SRayTraceRes& rtr)
{
...
void CParticleEntity::DrawHelperInformation(IPhysRenderer *pRenderer, i32 flags)
{
...
void CParticleEntity::GetMemoryStatistics(IDrxSizer *pSizer) const
{
