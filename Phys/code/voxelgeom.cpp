...
#include <drx3D/Phys/voxelgeom.h>


CVoxelGeom* CVoxelGeom::CreateVoxelGrid(grid3d *pgrid)
{
	i32 i,j,ipass,dummy,icell,ncells;
	Vec3i ic,iBBox[2];
	Vec3 c;
	float x;
	...
	memset(m_grid.pCellTris = new i32[ncells+1], 0, (ncells+1)*sizeof(i32));
	...
	for(ipass=0,m_grid.pTriBuf=&dummy; ipass<2; ipass++) {
		...
		if (ipass==0) {
			...
			m_grid.pTriBuf = new i32[m_grid.pCellTris[ncells]];
		}
	}
...
i32 CVoxelGeom::Intersect(IGeometry *pCollider, geom_world_data *pdata1,geom_world_data *pdata2, intersection_params *pparams, geom_contact *&pcontacts)
{
	if (pCollider->GetType()==GEOM_RAY) {
		geometry_under_test GRay;
		bool bKeepPrevContacts;
		i32 bThreadSafe,bActive,iCaller;
    ...
    ((CGeometry*)pCollider)->PrepareForIntersectionTest(&GRay, this, (geometry_under_test*)NULL, bKeepPrevContacts);
		...
		i32 i,j,idxcell,itri,bHasInters=0;
		float rscale,len;
...
i32 CVoxelGeom::DrawToOcclusionCubemap(const geom_world_data *pgwd, i32 iStartPrim,i32 nPrims, i32 iPass, SOcclusionCubeMap* cubeMap)
{
	i32 nTris=0,i,j,icell;
...
void CVoxelGeom::GetMemoryStatistics(IDrxSizer *pSizer)
{
