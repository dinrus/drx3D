...
#include <drx3D/Phys/heightfieldbv.h>
...
typedef i32 (Serializer::*ParseProc)(parse_context&, tuk);

unsigned short *g_hfData;
u8 *g_hfFlags;
i32 g_hfMats[256],g_nHfMats;
Vec2i g_hfStride;
float g_getHeightCallback(i32 ix,i32 iy) { return g_hfData[g_hfStride*Vec2i(ix,iy)]*(1.0f/256)-128.0f; }
u8 g_getSurfTypeCallback(i32 ix,i32 iy) { return g_hfFlags[g_hfStride*Vec2i(ix,iy)]; };
...
struct parse_context {
	...
	uk pobj,*pObjStack[16];
	i32 iStackPos;
	i32 bSaving;
	CTetrLattice **pLattices;
	i32 nLattices,nLatticesAlloc;
	CTriMesh **pGeoms;
	i32 nGeoms,nGeomsAlloc;
	...
	~parse_context() {
		...
		for(i=0;i<nGeoms;i++) pGeoms[i]->m_nSubtracts = 0;
		if (pGeoms) delete[] pGeoms;
	}

	void PushState() { 
		if (iStackPos<DRX_ARRAY_COUNT(pSerializerStack)) {
...
#define DECLARE_MEMBER(strname,type,member) { \
	...
	pitem->data.ptr = (uk )((tuk)&trg.member-(tuk)&trg);\
...
i32 proc(parse_context &ctx,char *str) { \
	if (ctx.bSaving) \
		sprintf(str,"%d",((type*)ctx.pobj)->member); \
	else { \
		i32 i; sscanf(str,"%d",&i);	\
	...
	virtual void Serialize(parse_context &ctx) {
		...
		if (ctx.bSaving) {
			...
			for(u32 i=0; i<list.size(); i++) {
				...
				uk pdata = (tuk)ctx.pobj+(INT_PTR)list[i]->data.ptr;
				switch (list[i]->itype) {
					case ft_int: sprintf(ptr0, g_strFormats[0][list[i]->itype], *(i32*)pdata); break;
					case ft_uint: sprintf(ptr0, g_strFormats[0][list[i]->itype], *(u32*)pdata); break;
					...
					case ft_entityptr: ppc = *(CPhysicalPlaceholder**)((tuk)ctx.pobj+(INT_PTR)list[i]->data.ptr);
			...
			do {
				...
				if (itor!=ctx.pSerializer->map.end()) {
					pitem = itor->second;
					uk pdata = (tuk)ctx.pobj+(INT_PTR)pitem->data.ptr;
					switch (pitem->itype) {
						...
						case ft_entityptr: *(IPhysicalEntity**)pdata = *ptr1!='N' ? ctx.pWorld->GetPhysicalEntityById(atol(ptr1)) : 0; break;
...
struct CMeshSerializer : Serializer {
	...
	i32 SerializeVtx(parse_context &ctx, char *str) {
	...
	i32 SerializeTri(parse_context &ctx, char *str) {
		CTriMesh *pMesh = (CTriMesh*)ctx.pobj;
		i32 i;
		if (ctx.bSaving) {
	...
	i32 SerializeMinTrisPerNode(parse_context &ctx, char *str) {
		CTriMesh *pMesh = (CTriMesh*)ctx.pobj;
		if (ctx.bSaving) {
			i32 n = 0;
	...
	i32 SerializeMaxTrisPerNode(parse_context &ctx, char *str) {
		CTriMesh *pMesh = (CTriMesh*)ctx.pobj;
		if (ctx.bSaving) {
			i32 n = 0;
			if (pMesh->m_flags & mesh_AABB)
	...
	i32 Finalize(parse_context &ctx, char *str) {
		if (!ctx.bSaving) {
			CTriMesh *pMesh = (CTriMesh*)ctx.pobj;
...
inline char hex2chr(i32 h) { return h+'0'+(9-h>>31 & 'A'-'0'-10); }
inline i32 chr2hex(char c) { return c-'0'-(9+'0'-c>>31 & 'A'-'0'-10); }
...
struct CHeightfieldSerializer : Serializer {
	...
	i32 SerializeMatMapping(parse_context &ctx,char *str) {
		i32 i,nMats;
		if (ctx.bSaving) {
	...
	i32 SerializeData(parse_context &ctx, char *str) {
		...
		if (ctx.bSaving) {
			...
			g_hfFlags = new u8[(phf->m_hf.size.x+1)*(phf->m_hf.size.y+1)];
			...
			for(j=0; j<=phf->m_hf.size.y; j++) {
				...
				for(i=0,ptr=buf; i<=phf->m_hf.size.x; i++) {
					...
					if (!strncmp(ptr,"end",3)) {
						fseek(ctx.f,-(i32)strlen(buf)-1,SEEK_CUR); goto hfend;
					}
...
struct CTetrahedronSerializer : Serializer {
	...
	i32 SerializeIinv(parse_context &ctx, char *str) {
	...
	i32 SerializeBuddies(parse_context &ctx, char *str) {
		STetrahedron *ptet = (STetrahedron*)ctx.pobj;
		if (ctx.bSaving)
	...
	i32 SerializeFaces(parse_context &ctx, char *str) {
		STetrahedron *ptet = (STetrahedron*)ctx.pobj;
		if (ctx.bSaving)
...
struct CTetrLatticeSerializer : Serializer {
	...
	i32 SerializeVtx(parse_context &ctx, char *str) {
		...
		if (ctx.bSaving) {
			...
			if (!pLattice->m_pVtxFlags) pLattice->m_pVtxFlags = new i32[pLattice->m_nVtx];
	...
	i32 SerializeTets(parse_context &ctx, char *str) {
		CTetrLattice *pLattice = (CTetrLattice*)ctx.pobj;
		i32 i;
...
struct CPhysGeometrySerializer : Serializer {
	...
	i32 SerializeGeometryPtr(parse_context &ctx, tuk str, IGeometry *&pGeom) {
		...
		if (ctx.bSaving) {
			...
			for(; *str && !isdigit(*str); str++);
			switch (atol(str)) {
				case GEOM_TRIMESH: pGeom=new CTriMesh; pSerializer=pMeshSerializer; 
					((CTriMesh*)pGeom)->m_bConvex[0]=2; ((CTriMesh*)pGeom)->m_bConvex[1]=4; break;
				case GEOM_HEIGHTFIELD: pGeom=new CHeightfield; pSerializer=pHeightfieldSerializer; break;
				case GEOM_BOX: pGeom=new CBoxGeom; pSerializer=pBoxSerializer; break;
				case GEOM_CYLINDER: pGeom=new CCylinderGeom; pSerializer=pCylinderSerializer; pCylinderSerializer->bCapsule=0; break;
				case GEOM_CAPSULE: pGeom=new CCapsuleGeom; pSerializer=pCylinderSerializer; pCylinderSerializer->bCapsule=1; break;
				case GEOM_SPHERE: pGeom=new CSphereGeom; pSerializer=pSphereSerializer; break;
				default: return 0;
			}
			ctx.PushState();
			ctx.pSerializer = pSerializer;
	...
	i32 SerializeGeometry(parse_context &ctx, tuk str) { return SerializeGeometryPtr(ctx,str,((phys_geometry*)ctx.pobj)->pGeom); }
};
...
struct CExplosionShapeSerializer : Serializer {
	...
	i32 SerializeGeometry(parse_context &ctx,char *str) {
...
struct CCrackSerializer : Serializer {
	...
	i32 SerializeRc(parse_context &ctx, char *str) {
...
struct CGeomanSerializer : Serializer {
	...
	CGeomanSerializer(CPhysGeometrySerializer *pgs,CExplosionShapeSerializer *ess,CCrackSerializer *cs,i32 flagged=0) : 
	...
	i32 SerializeExplosionShapes(parse_context &ctx, char *str) {
	...
	i32 SerializeCracks(parse_context &ctx, char *str) {
		CPhysicalWorld *pWorld = (CPhysicalWorld*)ctx.pobj;
		i32 i;
...
struct CPhysicalPlaceholderSerializer : Serializer {
	...
	i32 SerializeId(parse_context &ctx,char *str) {
		if (ctx.bSaving)
			sprintf(str,"%d",((CPhysicalPlaceholder*)ctx.pobj)->m_id);
		else {
			i32 id; sscanf(str,"%d",&id);
	...
	i32 Finalize(parse_context &ctx, char *str) {
		if (!ctx.bSaving) {
			...
			ctx.pWorld->RepositionEntity(ppc,3|8);
		}
		return 0;
	}
};
...
struct CPhysicalEntityPartSerializer : Serializer {
	...
	i32 SerializeMatMapping(parse_context &ctx,char *str) {
		...
		if (ctx.bSaving) {
			...
			sscanf(str,"%d",&nMats); 
			if (nMats) {
				pgeom->pMatMapping = new i32[pgeom->nMats = nMats];
				for(i=0;i<nMats;i++) {
					for(;*str && !isdigit(*str);str++);
					for(;*str && !isspace(*str);str++);
	...
	i32 SerializeGeom(parse_context &ctx,char *str) {
		i32 res = SerializeGeomPtr(ctx,str,((geom*)ctx.pobj)->pPhysGeom);
		if (!ctx.bSaving) {
	...
	i32 SerializeGeomProxy(parse_context &ctx,char *str) {
	...
	i32 SerializeGeomPtr(parse_context &ctx,char *str,phys_geometry *&pPhysGeom) {
		i32 i,j;
		if (ctx.bSaving) {
			for(i=0; i<ctx.pWorld->m_nGeomChunks && (u32)(j=pPhysGeom-ctx.pWorld->m_pGeoms[i])>=GEOM_CHUNK_SZ; i++);
	...
	i32 SerializeLattice(parse_context &ctx,char *str) {
...
struct CPhysicalEntitySerializer : CPhysicalPlaceholderSerializer {
	...
	CPhysicalEntitySerializer(CPhysicalEntityPartSerializer *peps, CPhysicalJointSerializerRead *pjsr, CPhysicalJointSerializerWrite *pjsw) : 
		pPartSerializer(peps),pJointSerializerRead(pjsr),pJointSerializerWrite(pjsw) 
	{
		CPhysicalEntity trg(0);
		DECLARE_MEMBER("flags", ft_uint, m_flags)
		DECLARE_MEMBER("pos", ft_vector, m_pos)
		DECLARE_PROC("angles", &CPhysicalEntitySerializer::SerializeAngles)
	...
	i32 SerializeAngles(parse_context &ctx, char *str) {
		CPhysicalEntity *pent = (CPhysicalEntity*)ctx.pobj;
		if (!ctx.bSaving) {
			Ang3 ang;
			sscanf(str, "(%f %f %f)", &ang.x,&ang.y,&ang.z);
			pent->m_qrot = Quat::CreateRotationXYZ(DEG2RAD(ang));
		}
		return ctx.bSaving;
	}
	i32 SerializePart(parse_context &ctx, char *str) {
		CPhysicalEntity *pent = (CPhysicalEntity*)ctx.pobj;
		i32 i,j;
		if (ctx.bSaving) for(i=0;i<pent->m_nParts;i++) {
			...
			if ((pent->m_nParts=max(pent->m_nParts,i+1)) > pent->m_nPartsAlloc) {
				...
				i32 nparts = pent->m_nPartsAlloc;
				memcpy(pent->m_parts = new geom[pent->m_nPartsAlloc=(pent->m_nParts-1&~3)+4], pparts, sizeof(geom)*j);
				for(i32 k=0; k<pent->m_nParts; k++) if (pent->m_parts[k].pNewCoords==(coord_block_BBox*)&pparts[k].pos)
...
#pragma warning(disable : 4390)
	...
	i32 SerializeJoint(parse_context &ctx, char *str) {
		CPhysicalEntity *pent = (CPhysicalEntity*)ctx.pobj;
		if (ctx.bSaving) for(i32 i=0;i<(pent->m_pStructure ? pent->m_pStructure->nJoints:0);i++) {
			...
			for(i32 i=0;i<2;i++) if (psj.partid[i]>=0) psj.partid[i]=pent->m_parts[psj.partid[i]].id;
			pent->SetParams(&psj);
		}
		return ctx.bSaving;
	}
	i32 SerializeGroundPlane(parse_context &ctx, char *str) {
		CPhysicalEntity *pent = (CPhysicalEntity*)ctx.pobj;
		CPlaneSerializer planeSerializer;
		if (ctx.bSaving) for(i32 i=0;i<pent->m_nGroundPlanes;i++) {
...
struct CRigidEntitySerializer : CPhysicalEntitySerializer {
	...
	CRigidEntitySerializer(CPhysicalEntityPartSerializer *peps, CPhysicalJointSerializerRead *pjsr, CPhysicalJointSerializerWrite *pjsw, CRigidBodySerializer *rbs, CConstraintSerializer *cs) : 
		CPhysicalEntitySerializer(peps,pjsr,pjsw),pRigidBodySerializer(rbs),pConstraintSerializer(cs) 
	{
		CRigidEntity trg(0);
		...
		DECLARE_PROC("angles", &CRigidEntitySerializer::SerializeAngles)
	...
	i32 SerializeBody(parse_context &ctx, char *str) {
	...
	i32 SerializeAngles(parse_context &ctx, char *str) {
		...
		if (!ctx.bSaving) {
			...
			sscanf(str, "(%f %f %f)", &ang.x,&ang.y,&ang.z);
			pent->m_qrot = Quat::CreateRotationXYZ(DEG2RAD(ang));
			pent->m_body.pos = pent->m_pos+pent->m_qrot*pent->m_body.offsfb;
			pent->m_body.q = pent->m_qrot*!pent->m_body.qfb;
			pent->m_body.Iinv = Matrix33(pent->m_body.q)*pent->m_body.Ibody_inv*Matrix33(!pent->m_body.q);
		}
		return ctx.bSaving;
	}
	i32 SerializeConstraints(parse_context &ctx, char *str) {
		CRigidEntity *pent = (CRigidEntity*)ctx.pobj;
		masktype mask=0;
		for(i32 i=0;i<pent->m_nColliders;mask|=pent->m_pColliderConstraints[i++]);
		if (ctx.bSaving) for(i32 i=0;i<NMASKBITS && getmask(i)<=mask;) {
...
struct CWheeledVehicleEntitySerializer : CRigidEntitySerializer {
	...
	i32 SerializeGears(parse_context &ctx, char *str) {
	...
	i32 SerializeWheel(parse_context &ctx, char *str) {
		CWheeledVehicleEntity *pent = (CWheeledVehicleEntity*)ctx.pobj;
		i32 i;
		if (ctx.bSaving) for(i=0;i<pent->m_nParts-pent->m_nHullParts;i++) {
			...
			if (i>=pent->m_suspAlloc) {
				...
				for(i32 j=pent->m_nHullParts; j<pent->m_nParts; j++) 
...
struct CArticulatedEntitySerializer : CRigidEntitySerializer {
	...
	CArticulatedEntitySerializer(CPhysicalEntityPartSerializer *peps, CPhysicalJointSerializerRead *pjsr, CPhysicalJointSerializerWrite *pjsw, CRigidBodySerializer *rbs, 
		CConstraintSerializer *cs, CJointSerializer *js, CAEPartInfoSerializer *aepis) : CRigidEntitySerializer(peps,pjsr,pjsw,rbs,cs), pJointSerializer(js), pPartInfoSerializer(aepis) 
	{
	...
	i32 SerializePartInfo(parse_context &ctx, char *str) {
	...
	i32 SerializeHost(parse_context &ctx, char *str) {
		CArticulatedEntity *pent = (CArticulatedEntity*)ctx.pobj;
		if (ctx.bSaving)
...
struct CLivingEntitySerializer : CPhysicalEntitySerializer {
	CLivingEntitySerializer(CPhysicalEntityPartSerializer *peps) : CPhysicalEntitySerializer(peps,0,0) {
		CLivingEntity trg(0);
		DECLARE_MEMBER("Gravity", ft_vector, m_gravity)
...
struct CWalkingRigidEntitySerializer : CRigidEntitySerializer {
	...
	i32 SerializeLegs(parse_context &ctx, char *str) {
		CWalkingRigidEntity *pent = (CWalkingRigidEntity*)ctx.pobj;
		if (!ctx.bSaving)
			pent->m_legs = new Vec3[pent->m_nLegs*3];
		for(i32 i=0;i<pent->m_nLegs*3;i++) for(i32 j=0;j<3;j++) if (ctx.bSaving) 
...
struct CRopeEntitySerializer : CPhysicalEntitySerializer {
	...
	CRopeEntitySerializer(CPhysicalEntityPartSerializer *peps, CRopeVtxSerializer *prvs, CRopeSegSerializer *prss) : 
		CPhysicalEntitySerializer(peps,0,0), pVtxSerializer(prvs), pSegSerializer(prss)
	{
		CRopeEntity trg(0);
	...
	i32 SerializeSegment(parse_context &ctx, char *str) {
	...
	i32 SerializeVertex(parse_context &ctx, char *str) {
		CRopeEntity *pent = (CRopeEntity*)ctx.pobj;
		i32 i;
		if (ctx.bSaving) for(i=0;i<pent->m_nVtx;i++) {
			...
			if (!pent->m_vtx) {
				...
				pent->m_idx = new i32[pent->m_nMaxSubVtx+2];
			}
			ctx.PushState();
			ctx.pobj = pent->m_vtx+i;
			(ctx.pSerializer = pVtxSerializer)->Serialize(ctx);
	...
	i32 Finalize(parse_context &ctx, char *str) {
		...
		if (!ctx.bSaving) {
			pent->m_pNewCoords->pos=pent->m_pos; pent->m_pNewCoords->q=pent->m_qrot;
			pent->m_ig[0].x = pent->m_flags & pef_traceable ? GRID_REG_PENDING : NO_GRID_REG;
			ctx.pWorld->RepositionEntity(pent,3|8);

			pe_params_rope pr;
			for(i32 i=0;i<2;i++) if (pr.pEntTiedTo[i]=pent->m_pTiedTo[i]) {
...
struct CSoftEntitySerializer : CPhysicalEntitySerializer {
	CSoftVtxSerializer *pVtxSerializer;
	CSoftEdgeSerializer *pEdgeSerializer;

	CSoftEntitySerializer(CPhysicalEntityPartSerializer *peps, CSoftVtxSerializer *psvs, CSoftEdgeSerializer *pses) : CPhysicalEntitySerializer(peps,0,0), pVtxSerializer(psvs), pEdgeSerializer(pses)
	{
		CSoftEntity trg(0);
	...
	i32 SerializePart(parse_context &ctx, char *str) {
		CPhysicalEntitySerializer::SerializePart(ctx,str);
		CSoftEntity *pent = (CSoftEntity*)ctx.pobj;
		if (!ctx.bSaving && pent->m_nParts==1) {
			i32 id = pent->m_parts[0].id;
			...
			++pgeom->nRefCount; pent->RemoveGeometry(id);
			pent->AddGeometry(pgeom,&gp,id); pgeom->nRefCount--;
		}
		return ctx.bSaving;
	}
	...
	i32 SerializeVertex(parse_context &ctx, char *str) {
		CSoftEntity *pent = (CSoftEntity*)ctx.pobj;
		if (ctx.bSaving) for(i32 i=0;i<pent->m_nVtx;i++) {
			fprintf(ctx.f,"%.*sVertex %d\n", ctx.iStackPos+1,g_strTabs, i);
			ctx.PushState();
			ctx.pobj = pent->m_vtx+i;
			(ctx.pSerializer = pVtxSerializer)->Serialize(ctx);
			...
			ctx.PushState();
			ctx.pobj = pent->m_vtx+min((i32)atol(str),pent->m_nVtx-1);
			(ctx.pSerializer = pVtxSerializer)->Serialize(ctx);
		}
		return ctx.bSaving;
	}
	i32 SerializeEdge(parse_context &ctx, char *str) {
		CSoftEntity *pent = (CSoftEntity*)ctx.pobj;
		if (ctx.bSaving) for(i32 i=0;i<pent->m_nEdges;i++) {
			...
			ctx.PushState();
			ctx.pobj = pent->m_edges+min((i32)atol(str),pent->m_nEdges-1);
			(ctx.pSerializer = pEdgeSerializer)->Serialize(ctx);
		}
		return ctx.bSaving;
	}
...
i32 IsAreaNonWater(CPhysArea *pArea);
i32 IsAreaGravity(CPhysArea *pArea);

struct CAreaSerializer : CPhysicalPlaceholderSerializer {
	CAreaSerializer(CPhysGeometrySerializer *pgs) : pGeomSerializer(pgs) {
		CPhysArea trg(0);
	...
	i32 SerializeGeometry(parse_context &ctx, char *str) { 
		return !ctx.bSaving || !((CPhysArea*)ctx.pobj)->m_pGeom || ((CPhysArea*)ctx.pobj)->m_pGeom->GetType()!=GEOM_HEIGHTFIELD ? pGeomSerializer->SerializeGeometryPtr(ctx,str,((CPhysArea*)ctx.pobj)->m_pGeom) : 1; 
	}
	i32 SerializePoints(parse_context &ctx, char *str) {
		CPhysArea *pArea = (CPhysArea*)ctx.pobj;
		if (ctx.bSaving) {
			for(i32 i=0;i<(pArea->m_pt ? pArea->m_npt:0);i++)
	...
	i32 SerializePt3d(parse_context &ctx, char *str, tukk tag, i32 npt, Vec3 *&pt3d) {
		if (ctx.bSaving) {
			for(i32 i=0;i<npt;i++)
	...
	i32 SerializeSpline(parse_context &ctx, char *str) { 
		CPhysArea *pArea = (CPhysArea*)ctx.pobj;
		return SerializePt3d(ctx,str,"pt3d",pArea->m_ptSpline || !ctx.bSaving ? pArea->m_npt:0,pArea->m_ptSpline);
	}
	i32 SerializeFlows(parse_context &ctx, char *str) { 
		CPhysArea *pArea = (CPhysArea*)ctx.pobj;
		return SerializePt3d(ctx,str,"flow",pArea->m_pFlows || !ctx.bSaving ? pArea->m_npt:0,pArea->m_pFlows);
	}
	i32 Finalize(parse_context &ctx, char *str) {
		CPhysArea *pArea = (CPhysArea*)ctx.pobj;
		if (!ctx.bSaving) {
...
struct CEntityGridSerializer : Serializer {
	...
	i32 SerializeAxisZ(parse_context &ctx, char *str) {
		if (ctx.bSaving)
			sprintf(str,"2");
		return 0;
	}
	...
	i32 Finalize(parse_context &ctx, char *str) {
		CPhysicalWorld *pWorld = (CPhysicalWorld*)ctx.pobj;
		if (!ctx.bSaving)
			pWorld->SetupEntityGrid(pWorld->m_entgrid.iup, pWorld->m_entgrid.origin, pWorld->m_entgrid.size.x, 
...
struct CPhysicalWorldSerializer : Serializer {
	...
	CPhysicalWorldSerializer(CPhysVarsSerializer *pvs, CEntityGridSerializer *egs, CPhysicalEntitySerializer *pes, CRigidEntitySerializer *res,	CWalkingRigidEntitySerializer *wres,
		CWheeledVehicleEntitySerializer *wves, CArticulatedEntitySerializer *aes, CLivingEntitySerializer *les, CRopeEntitySerializer *rpes, CSoftEntitySerializer *spes, CAreaSerializer *as, i32 flagged=0) : 
	...
	i32 SerializeVars(parse_context &ctx, char *str) {
		CPhysicalWorld *pWorld = (CPhysicalWorld*)ctx.pobj;
		if (ctx.bSaving)
	...
	i32 SerializeEntityGrid(parse_context &ctx, char *str) {
		CPhysicalWorld *pWorld = (CPhysicalWorld*)ctx.pobj;
		if (ctx.bSaving)
	...
	i32 SerializeMaterial(parse_context &ctx, char *str) {
		CPhysicalWorld *pWorld = (CPhysicalWorld*)ctx.pobj;
		i32 i;
		if (ctx.bSaving) {
	...
	template<class PhysEnt> PhysEnt* CreateEnt(CPhysicalWorld *pWorld, tukk name) {
		PhysEnt *ent = (PhysEnt*)DrxModuleMalloc(sizeof(PhysEnt)+strlen(name)+1);
		new(ent) PhysEnt(pWorld, nullptr);
		pWorld->SetGrid(ent, &pWorld->m_entgrid);
		pe_params_foreign_data pfd;
		strcpy((tuk)(pfd.pForeignData = (tuk)ent+sizeof(PhysEnt)), name);
		pfd.iForeignData = 100;
		ent->SetParams(&pfd);
		return ent;
	}

	i32 SerializeEntities(parse_context &ctx, char *str) {
		CPhysicalWorld *pWorld = (CPhysicalWorld*)ctx.pobj;
		...
		if (ctx.bSaving) {
			...
			switch (i) {
				...
				case PE_SOFT: pent = CreateEnt<CSoftEntity>(pWorld,name); ctx.pSerializer = pSoftEntitySerializer; break;
			}
			ctx.pobj = pent;
			ctx.pSerializer->Serialize(ctx);
		}
	...
	i32 SerializeAreas(parse_context &ctx, char *str) {
		CPhysicalWorld *pWorld = (CPhysicalWorld*)ctx.pobj;
		if (ctx.bSaving) for(CPhysArea *pArea=pWorld->m_pGlobalArea;pArea;pArea=pArea->m_next) {
...
bool SerializeWorldBin(CPhysicalWorld *pWorld, tukk fname,i32 bSave);
...
struct CDummySizer : public IDrxSizer {
...
inline bool nonptr(uk &p) { return nonptr(((uint*)&p)[0]) && nonptr(((uint*)&p)[1]) || nonptr(((float*)&p)[0]) && nonptr(((float*)&p)[1]); }

struct CSaverSizer : public CDummySizer {
	std::map<uk ,i32> &objs;
	CMemStream &stm;
	CSaverSizer(std::map<uk ,i32> &objs, CMemStream &stm) : objs(objs),stm(stm) {}
	virtual bool IsLoading() { return false; }
	virtual bool AddObject(ukk ptr, size_t size, i32 nCount=1) { return AddObjectRaw((uk )ptr,size,false,size>=nCount*sizeof(uk )); }
	virtual bool AddObjectRaw(uk ptr, size_t size, bool hasVMT=false, bool mapPtrs=true) {
		i32 i=0,sz=size/sizeof(uk );
		stm.Write(size);
		stm.Write(ptr, size);
		if (mapPtrs) for(uk *p=(uk *)ptr; i<sz; i++) if (p[i]) {// && !nonptr(p[i])) {
...
struct CLoaderSizer : public CDummySizer {
	...
	CMemStream &stm;
	CPhysicalWorld *pWorld;
	CLoaderSizer(std::vector<uk> &objs, CMemStream &stm, CPhysicalWorld *pWorld) : objs(objs),stm(stm),pWorld(pWorld) {}
	virtual bool IsLoading() { return true; }
#ifndef STANDALONE_PHYSICS
	virtual bool AddObject(ukk ptr, size_t size, i32 nCount=1)  { return false; }
#endif
	virtual bool AddPartsAlloc(struct geom* &parts, size_t size) { 
		i32 nparts = size/sizeof(geom);
		parts = size ? pWorld->AllocEntityParts(nparts) : &CPhysicalEntity::m_defpart;
		i32 bChuckStart = parts[0].bChunkStart;
	...
	virtual bool AddIntArrayAlloc(i32* &ptr, size_t size) { 
		ptr = *(size_t*)(stm.m_pBuf+stm.m_iPos) ? new i32[size/sizeof(i32)] : nullptr;
		return AddObjectRaw(ptr, size);
	}
	virtual bool AddObjectRaw(uk ptr, size_t sizeDst, bool hasVMT=false, bool mapPtrs=true) {
		size_t size=stm.Read<size_t>(), offs=0;
		if (hasVMT) {
			uk pVMT; stm.Read(pVMT);
			offs = sizeof(pVMT); 
		}
		stm.ReadRaw((tuk)ptr+offs, min(size,sizeDst)-offs);
		stm.m_iPos += max(0,(i32)size-(i32)sizeDst);
		for(i32 i; (i=stm.Read<i32>())>=0; )
			((uk *)ptr)[i] = objs[stm.Read<i32>()];
		return true;
	}
};
...
void PostLoadEntityRigid(CRigidEntity *pent, CLoaderSizer& sizer)	{
	...
	for(i32 i=0;i<pent->m_nConstraintsAlloc;i++) if (pent->m_constraintMask & 1ull<<i) for(i32 j=0;j<2;j++)
...
void PostLoadEntityArtic(CArticulatedEntity *pent, CLoaderSizer& sizer) {
	...
	for(i32 i=0;i<pent->m_nJoints;i++)
...
void PostLoadEntityVehicle(CWheeledVehicleEntity *pent, CLoaderSizer& sizer) {
	PostLoadEntityRigid(pent,sizer);
	for(i32 i=pent->m_nHullParts;i<pent->m_nParts;i++)
...
bool SerializeWorldBin(CPhysicalWorld *pWorld, tukk fname,i32 bSave)
{
	if (FILE *f = fopen(fname,bSave ? "wb":"rb")) {
		...
		i32 version = sizeof(CRigidEntity)+sizeof(CArticulatedEntity)+sizeof(CWheeledVehicleEntity)+sizeof(CRopeEntity)+sizeof(CSoftEntity)+sizeof(CParticleEntity)+sizeof(CLivingEntity)+
		...
		if (bSave) {
			...
			stm.Write(version);
			stm.Write(&w.m_vars, (i32)(INT_PTR)&((PhysicsVars*)0)->helperOffset);
			stm.Write(w.m_BouncinessTable, sizeof(w.m_SurfaceFlagsTable) + (i32)((INT_PTR)&((CPhysicalWorld*)0)->m_SurfaceFlagsTable-(INT_PTR)&((CPhysicalWorld*)0)->m_BouncinessTable));
			...
			if (w.m_pHeightfield[0]) {
				...
				stm.GrowBuf((phf->size.x+1)*(phf->size.y)*sizeof(i32));
				...
				for(i=0;i<=MAX_PHYS_THREADS;i++)
					objs.insert(std::pair<uk ,i32>(w.m_pHeightfield[i],objs.size()));
			}
			...
			for(i=0;i<w.m_nGeomChunks;i++) for(j=0;j<GEOM_CHUNK_SZ;j++) if (w.m_pGeoms[i][j].pGeom)	{
				i32 flags; mesh_data *md = nullptr;
				...
				objs.insert(std::pair<uk ,i32>(w.m_pGeoms[i]+j,objs.size()));
			}
			for(CLivingEntity *pent=(CLivingEntity*)w.m_pTypedEnts[3]; pent; pent=(CLivingEntity*)pent->m_next)	if (pent->m_pCylinderGeom) {
				...
				objs.insert(std::pair<uk ,i32>(pent->m_pCylinderGeom,objs.size()));
			}
			...
			for(CPhysArea *parea=(i=objs.size(),w.m_pGlobalArea); parea; parea=parea->m_next) {
				...
				for(i=0;i<3;i++) if (geoms[i]) {
					...
					objs.insert(std::pair<uk ,i32>(geoms[i],objs.size())); 
					if (!i) geoms[i]->AddRef();
				}
			}
			stm.Write(false);
			for(i=0;i<8;i++) for(CPhysicalEntity *pent=w.m_pTypedEnts[i];pent;pent=pent->m_next) 
				for(j=0;j<pent->m_nParts;j++) if (CTetrLattice *lat = pent->m_parts[j].pLattice) {
					...
					for(i32 itet=0; itet<lat->m_nTetr; itet++)
						stm.Write(lat->m_pTetr[itet].ivtx);
					objs.insert(std::pair<uk ,i32>(lat,objs.size()));
				}
			stm.Write(false);
			stm.Write(w.m_nExpl);
			...
			for(CWaterMan *pwater=(i=0,w.m_pWaterMan); pwater; pwater=pwater->m_next,i++)
				objs.insert(std::pair<uk ,i32>(pwater,objs.size()));
			stm.Write(i);

			for(SEntityGrid *pgrid = &w.m_entgrid; pgrid; pgrid=pgrid->m_next) {
				objs.insert(std::pair<uk ,i32>(pgrid,objs.size()));
				stm.Write(true);
				stm.Write(*static_cast<grid*>(pgrid));
				stm.Write(pgrid->iup); stm.Write(pgrid->log2PODscale); stm.Write(pgrid->bCyclic);
				sizer.AddObjectRaw(&pgrid->m_transInHost, sizeof(SEntityGrid)-(i32)(INT_PTR)&((SEntityGrid*)0)->m_transInHost);
				stm.GrowBuf(pgrid->size.x*pgrid->size.y*sizeof(i32));
			...
			for(i=0;i<8;i++) for(CPhysicalEntity *pent=w.m_pTypedEnts[i]; pent; pent=pent->m_next) {
				objs.insert(std::pair<uk ,i32>(pent,objs.size()));
				stm.Write(pent->GetType());
				stm.Write(pent->m_id);
			}
			stm.Write(-1); 
			objs.insert(std::pair<uk ,i32>(&g_StaticPhysicalEntity,objs.size()));
			for(CPhysArea *parea=(i=objs.size(),w.m_pGlobalArea); parea; parea=parea->m_next)
				objs.insert(std::pair<uk ,i32>(parea,objs.size()));
			stm.Write((i32)objs.size()-i);
			objs.insert(std::pair<uk ,i32>(pWorld,objs.size()));
			...
			for(i=0;i<=w.m_iLastPlaceholder;i++) if (w.m_pPlaceholderMap[i>>5] & 1<<(i&31)) {
				...
				objs.insert(std::pair<uk ,i32>(ppc,objs.size()));
			}
			stm.Write(1<<30);
			...
			for(i=0;i<8;i++) for(CPhysicalEntity *pent=w.m_pTypedEnts[i]; pent; pent=pent->m_next) {
				...
				tukk name = w.m_pRenderer->GetForeignName(pent->m_pForeignData,pent->m_iForeignData,pent->m_iForeignFlags);
				i32 len = strlen(name); 
			...
			if (stm.Read<i32>()!=version)
				return false;
			stm.ReadRaw(&w.m_vars, (i32)(INT_PTR)&((PhysicsVars*)0)->helperOffset);
			stm.ReadRaw(w.m_BouncinessTable, sizeof(w.m_SurfaceFlagsTable) + (i32)((INT_PTR)&((CPhysicalWorld*)0)->m_SurfaceFlagsTable-(INT_PTR)&((CPhysicalWorld*)0)->m_BouncinessTable));

			if (stm.Read<bool>()) {
				heightfield hf;	stm.Read(hf);
				g_hfData = new unsigned short[(hf.size.x+1)*(hf.size.y+1)];
				g_hfFlags = new u8[(hf.size.x+1)*(hf.size.y+1)];
				g_hfStride.set(1, hf.size.x+1);
				for(i=0; i<(hf.size.x+1)*(hf.size.y+1); i++) {
					i32 val = stm.Read<i32>();
			...
			for(i=0;i<w.m_nGeomChunks;i++) for(j=0;j<GEOM_CHUNK_SZ;j++) if (w.m_pGeoms[i][j].pGeom)	{
				...
				stm.ReadRaw(w.m_pGeoms[i][j].pMatMapping = w.m_pGeoms[i][j].nMats ? new i32[w.m_pGeoms[i][j].nMats]:nullptr, w.m_pGeoms[i][j].nMats*sizeof(i32));
			}
			i32 geom0 = objs.size();
			while(stm.Read<bool>())	{
			...
			i32 geom1 = objs.size();
			while(stm.Read<bool>()) {
				i32 nvtx = stm.Read<i32>(), ntet = stm.Read<i32>();
				Vec3 *vtx = (Vec3*)(stm.m_pBuf+stm.m_iPos); stm.m_iPos += nvtx*sizeof(Vec3);
				i32 *tet = (i32*)(stm.m_pBuf+stm.m_iPos); stm.m_iPos += ntet*4*sizeof(i32);
			...
			i32 water0=objs.size(), nwater=stm.Read<i32>();
			for(i=0;i<nwater;i++)
			...
			for (SEntityGrid *pgrid=nullptr; stm.Read<bool>();) {
				...
				sizer.AddObjectRaw(&pgrid->m_transInHost, sizeof(SEntityGrid)-(i32)(INT_PTR)&((SEntityGrid*)0)->m_transInHost);
				for(j=0;j<pgrid->size.y;j++) for(i=0;i<pgrid->size.x;i++)	if (i32 icell = stm.Read<i32>())
					pgrid->cells[(uint)pgrid->getcell_safe(i,j)] = icell;
				pgrid->cells[(uint)pgrid->getcell_safe(-1,-1)] = stm.Read<i32>();
			}

			i32 ent0 = objs.size();
			for(;(i=stm.Read<i32>())>=0;) {
				CPhysicalEntity *pent = (CPhysicalEntity*)w.CreatePhysicalEntity((pe_type)i,0,0,0,stm.Read<i32>());
				objs.push_back(pent);
				if (pent->m_parts != &pent->m_defpart)
					w.FreeEntityParts(pent->m_parts, pent->m_nPartsAlloc);
			}
			i32 nents=objs.size()-ent0, nareas;
			objs.push_back(&g_StaticPhysicalEntity);
			for(i=0,nareas=stm.Read<i32>(); i<nareas; i++)
				objs.push_back(new CPhysArea(pWorld));
			if (nareas)
				w.m_pGlobalArea = (CPhysArea*)objs[objs.size()-nareas];
			objs.push_back(pWorld);

			while((i=stm.Read<i32>())!=1<<30) {
