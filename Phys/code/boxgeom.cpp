...
i32 CBoxGeom::CalcPhysicalProperties(phys_geometry *pgeom)
{ 
	const box& _box = GetBox();
	pgeom->pGeom = this;
...
i32 CBoxGeom::PointInsideStatus(const Vec3 &pt)
{
	const box& _box = GetBox();
...
i32 CBoxGeom::PrepareForIntersectionTest(geometry_under_test *pGTest, CGeometry *pCollider,geometry_under_test *pGTestColl, bool bKeepPrevContacts)
{
...
void CBoxGeom::PrepareBox(box *pbox, geometry_under_test *pGTest)
{
	...
	if (!_box.bOriented)	pbox->Basis = pGTest->R.T();
	else pbox->Basis = _box.Basis*pGTest->R.T();
	pbox->bOriented = 1;
	pbox->size = _box.size*pGTest->scale;
}

i32 CBoxGeom::PreparePrimitive(geom_world_data *pgwd, primitive *&pprim, i32 iCaller)
{
	const box& _box = GetBox();
	...
	if (!_box.bOriented) pbox->Basis = pgwd->R.T();
	else pbox->Basis = _box.Basis*pgwd->R.T();
	pbox->bOriented = 1;
...
i32 CBoxGeom::GetFeature(i32 iPrim,i32 iFeature, Vec3 *pt)
{ 
	const box& _box = GetBox();
	...
	switch (iFeature & 0x60) {
		case 0:    // vertex
			pt->x=_box.size.x*((iFeature<<1&2)-1); pt->y=_box.size.y*((iFeature&2)-1); 
			pt->z=_box.size.z*((iFeature>>1&2)-1); pt[0] = pt[0]*_box.Basis+_box.center; return 1;
		case 0x20: // edge
			iz = iFeature>>2&3; ix = inc_mod3[iz]; iy = dec_mod3[iz];
			pt[0][ix]=pt[1][ix]=_box.size[ix]*((iFeature<<1&2)-1); pt[0][iy]=pt[1][iy]=_box.size[iy]*((iFeature&2)-1);
			pt[0][iz]=-_box.size[iz]; pt[1][iz]=_box.size[iz]; 
			pt[0] = pt[0]*_box.Basis+_box.center; pt[1] = pt[1]*_box.Basis+_box.center; return 2;
		case 0x40: // face
...
i32 CBoxGeom::GetPrimitiveList(i32 iStart,i32 nPrims, i32 typeCollider,primitive *pCollider,i32 bColliderLocal,
															 geometry_under_test *pGTest,geometry_under_test *pGTestOp, primitive *pRes,char *pResId)
{
	...
	if (pGTestOp->ptOutsidePivot.x<1E10f && szmin*8.0f<szmax) {
		i32 iz=idxmin3(pbox->size), ix=inc_mod3[iz], iy=dec_mod3[iz];
...
i32 CBoxGeom::GetUnprojectionCandidates(i32 iop,const contact *pcontact, primitive *&pprim,i32 *&piFeature, geometry_under_test *pGTest)
{
	...
	i32 iFeature = pcontact->iFeature[iop],ix,iy,iz;
...
i32 CBoxGeom::PreparePolygon(coord_plane *psurface, i32 iPrim,i32 iFeature, geometry_under_test *pGTest, Vec2 *&ptbuf,
				 										 i32 *&pVtxIdBuf,i32 *&pEdgeIdBuf)
{
	...
	Matrix33 R; 
	const box& _box = GetBox();
	if (!_box.bOriented) R = pGTest->R;
	else R = pGTest->R*_box.Basis.T();
	Vec3_tpl<i32> ic;
...
i32 CBoxGeom::PreparePolyline(coord_plane *psurface, i32 iPrim,i32 iFeature, geometry_under_test *pGTest, Vec2 *&ptbuf,
														  i32 *&pVtxIdBuf,i32 *&pEdgeIdBuf)
{
	i32 ix,iy,iz;
	Vec3 pt,offs;
	...
	offs[ix]=offs[iy] = 0; offs[iz] = _box.size[iz]*2;
...
i32 CBoxGeom::FindClosestPoint(geom_world_data *pgwd, i32 &iPrim,i32 &iFeature, const Vec3 &ptdst0,const Vec3 &ptdst1,
															 Vec3 *ptres, i32 nMaxIters)
{
	i32 i,iz,nFacesi,nFaces[2],bLine;
	...
	if (bLine = isneg(size.x*1E-5f-(ptdst0-ptdst1).len2())) {
		i32 ix,iy,iedge,ivtx,isg,iedgeBest;
		Vec3 ptbox,n;
		...
		for(iz=0;iz<3;iz++) {
			...
			{//for(iedge=0;iedge<4;iedge++) {
				...
				if (inrange(t0, (float)0,n2) & isneg(max(fabs_tpl(t1)-n2*size[iz],fabs_tpl((ptbox-ptloc[0])*n)*mindist.y-mindist.x*n2))) {
					iFeature = 0x20|iz<<2|iedge; rn2 = 1.0f/n2;
					ptbox[iz] = t1*rn2;
					...
					mindist.set(fabs_tpl((ptbox-ptloc[0])*n)*rn2,1.0f);	ptbox[iz]=0;
				}
			}
		}
		if (mindist<1E9f)
...
void CBoxGeom::BuildTriMesh(CTriMesh &mesh, i32 bStaticArrays)
{
	...
	i32 i; for(i=0;i<8;i++)	{
		...
		if (_box.bOriented) mesh.m_pVertices[i] = mesh.m_pVertices[i]*_box.Basis;
		mesh.m_pVertices[i] += _box.center;
	}
	for(i=0;i<6;i++) {
		mesh.m_pNormals[i*2].Set(0,0,0); mesh.m_pNormals[i*2][i>>1] = ((i&1)<<1)-1;
		if (_box.bOriented) mesh.m_pNormals[i*2] = mesh.m_pNormals[i*2]*_box.Basis;
...
IGeometry *CBoxGeom::GetTriMesh(i32 bClone)
{
...
i32 CBoxGeom::UnprojectSphere(Vec3 center,float r,float rsep, contact *pcontact)
{
	sphere sph;
	sph.center = center;
	sph.r = rsep;
	...
	umode.dir.Set(0,0,0);
	i32 res = box_sphere_lin_unprojection(&umode,&GetBox(),-1,&sph,-1,pcontact,0);
...
void CBoxGeom::CalculateMediumResistance(const plane *pplane, const geom_world_data *pgwd, Vec3 &dPres,Vec3 &dLres)
{
	...
	Matrix33 R; 
	if (!_box.bOriented) R = pgwd->R;
	else R = pgwd->R*_box.Basis.T();
...
i32 CBoxGeom::DrawToOcclusionCubemap(const geom_world_data *pgwd, i32 iStartPrim,i32 nPrims, i32 iPass, SOcclusionCubeMap* cubeMap)
{
	i32 i,j,ix,iy,iz;
	const box& _box = GetBox();
	Vec3 pt[4],n, size=_box.size*pgwd->scale, offset=pgwd->R*_box.center*pgwd->scale+pgwd->offset;;
...
void CBoxGeom::DrawWireframe(IPhysRenderer *pRenderer, geom_world_data *gwd, i32 iLevel, i32 idxColor)
{
	if (iLevel==0)
...
void CBoxGeom::GetMemoryStatistics(IDrxSizer *pSizer)
{
