...
phys_job_info& GetJobProfileInst(i32);
...
void RigidBody::Add(const Vec3 &center,const Vec3 Ibodyop,const quaternionf &qop, float volume,float mass)
{
	...
	float density=mass/volume; i32 i;
...
struct RBdata {
	...
	i32 nFollowers,idLastUpdate,nUnprojLoops;
	char SolverBuf[262144];
	i32 iSolverBufPos;
	char *pSolverBufAux;
	i32 iSolverBufAuxPos;
	i32 sizeSolverBufAux;
	bool bUsePreCG;
...
void GetRBMemStats(IDrxSizer *pSizer)
{
	...
	for(i32 i=0;i<MAX_PHYS_THREADS;i++) if (g_RBdata[i])
...
static i32 g_nFollowers,g_idLastUpdate,g_nUnprojLoops;
static char g_SolverBuf[262144];
static i32 g_iSolverBufPos;
bool g_bUsePreCG = true;*/
i32 __solver_step=0;


inline bool should_swap(entity_contact **pContacts,i32 i1,i32 i2,i32 &diff, i32 iCaller) { // sub-sorts contacts by pbody[1]
...
inline bool should_swap(RigidBody **pBodies, i32 i1,i32 i2,i32 &diff, i32 iCaller) { // sorts bodies by level
...
bool should_swap(entity_contact_unproj **pContacts,i32 i1,i32 i2,i32 &diff, i32 iCaller) { // sorts contacts by newly sorted bodies 
	i32 iop1 = isneg(g_infos[pContacts[i1]->pbody[0]->bProcessed[iCaller]].iLevel-g_infos[pContacts[i1]->pbody[1]->bProcessed[iCaller]].iLevel);
	i32 iop2 = isneg(g_infos[pContacts[i2]->pbody[0]->bProcessed[iCaller]].iLevel-g_infos[pContacts[i2]->pbody[1]->bProcessed[iCaller]].iLevel);
...
template<class dtype> static void qsort(dtype *pArray,i32 left,i32 right,i32 iCaller) {
...
void update_followers(i32 iBody,i32 idUpdate, i32 iCaller) {
...
void trace_unproj_route(i32 iMiddle,i32 iBread, i32 iCaller);

void add_route_follower(i32 iBody,i32 iFollower, i32 iCaller) {
	follower_thunk* pfollower;
...
void update_level(i32 iBody, i32 iNewLevel, i32 iCaller) {
	if ((u32)g_infos[iBody].iLevel>=(u32)iNewLevel)
...
real ComputeRc(RigidBody *body0, entity_contact **pContacts, i32 nAngContacts,i32 nContacts, i32 iCaller) // for MINRES unprojection solver
{
	i32 i;
	real rT_x_rc=0;
	Vec3 dP,r0;
...
char *AllocSolverTmpBuf(i32 size)
{
	...
	if (g_iSolverBufPos+size<sizeof(g_SolverBuf)) {
		...
		if (g_iSolverBufAuxPos+size>g_sizeSolverBufAux)	{
			...
			for(i32 i=0; i<g_nContacts; i++) {
				if ((u32)((tuk)g_pContacts[i]-pBufAux) < (u32)(g_sizeSolverBufAux-sizeof(g_SolverBuf)))
					(tuk&)g_pContacts[i] += g_pSolverBufAux-pBufAux;
				if ((u32)((tuk)g_pContacts[i]->pBounceCount-pBufAux) < (u32)(g_sizeSolverBufAux-sizeof(g_SolverBuf)))
					(tuk&)g_pContacts[i]->pBounceCount += g_pSolverBufAux-pBufAux;
			}
		}
...
void RegisterContact(entity_contact *pcontact)
{
	...
	g_nContacts = min(g_nContacts,(i32)(DRX_ARRAY_COUNT(g_pContacts))-1);
}
...
inline void ApplyImpulse(body_helper *pbody, const Vec3& dP, const Vec3& dL, i32 iCaller)
{
	if (pbody->M>=0.0f) {
...
i32 InvokeContactSolverMC(contact_helper *pContactsRB,contact_helper_constraint *pContactsC,body_helper *pBodies, 
													i32 nContacts,i32 nBodies, float Ebefore, i32 nMaxIters,float e,float minSeparationSpeed)
{
	DRX_PROFILE_REGION(PROFILE_PHYSICS, "LCPMC");
	i32 iCaller = get_iCaller_int();
	i32 i,j,bBounced,istart,iend,istep,nBounces=0,bContactBounced;
...
i32 ReadDelayedSolverResults(CMemStream &stm, entity_contact **&pContactsOut,RigidBody **&pBodiesOut)
{
	i32 iCaller = get_iCaller_int();
	i32 nBodies,flags,nMaxIters,bBounced;
	float dt,Ebefore,e,minSeparationSpeed;
#if defined(MEMSTREAM_DEBUG)
	{ i32 tag; stm.Read(tag); if (tag!=MEMSTREAM_DEBUG_TAG) {printf("begin %x!=%x\n",tag, MEMSTREAM_DEBUG_TAG); __debugbreak(); }}
#endif 
...
#if defined(MEMSTREAM_DEBUG)
	{ i32 tag; stm.Read(tag); if (tag!=MEMSTREAM_DEBUG_TAG+1) {printf("end %x!=%x\n",tag, MEMSTREAM_DEBUG_TAG+1); __debugbreak(); }}
#endif 
...
{
	i32 nContacts,nBodies,nMaxIters,flags,dummy;
	float time_interval,Ebefore,e,minSeparationSpeed;
#if defined(MEMSTREAM_DEBUG)
  {i32 tag; stm.Read(tag); if (tag!=MEMSTREAM_DEBUG_TAG) {printf("beg %x!=%x\n",tag, MEMSTREAM_DEBUG_TAG); __debugbreak(); }}
#endif 
	stm.Read(nContacts); stm.Read(nBodies);	
	stm.Read(time_interval); stm.Read(Ebefore);	
	stm.Read(nMaxIters); stm.Read(e); stm.Read(minSeparationSpeed);
	stm.Read(flags);
	i32 *bBounced = (i32*)(stm.m_pBuf+stm.m_iPos);
...
template<typename TransFull, typename TransRot, i32 ipass>
i32 PrepContacts(i32 iCaller, i32 &nConstraintsRes, SEntityGrid* &pgridRes, i32 &bMultigridRes)
{
	i32 nBodies=0, nConstraints=0, bMultigrid=0;
	SEntityGrid *pgrid = ipass==0 ? GetGridFromEntity(g_pContacts[0]->pent[0]) : pgridRes;
	RigidBody *pbodyGrid = g_pContacts[0]->pbody[0];
	Matrix33 Ctmp;

	for(i32 i=0,j,iop; i<g_nContacts; i++) {
		...
		if (cnt.flags & contact_constraint_3dof)
			...
			if (cnt.flags & contact_constraint_1dof) {
				Vec3 axes[2]; i32 k; float mtx[2][2]; Matrix33 mtx1;
...
void PrepContactsMore(i32 iCaller, SEntityGrid *pgrid=nullptr)
{
	for(i32 i=0; i<g_nContacts; i++) {
		entity_contact &cnt = *g_pContacts[i];
		...
		g_ContactsRB[i].iCountDst = ((entity_contact*)((tuk)cnt.pBounceCount-
			((tuk)&cnt.iCount-(tuk)g_pContacts[i])))->bProcessed;
		g_ContactsRB[i].Pn = 0;
	}
}

template<typename TransDyn>
i32 PrepBodies(i32 iCaller, SEntityGrid *pgrid=nullptr)
{
	i32 nArtic=0, nBodies=g_nBodies;
...
template<typename TransDyn>
void UpdateBodies(i32 iCaller, SEntityGrid *pgrid=nullptr)
{
	i32 nBodies = g_nBodies;
	if (sizeof(TransDyn) > sizeof(noGridTrans))
		for(i32 i=0;i<g_nContacts;i++) {
			...
			if (g_pContacts[i]->flags & contact_rope) {
				...
				for(i32 j=0; j<g_pContacts[i]->iCount; j++)
...
i32 InvokeContactSolver(float time_interval, SolverSettings *pss, float Ebefore, entity_contact **&pContactsOut,i32 &nContactsOut)
{
	DRX_PROFILE_FUNCTION(PROFILE_PHYSICS );

	i32 iCaller = get_iCaller()-1+FIRST_WORKER_THREAD;
	pContactsOut=g_pContacts;
	if (g_nContacts==0) return 0;
	i32 i,j,iop,nBodies,istart,iend,iter,bBounced,nConstraints,nMaxIters,nBuddies,bNeedLCPCG=0;
...
__solver_step++;
	...
	if (g_bUsePreCG && g_nContacts<16 && !bMultigrid) {
		DRX_PROFILE_REGION(PROFILE_PHYSICS, "PreCG");
	...
	// use several iterations of CG solver, solving only for non-separating contacts
		u32 iClass;
		i32 cgiter,bStateChanged,n1dofContacts,n2dofContacts,nAngContacts,nFric0Contacts,nFricInfContacts,
		...
		if (bBounced) {
			{ DRX_PROFILE_REGION(PROFILE_PHYSICS,"LCPCG");
			...
			if (bBounced) {
				DRX_PROFILE_REGION(PROFILE_PHYSICS, "LCPCG-unproj");

				///////////////////////////////////////////////////////////////////////////////////
				// now, use a separate solver for unprojections (unproject each body independently)
				entity_contact *pContacts[DRX_ARRAY_COUNT(g_pContacts)];
				i32 nSandwiches,iMinLevel,iMaxLevel,iLevel;
				float minMinv;
					...
					for(pbuddy0=g_infos[i].pbuddy; pbuddy0; pbuddy0=pbuddy0->next)
						for(pbuddy1=pbuddy0->next; pbuddy1; pbuddy1=pbuddy1->next)
							if (pbuddy0->vreq*pbuddy1->vreq<0 && !((pbuddy0->flags^pbuddy1->flags)&contact_angular)) {
								...
								if (nSandwiches==DRX_ARRAY_COUNT(g_sandwichbuf))
									goto endsandw;
							}
				endsandw:
