...
public:
	matrix_vector_product_tpl(i32 nrows,i32 ncols,i32 _istride,i32 _jstride,ftype *pmtxdata, ftype *pvecdata) {
		nRows=nrows; nCols=ncols; istride=_istride; jstride=_jstride; mtxdata=pmtxdata; vecdata=pvecdata;
	}
	i32 nRows,nCols;
	i32 istride,jstride;
...
template <class ftype> class vector_scalar_product_tpl {
public:
	vector_scalar_product_tpl(i32 ncols,ftype *pdata,ftype scalar) {
		len=ncols; data=pdata; op=scalar;
	}
	ftype *data;
	i32 len;
	ftype op;
...
public:
	vectorn_tpl(i32 _len,ftype *pdata=0) {
		len = _len;
	...
	vectorn_tpl& operator=(const vectorn_tpl<ftype> &src) {
		...
		if (src.len!=len && !(flags & mtx_foreign_data)) {
			delete data; data = new ftype[src.len];
		}
		len = src.len;
		for(i32 i=0;i<len;i++) data[i]=src.data[i];
		return *this;
	}
	// cppcheck-suppress operatorEqVarError
	...
	ftype len2() { ftype res=0; for(i32 i=0;i<len;i++) res += data[i]*data[i]; return res; }

	ftype& operator[](i32 idx) const { return data[idx]; }
	operator ftype*() { return data; }
	vectorn_tpl& zero() { for(i32 i=0;i<len;i++) data[i]=0; return *this; }
	vectorn_tpl& allocate() {
		if (flags & mtx_foreign_data) {
			ftype *newdata = new ftype[len];
			for(i32 i=0;i<len;i++) newdata[i]=data[i];
	...
	/*vectorn_tpl operator*(ftype op) const {
		...
		for(i32 i=0;i<len;i++) res.data[i] = data[i]*op;
		return res;
	}*/

	vectorn_tpl& operator*=(ftype op) {
		for(i32 i=0;i<len;i++) data[i] *= op;
		return *this;
	}
	...
	static i32 vecn_pool_pos;
	static i32 vecn_pool_size;
#endif
};

template<class ftype1,class ftype2>
ftype1 operator*(const vectorn_tpl<ftype1>& op1, const vectorn_tpl<ftype2>& op2) {
	ftype1 res=0; for(i32 i=0;i<op1.len;i++) res += op1.data[i]*op2.data[i];
	return res;
}
...
vectorn_tpl<ftype1> operator+(const vectorn_tpl<ftype1> &op1,const vectorn_tpl<ftype2> &op2) {
	...
	for(i32 i=0;i<len;i++) res.data[i] = op1.data[i]+op2.data[i];
	return res;
}
*/
template<class ftype1,class ftype2>
vectorn_tpl<ftype1> operator-(const vectorn_tpl<ftype1> &op1, const vectorn_tpl<ftype2> &op2) {
	...
	for(i32 i=0;i<op1.len;i++) res.data[i] = op1.data[i]-op2.data[i];
	return res;
}

template<class ftype1,class ftype2>
vectorn_tpl<ftype1>& operator+=(vectorn_tpl<ftype1> &op1, const vectorn_tpl<ftype2> &op2) {
	for(i32 i=0;i<op1.len;i++) op1.data[i] += op2.data[i];
	return op1;
}
...
vectorn_tpl<ftype1> operator*(const vectorn_tpl<ftype1> &vec, const matrix_tpl<ftype2> &mtx) {
	i32 i,j; vectorn_tpl<ftype1> res(mtx.nCols);
...
vectorn_tpl<ftype1> operator*(const matrix_tpl<ftype1> &mtx, const vectorn_tpl<ftype2> &vec) {
	i32 i,j; vectorn_tpl<ftype1> res(mtx.nRows);
...
typedef vectorn_tpl<real> vectorn;
    ...
    template<> i32 vectorn_tpl<ftype>::vecn_pool_pos=0;                     \
		template<> i32 vectorn_tpl<ftype>::vecn_pool_size=sz;
#else
	#define DECLARE_VECTORN_POOL(ftype,sz) template<> ftype vectorn_tpl<ftype>::vecn_pool[sz]; template<> i32 vectorn_tpl<ftype>::vecn_pool_pos=0; \
		template<> i32 vectorn_tpl<ftype>::vecn_pool_size=sz;
#endif
