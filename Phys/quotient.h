...
#define sgnnz_result_type(F) i32
#define isneg_result_type(F) i32
...
public:
    ...
    ILINE quotient_tpl& fixsign() { i32 sgny=::sgnnz(y); x*=sgny; y*=sgny; return *this; }
...
template<class ftype> ILINE i32 sgn(const quotient_tpl<ftype> &op) { return sgn(op.x); }
...
template<class ftype> ILINE i32 isnonneg(const quotient_tpl<ftype> &op) { return isnonneg(op.x); }
template<class ftype> ILINE i32 sgn_safe(const quotient_tpl<ftype> &op) { return sgn(op.x)*sgnnz(op.y); }
...
template<class ftype> ILINE i32 isnonneg_safe(const quotient_tpl<ftype> &op) { return isnonneg(op.x)*isnonneg(op.y); }
template<class ftype> ILINE i32 inrange(const quotient_tpl<ftype> &op, ftype end0, ftype end1) { return isneg(sgn_safe(op-end0)*sgn_safe(op-end1)); }
...
template<class ftype> ILINE quotient_tpl<ftype> sqr_signed(const quotient_tpl<ftype>& op)  { return op * drxmath::abs(op); }

template<class ftype> ILINE quotient_tpl<ftype> fake_atan2(ftype y,ftype x) {
    quotient_tpl<ftype> res;
    ftype src[2] = { x,y };
    i32 ix=isneg(x),iy=isneg(y),iflip=isneg(fabs_tpl(x)-fabs_tpl(y));
...
typedef quotient_tpl<i32> quotienti;
