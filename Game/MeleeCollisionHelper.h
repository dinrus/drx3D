...
public:
	...
	void Impulse(IPhysicalEntity *pCollider, const Vec3 &position, const Vec3 &impulse, i32 partId, i32 ipart, i32 hitTypeID);
	void GenerateArtificialCollision(IEntity* pUser, IPhysicalEntity *pCollider, const Vec3 &position, const Vec3& normal, const Vec3 &speed, i32 partId, i32 ipart, i32 surfaceIdx, i32 iPrim );
...
private:
	...
	ILINE void OnSuccesfulHit(const ray_hit& hitResult) { DRX_ASSERT(m_pUser); m_pUser->OnSuccesfulHit(hitResult); }
	ILINE void OnFailedHit() { DRX_ASSERT(m_pUser); m_pUser->OnFailedHit(); };
