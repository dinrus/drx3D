...
#include <drx3D/Game/ActionSpeakLineBasedOnVariable.h>
...
DRS::IResponseActionInstanceUniquePtr CActionSpeakLineBasedOnVariable::Execute(DRS::IResponseInstance* pResponseInstance)
{
	...
	if (!pSpeaker)
	{
		DrxWarning(VALIDATOR_MODULE_DRS, VALIDATOR_WARNING, "Triggered an SpeakLineAction, without a valid Speaker! LineID was: '%s', Speaker override was '%s'",
			lineToSpeak.GetText().c_str(), m_speakerOverrideName.GetText().c_str());
		return nullptr;
	}

	if (gEnv->pDynamicResponseSystem->GetSpeakerUpr()->StartSpeaking(pSpeaker, lineToSpeak))
	{
...
DRS::IResponseActionInstance::eCurrentState CActionSpeakLineBasedOnVariableInstance::Update()
{
	...
	const DRS::ISpeakerUpr* pSpeakerMgr = gEnv->pDynamicResponseSystem->GetSpeakerUpr();
...
{
	DRS::ISpeakerUpr* pSpeakerMgr = gEnv->pDynamicResponseSystem->GetSpeakerUpr();
	if (pSpeakerMgr->IsSpeaking(m_pSpeaker, m_LineID))
	{
