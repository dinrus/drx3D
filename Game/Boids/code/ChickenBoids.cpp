	...
	, m_landing(0)
{
	...
	m_maxIdleTime = drx_random(MIN_REST_TIME, MAX_REST_TIME);
	m_maxNonIdleTime = drx_random(0.0f, MAX_WALK_TIME);
}
...
//	float height = m_pos.z - bc.terrainZ;
	...
	*/
	...
	if (drx_random(0, (m_bScared ? 127 : 254)) == 0)
	...
	if (bc.avoidObstacles && m_speed > 0.0f && (frameID-m_lastRayCastFrame) > 3)
	{
		...
		i32 objTypes = ent_all|ent_no_ondemand_activation;
		i32 flags = rwi_stop_at_pierceable|rwi_ignore_terrain_holes;
		ray_hit hit;
		i32 col = physWorld->RayWorldIntersection( vPos,vDir,objTypes,flags,&hit,1 );
		if (col != 0 && hit.dist > 0)
		{
	...
	if (!m_landing)
	{
		...
		{
			// Play idle.
			PlayAnimationId( CHICKEN_IDLE_ANIM + drx_random(0, CHICKEN_IDLE_ANIM_NUM - 1), true );
			m_maxIdleTime = drx_random(MIN_REST_TIME, MAX_REST_TIME);
			m_landing = true;
			m_actionTime = 0;
