...
//////////////////////////////////////////////////////////////////////////
bool CBoidObject::PlayAnimation( tukk animation, bool bLooped, float fBlendTime )
{
	bool playing = false;
	...
	if (m_object)
	{
		...
		i32k amountAnimationsInFIFO = pSkeleton->GetNumAnimsInFIFO(0);
		u32k maxAnimationsAllowedInQueue = 2;
...
//////////////////////////////////////////////////////////////////////////
bool CBoidObject::PlayAnimationId( i32 nIndex,bool bLooped,float fBlendTime )
{
	if (nIndex >= 0 && nIndex < (i32)m_flock->m_bc.animations.size())
		return PlayAnimation( m_flock->m_bc.animations[nIndex],bLooped,fBlendTime );
	return false;
}

//////////////////////////////////////////////////////////////////////////
void CBoidObject::ExecuteTrigger(i32 nIndex)
{
	if ((nIndex >= 0) && (nIndex < (i32)m_flock->m_bc.audio.size()))
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CBoidObject::GetGeometrySurfaceType()
{
	if (m_object)
	{
		ISurfaceType *pSurfaceType = gEnv->pEntitySystem->GetBreakableUpr()->GetFirstSurfaceType(m_object);
		if (pSurfaceType)
	...
	else
	{
		...
		if (pEntity)
		{
			...
			if (pStatObj)
			{
				ISurfaceType *pSurfaceType = gEnv->pEntitySystem->GetBreakableUpr()->GetFirstSurfaceType(pStatObj);
				if (pSurfaceType)
					return pSurfaceType->GetId();
			}
		}
...
void CBoidObject::CalcFlockBehavior( SBoidContext &bc,Vec3 &vAlignment,Vec3 &vCohesion,Vec3 &vSeparation )
{
	...
	i32 numBoids = m_flock->GetBoidsCount();
	for (i32 i = 0; i < numBoids; i++)
	{
...
void CBoidObject::CreateRigidBox( SBoidContext &bc,const Vec3 &boxSize,float mass,float density )
{
	...
	geomBox.bOriented = 0;
	IGeometry *pGeom = bc.physics->GetGeomUpr()->CreatePrimitive( primitives::box::type,&geomBox );
	phys_geometry *physGeom = bc.physics->GetGeomUpr()->RegisterGeometry( pGeom );
	pGeom->Release();
	...
	bc.physics->GetGeomUpr()->UnregisterGeometry( physGeom );
