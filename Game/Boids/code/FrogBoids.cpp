...
{
	for (i32 i = 0,num = m_boids.size(); i < num; i++)
	{
	...
	, m_onGround(0)
{
	...
	m_maxActionTime = drx_random(0.0f, MAX_WALK_TIME);
}
...
void CFrogBoid::Think( float dt,SBoidContext &bc )
{
	...
	if (m_onGround)
	{
		...
		if (m_fTimeToNextJump <= 0 || sqrPlayerDist < fScareDist*fScareDist || bScaredJump)
		{
			...
			m_fTimeToNextJump = drx_random(2.0f, 7.0f);
			//m_fTimeToNextJump = 0;
			...
			m_speed = drx_random(bc.MinSpeed, bc.MaxSpeed);
		}
	}
