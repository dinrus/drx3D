...
CBoidBird::CBoidBird(SBoidContext &bc)
	...
	, m_lastThinkTime(0)
	, m_maxActionTime(0)
	, m_fNoCenterAttract(0.0f)
	, m_fNoKeepHeight(0.0f)
	, m_fAttractionFactor(0.0f)
	, m_elapsedSlowdownTime(0.0f)
	, m_startLandSpeed(0.0f)
	, m_onGroundStatus(Bird::EOnGroundStatus::OGS_WALKING)
	...
	, m_spawnFromPt(false)
{
	...
	m_desiredHeigh = drx_random(bc.MinHeight, bc.MaxHeight) + bc.flockPos.z;
...
void CBoidBird::Update( float dt,SBoidContext &bc )
{
	...
	char text[100];
	drx_sprintf(text, "%f - %i", m_speed, m_onGroundStatus);
	...
	if(m_actionTime > m_maxActionTime)
	{
		...
		case Bird::OGS_IDLE:
			...
			if(bc.walkSpeed > 0)
			{
				...
				m_actionTime = 0;
				m_maxActionTime = drx_random(bc.fOnGroundWalkDurationMin, bc.fOnGroundWalkDurationMax);
			}
			else
			{
				...
				m_actionTime = 0;
				m_maxActionTime = drx_random(bc.fOnGroundIdleDurationMin, bc.fOnGroundIdleDurationMax);
			}
			break;
		...
		default:
			DRX_ASSERT_MESSAGE(0, "CBoidBird::UpdateOnGroundAction: omitted EOnGroundStatus");
		}
	}
}	
...
//	m_maxActionTime = MIN_FLIGHT_TIME + (Boid::Frand()+1)/2*(MAX_FLIGHT_TIME-MIN_FLIGHT_TIME);
	...
	if (m_object)
	{
		tukk takeoffName = bc.GetAnimationName(Bird::ANIM_TAKEOFF);
		if(m_TakeOffAnimLength ==0 && takeoffName != NULL && strlen(takeoffName) )
		{
			i32 id = m_object->GetIAnimationSet()->GetAnimIDByName(takeoffName);
...
void CBoidBird::Kill( const Vec3 &hitPoint,const Vec3 &force )
{
	...
	if (pEntity)
	{
		...
		gEnv->pEntitySystem->GetBreakableUpr()->AttachSurfaceEffect( pEntity,0,SURFACE_BREAKAGE_TYPE("destroy"),params );
	}
