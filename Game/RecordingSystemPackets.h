...
struct SRecording_FPChar : SRecording_Packet
{
	...
	float fov;
	float frametime;
	u8 playerFlags;	// EFirstPersonFlags
};
...
struct SRecording_TPChar : SRecording_Packet
{
	...
	Vec3 aimdir;
	EntityId eid;
	u32 layerEffectParams;	// Used for suit mode effects
	u16 playerFlags;		// EThirdPersonFlags
	u8 healthPercentage;
};
...
struct SRecording_MannSetSlaveController : SRecording_Packet
{
	...
	u32 optionalDatabaseFilenameCRC;
	u8 targetContext;
	bool enslave;
};
...
struct SRecording_EntitySpawn : SRecording_Packet
{
	...
	IEntityClass* pClass;
	tukk szCharacterSlot[RECORDING_SYSTEM_MAX_SLOTS];
	IStatObj * pStatObj[RECORDING_SYSTEM_MAX_SLOTS];
	i32 slotFlags[RECORDING_SYSTEM_MAX_SLOTS];
...
struct SRecording_BattleChatter : SRecording_Packet
{
	...
	u16 entityNetId;
	u8 chatterType;
	u8 chatterVariation;		//0 is no specific variation, 1-255 is variation 0-254.
};
...
{
	static u32k MAX_FRAGMENT_TRIGGERS = 16;
