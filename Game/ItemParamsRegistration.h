...
$Id$
$DateTime$
Описание: Macros to help construct structs that automatically implement != and < 
...
for(i32 i = 0; i < varNum; i++) \
{ if(instanceA.varName[i] != instanceB.varName[i]) { return true; } }

#define INEQUALITY_COMPARE_MEMBER_VECTORS(varType, varName) \
	i32 numInstanceA = instanceA.varName.size(); \
	i32 numInstanceB = instanceB.varName.size(); \
	if(numInstanceA != numInstanceB) { return true; } \
	for(i32 i = 0; i < numInstanceA; i++) { if(instanceA.varName[i] != instanceB.varName[i]) { return true; } }
...
#define LESSTHAN_COMPARE_MEMBER_VECTORS(varType, varName) \
	...
	for(i32 i = 0; i < numInstanceA; i++) { if(instanceA.varName[i] != instanceB.varName[i]) { return (instanceA.varName[i] < instanceB.varName[i]); } }
