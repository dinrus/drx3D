...
#define RecSysQuatTValues(quat) (quat).t.x, (quat).t.y, (quat).t.z, (quat).q.v.x, (quat).q.v.y, (quat).q.v.z, (quat).q.w
    ...
    #define RecSysLog(...)								DrxLog("[RECSYS] " __VA_ARGS__)
    #define RecSysLogAlways(...)					DrxLogAlways("[RECSYS] " __VA_ARGS__)
	#define RecSysLogDebug(x,...)					{if(g_pGameCVars->kc_debug==(x)){DrxLogAlways("[RECSYS] " RecSysDebugStr_##x __VA_ARGS__);}}
	#define RecSysLogDebugIf(x,y,...)			{if(g_pGameCVars->kc_debug==(x) &&(y)){DrxLogAlways("[RECSYS] " RecSysDebugStr_##x __VA_ARGS__);}}
	#define RecSysLogFunc									do { RecSysLog("%s():%d", __FUNC__, __LINE__); } while (0);
	#define RecSysWatch(...)							DrxWatch(__VA_ARGS__);
	#define RecSysWatchDebug(x,...)				{if(g_pGameCVars->kc_debug==(x)){DrxWatch(__VA_ARGS__);}}
#else //RECSYS_DEBUG
...
struct SFirstPersonDataContainer
{
	...
	u32 m_firstPersonDataSize;
	u8 m_firstPersonData[FP_RECEIVE_BUFFER_SIZE];
...
struct SKillCamStreamData
{
	...
	u8 m_buffer[FP_RECORDING_BUFFER_SIZE];
	size_t m_finalSize;
	i32 m_packetReceived[2];
	i32 m_packetTargetNum[2];
...
private:
	static i32k kMaxSimultaneousStreams = 2;
...
struct SHighlightRules 
{
	...
	SFactorMap<i32> m_hitTypes;
	SFactorMap<i32> m_multiKills;
...
public:
	...
	struct View
	{
		...
		DrxFixedStringT<128> streamingWeaponFPModel;
		DrxFixedStringT<128> streamingWeaponTPModel;
...
struct SInteractiveObjectAnimation
{
	...
	SInteractiveObjectAnimation(TagID interactionTypeTag, u8 interactionIndex) : m_interactionTypeTag(interactionTypeTag), m_interactionIndex(interactionIndex) {}

	TagID m_interactionTypeTag;
	u8 m_interactionIndex;
};


typedef std::map<EntityId, Vec3> ChrVelocityMap;
typedef std::map<EntityId,AimIKInfo> EntityAimIKMap;
typedef std::map<EntityId,i32> EntityIntMap;
...
typedef std::map<u32, Vec3> TPendingRaycastMap;
...
typedef std::vector<u32> TCorpseADIKJointsVec;
