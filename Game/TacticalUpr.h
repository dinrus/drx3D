...
public:
	...
	typedef DrxFixedArray<TInterestPoints, eTacticalEntity_Last> TAllTacticalPoints;
	typedef std::map<EntityId, EntityId> TTacticalEntityToOverrideEntities;
public:

														CTacticalUpr();
	virtual										~CTacticalUpr();
	...
	void											SetEntityOverrideIcon(const EntityId id, u8k overrideIconType);
	u8											GetEntityOverrideIcon(const EntityId id) const;
	u8											GetOverrideIconType(tukk szOverrideIconName) const;
