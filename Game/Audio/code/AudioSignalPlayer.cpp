...
//void CAudioSignalPlayer::Play( EntityId entityID, tukk pParam, float param, ESoundSemantic semanticOverride, i32* const pSpecificRandomSoundIndex )
//{
...
//
//	u32k soundCount = pAudioSignal->m_sounds.size();
//
...
//
//			const size_t soundIndex = (pSpecificRandomSoundIndex && *pSpecificRandomSoundIndex>=0 && *pSpecificRandomSoundIndex<(i32)soundCount) ? (size_t)*pSpecificRandomSoundIndex : drx_random(0, soundCount - 1);
//			if(pSpecificRandomSoundIndex)
//				*pSpecificRandomSoundIndex = (i32)soundIndex;
...
//		else
//		{
//			DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "AudioSignalPlayer: PlayRandom flag on but no sounds '%s'", pAudioSignal->m_signalName.c_str());
//		}
//	}
//	else
//	{
...
void CAudioSignalPlayer::ExecuteCommands( EntityId entityID, const CGameAudio::CAudioSignal* pAudioSignal )
{
	...
	u32 numCommands = pAudioSignal->m_commands.size();
...
//{
//	u32k numPlayingSounds = m_playingSoundIDs.size();
//	for (u32 soundIndex = 0; soundIndex < numPlayingSounds; ++soundIndex)
//	{
...
void CAudioSignalPlayer::SetSignal( tukk pName )
{
	DRX_ASSERT(pName && pName[0]);
...
void CAudioSignalPlayer::SetSignalSafe( tukk pName )
{
	if (pName && pName[0]!=0)
		SetSignal( pName );
	else
...
void CAudioSignalPlayer::SetCurrentSamplePos( EntityId entityID, float relativePosition )
{
	...
	for (size_t s=0; s<numSounds; ++s)
	{
		...
		if (pSound)
		{
			u32k newPos = int_round((float)pSound->GetLengthMs() * relativePosition);
...
float CAudioSignalPlayer::GetCurrentSamplePos( EntityId entityID )
{
	...
	for (size_t s=0; s<numSounds; ++s)
	{
		...
		if (pSound)
		{
			i32k length = pSound->GetLengthMs();
			if(length>0)
			{
...
void CAudioSignalPlayer::SetParam( EntityId entityID, tukk paramName, float paramValue )
{
	/*IEntityAudioProxy* pProxy = CGameAudioUtils::GetEntityAudioProxy( entityID );
...
//tSoundID CAudioSignalPlayer::PlaySound( const string& name, const ESoundSemantic semantic, EntityId entityID, tukk pParam, float param, u32 flags )
//{
...
//		{
//			DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "AudioSignalPlayer: Unable to create sound '%s'", sFixSoundName);
//		}
//#endif
//		return INVALID_SOUNDID;
//	}
...
////////////////////////////////////////////////////////////////////////////
//
//tSoundID CAudioSignalPlayer::PlaySound( const string& name, const ESoundSemantic semantic, const Vec3& pos, u32 flags )
//{
//	// test if we can cull early
//	SSoundCacheInfo CullInfo;
//	CullInfo.bCanBeCulled = false;
...
//		if(!gEnv->IsDedicated() && gEnv->pConsole->GetCVar("s_SoundEnable")->GetIVal())
//		{
//			DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "AudioSignalPlayer: Unable to create sound '%s'", name.c_str());
//		}
//#endif
//		return INVALID_SOUNDID;
//	}
...
void CAudioSignalPlayer::JustPlay( tukk signal, EntityId entityID, tukk pParam, float param)
{
	JustPlay(g_pGame->GetGameAudio()->GetSignalID(signal), entityID, pParam, param);
}

void CAudioSignalPlayer::JustPlay( TAudioSignalID signalID, EntityId entityID, tukk pParam, float param)
{
	...
	if(pAudioSignal->m_flags & CGameAudio::CAudioSignal::eF_PlayRandom)
	{
		if(soundCount > 0)
		{
			const size_t soundIndex = drx_random(0, soundCount - 1);
			const CGameAudio::CSound& sound = pAudioSignal->m_sounds[soundIndex];
...
void CAudioSignalPlayer::JustPlay( tukk signal, const Vec3& pos )
{
...
float CAudioSignalPlayer::GetSignalLength(TAudioSignalID signalID)
{
	...
	if(pAudioSignal)
	{
	i32k soundsSize = pAudioSignal->m_sounds.size();
	for(i32 i = 0; i < soundsSize; i++)
	{
...
/*static*/ tukk CAudioSignalPlayer::GetSignalName(TAudioSignalID signalId)
{
	const CGameAudio::CAudioSignal* pAudioSignal = g_pGame->GetGameAudio()->GetAudioSignal( signalId );
...
tukk CAudioSignalPlayer::GetSignalName()
{
