...
typedef i16 TGameRulesScoreInt; // if this type changes update compression policy and score limits in ScoreChangeParams
...
#define EXPReasonFromEGRST(egrstEnumValue)			EXPReason(i32(egrstEnumValue-EGRST_PlayerKill)+i32(k_XPRsn_EGRST_PlayerKill))
#define EXPReasonFromEPP(eppEnumValue)					EXPReason(i32(eppEnumValue-EPP_TeamRadar)+i32(k_XPRsn_EPP_TeamRadar))
...
struct SGameRulesNotificationInfo
{
	...
	SGameRulesNotificationInfo(EGameRulesTargetType _type, tukk _message, tukk _arg1, tukk _arg2)
...
struct SGameRulesMissionObjectiveInfo
{
	...
	static EGameRulesMissionObjectives GetIconId(tukk nameOrNumber);
};
...
struct SBattleLogMessageInfo
{
	...
	DrxFixedStringT<35> weaponClassName;
	i32      hitType;
	EntityId shooterId;
	EntityId targetId;
