...
public:
	...
	void Load();
	void GetMemoryUsage(IDrxSizer* s) const; // only used in CMovementTransitionsSystem
...
private:
	...
	// private methods
	void FindBestMatch( const STransitionSelectionParams& transParams, const STransition** ppBestTransition, i32* pBestIndex, STransitionMatch* pBestMatch ) const;
	...
	{
		tukk MOVEMENTTRANSITIONS_FILENAME_PREFIX = "Libs/MovementTransitions/MovementTransitions_";
		tukk MOVEMENTTRANSITIONS_FILENAME_SUFFIX = ".xml";

		DrxFixedStringT<128> sResult;
...
struct STransitionSelectionParams
{
	...
	u32		m_context; // movement context 'flags' passed along by AI
...
public:
	...
	u32		context;											// bit flags describing the 'context' which goes beyond stance (could be describing things like "going into cover"). Default value = 0.
...
private:
	...
	bool ReadStanceParam( tukk name, bool required, const struct IItemParamsNode*const pParams, EStance*const pStance ) const;
	bool ReadPseudoSpeedParam( tukk name, bool required, const struct IItemParamsNode*const pParams, float*const pPseudoSpeed ) const;
	bool ReadIntParam( tukk name, bool required, i32 min, i32 max, const struct IItemParamsNode*const pParams, i32*const pI ) const;
	bool ReadFloatParam( tukk name, bool required, float min, float max, const struct IItemParamsNode*const pParams, float*const pF ) const;
	bool ReadAngleParam( tukk name, bool required, float min, float max, const struct IItemParamsNode*const pParams, float*const pAngle ) const;
};
