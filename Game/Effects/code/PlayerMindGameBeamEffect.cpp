...
#include <drx3D/Game/PlayerMindGameBeamEffect.h>
...
//
void CPlayerMindGameBeamEffectSimpleEffect::SetParticleEffectName(tukk particleFileName)
{
	...
	IParticleEffect* particleEffect = gEnv->pParticleUpr->FindEffect(particleFileName, "CEnergyBeamsBetweenPlayerAndEntitySimpleEffect::SetParticleEffect");
	if (particleEffect == NULL)
	{
		DrxLog("CPlayerMindGameBeamEffectSimpleEffect::SetParticleEffect(): Unable to load particle effect '%s'!", particleFileName);
		return;
	}
...
void CPlayerMindGameBeamEffectLinkedEffect::BetweenPoints(const Vec3& startPos, const Vec3& endPos)
{
	...
	if (m_LightningID == -1)
	{
		...
		m_LightningID = lightningGameEffect->TriggerSpark(
			...
			gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(m_EffectConfig->m_MaterialName),
		...
		m_RestartDelay = drx_random(m_EffectConfig->m_MinRestartDelay, m_EffectConfig->m_MaxRestartDelay);
	}
	else
	{
...
void CPlayerMindGameBeamEffect::Setup(
	tukk playersEffectName, 
	tukk targetEntityEffectName, 
	const CPlayerMindGameBeamEffectLinkedEffectConfig* linkedEffectConfig,
	tukk leftBossConnectAttachmentName,
	tukk rightBossConnectAttachmentName)
{
	...
	for (i32 beamIndex = 0 ; beamIndex < BeamsAmount ; beamIndex++)
	{
...
bool CPlayerMindGameBeamEffect::RetrieveStartPositionOnPlayer(Vec3* leftStartPosOnPlayer, Vec3* rightStartPosOnPlayer)
{
	...
	case ConnectionPositionOnPlayer_Hands:
		...
		{
			DRX_ASSERT_MESSAGE(false, "CPlayerMindGameBeamEffect::RetrieveStartPositionOnPlayer(): Failed to obtain player's hand position in world-space.");
			return false;
		}
		return true;
	...
	case ConnectionPositionOnPlayer_WaistArea:
		...
		{
			DRX_ASSERT_MESSAGE(false, "CPlayerMindGameBeamEffect::RetrieveStartPositionOnPlayer(): Failed to obtain player's waist position in world-space.");
			return false;
		}
		return true;
	...
	Vec3* playerLeftHandPos, Vec3* playerRightHandPos) const
{
	...
	i16k leftHandJointID = rIDefaultSkeleton.GetJointIDByName(s_PlayerLeftHandJointName);
	IF_UNLIKELY (leftHandJointID < 0)
	{
		DrxLog("CPlayerMindGameBeamEffect::RetrievePlayerHandPositions(): Unable to find joint '%s'.", s_PlayerLeftHandJointName);
		return false;
	}
	...
	i16k rightHandJointID = rIDefaultSkeleton.GetJointIDByName(s_PlayerRightHandJointName);
	IF_UNLIKELY (rightHandJointID < 0)
	{
		DrxLog("CPlayerMindGameBeamEffect::RetrievePlayerHandPositions(): Unable to find joint '%s'.", s_PlayerRightHandJointName);
		return false;
	}
	...
	Vec3* targetLeftAttachmentPos, Vec3* targetRightAttachmentPos) const
{
	...
	IAttachmentUpr* attachmentUpr = characterInstance->GetIAttachmentUpr();
	IF_UNLIKELY (attachmentUpr == NULL)
	{
		return false;
	}

	IAttachment* leftAttachment = attachmentUpr->GetInterfaceByName(m_LeftConnectionAttachmentName.c_str());
	IF_UNLIKELY (leftAttachment == NULL)
	{
		DrxLog("CPlayerMindGameBeamEffect::RetrieveTargetAttachmentPositions(): Unable to find attachment '%s'.", m_LeftConnectionAttachmentName.c_str());
		return false;
	}
	const QuatTS& leftPose = leftAttachment->GetAttWorldAbsolute();
	*targetLeftAttachmentPos = leftPose.t;

	IAttachment* rightAttachment = attachmentUpr->GetInterfaceByName(m_RightConnectionAttachmentName.c_str());
	IF_UNLIKELY (rightAttachment == NULL)
	{
		DrxLog("CPlayerMindGameBeamEffect::RetrieveTargetAttachmentPositions(): Unable to find attachment '%s'.", m_RightConnectionAttachmentName.c_str());
		return false;
	}
