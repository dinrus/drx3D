...
#include <drx3D/Game/KillCamGameEffect.h>
#include <drx3D/Game/ItemParams.h>
#include <drx3D/Game/Effects/Tools/CVarActivationSystem.h>
#include <drx3D/Game/Effects/Tools/PostEffectActivationSystem.h>
#include <drx3D/Game/Network/Lobby/GameLobby.h>
#include <drx3D/Game/UI/UIUpr.h>
#include <drx3D/Game/UI/UIHUD3D.h>
...
// Desc: Static data
//--------------------------------------------------------------------------------------------------
tukk KILL_CAM_GAME_EFFECT_NAME = "Kill Cam";
tukk KILL_CAM_GAME_EFFECT_MODE_NAMES[] = 
{
	"KillCam",
	"KillerCam",
	"IntroCam"
...
void CKillCamGameEffect::Initialise(const SGameEffectParams* gameEffectParams)
{
	...
	for( i32 i=0; i<eKCEM_TotalModes; i++ )
	{
...
//--------------------------------------------------------------------------------------------------
tukk CKillCamGameEffect::GetName() const 
{ 
...
void CKillCamGameEffect::LoadStaticData(IItemParamsNode* rootNode)
{
	...
	tukk  pLevelName = pGameLobby ? pGameLobby->GetCurrentLevelName() : "";
	i32k children = rootNode->GetChildCount();
	for( i32 m=0; m<eKCEM_TotalModes; m++ )
	{
		for( i32 i=0; i<children; i++ )
		{
			...
			if(!stricmp(paramNode->GetName(), KILL_CAM_GAME_EFFECT_MODE_NAMES[m]))
			{
				if(pLevelName)
				{
					if(tukk pSpecificLevel = paramNode->GetAttribute("level"))
					{
...
void CKillCamGameEffect::LoadStaticModeData( const IItemParamsNode* paramNode, SEffectData& data )
{
	if(paramNode)
	{
		...
		{
			i32k childCount = pPostEffectsNode->GetChildCount();
