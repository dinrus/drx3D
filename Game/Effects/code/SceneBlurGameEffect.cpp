...
#include <drx3D/Game/SceneBlurGameEffect.h>
...
tukk SCENEBLUR_GAME_EFFECT_NAME = "SceneBlur";
...
struct SSceneBlurGameEffectData
{
	...
	typedef DrxFixedArray<SEffectBlendParams, NUMBLENDEDSCENEBLURUSAGES> TSceneBlurBlendedEffectData; // Could use eGameEffectUsage_NUMTYPES but uses extra memory which won't be used for non blended effects

	SSceneBlurGameEffectData()
	{
		for (i32 i = 0; i < NUMBLENDEDSCENEBLURUSAGES; i++)
		{
...
{
	for (i32 i = 0; i < eGameEffectUsage_NUMTYPES; i++)
	{
...
//--------------------------------------------------------------------------------------------------
tukk CSceneBlurGameEffect::GetName() const
{
...
void CSceneBlurGameEffect::Update(float frameTime)
{
	...
	for (i32 i = NUMBLENDEDSCENEBLURUSAGES; i < eGameEffectUsage_NUMTYPES; i++)
	{
		SGameEffectExecutionData& executionData = m_executionData[i];
...
{
	DRX_ASSERT(usage >= 0 && usage < eGameEffectUsage_NUMTYPES);
...
//--------------------------------------------------------------------------------------------------
void CSceneBlurGameEffect::DebugOnInputEvent(i32 keyId)
{
	if(s_sceneBlurGEData.isInitialised)
	{
