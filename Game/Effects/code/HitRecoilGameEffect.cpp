...
#include <drx3D/Game/HitRecoilGameEffect.h>
...
namespace
{
	...
	{
	public:
		static i32k maxNumEntries = 16;
		static i32k stringSize = 16;

		typedef DrxFixedStringT<stringSize> TString;
		typedef DrxFixedArray<TString, maxNumEntries> TStringArray;

	public:

		i32 GetNameId(const string& name)
		{
		...
		DrxFixedStringT<stringSize> GetNameFromId(i32 id)
		{
			if (id < 0 || id >= (i32)m_nameRegistration.size())
				return "";
...
void CHitRecoilGameEffect::AddHit(CPlayer* pPlayer, IEntityClass *pProjectileClass, float damage, i32 damageTypeId, const Vec3& damageDirection)
{
...
i32 CHitRecoilGameEffect::GetHitRecoilId(const string& name)
{
...
void CHitRecoilGameEffect::Reset(const IItemParamsNode* pRootNode)
{
	...
	for (i32 i = 0; i < pParams->GetChildCount(); ++i)
	{
		...
		if(pGameRules)
		{
			i32 hitTypeId = pGameRules->GetHitTypeId(name.c_str());
			if(hitTypeId != 0)
			{
