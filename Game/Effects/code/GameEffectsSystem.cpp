...
#include <drx3D/Game/GameEffectsSystem.h>
...
#include <drx3D/Game/Effects/RenderNodes/IGameRenderNode.h>
...
//--------------------------------------------------------------------------------------------------
	...
	i32	CGameEffectsSystem::s_currentDebugEffectId = 0;

	tukk GAME_FX_DEBUG_VIEW_NAMES[eMAX_GAME_FX_DEBUG_VIEWS] = {	"None",
																																			"Profiling",
																																			"Effect List",
																																			"Bounding Box",
																																			"Bounding Sphere",
...
i32									CGameEffectsSystem::s_postEffectCVarNameOffset = 0;
//--------------------------------------------------------------------------------------------------
...
void CGameEffectsSystem::LoadData()
{
	if(s_hasLoadedData == false)
	{
		...
		if(paramNode)
		{
			...
			i32 callbackCount = s_dataLoadCallbackList.size();
			DataLoadCallback dataLoadCallbackFunc = NULL;
			for(i32 i=0; i<callbackCount; i++)
			{
...
void CGameEffectsSystem::ReleaseData()
{
	if(s_hasLoadedData)
	{
		...
		// Pass data to any callback functions registered
		i32 callbackCount = s_dataReleaseCallbackList.size();
...
void CGameEffectsSystem::ReloadData()
{
	...
	if(paramNode)
	{
		...
		// Pass data to any callback functions registered
		i32 callbackCount = s_dataReloadCallbackList.size();
...
{
	i32k callbackCount = s_enteredGameCallbackList.size();
...
void CGameEffectsSystem::SetPostEffectCVarCallbacks()
{
	...
	i32 postEffectNameCount = sizeof(postEffectNames)/sizeof(*postEffectNames);

	if(postEffectNameCount > 0)
	{
		// Calc name offset
		tukk postEffectName = postEffectNames[0];
		s_postEffectCVarNameOffset = 0;
		...
		for(i32 i=0; i<postEffectNameCount; i++)
		{
...
{
	tukk effectName = cvar->GetName() + s_postEffectCVarNameOffset;
...
void CGameEffectsSystem::DrawDebugDisplay()
{
	...
	i32 debugEffectCount = s_effectDebugList.Size();
	if((g_pGameCVars->g_gameFXSystemDebug) && (debugEffectCount > 0))
	{
		...
		if(m_debugView==eGAME_FX_DEBUG_VIEW_EffectList)
		{
			...
			tukk flagName[FLAG_COUNT] = {"Init","Rel","ARels","ADels","AUWA","AUWnA","Reg","Actv","DBG"};
			i32k flag[FLAG_COUNT] = {GAME_EFFECT_INITIALISED,
			...
			for(i32 l=0; l<EFFECT_LIST_COUNT; l++)
			{
	...
	if((g_pGameCVars->g_gameFXSystemDebug) && (debugEffectCount > 0))
	{
		if(inputEvent.deviceType == eIDT_Keyboard && inputEvent.state == eIS_Pressed)
		{
			switch(inputEvent.keyId)
			{
				...
				case GAME_FX_INPUT_ResetParticleUpr:
				{
					gEnv->pParticleUpr->Reset();
					break;
				}
				case GAME_FX_INPUT_PauseParticleUpr:
				{
#if DEBUG_GAME_FX_SYSTEM
					ICVar* pParticleDebugCVar = gEnv->pConsole->GetCVar("e_ParticlesDebug");
					if(pParticleDebugCVar)
					{
						i32 flagValue = pParticleDebugCVar->GetIVal();
...
//--------------------------------------------------------------------------------------------------
#if DEBUG_GAME_FX_SYSTEM
IGameEffect* CGameEffectsSystem::GetDebugEffect(tukk pEffectName) const
{
	i32k EFFECT_LIST_COUNT = 2;
	IGameEffect* pEffectListArray[EFFECT_LIST_COUNT] = {m_effectsToUpdate,m_effectsNotToUpdate};
...
//--------------------------------------------------------------------------------------------------
#if DEBUG_GAME_FX_SYSTEM
void CGameEffectsSystem::OnActivateDebugView(i32 debugView)
{
	switch(debugView)
	{
...
//--------------------------------------------------------------------------------------------------
#if DEBUG_GAME_FX_SYSTEM
void CGameEffectsSystem::OnDeActivateDebugView(i32 debugView)
{
	switch(debugView)
	{
		case eGAME_FX_DEBUG_VIEW_Profiling:
