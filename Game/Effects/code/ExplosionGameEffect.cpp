...
#include <drx3D/Game/ExplosionGameEffect.h>
...
{
	for (u32 i = 0; i < m_queuedRays.size(); ++i)
	{
...
{
	i32k queuedRayIdx = FindRequestedRay(rayID);

	DRX_ASSERT(queuedRayIdx != -1);
...
void CDeferredExplosionEffect::RequestRayCast( CDeferredExplosionEffect::EDeferredEffectType effectType, const Vec3 &startPos, const Vec3 &dir, float distance, float effectMaxDistance, i32 objTypes, i32 flags, IPhysicalEntity **pSkipEnts, i32 nSkipEnts )
{
...
i32 CDeferredExplosionEffect::FindRequestedRay( const QueuedRayID& rayID ) const
{
	i32k queuedCount = m_queuedRays.size();
	for (i32 i = 0; i < queuedCount; ++i)
	{
...
{
	DRX_ASSERT(maxBlurDistance > 0.0f);
...
void CExplosionGameEffect::SpawnScreenExplosionEffect(const SExplosionContainer &explosionContainer)
{
	...
	if(pClientActor != NULL && !pClientActor->IsDead())
	{
		...
		{		
			...
			{
				i32k intersectionObjTypes = ent_static | ent_terrain;
				u32k intersectionFlags = rwi_stop_at_pierceable|rwi_colltype_any;
			...
			|| (inFOV && (dist < explosionInfo.radius))))
		{
			...
			i32k intersectionObjTypes = ent_static | ent_terrain;
			u32k intersectionFlags = rwi_stop_at_pierceable|rwi_colltype_any;
			i32k intersectionMaxHits = 1;

			i32 collision = gEnv->pPhysicalWorld->RayWorldIntersection(	explosionInfo.pos, 
																																	-eyeToExplosion*dist, 
			...
			if(!collision)
			{
				...
				if(enabled)
				{
					...
					DRX_ASSERT_MESSAGE(pClientActor->IsPlayer(),"Effect shouldn't be spawned if not a player");
		...
		else if(inFOV && (dist < explosionInfo.radius))
		{
			if(explosionInfo.damage>10.0f || explosionInfo.pressure>100.0f)
			{
				...
				if (pClientActor)
				{
					static_cast<CActor*>(pClientActor)->AddAngularImpulse(Ang3(drx_random(-angleX*0.5f,angleX),0.0f,drx_random(-angleZ,angleZ)),0.0f,dt*2.0f);
				}
			}
		}
	}
...
void CExplosionGameEffect::QueueMaterialEffect(SExplosionContainer &explosionContainer)
{
	...
	i32k intersectionObjTypes = ent_all|ent_water;    
	u32k intersectionFlags = rwi_stop_at_pierceable|rwi_colltype_any;

	DRX_ASSERT(explosionContainer.m_mfxInfo.m_rayId == 0);
...
//--------------------------------------------------------------------------------------------------
void CExplosionGameEffect::DebugOnInputEvent(i32 keyId)
{
	...
	switch(keyId)
	{
		...
		case eKI_NP_1:
		{
			...
			ExplosionInfo& explosionInfo = explosionContainer.m_explosionInfo;
			explosionInfo.pParticleEffect = gEnv->pParticleUpr->FindEffect("Crysis2_weapon_explosives.frag.concrete");
		...
		case eKI_NP_2:
		{
			...
			SExplosionContainer explosionContainer;
			ExplosionInfo& explosionInfo = explosionContainer.m_explosionInfo;
			explosionInfo.pParticleEffect = gEnv->pParticleUpr->FindEffect("Crysis2_weapon_explosives.grenades.flash_explosion");
			explosionInfo.pos = playerPos + (playerDir * distFromPlayer);
			explosionInfo.dir.Set(0.0f,-1.0f,0.0f);
			explosionInfo.effect_scale = 1.0f;
			explosionInfo.pressure = 1000.0f;
		...
		case eKI_NP_3:
		{
			...
			SExplosionContainer explosionContainer;
			ExplosionInfo& explosionInfo = explosionContainer.m_explosionInfo;
			explosionInfo.pParticleEffect = gEnv->pParticleUpr->FindEffect("Crysis2_weapon_fx.l-tag.rico_explosion");
			explosionInfo.pos = playerPos + (playerDir * distFromPlayer);
			explosionInfo.dir.Set(0.0f,-1.0f,0.0f);
			explosionInfo.effect_scale = 1.0f;
			explosionInfo.pressure = 1000.0f;
