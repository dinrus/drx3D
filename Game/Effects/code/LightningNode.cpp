...
#include <drx3D/Game/LightningNode.h>
...
{
	i32 numTris = m_vertices.size() - m_firstVertex - 2;
	if (numTris < 0)
		numTris = 0;
	i32 vidx = m_vertices.size();
	if (numTris > 0)
	{
		if (numTris % 2)
		{
...
void CLightningRenderNode::CTriStrip::AddStats(SLightningStats* pStats) const
{
	...
	pStats->m_memory.Increment(m_indices.capacity() * sizeof(u16));
...
void CLightningRenderNode::CSegment::Create(const SLightningParams& desc, SPointData* m_pointData, i32 _parentSegmentIdx, i32 _parentPointIdx, Vec3 _origin, Vec3 _destany, float _duration, float _intensity)
{
	...
	i32k numFuzzySegs = numSegs*numSubSegs;

	m_pointData->m_points.push_back(Vec3(ZERO));
	m_pointData->m_velocity.push_back(Vec3(ZERO));
	for (i32 i = 1; i < numSegs; ++i)
	{
		m_pointData->m_points.push_back(drx_random_unit_vector<Vec3>());
		m_pointData->m_velocity.push_back(drx_random_unit_vector<Vec3>() + Vec3(0.0f, 0.0f, 1.0f));
	}
	m_pointData->m_points.push_back(Vec3(ZERO));
	m_pointData->m_velocity.push_back(Vec3(ZERO));

	m_pointData->m_fuzzyPoints.push_back(Vec3(ZERO));
	for (i32 i = 1; i < numFuzzySegs; ++i)
	{
		m_pointData->m_fuzzyPoints.push_back(drx_random_unit_vector<Vec3>());
	}
	m_pointData->m_fuzzyPoints.push_back(Vec3(ZERO));
...
void CLightningRenderNode::CSegment::Draw(const SLightningParams& desc, const SPointData& pointData, CTriStrip* strip, Vec3 cameraPosition, float deviationMult)
{
	...
	for (i32 i = 1; i < m_numFuzzyPoints-1; ++i)
	{
		p0 = p1;
		p1 = p2;
...
tukk CLightningRenderNode::GetEntityClassName() const
{
	return "Lightning";
}
...
tukk CLightningRenderNode::GetName() const
{
	return "Lightning";
}
...
void CLightningRenderNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
}
...
float CLightningRenderNode::TriggerSpark()
{
	...
	float lightningTime = drx_random(
...
void CLightningRenderNode::CreateSegment(Vec3 originPosition, i32 parentIdx, i32 parentPointIdx, float duration, i32 level)
{
	if (level > m_pLightningDesc->m_branchMaxLevel)
		return;
	if (m_segments.size() == m_pLightningDesc->m_maxNumStrikes)
		return;
	i32 segmentIdx = m_segments.size();
	...
	{
		i32 point = drx_random(0, m_pLightningDesc->m_strikeNumSegments * m_pLightningDesc->m_strikeNumPoints - 1);
		CreateSegment(
