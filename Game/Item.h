...
#include <drx3D/Animation/IDrxMannequinEditor.h>
...
public:
	...
	typedef DrxFixedStringT<32> TempAGInputName;
	...
	typedef DrxFixedArray<SAccessoryInfo, ITEM_MAX_NUM_ACCESSORIES>	TAccessoryArray;
...
public:
	...
	static tukk GetWeaponComponentType();
	virtual tukk GetType() const;
  ...
  virtual void ResetOwner();
	...
	virtual void SetHand(i32 hand);
	void SwitchToHand(i32 hand);
	...
	void CloakEnable(bool enable, bool fade, float cloakBlendSpeedScale = 1.0f, bool bFadeByDistance = false, u8 colorChannel = 0, bool bIgnoreCloakRefractionColor = false, EntityId syncedFromId = 0 );
  ...
  virtual void OnReset();
	...
	// accessories
	virtual void RemoveAccessory(const ItemString& name) { DRX_ASSERT_MESSAGE(0, "DEPRECATED: Use RemoveAccessory(const IEntityClass* pClass)"); };
	...
	void ShowAttachmentHelper(i32 slot, tukk name, bool show);
	...
	virtual void AddAccessoryAmmoToInventory(IEntityClass* pAmmoType, i32 count, IInventory* pOwnerInventory);
	...
	// effects
	EntityEffects::TAttachedEffectId AttachEffect(i32 slot,  
																								bool attachToAccessory = false, 
																								tukk effectName = NULL, 
																								tukk helper = NULL,
  ...
  EntityEffects::TAttachedEffectId AttachLight(i32 slot, tukk helper, const EntityEffects::SLightAttachParams& lightParams);
	...
	void EnableUpdate(bool enable, i32 slot=-1);
	void RequireUpdate(i32 slot=-1);
  ...
  IInventory *GetActorInventory(IActor *pActor) const;
	...
	virtual void SetViewMode(i32 mode);
  ...
  virtual void UpdateIKMounted(IActor* pActor, const Vec3& vGunXAxis);
	...
	bool CreateCharacterAttachment(i32 slot, tukk name, i32 type, tukk bone);
	void DestroyCharacterAttachment(i32 slot, tukk name);
	ICharacterInstance* GetAppropriateCharacter(i32 slot, bool owner);
	void ResetCharacterAttachment(i32 slot, tukk name, bool owner, EntityId attachedEntId = 0);
	IMaterial* GetCharacterAttachmentMaterial(i32 slot, tukk name, bool owner = false);
	tukk GetCharacterAttachmentBone(i32 slot, tukk name);
	void SetCharacterAttachment(i32 slot, tukk name, IEntity *pEntity, bool owner);
	void SetCharacterAttachment(i32 slot, tukk name, IStatObj *pObj, bool owner);
	void SetCharacterAttachment(i32 slot, tukk name, ICharacterInstance *pCharacter, bool owner);
	void SetCharacterAttachment(i32 slot, tukk name, IEntity *pEntity, i32 objSlot, bool owner);
	virtual void SetCharacterAttachmentLocalTM(i32 slot, tukk name, const Matrix34 &tm);
	void SetCharacterAttachmentWorldTM(i32 slot, tukk name, const Matrix34 &tm);
	Matrix34 GetCharacterAttachmentLocalTM(i32 slot, tukk name);
	Matrix34 GetCharacterAttachmentWorldTM(i32 slot, tukk name);
	void HideCharacterAttachment(i32 slot, tukk name, bool hide);
	void HideCharacterAttachmentMaster(i32 slot, tukk name, bool hide);

	void CreateAttachmentHelpers(i32 slot);
	void DestroyAttachmentHelpers(i32 slot);
	const THelperVector& GetAttachmentHelpers();

  // damage
  virtual void OnHit(float damage, i32 hitType);
  ...
  virtual void OnRepaired();
	...
	// resource
	virtual bool SetGeometry(i32 slot, const ItemString& name, const ItemString& material=ItemString(), bool useParentMaterial=false, const Vec3& poffset=Vec3(0,0,0), const Ang3& aoffset=Ang3(0,0,0), float scale=1.0f, bool forceReload=false);

	_smart_ptr<IAction> PlayAction(FragmentID action, i32 layer=0, bool loop=false, u32 flags = eIPAF_Default, float speedOverride = -1.0f, float animWeigth = 1.0f, float ffeedbackWeight = 1.0f);
	...
	i32 GetFragmentID(tukk actionName, const CTagDefinition** pTagDef = NULL);
	virtual void SetFragmentTags(CTagState& fragTags) {};

	u32 GetCurrentAnimationTime(i32 slot);
	void DrawSlot(i32 slot, bool draw, bool near=false);
	Vec3 GetSlotHelperPos(i32 slot, tukk helper, bool worldSpace, bool relative=false) const;
	const Matrix33 &GetSlotHelperRotation(i32 slot, tukk helper, bool worldSpace, bool relative=false);
	void Quiet();
	...
	struct AccessoryParams
	{
		...
		AccessoryParams(u16 _accessoryClassId): accessoryClassId(_accessoryClassId) {};
		void SerializeWith(TSerialize ser)
		{
			ser.Value("accessoryClassId", accessoryClassId, 'clas');
		};

		u16 accessoryClassId;
	};
	...
	// properties
	template<typename T>bool GetEntityProperty(tukk name, T &value) const
	{
		return EntityScripts::GetEntityProperty(GetEntity(), name, value);
	}
	template<typename T>bool GetEntityProperty(tukk table, tukk name, T &value) const
	{
    return EntityScripts::GetEntityProperty(GetEntity(), table, name, value);
	}

  template<typename T>void SetEntityProperty(tukk name, const T &value)
  {
	...
	virtual void SetSubContextID(i32 subContext)
	{
		m_subContext = subContext;
	}
	virtual i32 GetSubContextID()
	{
		return m_subContext;
	}
	
	virtual const string GetAttachedAccessoriesString(tukk separator = ",");
...
protected:
	...
	IAttachment * GetCharacterAttachment(ICharacterInstance * pCharacter, tukk name) const;
	...
	void AudioCacheItemAndAccessories(const bool enable, tukk type);
public:
	static void AudioCacheItem(const bool enable, const IEntityClass* pClass, tukk prefix, tukk postfix);

protected:
	bool IsAudioCached(const IEntityClass *pClass, tukk type);

	IAttachmentUpr *GetOwnerAttachmentUpr() const;
	...
	void UpdateScopeContexts(IActionController *pActionController, i32 nCharacterSlot = 0);
	...
	i32										m_delayedUnhideCntr;

	u16								m_attachedAccessoryHistory;
