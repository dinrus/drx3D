...
struct SSessionStats
{
	...
	struct SMap
	{
		typedef VectorMap<DrxFixedStringT<64>, i32, std::less<DrxFixedStringT<64> >, stl::STLGlobalAllocator<std::pair<DrxFixedStringT<64>, i32 > > > MapNameToCount;
		MapNameToCount m_map;

		SMap();

		void Update(tukk name, i32 amount = 1);
		void Clear();

		MapNameToCount::iterator FindOrInsert(tukk name);

		void SetStat(tukk name, i32 amount);
		i32 GetStat(tukk name) const;
		i32 GetTotal() const;
	...
	ILINE i32 GetStat(EIntPersistantStats stat) const
	{
		DRX_ASSERT(stat >= 0 && stat < DRX_ARRAY_COUNT(m_intStats));
		return m_intStats[ stat ];
	}
	ILINE float GetStat(EFloatPersistantStats stat) const
	{
		DRX_ASSERT(stat >= 0 && stat < DRX_ARRAY_COUNT(m_floatStats));
		return m_floatStats[ stat ];
	}
	ILINE i32 GetStat(EStreakIntPersistantStats stat) const
	{
		DRX_ASSERT(stat >= 0 && stat < DRX_ARRAY_COUNT(m_streakIntStats));
		return m_streakIntStats[ stat ].m_maxVal;
	}
	ILINE float GetStat(EStreakFloatPersistantStats stat) const
	{
		DRX_ASSERT(stat >= 0 && stat < DRX_ARRAY_COUNT(m_streakFloatStats));
		return m_streakFloatStats[ stat ].m_maxVal;
	}

	ILINE void SetStat( EIntPersistantStats stat, i32 newValue )
	{
	...
	ILINE void SetStat( EStreakIntPersistantStats stat, i32 newValue )
	{
	...
	ILINE i32 GetMaxThisSessionStat(EStreakIntPersistantStats stat) const
	{
		DRX_ASSERT(stat >= 0 && stat < DRX_ARRAY_COUNT(m_streakIntStats));
	...
	i32 GetStat(tukk name, EMapPersistantStats) const;
	const SMap::MapNameToCount& GetStat(EMapPersistantStats) const;
	i32 GetDerivedStat(EDerivedIntPersistantStats stat) const;
	float GetDerivedStat(EDerivedFloatPersistantStats stat) const;
	i32 GetDerivedStat(tukk name, EDerivedIntMapPersistantStats) const;
	float GetDerivedStat(tukk name, EDerivedFloatMapPersistantStats) const;
	tukk GetDerivedStat(EDerivedStringPersistantStats);
	tukk GetDerivedStat(EDerivedStringMapPersistantStats);

	float GetStatStrings(EIntPersistantStats stat, DrxFixedStringT<64>& valueString);
	float GetStatStrings(EFloatPersistantStats stat, DrxFixedStringT<64>& valueString);
	float GetStatStrings(EStreakIntPersistantStats stat, DrxFixedStringT<64>& valueString);
	float GetStatStrings(EStreakFloatPersistantStats stat, DrxFixedStringT<64>& valueString);

	float GetStatStrings(EDerivedIntPersistantStats stat, DrxFixedStringT<64>& valueString);
	float GetStatStrings(EDerivedFloatPersistantStats stat, DrxFixedStringT<64>& valueString);
	void GetStatStrings(EDerivedStringPersistantStats stat, DrxFixedStringT<64>& valueString);
	void GetStatStrings(EDerivedStringMapPersistantStats stat, DrxFixedStringT<64>& valueString);
	float GetStatStrings(tukk name, EDerivedIntMapPersistantStats stat, DrxFixedStringT<64>& valueString);
	float GetStatStrings(tukk name, EDerivedFloatMapPersistantStats stat, DrxFixedStringT<64>& valueString);
	float GetStatStrings(tukk name, EMapPersistantStats stat, DrxFixedStringT<64>& valueString);
	float GetStatStrings(EMapPersistantStats stat, DrxFixedStringT<64>& valueString);

	float GetStatStringsFromValue(EMapPersistantStats stat, i32 statValue, DrxFixedStringT<64>& valueString);

	i32 m_intStats[EIPS_Max];
	float m_floatStats[EFPS_Max];
	SStreak<i32> m_streakIntStats[ESIPS_Max];
	SStreak<float> m_streakFloatStats[ESFPS_Max];
	SMap m_mapStats[EMPS_Max];

	DrxFixedStringT<DRXLOBBY_USER_GUID_STRING_LENGTH> m_guid;
	float m_lastKillTime;

	DrxFixedStringT<32> m_dummyStr;

	// Just used by history
	u32 m_xpHistoryDelta;
};
