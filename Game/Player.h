...
/*************************************************************************
 -------------------------------------------------------------------------
  $Id$
  $DateTime$
  Описание: Implements the player.
...
#include <drx3D/Game/SpectacularKill.h>
#include <drx3D/Game/LargeObjectInteraction.h>
#include <drx3D/Game/PlayerPlugin_CurrentlyTargetting.h>
#include <drx3D/Game/PlayerModifiableValues.h>
#include <drx3D/Game/Effects/HitRecoilGameEffect.h>
#include <drx3D/Game/Effects/PlayerHealthEffect.h>
#include <drx3D/Game/HitDeathReactionsDefs.h>
#include <drx3D/Game/LookAim_Helper.h>
#include <drx3D/Game/DualCharacterProxy.h>
#include <drx3D/Game/MountedGunController.h>
#include <drx3D/Game/Item.h>
#include <drx3D/Game/PlayerStateSwim_WaterTestProxy.h>
...
struct SSpectatorInfo
{
	...
	u8								mode;  // see CActor::EActorSpectatorMode enum
	u8								state; // see CActor::EActorSpectatorState enum
...
struct SPlayerStats : public SActorStats
{
	...
	u8 flyMode;//0 no fly, 1 fly mode, 2 fly mode + noclip
#endif
	u8 cinematicFlags;
	u8 isAnimatedSlave;	// Don't serialize. Local.

	CCoherentValue<u8> followCharacterHead;
...
struct SPlayerRotationParams
{
	...
	{
		DRX_ASSERT((aimType >= 0) && (aimType < EAimType_TOTAL));
...
private:
	void ReadAimParams(const IItemParamsNode* pRootNode, tukk aimTypeName, EAimType aimType, bool firstPerson);
...
struct SXPEvents
{
	...
	SEvent				events[15];
	i32						numEvents;			// sent as ui4 - hence max is 15
...
public:
	...
	void	GetValues(i32* outXp, i32* outRank, i32* outDefault, i32* outStealth, i32* outArmour, i32* outReincarnations);
...
private:
	...
	void SetSerializedValues(i32 newXp, i32 newRank, i32 newDefault, i32 newStealth, i32 newArmour, i32 newReincarnations);
private:
	SSerVals  m_serVals;
...
public:
	...
	static i32k MAX_NETWORKED_LEDGE_COUNT = 1024;
	static i32k JUMP_COUNTER_MAX  = 8;
	...
	virtual void OnFootStepAnimEvent(ICharacterInstance* pCharacter, tukk boneName);
	virtual void OnFoleyAnimEvent(ICharacterInstance* pCharacter, tukk CustomParameter, tukk boneName);
	void OnSwimmingStrokeAnimEvent(ICharacterInstance* pCharacter);
	void ExecuteFootStep(ICharacterInstance* pCharacter, const float frameTime, i32k nFootJointID);
	void ExecuteFoleySignal(ICharacterInstance* pCharacter, const float frameTime, tukk CustomParameter, i32k nBoneJointID);
	...
	void ExecuteGroundEffectAnimEvent(ICharacterInstance* pCharacter, const float frameTime, tukk szEffectName, i32k nJointID);
	...
	virtual bool SetActorModel(tukk modelName = NULL) override;
	...
	virtual void StartInteractiveAction(EntityId entityId, i32 interactionIndex = 0) override;
	virtual void StartInteractiveActionByName(tukk interaction, bool bUpdateVisibility, float actionSpeed = 1.0f) override;
	...
	void AddAmmoToInventory(IInventory* pInventory, IEntityClass* pAmmoClass, IWeapon* pWeapon, i32 totalCount, i32 totalCapacity, i32 increase);
	void RefillAmmo();
	void RefillAmmoDone();
	...
	virtual i32 GetPhysicalSkipEntities(IPhysicalEntity** pSkipList, i32k maxSkipSize) const override;
	...
	const QuatT& GetAnimationRelativeMovement(i32 slot = 0) const;
	...
	virtual tukk GetActorClassName() const override;

#ifdef STATE_DEBUG
	static void DebugStateMachineEntity( tukk pName );
#endif
	...
	ILINE i32 GetLastInteractionIndex() const { return static_cast<i32>(m_lastCachedInteractionIndex); }
	ILINE void SetLastInteractionIndex(i32 interactionIndex) { m_lastCachedInteractionIndex = static_cast<int8>(interactionIndex); }
  ...
  virtual void DisableStumbling() override;
	...
	ILINE bool CanHandFire(i32 hand) const
	{
...
protected:
	...
	void UpdatePlayerCinematicStatus(u8 oldFlags, u8 newFlags);
	void ResetCinematicStatus(u8 oldFlags);
...
public:
	...
	struct SIntStatParams
	{
		...
		SIntStatParams(i32 stat) : m_stat(stat) {}
	...
	static void RMIBenchmarkCallback( ERMIBenchmarkLogPoint point0, ERMIBenchmarkLogPoint point1, int64 delay, uk pUserData );
	...
	virtual void RequestFacialExpression(tukk pExpressionName /* = NULL */, f32* sequenceLength = NULL) override;
	...
	virtual void SaveToProfile(IPlayerProfile* pProfile, bool online, u32 reason) override;
	virtual void LoadFromProfile(IPlayerProfile* pProfile, bool online, u32 reason) override;
	//~IPlayerProfileListener
...
#if !defined(_RELEASE) || defined(PERFORMANCE_BUILD)
	virtual void SetFlyMode(u8 flyMode) override;
	virtual u8 GetFlyMode() const override { return m_stats.flyMode; }
#else
	virtual u8 GetFlyMode() const override { return 0; }
#endif

	virtual void SetSpectatorState(u8 state) override;
	virtual EActorSpectatorState GetSpectatorState() const override { return (EActorSpectatorState)m_stats.spectatorInfo.state; }

	virtual void SetSpectatorModeAndOtherEntId(u8k _mode, const EntityId _othEntId, bool isSpawning=false) override;

	virtual u8 GetSpectatorMode() const override { return m_stats.spectatorInfo.mode; }
	...
	ILINE void SetRecentKiller(EntityId killerEid, i32 damageType) { m_stats.recentKiller = killerEid; m_stats.killedByDamageType = damageType; }

	EntityId GetLastAttacker() { return m_stats.lastAttacker; }

	virtual void SelectNextItem(i32 direction, bool keepHistory, i32 category) override;
	...
	virtual void SelectItemByName(tukk name, bool keepHistory, bool forceFastSelect=false) override;
	virtual void SelectItem(EntityId itemId, bool keepHistory, bool forceSelect) override;
	virtual bool ScheduleItemSwitch(EntityId itemId, bool keepHistory, i32 category = 0, bool forceFastSelect=false) override;
	...
	virtual bool SetAspectProfile(EEntityAspects aspect, u8 profile ) override;
	...
	virtual void CameraShake(float angle,float shift,float duration,float frequency,Vec3 pos,i32 ID,tukk source="") override;
	...
	virtual void PlayAction(tukk action,tukk extension, bool looping=false) override;
  ...
  virtual void SetForceLookAtTargetId(EntityId targetId, float interpolationTime=1.f) override;
	...
	virtual void DamageInfo(EntityId shooterID, EntityId weaponID, IEntityClass *pProjectileClass, float damage, i32 damageType, const Vec3 hitDirection) override;
	...
	void PlaySound(EPlayerSounds sound, bool play = true, tukk paramName = NULL, float paramValue = 0.0f, tukk paramName2 = NULL, float paramValue2 = 0.0f, float volume = 1.0f, bool playOnRemote = false);
	...
	void AddXPBonusMultiplier(i32 increment) { m_xpBonusMultiplier += increment; }
	i32 GetXPBonusModifiedXP(i32 baseXP);
	...
	void RegisterKill(IActor *pKilledActor, i32 hit_type);

	void NetSetInStealthKill(bool inKill, EntityId targetId, u8 animIndex);
	void StopStealthKillTargetMovement(EntityId playerId);
	void OnPickedUpPickableAmmo( IEntityClass* pAmmoType, i32 count );
	...
	i32  GetShadowCharacterSlot() const;
	...
	bool SetCurrentFollowCameraSettings(u32 crcName);
	...
	void GetPlayerProgressions(i32* outXp, i32* outRank, i32* outDefault, i32* outStealth, i32* outArmour, i32* outReincarnations );
	...
	void LogXPChangeToTelemetry(i32 inXPDelta, EXPReason inReason);
	...
	void SetTagByCRC( u32 tagCRC, bool enable );
	...
	i32 GetTeamWhenKilled() const { return m_teamWhenKilled; }

	ILINE u8 GetMPModelIndex() const {return m_mpModelIndex;}
...
private:
	...
	static void StrikeTargetPosition(i32k currentPoint, i32k numberOfPoints, Vec3& targetPos);
	...
	u8 GetJumpCounter() const;
	void  SetJumpCounter(u8 counter);
	void  HasClimbedLedge(u16k ledgeID, bool comingFromOnGround, bool comingFromSprint);
	u8 GetLedgeCounter() const;
	void  SetLedgeCounter(u8 counter);
...
protected:
	...
	virtual void SetModelIndex(u8 modelIndex) override { m_mpModelIndex = modelIndex; }
	...
	static i32k									k_maxActivePlayerPlugIns = 6;
	...
	i32																m_numActivePlayerPlugins;
	i32 															m_pendingLoadoutGroup;
...
private:
	...
	tukk GetFootstepEffectName() const;
	...
	i32							 m_reactionOverlayAnimIDs[EReaction_Total];
	...
	u8 m_meleeHitCounter;

	i32 m_xpBonusMultiplier;
	...
	u8                   m_RMIBenchmarkSeq;
#endif

//	typedef std::map<i32, AudioProxyId> TJointToAudioProxyLookup;
...
void SetupPlayerCharacterVisibility(IEntity* playerEntity, bool isThirdPerson, i32 shadowCharacterSlot = -1, bool forceDontRenderNearest = false);
