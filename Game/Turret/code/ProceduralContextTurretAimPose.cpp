...
DRXREGISTER_CLASS( CProceduralContextTurretAimPose );
...
void CProceduralContextTurretAimPose::Initialise( IEntity& entity, IActionController& actionController )
{
	...
	{
		DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "ProceduralContext TurretAimPose: Could not initialise character in slot '%d' for entity with name '%s'.", DEFAULT_CHARACTER_SLOT, entity.GetName() );
		return;
	}
...
void CProceduralContextTurretAimPose::UpdateSmoothedTargetWorldPosition( const float timePassedSeconds )
{
	...
	i16k originJointId = m_aimPosesOriginJointId;
...
bool CProceduralContextTurretAimPose::InitialiseCharacter( i32k characterSlot )
{
	if ( m_entity == NULL )
	{
		return false;
...
{
	DrxCreateClassInstance( "AnimationPoseModifier_OperatorQueue", m_pHorizontalAim );
}
...
void CProceduralContextTurretAimPose::UpdateFallbackAim( const Vec3& targetLocalPosition )
{
	...
	i16k fallbackJointId = m_weaponJointId;
...
void CProceduralContextTurretAimPose::SetHorizontalAimJointName( tukk horizontalAimJointName )
{
	...
	{
		DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "ProceduralContext TurretAimPose: Could not initialise horizontal joint with name '%s' for entity named '%s'.", horizontalAimJointName, m_entity->GetName() );
		SetHorizontalAimJointId( -1 );
		return;
	}

	i16k horizontalAimJointId = m_pIDefaultSkeleton->GetJointIDByName( horizontalAimJointName );
	SetHorizontalAimJointId( horizontalAimJointId );
}


void CProceduralContextTurretAimPose::SetWeaponJointName( tukk weaponJointName )
{
	...
	if ( m_pSkeletonPose == NULL )
	{
		DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "ProceduralContext TurretAimPose: Could not initialise weapon joint with name '%s' for entity named '%s'.", weaponJointName, m_entity->GetName() );
		SetWeaponJointId( -1 );
		return;
	}

	i16k weaponJointId = m_pIDefaultSkeleton->GetJointIDByName( weaponJointName );
	SetWeaponJointId( weaponJointId );
}


void CProceduralContextTurretAimPose::SetAimPosesOriginJointName( tukk aimPosesOriginJointName )
{
	...
	if ( m_pSkeletonPose == NULL )
	{
		DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "ProceduralContext TurretAimPose: Could not initialise aim poses origin joint with name '%s' for entity named '%s'.", aimPosesOriginJointName, m_entity->GetName() );
		SetAimPosesOriginJointId( -1 );
		return;
	}

	i16k aimPosesOriginJointId = m_pIDefaultSkeleton->GetJointIDByName( aimPosesOriginJointName );
	SetAimPosesOriginJointId( aimPosesOriginJointId );
}


void CProceduralContextTurretAimPose::SetHorizontalAimJointId( i16k horizontalAimJointId )
{
	m_horizontalAimJointId = horizontalAimJointId;
}


void CProceduralContextTurretAimPose::SetWeaponJointId( i16k weaponJointId )
{
	m_weaponJointId = weaponJointId;
}


void CProceduralContextTurretAimPose::SetAimPosesOriginJointId( i16k aimPosesOriginJointId )
{
	m_aimPosesOriginJointId = aimPosesOriginJointId;
}


i32 CProceduralContextTurretAimPose::GetVerticalAimLayer() const
{
