...
public:
	virtual void OnEnter( float blendTimeSeconds, float duration, const STurretAimIKParams& params )
	{
		...
		i32k aimAnimationId = pAnimationSet->GetAnimIDByCRC( aimAnimationCrc );

		if ( aimAnimationId < 0 && aimAnimationCrc != ANIM_CRC_INVALID )
		{
			DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "ProceduralClipTurretAimPose: Requested an aim pose with name '%s' that doesn't exist for entity '%s'. Will use fallback vertical aiming behaviour, and it will look ugly!", GetParams().animRef.c_str(), m_entity->GetName() );
			return;
		}
	...
	virtual void OnExit( float blendTimeSeconds )
	{
		...
		i32k verticalAimLayer = m_context->GetVerticalAimLayer();
