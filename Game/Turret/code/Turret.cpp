...
#include <drx3D/Game/AnimationActionTurret.h>
#include <drx3D/Game/ProceduralContextTurretAimPose.h>
#include <drx3D/Act/IItemSystem.h>
#include <drx3D/Game/TurretHelpers.h>
...
static tukk const FALLBACK_TURRET_MODEL_NAME = "";
static tukk const FALLBACK_TURRET_PRIMARY_WEAPON_CLASS_NAME = "Turret_Gun";
static tukk const FALLBACK_TURRET_PRIMARY_WEAPON_JOINT_NAME = "weaponjoint";
static const float FALLBACK_TURRET_MAX_HEALTH = 1000.f;
static tukk const FALLBACK_TURRET_FACTION_NAME = "";
...
static tukk const FALLBACK_TURRET_EYE_JOINT_NAME = FALLBACK_TURRET_PRIMARY_WEAPON_JOINT_NAME;
static tukk const FALLBACK_TURRET_CONTROLLER_DEF = "Animations/Mannequin/ADB/turretControllerDefs.xml";
static tukk const FALLBACK_TURRET_ANIMATION_DATABASE = "Animations/Mannequin/ADB/turret.adb";
...
static tukk const FALLBACK_TURRET_LASER_JOINT_NAME = FALLBACK_TURRET_PRIMARY_WEAPON_JOINT_NAME;
...
static tukk const FALLBACK_TURRET_LASER_GEOMETRY = "";
static tukk const FALLBACK_TURRET_LASER_DOT_EFFECT = "";
static tukk const FALLBACK_TURRET_LASER_SOURCE_EFFECT = "";
...
static i32k DEFAULT_TURRET_MODEL_SLOT = 0;
static tukk const DEFAULT_TURRET_PRIMARY_WEAPON_ATTACHMENT_NAME = "PrimaryWeapon";
...
static i32k DEFAULT_TURRET_INVALID_GROUPID = -1;
...
{
	tukk g_turretStateNames[ eTurretBehaviorState_Count ] =
	{
		"Undeployed",
		"PartiallyDeployed",
		"Deployed",
	...
	ETurretBehaviorState FindId( tukk const name )
	{
		...
		{
			tukk const stateName = g_turretStateNames[ i ];
...
void CTurret::InitClient( i32 channelId )
{
...
void CTurret::PostInitClient( i32 channelId )
{
...
void CTurret::FullSerialize( TSerialize serializer )
{
	...
	m_pSerializationInfo->stateId = static_cast< i32 >( GetStateId() );
...
bool CTurret::NetSerialize( TSerialize serializer, EEntityAspects aspect, u8 profile, i32 flags )
{
	return true;
}
...
void CTurret::Update( SEntityUpdateContext& context, i32 updateSlot )
{
...
void CTurret::ProcessEvent( SEntityEvent& event )
{
	...
	case ENTITY_EVENT_SCRIPT_EVENT:
		{
			tukk const eventName = reinterpret_cast< tukk >( event.nParam[ 0 ] );
	...
	if ( m_pSoundUpr.get() )
	{
		m_pSoundUpr->ProcessEvent( event );
	}
}


void CTurret::SetChannelId( u16 id )
{
...
void CTurret::GetMemoryUsage( IDrxSizer* pSizer ) const
{
	pSizer->Add( *this );
...
void CTurret::Reset( const bool enteringGameMode )
{
	...
	tukk const modelFilename = GetModelName();
	i32k loadCharacterStatus = pEntity->LoadCharacter( DEFAULT_TURRET_MODEL_SLOT, modelFilename );
...
void CTurret::InitAiRepresentation( const EInitAiRepresentationMode mode )
{
	...
	u8 factionId = GetDefaultFactionId();
	...
	i32k groupId = GetEntityInstanceProperty( pEntity, "groupId", DEFAULT_TURRET_INVALID_GROUPID );
	SetGroupId( groupId );
...
void CTurret::OnPrePhysicsUpdate()
{
	...
	UpdateTurretSoundUpr( frameTimeSeconds );
...
tukk CTurret::GetModelName() const
{
	const IEntity* const pEntity = GetEntity();

	tukk const characterModelName = GetEntityProperty< tukk >( pEntity, "objModel", FALLBACK_TURRET_MODEL_NAME );
	return characterModelName;
}


tukk CTurret::GetPrimaryWeaponClassName() const
{
	const IEntity* const pEntity = GetEntity();
	
	tukk const primaryWeaponClassName = GetEntityProperty< tukk >( pEntity, "PrimaryWeapon", FALLBACK_TURRET_PRIMARY_WEAPON_CLASS_NAME );
...
tukk CTurret::GetWeaponJointName() const
{
	const IEntity* const pEntity = GetEntity();

	tukk const primaryWeaponJointName = GetEntityProperty< tukk >( pEntity, "PrimaryWeaponJointName", FALLBACK_TURRET_PRIMARY_WEAPON_JOINT_NAME );
	return primaryWeaponJointName;
}


tukk CTurret::GetDefaultFactionName() const
{
	const IEntity* const pEntity = GetEntity();

	tukk const factionName = GetEntityProperty< tukk >( pEntity, "esFaction", FALLBACK_TURRET_FACTION_NAME );
	return factionName;
}


u8 CTurret::GetDefaultFactionId() const
{
	const IFactionMap& factionMap = gEnv->pAISystem->GetFactionMap();

	tukk const factionName = GetDefaultFactionName();
	u8k factionId = factionMap.GetFactionID( factionName );

	return factionId;
}


u8 CTurret::GetFactionId() const
{
	IEntity* const pEntity = GetEntity();
	const IAIObject* const pAiObject = pEntity->GetAI();
	if ( pAiObject != NULL )
	{
		u8k factionId = pAiObject->GetFactionID();
		return factionId;
	}
...
void CTurret::SetFactionId( u8k factionId )
{
...
void CTurret::OutputFlowEvent( tukk const eventName ) const
{
	IEntity* const pEntity = GetEntity();
	DRX_ASSERT( pEntity );
...
void CTurret::InitActionController()
{
	...
	IAnimationDatabaseUpr& animationDatabaseUpr = pMannequinInterface->GetAnimationDatabaseUpr();

	IEntity* const pEntity = GetEntity();
	SmartScriptTable pMannequinScriptTable = GetMannequinPropertiesTable( pEntity );

	tukk const controllerDefFilename = GetProperty< tukk >( pMannequinScriptTable, "fileControllerDef", FALLBACK_TURRET_CONTROLLER_DEF );
	const SControllerDef* pControllerDef = animationDatabaseUpr.LoadControllerDef( controllerDefFilename );
	...
	tukk const animationDatabaseFilename = GetProperty< tukk >( pMannequinScriptTable, "fileAnimationDatabase", FALLBACK_TURRET_ANIMATION_DATABASE );
	const IAnimationDatabase* const pAnimationDatabaseTurret = animationDatabaseUpr.Load( animationDatabaseFilename );
...
void CTurret::InitMannequinUserParams()
{
	...
	CMannequinUserParamsUpr& mannequinUserParams = pMannequinInterface->GetMannequinUserParamsUpr();
...
void CTurret::StartFragmentByName( tukk fragmentName )
{
...
void CTurret::RemoveItemAttachments()
{
	...
	IAttachmentUpr* const pAttachmentUpr = pCharacterInstance->GetIAttachmentUpr();
	assert( pAttachmentUpr != NULL );

	pAttachmentUpr->RemoveAttachmentByName( DEFAULT_TURRET_PRIMARY_WEAPON_ATTACHMENT_NAME );
}
...
void CTurret::CreateItemAttachments()
{
	...
	tukk const weaponJointName = GetWeaponJointName();
	IAttachment* const pPrimaryWeaponAttachment = pAttachmentUpr->CreateAttachment( DEFAULT_TURRET_PRIMARY_WEAPON_ATTACHMENT_NAME, CA_BONE, weaponJointName );
...
void CTurret::InitWeapons()
{
	...
	tukk const primaryWeaponClassName = GetPrimaryWeaponClassName();
	...
	if ( CWeapon* pWeapon = static_cast< CWeapon* >( pItem->GetIWeapon() ) )
	{
		...
		CTacticalUpr* const pTacticalUpr = GetTacticalUpr();
		if ( pTacticalUpr != NULL )
		{
			pTacticalUpr->RemoveEntity( m_primaryWeaponId, CTacticalUpr::eTacticalEntity_Item );
		}
	}
...
void CTurret::InitLaserBeam()
{
	...
	tukk const laserJointName = GetProperty< tukk >( pLaserTable, "JointName", FALLBACK_TURRET_LASER_JOINT_NAME );
	...
	m_pLaserParams->laser_geometry_tp = GetProperty< tukk >( pLaserTable, "objGeometry", FALLBACK_TURRET_LASER_GEOMETRY );
	m_pLaserParams->laser_dot[ 0 ] = GetProperty< tukk >( pLaserTable, "DotEffect", FALLBACK_TURRET_LASER_DOT_EFFECT );
	...
	tukk laserSourceEffectName = GetProperty< tukk >( pLaserTable, "SourceEffect", FALLBACK_TURRET_LASER_SOURCE_EFFECT );
	IParticleEffect* const pSourceEffect = gEnv->pParticleUpr->FindEffect( laserSourceEffectName );
	if ( pSourceEffect )
	{
...
bool CTurret::IsEntityHostileAndThreatening( IEntity* pTargetEntity ) const
{
	...
	{
		u8k turretFactionId = pTurretAiObject->GetFactionID();
		u8k targetFactionId = pTargetAiObject->GetFactionID();
...
i32 CTurret::GetAwarenessToActor( IAIObject* pAIObject, CActor* pActor ) const
{
	i32 awareness = 0;
...
void CTurret::ResetVision()
{
...

	{
		...
		tukk const eyeJointName = GetProperty< tukk >( pVisionPropertiesTable, "EyeJointName", FALLBACK_TURRET_EYE_JOINT_NAME );
...
void CTurret::AddToTacticalUpr()
{
	...
	if ( pTacticalUpr != NULL )
	{
		...
		pTacticalUpr->AddEntity( entityId, CTacticalUpr::eTacticalEntity_Unit );
	}
}


void CTurret::RemoveFromTacticalUpr()
{
	CTacticalUpr* const pTacticalUpr = GetTacticalUpr();
	if ( pTacticalUpr != NULL )
	{
		const EntityId entityId = GetEntityId();
		pTacticalUpr->RemoveEntity( entityId, CTacticalUpr::eTacticalEntity_Unit );
	}
}
...
void CTurret::UpdateTacticalIcon()
{
	...
	pTacticalUpr->SetEntityOverrideIcon( turretEntityId, overrideIcon );
}
...
void CTurret::AddToRadar()
{
	...
	hudEvent.AddData( SHUDEventData( static_cast< i32 >( entityId ) ) );
...
{
	ITargetTrackUpr* const pTargetTrackUpr = gEnv->pAISystem->GetTargetTrackUpr();
	IF_UNLIKELY ( pTargetTrackUpr == NULL )
	{
		return;
	}
	...
	pTargetTrackUpr->SetTargetClassThreat( aiObjectId, classThreat );
}


i16 CTurret::GetJointIdByName( tukk jointName ) const
{
	...
	i16k jointId = rIDefaultSkeleton.GetJointIDByName( jointName );
	return jointId;
}
...
ETurretBehaviorState CTurret::GetInitialBehaviorStateId() const
{
	...
	tukk const initialBehaviorStateName = GetEntityProperty< tukk >( pEntity, "esTurretState", "" );
...
void CTurret::CacheBodyDamageResources()
{
	...
	pBodyDamageUpr->CacheBodyDamage( pPropertiesTable );
}
...
void CTurret::CreateBodyDamage()
{
	...
	CBodyDamageUpr* const pBodyDamageUpr = g_pGame->GetBodyDamageUpr();
	DRX_ASSERT( pBodyDamageUpr );

	IEntity* const pEntity = GetEntity();
	const TBodyDamageProfileId bodyDamageProfileId = pBodyDamageUpr->GetBodyDamage( *pEntity );
	...
	if ( hasValidDamageProfile && hasValidCharacterForBodyDamage )
	{
		...
		pBodyDamageUpr->RegisterBodyDamageProfileIdBinding( entityId, bodyDamageProfileId );
		pBodyDamageUpr->PhysicalizePlayer( bodyDamageProfileId, *pEntity );
	}

	pBodyDamageUpr->GetBodyDestructibility( *pEntity, m_bodyDestructionInstance );
}
...
void CTurret::RemoveBodyDamage()
{
	...
	const EntityId entityId = GetEntityId();
	pBodyDamageUpr->UnregisterBodyDamageProfileIdBinding( entityId );
	...
	if ( profileId != INVALID_BODYDESTRUCTIBILITYPROFILEID )
	{
		...
		pBodyDamageUpr->ResetInstance( *pEntity, m_bodyDestructionInstance );
	}
}
...
void CTurret::UpdateBodyDestructionHit( const HitInfo& hitInfo, const float oldHealth, const float newHealth )
{
	...
	if ( profileId != INVALID_BODYDESTRUCTIBILITYPROFILEID )
	{
		...
		IEntity* const pEntity = GetEntity();
		pBodyDamageUpr->ProcessDestructiblesHit( *pEntity, m_bodyDestructionInstance, hitInfo, oldHealth, newHealth );
	}
}
...
void CTurret::InitTurretSoundUpr()
{
	m_pSoundUpr.reset();
	...
	m_pSoundUpr.reset( new CTurretSoundUpr( pSoundTable, pCharacterInstance, pIEntityAudioProxy ) );
}


void CTurret::UpdateTurretSoundUpr( const float frameTimeSeconds )
{
	if ( m_pSoundUpr.get() == NULL )
	{
		return;
	}
	...
	m_pSoundUpr->Update( pCharacterInstance, isActive, m_targetEntityId, frameTimeSeconds );
}
...
void CTurret::NotifyPreparingToFire( const CTimeValue& nextStartFireTime )
{
	...
	m_pSoundUpr->NotifyPreparingToFire( nextStartFireTime );
}
...
void CTurret::NotifyCancelPreparingToFire()
{
	...
	m_pSoundUpr->NotifyCancelPreparingToFire();
}

IComponent::ComponentEventPriority CTurret::GetEventPriority(i32k eventID) const
{
...
void CTurret::SetGroupId( i32k groupId )
{
	IEntity* const pEntity = GetEntity();
...
i32 CTurret::GetGroupId() const
{
	...
	i32k groupId = pAiObject->GetGroupId();
	return groupId;
}
...
void CTurret::NotifyGroupTargetSpotted( const IEntity* pTargetEntity )
{
	...
	ITargetTrackUpr* const pTargetTrackUpr = gEnv->pAISystem->GetTargetTrackUpr();
	assert( pTargetTrackUpr );
	pTargetTrackUpr->HandleStimulusEventInRange( groupMemberAiObjectId, "TurretSpottedTarget", stimulusEventInfo, radius );

	tukk const signalName = "GroupMemberEnteredCombat";
	u32k signalNameCrc32 = CCrc32::Compute( signalName );
...
void CTurret::InitAutoAimParams()
{
	...
	tukk const primaryBoneName = GetProperty< tukk >( pAutoAimParamsTable, "primaryTargetBone", "" );
	...
	tukk const secondaryBoneName = GetProperty< tukk >( pAutoAimParamsTable, "secondaryTargetBone", "" );
	...
	tukk const physicsBoneName = GetProperty< tukk >( pAutoAimParamsTable, "physicsTargetBone", "" );
...
void CTurret::RegisterInAutoAimSystem()
{
	...
	const EntityId entityId = GetEntityId();
	CAutoAimUpr& autoAimUpr = g_pGame->GetAutoAimUpr();
	m_registeredInAutoAimSystem = autoAimUpr.RegisterAutoaimTargetObject( entityId, m_autoAimParams );
}
...
void CTurret::UnRegisterFromAutoAimSystem()
{
	...
	const EntityId entityId = GetEntityId();
	CAutoAimUpr& autoAimUpr = g_pGame->GetAutoAimUpr();
	autoAimUpr.UnregisterAutoaimTarget( entityId );
	m_registeredInAutoAimSystem = false;
}
...
void CTurret::ProcessScriptEvent( tukk const eventName )
{
	static u32k s_dropPodLaunchCrc = CCrc32::Compute( "DropPod_Launch" );
	static u32k s_dropPodLandCrc = CCrc32::Compute( "DropPod_Land" );

	u32k eventNameCrc = CCrc32::Compute( eventName );
