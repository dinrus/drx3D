...
namespace
{
	void ReadAbsRadiansValueFromDegrees( SmartScriptTable pTable, tukk name, float& absRadiansValueInOut )
	{
		assert( *pTable );
		assert( name != NULL );
		assert( name[ 0 ] != 0 );
	...
	void ReadAbsValue( SmartScriptTable pTable, tukk name, float& absValueInOut )
	{
		assert( *pTable );
		assert( name != NULL );
		assert( name[ 0 ] != 0 );
	...
	template< typename T >
	void ReadValue( SmartScriptTable pTable, tukk name, T& valueInOut )
	{
		assert( *pTable );
		assert( name != NULL );
		assert( name[ 0 ] != 0 );
...
{
	const float fireDelaySeconds = drx_random( minFireDelaySeconds, maxFireDelaySeconds );
...
{
	const float stopFireSeconds = drx_random( minStopFireSeconds, maxStopFireSeconds );
...
void SLostTargetRuntime::UpdateFromEntity( IEntity* pEntity, const float lastValueWeight )
{
	...
	i32k getStatusDynamicsResult = pPhysicalEntity->GetStatus( &statusDynamics );
