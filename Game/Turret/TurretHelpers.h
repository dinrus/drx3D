...
namespace TurretHelpers
{
	...
	template< typename T >
	ILINE T GetProperty( SmartScriptTable pPropertiesTable, tukk propertyName, const T& defaultValue )
	{
		if ( ! pPropertiesTable )
		{
			return defaultValue;
	...
	template< typename T >
	ILINE T GetEntityProperty( const IEntity* pEntity, tukk propertyName, const T& defaultValue )
	{
		assert( pEntity != NULL );
	...
	template< typename T >
	ILINE T GetEntityInstanceProperty( const IEntity* pEntity, tukk propertyName, const T& defaultValue )
	{
		assert( pEntity != NULL );
	...
	ILINE SmartScriptTable GetSubPropertiesTable( const IEntity* pEntity, tukk subTableName )
	{
		assert( pEntity != NULL );
	...
	ILINE SmartScriptTable GetSubScriptTable( SmartScriptTable pScriptTable, tukk name )
	{
		assert( name != NULL );
		assert( name[ 0 ] != 0 );
	...
	ILINE CTacticalUpr* GetTacticalUpr()
	{
		assert( g_pGame != NULL );

		CTacticalUpr* pTacticalUpr = g_pGame->GetTacticalUpr();
		return pTacticalUpr;
	}
