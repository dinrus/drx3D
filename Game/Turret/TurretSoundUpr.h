...
class CTurretSoundUprHelper
{
public:
	CTurretSoundUprHelper( IEntityAudioProxyPtr pIEntityAudioProxy )
		: m_pIEntityAudioProxy( pIEntityAudioProxy )
	{
		assert( m_pIEntityAudioProxy != NULL );
	}

	~CTurretSoundUprHelper()
	{
		StopAllSounds();
	}

	void SetSoundName( const size_t soundId, tukk const soundName )
	{
	...
	void PlaySound( const size_t soundId )
	{
		assert( soundId < eTS_SoundCount );
		STurretSoundInfo& soundInfo = m_sounds[ soundId ];
		tukk const soundName = soundInfo.soundName.c_str();
	...
	void SetSoundParam( const size_t soundId, tukk const paramName, const float value )
	{
		assert( soundId < eTS_SoundCount );
		const STurretSoundInfo& soundInfo = m_sounds[ soundId ];
...
class CTurretSoundUpr
{
public:
	CTurretSoundUpr( SmartScriptTable pSoundPropertiesTable, ICharacterInstance* pCharacterInstance, IEntityAudioProxyPtr pIEntityAudioProxy )
		: m_soundUprHelper( pIEntityAudioProxy )
		, m_oldYawRadians( 0 )
		, m_oldPitchRadians( 0 )
		...
		, m_gameHintName( "" )
	{
		m_gameHintName = GetProperty< tukk >( pSoundPropertiesTable, "GameHintName", m_gameHintName.c_str() );
		CacheResources();
	...
	void Update( ICharacterInstance* pCharacterInstance, const bool isActive, const EntityId targetEntityId, const float frameTimeSeconds )
	{
		if ( ! isActive )
		{
			m_soundUprHelper.StopSound( eTS_VerticalMovementLoop );
			m_soundUprHelper.StopSound( eTS_HorizontalMovementLoop );
			return;
		}
	...
	void Update( const float yawRadians, const float pitchRadians, const float frameTimeSeconds )
	{
...

		{
			...
			if ( m_movingHorizontal )
			{
				...
				if ( m_soundUprHelper.IsLoopingSoundStarted( eTS_HorizontalMovementLoop ) )
				{
					m_soundUprHelper.SetSoundSpeedParam( eTS_HorizontalMovementLoop, horizontalRadiansPerSecond );
				}
				else
				{
					m_soundUprHelper.SetSoundSpeedParam( eTS_HorizontalMovementStart, horizontalRadiansPerSecond );
				}
			}
			else
			{
				...
				UpdateHorizontalMovement( timeNow, movingHorizontal );
				m_soundUprHelper.SetSoundSpeedParam( eTS_HorizontalMovementStop, horizontalRadiansPerSecond );
			}
		}

		{
			...
			if ( m_movingVertical )
			{
				...
				if ( m_soundUprHelper.IsLoopingSoundStarted( eTS_HorizontalMovementLoop ) )
				{
					m_soundUprHelper.SetSoundSpeedParam( eTS_VerticalMovementLoop, verticalRadiansPerSecond );
				}
				else
				{
					m_soundUprHelper.SetSoundSpeedParam( eTS_VerticalMovementStart, verticalRadiansPerSecond );
				}
			}
			else
			{
				...
				UpdateMovingVertical( timeNow, movingVertical );
				m_soundUprHelper.SetSoundSpeedParam( eTS_VerticalMovementStop, verticalRadiansPerSecond );
			}
		}
...
private:
	void InitSound( const size_t soundId, SmartScriptTable pSoundPropertiesTable, tukk const soundPropertyName )
	{
		tukk const soundName = GetProperty< tukk >( pSoundPropertiesTable, soundPropertyName, "" );
		m_soundUprHelper.SetSoundName( soundId, soundName );
	}
	...
	void InitJointIds( SmartScriptTable pSoundPropertiesTable, ICharacterInstance* pCharacterInstance )
	{
		...
		{
			tukk const yawJointName = GetProperty< tukk >( pSoundPropertiesTable, "HorizontalJointName", "" );
			m_yawJointId = rIDefaultSkeleton.GetJointIDByName( yawJointName );
		}

		{
			tukk const pitchJointName = GetProperty< tukk >( pSoundPropertiesTable, "VerticalJointName", "" );
	...
	void UpdateMovementSounds( const CTimeValue& timeNow, const bool moving, bool& movingOut, const size_t startSoundId, const size_t loopSoundId, const size_t stopSoundId, CTimeValue& loopTimeValueOut, const float delayLoopSeconds )
	{
		if ( movingOut )
		{
			if ( moving )
			{
				const bool playingLoopSound = m_soundUprHelper.IsLoopingSoundStarted( loopSoundId );
				if ( ! playingLoopSound )
				{
					if ( loopTimeValueOut <= timeNow )
					{
						m_soundUprHelper.PlaySound( loopSoundId );
					}
				}
			}
			else
			{
				m_soundUprHelper.StopSound( loopSoundId );
				m_soundUprHelper.PlaySound( stopSoundId );
		...
		else
		{
			...
			{
				m_soundUprHelper.StopSound( loopSoundId );
				m_soundUprHelper.PlaySound( startSoundId );
	...
	void UpdateAlertedSound( const EntityId targetEntityId )
	{
		...
		{
			m_soundUprHelper.PlaySound( eTS_Alerted );
	...
	void UpdateLockOnTargetSound()
	{
		...
		{
			m_soundUprHelper.PlaySound( eTS_LockedOnTarget );
			m_waitingForLockOnTarget = false;
		}
	}
	...
	void OnHide()
	{
		m_soundUprHelper.StopAllSounds();
		ReleaseResources();
	}
...
private:
	CTurretSoundUprHelper m_soundUprHelper;
