...
/*************************************************************************
 -------------------------------------------------------------------------
  $Id$
  $DateTime$
  Описание: GameAIModule for assisting an AI using defense (armor) mode
...
#include <drx3D/Game/ArmorModule.h>
...
{
	static tukk g_szArmorModule_ParamsXml		= "Libs\\AI\\ArmorModuleParams.xml";

	static tukk g_szArmorModule_ShieldStrengthLua	= "ArmorShieldStrength";
	static tukk g_szArmorModule_SpeedScaleLua	= "ArmorSpeedScale";
};
...
void ArmorModule::LoadParamsXml_WeaponInfo(XmlNodeRef &pNode)
{
	...
	{
		i32k childCount = pDamageMultipliers->getChildCount();
		for (i32 child = 0; child < childCount; ++child)
		{
			...
			{
				u16 weaponClassId = 0;
				tukk className = pWeapon->getAttr("class");
	...
	if (pDamageInstant)
	{
		i32k childCount = pDamageInstant->getChildCount();
		for (i32 child = 0; child < childCount; ++child)
		{
...
//////////////////////////////////////////////////////////////////////////
void ArmorModule::LoadParamsXml_Mode(TShaderParams &outShaderParams, XmlNodeRef &pNode, tukk szDebugName)
{
	Crc32Gen *pCrc32Gen = gEnv->pSystem->GetCrc32Gen();
	assert(pCrc32Gen);

	i32k childCount = pNode->getChildCount();
	for (i32 child = 0; child < childCount; ++child)
	{
...
//////////////////////////////////////////////////////////////////////////
void ArmorModule::LoadParamsXml_Modifiers(TShaderParams &outShaderParams, XmlNodeRef &pNode, tukk szDebugName)
{
	Crc32Gen *pCrc32Gen = gEnv->pSystem->GetCrc32Gen();
	assert(pCrc32Gen);
	...
	for (i32 child = 0; child < childCount; ++child)
	{
		...
		{
			tukk szTag = pChild ? pChild->getTag() : "Unknown";
			GameWarning("[ArmorModule::LoadParamsXml] Invalid child tag \'%s\' when parsing Shader Params - %s", szTag, szDebugName);
			continue;
		}
...
void ArmorModule::InitActiveParams(EntityId entityID, SActiveParams &params)
{
	...
	IAttachmentUpr* pAttachmentUpr = GetAttachmentUpr(entityID);
	if (pAttachmentUpr)
	{
		params.bodyShieldAttachmentId = pAttachmentUpr->GetIndexByName(m_sAttachmentName.c_str());

		IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByIndex(params.bodyShieldAttachmentId);
		...
		if (pAttachmentObj)
		{
			...
			IMaterial *pCloneMaterial = pMaterialUpr->CloneMaterial(pMaterial);
...
//////////////////////////////////////////////////////////////////////////
IAttachmentUpr* ArmorModule::GetAttachmentUpr(EntityId entityID)
{
	...
	IAttachmentUpr* pAttachmentUpr = pCharacter ? pCharacter->GetIAttachmentUpr() : NULL;

	return pAttachmentUpr;
}
