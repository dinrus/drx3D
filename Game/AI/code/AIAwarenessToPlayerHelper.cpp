...
#include <drx3D/Game/Turret/Turret/TurretHelpers.h>
...
void CAIAwarenessToPlayerHelper::Update( float frameTime )
{
	...
	i32 newIntAwareness = int_ceil(m_animatedAwarenessThatStrivesTowardsActualAwareness);
...
{
	i32 highestAlertnessInGameRightNow = 0;
	...
	u8 playerFactionID = factionMap.GetFactionID("Players");
	...
	if (actorIt)
	{
		while (IActor* actor = actorIt->Next())
		{
			...
			{
				i32k alertness = GetAlertnessAffectedByVisibility(*aiActor, *playerAiObject, playerIsCloaked);
	...
	// Go through non Actors
	{
		...
		for (; it != end; ++it)
		{
			...
			i32k awareness = pAwarenessEntity->GetAwarenessToActor( playerAiObject, playerActor );
...
//////////////////////////////////////////////////////////////////////////
i32 CAIAwarenessToPlayerHelper::GetAlertnessAffectedByVisibility(const IAIActor& aiActor, const IAIObject& playerAiObject, const bool playerIsCloaked) const
{
	i32 alertness = aiActor.GetProxy()->GetAlertnessState();
