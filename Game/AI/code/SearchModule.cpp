...
void SearchSpot::Init(const Vec3& pos, bool isTargetSpot /*= false*/ )
{
	...
	if (!m_visionID)
	{
		...
		static u32 searchSpotCounter = 0;
		string visionName;
...
void SearchSpot::DebugDraw(float searchTimeOut)
{
	...
	case SearchedTimingOut:
		if(searchTimeOut)
		{
			u8 green = (u8)(255 * clamp_tpl( (m_searchTimeoutLeft / (searchTimeOut / 2.0f)), 0.0f, 1.0f));
			u8 blue = (u8)(255 * clamp_tpl(((searchTimeOut - m_searchTimeoutLeft) / (searchTimeOut / 2.0f)), 0.0f, 1.0f));
...
void SearchGroup::Init(i32 groupID, const Vec3& targetPos, EntityId targetID, float searchSpotTimeout)
{
...
void SearchGroup::StoreActors(i32 groupID)
{
...
void SearchGroup::GenerateSearchSpots()
{
	...
	u32k MaxCoverLocationsPerSurface = 3;
	u32k MaxCoverLocations = 128;
...
void SearchModule::GroupEnter(i32 groupID, const Vec3& targetPos, EntityId targetID, float searchSpotTimeout)
{
...
void SearchModule::GroupLeave(i32 groupID)
{
...
bool SearchModule::GroupExist(i32 groupID) const
{
	return m_groups.find(groupID) != m_groups.end();
}

SearchGroup* SearchModule::GetGroup(i32 groupID)
{
	SearchGroupIter it = m_groups.find(groupID);
