...
#include <drx3D/Game/RateOfDeathSimple.h>
...
Vec3 CRateOfDeathSimple::CalculateTargetOffset( const float minRange, const float maxRange ) const
{
	...
	const float randomRange = drx_random( minRange, maxRange );

	const Vec3 randomOffset = drx_random_unit_vector<Vec3>() * randomRange;
