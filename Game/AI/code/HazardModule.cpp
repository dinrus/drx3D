...
#include <drx3D/Game/HazardModule.h>
...
void HazardModule::Reset(bool bUnload)
{
	...
	DRX_ASSERT_MESSAGE((m_UpdateLockCount == 0), "Not allowed to modify hazards to the hazard module while it is updating!");
...
{		
	...
	u32 hazardsAmount = (u32)m_ProjectileHazards.size();
	...
	hazardsAmount = (u32)m_SphereHazards.size();
...
{		
	...
	DRX_ASSERT_MESSAGE((m_UpdateLockCount == 0), "Not allowed to register hazards to the hazard module while it is updating!");
	...
	const Vec3& newPos, const Vec3& newNormal)
{
	...
	i32 containerIndex = IndexOfInstanceID(m_ProjectileHazards, hazardInstanceID.GetHazardID());
	if (containerIndex < 0)
	{
		assert(false);
		return false;
...
void HazardModule::ProcessCollisionsWithEntityProcessContainer(
	...
	tukk signalFunctionName, const EntityId entityID)
{
...
//
template<class CONTAINER>
i32 HazardModule::IndexOfInstanceID(const CONTAINER& container, const HazardID instanceID) const
{
...
HazardID HazardModule::GenerateHazardID()
{
	...
	DRX_ASSERT_MESSAGE(m_InstanceIDGen != gUndefinedHazardInstanceID, 
...
{
	DRX_ASSERT_MESSAGE((m_UpdateLockCount == 0), "Not allowed to unregister hazards to the hazard module while it is updating!");

	i32 index = IndexOfInstanceID(container, hazardInstanceID);
	if (index < 0)
	{
		return;
	}
