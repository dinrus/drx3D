...
namespace BehaviorTree
{
	...
	public:
		...
		{
			tukk formationName = xml->getAttr("name");
			if (!formationName)
			{
	...
	private:
		...
		bool JoinFormationInSquad(const UpdateContext& context)
		{
			...
			IEntity* pEntityToFollow = g_pGame->GetGameAISystem()->GetAISquadUpr().GetFormationLeaderForMySquad(context.entityId);
		...
		void SendSignal(IAIActor* pIAIActor, tukk signalName, IEntity* pSender)
		{
			assert(pIAIActor);
...
#define MELEE_FRAGMENT_NAME "melee"
	...
	public:
		...
		virtual void OnAnimationEvent(ICharacterInstance *pCharacter, const AnimEventInstance &event) override
		{
			...
			const static u32 meleeHitLowercaseCRC32 = CCrc32::ComputeLowercase("MeleeHit");
	...
	public:
		struct RuntimeData : public IMeleeActionListener
		{
			...
			i32 hitTypeID;
			DrxFixedStringT<64> materialEffectName;
			...
			void ApplyDamagesAndImpulse(IPhysicalEntity* pCollider, const Vec3& pos, const Vec3& normal, i32k surfaceIdx, 
				i32k partId, i32k iPart)
			{
			...
			void PlayHitMaterialEffect(const Vec3& position, const Vec3& normal, i32k surfaceIdx) const
			{
			...
			void ApplyDamage(const EntityId attackerId, IPhysicalEntity* pCollider, const Vec3& pos, const Vec3& normal, i32k surfaceIdx, i32k iPart, const float damages)
			{
			...
			void ApplyImpulse(const EntityId attackerId, IPhysicalEntity* pCollider, const Vec3& pos, i32k partId, i32k iPart, const float impulseScale, const float knockdownChance)
			{
				...
				if (knockdownChance > .0f && (drx_random(0.0f, 1.0f) < knockdownChance))
				{
			...
			void SendSignal(EntityId entityId, tukk signalName)
			{
	...
	protected:
		virtual void OnInitialize(const UpdateContext& context) override
		{
			...
			if (CanStartMeleeAttack(context, parameters.m_fMeleeAngleCosineThreshold))
			{
				...
				const static u32 FRAGID_MELEE = CCrc32::ComputeLowercase( MELEE_FRAGMENT_NAME );
	...
	private:
		...
		void QueueFragment(const EntityId entityId, u32k fragNameCrc32, RuntimeData& runtimeData)
		{
			Agent agent(entityId);
		...
		DrxFixedStringT<64>   m_materialEffectName;
	};
...
void RegisterGameBehaviorTreeNodes()
{
	...
	IBehaviorTreeUpr& manager = *gEnv->pAISystem->GetIBehaviorTreeUpr();
