...
#include <drx3D/Game/RateOfDeathHelper.h>
...
#define ROD_CVARS_LIST( x ) \
	...
	namespace Impl
	{
		...
		{
			DRX_ASSERT( cvar < eRateOfDeathCVarsCount );
			...
			{
				tukk const cvarName = g_rateOfDeathCVarsNames[ cvar ];
				DRX_ASSERT( cvarName );
...
namespace
{
	tukk const GetZoneName( const EAITargetZone zone )
	{
		switch( zone )
		{
