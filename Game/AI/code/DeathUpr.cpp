...
//   Victim Detection
//
//   When an agent dies the DeathUpr will collect group members that
//   witnessed the mate dying.  DeathUpr does this by performing
...
namespace GameAI
{
	...
	void DeathUpr::Update()
	{
		ProcessDeferredDeathReactions();
	}

	void DeathUpr::OnAgentGrabbedByPlayer(const EntityId agentID)
	{
	...
	void DeathUpr::AddDeferredDeathReactionFor(const Agent& deadAgent, const EntityId killerID)
	{
		...
		i32 groupID = deadAgent.GetGroupID();
		i32 memberCount = aiSystem.GetGroupCount(groupID);
		if (memberCount == 0)
			return;
		...
		for (i32 memberIndex = 0; memberIndex < memberCount; ++memberIndex)
		{
	...
	void DeathUpr::ProcessDeferredDeathReactions()
	{
	...
	void DeathUpr::DispatchDeferredDeathReaction(const DeferredDeathReaction& ddr)
	{
		IAISystem& aiSystem = *gEnv->pAISystem;
		...
		if (witness)
		{
			...
			{
				gEnv->pLog->Log("DeathUpr: Letting '%s' know he watched his mate die.", witness.GetName());
			}
			#endif
			...
			if (killer)
			{
				...
				if (g_pGameCVars->ai_DebugDeferredDeath)
				{
					...
					gEnv->pLog->Log("DeathUpr: Letting '%s' sense mate's killer. See orange line.", witness.GetName());
				}
				#endif
			}
		}
	...
	void DeathUpr::AddDeadBodyFor(const Agent& deadMember, EntityId killerID)
	{
		m_unseenDeadBodies.push_back(DeadBody());
		InitDeadBody(m_unseenDeadBodies.back(), deadMember, killerID);
	}

	void DeathUpr::CheckDeadBodyVisibilityFor(Agent& agent)
	{
		...
		for ( ; it != end; ++it)
		{
			...
			if (agent.CanSee(deadBody.visionID) && deadBody.groupID == agent.GetGroupID())
			{
				...
				if (g_pGameCVars->ai_DebugDeferredDeath)
				{
					gEnv->pLog->Log("DeathUpr: Letting '%s' know he spotted a dead body.", agent.GetName());
				}
				#endif
				...
				if (!agent.GetAttentionTarget())
				{
					...
					if (killer)
					{
						...
						if (g_pGameCVars->ai_DebugDeferredDeath)
						{
							gEnv->pLog->Log("DeathUpr: Letting '%s' get scared of the mate's body, but with the killer's ID.", agent.GetName());
						}
						#endif
					}
				}
	...
	void DeathUpr::ClearDeadBodiesForGroup(i32 groupID)
	{
	...
	void DeathUpr::ClearDeferredDeathReactionsForGroup(i32 groupID)
	{
	...
	void DeathUpr::QueueNextPotentialWitnessRay(DeferredDeathReaction& ddr)
	{
		assert(m_rayID == 0);
		...
		while (!potentialWitnesses.empty())
		{
			...
			if (agent.IsValid())
			{
				...
				m_rayID = g_pGame->GetRayCaster().Queue(
					...
					functor(*this, &DeathUpr::PotentialWitnessRayComplete));
	...
	void DeathUpr::InitDeadBody(DeadBody& deadBody, const Agent& agent, EntityId killerID)
	{
	...
	void DeathUpr::DestroyDeadBody(DeadBody& deadBody)
	{
	...
	void DeathUpr::OnAgentDeath(EntityId deadEntityID, EntityId killerID)
	{
	...
	void DeathUpr::OnAgentUpdate(EntityId entityID)
	{
	...
	void DeathUpr::PotentialWitnessRayComplete(const QueuedRayID& rayID, const RayCastResult& result)
	{
	...
	void DeathUpr::InjectDeadGroupMemberDataIntoScriptTable(IScriptTable* scriptTable, const EntityId victimID, const EntityId killerID, const Vec3& victimPosition)
	{
		if (scriptTable)
		{
