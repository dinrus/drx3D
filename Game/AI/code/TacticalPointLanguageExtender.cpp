...
#include <drx3D/Game/TacticalPointLanguageExtender.h>
...
{
	DrxLogAlways("Registering TPS Extensions...");
...
bool CTacticalPointLanguageExtender::GeneratePoints(TGenerateParameters& parameters, SGenerateDetails& details, TObjectType object, const Vec3& objectPos, TObjectType auxObject, const Vec3& auxObjectPos) const
{
	...
	if (stricmp(parameters.query, "advantagePoints") == 0)
	{
		...
		DRX_ASSERT_MESSAGE(gEnv->pAISystem != NULL, "Expected AI System to exist, but it didn't.");
...
{
	DRX_ASSERT_MESSAGE(gEnv->pAISystem->GetTacticalPointSystem() != NULL, "Expecting tactical point system to exist, but it doesn't.");
