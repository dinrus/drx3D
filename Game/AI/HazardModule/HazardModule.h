...
#include #include <drx3D/Game/GameAIHelpers.h>

#include <drx3D/Game/Hazard.h>
#include <drx3D/Game/HazardProjectile.h>
#include <drx3D/Game/HazardSphere.h>
...
public:
	...
	virtual tukk                 GetName() const { return "HazardModule"; }
...
private:
	...
	template<class CONTAINER> void      ProcessCollisionsWithEntityProcessContainer(CONTAINER& container, Agent& agent, tukk signalFunctionName, const EntityId entityID);
	...
	void                                SendSignalToAgent(Agent& agent, tukk warningName, const Vec3& estimatedHazardPos, const Vec3& hazardNormal);

	// Hazard instance management:
	template<class CONTAINER> i32       IndexOfInstanceID(const CONTAINER& container, const HazardSystem::HazardID instanceID) const;	
