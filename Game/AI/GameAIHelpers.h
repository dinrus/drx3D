...
#include <drx3D/Game/IGameAIModule.h>
...
public:
	...
	void SendSignal(tukk signal, IAISignalExtraData* data = NULL);
	void SendSignal(tukk signal, IAISignalExtraData* data, i32 nSignalID);
	IEntity* GetEntity() const { return gEnv->pEntitySystem->GetEntity(m_entityID); }
	EntityId GetEntityID() const { return m_entityID; }

#ifndef RELEASE
	tukk GetDebugEntityName() const { return m_debugEntityName.c_str(); }
	CGameAIInstanceBase(const CGameAIInstanceBase& rhs)
		: m_entityID(rhs.m_entityID)
		, m_debugEntityName(rhs.m_debugEntityName)
	{}
#else
	tukk GetDebugEntityName() const { return "NoNameInRelease"; }
	CGameAIInstanceBase(const CGameAIInstanceBase& rhs)
		: m_entityID(rhs.m_entityID)
	{}
#endif
...
struct InstanceID
{
	InstanceID() : id(std::numeric_limits<u16>::max()) {}
	InstanceID(const InstanceID& other) : id(other.id) {}

	operator u16 () const { return id; }
...
template <class Module, class Instance, u32 NumPreallocatedInstances, u32 GrowSize = 8>
		...
		if (m_free->empty())
		{
			if (m_pool.size() == m_pool.capacity())
			{
				...
				if (oldInstanceCount > 0)
				{
					...
					for (u32 i = 0; i < oldInstanceCount; ++i)
					{
			...
			instanceID = static_cast<u16> (m_pool.size() / sizeof(Instance));
