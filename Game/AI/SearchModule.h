...
public:
	void Init(i32 groupID, const Vec3& targetPos, EntityId targetID, float searchSpotTimeout);
	void Destroy();
	void Update();
...
public:
	...
	virtual tukk GetName() const { return "SearchModule"; }

	void GroupEnter(i32 groupID, const Vec3& targetPos, EntityId targetID, float searchSpotTimeout);
	void GroupLeave(i32 groupID);
	bool GetNextSearchPoint(EntityId entityID, SearchSpotQuery* query);
	void MarkAssignedSearchSpotAsUnreachable(EntityId entityID);
	bool GroupExist(i32 groupID) const;

private:
	SearchGroup* GetGroup(i32 groupID);
