...
#include <drx3D/Game/Effects/KillCamGameEffect.h>
...
struct SRecordedData
{
	...
	u32 m_tpdatasize;
	u8 m_tpdatabuffer[RECORDING_BUFFER_SIZE];
	...
	DrxFixedArray<STrackedCorpse, MAX_CORPSES> m_corpses;
};
...
public:
	...
	static const SRecording_Packet * GetFirstFPActionPacketOfType(const SFirstPersonDataContainer& rFirstPersonData, const SPlaybackInstanceData& rPlaybackInstanceData, u8 type);
	...
	// IBreakEventListener
	virtual void OnBreakEvent(u16 uBreakEventIndex);
	virtual void OnPartRemoveEvent(i32 iRemovePartEventIndex);
	virtual void OnEntityDrawSlot(IEntity * pEntity, i32 slot, i32 flags);
	virtual void OnEntityChangeStatObj(IEntity * pEntity, i32 iBrokenObjectIndex, i32 slot, IStatObj * pOldStatObj, IStatObj * pNewStatObj);
	virtual void OnSetSubObjHideMask(IEntity *pEntity, i32 nSlot, hidemask nSubObjHideMask);
	//~IBreakEventListener
	...
	bool OnEmitTracer(const CTracerUpr::STracerParams &params);

	void OnPlayerPluginEvent(CPlayer* pPlayer, EPlayerPlugInEvent pluginEvent, uk pData);
	...
	void OnMountedGunEnter(CPlayer* pPlayer, i32 upAnimId, i32 downAnimId);
	void OnMountedGunLeave(CPlayer* pPlayer);

	void OnBattleChatter(EBattlechatter chatter, EntityId actorId, i32 variation);

	void OnAttachAccessory(CItem* pItem);

	void OnPlayAnimation(IEntity *entity, i32 animId, const DrxCharAnimationParams& Params, float speedMultiplier);
	void OnPlayUpperAnimation(IEntity *entity, i32 animId, const DrxCharAnimationParams& Params, float speedMultiplier);
	...
	void OnSetTeam(EntityId entityId, i32 team);

	void OnItemSwitchToHand(CItem* pItem, i32 hand);
	...
	void OnForcedRagdollAndImpulse( EntityId playerId, Vec3 vecImpulse, Vec3 pos, i32 partid, i32 applyTime );
	...
	void OnMannequinSetSlaveController(EntityId inMasterActorId, EntityId inSlaveActorId, u32 inTargetContext, bool inEnslave, const IAnimationDatabase* piOptionalDatabase);
	void OnMannequinSetParam(EntityId inEntityId, tukk inParamName, QuatT &inQuatParam);
	void OnMannequinSetParamFloat(EntityId inEntityId, u32 inParamCRC, float inFloatParam);
	...
	void OnInteractiveObjectFinishedUse(EntityId objectId, TagID interactionTypeTag, u8 interactionIndex);
	...
	size_t GetFirstPersonData(u8 **data, EntityId victimId, float deathTime, bool skillKill, const float length);
	size_t GetFirstPersonDataForTimeRange(u8 **data, float fromTime, float toTime);
	size_t GetVictimDataForTimeRange(u8 **data, EntityId victimID, float fromTime, float toTime, bool bulletTimeKill, float timeOffset);
	void GetTPCameraData(float startTime);
	static void SetFirstPersonData(u8 *data, size_t datasize, SPlaybackInstanceData& rPlaybackInstanceData, SFirstPersonDataContainer& rFirstPersonDataContainer);
	...
	tukk CacheString(tukk name, eStringCache cache);
	...
	static EntityId NetIdToEntityId(u16 netId);
	static IEntityClass* GetEntityClass_NetSafe(u16k classId);
...
private:
	...
	struct SStackGuard
	{
		SStackGuard(i32& counter) : m_counter(counter) { counter++; }
	...
	typedef DrxFixedArray<SWeaponAudioCacheDetails, MAX_NUM_WEAPONS_AUDIO_CACHED> TWeaponAudioCacheArray;
...
#define REPLAY_EVENT_GUARD CRecordingSystem::SStackGuard __ReplayEventGuard__(CRecordingSystem::m_replayEventGuard);
	...
	template <u32 N>
	void ExtractVictimPositions(DrxFixedArray<SRecording_VictimPosition, N> &victimPositions, float startTime, EntityId victimId, float deathTime, float endTime=-1);
	...
	void SwapFPCameraEndian(u8* pBuffer, size_t size, bool sending);
	...
	void GetProceduralBreaksDuringKillcam(std::vector<u16>& indices);
	void CleanUpBrokenObjects();
	u16 EntityIdToNetId(EntityId entityId);
	ILINE bool FilterItemSuffix(tukk suffix) { return strcmp(suffix, "flag") == 0 || strcmp(suffix, "tick") == 0; }
	...
	// HIGHLIGHTS:
	void PlayHighlight(i32 index);
	void SaveHighlight( const SRecordedKill &details, i32k indexOverride = -1 );
	void CleanupHighlight(i32 index);
	float AnalysePotentialHighlight(const SRecordedKill& details, float& rEarliestTime);

	void SetPlayBackCameraView(bool bSetView);

#ifdef RECSYS_DEBUG
	void DebugStoreHighlight( const float length, i32k highlightIndex );
	static void FirstPersonDiscardingPacketStatic(SRecording_Packet *ps, float recordedTime, uk inUserData);
	void PrintCurrentRecordingBuffers ( tukk const msg = NULL );
#endif //RECSYS_DEBUG
	...
	std::vector<u16> m_breakEventIndicies;		//A list of indices into the broken objects array in DrxAction
	...
	std::set<u32> m_excludedSoundEffects;
	...
	tukk m_bulletGeometryName;
	tukk m_bulletEffectName;
	...
	//static u32k m_ambientSoundSemantics = eSoundSemantic_Ambience | eSoundSemantic_Ambience_OneShot | eSoundSemantic_SoundSpot | eSoundSemantic_Particle | eSoundSemantic_TrackView;
};
