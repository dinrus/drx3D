...
#include <drx3D/Game/ManualFrameStep.h>

#include <drx3D/Game/GameCVars.h>
#include <drx3D/Game/GameRules.h>
#include <drx3D/CoreX/Platform/DrxWindows.h>

#if DRX_PLATFORM_WINDOWS && ENABLE_MANUAL_FRAME_STEP
...
namespace
{
	...
	static u8k k_disableMask = ~0;
}

CManualFrameStepUpr::CManualFrameStepUpr()
	: m_rmiListenerIdx(-1) 
	, m_framesLeft(-1)
...
bool CManualFrameStepUpr::Update()
{
...
bool CManualFrameStepUpr::IsEnabled() const
{
	return m_framesLeft >= 0;
}

void CManualFrameStepUpr::Enable(const bool enable)
{
	if (IsEnabled() && enable)
	{
...
void CManualFrameStepUpr::Toggle()
{
	Enable(!IsEnabled());
}

void CManualFrameStepUpr::RequestFrames(u8k numFrames)
{
	m_framesLeft += numFrames;
}

bool CManualFrameStepUpr::OnInputEvent(const SInputEvent& inputEvent)
{
			...
			case eKI_Left:
				{
					...
					if (IsEnabled() && eIS_Pressed == inputEvent.state)
					{
						...
						if (false == bestPath.empty())
						{
							...
							drx_sprintf(szBuffer, "explorer.exe %s\\%s\\%s", szWorkingDirectory, framesFolder.c_str(), bestPath.c_str());
...
void CManualFrameStepUpr::OnSingleEntityRMI(CGameRules::SModuleRMIEntityParams params)
{
