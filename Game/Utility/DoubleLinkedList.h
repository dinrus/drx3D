...
class CDoubleLinkedElement
{
	...
	public:
		...
		void							InsertAfter(
										...
										{
											DRX_ASSERT_MESSAGE(!inLinkMe->IsInList(),"Adding an element to a linked list that is already in a linked list - this will corrupt the lists - Unlink() before adding");
		...
		bool							IsInList() const
										{
											...
											DRX_ASSERT_MESSAGE(linkPrev==linkNext,"Linked list element in inconsistant state");
