...
#include <drx3D/Game/Actor.h>										// CActor::KillParams
#include <drx3D/Act/IGameRulesSystem.h>					// HitInfo
...
public:
	...
	void Queue(i32 primitiveType, primitives::primitive &primitive, const Vec3 &sweepDir, i32 objTypes, i32 geomFlags);
...
private:
	...
	i32 GetPrimitiveRequestSlot();
	i32 GetSlotFromPrimitiveId(const QueuedIntersectionID& intID);
...
private:
	...
	struct SCustomAnim
	{
		...
		void 								SetAnimName(i32 animID, IAnimationSet* pAnimSet);
...
public:
	...
	bool							OnReaction(const HitInfo& hitInfo, i32* pReturnedAnimIndex);
	...
	void							RequestReactionAnims(u32 requestFlags);
	void							ReleaseReactionAnims(u32 requestFlags);
	...
	bool 							StartReactionAnim(const string& sAnimName, bool bLoop = false, float fBlendTime = 0.2f, i32 iSlot = 0, i32 iLayer = 0, u32 animFlags = (DEFAULT_REACTION_ANIM_FLAGS), float fAdditiveWeight = 0.0f, float fAniSpeed = 1.0f, bool bNoAnimCamera = false);
	bool 							StartReactionAnimByID(i32 animID, bool bLoop = false, float fBlendTime = 0.2f, i32 iSlot = 0, i32 iLayer = 0, u32 animFlags = (DEFAULT_REACTION_ANIM_FLAGS), float fAdditiveWeight = 0.0f, float fAniSpeed = 1.0f, bool bNoAnimCamera = false);
...
private:
	...
	void 										StopHigherLayers(i32 iSlot, i32 iLayer, float fBlendOut);

	u32									GetSynchedSeed(bool bKillReaction) const;
	...
	bool										SendAISignal(tukk szSignal, IAISignalExtraData *pData = NULL, bool bWaitOpOnly = false) const;
	...
	bool										StartFacialAnimation(ICharacterInstance* pCharacter, tukk szEffectorName, float fWeight = 1.0f, float fFadeTime = 0.1f);
	...
	u32									GetRootScopeID( const IActionController* piActionController, const FragmentID fragID, const TagState hitDeathFragTagState ) const;
	...
	i32															m_iGoalPipeId; // holds the Id of the goalpipe used by the actor
