...
public:
	...
	bool TestForIntersectionAtLocation(const eTestMethod testMethod, const Matrix34& wMat, EntityId testEntityId, EntityId ignoreEnt, QuatT& outAdjustedResult, const bool bCentreOnFocalEnt = false, bool bRenderOnFail = true, i32k index = -1); 
	...
	void SetEmbedPreventionResult(u8 index, const QuatT& result); 
...
private:
	...
	public:
		...
		void DoCheck( u8 index, const primitives::box& testBox, QuatT& resultIfSuccess, IPhysicalEntity* pIgnoreEntity = NULL );
		void IntersectionTestComplete(const QueuedIntersectionID& intersectionID, const IntersectionTestResult& result);
		void SetNumTestOffsets(u8 numTestOffsets); 
		void Reset();

		bool IsObstructed(i32 index);
	...
	bool TestForIntersection(const eTestMethod testMethod, const QuatT& qWOrient, const Quat& qRotOffset, const bool bCentreOnFocalEnt, i32 index);
	...
	typedef DrxFixedArray<Quat,NUM_TEST_OFFSETS> TRotationContainer; 
	...
	bool				m_bEnabled;
	u8				m_currentTestIndex;

#ifndef _RELEASE
	mutable i32	m_currentBestIndex; // *cough* mutable... 
#endif // #ifndef _RELEASE
};
