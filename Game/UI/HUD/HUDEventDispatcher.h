...
typedef DrxFixedStringT<32> TEventName;
...
public:
	...
	typedef DrxFixedArray<SHUDEventData,MAX_HUDEVENT_DATA> THUDEventData;
	typedef DrxFixedArray<THUDEventData,MAX_CONCURRENT_HUDEVENTS> THUDEventDataStack;
	...
	const SHUDEventData& GetData(u8 index) const
	{
		DRX_ASSERT_MESSAGE(index < m_data->size(), "More SHUDEventData requested than provided!");
		return (*m_data)[index];
	}

	u32k GetDataSize( void ) const
	{
		return m_data->size();
	}
	...
	{
		for(u8 i=0; i<MAX_CONCURRENT_HUDEVENTS; ++i)
		{
			THUDEventData data;
...
private:
	...
	static void FreeDataId(u8k dataId)
	{
...
struct SHUDEventData_String
{
	...
	DrxFixedStringT<32> string1;
	DrxFixedStringT<32> string2;
};
...
namespace CHUDEventDispatcher
{
	...
	void					AddHUDEventListener(IHUDEventListener* listener, tukk eventName);
	...
	EHUDEventType	GetEvent(tukk eventName);
