...
public:
	...
	static EFatalError GetFatalErrorFromLobbyError(EDrxLobbyError error);
...
private:
	...
	void ShowErrorDialog(tukk pErrorName, tukk pParam);

	SDisconnectError	m_error;
	THUDWarningId			m_warningId;
	i32								m_multiplayer;		// i32 because we defer the exit by a few frames in some cases
