...
/*static*/ void CHUDEventDispatcher::CheckEventListenersClear()
{
	...
	for (; vecIt != vecEnd; ++vecIt)
	{
		...
		for(; it!=end; ++it)
		{
			...
			DRX_ASSERT_MESSAGE(!pListener, tmpstr.Format("HUD: Event listener '%p' is still listening for events when it shouldn't be!", pListener).c_str());
		}
	}
#endif
}
...
/*static*/ void CHUDEventDispatcher::AddHUDEventListener( IHUDEventListener* pListener, tukk in_eventName )
{
#if !defined(DEDICATED_SERVER)
	DRX_ASSERT_MESSAGE(s_stlSafe, "HUD: Can't call AddHUDEventListener() in an OnHUDEvent() callback!" );
	if(s_stlSafe==false)
	{
...
{
#if !defined(DEDICATED_SERVER)
	DRX_ASSERT_MESSAGE(s_stlSafe, "HUD: Can't call RemoveHUDEventListener() in an OnHUDEvent() callback!" );
	if(s_stlSafe==false)
	{
...
/*static*/ void CHUDEventDispatcher::CallEvent( const SHUDEvent& event )
{
	...
	DRX_ASSERT_MESSAGE(s_safe, "HUD: Can't send HUDEvent whilst initialising the HUD! i.e. from CHUDObject() or CHUDObject::Init()!" );
...
/*static*/ EHUDEventType CHUDEventDispatcher::GetEvent(tukk eventName)
{
#if !defined(DEDICATED_SERVER)
	...
	for(size_t i=0; i<regdEventCount; ++i )
	{
		...
		tukk arrayName = eventInfo.key.c_str();
...
{
#if !defined(DEDICATED_SERVER)
	size_t regdEventCount = DRX_ARRAY_COUNT(s_eventLookUpTable);

	// Check for delta sizes
	DRX_ASSERT_MESSAGE( regdEventCount == eHUDEvent_LAST-1, string().Format("HUD: Wrong number of events registered (%d) when compared to the number defined (%d)! /FH", regdEventCount, (i32)eHUDEvent_LAST - 1 ).c_str() );
	...
	for(size_t defdEventId=eHUDEvent_None+1; defdEventId<eHUDEvent_LAST; ++defdEventId )
	{
		...
		DRX_ASSERT_MESSAGE( definedEventFound, string().Format("HUD: A defined event id was not found when looking for it in registered events. ID was %d. /FH", defdEventId).c_str() );
	}

	// Check for dupes
	for(size_t i=0; i<regdEventCount; ++i )
	{
		tukk regdName = s_eventLookUpTable[i].key.c_str();
		const EHUDEventType redgType = s_eventLookUpTable[i].value;
		for(size_t j=i+1; j<regdEventCount; ++j )
		{
			tukk checkName = s_eventLookUpTable[j].key.c_str();
			const EHUDEventType checkType = s_eventLookUpTable[j].value;
			DRX_ASSERT_MESSAGE( stricmp(regdName, checkName), string().Format("HUD: Duplicate event names found for %s(id%d). /FH", regdName, redgType ).c_str() );
			DRX_ASSERT_MESSAGE( redgType != checkType, string().Format("HUD: Duplicate event ID entries found names found for ID%d(%s). /FH", redgType, regdName).c_str() );
		}
	}
#endif
}
