...
#include <drx3D/Game/MenuRender3DModelMgr.h>
#include <drx3D/Game/FrontEndModelCache.h>
...
{
	DRX_ASSERT_MESSAGE( IsMenu3dModelEngineSupportActive(), 
	...
	DRX_ASSERT_MESSAGE(s_pInstance==NULL,"Some how there's still an instance of CMenuRender3DModelMgr");
	s_pInstance = this;
...
CMenuRender3DModelMgr::~CMenuRender3DModelMgr()
{
	DRX_ASSERT_MESSAGE(s_instanceCount,"No instances left to delete in destructor");
	DRX_ASSERT_MESSAGE(m_bReleaseMe,"Deleting an instance which isn't marked for release");

	s_instanceCount--;
	if(s_instanceCount == 0)
	{
		DRX_ASSERT(m_pPrevNonFreedModelMgr == NULL);
		DRX_ASSERT(m_pNextNonFreedModelMgr == NULL);
		gEnv->p3DEngine->SetPostEffectParam("Post3DRenderer_Active",0.0f,true);
	}

	const uint renderEntityCount = m_renderEntityData.size();
	FE_LOG("[MenuRender3DModelMgr] Destroying instance %p (%d entities) THREAD=%u", this, renderEntityCount, DrxGetCurrentThreadId());

	// Fix up linked list of instances
	if(m_pNextNonFreedModelMgr)
	{
		DRX_ASSERT(m_pNextNonFreedModelMgr->m_pPrevNonFreedModelMgr == this);
		m_pNextNonFreedModelMgr->m_pPrevNonFreedModelMgr = m_pPrevNonFreedModelMgr;
	}

	if (m_pPrevNonFreedModelMgr == NULL)
	{
		DRX_ASSERT(s_pFirstNonFreedModelMgr == this);
		s_pFirstNonFreedModelMgr = m_pNextNonFreedModelMgr;
	}
	else
	{
		DRX_ASSERT(s_pFirstNonFreedModelMgr != this);
		DRX_ASSERT(m_pPrevNonFreedModelMgr->m_pNextNonFreedModelMgr == this);
		m_pPrevNonFreedModelMgr->m_pNextNonFreedModelMgr = m_pNextNonFreedModelMgr;
	}

	// Remove entities
	const bool bForceRemoveNow = true;
	for(u8 i=0; i<renderEntityCount; i++)
	{
...
void CMenuRender3DModelMgr::Release(bool bImmediateDelete)
{
	...
	if(s_pInstance)
	{
		...
		DRX_ASSERT(!s_pInstance->m_bReleaseMe);
		s_pInstance->m_bReleaseMe = true;

		// Release lights
		s_pInstance->ReleaseLights();

		DRX_ASSERT(s_pInstance->m_pNextNonFreedModelMgr == NULL);
		DRX_ASSERT(s_pInstance->m_pPrevNonFreedModelMgr == NULL);
		...
		{
			DRX_ASSERT(s_pFirstNonFreedModelMgr->m_pPrevNonFreedModelMgr == NULL);
...
{
	i32k lightCount = m_sceneSettings.lights.size();
...
CMenuRender3DModelMgr::TAddedModelIndex CMenuRender3DModelMgr::AddModel(const SModelParams& modelParams)
{
	...
	if(IsMenu3dModelEngineSupportActive())
	{
		...
		const uint renderEntityCount = m_renderEntityData.size();
		DRX_ASSERT(modelParams.pFilename);
		DRX_ASSERT_MESSAGE((renderEntityCount < kAddedModelIndex_MaxEntities),"CMenuRender3DModelMgr has no free slots left for new models, failed to add one");
		...
		else if(renderEntityCount < kAddedModelIndex_MaxEntities && g_pGameCVars->menu3D_enabled)
		{
			...
			FE_LOG("[MenuRender3DModelMgr] Adding %s model='%s' material='%s' THREAD=%u", 
								...
								DrxGetCurrentThreadId());

			Quat rotQuat;
			rotQuat.SetRotationXYZ(modelParams.rot);

			char uniqueEntityName[64];
			drx_sprintf(uniqueEntityName,sizeof(uniqueEntityName),"MenuObj[%s %d]",modelParams.pName,renderEntityCount);
			...
			DRX_ASSERT_MESSAGE(pNewEntity,"Failed to create entity");
			...
			if(pNewEntity)
			{
				...
				i32 nIdx = m_renderEntityData.size();
...
// Desc: Hides model
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::HideModel(bool bHide,i32 nEntityIdx,bool bUpdateModelPos)
{
	...
	for (i32 i = 0, c = m_renderEntityData.size(); i != c; ++ i)
	{
...
//--------------------------------------------------------------------------------------------------
bool CMenuRender3DModelMgr::IsCharacterFile(tukk pFilename)
{
	bool bIsCharacter = false;
	if(pFilename)
	{
		tukk pFilenameExtension = PathUtil::GetExt(pFilename);
...
{
	DRX_ASSERT_MESSAGE(m_sceneSettings.fovScale!=0.0f,"Scene Settings FOV hasn't been set yet - must SetSceneSettings before adding models");
	...
	i32k viewSurfaceX = postRenderCamera.GetViewSurfaceX();
	i32k viewSurfaceZ = postRenderCamera.GetViewSurfaceZ();
...
bool CMenuRender3DModelMgr::HasCharacterStreamedIn(SRenderSingleEntityData& rd, ICharacterInstance* pCharacter)
{
	...
	if(IAttachmentUpr* pAttachmentMgr = pCharacter->GetIAttachmentUpr())
	{
		i32k attachmentCount = pAttachmentMgr->GetAttachmentCount();
...
{
	i32 nFlags = pStatObj->GetFlags();

	if ((nFlags & STATIC_OBJECT_COMPOUND) && pStatObj->GetSubObjectCount())
	{
		for(i32 s=0,num = pStatObj->GetSubObjectCount(); s<num; s++)
		{
	...
	if(IsMenu3dModelEngineSupportActive())
	{
		...
		for(uint i=0; i<modelCount; i++)
		{
			...
			if(modelIndex != kAddedModelIndex_Invalid)
			{
				...
				// Set animation
				tukk pAnimName = currentModelData.animName.c_str();
				...
				// Set attachment
				tukk pAttachmentParent = currentModelData.attachmentParent.c_str();
...
//--------------------------------------------------------------------------------------------------
bool CMenuRender3DModelMgr::GetModelIndexFromName(tukk pName,TAddedModelIndex& outModelIndex) const
{
	const uint renderEntityCount = m_renderEntityData.size();
...
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::UpdateModel(TAddedModelIndex modelIndex,tukk pFilename, tukk pMaterialOverrideFilename, bool bUpdateModelPos)
{
	if(IsMenu3dModelEngineSupportActive())
	{
		const uint renderEntityCount = m_renderEntityData.size();
			...
			if(pEntitySystem)
			{
				...
				if(pEntity)
				{
					...
					if(bIsNewModelCharacter)
					{
						...
						if(pCharInst)
						{
							u32k animationLayer = 0;
							ISkeletonAnim* pSkelAnim = pCharInst->GetISkeletonAnim();
							if(pSkelAnim)
							{
								i32k animationsInLayer = pSkelAnim->GetNumAnimsInFIFO(animationLayer);
					...
					if(bHasNewModel)
					{
						tukk pAnimName = renderEntityData.animName.c_str();
						if(pAnimName && pAnimName[0])
						{
...
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::SetAsPost3dRenderObject(IEntity* pEntity,u8 groupId,float* pScreenRect)
{
	if(pEntity)
	{
		IEntityRenderProxy* pRenderProxy = static_cast<IEntityRenderProxy*>(pEntity->GetProxy(ENTITY_PROXY_RENDER));
...
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::ApplyMaterialOverride(IEntity* pEntity,tukk pOverrideMaterialFilename)
{
	if(pEntity && pOverrideMaterialFilename && pOverrideMaterialFilename[0])
	{
		...
		if(pMaterialUpr)
		{
			IMaterial* pMaterialOverride = pMaterialUpr->FindMaterial(pOverrideMaterialFilename);
			if(!pMaterialOverride)
			{
				const bool bMakeIfNotFound = false;
				pMaterialOverride = pMaterialUpr->LoadMaterial(pOverrideMaterialFilename,bMakeIfNotFound);
...
void CMenuRender3DModelMgr::SetSceneSettings(const SSceneSettings& sceneSettings)
{
	...
	i32k lightCount = sceneSettings.lights.size();
...
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::UpdateAnim(TAddedModelIndex modelIndex, tukk animName,float speedMultiplier,i32 layer)
{
	if(IsMenu3dModelEngineSupportActive())
	{
...
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::AttachChildToParent(TAddedModelIndex childIndex,TAddedModelIndex parentIndex, tukk pAttachPointName)
{
	if(pAttachPointName && pAttachPointName[0])
	{
		...
		if(parentIndex < renderEntityCount && childIndex < renderEntityCount)
		{
			...
			if(pChildEntity)
			{
				u8k topParentGroupId = pTopParentRenderData->groupId;
				u8k groupIdToFree = childRenderData.groupId;
...
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::FreeUnusedGroup(u8 unusedGroupId)
{
	u8k endGroupIndex = m_groupCount-1;
...
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::AttachToStaticObject(TAddedModelIndex childIndex, TAddedModelIndex parentIndex, tukk pMountPointName)
{
...
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::AttachToCharacter(TAddedModelIndex childIndex, TAddedModelIndex parentIndex, tukk pAttachPointName)
{
	CACHE_LOG_FILE_ACCESSES_START;
	...
	if(pAttachPointName && pAttachPointName[0])
	{
		...
		if((parentIndex < renderEntityCount) && (childIndex < renderEntityCount))
		{
			...
			if(pParentEntity)
			{
				...
				if(pCharInst)
				{
					IAttachmentUpr* pAttachMgr = pCharInst->GetIAttachmentUpr();
					if(pAttachMgr)
					{
						SRenderSingleEntityData& childRenderEntityData = m_renderEntityData[childIndex];

						// Clear previous attachment binding
						tukk pPrevAttachmentName = childRenderEntityData.attachmentLocation.c_str();
...
// Desc: Updates light
//--------------------------------------------------------------------------------------------------
void CMenuRender3DModelMgr::UpdateLight(i32 lightIndex,float frameTime)
{
	i32k lightCount = m_sceneSettings.lights.size();
	if(lightIndex < lightCount)
	{
		...
		if(lightData.pLightSource)
		{
			...
			drx_sprintf(uniqueLightName,sizeof(uniqueLightName),"Menu light %d",lightIndex);
			...
			if(!bIsStreamedIn || pEntity->IsHidden())
			{
				...
				if(bHasModel)
				{
					...
					if(bIsCharacter && pAnimName && pAnimName[0])
					{
						...
						if(pCharInst)
						{
							...
							if(pAnimSet)
							{
								i32k animId = pAnimSet->GetAnimIDByName(pAnimName);
				...
				if(bHasStreamedIn && bHasAnimStreamedIn)
				{
					...
					if(bHasModel)
					{
						...
						for(u8 j=0; j<renderEntityCount; j++)
						{
