...
{
	DRX_TODO(01, 12, 2009, "Performance: consider using dynamic sized vector w/ reserve instead resize.");
...
void CHUDSilhouettes::SetSilhouette(IEntity *pEntity,float r,float g,float b,float a,float fDuration, bool bFlowGraphRequested /*= false*/)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CHUDSilhouettes: Too many silhouettes active, vector size should be increased!");
...
void CHUDSilhouettes::SetType(i32 iType)
{
	...
	for(TSilhouettesVector::iterator iter=m_silhouettesVector.begin(); iter!=m_silhouettesVector.end(); ++iter)
	{
		...
		if (pSilhouette->bValid)
		{
			...
			if (g_pGameCVars->g_displayDbgText_silhouettes)
			{
				...
				DrxWatch("$8[SILHOUETTES]$o eID=%u %s '%s' time=%.2f col=[%.2f %.2f %.2f %.2f]", pSilhouette->uiEntityId, e ? e->GetClass()->GetName() : "NULL entity", e ? e->GetName() : "N/A", pSilhouette->fTime, pSilhouette->r, pSilhouette->g, pSilhouette->b, pSilhouette->a);
			}
#endif
...
void CHUDSilhouettes::Serialize(TSerialize &ser)
{
	...
	i32 amount = m_silhouettesFGVector.size();
...
void CHUDSilhouettes::SetSilhouetteInInfiltrationMode(IEntity* pClientEntity, IEntity* pTargetEntity)
{
	...
	if(pAIObject != NULL)
	{
		if(pAIObject->IsHostile(pClientEntity->GetAI(), false))
		{
			...
			/*
			i32 targetAIGroup = pAIObject->GetGroupId();
			i32	maxAlertness = 0;
			IAISystem* pAISystem = gEnv->pAISystem;
			
			i32 groupCount = pAISystem->GetGroupCount(targetAIGroup, IAISystem::GROUP_ENABLED);
			for (i32 i = 0; i < n; i++)
			{
			...
			*/
			...
			i32k alertness = pAIObject->GetProxy() ? pAIObject->GetProxy()->GetAlertnessState() : 0;
...
void CHUDSilhouettes::DrawDebugInfiltrationInfo(const SSilhouette& silhoutte)
{
	...
	if(pTargetEntity)
	{
		...
		ColorB coneColor;
		coneColor.r = (u8) (silhoutte.debugInfiltrationInfo.r * 255.0f);
		coneColor.g = (u8) (silhoutte.debugInfiltrationInfo.g * 255.0f);
		coneColor.b = (u8) (silhoutte.debugInfiltrationInfo.b * 255.0f);
		coneColor.a = (silhoutte.debugInfiltrationInfo.fTime < 1.0f) ? (u8) (silhoutte.debugInfiltrationInfo.fTime * 128) : 128;
...
void CHUDSilhouettes::DrawDebugCombatInfo(const SSilhouette& silhoutte)
{
	...
	if(pCActor != NULL && pClientActor != NULL)
	{
		...
		color.g = 0;
		color.b = 0;
		color.a = (silhoutte.debugCombatInfo.fTime < 1.0f) ? (u8) (silhoutte.debugCombatInfo.fTime * 192) : 192;
