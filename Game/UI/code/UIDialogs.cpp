...
////////////////////////////////////////////////////////////////////////////
u32 CUIDialogs::DisplayDialog(EDialogType type, tukk title, tukk message, tukk paramMessage, IDialogCallback* pListener)
{
	if (m_dialogs.size() == 0)
		m_eventSender.SendEvent<eUIE_DisplayDialogAsset>();

	u32 id = GetNextFreeId();
	...
	{
		case eDT_DialogWait:
			m_eventSender.SendEvent<eUIE_AddDialogWait>((i32)id, title, message);
			break;
		case eDT_Warning:
			m_eventSender.SendEvent<eUIE_AddDialogWarning>((i32)id, title, message, param1, param2);
			break;
		case eDT_Error:
			m_eventSender.SendEvent<eUIE_AddDialogError>((i32)id, title, message, param1, param2);
			break;
		case eDT_AcceptDecline:
			m_eventSender.SendEvent<eUIE_AddDialogAcceptDecline>((i32)id, title, message, param1, param2);
			break;
		case eDT_Confirm:
			m_eventSender.SendEvent<eUIE_AddDialogConfirm>((i32)id, title, message, param1, param2);
			break;
		case eDT_Okay:
			m_eventSender.SendEvent<eUIE_AddDialogOkay>((i32)id, title, message, param1, param2);
			break;
		case eDT_Input:
			m_eventSender.SendEvent<eUIE_AddDialogInput>((i32)id, title, message, paramMessage);
			break;
		default:
			assert(false);
	}
...
////////////////////////////////////////////////////////////////////////////
void CUIDialogs::CancelDialog(u32 dialogId)
{
...
// ui events
////////////////////////////////////////////////////////////////////////////
void CUIDialogs::OnDialogResult( i32 dialogid, i32 result, tukk message )
{
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			...
			InputPortConfig<i32>( "Type", 0, _HELP("Dialog Type"), 0, _UICONFIG( "enum_int:WaitDialog=0,WarningDialog=1,ErrorDialog=2,AcceptDeclineDialog=3,ConfirmDialog=4,OkayDialog=5,InputDialog=5" ) ),
		...
		static const SOutputPortConfig outputs[] = {
			...
			OutputPortConfig<i32>("OnResult", _HELP("Returns once the dialog returns: 0=Yes/Ok/Confirm, 1=No/Cancel/Decline, 2=Canceled by system")),
	...
	virtual void DialogCallback(u32 dialogId, EDialogResponse response, tukk input)
	{
		assert(m_id == dialogId && m_bDisplayed);
		ActivateOutput(&m_ActInfo, eO_Param, string(input));
		ActivateOutput(&m_ActInfo, eO_Result, (i32)response);
		m_bDisplayed = false;
	}
