...
void CUIHUD3D::UpdateView(const SViewParams &viewParams)
{
	...
	if (gEnv->pRenderer && m_pHUDRootEntity && pCVars->hud_debug3dpos > 0)
	{
		...
		i32k children = m_pHUDRootEntity->GetChildCount();
		for (i32 i = 0; i < children && pCVars->hud_debug3dpos > 1; ++i)
		{
...
void CUIHUD3D::SpawnHudEntities()
{
	...
	if (node)
	{
		...
		{
			tukk name = node->getChild(i)->getAttr("Name");
		...
		if (prefab)
		{
			...
			i32k count = prefab->getChildCount();
			childs.reserve(count-1);

			for (i32 i = 0; i < count; ++i)
			{
				tukk name = prefab->getChild(i)->getAttr("Name");
			...
			if (pivotNode)
			{
				...
				for (std::vector<XmlNodeRef>::iterator it = childs.begin(); it != childs.end(); ++it)
				{
					...
					if (pEntClass)
					{
						tukk material = child->getAttr("Material");
						Vec3 pos;
						Vec3 scale;
						Quat rot;
						...
						if (pEntity)
						{
							...
							if (pScriptTable)
							{
								...
								if (probs && properties)
								{
									for (i32 k = 0; k < properties->getNumAttributes(); ++k)
									{
										tukk sKey;
										tukk sVal;
							...
							if (material)
							{
								IMaterial* pMat = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(material);
								if (pMat)
