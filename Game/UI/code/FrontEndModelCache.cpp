...
CFrontEndModelCache::CFrontEndModelCache()
{
	...
	i32 oldSaveLevelResourceList = 0;
	...
	DRX_ASSERT(s_pSingletonInstance == NULL);
	...
	if(pRootNode)
	{
		...
		// Pak
		tukk const pGameFolder = gEnv->pDrxPak->GetGameFolder();
		...
		if(pPakData && pPakData->getChildCount())
		{
			...
			if(pPak)
			{
				...
				bool bSucceeded = gEnv->pDrxPak->OpenPack(pGameFolder,pPakName,IDrxPak::FLAGS_FILENAMES_AS_CRC32);
				bSucceeded |= gEnv->pDrxPak->LoadPakToMemory(pPakName,IDrxPak::eInMemoryPakLocale_GPU);
		...
		{
			i32k characterModelCount = pCharacterModelList->getChildCount();
			if(characterModelCount)
			{
				CreateSupportForFrontEnd3dModels();
			}

			for(i32 i=0; i<characterModelCount; i++)
			{
				const XmlNodeRef pCharacterModel = pCharacterModelList->getChild(i);
				if(pCharacterModel)
				{
					tukk pCharacterModelName = pCharacterModel->getAttr("name");
		...
		if(pItemModelsList)
		{
			i32k itemModelCount = pItemModelsList->getChildCount();
			if(itemModelCount)
			{
				CreateSupportForFrontEnd3dModels();
			}

			for(i32 i=0; i<itemModelCount; i++)
			{
				const XmlNodeRef pItemModel = pItemModelsList->getChild(i);
				if(pItemModel)
				{
					tukk pItemModelName = pItemModel->getAttr("name");
		...
		if(pPakName)
		{
			gEnv->pDrxPak->LoadPakToMemory( pPakName,IDrxPak::eInMemoryPakLocale_Unload );
			bool bSucceeded = gEnv->pDrxPak->ClosePack( pPakName,0 );
		...
		string sResourceSetFilename = PathUtil::AddSlash("Levels/Multiplayer") + "mpmenu_list.txt";
		{
			...
			if(pFile)
			{
				for(std::set<string>::iterator it = fileset.begin(); it != fileset.end(); ++it)
				{
					...
					tukk const pExt = PathUtil::GetExt(pStr);
...
CFrontEndModelCache::~CFrontEndModelCache()
{
	...
	DRX_ASSERT(s_pSingletonInstance == this);
...
//--------------------------------------------------------------------------------------------------
void CFrontEndModelCache::CacheCharacterModel(tukk pModelFileName)
{
...
//--------------------------------------------------------------------------------------------------
void CFrontEndModelCache::CacheItemModel(tukk pItemName)
{
	...
	if(pItemSharedParams)
	{
		...
		if(pGeomDef)
		{
			tukk pModelFileName = pGeomDef->modelPath.c_str();
			FE_LOG ("Caching \"%s\" (for item '%s')", pModelFileName, pItemName);
			INDENT_LOG_DURING_SCOPE();
			m_myGeometryCache.CacheGeometry(pModelFileName, false, IStatObj::ELoadingFlagsIgnoreLoDs);

			tukk pMaterialName = pGeomDef->material.c_str();
			FE_LOG("  caching material '%s'", pMaterialName);
			IMaterial* pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(pMaterialName);
			if(pMaterial)
			{
...
//--------------------------------------------------------------------------------------------------
#if FEMC_CACHE_FILE_ACCESSES
void CFrontEndModelCache::ReportFileOpen( FILE *pFile, tukk pFullPath )
{
...
void CFrontEndModelCache::ReleaseSupportForFrontEnd3dModels()
{
	...
	{
		gEnv->pCharacterUpr->ClearResources(false);
...
{
	m_prevHeap = DrxGetIMemoryUpr()->LocalSwitchToLevelHeap();
}//-------------------------------------------------------------------------------------------------
...
{
	DrxGetIMemoryUpr()->LocalSwitchToHeap(m_prevHeap);
}//-------------------------------------------------------------------------------------------------
