...
float ScreenLayoutUpr::INV_VIRTUAL_SCREEN_WIDTH  = 1.0f/VIRTUAL_SCREEN_WIDTH;
float ScreenLayoutUpr::INV_VIRTUAL_SCREEN_HEIGHT = 1.0f/VIRTUAL_SCREEN_HEIGHT;

float ScreenLayoutUpr::RENDER_SCREEN_WIDTH = VIRTUAL_SCREEN_WIDTH;
float ScreenLayoutUpr::RENDER_SCREEN_HEIGHT = VIRTUAL_SCREEN_HEIGHT;

float ScreenLayoutUpr::INV_RENDER_SCREEN_WIDTH = ScreenLayoutUpr::INV_VIRTUAL_SCREEN_HEIGHT;
float ScreenLayoutUpr::INV_RENDER_SCREEN_HEIGHT = ScreenLayoutUpr::INV_VIRTUAL_SCREEN_HEIGHT;

const float ScreenLayoutUpr::TITLE_SAFE_AREA = 0.90f; //90%
const float ScreenLayoutUpr::SONY_PS4_TRC_SAFE_AREA = 0.85f; //85%
const float ScreenLayoutUpr::MICROSOFT_XBOXONE_TCR_SAFE_AREA = 0.85f; //85%
const float ScreenLayoutUpr::ACTION_SAFE_AREA = 0.80f; //80%

ScreenLayoutUpr* ScreenLayoutUpr::s_inst = NULL;

//-----------------------------------------------------------------------------------------------------
ScreenLayoutUpr::ScreenLayoutUpr( )
...
//-----------------------------------------------------------------------------------------------------
void ScreenLayoutUpr::SetState( ScreenLayoutStates flags )
{
	DRX_ASSERT_MESSAGE( !((flags&eSLO_ScaleMethod_None) && (flags&eSLO_ScaleMethod_WithY)), "HUD: Conflicting scale methods" );
	DRX_ASSERT_MESSAGE( !((flags&eSLO_ScaleMethod_None) && (flags&eSLO_ScaleMethod_WithX)), "HUD: Conflicting scale methods" );
	DRX_ASSERT_MESSAGE( !((flags&eSLO_ScaleMethod_WithY) && (flags&eSLO_ScaleMethod_WithX)), "HUD: Conflicting scale methods" );
	m_flags = flags; // |=  /?
}

//-----------------------------------------------------------------------------------------------------
ScreenLayoutStates ScreenLayoutUpr::GetState( void ) const
{
	return (ScreenLayoutStates)m_flags;
}

void ScreenLayoutUpr::UpdateHUDCanvasSize( void )
{
	i32k renderWidth = gEnv->pRenderer->GetOverlayWidth();
	i32k renderHeight = gEnv->pRenderer->GetOverlayHeight();
...
//---------------------------------------------------
void ScreenLayoutUpr::SetSafeArea( EHUDSafeAreaID eSafeAreaID ) 
{
#if !defined(_RELEASE)
...
//---------------------------------------------------
void ScreenLayoutUpr::SetSafeArea( const Vec2& safe_area_percentage ) 
{
#if !defined(_RELEASE)
...
//---------------------------------------------------
/*static*/ void ScreenLayoutUpr::SetSafeArea( IConsoleCmdArgs* pArgs )
{
	if( !s_inst )
	{
		DrxLogAlways( "No ScreenLayoutInstance available!" );
		return;
	}

	if( pArgs->GetArgCount() != 2 && pArgs->GetArgCount() != 3 )
	{
		DrxLogAlways( "Incorrect Number of params" );
		return;
	}

	if( pArgs->GetArgCount() == 2 )
	{
		i32 id = (i32)atoi( pArgs->GetArg(1) );

		// amount > 1.0f / probably an ID
		if( id <= 0 || id > eHSAID_END )
		{
			DrxLog( "unknown safe area id." );
			return;
		}
...
const Vec2 ScreenLayoutUpr::GetSafeAreaScreenProportion( EHUDSafeAreaID which /*= eHSAID_default*/ ) const
{
...
//---------------------------------------------------
const Vec2 ScreenLayoutUpr::GetSafeAreaBorderScreenProportion( EHUDSafeAreaID which /*= eHSAID_default*/ ) const
{
	...
	default :
		DRX_ASSERT_MESSAGE(0, "HUD: Unhandled SafeArea ID/type");
	}
...
//-----------------------------------------------------------------------------------------------------
void ScreenLayoutUpr::ConvertFromVirtualToRenderScreenSpace( float* inout_x, float* inout_y ) const
{
...
void ScreenLayoutUpr::ConvertFromRenderToVirtualScreenSpace( float* inout_x, float* inout_y ) const
{
...
//-----------------------------------------------------------------------------------------------------
void ScreenLayoutUpr::ConvertFromVirtualToNormalisedScreenSpace( float* inout_x, float* inout_y ) const
{
...
//
void ScreenLayoutUpr::AdjustToSafeArea( float *fX, float* fY, float* fSizeX /* = NULL*/, float* fSizeY /* = NULL*/,
                                            ...
                                            const EUIDRAWVERTICAL   eUIDrawVerticalDocking  ) const
{
	...
	DRX_ASSERT_MESSAGE( fSizeX && fSizeY || (fSizeX == NULL && fSizeY == NULL), "HUD: Invalid combination of size pointers, probably a mistake in calling code!" );
...
void ScreenLayoutUpr::AdjustToSafeAreaProportional( Vec2& p, Vec2& s, const float assetAspect,
                                                        const EUIDRAWHORIZONTAL eUIDrawHorizontal,        
                                                        const EUIDRAWVERTICAL   eUIDrawVertical,
                                                        const EUIDRAWHORIZONTAL eUIDrawHorizontalDocking, 
                                                        const EUIDRAWVERTICAL   eUIDrawVerticalDocking  ) const
...
const float ScreenLayoutUpr::GetTagetSpaceAspect( void ) const
{
...
const Vec2 ScreenLayoutUpr::ScaleProportionalPositionInToSafeArea( const Vec2 proportionalP ) const
{
...
const Vec2 ScreenLayoutUpr::GetAssetSizeAsProportionOfScreen( const float assetAspect, const float canvasAspect, const Vec2& proportionalSize ) const
{
	if(m_flags & eSLO_ScaleMethod_None)
...
//----------------------------------------------------------------------------------
void ScreenLayoutUpr::AlignAroundPivot( float* out_x, float* out_y,
...
//----------------------------------------------------------------------------------
void ScreenLayoutUpr::AlignToScreen( float* out_x, float* out_y,
                                         const float posX, const float posY,
...
//----------------------------------------------------------------------------------
void ScreenLayoutUpr::GetWorldPositionInScreenSpace( const Vec3& worldSpace, Vec3* pOut_ScreenSpace ) const
{
	i32 v[4] = {0, 0, (i32)VIRTUAL_SCREEN_WIDTH, (i32)VIRTUAL_SCREEN_HEIGHT};
...
void ScreenLayoutUpr::GetWorldPositionInNormalizedScreenSpace( const Vec3& worldSpace, Vec3* pOut_ScreenSpace ) const
{
	i32 v[4] = {0, 0, 1, 1};
...
void ScreenLayoutUpr::CacheViewProjectionMatrix()
{
