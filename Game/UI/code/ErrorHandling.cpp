...
#include <drx3D/Game/ErrorHandling.h>
	...
	, m_bIsSignInChange(false)
{
	...
	if (s_pInstance)
	{
		DrxFatalError("Multiple error handling instances, shouldn't happen");
	}
#endif
	s_pInstance = this;
}
...
CErrorHandling::~CErrorHandling()
{
	...
	if (s_pInstance != this)
	{
		DrxFatalError("Removing unknown error handling instance");
	}
#endif
	s_pInstance = NULL;
}
...
void CErrorHandling::Update()
{
	if (m_multiplayer && (m_multiplayer <= LEAVE_MP_ON_SIGNOUT_DELAY))
	{
		...
		if (m_multiplayer == 0)
		{
			DrxLog("CErrorHandling::Update() finished leaving MP");
		}
#endif
	}
...
{
	DrxLog("CErrorHandling::OnEnterMultiplayer()");
...
void CErrorHandling::OnLeaveMultiplayer( EExitMultiplayerReason reason )
{
	...
	{
		DrxLog("CErrorHandling::OnLeaveMultiplayer() reason=%d", reason);
		switch (reason)
		{
		case eEMR_Menu:
			m_multiplayer = 0;
...
{
	DrxLog("CErrorHandling::OnFatalError() error=%d, frameId=%d", error, gEnv->pRenderer->GetFrameID());
...
{
	DrxLog("CErrorHandling::OnNonFatalError() error=%d, frameId=%d", error, gEnv->pRenderer->GetFrameID());
	if (!HasFatalError())
	{
...
{
	CWarningsUpr *pWarningsUpr = g_pGame->GetWarnings();
	if (pWarningsUpr)
	{
		pWarningsUpr->RemoveGameWarning("LeaveGameLobbySquadLeader");
		pWarningsUpr->RemoveGameWarning("LeaveGameLobby");
	}
...
{
	DrxLog("CErrorHandling::SetErrorState() %d -> %d", m_error.m_currentState, state);
...
//-----------------------------------------------------------------------------
void CErrorHandling::ShowErrorDialog( tukk pErrorName, tukk pParam )
{
...
//-----------------------------------------------------------------------------
void CErrorHandling::OnLoadingError(ILevelInfo *pLevel, tukk error)
{
	...
	if (pLevel && error)
	{
		if (strstr(error, "Failed to read level") == error)  // eg. "Failed to read level info (level.pak might be corrupted)!"
		{
			...
			if (pDLCUpr && pDLCUpr->GetRequiredDLCsForLevel( pLevel->GetName() ) )
			{
...
bool CErrorHandling::HandleNonFatalError()
{
	...
	{
		DrxLog("CErrorHandling::HandleError() disconected from in game but with no context, need to reinitialise the render device");
	...
	else
	{
		...
		case eNFE_SessionEndFailed:
			{
				tukk pErrorName = (m_error.m_bWasPrivateGame ? "DisconnectHostMigrationFailedPrivate" : "DisconnectHostMigrationFailed");
		...
		default:
			{
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Unhandled type of non-fatal disconnect error, '%d'. Not displaying an Error Dialog!", m_error.m_nonFatalError);
			}
			break;
		}
	}
...
//-----------------------------------------------------------------------------
bool CErrorHandling::OnWarningReturn( THUDWarningId id, tukk returnValue )
{
	if (id == m_warningId)
	{
		DrxLog("CErrorHandling::OnWarningReturn()");
...
//-----------------------------------------------------------------------------
CErrorHandling::EFatalError CErrorHandling::GetFatalErrorFromLobbyError( EDrxLobbyError error )
{
	switch(error)
	{
