...
#include <drx3D/Game/HUDEventTranslator.h>
...
void CHUDEventTranslator::OnSetAmmoCount(IEntityClass* pAmmoType, i32 count)
{
	SHUDEvent event(eHUDEvent_OnSetInventoryAmmoCount);
	CHUDEventDispatcher::CallEvent(event);
}
...
void CHUDEventTranslator::OnAddAccessory(IEntityClass* pAccessoryClass)
{
	...
	DRX_ASSERT(pPersistantStats != NULL);
	if (!pPersistantStats)
		return;

	tukk accessoryName = pAccessoryClass->GetName();
	i32 value = pPersistantStats->GetStat(accessoryName, EMPS_AttachmentUnlocked);
	if (value == 0)
	{
...
void CHUDEventTranslator::OnFireModeChanged(IWeapon *pWeapon, i32 currentFireMode)
{
...
void CHUDEventTranslator::OnEntityKilled(const HitInfo &hitInfo)
{
	...
	if(clientDeath && !suicide)
	{
		...
		localPlayerKilled.AddData(SHUDEventData((i32)hitInfo.shooterId));
	...
	else if(!clientDeath)
	{
		...
		remotePlayerDeath.AddData(SHUDEventData((i32)hitInfo.targetId));
...
void CHUDEventTranslator::OnItemPickedUp(IActor* pActor, EntityId itemId)
{
	...
	event.AddData(SHUDEventData((i32)itemId));

	CHUDEventDispatcher::CallEvent(event);
}

void CHUDEventTranslator::OnPickedUpPickableAmmo(IActor* pActor, IEntityClass* pAmmoType, i32 count)
{
	SHUDEvent eventPickUp(eHUDEvent_OnAmmoPickUp);
	eventPickUp.AddData(SHUDEventData((uk )NULL));
...
void CHUDEventTranslator::OnStartUse(const EntityId playerId, IVehicle* pVehicle)
{
	...
	if(m_currentVehicle != vehicleId)
	{
		...
		vehicleEvent.AddData(SHUDEventData((i32)vehicleId));
...
void CHUDEventTranslator::OnEndUse(const EntityId playerId, IVehicle* pVehicle)
{
	...
	if(pVehicle)
	{
		...
		vehicleEvent.AddData(SHUDEventData((i32)m_currentVehicle));
		CHUDEventDispatcher::CallEvent(vehicleEvent);
	}
