...
#include <drx3D/Game/UIMultiPlayer.h>
...
{
	DrxLogAlways("[CUIMultiPlayer] PlayerJoined %i %s", playerid, name.c_str() );
...
////////////////////////////////////////////////////////////////////////////
void CUIMultiPlayer::UpdateScoreBoardItem(EntityId playerid, const string& name, i32 kills, i32 deaths, const string& team)
{
...
{
	DrxLogAlways("[CUIMultiPlayer] PlayerLeft %i %s", playerid, name.c_str() );
...
{
	DrxLogAlways("[CUIMultiPlayer] PlayerRenamed %i %s", playerid, newName.c_str() );
...
void CUIMultiPlayer::OnChatReceived(EntityId senderId, i32 teamFaction, tukk message)
{
...
void CUIMultiPlayer::RequestUpdatedScores()
{
	...
	i32 statCount = pPlayerStats->GetNumPlayerStats();
	
	for (i32 i = 0; i < statCount; i++)
	{
...
string CUIMultiPlayer::GetPlayerTeamById( EntityId playerid )
{
	...
	if (pActor)
	{
		...
		if (teamId > 0)
		{
			...
			tukk  const pTeamName = pGameRules ? pGameRules->GetTeamName(teamId) : NULL;
			if (pTeamName)
			{
				return pTeamName;
			}
