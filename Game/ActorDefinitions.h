...
/*************************************************************************
 -------------------------------------------------------------------------
  $Id$
  $DateTime$
  Описание: Общие вспомогательные определения для персонажа (Actor)
...
#include <drx3D/CoreX/Game/CoherentValue.h>
...
static i32k ZEROG_AREA_ID = PHYS_FOREIGN_ID_USER+1;
...
#define WATCH_ACTOR_STATE(format, ...) \
	do {                                 \
		g_pGameCVars && g_pGameCVars->g_displayDbgText_actorState && DrxWatch("[ACTOR STATE] <%s> %s '%s' " format, IsClient() ? "Client" : "Non-client", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), __VA_ARGS__); \
	} while(0)
#define WATCH_ACTOR_STATE_FOR(pIActor, format, ...) \
	do {                                 \
		g_pGameCVars && g_pGameCVars->g_displayDbgText_actorState && DrxWatch("[ACTOR STATE] <%s> %s '%s' " format, pIActor->IsClient() ? "Client" : "Non-client", pIActor->GetEntity()->GetClass()->GetName(), pIActor->GetEntity()->GetName(), __VA_ARGS__); \
	} while(0)
#else
...
extern tukk s_BONE_ID_NAME[BONE_ID_NUM];
...
struct SActorParams
{
	...
	i32 cornerSmoother; // default:1; 0 = none, 1 = C2 method: SmoothCD of movement angle, 2 = C3 method: spline prediction
	...
	u32 aimIKLayer; // [0..15] default: 1
	u32 lookIKLayer; // [0..15] default: 15
...
struct SActorIKLimbInfo
{
	...
	SActorIKLimbInfo(i32 slot, tukk limbName, tukk rootBone, tukk midBone, tukk endBone, i32 _flags) 
...
struct SIKLimb
{
	...
	i32 rootBoneID;
	i32 endBoneID;
	i32 middleBoneID;//optional for custom IK
	...
	void SetLimb(i32 slot,tukk limbName,i32 rootID,i32 midID,i32 endID,i32 iFlags);
	void SetWPos(IEntity *pOwner,const Vec3 &pos,const Vec3 &normal,float blend,float recover,i32 requestID);
...
struct SLinkStats
{
	...
	SLinkStats( EntityId eID, u32 flgs = 0) : linkID(eID), flags(flgs)
	{
	}
...
struct SActorAnimationEvents
{
	...
	u32 m_soundId;
	u32 m_plugginTriggerId;
	u32 m_footstepSignalId;
	u32 m_foleySignalId;
	u32 m_groundEffectId;
	u32 m_swimmingStrokeId;
	u32 m_footStepImpulseId;
	u32 m_forceFeedbackId;
	u32 m_grabObjectId;
	u32 m_stowId;
	u32 m_weaponLeftHandId;
	u32 m_weaponRightHandId;
	u32 m_deathReactionEndId;
	u32 m_reactionOnCollision;
	u32 m_forbidReactionsId;
	u32 m_ragdollStartId;
	u32 m_killId;
	u32 m_deathBlow;
	u32 m_startFire;
	u32 m_stopFire;
	u32 m_shootGrenade;
	u32 m_meleeHitId;
	u32 m_meleeStartDamagePhase;
	u32 m_meleeEndDamagePhase;
	u32 m_endReboundAnim;
	u32 m_detachEnvironmentalWeapon;
	u32 m_stealthMeleeDeath;
