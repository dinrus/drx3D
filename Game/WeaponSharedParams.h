...
struct SAIWeaponOffset
{
	...
	void GetMemoryUsage(IDrxSizer* s, const TStanceWeaponOffset& vm) const
	{
		//s->AddObject(vm);
		// explicitly add the objects as it's difficult to provide a generalized overload for AddObject() taking enums in DrxSizer.h
		if (vm.capacity())
...
struct SPickAndThrowParams
{
	...
	struct SGrabTypeParams
	{
		...
		i32 shieldingHits;  // how many hits the grabbed NPC protect the player
		i32 crossHairType;

		bool shieldsPlayer;  // Does this shield the player when held

		typedef u8 TPropertyFlags;
...
{
	typedef std::map<i32, _smart_ptr<IGameSharedParameters> > TSharedMap;
...
public:
	...
	virtual tukk GetDataType() const { return "WeaponParams"; }
	virtual void ReleaseLevelResources();
	//~IGameSharedParameters

	void ReadWeaponParams(const XmlNodeRef& paramsNode, CItemSharedParams* pItemParams, tukk weaponClassName);
	void CacheResources();
...
protected:
	...
	XmlNodeRef FindAccessoryPatchForMode(tukk modeType, tukk modeName, const SAccessoryParams* pAccessory);
