...
public:
	...
	struct SEntry
	{
		SEntry(tukk materialTypeName)
			: m_materialTypeName(materialTypeName)
			, m_materialTypeId(0) {}

		ItemString m_materialTypeName;
		i32 m_materialTypeId;
	};

	typedef std::pair<SEntry, i32> TBurnSurfaceIndexPair;
	typedef std::pair<IEntityClass*, i32> TBurnClassIndexPair;
	...
	const SBurnParams* GetBurnSurfaceParams(i32 surfaceType) const;
	void PreCacheLevelResources(CItemParticleEffectCache& particleCache);
...
struct SPierceabilityParams
{
	...
	{
		for (i32 i = 0; i < ePL_MaxLevel; i++)
		{
			damageFallOffLevels[i] = 100.0f;
		}
	}

	ILINE bool DestroyOnImpact(u32 pierceability) const
	{
	...
	float GetDamageFallOffForPierceability(u32 pierceability) const
	{
		assert(pierceability >= 0);

		static i32k	kPierceabilityToPenetrationLevel[s_kMaxPierceability] = {
			ePL_NonPierceable,
	...
	ILINE bool SurfaceRequiresBackFaceCheck(u32 pierceability) const
	{
...
private:
	...
	static u32k s_kMaxPierceability = 10;
};
...
struct SAmmoParams
{
	...
	float mass;
	float speed;
	i32		bounceableBullet;
	i32		maxLoggedCollisions;
	bool	traceable;
	bool	trackOnHud;
	Vec3	spin;
	Vec3	spinRandom;

	mutable i32					surfaceTypeId;
	...
	SExplosionParams*	GetExplosionParams(i32 index) const;
