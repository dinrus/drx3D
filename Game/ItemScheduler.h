...
$Id$
$DateTime$
Описание: Item Scheduler

-------------------------------------------------------------------------
История:
- 1:9:2005   11:33 : Created by M�rcio Martins
...
// second sizeof(uk ) is just for safety!
template <typename T>
class CSchedulerActionPool : public CSchedulerActionPoolBase, public stl::PoolAllocator<sizeof(T) + sizeof(uk ) + sizeof(uk ), stl::PoolAllocatorSynchronizationSinglethreaded>
{
...
public:
	...
	ILINE void TimerAction(u32 time, ISchedulerAction *action, bool persistent=false) { TimerAction((float)time*0.001f, action, persistent); }
	ILINE void TimerAction(i32 time, ISchedulerAction *action, bool persistent=false) { TimerAction((float)time*0.001f, action, persistent); }
