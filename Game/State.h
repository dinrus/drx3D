...
#if !defined(_RELEASE) && !DRX_PLATFORM_ORBIS && !defined(DEDICATED_SERVER)
...
struct SStateEventData
{
    ...
    SStateEventData(i32 val) : m_int(val), m_type(eSEDT_int) {}
    SStateEventData(float val) : m_float(val), m_type(eSEDT_float) {}
    SStateEventData(bool val) : m_bool(val), m_type(eSEDT_bool) {}
    SStateEventData(ukk val) : m_pointer(val), m_type(eSEDT_voidptr) {}

    i32 GetInt() const { assert(m_type==eSEDT_int); return m_int; }
    float GetFloat() const { assert(m_type==eSEDT_float); return m_float; }
    bool GetBool() const { assert(m_type==eSEDT_bool); return m_bool; }
    ukk GetPtr() const { assert( (m_pointer==NULL) || (m_type==eSEDT_voidptr) ); return m_pointer; }
};
...
public:
    ...
    typedef DrxFixedArray<SStateEventData, MAX_HSMEVENT_DATA>           TStateEventData;
...
#endif
    {}
    SStateEvent(i32 type)   : m_eventType(type)
#ifdef STATE_DEBUG
        , m_debugContextAt(-1)
#endif
    {}
#ifdef STATE_DEBUG
    SStateEvent(i32 type, SStateDebugContext& context)  : m_eventType(type)
...
#endif
    {
        for( u32 i=0; i<rhs.GetDataSize(); ++i )
        {
    ...
    const SStateEventData& GetData(u8 index) const { return m_data[index];   }
    ILINE i32 GetEventId( ) const   {   return m_eventType; }
    u32k GetDataSize( ) const {   return m_data.size();   }
    void ClearData( )   {   m_data.clear(); }

    static SStateEvent CreateStateEvent( i32 type, const SStateEventData& data ) { SStateEvent event(type); event.AddData(data); return event; }

#ifdef STATE_DEBUG
    mutable i32 m_debugContextAt;
...
struct SStateEventSerialize : public SStateEvent
{
    ...
    struct SHistory
    {
        ...
        typedef DrxFixedArray<TDebugType, 40> TStateHistory;
        ...
        void AddToHistory( tukk subState, ... )
        {
            DRX_ASSERT( (UINT_PTR)subState != 0xcdcdcdcd );

            char tmp[128];
            va_list args;
            va_start(args,subState);
            drx_vsprintf( tmp, subState, args);
            va_end(args);

            DrxFixedStringT<256> finalStr;
            finalStr.Format("%x %s", m_historyID++, tmp);

            m_states[m_front] = finalStr.c_str();
            m_front = (m_front < (i32)(m_states.max_size() - 1)) ? m_front + 1 : 0;
        }

        ILINE i32 GetFront() const { return m_front - 1; }
        ILINE i32 GetBack() const { return (i32)(m_states.size() - 1); }
        ILINE const TDebugType& GetStateAt(i32 index) const { return m_states[index]; }
    ...
    struct SStateDebugContext
    {
        ...
        template<typename HOST>
        static const SStateEvent StateDebugAndLog( CStateHierarchy<HOST>* pState, tukk stateName, SStateEvent stateEvent );
    };

#define DebugInit( pDebugName ) { m_pDebugName = pDebugName; }

    #define STATE_DEBUG_LOG( n,m, ... )
    //#define STATE_DEBUG_LOG( n,m, ... ) DrxLogAlways( m, __VA_ARGS__ );
/*  #define STATE_DEBUG_LOG( colour, horz, vert, n, ... ) \
    ...
    template<typename HOST>
    const SStateEvent STATE_DEBUG_RAW_EVENT_LOG( CStateHierarchy<HOST>* pState, tukk stateName, SStateEvent stateEvent )
    {
...
struct SStateIndex
{
    ...
    explicit SStateIndex(tukk pName)
        : m_name(DrxStringUtils::HashString(pName))
        , m_func(0)
        , m_parent(nullptr)
        , m_stateID(0)
        , m_hierarchy(0)
    ...
    SStateIndex(tukk pName, typename CStateProxy<HOST>::StatePtr func, const SStateIndex<HOST>* parent, uint stateID)
        : m_name(DrxStringUtils::HashString(pName))
        , m_func(func)
        , m_parent(parent)
...
public:
    ...
    static STATE* StateNew( HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, i32 stateIndex )
    {
...
protected:
    ...
    CStateHierarchy( i32 stateID, const SStateIndex<HOST>& defaultState, CStateMachineRegistration<HOST>& stateMachineReg )
        : State_Done(DrxHash(STATE_DONE))
        , State_Continue(DrxHash(STATE_CONTINUE))
        , m_stateID( stateID )
...
private:
    ...
    bool TransitionFromCurrentToSubState( HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, int64 stateHierarchy, i32 stateID, u32 stateName )
    {
        if( (m_currentState.m_stateID != stateID) || (m_currentState.m_hierarchy != stateHierarchy) || (m_currentState.m_name != stateName) )
        {
            ...
            for( ; it!=iEnd; ++it )
            {
                if( (*it)->m_stateID == stateID )
                {
                    ...
                    {
                        DrxLog( "StateMachine: Failed to Serialize state as the hierarchy has changed!" );
                        return false;
                    }
                    if( (*it)->m_name != stateName )
                    {
                        DrxLog( "StateMachine: Failed to Serialize state as the name has changed!" );
                        return false;
                    }
    ...
    void TransitionFromCurrentToSubState( HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, TStateIndex toSubState )
    {
        ...
        tukk debugFromStateName =  m_currentState.m_pDebugName;
        tukk debugIntoStateName = toSubState.m_pDebugName;
#endif
...
public:
    ...
    {
        DRX_ASSERT( !m_pCurrentStateHierarchy );
    ...
    void StateMachineHandleEvent( HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, const SStateEvent& event )
    {
        ...
        {
            DRX_ASSERT( sizeof(event) == sizeof(SStateEvent) );
    ...
    void StateMachineUpdateTime( HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, float frameTime, bool debug )
    {
        ...
        //  debugCtx.m_renderer->Draw2dLabel(debugCtx.m_baseHorizontal, debugCtx.m_currentVertical, 1.5f, white, false, "=== HISTORY ===" );
            ...
            for (i32 i = m_pCurrentStateHierarchy->m_historyDebug.GetFront(); i >= 0; --i)
            {
            ...
            for (i32 i = m_pCurrentStateHierarchy->m_historyDebug.GetBack(); i > m_pCurrentStateHierarchy->m_historyDebug.GetFront(); --i)
            {
                debugCtx.m_currentVertical += 15.0f;
        ...
        case STATE_EVENT_SERIALIZE:
            {
                ...
                if( ser.IsWriting() )
                {
                    i32 hierarchyID = m_pCurrentStateHierarchy->GetStateID();
    ...
    bool StateMachineActiveFlag( i32 flag ) const
    {
        DRX_ASSERT( m_pCurrentStateHierarchy );
    ...
    bool StateMachineAnyActiveFlag( i32 flag ) const
    {
        DRX_ASSERT( m_pCurrentStateHierarchy );
    ...
    void StateMachineAddFlag( i32 flag )
    {
        DRX_ASSERT( m_pCurrentStateHierarchy );
    ...
    void StateMachineClearFlag( i32 flag )
    {
        DRX_ASSERT( m_pCurrentStateHierarchy );
...
private:
...
#ifdef STATE_DEBUG
    template<typename HOST>
    const SStateEvent SStateDebugContext::StateDebugAndLog( CStateHierarchy<HOST>* pState, tukk stateName, SStateEvent stateEvent )
    {
    ...
    #define DEFINE_STATE_MACHINE( host, name )\
        ...
        {\
            DRX_ASSERT_TRACE( s_pStateMachineRegistration##name, ("HSM: Somehow the registration class is NULL for the <%s> State Machine", #name) );\
