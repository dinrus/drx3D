...
#include <drx3D/Game/QuatTBlender.h>
...
public:
	...
	virtual void Update(SEntityUpdateContext& ctx, i32 val );
	...
	struct SvRequestThrowParams
	{
		...
		SvRequestThrowParams(i32 requestedThrowState) : m_RequestedThrowState(requestedThrowState){}
...
private:
	...
	tukk GetGrabHelperName() const;

	bool OnActionAttack(EntityId actorId, const ActionId& actionId, i32 activationMode, float value);
	bool OnActionUse(EntityId actorId, const ActionId& actionId, i32 activationMode, float value);	
	bool OnActionMelee(EntityId actorId, const ActionId& actionId, i32 activationMode, float value);

	bool OnActionDrop(EntityId actorId, const ActionId& actionId, i32 activationMode, float value);
	bool OnActionChargedThrow(EntityId actorId, const ActionId& actionId, i32 activationMode, float value);

	void PerformMelee(i32 activationMode, const bool bForcePerformMelee = false);
	...
	IAttachment* GetOwnerAttachment(i32k slotIndex = 0) const;
	...
	i32	 DoSimpleMeleeHit( const ray_hit& hitResult, EntityId collidedEntityId);
	void DoSimpleMeleeImpulse( const ray_hit& hitResult, EntityId collidedEntityId, i32 hitTypeID );

	void SendMortalHitTo( CActor* pActor );
	void PlayCommunication( tukk pName, tukk pChannelName );
	...
	void PlayActionLocal( const ItemString& actionName, bool _loop = false, u32 _flags = eIPAF_Default, const float speedOverride = -1.0f);

	void PowerThrowPrepareSound(bool enable);
	
	// Called when the AI sees the object
	static void OnThrownObjectSeen(const Agent& agent, EntityId objectId, uk pVPickAndThrowWeapon);
	...
	i32												m_grabType; // index into m_weaponsharedparams->pPickAndThrowParams->grabTypesParams
	i32												m_hitsTakenByNPC;
	i32												m_objectHelperSlot;  // in which slot is the helper that is being used to grab the object. Not always is the actual active slot
	i32												m_currentMeleeAnimationId; 
	...
	{
		typedef u8 TCorrectOrientationFlags;
	...
	static tukk const	m_pHelperName;
};
