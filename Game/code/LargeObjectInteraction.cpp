...
void CLargeObjectInteraction::TXmlParams::TImpulseParams::ReadXmlData( const IItemParamsNode* pNode, tukk pChildNodeName )
{
...
void CLargeObjectInteraction::TXmlParams::TAssistanceParams::ReadXmlData( const IItemParamsNode* pNode, tukk pChildNodeName )
{
	const IItemParamsNode* pLocalNode = pNode->GetChild( pChildNodeName );
	if (pLocalNode)
	{
...
void CLargeObjectInteraction::TXmlParams::TMPParams::ReadXmlData( const IItemParamsNode* pNode, tukk pChildNodeName )
{
	const IItemParamsNode* pLocalNode = pNode->GetChild( pChildNodeName );
	if (pLocalNode)
	{
...
void CLargeObjectInteraction::NetSerialize(TSerialize ser)
{
	...
	u8 prevInteractionCount = m_interactionCount;
...
void CLargeObjectInteraction::ThrowObject()
{
	...
	if (g_pGameCVars->g_mpKickableCars && pVehicle)
	{
		...
		if (pMovementController)
		{
			...
			vehicleMovementEventParams.sParam = (tukk)&largeObjectInfo;
...
EntityId CLargeObjectInteraction::GetBestTargetAndDirection( const Vec3& playerDirection2D, Vec3& outputTargetDirection ) const
{
	...
	for (i32 i = 0; i < targetCount; ++i)
	{
		...
		DRX_ASSERT(target.entityId != playerId);
...
void CLargeObjectInteraction::AdjustObjectTrajectory( float frameTime )
{
	...
	{
		const SAutoaimTarget* pTargetInfo = g_pGame->GetAutoAimUpr().GetTargetInfo(m_hitTargetId);
		if (pTargetInfo)
		{
			...
			DRX_ASSERT(totalDeviationAngle >= 0.0f);
