...
#define MeleeDebugLog(...)				DrxLogAlways("[MELEE DEBUG] " __VA_ARGS__)
#else
...
{
	DRX_ASSERT_MESSAGE(pParams, "Melee Mode Params NULL! Have you set up the weapon xml correctly?");
...
void CMelee::InitFragmentData()
{
	if( m_pWeapon && m_pWeapon->GetOwnerActor() && m_pWeapon->GetOwnerActor()->GetAnimatedCharacter() )
	{
		...
		{
			DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CMelee: tagdefs for melee_weapon fragment missing for the player." );
		}
	}
}

//------------------------------------------------------------------------
void CMelee::Update(float frameTime, u32 frameId)
{
...
SMeleeTags::SMeleeFragData CMelee::GenerateFragmentData( const SMeleeTags::TTagParamsContainer& tagContainer ) const
{
	...
	if( numTags > 0 )
	{
		const size_t tagIndex = drx_random((size_t)0, numTags - 1);
...
void CMelee::GenerateAndQueueMeleeAction() const
{
	...
	case CWeaponMelee::EMeleeStatus_KillingBlow:
		...
		break;
	default:
		DrxLog( "[Melee] Attempted to run a melee action when the status was unknown" );
		break;
	}
}
...
{
	DRX_ASSERT_MESSAGE( !IsMeleeWeapon(), "SelectMeleeAction is deprecated for the melee weapon!" );
...
void CMelee::StartAttack()
{
	...
	{
		DRX_ASSERT(pOwner && (pOwner->GetActorClass() == CPlayer::GetActorClassType()));
	...
	if(!doingMultiMeleeAttack)
	{
		...
		u32 durationInMilliseconds = static_cast<u32>(GetDuration() * 1000.f);
		...
		if (durationInMilliseconds < (m_delayTimer + k_requireAdditionalTime) * 1000.0f)
		{
			if (!gEnv->IsDedicated())
			{
				DRX_ASSERT_MESSAGE(false, string().Format("CMelee<%p> Warning! Melee attack timeout (%f seconds) needs to be substantially longer than the damage delay (%f seconds)! Increasing...", this, durationInMilliseconds / 1000.f, m_delayTimer));
			}
			durationInMilliseconds = (u32) ((m_delayTimer + k_requireAdditionalTime) * 1000.0f + 0.5f);	// Add 0.5f to round up when turning into a u32
		}
...
//------------------------------------------------------------------------
i32 CMelee::Hit(const Vec3 &pt, const Vec3 &dir, const Vec3 &normal, IPhysicalEntity *pCollider, EntityId collidedEntityId, i32 partId, i32 ipart, i32 surfaceIdx, bool remote)
{
	...
	{	
		...
		if(pTargetActor)
		{
			...
			if(pTargetAC && pOwnerAC)
			{
				...
				if(ownerFacingDot > fromBehindDot)
				{
					...
					{
						DrxLog ("[DAMAGE] %s '%s' is%s meleeing %s '%s' from behind (because %f > %f)",
		...
		if(!isFriendlyHit)
		{
			...
			{
				DrxLog ("[DAMAGE] %s '%s' is%s meleeing %s '%s' applying damage = %.3f x %.3f = %.3f",
					pOwnerActor->GetEntity()->GetClass()->GetName(), pOwnerActor->GetEntity()->GetName(), silentHit ? " silently" : "",
					pTargetActor->GetEntity()->GetClass()->GetName(), pTargetActor->GetEntity()->GetName(),
			...
			if(pTarget)
			{
				...
				DRX_ASSERT_MESSAGE(pGameRules, "No game rules! Melee can not apply hit damage");
				...
				if (pGameRules)
				{
					...
					if (m_pMeleeParams->meleeparams.knockdown_chance>0 && drx_random(0, 99) < m_pMeleeParams->meleeparams.knockdown_chance)
...
//------------------------------------------------------------------------
/* static */ void CMelee::PlayHitMaterialEffect(const Vec3 &position, const Vec3 &normal, bool bBoostedMelee, i32 surfaceIdx)
{
	//Play Material FX
	tukk meleeFXType = bBoostedMelee ? "melee_combat" : "melee";  //Benito: Check with fx guys to update names
...
{
	DelayedImpulse(CMelee& melee, EntityId collidedEntityId, const Vec3& impulsePos, const Vec3& impulse, i32 partId, i32 ipart, i32 hitTypeID)
		: m_melee(melee)
...
void CMelee::Impulse(const Vec3 &pt, const Vec3 &dir, const Vec3 &normal, IPhysicalEntity *pCollider, EntityId collidedEntityId, i32 partId, i32 ipart, i32 surfaceIdx, i32 hitTypeID, i32 iPrim)
{
															...
															EntityId entityID, i32 partId, i32 ipart, i32 surfaceIdx, bool remote, i32 iPrim)
{
	...
	i32 hitTypeID = Hit(point, dir, normal, physicalEntity, entityID, partId, ipart, surfaceIdx, remote);
...
void CMelee::RequestAlignmentToNearestTarget()
{
	...
	if(pOwner && pOwner->IsClient())
	{
		...
		g_pGame->GetAutoAimUpr().SetCloseCombatSnapTarget(s_meleeSnapTargetId, 
...
EntityId CMelee::GetNearestTarget()
{
	...
	DRX_ASSERT(pOwnerActor->IsClient());
...
//-----------------------------------------------------------------------
void CMelee::GetMemoryUsage( IDrxSizer * s ) const
{
...
bool CMelee::StartMultiAnimMeleeAttack(CActor* pActor)
{
	if(pActor)
	{
		...
		i32 introID = fragmentIDs.Find(actions.attack_multipart.c_str());
...
CMelee::CMeleeAction::CMeleeAction( i32 priority, CWeapon* pWeapon, FragmentID fragmentID)
