...
void CPlayerStateLedge::OnEnter( CPlayer& player, const SStateEventLedge& ledgeEvent )
{
	...
	if( IAnimatedCharacter* pAnimChar = player.GetAnimatedCharacter() )
	{
		...
		const SLedgeInfo ledgeInfo = g_pGame->GetLedgeUpr()->GetLedgeById(nearestGrabbableLedgeId);
...
void CPlayerStateLedge::OnPrePhysicsUpdate( CPlayer& player, const SActorFrameMovementParams& movement, float frameTime )
{
	...
	Vec3 posDiff(ZERO);
	const SLedgeInfo ledgeInfo = g_pGame->GetLedgeUpr()->GetLedgeById( m_ledgeId );
	if ( ledgeInfo.IsValid() )
	{
		...
		//DrxWatch("ledge: desiredSpeed=%f", desiredSpeed);
			...
			|| desiredSpeed > 0.1f )
		{
			...
			//{
			//	DrxLog("CPlayerStateLedge::OnPrePhysicsUpdate() setting moveRequestVelocity when not animcontrolled. MCMH=%d; MCMV=%d", player.GetAnimatedCharacter()->GetMCMH(), player.GetAnimatedCharacter()->GetMCMV());
			//}
...
void CPlayerStateLedge::UpdateNearestGrabbableLedge( const CPlayer& player, SLedgeTransitionData* pLedgeState, const bool ignoreCharacterMovement )
{
	...
	const LedgeId ledgeId = g_pGame->GetLedgeUpr()->FindNearestLedge(vPosition, vScanDirection, GetLedgeGrabbingParams().m_ledgeNearbyParams.m_fMaxDistance, DEG2RAD(nearbyParams.m_fMaxAngleDeviationFromSearchDirInDegrees), DEG2RAD(nearbyParams.m_fMaxExtendedAngleDeviationFromSearchDirInDegrees));
	if (ledgeId.IsValid())
	{
...
{
	DRX_ASSERT_MESSAGE(ledgeId.IsValid(), "Ledge index should be valid");

	const CLedgeUpr* pLedgeUpr = g_pGame->GetLedgeUpr();
	const SLedgeInfo detectedLedge = pLedgeUpr->GetLedgeById(ledgeId);

	DRX_ASSERT( detectedLedge.IsValid() );
	...
	const Vec3 vToVec = pLedgeUpr->FindVectorToClosestPointOnLedge(refPosition, detectedLedge);
	...
	// if required could add handling to reject ledges based on marines within CLedgeUpr::FindNearestLedge()
	if (!usableByMarines)
	{
		return SLedgeTransitionData::eOLT_None;
	}
	...
	for (i32 i = 0; i < SLedgeTransitionData::eOLT_MaxTransitions; ++i)
	{
...
QuatT CPlayerStateLedge::CalculateLedgeOffsetLocation( const Matrix34& worldMat, const Vec3& vPositionOffset, const bool keepOrientation ) const
{
	...
	const SLedgeInfo& ledgeInfo = pLedgeUpr->GetLedgeById( m_ledgeId );
	...
	const Vec3 vToVec =	pLedgeUpr->FindVectorToClosestPointOnLedge(vWorldPos, ledgeInfo);
...
{
	DRX_TODO(15, 4, 2010, "Remove this CVAR once the ledge grab is sorted, is a useful debug tool for now...");
...
bool CPlayerStateLedge::IsCharacterMovingTowardsLedge( const CPlayer& player, const LedgeId& ledgeId)
{
	...
	if (inputMovement.y >= 0.2f) 
	{
		...
		{
			Vec3 vCharactertoLedge = pLedgeUpr->FindVectorToClosestPointOnLedge(vCharacterPos, detectedLedge);
...
{
	tukk gameModeParams = "LedgeGrabbingParams";
...
void CPlayerStateLedge::SLedgeBlendingParams::SetParamsFromXml(const IItemParamsNode* pParams)
{
	if (pParams)
	{
		...
		tukk pLedgeType = pParams->GetAttribute("ledgeType");
		if (pLedgeType)
		{
...
bool CPlayerStateLedge::TryLedgeGrab( CPlayer& player, const float expectedJumpEndHeight, const float startJumpHeight, const bool bSprintJump, SLedgeTransitionData* pLedgeState, const bool ignoreCharacterMovementWhenFindingLedge )
{
	...
	if( pLedgeState->m_ledgeTransition != SLedgeTransitionData::eOLT_None )
	{
		...
		if (!isFalling)
		{
			const SLedgeInfo ledgeInfo = g_pGame->GetLedgeUpr()->GetLedgeById( LedgeId(pLedgeState->m_nearestGrabbableLedgeId) );
			DRX_ASSERT( ledgeInfo.IsValid() );
