...
namespace
{
	...
	void DoRipOffPrompt(EntityId who, bool doIt)
	{
		...
		{
			tukk ripoffInteraction = "@ui_interaction_ripofforstopusing";
			if (doIt)
			{
...
void CHeavyMountedWeapon::OnAction(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
	if(m_rippingOff)
		return;
...
bool CHeavyMountedWeapon::OnActionRipOff(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CHeavyMountedWeapon::OnActionFiremode(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
...
void CHeavyMountedWeapon::PerformRipOff(CActor* pOwner)
{
	...
	if (pOwner)
	{
		...
		i32 timeDelay = GetCurrentAnimationTime(eIGS_Owner);
		timeDelay = (timeDelay > 0) ? timeDelay : 2000;
		i32 removeViewLimitDelay = i32(timeDelay * 0.65f);
...
void CHeavyMountedWeapon::Update( SEntityUpdateContext& ctx, i32 slot )
{
...
bool CHeavyMountedWeapon::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags)
{
...
void CHeavyMountedWeapon::InitClient(i32 channelId)
{
...
void CHeavyMountedWeapon::FinishRipOff()
{
	...
	if(IsClient() && gEnv->pGame->GetIGameFramework()->GetClientActorId()==GetOwnerId())
	{
		...
		{
			tukk collectibleId = pEntity->GetClass()->GetName();
