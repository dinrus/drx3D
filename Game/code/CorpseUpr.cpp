...
void CCorpseUpr::Update(float frameTime)
{
	i32 numberCorpses = m_activeCorpses.size();

	while(numberCorpses > 0 && numberCorpses > g_pGameCVars->g_corpseUpr_maxNum)
	{
		RemoveACorpse();
		numberCorpses--;
	}
	...
	if(g_pGameCVars->g_corpseUpr_timeoutInSeconds > 0.0f)
	{
		for(u32 iCorpse = 0; iCorpse < m_activeCorpses.size(); /* nothing */)
		{
			if(m_activeCorpses[iCorpse].age >= g_pGameCVars->g_corpseUpr_timeoutInSeconds)
			{
...
void CCorpseUpr::RegisterCorpse(EntityId corpseId, Vec3 corpsePos, float thermalVisionHeat)
{
	i32 numberCorpses = m_activeCorpses.size();

	if(numberCorpses > 0 && numberCorpses > g_pGameCVars->g_corpseUpr_maxNum)
	{
		RemoveACorpse();
	}
...
void CCorpseUpr::RemoveACorpse()
{
	float farthestDistSq = 0.f;
	i32 farthestCorpse = 0;		// default to removing corpse zero
	...
	i32 numCorpses = m_activeCorpses.size();
...
void CCorpseUpr::KeepAwake( const EntityId corpseId, IPhysicalEntity* pCorpsePhys )
{
	i32k numCorpses = m_activeCorpses.size();
	for(i32 i = 0; i < numCorpses; i++)
	{
...
void CCorpseUpr::UpdateCorpses(float frameTime)
{
	...
	float minHeatValue = g_pGameCVars->g_corpseUpr_thermalHeatMinValue;
	float heatFadeDuration = g_pGameCVars->g_corpseUpr_thermalHeatFadeDuration;
...
void CCorpseUpr::OnRemovedCorpse( const EntityId corpseId )
{
