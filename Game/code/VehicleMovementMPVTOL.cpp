...
bool CVehicleMovementMPVTOL::Init(IVehicle* pVehicle, const CVehicleParams& table)
{
	...
	i32 vtolmanagerControlled = 0;
	table.getAttr("vtolmanagerControlled", vtolmanagerControlled);
	if (vtolmanagerControlled && g_pGame->GetGameRules()->GetVTOLVehicleUpr())
	{
	...
	if(CVehicleParams wingRotationParams = table.findChild("WingRotation"))
	{
		...
		DRX_ASSERT_MESSAGE(m_maxAngleSpeed, "CVehicleMovementMPVTOL::Init - canReachMaxAngleAtSpeed is 0. This will result in divide by 0 error during update.");
	...
	{
		i32 count = noiseOverrideParams.getChildCount();
	...
	if(m_exhaustsCutOut || m_exhaustsTimer)
	{
		...
		if(m_exhaustsTimer < 0.f)
		{
			if(m_exhaustsCutOut)
			{
				...
				{
					m_exhaustsTimer = drx_random(m_minTimeBetweenStutters, m_maxTimeBetweenStutters);
					...
					m_exhaustStuttersRemaining = m_baseStutters + drx_random(0, m_maxExtraStutters); //Number of stutters next time
				}
			}
			else
			{
...
{
	CVTOLVehicleUpr* pVTOLUpr = g_pGame->GetGameRules()->GetVTOLVehicleUpr();

	if(pVTOLUpr && ser.GetSerializationTarget()==eST_Network)
	{
	...
	if(event == eVE_Damaged)
	{
		...
		if(params.fParam2 < 1.0f)
		{
			if(params.fParam2 >= m_stutterStartDamageRatio && m_prevDamageRatio < m_stutterStartDamageRatio)
			{
				...
				m_exhaustStuttersRemaining = m_baseStutters + drx_random(0, m_maxExtraStutters);
			}

			for(i32 i = 0; i < MAX_NOISE_OVERRIDES; ++i)
			{
...
{
	if( !g_pGame->GetGameRules()->GetVTOLVehicleUpr() )
	{
...
void CVehicleMovementMPVTOL::PostInit()
{
	...
	for (i32 i = 0; i < m_pVehicle->GetPartCount(); i++)
	{
	...
	DRX_ASSERT_MESSAGE(m_pLeftWing && m_pRightWing, "CVehicleMovementMPVTOL::PostInit - Wing parts not found");
}
...
void CVehicleMovementMPVTOL::ReceivedServerPathingData( const SVTOLPathPosParams& data )
{
	...
	//DrxLog( "[VTOL] CLUPDATE: path[%d] loc[%f]", data.pathId, data.location );
...
{
	//DrxWatch("VTOL: id[%d] path[%p] loc[%f] error[%f] wait[%f]", vtolId, pCachedPathPtr, pathingData.location, networkError, waitTime );
