...
DRX_IMPLEMENT_GTI(CRapid, CSingle);
...
//------------------------------------------------------------------------
void CRapid::Update(float frameTime, u32 frameId)
{
	...
	if ((m_speed >= m_fireParams->rapidparams.min_speed) && !(m_rapidFlags & eRapidFlag_decelerating))
	{
		...
		DRX_ASSERT(m_fireParams->fireparams.rate > 0);
...
//------------------------------------------------------------------------
void CRapid::StartReload(i32 zoomed)
{
	if (IsFiring())
		Accelerate(m_fireParams->rapidparams.deceleration);
	Firing(false);
...
//------------------------------------------------------------------------
i32 CRapid::GetShootAmmoCost(bool playerIsShooter)
{
	i32 cost = 1;
	...
	if (m_fireParams->fireparams.fake_fire_rate && playerIsShooter)
	{
		...
		{
			cost += drx_random(0, m_fireParams->fireparams.fake_fire_rate - 1);
		}
		else
		{
...
void CRapid::UpdateFiring(CActor* pOwnerActor, bool ownerIsClient, bool ownerIsPlayer, float frameTime)
{
	...
	if (m_speed >= 0.00001f)
	{			
		u32k decelAndRapid = eRapidFlag_decelerating|eRapidFlag_rapidFiring;
...
void CRapid::GetMemoryUsage(IDrxSizer * s) const
{
...
{
	i32 fragmentID = m_pWeapon->GetFragmentID("rapid_fire");
...
float CRapid::GetAmmoSoundParam()
{
	...
	{
		DrxWatch("ammo_left param %.2f", ammo);
	}
#endif
