...
#include <drx3D/Act/IItemSystem.h>	// For IItemParamsNode
...
void CSprintStamina::Debug( const IEntity* pEntity )
{
	if (g_pGameCVars->pl_movement.sprintStamina_debug)
	{
		...
		DrxWatch("Sprint Stamina: \"%s\": stamina = %.2f, regen delay = %.2f, reduced prev = %d", pEntity->GetName(), m_stamina, m_staminaRegenDelay, m_staminaReducedPrevUpdate);
	}
}
