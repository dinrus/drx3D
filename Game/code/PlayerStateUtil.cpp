...
$Id$
$DateTime$
Описание: Utility functions for the playerstate
...
void CPlayerStateUtil::FinalizeMovementRequest( CPlayer& player, const SActorFrameMovementParams & movement, SCharacterMoveRequest& request )
{
	...
	if (player.m_pAnimatedCharacter)
	{
		...
		DRX_ASSERT_TRACE(request.velocity.IsValid(), ("Invalid velocity %.2f %.2f %.2f for %s!", request.velocity.x, request.velocity.y, request.velocity.z, player.GetEntity()->GetEntityTextDescription()));
...
void CPlayerStateUtil::UpdateRemotePlayersInterpolation( CPlayer& player, const SActorFrameMovementParams& movement, SCharacterMoveRequest& frameRequest )
{
	...
	if (doVelInterpolation)
	{
		...
		if (isNetJumping)
		{
			...
			{
				DrxWatch("SetVel (%f, %f, %f)", player.m_jumpVel.x, player.m_jumpVel.y, player.m_jumpVel.z);
			}
#endif //_RELEASE
		...
		else
		{
			...
			IPhysicalEntity* pPhysEnt = player.GetEntity()->GetPhysics();
			DRX_ASSERT_MESSAGE(pPhysEnt, "Entity not physicalized! TomB would like to look at this.");
			...
			if (pPhysEnt && g_pGameCVars->pl_debugInterpolation)
			{
				...
				DrxWatch("Cur: (%f %f %f) Des: (%f %f %f)", dynStat.v.x, dynStat.v.y, dynStat.v.z, desiredVel.x, desiredVel.y, desiredVel.z);
				DrxWatch("Req: (%f %f %f) Type (%d)", frameRequest.velocity.x, frameRequest.velocity.y, frameRequest.velocity.z, frameRequest.type);
			}
#endif //!_RELEASE
		}
	}
...
bool CPlayerStateUtil::ShouldSprint( const CPlayer& player, const SActorFrameMovementParams& movement, IItem* pCurrentPlayerItem )
{
	...
	//DrxLogAlways("  restrictSprint = %d", restrictSprint);
	...
	{
		shouldSprint = movingForward && !restrictSprint && !isZooming;
		//DrxLogAlways("  shouldSprint = %d", shouldSprint);
	...
	//DrxLogAlways("  returning (for \"%s\") shouldSprint = %d", m_player.GetEntity()->GetName(), shouldSprint);
	return shouldSprint;
}
...
void CPlayerStateUtil::ApplyFallDamage( CPlayer& player, const float startFallingHeight, const float fHeightofEntity )
{
	...
	DRX_ASSERT(NumberValid(downwardsImpactSpeed));
	...
	if ((downwardsImpactSpeed > 0.0f) && (fallDist > MIN_FALL_DAMAGE_DISTANCE))
	{
		...
		DRX_ASSERT(NumberValid(velFraction));
