...
void CRecordingSystemDebug::PrintFirstPersonPacketData( u8* data, size_t size, tukk const msg ) const
{
...
void CRecordingSystemDebug::PrintFirstPersonPacketData( CRecordingBuffer& buffer, tukk const msg ) const
{
	if( g_pGameCVars->kc_debugPacketData )
	{
		RecSysLog(": FIRST PERSON KILL CAM DATA START [%s] :", msg);
		i32 size = (i32)buffer.size();
		i32 offset = 0;
		while( offset < size )
		{
...
void CRecordingSystemDebug::PrintThirdPersonPacketData( u8* data, size_t size, tukk const msg ) const
{
	if( g_pGameCVars->kc_debugPacketData )
	{
...
void CRecordingSystemDebug::PrintThirdPersonPacketData( CRecordingBuffer& buffer, tukk const msg ) const
{
	if( g_pGameCVars->kc_debugPacketData )
	{
		RecSysLog(": THIRD PERSON KILL CAM DATA START [%s] :", msg);
	...
	case eTPP_MannEvent:
		{
			...
			const SControllerDef* pControllerDef = gEnv->pGame->GetIGameFramework()->GetMannequinInterface().GetAnimationDatabaseUpr().LoadControllerDef("Animations/Mannequin/ADB/PlayerControllerDefs.xml");
