...
public:
	...
	template <class T>
	static ILINE void Serialise(CAdaptiveCompressor& compressor, uk  __restrict pInput, u32 stride, u32 numInputs)
	{
	...
	{
		compressor.m_stream.WriteVariableLengthValue((i32)in);
		return in;
	}
...
public:
	...
	static ILINE void Serialise(CAdaptiveCompressor& decompressor, S * __restrict pInput, u32 stride, u32 numInputs)
	{
...
SRecording_Packet* CRecordingSystemCompressor::SerialisePackets(CAdaptiveCompressor &serializer, SRecording_Packet *cur, char *outputBufferEnd)
{
	...
	while (true)
	{
		i32 packetType=T::SerialiseValue(serializer, cur->type);
		i32 packetCount=T::SerialiseValue(serializer, cur->size);
		...
		if (!T::IsWrite() && outputBufferEnd)
		{
			...
			default:
				DrxFatalError("Can't compress this type of packet %d %d\n", cur->type, packetType&0x7F);
				break;
			}
			if ((tuk)cur+packetSize*packetCount>outputBufferEnd)
			{
#ifdef _RELEASE
				DrxLogAlways("WARNING: Output buffer isn't big enough! Abandoning decompress so we don't overrun our buffer!");
#else
				DrxFatalError("WARNING: Output buffer isn't big enough! Abandoning decompress so we don't overrun our buffer!");
#endif
		...
		cur=(SRecording_Packet*)((tuk)cur+cur->size*packetCount);
	}

	return cur;
}

SRecording_Packet* CRecordingSystemCompressor::SortPackets(SRecording_Packet *dstBuffer, CRecordingBuffer *srcBuffer, u8 *pSrcAuxBuffer, u32 srcAuxBufferSize, SRecording_VictimPosition* startVictimPosition, SRecording_VictimPosition* endVictimPosition, EntityId killHitVictimID)
{
	u32 total[eFPP_Max]={0};
	size_t offset=0;
	
	i32 sortOrder[eFPP_Max]={eFPP_FPChar, eFPP_VictimPosition, eFPP_KillHitPosition};
	i32 sortOrderPos=0;
	sortOrder[sortOrderPos++]=eFPP_FPChar;
	sortOrder[sortOrderPos++]=eFPP_VictimPosition;
	sortOrder[sortOrderPos++]=eFPP_KillHitPosition;
	for (i32 i=0; i<eFPP_Max; i++)
	{
	...
	if (srcBuffer)
	{
		...
		total[eFPP_VictimPosition]=(u32)(endVictimPosition-startVictimPosition);
	...
	for (i32 idx=0; idx<DRX_ARRAY_COUNT(total); idx++)
	{
		i32 i=sortOrder[idx];
		if (total[i])
		{
			...
			case eFPP_VictimPosition:
				if (srcBuffer)
				{
					...
					dstBuffer=(SRecording_Packet*)((tuk)dstBuffer+victimPositionSize);
					break;
				}
				// no break
			case eFPP_KillHitPosition:
				if (srcBuffer)
				{
					...
					while (offset < srcBuffer->size())
					{
						...
						if (pPacket->type==eFPP_KillHitPosition && killHitVictimID==((SRecording_KillHitPosition*)pPacket)->victimId)
						{
							...
							dstBuffer=(SRecording_Packet*)((tuk)dstBuffer+pPacket->size);
							break;
						}
						offset += pPacket->size;
					}
...
struct SPacketSortObject
{
	float GetFrameTime(SRecording_Packet *p)
	{
		...
		default:
			DrxFatalError("Don't know how to reorder packet type %d\n", p->type);
		}
		return 0.0f;
	}
...
SRecording_Packet* CRecordingSystemCompressor::MovePacketsToBuffer(EFirstPersonPacket type, SRecording_Packet *dstBuffer, SRecording_Packet *srcBuffer, SRecording_Packet *endOfSrc)
{
	...
	while (srcBuffer<endOfSrc)
	{
		...
		{
			size_t size=(tuk)srcBuffer-(tuk)pStart;
			memcpy(dstBuffer, pStart, size);
			dstBuffer=(SRecording_Packet*)((tuk)dstBuffer+size);
			pStart=NULL;
		}
		srcBuffer=(SRecording_Packet*)((tuk)srcBuffer+srcBuffer->size);
	}
	if (pStart)
	{
		size_t size=(tuk)endOfSrc-(tuk)pStart;
		memcpy(dstBuffer, pStart, size);
		dstBuffer=(SRecording_Packet*)((tuk)dstBuffer+size);
	}
	return dstBuffer;
...
void CRecordingSystemCompressor::InterleavePackets(SRecording_Packet *dstBuffer, SRecording_Packet *srcBuffer, SRecording_Packet *endOfSrc)
{
	...
	for (std::vector<SRecording_Packet*>::iterator it=packetsForOrdering.begin(), end=packetsForOrdering.end(); it!=end; ++it)
	{
		...
		dstBuffer=(SRecording_Packet*)((tuk)dstBuffer+p->size);
	}
}

size_t CRecordingSystemCompressor::Compress(CRecordingBuffer *srcBuffer, u8* outBuffer, u32 maxOutputSize, SRecording_VictimPosition* startVictimPosition, SRecording_VictimPosition* endVictimPosition, EntityId killHitVictimID)
{
	i32 inputSize=srcBuffer->size()+(endVictimPosition-startVictimPosition)*sizeof(SRecording_VictimPosition);
	u8 *sortedPacketBuffer=new u8[inputSize+(eFPP_Max+1)*sizeof(SRecording_Packet)];
	...
	if ((tuk)pEndPacket>(tuk)sortedPackets+maxOutputSize)
		DrxLogAlways("Input size (%" PRISIZE_T ") exceeds output buffer size (%d). Will be truncated when decompressed!\n", (tuk)pEndPacket-(tuk)sortedPackets, maxOutputSize);
	...
	size_t compressedSize=(u8*)compressor.m_stream.GetEnd()-outBuffer;
#ifndef _RELEASE
	DrxLogAlways("KillCam Compressed Data %" PRISIZE_T "/%d (%.1f%%)\n", compressedSize, inputSize, 100.0f*compressedSize/(float)inputSize);
#endif
	return compressedSize;
}

size_t CRecordingSystemCompressor::CompressRaw(u8 *inBuffer, u32 inBufferSize, u8* outBuffer, u32 maxOutputSize)
{
	u8 *sortedPacketBuffer=new u8[inBufferSize+(eFPP_Max+1)*sizeof(SRecording_Packet)];
	SRecording_Packet *sortedPackets=(SRecording_Packet*)sortedPacketBuffer;
...
size_t CRecordingSystemCompressor::Decompress(u8 *compressedInput, size_t compressedSize, uk outputBuffer, size_t maxOutputSize, size_t streamAlignment)
{
	u8* sortedPacketBuffer=new u8[maxOutputSize];
	SRecording_Packet *sortedPackets=(SRecording_Packet*)sortedPacketBuffer;
	...
	{
		end=SerialisePackets<DecompressionSerializer>(decompressor, end, (tuk)sortedPackets+maxOutputSize);
		...
		DrxLog("Decompressed chunk: %p\n", decompressor.m_stream.m_out);
	}

#ifndef _RELEASE
	if ((size_t)((tuk)end-(tuk)sortedPackets)>maxOutputSize)
		DrxFatalError("Decompressed data size (%" PRISIZE_T ") exceeds output buffer size (%" PRISIZE_T "). This should not be possible due to truncation in SerialisePackets\n", ((tuk)end-(tuk)sortedPackets), maxOutputSize);
#endif

	InterleavePackets((SRecording_Packet*)outputBuffer, sortedPackets, end);
	size_t result = (tuk)end-(tuk)sortedPackets;
	delete[] sortedPackets;
