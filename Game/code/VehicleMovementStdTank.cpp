...
$Id$
$DateTime$
Описание: Implements movement type for tracked vehicles
...
{
	COMPILE_TIME_ASSERT(DRX_ARRAY_COUNT(m_drivingWheels) == 2);
...
{ 
  ...
  i32 currGear = m_vehicleStatus.iCurGear - 1; // indexing for convenience: -1,0,1,2,..
...
void CVehicleMovementTank::OnEvent(EVehicleMovementEvent event, const SVehicleMovementEventParams& params)
{
  ...
  { 
    ...
    i32 numTreads = m_treadParts.size();
    i32 treadsPrev = m_blownTires;
...
//------------------------------------------------------------------------
void CVehicleMovementTank::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
...
void CVehicleMovementTank::GetMemoryUsage(IDrxSizer * pSizer) const
{
	pSizer->Add(*this);
