...
public:
	...
	{
		IAttachment *oldAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByName("weapon");
		IAttachment *newAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByName("left_weapon");
	...
	{
		IAttachment *oldAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByName("left_weapon");
		IAttachment *newAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByName("weapon");
...
CPlayerBackgroundAction::CPlayerBackgroundAction(i32 priority, FragmentID fragID)
	:
...
CPlayerMovementAction::CPlayerMovementAction(i32 priority)
...
//				DrxWatch("Turn Angle %.4f %.4f %.1f %s", aimAngle, signedAngle, m_lastTurnDirection, m_spinning ? "SPINNING" : "");
			}
		}
	}
	else
