...
void CVehicleDamageBehaviorBurn::Activate(bool activate)
{
  if (activate && !m_isActive)
  {
    ...
    m_timerId = m_pVehicle->SetTimer(-1, 20000, this); // total burn of 60 secs
		...
		if( !m_pVehicle->IsDestroyed() && !m_pVehicle->IsFlipped() && pAISystem && pAISystem->IsEnabled() && pAISystem->GetSmartObjectUpr() )
		{
			gEnv->pAISystem->GetSmartObjectUpr()->SetSmartObjectState(m_pVehicle->GetEntity(), "Exploding");        
		}
