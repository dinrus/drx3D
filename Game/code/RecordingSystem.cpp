...
#include <drx3D/Game/DrxAction.h>
...
#include <drx3D/Game/RecordingSystemCompressor.h>
#include <drx3D/Game/GamePhysicsSettings.h>
#include <drx3D/Game/RecordingSystemDebug.h>
...
tukk NIGHT_VISION_PE		= "NightVision_Active";



// --------------------------------------------------------------------------------
static void DiscardingPacketStatic(SRecording_Packet *ps, float recordedTime, uk inUserData)
{
...
#ifdef RECSYS_DEBUG
// --------------------------------------------------------------------------------
void CRecordingSystem::FirstPersonDiscardingPacketStatic(SRecording_Packet *ps, float recordedTime, uk inUserData)
{
	...
	if( !s_savingHighlightsGuard && ageOfPacket<maxBufferTimeRequired && ((CRecordingSystem*)inUserData)->IsRecordingAndNotPlaying() )
	{
		...
		DRX_ASSERT_MESSAGE(false, "Recording first person buffer is out of memory, please tell Pete");
	}
}
...
void CRecordingSystem::DiscardingPacket(SRecording_Packet *packet, float recordedTime)
{
	...
	if( !s_savingHighlightsGuard && ageOfPacket<maxBufferTimeRequired && IsRecordingAndNotPlaying() )
	{
		...
		DRX_ASSERT_MESSAGE(false, "Recording third person buffer is out of memory, please tell Pete");
	}
#endif //RECSYS_DEBUG
	...
	case eTPP_PlaySound:
		{
			...
			//	DRX_ASSERT_MESSAGE(m_recordedData.m_discardedSounds.size() < 64, "We have recorded a lot of discarded looping sounds without stopping them, something may have gone wrong here");
			//}
		}
		break;
	case eTPP_StopSound:
	...
	case eTPP_CorpseSpawned:
		{
			DRX_ASSERT(m_recordedData.m_corpses.size() < m_recordedData.m_corpses.max_size());
				...
				if(IEntity* pCorpseEntity = gEnv->pEntitySystem->GetEntity(pCorpsePacket->corpseId))
				{
					if(ICharacterInstance* pChar = pCorpseEntity->GetCharacter(0))
					{
						...
						if(pPose)
						{
						  ...
						  DRX_ASSERT(rIDefaultSkeleton.GetJointCount() <= STrackedCorpse::k_maxJoints);
							corpse.m_corpseId = pCorpsePacket->corpseId;
							corpse.m_numJoints = min((i32) rIDefaultSkeleton.GetJointCount(), static_cast<i32>(STrackedCorpse::k_maxJoints));

							static u8 s_ADIKJoints[STrackedCorpse::k_maxJoints];
							...
							{
								i16k jointIdx = rIDefaultSkeleton.GetJointIDByCRC32(*it);
								if(jointIdx!=-1)
									s_ADIKJoints[jointIdx] = 1;
							}
							for (i32 i=0; i<corpse.m_numJoints; i++)
							{
								if(s_ADIKJoints[i])
								{
									QuatT rel(pPose->GetRelJointByID(i));
									rel.t.x = 0.f;
									i32k parent = rIDefaultSkeleton.GetJointParentIDByID(i);
									if(parent<0)
									{
	...
	case eTPP_CorpseRemoved:
		{
			...
			i32 numCorpses = m_recordedData.m_corpses.size();
			for (i32 i = 0; i < numCorpses; ++ i)
			{
	...
	if (playerEntityId != 0)
	{
		if (packet->type == eTPP_PlayerJoined)
		{
			...
			for (i32 i = 0; i < MAX_RECORDED_PLAYERS; ++ i)
			{
							...
							case SMannHistoryItem::Fragment:
								{
									...
									for (u32 j=0; j<SPlayerInitialState::MAX_FRAGMENT_TRIGGERS; j++)
									{
...
i32 CRecordingSystem::m_replayEventGuard = 0;
i32 CRecordingSystem::s_savingHighlightsGuard = 0;
	...
	,m_gameCameraLinkedToEntityId(0)
{
	...
	if (gEnv->pParticleUpr)
		gEnv->pParticleUpr->AddEventListener(this);
	...
	for (i32 i = 0; i < MAX_HIGHLIGHTS; ++ i)
	{
	...
	if (IBreakableUpr* pBreakMgr = gEnv->pEntitySystem->GetBreakableUpr())
	...
	IAnimationDatabaseUpr& adbUpr = gEnv->pGame->GetIGameFramework()->GetMannequinInterface().GetAnimationDatabaseUpr();
	m_pDatabaseInteractiveObjects = adbUpr.Load("Animations/Mannequin/ADB/interactiveObjects.adb");
	m_pPlayerControllerDef = adbUpr.LoadControllerDef("Animations/Mannequin/ADB/PlayerControllerDefs.xml");
	const IAnimationDatabase* pDatabasePlayer = adbUpr.Load("Animations/Mannequin/ADB/PlayerAnims1P.adb");
...
void CRecordingSystem::Init( XmlNodeRef root )
{
	...
	if(particles)
	{
		if(IParticleUpr* pParticleMan = gEnv->pParticleUpr)
		{
			XmlNodeRef excluded = particles->findChild("Excluded");
			if(excluded)
			{
				i32k count = excluded->getChildCount();
				for(i32 i=0; i<count; i++)
				{
			...
			if(replace)
			{
				i32k count = replace->getChildCount();
				for(i32 i=0; i<count; i++)
				{
			...
			if(persistant)
			{
				i32k count = persistant->getChildCount();
				for(i32 i=0; i<count; i++)
				{
		...
		{
			DrxFatalError("No Particle manager could be found for RecordingSystem XML Parsing.");
		}
	}
...
/*static*/ void CRecordingSystem::LoadClassData(const IEntityClassRegistry& classReg, XmlNodeRef xmlData, TEntityClassSet& data)
{
	if(xmlData)
	{
		i32k numChildren = xmlData->getChildCount();
		for(i32 i=0; i<numChildren; ++i)
		{
			...
			if(child->isTag("class"))
			{
				...
				else
				{
					...
					DRX_ASSERT_MESSAGE(false, "Could not find class.");
				}
			}
		}
	}
...
void CRecordingSystem::OnStartGame()
{
	...
	for (i32 seq = 0; seq < numSequences; ++ seq)
	{
		IAnimSequence* pSequence = pMovieSystem->GetSequence(seq);
		if (pSequence)
		{
			i32 numNodes = pSequence->GetNodeCount();
			for (i32 node = 0; node < numNodes; ++ node)
			{
...
void CRecordingSystem::Reset(void)
{
	...
	u32k numListeners = m_mannequinListeners.size();
	for (u32 i=0; i<numListeners; i++)
	{
...
{
	for(i32 i = 0; i < RECORDING_SYSTEM_MAX_SLOTS; i++)
	{
...
CRecordingSystem::~CRecordingSystem()
{
	...
	if (gEnv->pParticleUpr)
		gEnv->pParticleUpr->RemoveEventListener(this);

	if (IEntitySystem* pEntSys = gEnv->pEntitySystem)
	{
		pEntSys->RemoveSink(this);
		
		if (IBreakableUpr* pBreakMgr = pEntSys->GetBreakableUpr())
...
template <u32 N>
void CompressVictimData(DrxFixedArray<SRecording_VictimPosition, N> &victimPositions, float deathTime)
{
	...
	for (i32 i = 1; i < (i32)victimPositions.size(); i++)
	{
		...
		if (current->frametime - prev->frametime > 0.01f)
		{
			...
			else
			{
				...
				for (i32 j = (prevKeptIndex + 1); j <= i; j++)
				{
...
void CRecordingSystem::GetTPCameraData(float startTime)
{
	while (m_pBuffer->size() > 0)
	{
		...
		{
			DRX_ASSERT_MESSAGE(false, "The first packet in the buffer should always be of type eRBPT_FrameData");
		}
...
size_t CRecordingSystem::GetFirstPersonDataForTimeRange(u8 **data, float fromTime, float toTime)
{
	bool bCopying=false;
	size_t offset=0;
	size_t datasize = 0;
	...
	while (offset < m_pBufferFirstPerson->size())
	{
		...
		if (pPacket->type == eFPP_FPChar)
		{
			...
			if (bCopying)
			{
				...
				DRX_ASSERT_MESSAGE(datasize < sizeof(m_firstPersonSendBuffer), "Ran out of memory in m_firstPersonSendBuffer");
			}
		}
		offset += pPacket->size;
	}
...
size_t CRecordingSystem::GetVictimDataForTimeRange(u8 **data, EntityId victimID, float fromTime, float toTime, bool bulletTimeKill, float timeOffset)
{	
	size_t offset=0;
	size_t datasize = 0;
	bool bInRange=false;
	// Get victim's position data
	DrxFixedArray<SRecording_VictimPosition, 500> victimPositions;
...
size_t CRecordingSystem::GetFirstPersonData(u8 **data, EntityId victimId, float deathTime, bool bulletTimeKill, const float length)
{	
...
template <u32 N>
void CRecordingSystem::ExtractVictimPositions(DrxFixedArray<SRecording_VictimPosition, N> &victimPositions, float startTime, EntityId victimId, float deathTime, float endTime)
{
	float frametime = 0;
	for (CRecordingBuffer::iterator itPacket = m_pBuffer->begin(); itPacket != m_pBuffer->end(); ++itPacket)
	{
...
/*static*/ void CRecordingSystem::SetFirstPersonData(u8 *data, size_t datasize, SPlaybackInstanceData& rPlaybackInstanceData, SFirstPersonDataContainer& rFirstPersonDataContainer)
{
	DRX_ASSERT_MESSAGE(datasize <= sizeof(rFirstPersonDataContainer.m_firstPersonData), "Ran out of memory in m_firstPersonData");
	...
	if (rFirstPersonDataContainer.m_firstPersonDataSize > 0)
	{
		...
		{
			DRX_ASSERT_MESSAGE(false, "The first packet in the first person data buffer should be of type eFPP_FPChar");
		}
	}
	...
	u8* pPos = rFirstPersonDataContainer.m_firstPersonData;
...
void CRecordingSystem::OnBreakEvent(u16 uBreakEventIndex)
{
...
void CRecordingSystem::OnPartRemoveEvent(i32 iPartRemovalEventIndex)
{
	BreakLogAlways("OnPartRemoveEvent(), PartRemoveEvent Index: %d", iPartRemovalEventIndex);
}

void CRecordingSystem::OnEntityDrawSlot(IEntity * pEntity, i32 slot, i32 flags)
{
...
void CRecordingSystem::OnEntityChangeStatObj(IEntity * pEntity, i32 iBrokenObjectIndex, i32 slot, IStatObj * pOldStatObj, IStatObj * pNewStatObj)
{
	EntityId entityId = pEntity->GetId();
...
void CRecordingSystem::OnSetSubObjHideMask(IEntity *pEntity, i32 nSlot, hidemask nSubObjHideMask)
{
	EntityId entityId = pEntity->GetId();
	ClearBreakageSpawn(pEntity, entityId);
...
bool CRecordingSystem::QueueStartPlayback( const SPlaybackRequest& request )
{
	...
	if (!request.highlights)
	{
		...
		tukk killerName = pKiller ? pKiller->GetName() : "unknown";
		tukk victimName = pVictim ? pVictim->GetName() : "unknown";
		...
		RecSysLog("QueueStartPlayback: killer: %s(%d), victim: %s(%d), projectileId: %d, hitType: %d, impulse: %f, %f, %f, deathtime: %f, bulletTimeKill: %d, pProjectileClass: %s",
			...
			request.kill.deathTime, (i32)request.kill.bulletTimeKill, 
	...
	if (request.kill.winningKill)
	{
		...
		winningKillCam.AddData((i32)pGameRules->GetTeam(request.kill.killerId));
	...
	if (CGameLobby* pLobby = g_pGame->GetGameLobby())
	{
		for (i32 i = 0; i < MAX_RECORDED_PLAYERS; ++ i)
		{
			...
			if (initialState.playerId != 0)
			{
				...
				{
					u16 channelId = pPlayer->GetChannelId();

					u8 rank=0, reincarnations=0;
...
// 		duration of the kill cam, then triggers DrxAction to make copies of them so that they can
...
void CRecordingSystem::RevertBrokenObjectsToStateAtTime(float fTime)
{
	...
	if(gEnv->pEntitySystem && gEnv->pEntitySystem->GetBreakableUpr())
	{
		...
		i32 iNumProceduralBreakEvents = m_breakEventIndicies.size();
		...
		if(iNumProceduralBreakEvents > 0)
		{
			...
			i32k kFirstProceduralBreakEventIndex = m_breakEventIndicies[0];
			u16 * pBreakEventIndicies					= &(m_breakEventIndicies[0]);

			m_clonedNodes.resize(iNumProceduralBreakEvents);

			IRenderNode ** pClonedNodes = &(m_clonedNodes[0]);
			i32 iNumClonedNodes = 0;
...
void CRecordingSystem::GetProceduralBreaksDuringKillcam(std::vector<u16>& indices)
{
	u8 *startingplace = m_pTPDataBuffer;
	u8 *endingplace = startingplace + m_tpDataBufferSize;
...
{
	u16 * pBreakEventIndices = m_breakEventIndicies.empty() ? NULL : &(m_breakEventIndicies[0]);
	i32k iNumBreakEventIndices = m_breakEventIndicies.size();
	...
	i32k iNumClonedNodes = m_clonedNodes.size();
	for(i32 i = 0; i < iNumClonedNodes; i++)
	{
...
void CRecordingSystem::StopPlayback()
{
	...
	tukk killerName = "unknown";
	tukk victimName = "unknown";
...

	{
		...
		for (TReplayParticleMap::iterator itParticle=m_replayParticles.begin(); itParticle!=m_replayParticles.end(); ++itParticle) 
		{
			gEnv->pParticleUpr->DeleteEmitter(itParticle->second);
			itParticle->second->Release();
		}
		m_replayParticles.clear();
	...
	SetTimeScale(1.f);
	gEnv->pParticleUpr->SetTimer(gEnv->pTimer);
...
void CRecordingSystem::ApplyWeaponAccessories(const SRecording_WeaponAccessories *weaponAccessories)
{
	...
	if (pItem)
	{
		...
		for (i32 i=0; i<MAX_WEAPON_ACCESSORIES; i++)
		{
	...
	if (pReplayEntity && weaponSelect->weaponId && weaponSelect->isSelected)	
	{
		...
		if (pItemEnt)
		{
			...
			if (weaponSelect->ownerId == m_killer)
			{
				...
				weaponSwitch.AddData((i32)itemEntId);
		...
		{
			RecSysLog("ApplyWeaponSelect Unable to select weapon, replay weapon with id %d not found! ownerId = %d, isRippedOff = %d", weaponSelect->weaponId, weaponSelect->ownerId, (i32)weaponSelect->isRippedOff);
			DRX_ASSERT_MESSAGE(false, "Unable to select weapon, replay weapon not found");
		}
	}
}
...
void CRecordingSystem::ApplyMannequinSetSlaveController(const SRecording_MannSetSlaveController *pMannSetSlave)
{
	...
	if (pMasterActor && pSlaveActor)
	{
		...
		{
			const IAnimationDatabaseUpr& animationDatabase = gEnv->pGame->GetIGameFramework()->GetMannequinInterface().GetAnimationDatabaseUpr();
...
void CRecordingSystem::HideEntityKeepingPhysics(IEntity *pEntity, bool hide)
{
	...
	for (i32 i = 0; i < ENTITY_PROXY_LAST; i++)
	{
	...
	i32k childCount = pEntity->GetChildCount();
...
bool CRecordingSystem::GetWeaponAudioCacheDetailsForWeaponId(EntityId inWeaponId, SWeaponAudioCacheDetails &outDetails)
{
	...
	if (itDiscardedSpawn != m_pPlaybackData->m_discardedEntitySpawns.end())
	{
		...
		DRX_ASSERT(pEntitySpawn->entityId == inWeaponId);
	...
	else
	{
		...
		u8 *pPos = m_pPlaybackData->m_tpdatabuffer;
		u8 *pEnd = m_pPlaybackData->m_tpdatabuffer + m_pPlaybackData->m_tpdatasize;		
		while( pPos < pEnd )
		{
	...
	if (foundWeapon)
	{
		...
		if (itDiscardedAccessory != m_pPlaybackData->m_discardedWeaponAccessories.end())
		{
			...
			DRX_ASSERT(pWeaponAccessory->weaponId == inWeaponId);
			for (i32 i=0; i<MAX_WEAPON_ACCESSORIES; i++)
			{
...
{
	i32 len=m_cachedWeaponAudioDetails.size();
	for (i32 i=0; i<len; i++)
	{
		...
		// handle accessories
		for (i32 j=0; j<MAX_WEAPON_ACCESSORIES; j++)
		{
...
void CRecordingSystem::HandleAudioCachingOfKillersWeapons()
{
	...
	for (i32 i = 0; i < MAX_RECORDED_PLAYERS; ++ i)
	{
		...
		{
			DRX_ASSERT(initialState.weapon.ownerId == m_killer);
	...
	while( pPos < pEnd )
	{
		...
		if (pPacket->type == eTPP_WeaponSelect)
		{
			...
			if (pWeaponSelect->ownerId == m_killer)
			{
				...
				for (u32 i=0; i<m_cachedWeaponAudioDetails.size(); i++)
				{
	...
	case ePSS_AudioAndTracers:
		{
			...
			{
				SetTimeScale(1.f);
				gEnv->pParticleUpr->SetTimer(m_replayTimer.get());
			...
			g_pGame->GetWeaponSystem()->GetTracerUpr().ClearCurrentActiveTracers();

			// Filter out all sounds except replay & ambient sounds
			//Takes approx 0.5ms

			//gEnv->pSoundSystem->SetPlaybackFilter((i32)(eSoundSemantic_Replay | m_ambientSoundSemantics));
	...
	case ePSS_SpawnNewEntities:
		{
			...
			//	This takes ~20ms
			i32k numCorpses = m_pPlaybackData->m_corpses.size();
					...
					for (i32 i = 0; i < numCorpses; ++ i)
					{
					  STrackedCorpse &corpse = m_pPlaybackData->m_corpses[i];
						if (corpse.m_corpseId == pEntitySpawn->entityId)
						{
							...
							{
								DRX_ASSERT(pEntitySpawn->szCharacterSlot[0]);
	...
	case ePSS_HandleParticles:
		{
			...
			gEnv->pParticleUpr->DeleteEmitters( [](IParticleEmitter& e) { return e.GetSpawnParams().bNowhere; } );
	...
	case ePSS_SpawnRecordingSystemActors:
		{
			...
			assert(pEntityClass);
			i32 killerTeam = pGameRules->GetTeam(m_killer);
			...
			for (i32 i = 0; i < MAX_RECORDED_PLAYERS; ++ i)
			{
				...
				if (initialState.playerId && initialState.playerJoined.bIsPlayer)
				{
					...
					if (pCloneEntity)
					{
						...
						tukk pModelName = initialState.changedModel.pModelName;
						if (pModelName)
						{
							...
							i32k team = initialState.teamChange.entityId ? initialState.teamChange.teamId : 0;
							...
							i32 rank = (i32) initialState.playerJoined.rank;
							i32 reincarnations = (i32) initialState.playerJoined.reincarnations;
							...
							// Setup the replay actor's data.
							{
								...
								if(pTeamVisUpr)
								{
									pTeamVisUpr->RefreshTeamMaterial(pCloneEntity, true, isFriendly ); 
								}
	...
	case ePSS_ReInitHUD:
		{
			...
			killCamStart.AddData(static_cast<uk>(&m_replayActors));
			killCamStart.AddData((i32)m_killer);
...
void CRecordingSystem::FillOutFPCamera(SRecording_FPChar *cam, float playbacktime)
{
	...
	i32 loweroffset = upperoffset - 1;
	if (loweroffset >= 0)
	{
		DRX_ASSERT_MESSAGE(pFPCharData[loweroffset].type == eFPP_FPChar, "This packet type is not valid");
		DRX_ASSERT_MESSAGE(pFPCharData[upperoffset].type == eFPP_FPChar, "This packet type is not valid");
		...
		DRX_ASSERT_MESSAGE(timediff != 0, "The time difference must not be 0");
...
bool CRecordingSystem::GetVictimPosition(float playbackTime, Vec3& victimPosition) const
{
	...
	while (true)
	{
		if ((u8*)(pVictimData + upperoffset) >= m_pPlaybackData->m_FirstPersonDataContainer.m_firstPersonData + m_pPlaybackData->m_FirstPersonDataContainer.m_firstPersonDataSize || pVictimData[upperoffset].type != eFPP_VictimPosition)
		{
			upperoffset--;
			break;
		}
	...
	if (loweroffset >= 0)
	{
		DRX_ASSERT_MESSAGE(pVictimData[loweroffset].type == eFPP_VictimPosition, "This packet type is not valid");
		DRX_ASSERT_MESSAGE(pVictimData[upperoffset].type == eFPP_VictimPosition, "This packet type is not valid");
...
/*static*/ const SRecording_Packet * CRecordingSystem::GetFirstFPActionPacketOfType(const SFirstPersonDataContainer& rFirstPersonData, const SPlaybackInstanceData& rPlaybackInstanceData, u8 type)
{
	u8k* pPos = rFirstPersonData.m_firstPersonData + rPlaybackInstanceData.m_firstPersonActionsOffset;
...
void CRecordingSystem::PlaybackRecordedFirstPersonActions(float playbacktime)
{
	...
	u8* pPos = m_pPlaybackData->m_FirstPersonDataContainer.m_firstPersonData + m_PlaybackInstanceData.m_firstPersonActionsOffset;
	while (pPos < m_pPlaybackData->m_FirstPersonDataContainer.m_firstPersonData + m_pPlaybackData->m_FirstPersonDataContainer.m_firstPersonDataSize)
	{
		...
		case eFPP_PlayerHealthEffect:
			...
			break;
		default:
			DRX_ASSERT_MESSAGE(false, "Unrecognised packet type found");
			break;
		}
		if (packetProcessed)
		{
...
void CRecordingSystem::DropTPFrame()
{
	...
	m_tpDataBufferSize = (u32) (endingplace - startingplace);
}
...
void CRecordingSystem::QueueAddPacket(const SRecording_Packet &packet)
{
	if (CRecordingSystem::KillCamEnabled())
	{
		...
		{
			DRX_ASSERT_MESSAGE(false, "Ran out of memory queueing packets");
		}
	}
}
...
{	
	...
	if (CRecordingSystem::KillCamEnabled() && g_pGameCVars->kc_debugStressTest)
	{
		if (m_stressTestTimer <= 0.f)
		{
			...
			PlayHighlight(0);
			m_stressTestTimer = drx_random(30.0f, 60.0f);
		}
		else
	...
	if(IsPlaybackQueued())
	{
		...
		if( !dataReady )
		{
			...
			{
				DrxFatalError("We should always expect data if we get to here. QueueStartPlayback() should be the only place that sets StreamData expectations, and sets the m_playbackState to ePS_Queued.");
			}
		}
...
void CRecordingSystem::UpdateBulletPosition()
{
	...
	if (timeFromDeath > -m_bulletTravelTime)
	{
		if (m_bulletEntityId == 0 && m_bulletTimePhase == eBTP_Approach)
		{
			...
			IParticleEffect *pParticleEffect = gEnv->pParticleUpr->FindEffect(m_bulletEffectName);
				...
				if (m_timeSincePlaybackStart >= m_bulletHoverStartTime)
				{
					...
					else
					{
						if (m_bulletTimePhase != eBTP_PostHover)
						{
							...
							if (pTracerParams)
							{
								CTracerUpr::STracerParams params;
								...
								g_pGame->GetWeaponSystem()->GetTracerUpr().EmitTracer(params, m_bulletEntityId);
							}
...
void CRecordingSystem::UpdatePlayback(const float frameTime)
{
	...
	i32k MaxIterations = g_pGameCVars->kc_maxFramesToPlayAtOnce;
	i32 i;
...
{
	DRX_ASSERT_MESSAGE(lerpValue >= 0.f && lerpValue <= 1.f, "Lerp value should be between 0 and 1");
	...
	DrxFixedArray<SRecording_TPChar, MAX_RECORDED_PLAYERS> tpCharPackets[2];
	DrxFixedArray<SRecording_EntityLocation, 256> entityLocationPackets[2];
...
ExitLoop:
	...
	for (i=0; i < (i32)tpCharPackets[0].size(); ++i)
	{
		SRecording_TPChar* lowerPacket = &tpCharPackets[0][i];
		prevJ = j;
		foundMatch = false;
		while (j < (i32)tpCharPackets[1].size())
		{
	...
	i = 0;
	j = 0;
	for (i=0; i < (i32)entityLocationPackets[0].size(); ++i)
	{
		SRecording_EntityLocation* lowerPacket = &entityLocationPackets[0][i];
		prevJ = j;
		foundMatch = false;
		while (j < (i32)entityLocationPackets[1].size())
		{
...
float CRecordingSystem::PlaybackRecordedFrame(float frameTime)
{
	...
	while (startingplace < endingplace)
	{
		...
		default:
			{
				DRX_ASSERT_MESSAGE(false, "KillCam replay unrecognised packet type");
				break;
			}
		}
	...
	case SPlaybackInfo::eVM_FirstPerson:
		{
			...
			{
				i32k slotIndex = 0;
				u32k killerEntityFlags = pKillerEntity->GetSlotFlags(slotIndex);
	...
	case SPlaybackInfo::eVM_BulletTime:
		{
			...
			case eBTP_PostHit:
				...
				break;
			default:
				DRX_ASSERT_MESSAGE(false, "Unrecognised bullet time phase");
				break;
			}
		}
		break;
...
void CRecordingSystem::UpdateThirdPersonPosition(const SRecording_TPChar *tpchar, float frameTime, float playbackTime, const SRecording_TPChar *tpchar2, float lerpValue)
{
	...
	if (pReplayActor)
	{
		...
		u16 prevRAFlags = pReplayActor->m_flags;

#define SetFlags(o,i,oF,iF) { u32k mask = -iszero(i&iF); o = ((o|((~mask)&oF))&~(mask&oF)); }
...
#undef SetFlags
		...
		if(tpchar->eid==m_killer)
		{
			...
			if(((prevRAFlags^pReplayActor->m_flags)&eRAF_StealthKilling)!=0)
			{
				...
				{
					SHUDEvent weaponSwitch(eHUDEvent_OnKillCamWeaponSwitch);
					weaponSwitch.AddData((i32)pReplayActor->GetGunId());
					weaponSwitch.AddData(false);
					CHUDEventDispatcher::CallEvent(weaponSwitch);
				}
				else
		...
		if( isRagdoll != shouldBeRagdoll )
		{
			...
			else
			{
				...
				u8 *startingplace = m_pPlaybackData->m_tpdatabuffer;		// Annoyingly we can't use the current buffer pos since the packet ordering is not guaranteed
				u8 *endingplace = startingplace + m_pPlaybackData->m_tpdatasize;
	...
	if (pReplayEntity)
	{
		...
		if (pRenderProxy)
		{
			...
			if (pReplayActor && (isCloaked != wasCloaked))
			{
				...
				u8k cloakColorChannel = EntityEffects::Cloak::GetCloakColorChannel(isFriendly);
...
void CRecordingSystem::CloakEnable(IEntityRenderProxy* pRenderProxy, bool enable, bool fade)
{
	...
	u8 mask = pRenderProxy->GetMaterialLayersMask();
	u32 blend = pRenderProxy->GetMaterialLayersBlend();
	if (!fade)
	{
...
void CRecordingSystem::ApplyEntitySpawn(const SRecording_EntitySpawn *entitySpawn, float time)
{
	...
	if (entitySpawn->entityId == 0 || m_replayEntities.count(entitySpawn->entityId) > 0)
	{
		...
		DRX_ASSERT_MESSAGE(entitySpawn->entityId == 0, "CRecordingSystem::ApplyEntitySpawn Shouldn't be trying to spawn the same entity twice!");
		return;
	}

	SEntitySpawnParams params;
	tukk sClassType = "ReplayObject";
	...
	char sName[64];
	drx_sprintf(sName, "%s%s%d", entitySpawn->useOriginalClass?"Replay":"", params.pClass->GetName(), m_nReplayEntityNumber);
		...
		if(!entitySpawn->useOriginalClass)
		{
			for(i32 i = 0; i < RECORDING_SYSTEM_MAX_SLOTS; i++)
			{
				if (entitySpawn->szCharacterSlot[i])
				{
					...
					i32 length = strlen(entitySpawn->szCharacterSlot[i]);
					tukk pExtension = entitySpawn->szCharacterSlot[i] + length - 3;
					bool validString = (stricmp(pExtension, "cga") == 0 || stricmp(pExtension, "cdf") == 0 || stricmp(pExtension, "chr") == 0);
					DRX_ASSERT_MESSAGE(validString, "The string appears to be corrupt (please tell martinsh)");
#endif
		...
		if(entitySpawn->pClass == m_pInteractiveObjectExClass)
		{
			...
			if(result != m_interactiveObjectAnimations.end())
			{
				if (m_pDatabaseInteractiveObjects && m_pPlayerControllerDef && pReplayObject && pGameObject)
				{
					...
					u32k scopeContextID = PlayerMannequin.contextIDs.SlaveObject;
...
void CRecordingSystem::HideItem(IEntity* pEntity, bool hide)
{
	...
	if (pItem)
	{
		...
		i32k numAccessories = accessories.size();
		...
		if (particleCreated->entityId != 0)
		{
			...
			if (itReplayEntity != m_replayEntities.end())
			{
				...
				if (pEntity)
				{
					...
					i32 slot = pEntity->LoadParticleEmitter(particleCreated->entitySlot, pParticleEffect);
					if (slot >= 0)
					{
...
void CRecordingSystem::ApplyParticleDeleted(const SRecording_ParticleDeleted *particleDeleted)
{
	...
	if (itParticle != m_replayParticles.end())
	{
		...
		gEnv->pParticleUpr->DeleteEmitter(pReplayParticleEmitter);
...
void CRecordingSystem::DebugStoreHighlight( const float length, i32k highlightIndex )
{
	const float currTime = gEnv->pTimer->GetFrameStartTime().GetSeconds();
...
void CRecordingSystem::RecordEntitySpawnGeometry(SRecording_EntitySpawn* pEntitySpawn, IEntity* pEntity)
{
	...
	i32k numSlots = pEntity->GetSlotCount();
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CRecordingSystem::RecordEntitySpawn, %s has %d slots but only recording %d of them",
			pEntity->GetClass()->GetName(), numSlots, RECORDING_SYSTEM_MAX_SLOTS);
	}

	i32 maxSlots = min(numSlots, RECORDING_SYSTEM_MAX_SLOTS);
	...
	for (i32 i=0; i<maxSlots; ++i)
	{
		if (pEntity->GetSlotInfo(i, slotInfo))
		{
			...
			{
				DRX_ASSERT_MESSAGE(currentScaling != eS_NotScaled, "A previous slot has no scaling, but this one does!");
				...
				{
					DRX_ASSERT_MESSAGE(fabsf(sqr(fFoundScale) - fScaleXSqr) < 0.001f, "More than one different scale being applied to entity slots");
				}
			}
			else
			{
				DRX_ASSERT_MESSAGE(currentScaling == eS_NotScaled || currentScaling == eS_INVALID, "Non scaled slot on the same render proxy as a scaled slot");
	...
	if(!geometryFound && m_validWithNoGeometry.count(pEntitySpawn->pClass)==0)
	{
		...
		DRX_ASSERT_MESSAGE(false, "Neither a character nor a statobj found in the entity.");
	}
#endif
}
	...
	for (itAnimObject = m_recordingAnimEntities.begin(); itAnimObject != m_recordingAnimEntities.end(); )
	{
		...
		if (pEntity)
		{
			...
			if (pCharacter)
			{
				...
				if (pSkeletonAnim)
				{
					i32 numAnims = pSkeletonAnim->GetNumAnimsInFIFO(0);
					if (numAnims)
					{
...
void CRecordingSystem::RecordParticleInfo()
{
	...
	for (itParticle = m_newParticles.begin(); itParticle != m_newParticles.end(); ++itParticle)
	{
		...
		if (IsPlayingBack())
		{
			...
			REPLAY_EVENT_GUARD
			gEnv->pParticleUpr->DeleteEmitter(pEmitter);
		}
		else
		{
...
{
	DRX_ASSERT_MESSAGE(false, "Shouldn't be getting these events");
	return true;
}
...
{
	DRX_ASSERT_MESSAGE(false, "CRecordingSystem::OnReused needs implementing");
}
...
void CRecordingSystem::OnSetTeam(EntityId entityId, i32 team)
{
	if (m_replayEventGuard == 0 && IsRecording())
	{
...
void CRecordingSystem::OnItemSwitchToHand(CItem* pItem, i32 hand)
{
	if (m_replayEventGuard == 0 && IsRecording())
	{
...
//			static u32k k_recordedSemantics = eSoundSemantic_Physics_Collision | eSoundSemantic_Physics_Footstep | eSoundSemantic_Physics_General |
...
//				{
//					tukk name = pSound->GetName();
//					u32k nameHash = CCrc32::Compute(name);
//					if (!m_excludedSoundEffects.count(nameHash))
//					{
//						RecSysLogDebug(eRSD_Sound, "REC: Start: %d, %s - semantic: %d", pSound->GetId(), name, BitIndex((u32)pSound->GetSemantic()));
...
//							i32 numValidParameters = 0;
//							while(true)
//							{
//								tukk paramName;
//								i32 index = soundParameters.numParameters;
...
//									{
//										float val = 0.f;
//										DRX_ASSERT_MESSAGE(!pSound->GetParam(MAX_SOUND_PARAMETERS, &val), "Sound has too many parameters");
//										break;
//									}
//								}
//								else
...
bool CRecordingSystem::OnEmitTracer(const CTracerUpr::STracerParams &params)
{
	if(IsPlayingBack())
	{
		if (m_replayEventGuard == 0)
...
void CRecordingSystem::OnPlayerPluginEvent(CPlayer* pPlayer, EPlayerPlugInEvent pluginEvent, uk pData)
{
...
void CRecordingSystem::OnMountedGunEnter(CPlayer* pPlayer, i32 upAnimId, i32 downAnimId)
{
...
void CRecordingSystem::OnFireModeChanged(IWeapon *pWeapon, i32 currentFireMode)
{
...
void CRecordingSystem::OnBattleChatter(EBattlechatter chatter, EntityId actorId, i32 variation)
{
	if (m_replayEventGuard == 0)
	{
		...
		battleChatter.chatterVariation = (u8)variation+1;
...
void CRecordingSystem::DebugDrawAnimationData(void)
{
	...
	DRX_ASSERT_MESSAGE(false, "This function has not been implemented yet");
#if 0
			...
			case eTPP_TPAnim:
				{
					...
					if (replayEntity)
					{
						...
						tukk animName = pAnimationSet->GetNameByAnimID( sad->animID );
...
void CRecordingSystem::DebugDrawMemoryUsage()
{
	...
	{
		static i32 memory[eTPP_Last];
		for (i32 i=0; i<eTPP_Last; ++i)
		{
			memory[i] = 0;
		}
		i32 totalMemory = 0;
		...
		for (CRecordingBuffer::iterator itPacket = m_pBuffer->begin(); itPacket != m_pBuffer->end(); ++itPacket)
		{
			...
			{
				DRX_ASSERT_MESSAGE(false, "Unrecognised packet type in recording buffer");
			}
			if (itPacket->type == eRBPT_FrameData)
			{
				float frameTime = ((SRecording_FrameData*)&*itPacket)->frametime;
		...
		char temp[255];
		drx_sprintf(temp, "First person buffer size: %" PRISIZE_T ", Frames: %d, Time: %f", m_pBufferFirstPerson->size(), numFPFrames, lastFPFrameTime - firstFPFrameTime);
		...
		for (i32 i=0; i<eTPP_Last; ++i)
		{
			...
			if (i >= eRBPT_Custom)
			{
				i32k packetIndex = i - eRBPT_Custom;
				assert(packetIndex >= 0 && packetIndex < eTPP_Last - eRBPT_Custom);
				tukk typeName = s_thirdPersonPacketList[packetIndex];
	...
	else if (g_pGameCVars->kc_memStats == 2)
	{
		if (IsRecordingAndNotPlaying())
		{
			...
			DrxWatch("CRecordingSystem main buffer usage at %u, most used is %u", usedMemoryInBuffer, maxUsedMemory);
			...
			DrxWatch("CRecordingSystem first person buffer usage at %u, most used is %u", usedMemoryInBuffer2, maxUsedMemory2);

			DrxWatch("First person data: last seen: %d, max seen: %d", latestFirstPersonDataSize, maxFirstPersonDataSeen);
		}
	}
#endif //RECSYS_DEBUG
}

void CRecordingSystem::SwapFPCameraEndian(u8* pBuffer, size_t size, bool sending)
{
#if !defined(_RELEASE)
	if (eBigEndian)	// Swap endian on PC
	{
		for (u8* pPos = pBuffer; pPos < pBuffer + size; )
		{
			SRecording_Packet* pPacket = (SRecording_Packet*)pPos;
			u16 packetSize = 0;
			if (sending)
			...
			DRX_ASSERT_MESSAGE(packetSize <= 255, "Packet size is larger than expected, has something gone wrong with endian swapping?");
			DRX_ASSERT_MESSAGE(packetSize != 0, "Packet size is zero, the buffer is corrupt?");
			switch (pPacket->type)
			{
			case eFPP_FPChar:
				{
			...
			default:
				DRX_ASSERT_MESSAGE(false, "Unhandled packet type");
			}
			pPos += packetSize;
		}
	}
...
void CRecordingSystem::ApplyRenderNearestChange(const SRecording_RenderNearest *pRenderNearest)
{
	...
	if(pKillerEntity)
	{
		u32 flags = pKillerEntity->GetSlotFlags(0);
...
void CRecordingSystem::ApplyPlaySound(const SRecording_PlaySound* pPlaySound, float time)
{
	...
	//			DRX_ASSERT_MESSAGE(pIEntityAudioProxy.get(), "Failed to create sound proxy");
		...
		if (actorId)
		{
			...
			if(pInfo)
			{
				...
				if (itReplayActor != m_replayActors.end())
				{
					...
					REINST("needs verification!");
					//pBattleChatter->Play(pInfo, (EBattlechatter)pPacket->chatterType, pActor, itReplayActor->second, currentTime, eSoundSemantic_Replay, ((i32)pPacket->chatterVariation)-1, killerTeam);
				}
			}
		}
	}
...
tukk CRecordingSystem::CacheString(tukk name, eStringCache cache)
{
	char *pBuffer = NULL;
	i32 bufferSize = 0;
	if (cache == eSC_Sound)
	...
	if( pBuffer != NULL )
	{
		u16k newLength = static_cast<u16> (strlen(name));
		i32 offset = 0;
		while (true)
		{
			u16 stringLength = *(u16*)(pBuffer + offset);
			if (stringLength > 0)
			{
				offset += 2;
		...
		else
		{
			...
			DRX_ASSERT_MESSAGE(false, "Ran out of memory in the string cache");
			return NULL;
		}
	}
	return NULL;
...
void CRecordingSystem::RecordSoundParameters()
{
	...
	//	if (pSound)
	//	{
	//		for (i32 i=0; i<itSound->numParameters; i++)
	//		{
...
bool CRecordingSystem::ApplyEntityAttached(const SRecording_EntityAttached* pEntityAttached)
{
	...
	if(pEntity && pActor)
	{
		static u32k crcAttachmentName = CCrc32::ComputeLowercase("left_weapon");
...
void CRecordingSystem::ApplyObjectCloakSync( const SRecording_ObjectCloakSync* pObjectCloakSync )
{
	...
	if(pCloakSlave && pCloakMasterActor)
	{
		...
		else
		{
			...
			u8k colorChannel			    = pCloakMasterRP->GetCloakColorChannel();
...
void CRecordingSystem::ApplyPickAndThrowUsed( const SRecording_PickAndThrowUsed* pPickAndThrowUsed )
{
	...
	if (pEntity != NULL && pObject != NULL)
	{
		...
		if (pPickAndThrow)
		{
			...
			{
				SHUDEvent weaponSwitch(eHUDEvent_OnKillCamWeaponSwitch);
				weaponSwitch.AddData((i32)pEntity->GetId());
				weaponSwitch.AddData(false);
				CHUDEventDispatcher::CallEvent(weaponSwitch);
			}
		}
...
u16 CRecordingSystem::EntityIdToNetId(EntityId entityId)
{
...
/*static*/ EntityId CRecordingSystem::NetIdToEntityId(u16 netId)
{
	INetContext* pNetContext = gEnv->pGame->GetIGameFramework()->GetNetContext();
	if (pNetContext)
	{
...
void CRecordingSystem::OnPlayerJoined( EntityId playerId )
{
	if (m_replayEventGuard == 0 && IsRecording())
	{
		...
		CSquadUpr* pSM = g_pGame->GetSquadUpr();
		if (pLobby && pPlayer && pSM)
		{
			...
			DrxFixedStringT<DISPLAY_NAME_LENGTH> name;
			pLobby->GetPlayerDisplayNameFromEntity(playerId, name);

			drx_strcpy(joinedPacket.displayName, name.c_str());
...
void CRecordingSystem::OnMannequinSetSlaveController(EntityId inMasterActorId, EntityId inSlaveActorId, u32 inTargetContext, bool inEnslave, const IAnimationDatabase* piOptionalDatabase)
{
	if ((m_replayEventGuard == 0) && (IsRecording()))
	{
		...
		DRX_ASSERT_MESSAGE(inTargetContext < 256, "targetContext is stored in a u8 in killcam, yet the targetContext being set is out of range!");
		mannSetSlaveControllerPacket.targetContext = static_cast<u8>(inTargetContext);
		mannSetSlaveControllerPacket.enslave = inEnslave;
		if( piOptionalDatabase )
		{
			tukk pDBName = piOptionalDatabase->GetFilename();
...
void CRecordingSystem::OnMannequinSetParam(EntityId inEntityId, tukk inParamName, QuatT &inQuatParam)
{
	if ((m_replayEventGuard == 0) && (IsRecording()))
	{
...
void CRecordingSystem::OnMannequinSetParamFloat(EntityId inEntityId, u32 inParamCRC, float inFloatParam)
{
	if ((m_replayEventGuard == 0) && (IsRecording()))
	{
...
void CRecordingSystem::OnForcedRagdollAndImpulse( EntityId playerId, Vec3 vecImpulse, Vec3 pos, i32 partid, i32 applyTime )
{
	if ((m_replayEventGuard == 0) && (IsRecording()))
	{
...
void CRecordingSystem::RequestTrackerRaycast(const Vec3& position, const Vec3& forwardDirection)
{
	...
	i32k objTypes = ent_all & ~ent_living;
	i32k flags = (geom_colltype_ray<<rwi_colltype_bit)|rwi_colltype_any|rwi_force_pierceable_noncoll|rwi_ignore_solid_back_faces|rwi_pierceability(15);
	
	u32k rayID = g_pGame->GetRayCaster().Queue(
...
void CRecordingSystem::OnInteractiveObjectFinishedUse( EntityId objectId, TagID interactionTypeTag, u8 interactionIndex )
{
	if ((m_replayEventGuard == 0) && (IsRecording()))
	{
...
void CRecordingSystem::GetVictimAimTarget( Vec3 &aimTarget, const Vec3 &localPosition, const Vec3 &remotePosition, float lerpValue )
{
	...
	u8k *startingplace = m_pTPDataBuffer;
	u8k *endingplace = startingplace + m_tpDataBufferSize;
...
void CRecordingSystem::SaveHighlight( const SRecordedKill& details, i32k indexOverride /*= -1*/ )
{
	...
	i32 highlightIndex = indexOverride;
	if( highlightIndex == -1 )
	{
	...
	u8 *startingplace = data.m_data.m_tpdatabuffer;
	u8 *endingplace = startingplace + data.m_data.m_tpdatasize;
...
void CRecordingSystem::PlayHighlight(i32 index)
{
...
void CRecordingSystem::CleanupHighlight( i32 index )
{
	RecSysLogDebug(eRSD_General, "Cleanup Highlight: %d", index);
	if (m_highlightData[index].m_bUsed)
	{
		// Sort out reference counting
		u8 *startingplace = m_highlightData[index].m_data.m_tpdatabuffer;
		u8 *endingplace = startingplace + m_highlightData[index].m_data.m_tpdatasize;
...
IEntity * CRecordingSystem::SpawnCorpse( EntityId originalId, const Vec3 &position, const Quat &rotation, const Vec3 &scale )
{
	...
	char sName[64];
	drx_sprintf(sName, "ReplayCorpse%d", m_nReplayEntityNumber ++);
	params.sName = sName;
...
IEntityClass* CRecordingSystem::GetEntityClass_NetSafe( u16k classId )
{
	char className[128];
...
#ifdef RECSYS_DEBUG
void CRecordingSystem::PrintCurrentRecordingBuffers( tukk const msg /*= NULL*/ )
{
...
void SHighlightRules::Init( XmlNodeRef& node )
{
	...
	if(node)
	{
		...
		i32k numGroups = node->getChildCount();
		for(i32 g=0; g<numGroups; ++g)
		{
			XmlNodeRef group = node->getChild(g);
			//////////////////////////////////////////////////////////////////////////
			if(group->isTag("HitTypes"))
			{
				for(i32 c=0, num=group->getChildCount(); c<num; ++c)
				{
					...
					else if(tukk type = child->getAttr("type"))
					{
						if(i32k typeId = pGameRules->GetHitTypeId(type))
			...
			else if(group->isTag("Weapons"))
			{
				for(i32 c=0, num=group->getChildCount(); c<num; ++c)
				{
					...
					else if(tukk name = child->getAttr("name"))
					{
	...
	if(bPrecacheWeapons)
	{
		if(ICharacterUpr* pCharUpr = gEnv->pCharacterUpr)
		{
			...
			if(!streamingWeaponFPModel.empty())
			{
				...
				pCharUpr->StreamKeepCharacterResourcesResident(streamingWeaponFPModel.c_str(), eFPWeaponLOD, true);
			...
			if(!streamingWeaponTPModel.empty())
			{
				...
				pCharUpr->StreamKeepCharacterResourcesResident(streamingWeaponTPModel.c_str(), 0, true);
...
void SPlaybackInfo::View::ClearPrecachedWeapons()
{
	...
	if((precacheFlags&ePF_FPWeaponCached)!=0 && pCharUpr)
	{
		pCharUpr->StreamKeepCharacterResourcesResident(streamingWeaponFPModel.c_str(), eFPWeaponLOD, false);
	}
	precacheFlags &= ~ePF_FPWeaponCached;
	streamingWeaponFPModel.clear();

	// Clear TP Weapon Model.
	if((precacheFlags&ePF_TPWeaponCached)!=0 && pCharUpr)
	{
		pCharUpr->StreamKeepCharacterResourcesResident(streamingWeaponTPModel.c_str(), 0, false);
	}
	precacheFlags &= ~ePF_TPWeaponCached;
	streamingWeaponTPModel.clear();
}
