...
$Id$
$DateTime$
Описание: Implements a standard helicopter movements
...
#include <drx3D/Game/Vehicle/VehicleUtils.h>
...
void CVehicleMovementHelicopterArcade::PostInit()
{
	...
	for (i32 i = 0; i < m_pVehicle->GetActionCount(); i++)
	{
...
void CVehicleMovementHelicopterArcade::OnEvent(EVehicleMovementEvent event, const SVehicleMovementEventParams& params)
{
	...
	{
		float newSteeringDamage = (((float(drx_rand()) / float(DRX_RAND_MAX)) * 2.5f ) + 0.5f);
...
//------------------------------------------------------------------------
void CVehicleMovementHelicopterArcade::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
...
void CVehicleMovementHelicopterArcade::Update(const float deltaTime)
{
	...
	i32 profile = g_pGameCVars->v_profileMovement;
...
//------------------------------------------------------------------------
void CVehicleMovementHelicopterArcade::GetMemoryUsage( IDrxSizer * pSizer ) const
{
...
//------------------------------------------------------------------------
void CVehicleMovementHelicopterArcade::GetMemoryUsageInternal( IDrxSizer * pSizer ) const
{
