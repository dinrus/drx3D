...
{
#include <drx3D/Game/GameActions.actions"
}
#undef DECL_ACTION
...
CGameActions::~CGameActions()
{
	...
	if (pActionMapUpr)
	{
		pActionMapUpr->UnregisterActionMapEventListener(this);
	}
}

void CGameActions::OnActionMapEvent(const SActionMapEvent& event)
{
	if (event.event == SActionMapEvent::eActionMapUprEvent_ActionMapsInitialized)
	{
		InitActions();
	}
}
