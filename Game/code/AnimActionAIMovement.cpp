...
void CAnimActionAIMovement::OnInitialise()
{
	...
	DRX_ASSERT(m_moveStateInfo[eMS_Idle].m_fragmentID != FRAGMENT_ID_INVALID);
	...
	DRX_ASSERT(m_moveStateInfo[eMS_Move].m_fragmentID != FRAGMENT_ID_INVALID);
...
void CAnimActionAIMovement::OnSequenceFinished(i32 layer, u32 scopeID)
{
...
void CAnimActionAIMovement::RequestCoverAction(CPlayer& player, tukk actionName)
{
...
CAnimActionAIMovement::EMoveState CAnimActionAIMovement::CalculateState()
{
	...
	if (player.IsOnGround() || player.IsSwimming())
	{
		...
		else
		{
			...
			DRX_ASSERT(MIN_BIG_TURN_ANGLE >= MIN_TURN_ANGLE);
