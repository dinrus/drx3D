...
const static u32 s_awardFlags[EAMA_Max] = 
{
...
#define DbgLog(...) DRX_DEBUG_LOG(AFTER_MATCH_AWARDS, __VA_ARGS__)
#define DbgLogAlways(...) DRX_DEBUG_LOG_ALWAYS(AFTER_MATCH_AWARDS, __VA_ARGS__)
	...
	if (gEnv->bServer)
	{
		switch(m_state)
		{
			case k_state_server_game_ended_waiting_for_all_client_results:
			{
				...
				DRX_ASSERT_MESSAGE(m_timeOutLeftWaitingForClients > 0.f, string().Format("AfterMatchAwards Update() - timeOut has occurred waiting for awards from clients. haveReceived=%d; waitingForNum=%d. Continuing anyways with incomplete results", numReceived, numPresent));
...
//		return NULL;
	...
	DRX_ASSERT_MESSAGE(actorId, "GetAwardsForActor() has been passed an invalid NULL entityID.");
	if (actorId == 0)
	{
...
void CAfterMatchAwards::CalculateAwardForActor(EAfterMatchAwards inAward, EntityId inActorId)
{
	...
	DRX_ASSERT_MESSAGE(awards, "CalculateAwardForActor() failed to get award for actor!!");
	if (awards)
	{
		...
		switch(inAward)
		{
			...
			case EAMA_MostMotivated:
			{
				...
				DRX_ASSERT_MESSAGE(playerStats, "CalculateAwardForActor() MostMotivated award requires the playerStats");
				if (playerStats)
				{
					const SGameRulesPlayerStat* statsForPlayer=playerStats->GetPlayerStats(inActorId);
					DRX_ASSERT_MESSAGE(statsForPlayer, string().Format("CalculateAwardForActor() MostMotivated award, failed to find stats for actor=%s", GetEntityName(inActorId)));
					if (statsForPlayer)
					{
			...
			case EAMA_Untouchable:
				// TODO - this really should be stored in an i32!!! float for now for network send
      ...
      case EAMA_MoneyShot:
        ...
        break;
			...
			case EAMA_BangOn:
			{
				i32 numShotsFired=m_persistantStats->GetDerivedStatForActorThisSession(EDIPS_Shots, inActorId);
			...
			case EAMA_Relentless:
				{
					i32 highestKillsOfSinglePlayer = 0;
					std::vector<i32> stats;
			...
			case EAMA_Genocide:
				{
					...
					i32 enemyTeamCount = 0;
					CGameRules *pGameRules = g_pGame->GetGameRules();
					if ( pGameRules )
					{
						i32 clientTeam = pGameRules->GetTeam(inActorId);
			...
			case EAMA_BigBanger:
				{
					i32 highestKillsOfSingleExplosion = 0;
					std::vector<i32> stats;
...
EntityId CAfterMatchAwards::GetMaximumFloatFromWorking(EAfterMatchAwards inAward, float *outMaxFloat)
{
	...
	for ( ; it!=end; ++it)
	{
		DRX_ASSERT_MESSAGE(it->second.m_working[inAward].m_calculated, string().Format("GetMaximumFloatFromWorking() has encountered an working value for player=%s award=%s that hasn't been calculated. This shouldn't happen!", GetEntityName(it->first), GetNameForAward(inAward)));
...
EntityId CAfterMatchAwards::GetMinimumFloatFromWorking(EAfterMatchAwards inAward)
{
	...
	for ( ; it!=end; ++it)
	{
		DRX_ASSERT_MESSAGE(it->second.m_working[inAward].m_calculated, string().Format("GetMinimumFloatFromWorking() has encountered an working value for player=%s award=%s that hasn't been calculated. This shouldn't happen!", GetEntityName(it->first), GetNameForAward(inAward)));
		if (!it->second.m_working[inAward].m_calculated)
		{
...
void CAfterMatchAwards::GetAllFromWorkingWithFloat(EAfterMatchAwards inAward, float inVal, ActorsVector &results)
{
	...
	for ( ; it!=end; ++it)
	{
		DRX_ASSERT_MESSAGE(it->second.m_working[inAward].m_calculated, string().Format("GetAllFromWorkingWithFloat() has encountered an working value for player=%s award=%s that hasn't been calculated. This shouldn't happen!", GetEntityName(it->first), GetNameForAward(inAward)));
		if (!it->second.m_working[inAward].m_calculated)
		{
...
void CAfterMatchAwards::GetAllFromWorkingGreaterThanFloat(EAfterMatchAwards inAward, float inVal, ActorsVector &results)
{
	...
	for ( ; it!=end; ++it)
	{
		DRX_ASSERT_MESSAGE(it->second.m_working[inAward].m_calculated, string().Format("GetAllFromWorkingGreaterThanFloat() has encountered an working value for player=%s award=%s that hasn't been calculated. This shouldn't happen!", GetEntityName(it->first), GetNameForAward(inAward)));
		if (!it->second.m_working[inAward].m_calculated)
		{
...
void CAfterMatchAwards::ClCalculateAward(EAfterMatchAwards inAward)
{
	...
	DRX_ASSERT_MESSAGE(gEnv->IsClient(), "have to have a local client to try and calculate awards on clients");
	DRX_ASSERT_MESSAGE(GetFlagsForAward(inAward) & eAF_LocalClients, "award has to be an award calculated by local clients");
...
i32 CAfterMatchAwards::SvCalculateAward(EAfterMatchAwards inAward)
{
	...
	for (i32 i=0; i<numResults; i++)
	{
		...
		if (awards)
		{
			...
			i32 awardsLen=awards->m_awards.size();
			for (i32 j=0; j<awardsLen; j++)
			{
				DRX_ASSERT_MESSAGE(awards->m_awards[j] != inAward, string().Format("CAfterMatchAwards::SvCalculateAward() found we are pushing back a duplicate award=%d (%s) this shouldn't happen", inAward, GetNameForAward(inAward)));
...
i32 CAfterMatchAwards::CalculateAward(EAfterMatchAwards inAward)
{
	...
	else
	{
		...
		DRX_ASSERT_MESSAGE(0, "CalculateAward() unhandled state.. we're not a server or a client!!!");
	}
...
void CAfterMatchAwards::ClSendAwardsToServer()
{
	...
	DRX_ASSERT_MESSAGE(awards, "ClSendAwardsToServer() has failed to find the awards for local client");
	if (awards)
	{
...
void CAfterMatchAwards::SvSendAwardsToClients()
{
	...
	for ( ; it!=end; ++it)
	{
		...
		i32 len=it->second.m_awards.size();
		...
		{
			DRX_ASSERT_MESSAGE(0, "SvSendAwardsToClients() has failed to resolve an actor, assuming client has disconnected. Can't send awards when we don't know their channelId");
...
i32 CAfterMatchAwards::GetNumClientAwards()
{
	i32 numAwards = 0;
	if (SAwardsForPlayer *pAwards = GetAwardsForActor(m_localClientEntityIdWas))
	{
		numAwards = (i32)pAwards->m_awards.size();
	}

	return numAwards;
}

EAfterMatchAwards CAfterMatchAwards::GetNthClientAward(i32k Nth)
{
	if (SAwardsForPlayer *pAwards = GetAwardsForActor(m_localClientEntityIdWas))
	{
		if (Nth >= 0 && Nth < (i32)pAwards->m_awards.size())
		{
...
void CAfterMatchAwards::ClientReceivedAwards(i32 numAwards, u8k awardsArray[])
{
	SAwardsForPlayer *awards = GetAwardsForActor(m_localClientEntityIdWas);

#if ! defined _RELEASE
	DrxLog("CAfterMatchAwards::ClientReceivedAwards() numAwards=%d; localactor=%s; awards=%p", numAwards, GetEntityName(m_localClientEntityIdWas), awards);
#endif

	DRX_ASSERT_MESSAGE(awards, "ClientReceivedAwards() requires us to find our awards for what was our local client!!");
	if (awards)
	{
		for (i32 i=0; i<numAwards; i++)
		{
			if (awardsArray[i] < EAMA_Max)
			{
				EAfterMatchAwards award = (EAfterMatchAwards)awardsArray[i];
#if ! defined _RELEASE
				DrxLog("Award - %d %s", awardsArray[i], GetNameForAward(award));
#endif
			...
			else
			{
				...
				DRX_ASSERT_MESSAGE(0, string().Format("CAfterMatchAwards::ClientReceivedAwards() has received a bad out of range award=%d", awardsArray[i]));
			}
		}
	}
...
void CAfterMatchAwards::ServerReceivedAwardsWorkingFromPlayer(EntityId playerEntityId, i32 numWorkingValues, const CGameRules::SAfterMatchAwardWorkingsParams::SWorkingValue workingValues[])
{
	...
	DRX_ASSERT_MESSAGE(awards, string().Format("ServerReceivedAwardsWorkingFromPlayer() failed to find awards for player=%d (%s)", playerEntityId, GetEntityName(playerEntityId)));

	if (awards)
	{
		DbgLog("CAfterMatchAwards::ServerReceivedAwardsWorkingFromPlayer() found awards for player=%s", GetEntityName(playerEntityId));

		for (i32 i=0; i<numWorkingValues; i++)
		{
			...
			{
				DRX_ASSERT_MESSAGE(0, string().Format("ServerRecevedAwardsWorkingFromPlayer() player=%s; recieved bad award out of range=%d", GetEntityName(playerEntityId), workingValues[i].m_award));
...
void CAfterMatchAwards::FilterWinningAwards()
{
	...
	if (awards)
	{
		...
		i32 originalNumAwards=awards->m_awards.size();

		// filter out all awards already earned
		DrxFixedArray<EAfterMatchAwards, CAfterMatchAwards::kMaxLocalAwardsGiven> newlyWonAwards;
		...
		if (newlyWonAwards.size() >= kMaxLocalAwardsActuallyWon)
		{
			...
			for (i32 i=0; i<kMaxLocalAwardsActuallyWon; i++)
			{
				u32 randomIndex=drx_random(0U, newlyWonAwards.size() - 1);
		...
		else
		{
			...
			DrxFixedArray<EAfterMatchAwards, CAfterMatchAwards::kMaxLocalAwardsActuallyWon> finalAwards;
			...
			{
				u32 randomIndex=drx_random(0U, awards->m_awards.size() - 1);
...
void CAfterMatchAwards::HaveGotAwards()
{
	...
	DRX_ASSERT_MESSAGE(m_localClientEntityIdWas, "HaveGotAwards() failed, no local client entity set");

	SAwardsForPlayer *awards = GetAwardsForActor(m_localClientEntityIdWas);
	DRX_ASSERT_MESSAGE(awards, "HaveGotAwards() failed to find awards for localClient");
	if (awards)
	{
		FilterWinningAwards();
	}
...
void CAfterMatchAwards::DebugMe(bool useDrxLog/*=false*/)
{
	if (useDrxLog)
	{
		DrxLog("CAfterMatchAwards::DebugMe()");
	}
	else
	{
	...
	for ( ; it!=end; ++it)
	{
		i32k len = it->second.m_awards.size();

		for (i32 i=0; i<len; i++)
		{
			if (useDrxLog)
			{
				DrxLog("Actor=%d; (%s) - Award=%s", it->first, GetEntityName(it->first), GetNameForAward(it->second.m_awards[i]));
			}
			else
			{
...
tukk CAfterMatchAwards::GetNameForAward(EAfterMatchAwards inAward)
{
	return s_afterMatchAwardsNames[inAward];
}

i32 CAfterMatchAwards::GetFlagsForAward(EAfterMatchAwards inAward)
{
...
tukk CAfterMatchAwards::GetEntityName(EntityId inEntityId) const
{
