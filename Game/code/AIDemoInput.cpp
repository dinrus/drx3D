...
static tukk  s_visionModes[] = {"nanosuit_thermalvision",	"nanosuit_cryvision",	"nanosuit_nightvision"};

CDedicatedInput::CDedicatedInput(CPlayer* pPlayer) :
		m_pPlayer(pPlayer),
		m_rand( gEnv->bNoRandomSeed?0:(u32)gEnv->pTimer->GetAsyncCurTime() ),
		...
		m_move(eDB_Default)
{
	...
	m_timeUntilDie = drx_random(30.0f, 90.0f);
	m_timeUntilRequestRespawn = drx_random(1.0f, 7.0f);
	m_timeUntilChangeWeapon = drx_random(10.0f, 15.0f);
...
void CDedicatedInput::PreUpdate()	
{
	...
	{
		m_timeBeforeChangeMode = drx_random(0.5f, 7.5f);

		m_mode = drx_random(0, NUM_MODES-(shouldFireNoExplosives?2:1));
		...
		if (shouldMove == true)
		{
			if (m_pPlayer->IsClient() && (drx_random(0, 3) != 0))
			{
				...
				if (binWeapon)
				{
					i32 whichMode = drx_random(0, (i32)(DRX_ARRAY_COUNT(s_visionModes)) - 1);
	...
	if (m_interpolateToNewMoveLookInputs <= 0.f)
	{
		...
		m_interpolationSpeed = drx_random(0.2f, 1.2f);
		m_targetDeltaMovement.Set(drx_random(-5.0f, 5.0f), drx_random(-2.0f, 8.0f), drx_random(-5.0f, 5.0f));
		m_targetDeltaRotation.Set(THETA * drx_random(-0.5f, 0.5f), 0.f, THETA * drx_random(-2.5f, 2.5f));

		m_targetDeltaRotation *= drx_random(0.0f, 1.0f);
		m_targetDeltaMovement *= drx_random(0.0f, 1.0f);
		...
		m_jump = (m_stance == STANCE_STAND) ? (drx_random(0.0f, 1.0f) < g_pGameCVars->g_dummyPlayersJump) : 0;
	}
	else
	{
		float fractionDoneThisFrame = changeAmount / (m_interpolateToNewMoveLookInputs + changeAmount);
		DRX_ASSERT_TRACE (fractionDoneThisFrame >= 0.f && fractionDoneThisFrame <= 1.f, ("fractionDoneThisFrame = %f (changeAmount=%f, interpolate=%f)", fractionDoneThisFrame, changeAmount, m_interpolateToNewMoveLookInputs));
...
void CDedicatedInput::GiveItems()
{
	...
	i32k numberItems = (DRX_ARRAY_COUNT(itemNames));
	...
	{
		nameIdx = drx_random(0, numberItems - 1);
	}
	...
	tukk itemName = (tukk)(pItemSystem->Query(eISQ_Find_Item_By_Name, itemNames[nameIdx]));
	DRX_ASSERT_MESSAGE(itemName, string().Format("DummyPlayer : Trying to give DummyPlayer unknown/removed item '%s'", itemNames[nameIdx]).c_str() );
	if(itemName)
	{
		pItemSystem->GiveItem(m_pPlayer, itemName, true, true, true);
		DrxLog( "Giving new weapon item %s", itemName);
	}

	tukk explosiveItemNames[] =
	{
		"C4",
		"JAW",
		"FlashBangGrenades",
	...
	i32k numberExplosiveItems = (DRX_ARRAY_COUNT(explosiveItemNames));
	...
	{
		nameIdx = drx_random(0, numberExplosiveItems - 1);
	}

	itemName = (tukk)(pItemSystem->Query(eISQ_Find_Item_By_Name, explosiveItemNames[nameIdx]));
	DRX_ASSERT_MESSAGE(itemName, string().Format("DummyPlayer : Trying to give DummyPlayer unknown/removed explosive item '%s'", explosiveItemNames[nameIdx]).c_str() );
	if(itemName)
	{
		pItemSystem->GiveItem(m_pPlayer, itemName, true, false, true);
		DrxLog( "Giving new explosive item %s", itemName);
	}
}
	...
	if (pItem)
	{
		...
		if (pWeapon)
		{
			...
			if ( cfm )
			{
				...
				{
					i32 ac = max(pInventory->GetAmmoCount(cfm->GetAmmoType()), cfm->GetAmmoCount());
					if (0 == ac)
					{
			...
			if (shouldFire)
			{
				...
				if (pCurrentItem)
				{
					...
					{
						if (tukk const currentWeaponCat = iis->GetItemCategory(pCurrentItem->GetEntity()->GetClass()->GetName()))
						{
				...
				case E_ZOOM_IN_OUT:
					{
						//					DrxLog ("[DemoInput] Waiting to zoom in/out the %s belonging to %s '%s' - isZoomed=%d busy=%d zooming=%d selected=%d canZoom=%d", pItem->GetEntity()->GetName(), m_pPlayer->GetEntity()->GetClass()->GetName(), m_pPlayer->GetEntity()->GetName(), pWeapon->IsZoomed(), pItem->IsBusy(), pWeapon->IsZoomingInOrOut(), pItem->IsSelected(), pWeapon->CanZoom());
						if (! pWeapon->CanZoom() || !m_pPlayer->IsClient())
						{
							//						DrxLog ("[DemoInput] %s '%s' can't zoom with current weapon!", m_pPlayer->GetEntity()->GetClass()->GetName(), m_pPlayer->GetEntity()->GetName());
						...
						else if (! pItem->IsBusy() && !pWeapon->IsZoomingInOrOut() && pItem->IsSelected())
						{
							if (pWeapon->IsZoomed())
							{
								//							DrxLog ("[DemoInput] %s '%s' zooming out!", m_pPlayer->GetEntity()->GetClass()->GetName(), m_pPlayer->GetEntity()->GetName());
								pWeapon->StopZoom(m_pPlayer->GetEntityId());
							}
							else
							{
								//							DrxLog ("[DemoInput] %s '%s' zooming in!", m_pPlayer->GetEntity()->GetClass()->GetName(), m_pPlayer->GetEntity()->GetName());
...
void CDedicatedInput::SelectEquipmentPackage()
{
	...
	if (pEquipmentLoadout && pGameRules)
	{
		...
		{
			i32k packIndex = 0;	// TODO: Could get them to select random loadouts
...
void CDedicatedInput::HandleDeathAndSuicide(CGameRules* const pGameRules)
{
	if (m_pPlayer->IsDead())
	{
		...
		{
			DrxWatch ("$4%s '%s' is dead!", m_pPlayer->GetEntity()->GetClass()->GetName(), m_pPlayer->GetEntity()->GetName());
		}
#endif
	...
	else if (g_pGameCVars->g_dummyPlayersCommitSuicide)
	{
		...
		if (g_pGameCVars->g_dummyPlayersShowDebugText)
		{
			...
			DrxWatch ("$3%s '%s' [%s] will commit suicide in %.1f seconds", m_pPlayer->GetEntity()->GetClass()->GetName(), m_pPlayer->GetEntity()->GetName(), item ? item->GetEntity()->GetClass()->GetName() : "", m_timeUntilDie);
		}
#endif
...
void CDedicatedInput::OnAction( const ActionId& action, i32 activationMode, float value )
{
}
...
void CDedicatedInput::GetMemoryUsage(IDrxSizer * pSizer) const
{
	pSizer->Add(*this);
}
