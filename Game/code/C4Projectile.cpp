...
void CC4Projectile::Launch(const Vec3 &pos, const Vec3 &dir, const Vec3 &velocity, float speedScale)
{
	...
	{
		GetEntity()->SetTimer(ePTIMER_ACTIVATION, (i32)(m_pAmmoParams->armTime*1000.f));
		Arm(false);
	}
	else
	{
	...
	{
		if(i32 count = GetEntity()->GetSlotCount())
		{
			for(i32 i = 0; i < count; i++)
			{
				...
				if(info.pStatObj)
				{
					...
					if(pMaterial)
					{
						...
						if( m_pArmedMaterial = pMatUpr->LoadMaterial(pExplosiveParams->armedMaterial, false) )
						{
							m_pArmedMaterial->AddRef();
						}
						if( m_pDisarmedMaterial = pMatUpr->LoadMaterial(pExplosiveParams->disarmedMaterial, false) )
						{
	...
	if(CGameRules* pGameRules = g_pGame->GetGameRules())
	{
		if(projectileDesc.ownerId) //Can be 0 for late joiners
		{
			...
			if(!gEnv->IsDedicated())
			{
				...
				i32 localPlayerTeam = pGameRules->GetTeam( clientId );
...
//------------------------------------------------------------------------
bool CC4Projectile::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 pflags)
{
...
void CC4Projectile::Update(SEntityUpdateContext &ctx, i32 updateSlot)
{
	if(gEnv->bMultiplayer)
	{
		...
		if(!pActor || pActor->IsDead())
		{
			...
			if(m_isShowingUIIcon)
			{
				...
				hudEvent.AddData((i32)GetEntityId());
	...
	if(gEnv->bMultiplayer)
	{
		if(hitInfo.targetId == GetEntityId() && m_hitPoints > 0 && !CheckAnyProjectileFlags(ePFlag_destroying))
		{
			if(CGameRules* pGameRules = g_pGame->GetGameRules())
			{
				...
				{
					m_hitPoints -= (i32)hitInfo.damage;
					if(m_hitPoints <= 0)
					{
...
void CC4Projectile::OnChangedTeam( EntityId entityId, i32 oldTeamId, i32 newTeamId )
{
...
void CC4Projectile::SerializeSpawnInfo( TSerialize ser )
{
	...
	i32 prevTeamId = m_teamId;
	ser.Value("teamId", m_teamId, 'team');
	if(ser.IsReading() && prevTeamId != m_teamId)
	{
		const EntityId clientId = g_pGame->GetIGameFramework()->GetClientActorId();
		i32 localPlayerTeam = g_pGame->GetGameRules()->GetTeam( clientId );
