...
void CCinematicInput::OnBeginCutScene( i32 cutSceneFlags )
{
...
void CCinematicInput::OnEndCutScene( i32 cutSceneFlags )
{
...
void CCinematicInput::TogglePlayerThirdPerson(bool bEnable)
{
	...
	if (pClientActor)
	{
		DRX_ASSERT(pClientActor->GetActorClass() == CPlayer::GetActorClassType());
...
void CCinematicInput::OnAction( const EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
	...
	{
		i32k playingSequences = gEnv->pMovieSystem->GetNumPlayingSequences();
		for (i32 i = 0; i < playingSequences; ++i)
		{
...
void CCinematicInput::ReEnablePlayerAfterCutscenes()
{
	...
	if (pClientActor)
	{
		...
		{
			u32 playerFlags = pClientPlayer->GetEntity()->GetFlagsExtended();
	...
	g_pGame->GetIGameFramework()->GetIActionMapUpr()->EnableActionMap( "player_cine" , false );
}
...
void CCinematicInput::DisablePlayerForCutscenes()
{
	...
	g_pGame->GetIGameFramework()->GetIActionMapUpr()->EnableActionMap( "player_cine" , true );
}
