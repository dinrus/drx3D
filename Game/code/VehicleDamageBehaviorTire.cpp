...
void CVehicleDamageBehaviorBlowTire::Activate(bool activate)
{
  ...
  if (activate)
  {
    ...
    { 
      ...
      if (status.speed > 0.1f)
      {
        ...
        float amount = m_pVehicle->GetMass() * status.speed * drx_random(0.25f, 0.45f) * -sgn(wheelTM.GetTranslation().x);
    ...
    if (!gEnv->pSystem->IsSerializingFile())
    {
      ...
      float amount = m_pVehicle->GetMass() * drx_random(0.1f, 0.15f);
  ...
  { 
    if (pWheel)
    {
      ...
      for (i32 i=0; i<m_pVehicle->GetWheelCount(); ++i)
      { 
...
void CVehicleDamageBehaviorBlowTire::GetMemoryUsage(IDrxSizer * s) const
{
	s->Add(*this);
	s->Add(m_component);
}
