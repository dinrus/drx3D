...
namespace
{
	...
	struct FAllowedParts
	{
		...
		i32 operator()( tukk pName ) const
		{
			i32 iPartId = m_pCharInstance->GetIDefaultSkeleton().GetJointIDByName(pName);
			...
			else
			{
				...
				IAttachmentUpr* pAttachmentUpr = m_pCharInstance->GetIAttachmentUpr();
				i32 iAttachmentIdx = pAttachmentUpr->GetIndexByName(pName);
				if (iAttachmentIdx != -1)
				{
	...
	struct FAllowedHitTypes
	{
		...
		i32 operator()( tukk pName ) const 
		{ 
			i32k hitTypeID = m_pGameRules->GetHitTypeId(pName);
			if( hitTypeID > 0 )
			{
				return hitTypeID;
			}
	...
	TagID GenerateMannequinTargetTag( const FragmentID fragID, const IActionController* piActionController, tukk pTargetTag )
	{
		if( piActionController )
		{
			u32 crc = CCrc32::ComputeLowercase( pTargetTag );
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactionsSystem::SReactionsProfile::GetMemoryUsage(IDrxSizer * s) const
{
...
{
	SPredGetMemoryUsage(IDrxSizer* s) : m_pDrxSizer(s) {}
	void operator() (const ProfilesContainer::value_type& item)
	{
		item.second.GetMemoryUsage(m_pDrxSizer);
	}

	IDrxSizer* m_pDrxSizer;
};
...
{
	typedef std::set<u32> AnimCRCsContainer;

	SPredGetAnims(AnimCRCsContainer& totalAnims, AnimCRCsContainer& usedAnims, uint& redundantAnimations, EntityId entityId, u32& animationsSizeInMemory) : 
	...
	void operator() (const ReactionsContainer::value_type& item)
	{
		...
		if (!reactionAnim.animCRCs.empty())
		{
			...
			i32k iNextIdx = reactionAnim.GetNextReactionAnimIndex();
			if (iNextIdx != -1)
			{
				u32k animCRC = reactionAnim.animCRCs[iNextIdx];
				m_usedAnimIDs.insert(animCRC);
				
				if (m_pAnimationSet)
				{
					i32 animID = m_pAnimationSet->GetAnimIDByCRC(animCRC);
					if (animID != -1)
...
//////////////////////////////////////////////////////////////////////////
class CHitDeathReactionsSystem::CHitDeathReactionsDebugWidget : public IDrxPerfHUDWidget
{
public:
	virtual void Reset() {}
	...
	//////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////
	CHitDeathReactionsDebugWidget(minigui::IMiniCtrl* pParentMenu, IDrxPerfHUD* pPerfHud, CHitDeathReactionsSystem& hitDeathReactionsSystem) :
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
inline void AnimIDError(tukk animName)
{
#ifndef _RELEASE
	...
	DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "[CHitDeathReactions] Missing anim: %s", animName);
#endif
}

//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactionsSystem::Warning(tukk szFormat, ...)
{
#ifndef _RELEASE
	...
	if (profileId != INVALID_PROFILE_ID)
	{
		...
		// Instantiate new params
		{
			...
			if (pMainChar)
			{
				...
				{
					tukk szFilePath = pMainChar->GetIDefaultSkeleton().GetModelFilePath();
					DrxLogAlways("[HitDeathReactionsSystem] Instancing %s (%s)", szReactionsDataFilePath, szFilePath);
				}
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactionsSystem::RequestReactionAnimsForActor(const CActor& actor, u32 requestFlags)
{
						...
						if (profile.iRefCount == 0)
						{
							...
							else
							{
								profile.timerId = gEnv->pGame->GetIGameFramework()->AddTimer(CTimeValue(HYSTERESIS_REQUEST_TIMER_SECONDS), 
									false, functor(*this, &CHitDeathReactionsSystem::OnRequestAnimsTimer), reinterpret_cast<uk>(static_cast<uintptr_t>(profileId)));
							}
						}
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactionsSystem::ReleaseReactionAnimsForActor(const CActor& actor, u32 requestFlags)
{
	if (GetStreamingPolicy() == SCVars::eHDRSP_ActorsAliveAndNotInPool)
	{
		ProfileId profileId = GetActorProfileId(actor);
			...
			if (itFind != m_reactionProfiles.end())
			{
				...
				if (profile.IsValid())
				{
					...
					if (itEnt != profile.entitiesUsingProfile.end())
					{
						...
						if (bReactionAnimsWereLocked && !FlagsValidateLocking(itEnt->second))
						{
							DRX_ASSERT(profile.iRefCount > 0);
							...
							if (profile.iRefCount == 1)
							{
								...
								else
								{
									profile.timerId = gEnv->pGame->GetIGameFramework()->AddTimer(CTimeValue(HYSTERESIS_RELEASE_TIMER_SECONDS), 
										false, functor(*this, &CHitDeathReactionsSystem::OnReleaseAnimsTimer), reinterpret_cast<uk>(static_cast<uintptr_t>(profileId)));
								}
							}
...
void CHitDeathReactionsSystem::PreloadData()
{
	...
	tukk preloadList = gEnv->bMultiplayer ? REACTIONS_PRELOAD_LIST_FILE_MP : REACTIONS_PRELOAD_LIST_FILE;
	...
	if (xmlNode)
	{
		i32k iEntries = xmlNode->getChildCount();
		for (i32 i = 0; i < iEntries; ++i)
		{
			const XmlNodeRef pairElement = xmlNode->getChild(i); 
			if (pairElement->haveAttr(PRELOAD_REACTIONS_FILE))
			{
				tukk pReactionsFile = pairElement->getAttr(PRELOAD_REACTIONS_FILE);
...
void CHitDeathReactionsSystem::PreloadActorData(SmartScriptTable pActorPropertiesTable)
{
	...
	if(pReactionsConfigTable->GetValue(MANQ_SLAVE_ADB, szAnimDatabase) && szAnimDatabase)
	{
		...
		mannequinSys.GetAnimationDatabaseUpr().Load(szAnimDatabase);
	}
}

//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactionsSystem::GetMemoryUsage(IDrxSizer * s) const
{
...
void CHitDeathReactionsSystem::DumpHitDeathReactionsAssetUsage() const
{
	...
	DrxLogAlways("[HitDeathReactionsSystem] Logging stats for reaction anims. Streaming: %s", bStreaming ? "enabled" : "disabled");
	...
	for (ProfilesContainer::const_iterator it = m_reactionProfiles.begin(); it != itEnd; ++it)
	{
		...
		if (profile.IsValid())
		{
			...
			u32 reactionAnimsSizeInMemory = 0;
			...
			i32k iUsedAnims = profileUsedAnims.size();
			i32k iTotalAnims = profileTotalAnims.size();
			std::map<ProfileId, string>::const_iterator itFind = m_profileIdToReactionFileMap.find(it->first);
			DRX_ASSERT(itFind != m_profileIdToReactionFileMap.end());
			const string& sReactionsProfile = (itFind != m_profileIdToReactionFileMap.end()) ? itFind->second : string(DrxStackStringT<char, 9>().Format("%X", it->first));
			if (bStreaming)
			{
				DrxLogAlways("[HitDeathReactionsSystem] %s has %i/%i reaction animations loaded. Redundant anims: %u. Size in memory: %u KiB", sReactionsProfile.c_str(), iUsedAnims, iTotalAnims, redundantAnims, reactionAnimsSizeInMemory / 1024U);
			}
			else
			{
				DrxLogAlways("[HitDeathReactionsSystem] %s has %i reaction animations loaded. Redundant anims: %u. Size in memory: %u KiB", sReactionsProfile.c_str(), iTotalAnims, redundantAnims, reactionAnimsSizeInMemory / 1024U);
			}
	...
	if (bStreaming)
	{
		DrxLogAlways("[HitDeathReactionsSystem] Total usage stats: %i/%i assets (Saving memory for %i variations)", usedAnimsCount, totalAnimsCount, totalAnimsCount - usedAnimsCount);
	}
	else
	{
		DrxLogAlways("[HitDeathReactionsSystem] Total usage stats: %i assets", totalAnimsCount);
	}
...
ProfileId CHitDeathReactionsSystem::GetActorProfileId(const CActor& actor) const
{
	...
	DRX_ASSERT( actor.GetActorClass() == CPlayer::GetActorClassType() );
...
#endif
	{
			...
			if (pAnimChar)
			{
				...
				{
					DrxPathString sReactionsDataFilePath(szReactionsDataFilePath);
					DrxStringUtils::UnifyFilePath(sReactionsDataFilePath);
	...
	if (pHitDeathReactions != NULL)
	{
		...
		DRX_ASSERT_TRACE((cachedProfileId == INVALID_PROFILE_ID) || (cachedProfileId == key), ("IMPORTANT ASSERT! %s actor's cached ProfileId doesn't match its actual ProfileId!", actor.GetEntity()->GetName()));
	}
#endif
...
ScriptTablePtr CHitDeathReactionsSystem::LoadReactionsScriptTable(const CActor& actor) const
{
	...
	if ((propertiesTable.type == ANY_TTABLE) && propertiesTable.table->GetValue(REACTIONS_DATA_FILE_PROPERTY, szReactionsDataFilePath))
	{
		DrxPathString sReactionsDataFile(szReactionsDataFilePath);
		DrxStringUtils::UnifyFilePath(sReactionsDataFile);
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
ScriptTablePtr CHitDeathReactionsSystem::LoadReactionsScriptTable(tukk szReactionsDataFile) const
{
	DrxPathString sReactionsDataFile(szReactionsDataFile);
	DrxStringUtils::UnifyFilePath(sReactionsDataFile);
	...
	if (itFind == m_reactionsScriptTableCache.end())
	{
			...
			pHitDeathReactionsTable->GetValue(LOAD_XML_DATA_FUNCTION, loadXMLDataFnc))
		{
			...
			// [*DavidR | 23/Jun/2010] ToDo: We should expose DrxAction's XMLLoadScript functionality so it can be used outside
...
void CHitDeathReactionsSystem::GenerateTagMapping( ScriptTablePtr pTags, tukk pArrayName, i32k tagType, STagMappingHelper& tagMappingHelper )
{
	DRX_ASSERT_MESSAGE(tagType<STagMappingHelper::ETagType_NUM, "TagType index out of range!");
	...
	{
		{
			...
			i32k count = pAllowedParts->Count();
...
void CHitDeathReactionsSystem::LoadTagMapping( const CActor& actor, ScriptTablePtr pHitDeathReactionsTable, STagMappingHelper* pTagMappingHelper )
{
	...
	if (pHitDeathReactionsTable->GetValue("TagMap", pTagMap))
	{
		...
		{
			DRX_ASSERT(it.value.type == ANY_TTABLE);
...
{
	DRX_ASSERT(pHitDeathReactionsTable.GetPtr());
	DRX_ASSERT(pHitReactions.get());
	DRX_ASSERT(pDeathReactions.get());
	DRX_ASSERT(pCollisionReactions.get());

	IMannequin &mannequinSys = gEnv->pGame->GetIGameFramework()->GetMannequinInterface();
	const CTagDefinition* pTagDefinition = mannequinSys.GetAnimationDatabaseUpr().LoadTagDefs( MANQ_TAG_FILENAME, true );
	...
	if( pTagDefinition )
	{
		...
		// [*DavidR | 23/Feb/2010] DrxShared pointer doesn't have and overload for unary operator *
...
{	
	DRX_ASSERT(pHitDeathReactionsTable.GetPtr());
	DRX_ASSERT(pHitDeathReactionsConfig.get());

	bool bSuccess = false;

	tukk szCollisionBone = NULL;
	ScriptTablePtr pReactionsConfigTable;
	if (pHitDeathReactionsTable->GetValue(HIT_DEATH_REACTIONS_CONFIG, pReactionsConfigTable))
	{
		...
		{
			IMannequin &mannequinSys = gEnv->pGame->GetIGameFramework()->GetMannequinInterface();
			pHitDeathReactionsConfig->piOptionalAnimationADB = mannequinSys.GetAnimationDatabaseUpr().Load(szAnimDatabase);
		}

		if( const IActionController* piActionController = actor.GetAnimatedCharacter()->GetActionController() )
		{
			u32k MANQ_HITDEATH_FRAGMENT = CCrc32::Compute( MANQ_TAG_FRAGMENT );
	...
	if (pHitDeathReactionsTable->GetValue(szReactionParamsName, pReactionsTable))
	{
		...
		if (g_pGameCVars->g_hitDeathReactions_logReactionAnimsOnLoading)
		{
			...
			if (pAnimSet)
			{
				...
				typedef VectorMap<u32, tukk > animationsLogEntries;
				...
				for (ReactionsContainer::const_iterator itReactions = reactions.begin(); itReactions != itReactionsEnd; ++itReactions)
				{
					...
					if (!reactionAnim.animCRCs.empty())
					{
						...
						{
							u32 animCRC = *iter;
							tukk szAnim = bLogFilePaths ? pAnimSet->GetFilePathByID(pAnimSet->GetAnimIDByCRC(animCRC)) : pAnimSet->GetNameByAnimID(pAnimSet->GetAnimIDByCRC(animCRC));
				...
				{
					DrxLogAlways("* %s non-animation-graph-triggered animations:", szReactionParamsName);
					...
					{
						tukk szAnimName = itLogs->second;
						DrxLogAlways("--- %s", szAnimName);
					}
				}
			}
		}
...
void CHitDeathReactionsSystem::PreProcessStanceParams(SmartScriptTable pReactionTable) const
{
	...
	i32 iCount = pAllowedStancesArray ? pAllowedStancesArray->Count() : 0;
...
{
	DRX_ASSERT(pScriptTable.GetPtr());
	...
	// Cache validation properties
	{
		if (pScriptTable->HaveValue(VALIDATION_SECTION))
		{
			...
			i32 iCount = pValidationParamsArray->Count();
			if (iCount > 0)
			{
	...
	{
		u32 reactionOnCollision = NO_COLLISION_REACTION;
	...
	if (pScriptTable->HaveValue(NO_RAGDOLL_ON_END_PROPERTY))
	{
		...
		reactionParams.flags |= static_cast<i32>(bNoRagdollOnEnd) * SReactionParams::NoRagdollOnEnd;
	}
	...
	if (pScriptTable->HaveValue(REACTION_FINISHES_AIMING_PROPERTY))
	{
		...
		reactionParams.flags |= static_cast<i32>(!bReactionFinishesAiming) * SReactionParams::ReactionFinishesNotAiming;
	}
...
{
	i32k iCount = pScriptTable->Count();
	...
	for( i32 i=0; i<iCount; ++i )
	{
		...
		if( pScriptTable->GetAt( i+1, tagName ) )
		{
			...
			if( pTagMap )
			{
				...
				{
					i32k id = functor( it->second.c_str() );
...
//	PreProcessStanceParams(pScriptTable);
	...
	if (pScriptTable->HaveValue(ONLY_ON_HEALTH_THRESHOLDS))
	{
		...
		i32 iCount = pHealthThresholdsArray->Count();
		if (iCount > 0)
		{
			bHadValidationParams = true;
	...
	if (pScriptTable->HaveValue(ALLOWED_STANCES_ARRAY))
	{
		...
		{
			i32 iCount = pAllowedStancesArray->Count();
			for (i32 i = 0; i < iCount; ++i)
			{
				i32 iStance = -1;
	...
	if (pScriptTable->HaveValue(ALLOWED_HIT_TYPES_ARRAY))
	{
		...
		{
			i32k iCount = pAllowedHitTypesArray->Count();
			for (i32 i = 0; i < iCount; ++i)
			{
				tukk szHitType = NULL;
	...
	if (pScriptTable->HaveValue(ALLOWED_PROJECTILES_ARRAY))
	{
		...
		{
			i32 iCount = pAllowedProjectilesArray->Count();
			for (i32 i = 0; i < iCount; ++i)
			{
				tukk szProjClass = NULL;
	...
	if (pScriptTable->HaveValue(ALLOWED_WEAPONS_ARRAY))
	{
		...
		{
			i32 iCount = pAllowedWeapons->Count();
			for (i32 i = 0; i < iCount; ++i)
			{
				tukk szWeaponClass = NULL;
...
void CHitDeathReactionsSystem::GetReactionAnimParamsFromScript(const CActor& actor, ScriptTablePtr pScriptTable, SReactionParams::SReactionAnim& reactionAnim) const
{
	...
	if (pAnimSet)
	{
		if (pScriptTable->HaveValue(REACTION_ANIM_NAME_PROPERTY))
		{
			...
			{
				i32 iAnimID = pAnimSet->GetAnimIDByName(szAnimName);
				if (iAnimID >= 0)
				{
					u32 animCRC = NameCRCHelper::GetCRC(szAnimName);
						...
						if (pAnimationArray->GetAt(i + 1, pAnimation))
						{
							...
							if (pAnimation->GetValue(ANIM_NAME_PROPERTY, szReactionAnim) && szReactionAnim && (szReactionAnim[0] != '\0'))
							{
								...
								if (pAnimation->GetValue(ANIM_VARIANTS_PROPERTY, variants))
								{
									...
									DrxPathString variantName;

									for (i32 k = 0; k < variants; k++)
									{
										variantName.Format("%s%d", szReactionAnim, k + 1);

										i32 animID = pAnimSet->GetAnimIDByName(variantName);
										if (animID >= 0)
										{
											u32 animCRC = NameCRCHelper::GetCRC(variantName);
											reactionAnim.animCRCs.push_back(animCRC);
										}
										else
										{
								...
								else
								{
									...
									i32 iAnimID = pAnimSet->GetAnimIDByName(szReactionAnim);
									if (iAnimID >= 0)
									{
										u32 animCRC = NameCRCHelper::GetCRC(szReactionAnim);
										reactionAnim.animCRCs.push_back(animCRC);
									}
									else
									{
...
void CHitDeathReactionsSystem::FillAllowedPartIds(const CActor& actor, const ScriptTablePtr pScriptTable, const STagMappingHelper& tagMapping, bool bIsMannequinReaction, SReactionParams& reactionParams, SReactionParams::SValidationParams& validationParams ) const
{
	...
	if (pMainChar)
	{
		...
		{
			i32 iCount = pAllowedPartArray->Count();
			for (i32 i = 0; i < iCount; ++i)
			{
				tukk szPartName = NULL;
				pAllowedPartArray->GetAt(i + 1, szPartName);

				i32 iPartId = pMainChar->GetIDefaultSkeleton().GetJointIDByName(szPartName);
				...
				else
				{
					...
					IAttachmentUpr* pAttachmentUpr = pMainChar->GetIAttachmentUpr();
					i32 iAttachmentIdx = pAttachmentUpr->GetIndexByName(szPartName);
					if (iAttachmentIdx != -1)
					{
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
ECardinalDirection CHitDeathReactionsSystem::GetCardinalDirectionFromString(tukk szCardinalDirection) const
{
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
EAirState CHitDeathReactionsSystem::GetAirStateFromString( tukk szAirState ) const
{
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactionsSystem::OnRequestAnimsTimer(uk pUserData, IGameFramework::TimerID handler)
{
	ProfileId profileId = (ProfileId)(TRUNCATE_PTR)pUserData;
	ProfilesContainer::iterator itFind = m_reactionProfiles.find(profileId);
	DRX_ASSERT(itFind != m_reactionProfiles.end());
	if (itFind != m_reactionProfiles.end())
	{
		SReactionsProfile& profile = itFind->second;
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactionsSystem::OnReleaseAnimsTimer(uk pUserData, IGameFramework::TimerID handler)
{
	ProfileId profileId = (ProfileId)(TRUNCATE_PTR)pUserData;
	ProfilesContainer::iterator itFind = m_reactionProfiles.find(profileId);
	DRX_ASSERT(itFind != m_reactionProfiles.end());
...
void CHitDeathReactionsSystem::AddDirectionToTagState( const ECardinalDirection direction, tukk pPrefix, const STagMappingHelper& tagMapping, TagState& tagState ) const
{
	tukk pShotOrigin = NULL;
	switch( direction )
	{
	case eCD_Forward:
		pShotOrigin = "forward";
