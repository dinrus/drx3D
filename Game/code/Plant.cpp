...
DRX_IMPLEMENT_GTI(CPlant, CFireMode);
...
struct CPlant::StartPlantAction
{
	...
	void execute(CItem *_this)
	{
		...
		u32 animLength = pPlant->m_pWeapon->GetCurrentAnimationTime(eIGS_Owner);

		pPlant->m_pWeapon->GetScheduler()->TimerAction(u32(pPlant->m_fireParams->plantparams.delay*1000.f), CSchedulerAction<MidPlantAction>::Create(pPlant), false);
...
{
	i32 clipSize = m_fireParams->plantparams.clip_size;
	if (clipSize!=0)
		return m_fireParams->plantparams.ammo_type_class && clipSize != -1 && m_pWeapon->GetAmmoCount(m_fireParams->plantparams.ammo_type_class) < (i32)(thresholdPerCent*clipSize);

	return m_fireParams->plantparams.ammo_type_class && (m_pWeapon->GetInventoryAmmoCount(m_fireParams->plantparams.ammo_type_class) < (i32)(thresholdPerCent*clipSize));
}


//------------------------------------------------------------------------
void CPlant::NetShoot(const Vec3 &hit, i32 ph)
{
	NetShootEx(ZERO, ZERO, ZERO, hit, 1.0f, ph);
}

//------------------------------------------------------------------------
void CPlant::NetShootEx(const Vec3 &pos, const Vec3 &dir, const Vec3 &vel, const Vec3 &hit, float extra, i32 ph)
{
...
//------------------------------------------------------------------------
void CPlant::Plant(const Vec3 &pos, const Vec3 &dir, const Vec3 &vel, bool net, i32 ph)
{
...
//------------------------------------------------------------------------
i32 CPlant::GetNumProjectiles() const
{
...
//------------------------------------------------------------------------
void CPlant::GetMemoryUsage(IDrxSizer * s) const
{
