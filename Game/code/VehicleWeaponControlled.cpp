...
void CVehicleWeaponControlled::StartUse(EntityId userId)
{
  ...
  if (pVehicle)
  {
	  ...
	  hudEvent.AddData((i32)pVehicle->GetEntityId());
...
void CVehicleWeaponControlled::Update(SEntityUpdateContext& ctx, i32 update)
{
			...
			if(IVehiclePart *pParentPart = pPart->GetParent())
			{
				...
				if(ICharacterInstance *characterInst = pVehicle->GetEntity()->GetCharacter(pParentPart->GetSlot()))
				{
					if(ISkeletonPose* pose = characterInst->GetISkeletonPose())
					{
						...
						i16 joint = rIDefaultSkeleton.GetJointIDByName(pPart->GetName());
						...
						if (pl)
						{
							...
							else
							{
								...
								i32 boneId = pCharacter ? pCharacter->GetIDefaultSkeleton().GetJointIDByName("Spine03") : 7;
  ...
  if (player)
  {
    if (IEntity *entity = player->GetEntity())
    {
      ...
      if (ICharacterInstance *character = entity->GetCharacter(0))
      {
        ...
        u32 numAnimsLayer = pSkeletonAnim->GetNumAnimsInFIFO(0);
...
void CVehicleWeaponPulseC::Update(SEntityUpdateContext& ctx, i32 update)
{
	if(!m_vehicleId && GetEntity()->GetParent())
	{
		m_vehicleId = GetEntity()->GetParent()->GetId();
