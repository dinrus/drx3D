...
DRX_IMPLEMENT_GTI(CBurst, CSingle);
...
//------------------------------------------------------------------------
void CBurst::Update(float frameTime, u32 frameId)
{
	...
	if (!m_bursting && !m_fireTriggerDown)
	{
		...
		if (m_next_burst <= 0.0f)
		{
			...
			if(m_fired)	//Only set if first shot was successful
			{
				...
				i32 ammoCount = (m_pWeapon->GetAmmoCount(ammo) + 1); 

				const CTagDefinition* pTagDefinition = NULL;
					
				i32 fragmentID = m_pWeapon->GetFragmentID("burst_fire", &pTagDefinition);
				if (fragmentID != FRAGMENT_ID_INVALID)
				{
					...
					{
						i32 burstLength = m_fireParams->burstparams.nshots;
...
Vec3 CBurst::GetFiringDir(const Vec3 &probableHit, const Vec3& firingPos) const
{
	...
	i16k nShots = m_fireParams->burstparams.nshots;
	i16k spreadAngle = m_fireParams->burstparams.spreadAngle;
...
i32 CBurst::CBurstFiringLocator::GetCurrentWeaponSlot() const
{
