...
// -------------------------------------------------------------------------
//  Имя файла:   GameStateRecorder.cpp
//  Version:     v1.00
...
#include <drx3D/Game/Inventory.h>
...
///////////////////////////////////////////////////////////////////////
bool IsAmmoGrenade(tukk className)
{
...
void CGameStateRecorder::StartSession()
{
	...
	tukk filterName = m_demo_actorFilter->GetString();
...
//	}
	...
	{
		IAIObjectUpr* pAIObjMgr = gEnv->pAISystem->GetAIObjectUpr();
...
void CGameStateRecorder::DumpWholeGameState(const CActor* pActor)
{
		...
		if(pItem && itemName)
		{
			...
			if(pWeapon)
			{
				...
				for(TAmmoVector::const_iterator ammoCit = weaponAmmosVector.begin(); ammoCit != ammoEndCit; ++ammoCit)
				{
					...
					tukk ammoClass = weaponAmmo.pAmmoClass ? weaponAmmo.pAmmoClass->GetName() : NULL;
					if(ammoClass)
					{
						event.event = eGE_AmmoCount;
						event.value = (float)weaponAmmo.count;
						event.extra = (uk )ammoClass;
						event.description = (tukk)itemName;
						SendGamePlayEvent(pEntity,event);
					}
				}
			}
	...
	for(pInventory->AmmoIteratorFirst(); !pInventory->AmmoIteratorEnd(); pInventory->AmmoIteratorNext())
	{
		...
		if(pAmmoClass)
		{
			tukk ammoClassName = pAmmoClass->GetName();
			i32 ammoCount = pInventory->AmmoIteratorGetCount();
...
////////////////////////////////////////////////////////////////////////////
void CGameStateRecorder::OnRecordedGameplayEvent(IEntity *pEntity, const GameplayEvent &event, i32 currentFrame, bool bRecording)
{
	EntityId id;
	m_currentFrame = currentFrame;

	i32 demo_forceGameState = 0;
	if(!bRecording)
	{
		...
		case eGE_WeaponFireModeChanged:
			{
				...
				if(sel)
				{
					...
					u8 recFireModeIdx = u8(event.value);
					if(iti != Items.end())
		...
		case eGE_ItemSelected:
			{
				...
				if(m_bLogWarning)
				{
					...
					if(pActor)
					{
						...
						tukk curItemName= pItem && pItem->GetEntity() ? pItem->GetEntity()->GetName(): NULL;
		...
		case eGE_AmmoCount:
			{
				...
				if(it != Items.end())
				{
					...
					tukk ammoClassDesc = (tukk)(event.extra);
					if(ammoClassDesc)
					{
						...
						if(itemAmmo.find(szAmmoClassDesc)==itemAmmo.end())
							itemAmmo.insert(std::make_pair(szAmmoClassDesc,i32(event.value)));
						else
							itemAmmo[szAmmoClassDesc] = (u16)event.value;
									...
									if(pItem)
									{
										...
										if(pWeapon)
										{
											...
											if(pAmmoClass)
											{
												if(m_bLogWarning)
												{
													i32 curAmmoCount = (bAmmoMag ? pInventory->GetAmmoCount(pAmmoClass) :
														pWeapon->GetAmmoCount(pAmmoClass));
													CheckDifference( i32(event.value),curAmmoCount,"TimeDemo:GameState: Frame %d - Actor %s - AMMO COUNT mismatch for ammo class %s (rec:%d, cur:%d)",pEntity,ammoClassDesc);
												}

												if(demo_forceGameState==2)
													pWeapon->SetAmmoCount(pAmmoClass,i32(event.value));
...
void CGameStateRecorder::OnGameplayEvent(IEntity *pEntity, const GameplayEvent &event)
{
	...
	if(bPlayer || m_mode==GPM_AllActors)
	{
		...
		switch(eType)
		{
						...
						if(pCurrentItem)
						{
							...
							if(pCurrentItemClass != NULL && !strcmpi(pCurrentItemClass->GetName(),"SOCOM"))
							{
								...
								if(!currItemName)
									break;
								event3.value = 0;
								event3.description = (tukk)currItemName;
			...
			case eGE_WeaponFireModeChanged:
				{
					...
					if(!itemIdx)//if(itemIdx < 0)
						break;
					event2.description = (tukk)itemIdx;
					SendGamePlayEvent(pEntity,event2);
				}
				break;
			...
			case eGE_AmmoCount:
				{
					tukk itemIdx = event.description;
					if(!itemIdx)
						break;
						...
						{
							{
								...
								if(!weaponAmmo.pAmmoClass)
								{
									...
									if(IsAmmoGrenade((tukk)(event.extra)))
									{
										weaponAmmo.pAmmoClass = gEnv->pEntitySystem->GetClassRegistry()->FindClass((tukk)event.extra);
										weaponAmmo.count = (i32)event.value;
										bGrenade = true;
									}
								}
							}
							...
							{
								const SWeaponAmmo& weaponAmmo = *ammoCit;
								i32k ammoCount = weaponAmmo.count;
...
////////////////////////////////////////////////////////////////////////////
void CGameStateRecorder::GetMemoryStatistics(IDrxSizer * s)
{
...
float CGameStateRecorder::RenderInfo(float y, bool bRecording)
{
	...
	else
	{
		...
		tukk actorName = m_demo_actorInfo->GetString();
		...
		if(pActor)
		{
			...
			if(m_itSingleActorGameState != m_GameStates.end() && pActor->GetEntity() && m_itSingleActorGameState->first == pActor->GetEntity()->GetId())
			{
				...
				if(pVar)
				{
					i32 demo_forceGameState = pVar->GetIVal();
					if(demo_forceGameState)
					{
				...
				pRenderer->Draw2dLabel( xp,y+retY, 1.3f, bError? fColorWarning : fColor, false,"%d",(i32)health);
				pRenderer->Draw2dLabel( xr,y+retY, 1.3f, bError? fColorWarning : fColor, false,"%d",(i32)recordedHealth);
				retY +=15;
				...
				if(pInventory)
				{
					i32 nInvItems = pInventory->GetCount();
					...
					for(; i< nInvItems; i++)
					{
						...
						if(pItem)
						{
							...
							char itemName[32];
							tukk originalItemName = pItem->GetEntity() ? pItem->GetEntity()->GetName():"(null)";
							drx_strcpy(itemName,originalItemName);
							...
							else
							{
								...
								if(pWeapon)
								{
									...
									{
										const SWeaponAmmo& weaponAmmo = *ammoCit;
										i32 ammoCount = weaponAmmo.count;
										tukk ammoClass = weaponAmmo.pAmmoClass ? weaponAmmo.pAmmoClass->GetName() : NULL;
										if(ammoClass)
										{
											...
											{
												i32 recAmmoCount = recItem.Ammo[ammoClass];
									...
									// current fire mode
									i32 curFireModeIdx = pWeapon->GetCurrentFireMode();
									i32 recFireModeIdx = recItem.fireMode;
					...
					i32 nInvAccessories = pInventory->GetAccessoryCount();

					TAccessoryContainer& Accessories = gstate.Accessories;
					i32 nRecAccessories = Accessories.size();
					if(nRecAccessories)
					{
					...
					for(i32 j=0 ; j< nInvAccessories; j++,i++)
					{
						...
						if(pClass)
						{
							...
							char itemName[32];
							drx_strcpy(itemName,szItemName);
							pRenderer->Draw2dLabel( 1,y+retY, 1.3f, bError? fColorWarning : fColor, false,"     %s",itemName);
					...
					/// Ammo Mags
					i32 nInvAmmo = pInventory->GetAmmoPackCount();
					if(nInvAmmo)
					{
					...
					for(pInventory->AmmoIteratorFirst(); !pInventory->AmmoIteratorEnd(); pInventory->AmmoIteratorNext())
					{
						...
						if(pAmmoClass)
						{
							i32 invAmmoCount = pInventory->AmmoIteratorGetCount();
							tukk ammoClassName = pAmmoClass->GetName();
							bool bNotFound = Mags.find(ammoClassName) == Mags.end();
							i32 recAmmoCount = bNotFound ? 0 :Mags[ammoClassName];
	...
	if(pActor)
	{
		...
		if(itGS != m_GameStates.end())
		{
			...
			if(pInventory)
			{
				...
				i32 nInvItems = (bSingleItem ? 1 : pInventory->GetCount());
				TItemContainer& Items = gstate.Items;

				for(i32 i=0; i< nInvItems; i++)
				{
					if(!bSingleItem)
							...
							if(pWeapon)
							{
								...
								for(TAmmoVector::const_iterator ammoCit = weaponAmmosVector.begin(); ammoCit != ammoEndCit; ++ammoCit)
								{
									...
									if(ammoClass)
									{
										...
										if(recAmmo.find(ammoClass) == recAmmo.end())
											recAmmo.insert(std::make_pair(ammoClass,0));
										i32 recAmmoCount = recAmmo[ammoClass];
										if(ammoCount!=recAmmoCount)
										{
											...
											ammoCountEvent.extra = (uk )(ammoClass);
											ammoCountEvent.description = (tukk)szItemName;
											//eventHandler(pActor,event);
								...
								if(curFireModeIdx!= recFireModeIdx)
								{
									...
									fireModeEvent.description = (tukk)szItemName;
									//eventHandler(pActor,event);
				...
				for(i32 j=0 ; j< nInvAccessories; j++)
				{
					const IEntityClass* pClass = pInventory->GetAccessoryClass(j);
					if(pClass)
					{
...
///////////////////////////////////////////////////////////////////////////////////////
CActor* CGameStateRecorder::GetActorOfName( tukk name)
{
	if(!strcmpi(name,"all"))
		return NULL;
