...
void CPlayerStateJump::StartJump( CPlayer& player, const bool isHeavyWeapon, const float fVerticalSpeedModifier )
{
...

	{
		...
		{
			DrxWatch("Jumping: vec from player BaseQuat only = (%f, %f, %f)", jumpVec.x, jumpVec.y, jumpVec.z);
		}
#endif
		...
		if (g_pGameCVars->pl_debugInterpolation > 1)
		{
			DrxWatch("Jumping (%f, %f, %f)", jumpVec.x, jumpVec.y, jumpVec.z);
		}
#endif
	}
...
void CPlayerStateJump::SetJumpState(CPlayer& player, EJumpState jumpState)
{
	...
	{
		DRX_ASSERT( m_jumpState >= JState_None && m_jumpState < JState_Total );
...
void CPlayerStateJump::Landed(CPlayer& player, const bool isHeavyWeapon, float fallSpeed)
{
	...
	DRX_ASSERT_MESSAGE( player.GetLinkedEntity()==NULL || remoteControlled, "Cannot 'land' when you're linked to another entity!" );
#endif
	...
	const SActorPhysics& actorPhysics = player.GetActorPhysics();
	i32 matID = actorPhysics.groundMaterialIdx != -1 ? actorPhysics.groundMaterialIdx : mfx->GetDefaultSurfaceIndex();
	...
	if(player.m_isClient)
	{
		if (fallSpeed > 0.0f)
		{
			...
			const float direction = (drx_random(0, 1) == 0) ? -1.0f : 1.0f;
