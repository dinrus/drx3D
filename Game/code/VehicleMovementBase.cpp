...
$Id$
$DateTime$
Описание: Implements a base class for vehicle movements
...
{ 
	DRX_ASSERT(DRX_ARRAY_COUNT(m_pWind) == 2);
...
bool CVehicleMovementBase::Init(IVehicle* pVehicle, const CVehicleParams& table)
{
	...
	DRX_ASSERT(pForceFeedbackSystem);
	...
	{
		i32	numberOfChildren = forceFeedback.getChildCount();
...
{ 
  ...
  for (i32 i=0; i < (i32)m_pPaParams->GetExhaustParams()->GetExhaustCount(); ++i)
  {
    SExhaustStatus stat;
	...
	if (m_isEngineStarting)
	{
		...
		if (m_runSoundDelay>0.f && m_engineStartup >= m_runSoundDelay)
		{
			...
			if(m_pVehicle->IsPlayerPassenger())
			{
				...
				{
					DRX_ASSERT(g_pGame->GetIGameFramework()->GetIForceFeedbackSystem());
...
i32 CVehicleMovementBase::GetStatus(SVehicleMovementStatus* _status)
{
...
//------------------------------------------------------------------------
void CVehicleMovementBase::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
...
void CVehicleMovementBase::Boost(bool enable)
{
  ...
  if(m_pPaParams)
	{
		tukk boostEffect = m_pPaParams->GetExhaustParams()->GetBoostEffect();  
		if (IParticleEffect* pBoostEffect = gEnv->pParticleUpr->FindEffect(boostEffect))
		{
...
//------------------------------------------------------------------------
void CVehicleMovementBase::ApplyAirDamp(float angleMin, float angVelMin, float deltaTime, i32 threadSafe)
{
...
void CVehicleMovementBase::OnEvent(EVehicleMovementEvent event, const SVehicleMovementEventParams& params)
{
  ...
  else if (event == eVME_ToggleEngineUpdate)
  {
	...
	else if (event == eVME_NW_Discharge)
	{
		...
		{
			if(IParticleEffect* pEffect = gEnv->pParticleUpr->FindEffect(params.sParam))
			{
				SpawnParams sp;
...
{ 
  ...
  if (ignition)
  {
    if (IParticleEffect* pEff = gEnv->pParticleUpr->FindEffect(exParams->GetStartEffect()))    
    {
  ...
  if (IParticleEffect* pRunEffect = gEnv->pParticleUpr->FindEffect(exParams->GetRunEffect()))
  { 
    SEntitySlotInfo slotInfo;
...
//------------------------------------------------------------------------
void CVehicleMovementBase::FreeEmitterSlot(i32& slot)
{
  if (slot > -1)
  {
    //DrxLogAlways("[VehicleMovement]: Freeing slot %i", slot);
...
//------------------------------------------------------------------------
void CVehicleMovementBase::FreeEmitterSlot(i32k& slot)
{
  if (slot > -1)  
    m_pEntity->FreeSlot(slot);     
}
...
{ 
  ...
  {
    IParticleEffect* pEff = gEnv->pParticleUpr->FindEffect(exParams->GetStopEffect());
    if (pEff)    
    { 
      for (std::vector<SExhaustStatus>::iterator it = m_paStats.exhaustStats.begin(); it != m_paStats.exhaustStats.end(); ++it)
      {
        ...
        {          
          it->startStopSlot = m_pEntity->LoadParticleEmitter(it->startStopSlot, pEff);
					...
					it->pHelper->GetVehicleTM(tm);
          m_pEntity->SetSlotLocalTM(it->startStopSlot, tm);                  
          //DrxLogAlways("[VehicleMovement::StopExhaust]: Loaded to slot %i..", it->startStopSlot);
        }
      }
    }
  }
...
bool CVehicleMovementBase::IsProfilingMovement()
{
	...
	tukk entityName = m_pVehicle->GetEntity()->GetName();
...
//------------------------------------------------------------------------
SMFXResourceListPtr CVehicleMovementBase::GetEffectNode(i32 matId)
{
	if (matId <= 0)
		return 0;
	...
	tukk mfxRow = m_pVehicle->GetParticleParams()->GetEnvironmentParticles()->GetMFXRowName();
...
#if ENABLE_VEHICLE_DEBUG
		if (DebugParticles())
			DrxLog("GetEffectString for %s -> %i failed", mfxRow, matId);
#endif
	}
...
//------------------------------------------------------------------------
tukk CVehicleMovementBase::GetEffectByIndex(i32 matId, tukk username)
{  
    ...
    { 
      ...
      { 
        ...
        if (pGroundEffect)
        {
		  ...
		  tukk effectRow = m_pVehicle->GetParticleParams()->GetEnvironmentParticles()->GetMFXRowName(); 
...
#if ENABLE_VEHICLE_DEBUG
          if (DebugParticles())
            DrxLog("<%s> Ground effect loaded with height %f", m_pVehicle->GetEntity()->GetName(), layer.alignGroundHeight);
#endif
        }
      }      
      else if (layer.alignToWater)
      {
        ...
        if (DebugParticles())
        {
          ...
          DrxLog("<%s> water-aligned emitter %i, local pos: %.1f %.1f %.1f", m_pVehicle->GetEntity()->GetName(), i, loc.x, loc.y, loc.z);          
        }              
#endif
      }
    }
...
{ 
  ...
  if (!m_pWind[0])
  {
    ...
    IGeometry *pGeom1 = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive( primitives::box::type, &geomBox );    
    if (pGeom1)    
      m_pWind[0] = gEnv->pPhysicalWorld->AddArea( pGeom1, m_pEntity->GetWorldPos(), m_pEntity->GetRotation(), 1.f);

    IGeometry *pGeom2 = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive( primitives::box::type, &geomBox );    
    if (pGeom2)    
...
//------------------------------------------------------------------------
void CVehicleMovementBase::SetSoundParam(EVehicleMovementSound eSID, tukk param, float value)
{
...
void CVehicleMovementBase::CacheAudioControlIDs()
{
	...
	{
		string triggerName = "";
		tukk vehicleName = m_pVehicle->GetEntity()->GetClass()->GetName(); 
