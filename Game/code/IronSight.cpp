...
DRX_IMPLEMENT_GTI_BASE(CIronSight);
...
//------------------------------------------------------------------------
void CIronSight::InitZoomMode(IWeapon *pWeapon, const SParentZoomModeParams* pParams, u32 id)
{
...
//------------------------------------------------------------------------
void CIronSight::Update(float frameTime, u32 frameId)
{
...
//------------------------------------------------------------------------
bool CIronSight::StartZoom(bool stayZoomed, bool fullZoomout, i32 zoomStep)
{
	...
	{
		m_blendNearFoV = true;
		i32 currentStep = m_currentStep;
		i32 nextStep = currentStep+1;

		if (nextStep > (i32)m_zoomParams->zoomParams.stages.size())
		{
			if (!stayZoomed)
			{
				if (fullZoomout)
...
//------------------------------------------------------------------------
void CIronSight::EnterZoom(float time, bool smooth, i32 zoomStep)
{
	...
	{
		tukk zoomModeSuffix = m_zoomParams->zoomParams.suffix.c_str();

		if(zoomModeSuffix && zoomModeSuffix[0])
		{
			u32 zoommodeCrC = CCrc32::ComputeLowercase(zoomModeSuffix);
...
void CIronSight::TurnOff(bool enable, bool smooth, bool anim)
{
	...
	if (!enable && (m_savedFoVScale > 0.0f))
	{
		...
		m_pWeapon->GetScheduler()->TimerAction((u32)(zoomParams.zoom_out_time*1000), CSchedulerAction<DisableTurnOffAction>::Create(this), false);
		m_savedFoVScale = 0.0f;
	}
	else if (m_zoomed && enable)
	{
		...
		m_pWeapon->GetScheduler()->TimerAction((u32)(zoomParams.zoom_out_time*1000), CSchedulerAction<EnableTurnOffAction>::Create(this), false);
	}
}

//------------------------------------------------------------------------
void CIronSight::ZoomIn(float time, float from, float to, bool smooth, i32 nextStep)
{
	m_zoomTime = time;
	...
	if (m_pWeapon->IsOwnerClient())
	{
		tukk szZoomOverlay = GetShared()->zoomParams.zoom_overlay.c_str();
...
//------------------------------------------------------------------------
void CIronSight::ZoomOut(float time, float from, float to, bool smooth, i32 nextStep)
{
	m_zoomTimer = time;
...
{
	i32 currentStep = CLAMP(m_currentStep, 0, (i32)m_zoomParams->zoomParams.stages.size() - 1);
	if(m_pWeapon->IsOwnerClient())
	{
...
//------------------------------------------------------------------------
void CIronSight::OnZoom(float time, i32 targetStep)
{
	if(m_pWeapon->IsOwnerClient())
	{
		...
		i32k numSteps = m_zoomParams->zoomParams.stages.size();
...
//------------------------------------------------------------------------
float CIronSight::GetZoomFoVScale(i32 step) const
{
	if (!step)
		return 1.0f;
...
//-------------------------------------------------------------------------
void CIronSight::GetMemoryUsage(IDrxSizer * s) const
{
...
void CIronSight::FilterView(SViewParams &viewparams)
{
	...
	DRX_ASSERT_MESSAGE(pOwnerPlayer, "Ironsight:FilterView - This should never happen. This is supposed to be the player's current weapon, but it's not!");
	if(pOwnerPlayer)
	{
		...
		if(IFireMode* pFM = m_pWeapon->GetFireMode(m_pWeapon->GetCurrentFireMode()))
		{
			...
			if(pFM->IsFiring())
			{
				...
				if(currentRecoil>1.5f)
					currentRecoil = 1.5f + drx_random(-1.25f,0.65f);
				else if(currentRecoil>0.6f)
					currentRecoil = currentRecoil + drx_random(-0.4f,0.3f);
...
//===================================================
i32 CIronSight::GetMaxZoomSteps() const
{
...
{
	DRX_ASSERT_MESSAGE(!(m_zoomingIn && (m_currentStep == 0)), "GetZoomTransition zooming in whilst on step 0, this should never happen!");

	i32k zoomStep = m_zoomingIn ? -1 : 1;
	i32k prevStep = m_currentStep + zoomStep;
...
//===================================================
i32 CIronSight::GetCurrentStep() const
{
...
IMaterial* CIronSight::GetCrossHairMaterial() const
{
	...
	{
		for (i32 slot = 0; slot < pMaterial->GetSubMtlCount(); ++slot)
		{
...
{
	i32k currentStep = CLAMP(m_currentStep - 1, 0, (i32)m_zoomParams->zoomParams.stages.size() - 1);
