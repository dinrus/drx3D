...
, type("frag")
{
	...
	tukk friendlyFireType = reader.ReadParamValue("friendly_fire", "none");
	...
	{
		DRX_ASSERT_MESSAGE(false, "Unrecognised friendly_fire type");
	}
}
...
namespace
{
	...
	void PushBackSurfaceString(SMikeBulletParams::TBurnSurfaceIndexMap* pBurnSurfaceIndexMap, tukk pString, i32 index)
	{
	...
	void PushBackClassString(SMikeBulletParams::TBurnClassIndexMap* pBurnClassIndexMap, tukk pString, i32 index)
	{
	...
	struct ParseString
	{
		typedef std::vector<std::pair<Key, i32> > TVectorMap;
		typedef void (*TPushBackFn)(TVectorMap* pVectorMap, tukk pString, i32 index);

		void operator () (u8k* pString, i32 materialIndex, TVectorMap* pVectorMap, TPushBackFn pushBackFunction)
		{
			i32k stringSize = 128;
			if(pString)
			{
				char subString[stringSize];
				for (i32 idx = 0; ; ++pString)
				{
					if (isspace(*pString))
						continue;
...
:	m_covertedIndexes(false)
{
	...
	i32k numChildren = reader.GetUnfilteredChildCount();
	for (i32 i = 0; i < numChildren; ++i)
	{
		...
		if (childNode && strcmp(childNode->getTag(), "effect")==0)
		{
			...
			u8k* materialTypes = (u8*)childReader.ReadParamValue("material");
			u8k* classTypes = (u8*)childReader.ReadParamValue("class");
			...
			if (materialTypes || classTypes)
			{
				...
				i32 index = (i32)(m_burnEffects.size() - 1);
...
const SMikeBulletParams::SBurnParams* SMikeBulletParams::GetBurnSurfaceParams(i32 surfaceType) const
{
...
void SMikeBulletParams::CacheMaterialIds()
{
	...
	{
		ISurfaceType* pSurfaceType = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr()->GetSurfaceTypeByName(burnIt->first.m_materialTypeName.c_str());
		if (pSurfaceType)
		{
...
void SAmmoParams::GetMemoryUsage(IDrxSizer *pSizer) const
{
	pSizer->AddObject(this, sizeof(*this));
	pSizer->AddObject(fpGeometryName);
}

SExplosionParams*	SAmmoParams::GetExplosionParams(i32 index) const
{
	if(pExplosions)
	{
		DRX_ASSERT_MESSAGE(m_explosion_count > index, "SAmmoParams::GetExplosionParams - Rerquesting explosion with index > size of the array");
...
void SAmmoParams::LoadFlagsAndParams(const XmlNodeRef& ammoParamsNode)
{
	...
	if (flagsNode)
	{
		...
		flagsReader.ReadParamValue<i32>("ClientOnly", flag); flags |= flag?ENTITY_FLAG_CLIENT_ONLY:0; flag=0;
		flagsReader.ReadParamValue<i32>("ServerOnly", flag); flags |= flag?ENTITY_FLAG_SERVER_ONLY:0; flag=0;
	...
	if (paramsNode)
	{
		...
		paramsReader.ReadParamValue<i32>("bulletType", bulletType);
		paramsReader.ReadParamValue<i32>("hitPoints", hitPoints);
		...
		tukk typeName = paramsReader.ReadParamValue("aitype");
...
void SAmmoParams::LoadPhysics(const XmlNodeRef& ammoParamsNode)
{
	...
	tukk typ = physicsNode->getAttr("type");
	if (typ)
	{
	...
	if(physicalizationType != ePT_Static)
	{
		...
		physicsReader.ReadParamValue<i32>("max_logged_collisions", maxLoggedCollisions);
		...
		physicsReader.ReadParamValue<i32>("bounceableBullet", bounceableBullet);
	}
	...
	if (physicalizationType == ePT_Particle)
	{
		...
		physicsReader.ReadParamValue<i32>("pierceability", pParticleParams->iPierceability);
		physicsReader.ReadParamValue<Vec3>("rollAxis", pParticleParams->rollAxis);

		i32 flag = 0;
		physicsReader.ReadParamValue<i32>("single_contact", flag);
		pParticleParams->flags = flag?particle_single_contact:0; flag=0;
		physicsReader.ReadParamValue<i32>("constant_orientation", flag);
		pParticleParams->flags |= flag?particle_constant_orientation:0; flag=0;
		physicsReader.ReadParamValue<i32>("no_roll", flag);
		pParticleParams->flags |= flag?particle_no_roll:0; flag=0;
		physicsReader.ReadParamValue<i32>("no_spin", flag);
		pParticleParams->flags |= flag?particle_no_spin:0; flag=0;
		physicsReader.ReadParamValue<i32>("no_path_alignment", flag);
...
void SAmmoParams::LoadGeometry(const XmlNodeRef& ammoParamsNode)
{
	...
	if (firstpersonNode)
	{
		tukk modelName = firstpersonNode->getAttr("name");
...
void SAmmoParams::CacheResources() const
{
	...
	{
		ISurfaceType* pSurfaceType = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeByName(material_name.c_str());
		if (pSurfaceType)
	...
	if(pExplosions)
	{
		//t count = DRX_ARRAY_COUNT(pExplosions);
		for(i32 i = 0; i < m_explosion_count; ++i)
		{
...
{
	static i32k maxExplosions = 4;

	CGameXmlParamReader reader(ammoParamsNode);

	i32k childCount = ammoParamsNode->getChildCount();
	//i32 explosionCount = 0;
	XmlNodeRef explosions[maxExplosions];
	
	i32 finalExplosionCount = 0;
	for (i32 i= 0; (i < childCount) && (finalExplosionCount < maxExplosions); ++i)
	{
