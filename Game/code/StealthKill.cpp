...
CStealthKill::ECanExecuteOn CStealthKill::CanExecuteOn(CActor* pTarget, bool checkExtendedRange) const
{
	...
	const SAutoaimTarget* pTargetInfo = g_pGame->GetAutoAimUpr().GetTargetInfo(targetId);
				...
				if (distSquared >= sqr(maxNormalDistance))
				{
					...
					if (pPhysics && pPhysics->GetParams(&dimensions))
					{
						if (dimensions.bUseCapsule)
						{
							...
							DRX_ASSERT(numSkipEntities <= (sizeof(pSkipEntities) / sizeof *(pSkipEntities)));
			...
			if(pStats && pStats->bAttemptingStealthKill)
			{
				if(m_attemptTimer > 0.f)
				{
					...
					else
					{
						...
						g_pGame->GetAutoAimUpr().SetCloseCombatSnapTarget(m_targetId, s_stealthKill_attemptRange, s_stealthKill_attemptSpeed);
					}			
				}
				else
				{
...
void CStealthKill::DoDeathBlow(CActor* pTarget)
{
	if(pTarget)
	{
		...
		if( !pTarget->GetActorStats()->isRagDoll )
		{
			...
			{
				DrxLog("CStealthKill::DeathBlow tried to ragdollize player '%s' but he has already revived", pTarget->GetEntity()->GetName());
			}
		}
		...
		{	
			...
			if (pStealthKillParams)
			{
				...
				i32 boneId = pCharacter ? pCharacter->GetIDefaultSkeleton().GetJointIDByName("Spine04") : -1;
				if (boneId != -1)
				{
...
{
	DRX_ASSERT_MESSAGE(m_isBusy, "Stealth kill should be in progress when triggering the death blow");
	if (!m_isBusy)
		return;
...
{
	DRX_ASSERT_MESSAGE(m_isBusy, "Stealth kill cannot be stopped if it is not in progress");
	if (!m_isBusy)
		return;
...
//-----------------------------------------------------------------------
void CStealthKill::Enter(i32 targetEntityId, i32 animIndex)
{
	DRX_ASSERT_MESSAGE(!m_isBusy, "Stealth kill should not be initiated while a stealth kill is already in progress");
	if (m_isBusy)
		return;

	CActor *pTargetActor = static_cast<CActor*>(gEnv->pGame->GetIGameFramework()->GetIActorSystem()->GetActor(targetEntityId));
	DRX_ASSERT_MESSAGE(pTargetActor != NULL, "Stealth kill should act upon an actor");
	if (pTargetActor == NULL)
		return;

	DRX_ASSERT_MESSAGE(pTargetActor->GetActorClass() == CPlayer::GetActorClassType(), "TargetActor is not a CPlayer");
	...
	i32 randomAnim = 0;
	{
		...
		DRX_ASSERT_MESSAGE( !gEnv->bMultiplayer || animIndex < 4, "NetSerialize_StealthKill doesn't support more than 4 animation variation for stealthkill!!" );
...
const CStealthKill::SStealthKillParams* CStealthKill::GetParamsForClass( IEntityClass* pTargetClass )
{
	...
	i32k paramsCount = s_stealthKillParams.size();
...
{
	DRX_ASSERT_MESSAGE(!m_isBusy, "Trying to set an attempted target while mid attack");
	DRX_ASSERT_MESSAGE(m_targetId == 0, "Trying to override an existing target");
...
void CStealthKill::AddPhysics(IPhysicalEntity **pArr, i32 &pos, CActor *pPlayer)
{
