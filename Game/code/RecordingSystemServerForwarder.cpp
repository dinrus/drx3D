...
void CServerKillCamForwarder::Update()
{
	...
	{
		i32 maxCycles=m_forwarding.size();
		bool bSent=false;
		do 
		{
			...
			else
			{
				if (forward.m_sent!=forward.m_numPackets)
				{
					DrxLog("Underflowed forwarding packetID %d from actor %s after sending %d packets\n", forward.packetID, forward.pActor->GetEntity()->GetName(), forward.m_sent);
					if (forward.m_lastPacketTime.GetDifferenceInSeconds(now)>=k_timeout)
					{
						DrxLog("Underflowing packet timed out, did the client disconnect? Giving up :(\n");
					}
					else
					{
...
void CServerKillCamForwarder::DropOldestPacket()
{
	if (m_history.Empty())
	{
		DrxFatalError("We're trying to drop a packet to make space for new data but there's nothing left in the buffer\n");
	}
	...
	for (std::deque<SForwardingPacket>::iterator it=m_forwarding.begin(), end=m_forwarding.end(); it!=end; ++it)
	{
		...
		{
			DrxLog("Server forwarder queue is overflowing! Forcing packet send to free space\n");
			ForwardPacket(forward, pPacket);
		}
	}
}

void CServerKillCamForwarder::AddDataAndErase(ukk data, size_t size)
{
...
CActor::KillCamFPData* CServerKillCamForwarder::FindPacket(BufferType::iterator &it, IActor *pActor, i32 packetId)
{
...
void CServerKillCamForwarder::GetFirstPacket(BufferType::iterator &first, IActor *pActor, i32 packetId)
{
	// Find first packet for sender actor/packet id taking care to ignore packet ID overflow
	i32 numPackets=0, foundPackets=0;
	bool bSetFirst=false;
	BufferType::iterator it;
	i32 middlePacketId=(packetId+(CActor::KillCamFPData::UNIQPACKETIDS/2))%CActor::KillCamFPData::UNIQPACKETIDS;
