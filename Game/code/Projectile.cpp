...
DRX_IMPLEMENT_GTI_BASE(CProjectile);
...
void CProjectile::SMaterialLookUp::Init()
{
	...
	tukk materialNames[eType_Count] = 
	{
		"mat_water",
	};
	...
	DRX_ASSERT(pSurfaceUpr != NULL);

	for (i32 i = 0; i < eType_Count; ++i)
	{
		ISurfaceType* pSurfaceType = pSurfaceUpr->GetSurfaceTypeByName(materialNames[i]);
...
//------------------------------------------------------------------------
CProjectile::SProjectileDesc::SProjectileDesc(EntityId _ownerId, EntityId _hostId, EntityId _weaponId, i32 _damage, float _damageFallOffStart, float _damageFallOffAmount, float _damageFallOffMin, i32 _hitTypeId, int8 _bulletPierceabilityModifier, bool _aimedShot)
	:	ownerId(_ownerId)
	,	hostId(_hostId)
	,	weaponId(_weaponId)
	,	damage(_damage)
...
//------------------------------------------------------------------------
bool CProjectile::SetAspectProfile( EEntityAspects aspect, u8 profile )
{
	...
	if (aspect == eEA_Physics)
	{
		...
		{
			DrxLog("CProjectile::SetAspectProfile trying to set physics aspect profile to %d, but we've already done so", profile);
			return true;
		}

		Vec3 spin(m_pAmmoParams->spin);
		Vec3 spinRandom = drx_random_componentwise(-m_pAmmoParams->spinRandom, m_pAmmoParams->spinRandom);
		spin += spinRandom;
		spin = DEG2RAD(spin);
...
//------------------------------------------------------------------------
bool CProjectile::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 pflags)
{
...
bool CProjectile::Init(IGameObject *pGameObject)
{
	...
	if (lifetime > 0.0f)
		pEntity->SetTimer(ePTIMER_LIFETIME, (i32)(lifetime*1000.0f));

	float showtime = m_pAmmoParams->showtime;
	if (showtime > 0.0f)
	{
		pEntity->SetSlotFlags(0, pEntity->GetSlotFlags(0)&(~ENTITY_SLOT_RENDER));
		pEntity->SetTimer(ePTIMER_SHOWTIME, (i32)(showtime*1000.0f));
	}
	else
		pEntity->SetSlotFlags(0, pEntity->GetSlotFlags(0)|ENTITY_SLOT_RENDER);
...
void CProjectile::ReInitFromPool()
{
	...
	float lifetime = m_pAmmoParams->lifetime;
	if (lifetime > 0.0f)
		GetEntity()->SetTimer(ePTIMER_LIFETIME, (i32)(lifetime*1000.0f));

	float showtime = m_pAmmoParams->showtime;
	if (showtime > 0.0f)
	{
		GetEntity()->SetSlotFlags(0, GetEntity()->GetSlotFlags(0)&(~ENTITY_SLOT_RENDER));
		GetEntity()->SetTimer(ePTIMER_SHOWTIME, (i32)(showtime*1000.0f));
	}
	else
		GetEntity()->SetSlotFlags(0, GetEntity()->GetSlotFlags(0)|ENTITY_SLOT_RENDER);
...
bool CProjectile::ReloadExtension( IGameObject * pGameObject, const SEntitySpawnParams &params )
{
	ResetGameObject();
	Proj::RegisterEvents(*this,*pGameObject);
	DRX_ASSERT_MESSAGE(false, "CProjectile::ReloadExtension not implemented");
...
{
	DRX_ASSERT_MESSAGE(false, "CProjectile::GetEntityPoolSignature not implemented");
...
//------------------------------------------------------------------------
void CProjectile::Update(SEntityUpdateContext &ctx, i32 updateSlot)
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);

	DRX_ASSERT_MESSAGE(!RequiresDelayedDestruct() || gEnv->bMultiplayer, "The mpProjectileDestructDelay ammo params should only ever be greater than zero in Multiplayer");
	...
	if(!CheckAnyProjectileFlags(ePFlag_failedDetonation))
	{
		...
		{
			DrxWatch("SLOT #%d: Projectile: '%s' %p (physics type %d, bullet type %d)", updateSlot, GetEntity()->GetClass()->GetName(), this, m_pAmmoParams->physicalizationType, m_pAmmoParams->bulletType);
		}
#endif
	...
	case ENTITY_EVENT_TIMER:
		{
			...
			case ePTIMER_SHOWTIME:
				{
					IEntity * pEntity = GetEntity();
					u32k uNewFlags = pEntity->GetSlotFlags(0) | ENTITY_SLOT_RENDER;
...
//------------------------------------------------------------------------
void CProjectile::SetVelocity(const Vec3 &pos, const Vec3 &dir, const Vec3 &velocity, float speedScale, Vec3* appliedVelocityOut, i32 bThreadSafe)
{
	if (!m_pPhysicalEntity)
		return;
...
void CProjectile::OnLaunch() const
{
	if(gEnv->IsClient() && m_pAmmoParams->trackOnHud)
	{
		...
		if(pClientActor)
		{
			...
			eventGrenade.AddData(SHUDEventData((i32)projectileEntId));
...
//			pGameRules->RemoveHitListener(this);
	...
	{
		pWeaponSystem->GetTracerUpr().OnBoundProjectileDestroyed(m_boundToTracerIdx, projectileId, GetEntity()->GetWorldPos());
	}

	if (m_threatTrailTracerIdx != -1)
	{
		pWeaponSystem->GetTracerUpr().OnBoundProjectileDestroyed(m_threatTrailTracerIdx, projectileId, GetEntity()->GetWorldPos());
	}
	...
	if(!m_pAmmoParams->serverSpawn || gEnv->bServer)
	{
		...
		if (pExplosionParams)
		{
			...
			if (gEnv->bServer)
			{
				...
				{
					char msg[1024] = {0};
					drx_sprintf(msg, "missing network safe class id for entity class %s", GetEntity()->GetClass()->GetName());
					DRX_ASSERT_MESSAGE(false, msg);
				}
#endif
			}
		}
		...
		if(!gEnv->bMultiplayer)
		{
			...
			if (IsGrenade() && m_ownerId != g_pGame->GetIGameFramework()->GetClientActorId())
			{
				...
				eventGrenade.AddData(SHUDEventData((i32)GetEntityId()));
				CHUDEventDispatcher::CallEvent(eventGrenade);
			}
		}
...
void CProjectile::TrailEffect(bool enable)
{
	if (enable)
	{
		...
		tukk effectName = useFPTrail ? pTrail->effect_fp.c_str() : pTrail->effect.c_str();
...
//////////////////////////////////////////////////////////////////////////
IParticleEffect* CProjectile::GetCachedEffect(tukk effectName) const
{
...
void CProjectile::Ricochet(EventPhysCollision *pCollision)
{
	...
	if (matPierceability>=8 || drx_random(0.0f, 1.0f)>probability)
		return;
	...
	float rx=drx_random(-0.5f, 0.5f);
	float rz=drx_random(-0.5f, 0.5f);
	...
	if (distanceSq < 7.5*7.5 && (t>=0.0f && t<=1.0f))
	{
		if (distanceSq >= 0.25*0.25)
		{
			...
			Vec3 entry,exit;
			i32 intersect=Intersect::Lineseg_Sphere(line, s, entry,exit);
...
void CProjectile::OnHit(const HitInfo& hit)
{
	if (gEnv->bMultiplayer)
	{
		...
		{
			DRX_ASSERT(RequiresDelayedDestruct());
			return;
		}
	}
...
void CProjectile::OnServerExplosion(const ExplosionInfo& explosion)
{
    ...
    distance = max(0.0f, min(distance,explosion.radius));
		...
		m_hitPoints -= (i32)(effect*explosion.damage);
...
void CProjectile::GetMemoryUsage(IDrxSizer *pSizer) const
{
...
u8 CProjectile::GetDefaultProfile( EEntityAspects aspect )
{
	if (aspect == eEA_Physics)
...
void CProjectile::InitWithAI()
{
	...
	if (m_pAmmoParams->aiType != AIOBJECT_NONE)
	{
		if (IEntity* pOwnerEntity = gEnv->pEntitySystem->GetEntity(m_ownerId))
		{
			...
			{
				u16 nOwnerType = pOwnerAI->GetAIType();
...
//----------------------------------------------------------------
void CProjectile::ResolveTarget(EventPhysCollision* pCollision, i32& targetId, i32& sourceId, IEntity*& pTargetEntity) const 
{
	sourceId = 0;
	targetId = 1;
...
//-----------------------------------------------------------------------
void CProjectile::SetUpParticleParams(IEntity* pOwnerEntity, u8 pierceabilityModifier)
{
	DRX_ASSERT(m_pPhysicalEntity);

	pe_params_particle pparams;
	pparams.pColliderToIgnore = pOwnerEntity ? pOwnerEntity->GetPhysics() : NULL;
	if (m_pAmmoParams)
	{
		pparams.iPierceability = max(0, min(m_pAmmoParams->pParticleParams->iPierceability + pierceabilityModifier, (i32)sf_max_pierceable));
...
//-----------------------------------------------------------------------
void CProjectile::RegisterLinkedProjectile(u8 shotIndex)
{
...
bool CProjectile::ProximityDetector_MP(float proxyRadius)
{
	...
	const static IEntityClass* sVTOLClass					=	gEnv->pEntitySystem->GetClassRegistry()->FindClass(CVTOLVehicleUpr::s_VTOLClassName);

	IVehicleSystem * pVehicleSystem = g_pGame->GetIGameFramework()->GetIVehicleSystem();

	CAutoAimUpr& rAutoAimUpr = g_pGame->GetAutoAimUpr();

	const TAutoaimTargets& rAutoAimTargets = rAutoAimUpr.GetAutoAimTargets();
	...
	for(i32 i = 0, size = rAutoAimTargets.size(); i < size; i++)
	{
...
void CProjectile::ProcessElectricHit(const SElectricHitTarget& target)
{
	...
	CWaterPuddle* pPuddle = g_pGame->GetWaterPuddleUpr()->FindWaterPuddle(target.hitPosition);
	CGameRules* pGameRules = g_pGame->GetGameRules();
	i32k ownerTeam = pGameRules->GetTeam(m_ownerId);
