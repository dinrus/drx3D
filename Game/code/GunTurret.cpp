...
namespace 
{
  ...
  static u32k PROBE_TYPES = ent_terrain|ent_static|ent_sleeping_rigid|ent_rigid;
  static u32k PROBE_FLAGS = (geom_colltype_ray)<<rwi_colltype_bit|rwi_colltype_any|rwi_force_pierceable_noncoll|rwi_ignore_solid_back_faces|8;
...
namespace GT
{
	...
	{
		i32k eventID[] = {eCGE_Turret_LockedTarget, eCGE_Turret_LostTarget, eCGE_SetTeam};
		gameObject.UnRegisterExtForEvents( &goExt, NULL, 0 );
		gameObject.RegisterExtForEvents( &goExt, eventID, sizeof(eventID)/sizeof(i32) );
	}
}
...
//------------------------------------------------------------------------
bool CGunTurret::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
	// call base class
...
//------------------------------------------------------------------------
void CGunTurret::OnHit(float damage, i32 hitType)
{
...
//------------------------------------------------------------------------
void CGunTurret::OnAction(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
...
{ 
  ...
  // sweep on ground    
  i32 nhints = m_weaponsharedparams->pTurretParams->fireParams.hints.size();
...
bool CGunTurret::IsTargetHostile(IActor *pTarget) const
{
	...
	i32 team = g_pGame->GetGameRules()->GetTeam(pTarget->GetEntityId());
	...
	if (pVehicle)
	{
	i32 seatCount=pVehicle->GetSeatCount();
	for (i32 i=0; i<seatCount; i++)
	{
...
bool CGunTurret::IsInRange(const Vec3& pos, ETargetClass cl)const
{
	...
	if(dist>r*r+0.1f)
  {
    //if (DebugTurret()) DrxLog("Target out of range (%.1f > %.1f)", sqrt(dist), r);
		return false;
  }
  ...
  {
    //if (DebugTurret()) DrxLog("Target is not aimable");
		return false;
  }
	return true;
}
...
IEntity *CGunTurret::GetClosestTarget() const
{
	...
	for(i32 i=0; i<query.nCount; i++)
	{
		...
		if (IEntity* pParent = GetEntity()->GetParent())
		{
			...
			i32k nChildren = pParent->GetChildCount();
			for (i32 childIndex = 0; childIndex < nChildren; ++childIndex)
			{
...
void CGunTurret::UpdateGoal(IEntity* pTarget, float deltaTime)
{
  ...
  {
    if (DebugTurret()) DrxLog("UpdateGoal: %s IsTargetAimable failed (yaw: %.2f, pitch: %.2f)", pTarget->GetName(), goalYaw, goalPitch);
		return;
  }
...
{ 
  ...
  assert(m_goalYaw >= 0.f && m_goalYaw <= gf_PI2);
	...
	i32k turretModelSlot = eIGS_ThirdPerson;
...
{
  i32 slots[] = {eIGS_Aux0,eIGS_Aux1,eIGS_ThirdPerson};
  i32 slots_num = 3;
...
void CGunTurret::InternalStartFire(bool sec)
{
  if (g_pGameCVars->i_debug_turrets)
		DrxLog("%s InternalStartFire(%i)", GetEntity()->GetName(), (sec) ? 2 :1);
...
{
  if (g_pGameCVars->i_debug_turrets)
    DrxLog("%s InternalStopFire(%i)", GetEntity()->GetName(), (sec) ? 2 :1);
...
void CGunTurret::ChangeTargetTo(IEntity* pTarget)
{
	m_updateTargetTimer = 0.0f;
  m_rayTimer = 0.f;
	i32 new_id = pTarget?pTarget->GetId():0;
...
//------------------------------------------------------------------------
void CGunTurret::ServerUpdate(SEntityUpdateContext& ctx, i32 update)
{
...
//------------------------------------------------------------------------
void CGunTurret::Update( SEntityUpdateContext& ctx, i32 update)
{  
  ...
  { 
	...
	if(ang>2.0f)
	{
	DrxLog("%X Shooting while not in cone, %.3f",GetEntity()->GetId(),ang);
	}
	float yaw, pitch;
	GetTargetAngles(pos,yaw,pitch);
	if(!IsAiming(pos,5.0f))
	{
	DrxLog("Turret %X firing at %X. Goal %.2f %.2f, target %.2f %.2f, diff %.2f %.2f",GetEntity()->GetId(),m_targetId,m_goalYaw,m_goalPitch,yaw,pitch,fabs_tpl(m_goalYaw-yaw),fabs_tpl(m_goalPitch-pitch));
	}
	}*/
}
...
//------------------------------------------------------------------------
void CGunTurret::SetCharacterAttachmentLocalTM(i32 slot, tukk name, const Matrix34 &tm)
{
...
//------------------------------------------------------------------------
bool CGunTurret::SetAspectProfile( EEntityAspects aspect, u8 profile )
{
...
void CGunTurret::SetFaction(u8k factionID)
{
...
float CGunTurret::GetScaledDamage(float damage, i32 hitType) const
{
	static i32 explosionHitType = g_pGame->GetGameRules()->GetHitTypeId("explosion");
	static i32 fragHitType = g_pGame->GetGameRules()->GetHitTypeId("frag");
