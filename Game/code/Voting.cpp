...
bool  CVotingSystem::StartVoting(EntityId id, const CTimeValue& start, EVotingState st, EntityId eid, tukk subj)
{
  if(st == eVS_none)
    return false;
  if(IsInProgress())
...
{
  for(i32 i=m_votings.size()-1;i>=0;--i)
...
i32   CVotingSystem::GetNumVotesFor()const
{
  return m_votesFor.size();
}

i32   CVotingSystem::GetNumVotesAgainst()const
{
...
{
	DRX_ASSERT_MESSAGE(kickTargetId == GetEntityId(), "The requested vote breakdown and the target vote are for two different entities!");
...
//clients can vote
void  CVotingSystem::Vote(EntityId id, i32 team, bool yes)
{
  if(CanVote(id))
  {
		if(yes)
