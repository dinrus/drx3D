...
#include <drx3D/Game/ProceduralWeaponContext.h>
...
void CProceduralWeaponAnimationContext::Update(float timePassed)
{
	...
	i32k PWALayer = GetGameConstCVar(g_pwaLayer);
...
void CProceduralWeaponAnimationContext::Initialize(IScope* pScope)
{
	...
	if (pCharacter)
	{
		DrxCreateClassInstance("AnimationPoseModifier_OperatorQueue", m_pPoseModifier);
...
{
	--m_instanceCount;
	DRX_ASSERT(m_instanceCount >= 0);
}


DRXREGISTER_CLASS(CProceduralWeaponAnimationContext);
