...
void CShotDebug::Update( const float dt )
{
	...
	i32k projCount = m_projectiles.size();

	IEntity* pOwner = gEnv->pEntitySystem->GetEntity(m_ownerId);
	IEntity* pWeapon = gEnv->pEntitySystem->GetEntity(m_weaponId);

	DrxWatch("%s:%s [%d projectiles]", pOwner?pOwner->GetName():"Owner", pWeapon?pWeapon->GetName():"Weapon", projCount);
	...
	for(ProjectileList::iterator it = m_projectiles.begin(); it!=m_projectiles.end();)
	{
		...
		{
			i32 alpha = inspecting ? 255 : max(0,min(255,int_round(255*info.lifetime)));
		...
		if(isComplete && inspecting)
		{
			...
			if(pGameRules)
			{
				...
				DrxWatch("Projectile:%s%s%s Dmg[%f]%s%s%s Pos:[%.2f %.2f %.2f]%s",
