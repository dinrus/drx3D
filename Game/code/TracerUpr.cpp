...
-------------------------------------------------------------------------
История:
- 17:1:2006   11:18 : Created by M�rcio Martins
...
//------------------------------------------------------------------------
void CTracer::GetMemoryStatistics(IDrxSizer * s) const
{
	s->Add(*this);
}

//------------------------------------------------------------------------
void CTracer::SetGeometry(tukk name, float scale)
{
	if (IEntity *pEntity=gEnv->pEntitySystem->GetEntity(m_entityId))
	{
...
//------------------------------------------------------------------------
void CTracer::SetEffect(tukk name, float scale)
{
	IParticleEffect *pEffect = gEnv->pParticleUpr->FindEffect(name);
	if (!pEffect)
		return;
	...
	if (!(m_tracerFlags & kTracerFlag_dontTranslate))
  {
    ...
    newPos += dir * min(speed*frameTime, dist);
	  pos = newPos;
		...
		if(m_slideFrac > 0.f)
		{
			pos += drx_random(-0.5f, 1.5f) * m_slideFrac * speed * frameTime * dir;
		}
  }
	...
	if (IEntity *pEntity = gEnv->pEntitySystem->GetEntity(m_entityId))
	{
		...
		if (!(m_tracerFlags & kTracerFlag_dontTranslate) && tracerHalfLength > 0.f)
		{
			...
			if (dir.dot(dirFromBackOfTracerToStart) > 0)
			{
				...
				{
					pos = m_startingPos + (dir * drx_random(0.0f, 1.0f) * tracerHalfLength);
				}
...
//------------------------------------------------------------------------
i32 CTracerUpr::EmitTracer(const STracerParams &params, const EntityId bulletId)
{
	if(!gEnv->IsClient())
		return -1;
	...
	else
	{
		...
		for(i32 i = 0; i < kMaxNumTracers; i++)
		{
			DrxPrefetch(&m_tracerPool[i+4]);
...
//------------------------------------------------------------------------
void CTracerUpr::OnBoundProjectileDestroyed( i32k tracerIdx, const EntityId projectileId, const Vec3& newEndTracerPosition )
{
...
//------------------------------------------------------------------------
void CTracerUpr::Update(float frameTime)
{
	DrxPrefetch(m_tracerPool);
	...
	i32k kNumActiveTracers = m_numActiveTracers;

	for(i32 i = 0; i < kNumActiveTracers;)
	{
		i32 nextTracerIndex = i + 1;
		CTracer& tracer = m_tracerPool[i];
		DrxPrefetch(&m_tracerPool[nextTracerIndex]);
	...
	i32 numActiveTracers = kNumActiveTracers;
	
	CWeaponSystem* pWeaponSystem = g_pGame->GetWeaponSystem();

	for(i32 i = kNumActiveTracers - 1; i >= 0;)
	{
		i32 nextIndex = i - 1;
		CTracer& tracer = m_tracerPool[i];
		DrxPrefetch(&m_tracerPool[nextIndex]);
		...
		if(!(tracer.m_tracerFlags & kTracerFlag_active))
		{
			...
			i32k lastTracer = numActiveTracers - 1;
			static CTracer temp;
			temp = tracer;			
...
//------------------------------------------------------------------------
void CTracerUpr::ClearCurrentActiveTracers()
{
	i32k kNumActiveTracers = m_numActiveTracers;
	for(i32 i = 0; i < kNumActiveTracers; i++)
	{
		CTracer& tracer = m_tracerPool[i];
		DrxPrefetch(&m_tracerPool[i+4]);
...
void CTracerUpr::GetMemoryStatistics(IDrxSizer * s)
{
	SIZER_SUBCOMPONENT_NAME(s, "TracerUpr");
	//s->Add(*this);
	//s->AddObject(m_tracerPool);

	for (i16 i=0; i<m_numActiveTracers; i++)
