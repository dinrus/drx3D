...
-------------------------------------------------------------------------
История:
- 18:10:2005   18:00 : Created by M�rcio Martins
...
#include <drx3D/Game/ItemParams.h>
#include <drx3D/Game/WeaponSystem.h>
#include <drx3D/Game/GameRules.h>
#include <drx3D/Game/WeaponSharedParams.h>
#include <drx3D/Game/ProjectileAutoAimHelper.h>
#include <drx3D/Game/Projectile.h>
#include <drx3D/Game/Bullet.h>
#include <drx3D/Game/Rocket.h>
#include <drx3D/Game/HomingMissile.h>
#include <drx3D/Game/C4Projectile.h>
...
#include <drx3D/Game/Burst.h>
#include <drx3D/Game/Rapid.h>
#include <drx3D/Game/Throw.h>
#include <drx3D/Game/Plant.h>
#include <drx3D/Game/Detonate.h>
#include <drx3D/Game/Charge.h>
...
#include <drx3D/Game/Spammer.h>
#include <drx3D/Game/HommingSwarmProjectile.h>
#include <drx3D/Game/KVoltBullet.h>
...
public:
	...
	ILINE static void FreeMemoryPool()
	{
		...
		DRX_ASSERT_MESSAGE(memoryUsed == 0, "Freeing memory pool, but there is still objects using it!");
		if (memoryUsed != 0)
		{
			DrxLog("WeaponComponentAllocator: Memory pool for '%s' will be freed but is still in use! Current used size '%" PRISIZE_T "'", TImplementation::GetStaticType()->GetName(), memoryUsed);
		}
...
void CWeaponSystem::LoadItemParams(IItemSystem* pItemSystem)
{
	...
	i32 numItems = pItemSystem->GetItemParamsCount();
	...
	for(i32 i = 0; i < numItems; i++)
	{
		...
		tukk pItemName = pItemSystem->GetItemParamName(i);
		tukk itemFile = pItemSystem->GetItemParamsDescriptionFile(pItemName);
		...
		DRX_ASSERT_MESSAGE(pItemShared, "Failed to create item params");
		DRX_ASSERT_MESSAGE(itemRootNodeParams != NULL, "No xml found for item params");
		...
		tukk inheritItem = itemRootNodeParams->getAttr("inherit");
		bool isInherited = (inheritItem && inheritItem[0] != 0);
		if (isInherited)
		{
			tukk baseItemFile = pItemSystem->GetItemParamsDescriptionFile(inheritItem);
			overrideParamsNode = itemRootNodeParams;
			itemRootNodeParams = gEnv->pSystem->LoadXmlFromFile(baseItemFile);
			DRX_ASSERT_MESSAGE(itemRootNodeParams != NULL, "No xml found for base item params");
		...
		if (paramsRead)
		{
			...
			if(isWeapon)
			{
				...
				DRX_ASSERT_MESSAGE(pWeaponShared, "Failed to create weapon params");
...
void CWeaponSystem::OnLoadingStart(ILevelInfo *pLevel)
{
	...
	DRX_ASSERT(m_linkedProjectiles.size() == 0);
...
void CWeaponSystem::DestroyFireModePlugin(IFireModePlugin* pObject)
{
	...
	{
		DRX_ASSERT_MESSAGE(false, "Destroy fire mode plugin couldn't be executed, fire mode plugin type destructor method not found, leaking memory!");
		DrxLogAlways("Destroy fire mode plugin couldn't be executed for fire mode plugin type '%s', leaking memory!", pObject->GetRunTimeType()->GetName());
	}
}

//------------------------------------------------------------------------
CFireMode *CWeaponSystem::CreateFireMode(tukk name)
{
...
void CWeaponSystem::RegisterFireMode(tukk componentName)
{
...
void CWeaponSystem::DestroyFireMode(CFireMode* pObject)
{
	...
	{
		DRX_ASSERT_MESSAGE(false, "Destroy fire mode couldn't be executed, fire mode type destructor method not found, leaking memory!");
		DrxLogAlways("Destroy fire mode couldn't be executed for fire mode type '%s', leaking memory!", pObject->GetRunTimeType()->GetName());
	}
}
...
//------------------------------------------------------------------------
IZoomMode *CWeaponSystem::CreateZoomMode(tukk name)
{
...
void CWeaponSystem::DestroyZoomMode(CIronSight* pObject)
{
	...
	{
		DRX_ASSERT_MESSAGE(false, "Destroy zoom mode couldn't be executed, zoom mode type destructor method not found, leaking memory!");
		DrxLogAlways("Destroy zoom mode couldn't be executed for zoom mode type '%s', leaking memory!", pObject->GetRunTimeType()->GetName());
	}
}

//------------------------------------------------------------------------
template<typename ZoomType>
void CWeaponSystem::RegisterZoomMode(tukk componentName)
{
...
//-------------------------------------------	-----------------------------
void CWeaponSystem::RegisterProjectile(tukk name, IGameObjectExtensionCreatorBase *pCreator)
{
...
//-----------------------------------------------------------------------
void CWeaponSystem::AddLinkedProjectile(EntityId projId, EntityId weaponId, u8 shotId)
{
...
//------------------------------------------------------------------------
i32  CWeaponSystem::QueryProjectiles(SProjectileQuery& q)
{
    ...
    q.nCount = i32(m_queryResults.size());
    if(q.nCount)
...
//------------------------------------------------------------------------
void CWeaponSystem::Scan(tukk folderName)
{
	...
	if (!m_recursing)
		DrxComment("Loading ammo XML definitions from '%s'!", folderName);
	...
	if (!m_recursing)
		DrxLog("Finished loading ammo XML definitions from '%s'!", folderName);
...
//------------------------------------------------------------------------
bool CWeaponSystem::ScanXML(XmlNodeRef &root, tukk xmlFile)
{
	...
	//classDesc.pUserProxyData = (uk )it->second;
...
void CWeaponSystem::DebugGun(IConsoleCmdArgs *args)
{
  ...
  if (0 == itemId)        
  {
	...
	{
		DRX_ASSERT_MESSAGE(0, "DebugGun is not defined.");
		return;
	}
...
void CWeaponSystem::RefGun(IConsoleCmdArgs *args)
{
	...
	if (0 == itemId)        
	{
		...
		{
			DRX_ASSERT_MESSAGE(0, "RefWeapon is not defined.");
			return;
		}
...
//------------------------------------------------------------------------
u16 CWeaponSystem::GetPoolSize(IEntityClass *pClass)
{
	TAmmoPoolMap::iterator it=m_pools.find(pClass);
...
{
	DrxLog("Ammo Pool Statistics:");
	for (TAmmoPoolMap::iterator it=m_pools.begin(); it!=m_pools.end(); ++it)
	{
		i32 size=it->second.size;
		DrxLog("%s: %d", it->first->GetName(), size);
	}
}
...
void CWeaponSystem::GetMemoryStatistics(IDrxSizer * s)
{
	SIZER_SUBCOMPONENT_NAME(s, "WeaponSystem");
	s->AddObject(this,sizeof(*this));

	m_tracerUpr.GetMemoryStatistics(s);
...
void CWeaponSystem::SAmmoPoolDesc::GetMemoryUsage( IDrxSizer *pSizer ) const
{
...
//===================================================================================
void CWeaponSystem::OnProjectilePhysicsPostStep(CProjectile* pProjectile, EventPhysPostStep* pEvent, i32 bPhysicsThread)
{
