	...
	size_t		inTimelineId) const
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME,VALIDATOR_ERROR,"Statistics event %" PRISIZE_T " is larger than the max registered of %" PRISIZE_T ", event ignored",inTimelineId,m_numTimelines);
	}
	...
	size_t		inStateId) const
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME,VALIDATOR_ERROR,"Statistics state %" PRISIZE_T " is larger than the max registered of %" PRISIZE_T ", state ignored",inStateId,m_numStates);
	}
...
tukk CCircularBufferStatsContainer::GetEventName(
	size_t		inEventId)
{
...
void CCircularBufferStatsContainer::Validate() const
{
	for (size_t i=0; i<m_numTimelines; i++)
	{
		...
		if (pTL->m_type == eRBPT_Invalid)
		{
			...
			{
				DrxFatalError("Memory stomp, stats container %p timeline %" PRISIZE_T "has had nothing added to it, yet its head and tail have been altered (head %p , tail %p)", this, i, pHead, pTail);
			}
		}
		else
		{
			...
			{
				DrxFatalError("Memory stomp, stats container %p timeline %" PRISIZE_T "has its head or tail out of the accepted range (head %p , tail %p)", this, i, pHead, pTail);
			}
		}
	}
}
...
void CCircularBufferStatsContainer::AddEvent(size_t eventID, const CTimeValue& time, const SStatAnyValue& val)
{
	...
	else
	{
		...
		if (val.type==eSAT_TXML && m_storage->IsUsingCircularBuffer())		// if trying to add a serialisable xml class that's not been allocated from the circular buffer, emit a warning
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_GAME,VALIDATOR_ERROR,"Statistics event %s needs moving to work with the circular buffer storage, this event IS being recorded but is using memory suboptimally",GetEventName(eventID));
			...
			{
				DrxWarning(VALIDATOR_MODULE_GAME,VALIDATOR_ERROR,"Statistics event %s needs moving to work with the circular buffer storage, this event is being ignored",GetEventName(eventID));
			}
			record=false;
#endif
		}
...
void CCircularBufferStatsContainer::GetEventInfo(size_t eventID, size_t idx, CTimeValue& outTime, SStatAnyValue& outParam) const
{
	...
	for (CDoubleLinkedList::const_iterator iter=timeline->m_list.begin(); iter!=timeline->m_list.end(); ++iter)
	{
		if (i==idx)
		{
			switch (timeline->GetType())
			{
				...
				default:
					DRX_ASSERT_MESSAGE(0,"Found unrecognised timeline time in stats circular buffer - cannot return stats");
					break;
			}
			found=true;
			break;
	...
	DRX_ASSERT_MESSAGE(found==true, string().Format("Failed to find event info index %d for event %u, max events on timeline is %d",idx,eventID,maxEvents));
}
	...
	for (size_t i=0; i<m_numTimelines; i++)
	{
		...
		while (!timeline->m_list.empty())
		{
			switch (timeline->GetType())
			{
				...
				default:
					DRX_ASSERT_MESSAGE(0,"unknown data type found in timeline, cannot cleanly destruct it - possible memory leak");
					break;
			}
		}
	}
...
void CCircularBufferStatsContainer::GetMemoryStatistics(IDrxSizer *pSizer)
{
	pSizer->Add(*this);
	...
	m_serializeLocked(false)
{
	...
	DRX_ASSERT_MESSAGE(!s_storage,"Creating multiple CCircularBufferStatsStorage instances isn't currently supported, would need to remove default 'new' operator for CCircularBufferTimelineEntry and force the specification of which CCircularBufferStatsStorage should be used");
	s_storage=this;
}
...
{
	DRX_ASSERT_MESSAGE(!m_serializeLocked,"Attempted to lock stats storage for serialization, but it is already locked??");
...
void CCircularBufferStatsStorage::DebugUpdate()
{
	if (g_pGameCVars->g_telemetry_memory_display)
	{
		...
		{
			DrxWatch("[MT] Stats circular buffer, using %d / %d bytes, # discards %d, # legacy events %d, # new events %d",m_circularBuffer.size(),m_circularBuffer.capacity(),m_numDiscards,m_numLegacyEvents,m_numCircularEvents);
			DrxWatch("[MT] Stats circular buffer, total alloc requests %d bytes, peak %d bytes",m_totalBytesRequested,m_peakAlloc);
		}
		else
		{
			DrxWatch("[MT] Stats circular buffer set to UNLIMITED, current alloc %d, peak %d",m_totalBytesRequested,m_peakAlloc);
		}
		if (IsLockedForSerialization())
		{
			DrxWatch("[MT] Stats circular buffer is locked for serialization");
		}
	}
}
#endif
...
{
	DRX_ASSERT_MESSAGE(s_storage!=NULL,"Tried to access default circular buffer storage, but none exists");
	return s_storage;
}
...
uk CCircularBufferStatsStorage::Alloc(i32 inSize, u8 inType)
{
	DRX_ASSERT_MESSAGE(!m_serializeLocked,"Shouldn't be trying to allocate currently, circular storage is locked for serialisation (may corrupt data)");

	DRX_ASSERT_MESSAGE(this==GetDefaultStorage(),"CCircularBufferStatsStorage Alloc and Free need to know which circular buffer allocations belong to if allowing more than one circular buffer");

	void		*result=NULL;
	i32			headerSize=sizeof(SRecording_Packet);
	i32			totalSize=inSize+headerSize;
...
// static
// frees storage
void CCircularBufferStatsStorage::Free(uk inPtr)
{
	CCircularBufferStatsStorage		*storage=GetDefaultStorage();

	if (storage)
	{
		DRX_ASSERT_MESSAGE(!storage->m_serializeLocked,"Shouldn't be trying to free currently, circular storage is locked for serialisation (may corrupt data)");

		i32						headerSize=sizeof(SRecording_Packet);
		SRecording_Packet		*header=(SRecording_Packet*)(((tuk)inPtr)-headerSize);								// PTR ARITH

		header->type=eRBPT_Free;
		storage->m_totalBytesAlloced-=header->size;
		DRX_ASSERT_MESSAGE(storage->m_totalBytesAlloced>=0,"CCircularBufferStatsStorage memory stats aren't adding up, total used is negative");
		...
		if (storage->IsUsingCircularBuffer())
		{
			if (storage->m_totalBytesAlloced<=0)
			{
				...
				for (CRecordingBuffer::iterator iter=storage->m_circularBuffer.begin(), end=storage->m_circularBuffer.end(); iter!=end; ++iter)
				{
					...
					DRX_ASSERT_TRACE(packet.type==eRBPT_Free,("CCircularBufferStatsStorage thinks there should be no allocations in the circular buffer, but one of type %d remains",packet.type));
				}
#endif
...
// discard callback
void CCircularBufferStatsStorage::DiscardCallback(SRecording_Packet *ps, float recordedTime, uk inUserData)
{
	DRX_ASSERT_MESSAGE(false, "This should never happen, we no longer add data to the circular buffer if it is full");
	switch (ps->type)
	{
		case eRBPT_Free:
			// nothing to do
		...
		case eRBPT_TimelineEntry:
			{
				...
				i32									headerSize=sizeof(SRecording_Packet);
				CCircularBufferTimelineEntry		*entry=(CCircularBufferTimelineEntry*)(((tuk)ps)+headerSize);		// PTR ARITH
		...
		case eRBPT_StatAnyValue:
			{
				i32									headerSize=sizeof(SRecording_Packet);
				SCircularStatAnyValue				*entry=(SCircularStatAnyValue*)(((tuk)ps)+headerSize);		// PTR ARITH
		...
		default:
			DRX_ASSERT_MESSAGE(0,"Unknown packet type in CCircularBufferStatsStorage discard callback");
			break;
	}
}
	...
	const CDoubleLinkedElement		*inElement)
{
	...
	CCircularBufferTimelineEntry	*result=(CCircularBufferTimelineEntry*)(((tuk)inElement)-linkOffset);			// PTR ARITH
	...
	const CDoubleLinkedElement		*inElement)
{
	...
	SCircularStatAnyValue			*result=(SCircularStatAnyValue*)(((tuk)inElement)-linkOffset);			// PTR ARITH
...
{
	i32			expectedRefcount=(m_timelineLink.IsInList()) ? 1 : 0;

	DRX_ASSERT_MESSAGE(GetRefCount()==expectedRefcount,"Circular buffer stats entry is being purged due to lack of space, but some one some where is still holding a ref to it - this is very likely to crash! Code is not permitted to hold references to these objects");
	...
	i32					inDelta)
{
	...
	m_indentLevel=clamp_tpl<i32>(m_indentLevel,0,DRX_ARRAY_COUNT(m_indentStr));
}
...
{
	bool				result=true;
	i32					dataWritten=pIOState->dataWritten;
	i32					indentLevel=inDoIndent ? m_indentLevel : 0;
	...
	IGameStatistics													*pInStats)
{
	...
	u32 bufferSize=64*1024;
	tuk buffer=(tuk)malloc(bufferSize);
	...
	while (m_eventIterator!=end && !pIOState->full)
	{
		...
		switch (pTimeline->GetType())
		{
			...
			case eRBPT_StatAnyValue:
				{
					...
					DRX_ASSERT_MESSAGE(pVal->val.type!=eSAT_TXML,"adding SStatAnyValue() with xmlnodes is no longer supported, add a data type derived from IXMLSerializable instead");
					stack_string strValue;
			...
			default:
				DRX_ASSERT_MESSAGE(0,"unknown data type found in timeline, cannot save data into telemetry");
				break;
		}
	...
	IGameStatistics													*pInStats)
{
	...
	while (m_timelineIterator<=i32(numEventTimelines) && !pIOState->full)
	{
		...
		else if (m_timelineIterator==i32(numEventTimelines))
		{
			if (pInCont->HasAnyTimelineEvents())
			{
				IncreaseIndentation(-1);
	...
	IGameStatistics													*pInStats)
{
	...
	while (m_stateIterator<i32(numStates) && !pIOState->full)
	{
	...
	i32					*pOutWritten)
{
	...
	if (pStat)
	{
		...
		i32												containerSize=m_entries.size();
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME,VALIDATOR_ERROR,"Failed to serialise data - IGameStatistics interface removed during serialisation");		// possibly could happen during game tear down
	}
...
void CCircularXMLSerializer::VisitNode(const SNodeLocator& locator, tukk serializeName, IStatsContainer& container, EStatNodeState state)
{
	switch (state)
	{
		case eSNS_Alive:
			DrxWarning(VALIDATOR_MODULE_GAME,VALIDATOR_ERROR,"Live stats node encountered during stats serialization, ignoring %s\n",serializeName);
			break;
...
void CCircularXMLSerializer::LeaveNode(const SNodeLocator& locator, tukk serializeName, IStatsContainer& container, EStatNodeState state)
{
	switch (state)
	{
		case eSNS_Alive:
