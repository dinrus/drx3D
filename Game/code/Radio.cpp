...
#include <drx3D/Game/Radio.h>
#include <drx3D/Game/GameRules.h>
#include <drx3D/Game/Game.h>
#include <drx3D/Sys/ILocalizationUpr.h>
#include <drx3D/Game/Player.h>

static i32k RADIO_GROUPS=4;
static i32k RADIO_GROUP_SIZE=5;
i32k CRadio::RADIO_MESSAGE_NUM=RADIO_GROUPS*RADIO_GROUP_SIZE;//sizeof(RadioMessages)/sizeof(RadioMessage_s);
static i32k RADIO_AUTOCLOSE_TIME = 5;
...
static void PlaySound(tukk name)
{
...
static void PlayVoice(tukk name)
{
...
static bool GetRadioSoundName(CGameRules *gr,const string &teamName,i32k groupId,i32k keyId,char **ppSoundName=0,char **ppSoundText=0, i32* pVariations = 0)
{
...
bool CRadio::OnAction(const ActionId& actionId, i32 activationMode, float value)
{
	if(!gEnv->bMultiplayer)
		return false;

	//DrxLogAlways("RADIO: Action[%3.2f] %s", gEnv->pTimer->GetCurrTime(), m_ignoreMenuThisFrame ? "ignored" : "");
...
bool CRadio::OnInputEvent( const SInputEvent &event )
{
	...
	//DrxLogAlways("RADIO: Input[%3.2f] %s %d", gEnv->pTimer->GetCurrTime(), event.keyName.c_str(), event.state);

	// Signal that input events have been processed and sent to all listeners.
	m_waitForInputEvents = false;

	tukk sKey = event.keyName.c_str();
	...
	i32 id=(m_currentGroup*RADIO_GROUP_SIZE+iKey)-1;
...
void CRadio::OnRadioMessage(i32 id, EntityId fromId)
{
	i32 groupId=id/RADIO_GROUP_SIZE;
	i32 keyId=id%RADIO_GROUP_SIZE;
	...
	{		
/*
		ILocalizationUpr *pLocalizationMan = gEnv->pSystem->GetLocalizationUpr();
		...
		if(variations > 1)
		{
			i32 r = drx_random(0, variations - 1);
...
void CRadio::GetMemoryStatistics(IDrxSizer * s)
{
	s->Add(*this);
	s->Add(m_TeamName);
}
...
/*
	DrxLogAlways("RADIO: Update[%3.2f] state=[%d %d %d %d] ignore=[%d %d %d %d]", gEnv->pTimer->GetCurrTime(), 
