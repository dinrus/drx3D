...
void CKVoltBullet::HandleEvent(const SGameObjectEvent& event)
{
	if(event.event == eGFE_OnCollision)
	{
		...
		if(ISurfaceType *pSurfaceType = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceType(pCollision->idmat[trgId]))
		{
		...
		if(!IsRemote())
		{
			...
			if(gEnv->bMultiplayer)
			{
				...
				i32 nOwnerTeam = pGameRules->GetTeam(m_ownerId), nTargetTeam = pGameRules->GetTeam(targetId);
...
void CKVoltBullet::DamageEnemiesInRange(float range, Vec3 pos, EntityId ignoreId)
{
	...
	for (i32 i = 0, size = actorIds.size(); i < size; ++i)
	{
...
void CKVoltBullet::ProcessDamage(EntityId targetId, float damage, i32 hitPartId, Vec3 pos, Vec3 dir)
{
	if(targetId)
	{
		CGameRules *pGameRules = g_pGame->GetGameRules();
