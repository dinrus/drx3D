...
{
	DRX_ASSERT_MESSAGE(m_pControlledPlayer, "Controlled player not initialized");
...
void CMountedGunController::OnEnter(EntityId mountedGunId)
{
	...
	if (pCharacter)
	{
		...
		if (pRecordingSystem)
		{
			...
			i32k upAnimId = pAnimationSet->GetAnimIDByName(pWeaponParams.mountedTPAimAnims[MountedTPAimAnim::Up].c_str());
			i32k downAnimId = pAnimationSet->GetAnimIDByName(pWeaponParams.mountedTPAimAnims[MountedTPAimAnim::Down].c_str());
...
//		DrxWatch("Mount wrot: plr: %f vehicle: %f wpn: %f", pControlledPlayerEntity->GetWorldRotation().GetRotZ(), pParent ? pParent->GetWorldRotation().GetRotZ() : -99.0f, pMountedGun->GetEntity()->GetWorldRotation().GetRotZ());
//		DrxWatch("Mount lrot: plr: %f vehicle: %f wpn: %f", pControlledPlayerEntity->GetRotation().GetRotZ(), pParent ? pParent->GetRotation().GetRotZ() : -99.0f, pMountedGun->GetEntity()->GetRotation().GetRotZ());
	}		
}
...
void CMountedGunController::ReplayStartThirdPersonAnimations( ICharacterInstance* pCharacter, i32 upAnimId, i32 downAnimId )
{
	assert(pCharacter);
...
void CMountedGunController::ReplayUpdateThirdPersonAnimations( ICharacterInstance* pCharacter, float aimRad, float aimUp, float aimDown, bool firstPerson /*= false*/)
{
	if (pCharacter)
	{
		...
		u32 numAnimsLayer = pSkeletonAnim->GetNumAnimsInFIFO(REPLAY_PLAYER_ANIMATION_LAYER_BASE);
		for(u32 i=0; i<numAnimsLayer; i++)
		{
...
void CMountedGunController::UpdateFirstPersonAnimations( CItem* pMountedGun, const Vec3 &aimDirection )
{
	if (const CWeapon* pMountedWeapon = static_cast<CWeapon*>(pMountedGun->GetIWeapon()))
	{
		...
		i32k slotIndex = 0;
		u32k mountedWeaponSlotFlags = pMountedWeaponEntity->GetSlotFlags(slotIndex);
...
void CMountedGunController::InitMannequinParams()
{
	...
	DRX_ASSERT(m_pControlledPlayer);
	IAnimatedCharacter* pAnimatedCharacter = m_pControlledPlayer->GetAnimatedCharacter();
	DRX_ASSERT(pAnimatedCharacter);
