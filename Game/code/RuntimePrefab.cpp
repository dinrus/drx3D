...
void CRuntimePrefab::SpawnBrushes(CPrefab &pRef, const Matrix34 &matSource,AABB &box)
{
	...
	{		
		...
		if (!pParam.sMaterial.empty())		
			pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(pParam.sMaterial);		
		...
		else
		{
			...
			if (!pParam.sFilename.empty())
				...
				pObj=gEnv->p3DEngine->LoadDesignerObject(pParam.nBinaryDesignerObjectVersion,&pParam.vBinaryDesignerObject[0], (i32)pParam.vBinaryDesignerObject.size());
