...
DRX_IMPLEMENT_GTI(CThrow, CSingle);
...
//------------------------------------------------------------------------
void CThrow::Update(float frameTime, u32 frameId)
{
	...
	if(gEnv->bServer && pActor && pActor->IsPlayer() && m_primed && IsReadyToThrow() && m_fireParams->throwparams.prime_timer && (m_projectileLifeTime > 0.0f))
	{
		...
		if(primedTime >= m_projectileLifeTime)
		{
			...
			if (m_fireParams->fireparams.clip_size != -1)
			{
				...
				i32k weaponAmmoCount = m_pWeapon->GetAmmoCount(pAmmoClass);
				i32k inventoryAmmoCount = m_pWeapon->GetInventoryAmmoCount(pAmmoClass);
...
void CThrow::StartFireInternal()
{
	...
	if (m_fireParams->throwparams.prime_enabled)
	{
		...
		m_pWeapon->GetScheduler()->TimerAction((u32)(m_fireParams->throwparams.prime_delay * 1000.0f), CSchedulerAction<PrimeAction>::Create(this), false);
	...
	if (m_pWeapon->GetOwner() && clipSize >= 0)
	{
		...
		if (clipSize > 0)
		{
			...
			if (gEnv->bServer)
			{
				...
				{
					i32 refillAmmo = min(clipSize, inventoryAmmoCount);
...
void CThrow::DoThrow()
{
	...
	m_pWeapon->GetScheduler()->TimerAction((u32)(m_fireParams->throwparams.throw_delay * 1000.0f), CSchedulerAction<ThrowAction>::Create(this), false);
...
void CThrow::ShootInternal(Vec3 hit, Vec3 pos, Vec3 dir, Vec3 vel, bool clientIsShooter)
{
	...
	if (pAmmo)
	{
		...
		pAmmo->SetParams(CProjectile::SProjectileDesc(
			m_pWeapon->GetOwnerId(), m_pWeapon->GetHostId(), m_pWeapon->GetEntityId(), (i32)m_fireParams->fireparams.damage,
...
//------------------------------------------------------------------------
void CThrow::NetShootEx(const Vec3 &pos, const Vec3 &dir, const Vec3 &vel, const Vec3 &hit, float extra, i32 predictionHandle)
{
...
//-----------------------------------------------------
void CThrow::GetMemoryUsage(IDrxSizer * s) const
{
...
void CThrow::RenderTrajectory(const Vec3* trajectory, u32 sampleCount, const float maxTime)
{
	...
	float gap = 0.0f;
	bool isDash = true;
	i32 currentChunkSize = 0;
	float time = 0.0f;
	float timeStep = g_pGameCVars->i_grenade_trajectory_resolution;

	for(u32 i=0; (i<sampleCount)&&(time<maxTime); i+=1, time+=timeStep)
	{
		float dist = 0.0f;
		if(i>0)
		{
