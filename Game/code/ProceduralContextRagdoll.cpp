...
DRXREGISTER_CLASS( CProceduralContextRagdoll );
...
{
	DRX_ASSERT( !m_targetEntityId || m_targetEntityId==entityID );
...
void CProceduralContextRagdoll::QueueRagdoll( bool bAlive )
{
	...
	// NOTE: The case where piActor is NULL is when you're - in the DrxMann preview!
	if(piActor)
	{
	...
	if( m_bInBlendOut )
	{
		...
		if( IEntity* pEntity = gEnv->pEntitySystem->GetEntity( m_targetEntityId ) )
		{
			if(IPhysicalEntity* pent = pEntity->GetPhysics())
			{
				...
				i32 partCount = pent->GetStatus(&parts);
