...
#include <drx3D/Game/AnimActionAIAiming.h>
#include <drx3D/Game/AnimActionAILookPose.h>
#include <drx3D/Game/AnimActionAILooking.h>
...
void CPlayer::UpdateAIAnimationState(const SActorFrameMovementParams &frameMovementParams, CWeapon* pWeapon, ICharacterInstance* pICharInst, IActionController* pActionController, IMannequin& mannequinSys)
{
	...
	// ---------------------------------------------------------------------
	DRX_ASSERT(m_pAIAnimationComponent.get());
...
// ----------------------------------------------------------------------------
typedef u32 (*CRCRemapFunction)(u32 crc);

// ----------------------------------------------------------------------------
static void UpdateTagFromAGInput(CTagState* pTagState, tukk szInputValue, u32 defaultAGCRC = 0, u32 defaultTagCRC = 0)
{
	if (szInputValue && (szInputValue[0] != '\0'))
	{
		u32 inputValueCRC = CCrc32::ComputeLowercase(szInputValue);
...
// ----------------------------------------------------------------------------
static void UpdateTagFromAGInput(CTagState* pTagState, tukk szInputValue, CRCRemapFunction remapFunction)
{
	if (szInputValue && (szInputValue[0] != '\0'))
	{
		u32 inputValueCRC = CCrc32::ComputeLowercase(szInputValue);
...
// ----------------------------------------------------------------------------
static void UpdateTagFromAGInput(CTagState* pTagState, const IAnimationGraphState& AGState, IAnimationGraphState::InputID inputID, u32 defaultAGCRC = 0, u32 defaultTagCRC = 0)
{
...
namespace
{
	i32 GetStanceAlertness( const EStance stance )
	{
		switch( stance )
		{
		case STANCE_RELAXED:
...
void CAIAnimationComponent::UpdateAnimationStateRequests( CPlayer& player, const SActorFrameMovementParams &frameMovementParams )
{
	...
	case eCoverLocationRequest_Set:
		{
			DRX_ASSERT( coverRequest.coverLocation.IsValid() );

			const float TOO_FAR_AWAY = 50;
			DRX_ASSERT( coverRequest.coverLocation.t.GetSquaredDistance2D( player.GetEntity()->GetPos() ) < sqr( TOO_FAR_AWAY ) );
	...
	{
		tukk const coverActionName = frameMovementParams.coverRequest.coverActionName;
...
void CAIAnimationComponent::UpdateStanceAndCover( CPlayer& player, IActionController* pActionController )
{
	...
	// Set CoverLocation into TargetPos when needed
	{
		...
		if ( isCoverRequested )
		{
			...
			if ( pCoverLocation )
			{
				DRX_ASSERT( pCoverLocation->IsValid() );
				pActionController->SetParam( "TargetPos", *pCoverLocation );
			}
			else
			{
				DRX_ASSERT_MESSAGE( false, "Cover stance set but no cover location set" );
...
void CAIAnimationComponent::UpdateAimingState( ISkeletonPose* pSkeletonPose, const bool aimEnabled, const Vec3& aimTarget, u32k aimIkLayer, const float aimIkFadeoutTime )
{
...
void CAIAnimationState::Init( SAnimationContext* pContext )
{
	...
	DRX_ASSERT( m_pContext );
	DRX_ASSERT( m_pAiUserParams );
}
...
void CAIAnimationState::SetRequestedCoverAction( const ECoverBodyDirection coverBodyDirection, tukk const actionName )
{
...
void CAIAnimationState::SetRequestedCoverActionName( tukk const actionName )
{
	DRX_ASSERT( actionName );
...
void CAIAnimationState::SetCoverActionName( tukk const actionName )
{
	DRX_ASSERT( actionName );
