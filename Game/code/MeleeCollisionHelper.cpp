...
Описание: Helper class for melee collision detection
...
EntityId CMeleeCollisionHelper::GetBestAutoAimTargetForUser( EntityId userId,  const Vec3& scanPosition, const Vec3& scanDirection, float range, float angle ) const
{
	...
	i32 playerTeam = pGameRules->GetTeam(userId);
	...
	for (i32 i = 0; i < targetCount; ++i)
	{
		...
		DRX_ASSERT(target.entityId != userId);
...
void CMeleeCollisionHelper::Impulse( IPhysicalEntity *pCollider, const Vec3 &position, const Vec3 &impulse, i32 partId, i32 ipart, i32 hitTypeID )
{
	assert(pCollider);
...
void CMeleeCollisionHelper::GenerateArtificialCollision( IEntity* pUser, IPhysicalEntity *pCollider, const Vec3 &position, const Vec3& normal, const Vec3 &speed, i32 partId, i32 ipart, i32 surfaceIdx, i32 iPrim )
{
	ISurfaceTypeUpr *pSurfaceTypeUpr = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr();
	i32 matPlayerColliderId = pSurfaceTypeUpr->GetSurfaceTypeByName("mat_player_collider")->GetId();
...
{
	const SAutoaimTarget* pAutoTarget = g_pGame->GetAutoAimUpr().GetTargetInfo(targetId);
	...
	i32 jointID = pActor->GetBoneID(pAutoTarget->physicsBoneId);
	...
	if (pTargetPhysics->GetParams(&partParams))
	{
		...
		i32 geomSurfaceIdx = pGeom ? pGeom->surface_idx : -1;
