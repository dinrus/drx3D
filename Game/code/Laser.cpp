...
$Id$
$DateTime$
Описание: Laser accessory (re-factored from Crysis L.A.M.)
...
void CLaserBeam::TurnOnLaser()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "LASER PARAMS: Item of type CLaser is missing it's laser params!");
	}
}
...
IAttachmentUpr* CLaserBeam::GetLaserCharacterAttachmentUpr()
{
	...
	if (pOwnerItem)
	{
		...
		{
			return pCharacter->GetIAttachmentUpr();				
		}
	}
		...
		if(m_geometrySlot == eIGS_FirstPerson)
		{
			...
			if(pAttachedEntity)
			{
				...
				if (pCharacter)
				{
					...
					IAttachment *pLaserAttachment = pAttachmentUpr->GetInterfaceByName(LASER_ATTACH_NAME);

					if(!pLaserAttachment)
					{
						IAttachment *pAttachment = pAttachmentUpr->GetInterfaceByName(attach_helper);

						if(pAttachment)
						{
							tukk pBone = pCharacter->GetIDefaultSkeleton().GetJointNameByID(pAttachment->GetJointID());

							pLaserAttachment = pAttachmentUpr->CreateAttachment(LASER_ATTACH_NAME, CA_BONE, pBone);
	...
	if(m_pLaserParams)
	{
		...
		if(pLaserEntity)
		{
			if(freeSlots)
			{
				...
				{
					if(IAttachmentUpr* pAttachmentUpr = GetLaserCharacterAttachmentUpr())
					{
						if(IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByName(LASER_ATTACH_NAME))
						{
			...
			else
			{
				...
				{
					IParticleEffect* pEffect = gEnv->pParticleUpr->FindEffect(m_pLaserParams->laser_dot[GetIndexFromGeometrySlot()].c_str());
					if(pEffect)
...
void CLaserBeam::DestroyLaserEntity()
{
	if(IAttachmentUpr* pAttachmentUpr = GetLaserCharacterAttachmentUpr())
	{
		...
		{
			pAttachment->ClearBinding();
			pAttachmentUpr->RemoveAttachmentByInterface(pAttachment);
		}	
	}
...
void CLaserBeam::UpdateLaser(const CLaserBeam::SLaserUpdateDesc& laserUpdateDesc)
{
	if(m_pLaserParams && m_laserOn)
	{
		...
		if (pLaserEntity)
		{
			...
			m_laserUpdateTimer = drx_random(0.0f, LASER_UPDATE_TIME * 0.4f);
			...
			i32k objects = ent_terrain|ent_static|ent_rigid|ent_sleeping_rigid|ent_independent; //ent_living;
			i32k flags = (geom_colltype_ray << rwi_colltype_bit) | rwi_colltype_any | (10 & rwi_pierceability_mask) | (geom_colltype14 << rwi_colltype_bit);
...
i32 CLaserBeam::GetIndexFromGeometrySlot()
{
...
void CLaser::TurnOnLaser(bool manual /*= false*/)
{
	...
	i32 slot = pParentWeapon->IsOwnerFP() ? eIGS_FirstPerson: eIGS_ThirdPerson;
...
//-------------------------------------------
void CLaser::Update(SEntityUpdateContext& ctx, i32 slot)
{
...
{
	tukk entityLocationHelper = m_laserHelperFP.c_str();
	tukk laserTermHelper = "laser_term";
	const bool relative = false;
	const bool absolute = true;

	i32k slot = pParentWeapon->IsOwnerFP() ? eIGS_FirstPerson : eIGS_ThirdPerson;
...
bool CLaser::GetProbableHit(EntityId weaponId, const IFireMode* pFireMode, Vec3& hit)
{
	...
	CWeapon* pWeapon = GetWeapon();
	i32 slot = pWeapon->IsOwnerFP() ? eIGS_FirstPerson : eIGS_ThirdPerson;
