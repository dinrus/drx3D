...
void CLookAim_Helper::UpdateLook(CPlayer* pPlayer, ICharacterInstance* pCharacter, bool bEnabled, f32 FOV, const Vec3& targetGlobal, u32k lookIKLayer)
{
	if (!m_initialized)
	{
...
void CLookAim_Helper::Init(CPlayer* pPlayer, ICharacterInstance* pCharacter)
{
	...
	i16 lookAtSimpleHeadJoint = pCharacter->GetIDefaultSkeleton().GetJointIDByName(params.lookAtSimpleHeadBoneName);
	...
	if (m_canUseLookAtSimple)
	{
		if (!m_lookAtSimple.get())
			DrxCreateClassInstance<AnimPoseModifier::CLookAtSimple>("AnimationPoseModifier_LookAtSimple", m_lookAtSimple);
...
void CLookAim_Helper::UpdateDynamicAimPoses(CPlayer* pPlayer, ICharacterInstance* pCharacter, const SActorParams& params, const bool aimEnabled, i32k aimIKLayer, const Vec3& vAimTarget, const SMovementState& curMovementState)
{
	if (!m_initialized)
	{
		Init(pPlayer, pCharacter);
	...
	DRX_ASSERT(params.useDynamicAimPoses);
	...
	if (aimEnabled && !m_aimIsSwitchingArms)
	{
		...
		tukk animName = pose.bothArmsAimPose;
		m_availableHandsForFiring = (1<<IItem::eIH_Left) | (1<<IItem::eIH_Right);
		...
		i32k animID = pAnimSet->GetAnimIDByName(animName);
