...
u16 SkillKill::s_flashBangGrenadesClassId = 0;
...
bool SkillKill::IsHeadshot(CPlayer* pTargetPlayer, i32 hit_bodyPart, i32 material)
{
	HitInfo hitInfo;
...
bool SkillKill::IsMeleeTakedown(CGameRules* pGameRules, i32 hit_type)
{
...
{
	i32k multiKillStreak = g_pGame->GetPersistantStats()->GetCurrentStat(pShooterPlayer->GetEntityId(), ESIPS_MultiKillStreak);
...
bool SkillKill::IsPiercing(i32 penetration)
{
	return penetration > 0;
}

bool SkillKill::IsBlinding(CPlayer* pShooterPlayer, CPlayer* pTargetPlayer, u16 weaponClassId)
{
...
bool SkillKill::IsFlushed(CPlayer* pShooterPlayer, CPlayer* pTargetPlayer, CGameRules* pGameRules, i32 hit_type)
{
...
bool SkillKill::IsKickedCarKill(CPlayer* pKicker, CPlayer* pTargetPlayer, EntityId vehicleId)
{
	if(pKicker->IsClient() && g_pGameCVars->g_mpKickableCars)
	{
		...
		else
		{
			...
			DRX_ASSERT(pDestroyedVehicleClass);
