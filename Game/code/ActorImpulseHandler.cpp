...
#endif //_RELEASE
{
	//DrxCreateClassInstance("AnimationPoseModifier_OperatorQueue", m_poseModifier);
...
void CActorImpulseHandler::ReadXmlData(const IItemParamsNode* pRootNode)
{
	...
	// too often, though)
	tukk szEntityClassName = m_actor.GetEntityClassName();
	...
	if (!pParams)
	{
		...
		pSharedParams = pSharedParamsUpr->Get(sharedParamsName);
		if (!pSharedParams)
		{
	...
	if (pParams)
	{
		...
		if (pDeathImpulses)
		{
			...
			{
				DrxFixedStringT<64> massParamName = DrxFixedStringT<64>("mass") + "_" + m_actor.GetEntityClassName();
				if (!physicsParams->GetValue(massParamName.c_str(), newParams.fMass))
					physicsParams->GetValue("mass", newParams.fMass);
			}
		}

		m_pParams = CastSharedParamsPtr<SSharedImpulseHandlerParams>(pSharedParamsUpr->Register(sharedParamsName, newParams));
	}
}
...
void CActorImpulseHandler::AddLocalHitImpulse(const SHitImpulse& hitImpulse)
{
	...
	if (applyImpulse)
	{
		...
		if (bodyDamageProfileId != INVALID_BODYDAMAGEPROFILEID)
		{
			...
			pBodyDamageUpr->GetHitImpulseFilter(bodyDamageProfileId, *m_actor.GetEntity(), tmpHitInfo, impulseFilter);

			partFlags = pBodyDamageUpr->GetPartFlags( bodyDamageProfileId, *m_actor.GetEntity(), tmpHitInfo );
		...
		if (!hitImpulse.m_impulse.IsZero() && g_pGameCVars->pl_impulseEnabled)
		{
			...
			float tiltFactor = drx_random(-1.0f, 1.0f) * g_pGameCVars->pl_impulseMaxTwist;
...
void CActorImpulseHandler::ApplyDeathImpulse(const HitInfo& lastHit)
{
	...
	//DRX_ASSERT_MESSAGE(m_actor.GetEntity()->GetPhysics() && (m_actor.GetEntity()->GetPhysics()->GetType() == PE_ARTICULATED), "This method is intended to work only for ragdolls!");

	//************** Dir calculation
	Vec3 dir = lastHit.dir;

	// Up the direction vector
	dir.z += drx_random(0.1f, 0.7f);
	...
	if (itCandidate != m_pParams->impulseSets.end())
	{
		...
		for (SImpulseSet::ImpulseContainer::const_iterator it = impulseSet.impulses.begin(); it != itEnd; ++it)
		{
			...
			myDirection = myDirection.CompMul(drx_random_componentwise(Vec3(0.8f), Vec3(1.2f)));
			...
			// ToDo: Is this really needed?
			myImpulse *= drx_random(0.8f, 1.2f);
...
void CActorImpulseHandler::Update(float frameTime)
{
	...
	if(g_pGameCVars->pl_impulseDebug)
	{
		...
		for(i32 i=kDebugImpulseMax; i>0; i--)
		{
			SDebugImpulse& imp = m_debugImpulses[(m_currentDebugImpulse+kDebugImpulseMax-i)%kDebugImpulseMax];
			if(imp.valid)
			{
				DrxWatch( "Impulse: Age[%.2fs] Applied[%s] Impulse[%.2f,%.2f,%.2f]", time-imp.time, imp.applied?"True":"False", imp.impulse.impulse.x, imp.impulse.impulse.y, imp.impulse.impulse.z );
			}
		}
	}
#endif //_RELEASE
...
void CActorImpulseHandler::ApplyDeathAngularImpulse(float fAngularImpulseScale, const Vec3& hitpos, const Vec3& playerPos, const HitInfo& lastHit)
{
	...
	float impulse = std::min(650.0f, std::max(10.0f, lastHit.damage) * drx_random(6.0f, 9.0f));
...
void CActorImpulseHandler::LoadDeathImpulses(const IItemParamsNode* pDeathImpulses, ImpulseSetsContainer& impulseSets)
{
	...
	i32k iChildCount = pDeathImpulses->GetChildCount();
	for (i32 i = 0; i < iChildCount; ++i)
	{
		...
		if (strcmp(pChild->GetName(), "ImpulseSet") == 0)
		{
			...
			// Impulses
			i32k iImpulseCount = pImpulseSet->GetChildCount();
			for (i32 iImpulse = 0; iImpulse < iImpulseCount; ++iImpulse)
			{
				...
				if (pImpulse && (strcmp(pImpulse->GetName(), "Impulse") == 0))
				{
					...
					{
						tukk szPartID = pImpulse->GetAttribute("partID");
						if (szPartID && szPartID[0])
						{
							i32k iPartId = rIDefaultSkeleton.GetJointIDByName(szPartID);
							if (iPartId != -1)
							{
			...
			{
				i32 iCurPos = 0;
				DrxFixedStringT<64> sHitTypesTokenized = pImpulseSet->GetAttribute("hitTypes");
				DrxFixedStringT<64> sHitType = sHitTypesTokenized.Tokenize(",", iCurPos);
				while (!sHitType.empty())
				{
					i32k iHitType = g_pGame->GetGameRules()->GetHitTypeId(sHitType.c_str());
					if (iHitType > 0)
					{
			...
			{
				i32 iCurPos = 0;
				DrxFixedStringT<64> sPartIDsTokenized = pImpulseSet->GetAttribute("partIDs");
				DrxFixedStringT<64> sPartID = sPartIDsTokenized.Tokenize(",", iCurPos);
				while (!sPartID.empty())
				{
					i32k iPartID = rIDefaultSkeleton.GetJointIDByName(sPartID.c_str());
					if (iPartID != -1)
					{
			...
			// bulletTypes
			{
				...
				{
					i32 iCurPos = 0;
					DrxFixedStringT<64> sBulletTypesTokenized = pImpulseSet->GetAttribute("bulletTypes");
					DrxFixedStringT<64> sBulletType = sBulletTypesTokenized.Tokenize(",", iCurPos);
		...
		else if (strcmp(pChild->GetName(), "CopyImpulsesFrom") == 0)
		{
			...
			tukk szCopyFrom = pChild->GetAttribute("class");
		...
		{
			DRX_ASSERT_TRACE(false, ("Unexpected element: %s", pChild->GetName()));
		}
	}
