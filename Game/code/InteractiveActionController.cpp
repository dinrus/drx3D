...
$Id$
$DateTime$
Описание: Controls player when playing an interactive animation
...
#include <drx3D/Game/InteractiveActionController.h>

#include <drx3D/Game/Item.h>
#include <drx3D/Game/Player.h>

#include <drx3D/Game/Environment/InteractiveObjectRegistry.h>
...
public:
	...
	CActionInteractive(CPlayer &player, EntityId interactiveObjectId, FragmentID fragID, const TagState &fragTags, u32 optionIdx, CInteractiveActionController &IAController, const QuatT &targetLocation, const TagID tagId)
	...
	virtual void Install() override
	{
		...
		if (m_target)
		{
			...
			if (targetChar)
			{
				...
				const IAnimationDatabase *animDB = mannequinSys.GetAnimationDatabaseUpr().Load("Animations/Mannequin/ADB/interactiveObjects.adb");
...
void CInteractiveActionController::OnEnter( CPlayer& player, EntityId interactiveObjectId, i32 interactionIndex, float actionSpeed /*= 1.0f*/ )
{
	m_interactiveObjectId = interactiveObjectId;
...
void CInteractiveActionController::OnEnterByName( CPlayer& player, tukk interaction, float actionSpeed /*= 1.0f*/ )
{
...
{
	DRX_ASSERT_MESSAGE(m_state != eState_None, "Non valid state!");
...
void CInteractiveActionController::CalculateStartTargetLocation(CPlayer& player, i32 interactionIndex)
{
...
{
	DRX_ASSERT(IsInInteractiveAction());
