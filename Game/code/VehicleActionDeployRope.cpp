...
$Id$
$DateTime$
Описание: Implements a vehicle action to deploy a rope
...
//------------------------------------------------------------------------
void CVehicleActionDeployRope::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
...
{
	DRX_ASSERT(gEnv->pGame->GetIGameFramework()->GetIActorSystem());
...
/*
	...
	if(pCharacterInstance)
	{
		...
		if(pSkeletonPose)
		{
			i16	jointId = pSkeletonPose->GetJointIDByName("Bip01 L Hand");
...
//------------------------------------------------------------------------
void CVehicleActionDeployRope::GetMemoryUsage(IDrxSizer *pSizer) const
{
	pSizer->Add(*this);
}
...
EntityId CVehicleActionDeployRope::CreateRope(IPhysicalEntity *pLinkedEntity, const Vec3 &highPos, const Vec3 &lowPos)
{
	...
	drx_sprintf(ropeName, "%s_rope_%d", m_pVehicle->GetEntity()->GetName(), m_pSeat->GetSeatId());
