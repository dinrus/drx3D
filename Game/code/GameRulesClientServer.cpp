...
static i32k sSimulateExplosionMaxEntitiesToSkip = 20;
...
//DrxLogAlways ("ClientHit: %s damaged %s", pShooter ? pShooter->GetName() : "NULL", pTarget ? pTarget->GetName() : "NULL");
//DrxWatch3DAdd(GetHitType(hitInfo.type), hitInfo.pos, 2.f, NULL, -1.f);
			...
			else
			{
				if(pVehicle)
				{
					if( IActor* pDriver = pVehicle->GetDriver() )
					{
						if(GetTeamCount() > 1)
						{
							i32 shooterTeamId = GetTeam( hitInfo.shooterId );
							i32 targetTeamId = GetTeam( pDriver->GetEntityId() );
					...
					else
					{
						...
						DRX_ASSERT(pVTOLVehicleUpr);
						if (pVTOLVehicleUpr && pVTOLVehicleUpr->IsVTOL(pVehicle->GetEntityId()))
						{
							targetType = EGRTT_NeutralVehicle;
						}
					}
	...
	if (displayHudFeedback)
	{
		if (!gEnv->bMultiplayer)
		{
			...
			{
				SHUDEventWrapper::HitTarget( targetType, static_cast<i32>(eHUDEventHT_Bullet), hitInfo.targetId );
			}
		}
		else
		{
...
void CGameRules::ProcessServerHit(const HitInfo& hitInfo)
{
	...
	if (!gEnv->bMultiplayer)
	{
		...
		{
			const SAutoaimTarget* pTargetInfo = g_pGame->GetAutoAimUpr().GetTargetInfo(hitInfo.targetId);
	...
	if (bHandleRequest)
	{
		...
		DRX_ASSERT_MESSAGE(pDamageHandler, "No Damage handling module found!");
		if (pDamageHandler)
		{
		...
		if (pShooter)
		{
			if (hitInfo.shooterId!=hitInfo.targetId && hitInfo.weaponId!=hitInfo.shooterId && hitInfo.weaponId!=hitInfo.targetId && hitInfo.damage>=0)
			{
				...
				m_pGameplayRecorder->Event(pShooter->GetEntity(), GameplayEvent(eGE_WeaponHit, 0, 0, (uk )params));
			}

			m_pGameplayRecorder->Event(pShooter->GetEntity(), GameplayEvent(eGE_Hit, 0, 0, (uk )(EXPAND_PTR)hitInfo.weaponId));
			m_pGameplayRecorder->Event(pShooter->GetEntity(), GameplayEvent(eGE_Damage, 0, hitInfo.damage, (uk )(EXPAND_PTR)hitInfo.weaponId));
		}
...
void CGameRules::AddLocalHitImpulse(const HitInfo& hitInfo)
{
	...
	if (isValidEntity && (hitInfo.type != CGameRules::EHitType::Melee) && (hitInfo.type != CGameRules::EHitType::Mike_Burn))
	{
		if (gEnv->bMultiplayer)
		{
			...
			{
				i32 shooterTeamId = GetTeam(hitInfo.shooterId);
				i32 targetTeamId = GetTeam(hitInfo.targetId);
...
//------------------------------------------------------------------------
i32 CGameRules::GetFreeExplosionIndex()
{
	i32 index = -1;
...
{
	DRX_ASSERT_TRACE (explosionInfo.type, ("Queueing an explosion with an invalid hit type! (effect='%s'/'%s')", explosionInfo.effect_name.c_str(), explosionInfo.effect_class.c_str()));

	i32 index = GetFreeExplosionIndex();
...
{
  const static u8 nMaxExp = 1;

	if(gEnv->bServer)
	{
		for (u8 exp=0; !m_queuedExplosions.empty() && exp<nMaxExp; ++exp)
		{ 
...
void CGameRules::ProcessDeferredMaterialEffects()
{
	...
	{
		u32k numExplosionsAwaitingLinetests = m_queuedExplosionsAwaitingRaycasts.size();
		for (u32 i = 0; i < numExplosionsAwaitingLinetests; i++)
		{
...
{
	i32 index = (i32)(pExplosionInfo - m_explosions);
...
void CGameRules::CullEntitiesInExplosion(const ExplosionInfo &explosionInfo)
{
	...
	i32   removeThreshold = max(1, g_pGameCVars->g_ec_removeThreshold);

	IActor *pClientActor = g_pGame->GetIGameFramework()->GetClientActor();

	Vec3 radiusVec(radiusScale * explosionInfo.physRadius);
	i32 i = gEnv->pPhysicalWorld->GetEntitiesInBox(explosionInfo.pos-radiusVec,explosionInfo.pos+radiusVec,pents, ent_rigid|ent_sleeping_rigid);
	i32 removedCount = 0;
	...
	{
		i32 entitiesToRemove = i - removeThreshold;
		for(--i;i>=0;i--)
		{
...
void CGameRules::ClientExplosion(SExplosionContainer &explosionContainer)
{
	...
	if (gEnv->bServer)
	{
		...
		DRX_ASSERT_MESSAGE(pDamageHandling, "No Damage handling module found!");
		if (pDamageHandling)
		{
	...
	if (gEnv->IsClient())
	{
		...
		if (pClientActor)
		{
			DRX_ASSERT (pClientActor->IsPlayer());
...
void CGameRules::ProjectileExplosion(const SProjectileExplosionParams &projectileExplosionInfo)
{
	...
	if(g_pGame->GetIGameFramework()->GetNetworkSafeClassName(projectileNetClassName, sizeof(projectileNetClassName), projectileExplosionInfo.m_projectileClass))
	{
		...
		if(pExplosionParams)
		{
			...
			DRX_ASSERT_MESSAGE(pExplosionParams->hitTypeId, string().Format("Invalid hit type '%s' in explosion params for '%s'", pExplosionParams->type.c_str(), projectileNetClassName));
			DRX_ASSERT_MESSAGE(pExplosionParams->hitTypeId == GetHitTypeId(pExplosionParams->type.c_str()), "Sanity Check Failed: Stored explosion type id does not match the type string, possibly PreCacheLevelResources wasn't called on this ammo type");
			...
			/*DrxLogAlways("CGameRules::ExplosionInfo %d, %d, %d, %f, %f %f %f, %f %f %f, %f, %f, %f, %f, %f, %f, %f, %d", 
...
void CGameRules::CalculateExplosionAffectedEntities(const ExplosionInfo &explosionInfo, TExplosionAffectedEntities& affectedEntities )
{
	...
	if (gEnv->bServer)
	{
		...
		u32 vcount = pVehicleSystem->GetVehicleCount();
		if (vcount > 0)
		{
...
//-----------------------------------------------------
void CGameRules::GetEntitiesToSkipByExplosion(const ExplosionInfo& explosionInfo, IPhysicalEntity** skipList, i32& skipListSize) const
{
	skipListSize = 0;
	...
	if(explosionInfo.type == CGameRules::EHitType::Mike_Burn)
	{
		...
		i32 numEntities = gEnv->pPhysicalWorld->GetEntitiesInBox(explosionInfo.pos-Vec3(1,1,1)*radius, explosionInfo.pos+Vec3(1,1,1)*radius, ppEnts, ent_static, sSimulateExplosionMaxEntitiesToSkip);

		i32 chunkThreshold = g_pGameCVars->g_MicrowaveBeamStaticObjectMaxChunkThreshold;
	...
	if (gEnv->bMultiplayer && !gEnv->bServer && (g_pGame->GetHostMigrationState() == CGame::eHMS_NotMigrating))
	{
		...
		if (pLobby)
		{
			...
			if (pMatchMaking)
			{
				...
				if (pGameLobby)
				{
					DrxSessionHandle gh = pGameLobby->GetCurrentSessionHandle();
					...
					{
						DrxLog("CGameRules::UpdateNetLimbo() GameState:%d timing out server connection, actual timeSinceReceived=%" PRIi64, (i32)(GetStateModule()->GetGameState()), packetFromServerTime.GetMilliSecondsAsInt64());
					...
					else if (packetFromServerTime > 5.0f)
					{
						...
						{
							DrxLog("CGameRules::UpdateNetLimbo() GameState:%d 5.0f seconds timing out server connection, actual timeSinceReceived=%" PRIi64, (i32)(GetStateModule()->GetGameState()), packetFromServerTime.GetMilliSecondsAsInt64());
						}
					}
					else if (packetFromServerTime > 3.0f)
					{
						...
						{
							DrxLog("CGameRules::UpdateNetLimbo() GameState:%d 3.0f seconds timing out server connection, actual timeSinceReceived=%" PRIi64, (i32)(GetStateModule()->GetGameState()), packetFromServerTime.GetMilliSecondsAsInt64());
						}
					}
					else if (packetFromServerTime > 2.0f)
					{
						...
						{
							DrxLog("CGameRules::UpdateNetLimbo() GameState:%d 2.0f seconds timing out server connection, actual timeSinceReceived=%" PRIi64, (i32)(GetStateModule()->GetGameState()), packetFromServerTime.GetMilliSecondsAsInt64());
						}
					}
		...
		if (pStateModule && (pStateModule->GetGameState() == IGameRulesStateModule::EGRS_InGame))
		{
			...
			if (pGameLobby && !pGameLobby->IsPrivateGame() && pGameLobby->IsOnlineGame())
			{
				...
				if (pActor && !pActor->IsDead())
				{
					...
					{
						DrxLog("You have been kicked because you were idle for %f seconds", m_idleTime);
...
IMPLEMENT_RMI(CGameRules, ClRenameEntity)
{
	...
	if (pEntity)
	{
		...
		DrxLogAlways("$8%s$o renamed to $8%s", old.c_str(), params.name.c_str());
...
IMPLEMENT_RMI(CGameRules, SvRequestSpectatorMode)
{
		...
		if(spawnMod)
		{
			...
			if (it != playerData->end())
			{
				if(it->second.lastRevivedTime >= it->second.deathTime)
				{
					...
					if (gEnv->bMultiplayer)
					{
						DRX_ASSERT(pActor->GetEntityId() != g_pGame->GetIGameFramework()->GetClientActorId());  // i'm not expecting this RMI to get sent to the server from the server for the server's client
					...
					if (denyRequest)
					{
						DrxLog("%s requested spectator mode %d but the request was denied", pActor->GetEntity()->GetName(), params.mode);
						return true;
					}
				}
			}
	...
	DrxLog("%s requested spectator mode %d, (forced - %s)", pActor->GetEntity()->GetName(), params.mode, params.force ? "True" : "False");
...
//------------------------------------------------------------------------
void CGameRules::ClDoSetTeam(i32 teamId, EntityId entityId)
{
	bool bIsPlayer;
...
{
	DRX_TODO(05, 02, 2010, "This needs to be completely #if'd out for final release.");
#if !defined(_RELEASE)
...
IMPLEMENT_RMI(CGameRules, ClTaggedEntity)
{
	...
	else
	{
		...
		if (!doRadarOnly)
		{
			g_pGame->GetTacticalUpr()->SetEntityTagged(params.targetId, true);
		}

		// Show entity on radar and maybe via tag-names.
		SHUDEvent eventTempAddToRadar(eHUDEvent_TemporarilyTrackEntity);
		eventTempAddToRadar.AddData( static_cast<i32>(params.targetId) );
...
{
	DrxLog("[tlh] @ (CGameRules RMI) SvRequestTagEntity()");
	assert(gEnv->bServer);
...
IMPLEMENT_RMI(CGameRules, ClProcessHit)
{
  ...
  if (pDmgMo != NULL && pClientActor && params.hitTypeId)
  {
	...
	if (pActor)
	{
		...
		DRX_ASSERT_TRACE (params.hitTypeId, ("%s '%s' is taking damage of an unknown type %u - not informing entity it's been hit!", pActor->GetEntity()->GetClass()->GetName(), pActor->GetEntity()->GetName(), params.hitTypeId));

		if (params.hitTypeId)
		{
			pActor->DamageInfo(params.shooterId, params.weaponId, pProjectileClass, params.damage, params.hitTypeId, params.dir);

			DRX_ASSERT(pActor->IsClient());
		...
		case eKS_StartVote:
			{
				if (pEntity)
				{
					message.Set(DrxMatchMakingInvalidConnectionUID, -1, CHUDUtils::LocalizeString("@mp_vote_initialized_kick", pEntity->GetName()));
					...
					hudEvent.AddData(SHUDEventData(i32(params.entityId)));
					hudEvent.AddData(SHUDEventData(i32(params.lastVoterId)));
					hudEvent.AddData(SHUDEventData(params.voteEndTime));
					hudEvent.AddData(SHUDEventData(i32(params.totalRequiredVotes)));
					hudEvent.AddData(SHUDEventData(i32(params.votesFor)));
					hudEvent.AddData(SHUDEventData(i32(params.votesAgainst)));
		...
		case eKS_VoteEnd_Success:
			{
				...
				if (pEntity)
				{
					message.Set(DrxMatchMakingInvalidConnectionUID, -1, CHUDUtils::LocalizeString("@mp_vote_initialized_kick_success", pEntity->GetName()));
					pGameLobby->LocalMessage(&message);
				}
		...
		case eKS_VoteEnd_Failure:
			{
				...
				if (pEntity)
				{
					message.Set(DrxMatchMakingInvalidConnectionUID, -1, CHUDUtils::LocalizeString("@mp_vote_initialized_kick_fail"));
					pGameLobby->LocalMessage(&message);
				}
			...
			break;
		default:
			DRX_ASSERT(!"INCORRECT KICK STATUS ON UPDATE!");
			break;
		}
	}	
...
IMPLEMENT_RMI(CGameRules, ClEnteredGame)
{
	...
	if (pClientActor)
	{
		...
		if(!gEnv->bServer)
		{
			...
			m_pGameplayRecorder->Event(pClientEntity, GameplayEvent(eGE_Connected, 0, 0, (uk )status));
		}
...
{
	DrxLog("ClAddPoints: Local player has scored %d points", params.m_changeToScore);
	...
	newScoreEvent.AddData( (uk )(&scoreInfo) );
...
{
	DrxLog("CGameRules:SvRequestRevive");
	...
	{
		DRX_ASSERT_MESSAGE(0, "CGameRules::SvRequestRevive() RMI - failed to find our spawning module, this is required");
	}
...
{
	assert((i32)m_moduleRMIListenersVec.size() >	params.m_listenerIndex);
	...
	if(pPlayer)
	{
		...
		if(params.m_environmentalWeaponId != playerPickAndThrowEntity)
		{
			...
			if(params.m_environmentalWeaponId)
			{
				...
				{
					DrxLog("IMPLEMENT_RMI(CGameRules, SvHostMigrationRequestSetup) - Client thinks they're holding a pick & throw weapon %i", params.m_environmentalWeaponId);
					pPickAndThrowWeapon->QuickAttach();
				}
			}
			else
			{
				DrxLog("IMPLEMENT_RMI(CGameRules, SvHostMigrationRequestSetup) - Client thinks they've dropped their pick & throw weapon %i", playerPickAndThrowEntity);
...
IMPLEMENT_RMI(CGameRules, ClPredictionFailed)
{
	...
	DrxLog("ClPredictionFailed name %s id %d", pEntity ? pEntity->GetName() : "<null entity>", params.predictionHandle);
#endif
...
{
	DrxLog("CGameRules::ClMidMigrationJoin() state=%i, timeSinceChange=%f", params.m_state, params.m_timeSinceStateChanged);
...
IMPLEMENT_RMI(CGameRules, ClHostMigrationPlayerJoined)
{
	...
	DrxLog("CGameRules::ClHostMigrationPlayerJoined() '%s'", pPlayer ? pPlayer->GetName() : "<NULL>");
#endif

	SHUDEvent hostMigrationOnNewPlayer(eHUDEvent_HostMigrationOnNewPlayer);
	hostMigrationOnNewPlayer.AddData(SHUDEventData(i32(playerId)));
...
{
	u32 hashId = (u32) params.m_respawnHashId;
...
{
	m_mpTrackViewUpr->Client_SynchAnimationTimes(params);
...
{
	m_mpTrackViewUpr->AnimationRequested(params);
...
{
	m_mpPathFollowingUpr->RequestAttachEntityToPath(params);
...
IMPLEMENT_RMI(CGameRules, ClHandleCheatAccusation)
{
	...
	DrxFatalError("%s", params.message.c_str());
