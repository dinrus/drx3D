...
$Id$
$DateTime$
Описание: Implements a class which handle case (group) specific 
...
#include <drx3D/Game/VehicleMovementTweaks.h>
...
bool CVehicleMovementTweaks::Init(const CVehicleParams& table)
{
  m_groups.clear();
	...
	{
		i32 i = 0;
		i32 c = tweakGroupsTable.getChildCount();
...
bool CVehicleMovementTweaks::AddGroup(const CVehicleParams& table)
{
	...
	{
		i32 i = 0;
		i32 c = tweaksTable.getChildCount();
				...
				if (tweakRef.haveAttr("name"))
				{
					...
					if (valueId > -1)
					{
						...
						if (!v.isRestrictedToMult || v.isRestrictedToMult && valueInGroup.op == eTVO_Multiplier)
							...
							DrxLog("VehicleMovementTweaks Warning: the value <%s> can only be tweaked with a multiplyer.", 
...
//------------------------------------------------------------------------
void CVehicleMovementTweaks::AddValue(tukk valueName, float* pValue, bool isRestrictedToMult)
{
...
{
	if (groupId < 0 || ((i32)m_groups.size()) <= groupId)
		return false;
...
{ 
  assert(valueId >= 0 && valueId < (i32)m_values.size());
    
  m_values[valueId].blocked = block;  
}

//------------------------------------------------------------------------
CVehicleMovementTweaks::TTweakGroupId CVehicleMovementTweaks::GetGroupId(tukk name)
{
...
//------------------------------------------------------------------------
CVehicleMovementTweaks::TValueId CVehicleMovementTweaks::GetValueId(tukk name)
{
	TValueId valueId = 0;
