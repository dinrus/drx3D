...
#include <drx3D/Game/PlayerStateLadder.h>
...
#ifndef _RELEASE
#define LadderLog(...) do { if (g_pGameCVars->pl_ladderControl.ladder_logVerbosity > 0) { DrxLog ("[LADDER] " __VA_ARGS__); } } while(0)
...
static u32 s_ladderFractionCRC = 0;

class CLadderAction : public TPlayerAction
{
public:
	CLadderAction(CPlayerStateLadder * ladderState, CPlayer & player, FragmentID fragmentID, CPlayerStateLadder::ELadderAnimType animType, tukk cameraAnimFactorAtStart, tukk cameraAnimFactorAtEnd /*float cameraAnimFactorAtStart, float cameraAnimFactorAtEnd*/) :
	...
	void InitialiseWithParams(tukk  directionText, tukk  footText)
	{
	...
	{
#ifndef _RELEASE
		DRX_ASSERT_TRACE (0, ("Unexpected anim type %d (%s) for %s action", m_animType, s_onOffAnimTypeNames[m_animType], GetName()));
#endif
	}
...
void CPlayerStateLadder::SetClientPlayerOnLadder(IEntity * pLadder, bool onOff)
{
	...
	u32k applyRenderFlags[2] = {0, ENTITY_SLOT_RENDER_NEAREST};
	u32k oldFlags = pLadder->GetSlotFlags(0);
	u32k newFlags = (oldFlags & ~ENTITY_SLOT_RENDER_NEAREST) | applyRenderFlags[onOff && renderLadderLast];
...
void CPlayerStateLadder::OnUseLadder(CPlayer& player, IEntity* pLadder) 
{
...
#ifndef _RELEASE
	DRX_ASSERT_TRACE(m_dbgNumActions == 0, ("%s shouldn't have any leftover ladder actions but has %d", player.GetEntity()->GetEntityTextDescription(), m_dbgNumActions));
#endif
	...
	m_topRungNumber = (u32) max(0.f, numberOfRungsFloat + 0.5f);
...
void CPlayerStateLadder::SendLadderFlowgraphEvent(CPlayer & player, IEntity * pLadderEntity, tukk  eventName)
{
...
bool CPlayerStateLadder::OnPrePhysicsUpdate( CPlayer& player, const SActorFrameMovementParams &movement, float frameTime )
{
	...
	if (g_pGameCVars->pl_ladderControl.ladder_logVerbosity)
	{
		...
		if (m_mostRecentlyEnteredAction && m_mostRecentlyEnteredAction->GetStatus() == IAction::Installed)
		{
			...
			DrxWatch ("[LADDER] Animation: '%s' (timeActive=%.2f timeRemaining=%.2f speed=%.2f)", m_mostRecentlyEnteredAction->GetName(), m_mostRecentlyEnteredAction->GetActiveTime(), timeRemaining, m_mostRecentlyEnteredAction->GetSpeedBias());
		}
		else
		{
			DrxWatch ("[LADDER] Animation: %s", m_mostRecentlyEnteredAction ? "NOT PLAYING" : "NONE");
		}
	}
#endif
	...
	else if (player.GetActorStats())
	{
			...
			else
			{
				...
				if (m_topRungNumber == m_numRungsFromBottomPosition)
				{
					...
					if (pushUpDown > 0.5f)
					{
						...
						{
							DrxWatch ("[LADDER] %s can't climb up and off %s - top is blocked", player.GetEntity()->GetName(), pLadder->GetEntityTextDescription());
						}
#endif
					}
				}
		...
		if (m_mostRecentlyEnteredAction)
		{
			...
			if (g_pGameCVars->pl_ladderControl.ladder_logVerbosity)
			{
				DrxWatch ("[LADDER] $7Setting anim fraction to %.4f", animFraction);
			}
#endif
...
void CPlayerStateLadder::SetHeightFrac(CPlayer& player, float heightFrac)
{
	...
	m_numRungsFromBottomPosition = (i32)heightFrac;
...
bool CPlayerStateLadder::IsUsableLadder(CPlayer& player, IEntity* pLadder, const SmartScriptTable& ladderProperties)
{
	...
	if(pLadder && !player.IsOnLadder() && (player.CanTurnBody() || player.IsThirdPerson()))
	{
		...
		if(height > 0.f)
		{
			...
			tukk angleVariable = ((playerPos.z + 0.1f) > (ladderPos.z + height)) ? "approachAngleTop" : "approachAngle";
	...
	if (g_pGameCVars->pl_ladderControl.ladder_logVerbosity)
	{
		...
		DrxWatch ("[LADDER] Is %s usable by %s? %s", pLadder ? pLadder->GetEntityTextDescription() : "<NULL ladder entity>", player.GetEntity()->GetEntityTextDescription(), retVal ? "$3YES$o" : "$4NO$o");
	}
#endif
...
#ifndef _RELEASE
void CPlayerStateLadder::UpdateNumActions(i32 change)
{
