...
#include <drx3D/Game/FlowWeaponCustomizationNodes.h>
...
void CFlashUIInventoryNode::ProcessEvent( EFlowEvent event, SActivationInfo *pActInfo )
{
	if(event == eFE_Activate && IsPortActive(pActInfo, 0))
	{
		...
		if(pActor)
		{
			...
			{
				SUIArguments args;
				i32 inv_cap = gEnv->pConsole->GetCVar("i_inventory_capacity")->GetIVal();
				for (i32 i = 0; i < inv_cap; i++)
				{
					tukk weaponName = pInventory->GetItemString(i);
		...
		if(pActor)
		{
			...
			if(pInventory)
			{
				...
				if(is_inInventory)
				{
					...
					tukk weapon_name = GetPortString(pActInfo, eI_Weapon).c_str();
					...
					if(pEntity)
					{
						...
						CGameObject * pGameObject = (CGameObject*)pEntity->GetProxy(ENTITY_PROXY_USER);
						tukk ext = pGameObject->GetEntity()->GetClass()->GetName();
						CWeapon* pWeapon = (CWeapon*)pGameObject->QueryExtension(pGameObject->GetEntity()->GetClass()->GetName());
...
void CSetEquipmentLoadoutNode::GetConfiguration( SFlowNodeConfig &config )
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig<i32>( "pack", "which pack" ),
		{0}
	};
...
void CSetEquipmentLoadoutNode ::ProcessEvent( EFlowEvent event, SActivationInfo *pActInfo )
{
	if(event == eFE_Activate && IsPortActive(pActInfo, 0))
	{
		if(pActInfo->pEntity)
		{
			...
			if(pActor)
			{
				i32 packIndex = GetPortInt(pActInfo, eI_EquipLoadout);
