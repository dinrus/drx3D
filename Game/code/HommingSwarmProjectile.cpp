...
CHommingSwarmProjectile::CHommingSwarmProjectile()
	...
	,	m_curlDirection(fsgnf(drx_random(-1.0f, 1.0f)))
	,	m_curlTime(0.0f)
	,	m_curlShift(drx_random(0.0f, 1.0f))
{
  m_Roll = drx_random(0.0f, gf_PI);
}
...
void CHommingSwarmProjectile::Update(SEntityUpdateContext &ctx, i32 updateSlot)
{
...
Vec3 CHommingSwarmProjectile::Wander(const SVehicleStatus& vehicle, float deltaTime)
{
	...
	while (m_wanderTimer > invFrequency)
	{
		...
		m_wanderState += drx_random_componentwise(Vec3(-wanderRadius), Vec3(wanderRadius));
