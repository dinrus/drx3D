...
#include <drx3D/Game/Player.h> // for CPlayer, ...

#include <drx3D/Game/PlayerAnimation.h> // for priorities (for now)
...
public:
	...
	bool EarlyExitStartTransition()
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "I2M segmentation is not properly set up for %s.", m_rootScope->GetCharInst()->GetIAnimationSet()->GetNameByAnimID(pAnim->GetAnimationId()));
			return true;
		}
		...
		for(i32 dim = 0; dim < pParametricSampler->m_numDimensions; ++dim)
		{
	...
	virtual EStatus Update(float timePassed) override
	{
		...
		if (m_eStatus != Finished)
		{
			...
			if (m_pTransition->transitionType == eTT_Stop)
			{
				...
				DRX_ASSERT( pAnimationComponent );
				...
				if ( isGoingToCoverStance )
				{
					...
					{
						DRX_ASSERT_MESSAGE( false, "Going to cover but no cover location set" );
	...
	virtual void OnAnimationEvent(ICharacterInstance *pCharacter, const AnimEventInstance &event) override
	{
		...
		static u32 animExitCRC32 = CCrc32::ComputeLowercase("animexit");
...
private:
	...
	bool FragmentExistsInDatabase(const SAnimationContext& context, const IAnimationDatabase& database) const
	{
		...
		u32k optionCount = database.FindBestMatchingTag(SFragmentQuery(m_fragmentID, fragTagState));
	...
	static void FillMotionParameters(SPredictedCharacterStates* pMotionParameters, IEntity& entity, ETransitionType transitionType, const STransitionFuture& future)
	{
		...
		{
			DRX_ASSERT(transitionType == eTT_Stop);
	...
	static float CalculateStopLeg(IEntity& entity)
	{
		...
		i32k nAnimsInFIFO = pISkeletonAnim->GetNumAnimsInFIFO(layer);
...
//////////////////////////////////////////////////////////////////////////
bool CMovementTransitionsController::RequestTransition(tukk szFragmentID, const STransition*const pTransition, const STransitionFuture& future)
{
	DRX_ASSERT(pTransition);
	...
	else
	{
		...
		{
			DRX_ASSERT(!m_pPendingAnimAction);
...
i32 CCornerSmoother::m_lastPredictionFrameID = 0;
i32 CCornerSmoother::m_numPredictionsThisFrame = 0;
	...
	float*const pNewDesiredSpeed, Vec3*const pNewDesiredMovement)
{
	...
	{
		DRX_ASSERT(desiredMovement.IsUnit());
...
void CCornerSmoother::DebugRender(float z) 
{
	...
	from.z = z;
	for(i32 i=1; i<m_prediction.numSamples; ++i)
	{
	...
	from.z = z;
	for(i32 i=1; i<m_simplePrediction.numSamples; ++i)
	{
	...
	if (const IAIObject* pAI = pPlayer->GetEntity()->GetAI())
	{
		if (const IAIPathAgent* pAIAgent = pAI->CastToIAIActor())
		{
			if (const IPathFollower* pPathFollower = pAIAgent->GetPathFollower())
			{
				...
				for(i32 i=0; i<prediction.numSamples; ++i)
				{
...
void CCornerSmoother::SimplifyPrediction(const Vec3& playerPos, const SPrediction& input, SPrediction*const pOutput)
{
	...
	i32 lastIncludedIndex = -1; // we begin at the playerPos, which is not included in the list
	while(lastIncludedIndex < input.numSamples)
	{
		...
		if (nextIncludedIndex < input.numSamples)
		{
			while(nextIncludedIndex < input.numSamples - 1)
			{
				...
				i32 proposedNextIncludedIndex = nextIncludedIndex + 1;
	...
	DRX_ASSERT(pOutput->samples[pOutput->numSamples-1] == input.samples[input.numSamples-1]);
}
