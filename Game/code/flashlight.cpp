...
void CFlashLight::EnableLight(bool enable)
{
	...
	i32 slot = ownerIsFP ? eIGS_FirstPerson : eIGS_ThirdPerson;

	if (enable)
	{
		tukk attachHelper = "light_term";
...
void CFlashLight::EnableFogVolume(CWeapon* pWeapon, i32 slot, bool enable)
{
	...
	tukk attachHelper = "lightFog_term";
	...
	if (enable && pCharacter)
	{
		...
		IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByName(attachHelper);
