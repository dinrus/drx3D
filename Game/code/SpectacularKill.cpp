...
//-----------------------------------------------------------------------
void CSpectacularKill::DebugLog(tukk szFormat, ...)
{
#ifndef _RELEASE
...
bool CSpectacularKill::CanExecuteOnTarget(const CActor* pTarget, const SSpectacularKillAnimation& anim) const
{
	...
	ICooperativeAnimationUpr* pCooperativeAnimationUpr = gEnv->pGame->GetIGameFramework()->GetICooperativeAnimationUpr();
	if (pCooperativeAnimationUpr->IsActorBusy(m_pOwner->GetEntityId()) ||	pCooperativeAnimationUpr->IsActorBusy(m_targetId))
	{
...
bool CSpectacularKill::GetValidAnim(const CActor* pTarget, SSpectacularKillAnimation& anim) const
{
	...
	if (pTargetClassParams)
	{
		...
		i32 iNumValidAnims = static_cast<i32>(validAnims.size());
		if (iNumValidAnims > 0)
		{
			anim = validAnims[drx_random(0, iNumValidAnims - 1)];
			bSuccess = true;
		}
		else
		{
...
void CSpectacularKill::Update(float frameTime)
{
	...
	if (IsBusy())
	{
		...
		if (!pTargetActor || 
			(!pCooperativeAnimationUpr->IsActorBusy(m_pOwner->GetEntityId()) &&
			!pCooperativeAnimationUpr->IsActorBusy(pTargetActor->GetEntityId())) ||
...
{
	DRX_ASSERT_MESSAGE(m_isBusy, "spectacular kill should be in progress when triggering the death blow");
	if (!m_isBusy)
		return;
	...
	DRX_ASSERT(pSpectacularKillParams);
...
{
	DRX_ASSERT_MESSAGE(IsBusy(), "spectacular kill cannot be stopped if it is not in progress");
	if (!IsBusy())
		return;
	...
	if(pTarget)
	{
		pCooperativeAnimationUpr->StopCooperativeAnimationOnActor(m_pOwner->GetAnimatedCharacter(), pTarget->GetAnimatedCharacter());
	...
	{
		pCooperativeAnimationUpr->StopCooperativeAnimationOnActor(m_pOwner->GetAnimatedCharacter());
	}
...
bool CSpectacularKill::StartOnTarget(CActor* pTargetActor)
{
	...
	DRX_ASSERT(pTargetActor);
	DRX_ASSERT_MESSAGE(!IsBusy(), "spectacular kill should not be initiated while a spectacular kill is already in progress");
	...
	if (!IsBusy() && pTargetActor && GetValidAnim(pTargetActor, anim) && CanExecuteOnTarget(pTargetActor, anim))
	{
		...
		tukk pKillerAnim = anim.killerAnimation.c_str();
		tukk pVictimAnim = anim.victimAnimation.c_str();
		...
		ICooperativeAnimationUpr* pCooperativeAnimationUpr = gEnv->pGame->GetIGameFramework()->GetICooperativeAnimationUpr();
		bool bStarted = pCooperativeAnimationUpr->StartNewCooperativeAnimation(killerParams, targetParams, animParams);
		if (bStarted)
		{
...
{
	DRX_ASSERT_TRACE(m_pParams != NULL, ("Trying to get params from a spectacular kill object without params. Check if entity class %s has a param file in Scripts/actor/parameters", m_pOwner->GetEntityClassName()));
	if (m_pParams)
	{
		DRX_ASSERT(pTargetClass);
...
bool CSpectacularKill::CanSpectacularKillOn(const CActor* pTarget) const
{
	...
	if (pTarget)
	{
		...
		if (pTargetParams)
		{
			...
			{
				i32k numAnims = static_cast<i32>(pTargetParams->animations.size());
				if (numAnims > 0)
				{
...
void CSpectacularKill::ReadXmlData( const IItemParamsNode* pRootNode)
{
	...
	// too often, though)
	tukk szEntityClassName = m_pOwner->GetEntityClassName();
	...
	if (pParams)
	{
		...
		for (i32 i = 0; i < childCount; ++i)
		{
			...
			if (pTargetClass)
			{
				...
				if(pChildParamsNode)
				{
					...
					tukk szImpulseBone = pChildParamsNode->GetAttributeSafe("impulseBone");
				...
				if(pChildAnimsNode)
				{
					i32k animCount = pChildAnimsNode->GetChildCount();
					targetParams.animations.reserve(animCount);

					for(i32 j = 0; j < animCount; j++)
					{
				...
				DRX_ASSERT_MESSAGE(targetParams.animations.size() > 0, string().Format("No Animations defined for %s spectacular kill", pTargetClass->GetName()));
		...
		m_pParams = CastSharedParamsPtr<SSharedSpectacularParams>(pSharedParamsUpr->Register(sharedParamsName, newParams));
	}
}
...
bool CSpectacularKill::ObstacleCheck(const Vec3& vKillerPos, const Vec3& vTargetPos, const SSpectacularKillAnimation& anim) const
{
	...
	i32 collisionEntityTypes = ent_static | ent_terrain | ent_sleeping_rigid | ent_ignore_noncolliding;
