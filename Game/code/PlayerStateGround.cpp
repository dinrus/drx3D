...
#include <drx3D/Game/PlayerStateGround.h>
#include <drx3D/Game/Player.h>
#include <drx3D/Game/PlayerStateJump.h>
...
void CPlayerStateGround::OnPrePhysicsUpdate( CPlayer& player, const SActorFrameMovementParams &movement, float frameTime, const bool isHeavyWeapon, const bool isPlayer )
{
	...
	const CAutoAimUpr& autoAimUpr = g_pGame->GetAutoAimUpr();
	const EntityId closeCombatTargetId = autoAimUpr.GetCloseCombatSnapTarget();
	...
	{
		ProcessAlignToTarget(autoAimUpr, player, pCloseCombatTarget);
	}
	else
	{
...
#if 0
		DrxLogAlways("DesiredVel: (%f %f %f)", move.x, move.y, move.z);
#endif
...
#if 0
		DrxLogAlways("DesiredVel: (%f %f %f)", desiredVel.x, desiredVel.y, desiredVel.z);
#endif
		...
		if(g_pGameCVars->pl_debug_movement > 0)
		{
			...
			tukk name = player.GetEntity()->GetName();
...
#if 0
		{
			...
			DrxLogAlways("PlayerMovement: %s (%f %f %f) t:%f", player.GetEntity()->GetName(), m_request.velocity.x, m_request.velocity.y, m_request.velocity.z, gEnv->pTimer->GetCurrTime() - JumpTime);
			DrxLogAlways("DesiredVel: (%f %f %f) SM:%f", m_movement.desiredVelocity.x, m_movement.desiredVelocity.y, m_movement.desiredVelocity.z, m_params.strafeMultiplier);
			if (m_movement.jump)
			{
				if (!player.IsClient())
				{
				...
				DrxLogAlways("PlayerJump: %s (%f %f %f) DesVel: (%f %f %f)", player.GetEntity()->GetName(), jumpVec.x, jumpVec.y, jumpVec.z, desiredVel.x, desiredVel.y, desiredVel.z);
...
{
	i32k enableVaultFromStandingCVar = g_pGameCVars->pl_ledgeClamber.enableVaultFromStanding;
	...
	if (doCheck)
	{
		...
		{
			DRX_ASSERT( LedgeId(ledgeTransition.m_nearestGrabbableLedgeId).IsValid() );
			const SLedgeInfo ledgeInfo = g_pGame->GetLedgeUpr()->GetLedgeById( LedgeId(ledgeTransition.m_nearestGrabbableLedgeId) );
			
			DRX_ASSERT( ledgeInfo.IsValid() );
				...
				else
				{
					...
					if (g_pGameCVars->pl_ledgeClamber.debugDraw)
					{
						...
						switch (ledgeTransition.m_ledgeTransition)
						{
							...
							default:
							DRX_ASSERT_TRACE(0, ("Unexpected ledge transition #%d when trying to display HUD prompt for vault-from-standing!", ledgeTransition.m_ledgeTransition));
							break;
						}
...
//-----------------------------------------------------------------------------------------------
void CPlayerStateGround::ProcessAlignToTarget(const CAutoAimUpr& autoAimUpr, CPlayer& player, const IActor* pTarget)
{
	DRX_ASSERT(pTarget);
	...
	float targetDistance = autoAimUpr.GetCloseCombatSnapTargetRange();
	...
	if(displacement.len2() > sqr(targetDistance))
	{
		...
		moveRequest.velocity = (displacement * distanceToTravel * autoAimUpr.GetCloseCombatSnapTargetMoveSpeed()) + targetVelocity;
	}
}
