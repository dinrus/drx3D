...
#include <drx3D/Game/ActionMultiCoopAnim.h>
...
#ifdef MCALOG
#define MCALog(...)					DrxLog("[MCA] "__VA_ARGS__)
#define MCALogAlways(...)		DrxLogAlways("[MCA] "__VA_ARGS__)
#else
...
{
	DRX_ASSERT_MESSAGE(GetStatus()==None,"Slaves must be added before queueing the action!");
...
void CActionMultiCoopAnimation::Install()
{
	...
	m_master.Install();
	DRX_ASSERT_MESSAGE(m_master.GetActionController(), "Master must have ActionController.");
...
CActionMultiCoopAnimation::SParticipant::SParticipant( IAnimatedCharacter& animChar, ICharacterInstance* pCharInst, TagID tagId, u32 contextId, const IAnimationDatabase* pAnimDb, const ParamFlags params )
...
void CActionMultiCoopAnimation::SParticipant::SetRenderNearest( IEntity& entity, const bool bSet )
{
	...
	if(IEntityRenderProxy* pProxy = (IEntityRenderProxy*)entity.GetProxy(ENTITY_PROXY_RENDER))
	{
		...
		{
			i32k nslots = pRenderNode->GetSlotCount();
			u32 orFlags = 0;
			u32 andFlags = ~0;
			if(bSet)
			...
			{
				u32k slotFlags = entity.GetSlotFlags(i);
...
CActionMultiCoopAnimation::SPlayerParticipant::SPlayerParticipant( CPlayer &player, TagID tagId, u32 contextId, const IAnimationDatabase* pAnimDb, const ParamFlags params ) 
