...
#include <drx3D/AI/IPathfinder.h> // for pathfollower
...
namespace CornerSmoothing
{
	...
	static i32k TAKECORNER_MAX_SHOOTOUT_SPLINES = 3;
	...
	TWEAKABLE i32   SHOOTOUT_PREDICTION_MIN_NUM_STEPS = 3; // TODO: use higher number on PC
	...
	static i32k SHOOTOUT_PREDICTION_MAX_SAMPLES = 25; // with the above settings, should be enough for 50m long distance to movetarget -- TODO: Use higher number on PC
	...
	namespace Debug
	{
		...
		static i32 START_Y = 195; // TODO: Turn into 190?
		static i32 START_X = 10;
		static i32 LOG_Y_INCREMENT = 9;
		static i32 MAX_LOG_ENTRIES = 55; // TODO: Turn into 60??
		...
		static i32 s_currentY = START_Y;
		static i32 s_frameID = -1;
		...
		void AddLogEntry(tukk _pszFormat,...)
		{
			va_list args;
			va_start(args, _pszFormat);
			drx_vsprintf(s_buffer, _pszFormat, args);
			va_end(args);
		...
		void DrawLog()
		{
			...
			for(TLogEntries::const_iterator i = s_logEntries.begin(); i != iEnd; ++i)
			{
				...
				i32k age = s_frameID - entry.frameID;
...
#define CORNERSMOOTHER_LOG(format, ...)
	...
	static inline EFindPointAlongSplineResult FindPointAlongSplineUsingBinarySearch(const SHermiteSpline& data, const float curAlpha, const Vec2& curPos, const float minDistanceSq, const float maxDistanceSq, float& newAlpha, Vec2& newPos)
	{
		...
		DRX_ASSERT(minDistanceSq < maxDistanceSq);
...
#ifndef NDEBUG
		{
			...
			DRX_ASSERT((testPos - curPos).GetLength2() < square(0.001f));
		}
#endif
		...
		do
		{
			...
			DRX_ASSERT(tryAlpha >= minAlpha);
			DRX_ASSERT(tryAlpha <= maxAlpha);
	...
	{
		DRX_ASSERT(beginSample != NULL);
		DRX_ASSERT(endSample != NULL);
		DRX_ASSERT(endSample > beginSample);
		...
		for(const SSplineSample* currentSample = beginSample; currentSample != endSample; ++currentSample)
		{
			if (currentSample->alpha >= alpha)
			{
				...
				DRX_ASSERT(output.maximumSpeedSq >= 0.0f);
#if INCLUDE_CORNERSMOOTHER_DEBUGGING()
	...
	{
		DRX_ASSERT(minDistance < maxDistance);
...
// ===========================================================================
	...
	namespace Debug
	{
		void DrawSpline(const SHermiteSpline& spline, const ColorF& lineColor, const ColorF& tangentColor, const Vec2& extraPos1, const Vec2& extraPos2, const float sphereRadius = 0.0f, const ColorF& sphereColor = ColorF(1.0f,1.0f,1.0f, 1.0f))
		{
			...
			i32k flags3D = e_Mode3D | e_AlphaBlended | e_DrawInFrontOff | e_FillModeSolid | e_CullModeBack | e_DepthWriteOn | e_DepthTestOn;
			i32k flags2D = e_Mode2D | e_AlphaBlended;
			...
			for(i32 i=1; i<numSamples; ++i) // start from 1!
			{
				float alpha = float(i)/float(numSamples);
		...
		void DrawPrediction(const Vec3& startPosition, const SPrediction_tpl<Sample>& prediction, const ColorF& lineColor, const float sphereRadius = 0.0f, const ColorF& sphereColor = ColorF(1.0f,1.0f,1.0f, 1.0f))
		{
			...
			for(i32 i = 1; i < prediction.numSamples; ++i)
			{
	...
	{
		DRX_ASSERT(SPEED1 > SPEED0);
		DRX_ASSERT(TURNSPEED1 < TURNSPEED0);
	...
	static bool ILINE CanWeSlowDownInDistance(const SMovementLimits& limits, const float startSpeed, const float maxDistance, const float targetSpeed, float& minimumStartSpeed)
	{
		...
		DRX_ASSERT(limits.speedRange.min > 0.0f);
		...
		if (clampedSpeedAtDistSq <= square(targetSpeed))
		{
			...
			DRX_ASSERT(clampedSpeedAtDistSq >= 0.0f);
	...
	*/
	...
	{
		DRX_ASSERT(unitVector.GetLength() > 0.999f); // TODO: Write IsUnit() for Vec2
	...
	static i32 END_OF_PATH_FRAMES = 4;
	...
	public:
		...
		{
			DRX_ASSERT(state.hasMoveTarget);
			...
			if (m_lastAlpha > 0.0f)
			{
				DRX_ASSERT(m_lastOutput.desiredSpeed >= 0.0f);
		...
		EPlanExecutionResult Execute(const SState& state, SOutput& output) override
		{
			...
			bool reachedEnd = false;
			{
				...
				IF_UNLIKELY(result != eFPAS_Success)
				{
					...
					DRX_ASSERT((result == eFPAS_AtEndOfSpline) || (result == eFPAS_BeyondEndOfSpline));
					reachedEnd = true;
				}
			}
			...
			if (wasMoving)
			{
				if (distanceToTarget2DSq <= FLT_EPSILON)
				{
					...
					DRX_ASSERT(!reachedEnd); // if we reachedEnd already this should have been caught before. CCornerSmoother2::IsPlanNeeded() should have returned false as we are already at the target.
			...
			DRX_ASSERT(m_lastAlpha > 0.0f); // if this fails I need another variable to keep track of 'am I started yet'
	...
	static void SimplifyShootOutPrediction(const Vec3& playerPos, const SSplinePrediction& input, SSimplifiedPrediction*const pOutput)
	{
		...
		i32 lastIncludedIndex = 0; // we begin at the playerPos, which is not included in the list
		while(lastIncludedIndex < input.numSamples)
		{
			i32 nextIncludedIndex = lastIncludedIndex + 1;
			if (nextIncludedIndex < input.numSamples)
			{
				while(nextIncludedIndex < input.numSamples - 1)
				{
					// try to advance to the next
					i32k proposedNextIncludedIndex = nextIncludedIndex + 1;
		...
		DRX_ASSERT(pOutput->numSamples > 0);
		DRX_ASSERT(pOutput->samples[pOutput->numSamples-1].pos == input.samples[input.numSamples-1].pos);
	}


	void AdjustMaximumSpeedBasedOnDeceleration(const float maximumDeceleration, SSplinePrediction& prediction)
	{
		DRX_ASSERT(prediction.IsValid());
		DRX_ASSERT(prediction.numSamples >= 2);
		DRX_ASSERT(maximumDeceleration >= 0.0f);
		...
		for(i32 iLimitingSample = prediction.numSamples - 1; iLimitingSample >= 1; --iLimitingSample)
		{
	...
	{
		DRX_ASSERT(SHOOTOUT_PREDICTION_MAX_SAMPLES >= 1);
		...
		i32k unclampedNumSteps = SHOOTOUT_PREDICTION_MIN_NUM_STEPS + i32(SHOOTOUT_PREDICTION_NUM_STEPS_PER_HERMITE_METERS * hermiteSize);
		i32k clampedNumSteps = std::min(SHOOTOUT_PREDICTION_MAX_SAMPLES-1, unclampedNumSteps);
...
//#endif
			...
			DRX_ASSERT(data.prediction.samples[i-1].maximumSpeedSq >= 0.0f);
#if INCLUDE_CORNERSMOOTHER_DEBUGGING()
		...
		DRX_ASSERT(data.prediction.samples[i-1].alpha == 1.0f);
		DRX_ASSERT(data.prediction.samples[i-1].pos == data.spline.target2D);
		DRX_ASSERT(data.prediction.numSamples > 0);
	...
	static bool CheckWalkability(const IPathFollower* pPathFollower, const SSimplifiedPrediction& prediction)
	{
		...
		DRX_ASSERT(sizeof(prediction.samples[0]) == sizeof(Vec2));
		DRX_ASSERT((tuk)&prediction.samples[1] - (tuk)&prediction.samples[0] == sizeof(Vec2)); // check alignment
	...
	bool PlanShootOutToMoveTarget(const SState& state, SShootOutData& outputShootOut)
	{
		...
		do
		{
			...
			{
				DRX_ASSERT(testShootOut.IsValid());
		...
		for(i32 i = numBestShootOuts - 1; i >= 0; --i)
		{
		...
		if (weFoundBest)
		{
			...
			if (isEndOfPath)
			{
				DRX_ASSERT(state.hasInflectionPoint);
			...
			{
				DRX_ASSERT(state.hasInflectionPoint);
				DRX_ASSERT(!(state.inflectionPoint - state.moveTarget).IsZero());
				...
				else
				{
					...
					DRX_ASSERT(outputShootOut.spline.target2D == Vec2(state.moveTarget));
					...
					DRX_ASSERT(cornerState.hasMoveTarget == true);
					...
					{
						DRX_ASSERT(cornerOutput.IsValid());

						if (cornerOutput.GetMaximumEntrySpeedSq() < outputShootOut.GetMaximumExitSpeedSq())
						{
							DRX_ASSERT(cornerOutput.GetMaximumEntrySpeedSq() >= 0.0f);
...
#define CORNERSMOOTHER_DEBUGDRAW(state, retval, outputShootOut)
	...
	bool CCornerSmoother2::Update(const SState& state, SOutput& output)
	{
		...
		if (g_pGameCVars->ctrlr_corner_smoother_debug > 0)
		{
			...
			tukk name = state.pActor->GetEntity()->GetName();
	...
	{
		DRX_ASSERT(Debug::s_debugEnabledDuringThisUpdate);
