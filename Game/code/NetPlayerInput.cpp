...
struct CNetLerper::Debug
{
	...
	void Draw()
	{
		...
		ColorB desiredVelColour = ColorB(255,(i32)(128.f+127.f*sinf(8.f*m_timer)),0,255);  // Yellow/Red
		ColorB actualPosColour = ColorB(0,255,0,255);  // Green
		ColorB actualVelColour = ColorB(0,0,(i32)(128.f+127.f*sinf(5.f*m_timer)),255);  // blue/black
		ColorB snapPosColour = ColorB(255,255,255,255); // White
		ColorB lerpErrorColour = ColorB(255,0,0,255); // Red
		
		// Draw the desired positions
		for (u32 i=0; i<m_desired.size(); i++)
		{
...
i32 CNetLerper::GetLerpError(const Vec3& errorToDesiredPos, const Vec3& lerpError) const
{
	i32 error = k_noError;
...
void CNetLerper::Update(float dt, const Vec3& entityPos, SPrediction& predictionOut, const Vec3& velGround, bool bInAirOrJumping)
{
	...
	i32k snapError = GetLerpError(errorToDesiredPos, m_lerpedError);
...
void CNetLerper::DebugDraw(const SPrediction& prediction, const Vec3& entityPos, bool hadNewData)
{
	...
	{
		DrxWatch("Cur: (%.2f, %.2f, %.2f) Des: (%.2f, %.2f, %.2f) Pred: (%.2f, %.2f, %.2f) ", entityPos.x, entityPos.y, entityPos.z, desiredPosition.x, desiredPosition.y, desiredPosition.z, predictedPosition.x, predictedPosition.y, predictedPosition.z);
		DrxWatch("LerpErrorXY: (%.2f)", m_lerpedError.GetLength2D());
		DrxWatch("InputSpeed: (%.2f, %.2f, %.2f) lerpVel: (%.2f, %.2f, %.2f) lerpSpeed: %.2f", desiredVelocity.x, desiredVelocity.y, desiredVelocity.z, lerpVel.x, lerpVel.y, lerpVel.z, lerpVel.GetLength());
	}	
	if (hadNewData)
	{
...
void CNetPlayerInput::Update()
{
	if (gEnv->bServer && (g_pGameCVars->sv_input_timeout>0) && ((gEnv->pTimer->GetFrameStartTime()-m_lastUpdate).GetMilliSeconds()>=g_pGameCVars->sv_input_timeout))
	{
		...
		m_curInput.stance=(u8)STANCE_NULL;
...
{
	DRX_TODO(24, 3, 2010, "Consider feeding into a timeout onto the inputs here");
...
void CNetPlayerInput::DoSetState(const SSerializedPlayerInput& input )
{
	...
	{
		SInputEventData inputEventData( SInputEventData::EInputEvent_Sprint, m_pPlayer->GetEntityId(), CDrxName("sprint"), input.sprint ? eAAM_OnPress : eAAM_OnRelease, 0.f );
