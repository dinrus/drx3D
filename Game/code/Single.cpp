...
#include <drx3D/Game/AI/HazardModule/HazardModule.h>
#include <drx3D/Game/ItemAnimation.h>
#include <drx3D/Game/Melee.h>

DRX_IMPLEMENT_GTI(CSingle, CFireMode);
...
//------------------------------------------------------------------------
void CSingle::Update(float frameTime, u32 frameId)
{
	...
	if(pAimDebug->GetIVal()!=0)
	{
		...
		for(i32 dbgIdx(0);dbgIdx<DGB_curLimit; ++dbgIdx)
...
//------------------------------------------------------------------------
i32 CSingle::GetAmmoCount() const
{
	return m_pWeapon->GetAmmoCount(m_fireParams->fireparams.ammo_type_class);
}

//------------------------------------------------------------------------
i32 CSingle::GetClipSize() const
{
	i32k clipSize = m_fireParams->fireparams.clip_size;
...
//------------------------------------------------------------------------
i32 CSingle::GetChamberSize() const
{
...
bool CSingle::LowAmmo(float thresholdPerCent) const
{
	...
	if (clipSize != 0)
	{
		return m_fireParams->fireparams.ammo_type_class && clipSize != -1 && m_pWeapon->GetAmmoCount(m_fireParams->fireparams.ammo_type_class) < (i32)(thresholdPerCent*clipSize);
	}

	return m_fireParams->fireparams.ammo_type_class && (m_pWeapon->GetInventoryAmmoCount(m_fireParams->fireparams.ammo_type_class) < (i32)(thresholdPerCent*clipSize));
}
...
//------------------------------------------------------------------------
void CSingle::Reload(i32 zoomed)
{
...
{
	FillAmmoAction(CSingle *_single, i32 reloadStartFrame):
	single(_single), _reloadStartFrame(reloadStartFrame){};
	
	CSingle *single;
	i32 _reloadStartFrame;
...
{
	EndReloadAction(CSingle *_single, i32 zoomed, i32 reloadStartFrame):
...
void CSingle::PlayShootAction(i32 ammoCount)
{
	CActor *pActor = m_pWeapon->GetOwnerActor();
	const bool playerIsShooter = pActor ? pActor->IsPlayer() : false;
	i32 flags = CItem::eIPAF_Default;
...
void CSingle::StartReload(i32 zoomed)
{
	...
	i32 fragmentID = m_pWeapon->GetFragmentID("reload", &pTagDefinition);
	...
	if (pOwnerActor && pOwnerActor->IsPlayer())
	{
		...
		endReloadTime = u32(animTime * m_fireParams->fireparams.endReloadFraction);
		fillAmmoTime = u32(animTime * m_fireParams->fireparams.fillAmmoReloadFraction);
	}
	else
	{
		endReloadTime = (u32)(GetShared()->fireparams.ai_reload_time * 1000.0f);
...
//------------------------------------------------------------------------
void CSingle::SetReloadFragmentTags(CTagState& fragTags, i32 ammoCount)
{
	m_pWeapon->SetAmmoCountFragmentTags(fragTags, ammoCount);
}

//------------------------------------------------------------------------
void CSingle::EndReload(i32 zoomed)
{
	...
	m_reloadCancelled = false;
	m_pWeapon->SetBusy(false);
	u8k blockedActions = ( gEnv->bMultiplayer ? (CWeapon::eWeaponAction_Reload|CWeapon::eWeaponAction_Fire) : CWeapon::eWeaponAction_Reload );
...
bool CSingle::FillAmmo(bool fromInventory)
{
	...
	i32k currentAmmoCount = m_pWeapon->GetAmmoCount(ammo);
	i32k chamberSize = GetChamberSize();
	i32k clipSize = GetClipSize();
	...
	if (m_pWeapon->IsServer() && !m_reloadCancelled && fromInventory)
	{
		...
		i32 inventoryCount = m_pWeapon->GetInventoryAmmoCount(ammo);
		i32 giveAmmo = 0;
		...
		{
			i32 freeSpace = (clipSize + chamberSize) - currentAmmoCount;
		...
		tukk ammoName = ammo ? ammo->GetName() : "";

		g_pGame->GetIGameFramework()->GetIGameplayRecorder()->Event(m_pWeapon->GetOwner(), GameplayEvent(eGE_WeaponReload, ammoName, giveAmmo, (uk )(EXPAND_PTR)(m_pWeapon->GetEntityId())));
	}
	else if (!fromInventory && m_pWeapon->IsServer())
	{
		i32 fillAmmount = (clipSize + chamberSize);
...
bool CSingle::Shoot(bool resetAnimation, bool autoreload, bool isRemote)
{
	...
	i32k clipSize = GetClipSize();
	i32 ammoCount = (clipSize != 0) ? m_pWeapon->GetAmmoCount(pAmmoClass) : m_pWeapon->GetInventoryAmmoCount(pAmmoClass);
...
/*
	...
	i32 ammoCost = GetShootAmmoCost(playerIsShooter);
	ammoCost = min(ammoCount, ammoCost);

	EntityId ammoEntityId = 0;
	i32 ammoPredicitonHandle = 0;
	...
	if (pAmmo)
	{
		...
		i32 frequency = tracerParams->frequency;
	...
	{
		tukk ammoName = pAmmoClass != NULL ? pAmmoClass->GetName() : NULL;
	...
	//DrxLog("RequestShoot - pos(%f,%f,%f), dir(%f,%f,%f), hit(%f,%f,%f)", pos.x, pos.y, pos.z, dir.x, dir.y, dir.z, hit.x, hit.y, hit.z);
...
//------------------------------------------------------------------------
i32 CSingle::GetShootAmmoCost(bool playerIsShooter)
{
	i32 cost = 1;
...
Vec3 CSingle::GetFireHelperPos() const
{
  if (HasFireHelper())
  {
    i32 id = m_pWeapon->GetStats().fp?0:1;
    i32 slot = id ? eIGS_ThirdPerson: eIGS_FirstPerson;
...
void CSingle::GetSkipEntities(CWeapon* pWeapon, PhysSkipList& skipList)
{
	...
	if (CActor *pActor = pWeapon->GetOwnerActor())
	{
		...
		if (IVehicle* pVehicle = pActor->GetLinkedVehicle())
		{
			...
			i32 childCount = pVehicleEntity->GetChildCount(); 
...
Vec3 CSingle::GetProbableHit(float maxRayLength, bool *pbHit, ray_hit *pHit) const
{
	...
	if (requiresProbableHitTest)
	{
		...
		u32 flags=(geom_colltype_ray|geom_colltype13)<<rwi_colltype_bit|rwi_colltype_any|rwi_force_pierceable_noncoll|rwi_ignore_solid_back_faces;
		u8 pierceability = 8;
...
void CSingle::DeferGetProbableHit(float maxRayLength)
{
	...
	for(i32 i=0; i < MAX_PROBABLE_HITS; i++)
	{
	...
	if(probableHit)	// maybe too many shots during a client's loading screen?
	{
		...
		DRX_ASSERT(probableHit->m_queuedRayID == 0);
		DRX_ASSERT(m_queuedProbableHits.size() < m_queuedProbableHits.max_size());
...
//------------------------------------------------------------------------
Vec3 CSingle::ApplySpread(const Vec3 &dir, float spread, i32 quadrant) const
{
	...
	if(quadrant < 0)
	{
		...
		rx = drx_random(-0.5f, 0.5f);
		rz = drx_random(-0.5f, 0.5f);
	}
	else
	{
		CCCPOINT(Single_ApplySpreadQuadrant);
		DRX_ASSERT_MESSAGE(quadrant < 4, "Invalid quadrant provided to apply spread");

		rx = drx_random(0.0f, 0.5f);
		rz = drx_random(0.0f, 0.5f);
...
Vec3 CSingle::GetTracerPos(const Vec3 &firingPos, const STracerParams* useTracerParams)
{
	i32 id=m_pWeapon->GetStats().fp?0:1;
	i32 slot=id ? eIGS_ThirdPerson : eIGS_FirstPerson;
	tukk  helper = useTracerParams->helper[id].c_str();
...
void CSingle::SmokeEffect(bool effect)
{
	...
	{		
		...
		i32 slot = m_pWeapon->GetStats().fp ? eIGS_FirstPerson : eIGS_ThirdPerson;
		i32 id = m_pWeapon->GetStats().fp ? 0 : 1;
...
{ 
	...
	if (attach)
	{
		...
		m_spinUpTimer = (u32)(m_fireParams->spinup.time[id]);
	}
}

//------------------------------------------------------------------------
i32 CSingle::GetDamage() const
{
	CActor* pActor = m_pWeapon->GetOwnerActor();
	const bool playerIsShooter = pActor ? pActor->IsPlayer() : false;

	i32 damage = m_fireParams->fireparams.damage;
	if (!playerIsShooter)
	...
	if (canOvercharge)
	{
		...
		damage = (i32)(damage * overchargeModifier);
	}
...
bool CSingle::DoesFireRayIntersectFrustum(const Vec3& vPos, bool& firePosInFrustum)
{
	...
	else
	{
		...
		{
			for(i32 p=0; p<planeIntersectCount; p++)
			{
				for(i32 c=0; c<planeIntersectCount; c++)
				{
					if(c!=p)
					{
...
void CSingle::NetShoot(const Vec3 &hit, i32 predictionHandle)
{
...
//------------------------------------------------------------------------
void CSingle::NetShootEx(const Vec3 &pos, const Vec3 &dir, const Vec3 &vel, const Vec3 &hit, float extra, i32 predictionHandle)
{
	...
	i32 inventoryAmmoCount = m_pWeapon->GetInventoryAmmoCount(ammo);
	i32 ammoCount					= (clipSize == 0) ? inventoryAmmoCount : weaponAmmoCount;
...
void CSingle::ReplayShoot()
{
	...
	if(DoesFireRayIntersectFrustum(pos, firePosInFrustum))
	{
		...
		if (firePosInFrustum || DoesFireLineSegmentIntersectFrustum(pos, hit))
		{
			...
			{
				if(drx_random(0, tracerParams->frequency - 1) == 0)
				{
...
void CSingle::CheckNearMisses(const Vec3 &probableHit, const Vec3 &pos, const Vec3 &dir, float range, float radius)
{
	...
	if (pAISystem && (ownerId != 0) && !GetShared()->fireparams.is_silenced)
	{
		...
		SAIStimulus stim(AISTIM_DRX3D_WHIZZ, 0, ownerId, 0, pos, dir, range);
...
void CSingle::GetMemoryUsage(IDrxSizer * s) const
{	
	s->AddObject(this, sizeof(*this));
	GetInternalMemoryUsage(s);
}

void CSingle::GetInternalMemoryUsage(IDrxSizer * s) const
{
...
void CSingle::EmitTracer(const Vec3& pos, const Vec3& destination, const STracerParams * useTracerParams, CProjectile* pProjectile)
{
	...
	i32k tracerIdx = g_pGame->GetWeaponSystem()->GetTracerUpr().EmitTracer(params, pProjectile ? pProjectile->GetEntityId() : 0);
...
void CSingle::OnOutOfAmmo(const CActor* pActor, bool autoReload)
{
	...
	{
		m_pWeapon->SetBusy(true);
		i32k curTime = (i32)(m_pWeapon->GetParams().autoReloadDelay * 1000.0f);
