...
#include <CooperativeAnimationUpr/CooperativeAnimationUpr.h>
...
#include <drx3D/Game/ActionCoopAnim.h>
...
#define HitDeathLog(...) { DrxLogAlways("[HITDEATHREACTIONS]" __VA_ARGS__); }
#else
#define HitDeathLog(...) {}
...
namespace
{
	...
	u32k INVALID_FACIAL_CHANNEL_ID = ~0;

	u32k ANIM_USER_TOKEN = 'HdRA';
}
...
public:
	...
	virtual EStatus UpdatePending(float timePassed) override
	{
		...
		if( m_rootScope && !m_isForced )
		{
			...
			u32k numMatchingTags = m_rootScope->GetDatabase().FindBestMatchingTag( SFragmentQuery(m_fragmentID, fragTagState), &matchedTagState );
...
struct CHitDeathReactions::SPredFindValidReaction : public std::unary_function<bool, const SReactionParams&>
{
	...
	bool operator ()(const SReactionParams& reactionParams) const
	{
		...
		if( bFound && gEnv->bMultiplayer && reactionParams.mannequinData.actionType == SReactionParams::SMannequinData::EActionType_Coop )
		{
			...
			{
				SFragTagState matchedTagState; 
				u32k numAnimsPotential = piOptionalAnimationADB->FindBestMatchingTag( SFragmentQuery(PlayerMannequin.fragmentIDs.hitDeath, fragTagState), &matchedTagState );
			...
			if( numAnims )
			{
				reactionParams.mannequinData.m_animIndexMP = static_cast<i32>(m_owner.GetRandomProbability()*100.f) % numAnims;
			}
		}
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactions::SCustomAnim::SetAnimName(i32 animID, IAnimationSet* pAnimSet)
{
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
inline void AnimIDError_Dup(tukk animName)
{
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactions::RequestReactionAnims(u32 requestFlags)
{
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactions::ReleaseReactionAnims(u32 requestFlags)
{
...
void CHitDeathReactions::Update(float fFrameTime)
{
	...
	if (IsInReaction())
	{
		...
		if (eAIP_PipeUserPaused == m_AIPausedState)
		{
			...
			DRX_ASSERT_TRACE(!pPipeUser || pPipeUser->IsPaused(), ("The actor %s pipe user has been resumed before the reaction has ended! Investigate", m_actor.GetEntity()->GetName()));
		}
#endif
...
FragmentID CHitDeathReactions::GetHitDeathFragmentID( const IActionController* piActionController )
{
	...
	u32k FRAGID_HITDEATH = CCrc32::ComputeLowercase( "hitDeath" );
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
bool CHitDeathReactions::OnReaction(const HitInfo& hitInfo, i32* pAnimIndex)
{
	FUNCTION_PROFILER(gEnv->pSystem, PROFILE_GAME);
...
bool CHitDeathReactions::OnHit(const HitInfo& hitInfo, float fCausedDamage/* = 0.0f*/)
{
	...
	{
		// Generate and set seed for this event
		u32 uSeed = GetSynchedSeed(false);
		m_pseudoRandom.seed(gEnv->bNoRandomSeed?0:uSeed);
...
bool CHitDeathReactions::OnAnimationEvent(const AnimEventInstance &event)
{
	...
	if (IsInReaction())
	{
		if (m_actor.GetAnimationEventsTable().m_deathReactionEndId == event.m_EventNameLowercaseCRC32)
		{
			...
			// thread has the lock (inside DrxAction::PostUpdate)
		...
		{
			DRX_ASSERT_MESSAGE(m_actor.GetAnimatedCharacter(), "This class assumes that if this code is called, the actor has a valid animated character");
...
bool CHitDeathReactions::IsValidReaction(const HitInfo& hitInfo, const SReactionParams::SValidationParams& validationParams, float fCausedDamage) const
{
	...
	IF_UNLIKELY (bMinDistanceCheck || bMaxDistanceCheck)
	{
		...
		{
#ifndef _RELEASE
			DRX_ASSERT_MESSAGE(false, "Couldn't find shooter entity while checking distance to shooter criteria");
	...
	DRX_ASSERT_TRACE(allowedStances.empty() || (m_actor.GetStance() != STANCE_NULL), ("%s's current stance is NULL! This actor state is probably incoherent!", m_actor.GetEntity()->GetName()));
...
{
	DRX_ASSERT((reactionType == eRT_Death) || (reactionType == eRT_Hit) || (reactionType == eRT_Collision));
	...
	{
		tukk szDefaultReactionFnc = (reactionType == eRT_Hit) ? DEFAULT_HIT_REACTION_FUNCTION : DEFAULT_KILL_REACTION_FUNCTION;
		...
		if (!bSuccess)
		{
			DRX_ASSERT_MESSAGE(!g_pGameCVars->g_hitDeathReactions_useLuaDefaultFunctions, "Can't run default hit reaction lua method. Check HitDeathReactions.lua");
...
bool CHitDeathReactions::IsValidReaction(const HitInfo& hitInfo, const ScriptTablePtr pScriptTable, float fCausedDamage) const
{
	...
	if (pScriptTable->GetValue(REACTION_ID, reactionId) && IsValidReactionId(reactionId))
	{
		...
		i32 iValidationIdx = -1;
		if (pScriptTable->GetValue(VALIDATION_ID, iValidationIdx))
		{
			DRX_ASSERT((iValidationIdx >= 0) && (iValidationIdx < static_cast<i32>(reactionParams.validationParams.size())));

			// The table is the validationParams table
			if ((iValidationIdx >= 0) && (iValidationIdx < static_cast<i32>(reactionParams.validationParams.size())))
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
bool CHitDeathReactions::StartReactionAnim(const string& sAnimName, bool bLoop/* = false*/, float fBlendTime/* = 0.2f*/, i32 iSlot/* = 0*/, i32 iLayer/* = 0*/, u32 animFlags /*= (DEFAULT_REACTION_ANIM_FLAGS)*/, float fAdditiveWeight/* = 0.0f*/, float fAniSpeed/* = 1.0f*/, bool bNoAnimCamera/*=false*/)
{
	bool bSuccess = false;
	...
	if (pAnimSet)
	{
		i32 animID = pAnimSet->GetAnimIDByName(sAnimName.c_str());
		if (animID >= 0)
		{
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
bool CHitDeathReactions::StartReactionAnimByID(i32 animID, bool bLoop/* = false*/, float fBlendTime/* = 0.2f*/, i32 iSlot/* = 0*/, i32 iLayer/* = 0*/, u32 animFlags /*= (DEFAULT_REACTION_ANIM_FLAGS)*/, float fAdditiveWeight/* = 0.0f*/, float fAniSpeed/* = 1.0f*/, bool bNoAnimCamera/*=false*/)
{
	DRX_ASSERT(fAniSpeed > 0.0f);

	DRX_ASSERT_MESSAGE(IsInReaction(), "This method should be called only during a hit or death reaction");
	if (!IsInReaction())
	{
	...
	if (pCharacter)
	{
		...
		DRX_ASSERT_MESSAGE(pAnimatedCharacter, "This class assumes that if this code is called, the actor has a valid animated character");
		...
		if (pauseActionController)
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Trying to play a fullbody animation hit death reaction. This is not supported!");
...
bool CHitDeathReactions::IsPlayingReactionAnim() const
{
	...
	if (IsExecutionType(eET_ReactionAnim))
	{
		...
		else
		{
			...
			i32k iNumAnims = pSkel ? pSkel->GetNumAnimsInFIFO(m_currentCustomAnim.iLayer) : 0;
			if (iNumAnims > 0)
			{
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactions::GetMemoryUsage( IDrxSizer *pSizer ) const
{
...
{
	DRX_ASSERT(CanPlayDeathReaction() || hitInfo.forceLocalKill);
...
bool CHitDeathReactions::CanPlayHitReaction() const
{
	...
	return (g_pGameCVars->g_hitDeathReactions_enable != 0) && 
		...
		!gEnv->pGame->GetIGameFramework()->GetICooperativeAnimationUpr()->IsActorBusy(m_actor.GetEntityId()) && // Not in a cooperative animation
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
bool CHitDeathReactions::SendAISignal(tukk szSignal, IAISignalExtraData *pData, bool bWaitOpOnly) const
{
	DRX_ASSERT(szSignal && szSignal[0]);
...
bool CHitDeathReactions::ExecuteReactionCommon(const SReactionParams& reactionParams)
{
	...
	{
		DrxFatalError("HitDeathReactions: AnimationGraph Reactions are not supported anymore");
	}
	else if (!reactionParams.reactionAnim->animCRCs.empty())
	{
		...
		if (pAnimSet)
		{
			...
			i32 animID = reactionAnim.GetNextReactionAnimId(pAnimSet);
			if (animID >= 0)
			{
		...
		case SReactionParams::SMannequinData::EActionType_Fragment:
			{
				...
				DRX_ASSERT( reactionParams.mannequinData.IsCurrentFragmentLoaded() );

				if( m_currentReaction == eRT_Collision )
				{
					u32 optionIndex = m_reactionOnCollision - 1;
					...
					case eET_Custom: 
						{
							...
							DRX_ASSERT(IsValidReactionId(reactionId));
...
void CHitDeathReactions::PausePipeUser(bool bPause)
{
	if(!gEnv->bMultiplayer)
	{
		...
		DRX_ASSERT_MESSAGE(eAIP_ExecutingCustomSignal != m_AIPausedState, "CHitDeathReactions::PausePipeUser Attempting to pause while the current pause state is 'eAIP_ExecutingCustomSignal'. This must not happen!");
...
void CHitDeathReactions::LoadData(SmartScriptTable pSelfTable, bool bForceReload)
{
	...
	if (m_pScriptSystem->GetGlobalValue(HIT_DEATH_REACTIONS_SCRIPT_TABLE, aux))
	{
		...
		DRX_ASSERT(m_pHitReactions && m_pDeathReactions && m_pCollisionReactions && m_pHitDeathReactionsConfig);
	}
	else
...
void CHitDeathReactions::OnCustomAnimFinished(bool bInterrupted)
{
	...
	DRX_ASSERT(IsExecutionType(eET_ReactionAnim));
	SetExecutionType(eET_None);
...
{
	DRX_ASSERT(!m_actor.IsThirdPerson());
	...
	{
		DRX_ASSERT(CPlayer::GetActorClassType() == m_actor.GetActorClass());
...
void CHitDeathReactions::DoCollisionCheck()
{
	...
	i32k collisionEntityTypes = ent_static | ent_terrain | ent_ignore_noncolliding;
...
void CHitDeathReactions::StartCollisionReaction(const Vec3& vNormal, const Vec3& vPoint)
{
	...
	else
	{
		if (m_reactionOnCollision != 1)
		{
			...
			DRX_ASSERT_MESSAGE(vPlainNormal.IsValid() && !vPlainNormal.IsZero(), "Vertical normals shouldn't validate a Collision Reaction Investigate!");
		...
		DRX_ASSERT(m_reactionOnCollision != NO_COLLISION_REACTION);
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
u32 CHitDeathReactions::GetSynchedSeed(bool bKillReaction) const
{	
	...
	CCrc32::Compute(m_actor.GetEntity()->GetName()) + static_cast<u32>(m_actor.GetHealth() * 100.f);
}
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactions::StopHigherLayers(i32 iSlot, i32 iLayer, float fBlendOut)
{
	if (iLayer < (numVIRTUALLAYERS - 1))
	{
		...
		if (pISkeletonAnim)
		{
			for (i32 i = iLayer + 1; i < numVIRTUALLAYERS; ++i)
			{
...
bool CHitDeathReactions::IsValidReactionId(ReactionId reactionId) const
{
	...
	if (reactionId < 0)
	{
		i32 deathReactionsSize = static_cast<i32>(m_pDeathReactions->size());
	...
	return ((absReactionId <= static_cast<i32>(pReactionList->size())) && 
					(!pReactionList->empty()));
}
...
{
	DRX_ASSERT_MESSAGE(IsValidReactionId(reactionId), "this method shouldn't be called if IsValidReactionId is false");
...
string CHitDeathReactions::GetCurrentAGReactionAnimName() const
{
	...
	if (pMainChar)
	{
		...
		{
			i32 iNumAnims = pMainChar->GetISkeletonAnim()->GetNumAnimsInFIFO(eAnimationGraphLayer_FullBody);
			if (iNumAnims > 0)
		...
		if (pAnim)
		{
			...
			i32 topAnimId = anim.GetAnimationId();
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
bool CHitDeathReactions::StartFacialAnimation(ICharacterInstance* pCharacter, tukk szEffectorName, float fWeight, float fFadeTime)
{
	DRX_ASSERT(pCharacter);
...
{
	DRX_ASSERT(m_actor.GetGameObject()->GetAspectProfile(eEA_Physics) == eAP_Ragdoll);
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
u32 CHitDeathReactions::GetRootScopeID( const IActionController* piActionController, const FragmentID fragID, const TagState hitDeathFragTagState ) const
{
...
{
	u32k rootScopeID = GetRootScopeID( piActionController, fragID, hitDeathFragTagState );
	...
	DRX_ASSERT( pHitDeathFragTagDef );
	...
	for( TagID i = 0; i < pLastFragTagDef->GetNum(); ++i )
	{
		...
		u32k tagCRC = pLastFragTagDef->GetTagCRC( i );
		DRX_ASSERT( tagCRC );
...
{
	const CTagDefinition* pTagDefinition = gEnv->pGame->GetIGameFramework()->GetMannequinInterface().GetAnimationDatabaseUpr().LoadTagDefs( "Animations/Mannequin/ADB/hitDeathTags.xml", true );
	
	if (!pTagDefinition)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Missing Animations/Mannequin/ADB/hitDeathTags.xml tag definition file");
		return;
	}
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Could not generate \"post_death\" mannequin tags");
	}
}
...
CHitDeathReactionsPhysics::~CHitDeathReactionsPhysics()
{
	for (i32 i = 0; i < kMaxQueuedPrimitives; ++i)
	{
...
{
	DRX_ASSERT(m_pOwnerHitReactions == NULL);
	DRX_ASSERT(pOwner != NULL);
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void CHitDeathReactionsPhysics::Queue( i32 primitiveType, primitives::primitive &primitive, const Vec3 &sweepDir, i32 objTypes, i32 geomFlags )
{
	DRX_ASSERT(m_pOwnerHitReactions);

	i32 slot = GetPrimitiveRequestSlot();
...
void CHitDeathReactionsPhysics::IntersectionTestComplete( const QueuedIntersectionID& intID, const IntersectionTestResult& result )
{
	...
	i32 slot = GetSlotFromPrimitiveId(intID);
	DRX_ASSERT(slot != -1);
	...
	DRX_ASSERT_MESSAGE(result.normal.IsValid() && !result.normal.IsZero() && result.point.IsValid(), "Receiving incorrect data from physics events, check CPhysicalWorld::TracePendingRays method");
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
i32 CHitDeathReactionsPhysics::GetPrimitiveRequestSlot()
{
	for (i32 i = 0; i < kMaxQueuedPrimitives; ++i)
	{
	...
	u32 oldestCounter = m_requestCounter + 1;
	i32 oldestSlot = 0;
	for (i32 i = 0; i < kMaxQueuedPrimitives; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
i32 CHitDeathReactionsPhysics::GetSlotFromPrimitiveId( const QueuedIntersectionID& intID )
{
	for (i32 i = 0; i < kMaxQueuedPrimitives; ++i)
	{
