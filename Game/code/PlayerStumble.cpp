...
#include <drx3D/Game/PlayerStumble.h>
...
bool CPlayerStumble::Update(float deltaTime, const pe_status_dynamics &dynamics)
{
  ...
  if ( m_currentPastTime > m_stumbleParameter.periodicTime )
  {
    ...
    m_currentPeriodicScaleX = 1.0f - drx_random(0.0f, m_stumbleParameter.randomness);
    m_currentPeriodicScaleY = 1.0f - drx_random(0.0f, m_stumbleParameter.randomness);
  }
  m_finalActionImpulse = -drx_random(m_stumbleParameter.minDamping, m_stumbleParameter.maxDamping) * dynamics.v;
...
void CPlayerStumble::Start(StumbleParameters* stumbleParameters)
{
  ...
  m_currentPeriodicScaleX = 1.0f - drx_random(0.0f, stumbleParameters->randomness);
  m_currentPeriodicScaleY = 1.0f - drx_random(0.0f, stumbleParameters->randomness);
}
