...
#include <drx3D/Game/Utility/DrxWatch.h>
	...
	#define PTLogThrow(...)		{if(g_pGameCVars->pl_pickAndThrow.chargedThrowDebugOutputEnabled){DrxLogAlways("[PICKTHROW] [THROW] " __VA_ARGS__);}}
	#define PTLogCombo(...)		{if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponComboDebugEnabled){DrxLogAlways("[PICKTHROW] [COMBO] " __VA_ARGS__);}}
#else
	#define PTLogThrow(...)		{}
	#define PTLogCombo(...)		{}
...
{
	DelayedMeleeImpulse(CPickAndThrowWeapon& pickAndThrow, EntityId collidedEntityId, const Vec3& impulsePos, const Vec3& impulse, i32 partId, i32 ipart, i32 hitTypeID)
...
public:
	...
	virtual void OnEnter(float blendTime, float duration, const SAttachPnTParams &params)
	{
		...
		if (!pActor)
		{
			...
			IAttachment *pAttachment = m_pAttachment = pCharacter->GetIAttachmentUpr()->GetInterfaceByName(params.attachmentPt.c_str());
...
, m_logicFlags(ELogicFlag_None)
{
	...
	gEnv->pEntitySystem->GetIEntityPoolUpr()->AddListener( this, "PickAndthrowWeapon", IEntityPoolListener::EntityReturnedToPool );
}
...
//---------------------------------------------------------------------------
tukk CPickAndThrowWeapon::GetWeaponComponentType()
{
	return "CPickAndThrowWeapon";
}

//---------------------------------------------------------------------------
tukk CPickAndThrowWeapon::GetType() const
{
			...
			if (IsNPC())
			{
				...
				if (pTargetActor)
				{
					...
					if (!GetGrabTypeParams().dbaFile.empty())
					{
						...
						pAnimDB = mannequinSys.GetAnimationDatabaseUpr().Load(GetGrabTypeParams().dbaFile.c_str());
					}
...
void CPickAndThrowWeapon::OnSelected(bool selected)
{
	...
	if(pTargetEntity != NULL)
	{
		...
		if(EntityScripts::GetEntityProperty(pTargetEntity, "szGrabAnimTagOverride", pGrabTagOverride) && strlen(pGrabTagOverride) > 0)
		{
			if(pActionController)
			{
				...
				u32 grabTypeCRC  = CCrc32::ComputeLowercase(pGrabTagOverride);
	...
	tukk pRootedGrabTagOverride = NULL; 
	if( pTargetEntity != NULL )
	{
	...
	{
		CTagState &tagState = pActionController->GetContext().state;
		u32 grabTypeCRC  = CCrc32::ComputeLowercase(pRootedGrabTagOverride);
		tagState.SetByCRC(grabTypeCRC, selected);
	}
	...
	if (selected)
	{
		...
		if (pEnvWeapon)
		{
			...
			if( pTracker )
			{
				i16 health = (i16)pEnvWeapon->GetCurrentHealth();
...
{
	i32k numTypes = m_weaponsharedparams->pPickAndThrowParams->grabTypesParams.size();
	i32k activeSlot = PickAndThrow::FindActiveSlot( objectId );
...
void CPickAndThrowWeapon::UpdateTags(const class IActionController *pActionController, class CTagState &tagState, bool selected) const
{
	...
	u32 tagCrC = CCrc32::ComputeLowercase(GetGrabTypeParams().tag.c_str());
...
//---------------------------------------------------------------------------
void CPickAndThrowWeapon::OnAction(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
	CGameInputActionHandlers::TPickAndThrowWeaponActionHandler& pickAndThrowActionHandler = g_pGame->GetGameInputActionHandlers().GetCPickAndThrowWeaponActionHandler();
...
//---------------------------------------------------------------------------
bool CPickAndThrowWeapon::OnActionAttack(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
	if (m_state != eST_IDLE)
		return false;
...
//---------------------------------------------------------------------------
bool CPickAndThrowWeapon::OnActionUse(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{	
	CPlayer* pOwnerPlayer = GetOwnerPlayer();
...
//---------------------------------------------------------------------------
bool CPickAndThrowWeapon::OnActionDrop(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
...
void CPickAndThrowWeapon::PerformMelee(i32 activationMode,const bool bForcePerformMelee /*= false*/)
{
	// Cache these
	const SPickAndThrowParams::SGrabTypeParams& grabParams = GetGrabTypeParams(); 
	...
	else
	{
		...
		if (performMelee || bForcePerformMelee)
		{
			...
			{
				m_state = eST_MELEE;
				GetScheduler()->TimerAction((u32)(grabParams.melee_delay * 1000.0f), CSchedulerAction<DoSimpleMelee>::Create(DoSimpleMelee(*this)), false);
				GetScheduler()->TimerAction((u32)(grabParams.meleeAction_duration * 1000.0f), CSchedulerAction<FinishSimpleMelee>::Create(FinishSimpleMelee(*this)), false);
			}
...
//---------------------------------------------------------------------------
bool CPickAndThrowWeapon::OnActionMelee(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
	if( CEnvironmentalWeapon* pEnvWeapon = PickAndThrow::GetEnviromentalWeaponForEntity( m_objectId ) )
	{
		if(m_postMeleeActionCooldown <= 0.0f && activationMode == eAAM_OnPress)
		{
			...
			i32k size = grabParams.m_meleeComboList.size(); 
...
bool CPickAndThrowWeapon::OnActionChargedThrow(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
...
//---------------------------------------------------------------------------
void CPickAndThrowWeapon::Update(SEntityUpdateContext& ctx, i32 val )
{
	...
	switch (m_state)
	{
		...
		case eST_MELEE:
		{
			...
			{
				g_pGame->GetAutoAimUpr().SetCloseCombatSnapTarget(m_currentMeleeAutoTargetId, 
	...
	if (g_pGameCVars->pl_pickAndThrow.chargedThrowDebugOutputEnabled==1)
	{
		...
		DrxWatch(" ChargedThrowState = %s", stateName.c_str());
	}
	...
	if(m_currentMeleeAutoTargetId)
	{
		...
		if(pTarget && !pTarget->IsDead())
		{
			...
			if(lerpTargetSelectRange > 0.0f) // If we are too far from target with this attack.. we don't want lerp.. abort here. 
			{
				...
				{
					// Request the lerp
					g_pGame->GetAutoAimUpr().SetCloseCombatSnapTarget(m_currentMeleeAutoTargetId, lerpEnd, g_pGameCVars->pl_pickAndThrow.complexMelee_lerp_target_speed);
...
{
	DRX_ASSERT( desiredDuration > 0.0f ); 
...
{
	DRX_ASSERT_MESSAGE(pPlayer, "CPickAndThrowWeapon::PerformCameraShake(...) < error pPlayer is NULL");
	if(camShake.enabled && camShake.time > 0.0f)
	{
		CScreenEffects* pScreenFX = g_pGame->GetScreenEffects();
		DRX_ASSERT_MESSAGE(pScreenFX, "CPickAndThrowWeapon::PerformCameraShake(...) < error pScreenFX is NULL");
		...
		bool changeCamDir = camShake.randomDirection && drx_random(0, 1);
		if (changeCamDir)
		{
			shift.x = -shift.x;
			rotate.y = -rotate.y;
...
void CPickAndThrowWeapon::ResetTrackingOfExternalEntities()
{
	...
	// flying objects
	for (u32 i=0; i<m_infiniteViewEntities.size(); ++i)
	{
		...
		{							
			...
			if(m_state != eST_CHARGED_THROW_POST_RELEASE)
			{
				if(!gEnv->bMultiplayer)
				{
					...
					finalW = finalW.CompMul(drx_random_componentwise(Vec3(0.5f), Vec3(1.3f)));
...
Vec3 CPickAndThrowWeapon::CalculateChargedThrowDir(const Matrix34& attackerTransform, const Vec3& throwPos) const
{
	...
	if(pPlayer && pPlayer->IsClient() && g_pGameCVars->pl_pickAndThrow.chargedthrowAutoAimEnabled)
	{
		...
		if(pTargetPlayer)
		{
			const SAutoaimTarget* pAutoAimInfo = g_pGame->GetAutoAimUpr().GetTargetInfo(pTargetPlayer->GetId());
			if(pAutoAimInfo)
			{
	...
	if( !foundTarget )
	{
		...
		else
		{
			...
			{
				DrxLog( "PickAndThrow:: Panic no eyes!" );
...
IEntity* CPickAndThrowWeapon::CalculateBestChargedThrowAutoAimTarget(const Matrix34& attackerTransform) const
{
	...
	const TAutoaimTargets& players = g_pGame->GetAutoAimUpr().GetAutoAimTargets();
	i32k targetCount = players.size();
		...
		if(pEntity && PickAndThrow::AllowedToTargetPlayer(GetOwnerId(),targetEntityId))
		{
			...
			if(toTarget.GetLengthSquared() < autoAimDistSqrd)
			{
				...
				if(PickAndThrow::TargetEntityWithinFrontalCone(attackerPos,targetTestPos, attackerFacing,coneSizeRads, theta))
				{
					...
					if(g_pGameCVars->pl_pickAndThrow.chargedThrowDebugOutputEnabled)
					{
						DrxWatch("Entity [%d] DistScore [%.2f] , ConeScore[%.2f], totalScore [%.3f]", pEntity->GetId(), targetDistScore, targetConeScore, finalScore); 
...
//static---------------------------------------------------------------------------
void CPickAndThrowWeapon::OnThrownObjectSeen(const Agent& agent, EntityId objectId, uk pVPickAndThrowWeapon)
{
	...
	ITargetTrackUpr *pUpr = pAISystem ? pAISystem->GetTargetTrackUpr() : NULL;
	if (pUpr && agent.IsValid() && pPlayerAI)
	{
		...
		{
			i32k alertness = pTargetAIProxy->GetAlertnessState();
			bAlert = (alertness >= 2);
		}
		...
		pUpr->HandleStimulusEventForAgent(aiTargetId, aiPlayerId, "SeeThrownObject", eventInfo);
...
//---------------------------------------------------------------------------
IAttachment* CPickAndThrowWeapon::GetOwnerAttachment(i32k slotIndex) const
{
	...
	if (pEntity && pEntity->GetSlotInfo(slotIndex, info))
	{
		if (info.pCharacter)
		{
			pAttachment = info.pCharacter->GetIAttachmentUpr()->GetInterfaceByName( GetGrabTypeParams().attachment.c_str() );
		}
	}
...
//---------------------------------------------------------------------------
void CPickAndThrowWeapon::SetViewMode(i32 mode)
{
	if(m_objectId)
	{
...
{
	i32k mainCharacterSlot = 0;
	i32k shadowCharacterSlot = 5;
...
{
	assert( (i32)m_weaponsharedparams->pPickAndThrowParams->grabTypesParams.size()>m_grabType );
...
{
	i32 numTypes = m_weaponsharedparams->pPickAndThrowParams->grabTypesParams.size();
...
void CPickAndThrowWeapon::OnSuccesfulHit( const ray_hit& hitResult )
{
	...
	i32 hitTypeID = DoSimpleMeleeHit(hitResult, targetId);
...
//---------------------------------------------------------------------------
i32 CPickAndThrowWeapon::DoSimpleMeleeHit( const ray_hit& hitResult, EntityId collidedEntityId )
{
	// generate the damage
	CActor *pOwnerActor = GetOwnerActor();
	i32 hitTypeId = 0;
	...
	if (pOwnerActor)
	{
		...
		if (IAIObject* pOwnerAI = pOwnerEntity->GetAI())
		{
			...
			// Send target stimuli
			IAISystem *pAISystem = gEnv->pAISystem;
			ITargetTrackUpr *pTargetTrackUpr = pAISystem ? pAISystem->GetTargetTrackUpr() : NULL;
			if (pTargetTrackUpr && pTargetAI)
			{
				...
				pTargetTrackUpr->HandleStimulusEventForAgent(aiTargetId, aiOwnerId, "MeleeHit", eventInfo);
				pTargetTrackUpr->HandleStimulusEventInRange(aiOwnerId, "MeleeHitNear", eventInfo, 5.0f);
			}
		}
		...
		if(!isFriendlyHit)
		{
			...
			if(pTarget)
			{
				...
				info.knocksDown = (drx_random(20.0f, 50.0f) < m_objectMass);
...
//---------------------------------------------------------------------------
void CPickAndThrowWeapon::DoSimpleMeleeImpulse( const ray_hit& hitResult, EntityId collidedEntityId, i32 hitTypeID )
{
...
//---------------------------------------------------------------------------
void CPickAndThrowWeapon::PlayCommunication( tukk pCommName, tukk pChannelName )
{
	IEntity* pEntity = GetEntityObject();
	if (!pEntity)
		return;
	...
	{
		if (ICommunicationUpr* pCommUpr = pAISystem->GetCommunicationUpr())
		{
			SCommunicationRequest request;

			request.actorID = pEntity->GetId();
			request.channelID = pCommUpr->GetChannelID(pChannelName);
			request.commID = pCommUpr->GetCommunicationID(pCommName);
			request.configID = pCommUpr->GetConfigID(pAIProxy->GetCommunicationConfigName());
			request.contextExpiry = 0;
			request.ordering = SCommunicationRequest::Ordered;
			pCommUpr->PlayCommunication(request);
		}
	}
}
...
//////////////////////////////////////////////////////////////////////////
void CPickAndThrowWeapon::PlayActionLocal( const ItemString& actionName, bool loop, u32 flags, const float speedOverride)
{
...
bool CPickAndThrowWeapon::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
	if(aspect == ASPECT_STREAM)
	{
		NET_PROFILE_SCOPE("P&T WeaponStream", ser.IsReading());

		i32 previousThrowCount = m_throwCounter;
		ser.Value("throwCount", m_throwCounter, 'ui2');

		EState prevThrowState = m_RequestedThrowState;

		i32 tempcurrStateHolder = m_RequestedThrowState; 
	...
	else if(aspect == ASPECT_MELEE)
	{
		...
		i32 previousMeleeCount = m_meleeCounter;
...
void CPickAndThrowWeapon::OnDoComplexMelee(const bool bScheduleFinishComplexMelee)
{
	...
	{
		GetScheduler()->TimerAction((u32)(m_animDuration * 1000.0f), CSchedulerAction<FinishComplexMelee>::Create(FinishComplexMelee(*this)), false);
	}
...
tukk CPickAndThrowWeapon::GetGrabHelperName() const
{
	const SPickAndThrowParams::SGrabTypeParams& grabParams = GetGrabTypeParams(); 
...
{
	DRX_ASSERT(GetOwnerPlayer() && GetOwnerPlayer()->IsClient());
	SHUDEventWrapper::ForceCrosshairType( this, (ECrosshairTypes)GetGrabTypeParams().crossHairType );
}
...
void CPickAndThrowWeapon::DebugDraw()
{
	...
	// simple history debug code
	{
		...
		for (i32 i=0; i<s_stateListUsed; ++i)
		{
	...
	if (IsNPC() && GetNPCGrabbedActor())
	{
		...
		if (pPlayerCharacter && pNPCCharacter)
		{
			...
			tukk pPlayerAnimName = pPlayerCharacter->GetIAnimationSet()->GetNameByAnimID( playerAnim.GetAnimationId() );
			tukk pNPCAnimName = pNPCCharacter->GetIAnimationSet()->GetNameByAnimID( NPCAnim.GetAnimationId() );

			const float ANIMFPS = 30.f;
			i32 playerFrame = pos_round( (playerAnim.GetCurrentSegmentNormalizedTime() * playerAnim.GetCurrentSegmentExpectedDurationSeconds()) / (1/ANIMFPS) );
			i32 NPCFrame = pos_round( (NPCAnim.GetCurrentSegmentNormalizedTime() * NPCAnim.GetCurrentSegmentExpectedDurationSeconds()) / (1/ANIMFPS) );
	...
	if (pObject)
	{
		...
		tukk pHelperName = "No Helper";
		float textColor[] = {1.0f,0.0f,0.0f,1.0f};

		// hits to drop npc		
		if (IsNPC())
		{
			i32 hitsToDropNPC = GetGrabTypeParams().shieldingHits - m_hitsTakenByNPC;
