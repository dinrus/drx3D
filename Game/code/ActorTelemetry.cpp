...
#if USE_POSITION_COMPRESSION
// #if defined(_DEBUG)
// 		DrxLog("Flushing actor telemetry, added %d points, wrote %d points, reduction of %.2f%%",
	...
	if(tracker && pOwner && pMgr)
	{
		...
		if (pMgr->ShouldRecordEvent(eSE_LookRotation, pOwnerRaw))
		{
			if ( pInput && TimePassedCheck(m_lastLookRotationTime, time, LOOKROT_DUMP_INTERVAL))
			{
				...
				//DrxLogAlways("CActorTelemetry::Update() look=%f, %f, %f; angle=%f (RAD2DEG:%f)", look.x, look.y, look.z, angle, RAD2DEG(angle));
...
void CActorTelemetry::SubscribeToWeapon(EntityId weaponId)
{
	...
	if(pTracker && pMgr)
	{
		...
		if (pWeapon)
		{
			...
			{
				tukk bottomAttachment = "";
				tukk barrelAttachment = "";
				tukk scopeAttachment = "";
...
void CActorTelemetry::OnEndReload(IWeapon *pWeapon, EntityId shooterId, IEntityClass* pAmmoType)
{
	...
	DRX_ASSERT(pActor && shooterId == pActor->GetEntityId());
																 ...
																 const Vec3 &pos, const Vec3 &dir, const Vec3 &vel)
{
	...
	if ( IStatsTracker *tracker = GetStatsTracker() )
	{
		i32				fmidx=pWeapon->GetCurrentFireMode();
...
void CActorTelemetry::OnFireModeChanged(IWeapon *pWeapon, i32 currentFireMode)
{
	CActorPtr pActor = m_pOwner.lock();

	DRX_ASSERT(pActor && static_cast<CWeapon*>(pWeapon)->GetOwnerId() == pActor->GetEntityId());		// is this cast always safe?
...
#ifdef DEBUG
	CActorPtr pActor = m_pOwner.lock();
	DRX_ASSERT(pActor && actorId == pActor->GetEntityId());
#endif
...
void CActorTelemetry::OnPlayerAction(const ActionId &action, i32 activationMode, float value)
{
...
void CActorTelemetry::OnXPChanged(i32 inXPDelta, EXPReason inReason)
{
...
void CActorTelemetry::OnIncreasePoints(i32 score, EGameRulesScoreType type)
{
	CStatsRecordingMgr *pStatsRecordingMgr = g_pGame->GetStatsRecorder();
	CActorPtr pActor = m_pOwner.lock();
...
#endif
{
	...
	//m_recentData.reserve(inMaxRecentDataToTrack);	// FIXME need a DrxFixedDeque so we can actually do a reserve and manage this in a contiguous block
}
...
#define lerp(a,b,t)	((a)*(1.0f-t)+(b)*(t))	// FIXME can't find this in the cry code base
	...
	if (rangef>0.0f)
	{
		...
		DRX_ASSERT(frac>=0.0f && frac<=1.0f);
	...
	const CTimeValue		*inTime)
{
	...
	else
	{
		...
		if ((i32)m_recentData.size()<m_maxRecentDataLen)
		{
			interpolated=true;
