...
//------------------------------------------------------------------------
void CHomingMissile::Update(SEntityUpdateContext &ctx, i32 updateSlot)
{
...
void CHomingMissile::UpdateControlledMissile(float frameTime)
{
		...
		if (homingParams.m_controlled && !homingParams.m_autoControlled && shouldFollowHomingGuide && !m_targetId)
		{
			...
			if(pWeapon && pWeapon->IsSelected())
			{
				if (m_destinationRayId == 0)
				{
					...
					static i32k flags = (geom_colltype_ray << rwi_colltype_bit) | rwi_colltype_any | (7 & rwi_pierceability_mask) | (geom_colltype14 << rwi_colltype_bit);
	...
	if(HasTarget())
	{
		...
		if (pMissilePhysics)
		{
			...
			{
				DrxLogAlways("couldn't get physics status!");
				return;
			}
			
			if (!pMissilePhysics->GetStatus(&pos))
			{
				DrxLogAlways("couldn't get physics pos!");
				return;
			}
		}
		else
		{
			DrxLogAlways("Homing missile: Missile is not physicalized!");
			return;
		}
...
bool CHomingMissile::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags)
{
...
{
	i32 renderFlags = GetEntity()->GetSlotFlags(0);
...
{
	DRX_ASSERT(m_destinationRayId == rayID);
	DRX_ASSERT(m_pAmmoParams->pHomingParams);
