...
-------------------------------------------------------------------------
История:
- 30:8:2005   12:52 : Created by M�rcio Martins
...
//------------------------------------------------------------------------
void CItem::SetViewMode(i32 mode)
{
...
void CItem::AttachToShadowHand( bool attach )
{
	...
	IAttachment *pAttachment = pOwnerShadowCharacter->GetIAttachmentUpr()->GetInterfaceByName(m_sharedparams->params.attachment[m_stats.hand].c_str());
	...
	if (pAttachment != NULL)
	{
		...
		else
		{
			...
			{
				IAttachment *pAttachmentWeapon = GetOwnerAttachmentUpr()->GetInterfaceByName(m_sharedparams->params.attachment[m_stats.hand].c_str());
...
void CItem::CopyRenderFlags(IEntity *pOwner)
{
	...
	if (pRenderNode)
	{
		...
		if (pOwnerRenderNode)
		{
			i32 viewDistRatio = pOwnerRenderNode->GetViewDistRatio();
			i32 lodRatio = pOwnerRenderNode->GetLodRatio();
			if (!gEnv->bMultiplayer)
			{
				viewDistRatio = (i32)((float)viewDistRatio * g_pGameCVars->g_itemsViewDistanceRatioScale);
				lodRatio = (i32)((float)lodRatio * g_pGameCVars->g_itemsLodRatioScale);
			}

			pRenderNode->SetViewDistRatio(viewDistRatio);
			pRenderNode->SetLodRatio(lodRatio);

			u32 flags = pOwner->GetFlags()&(ENTITY_FLAG_CASTSHADOW);
			u32 mflags = GetEntity()->GetFlags()&(~(ENTITY_FLAG_CASTSHADOW));
