...
bool CVehicleMovementArcadeWheeled::Init(IVehicle* pVehicle, const CVehicleParams& table)
{
	...
	if(!m_pSharedParams)
	{
		...
		if(CVehicleParams fakeGearsParams = table.findChild("FakeGears"))
		{
			...
			{
				i32	count = min(ratios.getChildCount(), SVehicleGears::kMaxGears - 3);
				...
				for(i32 i = 0; i < sharedParams.gears.numGears; ++ i)
				{
...
{																																																																			\
	DrxLog("Movement Init (%s) - failed to init due to missing <%s> parameter", m_pVehicle->GetEntity()->GetClass()->GetName(), name);	\
																																																																			\
	return false;																																																												\
}
...
{ 
	...
	i32 numParts = pPhysics->GetStatus(&tmpStatus);
	i32 numWheels = m_pVehicle->GetWheelCount();
	...
	for (i32 iLayer=0; iLayer < (i32)envParams->GetLayerCount(); ++iLayer)
	{
		...
		for (i32 i=0; i < (i32)layer.GetGroupCount(); ++i)
		{ 
			...
			else
			{
				...
				for (i32 w=0; w < (i32)layer.GetWheelCount(i); ++w)
				{       
					i32 ipart = numParts - numWheels + layer.GetWheelAt(i,w)-1; // wheels are last

					if (ipart < 0 || ipart >= numParts)
					{
						DrxLog("%s invalid wheel index: %i, maybe asset/setup mismatch", m_pEntity->GetName(), ipart);
						continue;
					}
			...
			if (DebugParticles())
			{
				...
				DrxLog("WheelGroup %i local pos: %.1f %.1f %.1f", i, loc.x, loc.y, loc.z);        
			}      
#endif
		}
	}
...
void CVehicleMovementArcadeWheeled::Reset()
{
	...
	i32 n = 0;
	i32k nParts = m_pVehicle->GetPartCount();
	...
	{		
		...
		i32k numWheels = m_numWheels;
		for (i32 i=0; i<numWheels; i++)
		{
...
void CVehicleMovementArcadeWheeled::OnEvent(EVehicleMovementEvent event, const SVehicleMovementEventParams& params)
{
	...
	{		
		i32 wheelIndex = params.iValue;

		if (m_carParams.steerTrackNeutralTurn == 0.f)
		{
			i32 blownTiresPrev = m_blownTires;    
		...
		{ 
			...
			if (emitterIt->group >= 0)
			{
				...
				for (i32 i=0; i < (i32)layer.GetWheelCount(emitterIt->group); ++i)
				{
	...
	if (event == static_cast<EVehicleMovementEvent>(eVME_StartLargeObject))
	{
		...
		m_largeObjectFrictionType = drx_random(0, 1);
		...
		else if (m_largeObjectFrictionType==0)
		{
			...
			i32k c0 = m_chassis.contactIdx0;
			i32k c1 = m_chassis.contactIdx1;
			i32k c2 = m_chassis.contactIdx2;
			i32k c3 = m_chassis.contactIdx3;
...
//------------------------------------------------------------------------
void CVehicleMovementArcadeWheeled::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{

	DrxAutoCriticalSection netlk(m_networkLock);
...
//------------------------------------------------------------------------
void CVehicleMovementArcadeWheeled::SetEngineRPMMult(float mult, i32 threadSafe)
{
...
void CVehicleMovementArcadeWheeled::RemoveDebugGraphs()
{
	if (m_debugInfo)
	{
		SAFE_RELEASE(m_debugInfo->pDebugHistoryUpr);
...
void CVehicleMovementArcadeWheeled::DebugDrawMovement(const float deltaTime)
{
	...
	if (m_debugInfo==NULL)
	{
		...
		m_debugInfo->pDebugHistoryUpr = NULL;
		m_debugInfo->pDebugHistory = NULL;
		m_debugInfo->graphMode = SVehicleDebugInfo::k_graphNone;
	}
	...
	i32k width = pRenderer->GetWidth();
	i32k height = pRenderer->GetHeight();
	...
	i32 percent = (i32)(speed / m_maxSpeed * 100.f);
	...
	tukk viewName = pView ? pView->GetName() : "";
...
static i32 ChooseGraphMode(SVehicleDebugInfo* info, tukk cvarName, tukk graphName, i32 mode, float value, float min1, float max1, float min2, float max2)
{
...
{
	tukk graphName = g_pGameCVars->v_profile_graph;
	if (graphName[0]==0)
	{
		RemoveDebugGraphs();
		return;
	...
	///////// Create Graph /////////////////////////
	if (m_debugInfo->pDebugHistoryUpr==NULL)
	{
		static float leftX = 0.6f, topY = 0.1f, width = 0.4f, height = 0.4f, margin = 0.f;
		m_debugInfo->pDebugHistoryUpr = g_pGame->GetIGameFramework()->CreateDebugHistoryUpr();
		DRX_ASSERT(m_debugInfo->pDebugHistoryUpr);
		m_debugInfo->pDebugHistory = m_debugInfo->pDebugHistoryUpr->CreateHistory("vehicle-graph", "");
...
void CVehicleMovementArcadeWheeled::Update(const float deltaTime)
{
	...
	i32 notDistant = m_isProbablyDistant^1;
...
void CVehicleMovementArcadeWheeled::PostUpdateView(SViewParams &viewParams, EntityId playerId)
{
	...
	i32 viewIndex = (throttle>0.1f && m_action.bHandBrake) ? 1 : 0;
...
//------------------------------------------------------------------------
i32 CVehicleMovementArcadeWheeled::GetStatus(SVehicleMovementStatus* status)
{
		...
		else
		{
			if (m_brakeTimer > 0.f)
			{
				...
				{ 
					...
					{
						char name[256];
						drx_sprintf(name, "sounds/vehicles:%s:airbrake", m_pVehicle->GetEntity()->GetClass()->GetName());
...
void CVehicleMovementArcadeWheeled::EnableLowLevelPhysics(i32 state, i32 bThreadSafe)
{
...
void CVehicleMovementArcadeWheeled::InternalPhysicsTick(float dt)
{
	...
	if (numWheels>=4)
	{
		i32k c0 = c->contactIdx0;
		i32k c1 = c->contactIdx1;
		i32k c2 = c->contactIdx2;
		i32k c3 = c->contactIdx3;
	...
	i32 leftContact = contactMap[0][0]|contactMap[0][1];
	i32 rightContact = contactMap[1][0]|contactMap[1][1];
	...
	}
	{
		...
		i32 numWheelsThatCanHadnBrake = 0;
		...
		if (contact > 0.f)
		{
			...
			for (i32 repeat=1; repeat<numIterations; repeat++)
			{
				for (i32 i=0; i<numWheels; i++)
				{
					SVehicleWheel* w = &m_wheels[i];
...
void CVehicleMovementArcadeWheeled::TickGears(float dt, float averageWheelSpeed, float throttle, float forwardSpeed, float contact)
{
	...
	if (m_isEnginePowered)
	{
		...
		i32k iThrottle = sgn((i32)ivThrottle);
		...
		switch (m_gears.curGear)
		{
			case SVehicleGears::kReverse:
			{
				...
				i32k iSpeedGtz = 1-isneg((i32)ivSpeed);        
				i32k iThrottleGtz = (iThrottle+1);        
			...
			default:
			{
				...
				else
				{
					...
					{
						i32 noHandBrake = 1-m_action.bHandBrake;
						i32 throttleAndNoHandBrake = noHandBrake & (((iThrottle+1)>>1)^m_action.bHandBrake);
...
{ 
	...
	{ 
		...
		if(layerWheelCount && m_numWheels>0)
		{
			...
			{
				u32 idx = layer.GetWheelAt(emitterIt->group, i) - 1;
				DRX_ASSERT((unsigned)idx < (unsigned)m_numWheels);
		...
		{
			// change effect                        
			tukk effect = GetEffectByIndex(matId, layer.GetName());
			IParticleEffect* pEff = 0;   

			if (effect && (pEff = gEnv->pParticleUpr->FindEffect(effect)))
			{     
#if ENABLE_VEHICLE_DEBUG
				if (DebugParticles())          
					DrxLog("<%s> changes sfx to %s (slot %i)", pEntity->GetName(), effect, emitterIt->slot);
#endif
...
#if ENABLE_VEHICLE_DEBUG
				if (DebugParticles())
					DrxLog("<%s> found no effect for %i", pEntity->GetName(), matId);
#endif
			...
			ISurfaceType* pSurfaceType = pSurfaceUpr->GetSurfaceType(matId);
			...
			if (pSurfaceType)
			{
				...
				if (surfaceTypeName.find("mat_") != STxt::npos)
				{
...
void CVehicleMovementArcadeWheeled::GetMemoryUsage(IDrxSizer * pSizer) const
{
	pSizer->Add(*this);
