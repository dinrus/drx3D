...
namespace
{
	...
	bool IsInventoryFullForWeapon(IInventory* pPlayerInventory, IWeapon* pWeapon)
	{
		...
		i32 numAmmoTypes = weaponAmmo.size();

		for (i32 i = 0; i < numAmmoTypes; ++i)
		{
			i32 currentAmmo = pPlayerInventory->GetAmmoCount(weaponAmmo[i].pAmmoClass);
			i32 maxAmmo = pPlayerInventory->GetAmmoCapacity(weaponAmmo[i].pAmmoClass);
...
void CPlayerPlugin_Interaction::Update(float dt)
{
	...
	if (pItem)
	{
		...
		{
			DrxLog("[INTERACTION] %s '%s' is now near interactive %s '%s' (%s)", m_ownerPlayer->GetEntity()->GetClass()->GetName(), m_ownerPlayer->GetEntity()->GetName(), pItem->GetEntity()->GetClass()->GetName(), pItem->GetEntity()->GetName(), s_playerInteractionTypeNames[interactionInfo.interactionType]);
...
void CPlayerPlugin_Interaction::HandleEvent(EPlayerPlugInEvent theEvent, uk  data)
{
			...
			if (pInventory != NULL && pInventory->FindItem(pTargetItem->GetEntityId()) == -1 && pTargetItem->CanPickUp(ownerPlayerId))
			{
				...
				else
				{
					if (identicalItemMode != EIdenticalItem_Identical)
					{
						...
						{
							tukk cat = m_pItemSystem->GetItemCategory(pTargetItemClass->GetName());
...
{
	const SAutoaimTarget* pTargetInfo = g_pGame->GetAutoAimUpr().GetTargetInfo( targetActor.GetEntityId() );
