...
/*************************************************************************
	...
	$Id$
	$DateTime$
	Описание: Implements various IGameStatistics callbacks and listens to
	...
	m_requestNewSessionId(false)
{
	...
	DRX_ASSERT_MESSAGE(m_gameStats,"CStatsRecordingMgr instantiated without IGameStatistics being instantiated first, initialise IGameStatistics first - fatal");
	...
	//m_statsDirectory.Format(".\\%s/StatsLogs", gEnv->pDrxPak->GetAlias("%USER%"));

	m_statsDirectory = "%USER%/StatsLogs";
	gEnv->pDrxPak->MakeDir(m_statsDirectory.c_str());
...
//	m_profileStatsBinder.BindErrback(functor(*this, &CMissionStatistics::OnStatsError));
	...
	gEnv->pEntitySystem->GetIEntityPoolUpr()->AddListener(this, "CStatsRecordingMgr", 
	...
	DRX_ASSERT_MESSAGE(pDL || !gEnv->bMultiplayer,"CStatsRecordingMgr must be instantiated after the CDownloadMgr");		// need to get our settings from the web
	if (pDL)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_GAME,VALIDATOR_ERROR,"CStatsRecordingMgr failed to start downloading its online settings - will default to sending telemetry");
#if USER_marktu
			DRX_ASSERT_MESSAGE(0,"Failed to find permission resource");
#endif
		}
	}
}
...
CStatsRecordingMgr::~CStatsRecordingMgr()
{
	...
	gEnv->pEntitySystem->GetIEntityPoolUpr()->RemoveListener(this);
}
	...
	const char									*pValue)
{
	...
	for (size_t i=0; i<DRX_ARRAY_COUNT(map); i++)
	{
		if (stricmp(map[i].pKey,pKey)==0 )
		{
			...
			if(tukk slashPtr = strchr(pValue,'/'))
			{
				// Sampling case...
				u32 numerator;
				u32 denominator;
				...
				if(denominator!=0)
				{
					...
					tukk profileName = tUserName.c_str();
#if defined( DEDICATED_SERVER )
					if( ICVar* pServer = gEnv->pConsole->GetCVar( "sv_servername" ) )
					{
						if( tukk pServerName = pServer->GetString() )
						{
							profileName = pServerName;
						}
					}
#endif					
					u32k profileNameCrc = CCrc32::Compute( profileName );

					u32k profileNameModulo = profileNameCrc % denominator;
			...
			if (permission)
			{
				...
				DrxLog("online submit permission '%s' set to 1",map[i].pKey);
			}
			else
			{
				m_submitPermissions&=~map[i].bit;
				DrxLog("online submit permission '%s' set to 0",map[i].pKey);
			}
	...
	CDownloadableResourcePtr		inResource)
{
	...
	if (!root)
	{
		DrxWarning(VALIDATOR_MODULE_GAME,VALIDATOR_ERROR,"CStatsRecordingMgr failed to parse the permissions.xml it downloaded");
	}
		...
		for (i32 i=0; i<numChildren; i++)
		{
			if (XmlNodeRef xmlChild=root->getChild(i))
			{
				if (stricmp(xmlChild->getTag(),"config")==0)
				{
					...
					tukk pVal=xmlChild->getAttr("value");
	...
	CDownloadableResourcePtr		inResource)
{
	...
	DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Failed to download online data posting permissions, will use defaults");
...
void CStatsRecordingMgr::BeginSession()
{
	if (IsTrackingEnabled())
	{
		...
		{
			DrxLog("** Previous session's stats are still being serialized and uploaded, will not be able to record stats for this session");
...
i32 CStatsRecordingMgr::GetIntAttributeFromProfile(IPlayerProfile *inProfile, tukk inAttributeName)
{
	TFlowInputData data;
	i32 retData=0;

	DRX_ASSERT(inProfile);
	DRX_ASSERT(inAttributeName);
	...
	if (inProfile->GetAttribute(inAttributeName, data))
	{
		switch(data.GetType())
		{
			...
			case eFDT_Float:
			{
				...
				DRX_ASSERT(pFloatData);
				retData = static_cast<i32>(*pFloatData);
				break;
			}
			case eFDT_String:
			{
				const string *pStringData = data.GetPtr<string>();
				DRX_ASSERT(pStringData);
				retData = atoi(pStringData->c_str());
				break;
			}
			default:
				DRX_ASSERT_MESSAGE(0, string().Format("unexpected attribute data type = %d", data.GetType()).c_str());
				break;
		}
	}
...
void CStatsRecordingMgr::StateEndOfSessionStats()
{
	if (m_sessionTracker)
	{
		...
		if( IPlayerProfileUpr *pProfileMan = gEnv->pGame->GetIGameFramework()->GetIPlayerProfileUpr() )
		{
			...
			if (pProfile)
			{
				i32 runOnceVersion=GetIntAttributeFromProfile(pProfile, "RunOnceVersion");
				i32 runOnceTrackingVersion=GetIntAttributeFromProfile(pProfile, "RunOnceTrackingVersion");
		...
		else
		{
			if (m_matchBonusXp.size() > 0)
			{
				...
				DrxFixedArray<SBonusMatchXp, MAX_PLAYER_LIMIT>::iterator itBonusXp;
				for (itBonusXp = m_matchBonusXp.begin(); itBonusXp != m_matchBonusXp.end(); ++itBonusXp)
				{
					...
					tukk szOnlineGUID = pGameLobby ? pGameLobby->GetGUIDFromActorID(itBonusXp->entityId) : "";
#if defined( _RELEASE )
					u32k guidHash = CCrc32::Compute( szOnlineGUID );
			...
			if( m_interactiveObjects.size() > 0 || m_environmentalWeapons.size() > 0 )
			{
				...
				for( itObjects = m_interactiveObjects.begin(); itObjects != m_interactiveObjects.end(); ++itObjects )
				{
					...
					if( pEntity )
					{
						...
						tukk name = pEntity->GetName();
						objectNode->setAttr( "name", name );
												
						if( pEntity->GetArchetype() )
						{
							tukk arcName = pEntity->GetArchetype()->GetName();
...
void CStatsRecordingMgr::Update(float frameTime)
{
	...
	if (m_scheduleRemoveIDataListener)
	{
		...
		{
			DrxLog("CStatsRecordingMgr::Update() had scheduled an IDataListener removal but ended up not actually being a listener. This shouldn't happen!");
		}
...
void CStatsRecordingMgr::BeginRound()
{
	...
	{
		DRX_ASSERT_MESSAGE(m_sessionTracker,"BeginRound() called when no session is open");
		if (!m_sessionTracker)
		{
			BeginSession();
		}
		DRX_ASSERT_MESSAGE(m_gameStats->GetScopeID()==eGSC_Session,"stats error, session scope not active as aspected");
	...
	string		inTeamName)
{
	if (IsTrackingEnabled() && m_sessionTracker)
	{
		...
		if(track)
		{
			track->StateValue(eGSS_TeamName, inTeamName.c_str());		// FIXME SStatAnyValue is missing constructor for DrxString and will only do a shallow copy of this string
			track->StateValue(eGSS_TeamID, inTeamId);
			if (inTeamId >= 0 && inTeamId <= MAX_TEAM_INDEX)
			{
				DRX_ASSERT_MESSAGE(m_teamTrackers[inTeamId] == NULL, "Trying to add a team with the same team id");
				m_teamTrackers[inTeamId] = track;
			}
			else
			{
				DRX_ASSERT_MESSAGE(false, "Team ID is out of range, consider increasing MAX_TEAM_INDEX if necessary");
			}
		}
	}
}

void CStatsRecordingMgr::OnTeamScore(i32 teamId, i32 score, EGameRulesScoreType pointsType)
{
...
void CStatsRecordingMgr::OnMatchBonusXp(IActor* pActor, i32 amount)
{
	m_matchBonusXp.push_back(SBonusMatchXp(pActor->GetEntityId(), amount));
}

void CStatsRecordingMgr::InteractiveObjectEntry( EntityId entity, i32 type )
{
	m_interactiveObjects.push_back(SIntrestEntity( entity, type ));
}

void CStatsRecordingMgr::EnvironmentalWeaponEntry( EntityId entity, i32 type )
{
	...
	IActor			*inPlayerActor)
{
	...
	if ( IStatsTracker *tracker = GetStatsTracker(inPlayerActor) )
	{
		...
		tukk szOnlineGUID = g_pGame->GetGameLobby() ? g_pGame->GetGameLobby()->GetGUIDFromActorID(inPlayerActor->GetEntityId()) : "";
		...
		tracker->StateValue( eSS_EntityId,		static_cast<i32>(eid) );
		i32 teamId = g_pGame->GetGameRules()->GetTeam(eid);
		if(!gEnv->bMultiplayer)
		{
		...
		else
		{
			if (inPlayerActor->IsPlayer())
			{
				...
				i32 xp, rank, defaultMode, stealth, armour, reincarnations;
				...
				{
					u16	skill=pGameLobby->GetSkillRanking(inPlayerActor->GetChannelId());
	...
	IActor				*inActor)
{
	...
	DRX_ASSERT_MESSAGE(tracker,"actor not currently in stats tracker, weapons won't be recorded - call ordering problem?");

	if (tracker)
	{
		IInventory *pInv = inActor->GetInventory();

		DRX_ASSERT_MESSAGE(pInv,"expected actor to have an inventory");

		i32			invCount=pInv->GetCount();

		if (invCount>0)
		{
			for (i32 invIndex=0; invIndex<invCount; invIndex++)
			{
					...
					for (itAccessory = accessories.begin(); itAccessory != accessories.end(); ++itAccessory)
					{
						...
						if (pParams)
						{
							if (pParams->category == g_pItemStrings->bottom)
								...
								DRX_ASSERT_MESSAGE(pParams->category.empty(), string().Format("Unrecognised attachment category %s", pParams->category.c_str()));
						}
						else
						{
							DRX_ASSERT_MESSAGE(0, "Unable to find accessory params");
						}
					}
		...
		{
			DrxLog("Stats recording - empty inv for actor '%s'",inActor->GetActorClassName());
		}
	...
	IActor		*inActor)
{
	if (IsTrackingEnabled() && m_roundTracker)
	{
		...
		DrxLog ("[STATS TRACKER] Start tracking stats for %s '%s'%s", pActor->GetEntity()->GetClass()->GetName(), pActor->GetEntity()->GetName(), pActor->m_telemetry.GetStatsTracker() ? " (WARNING: there are already stats for this actor)" : "");

		if (inActor->IsPlayer())
		{
			DRX_ASSERT_TRACE(!pActor->m_telemetry.GetStatsTracker(),("already tracking stats for player %s\n",inActor->GetEntity()->GetName()));
			m_gameStats->AddGameElement(SNodeLocator(eGSEL_Player,eGSC_Round,eGNLT_ChannelID,inActor->GetChannelId()),NULL);
			DRX_ASSERT(pActor->m_telemetry.GetStatsTracker());
		}
		else
		{
			DRX_ASSERT_TRACE(!pActor->m_telemetry.GetStatsTracker(),("already tracking stats for actor %s\n",inActor->GetEntity()->GetName()));
	...
	IActor		*inActor)
{
	...
	DrxLog ("[STATS TRACKER] Stop tracking stats for %s '%s'%s", pActor->GetEntity()->GetClass()->GetName(), pActor->GetEntity()->GetName(), tracker ? "" : " (NB: there are currently no stats for this actor)");

	if (tracker)
	{
		m_gameStats->RemoveElement(m_gameStats->GetTrackedNode(tracker));
		DRX_ASSERT(pActor->m_telemetry.GetStatsTracker()==NULL);
	}
}
	...
	ITelemetryProducer				*pInProducer)
{
	...
	if (!submitted)
	{
		...
		{
			DrxLog("Failed to submit game telemetry");
		}
		else
		{
			DrxLog("Did not submit stats logs as online permissions forbade it");
		}
		delete pInProducer;
...
void CStatsRecordingMgr::GetUniqueFilePath(
	...
	string									*pRemotePath)
{
	static i32k MAX_SAVE_ATTEMPTS = 20;
	...
	while (offsetSeconds < MAX_SAVE_ATTEMPTS)
	{
		...
		if ((!g_pGameCVars->g_telemetry_gameplay_save_to_disk) || (!gEnv->pDrxPak->IsFileExist(pLocalPath->c_str())))
		{
			break;
		}
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Too many attempts to find statistics session name. Will overwrite prev file...");
	}
}
...
void CStatsRecordingMgr::SaveSessionData(XmlNodeRef node)
{
	...
	static i32k k_chunkSize=12*1024;		// on consoles, xmlnode::savetofile won't write in chunks larger than ~15000 bytes

	FILE	*file=gEnv->pDrxPak->FOpen( filePath.c_str(),"wt");
	bool	success=false;
...
void CStatsRecordingMgr::OnNodeAdded(const SNodeLocator& locator)
{
	...
	if(locator.isScope()) // Scope pushed
	{
		switch(locator.scopeID)
		{
			case eGSC_Round:
				{
					DRX_ASSERT_MESSAGE(!m_roundTracker,"BeginRound() called when a round is already in progress");
					if (m_roundTracker)
					{
						EndRound();
					}
			...
			case eGSC_Session:
				{
					DRX_ASSERT_MESSAGE(!m_sessionTracker,"BeginSession() called when session already active");
					if (m_sessionTracker)
					{
						EndSession();
					}
	...
	else // Element added
	{
		switch(locator.elemID)
		{
			case eGSEL_Player:
				{
					DRX_ASSERT(locator.scopeID == eGSC_Round);
					DRX_ASSERT(locator.locatorType == eGNLT_ChannelID);

					tracker->Event( eSE_Lifetime, "begin" );

					i32		channel=locator.locatorValue;
					IActor	*actor=g_pGame->GetGameRules()->GetActorByChannelId(channel);
					DRX_ASSERT_TRACE(actor,("Failed to find actor for channel %d",channel));
					if (actor)
					{
						if (actor->IsPlayer())
						{
							CPlayer	*player=static_cast<CPlayer*>(actor);
							DRX_ASSERT_MESSAGE(player->m_telemetry.GetStatsTracker()==NULL,"setting stats tracker for player, but one is already set?!");
			...
			case eGSEL_AIActor:
				{
					DRX_ASSERT(locator.scopeID == eGSC_Round);
					DRX_ASSERT(locator.locatorType == eNLT_EntityID);

					tracker->Event( eSE_Lifetime, "begin" );

					EntityId id = locator.locatorValue;
					CActor	*pActor=static_cast<CActor*>(g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(id));
					DRX_ASSERT_TRACE(pActor,("Failed to find actor %d",id));
					if (pActor)
					{
						DRX_ASSERT(!pActor->IsPlayer());
						DRX_ASSERT_MESSAGE(pActor->m_telemetry.GetStatsTracker()==NULL,"setting stats tracker for AI, but one is already set?!");
...
void CStatsRecordingMgr::OnNodeRemoved(const SNodeLocator& locator, IStatsTracker* tracker)
{
	...
	if(locator.isScope()) // Scope popped
	{
		switch(locator.scopeID)
		{
			case eGSC_Round:
				{
					DRX_ASSERT_MESSAGE(tracker==m_roundTracker,"Stats messed up, ended a round that wasn't the round being tracked?!");
			...
			case eGSC_Session:
				{
					DRX_ASSERT_MESSAGE(tracker==m_sessionTracker,"Stats messed up, ended a session that wasn't the session being tracked?!");
					m_sessionTracker=NULL;
				}
				break;
		}
	...
	else // Element removed
	{
		switch(locator.elemID)
		{
			case eGSEL_Player:
				{
					...
					{
						CPlayer		*player=static_cast<CPlayer*>(actor);
						DRX_ASSERT_MESSAGE(player->m_telemetry.GetStatsTracker()==tracker || player->m_telemetry.GetStatsTracker()==NULL,"stats tracker mismatch for player");
			...
			case eGSEL_AIActor:
				{
					...
					if (pActor)
					{
						...
						DRX_ASSERT_MESSAGE(pActor->m_telemetry.GetStatsTracker()==tracker || pActor->m_telemetry.GetStatsTracker()==NULL,"stats tracker mismatch for AI actor");
			...
			case eGSEL_Team:
				{
					for (i32 i=0; i<=MAX_TEAM_INDEX; ++i)
					{
	...
	if (actor)
	{
		...
		if (IStatsTracker *tracker=GetStatsTracker(actor))
		{
			...
			if (hit.partId >= 0)
			{
				...
				if (pCharacter)
				{
					// 1000 is a magic number in DrxAnimation that establishes the number of the first attachment identifier
					...
					if (hit.partId >= FIRST_ATTACHMENT_PARTID)
					{
						...
						IAttachment* pAttachment = pAttchmentUpr->GetInterfaceByIndex(hit.partId - FIRST_ATTACHMENT_PARTID);
						if (pAttachment)
						{
							hit_part = "attachment "; 
					...
					else
					{
						...
						tukk szJointName = rIDefaultSkeleton.GetJointNameByID(pSkeletonPose->getBonePhysParentOrSelfIndex(hit.partId));
...
//////////////////////////////////////////////////////////////////////////
void CStatsRecordingMgr::LoadEventConfig(tukk configName)
{
	...
	if(eventsNode)
	{
		...
		i32 numEvents = eventsNode->getChildCount();
...
#endif
{
	if (!gEnv->pThreadUpr->SpawnThread(this, "TelemetryCopier"))
	{
		DrxFatalError("Error spawning \"TelemetryCopier\" thread.");
	}
}
...
CTelemetryCopierThread::~CTelemetryCopierThread()
{
	if (m_pInProducer)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "CTelemetryCollector: Producer wasn't deleted. Did you call flush?");
		Flush();
	}
}
...
void CTelemetryCopierThread::ThreadEntry()
{
	...
	GetISystem()->GetXmlUtils()->SetStatsOwnerThread(DrxGetCurrentThreadId());
...
#endif
#ifndef _RELEASE
	DrxLog("CTelemetryCopierThread had %fs to do work and took %fs", m_time, m_runTime);
#endif
	GetISystem()->GetXmlUtils()->SetStatsOwnerThread(DrxGetCurrentThreadId());
