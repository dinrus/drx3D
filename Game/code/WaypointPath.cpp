...
bool CWaypointPath::CreatePath(IEntity* pPathEntity)
{
	...
	IGameVolumes* pGameVolumes = gEnv->pGame->GetIGameFramework()->GetIGameVolumesUpr();
	if (pGameVolumes != NULL)
	{
		...
		if(pGameVolumes->GetVolumeInfoForEntity(pPathEntity->GetId(), &volumeInfo))
		{
			...
			i32 count = volumeInfo.verticesCount;
			if(count > MAX_PATH_NODES)
			{
				DRX_ASSERT_MESSAGE( false, string().Format("CWaypointPath::CreatePath - path %s has too many nodes - %i/%i. Truncating end nodes.", pPathEntity->GetName(), count, MAX_PATH_NODES) );
			...
			if(count)
			{
				...
				if(EntityScripts::GetEntityProperty(pPathEntity, "fileNodeDataFile", nodeDataFile) && strcmpi("", nodeDataFile) )
				{
					...
					if (rootNode)
					{
						...
						{
							i32 nodeCount = pathNode->getChildCount();
							i32 index = 0;
							float fValue = 0.f;
							for(i32 i = 0; i < nodeCount; ++i)
							{
								...
								if(nodeNode->getAttr("index", index))
								{
									...
									{
										string str;
										DrxLog("CWaypointPath::CreatePath - Adding node data to map but node %i does not exist. Remove from XML file %s.", index, nodeDataFile);
									}
#endif
...
Matrix34 CWaypointPath::GetMatrixAtNode(TNodeId currentNode, bool loop) const
{
	...
	DRX_ASSERT_MESSAGE(false, "CWaypointPath::GetMatrix - currentNode input is out of range");
	return Matrix34();
}
	...
	if(!crossingStartEndNode || loop)
	{
		...
		if(currentNode >= 0)
		{
			...
			// Update the newPathLoc.
			{
				...
				i32k totalNodes = m_MaxNodeIndex+1;

				float localPathLoc = 100.f;
				for(i32 i=0;i<3&&localPathLoc>=1.0f;i++)
				{
