...
#include <drx3D/Game/BlendTypes.h>
#include <drx3D/Game/BlendedEffect.h>
#include <drx3D/Game/BlendNode.h>
...
void CScreenEffects::Update(float frameTime)
{
	if (!m_enableBlends)
		return;
	std::map<i32, CBlendGroup*>::iterator it = m_blends.begin();
...
//---------------------------------
i32 CScreenEffects::GetUniqueID()
{
...
//---------------------------------
void CScreenEffects::StartBlend(IBlendedEffect *effect, IBlendType *blendType, float speed, i32 blendGroup)
{
	if(!effect || !blendType)
		return;
...
//---------------------------------
bool CScreenEffects::HasJobs(i32 blendGroup)
{
	if (m_blends.count(blendGroup) == 0)
	{
		return false;
...
//---------------------------------
void CScreenEffects::ClearBlendGroup(i32 blendGroup, bool resetScreen)
{
	if (m_blends.count(blendGroup) != 0)
	{
		CBlendGroup *group = m_blends[blendGroup];
...
//---------------------------------
void CScreenEffects::ResetBlendGroup(i32 blendGroup, bool resetScreen)
{
	if (m_blends.count(blendGroup) != 0)
	{
		CBlendGroup *group = m_blends[blendGroup];
...
void CScreenEffects::SetUpdateCoords(tukk coordsXname, tukk coordsYname, Vec3 pos)
{
...
void CScreenEffects::EnableBlends(bool enable, i32 blendGroup)
{
	m_enabledGroups[blendGroup] = enable;
}

//--------------------------------------------------------------
void CScreenEffects::GetMemoryStatistics(IDrxSizer * s)
{
