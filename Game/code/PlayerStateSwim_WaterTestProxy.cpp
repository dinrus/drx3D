...
$Id$
$DateTime$
Описание: Contains the ray testing code extracted from CPlayer.
...
void CPlayerStateSwim_WaterTestProxy::OnEnterWater(const CPlayer& player)
{
	...
	DRX_ASSERT(m_waterLevel > WATER_LEVEL_UNKNOWN);
	Update(player, 0.0f);
}
...
{
	DRX_ASSERT(m_bottomLevelRayID == rayID);
...
void CPlayerStateSwim_WaterTestProxy::RayTestBottomLevel( const CPlayer& player, const Vec3& referencePosition, float maxRelevantDepth )
{
	...
	i32 entityFlags = ent_terrain|ent_static|ent_sleeping_rigid|ent_rigid;
	...
	DRX_ASSERT(m_bottomLevelRayID == 0); 
