...
void CHeavyWeapon::Use(EntityId userId)
{
	if(m_properties.usable)
	{
		...
		{
			u32 deselectionTime = StartDeselection(false);
...
{
	u32 deselectionTime = StartDeselection(false);
	u32 detachTime = (u32)(deselectionTime * g_pGameCVars->i_hmg_detachWeaponAnimFraction);
...
void CHeavyWeapon::InitClient( i32 channelId )
{
