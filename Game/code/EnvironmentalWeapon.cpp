...
tukk const s_EnvironmentalWeaponName = "PickAndThrowWeapon";
static IEntityClass* s_pEnvironmentalWeaponClass;

tukk  CEnvironmentalWeapon::kHealthScriptName = "currentHealth";
...
// Sweep testing
const static i32 sMaxEntitiesToConsiderPerSweep = 2; 
...
#endif //#ifndef _RELEASE
{
	...
	DRX_ASSERT_MESSAGE(s_pEnvironmentalWeaponClass, "CEnvironmentalWeapon::CEnvironmentalWeapon - couldn't find weapon class");
}
...
bool CEnvironmentalWeapon::Init( IGameObject *pGameObject )
{
	...
	if( m_logicFlags & ELogicFlag_hasParentVehicle )
	{
		...
		{
			DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "EnvironmentalWeapon: Missing parent vehicle data in client init, aborting" );
			return false;
		}
	}
...
{	
	...
	if(pOwnerActor)
	{
		...
		DRX_ASSERT_MESSAGE(pOwnerActor->GetActorClass() == CPlayer::GetActorClassType(), "CEnvironmentalWeapon::InitWeaponState_Held - Expected player, got something else");
		...
		if(pScriptTable)
		{
			...
			if(val == 1)
			{
				...
				if(pActor)
				{
					...
					DRX_ASSERT_MESSAGE(pActor->GetActorClass() == CPlayer::GetActorClassType(), "CEnvironmentalWeapon::Use - Expected player, got something else");
	...
	if(m_OwnerId != ownerId)
	{
		...
		if(m_OwnerId && !ownerId)
		{
			...
			if( !m_throwerId )
			{
				...
				{
					pTracker->Event( eGSE_EnvironmentalWeaponEvent, new CEnvironmentalWeaponEvent( GetEntityId(), eEVE_Drop, (i16)health ) );
				}
			}
...
bool CEnvironmentalWeapon::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
	if(aspect == ASPECT_HEALTH_STATUS)
	{
		...
		DRX_ASSERT_MESSAGE( fCurrentHealth <= 8191, "ERROR - edmg policy max cannot store currentHealth ");
...
void CEnvironmentalWeapon::Update(SEntityUpdateContext &ctx, i32 updateSlot)
{
...
void CEnvironmentalWeapon::DoInitWeaponState()
{
	...
	if(m_State != EWS_Initial)
	{
		...
		DRX_ASSERT_MESSAGE((m_OwnerId > 0 && m_State == EWS_Held) || (m_OwnerId == 0 && m_State == EWS_OnGround), "CEnvironmentalWeapon::PostInit() < ERROR - properties in invalid state");
...
void CEnvironmentalWeapon::UpdateDebugOutput() const
{
	...
	if(m_OwnerId > 0)
	{
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponHealthDebugEnabled )
		{
			...
			if(pActor)
			{
				...
				if(pPlayer->IsClient())
				{
					...
					DrxWatch("CurrentHealthVals [ %.1f | %.1f ] ", currentHealth, initialHealth);
					DrxWatch("CurrentHealthPercentage [ %.1f Percent ]", initialHealth > 0.0f ? (currentHealth / initialHealth * 100.0f) : 9999.0f);

					bool isDamageable = false; 
					EntityScripts::GetEntityProperty(GetEntity(),"damageable", isDamageable);
					DrxWatch("Damageable : %s", isDamageable ? "TRUE" : "FALSE" );
				}
			}
		}
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			...
			DrxWatch("Current Attack State : [ %s ]", attackStateName);

			// Anim event debugging
			DrxWatch("Animation Damage Phase : [ %s ]", (m_logicFlags & ELogicFlag_damagePhaseActive) ? "TRUE": "FALSE");
			...
			if(m_pDamageZoneHelper)
			{
				...
				if(!m_debugCollisionPts_Geom.empty())
				{
					...
					for(std::vector<debugCollisionPt>::const_iterator iter = m_debugCollisionPts_Geom.begin(); iter != endIter; ++iter)
					{
						...
						// Render number at pt
						i32k count = static_cast<i32>(iter - m_debugCollisionPts_Geom.begin()); 
	...
	case ENTITY_EVENT_DELINK:
		{
			if( m_OwnerId == 0 )
			{
				...
				velocity.v  = dynamics.v + direction * drx_random( 1.0f, 3.0f );
...
void CEnvironmentalWeapon::GetMemoryUsage( IDrxSizer *pSizer ) const
{
	pSizer->Add(*this);
}
...
bool CEnvironmentalWeapon::ReloadExtension( IGameObject * pGameObject, const SEntitySpawnParams &params )
{
	...
	DRX_ASSERT_MESSAGE(false, "CEnvironmentalWeapon::ReloadExtension not implemented");
...
{
	DRX_ASSERT_MESSAGE(false, "CEnvironmentalWeapon::GetEntityPoolSignature not implemented");

	return true;
}

void CEnvironmentalWeapon::InitClient(i32 channelId)
{
...
bool AllowedToDamagePlayer(const EntityId attackerId, const EntityId victimEntityId)
{
	...
	{
		i32 clientTeamId = pGameRules->GetTeam(attackerId);
		i32 victimTeamId = pGameRules->GetTeam(victimEntityId);
...
void CEnvironmentalWeapon::HandleMeleeCollisionEvent( const EventPhysCollision	*pEventPhysCollision, const bool bAllowActors )
{
	...
	if(pOwnerActor && pOwnerActor->IsPlayer())
	{
		...
		if(pPhysics)
		{
			...
			DRX_ASSERT(pVictimPhysicalEntity); 
...
void CEnvironmentalWeapon::HandleMeleeCollisionEvent_Object(const EventPhysCollision *pEventPhysCollision, IEntity* pVictimEntity, CPlayer* pOwnerPlayer, i32 victimIdx, IPhysicalEntity* pVictimPhysicalEntity)
{
	// Rejection tests 
	EntityId victimId(0);
	if (pVictimEntity)						
	...
	if(!IsMeleeCollisionWithinDamageZone(collisionLocation))
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			DrxLog("[ENV-WEAPON]: Object hit by EnvWeapon but not in marked damage zone ");
		}
#endif //#ifndef _RELEASE
		return;
	}
...
bool CEnvironmentalWeapon::PassesHitzoneConeCheck(const EventPhysCollision	*pEventPhysCollision) const
{
	...
	if(!LocationWithinFrontalCone(ownerPosition, collisionLocation, attackerFacingdir,DEG2RAD(g_pGameCVars->pl_pickAndThrow.environmentalWeaponHitConeInDegrees)) )
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			DrxLog("[ENV-WEAPON]: Something hit by EnvWeapon but not in %.f frontal hit cone", g_pGameCVars->pl_pickAndThrow.environmentalWeaponHitConeInDegrees);
		}
#endif //#ifndef _RELEASE
		return false;
	}
...
void CEnvironmentalWeapon::HandleMeleeCollisionEvent_Actor(const EventPhysCollision *pEventPhysCollision, EntityId victimEntityId, CPlayer* pOwnerPlayer, i32 victimIdx, IPhysicalEntity* pVictimPhysicalEntity)
{
	...
	if(!IsMeleeCollisionWithinDamageZone(collisionLocation))
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			DrxLog("[ENV-WEAPON]: Entity %d hit by EnvWeapon but not in marked damage zone ", victimEntityId);
		}
#endif //#ifndef _RELEASE
		return;
	}
...
{
	static tukk pDamageZoneHelper = "damage_zone_helper"; 
	EntityId id							 = GetEntityId(); 
	i32 slot							   = PickAndThrow::FindActiveSlot(id);
...
void CEnvironmentalWeapon::HandleMeleeCollisionEvent_BreakableGeometry(const EventPhysCollision *pEventPhysCollision,  CPlayer* pOwnerPlayer, i32 victimIdx, IPhysicalEntity* pVictimPhysicalEntity)
{
	// 1) Abort if collision not within our 'hitzone' cone 
	if(!PassesHitzoneConeCheck(pEventPhysCollision))
	{
	...
	if(!IsMeleeCollisionWithinDamageZone(collisionLocation))
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			DrxLog("[ENV-WEAPON]: Breakable Geom hit by EnvWeapon but not in marked damage zone ");
		}
#endif //#ifndef _RELEASE
		return;
	}
...
void CEnvironmentalWeapon::GenerateArtificialCollision(IPhysicalEntity* pAttackerPhysEnt, IPhysicalEntity *pCollider, i32 colliderMatId, const Vec3 &position, const Vec3& normal, const Vec3 &vel, i32 partId, i32 surfaceIdx, i32 iPrim, const Vec3& impulse )
{
	//////////////////////////////////////////////////////////////////////////
...
void CEnvironmentalWeapon::HandleMeleeCollisionEvent_Geometry(const EventPhysCollision	*pEventPhysCollision, CPlayer* pOwnerPlayer, i32 victimIdx)
{
	// 1) Abort if collision not within our 'hitzone' cone 
	if(!PassesHitzoneConeCheck(pEventPhysCollision))
	{
	...
	if(!IsMeleeCollisionWithinDamageZone(collisionLocation))
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			DrxLog("[ENV-WEAPON]: Geom hit by EnvWeapon but not in marked damage zone ");
		}
...
void CEnvironmentalWeapon::GenerateMaterialEffects(const Vec3& pos, i32k surfaceIndex)
{
					...
					else
					{
						...
						if(pActor && pActor->IsPlayer())
						{
							...
							if (!blocked)
							{
								...
								{
									DrxLog("[ENV-WEAPON]: Entity %d hit by THROWN EnvWeapon but [%.f] below required velocity [%.f]", victimEntityId, impactVelocity.GetLength(), requiredKillVelocity);
								}
#endif //#ifndef _RELEASE
							}
							else
...
void CEnvironmentalWeapon::GenerateHitInfo(EntityId attackerEntityId, float damage,  HitInfo& hitInfo, const EntityHitRecord& entityHitRecord, const Vec3& nImpactDir, i32 hitTypeId ) const
{
	...
	i32 surfaceIdx = entityHitRecord.m_surfaceIdx;
	i32 partId		 = entityHitRecord.m_partId; 
...
Vec3 CEnvironmentalWeapon::CalculateImpulse_Player( const HitInfo& hitInfo, CPlayer& rPlayer, const Vec3& nImpactDir, EntityId attackerId ) const
{
	...
	if (IPhysicalEntity* pVictimPhysicalEntity = rPlayer.GetEntity()->GetPhysics())
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			DrxLog("[ENV-WEAPON]: Victim Mass calculated at %.f", victimMass);
		}
#endif //#ifndef _RELEASE
		...
		{
			const float hitOverrideDirRatio = (m_overrideHitDirMaxRatio-m_overrideHitDirMinRatio)*drx_random(0.0f, 1.0f) + m_overrideHitDirMinRatio;
	...
	// Failed
	DrxLog("[ENV-WEAPON]: Player has no physics, Zero impulse returned ");
...
void CEnvironmentalWeapon::ApplyImpulse(const HitInfo& hitInfo, const EntityId victimEntityId) const
{
	...
	if (pVictimPhysicalEntity)
	{
		...
		if(pActor) // We ragdollize the actor even though they aren't dead (cue monty python 'im not dead!' shouts).
		{
			...
			if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
			{
				DrxLog("[ENV-WEAPON]: Ragdollizing Actor %d", pActor->GetEntityId());
				DrxLog("[ENV-WEAPON]: Applying impulse - living - [ %.f, %.f, %.f ]to actor %d", livingEntImpulse.impulse.x, livingEntImpulse.impulse.y, livingEntImpulse.impulse.z,pActor->GetEntityId());
				DrxLog("[ENV-WEAPON]: Applying impulse - articulated - [ %.f, %.f, %.f ]to actor %d to part %d, pt <%.1f,%.1f,%.1f>", articulatedEntImpulse.impulse.x, articulatedEntImpulse.impulse.y, articulatedEntImpulse.impulse.z,pActor->GetEntityId(), articulatedEntImpulse.partid, articulatedEntImpulse.point.x, articulatedEntImpulse.point.y, articulatedEntImpulse.point.z);
			}
#endif //#ifndef _RELEASE
		}
		// OBJECTS
		else if(IEntityPhysicalProxy* pPhysicsProxy = (IEntityPhysicalProxy*)pEntity->GetProxy(ENTITY_PROXY_PHYSICS))
		{
			...
			if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
			{
				DrxLog("[ENV-WEAPON]: Applying impulse [ %.f, %.f, %.f ]to physics proxy with entityId: %d", vImpulseVec.x, vImpulseVec.y, vImpulseVec.z,pEntity->GetId());
			}
#endif //#ifndef _RELEASE
		}
	}
...
void CEnvironmentalWeapon::ProcessHitPlayer( IActor* pVictimActor, const EntityHitRecord& entityHitRecord, const Vec3& nImpactDir, EntityId attackerId )
{
	...
	if(!pVictimPlayer->IsDead() && AllowedToDamagePlayer(attackerId, entityHitRecord.m_entityId))
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			DrxLog("[ENV-WEAPON]: Enemy Player %d hit by EnvWeapon", pVictimPlayer->GetEntityId());
		}
#endif //#ifndef _RELEASE
		...
		{
			// Charged throw
			DRX_ASSERT_MESSAGE(m_currentAttackState & EAttackStateType_ChargedThrow, "CEnvironmentalWeapon::ProcessHitPlayer < ERROR - attempting to process player hit with unknown attack state");
		...
		{
			DrxLogAlways("[ENV-WEAPON]: CEnvironmentalWeapon::ProcessHitPlayer() < Warning - impulse scale greater than compression policy capacity. This should probably be adjusted"); 
		}
#endif //#ifndef _RELEASE
	}
}
...
void CEnvironmentalWeapon::ProcessHitObject( const EntityHitRecord& entityHitRecord, const Vec3& impactVel, IPhysicalEntity* pVictimPhysicalEntity, EntityId attackerId ) const
{
	...
	if (g_pGame->GetIGameFramework()->GetIVehicleSystem()->GetVehicle(entityHitRecord.m_entityId))
	{
		...
		{
			// Charged throw
			DRX_ASSERT_MESSAGE(m_currentAttackState & EAttackStateType_ChargedThrow, "CEnvironmentalWeapon::GenerateHitInfo < ERROR - attempting to process hit when in an unknown state");
...
void CEnvironmentalWeapon::OnStartChargedThrow()
{
	...
	i16 health = (i16)GetCurrentHealth();
...
void CEnvironmentalWeapon::OnMeleeStartDamagePhaseEvent()
{
	...
	if(g_pGame->GetIGameFramework()->GetClientActorId() == m_OwnerId)
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			DrxLog("[ENV-WEAPON]: Start Damage event received");
		}
#endif //#ifndef _RELEASE
...
void CEnvironmentalWeapon::OnMeleeEndDamagePhaseEvent()
{
	...
	if(g_pGame->GetIGameFramework()->GetClientActorId() == m_OwnerId)
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
		{
			DrxLog("[ENV-WEAPON]: End Damage event received" );
		}
#endif //#ifndef _RELEASE
...
void CEnvironmentalWeapon::RefreshObjectVelocityTracker(const bool bAllowFallbackHelper)
{
	...
	if(!m_pVelocityTracker && bAllowFallbackHelper)
	{
		...
		DrxLog("[ENV-WEAPON]: unable to locate valid velocity_tracker helper for entity ID[%d] - attempting to fall back on 'damage_zone_helper' ",id);
	}

#ifndef _RELEASE
	if(!m_pVelocityTracker && g_pGameCVars->pl_pickAndThrow.environmentalWeaponImpactDebugEnabled)
	{
		DrxLog("[ENV-WEAPON]: unable to locate valid velocity_tracker helper for entity ID[%d]",id);
	}
#endif
}

void CEnvironmentalWeapon::PostInitClient( i32 channelId )
{
...
void CEnvironmentalWeapon::DelegateAuthorityOnOwnershipChanged(EntityId prevOwnerId, EntityId newOwnerId)
{
	...
	if(gEnv->bServer && prevOwnerId != newOwnerId)
	{
		...
		if(newOwnerId)
		{
			...
			DRX_ASSERT_MESSAGE(pActor, "CEnvironmentalWeapon::DelegateAuthorityOnOwnershipChanged < Something has gone wrong here - perhaps trying to hand ownership to a player that has left the game?"); 
			if(pActor) 
			{
...
void CEnvironmentalWeapon::RegisterListener( IEnvironmentalWeaponEventListener* pListener )
{
	...
	DRX_ASSERT_MESSAGE(iter == m_eventListeners.end(), "CEnvironmentalWeapon::RegisterListener - this listener has already been registered!");
#endif

	DRX_ASSERT_MESSAGE(m_eventListeners.size() < s_maxListeners, "CEnvironmentalWeapon::RegisterListener < ERROR - adding more listeners than supported. Should increase listener container size - Let JONB know");
...
{
	TListenerList::iterator iter = std::find(m_eventListeners.begin(), m_eventListeners.end(), pListener);
	DRX_ASSERT_MESSAGE(iter != m_eventListeners.end(), "CEnvironmentalWeapon::UnregisterListener - this listener is not currently registered!");

	if(iter != m_eventListeners.end())
	{
		m_eventListeners.removeAt(static_cast<u32>(iter - m_eventListeners.begin())); 
	}
}
...
void CEnvironmentalWeapon::CloakSyncWithEntity( const EntityId masterEntityId, const bool forceDecloak )
{
	...
	i32k children = GetEntity()->GetChildCount();
...
void CEnvironmentalWeapon::PerformWeaponSweepTest( const SWeaponTrackSlice& prevSlice, SWeaponTrackSlice& currSlice, const Vec3& eyePos )
{
	...
	if(i32 numEntities = gEnv->pPhysicalWorld->GetEntitiesInBox(wPos- s_radiusAdjust, wPos + s_radiusAdjust, ppEnts, ent_living, sMaxEntitiesToConsiderPerSweep))
	{
		// Calc the sweep test values.
		Vec3 vSweepTests[SWeaponTrackSlice::k_numTrackPoints][2];
		{
			for(i32 sweep=0; sweep<SWeaponTrackSlice::k_numTrackPoints; ++sweep)
			{
...
void CEnvironmentalWeapon::RenderDebugWeaponSweep() const
{
	...
	for(i32 i = 0; i < numSlices; ++i)
	{
		...
		i32k iCurr = i&1;
		for(i32 j = 0; j < SWeaponTrackSlice::k_numTrackPoints; ++j)
		{
		...
		if(pPrevSlice)
		{
			...
			ColorB activeColorFaded   = ColorB((u8)((float)(activeColor.r) * finalLightVal),(u8)((float)(activeColor.g) * finalLightVal),(u8)((float)(activeColor.b) * finalLightVal),255); 
...
tukk CEnvironmentalWeapon::GetClassificationName() const
{
	tukk pClassificationName = NULL;
