...
bool CVehicleMovementStdWheeled::InitPhysics(const CVehicleParams& table)
{
  ...
  m_stabi = m_carParams.kStabilizer;
	...
	{
		DrxLog("Movement Init (%s) - failed to init due to missing <%s> parameter", m_pVehicle->GetEntity()->GetClass()->GetName(), "enginePower");
		return false;
	}
  ...
  m_carParams.pullTilt = DEG2RAD(m_carParams.pullTilt);
	...
	{
		i32 count = min(gearRatiosTable.getChildCount(), 16);
...
void CVehicleMovementStdWheeled::PostInit()
{
  ...
  i32 nParts = m_pVehicle->GetPartCount();
...
void CVehicleMovementStdWheeled::PostPhysicalize()
{
  ...
  if (m_maxSpeed == 0.f)
  {
    ...
    if (g_pGameCVars->v_profileMovement)
      DrxLog("%s maxSpeed: %f", m_pVehicle->GetEntity()->GetClass()->GetName(), m_maxSpeed);
#endif
  }
}
...
//------------------------------------------------------------------------
void CVehicleMovementStdWheeled::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
...
//------------------------------------------------------------------------
void CVehicleMovementStdWheeled::SetEngineRPMMult(float mult, i32 threadSafe)
{
...
void CVehicleMovementStdWheeled::UpdateAxleFriction(float pedal, bool backward, const float deltaTime)
{
  ...
  i32 pedalGearDir = sgn(m_vehicleStatus.iCurGear-1) * sgn(pedal);
...
//  float red[4] = {1,0,0,1};
  ...
  i32 count = m_pVehicle->GetWheelCount();
...
void CVehicleMovementStdWheeled::UpdateSuspension(const float deltaTime)
{
  ...
  i32 numSlip = 0;
  i32 numRot = 0;  
  i32 numDriving = 0, numDrivingContacts = 0;
  m_wheelContactsLeft = 0; 
  m_wheelContactsRight = 0;
  ...
  { 
    ...
    if (ws.bContact != (i32)m_wheelStats[i].bContact)
    {
...
void CVehicleMovementStdWheeled::UpdateSuspensionSound(const float deltaTime)
{
  ...
  i32k numWheels = m_pVehicle->GetWheelCount();
...
{ 
  ...
  { 
    ...
    i32 wheelCount = layer.GetWheelCount(emitterIt->group);
		{
			Vec3 averageWheelPosition(ZERO);
			i32 foundWheels = 0;
			for (i32 w=0; w<wheelCount; ++w)
			{
...
void CVehicleMovementStdWheeled::GetMemoryUsage(IDrxSizer * pSizer) const
{
	pSizer->Add(*this);
	GetMemoryUsageInternal(pSizer);
}

void CVehicleMovementStdWheeled::GetMemoryUsageInternal(IDrxSizer * pSizer) const
{
