...
Описание: Stores all shared weapon parameters (shared by class)...
...
{
	i32k numFiremodes = firemodeParams.size();
	...
	i32k numMeleeParams = meleeParams.size();
	for (i32 i = 0; i < numMeleeParams; ++i)
	{
...
//============================================================
void CWeaponSharedParams::GetMemoryUsage(IDrxSizer *pSizer) const
{
...
void CWeaponSharedParams::ReadWeaponParams(const XmlNodeRef& rootNode, CItemSharedParams* pItemParams, tukk weaponClassName)
{
...
void CWeaponSharedParams::ReadZoomModeParams(const XmlNodeRef& paramsNode, CItemSharedParams* pItemParams)
{
	...
	i32k numZoommodes = reader.GetUnfilteredChildCount();
	...
	for (i32 i = 0; i < numZoommodes; i++)
	{
		...
		tukk typ = (zmParamsNode != NULL) ? zmParamsNode->getAttr("type") : "";
	...
	for (i32 i = 0; i < numZoommodes; i++)
	{
		...
		i32 enabled = 1;
		tukk name = zmParamsNode->getAttr("name");
		tukk typ = zmParamsNode->getAttr("type");
		...
		i32k numAccessoryParams = pItemParams->accessoryparams.size();
		...
		for (i32 j = 0; j < numAccessoryParams; j++)
		{
			...
			{	
				...
				for(i32 k = j; k < numAccessoryParams; k++)
				{
					...
					if(pSecondParams->category != pAccessoryParams->category) //category is an itemstring so only requires pointer comparison
					{
						...
						if(secondDefaultPatchNode || secondZMPatchNode)
						{
							...
							for(i32 l = k; l < numAccessoryParams; l++)
							{
...
void CWeaponSharedParams::ReadFireModeParams(const XmlNodeRef& paramsNode, CItemSharedParams* pItemParams)
{
	...
	i32k numFiremodeParams = firemodeParams.size();

	for (i32 i = 0; i < numFiremodeParams; i++)
	{
		firemodeParams[i].Release();
	}
	...
	i32k numFiremodes = reader.GetUnfilteredChildCount();
	...
	for (i32 i = 0; i < numFiremodes; i++)
	{
		...
		tukk typ = (fmParamsNode != NULL) ? fmParamsNode->getAttr("type") : "";
	...
	for (i32 i = 0; i < numFiremodes; i++)
	{
		...
		tukk name = fmParamsNode->getAttr("name");
		tukk typ = fmParamsNode->getAttr("type");
		...
		{
			DRX_ASSERT_MESSAGE(0, "Multiple melee modes found, this isn't supported");
...
namespace
{
	...
	void ReadPlugin(const CGameXmlParamReader& reader, SFireModeParamsUnpacked* pFireMode, XmlNodeRef actionsNode, tukk nodeName, bool defaultInit)
	{
...
void CWeaponSharedParams::ReadAmmoParams(const XmlNodeRef& paramsNode)
{
	...
	i32k numAmmo = reader.GetUnfilteredChildCount();
	...
	for (i32 i = 0; i < numAmmo; i++)
	{
		...
		if (!strcmpi(ammoNode->getTag(), "ammo"))
		{
			...
			DRX_ASSERT_TRACE(pClass, ("Ammo '%s' class not found in class registry, for weapon", name));
		...
		{
			DRX_ASSERT(ammoParams.extraItems == 0);
...
void CWeaponSharedParams::ReadReloadMagazineParams(const XmlNodeRef& paramsNode)
{
	...
	tukk magazineEvent = paramsNode->getAttr("magazineEvent");
...
void CWeaponSharedParams::ReadBulletBeltParams(const XmlNodeRef& paramsNode)
{
	if (paramsNode)
	{
		...
		tukk pAmmoName = paramsNode->getAttr("ammoType");
		...
		{
			bulletBeltParams.numBullets = (u16)numOfBullets;
		}
	}
}
...
void CWeaponSharedParams::ReadMovementModifierParams(const XmlNodeRef& paramsNode)
{
	...
	i32k numZoommodes = zoommodeParams.size();
...
//------------------------------------------------------------------------
XmlNodeRef CWeaponSharedParams::FindAccessoryPatchForMode(tukk modeType, tukk modeName, const SAccessoryParams* pAccessory)
{
	...
	if (modesNode)
	{
		...
		i32k numOfModes = modesReader.GetUnfilteredChildCount();
		for (i32 i = 0; i < numOfModes; i++)
		{
			XmlNodeRef modePatchNode = modesReader.GetFilteredChildAt(i);
			
			if (modePatchNode)
			{
				tukk name = modePatchNode->getAttr("name");

				if (name!=0 && !stricmp(name, modeName))
				{
					return modePatchNode;
				}
				tukk typ = modePatchNode->getAttr("type");
...
void CWeaponSharedParams::ReadPickAndThrowParams(const XmlNodeRef& paramsNode)
{
	...
	if (paramsNode)
	{
		...
		i32k numTypes = reader.GetUnfilteredChildCount();
		if (numTypes>0)
		{
...
void SPickAndThrowParams::SGrabTypeParams::Read( const XmlNodeRef& paramsNode )
{
	...
	tukk pAudioSignalName = reader.ReadParamValue("audioSignalHitOnNPC");
	...
	DRX_ASSERT( meleeAction_duration >= melee_delay );
	...
	bool bContinue = true; 
	i32 i = 1; 
	DrxFixedStringT<32> comboStringName;
	DrxFixedStringT<32> comboRecoveryStringName;
	while(bContinue)
	{
	...
	DRX_ASSERT(vehicleThrowMinVelocity < vehicleThrowMaxVelocity);
	...
	reader.ReadParamValue<i32>("shieldingHits", shieldingHits );
	reader.ReadParamValue<i32>("crossHairType", crossHairType, (i32)eHCH_None);
...
void SPickAndThrowParams::SGrabTypeParams::CacheResources()
{
	if (!dbaFile.empty())
	{
		...
		mannequinSys.GetAnimationDatabaseUpr().Load(dbaFile.c_str());
	}
}
...
void SPickAndThrowParams::SGrabTypeParams::ReadCameraShake( const XmlNodeRef& params, SPickAndThrowParams::SCameraShakeParams* pCameraShake )
{
	if(pCameraShake)
	{
		...
		i32 viewAngleAttenuation = 1;
		i32 randomDirection = 0;
...
void CWeaponSharedParams::ReadAIParams(const XmlNodeRef& paramsNode)
{
	...
	i32k numAmmoTypes = ammoParams.ammo.size();
...
void CWeaponSharedParams::ReadAIOffsets(const XmlNodeRef& paramsNode)
{
	...
	{
		i32k offsetCount = reader.GetUnfilteredChildCount();
		for (i32 i = 0; i < offsetCount; ++i)
		{
			...
			if (offsetNode)
			{
				tukk stance = offsetNode->getAttr("stanceId");
				// retrieve enum value from scripts, using stance name
				i32 curStanceInt(STANCE_NULL);
...
{
	i32k numAccessoryParams = meleeParams.size();
...
void STurretSearchParams::Reset( const XmlNodeRef& paramsNode, bool defaultInit /*= true*/ )
{
	...
	if (paramsNode)
	{
		...
		if (hintsNode && hintsNode->getChildCount())
		{
			...
			Vec2 hintEntry;
			i32k hintCount = hintsReader.GetUnfilteredChildCount();
			hints.reserve(hintCount);

			for (i32 i = 0; i < hintCount; ++i)
			{
...
void SZoomParams::Reset( const XmlNodeRef& paramsNode, bool defaultInit /*= true*/ )
{
	...
	if (paramsNode)
	{
		...
		if (stagesNode)
		{
			...
			stages.resize(0); 
			i32k stagesCount = stagesReader.GetUnfilteredChildCount();
...
void SC4Params::Reset(const XmlNodeRef& paramsNode, bool defaultInit)
{
	...
	if (paramsNode)
	{
		...
		reader.ReadParamValue<i32>("armed_light_material_index", armedLightMatIndex);
