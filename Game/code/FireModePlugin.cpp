...
DRX_IMPLEMENT_GTI_BASE(IFireModePlugin);


//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////

DRX_IMPLEMENT_GTI(CFireModePlugin_Overheat, IFireModePlugin);
...
//------------------------------------------------------------------------
bool CFireModePlugin_Overheat::Update(float frameTime, u32 frameId)
{
	...
	else
	{
		...
		if (m_heat >= 0.999f && oldheat<0.999f)
		{
			...
			i32 slot = pWeapon->GetStats().fp ? eIGS_FirstPerson : eIGS_ThirdPerson;
...
DRX_IMPLEMENT_GTI(CFireModePlugin_Reject, IFireModePlugin);
...
void CFireModePlugin_Reject::Activate(bool activate)
{
	if (m_shellFXSpeed != 0.0f && !activate)
	{
		...
		IParticleEffect* pParticleEffect = gEnv->pParticleUpr->FindEffect(rejectEffect.effect[0].c_str());
		if (pParticleEffect)
		{
		...
		pParticleEffect = gEnv->pParticleUpr->FindEffect(rejectEffect.effect[1].c_str());
		if (pParticleEffect)
		{
			ParticleParams particleParams = pParticleEffect->GetParticleParams();
			particleParams.fSpeed = m_shellFXSpeed;
...
void CFireModePlugin_Reject::OnShoot()
{
	...
	i32k slot = pWeapon->GetStats().fp ? 0 : 1;
	...
	if (doRejectEffect)
	{
		...
		IParticleEffect* pParticleEffect = gEnv->pParticleUpr->FindEffect(rejectEffect.effect[slot].c_str());
...
//------------------------------------------------------------------------
bool CFireModePlugin_Reject::Update(float frameTime, u32 frameId)
{
	CWeapon* pWeapon = m_pOwnerFiremode->GetWeapon();
...
DRX_IMPLEMENT_GTI(CFireModePlugin_RecoilShake, IFireModePlugin);
