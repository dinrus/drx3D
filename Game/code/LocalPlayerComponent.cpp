...
#ifndef _RELEASE
static i32		g_spectate_follow_debug_enable;
static i32		g_spectate_follow_debug_setDebugSettingsToCurrentViewSettings;
...
static i32		g_spectate_follow_debug_allowOrbitYaw;
static i32		g_spectate_follow_debug_allowOrbitPitch;
static i32		g_spectate_follow_debug_useEyeDir;
static i32		g_spectate_follow_debug_userSelectable;
#endif //_RELEASE
...
{
	i32 numSettings = pFollowCameraNode->GetChildCount();
	...
	for(i32 i = 0; i < numSettings; ++i)
	{
		...
		i32 allowOrbitYaw = 0;
		i32 allowOrbitPitch = 0;
		i32 useEyeDir = 0;
		i32 userSelectable = 0;
		i32 disableHeightAdj = 0;
		...
		DRX_ASSERT_MESSAGE(success, string().Format("CLocalPlayerComponent::InitFollowCameraSettings - Invalid follow camera settings - mode %i. Check XML data.", i+1));
...
void CLocalPlayerComponent::UpdateFPBodyPartsVisibility()
{
	if (m_rPlayer.m_stats.isThirdPerson == false)
	{
		...
		IAttachmentUpr *pAttachmentUpr		= pMainCharacter ? pMainCharacter->GetIAttachmentUpr() : NULL;

		if (pAttachmentUpr)
		{
			DRX_ASSERT(m_rPlayer.HasBoneID(BONE_CALF_L));
			DRX_ASSERT(m_rPlayer.HasBoneID(BONE_CALF_R));
			...
			i32k kNumParstToToggle = 2;
			tukk tooglePartsTable[kNumParstToToggle] = {"lower_body", "upper_body"};
			for (i32 i=0; i<kNumParstToToggle; i++)
			{
				IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByName(tooglePartsTable[i]);
				if (pAttachment)
				{
...
void CLocalPlayerComponent::InitSoundmoods()
{
	...
	tukk lowHealthEnter = gEnv->bMultiplayer ? "Player_LowHealth_Enter_MP" : "Player_LowHealth_Enter";
	tukk lowHealthExit = gEnv->bMultiplayer ? "Player_LowHealth_Exit_MP" : "Player_LowHealth_Exit";
...
void CLocalPlayerComponent::UpdateFPIKTorso(float fFrameTime, IItem * pCurrentItem, const Vec3& cameraPosition)
{
	...
	{
		DrxWatch("CPlayer:UpdateFPIKTorso: RawCamera Pos(%f, %f, %f) Rot(%f, %f, %f, %f)", cameraTran.t.x, cameraTran.t.x, cameraTran.t.x, cameraTran.q.v.x, cameraTran.q.v.y, cameraTran.q.v.z, cameraTran.q.w );
		DrxWatch("CPlayer:UpdateFPIKTorso: MountedTarget (%f, %f, %f)", m_rPlayer.m_params.mountedWeaponCameraTarget.x, m_rPlayer.m_params.mountedWeaponCameraTarget.y, m_rPlayer.m_params.mountedWeaponCameraTarget.z );
		DrxWatch("CPlayer:UpdateFPIKTorso: CamPos(%f, %f, %f) Dir(%f, %f, %f)", cameraPosition.x, cameraPosition.y, cameraPosition.z, info.aimDirection.x, info.aimDirection.y, info.aimDirection.z);
		DrxWatch("CPlayer:UpdateFPIKTorso: Anim Pos(%f, %f, %f) Rot(%f, %f, %f, %f)", m_lastSTAPCameraDelta.t.x, m_lastSTAPCameraDelta.t.y, m_lastSTAPCameraDelta.t.z, m_lastSTAPCameraDelta.q.v.x, m_lastSTAPCameraDelta.q.v.y, m_lastSTAPCameraDelta.q.v.z, m_lastSTAPCameraDelta.q.w);
		if (g_pGameCVars->pl_debug_view >= 2)
		{
			DrxLog("CPlayer:UpdateFPIKTorso: CamPos(%f, %f, %f) Dir(%f, %f, %f)", cameraPosition.x, cameraPosition.y, cameraPosition.z, info.aimDirection.x, info.aimDirection.y, info.aimDirection.z);
			DrxLog("CPlayer:UpdateFPIKTorso: EyeOffset(%f, %f, %f)", m_rPlayer.m_eyeOffset.x, m_rPlayer.m_eyeOffset.y, m_rPlayer.m_eyeOffset.z);
		}
...
void CLocalPlayerComponent::UpdatePlayerLowHealthStatus( const float oldHealth )
{
	...
	DRX_ASSERT( maxHealth > minHealth );
...
const SFollowCameraSettings& CLocalPlayerComponent::GetCurrentFollowCameraSettings() const
{
	...
	DRX_ASSERT(m_currentFollowCameraSettingsIndex >= 0 && m_currentFollowCameraSettingsIndex < (int8)m_followCameraSettings.size());
...
{
	i32k initial = m_currentFollowCameraSettingsIndex;
	bool valid = false;
	do
	{
...
bool CLocalPlayerComponent::SetCurrentFollowCameraSettings( u32 crcName )
{
	i32k total = m_followCameraSettings.size();
