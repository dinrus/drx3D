...
#else
#define PR_CHECKQNAN_FLT(x) (uk )0
#endif
...
static void GetTalosInput(CPlayerRotation * pPlayerRotation, const CPlayer& rPlayer, float& x, float& z, const Vec3 playerView[4], float fFrameTime)
{
	...
	i32 playerTeam = pGameRules->GetTeam(rPlayer.GetEntity()->GetId());
	float cloakedPlayerMultiplier = g_pGameCVars->pl_aim_cloaked_multiplier;
	const bool multipleTeams = pGameRules->GetTeamCount() > 0;

	const TAutoaimTargets& aaTargets = g_pGame->GetAutoAimUpr().GetAutoAimTargets();
	i32k targetCount = aaTargets.size();
	...
	for (i32 i = 0; i < targetCount; ++i)
	{
		...
		i32k kAutoaimVisibilityLatency = 1;
	...
	if(follow_target_id != 0)
	{
		...
		//DrxWatch("Angle to target: %.6f", RAD2DEG(angle));
		//DrxWatch("Final Angle to target: %.6f", RAD2DEG(fHorzFinalAngle));
...
void CPlayerRotation::Process(IItem* pCurrentItem, const SActorFrameMovementParams& movement, const SAimAccelerationParams& verticalAcceleration, float frameTime)
{
	...
	if(stricmp(g_pGameCVars->pl_talos->GetString(), m_player.GetEntity()->GetName()) == 0)
	{
		...
		DRX_ASSERT(pMovementController);
...
void CPlayerRotation::TargetAimAssistance(CWeapon* pWeapon, float& followH, float& followV, float& scale, float& _fZoomAmount, const Vec3 playerView[4])
{
	...
	i32 playerTeam = pGameRules->GetTeam(m_player.GetEntity()->GetId());
	float cloakedPlayerMultiplier = g_pGameCVars->pl_aim_cloaked_multiplier;
	const bool multipleTeams = pGameRules->GetTeamCount() > 0;
	...
	for (i32 i = 0; i < targetCount; ++i)
	{
		...
		DRX_ASSERT(target.entityId != m_player.GetEntityId());
		...
		i32k kAutoaimVisibilityLatency = 2;
...
void CPlayerRotation::ProcessTargetAssistance( IItem* pCurrentPlayerItem )
{
	...
	{
		DRX_ASSERT(pIPlayerInput->GetType() == IPlayerInput::PLAYER_INPUT);
	...
	const EntityId closeCombatSnapTargetId = g_pGame->GetAutoAimUpr().GetCloseCombatSnapTarget();
...
void CPlayerRotation::ProcessLeanAndPeek( const SActorFrameMovementParams& movement )
{
	...
	else
	{
		...
		i32k rayFlags(rwi_stop_at_pierceable|rwi_colltype_any);
	...
	// TODO(M�rcio): Maybe do some checks here!
...
{
	const SAutoaimTarget* pAATarget = g_pGame->GetAutoAimUpr().GetTargetInfo(snapTargetId);
	if(pAATarget)
	{
...
const Vec3& CPlayerRotation::SLeanAndPeekInfo::GetLeanLimit( const Vec3 &startPos, const Vec3 &dir, i32 objTypes, i32 flags, IPhysicalEntity **pSkipEnts /*= NULL*/, i32 nSkipEnts /*= 0*/ )
{
