...
const static i32 LABEL_LENGTH = 100;
const static i32 BUFF_SIZE = LABEL_LENGTH + 3;

CCodeCheckpointDebugMgr* s_pCodeCheckPointDebugUpr = NULL;
...
void CmdCodeCheckPointSearch(IConsoleCmdArgs *pArgs)
{
	if(pArgs->GetArgCount() > 2)
		DrxLogAlways("Usage: %s <substring>", pArgs->GetArg(0));
	...
	DrxLogAlways("Search with substring:%s return the following %" PRISIZE_T " hit checkpoints", searchString.c_str(), foundPointNames.size());
	for(CCodeCheckpointDebugMgr::RecordNameCountPairs::iterator fIt = foundPointNames.begin(); fIt != foundPointNames.end(); ++fIt)
		DrxLogAlways("Checkpoint:%s Count:%i", fIt->first.c_str(), fIt->second);
}

CCodeCheckpointDebugMgr* CCodeCheckpointDebugMgr::s_pCodeCheckPointDebugUpr = NULL;

CCodeCheckpointDebugMgr* CCodeCheckpointDebugMgr::RetrieveCodeCheckpointDebugMgr()
{
	if(!s_pCodeCheckPointDebugUpr)
		s_pCodeCheckPointDebugUpr = new CCodeCheckpointDebugMgr();

	return s_pCodeCheckPointDebugUpr;
}
...
void CCodeCheckpointDebugMgr::ReadFile(tukk fileName)
{
	FILE * cpFile = gEnv->pDrxPak->FOpen(fileName, "r");
	...
	if(cpFile)
	{
		...
		tuk lineBlock = new char[BUFF_SIZE + 1];
		//tuk appendBlock = lineBlock;

		while(i32 numRead = GetLine(lineBlock, cpFile))
		{
			ICodeCheckpointMgr* pCheckpointUpr = gEnv->pCodeCheckpointMgr;
			if(pCheckpointUpr)
				pCheckpointUpr->GetCheckpointIndex(lineBlock);
			RegisterWatchPoint(lineBlock);
		}

		SAFE_DELETE_ARRAY(lineBlock);

		gEnv->pDrxPak->FClose(cpFile);

	}
}


///Getline wrapper to break on newlines as adapted from the old AI Code Coverage Upr
i32 CCodeCheckpointDebugMgr::GetLine( char * pBuff, FILE * fp )
{	
	DRX_ASSERT(pBuff && fp);

	/// Wraps fgets to remove newlines and use correct buffer/string lengths

	/// Must use DrxPak FGets on DrxPak files
	/// Will retrieve up to the next newline character in the file buffer.
	if (!gEnv->pDrxPak->FGets( pBuff, BUFF_SIZE, fp ))
		return 0;

	/// Convert newlines to \0 and discover length
	i32 i=0;
	i32 nLimit = LABEL_LENGTH + 1;
	for (; i < nLimit ; i++)
	{
		char c = pBuff[i];
		if (c == '\n' || c == '\r' || c== '\0') 
...
{
	i32 displayLevel = m_debug_ccoverage;
	
	IRenderer* pRenderer = gEnv->pRenderer;

	i32 totalHit = (i32) std::count_if(m_unwatchedPoints.begin(), m_unwatchedPoints.end(), RecordHasHits);
	i32 watchedHit = (i32) std::count_if(m_watchedPoints.begin(), m_watchedPoints.end(), RecordHasHits);
	...
	i32 numberOutput = 0;
	i32 maxNumberOutput =  m_debug_ccoverage_maxlines;
		...
		++outputIt)
	{
		...
		i32 filterMin = m_debug_ccoverage_filter_mincount, filterMax = m_debug_ccoverage_filter_maxcount;
		if(filterMax && (i32)outputIt->m_currHitcount > filterMax)
			continue;
		else if(filterMin && (i32)outputIt->m_currHitcount < filterMin)
			continue;
...
void CCodeCheckpointDebugMgr::UpdateRecords()
{
	...
	if (pCodeCheckpointMgr)
	{
		...
		{
			tukk name = pCodeCheckpointMgr->GetCheckPointName(currIdx);
			const CCodeCheckpoint* pCheckpoint = pCodeCheckpointMgr->GetCheckpoint(currIdx);
