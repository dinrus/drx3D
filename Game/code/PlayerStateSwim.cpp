...
static i32k s_maxWaterVolumesToConsider = 2;
...
bool CPlayerStateSwim::OnPrePhysicsUpdate( CPlayer& player, const SActorFrameMovementParams& movement, float frameTime )
{
	...
	if (i32 count = gEnv->pPhysicalWorld->CheckAreas(entityPos, gravity, &buoyancy[0], s_maxWaterVolumesToConsider))
	{
		for(i32 i = 0; i < count && i < s_maxWaterVolumesToConsider; ++i)
		{
			// 0 is water
