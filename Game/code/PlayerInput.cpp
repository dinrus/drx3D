...
#include <drx3D/Game/UI/HUD/HUDSilhouettes.h>
...
#include <drx3D/Game/IUIDraw.h>

#include <drx3D/Game/EntityUtility/EntityScriptCalls.h>
#include <drx3D/Game/PlayerEntityInteraction.h>
...
//////////////////////////////////////////////////////////////////////////
void CPlayerInput::OnAction( const ActionId& actionId, i32 activationMode, float value )
{
	...
	DrxLogAlways ("$7PLAYER INPUT:$o <FRAME %05d> $6%s $%c[MODE=0x%x] $4value=%.3f$o", gEnv->pRenderer->GetFrameID(false), actionId.c_str(), (char) ((activationMode > 7) ? '8' : (activationMode + '1')), activationMode, value);
#endif
...
const bool CPlayerInput::AllowToggleWeapon(i32k activationMode, const float currentTime)
{
...
{
	//DrxWatch("deltaRot=%f,%f,%f; deltaMove=%f,%f,%f", deltaRotation.x, deltaRotation.y, deltaRotation.z, deltaMovement.x, deltaMovement.y, deltaMovement.z);
	...
	if (g_pGameCVars->g_detachedCameraDebug)
	{
		...
		for (i32 num=0; num < MAX_FREE_CAM_DATA_POINTS; ++num)
		{
...
void CPlayerInput::Update()
{
	...
	if(m_autoPickupMode)
	{
		...
		{
			const CGameActions& actions = g_pGame->Actions();
			OnActionPrePickUpItem(interactionInfo.interactiveEntityId, CDrxName("itemPickup"), eAAM_OnHold, 0.f);
			CPlayerEntityInteraction& playerEntityInteractor = m_pPlayer->GetPlayerEntityInteration();
...
void CPlayerInput::PostUpdate()
{
	...
	if (pGameRules && pEnt && pEnt->GetStatus(&psl) && psl.pGroundCollider)
	{
		...
		if (pGroundEntity)
		{
			i32 id = pGroundEntity->GetId();
			if (pGameRules->GetVTOLVehicleUpr() && pGameRules->GetVTOLVehicleUpr()->IsVTOL(id))
				standingOn = id;
		}
	}
...
bool CPlayerInput::OnActionMoveForward(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionMoveBack(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	bool bClearDeltaMovement = false;
	if (CanMove() || (activationMode == eAAM_OnRelease))
	{
...
bool CPlayerInput::OnActionMoveLeft(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	bool bClearDeltaMovement = false;
	if (CanMove() || (activationMode == eAAM_OnRelease))
	{
...
bool CPlayerInput::OnActionMoveRight(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	bool bClearDeltaMovement = false;
	if (CanMove() || (activationMode == eAAM_OnRelease))
	{
...
bool CPlayerInput::OnActionRotateYaw(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionRotatePitch(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionHMDRotateYaw(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionHMDRotatePitch(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionHMDRotateRoll(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionVRotateYaw(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	m_deltaRotation.z -= value;
...
bool CPlayerInput::OnActionVRotatePitch(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	m_deltaRotation.x -= value;
	if(g_pGameCVars->cl_invertMouse)
...
bool CPlayerInput::OnActionJump(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::PerformJump()
{
	...
	{
		DrxLog("			> either can't move or is spectating, aborting.");
		return false;
	}
...
#ifndef _RELEASE
	DRX_ASSERT_TRACE (m_inputCancelHandler[slot] == NULL, ("%s already has an input cancel handler \"%s\" installed in slot %d - can't add \"%s\" too", m_pPlayer->GetEntity()->GetEntityTextDescription(), m_inputCancelHandler[slot]->DbgGetCancelHandlerName().c_str(), slot, handler->DbgGetCancelHandlerName().c_str()));
#endif

	m_inputCancelHandler[slot] = handler;
}

bool CPlayerInput::OnActionCrouch(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	...
	u32 oldCrouchAction = (m_actions & ACTION_CROUCH);
...
bool CPlayerInput::OnActionSprint(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionSprintXI(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
{
	SInputEventData inputEventData( SInputEventData::EInputEvent_Sprint, m_pPlayer->GetEntityId(), CDrxName("sprint"), eAAM_OnRelease, 0.0f );
	m_pPlayer->StateMachineHandleEventMovement( SStateEventPlayerInput( &inputEventData ) );
}


bool CPlayerInput::OnActionAttackRightTrigger(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (!m_pPlayer->IsDead())
	{
...
bool CPlayerInput::OnActionUse(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	bool filterOut = true;
	IVehicle* pVehicle = m_pPlayer->GetLinkedVehicle();
...
bool CPlayerInput::OnActionThirdPerson(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
#ifdef INCLUDE_DEBUG_ACTIONS
bool CPlayerInput::OnActionFlyMode(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
 	if (!gEnv->pSystem->IsDevMode())
		return false;
	...
	if (!m_pPlayer->GetSpectatorMode() && m_pPlayer->m_pGameFramework->CanCheat())
	{
		u8 flyMode=m_pPlayer->GetFlyMode()+1;
		m_pPlayer->StateMachineHandleEventMovement( SStateEventFly( flyMode ) );
	}
	return false;
}
#endif
bool CPlayerInput::OnActionGodMode(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (!gEnv->pSystem->IsDevMode())
		return false;
...
bool CPlayerInput::OnActionAIDebugDraw(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (!gEnv->pSystem->IsDevMode())
		return false;
...
bool CPlayerInput::OnActionPDrawHelpers(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (!gEnv->pSystem->IsDevMode())
		return false;
...
bool CPlayerInput::OnActionDMode(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (!gEnv->pSystem->IsDevMode())
		return false;
...
bool CPlayerInput::OnActionRecordBookmark(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
#ifdef INCLUDE_DEBUG_ACTIONS
bool CPlayerInput::OnActionMannequinDebugAI(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (!gEnv->pSystem->IsDevMode())
		return false;
...
bool CPlayerInput::OnActionMannequinDebugPlayer(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (!gEnv->pSystem->IsDevMode())
		return false;
...
bool CPlayerInput::OnActionAIDebugCenterViewAgent(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionXIRotateYaw(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionXIRotatePitch(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionXIMoveX(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (CanMove())
	{
...
bool CPlayerInput::OnActionXIMoveY(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (CanMove())
	{
...
bool CPlayerInput::OnActionXIDisconnect(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	m_xi_deltaRotation.Set(0,0,0);
	m_xi_deltaMovement.zero();
	m_bUseXIInput = false;
...
bool CPlayerInput::OnActionInvertMouse(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionFlyCamMoveX(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionFlyCamMoveY(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionFlyCamMoveUp(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionFlyCamMoveDown(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	m_flyCamDeltaMovement.z = -value;
...
bool CPlayerInput::OnActionFlyCamSpeedUp(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if(activationMode == eAAM_OnPress)
	{
...
bool CPlayerInput::OnActionFlyCamSpeedDown(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if(activationMode == eAAM_OnPress)
	{
		g_pGameCVars->g_detachedCameraMoveSpeed = CLAMP(g_pGameCVars->g_detachedCameraMoveSpeed - 0.5f, 0.5f, 30.f);
...
bool CPlayerInput::OnActionFlyCamTurbo(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if(activationMode == eAAM_OnPress)
	{
		m_flyCamTurbo=true;
...
bool CPlayerInput::OnActionFlyCamRotateYaw(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionFlyCamRotatePitch(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionFlyCamSetPoint(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
#if FREE_CAM_SPLINE_ENABLED
	if(activationMode == eAAM_OnPress)
	{
...
void CPlayerInput::AddFlyCamPoint(Vec3 pos, Vec3 lookAtPos)
{
	...
	if (m_freeCamCurrentIndex < MAX_FREE_CAM_DATA_POINTS)
	{
		...
		DrxLog("FREECAM Added new FreeCamPointData - index:%d pos:%f %f %f lookAt:%f %f %f", m_freeCamCurrentIndex, pos.x, pos.y, pos.z, lookAtPos.x, lookAtPos.y, lookAtPos.z);

		++m_freeCamCurrentIndex;
	}
	else
	{
		DrxLog("FREECAM Have reached the maximum (%d) num of FreeCamPointData, no more can be added!", MAX_FREE_CAM_DATA_POINTS);
	}
#endif
}
...
void CPlayerInput::FlyCamPlay()
{
	...
	if (m_freeCamPlaying)
	{
		...
		if (num > 0)
			DrxLog("FREECAM Starting to play free cam spline numpoints:%d distance:%f totaltime:%f", num, totalDistance, m_freeCamTotalPlayTime);
		else
			DrxLog("FREECAM Cannot start playing free cam spline - no points set");
	}
	else
	{
		DrxLog("FREECAM Stop playing free cam spline");
	}
#endif
}

bool CPlayerInput::OnActionFlyCamPlay(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if(activationMode == eAAM_OnPress)
	{
		FlyCamPlay();
...
bool CPlayerInput::OnActionFlyCamClear(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
#if FREE_CAM_SPLINE_ENABLED
	if(activationMode == eAAM_OnPress)
	{
		for (i32 i=0; i<MAX_FREE_CAM_DATA_POINTS; ++i)
		{
...
bool CPlayerInput::OnActionSpecial(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionChangeFireMode(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionToggleVision(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	m_openingVisor = false;
	return false;
}

bool CPlayerInput::OnActionMindBattle(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
...
bool CPlayerInput::OnActionLookAt( EntityId entityId, const ActionId& actionId, i32 activationMode, float value )
{
...
bool CPlayerInput::OnActionPrePickUpItem( EntityId entityId, const ActionId& actionId, i32 activationMode, float value )
{
	const SInteractionInfo& interactionInfo = m_pPlayer->GetCurrentInteractionInfo(); 
	...
	else if( IInventory* pInventory = m_pPlayer->GetInventory() )
	{
		...
		{
			tukk category = g_pGame->GetIGameFramework()->GetIItemSystem()->GetItemCategory(pWeapon->GetEntity()->GetClass()->GetName());
...
//////////////////////////////////////////////////////////////////////////
bool CPlayerInput::OnActionMoveOverlayTurnOn( EntityId entityId, const ActionId& actionId, i32 activationMode, float value )
{
...
//////////////////////////////////////////////////////////////////////////
bool CPlayerInput::OnActionMoveOverlayTurnOff( EntityId entityId, const ActionId& actionId, i32 activationMode, float value )
{
...
//////////////////////////////////////////////////////////////////////////  
bool CPlayerInput::OnActionMoveOverlayWeight( EntityId entityId, const ActionId& actionId, i32 activationMode, float value )
{
...
//////////////////////////////////////////////////////////////////////////  
bool CPlayerInput::OnActionMoveOverlayX( EntityId entityId, const ActionId& actionId, i32 activationMode, float value )
{
...
//////////////////////////////////////////////////////////////////////////  
bool CPlayerInput::OnActionMoveOverlayY( EntityId entityId, const ActionId& actionId, i32 activationMode, float value )
{
  m_moveOverlay.moveY = clamp_tpl(-value, -1.0f, 1.0f);
  return true;
}

bool CPlayerInput::OnActionRespawn(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if (m_pPlayer->IsDead())
	{
...
bool CPlayerInput::OnActionMouseWheelClick(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	if(activationMode == eAAM_OnPress)
	{
...
void CPlayerInput::AdjustMoveButtonState(EMoveButtonMask buttonMask, i32 activationMode )
{
...
bool CPlayerInput::CheckMoveButtonStateChanged(EMoveButtonMask buttonMask, i32 activationMode)
{
...
void CPlayerInput::ClearAllExceptAction( u32 actionFlags )
{
	u32 actionsToKeep = m_actions & actionFlags;
	Reset();
	m_actions |= actionsToKeep;
}

bool CPlayerInput::OnActionSelectNextItem(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
{
	const CGameActions& actions = g_pGame->Actions();
	...
	if(allowSwitch && !CallTopCancelHandler(kCancelPressFlag_switchItem))
	{
			...
			if (itemId)
			{
				...
				if (pWeapon)
				{
					...
					if (pEntityClass != NULL)
					{
						...
						tukk category = g_pGame->GetIGameFramework()->GetIItemSystem()->GetItemCategory(pEntityClass->GetName());
						i32 categoryType = GetItemCategoryType(category);
		...
		if (allowSwitch)
		{
			...
			{
				i32 category = gEnv->bMultiplayer ? eICT_Grenade|eICT_Explosive : eICT_Grenade;

				SelectNextItem(1, true, category, suitVisorOn);
			}
			else if (actions.toggle_explosive == actionId)
			{
				i32 primaryCategory = eICT_Primary | eICT_Secondary;
				i32 secondaryCategory = gEnv->bMultiplayer ? eICT_Grenade|eICT_Explosive : eICT_Explosive;
			...
			{
				i32 primaryCategory = eICT_Primary | eICT_Secondary;
				i32 secondaryCategory = eICT_Special;
				ToggleItem(primaryCategory, secondaryCategory, suitVisorOn);
			}
			else if (actions.toggle_weapon ==actionId)
			{
				...
				else
				{
					...
					i32k numOptions = m_pPlayer->GetInventory()->GetSlotCount(IInventory::eInventorySlot_Weapon);
			...
			else if (actions.debug==actionId)
			{
				...
				i32 currentItemCategory = GetItemCategoryType(actionId.c_str());
...
bool CPlayerInput::OnActionQuickGrenadeThrow( EntityId entityId, const ActionId& actionId, i32 activationMode, float value )
{
	...
	if (actions.grenade == actionId ||(gEnv->bMultiplayer && actions.xi_grenade == actionId))
	{
		...
		i32k grenadeCategories = eICT_Grenade;
			...
			if(bCanActivate)
			{
				...
				if (pInventory)
				{
					...
					for (i32 i = 0; i < numItems; i ++)
					{
						...
						{
							tukk category = pItemSystem->GetItemCategory(pItem->GetEntity()->GetClass()->GetName());
							i32 categoryType = GetItemCategoryType(category);	
...
bool CPlayerInput::DoubleTapGrenadeAvailable()
{
	...
	{
		IItemSystem* pItemSystem = g_pGame->GetIGameFramework()->GetIItemSystem();
		i32 grenadeCategories = DoubleTapGrenadeCategories();
...
void CPlayerInput::SelectNextItem( i32 direction, bool keepHistory, i32 category, bool disableVisorFirst )
{
	m_pPlayer->SelectNextItem(direction, keepHistory, category);
}

void CPlayerInput::ToggleItem(i32 primaryCategory, i32 secondaryCategory, bool disableVisorFirst)
{
	IItemSystem* pItemSystem = g_pGame->GetIGameFramework()->GetIItemSystem();
	IItem* pCurrentItem = m_pPlayer->GetCurrentItem();
	if (pCurrentItem)
	{
		tukk name = pCurrentItem->GetEntity()->GetClass()->GetName();
		tukk nextWeaponCatStr = pItemSystem->GetItemCategory(name);
		i32 currentCategory = GetItemCategoryType(nextWeaponCatStr);
		i32 nextCategory = secondaryCategory;
...
bool CPlayerInput::IsItemPickUpScriptAction(const ActionId& actionId) const
{
	...
	i32k numEvents = DRX_ARRAY_COUNT(itemPickUpEvents);
...
void SSerializedPlayerInput::Serialize( TSerialize ser, EEntityAspects aspect )
{
	...
	else if (aspect & CPlayer::ASPECT_INPUT_CLIENT_AUGMENTED)
	{
		...
		// Did we get a valid entity? This is a failsafe for late joiners to get DrxNetwork to send the data again!
