...
void SRecoilParams::GetMemoryUsage( IDrxSizer * s ) const 
{
...
void SRecoilHints::GetMemoryUsage(IDrxSizer * s) const
{
...
void SProceduralRecoilParams::Reset( const XmlNodeRef& paramsNode, bool defaultInit /* = true */ )
{
	...
	reader.ReadParamValue<i32>("arms", arms);
...
void CRecoil::Update(CActor* pOwnerActor, float frameTime, bool weaponFired, i32 frameId, bool firstShot)
{
...
void CRecoil::RecoilShoot(bool firstShot, float maxRecoil)
{
	...
	i32k recoilHintsCount = m_pRecoilHints ? m_pRecoilHints->hints.size() : 0;
	...
	const Vec3 randomDirectionAdd = Vec3(
		drx_random(0.0f, m_recoilParams.randomness),
		drx_random(-1.0f, 1.0f) * m_recoilParams.randomness,
		drx_random(0.0f, m_recoilParams.tilt));
...
void CRecoil::GetMemoryUsage(IDrxSizer* s) const
{	
