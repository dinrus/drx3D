...
#include <drx3D/Game/PersistantDebug.h> // to show target point
#include <drx3D/Game/ExactPositioning.h>
...
bool CExactPositioning::SetActorTarget( const SActorTargetParams& actorTargetParams )
{
	...
	DRX_ASSERT( actorTargetParams.location.GetLength() > 0.1f );
	DRX_ASSERT( actorTargetParams.direction.GetLength() > 0.5f );
	DRX_ASSERT( fabs( actorTargetParams.startArcAngle ) <= gf_PI );
	DRX_ASSERT( actorTargetParams.directionTolerance >= 0.0f );
	DRX_ASSERT( actorTargetParams.prepareRadius > 1.0f );
	DRX_ASSERT( actorTargetParams.startWidth >= 0.0f );
	DRX_ASSERT( actorTargetParams.pQueryStart != NULL );
	DRX_ASSERT( actorTargetParams.pQueryEnd != NULL );

	DRX_ASSERT( actorTargetParams.pAction != NULL );
...
void CExactPositioning::StateMachine_Initialize( ETriggerState newState )
{
	...
	{
		DRX_ASSERT(false); // recursion not allowed!
		return;
	}
	...
	do
	{
		...
		DRX_ASSERT(m_state <= eTS_Completing);
...
CExactPositioning::ETriggerState CExactPositioning::StateWaiting_HandleEvent( SStateMachineEvent& event )
{
	...
	{
	case ESME_Enter:
		DRX_ASSERT( !m_pExactPositioningTarget->preparing );
		DRX_ASSERT( !m_pExactPositioningTarget->activated );
		break;
...
CExactPositioning::ETriggerState CExactPositioning::StateRunning_HandleEvent( SStateMachineEvent& event )
{
	...
	case ESME_Update:
		...
		{
		case IAction::Pending:
			DRX_ASSERT( false ); // should not happen, we already entered before, but paranoia!
			return eTS_Disabled;
			break;
		case IAction::Installed:
			break;
...
CExactPositioning::ETriggerState CExactPositioning::StateCompleting_HandleEvent( SStateMachineEvent& event )
{
	...
	case ESME_Update:
		DRX_ASSERT(false); // this whole state seems unnecessary
		return eTS_Disabled;
...
void CExactPositioning::UpdateTargetPointToFinishPoint() 
{
	...
	if ( m_pExactPositioningTarget->isNavigationalSO && movement.translation.IsZero( FLT_EPSILON ) )
	{
		...
		tukk fragmentName = pActionController->GetContext().controllerDef.m_fragmentIDs.GetTagName( m_fragmentID );
#else
		tukk fragmentName = "<n/a>";
#endif
	...
	*/
	...
	if (m_state == eTS_Completing)
	{
		...
		{
			ray_hit hit;
			i32 rayFlags = rwi_stop_at_pierceable|(geom_colltype_player<<rwi_colltype_bit);
		...
		DRX_ASSERT(m_pExactPositioningTarget->location.IsValid());
		DRX_ASSERT(NumberValid(m_pExactPositioningTarget->startWidth));
		DRX_ASSERT(NumberValid(m_pExactPositioningTarget->positionWidth));
		DRX_ASSERT(NumberValid(m_pExactPositioningTarget->positionDepth));
		DRX_ASSERT(NumberValid(m_pExactPositioningTarget->orientationTolerance));
	}
...
/*
// ----------------------------------------------------------------------------
void CExactPositioning::DebugDraw( IRenderer * pRend, i32& x, i32& y, i32 YSPACE )
{
	...
	if (m_ds.state != eTS_Disabled)
	{
		...
		tukk  stateName = "<<erm, better fix me>>";
		float radius = 0.0f;
		...
		for (i32 i=0; i<CAnimationGraph::MAX_INPUTS; i++)
		{
			if (m_triggerInputs[i] != CStateIndex::INPUT_VALUE_DONT_CARE)
			{
				...
				if (m_pState->GetCurrentState() != INVALID_STATE)
					correctness |= (i32) m_pState->GetGraph()->m_stateIndex.StateMatchesInput( m_pState->GetCurrentState(), i, m_triggerInputs[i], eSMIF_ConsiderMatchesAny );
