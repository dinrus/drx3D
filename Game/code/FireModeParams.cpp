...
void SHazardDescriptor::Reset(const XmlNodeRef& paramsNode, bool defaultInit)
{
	...
	if (paramsNode)
	{
		...
		DRX_ASSERT_MESSAGE(descriptor.maxHazardDistance >= 0.0f, "maxHazardDistance must be >= 0.0f!");
		DRX_ASSERT_MESSAGE(descriptor.hazardRadius >= 0.0f, "hazardRadius must be >= 0.0f!");
		DRX_ASSERT_MESSAGE(descriptor.maxHazardApproxPosDeviation >= 0.0f, "maxHazardApproxPosDeviation must be >= 0.0f!");
		DRX_ASSERT_MESSAGE(descriptor.maxHazardApproxAngleDeviationDeg >= 0.0f, "maxHazardApproxAngleDeviationDeg must be >= 0.0f!");
	}
}
...
void SMeleeParams::Reset( const XmlNodeRef& paramsNode, bool defaultInit/*=true*/ )
{
	...
	if (paramsNode)
	{
		...
		tukk NameFPSignal=NULL, *Name3PSignal=NULL;
...
SMeleeTags::STagParams::STagParams( tukk pTag, tukk pHitType )
	:
...
void SMeleeTags::Reset( const XmlNodeRef& tagsNode, bool defaultInit /* = true */ )
{
	if(tagsNode)
	{
		...
		i32k numTags = reader.GetUnfilteredChildCount();
		...
		for( i32 i=0; i<numTags; ++i )
		{
			...
			if( pName )
			{
				...
				{
					DrxLog( "[melee] Unknown tag in melee container <%s> Should contain 'left, right or killingblow'.", pName );
				}
			}
		}
	}
...
void SFireModeParams::CacheResources()
{
	...
	i32k numPlugins = pluginParams.size();
...
void SPlantParams::Reset( const XmlNodeRef& paramsNode, bool defaultInit/*=true*/ )
{
	...
	if (paramsNode)
	{
		...
		reader.ReadParamValue<i32>("clip_size", clip_size);	
		reader.ReadParamValue<i32>("damage", damage);	
		reader.ReadParamValue<float>("delay", delay);	
...
void SLTagGrenades::PreLoadAssets() const 
{
	...
	for (i32 i = 0; i < ELTAGGrenadeType_LAST; ++i)
	{
		if (!grenades[i].empty())
		{
...
void SChargeParams::Reset( const XmlNodeRef& paramsNode, bool defaultInit/*=true*/ )
{
	...
	if (paramsNode)
	{
		...
		reader.ReadParamValue<i32>("max_charges", max_charges);
...
void STracerParams::Reset( const XmlNodeRef& paramsNode, bool defaultInit/*=true*/ )
{
	...
	if (paramsNode)
	{
		...
		reader.ReadParamValue<i32>("frequency", frequency);
...
{
	DrxFixedStringT<32> ammo_type = "";
	DrxFixedStringT<32> ammo_spawn_type = "";
	...
	if (paramsNode)
	{
		...
		if (!defaultInit && endReloadFraction < fillAmmoReloadFraction)
		{
			...
			tukk fireModeName = fireModeNode->getAttr("name");
		...
		i32 bulletChamber = 0;
		if (reader.ReadParamValue<i32>("bullet_chamber", bulletChamber))
		{
			bullet_chamber = (u8)bulletChamber;
		}
		...
		reader.ReadParamValue<i32>("npc_additional_damage", npc_additional_damage);

		reader.ReadParamValue<float>("ai_reload_time", ai_reload_time);

		i32 crosshairType = (ECrosshairTypes)eHCH_Normal;
		if (reader.ReadParamValue<i32>("crosshair", crosshairType))
		{
		...
		reader.ReadParamValue<i32>("knockdown_chance_leg", knockdown_chance_leg);
		reader.ReadParamValue<float>("min_damage_for_knockDown_leg", min_damage_for_knockDown_leg);

		i32 pierceabilityMod = 0;
		reader.ReadParamValue<i32>("bullet_pierceability_modifier", pierceabilityMod);
...
void SSpammerParams::Reset(const XmlNodeRef& paramsNode, bool defaultInit)
{
	...
	if (paramsNode)
	{
		...
		reader.ReadParamValue<i32>("maxNumRockets", maxNumRockets);
		reader.ReadParamValue<i32>("loadInRate", loadInRate);
		reader.ReadParamValue<i32>("burstRate", burstRate);
...
{
	i32 numAccessoryParams = accessoryChangedParams.size();
