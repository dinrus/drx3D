...
$Id:$
$DateTime$
Описание:  AI grenade weapon implemenation
-------------------------------------------------------------------------
История:
...
{
	DRX_ASSERT_MESSAGE(false, "CAIGrenade::StartFire Called without launch params, ignoring");
}
...
void CAIGrenade::SetFiringPos(tukk boneName)
{ 
 	if(IEntity* ownerEntity = GetOwner())
	{
 		if(ICharacterInstance* character = ownerEntity->GetCharacter(0))
		{
			if(ISkeletonPose* skeletonPose = character->GetISkeletonPose())
			{
			  IDefaultSkeleton& rIDefaultSkeleton = character->GetIDefaultSkeleton();
				i32 jointID = rIDefaultSkeleton.GetJointIDByName(boneName);
				if(jointID != -1)
				{
	...
	DRX_ASSERT_MESSAGE(false, "CAIGrenade::SetFiringPos - Failed to get the position of the bone");
 	m_grenadeLaunchPosition.zero();
}
...
void CAIGrenade::SetCurrentFireMode(i32 idx)
{
