...
#include <drx3D/Game/ActionMapUpr.h>
...
//------------------------------------------------------------------------
tukk CItem::GetWeaponComponentType()
{
	return "CItem";
}

//------------------------------------------------------------------------
tukk CItem::GetType() const
{
...
bool CItem::InitActionController( IEntity* pEntity )
{
	if(!m_sharedparams->params.actionControllerFile.empty())
	{
		...
		IAnimationDatabaseUpr& animationDatabaseUpr = mannequinSys.GetAnimationDatabaseUpr();

		const SControllerDef* pControllerDef = animationDatabaseUpr.LoadControllerDef( m_sharedparams->params.actionControllerFile.c_str() );
		if (pControllerDef == NULL)
		{
			return false;
		}

		DRX_ASSERT(m_pItemAnimationContext == NULL);
		m_pItemAnimationContext = new SAnimationContext(*pControllerDef);
		DRX_ASSERT(m_pItemActionController == NULL);
		...
		{
			const IAnimationDatabase* pSoundDatabase = animationDatabaseUpr.Load(m_sharedparams->params.soundAdbFile.c_str());
			i32k scopeContextSound = pControllerDef->m_scopeContexts.Find("Audio");
...
bool CItem::ReloadExtension( IGameObject * pGameObject, const SEntitySpawnParams &params )
{
	...
	DRX_ASSERT_MESSAGE(false, "CItem::ReloadExtension not implemented");
...
{
	DRX_ASSERT_MESSAGE(false, "CItem::GetEntityPoolSignature not implemented");
...
//------------------------------------------------------------------------
void CItem::Update( SEntityUpdateContext& ctx, i32 slot )
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);
		...
		case eIUS_General:
			{
				...
				if (m_delayedUnhideCntr > 0)
				{
					...
					if (m_delayedUnhideCntr == 0)
					{
						IAttachmentUpr *pAttachmentUpr = GetOwnerAttachmentUpr();
						if (pAttachmentUpr)
						{
							IAttachment *pAttachment = pAttachmentUpr->GetInterfaceByName(m_sharedparams->params.attachment[m_stats.hand].c_str());
							if (pAttachment)
							{
...
//------------------------------------------------------------------------
bool CItem::SetAspectProfile( EEntityAspects aspect, u8 profile )
{
	//DrxLogAlways("%s::SetProfile(%d: %s)", GetEntity()->GetName(), profile, profile==eIPhys_Physicalized?"Physicalized":"NotPhysicalized");
...
u8 CItem::GetDefaultProfile( EEntityAspects aspect )
{
	if (aspect == eEA_Physics)
	{
		...
		if (gEnv->IsEditor() && !m_stats.mounted)
		{
			...
			else // leaving game mode
			{
				...
				if (m_editorstats.ownerId)
				{
					...
					i32 previousValue = g_pGameCVars->g_inventoryNoLimits;
...
//------------------------------------------------------------------------
bool CItem::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 pflags )
{
	if (aspect == eEA_Physics)
	{
...
void CItem::FullSerialize( TSerialize ser )
{
	...
	i32 attachmentAmount = m_accessories.size();
	ser.Value("attachmentAmount", attachmentAmount);
	if(ser.IsWriting())
	{
		for(i32 i = 0; i < attachmentAmount; i++)
		{
...
void CItem::PostSerialize()
{
	...
	else 
	{
		if (m_owner.GetId()!=0)
		{
			...
			if (pEntityOwner)
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "PostSerialize(). Item %s has ownerId %i that is an entity (%s) but not an actor.", GetEntity()->GetName(), m_owner.GetId(), pEntityOwner->GetName());
			else
			{
				bool inPool = gEnv->pEntitySystem->GetIEntityPoolUpr()->IsEntityBookmarked( m_owner.GetId() );
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "PostSerialize(). Item %s has ownerId %i but owner actor does not exist! %s", GetEntity()->GetName(), m_owner.GetId(), inPool ? " cant find actor because is bookmarked!" : "");
			}
		}
...
//------------------------------------------------------------------------
void CItem::SwitchToHand(i32 hand)
{
...
{
	DRX_ASSERT_MESSAGE(AreAnyItemFlagsSet(eIF_SelectGrabbingWeapon), "Triggering delayed weapon grab when weapon is not expecting it!");
...
void CItem::Select(bool select)
{
	...
	if (select)
	{
		...
		u32 selectBusyTimer = 0;
		if (m_sharedparams->params.select_override == 0.0f)
			selectBusyTimer = max(250U, (u32)(selectBiasTime * GetCurrentAnimationTime(eIGS_Owner))) - 250;
		else
			selectBusyTimer = (u32)(m_sharedparams->params.select_override*selectBiasTime*1000);
		SetBusy(true);
		...
		if (pOwnerActor)
		{
			...
			if (!gEnv->bMultiplayer && pAISystem && pAISystem->GetSmartObjectUpr())
			{
				pAISystem->GetSmartObjectUpr()->ModifySmartObjectStates( pOwnerEntity, GetEntity()->GetClass()->GetName() );
				pAISystem->GetSmartObjectUpr()->ModifySmartObjectStates( pOwnerEntity, "WeaponDrawn" );
			}
		}    

		for (u32 i = 0; i < m_accessories.size(); ++i)
		{
			const SAccessoryInfo& accessory = m_accessories[i];
			i32 slot = m_stats.fp ? 0 : 1;
	...
	else
	{
		...
		// update smart objects states
		if ( !gEnv->bMultiplayer && pAISystem && pAISystem->GetSmartObjectUpr() && pOwnerActor && !pOwnerActor->IsDead() )
		{
			DrxFixedStringT<256> tmpString( "-WeaponDrawn," );
			tmpString += GetEntity()->GetClass()->GetName();
			pAISystem->GetSmartObjectUpr()->ModifySmartObjectStates( pOwnerActor->GetEntity(), tmpString.c_str() );
		}
...
//------------------------------------------------------------------------
u32 CItem::StartDeselection(bool fastDeselect)
{
	return 0;
}
			...
			if (IMovementController * pMC = pOwnerActor->GetMovementController())
			{
				...
				if(pOwnerActor->IsPlayer())
				{
					if(IPhysicalEntity *pItemPhysicsEntity = pThisItemEntity->GetPhysics())
					{
						...
						i32 entityTypes = ent_static|ent_terrain|ent_rigid|ent_sleeping_rigid;
						i32 flags = rwi_stop_at_pierceable|14;
	...
	if (pThisItemEntity->IsHidden())
	{
		...
		if (pAttachmentUpr)
		{
			if (pOwnerActor && !pOwnerActor->IsPlayer())
				pAttachment = pAttachmentUpr->GetInterfaceByName(m_sharedparams->params.aiAttachment[m_stats.hand].c_str());
			if (!pAttachment)
				pAttachment = pAttachmentUpr->GetInterfaceByName(m_sharedparams->params.attachment[m_stats.hand].c_str());
			if (pAttachment)
			{
...
//------------------------------------------------------------------------
void CItem::PickUp(EntityId pickerId, bool sound, bool select, bool keepHistory, tukk setup)
{
	...
	tukk itemClassName = GetEntity()->GetClass()->GetName();
...
void CItem::UnlockDefaultAttachments(CActor* pActor)
{
	...
	i32 numDefaultAccessories = defaultAccessoires.size();
	for (i32 i = 0; i < numDefaultAccessories; ++i)
	{
...
bool CItem::CanPickUp(EntityId userId) const
{
	...
	i32 teamId = g_pGame->GetGameRules()->GetTeam(GetEntity()->GetId());
...
{
	u32 weaponClassCrC = CCrc32::ComputeLowercase(GetParams().itemClass.c_str());
	u32 weaponTypeCrC  = CCrc32::ComputeLowercase(GetParams().tag.c_str());
...
//------------------------------------------------------------------------
void CItem::UpdateScopeContexts( IActionController *pController, i32 nCharacterSlot /*= 0*/ )
{
	bool foundTop = false;
	...
	IMannequin &mannequinSys = gEnv->pGame->GetIGameFramework()->GetMannequinInterface();
	const SMannequinItemParams *pParams = mannequinSys.GetMannequinUserParamsUpr().FindOrCreateParams<SMannequinItemParams>(pController);
	...
	if (pParams->contextIDs.Weapon != TAG_ID_INVALID)
	{
		if (pCharInst)
		{
			const IAnimationDatabase* pDatabase = !m_sharedparams->params.adbFile.empty() ? mannequinSys.GetAnimationDatabaseUpr().Load(m_sharedparams->params.adbFile.c_str()) : NULL;
			pController->SetScopeContext(pParams->contextIDs.Weapon, *GetEntity(), pCharInst, pDatabase);
	
			i32k numAccessories = m_accessories.size();

			for (i32 i = 0; i < numAccessories && !(foundTop && foundBottom); i++)
			{
				...
				if (!foundTop && (aparams->category == g_pItemStrings->scope) && (pParams->contextIDs.attachment_top != TAG_ID_INVALID)) 
				{
					...
					{
						const IAnimationDatabase* pAccessoryDatabase = !pAccessory->GetParams().adbFile.empty() ? mannequinSys.GetAnimationDatabaseUpr().Load(pAccessory->GetParams().adbFile.c_str()) : NULL;
...
void CItem::UpdateAccessoryTags(const SMannequinItemParams* pParams, CTagState &tagState, bool selected) const
{
	...
	if(selected)
	{
		...
		for(i32 i = 0; i < numAccessories; i++)
		{
			...
			{
				if(u32 accessoryTypeCrC = CCrc32::ComputeLowercase(pAccessory->GetParams().tag.c_str()))
				{
...
const Matrix34 CItem::GetWorldTM() const
{
	if (m_stats.attachment == eIA_WeaponCharacter)
	{
		...
		DRX_ASSERT_MESSAGE(0, "Weapon thinks it is attached but isn't");
...
//------------------------------------------------------------------------
tukk CItem::GetDisplayName() const
{
...
//------------------------------------------------------------------------
void CItem::OnAction(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
}
...
bool CItem::AttachToHand(bool attach, bool checkAttachment)
{
	...
	{
		DRX_ASSERT_MESSAGE(!IsAttachedToBack(), "Logic flow error, should never seek to attach to the hand whilst attached to the back!");

		bool firstPerson = (m_stats.viewmode & eIVM_FirstPerson);
		i32 slot = firstPerson ? eIGS_FirstPerson : eIGS_ThirdPerson;
...
CItem::eItemAttachment CItem::ChooseAttachmentPoint(bool attach, IAttachment **attachmentPt) const
{
	...
	if (pAttachmentUpr)
	{
		if (attach)
		{
			attachRet = pAttachmentUpr->GetInterfaceByName(m_sharedparams->params.bone_attachment_01.c_str());
			ret = eIA_StowPrimary;
			if (!attachRet)
			{
			...
			{
				attachRet = pAttachmentUpr->GetInterfaceByName(m_sharedparams->params.bone_attachment_02.c_str());
				if (!attachRet)
				{
	...
	if (m_sharedparams && m_sharedparams->params.attach_to_back)
	{
		...
		if(CActor* pActor = GetOwnerActor())
		{
			...
			if (pAttachment)
			{
				...
				{
					DRX_ASSERT_MESSAGE(!IsAttachedToHand(), "Logic flow error, should never seek to attach to the back whilst attached to the hand!");
...
//------------------------------------------------------------------------
void CItem::RequireUpdate(i32 slot)
{
	if (slot==-1)
		for (i32 i=0;i<4;i++)
...
//------------------------------------------------------------------------
void CItem::EnableUpdate(bool enable, i32 slot)
{
	if (enable)
	{
		if (slot==-1)
...
void CItem::CloakEnable(bool enable, bool fade, float cloakBlendSpeedScale, bool bFadeByDistance, u8 colorChannel, bool bIgnoreCloakRefractionColor, EntityId syncedFromId /*= 0*/ )
{
...
void CItem::CloakSync(bool fade)
{
	...
	if (pOwnerRP)
	{
		u8 ownerMask = pOwnerRP->GetMaterialLayersMask();
		...
		u8 colorChannel = pOwnerRP->GetCloakColorChannel();
...
void CItem::AddAccessoryAmmoToInventory(IEntityClass* pAmmoType, i32 count, IInventory* pOwnerInventory)
{
	if(pOwnerInventory)
	{
		i32 capacity = pOwnerInventory->GetAmmoCapacity(pAmmoType);
		i32 current = pOwnerInventory->GetAmmoCount(pAmmoType);
		...
		for (TAccessoryAmmoMap::const_iterator it = m_sharedparams->bonusAccessoryAmmo.begin(); it != m_sharedparams->bonusAccessoryAmmo.end(); ++it)
		{
			...
			if(pParentWeapon)
			{
				...
				if(capacity != m_sharedparams->accessoryAmmoCapacity.end() && count > capacity->second)
				{
					...
					i32 parentCount = pParentWeapon->GetAmmoCount(pAmmoClass);

					i32 numToAdd = count - capacity->second;
...
void CItem::ProcessAccessoryAmmoCapacities(IInventory* pOwnerInventory, bool addCapacity)
{
	if (pOwnerInventory)
	{
		...
		{
			i32 currentCapacity = pOwnerInventory->GetAmmoCapacity(iter->first);
			i32 additionalCapacity = iter->second;
			i32 newCapacity = 0;
			...
			DRX_ASSERT_MESSAGE(newCapacity >= 0, string().Format("Trying to set ammo capacity for '%s' to a negative number", iter->first->GetName()));
...
bool CItem::CheckAmmoRestrictions(IInventory * pInventory)
{
	...
	if(AreAnyItemFlagsSet(eIF_AccessoryAmmoAvailable) && !m_sharedparams->bonusAccessoryAmmo.empty())
	{
		for (TAccessoryAmmoMap::const_iterator it = m_sharedparams->bonusAccessoryAmmo.begin(); it != m_sharedparams->bonusAccessoryAmmo.end(); ++it)
		{
			i32 invAmmo  = pInventory->GetAmmoCount(it->first);
			i32 invLimit = pInventory->GetAmmoCapacity(it->first);
...
void CItem::GetMemoryUsage(IDrxSizer *s) const
{	
	s->AddObject(this, sizeof(*this));
	GetInternalMemoryUsage(s);
}

void CItem::GetInternalMemoryUsage(IDrxSizer *s) const
{
	...
	if (pItem)
	{
		if (pItem->GetOwnerId())
		{
			...
			if (pActor)
			{
				...
				{
					i32 category = GetItemCategoryType(m_pItemSystem->GetItemCategory(pItem->GetEntity()->GetClass()->GetName()));
...
{
	DRX_ASSERT(!AreAnyItemFlagsSet(eIF_RegisteredAs1pUser));

	if (!m_sharedparams->animationPrecache.empty())
	{
		CItemAnimationDBAUpr& fpDbaUpr = g_pGame->GetGameSharedParametersStorage()->GetItemResourceCache().Get1pDBAUpr();
		
		fpDbaUpr.AddDBAUser(m_sharedparams->animationGroup, m_sharedparams, GetEntityId());
	}
}
...
void CItem::UnRegisterAs1pDbaUser()
{
	...
	fpDbaUpr.RemoveDBAUser(m_sharedparams->animationGroup, GetEntityId());
}
...
void CItem::RegisterAs1pChrUser()
{
	...
	if (pGD)
	{
		gEnv->pCharacterUpr->StreamKeepCharacterResourcesResident(pGD->modelPath.c_str(), 0, true);
	}
}
...
void CItem::UnRegisterAs1pChrUser()
{
	...
	if (pGD)
	{
		gEnv->pCharacterUpr->StreamKeepCharacterResourcesResident(pGD->modelPath.c_str(), 0, false);
	}
}
...
{
	DRX_ASSERT(!AreAnyItemFlagsSet(eIF_RegisteredAs3pUser));
...
void CItem::AudioCacheItemAndAccessories(const bool enable, tukk type)
{
...
/*static*/ void CItem::AudioCacheItem(const bool enable, const IEntityClass* pClass, tukk prefix, tukk postfix)
{
	/*DrxFixedStringT<32> soundCache;
...
bool CItem::IsAudioCached(const IEntityClass *pClass, tukk type)
{
	return true;
	/*DrxFixedStringT<32> soundCache;
...
IAttachmentUpr *CItem::GetOwnerAttachmentUpr() const
{
	IEntity *pOwner = GetOwner();
	if (pOwner)
	{
		...
		if (pOwnerCharacter)
		{
			return pOwnerCharacter->GetIAttachmentUpr();
		}
	}
...
void CItem::Prepare1pAnimationDbas()
{
	if (!m_sharedparams->animationPrecache.empty())
	{
		...
		fpDbaUpr.RequestDBAPreLoad(m_sharedparams->animationGroup, m_sharedparams);
	}
}
...
void CItem::Prepare1pChrs()
{
	...
	if (pGD)
	{
		CItemPrefetchCHRUpr& pf = g_pGame->GetGameSharedParametersStorage()->GetItemResourceCache().GetPrefetchCHRUpr();
...
const string CItem::GetAttachedAccessoriesString(tukk separator)
{
	string accessories = "";
