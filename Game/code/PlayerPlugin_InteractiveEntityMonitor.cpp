...
void CPlayerPlugin_InteractiveEntityMonitor::Update( const float dt )
{
	...
	while(mapIter != mapEnd)
	{
		...
		{
			DrxLog("[ERROR] InteractiveEntityMonitor. About to crash. Registered entity no longer exists: '%s'. Tell Gary (Really this time).", mapIter->second.c_str());
...
void CPlayerPlugin_InteractiveEntityMonitor::HandleEvent( EPlayerPlugInEvent theEvent, uk  data )
{
	switch(theEvent)
	{
	case EPE_Die:
...
void CPlayerPlugin_InteractiveEntityMonitor::Register( IEntity* pEntity, u8 initialFlags )
{
	if(!pEntity)
		return;
	...
	while(iter != end)
	{
		...
		{
#ifndef _RELEASE
			DrxLog("[ERROR] InteractiveEntityMonitor. Registering the same entity twice: '%s'. Tell Gary (Really this time).", pEntity->GetName());
...
void CPlayerPlugin_InteractiveEntityMonitor::PrecacheLevel()
{
	...
	if(rootNode)
	{
		tukk pLevelName = gEnv->pGame->GetIGameFramework()->GetLevelName();
		if(!pLevelName)
		{
			pLevelName = "default";
		}
		else if( tukk  pTrim = strstr(pLevelName, "/") )
		{
			pLevelName = pTrim+1;
		}
