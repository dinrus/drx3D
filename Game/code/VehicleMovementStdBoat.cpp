...
$Id$
$DateTime$
Описание: Implements a standard boat movement
...
bool CVehicleMovementStdBoat::Init(IVehicle* pVehicle, const CVehicleParams& table)
{
  ...
  //DrxLog("[StdBoat movement]: got mass offset (%f, %f, %f)", m_massOffset.x, m_massOffset.y, m_massOffset.z);
	...
	m_pWaveEffect = gEnv->pParticleUpr->FindEffect(waveEffect, "MovementStdBoat");

  m_waveTimer = drx_random(0.0f, gf_PI);
  m_diving = false;
  m_wakeSlot = -1;   
...
//------------------------------------------------------------------------
void CVehicleMovementStdBoat::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
...
void CVehicleMovementStdBoat::UpdateSurfaceEffects(const float deltaTime)
{
  ...
  { 
...
#if ENABLE_VEHICLE_DEBUG
        if (DebugParticles())              
					DrxLog("%s changes water sfx to %s (slot %i)", pEntity->GetName(), effect, emitterIt->slot);
#endif
    ...
    {          
      ...
      tukk effect = info.pParticleEmitter ? info.pParticleEmitter->GetName() : "";
  ...
  if (!m_diving && localW.x < -0.03f && status.speed > 10.f && wakePos.z < m_lastWakePos.z && wakeWaterLevel+0.1f >= wakePos.z)
  {
    ...
    if (m_pWaveEffect)
    {
      ...
      spawnParams.fCountScale += drx_random(0.0f, 0.4f);
...
void CVehicleMovementStdBoat::ProcessAI(const float deltaTime)
{
	...
	else
	{
		...
		m_prevAngle =  angle;
		//DrxLog("steering	%4.2f	%4.2f %4.2f	%4.2f	%4.2f	%4.2f	%d", deltaTime,inputSpeed - currentSpeed,angle,currentAngleSpeed, m_movementAction.rotateYaw,currentAngleSpeed-m_prevAngle,step);
...
{  
  ...
  if (m_waveTimer >= gf_PI && waveTimerPrev < gf_PI) 
    m_waveRandomMult = drx_random(0.0f, 1.0f);  
  ...
  float turnAccel = 0, turnAccelNorm = 0;
    ...
    { 
      ...
      {
        i32 iDir = m_movementAction.power != 0.f ? sgn(m_movementAction.power) : sgn(localVel.y);
        ...
        i32 sgnSteerW = sgn(m_movementAction.rotateYaw) * iDir * sgn(-localW.z);
...
void CVehicleMovementStdBoat::GetMemoryUsage(IDrxSizer * pSizer) const
{
	pSizer->Add(*this);
