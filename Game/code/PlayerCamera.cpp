...
#define PLAYER_CAMERA_WATCH(...)  do { if (g_pGameCVars->pl_debug_watch_camera_mode)       DrxWatch    ("[PlayerCamera] " __VA_ARGS__); } while(0)
#define PLAYER_CAMERA_LOG(...)    do { if (g_pGameCVars->pl_debug_log_camera_mode_changes) DrxLogAlways("[PlayerCamera] " __VA_ARGS__); } while(0)
...
CPlayerCamera::~CPlayerCamera()
{
	...
	for (u32 i = 0; i < eCameraMode_Last; ++i)
	{
...
void CPlayerCamera::SetCameraMode(ECameraMode newMode, tukk  why)
{
...
void CPlayerCamera::SetCameraModeWithAnimationBlendFactors( ECameraMode newMode, const ICameraMode::AnimationSettings& animationSettings, tukk why )
{
...
bool CPlayerCamera::Update(SViewParams& viewParams, float frameTime )
{
    ...
    assert(0 <= m_transitionCameraMode && m_transitionCameraMode < eCameraMode_Last);
		...
		{
			DrxWatch("Transitioning %f (%.2f) Pos (%.2f, %.2f, %.2f) Rot (%.2f, %.2f, %.2f, %.2f)", t, m_totalTransitionTime, viewParams.position.x, viewParams.position.y, viewParams.position.z, viewParams.rotation.v.x, viewParams.rotation.v.y, viewParams.rotation.v.z, viewParams.rotation.w);
		}
#endif //!_RELEASE
	}
	...
	if (g_pGameCVars->pl_debug_view)
	{
		...
		DrxWatch("PlayerCamera: fwd %f, %f, %f)", fwd.x, fwd.y, fwd.z);
	}
...
void CPlayerCamera::PostUpdate(const QuatT &_camDelta)
{
	...
	if ((blendFactorTran > 0.0f) || (blendFactorRot > 0.0f))
	{
		...
		{
			i32k slotIndex = 0;
			u32 entityFlags = pPlayerEntity->GetSlotFlags(slotIndex);
		...
		if (g_pGameCVars->pl_debug_view)
		{
			DrxWatch("CamAnimDelta factor Pos (%.2f, %.2f, %.2f):%f Rot (%.2f, %.2f, %.2f, %.2f):%f", camDelta.t.x, camDelta.t.y, camDelta.t.z, blendFactorTran, camDelta.q.v.x, camDelta.q.v.y, camDelta.q.v.z, camDelta.q.w, blendFactorRot);
		}
#endif //!_RELEASE
	}
}
