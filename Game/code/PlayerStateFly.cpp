	...
	if( player.m_stats.spectatorInfo.mode == 0 )
	{
		...
		else
		{
			if( m_flyModeDisplayTime > 0.f )
			{
				...
				{
				case 1:
					drx_sprintf( buffer,"FlyMode ON" );
					break;
				case 2:
					drx_sprintf( buffer,"FlyMode/NoClip ON" );
					break;
				}
...
void CPlayerStateSpectate::DrawSpectatorFade()
{
	...
	float width  = pLayoutUpr->GetVirtualWidth();
	float height = pLayoutUpr->GetVirtualHeight();
