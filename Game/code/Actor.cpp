...
#include <drx3D/Act/IInteractor.h>
...
#include <drx3D/Game/AnimActionBlendFromRagdoll.h>
...
static void DoNotDeleteThisPointer(uk pPointer) {}

const char CActor::DEFAULT_ENTITY_CLASS_NAME[] = "Default";

//------------------------------------------------------------------------
// "W" stands for "world"
void SIKLimb::SetWPos(IEntity *pOwner,const Vec3 &pos,const Vec3 &normal,float blend,float recover,i32 requestID)
{
...
void SIKLimb::SetLimb(i32 slot,tukk limbName,i32 rootID,i32 midID,i32 endID,i32 iFlags)
{
	rootBoneID = rootID;
	endBoneID = endID;
	middleBoneID = midID;

	drx_strcpy(name, limbName);
...
, m_lastNetItemId(0)
{
	...
	DRX_ASSERT(m_pImpulseHandler != NULL);
...
CActor::~CActor()
{
	...
	GetGameObject()->ReleaseProfileUpr( this );
	...
	CActorUpr::GetActorUpr()->ActorRemoved(this);
}
...
void CActor::PhysicalizeBodyDamage()
{
	...
	{
		pBodyDamageUpr->PhysicalizePlayer(bodyDamageProfileId, *GetEntity());
	}
}
...
bool CActor::Init( IGameObject * pGameObject )
{
	...
	if (!GetGameObject()->CaptureView(this))
		return false;
	if (!GetGameObject()->CaptureProfileUpr(this))
		return false;
	...
	u32 uNewFlags = (ENTITY_FLAG_CUSTOM_VIEWDIST_RATIO);
	...
	g_pGame->GetTacticalUpr()->AddEntity(GetEntityId(), CTacticalUpr::eTacticalEntity_Unit);
...
{
	DRX_ASSERT(GetGameObject() == pGameObject);
	...
	DRX_ASSERT(GetLinkedVehicle() == NULL);

	g_pGame->GetIGameFramework()->GetIActorSystem()->RemoveActor(params.prevId);
	g_pGame->GetIGameFramework()->GetIActorSystem()->AddActor(GetEntityId(), this);

	g_pGame->GetTacticalUpr()->RemoveEntity(params.prevId, CTacticalUpr::eTacticalEntity_Unit);
	g_pGame->GetTacticalUpr()->AddEntity(GetEntityId(), CTacticalUpr::eTacticalEntity_Unit);
...
void CActor::PostReloadExtension( IGameObject *pGameObject, const SEntitySpawnParams &params )
{
	...
	{
		g_pGame->GetAutoAimUpr().UnregisterAutoaimTarget(params.prevId);
		m_registeredInAutoAimMng = false;
	}
...
void CActor::HideAllAttachments(bool isHiding)
{
	...
	if (!IsPlayer())
	{
		i32 totalItems = m_pInventory->GetCount();
...
//------------------------------------------------------------------------
void CActor::InitClient(i32 channelId)
{
...
void CActor::Revive( EReasonForRevive reasonForRevive )
{
	...
	{
		g_pGame->GetBodyDamageUpr()->ResetInstance(*GetEntity(), m_bodyDestructionInstance);
	}
  ...
  if (IPhysicalEntity* pPhysics = GetEntity()->GetPhysics())
  {
	...
	if (reasonForRevive == kRFR_FromInit)
	{
		CBodyDamageUpr *pBodyDamageUpr = g_pGame->GetBodyDamageUpr();
		DRX_ASSERT(pBodyDamageUpr);
		m_DefaultBodyDamageProfileId = pBodyDamageUpr->GetBodyDamage(*GetEntity());

		pBodyDamageUpr->GetBodyDestructibility(*GetEntity(), m_bodyDestructionInstance);
	}
...
//------------------------------------------------------------------------
bool CActor::LoadPhysicsParams(SmartScriptTable pEntityTable, tukk szEntityClassName, SEntityPhysicalizeParams &outPhysicsParams,
	...
	if (pEntityTable)
	{
		...
		if (pEntityTable->GetValue("physicsParams", physicsParams))
		{
			...
			if (physicsTableChain.GetValue("Living", livingTab))
			{
				...
				{
					if (ISurfaceType *pSurfaceType=gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeByName(colliderMat))
...
//------------------------------------------------------------------------
bool CActor::SetActorModel(tukk modelName)
{
...
//------------------------------------------------------------------------
bool CActor::SetActorModelInternal(tukk modelName)
{
	bool hasChangedModel = false;
...
bool CActor::SetActorModelInternal(const SActorFileModelInfo &fileModelInfo)
{
	...
	else if (strcmpi(m_currModel.c_str(), modelVariationFileName.c_str()) != 0)
	{
		...
		if (bShouldHide)
		{
			u32k flags = (pEntity->GetSlotFlags(0) & ~ENTITY_SLOT_RENDER);
		...
		if (pCharacter)
		{
			...
			GetOrCreateAttachment(pAttachmentUpr, "weapon_bone", "right_item_attachment");
			GetOrCreateAttachment(pAttachmentUpr, "weapon_bone", "weapon");
			GetOrCreateAttachment(pAttachmentUpr, "alt_weapon_bone01", "left_item_attachment");
			GetOrCreateAttachment(pAttachmentUpr, "weapon_bone", "laser_attachment");
...
//------------------------------------------------------------------------
IAttachment* CActor::GetOrCreateAttachment(IAttachmentUpr *pAttachmentUpr, tukk boneName, tukk attachmentName)
{
	assert(pAttachmentUpr);

	IAttachment* pIAttachment = pAttachmentUpr->GetInterfaceByName(attachmentName);
	if (!pIAttachment)
	{
		pIAttachment = pAttachmentUpr->CreateAttachment(attachmentName, CA_BONE, boneName);
	}
...
//	if (IsPlayer())
	...
	//DrxLogAlways("CActor::PostPhysicalize: %s, inertia: %f, inertiaAccel: %f", GetEntity()->GetName(), m_inertia, m_inertiaAccel);
...
{
	DRX_ASSERT( m_pAnimatedCharacter->GetActionController() );
...
{
	DRX_ASSERT_TRACE(desiredStance >= 0 && desiredStance < STANCE_LAST, ("%s '%s' desired stance %d is out of range 0-%d", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), desiredStance, STANCE_LAST - 1));
...
void CActor::ProcessEvent(SEntityEvent& event)
{
	...
	case ENTITY_EVENT_HIDE:
		{
			g_pGame->GetTacticalUpr()->RemoveEntity(GetEntityId(), CTacticalUpr::eTacticalEntity_Unit);
			if(!gEnv->bMultiplayer)
			{
	...
	case ENTITY_EVENT_RELOAD_SCRIPT:
		{
			...
			pBodyDamageUpr->ReloadBodyDamage(*this);
			break;
		}
	case ENTITY_EVENT_ADD_TO_RADAR:
		{
			SHUDEvent hudevent(eHUDEvent_AddEntity);
			hudevent.AddData(SHUDEventData(static_cast<i32>(GetEntityId())));
			CHUDEventDispatcher::CallEvent(hudevent);
			break;
		}
...
void CActor::Update(SEntityUpdateContext& ctx, i32 slot)
{
	...
	{
		g_pGame->GetBodyDamageUpr()->DebugBodyDestructionInstance(*GetEntity(), m_bodyDestructionInstance);
	}
#endif
...
bool CActor::UpdateStance()
{
	if (m_stance != m_desiredStance)
	{
		...
		if (!TrySetStance(m_desiredStance))
		{
			...
			if (m_stance == STANCE_NULL && GetEntity()->GetPhysics() && IsPlayer())
			{
				...
				switch (m_tryToChangeStanceCounter)
				{
					...
					DesignerWarning(false, "%s '%s' (%g %g %g) can't change stance from %d '%s' to %d '%s' (tried and failed for %u consecutive frames). Possibly a spawn-point intersecting with level geometry?", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), pos.x, pos.y, pos.z, (i32) m_stance, GetStanceName(m_stance), (i32) m_desiredStance, GetStanceName(m_desiredStance), m_tryToChangeStanceCounter);
					++ m_tryToChangeStanceCounter;
					break;

					default:
					++ m_tryToChangeStanceCounter;
					DrxLog("%s '%s' (%g %g %g) can't change stance from %d '%s' to %d '%s' [Attempt %u]", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), pos.x, pos.y, pos.z, (i32) m_stance, GetStanceName(m_stance), (i32) m_desiredStance, GetStanceName(m_desiredStance), m_tryToChangeStanceCounter);
					break;
				}
			}
#endif
		...
		{
			const Vec3 pos = GetEntity()->GetWorldPos();
			DrxLog("%s '%s' (%g %g %g) has finally changed stance from %d '%s' to %d '%s'", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), pos.x, pos.y, pos.z, (i32) oldStance, GetStanceName(oldStance), (i32) newStance, GetStanceName(m_desiredStance));
			m_tryToChangeStanceCounter = 0;
		}
#endif
...
void CActor::SetCloakLayer(bool set, eFadeRules config /*= eAllowFades*/)
{
	...
	u8k cloakColorChannel = EntityEffects::Cloak::GetCloakColorChannel(entityId);
...
void CActor::CloakSyncAttachments(bool bFade)
{
	...
	{
		if (IAttachmentUpr *pAttachmentUpr = pOwnerCharacter->GetIAttachmentUpr())
		{
			u32k attachmentCount = pAttachmentUpr->GetAttachmentCount();
			for (u32 attachmentIndex = 0; attachmentIndex < attachmentCount; ++attachmentIndex)
			{
				if (IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByIndex(attachmentIndex))
				{
...
//------------------------------------------------------------------------
void CActor::OnAction(const ActionId& actionId, i32 activationMode, float value)
{
	if (!gEnv->IsEditing())
	{
...
//------------------------------------------------------------------------
void CActor::CreateScriptEvent(tukk event,float value,tukk str)
{
...
void CActor::RequestFacialExpression(tukk pExpressionName /* = NULL */, f32* sequenceLength)
{
	ICharacterInstance* pCharacter = GetEntity()->GetCharacter(0);
	IFacialInstance* pFacialInstance = (pCharacter ? pCharacter->GetFacialInstance() : 0);
	if (pFacialInstance)
...
void CActor::PrecacheFacialExpression(tukk pExpressionName)
{
	ICharacterInstance* pCharacter = GetEntity()->GetCharacter(0);
	IFacialInstance* pFacialInstance = (pCharacter ? pCharacter->GetFacialInstance() : 0);
	if (pFacialInstance)
...
void CActor::SetChannelId(u16 id)
{
}
...
void CActor::DamageInfo(EntityId shooterID, EntityId weaponID, IEntityClass *pProjectileClass, float damage, i32 damageType, const Vec3 hitDirection)
{
}
...
#ifndef _RELEASE
		//DrxLog ("CActor::SetMaxHealth() - Scaling max health %d for %s '%s' by %.3f, giving %d", oldMaxHealth, GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), g_pGameCVars->g_maxHealthMultiplier, maxHealth);
#endif
	}
...
//------------------------------------------------------------------------
bool CActor::LoadDynamicAimPoseElement(CScriptSetGetChain& gameParamsTableChain, tukk szName, string& output)
{
	tukk pose;
	pose = 0;
...
bool CActor::LoadGameParams(SmartScriptTable pEntityTable, SActorGameParams &outGameParams)
{
	...
	if (pEntityTable && pEntityTable->GetValue("gameParams", pGameParams))
	{
		...
		tukk lookAtSimpleHeadBoneName = 0;
		...
		{
			i32k tableElements = characterDBAs->Count();
			for (i32 i = 1; i <= tableElements; ++i)
			{
				if(characterDBAs->GetAtType(i) == svtString)
				{
					tukk dbaGroupName = NULL;
					characterDBAs->GetAt(i, dbaGroupName);

					DRX_ASSERT(dbaGroupName);
	...
	if (pEntityTable && pEntityTable->GetValue("Properties", propertiesTable))
	{
		...
		if (propertiesTable->GetValue("CharacterSounds", characterSoundsTable))
		{
			...
			tukk footstepIndGearAudioSignal_Walk = 0;
			...
			tukk footstepIndGearAudioSignal_Run = 0;
...
//------------------------------------------------------------------------
void CActor::SetStanceMaxSpeed(u32 stance, float fValue)
{
...
{
	// Copy stances over
	for (u32 stance = 0; stance < STANCE_LAST; ++stance)
	{
	...
	if (pSkelPose)
	{
		...
		for (u32 boneId = 0; boneId < BONE_ID_NUM; ++boneId)
		{
...
bool CActor::SetAspectProfile( EEntityAspects aspect, u8 profile )
{
	bool res(false);
				...
				if(!gEnv->bMultiplayer)
				{
					...
					if (profile==eAP_Alive && (m_currentPhysProfile==eAP_Sleep || m_currentPhysProfile==eAP_Ragdoll))
					{
						...
						if (pCharacter && pCharacter->GetISkeletonAnim())
						{
							...
							{
								DRX_ASSERT(m_currentPhysProfile==eAP_Ragdoll);
...
bool CActor::AllowPhysicsUpdate(u8 newCounter) const
{
	return AllowPhysicsUpdate(newCounter, m_netPhysCounter);
}

bool CActor::AllowPhysicsUpdate(u8 newCounter, u8 oldCounter)
{
	return (newCounter == oldCounter);
}

bool CActor::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 pflags )
{
...
void CActor::ProceduralRecoil( float duration, float kinematicImpact, float kickIn/*=0.8f*/, i32 arms/*=0*/ )
{
	if(m_pAnimatedCharacter)
	{
...
void CActor::HandleEvent( const SGameObjectEvent& event )
{
	...
	case eGFE_BecomeLocalPlayer:
		{
			DrxLog ("%s '%s' is becoming local actor", GetEntity()->GetClass()->GetName(), GetEntity()->GetName());
...
void CActor::QueueAnimationState( tukk  state )
{
	if (m_pAnimatedCharacter)
...
QuatT CActor::GetCameraTran() const
{
	if (HasBoneID(BONE_CAMERA))
	{
		...
		DRX_ASSERT(camera.IsValid());

		return camera;
	}

	DRX_ASSERT(GetSpectatorState() == eASS_SpectatorMode);
	QuatT returnQuatT;
...
bool CActor::CheckInventoryRestrictions(tukk itemClassName)
{
...
//IK stuff
void CActor::SetIKPos(tukk pLimbName, const Vec3& goalPos, i32 priority)
{
	i32 limbID = GetIKLimbIndex(pLimbName);
	if (limbID > -1)
	{
		Vec3 pos(goalPos);
...
i32 CActor::GetIKLimbIndex(tukk limbName)
{
	for (size_t i=0;i<m_IKLimbs.size();++i)
...
void CActor::SetFacialAlertnessLevel(i32 alertness)
{
	if (m_pAnimatedCharacter)
...
void CActor::SwitchToWeaponWithAccessoryFireMode()
{
	...
	i32 currentSlot = startSlot;
	i32 skip = m_pInventory->GetCount(); // maximum number of interactions

	while(skip)
	{
		i32 slot = currentSlot + 1;
		...
		{
			i32 numFiremodes = pWeapon->GetNumOfFireModes();
			i32 accessoryEnabledMode = -1;

			for(i32 i = 0; i < numFiremodes && accessoryEnabledMode < 0; i++)
			{
...
//------------------------------------------------------------------------
void CActor::SelectNextItem(i32 direction, bool keepHistory, i32 category)
{
	if (!m_pInventory)
		return;
	...
	if (pActorStats->exchangeItemStats.switchingToItemID != 0)
	{
		...
		if (pSwitchingItem != NULL)
		{
			...
			DRX_ASSERT(pEntityClass != NULL);
			tukk switchingItemCategory = m_pItemSystem->GetItemCategory(pEntityClass->GetName());
			i32 switchingItemCategoryType = GetItemCategoryType(switchingItemCategory);
	...
	if (pCurrentItem)
	{
		if (tukk const currentWeaponCat = m_pItemSystem->GetItemCategory(pCurrentItem->GetEntity()->GetClass()->GetName()))
		{
...
//------------------------------------------------------------------------
EntityId CActor::ComputeNextItem(i32k startSlot, i32k category, i32k delta, bool& inOutKeepHistory, IItem* pCurrentItem, const bool currWeaponExplosive) const
{
	i32 slot = startSlot;
	i32 skip = m_pInventory->GetCount(); // maximum number of interactions
	...
	while(skip)
	{
		...
		if (pItem)
		{
			tukk name = pItem->GetEntity()->GetClass()->GetName();
			tukk nextWeaponCatStr = m_pItemSystem->GetItemCategory(name);
			i32 nextWeaponCat = GetItemCategoryType(nextWeaponCatStr);
...
void CActor::SelectWeaponWithAmmo(EntityId currentOutOfAmmoId, bool keepHistory)
{
	...
	if (m_pInventory)
	{
		...
		i32 numItems = m_pInventory->GetCount();
...
void CActor::ClearItemActionControllers()
{
	...
	if(m_pInventory)
	{
		i32k numItems = m_pInventory->GetCount();
		for(i32 i = 0; i<numItems; i++)
		{
...
void CActor::AttemptToRecycleAIActor()
{
	...
	if (removalAllowed)
	{
		...
		{
			gEnv->pEntitySystem->GetIEntityPoolUpr()->ReturnToPool( GetEntityId() );
		}
		else
		{
			SetHealth( 0.0f );
...
//-----------------------------------------------------------------------
bool CActor::ScheduleItemSwitch(EntityId itemId, bool keepHistory, i32 category, bool forceFastSelect)
{
	...
	if (!isMounted && (gEnv->IsEditing() == false))
	{
		...
		u32 deselectDelay = pCurrentItem ? pCurrentItem->StartDeselection(fastDeselect) : 0;
...
//------------------------------------------------------------------------
void CActor::SelectItemByName(tukk name, bool keepHistory, bool forceFastSelect)
{
	if (!m_pInventory)
		return;
...
bool CActor::UseItem(EntityId itemId)
{
	...
	DrxLog ("UseItem: %s %s wants to use %s %s %s", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), pItem ? pItem->GetEntity()->GetClass()->GetName() : "NULL", pItem ? pItem->GetEntity()->GetName() : "item", canBeUsed ? "which is fine!" : "but it's not allowed!");
...
bool CActor::PickUpItem(EntityId itemId, bool sound, bool select)
{
	...
	if (gEnv->bServer || (pItem->GetEntity()->GetFlags()&(ENTITY_FLAG_CLIENT_ONLY|ENTITY_FLAG_SERVER_ONLY)))
	{
		...
		m_pGameplayRecorder->Event(GetEntity(), GameplayEvent(eGE_ItemPickedUp, 0, 0, (uk )(EXPAND_PTR)pItem->GetEntityId()));
	}
	else
	{
		if(!m_bAwaitingServerUseResponse)
...
bool CActor::PickUpItemAmmo(EntityId itemId)
{
	...
	if (gEnv->bServer || (pItem->GetEntity()->GetFlags()&(ENTITY_FLAG_CLIENT_ONLY|ENTITY_FLAG_SERVER_ONLY)))
	{
		...
		//m_pGameplayRecorder->Event(GetEntity(), GameplayEvent(eGE_ItemPickedUp, 0, 0, (uk )pItem->GetEntityId()));
	}
	else
	{
		if(!m_bAwaitingServerUseResponse)
	...
	if (pItem->CanDrop())
	{
		...
		if (gEnv->bServer)
		{
			...
			if (!bydeath)
			{
				...
				m_pGameplayRecorder->Event(GetEntity(), GameplayEvent(eGE_ItemDropped, 0, 0, (uk )(EXPAND_PTR)(itemId)));
			}
		}
		else
		{
...
void CActor::ServerExchangeItem(CItem* pCurrentItem, CItem* pNewItem)
{
	if (pCurrentItem && pNewItem)
	{
		...
		{
			DrxLog("%s tried to exchange %s for %s, but the new item already has an owner %d", GetEntity()->GetName(), pCurrentItem->GetEntity()->GetName(), pNewItem->GetEntity()->GetName(), pNewItem->GetOwnerId());
			return;
		}
		...
		if (pCurrentWeapon && pNewWeapon)
		{
			...
			for (size_t i = 0; i < pCurrentWeaponParams->ammoParams.ammo.size(); ++i)
			{
				...
				i32 bonusAmmo = pCurrentWeapon->GetBonusAmmoCount(pAmmoType);
				...
				for (size_t j = 0; j < pNewWeaponParams->ammoParams.ammo.size(); ++j)
				{
					...
					i32 currentCapacity = pInventory->GetAmmoCapacity(pAmmoType);
					i32 currentAmmo = pInventory->GetAmmoCount(pAmmoType);
					i32 newIventoryAmmount = min(currentAmmo + bonusAmmo, currentCapacity);
					i32 newDropAmmount = max((currentAmmo + bonusAmmo) - currentCapacity, 0);
...
void CActor::DropAttachedItems()
{
	...
	if (gEnv->bServer)
	{
		...
		for (i32 i = 0; i < numItems; i++)
		{
			...
			m_pGameplayRecorder->Event(GetEntity(), GameplayEvent(eGE_ItemDropped, 0, 0, (uk )(EXPAND_PTR)entId));
		}
	}
}
...
float CActor::GetBodyDamageMultiplier(const HitInfo &hitInfo) const
{
	...
	if (bodyDamageProfileId != INVALID_BODYDAMAGEPROFILEID)
	{
		...
		fMultiplier = pBodyDamageUpr->GetDamageMultiplier(bodyDamageProfileId, *GetEntity(), hitInfo);
	}
...
float CActor::GetBodyExplosionDamageMultiplier(const HitInfo &hitInfo) const
{
	...
	if (bodyDamageProfileId != INVALID_BODYDAMAGEPROFILEID)
	{
		...
		fMultiplier = pBodyDamageUpr->GetExplosionDamageMultiplier(bodyDamageProfileId, *GetEntity(), hitInfo);
	}
...
//
u32 CActor::GetBodyDamagePartFlags(i32k partID, i32k materialID) const
{
	const TBodyDamageProfileId bodyDamageProfileId = GetCurrentBodyDamageProfileId();
	if (bodyDamageProfileId != INVALID_BODYDAMAGEPROFILEID)
	{
		CBodyDamageUpr *pBodyDamageUpr = g_pGame->GetBodyDamageUpr();
		DRX_ASSERT(pBodyDamageUpr != NULL);

		HitInfo hitInfo;
		hitInfo.partId = partID;
		hitInfo.material = materialID;
		return pBodyDamageUpr->GetPartFlags(bodyDamageProfileId, *GetEntity(), hitInfo);
	}
...
TBodyDamageProfileId CActor::GetBodyDamageProfileID(
	tukk bodyDamageFileName, tukk bodyDamagePartsFileName) const
{
	CBodyDamageUpr *bodyDamageUpr = g_pGame->GetBodyDamageUpr();
	DRX_ASSERT(bodyDamageUpr != NULL);

	IEntity* entity = GetEntity();
	assert(entity != NULL);
	return bodyDamageUpr->GetBodyDamage(*entity, bodyDamageFileName, bodyDamagePartsFileName);
}
...
bool CActor::IsBodyDamageFlag(const HitInfo &hitInfo, const EBodyDamagePIDFlags flag) const
{
	...
	if (bodyDamageProfileId != INVALID_BODYDAMAGEPROFILEID)
	{
		...
		bResult = ((pBodyDamageUpr->GetPartFlags( bodyDamageProfileId, *GetEntity(), hitInfo ) & flag) == flag);
	}
...
void CActor::ProcessDestructiblesHit( const HitInfo& hitInfo, const float previousHealth, const float newHealth )
{
	...
	{
		g_pGame->GetBodyDamageUpr()->ProcessDestructiblesHit(*GetEntity(), m_bodyDestructionInstance, hitInfo, previousHealth, newHealth);
	}
}
...
void CActor::ProcessDestructiblesOnExplosion( const HitInfo& hitInfo, const float previousHealth, const float newHealth )
{
	if (m_bodyDestructionInstance.GetProfileId() != INVALID_BODYDAMAGEPROFILEID)
	{
		g_pGame->GetBodyDamageUpr()->ProcessDestructiblesOnExplosion(*GetEntity(), m_bodyDestructionInstance, hitInfo, previousHealth, newHealth);
	}
}
...
void CActor::SerializeLevelToLevel( TSerialize &ser )
{
	...
	if(ser.IsReading() && IsClient())
	{
		...
		hudEvent_initLocalPlayer.AddData(static_cast<i32>(GetEntityId()));
...
IMPLEMENT_RMI(CActor, SvRequestDropItem)
{
	...
	//DrxLogAlways("%s::SvRequestDropItem(%s)", GetEntity()->GetName(), pItem->GetEntity()->GetName());
...
{
	DRX_ASSERT(m_bAwaitingServerUseResponse);
...
//------------------------------------------------------------------------
CItem * CActor::StartRevive(i32 teamId)
{
...
//------------------------------------------------------------------------
void CActor::NetReviveAt(const Vec3 &pos, const Quat &rot, i32 teamId, u8 modelIndex)
{
	DrxLog("CActor::NetReviveAt: %s", GetEntity()->GetName());
	INDENT_LOG_DURING_SCOPE();
	...
	CActorUpr::GetActorUpr()->ActorRevived(this);
}

//------------------------------------------------------------------------
void CActor::NetReviveInVehicle(EntityId vehicleId, i32 seatId, i32 teamId)
{
	CItem * currentItem = StartRevive(teamId);
...
void CActor::FillHitInfoFromKillParams(const CActor::KillParams& killParams, HitInfo &hitInfo) const
{
	...
	if ((hitInfo.partId != u16(-1)) && (pCharacter = GetEntity()->GetCharacter(0)))
	{
		...
		else
		{
			...
			IAttachment* pAttachment = pAttchmentUpr->GetInterfaceByIndex(hitInfo.partId - FIRST_ATTACHMENT_PARTID);
			if (pAttachment)
			{
...
{
	DrxLog("CActor::NetSimpleKill() '%s", GetEntity()->GetName());
	if (gEnv->bMultiplayer && IsPlayer())
	{
...
//------------------------------------------------------------------------
void CActor::GetMemoryUsage(IDrxSizer * s) const
{
...
void CActor::GetInternalMemoryUsage(IDrxSizer * s) const
{
...
IMPLEMENT_RMI(CActor, ClRevive)
{
	...
	if (pSpawnPoint == NULL)
	{
		...
		DrxLog("CActor::ClRevive() Unable to find spawn point index %d (num known spawns=%d), disconnecting", params.spawnPointIdx, pSpawningModule->GetSpawnLocationCount());
		return false;
	}
	...
	DrxLog("CActor::ClRevive(), Actor: '%s' - Spawning at EntityID: %d, '%s', class '%s', position: %.2f %.2f %.2f\n",
...
IMPLEMENT_RMI(CActor, ClKill)
{
	...
	{
		DrxLog("CActor::ClKill - %d '%s' killed by %d '%s' (damage=%f type=%s)", params.targetId, GetEntity()->GetName(), params.shooterId, pGameRules->GetEntityName(params.shooterId), params.damage, pGameRules->GetHitType(params.hit_type));
...
IMPLEMENT_RMI(CActor, ClMoveTo)
{
	...
	DrxLog("ClMoveTo: %s '%s' moving from (%.2f %.2f %.2f) to (%.2f %.2f %.2f) while state=%s", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), worldPos.x, worldPos.y, worldPos.z, params.pos.x, params.pos.y, params.pos.z, GetActorStats()->isRagDoll ? ", ragdoll" : "");
#endif
...
IMPLEMENT_RMI(CActor, ClPickUp)
{
	...
	{
		//DrxLog("%s::ClPickUp %s", GetEntity()->GetName(), pItem->GetEntity()->GetName());
		...
		else
		{
			...
			tukk displayName = pItem->GetDisplayName();
...
{
	//DrxLog("%s::ClClearInventory", GetEntity()->GetName());
...
IMPLEMENT_RMI(CActor, ClAssignWeaponAttachments)
{
	...
	i32 numAttachments = params.m_attachments.size();
	...
	DrxLog("%s::ClAssignWeaponAttachments numAttachments:%d", GetEntity()->GetName(), numAttachments);
	INDENT_LOG_DURING_SCOPE();
...
void CActor::DumpActorInfo()
{
  ...
  DrxLog("ActorInfo for %s", pEntity->GetName());
  DrxLog("=====================================");

  Vec3 entPos(pEntity->GetWorldPos());
  DrxLog("Entity Pos: %.f %.f %.f", entPos.x, entPos.y, entPos.z);
  DrxLog("Active: %i", pEntity->IsActive());
  DrxLog("Hidden: %i", pEntity->IsHidden());
  DrxLog("Invisible: %i", pEntity->IsInvisible());
  DrxLog("Profile: %i", m_currentPhysProfile);
  DrxLog("Health: %8.2f", m_health.GetHealth());

  if (IPhysicalEntity* pPhysics = pEntity->GetPhysics())
  {
    DrxLog("Physics type: %i", pPhysics->GetType());

    pe_status_pos physicsPos;
    if (pPhysics->GetStatus(&physicsPos))
    {
      DrxLog("Physics pos: %.f %.f %.f", physicsPos.pos.x, physicsPos.pos.y, physicsPos.pos.z);
    }

    pe_status_dynamics dyn;
    if (pPhysics->GetStatus(&dyn))
    {
      DrxLog("Mass: %.1f", dyn.mass);
      DrxLog("Vel: %.2f %.2f %.2f", dyn.v.x, dyn.v.y, dyn.v.z);
    }
  }

  if (IVehicle* pVehicle = GetLinkedVehicle())
  {
    DrxLog("Vehicle: %s (destroyed: %i)", pVehicle->GetEntity()->GetName(), pVehicle->IsDestroyed());

    IVehicleSeat* pSeat = pVehicle->GetSeatForPassenger(GetEntityId());
    DrxLog("Seat %i", pSeat ? pSeat->GetSeatId() : 0);
  }

  if (IItem* pItem = GetCurrentItem())
  {
    DrxLog("Item: %s", pItem->GetEntity()->GetName());
  }
...
//-------------------------------------------------------------------
//This function is called from Equipment Upr only for the client actor
void CActor::NotifyInventoryAmmoChange(IEntityClass* pAmmoClass, i32 amount)
{
	if(!pAmmoClass)
		return;
}
...
bool CActor::IsFriendlyEntity(EntityId entityId, bool bUsingAIIgnorePlayer/* = true*/) const
{
	...
	else
	{
		if(pGameRules)
		{
			...
			{
				i32 iMyTeam=pGameRules->GetTeam(GetEntityId());
				i32 iClientTeam = pGameRules->GetTeam(entityId);
...
bool CActor::LoadAutoAimParams(SmartScriptTable pEntityTable, SAutoaimTargetRegisterParams &outAutoAimParams)
{
	...
	if (pEntityTable && pEntityTable->GetValue("gameParams", pGameParams))
	{
		...
		if (pGameParams->GetValue("autoAimTargetParams", pAutoAimParams))
		{
			...
			i32 primaryTargetId = -1, secondaryTargetId = -1, physicsTargetId = -1;
...
//------------------------------------------------------------------------
void CActor::RegisterInAutoAimUpr()
{
	if (m_registeredInAutoAimMng)
		return;
	...
	if (m_LuaCache_GameParams)
	{
		RegisterInAutoAimUpr(m_LuaCache_GameParams->autoAimParams);
	}
	else
	{
		IEntity *pEntity = GetEntity();
		...
		{
			RegisterInAutoAimUpr(autoAimParams);
		}
		else
		{
...
//------------------------------------------------------------------------
void CActor::RegisterInAutoAimUpr(const SAutoaimTargetRegisterParams &autoAimParams)
{
	m_registeredInAutoAimMng = ShouldRegisterAsAutoAimTarget() && g_pGame->GetAutoAimUpr().RegisterAutoaimTargetActor(*this, autoAimParams);
}

//------------------------------------------------------------------------
void CActor::UnRegisterInAutoAimUpr()
{
	if (!m_registeredInAutoAimMng)
		return;

	g_pGame->GetAutoAimUpr().UnregisterAutoaimTarget(GetEntityId());
...
void CActor::StartInteractiveAction( EntityId entityId, i32 interactionIndex )
{

}

void CActor::StartInteractiveActionByName( tukk interaction, bool bUpdateVisibility, float actionSpeed /*= 1.0f*/ )
{
...
tukk  CActor::GetShadowFileModel()
{
	if (m_LuaCache_Properties)
	{
...
void CActor::ReloadBodyDestruction()
{
	if (m_bodyDestructionInstance.GetProfileId() != INVALID_BODYDESTRUCTIBILITYPROFILEID)
	{
		g_pGame->GetBodyDamageUpr()->GetBodyDestructibility(*GetEntity(), m_bodyDestructionInstance);
	}
}
...
void CActor::GenerateBlendRagdollTags()
{
	...
	const CTagDefinition* pTagDefinition = mannequinSys.GetAnimationDatabaseUpr().FindTagDef( "Animations/Mannequin/ADB/blendRagdollTags.xml" );
...
void CActor::PhysicalizeLocalPlayerAdditionalParts()
{
	...
	i32k vegetationBendingPartId = 101;
	...
	//Offset slightly to the front so we get good/visible bending for the camera distance
	{
		...
		phys_geometry *pGeom = gEnv->pPhysicalWorld->GetGeomUpr()->RegisterGeometry(pPrimGeom, 0);
...
//------------------------------------------------------------------------
EntityId CActor::SimpleFindItemIdInCategory(tukk category) const
{
	EntityId retItem=0;
	...
	if(IInventory* pInventory = GetInventory())
	{
		...
		i32 numItems = pInventory->GetCount();
		...
		for(i32 i = 0; i < numItems; i++)
		{
			...
			if(pItem && pItem->CanSelect())
			{
				tukk itemCategory = pItemSystem->GetItemCategory(pItem->GetEntity()->GetClass()->GetName());
				i32 itemCategoryType = GetItemCategoryType(itemCategory);
...
void CActor::SetTagByCRC(u32 tagId, bool enable)
{
	if(IActionController* pActionController = GetAnimatedCharacter()->GetActionController())
	{
		SAnimationContext &animContext = pActionController->GetContext();
...
void CActor::OnSpectateModeStatusChanged( bool spectate )
{
	...
	{
		i32k channelId = GetGameObject()->GetChannelId();
		...
		if(gEnv->bServer)
		{
			...
			if(!spectate)
			{
				DRX_ASSERT_MESSAGE( pGameRules->GetTeam(GetEntityId()) == 0, "CActor::SetSpectateStatus - Trying to add ex-spectator to a team but they already have a team" );
...
{
	DRX_ASSERT_MESSAGE(IsClient(), "CActor::RequestChangeSpectatorStatus is being called on a non-client player");
...
bool CActor::CanSwitchSpectatorStatus() const
{
	...
	i32k numberOfSpectators = pGameRules->GetSpectatorCount();
	i32k numberOfPlayers = pGameRules->GetPlayerCountClient() - numberOfSpectators;
...
IComponent::ComponentEventPriority CActor::GetEventPriority( i32k eventID ) const
{
	switch( eventID )
	{
	case ENTITY_EVENT_PREPHYSICSUPDATE:
...
void CActor::SetStillWaitingOnServerUseResponse( bool waiting )
{
	...
	{
		DrxFatalError("CActor::SetStillWaitingOnServerUseResponse - Two systems are triggering to wait for a server response at the same time.");
	}
#endif
	m_fAwaitingServerUseResponse = 0.f;
		...
		if (IPhysicalEntity *pCharPhys = pChar->GetISkeletonPose()->GetCharacterPhysics())
			if (!m_pLegsCollider[0])
			{
				...
				if (pCharPhys->GetParams(&ppart))
				{
					...
					i32 bFrameQueued=m_pLegsFrame->SetParams(&pf)-1, bLegsQueued=0;
					pe_geomparams gp;
					...
					for(i32 i=0; i<2; i++,boneName[6]='R')
			...
			}	else for(i32 i=0;i<2;i++)	if (m_pLegsCollider[i])
			{
				QuatT qbone = pChar->GetISkeletonPose()->GetAbsJointByID(m_iboneLeg[i]);
				i32 bActive = isneg((gEnv->pSystem->GetViewCamera().GetPosition()-pos).len2()-sqr(g_pGameCVars->pl_legs_colliders_dist));
...
{
	for(i32 i=0;i<2;i++) if (m_pLegsCollider[i])
