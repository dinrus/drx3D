...
/*
	allows game data files to be downloaded from a http server via DrxNetwork's
	CDrxTCPService module

	not to be confused with CDownloadUpr in DrxSystem, which is windows only
	and doesn't go through the crynetwork abstraction API

	CDownloadUpr should probably be replaced with this one
*/
...
static tukk k_platformPrefix="pc";
...
CDownloadableResource::~CDownloadableResource()
{
	...
	DRX_ASSERT_MESSAGE((m_state&k_callbackInProgressMask)==0,"Deleting a resource which is being downloaded - CRASH LIKELY!");	// shouldn't happen due to ref counting
...
#if defined(DEDICATED_SERVER)
	CPatchPakUpr *pPatchPakUpr = g_pGame->GetPatchPakUpr();
	if (pPatchPakUpr)
	{
		pPatchPakUpr->UnregisterPatchPakUprEventListener(this);
	}
#endif
}
...
void CDownloadableResource::BroadcastFail()
{
	...
	i32k numElements = m_listeners.size();
	...
	if (gEnv->IsDedicated())
	{
		if (m_roundsUntilQuit > 0)
		{
			...
			if (m_timeSinceLastShutdownMessage > g_pGameCVars->g_shutdownMessageRepeatTime)
			{
				...
				if(pLobby)
				{
					i32 numPlayers = pLobby->GetNumberOfExpectedClients();
					if (numPlayers == 0)
					{
...
{
	DrxLog("CDownloadMgr::WeNeedToQuit() quitOnNewDataFound=%d", g_pGameCVars->g_quitOnNewDataFound);
...
{
	DrxLog("CDownloadMgr::OnResourceChanged() so calling WeNeedToQuit()");
	WeNeedToQuit();
}
...
void CDownloadMgr::InitShutdown()
{
	...
	DrxLogAlways("Server is shutting down now to apply data patch");
	...
	if (pLobby)
	{
		...
		if (pMatchmaking)
		{
			...
			if (pGameLobby)
			{
				...
				i32k namesCount = sessionNames.m_sessionNames.size();
...
#ifndef _RELEASE
	DrxLog ("Loaded downloadable resource config (server='%s', name='%s', URL prefix='%s', URL='%s', port=%d, max size=%d)", m_server.c_str(), m_descName.c_str(), m_urlPrefix.c_str(), m_url.c_str(), m_port, m_maxDownloadSize);
#endif
}
	...
	const char		*pInResourceDescriptionsFile)
{
	...
	for (i32 i=0, n=node->getChildCount(); i<n; i++)
	{
	...
	if (resources)
	{
		i32 numResources=resources->getChildCount();
...
#if defined(DEDICATED_SERVER)
	CPatchPakUpr *pPatchPakUpr = g_pGame->GetPatchPakUpr();
	DRX_ASSERT(pPatchPakUpr);
	pPatchPakUpr->RegisterPatchPakUprEventListener(this);
#endif
}
...
{
#if defined(DEDICATED_SERVER)
	DrxLog("CDownloadUpr::UpdatedPermissionsNowAvailable() so calling WeNeedToQuit()");
	WeNeedToQuit();
#endif
}
	...
	size_t							inDataLen)
{
	...
	if ( pInData && inDataLen )
	{
		...
		{
			i32						expectedSize=m_contentLength+m_contentOffset;
			...
			if (newUsed<=m_maxDownloadSize)
			{
				...
				newSize = max(newUsed, max((i32)k_httpHeaderSize,expectedSize));
	...
	if (m_state==k_awaitingHTTPResponse)
	{
		...
		if (m_bufferUsed>=k_httpHeaderSize || inReceivedEndOfStream)
		{
			...
			if (m_bufferUsed>=(sizeof(k_httpOKv10)-1) && (memcmp(k_httpOKv10,m_pBuffer,sizeof(k_httpOKv10)-1)==0 || memcmp(k_httpOKv11,m_pBuffer,sizeof(k_httpOKv11)-1)==0))
			{
				...
				char		lengthStr[12];
				const char	*contentLength=DrxStringUtils::strnstr(m_pBuffer,k_httpContentLength,m_bufferUsed);

				if (contentLength && ((contentLength-m_pBuffer)+sizeof(lengthStr)) < (size_t)m_bufferUsed)
				{
					size_t	found=drx_copyStringUntilFindChar(lengthStr,contentLength+sizeof(k_httpContentLength)-1,sizeof(lengthStr),'\n');

					if (found!=0)
					{
						i32	len=atoi(lengthStr);
						...
						if (len>=0 && len<m_maxDownloadSize)
						{
							...
							for (tukk iter=contentLength; iter<(m_pBuffer+m_bufferUsed-3); ++iter)
							{
								if (iter[0]=='\r' && iter[1]=='\n' && iter[2]=='\r' && iter[3]=='\n')			// some bad http servers don't do CRLF, but do LF LF instead. we won't use those servers
								{
									m_contentOffset=i32(iter+4-m_pBuffer);
									badHeader=false;
									break;
								}
							}
				...
				{ 
					if (inReceivedEndOfStream)
					{
						...
						// now scan for end of header, which is denoted by two CRLFs in http protocol
						for (tukk iter=m_pBuffer; iter<(m_pBuffer+m_bufferUsed-3); ++iter)
						{
							if (iter[0]=='\r' && iter[1]=='\n' && iter[2]=='\r' && iter[3]=='\n')			// some bad http servers don't do CRLF, but do LF LF instead. we won't use those servers
							{
								contentOffset=i32(iter+4-m_pBuffer);
								break;
							}
						}
						...
						if (contentOffset > 0)
						{
							i32 currentContentLength = m_bufferUsed - contentOffset;
	...
	const char									*inResourceName)
{
	...
	if (templateResource)
	{
		DrxFixedStringT<128>			url=templateResource->m_url;
		ICVar											*pLanguage=gEnv->pConsole->GetCVar("g_language");

		DRX_ASSERT_MESSAGE(pLanguage,"can't find language cvar??");
		if (pLanguage)
		{
...
void CDownloadableResource::InitHTTPParser()
{
	...
	static i32k	k_maxDownloadSizeForHTTPHeader=4*1024;
...
void CDownloadableResource::GetUserAgentString(DrxFixedStringT<64> &ioAgentStr)
{
	tukk gameTitle="GameSDK";
	tukk version="1.0";
...
void CDownloadableResource::StartDownloading()
{
	if (m_state==k_notStarted)
	{
		...
		else
		{
			...
			IDrxLobbyService* pLobbyService = pLobby->GetLobbyService();
			...
			if (m_pService)
			{
				static i32k					MAX_HEADER_SIZE=400;
				DrxFixedStringT<MAX_HEADER_SIZE>	httpHeader;
				DrxFixedStringT<64> agentStr;
...
void CDownloadableResource::SetDownloadInfo(tukk pUrl, tukk pUrlPrefix, tukk pServer, i32k port, i32k maxDownloadSize, tukk pDescName/*=NULL*/)
{
	DRX_ASSERT(pUrl && pServer && maxDownloadSize>0);

	DRX_ASSERT_MESSAGE(m_port==0,"You cannot call CDownloadableResource::SetDownloadInfo() twice on the same resource");		// not unless the code is strengthened anyway
	DRX_ASSERT_MESSAGE(port!=0,"You cannot request a download from port 0");		// invalid generally, but also we use 0 as not initialized here
	...
	i32					signingSaltLength)
{
	...
	if (inDataLen>16)
	{
		...
		TCipher						cipher=pNet->BeginCipher((u8*)pDecryptionKey,decryptionKeyLength);
		if (cipher)
		{
			...
			{
				DrxLog("MD5 fail on downloaded data");
			}
			else
			{
				DrxLog("data passed decrypt and MD5 checks");
				valid=true;
			}
	...
	i32					signingSaltLength)
{
	...
	if (m_state==k_dataAvailable)
	{
		tuk pEncryptedBuffer = m_pBuffer+m_contentOffset;
		i32 encryptedLength = m_contentLength;
		tuk pDecryptedBuffer = NULL;
		i32 decryptedLength = 0;
		...
		{
			u64 destSize = *pOutLen;
			i32 error = gEnv->pDrxPak->RawUncompress(pBuffer, &destSize, pDecryptedBuffer, (u64)decryptedLength);

			if (error == 0)
			{
				*pOutLen = (i32)destSize;
				success = true;
			}
			else
			{
				DrxLog("Failed to uncompress data");
			}
...
{
	DRX_ASSERT_MESSAGE(std::find(m_listeners.begin(), m_listeners.end(), pInListener) == m_listeners.end(), "A downloadable resource should not have two instances of the same listener");
...
//	DRX_ASSERT_MESSAGE(std::find(m_listenersToRemove.begin(), m_listenersToRemove.end(), pInListener) == m_listenersToRemove.end(), "Trying to remove a data listener twice!");
//	DRX_ASSERT_MESSAGE(std::find(m_listeners.begin(), m_listeners.end(), pInListener) != m_listeners.end(), "trying to remove a listener thats not in the listners list");
...
IDataListener::~IDataListener()
{
	...
	if (g_pGame)
	{
		...
		if (pMgr)
		{
			DRX_ASSERT_MESSAGE(!pMgr->IsListenerListening(this),"IDataListener is being deleted but it is still registered as a listener - bad code!");
		}
	}
#endif
...
void CDownloadMgr::WaitForDownloadsToFinish(tukk* resources, i32 numResources, float timeout)
{
...
void CDownloadMgr::OnReturnToLobby()
{
	if (gEnv->IsDedicated() && g_pGameCVars->g_quitOnNewDataFound)
	{
		...
		{
			--m_roundsUntilQuit;
			i32 numPlayers = g_pGame->GetGameLobby()->GetNumberOfExpectedClients();
			...
			{
				DrxLogAlways("Server is shutting down in %d rounds to apply data patch", m_roundsUntilQuit);
			}
		}
	}
}

void CDownloadMgr::CheckUpdates(tukk pInResourceName)
{
...
string CDownloadableResource::GetStateAsString(
	TState					inState)
{
	return AutoEnum_GetStringFromBitfield(inState,k_downloadableResourceStateNames,DRX_ARRAY_COUNT(k_downloadableResourceStateNames));
}
	...
	IConsoleCmdArgs			*pInArgs)
{
	...
	for (TResourceVector::iterator iter=pDlm->m_resources.begin(); iter!=pDlm->m_resources.end(); ++iter)
	{
		...
		DrxLogAlways("Resource %d : %s : state %s    : content %d B / %d B (%.2f bytes/sec) (%.2f elapsed)",
	...
	DrxLogAlways("%d resources",count);
}
	...
	IConsoleCmdArgs				*pInArgs)
{
	...
	if (pInArgs->GetArgCount()==2)
	{
		...
		if (!pRes)
		{
			DrxLogAlways("Unknown resource '%s'",resName.c_str());
		}
		else
		{
			...
			if (pRes->Purge())
			{
				DrxLogAlways("Resource '%s' purged, was in state %s",resName.c_str(),wasState.c_str());
			}
			else
			{
				DrxLogAlways("Resource '%s' NOT purged, callback in progress, in state %s",resName.c_str(),wasState.c_str());
			}
		}
	}
	else
	{
		DrxLogAlways("usage: dlm_purge <resourcename>");
	}
}
		...
		else
		{
			...
			if (state==CDownloadableResource::k_dataAvailable)
			{
				...
				if (pMyBigBuffer)
				{
					...
					pMyBigBuffer[size]=0;
					DrxLogAlways("%s",pMyBigBuffer);
					delete [] pMyBigBuffer;
					// ... i don't even want to think about what all the above just did to the memory state...

					DrxLogAlways("Resource '%s' %d bytes downloaded",pRes->m_descName.c_str(),size);
				}
				else
				{
					DrxLogAlways("Out of mem");
				}
			}
			else
			{
				DrxLogAlways("Resource '%s' state %s",pRes->m_descName.c_str(),pRes->GetStateAsString().c_str());
			}
		}
	}
	else
	{
		DrxLogAlways("usage: dlm_cat <resourcename>");
	}
}

void CDownloadableResource::DebugContentsToString(DrxFixedStringT<512> &outStr)
{
	tukk ss=CDownloadableResource::GetStateAsString(m_state).c_str();
	float				timeElapsed=0.0f;
	if (!(m_state&k_notStarted))
	{
		if (m_state&k_dataAvailable)
...
{
	DrxFixedStringT<512> debugString;

	DebugContentsToString(debugString);
	DrxWatch(debugString.c_str());
}
