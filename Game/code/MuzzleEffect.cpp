...
namespace
{
	...
	i32 BarrelCount(const CFireMode* pFireMode)
	{
...
void CMuzzleEffect::Shoot(CFireMode* pFireMode, Vec3 potentialTarget, i32 barrel)
{
...
void CMuzzleEffect::MuzzleFlashEffect(CFireMode* pFireMode, i32 barrel)
{
	...
	i32 slot	= stats.fp ? eIGS_FirstPerson : eIGS_ThirdPerson;
	i32 id		= stats.fp ? 0 : 1;

	if (!muzzleFlash.effect[id].empty() && !pWeapon->GetEntity()->IsHidden())
	{
		if (barrel < (i32)m_mfIds.size())
		{
...
void CMuzzleEffect::AttachEmitters(CFireMode* pFireMode, i32 barrel)
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_AI);
	...
	i32 id = pWeapon->GetStats().fp ? 0 : 1;
	Vec3 offset(ZERO);
...
void CMuzzleEffect::DetachEmitters(CFireMode* pFireMode, i32 barrel)
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_AI);
...
void CMuzzleEffect::GetMemoryUsage(IDrxSizer * s) const
{
