...
-------------------------------------------------------------------------
История:
- 2:3:2005   16:06 : Created by M�rcio Martins
...
void CC4::InitFireModes()
{
	...
	for(i32 i = 0; i < firemodeCount; i++)
	{
		...
		{
			DRX_ASSERT_MESSAGE((m_plantFM == -1), "Multiple Plant firemodes assigned to weapon");

			m_plantFM = i;
		}
		else if(crygti_isof<CDetonate>(m_firemodes[i]))
		{
			DRX_ASSERT_MESSAGE((m_detonateFM == -1), "Multiple Detonate firemodes assigned to weapon");

			m_detonateFM = i;
		}
	}

	DRX_ASSERT_MESSAGE(m_detonateFM >= 0, "No Detonate firemode assigned to weapon");
	DRX_ASSERT_MESSAGE(m_plantFM >= 0, "No Plant firemode assigned to weapon");

	SetCurrentFireMode(m_plantFM);
}

//------------------------------------------------------------------------
bool CC4::OnActionZoom(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
	return OnActionAttackSecondary(actorId, actionId, activationMode, value);
}

//------------------------------------------------------------------------
bool CC4::OnActionAttackSecondary(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
...
//------------------------------------------------------------------------
void CC4::Update(SEntityUpdateContext& ctx, i32 update)
{
...
//---------------------------------------------------------------------------------
bool CC4::OnActionFiremode(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
	return true;
}

//---------------------------------------------------------------------------------
bool CC4::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags)
{
...
void CC4::OnEnterFirstPerson()
{
	...
	if(!m_pDetonatorArmedMaterial && m_weaponsharedparams->pC4Params)
	{
		...
		if(params)
		{
			...
			{
				i32 count = pOriginalMaterial->GetSubMtlCount();
				i32 armedIndex = m_weaponsharedparams->pC4Params->armedLightMatIndex;
...
//---------------------------------------------------------------------------------
void CC4::PickUp(EntityId picker, bool sound, bool select, bool keepHistory, tukk setup)
{
	IEntityClass* pC4Ammo = GetProjectileClass();
	CPlant* pPlant = crygti_cast<CPlant*>(GetCFireMode(m_plantFM));
	i32 numProjectiles = pPlant ? pPlant->GetNumProjectiles() : 0;
	i32 capacity = SWeaponAmmoUtils::GetAmmoCount(GetWeaponSharedParams()->ammoParams.capacityAmmo, pC4Ammo);
...
void CC4::Drop(float impulseScale, bool selectNext, bool byDeath)
{
	...
	IEntityClass* pC4Ammo = GetProjectileClass();
	i32 dropItems = GetInventoryAmmoCount(pC4Ammo) - 1;
