...
-------------------------------------------------------------------------
История:
- 9:12:2005   10:50 : Created by M�rcio Martins
...
{
	DRX_ASSERT(IsSet());
	DRX_ASSERT(pFiringLocator != this);
...
void CWeapon::OnShoot(EntityId shooterId, EntityId ammoId, IEntityClass* pAmmoType, const Vec3 &pos, const Vec3 &dir, const Vec3&vel)
{
	...
	if (sendHUDEvent)
	{
		...
		event.AddData(SHUDEventData((i32)shooterId));
...
//------------------------------------------------------------------------
void CWeapon::OnFireModeChanged(i32 currentFireMode)
{
...
void CWeapon::OnStartReload(EntityId shooterId, IEntityClass* pAmmoType)
{
	...
	if (CActor *pActor = GetOwnerActor())
	{
		...
		if(pActor->IsClient())
		{
			...
			{
				u32k refillTime = (u32)(GetCurrentAnimationTime(eIGS_Owner) * m_weaponsharedparams->bulletBeltParams.beltRefillReloadFraction);
...
void CWeapon::OnEndReload(EntityId shooterId, IEntityClass* pAmmoType)
{
	...
	{
		tukk magazineAttachment = m_weaponsharedparams->reloadMagazineParams.magazineAttachment.c_str();
...
static bool IsEntityOfClass(IEntity* pEntity, tukk name)
{
	if (pEntity)
	{
...
void CWeapon::OnDropped(EntityId actorId, bool ownerWasAI)
{
	...
	if(gEnv->bServer)
	{
			...
			for (TFireModeVector::const_iterator firemodeCit = m_firemodes.begin(); firemodeCit != firemodesEndIt && removeWeapon; ++firemodeCit)
			{
				...
				if (pFiremode)
				{
					...
					if (pFiremodeAmmo)
					{
						...
						{
							i32 currentClipAmount = GetAmmoCount(pFiremodeAmmo);
		...
		{
			for(u32 i = 0; i < m_bonusammo.size(); ++i)
			{
	...
	for (TFireModeVector::const_iterator firemodeCit = m_firemodes.begin(); firemodeCit != firemodesEndIt ; ++firemodeCit)
	{
		...
		if (pFiremode)
		{
			...
			if (pFiremodeAmmo)
			{
				...
				{
					i32k ammoCount = pAIInventory->GetAmmoCount(pFiremodeAmmo);
					...
					else
					{
						...
						{
							SetInventoryAmmoCount(pFiremodeAmmo, 0);
							SWeaponAmmoUtils::SetAmmo(m_bonusammo, pFiremodeAmmo, std::max(minAmmoDrop, drx_random(0, pDefaultBonusAmmo->count - 1)));
						}
					}
				}
			}
...
void CWeapon::OnDroppedByPlayer(IInventory* pPlayerInventory)
{
	...
	for (TFireModeVector::const_iterator firemodeCit = m_firemodes.begin(); firemodeCit != firemodesEndIt ; ++firemodeCit)
	{
		...
		if (pFiremode)
		{
			...
			{
				// Exchange also ammo pool from inventory to bonus ammo map, for next user who picks it up
				i32 ammoCount = pPlayerInventory->GetAmmoCount(pFiremodeAmmo);
				if (ammoCount > 0)
				{
					if(gEnv->bMultiplayer)
					{
						...
						i32 numForClip = clamp_tpl(clipSize - currentClipAmount, 0, ammoCount);
...
void CWeapon::AnimationEvent(ICharacterInstance *pCharacter, const AnimEventInstance &event)
{
	...
	{
		i32 slot = (GetEntity()->GetCharacter(eIGS_FirstPerson) == pCharacter) ? eIGS_FirstPerson : eIGS_ThirdPerson;

		//Spawn effect only once (in FP the event might come twice, one for FP, one for shadow character)
		if (!m_doingMagazineSwap)
		{
			tukk helperBone = (event.m_BonePathName && event.m_BonePathName[0]) ? event.m_BonePathName : "";
...
//--------------------------------------------------------
void CWeapon::OnAnimationEventStartFire(tukk szCustomParameter)
{
...
//--------------------------------------------------------
void CWeapon::OnZoomChanged(bool zoomed, i32 idx)
{
