...
#define CHECK_OWNER_REQUEST()	\
	{ \
		...
		{ \
			DrxLogAlways("[gamenet] Disconnecting %s. Bogus weapon action '%s' request! %s %d!=%d (%s!=%s)", \
...
//------------------------------------------------------------------------
i32 CWeapon::NetGetCurrentAmmoCount() const
{
	if (!m_fm)
		return 0;
...
//------------------------------------------------------------------------
void CWeapon::NetSetCurrentAmmoCount(i32 count)
{
	if (!m_fm)
		return;
...
i32 CWeapon::GetReloadState() const
{
	return m_reloadState;
}

void CWeapon::SvSetReloadState(i32 state)
{
	m_reloadState = state;
	CHANGED_NETWORK_STATE(this, ASPECT_RELOAD);
}

void CWeapon::ClSetReloadState(i32 state)
{
	assert(!gEnv->bServer);
...
//------------------------------------------------------------------------
void CWeapon::NetShoot(const Vec3 &hit, i32 predictionHandle, i32 fireModeId)
{
...
//------------------------------------------------------------------------
void CWeapon::NetShootEx(const Vec3 &pos, const Vec3 &dir, const Vec3 &vel, const Vec3 &hit, float extra, i32 predictionHandle, i32 fireModeId)
{
	IFireMode *pFM = GetFireMode(fireModeId);
	if (pFM)
	{
...
//------------------------------------------------------------------------
void CWeapon::RequestShoot(IEntityClass* pAmmoType, const Vec3 &pos, const Vec3 &dir, const Vec3 &vel, const Vec3 &hit, float extra, i32 predictionHandle, bool forceExtended)
{
...
void CWeapon::RequestStartMeleeAttack(bool weaponMelee, bool boostedAttack, int8 attackIndex /*= -1*/)
{
	...
	if (!gEnv->bServer && pActor && pActor->IsClient())
	{
		...
		{
			DrxLogAlways("COMBO - RequestStartMeleeAttack - called by NON SERVER client with attack index [%d]",attackIndex);
			DrxLogAlways("COMBO - RequestStartMeleeAttack - calling SvRequestStartMeleeAttack");
		}
	...
	else if (IsServer())
	{
		...
		if(g_pGameCVars->pl_pickAndThrow.environmentalWeaponComboDebugEnabled)
		{
			DrxLogAlways("COMBO - RequestStartMeleeAttack - called by server, about to netserialising to clients w A.Index. [%d]", m_attackIndex);
		}
#endif //#ifndef _RELEASE
...
//------------------------------------------------------------------------
void CWeapon::RequestFireMode(i32 fmId)
{
