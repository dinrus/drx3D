...
#include <drx3D/Game/IPlayerUpdateListener.h>

#include <drx3D/Game/State.h>
#include <drx3D/Game/PlayerStateSwim.h>
#include <drx3D/Game/PlayerStateLedge.h>
#include <drx3D/Game/PlayerStateFly.h>
#include <drx3D/Game/PlayerCamera.h>
#include <drx3D/Game/PlayerRotation.h>
#include <drx3D/Game/PlayerInput.h>
#include <drx3D/Game/PlayerStateUtil.h>
...
#include <drx3D/Game/ICooperativeAnimationUpr.h>
...
#include <drx3D/Game/IForceFeedbackSystem.h>
...
#include <drx3D/Game/MovementAction.h>
#include <drx3D/Game/AnimActionAIMovement.h>

#include <drx3D/Game/SprintStamina.h>
...
#include <drx3D/Game/Nodes/FlowFadeNode.h>

#include <drx3D/Sys/VR/IHMDDevice.h>
#include <drx3D/Sys/VR/IHMDUpr.h>
...
namespace
{
	void RegisterGOEvents( CPlayer& player, IGameObject& gameObject )
	{
		...
		gameObject.RegisterExtForEvents( &player, eventToRegister, sizeof(eventToRegister)/sizeof(i32) );
	}
}

//--------------------
//this function will be called from the engine at the right time, since bones editing must be placed at the right time.
i32 PlayerProcessBones(ICharacterInstance *pCharacter,uk pvPlayer)
{
	...
	u32 numAnim = pISkeletonAnim->GetNumAnimsInFIFO(0);
	if (numAnim)
...
{
	i32k curFrameId = gEnv->nMainFrameID;

	if (IsClient() && m_playerCamera && m_isControllingCamera)
	{
		i32 cameraJnt = GetBoneID(BONE_CAMERA);
		if (cameraJnt >= 0)
		{
			...
			Vec3 worldPos = ZERO;
			i32k slotIndex = 0;
			u32k entityFlags = pEntity->GetSlotFlags(slotIndex);
			...
			if (!IsThirdPerson())
			{
				...
				if (fadeoutNearWallThreshold > FLT_EPSILON)
				{
					...
					i32 cameraJoint = GetBoneID(BONE_CAMERA);
					...
					{
						DrxLogAlways("ERROR: Player Rig does not have a camera bone [BONE_CAMERA]");
					}
					...
					if (pHmdUpr && pHmdUpr->IsStereoSetupOk())
					{
						const HmdTrackingState& sensorState = pHmdUpr->GetHmdDevice()->GetLocalTrackingState();
					...
					i32k numSkipEntities = GetPhysicalSkipEntities(pSkipEntities, 2);
					...
					i32k entTypes = ent_all;
					i32k rwiFlags = rwi_stop_at_pierceable;
...
#define CLOAK_AWARENESS_PARTICLES								"perk_fx.sonar_hearing.cloak_awareness"
	...
	void CPlayer::DebugStateMachineEntity( tukk pName )
	{
		assert (pName != 0);
...
, m_waterInOutSpeed(INVALID_AUDIO_CONTROL_ID)
{
	...
	DRX_ASSERT( m_pPlayerRotation );

	m_pPlayerPluginEventDistributor = new CPlayerPluginEventDistributor;
	DRX_ASSERT( m_pPlayerPluginEventDistributor );
...
//	m_pIEntityAudioProxy = IEntityAudioProxyPtr();
	...
	for (i32 i=0; tmpSoundTable[i].soundID != ESound_Player_Last; ++i)
	{
...
bool CPlayer::Init(IGameObject * pGameObject)
{
	...
	if (pGameRules)
	{
		...
		DRX_ASSERT_MESSAGE(spawningModule, "CPlayer::Init() failed to find required gamerules spawning module");
	...
	if (gEnv->bServer && gEnv->bMultiplayer && pGameRules)
	{
		...
		if (playerStats)
		{
			...
			if (stats)
			{
				...
				{
					DrxLog("CPlayer::Init() creating a player who is NOT allowed to spawn this round. So setting them to spectator state ingame so they can spectate whilst they wait");
...
void CPlayer::RegisterOnHUD( void )
{
	if( IsPlayer() && !m_registeredOnHUD )
	{
				 ...
				 !pGameRules->IsIntroSequenceRegistered() || pGameRules->IntroSequenceHasCompletedPlaying())
		{
			...
			hudEvent_remoteEnterGame.AddData(static_cast<i32>(GetEntityId()));
...
{
	DRX_ASSERT_MESSAGE(IsClient(), "This function should be called only for the client!");
...
void CPlayer::ReadDataFromXML(bool isClientReloading /*= false*/)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Could not find actor params xml for the player.");
	}
...
void CPlayer::InitLocalPlayer()
{
	...
	{
		DRX_ASSERT(!m_pLocalPlayerInteractionPlugin);
		...
		DRX_ASSERT_TRACE (m_playerCamera == NULL, ("%s '%s' already has a camera instance when allocating another = memory leak!", GetEntity()->GetClass()->GetName(), GetEntity()->GetName()));
	...
	CGameMechanismUpr::GetInstance()->Inform(kGMEvent_LocalPlayerInit);
			...
			if(gEnv->IsEditor())
			{
				if( event.nParam[0] )
				{
					if (IsClient())
					{
						...
						hudEvent_initLocalPlayerEditor.AddData(static_cast<i32>(GetEntityId()));
						CHUDEventDispatcher::CallEvent(hudEvent_initLocalPlayerEditor);
					}
					else
					{
						RegisterInAutoAimUpr();
					}
				}
	...
	case ENTITY_EVENT_DONE:
		{
			...
			if (m_pLocalPlayerInteractionPlugin)
			{
				...
				if (bIsRealActor)
				{
					...
					CGameMechanismUpr::GetInstance()->Inform(kGMEvent_LocalPlayerDeinit);
				}
			}
			else
			{
				...
				hudEvent_remoteLeaveGame.AddData(static_cast<i32>(GetEntityId()));
...
void CPlayer::SetChannelId(u16 id)
{
...
void CPlayer::Update(SEntityUpdateContext& ctx, i32 updateSlot)
{
	...
	{
		i32 damageTaken = (i32)(0.5f + m_healthAtStartOfUpdate - max(0.f, m_health.GetHealth()));
		if (damageTaken > 0)
		{
			Vec3 velocity(drx_random(-1.0f, 1.0f), drx_random(-1.0f, 1.0f), drx_random(1.0f, 2.0f));
			DrxWatch3DAdd(string().Format("%d", damageTaken), GetLocalEyePos() + GetEntity()->GetWorldPos(), 2.f, & velocity, 3.f);
		}
	}
	...
	if( gEnv->bMultiplayer )
	{
		...
		if( m_logPingTimer > LOG_PING_FREQUENCY )
		{
			...
			{
				DrxPing ping;
				SDrxMatchMakingConnectionUID conUID = pGameLobby->GetConnectionUIDFromChannelID( GetChannelId() );
				gEnv->pNetwork->GetLobby()->GetMatchMaking()->GetSessionPlayerPing( conUID, &ping );
				DrxUserID guid = pGameLobby->GetUserIDFromChannelID( GetChannelId() );
	...
	if (client)
	{
		...
		{
			ICharacterInstance* pCharacter = GetEntity()->GetCharacter(0);
			IAttachmentUpr* pAttachMan = pCharacter->GetIAttachmentUpr();
			ISkeletonPose* pSkel = pCharacter->GetISkeletonPose();
			IDefaultSkeleton& rIDefaultSkeleton = pCharacter->GetIDefaultSkeleton();
			i32 jntID = rIDefaultSkeleton.GetJointIDByName("Bip01 LHand2Weapon_IKBlend");
			...
			if (jntID >= 0)
			{
				float blendFactor = pSkel->GetRelJointByID(jntID).t.x;
				 static float XPOS = 500.0f;
				 static float YPOS = 100.0f;
		...
		if(g_pGameCVars->g_hideArms != sHideArms)
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_ENTITYSYSTEM, VALIDATOR_WARNING, "upperbody attachment not found while hiding arms");
			}
		...
		{
			tukk const name = "PickAndThrowWeapon";
...
void CPlayer::SetTagByCRC( u32 tagCRC, bool enable )
{
...
void CPlayer::UpdateAnimationState(const SActorFrameMovementParams &frameMovementParams)
{
	...
	else if (IsPlayer() && pActionController)
	{
		...
		if (pWeapon)
		{
			i32 firemodeIdx = pWeapon->GetCurrentFireMode();
			IFireMode *pFireMode = pWeapon->GetFireMode(firemodeIdx);
			if (pFireMode)
			{

				IEntityClass* pAmmoClass = pFireMode->GetAmmoType();
				i32 weaponAmmoCount = pWeapon->GetAmmoCount(pAmmoClass);
				i32 inventoryAmmoCount = pWeapon->GetInventoryAmmoCount(pAmmoClass);
		...
		//		DrxWatch("Vel (%f, %f, %f)", m_stats.velocity.x, m_stats.velocity.y, m_stats.velocity.z);
		//		DrxWatch("LastRequestedVel (%f, %f, %f)", m_lastRequestedVelocity.x, m_lastRequestedVelocity.y, m_lastRequestedVelocity.z);
		//		DrxWatch("DesVel (%f, %f, %f)", desiredVelocity.x, desiredVelocity.y, desiredVelocity.z);
...
void CPlayer::PrePhysicsUpdate()
{
	...
	if (m_pMovementController)
	{
		...
		if (g_pGameCVars->pl_watchPlayerState >= (bIsClient? 1 : 2))
		{
			...
			DrxWatch ("%s stance=%s flyMode=%d %s %s%s%s%s", GetEntity()->GetEntityTextDescription(), GetStanceName(GetStance()), m_stats.flyMode, IsOnGround() ? "ON-GROUND" : "IN-AIR", IsThirdPerson() ? "THIRD-PERSON" : "FIRST-PERSON", IsDead() ? "DEAD" : "ALIVE", m_stats.isScoped ? " SCOPED" : "", m_stats.isInBlendRagdoll ? " FALLNPLAY" : "");
		}
#endif
		...
		if (bIsClient)
		{
			...
			g_pGame->GetAutoAimUpr().SetCloseCombatSnapTarget(0, 0.f, 0.f);
		}
...
void CPlayer::SetIK( const SActorFrameMovementParams& frameMovementParams )
{
	...
	else
	{
		...
		if (IsAIControlled())
		{
			...
			DRX_ASSERT(pAiAnimationComponent);
		...
		{
			u32k lookIKLayer = GetLookIKLayer(m_params);
	...
	if (allowAimIK)
	{
		if (IsThirdPerson())
		{
			...
			ray_hit hit;
			u32k flags = rwi_colltype_any(geom_collides) | rwi_force_pierceable_noncoll | rwi_stop_at_pierceable;
	...
	i32k aimIKLayer = GetAimIKLayer(m_params);
...
void CPlayer::UpdateView(SViewParams &viewParams)
{
	...
	if (g_pGameCVars->pl_debug_view != 0)
		DrxLog("CPlayer::UpdateView: Pre Pos(%f, %f, %f) Rot(%f, %f, %f, %f)", viewParams.position.x, viewParams.position.y, viewParams.position.z, viewParams.rotation.v.x, viewParams.rotation.v.y, viewParams.rotation.v.z, viewParams.rotation.w);
#endif //_RELEASE
	...
	if (g_pGameCVars->pl_debug_view != 0)
	{
		DrxLog("CPlayer::UpdateView: Post Pos(%f, %f, %f) Rot(%f, %f, %f, %f)", viewParams.position.x, viewParams.position.y, viewParams.position.z, viewParams.rotation.v.x, viewParams.rotation.v.y, viewParams.rotation.v.z, viewParams.rotation.w);
		DrxWatch("CPlayer::UpdateView: (%f %f %f)  Rot(%f, %f, %f, %f)", viewParams.position.x, viewParams.position.y, viewParams.position.z, viewParams.rotation.v.x, viewParams.rotation.v.y, viewParams.rotation.v.z, viewParams.rotation.w);
	}
#endif //_RELEASE
...
void CPlayer::StartInteractiveAction(EntityId entityId, i32 interactionIndex)
{
	if(IsPlayer())
	{
...
void CPlayer::StartInteractiveActionByName( tukk interaction, bool bUpdateVisibility, float actionSpeed /*= 1.0f*/ )
{
	if(IsPlayer())
	{
		gEnv->pGame->GetIGameFramework()->AllowSave( false );
...
bool CPlayer::CanMove() const
{
	return (!m_pHitDeathReactions || m_pHitDeathReactions->CanActorMove()) &&
		!gEnv->pGame->GetIGameFramework()->GetICooperativeAnimationUpr()->IsActorBusy(GetEntityId());
}
...
tukk CPlayer::GetActorClassName() const
{
	return "CPlayer";
}
...
void CPlayer::OnStanceChanged(EStance newStance, EStance oldStance)
{
    ...
    DRX_ASSERT_TRACE(newStance == m_stance, ("Expected 'newStance' (%d) to be equal to 'm_stance' (%d)", newStance, m_stance.Value()));
	DRX_ASSERT_TRACE(newStance != oldStance, ("Expected 'newStance' (%d) to be different to 'oldStance' (%d)", newStance, oldStance));
...
void CPlayer::SetParamsFromLua(SmartScriptTable &rTable)
{
	...
	i32 followHead = m_stats.followCharacterHead.Value();
...
void CPlayer::UpdateStats(float frameTime)
{
	...
	bool inSmoke = CSmokeUpr::GetSmokeUpr()->IsPointInSmoke(smokeTestPos, insideFactor);
...
void CPlayer::SetReactionOverlay(EReactionOverlay overlay)
{
	...
	if (pCharInstance)
	{
		if (m_reactionAnims[overlay].animID < 0)
		{
			...
			for (i32 i=EReaction_None+1; i<EReaction_Total; i++)
			{
...
void CPlayer::UpdateReactionOverlay(float frameTime)
{
	if (IsThirdPerson() && gEnv->bMultiplayer)
	{
		...
		i32 oldReactionOverlay = m_reactionOverlay;
...
void SetupPlayerCharacterVisibility(IEntity* playerEntity, bool isThirdPerson, i32 shadowCharacterSlot, bool forceDontRenderNearest)
{
	...
	IAttachmentUpr *attachmentMan		= mainChar->GetIAttachmentUpr();
	ICharacterInstance *shadowChar			= NULL;
	IAttachmentUpr *attachmentManShadow	= NULL;
	if (shadowCharacterSlot >= 0)
	{
		shadowChar = playerEntity->GetCharacter(shadowCharacterSlot);
		if (shadowChar)
		{
			attachmentManShadow = shadowChar->GetIAttachmentUpr();
		}
	}

	bool showShadowChar = g_pGameCVars->g_showShadowChar != 0;

	u32 flags = playerEntity->GetSlotFlags(0);
	u32 currentFlags = flags;
	...
	{
		for (i32 i=0; i<attachmentManShadow->GetAttachmentCount(); i++)
		{
	...
	tukk ppFirstPersonParts[] = { "arms_1p" };
	tukk ppThirdPersonParts[] = { "arms_3p", "head", "eye_left", "eye_right", "lower_body", "upper_body", "googles", "bag_01", "bag_02", "bag_03", "bag_04", "bag_05", "bag_06", "visor" };

	i32k numFirstPersonParts = DRX_ARRAY_COUNT(ppFirstPersonParts);
	i32k numThirdPersonParts = DRX_ARRAY_COUNT(ppThirdPersonParts);

	for (i32 i = 0; i < numFirstPersonParts; ++ i)
	{
	...
	for (i32 i = 0; i < numThirdPersonParts; ++ i)
	{
	...
	if (isThirdPerson)
	{
		for (i32 i=0; i<attachmentMan->GetAttachmentCount(); i++)
		{
...
void CPlayer::UpdateThirdPersonState()
{
	...
	if (m_stats.isThirdPerson)
	{
		...
		if (pEntity)
		{
			i32k nSlotCount = pEntity->GetSlotCount();
			for (i32 i = 0; i < nSlotCount; ++i)
			{
				u32k flags = pEntity->GetSlotFlags(i);
...
void CPlayer::SpawnCorpse()
{
	...
	if(gEnv->bMultiplayer && pEntity && m_dropCorpseOnDeath && g_pGameCVars->pl_spawnCorpseOnDeath)
	{
		if(IPhysicalEntity* pOriginalPhysEnt = pEntity->GetPhysics())
		{
			...
			{
				DrxLog("[CORPSE] Forcing to Ragdoll instantly before corpsing");
		...
		if (pEntity->GetPhysics())
		{
			...
			if (pCharInst)
			{
				...
				{
#ifndef _RELEASE
					DrxLogAlways("[ERROR] Spawned corpse while in stealth kill. Game used to crash shortly, now hopefully won't... Contact Rich S");
#endif
				...
				CTeamVisualizationUpr* pTeamVisUpr = g_pGame->GetGameRules()->GetTeamVisualizationUpr();
				if(pTeamVisUpr)
				{
					CGameRules* pGameRules = g_pGame->GetGameRules();
					pTeamVisUpr->RefreshTeamMaterial(pCloneEntity, false, pGameRules ? pGameRules->GetThreatRating(g_pGame->GetClientActorId(), GetEntityId())==CGameRules::eFriendly : true );
				}
				...
				CCorpseUpr* pCorpseUpr = g_pGame->GetGameRules()->GetCorpseUpr();
				DRX_ASSERT_MESSAGE(pCorpseUpr, "NO CORPSE MANAGER INSTANTIATED");
				pCorpseUpr->RegisterCorpse(corpseId, pCloneEntity->GetWorldPos(), GetBaseHeat());
...
void CPlayer::Revive( EReasonForRevive reasonForRevive )
{
	...
	if (reasonForRevive == kRFR_Spawn)
	{
		...
		{
			//DrxLogAlways("[Visible on next spawn] %d %s - REVIVING - m_bMakeVisibleOnNextSpawn set, making visible!]", GetEntityId(), GetEntity()->GetName());
	...
	if(gEnv->bMultiplayer && reasonForRevive == kRFR_Spawn)
	{
		...
		CTeamVisualizationUpr* pTeamVisUpr = pGameRules->GetTeamVisualizationUpr();
		if(pTeamVisUpr)
		{
			pTeamVisUpr->RefreshPlayerTeamMaterial(GetEntityId());
		}
	}
...
void CPlayer::SetRagdollPhysicsParams(IPhysicalEntity * pPhysEnt, bool fallAndPlay)
{
	...
	if(sp.mass <= 0.0f)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Tried ragdollizing player with 0 mass.");
		sp.mass = 80.0f;
	}
...
{
	DRX_ASSERT(gEnv->bMultiplayer && !gEnv->bServer);
	DrxLog("CPlayer::UnRagdollize() '%s' isRagDoll=%s", GetEntity()->GetName(), m_stats.isRagDoll ? "true" : "false");
	if(m_stats.isRagDoll)
	{
		if (m_pAnimatedCharacter)
		{
...
void CPlayer::UpdateSpectator(float frameTime)
{
		...
		case eASM_Follow:
			{
				...
				i32 numLives = pSpawningMo->GetNumLives();
				if (numLives > 0)
				{
					i32 remainingLives = pSpawningMo->GetRemainingLives(spectatorTarget);
					...
					if (remainingLives == 0)
					{
						...
						{
							DrxLog("CPlayer::UpdateSpectator() we've been looking at an invalid target for long enough, finding a new valid target");
							newTime=0.f;
	...
	if(g_pGameCVars->g_spectate_Debug)
	{
		if ((g_pGameCVars->g_spectate_Debug == 1 && IsClient()) || g_pGameCVars->g_spectate_Debug==2)
		{
			...
			{
				DrxWatch("Player: %s (isLocal=%d)", GetEntity()->GetName(), IsClient());
			}
			...
			tukk szWatching = "not watching anyone";
			if(pWatchingEnt)
			{
				szWatching = pWatchingEnt->GetName();
			}

			DrxWatch("Spectator State  : %s", szSpecState);
			DrxWatch("Spectator Mode   : %s", szSpecMode);
			if(spectatorMode == eASM_Follow)
			{
				tukk settingsName = GetCurrentFollowCameraSettings().m_settingsName;
				DrxWatch("	Follow Settings: %s", settingsName);
			}
			DrxWatch("Spectator Target : %s", szWatching);
		}
	}
...
void CPlayer::CameraShake(float angle,float shift,float duration,float frequency,Vec3 pos,i32 ID,tukk source)
{
  ...
  if (IVehicle* pVehicle = GetLinkedVehicle())
  {
	...
	Ang3 shakeAngle(
		drx_random(0.0f,1.0f)*angleAmount*0.15f,
		(angleAmount*std::min(1.0f,std::max(-1.0f,drx_random(-1.0f, 1.0f)*7.7f)))*1.15f,
		drx_random(-1.0f,1.0f)*angleAmount*0.05f
		);

	Vec3 shakeShift(drx_random(-1.0f, 1.0f)*shiftAmount,0,drx_random(-1.0f, 1.0f)*shiftAmount);
...
float CPlayer::GetRegenerationAmount(float frameTime)
{
	...
	if (!m_isHeadUnderWater)
	{
		if (gEnv->bMultiplayer)
		{
			...
			i32k regenRate = g_pGameCVars->g_mpRegenerationRate;
			if (regenRate == 1)
			{
...
void CPlayer::SelectNextItem(i32 direction, bool keepHistory, i32 category)
{
...
void CPlayer::SelectItemByName(tukk name, bool keepHistory, bool forceFastSelect)
{
	if (IsInPickAndThrowMode())
		return;
...
bool CPlayer::ScheduleItemSwitch(EntityId itemId, bool keepHistory, i32 category, bool forceFastSelect)
{
...
bool CPlayer::SetAspectProfile(EEntityAspects aspect, u8 profile )
{
	CProceduralContextRagdoll* pRagdollContext = NULL;
	...
	u8 currentphys=GetGameObject()->GetAspectProfile(eEA_Physics);
...
bool CPlayer::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
...
void CPlayer::NetSerialize_LedgeGrab( TSerialize ser, bool bReading )
{
	...
	ser.Value("ledgeId", m_ledgeID, 'u16');

	DRX_ASSERT_MESSAGE(m_ledgeFlags <= 3, "ledgeflags are out of range of compression policy ui2!" );
...
{
	DRX_ASSERT_MESSAGE(m_pPlayerTypeComponent, "CPlayer::CanSpectatorOrbitYaw() - Localplayercomponent not found. This function should only be called on the client player");
...
{
	DRX_ASSERT_MESSAGE(m_pPlayerTypeComponent, "CPlayer::CanSpectatorOrbitPitch() - Localplayercomponent not found. This function should only be called on the client player");
...
void CPlayer::NetSerialize_Spectator( TSerialize ser, bool bReading )
{
	...
	u8 state = m_stats.spectatorInfo.state;
	u8 stateWas = state;
	...
	if (bReading)
	{
		...
		{
			DrxLogAlways("[SPECTATE NETSERIALISE]: Still in intro state, but intro has completed playing and we are being told to enter spec-tate mode - did we finish intro before server? - will handle and accept netserialize request");
		}
...
#ifdef SEG_WORLD
	ISegmentsUpr *pSM = gEnv->p3DEngine->GetSegmentsUpr();
#endif
...
void CPlayer::NetSerialize_SnapTarget( TSerialize ser, bool bReading )
{
	...
	if(!bReading)
	{
		m_netCloseCombatSnapTargetId = g_pGame->GetAutoAimUpr().GetCloseCombatSnapTarget();
	}
...
void CPlayer::NetSerialize_StealthKill( TSerialize ser, bool bReading )
{
	...
	if(animIndex > 3)
		DrxFatalError("Index used for stealth kill too high. Value %d, max 3 Change the compression policy of sk_animIdx to use more bits", animIndex);
...
{
	u8 previousHitCounter = m_meleeHitCounter;
	...
	DRX_ASSERT(pHitParams->m_surfaceIdx < 256);
	ser.Value("surfaceIdx", pHitParams->m_surfaceIdx, 'u8');
...
void CPlayer::FullSerialize( TSerialize ser )
{
	...
	//serializing stats
	u8 oldCinematicFlags = m_stats.cinematicFlags;
...
void SPlayerStats::Serialize(TSerialize ser, EEntityAspects aspects)
{
	...
	if (ser.GetSerializationTarget() != eST_Network)
	{
		...
		//i32 godMode(g_pGameCVars->g_godMode);
...
void CPlayer::PlayAction(tukk action,tukk extension, bool looping)
{
	DrxFatalError("CPlayer::PlayAction: This should not be called anymore");
}
				...
				if( pHitByEntity != GetEntity() )
				{
					if( IActor* pActor = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pHitByEntity->GetId()) )
					{
						if(pActor->IsPlayer())
						{
							...
							{
#if !defined(_RELEASE)
								DrxLog("CPlayer::OnCollision() has detected closeup death, turning on ragdoll immediately for players attacker=%s; (local=%d) victim=%s (local=%d) is already ragdoll='%s'", pActor->GetEntity()->GetName(), pActor->IsClient(), GetEntity()->GetName(), IsClient(), static_cast<CActor*>(pActor)->GetActorStats()->isRagDoll ? "true" : "false");
#endif
								RagDollize(false);
							}
						}
...
void CPlayer::HandleEvent( const SGameObjectEvent& event )
{
	...
	case eCGE_AnimateHands:
		CreateScriptEvent("AnimateHands",0,(tukk)event.param);
		break;
	case eCGE_SetTeam:
		{
			const STeamChangeInfo * teamChangeInfo = (const STeamChangeInfo *)event.param;
			//DrxLog("CPlayer::HandleEvent being told SetTeam(%d=>%d) for '%s'", teamChangeInfo->oldTeamNum, teamChangeInfo->newTeamNum, GetEntity()->GetName());
			...
			hudevent.AddData(SHUDEventData((i32)GetEntityId()));
			...
			if(pTeamVisUpr)
			{
				pTeamVisUpr->OnPlayerTeamChange(GetEntityId());
			}
	...
	default:
		{
			...
			if (!bHandled)
			{
				...
				if (event.event == eGFE_BecomeLocalPlayer)
				{
					...
					if (gEnv->bMultiplayer)
					{
						DrxLog("Local player name = %s", GetEntity()->GetName());
					}
...
void CPlayer::ExecuteFootStep(ICharacterInstance* pCharacter, const float frameTime, i32k nFootJointID)
{
	DRX_ASSERT(nFootJointID >= 0);
	...
	if (GetBoneID(boneId) >= 0)
	{
		...
		DrxFixedStringT<16> sEffectWater = "water_shallow";
		...
		if (feetWaterLevel != WATER_LEVEL_UNKNOWN)
		{
			...
			if (depth >= 0.0f)
			{
				...
				if(!gEnv->bMultiplayer && IsClient() && IsSprinting())
				{
					...
					gEnv->pAISystem->GetTargetTrackUpr()->HandleStimulusEventInRange( targetAIObjectId, "SoundWaterRipple", stimulusEventInfo, radius );

					g_pGame->GetGameAISystem()->GetEnvironmentDisturbanceUpr().AddObservableEvent( GetEntity()->GetWorldPos(), 2.0f, "OnWaterRippleSeen", GetEntityId() );
				}
		...
		if (!gEnv->bMultiplayer && m_params.footstepGearEffect)
		{
			...
			else if ((stance == STANCE_STEALTH) && (drx_random(0.0f, 1.0f) < 0.3f))
			{
			...
			if(drx_random(0.0f, 1.0f) < gearEffectPossibility)
			{
		...
		if (g_pGameCVars->g_FootstepSoundsDebug != 0)
		{
			...
			if (gEnv->IsEditor())
				DrxLog("%s[%s] speed=%3.2f, turn=%3.2f, acceleration=%3.1f", color, GetEntity()->GetName(), relativeSpeed, fZRotation, vDeltaMovment.y);
			else
				DrxWatch("%s[%s] speed=%3.2f, turn=%3.2f, acceleration=%3.1f", color, GetEntity()->GetName(), relativeSpeed, fZRotation, vDeltaMovment.y);
		}
		//////////////////////////////////////////////////////////////////////////
#endif // _RELEASE
...
//------------------------------------------------------------------------
tukk CPlayer::GetFootstepEffectName() const
{
	if(IsRemote())
	{
...
void CPlayer::ExecuteFoleySignal(ICharacterInstance* pCharacter, const float frameTime, tukk sFoleyActionName, i32k nBoneJointID)
{
	DRX_ASSERT(nBoneJointID >= 0);
	...
	//create effects
	IMaterialEffects* pMaterialEffects = gEnv->pMaterialEffects;
	DrxFixedStringT<32> sEffectName = "default";
	...
	if (g_pGameCVars->g_FootstepSoundsDebug != 0) //TODO maybe create specific Foley Debug CVar
	{
		...
		if (gEnv->IsEditor())
			DrxLog("Foley %s[%s] Effect: %s speed=%3.2f, turn=%3.2f, acceleration=%3.1f", color, GetEntity()->GetName(), sEffectName.c_str(), relativeSpeed, fZRotation, vDeltaMovment.y);
		else
			DrxWatch("Foley %s[%s] Effect: %s speed=%3.2f, turn=%3.2f, acceleration=%3.1f", color, GetEntity()->GetName(), sEffectName.c_str(), relativeSpeed, fZRotation, vDeltaMovment.y);
	}
	//////////////////////////////////////////////////////////////////////////
#endif // _RELEASE
}
...
//////////////////////////////////////////////////////////////////////////
void CPlayer::ExecuteGroundEffectAnimEvent(ICharacterInstance* pCharacter, const float frameTime, tukk szEffectName, i32k nJointID)
{
	ISkeletonAnim* pSkeletonAnim = pCharacter->GetISkeletonAnim();
	DRX_ASSERT(pSkeletonAnim);
...
void CPlayer::ExecuteFootStepsAIStimulus(const float relativeSpeed, const float noiseSupression)
{
	if (gEnv->pAISystem)
	{
		...
		if (g_pGameCVars->ai_perception.movement_useSurfaceType > 0)
		{
			...
			DRX_ASSERT(pMaterialUpr);
			ISurfaceType *pSurface = pMaterialUpr->GetSurfaceTypeUpr()->GetSurfaceType(GetActorPhysics().groundMaterialIdx);
			if (pSurface)
			{
...
//------------------------------------------------------
void CPlayer::SetFlyMode(u8 flyMode)
{
	if (GetSpectatorMode())
		return;
...
void CPlayer::SetSpectatorModeAndOtherEntId(u8k _mode, const EntityId _othEntId, bool isSpawning)
{
	//DrxLog("%s setting spectator mode %d", GetEntity()->GetName(), mode);
	...
	if (!othEntId && mode)
	{
		...
		{
		case eASM_Fixed:
			DRX_ASSERT_MESSAGE(!server, "The server should've already worked out a valid spectator \"other entity\" for himself!");
		...
		case eASM_Follow:
			DRX_ASSERT_MESSAGE(!server, "The server should've already worked out a valid spectator \"other entity\" for himself!");
			DRX_TODO(18,3,2010, "Could clients choose their own Follow subject here too, meaning no spectator \"other entities\" would need to be sent over the network...?");
			break;
		default:
			DRX_ASSERT_MESSAGE(0, string().Format("Unexpected spectator mode '%d'", mode));
		}
	}
	...
	if (mode && !spinf->mode)  // turning (any) spectator mode ON
	{
		...
		case eASM_Follow:
			{
				...
				if(isLocalPlayer)
				{
					static u32k kDefaultCRC = CCrc32::ComputeLowercase("Default");
					const bool setOk = SetCurrentFollowCameraSettings(kDefaultCRC);
					DRX_ASSERT_MESSAGE(setOk,"Could not set the view mode to \"Default\"");
				}
			}
			break;
		case eASM_Killer:
			{
				...
				if(isLocalPlayer)
				{
					static u32k kKillerCRC = CCrc32::ComputeLowercase("Killer");
					const bool setOk = SetCurrentFollowCameraSettings(kKillerCRC);
					DRX_ASSERT_MESSAGE(setOk,"Could not set the view mode to \"Killer\"");
				}
			}
			break;
		}
		...
		DRX_ASSERT(m_pAnimatedCharacter);
		m_pAnimatedCharacter->ForceRefreshPhysicalColliderMode();
...
bool CPlayer::PickUpItem(EntityId itemId, bool sound, bool select)
{
    ...
    hudevent.AddData(SHUDEventData((i32)itemId));
    CHUDEventDispatcher::CallEvent(hudevent);
...
{
	//DrxLog("CPlayer::NetKill: %s", GetEntity()->GetName());

	m_teamWhenKilled = killParams.targetTeam;

#if !defined(_RELEASE)
	DRX_ASSERT_TRACE(!m_dropCorpseOnDeath, ("Getting multiple deaths for %s '%s' (%s%shealth=%8.2f)", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), IsClient() ? "client, " : "", GetEntity()->GetAI() ? "AI, " : "", m_health.GetHealth()));
#endif
...
void CPlayer::PlaySound(EPlayerSounds soundID, bool play, tukk paramName, float paramValue, tukk paramName2, float paramValue2, float volume, bool playOnRemote )
{
	...
	if (IsClient() || playOnRemote) //currently this is only supposed to be heard by the client (not 3D, not MP)
	{
		...
		case CPlayer::ESound_Player_Last:
			break;
		default:
			DRX_ASSERT(soundID < ESound_Player_Last);
			break;
		}
	}
}
...
//-----------------------------------------------------------------------
void CPlayer::GetMemoryUsage(IDrxSizer * pSizer) const
{
	pSizer->AddObject(this,sizeof(*this));
	GetInternalMemoryUsage(pSizer);
}

void CPlayer::GetInternalMemoryUsage(IDrxSizer * pSizer) const
{
...
#ifdef PLAYER_MOVEMENT_DEBUG_ENABLED
void CPlayer::DebugGraph_AddValue(tukk id, float value) const
{
...
{
	i32k flags = EPlayerStateFlags_OnLadder;
...
void CPlayer::OnIntroSequenceFinished()
{
	...
	IEntity* pEntity = GetEntity();
	// DrxLogAlways("[OnIntroSequenceFinished()] [%d %s] [Time First spawned %.3f, IsDead %s]", pEntity->GetId(), pEntity->GetName(), m_timeFirstSpawned, IsDead() ? "TRUE" : "FALSE");
	...
	{
		// DrxLogAlways("[OnIntroSequenceFinished() - making entity VISIBLE DIRECTLY! - not dead + has spawned]");
		GetEntity()->Invisible(false);
		GetEntity()->Hide(false);
	}
	else
	{
		// DrxLogAlways("[OnIntroSequenceFinished() - making entity visible on next spawn - dead or has not already spawned]");
...
void CPlayer::NotifyObjectGrabbed(bool bIsGrab, EntityId objectId, bool bIsNPC, bool bIsTwoHanded /*= false*/, float fThrowSpeed)
{
	...
	if (!gEnv->bMultiplayer)
	{
		if (bIsNPC && bIsGrab)
		{
			...
			{
				if (GameAI::DeathUpr* pDeathUpr = pGameAISystem->GetDeathUpr())
				{
					pDeathUpr->OnAgentGrabbedByPlayer(objectId);
				}
			}
		}
	}
...
{
	//stop sounds
	for(i32 i = (i32)ESound_Player_First+1; i < (i32)ESound_Player_Last;++i)
...
void CPlayer::AnimationEvent(ICharacterInstance *pCharacter, const AnimEventInstance &event)
{
	...
	if (isMainCharacter)
	{
		...
		static u32k audioTriggerCRC = CCrc32::ComputeLowercase("audio_trigger");
		...
		else if (event.m_EventNameLowercaseCRC32 == audioTriggerCRC)
		{
			...
			if (isClient && m_pIEntityAudioProxy)
			{
				...
				if (event.m_BonePathName && event.m_BonePathName[0] && pCharacter)
				{
					ISkeletonPose* pSkeletonPose = pCharacter->GetISkeletonPose();
					IDefaultSkeleton& rIDefaultSkeleton = pCharacter->GetIDefaultSkeleton();
					i32 nJointID = rIDefaultSkeleton.GetJointIDByName(event.m_BonePathName);
					if (nJointID >= 0)
					{
				...
				/*i32 flags = FLAG_SOUND_DEFAULT_3D;
		...
		else if (animEventsTable.m_ragdollStartId == event.m_EventNameLowercaseCRC32)
		{
			if( !m_stats.isInBlendRagdoll )
			{
				...
				if (0 <= expectedDurationSeconds)
				{
					m_ragdollTime = drx_random(0.0f, 1.0f) * expectedDurationSeconds;
				}
			}
		}
...
//////////////////////////////////////////////////////////////////////////
void CPlayer::OnFootStepAnimEvent(ICharacterInstance* pCharacter, tukk boneName)
{
	i32 nFootJointId = 0;
...
//////////////////////////////////////////////////////////////////////////
void CPlayer::OnFoleyAnimEvent(ICharacterInstance* pCharacter, tukk CustomParameter, tukk boneName)
{
	i32 nBoneJointId = 0;
...
void CPlayer::OnFootStepImpulseAnimEvent(ICharacterInstance* pCharacter, const AnimEventInstance &event)
{
	...
	IDefaultSkeleton& rIDefaultSkeleton = pCharacter->GetIDefaultSkeleton();
	i32k nFootJointId = rIDefaultSkeleton.GetJointIDByName(event.m_BonePathName);
...
void CPlayer::EnterPickAndThrow( EntityId entityPicked, bool selectImmediately /*=true*/, bool forceSelect /*=false*/ )
{
	...
	if (!(IsInCinematicMode() && IsCinematicFlagActive(flags)))
	{
		...
		DRX_ASSERT_MESSAGE( pickAndThrowWeaponId, "Player does not have the 'PickAndThrowWeapon' item" );
...
i32  CPlayer::GetShadowCharacterSlot() const
{
	return 5;
}
...
void CPlayer::DamageInfo(EntityId shooterID, EntityId weaponID, IEntityClass *pProjectileClass, float damage, i32 damageType, const Vec3 hitDirection)
{
	DRX_ASSERT (damageType);
...
void SPlayerRotationParams::ReadAimParams(const IItemParamsNode* pRootNode, tukk aimTypeName, EAimType aimType, bool firstPerson)
{
...
void CPlayer::RegisterKill(IActor *pKilledActor, i32 hit_type)
{
}
...
void CPlayer::AddAmmoToInventory(IInventory* pInventory, IEntityClass* pAmmoClass, IWeapon* pWeapon, i32 totalCount, i32 totalCapacity, i32 increase)
{
	...
	if (IsClient())
	{
		...
		eventPickUp.AddData(SHUDEventData((uk )pWeapon));
		eventPickUp.AddData(SHUDEventData(increase));
		eventPickUp.AddData(SHUDEventData((uk )pAmmoClass));
...
{
	for(i32 i = 0; i < m_numActivePlayerPlugins; ++i)
	{
		m_activePlayerPlugins[i]->Update(dt);
	}
}

i32 CPlayer::GetXPBonusModifiedXP(i32 baseXP)
{
	...
	i32k iModified = (i32)(((f32)baseXP * fModifier) + 0.5f);   //+0.5f means that we get rounding up if >= x.5f and rounding down if <x.5f
 	return iModified;
}

#undef raycast

bool CPlayer::SetActorModel(tukk modelName)
{
	if (m_pAnimatedCharacter)
	{
...
#if !defined(_RELEASE)
		if(IsClient())
			DrxLog("[PLAYER PLUG-IN] %s '%s' attempting to enter plug-in '%s'", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass));
#endif

		if (pluginClass->IsEntered())
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[PLAYER PLUG-IN] %s '%s' tried to enter player plug-in '%s' which is already entered", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass));
			bOk = false;
		}
		else if (pluginClass->GetOwnerPlayer() == NULL)
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[PLAYER PLUG-IN] %s '%s' tried to enter player plug-in '%s' with no owner set", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass));
			bOk = false;
		}
		else if (pluginClass->GetOwnerPlayer() != this)
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[PLAYER PLUG-IN] %s '%s' tried to enter player plug-in '%s' assigned to different owner", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass));
			bOk = false;
		}

		DRX_ASSERT_TRACE (bOk, ("%s %s can't enter player plug-in class '%s'", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass)));

		if (bOk)
		{
			if (m_numActivePlayerPlugins < k_maxActivePlayerPlugIns)
			{
				DRX_ASSERT_MESSAGE(this == pluginClass->GetOwnerPlayer(), string().Format("Player '%s' shouldn't enter a player plug-in owned by player '%s'", GetEntity()->GetName(), pluginClass->GetOwnerPlayer() ? pluginClass->GetOwnerPlayer()->GetEntity()->GetName() : "NULL"));
				pluginClass->Enter(m_pPlayerPluginEventDistributor);

				DRX_ASSERT_MESSAGE(pluginClass->IsEntered(), "CPlayerPlugin was entered but IsEntered() is still returning false! Maybe the base class Enter function wasn't called from the subclass?");
			...
			{
				DrxLog ("Can't enter plug-in \"%s\" because array of size %d is full:", pluginClass->DbgGetClassDetails().c_str(), k_maxActivePlayerPlugIns);
				for (i32 i = 0; i < k_maxActivePlayerPlugIns; ++ i)
				{
					DrxLog ("    %d/%d: %s", i + 1, k_maxActivePlayerPlugIns, m_activePlayerPlugins[i]->DbgGetClassDetails().c_str());
				}
				DRX_ASSERT_MESSAGE(0, string().Format("Can't enter plug-in \"%s\" because array of size %d is full! Maybe you need to increase k_maxActivePlayerPlugIns...", pluginClass->DbgGetClassDetails().c_str(), k_maxActivePlayerPlugIns));
			}
#endif
		}
	}
...
#if !defined(_RELEASE)
		if(IsClient())
			DrxLog("[PLAYER PLUG-IN] %s '%s' attempting to leave plug-in '%s'", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass));
#endif

		if (! pluginClass->IsEntered())
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[PLAYER PLUG-IN] %s '%s' tried to leave player plug-in '%s' which isn't currently entered", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass));
			bOk = false;
		}
		else if (pluginClass->GetOwnerPlayer() == NULL)
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[PLAYER PLUG-IN] %s '%s' tried to leave player plug-in '%s' with no owner set", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass));
			bOk = false;
		}
		else if (pluginClass->GetOwnerPlayer() != this)
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[PLAYER PLUG-IN] %s '%s' tried to leave player plug-in '%s' assigned to different owner", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass));
			bOk = false;
		}

		DRX_ASSERT_TRACE (bOk, ("%s %s can't leave player plug-in class '%s'", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), PLAYER_PLUGIN_DETAILS(pluginClass)));

		if (bOk)
		{
			pluginClass->Leave();
			DRX_ASSERT_MESSAGE(!pluginClass->IsEntered(), "CPlayerPlugin was left but IsEntered() is still returning true! Maybe the base class Leave function wasn't called from the subclass?");
			...
			DRX_ASSERT_MESSAGE(false, string().Format("Player was told to leave plug-in %p which isn't in the list of active plug-ins!", pluginClass));
		}
	}
}
...
void CPlayer::LeaveAllPlayerPlugins()
{
	for(i32 i = 0; i < m_numActivePlayerPlugins; i++)
	{
		...
		DRX_ASSERT_MESSAGE(!m_activePlayerPlugins[i]->IsEntered(), "CPlayerPlugin was left but IsEntered() is still returning true! Maybe the base class Leave function wasn't called from the subclass?");
...
u8 CPlayer::GetJumpCounter() const
{
	return m_jumpCounter;
}

void CPlayer::SetJumpCounter(u8 counter)
{
...
void CPlayer::HasClimbedLedge(u16k ledgeID, bool comingFromOnGround, bool comingFromSprint)
{
	...
	{
		DrxLog("Not net-serialising a ledge grab as the id %d is not valid", ledgeID);
	}
}

u8 CPlayer::GetLedgeCounter() const
{
	return m_ledgeCounter;
}

void CPlayer::SetLedgeCounter( u8 counter)
{
	if(IsRemote() && m_ledgeCounter != counter)
	{
		...
		DRX_ASSERT( LedgeId(m_ledgeID).IsValid() );
...
{
	DRX_ASSERT(ladderId != 0 && m_ladderId == 0);
	m_ladderId = ladderId;
...
//////////////////////////////////////////////////////////////////////////
/*virtual*/
void CPlayer::RequestFacialExpression(tukk pExpressionName /* = NULL */, f32* sequenceLength/* = NULL*/)
{
...
//////////////////////////////////////////////////////////////////////////
const QuatT& CPlayer::GetAnimationRelativeMovement(i32 slot /* = 0 */) const
{
...
//////////////////////////////////////////////////////////////////////////
void CPlayer::GetPlayerProgressions(i32* outXp, i32* outRank, i32* outDefault, i32* outStealth, i32* outArmour, i32* outReincarnations  )
{
...
void CPlayer::NetSetInStealthKill(bool inKill, EntityId targetId, u8 animIndex)
{
	if(!IsDead() && !IsClient())
	{
...
void CPlayer::StopStealthKillTargetMovement(EntityId playerId)
{
	...
	if(pTargetActor)
	{
		DRX_ASSERT_MESSAGE(pTargetActor->GetActorClass() == CPlayer::GetActorClassType(), "CPlayer::NetSetInStealthKill - Expected player; got something else");
...
void CPlayer::CaughtInStealthKill(EntityId killerId)
{
	...
	if(IsClient())
	{
		...
		DRX_ASSERT_MESSAGE(pKillerEntity, "Invalid killer ID for stealth kill");
		if(pKillerEntity)
		{
...
void SNetPlayerProgression::SetRandomValues()
{
	...
	{
		i32 maxRank = pPlayerProgression->GetData(EPP_MaxRank);
		if (maxRank == 0)
		{
			maxRank = 10;
		}
		m_serVals.rank = drx_random(1, maxRank);
		m_serVals.xp = pPlayerProgression->GetXPForRank(m_serVals.rank);
		m_serVals.reincarnations = std::max(drx_random(-10, 9), 0);	// make it more likely to be 0
	}
}
#endif
...
{
	DRX_ASSERT(aspect == CPlayer::ASPECT_RANK_CLIENT);
...
#define PLAYER_PRBLOB_REINCAR_MASK		((0x0f) << PLAYER_PRBLOB_REINCAR_SHIFT)		// (0x0f)==00001111
	...
	DRX_ASSERT((sizeof(blob) * 8) == PLAYER_PRBLOB_NUMBITS);
	...
	if(!reading)
	{
		...
		//DrxLog("[tlh] SNetPlayerProgression::Serialize: WRITING progressions: 0x%x, rank=%d, reincarnations=%d, stealth=%d, armour=%d, default=%d", blob, m_serVals.rank, m_serVals.reincarnations, m_serVals.stealth, m_serVals.armour, m_serVals.defaultMode);
	}

	ser.Value("progress_xp", xp, 'u16');
	ser.Value("progressions", blob, 'i32');

	if(reading)
	{
		i32  defaultMode =	(((blob & PLAYER_PRBLOB_DEFPRO_MASK) >> PLAYER_PRBLOB_DEFPRO_SHIFT) - 1);
		i32  stealth =	(((blob & PLAYER_PRBLOB_STLPRO_MASK) >> PLAYER_PRBLOB_STLPRO_SHIFT) - 1);
		i32  armour =		(((blob & PLAYER_PRBLOB_ARMPRO_MASK) >> PLAYER_PRBLOB_ARMPRO_SHIFT) - 1);
		i32  rank =			(((blob & PLAYER_PRBLOB_RANK_MASK) >> PLAYER_PRBLOB_RANK_SHIFT) - 1);
		i32  reincarnations = (((blob & PLAYER_PRBLOB_REINCAR_MASK) >> PLAYER_PRBLOB_REINCAR_SHIFT) - 1);

		//DrxLog("[tlh] CPlayer::SNetPlayerProgression::Serialize: READING progressions: 0x%x, rank=%d, reincarnations=%d, stealth=%d, armour=%d, default=%d", blob, rank, reincarnations, stealth, armour, defaultMode);
...
void SNetPlayerProgression::GetValues( i32* outXp, i32* outRank, i32* outDefault, i32* outStealth, i32* outArmour, i32* outReincarnations )
{
	if (m_player->IsClient())
		SyncOnLocalPlayer();
...
void SNetPlayerProgression::SetSerializedValues( i32 newXp, i32 newRank, i32 newDefault, i32 newStealth, i32 newArmour, i32 newReincarnations )
{
	if (!m_player->IsClient() && m_serVals.rank != newRank && m_serVals.rank != -1 && newRank != -1)
	{
		...
		promotionEvent.AddData( static_cast<i32>(m_player->GetEntityId()) );
...
void SXPEvents::SerializeWith(TSerialize ser)
{
	...
	for (i32 i=0, m=numEvents; i<m; i++)
	{
		ser.Value("xpdelta", events[i].xpDelta, 'u16');
		i32 reason=events[i].xpReason;
	...
	EXPReason	inReason)
{
	if (gEnv->bMultiplayer && IsClient())
	{
		...
		{
			if (m_netXPEvents.numEvents>=DRX_ARRAY_COUNT(m_netXPEvents.events))
			{
				// full? flush
				CheckSendXPChanges();
			}
			...
			{
				index=DRX_ARRAY_COUNT(m_netXPEvents.events)-1;
			}
...
void CPlayer::CheckSendXPChanges()
{
	...
	{
		if ((now-m_netXPSendTime)>=g_pGameCVars->g_telemetry_xp_event_send_interval || m_netXPEvents.numEvents>=DRX_ARRAY_COUNT(m_netXPEvents.events))
		{
...
{
	for (i32 i=0, m=params.numEvents; i<m; i++)
	{
		EXPReason reason = params.events[i].xpReason;
		i32 xpAwarded = params.events[i].xpDelta;
...
IMPLEMENT_RMI(CPlayer, ClDelayedDetonation)
{
	if(params.entityId)
	{
		...
		eventDelay.AddData(SHUDEventData((i32)params.entityId));
...
void CPlayer::OnChangeTeam()
{
	CGameRules *pGameRules = g_pGame->GetGameRules();
	CGameLobby *pGameLobby = g_pGame->GetGameLobby();
	i32 currentTeamId = pGameRules->GetTeam(this->GetEntityId());

	DrxComment("[CG] CPlayer::OnChangeTeam() '%s' now on team '%i', channelId=%i", GetEntity()->GetName(), currentTeamId, GetChannelId());
	...
	if (pGameLobby && IsPlayer() && currentTeamId && GetChannelId())
	{
		...
		if (!IsClient())
		{
			DrxComment("    not local player");
			// Someone else has changed team
			IActor *pClientActor = g_pGame->GetIGameFramework()->GetClientActor();
			if (pClientActor)
			{
				i32 clientTeamId = pGameRules->GetTeam(pClientActor->GetEntityId());
				DrxComment("        local player is on team %i", clientTeamId);
				if (clientTeamId)
				{
...
void CPlayer::OnLocalPlayerChangeTeam()
{
	...
	CGameLobby *pGameLobby = g_pGame->GetGameLobby();
	i32 clientTeamId = pGameRules->GetTeam(GetEntityId());
	if (clientTeamId && pGameLobby)
	{
		...
		while (IActor* pActor = pActorIterator->Next())
		{
			...
			{
				i32 teamId = pGameRules->GetTeam(pActor->GetEntityId());
...
void CPlayer::UpdatePlayerCinematicStatus(u8 oldFlags, u8 newFlags)
{
...
void CPlayer::ResetCinematicStatus(u8 oldFlags)
{
	if ((oldFlags & SPlayerStats::eCinematicFlag_HolsterWeapon) != 0)
	{
...
void CPlayer::UpdateFrameMovementModifiersAndWeaponStats(CWeapon* pWeapon, float currentTime)
{
	...
	for (i32 smr = 0; smr < SActorParams::eSMR_COUNT; ++smr)
	{
...
void CPlayer::SetSpectatorState( u8 state )
{
	if (gEnv->bServer && IsClient())
	{
		OnLocalSpectatorStateSerialize((CActor::EActorSpectatorState)state, (CActor::EActorSpectatorState)m_stats.spectatorInfo.state);
...
void CPlayer::InitHitDeathReactions()
{
	if (!m_pHitDeathReactions && !IsPoolEntity())
	{
		...
		DRX_ASSERT(m_pHitDeathReactions.get());
	}
}
...
void CPlayer::RefillAmmo()
{
	...
	if (!IsCinematicFlagActive(SPlayerStats::eCinematicFlag_LowerWeapon))
	{
		...
		GetEntity()->SetTimer(REFILL_AMMO_TIMER_ID, (i32)(g_pGameCVars->pl_refillAmmoDelay * 1000.0f));
	}
}

void CPlayer::RefillAmmoDone()
{
	u8k newFlags = m_stats.cinematicFlags &(~SPlayerStats::eCinematicFlag_LowerWeapon);
...
{
	DRX_ASSERT(rayID == m_queuedRayId);
...
void CPlayer::SaveToProfile(IPlayerProfile* pProfile, bool online, u32 reason)
{
}

void CPlayer::LoadFromProfile(IPlayerProfile* pProfile, bool online, u32 reason)
{
}
...
void CPlayer::SetMultiplayerModelName()
{
	...
	CGameRules *pGameRules = g_pGame->GetGameRules();
	i32 teamId = m_teamId;
	i32 teamDiff = g_pGameCVars->g_teamDifferentiation;
	...
	if(bUpdatedActorModel)
	{
		...
		if(pTeamVisUpr)
		{
			pTeamVisUpr->RefreshPlayerTeamMaterial(pEntity->GetId());
		}
	}
...
{
	DRX_ASSERT_MESSAGE(m_pPlayerTypeComponent, "CPlayer::GetCurrentFollowCameraSettings() - Localplayercomponent not found. This function should only be called on the client player");
...
{
	DRX_ASSERT_MESSAGE(m_pPlayerTypeComponent, "CPlayer::ChangeCurrentFollowCameraSettings() - Localplayercomponent not found. This function should only be called on the client player");

	return m_pPlayerTypeComponent->ChangeCurrentFollowCameraSettings(increment);
}

bool CPlayer::SetCurrentFollowCameraSettings(u32 crcName)
{
	DRX_ASSERT_MESSAGE(m_pPlayerTypeComponent, "CPlayer::SetCurrentFollowCameraSettings() - Localplayercomponent not found. This function should only be called on the client player");
...
IMPLEMENT_RMI(CPlayer, SvRequestUseLadder)
{
	if(!IsDead())
	{
		...
		{
			DrxLog("CPlayer::SvRequestUseLadder - %s unable to find entity with id: %d", GetEntity()->GetName(), params.ladderId);
		}
	}
	else
	{
		DrxLog("CPlayer::SvRequestUseLadder - %s cannot use the ladder as they are dead", GetEntity()->GetName());
	}
...
i32 CPlayer::GetPhysicalSkipEntities(IPhysicalEntity** pSkipList, i32k maxSkipSize) const
{
	i32 skipCount = 0;
...
void CPlayer::OnPickedUpPickableAmmo( IEntityClass* pAmmoType, i32 count )
{
...
void CPlayer::RMIBenchmarkCallback( ERMIBenchmarkLogPoint point0, ERMIBenchmarkLogPoint point1, int64 delay, uk pUserData )
{
	static tukk const  pEnds[] = { "client", "server" };
	static tukk const  pActions[] = { "queueing", "sending", "receiving", "handling" };
	static tukk const  pThreads[] = { "game", "network" };
	static tukk const  pMessages[] = { "ping", "pong", "pang", "peng" };
	...
	DrxFixedStringT< 128 >    description;
	...
	{
		DrxLogAlways( "%s: %" PRIi64, description.c_str(), delay );
	}
}
