...
#include <drx3D/Game/UI/Utils/ILoadingMessageProvider.h>
...
{
		g_pGame->GetUI()->GetLayoutUpr()->UpdateHUDCanvasSize();
}
...
static void OnPLRenderInNearestChanged(ICVar* pCVar = NULL)
{
	if(pCVar)
	{
		...
		if(pLocalActor)
		{
			...
			if(pEntity)
			{
				i32 slotFlags = pEntity->GetSlotFlags(eIGS_FirstPerson);
...
{
	enum { count = 4 };
	static tukk s_names[count];
	// Gets number of matches for the argument to auto complete.
	virtual i32 GetCount() const { return count; }
	// Gets argument value by index, nIndex must be in 0 <= nIndex < GetCount()
	virtual tukk GetValue( i32 nIndex ) const { return s_names[nIndex]; }
};

tukk SAutoComplete_v_profile_graph::s_names[SAutoComplete_v_profile_graph::count] =
{
	"slip-speed",
	"slip-speed-lateral",
	"centrif",
...
{
	DRX_ASSERT(gEnv);
	DRX_ASSERT(gEnv->pSystem);
	DRX_ASSERT(g_pGame);

//	if (gEnv->pSystem->IsDevMode())
	{
		i32k iCVarDifficulty = difficultyLevel->GetIVal();
...
void ReloadPickAndThrowProxiesOnChange(ICVar* useProxies)
{
	...
	gEnv->pGame->GetIGameFramework()->GetISharedParamsUpr()->RemoveByType(CPickAndThrowProxy::SPnTProxyParams::s_typeInfo);
...
static void SetControllerLayout( IConsoleCmdArgs* pArgs )
{
	...
	tukk layoutType = pArgs->GetArg(1);
	tukk layoutName = pArgs->GetArg(2);
...
void WeaponOffsetToMannequin(IConsoleCmdArgs* pArgs)
{
	...
	drx_sprintf(cmdBuffer, "e_mannequin_setkeyproperty position_x %f", offset.m_position.x);
	gEnv->pConsole->ExecuteString(cmdBuffer);
	drx_sprintf(cmdBuffer, "e_mannequin_setkeyproperty position_y %f", offset.m_position.y);
	gEnv->pConsole->ExecuteString(cmdBuffer);
	drx_sprintf(cmdBuffer, "e_mannequin_setkeyproperty position_z %f", offset.m_position.z);
	gEnv->pConsole->ExecuteString(cmdBuffer);

	drx_sprintf(cmdBuffer, "e_mannequin_setkeyproperty rotation_x %f", RAD2DEG(offset.m_rotation.x));
	gEnv->pConsole->ExecuteString(cmdBuffer);
	drx_sprintf(cmdBuffer, "e_mannequin_setkeyproperty rotation_y %f", RAD2DEG(offset.m_rotation.y));
	gEnv->pConsole->ExecuteString(cmdBuffer);
	drx_sprintf(cmdBuffer, "e_mannequin_setkeyproperty rotation_z %f", RAD2DEG(offset.m_rotation.z));
	gEnv->pConsole->ExecuteString(cmdBuffer);
}
...
void SCVars::InitCVars(IConsole *pConsole)
{
	...
	{
		i32k iCurValue = pAntialiasingModeCVar->GetIVal();
	...
	REGISTER_CVAR(g_corpseUpr_maxNum, 6, VF_NULL, "Limit for number of corpses active at any time");
	REGISTER_CVAR(g_corpseUpr_thermalHeatFadeDuration, 20.0f, VF_NULL, "Time Duration it takes for corpses heat to fade in thermal vision mode");
	REGISTER_CVAR(g_corpseUpr_thermalHeatMinValue, 0.22f, VF_NULL, "Min heat value in thermal vision mode");
	REGISTER_CVAR(g_corpseUpr_timeoutInSeconds, 0.0f, VF_NULL, "Number of seconds to keep corpses around before removing them. A value <= 0 means: never remove due to timeout.");
	...
	// [pb] Also be aware there is a default that will override this in  PlaylistUpr.cpp ~Ln426 AddGameModeOptionFloat("g_autoReviveTime", "MP/Options/%s/RespawnDelay", true, 1.f, 1.f, XXXX, 0);
	...
	REGISTER_CVAR(ai_SquadUpr_DebugDraw, 0, VF_CHEAT|VF_DUMPTODISK, "1 = Toggles debugging of the Squad Upr [1=on - 0=off]\n");
	REGISTER_CVAR2("ai_SquadUpr_MaxDistanceFromSquadCenter", &ai_SquadUpr_MaxDistanceFromSquadCenter,
		40.0f, 0, "Max distance to be considered into a Squad.\n");
	REGISTER_CVAR2("ai_SquadUpr_UpdateTick", &ai_SquadUpr_UpdateTick,
...
#define DEFAULT_TELEMETRY_MEMORY_SIZE_MP 2*1024*1024
  ...
  REGISTER_CVAR(g_testStatus, 0, 0, "Provided as a mechanism for tests to signal their status to Amble scripts");
	...
	// VTOL Vehicles
	REGISTER_CVAR(g_VTOLVehicleUprEnabled, 1, 0, "Enable VTOLVehicle manager update logic");
...
#ifndef _RELEASE
	REGISTER_CVAR(g_VTOLVehicleUprDebugEnabled, 0, 0, "Enable VTOLVehicle manager Debug logic");
	REGISTER_CVAR(g_VTOLVehicleUprDebugPathingEnabled, 0, 0, "Enable VTOLVehicle manager Debug logic");
	REGISTER_CVAR(g_VTOLVehicleUprDebugSimulatePath, 0, 0, "Simulate VTOLs moving around the path to provide a more accurate debug drawing of the VTOLs path");
	REGISTER_CVAR(g_VTOLVehicleUprDebugSimulatePathTimestep, 0.1f, 0, "Time increment when simulating movement. Lower = more accurate but slower");
	REGISTER_CVAR(g_VTOLVehicleUprDebugSimulatePathMinDotValueCheck, 0.99f, 0, "If the dot from 3 neighboring nodes is greater than this value we can disregard the middle node (Higher = more accurate but more memory/render work)");
#endif
	// ~VTOL Vehicles
...
#ifndef _RELEASE
	REGISTER_CVAR(g_vtolUprDebug, 0, 0, "1: Enable debug options in the vtol manager");
	...
	CBodyUprCVars::RegisterVariables();
...
void SCVars::ReleaseCVars()
{
	...
	pConsole->UnregisterVariable("g_corpseUpr_maxNum", true);
	pConsole->UnregisterVariable("g_corpseUpr_thermalHeatFadeDuration", true);
	pConsole->UnregisterVariable("g_corpseUpr_thermalHeatMinValue", true);
	pConsole->UnregisterVariable("g_corpseUpr_timeoutInSeconds", true);
...
#endif // #if (USE_DEDICATED_INPUT)
	CBodyUprCVars::UnregisterVariables(pConsole);
	...
	pConsole->UnregisterVariable("g_VTOLVehicleUprEnabled", true);
...
#ifndef _RELEASE
	pConsole->UnregisterVariable("g_VTOLVehicleUprDebugEnabled", true);
	pConsole->UnregisterVariable("g_VTOLVehicleUprDebugPathingEnabled", true);
	pConsole->UnregisterVariable("g_VTOLVehicleUprDebugSimulatePath", true);
	pConsole->UnregisterVariable("g_VTOLVehicleUprDebugSimulatePathTimestep", true);
	pConsole->UnregisterVariable("g_VTOLVehicleUprDebugSimulatePathMinDotValueCheck", true);
#endif
	// ~ VTOL Vehicle variables
...
#ifndef _RELEASE
	pConsole->UnregisterVariable("g_vtolUprDebug", true);
...
static bool CB_OpenFile(tukk  filename, bool append)
{
	s_debugSaveFile = fxopen(filename, append ? "ab" : "wb");
	return s_debugSaveFile != 0;
}

static bool CB_WriteToFile(ukk  ptr, u32 size, u32 count)
{
...
{
	i32 iArgCount = pArgs->GetArgCount();
	if (iArgCount == 4 || iArgCount == 7)
	{
		...
		if (pPlayer)
		{
			...
			DrxLogAlways("CmdPlayerGoto() %s '%s' (%s, health=%8.2f/%8.2f%s) BEFORE pos=<%.2f %.2f %.2f> dir=<%.2f %.2f %.2f>", pEntity->GetClass()->GetName(), pEntity->GetName(),
			...
			DrxLogAlways("CmdPlayerGoto() %s '%s' pos=%f, %f, %f", pEntity->GetClass()->GetName(), pEntity->GetName(), pos.x, pos.y, pos.z);
			if (iArgCount == 7)
			{
				rot.x = (float)atof(pArgs->GetArg(4));
				rot.y = (float)atof(pArgs->GetArg(5));
				rot.z = (float)atof(pArgs->GetArg(6));
				DrxLogAlways("CmdPlayerGoto() %s '%s' rot=%f, %f, %f", pEntity->GetClass()->GetName(), pEntity->GetName(), rot.x, rot.y, rot.z);
			}
			...
			DrxLogAlways("CmdPlayerGoto() %s '%s' (%s, health=%8.2f/%8.2f%s) AFTER pos=<%.2f %.2f %.2f> dir=<%.2f %.2f %.2f>", pEntity->GetClass()->GetName(), pEntity->GetName(),
				pPlayer->IsDead() ? "dead" : "alive", pPlayer->GetHealth(), pPlayer->GetMaxHealth(),
				pPlayer->GetActorStats()->isRagDoll ? ", RAGDOLL" : "", pEntity->GetWorldPos().x, pEntity->GetWorldPos().y, pEntity->GetWorldPos().z, pEntity->GetForwardDir().x, pEntity->GetForwardDir().y, pEntity->GetForwardDir().z);
		}
		else
...
void CmdGoto(IConsoleCmdArgs *pArgs)
{
		...
		&& sscanf(pArgs->GetArg(3),"%f",&vPos.z)==1)
	{
		...
		if(ISegmentsUpr *pSM = gEnv->p3DEngine->GetSegmentsUpr())
		{
		...
		char str[256];
		drx_sprintf(str,"ED_GOTO %.2f %.2f %.2f %.2f %.2f %.2f",vPos.x,vPos.y,vPos.z,aAngDeg.x,aAngDeg.y,aAngDeg.z);
...
void CGame::RegisterConsoleCommands()
{
	...
	CBodyUprCVars::RegisterCommands();
...
void CGame::UnregisterConsoleCommands()
{
	...
	CBodyUprCVars::UnregisterCommands(m_pConsole);
...
void CGame::CmdFreeCamEnable(IConsoleCmdArgs *pArgs)
{
	if (s_freeCamActive)
	{
		DrxLogAlways("free cam already enabled");
	}
	else
	{
		DrxLogAlways("Enabling free cam");
...
void CGame::CmdFreeCamDisable(IConsoleCmdArgs *pArgs)
{
	if (s_freeCamActive)
	{
		DrxLogAlways("Disabling free cam");
	...
	{
		DrxLogAlways("free cam is not enabled");
	}
}
...
void CGame::CmdFreeCamLockCamera(IConsoleCmdArgs *pArgs)
{
	if (s_freeCamActive)
	{
		DrxLogAlways("free cam - locking camera, unlocking player");
...
void CGame::CmdFreeCamUnlockCamera(IConsoleCmdArgs *pArgs)
{
	if (s_freeCamActive)
	{
		DrxLogAlways("free cam - unlocking camera, locking player");
...
{
#if DRX_PLATFORM_DURANGO
	g_pGame->GetIGameFramework()->LoadGame(DRX_SAVEGAME_FILENAME DRX_SAVEGAME_FILE_EXT, true);
#else
...
#if !defined(_RELEASE)
		CActor* pActor = (CActor*)pClientActor;
		DrxLogAlways ("Forcing damage %s '%s' (health=%8.2f/%8.2f, %s%s%s, stance=%s, spectating=%d)", pClientActor->GetEntity()->GetClass()->GetName(), pClientActor->GetEntity()->GetName(), pClientActor->GetHealth(), pClientActor->GetMaxHealth(), pClientActor->IsDead() ? "DEAD" : "ALIVE", pClientActor->IsGod() ? ", GOD" : "", pClientActor->IsFallen() ? ", FALLEN" : "", pActor->GetStanceInfo(pActor->GetStance())->name, pActor->GetSpectatorMode());
#endif
...
void CGame::CmdSay(IConsoleCmdArgs *pArgs)
{
	if (gEnv->bServer)
	{
		...
		{
			tukk msg=pArgs->GetCommandLine()+strlen(pArgs->GetArg(0))+1;
			g_pGame->GetGameRules()->SendTextMessage(eTextMessageServer, msg, eRMI_ToAllClients);

			if (!gEnv->IsClient())
				DrxLogAlways("** Server: %s **", msg);
		}
	}
	else
	{
...
{
	tukk message = pArgs->GetCommandLine();
	if (strlen(message) > 5)
	{
		DrxLogAlways("%s", pArgs->GetCommandLine() + 5);
	}
}
...
void CGame::CmdReloadGameRules(IConsoleCmdArgs *pArgs)
{
	...
	if (pEntityClass)
	{
		...
		if (pGameRulesSystem->CreateGameRules(name))
			DrxLog("reloaded GameRules <%s>", name);
		else
...
{
	g_pGame->GetIGameFramework()->GetIActorSystem()->Reload();
	gEnv->pGame->GetIGameFramework()->GetISharedParamsUpr()->RemoveByType(CSpectacularKill::SSharedSpectacularParams::s_typeInfo);
...
{
	g_pGame->GetIGameFramework()->GetIActorSystem()->Reload();
	gEnv->pGame->GetIGameFramework()->GetISharedParamsUpr()->RemoveByType(CActorImpulseHandler::SSharedImpulseHandlerParams::s_typeInfo);
	...
	while (IActor* pIActor = pIt->Next())
	{
		...
		if (pEntityClassParamsNode)
		{
			...
			DRX_ASSERT(pImpulseHandler != NULL);
...
void CGame::CmdGiveGameAchievement(IConsoleCmdArgs* pArgs)
{
	...
	{
		i32k achievementId = atoi(pArgs->GetArg(1));
...
void CGame::CmdSetPlayerHealth(IConsoleCmdArgs* pArgs)
{
	...
	if(pActor)
	{
		if( pArgs->GetArgCount() != 2 )
		{
			DrxLog("pl_health : invalid input : 'pl_health <new_health>' (max health is %8.2f)", pActor->GetMaxHealth() );
			return;
		}
		...
		{
			DrxLog("pl_health : float required for input. 'pl_health <new_health>' (max health is %8.2f)", pActor->GetMaxHealth() );
			return;
		}
	...
	{
		DrxLog("pl_health : no actor found!");
	}
}
...
void CGame::CmdSwitchGameMultiplayer(IConsoleCmdArgs* pArgs)
{
	if( pArgs->GetArgCount() != 2 )
	{
		DrxLogAlways("switch_game_multiplayer : invalid input : 'switch_game_multiplayer <0 - singleplayer/1 - multiplayer>'" );
		return;
	}
}
...
void CGame::CmdSpawnActor(IConsoleCmdArgs *pArgs)
{
	...
	if (argCount >= 2)
	{
		tukk actorClass = pArgs->GetArg(1);
		...
		do
	{
		...
		DrxLogAlways("CmdSpawnActorAtPos() spawning actorClass=%s; with name=%s; at pos=(%f, %f, %f)", actorClass, actorName.c_str(), spawnPosition.x, spawnPosition.y, spawnPosition.z);
		...
		if (pActor)
		{
			DrxLogAlways("CmdSpawnActorAtPos() successfully created actor");
		}
		else
		{
			DrxLogAlways("CmdSpawnActorAtPos() failed to create actor");
		}
	}
	else
	{
		DrxLogAlways("spawnActor actorClass [posx posy posz]");
	}
}
...
void CGame::CmdSpawnDummyPlayer(IConsoleCmdArgs* pArgs)
{
	...
	if(pGameRules)
	{
		for (i32 i=0; i<numDummies; i++)
		{
			pos += forward * 2;
			DrxFixedStringT<16> name("DummyPlayer");
...
void CGame::CmdRemoveDummyPlayers(IConsoleCmdArgs* pArgs)
{
	...
	DrxLogAlways("CmdRemoveDummyPlayers() removed %d dummy players", numPlayersRemoved);
}
...
void CGame::CmdSetDummyPlayerState(IConsoleCmdArgs* pArgs)
{
	if (pArgs->GetArgCount() > 1)
	{
		tukk entityName = pArgs->GetArg(1);
		...
		if (pEntity)
		{
			...
			if (pEntity->GetClass() == pDummyClass)
			{
				...
				for (i32 i = 2; i<pArgs->GetArgCount(); i++)
				{
					bool parsed = false;
					...
					if (!key.empty())
					{
						...
						if (!valueString.empty())
						{
							...
							{
								validValue = false;
								DrxLogAlways("CmdSetDummyPlayerState() unrecognised value: %s in %s", valueString.c_str(), stateString.c_str());
							}
							if (validValue)
							{
								...
								{
									DrxLogAlways("CmdSetDummyPlayerState() unrecognised key: %s in %s", key.c_str(), stateString.c_str());
								}
							}
						}
					}
					if (!parsed)
					{
						DrxLogAlways("CmdSetDummyPlayerState() unrecognised argument: %s", stateString.c_str());
					}
				}
			}
			else
			{
				DrxLogAlways("CmdSetDummyPlayerState() wrong type: %s is a %s and not a DummyPlayer", entityName, pEntity->GetClass()->GetName());
			}
		}
		else
		{
			DrxLogAlways("CmdSetDummyPlayerState() couldn't find %s", entityName);
		}
	}
	else
	{
		DrxLogAlways("CmdSetDummyPlayerState() must be given some parameters e.g. setDummyPlayerState DummyPlayer0 fire:true move:default");
	}
}
...
void CGame::CmdInspectConnectedStorage(IConsoleCmdArgs* pArgs)
{
	if (pArgs->GetArgCount() > 1)
	{
		tukk blobName = pArgs->GetArg(1);
		IPlatformOS::EConnectedStorageType type = IPlatformOS::eCST_ERA;
		tukk containerName = "PlayerProfileAttributes";
		...
		static u8 readBuffer[expectedBlockSize] = {0};
		...
		if (pOS->LoadFromStorage(type, containerNameW.c_str(), block))
		{
			if (dumpToFile)
			{
				...
				gEnv->pDrxPak->AdjustFileName(string("%USER%\\ConnectedStorageDump\\") + containerName + "\\", path, IDrxPak::FLAGS_PATH_REAL | IDrxPak::FLAGS_FOR_WRITING);
				if (gEnv->pDrxPak->MakeDir(path))
				{
					drx_strcat(path, blobName);

					if (FILE* pFile = gEnv->pDrxPak->FOpen(path, "wt"))
					{
						gEnv->pDrxPak->FWrite(block[0].pDataBlock, 1, block[0].dataBlockSize, pFile);
						gEnv->pDrxPak->FClose(pFile);

						DrxLogAlways("CmdInspectConnectedStorage: Container:%s blob:%s dump to:%s", containerName, blobName, path);
					}
				}
			}
			else
			{
				...
				DrxLogAlways("CmdInspectConnectedStorage: Container:%s blob:%s\n%s", containerName, blobName, block[0].pDataBlock);

				string blob((tuk)block[0].pDataBlock, block[0].dataBlockSize);
				while (!blob.empty())
				{
					const size_t charsPerLine = 80;
					string display = blob.Left(charsPerLine);
					DrxLogAlways("> %s", display.c_str());
