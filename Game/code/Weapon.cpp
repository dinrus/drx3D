...
u8 CWeapon::s_requestedActions = 0;
...
{
	MeleeReactionTimer(CWeapon* pWeapon, i32 layer) : m_pWeapon(pWeapon), m_layer(layer) {}
	...
	m_weaponNextShotTimer(0.f)
{
	...
	DrxCreateClassInstance("AnimationPoseModifier_OperatorQueue", m_BeltModifier);
}
...
void CWeapon::InitCompatibleAccessories()
{
	...
	i32 n = m_sharedparams->accessoryparams.size();
	for (i32 i = 0; i < n; i++)
	{
...
void CWeapon::InitFireModes()
{
	...
	i32 n = m_weaponsharedparams->firemodeParams.size();
	...
	for (i32 i = 0; i<n; i++)
	{
		...
		m_fmIds.insert(TFireModeIdMap::value_type(DrxHashStringId(pNewParams->initialiseParams.modeName.c_str()), fireModeId));
...
void CWeapon::InitZoomModes()
{
	...
	i32 n = m_weaponsharedparams->zoommodeParams.size();
	...
	for (i32 i = 0; i < n; i++)
	{
		...
		i32 idx = m_zoommodes.size()-1;
		m_zmIds.insert(TZoomModeIdMap::value_type(DrxHashStringId(pNewParams->initialiseParams.modeName.c_str()), idx));
...
void CWeapon::InitAIData()
{
	...
	{		
		...
		{
			tukk smartObjectClassProperties = NULL;
...
bool CWeapon::Init( IGameObject * pGameObject )
{
	...
	if(!gEnv->bMultiplayer)
		g_pGame->GetTacticalUpr()->AddEntity(GetEntityId(), CTacticalUpr::eTacticalEntity_Item);
...
{
	if (!gEnv->bMultiplayer && g_pGame && g_pGame->GetTacticalUpr())
	{
		g_pGame->GetTacticalUpr()->RemoveEntity(GetEntityId(), CTacticalUpr::eTacticalEntity_Item);
	}
...
void CWeapon::SNetWeaponData::NetSerialise(TSerialize ser)
{
	...
	ser.Value("fired", m_fireCounter, 'u8'); // 0 - 255
}
...
void CWeapon::SNetWeaponReloadData::NetSerialise(TSerialize ser)
{
	...
	ser.Value("expendedAmmo", m_expended_ammo, 'u8');
}
...
static i32 calcNumShots(u8 prevCount, u8 newCount)
{
	i32 result = 0;
...
//------------------------------------------------------------------------
bool CWeapon::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
...
void CWeapon::FullSerialize( TSerialize ser )
{
	...
	if(GetOwnerId())
	{
		//if (m_fm)
     // m_fm->Serialize(ser);
		i32 numFiremodes = GetNumOfFireModes();
		ser.Value("numFiremodes", numFiremodes);
		if(ser.IsReading())
		{
			assert ( numFiremodes == GetNumOfFireModes() );
			if(numFiremodes != GetNumOfFireModes())
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Num of firemodes changed - loading will be corrupted.");
		}
		for(i32 i = 0; i < numFiremodes; ++i)
...
void CWeapon::SerializeLTL(TSerialize ser)
{
	...
	if(GetOwnerId())
	{
		...
		i32 currentFireMode = GetCurrentFireMode();
		...
		if(hasZoom)
		{
			...
			i32 zoomStep = m_zm->GetCurrentStep();
...
//------------------------------------------------------------------------
void CWeapon::Update( SEntityUpdateContext& ctx, i32 update)
{
	FUNCTION_PROFILER(GetISystem(), PROFILE_GAME);
...
//---------------------------------------------------------------------
u32 CWeapon::StartDeselection(bool fastDeselect)
{
	CActor * pOwnerActor = GetOwnerActor();
	...
	if (m_isDeselecting)
	{
		if (m_deselectAction)
		{
			...
			return (u32)(timeRemaining * 1000.0f);
		}
		else
		{
			//--- Adjust existing animation play speed if necessary
			u32 curTime = GetCurrentAnimationTime(eIGS_Owner);
			if (pOwnerActor)
			{
...
void CWeapon::AddAmmoCapacity()
{
	...
	for (size_t i = 0; i < pWeaponParams->ammoParams.capacityAmmo.size(); ++i)
	{
		...
		i32 additionalCapacity = pWeaponParams->ammoParams.capacityAmmo[i].count;
		i32 currentCapacity = pOwnerInventory->GetAmmoCapacity(pAmmoType);
...
void CWeapon::DropAmmoCapacity()
{
	...
	for (size_t i = 0; i < pWeaponParams->ammoParams.capacityAmmo.size(); ++i)
	{
		...
		i32 additionalCapacity = pWeaponParams->ammoParams.capacityAmmo[i].count;
		i32 currentCapacity = pOwnerInventory->GetAmmoCapacity(pAmmoType);
		i32 currentInventory = pOwnerInventory->GetAmmoCount(pAmmoType);
		i32 currentBonus = GetBonusAmmoCount(pAmmoType);
		i32 newCapacity = currentCapacity - additionalCapacity;
		i32 newBonus = currentBonus + max(currentInventory - newCapacity, 0);
		pOwnerInventory->SetAmmoCapacity(pAmmoType, currentCapacity - additionalCapacity);
...
//------------------------------------------------------------------------
void CWeapon::PickUp(EntityId picker, bool sound, bool select, bool keepHistory, tukk setup)
{
	SetOwnerId(picker);
	if(IsClient() && gEnv->pGame->GetIGameFramework()->GetClientActorId()==picker)
	{
		if(IEntity* pEntity = GetEntity())
		{
			...
			tukk collectibleId = pEntityClass->GetName();
...
//------------------------------------------------------------------------
void CWeapon::AddEventListener(IWeaponEventListener *pListener, tukk who)
{
...
//------------------------------------------------------------------------
void CWeapon::StartZoom(EntityId actorId, i32 zoomed)
{
	if (m_zm == NULL)
		return;
...
bool CWeapon::IsValidAssistTarget(IEntity *pEntity, IEntity *pSelf,bool includeVehicles/*=false*/)
{
	...
	if (!pAI)
	{
		...
		{
			i32 ownteam=g_pGame->GetGameRules()->GetTeam(pSelf->GetId());
			i32 targetteam=g_pGame->GetGameRules()->GetTeam(pEntity->GetId());
...
void CWeapon::Reload(bool force)
{
	...
	if (m_fm)
	{
		if (canReload || force)
		{
			i32 zoomStep = m_zm ? m_zm->GetCurrentStep() : 0;
...
bool CWeapon::OutOfAmmoTypes() const
{
	...
	for (size_t i = 0; i < m_ammo.size(); ++i)
	{
		...
		i32 inventoryCount = pInventory->GetAmmoCount(pAmmoType);
		i32 clipCount = GetAmmoCount(pAmmoType);
...
//------------------------------------------------------------------------
void CWeapon::SetBonusAmmoCount(IEntityClass* pAmmoType, i32 amount)
{
	SWeaponAmmoUtils::SetAmmo(m_bonusammo, pAmmoType, amount);
}

//------------------------------------------------------------------------
i32 CWeapon::GetBonusAmmoCount(IEntityClass* pAmmoType) const
{
	return SWeaponAmmoUtils::GetAmmoCount(m_bonusammo, pAmmoType);
}

//------------------------------------------------------------------------
i32 CWeapon::GetAmmoCount(IEntityClass* pAmmoType) const
{
	CActor* pOwnerActor = GetOwnerActor();
...
//------------------------------------------------------------------------
void CWeapon::SetAmmoCount(IEntityClass* pAmmoType, i32 count)
{
	CActor* pOwnerActor = GetOwnerActor();
	i32 currentCount = 0;
	i32 currentCapacity = 0;
	...
	if(pOwnerEntity)
	{
		m_pGameFramework->GetIGameplayRecorder()->Event(pOwnerEntity,GameplayEvent(eGE_AmmoCount,GetEntity()/*->GetClass()*/->GetName(),float(count),(uk )(EXPAND_PTR)GetFireModeIdxWithAmmo(pAmmoType)));
	}
...
bool CWeapon::CanPickUpAmmo(IInventory* pDestinationInventory)
{
	for (TAmmoVector::iterator it = m_ammo.begin(); it != m_ammo.end(); ++it)
	{
		...
		i32 actualCount = pDestinationInventory->GetAmmoCount(pAmmoClass);
		i32 maxCapacity = pDestinationInventory->GetAmmoCapacity(pAmmoClass);
...
//------------------------------------------------------------------------
i32 CWeapon::GetInventoryAmmoCount(IEntityClass* pAmmoType) const
{
	if (m_hostId)
	{
	...
	{
		tukk levelName = m_pGameFramework->GetLevelName();
...
//------------------------------------------------------------------------
void CWeapon::SetInventoryAmmoCount(IEntityClass* pAmmoType, i32 count)
{
	if(!pAmmoType)
		return;
...
//-----------------------------------------------------------------------
bool CWeapon::SetInventoryAmmoCountInternal(IInventory* pInventory, IEntityClass* pAmmoType, i32 count)
{
	...
	if (pInventory)
	{
		...
		i32k capacity = pInventory->GetAmmoCapacity(pAmmoType);
		i32k current = pInventory->GetAmmoCount(pAmmoType);
		if(count >= capacity)
		{
		...
		{
			i32k difference = min(count, capacity) - current;

			SHUDEvent eventPickUp(eHUDEvent_OnAmmoPickUp);
			eventPickUp.AddData(SHUDEventData((uk )GetIWeapon()));
			eventPickUp.AddData(SHUDEventData(difference));
			eventPickUp.AddData(SHUDEventData((uk )pAmmoType));
...
//------------------------------------------------------------------------
IFireMode* CWeapon::GetFireMode(i32 idx) const
{
	return GetCFireMode(idx);
}

//------------------------------------------------------------------------
IFireMode* CWeapon::GetFireMode(tukk name) const
{
	return GetCFireMode(name);
}

//------------------------------------------------------------------------
CFireMode* CWeapon::GetCFireMode(i32 idx) const
{
	if (idx >= 0 && idx < (i32)m_firemodes.size())
		return m_firemodes[idx];
	return 0;
}

//------------------------------------------------------------------------
CFireMode* CWeapon::GetCFireMode(tukk name) const
{
	TFireModeIdMap::const_iterator it = m_fmIds.find(DrxHashStringId::GetIdForName(name));
	if (it == m_fmIds.end())
		return 0;
...
//------------------------------------------------------------------------
i32 CWeapon::GetFireModeIdxWithAmmo(const IEntityClass* pAmmoClass) const
{
...
//------------------------------------------------------------------------
i32 CWeapon::GetFireModeIdx(tukk name) const
{
	TFireModeIdMap::const_iterator it = m_fmIds.find(DrxHashStringId::GetIdForName(name));
	if (it != m_fmIds.end())
		return it->second;
...
//------------------------------------------------------------------------
i32 CWeapon::GetCurrentFireMode() const
{
	return m_firemode;
}

//------------------------------------------------------------------------
i32 CWeapon::GetPreviousFireMode() const
{
	return m_prevFiremode;
}

//------------------------------------------------------------------------
void CWeapon::SetCurrentFireMode(i32 idx)
{
	...
	if (m_fm)
	{
		...
		if (IsServer())
		{
			if(GetOwnerId())
			{
				m_pGameplayRecorder->Event(GetOwner(), GameplayEvent(eGE_WeaponFireModeChanged, m_fm->GetName(), (float)idx, (uk )(EXPAND_PTR)GetEntityId()));
			}
...
//------------------------------------------------------------------------
void CWeapon::SetCurrentFireMode(tukk name)
{
	TFireModeIdMap::iterator it = m_fmIds.find(DrxHashStringId::GetIdForName(name));
	if (it == m_fmIds.end())
		return;
...
{
	i32k iCurrentFireMode = GetCurrentFireMode();
	i32k newId = GetNextFireMode(iCurrentFireMode);
...
//------------------------------------------------------------------------
i32 CWeapon::GetNextFireMode(i32 currMode) const
{
	if (m_firemodes.empty() || (currMode > ((i32)m_firemodes.size()-1)))
		return 0;
...
//------------------------------------------------------------------------
bool CWeapon::IsFiremodeDisabledByAccessory(i32 idx) const
{
...
//------------------------------------------------------------------------
IZoomMode *CWeapon::GetZoomMode(i32 idx) const
{
	if (idx >= 0 && idx < (i32)m_zoommodes.size())
		return m_zoommodes[idx];
	return 0;
}

//------------------------------------------------------------------------
IZoomMode *CWeapon::GetZoomMode(tukk name) const
{
	TZoomModeIdMap::const_iterator it = m_zmIds.find(DrxHashStringId::GetIdForName(name));
	if (it == m_zmIds.end())
		return 0;
...
//------------------------------------------------------------------------
tukk CWeapon::GetZoomModeName(i32 idx) const
{
...
//------------------------------------------------------------------------
i32 CWeapon::GetZoomModeIdx(tukk name) const
{
	TZoomModeIdMap::const_iterator it = m_zmIds.find(DrxHashStringId::GetIdForName(name));
	if (it != m_zmIds.end())
		return it->second;
...
//------------------------------------------------------------------------
i32 CWeapon::GetCurrentZoomMode() const
{
	return m_zmId;
}

//------------------------------------------------------------------------
void CWeapon::SetCurrentZoomMode(i32 idx)
{
...
//------------------------------------------------------------------------
void CWeapon::SetCurrentZoomMode(tukk name)
{
	TZoomModeIdMap::iterator it = m_zmIds.find(DrxHashStringId::GetIdForName(name));
	if (it == m_zmIds.end())
		return;
...
//------------------------------------------------------------------------
void CWeapon::EnableZoomMode(i32 idx, bool enable)
{
...
void CWeapon::SetCrosshairMode(EWeaponCrosshair mode)
{
	...
	if(GetOwnerId() == g_pGame->GetIGameFramework()->GetClientActorId())
	{
		...
		event.AddData(SHUDEventData((i32)mode));
		CHUDEventDispatcher::CallEvent(event);
	}
}
...
{
	i32k numFiremodes = m_firemodes.size();
	i32k numZoommodes = m_zoommodes.size();
			...
			if((pFireMode == m_fm) && (pNewParams->fireparams.clip_size != pCurrentParams->fireparams.clip_size))
			{
				...
				if (pInventory && pCurrentAmmoClass)
				{
					...
					if ((pCurrentAmmo) && (pCurrentAmmo->count > 0))
					{
						if (gEnv->bMultiplayer)
						{
							i32k clipSize = pFireMode->GetClipSize(); //still call getclipsize in case there are any modifiers applied
							i32k clipDiff = clipSize - pCurrentAmmo->count;
							...
							{
								i32k inventoryCount = pInventory->GetAmmoCount(pCurrentAmmoClass);
						...
						else
						{
							...
							i32 currentOnClip = pCurrentAmmo->count;
							i32 currentOnInventory = pInventory->GetAmmoCount(pCurrentAmmoClass);
							i32 currentInventoryCapacity = pInventory->GetAmmoCapacity(pCurrentAmmoClass);
							i32 addToClip = clipSize - currentOnClip;
							i32 capacityDiference = pCurrentParams->fireparams.clip_size - pNewParams->fireparams.clip_size;
...
{
	i32k numAccessoryParams = m_sharedparams->accessoryparams.size();
	for(i32 i = 0; i < numAccessoryParams; i++)
	{
		...
		if(HasAccessory(pAccessory))
		{
			...
			for(i32 j = i+1; j < numAccessoryParams; j++)
			{
				...
				if(HasAccessory(pSecondAccessory))
				{
					...
					for(i32 k = j+1; k < numAccessoryParams; k++)
					{
						...
						if(HasAccessory(pThirdAccessory))
						{
							...
							for(i32 l = k+1; l < numAccessoryParams; l++)
							{
...
const SFireModeParams* CWeapon::GetAccessoryAlteredFireModeParams(CFireMode* pFireMode, IEntityClass** pAccessories)
{
	...
	for(i32 i = 0; i < ITEM_MAX_NUM_ACCESSORIES; ++i)
	{
		for(i32 j = i+1; j < ITEM_MAX_NUM_ACCESSORIES; ++j)
		{
			if(pAccessories[j])
			{
...
void CWeapon::FixAccessories(const SAccessoryParams *params, bool attach)
{
	if(params)
	{
		...
		else //!attach
		{
			...
			if (GetZoomModeIdx(params->zoommode.c_str()) != -1)
			{
				i32 zoomModeId = GetZoomModeIdx(params->zoommode.c_str());
			...
			i32 secondaryZoommode = GetZoomModeIdx(params->zoommodeSecondary.c_str());
...
bool CWeapon::PredictProjectileHit(IPhysicalEntity *pShooter,
	...
	u32* trajectorySizeInOut, float timeStep, Vec3* pTrajectoryVelocities,
	const bool predictionForAI) const
{
	...
	u32 n = 0;
	u32k maxSize = trajectorySizeInOut ? *trajectorySizeInOut : 0;
...
bool CWeapon::HasAttachmentAtHelper(tukk helper)
{
	...
	if(pPlayer)
	{
		...
		if(pInventory)
		{
			...
			for (i32 i = 0; i < pInventory->GetAccessoryCount(); i++)
			{
...
//--------------------------------------------------------------------------
void CWeapon::GetAttachmentsAtHelper(tukk helper, CDrxFixedStringListT<5, 30> &attachments)
{
	CPlayer *pPlayer = static_cast<CPlayer *>(m_pGameFramework->GetClientActor());
	if(pPlayer)
	{
...
void CWeapon::StartChangeFireMode()
{
	...
	{
		u32k endChangeTime = u32(GetCurrentAnimationTime(eIGS_Owner) * pNewParams->fireparams.changeFMFireDelayFraction);
...
//----------------------------------------------------------
void CWeapon::GetMemoryUsage(IDrxSizer * s) const
{
	s->AddObject(this, sizeof(*this));
	GetInternalMemoryUsage(s);
}

void CWeapon::GetInternalMemoryUsage(IDrxSizer * s) const	
{
	{
...
//--------------------------------------------------------
bool CWeapon::Query(EWeaponQuery query, ukk param /*=NULL*/)
{
	switch(query)
	{
...
//--------------------------------------------------------
void CWeapon::ApplyFPViewRecoil(i32 nFrameId, Ang3 recoilAngles)
{
...
const SPlayerMovementModifiers& CWeapon::GetPlayerMovementModifiers() const
{
	...
	{
		assert((m_zmId >= 0) && (m_zmId < (i32)m_weaponsharedparams->zoomedMovementModifiers.size()));
			...
			for (i32 i = 0; i < numItems; ++ i)
			{
				...
				if (pItem)
				{
					...
					if (pWeapon && !pWeapon->OutOfAmmo(false))
					{
						...
						tukk const pWeaponCategory = m_pItemSystem->GetItemCategory(pWeapon->GetEntity()->GetClass()->GetName());
...
void CWeapon::PickUpAmmo( EntityId pickerId )
{
	...
	i32k bonusAmmoTypeCount = m_bonusammo.size();
	i32 bonusAmmoTypeEmptied = 0;
	i32 iTotalRemainingAmmo = 0;
	...
	for (TAmmoVector::iterator bonusAmmoIt = m_bonusammo.begin(); bonusAmmoIt != bonusAmmoEndIt; ++bonusAmmoIt)
	{
		...
		i32k bonusAmmoCount = bonusAmmoIt->count;
		i32k currentAmmoCount = pInventory->GetAmmoCount(pAmmoClass);
		i32k maxAmmoCapacity = pInventory->GetAmmoCapacity(pAmmoClass);

		if(bonusAmmoCount > 0)
		{
			if ((bonusAmmoCount + currentAmmoCount) >= maxAmmoCapacity)
			{
				i32k remainingBonusAmmo = bonusAmmoCount - (maxAmmoCapacity - currentAmmoCount);
				bonusAmmoIt->count = remainingBonusAmmo;
				i32 pickedRemainingAmmo = (maxAmmoCapacity - currentAmmoCount);
	...
	if (pickerHasThisWeapon)
	{
		...
		for (TAmmoVector::iterator ammoCit = m_ammo.begin(); ammoCit != ammoEndCit; ++ammoCit)
		{
			...
			i32k magazineAmmoCount = ammoCit->count;
		
			if(magazineAmmoCount > 0)
			{	
				i32 collectedIndex = -1;
				i32 count = 0;
				...
				i32k currentAmmoCount = collectedIndex >= 0 ? collectedAmmo[collectedIndex].count : pInventory->GetAmmoCount(pAmmoClass);
				i32k maxAmmoCapacity = pInventory->GetAmmoCapacity(pAmmoClass);

				if ((magazineAmmoCount + currentAmmoCount) >= maxAmmoCapacity)
				{
					i32k remainingMagazineAmmo = magazineAmmoCount - (maxAmmoCapacity - currentAmmoCount);
...
bool CWeapon::CheckAmmoRestrictionsForBonusAndMagazineAmmo( IInventory& inventory ) const
{
	if (!m_bonusammo.empty())
	{
		...
		for (TAmmoVector::const_iterator bonusAmmoCit = m_bonusammo.begin(); bonusAmmoCit != bonusAmmoEndCit; ++bonusAmmoCit)
		{
			...
			i32k invAmmo  = inventory.GetAmmoCount(pAmmoClass);
			i32k invLimit = inventory.GetAmmoCapacity(pAmmoClass);
...
bool CWeapon::CanRefillAmmoType(IEntityClass* pAmmoType, tukk refillType) const
{
	if (refillType != 0)
	{
		const SAmmoParams* pParams = g_pGame->GetWeaponSystem()->GetAmmoParams(pAmmoType);
		tukk ammoCategory = pParams->ammo_category.c_str();
...
bool CWeapon::RefillAllAmmo(tukk refillType, bool refillAll)
{
	...
	i32k numAccessoryParams = allAccessories.size();
	for (i32 i = 0; i < numAccessoryParams; ++i)
	{
...
bool CWeapon::RefillInventoryAmmo(IInventory* pInventory, IEntityClass* pAmmoTypeClass, CFireMode* pFireMode)
{
	...
	i32k ammoTypeCount = pInventory->GetAmmoCount(pAmmoTypeClass);
	i32k ammoTypeCapacity = pInventory->GetAmmoCapacity(pAmmoTypeClass);
	i32k currentClipCapacity = pFireMode ? pFireMode->GetClipSize() + pFireMode->GetChamberSize() : 0;
	i32k currentClipCount = GetAmmoCount(pAmmoTypeClass);
	...
	if (currentClipCount < currentClipCapacity)
	{
		...
		i32k difference = currentClipCapacity - currentClipCount;
		SHUDEvent eventPickUp(eHUDEvent_OnAmmoPickUp);
		eventPickUp.AddData(SHUDEventData((uk )GetIWeapon()));
		eventPickUp.AddData(SHUDEventData(difference));
		eventPickUp.AddData(SHUDEventData((uk )pAmmoTypeClass));
		CHUDEventDispatcher::CallEvent(eventPickUp);
	}
...
//			ISkeletonPose* pSkelPose = pCharacter->GetISkeletonPose();
			...
			{
				DrxFixedStringT<32>	jointName;
				i32k bulletNum = max(ammoCount, 1);

				jointName.Format("%s%d", beltParams.jointName.c_str(), bulletNum);

				i16k jointId = rIDefaultSkeleton.GetJointIDByName(jointName.c_str());

				DRX_ASSERT_TRACE(jointId >= 0, ("Invalid joint name '%s' in bullet belt params of %s belonging to %s", jointName.c_str(), GetEntity()->GetEntityTextDescription(), GetOwnerActor() ? GetOwnerActor()->GetEntity()->GetEntityTextDescription() : "nobody"));
...
void CWeapon::SetFragmentTags(CTagState& fragTags)
{
	if(m_fm)
	{
		i32 ammoCount = m_fm->GetAmmoCount();
		SetAmmoCountFragmentTags(fragTags, ammoCount);
	}
}

//------------------------------------------------------------------------
void CWeapon::SetAmmoCountFragmentTags(CTagState& fragTags, i32 ammoCount)
{	
	if(ammoCount > 1)
	{
...
void CWeapon::PlayChangeFireModeTransition(CFireMode* pNewFiremode)
{
	if(m_fm && pNewFiremode != m_fm)
	{
		...
		i32 fragmentID = GetFragmentID("change_firemode", &pTagDefinition);
		if(fragmentID != FRAGMENT_ID_INVALID)
		{
			...
			if (pTagDefinition && pNewFiremode)
			{
				...
				i32k sz = 64;
				DrxFixedStringT<sz> sourceName = static_cast<CFireMode*>(m_fm)->GetShared()->fireparams.tag.c_str();
				sourceName = DrxFixedStringT<sz>("from_") + sourceName;

				DrxFixedStringT<sz> targetName = pNewFiremode->GetShared()->fireparams.tag.c_str();
				targetName = DrxFixedStringT<sz>("to_") + targetName;
