...
namespace
{
	i32 GetJointIdFromPartId(IEntity& entity, i32k partId, Vec3* pReturnPos = NULL)
	{
		ICharacterInstance* pCharInst = entity.GetCharacter(0);
		i32 jointId = GetSlotIdx(partId, pCharInst && pCharInst->GetObjectType()==CGA);
		DRX_ASSERT(pCharInst);
		if(pCharInst)
		{
			if(IAttachment* pAttachment = pCharInst->GetIAttachmentUpr()->GetInterfaceByPhysId(jointId))
			{
				if(pReturnPos)
				{
	...
	, m_bStickToFriendlies(false)
{
	...
	if((rHitInfo.partId != u16(-1)) && pTargetEntity && pTargetEntity->GetCharacter(0))
	{
...
void CStickyProjectile::Stick(const SStickParams& stickParams)
{
	...
	if(ISurfaceType *pSurfaceType = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceType(stickParams.m_targetMatId))
	{
	...
	i32k peType = stickParams.m_pTarget->GetType();
	if( peType == PE_PARTICLE )
		return;

	i32k iForiegnData = stickParams.m_pTarget->GetiForeignData();
...
bool CStickyProjectile::StickToEntity( const SStickParams& stickParams, IEntity* pTargetEntity )
{
	...
	if( pCharInstance)	
	{
		...
		if (!pActor || (stickParams.m_bStickToFriendlies || !pActor->IsFriendlyEntity(stickParams.m_ownerId)) && (gEnv->bMultiplayer || !pActor->IsDead()))
		{
			...
			tukk boneName = rIDefaultSkeleton.GetJointNameByID(m_stuckJoint);
...
bool CStickyProjectile::AttachToCharacter(CProjectile* pProjectile, IEntity& pEntity, ICharacterInstance& pCharacter, tukk boneName)
{
	IEntity* pProjectileEntity = pProjectile->GetEntity();

	char attachName[16];
	drx_sprintf(attachName, "StickyProj_%d", s_attachNameID++);

	IAttachment* pCharacterAttachment = pCharacter.GetIAttachmentUpr()->CreateAttachment(attachName, CA_BONE, boneName, false); 

	if(!pCharacterAttachment)
	{
		DrxLogAlways("Could not create attachment for StickyProjectile[%s]. AttachmentName[%s] BoneName[%s]", pProjectileEntity->GetName(), attachName, boneName );
		DRX_ASSERT_MESSAGE(pCharacterAttachment, "Could not create attachment for StickyProjectile. This must be fixed.");
		return false;
	}
...
void CStickyProjectile::UnStick()
{
	...
	if(m_characterAttachmentCrC)
	{
		if(IEntity* pParentEntity = gEnv->pEntitySystem->GetEntity(m_parentId))
		{
			...
			{
				IAttachmentUpr* pAttachUpr = pCharacterInstance->GetIAttachmentUpr();
				Reset();
				pAttachUpr->RemoveAttachmentByNameCRC(m_characterAttachmentCrC);
				m_characterAttachmentCrC = 0;
			}
		}
	}
...
void CStickyProjectile::NetSetStuck(CProjectile* pProjectile, bool stuck)
{
	if(stuck && ((m_flags&eSF_IsStuck)==0))
	{
		...
		if(pTargetEntity)
		{
			...
			{
				tukk boneName = pTargetCharacter->GetIDefaultSkeleton().GetJointNameByID(m_stuckJoint);
