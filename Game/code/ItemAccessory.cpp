...
-------------------------------------------------------------------------
История:
- 8:9:2005   12:52 : Created by M�rcio Martins
...
CItem *CItem::AddAccessory(IEntityClass* pClass)
{
	...
	char namebuf[128];
	drx_sprintf(namebuf, "%s_%s", GetEntity()->GetName(), pClass->GetName());
...
void CItem::DetachAllAccessories()
{
	...
	for (u32 i = 0; i < m_accessories.size(); ++i)
	{
		...
		bool found = false;
		for (u32 j = 0; j < m_initialSetup.size(); ++j)
		{
...
void CItem::AttachAccessory(IEntityClass* pAccessoryClass, bool attach, bool noanim, bool force, bool firstTimeAttached, bool initialLoadoutSetup)
{
	...
	if (attach && pAccessory == 0)
	{
		...
		if (pAccessory = AddAccessory(pAccessoryClass))
		{
			...
			{
				i32k numAccessories = m_sharedparams->accessoryparams.size();
			...
			if(gEnv->bMultiplayer && pInventory) 	  	 
			{
				...
				i32k numAccessories = m_sharedparams->accessoryparams.size();
				u32 bit = 1;
				for(i32 i = 0; i < numAccessories; ++i, bit <<= 1)
				{
					if(m_sharedparams->accessoryparams[i].pAccessoryClass == pAccessoryClass)
					{
						DRX_ASSERT_MESSAGE(i < 16, "CItem::AttachAccessory - attachment history only supports 16 attachments. Need to make m_attachedAccessoryHistory larger.");
	...
	{
		g_pGame->GetIGameFramework()->GetIGameplayRecorder()->Event(GetOwner(), GameplayEvent(eGE_AttachedAccessory, pAccessoryClass->GetName(), (float)attach, (uk )(EXPAND_PTR)GetEntityId()));
...
//------------------------------------------------------------------------
void CItem::ShowAttachmentHelper(i32 slot, tukk name, bool show)
{
	const size_t nChars = 32;
	i32k numSubHelpers = 3;
	DrxFixedStringT<nChars> attachmentName[nChars];
	...
	if (pCharacter)
	{
		for (i32 i = 0; i < numSubHelpers; ++i)
		{
			IAttachmentUpr* pAttachmentUpr = pCharacter->GetIAttachmentUpr();
			IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByName(attachmentName[i].c_str());
			if (pAttachment)
...
{
	i32k numParams = m_sharedparams->accessoryparams.size();
...
{
	i32k numAttachments = m_accessories.size();
...
void CItem::SwitchAccessory(const ItemString& accessory)
{
	...
	{
		char errorMsg[256];
		drx_sprintf(errorMsg, "CItem::SwitchAccessory failed to find network safe class id for %s", accessory.c_str());
...
void CItem::DetachAccessory(const ItemString& accessory)
{
	...
	{
		char errorMsg[256];
		drx_sprintf(errorMsg, "CItem::DetachAccessory failed to find network safe class id for %s", accessory.c_str());
		DRX_ASSERT_MESSAGE(result, errorMsg);
	}
#endif
...
void CItem::PatchInitialSetup()
{
	...
	if(!m_properties.initialSetup.empty())
	{
		...
		while ((string::npos != pos || string::npos != lastPos) && (m_initialSetup.size() < m_initialSetup.max_size()))
		{
			...
			tukk name = m_properties.initialSetup.substr(lastPos, pos - lastPos).c_str();
