...
struct SPhysicsRayWrapper
{
		...
		: m_pRay(NULL)
	{
		...
		m_pRay = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::ray::type, &rayData);
	}
...
#if DRX3D_PENETRATION_BACKSIDE_FX_ENABLED_SP
...
//------------------------------------------------------------------------
i32 CBullet::GetRopeBoneId(const EventPhysCollision& collision, IEntity& target, IPhysicalEntity* pRopePhysicalEntity) const
{
	i32 boneId = -1;
...
{
	i32 numHitActors = m_hitActors.size();
...
//------------------------------------------------------------------------
void CBullet::ProcessHit(CGameRules& gameRules, const EventPhysCollision& collision, IEntity& target, float damage, i32 hitMatId, const Vec3& hitDir)
{
	if(damage > 0.f)
	{
		...
		if(!alreadyHit)
		{
			...
			hitInfo.knocksDownLeg = m_chanceToKnockDownLeg>0 && damage>m_minDamageForKnockDownLeg && m_chanceToKnockDownLeg>drx_random(0, 99);
...
void CBullet::HandleEvent(const SGameObjectEvent &event)
{
	...
	if (event.event == eGFE_OnCollision)
	{
		...
		i32k hitMatId = pCollision->idmat[1];
		...
		if (bProcessCollisionEvent)
		{
			...
			if (gEnv->pAISystem)
			{
				...
				{
					ISurfaceType *pSurfaceType = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceType(hitMatId);
					...
					SAIStimulus stim(AISTIM_DRX3D_HIT, 0, m_ownerId, pTarget ? pTarget->GetId() : 0, pCollision->pt, pCollision->vloc[0].GetNormalizedSafe(ZERO), radius);
					gEnv->pAISystem->RegisterStimulus(stim);

					SAIStimulus stimSound(AISTIM_SOUND, AISTIM_DRX3D_HIT, m_ownerId, 0, pCollision->pt, ZERO, soundRadius);
...
void CBullet::EmitUnderwaterTracer(const Vec3& pos, const Vec3& destination)
{
	...
	g_pGame->GetWeaponSystem()->GetTracerUpr().EmitTracer(params);
}
...
void CBullet::HandlePierceableSurface( const EventPhysCollision* pCollision, IEntity* pHitTarget, const Vec3& hitDirection, bool bProcessedCollisionEvent )
{
	...
	if (backFace == false)
	{
		...
		const bool collisionStopsBullet = (!bProcessedCollisionEvent) ||
			...
			((i16)pierceabilityMat <= GetBulletPierceability()) ||
...
//////////////////////////////////////////////////////////////////////////
void CBullet::SetUpParticleParams(IEntity* pOwnerEntity, u8 pierceabilityModifier)
{
...
bool CBullet::RayTraceGeometry( const EventPhysCollision* pCollision, const Vec3& pos, const Vec3& hitDirection, SBackHitInfo* pBackHitInfo )
{
	...
	if (pCollider->GetParams(&partParams) && pCollider->GetStatus(&posStatus))
	{
		if (partParams.pPhysGeom && partParams.pPhysGeom->pGeom)
		{
			...
			{ WriteLockCond lock;
			i32k contactCount = partParams.pPhysGeom->pGeom->IntersectLocked(pRayGeometry,&geomWorldData,0,&intersectionParams,pContacts,lock);
			if (contactCount > 0)
			{
				float bestDistance = 10.0f;

				for (i32 i = (contactCount-1); (i >= 0) && (pContacts[i].t < bestDistance) && ((pContacts[i].n*hitDirectionNormalized) < 0); i--)
				{
...
{
	if (m_nextHit == MAX_DEBUG_DRX3D_HITS)
	{
		m_nextHit = 0;
	}
	...
	m_hitsList[m_nextHit].lifeTime = (g_pGameCVars->g_bulletPenetrationDebugTimeout > 0.0f) ? g_pGameCVars->g_bulletPenetrationDebugTimeout : DEFAULT_DEBUG_DRX3D_HIT_LIFETIME;
...
void SDebugBulletPenetration::Update(float frameTime)
{
	...
	const float baseDebugTimeOut = (g_pGameCVars->g_bulletPenetrationDebugTimeout > 0.0f) ? g_pGameCVars->g_bulletPenetrationDebugTimeout : DEFAULT_DEBUG_DRX3D_HIT_LIFETIME;

	for (i32 i = 0; i < MAX_DEBUG_DRX3D_HITS; ++i)
	{
		...
		const ColorB red(255, 0, 0, (u8)(192 * alpha)), green(0, 255, 0, (u8)(192 * alpha));
...
//////////////////////////////////////////////////////////////////////////
tukk SDebugBulletPenetration::GetPenetrationLevelByPierceability( int8 surfacePierceability ) const
{
