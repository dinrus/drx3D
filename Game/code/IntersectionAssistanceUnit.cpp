...
static u8k kMaxNumOOBBIntersectionTestsPerFrame	= 2; // Timeslicing
...
void CIntersectionAssistanceUnit::Init( EntityId subjectEntityId, eResolutionPolicy resolutionPolicy )
{
	...
	i32k num = s_embedPreventRotations.size();
...
bool CIntersectionAssistanceUnit::TestForIntersectionAtLocation(const eTestMethod testMethod, const Matrix34& wMat, EntityId testEntityId, EntityId ignoreEnt, QuatT& outAdjustedResult, const bool bCentreOnFocalEnt /* = false */, bool bRenderOnFail /* = true */, i32k index /* = -1*/) 
{
...
void CIntersectionAssistanceUnit::UpdateEmbeddingPrevention()
{
	...
	if(pFocalEnt)
	{
		if(!s_embedPreventRotations.empty())
		{
			...
			i32 count = 0; 
			u8k numTestRotations = s_embedPreventRotations.size(); 
			while(true)
			{
				DRX_ASSERT(m_currentTestIndex < m_lastKnownGoodPositions.size());
...
bool CIntersectionAssistanceUnit::TestForIntersection(const eTestMethod testMethod, const QuatT& qWOrient, const Quat& qRotOffset, const bool bCentreOnFocalEnt, i32 index )
{
	QuatT temp = qWOrient; 
	temp.q *= qRotOffset;
...
bool CIntersectionAssistanceUnit::GetHighestScoringLastKnownGoodPosition( const QuatT& baseOrientation, QuatT& outQuat ) const
{
	...
	if(!m_lastKnownGoodPositions.empty())
	{
		...
		for(u8 i = 0; i < m_lastKnownGoodPositions.size(); ++i)
		{
			...
			if(IsPositionWithinAcceptedLimits(qLastKnownGood.t, baseOrientation.t, kDistanceTolerance))
			{
				...
				if(g_pGameCVars->pl_pickAndThrow.intersectionAssistDebugEnabled == 2)
				{
					DrxWatch("[INDEX(%d)] : D[%.3f] O[%.3f] T[%.3f] (%s)", i, distanceScore, orientationScore, fCandidateScore, flippedOrientationScore > regularOrientationScore ? "*F*" : "R");
				}
#endif //#ifndef _RELEASE
		...
		if(bestIndex >= 0)
		{
			...
			if(g_pGameCVars->pl_pickAndThrow.intersectionAssistDebugEnabled == 2)
			{
				...
				DrxWatch("[BEST INDEX] : %d", bestIndex);
			}
#endif // ifndef _RELEASE
...
void CIntersectionAssistanceUnit::SetEmbedPreventionResult( u8 index, const QuatT& result )
{
	DRX_ASSERT(index < m_lastKnownGoodPositions.size()); 
...
{
	for(u8 i = 0; i < m_testContainer.size(); ++i)
	{
...
void CIntersectionAssistanceUnit::CPrimitiveIntersectionTester::DoCheck( u8 index, const primitives::box& testBox, QuatT& resultIfSuccess, IPhysicalEntity* pIgnoreEntity /*= NULL*/ )
{
...
void CIntersectionAssistanceUnit::CPrimitiveIntersectionTester::SetNumTestOffsets( u8 numTestOffsets )
{
