...
CRecordingBuffer::CRecordingBuffer(size_t size, u8 *buffer)
{
	m_pDiscardCallback = NULL;
	m_pDiscardCallbackUserData = NULL;
...
void CRecordingBuffer::Init(size_t size, u8 *buffer)
{
	DRX_ASSERT_MESSAGE(m_pBuffer==NULL, "CRecordingBuffer initialised twice");
...
{
	DRX_ASSERT_MESSAGE(m_usedSize > 0, "Unable to remove frame, no packets found");
...
{
	DRX_ASSERT_MESSAGE(m_usedSize > 0, "Unable to remove packet, no packets found");
	SRecording_Packet* pPacket = (SRecording_Packet*)m_pStart;
	DRX_ASSERT_MESSAGE(pPacket->size <= m_usedSize, "Unable to remove more than we are using");
	...
	{
		DRX_ASSERT_MESSAGE(m_pStart == m_pBuffer + m_dynamicBufferSize, "The packet size has overrun the size of the dynamic buffer");
		m_pStart = m_pBuffer;
		m_dynamicBufferSize = m_actualBufferSize;
	}
...
{
	DRX_ASSERT_MESSAGE(size <= m_actualBufferSize, "Unable to clear enough space");
	while (true)
	{
		u8* pEnd = GetEnd();
	...
	u8		inType)
{
	...
	DRX_ASSERT_MESSAGE((inSize % 4) == 0, "The packet size must be 4 byte aligned.");
	DRX_ASSERT_MESSAGE(inSize >= sizeof(SRecording_Packet), "The size of the packet must not be large enough to hold a SRecording_Packet header.");
	DRX_ASSERT_MESSAGE(inType != eRBPT_Invalid, "The type of the packet must not be eRBT_Invalid. Has it been initialised properly?");
	...
	DRX_ASSERT_MESSAGE(m_usedSize <= m_dynamicBufferSize, "Can't use more memory than we have in the buffer, something has gone wrong here");
...
size_t CRecordingBuffer::GetData(u8 *pBuffer, size_t bufferSize) const
{
	DRX_ASSERT_MESSAGE(bufferSize >= m_usedSize, "The buffer is not large enough to contain all the data");
