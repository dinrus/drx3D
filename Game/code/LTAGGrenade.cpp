...
DRX_IMPLEMENT_GTI(CLTAGGrenade, CProjectile);
		...
		if (m_netState==k_netStateReading)
		{
			if (m_lastPosSet!=m_netDesiredPos)
			{
				...
				if (m_wasLaunchedByLocalPlayer)
				{
					...
					for (i32 i=0; i<k_maxNetPoints-1; i++)
					{
						Vec3 p0 = m_lastPos[i];
...
void CLTAGGrenade::Launch(const Vec3 &pos, const Vec3 &dir, const Vec3 &velocity, float speedScale)
{
	...
	if (gEnv->bMultiplayer)
	{
		if (IPhysicalEntity* pent=GetEntity()->GetPhysics())
		{
			...
			m_netState = k_netStateInvalid;
			for (i32 i=0; i<k_maxNetPoints; i++)
			{
...
//------------------------------------------------------------------------
void CLTAGGrenade::Update(SEntityUpdateContext &ctx, i32 updateSlot)
{
...
//			IEntity* pOwner = gEnv->pEntitySystem->GetEntity(m_ownerId);
				...
				i32 clientActorTeam = g_pGame->GetGameRules()->GetTeam(clientActorId);
				i32 ownerTeam = g_pGame->GetGameRules()->GetTeam(m_ownerId);
#endif
				...
				{
					DrxLog("LTAGGrenade::StateEnter() entering EStickyState_UNSAFE - grenade owner is friendlyEntity to current player clientActor - setting safe armed material");
				...
				{
					DrxLog("LTAGGrenade::StateEnter() entering EStickyState_UNSAFE - grenade owner is NOT friendlyEntity to current player clientActor - setting unsafe armed material");
					ChangeTexture(grenadeParams.m_armedMaterial);
					ChangeTrail(grenadeParams.m_armedTrail);
				}
			}
...
{
	//DrxLog("CLTAGGrenade::ChangeState() from %d to %d\n", m_state, state);
	StateExit(m_state);
	m_state = state;
	StateEnter(m_state);
...
void CLTAGGrenade::OnCollision(const SGameObjectEvent &event)
{
	...
	u32 pierceabilityProj = GetAmmoParams().pParticleParams ? GetAmmoParams().pParticleParams->iPierceability : 13;
...
i32 CLTAGGrenade::PlaySound(const string& soundName)
{
	i32 id = 0; //GetAudioProxy()->PlaySound(
...
//------------------------------------------------------------------------
void CLTAGGrenade::StopSound(i32 id)
{
...
void CLTAGGrenade::ChangeTexture(const string& textureName)
{
	...
	IEntity* pEntity = GetEntity();
	IMaterial* pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(textureName.c_str());
	if (pMaterial)
...
//------------------------------------------------------------------------
bool CLTAGGrenade::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 pflags)
{
