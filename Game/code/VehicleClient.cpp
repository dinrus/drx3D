...
void CVehicleClient::InsertActionMapValue(tukk  pString, i32 eVehicleAction)
{
	m_actionNameIds.insert(TActionNameIdMap::value_type(CDrxName(pString), eVehicleAction));
}
...
//------------------------------------------------------------------------
void CVehicleClient::OnAction(IVehicle* pVehicle, EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
	assert(pVehicle);
 	if (!pVehicle)
		return;
...
void CVehicleClient::EnableActionMaps(IVehicleSeat* pSeat, bool enable)
{
	...
	if (!IsDemoPlayback() && pSeat)
	{
		...
		tukk actionMap = pSeat->GetActionMap();
