...
void CJaw::Update(SEntityUpdateContext& ctx, i32 slot)
{
...
//////////////////////////////////////////////////////////////////////////
bool CJaw::SetAspectProfile(EEntityAspects aspect, u8 profile)
{
	...
	i32 slot = (m_auxSlotUsed||ok)?eIGS_ThirdPersonAux:eIGS_ThirdPerson;
...
bool CJaw::GiveExtraTubeToInventory(IActor* pPickerActor, IItemSystem* pItemSystem) const
{
	...
	i32 jawUniqueId = pItemSystem->GetItemUniqueId(pJawClass->GetName());
	i32 currentNumJaws = pPickerInventory ? pPickerInventory->GetCountOfUniqueId(jawUniqueId) : 0;
...
//////////////////////////////////////////////////////////////////////////
void CJaw::PickUp(EntityId pickerId, bool sound, bool select, bool keepHistory, tukk setup)
{
	...
	if (giveExtraTube)
	{
		i32 numJaws = m_weaponsharedparams->ammoParams.extraItems;
		while(numJaws-- > 0)
		{
...
bool CJaw::CanPickUp(EntityId userId) const
{
	...
	u8 uniqueId = m_pItemSystem->GetItemUniqueId(GetEntity()->GetClass()->GetName());

	i32 maxNumRockets = GetWeaponSharedParams()->ammoParams.capacityAmmo[0].count;
...
//////////////////////////////////////////////////////////////////////////
void CJaw::GetAttachmentsAtHelper(tukk helper, CDrxFixedStringListT<5, 30> &attachments)
{
	//Do nothing...
...
//////////////////////////////////////////////////////////////////////////
void CJaw::OnAction(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
	CGameInputActionHandlers::TJawActionHandler& jawActionHandler = g_pGame->GetGameInputActionHandlers().GetCJawActionHandler();
...
bool CJaw::OutOfAmmo(bool allFireModes) const
{
	...
	IItemSystem* pItemSystem = gEnv->pGame->GetIGameFramework()->GetIItemSystem();
	i32 jawUniqueId = pItemSystem->GetItemUniqueId(pJawClass->GetName());
	i32 currentNumJaws = pOwnerInventory->GetCountOfUniqueId(jawUniqueId);
...
void CJaw::SvActivateMissileCountermeasures(EntityId shooterId, const Vec3 &pos, const Vec3 &dir)
{
	if(gEnv->bServer)
	{
		...
		i32k kShooterTeam		= g_pGame->GetGameRules()->GetTeam(shooterId);
...
bool CJaw::OnActionJawZoom(EntityId actorId, const ActionId& actionId, i32 activationMode, float value)
{
...
void CJaw::AddAmmoCapacity()
{
	...
	for (size_t i = 0; i < pWeaponParams->ammoParams.capacityAmmo.size(); ++i)
	{
		...
		i32 capacity = pWeaponParams->ammoParams.capacityAmmo[i].count;
...
void CJaw::DropAmmoCapacity()
{
	...
	{
		IEntityClass* pAmmoType = pWeaponParams->ammoParams.capacityAmmo[i].pAmmoClass;
		i32 currentAmmo = pOwnerInventory->GetAmmoCount(pAmmoType);
		if (currentAmmo == 0)
...
void CJaw::NetShootEx( const Vec3 &pos, const Vec3 &dir, const Vec3 &vel, const Vec3 &hit, float extra, i32 predictionHandle, i32 fireModeId )
{
...
void CJaw::NetSetCurrentAmmoCount( i32 count )
{
	...
	if(!gEnv->bServer && m_firedRockets)
	{
		...
		{
			i32k currentCount = GetAmmoCount(m_fm->GetAmmoType());
...
void CJaw::AnimationEvent( ICharacterInstance *pCharacter, const AnimEventInstance &event )
{
	if(event.m_EventNameLowercaseCRC32 == m_dropJAWAnimEvent)
	{
		...
		{
			u8k uniqueId = m_pItemSystem->GetItemUniqueId(GetEntity()->GetClass()->GetName());
