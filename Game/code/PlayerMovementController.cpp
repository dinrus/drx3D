...
#include <drx3D/AI/IAIActor.h> // to get to the movement ability & path follower
...
#else
#define PMC_CHECKQNAN_FLT(x) (uk )0
#endif
...
{
	DRX_ASSERT(fabs_tpl(vForwardDir.z) <= 0.0001f);
	DRX_ASSERT(vForwardDir.IsUnit());
...
void CPlayerMovementController::Update( float frameTime, SActorFrameMovementParams& params )
{
	...
	m_animTargetSpeedCounter -= static_cast<i32>(m_animTargetSpeedCounter != 0);
	...
	if (!isClient && hasControl)
	{
		...
		else
		{
			...
			DRX_ASSERT(!m_pPlayer->IsPlayer() || (fabsf(maxAimYawAngle - gf_PI*0.5f) < FLT_EPSILON));
			DRX_ASSERT(!m_pPlayer->IsPlayer() || (fabsf(maxAimPitchAngle - gf_PI*0.5f) < FLT_EPSILON));
	...
	{
			DRX_ASSERT(!(m_state.HasDeltaMovement() && m_state.HasMoveTarget()));
	...
	if (m_pPlayer->IsAIControlled())
	{
		...
		i32k cornerSmoother = disableSmoother ? 0 : std::min((i32)g_pGame->GetCVars()->ctrlr_corner_smoother, (i32)m_pPlayer->GetActorParams().cornerSmoother);
...
void CPlayerMovementController::GetMemoryUsage( IDrxSizer *pSizer ) const
{
