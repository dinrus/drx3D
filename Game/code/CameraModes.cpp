...
$Id$
$DateTime$
Описание: Camera mode implementations
...
#include <drx3D/Game/CameraModes.h>
...
bool CDefaultCameraMode::UpdateView(const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime)
{
	...
	{
		DrxWatch("DefaultCamera Pos (%.2f, %.2f, %.2f)", viewParams.position.x, viewParams.position.y, viewParams.position.z);
		Vec3 fwd = viewParams.rotation.GetColumn1();
		DrxWatch("DefaultCamera Rot (%.2f, %.2f, %.2f, %.2f) (%f, %f, %f)", viewParams.rotation.v.x, viewParams.rotation.v.y, viewParams.rotation.v.z, viewParams.rotation.w, fwd.x, fwd.y, fwd.z);
	}
#endif //_RELEASE
	...
	if (!isThirdPerson)
	{
		...
		{
			Vec3 fwd = viewParams.rotation.GetColumn1();
			DrxWatch("DefaultCamera(PostItem) Rot (%.2f, %.2f, %.2f, %.2f) (%f, %f, %f)", viewParams.rotation.v.x, viewParams.rotation.v.y, viewParams.rotation.v.z, viewParams.rotation.w, fwd.x, fwd.y, fwd.z);
		}
#endif //_RELEASE
	}
...
CCameraPose CDefaultCameraMode::FirstPersonCameraPose(const CPlayer& clientPlayer, IItem* pCurrentItem, float frameTime)
{
	...
	u32 entityFlags = (pEntity) ? pEntity->GetSlotFlags(eIGS_FirstPerson) : 0;
...
CCameraPose CDefaultCameraMode::ThirdPersonCameraPose(const CPlayer& clientPlayer, const SViewParams& viewParams)
{
	...
	if (g_pGameCVars->cl_tpvCameraCollision)
	{
		...
		i32k nSkipEnts = clientPlayer.GetPhysicalSkipEntities(pSkipEnts, DRX_ARRAY_COUNT(pSkipEnts));
...
bool CSpectatorFollowCameraMode::UpdateView( const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime )
{
	...
	if (pTargetActor)
	{
		...
		{
			if(CVTOLVehicleUpr* pVTOLUpr = pGameRules->GetVTOLVehicleUpr())
			{
				if(pVTOLUpr->IsVTOL(pTargetVehicle->GetEntityId()))
				{
					pTargetVehicle = NULL;
				}
			}
	...
	{
		IPhysicalEntity* pSkipEntities[10];
		i32 nSkip = pTargetActor->GetPhysicalSkipEntities(pSkipEntities,10); 
		for(i32 i = 0; i < nSkip; ++i)
		{
			stl::push_back_unique(skipList, pSkipEntities[i]);
		}
	...
	i32k thisFrameId = gEnv->pRenderer->GetFrameID();
...
{
	u8k prevMode = m_postEffectsForSpecMode;
	u8k specMode = clientPlayer.GetSpectatorMode();
	...
	if( prevMode != specMode )
	{
		...
		if( specMode == CActor::eASM_Killer )
		{
			...
			pAreaUpr->ExitAllAreas(clientPlayer.GetEntity());
		}
	}
...
void CDeathCameraMode::Activate(const CPlayer& clientPlayer)
{
	...
	m_turnAngle = drx_random(-kTurnAngleMax, kTurnAngleMax);
	...
	DRX_ASSERT_MESSAGE( pSpectatorModule, "Needs a spectator module to change the mode!");
...
void CDeathCameraMode::Init(const CPlayer* subject, const EntityId killerEid, i32k hitType)
{
...
bool CDeathCameraMode::UpdateView(const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime)
{
	...
	DRX_ASSERT(lookPosType != eLPT_Null);
	...
	// calculate the look pos and (desired) cam pos based on the type of look
	{
		...
		DRX_ASSERT((lookPos.x < FLT_MAX) && (lookPos.y < FLT_MAX) && (lookPos.z < FLT_MAX));
...
bool CVehicleCameraMode::UpdateView( const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime )
{
	if (IVehicle* pVehicle = clientPlayer.GetLinkedVehicle())
	{
		...
		if (isFirstPerson)
		{
			...
			if (g_pGameCVars->pl_debug_view)
			{
				DrxWatch("VehicleCamera AnimFactor (%.2f)", cameraFactor);
				DrxWatch("VehicleCameraFP Pos (%.2f, %.2f, %.2f)", firstPersonCameraPose.GetPosition().x, firstPersonCameraPose.GetPosition().y, firstPersonCameraPose.GetPosition().z);
				DrxWatch("VehicleCameraFP Rot (%.2f, %.2f, %.2f, %.2f)", firstPersonCameraPose.GetRotation().v.x, firstPersonCameraPose.GetRotation().v.y, firstPersonCameraPose.GetRotation().v.z, firstPersonCameraPose.GetRotation().w);
				DrxWatch("VehicleCameraVP Pos (%.2f, %.2f, %.2f)", vehicleCameraPose.GetPosition().x, vehicleCameraPose.GetPosition().y, vehicleCameraPose.GetPosition().z);
				DrxWatch("VehicleCameraVP Rot (%.2f, %.2f, %.2f, %.2f)", vehicleCameraPose.GetRotation().v.x, vehicleCameraPose.GetRotation().v.y, vehicleCameraPose.GetRotation().v.z, vehicleCameraPose.GetRotation().w);
				DrxWatch("VehicleCameraBase Pos (%.2f, %.2f, %.2f)", baseCameraPose.GetPosition().x, baseCameraPose.GetPosition().y, baseCameraPose.GetPosition().z);
				DrxWatch("VehicleCameraBase Rot (%.2f, %.2f, %.2f, %.2f)", baseCameraPose.GetRotation().v.x, baseCameraPose.GetRotation().v.y, baseCameraPose.GetRotation().v.z, baseCameraPose.GetRotation().w);
				DrxWatch("VehicleCamera Pos (%.2f, %.2f, %.2f)", viewParams.position.x, viewParams.position.y, viewParams.position.z);
				DrxWatch("VehicleCamera Rot (%.2f, %.2f, %.2f, %.2f)", viewParams.rotation.v.x, viewParams.rotation.v.y, viewParams.rotation.v.z, viewParams.rotation.w);
			}
#endif //_RELEASE
		}
	}
...
bool CPartialAnimationControlledCameraMode::UpdateView(const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime)
{
	...
	else
	{
		...
		if (g_pGameCVars->pl_debug_view)
		{
			...
			DrxWatch("ViewQuatFinal Fwd (%f, %f, %f) Wrld fwd (%f, %f, %f)", fwd.x, fwd.y, fwd.z, wldFwd.x, wldFwd.y, wldFwd.z);
			DrxWatch("PartialAnimControl Pos (%.2f, %.2f, %.2f) Blend(%.2f)", viewParams.position.x, viewParams.position.y, viewParams.position.z, m_currentPositionFactor);
			DrxWatch("PartialAnimControl Rot (%.2f, %.2f, %.2f, %.2f) Blend(%.2f)", viewParams.rotation.v.x, viewParams.rotation.v.y, viewParams.rotation.v.z, viewParams.rotation.w, m_currentOrientationFactor);
			fwd = viewParams.rotation.GetColumn1();
			DrxWatch("PartialAnimControl Fwd (%f, %f, %f)", fwd.x, fwd.y, fwd.z);
		}
#endif //!_RELEASE
	}
