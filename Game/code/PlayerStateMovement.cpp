...
#include <drx3D/Game/Effects/WaterEffects.h>
...
{
	DRX_ASSERT( !player.IsAIControlled() );
	...
	case STATE_EVENT_DEBUG:
		{
			...
			STATE_DEBUG_EVENT_LOG( this, event, false, state_red, "Active: StateMovement: CurrentFlags: %s", AutoEnum_GetStringFromBitfield( m_flags.GetRawFlags(), stateFlags, sizeof(stateFlags)/sizeof(tuk) ).c_str() );
		}
		break;
	}
...
const CPlayerStateMovement::TStateIndex CPlayerStateMovement::Fly( CPlayer& player, const SStateEvent& event )
{
	...
	case PLAYER_EVENT_FLY:
		{
			i32k flyMode = static_cast<const SStateEventFly&> (event).GetFlyMode();
...
{
	DRX_ASSERT( inputEvent.m_inputEvent == SInputEventData::EInputEvent_Sprint );
...
void CPlayerStateMovement::ProcessSprint( CPlayer& player, const SPlayerPrePhysicsData& prePhysicsEvent )
{
	...
	u32 flagsToCheck = g_pGameCVars->cl_sprintToggle ? (EPlayerStateFlags_SprintPressed | EPlayerStateFlags_Sprinting) : (EPlayerStateFlags_SprintPressed);
