...
public:
	...
	void SetWeaponParams(const SParams_WeaponFPAiming& params, float runFactor)
	{
		...
		const float stabilizeWeight = stabilizeFactorsTable[ (u32)(params.flags.AreAllFlagsActive( eWFPAF_aimStabilized ) == true) ];
		...
		u32k playbackSpeedIdx = (params.flags.AreAllFlagsActive( eWFPAF_crouched) == true);
		DRX_ASSERT(playbackSpeedIdx < 2);
...
void CWeaponFPAiming::Update(const SParams_WeaponFPAiming &params)
{
	...
	u32k speedIdx = (params.flags.AreAnyFlagsActive( eWFPAF_zoomed | eWFPAF_crouched ) == true);
	DRX_ASSERT(speedIdx < 2);
	...
	u32k sprintTransitionIdx = (params.flags.AreAllFlagsActive( eWFPAF_sprinting ) == true);
	DRX_ASSERT(sprintTransitionIdx < 2);
