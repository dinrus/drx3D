...
//------------------------------------------------------------------------
void CReplayActor::SetupActionController(tukk controllerDef, tukk animDB1P, tukk animDB3P)
{
	IMannequin &mannequinSys = gEnv->pGame->GetIGameFramework()->GetMannequinInterface();
	const SControllerDef *pContDef = NULL;
	if (controllerDef)
	{
		pContDef = mannequinSys.GetAnimationDatabaseUpr().LoadControllerDef(controllerDef);
	}
	if (animDB1P)
	{
		m_pAnimDatabase1P = mannequinSys.GetAnimationDatabaseUpr().Load(animDB1P);
	}
	if (animDB3P)
	{
		m_pAnimDatabase3P = mannequinSys.GetAnimationDatabaseUpr().Load(animDB3P);
	}
...
void CReplayActor::UpdateScopeContexts()
{
	if (m_pActionController)
	{
		...
		i32k contextID = PlayerMannequin.contextIDs.Weapon;
		if (contextID >= 0)
		{
			u32k numScopes = m_pActionController->GetContext().controllerDef.m_scopeDef.size();
			...
			if (scopeCharInst != pICharInst)
			{
				if (pICharInst)
				{
					...
					const IAnimationDatabase *animDB = !weaponParams.adbFile.empty() ? mannequinSys.GetAnimationDatabaseUpr().Load(weaponParams.adbFile.c_str()) : NULL;
					if (animDB)
					{
		...
		u32k NUM_ACCESSORY_SLOTS = 2;
		u32 ACCESSORY_CONTEXT_IDS[NUM_ACCESSORY_SLOTS] = {PlayerMannequin.contextIDs.attachment_top, PlayerMannequin.contextIDs.attachment_bottom};
		for (u32 attachmentSlot=0; attachmentSlot<NUM_ACCESSORY_SLOTS; attachmentSlot++)
		{
			i32k attachmentContextID = ACCESSORY_CONTEXT_IDS[attachmentSlot];
			ICharacterInstance *scopeCharInst = NULL;
			ICharacterInstance *accessoryCharInst = NULL;
			CItem *accessory = NULL;
			tukk contextName = m_pActionController->GetContext().controllerDef.m_scopeContexts.GetTagName(attachmentContextID);
			...
			if (scopeCharInst != accessoryCharInst)
			{
				...
				{
					const IAnimationDatabase *animDB = mannequinSys.GetAnimationDatabaseUpr().Load(accessory->GetParams().adbFile.c_str());
					if (animDB)
					{
...
void CReplayActor::LoadCharacter(tukk filename)
{
...
//------------------------------------------------------------------------
void CReplayActor::PlayAnimation(i32 animId, const DrxCharAnimationParams& params, float speedMultiplier, float animTime)
{
...
//------------------------------------------------------------------------
void CReplayActor::PlayUpperAnimation(i32 animId, const DrxCharAnimationParams& params, float speedMultiplier, float animTime)
{
	float adjustedLayerTime = 0;
	ICharacterInstance* pCharacterInstance = GetEntity()->GetCharacter(0);
	if (animTime == 0 || GetAdjustedLayerTime(pCharacterInstance, animId, params, speedMultiplier, animTime, adjustedLayerTime))
...
//------------------------------------------------------------------------
bool CReplayActor::GetAdjustedLayerTime(ICharacterInstance* pCharacterInstance, i32 animId, const DrxCharAnimationParams& params, float speedMultiplier, float animTime, float &adjustedLayerTime)
{
	ISkeletonAnim* pSkeletonAnim = pCharacterInstance->GetISkeletonAnim();
	...
	DRX_ASSERT_MESSAGE(animTime >= 0, "Animation time shouldn't be less than 0");
...
void SBasicReplayMovementParams::SetDesiredLocalLocation2( ISkeletonAnim* pSkeletonAnim, const QuatT& desiredLocalLocation, float lookaheadTime, float fDeltaTime, float turnSpeedMultiplier)
{
	...
	u32 NumAnimsInQueue = pSkeletonAnim->GetNumAnimsInFIFO(0);
	...
	u32 active=0;
	for (u32 a=0; a<NumAnimsInQueue; a++)
	{
	...
	for (i32 a=nMaxActiveInQueue-1; (a >= 0) && !pLMG; --a)
	{
	...
	{
		//	DrxFatalError("DrxAnimation: no smooth");
...
void CReplayActor::GunRemovalListener::OnEntityEvent( IEntity *pEntity,SEntityEvent &event )
{
	...
	{
		DRX_ASSERT_MESSAGE(pEntity->GetId() == pReplayActor->GetGunId(), "Received event for an entity that wasn't the gun we subscribed to");
...
void CReplayActor::OnRagdollized()
{
	...
	{
		IPhysicalEntity* pPhys = GetEntity()->GetPhysics();
		DRX_ASSERT(pPhys && pPhys->GetType()==PE_ARTICULATED);
...
void CReplayActor::TransitionToCorpse(IEntity& corpse)
{
	...
	if(ICharacterInstance *pCharInst = pEntity->GetCharacter(0))
	{
		...
		if(CTeamVisualizationUpr* pTeamVisUpr = g_pGame->GetGameRules()->GetTeamVisualizationUpr())
		{
			pTeamVisUpr->RefreshTeamMaterial(&corpse, false, m_isFriendly); 
		}
	}
}
