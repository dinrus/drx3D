...
DRX_IMPLEMENT_GTI(CSpammer, CSingle);
...
i32 CSpammerCloudTargets::GetNumLockOns() const
{
	return m_numLockOns;
}


i32 CSpammerCloudTargets::GetNumLockOns(EntityId target) const
{
	for (size_t i = 0; i < m_targets.size(); ++i)
		if (m_targets[i].m_target == target)
...
const CSpammerCloudTargets::STarget& CSpammerCloudTargets::GetTarget(i32 idx) const
{
	return m_targets[idx];
}


i32 CSpammerCloudTargets::GetNumTargets() const
{
	return static_cast<i32>(m_targets.size());
}
...
void CSpammer::Update(float frameTime, u32 frameId)
{
...
bool CSpammer::ShootRocket(EntityId target)
{
	...
	i32 ammoCount = m_pWeapon->GetAmmoCount(pAmmoClass);
...
{
	i32k currentAmmoCount = GetAmmoCount();
...
void CSpammer::UpdatePotentialTargets()
{
	...
	const TAutoaimTargets& aaTargets = autoAimUpr.GetAutoAimTargets();
	...
	for (i32 i = 0; i < targetCount; ++i)
	{
		...
		DRX_ASSERT(target.entityId != m_pWeapon->GetOwnerId());
		...
		i32k kAutoaimVisibilityLatency = 0;
...
EntityId CSpammer::GetNextLockOnTarget() const
{
	...
	{
		float randomProbability = drx_random(0.0f, m_potentialTargets.m_totalProbability);
		i32 selectedIdx;
