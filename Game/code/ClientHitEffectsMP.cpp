...
void CClientHitEffectsMP::Initialise()
{
	...
	if(tukk libraryName = rootNode->getAttr("library"))
	{
...
void CClientHitEffectsMP::ProcessEffectInfo(SHitEffectInfoSet& hitEffectSet, XmlNodeRef xmlNode, tukk libraryName)
{
	...
	for (uint i = 0; i < numEffects; i++)
	{
		...
		{
			if(tukk nameTag = childNode->getTag())
			{
				if(!foundDefault && !strcmp("default", nameTag))
				{
					tukk effectName = childNode->getAttr("effect");
...
void CClientHitEffectsMP::SpawnMaterialEffect(const CGameRules* pGameRules, const CPlayer* pTargetPlayer, const HitInfo &hitInfo, bool inArmour)
{
	...
	if(inArmour)
	{
		...
		i32k effect = CLAMP(threat==CGameRules::eHostile ? 1 : 0, 0, static_cast<i32>(m_armourTeamEffects.size()) - 1);
