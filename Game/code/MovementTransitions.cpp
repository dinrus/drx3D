...
#include <drx3D/Game/Player.h> // for CPlayer, IMovementController, SActorFrameMovementParams, ...
#include <drx3D/Game/MovementTransitionsUtil.h>
#include <drx3D/Game/MovementTransitionsController.h>
#include <drx3D/Game/MovementTransitions.h>

#include <drx3D/AI/IAIActor.h> // for IAIObject, IAIActor, IAIPathAgent

#include <drx3D/CoreX/Game/GameUtils.h> // for AngleDifference
#include <drx3D/CoreX/TypeInfo_impl.h> // for DRX_ARRAY_COUNT
#include <drx3D/Game/GameCVars.h>
#include <drx3D/AI/IAIDebugRenderer.h>

#define MovementTransitionsLog if (false) DrxLog
//#define MovementTransitionsLog DrxLog
...
namespace MovementTransitionsUtil
{
	...
	i32k NUM_PSEUDOSPEEDS = DRX_ARRAY_COUNT(s_pseudoSpeeds);
	...
	i32k NUM_TRANSITIONTYPES = DRX_ARRAY_COUNT(s_transitionTypes);
}

#ifndef _RELEASE
namespace MovementTransitionsDebug
{
	static i32 s_debug_y = 50;
	static i32 s_debug_frame = -1;
	static float s_dbg_color_unsignaled[4] = {0.9f,0.7f,0.5f,1};
	static float s_dbg_color_signaled[4] = {0.9f,1.0f,0.5f,1};

	//////////////////////////////////////////////////////////////////////////
	static tukk GetTransitionTypeName(ETransitionType transitionType)
	{
		for(i32 iTransitionType = 0; iTransitionType < MovementTransitionsUtil::NUM_TRANSITIONTYPES; ++iTransitionType)
		{
	...
	//////////////////////////////////////////////////////////////////////////
	static tukk GetPseudoSpeedName(float pseudoSpeed)
	{
		for(i32 iPseudoSpeed = 0; iPseudoSpeed < MovementTransitionsUtil::NUM_PSEUDOSPEEDS; ++iPseudoSpeed)
		{
...
bool STransition::ReadParams( ETransitionType _transitionType, const struct IItemParamsNode*const pParams )
{
	...
	DRX_ASSERT_TRACE(animGraphSignal.length() > 0, ("animGraphSignal not found or empty"));
	...
	i32 iContext = 0; // default = don't care
	...
	if (transitionType == eTT_Stop)
	{
		...
		DRX_ASSERT_MESSAGE(!hasDesiredArrivalAngle ||  hasArrivalAngleTolerance, "Transition has arrivalAngle but no arrivalAngleTolerance");
		DRX_ASSERT_MESSAGE( hasDesiredArrivalAngle || !hasArrivalAngleTolerance, "Transition has arrivalAngleTolerance but no arrivalAngle");
	}
	else
	{
		...
		DRX_ASSERT_MESSAGE(!hasDesiredTargetTravelAngle ||  hasTargetTravelAngleTolerance, "Transition has targetTravelAngle but no targetTravelAngleTolerance");
		DRX_ASSERT_MESSAGE( hasDesiredTargetTravelAngle || !hasTargetTravelAngleTolerance, "Transition has targetTravelAngleTolerance but no targetTravelAngle");
...
//////////////////////////////////////////////////////////////////////////
bool STransition::ReadPseudoSpeedParam( tukk name, bool required, const struct IItemParamsNode*const pParams, float*const pPseudoSpeed ) const
{
	tukk szPseudoSpeed = pParams->GetAttribute(name);

	if (!szPseudoSpeed && !required)
		return false;

	DRX_ASSERT_TRACE(szPseudoSpeed, ("%s not found", name));
	if (!szPseudoSpeed)
		return false;
	...
	if (pseudoSpeed == -1.0f)
	{
		DRX_ASSERT_TRACE(false, ("Invalid pseudospeed '%s'", szPseudoSpeed));
		return false;
	}
...
//////////////////////////////////////////////////////////////////////////
bool STransition::ReadStanceParam( tukk name, bool required, const struct IItemParamsNode*const pParams, EStance*const pStance ) const
{
	tukk szStance = pParams->GetAttribute(name);

	if (!szStance && !required)
		return false;

	DRX_ASSERT_TRACE(szStance, ("%s not found", name));
	if (!szStance)
		return false;
	...
	{
		DRX_ASSERT_TRACE(false, ("Invalid stance '%s'", szStance));
		return false;
	}

	DRX_ASSERT(stance >= STANCE_NULL);
	DRX_ASSERT(stance < STANCE_LAST);
...
//////////////////////////////////////////////////////////////////////////
bool STransition::ReadIntParam( tukk name, bool required, i32 min, i32 max, const struct IItemParamsNode*const pParams, i32*const pI ) const
{
	i32 i;
	...
	DRX_ASSERT_TRACE(success, ("%s not found", name));
	if (!success) return false;
	DRX_ASSERT_TRACE(i >= min, ("%s (%d) too small (should be > %d)", name, i, min));
	if (!(i >= min)) return false;
	DRX_ASSERT_TRACE(i <= max, ("%s (%d) too big (should be < %d)", name, i, max));
...
//////////////////////////////////////////////////////////////////////////
bool STransition::ReadFloatParam( tukk name, bool required, float min, float max, const struct IItemParamsNode*const pParams, float*const pF ) const
{
	float f;
	...
	DRX_ASSERT_TRACE(success, ("%s not found", name));
	if (!success) return false;
	DRX_ASSERT_TRACE(f >= min, ("%s (%f) too small (should be > %f)", name, f, min));
	if (!(f >= min)) return false;
	DRX_ASSERT_TRACE(f <= max, ("%s (%f) too big (should be < %f)", name, f, max));
...
//////////////////////////////////////////////////////////////////////////
bool STransition::ReadAngleParam( tukk name, bool required, float min, float max, const struct IItemParamsNode*const pParams, float*const pAngle ) const
{
	float angleDegrees;
...
bool STransition::CheckSpaceToPerformTransition( const CMovementTransitions& transitions, const STransitionSelectionParams &transParams, const Vec3 &oldMoveDirection, const Vec3& playerPos, CPlayer*const pPlayer) const
{
	if ((transitionType == eTT_DirectionChange) && !transParams.m_bPredicted)
	{
		...
		if (const IAIPathAgent* pAIAgent = pAI->CastToIAIActor())
		{
			...
			{
				Vec2 path[2];
				DRX_ASSERT(oldMoveDirection.IsUnit());
	...
	SActorFrameMovementParams*const pMoveParams) : m_transitionType(eTT_None), m_transitionDistance(0.0f), m_pseudoSpeed(0.0f), m_travelAngle(0.0f), m_jukeAngle(0.0f), m_stance(stance)
{
	...
	u8 allowedTransitionFlags = _allowedTransitionFlags;
	...
	if (newSample.pseudoSpeed > 0.0f)
	{
		...
		else // at the moment start & stop are mutually exclusive
		{
			...
			if (allowedTransitionFlags & (1<<eTT_DirectionChange))
			{
				...
				{
					if (gEnv->pAISystem && !gEnv->pAISystem->GetSmartObjectUpr()->CheckSmartObjectStates(player.GetEntity(), "Busy"))
					{
						...
						DRX_ASSERT(assumedDistanceToJuke > FLT_EPSILON);
						...
						if (pTransition)
						{
							...
							DRX_ASSERT(proposedTransitionDistance > FLT_EPSILON);
	...
	else // if (newSample.pseudoSpeed <= 0.0f)
	{
		...
		)
		{
			if (allowedTransitionFlags & (1<<eTT_Stop))
			{
				...
				if( gEnv->pAISystem )
				{
					ISmartObjectUpr* pSmartObjectUpr = gEnv->pAISystem->GetSmartObjectUpr();
					if (!pSmartObjectUpr || !pSmartObjectUpr->CheckSmartObjectStates(player.GetEntity(), "Busy"))
					{
...
///////////////////////////////////////////////////////////////////////////////
void CMovementTransitions::GetMemoryUsage(IDrxSizer* s) const
{
...
EMovementTransitionState CMovementTransitions::Update(
	...
	float*const pJukeTurnRateFraction,
	Vec3*const pBodyTarget,
	tukk*const pBodyTargetType ) const
{
	if (!g_pGame->GetCVars()->g_movementTransitions_enable)
		return eMTS_None;
...
#ifndef _RELEASE
	{
		...
		{
			DRX_ASSERT(index < (i32)m_transitions.size());
			DrxLog("Transition\tentity=%s\tindex=%i\t%s\t%s", 
...
//////////////////////////////////////////////////////////////////////////
void CMovementTransitions::FindBestMatch( const STransitionSelectionParams& transParams, const STransition** ppBestTransition, i32* pBestIndex, STransitionMatch* pBestMatch ) const
{
...
bool CMovementTransitions::ReadTransitionsParams( const struct IItemParamsNode* pParams )
{
	...
	for(i32 i=0; i<pParams->GetChildCount(); ++i)
	{
		const IItemParamsNode* pEntry = pParams->GetChild(i);
		if (!pEntry)
			continue;

		tukk entryName = pEntry->GetName();

		bool bCorrectEntry = false;
		for(i32 t = 0; t < MovementTransitionsUtil::NUM_TRANSITIONTYPES; ++t)
		{
