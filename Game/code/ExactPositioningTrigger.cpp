...
#include <drx3D/Game/ExactPositioningTrigger.h>
#include <drx3D/Game/AnimationGraph/AnimationGraphCVars.h>
...
CExactPositioningTrigger::CExactPositioningTrigger( const Vec3& pos, float width, const Vec3& triggerSize, const Quat& orient, float orientTolerance, float animMovementLength )
	...
	, m_userOrient(orient)
{
	DRX_ASSERT(m_pos.IsValid());
	DRX_ASSERT(m_userPos.IsValid());
	DRX_ASSERT(m_orient.IsValid());
	DRX_ASSERT(m_userOrient.IsValid());
	DRX_ASSERT(m_posSize.IsValid());
	DRX_ASSERT(NumberValid(m_cosOrientTolerance));
	DRX_ASSERT(NumberValid(m_width));
...
void CExactPositioningTrigger::Update( float frameTime, Vec3 userPos, Quat userOrient, bool allowTriggering )
{
	...
	DRX_ASSERT(userPos.IsValid());
	DRX_ASSERT(userOrient.IsValid());
