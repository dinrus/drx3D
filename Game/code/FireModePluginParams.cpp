...
$Id$
$DateTime$
Описание: Fire Mode Plugin Parameters
...
void SFireModePlugin_HeatingParams::PreLoadAssets()
{
	for (i32 i = 0; i < 2; i++)
	{
		if(!gEnv->pParticleUpr->FindEffect(effect[i].c_str()))
		{
			effect[i].clear();
		}
	}
...
//------------------------------------------------------------------------
void SFireModePlugin_HeatingParams::GetMemoryUsage(IDrxSizer * s) const
{
	for (i32 i=0; i<2; i++)
	{
...
//------------------------------------------------------------------------
void SFireModePlugin_RejectParams::GetMemoryUsage(IDrxSizer * s) const
{
...
void SFireModePlugin_RecoilShakeParams::GetMemoryUsage(IDrxSizer * s) const
{
