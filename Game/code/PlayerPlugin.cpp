...
CPlayerPlugin::~CPlayerPlugin()
{
	if (m_ownerPlayer)
	{
		DRX_ASSERT_MESSAGE(!m_entered, ("[PLAYER PLUG-IN] <%s %s \"%s\"> %s", m_ownerPlayer->IsClient() ? "Local" : "Remote", m_ownerPlayer->GetEntity()->GetClass()->GetName(), m_ownerPlayer->GetEntity()->GetName(), string().Format("Player plug-in is being destroyed without having been shut down cleanly!").c_str()));
	}
	else
	{
		DRX_ASSERT_MESSAGE(!m_entered, ("[PLAYER PLUG-IN] <NULL> %s", string().Format("Player plug-in is being destroyed without having been shut down cleanly!").c_str()));
	}
}
...
void CPlayerPlugin::HandleEvent(EPlayerPlugInEvent theEvent, uk  data)
{
}

ukk CPlayerPlugin::DefaultData(EPlayerPlugInData dataType)
{
	const static bool  m_defaultBool  = false;
	const static float m_defaultFloat = 0.0f;
	const static float m_defaultFloatOne = 1.0f;
	const static i32   m_defaultInt = 0;
	...
	DRX_ASSERT_MESSAGE(false, "We always want to provide default data");
	return NULL;
}
