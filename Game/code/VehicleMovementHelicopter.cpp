...
//------------------------------------------------------------------------
void CVehicleMovementHelicopter::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
...
{
	u32 seatCount = m_pVehicle->GetSeatCount();

	for (u32 seatId = 1; !(seatId > seatCount); ++seatId)
	{
...
//------------------------------------------------------------------------
void CVehicleMovementHelicopter::GetMemoryUsage( IDrxSizer * pSizer ) const
{
...
//------------------------------------------------------------------------
void CVehicleMovementHelicopter::GetMemoryUsageInternal( IDrxSizer * pSizer ) const
{
...
void CVehicleMovementHelicopter::UpdateNetworkError(const Vec3& netPosition, const Quat& netRotation)
{
	...
	if(!physStatus->pos.IsZero(0.5f))
	{
		...
		// DrxWatch("posError=%.2f", posError.GetLength());
