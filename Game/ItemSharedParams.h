...
$Id$
$DateTime$
Описание: Stores all item parameters that don't mutate in instances...
...
struct SParams
{
	...
	void GetMemoryUsage(IDrxSizer * s) const
	{
		...
		for (i32 i = 0; i < IItem::eIH_Last; i++)
		{
...
struct SCachedItemAnimation
{
	...
	SCachedItemAnimation( u32k hash, const int8 hand, u32k suffixHash, u32k modelHash)
		: m_stringHash(hash)
		, m_hand(hand)
	...
	inline u32 GetChangeFlags(const SCachedItemAnimation &n) const
	{
		u32 ret = 0;
	...
	ILINE void SetAnimationId(i32 animationId) { m_animationId = animationId; }
	ILINE i32 GetAnimationId() const { return m_animationId; }
...
public:
	...
	virtual tukk GetDataType() const { return "ItemParams"; }
...
protected:
	...
	i32	 TargetToSlot(tukk name);
	void PrefixPathIfFilename(tukk pPath, ItemString& filename);
