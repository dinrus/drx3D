...
public:
	...
	i32 GetNumClientAwards();
	EAfterMatchAwards GetNthClientAward(i32k Nth);

	void ClientReceivedAwards(i32 numAwards, u8k awardsArray[]);
	void ServerReceivedAwardsWorkingFromPlayer(EntityId playerEntityId, i32 numWorkingValues, const CGameRules::SAfterMatchAwardWorkingsParams::SWorkingValue workingValues[]);

	tukk GetNameForAward(EAfterMatchAwards inAward);
	
	void Clear();

protected:

	static i32k kMaxLocalAwardsGiven = EAMA_Max;
	static i32k kMaxLocalAwardsActuallyWon = 3;
	...
	struct SAwardsForPlayer
	{
		...
		DrxFixedArray<EAfterMatchAwards, CAfterMatchAwards::kMaxLocalAwardsGiven> m_awards;	// stores awards actually awarded
	...
	i32 SvCalculateAward(EAfterMatchAwards inAward);
	i32 CalculateAward(EAfterMatchAwards inAward);

	void FilterWinningAwards();
	void HaveGotAwards();
	void DebugMe(bool useDrxLog=false);

	i32 GetFlagsForAward(EAfterMatchAwards inAward);
