...
{
	SLedgeTransitionData( u16k ledgeId ) 
...
struct SStateEventLedge : public SStateEvent
{
	...
	u16 GetLedgeId() const { return (u16)GetData(1).GetInt(); }
...
{
	SStateEventInteractiveAction(const EntityId objectId, bool bUpdateVisibility, i32 interactionIndex = 0, float actionSpeed = 1.0f)
		: 
	SStateEvent( PLAYER_EVENT_INTERACTIVE_ACTION )
	{
		AddData( static_cast<i32> (objectId) );
		AddData( bUpdateVisibility );
		AddData( actionSpeed );
		AddData( static_cast<i32> (interactionIndex) ); 
	}

	SStateEventInteractiveAction(tukk interactionName, bool bUpdateVisibility, float actionSpeed = 1.0f )
		: 
	SStateEvent( PLAYER_EVENT_INTERACTIVE_ACTION )
	{
	...
	tukk GetObjectInteractionName() const
	{
		if( GetData(0).m_type == SStateEventData::eSEDT_voidptr )
		{
			return static_cast<tukk > (GetData(0).GetPtr());
		}
		return NULL;
	}

	i32 GetInteractionIndex() const
	{
...
struct SInputEventData
{
	...
	SInputEventData( EInputEvent inputEvent, EntityId entityId, const ActionId& actionId, i32 activationMode, float value )
		:
...
{
	SStateEventStanceChanged( i32k targetStance )
		:
	...
	ILINE i32 GetStance() const { return GetData(0).GetInt(); }
};
...
struct SStateEventCoopAnim : public SStateEvent
{
	...
	{
		AddData( static_cast<i32>(targetID) );
	}
...
struct SStateEventButtonMashingSequence : public SStateEvent
{
	...
	{
		AddData( (i32)sequenceType );
		AddData( (ukk )&params );
		AddData( (ukk )&callbacks );
	}
