...
#include <drx3D/Game/Testing/FeatureTestMgr.h>		// For IFeatureTest
...
public:
	...
	static i32k SEQ_ENTITY_COUNT = 6;
	static i32k SEQ_ENTITY_FIRST_INPUT_PORT = eInputPorts_Entity1;
	static i32k SEQ_ENTITY_FIRST_OUTPUT_PORT = eInputPorts_Entity1Passed;
...
protected:
	...
	void OnTestResult(bool result, tukk reason);

	/// Returns the number of attached entities
	i32 GetTestEntityCount();
	...
	bool GetEntityAtIndex(i32 index, IEntity* &outEntity, bool bPrepareFromPool = false);

	bool GetEntityDataAtIndex(i32 index, SEntityData& data);

	/// Utility function for returning a test result to the manager and updating any associated entity passed trigger
	void SetResult(bool result, tukk reason);
...
private:
	...
	i32                         m_entitySeqIndex;		// The current (virtual - not port) entity index (or -1)
	u32                      m_failureCount;			// How many failures during a sequence run (used for AllPassed output)
...
private:
	...
	i32 m_prevHitCount;								/// The hit count of the associated CCCPOINT at the last sync

	bool	m_watchRequested;
};

///This class will listen for communication events within a timeout window
class CFlowNode_ListenForCommunication : public CFlowBaseNode<eNCT_Instanced>, public ICommunicationUpr::ICommGlobalListener
{
public:
	...
	virtual void OnCommunicationEvent(ICommunicationUpr::ECommunicationEvent event, EntityId actorID, const CommID& playID);
