...
-------------------------------------------------------------------------
–ò—Å—Ç–æ—Ä–∏—è:
- 10:3:2009  : Created by Jan M¸ller, integrated from G04
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			...
			InputPortConfig<i32>("Key", 0, _HELP("XBoxOne controller key"), NULL, _UICONFIG(XBoxKeyEnum)),
	...
	virtual bool OnInputEvent( const SInputEvent &event )
	{
		...
		{
#if DRX_PLATFORM_ORBIS
			i32 nThisKey = event.keyId;
			i32 nKey = -1;
			i32 nInput = GetPortInt(&m_actInfo, EIP_Key);
			i32 tableSize = DRX_ARRAY_COUNT(OrbisKeyTable);
			if ( nInput>=0 && nInput<tableSize )
				nKey = OrbisKeyTable[nInput];
#else
			// Translate key, check value
			i32k nThisKey = TranslateKey(event.keyId);
			i32k nKey = GetPortInt(&m_actInfo, EIP_Key);
#endif
			if (nKey == nThisKey)
			{
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			InputPortConfig_Void("Enable", _HELP("Enable reporting")),
			InputPortConfig_Void("Disable", _HELP("Disable reporting")),
			InputPortConfig<i32>("Key", 0, _HELP("XBoxOne controller key"), NULL, _UICONFIG(XBoxAnalogEnum)),
			InputPortConfig<bool>("NonDevMode", false, _HELP("If set to true, can be used in Non-Devmode as well [Debugging backdoor]")),
			{0}
		};
		static const SOutputPortConfig outputs[] = {
	...
	EXBoxKey TranslateKeyIntoFGInputEnum(i32 nKeyId, bool& isXAxis )
	{
		isXAxis = true;
		switch (nKeyId)
		{
	...
	void DisableOverlay()
	{
		...
		if (pClientActor)
		{
			if (pClientActor->GetActorClass() == CPlayer::GetActorClassType())
			{
				...
				if (pPlayerInput)
				{
					pPlayerInput->OnAction(CDrxName("move_overlay_disable"), eAAM_OnPress, 0.f);
				}
			}
		}
	}
		...
		if (pClientActor)
		{
			if (pClientActor->GetActorClass() == CPlayer::GetActorClassType())
			{
				...
				if (pPlayerInput)
				{
					...
					pPlayerInput->OnAction(CDrxName("move_overlay_enable"), eAAM_OnHold, 0.f);
					pPlayerInput->OnAction(CDrxName("move_overlay_x"), eAAM_OnHold, moveX);
					pPlayerInput->OnAction(CDrxName("move_overlay_y"), eAAM_OnHold, moveY);
					pPlayerInput->OnAction(CDrxName("move_overlay_weight"), eAAM_OnHold, weight);
				}
			}
		}
	}
