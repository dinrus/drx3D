...
public:
	...
	void GetConfiguration( SFlowNodeConfig& config )
	{
		static const SInputPortConfig in_ports[] = 
		{
			...
			InputPortConfig<i32> ("EventType", 0, _HELP("Type of event"), 0, _UICONFIG("enum_int:LEVELGAMEPLAYSTART=0")),
			{0}
		};
		...
		case eFE_Activate:
			{
				...
				{
					i32k iEventType = GetPortInt(pActInfo, EIP_EventType);
...
public:
	...
	virtual void Serialize(SActivationInfo *pActInfo, TSerialize ser)
	{
		...
		{
			m_state = ES_Notify;  // because we are not going to receive any event notification from DrxAction, and we still want to notify the Done output.
		}
...
private:
	...
	EState	m_state;
	string	m_name;
	i32		m_extraCheckDeadTimerCounter; 
