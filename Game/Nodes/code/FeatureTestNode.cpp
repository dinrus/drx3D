...
#include <drx3D/Game/FeatureTestNode.h>

#include <drx3D/Game/Testing/FeatureTester.h>	// For ENABLE_FEATURE_TESTER definition(!)
#include <drx3D/AI/ICommunicationUpr.h>
#include <drx3D/Sys/ICodeCheckpointMgr.h>			// For ICodeCheckpointMgr & CCodeCheckPoint
#include <drx3D/Sys/IStatoscope.h>
#include <drx3D/AI/IGoalPipe.h>							// For Entity debug info
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eInputPorts_Ready))
				{
					...
					{
						m_ready = readyInput;
						DrxLogAlways("Test %s now: %s", Name(), m_ready ? "Ready to run" : "Not ready to run");
					}
				}
...
void CFlowNode_FeatureTest::GetMemoryUsage(IDrxSizer* sizer) const
{
	sizer->Add(*this);
}
...
bool CFlowNode_FeatureTest::Start()
{
	...
	if (!m_ready)
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Test %s is being forced to run despite not being ready.", Name());
	...
	if (!m_running)
	{
		...
		if (result)
		{
			...
			if (maxTime <= 0.0f)
				DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "Test %s is been run with max time set to %f - test may never end!", Name(), maxTime);
		}
		else
		{
			DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "Test %s failed to start! Have you already run it?", Name());
		}
	}
	else
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Feature test %s already running (ignoring run request).", Name());
	}
	...
	if (m_running)
	{
...
#if ENABLE_FEATURE_TESTER
		DrxWatch("$7[FG FeatureTest]$o Running test '%s'", Name());

		const string& description = GetPortString(&m_actInfo, eInputPorts_Description);
		if (!description.empty())
		{
			DrxWatch("$7[FG FeatureTest]$o %s", description.c_str());
		}
		...
		if (!bHasEntry && !pFollowEntity)
		{
			...
			if (!bHasEntry && !pFollowEntity)
			{
				...
				for (i32 i = 0; i < SEQ_ENTITY_COUNT; ++i)
				{
...
tukk CFlowNode_FeatureTest::Name()
{
...
{	
	i32k entityCount = GetTestEntityCount();


	//useful for debugging
	if(entityCount == 0)
	{
		DrxLogAlways("Parsing feature testNode \"%s\". No entities are attached to it, returning...", Name());
	}
	...
	DrxLogAlways("Parsing feature testNode \"%s\". %d entities are attached to it", Name(), entityCount );
...
bool CFlowNode_FeatureTest::StartNextTestRun()
{
	...
	if (sequential)
	{
		...
		DRX_ASSERT(m_entitySeqIndex >= -1 && m_entitySeqIndex < SEQ_ENTITY_COUNT);

		// If first run
		if (!TestHasRun())
		{
			DrxLogAlways("Running sequential test \"%s\" for %d entities...", Name(), entityCount);
		}

		bool bHasEntry = false;
		IEntity* pSeqEntity = NULL;
		for (i32 i = m_entitySeqIndex + 1; i < SEQ_ENTITY_COUNT; ++i)
		{
		...
		if (pSeqEntity)
		{
			...
			DrxLogAlways("Starting test: \"%s[%s]\". Max time: %fs. Entity start pos: (%f, %f, %f)",
										Name(),
		...
		else if (!bHasEntry)
		{
			...
			m_entitySeqIndex = -1;
			DrxLogAlways("Finished running sequential test \"%s\" for %d entities.", Name(), entityCount);
		}
	}
	else if (!TestHasRun())	// If test has not yet been run
	{
		...
		DrxLogAlways("Starting test: \"%s\". Max time: %fs.",
									Name(),
									GetPortFloat(&m_actInfo, eInputPorts_MaxTime));
...
void CFlowNode_FeatureTest::OnTestResult(bool result, tukk reason)
{
	if (m_running)
	{
		...
		{
			DrxLogAlways("FeatureTest %s received %s signal when test was inactive (ignoring). Timeout too short?", Name(), result ? "success" : "fail");
		}
}

/// Returns the number of attached entities
i32 CFlowNode_FeatureTest::GetTestEntityCount()
{
	i32 count = 0;
	for (i32 i = 0; i < SEQ_ENTITY_COUNT; ++i)
	{
...
bool CFlowNode_FeatureTest::GetEntityAtIndex(i32 index, IEntity* &outEntity, bool bPrepareFromPool)
{
	IEntitySystem *pEntitySystem = gEnv->pEntitySystem;
	DRX_ASSERT(pEntitySystem);

	IEntityPoolUpr *pEntityPoolUpr = pEntitySystem->GetIEntityPoolUpr();
	DRX_ASSERT(pEntityPoolUpr);
	...
	{
		EntityId id = GetPortEntityId(&m_actInfo, i32(SEQ_ENTITY_FIRST_INPUT_PORT + index));
		...
		if (id)
		{
			...
			if (!outEntity && bPrepareFromPool && pEntityPoolUpr->IsEntityBookmarked(id))
			{
				if (pEntityPoolUpr->PrepareFromPool(id, true))
				{
					outEntity = pEntitySystem->GetEntity(id);
				}
				if (!outEntity)
				{
					DrxLogAlways("Error: Test \"%s\" failed to prepare entity with id \'%u\' from the pool", Name(), id);
				}
			}
		}
	}
...
/// instead this one doesn't call ***pEntityPoolUpr->PrepareFromPool***
/// which could cause undesired behavior 
bool CFlowNode_FeatureTest::GetEntityDataAtIndex(i32 index, SEntityData& outData)
{
	...
	if (index >= -1 && index < SEQ_ENTITY_COUNT)
	{
		...
		if (id)
		{
			...
			else if (pEntitySystem->GetIEntityPoolUpr()->IsEntityBookmarked(id))
			{
				//the entity might be bookmarked
				IEntityPoolUpr *pEntityPoolUpr = pEntitySystem->GetIEntityPoolUpr();
				tukk entityName = pEntityPoolUpr->GetBookmarkedEntityName(id);
				outData.m_name = entityName;				
				tukk className = pEntityPoolUpr->GetBookmarkedClassName(id);
...
{
public:
	EntityLogger(u32 maxDepth)
		: m_maxDepth(maxDepth)
	{
		m_prefixBuff.reserve(maxDepth + 1);
	}

	bool TooDeep(u32& depth)
	{
		return ++depth > m_maxDepth;
	}

	tukk Prefix(u32 depth)
	{
	...
	void Log(tukk name, tukk value, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: \"%s\"", Prefix(depth), name, value);
	}

	void Log(tukk name, const bool& value, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: %s", Prefix(depth), name, value ? "true" : "false");
	}

	void Log(tukk name, u32k& value, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: %u", Prefix(depth), name, value);
	}

	void Log(tukk name, const uint64& value, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: %" PRIu64, Prefix(depth), name, value);
	}

	void Log(tukk name, i32k& value, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: %d", Prefix(depth), name, value);
	}

	void Log(tukk name, const float& value, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: %f", Prefix(depth), name, value);
	}

	void Log(tukk name, const Vec3& value, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: Vec3(%f, %f, %f)", Prefix(depth), name, value.x, value.y, value.z);
	}

	void Log(tukk name, const AABB& value, u32 depth)
	{
		if (TooDeep(depth))
			return;
		...
		DrxLogAlways("%s%s: AABB min(%f, %f, %f) max(%f, %f, %f) size(%f, %f, %f)", Prefix(depth), name,
								value.min.x, value.min.y, value.min.z,
								value.max.x, value.max.y, value.max.z,
								size.x, size.y, size.z);
	}

	void Log(tukk name, IAIObject* pAI, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: IAIObject", Prefix(depth), name);

		Log("AIType", (u32)pAI->GetAIType(), depth);
...
// 		Log("VisionID", visionID, depth);
	...
	void Log(tukk name, IPipeUser* pPipeUser, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: IPipeUser", Prefix(depth), name);
	...
	void Log(tukk name, IPuppet* pPuppet, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: IPuppet", Prefix(depth), name);
	...
	void Log(tukk name, IAIActorProxy* pProxy, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: IAIActorProxy", Prefix(depth), name);
	...
	void Log(tukk name, IEntityArchetype* pArchetype, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: IEntityArchetype", Prefix(depth), name);

		Log("Name", pArchetype->GetName(), depth);
		Log("Class", pArchetype->GetClass(), depth);
	}

	void Log(tukk name, IEntityClass* pClass, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: IEntityClass", Prefix(depth), name);
	...
	void Log(tukk name, ICharacterInstance* pCharacter, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: ICharacterInstance", Prefix(depth), name);
		...
		if (ISkeletonAnim* pSkeletonAnim =  pCharacter->GetISkeletonAnim())
		{
			...
			for (i32 layerIndex = 0; layerIndex < 15; ++layerIndex)
			{
				i32 numAnims = pSkeletonAnim->GetNumAnimsInFIFO(layerIndex);
				if (numAnims > 0)
				{
					DrxLogAlways("%sFound %d animations in layer %d:", Prefix(depth), numAnims, layerIndex);
					for (i32 i = 0; i < numAnims; ++i)
					{
						const CAnimation& anim = pSkeletonAnim->GetAnimFromFIFO(layerIndex, i);

						tukk animName = (pAnimSet) ? pAnimSet->GetNameByAnimID(anim.GetAnimationId()) : "unknown";

						const float normalizedTime = pSkeletonAnim->GetAnimationNormalizedTime( &anim );

						DrxLogAlways("%s CAnimation[%d]: ID[%d] \"%s\" active: %d animTime: %f duration: %f loopCount: %d repeatCount: %d", Prefix(depth), i,
							anim.GetAnimationId(),
							animName,
	...
	void Log(tukk name, IAnimatedCharacter* pAnimCharacter, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: IAnimatedCharacter", Prefix(depth), name);
	...
	void Log(tukk name, IMovementController* pMovementController, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: IMovementController", Prefix(depth), name);

		if (const SExactPositioningTarget* pExactPositioningTarget = pMovementController->GetExactPositioningTarget())
		{
			DrxLogAlways("%s SExactPositioningTarget", Prefix(depth));
	...
	void Log(tukk name, IEntity* pEntity, u32 depth)
	{
		if (TooDeep(depth))
			return;

		DrxLogAlways("%s%s: IEntity", Prefix(depth), name);
...
void CFlowNode_FeatureTest::SetResult(bool result, tukk reason)
{
#if ENABLE_FEATURE_TESTER
	CFeatureTester* pFeatureTester = CFeatureTester::GetInstance();
	if (pFeatureTester)
	{
		...
		if (m_entitySeqIndex >= 0 && m_entitySeqIndex < SEQ_ENTITY_COUNT)
		{
			...
			if (pEnt)
			{
				...
				if (m_running)
				{
					...
					DrxLogAlways("Final position for %s: (%f, %f, %f). Translation since start: (%f, %f, %f)",
												pEnt->GetName(),
					...
					tukk owners = m_owners.empty() ? NULL : m_owners.c_str();

					ftMgr.OnTestResults(testSeqName.c_str(), desc.c_str(), (result) ? NULL : reason, m_timeRunning, owners);
				}
				else
				{
					DrxLogAlways("ERROR: Received result for a sequential test that hasn't yet started: %s", Name());
				}
			}
		}
		else	// Single test (not sequence)
		{
			...
			{
				DrxLogAlways("ERROR: Received result for a non-sequential test that hasn't yet started: %s", Name());
			}
		}
	}
#endif
	...
	for (i32 i = 0; i < SEQ_ENTITY_COUNT; ++i)
	{
		...
		if (pEnt)
		{
			...
			if (!activate && pEnt->IsFromPool())
			{
				...
				tukk entityName = pEnt->GetName();

				bool returnedToPool = pEntityPoolUpr->ReturnToPool(id, false);

				if (!returnedToPool)
					DrxLogAlways("Error: Test \"%s\" failed to return entity \"%s\" with id \'%u\' to the pool", Name(), entityName, id);
			}
			else
			{
				pEnt->Hide(!activate);
...
void CFlowNode_WatchCodeCheckpoint::GetConfiguration(SFlowNodeConfig& config)
{
	...
	static const SOutputPortConfig outputs[] = {
		OutputPortConfig<i32>("Count",		_HELP("Triggered on StopWatching input. Provides the number of hits since the last StartWatching request.")),
		...
		OutputPortConfig<i32>("TotalHits",		_HELP("Triggered on StopWatching input. Outputs the total number of hits for the checkpoint since program start.")),
		{0}
	};
...
void CFlowNode_WatchCodeCheckpoint::GetMemoryUsage(IDrxSizer* sizer) const
{
	sizer->Add(*this);
}
...
void CFlowNode_WatchCodeCheckpoint::StartWatching(SActivationInfo* pActInfo)
{
	...
	if(m_pCheckPoint)
		m_prevHitCount = static_cast<i32>(m_pCheckPoint->HitCount());
...
void CFlowNode_WatchCodeCheckpoint::StopWatching(SActivationInfo* pActInfo)
{
	...
	if(m_watchRequested)
	{
		...
		{
			i32 updatedHitcount = static_cast<i32>(m_pCheckPoint->HitCount());
			i32 difference = updatedHitcount - m_prevHitCount;

			ActivateOutput(pActInfo, eOutputPorts_RecentHits, static_cast<i32>(difference));
			...
			m_prevHitCount = static_cast<i32>(updatedHitcount);
		}
...
void CFlowNode_ListenForCommunication::GetMemoryUsage(IDrxSizer* sizer) const
{
	sizer->Add(*this);
}
...
void CFlowNode_ListenForCommunication::RegisterAsListener(SActivationInfo* pActInfo)
{
	...
	DRX_ASSERT(pCommunicationUpr);

	m_commId = pCommunicationUpr->GetCommunicationID(m_communicationName);
	...
	pCommunicationUpr->RegisterListener(this, "CommunicationFeatureTest");
...
void CFlowNode_ListenForCommunication::RemoveAsListener()
{
	...
	m_isListening = false;
	pCommunicationUpr->UnregisterListener(this);
}

void CFlowNode_ListenForCommunication::OnCommunicationEvent(ICommunicationUpr::ECommunicationEvent event, EntityId actorID, const CommID& playID)
{
	if( (m_entityId == 0 || actorID == m_entityId) && playID == m_commId)
	{
		switch(event)
		{
		case ICommunicationUpr::CommunicationFinished:
		...
		if (triggerResponse)
		{
			...
			if (pClientActor)
			{
				if (pClientActor->GetActorClass() == CPlayer::GetActorClassType())
				{
					...
					tukk action = GetPortString(pActInfo, eInputPorts_Action).c_str();
					...
					if (pPlayerInput)
					{
						...
						pPlayerInput->OnAction(CDrxName(action), playerInputEvent, isReleasePort ? 0.0f : clamp_tpl(inputValue, 0.0f, 1.0f));
					}
				}
			}
...
void CFlowNode_SimulateInput::GetMemoryUsage( IDrxSizer* sizer ) const
{
	sizer->Add(*this);
}
