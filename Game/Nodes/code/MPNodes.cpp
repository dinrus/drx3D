		...
		case eFE_Update:
			{
				...
				if(m_MDList.empty() && m_endGameNear)
				{
					...
					if(cancel)
					{
						...
						{
							DrxLog("MP flowgraph: EndGameInvalid");
						}
					...
					for(; it != m_MDList.end(); it = next)
					{
						...
						if(gEnv->pEntitySystem->GetEntity(*it))
						{
							if(!m_endGameNear)
							{
								if(g_pGame->GetCVars()->i_debug_mp_flowgraph != 0)
								{
									DrxLog("--MP flowgraph: EndGameNear");
								}
				...
				// also check whether the local player is in game yet
  				if(pPlayer->GetSpectatorMode() == 0 && m_localPlayerSpectatorMode != 0)
  				{
						if(g_pGame->GetCVars()->i_debug_mp_flowgraph != 0)
						{
							DrxLog("--MP flowgraph: EnteredGame");
						}
...
// 			}
	...
	i32 m_localPlayerSpectatorMode;
	bool m_endGameNear;
...
public:
	...
	void GetConfiguration( SFlowNodeConfig& config )
	{
		...
		if (!bInitialised)
		{
			...
			for (i32 i = 0; i < eGM_NUM_GAMEMODES; ++ i)
			{
				tukk pGameModeName = CGameRules::S_GetGameModeNamesArray()[i];
	...
	void TriggerGameTypeOutput()
	{
		...
		if (pGameRules)
		{
			ActivateOutput(&m_actInfo, (i32)pGameRules->GetGameMode() + EOP_SpecificModeStart, true);

			// output the name as well (for supporting any additional modes that might be added)
			tukk gameRulesName = pGameRules->GetEntity()->GetClass()->GetName();
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, EIP_Request))
				{
					...
					DrxHashStringId id(sequenceName);
					CGameRules* pGameRules = NULL;
					if( (gEnv->bMultiplayer || gEnv->IsEditor()) && id.id && (pGameRules = g_pGame->GetGameRules()) )
					{
						CMPTrackViewUpr* pMPTrackViewUpr = pGameRules->GetMPTrackViewUpr();

						IAnimSequence* pSequence = pMPTrackViewUpr->FindTrackviewSequence(id.id);
						if(pSequence)
						{
							IMovieSystem* pMovieSystem = gEnv->pMovieSystem;
							...
							if(gEnv->bServer)
							{
								pGameRules->GetMPTrackViewUpr()->AnimationRequested(params);
							}
							else
							{
				...
				else if (IsPortActive(pActInfo, EIP_Listen))
				{
					...
					if( (gEnv->bMultiplayer || gEnv->IsEditor()) && id.id && (pGameRules = g_pGame->GetGameRules()) )
					{
						...
						if(!gEnv->bServer)
						{
							if(pMPTrackViewUpr->HasTrackviewFinished(id))
							{
...
public:
	...
	~CMPAttachToPath_Node()
	{
		...
		{
			if( CMPPathFollowingUpr* pMPPathFollowingUpr = pGameRules->GetMPPathFollowingUpr() )
			{
				pMPPathFollowingUpr->UnregisterListener(m_EntityId);
			}
		}
	};
				...
				else if(IsPortActive(pActInfo, EIP_Speed))
				{
					if(gEnv->bMultiplayer && m_EntityId)
					{
						...
						if( pEntity && (pGameRules = g_pGame->GetGameRules()) )
						{
							...
							pMPPathFollowingUpr->RequestUpdateSpeed(classId, m_EntityId, speed);
						}
					}
				}
			}
		...
		case eFE_SetEntityId:
			{
				if(pActInfo->pEntity)
				{
					...
					{
						g_pGame->GetGameRules()->GetMPPathFollowingUpr()->UnregisterListener(m_EntityId);
	...
	{
		DRX_ASSERT_MESSAGE(attachedEntityId==m_EntityId, "CMPAttachToPath_Node::OnPathCompleted - Path listener listening to unexpected entity");

		CMPPathFollowingUpr* pMPPathFollowingUpr = g_pGame->GetGameRules()->GetMPPathFollowingUpr();
		pMPPathFollowingUpr->UnregisterListener(attachedEntityId);
		m_ListeningForFinishedCallback = false;
...
public:
	...
	float GetIntroSequenceDuration(i32 teamId)
	{
	...
	void OnDedicatedServerIntroStart()
	{
		...
		// DrxLog("OnDedicatedServerIntroStart() - Setting intro duration [%.3f]", longestSequenceDuration);
	...
	void SetupIntroStart(const EntityId cameraPoint, const float introDuration, tukk pIntroSound)
	{
		// For now don't support separate code path to position / lock player in editor (trackview can still be played manually to edit/test)
		if(gEnv->IsEditor())
		{
...
public:
	...
	virtual void GetConfiguration( SFlowNodeConfig& config )
	{
		...
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<i32>( "IntValue", _HELP("Current Int Value of the CVar [Int or Float]") ),
		...
		if (event == eFE_Activate)
		{
			...
			if (pICVar != 0)
			{
				...
				if(pICVar->GetType()==CVAR_FLOAT)
				{
					...
					ActivateOutput(pActInfo, INTVALUE, (i32)fval);
				}
			...
			{
				DrxLogAlways("[flow] Cannot find console variable '%s'", cvar.c_str());
			}
		}
	}
		...
		case eFE_Activate:
			{
				if(IsPortActive(pActInfo, EIP_Set))
				{
					if(gEnv->bMultiplayer)
					{
						...
						if (pPlaylistUpr)
						{
							pPlaylistUpr->SetGameModeOption(option.c_str(), val.c_str());
						}
					}
				...
				if(IsPortActive(pActInfo, EIP_Get))
				{
					if(gEnv->bMultiplayer)
					{
						...
						CPlaylistUpr *pPlaylistUpr = g_pGame->GetPlaylistUpr();
						DrxFixedStringT<32> out_val;
						if (pPlaylistUpr)
						{
							pPlaylistUpr->GetGameModeOption(option.c_str(), out_val);
