...
public:
	...
	tukk GetHitAttachmentName(const HitInfo& hit) const
	{
		...
		if (pPE->GetParams(&partParams) && pPE->GetStatus(&posStatus))
		{
			...
			i32 attachmentCount = pAttchmentUpr->GetAttachmentCount();

			for (i32 i = 0; i < attachmentCount; ++i)
			{
				IAttachment* pAttachment = pAttchmentUpr->GetInterfaceByIndex(i);
	...
	virtual void OnHit(const HitInfo& hitInfo)
	{
		...
		tukk hitType = 0;
		tukk surfaceType = 0;
		tukk attachmentName = GetHitAttachmentName(hitInfo);

		if (CGameRules* pGameRules = g_pGame->GetGameRules())
		{
			hitType = pGameRules->GetHitType(hitInfo.type);
			if (ISurfaceType* pSurface = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceType(hitInfo.material))
