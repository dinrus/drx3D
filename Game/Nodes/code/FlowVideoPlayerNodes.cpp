...
public:
	CFlowFlashVideoPlayerNode(SActivationInfo * pActInfo)
		...
		, m_bPlaying(false)
	{
		m_pActionMapMan = gEnv->pGame->GetIGameFramework()->GetIActionMapUpr();
	...
	virtual ~CFlowFlashVideoPlayerNode()
	{
		...
		if (gEnv->pGame && gEnv->pGame->GetIGameFramework() && gEnv->pGame->GetIGameFramework()->GetIActionMapUpr())
			gEnv->pGame->GetIGameFramework()->GetIActionMapUpr()->RemoveExtraActionListener(this);
	}
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			...
			InputPortConfig<i32>    ("InstanceID", -1, _HELP("Instance ID of USMPlayer (e.g. to use USMPlayer on dynamic textures)")),
			...
			InputPortConfig<i32>    ("AudioChannel", 0, _HELP("Audio channel")),
			InputPortConfig<i32>    ("SubtitleChannel", 0, _HELP("Subtitle channel")),
			{0}
		};
		...
		case eFE_Activate:
				if (IsPortActive(pActInfo, eI_Play) || (m_bPlaying && IsPortActive(pActInfo, eI_VideoFile)))
				{
					if (gEnv->pFlashUI && !m_pElement)
					{
						...
						i32 instanceId = GetPortInt(pActInfo, eI_InstanceID);
	...
	bool OnActionSkipVideo(EntityId entityId, const ActionId& actionId, i32 activationMode, float value)
	{
