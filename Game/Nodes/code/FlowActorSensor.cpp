...
public:
	...
	void GetConfiguration( SFlowNodeConfig& config )
	{
		...
		static const SOutputPortConfig out_ports[] = 
		{
			...
			OutputPortConfig<i32>      ( "SeatChange", _HELP("Triggered when seat has changed")),
			...
			OutputPortConfig<i32>      ( "StanceChanged", _HELP("Triggered when Stance changed. 0=Stand,1=Crouch,2=Prone,3=Relaxed,4=Stealth,5=Swim,6=ZeroG")),
			OutputPortConfig<i32>      ( "Jump", _HELP("Triggered On Jump")),
			OutputPortConfig<i32>      ( "Sprint", _HELP("Triggered on Sprint")),
			OutputPortConfig<i32>      ( "SpecialMove", _HELP("Triggered On SpecialMove. 0=Jump,1=SpeedSprint")),
			OutputPortConfig<i32>      ( "OnDeath", _HELP("Triggered when Actor dies. Outputs killer's entityId")),
			OutputPortConfig<i32>      ( "OnRevive", _HELP("Triggered when Actor revives. Outputs 0 if not god. 1 if god.")),
			OutputPortConfig<i32>      ( "OnEnterSpecator", _HELP("Triggered when Actor enter specator mode. Outputs spectator mode uint")),
			OutputPortConfig<bool>     ( "OnLeaveSpecator", _HELP("Triggered when Actor leaves specator mode.")),
			OutputPortConfig<i32>	     ( "OnHealthChange", _HELP("Triggered when Actors health changed. Outputs current health.")),
			...
			OutputPortConfig<i32>      ( "MaxHealth", _HELP("Triggered when Actors health changed or when get is triggered. Outputs max health.")),
	...
	void TriggerPorts(SActivationInfo *pActInfo)
	{
		...
		if (pPlayer)
		{
			ActivateOutput(pActInfo, EOP_STANCECHANGED, static_cast<i32> (pPlayer->GetStance()));
			...
			if (pPlayer->GetSpectatorMode() == CActor::eASM_None)
				...
				ActivateOutput(pActInfo, EOP_ONENTERSPECATOR, static_cast<i32> (pPlayer->GetSpectatorMode()));

			ActivateOutput(pActInfo, EOP_ONHEALTHCHANGE, (i32) pPlayer->GetHealth() );
			ActivateOutput(pActInfo, EOP_MAXHEALTH, (i32) pPlayer->GetMaxHealth() );
		}
	}
	...
	{
		ActivateOutput(&m_actInfo, EOP_STANCECHANGED, static_cast<i32> (stance));
	}
	virtual void OnSpecialMove(IActor* pActor, ESpecialMove move)
	{
		if (move == IPlayerEventListener::eSM_Jump)
		{
			ActivateOutput(&m_actInfo, EOP_JUMPED, static_cast<i32> (move));
		}
		else if (move == IPlayerEventListener::eSM_SpeedSprint)
		{
			ActivateOutput(&m_actInfo, EOP_SPRINTED, static_cast<i32> (move));
		}
		else
		{
			ActivateOutput(&m_actInfo, EOP_SPECIALMOVE, static_cast<i32> (move));
		}
	}
	...
	virtual void OnSpectatorModeChanged(IActor* pActor, u8 mode)
	{
		if (mode == CActor::eASM_None)
			...
			ActivateOutput(&m_actInfo, EOP_ONENTERSPECATOR, static_cast<i32> (mode));
	}
	...
	{
		ActivateOutput(&m_actInfo, EOP_ONHEALTHCHANGE, (i32) (fHealth));

		CPlayer* pPlayer = (CPlayer*)pActor;
		if(pPlayer)
			ActivateOutput(&m_actInfo, EOP_MAXHEALTH, (i32) pPlayer->GetMaxHealth() );
	}
...
public:
	...
	void GetConfiguration( SFlowNodeConfig& config )
	{
		...
		static const SOutputPortConfig outputs[] = {
			...
			OutputPortConfig<i32>     ( "AmmoCount",	_HELP( "Current ammo remaining in weapon (including chambered rounds)" ) ),
			OutputPortConfig<i32>     ( "ClipSize",	_HELP( "Amount of ammo the weapon clip can hold" ) ),
			OutputPortConfig<i32>     ( "AmmoPool",	_HELP( "Current amount of ammo remaining in player inventory" ) ),

			OutputPortConfig<float>   ( "Spread",	_HELP( "Spread of current weapon" ) ),

			OutputPortConfig<bool>    ( "Zoomed",	_HELP( "Is weapon zoomed" ) ),
			OutputPortConfig<i32>     ( "ZoomMode",	_HELP( "Weapon zoom mode number, as ordered in weapon script" ) ),
			OutputPortConfig<string>  ( "ZoomName",	_HELP( "Weapon zoom mode name" ) ),
			OutputPortConfig<i32>     ( "CurrZoomStep",	_HELP( "Current zoom step, defined as Stages in ZoomMode params" ) ),
			OutputPortConfig<i32>     ( "MaxZoomStep",	_HELP( "Maximum zoom steps available" ) ),
			{0}
		};
	...
	// ~IInventoryListener
	virtual void OnSetAmmoCount(IEntityClass* pAmmoType, i32 count) 
	{
		CWeapon* pWeapon = GetCurrentWeapon();
	...
	virtual void OnZoomChanged(IWeapon* pWeapon, bool zoomed, i32 idx)
	{
		UpdateWeaponChange(GetCurrentWeapon());
...
private:
	...
	{
		i32 iFireMode = pWeapon ? pWeapon->GetCurrentFireMode() : -1;
		IFireMode* pFireMode = pWeapon ? pWeapon->GetFireMode(iFireMode) : NULL;
		i32 iZoomMode = pWeapon ? pWeapon->GetCurrentZoomMode() : -1;
...
public:
	...
	void ProcessEvent( EFlowEvent event, SActivationInfo *pActInfo )
	{
		...
		{
			i32k level = g_pGameCVars->g_difficultyLevel - 1; // [0] or 1-4
...
public:
	...
	// IPlayerEventListener
	void OnPickedUpPickableAmmo(IActor* pActor, IEntityClass* pAmmoType, i32 count)
	{
		if (m_enabled)
		{
...
public:
	...
	void GetConfiguration( SFlowNodeConfig& config )
	{
		...
		static const SOutputPortConfig out_ports[] = 
		{
			...
			OutputPortConfig<i32>  ( "FalloffX", _HELP("-1 if entity is left from viewfrustrum, 0 if inside, 1 if right") ),
			OutputPortConfig<i32>  ( "FalloffY", _HELP("-1 if entity is above the viewfrustrum, 0 if inside, 1 if beneath") ),
			{0}
		};
		config.nFlags |= EFLN_TARGET_ENTITY;
		config.pInputPorts = in_ports;
	...
	virtual void OnPostUpdate(float fDelta)
	{
		if ( IEntity* pEntity = gEnv->pEntitySystem->GetEntity(m_entityId) )
		{
			...
			screenPos.x *= 0.01f;
			screenPos.y *= 0.01f;
			i32 falloffx = screenPos.x < 0 ? -1 : screenPos.x > 1 ? 1 : screenPos.z < 1 ? 0 : -1;
			i32 falloffy = screenPos.y < 0 ? -1 : screenPos.y > 1 ? 1 : screenPos.z < 1 ? 0 : -1;
