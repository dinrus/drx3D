...
/*************************************************************************
 -------------------------------------------------------------------------
  $Id$
  $DateTime$
  Описание: Flow node to check if an entity is inside an area
...
public:
	...
	void OnEntityEvent(IEntity *pEntity, SEntityEvent &event)
	{
		switch (event.event)
		{
			...
			default:
				DRX_ASSERT_MESSAGE(false, "CFlowNode_IsInArea::OnEntityEvent Received unhandled event");
				break;
		}
	}
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo *pActInfo)
	{
		...
		else if (eFE_SetEntityId == event)
		{
			...
			else
			{
				...
				if (gEnv->pEntitySystem->GetIEntityPoolUpr()->IsEntityBookmarked(graphEntityId))
