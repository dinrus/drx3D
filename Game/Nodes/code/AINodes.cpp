...
#include <drx3D/Game/AI/AIBattleFront.h>
...
void CFlowNode_BattleFrontControl::GetConfiguration( SFlowNodeConfig& config )
{
	...
	{
		InputPortConfig<i32>("Group", 0, _HELP("Target battle front group id.")),
...
void CFlowNode_BattleFrontControl::ProcessEvent( EFlowEvent event, SActivationInfo *activationInformation )
{
	...
	{
		i32	groupID = GetPortInt(activationInformation, EIP_Group);	
...
void CFlowNode_BattleFrontControl::GetMemoryUsage( IDrxSizer * sizer ) const
{
	sizer->Add(*this);
}
...
{
	tukk entityClassName = entity.GetClass()->GetName();
...
void CFlowNode_SetTurretFaction::SetFaction(SActivationInfo* pActInfo)
{
	if (IEntity* entity = pActInfo->pEntity)
	{
		...
		{
			u8 factionID = gEnv->pAISystem->GetFactionMap().GetFactionID(GetPortString(pActInfo, InputPort_Faction));
...
public:
	...
	{
		i32 val = m_lastColourPortTriggered;
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig outputs[] = {
			OutputPortConfig<i32>("Awareness", _HELP("0-100")),
	...
	void AwarenessChanged( i32 value )
	{
		UpdateOutputs( value );
	}
...
public:
	...
	void AlertnessChanged( i32 alertnessGlobal )
	{
		UpdateOutputs( alertnessGlobal );
	}

	void UpdateOutputs( i32 alertnessGlobal )
	{
		...
		u32 colorPort = clamp_tpl( alertness+OUT_GREEN, (i32)OUT_GREEN, (i32)OUT_RED );
...
void CFlowNode_AICorpses::ProcessEvent( EFlowEvent event, SActivationInfo* pActInfo )
{
	if(event == eFE_Activate)
	{
		if(IsPortActive(pActInfo, InputPort_CleanCorpses))
		{
			...
			{
				pAICorpseUpr->RemoveAllCorpses( "FlowNode_AICorpses" );
			}
		}
	}
}

void CFlowNode_AICorpses::GetMemoryUsage( IDrxSizer * sizer ) const
{
	sizer->Add(*this);
}
