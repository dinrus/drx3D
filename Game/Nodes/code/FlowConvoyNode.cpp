...
#include <drx3D/Game/FlowConvoyNode.h>
...
{
	i32 num_path_nodes = path.size();
	m_path.resize(num_path_nodes);
	m_totalLength = 0.0f;
	for (i32 i=0; i<num_path_nodes; i++)
	{
...
i32 CFlowConvoyNode::OnPhysicsPostStep_static(const EventPhys * pEvent)
{
...
void CFlowConvoyNode::GetConfiguration( SFlowNodeConfig &config )
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig<i32>( "LoopCount",0,_HELP("How many times to loop along the path (-1 for infinite)") ),
		...
		InputPortConfig<i32>( "SplitCoachNumber",_HELP("If acticated, the train splits from this coach") ),
	...
	static const SOutputPortConfig out_config[] = {
		...
		OutputPortConfig<i32> ("PlayerCoachIndex", _HELP("gives the coach index Player is standing on, (-1) if not on train for at least 4 seconds")),
		{0}
	};
...
i32 CFlowConvoyNode::OnPhysicsPostStep(const EventPhys * pEvent)
{
  ...
  if(m_bFirstUpdate)
	...
	for (size_t i = 0; i < m_coaches.size(); ++i)
	{
		...
		if (pEventPhysEnt == pPhysEnt)
		{
			...
			float speed = (m_splitCoachIndex > 0 && (i32)i >= m_splitCoachIndex) ? m_splitSpeed : m_speed;
...
void CFlowConvoyNode::DiscoverConvoyCoaches(IEntity *pEntity)
{
  ...
  //  pEntity->SetConvoyEntity();
		//for (i32 i = 0; i < pEntity->GetChildCount(); i++)
...
void CFlowConvoyNode::InitConvoyCoaches()
{
	...
	for (i32 i=m_coaches.size()-1; i>=0; --i)
	{
...
i32 CFlowConvoyNode::GetCoachIndexPlayerIsOn()
{
	...
	for (size_t i = 0; i < m_coaches.size(); ++i)
	{
		...
		{//attached objects
			...
			for (i32 j = 0; j < pCoachEntity->GetChildCount(); ++j)
			{
...
i32 CFlowConvoyNode::GetCoachIndexPlayerIsOn2()
{
	IActor *pPlayerActor = gEnv->pGame->GetIGameFramework()->GetClientActor();
...
void CFlowConvoyNode::StartBreakSoundShifted()
{
	...
	//	i32 index=m_startBreakSoundShifted/SHIFT_FRAME;
...
void CFlowConvoyNode::ProcessEvent( EFlowEvent event, SActivationInfo *pActInfo )
{
	...
	case eFE_Activate:
		{
			...
			else if(IsPortActive(pActInfo, IN_SPLIT_COACH))
			{
				...
				{
					i32 splitCoachIndex=GetPortInt(pActInfo, IN_SPLIT_COACH);
					if(splitCoachIndex > 0 && splitCoachIndex < (i32)m_coaches.size())
					{
					...
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "SplitCoachIndex has been set! Do not set it again! FlowConvoyNode: %s", m_coaches[0].m_pEntity->GetName());
			}
	...
	case eFE_Initialize:
		...
		temp.resize(1000);
		u32 count = gEnv->pAISystem->GetINavigation()->GetPath(GetPortString(pActInfo, IN_PATH), &temp[0], 1000);
		temp.resize(count);
...
void CFlowConvoyNode::Update(SActivationInfo *pActInfo)
{
	...
	//i32 coachIndex=GetCoachIndexPlayerIsOn();
	i32 coachIndex=GetCoachIndexPlayerIsOn2();
 ...
 float CFlowConvoyNode::GetSpeedSoundParam(i32 coachIndex)
{
	float speedParam;
...
{
	//for (i32 i=0; i<m_coaches.size(); ++i)
	//{
	...
	//		//else
	//			//DrxLogAlways("No ISound:%d", i);
	//	}
	//}
}

//tSoundID CFlowConvoyNode::PlayLineSound(i32 coachIndex, tukk sGroupAndSoundName, const Vec3 &vStart, const Vec3 &vEnd)
//{
...
void CFlowConvoyNode::GetMemoryUsage(IDrxSizer * s) const
{
	s->Add(*this);
}
