...
$Id$
$DateTime$
Описание:  Flow nodes for working with XBox 360 controller
-------------------------------------------------------------------------
История:
...
public:
	...
	float GetDuration(i32 iA, i32 iB)
	{
	...
	void Update()
	{
		...
		for (i32 i = 0; i < (i32)m_stickHistory.size(); ++i)
		{
			if (m_iMinStartX >= 0)
			{
		...
		i32 iYes = m_iFirstEndDeadY;
		i32 iNo = m_iFirstEndDeadX;
		bool bYes = (iYes >= 0);
		bool bNo = (iNo >= 0);
...
private:
	...
	i32 m_iLastStartDeadX;
	i32 m_iLastStartDeadY;
	i32 m_iFirstEndDeadX;
	i32 m_iFirstEndDeadY;
	i32 m_iMinStartX;
	i32 m_iMinEndX;
	i32 m_iMaxStartX;
	i32 m_iMaxEndX;
