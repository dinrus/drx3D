...
#include <drx3D/Game/ModelToHudFlowNodes.h>
	...
	case eFE_Activate:
		...
		if (IsPortActive(pActInfo, IN_START))
		{
			...
			if(pUIElement)
				...
				DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "CFlowAdd3DModelToHUD: Movieclip not found");
...
//--------------------------------------------------------------------------------------------
void CFlowSetupModelPostRender::GetMemoryUsage(IDrxSizer * s) const
{
	s->Add(*this);
}
	...
	case eFE_Activate:
		if (IsPortActive(pActInfo, IN_ADD))
		{
			...
			if (renderModels)
			{
				...
				tukk  usePlayerModelName = GetPortString(pActInfo, IN_MODEL);
				tukk  useAnimation = GetPortString(pActInfo, IN_ANIM);
...
//--------------------------------------------------------------------------------------------
void CFlowAddModelToPostRender::GetMemoryUsage(IDrxSizer * s) const
{
	s->Add(*this);
}
