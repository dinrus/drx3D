...
// -------------------------------------------------------------------------
//  Имя файла:   FlowHudEventsNodes.cpp
//  Version:     v1.00
...
#define HUDEVENT_UICONFIG_ENTRY_FIRST(x) string("enum_int:"#x"=").append(ToString((i32)x))
#define HUDEVENT_UICONFIG_ENTRY_ADDITIONAL(x) .append(string(","#x"=").append(ToString((i32)x)))
...
public:
	...
	virtual void GetConfiguration( SFlowNodeConfig& config )
	{
		...
		static const SOutputPortConfig out_ports[] = 
		{
			...
			OutputPortConfig<i32>("Status", "0=deactivated, 1=completed, 2=failed, 3=activated"),
	...
	virtual void OnHUDEvent(const SHUDEvent& event)
	{
		...
		{
			for(u32 i = 0; i < event.GetDataSize(); i++)
			{
				...
				default:
					DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "[CFlowNode_MissionStateListener] HudEvent data unknown.");
					break;
				}
			}
			if(pMissionObjective && !bIsSilent)
			{
				...
				ActivateOutput<i32>(&m_pActInfo, eOP_State, pMissionObjective->GetStatus());
...
public:
	...
	//////////////////////////////////////////////////////////////////////////
	inline void splitStringList(std::vector<string>* result, tukk str, char delimeter)
	{
		result->clear();
	...
	//////////////////////////////////////////////////////////////////////////
	bool IsClassAllowed(tukk entClass)
	{
		tukk classFilter = GetPortString(&m_pActInfo, eIP_Class).c_str();
		...
		else if(strcmp(classFilter, "CustomClasses") == 0)
		{
			...
			for (u32 i = 0; i < CustomClassList.size(); i++)
			{
	...
	virtual void OnHUDEvent(const SHUDEvent& event)
	{
		// Only add/remove entity is handled here, so we only care about the i32 param
		EntityId entityId = 0;
		...
		if(event.eventType == eHUDEvent_AddEntity || event.eventType == eHUDEvent_RemoveEntity)
		{
			for(u32 i = 0; i < event.GetDataSize(); i++)
			{
				...
				case SHUDEventData::eSEDT_undef:
				default:
					DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "[CFlowNode_EntityTrackedListener] HudEvent data unknown.");
					break;
				}
			}
			if(entityId != 0)
...
public:
	...
	virtual void GetConfiguration( SFlowNodeConfig& config )
	{
		...
		static const SInputPortConfig in_ports[] = 
		{
			InputPortConfig<i32>("Event", 2, _HELP(""), "Event", uiconfig.c_str()),
			{0}
		};
	...
	virtual void OnHUDEvent(const SHUDEvent& event)
	{
		i32 eventID = GetPortInt(&m_pActInfo, 0);
		if(event.eventType == eventID)
		{
			...
			for(u32 i = 0; i < event.GetDataSize(); i++)
			{
				...
				case SHUDEventData::eSEDT_undef:
				default:
					DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "[CFlowNode_BattleAreaListener] HudEvent data unknown.");
					break;
				}
			}
		}
