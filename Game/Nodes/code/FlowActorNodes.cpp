...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			...
			InputPortConfig<i32>("Layer", _HELP("Layer to play that sequence")),
		...
		case eFE_Activate:
			if (IsPortActive(pActInfo, eIP_Start) && pActInfo->pEntity)
			{
				if (ICharacterInstance* pCharacter = pActInfo->pEntity->GetCharacter(0))
				{
					...
					{
						tukk sSequence = GetPortString(pActInfo, eIP_Sequence).c_str();
						if (IFacialAnimSequence* pSequence = pInstance->LoadSequence(sSequence))
						{
							i32 iLayer = GetPortInt(pActInfo, eIP_Layer);
...
private:
	...
	CPlayer* GetLocalPlayer() const
	{
		...
		if (pActor)
		{
			DRX_ASSERT(pActor->GetActorClass() == CPlayer::GetActorClassType());
		...
		case eFE_Initialize:
			{
				...
				m_pPlayerModel = gEnv->pCharacterUpr->CreateInstance(modelName.c_str());
			}
			break;
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eIN_Set))
				{
					...
					if (pLocalActor != NULL && pLocalActor->GetEntity())
					{
						...
						{
							DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Actor:SetPlayerModel:Character %s not valid",modelName.c_str());
...
public:
	...
	virtual void GetConfiguration( SFlowNodeConfig &config )
	{
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<i32>("Faction", _HELP("Faction of the nearest actor")),
		...
		static const SInputPortConfig inp_config[] = {
			...
			InputPortConfig<i32>("FactionFilterType", 0, _HELP("Defines how the 'Faction' input is used for filtering."), 0, _UICONFIG("enum_int:(0)No_faction_check=0,(1)Only_that_faction=1,(2)All_but_that_faction=2,(3)Any_friendly=3,(4)Any_Hostile=4")),
			InputPortConfig<i32>("DeadAliveFilter", 0, _HELP("Filters actors by death/alive status."), 0, _UICONFIG("enum_int:(0)Any,(1)Alive=1,(2)Dead=2")),
			{0}
		};
	...
	void CheckNearestActor( SActivationInfo* pActInfo )
	{
		...
		u8 factionID = factionMap.GetFactionID( GetPortString(pActInfo, eINP_Faction ));
		...
		while (IActor *pActor=actorIt->Next())
		{
			if (pActor)
			{
				...
				if (pAIObject)
				{
					u8 actorFactionID = pAIObject->GetFactionID();
					bool bValid = true;
		...
		if (pNearestActor)
		{
			...
			ActivateOutput( pActInfo, eOUT_Faction, i32(pNearestActor->GetEntity()->GetAI()->GetFactionID()) );
    ...
    case eFE_Activate:
      {
        ...
        if (IsPortActive(pActInfo, EIP_Ragdollize))
        {
          if(pActor->CanRagDollize())
          {
            ...
            Vec3 impulseValue = drx_random_componentwise(
              GetPortVec3(pActInfo, EIP_ImpulseMin),
              GetPortVec3(pActInfo, EIP_ImpulseMax));
            Vec3 angImpulseValue = drx_random_componentwise(
