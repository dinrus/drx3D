...
public:
	...
	tukk GetConnectorTypeLinkName(i32 flow)
	{
		if (flow == 0)
			flow = m_flow;
	...
	void MarkElectricConnector(EntityId eid)
	{
		...
		tukk tag = GetConnectorTypeLinkName(0);
		if (tag)
	...
	void OnEntityEvent( IEntity *pEntity,SEntityEvent &event )
	{
		...
		if(event.event == ENTITY_EVENT_ACTIVATE_FLOW_NODE_OUTPUT)
		{
			...
			for(i32 i = 0; i < eOP_Last; i++)
			{
				if(strcmp(config.pOutputPorts[i].name, (tukk)event.nParam[0]) == 0)
				{
