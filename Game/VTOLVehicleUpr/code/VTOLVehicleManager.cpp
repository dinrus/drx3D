...
#include <StdAfx.h>
#include <drx3D/Game/VTOLVehicleUpr.h>
...
tukk  CVTOLVehicleUpr::s_explosionEffectName = "vehicles.MH60_BlackHawk.Damage.Destroyed";
tukk  CVTOLVehicleUpr::s_VTOLClassName		= "US_VTOL_GunShip";

CVTOLVehicleUpr::CVTOLVehicleUpr() :	
...
void CVTOLVehicleUpr::Init()
{
	CGameAudio *pGameAudio = g_pGame->GetGameAudio();

	m_explosionSignalId = pGameAudio->GetSignalID("ExplosionGenericLarge");
	//DRX_ASSERT(m_explosionSignalId != INVALID_AUDIOSIGNAL_ID);

	m_pExplosionEffect = gEnv->pParticleUpr->FindEffect(s_explosionEffectName);
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to find explosion effect %s", s_explosionEffectName);
	}
...
void CVTOLVehicleUpr::InitClient(i32 channelID)
{
	i32 numVehicles=0;
	for(TVTOLList::const_iterator iter = m_vtolList.begin(), end = m_vtolList.end(); iter!=end; ++iter )
	{
		numVehicles++;
		DRX_ASSERT_MESSAGE(numVehicles <= 1, "We only currently support handling a single VTOL here");
		...
		if (info.state == EVS_Invisible)
		{
			...
			DrxLog("CVTOLVehicleUpr::OnClientEnteredGame() has found the VTOL is invisible, telling clients to hide it");
...
void CVTOLVehicleUpr::Update(float frameTime)
{
		...
		for (TVTOLList::iterator iter=m_vtolList.begin(), end=m_vtolList.end(); iter!=end; ++iter)
		{
			...
			if(pVehicle)
			{
				...
				if(info.state==EVS_Normal)
				{
					...
					{
						u8k pathComplete = pMovement->GetPathComplete();
						if(pathComplete==1)
						{
							if(CMPPathFollowingUpr* pMPPathFollowingUpr = g_pGame->GetGameRules()->GetMPPathFollowingUpr())
							{
								pMPPathFollowingUpr->NotifyListenersOfPathCompletion(info.entityId);
							}
				...
				i32 currentId;
				for(u32 prev = 0; prev < currentPlayerList.size(); ++prev)
				{
				...
				for(u32 old = 0; old < oldPlayerList.size(); ++old) 
				{
...
void CVTOLVehicleUpr::UpdateEntityInVTOL( SVTOLInfo& vtolInfo, EntityId entityId )
{
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
bool CVTOLVehicleUpr::TestIsInVTOL( const SVTOLInfo& info, const AABB& aabb ) const
{
	OBB obb;
...
void CVTOLVehicleUpr::UpdateRotationOfInternalPlayers(SVTOLInfo& info, const TPlayerStatusList& playerStatuses, const Quat& rotationDifference)
{
	...
	if(i32k numPlayers = playerStatuses.size())
	{
...
void CVTOLVehicleUpr::OnPlayerEntered(EntityId playerId)
{
...
void CVTOLVehicleUpr::OnPlayerExited(EntityId playerId)
{
	IEntity* pEntity = gEnv->pEntitySystem->GetEntity(playerId);
	if(pEntity)
	{
...
CPlayer* CVTOLVehicleUpr::GetPlayerFromEntityId(const EntityId entityId) const
{
	if(IActorSystem* pActorSystem = g_pGame->GetIGameFramework()->GetIActorSystem())
	{
...
void CVTOLVehicleUpr::RegisterVTOL(EntityId entityId)
{
...
void CVTOLVehicleUpr::RespawnVTOL(IVehicle* pVehicle, SVTOLInfo& info)
{
	DrxLog("CVTOLVehicleUpr::RespawnVTOL()");

	//Reset parts and component damage
	i32 numComponents = pVehicle->GetComponentCount();
	for(i32 c = 0; c < numComponents; ++c)
	{
		IVehicleComponent* pComponent = pVehicle->GetComponent(c);
		i32 numParts = pComponent->GetPartCount();
	...
	hudEvent.AddData((i32)info.entityId);
	CHUDEventDispatcher::CallEvent(hudEvent);
}

void CVTOLVehicleUpr::OnEntityEvent( IEntity *pEntity, SEntityEvent &event )
{
	...
	else if(event.event == ENTITY_EVENT_HIDE)
	{
		...
		hudEvent.AddData((i32)pEntity->GetId());
		CHUDEventDispatcher::CallEvent(hudEvent);
	}
...
void CVTOLVehicleUpr::CalculateBounds(IEntity* pVTOLEntity, SVTOLInfo& info)
{
	// Work out final local bounds - includes all children's bounds
	AABB aabb;
	pVTOLEntity->GetLocalBounds(aabb);

	i32k numChildren = pVTOLEntity->GetChildCount();
	AABB childAABB;
	for(i32 child = 0; child < numChildren; ++child)
	{
...
void CVTOLVehicleUpr::LockSeats(IVehicle* pVehicle, bool lock)
{
	EVehicleSeatLockStatus newStatus = lock ? eVSLS_LockedForPlayers : eVSLS_Unlocked;

	i32k numSeats = pVehicle->GetSeatCount();
	for(i32 i = 1; i <= numSeats; ++i)
	{
...
void CVTOLVehicleUpr::CreateExplosion(IParticleEffect *inParticleEffect, const Vec3& pos, const float inEffectScale, TAudioSignalID inAudioSignal/*=INVALID_AUDIOSIGNAL_ID*/)
{
#if !defined(DEDICATED_SERVER)
	DRX_ASSERT_MESSAGE(inParticleEffect, "CreateExplosion() passsed a NULL inParticleEffect");
#endif
	...
	if (gEnv->bServer)
	{
		...
		explosionInfo.effect_scale = drx_random(0.9f,1.1f);
...
void CVTOLVehicleUpr::DestroyVTOL(IEntity *pVTOL, SVTOLInfo &info)
{
	...
	//Remove from HUD radar
	SHUDEvent hudEvent(eHUDEvent_RemoveEntity);
	hudEvent.AddData((i32)pVTOL->GetId());
	CHUDEventDispatcher::CallEvent(hudEvent);
}

void CVTOLVehicleUpr::DestructionDamageRatioReached(IVehicle* pVehicle, SVTOLInfo& info, float frameTime)
{
...
void CVTOLVehicleUpr::OnAttachRequest( const SPathFollowingAttachToPathParameters& params, const CWaypointPath* pPath )
{
	...
	{
		DRX_ASSERT_MESSAGE(false, "CVTOLVehicleUpr::OnAttachRequest - VTOL doesn't exist. And it really, really should.");
	}
}

void CVTOLVehicleUpr::OnUpdateSpeedRequest( EntityId entityId, float speed )
{
...
void CVTOLVehicleUpr::OnSingleEntityRMI(CGameRules::SModuleRMIEntityParams params)
{
	switch(params.m_data)
	{
		case eRMITypeSingleEntity_vtol_destroyed:
		{
			DrxLog("CVTOLVehicleUpr::OnSingleEntityRMI() eRMITypeSingleEntity_vtol_destroyed");
			IVehicle* pVehicle = m_pVehicleSystem->GetVehicle( params.m_entityId );
			DRX_ASSERT_MESSAGE(pVehicle, "have received destroyed VTOL RMI, but cannot find the vehicle for specified entity id");
			if (pVehicle)
			{
		...
		case eRMITypeSingleEntity_vtol_hidden:				// for late joining clients only
		{
			...
			DrxLog("CVTOLVehicleUpr::OnSingleEntityRMI() eRMITypeSingleEntity_vtol_hidden");
			IVehicle* pVehicle = m_pVehicleSystem->GetVehicle(params.m_entityId);
			DRX_ASSERT_MESSAGE(pVehicle, "have received hidden VTOL RMI, but cannot find the vehicle for specified entity id");
			if (pVehicle)
			{		
				//Hide existing vehicle
				...
				SHUDEvent hudEvent(eHUDEvent_RemoveEntity);
				hudEvent.AddData((i32)params.m_entityId);
				CHUDEventDispatcher::CallEvent(hudEvent);
			}
			break;
		}
		...
		{
			DrxLog("CVTOLVehicleUpr::OnSingleEntityRMI() eRMITypeSingleEntity_vtol_respawned");
			IVehicle* pVehicle = m_pVehicleSystem->GetVehicle(params.m_entityId);
			DRX_ASSERT_MESSAGE(pVehicle, "have received respawned VTOL RMI, but cannot find the vehicle for specified entity id");
			if (pVehicle)
			{
		...
		default:
			DRX_ASSERT_MESSAGE(0, string().Format("unhandled RMI data %d", params.m_data));
			break;
	}
}

void CVTOLVehicleUpr::RegisterVTOLWithPathFollower(bool registerVTOL)
{
	if(!registerVTOL || !m_bRegisteredWithPathFollower)
	{
		...
		if(pGameRules)
		{
			CMPPathFollowingUpr* pPathFollowingUpr = pGameRules->GetMPPathFollowingUpr();
			if(pPathFollowingUpr)
			{
				if(registerVTOL)
				{
					...
					pPathFollowingUpr->RegisterClassFollower(m_classId, this);
					m_bRegisteredWithPathFollower = true;
				}
				else
				{
					pPathFollowingUpr->UnregisterClassFollower(m_classId);
					m_bRegisteredWithPathFollower = false;
				}
			}
		}	
...
void CVTOLVehicleUpr::SetupMovement(EntityId entityId)
{
...
void CVTOLVehicleUpr::ResetVehicle(SVTOLInfo& info)
{
	info.Reset();
	SetupMovement(info.entityId);
}

bool CVTOLVehicleUpr::IsVTOL( EntityId entityId )
{
	return m_vtolList.count(entityId)!=0;
}

void CVTOLVehicleUpr::OnOwnClientEnteredGame()
{
	for(TVTOLList::const_iterator iter=m_vtolList.begin(), end=m_vtolList.end(); iter!=end; ++iter)
	{
...
bool CVTOLVehicleUpr::IsPlayerInVTOL( EntityId entityId ) const
{
	for(TVTOLList::const_iterator iter=m_vtolList.begin(), end=m_vtolList.end(); iter!=end; ++iter)
	{
		const SVTOLInfo& info = iter->second; 
...
bool CVTOLVehicleUpr::TestEntityInVTOL( const IEntity& entity ) const
{
	AABB aabb;
...
bool CVTOLVehicleUpr::AnyEnemiesInside( EntityId friendlyPlayerId ) const
{
...
void CVTOLVehicleUpr::SVTOLInfo::Reset()
{
	state = EVS_Normal;
	stateTime = 0.f;
	playersInside.clear();
