...
#include <drx3D/Game/IntersectionAssistanceUnit.h>
...
public:
	...
	tukk GetClassificationName() const;
...
private:
	...
	{
		EntityHitRecord(EntityId entityId, const Vec3& normal, i32 surfaceIdx, i32 partId ,const Vec3& wSpaceHitLoc)
		{
			m_entityId		     = entityId;
	...
	struct SWeaponTrackSlice
	{
			...
			, m_bVisible(visible)
		{
			...
			for(i32 i=0; i<k_numTrackPoints; i++)
			{
				m_fSteps[i] = currStep;
				currStep += stepScaled;
			}
		...
		static i32k k_numTrackPoints = 3;
	...
	void GenerateHitInfo(EntityId attackerEntityId, float damage,  HitInfo& hitInfo, const EntityHitRecord& entityHitRecord, const Vec3& nImpactDir, i32 hitType ) const;
	...
	void GenerateMaterialEffects(const Vec3& pos, i32k surfaceIndex);
	void GenerateArtificialCollision(IPhysicalEntity* pAttackerPhysEnt, IPhysicalEntity *pCollider, i32 colliderMatId, const Vec3 &position, const Vec3& normal, const Vec3 &vel, i32 partId, i32 surfaceIdx, i32 iPrim, const Vec3& impulse );

	// Event Handlers
	void HandleMeleeCollisionEvent(const EventPhysCollision	*pEventPhysCollision, const bool bAllowActors);
	void HandleMeleeCollisionEvent_Actor(const EventPhysCollision *pEventPhysCollision, EntityId victimEntityId, CPlayer* pOwnerPlayer, i32 victimIdx, IPhysicalEntity* pVictimPhysicalEntity);
	void HandleMeleeCollisionEvent_Geometry(const EventPhysCollision *pEventPhysCollision, CPlayer* pOwnerPlayer, i32 victimIdx);
	void HandleMeleeCollisionEvent_BreakableGeometry(const EventPhysCollision *pEventPhysCollision, CPlayer* pOwnerPlayer, i32 victimIdx, IPhysicalEntity* pVictimPhysicalEntity);
	void HandleMeleeCollisionEvent_Object(const EventPhysCollision *pEventPhysCollision, IEntity* pVictimEntity, CPlayer* pOwnerPlayer, i32 victimIdx, IPhysicalEntity* pVictimPhysicalEntity);
	...
	// Listeners
	const static i32 s_maxListeners = 12;
	typedef  DrxFixedArray<IEnvironmentalWeaponEventListener*, s_maxListeners> TListenerList;
	...
	const static i32 s_MaxEntitiesAllowedHitsPerSwing = 8;
	typedef  DrxFixedArray<EntityId, s_MaxEntitiesAllowedHitsPerSwing> TEntityHitList;
	...
	typedef u16 TLogicFlags;
	typedef u8 TAttackState;
