...
#define FeatureTesterLog(...)       DrxLog("$3[FEATURETESTER]$1 %s%s", CFeatureTester::GetContextString().c_str(), string().Format(__VA_ARGS__).c_str())
#define FeatureTesterWarning(...)   DrxLog("$3[FEATURETESTER] <WARN> %s%s", CFeatureTester::GetContextString().c_str(), string().Format(__VA_ARGS__).c_str())
...
#define MAKE_FUNCTIONS(enumVal, theType, isValid, strFormat, ...)                          \
	...
	{                                                                                        \
		...
		{																									                                     \
		  string contents;                                                                     \
			DRX_ASSERT_TRACE(0, ("Expected next item to be a '%s' but it's a '%s' value=%s",     \
				s_featureTestValTypeNames[enumVal], s_featureTestValTypeNames[m_type],             \
				GetContentsAsString(contents)));                                                   \
		}                                                                                      \
		else if (! (isValid))                                                                  \
		{																									                                     \
			DRX_ASSERT_TRACE(0, ("Next item is right type (%s) but value is invalid!",           \
...
struct SFeatureTestInstructionOrParam
{
	...
	tukk  GetContentsAsString(string & strOutput) const
	{
		switch (m_type)
		{
...
struct SFeatureTestDataLoadWorkspace
{
	...
	ILINE i32 GetNumInstructions()                                             { return m_count;             }
	...
	ILINE void RewindNumInstructions(i32 num)
	{
		assert (num >= 0);
		assert (num <= m_count);
	...
	void SetNameOfTestBeingLoaded(tukk  newName)
	{
...
{
	virtual i32 GetCount() const { return CFeatureTester::s_instance->m_numTests; }
	virtual tukk GetValue( i32 nIndex ) const { return CFeatureTester::s_instance->m_featureTestArray[nIndex].m_testName; }
};
...
class CFeatureTestFilenameAutoComplete : public IConsoleArgumentAutoComplete
{
	...
	{
		i32 curFrame = gEnv->pRenderer->GetFrameID(false);
	...
	virtual tukk GetValue( i32 nIndex ) const { return m_filenames[nIndex].c_str(); }

	void FindAllFiles(i32 curFrame)
	{
		...
		if (handle > -1)
		{
			...
			{
				char buffer[64];
				size_t charsCopied = drx_copyStringUntilFindChar(buffer, fd.name, sizeof(buffer), '.');
...
string CFeatureTester::GetContextString()
{
	...
	else
	{
		...
		if (s_instance->m_currentTest)
		{
			...
			{
				for (i32 i = 0; i < s_instance->m_iterateOverParams.m_numParams; ++ i)
				{
					reply.append(i ? ", " : "<");
			...
			for (i32 i = 0; i < s_instance->m_runFeatureTestStack.m_count; ++ i)
			{
				reply.append("=>");
...
{
	i32 numCommandsInNode = node->GetChildCount();
	for (i32 readCommandNum = 0; readCommandNum < numCommandsInNode; ++ readCommandNum)
	{
...
//-------------------------------------------------------------------------------
i32 CFeatureTester::PreprocessTestSet(const IItemParamsNode *testsListNode)
{
	i32 numTests = 0;
	i32 numNodesInThisSet = testsListNode->GetChildCount();
	tukk  setName = testsListNode->GetAttribute("setName");
	...
	for (i32 readChildNum = 0; readChildNum < numNodesInThisSet; ++ readChildNum)
	{
...
bool CFeatureTester::ReadTestSet(const IItemParamsNode *testsListNode, SFeatureTestDataLoadWorkspace * loadWorkspace)
{
	...
	tukk  setName = m_singleAllocTextBlock.StoreText(testsListNode->GetAttribute("setName"));
	i32 numChildrenInThisSet = testsListNode->GetChildCount();

	for (i32 readChildNum = 0; bOk && (readChildNum < numChildrenInThisSet); ++ readChildNum)
	{
		const IItemParamsNode * oneTest = testsListNode->GetChild(readChildNum);
		...
		if (0 == stricmp (oneTest->GetName(), "FeatureTest"))
		{
			...
			drx_displayMemInHexAndAscii(string().Format("[FEATURETESTER] Test %d \"%s\": ", readChildNum + 1, createTest.m_testName), loadWorkspace->m_instructionBuffer + createTest.m_offsetIntoInstructionBuffer, (loadWorkspace->m_count - createTest.m_offsetIntoInstructionBuffer) * sizeof(loadWorkspace->m_instructionBuffer[0]), CDrxLogOutputHandler(), sizeof(loadWorkspace->m_instructionBuffer[0]));
#endif
		}
	}
...
{
	bool bOk = true;
	i32 numCasesInSwitch = cmdParams->GetChildCount();
	i32 numInstructionsAtStartOfSwitch = loadWorkspace->GetNumInstructions();

	SFeatureTestInstructionOrParam * buffer = loadWorkspace->GetInstructionBufferModifiable();

	for (i32 caseNum = 0; bOk && (caseNum < numCasesInSwitch); ++ caseNum)
	{
		const IItemParamsNode * caseParams = cmdParams->GetChild(caseNum);
		tukk  caseType = caseParams->GetName();
		if (0 == stricmp ("Case", caseType))
		{
			...
			i32 writeJumpDestinationHere = loadWorkspace->GetNumInstructions();
	...
	if (bOk)
	{
		for (i32 fixInstruction = numInstructionsAtStartOfSwitch; fixInstruction < loadWorkspace->GetNumInstructions(); ++ fixInstruction)
		{
...
bool CFeatureTester::LoadCommands(const IItemParamsNode * node, SFeatureTest & createTest, SFeatureTestDataLoadWorkspace * loadWorkspace)
{
	...
	for (i32 readCommandNum = 0; bOk && (readCommandNum < numCommandsInNode); ++ readCommandNum)
	{
		const IItemParamsNode * cmdParams = node->GetChild(readCommandNum);
		tukk  cmdName = cmdParams->GetName();
		i32 commandIndex = -1;
...
{
	i32 numSettingsTags = topSettingsNode->GetChildCount();

	for (i32 i = 0; i < numSettingsTags; ++ i)
	{
		const IItemParamsNode * singleSetting = topSettingsNode->GetChild(i);
		tukk  singleSettingType = singleSetting->GetName();

		if (0 == stricmp(singleSettingType, "ListEntitiesWhenFail"))
		{
			tukk  className = singleSetting->GetAttribute("className");
			if (className)
			{
		...
		{
			tukk  checkpointName = singleSetting->GetAttribute("checkpointName");
			if (checkpointName)
			{
...
//-------------------------------------------------------------------------------
void CFeatureTester::LoadTestData(tukk  filenameNoPathOrExtension) PREFAST_SUPPRESS_WARNING(6262)
{
	...
	tukk  filename = filenameStr.c_str();
	...
	if (rootNode && 0 == strcmpi(rootNode->getTag(), "FeatureTester"))
	{
		...
		i32 numChildNodes = paramNode->GetChildCount();
		i32 numTestsToRead = 0;

		for (i32 childNodeNum = 0; childNodeNum < numChildNodes; ++ childNodeNum)
		{
			const IItemParamsNode * thisNode = paramNode->GetChild(childNodeNum);
			tukk  nodeName = thisNode->GetName();
		...
		for (i32 i = 0; i < m_checkpointsWhichAlwaysFailATest.m_num; ++ i)
		{
			tukk  oldBuffer = m_checkpointsWhichAlwaysFailATest.m_checkpointName[i];
			tukk  newBuffer = m_singleAllocTextBlock.StoreText(oldBuffer);
			DRX_ASSERT_TRACE (m_checkpointsWhichAlwaysFailATest.m_checkpointName[i] && 0 == strcmp(oldBuffer, newBuffer), ("Old string %p '%s' doesn't match new string %p '%s'", oldBuffer, oldBuffer, newBuffer, newBuffer));
		...
		for (i32 childNodeNum = 0; failureReason == NULL && (childNodeNum < numChildNodes); ++ childNodeNum)
		{
			const IItemParamsNode *testsListNode = paramNode->GetChild(childNodeNum);
			tukk  nodeName = testsListNode->GetName();
...
TBitfield CFeatureTester::ReadPlayerSelection(const IItemParamsNode * paramsNode)
{
	...
	tukk  whoString = paramsNode->GetAttribute("who");
...
bool CFeatureTester::AddInstructionAndParams(EFeatureTestCommand cmd, const IItemParamsNode * paramsNode, SFeatureTestDataLoadWorkspace * loadWorkspace)
{
	...
	switch (cmd)
	{
		case kFTC_MovePlayerToOtherEntity:
		{
			...
			i32 onTeamAsInt = 0;
			tukk  onTeamText = paramsNode->GetAttribute("onTeam");
		...
		case kFTC_SetResponseToHittingCCCPoint:
		{
			...
			{
				tukk  customMessage = m_singleAllocTextBlock.StoreText(paramsNode->GetAttribute("customMessage"));
...
//-------------------------------------------------------------------------------
void CFeatureTester::SendInputToLocalPlayer(tukk  inputName, EActionActivationMode mode, float value)
{
	CPlayer *pPlayer = static_cast<CPlayer *>(gEnv->pGame->GetIGameFramework()->GetClientActor());
	if(pPlayer != NULL && pPlayer->GetPlayerInput())
	{
		pPlayer->GetPlayerInput()->OnAction(CDrxName(inputName), mode, value);

		tukk  releaseName = inputName;
...
//-------------------------------------------------------------------------------
tukk  CFeatureTester::GetTextParam()
{
	tukk  param = GetNextInstructionOrParam()->GetData_kFTVT_Text();

	if (param  != NULL&& param[0] == '%' && param[1] >= '1' && param[1] < ('1' + m_iterateOverParams.m_numParams) && param[2] == '\0')
	{
		i32 paramNum = param[1] - '1';
...
{
	i32 toHere = GetNextInstructionOrParam()->GetData_kFTVT_Int();
	tukk  level = GetNextInstructionOrParam()->GetData_kFTVT_Text();
	bool match = true;
...
{
	for (i32 i = 0; i < m_numWatchedCheckpoints; ++ i)
	{
...
{
	tukk  itemClassName = GetTextParam();
...
void CFeatureTester::Instruction_kFTC_SetAmmo()
{
	...
	i32 requiredAmmo = GetNextInstructionOrParam()->GetData_kFTVT_Int();
	i32 requiredClip = GetNextInstructionOrParam()->GetData_kFTVT_Int();
...
{
	tukk  checkpointName = GetTextParam();
	i32 expectedNumTimesHit = GetNextInstructionOrParam()->GetData_kFTVT_Int();
	i32 actualNumTimesHit = 0;
...
void CFeatureTester::Instruction_kFTC_MovePlayerToOtherEntity()
{
	...
	tukk  className = GetTextParam();
	IEntityClass* classPtr = gEnv->pEntitySystem->GetClassRegistry()->FindClass(className);
	u32 requireFlags = GetNextInstructionOrParam()->GetData_kFTVT_EntityFlags();
	u32 skipWithFlags = GetNextInstructionOrParam()->GetData_kFTVT_EntityFlags();
		...
		for (TBitfield b = BIT(0); i < FeatureTestPlayerSelectionList_numBits; i ++, b <<= 1)
		{
			if (b & whichPlayers)
			{
				...
				if (CheckFeatureTestFailure(pPlayer, "There's no %s player", s_featureTestPlayerSelectionNames[i]))
				{
					...
					i32 playerTeam = rules->GetTeam(pPlayer->GetEntityId());
					...
					if (itEntity)
					{
						...
						while (!itEntity ->IsEnd())
						{
							...
							{
								u32 entityFlags = pEntity->GetFlags();
					...
					if (foundEntity && CheckFeatureTestFailure(foundEntity, "Didn't find an appropriate entity (was looking for class '%s', with flags %u but without flags %u)", className, requireFlags, skipWithFlags))
					{
						...
						moveToPos.y += 1.f;
						moveToPos.z += 1.f;
						drx_sprintf(command, "%splayerGoto %.2f %.2f %.2f 0 0 135", (b == kFTPS_localPlayer) ? "" : "sv_sendConsoleCommand ", moveToPos.x, moveToPos.y, moveToPos.z);
	...
	if (CheckFeatureTestFailure(pPlayer, "Failed to find relevant player (%s)", GET_PLAYER_SELECTION_TEXT(useWhichPlayer)))
	{
		if (!IsActorAliveAndPlaying(pPlayer))
		{
			...
			if (CheckFeatureTestFailure(pSpawningModule, "No spawning module present"))
			{
				...
				i32 teamNum = g_pGame->GetGameRules()->GetTeam(id);
				DrxFixedStringT<256> strBuffer;
...
//-------------------------------------------------------------------------------
CFeatureTester::SCheckpointCount * CFeatureTester::WatchCheckpoint(tukk  cpName)
{
...
//-------------------------------------------------------------------------------
bool CFeatureTester::FeatureTestFailureFunc(tukk  conditionTxt, tukk  messageTxt)
{
	string reply = messageTxt;
	tukk  testName = m_currentTest ? m_currentTest->m_testName : "N/A";
	...
	if (m_runFeatureTestStack.m_count > 0)
	{
		...
		for (i32 i = 1; i < m_runFeatureTestStack.m_count; ++ i)
		{
			reply.append("=>");
...
void CFeatureTester::Update(float dt)
{
	...
	while (IActor * iActor = pIter->Next())
	{
		...
		{
			DrxWatch ("$4[WARNING]$o %s '%s' is not updating!", iActor->GetEntity()->GetClass()->GetName(), iActor->GetEntity()->GetName());
		}
	}
#endif
	...
	if (m_numOverriddenInputs)
	{
		...
		if (playerInput)
		{
			for (i32 i = 0; i < m_numOverriddenInputs; ++ i)
			{
				SCurrentlyOverriddenInput & overriddenInput = m_currentlyOverriddenInputs[i];
				DrxWatch ("$7[FEATURETESTER]$o Overridden input '%s' mode=%d", overriddenInput.m_inputName, overriddenInput.m_mode);

				if (overriddenInput.m_sendHoldEventEveryFrame)
				{
					pPlayer->GetPlayerInput()->OnAction(CDrxName(overriddenInput.m_inputName), eAAM_OnHold, 1.f);
				}
			}
		}
	}
	...
	if (m_currentTestNextInstruction == NULL)
	{
		...
		else if (m_numFeatureTestsLeftToAutoRun)
		{
			...
			{
				i32 i = m_numTests - m_numFeatureTestsLeftToAutoRun;
...
//				DrxWatch ("Local player currentAngle=%.3f wantAngle=%.3f diff=%.3f ROTATE=%.3f",
//					currentRotation, targetRotation, rotationDiff, rotateAmountYaw);
				...
				switch (m_pause_state)
				{
					...
					case kFTPauseReason_untilCCCPointsHit:
					{
						if (g_pGameCVars->autotest_verbose)
						{
							...
							while (IActor * iActor = pIter->Next())
							{
								DrxFixedStringT<256> singleLine;
		...
		DrxWatch ("$7[FEATURETESTER]$o %sWait=%s Time=%.1f/%.1f", GetContextString().c_str(), s_featureTestPauseReasonNames[m_pause_state], m_pause_timeLeft, m_pause_originalTimeOut);

		for (i32 i = 0; i < m_numWatchedCheckpoints; ++ i)
		{
			EFTCheckpointHitResponse response = m_checkpointCountArray[i].m_hitResponse;
			DrxWatch ("  $%c[%s]$%c %s#%d", response + '2', s_featureTestHitResponseNames[response], (m_checkpointCountArray[i].m_stackLevelAtWhichAdded == m_runFeatureTestStack.m_count) ? '1' : '9', m_checkpointCountArray[i].m_checkpointName, m_checkpointCountArray[i].m_stackLevelAtWhichAdded);
		}
	}
}
...
/* static */ void CFeatureTester::CmdStartTest(IConsoleCmdArgs *pArgs)
{
	...
	{
		for (i32 i = 0; i < s_instance->m_numTests; ++ i)
		{
...
/* static */ void CFeatureTester::CmdRunAll(IConsoleCmdArgs *pArgs)
{
	if (pArgs->GetArgCount() == 2)
	{
		...
		tukk  setNames = pArgs->GetArg(1);
		tukk  parseSetNames = setNames;
		char oneSetName[64];
		i32 total = 0;
		...
		while (parseSetNames)
		{
			size_t skipChars = drx_copyStringUntilFindChar(oneSetName, parseSetNames, sizeof(oneSetName), '+');
			parseSetNames = skipChars ? (parseSetNames + skipChars) : NULL;
			i32 countInThisSet = 0;
	...
	{
		static i32k kMaxSetNamesToList = 32;
		tukk  setNamesDisplayed[kMaxSetNamesToList];
		tukk  cmdName = pArgs->GetArg(0);
		i32 numSetNamesDisplayed = 0;
		FeatureTesterLog("Please specify which feature test set(s) to run, e.g.");
		for (i32 i = 0; i < s_instance->m_numTests; ++ i)
		{
			i32 j;
			tukk  mySetName = s_instance->m_featureTestArray[i].m_setName;
...
//-------------------------------------------------------------------------------
CFeatureTester::SFeatureTest * CFeatureTester::FindTestByName(tukk  name)
{
	for (i32 i = 0; i < m_numTests; ++ i)
	{
...
//-------------------------------------------------------------------------------
void CFeatureTester::RemoveWatchedCheckpointsAddedAtCurrentStackLevel(tukk  reason)
{
...
//-------------------------------------------------------------------------------
tukk  CFeatureTester::GetActorInfoString(IActor * iActor, DrxFixedStringT<256> & reply)
{
	if (iActor)
	{
		...
		i32 teamNum = gameRules->GetTeam(pEntity->GetId());
		IItem * item = pActor->GetCurrentItem();
		SActorStats * stats = pActor->GetActorStats();
		IGameObject * gameObject = pActor->GetGameObject();

		DrxFixedStringT<128> itemInfo;
		if (item)
		{
			IWeapon * weapon = item->GetIWeapon();
			if (weapon)
			{
				i32 currentFireModeNum = weapon->GetCurrentFireMode();
				tukk  zoomedText = weapon->IsZoomingInOrOut() ? (weapon->IsZoomed() ? ", zooming out" : ", zooming in") : (weapon->IsZoomed() ? ", zoomed in" : "");
		...
		DrxFixedStringT<128> targetInfo;
...
void CFeatureTester::SubmitTestStartedMessageToAutoTester(const SFeatureTest * test)
{
	if (m_informAutoTesterOfResults && test != m_haveJustWrittenMessageAboutUnfinishedTest)
	{
		...
		{
			for (i32 i = 0; i < m_iterateOverParams.m_numParams; ++ i)
			{
				testName.append(i ? ", " : " <");
...
//-------------------------------------------------------------------------------
void CFeatureTester::SubmitResultToAutoTester(const SFeatureTest * test, float timeTaken, tukk  failureMessage)
{
	DrxFixedStringT<1024> actuallySendFailureMessage;
	CGameRules * gameRules = g_pGame->GetGameRules();
	...
	if (failureMessage && failureMessage[0])
	{
		...
		if (gameRules)
		{
			...
			for (i32 i = 0; i < m_listEntityClassesWhenFail.m_num; ++ i)
			{
		...
		if (test == m_currentTest)
		{
			...
			i32 numInStack = m_runFeatureTestStack.m_count;
		...
		for (i32 i = 0; i < m_numWatchedCheckpoints; ++ i)
		{
			...
			i32 stackLevel = m_checkpointCountArray[i].m_stackLevelAtWhichAdded;
		...
		{
			DrxFixedStringT<256> screenShotFileName;
...
//-------------------------------------------------------------------------------
void CFeatureTester::DoneWithTest(const SFeatureTest * doneWithThisTest, tukk  actionName)
{
...
//-------------------------------------------------------------------------------
void CFeatureTester::StopTest(tukk  failureMessage)
{
	DRX_ASSERT_TRACE (m_currentTest, ("Should only call StopTest function when there's a test running!"));
	DRX_ASSERT_TRACE (m_currentTestNextInstruction, ("Should only call StopTest function when there's a test running!"));
	...
	CGameCodeCoverageUpr::GetInstance()->DisableListener(this);
#endif
	...
	if(pPlayer != NULL && pPlayer->GetPlayerInput())
	{
		for (i32 i = 0; i < m_numOverriddenInputs; ++ i)
		{
			...
			DRX_ASSERT_TRACE(currentMode == eAAM_OnPress || currentMode == eAAM_Always, ("Unexpected mode 0x%x", currentMode));

			EActionActivationMode stopMode = (currentMode == eAAM_OnPress) ? eAAM_OnRelease : eAAM_Always;
			tukk  releaseName = overriddenInput.m_inputName;
			pPlayer->GetPlayerInput()->OnAction(CDrxName(releaseName), stopMode, 0.f);
		}
	}
...
//-------------------------------------------------------------------------------
void CFeatureTester::SendTextMessageToRemoteClients(tukk  msg)
{
...
//-------------------------------------------------------------------------------
void CFeatureTester::AttemptStartTestWithTimeout(const SFeatureTest * test, tukk  actionName, i32 offsetIntoIterationList, float dt)
{
	tukk  failReason = StartTest(test, actionName, 0.f, offsetIntoIterationList);
	...
	if (failReason)
	{
		...
		{
			DrxWatch("$7[FEATURETESTER]$o Waiting for \"%s\"", listOfFlags.c_str());
		}
	}
}

//-------------------------------------------------------------------------------
tukk  CFeatureTester::StartTest(const SFeatureTest * test, tukk  actionName, float delay, i32 offsetIntoIterationList)
{
	InterruptCurrentTestIfOneIsRunning();
	...
	if (failReason == NULL)
	{
...
#if ENABLE_GAME_CODE_COVERAGE
		CGameCodeCoverageUpr::GetInstance()->EnableListener(this);
#endif
		...
		{
			assert (offsetIntoIterationList < (i32)strlen(test->m_iterateOverParams));
			tukk  paramsStartHere = test->m_iterateOverParams + offsetIntoIterationList;
			char curParams[256];
			size_t readChars = drx_copyStringUntilFindChar(curParams, paramsStartHere, sizeof(curParams), ';');
			...
			// TODO: split on ',' and support multiple parameters
			{
				...
				// [See JB/OF] - Adding this alloc to Global Heap - JIRA DRXIII-1032
				//ScopedSwitchToGlobalHeap useGlobalHeap;
				m_iterateOverParams.m_currentParams[m_iterateOverParams.m_numParams] = new char[paramLength];
				drx_strcpy(m_iterateOverParams.m_currentParams[m_iterateOverParams.m_numParams], paramLength, curParams);
		...
		for (i32 i = 0; i < m_checkpointsWhichAlwaysFailATest.m_num; ++ i)
		{
			tukk  cpName = m_checkpointsWhichAlwaysFailATest.m_checkpointName[i];
			SCheckpointCount * watchedCheckpoint = WatchCheckpoint(cpName);
	...
	else
	{
		...
		u32 blinker = (u32)(m_timeSinceCurrentTestBegan * 3.f);
		DrxWatch ("$4%s can't %s: %s $%c(timeout=%.1f/%.1f)", test->m_testName, actionName, failReason, (blinker & 1) ? '9' : 'o', m_timeSinceCurrentTestBegan, test->m_maxTime);
	}
...
//-------------------------------------------------------------------------------
CFeatureTester::SCheckpointCount * CFeatureTester::FindWatchedCheckpointDataByName(tukk  name, i32 stackLevel)
{
	for (i32 i = 0; i < m_numWatchedCheckpoints; ++ i)
	{
...
//-------------------------------------------------------------------------------
void CFeatureTester::InformCodeCoverageLabelHit(tukk  cpLabel)
{
	i32 countNumDealtWith = 0;

	for (i32 i = 0; i <= m_runFeatureTestStack.m_count; ++ i)
	{
...
void CFeatureTester::InformWatchedCheckpointHit(SCheckpointCount * watchedCheckpoint)
{
	FeatureTesterLog ("Watched checkpoint '%s#%d' has been hit (response is \"%s\") while state=%s", watchedCheckpoint->m_checkpointName, watchedCheckpoint->m_stackLevelAtWhichAdded, s_featureTestHitResponseNames[watchedCheckpoint->m_hitResponse], s_featureTestPauseReasonNames[m_pause_state]);
		...
		switch (response)
		{
			...
			DRX_ASSERT_TRACE (watchedCheckpoint->m_customMessage != NULL, ("Custom failure message for hitting checkpoint '%s' is NULL", watchedCheckpoint->m_checkpointName));
...
//-------------------------------------------------------------------------------
IActor * CFeatureTester::GetNthNonLocalActor(i32 skipThisManyBeforeReturning)
{
	IActorSystem * pActorSystem = gEnv->pGame->GetIGameFramework()->GetIActorSystem();
...
void CFeatureTester::PollForNewCheckpointHits()
{
	...
	for (i32 i = 0; i < m_numWatchedCheckpoints; ++ i)
	{
		...
		u32 numHits = checkpoint ? checkpoint->HitCount() : 0u;
