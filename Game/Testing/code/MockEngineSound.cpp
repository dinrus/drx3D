...
#include <drx3D/Game/MockEngineSound.h>
...
namespace GameTesting
{
	...
	CMockEngineSound::~CMockEngineSound()
	{
		for (u32 i=0; i<m_obtainedSignals.size();++i)
		{
	...
	i32 CMockEngineSound::GetAmountAudioSignalsPlayed () const
	{
		return m_playedSignals.size();
	}

	i32 CMockEngineSound::GetAmountAudioSignalsStopped () const
	{
	...
	{
		for (u32 i=0; i<m_playedSignals.size(); ++i)
		{
			i32 ind = m_playedSignals[i].m_IndSignalInObtainedVector;
	...
	{
		for (u32 i=0; i<m_stoppedSignals.size(); ++i)
		{
			i32 ind = m_stoppedSignals[i];
			if ( m_obtainedSignals[ind]->m_signalName==audioSignalName)
				return true;
		}
		return false;
	...
	{
		u32 ind = soundID - 1;
		DRX_ASSERT( ind<m_playedSounds.size() );
