...
$Id$
$DateTime$
Описание: GunTurret Implementation
...
protected:
  ...
  template<typename T> struct TRandomVal
  {
    ...
    ILINE void New() { val = drx_random(-range, range); }
...
public:
	...
	void SetTeamNum(i32 num) { m_turretparams.team = num; }
...
private:	// For the time being, let subclasses (there's only 1 at time of writing) access ALL internal functions and data [TF]
  ...
  bool IsTargetCloaked(IActor* pTarget) const;
	...
	void    ServerUpdate(SEntityUpdateContext& ctx, i32 update);
  ...
  void DrawDebug();
	float GetScaledDamage(float damage, i32 hitType) const;
