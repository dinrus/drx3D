...
struct TEnvEmitter
{
	...
	i32 layer; // layer idx the emitter belongs to    
	i32 slot;  // emitter slot
	i32 matId; // last surface idx    
	i32 group; // optional     
...
struct SEnvParticleStatus
{
	...
	{    
		...
		i32 readCount = emitters.size();
		ser.Value("NumEnvEmitters", readCount);
		i32k count = min(readCount, static_cast<i32>(emitters.size()));
...
// enum continuation of DrxAction EVehicleMovementEvent for Game-Specific events
...
public:  
	...
	virtual i32 GetStatus(SVehicleMovementStatus* status) override;
	...
	virtual i32 GetWheelContacts() const override { return 0; }
	...
	virtual DrxCriticalSection* GetNetworkLock() override { return &m_networkLock; }

	virtual float GetEnginePedal() override { return m_movementAction.power; }

	const SVehiclePhysicsStatus& GetPhysicsStatus(u32 thread) { DRX_ASSERT(thread<k_numThreads); return m_physStatus[thread]; }
...
protected:
	...
	void SetSoundParam(EVehicleMovementSound eSID, tukk param, float value);
	...
	void ApplyAirDamp(float angleMin, float angVelMin, float deltaTime, i32 threadSafe);
	...
	void FreeEmitterSlot(i32& slot);
	void FreeEmitterSlot(i32k& slot);
	...
	SMFXResourceListPtr GetEffectNode(i32 matId);
	tukk GetEffectByIndex(i32 matId, tukk username);
...
protected:
	...
	u8 m_isProbablyDistant : 1;
	u8 m_isProbablyVisible : 1;

	float m_engineStartup;
	float m_engineIgnitionTime;
	u32 m_engineDisabledTimerId;


	u32 m_engineDisabledFXId;
	...
	DrxCriticalSection	m_networkLock;
};
