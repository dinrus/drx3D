...
//------------------------------------------------------------------------------------------
i32 CMovingPlatformMgr::OnCollisionLogged( const EventPhys* pEvent )
{
	...
	i32k PLAT = iszero(ppf[0].iForeignFlags&PFF_MOVING_PLATFORM);
	i32k PASS = 1-PLAT;
	...
	if( passengerType==PE_ARTICULATED || pPassEntity->GetClass()==pCorpseClass )
	{
		...
		pCorpseUpr->KeepAwake(passengerEntityId, pPassPhys);
		return 1;
	}
...
//------------------------------------------------------------------------------------------
i32 CMovingPlatformMgr::OnDeletedLogged( const EventPhys* pEvent )
{
...
void CMovingPlatformMgr::Update( const float dt )
{
	...
	for(TContactList::iterator it=m_contacts.begin(); it!=m_contacts.end(); )
	{
		...
		else
		{
			...
			{
				// Valid.
				//DrxWatch("%s: [%.2f / %.2f]", pEntity->GetName(), contact.fTimeSinceFirstContact, contact.fTimeSinceLastContact);
				++it;
			}
		}
	}
...
i32 CMovingPlatformMgr::StaticOnCollision( const EventPhys * pEvent )
{
	return g_pGame->GetMovingPlatformUpr()->OnCollisionLogged(pEvent);
}

i32 CMovingPlatformMgr::StaticOnDeleted( const EventPhys * pEvent )
{
	return g_pGame->GetMovingPlatformUpr()->OnDeletedLogged(pEvent);
}
