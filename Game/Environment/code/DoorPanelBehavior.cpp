...
Описание: Flash animated door panel HSM
...
#include #include <drx3D/Game/GameCache.h>

#include #include <drx3D/Game/SShootHelper.h>
#include #include <drx3D/Game/WeaponSystem.h>
#include #include <drx3D/Game/Audio/AudioSignalPlayer.h>
...
typedef DrxFixedStringT<64> TDoorPanelMaterialName;
...
private:
	...
	void DrawSlot( CDoorPanel& doorPanel, i32k iSlot, const bool bEnable );
	void SwitchSlot( CDoorPanel& doorPanel, i32k iSlot );
	IMaterial* CreateClonedMaterial( IEntity* pEntity, i32k iSlotIndex );
	...
	tukk GetDestroyedExplosionType( IEntity* pEntity ); // Temporary till destroyed details worked out
	
	void ChangeMaterial( CDoorPanel& doorPanel, i32k iSlot, IMaterial* pMaterial );
	void ChangeMaterial( CDoorPanel& doorPanel, i32k iSlot, tukk szMaterial );
	...
	DrxFixedStringT<32>		m_flashState;
	...
	i32										m_iCurrentVisibleSlot;
	bool									m_bScannable;

	static void IncrementNumUniqueFlashInstances(tukk szMaterialName);
	static void DecrementNumUniqueFlashInstances(tukk szMaterialName);
	static i32 GetNumNumUniqueFlashInstances(tukk szMaterialName);
	...
	case STATE_EVENT_ENTER:
		{
			...
			if (m_bScannable)
			{
				doorPanel.RemoveFromTacticalUpr();
			}

			tukk szFlashState = "ScanSuccessComplete";
			m_flashState = szFlashState;
			if (m_pFlashObjectRoot)
			{
				m_pFlashObjectRoot->GotoAndStop(szFlashState);
	...
	case STATE_EVENT_ENTER:
		{
			...
			tukk szFlashState = "BrokenScreen";
			m_flashState = szFlashState;
...
{
	tukk szModelName = DOOR_PANEL_MODEL_NORMAL;
	tukk szModelDestroyedName = DOOR_PANEL_MODEL_DESTROYED;
...
bool CDoorPanelBehavior::SetupFlash( CDoorPanel& doorPanel )
{
	...
	if (pOriginalMaterial)
	{
		...
		else
		{
			...
			DRX_ASSERT(pEntityToShareScreen != NULL);
...
void CDoorPanelBehavior::DrawSlot( CDoorPanel& doorPanel, i32k iSlot, const bool bEnable )
{
	IEntity* pEntity = doorPanel.GetEntity();
	
	if (pEntity)
	{
		i32 flags = pEntity->GetSlotFlags(iSlot);
...
void CDoorPanelBehavior::SwitchSlot( CDoorPanel& doorPanel, i32k iSlot )
{
...
IMaterial* CDoorPanelBehavior::CreateClonedMaterial( IEntity* pEntity, i32k iSlotIndex )
{
	DRX_ASSERT( m_pClonedAnimatedMaterial == NULL );
	...
	if (pRenderProxy)
	{
		...
		if (pOriginalMaterial)
		{
			IMaterialUpr* pMatUpr = gEnv->p3DEngine->GetMaterialUpr();
			DRX_ASSERT(pMatUpr);

			pClonedMaterial = pMatUpr->CloneMaterial( pOriginalMaterial );
			if (pClonedMaterial)
			{
...
tukk CDoorPanelBehavior::GetDestroyedExplosionType( IEntity* pEntity )
{
	DRX_ASSERT(pEntity != NULL);
...
void CDoorPanelBehavior::ChangeMaterial( CDoorPanel& doorPanel, i32k iSlot, IMaterial* pMaterial )
{
	if (pMaterial != NULL)
	{
...
void CDoorPanelBehavior::ChangeMaterial( CDoorPanel& doorPanel, i32k iSlot, tukk szMaterial )
{
	IMaterial* pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial( szMaterial );
...
void CDoorPanelBehavior::IncrementNumUniqueFlashInstances(tukk szMaterialName)
{
...
void CDoorPanelBehavior::DecrementNumUniqueFlashInstances(tukk szMaterialName)
{
	...
	if (iter != s_materialNameToInstanceCountMap.end())
	{
		...
		i32k iCount = countData.m_iCount - 1;
...
i32 CDoorPanelBehavior::GetNumNumUniqueFlashInstances(tukk szMaterialName)
{
	TDoorPanelMaterialNameToInstanceCountMap::iterator iter = s_materialNameToInstanceCountMap.find(szMaterialName);
	if (iter != s_materialNameToInstanceCountMap.end())
	{
