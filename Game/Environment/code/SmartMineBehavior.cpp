...
Описание: Smart proximity mine HSM
...
struct SmartMinePersistantProperties
{
	...
	tukk m_explosionType;
	float m_triggerToDetonateDelay;
	float m_detonationRadius;
	i32   m_damage;
	i32   m_initialPhysicsType;
	i32   m_currentPhysicsType;
	float m_jumpImpulse;
...
struct SmartMineSetupProperties
{
	...
	Vec3				m_triggerBox;
	tukk m_modelName;
	tukk m_materialArmed;
	tukk m_materialDisarmed;
	tukk m_faction;
...
private:
	...
	void Physicalize( CSmartMine& smartMine, i32k physicalizeType );
...
const CSmartMineBehavior::TStateIndex CSmartMineBehavior::Root( CSmartMine& smartMine, const SStateEvent& event )
{
	...
	case STATE_EVENT_INIT:
		{
			for(i32 i = 0; i < eVisualState_Count; ++i)
			{
	...
	case STATE_EVENT_SERIALIZE:
		{
			...
			i32  physicsType = m_properties.m_currentPhysicsType;
	...
	case STATE_EVENT_ENTER:
		{
			...
			smartMine.RemoveFromTacticalUpr();
			smartMine.GetEffectsController().DetachEffect( m_lightEffectId );
			m_lightEffectId = 0;
...
void CSmartMineBehavior::SetupSmartMine( CSmartMine& smartMine, const SmartMineSetupProperties& smartMineProperties )
{
	...
	struct 
	{
		...
		tukk signalName[eAudioType_Count];
	...
	for (i32 i = 0; i < eSmartMineSound_Count; ++i)
	{
...
void CSmartMineBehavior::Physicalize( CSmartMine& smartMine, i32k physicalizeType )
{
...
void CSmartMineBehavior::PreCacheResources( const SmartMineSetupProperties& smartMineProperties )
{
	...
	m_mineMaterials[eVisualState_Armed] = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial( smartMineProperties.m_materialArmed );
	m_mineMaterials[eVisualState_Disarmed] = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial( smartMineProperties.m_materialDisarmed );
...
void CSmartMineBehavior::SendSoundSystemGameHint( const EAudioHint audioHint )
{
	...
	tukk AUDIO_HINT_NAMES[eAudioType_Count] = { "w_smart_mine" };
...
{
	DRX_ASSERT( (visualState >= 0) && (visualState < eVisualState_Count) );
...
bool CSmartMineBehavior::UpdateArmedState( CSmartMine& smartMine, const float frameTime )
{
	...
	u32 targetCount = smartMine.GetTrackedEntitiesCount();
	...
	for (u32 i = 0; i < entitiesToRemove.size(); ++i)
	{
