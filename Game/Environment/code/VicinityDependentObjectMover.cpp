...
#include <drx3D/Game/VicinityDependentObjectMover.h>
...
bool CVicinityDependentObjectMover::ReloadExtension( IGameObject * pGameObject, const SEntitySpawnParams &params )
{
	...
	DRX_ASSERT_MESSAGE( false, "CVicinityDependentObjectMover::ReloadExtension not implemented" );
...
{
	DRX_ASSERT_MESSAGE( false, "CVicinityDependentObjectMover::GetEntityPoolSignature not implemented" );
...
{
	serializer.Value( "m_currentState", (i32&)m_currentState );
...
void CVicinityDependentObjectMover::Update( SEntityUpdateContext& ctx, i32 slot )
{
#if !defined(_RELEASE)
	if ( gEnv->IsEditing() )
	{
...
void CVicinityDependentObjectMover::GetMemoryUsage( IDrxSizer *pSizer ) const
{
...
void CVicinityDependentObjectMover::ActivateOutputPortBool( tukk szPortName )
{
	IEntity* pEntity = GetEntity();
	DRX_ASSERT( pEntity );
...
{
	tukk szModelName = VICINITYDEPENDENTOBJECTMOVER_MODEL_NORMAL;
...
void CVicinityDependentObjectMover::DrawSlot( i32k iSlot, const bool bEnable )
{
	IEntity* pEntity = GetEntity();
