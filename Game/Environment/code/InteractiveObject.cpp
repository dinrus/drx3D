...
#include #include <drx3D/Game/Player.h>
...
void CInteractiveObjectEx::InitClient( i32 channelId )
{
...
void CInteractiveObjectEx::PostInitClient( i32 channelId )
{
...
bool CInteractiveObjectEx::ReloadExtension( IGameObject * pGameObject, const SEntitySpawnParams &params )
{
	...
	DRX_ASSERT_MESSAGE(false, "CInteractiveObjectEx::ReloadExtension not implemented");
...
{
	DRX_ASSERT_MESSAGE(false, "CInteractiveObjectEx::GetEntityPoolSignature not implemented");
...
bool CInteractiveObjectEx::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
...
void CInteractiveObjectEx::Update( SEntityUpdateContext &ctx, i32 updateSlot )
{
...
void CInteractiveObjectEx::SetChannelId( u16 id )
{
...
void CInteractiveObjectEx::GetMemoryUsage(IDrxSizer *pSizer) const
{
...
bool CInteractiveObjectEx::Reset()
{
	...
	std::vector<tuk> interactionNames;  // Store these locally and discard after processing and storing CRCs 
	...
	if (entityProperties->GetValue("Interaction", interactionProperties))
	{
		...
		DRX_ASSERT_MESSAGE(interactionNames.size() == m_interactionDataSets.size(), "bool CInteractiveObjectEx::Reset() < Error - differing number of animation names than anim sets");

		tukk objectModel = NULL;
		interactionProperties->GetValue("object_Model", objectModel);
		assert(objectModel);

		u32 crcForModel = GetCrcForName(objectModel);
...
void CInteractiveObjectEx::ParseAllInteractions(const SmartScriptTable& interactionProperties, std::vector<tuk>& interactionNames)
{
...
void CInteractiveObjectEx::InitAllInteractionsFromMannequin() 
{
	...
	u32k numHelpers = interactions.size();
...
i32 CInteractiveObjectEx::CanUse( EntityId entityId ) const
{
...
void CInteractiveObjectEx::StartUseSpecific( EntityId entityId, i32 interactionIndex )
{
	//force animation on this object to update even if we can't see it
	ForceSkeletonUpdate( true );
...
i32 CInteractiveObjectEx::CalculateSelectedInteractionIndex( const EntityId entityId ) const
{
	...
	{
		DrxFixedArray<i32, 8> interactionCandidates; // Should be more than enough
		...
		{
			return interactionCandidates[drx_random(0U, interactionCandidates.size() - 1)];
		}
	}
...
void CInteractiveObjectEx::ForceInstantStandaloneUse(i32k interactionIndex)
{
	m_activeInteractionIndex = interactionIndex;
...
void CInteractiveObjectEx::CalculateHelperLocation( tukk helperName, QuatT& helper ) const
{
	...
	if (ICharacterInstance* pObjectCharacter = GetEntity()->GetCharacter(0))
	{
		i16 jointId = pObjectCharacter->GetIDefaultSkeleton().GetJointIDByName(helperName);
		if (jointId >= 0)
		{
...
i32 CInteractiveObjectEx::CanUserPerformAnyInteraction( EntityId userId ) const
{
...
bool CInteractiveObjectEx::IsValidName( tukk name ) const
{
	return (name && (name[0]!='\0'));
}

u32 CInteractiveObjectEx::GetCrcForName( tukk name ) const
{
	assert(name);
...
void CInteractiveObjectEx::OnExploded( const Vec3& explosionSource )
{
	...
	{
		//Pick best fit
		i32k interactions = m_interactionDataSets.size();
		if(interactions)
		{
			i32 bestFitIndex = 0;
			float bestFitDistance = FLT_MAX;
			for(i32 i = 0; i < interactions; ++i)
			{
