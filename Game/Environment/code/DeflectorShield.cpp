...
#include <drx3D/Game/DeflectorShield.h>
#include <drx3D/Game/Projectile.h>
#include <drx3D/Game/WeaponSystem.h>
#include <drx3D/AI/IAIObjectUpr.h>


void CDeflectorShield::GetMemoryUsage(IDrxSizer *pSizer) const {}
void CDeflectorShield::InitClient(i32 channelId) {}
void CDeflectorShield::PostInitClient(i32 channelId) {}
...
bool CDeflectorShield::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 pflags) {return true;}
...
void CDeflectorShield::SetChannelId(u16 id) {}
void CDeflectorShield::SetAuthority(bool auth ) {}
ukk  CDeflectorShield::GetRMIBase() const {return 0;}
...
void CDeflectorShield::Update(SEntityUpdateContext& ctx, i32 updateSlot)
{
	if (updateSlot != 0)
		return;
...
//
void CDeflectorShield::SetNonDeflectedOwnerGroup(i32 groupID)
{
...
void CDeflectorShield::LoadScriptProperties()
{
	...
	if (particleFx)
		m_pDeflectedEffect = gEnv->p3DEngine->GetParticleUpr()->FindEffect(particleFx);
	...
	if (m_pAmmoClass)
		...
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "No ammo type specified for Deflector Shield '%s'", GetEntity()->GetName());
}
...
void CDeflectorShield::PreCacheAmmoResources()
{
	...
	if (pParams)
	 ...
	 DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Deflector Shield '%s' specifies invalid ammo type", GetEntity()->GetName());
}
...
void CDeflectorShield::ShootDeflectedEnergy(const CDeflectorShield::SDeflectedEnergy& energy)
{
	...
	const float spreadOffset = drx_random(0.0f, m_spread);
	
	const Vec3 position = worldReflectPos + worldReflectDir * positionBias;
	Vec3 direction = 
		worldReflectDir + 
		(worldSpreadU * drx_random(0.0f, m_spread)) + 
		(worldSpreadV * drx_random(0.0f, m_spread));
	direction.Normalize();
...
bool CDeflectorShield::CanProjectilePassThroughShield(const CProjectile* pProjectile) const
{
	...
	const IAIObject* pOwnerAIObject = gEnv->pAISystem->GetAIObjectUpr()->GetAIObject(
