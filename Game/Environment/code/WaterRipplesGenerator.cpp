...
#include <drx3D/Game/WaterRipplesGenerator.h>
...
bool CWaterRipplesGenerator::ReloadExtension( IGameObject * pGameObject, const SEntitySpawnParams &params )
{
	...
	DRX_ASSERT_MESSAGE(false, "CWaterRipplesGenerator::ReloadExtension not implemented");
...
{
	DRX_ASSERT_MESSAGE(false, "CWaterRipplesGenerator::GetEntityPoolSignature not implemented");
...
void CWaterRipplesGenerator::Update(SEntityUpdateContext &ctx, i32 updateSlot)
{
	DRX_ASSERT( updateSlot == WATER_RIPPLES_GENERATOR_UPDATE_SLOT );

	float fFrequency = m_properties.m_frequency + drx_random(-1.0f, 1.0f)*m_properties.m_randFrequency;
...
void CWaterRipplesGenerator::ProcessHit(bool isMoving)
{
	if ((m_properties.m_enabled))
	{
		...
		{
			vWorldPos += Vec3(m_properties.m_randomOffset).CompMul(drx_random_componentwise(Vec3(-1, -1, 0), Vec3(1, 1, 0)));
		}

		float fScale = m_properties.m_scale + drx_random(-1.0f, 1.0f) * m_properties.m_randScale;
		float fStrength = m_properties.m_strength + drx_random(-1.0f, 1.0f) * m_properties.m_randStrength;
