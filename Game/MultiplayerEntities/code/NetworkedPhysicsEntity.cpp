...
bool CNetworkedPhysicsEntity::ReloadExtension( IGameObject * pGameObject, const SEntitySpawnParams &params )
{
	...
	DRX_ASSERT_MESSAGE(false, "CCTFFlag::ReloadExtension not implemented");
...
{
	DRX_ASSERT_MESSAGE(false, "CCTFFlag::GetEntityPoolSignature not implemented");
...
//-----------------------------------------------------------------------------
void CNetworkedPhysicsEntity::GetMemoryUsage(IDrxSizer *pSizer) const
{
	pSizer->Add(*this);
}

//-----------------------------------------------------------------------------
bool CNetworkedPhysicsEntity::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
...
//------------------------------------------------------------------------
bool CNetworkedPhysicsEntity::SetAspectProfile( EEntityAspects aspect, u8 profile )
{
	bool bRetVal = false;
	if (aspect == eEA_Physics)
	{
		switch (profile)
		{
			...
			default:
				{
					DrxLog("Unsupported physicalization type in CNetworkedPhysicsEntity!");
				}
		}
	}
	...
	if(pPhysEnt)
	{
			...
			physicsAction.w = drx_random_componentwise(Vec3(-7.5f), Vec3(7.5f));
...
//------------------------------------------------------------------------
u8 CNetworkedPhysicsEntity::GetDefaultProfile( EEntityAspects aspect )
{
	if (aspect == eEA_Physics)
	{
...
void CNetworkedPhysicsEntity::ReadPhysicsParams()
{
	...
	DRX_ASSERT(!"Failed to read physics params from script for NetworkedPhysicsEntity");
}
