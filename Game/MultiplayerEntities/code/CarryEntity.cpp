...
#include <drx3D/Game/CarryEntity.h>
...
//------------------------------------------------------------------------
void CCarryEntity::GetMemoryUsage( IDrxSizer *pSizer ) const
{
	pSizer->Add(*this);
}
...
void CCarryEntity::Attach( EntityId actorId )
{
	...
	if (pPlayer)
	{
		...
		if(bDoAttach)
		{
			...
			AttachmentUtils::AttachObject( bIsOwnerFP, pPlayerEntity, GetEntity(), CARRYENTITY_ATTACHMENT_NAME );
			IAttachment* pAttachment = pPlayerEntity->GetCharacter(CARRYENTITY_CHARACTER_SLOT)->GetIAttachmentUpr()->GetInterfaceByName(CARRYENTITY_ATTACHMENT_NAME);
			if(pAttachment)
			{
				...
				IAttachment* pShadowAttachment =  pShadowCharInst ? pShadowCharInst->GetIAttachmentUpr()->GetInterfaceByName(CARRYENTITY_ATTACHMENT_NAME) : NULL;
