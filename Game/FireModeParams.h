...
#	define AI_WEAPON_DESCRIPTOR_PARAMS_MEMBERS(f) \
		...
		f(i32,		burstBulletCountMin) \
		f(i32,		burstBulletCountMax) \
...
struct SMeleeTags
{
	struct STagParams
	{
		...
		STagParams( tukk pTag, tukk pHitType );
	...
	struct SMeleeFragData
	{
		...
		SMeleeFragData( u32 animIndex, const SMeleeTags::STagParams& meleeTags )
			:
...
#define TRACER_PARAMS_ARRAYS(f) \
	...
	void PreLoadAssets() const
	{
		...
		if(!gEnv->IsDedicated())
		{
			if(!effect.empty() && !gEnv->pParticleUpr->FindEffect(effect.c_str()))
			{
				if (gEnv->IsEditor())
			...
			if(!effectFP.empty() && !gEnv->pParticleUpr->FindEffect(effectFP.c_str()))
			{
				if (gEnv->IsEditor())
