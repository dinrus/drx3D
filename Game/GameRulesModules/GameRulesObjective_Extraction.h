...
protected:
	...
	static i32k k_maxNumPlayers=MAX_PLAYER_LIMIT;
	typedef DrxFixedArray<EntityId, k_maxNumPlayers> TPlayers;
	...
	typedef DrxFixedArray<SDefender, k_maxNumPlayers> TDefenders;
	...
	struct SPickup
	{
		...
		i32									m_currentObjectivePriority;
		i32									m_currentObjectiveCountDown;
		...
		SPickup()
		{
			...
			DrxLog("[state] CGameRulesObjective_Extraction::SPickup::SPickup: setting pickup's (*%p) state to ePickupState_None", this);
	...
	static i32k k_maxNumPickups=5;
	typedef DrxFixedArray<SPickup, k_maxNumPickups> TPickupElements;
	TPickupElements m_pickups;

	struct SSpawnAt
	{
		EntityId	m_entityId;
		i32				m_numInProximity;
		SSpawnAt()
		{
			m_entityId				= 0;
			m_numInProximity	= 0;
	...
	static i32k k_maxNumSpawnAts=20;
	typedef DrxFixedArray<SSpawnAt, k_maxNumSpawnAts> TSpawnAtElements;
	TSpawnAtElements m_spawnAtElements;
	typedef DrxFixedArray<SSpawnAt*, k_maxNumSpawnAts> TAvailableSpawnAtElements;
	...
	static i32k k_maxNumExtractAts=20;
	typedef DrxFixedArray<SExtractAt, k_maxNumExtractAts> TExtractAtElements;
	TExtractAtElements m_extractAtElements;
	bool m_haveStartedGame;
	float m_timeLimit;
	float m_previousTimeTaken;
	i32 m_numberToExtract;
  ...
  TFixedString m_statusDefendersTickCarriedDropped;
	...
	IEntity *SpawnEntity( EntityId spawnAt, IEntityClass *spawnEntityClass, i32 teamId, tukk name );
	...
	void CallEntityScriptFunction(EntityId entityId, tukk function, bool active);

	i32 FindCarrierInPickupCarriers(SPickup *pickup, EntityId carrierId);
	...
	ILINE tukk GetPickupString(SPickup *pickup)
	{
