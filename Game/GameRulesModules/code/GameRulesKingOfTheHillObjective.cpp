...
#include <drx3D/Game/GameRulesKingOfTheHillObjective.h>
...
void CGameRulesKingOfTheHillObjective::Init( XmlNodeRef xml )
{
	...
	{
		DrxLog("CGameRulesKingOfTheHillObjective::Init, using score timer, length=%f", m_scoreTimerMaxLength);
	}
	...
	DrxLog("CGameRulesKingOfTheHillObjective::Init, multiplier for additional players=%f", m_scoreTimerAdditionalPlayerMultiplier);
	...
	for (i32 childIdx = 0; childIdx < numChildren; ++ childIdx)
	{
		...
		{
			DRX_ASSERT_MESSAGE(!m_useIcons, "KingOfTheHillObjective xml contains more than one 'Icons' node, we only support one");
			m_useIcons = true;
			...
			DrxLog("CGameRulesKingOfTheHillObjective::Init, using on-screen icons [%i %i %i %i]", m_neutralIcon, m_friendlyIcon, m_hostileIcon, m_contestedIcon);
		}
...
void CGameRulesKingOfTheHillObjective::Update( float frameTime )
{
	...
	i32k localTeamId = pGameRules->GetTeam(g_pGame->GetIGameFramework()->GetClientActorId());
	...
	for (i32 i = 0; i < HOLD_OBJECTIVE_MAX_ENTITIES; ++ i)
	{
		...
		if (gEnv->bServer && pScoringModule)
		{
			...
			if (g_pGameCVars->g_KingOfTheHillObjective_watchLvl)
			{
				...
				tukk pEntName = pEntity ? pEntity->GetName() : "<NULL>";
				if (pDetails->m_controllingTeamId == CONTESTED_TEAM_ID)
				{
					DrxWatch("KotH entity '%s' is contested", pEntName);
				}
				else if (pDetails->m_controllingTeamId == 0)
				{
					DrxWatch("KotH entity '%s' has no players nearby", pEntName);
				}
				else
				{
					DrxWatch("KotH entity '%s' controlled by team %i, scoreTimerLength='%.2f', timeSinceLastScore='%.2f'", 
			...
			{
				i32k teamIndex = pKotHEntity->m_scoringTeamId - 1;
				DRX_ASSERT_MESSAGE(teamIndex >= 0 && teamIndex < NUM_TEAMS, "Update() scoringTeamId is out of range");
...
void CGameRulesKingOfTheHillObjective::OnInsideStateChanged( SHoldEntityDetails *pDetails )
{
	...
	{
		i32 oldTeamId = pKotHEntity->m_scoringTeamId;
	...
	if (pKotHEntity->m_scoringTeamId)
	{
		i32 teamIndex = pKotHEntity->m_scoringTeamId - 1;
...
//------------------------------------------------------------------------
void CGameRulesKingOfTheHillObjective::OnNewHoldEntity(SHoldEntityDetails *pDetails, i32 index)
{
...
//------------------------------------------------------------------------
void CGameRulesKingOfTheHillObjective::ClSiteChangedOwner( SHoldEntityDetails *pDetails, i32 oldTeamId )
{
	// Site has been captured
	CGameRules *pGameRules = g_pGame->GetGameRules();
	EntityId clientActorId = g_pGame->GetIGameFramework()->GetClientActorId();
	i32 localTeam = pGameRules->GetTeam(clientActorId);

	SKotHEntity *pKotHEntity = static_cast<SKotHEntity*>(pDetails->m_pAdditionalData);
	DRX_ASSERT(pKotHEntity);

	i32k ownerTeamId = pDetails->m_controllingTeamId;
	if (ownerTeamId > 0)
	{
		...
		else
		{
			...
			{
				tukk localisedMessage = CHUDUtils::LocalizeString(m_gameStateEnemyString.c_str());
	...
	else if (oldTeamId > 0)
	{
		...
		{
			tukk localisedMessage = CHUDUtils::LocalizeString(m_gameStateNeutralString.c_str());
	...
	for (i32 i = 0; i < HOLD_OBJECTIVE_MAX_ENTITIES; i ++)
	{
		...
		DRX_TODO( 23,03,2010, "HUD: OnSiteCaptured events are being sent multiple times from multiple places. /FH");
...
//------------------------------------------------------------------------
void CGameRulesKingOfTheHillObjective::ClSiteChangedOwnerAnnouncement(SHoldEntityDetails *pDetails, EntityId clientActorId, i32 ownerTeamId, i32 localTeam)
{
...
//------------------------------------------------------------------------
void CGameRulesKingOfTheHillObjective::OnChangedTeam( EntityId entityId, i32 oldTeamId, i32 newTeamId )
{
...
//------------------------------------------------------------------------
EGameRulesMissionObjectives CGameRulesKingOfTheHillObjective::GetIcon( SHoldEntityDetails *pDetails, tukk* ppOutName, tukk* ppOutColour )
{
	SKotHEntity *pKotHEntity = static_cast<SKotHEntity*>(pDetails->m_pAdditionalData);
	DRX_ASSERT(pKotHEntity);
	...
	else
	{
		...
		if (iconAllowed)
		{
			tukk pFriendlyColour = "#9AD5B7";
			tukk pHostileColour = "#AC0000";
			tukk pNeutralColour = "#666666";
...
void CGameRulesKingOfTheHillObjective::Announce(tukk announcement, TAnnounceType inType, const bool shouldPlayAudio) const
{
	...
	switch(inType)
	{
		case k_announceType_CS_Incoming:
			...
			{
				tukk localisedMessage = CHUDUtils::LocalizeString(m_gameStateIncomingString.c_str());
				SHUDEventWrapper::OnGameStatusUpdate(eGBNFLP_Neutral, localisedMessage);
			}
			break;
		case k_announceType_CS_Destruct:
			...
			{
				tukk localisedMessage = CHUDUtils::LocalizeString(m_gameStateDestructingString.c_str());
				SHUDEventWrapper::OnGameStatusUpdate(eGBNFLP_Neutral, localisedMessage);
			}
			break;
	}
...
//------------------------------------------------------------------------
float CGameRulesKingOfTheHillObjective::CalculateScoreTimer( i32 playerCount )
{
...
//------------------------------------------------------------------------
void CGameRulesKingOfTheHillObjective::OnTimeTillRandomChangeUpdated( i32 type, float fPercLiveSpan )
{
	for (i32 i = 0; i < HOLD_OBJECTIVE_MAX_ENTITIES; ++ i)
	{
		SHoldEntityDetails *pDetails = &m_entities[i];
