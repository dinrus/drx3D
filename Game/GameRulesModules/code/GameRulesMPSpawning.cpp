...
void CGameRulesMPSpawningBase::Init(XmlNodeRef xml)
{
	...
	{
		DRX_ASSERT_MESSAGE(0, "CGameRulesMPSpawningBase failed to find valid teamGame param");
	}
	...
	DRX_ASSERT_MESSAGE(typeOk, "CGameRulesMPSpawningBase failed to find valid teamSpawnsType param");
	...
	DrxLog("CGameRulesMPSpawningBase::Init() set params to teamGame=%d; teamSpawnsType=%d; midRoundJoiningBeforeFirstDeath=%d", m_teamGame, m_teamSpawnsType, m_allowMidRoundJoiningBeforeFirstDeath);
...
{
	DrxLog("CGameRulesMPSpawningBase::OnInGameBegin() creating list of initial players");
	if (CGameLobby *pGameLobby = g_pGame->GetGameLobby())
	{
		const SSessionNames &sessionNames = pGameLobby->GetSessionNames();
		for (u32 i=0, numSessionNames = sessionNames.Size(); i<numSessionNames; i++)
		{
			const SSessionNames::SSessionName &player = sessionNames.m_sessionNames[i];
			DrxLog("  %s (uid=%u)", player.m_name, player.m_conId.m_uid);
	...
	{
		DrxLog("CGameRulesMPSpawningBase::OnGameStart() equipment screen already shown, starting auto spawn timer");
...
void CGameRulesMPSpawningBase::Update(float frameTime)
{
	...
	if (gEnv->bServer)
	{
		if (!stateModule || (stateModule->GetGameState() != IGameRulesStateModule::EGRS_PostGame))
		{
			...
			for(u32 i = 0; i < m_spawnHistory.size(); i++)
			{
			...
			for (; it != m_playerValues.end(); ++it)
			{
				...
				if (!pObjectivesModule || pObjectivesModule->CanPlayerRevive(playerId))
				{
					...
					if (pActorImpl)
					{
						if(it->second.lastRevivedTime < it->second.deathTime && !it->second.triedAutoRevive)
						{
							if (pActorImpl->IsDead() && (pActorImpl->GetSpectatorState() == CActor::eASS_Ingame))
							{
								...
								i32k  actorTeam = static_cast<CPlayer*>(pActorImpl)->GetTeamWhenKilled();
								...
								{
									DrxLog("CGameRulesMPSpawning::Update() %s '%s' has been dead for %.3f seconds - that's over %.3f so auto reviving - FORCED", pActorImpl->GetEntity()->GetClass()->GetName(), pActorImpl->GetEntity()->GetName(), timeSinceDeath / 1000.f, timeTillAutoRevive / 1000.f);
					...
					{
						DrxLog("CGameRulesMPSpawningBase::Update() failed to find an actor for a player in m_playerValues, this should NOT happen");
					}
				}
			}
		}
	...
	if(m_bGameHasStarted && pLocalActorImpl != NULL && ((pLocalActorImpl->IsDead() && pLocalActorImpl->GetSpectatorState() == CActor::eASS_Ingame) || (pLocalActorImpl->GetSpectatorState() == CActor::eASS_None) || (m_bPrecacheContinuousBestSpawns)))
	{
		...
		{
			DrxLogAlways("WOULD HAVE CRASHED HERE - LOOKING FOR A SPAWN POINT WITH NO TEAM SET");
		}
#endif
	}
	...
	if(pLocalActorImpl != NULL && pLocalActorImpl->IsDead() && (!stateModule || (stateModule->GetGameState() != IGameRulesStateModule::EGRS_PostGame)) && (spectatorState == CActor::eASS_Ingame || shouldRequestRevive))
	{
		...
		{
			i32k  actorTeam = m_pGameRules->GetTeam(localActorEntityId);
...
void CGameRulesMPSpawningBase::AddSpawnLocation(EntityId location, bool isInitialSpawn, bool doVisTest, tukk pGroupName)
{
...
void CGameRulesMPSpawningBase::AddAvoidPOI(EntityId entityId, float avoidDistance, bool startActive, bool bUseStaticPosn)
{
	...
	DrxLog("CGameRulesMPSpawningBase::AddAvoidPOI() entity=%s (id=%d); avoidDistance=%f; startActive=%d, posn: (%.2f, %.2f, %.2f)", poiEntity ? poiEntity->GetName() : "NULL", entityId, avoidDistance, startActive, posn.x, posn.y, posn.z);
#endif
...
void CGameRulesMPSpawningBase::RemovePOI(EntityId entityId)
{
#if DEBUG_NEW_SPAWNING
	IEntity *poiEntity = gEnv->pEntitySystem->GetEntity(entityId);
	DrxLog("CGameRulesMPSpawningBase::RemovePOI() entity=%s; (id=%d)", poiEntity ? poiEntity->GetName() : "NULL", entityId);
#endif
	...
	DrxLog("CGameRulesMPSpawningBase::RemovePOI() failed to find POI for entity");
}

void CGameRulesMPSpawningBase::OnSetTeam(EntityId playerId, i32 teamId)
{
...
void CGameRulesMPSpawningBase::EnablePOI(EntityId entityId)
{
#if DEBUG_NEW_SPAWNING
	IEntity *poiEntity = gEnv->pEntitySystem->GetEntity(entityId);
	DrxLog("CGameRulesMPSpawningBase::EnablePOI() entity=%s; (id=%d)", poiEntity ? poiEntity->GetName() : "NULL", entityId);
#endif
	...
	DrxLog("CGameRulesMPSpawningBase::EnablePOI() failed to find POI for entity");
}
...
void CGameRulesMPSpawningBase::DisablePOI(EntityId entityId)
{
#if DEBUG_NEW_SPAWNING
	IEntity *poiEntity = gEnv->pEntitySystem->GetEntity(entityId);
	DrxLog("CGameRulesMPSpawningBase::DisablePOI() entity=%s; (id=%d)", poiEntity ? poiEntity->GetName() : "NULL", entityId);
#endif
	...
	DrxLog("CGameRulesMPSpawningBase::DisablePOI() failed to find POI for entity");
}
...
void CGameRulesMPSpawningBase::ClRequestRevive(EntityId playerId)
{
	...
	DrxLog("CGameRulesMPSpawningBase::ClRequestRevive()");
	...
	if (pActor)
	{
		DrxLog("CGameRulesMPSpawningBase::RequestRevive() player (%s)", pActor->GetEntity()->GetName());

		// Check if the player can revive
		if ((pActor->IsDead()) || (pActor->GetSpectatorState()==CActor::eASS_None))
		{
			DrxLog("[SPAWN] CGameRulesMPSpawningBase::ClRequestRevive() Requesting revive via RMI ");
			...
			else
			{
				...
				if(IEntity * pSpawnPoint = gEnv->pEntitySystem->GetEntity(spawnPointId))
				{
					...
					DrxLog("[SPAWN]   requesting spawn point '%s' at position (%.2f, %.2f, %.2f)", pSpawnPoint->GetName(), posn.x, posn.y, posn.z);
				}
#endif
		...
		{
			DrxLogAlways("CGameRulesMPSpawningBase::ClRequestRevive() Player %s is not dead", pActor->GetEntity()->GetName());
			CCCPOINT(PlayerState_CannotSpawnBecauseIsAlive);
		}
	}
	else
	{
		DRX_ASSERT_MESSAGE(0, "CGameRulesMPSpawningBase::ClRequestRevive() failed to find a playerEntity for player that died");
	}
}
...
bool CGameRulesMPSpawningBase::SvRequestRevive(EntityId playerId, EntityId preferredSpawnId)
{
	...
	if (pEntity != NULL && !pEntity->IsGarbage())
	{
		...
		{
#ifndef _RELEASE
			DrxLog("CGameRulesMPSpawningBase::SvRequestRevive() request received from %s but we're not in InGame state we're in state=%d, refusing", pEntity ? pEntity->GetName() : "<NULL>", pStateModule->GetGameState());
#endif
			return false;
		}
		...
		{
			DrxLog("CGameRulesMPSpawningBase::SvRequestRevive() request received from %s but we're currently between rounds, refusing", pEntity ? pEntity->GetName() : "<NULL>");
			return false;
		}
		...
		if(pObjectivesModule && !pObjectivesModule->CanPlayerRevive(playerId))
		{
			DrxLog("CGameRulesMPSpawningBase::SvRequestRevive() request received from %s but the game objective refused it", pEntity ? pEntity->GetName() : "<NULL>");
			return false;
		}
		...
		{
			CEquipmentLoadout *pEquipmentLoadout = g_pGame->GetEquipmentLoadout();
			u16 uActorChannel = pActor->GetChannelId();
			if (pEquipmentLoadout && uActorChannel && pEquipmentLoadout->SvHasClientEquipmentLoadout(uActorChannel) == false)
			{
				DrxLog("CGameRulesMPSpawningBase::SvRequestRevive() request received from %s but they have no loadout set, refusing", pEntity ? pEntity->GetName() : "<NULL>");
				return false;
			}
			...
			if (g_pGame->GetHostMigrationState() == CGame::eHMS_NotMigrating)
			{
				...
				{
					i32 nPlayerTeam = m_pGameRules->GetTeam(playerId); 
					if (m_teamGame==(nPlayerTeam!=0))
					{
						...
						if (pNetChannel)
						{
							DrxLog("CGameRulesMPSpawningBase::SvRequestRevive() request received from %s ping = %f, lag = %f, g_autoReviveTime = %f, autoReviveTimeMs = %f", pEntity ? pEntity->GetName() : "<NULL>", pNetChannel->GetPing(true), netLag, autoReviveTime, autoReviveTimeMs);
						}
						...
						{
							DrxLog("CGameRulesMPSpawningBase::SvRequestRevive() abandoning revive. Spectator State %d, CurrentTime: %f, DeathTime: %f, autoReviveTime %f, should revive based on Time: %s", SpectatorState, GetTime(), it->second.deathTime, autoReviveTimeMs, ((GetTime() - it->second.deathTime) > autoReviveTimeMs) ? "true" : "false");
							CCCPOINT(PlayerState_SvDoNotReviveTooSoonAfterDeath);
						}
					}
					else
					{
						DrxLog("CGameRulesMPSpawningBase::SvRequestRevive() %s", m_teamGame?"team game trying to revive a player who doesn't have team":"non-team game trying to revive a player who has a team");
					}
				}
			}
		}
		...
		{
			DrxLog("CGameRulesMPSpawningBase::SvRequestRevive() request received from %s but we're mid hostmigration, refusing", pEntity ? pEntity->GetName() : "<NULL>");
		}
#endif	
	}
...
void CGameRulesMPSpawningBase::PerformRevive(EntityId playerId, i32 teamId, EntityId preferredSpawnId)
{
	// no inherited call - not implemented in CGameRulesSpawningBase
	CActor *pActor = static_cast<CActor*>(gEnv->pGame->GetIGameFramework()->GetIActorSystem()->GetActor(playerId));

	DRX_ASSERT_MESSAGE(gEnv->bServer, "CGameRulesMPSpawningBase::PerformRevive() being called when NOT a server. This is wrong!");

	if (pActor)
	{
		DrxLog("CGameRulesMPSpawningBase::PerformRevive() player (%s) teamId=%d", pActor->GetEntity()->GetName(), teamId);
		INDENT_LOG_DURING_SCOPE();
		...
		{
			DRX_ASSERT_MESSAGE(m_playerValues.find(playerId) != m_playerValues.end(), string().Format("CGameRulesMPSpawningBase::PerformRevive() failed to find a playervalue for player %s. This should not happen", pActor->GetEntity()->GetName()));

			if (CheckMidRoundJoining(pActor, playerId))
			{
				if(!preferredSpawnId)
				{
					DrxLog("SPAWN-SPEC: No preferred spawn specified, calculating spawn location");
					preferredSpawnId = GetSpawnLocation(pActor->GetEntityId());
				}
				else
				{
					DrxLog("SPAWN-SPEC: Spawn specified avoiding calculation");
				}
				...
				if (pSpawnPoint)
				{
					...
					DrxLog("CGameRulesMPSpawningBase::PerformRevive, reviving %s '%s' at %s '%s', posn %.2f %.2f %.2f", pActor->GetEntity()->GetClass()->GetName(), pActor->GetEntity()->GetName(), pSpawnPoint->GetClass()->GetName(), pSpawnPoint->GetName(), posn.x, posn.y, posn.z);
#endif
				...
				{
					DrxLog("CGameRulesMPSpawningBase::PerformRevive() failed to find spawn point");
				}
			}
			else
			{
				DrxLog("CGameRulesMPSpawningBase::PerformRevive() failed because mid-round joins are disabled and we're currently mid-round (ingame) and the player to be spawned is not the server");
			}
		}
		else
		{
			DrxLog("CGameRulesMPSpawningBase::PerformRevive() Player %s is %s, Spectator State %d, preferred spawn %d, validity: %s, %s migrating",
	...
	{
		DrxLog("CGameRulesMPSpawningBase::PerformRevive() failed to find the actor to revive");
	}
}
...
bool CGameRulesMPSpawningBase::SpawnIsValid(EntityId spawnId, EntityId playerId) const
{
	...
	if(m_pGameRules->GetTeamCount() > 1)
	{
		...
		i32k iSpawnTeam = GetSpawnLocationTeam(spawnId);
	...
	if(IsPlayerInitialSpawning(playerId) && m_activeInitialSpawnGroupIndex < m_initialSpawnLocations.size())
	{
		...
		if(!bFound)
		{
			if(g_pGameCVars->g_debugSpawnPointValidity)
				DrxFatalError("Client '%s' was requesting a spawn that was not in the initial spawns list", gEnv->pEntitySystem->GetEntity(playerId)->GetName());

			return false;
		}
	}
	
	i32k		iNumSpawnRecords				= m_spawnHistory.size();
	for(i32 i = 0; i < iNumSpawnRecords; i++)
	{
	...
	i32k kNumActors = pActorUpr->GetNumActorsIncludingLocalPlayer();
...
EntityId CGameRulesMPSpawningBase::GetSpawnLocation(EntityId playerId)
{
	...
	if (!playerEntity)
	{
		DrxLog("ERROR: GetSpawnLocation() failed to find a playerEntity to work with. Returning.");
		return 0;
	}
	...
	{
		DRX_ASSERT_MESSAGE(0, string().Format("CGameRulesMPSpawningBase::GetSpawnLocation() failed to find a playervalue for player %s. This should not happen", playerEntity->GetName()));
	}
...
#ifndef _RELEASE
		if(!gEnv->IsEditor())
			DrxFatalError("GetSpawnLocation() player=%s; has failed to find a valid spawn. Was using %s spawning", playerEntity->GetName(), m_teamGame ? "team" : "non-team");
#endif //!_RELEASE
...
i32 CGameRulesMPSpawningBase::GetSpawnLocationTeam(EntityId spawnLocEid) const
{
	i32  team = 0;
	...
	case eTST_RoundSwap:
		{
			...
			if (team > 0)
			{
				...
				if (pRoundsModule)
				{
					i32  prim = pRoundsModule->GetPrimaryTeam();
...
bool CGameRulesMPSpawningBase::IsSpawnLocationSafeFromExplosives(const IEntity *pSpawn) const
{
	...
	for (i32 i=0; i<projQuery.nCount; i++)
	{
		...
		{
#if DEBUG_NEW_SPAWNING
			DrxLog("CGameRulesMPSpawningBase::IsSpawnLocationSafeFromExplosives() has found a non-bullet entity %s within range %f", pEntity->GetName(), g_pGameCVars->g_spawn_explosiveSafeDist);
#endif
			return false;
		}
	}
...
bool CGameRulesMPSpawningBase::IsSpawnLocationSafe(EntityId playerId, const IEntity * pSpawn, float safeDistance, bool ignoreTeam, float zoffset) const
{
	...
	if (zoffset<=0.0001f)
	{
		for (i32 i=0; i<query.nCount; i++)
		{
			...
			{
#if !defined(_RELEASE)
				DrxLog("[SPAWN] Spawn point '%s' at (%.2f, %.2f, %.2f) unsafe due to player entity '%s' at a distance of %f (within %f)", pSpawn->GetName(), c.x, c.y, c.z, query.pEntities[i]->GetName(), sqrtf(fDistSq), safeDistance);
#endif		
				result=false;
				break;
			}
			...
			if(posn)
			{
				...
				switch (poi.m_state)
				{
					case EPOIS_AVOID:
						...
						{
#if DEBUG_NEW_SPAWNING
							DrxLog("CGameRulesMPSpawningBase::DoesSpawnLocationRespectPOIs() found spawn %s is too close (%f) to avoid POI %s", pSpawn->GetName(), sqrtf(dist), pPOI->GetName());
#endif
							return false;
						}
						break;
					case EPOIS_ATTRACT:
						...
						{
#if DEBUG_NEW_SPAWNING
							DrxLog("CGameRulesMPSpawningBase::DoesSpawnLocationRespectPOIs() found spawn %s is too far away (%f) from attract POI %s", pSpawn->GetName(), sqrtf(dist), pPOI->GetName());
#endif
							return false;
						}
						break;
					default:
#if DEBUG_NEW_SPAWNING
						DRX_ASSERT_TRACE(0, ("CGameRulesSpawningBase::DoesSpawnLocationRespectPOIs() found a POI (entity=%s) with unhandled state %d", pPOI->GetName(), poi.m_state));
#else
						DRX_ASSERT_TRACE(0, ("CGameRulesSpawningBase::DoesSpawnLocationRespectPOIs() found a POI (entity=%d) with unhandled state %d", poi.m_entityId, poi.m_state));
#endif
						break;
				}
			}
...
void CGameRulesMPSpawningBase::DebugPOIs() const
{
	...
	for (it=m_pointsOfInterest.begin(); it!=m_pointsOfInterest.end(); ++it)
	{
		...
		DrxWatch("POI: entity=%s (%d); state=%d; enabled=%d", pPOI ? pPOI->GetName() : "NULL", poi.m_entityId, poi.m_state, poi.m_flags & EPOIFL_ENABLED ? 1 : 0);
	}	
}
#endif
...
{
	DrxLog("CGameRulesMPSpawningBase::ReviveAllPlayers() isReset=%s bOnlyIfDead=%d", isReset ? "true" : "false", bOnlyIfDead);
	...
	if(pEquipmentLoadout)
	{
			...
			for (i32 teamId = 1; teamId < 3; ++ teamId)
			{
				...
				for (i32 i = 0; i < numPlayers; ++ i)
				{
					...
					if (pActor && (!bOnlyIfDead || pActor->IsDead()))
					{
						...
						{
							i32 numPoints = spawnPoints.size();
							if (!numPoints)
							{
								numPoints = GetTeamSpawnLocations(teamId, false, spawnPoints);		// Run out of initial spawns, switch to regular spawns
								if (!numPoints)
								{
									DRX_ASSERT_MESSAGE(false, "Designer ASSERT: No team spawn points");
									return;
								}
							}

							i32 index = g_pGame->GetRandomNumber() % numPoints;
						...
						{
							DrxLog("can't revive player %s (%d) as they haven't set a loadout yet", pActor->GetEntity()->GetName(), playerId);
						}
					}
				}
			}
		...
		{
			DRX_ASSERT_MESSAGE(!m_pGameRules->GetRoundsModule(), "TODO! At the time of writing there are no rounds-based non-team gamemodes. If there ever are any, then this needs implementing... (Shouldn't be hard.)");
...
void CGameRulesMPSpawningBase::ReviveAllPlayers_NoTeam(bool isReset, bool bOnlyIfDead)
{
	...
	for (i32 i = 0; i < numPlayers; ++ i)
	{
		...
		if (pActor && static_cast<CActor*>(pActor)->GetSpectatorState() != CActor::eASS_SpectatorMode && (!bOnlyIfDead || pActor->IsDead()))
		{
			...
			{
				EntityId spawnId = 0;
				i32k numEligibleSpawnPoints = eligibleSpawns.size();
				if (numEligibleSpawnPoints)
				{
					i32 index = g_pGame->GetRandomNumber() % numEligibleSpawnPoints;
				...
				else
				{
					...
					DrxFatalError("CGameRulesMPSpawningBase::ReviveAllPlayers_NoTeam() failed to find elegible spawn point");
				}			
...
//------------------------------------------------------------------------
void CGameRulesMPSpawningBase::ReviveAllPlayersOnTeam(i32 teamId)
{
	CEquipmentLoadout *pEquipmentLoadout = g_pGame->GetEquipmentLoadout();
	...
	for (i32 i = 0; i < numPlayers; ++ i)
	{
		...
		if (pActor && pActor->IsDead())
		{
			if (pEquipmentLoadout->SvHasClientEquipmentLoadout(pActor->GetChannelId()))
			{
				...
				if (!numPoints)
				{
					DrxLog("CGameRulesMPSpawningBase::ReviveAllPlayersOnTeam() out of spawn points");
					return;
				}
...
//------------------------------------------------------------------------
void CGameRulesMPSpawningBase::ReviveAllPlayers_Internal( IActor * pActor, EntityId spawnId, i32 nPlayerTeam )
{
	IGameRulesObjectivesModule *pObjectivesModule = m_pGameRules->GetObjectivesModule();
...
//------------------------------------------------------------------------
float CGameRulesMPSpawningBase::GetTimeFromDeathTillAutoReviveForTeam(i32 teamId) const
{ 
...
//------------------------------------------------------------------------
float CGameRulesMPSpawningBase::GetAutoReviveTimeScaleForTeam(i32 teamId) const
{
...
//------------------------------------------------------------------------
void CGameRulesMPSpawningBase::SetAutoReviveTimeScaleForTeam(i32 teamId, float newScale)
{
	DrxLog("CGameRulesMPSpawningBase::SetAutoReviveTimeScaleForTeam() teamId=%d; newScale=%f", teamId, newScale);
...
//------------------------------------------------------------------------
i32 CGameRulesMPSpawningBase::GetTeamSpawnLocations( i32 teamId, bool bInitialSpawns, TSpawnLocations &results )
{
	...
	i32 numSpawnLocations = spawnLocations.size();
	results.reserve(numSpawnLocations);
	for (i32 i = 0; i < numSpawnLocations; ++ i)
	{
		EntityId spawnId = spawnLocations[i];
		i32 nSpawnLocationTeam = GetSpawnLocationTeam(spawnId);
...
void CGameRulesMPSpawningBase::HostMigrationInsertIntoReviveQueue( EntityId playerId, float timeTillRevive )
{
	...
	for (; it != m_playerValues.end(); ++it)
	{
		if (it->first == playerId)
		{
			...
			float gameTime = GetTime();
			i32k  playerTeam = m_pGameRules->GetTeam(playerId);
	...
	if (!m_allowMidRoundJoiningBeforeFirstDeath && !m_allowMidRoundJoining)
	{
		...
		if (!ok)
		{
			...
			if (IGameRulesPlayerStatsModule *pStatsModule = m_pGameRules->GetPlayerStatsModule())
			{
				...
				{
					DrxLog("CGameRulesMPSpawningBase::CheckMidRoundJoining() player id %u has PLYSTATFL_CANTSPAWNTHISROUND flag set, probably refusing spawn", playerId);
					ok = false;
				}
			}
			...
			if (!ok)
			{
				...
				if (m_pGameRules->GetTeamCount() > 1)
				{
					...
					i32 numTeamMembers = m_pGameRules->GetTeamPlayerCount(teamId, true);
					if (numTeamMembers == 1)
					{
						DrxLog("CGameRulesMPSpawningBase::CheckMidRoundJoining() playerId=%d is the only person on their team, allowing spawn", playerId);
						ok = true;
					}
				}
				else
				{
					i32 numPlayers = m_pGameRules->GetPlayerCount(true);
					if (numPlayers <= 2)
					{
						DrxLog("CGameRulesMPSpawningBase::CheckMidRoundJoining() not enough players in game yet, allowing spawn for playerId=%d", playerId);
						ok = true;
					}
				}
			}
...
//------------------------------------------------------------------------
EntityId CGameRulesMPSpawningBase::GetRandomSpawnLocation_Cached( const TSpawnLocations &spawnLocations, EntityId playerId, i32 playerTeamId, bool isInitalSpawn )
{
	...
	if(m_fCachedRandomSpawnTimeOut<=0.f)
	{
		//DrxLogAlways("[SPAWN] Random spawn occurring");
	...
	{
		//DrxLogAlways("[SPAWN] Using cached spawn point [%d]", m_cachedAloneSpawnId);
...
//------------------------------------------------------------------------
EntityId CGameRulesMPSpawningBase::GetRandomSpawnLocation( const TSpawnLocations &spawnLocations, EntityId playerId, i32 playerTeamId, bool isInitalSpawn ) const
{
	//DrxLog("CGameRulesMPSpawningBase::GetRandomSpawnLocation() playerTeamId=%d", playerTeamId);

	TSpawnLocations eligibleSpawns;
	GetEligibleSpawnListForTeam(spawnLocations, eligibleSpawns, playerTeamId, isInitalSpawn);

	EntityId spawnId = 0;
	i32k numElegibleSpawnPoints = eligibleSpawns.size();
	if (numElegibleSpawnPoints)
	{
		...
		do 
		{
			i32 index = g_pGame->GetRandomNumber() % numElegibleSpawnPoints;
			spawnId = eligibleSpawns[index];
	...
	else
	{
		...
		DrxFatalError("CGameRulesMPSpawningBase::GetRandomSpawnLocation() failed to find elegible spawn point");
	}
...
//------------------------------------------------------------------------
void CGameRulesMPSpawningBase::GetEligibleSpawnListForTeam(const TSpawnLocations& rAllLocations, TSpawnLocations& rEligibleLocations, i32 nTeam, bool bInitialSpawn) const
{
	i32k numSpawnPoints = rAllLocations.size();
	rEligibleLocations.reserve(numSpawnPoints);

	for (i32 i = 0; i < numSpawnPoints; ++ i)
	{
		EntityId spawnId = rAllLocations[i];

		if (nTeam > 0)
		{
			i32 spawnTeam = GetSpawnLocationTeam(spawnId);
...
void CGameRulesMPSpawningBase::OnInitialEquipmentScreenShown()
{
	...
	{
		DrxLog("CGameRulesMPSpawningBase::OnInitialEquipmentScreenShown() game in progress, starting auto spawn timer");
		m_fTimeTillInitialAutoSpawn = m_fInitialAutoSpawnTimer;
		m_bNeedToStartAutoReviveTimer = false;
	}
}
...
void CGameRulesMPSpawningBase::OnRoundStart()
{
	...
	{
		DrxLog("CGameRulesMPSpawningBase::OnRoundStart() equipment shown, starting auto spawn timer");
...
void CGameRulesMPSpawningBase::OnEntityKilled(const HitInfo &hitInfo)
{
	if (gEnv->bServer)
	{
		...
		DrxLog("[tlh] @ CGameRulesMPSpawningBase::OnEntityKilled(), entity=%s", (e?e->GetName():"!"));
#endif
		IActor *pActor = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(entityId);
		if (pActor != NULL && pActor->IsPlayer())
		{
			DrxLog("      > got OnEntityKilled broadcast, frame = %d", gEnv->nMainFrameID);
...
//------------------------------------------------------------------------
bool CGameRulesMPSpawningBase::IsInInitialChannelsList( u32 channelId ) const
{
	if (m_initialChannelIDs.size() != 0)
	{
		for (u32 i=0; i<m_initialChannelIDs.size(); i++)
		{
