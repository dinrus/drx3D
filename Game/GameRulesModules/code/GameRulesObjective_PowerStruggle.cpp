...
#include <drx3D/Game/GameRulesObjective_PowerStruggle.h>
...
#define DbgLog(...) DRX_DEBUG_LOG(GAMEMODE_POWERSTRUGGLE, __VA_ARGS__)
#define DbgLogAlways(...) DRX_DEBUG_LOG_ALWAYS(GAMEMODE_POWERSTRUGGLE, __VA_ARGS__)

	#define FATAL_ASSERT(expr) if (!(expr)) { DrxFatalError("CGameRulesObjective_PowerStruggle - FatalError"); }
...
void CGameRulesObjective_PowerStruggle::SNodeInfo::SetState(ENodeState inState)
{
	if (inState != m_state)
	{
		...
		DrxLog("CGameRulesObjective_PowerStruggle::SNodeInfo::SetState() entity=%s state %d -> %d", pGameRules->GetEntityName(m_id), m_state, inState);
...
float CGameRulesObjective_PowerStruggle::CalculateCapturePointQuantity(const SNodeInfo *pNodeInfo, float frameTime)
{
	...
	i32k team1Count = pNodeInfo->m_insideCount[0];
	i32k team2Count = pNodeInfo->m_insideCount[1];
	i32		deltaNumPlayersCapturing = abs(team1Count - team2Count); 
	...
	for( i32 i = 1; i < deltaNumPlayersCapturing; ++i)
	{
...
void CGameRulesObjective_PowerStruggle::Client_UpdateSoundSignalForNode(SNodeInfo *pNodeInfo, float frameTime)
{
	...
	{
		DrxWatch("Spear=%s is not playing audio signal!", m_pGameRules->GetEntityName(pNodeInfo->m_id));

		DRX_ASSERT_MESSAGE(!pNodeInfo->m_havePlayedSignal, "we should only ever not be playing our signals if we've missed playing them by being a late joiner. Any other cases indicate something has gone wrong with the low-level sound code");
		...
		{
			DrxLog("CGameRulesObjective_PowerStruggle::Client_UpdateSoundSignalForNode() spear %s has never played it's audio signal.. playing it now!", m_pGameRules->GetEntityName(pNodeInfo->m_id));
...
void CGameRulesObjective_PowerStruggle::Common_HandleSpearCaptureStarting(SNodeInfo *pNodeInfo)
{
	...
	i32 localTeamId = m_pGameRules->GetTeam(localActorId);
	...
	if (pNodeInfo->m_lastCapturedTeamId == localTeamId)
	{
		...
		switch (pNodeInfo->m_activeIdentityId)
		{
			...
			case eAII_spearC_id:
				...
				break;
			default:
				DRX_ASSERT_MESSAGE(0, string().Format("unhandled activeIdentityId=%d", pNodeInfo->m_activeIdentityId));
				break;
		}
	}
#endif
...
void CGameRulesObjective_PowerStruggle::Common_HandleCapturedSpear(SNodeInfo *pNodeInfo)
{
	if( pNodeInfo->m_soloCaptureState == ESolo_Activated )
	{
		...
		DrxFixedArray<EntityId, MAX_PLAYER_LIMIT>::iterator entIt = std::find( m_soloCapturingPlayers.begin(), m_soloCapturingPlayers.end(), pNodeInfo->m_soloCapturerId );
	...
	if (gEnv->bServer)
	{
		if (IGameRulesScoringModule* pScoringModule = m_pGameRules->GetScoringModule())
		{
			...
			i32 numInside=pNodeInfo->m_insideCount[pNodeInfo->m_controllingTeamId-1];

			for (i32 i=0; i<numInside; i++)
			{
...
void CGameRulesObjective_PowerStruggle::Common_UpdateActiveNode(SNodeInfo *pNodeInfo, float frameTime, bool &bAspectChanged)
{
	...
	for (i32 teamIndex = 0; teamIndex < NUM_TEAMS; ++ teamIndex)
	{
		...
		for (i32 playerIndex = 0, numInsideBox = boxEntities.size(); playerIndex < numInsideBox; ++ playerIndex)
		{
			...
			if ( CGameRulesObjective_PowerStruggle::CanActorCaptureNodes(pActor) )
			{
				...
				if (IsPointInsideCone(playerPos, conePos, pNodeInfo->m_controlRadius, pNodeInfo->m_controlHeight))
				{
					...
					if (pActor->IsClient())
					{
						...
						DRX_ASSERT(!m_haveDisplayedProgressBar); // only one node should cause the progress bar to be displayed
			...
			{
				DRX_ASSERT_MESSAGE(!pActor, "now that the kill listener is correctly setup this shouldn't be possible, aside from perhaps some untracked disconnect resulting in a null actor");
	...
	{
		i32 previousOwnerId = pNodeInfo->m_controllingTeamId;
		i32 team1Count = pNodeInfo->m_insideCount[0];
		i32 team2Count = pNodeInfo->m_insideCount[1];
		i32 newControllingTeamId;
		...
		if( pNodeInfo->m_state == ENS_capturing_from_capture || pNodeInfo->m_state == ENS_capturing_from_neutral )
		{
			if( pNodeInfo->m_controllingTeamId == 1 || pNodeInfo->m_controllingTeamId == 2 )
			{
				...
				if( capturingEntities.size() == 1 )
				{
					...
					{
						i32 oldTeam = (pNodeInfo->m_soloCapturerId!=-1)? m_pGameRules->GetTeam( pNodeInfo->m_soloCapturerId ) : 0;
				...
				else
				{
					...
					else
					{
						...
						if (captureStatus <= 0.f)
						{
							...
							{
								DRX_ASSERT(pNodeInfo->m_state == ENS_capturing_from_capture);
								pNodeInfo->SetState(ENS_captured);
...
void CGameRulesObjective_PowerStruggle::Server_ActivateNode(i32 index)
{
	...
	{
		DRX_ASSERT_MESSAGE(pNodeInfo->m_activeIdentityId != eAII_unknown_id, "we expect all nodes to have assigned active identity id when its been specified within the setup");
	}
	else
	{
		DRX_ASSERT_MESSAGE(pNodeInfo->m_activeIdentityId == eAII_unknown_id, "we expect inactive nodes to have no assigned active identity id");
	...
	if (m_timeTillScore < 0.f)
	{
		if (gEnv->bServer)
		{
			...
			for(i32 i = 0; i < NUM_TEAMS; ++i)
			{
				...
				{
					if( (i32)(chargeeInfo.m_PrevChargeValue*100.f) != (i32)(chargeeInfo.m_ChargeValue*100.f) ) //Crossed a 0.01f threshold then give score
					{
						if(pScoringModule)
						{
...
void CGameRulesObjective_PowerStruggle::Common_UpdateDebug(float frameTime)
{
	...
	for (i32 i = 0; i < MAX_NODES; ++ i)
	{
		...
		DrxWatch("Node: entity=%s; Invisible: %d; Hidden: %d", pEntity ? pEntity->GetName() : "NULL", pEntity ? pEntity->IsInvisible() : 0, pEntity ? pEntity->IsHidden() : 0);
	}

	i32 clientTeamId = m_pGameRules->GetTeam(g_pGame->GetIGameFramework()->GetClientActorId());

	for (i32 i = 0; i < MAX_NODES; ++ i)
	{
		SNodeInfo *pNodeInfo = &m_nodes[i];
		DrxWatch("Node: entity=%s; state=%d; teamOwner=%d; lastTeamOwner=%d; lastCapTeam=%d; capOwner=%d; ", m_pGameRules->GetEntityName(pNodeInfo->m_id), pNodeInfo->m_state, pNodeInfo->m_controllingTeamId, pNodeInfo->m_lastControllingTeamId, pNodeInfo->m_lastCapturedTeamId, pNodeInfo->m_captureStatusOwningTeam);
		DrxWatch("      capture=%.2f; activeId=%d; insideCount=%d,%d; audio: cap=%.1f; con=%.1f; com=%.1f", pNodeInfo->m_captureStatus, pNodeInfo->m_activeIdentityId, pNodeInfo->m_insideCount[0], pNodeInfo->m_insideCount[1], pNodeInfo->m_currentCapturingAudioParam, pNodeInfo->m_currentContendedAudioParam, pNodeInfo->m_currentCompletedAudioParam);
	}
	...
	{	
		...
		{	
			i32 chargePercent = static_cast<i32>(chargeeInfo.m_ChargeValue * 100.0f); 
			chargePercent = min(chargePercent, 100); 
			DrxWatch("Friendly Charge %d %%", chargePercent);	
		}
	}
	...
	for(i32 i = 0; i < NUM_TEAMS; ++i)
	{
		...
		{	
			i32 chargePercent = static_cast<i32>(chargeeInfo.m_ChargeValue * 100.0f); 
			chargePercent = min(chargePercent, 100); 
			DrxWatch("Enemy   Charge %d %%", chargePercent);	
		}
	}
...
{
	DrxLog("GameRulesObjective_PowerStruggle::OnStartGame()");

	if (g_pGame->GetHostMigrationState() != CGame::eHMS_NotMigrating)
	{
		DrxLog("GameRulesObjective_PowerStruggle::OnStartGame() we're host migrating. Early returning. We dont want to do another initialisation, all our entities are still valid");
		return;
	}
...
//------------------------------------------------------------------------
void CGameRulesObjective_PowerStruggle::PostInitClient( i32 channelId )
{
	DRX_ASSERT(gEnv->bServer);
...
bool CGameRulesObjective_PowerStruggle::NetSerializeNode(SNodeInfo *pNodeInfo, TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags)
{
	bool capStatusChanged=false;

	i32 oldControllingTeam = pNodeInfo->m_controllingTeamId;

	i32 controllingTeam = pNodeInfo->m_controllingTeamId + 1;
	ser.Value("controllingTeamId", controllingTeam, 'ui2');

	i32 newControllingTeam = controllingTeam -1;
	...
	i32 lastControllingTeam = pNodeInfo->m_lastControllingTeamId + 1;
	...
	if (ser.IsReading())
	{
		...
		if (state != pNodeInfo->m_state)
		{
			...
			if(pNodeInfo->m_state == ENS_hidden)
			{
				...
				if (pNodeEntity)
				{
					DrxLog("CGameRulesObjective_Powerstruggle::NetSerializeNode() showing node entity %s", pNodeEntity->GetName());
					pNodeEntity->Invisible(false);
				...
				{
					DrxLog("CGameRulesObjective_Powerstruggle::NetSerializeNode() failed to find node entity");
				}
...
//------------------------------------------------------------------------
bool CGameRulesObjective_PowerStruggle::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
...
//------------------------------------------------------------------------
void CGameRulesObjective_PowerStruggle::OnEntitySignal(EntityId entityId, i32 signal)
{
	if (gEnv->IsEditing())
		return;
	...
	if (gEnv->bServer)
	{
		switch(signal)
		{
			...
			default:
				DRX_ASSERT_MESSAGE(0, string().Format("OnEntitySignal() unhandled signal %d", signal));
				break;
		}
	}
...
void CGameRulesObjective_PowerStruggle::OnEntityEvent( IEntity *pEntity, SEntityEvent &event )
{
	...
	DRX_ASSERT_MESSAGE(pNodeInfo, "unable to find a node from the entity who's event we've received. This shouldn't happen");
	...
	if (event.event == ENTITY_EVENT_ENTERAREA)
	{
		...
		if ( CGameRulesObjective_PowerStruggle::CanActorCaptureNodes(pActor) )
		{
			i32 teamId = m_pGameRules->GetTeam(insideId);
			i32 teamIndex = teamId - 1;
...
//------------------------------------------------------------------------
void CGameRulesObjective_PowerStruggle::OnClientDisconnect(i32 channelId, EntityId playerId)
{
...
{
	i32 teamId = pNodeInfo->m_lastCapturedTeamId;
	
	if (teamId > 0 && gEnv->IsClient())
	{
		bool announcedAll=false;

		i32 team1Count=0, team2Count=0;
		for (i32 i = 0; i < MAX_NODES; ++ i)
		{
...
void CGameRulesObjective_PowerStruggle::Client_SetIconForNode( SNodeInfo *pNodeInfo )
{
	...
	switch(pNodeInfo->m_state)
	{
		...
		case ENS_captured:	
			...
			{
				DRX_ASSERT(pNodeInfo->m_lastCapturedTeamId == 2);
				iconType = team2Node;
	...
	if (iconType != pNodeInfo->m_currentObjectiveIconType && g_pGame->GetIGameFramework()->GetClientActor() != NULL)
	{
		...
		{			
			tukk pText = s_nodeIdentityInfo[pNodeInfo->m_activeIdentityId].pIconName;
...
//------------------------------------------------------------------------
void CGameRulesObjective_PowerStruggle::Common_AddNode(IEntity *pNodeEntity, i32 indexToUse)
{
	DrxLog("CGameRulesObjective_PowerStruggle::Common_AddNode() adding %s", pNodeEntity->GetName());
	...
	{
		DrxLog("CGameRulesObjective_PowerStruggle::Common_AddNode() pNodeEntity=%s has specified letter of %s", pNodeEntity->GetName(), letter);
		...
		{
			DrxLog("CGameRulesObjective_PowerStruggle::Common_AddNode() setting activeIdentityId to eAII_spearA_id");
			pNodeInfo->m_activeIdentityId = eAII_spearA_id;
		}
		else if (letter[0]=='b' || letter[0]=='B')
		{
			DrxLog("CGameRulesObjective_PowerStruggle::Common_AddNode() setting activeIdentityId to eAII_spearB_id");
			pNodeInfo->m_activeIdentityId = eAII_spearB_id;
		}
		else if (letter[0]=='c' || letter[0]=='C')
		{
			DrxLog("CGameRulesObjective_PowerStruggle::Common_AddNode() setting activeIdentityId to eAII_spearC_id");
			pNodeInfo->m_activeIdentityId = eAII_spearC_id;
		}
		else if (!gEnv->IsEditor())
		{
			DrxLog("CGameRulesObjective_PowerStruggle::Common_AddNode() failed to set an activeIdentityId from letter string=%s", letter);
...
void CGameRulesObjective_PowerStruggle::Common_RemovePlayerFromNode(EntityId playerId, SNodeInfo *pNodeInfo)
{
	for (i32 teamIndex = 0; teamIndex < NUM_TEAMS; ++ teamIndex)
	{
		i32 numInside = pNodeInfo->m_insideBoxEntities[teamIndex].size();
		for (i32 i = 0; i < numInside; ++ i)
		{
...
{
	for (i32 i = 0; i < MAX_NODES-1; i++)
	{
		if(m_nodes[i].m_id > m_nodes[i+1].m_id)
			DrxFatalError("Node entity IDs are out of order, sorting code needs to be reinstated");
	}

#if 0
	for(i32 i = 0; i < MAX_NODES - 1; ++i)
	{
		for(i32 j = i+1; j < MAX_NODES; ++j)
		{
...
i32 CGameRulesObjective_PowerStruggle::Server_CalculateWinningTeam()
{
...
bool CGameRulesObjective_PowerStruggle::Client_UpdateCaptureProgressBar( const SNodeInfo *pNodeInfo )
{
	...
	case ECaptureFromFriendly:
		...
		break;
	default:
		DRX_ASSERT_MESSAGE(0, string().Format("unhandled capturingFrom=%d", capturingFrom));
		break;
	}
...
void CGameRulesObjective_PowerStruggle::LogSpearStateToTelemetry( u8 state, SNodeInfo* pSpearInfo )
{
...
{
	DrxFixedArray<EntityId, MAX_PLAYER_LIMIT>::iterator entIt = std::find( m_soloCapturingPlayers.begin(), m_soloCapturingPlayers.end(), playerId );
	if( entIt != m_soloCapturingPlayers.end() )
	{
		//found
		return true;
