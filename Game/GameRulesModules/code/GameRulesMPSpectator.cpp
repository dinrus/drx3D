...
#include <drx3D/Game/GameRulesMPSpectator.h>
...
{
	DrxLog("CGameRulesMPSpectator::Init()");
	...
	{
		DRX_ASSERT_MESSAGE(0, "CGameRulesMPSpectator failed to find valid eatsActorActions param");
	}
	...
	{
		DRX_ASSERT_MESSAGE(0, "CGameRulesMPSpectator failed to find valid enableFree param");
	}
	...
	{
		DRX_ASSERT_MESSAGE(0, "CGameRulesMPSpectator failed to find valid enableFollow param");
	}
	...
	{
		DRX_ASSERT_MESSAGE(0, "CGameRulesMPSpectator failed to find valid timeFromDeathTillAutoSpectate param");
	}

	DrxLog("Init() set params to eatsActorActions=%d; enableFree=%d; enableFollow=%d; enableKiller=%d, timeFromDeathTillAutoSpectate=%.1f", m_eatsActorActions, m_enableFree, m_enableFollow, m_enableKiller, m_timeFromDeathTillAutoSpectate);
}

// -----------------------------------------------------------------------
void CGameRulesMPSpectator::PostInit()
{
	DrxLog("CGameRulesMPSpectator::PostInit()");
}
...
// -----------------------------------------------------------------------
bool CGameRulesMPSpectator::ModeIsEnabledForPlayer(i32k mode, const EntityId playerId) const
{
	bool  is = false;
	switch (mode)
	{
...
// -----------------------------------------------------------------------
bool CGameRulesMPSpectator::ModeIsAvailable(const EntityId playerId, i32k mode, EntityId* outOthEnt)
{
	bool  is = false;
	EntityId  othEnt = 0;
...
i32 CGameRulesMPSpectator::GetBestAvailableMode(const EntityId playerId, EntityId* outOthEnt)
{
	...
	{
		DRX_ASSERT_MESSAGE(ModeIsAvailable(playerId, CActor::eASM_Free, outOthEnt), "DESIGNER ASSERT: all levels need at least one of either a spectator point");
...
// -----------------------------------------------------------------------
i32 CGameRulesMPSpectator::GetNextMode(EntityId playerId, i32 inc, EntityId* outOthEid)
{
	DRX_ASSERT(inc==-1 || inc==1);
	//inc = ((inc < 0) ? -1 : 1);
	i32  mode = (i32) CActor::eASM_None;
	EntityId  othEntId = 0;
...
// -----------------------------------------------------------------------
void CGameRulesMPSpectator::SvOnChangeSpectatorMode( EntityId playerId, i32 mode, EntityId targetId, bool resetAll )
{
	DrxLog("CGameRulesMPSpectator::SvOnChangeSpectatorMode");
	ASSERT_IS_SERVER;
...
// -----------------------------------------------------------------------
void CGameRulesMPSpectator::SvRequestSpectatorTarget( EntityId playerId, i32 change )
{
	DrxLog("CGameRulesMPSpectator::SvRequestSpectatorTarget");
	ASSERT_IS_SERVER;

}

// -----------------------------------------------------------------------
void CGameRulesMPSpectator::ClOnChangeSpectatorMode( EntityId playerId, i32 mode, EntityId targetId, bool resetAll )
{
	DrxLog("CGameRulesMPSpectator::ClOnChangeSpectatorMode");
	ASSERT_IS_CLIENT;

}

// -----------------------------------------------------------------------
EntityId CGameRulesMPSpectator::GetNextSpectatorTarget( EntityId playerId, i32 change )
{
	...
	if(pActor)
	{
		...
		i32  teamCount = m_pGameRules->GetTeamCount();
		i32  friendlyTeam = m_pGameRules->GetTeam(playerId);

		i32 teamIndex = max(friendlyTeam - 1, 0);
		if ((teamCount == 0) || (friendlyTeam == 0))
		{
			...
			i32  numStats = pPlayStatsMod->GetNumPlayerStats();
			for (i32 i=0; i<numStats; i++)
			{
				...
				if (!loopPlr)
				{
					DrxLog(" > skipping entity because its actor is NULL");
					continue;
				}
		...
		i32 index = (specTarg ? 0 : numPlayers);  // (only do the for-loop if we've actually got something to find)
		...
		if(numPlayers > 0)
		{
			i32 newTargetIndex = index;
			i32 numAttempts = numPlayers;
			do
			{
...
tukk CGameRulesMPSpectator::GetActorSpectatorModeName(u8 mode)
{
	static tukk  modenames[] = {
...
void CGameRulesMPSpectator::ChangeSpectatorMode(const IActor *pActor, u8 mode, EntityId targetId, bool resetAll, bool force)
{
	if (!pActor)
		return;
		
	DrxLog("[tlh] @ CGameRulesMPSpectator::ChangeSpectatorMode(), actor='%s', mode=%u, target=%u, resetAll=%s, force=%s, server=%s", pActor->GetEntity()->GetName(), mode, targetId, resetAll ? "true" : "false", force ? "true" : "false", gEnv->bServer ? "true" : "false");
	...
	else if (pActor->IsClient())
	{
		if(m_ClientInfo.m_mode != mode || m_ClientInfo.m_targetId != targetId || m_ClientInfo.m_fRequestTimer < 0.0f)
		{
			...
			if(IDrxLobby * pLobby = gEnv->pNetwork->GetLobby())
			{
				DrxPing ping = 0;
				if(CGameLobby * pGameLobby = g_pGame->GetGameLobby())
				{
					SDrxMatchMakingConnectionUID mmCxUID = pGameLobby->GetConnectionUIDFromChannelID(pActor->GetChannelId());
...
{
	EntityId  othEnt;	
	i32  mode = GetBestAvailableMode(pActor->GetEntityId(), &othEnt);
...
//------------------------------------------------------------------------
i32 CGameRulesMPSpectator::GetSpectatorLocationCount() const
{
	return (i32)m_spectatorLocations.size();
}

//------------------------------------------------------------------------
EntityId CGameRulesMPSpectator::GetSpectatorLocation(i32 idx) const
{
	if (idx >= 0 && idx < (i32)m_spectatorLocations.size())
...
{
	EntityId  e = 0;
	if (i32 count=GetSpectatorLocationCount())
		e = GetSpectatorLocation(drx_random(0, count - 1));
	return e;
}
...
{
	DrxLog("[tlh] @ CGameRulesMPSpectator::GetNextSpectatorLocation()");
	i32k  sz = m_spectatorLocations.size();
	if (sz > 0)
	{
		EntityId  cur = pPlayer->GetSpectatorFixedLocation();
		DrxLog("[tlh] > has cur loc entity %d...", cur);
		i32  idx = (cur ? 0 : sz);  // no need to search for cur if haven't even got one
		for (; idx<sz; idx++)
		{
		...
		{
			DrxLog("[tlh] > cur %d found at idx %d...", cur, idx);
			idx++;
		}
		if (idx == sz)
		{
			EntityId  ret = m_spectatorLocations[0];
			DrxLog("[tlh] > cur at end, returning first entry, %d", ret);
			return ret;
		}
		else
		{
			EntityId  ret = m_spectatorLocations[idx];
			DrxLog("[tlh] > cur NOT at end, returning next entry, %d", ret);
			return ret;
		}
	}
	DrxLog("[tlh] > no spectator locs in level or gamemode, returning 0");
	return 0;
}
...
{
	DrxLog("[tlh] @ CGameRulesMPSpectator::GetPrevSpectatorLocation()");
	i32k  sz = m_spectatorLocations.size();
	if (sz > 0)
	{
		EntityId  cur = pPlayer->GetSpectatorFixedLocation();
		...
		if (idx < 0)
		{
			...
			DrxLog("[tlh] > cur at beginning, returning last entry, %d", ret);
			return ret;
		}
		else
		{
			EntityId  ret = m_spectatorLocations[idx];
			DrxLog("[tlh] > cur NOT at beginning, returning previous entry, %d", ret);
			return ret;
		}
	}
	DrxLog("[tlh] > no spectator locs in level or gamemode, returning 0");
...
// -----------------------------------------------------------------------
bool CGameRulesMPSpectator::GetModeFromChannelSpectatorMap(i32 channelId, i32* outMode) const
{
	bool  found = false;
...
// -----------------------------------------------------------------------
void CGameRulesMPSpectator::FindAndRemoveFromChannelSpectatorMap(i32 channelId)
{
...
void CGameRulesMPSpectator::GetPostDeathDisplayDurations( EntityId playerId, i32 teamWhenKilled, bool isSuicide, bool isBulletTimeKill, float& rDeathCam, float& rKillerCam, float& rKillCam ) const
{
	i32k team = teamWhenKilled;
