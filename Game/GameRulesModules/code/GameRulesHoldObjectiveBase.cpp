...
#include <drx3D/Game/GameRulesHoldObjectiveBase.h>
...
CGameRulesHoldObjectiveBase::~CGameRulesHoldObjectiveBase()
{
	...
	{
		DrxLog("CGameRulesHoldObjectiveBase::CGameRulesHoldObjectiveBase() removing movie listener");
	...
	for (i32 childIdx = 0; childIdx < numChildren; ++ childIdx)
	{
		...
		if (!stricmp(xmlChild->getTag(), "SpawnParams"))
		{
			...
			if (xmlChild->getAttr("type", &pType))
			{
				...
				{
					DrxLog("CGameRulesHoldObjectiveBase::Init: ERROR: Unknown spawn point of interest type ('%s')", pType);
				}
...
{
	i32k numChildren = xmlEffectNode->getChildCount();
	for(i32 childIdx=0; childIdx<numChildren; childIdx++)
	{
		XmlNodeRef xmlChild = xmlEffectNode->getChild(childIdx);
		if(!stricmp(xmlChild->getTag(), "ParticleEffect"))
		{
			tukk pParticleEffectName = NULL;
			if (xmlChild->getAttr("name", &pParticleEffectName))
			{
				m_effectData.pParticleEffect = gEnv->pParticleUpr->FindEffect(pParticleEffectName, "CGameRulesHoldObjectiveBase");
				if(m_effectData.pParticleEffect)
				{
					m_effectData.pParticleEffect->AddRef();
				}
			}
			tukk pParticleGeomMaterialName = NULL;
			if (xmlChild->getAttr("particleGeomMaterial", &pParticleGeomMaterialName))
			{
				m_effectData.pParticleGeomMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(pParticleGeomMaterialName);
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::AddEntityId(i32 type, EntityId entityId, i32 index, bool isNewEntity, const CTimeValue &timeAdded)
{
	if(index >= HOLD_OBJECTIVE_MAX_ENTITIES)
	{
		DrxFatalError("Too Many Hold Objective Entities Active!");
	}
	DrxLog("CGameRulesHoldObjectiveBase::AddEntityId() received objective, eid=%i, index=%i, isNewEntity=%s, fTimeAdded=%" PRIi64 ", adding to pending queue", entityId, index, isNewEntity ? "true" : "false", timeAdded.GetMilliSecondsAsInt64());
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::DoAddEntityId(i32 type, EntityId entityId, i32 index, bool isNewEntity)
{
	DRX_ASSERT(index < HOLD_OBJECTIVE_MAX_ENTITIES);
	DrxLog("CGameRulesHoldObjectiveBase::DoAddEntityId() received objective, eid=%i, index=%i", entityId, index);
	...
	if (pEntity)
	{
		...
		DRX_TODO(11, 02, 2009, "function name from xml?");
		if (pScript)
		{
			if (pScript->GetValueType("ActivateCapturePoint") == svtFunction)
			{
				if (isNewEntity)
				{
					...
					if (!m_bAddedMovieListener)
					{
						...
						{
							DrxLog("CGameRulesHoldObjectiveBase::CGameRulesHoldObjectiveBase() adding movie listener");
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::RemoveEntityId( i32 type, EntityId entityId )
{
	for (i32 i = 0; i < HOLD_OBJECTIVE_MAX_ENTITIES; ++ i)
	{
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::ClearEntities(i32 type)
{
	for (i32 i = 0; i < HOLD_OBJECTIVE_MAX_ENTITIES; ++ i)
	{
		SHoldEntityDetails *pDetails = &m_entities[i];
...
void CGameRulesHoldObjectiveBase::OnEntityEvent( IEntity *pEntity,SEntityEvent &event )
{
	...
	i32 teamId = g_pGame->GetGameRules()->GetTeam(insideId);
		...
		if (pDetails)
		{
			if (event.event == ENTITY_EVENT_ENTERAREA)
			{
				if (IsActorEligible(pActor))
				{
					...
					DrxLog("CGameRulesHoldObjectiveBase::OnEntityEvent(), entity '%s' entered capture entity", pActor->GetEntity()->GetName());
				}
			}
			else if (event.event == ENTITY_EVENT_LEAVEAREA)
			{
				...
				DrxLog("CGameRulesHoldObjectiveBase::OnEntityEvent(), entity '%s' left capture entity", pActor->GetEntity()->GetName());
			}
...
void CGameRulesHoldObjectiveBase::InsideStateChanged(SHoldEntityDetails *pDetails)
{
	...
	i32 team1Count = pDetails->m_insideCount[0];
	i32 team2Count = pDetails->m_insideCount[1];

	i32 oldControllingTeamId = pDetails->m_controllingTeamId;
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::DetermineControllingTeamId(SHoldEntityDetails *pDetails, i32k team1Count, i32k team2Count)
{
	if (team1Count && !team2Count)
	{
		pDetails->m_controllingTeamId = 1;
		DrxLog("CGameRulesHoldObjectiveBase::InsideStateChanged: entity controlled by team 1");
		CCCPOINT(HoldObjective_TeamMarinesNowInProximity);
	}
	else if (!team1Count && team2Count)
	{
		pDetails->m_controllingTeamId = 2;
		DrxLog("CGameRulesHoldObjectiveBase::InsideStateChanged: entity controlled by team 2");
		CCCPOINT(HoldObjective_TeamCellNowInProximity);
	}
	else if (!team1Count && !team2Count)
	{
		pDetails->m_controllingTeamId = 0;
		DrxLog("CGameRulesHoldObjectiveBase::InsideStateChanged: entity controlled by neither team");
		CCCPOINT(HoldObjective_NeitherTeamNowInProximity);
	}
	else
	{
		pDetails->m_controllingTeamId = CONTESTED_TEAM_ID;
		DrxLog("CGameRulesHoldObjectiveBase::InsideStateChanged: entity is contested");
...
void CGameRulesHoldObjectiveBase::OnLocalPlayerInsideStateChanged( SHoldEntityDetails *pDetails )
{
	if (pDetails->m_localPlayerIsWithinRange)
	{
		...
		else
		{
			...
			i32k localTeamId = g_pGame->GetGameRules()->GetTeam(localActorId);
...
//------------------------------------------------------------------------
// NOTE CGameRulesCombiCaptureObjective now overrides this completely (eg. for Assault)
void CGameRulesHoldObjectiveBase::OnControllingTeamChanged( SHoldEntityDetails *pDetails, i32k oldControllingTeam )
{
	if (pDetails->m_localPlayerIsWithinRange)
	{
		if (pDetails->m_controllingTeamId == CONTESTED_TEAM_ID)
...
//------------------------------------------------------------------------
bool CGameRulesHoldObjectiveBase::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
	if (aspect == HOLD_OBJECTIVE_STATE_ASPECT)
	{
		for (i32 i = 0; i < HOLD_OBJECTIVE_MAX_ENTITIES; ++ i)
		{
			...
			i32 oldTeam1Count = pDetails->m_insideCount[0];
			i32 oldTeam2Count = pDetails->m_insideCount[1];

			i32 team1Count = pDetails->m_insideEntities[0].size();
			i32 team2Count = pDetails->m_insideEntities[1].size();
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::OnClientDisconnect( i32 channelId, EntityId playerId )
{
	for (i32 i = 0; i < HOLD_OBJECTIVE_MAX_ENTITIES; ++ i)
	{
		SHoldEntityDetails *pDetails = &m_entities[i];
		...
		for (i32 teamIdx = 0; teamIdx < 2; ++ teamIdx)
		{
...
void CGameRulesHoldObjectiveBase::OnEntityKilled( const HitInfo &hitInfo )
{
	if (gEnv->bServer && !hitInfo.hitViaProxy)
	{
		...
		{
			i32k  defKillPoints = pScoringModule->GetPlayerPointsByType(EGRST_HoldObj_DefensiveKill);
			i32k  offKillPoints = pScoringModule->GetPlayerPointsByType(EGRST_HoldObj_OffensiveKill);
...
void CGameRulesHoldObjectiveBase::AwardPlayerPoints( TEntityIdVec *pEntityVec, EGRST scoreType )
{
	...
	if (pScoringModule)
	{
		i32 numEntities = pEntityVec->size();
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::ReadAudioSignal( const XmlNodeRef node, tukk name, CAudioSignalPlayer* signalPlayer )
{
	if(node->haveAttr(name))
	{
		char signalName[32];
		drx_strcpy(signalName, node->getAttr(name));
...
CGameRulesHoldObjectiveBase::eRadiusPulseType CGameRulesHoldObjectiveBase::GetPulseType(SHoldEntityDetails *pDetails) const
{
	...
	if(pDetails)
	{
		i32k localTeamId = g_pGame->GetGameRules()->GetTeam(g_pGame->GetIGameFramework()->GetClientActorId());
...
void CGameRulesHoldObjectiveBase::Update( float frameTime )
{
	if (m_bHasNetSerialized && !gEnv->bServer)
	{
		for (i32 i = 0; i < HOLD_OBJECTIVE_MAX_ENTITIES; ++ i)
		{
			...
			i32 team1Count = pDetails->m_serializedInsideCount[0];
			i32 team2Count = pDetails->m_serializedInsideCount[1];
	...
	if (m_pStartingAnimSequence && gEnv->pMovieSystem)
	{
		...
		if (m_deferredTrackViewTime < 0.f)
		{
			...
			DrxLog("CGameRulesHoldObjectiveBase::Update() anim sequence %s at %f seconds (should be at least %f)", m_pStartingAnimSequence->GetName(), playingTime, timeSinceStarted);
...
void CGameRulesHoldObjectiveBase::CheckCylinder( SHoldEntityDetails *pDetails, EntityId localPlayerId )
{
	...
	if (pHoldEntity)
	{
		...
		for (i32 teamIdx = 0; teamIdx < NUM_TEAMS; ++ teamIdx)
		{
			i32k previousNumEntities = pDetails->m_insideEntities[teamIdx].size();
			pDetails->m_insideEntities[teamIdx].clear();

			const TEntityIdVec &boxEntities = pDetails->m_insideBoxEntities[teamIdx];
			i32k numBoxEntities = boxEntities.size();
			pDetails->m_totalInsideBoxCount += numBoxEntities;
			for (i32 entIdx = 0; entIdx < numBoxEntities; ++ entIdx)
			{
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::OnChangedTeam( EntityId entityId, i32 oldTeamId, i32 newTeamId )
{
...
void CGameRulesHoldObjectiveBase::CheckLocalPlayerInside(SHoldEntityDetails *pDetails, const IEntity *pHoldEntity, const IEntity *pLocalPlayer)
{
	...
	i32k teamId = g_pGame->GetGameRules()->GetTeam(localPlayerId);
	i32k teamIdx = teamId - 1;
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::Announce(tukk announcement, TAnnounceType inType, const bool shouldPlayAudio) const
{
	if(shouldPlayAudio)
	{
...
void CGameRulesHoldObjectiveBase::OnMovieEvent( IMovieListener::EMovieEvent movieEvent, IAnimSequence* pAnimSequence )
{
	...
	{
		DrxLog("CGameRulesHoldObjectiveBase::OnMovieEvent() expected trackview has started - %s; setting eSeqFlags_NoMPSyncingNeeded flag on it", pAnimSequence->GetName());
