...
: m_pGameRules(NULL)
{
	DrxLog("GameRulesCommonDamageHandling::GameRulesCommonDamageHandling()");
...
CGameRulesCommonDamageHandling::~CGameRulesCommonDamageHandling()
{
	DrxLog("CGameRulesCommonDamageHandling::~GameRulesCommonDamageHandling()");
}
...
void CGameRulesCommonDamageHandling::Init( XmlNodeRef xml )
{
	...
	DRX_ASSERT(m_pGameRules->GetHitTypesCount() == CGameRules::EHitType::Unreserved);
	...
	if( xmlNode )
	{
		i32k numEntries = xmlNode->getChildCount();
		for (i32 i = 0; i < numEntries; ++i)
		{
			...
			if( tukk pHitType = hitTypeXML->getAttr("name") )
			{
				TBitfield flags = CGameRules::EHitTypeFlag::None;

				if( tukk  pHitTypeFlags = hitTypeXML->getAttr("flags"))
				{
...
{
	DrxLog("CGameRulesCommonDamageHandling::PostInit()");
}
...
//------------------------------------------------------------------------
bool CGameRulesCommonDamageHandling::AllowHitIndicatorForType(i32 hitTypeId)
{
...
//------------------------------------------------------------------------
void CGameRulesCommonDamageHandling::ClProcessHit(Vec3 dir, EntityId shooterId, EntityId weaponId, float damage, u16 projectileClassId, u8 hitTypeId)
{
	tukk hit = g_pGame->GetGameRules()->GetHitType(hitTypeId);
	if(hit)
	{
...
float CGameRulesCommonDamageHandling::GetCollisionEnergy( const IEntity *pVictim, const CGameRules::SCollisionHitInfo& colHitInfo ) const
{
	...
	if (debugColl)
	{
		DrxLog("GetCollisionEnergy %s (%.1f) <-> %s (%.1f)", pVictim?pVictim->GetName():"[no entity]", m0, pOffender?pOffender->GetName():"[no entity]", m1);
	}
	...
	if (bCollider)	// non-static
	{
		...
		if (debugColl)
		{
			...
			{
				DrxLog("normal velocities: rel %.1f, <%s> %.1f / <%s> %.1f", sqrt(vrelSq), pVictim?pVictim->GetName():"none", v0dotN, pOffender?pOffender->GetName():"none", v1dotN); 
				DrxLog("target_type: %i, target_velocity: %.2f %.2f %.2f", colHitInfo.target_type, colHitInfo.target_velocity.x, colHitInfo.target_velocity.y, colHitInfo.target_velocity.z);
			}
		}
	}
	else
	...
	if (pVictim != NULL && pOffender != NULL)
	{
		if(IActor* pVictimActor = g_pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pVictim->GetId()))
		{
			...
			if (debugColl)
			{
				DrxLog("colliderEnergyScale: %.1f", colliderEnergyScale);
			}
		}
	}
...
float CGameRulesCommonDamageHandling::GetCollisionDamageMult( const IEntity *pEntity, const IEntity *pCollider, const CGameRules::SCollisionHitInfo& colHitInfo ) const
{
	...
	if (colliderInfo.pEntity)
	{
		...
		{
			DrxLog("<%s>: collider <%s> has ForeignCollisionMult %.2f", pEntity ? pEntity->GetName() : "NULL", pCollider ? pCollider->GetName() : "NULL", damageMultiplier);
		}
	}
	...
	if (entityInfo.pEntity)
	{
		...
		{
			DrxLog("<%s>: returned SelfCollisionMult %.2f", pEntity ? pEntity->GetName() : "NULL", damageMultiplier);
		}
	}
...
float CGameRulesCommonDamageHandling::GetAIPlayerAgainstColliderEnergyScale(const IEntity& collider) const
{
	...
	if (pColliderActor)
	{
		u8 physicsAspectProfile = pColliderActor->GetGameObject()->GetAspectProfile(eEA_Physics) ;
...
float CGameRulesCommonDamageHandling::GetVehicleForeignCollisionMultiplier( const IVehicle& vehicle, const SCollisionEntityInfo& colliderInfo, const CGameRules::SCollisionHitInfo& colHitInfo ) const
{
	...
	if (colliderInfo.pEntityVehicle)
	{
		...
		if ((vehicleMass > vehicleColliderMass * 1.5f) && (targetSpeedSqr > 0.01f))
		{
			...
			if (DebugCollisions())
			{
				DrxLog("Vehicle/Vehicle (%s <- %s), collision mult: %.2f", vehicle.GetEntity()->GetName(), colliderInfo.pEntity->GetName(), result);
			}
		}
	}
...
void CGameRulesCommonDamageHandling::LogHit(const HitInfo& hit, bool extended, bool dead)
{
	...
	DrxLog("'%s' hit '%s' for %f with '%s'... %s", shooter?shooter->GetName():"null", target?target->GetName():"null", hit.damage, weapon?weapon->GetName():"null",dead?"*DEADLY*":"");

	if (extended)
	{
		DrxLog("  shooterId..: %d", hit.shooterId);
		DrxLog("  targetId...: %d", hit.targetId);
		DrxLog("  weaponId...: %d", hit.weaponId);
		DrxLog("  type.......: %d", hit.type);
		DrxLog("  material...: %d", hit.material);
		DrxLog("  damage.....: %f", hit.damage);
		DrxLog("  partId.....: %d", hit.partId);
		DrxLog("  pos........: %f %f %f", hit.pos.x, hit.pos.y, hit.pos.z);
		DrxLog("  dir........: %f %f %f", hit.dir.x, hit.dir.y, hit.dir.z);
		DrxLog("  radius.....: %.3f", hit.radius);
		DrxLog("  remote.....: %d", hit.remote);

		if (IActor* targetActor = gEnv->pGame->GetIGameFramework()->GetIActorSystem()->GetActor(hit.targetId))
			DrxLog("  health.....: %f", targetActor->GetHealth());
		else
			DrxLog("  health.....: N/A");
	}
}
