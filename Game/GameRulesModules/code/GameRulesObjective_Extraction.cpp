...
#include <drx3D/Game/GameRulesObjective_Extraction.h>
...
//#include <drx3D/Game/DrxFixedString.h>
...
i32k k_defaultExtractionDebugging=0;	
...
//tukk localisedMessage = CHUDUtils::LocalizeString(m_gameStateFriendlyString.c_str());
	...
	for (i32 i = 0; i < numChildren; ++ i)
	{
		...
		if (!stricmp(xmlChild->getTag(), "Pickup"))
		{
			...
			if (m_pickups.size() < m_pickups.max_size())
			{
				...
				tukk pSuitModeName=NULL;
				if (xmlChild->getAttr("suitMode", &pSuitModeName))
				{
					DrxFixedStringT<128> suitModeStr;
...
void CGameRulesObjective_Extraction::DebugMe()
{
	...
	if (pRoundsModule)
	{
		...
		DrxWatch("primaryTeam Attacking=%d; secondaryTeam Defending=%d", primaryTeam, secondaryTeam);

		i32 localTeamId = pGameRules->GetTeam(localActorId);
		
		DrxWatch("local player: team=%d; %s", localTeamId, localTeamId == primaryTeam ? "attacking" : "defending" );
	}

	bool anyDropped=false;

	i32 numPickups=m_pickups.size();
	for (i32 i=0; i<numPickups; i++)
	{
		SPickup &pickup = m_pickups[i];
		...
		if (pickup.m_state == ePickupState_Dropped)
		{
			...
			for (u32 j=0; j<pickup.m_droppedDefenders.size(); j++)
			{
				DrxWatch("  defenders[%d]=%s (%d) for %.2fs", j, pGameRules->GetEntityName(pickup.m_droppedDefenders[j].m_playerId), pickup.m_droppedDefenders[j].m_playerId, pickup.m_droppedDefenders[j].m_timeInRange);
			}
		}
	}
	...
	tukk useButton = m_useButtonHeld ? "true" : "false";
	tukk attemptPickup = m_attemptPickup ? "true" : "false";

	DrxWatch("m_entPickupableByButton=%d (%s); m_useButtonHeld=%s; m_attemptPickup=%s",	pickupId, pickupableEntity ? pickupableEntity->GetName() : "NULL", useButton, attemptPickup);

	if (anyDropped)
	{
		float serverTime = pGameRules->GetServerTime();
		DrxWatch("serverTime=%.2f", serverTime/1000.f);
	}
}
...
void CGameRulesObjective_Extraction::UpdateButtonPresses()
{
	...
	if (pClientPlayer && m_useButtonHeld && m_attemptPickup)
	{
		...
		if(interaction.interactionType == eInteraction_GameRulesPickup)
		{
			...
			DRX_ASSERT_MESSAGE(pickup, "UpdateButtonPresses() has failed to find a pickup for the ent pickupableByButton. this shouldn't happen!");
...
{
	DRX_ASSERT(pickup);
	DRX_ASSERT(pickup->m_state == ePickupState_Dropped);

	IEntity *pickupEntity = gEnv->pEntitySystem->GetEntity(pickup->m_spawnedPickupEntityId);
	
	CGameRules *pGameRules = g_pGame->GetGameRules();
	i32 pickupTeamId = pGameRules->GetTeam(pickup->m_spawnedPickupEntityId);
	DRX_ASSERT(pickupTeamId);
	...
	for (i32 i = 0; i < numPlayers; ++ i)
	{
		...
		{
			// in range
			i32 numPlayersInRange = pickup->m_droppedDefenders.size();
			i32 j;
...
void CGameRulesObjective_Extraction::Update(float frameTime)
{
	...
	for (i32 i=0; i<numPickups; i++)
	{
		...
		if (pickup->m_state == ePickupState_Dropped)
		{
			if(pickup->m_timeSinceDropped >= pickup->m_timeToCheckForbiddenArea)
			{
				...
				if(pEntity)
				{
					if(pGameRules->IsInsideForbiddenArea(pEntity->GetWorldPos(), true, NULL))
					{
						...
						DrxLog("Resetting due to being inside the ForbiddenArea");
					}
				}
			...
			i32 timeLeftFrame = min(100, max(2, static_cast<i32>(100*timeLeftFraction)));

			if (timeLeftFrame != pickup->m_currentObjectiveCountDown)
			{
				SHUDEvent newUpdateObjectiveClock(eHUDEvent_ObjectiveUpdateClock);
				newUpdateObjectiveClock.AddData(static_cast<i32>(pickup->m_currentObjectiveFollowEntity));
	...
	i32k numExtractAts=m_extractAtElements.size();
	for (i32 i=0; i<numExtractAts; i++)
	{
	...
	for (i32 i=0; i<numPickups; i++)
	{
		...
		if (pickup->m_state == ePickupState_Dropped)
		{
			if (pickup->m_timeSinceDropped > m_droppedRemoveTime)
			{
				...
				{
					i32 numDefenders = pickup->m_droppedDefenders.size();
					for (i32 j=0; j<numDefenders; j++)
					{
						SDefender &defender = pickup->m_droppedDefenders[j];
	...
	CGameRulesObjective_Extraction::GetSpawnedPickupRMITypeFromSuitMode(EExtractionSuitMode suitMode)
{
	...
	switch(suitMode)
	{
		...
		case eExtractionSuitMode_Stealth:
			...
			break;
		default:
			DRX_ASSERT_MESSAGE(0, string().Format("GetSpawnedPickupRMITypeFromSuitMode() has encountered and unhandled suitmode=%d", suitMode));
			break;
	}
...
{	
	...
	DRX_ASSERT_MESSAGE(pickup, "failed to find pickup for entity id");
	if (pickup)
	{
		...
		DRX_ASSERT_MESSAGE(pickupEntity, "NewFallbackGunForPickup() failed to get the pickup entity");

		if (pickupEntity)
		{
			IEntity *spawnedFallbackGun = gEnv->pEntitySystem->GetEntity(fallbackGunEntityId);
			DRX_ASSERT_MESSAGE(spawnedFallbackGun, "NewFallbackGunForPickup() failed to get the hammer entity");
...
void CGameRulesObjective_Extraction::SpawnAllPickups()
{
	...
	if (pRoundsModule)
	{
		...
		i32 numSpawnAts=m_spawnAtElements.size();
		for (i32 i=0; i<numSpawnAts; i++)
		{
		...
		for (i32 i=0; i<numPickups; i++)
		{
			...
			if (pickup->m_entityClass)
			{
				...
				if (spawnAtEntityId)
				{
					...
					tukk enumStr=s_extractionSuitModeListStr[pickup->m_suitModeEffected];
					tukk enumName=strchr(enumStr, '_');
					if (enumName)
					{
						enumName++;
					}
					...
					DRX_ASSERT_MESSAGE(spawnedMagicHammer, "failed to spawn magic hammer. This shouldn't happen");
				...
				{
					DRX_ASSERT_MESSAGE(0, "Failed to find and entityId to spawn at. Unable to spawn a pickup!!!");
				}
			}
			else
			{
				DRX_ASSERT_MESSAGE(0, "failed to try and spawn pickup. entityClass is invalid");
			}
		}

		DRX_ASSERT_MESSAGE(gEnv->bServer, "we're about to change aspects after spawning pickups, we better be on the server, and we aint!!");
...
void CGameRulesObjective_Extraction::OnStartGame()
{
	if (gEnv->bServer)
	{
		...
		if (m_haveStartedGame)
		{
			...
			i32 numExtractAts=m_extractAtElements.size();
			for (i32 i=0; i<numExtractAts; i++)
			{
				SExtractAt &extractAt = m_extractAtElements[i];
	...
	if (m_spawnAtEntityClass)
	{
		...
		if (pRoundsModule)
		{
			...
			while(pEntity = pIt->Next())
			{
				...
				i32 teamId = g_pGame->GetGameRules()->GetTeam(entId);
...
bool CGameRulesObjective_Extraction::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
	return true;
}
...
EntityId CGameRulesObjective_Extraction::FindNewEntityIdToSpawnAt()
{
	...
	{
		i32 index=g_pGame->GetRandomNumber() % m_availableSpawnAtElements.size();
		ret=m_availableSpawnAtElements[index]->m_entityId;
		m_availableSpawnAtElements.removeAt(index);
	}
	else
	{
		DRX_ASSERT_MESSAGE(0, "Failed to find an entityId to spawn at. No available spawnAts left!");
	}
...
void AttachToDevice(IEntity *deviceEnt, EntityId tickEnt, bool attach)
{
	...
	if (pCharInst)
	{
		IAttachment *pAttachment = pCharInst->GetIAttachmentUpr()->GetInterfaceByName("handle");
		if (pAttachment)
		{
			if (attach)
			{
...
//------------------------------------------------------------------------
IEntity *CGameRulesObjective_Extraction::SpawnEntity( EntityId spawnAt, IEntityClass *spawnEntityClass, i32 teamId, tukk name )
{
	...
	if (pSpawnAtEntity)
	{
		...
		if (pSpawnedEntity)
		{
			...
			/*
			i32 itemTeamId = GetItemTeamId(teamId);
			i32 itemTeamIndex = itemTeamId - 1;
			...
			CGameRules::SModuleRMITwoEntityParams params(m_moduleRMIIndex, pSpawnedEntity->GetId(), pDetails->m_positionEntityId, i32(pDetails->m_state[itemTeamIndex]));
		...
		{
			DRX_ASSERT_MESSAGE(0, "failed to spawn entity");
		}
	}
	else
	{
		DRX_ASSERT_MESSAGE(0, "failed to find entity to spawnAt");
	}
...
{
	DrxLog("CGameRulesObjective_Extraction::OnHostMigration becomeServer %d", becomeServer);
	...
	if(becomeServer)
	{
		...
		for (i32 i=0; i<numPickups; i++)
		{
			...
			if (pPickup->m_state == ePickupState_Carried && pPickup->m_carrierId)
			{
				...
				if(!pCarrier)
				{
					DrxLog("entity %d is carried by entity %d but entity does not exist, dropping...", pPickup->m_spawnedPickupEntityId, pPickup->m_carrierId);
...
CGameRulesObjective_Extraction::SPickup *CGameRulesObjective_Extraction::GetPickupForPickupEntityId(EntityId inEntityId)
{
	SPickup *ret=NULL;
	i32 numPickups=m_pickups.size();
	DRX_ASSERT_MESSAGE(inEntityId, "GetPickupForPickupEntityId() doesn't really want to ever pass inEntityId=0");
	if (inEntityId)
	{
		for (i32 i=0; i<numPickups; i++)
		{
...
CGameRulesObjective_Extraction::SPickup *CGameRulesObjective_Extraction::GetPickupForCarrierEntityId(EntityId inEntityId)
{
	SPickup *ret=NULL;
	i32 numPickups=m_pickups.size();
	DRX_ASSERT_MESSAGE(inEntityId, "GetPickupForCarrierEntityId() doesn't really want to ever pass inEntityId=0");
	if (inEntityId)
	{
		for (i32 i=0; i<numPickups; i++)
		{
...
CGameRulesObjective_Extraction::SExtractAt *CGameRulesObjective_Extraction::GetExtractAtForEntityId(EntityId inEntityId)
{
	...
	DRX_ASSERT_MESSAGE(outExtractAt, "failed to find extractAt for entityId");
	return outExtractAt;
}


i32 CGameRulesObjective_Extraction::FindCarrierInPickupCarriers(SPickup *pickup, EntityId carrierId)
{
	DbgLog("CGameRulesObjective_Extraction::FindCarrierInPickupCarriers()");	
	i32 foundAt=-1;

	i32 numCarriers=pickup->m_carriers.size();
	for (i32 i=0; i<numCarriers; i++)
	{
...
void CGameRulesObjective_Extraction::CallEntityScriptFunction(EntityId entityId, tukk function, bool active)
{
...
void CGameRulesObjective_Extraction::PlayerCollectsPickupCommon(EntityId playerId, SPickup *pickup, bool newPickup)
{
	...
	if (newPickup)
	{
		EntityId localActorId = g_pGame->GetIGameFramework()->GetClientActorId();
		i32 localTeamId = pGameRules->GetTeam(localActorId);
		i32 playerTeamId = pGameRules->GetTeam(playerId);
		bool localPlayerPlayerSameTeam = (localTeamId == playerTeamId);
		tukk pickupString = GetPickupString(pickup);
...
void CGameRulesObjective_Extraction::PlayerExtractsPickupCommon(EntityId playerId, SPickup *pickup)
{
	...
	i32k localTeamId = pGameRules->GetTeam(localActorId);
	i32k playerTeamId = pGameRules->GetTeam(playerId);
	...
	DRX_ASSERT(pickupEntity);
	DRX_ASSERT(spawnedAtEntity);
...
void CGameRulesObjective_Extraction::PlayerExtractsPickup(EntityId playerId, SPickup *pickup, IEntity *extractionPointEntity)
{
	...
	i32 teamId = pGameRules->GetTeam(playerId);
...
//		i32  numEntities = m_entities.size();
...
void CGameRulesObjective_Extraction::PlayerDropsPickupCommon(EntityId playerId, SPickup *pickup, bool newDrop)
{
	...
	DRX_ASSERT_MESSAGE(playerId == pickup->m_carrierId, "PlayerDropsPickupCommon() requires the player doing the dropping to already be the carrier, and it isn't!!");
	...
	if (gEnv->IsClient())
	{
		...
		static u32 PROBE_TYPES = ent_terrain|ent_static|ent_sleeping_rigid|ent_rigid;
		static u32 PROBE_FLAGS = ((geom_colltype_ray/*|geom_colltype13*/)<<rwi_colltype_bit)|rwi_colltype_any|rwi_force_pierceable_noncoll|rwi_ignore_solid_back_faces|8;
	...
	if (newDrop)
	{
		EntityId localActorId = g_pGame->GetIGameFramework()->GetClientActorId();
		i32 localTeamId = pGameRules->GetTeam(localActorId);
		i32 playerTeamId = pGameRules->GetTeam(carrierIdWas);
		bool localPlayerPlayerSameTeam = (localTeamId == playerTeamId);
		IEntity *playerEntity = gEnv->pEntitySystem->GetEntity(playerId);
...
void CGameRulesObjective_Extraction::PickupReturnsCommon(SPickup *pickup, IEntity *pickupEntity, IEntity *spawnedAtEntity, bool wasExtracted)
{
	...
	if (!wasExtracted)
	{
		...
		CGameRules *pGameRules = g_pGame->GetGameRules();
		i32 localTeamId = pGameRules->GetTeam(localActorId);
		i32 pickupTeamId = pGameRules->GetTeam(pickup->m_spawnedPickupEntityId); // pickup->m_teamAffected will be the defending team
		...
		IGameRulesRoundsModule*  pRoundsModule = pGameRules->GetRoundsModule();
		i32 primaryTeamId = pRoundsModule->GetPrimaryTeam(); 
...
void CGameRulesObjective_Extraction::OnEntityEvent( IEntity *pEntity, SEntityEvent &event )
{
	if (gEnv->IsClient() && event.event == ENTITY_EVENT_DONE)
	{
		...
		if (pickup)
		{
			...
			DRX_ASSERT_MESSAGE(pickup->m_spawnedPickupEntityId == pEntity->GetId(), "OnEntityEvent() EVENT_DONE trying to remove the hud icons for a removed pickup entity but the pickup's entityId and pEntity aren't the same");
...
void CGameRulesObjective_Extraction::OnEntityKilled( const HitInfo &hitInfo )
{
	...
	if (gEnv->bServer)
	{
		...
		for (size_t i = 0; i < numPickups; i++)
		{
			...
			i32 index=FindCarrierInPickupCarriers(pickupElement, hitInfo.targetId);
			if (index >= 0)
			{
		...
		if (pRoundsModule)
		{
			...
			i32 shooterTeamId = pGameRules->GetTeam(hitInfo.shooterId);
			i32 targetTeamId = pGameRules->GetTeam(hitInfo.targetId);
			...
			if(pActor && shooterTeamId != targetTeamId)
			{
				if (IGameRulesScoringModule* pScoringModule = pGameRules->GetScoringModule())
				{
					if (shooterTeamId == primaryTeam)
					{
						...
						i32k killPoints = pScoringModule->GetPlayerPointsByType(EGRST_Extraction_AttackingKill);
					...
					else if (shooterTeamId == secondaryTeam)
					{
						...
						i32k killPoints = pScoringModule->GetPlayerPointsByType(EGRST_Extraction_DefendingKill);
					...
					else
					{
						...
						DRX_ASSERT_MESSAGE(0, string().Format("OnEntityKilled() but the shooter team=%d isn't the attacking=%d or defending team=%d", shooterTeamId, primaryTeam, secondaryTeam));
					}
				}
			}
		}
...
void CGameRulesObjective_Extraction::OnClientDisconnect(i32 channelId, EntityId playerId)
{
	if (gEnv->bServer)
	{
...
void CGameRulesObjective_Extraction::OnClientEnteredGame(i32 channelId, bool isReset, EntityId playerId)
{
	DbgLog("CGameRulesObjective_Extraction::OnClientEnteredGame()");
	EntityId localActorId = g_pGame->GetIGameFramework()->GetClientActorId();
	if (gEnv->bServer)
	{
		i32  plyrTeam = g_pGame->GetGameRules()->GetTeam(playerId);
...
void CGameRulesObjective_Extraction::OnSingleEntityRMI(CGameRules::SModuleRMIEntityParams params)
{
	...
	switch(params.m_data)
	{
		...
		default:
			DRX_ASSERT_MESSAGE(0, string().Format("OnSingleEntityRMI() unknown rmitype=%d", params.m_data));
	}
}
...
void CGameRulesObjective_Extraction::OnPickupSpawned(EExtractionSuitMode suitMode, EntityId pickupEnt, EntityId spawnAtEnt)
{
	...
	DRX_ASSERT_TRACE(pickup, ("OnDoubleEntityRMI() failed to find pickup for suitmode %d", suitMode));
	if (pickup)
	{
		if (pickup->m_spawnedPickupEntityId)
		{
...
void CGameRulesObjective_Extraction::OnDoubleEntityRMI(CGameRules::SModuleRMITwoEntityParams params)
{
	...
	switch(params.m_data)
	{
		...
		case eRMITypeDoubleEntity_pickup_already_collected:
		{
			...
			{
				DrxLog("OnDoubleEntityRMI() - pickup collected - failed to find pickup for collected entity - relying on additional RMIs sent from Server in OnClientEnteredGame() to happen later to sync us up");
			}
			break;
		}
		...
		case eRMITypeDoubleEntity_pickup_already_dropped:
		{
			...
			{
				DrxLog("OnDoubleEntityRMI() - pickup dropped - failed to find pickup for dropped entity - relying on additional RMIs sent from Server in OnClientEnteredGame() to happen later to sync us up");
			}
		...
		case eRMITypeDoubleEntity_pickup_returned_to_base:
		{
			...
			{
				DrxLog("OnDoubleEntityRMI() - pickup dropped - failed to find pickup for returned to base entity - relying on additional RMIs sent from Server in OnClientEnteredGame() to happen later to sync us up");
			}
			break;
		}
		case eRMITypeDoubleEntity_pickup_extracted:
		{
			...
			{
				DrxLog("OnDoubleEntityRMI() - pickup extracted - failed to find pickup for carrier entity - relying on additional RMIs sent from Server in OnClientEnteredGame() to happen later to sync us up");
			}
			break;
		}
		default:
			DRX_ASSERT_MESSAGE(0, string().Format("OnDoubleEntityRMI() unknown rmitype=%d", params.m_data));
	}
}
...
void CGameRulesObjective_Extraction::OnSvClientActionRMI(CGameRules::SModuleRMISvClientActionParams params, EntityId fromEid)
{
	...
	switch (params.m_action)
	{
		case CGameRules::SModuleRMISvClientActionParams::eACT_HelperCarry_Drop:
		{
			...
			DRX_ASSERT_MESSAGE(pickup, "OnSvClientActionRMI() dropping a pickup but no pickup found for carrier!!");
			if (pickup)
			{
		...
		case CGameRules::SModuleRMISvClientActionParams::eACT_HelperCarry_Pickup:
		{
			...
			else
			{
				...
				DRX_ASSERT_MESSAGE(pickup, "OnSvClientActionRMI() collecting a pickup but failed to find a pickup for the item to be collected");
				if (pickup)
				{
					DRX_ASSERT_MESSAGE(pickup->m_state != ePickupState_Carried, "OnSvClientActionRMI() trying to collect a pickup but the pickup is already in state carried.. can't pick it up again!!!");
...
void CGameRulesObjective_Extraction::OnAction(const ActionId& action, i32 activationMode, float value)
{
...
void CGameRulesObjective_Extraction::SetIconForExtractionPoint(EntityId extractionPointEntityId)
{
	...
	CGameRules *pGameRules=g_pGame->GetGameRules();
	i32 localTeamId = pGameRules->GetTeam(localActorId);
	i32 extractionPointTeamId = pGameRules->GetTeam(extractionPointEntityId);
	bool sameTeam=(localTeamId == extractionPointTeamId);
	tukk newIconName=m_iconTextExtraction;
	bool showIcon=false;
	...
	tukk localisedIconName = CHUDUtils::LocalizeString(newIconName);		
	DrxFixedStringT<64> localisedIconNameCache = localisedIconName;
...
void CGameRulesObjective_Extraction::SetIconForPickup(SPickup *pickup)
{
	...
	i32 pickupTeamId = pGameRules->GetTeam(pickup->m_spawnedPickupEntityId);
	bool localPlayerPickupSameTeam = (localTeamId == pickupTeamId);
	tukk newIconName=NULL;
	tukk newIconColour=NULL;

	tukk friendlyColour="#9AD5B7";
	tukk enemyColour="#AC0000";
	...
	if (!localPlayerCarryingAPickup)
	{
		...
		{
			case ePickupState_AtBase:
				DRX_ASSERT_TRACE(localTeamId && pickupTeamId, ("SetIconForPickup() requires player and pickup to both have teams set"));
				if (localPlayerPickupSameTeam)
				{
					...
					{
						DRX_ASSERT_MESSAGE(0, "Extraction: unhandled pickup suitmode effected");
					}
			...
			{
				IEntity *pickupEntity = gEnv->pEntitySystem->GetEntity(pickup->m_spawnedPickupEntityId);
				i32 carrierTeamId=pGameRules->GetTeam(pickup->m_carrierId);
				bool localPlayerPickupCarrierSameTeam = (localTeamId == carrierTeamId);
				DRX_ASSERT_TRACE(localTeamId && carrierTeamId, ("SetIconForPickup() requires player and carrier to both have teams set"));
			...
			case ePickupState_Extracted:
				...
				break;
			default:
				DRX_ASSERT_TRACE(0, ("SetIconForPickup() encountered unknown pickupstate=%d", pickup->m_state));
				break;
		}
	}
...
{
	i32k numPickups = m_pickups.size();
...
void CGameRulesObjective_Extraction::UpdateGameStateText(EGameStateUpdate inUpdate, SPickup *inPickup)
{
...
#ifndef _RELEASE
	tukk stringForPickup=inPickup ? GetPickupString(inPickup) : "NULL";
	if (!stringForPickup)
	{
	...
	i32 secondaryTeam = (primaryTeam == 1) ? 2 : 1;			// defending
	EntityId localActorId = g_pGame->GetIGameFramework()->GetClientActorId();
	DRX_ASSERT(localActorId);
	i32 localTeamId = pGameRules->GetTeam(localActorId);
	DRX_ASSERT(localTeamId);

	tukk localisedMessage=NULL;
	...
	switch(inUpdate)
	{
		...
		case eGameStateUpdate_TickCollected:
			DRX_ASSERT_MESSAGE(inPickup, "UpdateGameStateText() tick collected update needs a valid inPickup");
			if (localTeamId == primaryTeam)
			{
		...
		case eGameStateUpdate_TickDropped:
			DRX_ASSERT_MESSAGE(inPickup, "UpdateGameStateText() tick dropped update needs a valid inPickup");
			if (localTeamId == primaryTeam)
			{
		...
		case eGameStateUpdate_TickExtracted:
			DRX_ASSERT_MESSAGE(inPickup, "UpdateGameStateText() tick extracted update needs a valid inPickup");
			if (localTeamId == primaryTeam)
			{
...
void CGameRulesObjective_Extraction::CheckForInteractionWithEntity(EntityId interactId, EntityId playerId, SInteractionInfo& interactionInfo) 
{
	...
	if (SPickup* pPickup = GetPickupForPickupEntityId(interactId))
	{
		...
		{
			i32  entTeam = pGameRules->GetTeam(interactId);
			i32  plyrTeam = pGameRules->GetTeam(playerId);
			if(entTeam != plyrTeam)
			{
...
void CGameRulesObjective_Extraction::OnChangedTeam( EntityId entityId, i32 oldTeamId, i32 newTeamId )
{
	if (gEnv->IsClient())
	{
...
float CGameRulesObjective_Extraction::GetTimeLimit()
{
	...
	if (pRoundsModule)
	{
		if(pRoundsModule->GetRoundNumber() & 0x1)	//Is Odd
		{
			...
			{
				i32k primaryTeamIndex = 0;	//for previous even round
				if(previousRoundsTeamScores[primaryTeamIndex] == m_pickups.size())
				{
					if (IGameRulesVictoryConditionsModule *pVictoryModule = pGameRules->GetVictoryConditionsModule())
					{
						...
						DRX_ASSERT(timeLimitData);
						if(timeLimitData)
						{
							DRX_ASSERT(timeLimitData->m_dataType == SDrawResolutionData::eDataType_float);
...
void CGameRulesObjective_Extraction::OnRoundAboutToStart()
{
	...
	if (pGameRules)
	{
		...
		if (pRoundsModule)
		{
			...
			i32k attackingTeam = 3 - pRoundsModule->GetPrimaryTeam();
			EntityId clientActorId = g_pGame->GetIGameFramework()->GetClientActorId();
			i32k clientTeamId = pGameRules->GetTeam(clientActorId);

			tukk pLocalisedMessage = NULL;
