...
#include <drx3D/Game/GameRulesSPDamageHandling.h>
...
#include <drx3D/Game/AI/VisibleObjectsHelper.h>
...
void CMercyTimeFilter::Init( const XmlNodeRef& xml )
{
	...
	{
		i32k childCount = projectileFilterNode->getChildCount();
		...
		for (i32 i = 0; i < childCount; ++i)
		{
			...
			{
				tukk filterType = childNode->haveAttr("filter") ? childNode->getAttr("filter") : "none";
...
bool CMercyTimeFilter::Filtered( u16k projectileClassId, const EntityId ownerId, const EntityId targetId ) const
{
	...
	u32k currentDifficulty = (u32)CLAMP(g_pGameCVars->g_difficultyLevel, 1, MaxDifficultyLevels);
	u32k difficultyKillerHitsTolerance = m_difficultyTolerance[currentDifficulty-1];
	...
	if (victimID != 0)
	{
		...
		else
		{
			...
			else
			{
				...
				const TBodyDamageProfileId bodyDamageProfileId = pBodyDamageUpr->FindBodyDamageProfileIdBinding(victimID);
				if (bodyDamageProfileId != INVALID_BODYDAMAGEPROFILEID)
				{
					IEntity* pEntity = gEnv->pEntitySystem->GetEntity(victimID);
					const float multiplier = pBodyDamageUpr->GetDamageMultiplier(bodyDamageProfileId, *pEntity, hit);
	...
	for ( ; it != affectedEntities.end(); ++it)
	{
		...
		if (incone)
		{
			...
			else if (pVictimActor == NULL)
			{
				...
				const TBodyDamageProfileId bodyDamageProfileId = pBodyDamageUpr->FindBodyDamageProfileIdBinding(explHit.targetId);
				if (bodyDamageProfileId != INVALID_BODYDAMAGEPROFILEID)
				{
					const float multiplier = pBodyDamageUpr->GetExplosionDamageMultiplier(bodyDamageProfileId, *entity, explHit);
...
float CGameRulesSPDamageHandling::CalcExplosionDamage(IEntity* entity, const ExplosionInfo &explosionInfo, float obstruction)
{
	...
	DRX_ASSERT_TRACE (effect >= 0.0f && effect <= 1.0f, ("Effectiveness of explosion should be between 0 and 1 but it's %.3f (minRadius=%.3f, maxRadius=%.3f)", effect, explosionInfo.minRadius, explosionInfo.radius));
...
void CGameRulesSPDamageHandling::SvOnCollision(const IEntity* pVictimEntity, const CGameRules::SCollisionHitInfo& collisionHitInfo)
{
	...
	if (pVictimEntity)
	{
		...
		if (stl::find(m_entityClassesWithTrackedMovement, pVictimEntity->GetClass()))
		{
			...
			u32 visibleObjectRules = eVOR_OnlyWhenMoving|eVOR_UseMaxViewDist|eVOR_FlagDropOnceInvisible;
			if (offenderID)
			{
			...
			else // victim is AI
			{
				...
				if (pOffenderEntity)
				{
					...
					if (offenderActor)
					{
						...
						{
							const SAutoaimTarget* pTargetInfo = g_pGame->GetAutoAimUpr().GetTargetInfo(victimID);
...
//							const bool playerIsSprinting = offenderPlayer->IsSprinting();
		...
		{
			DrxLog("---Collision. victim: ID:%d '%s'   offender: ID:%d '%s'   relativeSpeed: %f  offenderSpeed: %f  offenderMass: %f  damage: %f  fallAndPlay: %s",
...
float CGameRulesSPDamageHandling::AdjustVehicleActorCollisionDamage(IVehicle* pOffenderVehicle, IActor* pVictimActor, const CGameRules::SCollisionHitInfo& colHitInfo, float damage)
{
	...
	else
	{
		...
		{
			// AI hitting AI
			u8 driverFaction = pActorDriver->GetEntity()->GetAI()->GetFactionID();
			u8 victimFaction = pVictimActor->GetEntity()->GetAI()->GetFactionID();
...
//// 	if (colliderActor)
//// 	{
//// 		u8 profile = colliderActor->GetGameObject()->GetAspectProfile(eEA_Physics);
