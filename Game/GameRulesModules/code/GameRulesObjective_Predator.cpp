...
#include <drx3D/Game/GameRulesObjective_Predator.h>
...
#include <drx3D/Game/IGameRulesSpectatorModule.h>
...
#ifndef _RELEASE
	#define HUNTER_DEBUG_LOG(...) DrxLogAlways("[HUNTER MODE]: " __VA_ARGS__)
#else
...
void CGameRulesObjective_Predator::Init( XmlNodeRef xml )
{
	...
	i32k numChildren = xml->getChildCount();
	for (i32 i = 0; i < numChildren; ++ i)
	{
		...
		if (!stricmp(pTag, "Params"))
		{
			...
			{
				i32 numBands = survivalPointsRoot->getChildCount();
				for (i32 j = 0; j < numBands; ++ j)
				{
					i32 soldiers;
					i32 points;
					...
					{
						DesignerWarning((soldiers & 0xFF) == soldiers, "Predator: Soldiers count (%d) won't fit in a u8", soldiers);
						DesignerWarning((points & 0xFF) == points, "Predator: Points (%d) won't fit in a u8", points);

						SSurvivalPointsData band;
						band.m_soldierCount = (u8) soldiers;
						band.m_score = (u8) points;
			...
			if (ammoOverridesRoot)
			{
				...
				i32 numWeapons = ammoOverridesRoot->getChildCount();
				m_weaponAmmoOverrides.reserve(numWeapons);
				for (i32 j = 0; j < numWeapons; ++ j)
				{
					...
					if (!stricmp(weaponOverrideNode->getTag(), "Weapon"))
					{
						if (weaponOverrideNode->getAttr("Class", &pClassName))
						{
							...
							if (pClass)
							{
								...
								i32 numAmmoTypes = weaponOverrideNode->getChildCount();
								weapon.m_pWeaponClass = pClass;
								weapon.m_bonusAmmo.reserve(numAmmoTypes);
								for (i32 k = 0; k < numAmmoTypes; ++ k)
								{
...
void CGameRulesObjective_Predator::Update( float fFrameTime )
{
	...
	if (gEnv->bServer)
	{
		...
		{
			i32 remainder = (i32)currentGameTime % m_SoldierSurvivalScorePeriod;
			i32 prevRemainder = (i32)(currentGameTime - gEnv->pTimer->GetFrameTime()) % m_SoldierSurvivalScorePeriod;
...
//------------------------------------------------------------------------
void CGameRulesObjective_Predator::Client_AnnounceTimeRemaining( EPredatorFlags flag, tukk pMarineEvent, tukk pHunterEvent )
{
...
void CGameRulesObjective_Predator::OnStartGame()
{
	...
	u32 numCacheGroups = 0;

	// Construct our ammo cache groups
	const CTacticalUpr::TInterestPoints& ammoPoints = g_pGame->GetTacticalUpr()->GetTacticalPoints(CTacticalUpr::eTacticalEntity_Ammo);
	i32 numPoints = ammoPoints.size();
	for (i32 i = 0; i < numPoints; ++ i)
	{
		...
		if (pEntity)
		{
			...
			{
				u32 groupId = CCrc32::ComputeLowercase(pGroupName);
				bool bFound = false;
				for (u32 j = 0; j < numCacheGroups; ++ j)
				{
...
void CGameRulesObjective_Predator::OnStartGamePost()
{
	...
	for (u32 i = 0, numCacheGroups = m_ammoCacheGroups.size(); i < numCacheGroups; ++ i)
	{
		SAmmoCacheGroup &group = m_ammoCacheGroups[i];
		if (group.m_groupId != m_ammoCacheGroupId)
		{
			i32 numCaches = group.m_entityIds.size();
			for (i32 j = 0; j < numCaches; ++ j)
			{
...
//-------------------------------------------------------------------------
void CGameRulesObjective_Predator::PostInitClient( i32 channelId )
{
}

//-------------------------------------------------------------------------
bool CGameRulesObjective_Predator::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
	if (aspect == PREDATOR_OBJECTIVE_STATE_ASPECT)
	{
		i32 previousRemainingSoldiers = m_numRemainingSoldiers;
...
//-------------------------------------------------------------------------
bool CGameRulesObjective_Predator::HasCompleted( i32 teamId )
{
...
void CGameRulesObjective_Predator::AssignTeamsFromLobby()
{
	...
	if (pGameLobby)
	{
		...
		i32 numPlayers = sessionNames.Size();

		i32 numPredatorsNeeded = Common_GetMaxStartPredators(numPlayers);

#ifndef _RELEASE
		tukk pForceName[] = {	g_pGameCVars->g_predator_forcePredator1->GetString(),
		...
		i32 numForcePredatorsChosen = 0;
#endif

		for (i32 i = 0; i < numPlayers; ++ i)
		{
			i32 uid = (i32) sessionNames.m_sessionNames[i].m_conId.m_uid;
		...
		{
			for (i32 i = numPredatorsNeeded; i < 0; ++ i)
			{
		...
		for (i32 i = 0; i < numPredatorsNeeded && numPlayers > 0; ++ i)
		{
			u32 randomNum = g_pGame->GetRandomNumber();
			u32 index = randomNum % numPlayers;
...
//-------------------------------------------------------------------------
i32 CGameRulesObjective_Predator::GetAutoAssignTeamId( i32 channelId )
{
	...
	{
		DrxLog( "[HUNTER] Spawning as hunter as designated in the lobby (channelId=%d)", channelId);
		return TEAM_PREDATOR;
	}

	// They may have joined after initial selection, Do we *STILL* require more predators?
	i32 startPredCount = 0, currPredCount = 0, startMarineCount = 0;
	...
	{
		DrxLog( "[HUNTER] %d players started as marine, %d started as hunter", startMarineCount, startPredCount);
		...
		{
			DrxLog( "[HUNTER] Forced team to hunter as joining late (channelId=%d)", channelId);
			m_PredatorData[channelId].bIsPredator = true;
			return TEAM_PREDATOR;
		}

		// If we are not up to our quota, this joiner can be a predator.
		i32 desiredNumPredators = Common_GetMaxStartPredators(m_PredatorData.size());
		if(currPredCount < desiredNumPredators)
		{
			DrxLog( "[HUNTER] Forced team to hunter as currently %d hunters out of a desired %d (channelId=%d)", currPredCount, desiredNumPredators, channelId);
	...
	DrxLog( "[HUNTER] Spawning as marine (channelId=%d)", channelId);
	...
	case SINGLE_ENTITY_RMI_SET_PREDATOR_LOADOUT:
		{
			...
			{
				DrxLog("PREDATORDEBUG %i Received set loadout message from server", params.m_entityId);
...
//-------------------------------------------------------------------------
void CGameRulesObjective_Predator::OnChangedTeam( EntityId entityId, i32 oldTeamId, i32 newTeamId )
{
	if (entityId == g_pGame->GetClientActorId())
	{
		...
		i32k numCacheGroups = m_ammoCacheGroups.size();
		for (i32 i = 0; i < numCacheGroups; ++ i)
		{
			const SAmmoCacheGroup& ammoCacheGroup = m_ammoCacheGroups[i];
			if (ammoCacheGroup.m_groupId == m_ammoCacheGroupId)
			{
				i32 numEntities = ammoCacheGroup.m_entityIds.size();
...
void CGameRulesObjective_Predator::EntityRevived( EntityId entityId )
{
	...
	if (entityId == clientId)
	{
		...
		if (!Common_IsFlagSet(ePF_HasSpawned))
		{
			...
			i32 numCacheGroups = m_ammoCacheGroups.size();
			for (i32 i = 0; i < numCacheGroups; ++ i)
			{
				SAmmoCacheGroup &group = m_ammoCacheGroups[i];
				if (group.m_groupId == m_ammoCacheGroupId)
				{
					i32 numEntities = group.m_entityIds.size();
					for (i32 j = 0; j < numEntities; ++ j)
					{
						SHUDEvent hudevent(eHUDEvent_AddEntity);
						hudevent.AddData(SHUDEventData((i32)group.m_entityIds[j]));
...
void CGameRulesObjective_Predator::Server_GiveSoldierSurvivingPoints(bool survivedToEnd)
{
	...
	{
		i32 numSurvivingSoldiers = 0;
		EntityId temp = 0;
		Server_GetMarinesStatus(0, temp, numSurvivingSoldiers);

		for (i32 i = 0, numBands = m_survivalPoints.size(); i < numBands; ++ i)
		{
			if ((i32) m_survivalPoints[i].m_soldierCount <= numSurvivingSoldiers)
			{
...
//-------------------------------------------------------------------------
void CGameRulesObjective_Predator::Server_UpdateRemainingSoldierCount( i32 deltaCount )
{
	if (deltaCount != 0)
	{
		i32 previous = m_numRemainingSoldiers;
...
//-------------------------------------------------------------------------
i32 CGameRulesObjective_Predator::Common_GetMaxStartPredators(i32 numPlayers) const
{
...
void CGameRulesObjective_Predator::Common_OnPlayerKilledOrLeft( EntityId targetId, EntityId shooterId )
{
	if (gEnv->bServer)
	{
		i32k targetTeamId = m_pGameRules->GetTeam(targetId);
		...
		{
			i32 shooterTeamId = m_pGameRules->GetTeam(shooterId);
...
//-------------------------------------------------------------------------
void CGameRulesObjective_Predator::OnClientDisconnect( i32 channelId, EntityId playerId )
{
	...
	if(m_pGameRules->HasGameActuallyStarted() && (m_pGameRules->GetTeam(hitInfo.targetId) == TEAM_SOLDIER) )
	{
		...
		if (m_pGameRules->GetTeam(hitInfo.shooterId) == TEAM_PREDATOR)
		{
			if (m_clientTeam == TEAM_SOLDIER)
			{
				...
				eventTempAddToRadar.AddData( static_cast<i32>(hitInfo.shooterId) );
...
void CGameRulesObjective_Predator::Server_PrepareChangeTeamPredator( EntityId playerId, bool wasSuicide)	
{
	...
	{
		DrxLog("PREDATORDEBUG - Final marine has been killed!");
	}
}
...
{
	DrxLog("PREDATORDEBUG - Received client ready to respawn %i", playerId);
	...
	{
		DrxLog("PREDATORDEBUG - But round almost over. Denied.");
		return;
	}

	bool found = false;
	for(u32 i = 0; i < m_deadSoldierIds.size(); ++i)
	{
		if(m_deadSoldierIds[i] == playerId)
		{
			DrxLog("PREDATORDEBUG - Found dead player");
	...
	if(!found)
	{
		DrxLog("PREDATORDEBUG - Player is apparently not dead. Client and server are out of synch. Re-spawning is going to go horribly wrong.");
	}
...
void CGameRulesObjective_Predator::Client_SetPredatorLoadout()
{
	...
	tukk pMessage = CHUDUtils::LocalizeString("@ui_msg_hr_status_kill_marines");
...
void CGameRulesObjective_Predator::Server_OnStartNewRound()
{
	...
	i32 numPredatorsToChoose = Common_GetMaxStartPredators(m_PredatorData.size());
...
//-------------------------------------------------------------------------
void CGameRulesObjective_Predator::Server_GetMarinesStatus( EntityId excludeId, EntityId &outLastMarineId, i32 &outCount ) const
{
	outCount = 0;
	outLastMarineId = 0;
...
{
	i32 numOverrides = m_weaponAmmoOverrides.size();
	for (i32 i = 0; i < numOverrides; ++ i)
	{
		SWeaponAmmoOverride &weaponOverride = m_weaponAmmoOverrides[i];
		if (weaponOverride.m_pWeaponClass == pWeaponClass)
		{
			i32 numAmmoTypes = weaponOverride.m_bonusAmmo.size();
			for (i32 j = 0; j < numAmmoTypes; ++ j)
			{
...
//-------------------------------------------------------------------------
void CGameRulesObjective_Predator::Common_SetAmmoCacheGroupActive( u32 groupId, bool bActive )
{
	i32 iActive = (bActive ? 1 : 0);
	i32 numCacheGroups = m_ammoCacheGroups.size();
	for (i32 i = 0; i < numCacheGroups; ++ i)
	{
		SAmmoCacheGroup &group = m_ammoCacheGroups[i];
...
//-------------------------------------------------------------------------
void CGameRulesObjective_Predator::Client_OnSoldiersRemainingCountChanged(i32 previousCount)
{
	...
	if (pActor)
	{
		...
		if ((m_numRemainingSoldiers == 1) && (previousCount > 1))
		{
			if (bSoldier)
			{
				...
				DrxLog("Play last marine signal (marine)");
			}
			else
			{
				m_signalPlayer.SetSignal(m_lastManSignalIdHunter);
				DrxLog("Play last marine signal (hunter)");
			}
		}
...
//-------------------------------------------------------------------------
void CGameRulesObjective_Predator::OnNewGroupIdSelected( u32 id )
{
...
{
	DrxLog("CGameRulesObjective_Predator::OnGameReset()");
	Common_SetAmmoCacheGroupActive(m_ammoCacheGroupId, true);
}
...
bool CGameRulesObjective_Predator::EndGameIfOnlyOnePlayerRemains( i32 channelId )
{
	CGameLobby *pGameLobby = g_pGame->GetGameLobby();
	if (pGameLobby)
	{
		SDrxMatchMakingConnectionUID connUID = pGameLobby->GetConnectionUIDFromChannelID(channelId);

		// Don't rely on session names always being up to date at this point (d/c player may not have been removed yet)
		u8 livePlayerCount = 0;
		i32 survivorTeamId		= 1;
		const SSessionNames &sessionNames = g_pGame->GetGameLobby()->GetSessionNames();
		const DrxFixedArray<SSessionNames::SSessionName, MAX_SESSION_NAMES>& sessionNameArray = sessionNames.m_sessionNames;
		u8k sessionNameArraySize = sessionNameArray.size();
		for(i32 i = 0; i < sessionNameArraySize; ++i)
		{
