...
#include <drx3D/Game/GameRulesMPDamageHandling.h>
...
{
	DrxLog("CGameRulesMPDamageHandling::CGameRulesMPDamageHandling()");
...
void CGameRulesMPDamageHandling::Init( XmlNodeRef xml )
{
	...
	{
		if (tukk path = table->getAttr("path"))
		{
...
bool CGameRulesMPDamageHandling::SvOnHit( const HitInfo &hitInfo )
{
...
#if !defined(_RELEASE)
	if(!pHitTypeInfo)
		DrxFatalError("By ::SvOnHit() all hit info should have a hit type that is valid and registered in the GameRules. This isn't true of type %d!", hitInfo.type);
#endif
	...
	if( bool bCheckFriendlyFire = ((hitInfo.targetId!=hitInfo.shooterId) && (hitInfo.type!=CGameRules::EHitType::EventDamage)) )
	{
		...
		if(bCheckFriendlyFire)
		{
			...
			{
				i32 shooterTeamId = m_pGameRules->GetTeam(hitInfo.shooterId);
				i32 targetTeamId = m_pGameRules->GetTeam(hitInfo.targetId);
	...
	if(pPlayer)
	{
		...
		if(pStats)
		{
			...
			if (g_pGameCVars->g_LogDamage)
			{
				...
				DrxLog ("[DAMAGE] %s '%s' took %.3f '%s' damage (%.3f x %.3f) weapon=%s projectile=%s",
...
bool CGameRulesMPDamageHandling::SvOnHitScaled( const HitInfo &hitInfo )
{
	...
	if(!pActor)
	{
		...
		if(pVictimVehicle)
		{
			...
			{
				bodyDamageProfileId = pBodyDamageUpr->GetBodyDamage( *pTarget );
				m_vehicleBodyDamageMap[hitInfo.targetId] = bodyDamageProfileId;
			}

			if(bodyDamageProfileId != INVALID_BODYDAMAGEPROFILEID)
			{
				vehicleDamageMultiplier *= pBodyDamageUpr->GetDamageMultiplier(bodyDamageProfileId, *pTarget, hitInfo);
			}
...
{
	DRX_TODO(08, 09, 2009, "Implement ProcessVehicleDeath");
}
...
float CGameRulesMPDamageHandling::CalcExplosionDamage(IEntity* entity, const ExplosionInfo &explosionInfo, float obstruction)
{
	...
	DRX_ASSERT_TRACE (effect >= 0.0f && effect <= 1.0f, ("Effectiveness of explosion should be between 0 and 1 but it's %.3f (distance = %.3f, minRadius=%.3f, maxRadius=%.3f)", effect, sqrtf(distanceSq), explosionInfo.minRadius, explosionInfo.radius));
...
{
	if (i32k N = m_numKickableCarRecords)
	{
		i32 n = 0;
...
{
	for (i32 i=0; i<m_numKickableCarRecords; i++)
	{
...
float CGameRulesMPDamageHandling::ProcessActorKickedVehicle(IActor* victimActor, EntityId victimId, EntityId kickerId, EntityId vehicleId, float damage, const CGameRules::SCollisionHitInfo& collisionHitInfo)
{
	...
	if (g_pGameCVars->g_mpKickableCars)
	{
		...
		{
			i32 kickerTeamId = pGameRules->GetTeam(kickerId);
			i32 victimTeamId = pGameRules->GetTeam(victimId);
...
void CGameRulesMPDamageHandling::SvOnCollision(const IEntity *pVictimEntity, const CGameRules::SCollisionHitInfo& collisionHitInfo)
{
	...
	if (relativeSpeedSq >= minSpeedToCareAboutCollisionSq)
	{
		...
		if (damage >= DAMAGE_THRESHOLD_COLLISIONS)
		{
			...
			if (pGameRules->GetTeamCount() > 1)
			{
				i32 shooterTeamId = pGameRules->GetTeam(hit.shooterId);
				i32 targetTeamId = pGameRules->GetTeam(hit.targetId);
...
//	if (pColliderActor)
//	{
//		u8 profile=pColliderActor->GetGameObject()->GetAspectProfile(eEA_Physics);
...
//------------------------------------------------------------------------
void CGameRulesMPDamageHandling::ClProcessHit(Vec3 dir, EntityId shooterId, EntityId weaponId, float damage, u16 projectileClassId, u8 hitTypeId)
{
	DrxLog("CGameRulesMPDamageHandling::ClProcessHit() type='%s' from weapon %d, shooter %d", m_pGameRules->GetHitTypeInfo(hitTypeId)->m_name.c_str(), weaponId, shooterId);
...
{
	DRX_ASSERT(gEnv->IsClient());
	DRX_ASSERT(m_localMeleeScreenFxTimer > 0.f);
...
float CGameRulesMPDamageHandling::CalculateFriendlyFireRatio(EntityId entityId1, EntityId entityId2)
{
	...
	{
		i32 team1 = pGameRules->GetTeam(entityId1);
