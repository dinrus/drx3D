...
#include <drx3D/Game/GameRulesAssistScoring.h>
...
void CGameRulesAssistScoring::Init(XmlNodeRef xml)
{
	...
	{
		DRX_ASSERT_MESSAGE(0, "CGameRulesMPSpawning failed to find valid maxTimeBetweenAttackAndKillForAssist param");
	}
	i32 iValue = 0;
	...
	{
		DRX_ASSERT_MESSAGE(0, "CGameRulesMPSpawning failed to find valid maxAssistScore param");
	}

	if (!xml->getAttr("assistScoreMultiplier", m_assistScoreMultiplier))
	{
		DRX_ASSERT_MESSAGE(0, "CGameRulesMPSpawning failed to find valid assistScoreMultiplier param");
	}
...
//	i32 playerPoints=scoringModule->GetPlayerPointsByName("assistKill");
...
void CGameRulesAssistScoring::SvDoScoringForDeath(const EntityId victimId, const EntityId shooterId, tukk weaponClassName, i32 damage, i32 material, i32 hit_type)
{
	DbgLog("CGameRulesAssistScoring::SvDoScoringForDeath()");
	DRX_ASSERT_MESSAGE(gEnv->bServer, "CGameRulesAssistScoring::SvDoScoringForDeath() should only be called on the server");
	...
	if(it!=m_targetAttackerMap.end())
	{
		...
		{
			SAttackerData &data = *vit;
			DRX_ASSERT_MESSAGE(data.m_entityId, string().Format("List of %s's recent attackers includes entity ID 0! Why has a NULL entity been allocated a slot in the list?", m_pGameRules->GetEntityName(victimId)));
			if ( (data.m_lastShootTime > fEarliestTimeForAssistShoot) || (time < data.m_tagExpirationTime))
			{
				...
				else
				{
					...
					IGameRulesScoringModule *scoringModule = m_pGameRules->GetScoringModule();
					DRX_ASSERT_MESSAGE(scoringModule, "CGameRulesAssistScoring::SvDoScoringForDeath() failed to find a scoring module, this should NOT happen");
					if (scoringModule)
					{
						EGRST type = EGRST_PlayerKillAssist;
						if (m_bTeamSpecificScoring)
						{
							i32 teamId = m_pGameRules->GetTeam(data.m_entityId);
							if (teamId == 1)
							{
...
void CGameRulesAssistScoring::OnEntityKilled(const HitInfo &hitInfo)
{
	...
	if (gEnv->bServer)
	{
		...
		DRX_ASSERT_MESSAGE(pVictimEntity, "failed to resolve our victim that has been killed!!!");

		if(pVictimEntity)
		{
			SvDoScoringForDeath(victimId, hitInfo.shooterId, weaponClassName, (i32)hitInfo.damage, hitInfo.partId, hitInfo.type);
		}
	}
	else
	{
...
EntityId CGameRulesAssistScoring::SvGetMostRecentAttacker(EntityId targetId)
{
	...
	DRX_ASSERT_MESSAGE(gEnv->bServer, "CGameRulesAssistScoring::SvGetMostRecentAttacker should only be called on the server");

	TTargetAttackerMap::iterator it=m_targetAttackerMap.find(targetId);

	if (it==m_targetAttackerMap.end())
	{
		DRX_ASSERT_MESSAGE(0, string().Format("CGameRulesAssistScoring::SvGetMostRecentAttacker failed to find a targetAttacker mapping element for target %s", m_pGameRules->GetEntityName(targetId)));
	}
	else
	{
		float time=m_pGameRules->GetServerTime();
