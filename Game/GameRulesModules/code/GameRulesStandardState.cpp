...
#include <drx3D/Game/GameRulesStandardState.h>
...
{
	DrxLog("CGameRulesStandardState::CGameRulesStandardState()");
...
void CGameRulesStandardState::PostInit()
{
	...
	if (CGameLobby* pGameLobby = g_pGame->GetGameLobby())
	{
		...
		i32k numPlayers = sessionNames.Size();
		for (i32 i = 0; i < numPlayers; ++ i)
		{
			i32k channelId = (i32)sessionNames.m_sessionNames[i].m_conId.m_uid;
			m_initialChannelIds.push_back(channelId);
			DrxLog("CGameRulesStandardState::PostInit(), Initial player channels, adding %d)", channelId);
		}
	}
}
...
{
	DrxLog("OwnClientEnteredGame() for '%s', ShouldPlayIntro: %s, state: %i", rPlayer.GetEntity()->GetName(), rPlayer.ShouldPlayIntro() ? "true" : "false", m_state);
	...
	if(!rPlayer.ShouldPlayIntro() && m_state == IGameRulesStateModule::EGRS_Intro)
	{
		...
		DrxLog("CGameRulesStandardState::OwnClientEnteredGame(), state changed from %i to %i", (i32)m_state, (i32)m_lastReceivedServerState);
...
void CGameRulesStandardState::Add3DWinningTeamMember()
{
	...
	if (localActor != NULL && static_cast<CActor*>(localActor)->GetSpectatorState() != CActor::eASS_SpectatorMode && !g_pGame->GetUI()->IsInMenu())
	{
		...
		if (m_pGameRules->GetTeamCount() > 1)
		{
			...
			i32k teamId = m_pGameRules->GetTeam(localActor->GetEntityId());
			assert(teamId == 1 || teamId == 2);
			i32k enemyTeamId = 3 - teamId;

			i32k clientTeamScore = m_pGameRules->GetTeamsScore(teamId);
			i32k nonClientTeamScore = m_pGameRules->GetTeamsScore(enemyTeamId);
			bool winning = (clientTeamScore >= nonClientTeamScore);

			DrxLog ("Want to add 3D model of winning team member... local player '%s' is %s", localActor->GetEntity()->GetName(), winning ? " on the winning team" : "not on the winning team");
			if (! winning)
			{
		...
		{
			DrxLog ("Adding 3D model of '%s' to accompany scoreboard", renderModelFromEntity->GetName());
			INDENT_LOG_DURING_SCOPE();

			ICharacterInstance * characterInst = renderModelFromEntity->GetCharacter(0);
			if (characterInst != NULL)
			{
				CMenuRender3DModelMgr *pModelUpr = new CMenuRender3DModelMgr();
				...
				pModelUpr->SetSceneSettings(sceneSettings);

				tukk  usePlayerModelName = characterInst->GetFilePath();
				...
				const CMenuRender3DModelMgr::TAddedModelIndex characterModelIndex = pModelUpr->AddModel(params);
				pModelUpr->UpdateAnim(characterModelIndex, "stand_tac_idle_nw_3p_01", ANIM_SPEED_MULTIPLIER);
			}
		}
	}
}
...
bool CGameRulesStandardState::CheckInitialChannelPlayers()
{
	...
	if (CGameLobby* pGameLobby = g_pGame->GetGameLobby())
	{
		...
		i32 inGameCount = 0;
		i32 initialNumPlayers = m_initialChannelIds.size();
		for (i32 i = 0; i < initialNumPlayers; ++ i)
		{
			i32k channelId = m_initialChannelIds[i];

			SDrxMatchMakingConnectionUID connectionId;
			...
			if (sessionNames.FindIgnoringSID(connectionId) == SSessionNames::k_unableToFind)
			{
				...
				DrxLog("CGameRulesStandardState::CheckInitialChannelPlayers(), Client channel %d is in the initial channel list but no longer in the lobby, removing)", channelId);
		...
		//DrxLog("CGameRulesStandardState::CheckInitialChannelPlayers(), Waiting for channels to join %d of %d clients have joined (%.2f / %.2f required, %f sec til starting anyway)", inGameCount, initialNumPlayers, currentRatio, startTimerPlayerRatio, m_startTimerOverrideWait);
...
void CGameRulesStandardState::Update( float frameTime )
{
	if (gEnv->bServer)
	{
		...
		if (m_state == EGRS_PreGame)
		{
			if (m_isStarting)
			{
				...
				{
					DrxLog("CGameRulesStandardState::Update(), starting game");
			...
			else
			{
				...
				i32k numPlayers = m_pGameRules->GetPlayerCount(true);
						...
						if (!bOk)
						{
							...
							if (publicGame && onlineGame)
							{
								...
								if (m_pGameRules->GetTeamCount() > 1)
								{
									...
									i32k numPlayersPerTeamMin = g_pGameCVars->g_gameRules_startTimerMinPlayersPerTeam;
									i32k numPlayersTeam1 = m_pGameRules->GetTeamPlayerCount(1, true);
									i32k numPlayersTeam2 = m_pGameRules->GetTeamPlayerCount(2, true);
								...
								else
								{
									...
									i32k numPlayersMin = g_pGameCVars->g_gameRules_startTimerMinPlayers;
									bOk = (numPlayers >= numPlayersMin);
								}
								i32k numPlayersInLobby = pGameLobby->GetSessionNames().Size();
				...
				if (bOk)
				{
					DrxLog("CGameRulesStandardState::Update(), we have %i players, starting the game", numPlayers);
					...
					{
						i32 numRequiredPlayers = g_pGameCVars->g_minPlayersForRankedGame - m_pGameRules->GetPlayerCount(true);
						if ((numRequiredPlayers > 0) || (pGameLobby && pGameLobby->UseLobbyTeamBalancing() && !pGameLobby->IsGameBalanced()))
						{
							...
							{
								if (!pGameLobby->IsRankedGame() || pPlaylistUpr->IsUsingCustomVariant())
								{
		...
		else if (m_state == EGRS_PostGame)
		{
			...
			if (timeInPost > totalPostGameTime)
			{
				...
				if (!pRecordingSystem || (!pRecordingSystem->IsPlaybackQueued() && !pRecordingSystem->IsPlayingBack()))
				{
					...
					if (pGameLobby)
					{
						DrxLog("[GameRules] Server trying to return to lobby");
	...
	if(g_pGameCVars->g_hud_postgame_debug)
	{
		...
		DrxWatch("GameRulesStandardState - State = %s", stateName);

		if(m_state == EGRS_PostGame)
		{
			tukk postGameStateName = "";
			...
			DrxWatch("GameRulesStandardState -PostGameState = %s", postGameStateName);
		...
		else if (m_state == EGRS_InGame && !gEnv->IsDedicated() )
		{
			if (m_introMessageShown == false)	// Show only once
			{
				...
				if (pGameRules && pGameRules->HasGameActuallyStarted())
				{
					...
					{
						i32 teamId = g_pGame->GetGameRules()->GetTeam(localPlayerId);
						...
						if (pActor->GetSpectatorMode()==CActor::eASM_None && !pActor->IsDead() && (!bTeamGame || teamId!=0))
						{
							if (IGameRulesPlayerStatsModule *statsModule = pGameRules->GetPlayerStatsModule())
							{
								...
								if (stats->deaths <= 0) // Not died ever
								{
									...
									{
										tukk gamemodeName = g_pGame->GetGameRules()->GetEntity()->GetClass()->GetName();
										...
										DrxFixedStringT<64> localisedStartString = CHUDUtils::LocalizeString( m_startMatchString.c_str() );
		...
		else if(m_state == EGRS_PostGame && !gEnv->IsDedicated())
		{
			...
			switch (m_postGameState)
			{
				...
				case ePGS_FinalKillcam:
				{
					...
					{
						DrxWatch("PostGameState - Waiting for final killcam to end:"); 
						DrxWatch("IsPlaybackQueued: %s, IsPlayingBack: %s", 
				...
				case ePGS_HighlightReel:
				{
					...
					if (!pRecordingSystem || (!pRecordingSystem->IsPlaybackQueued() && !pRecordingSystem->IsPlayingBack()))
					{
						...
						if (pGameLobby)
						{
							DrxLog("[GameRules] Client trying to return to lobby");
							pGameLobby->SvFinishedGame(frameTime);
					...
					{
						DrxWatch("PostGameState - Waiting for highlight reel to end:");
						DrxWatch("IsPlaybackQueued: %s, IsPlayingBack: %s, IsPlayingHighlightsReel: %s", 
				...
				case ePGS_Scoreboard:
				{
					if(m_timeInCurrentPostGameState > g_pGameCVars->g_gameRules_postGame_ScoreboardTime)
					{
						...
						else
						{
							...
							if (pGameLobby)
							{
								DrxLog("[GameRules] Client trying to return to lobby [No highlight reel]");
								pGameLobby->SvFinishedGame(frameTime);
							}
						}
					}
...
void CGameRulesStandardState::EnterPostGameState( EPostGameState state )
{
	...
	if(g_pGameCVars->g_hud_postgame_debug)
	{
		DrxLog("HUD PostGame - Entering new state %i from %i after %.2f seconds", (i32)state, (i32)m_postGameState, m_timeInCurrentPostGameState);
	}
#endif
	...
	switch (m_postGameState)
	{
			...
			if (g_pGameCVars->kc_enableWinningKill)
			{
				...
				if (pRecordingSystem)
				{
					if(pRecordingSystem->IsPlaybackQueued())
					{
						...
						{
							DRX_ASSERT_MESSAGE(false, "There should be a Winning Kill queued up when we get here...");
						}
					}
					else
					{
		...
		case ePGS_HighlightReel:
		{
			...
			if (pRecordingSystem)
			{
				if (!(pRecordingSystem->IsPlayingBack() || pRecordingSystem->IsPlaybackQueued()))
				{
					...
					if(g_pGameCVars->g_hud_postgame_debug)
					{
						DrxLog("HUD PostGame - Could play highlight reel = %s", canPlayHighlightReel ? "True" : "False");
					}
#endif
				}
			}
			...
			{
				DrxLog("HUD PostGame - Failed to change spectator mode when entering highlight reel");
			}
#endif
		...
		case ePGS_Scoreboard:
		{
			DrxLog("HUD PostGame - Activating default state for scoreboard");
...
//-------------------------------------------------------------------------
bool CGameRulesStandardState::NetSerialize( TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags )
{
			...
			if(!m_pGameRules->IsIntroSequenceCurrentlyPlaying())
			{
				...
				if(bApplyChange)
				{
					if (m_state != m_lastReceivedServerState)
					{
						...
						DrxLog("CGameRulesStandardState::NetSerialize(), state changed from %i to %i", (i32)m_state, (i32)m_lastReceivedServerState);
						ChangeState(m_lastReceivedServerState);
...
void CGameRulesStandardState::OnGameReset()
{
	if (gEnv->bServer)
	{
		...
		DrxLog("CGameRulesStandardState::OnGameReset(), setting state to InGame");
		ChangeState(EGRS_InGame);
	}
	else
	{
...
void CGameRulesStandardState::ChangeState( EGR_GameState newState )
{
	if (gEnv->bServer)
	{
		...
		else if (newState == EGRS_PostGame)
		{
			...
			if (pGameLobby)
			{
				DrxLog("[GameRules] GameFinished, telling clients");
...
void CGameRulesStandardState::CmdSetState( IConsoleCmdArgs *pArgs )
{
	if (gEnv->bServer)
	{
		tukk pState = pArgs->GetArg(1);
		if (pState)
		{
			i32 state = atoi(pState);
			if (state >= 0 && state < (i32) EGRS_MAX)
			{
...
bool CGameRulesStandardState::IsInitialChannelId( i32 channelId ) const
{
	u32 initialChannelsSize = m_initialChannelIds.size();
	for(u32 i = 0; i < initialChannelsSize; ++i)
	{
...
void CGameRulesStandardState::OnIntroCompleted()
{
	...
	else
	{
		...
		DrxLog("CGameRulesStandardState::OnIntroCompleted(), state changed from %i to %i", (i32)m_state, (i32)m_lastReceivedServerState);
		ChangeState(m_lastReceivedServerState);
	}
}
