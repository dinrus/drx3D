...
#include <drx3D/Game/GameRulesObjective_CTF.h>
...
#ifndef _RELEASE
	#define FATAL_ASSERT(expr) if (!(expr)) { DrxFatalError("CGameRulesObjective_CTF - FatalError"); }
#else
	#define FATAL_ASSERT(...) {}
#endif
...
//-------------------------------------------------------------------------
template <typename T>
void GameRulesUtils_CallEntityScriptFunction(EntityId scriptEntId, tukk function, T param)
{
...
void GameRulesUtils_CallEntityScriptFunction(EntityId scriptEntId, tukk function, T1 param1, T2 param2)
{
	if (IEntity* pEntity=gEnv->pEntitySystem->GetEntity(scriptEntId))
	{
		IScriptTable*  pScript = pEntity->GetScriptTable();
...
CGameRulesObjective_CTF::~CGameRulesObjective_CTF()
{
	...
	for (i32 teamIndex = 0; teamIndex < 2; ++ teamIndex)
	{
	...
	for (i32 teamIndex = 0; teamIndex < 2; ++ teamIndex)
	{
		...
		if (state == ESES_Dropped)
		{
			...
			else
			{
				...
				i32 timeLeftFrame = (i32) (100 * timeLeftFraction);
				...
				if (timeLeftFrame != pTeamInfo->m_droppedIconFrame)
				{
					...
					newUpdateObjectiveClock.AddData(static_cast<i32>(pTeamInfo->m_flagId));
...
//-------------------------------------------------------------------------
void CGameRulesObjective_CTF::PostInitClient( i32 channelId )
{
	for (i32 teamIndex = 0; teamIndex < 2; ++ teamIndex)
	{
		STeamInfo *pTeamInfo = &m_teamInfo[teamIndex];
	...
	if (becomeServer)
	{
		...
		for (i32 teamIndex = 0; teamIndex < 2; ++ teamIndex)
		{
			...
			if ((pTeamInfo->m_state == ESES_Carried) && (pTeamInfo->m_carrierId))
			{
				...
				if (!pCarrier)
				{
					DrxLog("CGameRulesObjective_CTF::OnHostMigration() found carrier that doesn't exist anymore (%u), dropping", pTeamInfo->m_carrierId);
...
{
	DrxLog("CGameRulesObjective_CTF::OnSingleEntityRMI() RMI type=%d, entityId=%u", params.m_data, params.m_entityId);
...
{
	DrxLog("CGameRulesObjective_CTF::OnDoubleEntityRMI() RMI type=%d, entityId1=%u, entityId2=%u", params.m_data, params.m_entityId1, params.m_entityId2);
	switch (params.m_data)
	{
		case eRMI_ResetFlag:
...
{
	DrxLog("CGameRulesObjective_CTF::OnSvClientActionRMI() RMI type=%d, fromEid=%u", params.m_action, fromEid);
...
//-------------------------------------------------------------------------
void CGameRulesObjective_CTF::OnChangedTeam(EntityId entityId, i32 oldTeamId, i32 newTeamId)
{
	for (i32 teamIndex = 0; teamIndex < 2; ++ teamIndex)
	{
		STeamInfo *pTeamInfo = &m_teamInfo[teamIndex];
...
//-------------------------------------------------------------------------
void CGameRulesObjective_CTF::OnAction(const ActionId &action, i32 activationMode, float value)
{
	...
	if (event.event == ENTITY_EVENT_ENTERAREA)
	{
		...
		if (pActor != NULL && pActor->IsPlayer() && !pActor->IsDead())
		{
			for (i32 teamIndex = 0; teamIndex < 2; ++ teamIndex)
			{
				...
				{
					i32 otherTeamIndex = (1 - teamIndex);
...
void CGameRulesObjective_CTF::OnPlayerKilledOrLeft(EntityId targetId, EntityId shooterId)
{
	if (gEnv->bServer)
	{
		...
		{
			DrxLog("CGameRulesObjective_CTF::OnPlayerKilledOrLeft() carrier (%d) has died, dropping flag", targetId);
...
//-------------------------------------------------------------------------
i32 CGameRulesObjective_CTF::FindTeamIndexByCarrierId(EntityId playerId) const
{
	for (i32 teamIndex = 0; teamIndex < 2; ++ teamIndex)
	{
...
{
	i32 teamIndex = FindTeamIndexByCarrierId(playerId);
	if (teamIndex >= 0)
	{
...
{
	i32 teamId = m_pGameRules->GetTeam(baseId);
...
{
	i32 teamId = m_pGameRules->GetTeam(flagId);
	FATAL_ASSERT((teamId == 1) || (teamId == 2));
...
void CGameRulesObjective_CTF::Server_ResetAll()
{
	...
	if (m_teamInfo[0].m_baseId == 0)
	{
		...
		while (IEntity *pEntity = pIt->Next())
		{
			...
			{
				EntityId entityId = pEntity->GetId();
				i32 teamId = m_pGameRules->GetTeam(entityId);
				...
				if ((teamId == 1) || (teamId == 2))
				{
					...
					DrxLog("CGameRulesObjective_CTF::Server_ResetAll() setup state for base, teamId=%d, base=%d, flag=%d, weapon=%d", teamId, pTeamInfo->m_baseId, pTeamInfo->m_flagId, pTeamInfo->m_fallbackWeaponId);
				}
			}
		}
	}
...
void CGameRulesObjective_CTF::Server_SwapSides()
{
	...
	i32 base1Team = m_pGameRules->GetTeam(base1);
...
{
	if (CActorUpr* pActorUpr = CActorUpr::GetActorUpr())
	{
		i32k numActors	= pActorUpr->GetNumActorsIncludingLocalPlayer();
...
void CGameRulesObjective_CTF::Common_Pickup(EntityId playerId)
{
	...
	if ((teamId == 1) || (teamId == 2))
	{
	 	...
	 	if (previousState	== ESES_AtBase)
		{
			...
			DrxLog("CGameRulesObjective_CTF::Common_Pickup() m_baseAlarmSound[%d] play for entityId=%d", teamIndex, pTeamInfo->m_baseId);
...
void CGameRulesObjective_CTF::Common_Drop(EntityId playerId)
{
	...
	if (STeamInfo *pTeamInfo = FindInfoByCarrierId(playerId))
	{
		...
		if (gEnv->IsClient())
		{
			i32 teamIndex = (i32)(pTeamInfo - m_teamInfo);
...
void CGameRulesObjective_CTF::Common_ResetFlag(EntityId flagId, EntityId playerId)
{
	if (STeamInfo *pTeamInfo = FindInfoByFlagId(flagId))
	{
		...
		if (gEnv->IsClient())
		{
			...
			DrxLog("CGameRulesObjective_CTF::Common_ResetFlag() m_baseAlarmSound[%d] stop for entityId=%d", teamIndex, pTeamInfo->m_baseId);
...
void CGameRulesObjective_CTF::Common_Scored(EntityId playerId)
{
	...
	if (teamIndex >= 0)
	{
		...
		//m_baseAlarmSound[teamIndex].Stop(pTeamInfo->m_baseId);
		DrxLog("CGameRulesObjective_CTF::Common_Scored() m_baseAlarmSound[%d] stop for entityId=%d", teamIndex, pTeamInfo->m_baseId);
	...
	for (i32 teamIndex = 0; teamIndex < 2; ++ teamIndex)
	{
		...
		switch (pTeamInfo->m_state)
		{
			...
			case eSES_Unknown:
				AttachFlagToBase(pTeamInfo);
				//m_baseAlarmSound[teamIndex].Stop(pTeamInfo->m_baseId);
				DrxLog("CGameRulesObjective_CTF::Common_ResetAll() m_baseAlarmSound[%d] stop for entityId=%d", teamIndex, pTeamInfo->m_baseId);
				break;
		}
	}
...
//-------------------------------------------------------------------------
void CGameRulesObjective_CTF::Client_SetIconForBase(i32 teamIndex)
{
	EntityId clientActorId = g_pGame->GetIGameFramework()->GetClientActorId();
	i32 localTeamId = m_pGameRules->GetTeam(clientActorId);
...
//-------------------------------------------------------------------------
void CGameRulesObjective_CTF::Client_SetIconForFlag(i32 teamIndex)
{
	FATAL_ASSERT(gEnv->IsClient());
	EntityId flagId = m_teamInfo[teamIndex].m_flagId;
	if (m_teamInfo[teamIndex].m_state == ESES_Dropped)
	{
		i32 localTeamId = m_pGameRules->GetTeam(g_pGame->GetIGameFramework()->GetClientActorId());
		bool bFriendly = (localTeamId == (teamIndex + 1));
		if (bFriendly)
		{
...
//-------------------------------------------------------------------------
void CGameRulesObjective_CTF::Client_SetIconForCarrier(i32 teamIndex, EntityId carrierId, bool bRemoveIcon)
{
	FATAL_ASSERT(gEnv->IsClient());
	if (bRemoveIcon)
	{
