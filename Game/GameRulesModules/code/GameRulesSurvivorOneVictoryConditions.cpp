...
#include <drx3D/Game/GameRulesSurvivorOneVictoryConditions.h>
...
#define DbgLog(...) DRX_DEBUG_LOG(GAMEMODE_ALLORNOTHING, __VA_ARGS__)
#define DbgLogAlways(...) DRX_DEBUG_LOG_ALWAYS(GAMEMODE_ALLORNOTHING, __VA_ARGS__)

#if DRX_WATCH_ENABLED
#define WATCH_LV1  g_pGameCVars->g_SurvivorOneVictoryConditions_watchLvl<1 ? (NULL) : DrxWatch
#define WATCH_LV2  g_pGameCVars->g_SurvivorOneVictoryConditions_watchLvl<2 ? (NULL) : DrxWatch
#else
#define WATCH_LV1
#define WATCH_LV2
#endif
...
#define WATCH_LATEST_SURVIVOR_COUNT  (1 && DRX_WATCH_ENABLED)
...
CGameRulesSurvivorOneVictoryConditions::CGameRulesSurvivorOneVictoryConditions()
{
	if (gEnv->bServer)
	{
		...
		memset(m_svLatestSurvList, 0, (DRX_ARRAY_COUNT(m_svLatestSurvList)));
	}
			...
			if (m_radarPingTimer <= 0.f)
			{
				...
				while (IActor* pActor = pActorIterator->Next())
				{
					...
					if (actorEntityId != localPlayerId && pActor->IsPlayer() && pActor->GetChannelId() && !pActor->IsDead())
					{
						...
						eventTempAddToRadar.AddData( static_cast<i32>(actorEntityId) );
...
void CGameRulesSurvivorOneVictoryConditions::Update( float frameTime )
{
	...
	if (pScoringModule)
	{
		...
		DrxWatch("deathScoreModifier=%d", currentDeathScoringModifier);
	}
#endif
	...
	for (i32 i=0; i<m_svLatestSurvCount; i++)
	{
...
//-------------------------------------------------------------------------
void CGameRulesSurvivorOneVictoryConditions::ClVictoryPlayer( i32 playerId, EGameOverReason reason, const EntityId killedEntity, const EntityId shooterEntity )
{
...
void CGameRulesSurvivorOneVictoryConditions::SvSurvivorCountRefresh(i32 count, const EntityId survivors[], i32 numKills)
{
	...
	if (count != m_svLatestSurvCount && pScoringModule != NULL)
	{
		...
		{
			i32 survivorDiff=m_svLatestSurvCount-count;
			TGameRulesScoreInt currentDeathScoringModifier = pScoringModule->GetDeathScoringModifier();
	...
	i32  sz = (count * sizeof(survivors[0]));
	DRX_ASSERT_MESSAGE(sz <= sizeof(m_svLatestSurvList), "Memory overwrite!");
