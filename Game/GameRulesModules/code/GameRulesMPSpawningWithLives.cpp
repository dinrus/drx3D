...
#include <drx3D/Game/GameRulesMPSpawningWithLives.h>
...
void CGameRulesMPSpawningWithLives::Init(XmlNodeRef xml)
{
	...
	if (xml->getAttr("numLives", m_numLives))
	{
		...
		DRX_ASSERT_MESSAGE(g_pGame->GetGameRules()->GetGameMode() == eGM_Assault, string().Format("SpawningWithLives expects only the assault gamemode to override numLives within the xml. Yet gamemode=%d is trying to", g_pGame->GetGameRules()->GetGameMode()));
		DrxLog("CGameRulesMPSpawningWithLives::Init() is setting numLives from the xml to %d", m_numLives);
	}
	else
	{
		m_numLives = g_pGameCVars->g_numLives;
		DrxLog("CGameRulesMPSpawningWithLives::Init() has no numLives set in the xml. Using the current cvar value=%d", m_numLives);
	...
	DRX_TODO(23,11,09, "Need to use localisable strings from string tables for these.");
			...
			{
				{
					...
					{
						i32  livesRem = (m_numLives - s->deathsThisRound);
...
void CGameRulesMPSpawningWithLives::PerformRevive(EntityId playerId, i32 teamId, EntityId preferredSpawnId)
{
	...
	DrxLog("[tlh] @ CGameRulesMPSpawningWithLives::PerformRevive(playerId=%d, teamId=%d), player='%s'", playerId, teamId, (pEntity?pEntity->GetName():"!"));
	...
	if( pPlayer != NULL && (pPlayer->IsDead() || pPlayer->GetSpectatorMode() != CActor::eASM_None/*Spectating*/) )
	{
		...
		{
			DrxLog("      > game state = %d", pStateModule->GetGameState());
						...
						if (const SGameRulesPlayerStat* s=pPlayStatsMod->GetPlayerStats(playerId))
						{
							if (!isSuddenDeath || !(s->flags & SGameRulesPlayerStat::PLYSTATFL_DIEDINEXTRATIMETHISROUND))
							{
										...
										(IsInInitialChannelsList(pPlayer->GetChannelId())) ) // mid-game joining is only allowed whilst no deaths have happened this round
								{
									...
									{
										DrxLog("      > performing life-limited revive, frame = %d", gEnv->pRenderer->GetFrameID());
										CCCPOINT(MPSpawningWithLives_SvPerformReviveAllowed);
									}
									else
									{
										perform = false;
										DrxLog("      > not reviving player %s - no lives remaining", pPlayer->GetEntity()->GetName());
								...
								{
									perform = false;
									DrxLog("      > not reviving player %s - others died already", pPlayer->GetEntity()->GetName());
							...
							else
							{
								...
								DrxLog("      > not reviving player %s - sudden death", pPlayer->GetEntity()->GetName());
							}
						}
						else
						{
							DRX_ASSERT_MESSAGE(0, string().Format("CGameRulesMPSpawningWithLives::PerformRevive() failed to find playerstats for player"));
						}
					}
					break;
				}
	...
	{
		DrxLog("      > player is still/already alive");
	}
}
...
void CGameRulesMPSpawningWithLives::EntityRevived(EntityId entityId)
{
	...
	if (gEnv->bServer)
	{
		...
		if (IGameRulesPlayerStatsModule* pPlayStatsMod=m_pGameRules->GetPlayerStatsModule())
		{
			...
			{
				DRX_ASSERT_MESSAGE((s->flags & SGameRulesPlayerStat::PLYSTATFL_HASSPAWNEDTHISROUND), "The PlayerStats module needs to come before the Spawning module in the XML for this game mode.");
...
void CGameRulesMPSpawningWithLives::ClPlayerStatsNetSerializeReadDeath(const SGameRulesPlayerStat* s, u16 prevDeathsThisRound, u8 prevFlags)
{
	DrxLog("[tlh] @ CGameRulesMPSpawningWithLives::ClPlayerStatsNetSerializeReadDeath(s=%p, prevDeathsThisRound=%u, prevFlags=%u)", s, prevDeathsThisRound, prevFlags);
...
void CGameRulesMPSpawningWithLives::ClRoundsNetSerializeReadState(i32 newState, i32 curState)
{
	DRX_ASSERT(!gEnv->bServer);
	DRX_ASSERT(gEnv->IsClient());
...
void CGameRulesMPSpawningWithLives::OnPlayerElimination(EntityId playerId)
{
	if (!m_tmpPlayerEliminatedMsg.empty())
	{
		...
		tukk sz_msg = msg.c_str();
		*/

		DRX_TODO( 22, 07, 2010, "HUD: Notify player of player elimination in All or Nothing.")
	}
...
void CGameRulesMPSpawningWithLives::PlayerJoined(EntityId playerId)
{
	if (m_numLives)
	{
		DRX_ASSERT(!m_pGameRules->GetPlayerStatsModule() || m_pGameRules->GetPlayerStatsModule()->GetPlayerStats(playerId));  // it's important that the playerstats for this player are created before this function is called
		if (gEnv->bServer)
		{
...
void CGameRulesMPSpawningWithLives::PlayerLeft(EntityId playerId)
{
	if (m_numLives)
	{
		DRX_ASSERT(!m_pGameRules->GetPlayerStatsModule() || !m_pGameRules->GetPlayerStatsModule()->GetPlayerStats(playerId));  // it's important that the playerstats for the player are removed before this function is called
		if (gEnv->bServer)
		{
...
void CGameRulesMPSpawningWithLives::OnEntityKilled(const HitInfo &hitInfo)
{
	...
	if (gEnv->bServer)
	{
		...
		DrxLog("[tlh] @ CGameRulesMPSpawningWithLives::OnEntityKilled(), entity=%s", (e?e->GetName():"!"));
		...
		if (IGameRulesPlayerStatsModule* pPlayStatsMod=m_pGameRules->GetPlayerStatsModule())
		{
			...
			if (pActor != NULL && pActor->IsPlayer())
			{
				...
				{
					DrxLog("      > got OnEntityKilled broadcast, frame = %d", gEnv->pRenderer->GetFrameID());
					bool  isSuddenDeath = IsInSuddenDeath(); 
					if (IGameRulesStateModule* pStateModule=m_pGameRules->GetStateModule())
					{
				...
				{
					DRX_ASSERT_MESSAGE(0, string().Format("CGameRulesMPSpawningWithLives::OnEntityKilled() failed to find a playerstats for player"));
				}
			}
		}
	}
...
{
	DrxLog("[tlh] @ CGameRulesMPSpawningWithLives::SvNotifySurvivorCount()");
	DRX_ASSERT(gEnv->bServer);
				...
				if (s->deathsThisRound < m_numLives)
				{
					if (!isSuddenDeath || !(s->flags & SGameRulesPlayerStat::PLYSTATFL_DIEDINEXTRATIMETHISROUND))
					{
						...
						}
						{
							...
							DrxLog("      > added player '%s' to survivor array at idx %d", (e?e->GetName():"!"), (count - 1));
						}
					}
				}
			}
...
i32 CGameRulesMPSpawningWithLives::GetRemainingLives(EntityId playerId)
{
	if (IGameRulesStateModule* pStateModule = m_pGameRules->GetStateModule())
	{
...
{
	DRX_ASSERT(m_elimMarkerDuration > 0.f);
	if (IEntity* e=gEnv->pEntitySystem->GetEntity(playerId))
	{
		DRX_ASSERT_MESSAGE(m_freeElimMarker, "Ran out of space in ElimMarkers array.");
		if (m_freeElimMarker)
		{
			SEntitySpawnParams  params;
			params.sName = string().Format(" %s%s ", e->GetName(), m_tmpPlayerEliminatedMarkerTxt.c_str());
			DRX_ASSERT(gEnv->pGame->GetIGameFramework()->GetIActorSystem()->GetActor(gEnv->pEntitySystem->FindEntityByName(e->GetName())->GetId())->IsPlayer());
			...
			DRX_ASSERT_MESSAGE(pMarkerEnt, "Couldn't create local client entity for ElimMarker.");
			if (pMarkerEnt)
			{
				...
				DRX_ASSERT_MESSAGE(!m_freeElimMarker || ((m_freeElimMarker >= &m_elimMarkers[0]) && (m_freeElimMarker <= &m_elimMarkers[m_kMaxElimMarkers - 1])), "Stomping memory!!");
...
{
	for (i32 i=0; i<m_kMaxElimMarkers; i++)
	{
		SElimMarker*  pMarker = &m_elimMarkers[i];
		if (pMarker->markerEid)
		{
			DRX_ASSERT(gEnv->pEntitySystem->GetEntity(pMarker->markerEid));
