...
void CGameRulesRSSpawning::PopulateSpawnData(SUsefulSpawnData& spawnData, EntityId playerId)
{
	...
	i32 playerTeamId = m_pGameRules->GetTeam(playerId);
	i32 enemyTeamId	 = m_pGameRules->GetEnemyTeamId(playerTeamId);
	...
	i32 numActiveFriendlyPlayers = 0;
...
{
	//This is now a blueprint for switching over the other functions to use the CActorUpr.
...
void CGameRulesRSSpawning::UpdateMapCentre()
{
	...
	i32 numInitialSpawnGroups = m_initialSpawnLocations.size();
	for (i32 i = 0; i < numInitialSpawnGroups; ++ i)
	{
...
#if !defined(_RELEASE)
	i32 nWrongTeamSpawns = 0, nUnsafeSpawns = 0;
#endif
...
float CGameRulesRSSpawning::GetScoreFromProximityToEnemies( SUsefulSpawnData& spawnData, TPositionList& EnemyPlayerPositions, const Vec3& potentialSpawnPosition )
{
	...
	if(spawnData.numActiveEnemyPlayers > 0)
	{
		...
		for(i32 i = EnemyPlayerPositions.size() - 1; i >= 0; i--)
		{
																																...
																																TMultiplierList& FriendlyScoreMultipliers, const Vec3& potentialSpawnPosition )
{
	...
	if(spawnData.numActiveFriendlyPlayers > 0)
	{
		...
		for(i32 i = FriendlyPlayerPositions.size() - 1; i >= 0; i--)
		{
