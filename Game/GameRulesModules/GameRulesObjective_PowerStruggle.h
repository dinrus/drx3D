...
private:
	static i32k MAX_NODES = 3;
	static i32k MAX_PLAYERS_PER_TEAM  = (MAX_PLAYER_LIMIT / 2);
	static i32k NUM_TEAMS = 2;
	static i32k CONTESTED_TEAM_ID = -1;

	typedef DrxFixedArray<EntityId, MAX_PLAYERS_PER_TEAM> TPlayerIdArray;
	typedef DrxFixedStringT<32> TFixedString;

	struct SNodeIdentityInfo
	{
		SNodeIdentityInfo(EActiveIdentityId _eIdentity, tukk  _pIconName) { eIdentity = _eIdentity; pIconName = _pIconName; }
	...
	struct SNodeInfo
	{
		...
		EntityId m_id;
		i32 m_controllingTeamId;		// describes the current state of this node. neutral, team1, team2, contested
		i32 m_lastControllingTeamId;	// describes the last set current state of this node. is consulted when in contention to see who the last controllingTeam was
		...
		i32 m_cameraShakeId;
		u8 m_lastCapturedTeamId;		// 0: never captured; 1,2 for team that last captured it
		u8 m_captureStatusOwningTeam; // 0: never captured; 1,2 for the team who's owning captureStatus
	...
	bool NetSerializeNode(SNodeInfo *pNodeInfo, TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags);
	...
	void Common_AddNode(IEntity *pNode, i32 forceIndex);
	...
	void Server_ActivateNode(i32 index);
	...
	i32 Server_CalculateWinningTeam(); 
	...
	void LogSpearStateToTelemetry( u8 state, SNodeInfo* pSpearInfo );

	// TODO - move these arrays into DrxFixedArray for added range safety
	SNodeInfo m_nodes[MAX_NODES];
	SChargeeInfo m_Chargees[NUM_TEAMS];

	DrxFixedArray< EntityId, MAX_PLAYER_LIMIT > m_soloCapturingPlayers;
