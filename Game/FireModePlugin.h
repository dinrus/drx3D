...
{
public:
	DRX_INTERFACE_GTI(IFireModePlugin);

	IFireModePlugin() {}
	virtual ~IFireModePlugin () {}

	virtual bool Update(float frameTime, u32 frameId) = 0;
...
public:
	...
	{
		DRX_ASSERT_MESSAGE(pFiremode, "NULL firemode passed to plugin, not possible!");
		...
		{
			DRX_ASSERT_MESSAGE(0, "Incorrect or NULL parameters for plugin");
			return false;
		}
	...
	virtual bool Update(float frameTime, u32 frameId) { return false; }
...
{
public:
	DRX_DECLARE_GTI(CFireModePlugin_Overheat);

	CFireModePlugin_Overheat();
	virtual ~CFireModePlugin_Overheat() {};

	virtual bool Update(float frameTime, u32 frameId) override;
...
{
public:
	DRX_DECLARE_GTI(CFireModePlugin_Reject);
...
{
public:
	DRX_DECLARE_GTI(CFireModePlugin_RecoilShake);
