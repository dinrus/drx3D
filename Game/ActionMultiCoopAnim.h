...
$DateTime$
Описание:
	Реализует Multiple Actor Coop Anims.
...
public:
	...
	struct SParticipant
	{
		...
		SParticipant( IAnimatedCharacter& animChar, ICharacterInstance* pCharInst, TagID tagId, u32 contextId, const IAnimationDatabase* pAnimDb, const ParamFlags params );
	...
	struct SPlayerParticipant : public SParticipant
	{
		...
		SPlayerParticipant( CPlayer& player, TagID tagId, u32 contextId, const IAnimationDatabase* pAnimDb, const ParamFlags params );
		virtual void Enter();
		virtual void Exit();
		virtual void OnSlaveEnter(SParticipant& slave);
		virtual void OnSlaveExit(SParticipant& slave);
