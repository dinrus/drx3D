...
#include <drx3D/Game/MatchMakingUtils.h>
#include <drx3D/Game/Lobby/GameLobbyData.h>
...
namespace MatchmakingUtils
{
	...
	static DrxCriticalSection s_DrxCritsec;
	...
	void OnPostUpdate( float )
	{
		...
		DrxAutoLock<DrxCriticalSection> lock(s_DrxCritsec);
	...
	std::shared_ptr< Live::State::User > GetCurrentUser()
	{
		...
		return Live::State::Upr::Instance().GetUserById( pPlatformOS->UserGetId(userIndex) );
	}
#endif
	...
	void ShowAchievementsUI()
	{
		...
		if ( user != nullptr )
		{
			...
			task.then([](HRESULT)
			{
				PushDeferredAction([]()
				{
					DrxLogAlways("Achievement UI dismissed");
				});
			});
		}
#endif
...
#define MAX_PARTY 3
	...
	void ShowPartyUI()
	{
		...
		if ( user != nullptr )
		{
			...
			task.then([](HRESULT)
			{
				PushDeferredAction([]()
				{
					DrxLogAlways("Party UI dismissed");
				});
			});
		}
#endif
	...
	void SendMatchmakingStatus( Live::MPSessionState state )
	{
		...
		{
			DrxLogAlways( "MatchmakingUtils: Setting Matchmaking Status: %d", state );
		});
	...
	concurrency::task< HRESULT > MonitorMatchmaking( std::shared_ptr< Live::State::User > user )
	{
		...
		if( IsEqualGUID( s_matchStatusChangedGuid, NULL_GUID ) )
		{
				...
				[]( Live::MultiplayerSession::Ref const & sessionRef, TicketStatus status ) -> void
			{
				...
				DrxLogAlways( "MatchmakingUtils: Matchmaking status updated to: %d, %d", status, uiState );

			}, &s_matchStatusChangedGuid);

			assert( hr == S_OK );
			DrxLogAlways( "MatchmakingUtils: MonitorMatchmaking returned 0x%08x", hr );
		}
	...
	HRESULT PrepareConstants( string const & gameModeId, string const & playlistId, i32 mpRank, HSTRING * constants )
	{
		...
		// MatchpartitionId
		{
			u32 val = GameLobbyData::GetVersion();

			DrxLogAlways( "[MatchmakingUtils] GameLobbyData::GetVersion() (MatchPartitionId) = '%d'", val );
	...
	concurrency::task< HRESULT > MatchTest( std::shared_ptr< Live::State::User > user, const string& gameModeId, const string& playlistId, i32 mpRank )
	{
		...
		{
			DrxLogAlways( "[MatchmakingUtils] MatchTest: Matchmaking in progress, resuming from state: %d", matchTicketStatus );
			return concurrency::create_task([](){ return S_OK; });
		}
	...
	{
		DrxLogAlways( "ESraLibCore: %s", str );
	}

	HRESULT OnActivated( i32 const activationType )
	{
		DrxLogAlways( "MatchmakingUtils: OnActivated called with activationType %d.", activationType );

		{
			Live::State::s_loggingCall = &loggingCall;
			HRESULT hr = Live::State::Party::Instance().ReregisterCallbacks();
			DrxLogAlways( "MatchmakingUtils: OnActivated Party ReregisterCallbacks returned 0x%08x", hr );
		}
		...
		if( IsEqualGUID( partySessionChangedGuid, NULL_GUID ) )
		{
			HRESULT hr = Live::State::Party::Instance().RegisterPartySessionCallback(
				[]()
			{
				DrxLogAlways( "MatchmakingUtils: GameSessionReady callback fired" );
				PartySessionReady();
			...
			assert( hr == S_OK );
			DrxLogAlways( "MatchmakingUtils: OnActivated RegisterPartySession returned 0x%08x", hr );
		}
		...
		if( IsEqualGUID( partyMembersChangedGuid, NULL_GUID ) )
		{
			...
			{
				DrxLogAlways( "MatchmakingUtils: PartyMembersChanged callback fired" );
				enum
				{
					Xuid = 0,
					UserPtr
			...
			assert( hr == S_OK );
			DrxLogAlways( "MatchmakingUtils: OnActivated RegisterPartyMembers returned 0x%08x", hr );
		}
		...
		if(!resumeRegistered)
		{
			...
			{
				HRESULT hr = Live::State::Party::Instance().ReregisterCallbacks();
				DrxLogAlways( "MatchmakingUtils: OnActivated::Resume Party ReregisterCallbacks returned 0x%08x", hr );
				return S_OK;
			}).Get(), &resumeToken);
			...
			DrxLogAlways( "MatchmakingUtils: OnActivated Resume registration returned 0x%08x", hr );
		}
		...
		if(!networkStatusChangedRegistered)
		{
			...
			{
				DrxLogAlways( "MatchmakingUtils: Network status changed" );
				...
				DrxLogAlways( "MatchmakingUtils: Network status changed to %d", level );

				if( level != NetworkConnectivityLevel_XboxLiveAccess )
				{
					DrxLogAlways( "MatchmakingUtils: Reporting Live connectivity lost to UI" );
					SendMatchmakingStatus( Live::MPSessionState::Error );
				}
			...
			DrxLogAlways( "MatchmakingUtils: OnActivated Network status registration returned 0x%08x", hr );
		}
	...
	concurrency::task<bool> IsPartySessionValid( std::shared_ptr<Live::State::User> user )
	{
			...
			[user](HRESULT res)
		{
			...
			if( Live::State::Party::Instance().GetMatchType() == Live::State::MatchType::PartySession )
			{
				...
				DrxLog( "MatchmakingUtils: IsPartySessionValid: PartySession Found" );
					...
					[user, session]( HRESULT res )
				{
					...
					if( SUCCEEDED(res) && session->Members_.size() >= 2 && userIsInSession )
					{
						...
						DrxLog( "MatchmakingUtils: IsPartySessionValid: Status in Session: %d", multiplayerSessionStatus );
					...
					DrxLog( "MatchmakingUtils: IsPartySessionValid: PartySession is stale, clearing." );
			...
			[user]( HRESULT res )
		{
				...
				[user]()
			{
					...
					[user]( HRESULT res )
				{
					...
					{
						DrxLogAlways("Error from Polling session");
				...
				[]()
			{
				...
				{
					DrxLogAlways("UI Update party list");
				});
			});
	...
	{
		DrxLog( "MatchmakingUtils: PartySessionReady fired." );
		using namespace ABI::Microsoft::Xbox::Services::Matchmaking;
	...
	HRESULT SetReadyState( const bool isReady )
	{
		...
		if (user == nullptr)
		{
			DrxLogAlways( "[MatchmakingUtils]" __FILE__ "(" STRINGIFY(__LINE__) "): user fetch failed" );
			return E_NOT_VALID_STATE;
		}
			...
			[user, isReady]( HRESULT hr )
		{
			PushDeferredAction([user, isReady]()
			{
				...
				wstring x = DrxStringUtils::UTF8ToWStr( xuidStr.c_str() );
				DrxLogAlways("Send ready state to UI with XUID: %s", x);
			});
		});
	...
	HRESULT InitiateSession( const string& gameModeId, const string& playlistId, i32 mpRank )
	{
		...
		{
			HRESULT hr = Live::State::Party::Instance().ReregisterCallbacks();
			DrxLogAlways( "MatchmakingUtils: InitiateSession Party ReregisterCallbacks returned 0x%08x", hr );
		}
		...
		{
			DrxLogAlways( "[MatchmakingUtils]" __FILE__ "(" STRINGIFY(__LINE__) "): user.get failed or party invalid" );
			DrxLogAlways("UI: Set party is valid");
			return E_NOT_VALID_STATE;
		}

		if( Live::State::Party::Instance().PartySessionRef().IsValid() )
		{
			DrxLogAlways( "MatchmakingUtils: InitiateSession: Pre-existing Party Session Found" );
			TryPartySessionReady( user ).then(
				[user, gameModeId, playlistId, mpRank]( HRESULT res )
			{
				if( FAILED(res) )
				{
					DrxLogAlways( "MatchmakingUtils: InitiateSession: Pre-existing Party Session was stale, deleting and matching." );
		...
		{
			DrxLogAlways( "MatchmakingUtils: InitiateSession: Matchmaking" );
				...
				[user](HRESULT hr)
			{
				...
				if( FAILED( hr ) )
				{
					DrxLogAlways( "[MatchmakingUtils]" __FILE__ "(" STRINGIFY(__LINE__) "): MatchTest call failed: 0x%08X", hr );
					SendMatchmakingStatus( Live::MPSessionState::Error );
				}
				return hr;
			});
		...
		DrxLogAlways( "MatchmakingUtils: InitiateSession: Creating Session for Party" );
			...
			[user](HRESULT hr)
		{
			...
			if(FAILED( hr))
			{
				DrxLogAlways( "[MatchmakingUtils]" __FILE__ "(" STRINGIFY(__LINE__) "): PartySession call failed: 0x%08X", hr );
				SendMatchmakingStatus( Live::MPSessionState::Error );
				return hr;
			}
	...
	{
#if DRX_PLATFORM_DURANGO && defined(SUPPORT_DURANGO_LEGACY_MULTIPLAYER)
		DrxLog( "MatchmakingUtils: ShouldShowMatchFoundPopup called" );
			...
			[]( HRESULT )
		{
			...
			DrxLog( "MatchmakingUtils: ShouldShowMatchFoundPopup PartyView retrieved." );

			auto user = MatchmakingUtils::GetCurrentUser();
			if(!user)
			{
				DrxLog( "MatchmakingUtils: ShouldShowMatchFoundPopup no user found." );
				return;
			}

			auto const pendingMatchType = user->GetPendingMatchType();

			DrxLog( "MatchmakingUtils: ShouldShowMatchFoundPopup pending match type: %d.", pendingMatchType );

			if( pendingMatchType == Live::State::MatchType::PartyMatchSession )
			{
				DrxLog( "MatchmakingUtils: PartyMatchSession Found" );

				user->ChangeToPartyMatchSessionAsync().then(
					[user]( HRESULT res )
				{
					DrxLog( "MatchmakingUtils: Change to PartyMatchSession result: 0x%08x", res );
					...
					DrxLog( "MatchmakingUtils: TicketStatus: %d", currentStatus );
			...
			else if( pendingMatchType == Live::State::MatchType::PartySession )
			{
					...
					[]( bool isValid )
				{
					...
					PushDeferredAction([]()
					{
						DrxLog( "MatchmakingUtils: GAME_SESSION_READY");
						gEnv->pSystem->GetISystemEventDispatcher()->OnSystemEvent( ESYSTEM_EVENT_MATCHMAKING_GAME_SESSION_READY, 0, 0 );
					});
					return;
				});
	...
	void AssignLocalUserToGroup( i32k groupNumber )
	{
			...
			[]( HRESULT res )
		{
			PushDeferredAction([res]()
			{
				...
				{
					DrxLogAlways("UI: Group settings failed");
				}
			});
		});
#endif
	...
	wstring GetGamertag( const zu64 xuid )
	{
		...
		if (user != nullptr)
		{
			if (xuid == user->Xuid())
			{
				i32 userIndex = gEnv->pSystem->GetPlatformOS()->GetFirstSignedInUser();
				i32 userId = gEnv->pSystem->GetPlatformOS()->UserGetId( userIndex );
	...
	void ShowGamercard( zu64 xuid )
	{
		...
		if (userIndex != IPlatformOS::Unknown_User)
		{
			u32 requestor = gEnv->pSystem->GetPlatformOS()->UserGetId(userIndex);
	...
	string GetPlaylistId()
	{
		...
		return STxt( playlistId.begin(), playlistId.end()).c_str();
#else
		return "";
#endif
	}
