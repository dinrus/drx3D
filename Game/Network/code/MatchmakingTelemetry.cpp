...
$Id$
$DateTime$
Описание: Class for recording and sending matchmaking data.
...
CMatchmakingTelemetry::~CMatchmakingTelemetry()
{
	...
	{
		DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_COMMENT, "Destroying Matchmaking Telemetry object with non-transfered data present" );
	}
...
{
	DRX_ASSERT( m_currentTranscript == eMMTelTranscript_None );
...
CMatchmakingTelemetry::EMMTelRetVal CMatchmakingTelemetry::EndMatchmakingTranscript( EMMTelTranscriptType transcript, bool logLocally )
{
	...
	{
		DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_COMMENT, "MMTel: Tried to close a type %d transcript block but current type is %d", transcript, m_currentTranscript );
		retVal = eMMTelRV_ErrorWrongState;
	}
	else if( m_pBuffer == NULL || m_pBuffer->size() == 0 )
	{
		//there is actually no data to send
		DrxWarning( VALIDATOR_MODULE_GAME, VALIDATOR_COMMENT, "MMTel: When close a type %d transcript block there was no data to send", m_currentTranscript );
		retVal = eMMTelRV_ErrorWrongState;
	}
	else
	{
		...
		if( logLocally )
		{
			DrxFixedStringT<24> localFilename;
		...
		DrxFixedStringT<24> remoteFilename;
	...
	DrxLog( "MMT: End Transcript call finished" );
...
//called on the server for every player. called after OnOwnClientEnteredGame
	...
	{
		SDrxMatchMakingConnectionUID conUID = pGameLobby->GetConnectionUIDFromChannelID( channelId );
		DrxUserID guid = pGameLobby->GetUserIDFromChannelID( channelId );

		DrxFixedStringT<DRXLOBBY_USER_NAME_LENGTH> name;
...
//-------------------------------------------------------------------------
void CMatchmakingTelemetry::OnClientDisconnect( i32 channelId, EntityId playerId )
{
	DrxLog("MMT: OnClienDisconnect");
...
//-------------------------------------------------------------------------
ITelemetryProducer::EResult CMMTelemetryProducer::ProduceTelemetry( char *pOutBuffer, i32 inMinRequired, i32 inBufferSize, i32 *pOutWritten )
{
	ITelemetryProducer::EResult retVal = eTS_Available;
	u32 writtingIndex = 0;
	//change to unsigned for signed/unsigned mis-match protection
	u32 bufferSize = inBufferSize;
	...
	while( !writtenEnough && m_currentElement != end )
	{
		...
		{
			DRX_ASSERT_MESSAGE( (i32)writtingIndex >= inMinRequired, "MatchMaking Telemetry event is too large to send" );
			writtenEnough = true;
		}
	}
...
void CMMTelemetryProducer::OutputStartSearchData( SRecording_Packet& eventPacket, EventDataString& entryString )
{
	...
	if( CPlaylistUpr* pPlaylistmanager = g_pGame->GetPlaylistUpr() )
	{
