...
CGameServerLists::CGameServerLists()
{
	...
	IPlayerProfileUpr *pProfileMgr = g_pGame->GetIGameFramework()->GetIPlayerProfileUpr();
	if(pProfileMgr)
	{
...
//---------------------------------------
const bool CGameServerLists::Add(const EGameServerLists list, tukk name, u32k favouriteId, bool bFromProfile)
{
	if(name && name[0])
	{
		//DrxLog("[UI] CGameServerLists::Add %s %u", name, favouriteId);
...
//---------------------------------------
const bool CGameServerLists::Remove(const EGameServerLists list, u32k favouriteId)
{
	//DrxLog("[UI] CGameServerLists::Remove %u", favouriteId);
...
//---------------------------------------
i32k CGameServerLists::GetTotal(const EGameServerLists list) const
{
	return m_list[list].size();
}

//---------------------------------------
const bool CGameServerLists::InList(const EGameServerLists list, u32k favouriteId) const
{
...
void CGameServerLists::PopulateMenu(const EGameServerLists list) const
{
	...
	{
		u32 numIds = 0;
		u32 favouriteIdList[k_maxServersStoredInList];
...
//---------------------------------------
void CGameServerLists::SaveToProfile(IPlayerProfile* pProfile, bool online, u32 reason)
{
...
//---------------------------------------
void CGameServerLists::LoadFromProfile(IPlayerProfile* pProfile, bool online, u32 reason)
{
	if(reason & ePR_Options)
	{
		Reset();
...
//---------------------------------------
void CGameServerLists::ServerFound( const SServerInfo &serverInfo, const EGameServerLists list, u32k favouriteId )
{
	//DrxLog("[UI] CGameServerLists::ServerFound %s %d", serverInfo.m_hostName.c_str(), serverInfo.m_sessionFavouriteKeyId);
...
//---------------------------------------
void CGameServerLists::ServerNotFound( const EGameServerLists list, u32k favouriteId )
{
	//DrxLog("[UI] CGameServerLists::ServerNotFound %d", favouriteId);
...
void CGameServerLists::CmdAddFavourite(IConsoleCmdArgs* pCmdArgs)
{
	...
	{
		DrxLogAlways("Expected: gsl_addFavourite <server name>");
	}
}
...
void CGameServerLists::CmdRemoveFavourite(IConsoleCmdArgs* pCmdArgs)
{
	...
	{
		DrxLogAlways("Expected: gsl_removeFavourite <server name>");
	}
}
...
void CGameServerLists::CmdListFavourites(IConsoleCmdArgs* pCmdArgs)
{
	...
	{
		DrxLogAlways("%s", (*it).m_name.c_str());
	}
}
...
void CGameServerLists::CmdAddRecent(IConsoleCmdArgs* pCmdArgs)
{
	...
	{
		DrxLogAlways("Expected: gsl_addRecent <server name>");
	}
}
...
void CGameServerLists::CmdRemoveRecent(IConsoleCmdArgs* pCmdArgs)
{
	...
	{
		DrxLogAlways("Expected: gsl_removeRecent <server name>");
	}
}
...
/////////////////////////////////////////
CGameServerLists::SServerInfoInt::SServerInfoInt(tukk name, u32 favouriteId)
{
	m_name = name;
