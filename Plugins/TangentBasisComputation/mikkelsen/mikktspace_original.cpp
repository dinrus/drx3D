...
static void DegenEpilogue(STSpace psTspace[], STriInfo pTriInfos[], i32 piTriListIn[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn, i32k iTotTris);
...
static void DegenEpilogue(STSpace psTspace[], STriInfo pTriInfos[], i32 piTriListIn[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn, i32k iTotTris)
{
	...
	for(i32 t=iNrTrianglesIn; t<iTotTris; t++)
	{
		...
		if(!bSkip)
		{
			for(i32 i=0; i<3; i++)
			{
				i32k index1 = piTriListIn[t*3+i];
				// search through the good triangles
				tbool bNotFound = TTRUE;
				i32 j=0;
				while(bNotFound && j<(3*iNrTrianglesIn))
				{
					i32k index2 = piTriListIn[j];
