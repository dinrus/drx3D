...
typedef struct
{
	...
	SVec3 vOt;
	float fMagT;
	i32 iCounter;	// this is to average back into quads.
	tbool bOrient;
} STSpace;

static i32 GenerateInitialVerticesIndexList(STriInfo pTriInfos[], i32 piTriList_out[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn);
static void GenerateSharedVerticesIndexList(i32 piTriList_in_and_out[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn);
static void InitTriInfo(STriInfo pTriInfos[], i32k piTriListIn[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn);
static i32 Build4RuleGroups(STriInfo pTriInfos[], SGroup pGroups[], i32 piGroupTrianglesBuffer[], i32k piTriListIn[], i32k iNrTrianglesIn);
static tbool GenerateTSpaces(STSpace psTspace[], const STriInfo pTriInfos[], const SGroup pGroups[],
					 i32k iNrActiveGroups, i32k piTriListIn[], const float fThresCos,
					 const SMikkTSpaceContext * pContext);

static i32 MakeIndex(i32k iFace, i32k iVert)
{
	assert(iVert>=0 && iVert<4 && iFace>=0);
	return (iFace<<2) | (iVert&0x3);
}

static void IndexToData(i32 * piFace, i32 * piVert, i32k iIndexIn)
{
...
static SVec3 GetPosition(const SMikkTSpaceContext * pContext, i32k index);
static SVec3 GetNormal(const SMikkTSpaceContext * pContext, i32k index);
static SVec3 GetTexCoord(const SMikkTSpaceContext * pContext, i32k index);


// degen triangles
static void DegenPrologue(STriInfo pTriInfos[], i32 piTriList_out[], i32k iNrTrianglesIn, i32k iTotTris);
static tbool DegenEpilogue(STSpace psTspace[], STriInfo pTriInfos[], i32 piTriListIn[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn, i32k iTotTris);
...
{
	// count nr_triangles
	i32 * piTriListIn = NULL, * piGroupTrianglesBuffer = NULL;
	STriInfo * pTriInfos = NULL;
	SGroup * pGroups = NULL;
	STSpace * psTspace = NULL;
	i32 iNrTrianglesIn = 0, f=0, t=0, i=0;
	i32 iNrTSPaces = 0, iTotTris = 0, iDegenTriangles = 0, iNrMaxGroups = 0;
	i32 iNrActiveGroups = 0, index = 0;
	i32k iNrFaces = pContext->m_pInterface->m_getNumFaces(pContext);
	tbool bRes = TFALSE;
	...
	{
		i32k verts = pContext->m_pInterface->m_getNumVerticesOfFace(pContext, f);
	...
	piTriListIn = (i32 *) malloc(sizeof(i32)*3*iNrTrianglesIn);
	...
	{
		i32k i0 = piTriListIn[t*3+0];
		i32k i1 = piTriListIn[t*3+1];
		i32k i2 = piTriListIn[t*3+2];
	...
	piGroupTrianglesBuffer = (i32 *) malloc(sizeof(i32)*iNrTrianglesIn*3);
...
NOINLINE i32 FindGridCell(const float fMin, const float fMax, const float fVal)
{
	const float fIndex = g_iCells * ((fVal-fMin)/(fMax-fMin));
	i32k iIndex = fIndex<0 ? 0 : ((i32)fIndex);
	return iIndex<g_iCells ? iIndex : (g_iCells-1);
}

static void MergeVertsFast(i32 piTriList_in_and_out[], STmpVert pTmpVert[], const SMikkTSpaceContext * pContext, i32k iL_in, i32k iR_in);
static void MergeVertsSlow(i32 piTriList_in_and_out[], const SMikkTSpaceContext * pContext, i32k pTable[], i32k iEntries);
static void GenerateSharedVerticesIndexListSlow(i32 piTriList_in_and_out[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn);

static void GenerateSharedVerticesIndexList(i32 piTriList_in_and_out[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn)
{

	// Generate bounding box
	i32 * piHashTable=NULL, * piHashCount=NULL, * piHashOffsets=NULL, * piHashCount2=NULL;
	STmpVert * pTmpVert = NULL;
	i32 iChannel=0, k=0, e=0;
	i32 iMaxCount=0;
	SVec3 vMin = GetPosition(pContext, 0), vMax = vMin, vDim;
	float fMin, fMax;
	for(i32 i=1; i<(iNrTrianglesIn*3); i++)
	{
		i32k index = piTriList_in_and_out[i];
	...
	// make allocations
	piHashTable = (i32 *) malloc(sizeof(i32)*iNrTrianglesIn*3);
	piHashCount = (i32 *) malloc(sizeof(i32)*g_iCells);
	piHashOffsets = (i32 *) malloc(sizeof(i32)*g_iCells);
	piHashCount2 = (i32 *) malloc(sizeof(i32)*g_iCells);
	...
	memset(piHashCount, 0, sizeof(i32)*g_iCells);
	memset(piHashCount2, 0, sizeof(i32)*g_iCells);

	// count amount of elements in each cell unit
	for(i32 i=0; i<(iNrTrianglesIn*3); i++)
	{
		i32k index = piTriList_in_and_out[i];
		const SVec3 vP = GetPosition(pContext, index);
		const float fVal = iChannel==0 ? vP.x : (iChannel==1 ? vP.y : vP.z);
		i32k iCell = FindGridCell(fMin, fMax, fVal);
		++piHashCount[iCell];
	}
	...
	for(k=0; k<g_iCells; k++)
	{
		...
		i32 * pTable = &piHashTable[piHashOffsets[k]];
		i32k iEntries = piHashCount[k];
...
static void MergeVertsFast(i32 piTriList_in_and_out[], STmpVert pTmpVert[], const SMikkTSpaceContext * pContext, i32k iL_in, i32k iR_in)
{
	...
	if(fSep>=fvMax[channel] || fSep<=fvMin[channel])
	{
		...
		for(l=iL_in; l<=iR_in; l++)
		{
			...
			i32 l2=iL_in, i2rec=-1;
			while(l2<l && bNotFound)
			{
				i32k i2 = pTmpVert[l2].index;
				i32k index2 = piTriList_in_and_out[i2];
...
static void GenerateSharedVerticesIndexListSlow(i32 piTriList_in_and_out[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn)
{
	i32 iNumUniqueVerts = 0, t=0, i=0;
	for(t=0; t<iNrTrianglesIn; t++)
	{
		for(i=0; i<3; i++)
		{
			i32k offs = t*3 + i;
			i32k index = piTriList_in_and_out[offs];
			...
			tbool bFound = TFALSE;
			i32 t2=0, index2rec=-1;
			while(!bFound && t2<=t)
			{
				i32 j=0;
				while(!bFound && j<3)
				{
					i32k index2 = piTriList_in_and_out[t2*3 + j];
					const SVec3 vP2 = GetPosition(pContext, index2);
					const SVec3 vN2 = GetNormal(pContext, index2);
					const SVec3 vT2 = GetTexCoord(pContext, index2);
...
static i32 GenerateInitialVerticesIndexList(STriInfo pTriInfos[], i32 piTriList_out[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn)
{
	...
	for(f=0; f<pContext->m_pInterface->m_getNumFaces(pContext); f++)
	{
		...
		if(verts==3)
		{
			u8 * pVerts = pTriInfos[iDstTriIndex].vert_num;
		...
		else
		{
...

			{
				...
				i32k i0 = MakeIndex(f, 0);
				i32k i1 = MakeIndex(f, 1);
				i32k i2 = MakeIndex(f, 2);
				i32k i3 = MakeIndex(f, 3);
				...
				{
					{
						u8 * pVerts_A = pTriInfos[iDstTriIndex].vert_num;
					...
					++iDstTriIndex;	// next
					{
						u8 * pVerts_B = pTriInfos[iDstTriIndex].vert_num;
...
static void BuildNeighborsFast(STriInfo pTriInfos[], SEdge * pEdges, i32k piTriListIn[], i32k iNrTrianglesIn);
static void BuildNeighborsSlow(STriInfo pTriInfos[], i32k piTriListIn[], i32k iNrTrianglesIn);

// returns the texture area times 2
static float CalcTexArea(const SMikkTSpaceContext * pContext, i32k indices[])
{
...
static void InitTriInfo(STriInfo pTriInfos[], i32k piTriListIn[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn)
{
	...
	{
		i32k iFO_a = pTriInfos[t].iOrgFaceNumber;
		i32k iFO_b = pTriInfos[t+1].iOrgFaceNumber;
		if(iFO_a==iFO_b)	// this is a quad
		{
			...
			if((bIsDeg_a||bIsDeg_b)==TFALSE)
			{
				...
				if(bOrientA!=bOrientB)
				{
					...
					// force match
					{
						i32k t0 = bChooseOrientFirstTri ? t : (t+1);
						i32k t1 = bChooseOrientFirstTri ? (t+1) : t;
...
static tbool AssignRecur(i32k piTriListIn[], STriInfo psTriInfos[], i32k iMyTriIndex, SGroup * pGroup);
static void AddTriToGroup(SGroup * pGroup, i32k iTriIndex);

static i32 Build4RuleGroups(STriInfo pTriInfos[], SGroup pGroups[], i32 piGroupTrianglesBuffer[], i32k piTriListIn[], i32k iNrTrianglesIn)
{
	i32k iNrMaxGroups = iNrTrianglesIn*3;
	i32 iNrActiveGroups = 0;
	i32 iOffset = 0, f=0, i=0;
	for(f=0; f<iNrTrianglesIn; f++)
	{
		for(i=0; i<3; i++)
		{
			...
			{
				tbool bOrPre;
				i32 neigh_indexL, neigh_indexR;
				i32k vert_index = piTriListIn[f*3+i];
...
static tbool AssignRecur(i32k piTriListIn[], STriInfo psTriInfos[],
				 i32k iMyTriIndex, SGroup * pGroup)
{
	STriInfo * pMyTriInfo = &psTriInfos[iMyTriIndex];

	// track down vertex
	i32k iVertRep = pGroup->iVertexRepresentitive;
	i32k * pVerts = &piTriListIn[3*iMyTriIndex+0];
	i32 i=-1;
	...
	{
		i32k neigh_indexL = pMyTriInfo->FaceNeighbors[i];
		i32k neigh_indexR = pMyTriInfo->FaceNeighbors[i>0?(i-1):2];
		if(neigh_indexL>=0)
...
static void QuickSort(i32* pSortBuffer, i32 iLeft, i32 iRight, u32 uSeed);
static STSpace EvalTspace(i32 face_indices[], i32k iFaces, i32k piTriListIn[], const STriInfo pTriInfos[], const SMikkTSpaceContext * pContext, i32k iVertexRepresentitive);
					 ...
					 const SMikkTSpaceContext * pContext)
{
	...
	i32 * pTmpMembers = NULL;
	i32 iMaxNrFaces=0, iUniqueTspaces=0, g=0, i=0;
	...
	pTmpMembers = (i32 *) malloc(sizeof(i32)*iMaxNrFaces);
	...
	for(g=0; g<iNrActiveGroups; g++)
	{
		...
		i32 iUniqueSubGroups = 0;

		for(i=0; i<pGroup->iNrFaces; i++)	// triangles
		{
			i32k f = pGroup->pFaceIndices[i];	// triangle number
			i32 index=-1, iVertIndex=-1, iOF_1=-1, iMembers=0, j=0, l=0;
			SSubGroup tmp_group;
			tbool bFound;
			SVec3 n, vOs, vOt;
			...
			{
				i32k t = pGroup->pFaceIndices[j];	// triangle number
				i32k iOF_2 = pTriInfos[t].iOrgFaceNumber;
			...
			if(iMembers>1)
			{
				u32 uSeed = INTERNAL_RND_SORT_SEED;	// could replace with a random seed?
			...
			{
				// insert new subgroup
				i32 * pIndices = (i32 *) malloc(sizeof(i32)*iMembers);
				if(pIndices==NULL)
				{
					// clean up and return false
					for(i32 s=0; s<iUniqueSubGroups; s++)
				...
				memcpy(pIndices, tmp_group.pTriMembers, iMembers*sizeof(i32));
			...
			// output tspace
			{
				PREFAST_ASSUME(f >= 0);
				assert(f >= 0);
				i32k iOffs = pTriInfos[f].iTSpacesOffs;
				assert(index >= 0);
				PREFAST_ASSUME(index >= 0);
				i32k iVert = pTriInfos[f].vert_num[index];
...
static STSpace EvalTspace(i32 face_indices[], i32k iFaces, i32k piTriListIn[], const STriInfo pTriInfos[],
				   const SMikkTSpaceContext * pContext, i32k iVertexRepresentitive)
{
	STSpace res;
	float fAngleSum = 0;
	i32 face=0;
	...
	for(face=0; face<iFaces; face++)
	{
		...
		if( (pTriInfos[f].iFlag&GROUP_WITH_ANY)==0 )
		{
			...
			i32 i=-1, index=-1, i0=-1, i1=-1, i2=-1;
...
static void QuickSortEdges(SEdge * pSortBuffer, i32 iLeft, i32 iRight, i32k channel, u32 uSeed);
static void GetEdge(i32 * i0_out, i32 * i1_out, i32 * edgenum_out, i32k indices[], i32k i0_in, i32k i1_in);

static void BuildNeighborsFast(STriInfo pTriInfos[], SEdge * pEdges, i32k piTriListIn[], i32k iNrTrianglesIn)
{
	// build array of edges
	u32 uSeed = INTERNAL_RND_SORT_SEED;				// could replace with a random seed?
	i32 iEntries=0, iCurStartIndex=-1, i=0;
	for(i32 f=0; f<iNrTrianglesIn; f++)
		for(i=0; i<3; i++)
		{
			i32k i0 = piTriListIn[f*3+i];
			i32k i1 = piTriListIn[f*3+(i<2?(i+1):0)];
	...
	// could replace this with a 64 bit i32 sort over (i0,i1)
	...
	for(i=0; i<iEntries; i++)
	{
		...
		i32 i0_A, i1_A;
		i32 edgenum_A, edgenum_B=0;	// 0,1 or 2
...
static void BuildNeighborsSlow(STriInfo pTriInfos[], i32k piTriListIn[], i32k iNrTrianglesIn)
{
	...
	for(f=0; f<iNrTrianglesIn; f++)
	{
		for(i=0; i<3; i++)
		{
			...
			{
				i32k i0_A = piTriListIn[f*3+i];
				i32k i1_A = piTriListIn[f*3+(i<2?(i+1):0)];
				...
				while(!bFound && t<iNrTrianglesIn)
				{
					if(t!=f)
					{
						...
						{
							// in rev order
							i32k i1_B = piTriListIn[t*3+j];
							i32k i0_B = piTriListIn[t*3+(j<2?(j+1):0)];
							//assert(!(i0_A==i1_B && i1_A==i0_B));
...
{
	u32 t;
	i32 iL, iR, n, index, iMid;

	// early out
	SEdge sTmp;
	i32k iElems = iRight-iLeft+1;
	if(iElems<2) return;
	else if(iElems==2)
	{
...
static void DegenPrologue(STriInfo pTriInfos[], i32 piTriList_out[], i32k iNrTrianglesIn, i32k iTotTris)
{
	i32 iNextGoodTriangleSearchIndex=-1;
	...
	while(t<iNrTrianglesIn && bStillFindingGoodOnes)
	{
		...
		else
		{
			...
			if(!bJustADegenerate)
			{
				...
				for(i=0; i<3; i++)
				{
					i32k index = piTriList_out[t0*3+i];
...
struct IndexLess
{
	i32k* const m_pIndices;

	IndexLess(i32k* const pIndices)
		: m_pIndices(pIndices) 
	{
	}
...
static tbool DegenEpilogue(STSpace psTspace[], STriInfo pTriInfos[], i32 piTriListIn[], const SMikkTSpaceContext * pContext, i32k iNrTrianglesIn, i32k iTotTris)
{
	...
	if (iTotTris > iNrTrianglesIn)
	{
		...
		i32k iNrIndicesIn = iNrTrianglesIn*3;

		i32* const piSort = (i32 *)malloc(sizeof(i32)*iNrIndicesIn);
		if(piSort==NULL)
			return TFALSE;

		for (i32 i=0; i<iNrIndicesIn; i++)
			piSort[i] = i;

		std::stable_sort(&piSort[0], &piSort[iNrIndicesIn], less);

		for(i32 t=iNrTrianglesIn; t<iTotTris; t++)
		{
			...
			if(!bSkip)
			{
				for(i32 i=0; i<3; i++)
				{
					...
					if (imin<iNrIndicesIn && piTriListIn[piSort[imin]]==index)
					{
						...
						i32k iTri = j/3;
						i32k iVert = j%3;
						i32k iSrcVert=pTriInfos[iTri].vert_num[iVert];
						i32k iSrcOffs=pTriInfos[iTri].iTSpacesOffs;
						i32k iDstVert=pTriInfos[t].vert_num[i];
						i32k iDstOffs=pTriInfos[t].iTSpacesOffs;
	...
	for(i32 t=0; t<iNrTrianglesIn; t++)
	{
		...
		if( (pTriInfos[t].iFlag&QUAD_ONE_DEGEN_TRI)!=0 )
		{
			...
			i32 iOrgF=-1, i=0;
			tbool bNotFound;
			u8 * pV = pTriInfos[t].vert_num;
			i32 iFlag = (1<<pV[0]) | (1<<pV[1]) | (1<<pV[2]);
			i32 iMissingIndex = 0;
			...
			while(bNotFound && i<3)
			{
				...
				{
					i32k iOffs = pTriInfos[t].iTSpacesOffs;
