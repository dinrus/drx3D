 ...
 *
 * Разработка (c) 2001-16 David Ireland, D.I. Management Services Pty Limited
...
*/
i32 mpModulo(DIGIT_T r[], const DIGIT_T u[], size_t udigits, DIGIT_T v[], size_t vdigits);
...
i32 mpSqrt(DIGIT_T s[], const DIGIT_T x[], size_t ndigits);

/** Computes integer cube root s = floor(cuberoot(x)) */
i32 mpCubeRoot(DIGIT_T s[], const DIGIT_T x[], size_t ndigits);
...
i32 mpModExp(DIGIT_T y[], const DIGIT_T x[], const DIGIT_T e[], DIGIT_T m[], size_t ndigits);
...
i32 mpModMult(DIGIT_T a[], const DIGIT_T x[], const DIGIT_T y[], DIGIT_T m[], size_t ndigits);
...
i32 mpModInv(DIGIT_T inv[], const DIGIT_T u[], const DIGIT_T m[], size_t ndigits);

/** Computes g = gcd(x, y), the greatest common divisor of x and y */
i32 mpGcd(DIGIT_T g[], const DIGIT_T x[], const DIGIT_T y[], size_t ndigits);
...
i32 mpSetBit(DIGIT_T a[], size_t ndigits, size_t n, i32 value);

/** Returns value 1 or 0 of bit n (0..nbits-1) */
i32 mpGetBit(const DIGIT_T a[], size_t ndigits, size_t n);
...
void mpPrintHex(tukk prefix, const DIGIT_T *a, size_t ndigits, tukk suffix);
/** Print in decimal format with optional prefix and suffix strings */
void mpPrintDecimal(tukk prefix, const DIGIT_T *a, size_t ndigits, tukk suffix);
 ...
 *  @remark Expects a negative number to be stored in two's-complement representation.
 */
void mpPrintDecimalSigned(tukk prefix, DIGIT_T *a, size_t ndigits, tukk suffix);
