...
public:
	...
	void Init(PhysicsCommandProcessorInterface* comProc, const STxt& hostNamePort)
	{
		...
		std::cout << "grpcPlugin drx3D Physics GRPC server listening on " << hostNamePort << std::endl;
...
private:
	...
	void GatherRequests() {
		uk tag;  // uniquely identifies a request.
		bool ok;
...
DRX3D_SHARED_API i32 executePluginCommand_grpcPlugin(struct PluginContext* context, const struct b3PluginArguments* arguments)
{
	...
	else
	{
		...
		{
			for (i32 i = 0; i < arguments->m_ints[0]; i++)
			{
				if (obj->m_grpcInitialized && !obj->m_grpcTerminated)
				{
					obj->m_grpcTerminated = obj->m_grpcServer.HandleSingleRpc();
