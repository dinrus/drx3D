...
static u32 s_floor_ilog2(i32 value)
{
   u32 r = 0;
   while ((value >>= 1) != 0) {
      r++;
...
mp_err mp_prime_is_prime(const mp_int *a, i32 t, mp_bool *result)
{
   mp_int  b;
   i32     ix, p_max = 0, size_a, len;
   mp_bool res;
   mp_err  err;
   u32 fips_rand, mask;
   ...
   */
       ...
       * array a.dp is a simple 'i32' and this library can not assume full
      ...
      */
      for (ix = 0; ix < t; ix++) {
          ...
          * an u32 and "mask" on the other side is most probably not.
          */
         fips_rand = (u32)(b.dp[0] & (mp_digit) mask);
#ifdef MP_8BIT
         /*
          * One 8-bit digit is too small, so concatenate two if the size of
          * u32 allows for it.
          */
         if ((MP_SIZEOF_BITS(u32)/2) >= MP_SIZEOF_BITS(mp_digit)) {
            if ((err = mp_rand(&b, 1)) != MP_OKAY) {
               goto LBL_B;
            }
            fips_rand <<= MP_SIZEOF_BITS(mp_digit);
            fips_rand |= (u32) b.dp[0];
            fips_rand &= mask;
         }
#endif
         if (fips_rand > (u32)(INT_MAX - MP_DIGIT_BIT)) {
            len = INT_MAX / MP_DIGIT_BIT;
         } else {
            len = (((i32)fips_rand + MP_DIGIT_BIT) / MP_DIGIT_BIT);
         }
         /*  Unlikely. */
         if (len < 0) {
            ix--;
          ...
          * an "u32" is smaller than 16 bit a simple test
