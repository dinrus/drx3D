...
static mp_err s_read_arc4random(uk p, size_t n)
{
   arc4random_buf(p, n);
   return MP_OKAY;
}
...
static mp_err s_read_wincsp(uk p, size_t n)
{
...
static mp_err s_read_getrandom(uk p, size_t n)
{
   char *q = (char *)p;
   while (n > 0u) {
...
static mp_err s_read_urandom(uk p, size_t n)
{
   i32 fd;
   char *q = (char *)p;
...
u64 (*ltm_rng)(u8 *out, u64 outlen, void (*callback)(void));
void (*ltm_rng_callback)(void);

static mp_err s_read_ltm_rng(uk p, size_t n)
{
   u64 res;
...
mp_err s_read_arc4random(uk p, size_t n);
mp_err s_read_wincsp(uk p, size_t n);
mp_err s_read_getrandom(uk p, size_t n);
mp_err s_read_urandom(uk p, size_t n);
mp_err s_read_ltm_rng(uk p, size_t n);

mp_err s_mp_rand_platform(uk p, size_t n)
{
   mp_err err = MP_ERR;
