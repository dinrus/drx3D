               ...
               mp_endian endian, size_t nails, const mp_int *op)
{
   ...
   for (i = 0u; i < count; ++i) {
      for (j = 0u; j < size; ++j) {
         ...
         *byte = (u8)((j == ((size - nail_bytes) - 1u)) ? (t.dp[0] & odd_nail_mask) : (t.dp[0] & 0xFFuL));

         if ((err = mp_div_2d(&t, (j == ((size - nail_bytes) - 1u)) ? (i32)(8u - odd_nails) : 8, &t, NULL)) != MP_OKAY) {
            goto LBL_ERR;
         }
