...
mp_err s_mp_prime_random_ex(mp_int *a, i32 t, i32 size, i32 flags, private_mp_prime_callback cb, uk dat)
{
   u8 *tmp, maskAND, maskOR_msb, maskOR_lsb;
   i32 bsize, maskOR_msb_offset;
   mp_bool res;
   mp_err err;
   ...
   tmp = (u8*) MP_MALLOC((size_t)bsize);
   if (tmp == NULL) {
      return MP_MEM;
   }

   /* calc the maskAND value for the MSbyte*/
   maskAND = ((size&7) == 0) ? 0xFFu : (u8)(0xFFu >> (8 - (size & 7)));

   /* calc the maskOR_msb */
   maskOR_msb        = 0;
   maskOR_msb_offset = ((size & 7) == 1) ? 1 : 0;
   if ((flags & MP_PRIME_2MSB_ON) != 0) {
      maskOR_msb       |= (u8)(0x80 >> ((9 - size) & 7));
   }
...
static i32 s_mp_rand_cb(u8 *dst, i32 len, uk dat)
{
   (void)dat;
   if (len <= 0) {
      return len;
...
mp_err mp_prime_rand(mp_int *a, i32 t, i32 size, i32 flags)
{
