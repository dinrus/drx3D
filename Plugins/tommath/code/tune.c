...
static uint64_t s_time_mul(i32 size);
static uint64_t s_time_sqr(i32 size);
...
static i32 s_check_result;
static i32 s_number_of_test_loops;
static i32 s_stabilization_extra;
static i32 s_offset = 1;
...
struct tune_args {
   ...
   i32 bncore;
   i32 terse;
   i32 upper_limit_print;
   i32 increment_print;
} args;

static void s_run(tukk name, uint64_t (*op)(i32), i32 *cutoff)
{
   i32 x, count = 0;
   uint64_t t1, t2;
...
static long s_strtol(tukk str, char **endptr, tukk err)
{
   i32k base = 10;
   char *_endptr;
   long val;
...
static i32 s_exit_code = EXIT_FAILURE;
static void s_usage(char *s)
{
...
struct cutoffs {
   i32 KARATSUBA_MUL, KARATSUBA_SQR;
   i32 TOOM_MUL, TOOM_SQR;
};
   ...
   if (argc != 1) {
      for (opt = 1; (opt < argc) && (argv[opt][0] == '-'); opt++) {
         ...
         case 'L':
            ...
            s_stabilization_extra = (i32)s_strtol(argv[opt], NULL, "No value for option \"-L\"given");
            break;
         case 'o':
            opt++;
            if (opt >= argc) {
            ...
            s_offset = (i32)s_strtol(argv[opt], NULL, "No value for the offset given");
            break;
         case 'r':
            opt++;
            if (opt >= argc) {
            ...
            s_number_of_test_loops = (i32)s_strtol(argv[opt], NULL, "No value for the number of rounds given");
            break;
         ...
         case 'M':
            ...
            args.upper_limit_print = (i32)s_strtol(argv[opt], NULL, "No value for the upper limit of T-C tests given");
            break;
         case 'm':
            opt++;
            if (opt >= argc) {
            ...
            args.increment_print = (i32)s_strtol(argv[opt], NULL, "No value for the increment for the T-C tests given");
            break;
         case 's':
            printpreset = 1;
            args.print = 1;
            ...
            str = argv[opt];
            KARATSUBA_MUL_CUTOFF = (i32)s_strtol(str, &endptr, "[1/4] No value for KARATSUBA_MUL_CUTOFF given");
            str = endptr + 1;
            KARATSUBA_SQR_CUTOFF = (i32)s_strtol(str, &endptr, "[2/4] No value for KARATSUBA_SQR_CUTOFF given");
            str = endptr + 1;
            TOOM_MUL_CUTOFF = (i32)s_strtol(str, &endptr, "[3/4] No value for TOOM_MUL_CUTOFF given");
            str = endptr + 1;
            TOOM_SQR_CUTOFF = (i32)s_strtol(str, &endptr, "[4/4] No value for TOOM_SQR_CUTOFF given");
            break;
         case 'h':
