...
#define PRIVATE_MP_WARRAY (i32)(1uLL << (((CHAR_BIT * sizeof(private_mp_word)) - (2 * MP_DIGIT_BIT)) + 1))
...
typedef i32 private_mp_prime_callback(u8 *dst, i32 len, uk dat);
typedef private_mp_prime_callback MP_DEPRECATED(mp_rand_source) ltm_prime_callback;

/* error code to tuk string */
tukk mp_error_to_string(mp_err code) MP_WUR;
...
/* grow an i32 to a given size */
mp_err mp_grow(mp_int *a, i32 size) MP_WUR;

/* init to a given number of digits */
mp_err mp_init_size(mp_int *a, i32 size) MP_WUR;
...
u64 mp_get_mag_ul(const mp_int *a) MP_WUR;
zu64 mp_get_mag_ull(const mp_int *a) MP_WUR;
...
/* get integer, set integer (u64) */
#define mp_get_ul(a) ((u64)mp_get_l(a))
void mp_set_ul(mp_int *a, u64 b);
mp_err mp_init_ul(mp_int *a, u64 b) MP_WUR;
...
/* get integer, set integer (zu64) */
#define mp_get_ull(a) ((zu64)mp_get_ll(a))
void mp_set_ull(mp_int *a, zu64 b);
mp_err mp_init_ull(mp_int *a, zu64 b) MP_WUR;
...
MP_DEPRECATED(mp_get_mag_u32/mp_get_u32) u64 mp_get_int(const mp_int *a) MP_WUR;
MP_DEPRECATED(mp_get_mag_ul/mp_get_ul) u64 mp_get_long(const mp_int *a) MP_WUR;
MP_DEPRECATED(mp_get_mag_ull/mp_get_ull) zu64 mp_get_long_long(const mp_int *a) MP_WUR;
MP_DEPRECATED(mp_set_ul) mp_err mp_set_int(mp_int *a, u64 b);
MP_DEPRECATED(mp_set_ul) mp_err mp_set_long(mp_int *a, u64 b);
MP_DEPRECATED(mp_set_ull) mp_err mp_set_long_long(mp_int *a, zu64 b);
MP_DEPRECATED(mp_init_ul) mp_err mp_init_set_int(mp_int *a, u64 b) MP_WUR;
...
/* export binary data */
MP_DEPRECATED(mp_pack) mp_err mp_export(uk rop, size_t *countp, i32 order, size_t size,
                                        i32 endian, size_t nails, const mp_int *op) MP_WUR;

/* import binary data */
MP_DEPRECATED(mp_unpack) mp_err mp_import(mp_int *rop, size_t count, i32 order,
      size_t size, i32 endian, size_t nails,
      ukk op) MP_WUR;

/* unpack binary data */
mp_err mp_unpack(mp_int *rop, size_t count, mp_order order, size_t size, mp_endian endian,
                 size_t nails, ukk op) MP_WUR;

/* pack binary data */
size_t mp_pack_count(const mp_int *a, size_t nails, size_t size) MP_WUR;
mp_err mp_pack(uk rop, size_t maxcount, size_t *written, mp_order order, size_t size,
               mp_endian endian, size_t nails, const mp_int *op) MP_WUR;

/* ---> digit manipulation <--- */

/* right shift by "b" digits */
void mp_rshd(mp_int *a, i32 b);

/* left shift by "b" digits */
mp_err mp_lshd(mp_int *a, i32 b) MP_WUR;

/* c = a / 2**b, implemented as c = a >> b */
mp_err mp_div_2d(const mp_int *a, i32 b, mp_int *c, mp_int *d) MP_WUR;
...
/* c = a * 2**b, implemented as c = a << b */
mp_err mp_mul_2d(const mp_int *a, i32 b, mp_int *c) MP_WUR;

/* b = a*2 */
mp_err mp_mul_2(const mp_int *a, mp_int *b) MP_WUR;

/* c = a mod 2**b */
mp_err mp_mod_2d(const mp_int *a, i32 b, mp_int *c) MP_WUR;

/* computes a = 2**b */
mp_err mp_2expt(mp_int *a, i32 b) MP_WUR;

/* Counts the number of lsbs which are zero before the first zero bit */
i32 mp_cnt_lsb(const mp_int *a) MP_WUR;

/* I Love Earth! */

/* makes a pseudo-random mp_int of a given size */
mp_err mp_rand(mp_int *a, i32 digits) MP_WUR;
/* makes a pseudo-random small i32 of a given size */
MP_DEPRECATED(mp_rand) mp_err mp_rand_digit(mp_digit *r) MP_WUR;
/* use custom random data source instead of source provided the platform */
void mp_rand_source(mp_err(*source)(uk out, size_t size));
...
extern u64 (*ltm_rng)(u8 *out, u64 outlen, void (*callback)(void));
 ...
 * in case of error
 */
MP_DEPRECATED(s_mp_get_bit) i32 mp_get_bit(const mp_int *a, i32 b) MP_WUR;
...
MP_DEPRECATED(mp_signed_rsh) mp_err mp_tc_div_2d(const mp_int *a, i32 b, mp_int *c) MP_WUR;
mp_err mp_signed_rsh(const mp_int *a, i32 b, mp_int *c) MP_WUR;
...
MP_DEPRECATED(mp_root_u32) mp_err mp_n_root_ex(const mp_int *a, mp_digit b, mp_int *c, i32 fast) MP_WUR;
...
MP_DEPRECATED(mp_kronecker) mp_err mp_jacobi(const mp_int *a, const mp_int *n, i32 *c) MP_WUR;

/* computes the Kronecker symbol c = (a | p) (like jacobi() but with {a,p} in Z */
mp_err mp_kronecker(const mp_int *a, const mp_int *p, i32 *c) MP_WUR;
 ...
 */
i32 mp_prime_rabin_miller_trials(i32 size) MP_WUR;
 ...
 */
mp_err mp_prime_is_prime(const mp_int *a, i32 t, mp_bool *result) MP_WUR;
 ...
 */
mp_err mp_prime_next_prime(mp_int *a, i32 t, i32 bbs_style) MP_WUR;
 ...
 *
 */
MP_DEPRECATED(mp_prime_rand) mp_err mp_prime_random_ex(mp_int *a, i32 t, i32 size, i32 flags,
      private_mp_prime_callback cb, uk dat) MP_WUR;
mp_err mp_prime_rand(mp_int *a, i32 t, i32 size, i32 flags) MP_WUR;
...
MP_DEPRECATED(mp_expt_u32) mp_err mp_expt_d_ex(const mp_int *a, mp_digit b, mp_int *c, i32 fast) MP_WUR;

/* ---> radix conversion <--- */
i32 mp_count_bits(const mp_int *a) MP_WUR;


MP_DEPRECATED(mp_ubin_size) i32 mp_unsigned_bin_size(const mp_int *a) MP_WUR;
MP_DEPRECATED(mp_from_ubin) mp_err mp_read_unsigned_bin(mp_int *a, u8k *b, i32 c) MP_WUR;
MP_DEPRECATED(mp_to_ubin) mp_err mp_to_unsigned_bin(const mp_int *a, u8 *b) MP_WUR;
MP_DEPRECATED(mp_to_ubin) mp_err mp_to_unsigned_bin_n(const mp_int *a, u8 *b, u64 *outlen) MP_WUR;

MP_DEPRECATED(mp_sbin_size) i32 mp_signed_bin_size(const mp_int *a) MP_WUR;
MP_DEPRECATED(mp_from_sbin) mp_err mp_read_signed_bin(mp_int *a, u8k *b, i32 c) MP_WUR;
MP_DEPRECATED(mp_to_sbin) mp_err mp_to_signed_bin(const mp_int *a,  u8 *b) MP_WUR;
MP_DEPRECATED(mp_to_sbin) mp_err mp_to_signed_bin_n(const mp_int *a, u8 *b, u64 *outlen) MP_WUR;

size_t mp_ubin_size(const mp_int *a) MP_WUR;
mp_err mp_from_ubin(mp_int *a, u8k *buf, size_t size) MP_WUR;
mp_err mp_to_ubin(const mp_int *a, u8 *buf, size_t maxlen, size_t *written) MP_WUR;

size_t mp_sbin_size(const mp_int *a) MP_WUR;
mp_err mp_from_sbin(mp_int *a, u8k *buf, size_t size) MP_WUR;
mp_err mp_to_sbin(const mp_int *a, u8 *buf, size_t maxlen, size_t *written) MP_WUR;

mp_err mp_read_radix(mp_int *a, tukk str, i32 radix) MP_WUR;
MP_DEPRECATED(mp_to_radix) mp_err mp_toradix(const mp_int *a, char *str, i32 radix) MP_WUR;
MP_DEPRECATED(mp_to_radix) mp_err mp_toradix_n(const mp_int *a, char *str, i32 radix, i32 maxlen) MP_WUR;
mp_err mp_to_radix(const mp_int *a, char *str, size_t maxlen, size_t *written, i32 radix) MP_WUR;
mp_err mp_radix_size(const mp_int *a, i32 radix, i32 *size) MP_WUR;

#ifndef MP_NO_FILE
mp_err mp_fread(mp_int *a, i32 radix, FILE *stream) MP_WUR;
mp_err mp_fwrite(const mp_int *a, i32 radix, FILE *stream) MP_WUR;
#endif
