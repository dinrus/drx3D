...
#ifndef LTC_FAST
/* right shift */
static void _gcm_rightshift(u8 *a)
{
   i32 x;
   for (x = 15; x > 0; x--) {
...
/* c = b*a */
static u8k mask[] = { 0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01 };
static u8k poly[] = { 0x00, 0xE1 };
...
void gcm_gf_mult(u8k *a, u8k *b, u8 *c)
{
   ...
   for (i = (BPD/4)-1; i >= 0; i--) {
     ...
     /* shift result up by 4 bits */
       if (i != 0) {
          for (z = j = 0; j < (i32)(32 / sizeof(LTC_FAST_TYPE)); j++) {
              zz = tmp[j] << (BPD-4);
              tmp[j] = (tmp[j] >> 4) | z;
              z = zz;
          }
