...

{
  ...
  u64 taglen, x, y;
  u8 buf[64], buf2[64], tag[16], tag2[16], tag3[16], zero[64];
  i32           err, idx;
  symmetric_key skey;
  ccm_state ccm;
  ...
  for (x = 0; x < (sizeof(tests)/sizeof(tests[0])); x++) {
    for (y = 0; y < 2; y++) {
      ...
      if (y == 0) {
         ...
         if ((err = ccm_process(&ccm, (u8*)tests[x].pt, tests[x].ptlen, buf, CCM_ENCRYPT)) != CRYPT_OK) {
            return err;
         }
