...
i32 rc4_import(u8k *in, u64 inlen, prng_state *prng)
{
   ...
   LTC_ARGCHK(prng != NULL);
   LTC_ARGCHK(in   != NULL);
   if (inlen < (u64)rc4_desc.export_size) return CRYPT_INVALID_ARG;
...
i32 rc4_test(void)
{
   ...
   u8 out[1000];
   u8 t1[] = { 0xE0, 0x4D, 0x9A, 0xF6, 0xA8, 0x9D, 0x77, 0x53, 0xAE, 0x09 };
   u8 t2[] = { 0xEF, 0x80, 0xA2, 0xE6, 0x50, 0x91, 0xF3, 0x17, 0x4A, 0x8A };
   u8 t3[] = { 0x4B, 0xD6, 0x5C, 0x67, 0x99, 0x03, 0x56, 0x12, 0x80, 0x48 };
   i32 err;
