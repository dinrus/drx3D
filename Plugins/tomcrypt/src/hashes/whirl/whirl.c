...
static i32 _whirlpool_compress(hash_state *md, u8 *buf)
#else
static i32 whirlpool_compress(hash_state *md, u8 *buf)
#endif
{
   ulong64 K[2][8], T[3][8];
   i32 x, y;
...
static i32 whirlpool_compress(hash_state *md, u8 *buf)
{
   ...
   burn_stack((5 * 8 * sizeof(ulong64)) + (2 * sizeof(i32)));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
i32 whirlpool_done(hash_state * md, u8 *out)
{
    i32 i;
    ...
    /* append the '1' bit */
    md->whirlpool.buf[md->whirlpool.curlen++] = (u8)0x80;
    ...
    if (md->whirlpool.curlen > 32) {
        while (md->whirlpool.curlen < 64) {
            md->whirlpool.buf[md->whirlpool.curlen++] = (u8)0;
        }
   ...
   /* 512-bits, leading set of last byte */
{
  ...
  for (i = 0; i < (i32)(sizeof(tests)/sizeof(tests[0])); i++) {
      whirlpool_init(&md);
      whirlpool_process(&md, (u8*)tests[i].msg, tests[i].len);
