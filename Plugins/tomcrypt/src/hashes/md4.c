...
static i32 _md4_compress(hash_state *md, u8 *buf)
#else
static i32  md4_compress(hash_state *md, u8 *buf)
#endif
{
...
static i32 md4_compress(hash_state *md, u8 *buf)
{
   ...
   burn_stack(sizeof(ulong32) * 20 + sizeof(i32));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
i32 md4_done(hash_state * md, u8 *out)
{
    i32 i;
    ...
    /* append the '1' bit */
    md->md4.buf[md->md4.curlen++] = (u8)0x80;
...
i32 md4_test(void)
{
    ...
    for(i = 0; i < (i32)(sizeof(tests) / sizeof(tests[0])); i++) {
        md4_init(&md);
        md4_process(&md, (u8*)tests[i].input, (u64)strlen(tests[i].input));
        md4_done(&md, tmp);
