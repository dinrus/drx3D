...
static i32 _rmd160_compress(hash_state *md, u8 *buf)
#else
static i32  rmd160_compress(hash_state *md, u8 *buf)
#endif
{
...
static i32 rmd160_compress(hash_state *md, u8 *buf)
{
   ...
   burn_stack(sizeof(ulong32) * 26 + sizeof(i32));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
i32 rmd160_done(hash_state * md, u8 *out)
{
    i32 i;
    ...
    /* append the '1' bit */
    md->rmd160.buf[md->rmd160.curlen++] = (u8)0x80;
    ...
    if (md->rmd160.curlen > 56) {
        while (md->rmd160.curlen < 64) {
            md->rmd160.buf[md->rmd160.curlen++] = (u8)0;
        }
...
i32 rmd160_test(void)
{
   ...
   for (i = 0; i < (i32)(sizeof(tests)/sizeof(tests[0])); i++) {
       rmd160_init(&md);
       rmd160_process(&md, (u8*)tests[i].msg, strlen(tests[i].msg));
       rmd160_done(&md, tmp);
