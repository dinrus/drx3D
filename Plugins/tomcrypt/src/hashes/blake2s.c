...
struct blake2s_param {
   ...
   u8 node_depth;
   u8 inner_length;
   u8 salt[BLAKE2S_SALTBYTES];
   u8 personal[BLAKE2S_PERSONALBYTES];
};
*/
...
static u8k blake2s_sigma[10][16] = {
...
static i32 blake2s_is_lastblock(const hash_state *md) { return md->blake2s.f[0] != 0; }
...
static i32 blake2s_init0(hash_state *md)
{
   i32 i;
...
static i32 blake2s_init_param(hash_state *md, u8k *P)
{
   u64 i;
...
i32 blake2s_init(hash_state *md, u64 outlen, u8k *key, u64 keylen)
{
   ...
   P[O_DIGEST_LENGTH] = (u8)outlen;
   P[O_KEY_LENGTH] = (u8)keylen;
   P[O_FANOUT] = 1;
   P[O_DEPTH] = 1;
...
i32 blake2s_128_init(hash_state *md) { return blake2s_init(md, 16, NULL, 0); }

i32 blake2s_160_init(hash_state *md) { return blake2s_init(md, 20, NULL, 0); }

i32 blake2s_224_init(hash_state *md) { return blake2s_init(md, 28, NULL, 0); }

i32 blake2s_256_init(hash_state *md) { return blake2s_init(md, 32, NULL, 0); }
...
#ifdef LTC_CLEAN_STACK
static i32 _blake2s_compress(hash_state *md, u8k *buf)
#else
static i32 blake2s_compress(hash_state *md, u8k *buf)
#endif
{
   u64 i;
   ulong32 m[16];
...
static i32 blake2s_compress(hash_state *md, u8k *buf)
{
   ...
   burn_stack(sizeof(ulong32) * (32) + sizeof(u64));
   return err;
}
#endif
...
i32 blake2s_process(hash_state *md, u8k *in, u64 inlen)
{
   ...
   if (inlen > 0) {
      u64 left = md->blake2s.curlen;
      u64 fill = BLAKE2S_BLOCKBYTES - left;
      if (inlen > fill) {
         md->blake2s.curlen = 0;
...
i32 blake2s_done(hash_state *md, u8 *out)
{
...
i32 blake2s_256_test(void)
{
   ...
   for (i = 0; tests[i].msg != NULL; i++) {
      blake2s_256_init(&md);
      blake2s_process(&md, (u8*)tests[i].msg, (u64)strlen(tests[i].msg));
      blake2s_done(&md, tmp);
