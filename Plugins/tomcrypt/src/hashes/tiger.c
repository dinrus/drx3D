...
INLINE static void tiger_round(ulong64 *a, ulong64 *b, ulong64 *c, ulong64 x, i32 mul)
{
    ulong64 tmp;
    tmp = (*c ^= x);
...
/* one complete pass */
static void pass(ulong64 *a, ulong64 *b, ulong64 *c, ulong64 *x, i32 mul)
{
...
static i32 _tiger_compress(hash_state *md, u8 *buf)
#else
static i32  tiger_compress(hash_state *md, u8 *buf)
#endif
{
    ulong64 a, b, c, x[8];
    u64 i;
...
static i32 tiger_compress(hash_state *md, u8 *buf)
{
   ...
   burn_stack(sizeof(ulong64) * 11 + sizeof(u64));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
i32 tiger_done(hash_state * md, u8 *out)
{
    LTC_ARGCHK(md  != NULL);
    LTC_ARGCHK(out != NULL);
    ...
    /* append the '1' bit */
    md->tiger.buf[md->tiger.curlen++] = (u8)0x01;
    ...
    if (md->tiger.curlen > 56) {
        while (md->tiger.curlen < 64) {
            md->tiger.buf[md->tiger.curlen++] = (u8)0;
        }
...
i32  tiger_test(void)
{
  ...
  for (i = 0; i < (i32)(sizeof(tests) / sizeof(tests[0])); i++) {
      tiger_init(&md);
      tiger_process(&md, (u8*)tests[i].msg, (u64)strlen(tests[i].msg));
      tiger_done(&md, tmp);
