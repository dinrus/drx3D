...
static u8k blake2b_sigma[12][16] =
{
...
static i32 blake2b_is_lastblock(const hash_state *md) { return md->blake2b.f[0] != 0; }
...
static i32 blake2b_init_param(hash_state *md, u8k *P)
{
   u64 i;
...
i32 blake2b_160_init(hash_state *md) { return blake2b_init(md, 20, NULL, 0); }

i32 blake2b_256_init(hash_state *md) { return blake2b_init(md, 32, NULL, 0); }

i32 blake2b_384_init(hash_state *md) { return blake2b_init(md, 48, NULL, 0); }

i32 blake2b_512_init(hash_state *md) { return blake2b_init(md, 64, NULL, 0); }
...
#ifdef LTC_CLEAN_STACK
static i32 _blake2b_compress(hash_state *md, u8k *buf)
#else
static i32 blake2b_compress(hash_state *md, u8k *buf)
#endif
{
   ulong64 m[16];
   ulong64 v[16];
...
static i32 blake2b_compress(hash_state *md, u8k *buf)
{
   ...
   burn_stack(sizeof(ulong64) * 32 + sizeof(u64));
   return err;
}
#endif
...
i32 blake2b_process(hash_state *md, u8k *in, u64 inlen)
{
   ...
   if (inlen > 0) {
      u64 left = md->blake2b.curlen;
      u64 fill = BLAKE2B_BLOCKBYTES - left;
      if (inlen > fill) {
         md->blake2b.curlen = 0;
...
i32 blake2b_done(hash_state *md, u8 *out)
{
...
i32 blake2b_512_test(void)
{
   ...
   for (i = 0; tests[i].msg != NULL; i++) {
      blake2b_512_init(&md);
      blake2b_process(&md, (u8*)tests[i].msg, (u64)strlen(tests[i].msg));
      blake2b_done(&md, tmp);
