...
static i32 _noekeon_ecb_encrypt(u8k *pt, u8 *ct, symmetric_key *skey)
#else
...
i32 noekeon_ecb_encrypt(u8k *pt, u8 *ct, symmetric_key *skey)
{
   i32 err = _noekeon_ecb_encrypt(pt, ct, skey);
   burn_stack(sizeof(ulong32) * 5 + sizeof(i32));
   return err;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static i32 _noekeon_ecb_decrypt(u8k *ct, u8 *pt, symmetric_key *skey)
#else
...
i32 noekeon_ecb_decrypt(u8k *ct, u8 *pt, symmetric_key *skey)
{
   i32 err = _noekeon_ecb_decrypt(ct, pt, skey);
   burn_stack(sizeof(ulong32) * 5 + sizeof(i32));
   return err;
}
#endif
