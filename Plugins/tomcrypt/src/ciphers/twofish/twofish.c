...
static ulong32 _sbox(i32 i, ulong32 x)
#else
static ulong32 sbox(i32 i, ulong32 x)
#endif
{
   u8 a0,b0,a1,b1,a2,b2,a3,b3,a4,b4,y;
...
#ifndef LTC_TWOFISH_TABLES
static ulong32 mds_column_mult(u8 in, i32 col)
{
   ulong32 x01, x5B, xEF;
...
static void mds_mult(u8k *in, u8 *out)
{
  i32 x;
  ulong32 tmp;
  for (tmp = x = 0; x < 4; x++) {
...
static void rs_mult(u8k *in, u8 *out)
{
   ulong32 tmp;
...
/* computes h(x) */
static void h_func(u8k *in, u8 *out, u8 *M, i32 k, i32 offset)
{
  i32 x;
  u8 y[4];
  for (x = 0; x < 4; x++) {
  ...
  switch (k) {
     case 4:
            y[0] = (u8)(sbox(1, (ulong32)y[0]) ^ M[4 * (6 + offset) + 0]);
            y[1] = (u8)(sbox(0, (ulong32)y[1]) ^ M[4 * (6 + offset) + 1]);
            y[2] = (u8)(sbox(0, (ulong32)y[2]) ^ M[4 * (6 + offset) + 2]);
            y[3] = (u8)(sbox(1, (ulong32)y[3]) ^ M[4 * (6 + offset) + 3]);
            /* FALLTHROUGH */
     case 3:
            y[0] = (u8)(sbox(1, (ulong32)y[0]) ^ M[4 * (4 + offset) + 0]);
            y[1] = (u8)(sbox(1, (ulong32)y[1]) ^ M[4 * (4 + offset) + 1]);
            y[2] = (u8)(sbox(0, (ulong32)y[2]) ^ M[4 * (4 + offset) + 2]);
            y[3] = (u8)(sbox(0, (ulong32)y[3]) ^ M[4 * (4 + offset) + 3]);
            /* FALLTHROUGH */
     case 2:
            y[0] = (u8)(sbox(1, sbox(0, sbox(0, (ulong32)y[0]) ^ M[4 * (2 + offset) + 0]) ^ M[4 * (0 + offset) + 0]));
            y[1] = (u8)(sbox(0, sbox(0, sbox(1, (ulong32)y[1]) ^ M[4 * (2 + offset) + 1]) ^ M[4 * (0 + offset) + 1]));
            y[2] = (u8)(sbox(1, sbox(1, sbox(0, (ulong32)y[2]) ^ M[4 * (2 + offset) + 2]) ^ M[4 * (0 + offset) + 2]));
            y[3] = (u8)(sbox(0, sbox(1, sbox(1, (ulong32)y[3]) ^ M[4 * (2 + offset) + 3]) ^ M[4 * (0 + offset) + 3]));
            /* FALLTHROUGH */
  }
  mds_mult(y, out);
}
...
static ulong32 g_func(ulong32 x, symmetric_key *key)
{
    ulong32 y;
    y = _g_func(x, key);
    burn_stack(sizeof(u8) * 4 + sizeof(ulong32));
    return y;
}
#endif /* LTC_CLEAN_STACK */
 ...
 */
#ifdef LTC_CLEAN_STACK
static i32 _twofish_setup(u8k *key, i32 keylen, i32 num_rounds, symmetric_key *skey)
#else
i32 twofish_setup(u8k *key, i32 keylen, i32 num_rounds, symmetric_key *skey)
#endif
{
#ifndef LTC_TWOFISH_SMALL
   u8 S[4*4], tmpx0, tmpx1;
#endif
   i32 k, x, y;
   u8 tmp[4], tmp2[4], M[8*4];
   ulong32 A, B;
...
i32 twofish_setup(u8k *key, i32 keylen, i32 num_rounds, symmetric_key *skey)
{
   ...
   burn_stack(sizeof(i32) * 7 + sizeof(u8) * 56 + sizeof(ulong32) * 2);
   return x;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static i32 _twofish_ecb_encrypt(u8k *pt, u8 *ct, symmetric_key *skey)
#else
...
i32 twofish_ecb_encrypt(u8k *pt, u8 *ct, symmetric_key *skey)
{
   i32 err = _twofish_ecb_encrypt(pt, ct, skey);
   burn_stack(sizeof(ulong32) * 10 + sizeof(i32));
   return err;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static i32 _twofish_ecb_decrypt(u8k *ct, u8 *pt, symmetric_key *skey)
#else
...
i32 twofish_ecb_decrypt(u8k *ct, u8 *pt, symmetric_key *skey)
{
   i32 err =_twofish_ecb_decrypt(ct, pt, skey);
   burn_stack(sizeof(ulong32) * 10 + sizeof(i32));
   return err;
}
#endif
