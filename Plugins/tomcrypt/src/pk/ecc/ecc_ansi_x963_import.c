...
i32 ecc_ansi_x963_import_ex(u8k *in, u64 inlen, ecc_key *key, ltc_ecc_set_type *dp)
{
   ...
   /* read data */
   if ((err = mp_read_unsigned_bin(key->pubkey.x, (u8*)in+1, (inlen-1)>>1)) != CRYPT_OK) {
      goto error;
   }

   if ((err = mp_read_unsigned_bin(key->pubkey.y, (u8*)in+1+((inlen-1)>>1), (inlen-1)>>1)) != CRYPT_OK) {
      goto error;
   }
