...
static i32 _ecc_sign_hash(u8k *in,  u64 inlen,
                                u8 *out, u64 *outlen,
                                prng_state *prng, i32 wprng, ecc_key *key, i32 sigformat)
{
   ecc_key       pubkey;
   void          *r, *s, *e, *p, *b;
   i32           err, max_iterations = LTC_PK_MAX_RETRIES;
   ...
   pbytes = (pbits+7) >> 3;
   if (pbits > inlen*8) {
      if ((err = mp_read_unsigned_bin(e, (u8*)in, inlen)) != CRYPT_OK)    { goto errnokey; }
   }
   else if (pbits % 8 == 0) {
      if ((err = mp_read_unsigned_bin(e, (u8*)in, pbytes)) != CRYPT_OK)   { goto errnokey; }
   }
   else {
      ...
      if ((err = mp_read_unsigned_bin(e, (u8*)buf, pbytes)) != CRYPT_OK)  { goto errnokey; }
   }
