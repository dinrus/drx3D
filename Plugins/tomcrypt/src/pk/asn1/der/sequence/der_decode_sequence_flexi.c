...
static u64 _fetch_length(u8k *in, u64 inlen, u64 *data_offset)
{
   u64 x, z;
...
static i32 _new_element(ltc_asn1_list **l)
{
   /* alloc new link */
   if (*l == NULL) {
...
{
   ltc_asn1_list *l, *t;
   u64 err, type, len, totlen, data_offset, len_len;
   void          *realloc_tmp;
     ...
     /* now switch on type */
      switch (type) {
         ...
         case 0x06: /* OID */
            ...
            if ((l->data = XCALLOC(len, sizeof(u64))) == NULL) {
               err = CRYPT_MEM;
               goto error;
            }
            ...
            if ((realloc_tmp = XREALLOC(l->data, l->size * sizeof(u64))) == NULL) {
