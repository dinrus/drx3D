...
i32 blake2smac_test(void)
{
   ...
   u64 ilen, klen = sizeof(key), mlen = 32;
   blake2smac_state st;

   for (ilen = 0; ilen < 256; ilen++) inp[ilen] = (u8)ilen;
   ...
   for (ilen = 0; ilen < 256; ilen++) {
      ...
      if (ilen > 15) {
        ...
        blake2smac_process(&st, (u8*)inp,      5);
        blake2smac_process(&st, (u8*)inp + 5,  4);
        blake2smac_process(&st, (u8*)inp + 9,  3);
        blake2smac_process(&st, (u8*)inp + 12, 2);
        blake2smac_process(&st, (u8*)inp + 14, 1);
        blake2smac_process(&st, (u8*)inp + 15, ilen - 15);
        blake2smac_done(&st, out, &olen);
        if (compare_testvector(out, olen, mac, mlen, "BLAKE2S MAC multi", ilen) != 0) return CRYPT_FAIL_TESTVECTOR;
      }
      /* process in one go */
      blake2smac_init(&st, olen, key, klen);
      blake2smac_process(&st, (u8*)inp, ilen);
      blake2smac_done(&st, out, &olen);
