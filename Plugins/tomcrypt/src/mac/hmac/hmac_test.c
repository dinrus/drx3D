...
i32 hmac_test(void)
{
    ...
    return CRYPT_NOP;
 #else
    u8 digest[MAXBLOCKSIZE];
    i32 i;

    static u8k hmac_test_case_keys[][136] = {
        { /* 1 */
    ...
    static u8k hmac_test_case_data[][153] = {
        {
            "Hi There"
        },
#ifdef LTC_TEST_EXT
    ...
    for(i=0; i < (i32)(sizeof(cases) / sizeof(cases[0])); i++) {
        i32 hash = find_hash(cases[i].algo);
        if (hash == -1) continue;
        ++tested;
