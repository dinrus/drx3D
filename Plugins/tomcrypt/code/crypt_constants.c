...
i32 crypt_get_constant(tukk namein, i32 *valueout) {
    i32 i;
    i32 _crypt_constants_len = sizeof(_crypt_constants) / sizeof(_crypt_constants[0]);
...
i32 crypt_list_all_constants(char *names_list, u32 *names_list_size) {
    ...
    char *ptr;
    i32 number_len;
    i32 count = sizeof(_crypt_constants) / sizeof(_crypt_constants[0]);
    ...
    if (names_list == NULL) {
        *names_list_size = total_len;
        ...
        for (i=0; i<count; i++) {
            ...
            if ((u32)number_len > total_len) return -1;
            total_len -= number_len;
            ptr += number_len;
        }
