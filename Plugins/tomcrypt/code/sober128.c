...
{
   u64 len = sober128_desc.export_size;
...
i32 sober128_import(u8k *in, u64 inlen, prng_state *prng)
{
   ...
   LTC_ARGCHK(prng != NULL);
   LTC_ARGCHK(in   != NULL);
   if (inlen < (u64)sober128_desc.export_size) return CRYPT_INVALID_ARG;
...
i32 sober128_test(void)
{
   ...
   u8 out[500];
   u8 t1[] = { 0x31, 0x82, 0xA7, 0xA5, 0x8B, 0xD7, 0xCB, 0x39, 0x86, 0x1A };
   u8 t2[] = { 0x6B, 0x43, 0x9E, 0xBC, 0xE7, 0x62, 0x9B, 0xE6, 0x9B, 0x83 };
   u8 t3[] = { 0x4A, 0x0E, 0x6C, 0xC1, 0xCF, 0xB4, 0x73, 0x49, 0x99, 0x05 };
   i32 err;
