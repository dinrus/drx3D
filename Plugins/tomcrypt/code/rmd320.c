...
static i32 _rmd320_compress(hash_state *md, u8 *buf)
#else
static i32  rmd320_compress(hash_state *md, u8 *buf)
#endif
{
...
static i32 rmd320_compress(hash_state *md, u8 *buf)
{
   ...
   burn_stack(sizeof(ulong32) * 27 + sizeof(i32));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
i32 rmd320_done(hash_state * md, u8 *out)
{
    i32 i;
    ...
    /* append the '1' bit */
    md->rmd320.buf[md->rmd320.curlen++] = (u8)0x80;
    ...
    if (md->rmd320.curlen > 56) {
        while (md->rmd320.curlen < 64) {
            md->rmd320.buf[md->rmd320.curlen++] = (u8)0;
        }
...
i32 rmd320_test(void)
{
   ...
   for (i = 0; i < (i32)(sizeof(tests)/sizeof(tests[0])); i++) {
       rmd320_init(&md);
       rmd320_process(&md, (u8*)tests[i].msg, strlen(tests[i].msg));
       rmd320_done(&md, tmp);
