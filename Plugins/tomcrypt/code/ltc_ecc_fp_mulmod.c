...
static struct {
   ...
   i32        lru_count;      /* amount of times this entry has been used */
   i32        lock;           /* flag to indicate cache eviction permitted (0) or not (1) */
} fp_cache[FP_ENTRIES];
...
static i32 _find_base(ecc_point *g)
{
   i32 x;
   for (x = 0; x < FP_ENTRIES; x++) {
...
static i32 _add_entry(i32 idx, ecc_point *g)
{
   unsigned x, y;
 ...
 */
static i32 _build_lut(i32 idx, uk modulus, uk mp, uk mu)
{
   ...
   for (x = 2; x <= FP_LUT; x++) {
       for (y = 0; y < (1UL<<FP_LUT); y++) {
           if (lut_orders[y].ham != (i32)x) continue;
...
static i32 _accel_fp_mul(i32 idx, uk k, ecc_point *R, uk modulus, uk mp, i32 map)
{
   u8 kb[128];
   i32      x;
...
/* perform a fixed point ECC mulmod */
static i32 _accel_fp_mul2add(i32 idx1, i32 idx2,
                            uk kA, uk kB,
                            ecc_point *R, uk modulus, uk mp)
{
   u8 kb[2][128];
   i32      x;
...
*/
i32
ltc_ecc_fp_add_point(ecc_point *g, uk modulus, i32 lock)
{
   i32 idx;
   i32 err;
   uk mp = NULL;
...
i32 ltc_ecc_fp_save_state(u8 **out, u64 *outlen)
{
   ...
   u32   i, j, k;
   u64  fp_entries, fp_lut, num_entries;
   i32            err;
...
i32 ltc_ecc_fp_restore_state(u8 *in, u64 inlen)
{
   ...
   u64  num_entries, fp_entries, fp_lut;
   u64  i, j;
   u32   x;
