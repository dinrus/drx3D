...
static i32 _ecc_verify_hash(u8k *sig,  u64 siglen,
                            u8k *hash, u64 hashlen,
                            i32 *stat, ecc_key *key, i32 sigformat)
{
   ecc_point    *mG, *mQ;
   void          *r, *s, *v, *w, *u1, *u2, *e, *p, *m;
   void          *mp;
   i32           err;
   u64 pbits, pbytes, i, shift_right;
   u8 ch, buf[MAXBLOCKSIZE];
   ...
   *stat = 0;
   ...
   if (sigformat == 1) {
      ...
      i = siglen / 2;
      if ((err = mp_read_unsigned_bin(r, (u8*)sig,   i)) != CRYPT_OK)                       { goto error; }
      if ((err = mp_read_unsigned_bin(s, (u8*)sig+i, i)) != CRYPT_OK)                       { goto error; }
   }
   else {
      /* ASN.1 format */
   ...
   pbytes = (pbits+7) >> 3;
   if (pbits > hashlen*8) {
      if ((err = mp_read_unsigned_bin(e, (u8*)hash, hashlen)) != CRYPT_OK)                  { goto error; }
   }
   else if (pbits % 8 == 0) {
      if ((err = mp_read_unsigned_bin(e, (u8*)hash, pbytes)) != CRYPT_OK)                   { goto error; }
   }
   else {
      ...
      if ((err = mp_read_unsigned_bin(e, (u8*)buf, pbytes)) != CRYPT_OK)                    { goto error; }
   }
