...
static i32 _rijndael_ecb_encrypt(u8k *pt, u8 *ct, symmetric_key *skey)
#else
i32 ECB_ENC(u8k *pt, u8 *ct, symmetric_key *skey)
#endif
{
...
i32 ECB_ENC(u8k *pt, u8 *ct, symmetric_key *skey)
{
   i32 err = _rijndael_ecb_encrypt(pt, ct, skey);
   burn_stack(sizeof(u64)*8 + sizeof(u64*) + sizeof(i32)*2);
   return err;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static i32 _rijndael_ecb_decrypt(u8k *ct, u8 *pt, symmetric_key *skey)
#else
i32 ECB_DEC(u8k *ct, u8 *pt, symmetric_key *skey)
#endif
{
    ulong32 s0, s1, s2, s3, t0, t1, t2, t3, *rk;
    i32 Nr, r;
...
i32 ECB_DEC(u8k *ct, u8 *pt, symmetric_key *skey)
{
   i32 err = _rijndael_ecb_decrypt(ct, pt, skey);
   burn_stack(sizeof(u64)*8 + sizeof(u64*) + sizeof(i32)*2);
   return err;
}
#endif
