...
static i32 _rmd256_compress(hash_state *md, u8 *buf)
#else
static i32  rmd256_compress(hash_state *md, u8 *buf)
#endif
{
...
static i32 rmd256_compress(hash_state *md, u8 *buf)
{
   ...
   burn_stack(sizeof(ulong32) * 25 + sizeof(i32));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
i32 rmd256_done(hash_state * md, u8 *out)
{
    i32 i;
    ...
    /* append the '1' bit */
    md->rmd256.buf[md->rmd256.curlen++] = (u8)0x80;
    ...
    if (md->rmd256.curlen > 56) {
        while (md->rmd256.curlen < 64) {
            md->rmd256.buf[md->rmd256.curlen++] = (u8)0;
        }
...
i32 rmd256_test(void)
{
   ...
   for (i = 0; i < (i32)(sizeof(tests)/sizeof(tests[0])); i++) {
       rmd256_init(&md);
       rmd256_process(&md, (u8*)tests[i].msg, strlen(tests[i].msg));
       rmd256_done(&md, tmp);
