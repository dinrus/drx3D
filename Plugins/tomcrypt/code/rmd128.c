...
static i32 _rmd128_compress(hash_state *md, u8 *buf)
#else
static i32  rmd128_compress(hash_state *md, u8 *buf)
#endif
{
...
static i32 rmd128_compress(hash_state *md, u8 *buf)
{
   ...
   burn_stack(sizeof(ulong32) * 24 + sizeof(i32));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
i32 rmd128_done(hash_state * md, u8 *out)
{
    i32 i;
    ...
    /* append the '1' bit */
    md->rmd128.buf[md->rmd128.curlen++] = (u8)0x80;
    ...
    if (md->rmd128.curlen > 56) {
        while (md->rmd128.curlen < 64) {
            md->rmd128.buf[md->rmd128.curlen++] = (u8)0;
        }
...
i32 rmd128_test(void)
{
   ...
   for (i = 0; i < (i32)(sizeof(tests)/sizeof(tests[0])); i++) {
       rmd128_init(&md);
       rmd128_process(&md, (u8*)tests[i].msg, strlen(tests[i].msg));
       rmd128_done(&md, tmp);
