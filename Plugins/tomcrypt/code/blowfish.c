 ...
 */
i32 blowfish_setup(u8k *key, i32 keylen, i32 num_rounds,
                   symmetric_key *skey)
{
   ulong32 x, y, z, A;
   u8 B[8];
...
*/
#ifdef LTC_CLEAN_STACK
static i32 _blowfish_ecb_encrypt(u8k *pt, u8 *ct, symmetric_key *skey)
#else
...
i32 blowfish_ecb_encrypt(u8k *pt, u8 *ct, symmetric_key *skey)
{
    i32 err = _blowfish_ecb_encrypt(pt, ct, skey);
...
*/
#ifdef LTC_CLEAN_STACK
static i32 _blowfish_ecb_decrypt(u8k *ct, u8 *pt, symmetric_key *skey)
#else
...
i32 blowfish_ecb_decrypt(u8k *ct, u8 *pt, symmetric_key *skey)
{
    i32 err = _blowfish_ecb_decrypt(ct, pt, skey);
    burn_stack(sizeof(ulong32) * 2 + sizeof(i32));
    return err;
}
#endif
