...
/* reseed the PRNG */
static i32 _fortuna_reseed(prng_state *prng)
{
   u8 tmp[MAXBLOCKSIZE];
   hash_state    md;
   ulong64       reset_cnt;
...
*/
static i32 _fortuna_update_seed(u8k *in, u64 inlen, prng_state *prng)
{
   i32           err;
   u8 tmp[MAXBLOCKSIZE];
   hash_state    md;
...
i32 fortuna_export(u8 *out, u64 *outlen, prng_state *prng)
{
   i32         x, err;
   hash_state *md;
   u64 len = fortuna_desc.export_size;
...
i32 fortuna_import(u8k *in, u64 inlen, prng_state *prng)
{
   ...
   if (inlen < (u64)fortuna_desc.export_size) {
