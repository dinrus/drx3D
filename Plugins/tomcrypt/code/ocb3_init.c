...
static void _ocb3_int_calc_offset_zero(ocb3_state *ocb, u8k *nonce, u64 noncelen, u64 taglen)
{
   i32 x, y, bottom;
   i32 idx, shift;
   u8 iNonce[MAXBLOCKSIZE];
   u8 iKtop[MAXBLOCKSIZE];
   u8 iStretch[MAXBLOCKSIZE+8];

   /* Nonce = zeros(127-bitlen(N)) || 1 || N          */
   zeromem(iNonce, sizeof(iNonce));
   for (x = ocb->block_len-1, y=0; y<(i32)noncelen; x--, y++) {
             ...
             u64 taglen)
{
   ...
   x = (i32)(sizeof(polys)/sizeof(polys[0]));
