...
/* caution, the RFC says its "C[j] = S[M[i*16+j] xor L]" but the reference source code [and test vectors] say
   otherwise.
*/
       L = (md->md2.chksum[j] ^= PI_SUBST[(i32)(md->md2.buf[j] ^ L)] & 255);
   }
}
...
static void md2_compress(hash_state *md)
{
   ...
   for (j = 0; j < 18; j++) {
       for (k = 0; k < 48; k++) {
           t = (md->md2.X[k] ^= PI_SUBST[(i32)(t & 255)]);
       }
       t = (t + (u8)j) & 255;
   }
}
...
/**
   ...
   @return CRYPT_OK if successful
*/
i32 md2_done(hash_state * md, u8 *out)
{
    u64 i, k;
...
i32 md2_test(void)
{
   ...
   for (i = 0; i < (i32)(sizeof(tests) / sizeof(tests[0])); i++) {
       md2_init(&md);
       md2_process(&md, (u8*)tests[i].msg, (u64)strlen(tests[i].msg));
       md2_done(&md, tmp);
