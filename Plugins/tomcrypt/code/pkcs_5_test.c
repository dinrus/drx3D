...
i32 pkcs_5_test (void)
{
    ...
    u64 dkLen;
    i32 i, err;
    i32 tested=0, failed=0;
    i32 hash = find_hash("sha1");
    if (hash == -1)
    {
#ifdef LTC_TEST_DBG
    ...
    /* testing alg 2 */
    for(i=0; i < (i32)(sizeof(cases_5_2) / sizeof(cases_5_2[0])); i++) {
        ++tested;
        dkLen = cases_5_2[i].dkLen;
        if((err = pkcs_5_alg2((u8*)cases_5_2[i].P, cases_5_2[i].P_len,
                              (u8*)cases_5_2[i].S, cases_5_2[i].S_len,
                              cases_5_2[i].c, hash,
                              DK, &dkLen)) != CRYPT_OK) {
#ifdef LTC_TEST_DBG
    ...
    /* testing alg 1 */
    for(i=0; i < (i32)(sizeof(cases_5_1) / sizeof(case_item)); i++, tested++) {
        dkLen = cases_5_1[i].dkLen;
        if((err = pkcs_5_alg1((u8*)cases_5_1[i].P, cases_5_1[i].P_len,
                              (u8*)cases_5_1[i].S,
                              cases_5_1[i].c, hash,
                              DK, &dkLen)) != CRYPT_OK) {
#ifdef LTC_TEST_DBG
    ...
    for(i = 0; i < (i32)(sizeof(cases_5_1o) / sizeof(cases_5_1o[0])); i++, tested++) {
        dkLen = cases_5_1o[i].dkLen;
        if ((err = pkcs_5_alg1_openssl((u8*)cases_5_1o[i].P, cases_5_1o[i].P_len,
                                       (u8*)cases_5_1o[i].S,
                                       cases_5_1o[i].c, hash,
                                       DK, &dkLen)) != CRYPT_OK) {
#ifdef LTC_TEST_DBG
