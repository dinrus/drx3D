...
#include <drx/Eigen/StdVector>
#include <drx/Eigen/Geometry>
...
public:
	...
	static tQuaternion VecToQuaternion(const tVector& v);
	static tVector QuaternionToVec(const tQuaternion& q);
	static tQuaternion QuaternionDiff(const tQuaternion& q0, const tQuaternion& q1);
	static double QuaternionDiffTheta(const tQuaternion& q0, const tQuaternion& q1);
	static double QuaternionTheta(const tQuaternion& dq);
	static tQuaternion VecDiffQuaternion(const tVector& v0, const tVector& v1);
	static tVector QuaternionRotVec(const tQuaternion& q, const tVector& dir);
	static tQuaternion MirrorQuaternion(const tQuaternion& q, eAxis axis);

	static double Sign(double val);
	static i32 Sign(i32 val);

	static double AddAverage(double avg0, i32 count0, double avg1, i32 count1);
	static tVector AddAverage(const tVector& avg0, i32 count0, const tVector& avg1, i32 count1);
	static void AddAverage(const Eigen::VectorXd& avg0, i32 count0, const Eigen::VectorXd& avg1, i32 count1, Eigen::VectorXd& out_result);
	...
	static void QuaternionSwingTwistDecomposition(const tQuaternion& q, const tVector& dir, tQuaternion& out_swing, tQuaternion& out_twist);
	static tQuaternion ProjectQuaternion(const tQuaternion& q, const tVector& dir);
