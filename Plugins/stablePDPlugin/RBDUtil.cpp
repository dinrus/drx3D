...
void cRBDUtil::SolveInvDyna(const cRBDModel& model, const Eigen::VectorXd& acc, Eigen::VectorXd& out_tau)
{
	...
	for (i32 j = 0; j < num_joints; ++j)
	{
		if (cKinTree::IsValidBody(body_defs, j))
		{
			...
			{
				i32 parent_id = cKinTree::GetParent(joint_mat, j);
...
{
	i32k svs = gSpVecSize;
	i32 num_joints = model.GetNumJoints();
...
void cRBDUtil::BuildMassMat(const cRBDModel& model, Eigen::MatrixXd& inertia_buffer, Eigen::MatrixXd& out_mass_mat)
{
	...
	for (i32 j = num_joints - 1; j >= 0; --j)
	{
		if (cKinTree::IsValidBody(body_defs, j))
		{
			...
			const Eigen::Block<const Eigen::MatrixXd> S = model.GetJointSubspace(j);
			i32 dim = cKinTree::GetParamSize(joint_mat, j);
			if (dim > 0)
			{
				i32 offset = cKinTree::GetParamOffset(joint_mat, j);
				...
				while (cKinTree::HasParent(joint_mat, curr_id))
				{
					...
					i32 curr_offset = cKinTree::GetParamOffset(joint_mat, curr_id);
					i32 curr_dim = cKinTree::GetParamSize(joint_mat, curr_id);
...
void cRBDUtil::BuildEndEffectorJacobian(const cRBDModel& model, i32 joint_id, Eigen::MatrixXd& out_J)
{
	// jacobian in world coordinates
	const Eigen::MatrixXd& joint_mat = model.GetJointMat();
	const Eigen::VectorXd& pose = model.GetPose();
	
	i32 num_dofs = cKinTree::GetNumDof(joint_mat);
	...
	{
		i32 offset = cKinTree::GetParamOffset(joint_mat, curr_id);
		i32 size = cKinTree::GetParamSize(joint_mat, curr_id);
		...
		i32 parent_id = cKinTree::GetParent(joint_mat, curr_id);
...
void cRBDUtil::BuildEndEffectorJacobian(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, i32 joint_id, Eigen::MatrixXd& out_J)
{
	// jacobian in world coordinates
	i32 num_dofs = cKinTree::GetNumDof(joint_mat);
	out_J = Eigen::MatrixXd::Zero(gSpVecSize, num_dofs);
...
Eigen::MatrixXd cRBDUtil::MultJacobianEndEff(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& q, const Eigen::MatrixXd& J, i32 joint_id)
{
...
void cRBDUtil::BuildJacobian(const cRBDModel& model, Eigen::MatrixXd& out_J)
{
	...
	for (i32 j = 0; j < num_joints; ++j)
	{
		...
		i32 offset = cKinTree::GetParamOffset(joint_mat, j);
		i32 size = cKinTree::GetParamSize(joint_mat, j);
...
Eigen::MatrixXd cRBDUtil::ExtractEndEffJacobian(const Eigen::MatrixXd& joint_mat, const Eigen::MatrixXd& J, i32 joint_id)
{
	i32 curr_id = joint_id;
...
cSpAlg::tSpVec cRBDUtil::CalcVelProdAcc(const cRBDModel& model, const Eigen::MatrixXd& Jd, i32 joint_id)
{
	const Eigen::MatrixXd& joint_mat = model.GetJointMat();
	const Eigen::VectorXd& pose = model.GetPose();
	const Eigen::VectorXd& vel = model.GetVel();
...
cSpAlg::tSpVec cRBDUtil::CalcJointWorldVel(const cRBDModel& model, i32 joint_id)
{
	cSpAlg::tSpVec joint_vel = CalcJointWorldVel(model.GetJointMat(), model.GetPose(), model.GetVel(), joint_id);
	return joint_vel;
}

cSpAlg::tSpVec cRBDUtil::CalcJointWorldVel(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, const Eigen::VectorXd& vel, i32 joint_id)
{
	cSpAlg::tSpVec joint_vel = CalcWorldVel(joint_mat, state, vel, joint_id);
	return joint_vel;
}

cSpAlg::tSpVec cRBDUtil::CalcWorldVel(const cRBDModel& model, i32 joint_id)
{
	return CalcWorldVel(model.GetJointMat(), model.GetPose(), model.GetVel(), joint_id);
}

cSpAlg::tSpVec cRBDUtil::CalcWorldVel(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, const Eigen::VectorXd& vel, i32 joint_id)
{
	Eigen::MatrixXd J;
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertia(const Eigen::MatrixXd& body_defs, i32 part_id)
{
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertiaBox(const Eigen::MatrixXd& body_defs, i32 part_id)
{
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertiaCapsule(const Eigen::MatrixXd& body_defs, i32 part_id)
{
	const cKinTree::tBodyDef& def = body_defs.row(part_id);
	double mass = cKinTree::GetBodyMass(body_defs, part_id);
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertiaSphere(const Eigen::MatrixXd& body_defs, i32 part_id)
{
	const cKinTree::tBodyDef& def = body_defs.row(part_id);
	double mass = cKinTree::GetBodyMass(body_defs, part_id);
	double r = 0.5 * def(cKinTree::eBodyParam0);
...
cSpAlg::tSpMat cRBDUtil::BuildMomentInertiaCylinder(const Eigen::MatrixXd& body_defs, i32 part_id)
{
	const cKinTree::tBodyDef& def = body_defs.row(part_id);
	double mass = cKinTree::GetBodyMass(body_defs, part_id);
	double r = 0.5 * def(cKinTree::eBodyParam0);
...
cSpAlg::tSpMat cRBDUtil::BuildInertiaSpatialMat(const Eigen::MatrixXd& body_defs, i32 part_id)
{
...
cSpAlg::tSpTrans cRBDUtil::BuildParentChildTransform(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, i32 j)
{
...
cSpAlg::tSpTrans cRBDUtil::BuildChildParentTransform(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, i32 j)
{
...
bool cRBDUtil::IsConstJointSubspace(const Eigen::MatrixXd& joint_mat, i32 j)
{
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspace(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, i32 j)
{
...
{
	i32 dim = cKinTree::gRootDim;
	i32 pos_dim = cKinTree::gPosDim;
	i32 rot_dim = cKinTree::gRotDim;
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspaceRevolute(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, i32 j)
{
	i32 dim = cKinTree::GetJointParamSize(cKinTree::eJointTypeRevolute);
	Eigen::MatrixXd S = Eigen::MatrixXd::Zero(gSpVecSize, dim);
	S(2, 0) = 1;
	return S;
}

Eigen::MatrixXd cRBDUtil::BuildJointSubspacePrismatic(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, i32 j)
{
	i32 dim = cKinTree::GetJointParamSize(cKinTree::eJointTypePrismatic);
	Eigen::MatrixXd S = Eigen::MatrixXd::Zero(gSpVecSize, dim);
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspacePlanar(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, i32 j)
{
	i32 dim = cKinTree::GetJointParamSize(cKinTree::eJointTypePlanar);
	Eigen::MatrixXd S = Eigen::MatrixXd::Zero(gSpVecSize, dim);
	S(3, 0) = 1;
	S(4, 1) = 1;
	S(2, 2) = 1;
...
Eigen::MatrixXd cRBDUtil::BuildJointSubspaceFixed(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, i32 j)
{
	i32 dim = cKinTree::GetJointParamSize(cKinTree::eJointTypeFixed);
	Eigen::MatrixXd S = Eigen::MatrixXd::Zero(gSpVecSize, dim);
	return S;
}

Eigen::MatrixXd cRBDUtil::BuildJointSubspaceSpherical(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, i32 j)
{
	i32 dim = cKinTree::GetJointParamSize(cKinTree::eJointTypeSpherical);
	Eigen::MatrixXd S = Eigen::MatrixXd::Zero(gSpVecSize, dim);
	S(0, 0) = 1;
	S(1, 1) = 1;
	S(2, 2) = 1;
...
cSpAlg::tSpVec cRBDUtil::BuildCj(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& q, const Eigen::VectorXd& q_dot, i32 j)
{
	cKinTree::eJointType j_type = cKinTree::GetJointType(joint_mat, j);
	bool is_root = cKinTree::IsRoot(joint_mat, j);
	cSpAlg::tSpVec cj;
...
cSpAlg::tSpVec cRBDUtil::BuildCjRoot(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& q, const Eigen::VectorXd& q_dot, i32 j)
{
	...
	tQuaternion dquat = cMathUtil::VecToQuaternion(dq_vec);
...
cSpAlg::tSpVec cRBDUtil::BuildCjRevolute(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& q_dot, i32 j)
{
	return cSpAlg::tSpVec::Zero();
}

cSpAlg::tSpVec cRBDUtil::BuildCjPrismatic(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& q_dot, i32 j)
{
	return cSpAlg::tSpVec::Zero();
}

cSpAlg::tSpVec cRBDUtil::BuildCjPlanar(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& q_dot, i32 j)
{
	return cSpAlg::tSpVec::Zero();
}

cSpAlg::tSpVec cRBDUtil::BuildCjFixed(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& q_dot, i32 j)
{
	return cSpAlg::tSpVec::Zero();
}

cSpAlg::tSpVec cRBDUtil::BuildCjSpherical(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& q_dot, i32 j)
{
	return cSpAlg::tSpVec::Zero();
}
