...
i32 cMathUtil::Clamp(i32 val, i32 min, i32 max)
{
...
tQuaternion cMathUtil::VecToQuaternion(const tVector& v)
{
	return tQuaternion(v[0], v[1], v[2], v[3]);
}

tVector cMathUtil::QuaternionToVec(const tQuaternion& q)
{
	return tVector(q.w(), q.x(), q.y(), q.z());
}

tQuaternion cMathUtil::QuaternionDiff(const tQuaternion& q0, const tQuaternion& q1)
{
	return q1 * q0.conjugate();
}

double cMathUtil::QuaternionDiffTheta(const tQuaternion& q0, const tQuaternion& q1)
{
	tQuaternion dq = QuaternionDiff(q0, q1);
	return QuaternionTheta(dq);
}

double cMathUtil::QuaternionTheta(const tQuaternion& dq)
{
	double theta = 0;
	tQuaternion q1 = dq;
	if (q1.w() > 1)
...
tQuaternion cMathUtil::VecDiffQuaternion(const tVector& v0, const tVector& v1)
{
	return tQuaternion::FromTwoVectors(v0.segment(0, 3), v1.segment(0, 3));
}

tVector cMathUtil::QuaternionRotVec(const tQuaternion& q, const tVector& dir)
{
...
i32 cMathUtil::Sign(i32 val)
{
	return SignAux<i32>(val);
}

double cMathUtil::AddAverage(double avg0, i32 count0, double avg1, i32 count1)
{
	double total = count0 + count1;
	return (count0 / total) * avg0 + (count1 / total) * avg1;
}

tVector cMathUtil::AddAverage(const tVector& avg0, i32 count0, const tVector& avg1, i32 count1)
{
	double total = count0 + count1;
	return (count0 / total) * avg0 + (count1 / total) * avg1 ;
}

void cMathUtil::AddAverage(const Eigen::VectorXd& avg0, i32 count0, const Eigen::VectorXd& avg1, i32 count1, Eigen::VectorXd& out_result)
{
	double total = count0 + count1;
...
void cMathUtil::CalcSoftmax(const Eigen::VectorXd& vals, double temp, Eigen::VectorXd& out_prob)
{
	...
	i32 num_vals = static_cast<i32>(vals.size());
	double sum = 0;
	double max_val = vals.maxCoeff();
	for (i32 i = 0; i < num_vals; ++i)
	{
		double val = vals[i];
...
{
	i32 data_size = static_cast<i32>(covar.size());
...
bool cMathUtil::CheckNextInterval(double delta, double curr_val, double int_size)
{
	...
	i32 curr_count = static_cast<i32>(std::floor((curr_val + pad) / int_size));
	i32 prev_count = static_cast<i32>(std::floor((curr_val + pad - delta) / int_size));
...
void cMathUtil::QuaternionSwingTwistDecomposition(const tQuaternion& q, const tVector& dir, tQuaternion& out_swing, tQuaternion& out_twist)
{
...
tQuaternion cMathUtil::ProjectQuaternion(const tQuaternion& q, const tVector& dir)
{
	...
	tVector rot_dir1 = cMathUtil::QuaternionRotVec(q, rot_dir0);
	...
	{
		rot_dir1 /= dir1_norm;
		p_rot = cMathUtil::VecDiffQuaternion(rot_dir0, rot_dir1);
	}
	return p_rot;
}
...
void cMathUtil::ButterworthFilter(double dt, double cutoff, Eigen::VectorXd& out_x)
{
	...
	i32 n = static_cast<i32>(out_x.size());
