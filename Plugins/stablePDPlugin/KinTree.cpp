...
i32k cKinTree::gPosDim = 3;
i32k cKinTree::gRotDim = 4;
i32k cKinTree::gRootDim = gPosDim + gRotDim;
i32k cKinTree::gInvalidJointID = -1;

// Json keys
const STxt gJointTypeNames[cKinTree::eJointTypeMax] =
{
	"revolute",
	"planar",
	"prismatic",
...
const STxt gJointsKey = "Joints";
const STxt gJointDescKeys[cKinTree::eJointDescMax] = 
{
	"Type",
	"Parent",
	"AttachX",
...
const STxt gBodyDefsKey = "BodyDefs";
const STxt gBodyDescKeys[cKinTree::eBodyParamMax] =
{
	"Shape",
	"Mass",
	"ColGroup",
...
const STxt gDrawShapeDefsKey = "DrawShapeDefs";
const STxt gDrawShapeDescKeys[cKinTree::eDrawShapeParamMax] =
{
	"Shape",
	"ParentJoint",
	"AttachX",
...
i32 cKinTree::GetRoot(const Eigen::MatrixXd& joint_desc)
{
	// this should always be true right?
	return 0;
}

void cKinTree::FindChildren(const Eigen::MatrixXd& joint_desc, i32 joint_id, Eigen::VectorXi& out_children)
{
	i32k max_size = 128;
	i32 children_buffer[max_size];
	i32 num_children = 0;
	i32 num_joints = GetNumJoints(joint_desc);

	for (i32 i = 0; i < num_joints; ++i)
	{
		i32 parent = GetParent(joint_desc, i);
		if (parent == joint_id)
		{
...
#ifdef USE_JSON
bool cKinTree::LoadBodyDefs(const STxt& char_file, Eigen::MatrixXd& out_body_defs)
{
	bool succ = true;
	STxt str;
	...
	if (succ)
	{
		...
		if (!root[gBodyDefsKey].isNull())
		{
			...
			i32 num_bodies = body_defs.size();

			succ = true;
			out_body_defs.resize(num_bodies, eBodyParamMax);
			for (i32 b = 0; b < num_bodies; ++b)
			{
...
{
	STxt shape_str = root.get(gBodyDescKeys[eBodyParamShape], "").asString();
	cShape::eShape shape;
	bool succ = cShape::ParseShape(shape_str, shape);
	if (succ)
	{
		out_def(eBodyParamShape) = static_cast<double>(static_cast<i32>(shape));
	}

	for (i32 i = 0; i < eBodyParamMax; ++i)
	{
		const STxt& curr_key = gBodyDescKeys[i];
...
bool cKinTree::LoadDrawShapeDefs(const STxt& char_file, Eigen::MatrixXd& out_draw_defs)
{
	bool succ = true;
	STxt str;
	...
	if (succ)
	{
		if (!root[gDrawShapeDefsKey].isNull())
		{
			...
			i32 num_shapes = shape_defs.size();

			succ = true;
			out_draw_defs.resize(num_shapes, eDrawShapeParamMax);
			for (i32 b = 0; b < num_shapes; ++b)
			{
...
{
	STxt shape_str = root.get(gDrawShapeDescKeys[eDrawShapeShape], "").asString();
	cShape::eShape shape;
	bool succ = cShape::ParseShape(shape_str, shape);
	if (succ)
	{
		out_def(eDrawShapeShape) = static_cast<double>(static_cast<i32>(shape));
	}

	for (i32 i = 0; i < eDrawShapeParamMax; ++i)
	{
		const STxt& curr_key = gDrawShapeDescKeys[i];
		if (!root[curr_key].isNull()
			&& root[curr_key].isNumeric())
		{
			Json::Value json_val = root[curr_key];
...
bool cKinTree::Load(const Json::Value& root, Eigen::MatrixXd& out_joint_mat)
{
	...
	if (!root[gJointsKey].isNull())
	{
		...
		for (i32 j = 0; j < num_joints; ++j)
		{
			...
			i32 parent_id = static_cast<i32>(curr_desc(eJointDescParent));
			if (parent_id >= j)
			{
...
bool cKinTree::ParseJoint(const Json::Value& root, tJointDesc& out_joint_desc)
{
	...
	{
		STxt type_str = type_json.asString();
		ParseJointType(type_str, joint_type);
		out_joint_desc[eJointDescType] = static_cast<double>(static_cast<i32>(joint_type));
	}

	for (i32 i = 0; i < eJointDescMax; ++i)
	{
		if (i != eJointDescType)
		{
			const STxt& key = gJointDescKeys[i];
			if (!root[key].isNull())
			{
...
STxt cKinTree::BuildJointMatJson(const Eigen::MatrixXd& joint_mat)
{
	STxt json = "";
	json += "{\n\"" + gJointsKey + "\":\n[\n";

	i32 num_joints = GetNumJoints(joint_mat);
	for (i32 j = 0; j < num_joints; ++j)
	{
		tJointDesc curr_desc = joint_mat.row(j);
		STxt joint_json = BuildJointJson(j, curr_desc);
...
STxt cKinTree::BuildJointJson(i32 id, const tJointDesc& joint_desc)
{
	STxt json = "";
	json += "{\n";
	...
	for (i32 i = 0; i < eJointDescMax; ++i)
	{
		STxt param_name = gJointDescKeys[i];
		if (i == eJointDescType)
		{
			json += ",\n";
			i32 type_id = static_cast<i32>(joint_desc[i]);
			STxt type_str = gJointTypeNames[type_id];
...
tVector cKinTree::CalcBodyPartPos(const Eigen::MatrixXd& joint_mat, const Eigen::MatrixXd& body_defs, const Eigen::VectorXd& state, i32 part_id)
{
...
tVector cKinTree::CalcBodyPartVel(const Eigen::MatrixXd& joint_mat, const Eigen::MatrixXd& body_defs, const Eigen::VectorXd& state, const Eigen::VectorXd& vel, i32 part_id)
{
	tVector attach_pt = cKinTree::GetBodyAttachPt(body_defs, part_id);
	return CalcWorldVel(joint_mat, state, vel, part_id, attach_pt);
}

cShape::eShape cKinTree::GetBodyShape(const Eigen::MatrixXd& body_defs, i32 part_id)
{
	cShape::eShape shape = static_cast<cShape::eShape>(static_cast<i32>(body_defs(part_id, cKinTree::eBodyParamShape)));
	return shape;
}

tVector cKinTree::GetBodyAttachPt(const Eigen::MatrixXd& body_defs, i32 part_id)
{
...
tVector cKinTree::GetBodyAttachTheta(const Eigen::MatrixXd& body_defs, i32 part_id)
{
...
void cKinTree::GetBodyRotation(const Eigen::MatrixXd& body_defs, i32 part_id, tVector& out_axis, double& out_theta)
{
	tVector theta = GetBodyAttachTheta(body_defs, part_id);
	cMathUtil::EulerToAxisAngle(theta, out_axis, out_theta);
}

double cKinTree::GetBodyMass(const Eigen::MatrixXd& body_defs, i32 part_id)
{
	double mass = body_defs(part_id, eBodyParamMass);
	return mass;
}

i32 cKinTree::GetBodyColGroup(const Eigen::MatrixXd& body_defs, i32 part_id)
{
	i32 col_group = static_cast<i32>(body_defs(part_id, eBodyParamColGroup));
	return col_group;
}

bool cKinTree::GetBodyEnableFallContact(const Eigen::MatrixXd& body_defs, i32 part_id)
{
	double fall_val = body_defs(part_id, eBodyParamEnableFallContact);
	return fall_val != 0;
}

void cKinTree::SetBodyEnableFallContact(i32 part_id, bool enable, Eigen::MatrixXd& out_body_defs)
{
	out_body_defs(part_id, eBodyParamEnableFallContact) = (enable) ? 1 : 0;
}

tVector cKinTree::GetBodySize(const Eigen::MatrixXd& body_defs, i32 part_id)
{
...
tVector cKinTree::GetBodyColor(const Eigen::MatrixXd& body_defs, i32 part_id)
{
	const tBodyDef& def = body_defs.row(part_id);
...
{
	double total_mass = 0;
	for (i32 i = 0; i < body_defs.rows(); ++i)
	{
...
bool cKinTree::IsValidBody(const Eigen::MatrixXd& body_defs, i32 part_id)
{
...
tVector cKinTree::GetBodyLocalCoM(const Eigen::MatrixXd& body_defs, i32 part_id)
{
	cShape::eShape shape = GetBodyShape(body_defs, part_id);
...
i32 cKinTree::GetDrawShapeParentJoint(const tDrawShapeDef& shape)
{
	return static_cast<i32>(shape[eDrawShapeParentJoint]);
}
...
i32 cKinTree::GetDrawShapeMeshID(const tDrawShapeDef& shape)
{
	return static_cast<i32>(shape[eDrawShapeMeshID]);
}

void cKinTree::CalcBodyPartRotation(const Eigen::MatrixXd& joint_mat, const Eigen::MatrixXd& body_defs, const Eigen::VectorXd& state, i32 part_id, tVector& out_axis, double& out_theta)
{
...
tQuaternion cKinTree::GetRootRot(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state)
{
	i32 root_id = GetRoot(joint_mat);
	i32 param_offset = GetParamOffset(joint_mat, root_id);
	tQuaternion rot = cMathUtil::VecToQuaternion(state.segment(param_offset + gPosDim, gRotDim));
	return rot;
}
...
void cKinTree::SetRootRot(const Eigen::MatrixXd& joint_mat, const tQuaternion& rot, Eigen::VectorXd& out_state)
{
	i32 root_id = GetRoot(joint_mat);
	i32 param_offset = GetParamOffset(joint_mat, root_id);
	out_state.segment(param_offset + gPosDim, gRotDim) = cMathUtil::QuaternionToVec(rot).segment(0, gRotDim);
}
...
tVector cKinTree::CalcJointWorldPos(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
	tVector pos = LocalToWorldPos(joint_mat, state, joint_id, tVector::Zero());
	return pos;
}

tVector cKinTree::LocalToWorldPos(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 parent_id, const tVector& attach_pt)
{
...
tQuaternion cKinTree::CalcJointWorldRot(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
	tMatrix mat = JointWorldTrans(joint_mat, state, joint_id);
	return cMathUtil::RotMatToQuaternion(mat);
}

void cKinTree::CalcJointWorldTheta(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id,
...
tVector cKinTree::CalcJointWorldVel(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, const Eigen::VectorXd& vel, i32 joint_id)
{
	return CalcWorldVel(joint_mat, state, vel, joint_id, tVector::Zero());
}

tVector cKinTree::CalcWorldVel(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, const Eigen::VectorXd& vel, i32 parent_id, const tVector& attach_pt)
{
...
tVector cKinTree::CalcJointWorldAngularVel(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, const Eigen::VectorXd& vel, i32 joint_id)
{
	return CalcWorldAngularVel(joint_mat, state, vel, joint_id, tVector::Zero());
}

tVector cKinTree::CalcWorldAngularVel(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, const Eigen::VectorXd& vel, i32 parent_id, const tVector& attach_pt)
{
	cSpAlg::tSpVec sv = cRBDUtil::CalcWorldVel(joint_mat, state, vel, parent_id);
	tVector pos = cKinTree::LocalToWorldPos(joint_mat, state, parent_id, attach_pt);
	cSpAlg::tSpTrans world_to_pt = cSpAlg::BuildTrans(pos);
...
i32 cKinTree::GetNumJoints(const Eigen::MatrixXd& joint_mat)
{
	return static_cast<i32>(joint_mat.rows());
}

i32 cKinTree::GetNumDof(const Eigen::MatrixXd& joint_mat)
{
	i32 num_joints = GetNumJoints(joint_mat);
	i32 num_dof = cKinTree::GetParamOffset(joint_mat, num_joints - 1) + cKinTree::GetParamSize(joint_mat, num_joints - 1);
	return num_dof;
}
...
{
	i32 root_id = GetRoot(joint_mat);
	i32 num_joints = cKinTree::GetNumJoints(joint_mat);
	out_pose += step;
}


Eigen::VectorXi cKinTree::FindJointChain(const Eigen::MatrixXd& joint_mat, i32 joint_beg, i32 joint_end)
{
	Eigen::VectorXi chain;

	i32k max_length = 128;
	i32 chain_buffer[max_length];
	i32 buffer_idx = 0;
	...
	i32 common_ancestor = gInvalidJointID;
	i32 curr_id = joint_beg;
	i32 end_len = 0;
	...
	if (found)
	{
		...
		i32 num_flips = static_cast<i32>(chain.size()) - buffer_idx;
...
bool cKinTree::IsAncestor(const Eigen::MatrixXd& joint_mat, i32 child_joint, i32 ancestor_joint, i32& out_len)
{
	i32 curr_id = child_joint;
	out_len = 0;
	while (curr_id != gInvalidJointID)
	{
...
{
	double len = 0;
	i32 num_joints = static_cast<i32>(chain.size());
	for (i32 i = 1; i < num_joints; ++i)
	{
		i32 curr_id = chain(i);
		i32 prev_id = chain(i - 1);

		if (prev_id != gInvalidJointID)
		{
			i32 prev_parent = GetParent(joint_mat, prev_id);
		...
		{
			i32 curr_parent = GetParent(joint_mat, curr_id);
...
void cKinTree::CalcAABB(const Eigen::MatrixXd& joint_desc, const Eigen::VectorXd& state, tVector& out_min, tVector& out_max)
{
	...
	for (i32 i = 0; i < GetNumJoints(joint_desc); ++i)
	{
...
i32 cKinTree::GetParamOffset(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	i32 offset = static_cast<i32>(joint_mat(joint_id, eJointDescParamOffset));
	return offset;
}

i32 cKinTree::GetParamSize(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	eJointType joint_type = cKinTree::GetJointType(joint_mat, joint_id);
	bool is_root = cKinTree::IsRoot(joint_mat, joint_id);
	i32 size = (is_root) ? gRootDim : GetJointParamSize(joint_type);
	return size;
}

i32 cKinTree::GetJointParamSize(eJointType joint_type)
{
	i32 size = 0;
	switch (joint_type)
	{
...
void cKinTree::GetJointParams(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 j, Eigen::VectorXd& out_params)
{
	i32 offset = GetParamOffset(joint_mat, j);
	i32 dim = GetParamSize(joint_mat, j);
	if (dim > 0)
	{
...
void cKinTree::SetJointParams(const Eigen::MatrixXd& joint_mat, i32 j, const Eigen::VectorXd& params, Eigen::VectorXd& out_state)
{
	i32 offset = GetParamOffset(joint_mat, j);
	i32 dim = GetParamSize(joint_mat, j);
...
cKinTree::eJointType cKinTree::GetJointType(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	eJointType type = static_cast<eJointType>(static_cast<i32>(joint_mat(joint_id, cKinTree::eJointDescType)));
	return type;
}

i32 cKinTree::GetParent(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	i32 parent = static_cast<i32>(joint_mat(joint_id, cKinTree::eJointDescParent));
	assert(parent < joint_id); // joints should always be ordered as such
								// since some algorithms will assume this ordering
	return parent;
}

bool cKinTree::HasParent(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	i32 parent = GetParent(joint_mat, joint_id);
	return parent != gInvalidJointID;
}

bool cKinTree::IsRoot(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	return !HasParent(joint_mat, joint_id);
}

bool cKinTree::IsJointActuated(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	return !IsRoot(joint_mat, joint_id);
}

double cKinTree::GetTorqueLimit(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	double torque_lim = joint_mat(joint_id, cKinTree::eJointDescTorqueLim);
	return torque_lim;
}

double cKinTree::GetForceLimit(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	double force_lim = joint_mat(joint_id, cKinTree::eJointDescForceLim);
	return force_lim;
}

bool cKinTree::IsEndEffector(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	double end_eff_val = joint_mat(joint_id, cKinTree::eJointDescIsEndEffector);
	return end_eff_val != 0;
}

tVector cKinTree::GetJointLimLow(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
...
tVector cKinTree::GetJointLimHigh(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
...
double cKinTree::GetJointDiffWeight(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	return joint_mat(joint_id, cKinTree::eJointDescDiffWeight);
}

double cKinTree::CalcLinkLength(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
...
tVector cKinTree::GetAttachPt(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
...
tVector cKinTree::GetAttachTheta(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
...
{
	i32 num_joints = static_cast<i32>(joint_mat.rows());
	out_lengths = Eigen::VectorXd::Zero(num_joints);

	for (i32 j = num_joints - 1; j >= 0; --j)
	{
		i32 parent_id = GetParent(joint_mat, j);
...
bool cKinTree::ParseJointType(const STxt& type_str, eJointType& out_joint_type)
{
	for (i32 i = 0; i < eJointTypeMax; ++i)
	{
		const STxt& name = gJointTypeNames[i];
		if (type_str == name)
		{
...
{
	i32 num_joints = GetNumJoints(out_joint_mat);
	i32 offset = 0;
	for (i32 j = 0; j < num_joints; ++j)
	{
		i32 curr_size = GetParamSize(out_joint_mat, j);
		out_joint_mat(j, eJointDescParamOffset) = offset;
		offset += curr_size;
	}
	i32 root_id = GetRoot(out_joint_mat);
...
tMatrix cKinTree::BuildAttachTrans(const Eigen::MatrixXd& joint_mat, i32 joint_id)
{
	// child to parent
	tVector attach_pt = GetAttachPt(joint_mat, joint_id);
...
tMatrix cKinTree::ChildParentTrans(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
	tMatrix mat;
...
tMatrix cKinTree::ParentChildTrans(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
...
tMatrix cKinTree::JointWorldTrans(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
	tMatrix m = tMatrix::Identity();
	i32 curr_id = joint_id;
...
tMatrix cKinTree::WorldJointTrans(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
...
tMatrix cKinTree::BodyWorldTrans(const Eigen::MatrixXd& joint_mat, const Eigen::MatrixXd& body_defs, const Eigen::VectorXd& state, i32 part_id)
{
...
tMatrix cKinTree::BodyJointTrans(const Eigen::MatrixXd& body_defs, i32 part_id)
{
...
cKinTree::tJointDesc cKinTree::BuildJointDesc(eJointType joint_type, i32 parent_id, const tVector& attach_pt)
{
...
{
	i32 num_dof = GetNumDof(joint_mat);
...
void cKinTree::CalcPoseDiff(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose0, const Eigen::VectorXd& pose1, Eigen::VectorXd& out_diff)
{
	...
	for (i32 j = 0; j < num_joints; ++j)
	{
		...
		i32 param_offset = GetParamOffset(joint_mat, j);
		i32 param_size = GetParamSize(joint_mat, j);
...
tQuaternion cKinTree::CalcRootRotDiff(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose0, const Eigen::VectorXd& pose1)
{
	...
	return cMathUtil::QuaternionDiff(root_rot0, root_rot1);
}

double cKinTree::CalcPoseErr(const Eigen::MatrixXd& joint_mat, i32 joint_id, const Eigen::VectorXd& pose0, const Eigen::VectorXd& pose1)
{
	bool is_root = IsRoot(joint_mat, joint_id);
	double err = 0;
	if (is_root)
	...
	else
	{
		...
		case eJointTypeSpherical:
		{
			tQuaternion dq = cMathUtil::VecToQuaternion(diff);
			double theta = cMathUtil::QuaternionTheta(dq);
			err = theta * theta;
			break;
		}
		default:
...
double cKinTree::CalcRootRotErr(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose0, const Eigen::VectorXd& pose1)
{
	...
	double theta = cMathUtil::QuaternionTheta(diff);
	return theta * theta;
}
...
double cKinTree::CalcVelErr(const Eigen::MatrixXd& joint_mat, i32 joint_id, const Eigen::VectorXd& vel0, const Eigen::VectorXd& vel1)
{
	bool is_root = IsRoot(joint_mat, joint_id);
	double err = 0;
	if (is_root)
...
void cKinTree::CalcJointPoseDiff(const Eigen::MatrixXd& joint_mat, i32 joint_id, const Eigen::VectorXd& pose0, const Eigen::VectorXd& pose1, Eigen::VectorXd& out_diff)
{
	i32 param_offset = GetParamOffset(joint_mat, joint_id);
	i32 param_size = GetParamSize(joint_mat, joint_id);
	bool is_root = cKinTree::IsRoot(joint_mat, joint_id);
	...
	if (is_root)
	{
		...
		out_diff.segment(gPosDim, gRotDim) = cMathUtil::QuaternionToVec(root_rot_diff);
	}
	else
	{
		eJointType joint_type = GetJointType(joint_mat, joint_id);
		...
		case eJointTypeSpherical:
		{
			tQuaternion q0 = cMathUtil::VecToQuaternion(pose0.segment(param_offset, param_size));
			tQuaternion q1 = cMathUtil::VecToQuaternion(pose1.segment(param_offset, param_size));
			tQuaternion q_diff = cMathUtil::QuaternionDiff(q0, q1);
			out_diff = cMathUtil::QuaternionToVec(q_diff);
			break;
		}
		default:
...
void cKinTree::CalcJointVelDiff(const Eigen::MatrixXd& joint_mat, i32 joint_id, const Eigen::VectorXd& vel0, const Eigen::VectorXd& vel1, Eigen::VectorXd& out_diff)
{
	i32 param_offset = GetParamOffset(joint_mat, joint_id);
	i32 param_size = GetParamSize(joint_mat, joint_id);
...
void cKinTree::PostProcessPose(const Eigen::MatrixXd& joint_mat, Eigen::VectorXd& out_pose)
{
	...
	i32 num_joints = GetNumJoints(joint_mat);
	i32 root_id = GetRoot(joint_mat);
	i32 root_offset = GetParamOffset(joint_mat, root_id);
...
void cKinTree::LerpPoses(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose0, const Eigen::VectorXd& pose1, double lerp, Eigen::VectorXd& out_pose)
{
	...
	for (i32 j = 1; j < num_joints; ++j)
	{
		eJointType joint_type = GetJointType(joint_mat, j);
		i32 offset = GetParamOffset(joint_mat, j);
		i32 size = GetParamSize(joint_mat, j);
		if (joint_type == eJointTypeSpherical)
		{
			tQuaternion rot0 = cMathUtil::VecToQuaternion(pose0.segment(offset, size));
			tQuaternion rot1 = cMathUtil::VecToQuaternion(pose1.segment(offset, size));
			assert(std::abs(rot0.norm() - 1) < 0.000001);
			assert(std::abs(rot1.norm() - 1) < 0.000001);
			tQuaternion rot_lerp = rot0.slerp(lerp, rot1);
			out_pose.segment(offset, size) = cMathUtil::QuaternionToVec(rot_lerp).segment(0, size);
		}
		else
		{
...
void cKinTree::VelToPoseDiff(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose, const Eigen::VectorXd& vel, Eigen::VectorXd& out_pose_diff)
{
	...
	tQuaternion root_quat = cMathUtil::VecToQuaternion(root_diff);
	...
	for (i32 j = 1; j < num_joints; ++j)
	{
		...
		if (joint_type == eJointTypeSpherical)
		{
			tQuaternion rot = cMathUtil::VecToQuaternion(pose.segment(offset, size));
...
double cKinTree::CalcHeading(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose)
{
	...
	tVector rot_dir = cMathUtil::QuaternionRotVec(root_rot, ref_dir);
...
void cKinTree::NormalizePoseHeading(const Eigen::MatrixXd& joint_mat, Eigen::VectorXd& out_pose, Eigen::VectorXd& out_vel)
{
	...
	if (out_vel.size() > 0)
	{
		...
		root_vel = cMathUtil::QuaternionRotVec(heading_q, root_vel);
		root_ang_vel = cMathUtil::QuaternionRotVec(heading_q, root_ang_vel);
...
tMatrix cKinTree::ChildParentTransRoot(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
...
tMatrix cKinTree::ChildParentTransRevolute(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
	i32 param_offset = cKinTree::GetParamOffset(joint_mat, joint_id);
...
tMatrix cKinTree::ChildParentTransPlanar(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
	i32 param_offset = cKinTree::GetParamOffset(joint_mat, joint_id);
...
tMatrix cKinTree::ChildParentTransFixed(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
	tMatrix A = BuildAttachTrans(joint_mat, joint_id);
	tMatrix mat = A;
	return mat;
...
tMatrix cKinTree::ChildParentTransPrismatic(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
	i32 param_offset = cKinTree::GetParamOffset(joint_mat, joint_id);
	tVector offset = tVector::Zero();
	offset[0] = state(param_offset);
...
tMatrix cKinTree::ChildParentTransSpherical(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, i32 joint_id)
{
	i32 param_offset = cKinTree::GetParamOffset(joint_mat, joint_id);
	i32 param_size = cKinTree::GetParamSize(joint_mat, joint_id);
	tQuaternion q = cMathUtil::VecToQuaternion(state.segment(param_offset, param_size));
