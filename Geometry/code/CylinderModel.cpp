...
std::shared_ptr<CylinderModel> CylinderModel::Create(const Node &node) {
	...
	auto result = std::make_shared<CylinderModel>(0.0f, 0.0f);
...
std::shared_ptr<CylinderModel> CylinderModel::Create(float radiusBase, float radiusTop, float height, uint32_t slices, uint32_t stacks) {
	CylinderModel temp(radiusBase, radiusTop, height, slices, stacks, false);
	Node node;
	node << temp;
	return Create(node);
}

CylinderModel::CylinderModel(float radiusBase, float radiusTop, float height, uint32_t slices, uint32_t stacks, bool load) :
...
const Node &operator>>(const Node &node, CylinderModel &model) {
...
Node &operator<<(Node &node, const CylinderModel &model) {
