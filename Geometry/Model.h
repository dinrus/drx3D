...
public:
	...
	using TRegistryMapNode = std::unordered_map<STxt, TCreateMethodNode>;

	using TCreateMethodFilename = std::function<TCreateReturn(const std::filesystem::path &)>;
	using TRegistryMapFilename = std::unordered_map<STxt, TCreateMethodFilename>;
	...
	static TCreateReturn Create(const Node &node) {
		auto typeName = node["type"].Get<STxt>();
	...
	/**
	 * A class used to help DoRegister subclasses of Model to the factory.
	...
	protected:
		template<i32 Dummy = 0>
		static bool Register(const STxt &typeName) {
			name = typeName;
		...
		template<i32 Dummy = 0>
		static bool Register(const STxt &typeName, const STxt &extension) {
			Register(typeName);
 ...
 */
class DRX3D_EXPORT Model : public ModelFactory<Model>, public Resource {
public:
	/**
...
void Model::SetVertices(const std::vector<T> &vertices) {
	...
	Buffer vertexStaging(sizeof(T) * vertices.size(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, (unsigned)VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | (unsigned)VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
		vertices.data());
	vertexBuffer = std::make_unique<Buffer>(vertexStaging.GetSize(), (unsigned) VK_BUFFER_USAGE_VERTEX_BUFFER_BIT |(unsigned) VK_IMAGE_USAGE_TRANSFER_SRC_BIT |(unsigned) VK_BUFFER_USAGE_TRANSFER_DST_BIT,
...
void Model::Initialize(const std::vector<T> &vertices, const std::vector<uint32_t> &indices) {
	...
	for (const auto &vertex : vertices) {
		...
		minExtents = minExtents.d3Min(position);
		maxExtents = maxExtents.d3Max(position);
	}
