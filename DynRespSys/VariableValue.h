...
/************************************************************************
   ...
   A DynamicResponseVariableValue is the actual Value of a Variable, it can be of type string/i32/float. All values are converted and stored in a i32

************************************************************************/

#pragma once

//in this implementation we use a (some may call it hacky) way to store everything as a i32. For strings we simply store the generated hash value, and for floats we multiply them with 100 and store that result as a i32
...
public:
	...
	CVariableValue(i32 value) { m_value = value; SetTypeInfoOfValue(eDRVT_Int); }
	CVariableValue(bool value) { m_value = (value) ? 1 : 0; SetTypeInfoOfValue(eDRVT_Boolean); }
	CVariableValue(float value) { m_value = (i32)(value * 100.0f); SetTypeInfoOfValue(eDRVT_Float); }   //do we want/need to roundf this value?
...
private:
	VariableValueData m_value;    //holds the converted float or the string-hash or the actual i32 value
