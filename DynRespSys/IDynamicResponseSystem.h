...
typedef DrxStackStringT<char, 64>                                   ValuesString;
...
typedef i32                                                         SignalInstanceId;
static const SignalInstanceId s_InvalidSignalId = -1;

typedef i32 LipSyncID;
...
struct ISpeakerUpr
{
	...
	virtual bool CancelSpeaking(const IResponseActor* pActor, i32 maxPrioToCancel = -1, const CHashedString& lineID = CHashedString::GetEmpty(), bool bCancelQueuedLines = true) = 0;
...
struct IVariable
{
	...
	virtual void                 SetValue(i32 newValue) = 0;
	...
	//So if you set the value to 1(i32) and then call GetValueAsFloat you wont get 1.0f! So avoid changing the type of variables
	virtual i32           GetValueAsInt() const = 0;
...
struct IVariableCollection
{
	 ...
	 *
	 * Note: CreateVariable("VariableName", i32/float/string InitialValue)
	 * @param name - unique name of the new variable to be generated. Must not exist in the collection before
	 * @param initial value - the initial value (of type i32, float or tukk ) of the newly generated variable. the string pointer can be released after calling this function.
	 ...
	 */
	virtual IVariable* CreateVariable(const CHashedString& name, i32 initialValue) = 0;
	 ...
	 * @param resetTime - defines the time, after which the setVal is changed backed to the original value. Specify 0.0f (or less) to set it permanently. Can be used as cooldowns to prevent spaming of responses.
	 * @return Returns if the Set operation was successful.
	 * @see CreateVariable, SetVariableValue (by Variable-Pointer), GetVariable
	 */
	virtual bool SetVariableValue(const CHashedString& name, i32 newValue, bool createIfNotExisting = true, float resetTime = -1.0f) = 0;
	...
	virtual void        SetUserString(tukk szUserString) = 0;
	//! Returns the additional user data string
	virtual tukk GetUserString() const = 0;
...
private:
	...
	bool       SetVariableValue(const CHashedString& name, tukk newValue, bool createIfNotExisting = true, float resetTime = -1.0f) { return false; }
	IVariable* CreateVariable(const CHashedString& name, tukk initialValue)                                                         { return nullptr; }
};
...
struct IResponseInstance
{
	 ...
	 * @see DRS.IResponseUpr.AddListener
	 */
...
struct IDynamicResponseSystemEngineModule : public Drx::IDefaultModule
{
	DRXINTERFACE_DECLARE_GUID(IDynamicResponseSystemEngineModule, "a7c12111-e4d6-413e-afd1-bf5930dd8c6a"_drx_guid);
};
...
public:
	 ...
	 */
	virtual IResponseActor* CreateResponseActor(tukk szActorName, EntityId entityID = INVALID_ENTITYID, tukk szGlobalVariableCollectionToUse = nullptr) = 0;
	 ...
	 * @param resetHint - a hint what to reset. For example you could decide to only reset variables instead of destroy them.
	 */
	virtual void Reset(u32 resetHint = (u32)eResetHint_All) = 0;
	...
	//! Gets a pointer to the IResponseUpr
	//! \return A pointer to the IResponseUpr
	virtual IResponseUpr* GetResponseUpr() const = 0;

	//! Gets a pointer to the ISpeakerUpr
	//! \return A pointer to the ISpeakerUpr
	virtual ISpeakerUpr* GetSpeakerUpr() const = 0;
	...
	virtual ValuesListPtr GetCurrentState(u32 saveHints = IDynamicResponseSystem::SaveHints_Variables | IDynamicResponseSystem::SaveHints_SkipDefaultValues) const = 0;
	...
	virtual void        SetCurrentDrsUserName(tukk szNewDrsUserName) {}
	virtual tukk GetCurrentDrsUserName() const                       { return "unknown"; }
};
...
struct IResponseUpr
{
	...
	virtual ~IResponseUpr() = default;

	/**
	 * will register the given class (derived from DRS.IResponseUpr.IListener) as a listener to signal-processing. If a signalInstanceId is provided, only callbacks for that specific instance are sent
	 * @return returns true if successful
	 * @see DRS.IResponseUpr.IListener, DrxDRS.CResponseActor.QueueSignal
	 */
	 ...
	 * @return returns true if successful
	 * @see DRS.IResponseUpr.IListener
	 */
	virtual bool RemoveListener(IListener* pListenerToRemove) = 0;

	//////////////////////////////////////////////////////////////////////////
	//mainly for the UI
	virtual DynArray<tukk > GetRecentSignals(eSignalFilter filter = eSF_All) = 0;
	...
	virtual void                  SerializeRecentResponse(Serialization::IArchive& ar, const DynArray<stack_string>& signalNames, i32 maxElemets = -1) = 0;
	virtual void                  SerializeVariableChanges(Serialization::IArchive& ar, const stack_string& variableName = "ALL", i32 maxElemets = -1) = 0;
...
struct IResponseActor
{
	...
	// with this method you can specify an aux proxy that should be used by the DRS to use for audio playback (if not called or called with DrxAudio::InvalidAuxObjectId, then the Drs::SpeakerUpr will create it`s own aux-proxy)
	virtual void SetAuxAudioObjectID(DrxAudio::AuxObjectId overrideAuxProxy) = 0;
	virtual DrxAudio::AuxObjectId GetAuxAudioObjectID() const = 0;
	 ...
	 */
	virtual SignalInstanceId QueueSignal(const CHashedString& signalName, IVariableCollectionSharedPtr pSignalContext = nullptr, IResponseUpr::IListener* pSignalListener = nullptr) = 0;
};
//////////////////////////////////////////////////////////////////////////
...
{
	SCurrentDrsUserScopeHelper(tukk szCurrentSource) { SET_DRS_USER(szCurrentSource); }
...
struct IDialogLineSet
{
	...
	virtual IDialogLine*  GetLineByIndex(u32 index) = 0;
	virtual IDialogLine*  InsertLine(u32 index = -1) = 0;
	virtual bool          RemoveLine(u32 index) = 0;
};
...
struct IDialogLineDatabase
{
	...
	virtual bool                  Save(tukk szFilePath) = 0;
	virtual IDialogLineSet*       GetLineSetById(const CHashedString& lineID) = 0;
	virtual void                  Serialize(Serialization::IArchive& ar) = 0;

	//mainly for the editor
	virtual u32                GetLineSetCount() const = 0;
	virtual IDialogLineSet*       GetLineSetByIndex(u32 index) = 0;
	virtual IDialogLineSet*       InsertLineSet(u32 index = -1) = 0;
	virtual bool                  RemoveLineSet(u32 index) = 0;
	virtual bool                  ExecuteScript(u32 index) = 0;
...
struct IDataImportHelper
{
	...
	typedef IConditionSharedPtr (*      CondtionCreatorFct)(const string&, tukk szFormatName);
	typedef IResponseActionSharedPtr (* ActionCreatorFct)(const string&, tukk szFormatName);

	static u32k INVALID_ID = -1;
	typedef u32 ResponseID;
	typedef u32 ResponseSegmentID;
	...
	virtual IResponseActionSharedPtr CreateActionFromString(const CHashedString& type, const string& data, tukk szFormatName) = 0;
	virtual IConditionSharedPtr      CreateConditionFromString(const CHashedString& type, const string& data, tukk szFormatName) = 0;
...
static tukk GetTypeHelperFct(const DRS::IEditorObject& pObject)
{
