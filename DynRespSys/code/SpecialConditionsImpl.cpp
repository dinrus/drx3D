...
{
	return drx_random(0, 100) < m_randomFactor;
}
...
//--------------------------------------------------------------------------------------------------
void CGameTokenCondition::GetMemoryUsage(class IDrxSizer* pSizer) const
{
	pSizer->Add(*this);
}
...
bool CGameTokenCondition::IsMet(DRS::IResponseInstance* pResponseInstance)
{
	if (!m_pCachedToken)
	{
		...
		if (!m_pCachedToken)
		{
			DrxWarning(VALIDATOR_MODULE_DRS, VALIDATOR_ERROR, "DRS: Could not find GameToken for condition-check, token-name: '%s'", m_tokenName.c_str());
			return false;
		}
		else
		{
	...
	default:
		{
			DrxWarning(VALIDATOR_MODULE_DRS, VALIDATOR_ERROR, "Only Int/Float/String and Bool GameTokens can be used in DRS-Conditions. TokenName: %s", m_tokenName.c_str());
			return false;
		}
	}
...
struct SDepthCounterHelper
{
	...
	static i32 s_depthCounter;
};
i32 SDepthCounterHelper::s_depthCounter = 0;
...
bool CInheritConditionsCondition::IsMet(DRS::IResponseInstance* pResponseInstance)
{
	...
	{
		m_pCachedResponse = CResponseSystem::GetInstance()->GetResponseUpr()->GetResponse(m_responseToReuse);
	}
	if (m_pCachedResponse)
	{
...
void CInheritConditionsCondition::Serialize(Serialization::IArchive& ar)
{
	...
	if (ar.isEdit())
	{
		...
		else if (!CResponseSystem::GetInstance()->GetResponseUpr()->HasMappingForSignal(m_responseToReuse))
		{
...
string CTimeSinceCondition::GetVerboseInfo() const
{
	...
	{
		return GetVariableVerboseName() + string(" more than ") + DrxStringUtils::toString(m_minTime) + " seconds away from current time: "
		       + DrxStringUtils::toString(CResponseSystem::GetInstance()->GetCurrentDrsTime()) + " and less then " + DrxStringUtils::toString(m_maxTime);
	}
	else
	{
		return GetVariableVerboseName() + string(" more than ") + DrxStringUtils::toString(m_minTime) + " seconds and less then " + DrxStringUtils::toString(m_maxTime);
	}
}
...
bool CExecutionLimitCondition::IsMet(DRS::IResponseInstance* pResponseInstance)
{
	...
	{
		ResponsePtr pOverrideResponse = CResponseSystem::GetInstance()->GetResponseUpr()->GetResponse(m_ResponseToTest);
	...
	if (pResponse)
	{
		u32k executionCounter = pResponse->GetExecutionCounter();
...
{
	return string("min: ") + DrxStringUtils::toString(m_minExecutions) + string(" max: ") + DrxStringUtils::toString(m_maxExecutions);
}
...
bool CTimeSinceResponseCondition::IsMet(DRS::IResponseInstance* pResponseInstance)
{
	...
	{
		ResponsePtr pOverrideResponse = CResponseSystem::GetInstance()->GetResponseUpr()->GetResponse(m_responseId);
		if (pOverrideResponse)
		{
			pResponse = pOverrideResponse.get();
		}
...
string CTimeSinceResponseCondition::GetVerboseInfo() const
{
	...
	return "Execution of '" + responseName + string("' more than ") + DrxStringUtils::toString(m_minTime) + " seconds and less then " + DrxStringUtils::toString(m_maxTime);
}
