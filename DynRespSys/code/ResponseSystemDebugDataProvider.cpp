...
//////////////////////////////////////////////////////////////////////////
tukk GetStringFromEnum(CResponseSystemDebugDataProvider::EStatus valueToConvert)
{
	...
	return DrxStringUtils::toString(static_cast<i32>(valueToConvert));
}
...
CResponseSystemDebugDataProvider::~CResponseSystemDebugDataProvider()
{
	...
	{
		pResponseSystem->GetSpeakerUpr()->RemoveListener(this);
	}
...
{
	if (drx_strncmp(collectionName.c_str(), "Context") == 0 || drx_strncmp(variableName.c_str(), "CurrentTime") == 0)
	{
	...
	{
		DrxLogAlways("<DRS> <%.3f> Variable Change: '%s' '%s'", CResponseSystem::GetInstance()->GetCurrentDrsTime(), newEntry.variableName.c_str(), newEntry.change.c_str());
	}
}
...
void CResponseSystemDebugDataProvider::AddResponseStarted(const string& signalName)
{
	...
	{
		DrxLogAlways("<DRS> <%.3f> Response started: '%s'", CResponseSystem::GetInstance()->GetCurrentDrsTime(), signalName.c_str());
	}

	const size_t numExecutedResponses = m_executedResponses.size();

	for (i32 i = 0; i < numExecutedResponses; i++)
	{
...
bool CResponseSystemDebugDataProvider::AddResponseSegmentEvaluated(CResponseSegment* pResponseSegment)
{
	...
	if (m_loggingOptions & eDrsLoggingOptions_Responses)
	{
		DrxLogAlways("<DRS> <%.3f> ResponseSegment started: '%s'", CResponseSystem::GetInstance()->GetCurrentDrsTime(), pResponseSegment->GetName());
	}
...
bool CResponseSystemDebugDataProvider::AddConditionChecked(const CConditionsCollection::SConditionInfo* pCondition, bool result)
{
	...
	{
		DrxLogAlways("<DRS> <%.3f> Condition checked: '%s', Met: %i", CResponseSystem::GetInstance()->GetCurrentDrsTime(), newCondition.conditionDesc.c_str(), (i32)newCondition.bMet);
	}
...
bool CResponseSystemDebugDataProvider::AddActionStarted(const string& actionDesc, DRS::IResponseActionInstance* pInstance, DRS::IResponseActor* pActor, CResponseSegment* pResponseSegment)
{
	...
	for (SStartedResponsesSegment& currentSegment : response.responseSegments)
	{
		if (pResponseSegment == currentSegment.pResponseSegment)
		{
			...
			{
				DrxLogAlways("<DRS> <%.3f> Action started: '%s', Actor '%s'", CResponseSystem::GetInstance()->GetCurrentDrsTime(), newAction.actionDesc.c_str(), newAction.actorName.c_str());
			}
			currentSegment.bRunning = true;
			currentSegment.bStarted = true;
	...
	for (SStartedResponsesSegment& currentSegment : response.responseSegments)
	{
		for (SExecutedAction& currentAction : currentSegment.executedAction)
		{
			if (currentAction.pInstance == pInstance && !currentAction.bEnded)
			{
				...
				if (m_loggingOptions & eDrsLoggingOptions_Actions)
				{
					DrxLogAlways("<DRS> <%.3f> Action finished: '%s', Source: '%s'", CResponseSystem::GetInstance()->GetCurrentDrsTime(), currentAction.actionDesc.c_str(), currentSegment.segmentName.c_str());
				}
				break;
			}
		}
...
bool CResponseSystemDebugDataProvider::AddResponseInstanceFinished(EStatus reason)
{
	...
	if (m_loggingOptions & eDrsLoggingOptions_Responses)
	{
		DrxLogAlways("<DRS> <%.3f> Response finished: '%s', Reason '%s'", CResponseSystem::GetInstance()->GetCurrentDrsTime(), response.signalName.c_str(), GetStringFromEnum(reason));
	}
...
void CResponseSystemDebugDataProvider::SStartedResponses::Serialize(Serialization::IArchive& ar)
{
	if (ar.openBlock("signalInfos2", "!+ "))
	{
		...
		ar(DrxStringUtils::toString(numStartedResponses), "responses", "!^^ StartedResponses:");
		ar.closeBlock();
	}
	...
	for (i32 i = 0; i < currentBlockDepth; i++)
	{
		ar.closeBlock();
		ar.closeBlock();
	}
...
{
	CResponseSystem::GetInstance()->GetSpeakerUpr()->AddListener(this);
}
...
void CResponseSystemDebugDataProvider::AddSignalFired(const string& signalName, const string& senderName, const string& contextVariables)
{
	...
	{
		DrxLogAlways("<DRS> <%.3f> Signal fired: '%s', sender '%s', '%s'", CResponseSystem::GetInstance()->GetCurrentDrsTime(), signalName.c_str(), senderName.c_str(), contextVariables.c_str());
	}
}
...
void CResponseSystemDebugDataProvider::AddDialogLineFinished(const CHashedString& lineID, const string& speakerName, EStatus status, const string& reason)
{
	...
	{
		DrxLogAlways("<DRS> <%.3f> Line Finished: Speaker: '%s' ID: '%s' State: %s Reason: %s", CResponseSystem::GetInstance()->GetCurrentDrsTime(), speakerName.c_str(), lineID.GetText().c_str(), GetStringFromEnum(status), reason.c_str());
	}
}
...
void CResponseSystemDebugDataProvider::AddDialogLineStarted(const CHashedString& lineID, const string& lineText, const string& speakerName, EStatus status, const string& reason)
{
	...
	if (m_loggingOptions & eDrsLoggingOptions_Lines)
	{
		DrxLogAlways("<DRS> <%.3f> Line Start Requested: Speaker: '%s' ID: '%s' State: '%s'. Reason: '%s' (Text: \"%s\") Source: '%s'", CResponseSystem::GetInstance()->GetCurrentDrsTime(), speakerName.c_str(), lineID.GetText().c_str(), GetStringFromEnum(status), temp.description.c_str(), lineText.c_str(), temp.source.c_str());
	}
}

//////////////////////////////////////////////////////////////////////////
void CResponseSystemDebugDataProvider::GetRecentSignals(DynArray<tukk >& outResults, DRS::IResponseUpr::eSignalFilter filter /* = eSF_All */)
{
	for (StartedResponsesList::const_iterator it = m_executedResponses.begin(); it != m_executedResponses.end(); ++it)
	{
		if (filter == DRS::IResponseUpr::eSF_All ||
		    (filter == DRS::IResponseUpr::eSF_OnlyWithoutResponses && it->currentState == eER_NoResponse) ||
		    (filter == DRS::IResponseUpr::eSF_OnlyWithResponses && it->currentState != eER_NoResponse))
		{
...
//////////////////////////////////////////////////////////////////////////
void CResponseSystemDebugDataProvider::SerializeRecentResponse(Serialization::IArchive& ar, const DynArray<stack_string>& signalNames, i32 maxElemets)
{
	if (ar.getFilter() != 0)
	{
...
//////////////////////////////////////////////////////////////////////////
void CResponseSystemDebugDataProvider::SerializeVariableChanges(Serialization::IArchive& ar, const string& variableName, i32 maxElemets)
{
