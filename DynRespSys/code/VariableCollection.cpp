...
#include <drx3D/DynRespSys/Variable.h>
...
CVariable* CVariableCollection::CreateVariable(const CHashedString& name, const CVariableValue& initialValue)
{
	...
	DRX_ASSERT_MESSAGE(GetVariable(name) == nullptr, "Variable with this name already exists");
#endif
	if (!name.IsValid())
		return nullptr;
...
//--------------------------------------------------------------------------------------------------
CVariable* CVariableCollection::CreateVariable(const CHashedString& name, i32 initialValue)
{
...
//--------------------------------------------------------------------------------------------------
bool CVariableCollection::SetVariableValue(const CHashedString& name, i32 newValue, bool createIfNotExisting /*= true*/, float resetTime /*= -1.0f*/)
{
...
void CVariableCollection::Serialize(Serialization::IArchive& ar)
{
	...
	if (ar.isInput())
	{
		...
		for (const DrxDRS::CVariable& variable : variablesCopy)
		{
...
//--------------------------------------------------------------------------------------------------
CVariableCollectionUpr::~CVariableCollectionUpr()
{
...
//--------------------------------------------------------------------------------------------------
CVariableCollection* CVariableCollectionUpr::CreateVariableCollection(const CHashedString& collectionName)
{
...
//--------------------------------------------------------------------------------------------------
void CVariableCollectionUpr::ReleaseVariableCollection(CVariableCollection* pCollectionToFree)
{
...
//--------------------------------------------------------------------------------------------------
CVariableCollection* CVariableCollectionUpr::GetCollection(const CHashedString& collectionName) const
{
...
//--------------------------------------------------------------------------------------------------
void CVariableCollectionUpr::Update()
{
	for (CVariableCollection* const pCollection : m_variableCollections)
	{
		pCollection->Update();
...
//--------------------------------------------------------------------------------------------------
void CVariableCollectionUpr::Reset()
{
	for (CVariableCollection* const pCollection : m_variableCollections)
	{
		pCollection->Reset();
...
//--------------------------------------------------------------------------------------------------
void CVariableCollectionUpr::Serialize(Serialization::IArchive& ar)
{
...
//--------------------------------------------------------------------------------------------------
void CVariableCollectionUpr::GetAllVariableCollections(DRS::ValuesList* pOutCollectionsList, bool bSkipDefaultValues)
{
	DRX_ASSERT(pOutCollectionsList);
	...
	for (CVariableCollection* const pCollection : m_variableCollections)
	{
		...
		for (DrxDRS::CVariable* const pVariable : variables)
		{
			...
			for (const DrxDRS::CVariableCollection::VariableCooldownInfo& cooldownInfo : coolingDownVariables)
			{
...
//--------------------------------------------------------------------------------------------------
void CVariableCollectionUpr::SetAllVariableCollections(DRS::ValuesListIterator start, DRS::ValuesListIterator end)
{
	...
	for (DRS::ValuesListIterator it = start; it != end; ++it)
	{
		...
		DrxDRS::CVariable* pVariable = pCollection->CreateOrGetVariable(variableName);
