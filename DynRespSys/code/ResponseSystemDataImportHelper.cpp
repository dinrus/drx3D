...
#define CHECK_CORRECT_FORMAT                                                                                                                                                                                       \
  if (strcmp(szFormatName, "DrxDrsInternal") != 0)                                                                                                                                                                 \
  {                                                                                                                                                                                                                \
    DrsLogWarning((string("Could not create Action from string with the given Format '") + string(szFormatName) + "', only the Format 'DrxDrsInternal' is supported. CreationData was: " + creationData).c_str()); \
    return nullptr;                                                                                                                                                                                                \
  }

//--------------------------------------------------------------------------------------------------
DRS::IResponseActionSharedPtr CreatorCancelSignalAction(const string& creationData, tukk szFormatName)
{
	CHECK_CORRECT_FORMAT
	return DRS::IResponseActionSharedPtr(new CActionCancelSignal(creationData));
}
//--------------------------------------------------------------------------------------------------
DRS::IResponseActionSharedPtr CreatorSendSignalAction(const string& creationData, tukk szFormatName)
{
	CHECK_CORRECT_FORMAT
	return DRS::IResponseActionSharedPtr(new CActionSendSignal(creationData, true));
}
//--------------------------------------------------------------------------------------------------
DRS::IResponseActionSharedPtr CreatorSetActorAction(const string& creationData, tukk szFormatName)
{
	CHECK_CORRECT_FORMAT
	return DRS::IResponseActionSharedPtr(new CActionSetActor(creationData, nullptr));
}
//--------------------------------------------------------------------------------------------------
DRS::IResponseActionSharedPtr CreatorWaitAction(const string& creationData, tukk szFormatName)
{
	CHECK_CORRECT_FORMAT
	return DRS::IResponseActionSharedPtr(new CActionWait((float)atof(creationData.c_str())));
}
//--------------------------------------------------------------------------------------------------
DRS::IResponseActionSharedPtr CreateSpeakLineAction(const string& creationData, tukk szFormatName)
{
	CHECK_CORRECT_FORMAT
	return DRS::IResponseActionSharedPtr(new CActionSpeakLine(creationData));
}
//--------------------------------------------------------------------------------------------------
DRS::IResponseActionSharedPtr CreatorSetVariableAction(const string& creationData, tukk szFormatName)
{
	CHECK_CORRECT_FORMAT
...
//--------------------------------------------------------------------------------------------------
DRS::IConditionSharedPtr CreatorVariableCondition(const string& creationData, tukk szFormatName)
{
	CHECK_CORRECT_FORMAT
...
//--------------------------------------------------------------------------------------------------
DRS::IConditionSharedPtr CreatorRandomCondition(const string& creationData, tukk szFormatName)
{
	CHECK_CORRECT_FORMAT
...
{
	CHashedString signalName((u32)responseID);
	CResponseUpr::MappedSignals::iterator itFound = m_mappedSignals.find(signalName);
...
{
	CHashedString signalName((u32)parentResponse);
	CResponseUpr::MappedSignals::iterator itFound = m_mappedSignals.find(signalName);
	if (itFound == m_mappedSignals.end())
		return INVALID_ID;
	ResponsePtr& response = itFound->second;
...
//--------------------------------------------------------------------------------------------------
DRS::IResponseActionSharedPtr CDataImportHelper::CreateActionFromString(const CHashedString& type, const string& data, tukk szFormatName)
{
...
//--------------------------------------------------------------------------------------------------
DRS::IConditionSharedPtr CDataImportHelper::CreateConditionFromString(const CHashedString& type, const string& data, tukk szFormatName)
{
...
bool CDataImportHelper::splitStringList(const string& stringToSplitUp, const char delimeter1, const char delimeter2, std::vector<string>* outResult, bool bTrim)
{
	...
	i32k stringLength = stringToSplitUp.size();
	if (stringLength > 1)
	{
		tukk szCurrentPos = stringToSplitUp.c_str();
		tukk szCurrentPosStart = stringToSplitUp.c_str();
