...
void CResponseInstance::ExecuteSegment(CResponseSegment* pSegment)
{
	...
	if (pSegment)
	{
		...
		for (CResponseSegment::ActionsInfo& actionInfo : pSegment->GetActions())
		{
			...
			{
				DRX_ASSERT(actionInfo.m_pAction);
				...
				if (actionInfo.m_delay > FLT_EPSILON)
				{
					...
					DRS_DEBUG_DATA_ACTION(AddActionStarted(actionInfo.m_pAction->GetVerboseInfoWithType() + ", delay: " + DrxStringUtils::toString(actionInfo.m_delay) + "s ", pAction.get(), GetCurrentActor(), pSegment));
				}
				else
				{
