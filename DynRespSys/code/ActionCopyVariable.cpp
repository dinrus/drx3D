...
{	
	...
	if (CVariable* pSourceVar = m_sourceVariable.GetCurrentVariable(pInstanceImpl))
	{
		...
		{
			pTargetCollection = CResponseSystem::GetInstance()->GetVariableCollectionUpr()->CreateVariableCollection(m_targetVariable.m_collectionName);
		}
...
void CActionCopyVariable::Serialize(Serialization::IArchive& ar)
{
	...
	if (ar.isEdit())
	{
		...
		else if (ar.isOutput())
		{
			...
			{
				const CVariableCollectionUpr* pVariableCollectionMgr = CResponseSystem::GetInstance()->GetVariableCollectionUpr();
