...
void CActionSetGameToken::Serialize(Serialization::IArchive& ar)
{
	...
	if (ar.isEdit() && ar.isOutput())
	{
		...
		{
			CVariableCollection* pCollection = CResponseSystem::GetInstance()->GetVariableCollectionUpr()->GetCollection(m_collectionName);
			if (pCollection)
			{
				DrxDRS::CVariable* pVariable = pCollection->GetVariable(m_variableName);
				if (pVariable)
				{
	...
	case CActionSetGameToken::EValueToSet::BoolVariableValue:
		{
			...
			if (pCollection)
			{
				if (DrxDRS::CVariable* pVariable = pCollection->CreateOrGetVariable(m_variableName))
				{
...
//--------------------------------------------------------------------------------------------------
string DrxDRS::CActionSetGameToken::GetVerboseInfo() const
{
	switch (m_typeToSet)
	{
	case CActionSetGameToken::EValueToSet::IntVariableValue:
