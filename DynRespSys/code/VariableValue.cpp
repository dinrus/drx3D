...
#include <drx3D/DynRespSys/VariableValue.h>
...
SERIALIZATION_ENUM(eDRVT_Int, "i32", "Int")
...
{
	m_value = static_cast<i32>(value.GetHash());
#if defined(ENABLE_VARIABLE_VALUE_TYPE_CHECKINGS)
...
//--------------------------------------------------------------------------------------------------
i32 CVariableValue::GetValueAsInt() const
{
#if defined(ENABLE_VARIABLE_VALUE_TYPE_CHECKINGS)
	if (m_type != eDRVT_Int)
	{
...
CHashedString CVariableValue::GetValueAsHashedString() const
{
	...
	return m_hashedText;
#endif
	return CHashedString((u32)m_value);
}

//--------------------------------------------------------------------------------------------------
tukk CVariableValue::GetTypeAsString() const
{
#if defined(ENABLE_VARIABLE_VALUE_TYPE_CHECKINGS)
	switch (m_type)
	{
...
string CVariableValue::GetValueAsString() const //should be used for debug output only
{
	...
	{
	case eDRVT_Int:
		return DrxStringUtils::toString(m_value);
	case eDRVT_Float:
		return DrxStringUtils::toString(m_value / 100.0f);
	case eDRVT_String:
		return m_hashedText;
	case eDRVT_Boolean:
				...
				case eDRVT_Float:
					{
						float temp;
						ar(temp, "value", "^");
						m_value = (i32)(temp * 100.0f); // CVariableValue(temp).GetValue();
						break;
					}
				case eDRVT_PosInfinite:
				case eDRVT_NegInfinite:
