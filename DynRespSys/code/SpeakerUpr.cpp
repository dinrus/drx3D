...
#include <drx3D/DynRespSys/SpeakerUpr.h>
...
namespace
{
static u32k s_attachmentPosName = CCrc32::ComputeLowercase("voice");
static u32k s_attachmentPosNameFallback = CCrc32::ComputeLowercase("eye_left");
...
static i32k s_characterSlot = 0;                        //todo: make this a property of the drs actor
...
float CSpeakerUpr::s_defaultPauseAfterLines = 0.2f;

//--------------------------------------------------------------------------------------------------
CSpeakerUpr::CSpeakerUpr() : m_listeners(2)
{
  ...
  DrxAudio::ESystemEvents events = DrxAudio::ESystemEvents::TriggerExecuted | DrxAudio::ESystemEvents::TriggerFinished | DrxAudio::ESystemEvents::FilePlay | DrxAudio::ESystemEvents::FileStarted | DrxAudio::ESystemEvents::FileStopped;
	gEnv->pAudioSystem->AddRequestListener(&CSpeakerUpr::OnAudioCallback, this, events);

	m_audioParameterIdGlobal = DrxAudio::InvalidControlId;
	m_audioParameterIdLocal = DrxAudio::InvalidControlId;
...
//--------------------------------------------------------------------------------------------------
CSpeakerUpr::~CSpeakerUpr()
{
	Shutdown();
}

//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::Init()
{
	...
	{
		m_audioParameterIdLocal = DrxAudio::StringToId(m_pDrsDialogDialogRunningEntityParameterName->GetString());
	}
	if (m_pDrsDialogDialogRunningGlobalParameterName)
	{
		m_audioParameterIdGlobal = DrxAudio::StringToId(m_pDrsDialogDialogRunningGlobalParameterName->GetString());
	}
}

//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::Update()
{
	SET_DRS_USER_SCOPED("SpeakerUpr Update");
	...
	for (SSpeakInfo& currentSpeaker : m_activeSpeakers)
	{
		...
		else if (gEnv->pEntitySystem && gEnv->pRenderer)
		{
			...
			if (pActor)
			{
				...
				if (pEntity)
				{
					if (currentSpeaker.speechAuxObjectId != pActor->GetAuxAudioObjectID() && currentSpeaker.speechAuxObjectId != DrxAudio::DefaultAuxObjectId)
					{
				...
				{
					DrxWarning(VALIDATOR_MODULE_DRS, VALIDATOR_WARNING, "An DRS-actor without an Entity detected: LineText: '%s', Actor: '%s'\n", (!currentSpeaker.text.empty()) ? currentSpeaker.text.c_str() : "no text", pActor->GetName().c_str());
				}
			}
			else
			{
				DrxWarning(VALIDATOR_MODULE_DRS, VALIDATOR_WARNING, "An active speaker without a DRS-Actor detected: LineText: '%s' \n", (!currentSpeaker.text.empty()) ? currentSpeaker.text.c_str() : "no text");
			}
		}
	}
...
//--------------------------------------------------------------------------------------------------
bool CSpeakerUpr::IsSpeaking(const DRS::IResponseActor* pActor, const CHashedString& lineID /* = CHashedString::GetEmpty() */, bool bCheckQueuedLinesAsWell /* = false */) const
{
...
//--------------------------------------------------------------------------------------------------
DRS::ISpeakerUpr::IListener::eLineEvent CSpeakerUpr::StartSpeaking(DRS::IResponseActor* pIActor, const CHashedString& lineID)
{
	if (!pIActor)
	{
		InformListener(pIActor, lineID, IListener::eLineEvent_SkippedBecauseOfFaultyData, nullptr);
		DrxWarning(VALIDATOR_MODULE_DRS, VALIDATOR_WARNING, "StartSpeaking was called without a valid DRS Actor.");
	...
	if (!pEntity)
	{
		...
		DrxWarning(VALIDATOR_MODULE_DRS, VALIDATOR_WARNING, "StartSpeaking was called with a DRS Actor that has no Entity assigned to it.");
		return IListener::eLineEvent_SkippedBecauseOfFaultyData;
	}
	...
	i32k priority = (pLineSet) ? pLineSet->GetPriority() : 50; // 50 = default priority
	...
	for (SSpeakInfo& activateSpeaker : m_activeSpeakers)
	{
		if (activateSpeaker.pActor == pActor)
		{
			    ...
			    || (m_samePrioCancelsLinesCVar != 0 && priority >= activateSpeaker.priority && lineID != activateSpeaker.lineID)) //if the new line is not less important, stop the old one
			{
				...
				if (activateSpeaker.stopTriggerID != DrxAudio::InvalidControlId && m_playAudioCVar != 0)
				{
					...
					DrxAudio::SRequestUserData const userData(DrxAudio::ERequestFlags::DoneCallbackOnExternalThread | DrxAudio::ERequestFlags::CallbackOnExternalOrCallingThread, this, (uk const)(pLine), (uk const)(activateSpeaker.pActor));
				...
				{
					if (activateSpeaker.startTriggerID != DrxAudio::InvalidControlId)
					{
						pEntityAudioProxy->StopTrigger(activateSpeaker.startTriggerID, activateSpeaker.speechAuxObjectId);
					}
	...
	return ISpeakerUpr::IListener::eLineEvent_Started;
}

//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::UpdateAudioProxyPosition(IEntity* pEntity, const SSpeakInfo& speakerInfo)
{
	...
	if (pEntityAudioProxy)
	{
		...
		if (pCharacter)
		{
			const IAttachmentUpr* pAttachmentUpr = pCharacter->GetIAttachmentUpr();
			if (pAttachmentUpr)
			{
				const IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByIndex(speakerInfo.voiceAttachmentIndex);
				if (pAttachment)
				{
...
//--------------------------------------------------------------------------------------------------
bool CSpeakerUpr::CancelSpeaking(const DRS::IResponseActor* pActor, i32 maxPrioToCancel /* = -1 */, const CHashedString& lineID /* = CHashedString::GetEmpty() */, bool bCancelQueuedLines /* = true */)
{
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::Reset()
{
	m_queuedSpeakers.clear();

	for (SSpeakInfo& speakerInfo : m_activeSpeakers)
	{
		SET_DRS_USER_SCOPED("SpeakerUpr Reset");
		InformListener(speakerInfo.pActor, speakerInfo.lineID, IListener::eLineEvent_Canceled, speakerInfo.pPickedLine);
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::ReleaseSpeakerAudioProxy(SSpeakInfo& speakerInfo, bool stopTrigger)
{
	if ((speakerInfo.speechAuxObjectId != DrxAudio::DefaultAuxObjectId && speakerInfo.speechAuxObjectId != speakerInfo.pActor->GetAuxAudioObjectID()) || stopTrigger)
	{
		...
		if (pEntity)
		{
			...
			if (pEntityAudioProxy)
			{
				if (stopTrigger)
				{
					if (speakerInfo.startTriggerID != DrxAudio::InvalidControlId)
					{
				...
				if ((speakerInfo.speechAuxObjectId != DrxAudio::DefaultAuxObjectId && speakerInfo.speechAuxObjectId != speakerInfo.pActor->GetAuxAudioObjectID()))
				{
					pEntityAudioProxy->RemoveAudioAuxObject(speakerInfo.speechAuxObjectId);
					speakerInfo.speechAuxObjectId = DrxAudio::DefaultAuxObjectId;
				}
			}
		}
	}
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::OnAudioCallback(const DrxAudio::SRequestInfo* const pAudioRequestInfo)
{
	CSpeakerUpr* pSpeakerUpr = CResponseSystem::GetInstance()->GetSpeakerUpr();
	...
	if (pAudioRequestInfo->requestResult == DrxAudio::ERequestResult::Failure &&
	    (pAudioRequestInfo->systemEvent == DrxAudio::ESystemEvents::FilePlay ||
	     pAudioRequestInfo->systemEvent == DrxAudio::ESystemEvents::FileStarted))
	{
		//handling of failure executing the start / stop trigger or the standalone file
		for (SSpeakInfo& speakerInfo : pSpeakerUpr->m_activeSpeakers)
		{
			if (speakerInfo.pActor == pActor && pDialogLine == speakerInfo.pPickedLine)
			{
				...
				{
					i32 textLength = (pDialogLine) ? strlen(pDialogLine->GetText()) : 16;
	...
	else if (pAudioRequestInfo->systemEvent == DrxAudio::ESystemEvents::TriggerFinished ||
	         pAudioRequestInfo->systemEvent == DrxAudio::ESystemEvents::FileStopped)
	{
		for (SSpeakInfo& speakerInfo : pSpeakerUpr->m_activeSpeakers)
		{
			if (speakerInfo.pActor == pActor && pDialogLine == speakerInfo.pPickedLine)
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::OnActorRemoved(const CResponseActor* pActor)
{
	for (SpeakerList::iterator it = m_activeSpeakers.begin(); it != m_activeSpeakers.end(); )
	{
		if (it->pActor == pActor)
...
//--------------------------------------------------------------------------------------------------
bool CSpeakerUpr::AddListener(IListener* pListener)
{
	return m_listeners.Add(pListener);
}

//--------------------------------------------------------------------------------------------------
bool CSpeakerUpr::RemoveListener(IListener* pListener)
{
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::InformListener(const DRS::IResponseActor* pSpeaker, const CHashedString& lineID, IListener::eLineEvent event, const CDialogLine* pLine)
{
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::SetCustomLipsyncProvider(ILipsyncProvider* pProvider)
{
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::SetNumActiveSpeaker(i32 newAmountOfSpeaker)
{
	if (m_numActiveSpeaker != newAmountOfSpeaker)
	{
		...
		if (m_audioParameterIdGlobal != DrxAudio::InvalidControlId)
		{
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::ExecuteStartSpeaking(SSpeakInfo* pSpeakerInfoToUse)
{
	pSpeakerInfoToUse->startTriggerID = DrxAudio::InvalidControlId;
	pSpeakerInfoToUse->stopTriggerID = DrxAudio::InvalidControlId;
	...
	if (pSpeakerInfoToUse->pPickedLine)
	{
		...
		{
			pSpeakerInfoToUse->startTriggerID = DrxAudio::StringToId(pSpeakerInfoToUse->pPickedLine->GetStartAudioTrigger().c_str());
		}
		...
		{
			pSpeakerInfoToUse->stopTriggerID = DrxAudio::StringToId(pSpeakerInfoToUse->pPickedLine->GetEndAudioTrigger().c_str());
		}
	}
	else
	{
		DrxWarning(VALIDATOR_MODULE_DRS, VALIDATOR_WARNING, "Triggered an SpeakLineAction, with a line that did not exist in the database! LineID was: '%s'", pSpeakerInfoToUse->lineID.GetText().c_str());
		pSpeakerInfoToUse->text.Format("Missing: %s", pSpeakerInfoToUse->lineID.GetText().c_str());
	}

	bool bHasAudioAsset = (pSpeakerInfoToUse->startTriggerID != DrxAudio::InvalidControlId || !pSpeakerInfoToUse->standaloneFile.empty()) && m_playAudioCVar != 0;

	if (bHasAudioAsset)
	{
		IEntityAudioComponent* pEntityAudioProxy = pSpeakerInfoToUse->pEntity->GetOrCreateComponent<IEntityAudioComponent>();

		if (m_audioParameterIdLocal != DrxAudio::InvalidControlId)
		{
			pEntityAudioProxy->SetParameter(m_audioParameterIdLocal, 1.0f, DrxAudio::InvalidAuxObjectId);
		}

		//if the actor does not specify a auxProxyId to use, we are trying to create a default one
		if (pSpeakerInfoToUse->speechAuxObjectId == DrxAudio::InvalidAuxObjectId)
		{
			...
			if (pCharacter)
			{
				...
				if (pAttachmentUpr)
				{
					pSpeakerInfoToUse->voiceAttachmentIndex = pAttachmentUpr->GetIndexByNameCRC(s_attachmentPosName);
					if (pSpeakerInfoToUse->voiceAttachmentIndex == -1)
						pSpeakerInfoToUse->voiceAttachmentIndex = pAttachmentUpr->GetIndexByNameCRC(s_attachmentPosNameFallback);
					if (pSpeakerInfoToUse->voiceAttachmentIndex != -1)
					{
			...
			if (pSpeakerInfoToUse->speechAuxObjectId == DrxAudio::InvalidAuxObjectId)  //if creating an aux-proxy at the estimated head position failed, we fall back to using the default proxy
				pSpeakerInfoToUse->speechAuxObjectId = DrxAudio::DefaultAuxObjectId;
		}

		DrxAudio::SRequestUserData const userData(DrxAudio::ERequestFlags::DoneCallbackOnExternalThread | DrxAudio::ERequestFlags::CallbackOnExternalOrCallingThread, this, (uk const)(pSpeakerInfoToUse->pPickedLine), (uk const)(pSpeakerInfoToUse->pActor));
		...
		{
			bAudioPlaybackStarted = pEntityAudioProxy->PlayFile(DrxAudio::SPlayFileInfo(pSpeakerInfoToUse->standaloneFile.c_str()), pSpeakerInfoToUse->speechAuxObjectId, userData);
		}
		...
		else
		{
			...
			pSpeakerInfoToUse->startTriggerID = DrxAudio::InvalidControlId;
			pSpeakerInfoToUse->stopTriggerID = DrxAudio::InvalidControlId;
			pSpeakerInfoToUse->speechAuxObjectId = DrxAudio::InvalidAuxObjectId;
			pSpeakerInfoToUse->standaloneFile.clear();
		}
	}
	...
	SET_DRS_USER_SCOPED("SpeakerUpr Start Speaking");
	...
	float pointInFarFuture = (((i32)CResponseSystem::GetInstance()->GetCurrentDrsTime() / 5000) + 1) * 5000.0f; //we set the 'LastFinishTime' to a point in the far future, so that conditions like 'if TimeSince LastFinishTime > 3' (to detect longer pauses) will savely work.
	...
	SetNumActiveSpeaker((i32)m_activeSpeakers.size());
}

//--------------------------------------------------------------------------------------------------
bool CSpeakerUpr::OnLineAboutToStart(const DRS::IResponseActor* pSpeaker, const CHashedString& lineID)
{
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::QueueLine(CResponseActor* pActor, const CHashedString& lineID, float maxQueueDuration, i32k priority)
{
