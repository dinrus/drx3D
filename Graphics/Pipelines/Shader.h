 ...
 */
class DRX3D_EXPORT Shader {
public:
	/**
	 * A define added to the start of a shader, first value is the define name and second is
	  the value to be set.
	 */
	using Define = std::pair<STxt, STxt>;
	...
	class Uniform {
		friend class Shader;
	public:
		explicit Uniform(int32_t binding = -1, int32_t offset = -1, int32_t size = -1,
		                       int32_t glType = -1, bool readOnly = false,
		...
		bool operator==(const Uniform &rhs) const {
			return binding == rhs.binding && offset == rhs.offset && size == rhs.size
		               && glType == rhs.glType && readOnly == rhs.readOnly &&
	...
	public:
		...
		const std::map<STxt, Uniform> &GetUniforms() const { return uniforms; }

		std::optional<Uniform> GetUniform(const STxt &name) const {
	...
	private:
		...
		VkShaderStageFlags stageFlags;
		Type type;
		std::map<STxt, Uniform> uniforms;
	};

	class Attribute {
		friend class Shader;
	public:
		explicit Attribute(int32_t set = -1, int32_t location = -1, int32_t size = -1,
		                                                        int32_t glType = -1) :
			set(set),
			location(location),
			size(size),
	...
	class Constant {
		friend class Shader;
	public:
		explicit Constant(int32_t binding = -1, int32_t size = -1,
		                            VkShaderStageFlags stageFlags = 0, int32_t glType = -1) :
			binding(binding),
			size(size),
			stageFlags(stageFlags),
			glType(glType) {
	...
	bool ReportedNotFound(const STxt &name, bool reportIfFound) const;
	static VkFormat GlTypeToVk(int32_t type);
	std::optional<uint32_t> GetDescriptorLocation(const STxt &name) const;
	std::optional<uint32_t> GetDescriptorSize(const STxt &name) const;
	std::optional<Uniform> GetUniform(const STxt &name) const;
	std::optional<UniformBlock> GetUniformBlock(const STxt &name) const;
	std::optional<Attribute> GetAttribute(const STxt &name) const;
	...
	VkShaderModule CreateShaderModule(const std::filesystem::path &moduleName,
	              const STxt &moduleCode, const STxt &preamble, VkShaderStageFlags moduleFlag);
	...
	const std::map<STxt, Uniform> &GetUniforms() const { return uniforms; };
	const std::map<STxt, UniformBlock> &GetUniformBlocks() const { return uniformBlocks; };
	const std::map<STxt, Attribute> &GetAttributes() const { return attributes; };
	const std::map<STxt, Constant> &GetConstants() const { return constants; };
...
private:
	static void IncrementDescriptorPool(std::map<VkDescriptorType, uint32_t> &descriptorPoolCounts,
	...
	std::map<STxt, Uniform> uniforms;
	std::map<STxt, UniformBlock> uniformBlocks;
	std::map<STxt, Attribute> attributes;
	std::map<STxt, Constant> constants;

	std::array<std::optional<uint32_t>, 3> localSizes;

	std::map<STxt, uint32_t> descriptorLocations;
	std::map<STxt, uint32_t> descriptorSizes;
	...
	mutable std::vector<STxt> notFoundNames;
};
