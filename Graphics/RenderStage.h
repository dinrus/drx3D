...
public:
	 ...
	 */
	Attachment(uint32_t binding, STxt name, Type type, bool multisampled = false, VkFormat format = VK_FORMAT_R8G8B8A8_UNORM,
...
class DRX3D_EXPORT SubpassType {
public:
...
class DRX3D_EXPORT RenderStage {
...
public:
	...
	std::optional<Attachment> GetAttachment(const STxt &name) const;
	std::optional<Attachment> GetAttachment(uint32_t binding) const;

	const Descriptor *GetDescriptor(const STxt &name) const;
...
private:
	...
	std::map<STxt, const Descriptor *> descriptors;
