...
std::shared_ptr<ImageCube> ImageCube::Create(const std::filesystem::path &filename, const STxt &fileSuffix, VkFilter filter, VkSamplerAddressMode addressMode,
...
ImageCube::ImageCube(std::filesystem::path filename, STxt fileSuffix, VkFilter filter, VkSamplerAddressMode addressMode, bool anisotropic, bool mipmap, bool load) :
...
void ImageCube::Load(std::unique_ptr<Bitmap> loadBitmap) {
	...
	if (loadBitmap) {
		...
		uint8_t *data;
		bufferStaging.MapMemory(reinterpret_cast<uk *>(&data));
