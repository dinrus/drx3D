...
public:
	IncludeResult *includeLocal(tukk headerName, tukk includerName, size_t inclusionDepth) override {
	...
	IncludeResult *includeSystem(tukk headerName, tukk includerName, size_t inclusionDepth) override {
...
bool Shader::ReportedNotFound(const STxt &name, bool reportIfFound) const {
...
std::optional<uint32_t> Shader::GetDescriptorLocation(const STxt &name) const {
	if (auto it = descriptorLocations.find(name); it != descriptorLocations.end())
		return it->second;
	return std::nullopt;
}

std::optional<uint32_t> Shader::GetDescriptorSize(const STxt &name) const {
	if (auto it = descriptorSizes.find(name); it != descriptorSizes.end())
		return it->second;
	return std::nullopt;
}

std::optional<Shader::Uniform> Shader::GetUniform(const STxt &name) const {
	if (auto it = uniforms.find(name); it != uniforms.end())
		return it->second;
	return std::nullopt;
}

std::optional<Shader::UniformBlock> Shader::GetUniformBlock(const STxt &name) const {
	if (auto it = uniformBlocks.find(name); it != uniformBlocks.end())
		return it->second;
	return std::nullopt;
}

std::optional<Shader::Attribute> Shader::GetAttribute(const STxt &name) const {
...
VkShaderModule Shader::CreateShaderModule(const std::filesystem::path &moduleName, const STxt &moduleCode, const STxt &preamble, VkShaderStageFlags moduleFlag) {
...
void Shader::LoadUniformBlock(const glslang::TProgram &program, VkShaderStageFlags stageFlag, int32_t i) {
	...
	if (reflection.getType()->getQualifier().storage == EvqUniform)
		type = UniformBlock::Type::Uniform;
	if (reflection.getType()->getQualifier().storage == EvqBuffer)
