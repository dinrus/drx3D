...
namespace drx3d {
Buffer::Buffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, ukk data) :
	size(size) {
...
void Buffer::MapMemory(uk *data) const {
	auto logicalDevice = Graphics::Get()->GetLogicalDevice();
...
uint32_t Buffer::FindMemoryType(uint32_t typeFilter, const VkMemoryPropertyFlags &requiredProperties) {
	...
	throw drx::Exc("Failed to find a valid memory type for buffer");
}
