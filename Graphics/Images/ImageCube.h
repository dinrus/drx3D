 ...
 */
class DRX3D_EXPORT ImageCube : public Image, public Resource {
public:
	/**
	 ...
	 * @return The cubemap image with the requested values.
	 */
	static std::shared_ptr<ImageCube> Create(const std::filesystem::path &filename, const STxt &fileSuffix, VkFilter filter = VK_FILTER_LINEAR,
	 ...
	 */
	explicit ImageCube(std::filesystem::path filename, STxt fileSuffix = ".png", VkFilter filter = VK_FILTER_LINEAR,
	...
	const STxt &GetFileSuffix() const { return fileSuffix; }
	const std::vector<STxt> &GetFileSides() const { return fileSides; }
...
private:
	...
	STxt fileSuffix;
	/// X, -X, +Y, -Y, +Z, -Z
	std::vector<STxt> fileSides = {"Right", "Left", "Top", "Bottom", "Back", "Front"};
