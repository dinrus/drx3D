...
struct SVClothParams
{
	...
	i32   timeStepsMax;                 //!< Number of maximum iterations for the time discretization in a single step.
	i32   numIterations;                //!< Number of iterations for the positional stiffness & collision solver (contacts & edges).
	i32   collideEveryNthStep;          //!< For stiffness & collision solver: collide only every Nth step.
	...
	// Test Reset Damping
	i32   resetDampingRange;            //!< No of frames, within resetDampingFactor is used within simulation for dampening the system.
	...
	i32    debugDrawCloth;
	i32    debugDrawNndc;      //!< Debug Nearest Neighbor Distance Constraints (nndc).
	i32    debugPrint;
...
{
	SVClothAttachmentParams(tukk attachmentName, const SVClothParams& vclothParams, i32 skinLoadingFlags, i32 flags)
...
struct SimulationParams
{
	...
	Vec2               m_vCapsule;
	CDrxName           m_strProcFunction;

	i32              m_nProjectionType;
	CDrxName           m_strDirTransJoint;
	DynArray<CDrxName> m_arrProxyNames; //!< Test capsules/sphere joint against these colliders.
...
struct IAttachmentUpr
{
	...
	virtual ~IAttachmentUpr(){}
	virtual u32              LoadAttachmentList(tukk pathname) = 0;

	virtual IAttachment*        CreateAttachment(tukk szName, u32 type, tukk szJointName = 0, bool bCallProject = true) = 0;
	virtual IAttachment*        CreateVClothAttachment(const SVClothAttachmentParams& params) = 0;

	virtual i32               RemoveAttachmentByInterface(const IAttachment* pAttachment, u32 loadingFlags = 0) = 0;
	virtual i32               RemoveAttachmentByName(tukk szName, u32 loadingFlags = 0) = 0;
	virtual i32               RemoveAttachmentByNameCRC(u32 nameCrc, u32 loadingFlags = 0) = 0;

	virtual IAttachment*        GetInterfaceByName(tukk szName) const = 0;
	virtual IAttachment*        GetInterfaceByIndex(u32 c) const = 0;
	virtual IAttachment*        GetInterfaceByNameCRC(u32 nameCRC) const = 0;
	virtual IAttachment*        GetInterfaceByPhysId(i32 id) const = 0;    //!< Takes physical partid as input.

	virtual i32               GetAttachmentCount() const = 0;
	virtual i32               GetIndexByName(tukk szName) const = 0;
	virtual i32               GetIndexByNameCRC(u32 nameCRC) const = 0;

	virtual u32              ProjectAllAttachment() = 0;

	virtual void                PhysicalizeAttachment(i32 idx, IPhysicalEntity* pent = 0, i32 nLod = 0) = 0;
	virtual void                DephysicalizeAttachment(i32 idx, IPhysicalEntity* pent = 0) = 0;

	virtual ICharacterInstance* GetSkelInstance() const = 0;

	virtual i32               GetProxyCount() const = 0;
	virtual IProxy*             CreateProxy(tukk szName, tukk szJointName) = 0;
	virtual IProxy*             GetProxyInterfaceByIndex(u32 c) const = 0;
	virtual IProxy*             GetProxyInterfaceByName(tukk szName) const = 0;
	virtual IProxy*             GetProxyInterfaceByCRC(u32 nameCRC) const = 0;
	virtual i32               GetProxyIndexByName(tukk szName) const = 0;
	virtual i32               RemoveProxyByInterface(const IProxy* ptr) = 0;
	virtual i32               RemoveProxyByName(tukk szName) = 0;
	virtual i32               RemoveProxyByNameCRC(u32 nameCRC) = 0;
	virtual void                DrawProxies(u32 enable) = 0;
	virtual u32              GetProcFunctionCount() const = 0;
	virtual tukk         GetProcFunctionName(u32 idx) const = 0;
	// </interfuscator:shuffle>
};
...
struct IAttachment
{
	...
	virtual u32      GetNameCRC() const = 0;
	virtual u32      ReName(tukk szSocketName, u32 crc) = 0;

	virtual u32      GetType() const = 0;
	bool                IsMerged() const { return (GetFlags() & FLAGS_ATTACH_MERGED_FOR_SHADOWS) != 0; }

	virtual u32      SetJointName(tukk szJointName) = 0;
	...
	virtual void               HideAttachment(u32 x) = 0;
	virtual u32             IsAttachmentHidden() const = 0;

	virtual void               HideInRecursion(u32 x) = 0;
	virtual u32             IsAttachmentHiddenInRecursion() const = 0;

	virtual void               HideInShadow(u32 x) = 0;
	virtual u32             IsAttachmentHiddenInShadow() const = 0;

	virtual void               AlignJointAttachment() = 0;

	virtual u32             GetJointID() const = 0;
	...
	virtual void AddBinding(IAttachmentObject* pModel, ISkin* pISkin = 0, u32 nLoadingFlags = 0) = 0;
	virtual void ClearBinding(u32 nLoadingFlags = 0) = 0;
...
struct IAttachmentSkin
{
	...
	virtual void TriggerMeshStreaming(u32 nDesiredRenderLOD, const SRenderingPassInfo& passInfo) = 0;
...
struct IAttachmentObject
{
	...
	virtual bool                PhysicalizeAttachment(IAttachmentUpr* pUpr, i32 idx, i32 nLod, IPhysicalEntity* pent, const Vec3& offset) { return false; }
	virtual bool                UpdatePhysicalizedAttachment(IAttachmentUpr* pUpr, i32 idx, IPhysicalEntity* pent, const QuatT& offset)   { return false; }
	...
	virtual tukk         GetObjectFilePath() const
	{
	...
	virtual IMaterial* GetBaseMaterial(u32 nLOD = 0) const = 0;
	virtual void       SetReplacementMaterial(IMaterial* pMaterial, u32 nLOD = 0) = 0;
	virtual IMaterial* GetReplacementMaterial(u32 nLOD = 0) const = 0;

	virtual void       OnRemoveAttachment(IAttachmentUpr* pUpr, i32 idx) {}
	virtual void       Release() = 0;
	// </interfuscator:shuffle>
};
...
struct CCGFAttachment : public IAttachmentObject
{
	...
	IMaterial*    GetBaseMaterial(u32 nLOD) const override;
	void          SetReplacementMaterial(IMaterial* pMaterial, u32 nLOD = 0) override { m_pReplacementMaterial = pMaterial; };
	IMaterial*    GetReplacementMaterial(u32 nLOD) const override                     { return m_pReplacementMaterial; }
...
inline IMaterial* CCGFAttachment::GetBaseMaterial(u32 nLOD) const
{
...
struct CSKELAttachment : public IAttachmentObject
{
	...
	IMaterial* GetBaseMaterial(u32 nLOD) const override;
	void       SetReplacementMaterial(IMaterial* pMaterial, u32 nLOD) override { m_pReplacementMaterial = pMaterial; }
	IMaterial* GetReplacementMaterial(u32 nLOD) const override                 { return m_pReplacementMaterial; }
...
inline IMaterial* CSKELAttachment::GetBaseMaterial(u32 nLOD) const
{
...
struct CSKINAttachment : public IAttachmentObject
{
	...
	IMaterial*       GetBaseMaterial(u32 nLOD) const override;
	void             SetReplacementMaterial(IMaterial* pMaterial, u32 nLOD) override
	{
		nLOD = clamp_tpl<u32>(nLOD, 0, g_nMaxGeomLodLevels - 1);
		m_pReplacementMaterial[nLOD] = pMaterial;
	}
	IMaterial* GetReplacementMaterial(u32 nLOD) const override
	{
		nLOD = clamp_tpl<u32>(nLOD, 0, g_nMaxGeomLodLevels - 1);
...
inline IMaterial* CSKINAttachment::GetBaseMaterial(u32 nLOD) const
{
...
public:
	...
	IMaterial* GetBaseMaterial(u32 nLOD) const override;
	void       SetReplacementMaterial(IMaterial* pMaterial, u32 nLOD) override {}
	IMaterial* GetReplacementMaterial(u32 nLOD) const override                 { return 0; }
...
inline IMaterial* CEntityAttachment::GetBaseMaterial(u32 nLOD) const
{
	return 0;
}
...
inline IMaterial* CLightAttachment::GetBaseMaterial(u32 nLOD) const
{
	return 0;
}
...
public:
	...
	CEffectAttachment(tukk effectName, const Vec3& offset, const Vec3& dir, f32 scale = 1.0f, bool prime = false)
		: m_loc(IParticleEffect::ParticleLoc(offset, dir, scale)), m_bPrime(prime)
	{
		m_pEffect = gEnv->pParticleUpr->FindEffect(effectName, "Character Attachment", true);
	}
...
inline IMaterial* CEffectAttachment::GetBaseMaterial(u32 nLOD) const
{
	return 0;
}
...
struct IProxy
{
	...
	virtual u32       GetNameCRC() const = 0;
	virtual u32       ReName(tukk szSocketName, u32 crc) = 0;
	virtual u32       SetJointName(tukk szJointName = 0) = 0;
	virtual u32       GetJointID() const = 0;
	...
	virtual void         SetHideProxy(u8 nHideProxy) = 0;
