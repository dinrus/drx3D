...
{
	i32 nAnimID = rCurAnim.GetAnimationId();
	...
	if (nError == 0)
	{
		...
		//---------------------------------------------------------------------
		u32 nDimensions = rLMG.m_Dimensions;
	...
	else
	{
		...
		gEnv->pLog->LogError("DinrusXAnimation: %s: '%s'", rLMG.m_Status.c_str(), rLMG.GetFilePath());
	}
	//---------------------------------------------------------------------
...
#if !defined(_RELEASE)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_ERROR, "Parametric sampler encountered an error while processing '%s': Blend weights do not sum up to 1.0f.", rLMG.GetFilePath());
		}
	}
#endif
	...
	if (0)
	{
		...
		for (u32 i = 0; i < m_numExamples; i++)
		{
			...
			i32 segcount = m_nSegmentCounter[0][i];
	...
	if (1)
	{
		...
		for (u32 i = 0; i < m_numExamples; i++)
		{
			...
			/*if (AllowDebug)
			   {
			   tukk pAnimName = pAnimPS->GetAnimName();
			   float fColor[4] = {1,1,1,1};
			   g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.6f, fColor, false,"---fWeight: %20.16f %s",m_fBlendWeight[i],pAnimName );
			   g_YLine+=16.0f;
			   }*/

			GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[pAnimPS->m_nGlobalAnimId];
			i32k segmentIndex = m_nSegmentCounter[0][i];
			...
			if (pController)
			{
				u32 numKeys = u32(rCAF.m_fTotalDuration * ANIMATION_30Hz + 1);
				u32 maxsize = (sizeof(root_keys) / sizeof(Vec3)) - 1;
				numKeys = (numKeys > maxsize) ? maxsize : numKeys;
				if (numKeys > 1)
				{
					//more then 1 keyframe.
					f32 fk = rCAF.m_fStartSec * ANIMATION_30Hz;
					for (u32 k = 0; k < numKeys; k++, fk += 1.0f)
			...
			//	float fColor[4] = {1,1,1,1};
			//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.6f, fColor, false,"fDistance: %f   fMoveSpeed1: %f  fMoveSpeed2: %f  fSegDuration: %f",fDistance,fMoveSpeed,  fDistance/(fSegDuration+1.0f/ANIMATION_30Hz), fSegDuration );
			//	g_YLine+=16.0f;
			//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.6f, fColor, false,"fDistance: %f   fMoveSpeed1: %f  fMoveSpeed2: %f  fSegDuration: %f",fDistance,fMoveSpeed1,fMoveSpeed2, fSegDuration );
			//	g_YLine+=16.0f;

			fTWDuration += m_fBlendWeight[i] * segmentDuration;
		...
		u32 numParameters = rLMG.m_arrParameter.size();
...
void SParametricSamplerInternal::BlendSpace1DVisualize(GlobalAnimationHeaderLMG& rLMG, const CAnimation& rCurAnim, const CAnimationSet* pAnimationSet, float fPlaybackScale, i32 nInstanceOffset, u32 fDebugConfig, float fUniScale) const
{
	...
	i32 selFace = -1;
	i32 numExamples = rLMG.m_numExamples;
	...
	float arrWeights[256] = { 0 };
	{
		...
		if (fDebugConfig & 4)
		{
			...
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fColDebug6, false, "idx0: %d w0: %f", ve.i0, ve.w0);
			g_YLine += 21.0f;
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fColDebug6, false, "idx1: %d w1: %f", ve.i1, ve.w1);
			g_YLine += 21.0f;
		}
	}

	u32 numVExamples = rLMG.m_VirtualExampleGrid1D.size();
	if (numVExamples)
	{
		SAuxGeomRenderFlags renderFlags(e_Def3DPublicRenderflags);
		g_pAuxGeom->SetRenderFlags(renderFlags);
...

	{
		SAuxGeomRenderFlags renderFlags(e_Def3DPublicRenderflags);
		g_pAuxGeom->SetRenderFlags(renderFlags);
		u32 initD1 = m_MotionParameterFlags[0] & CA_Dim_Initialized;
		g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fCol1111, false, "%s: %f %d", rLMG.m_DimPara[0].m_strParaName.c_str(), vDesiredParameter.x, initD1);
		g_YLine += 20.0f;
...
void SParametricSamplerInternal::BlendSpace1D(GlobalAnimationHeaderLMG& rLMG, const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, const CAnimation& rCurAnim, f32 fFrameDeltaTime, f32 fPlaybackScale, Vec3 off, bool AllowDebug, u32 nInstanceOffset)
{
	u32 nDimensions = rLMG.m_Dimensions;
	u32 numExamples = rLMG.m_numExamples;
	u32 numParameter = rLMG.m_arrParameter.size();
	...
	//-------------------------------------------------------------
	//Dimension=1
	tukk pParaName = rLMG.m_DimPara[0].m_strParaName; //just for debugging
	u32 nParaID = rLMG.m_DimPara[0].m_ParaID;
	if (nParaID > eMotionParamID_COUNT)
		DrxFatalError("DinrusXAnimation: nParaID not initialized");
	...
	u32 numParameters = rLMG.m_arrParameter.size();
	for (u32 i = numExamples; i < numParameters; i++)
	{
		u32 i0 = rLMG.m_arrParameter[i].i0;
		f32 w0 = rLMG.m_arrParameter[i].w0;
		u32 i1 = rLMG.m_arrParameter[i].i1;
	...
	u32 initialized = rLMG.m_VirtualExampleGrid1D.size();
	if (initialized == 0)
	{
		rLMG.m_VirtualExampleGrid1D.resize(rLMG.m_DimPara[0].m_cells);
		for (u32 c0 = 0; c0 < rLMG.m_DimPara[0].m_cells; c0++)
		{
			...
			u32 c = 0;
			u8* pI = &(rLMG.m_VirtualExampleGrid1D[c0].i0);
			...
			{
				g_pAuxGeom->Draw2dLabel(1, g_YLine, 3.0f, ColorF(1, 1, 1, 1), false, "invalid sum: %f", sum);
				g_YLine += 31.0f;
			}
		}
	}
	...
	if (numVExamples)
	{
		...
		if (AllowDebug && Console::GetInst().ca_SnapToVGrid)
		{
			...
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 3.0f, ColorF(1, 1, 1, 1), false, "Snapped to VGrid: %f", vDesiredParameter.x);
			g_YLine += 31.0f;
		}
#endif
		...
		i32 ix = i32(fx);
		f32 px = fx - f32(ix);
		//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 2.0f, fCol1111, false,"dpx:%f  ix:%d  px:%f",vDesiredParameter.x,ix,px);
		//	g_YLine+=21.0f;
		for (u32 i = 0; i < numExamples; i++)
			arrWeights[i] = 0;
	...
	if (AllowDebug)
	{
		u32k fDebugConfig = u32(floorf(Console::GetInst().ca_DrawVEGInfo));
...
#ifdef BLENDSPACE_VISUALIZATION
void SParametricSamplerInternal::BlendSpace2DVisualize(GlobalAnimationHeaderLMG& rLMG, const CAnimation& rCurAnim, const CAnimationSet* pAnimationSet, float fPlaybackScale, i32 nInstanceOffset, u32 fDebugConfig, float fUniScale) const
{
	float fCol1111[4] = { 1, 1, 1, 1 };
	float fCol1011[4] = { 1, 0, 1, 1 };
	float fCol1001[4] = { 1, 0, 0, 1 };
	...
	i32 selFace = -1;
	i32 numExamples = rLMG.m_numExamples;
	u32 numVExamples = rLMG.m_VirtualExampleGrid2D.size();
	...
	i32 selectedFace = -1;
	{
		...
		//	float fColDebug6[4]={1,0,1,1};
		//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 2.0f, fColDebug6, false,"weights: %f %f %f %f",ve.w0,ve.w1,ve.w2,ve.w3 );
		//	g_YLine+=21.0f;
	}
	{
		//---------------------------------------------------------------------------------------------
		...
		if (fDebugConfig & 2)
		{
			...
			u32 mem = rLMG.m_VirtualExampleGrid2D.get_alloc_size();
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fCol1111, false, "mem: %d", mem);
			g_YLine += 21.0f;
		}
	}
	{
		...
		{
			u32 initD = m_MotionParameterFlags[d] & CA_Dim_Initialized;
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fCol1111, false, "%s: %f %d", rLMG.m_DimPara[d].m_strParaName.c_str(), vDesiredParameter[d], initD);
			g_YLine += 20.0f;
		}
		if (fDebugConfig & 1)
		{
...
void SParametricSamplerInternal::BlendSpace2D(GlobalAnimationHeaderLMG& rLMG, const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, const CAnimation& rCurAnim, f32 fFrameDeltaTime, f32 fPlaybackScale, Vec3 off, bool AllowDebug, u32 nInstanceOffset)
{
	float fCol1111[4] = { 1, 1, 1, 1 };
	u32 numExamples = rLMG.m_numExamples;//m_arrBSAnimations.size();
	...
	for (u32 d = 0; d < rLMG.m_Dimensions; d++)
	{
		tukk pParaName = rLMG.m_DimPara[d].m_strParaName; //just for debugging
		u32 nParaID = rLMG.m_DimPara[d].m_ParaID;
		if (nParaID > eMotionParamID_COUNT)
			DrxFatalError("DinrusXAnimation: nParaID not initialized");
	...
	const Diag33 scl(rLMG.m_DimPara[0].m_scale, rLMG.m_DimPara[1].m_scale, 1.0f);
	f32 xstep = (rLMG.m_DimPara[0].m_max - rLMG.m_DimPara[0].m_min) / f32(rLMG.m_DimPara[0].m_cells - 1);
	f32 ystep = (rLMG.m_DimPara[1].m_max - rLMG.m_DimPara[1].m_min) / f32(rLMG.m_DimPara[1].m_cells - 1);
	u32 initialized = rLMG.m_VirtualExampleGrid2D.size();
	if (initialized == 0)
	{
		...
		for (f32 y = rLMG.m_DimPara[1].m_min; y <= (rLMG.m_DimPara[1].m_max + 0.001f); y = y + ystep, c1++)
		{
			...
			for (f32 x = rLMG.m_DimPara[0].m_min; x <= (rLMG.m_DimPara[0].m_max + 0.001f); x = x + xstep, c0++)
			{
				...
				u32 cell = c1 * rLMG.m_DimPara[0].m_cells + c0;
				...
				u32 c = 0;
				u8* pI = &(rLMG.m_VirtualExampleGrid2D[cell].i0);
	...
	if (numVExamples)
	{
		...
		if (AllowDebug && Console::GetInst().ca_SnapToVGrid)
		{
			...
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 3.0f, fCol1111, false, "Snapped to VGrid: %f  %f", vDesiredParameter.x, vDesiredParameter.y);
			g_YLine += 31.0f;
		}
#endif
		...
		VirtualExample2D ve;
		f32 ew0 = (1 - px) * (1 - py);
		i32 idx0 = (rLMG.m_DimPara[0].m_cells * (iy + 0)) + (ix + 0);
		f32 ew1 = px * (1 - py);
		i32 idx1 = (rLMG.m_DimPara[0].m_cells * (iy + 0)) + (ix + 1);

		f32 ew2 = (1 - px) * py;
		i32 idx2 = (rLMG.m_DimPara[0].m_cells * (iy + 1)) + (ix + 0);
		f32 ew3 = px * py;
		i32 idx3 = (rLMG.m_DimPara[0].m_cells * (iy + 1)) + (ix + 1);
...
NO_INLINE void UpdateWeights(f32 ew, u32 idx, f32* __restrict arrWeights, const GlobalAnimationHeaderLMG& rLMG)
{
...
#ifdef BLENDSPACE_VISUALIZATION
void SParametricSamplerInternal::BlendSpace3DVisualize(GlobalAnimationHeaderLMG& rLMG, const CAnimation& rCurAnim, const CAnimationSet* pAnimationSet, float fPlaybackScale, i32 nInstanceOffset, u32 fDebugConfig, float fUniScale) const
{
	float fCol1111[4] = { 1, 1, 1, 1 };
	float fCol1011[4] = { 1, 0, 1, 1 };
	float fCol1001[4] = { 1, 0, 0, 1 };
	...
	i32 numExamples = rLMG.m_numExamples;
	u32 numVExamples = rLMG.m_VirtualExampleGrid3D.size();
	if (numExamples == 0 || numVExamples == 0)
		return;
	...
	//	float fColDebug6[4]={1,0,1,1};
	//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 2.0f, fColDebug6, false,"weights: %f %f %f %f",ve.w0,ve.w1,ve.w2,ve.w3,ve.w3 );
	//	g_YLine+=21.0f;
	...
	if (fDebugConfig & 2)
	{
		...
		u32 mem = rLMG.m_VirtualExampleGrid3D.get_alloc_size();
		g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fCol1111, false, "mem: %d", mem);
		g_YLine += 21.0f;
	}
	...
	SAuxGeomRenderFlags renderFlags(e_Def3DPublicRenderflags);
	g_pAuxGeom->SetRenderFlags(renderFlags);
	i32 nDimensions = rLMG.m_Dimensions;
	for (u32 d = 0; d < nDimensions; d++)
	{
		u32 initD = m_MotionParameterFlags[d] & CA_Dim_Initialized;
		g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fCol1111, false, "%s: %f %d", rLMG.m_DimPara[d].m_strParaName.c_str(), vDesiredParameter[d], initD);
		g_YLine += 20.0f;
...
void SParametricSamplerInternal::BlendSpace3D(GlobalAnimationHeaderLMG& rLMG, const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, const CAnimation& rCurAnim, f32 fFrameDeltaTime, f32 fPlaybackScale, Vec3 off, bool AllowDebug, u32 nInstanceOffset)
{
	float fCol1111[4] = { 1, 1, 1, 1 };
	float fCol1011[4] = { 1, 0, 1, 1 };
	u32 nDimensions = rLMG.m_Dimensions;
	...
	const Diag33 scl(rLMG.m_DimPara[0].m_scale, rLMG.m_DimPara[1].m_scale, rLMG.m_DimPara[2].m_scale);
	f32 xstep = (rLMG.m_DimPara[0].m_max - rLMG.m_DimPara[0].m_min) / f32(rLMG.m_DimPara[0].m_cells - 1);
	f32 ystep = (rLMG.m_DimPara[1].m_max - rLMG.m_DimPara[1].m_min) / f32(rLMG.m_DimPara[1].m_cells - 1);
	f32 zstep = (rLMG.m_DimPara[2].m_max - rLMG.m_DimPara[2].m_min) / f32(rLMG.m_DimPara[2].m_cells - 1);
	u32 initialized = rLMG.m_VirtualExampleGrid3D.size();
	if (initialized == 0)
	{
		...
		for (f32 z = rLMG.m_DimPara[2].m_min; z <= (rLMG.m_DimPara[2].m_max + 0.001f); z = z + zstep, c2++)
		{
			...
			for (f32 y = rLMG.m_DimPara[1].m_min; y <= (rLMG.m_DimPara[1].m_max + 0.001f); y = y + ystep, c1++)
			{
				...
				for (f32 x = rLMG.m_DimPara[0].m_min; x <= (rLMG.m_DimPara[0].m_max + 0.001f); x = x + xstep, c0++)
				{
...
#endif
					u32 cell = c2 * rLMG.m_DimPara[1].m_cells * rLMG.m_DimPara[0].m_cells + c1 * rLMG.m_DimPara[0].m_cells + c0;
					...
					u32 c = 0;
					u8* pI = &(rLMG.m_VirtualExampleGrid3D[cell].i0);
	...
	if (numVExamples)
	{
		...
		if (AllowDebug && Console::GetInst().ca_SnapToVGrid)
		{
			...
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 3.0f, fCol1111, false, "Snapped to VGrid: %f  %f  %f", vDesiredParameter.x, vDesiredParameter.y, vDesiredParameter.z);
			g_YLine += 31.0f;
		}
#endif
		...
		i32 idx[8] =
		{
			(iz + 0) * rLMG.m_DimPara[1].m_cells * rLMG.m_DimPara[0].m_cells + (iy + 0) * rLMG.m_DimPara[0].m_cells + (ix + 0),
			(iz + 0) * rLMG.m_DimPara[1].m_cells * rLMG.m_DimPara[0].m_cells + (iy + 0) * rLMG.m_DimPara[0].m_cells + (ix + 1),
			(iz + 0) * rLMG.m_DimPara[1].m_cells * rLMG.m_DimPara[0].m_cells + (iy + 1) * rLMG.m_DimPara[0].m_cells + (ix + 0),
			(iz + 0) * rLMG.m_DimPara[1].m_cells * rLMG.m_DimPara[0].m_cells + (iy + 1) * rLMG.m_DimPara[0].m_cells + (ix + 1),
			(iz + 1) * rLMG.m_DimPara[1].m_cells * rLMG.m_DimPara[0].m_cells + (iy + 0) * rLMG.m_DimPara[0].m_cells + (ix + 0),
			(iz + 1) * rLMG.m_DimPara[1].m_cells * rLMG.m_DimPara[0].m_cells + (iy + 0) * rLMG.m_DimPara[0].m_cells + (ix + 1),
			(iz + 1) * rLMG.m_DimPara[1].m_cells * rLMG.m_DimPara[0].m_cells + (iy + 1) * rLMG.m_DimPara[0].m_cells + (ix + 0),
			(iz + 1) * rLMG.m_DimPara[1].m_cells * rLMG.m_DimPara[0].m_cells + (iy + 1) * rLMG.m_DimPara[0].m_cells + (ix + 1)
		};
		...
		u32k increment = 128 / sizeof(m_fBlendWeight[0]);
		for (u32 i = 0; i < numExamples; i += increment)
...
i32 SParametricSamplerInternal::GetWeights1D(f32 fDesiredParameter, const GlobalAnimationHeaderLMG& rLMG, f32 arrWeights[], Diag33 scl, Vec3 off) const
{
	u32k fDebugConfig = u32(floorf(Console::GetInst().ca_DrawVEGInfo));
	...
	u32 nInsideFace = 0;
	u32 numLines = rLMG.m_arrBSAnnotations.size();
	for (u32 f = 0; f < numLines; f++)
	{
		if (rLMG.m_arrBSAnnotations[f].num == 2)
		{
			i32 i0 = rLMG.m_arrBSAnnotations[f].idx[0];
			i32 i1 = rLMG.m_arrBSAnnotations[f].idx[1];
			f32 x0 = rLMG.m_arrParameter[i0].m_Para.x;
			f32 x1 = rLMG.m_arrParameter[i1].m_Para.x;

#if !defined(_RELEASE)
			if (x0 >= x1)
				g_pISystem->Warning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, VALIDATOR_FLAG_FILE, 0, "!DinrusXAnimation: motion parameter must be sorted by size in the 1D-Blend-Space. Lowest parameter must come first: %s", rLMG.GetFilePath());
#endif
			...
			if (x0 <= fDesiredParameter && x1 >= fDesiredParameter)
			{
				...
				u32 ex00 = rLMG.m_arrParameter[i0].i0;
				arrWeights[ex00] += rLMG.m_arrParameter[i0].w0 * w0;
				u32 ex01 = rLMG.m_arrParameter[i0].i1;
				arrWeights[ex01] += rLMG.m_arrParameter[i0].w1 * w0;

				u32 ex10 = rLMG.m_arrParameter[i1].i0;
				arrWeights[ex10] += rLMG.m_arrParameter[i1].w0 * w1;
				u32 ex11 = rLMG.m_arrParameter[i1].i1;
				...
				{
					float fColDebug6[4] = { 1, 0, 1, 1 };
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fColDebug6, false, "ex00: %d w0: %f", ex00, rLMG.m_arrParameter[i0].w0);
					g_YLine += 21.0f;
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fColDebug6, false, "ex01: %d w1: %f", ex01, rLMG.m_arrParameter[i0].w1);
					g_YLine += 21.0f;
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fColDebug6, false, "ex10: %d w0: %f", ex10, rLMG.m_arrParameter[i1].w0);
					g_YLine += 21.0f;
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fColDebug6, false, "ex11: %d w1: %f", ex11, rLMG.m_arrParameter[i1].w1);
					g_YLine += 21.0f;
				}
#endif
				nInsideFace++;
	...
	if (nInsideFace == 0)
	{
		...
		if (nLineNo != -1)
		{
			i32 i0 = rLMG.m_arrBSAnnotations[nLineNo].idx[0];
			i32 i1 = rLMG.m_arrBSAnnotations[nLineNo].idx[1];
			f32 x0 = rLMG.m_arrParameter[i0].m_Para.x;
			f32 x1 = rLMG.m_arrParameter[i1].m_Para.x;
#ifdef BLENDSPACE_VISUALIZATION
			if (fDebugConfig)
...
i32 SParametricSamplerInternal::GetWeights2D(const Vec2& vDesiredParameter, const GlobalAnimationHeaderLMG& rLMG, f32 arrWeights[], Diag33 scl, Vec3 off) const
{
	float fCol1111[4] = { 1, 1, 1, 1 };
	u32 numExamples = rLMG.m_numExamples;//m_arrBSAnimations.size();
	for (u32 i = 0; i < numExamples; i++)
	...
	u8 i[4];
	Vec3 v[4];
	u32 numBlocks = rLMG.m_arrBSAnnotations.size();
	for (f32 d = 0.0f; d < 2.35f; d = d + 0.05f)
	{
		for (u32 b = 0; b < numBlocks; b++)
		{
			u32 numPoints = rLMG.m_arrBSAnnotations[b].num;
			for (u32 e = 0; e < numPoints; e++)
			{
...
i32 SParametricSamplerInternal::GetWeights3D(const Vec3& vDesiredParameter, const GlobalAnimationHeaderLMG& rLMG, f32 arrWeights[], Diag33 scl, Vec3 off) const
{
	float fCol1111[4] = { 1, 1, 1, 1 };
	u32 numExamples = rLMG.m_numExamples;//m_arrBSAnimations.size();
	for (u32 i = 0; i < numExamples; i++)
	...
	for (f32 d = 0.0f; d < 2.35f; d = d + 0.05f)
	{
		for (u32 b = 0; b < numBlocks; b++)
		{
			...
			if (InsideHull)
			{
				if (Console::GetInst().ca_DrawVEGInfo)
				{
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 4.0f, fCol1111, false, "Inside Block1: %d", b);
					g_YLine += 40.0f;
				}
				return b;
			}
...
BC4 SParametricSamplerInternal::GetConvex4(u32 numPoints, const Vec2& vDesiredParameter, const GlobalAnimationHeaderLMG& rLMG, f32 arrWeights[], u8* idx, Vec3* vtx, Diag33 scl, Vec3 off) const
{
	const ColorB faceColor = RGBA8(0xff, 0xff, 0xff, 0x40);
	float fCol1111[4] = { 1, 1, 1, 1 };
	u32 numExamples = rLMG.m_numExamples;//m_arrBSAnimations.size();
	for (u32 e = 0; e < numExamples; e++)
		arrWeights[e] = 0.0f;

	u32 numParameter = rLMG.m_arrParameter.size();
	ColorB col[256];
	Vec3 examples[256];
	for (u32 p = 0; p < numParameter; p++)
	{
		if (p < numExamples)
	...
	if (numPoints == 3)
	{
		...
		u32 ex20 = rLMG.m_arrParameter[i2].i0;
		arrWeights[ex20] += rLMG.m_arrParameter[i2].w0 * bw4.w2;
		u32 ex21 = rLMG.m_arrParameter[i2].i1;
	...
	if (numPoints == 4)
	{
		...
		u32 ex30 = rLMG.m_arrParameter[i3].i0;
		arrWeights[ex30] += rLMG.m_arrParameter[i3].w0 * bw4.w3;
		u32 ex31 = rLMG.m_arrParameter[i3].i1;
...
BC8 SParametricSamplerInternal::GetConvex8(u32 numPoints, const Vec3& vDesiredParameter, const GlobalAnimationHeaderLMG& rLMG, f32 arrWeights[], u8* i, Vec3* v, Diag33 scl, Vec3 off) const
{
	...
	if (numPoints == 4)
	{
		...
		u32 pl0 = fabsf(v0.z) < 0.01f;
		u32 pl1 = fabsf(v1.z) < 0.01f;
		u32 pl2 = fabsf(v2.z) < 0.01f;
		u32 pl3 = fabsf(v3.z) < 0.01f;
		if (pl0 && pl1 && pl2 && pl3)
			return bw;
	...
	if (numPoints == 5)
	{
		...
		u32 ex40 = rLMG.m_arrParameter[i4].i0;
		arrWeights[ex40] += rLMG.m_arrParameter[i4].w0 * bw.w4;
		u32 ex41 = rLMG.m_arrParameter[i4].i1;
	...
	if (numPoints == 6)
	{
		...
		u32 ex50 = rLMG.m_arrParameter[i5].i0;
		arrWeights[ex50] += rLMG.m_arrParameter[i5].w0 * bw.w5;
		u32 ex51 = rLMG.m_arrParameter[i5].i1;
...
void SParametricSamplerInternal::CombinedBlendSpaces(GlobalAnimationHeaderLMG& rMasterParaGroup, const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, const CAnimation& rCurAnim, f32 fFrameDeltaTime, f32 fPlaybackScale, bool AllowDebug)
{
	...
	u32 numBlendSpaces = rMasterParaGroup.m_arrCombinedBlendSpaces.size();
	u32 viableBSpaces = 0xffffffff;
	for (u32 s = 0; s < rMasterParaGroup.m_Dimensions; s++)
	{
		if (rMasterParaGroup.m_DimPara[s].m_ChooseBlendSpace)
		{
			float bestDist = FLT_MAX;
			u32 blendSpaceMask = 0;
			u32 closestBSpaceMask = 0;
			u32 bSpaceMask = 1;
			const bool isAngular = (m_MotionParameterID[s] == eMotionParamID_TravelAngle) || (m_MotionParameterID[s] == eMotionParamID_TurnAngle);
			for (u32 bs = 0; bs < numBlendSpaces; bs++, bSpaceMask <<= 1)
			{
				i32 nGlobalID = rMasterParaGroup.m_arrCombinedBlendSpaces[bs].m_ParaGroupID;
				...
				if (nGlobalID >= 0)
				{
					...
					for (u32 d = 0; (d < rLMG.m_Dimensions) && (bestDist > 0.0f); d++)
					{
	...
	for (u32 bs = 0; bs < numBlendSpaces; bs++)
	{
		...
		if (nGlobalID >= 0)
		{
			...
			for (u32 i = 0; i < psi.m_numExamples; i++)
			{
				u16k idx = m_nLMGAnimIdx[exampleIdx + i];
...
void DrawPoly(Vec3* pts, u32 numPts, bool wireframe, ColorB color, ColorB backcolor = ColorB(0, 0, 0, 0), bool drawBackface = false)
{
	if (wireframe)
	{
		for (u32 i = 0; i < numPts - 1; i++)
		{
...
void SParametricSamplerInternal::VisualizeBlendSpace(const IAnimationSet* _pAnimationSet, const CAnimation& rCurAnim, f32 fPlaybackScale, u32 nInstanceOffset, const GlobalAnimationHeaderLMG& rLMG, Vec3 off, i32 selectedFace, float fUniScale) const
{
	const CAnimationSet* pAnimationSet = (const CAnimationSet*)_pAnimationSet;
	u32 numDebugInstances = g_pCharacterUpr->m_arrCharacterBase.size();

	const f32 fAnimTime = rCurAnim.GetCurrentSegmentNormalizedTime();
	i32k nEOC = rCurAnim.GetEndOfCycle();        //value from the last frame
	...
	u32 numExamples = rLMG.m_numExamples; //m_arrBSAnimations2.size();
	u32 numParameter = rLMG.m_arrParameter.size();
	...
	u32k numFaces = rLMG.m_arrBSAnnotations.size();
	Vec3 facePts[8];

	for (u32 passes = 0; passes < 2; passes++)
	{
	...
	if (!g_pCharacterUpr->HasAnyDebugInstancesCreated())
		return;
	...
	for (u32 p = 0; p < numParameter; p++)
	{
		...
		ICharacterInstance* pExampleInst = g_pCharacterUpr->m_arrCharacterBase[p + nInstanceOffset].m_pInst;
		...
		Quat rot = g_pCharacterUpr->m_arrCharacterBase[p + nInstanceOffset].m_GridLocation.q;
		...
		if (p < numExamples)
		{
			...
			tukk pAnimName = rLMG.m_arrParameter[p].m_animName.GetName_DEBUG();
			pSkeletonAnim->StartAnimation(pAnimName, AParams);

			i32 nAnimIDPS = m_nAnimID[p];
			const ModelAnimationHeader* pAnimPS = pAnimationSet->GetModelAnimationHeader(nAnimIDPS);
			GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[pAnimPS->m_nGlobalAnimId];
			u8 segcount = m_nSegmentCounter[0][p];
			...
			g_pCharacterUpr->m_arrCharacterBase[p + nInstanceOffset].m_GridLocation.q *= relmove.q;
			pExampleInst->FinishAnimationComputations();

			g_pCharacterUpr->m_arrCharacterBase[p + nInstanceOffset].m_AmbientColor.r = vCharCol.x;
			g_pCharacterUpr->m_arrCharacterBase[p + nInstanceOffset].m_AmbientColor.g = vCharCol.y;
			g_pCharacterUpr->m_arrCharacterBase[p + nInstanceOffset].m_AmbientColor.b = vCharCol.z;
			g_pCharacterUpr->m_arrCharacterBase[p + nInstanceOffset].m_AmbientColor.a = 1.0f;
		}
		else
		{
			...
			i32 i0 = rLMG.m_arrParameter[p].i0;
			f32 w0 = rLMG.m_arrParameter[p].w0;
			i32 i1 = rLMG.m_arrParameter[p].i1;
			f32 w1 = rLMG.m_arrParameter[p].w1;
			GlobalAnimationHeaderLMG& rInternalPara1D = g_AnimationUpr.m_arrGlobalLMG[0];
			if (rInternalPara1D.IsAssetInternalType() == 0)
				DrxFatalError("Not an Internal Blendspace");

			tukk pAnimName0 = rLMG.m_arrParameter[i0].m_animName.GetName_DEBUG();
			rInternalPara1D.m_arrParameter[0].m_animName.SetName(pAnimName0);
			tukk pAnimName1 = rLMG.m_arrParameter[i1].m_animName.GetName_DEBUG();
			...
			i32 nAnimIDPS0 = m_nAnimID[i0];
			i32 nAnimIDPS1 = m_nAnimID[i1];
			const ModelAnimationHeader* pAnimPS = pAnimationSet->GetModelAnimationHeader(nAnimIDPS1);
			GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[pAnimPS->m_nGlobalAnimId];

			i32 segcount0 = m_nSegmentCounter[0][i0];
			i32 segcount1 = m_nSegmentCounter[0][i1];
			pSkeletonAnim->SetLayerNormalizedTimeAndSegment(fAnimTime, nEOC, nAnimIDPS0, nAnimIDPS1, segcount0, segcount1);

			/*float fCol1111[4] = {1,1,1,1};
			   g_pAuxGeom->Draw2dLabel( 1,g_YLine, 2.0f, fCol1111, false,"id: %d  w: %f seg: %d  name: %s",i0,w0,segcount0, pAnimName0);
			   g_YLine+=20.0f;
			   g_pAuxGeom->Draw2dLabel( 1,g_YLine, 2.0f, fCol1111, false,"id: %d  w: %f seg: %d  name: %s",i1,w1,segcount1, pAnimName1);
			   g_YLine+=20.0f;*/
		...
		g_pCharacterUpr->m_arrCharacterBase[p + nInstanceOffset].m_GridLocation.t = params.locationAnimation.t;
		g_pCharacterUpr->m_arrCharacterBase[p + nInstanceOffset].m_GridLocation.s = params.locationAnimation.s;
	}
}
#endif
