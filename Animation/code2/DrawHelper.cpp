...
void BonePrism(const Vec3& origin, const Vec3& forward, const Vec3& up, const ColorB color)
{
	...
	const ColorB color0(u8(color.r * 0.5f), u8(color.g * 0.5f), u8(color.b * 0.5f), u8(0xff * 1.0f));
	const ColorB color1(u8(color.r * 0.75f), u8(color.g * 0.75f), u8(color.b * 0.75f), u8(0xff * 1.0f));

	const ColorB color00 = ColorB(u8(color0.r * 0.9f), u8(color0.g * 0.9f), u8(color0.b * 0.9f), u8(color0.a * 1.0f));
	const ColorB color01 = ColorB(u8(color0.r * 0.75f * 0.9f), u8(color0.g * 0.75f * 0.9f), u8(color0.b * 0.75f * 0.9f), u8(color0.a * 1.0f));
	const ColorB color11 = ColorB(u8(color0.r * 0.75f), u8(color0.g * 0.75f), u8(color0.b * 0.75f), u8(color0.a * 1.0f));
	const ColorB color21 = ColorB(u8(color1.r * 0.75f), u8(color1.g * 0.75f), u8(color1.b * 0.75f), u8(color1.a * 1.0f));
...
void Pose(const CDefaultSkeleton& rDefaultSkeleton, const Skeleton::CPoseData& poseData, const QuatT& location, ColorB color)
{
	...
	for (uint i = 0; i < jointCount; ++i)
	{
		...
		float length = 1.0f;
		i32 parentIndex = poseData.GetParentIndex(rDefaultSkeleton, i);
		if (parentIndex > -1)
		{
