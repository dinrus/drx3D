...
ITrackPositionStorage* GetPositionControllerPtr(u32 format, uk pData, size_t numElements)
{
	switch (format)
	{
	case eNoCompress:
...
ITrackPositionStorage* GetScalingControllerPtr(u32 format, uk pData, size_t numElements)
{
	return GetPositionControllerPtr(format, pData, numElements);
}

ITrackRotationStorage* GetRotationControllerPtr(u32 format, uk pData, size_t numElements)
{
	switch (format)
	{
	case eNoCompress:
...
IKeyTimesInformation* GetKeyTimesControllerPtr(u32 format, uk pData, size_t numElements)
{
	switch (format)
	{
	case eF32:
	...
	case eUINT16:
		return new UINT16KeyTimesInformation(reinterpret_cast<u16*>(pData), numElements);

	case eByte:
		return new ByteKeyTimesInformation(reinterpret_cast<u8*>(pData), numElements);

	case eF32StartStop:
		return new F32SSKeyTimesInformation(reinterpret_cast<f32*>(pData), numElements);

	case eUINT16StartStop:
		return new UINT16SSKeyTimesInformation(reinterpret_cast<u16*>(pData), numElements);

	case eByteStartStop:
		return new ByteSSKeyTimesInformation(reinterpret_cast<u8*>(pData), numElements);

	case eBitset:
		return new CKeyTimesInformationBitSet(reinterpret_cast<u16*>(pData), numElements);
	}
	return 0;
}
...
void CCompressedController::UnCompress()
{
	...
	if (m_Rotations0.size() > 0)
	{
		...
		u32 size = m_pRotationController->GetKeyTimesInformation()->GetNumKeys();//Rotations[0].m_Data.size();
	...
	if (m_Positions0.size() > 0)
	{
		...
		u32 size = m_pPositionController->GetKeyTimesInformation()->GetNumKeys(); //Positions[0].m_Data.size();
...
const DRX_ALIGN(16) u8 m_byteTable[256] = {
...
const DRX_ALIGN(16) u8 m_byteLowBit[256] = {
...
const DRX_ALIGN(16) u8 m_byteHighBit[256] = {
...
} //endns ControllerHelper

void IPositionController::GetValueFromKey(u32 key, Vec3& val)
{
	u32 numKeys = GetNumKeys();
...
void IRotationController::GetValueFromKey(u32 key, Quat& val)
{
	u32 numKeys = GetNumKeys();
...
i32 CController::GetO_numKey() const
{
...
i32 CController::GetP_numKey() const
{
...
i32 Init48b = SmallTree48BitQuat::Init();
#endif
