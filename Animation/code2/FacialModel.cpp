...
#include <drx3D/Animation/DrxModEffMorph.h> //embedded
...
void CFacialModel::InitMorphTargets()
{
	...
	i32 nMorphTargetsCount = (i32)pModelMesh->m_morphTargets.size();
	m_effectors.reserve(nMorphTargetsCount);
	m_morphTargets.reserve(nMorphTargetsCount);
	for (i32 i = 0; i < nMorphTargetsCount; i++)
	{
...
void CFacialModel::AssignLibrary(IFacialEffectorsLibrary* pLibrary)
{
	...
	u32 numEffectors = m_effectors.size();
	for (u32 i = 0; i < numEffectors; i++)
	{
	...
	i32 nIndices = m_pEffectorsLibrary->GetLastIndex();
	m_faceStateIndexToMorphTargetId.resize(nIndices);
	for (i32 i = 0; i < nIndices; i++)
	{
	...
	for (CFacialEffectorsLibrary::CrcToEffectorMap::iterator it = m_pEffectorsLibrary->m_crcToEffectorMap.begin(); it != m_pEffectorsLibrary->m_crcToEffectorMap.end(); ++it)
	{
		...
		i32 nStateIndex = pEffector->GetIndexInState();
		if (nStateIndex >= 0 && nStateIndex < nIndices)
		{
			...
			{
				m_faceStateIndexToMorphTargetId[nStateIndex] = (i32)std::distance(m_morphTargets.begin(), it2);
			}
		}
	}
}
...
//////////////////////////////////////////////////////////////////////////
i32 CFacialModel::GetMorphTargetCount() const
{
	return m_morphTargets.size();
}

//////////////////////////////////////////////////////////////////////////
tukk CFacialModel::GetMorphTargetName(i32 morphTargetIndex) const
{
#ifdef FACE_STORE_ASSET_VALUES
	return (morphTargetIndex >= 0 && morphTargetIndex < i32(m_morphTargets.size()) ? m_morphTargets[morphTargetIndex].name.GetString().c_str() : 0);
#else
	DrxFatalError("Not Supported on this platform.");
	return NULL;
#endif
}

void CFacialModel::PushMorphEffectors(i32 i, const MorphTargetsInfo& morphTargets, f32 fWeight, f32 fBalance, DynArray<DrxModEffMorph>& arrMorphEffectors)
{
	// This is a morph target.
	i32 nMorphIndex = m_faceStateIndexToMorphTargetId[i];
	if (nMorphIndex >= 0 && nMorphIndex < (i32)m_morphTargets.size())
	{
		i32 nMorphTargetId = m_morphTargets[nMorphIndex].nMorphTargetId;

		// Add morph target to character.
		DrxModEffMorph morph;
		morph.m_fTime = 0;
		morph.m_nFlags = DrxCharMorphParams::FLAGS_FREEZE | DrxCharMorphParams::FLAGS_NO_BLENDOUT | DrxCharMorphParams::FLAGS_INSTANTANEOUS;
...
//////////////////////////////////////////////////////////////////////////
void CFacialModel::FillInfoMapFromFaceState(CFacialDisplaceInfo& info, CFaceState* const pFaceState, CCharInstance* const pInstance, i32 numForcedRotations, const CFacialAnimForcedRotationEntry* const forcedRotations, float blendBoneRotations)
{
	...
	i32k nNumIndices = pFaceState->GetNumWeights();
	i32k faceStateIndexToMorphTargetSize = (i32)m_faceStateIndexToMorphTargetId.size();
	for (i32 i = 0; i < nNumIndices; i++)
	{
		...
		if (!isMorph)
		{
			...
			if (pEffector)
			{
				...
				case EFE_TYPE_BONE:
					{
						QuatT qt;
						i32k boneID = ApplyBoneEffector(pInstance, pEffector, fWeight, qt);
						if (boneID >= 0)
						{
				...
				case EFE_TYPE_ATTACHMENT:
					...
					DrxFatalError("Attachment Effectors not longer supported!");
					break;
				}
			}
		}
		else
		{
			...
			DrxFatalError("Morphs not longer supported!");
		}
	}
	...
	/*
	   u32 numFaceJoints = pSkeletonPose->m_FaceAnimPosSmooth.size();
	...
	//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.4f, fColor, false,"Facial Animation" );
	//	g_YLine+=16.0f;

	/*
	   if (blendBoneRotations)
	   {
	   g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.4f, fColor, false,"blendBoneRotations" );	g_YLine+=16.0f;
	   ...
	   for (u32 boneIndex=0; boneIndex<numJoints; ++boneIndex)
	   {
	...
	// support the code interface to force the neck and eye orientations.
	{
		...
		{
			for (i32 rotIndex = 0; rotIndex < numForcedRotations; ++rotIndex)
			{
				//CSkeletonPose* pSkeletonPose = &pMasterCharacter->m_SkeletonPose;
				i32 id = forcedRotations[rotIndex].jointId;
...
void CFacialModel::TransformTranslations(CFacialDisplaceInfo& info, const CCharInstance* const pInstance)
{
	...
	for (size_t i = 0; i < infoCount; ++i)
	{
		...
		if (isUsed)
		{
			i32k p = pInstance->m_pDefaultSkeleton->m_arrModelJoints[i].m_idxParent;
...
/*
   void CFacialModel::HandleLookIK(FacialDisplaceInfoMap& info, CLookAt& lookIK, bool physicsRelinquished, i32 head, i32 leye, i32 reye)
   {
   ...
   {
    //	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.4f, fColor, false,"Head: %f (%f %f %f)",additionalRotPos[head].q.w,additionalRotPos[head].q.v.x,additionalRotPos[head].q.v.y,additionalRotPos[head].q.v.z );	g_YLine+=16.0f;
   ...
   {
    //	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.4f, fColor, false,"LEye: %f (%f %f %f)",additionalRotPos[leye].q.w,additionalRotPos[leye].q.v.x,additionalRotPos[leye].q.v.y,additionalRotPos[leye].q.v.z);	g_YLine+=16.0f;
   ...
   {
    //	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.4f, fColor, false,"REye: %f (%f %f %f)",additionalRotPos[reye].q.w,additionalRotPos[reye].q.v.x,additionalRotPos[reye].q.v.y,additionalRotPos[reye].q.v.z  );	g_YLine+=16.0f;
    //	i32 p = pSkeletonPose->m_parrModelJoints[reye].m_idxParent;
...
{
	DEFINE_PROFILER_FUNCTION();
	u32 numJoints = pPoseData->GetJointCount();

	// for dirty joints, the absolute pose has to be recalculated -- see below
	u32 dirty[MAX_JOINT_AMOUNT];
	u32 firstDirty = MAX_JOINT_AMOUNT;
	memset(dirty, 0x00, sizeof(u32) * numJoints);

	u32k infoCount = static_cast<u32>(info.GetCount());
	for (u32 i = 1; i < infoCount; ++i)
	{
		const bool isUsed = info.IsUsed(i);
		if (isUsed)
		{
	...
	for (u32 i = firstDirty; i < numJoints; ++i)
	{

		i32 p = pSkeleton->m_arrModelJoints[i].m_idxParent;
...
/* // Attachment Effectors do not exist anymore
   ...
   {
   tukk sAttachmentName = pEffector->GetParamString( EFE_PARAM_BONE_NAME );

   IAttachment *pIAttachment = pSkinInstance->GetIAttachmentUpr()->GetInterfaceByName( sAttachmentName );
   if (pIAttachment)
   {
   ...
   else
   {
    ...
    {
      u32 type = pMasterAttachment->GetType();
      if (pMasterAttachment->GetType() == CA_SKIN)
      {
        // Apply on parent bones.
        CCharInstance* pCharacter = pMasterAttachment->m_pAttachmentUpr->m_pSkelInstance;
        IAttachment *pAttachment = pCharacter->GetIAttachmentUpr()->GetInterfaceByName( sAttachmentName );
        if (pAttachment)
        {
          Ang3 vRot = Ang3(pEffector->GetParamVec3( EFE_PARAM_BONE_ROT_AXIS ));
          vRot = vRot * fWeight;
...
//////////////////////////////////////////////////////////////////////////
i32 CFacialModel::ApplyBoneEffector(CCharInstance* pInstance, CFacialEffector* pEffector, float fWeight, QuatT& quatT)
{
	u32k crc32 = pEffector->GetAttachmentCRC();
	i32k nBoneId = pInstance->m_pDefaultSkeleton->GetJointIDByCRC32(crc32);
...
void CFacialModel::GetMemoryUsage(IDrxSizer* pSizer) const
{
