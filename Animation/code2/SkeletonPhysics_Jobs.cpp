...
u32 CSkeletonPhysics::getBoneParentIndex(u32 nBoneIndex) const
{
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
	i32 parentIndex = rDefaultSkeleton.m_arrModelJoints[nBoneIndex].m_idxParent;
	if (parentIndex < 0)
		parentIndex = 0;  // -1 clamped to 0 ????
	return parentIndex;
}

void CSkeletonPhysics::ResetNonphysicalBoneRotations(Skeleton::CPoseData& poseData, i32 nLod, float fBlend)
{
	...
	u32 numBones = m_arrPhysicsJoints.size();
	//	assert(numBones); having 0 physical bones is a legal case

	for (u32 nBone = 1; nBone < numBones; ++nBone)
	{
...
void CSkeletonPhysics::UnconvertBoneGlobalFromRelativeForm(Skeleton::CPoseData& poseData, bool bNonphysicalOnlyArg, i32 nLod, bool bRopeTipsOnly)
{
...
i32 CSkeletonPhysics::getBonePhysParentIndex(i32 iBoneIndex, i32 nLod)
{
	i32 iPrevBoneIndex;
	do
	{
...
void CSkeletonPhysics::Job_Physics_SynchronizeFromEntityPrepare(Memory::CPool& memoryPool, IPhysicalEntity* pPhysicalEntity)
{
	...
	if (pPhysicalEntity)
	{
		...
		statusTmp2.lag = 2 + g_pCharacterUpr->m_nUpdateCounter - m_idLastSyncFrame;
	...
	i32 nLod = (m_pCharPhysics != pPhysicalEntity || m_bPhysicsRelinquished) ? GetPhysicsLod() : 0;
	...
	for (u32 i = 0; i < jointCount; ++i)
	{
		...
		partpos.partid = i;
		i32 status = pPhysicalEntity->GetStatus(&partpos);
		if (!status)
			continue;
	...
	for (u32 i = 0; i < jointCount; ++i)
	{
		...
		sj.idChildBody = i;
		i32 status = pPhysicalEntity->GetStatus(&sj);
		if (!status)
			continue;
	...
	pe_status_pos sp;
	i32 status2 = pPhysicalEntity->GetStatus(&sp);
	if (status2 == 0)
	{
...
void CSkeletonPhysics::Job_Physics_SynchronizeFromEntity(Skeleton::CPoseData& poseData, IPhysicalEntity* pPhysicalEntity, QuatT offset)
{
	...
	if (pPhysicalEntity)
	{
		...
		for (uint i = 0; i < jointCount; ++i)
		{
			...
			i32 parent = rDefaultSkeleton.m_arrModelJoints[index].m_idxParent;
			if (parent > -1)
			{
		...
		for (uint i = 0; i < jointCount; ++i)
		{
			...
			for (i32 k = getBoneParentIndex(i); k != parentIndex; k = getBoneParentIndex(k))
				parentMatrix = m_pPhysBuffer[k].location.q * parentMatrix;

			Quat physicsJointFrame(IDENTITY);
			DrxBonePhysics& physInfo = GetModelJointPointer(i)->m_PhysInfo;
			if (physInfo.flags != -1 &&
	...
	m_idLastSyncFrame = g_pCharacterUpr->m_nUpdateCounter;
}
	...
	for (i32 j = 0; j < m_nAuxPhys; ++j)
	{
		...
		else
		{
			...
			if (!m_auxPhys[j].bTied0)
				m_pPhysAuxBuffer[j].matrix = QuatT(Quat(IDENTITY), sr.pPoints[0]);
...
void CSkeletonPhysics::Job_Physics_SynchronizeFromAux(Skeleton::CPoseData& poseData)
{
	...
	for (i32 j = 0; j < m_nAuxPhys; ++j)
	{
		...
		if (!rCurPhysAuxData.bSet || !m_auxPhys[j].nBones)
			continue;
		...
		{
			iRoot = getBonePhysParentIndex(r_cur_aux_phys_data.iBoneTiedTo[0], nLod);
			for (i32 i = rDefaultSkeleton.m_arrModelJoints[r_cur_aux_phys_data.iBoneTiedTo[0]].m_idxParent; i != iRoot; i = rDefaultSkeleton.m_arrModelJoints[i].m_idxParent)
				qparent = pJointRelative[i] * qparent;
			qparent0 = qparent;
		}
		else if ((iRoot = rDefaultSkeleton.m_arrModelJoints[r_cur_aux_phys_data.pauxBoneInfo[0].iBone].m_idxParent) >= 0)
		{
			qparent0 = qparent = QuatT(r_cur_aux_phys_data.pauxBoneInfo[0].quat0, pJointAbsolute[r_cur_aux_phys_data.pauxBoneInfo[0].iBone].t).GetInverted() * pJointAbsolute[iRoot];
		}

		i32 nAuxBones = r_cur_aux_phys_data.nBones;
		for (i32 i = 0; i < nAuxBones; ++i)
		{
		...
		i32 iParent = r_cur_aux_phys_data.pauxBoneInfo[nAuxBones - 1].iBone;
...
void CSkeletonPhysics::Job_Physics_SynchronizeFromImpact(Skeleton::CPoseData& poseData, float timeDelta)
{
	...
	i32 iRoot = -1, jointCount = poseData.GetJointCount();
	for (i32 i = 0; i < jointCount; ++i)
	{
		...
		i32 parentNextIndex = parentIndex;
		i32 j;
		while (
...
void CSkeletonPhysics::Job_Physics_BlendFromRagdoll(Skeleton::CPoseData& poseData, float timeDelta)
{
	...
	i32 rootJointParentIndex = poseData.GetParentIndex(*m_pInstance->m_pDefaultSkeleton, rootJointIndex);
...
void CSkeletonPhysics::Job_SynchronizeWithPhysicsPrepare(Memory::CPool& memoryPool)
{
	...
	bool bSyncPhysics =
	  ...
	  (gEnv->pPhysicalWorld->GetPhysVars()->lastTimeStep > 0 || m_pInstance->m_CharEditMode) &&
