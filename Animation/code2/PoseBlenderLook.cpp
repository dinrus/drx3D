...
DRXREGISTER_CLASS(CPoseBlenderLook)
...
bool CPoseBlenderLook::PrepareInternal(const SAnimationPoseModifierParams& params)
{
	...
	//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.4f, fTextColor, false,"Prepare Look-IK" );
	//	g_YLine+=16.0f;
	...
	u32 useLookIK = (m_blender.m_dataIn.bUseDirIK || m_blender.m_dataOut.fDirIKBlend);
	if (useLookIK == 0)
		return false;

	const CDefaultSkeleton& defaultSkeleton = PoseModifierHelper::GetDefaultSkeleton(params);
	u32 numDirBlends = defaultSkeleton.m_poseBlenderLookDesc.m_blends.size();
	if (numDirBlends == 0)
		return false;  //error-check: we shouldn't execute the PM any more
	for (u32 d = 0; d < numDirBlends; d++)
	{
		if (defaultSkeleton.m_poseBlenderLookDesc.m_blends[d].m_nParaJointIdx < 0 || defaultSkeleton.m_poseBlenderLookDesc.m_blends[d].m_nStartJointIdx < 0 || defaultSkeleton.m_poseBlenderLookDesc.m_blends[d].m_nReferenceJointIdx < 0)
		{
			...
			g_pISystem->Warning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, VALIDATOR_FLAG_FILE, pInstance->GetFilePath(), "DinrusXAnimation: No look-bone specified for model: %s", pModelName);
			return false;  //error-check: we shouldn't execute the PM any more
		}
	}

	u32 numRotJoints = defaultSkeleton.m_poseBlenderLookDesc.m_rotations.size();
	if (numRotJoints == 0)
	{
		tukk pModelName = pInstance->m_pDefaultSkeleton->GetModelFilePath();
		g_pISystem->Warning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, VALIDATOR_FLAG_FILE, pInstance->GetFilePath(), "DinrusXAnimation: No LookIK-Setup for model: %s", pModelName);
		return false;   //error-check: we shouldn't execute the PM any more
	}

	if (defaultSkeleton.m_poseBlenderLookDesc.m_blends.size())
	{
		i32 nParaJointIdx = defaultSkeleton.m_poseBlenderLookDesc.m_blends[0].m_nParaJointIdx;
		if (nParaJointIdx > 0)
		{
	...
	IAttachmentUpr* pIAttachmentUpr = params.pCharacterInstance->GetIAttachmentUpr();
	tukk pstrLEye = defaultSkeleton.m_poseBlenderLookDesc.m_eyeAttachmentLeftName.c_str();
	i32 alidx = pIAttachmentUpr->GetIndexByName(pstrLEye);
	if (alidx >= 0)
	{
		m_pAttachmentEyeLeft = (CAttachmentBONE*)pIAttachmentUpr->GetInterfaceByIndex(alidx);
	}
	else
	{
	...
	tukk pstrREye = defaultSkeleton.m_poseBlenderLookDesc.m_eyeAttachmentRightName.c_str();
	i32 aridx = pIAttachmentUpr->GetIndexByName(pstrREye);
	if (aridx >= 0)
	{
		m_pAttachmentEyeRight = (CAttachmentBONE*)pIAttachmentUpr->GetInterfaceByIndex(aridx);
	}
	else
	{
...
bool CPoseBlenderLook::Execute(const SAnimationPoseModifierParams& params)
{
	...
	//	float fTextColor[4] = {1,0,0,1};
	//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.4f, fTextColor, false,"Execute Look-IK" );
	//	g_YLine+=16.0f;

	const CDefaultSkeleton& defaultSkeleton = PoseModifierHelper::GetDefaultSkeleton(params);
	u32 numDB = defaultSkeleton.m_poseBlenderLookDesc.m_blends.size();
	u32 numRotJoints = defaultSkeleton.m_poseBlenderLookDesc.m_rotations.size();
	u32 numPosJoints = defaultSkeleton.m_poseBlenderLookDesc.m_positions.size();
	...
	//--------------------------------------------------------------------------
	{
		...
		if (m_pAttachmentEyeLeft)
		{
			...
			if (Console::GetInst().ca_DrawLookIK == 2)
			{
				...
				float fTextColor[4] = { 1, 0, 0, 1 };
				g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.4f, fTextColor, false, "EyeLeft: %f %f %f %f  m_IKBlend: %f", q.w, q.v.x, q.v.y, q.v.z, m_blender.m_dataOut.fDirIKInfluence);
				g_YLine += 16.0f;
			}
#endif
		}
		...
		if (m_pAttachmentEyeRight)
		{
			...
			if (Console::GetInst().ca_DrawLookIK == 2)
			{
				...
				float fTextColor[4] = { 1, 0, 0, 1 };
				g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.4f, fTextColor, false, "EyeRight: %f %f %f %f  m_IKBlend: %f", q.w, q.v.x, q.v.y, q.v.z, m_blender.m_dataOut.fDirIKInfluence);
				g_YLine += 16.0f;
			}
#endif
		}
