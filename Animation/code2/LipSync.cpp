...
{
	i32 nPhonemes = DRX_ARRAY_COUNT(s_english_phonemes);
	m_phonemes.reserve(nPhonemes);
	for (i32 i = 0; i < nPhonemes; i++)
	{
		SPhoneme ph;
		memset(ph.ASCII, 0, sizeof(ph.ASCII));
		drx_strcpy(ph.ASCII, s_english_phonemes[i].name);
...
//////////////////////////////////////////////////////////////////////////
i32 CPhonemesLibrary::GetPhonemeCount() const
{
	return m_phonemes.size();
}

/////////////////////////////////////////////////////////////////////////
bool CPhonemesLibrary::GetPhonemeInfo(i32 nIndex, SPhonemeInfo& phoneme)
{
	if (nIndex >= 0 && nIndex < (i32)m_phonemes.size())
	{
...
//////////////////////////////////////////////////////////////////////////
SPhoneme& CPhonemesLibrary::GetPhoneme(i32 nIndex)
{
	assert(nIndex >= 0 && nIndex < (i32)m_phonemes.size());
	return m_phonemes[nIndex];
}

//////////////////////////////////////////////////////////////////////////
void CPhonemesLibrary::LoadPhonemes(tukk filename)
{
}

//////////////////////////////////////////////////////////////////////////
i32 CPhonemesLibrary::FindPhonemeByName(tukk sPhonemeName)
{
	SPhoneme findPhoneme;
	drx_strcpy(findPhoneme.ASCII, sPhonemeName);
	...
	if (it != m_phonemes.end())
		return (i32)(it - m_phonemes.begin());
...
{
	return gEnv->pCharacterUpr->GetIFacialAnimation()->GetPhonemeLibrary();
}

//////////////////////////////////////////////////////////////////////////
bool CFacialSentence::GetPhoneme(i32 index, Phoneme& ph)
{
	if (index < 0 || index >= (i32)m_phonemes.size())
		return false;
...
//////////////////////////////////////////////////////////////////////////
i32 CFacialSentence::AddPhoneme(const Phoneme& ph)
{
	m_phonemes.push_back(ph);
	++m_nValidateID;
	return (i32)m_phonemes.size() - 1;
}

//////////////////////////////////////////////////////////////////////////
bool CFacialSentence::GetWord(i32 index, Word& wrd)
{
	if (index < 0 || index >= (i32)m_words.size())
		return false;
...
//////////////////////////////////////////////////////////////////////////
i32 CFacialSentence::GetPhonemeFromTime(i32 timeMs, i32 nFirst)
{
	if (nFirst > (i32)m_phonemes.size() - 1)
		nFirst = 0;
	if (nFirst < 0)
		nFirst = 0;
	if (nFirst > 0)
	...
	for (i32 i = nFirst; i < (i32)m_phonemes.size(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CFacialSentence::GetPhonemeInfo(i32 phonemeId, SPhonemeInfo& phonemeInfo) const
{
	if (!gEnv->pCharacterUpr->GetIFacialAnimation()->GetPhonemeLibrary()->GetPhonemeInfo(phonemeId, phonemeInfo))
		return false;
	return true;
}
...
{
	CPhonemesLibrary* pPhonemeLib = (CPhonemesLibrary*)gEnv->pCharacterUpr->GetIFacialAnimation()->GetPhonemeLibrary();
	if (bLoading)
	{
		m_phonemes.clear();
		...
		{
			// Loading.
			i32 num = wordsNode->getChildCount();
			for (i32 i = 0; i < num; i++)
			{
		...
		if (phonemesNode)
		{
			...
			while (!key.empty())
			{
				...
				Phoneme ph;
				drx_strcpy(ph.phoneme, sPhoneme);
				ph.intensity = 1;
				ph.time = start;
				ph.endtime = end;
				m_phonemes.push_back(ph);
	...
	else
	{
		...
		// Saving.
		i32 numPhonemes = (i32)m_phonemes.size();
		if (numPhonemes > 0)
		{
...
i32 CFacialSentence::Evaluate(float fTime, float fInputPhonemeStrength, i32 maxSamples, ChannelSample* samples)
{
	i32 numSamples = 0;
	...
	for (i32 phonemeIndex = 0, phonemeCount = GetPhonemeCount(); phonemeIndex < phonemeCount; ++phonemeIndex)
	{
		...
		{
			for (i32 phonemeOffset = -1; phonemeOffset <= 1; ++phonemeOffset)
			{
	...
	for (i32 entryIndex = 0; entryIndex < entryCount; ++entryIndex)
	{
		...
		i32 direction = (timeInMilliseconds > entry.modeTime ? 1 : 0);
...
void CFacialSentence::Animate(const QuatTS& rAnimLocationNext, CFacialAnimationContext* pAnimContext, float fTime, float fInputPhonemeStrength, const VectorSet<CFaceIdentifierHandle, SFaceIdentifierHandleLess>& overriddenPhonemes)
{
	...
	enum {MAX_SAMPLES = DRX_ARRAY_COUNT(samples)};
	i32 sampleCount = Evaluate(fTime, fInputPhonemeStrength, MAX_SAMPLES, samples);
