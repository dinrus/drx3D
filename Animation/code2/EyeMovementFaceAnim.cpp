...
{
	IFacialAnimation* pFaceAnim = g_pCharacterUpr->GetIFacialAnimation();
	if (!pFaceAnim)
		return NULL;
...
tukk CEyeMovementFaceAnim::szEyeBoneNames[] = {
	"eye_left_bone",
	"eye_right_bone"
};

u32 CEyeMovementFaceAnim::ms_eyeBoneNamesCrc[] = { 0, 0 };
	...
	m_bInitialized(false)
{
	...
	{
		for (i32 i = 0; i < EyeCOUNT; ++i)
		{
			tukk const eyeName = szEyeBoneNames[i];
...
u32 CEyeMovementFaceAnim::GetChannelForEffector(EffectorID effector)
{
...
u32 CEyeMovementFaceAnim::CreateChannelForEffector(EffectorID effector)
{
	assert(effector >= 0 && effector < EffectorCOUNT);
	if (effector < 0 || effector >= EffectorCOUNT)
		return ~0;
...
void CEyeMovementFaceAnim::UpdateEye(const QuatTS& rAnimLocationNext, EyeID eye, const QuatT& additionalRotation)
{
	...
	{
		u32 id = GetChannelForEffector(EffectorFromEyeAndDirection(eye, direction));
		if (id != ~0)
	...
	DirectionID directionInterpolateLow = DirectionID(i32(floorf(octantAngle)));
	if (directionInterpolateLow >= DirectionCOUNT)
		directionInterpolateLow = DirectionID(directionInterpolateLow - DirectionCOUNT);

	DirectionID directionInterpolateHigh = DirectionID(i32(ceilf(octantAngle + FLT_MIN)));
	...
	//float fC2[4] = {1,1,0,1};
	//g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.6f, fC2, false,"directionInterpolateLow: %d   directionInterpolateHigh: %d  interpolationFraction: %f",directionInterpolateLow,directionInterpolateHigh,interpolationFraction);
	//g_YLine+=16.0f;

	// Set the weight of the two channels.
	{
		u32 id = GetChannelForEffector(EffectorFromEyeAndDirection(eye, directionInterpolateLow));
		if (id != ~0)
			m_pInstance->GetAnimContext()->SetChannelWeight(id, sinf((1.0f - interpolationFraction) * gf_PI * 0.5f) * strength);
	}
	{
		u32 id = GetChannelForEffector(EffectorFromEyeAndDirection(eye, directionInterpolateHigh));
		if (id != ~0)
...
{
	CAttachmentUpr* pMasterAttachmentUpr = (CAttachmentUpr*)m_pInstance->GetMasterCharacter()->GetIAttachmentUpr();
	if (pMasterAttachmentUpr)
	{
		CCharInstance* pMasterCharacterInstance = pMasterAttachmentUpr->m_pSkelInstance;
		CDefaultSkeleton& rDefaultSkeleton = *pMasterAttachmentUpr->m_pSkelInstance->m_pDefaultSkeleton;
		for (EyeID eye = EyeID(0); eye < EyeCOUNT; eye = EyeID(eye + 1))
...
void CEyeMovementFaceAnim::FindLookAngleAndStrength(EyeID eye, float& angle, float& strength, const QuatT& additionalRotation)
{
	...
	i32 aidx = -1;
	if (eye == 0)
		aidx = pMasterAttachmentUpr->GetIndexByName("eye_left");
	if (eye == 1)
		aidx = pMasterAttachmentUpr->GetIndexByName("eye_right");
	if (aidx >= 0)
	{
		pAttachment = (CAttachmentBONE*)pMasterAttachmentUpr->GetInterfaceByIndex(aidx);
	...
	//	float fC1[4] = {1,0,0,1};
	//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.6f, fC1, false,"x: %f  y: %f  z: %f    angle:%f strength:%f",forward.x,forward.y,forward.z,angle,strength);
	//	g_YLine+=16.0f;
}
...
void CEyeMovementFaceAnim::DisplayDebugInfoForEye(const QuatTS& rAnimLocationNext, EyeID eye, const string& text)
{
	...
	IRenderAuxText::DrawLabelEx(pos, 2.0f, color, true, true, text.c_str());
}
...
void CEyeMovementFaceAnim::CalculateEyeAdditionalRotation(CCharInstance* pCharacter, CFaceState* pFaceState, CFacialEffectorsLibrary* pEffectorsLibrary, QuatT additionalRotation[EyeCOUNT])
{
	...
	{
		i32 nNumIndices = pFaceState->GetNumWeights();
				...
				case EFE_TYPE_BONE:
					{
						...
						for (EyeID eye = EyeID(0); eye < EyeCOUNT; eye = EyeID(eye + 1))
						{
							u32k bonEffectorBoneCrc = pBoneEffector->GetAttachmentCRC();
							u32k eyeCrc = ms_eyeBoneNamesCrc[eye];
...
void CEyeMovementFaceAnim::GetMemoryUsage(IDrxSizer* pSizer) const
{
