...
void GatherControllers(const GlobalAnimationHeaderCAF& rGAH, const Command::CState& state, IController** controllers)
{
	...
	if (rGAH.m_nControllers2 > 0 && rGAH.m_nControllers == 0)
	{
		...
		{
			const auto itSearch = std::find_if(g_AnimationUpr.m_arrGlobalHeaderDBA.begin(), g_AnimationUpr.m_arrGlobalHeaderDBA.end(), [&rGAH](const CGlobalHeaderDBA& dbaHeader)
			{
				return dbaHeader.m_FilePathDBACRC32 == rGAH.m_FilePathDBACRC32;
			});

			if (itSearch != g_AnimationUpr.m_arrGlobalHeaderDBA.end())
			{
	...
	{
		jointList.first = DrxStackAllocVector(CDefaultSkeleton::SJointDescriptor, state.m_jointCount, alignof(CDefaultSkeleton::SJointDescriptor));
	...
	i32k forcedLod = Console::GetInst().ca_ForceAnimationLod;
	const std::pair<u32k*, u32k*> lodMask = state.m_pDefaultSkeleton->FindClosestAnimationLod(forcedLod < 0 ? state.m_lod : forcedLod);
	const std::pair<u32k*, u32k*> stateMask = { state.m_pJointMask, state.m_pJointMask + state.m_jointMaskCount };

	struct CrcOderingComparator
	{
		bool operator()(const CDefaultSkeleton::SJointDescriptor& lhs, u32k& rhs) const { return lhs.crc32 < rhs; }
		bool operator()(u32k& lhs, const CDefaultSkeleton::SJointDescriptor& rhs) const { return lhs < rhs.crc32; }
	};
...
void ClearPoseBuffer::Execute(const CState& state, CEvaluationContext& context) const
{
	...
	for (u32 j = 0; j < state.m_jointCount; ++j)
	{
...
void SampleAddAnimFull::Execute(const CState& state, CEvaluationContext& context) const
{
	...
	const auto& rCAF = [&]() -> const GlobalAnimationHeaderCAF&
	{
		...
		return g_AnimationUpr.m_arrGlobalCAF[pMAG->m_nGlobalAnimId];
	} ();
	...
	i32k bufferIndex = (m_flags & Flag_TmpBuffer) ? Command::TmpBuffer : Command::TargetBuffer;
	...
	u32k startingJointIndex = (m_flags & Flag_ADMotion) ? (1) : (0);
	...
	{
		DRX_PROFILE_REGION(PROFILE_ANIMATION, "SampleAddAnimFull::Execute:UpdatePoseAdd");

		for (u32 j = startingJointIndex; j < state.m_jointCount; ++j)
		{
	...
	{
		DRX_PROFILE_REGION(PROFILE_ANIMATION, "SampleAddAnimFull::Execute:UpdatePose");
	...
	g_pCharacterUpr->g_SkeletonUpdates++;
...
void SampleAddPoseFull::Execute(const CState& state, CEvaluationContext& context) const
{
	...
	const auto& rGlobalAnimHeaderAIM = [&]() -> const GlobalAnimationHeaderAIM&
	{
		...
		return g_AnimationUpr.m_arrGlobalAIM[pMAG->m_nGlobalAnimId];
	...
	for (u32 i = 0; i < state.m_jointCount; ++i)
	{
		parrController[i] = rGlobalAnimHeaderAIM.GetControllerByJointCRC32(pModelJoint[i].m_nJointCRC32);
	}

	i32k nBufferID = (m_flags& SampleAddAnimFull::Flag_TmpBuffer) ? Command::TmpBuffer : Command::TargetBuffer;
...
void AddPoseBuffer::Execute(const CState& state, CEvaluationContext& context) const
{
	...
	u32 numJoints = state.m_jointCount;
...
void SampleAddAnimPart::Execute(const CState& state, CEvaluationContext& context) const
{
	...
	if (rCAF.IsAssetAdditive())
	{
		for (u32 j = 1; j < state.m_jointCount; ++j)
		{
			if (parrController[j])
			{
				Quat rot;
...
void PoseModifier::Execute(const CState& state, CEvaluationContext& context) const
{
	...
	/*
	   #ifdef _DEBUG
	   for (u32 j=0; j<params.jointCount; j++)
	   {
...
void PrepareAllRedirectedTransformations::Execute(const CState& state, CEvaluationContext& context) const
{
	...
	state.m_pInstance->m_AttachmentUpr.PrepareAllRedirectedTransformations(*state.m_pPoseData);
}
...
void GenerateProxyModelRelativeTransformations::Execute(const CState& state, CEvaluationContext& context) const
{
	...
	state.m_pInstance->m_AttachmentUpr.GenerateProxyModelRelativeTransformations(*state.m_pPoseData);
}
...
void ProcessAnimationDrivenIkFunction(CCharInstance& instance, IAnimationPoseData* pAnimationPoseData, const QuatTS location)
{
	...
	i32 jointCount = i32(pPoseData->GetJointCount());
	...
	for (uint i = 0; i < targetCount; i++)
	{
		...
		i32 targetJointIndex = modelSkeleton.m_ADIKTargets[i].m_idxTarget;
		assert(targetJointIndex < (i32)jointCount);
		if (targetJointIndex < 0)
			continue;

		i32 targetWeightJointIndex = modelSkeleton.m_ADIKTargets[i].m_idxWeight;
		assert(targetWeightJointIndex < (i32)jointCount);
		if (targetWeightJointIndex < 0)
			continue;

		i32 limbDefinitionIndex = modelSkeleton.GetLimbDefinitionIdx(nHandle);
		...
		if (targetBlendWeight > 0.01f)
		{
			...
			i32 endEffectorJointIndex = limbType.m_arrJointChain[numLinks - 1].m_idxJoint;
			i32 endEffectorParentJointIndex = modelSkeleton.m_arrModelJoints[endEffectorJointIndex].m_idxParent;

			pPoseAbsolute[0] = pPoseRelative[0];
			ANIM_ASSET_ASSERT(pPoseRelative[0].q.IsUnit());
			for (uint j = 1; j < numLinks; ++j)
			{
				i32 p = limbType.m_arrRootToEndEffector[j - 1];
				i32 c = limbType.m_arrRootToEndEffector[j];
			...
			i16k* pJointIdx = &modelSkeleton.m_IKLimbTypes[limbDefinitionIndex].m_arrLimbChildren[0];
			for (uint u = 0; u < numChilds; u++)
			{
				i32 c = pJointIdx[u];
				i32 p = modelSkeleton.m_arrModelJoints[c].m_idxParent;
				pPoseAbsolute[c] = pPoseAbsolute[p] * pPoseRelative[c];
#ifdef _DEBUG
				ANIM_ASSET_ASSERT(pPoseAbsolute[c].q.IsUnit());
		...
		{
			tukk pName = modelSkeleton.m_ADIKTargets[i].m_strTarget;
