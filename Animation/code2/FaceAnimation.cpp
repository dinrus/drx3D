...
//////////////////////////////////////////////////////////////////////////
u32 CFacialAnimationContext::StartChannel(SFacialEffectorChannel& channel)
{
...
//////////////////////////////////////////////////////////////////////////
u32 CFacialAnimationContext::FadeInChannel(IFacialEffector* pEffector, float fWeight, float fFadeTime, float fLifeTime, i32 nRepeatCount)
{
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimationContext::StopChannel(u32 nChannelId, float fFadeTime)
{
	u32 nChannels = m_channels.size();
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimationContext::RemoveChannel(u32 nChannelId)
{
	u32 nChannels = (u32)m_channels.size();
	for (u32 i = 0; i < nChannels; ++i)
	{
		if (m_channels[i].nChannelId == nChannelId)
		{
...
{
	i32 nChannels = (i32)m_channels.size();
	for (i32 i = nChannels - 1; i >= 0; i--)
	{
		m_bForceLastUpdate = true;
...
//////////////////////////////////////////////////////////////////////////
SFacialEffectorChannel* CFacialAnimationContext::GetChannel(u32 nChannelId)
{
	u32 nChannels = (u32)m_channels.size();
	for (u32 i = 0; i < nChannels; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimationContext::SetChannelWeight(u32 nChannelId, float fWeight)
{
...
bool CFacialAnimationContext::Update(CFaceState& faceState, const QuatTS& rAnimLocationNext)
{
	...
	for (u32 nCurChannel = 0; nCurChannel < m_channels.size(); )
	{
		SFacialEffectorChannel& channel = m_channels[nCurChannel];
		DrxPrefetch(channel.pEffector);
	...
	IF_UNLIKELY (Console::GetInst().ca_DebugFacial == 2)
	{
		...
		if (pChar)
		{
			...
			i16 id = pChar->GetIDefaultSkeleton().GetJointIDByName("Bip01 Head");
			if (id >= 0)
	...
	// Update the amount of time left for bone rotation smoothing.
	{
		...
		IF_UNLIKELY (Console::GetInst().ca_DebugFacial)
		{
			if (oldBoneRotationSmoothingRemainingTime)
			{
				...
				{
					DrxLogAlways("CFacialAnimationContext::Update(this=%p) disabling bone rotation smoothing.", this);
				}
			}
		}
	}
...
{
	i32k numWeights = faceState.GetNumWeights();
...
{
	i32 nNumWeights = faceState.GetNumWeights();
	for (i32 i = 0; i < nNumWeights; i++)
	{
...
void CFacialAnimationContext::ApplyEffector(CFaceState& faceState, CFacialEffector* pEffector, float fWeight, float fBalance, bool bIsLipSync)
{
	...
	i32 nSubEff = pEffector->GetSubEffectorCount();
	for (i32 i = 0; i < nSubEff; i++)
	{
...
void CFacialAnimationContext::UpdatePlayingSequences(const QuatTS& rAnimLocationNext)
{
	...
	i32 numSequences = (i32)m_sequences.size();
	...
	{
		i32 toStopSize = (i32)tostop.size();
		for (i32 i = 0; i < toStopSize; i++)
		{
...
void CFacialAnimationContext::PlaySequence(CFacialAnimSequence* pSequence, bool bExclusive, bool bLooping)
{
	...
	IF_UNLIKELY (Console::GetInst().ca_DebugFacial)
	{
		DrxLogAlways("Play Facial Sequence %s", pSequence->GetName());
	}
}
...
void CFacialAnimationContext::StopSequence(CFacialAnimSequence* pSequence)
{
	...
	{
		tukk szSequenceName = pSequence ? pSequence->GetName() : "NULL";
		DrxLogAlways("Stop Facial Sequence %s", szSequenceName);
	}
}
...
void CFacialAnimationContext::StopAllSequences()
{
	...
	IF_UNLIKELY (Console::GetInst().ca_DebugFacial)
	{
		DrxLogAlways("Stop All Facial Sequences");
	}
}
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimation::ClearEffectorsLibraryFromCache(tukk filename)
{
	stack_string file(filename);
	PathUtil::ToUnixPath(file);

	i32 libIndex = -1;
	for (i32 i = 0, end = (i32)m_effectorLibs.size(); i < end; i++)
	{
...
//////////////////////////////////////////////////////////////////////////
IFacialEffectorsLibrary* CFacialAnimation::LoadEffectorsLibrary(tukk filename)
{
	stack_string file(filename);
	PathUtil::ToUnixPath(file);
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimation::RenameAnimSequence(CFacialAnimSequence* pSeq, tukk sNewName, bool addToCache)
{
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimation::ClearSequenceFromCache(tukk filename)
{
	m_loadedSequences.erase(filename);
}

//////////////////////////////////////////////////////////////////////////
IFacialAnimSequence* CFacialAnimation::LoadSequence(tukk filename, bool bNoWarnings, bool addToCache)
{
...
IFacialAnimSequence* CFacialAnimation::StartStreamingSequence(tukk filename)
{
	const string file = PathUtil::ReplaceExtension(PathUtil::ToUnixPath(filename), FACIAL_SEQUENCE_EXT);
...
//////////////////////////////////////////////////////////////////////////
IFacialAnimSequence* CFacialAnimation::FindLoadedSequence(tukk filename) const
{
	const string file = PathUtil::ReplaceExtension(PathUtil::ToUnixPath(filename), FACIAL_SEQUENCE_EXT);
...
//////////////////////////////////////////////////////////////////////////
IJoystickSet* CFacialAnimation::LoadJoystickSet(tukk filename, bool bNoWarnings)
{
...
void CFacialAnimation::BindJoystickSetToSequence(IJoystickSet* pJoystickSet, IFacialAnimSequence* pSequence)
{
	...
	if (pFaceJoystickSet && pFacialAnimation)
	{
		...
		public:
			...
			virtual IFacialAnimChannel* FindChannel(tukk szName)
			{
		...
		ChannelMap channelMap;
		{
			...
			for (i32 i = 0; i < pSequence->GetChannelCount(); ++i)
			{
				...
				tukk szChannelName = pChannel ? pChannel->GetName() : 0;
				szChannelName = szChannelName ? szChannelName : "";
...
#else
	DrxFatalError("Not Supporeted on this platform!");
#endif
}

void CFacialAnimation::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
CFaceIdentifierHandle CFacialAnimation::CreateIdentifierHandle(tukk identifierString)
{
...
void CFacialAnimationContext::GetMemoryUsage(IDrxSizer* pSizer) const
{
	pSizer->AddObject(this, sizeof(*this));
