...
//-----------------------------------------------------------------------------------------------------
bool SDirectionalBlender::ExecuteDirectionalIK(const SAnimationPoseModifierParams& params, const DirectionalBlends* rDirBlends, u32k numDB, const SJointsAimIK_Rot* rRot, u32k numRotJoints, const SJointsAimIK_Pos* rPos, u32k numPosJoints)
{
	...
	u32k LR = isneg(LR_Plane | m_dataIn.vDirIKTarget);
	u32k FB = isneg(FB_Plane | m_dataIn.vDirIKTarget);
	...
	//------------------------------------------------------------------------------------
	u32k clampedNumDB = min(numDB, (u32)MAX_NUM_DB);
	for (u32 d = 0; d < clampedNumDB; d++)
	{
	...
	{
		i32k parameterJoint0Idx = m_dbw[0].m_nParaJointIdx;
	...
	for (i32 i = 0; i < m_numActiveDirPoses; i++)
	...
	{
		i32 j = pAimIK_Rot[r].m_nJointIdx;
	...
	f32 fTotalWeight = t0;
	const GlobalAnimationHeaderAIM* parrGlobalAIM = &g_AnimationUpr.m_arrGlobalAIM[0];
	for (i32 a = 0; a < m_numActiveDirPoses; a++)
	{
		assert(m_DirInfo[a].m_nGlobalDirID0 >= 0);
		const GlobalAnimationHeaderAIM& rAIM = parrGlobalAIM[m_DirInfo[a].m_nGlobalDirID0];
		u32k nAnimTokenCRC32 = rAIM.m_AnimTokenCRC32;
		const f32 fAimPoseWeight = m_DirInfo[a].m_fWeight;
		i32k polarCoordinatesIndex = GetOrAssignPolarCoordinateSmoothIndex(m_DirInfo[a].m_nGlobalDirID0);
		for (u32 d = 0; d < clampedNumDB; d++)
		{
	...
	{
		i32 nDirJointIdx = m_dbw[g].m_nParaJointIdx;
...
void SDirectionalBlender::AccumulateAimPoses(const SAnimationPoseModifierParams& params, const GlobalAnimationHeaderAIM& rAim, const QuatT* arrRelPose, const f32 fIKBlend, const Vec3& vLocalAimIKTarget, i32k nParaJointIdx, i32k nRotParaJointIdx, i32k nStartJointIdx, i32k nRotStartJointIdx, const SJointsAimIK_Rot* rRot, u32k numRotJoints, const SJointsAimIK_Pos* rPos, const f32 CosineY, u32k sides, u32k polarCoordinatesIndex)
{
	...
	u32k numAimPosesCAF = rAim.m_arrAimIKPosesAIM.size();
	assert(numAimPosesCAF == 9 || numAimPosesCAF == 15 || numAimPosesCAF == 21);
	if (numAimPosesCAF == 0)
		return;

	u32k AimPoseMid = u32(numAimPosesCAF / 2);
	...
	if (needToCalculatePolarCoordinates)
	{
		...
		for (u32 r = 0; r < numRotJoints; ++r)
		{
			...
			{
				i32k jointIdx = rotJoint.m_nJointIdx;

				i32k parentJointIdx = parrModelJoints[jointIdx].m_idxParent;
				qtemp = arrRelPose[r];
				if (rAim.m_nExist & (uint64(1) << r))
				{
					...
					i32k posIdx = rotJoint.m_nPosIndex;
				...
				i32k rotJointParentIndex = rotJoint.m_nRotJointParentIdx;
		...
		if (Console::GetInst().ca_DrawAimIKVEGrid)
		{
			...
			u32k colorIndex = min(polarCoordinatesIndex, MAX_POLAR_COORDINATES_SMOOTH);
	...
	f32 weights[256];
	for (u32 i = 0; i < numAimPosesCAF; i++)
		weights[i] = 0;
		...
		if (w)
		{
			...
			for (u32 r = 0; r < numRotJoints; r++)
			{
				...
				if (rAim.m_nExist & (uint64(1) << r))
				{
					...
					i32k posIdx = rRot[r].m_nPosIndex;
	...
	/*
	   ...
	   i32 j	= pAimIK_Pos[p].m_nJointIdx;
...
void SDirectionalBlender::DebugVEGrid(const SAnimationPoseModifierParams& params, const SDirInfo& rAimInfo, const f32 fAimPoseWeight, const f32 fIKBlend, i32k widx, const SJointsAimIK_Rot* rRot, u32k numRotJoints, const SJointsAimIK_Pos* rPos, u32k numPosJoints)
{
	//VExampleInit ve;
	float fTextColor[4] = { 1, 0, 0, 1 };
	...
	const GlobalAnimationHeaderAIM& rAIM0 = g_AnimationUpr.m_arrGlobalAIM[rAimInfo.m_nGlobalDirID0];

	QuadIndices arrQuat[MAX_JOINT_AMOUNT];
	u32 numAimPoses = u32(rAIM0.m_fTotalDuration * ANIMATION_30Hz + 1.1f);
	u32 numQuats = rAIM0.Debug_AnnotateExamples2(numAimPoses, arrQuat);
	...
	drxMemcpy(arrRelPose0, pDefaultSkeleton->m_poseDefaultData.GetJointsRelativeMain(), numJoints * sizeof(QuatT));
	...
	drxMemcpy(arrRelPose1, pDefaultSkeleton->m_poseDefaultData.GetJointsRelativeMain(), numJoints * sizeof(QuatT));
	for (u32 i = 0; i < numJoints; i++)
	...
	drxMemcpy(arrRelPose2, pDefaultSkeleton->m_poseDefaultData.GetJointsRelativeMain(), numJoints * sizeof(QuatT));
	for (u32 i = 0; i < numJoints; i++)
	...
	drxMemcpy(arrRelPose3, pDefaultSkeleton->m_poseDefaultData.GetJointsRelativeMain(), numJoints * sizeof(QuatT));
	for (u32 i = 0; i < numJoints; i++)
	...
	drxMemcpy(arrRelPose, pDefaultSkeleton->m_poseDefaultData.GetJointsRelativeMain(), numJoints * sizeof(QuatT));
	for (u32 i = 0; i < numJoints; i++)
	...
	for (u32 aq = 0; aq < numQuats; aq++)
	{
		...
		for (f32 i = step0; i < 3.0f; i += step0)
		{
			...
			col2.r = u8(arrQuat[aq].col.r * fAimPoseWeight);
			col2.g = u8(arrQuat[aq].col.g * fAimPoseWeight);
			col2.b = u8(arrQuat[aq].col.b * fAimPoseWeight);
	...
	for (i32 y = 0; y < CHUNK_GAHAIM_INFO::YGRID; y++)
	{
		for (i32 x = 0; x < CHUNK_GAHAIM_INFO::XGRID; x++)
		{
			...
			{
				i32 i0 = rAIM0.m_PolarGrid[y * CHUNK_GAHAIM_INFO::XGRID + x].i0;
				i32 i1 = rAIM0.m_PolarGrid[y * CHUNK_GAHAIM_INFO::XGRID + x].i1;
				i32 i2 = rAIM0.m_PolarGrid[y * CHUNK_GAHAIM_INFO::XGRID + x].i2;
				i32 i3 = rAIM0.m_PolarGrid[y * CHUNK_GAHAIM_INFO::XGRID + x].i3;
				Vec4 w;
	...
	{
		i32 i0 = rAIM0.VE2[i].i0;
		i32 i1 = rAIM0.VE2[i].i1;
		i32 i2 = rAIM0.VE2[i].i2;
		i32 i3 = rAIM0.VE2[i].i3;
		Vec2 polcor;
