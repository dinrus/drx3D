...
#include <drx3D/Animation/FaceJoystick.h>
#include <drx3D/Animation/FaceChannelKeyCleanup.h>
#include <drx3D/Animation/FaceChannelSmoothing.h>
...
//////////////////////////////////////////////////////////////////////////
CFacialAnimChannel::CFacialAnimChannel(i32 index)
{
	m_nFlags = 0;
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimChannel::DeleteInterpolator(i32 i)
{
	if (i >= 0 && i < i32(m_splines.size()))
...
void CFacialAnimChannel::SetName(tukk name)
{
	m_name = g_pCharacterUpr->GetFacialAnimation()->CreateIdentifierHandle(name);
}
#endif
...
//////////////////////////////////////////////////////////////////////////
bool CFacialAnimSequence::StartStreaming(tukk sFilename)
{
	if (m_pStream)
	{
		m_pStream->Abort();
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimSequence::SetName(tukk sNewName)
{
	m_pFaceAnim->RenameAnimSequence(this, sNewName);
};

//////////////////////////////////////////////////////////////////////////
IFacialAnimChannel* CFacialAnimSequence::GetChannel(i32 nIndex)
{
	assert(nIndex >= 0 && nIndex < (i32)m_data.m_channels.size());
...
void CFacialAnimSequence::RemoveChannel(IFacialAnimChannel* pChannelToRemove)
{
	if (pChannelToRemove)
	{
		...
		for (i32 i = 0, end = (i32)m_data.m_channels.size(); i < end; ++i)
		{
...
//////////////////////////////////////////////////////////////////////////
i32 CFacialAnimSequence::GetSoundEntryCount()
{
	return m_data.m_soundEntries.size();
}

//////////////////////////////////////////////////////////////////////////
void CFacialAnimSequence::InsertSoundEntry(i32 index)
{
	if (index < 0 || index > i32(m_data.m_soundEntries.size()))
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimSequence::DeleteSoundEntry(i32 index)
{
	if (index < 0 || index > i32(m_data.m_soundEntries.size()))
...
//////////////////////////////////////////////////////////////////////////
IFacialAnimSoundEntry* CFacialAnimSequence::GetSoundEntry(i32 index)
{
	return ((index >= 0 && index < i32(m_data.m_soundEntries.size())) ? &m_data.m_soundEntries[index] : 0);
}
...
void CFacialAnimSequence::Animate(const QuatTS& rAnimLocationNext, CFacialAnimSequenceInstance* pInstance, float fTime)
{
	if (IsInMemory())
	{
		...
		// MichaelS - Loop through balance channels and evaluate them first.
		{
			...
			i32* stateIndices = 0;
			u32 num = pInstance->m_balanceChannelStateIndices.size();
			if (num)
				stateIndices = &pInstance->m_balanceChannelStateIndices[0];

			u32 end = pInstance->m_balanceChannelEntries.size();
			for (u32 i = 0; i < end; ++i)
			{
				...
				for (i32 stateIndexIndex = 0; stateIndexIndex < entry.nMorphIndexCount; ++stateIndexIndex)
				{
					assert(stateIndices != 0 && !"Zero Pointer Crash");
					i32 stateIndex = stateIndices[entry.nMorphIndexStartIndex + stateIndexIndex];
		...
		i32 numChannels = (i32)m_data.m_channels.size();
			...
			if (flags & IFacialAnimChannel::FLAG_LIPSYNC_CATEGORY_STRENGTH)
			{
				...
				if (pEffector)
				{
					for (size_t subEffectorIndex = 0, subEffectorCount = pEffector->GetSubEffectorCount(); subEffectorIndex < subEffectorCount; ++subEffectorIndex)
					{
						...
						i32 index = pSubEffector->GetIndexInState();
						if (index >= 0)
		...
		for (i32 i = 0; i < CProceduralChannelSet::ChannelType_count; i++)
		{
		...
		if (bHasBakedLipsynch)
		{
			...
			for (i32 channelIndex = 0, channelCount = (i32)m_data.m_channels.size(); channelIndex < channelCount; ++channelIndex)
			{
		...
		for (i32 i = 0, count = m_data.m_soundEntries.size(); i < count; ++i)
		{
...
void CFacialAnimSequence::SerializeLoad(Data& data, XmlNodeRef& xmlNode, ESerializationFlags flags)
{
	...
	for (i32 i = 0; i < xmlNode->getChildCount(); i++)
	{
		...
		else if (childNode->isTag("SoundEntry"))
		{
			if (flags & SFLAG_SOUND_ENTRIES)
			{
				i32 soundEntry = i32(data.m_soundEntries.size());
		...
		else if (childNode->isTag("SkeletonAnimationEntry"))
		{
			if (flags & SFLAG_ANIMATION)
			{
				i32 skeletonAnimationEntry = i32(data.m_skeletonAnimationEntries.size());
				data.m_skeletonAnimationEntries.resize(data.m_skeletonAnimationEntries.size() + 1);
				tukk szAnimationName = childNode->getAttr("AnimationName");
...
void CFacialAnimSequence::SerializeChannelLoad(Data& data, IFacialAnimChannel* pChannel, XmlNodeRef& node)
{
	...
	CFacialAnimation* pFaceAnim = g_pCharacterUpr->GetFacialAnimation();
	if (!pFaceAnim)
		return;

	i32 flags = 0;
	node->getAttr("Flags", flags);
	tukk sName = node->getAttr("Name");
	...
	else
	{
		...
		// This is an effector.
		i32 splineCount = 0;
		for (i32 childIndex = 0, childCount = node->getChildCount(); childIndex < childCount; ++childIndex)
		{
...
void CFacialAnimSequence::SerializeChannelSave(IFacialAnimChannel* pChannel, XmlNodeRef& node)
{
	...
	{
		for (i32 j = 0; j < (i32)m_data.m_channels.size(); j++)
		{
...
//////////////////////////////////////////////////////////////////////////
i32 CFacialAnimSequence::GetSkeletonAnimationEntryCount()
{
	return i32(m_data.m_skeletonAnimationEntries.size());
}

//////////////////////////////////////////////////////////////////////////
void CFacialAnimSequence::InsertSkeletonAnimationEntry(i32 index)
{
	if (index < 0 || index > i32(m_data.m_skeletonAnimationEntries.size()))
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimSequence::DeleteSkeletonAnimationEntry(i32 index)
{
	if (index < 0 || index >= i32(m_data.m_skeletonAnimationEntries.size()))
...
//////////////////////////////////////////////////////////////////////////
IFacialAnimSkeletonAnimationEntry* CFacialAnimSequence::GetSkeletonAnimationEntry(i32 index)
{
	if (index < 0 || index >= i32(m_data.m_skeletonAnimationEntries.size()))
	{
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimSequence::SetJoystickFile(tukk joystickFile)
{
	m_data.m_joystickFile = joystickFile;
}

//////////////////////////////////////////////////////////////////////////
tukk CFacialAnimSequence::GetJoystickFile() const
{
...
void CFacialAnimSequence::Serialize(XmlNodeRef& xmlNode, bool bLoading, ESerializationFlags flags)
{
	...
	else
	{
		...
		if (flags & SFLAG_SOUND_ENTRIES)
		{
			for (i32 soundEntry = 0, soundEntryCount = m_data.m_soundEntries.size(); soundEntry < soundEntryCount; ++soundEntry)
			{
		...
		if (flags & SFLAG_ANIMATION)
		{
			for (i32 skeletonAnimationEntry = 0, skeletonAnimationCount = m_data.m_skeletonAnimationEntries.size(); skeletonAnimationEntry < skeletonAnimationCount; ++skeletonAnimationEntry)
			{
...
//////////////////////////////////////////////////////////////////////////
void CFacialAnimSequence::MergeSequence(IFacialAnimSequence* pMergeSequence, const Functor1wRet<tukk , MergeCollisionAction>& collisionStrategy)
{
#ifdef FACE_STORE_ASSET_VALUES
	if (this == pMergeSequence)
	{
		DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "Attempting to merge sequence with itself - aborting.");
		return;
	}

	if (!pMergeSequence)
	{
		DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "No merge sequence supplied (NULL) - aborting.");
		return;
	}
	...
	for (i32 instance = 0; instance < 2; ++instance)
	{
	...
	// Merge in the channels.
	{
		...
		for (i32 channelIndex = 0; channelIndex < pMergeSequence->GetChannelCount(); ++channelIndex)
		{
			...
			if (pTargetChannel && copyChannel)
			{
				...
				if (pMergeSpline && pTargetSpline)
				{
					...
					for (i32 key = 0; key < pMergeSpline->GetKeyCount(); ++key)
					{
						ISplineInterpolator::ValueType value;
						pMergeSpline->GetKeyValue(key, value);
						i32 originalKeyIndex = pTargetSpline->InsertKey(FacialEditorSnapTimeToFrame(pMergeSpline->GetKeyTime(key)), value);
	...
	// Update the parents of the channels.
	{
		...
		typedef VectorMap<IFacialAnimChannel*, i32> ChannelIndexMap;
		ChannelIndexMap channelIndexMap;
		{
			...
			for (i32 i = 0; i < pMergeSequence->GetChannelCount(); ++i)
...
#else
	DrxFatalError("Not available on this platform");
#endif
}
...
void CFacialAnimSequence::StreamAsyncOnComplete(IReadStream* pStream, unsigned nError)
{
	...
	tukk buf = (tukk) pStream->GetBuffer();
	u32 lengthBuf = pStream->GetBytesRead();
...
void CFacialAnimSequence::GenerateProceduralChannels(Data& data)
{
	...
	for (i32 soundEntry = 0, soundEntryCount = data.m_soundEntries.size(); soundEntry < soundEntryCount; ++soundEntry)
	{
		...
		for (i32 wordIndex = 0; wordIndex < data.m_soundEntries[soundEntry].m_pSentence->GetWordCount(); ++wordIndex)
		{
			...
			float fRandLeftRight = drx_random(-1.0f, 1.0f);
			...
			float fRandUpDown = drx_random(-1.0f, 1.0f);
...
void CFacialAnimSequence::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void CFacialAnimSoundEntry::SetSoundFile(tukk sSoundFile)
{
	m_sound = sSoundFile;
}

tukk CFacialAnimSoundEntry::GetSoundFile()
{
...
void CFacialAnimSkeletalAnimationEntry::SetName(tukk skeletonAnimationFile)
{
	m_animationName = skeletonAnimationFile;
}

tukk CFacialAnimSkeletalAnimationEntry::GetName() const
{
...
{
public:
	EffectorChannelPair(IFacialEffector* pEffector, i32 channelIndex) : pEffector(pEffector), channelIndex(channelIndex) {}
...
void CFacialAnimSequenceInstance::BindChannels(CFacialAnimationContext* pContext, CFacialAnimSequence* pSequence)
{
	...
	u32 numChannels = pSequence->GetChannelCount();
	...
	i32 rootBalanceChannelEntryIndex = -1;
	for (u32 i = 0; i < numChannels; i++)
	{
	...
	for (u32 i = 0; i < numChannels; ++i)
	{
		...
		{
			CFacialAnimChannel* pParentChannel = (CFacialAnimChannel*)pSeqChannel->GetParent();
			i32 nBalanceChannelListIndex = rootBalanceChannelEntryIndex;
			while (pParentChannel)
			{
				assert(pParentChannel->GetInstanceChannelId() >= 0 && pParentChannel->GetInstanceChannelId() < i32(m_channels.size()));
				ChannelInfo& parentInfo = m_channels[pParentChannel->GetInstanceChannelId()];
	...
	if (bFoundCategoryBalance)
	{
		...
		for (i32 channelIndex = 0, end = i32(m_channels.size()); channelIndex < end; ++channelIndex)
		...
		for (i32 balanceChannelIndex = 0; balanceChannelIndex < i32(m_balanceChannelEntries.size()); ++balanceChannelIndex)
		{
			...
			if (m_channels[m_balanceChannelEntries[balanceChannelIndex].nChannelIndex].pEffector)
			{
				...
				entry.nMorphIndexStartIndex = i32(m_balanceChannelStateIndices.size());
				entry.nMorphIndexCount = 0;

				class Recurser
				{
				public:
					Recurser(EffectorMultiMap& effectorChannelMultiMap, Channels& channels, i32 balanceChannelIndex, std::vector<i32>& balanceChannelStateIndices)
					...
					void operator()(IFacialEffector* pEffector)
					{
						...
						for (i32 subEffectorIndex = 0; subEffectorIndex < pEffector->GetSubEffectorCount(); ++subEffectorIndex)
				...
				private:
					EffectorMultiMap& effectorChannelMultiMap;
					Channels&         channels;
					i32               balanceChannelIndex;
					std::vector<i32>& balanceChannelStateIndices;
				};
				...
				entry.nMorphIndexCount = i32(m_balanceChannelStateIndices.size()) - entry.nMorphIndexStartIndex;
			}
		}
	}
...
void CFacialAnimSequenceInstance::UnbindChannels()
{
	if (!m_pAnimContext)
		return;
	i32 numChannels = (i32)m_channels.size();
	for (i32 i = 0; i < numChannels; i++)
	{
		ChannelInfo& chinfo = m_channels[i];
...
void CFacialAnimSequenceInstance::UnbindProceduralChannels()
{
	if (!m_pAnimContext)
		return;
	i32 numChannels = (i32)m_proceduralChannels.size();
	for (i32 i = 0; i < numChannels; i++)
	{
		ChannelInfo& chinfo = m_proceduralChannels[i];
		if (chinfo.nChannelId != INVALID_CHANNEL_ID)
...
void CFacialAnimSequenceInstance::GetMemoryUsage(IDrxSizer* pSizer) const
{
