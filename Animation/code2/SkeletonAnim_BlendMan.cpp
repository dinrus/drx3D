...
tukk AnimFlags(u32 flags)
{
	...
	for (i32 i = 0; result[i] = codes[i]; ++i)
...
/////////////////////////////////////////////////////////////////////////////////
u32 CSkeletonAnim::BlendUpr(f32 fDeltaTime, DynArray<CAnimation>& arrAFIFO, u32 nLayer)
{
	DEFINE_PROFILER_FUNCTION();

	u32 NumAnimsInQueue = arrAFIFO.size();
	...
	if (!arrAnimFiFo[0].IsActivated())
	{
		...
		{
			u32 counter = 0;
			u32 num = pParametric->m_numExamples;
			for (u32 a = 0; a < num; a++)
			{
				i32 nAnimID = pParametric->m_nAnimID[a];
	...
	//	u32 nMaxActiveInQueue=2;
	u32 nMaxActiveInQueue = MAX_EXEC_QUEUE;
	...
	u32 aq = EvaluateTransitionFlags(arrAnimFiFo, NumAnimsInQueue);
...
#ifdef _DEBUG
	for (u32 a = 0; a < nMaxActiveInQueue; a++)
	{
		assert(arrAnimFiFo[a].IsActivated());
		f32 t = arrAnimFiFo[a].m_fAnimTime[0];
		assert(t >= 0.0f && t <= 1.0f);  //most likely this is coming from outside if DinrusXAnimation
	}
#endif
...
#ifdef _DEBUG
	for (u32 j = 0; j < nMaxActiveInQueue; j++)
	{
	...
	for (u32 i = 0; i < nMaxActiveInQueue; i++)
	...
	if (fTotal > 0.01f)
	{
		...
		DrxFatalError("DinrusXAnimation: sum of transition-weights is wrong: %f %s", fTotal, m_pInstance->m_pDefaultSkeleton->GetModelFilePath());
	}
	...
	for (u32 a = 0; a < nMaxActiveInQueue; a++)
	{
		...
		AnimCallback(rAnimation);
	}

	return 0;
}

u32 CSkeletonAnim::CheckIsCAFLoaded(CAnimationSet* pAnimationSet, i32 nAnimID)
{
...
void CSkeletonAnim::UpdateParameters(CAnimation* arrAnimFiFo, u32 nMaxActiveInQueue, u32 nLayer, f32 fFrameDeltaTime)
{
	DEFINE_PROFILER_FUNCTION();
	...
	for (u32 a = 0; a < nMaxActiveInQueue; a++)
	{
		...
		if (pParametric)
		{
			...
			if (nMaxActiveInQueue != 1 && Console::GetInst().ca_DrawVEGInfo && m_pInstance->GetCharEditMode() && g_pCharacterUpr->HasAnyDebugInstancesCreated())
			{
				u32 nMaxDebugInstances = g_pCharacterUpr->m_arrCharacterBase.size();
				for (u32 i = 0; i < nMaxDebugInstances; i++)
				{
					ISkeletonAnim* pISkeletonAnim = g_pCharacterUpr->m_arrCharacterBase[i].m_pInst->GetISkeletonAnim();
					pISkeletonAnim->StopAnimationsAllLayers();
					g_pCharacterUpr->m_arrCharacterBase[i].m_GridLocation.q.SetRotationZ(-gf_PI * 0.5f);
					g_pCharacterUpr->m_arrCharacterBase[i].m_GridLocation.t = Vec3(ZERO);
				}
			}
#endif
			...
			for (u32 i = 0; i < pParametric->m_numExamples; i++)
			{
				if (pParametric->m_fBlendWeight[i] == 0)
					continue;
				i32 nAnimIDPS = pParametric->m_nAnimID[i];
				if (nAnimIDPS < 0)
					DrxFatalError("DinrusXAnimation: negative");
				const ModelAnimationHeader* pAnimPS = pAnimationSet->GetModelAnimationHeader(nAnimIDPS);
				GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[pAnimPS->m_nGlobalAnimId];
				i32 segcount = pParametric->m_nSegmentCounter[0][i];
				...
				u32k totSegs = rCAF.GetTotalSegments();
				for (u32 s = 0; s < totSegs; s++)
				{
		...
		else
		{
			...
			{
				GlobalAnimationHeaderCAF& rGAH = g_AnimationUpr.m_arrGlobalCAF[pAnim->m_nGlobalAnimId];
				i32k segcount = rCurAnim.m_currentSegmentIndex[0];
			...
			{
				GlobalAnimationHeaderAIM& rAIM = g_AnimationUpr.m_arrGlobalAIM[pAnim->m_nGlobalAnimId];
				i32k segcount = rCurAnim.m_currentSegmentIndex[0];
...
void CSkeletonAnim::UpdateAnimationTime(CAnimation& rAnimation, u32 nLayer, u32 NumAnimsInQueue, u32 AnimNo, u32 idx)
{
	DEFINE_PROFILER_FUNCTION();
	...
	i32 numLoops = (i32)rAnimation.m_fAnimTime[idx];
	if (numLoops > 0)
	{
		...
		if (LoopAnimation == 0)
		{
			...
			{
				u32 nClamped = 0;
				u32k numAnims = pParametric->m_numExamples;
				for (u32 i = 0; i < numAnims; i++)
				{
					i32k nAnimID = pParametric->m_nAnimID[i];
					const ModelAnimationHeader* pAnim = pAnimationSet->GetModelAnimationHeader(nAnimID);
					if (pAnim->m_nAssetType != CAF_File)  DrxFatalError("DinrusXAnimation: weird error");   //can happen only in weird cases (mem-corruption, etc)
					GlobalAnimationHeaderCAF& rParaCAF = g_AnimationUpr.m_arrGlobalCAF[pAnim->m_nGlobalAnimId];
					if (++pParametric->m_nSegmentCounter[idx][i] >= rParaCAF.m_Segments)
					{
						pParametric->m_nSegmentCounter[idx][i] = rParaCAF.m_Segments - 1;
						nClamped++;
					}
					//float fColor[4] = {1,1,0,1};
					//g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.7f, fColor, false,"SegCounter: %d  idx: %d  nClamped: %d",pParametric->m_nSegmentCounter[idx][i],idx,nClamped );
					//g_YLine+=16.0f;
				}
			...
			//Automatic Fadeout: could this be a job for DrxMannequin???
			if (NumAnimsInQueue == 1 && nLayer)
			{
				u32 flags = rAnimation.m_nStaticFlags;
				u32 rlk = flags & CA_REPEAT_LAST_KEY;
				u32 fo = flags & CA_FADEOUT;  //this is something that DrxMannequin should do
				if (rlk && fo)
		...
		if (LoopAnimation == 1 && RepeatLastKey == 0)
		{
			if (pParametric)
			{
				//i32 nEOC				=	rAnimation.GetEndOfCycle(); //value from the last frame
				//float fColor[4] = {1,1,0,1};
				//g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.7f, fColor, false,"fAnimTime: %f  EOC: %d  idx: %d",rAnimation.m_fAnimTime[idx],nEOC, idx );
				//g_YLine+=16.0f;
				u32k numAnims = pParametric->m_numExamples;
				for (u32 i = 0; i < numAnims; i++)
				{
				...
				for (u32 i{0}; i<MAX_LMG_DIMENSIONS; ++i)
				{
...
u32 CSkeletonAnim::GetMaxSegments(const CAnimation& rAnimation) const
{
	u32 nMaxSegments = 1;
	CAnimationSet* pAnimationSet = m_pInstance->m_pDefaultSkeleton->m_pAnimationSet;
	...
	if (pParametric)
	{
		...
		for (u32 i = 0; i < numAnims; i++)
		{
			...
			const ModelAnimationHeader* pAnim = pAnimationSet->GetModelAnimationHeader(nAnimID);
			if (pAnim->m_nAssetType != CAF_File)
				DrxFatalError("DinrusXAnimation: weird error"); //can happen only in weird cases (mem-corruption, etc)

			i32k globalID = pAnim->m_nGlobalAnimId;
			GlobalAnimationHeaderCAF& rParaCAF = g_AnimationUpr.m_arrGlobalCAF[globalID];
...
void CSkeletonAnim::AnimCallbackInternal(bool sendAnimEventsForTimeOld, f32 normalizedTimeOld, f32 normalizedTimeNew, SAnimCallbackParams& params)
{
	...
	u32k animEventCount = params.pAnimEventList->GetCount();
	for (u32 i = 0; i < animEventCount; ++i)
	{
		...
		{
			tukk const strModelName = animEventData.GetModelName();
			assert(strModelName);

			tukk const strFilePath = m_pInstance->CCharInstance::GetFilePath();
			assert(strFilePath);
...
void CSkeletonAnim::AnimCallback(CAnimation& rAnimation)
{
	if (!m_pEventCallback)
	{
		return;
	...
	u8k segmentIndexOld = rAnimation.m_currentSegmentIndexPrev[0];
	u8k segmentIndexNew = rAnimation.m_currentSegmentIndex[0];
	...
	else
	{
		...
		{
			GlobalAnimationHeaderLMG& rGlobalAnimHeaderLMG = g_AnimationUpr.m_arrGlobalLMG[pAnim->m_nGlobalAnimId];

			nAnimID = -1;
			normalizedTimeOld = segmentNormalizedTimeOld;
			normalizedTimeNew = segmentNormalizedTimeNew;
...

			{
				...
				u32 longestCafIndexOld = 0;
				u32 longestCafIndexNew = 0;
				{
					const SParametricSamplerInternal* pParametricSampler = static_cast<const SParametricSamplerInternal*>(rAnimation.GetParametricSampler());
					for (u32 i = 0; i < pParametricSampler->m_numExamples; ++i)
					{
						const GlobalAnimationHeaderCAF& caf = g_AnimationUpr.m_arrGlobalCAF[pAnimationSet->GetModelAnimationHeader(pParametricSampler->m_nAnimID[i])->m_nGlobalAnimId];
						if (!pLongestCaf || pLongestCaf->GetTotalSegments() < caf.GetTotalSegments())
						{
							pLongestCaf = &caf;
							longestCafIndexNew = pParametricSampler->m_nSegmentCounter[0][i];
							longestCafIndexOld = pParametricSampler->m_nSegmentCounterPrev[0][i];
						}
					}
				}
...
void CSkeletonAnim::SetLayerNormalizedTimeAndSegment(f32 normalizedTime, i32 nEOC, i32 nAnimID0, i32 nAnimID1, u8 nSegment0, u8 nSegment1)
{
	...
	if (animationCountInLayer == 1)
	{
		...
		else
		{
			...
			u32 numExamples = pPara->m_numExamples;
			for (u32 i = 0; i < numExamples; i++)
			{
...
void CSkeletonAnim::SetLayerNormalizedTime(u32 layer, f32 normalizedTime)
{
...
f32 CSkeletonAnim::GetLayerNormalizedTime(u32 layer) const
{
...
void CSkeletonAnim::SetAnimationNormalizedTime(CAnimation* pAnimation, f32 uncheckedNormalizedTime, bool entireClip)
{
	...
	i16k nAnimId = pAnimation->GetAnimationId();
	...
	if (pParametricSampler)
	{
		...
		for (u32 i = 0; i < pParametricSampler->m_numExamples; i++)
		{
			...
			if (pParametricSampler->m_fBlendWeight[i] == 0.0f)
				continue;
			i32 nAnimIDPS = pParametricSampler->m_nAnimID[i];
			...
			GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[pABS->m_nGlobalAnimId];
			u32 sc = rCAF.GetTotalSegments();
			if (nMaxSegCount < sc)
		...
		i32k globalAnimationId = pAnimBS->m_nGlobalAnimId;
		GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[globalAnimationId];
		u32 nMaxSegments = rCAF.GetTotalSegments();

		u8 nSegmentIndexNew = 0;
		for (u32 i = 0; i < nMaxSegments; ++i)
		{
		...
		for (u32 i = 0; i < pParametricSampler->m_numExamples; i++)
		{
			i32 nAnimIDPS = pParametricSampler->m_nAnimID[i];
			const ModelAnimationHeader* pABS = pAnimationSet->GetModelAnimationHeader(nAnimIDPS);
			GlobalAnimationHeaderCAF& rExampleCAF = g_AnimationUpr.m_arrGlobalCAF[pABS->m_nGlobalAnimId];
			u32 sc = rExampleCAF.GetTotalSegments();
	...
	{
		//u8 oldsi = pAnimation->m_currentSegmentIndex[0];
		i32k globalAnimationId = pModelAnimationHeader->m_nGlobalAnimId;
		GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[globalAnimationId];
		u32 nMaxSegments = rCAF.GetTotalSegments();
		u8 currentSegmentIndex = 0;
		for (u32 i = 0; i < nMaxSegments; ++i)
		{
			const f32 nextSegmentTime = rCAF.m_SegmentsTime[i + 1];
			if (fNormalizedTime <= nextSegmentTime)
...
f32 CSkeletonAnim::GetAnimationNormalizedTime(u32 layer, u32 index) const
{
...
f32 CSkeletonAnim::GetAnimationNormalizedTime(const CAnimation* pAnimation) const
{
	...
	u32k nMaxSegments = GetMaxSegments(*pAnimation);
	if (nMaxSegments <= 1)
		return pAnimation->m_fAnimTime[0];
	...
	if (pParametricSampler)
	{
		...
		const ModelAnimationHeader* pAnimBS = pAnimationSet->GetModelAnimationHeader(nMaxAnimID);
		i32k nGlobalId = pAnimBS->m_nGlobalAnimId;
	...
	{
		u8k currentSegmentIndex = pAnimation->m_currentSegmentIndex[0];
		const f32 currentSegmentNormalizedPosition = pAnimation->m_fAnimTime[0];
		GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[pModelAnimationHeader->m_nGlobalAnimId];
...
{
	u32 smallest = min(a, b);
	u32 largest = max(a, b);
	u32 remainder = largest % smallest;
...
static u32 LeastCommonMultiple(u8 (&segments)[MAX_LMG_ANIMS], u32 count)
{
	u32 result = 1;
...
f32 CSkeletonAnim::CalculateCompleteBlendSpaceDuration(const CAnimation& rAnimation) const
{
	...
	if (pParametric)
	{
		u8 segmentCount[MAX_LMG_ANIMS] = { 0 };
		for (u32 i = 0; i < pParametric->m_numExamples; ++i)
		{
			...
			GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[pHeader->m_nGlobalAnimId];
			segmentCount[i] = rCAF.m_Segments;
		}

		i32 totalSegmentCount = LeastCommonMultiple(segmentCount, pParametric->m_numExamples);
		...
		for (u32 i = 0; i < pParametric->m_numExamples; ++i)
		{
			if (pParametric->m_fBlendWeight[i] == 0.0f)
				continue;
			i32 nAnimID = pParametric->m_nAnimID[i];
			const ModelAnimationHeader* pHeader = pAnimationSet->GetModelAnimationHeader(nAnimID);
			const GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[pHeader->m_nGlobalAnimId];

			for (u32 s = 0; s < segmentCount[i]; ++s)
			{
				i32 repetitionCount = totalSegmentCount / segmentCount[i];
...
void CSkeletonAnim::SetAnimationDrivenMotion(u32 ts)
{
	m_AnimationDrivenMotion = ts;
};
void CSkeletonAnim::SetMirrorAnimation(u32 ts)
{
	m_MirrorAnimation = ts;
};

// sets the animation speed scale for layers
void CSkeletonAnim::SetLayerPlaybackScale(i32 nLayer, f32 fSpeed)
{
...
f32 CSkeletonAnim::GetLayerBlendWeight(i32 nLayer)
{
	if (nLayer>=numVIRTUALLAYERS || nLayer<0)
	{
...
void CSkeletonAnim::SetLayerBlendWeight(i32 nLayer, f32 fMult)
{
	if (nLayer >= numVIRTUALLAYERS || nLayer < 0)
	{
		g_pILog->LogError("invalid layer id: %d", nLayer);
...
u32 CSkeletonAnim::EvaluateTransitionFlags(CAnimation* arrAnimFiFo, u32 numAnims)
{
	CAnimationSet* pAnimationSet = m_pInstance->m_pDefaultSkeleton->m_pAnimationSet;
	...
	for (aq = 1; aq < numAnims; aq++)
	{
		...
		u32 IsLooping = rPrevAnimation.m_nStaticFlags & CA_LOOP_ANIMATION;

		u32 StartAtKeytime = rCurAnimation.m_nStaticFlags & CA_START_AT_KEYTIME;
		u32 StartAfter = rCurAnimation.m_nStaticFlags & CA_START_AFTER;
		u32 Idle2Move = rCurAnimation.m_nStaticFlags & CA_IDLE2MOVE;
		//		u32 Move2Idle			= rCurAnimation.m_nStaticFlags & CA_MOVE2IDLE;
		u32 InMemory = IsAnimationInMemory(pAnimationSet, &rCurAnimation);
		...
		if (Console::GetInst().ca_DebugAnimationStreaming && InMemory == 0)
		{
			...
			float fC1[4] = { 1, 1, 0, 1 };
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.6f, fC1, false, "Streaming Assets: %s ", pName);
			g_YLine += 16.0f;

			g_pISystem->Warning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, VALIDATOR_FLAG_FILE, m_pInstance->GetFilePath(), "DinrusXAnimation: streaming file: %s", pName);
		}
		...
		if (Idle2Move)
		{
			...
			if (rPrevAnimation.GetParametricSampler() != NULL)
			{
				...
				GlobalAnimationHeaderLMG& rGlobalAnimHeader = g_AnimationUpr.m_arrGlobalLMG[pAnim->m_nGlobalAnimId];
		...
		//all thisn stuff belongs into DrxManequinn
		u32 TransitionDelay = StartAtKeytime + StartAfter + Idle2Move + (InMemory == 0);
		if (TransitionDelay)
		{
			if (InMemory == 0)
			...
			if (rPrevAnimation.GetParametricSampler() != NULL)
			{
				if (Idle2Move == 2)
				{
					...
					if (dim < 0)
						DrxFatalError("DinrusXAnimation: I2M without Turn-Parameter"); //bad VEG

					u32 nSegCount = lmg.m_nSegmentCounter[0][0];
...
u32 CSkeletonAnim::IsAnimationInMemory(CAnimationSet* pAnimationSet, CAnimation* pAnimation)
{
	...
	{
		u32 numExamples = pParametric->m_numExamples;
		for (u32 a = 0; a < numExamples; a++)
		{
			i32 nAnimID = pParametric->m_nAnimID[a];
...
void CSkeletonAnim::BlendUprDebug()
{
#ifndef _RELEASE
...

	{
		...
		g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "Queue for '%s':", m_pInstance->m_strFilePath.c_str());
		g_YLine += 0x0e;
	}
	...
	for (u32 nVLayerNo = 0; nVLayerNo < numVIRTUALLAYERS; nVLayerNo++)
	{
		...
		u32 numAnimsPerLayer = rCurLayer.size();

		BlendUprDebug(rCurLayer, nVLayerNo);
	}

	f32 fColor[4] = { 1, 0, 0, 1 };

	const CDefaultSkeleton& modelSkeleton = *m_pInstance->m_pDefaultSkeleton;
	u32k adikTargetCount = modelSkeleton.m_ADIKTargets.size();

	u32 adikLineLength = 0;
	for (u32 adikIndex = 0; adikIndex < adikTargetCount; ++adikIndex)
	{
		const ADIKTarget& adikTarget = modelSkeleton.m_ADIKTargets[adikIndex];

		i32k targetWeightJointIndex = adikTarget.m_idxWeight;
		if (targetWeightJointIndex < 0)
			continue;

		if (adikLineLength == 0)
		{
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "ADIK");
		}
		...
		u32k startOffsetTextLength = 6;
		g_pAuxGeom->Draw2dLabel(textWidthMultiplier * (startOffsetTextLength + adikLineLength), g_YLine, 1.2f, fColor, false, "%s: %.3f", adikTarget.m_strTarget, targetBlendWeight);
		u32k fixedLineTextLength = 10;
		adikLineLength += strlen(adikTarget.m_strTarget) + fixedLineTextLength;
		u32k maxLineLength = 120;
...
u32 CSkeletonAnim::BlendUprDebug(DynArray<CAnimation>& arrAFIFO, u32 nVLayer)
{
#ifndef _RELEASE
	f32 fColor[4] = { 1, 0, 0, 1 };
	...
	u32 NumAnimsInQueue = arrAFIFO.size();
	for (u32 i = 0; i < NumAnimsInQueue; i++)
	{
		...
		tukk pAnimName = pAnimationSet->GetModelAnimationHeaderRef(animation.GetAnimationId()).GetAnimName();
		...
		if (nVLayer == 0)
		{
			...
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "(Try)UseAimIK: %d  AimIKBlend: %.3f  AimIKInfluence: %.3f   (Try)UseLookIK: %d  LookIKBlend: %.3f  LookIKInfluence: %.3f", nUseDirIK, fDirIKBlend, fDirIKInfluence, nUseLookIK, fLookIKBlend, fLookIKInfluence);
			g_YLine += 0x0e;
		}
		else
		{
		...
		if (pParametric)
		{
			for (u32 d = 0; d < pParametric->m_numDimensions; d++)
			{
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_TravelSpeed)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "MoveSpeed: %f  locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_TurnSpeed)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "TurnSpeed: %f  locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_TravelSlope)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "Slope: %f   locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_TravelAngle)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "TravelAngle: %f  locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_TravelDist)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "TravelDist: %f  locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_TurnAngle)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "TurnAngle: %f  locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_StopLeg)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "StopLeg: %f   locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_BlendWeight)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "BlendWeight: %f   locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_BlendWeight2)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "BlendWeight2: %f   locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_BlendWeight3)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "BlendWeight3: %f   locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_BlendWeight4)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "BlendWeight4: %f   locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_BlendWeight5)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "BlendWeight5: %f   locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_BlendWeight6)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "BlendWeight6: %f   locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
				if (pParametric->m_MotionParameterID[d] == eMotionParamID_BlendWeight7)
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, fColor, false, "BlendWeight7: %f   locked: %d", pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter), g_YLine += 0x0e;
			}
		}
		g_YLine += 0x02;
	...
	{
		g_pAuxGeom->Draw2dLabel(1, g_YLine, 1.2f, ColorF(1.0f, 1.0f, 1.0f, 1.0f), false,
		                        "PM class: %s, name %s",
...
void CSkeletonAnim::DebugLogTransitionQueueState()
{
	...
	for (u32 nVLayer = 0; nVLayer < numVIRTUALLAYERS; nVLayer++)
	{
		DynArray<CAnimation>& arrAFIFO = m_layers[nVLayer].m_transitionQueue.m_animations;
		if (!arrAFIFO.empty())
		{
			u32k numAnimsInQueue = arrAFIFO.size();
			for (u32 i = 0; i < numAnimsInQueue; i++)
			{
				...
				DrxLogAlways("AnimInAFIFO %02u: t:%u  '%s'  ATime:%.2f (%.2fs/%.2fs) ASpd:%.2f Flag:%08x (%s) TTime:%.2f TWght:%.2f PWght:%.2f seg:%02u inmem:%u",
				             nVLayer,
				...
				if (nVLayer == 0)
				{
					...
					DrxLogAlways("(Try)UseAimIK: %u  AimIKBlend: %.2f  AimIKInfluence: %.2f   (Try)UseLookIK: %u  LookIKBlend: %.2f  LookIKInfluence: %.2f", nUseDirIK, fDirIKBlend, fDirIKInfluence, nUseLookIK, fLookIKBlend, fLookIKInfluence);
				}
				...
				if (pParametric)
				{
					for (u32 d = 0; d < pParametric->m_numDimensions; d++)
					{
						...
						case eMotionParamID_BlendWeight7:
							...
							break;
						default:
							motionParamName.Format("<Unknown>(%u)", pParametric->m_MotionParameterID[d]);
							break;
						}

						DrxLogAlways("%s: %f, locked: %d", motionParamName.c_str(), pParametric->m_MotionParameter[d], pParametric->m_MotionParameterFlags[d] & CA_Dim_LockedParameter);
					}
				}
			}
		}
		...
		for (uint i = 0; i < poseModifierCount; ++i)
		{
			DrxLogAlways("PM class: %s, name %s",
			             m_layers[nVLayer].m_poseModifierQueue.GetEntries()[i].poseModifier->GetFactory()->GetName(),
			             m_layers[nVLayer].m_poseModifierQueue.GetEntries()[i].name);
		}
	}
	...
	for (u32 adikIndex = 0; adikIndex < adikTargetCount; ++adikIndex)
	{
		...
		DrxLogAlways("ADIK: target: %s weight: %.2f", adikTarget.m_strTarget, targetBlendWeight);
	}
#endif
}
...
/////////////////////////////////////////////////////////////////////////////////
void CSkeletonAnim::LayerBlendUpr(f32 fDeltaTime, u32 nLayer)
{
	if (nLayer == 0)
		return;
	...
	u32 numAnims = transitionQueue.m_animations.size();

	DRX_ASSERT(!status || numAnims > 0);
...
bool CSkeletonAnim::ExportHTRAndICAF(tukk szAnimationName, tukk savePath) const
{
	...
	if (pLMGHead)
	{
		i32 nAnimID = pAnimationSet->GetAnimIDByName(szAnimationName);
		if (nAnimID < 0)
		{
...
bool CSkeletonAnim::ExportVGrid(tukk szAnimationName) const
{
	const CDefaultSkeleton* pDefaultSkeleton = m_pInstance->m_pDefaultSkeleton;
	const CAnimationSet* pAnimationSet = pDefaultSkeleton->m_pAnimationSet;
	const GlobalAnimationHeaderLMG* pLMGHead = pAnimationSet->GetGAH_LMG(szAnimationName);
