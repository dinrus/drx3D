...
u32 CAttachmentSKIN::Immediate_AddBinding( IAttachmentObject* pIAttachmentObject,ISkin* pISkin,u32 nLoadingFlags )
{
	...
	{
		ReleaseModelSkin();
		g_pCharacterUpr->RegisterInstanceSkin(pCSkinModel,this); //register this CAttachmentSKIN instance in CSkin.
	...
	tukk pSkinFilePath = m_pModelSkin->GetModelFilePath();

	u32 numJointsSkel = pDefaultSkeleton->m_arrModelJoints.size();
	u32 numJointsSkin = m_pModelSkin->m_arrModelJoints.size();
	...
	if (NotMatchingNames)
	{
		...
		else
		{
			if (nLogWarnings)
			{
				...
				DrxLogAlways("SKIN: %s",m_pModelSkin->GetModelFilePath() );
...
void CAttachmentSKIN::Immediate_ClearBinding(u32 nLoadingFlags)
{
...
void CAttachmentSKIN::RecreateDefaultSkeleton(CCharInstance* pInstanceSkel, u32 nLoadingFlags)
{
	...
	for (auto&& pAttachment : m_pAttachmentUpr->m_arrAttachments)
	{
		...
		tukk pSkinFilename = pSkin->GetModelFilePath();
...
void CAttachmentSKIN::UpdateRemapTable()
{
	...
	for(u32 js=0; js<numJointsSkin; js++)
	{
		i32k nID = pDefaultSkeleton->GetJointIDByCRC32(m_pModelSkin->m_arrModelJoints[js].m_nJointCRC32Lower);
		if (nID>=0)
			m_arrRemapTable[js]=nID;
		else
...
u32 CAttachmentSKIN::Immediate_SwapBinding(IAttachment* pNewAttachment)
{
	DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "CAttachmentSKIN::SwapBinding attempting to swap skin attachment bindings this is not supported");
	return 0;
}
...
float CAttachmentSKIN::GetExtent(EGeomForm eForm)
{
	if (m_pModelSkin)
	{
		i32 nLOD = m_pModelSkin->SelectNearestLoadedLOD(0);
...
i32 CAttachmentSKIN::GetGuid() const
{
...
void CAttachmentSKIN::ReleaseModelSkin()
{
	if (m_pModelSkin)
	{
		g_pCharacterUpr->UnregisterInstanceSkin(m_pModelSkin,this);
		ReleaseRemapTablePair();
		m_pModelSkin = 0;
	}
}
...
CAttachmentSKIN::~CAttachmentSKIN()
{
	...
	{
		i32 nFrameID = gEnv->pRenderer ? gEnv->pRenderer->EF_GetSkinningPoolID() : 0;
...
void CAttachmentSKIN::GetMemoryUsage( IDrxSizer *pSizer ) const
{
...
_smart_ptr<IRenderMesh> CAttachmentSKIN::CreateVertexAnimationRenderMesh(uint lod, uint id)
{
	...
	u32 numModelMeshes = m_pModelSkin->m_arrModelMeshes.size();
...
void CAttachmentSKIN::DrawAttachment(SRendParams& RendParams, const SRenderingPassInfo &passInfo, const Matrix34& rWorldMat34, f32 fZoomFactor)
{
	...
	//-----------------------------------------------------------------------------
	i32 numLODs = m_pModelSkin->m_arrModelMeshes.size();
	if (numLODs==0)
		return;
	...
	i32 nRenderLOD = m_pModelSkin->SelectNearestLoadedLOD(nDesiredRenderLOD);  //we can render only loaded LODs
...
//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.3f, fColor, false,"CSkin:  numLODs: %d  nLodLevel: %d   nRenderLOD: %d   Model: %s",numLODs, RendParams.nLod, nRenderLOD, m_pModelSkin->GetModelFilePath() ); g_YLine+=0x10;
  ...
  pObj->m_DissolveRef = RendParams.nDissolveRef;
	...
	if (bNewFrame)
	{
		...
		{
			m_vertexAnimation.UpdateFrameWeightsFromPose(m_pAttachmentUpr->m_pSkelInstance->m_SkeletonPose.GetPoseData());
	...
	if(pRenderMesh)
	{
		...
		if (!bUseCPUDeformation && (pMaster->m_CharEditMode & CA_CharacterTool))
		{
			...
			{
				CModelMesh* pModelMesh = m_pModelSkin->GetModelMesh(nRenderLOD);
				gEnv->pJobUpr->WaitForJob(*pD->m_pSkinningData->pAsyncJobs);
...
//-----------------------------------------------------------------------------
void CAttachmentSKIN::TriggerMeshStreaming(u32 nDesiredRenderLOD, const SRenderingPassInfo &passInfo)
{
	if (!m_pModelSkin)
		return;

	u32 numLODs = m_pModelSkin->m_arrModelMeshes.size();
	if (numLODs==0)
		return;
...
SSkinningData* CAttachmentSKIN::GetVertexTransformationData(bool useSwSkinningCpu, u8 nRenderLOD, const SRenderingPassInfo& passInfo)
{
	...
	u32k skinningQuatCount = m_arrRemapTable.size();
	u32k skinningQuatCountMax = pMaster->arrSkinningRendererData[nList].pSkinningData->nNumBones;
	u32k nNumBones = std::min(skinningQuatCount, skinningQuatCountMax);
...
void CAttachmentSKIN::HideAttachment( u32 x )
{
...
void CAttachmentSKIN::HideInRecursion( u32 x )
{
...
void CAttachmentSKIN::HideInShadow( u32 x )
{
...
void CAttachmentSKIN::DrawWireframeStatic( const Matrix34& m34, i32 nLOD, u32 color)
{
...
//////////////////////////////////////////////////////////////////////////
void CAttachmentSKIN::SoftwareSkinningDQ_VS_Emulator( CModelMesh* pModelMesh, Matrix34 rRenderMat34, u8 tang,u8 binorm,u8 norm,u8 wire, const DualQuat* const pSkinningTransformations)
{
	...
	static DynArray<u8> arrSkinned;

	u32 numExtIndices	= pModelMesh->m_pIRenderMesh->GetIndicesCount();
	u32 numExtVertices	= pModelMesh->m_pIRenderMesh->GetVerticesCount();
	assert(numExtVertices && numExtIndices);

	u32 ssize=g_arrExtSkinnedStream.size();
	...
	if (pIndices==0)
		return;
	i32		nPositionStride;
	u8*	pPositions			= pModelMesh->m_pIRenderMesh->GetPosPtr(nPositionStride, FSL_READ);
	if (pPositions==0)
		return;
	i32		nQTangentStride;
	u8*	pQTangents			= pModelMesh->m_pIRenderMesh->GetQTangentPtr(nQTangentStride, FSL_READ);
	if (pQTangents==0)
		return;
	i32		nSkinningStride;
	u8 * pSkinningInfo				= pModelMesh->m_pIRenderMesh->GetHWSkinPtr(nSkinningStride, FSL_READ); //pointer to weights and bone-id
	if (pSkinningInfo==0)
		return;
	...
	{
		u16k sidx = m_arrRemapTable[b]; //is array can be different for every skin-attachment
		arrRemapSkinQuat[b] = pSkinningTransformations[sidx];
	}

	u32k numSubsets = pModelMesh->m_arrRenderChunks.size();
	if (numSubsets)
	...
	for (u32 s=0; s<numSubsets; s++)
	{
		u32 startIndex		= pModelMesh->m_arrRenderChunks[s].m_nFirstIndexId;
		u32 endIndex			= pModelMesh->m_arrRenderChunks[s].m_nFirstIndexId+pModelMesh->m_arrRenderChunks[s].m_nNumIndices;
		...
		for (u32 idx=startIndex; idx<endIndex; ++idx)
		{
			...
			u16*	hwIndices   = ((SVF_W4B_I4S*)(pSkinningInfo + e*nSkinningStride))->indices;
	...
	u32 numBuffer	=	g_arrExtSkinnedStream.size();
	...
	if (tang)
	{
		...
		u32 csize = arrExtVColors.size();
		if (csize<(numExtVertices*2)) arrExtVColors.resize( numExtVertices*2 );
		for(u32 i=0; i<numExtVertices*2; i=i+2)
		{
	...
	if (wire)
	{
		u32 color = RGBA8(0x00,0xff,0x00,0x00);
		SAuxGeomRenderFlags renderFlags( e_Def3DPublicRenderflags );
