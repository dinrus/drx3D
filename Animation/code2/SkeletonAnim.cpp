...
void CSkeletonAnim::FinishAnimationComputations()
{
	...
	if (pCtx)
	{
		g_pCharacterUpr->GetContextSyncQueue().ExecuteForContextAndAllChildrenRecursively(
...
constexpr DrxGUID GUID_AnimationPoseModifier_TransformationPin = "cc34ddea-972e-47da-93f9-cdcb98c28c8e"_drx_guid;
bool CSkeletonAnim::PushPoseModifier(u32 layer, IAnimationPoseModifierPtr poseModifier, tukk name)
{
	if (poseModifier)
	{
...
void CSkeletonAnim::ProcessAnimations(const QuatTS& rAnimLocationCurr)
{
	...
	if (pInstance->FacialAnimationEnabled())
	{
		...
		if (m_pInstance->m_fPostProcessZoomAdjustedDistanceFromCamera < Console::GetInst().ca_FacialAnimationRadius)
		{
			...
			if (pFaceInst)
			{
				...
				u32k fps = u32(Console::GetInst().ca_FacialAnimationFramerate * fraction);
	...
	if (m_pInstance->m_CharEditMode == 0)
	{
		...
		if (m_pInstance->m_LastUpdateFrameID_Pre == nCurrentFrameID)
		{
			...
			tukk name = m_pInstance->m_pDefaultSkeleton->GetModelFilePath();
	...
	m_pInstance->m_LastUpdateFrameID_Pre = g_pCharacterUpr->m_nUpdateCounter;
...
void CSkeletonAnim::ProcessAnimationUpdate(const QuatTS rAnimLocationCurr)
{
	...
	u32 numAnimsInLayer = rCurLayer0.size();
	...
	for (u32 a = 0; a < numAnimsInLayer; a++)
	{
		...
		{
			GlobalAnimationHeaderLMG& rLMG = g_AnimationUpr.m_arrGlobalLMG[pAnimLMG->m_nGlobalAnimId];
	...
	if (1)
	{
		...
		for (u32 nVLayerNo = 0; nVLayerNo < numVIRTUALLAYERS; nVLayerNo++)
		{
			...
			if (numAnimsPerLayer)
			{
				BlendUpr(m_pInstance->m_fDeltaTime * m_layers[nVLayerNo].m_transitionQueue.m_fLayerPlaybackScale, rCurLayer, nVLayerNo);
			}

			LayerBlendUpr(m_pInstance->m_fDeltaTime, nVLayerNo);
		}
...
void CSkeletonAnim::Serialize(TSerialize ser)
{
	...
	if (ser.GetSerializationTarget() != eST_Network)
	{
		...
		for (u32 nLayer = 0; nLayer < numVIRTUALLAYERS; nLayer++)
		{
			ser.BeginGroup("FIFO");
			u32 nAnimsInFIFO = m_layers[nLayer].m_transitionQueue.m_animations.size();
			ser.Value("nAnimsInFIFO", nAnimsInFIFO);
			if (ser.IsReading())
				m_layers[nLayer].m_transitionQueue.m_animations.resize(nAnimsInFIFO);

			for (u32 a = 0; a < nAnimsInFIFO; a++)
			{
				...
				if (ser.IsReading())
				{
					if (pParametric == NULL)
					{
						...
						GlobalAnimationHeaderCAF& rGlobalAnimHeader = g_AnimationUpr.m_arrGlobalCAF[AnimHeader.m_nGlobalAnimId];
					...
					{
						i32 numAnims = pParametric->m_numExamples;
...
void CSkeletonAnim::SetDebugging(u32 debugFlags)
{
...
void CSkeletonAnim::GetMemoryUsage(IDrxSizer* pSizer) const
{
	for (u32 i = 0; i < numVIRTUALLAYERS; i++)
	{
...
void CSkeletonAnim::SetTrackViewExclusive(u32 i)
{
	m_TrackViewExclusive = i > 0;
}

void CSkeletonAnim::SetTrackViewMixingWeight(u32 layer, f32 weight)
{
	if (!m_TrackViewExclusive)
	{
		DrxLog("Trying to set manual mixing weight without being in TrackViewExclusive mode");
		return;
	}
...
{
	for (i32 i = 0; i < g_totalParametrics; i++)
	{
	...
	DRX_ASSERT_TRACE(0, ("Run out of free parametric slots! If current usage is typical increase pool size via ca_ParametricPoolSize. Current Size: %d", g_totalParametrics));
	DrxLogAlways("Run out of free parametric slots! If current usage is typical increase pool size via ca_ParametricPoolSize. Current Size: %d", g_totalParametrics);
	return NULL;
}
