...
void CBuffer::Execute()
{
	...
	u8* pCommands = m_pBuffer;
	u32 commandOffset = 0;
	for (u32 i = 0; i < m_commandCount; ++i)
	{
		u8& command = pCommands[commandOffset];
		switch (command)
		{
...
#endif
		default:
			DrxFatalError("DinrusXAnimation: Command-Buffer Invalid: Command %u  Model: %s", command, m_state.m_pDefaultSkeleton->GetModelFilePath());
			break;
		}
...
void CBuffer::DebugDraw()
{
	...
	u8* pCommands = m_pBuffer;
	u32 commandOffset = 0;
	for (u32 c = 0; c < m_commandCount; c++)
	{
		i32 anim_command = pCommands[commandOffset];
		switch (anim_command)
		{
		case ClearPoseBuffer::ID:
		...
		default:
			DrxFatalError("DinrusXAnimation: Command-Buffer Invalid: Command %d  Model: %s", anim_command, m_state.m_pDefaultSkeleton->GetModelFilePath());
			break;
		}
	}
}
