...
bool IK_Solver(const CDefaultSkeleton& defaultSkeleton, LimbIKDefinitionHandle limbHandle, const Vec3& vLocalPos, Skeleton::CPoseData& poseData)
{
	...
	i32 idxDefinition = defaultSkeleton.GetLimbDefinitionIdx(limbHandle);
	if (idxDefinition < 0)
		return false;
	...
	{
		i32 p = rIKLimbType.m_arrRootToEndEffector[i - 1];
		i32 c = rIKLimbType.m_arrRootToEndEffector[i];
	...
	if (rIKLimbType.m_nSolver == *(u32*)"2BIK")
		IK_Solver2Bones(vLocalPos, rIKLimbType, poseData);
	if (rIKLimbType.m_nSolver == *(u32*)"3BIK")
		IK_Solver3Bones(vLocalPos, rIKLimbType, poseData);
	if (rIKLimbType.m_nSolver == *(u32*)"CCDX")
	...
	u32 numChilds = rIKLimbType.m_arrLimbChildren.size();
	i16k* pJointIdx = &rIKLimbType.m_arrLimbChildren[0];
	for (u32 i = 0; i < numChilds; ++i)
	{
		i32 c = pJointIdx[i];
		i32 p = parrModelJoints[c].m_idxParent;
		pAbsPose[c] = pAbsPose[p] * pRelPose[c];
...
void IK_Solver2Bones(const Vec3& goal, const IKLimbType& rIKLimbType, Skeleton::CPoseData& poseData)
{
	...
	i32 b0 = rIKLimbType.m_arrJointChain[0].m_idxJoint;
	assert(b0 > 0);                                                     //BaseRoot
	i32 b1 = rIKLimbType.m_arrJointChain[1].m_idxJoint;
	assert(b1 > 0);                                                     //Ball Joint (and Root of Chain)
	i32 b2 = rIKLimbType.m_arrJointChain[2].m_idxJoint;
	assert(b2 > 0);                                                     //Hinge Joint
	i32 b3 = rIKLimbType.m_arrJointChain[3].m_idxJoint;
...
void IK_Solver3Bones(const Vec3& goal, const IKLimbType& rIKLimbType, Skeleton::CPoseData& poseData)
{
	...
	i32 b4 = rIKLimbType.m_arrJointChain[4].m_idxJoint;
...
void IK_SolverCCD(const Vec3& vTarget, const IKLimbType& rIKLimbType, Skeleton::CPoseData& poseData)
{
	...
	{
		i32 p = rIKLimbType.m_arrJointChain[i - 1].m_idxJoint;
		i32 c = rIKLimbType.m_arrJointChain[i].m_idxJoint;
		fTransCompensation += (pAbsPose[c].t - pAbsPose[p].t).GetLengthFast();
	}

	f32 inumLinks = 1.0f / f32(numLinks);
	i32 nRootIdx = rIKLimbType.m_arrJointChain[1].m_idxJoint;              //Root
	i32 nEndEffIdx = rIKLimbType.m_arrJointChain[numLinks - 1].m_idxJoint; //EndEffector
	ANIM_ASSET_ASSERT(nRootIdx < nEndEffIdx);
	i32 iJointIterator = 1;   //numLinks-2;

	// Cyclic Coordinate Descent
	for (u32 i = 0; i < rIKLimbType.m_nInterations; i++)
	{
		Vec3 vecEnd = pAbsPose[nEndEffIdx].t;             // Position of end effector
		i32 parentLinkIdx = rIKLimbType.m_arrJointChain[iJointIterator - 1].m_idxJoint;
		ANIM_ASSET_ASSERT(parentLinkIdx >= 0);
		i32 LinkIdx = rIKLimbType.m_arrJointChain[iJointIterator].m_idxJoint;
		...
		{
			i32 c = rIKLimbType.m_arrJointChain[j].m_idxJoint;
			i32 p = rIKLimbType.m_arrJointChain[j - 1].m_idxJoint;
			assert(p >= 0);
			ANIM_ASSET_ASSERT(pRelPose[c].q.IsUnit());
		...
		if (iJointIterator > (i32)(numLinks) - 3) // is it correct, uint - 3 can cause serious problems
			iJointIterator = 1;
	}
