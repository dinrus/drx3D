...
#include <drx3D/Animation/PoseModifierDesc.h>
...
public:
	...
	DRXINTERFACE_ADD(IAnimationSerializable)
	DRXINTERFACE_END()

	DRXGENERATE_CLASS_GUID(CConstraintPoint, "AnimationPoseModifier_ConstraintPoint", "705fd8b7-906f-42a1-b7d6-d5dee73d3b54"_drx_guid)
...
DRXREGISTER_CLASS(CConstraintPoint)
...
public:
	...
	DRXGENERATE_CLASS_GUID(CConstraintLine, "AnimationPoseModifier_ConstraintLine", "705fd8b7-906f-42d2-b7d6-d5dee73d3c64"_drx_guid)
...
DRXREGISTER_CLASS(CConstraintLine)
...
public:
	...
	DRXGENERATE_CLASS_GUID(CConstraintAim, "AnimationPoseModifier_ConstraintAim", "9d07deeb-5408-413d-ad47-1fabc571f964"_drx_guid)
...
DRXREGISTER_CLASS(CConstraintAim)
...
public:
	...
	DRXGENERATE_CLASS_GUID(CDrivenTwist, "AnimationPoseModifier_DrivenTwist", "4d9ef006-1e06-4b8d-b1a6-d24fd84c599b"_drx_guid)
...
DRXREGISTER_CLASS(CDrivenTwist)
...
public:
	...
	DRXGENERATE_CLASS_GUID(CIk2Segments, "AnimationPoseModifier_Ik2Segments", "6a078d00-c194-41eb-b891-9c52d094076d"_drx_guid);
...
DRXREGISTER_CLASS(CIk2Segments)
...
public:
	...
	DRXGENERATE_CLASS_GUID(CIkCCD, "AnimationPoseModifier_IkCcd", "6a078d00-c194-41e2-b891-9c52d094076d"_drx_guid);
...
private:
	...
	DynArray<u32> m_arrJointChain;

	bool             m_bInitialized;
	bool             m_bDraw;
};

DRXREGISTER_CLASS(CIkCCD)
...
void CIkCCD::Draw(const QuatT& location, const QuatT& startAbsolute, const QuatT& targetAbsolute, const float weight)
{
	...
	pAuxGeom->Draw2dLabel(10, g_YLine+=12, 1.2f, pCol, false, "IkCCD %s -> %s:\t%d   Pos: %f  %f  %f   Rot: %f  %f  %f", m_desc.rootNode.name.c_str(), m_desc.endNode.name.c_str(), (u8)(weight*100.0f), targetAbsolute.t.x, targetAbsolute.t.y, targetAbsolute.t.z, RAD2DEG(angles.x), RAD2DEG(angles.y), RAD2DEG(angles.z));
	pAuxGeom->Submit();
}
...
bool CIkCCD::Prepare(const SAnimationPoseModifierParams& params)
{
	...
	if (m_rootNodeIndex != uint(-1) && m_endNodeIndex != uint(-1))
	{
		...
		u32 joint = m_endNodeIndex;
		bool finishedInit = false;
		while (joint > 0 && !finishedInit)
		{
			u32 parent = defaultSkeleton.GetJointParentIDByID(joint);
		...
		{
			if (u32 size = m_arrJointChain.size())
			{
...
bool CIkCCD::Execute(const SAnimationPoseModifierParams& params)
{
	...
	const CDefaultSkeleton& defaultSkeleton = PoseModifierHelper::GetDefaultSkeleton(params);
	u32 numLinks = m_arrJointChain.size();
	...
	for (u32 i = 2; i < numLinks; i++)
	{
		i32 p = m_arrJointChain[i - 1];
		i32 c = m_arrJointChain[i];
	...
	i32 nRootIdx = m_arrJointChain[1];              //Root
	i32 nEndEffIdx = m_arrJointChain[numLinks - 1]; //EndEffector
	ANIM_ASSET_ASSERT(nRootIdx < nEndEffIdx);
	i32 iJointIterator = 1;

	// Cyclic Coordinate Descent
	for (u32 i = 0; i < m_nIterations; i++)
	{
		Vec3 vecEnd = pAbsPose[nEndEffIdx].t; // Position of end effector
		i32 parentLinkIdx = m_arrJointChain[iJointIterator - 1];
		ANIM_ASSET_ASSERT(parentLinkIdx >= 0);
		i32 LinkIdx = m_arrJointChain[iJointIterator];
		...
		for (u32 j = iJointIterator; j < numLinks; j++)
		{
			i32 c = m_arrJointChain[j];
			i32 p = m_arrJointChain[j - 1];
			assert(p >= 0);
		...
		if (iJointIterator > (i32)(numLinks) - 3)
			iJointIterator = 1;
	}
	...
	for (u32 i = m_rootNodeIndex; i < numJoints; i++)
	{
		if (u32 p = defaultSkeleton.GetJointParentIDByID(i))
		{
...
public:
	...
	DRXGENERATE_CLASS_GUID(CDynamicsSpring, "AnimationPoseModifier_DynamicsSpring", "92e070d5-701b-4f8a-a761-42e967579948"_drx_guid)
...
DRXREGISTER_CLASS(CDynamicsSpring)
...
public:
	...
	DRXGENERATE_CLASS_GUID(CDynamicsPendulum, "AnimationPoseModifier_DynamicsPendulum", "f6c1b4da-5caf-4b9e-bb97-c28f9f17b003"_drx_guid)
...
DRXREGISTER_CLASS(CDynamicsPendulum)
...
public:
	...
	DRXGENERATE_CLASS_GUID(CTransformBlender, "AnimationPoseModifier_TransformBlender", "92e070d5-601b-4f9a-a761-43e967579958"_drx_guid)
...
DRXREGISTER_CLASS(CTransformBlender)
...
void CTransformBlender::Draw(const QuatT& targetAbsolute, const QuatT& defaultAbsolute)
{
	...
	for (i32 i = 0; i < m_targets.size(); i++)
	{
		u8 alpha = (u8)(200 * m_targets[i].weight);
...
bool CTransformBlender::Prepare(const SAnimationPoseModifierParams& params)
{
	...
	{
		for (i32 i = 0; i < m_desc.targets.size(); i++)
		{
...
DRXREGISTER_CLASS(CPoseModifierStack)
...
bool Serialize(Serialization::IArchive& ar, IAnimationPoseModifierPtr& pointer, tukk name, tukk label)
{
	Serialization::DrxExtensionPointer<IAnimationPoseModifier, IAnimationSerializable> serializer(pointer);
...
void CPoseModifierSetup::Entry::Serialize(Serialization::IArchive& ar)
{
	...
	if (!ar(instance, "instance", "^"))
	{
		...
		DrxClassID classId = DrxClassID::Null();
		...
		if (classId.hipart != 0 || classId.lopart != 0)
		{
			...
			if (IDrxFactory* factory = factoryRegistry->GetFactory(classId))
				instance = std::static_pointer_cast<IAnimationPoseModifier>(factory->CreateClassInstance());
			if (IAnimationSerializable* ser = drxinterface_cast<IAnimationSerializable>(instance.get()))
				ser->Serialize(ar);
			else
				instance.reset();
		}
...
DRXREGISTER_CLASS(CPoseModifierSetup)
...
IAnimationPoseModifier* CPoseModifierSetup::GetEntry(i32 index)
{
...
i32 CPoseModifierSetup::GetEntryCount()
{
