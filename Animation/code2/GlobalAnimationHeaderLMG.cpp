...
bool ReadVGrid(const XmlNodeRef& xmlNode, u32k expectedVirtualExampleCount, DynArray<TVirtualExample>& virtualExampleGrid)
{
	DRX_ASSERT(xmlNode);

	u32k virtualExampleCount = xmlNode->getChildCount();
	...
	for (u32 i = 0; i < virtualExampleCount; ++i)
	{
...
bool WriteVGrid(const XmlNodeRef& xmlNode, const DynArray<TVirtualExample>& virtualExampleGrid)
{
	...
	u32k virtualExampleCount = virtualExampleGrid.size();
	for (u32 i = 0; i < virtualExampleCount; ++i)
	{
...
bool ReadVGrid(const XmlNodeRef& xmlNode, GlobalAnimationHeaderLMG& gah)
{
	...
	{
		u32k expectedVirtualExampleCount = gah.m_DimPara[0].m_cells;
		return ReadVGrid(xmlNode, expectedVirtualExampleCount, gah.m_VirtualExampleGrid1D);
	}
	else if (gah.m_Dimensions == 2)
	{
		u32k expectedVirtualExampleCount = gah.m_DimPara[0].m_cells * gah.m_DimPara[1].m_cells;
		return ReadVGrid(xmlNode, expectedVirtualExampleCount, gah.m_VirtualExampleGrid2D);
	}
	else if (gah.m_Dimensions == 3)
	{
		u32k expectedVirtualExampleCount = gah.m_DimPara[0].m_cells * gah.m_DimPara[1].m_cells * gah.m_DimPara[2].m_cells;
...
{
	if (IsAssetLMG() == 0)
		DrxFatalError("DinrusXAnimation: data mismatch");

	if (nForceReloading == 0)
	{
		u32 numAnims = m_numExamples;//m_arrBSAnimations.size();
		if (numAnims)
			return true; //already loaded and initialized
	}

	tukk pathnameLMG = GetFilePath();
	tukk fileExt = PathUtil::GetExt(pathnameLMG);

	u32 IsLMG = (stricmp(fileExt, "lmg") == 0);
	if (IsLMG)
		DrxFatalError("DinrusXAnimation:  LMGs not supported any more: %s! please use only 'BSpace' and 'COMB' from now on", pathnameLMG);
...
bool GlobalAnimationHeaderLMG::LoadFromXML(CAnimationSet* pAnimationSet, XmlNodeRef root)
{
	...
	if (root == 0)
	{
		i32 bPakPriority = gEnv->pDrxPak->GetPakPriority();
		bool bIsOnDisk = gEnv->pDrxPak->IsFileExist(pathnameLMG, IDrxPak::eFileLocation_OnDisk);
		bool bIsInPak = gEnv->pDrxPak->IsFileExist(pathnameLMG, IDrxPak::eFileLocation_InPak);
		if (bPakPriority == 0)
		{
			...
			if (bIsOnDisk)
			{
				...
				gEnv->pLog->LogError("DinrusXAnimation %s: %s", m_Status.c_str(), pathnameLMG);
				return false;
			}
			if (bIsInPak)
			{
		...
		gEnv->pLog->LogError("DinrusXAnimation %s: %s", m_Status.c_str(), pathnameLMG);
		//DrxFatalError("DinrusXAnimation:  Parametric-Group not found: %s", pathnameLMG);
		return false;
	}
	...
	if (strcmp(XMLTAG, "ParaGroup") == 0)
	{
		...
		for (u32 c = 0; c < numChilds; c++)
		{
			...
			{
				u32 nFlags = 0;
				nodeList->getAttr("Idle2Move", nFlags);                       //will go as soon as DrxMannegin is up and running
				if (nFlags)
					m_VEG_Flags |= CA_VEG_I2M; //this is a case for DrxMannequin
				continue;
			}
			...
			if (strcmp(ListTag, "Dimensions") == 0)
			{
				...
				for (u32 d = 0; d < m_Dimensions; d++)
				{
					XmlNodeRef nodeExample = nodeList->getChild(d);
					tukk ExampleTag = nodeExample->getTag();
					if (strcmp(ExampleTag, "Param") == 0)
					{
						...
						for (i32 iParam = 0; iParam < eMotionParamID_COUNT; ++iParam)
						{
							SMotionParameterDetails details;
							gEnv->pCharacterUpr->GetMotionParameterDetails(details, static_cast<EMotionParamID>(iParam));
			...
			if (strcmp(ListTag, "AdditionalExtraction") == 0)
			{
				...
				for (u32 d = 0; d < m_ExtractionParams; d++)
				{
					XmlNodeRef nodeExample = nodeList->getChild(d);
					tukk ExampleTag = nodeExample->getTag();
					if (strcmp(ExampleTag, "Param") == 0)
			...
			if (strcmp(ListTag, "ExampleList") == 0)
			{
				...
				for (u32 i = 0; i < m_numExamples; i++)
				{
					...
					if (strcmp(ExampleTAG, "Example") == 0)
					{
						...
						//	tukk pDebugName2 = aName.GetName_DEBUG();
						//	i32 id2=pAnimationSet->GetAnimIDByName(pDebugName2);
						i32 id = pAnimationSet->GetAnimIDByCRC(aName.m_CRC32);
						if (id < 0)
						{
							//name not found in animation-list (chrparams file error)
							tukk pDebugName = aName.GetName_DEBUG();
						...
						else
						{
							...
							i32 gaid = pAnim->m_nGlobalAnimId;
							GlobalAnimationHeaderCAF& rCAF = g_AnimationUpr.m_arrGlobalCAF[gaid];
			...
			if (strcmp(ListTag, "ExamplePseudo") == 0)
			{
				u32 num = nodeList->getChildCount();
				for (u32 i = 0; i < num; i++)
				{
					XmlNodeRef nodeExample = nodeList->getChild(i);
					tukk ExampleTag = nodeExample->getTag();
			...
			if (strcmp(ListTag, "Blendable") == 0)
			{
				...
				for (u32 i = 0; i < num; i++)
				{
					...
					if (strcmp(ExampleTag, "Face") == 0)
					{
						...
						tukk facePointNames[] = { "p0", "p1", "p2", "p3", "p4", "p5", "p6", "p7" };
						static_assert(DRX_ARRAY_COUNT(facePointNames) == DRX_ARRAY_COUNT(face.idx), "facePointNames[]'s size is expected to match BSBlendable::idx[]");

						for (size_t p = 0; p < DRX_ARRAY_COUNT(facePointNames); ++p)
						{
							if (nodeExample->getAttr(facePointNames[p], face.idx[p]))
							{
								if (face.idx[p] >= (numExamples + numPseudo))
								{
									m_Status.Format("Error: Blend annotation %d contains a reference to non-existing example with index %d.", i32(i), i32(face.idx[p]));
									gEnv->pLog->LogError("DinrusXAnimation %s: %s", m_Status.c_str(), pathnameLMG);
									return false;
								}
								face.num++;
			...
			{
				gEnv->pLog->LogError("DinrusXAnimation: <MotionCombination> element found in bspace '%s'. Those are not supported anymore. Please use dedicated animation layers to achieve the same result instead.", pathnameLMG);
				continue;
			}
		}
			...
			if (strcmp(ListTag, "BlendSpaces") == 0)
			{
				...
				for (u32 i = 0; i < num; i++)
				{
					...
					if (strcmp(ExampleTag, "BlendSpace") == 0)
					{
						...
						u32 nCRC32 = CCrc32::ComputeLowercase(strFilePath.c_str());
						m_arrCombinedBlendSpaces[i].m_FilePathCRC32 = nCRC32;
						u32 numLMG = g_AnimationUpr.m_arrGlobalLMG.size();
						for (u32 id = 0; id < numLMG; id++)
						{
							if (g_AnimationUpr.m_arrGlobalLMG[id].m_FilePathCRC32 == nCRC32)
							{
								u32 isBSValid = g_AnimationUpr.m_arrGlobalLMG[id].IsAssetLMGValid();
								if (isBSValid == 0)
								{
									tukk strBSFilePath = g_AnimationUpr.m_arrGlobalLMG[id].GetFilePath();
...
bool GlobalAnimationHeaderLMG::Export2HTR(tukk szAnimationName, tukk savePath, const CDefaultSkeleton* pDefaultSkeleton, const CSkeletonAnim* pSkeletonAnim) const
{
	u32 num = pSkeletonAnim->GetNumAnimsInFIFO(0);
	if (num == 0)
		return 0; //fail
	...
	u32 nFrames = u32(1.0f / fTWNDeltaTime + 1.5f);
	...
	for (u32 a = 0; a < lmg.m_numExamples; a++)
	{
		i32 nAnimID = lmg.m_nAnimID[a];
...
bool GlobalAnimationHeaderLMG::ExportVGrid() const
{
	...
	tukk const filepath = GetFilePath();
	XmlNodeRef xmlRoot = gEnv->pSystem->LoadXmlFromFile(filepath);
	if (!xmlRoot)
	{
		gEnv->pLog->LogError("DinrusXAnimation: Failed to load xml file '%s', will try to create it", filepath);
		return false;
	}
	...
	if (!saveFileSuccess)
	{
		gEnv->pLog->LogError("DinrusXAnimation: Failed to save xml file '%s'", filepath);
		return false;
	}
...
void GlobalAnimationHeaderLMG::ParameterExtraction(const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, u32 nParaID, u32 d)
{
...
void GlobalAnimationHeaderLMG::Init_MoveSpeed(const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, u32 dim)
{
	u32 init_count = 0;
	u32 numAssets = m_numExamples;//m_arrBSAnimations2.size();
	for (u32 i = 0; i < numAssets; i++)
	{
		i32 animID = pAnimationSet->GetAnimIDByCRC(m_arrParameter[i].m_animName.m_CRC32);
		assert(animID >= 0);
		i32 globalID = pAnimationSet->GetGlobalIDByAnimID_Fast(animID);
		assert(globalID >= 0);
		if (globalID < 0)
			break;
		...
		{
			DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_ERROR, "Asset for parameter-extraction not in memory: '%s'", rCAF.GetFilePath());
			break;
		}
		...
		u32 numKeys = u32(duration * ANIMATION_30Hz + 1);
		if (numKeys == 1)
			continue;
		init_count++;
		...
		u32 skey = u32(m_DimPara[dim].m_skey * (numKeys - 1));
		u32 ekey = u32(m_DimPara[dim].m_ekey * (numKeys - 1));
...
void GlobalAnimationHeaderLMG::Init_TurnSpeed(const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, u32 dim)
{
	u32 init_count = 0;
	u32 numAssets = m_numExamples;//m_arrBSAnimations2.size();
	for (u32 i = 0; i < numAssets; i++)
...
void GlobalAnimationHeaderLMG::Init_TurnAngle(const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, u32 dim)
{
	u32 init_count = 0;
	u32 numAssets = m_numExamples; //m_arrBSAnimations2.size();
	for (u32 i = 0; i < numAssets; i++)
...
void GlobalAnimationHeaderLMG::Init_TravelAngle(const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, u32 dim)
{
	u32 init_count = 0;
	u32 numAssets = m_numExamples;//m_arrBSAnimations2.size();
	for (u32 i = 0; i < numAssets; i++)
...
void GlobalAnimationHeaderLMG::Init_SlopeAngle(const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, u32 dim)
{
	u32 init_count = 0;
	u32 numAssets = m_numExamples;//m_arrBSAnimations2.size();
	for (u32 i = 0; i < numAssets; i++)
...
void GlobalAnimationHeaderLMG::Init_TravelDist(const CAnimationSet* pAnimationSet, const CDefaultSkeleton* pDefaultSkeleton, u32 dim)
{
	u32 init_count = 0;
	u32 numAssets = m_numExamples; //m_arrBSAnimations.size();
	for (u32 i = 0; i < numAssets; i++)
	{
		i32 animID = pAnimationSet->GetAnimIDByCRC(m_arrParameter[i].m_animName.m_CRC32);
		assert(animID >= 0);
