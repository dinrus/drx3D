...
#include <drx3D/Animation/PoseAlignerChain.h>
...
ILINE const IKLimbType* FindIkLimbType(const SAnimationPoseModifierParams& params, LimbIKDefinitionHandle limbHandle)
{
	...
	i32 limbIndex = defaultSkeleton.GetLimbDefinitionIdx(limbHandle);
	if (limbIndex < 0)
		return NULL;
...
ILINE bool AlignJointToPlane(const SAnimationPoseModifierParams& params, u32 jointIndex, const Vec3& jointVector, const Vec3& planeNormal)
{
	Skeleton::CPoseData* pPoseData = Skeleton::CPoseData::GetPoseData(params.pPoseData);
	if (!pPoseData)
		return false;
...
DRXREGISTER_CLASS(CPoseAlignerChain)
...
void CPoseAlignerChain::Initialize(LimbIKDefinitionHandle solver, i32 contactJointIndex)
{
...
bool CPoseAlignerChain::Execute(const SAnimationPoseModifierParams& params)
{
	...
	i16k* pPoseTransformationIndices = &m_stateExecute.m_pIkLimbType->m_arrLimbChildren[0];
