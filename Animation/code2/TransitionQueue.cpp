...
bool IsAnimationLoaded(const CAnimationSet& animationSet, const CAnimation& animation)
{
	...
	if (pParametric == 0)
	{
		i32 nAnimID = animation.GetAnimationId();
	...
	u32k num = pParametric->m_numExamples;
	for (u32 a = 0; (a < num) && bInMemory; ++a)
	{
		i32k nAnimID = pParametric->m_nAnimID[a];
		const ModelAnimationHeader* pAnim = animationSet.GetModelAnimationHeader(nAnimID);
		if (pAnim->m_nAssetType == CAF_File)
		{
			i32 nGlobalID = pAnim->m_nGlobalAnimId;
...
void TriggerEvent(tukk animationName, uint animationId, const CAnimEventData& event, CCharInstance& instance)
{
...
void TriggerEvents(tukk animationName, uint animationId, const IAnimEventList& events, float timeFrom, float timeTo, CCharInstance& instance)
{
	u32k eventCount = events.GetCount();
...
{
	u32k numAnimsInQueue = m_animations.size();

	// search for delay conditions and get rid of them
	u32 nMaxActiveInQueue = min(MAX_EXEC_QUEUE, numAnimsInQueue);
...
void CTransitionQueue::PushAnimation(const CAnimation& animation)
{
	...
	for (u32 i = 0; i < nMaxActiveInQueue; i++)
	{
		...
		u32 NotInMemory = !IsAnimationLoaded(*m_pAnimationSet, anim);
		...
		u32 StartAtKeytime = anim.HasStaticFlag(CA_START_AT_KEYTIME);
		u32 StartAfter = anim.HasStaticFlag(CA_START_AFTER);
		u32 Idle2Move = anim.HasStaticFlag(CA_IDLE2MOVE);
		u32 Move2Idle = anim.HasStaticFlag(CA_MOVE2IDLE);
		if ((StartAtKeytime + StartAfter + Idle2Move + Move2Idle + NotInMemory) == 0)
			anim.m_DynFlags[0] |= CA_ACTIVATED;
	}
}

void CTransitionQueue::UnloadAnimationAssets(i32 index)
{
	CAnimation& animation = m_animations[index];

	i32 nAnimID = animation.GetAnimationId();
	i32 nGlobalID = m_pAnimationSet->GetGlobalIDByAnimID_Fast(nAnimID);
	...
	if (pParametric == NULL)
	{
		...
		{
#ifdef _DEBUG
			tukk pname = pAnim->GetAnimName();
#endif
			GlobalAnimationHeaderCAF& rGAH = g_AnimationUpr.m_arrGlobalCAF[nGlobalID];
	...
	{
		u32 numAnims0 = pParametric->m_numExamples;
		for (u32 i = 0; i < numAnims0; i++)
		{
			...
			{
#ifdef _DEBUG
				tukk pname = pSamplerAnim->GetAnimName();
#endif
				i32 nGlobalIDByAnimID = m_pAnimationSet->GetGlobalIDByAnimID_Fast(nAnimID);
				GlobalAnimationHeaderCAF& rGAH = g_AnimationUpr.m_arrGlobalCAF[nGlobalIDByAnimID];
		...
		{
			DRX_ASSERT_TRACE(g_usedParametrics[slot], ("Releasing unallocated parametric %d", slot));
			g_usedParametrics[slot] = 0;
		}
		else
		{
			DRX_ASSERT_TRACE(0, ("Releasing unpooled parametric %d", pParametric));
		}
...
{
	if (index >= (u32)m_animations.size())
		return false;
...
{
	u32 numAnimsInFifo = m_animations.size();
...
{
	u32 numAnimsInLayer = m_animations.size();
...
void CTransitionQueue::ManualSeekAnimation(uint index, float time2, bool bTriggerEvents, CCharInstance& instance)
{
	...
	if (animation.HasStaticFlag(CA_MANUAL_UPDATE) || animation.HasStaticFlag(CA_TRACK_VIEW_EXCLUSIVE))
	{
		...
		{
			// this is a CAF file
			i32 animID = animation.GetAnimationId();
			assert(animID >= 0);
			i32 globalID = m_pAnimationSet->GetGlobalIDByAnimID_Fast(animID);
			assert(globalID >= 0);

			u32 numCAFs = g_AnimationUpr.m_arrGlobalCAF.size();
			assert((u32)globalID < numCAFs);
			if ((globalID >= 0) && (globalID < (i32)numCAFs))
			{
				const ModelAnimationHeader* pAnim = m_pAnimationSet->GetModelAnimationHeader(animID);
				if (pAnim->m_nAssetType == CAF_File)
				{
					const GlobalAnimationHeaderCAF& rGlobalAnimHeader = g_AnimationUpr.m_arrGlobalCAF[globalID];
		...
		else
		{
			...
			i32 globalID = pAnim->m_nGlobalAnimId;
			assert(globalID >= 0 && globalID < i32(g_AnimationUpr.m_arrGlobalLMG.size()));
			if (globalID >= 0 && globalID < i32(g_AnimationUpr.m_arrGlobalLMG.size()))
			{
				const GlobalAnimationHeaderLMG& rGlobalAnimHeaderLMG = g_AnimationUpr.m_arrGlobalLMG[globalID];
...
void CTransitionQueue::ApplyManualMixingWeight(uint numAnims)
{
	...
	case 0:
		break;
	default:
		DrxLog("Too many ( > 2 ) animations in transition queue in TrackViewExclusive mode");
	}
}
...
void CTransitionQueue::TransitionsBetweenSeveralAnimations(uint numAnims)
{
	...
	for (u32 i = 1; i < numAnims; i++)
	{
		...
		u32 timewarp = rCurAnimation.m_nStaticFlags & CA_TRANSITION_TIMEWARPING;
		if (timewarp)
		{
...
void CTransitionQueue::AdjustAnimationTimeForTimeWarpedAnimations(uint numAnims)
{
	...
	for (u32 a = start; a < (start + accumented); a++)
...
void CPoseModifierQueue::SEntry::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
bool CPoseModifierQueue::Push(const IAnimationPoseModifierPtr pPoseModifier, tukk name, const bool bQueued)
{
	i32k queueIndex = bQueued ? !m_currentIndex : m_currentIndex;
...
u32 CPoseModifierQueue::GetSize()
{
	return m_entries[0].get_alloc_size() + m_entries[1].get_alloc_size();
}

void CPoseModifierQueue::AddToSizer(IDrxSizer* pSizer) const
{
