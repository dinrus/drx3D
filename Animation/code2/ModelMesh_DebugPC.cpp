...
void CModelMesh::ExportModel(IRenderMesh* pIRenderMesh)
{
	...
	u32 numExtIndices = pIRenderMesh->GetIndicesCount();
	u32 numExtVertices = pIRenderMesh->GetVerticesCount();
	u32 vsize = m_arrSrcPositions.size();
	...
	i32 nPositionStride;
	u8* pPositions = pIRenderMesh->GetPosPtr(nPositionStride, FSL_READ);
	if (pPositions == 0)
		return;
	i32 nQTangentStride;
	u8* pQTangents = pIRenderMesh->GetQTangentPtr(nQTangentStride, FSL_READ);
	if (pQTangents == 0)
		return;
	i32 nUVStride;
	u8* pUV = pIRenderMesh->GetUVPtr(nUVStride, FSL_READ);
	if (pUV == 0)
		return;
	i32 nSkinningStride;
	u8* pSkinningInfo = pIRenderMesh->GetHWSkinPtr(nSkinningStride, FSL_READ);    //pointer to weights and bone-id
	if (pSkinningInfo == 0)
		return;
	...
	if (m_arrSrcBoneIDs.size() != numExtVertices)
	{
		...
		u32 numSubsets = m_arrRenderChunks.size();
		for (u32 s = 0; s < numSubsets; s++)
		{
			u32 startIndex = m_arrRenderChunks[s].m_nFirstIndexId;
			u32 endIndex = m_arrRenderChunks[s].m_nFirstIndexId + m_arrRenderChunks[s].m_nNumIndices;
			for (u32 idx = startIndex; idx < endIndex; ++idx)
			{
				...
				u32 _id0 = hwBoneIDs[0];
				u32 _id1 = hwBoneIDs[1];
				u32 _id2 = hwBoneIDs[2];
				u32 _id3 = hwBoneIDs[3];
				if (hwWeights[0])
	...
	for (u32 v = 0; v < numExtVertices; v++)
	{
	...
	for (u32 f = 0; f < numExtIndices; f = f + 3)
	{
	...
	//--   dump all subsets
	//--------------------------------------------------------------------------------
	/*u32 numSubsets = m_arrSubsets.size();
...
void CModelMesh::DrawWireframeStatic(const Matrix34& rRenderMat34, u32 color)
{
	if (m_pIRenderMesh == 0)
		return;

	u32 numExtIndices = m_pIRenderMesh->GetIndicesCount();
	u32 numExtVertices = m_pIRenderMesh->GetVerticesCount();
	...
	for (u32 e = 0; e < numExtVertices; e++)
	{
...
void CModelMesh::DrawDebugInfo(CDefaultSkeleton* pCSkel, i32 nLOD, const Matrix34& rRenderMat34, i32 DebugMode, IMaterial* pMaterial, CRenderObject* pObj, const SRendParams& RendParams, bool isGeneralPass, IRenderNode* pRenderNode, const AABB& aabb,const SRenderingPassInfo &passInfo)
{
	if (m_pIRenderMesh == 0)
		return;
	...
	i32 numLODs = frameLodInfo.nMaxLod;
	i32 index = 0;
	float color[4] = { 1, 1, 1, 1 };

	i32 nTris = m_pIRenderMesh->GetVerticesCount();
	i32 nMats = m_pIRenderMesh->GetChunks().size();
	i32 nRenderMats = 0;
	...
	case 2:
		{
			...
			IMaterialUpr* pMatMan = g_pI3DEngine->GetMaterialUpr();
			i32 fMult = 1;
			//i32 nTris = m_pDefaultSkeleton->GetRenderMesh(nLOD)->GetSysVertCount();
	...
	case 4:
		if (m_pIRenderMesh)
		{
			i32 nTexMemUsage = m_pIRenderMesh->GetTextureMemoryUsage(pMaterial);
	...
	case 6:
		...
		IRenderAuxText::DrawLabelExF(trans, 1.3f, color, true, true, "%d,%d,%d,%d",
			                          (i32)(RendParams.AmbientColor.r * 255.0f), (i32)(RendParams.AmbientColor.g * 255.0f), (i32)(RendParams.AmbientColor.b * 255.0f), (i32)(RendParams.AmbientColor.a * 255.0f)
			                          );
		}
		break;
	...
	case 19:  // Displays the triangle count of physic proxies.
		if (!bNoText)
		{
			...
			for (i32 i = pCSkel->GetJointCount() - 1; i >= 0; --i)
			{
				if (pgeom = pCSkel->GetJointPhysGeom((u32)i))
	...
	case 24:
	{
		...
		if (!bNoText && isGeneralPass)
		{
			...
			if (pRenderNode)
			{
				...
				if (bLodFaceArea)
				{
					for (u32 i = 0; i < SMeshLodInfo::s_nMaxLodCount; ++i)
					{
			...
			i32k   viewDistRatio = pRenderNode ? pRenderNode->GetViewDistRatioVal() : -1;
