...
CAttachmentMerger::MeshStreams::MeshStreams(IRenderMesh* pRenderMesh, u32 lockFlags, bool bGetSkinningInfos)
	: pMesh(pRenderMesh)
	, pSkinningInfos(NULL)
{
...
SVF_W4B_I4S& CAttachmentMerger::MeshStreams::GetSkinningInfo(i32 nVtxIndex)
{
	DRX_ASSERT(nVtxIndex < pMesh->GetVerticesCount());
	return static_cast<SVF_W4B_I4S*>(pSkinningInfos)[nVtxIndex];
}

vtx_idx& CAttachmentMerger::MeshStreams::GetVertexIndex(i32 i)
{
	DRX_ASSERT(i < pMesh->GetIndicesCount());
	return pIndices[i];
}
...
uint CAttachmentMerger::MergeContext::EstimateRequiredMemory() const
{
	...
	uint nResult = 0;
	for (i32 lod = 0; lod < nMaxLodCount; ++lod)
	{
...
CAttachmentMerger::AttachmentRenderData::AttachmentRenderData(IAttachment* pAtt)
{
	...
	if (nLodCount > 0)
	{
		for (i32 i = nLodCount; i < MAX_STATOBJ_LODS_NUM; ++i)
...
IMaterial* CAttachmentMerger::GetAttachmentMaterial(const IAttachment* pAttachment, i32 lod)
{
...
void CAttachmentMerger::MergeAttachments(DynArray<_smart_ptr<SAttachmentBase>>& arrAttachments, DynArray<_smart_ptr<CAttachmentMerged>>& arrMergedAttachments, CAttachmentUpr* pAttachmentUpr)
{
	...
	for (uint i = 0; i < mergeableAttachments.size(); ++i)
	{
		...
		for (uint j = i + 1; j < mergeableAttachments.size(); ++j)
		{
			...
			{
				DRX_ASSERT(!mergeableAttachments[j].pAttachment->IsMerged());
		...
		if (curAttachmentList.size() > 1)
		{
			...
			if (!pMergeTarget)
			{
				...
				pMergeTarget = new CAttachmentMerged(attachmentName, pAttachmentUpr);
				...
				u32k loadingFlags = (pAttachmentUpr->m_pSkelInstance->m_CharEditMode ? CA_CharEditModel : 0);
				g_pCharacterUpr->RegisterModelSKIN(pSkin, loadingFlags);
				g_pCharacterUpr->RegisterInstanceSkin(pSkin, pMergeTarget->m_pMergedSkinAttachment);
				pMergeTarget->m_pMergedSkinAttachment->m_pModelSkin = pSkin;

				arrMergedAttachments.push_back(pMergeTarget);
			}

			Merge(pMergeTarget, curAttachmentList, pAttachmentUpr);
...
void CAttachmentMerger::OnDeleteMergedAttachment(CAttachmentMerged* pAttachment)
{
	...
	DRX_ASSERT(nSize <= m_nBytesAllocated);
...
{
	DRX_ASSERT(srcStreams.pSkinningInfos);
	...
	for (uint k = 0; k < numVertices; ++k)
	{
		...
		DRX_ASSERT(maxBoneIndex <= std::numeric_limits<JointIdType>::max());
...
{
	DRX_ASSERT(dstStreams.pSkinningInfos);
...
void CAttachmentMerger::Merge(CAttachmentMerged* pDstAttachment, const DynArray<AttachmentRenderData>& attachmentData, CAttachmentUpr* pAttachmentUpr)
{
	CSkin* pDstSkin = static_cast<CSkin*>(pDstAttachment->m_pMergedSkinAttachment->m_pModelSkin);
	DRX_ASSERT(pDstSkin->m_arrModelMeshes.size() > 0);

	DynArray<CSkin::SJointInfo> arrModelJoints = pDstSkin->m_arrModelJoints;
	const uint maxLODCount = pDstSkin->GetNumLODs();

	CCharInstance* pInstanceSkel = pAttachmentUpr->m_pSkelInstance;
	...
	for (i32 i = 0; i < attachmentData.size(); ++i)
	{
	...
	for (uint lod = 0; lod < maxLODCount; ++lod)
	{
		...
		// Copy data from attachments
		{
			...
			for (i32 i = 0; i < attachmentData.size(); ++i)
			{
				...
				{
					DRX_ASSERT(lod != 0 || pAttachmentUpr->FindExtraBone(curAttachmentData.pAttachment) == -1);

					JointIdType jointID = pAttachmentUpr->FindExtraBone(curAttachmentData.pAttachment);
					if (jointID == (JointIdType) - 1)
					{
						jointID = pAttachmentUpr->AddExtraBone(curAttachmentData.pAttachment);

						QuatT jointDefaultAbsolute(IDENTITY);
						JointIdType idAttachedTo = curAttachmentData.pAttachment->GetJointID();

						if (idAttachedTo < pAttachmentUpr->GetSkelInstance()->GetIDefaultSkeleton().GetJointCount())
							jointDefaultAbsolute = pAttachmentUpr->GetSkelInstance()->GetIDefaultSkeleton().GetDefaultAbsJointByID(idAttachedTo);
			...
			DRX_ASSERT(vertexBase == numVertices);
			DRX_ASSERT(indexBase == numIndices);
			DRX_ASSERT(maxBoneIndex < std::numeric_limits<JointIdType>::max());
		}
...
void CAttachmentMerger::UpdateIndices(CAttachmentMerged* pAttachment, i32 lod, const DynArray<std::pair<u32, u32>>& srcRanges)
{
	DRX_ASSERT(pAttachment && pAttachment->m_pMergedSkinAttachment->m_pModelSkin);
	DRX_ASSERT(lod >= 0 && lod <= MAX_STATOBJ_LODS_NUM);
