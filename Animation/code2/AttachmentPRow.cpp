...
void CAttachmentPROW::UpdateRow(Skeleton::CPoseData& rPoseData)
{
	...
	if (pSkelInstance->m_CharEditMode & CA_CharacterTool)
	{
		...
		for (u32 i = 0; i < m_rowparams.m_nParticlesPerRow; i++)
		{
			if (m_rowparams.m_arrParticles[i].m_childID != JointIdType(-1))
			{
				...
				tukk pname = rDefaultSkeleton.GetJointNameByID(jid);
				...
				if (dist > 0.0001f)
				{
					g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.3f, ColorF(1, 0, 0, 1), false, "Joint '%s' not aligned with X-Axis", pname), g_YLine += 26.0f;
	...
	for (u32 i = 0; i < m_rowparams.m_nParticlesPerRow; i++)
	{
		...
		{
			u32k pid = rDefaultSkeleton.GetJointParentIDByID(dm.m_jointID);
	...
	m_rowparams.UpdatePendulumRow(m_pAttachmentUpr, rPoseData, GetName());
};
...
u32 CAttachmentPROW::SetJointName(tukk szJointName)
{
	m_nRowJointID = -1;
	...
	if (nJointID < 0)
	{
		...
		g_pILog->LogError("AttachObjectToJoint is called for bone \"%s\", which is not in the model \"%s\". Ignoring, but this may cause a crash because the corresponding object won't be detached after it's destroyed", szJointName, m_pAttachmentUpr->m_pSkelInstance->m_pDefaultSkeleton->GetModelFilePath());
		return 0;
	}
...
void CAttachmentPROW::PostUpdateSimulationParams(bool bAttachmentSortingRequired, tukk pJointName)
{
	m_pAttachmentUpr->m_TypeSortingRequired += bAttachmentSortingRequired;
	m_rowparams.PostUpdate(m_pAttachmentUpr, 0);
};

void CAttachmentPROW::HideInRecursion(u32 x)
{
...
void CAttachmentPROW::HideInShadow(u32 x)
{
...
void CPendulaRow::PostUpdate(const CAttachmentUpr* pAttachmentUpr, tukk pJointName)
{
...
void CPendulaRow::UpdatePendulumRow(const CAttachmentUpr* pAttachmentUpr, Skeleton::CPoseData& rPoseData, tukk pAttName)
{
	const CCharInstance* pSkelInstance = pAttachmentUpr->m_pSkelInstance;
	const CDefaultSkeleton& rDefaultSkeleton = *pSkelInstance->m_pDefaultSkeleton;
	QuatTS rPhysLocation = pSkelInstance->m_location;
	...
	if (m_nClampMode == RowSimulationParams::TRANSLATIONAL_PROJECTION)
	{
		...
		u32k numProxies = allowTranslation && m_nProjectionType && (m_vCapsule.x + m_vCapsule.y) ? m_arrProxyIndex.size() : 0;
		for (u32 i = 0; i < m_nParticlesPerRow; i++)
		{
	...
	u16 arrSetupError[MAX_JOINTS_PER_ROW];
	memset(arrSetupError, 0, m_nParticlesPerRow * sizeof(u16));
	...
	if (allowSimulation)
	{
		...
		for (u32 i = 0; i < m_nParticlesPerRow; i++)
		{
			...
			(f32&)pAttachmentUpr->m_fTurbulenceLocal += m_vTurbulence.x;
			f32 val = pAttachmentUpr->m_fTurbulenceGlobal + pAttachmentUpr->m_fTurbulenceLocal;
		...
		u32 relaxLinks = m_nParticlesPerRow - !m_cycle;
		for (u32 c = 0; c < m_relaxationLoops; c++)
		{
			for (u32 i = 0; i < m_nParticlesPerRow; i++)
			{
				Particle& dm = m_arrParticles[i];
			...
			for (u32 i0 = 0; i0 < relaxLinks; i0++)
			{
				...
				u32 i1 = (i0 + 1) % m_nParticlesPerRow;
		...
		if (nDrawDynamicProxies || m_useDebugSetup || Console::GetInst().ca_DrawAllSimulatedSockets)
		{
			...
			for (u32 i = 0; i < m_nParticlesPerRow && nBindPose == 0; i++)
			{
				u32 i0 = i;
				u32 i1 = (i + 1) % m_nParticlesPerRow;
				Vec3 p0 = arrParticles[i0];
				Vec3 p1 = arrParticles[i1];
				if (i0 < i1 || m_cycle)
	...
	if (nDrawDynamicProxies || m_useDebugSetup || Console::GetInst().ca_DrawAllSimulatedSockets)
	{
		for (u32 i = 0; i < m_nParticlesPerRow; i++)
		{
			...
			u32 res = (m_useDebugSetup || Console::GetInst().ca_DrawAllSimulatedSockets);
			if (res)
			{
...
void CPendulaRow::Draw(const QuatTS& qt, const ColorB clr, u32k tesselation, const Vec3& vdir)
{
