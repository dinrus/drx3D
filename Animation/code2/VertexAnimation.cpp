...
{
	u32 numMeshes = s_softwareRenderMeshes.size();
	...
	u32 numMeshesVCloth = s_softwareRenderMeshesVCloth.size();
...
void CVertexAnimation::OverrideFrameWeightByName(const ISkin* pISkin, tukk name, float weight)
{
...
uint FindVertexFrameIndex(const CSoftwareVertexFrames& vertexFrames, tukk name)
{
...
void CVertexAnimation::CreateFrameStates(const CSoftwareVertexFrames& vertexFrames, const CDefaultSkeleton& skeleton)
{
	...
	for (u32 i = 0, limit = vertexFrames.GetCount(); i < limit; ++i)
	{
...
void CVertexAnimation::DrawVertexDebug(IRenderMesh* pRenderMesh, const QuatT& location, const SVertexAnimationJob* pVertexAnimation) const
{
	...
	if (parrDstPositions && parrDstTangents)
	{
		...
		u32 numDstPositions = arrDstPositions.size();
		...
		if (drawWireframe)
		{
			for (uint i = 0; i < DRX_ARRAY_COUNT(SKINNING_COLORS); ++i)
			...
			u32	numIndices = pIRenderMesh->GetIndicesCount();
		...
		if (drawNormals)
		{
			...
			u32 numExtSkinnedStream = arrExtSkinnedStream.size();
