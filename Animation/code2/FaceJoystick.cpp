	...
	m_videoMarkerScale(1.0f)
{
	...
	while (pAncestor)
	{
		tukk szChannelName = pAncestor ? pAncestor->GetIdentifier().GetString() : 0;
...
tukk CFacialJoystickChannel::GetName() const
{
	return (m_pChannel ? m_pChannel->GetIdentifier().GetString() : 0);
}

uk CFacialJoystickChannel::GetTarget()
{
	return m_pChannel;
}

i32 CFacialJoystickChannel::GetSplineCount()
{
	return (m_pChannel ? m_pChannel->GetInterpolatorCount() : 0);
}

ISplineInterpolator* CFacialJoystickChannel::GetSpline(i32 splineIndex)
{
...
tukk CFacialJoystickChannel::GetPath() const
{
...
void CFacialJoystick::SetName(tukk szName)
{
	m_name = szName;
}

tukk CFacialJoystick::GetName()
{
	return m_name.c_str();
}
...
{
	for (uint axis = 0; axis < NumChannelTypes; ++axis)
	{
		if (m_channels[axis])
...
void CFacialJoystickSet::HandleRemovedChannel(IFacialAnimChannel* pRemovedChannel)
{
	for (JoystickContainer::iterator itJoystick = m_joysticks.begin(); itJoystick != m_joysticks.end(); ++itJoystick)
	{
		...
		{
			IJoystick::ChannelType axis = IJoystick::ChannelType(i);
...
void CFacialJoystickSet::SetName(tukk name)
{
	m_name = name;
}

tukk CFacialJoystickSet::GetName() const
{
	return m_name.c_str();
}
...
i32 CFacialJoystickSet::GetJoystickCount() const
{
	return i32(m_joysticks.size());
}

IJoystick* CFacialJoystickSet::GetJoystick(i32 index)
{
...
void CFacialJoystickSet::Serialize(XmlNodeRef& node, bool bLoading)
{
	...
	{
	public:
		VecProperty(VecGetter getter, VecSetter setter, tukk name) : getter(getter), setter(setter), name(name) {}
	...
	enum {NUM_VEC_PROPERTIES = DRX_ARRAY_COUNT(vecProperties)};

	if (bLoading)
	{
		m_joysticks.clear();

		for (i32 childIndex = 0; node && childIndex < node->getChildCount(); ++childIndex)
		{
			XmlNodeRef childNode = (node ? node->getChild(childIndex) : (XmlNodeRef)0);
			if (childNode && stricmp(childNode->getTag(), "Joystick") == 0)
			{
				...
				{
					tukk szName = joystickNode->getAttr("Name");
				...
				for (i32 joystickChildIndex = 0; joystickNode && joystickChildIndex < joystickNode->getChildCount(); ++joystickChildIndex)
				{
					XmlNodeRef joystickChildNode = (joystickNode ? joystickNode->getChild(joystickChildIndex) : (XmlNodeRef)0);

					if (joystickChildNode && stricmp(joystickChildNode->getTag(), "Channel") == 0)
					{
						i32 axis = -1;
						if (joystickChildNode && joystickChildNode->haveAttr("Axis"))
						{
							if (stricmp(joystickChildNode->getAttr("Axis"), "Horizontal") == 0)
								axis = IJoystick::ChannelTypeHorizontal;
							else if (stricmp(joystickChildNode->getAttr("Axis"), "Vertical") == 0)
								axis = IJoystick::ChannelTypeVertical;
						}

						tukk szChannelName = 0;
					...
					{
						for (i32 vecIndex = 0; vecIndex < NUM_VEC_PROPERTIES; ++vecIndex)
						{
							if (joystickChildNode && vecProperties[vecIndex].name && stricmp(vecProperties[vecIndex].name, joystickChildNode->getTag()) == 0)
							{
								Vec2 v(10, 10);
	...
	{
		for (i32 joystickIndex = 0, end = i32(m_joysticks.size()); joystickIndex < end; ++joystickIndex)
		{
			...
			tukk axisNames[] = { "Horizontal", "Vertical" };
			enum {NUM_AXES = DRX_ARRAY_COUNT(axisNames)};
			for (i32 axis = 0; axis < NUM_AXES; ++axis)
			{
				...
				if (channelNode)
				{
					...
					tukk name = (pChannel ? pChannel->GetPath() : 0);
