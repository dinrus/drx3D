...
//////////////////////////////////////////////////////////////////////////
u32 GlobalAnimationHeaderCAF::LoadCAF()
{
	LOADING_TIME_PROFILE_SECTION(GetISystem());
	tukk pname = m_FilePath.c_str();
	...
	content.m_id = (i32)std::distance(g_AnimationUpr.m_arrGlobalCAF.begin(), this);
	...
	{
		u32 numChunks = pChunkFile->NumChunks();

		IControllerRelocatableChain* pRelocateHead = NULL;
		tuk pStorage = (tuk)g_controllerHeap.WeakPin(content.m_hControllerData);
...
void GlobalAnimationHeaderCAF::StartStreamingCAF()
{
	...
	u32 IsLoaded = IsAssetLoaded();
	if (IsLoaded)
		return;
	u32 IsRequested = IsAssetRequested();
	if (IsRequested)
		return;
	...
	if (IsAssetOnDemand())
	{
		...
		pContent->m_id = (i32)std::distance(g_AnimationUpr.m_arrGlobalCAF.begin(), this);
...
uk GlobalAnimationHeaderCAFStreamContent::StreamOnNeedStorage(IReadStream* pStream, unsigned nSize, bool& bAbortOnFailToAlloc)
{
	tuk pData = NULL;

	if (static_cast<i32>(nSize) >= Console::GetInst().ca_MinInPlaceCAFStreamSize)
	{
		MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_CAF, 0, m_pPath);

		m_hControllerData = g_controllerHeap.AllocPinned(nSize, NULL);
		if (m_hControllerData.IsValid())
			pData = (tuk)g_controllerHeap.WeakPin(m_hControllerData);
	}
...
void GlobalAnimationHeaderCAFStreamContent::StreamAsyncOnComplete(IReadStream* pStream, unsigned nError)
{
	...
	tuk pStorage = (tuk)(hStorage.IsValid() ? g_controllerHeap.WeakPin(hStorage) : NULL);
	IControllerRelocatableChain* pRelocateHead = NULL;
...
void GlobalAnimationHeaderCAFStreamContent::StreamOnComplete(IReadStream* pStream, unsigned nError)
{
	...
	GlobalAnimationHeaderCAF& caf = g_AnimationUpr.m_arrGlobalCAF[m_id];
...
void GlobalAnimationHeaderCAF::FinishLoading(unsigned nError)
{
	...
	{
		i32 globalID = g_AnimationUpr.GetGlobalIDbyFilePath_CAF(GetFilePath());
...
bool GlobalAnimationHeaderCAFStreamContent::ParseChunkRange(IChunkFile* pChunkFile, u32 min, u32 max, bool bLoadOldChunks, tuk& pStorage, IControllerRelocatableChain*& pRelocateHead)
{
	assert((i32)max <= pChunkFile->NumChunks());
...
u32 GlobalAnimationHeaderCAF::DoesExistCAF()
{
	LOADING_TIME_PROFILE_SECTION(GetISystem());
	_smart_ptr<IChunkFile> pChunkFile = g_pI3DEngine->CreateChunkFile(true);
	if (!pChunkFile->Read(m_FilePath))
...
{
	u32k numController = m_arrController.size();
	...
	{
		DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "DinrusXAnimation: CAF file has no controllers: %s", GetFilePath());
	}
...
u32 GlobalAnimationHeaderCAFStreamContent::ParseChunkHeaders(IChunkFile* pChunkFile, bool& bLoadUncompressedChunksOut, bool& bLoadInPlaceOut, size_t& nUsefulSizeOut)
{
	u32k numChunk = pChunkFile->NumChunks();

	u32 numUncompressedControllers = 0;
	u32 numCompressedControllers = 0;
	...
	for (u32 i = 0; i < numChunk; i++)
	{
		...
		if (pChunkDesc->chunkType == ChunkType_Controller)
		{
			...
			default:
				{
					DrxFatalError("Unsupported controller chunk version: 0x%04x", pChunkDesc->chunkVersion);
					break;
				}
			}
		}
	...
	u32k numControllers = bLoadUncompressedChunks ? numUncompressedControllers : numCompressedControllers;
	...
	case TIMING_CHUNK_DESC_0919::VERSION:
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "Timing chunk (0x%04x) declares a nonpositive number of samples (%d). File: '%s'", pChunk->VERSION, pChunk->numberOfSamples, m_pPath);
				return false;
			}

			if ((std::abs(pChunk->samplesPerSecond - ANIMATION_30Hz) / ANIMATION_30Hz) > std::numeric_limits<float>::epsilon())
			{
				DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "Timing chunk (0x%04x) declares a currently unsupported framerate value (%.2f fps). Framerate will be overridden to %.2f fps. File: '%s'", pChunk->VERSION, pChunk->samplesPerSecond, ANIMATION_30Hz, m_pPath);
			}
...
bool GlobalAnimationHeaderCAFStreamContent::ReadMotionParameters(IChunkFile::ChunkDesc* pChunkDesc)
{
	...
	{
		DrxFatalError("SPEED_CHUNK_DESC not supported any more");
		return true;
	}

	if (pChunkDesc->chunkVersion == SPEED_CHUNK_DESC_1::VERSION)
	{
		DrxFatalError("SPEED_CHUNK_DESC_1 not supported any more");
		return true;
	}
	...
	if (pChunkDesc->chunkVersion == CHUNK_MOTION_PARAMETERS::VERSION)
	{
		...
		m_FilePathDBACRC32 = 0;        //if 0, then this is a streamable CAF file
		u32 nCompression = pChunk->mp.m_nCompression;
...
//////////////////////////////////////////////////////////////////////////
bool GlobalAnimationHeaderCAFStreamContent::ReadController(IChunkFile::ChunkDesc* pChunkDesc, bool bLoadUncompressedChunks, tuk& pStorage, IControllerRelocatableChain*& pRelocateHead)
{

	if (pChunkDesc->chunkVersion == CONTROLLER_CHUNK_DESC_0826::VERSION)
	{
		DrxFatalError("CONTROLLER_CHUNK_DESC_0826");  // TODO: Shouldn't we handle these cases gracefully instead of raising a fatal error?
		return false;
	}
	...
	if (pChunkDesc->chunkVersion == CONTROLLER_CHUNK_DESC_0827::VERSION)
	{
		...
		auto pData = static_cast<tuk>(pChunkDesc->data);
		...
		const auto pDrxKey = reinterpret_cast<DrxKeyPQLog*>(pData);
		SwapEndian(pDrxKey, pCtrlChunk->numKeys, bSwapEndianness);
		...
		i32k startTime = (pCtrlChunk->numKeys > 0) ? pDrxKey[0].nTime : 0;
	...
	if (pChunkDesc->chunkVersion == CONTROLLER_CHUNK_DESC_0829::VERSION)
	{
		...
		auto pData = static_cast<tuk>(pChunkDesc->data); // TODO: make const

		const auto pCtrlChunk = reinterpret_cast<const CONTROLLER_CHUNK_DESC_0829*>(pData);
		pData += sizeof(CONTROLLER_CHUNK_DESC_0829);

		auto consumeData = [&pData, &pStorage, &pCtrlChunk](const size_t bytesCount) -> tuk
		{
			if (pStorage)
			{
			...
			tuk out = pStorage ? pStorage : pData;
	...
	{
		DrxFatalError("CONTROLLER_CHUNK_DESC_0831");
		return true;
	}
	...
	if (pChunkDesc->chunkVersion == CONTROLLER_CHUNK_DESC_0832::VERSION)
	{
		...
		if (pChunkDesc->bSwapEndian)
			DrxFatalError("%s: provided data is stored in a non-native endian format", __FUNCTION__);
		...
		auto consumeData = [&pData, &pStorage, &controllerChunk](size_t bytesCount) -> tuk
		{
			tuk out = nullptr;
			if (pStorage)
			{
...
u32 GlobalAnimationHeaderCAFStreamContent::FlagsSanityFilter(u32 flags)
{
	// sanity check
	u32 validFlags = (CA_ASSET_BIG_ENDIAN | CA_ASSET_CREATED | CA_ASSET_ONDEMAND | CA_ASSET_LOADED | CA_ASSET_ADDITIVE | CA_ASSET_CYCLE | CA_ASSET_NOT_FOUND);
	if (flags & ~validFlags)
	{
		DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "Badly exported animation-asset: flags: %08x %s", flags, m_pPath);
		flags &= validFlags;
	}
	return flags;
}

u32 GlobalAnimationHeaderCAF::GetTotalPosKeys() const
{
	u32 pos = 0;
...
u32 GlobalAnimationHeaderCAF::GetTotalRotKeys() const
{
	u32 rot = 0;
	for (u32 i = 0; i < m_nControllers; i++)
...
bool GlobalAnimationHeaderCAF::Export2HTR(tukk szAnimationName, tukk savePath, const CDefaultSkeleton* pDefaultSkeleton)
{
	...
	for (u32 j = 0; j < numJoints; j++)
	{
		...
		i16 parentID = pJoint->m_idxParent;
		if (parentID == -1)
	...
	u32 nFrames = u32(fFrames + 1.5f);                              //roundup
...
// replace ' ' with '_'
void ChangeBoneName(tukk name, char newName[])
{
	i32 lenName = strlen(name);
	memcpy(newName, name, lenName * sizeof(char));
	for (i32 i = 0; i < lenName; ++i)
	{
		if (newName[i] == ' ')
			newName[i] = '_';
	}
...
bool GlobalAnimationHeaderCAF::SaveHTR(tukk szAnimationName, tukk savePath, const std::vector<string>& jointNameArray, const std::vector<string>& jointParentArray, const std::vector<DynArray<QuatT>>& arrAnimation, const QuatT* parrDefJoints)
{
	...
	u32 numFrames = arrAnimation[0].size();
	u32 numBipedJoints = jointNameArray.size();
	...
	i32 nFrameRate = 30;                             // ivo are you reading framrate from Vicon? //Xiaomao: make sure if we need to fix it.
	...
	for (u32 v = 0; v < numBipedJoints; v++)
	{
		tukk pBonename = jointNameArray[v];

		ChangeBoneName(pBonename, newName);             //replace " " with "_"
		tukk pParentBonename = jointParentArray[v];
...
bool GlobalAnimationHeaderCAF::SaveICAF(tukk szAnimationName, tukk savePath, const std::vector<string>& arrJointNames, const std::vector<DynArray<QuatT>>& arrAnimation)
{
	const string saveName = string(savePath) + szAnimationName + string(".i_caf");

	I3DEngine* const p3DE = GetISystem()->GetI3DEngine();

	ChunkFile::IChunkFileWriter* const pWriter = p3DE->CreateChunkFileWriter(I3DEngine::eChunkFileFormat_0x745, gEnv->pDrxPak, saveName.c_str());
	if (!pWriter)
	{
		return false;                                         //fail
	}
	...
	{
		u32k numBipedJoints = arrAnimation.size();
		...
		//-------------------------------------
		for (u32 j = 0; j < numBipedJoints; j++)
		{
			CONTROLLER_CHUNK_DESC_0827 controller;
			controller.numKeys = u32(numKeys);
			...
			std::vector<DrxKeyPQLog> arrPQLogs;
...
size_t GlobalAnimationHeaderCAF::SizeOfCAF(const bool bForceControllerCalcu) const
{
	...
	if (m_FilePathDBACRC32 && m_FilePathDBACRC32 != -1)
	{
		...
		bool InMem = g_AnimationUpr.IsDatabaseInMemory(m_FilePathDBACRC32);
...
void GlobalAnimationHeaderCAF::GetMemoryUsage(IDrxSizer* pSizer) const
{
