...
CSkeletonPhysics::CSkeletonPhysics()
	...
	, m_location(IDENTITY)
	, m_pPrevCharHost(nullptr)
	, m_physicsJointRootPosition(ZERO)
	, m_pPhysBuffer(nullptr)
	, m_bPhysicsSynchronize(false)
	, m_bPhysicsSynchronizeFromEntity(false)
	, m_bPhysBufferFilled(false)
	, m_pPhysAuxBuffer(nullptr)
	, m_bPhysicsSynchronizeAux(false)
	, m_pPhysImpactBuffer(nullptr)
	, m_vOffset(ZERO)
	, m_nSpineBones(0)
	, m_nAuxPhys(0)
	, m_iSurfaceIdx(0)
	...
	, m_velPivot(ZERO)
	, m_nPhysJoints(0)
	, m_pPhysUpdateValidator(nullptr)
	, m_pInstance(nullptr)
...
i16                      CSkeletonPhysics::GetJointIDByName(tukk szJointName) const
{
...
void CSkeletonPhysics::InitPhysicsSkeleton()
{
	...
	u32 numJoints = rDefaultSkeleton.m_arrModelJoints.size();
...
void CSkeletonPhysics::SetPhysEntOnJoint(i32 nId, IPhysicalEntity* pPhysEnt)
{
...
i32 CSkeletonPhysics::GetPhysIdOnJoint(i32 nId) const
{
	if (nId < 0 || nId >= (i32)GetPoseDataDefault().GetJointCount())
	{
		assert(0);
...
void ResetClothAttachment(const aux_phys_data& auxPhys, CAttachmentUpr& AttMan)
{
	if (!auxPhys.nBones)
...
void CSkeletonPhysics::DestroyPhysics()
{
	...
	if (m_ppBonePhysics)
	{
		for (i32 i = 0; i < (i32)GetPoseDataDefault().GetJointCount(); i++)
		{
	...
	for (i32 i = 0; i < m_nAuxPhys; i++)
	{
		ResetClothAttachment(m_auxPhys[i], m_pInstance->m_AttachmentUpr);
...
i32 CSkeletonPhysics::getBonePhysParentOrSelfIndex(i32 iBoneIndex, i32 nLod) const
{
	if (iBoneIndex >= m_pInstance->m_pDefaultSkeleton->m_arrModelJoints.size())
		return -1;
	i32 iNextBoneIndex;
...
const CDefaultSkeleton::SJoint* CSkeletonPhysics::GetModelJointPointer(i32 nBone) const
{
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
	return &rDefaultSkeleton.m_arrModelJoints[nBone];
}

CDefaultSkeleton::SJoint* CSkeletonPhysics::GetModelJointPointer(i32 nBone)
{
...
i32 CSkeletonPhysics::getBonePhysChildIndex(i32 iBoneIndex, i32 nLod) const
{
	...
	{
		i32 nResult = getBonePhysChildIndex(nFirstChild + nChild, nLod);
		if (nResult >= 0)
			return nResult;
	}
	return -1;
...
i32 CSkeletonPhysics::GetPhysRoot() const 
{
	i32 i, n = m_pInstance->m_pDefaultSkeleton->GetJointCount();
	for(i = 0; i < n && !GetModelJointPointer(i)->m_PhysInfo.pPhysGeom; i++);
	return i < n ? i : -1;
}

i32 CSkeletonPhysics::GetBoneSurfaceTypeId(i32 nBoneIndex, i32 nLod) const
{
...
i32 CSkeletonPhysics::TranslatePartIdToDeadBody(i32 partid)
{
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
	u32 numJoints = rDefaultSkeleton.GetJointCount();
	if ((u32)partid >= (u32)numJoints)
		return -1;

	i32 nLod = GetPhysicsLod();
  ...
  const Matrix34& mtxloc)
{
	...
	if (rDefaultSkeleton.m_ObjectType == CGA)
	{
		...
		u32 numJoints = m_arrCGAJoints->size();
		assert(numJoints);
		...
		//	u32 numJoints = m_arrJoints.size();
		//	assert(numJoints);
		for (i = 0; i < numJoints; i++)
		{
			CCGAJoint* joint = &(*m_arrCGAJoints)[i];
			...
			{
				params.density = 0;
				for (; *pstrMass && !isdigit((u8)(*pstrMass)); pstrMass++)
					;
			...
			params.idbody = i;
			if (geom)
				joint->m_qqqhasPhysics = rDefaultSkeleton.m_arrModelJoints[i].m_NodeID = m_pCharPhysics->AddGeometry(geom, &params, partid0 + i);
			if (geomProxy)
			{
		...
		i32 j, numSlots = (rDefaultSkeleton.m_pCGA_Object) ? rDefaultSkeleton.m_pCGA_Object->GetSubObjectCount() : 0;
		IStatObj::SSubObject* pSubObj;
		for (i = 0; i < numJoints; i++)
			if ((*m_arrCGAJoints)[i].m_CGAObjectInstance && (*m_arrCGAJoints)[i].m_CGAObjectInstance->GetPhysGeom())
				for (j = 0; j < min(numSlots, (i32)numJoints); j++)
		...
		if (rDefaultSkeleton.m_pCGA_Object)
		{
			rDefaultSkeleton.m_pCGA_Object->PhysicalizeSubobjects(m_pCharPhysics, &mtxloc, mass, density, partid0,
			                                                      strided_pointer<i32>(&(*m_arrCGAJoints)[0].m_qqqhasPhysics, sizeof((*m_arrCGAJoints)[0])));
		}
	...
	else
	{
		...
		i32k nJointCount = (i32)rDefaultSkeleton.GetJointCount();
		...
		for (i = 0; i < nJointCount; i++)
		{
			if (GetModelJointPointer(i)->m_PhysInfo.pPhysGeom)
			{
				...
				pgp->mass *= k;
				rDefaultSkeleton.m_arrModelJoints[i].m_NodeID = pent->AddGeometry(GetModelJointPointer(i)->m_PhysInfo.pPhysGeom, pgp, partid0 + i);
				if (GetModelJointPointer(i)->m_PhysInfo.pPhysGeom->pForeignData)
				{
					pgp->mass = 0.0f;
					pgp->flags = geom_colltype6;
		...
		if (pentype == PE_ARTICULATED)
		{
			...
			for (i = 0; i < nJointCount; i++)
			{
				if (GetModelJointPointer(i)->m_PhysInfo.pPhysGeom)
				{
					...
					{
						for (j = 0; j < nJointCount && pj.nSelfCollidingParts < DRX_ARRAY_COUNT(iParts); j++)
						{
					...
					{
						u32 numJoints = rDefaultSkeleton.GetJointCount();
						for (j = 0; j < (i32)numJoints && pj.nSelfCollidingParts < DRX_ARRAY_COUNT(iParts); j++)
						{
							const CDefaultSkeleton::SJoint& modelJoint = *GetModelJointPointer(j);
							if (modelJoint.m_PhysInfo.pPhysGeom &&
	...
	for (i32 i = m_pInstance->m_AttachmentUpr.GetAttachmentCount() - 1; i >= 0; i--)
		m_pInstance->m_AttachmentUpr.PhysicalizeAttachment(i, nLod, pent, offset);
}

IPhysicalEntity* CSkeletonPhysics::GetCharacterPhysics(tukk pRootBoneName) const
{
	if (!pRootBoneName)
...
IPhysicalEntity* CSkeletonPhysics::GetCharacterPhysics(i32 iAuxPhys) const
{
	if (iAuxPhys < 0)
		return m_pCharPhysics;
...
void CSkeletonPhysics::DestroyCharacterPhysics(i32 iMode)
{
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
  ...
  const Matrix34& mtxloc)
{
	...
	if (iRoot >= 0)
	{
		...
		BuildPhysicalEntity(m_pCharPhysics, mass, surface_idx, stiffness_scale, 0, -1, mtxloc);
	...
	/*IAttachmentObject* pAttachment;
	   for(i32 i=m_AttachmentUpr.GetAttachmentCount()-1; i>=0; i--)
	   if ((pAttachment = m_AttachmentUpr.GetInterfaceByIndex(i)->GetIAttachmentObject()) && pAttachment->GetICharacterInstance())
	   pAttachment->GetICharacterInstance()->CreateCharacterPhysics(m_pCharPhysics, mass,surface_idx, stiffness_scale, nLod);*/

	return m_pCharPhysics;
}

i32 CSkeletonPhysics::CreateAuxilaryPhysics(IPhysicalEntity* pHost, const Matrix34& mtx, i32 nLod)
{
	return CreateAuxilaryPhysics(pHost, mtx, mtx.GetColumn(0).len(), m_vOffset, nLod);
}

void CSkeletonPhysics::SetJointPhysInfo(u32 iJoint, const DrxBonePhysics& pi, i32 nLod)
{
	i32 i;
	...
	memcpy(&m_extraPhysInfo[i].info, &pi, sizeof(DrxBonePhysics));
}
const DrxBonePhysics& CSkeletonPhysics::GetJointPhysInfo(u32 iJoint, i32 nLod) const
{
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
	const DrxBonePhysics& piModel = rDefaultSkeleton.m_arrModelJoints[iJoint].m_PhysInfo;
	for (i32 i = m_extraPhysInfo.size() - 1; i >= 0; i--)
		if (m_extraPhysInfo[i].iJoint == iJoint && m_extraPhysInfo[i].nLod == nLod)
		{
			*(i32*)(m_extraPhysInfo[i].info.spring_angle + 1) = *(i32*)(piModel.spring_angle + 1);
			return m_extraPhysInfo[i].info;
		}
	return piModel;
}

DynArray<SJointProperty> CSkeletonPhysics::GetJointPhysProperties_ROPE(u32 jointIndex, i32 nLod) const
{
	CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
	if (jointIndex >= (u32)rDefaultSkeleton.m_arrModelJoints.size())
	{
		DrxBonePhysics bp;
		memset(&bp, 0, sizeof(bp));
		return rDefaultSkeleton.GetPhysInfoProperties_ROPE(bp, -(i32)jointIndex - 1);
	}

	tukk pJointName = rDefaultSkeleton.m_arrModelJoints[jointIndex].m_strJointName.c_str();
	if (strnicmp(pJointName, "rope", 4) == 0)
	{
		const CDefaultSkeleton::SJoint* pParentJoint = rDefaultSkeleton.GetParent(jointIndex);
		tukk pParentJointName = pParentJoint ? pParentJoint->m_strJointName.c_str() : 0;
		if (pParentJointName == 0 || (strnicmp(pParentJointName, "rope", 4)))
		{
			tukk ptr;
			i32 nRopeOrGrid = 0;
			ptrdiff_t len = (ptr = strchr(pJointName, ' ')) ? ptr - pJointName : ((ptr = strchr(pJointName, '_')) ? ptr - pJointName : strlen(pJointName));
			i32 numJoints = rDefaultSkeleton.m_arrModelJoints.size();
			for (i32 i = numJoints - 1; i >= 0; i--)
			{
				if (i == jointIndex)
					continue;
				tukk pJoint1Name = rDefaultSkeleton.m_arrModelJoints[i].m_strJointName.c_str();
				if (strncmp(pJoint1Name, pJointName, len) == 0)
				{
					const CDefaultSkeleton::SJoint* pParent1Joint = rDefaultSkeleton.GetParent(i);
					tukk pParentJoint1Name = pParent1Joint ? pParent1Joint->m_strJointName.c_str() : 0;
					if (pParentJoint1Name == 0 || strnicmp(pParentJoint1Name, "rope", 4))
					{
						nRopeOrGrid++;
	...
	const DrxBonePhysics& pi = GetJointPhysInfo(jointIndex, nLod);
...
bool CSkeletonPhysics::SetJointPhysProperties_ROPE(u32 jointIndex, i32 nLod, const DynArray<SJointProperty>& props)
{
	DrxBonePhysics bonePhysics;
...
void ParsePhysInfoProps(const DrxBonePhysics& physInfo, i32 nLod, pe_params_rope& pr, pe_simulation_params& simp, pe_params_flags& pf, const Quat& qrel)
{
...
void ParsePhysInfoPropsCloth(const DrxBonePhysics& physInfo, i32 nLod, pe_params_softbody& psb, pe_simulation_params& simp)
{
...
i32 CSkeletonPhysics::CreateAuxilaryPhysics(IPhysicalEntity* pHost, const Matrix34& mtx, float scale, Vec3 offset, i32 nLod)
//float *pForcedRopeLen,i32 *piForcedRopeIdx,i32 nForcedRopes)
{
	if (m_pSkeletonPose->m_bSetDefaultPoseExecute)
	{
		m_pSkeletonPose->SetDefaultPoseExecute(false);
	...
	for (i = 0; i < nJointCount; i++)
	{
		...
		tukk szBoneName = pBoneInfo->m_strJointName.c_str();

		if (!strnicmp(szBoneName, "rope", 4) && pBoneInfo->m_numChildren > 0 || &GetJointPhysInfo(i, nLod) != &m_pInstance->m_pDefaultSkeleton->m_arrModelJoints[i].m_PhysInfo)
		{
			if ((pspace = strchr(szBoneName, ' ')) || (pspace = strchr(szBoneName, '_')))
				nchars = (i32)(pspace - szBoneName);
			else
	...
	// if we have ropes but no physics proxies, then consider physics was relinquished (fix for RSE-569)
	...
	for (j = 0; j < m_nAuxPhys; ++j)
	{
		...
		pe_params_rope pr;
		i32 iLastBone = 0, iFirstBone = 0;
		i32 bCloth = 0, nStrands = 0, iSavedRoot, i1, imin;
		pr.nSegments = 0;
		bool bAutoRope = strnicmp(m_auxPhys[j].strName, "rope", 4) != 0;
		i32 idxAttachment = -1;
		u32 numJoints = rDefaultSkeleton.GetJointCount();
		if (bAutoRope)
		{
			...
			if (!(GetJointPhysInfo(m_auxPhys[j].iBoneTiedTo[0], nLod).flags & joint_isolated_accelerations))
				for (i = iLastBone = m_auxPhys[j].iBoneTiedTo[0]; GetModelJointPointer(i)->m_numChildren > 0; i = GetModelJointChildIndex(i, 0), pr.nSegments++)
					;
			else
			{
				for (i = m_pInstance->m_AttachmentUpr.GetAttachmentCount() - 1, iFirstBone = m_auxPhys[j].iBoneTiedTo[0]; i >= 0; i--)
					if (m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(i)->GetJointID() == iFirstBone)
						if (IAttachmentObject* pObj = m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(i)->GetIAttachmentObject())
							if (pObj && pObj->GetIStatObj() && (idxAttachment < 0 || strstr(m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(i)->GetName(), "cloth")))
							{
								m_auxPhys[j].iBoneTiedTo[1] = m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(i)->GetNameCRC();
								bCloth = 1;
								idxAttachment = i;
								k = 0;
							}
		...
		for (i = k = 0; i < (i32)numJoints; )
			if ((!strncmp(m_auxPhys[j].strName, GetModelJointPointer(i)->m_strJointName.c_str(), m_auxPhys[j].nChars) || bAutoRope && i == iLastBone) && GetModelJointPointer(i)->m_numChildren > 0)
			{
				...
				{
					for (i1 = 0, imin = i; i1 < (i32)numJoints; i1++)
				...
				pr.pPoints[k + 1] = GetPoseData().GetJointAbsolute(GetModelJointChildIndex(i, 0)).t * scale + offset;
				...
				if (bCloth)
				{
					...
					{
						for (i = iSavedRoot, imin = -1; i < (i32)numJoints; i++)
						{
		...
		{
cloth_aux:
			u16 * pIdx = new u16[k * 6];
			i32 nTris = 0, i2, i3, i0, idx;
			...
			for (i = 0; i < k; i++)
				pr.pPoints[i] -= sp.pos;
			IGeometry* pMesh = g_pIPhysicalWorld->GetGeomUpr()->CreateMesh(pr.pPoints, pIdx, 0, 0, nTris, mesh_SingleBB);
			delete[] pIdx;
			// get defSurfaceIdx from udp mat?
			phys_geometry* pgeom = pMesh ? g_pIPhysicalWorld->GetGeomUpr()->RegisterGeometry(pMesh) : 0;
			if (pgeom)
				pgeom->nRefCount = 0;
			g_pIPhysicalWorld->DestroyPhysicalEntity(m_auxPhys[j].pPhysEnt);
			pe_params_pos pp;
			...
			if (idxAttachment >= 0)
			{
				IAttachment* pAtt = m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(idxAttachment);
				if (IStatObj* pSrcObj = pAtt->GetIAttachmentObject()->GetIStatObj()->GetCloneSourceObject())
					((CCGFAttachment*)pAtt->GetIAttachmentObject())->pObj = pSrcObj;
				m_pInstance->m_AttachmentUpr.DephysicalizeAttachment(idxAttachment, pHost);
				m_pInstance->m_AttachmentUpr.PhysicalizeAttachment(idxAttachment, m_auxPhys[j].pPhysEnt, nLod);
			}
			...
			if (idxAttachment >= 0)
			{
				IStatObj* pStatObj = m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(idxAttachment)->GetIAttachmentObject()->GetIStatObj();
				...
				if (IRenderMesh* pRM = pStatObj->GetRenderMesh())
				{
					...
					if ((pColors.data = (ColorB*)pRM->GetColorPtr(pColors.iStride, FSL_READ)))
					{
						for (aap.nPoints = 0, i = 0; i < pRM->GetVerticesCount(); aap.nPoints += iszero(pColors[i++].g))
							;
						aap.piVtx = new i32[aap.nPoints + 1];
			...
			i32 idxParent = m_auxPhys[j].iBoneTiedTo[0] >= 0 ? getBonePhysParentIndex(m_auxPhys[j].iBoneTiedTo[0], nLod) : -1;
			if (idxParent >= 0)
			{
		...
		i32 iOtherBone = -1;
		drx_sprintf(boneAttachName, "attach_%.*s", m_auxPhys[j].nChars, m_auxPhys[j].strName);
		...
		const DrxBonePhysics& physInfo = GetJointPhysInfo(iFirstBone, nLod);
		if (!(physInfo.flags & joint_isolated_accelerations))
		{
			...
			{
				drx_sprintf(boneAttachName, "rope_stiffness_controller_%d", FtoI(physInfo.framemtx[0][1] - 1.0f));
		...
		pr.surface_idx = *(i32*)(physInfo.spring_angle + 1);
	...
	/* IZF temp
	   ...
	   for(i32 ic=m_pInstance->m_AttachmentUpr.GetAttachmentCount()-1; ic>=0; ic--)
	   {
	    if (m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(ic)->GetType()==CA_BONE && (pAttachment = m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(ic)->GetIAttachmentObject()) && (pChar=pAttachment->GetICharacterInstance()))
	    {
	      ...
	      for(i32 j0=0; pRope=pChar->GetISkeletonPose()->GetCharacterPhysics(j0); j0++)
	      {
	        pe_params_rope pr;
	        pr.pEntTiedTo[0] = pHost;
	        pr.idPartTiedTo[0] = getBonePhysParentOrSelfIndex(m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(ic)->GetBoneID());
	        pRope->SetParams(&pr);
	      }
	    }
	   }*/
...
i32 CSkeletonPhysics::FillRopeLenArray(float* len, i32 i0, i32 sz)
{
	i32 i, i0org = i0;
	pe_params_rope pr;
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
	...
	{
		i32 status = m_auxPhys[i].pPhysEnt->GetParams(&pr);
		if (status == 0)
		{
	...
	/* IZF: Refactor
	   IAttachmentObject* pAttachment;
	   ICharacterInstance *pChar;
	   for(i=m_pInstance->m_AttachmentUpr.GetAttachmentCount()-1; i>=0; i--)
	    if (m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(i)->GetType()==CA_BONE &&
	      (pAttachment = m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(i)->GetIAttachmentObject()) && (pChar=pAttachment->GetICharacterInstance()))
...
void CSkeletonPhysics::InitializeAnimToPhysIndexArray()
{
	...
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
	u32 jointCount = rDefaultSkeleton.GetJointCount();
	m_nPhysJoints = 0;
...
void CSkeletonPhysics::SynchronizeWithPhysicalEntity(Skeleton::CPoseData& poseData, IPhysicalEntity* pent, const Vec3& posMaster, const Quat& qMaster, QuatT offset, i32 iDir)
{
...
// IZF: We use this for calls outside DinrusXAnimation, which is bad since this
...
void CSkeletonPhysics::CreateRagdollDefaultPose(Skeleton::CPoseData& poseData)
{
	...
	u32 numJoints = poseData.GetJointCount();
	...
	{
		i32 pidx = parrModelJoints[i].m_idxParent;
		if (pidx >= 0)
		{
			i32 ParentIdx = parrModelJoints[i].m_idxParent;
...
IPhysicalEntity* CSkeletonPhysics::RelinquishCharacterPhysics(const Matrix34& mtx, float stiffness, bool bCopyJointVelocities, const Vec3& velHost)
{
	...
	i32 nLod = 1;
	i32 iRoot = GetPhysRoot(), numJoints = rDefaultSkeleton.GetJointCount();
	if (iRoot < 0)
		return nullptr;
	...
	res->SetParams(&sp);
	m_vOffset.zero();
	BuildPhysicalEntity(res, m_fMass, m_iSurfaceIdx, stiffness, nLod, -1, Matrix34(Vec3(m_fScale), Quat(IDENTITY), Vec3(ZERO)));
	...
	i32 bSkelQueued = res->SetParams(&pp) - 1;
	...
	for (i32 i = 0; i < m_nAuxPhys; i++)
	{
		...
		{
			const DrxBonePhysics& physInfo = GetJointPhysInfo(m_auxPhys[i].nBones ? m_auxPhys[i].pauxBoneInfo[0].iBone : m_auxPhys[i].iBoneTiedTo[0], 1);
			...
			else
			{
				...
				if (m_auxPhys[i].bTied1)
				{
					...
					i32 iBone = m_auxPhys[i].pauxBoneInfo[m_auxPhys[i].nBones - 1].iBone;
					iBone = GetModelJointPointer(iBone)->m_numChildren > 0 ? GetModelJointChildIndex(iBone, 0) : pr.idPartTiedTo[1];
	...
	{
		if (m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(i)->GetType() == CA_BONE
		    && (pAttachment = m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(i)->GetIAttachmentObject())
		    && (pChar = pAttachment->GetICharacterInstance()))
		{
			for (i32 j = 0; pRope = pChar->GetISkeletonPose()->GetCharacterPhysics(j); j++)
			{
				pr.pEntTiedTo[0] = res;
				pr.idPartTiedTo[0] = getBonePhysParentOrSelfIndex(m_pInstance->m_AttachmentUpr.GetInterfaceByIndex(i)->GetJointID());
				pRope->SetParams(&pr);
				pRope->SetParams(&poe);
			}
		}
...
bool CSkeletonPhysics::AddImpact(i32k partid, Vec3 point, Vec3 impact)
{
	if (m_pCharPhysics)
	{
		const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
		...
		if (!Console::GetInst().ca_physicsProcessImpact ||
		    rDefaultSkeleton.m_ObjectType == CGA || (u32)partid >= (u32)numJoints)
		{
		...
		tukk szImpactBoneName = GetModelJointPointer(partid)->m_strJointName.c_str();

		//for(i=0; i<4 && !(m_pIKEffectors[i] && m_pIKEffectors[i]->AffectsBone(partid)); i++);
		tukk ptr = strstr(szImpactBoneName, "Spine");
...
void CSkeletonPhysics::Physics_SynchronizeToAux(const Skeleton::CPoseData& poseData)
{
	...
	i32 nLod = m_bPhysicsRelinquished ? GetPhysicsLod() : 0;

	pe_action_target_vtx atv;
	Matrix34 mtx = Matrix34(m_location);
	i32 bRopesAwake = 0;
	for (i32 j = 0; j < m_nAuxPhys; ++j)
	{
		...
		if (m_auxPhys[j].bTied0)
		{
			i32 parentIndex = getBonePhysParentIndex(m_auxPhys[j].iBoneTiedTo[0], nLod);

			if (m_pPhysBuffer && m_bPhysBufferFilled)
				for (i32 i = getBoneParentIndex(m_auxPhys[j].iBoneTiedTo[0]); i != parentIndex; i = getBoneParentIndex(i))
		...
		{
			pr.bTargetPoseActive = 1;
			for (i32 i = 0; i < m_auxPhys[j].nBones; ++i)
			{
		...
		if (peType == PE_SOFT)
		{
			...
			if (m_auxPhys[j].pPhysEnt->GetStatus(&ssv))
			{
				...
				{
					if (IAttachment* pAtt = m_pInstance->m_AttachmentUpr.GetInterfaceByNameCRC(m_auxPhys[j].iBoneTiedTo[1]))
				...
				const DrxBonePhysics& physInfo = GetJointPhysInfo(m_auxPhys[j].iBoneTiedTo[0], nLod);
		...
		else
		{
			...
			if (m_auxPhys[j].iBoneStiffnessController >= 0)
			{
				...
				DrxBonePhysics& physInfo = GetModelJointPointer(m_auxPhys[j].iBoneTiedTo[0])->m_PhysInfo;
			...
			/*
			   ...
			   ++ivtx;
			        ...
			        if (i+1<m_auxPhys[j].nBones)
			        {
			          ...
			          for (i32 ivtx = i+1; ivtx < m_auxPhys[j].nBones; ++ivtx)
		...
		i32 iParent = m_auxPhys[j].pauxBoneInfo[m_auxPhys[j].nBones - 1].iBone;
		if (GetModelJointPointer(iParent)->m_numChildren > 0)
		{
			i32 childIndex = GetModelJointChildIndex(iParent, 0);
...
void CSkeletonPhysics::Physics_SynchronizeToImpact(float timeDelta)
{
	...
	i32 physicsLod = m_bPhysicsRelinquished ? GetPhysicsLod() : 0;
	...
	if (m_bPhysicsAwake)
	{
		...
		i32 jointCount = rDefaultSkeleton.GetJointCount();
		for (i32 i = 0; i < jointCount; ++i)
		{
			if (!m_pPhysImpactBuffer[i].bSet)
				continue;

			i32 parentIndex = getBonePhysParentIndex(i, physicsLod);
			if (parentIndex < 0)
				continue;
...
void CSkeletonPhysics::ProcessPhysics(Skeleton::CPoseData& poseData, float timeDelta)
{
	...
	if (m_pCharPhysics)
	{
		...
		i32k boneIndex = GetPhysRoot();
		if (boneIndex > -1)
		{
			Vec3 offs = -poseData.GetJointAbsolute(boneIndex).t;
			for (i32 i = m_pInstance->m_AttachmentUpr.GetAttachmentCount() - 1; i >= 0; i--)
				m_pInstance->m_AttachmentUpr.UpdatePhysAttachmentHideState(i, m_pCharPhysics, offs);
		}
...
void CSkeletonPhysics::SynchronizeWithPhysicsPost()
{
	...
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
	u32 objectType = rDefaultSkeleton.m_ObjectType;
	...
	if (objectType == CGA || pCharPhys && pCharPhys->GetType() == PE_ARTICULATED && pCharPhys->GetParams(&pf1) && pf1.flags & aef_recorded_physics)
	{
		...
		if (m_bFullSkeletonUpdate)
		{
			...
			i32 numAttachments = m_pInstance->m_AttachmentUpr.GetAttachmentCount() - 1;

			for (i32 i = numAttachments - 1; i >= 0; i--)
				m_pInstance->m_AttachmentUpr.UpdatePhysicalizedAttachment(i, pCharPhys, Offset);
...
void CSkeletonPhysics::SynchronizeWithPhysics(Skeleton::CPoseData& poseData)
{
	...
	if (m_bFullSkeletonUpdate)
	{
		if (gEnv->pPhysicalWorld->GetPhysVars()->lastTimeStep > 0 || m_pInstance->m_CharEditMode)
		{
	...
	m_pInstance->SetWasVisible(m_pInstance->GetWasVisible() && (((i32)m_bFullSkeletonUpdate) > 0));
	SetAuxParams(&pf);
}
...
SBatchUpdateValidator SBatchUpdateValidator::g_firstValidator;
 i32 SBatchUpdateValidator::g_lockList;
...
void CSkeletonPhysics::Physics_SynchronizeToEntity(IPhysicalEntity& physicalEntity, QuatT offset)
{
	...
	i32k physicsChildIndex = GetPhysRoot();
	...
	if (pent->GetType() == PE_ARTICULATED && physicsChildIndex > -1)
	{
		...
		{
			for (u32 i = 0; i < (unsigned)m_nPhysJoints; ++i)
	...
	i32 bStateChanged = 0;
	for (i32 i = m_pInstance->m_AttachmentUpr.GetAttachmentCount() - 1; i >= 0; i--)
		bStateChanged |= m_pInstance->m_AttachmentUpr.UpdatePhysicalizedAttachment(i, pent, offset);
	if (bStateChanged)
	{
		for (u32 i = 0; i < (unsigned)m_nAuxPhys; ++i)
		{
			if (!m_auxPhys[i].bTied0)
				continue;
...
void CSkeletonPhysics::FindSpineBones() const
{
	...
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
	u32 jointCount = rDefaultSkeleton.GetJointCount();
	for (u32 i = m_nSpineBones = 0; i < jointCount && m_nSpineBones < 3; ++i)
	{
...
i32 CSkeletonPhysics::GetFallingDir() const
{
	if (!m_pCharPhysics)
		return -1;
	...
	i32 status = m_pCharPhysics->GetStatus(&sd);

	Vec3 n(ZERO);
	for (u32 i = 0; i < m_nSpineBones; ++i)
		n += sd.v * pJointsAbsolute[m_iSpineBone[i]].q;

	Vec3 nAbs = n.abs();
	i32 i = idxmax3((f32*)&nAbs);
	i = i * 2 + isneg(n[i]);
	return i;
}
...
ILINE void InitializePoseAbsolute(const CDefaultSkeleton& skeleton, QuatT* pResult)
{
	...
	u32 jointCount = skeleton.GetPoseData().GetJointCount();
	for (u32 i = 0; i < jointCount; ++i)
...
ILINE void SamplePoseAbsolute(const CDefaultSkeleton& skeleton, const GlobalAnimationHeaderCAF& animation, f32 t, QuatT* pResult)
{
	...
	u32 jointCount = poseData.GetJointCount();
	for (u32 i = 0; i < jointCount; ++i)
	{
...
ILINE bool SamplePoseAbsolute(const CDefaultSkeleton& rDefaultSkeleton, i32 animationId, f32 t, QuatT* pResult)
{
	...
	GlobalAnimationHeaderCAF& animation = g_AnimationUpr.m_arrGlobalCAF[globalId];
...
{
	auto ctx = m_pInstance->GetProcessingContext();
	if (ctx && ctx->state == CharacterInstanceProcessing::SContext::EState::StartAnimationProcessed)
		return false;
	if (!m_bPhysicsRelinquished)
		return false;
	...
	if (m_arrPhysicsJoints.empty())
		InitPhysicsSkeleton();
	FindSpineBones();
	i32 rootJointIndex = m_iSpineBone[0];
	if (rootJointIndex < 0 || rootJointIndex >= i32(jointCount))
		return false;

	//

	QuatT joints[MAX_JOINT_AMOUNT];
	i32 animationId = m_pSkeletonAnim->GetAnimFromFIFO(0, 0).GetAnimationId();
