...
inline tukk EffectorTypeToName(EFacialEffectorType eftype)
{
	switch (eftype)
	{
	case EFE_TYPE_GROUP:
...
inline EFacialEffectorType NameToEffectorType(tukk name)
{
...
IFacialEffector* CFacialEffectorsLibrary::Find(tukk identStr)
{
	return Find(g_pCharacterUpr->GetFacialAnimation()->CreateIdentifierHandle(identStr));
}
...
IFacialEffector* CFacialEffectorsLibrary::CreateEffector(EFacialEffectorType nType, CFaceIdentifierHandle ident)
{
	...
	if (pEffector)
	{
		DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "Effector with name %s already exist", ident.GetString());
		return pEffector;
	}
	switch (nType)
	{
...
IFacialEffector* CFacialEffectorsLibrary::CreateEffector(EFacialEffectorType nType, tukk identStr)
{
	CFacialAnimation* pFaceAnim = g_pCharacterUpr->GetFacialAnimation();
	if (pFaceAnim)
	{
...
void CFacialEffectorsLibrary::RemoveEffector(IFacialEffector* pEff)
{
	...
	private:
		...
		{
			for (i32 i = 0; i < pEffector->GetSubEffectorCount(); )
			{
...
void CFacialEffectorsLibrary::ReplaceEffector(CFacialEffector* pOriginalEffector, CFacialEffector* pNewEffector)
{
	...
	private:
		void Recurse(IFacialEffector* pEffector)
		{
			for (i32 i = 0; i < pEffector->GetSubEffectorCount(); ++i)
			{
				if (pEffector->GetSubEffector(i) == pEffectorToRemove)
				{
...
//////////////////////////////////////////////////////////////////////////
CFacialEffector* CFacialEffectorsLibrary::GetEffectorFromIndex(i32 nIndex)
{
	if (nIndex >= 0 && nIndex < (i32)m_indexToEffector.size())
	{
...
//////////////////////////////////////////////////////////////////////////
void CFacialEffectorsLibrary::MergeLibrary(IFacialEffectorsLibrary* pMergeLibrary, const Functor1wRet<tukk , MergeCollisionAction>& collisionStrategy)
{
#ifdef FACE_STORE_ASSET_VALUES
	if (this == pMergeLibrary)
	{
		DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "Attempting to merge library with itself - aborting.");
		return;
	}

	typedef Functor1wRet<tukk , MergeCollisionAction> OverwriteConfirmationRequester;
	...
	private:
		...
		{
			for (i32 subEffectorIndex = 0; subEffectorIndex < pMergeEffector->GetSubEffectorCount(); ++subEffectorIndex)
			{
					...
					else
					{
						...
						if (itEffector == copiedEffectors.end())
						{
							...
							case EFE_TYPE_MATERIAL:
								break;
							case EFE_TYPE_ATTACHMENT:
								DrxFatalError("Attachment Effectors no longer supported in Facial Animation system.");
								break;
							default:
								assert(0);
								break;
				...
				for (i32 i = 0; i < pEffector->GetSubEffectorCount() && pNewSubEffector; ++i)
				{
		...
		void CopyController(IFacialEffCtrl* pCloneSubCtrl, IFacialEffCtrl* pOriginalSubCtrl)
		{
			...
			for (i32 key = 0; key < pOriginalSpline->GetKeyCount(); ++key)
			{
				...
				i32 cloneKeyIndex = pCloneSpline->InsertKey(pOriginalSpline->GetKeyTime(key), value);
...
#else
	DrxFatalError("Merging of Libraries is not supported on this platform");
#endif
}
...
void CFacialEffectorsLibrary::Serialize(XmlNodeRef& node, bool bLoading)
{
	if (bLoading)
	{
		...
		if (effectorsNode)
		{
			for (i32 i = 0; i < effectorsNode->getChildCount(); i++)
			{
		...
		if (controllersNode)
		{
			for (i32 i = 0; i < controllersNode->getChildCount(); i++)
			{
...
void CFacialEffectorsLibrary::SerializeEffectorSelf(CFacialEffector* pEffector, XmlNodeRef& node, bool bLoading)
{
	...
	else
	{
		...
		i32 nFlags = pEffector->GetFlags() & (~(EFE_FLAG_UI_MODIFIED | EFE_FLAG_UI_EXTENDED));
		if (nFlags != 0)
...
void CFacialEffectorsLibrary::SerializeEffectorSubCtrls(CFacialEffector* pEffector, XmlNodeRef& node, bool bLoading)
{
	...
	else
	{
		...
		i32 numSub = pEffector->GetSubEffectorCount();
		for (i32 i = 0; i < numSub; ++i)
		{
...
void CFacialEffectorsLibrary::GetMemoryUsage(IDrxSizer* pSizer) const
{
	...
	pSizer->AddObject(&m_indexToEffector, sizeof(m_indexToEffector) + m_indexToEffector.capacity() * sizeof(uk ));
}
