...
u32 CAttachmentVCLOTH::Immediate_AddBinding(IAttachmentObject* pIAttachmentObject, ISkin* pISkinRender, u32 nLoadingFlags)
{
	if (pIAttachmentObject == 0)
		return 0; //no attachment objects

	if (pISkinRender == 0)
		DrxFatalError("DinrusXAnimation: if you create the binding for a Skin-Attachment, then you have to pass the pointer to an ISkin as well");
	...
	{
		ReleaseRenderSkin();
		g_pCharacterUpr->RegisterInstanceVCloth(pCSkinRenderModel, this); //register this CAttachmentVCLOTH instance in CSkin.
		m_pRenderSkin = pCSkinRenderModel;            //increase the Ref-Counter
	}

	CCharInstance* pInstanceSkel = m_pAttachmentUpr->m_pSkelInstance;
	CDefaultSkeleton* pDefaultSkeleton = pInstanceSkel->m_pDefaultSkeleton;

	tukk pSkelFilePath = pDefaultSkeleton->GetModelFilePath();
	tukk pSkinFilePath = m_pRenderSkin->GetModelFilePath();

	u32 numJointsSkel = pDefaultSkeleton->m_arrModelJoints.size();
	u32 numJointsSkin = m_pRenderSkin->m_arrModelJoints.size();

	u32 NotMatchingNames = 0;
	m_arrRemapTable.resize(numJointsSkin, 0);
	for (u32 js = 0; js < numJointsSkin; js++)
	{
		i32k nID = pDefaultSkeleton->GetJointIDByCRC32(m_pRenderSkin->m_arrModelJoints[js].m_nJointCRC32Lower);
		if (nID >= 0)
	...
	if (NotMatchingNames)
	{
		...
		else
		{
			if (nLogWarnings)
			{
				DrxLogAlways("SKEL: %s", pDefaultSkeleton->GetModelFilePath());
				DrxLogAlways("SKIN: %s", m_pRenderSkin->GetModelFilePath());
				u32 numJointCount = pDefaultSkeleton->GetJointCount();
				for (u32 i = 0; i < numJointCount; i++)
				{
					tukk pJointName = pDefaultSkeleton->GetJointNameByID(i);
					DrxLogAlways("%03d JointName: %s", i, pJointName);
				}
			}
...
{
	u32 keySimMesh = CCrc32::ComputeLowercase(m_pSimSkin->GetModelFilePath());
	u32 keyRenderMesh = CCrc32::ComputeLowercase(m_pRenderSkin->GetModelFilePath());
...
u32 CAttachmentVCLOTH::AddSimBinding(const ISkin& pISkinRender, u32 nLoadingFlags)
{
	u32 nLogWarnings = (nLoadingFlags&CA_DisableLogWarnings) == 0;
	CSkin* pCSkinRenderModel = (CSkin*)&pISkinRender;
	...
	tukk pSkelFilePath = pDefaultSkeleton->GetModelFilePath();
	tukk pSkinFilePath = m_pSimSkin->GetModelFilePath();

	u32 numJointsSkel = pDefaultSkeleton->m_arrModelJoints.size();
	u32 numJointsSkin = m_pSimSkin->m_arrModelJoints.size();

	u32 NotMatchingNames = 0;
	m_arrSimRemapTable.resize(numJointsSkin, 0);
	for (u32 js = 0; js < numJointsSkin; js++)
	{
		i32k nID = pDefaultSkeleton->GetJointIDByCRC32(m_pSimSkin->m_arrModelJoints[js].m_nJointCRC32Lower);
		if (nID >= 0)
	...
	if (NotMatchingNames)
	{
		DrxFatalError("DinrusXAnimation: the simulation-attachment is supposed to have the same skeleton as the render-attachment");
		return 0; //critical! incompatible skeletons. cant create skin-attachment
	}
...
void CAttachmentVCLOTH::Immediate_ClearBinding(u32 nLoadingFlags)
{
...
void CAttachmentVCLOTH::RecreateDefaultSkeleton(CCharInstance* pInstanceSkel, u32 nLoadingFlags)
{
	CDefaultSkeleton* const pDefaultSkeleton = pInstanceSkel->m_pDefaultSkeleton;

	tukk pOriginalFilePath = pDefaultSkeleton->GetModelFilePath();
	...
	CDefaultSkeleton* pOrigDefaultSkeleton = g_pCharacterUpr->CheckIfModelSKELLoaded(pOriginalFilePath, nLoadingFlags);
	...
	for (auto && pAttachment : m_pAttachmentUpr->m_arrAttachments)
	{
		...
		tukk const pSkinFilename = pSkin->GetModelFilePath();
		mismatchingSkins.push_back(pSkinFilename);
		nExtendedCRC64 += CCrc32::ComputeLowercase(pSkinFilename);
	}

	CDefaultSkeleton* const pExtDefaultSkeleton = g_pCharacterUpr->CreateExtendedSkel(pInstanceSkel, pOrigDefaultSkeleton, nExtendedCRC64, mismatchingSkins, nLoadingFlags);
	if (pExtDefaultSkeleton)
	{
		pInstanceSkel->RuntimeInit(pExtDefaultSkeleton);
		m_pAttachmentUpr->m_TypeSortingRequired++;
		m_pAttachmentUpr->UpdateAllRemapTables();
	}
}
...
void CAttachmentVCLOTH::UpdateRemapTable()
{
	...
	for (u32 js = 0; js < numJointsSkin; js++)
	{
		...
		if (nID >= 0)
			...
			DrxFatalError("ModelError: data-corruption when executing UpdateRemapTable for SKEL (%s) and SKIN (%s) ", pSkelFilePath, pSkinFilePath); //a fail in this case is virtually impossible, because the SKINs are alread attached
	}
...
u32 CAttachmentVCLOTH::Immediate_SwapBinding(IAttachment* pNewAttachment)
{
	DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "[Cloth] CAttachmentVCLOTH::SwapBinding attempting to swap skin attachment bindings this is not supported");
	return 0;
}
...
float CAttachmentVCLOTH::GetExtent(EGeomForm eForm)
{
	if (m_pRenderSkin)
	{
		i32 nLOD = m_pRenderSkin->SelectNearestLoadedLOD(0);
...
void CAttachmentVCLOTH::GetRandomPoints(Array<PosNorm> points, CRndGen& seed, EGeomForm eForm) const
{
	...
	i32 nFrameID = gEnv->pRenderer->EF_GetSkinningPoolID();
	for (i32 n = 0; n < 3; n++)
	{
		i32 nList = (nFrameID - n) % 3;
...
i32 CAttachmentVCLOTH::GetGuid() const
{
	return m_pAttachmentUpr->m_pSkelInstance->m_pDefaultSkeleton->GetGuid();
}
...
void CAttachmentVCLOTH::ReleaseRenderSkin()
{
	if (m_pRenderSkin)
	{
		g_pCharacterUpr->UnregisterInstanceVCloth(m_pRenderSkin, this);
		ReleaseRenderRemapTablePair();
		m_pRenderSkin = 0;
	}
}
...
void CAttachmentVCLOTH::ReleaseSimSkin()
{
	if (m_pSimSkin)
	{
		g_pCharacterUpr->UnregisterInstanceVCloth(m_pSimSkin, this);
		ReleaseSimRemapTablePair();
		m_pSimSkin = 0;
	}
}
...
{
	i32k nFrameID = gEnv->pRenderer ? gEnv->pRenderer->EF_GetSkinningPoolID() : 0;
	i32k nList = nFrameID % 3;
	if (m_arrSkinningRendererData[nList].nFrameID == nFrameID && m_arrSkinningRendererData[nList].pSkinningData)
	{
		if (m_arrSkinningRendererData[nList].pSkinningData->pAsyncJobs)
			gEnv->pJobUpr->WaitForJob(*m_arrSkinningRendererData[nList].pSkinningData->pAsyncJobs);
	}
...
void CAttachmentVCLOTH::GetMemoryUsage(IDrxSizer *pSizer) const
{
...
_smart_ptr<IRenderMesh> CAttachmentVCLOTH::CreateVertexAnimationRenderMesh(uint lod, uint id)
{
	...
	u32 numModelMeshes = m_pRenderSkin->m_arrModelMeshes.size();
	...
	u32 success = pModelMesh->InitSWSkinBuffer();
	if (success == 0)
		return _smart_ptr<IRenderMesh>(NULL);
	;
...
#ifndef _RELEASE
	m_vertexAnimation.m_vertexAnimationStats.sCharInstanceName = m_pAttachmentUpr->m_pSkelInstance->GetFilePath();
	m_vertexAnimation.m_vertexAnimationStats.sAttachmentName = "";//m_Name; TODO fix
	m_vertexAnimation.m_vertexAnimationStats.pCharInstance = m_pAttachmentUpr->m_pSkelInstance;
#endif
...
void CAttachmentVCLOTH::DrawAttachment(SRendParams& RendParams, const SRenderingPassInfo &passInfo, const Matrix34& rWorldMat34, f32 fZoomFactor)
{
	...
	bool bNeedSWskinning = (m_pAttachmentUpr->m_pSkelInstance->m_CharEditMode&CA_CharacterTool); // in character tool always use software skinning
	if (!bNeedSWskinning)
	{
	...
	//-----------------------------------------------------------------------------
	i32 numLODs = m_pRenderSkin->m_arrModelMeshes.size();
	if (numLODs == 0)
		return;
	i32 nDesiredRenderLOD = RendParams.lodValue.LodA();
	...
	i32 nRenderLOD = m_pRenderSkin->SelectNearestLoadedLOD(nDesiredRenderLOD);  //we can render only loaded LODs
	...
	if (bNeedSWskinning && bNewFrame && nRenderLOD < m_clothPiece.GetNumLods())
	{
		...
		//  			|| g_pCharacterUpr->GetClothUpr()->QueryVisibility((uint64)m_pAttachmentUpr, center, passInfo.IsGeneralPass());
		if (m_clothPiece.PrepareCloth(m_pAttachmentUpr->m_pSkelInstance->m_SkeletonPose, rWorldMat34, visible, nRenderLOD))
		{
		...
		else
		{
			...
			DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "[Cloth] Character cloth buffer exceeds maximum (results in flickering between skinning & simulation). Increase buffer size with global 'ca_ClothMaxChars'.");
		}
	}
	...
	CCharInstance* pMaster = m_pAttachmentUpr->m_pSkelInstance;
	...
	// get skinning data
	i32k numClothLods = m_clothPiece.GetNumLods();
	...
	if (swSkin && pRenderMesh)
	{
		...
		IF(pRenderMesh && bNewFrame, 1)
		{
			...
			DRX_ASSERT(pRenderMesh->GetVerticesCount() == geometry.GetVertexCount());
			...
			if (pD->m_pSkinningData->pPreviousSkinningRenderData)
				gEnv->pJobUpr->WaitForJob(*pD->m_pSkinningData->pPreviousSkinningRenderData->pAsyncJobs);
			...
			pVertexAnimation->vertexData.pColors.data = (u32*)(&pGeneral[0].color);
			...
			if (pVertexAnimation->commandBufferLength)
			{
				CVertexCommandBufferAllocatorStatic commandBufferAllocator(
					(u8*)pD->m_pSkinningData->pCustomData + sizeof(SVertexAnimationJob),
			...
			else
			{
				...
				uk pUpdatedJobSkinningData = DrxInterlockedCompareExchangePointer(alias_cast<uk  *>(pD->m_pSkinningData->pMasterSkinningDataList), pD->m_pSkinningData, pCurrentJobSkinningData);
	...
	//	DrxFatalError("DinrusXAnimation: pMaster is zero");
	if (pRenderMesh)
	{
		...
		if (p_e_debug_draw && p_e_debug_draw->GetIVal() > 0)
			pModelMesh->DrawDebugInfo(pMaster->m_pDefaultSkeleton, nRenderLOD, RenderMat34, p_e_debug_draw->GetIVal(), pMaterial, pObj, RendParams, passInfo.IsGeneralPass(), (IRenderNode*)RendParams.pRenderNode, m_pAttachmentUpr->m_pSkelInstance->m_SkeletonPose.GetAABB(),passInfo);
#endif
...
//-----------------------------------------------------------------------------
void CAttachmentVCLOTH::TriggerMeshStreaming(u32 nDesiredRenderLOD, const SRenderingPassInfo &passInfo)
{
	if (!m_pRenderSkin)
		return;

	u32 numLODs = m_pRenderSkin->m_arrModelMeshes.size();
	if (numLODs == 0)
		return;
...
SSkinningData* CAttachmentVCLOTH::GetVertexTransformationData(bool bVertexAnimation, u8 nRenderLOD, const SRenderingPassInfo& passInfo)
{
	...
	u32 skinningQuatCount = m_arrRemapTable.size();
	u32 skinningQuatCountMax = pMaster->GetSkinningTransformationCount();
	u32 nNumBones = min(skinningQuatCount, skinningQuatCountMax);

	// get data to fill
	i32 nFrameID = gEnv->pRenderer->EF_GetSkinningPoolID();
	i32 nList = nFrameID % 3;
	i32 nPrevList = (nFrameID - 1) % 3;
	...
	if (m_arrSkinningRendererData[nPrevList].nFrameID == (nFrameID - 1) && m_arrSkinningRendererData[nPrevList].pSkinningData)
	{
		...
		if (pSkinningData->pPreviousSkinningRenderData->pAsyncJobs)
			gEnv->pJobUpr->WaitForJob(*pSkinningData->pPreviousSkinningRenderData->pAsyncJobs);
	}
	else
	{
...
#ifdef EDITOR_PCDEBUGCODE
void CAttachmentVCLOTH::DrawWireframeStatic(const Matrix34& m34, i32 nLOD, u32 color)
{
...
void CAttachmentVCLOTH::HideAttachment(u32 x)
{
...
void CAttachmentVCLOTH::HideInRecursion(u32 x)
{
...
void CAttachmentVCLOTH::HideInShadow(u32 x)
{
...
i32 CClothSimulator::SetParams(const SVClothParams& params, float* weights)
{
	m_config = params;
	m_config.weights = weights;

	for (i32 i = 0; i < m_nVtx && m_config.weights; i++)
	{
...
bool CClothSimulator::IsVisible()
{
	...
	if (m_pAttachmentUpr->m_pSkelInstance->GetISkeletonPose() && !m_pAttachmentUpr->m_pSkelInstance->IsCharacterVisible()) return false;
	return true;
}
...
void CClothSimulator::StartStep(float time_interval, const QuatT& location)
{
	...
	u32 numClothProxie = 0;
	u32 numProxies = m_pAttachmentUpr->GetProxyCount();
	for (u32 i = 0; i < numProxies; i++)
	{
		if (m_pAttachmentUpr->m_arrProxies[i].m_nPurpose == 1) // ==1: cloth
			numClothProxie++;
	}
	//	g_pAuxGeom->Draw2dLabel( 1,g_YLine, 1.3f, ColorF(0,1,0,1), false,"numProxies: %d",numClothProxie),g_YLine+=16.0f;

	u32 numCollidables = m_permCollidables.size();
	...
	for (u32 i = 0; i < numProxies; i++)
	{
		...
		const CProxy* pProxy = &m_pAttachmentUpr->m_arrProxies[i];
	...
	if (numProxies > 0)
	{
		...
		const Vec3& collidablePos = m_pAttachmentUpr->GetSkelInstance()->GetISkeletonPose()->GetAbsJointByID(0).t;
	...
	static ILINE bool PrimitiveRayCast(const f32 cr, const f32 ca, const SRay& ray, SIntersection& inters)
	{
		...
		if (ca == 0)
		{
			...
			if (d >= 0)
			{
				d = sqrt_tpl(d);
				Quotient t(-b - d, a);
				i32 bHit = isneg(fabs_tpl(t.x * 2 - t.y) - t.y);
				t.x += d * (bHit ^ 1) * 2;
		...
		if (ca)
		{
			...
			Quotient tmin(1, 1);
			i32 iFeature = -1; // TODO: remove or change
			float a = ray.dir.len2();
			for (i32 icap = 0; icap < 2; icap++)
			{
				...
				if (d >= 0)
				{
					d = sqrt_tpl(d);
					Quotient t(-b - d, a);
					i32 bHit = inrange(t.x * tmin.y, 0.f, t.y * tmin.x) & isneg(ca * t.y - fabs_tpl(axcdiff * t.y + axdir * t.x));
			...
			if (d >= 0)
			{
				d = sqrt_tpl(d);
				Quotient t(-b - d, a);
				i32 bHit = inrange(t.x * tmin.y, 0.f, t.y * tmin.x) & isneg(fabs_tpl(((ray.origin) * t.y + ray.dir * t.x) * axis) - ca * t.y);
				tmin.x += (t.x - tmin.x) * bHit;
				tmin.y += (t.y - tmin.y) * bHit;
...
void CClothSimulator::NearestNeighborDistanceConstraintsSolve()
{
	...
	for (auto it = m_nndcNotAttachedOrderedIdx.begin(); it != m_nndcNotAttachedOrderedIdx.end(); ++it)
	{
		...
		i32k i = *it;
		i32k& idx = m_particlesHot[i].nndcIdx;
		...
		if (nndcDistSqr > m_particlesHot[i].nndcDist * m_particlesHot[i].nndcDist * allowedExtensionSqr)
		{
			...
			i32 idxNextClosest = m_particlesHot[i].nndcNextParent;
...
{
	DRX_PROFILE_REGION(PROFILE_ANIMATION, "CClothSimulator::PositionsProjectToProxySurface");

	std::vector<SCollidable>& collidables = m_permCollidables;
	i32 colliderId[2];     // special handling for collision with two colliders at the same time, thus store id
	...
	for (i32 i = 0; i < m_nVtx; i++)
	{
		...
		i32 nCollisionsDetected = 0; // number of detected collisions
		...
		if (instantResolve)
		{
			...
			case 2:
				...
				{
					Vector4 collisionResolvePosition(ZERO);
					for (i32 j = 0; j < nCollisionsDetected; j++)
					{
...
{
	Vec3 distV = gEnv->p3DEngine->GetRenderingCamera().GetPosition() - m_pAttachmentUpr->m_pSkelInstance->m_location.t; // distance vector to camera (animation pivot)
	distV -= m_pAttachmentUpr->m_pSkelInstance->GetAABB().GetCenter();                                                  // use center of actual position (determined by BB), not of pivot
...
bool CClothSimulator::CheckSSRatioLargerThan(float ssAxisSizePercThresh) const
{
	...
	i32 screenBounds[4];
	const auto aabb_MS = m_pAttachmentUpr->m_pSkelInstance->GetAABB();
	const auto& translation = m_pAttachmentUpr->m_pSkelInstance->m_location.t;
	const AABB aabb_WS(translation + aabb_MS.min, translation + aabb_MS.max);

	const auto& camera = gEnv->p3DEngine->GetRenderingCamera();
	i32k w = camera.GetViewSurfaceX();
	i32k h = camera.GetViewSurfaceZ();
...
bool CClothSimulator::CheckAnimationRewind()
{
	...
	float normalizedTime = m_pAttachmentUpr->m_pSkelInstance->GetISkeletonAnim()->GetAnimationNormalizedTime(&m_pAttachmentUpr->m_pSkelInstance->GetISkeletonAnim()->GetAnimFromFIFO(0, 0));
	i32 isAnimPlaying = m_pAttachmentUpr->m_pSkelInstance->m_SkeletonAnim.m_IsAnimPlaying;
...
{
	// DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "[Cloth] Animation rewind occured.");
	PositionsSetToSkinned();
...
i32 CClothSimulator::Step()
{
	...
	if (m_nVtx <= 0)
	{
		DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "[Cloth] No of vertices <= 0");
		return 1;
	}
	...
	if (IsSimulationEnabled())
	{
		...
		if (springDamping)
		{
			DRX_PROFILE_REGION(PROFILE_ANIMATION, "CClothSimulator::Step::Damping");
			for (i32 i = 0; i < m_nEdges; i++)
			{
		...
		DRX_PROFILE_REGION(PROFILE_ANIMATION, "CClothSimulator::Step::ConstraintsAndCollisions");

		// interpolate transformation of collisionProxies
		UpdateCollidablesLerp(stepTime01);

		// constraint solver
		for (i32 iter = 0; iter < m_config.numIterations; iter++)
		{
			...
			{
				DRX_PROFILE_REGION(PROFILE_ANIMATION, "CClothSimulator::Step::SolveEdges");
				if (stretchStiffness)
				{
					for (i32 i = 0; i < m_nEdges; i++)
					{
			...
			{
				DRX_PROFILE_REGION(PROFILE_ANIMATION, "CClothSimulator::Step::DampEdges");
...
void CClothSimulator::LaplaceFilterPositions(Vector4* positions, float intensity)
{
	...
	for (i32 i = 0; i < m_nEdges; i++)
	{
		i32 idx0 = m_links[i].i1;
		i32 idx1 = m_links[i].i2;
...
void CClothSimulator::DebugOutput(float stepTime01)
{
	...
	case 1:
	{
		...
		g_pAuxGeom->Draw2dLabel(10, 80 + offs, 2.3f, ColorF(0, 1, 0, 1), false, "No:%i", (i32)(m_timeInterval / m_config.timeStep + 0.999f));
	};
	break;
	case 2:
		m_dtPrev = m_dt / m_dtNormalize;
		DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "[Cloth] Force m_dtPrev = m_dt = %f", m_dt / m_dtNormalize);
		break;
	case 3:
		DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "[Cloth] GlobalLocatorPos: %f, %f, %f", m_location.t.x, m_location.t.y, m_location.t.z); // global position of character (locator)
		break;
	}
}
...
void CClothSimulator::DrawHelperInformation()
{
	...
	Vec3 offs = m_pAttachmentUpr->m_pSkelInstance->m_location.t;
	...
	if (sphereRadius > 0.0f)
	{
		for (i32 i = 0; i < m_nVtx; i++)
		{
			//ColorB colorDynamic((i32)(255 * m_particlesHot[i].alpha), (i32)(128 * m_particlesHot[i].alpha), (i32)(255 - 128 * m_particlesHot[i].alpha));
	...
	case 1: // draw closest attachment
		for (i32 i = 0; i < m_nVtx; i++)
		{
			i32k nndcIdx = m_particlesHot[i].nndcIdx;
			if (nndcIdx >= 0)
	...
	case 2: // draw closest attachment ordered
		for (i32 i = 0; i < m_nndcNotAttachedOrderedIdx.size(); i++)
		{
			i32k idx = m_nndcNotAttachedOrderedIdx[i];
			i32k nndcIdx = m_particlesHot[idx].nndcIdx;
	...
	case 2: // draw shear links
		for (i32 i = 0; i < m_shearLinks.size(); i++)
		{
	...
	case 3: // draw bend links
		for (i32 i = 0; i < m_bendLinks.size(); i++)
		{
	...
	// for (i32 i = 0; i < m_nContacts; i++) { pRendererAux->DrawSphere(SseToVec3(m_contacts[i].pt) + offs, sphereRadius, colorGreen); } break;
	...
	i32 noOfProxies = c.size();
	if ((m_config.debugPrint >= 100) && (noOfProxies > 0))
	{
		i32 proxyId = m_config.debugPrint - 100;
		...
		u32 numAllProxies = m_pAttachmentUpr->GetProxyCount();
		i32 numClothProxy = -1;
		CProxy* pProxy = nullptr;
		for (u32 i = 0; i < numAllProxies; i++)
		{
			if (numClothProxy == proxyId) break;                       // already found
			if (m_pAttachmentUpr->m_arrProxies[i].m_nPurpose == 1) // ==1: cloth
			{
				pProxy = const_cast<CProxy*>(&m_pAttachmentUpr->m_arrProxies[i]);
				numClothProxy++;
			}
		}
		if (pProxy)
		{
			...
			for (i32 i = 0; i < m_debugCollidableSubsteppingQuatT.size(); i++)
			{
...
void CClothSimulator::DampPositionBasedDynamics()
{
	...
	if (num == 0)
	{
		DrxLog("[Character cloth] All vertices are attached");
		return;
	}
	xcm *= 1.f / (float)num;
	...
	{
		// DrxLog("[Character cloth] Singular matrix"); // matrix sometimes not invertible - without big side-effects, so no message anymore
		return;
	}
...
bool CClothSimulator::GetMetaData(mesh_data* pMesh, CSkin* pSimSkin)
{
	// get loaded NNDC data
	{
		...
		std::vector<i32> const& loadedNndcNotAttachedOrderedIdx = pSimSkin->GetVClothData().m_nndcNotAttachedOrderedIdx;
...
void CClothSimulator::GenerateMetaData(mesh_data* pMesh, CSkin* pSimSkin, float* weights)
{
	...
	// get generated NNDC data
	{
		...
		std::vector<i32> const& preNndcNotAttachedOrderedIdx = pre.GetNndcNotAttachedOrderedIdx();
		{
			i32 i = 0;
...
bool CClothPiece::Initialize(const CAttachmentVCLOTH* pVClothAttachment)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_ERROR, "[Cloth] Skin render attachment '%s' has no model mesh.", pVClothAttachment->GetName());
		return false;
	}

	m_pVClothAttachment = (CAttachmentVCLOTH*)pVClothAttachment;
	m_simulator.SetAttachmentUpr(pVClothAttachment->m_pAttachmentUpr);
	...
	m_clothGeom = g_pCharacterUpr->LoadVClothGeometry(*m_pVClothAttachment, pRenderMeshes);
	if (!m_clothGeom)
		return false;
	...
	{
		// DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "[Cloth] Loaded VCloth from file...");
	...
	{
		DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "[Cloth] VCloth metadata is not stored in character file. Please regenerate skin with RessourceCompiler and cloth flag set. Metadata is generated on the fly (expensive at runtime).");
...
void CClothPiece::Dettach()
{
	...
	u32 flags = m_pVClothAttachment->GetFlags();
...
void CClothPiece::WaitForJob(bool bPrev)
{
	...
	if (m_pVClothAttachment->m_arrSkinningRendererData[nList].nFrameID == nFrameID && m_pVClothAttachment->m_arrSkinningRendererData[nList].pSkinningData)
	{
		...
		{
			gEnv->pJobUpr->WaitForJob(*m_pVClothAttachment->m_arrSkinningRendererData[nList].pSkinningData->pAsyncJobs);
		}
	}
}

bool CClothPiece::PrepareCloth(CSkeletonPose& skeletonPose, const Matrix34& worldMat, bool visible, i32 lod)
{
	DRX_PROFILE_FUNCTION(PROFILE_ANIMATION);
	...
	if (m_buffers != NULL)
	{
		DrxLog("[Character Cloth] the previous job is not done: %s - %s", m_pVClothAttachment->GetName(), skeletonPose.m_pInstance->GetFilePath());
		assert(false);
		return false;
	}
	if (m_poolIdx < 0)
	...
	// 		gEnv->pJobUpr->WaitForJob( *pSkinningData->pAsyncJobs ); // stall still the skinning related jobs have finished
...
void CClothPiece::UpdateSimulation(const DualQuat* pTransformations, const uint transformationCount)
{
	...
	if (!Console::GetInst().ca_ClothBypassSimulation && m_simulator.IsInitialized())
	{
		...
		for (i32 i = 0; i < arrDstPositions.size(); i++)
		{
#ifdef CLOTH_SSE
			Vector4 v;
		...
		else
		{
			...
			for (i32 i = 0; i < m_clothGeom->nUniqueVtx; i++)
			{
				Quaternion ssequat(m_charLocation.q);
...
void CClothPiece::SkinSimulationToRenderMesh(i32 lod, CVertexData& vertexData, const strided_pointer<const Vec3>& pVertexPositionsPrevious)
{
	DRX_PROFILE_FUNCTION(PROFILE_ANIMATION);
	if (m_clothGeom->skinMap[lod] == NULL || m_buffers == NULL)
		return;

	i32k nVtx = vertexData.GetVertexCount();
	...
	for (i32 i = 0; i < md->nVertices; i++)
		normals[i].zero();
	for (i32 i = 0; i < md->nTris; i++)
	{
		i32 base = i * 3;
		i32k idx1 = md->pIndices[base++];
		i32k idx2 = md->pIndices[base++];
		i32k idx3 = md->pIndices[base];
	...
	i32 nTris = m_clothGeom->numIndices[lod] / 3;
	...
	DRX_ALIGN(16) Vec4sf tangentBitangent[2];
#endif
	for (i32 i = 0; i < nVtx; i++)
	{
...
void SClothGeometry::AllocateBuffer()
{
	...
	u32 maxChars = Console::GetInst().ca_ClothMaxChars;
...
SBuffers* SClothGeometry::GetBufferPtr(i32 idx)
{
	u32 numSize = pool.size();
	if (idx >= numSize)
		return NULL;
...
i32 SClothGeometry::GetBuffers()
{
	WriteLock lock(poolLock);
	if (freePoolSlots.size())
	{
		i32 idx = freePoolSlots.back();
...
void SClothGeometry::ReleaseBuffers(i32 idx)
{
	WriteLock lock(poolLock);
...
void SClothGeometry::Cleanup()
{
	if (pPhysGeom)
	{
		g_pIPhysicalWorld->GetGeomUpr()->UnregisterGeometry(pPhysGeom);
		pPhysGeom = NULL;
	}
