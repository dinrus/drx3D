...
CLodValue CCharInstance::ComputeLod(i32 wantedLod, const SRenderingPassInfo& passInfo)
{
...
void CCharInstance::Render(const struct SRendParams& RendParams, const SRenderingPassInfo& passInfo)
{
	...
	u32 nFrameID = g_pCharacterUpr->m_nUpdateCounter;
	...
	m_AttachmentUpr.DrawMergedAttachments(attachmentRendParams, FinalMat, passInfo, fZoomFactor, m_fZoomDistanceSq);
	...
	m_AttachmentUpr.DrawAttachments(attachmentRendParams, FinalMat, passInfo, fZoomFactor, m_fZoomDistanceSq);

#ifndef _RELEASE
	// in-game debug rendering of characters attachments proxies 
	u32k uiProxies = (u32)Console::GetInst().ca_DebugAttachmentsProxies;
	m_AttachmentUpr.DrawProxies(uiProxies ? uiProxies : 0x80);
#endif
}
...
{
	i32 nList = (i32)CharacterUpr::GetRendererMainThreadId();

	if (GetSkinningTransformationCount())
		DrxFatalError("DinrusXAnimation: CGA should not have Dual-Quaternions for Skinning");

	SRendParams nodeRP = RendParams;

	Matrix34 orthoTm34 = RenderMat34;
	IMaterial* pMaterial = nodeRP.pMaterial;
	u32 numJoints = m_SkeletonPose.GetPoseData().GetJointCount();
	assert(numJoints > 0);
...
SSkinningData* CCharInstance::GetSkinningData(const SRenderingPassInfo& passInfo)
{
	...
	u32 numSkinningBones = GetSkinningTransformationCount() + pAttachmentUpr->GetExtraBonesCount();
