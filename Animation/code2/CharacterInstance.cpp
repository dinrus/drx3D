...
CCharInstance::CCharInstance(const string& strFileName, CDefaultSkeleton* pDefaultSkeleton) : m_skinningTransformationsCount(0), m_skinningTransformationsMovement(0)
{
	...
	g_pCharacterUpr->RegisterInstanceSkel(pDefaultSkeleton, this);
	...
	m_AttachmentUpr.m_pSkelInstance = this;
...
void CCharInstance::RuntimeInit(CDefaultSkeleton* pExtDefaultSkeleton)
{
	for (u32 i = 0; i < 3; i++)
	{
		if (arrSkinningRendererData[i].pSkinningData)
		{
			if (arrSkinningRendererData[i].pSkinningData->pPreviousSkinningRenderData->pAsyncJobs)
				gEnv->pJobUpr->WaitForJob(*arrSkinningRendererData[i].pSkinningData->pPreviousSkinningRenderData->pAsyncJobs);
		}
	}
	memset(arrSkinningRendererData, 0, sizeof(arrSkinningRendererData));
	...
	{
		g_pCharacterUpr->UnregisterInstanceSkel(m_pDefaultSkeleton, this);
		g_pCharacterUpr->RegisterInstanceSkel(pExtDefaultSkeleton, this);
		m_pDefaultSkeleton = pExtDefaultSkeleton;
	}

	u32 numJoints = m_pDefaultSkeleton->m_arrModelJoints.size();
...
CCharInstance::~CCharInstance()
{
	...
	m_AttachmentUpr.UpdateBindings();
	...
	tukk pFilePath = GetFilePath();
	g_pCharacterUpr->ReleaseCDF(pFilePath);
...
void CCharInstance::StartAnimationProcessing(const SAnimationProcessParams& params)
{
	...
	i32 numberOfChildren = m_AttachmentUpr.GenerateAttachedInstanceContexts();
...
void CCharInstance::CopyPoseFrom(const ICharacterInstance& rICharInstance)
{
	...
	DRX_ASSERT_TRACE(srcIPose, ("CopyPoseFrom: %s Copying from an invalid pose!", GetFilePath()));
	if (srcIPose)
	{
		DRX_ASSERT_TRACE(m_pDefaultSkeleton->GetJointCount() == rIDefaultSkeleton.GetJointCount(), ("CopyPoseFrom: %s Joint count mismatch %d vs %d", GetFilePath(), rIDefaultSkeleton.GetJointCount(), m_pDefaultSkeleton->GetJointCount()));
...
void CCharInstance::UpdatePhysicsCGA(Skeleton::CPoseData& poseData, float fScale, const QuatTS& rAnimLocationNext)
{
	...
	pe_status_pos sp;
	i32 numNodes = poseData.GetJointCount();
	assert(numNodes);

	i32 i, iLayer, iLast = -1;
	bool bSetVel = 0;
	u32 nType = m_SkeletonPose.GetCharacterPhysics()->GetType();
...
{
	i32 i, iParent, numNodes = m_SkeletonPose.GetPoseData().GetJointCount();
	QuatT qParent;
	...
	i32 idGlobal = pAnimationSet->GetGlobalIDByAnimID_Fast(anim.GetAnimationId());
	GlobalAnimationHeaderCAF& animHdr = g_AnimationUpr.m_arrGlobalCAF[idGlobal];
...
SMeshLodInfo CCharInstance::ComputeGeometricMean() const
{
	...
	i32k attachmentCount = m_AttachmentUpr.GetAttachmentCount();
	for (i32 i = 0; i < attachmentCount; ++i)
	{
		const IAttachment* const pIAttachment = m_AttachmentUpr.GetInterfaceByIndex(i);
	...
	if (GetObjectType() == CGA)
	{
		...
		{
			u32 numJoints = GetIDefaultSkeleton().GetJointCount();
...
phys_geometry* CCharInstance::GetPhysGeom(i32 nType) const
{
...
float CCharInstance::GetExtent(EGeomForm eForm)
{
	...
	extent.AddPart(m_AttachmentUpr.GetExtent(eForm));
...
void CCharInstance::GetRandomPoints(Array<PosNorm> points, CRndGen& seed, EGeomForm eForm) const
{
	...
	for (auto part : ext.RandomPartsAliasSum(points, seed))
	{
		...
		{
			// Choose attachment.
			m_AttachmentUpr.GetRandomPoints(part.aPoints, seed, eForm);
		}
		else
			part.aPoints.fill(ZERO);
	}
...
void CCharInstance::DrawWireframeStatic(const Matrix34& m34, i32 nLOD, u32 color)
{
	(void)nLOD; // TODO
...
{
	g_pCharacterUpr->StopAnimationsOnAllInstances(*GetIAnimationSet());
	g_pCharacterUpr->SyncAllAnimations();
	...
	//--------------------------------------------------------------------------
	u32 numAttachments = m_AttachmentUpr.m_arrAttachments.size();
	for (u32 i = 0; i < numAttachments; i++)
	{
		IAttachment* pIAttachment = m_AttachmentUpr.m_arrAttachments[i];
		...
		DrxLog("Reloading %s", paramFileName.c_str());
		m_pDefaultSkeleton->m_pAnimationSet->NotifyListenersAboutReloadStarted();
...
void CCharInstance::EnableFacialAnimation(bool bEnable)
{
	...
	for (i32 attachmentIndex = 0, end = i32(m_AttachmentUpr.m_arrAttachments.size()); attachmentIndex < end; ++attachmentIndex)
	{
		IAttachment* pIAttachment = m_AttachmentUpr.m_arrAttachments[attachmentIndex];
...
void CCharInstance::SkeletonPostProcess()
{
	...
	m_skeletonEffectUpr.Update(&m_SkeletonAnim, &m_SkeletonPose, rPhysLocation);
};

void CCharInstance::SkinningTransformationsComputation(SSkinningData* pSkinningData, CDefaultSkeleton* pDefaultSkeleton, i32 nRenderLod, const Skeleton::CPoseData* pPoseData, f32* pSkinningTransformationsMovement)
{
	DEFINE_PROFILER_FUNCTION();
...
#ifndef _DEBUG
	DrxPrefetch(&pJointsAbsoluteDefault[0]);
	DrxPrefetch(&absPose_PreFetchOnly[0]);
	DrxPrefetch(&pSkinningTransformations[0]);
	DrxPrefetch(&pJointsAbsoluteDefault[1]);
	DrxPrefetch(&absPose_PreFetchOnly[4]);
	DrxPrefetch(&pSkinningTransformations[4]);
#endif
	...
	{
#ifndef _DEBUG
		DrxPrefetch(&pJointsAbsoluteDefault[i + 1]);
		DrxPrefetch(&absPose_PreFetchOnly[i + 4]);
		DrxPrefetch(&pSkinningTransformations[i + 4]);
	...
	CAttachmentUpr* pAttachmentUpr = static_cast<CAttachmentUpr*>(GetIAttachmentUpr());
	u32k numJoints = pDefaultSkeleton->m_arrModelJoints.size();
	u32k extraBonesCount = pAttachmentUpr->GetExtraBonesCount();
	for (u32 i = 0; i < extraBonesCount; ++i)
	{
		if (IAttachment* pAttachment = pAttachmentUpr->m_extraBones[i])
		{
	...
	do
	{
		pSkinningJobList = *(const_cast<SSkinningData* *>(pSkinningData->pMasterSkinningDataList));
	}
	while (DrxInterlockedCompareExchangePointer(alias_cast<uk *>(pSkinningData->pMasterSkinningDataList), NULL, pSkinningJobList) != pSkinningJobList);
...
void CCharInstance::GetMemoryUsage(IDrxSizer* pSizer) const
{
	...
	{
		SIZER_COMPONENT_NAME(pSizer, "CAttachmentUpr");
		pSizer->AddObject(m_AttachmentUpr);
	}

	{
		SIZER_COMPONENT_NAME(pSizer, "skeletonEffectUpr");
		pSizer->AddObject(m_skeletonEffectUpr);
	}
...
CharacterInstanceProcessing::SContext* CCharInstance::GetProcessingContext()
{
	...
	{
		return &g_pCharacterUpr->GetContextSyncQueue().GetContext(m_processingContext);
	}
	return nullptr;
}
...
void CCharInstance::WaitForSkinningJob() const
{
	...
	if (arrSkinningRendererData[nList].nFrameID == nFrameID && pSkinningData && pSkinningData->pAsyncJobs)
		gEnv->pJobUpr->WaitForJob(*pSkinningData->pAsyncJobs);
}

void CCharInstance::SetupThroughParent(const CCharInstance* pParent)
{
	DRX_ASSERT(pParent != nullptr);
...
void CCharInstance::SetupThroughParams(const SAnimationProcessParams* pParams)
{
	...
	u32 nErrorCode = 0;
	u32 minValid = m_SkeletonPose.m_AABB.min.IsValid();
	if (minValid == 0)
		nErrorCode |= 0x8000;
	u32 maxValid = m_SkeletonPose.m_AABB.max.IsValid();
	if (maxValid == 0)
		nErrorCode |= 0x8000;
	...
	i32 nCurrentFrameID = g_pCharacterUpr->m_nUpdateCounter; // g_pIRenderer->GetFrameID(false);
	u32 dif = nCurrentFrameID - m_LastRenderedFrameID;
