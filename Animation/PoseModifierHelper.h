...
ILINE const QuatT& GetJointParentAbsoluteSafe(const CDefaultSkeleton& defaultSkeleton, Skeleton::CPoseData& poseData, const uint index)
{
	...
	i32k parentIndex = defaultSkeleton.GetJointParentIDByID(index);
...
ILINE void SetJointRelativeLocation(const CDefaultSkeleton& defaultSkeleton, Skeleton::CPoseData& poseData, const uint index, const QuatT& location)
{
	...
	i32k parentIndex = pModelJoints[index].m_idxParent;
...
ILINE void SetJointAbsolute(const SAnimationPoseModifierParams& params, u32 index, const Quat* pOrientation, const Vec3* pPosition)
{
	...
	i32 parentIndex = params.GetIDefaultSkeleton().GetJointParentIDByID(index);
	if (parentIndex > -1)
	{
