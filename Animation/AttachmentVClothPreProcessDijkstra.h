...
class GraphDijkstra
{
	...
	typedef std::pair<u32, T> tEdgeTo;

public:

	GraphDijkstra(
		u32 nNodes,                                                     //!< Maximum number of nodes in graph (nodeIds must be within this range).
		T weightInit = T(0),                                               //!< Initial weight, e.g. for distances start with 0.0.
		u32 targetNodeLinkId = std::numeric_limits<u32>::max() - 1,  //!< NodeId-value, which is set as neighbor id for target nodes.
		u32 islandNodeLinkId = std::numeric_limits<u32>::max()       //!< NodeId-value, which is set as neighbor id for island nodes (i.e., no connection to any target node exists).
	);

	void AddEdge(u32 nodeId0, u32 nodeId1, T weight);                //!< Add edge to graph with given weight.
	void FindShortestPaths(u32 targetNodeId);                           //!< Do dijkstra's algorithm with a single target
	void FindShortestPaths(std::vector<u32> const& listTargetNodes);    //!< Do dijkstra's algorithm with multiple targets

	bool IsTargetNode(u32 idx) const;  //!< True, if node is target node; false otherwise.
	bool IsIslandNode(u32 idx) const;  //!< True, if node has connetion to any target node; false otherwise (island).

	struct ResultPerNode
	{
		u32 nextNodeIdOnShortestPath;  //!< Neighbor node in shortest path direction.
		T weightTotal;                    //!< Weight to target node.
		T weightToNextNodeOnShortestPath; //!> Weight to closest neighbor on shortest path.

		ResultPerNode()
			: nextNodeIdOnShortestPath(std::numeric_limits<u32>::max())
			, weightToNextNodeOnShortestPath(std::numeric_limits<T>::max())
			, weightTotal(std::numeric_limits<T>::max())
		{}
		ResultPerNode(u32 nextNodeIdOnShortestPath, T weightToNextNodeOnShortestPath, T weightTotal)
	...
	ResultPerNode const& Results(u32 idx) const    { assert(idx < m_nNodes); return m_results[idx]; }

private:

	u32 m_nNodes;                             //!< Number of nodes.
	...
	u32 m_targetNodeLinkId;                   //!< Id which is set as neighbor id to target nodes.
	u32 m_islandNodeLinkId;                   //!< Id which is set as neighbor id to target nodes.
...
GraphDijkstra<T>::GraphDijkstra(
	u32 nNodes,
	T weightInit,
	u32 targetNodeLinkId,
	u32 islandNodeLinkId
	)
	: m_nNodes(nNodes)
...
template<class T>
void GraphDijkstra<T>::AddEdge(u32 nodeId0, u32 nodeId1, T weight)
{
...
template<class T>
void GraphDijkstra<T>::FindShortestPaths(std::vector<u32> const& listTargetNodes)
{
	...
	i32k nNodesOriginal = m_nNodes;
	i32k nNodesActual   = m_nNodes + 1;
	...
	i32k virtualTargetNodeId = GraphDijkstra<T>::m_nNodes - 1;
...
template<class T>
bool GraphDijkstra<T>::IsTargetNode(u32 idx) const
{
...
template<class T>
bool GraphDijkstra<T>::IsIslandNode(u32 idx) const
{
	assert(idx < GraphDijkstra<T>::m_nNodes);
...
{
	for (u32 i = 0; i < g.Results().size(); ++i)
	{
