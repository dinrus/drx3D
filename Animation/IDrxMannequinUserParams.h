...
#ifndef __I_DRX_MANNEQUIN_USER_PARAMS__H__
#define __I_DRX_MANNEQUIN_USER_PARAMS__H__
...
{
public:
	CMannequinUserParamId(const SControllerDef& controllerDef, u32k paramType)
	...
	u32 GetControllerDefCrc() const { return m_controllerDefPathCrc; }

private:
	u32 m_controllerDefPathCrc;
	u32 m_paramType;
};
...
public:
	...
	void ReloadAll(IAnimationDatabaseUpr& animationDatabaseUpr)
	{
		...
		for (TMannequinUserParams::iterator it = m_userParams.begin(); it != m_userParams.end(); ++it)
		{
			...
			u32k controllerDefCrc = mannequinUserParamId.GetControllerDefCrc();
			const SControllerDef* pControllerDef = animationDatabaseUpr.FindControllerDef(controllerDefCrc);
			DRX_ASSERT(pControllerDef != NULL);
	...
	{
		static u32 s_mannequinParamType = GetAutoIncParamTypeValue();
		...
		AddParams(mannequinUserParamId, pParams, eMPO_UprOwnership);
	...
	void RegisterParams(const SControllerDef& controllerDef, IMannequinUserParams* pParams)
	{
		...
		{
			DRX_ASSERT(pParams == pParamsOther);
			return;
		}
...
protected:
	...
	void AddParams(const CMannequinUserParamId& paramId, IMannequinUserParams* pParams, const EMannequinParamsOwnership ownership)
	{
		...
		if (ownership == eMPO_UprOwnership)
		{
...
#define MANNEQUIN_AUTO_CRC__DECLARE_ELEMENT(elem) u32 elem;
