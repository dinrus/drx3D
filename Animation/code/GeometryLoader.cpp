...
GeometryLoader::GeometryLoader(NodeConstView &&libraryGeometries, std::vector<VertexWeights> vertexWeights, const Matrix4 &correction) :
	...
	auto indexRawData = String::Split(meshData.GetPropertyWithBackup("polylist", "triangles")["p"].Get<STxt>(), ' ');
...
std::vector<Vector3f> GeometryLoader::GetPositions() const {
	auto positionsSource = meshData["vertices"]["input"]["@source"].Get<STxt>().substr(1);
	...
	auto positionsRawData = String::Split(positionsData.Get<STxt>(), ' ');
...
std::vector<Vector2f> GeometryLoader::GetUvs() const {
	auto uvsSource = meshData.GetPropertyWithBackup("polylist", "triangles")["input"].GetPropertyWithValue("@semantic", "TEXCOORD")["@source"].Get<STxt>().substr(1);
	auto uvsData = meshData["source"].GetPropertyWithValue("@id", uvsSource)["float_array"];
	auto uvsCount = uvsData["@count"].Get<uint32_t>();
	auto uvsRawData = String::Split(uvsData.Get<STxt>(), ' ');
...
std::vector<Vector3f> GeometryLoader::GetNormals() const {
	auto normalsSource = meshData.GetPropertyWithBackup("polylist", "triangles")["input"].GetPropertyWithValue("@semantic", "NORMAL")["@source"].Get<STxt>().substr(1);
	...
	auto normalsRawData = String::Split(normalsData.Get<STxt>(), ' ');
