...
void SkinLoader::LoadJointsList() {
	...
	auto jointDataIdNode = inputNode["input"].GetPropertyWithValue("@semantic", "JOINT")["@source"].Get<STxt>().substr(1);
	auto jointDataId = inputNode["input"].GetPropertyWithValue("@semantic", "JOINT")["@source"].Get<STxt>().substr(1);
	auto jointsNode = skinData["source"].GetPropertyWithValue("@id", jointDataId)["Name_array"];
	jointOrder = String::Split(jointsNode.Get<STxt>(), ' ');
}

std::vector<float> SkinLoader::LoadWeights() const {
	auto inputNode = skinData["vertex_weights"];
	auto weightsDataId = inputNode["input"].GetPropertyWithValue("@semantic", "WEIGHT")["@source"].Get<STxt>().substr(1);
	auto weightsNode = skinData["source"].GetPropertyWithValue("@id", weightsDataId)["float_array"];

	auto rawDatas = String::Split(weightsNode.Get<STxt>(), ' ');
...
std::vector<uint32_t> SkinLoader::GetEffectiveJointsCounts(const Node &weightsDataNode) const {
	auto rawData = String::Split(weightsDataNode["vcount"].Get<STxt>(), ' ');
...
void SkinLoader::GetSkinWeights(const Node &weightsDataNode, const std::vector<uint32_t> &counts, const std::vector<float> &weights) {
	auto rawData = String::Split(weightsDataNode["v"].Get<STxt>(), ' ');
	uint32_t pointer = 0;
