...
//! using 3 i16 without compression
...
struct Float4Storage
{
	...
	Float4Storage() : x(0), y(0), z(0), w(0)
	{

	};

	Float4Storage(float _x, float _y, float _z, float _w) : x(_x), y(_y), z(_z), w(_w)
	{

	};

	Float4Storage(const Quat& q) : x(q.v.x), y(q.v.y), z(q.v.z), w(q.w)
	{
...
struct NoCompressQuat
{
	...
	inline void   ToInternalType(const Quat& q)
	{
		val = q;
	}
...
//		m_x = static_cast<i16>(floor(tmp.x * MAX_SHORTINT + 0.5f));
//		m_y = static_cast<i16>(floor(tmp.y * MAX_SHORTINT + 0.5f));
//		m_z = static_cast<i16>(floor(tmp.z * MAX_SHORTINT + 0.5f));
//		m_w = static_cast<i16>(floor(tmp.w * MAX_SHORTINT + 0.5f));
...
struct NoCompressVec3
{
	...
	inline void ToInternalType(const Vec3& v)
	{
		x = v.x;
		y = v.y;
		z = v.z;
...
//		}
//
//		m_x = static_cast<i16>(floor(tmp.v.x * MAX_SHORTINT + 0.5f));
//		m_y = static_cast<i16>(floor(tmp.v.y * MAX_SHORTINT + 0.5f));
//		m_z = static_cast<i16>(floor(tmp.v.z * MAX_SHORTINT + 0.5f));
//		m_w = static_cast<i16>(floor(tmp.w * MAX_SHORTINT + 0.5f));
//	}
//
//	Quat ToQuat()
...
struct SmallTreeDWORDQuat
{
	...
	void         ToInternalType(Quat& q)
	{
		...
		for (i32 i = 0; i < 4; ++i)
		{
			...
			{
				//pack
				i16 packed = static_cast<i16>(floor((tmp.comp[i] + drange) * MAX_10BITf /* / 2.0f / drange*/ + 0.5f));// & 0x3FF;
	...
	void ToExternalType(Quat& q) const
	{
		...
		for (i32 i = 0; i < 4; ++i)
		{
			if (i != index)
			{
				...
				i16 packed = static_cast<i16>((m_Val >> c) & 0x3FF);
...
struct SmallTree48BitQuat
{
...
#if DRX_PLATFORM_SSE2 && !defined(_DEBUG)
	...
	void ToInternalType(const Quat& q)
	{
		...
		for (i32 i = 0; i < 4; ++i)
		{
			...
			{
				//pack
				i16 packed = static_cast<i16>(floor((tmp.comp[i] + drange) * MAX_15BITf /* / 2.0f / drange*/ + 0.5f));// & 0x3FF;
		...
		//save m_xxx
		m_1 = static_cast<u16>(m_Val & 0xFFFF);
		m_2 = static_cast<u16>((m_Val >> 16) & 0xFFFF);
		m_3 = static_cast<u16>((m_Val >> 32) & 0xFFFF);
...
//	#if DRX_PLATFORM_ORBIS || DRX_PLATFORM_APPLE || DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID
		#define TEMPLATE_PROCESSING(index, offset0, offset1, offset2)              \
		  m_Res[offset0] = (float)((u16)(m_1 & 0x7FFF));                        \
		  m_Res[offset1] = (float)((u16)(((m_1 >> 15) + (m_2 << 1)) & 0x7FFF)); \
		  m_Res[offset2] = (float)((u16)(((m_2 >> 14) + (m_3 << 2)) & 0x7FFF)); \
		  __m128 src = _mm_load_ps(m_Res);                                         \
		  __m128 a = _mm_mul_ps(src, div);                                         \
		  __m128 b = _mm_sub_ps(a, ran);                                           \
		  _mm_store_ps(m_Res, b);                                                  \
		  union {__m128 q; float fl[4]; } sq;                                      \
		  sq.q = _mm_mul_ps(b, b);                                                 \
...
/*	#else
	...
	*/
		//#define TEMPLATE_PROCESSING(index, offset0, offset1, offset2) \
		//    //			float packed = (float)((u16)(m_1 & 0x7FFF)); \
		//    //			m_Res[offset0] = (packed  / MAX_15BITf ) - RANGE_15BIT; \
		//    //			packed = (float)((u16)(((m_1 >> 15) + (m_2 << 1)) & 0x7FFF)); \
		//    //			m_Res[offset1] = (packed  / MAX_15BITf ) - RANGE_15BIT; \
		//    //		  packed = (float)((u16)(((m_2 >> 14) + (m_3 << 2)) & 0x7FFF)); \
		...
		//m_Res[offset[0]] = (float)((u16)(m_1 & 0x7FFF));
		//m_Res[offset[1]] = (float)((u16)(((m_1 >> 15) + (m_2 << 1)) & 0x7FFF));
		//m_Res[offset[2]] = (float)((u16)(((m_2 >> 14) + (m_3 << 2)) & 0x7FFF));
		...
		DRX_ALIGN(16) Float4Storage m_Res(Float4Storage::eZero);
		//		LONGDWORD m_Val;
...
public:
	...
	void         ToInternalType(Quat& q)
	{
		...
		m_1 = static_cast<u32>(m_Val);
		m_2 = static_cast<u32>(m_Val >> 32);
	...
	void ToExternalType(Quat& q) const
	{
		...
		u16 index = static_cast<u16>((m_2 >> 30) & 3);
		...
		//i32 i = offset[0];
		//i32 packed = static_cast<i32>((m_Val >> c) & mask);
		//res.comp[i] = static_cast<float>((float)packed  / MAX_20BITf ) - RANGE_20BIT;
		//sqrsumm += res.comp[i]* res.comp[i];
		//c += 20;

		//i = offset[1];
		//packed = static_cast<i32>((m_Val >> c) & mask);
		//res.comp[i] = static_cast<float>((float)packed  / MAX_20BITf ) - RANGE_20BIT;
		//sqrsumm += res.comp[i]* res.comp[i];
		//c += 20;
		...
		DRX_ALIGN(16) Float4Storage m_Res;
#if DRX_PLATFORM_SSE2 && !defined(_DEBUG)

		m_Res.comp[offset[0]] = (float)((i32)(m_1 & 0xFFFFF));
		m_Res.comp[offset[1]] = (float)((i32)(((m_1 >> 20) + (m_2 << 12)) & 0xFFFFF));
		m_Res.comp[offset[2]] = (float)((i32)((m_2 >> 8) & 0xFFFFF));
		...
		DRX_ALIGN(16) Float4Storage squares;
		...
		i32 i = offset[0];
		i32 packed = i32(m_1 & 0xFFFFF);
		m_Res.comp[i] = ((float)packed / MAX_20BITf) - RANGE_20BIT;
		sqrsumm += m_Res.comp[i] * m_Res.comp[i];
		c += 20;

		i = offset[1];
		packed = i32(((m_1 >> 20) + (m_2 << 12)) & 0xFFFFF);
		m_Res.comp[i] = ((float)packed / MAX_20BITf) - RANGE_20BIT;
		sqrsumm += m_Res.comp[i] * m_Res.comp[i];
		c += 20;

		i = offset[2];
		packed = i32((m_2 >> 8) & 0xFFFFF);
		m_Res.comp[i] = ((float)packed / MAX_20BITf) - RANGE_20BIT;
		sqrsumm += m_Res.comp[i] * m_Res.comp[i];
...
public:
	...
	void ToExternalType(Quat& q) const
	{
		u32 index = static_cast<u32>((m_2 >> 30) & 3);
		i32 offset[3];
		for (i32 i = 0, c = 0; i < 4; i++)
		{
			if (i != index)
		...
		DRX_ALIGN(16) Float4Storage m_Res;
#if DRX_PLATFORM_SSE2 && !defined(_DEBUG)
		DRX_ALIGN(16) Float4Storage max;
		DRX_ALIGN(16) Float4Storage range;

		m_Res.comp[offset[0]] = (float)((i32)(m_1 & 0x1FFFFF));
		m_Res.comp[offset[1]] = (float)((i32)(((m_1 >> 21) + (m_2 << 11)) & 0x1FFFFF));
		m_Res.comp[offset[2]] = (float)((i32)((m_2 >> 10) & 0xFFFFF));
		...
		i32 i = offset[0];
		i32 packed = (/*(i32)*/ (m_1 & 0x1FFFFF));
		m_Res.comp[i] = ((float)packed / MAX_21BITf) - RANGE_21BIT;
		sqrsumm += m_Res.comp[i] * m_Res.comp[i];
		c += 21;

		i = offset[1];
		packed = (/*(i32)*/ (((m_1 >> 21) + (m_2 << 11)) & 0x1FFFFF));
		m_Res.comp[i] = ((float)packed / MAX_21BITf) - RANGE_21BIT;
		sqrsumm += m_Res.comp[i] * m_Res.comp[i];
		c += 21;

		i = offset[2];
		packed = (/*(i32)*/ ((m_2 >> 10) & 0xFFFFF));
		m_Res.comp[i] = ((float)packed / MAX_20BITf) - RANGE_20BIT;
		sqrsumm += m_Res.comp[i] * m_Res.comp[i];
		...
		m_Res.comp[index] = sqrtf(1.0f - sqrsumm);
		//	else
		//			m_Res.comp[index] = 0.0f;

		//DRX_ALIGN(16) Float4Storage m_Res1;
