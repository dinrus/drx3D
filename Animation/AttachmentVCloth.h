...
{
	i32   iMap; // vertex index in the sim mesh
	i32   iTri; // triangle index in the sim mesh
...
struct SClothGeometry
{
	...
	void      Cleanup();
	i32       GetBuffers();
	void      ReleaseBuffers(i32 idx);
	SBuffers* GetBufferPtr(i32 idx);
	void      AllocateBuffer();
};
...
struct SClothInfo
{
	...
	SClothInfo() {}
	SClothInfo(uint64 k, float d, bool v, i32 f) : key(k), distance(d), visible(v), frame(f) {}
};
...
struct SParticleHot
{
	...
	i32   nndcIdx;        //!< index of closest constraint / attached vtx
...
public:
	...
	i32                  SetParams(const SVClothParams& params, float* weights);
	...
	bool                 IsParticleAttached(u32 idx) const { assert(idx < m_nVtx); return m_particlesCold[idx].bAttached != 0; }
	...
	void SetAttachmentUpr(const CAttachmentUpr* am) { m_pAttachmentUpr = am; }
...
private:
	...
	const CAttachmentUpr* m_pAttachmentUpr;

	i32                       m_nVtx;          //!< the number of particles
	i32                       m_nEdges;        //!< the number of links between particles
	...
	i32                       m_fadeInOutPhysicsDirection;        //!< -1 fade out, 1 fade in
	i32                       m_doSkinningForNSteps;              //!< use skinning if any position change has occured, to keep simulation stable
	i32                       m_forceSkinningAfterNFramesCounter; //!< safety mechanism, i.e. local counter: if framerate falls below threshold for n-frames, skinning is forced to avoid performance issues
	...
	// Nearest Neighbor Distance Constraints
	std::vector<i32> m_nndcNotAttachedOrderedIdx; //!< not attached particles: ordered by distance to constraints
	...
	i32                m_debugCollidableSubsteppingId;
...
{
	i32 n1 = !(m_particlesCold[link.i1].bAttached);
	i32 n2 = !(m_particlesCold[link.i2].bAttached);
	link.skip = n1 + n2 == 0;
	if (link.skip)
		return;
...
public:
	...
	i32                  GetNumLods()      { return m_numLods; }
	//	bool IsSingleThreaded() { return m_bSingleThreaded; }
	bool                 IsAlwaysVisible() { return m_bAlwaysVisible; }

	bool                 PrepareCloth(CSkeletonPose& skeletonPose, const Matrix34& worldMat, bool visible, i32 lod);
...
private:
	...
	bool    PrepareRenderMesh(i32 lod);
	Vector4 SkinByTriangle(i32 i, strided_pointer<Vec3>& pVtx, i32 lod);

	void    UpdateSimulation(const DualQuat* pTransformations, const uint transformationCount);
	template<bool PREVIOUS_POSITIONS>
	void    SkinSimulationToRenderMesh(i32 lod, CVertexData& vertexData, const strided_pointer<const Vec3>& pVertexPositionsPrevious);
...
ILINE Vector4 CClothPiece::SkinByTriangle(i32 i, strided_pointer<Vec3>& pVtx, i32 lod)
{
	...
	i32 tri = skinMap.iTri;
	if (tri >= 0)
	{
		mesh_data* md = (mesh_data*)m_clothGeom->pPhysGeom->pGeom->GetData();
		i32k i2 = skinMap.iMap;
		i32k base = tri * 3;
		i32k idx = md->pIndices[base + i2];
		i32k idx0 = md->pIndices[base + inc_mod3[i2]];
		i32k idx1 = md->pIndices[base + dec_mod3[i2]];
...
public:
	...
	virtual u32             GetType() const override                               { return CA_VCLOTH; }
	...
	virtual u32             GetFlags() const override                              { return m_AttFlags | FLAGS_ATTACH_MERGED_FOR_SHADOWS; } // disable merging for vcloth shadows
	...
	virtual u32             Immediate_AddBinding(IAttachmentObject* pModel, ISkin* pISkinRender = 0, u32 nLoadingFlags = 0) override;
	...
	u32                     AddSimBinding(const ISkin& pISkinRender, u32 nLoadingFlags = 0);
	...
	virtual void         PostUpdateSimulationParams(bool bAttachmentSortingRequired, tukk pJointName = 0) override {};
...
public:
	SSkinningData*          GetVertexTransformationData(const bool bVertexAnimation, u8 nRenderLOD, const SRenderingPassInfo& passInfo);
