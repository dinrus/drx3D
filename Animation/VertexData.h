...
#include <drx3D/Animation/IVertexAnimation.h>

struct RChunk;

typedef u16 SoftwareVertexBlendIndex;
typedef u8  SoftwareVertexBlendWeight;

struct DRX_ALIGN(64) SSoftwareVertexFormat
{
	Vec3 position;
	u32 color;
	Quat tangent;
...
public:
	...
	tukk                 GetNameByIndex(const uint index) const { return m_frames[index].name; }
	uint                        GetIndexByName(tukk name) const;
...
public:
	...
	strided_pointer<u32k>                    GetColors() const       { return strided_pointer<u32k>(&m_vertices[0].color, sizeof(m_vertices[0])); }
...
private:
	...
	strided_pointer<u32>                    GetWriteColors()       { return strided_pointer<u32>(&m_vertices[0].color, sizeof(m_vertices[0])); }
...
public:
	...
	strided_pointer<u32>       GetColors()            { return pColors; }
