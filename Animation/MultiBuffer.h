...
template<typename TTypeList>
inline void MultiBuffer<TTypeList >::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
template<typename TTypeList>
inline uk MultiBuffer<TTypeList >::GetBuffer()
{
	return Align(m_pBuffer, GetBufferAlignmentRequirement());
}

template<typename TTypeList>
inline uk MultiBuffer<TTypeList >::GetField(size_t index)
{
	assert(GetBuffer());

	const size_t offset = (index > 0) ? (m_offsets[index - 1]) : (0);
	return static_cast<tuk>(GetBuffer()) + offset;
}
