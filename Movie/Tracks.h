	...
	virtual void           SerializeKey(SCommentKey& key, XmlNodeRef& keyNode, bool bLoading) override
	{
		...
		if (bLoading)
		{
			...
			if (gEnv->IsEditor())
			{
				...
				tukk strFont = keyNode->getAttr("font");
				drx_strcpy(key.m_font, strFont);
		...
		else
		{
			...
			keyNode->setAttr("align", (i32)key.m_align);
		}
	}
};
...
public:
	...
	virtual bool IsMasked(u32k mask) const override { return (mask & eTrackMask_MaskSound) != 0; }
	...
	virtual void SerializeKey(SAudioTriggerKey& key, XmlNodeRef& keyNode, bool bLoading) override
	{
		if (bLoading)
		{
			...
			key.m_startTriggerId = DrxAudio::StringToId(key.m_startTriggerName.c_str());
			key.m_stopTriggerId = DrxAudio::StringToId(key.m_stopTriggerName.c_str());
	...
	virtual void           SerializeKey(SAudioFileKey& key, XmlNodeRef& keyNode, bool bLoading) override
	{
		...
		else
		{
			...
			if (bIsLocalized)
			{
				...
				i32 pathLength = audioFilePath.find(szLanguage);
				if (pathLength > 0)
				{
		...
		{
			i32 pathLength = key.m_audioFile.find(PathUtil::GetGameFolder());
			const string tempFilePath = (pathLength == -1) ? PathUtil::GetGameFolder() + DRX_NATIVE_PATH_SEPSTR + key.m_audioFile : key.m_audioFile;

			DrxAudio::SFileData fileData;
...
public:
	...
	virtual void           SerializeKey(SAudioSwitchKey& key, XmlNodeRef& keyNode, bool bLoading) override
	{
		if (bLoading)
		{
			...
			key.m_audioSwitchId = DrxAudio::StringToId(key.m_audioSwitchName.c_str());
			key.m_audioSwitchStateId = DrxAudio::StringToId(key.m_audioSwitchStateName.c_str());
		}
		else
		{
...
public:
	...
	virtual void           Serialize(Serialization::IArchive& ar) override
	{
		...
		if (ar.isInput())
		{
			m_audioParameterId = DrxAudio::StringToId(m_audioParameterName.c_str());
		}
	}
...
public:
	...
	virtual void           SerializeKey(SExprKey& key, XmlNodeRef& keyNode, bool bLoading) override
	{
		if (bLoading)
		{
			...
			drx_strcpy(key.m_name, desc, sizeof(key.m_name));
...
public:
	...
	virtual void           SerializeKey(SSequenceKey& key, XmlNodeRef& keyNode, bool bLoading) override
	{
		if (bLoading)
		{
			...
			if (key.m_sequenceGUID == DrxGUID::Null())
			{
				tukk pSelection = keyNode->getAttr("node");
				if (pSelection)
				{
					IAnimSequence* pSequence = gEnv->pMovieSystem->FindSequence(pSelection);
					key.m_sequenceGUID = pSequence ? pSequence->GetGUID() : DrxGUID::Null();
				}
			}
...
public:
	...
	virtual void           SerializeKey(SConsoleKey& key, XmlNodeRef& keyNode, bool bLoading) override
	{
		if (bLoading)
		{
			...
			drx_strcpy(key.m_command, str, sizeof(key.m_command));
...
public:
	...
	virtual void           SerializeKey(SMannequinKey& key, XmlNodeRef& keyNode, bool bLoading) override
	{
		...
		if (bLoading)
		{
			...
			drx_strcpy(key.m_fragmentName, str, sizeof(key.m_fragmentName));
			key.m_fragmentName[sizeof(key.m_fragmentName) - 1] = 0;

			str = keyNode->getAttr("tags");
			drx_strcpy(key.m_tags, str, sizeof(key.m_tags));
...
public:
	...
	virtual void           SerializeKey(SCaptureKey& key, XmlNodeRef& keyNode, bool bLoading) override
	{
		if (bLoading)
		{
			...
			drx_strcpy(key.m_folder, desc);
			keyNode->getAttr("once", key.m_bOnce);
			desc = keyNode->getAttr("prefix");

			if (desc)
			{
				drx_strcpy(key.m_prefix, desc);
			}

			keyNode->getAttr("bufferToCapture", reinterpret_cast<i32&>(key.m_bufferToCapture));
		}
		else
		{
