...
void CAnimSequence::SetName(tukk name)
{
...
tukk CAnimSequence::GetName() const
{
	return m_name.c_str();
}

void CAnimSequence::SetFlags(i32 flags)
{
	m_flags = flags;
}

i32 CAnimSequence::GetFlags() const
{
	return m_flags;
}

i32 CAnimSequence::GetCutSceneFlags(const bool localFlags) const
{
	i32 currentFlags = m_flags & (eSeqFlags_NoUI | eSeqFlags_NoPlayer | eSeqFlags_NoAbort);
...
i32 CAnimSequence::GetNodeCount() const
{
	return m_nodes.size();
}

IAnimNode* CAnimSequence::GetNode(i32 index) const
{
...
bool CAnimSequence::AddNode(IAnimNode* pAnimNode)
{
	...
	i32k nodeId = static_cast<CAnimNode*>(pAnimNode)->GetId();

	if (nodeId >= (i32)m_lastGenId)
	{
...
IAnimNode* CAnimSequence::CreateNodeInternal(EAnimNodeType nodeType, u32 nNodeId)
{
...
void CAnimSequence::Serialize(XmlNodeRef& xmlNode, bool bLoading, bool bLoadEmptyTracks, u32 overrideId, bool bResetLightAnimSet)
{
	if (bLoading)
	{
		// Load.
		...
		if (nodes)
		{
			...
			for (i32 i = 0; i < nodes->getChildCount(); ++i)
			{
			...
			for (AnimNodes::const_iterator it = m_nodes.begin(); it != m_nodes.end(); ++it)
			{
				...
				if (pAnimNode->GetId() >= (i32)m_lastGenId)
				{
		...
		if (events)
		{
			...
			for (i32 i = 0; i < events->getChildCount(); i++)
			{
		...
		if (GetFlags() & IAnimSequence::eSeqFlags_LightAnimationSet)
		{
			...
			{
				DRX_ASSERT_MESSAGE(0, "Track Sequence flagged as LightAnimationSet have LightAnimationSet pointer already set");
			}
...
bool CAnimSequence::AddTrackEvent(tukk szEvent)
{
	DRX_ASSERT(szEvent && szEvent[0]);
...
bool CAnimSequence::RemoveTrackEvent(tukk szEvent)
{
	DRX_ASSERT(szEvent && szEvent[0]);
...
bool CAnimSequence::RenameTrackEvent(tukk szEvent, tukk szNewEvent)
{
	DRX_ASSERT(szEvent && szEvent[0]);
	DRX_ASSERT(szNewEvent && szNewEvent[0]);
...
bool CAnimSequence::MoveUpTrackEvent(tukk szEvent)
{
	DRX_ASSERT(szEvent && szEvent[0]);
...
bool CAnimSequence::MoveDownTrackEvent(tukk szEvent)
{
	DRX_ASSERT(szEvent && szEvent[0]);
...
i32 CAnimSequence::GetTrackEventsCount() const
{
	return (i32)m_events.size();
}

char const* CAnimSequence::GetTrackEvent(i32 iIndex) const
{
...
void CAnimSequence::NotifyTrackEvent(ITrackEventListener::ETrackEventReason reason,
                                     tukk event, tukk param)
{
	// Notify listeners
	for (TTrackEventListeners::iterator j = m_listeners.begin(); j != m_listeners.end(); ++j)
	{
		(*j)->OnTrackEvent(this, reason, event, (uk )param);
	}
}

void CAnimSequence::TriggerTrackEvent(tukk event, tukk param)
{
...
IAnimNode* CAnimSequence::FindNodeById(i32 nNodeId)
{
	for (AnimNodes::const_iterator it = m_nodes.begin(); it != m_nodes.end(); ++it)
	{
		IAnimNode* pAnimNode = *it;
...
IAnimNode* CAnimSequence::FindNodeByName(tukk sNodeName, const IAnimNode* pParentDirector)
{
	for (AnimNodes::const_iterator it = m_nodes.begin(); it != m_nodes.end(); ++it)
	{
		IAnimNode* pAnimNode = *it;
	...
	for (AnimNodes::const_iterator it = m_nodes.begin(); it != m_nodes.end(); ++it)
	{
		...
		if (pNode->GetType() == eAnimNodeType_Director)
		{
			...
			if (pSequenceTrack)
			{
				...
				for (i32 i = 0; i < pSequenceTrack->GetNumKeys(); ++i)
				{
					SSequenceKey key;
...
void CAnimSequence::ExecuteAudioTrigger(const DrxAudio::ControlId audioTriggerId)
{
