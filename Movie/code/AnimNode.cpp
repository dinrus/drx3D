...
static i32k OLD_APARAM_USER = 100;
static i32k OLD_ACURVE_GOTO = 21;
static i32k OLD_APARAM_PARTICLE_COUNT_SCALE = 95;
static i32k OLD_APARAM_PARTICLE_PULSE_PERIOD = 96;
static i32k OLD_APARAM_PARTICLE_SCALE = 97;
static i32k OLD_APARAM_PARTICLE_SPEED_SCALE = 98;
static i32k OLD_APARAM_PARTICLE_STRENGTH = 99;

void CAnimNode::Activate(bool bActivate)
{
}

i32 CAnimNode::GetTrackCount() const
{
	return m_tracks.size();
}

tukk CAnimNode::GetParamName(const CAnimParamType& paramType) const
{
	SParamInfo info;
...
{
	for (i32 i = 0, num = (i32)m_tracks.size(); i < num; i++)
	{
		...
		for (i32 k = 0; k < m_tracks[i]->GetSubTrackCount(); ++k)
		{
...
IAnimTrack* CAnimNode::GetTrackForParameter(const CAnimParamType& paramType, u32 index) const
{
...
u32 CAnimNode::GetTrackParamIndex(const IAnimTrack* pTrack) const
{
	assert(pTrack);
	u32 index = 0;
...
IAnimTrack* CAnimNode::GetTrackByIndex(i32 nIndex) const
{
	if (nIndex >= (i32)m_tracks.size())
	{
...
void CAnimNode::SerializeAnims(XmlNodeRef& xmlNode, bool bLoading, bool bLoadEmptyTracks)
{
	...
	if (bLoading)
	{
		...
		for (i32 i = 0; i < num; i++)
		{
			...
			{
				i32k APARAM_ANIMATION_OLD = eAnimParamType_AudioTrigger;
				i32k APARAM_SOUND_OLD = eAnimParamType_Animation;
			...
			i32 valueType = eAnimValue_Unknown;
	...
	else
	{
		...
		for (u32 i = 0; i < m_tracks.size(); i++)
		{
			...
			if (pTrack)
			{
				...
				i32 valueType = pTrack->GetValueType();
				if (valueType != eAnimValue_Unknown)
				{
...
CAnimNode::CAnimNode(i32k id) : m_id(id)
{
...
void CAnimNode::SetFlags(i32 flags)
{
	m_flags = flags;
}

i32 CAnimNode::GetFlags() const
{
	return m_flags;
}
