...
void CAnimSplineTrack::SerializeKey(S2DBezierKey& key, XmlNodeRef& keyNode, bool bLoading)
{
	if (bLoading)
	{
		...
		if (!keyNode->getAttr("value", vecValue))
		{
			...
			{
				DrxLog("[DRXMOVIE:TAnimSplineTrack<Vec2>::Serialize]Ill formed legacy track:missing value information.");
				return;
			}
		}
		else
		...
		{
			DrxLog("[DRXMOVIE:TAnimSplineTrack<Vec2>::Serialize]Ill formed legacy track:missing ds spline information.");
			return;
		}

		if (!keyNode->getAttr("dd", key.m_controlPoint.m_outTangent))
		{
			DrxLog("[DRXMOVIE:TAnimSplineTrack<Vec2>::Serialize]Ill formed legacy track:dd spline information.");
			return;
		}
