...
#include <drx3D/Movie/CharacterTrack.h>
...
#include <drx3D/Movie/AnimSequence.h>
...
static tukk s_VariablePrefixes[] =
{
	"n",                                                        "i",               "b",           "f",            "s",         "ei", "es",
...
void AddSupportedParam(std::vector<CAnimNode::SParamInfo>& nodeParams, tukk sName, i32 paramId, EAnimValue valueType, i32 flags = 0)
{
...
void NotifyEntityScript(const IEntity* pEntity, tukk funcName)
{
...
void NotifyEntityScript(const IEntity* pEntity, tukk funcName, tukk strParam)
{
	IScriptTable* pEntityScript = pEntity->GetScriptTable();
...
CAnimEntityNode::CAnimEntityNode(i32k id) : CAnimNode(id)
{
	m_EntityId = 0;
	m_target = NULL;
...
{
public:
	CComponentSerializer(tukk szGroupName, tukk szGroupLabelName, CAnimEntityNode& node, const DrxGUID& componentGUID)
	...
	void AddComponentProperty(EAnimValue valueType, TActual& value, tukk name, tukk label)
	{
	...
	YASLI_INLINE bool operator()(char& value, tukk name = "", tukk label = 0)  override { return false; }
	...
	YASLI_INLINE bool operator()(double& value, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::i8& value, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::i16& value, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::i32& value, tukk name = "", tukk label = 0)  override
	{
		AddComponentProperty<float, i32>(eAnimValue_Float, value, name, label);

		return true;
	}
	YASLI_INLINE bool operator()(yasli::i64& value, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::u8& value, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::u16& value, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::u32& value, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::u64& value, tukk name = "", tukk label = 0)  override { return false; }

	YASLI_INLINE bool operator()(yasli::StringInterface& value, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::WStringInterface& value, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(const yasli::Serializer& ser, tukk name = "", tukk label = 0)  override
	{
		...
		else if (ser.type() == yasli::TypeID::get<DrxTransform::CRotation>())
		{
			CAnimEntityNode::SComponentPropertyParamInfo paramInfo;
			...
			DrxTransform::CRotation* pRotation = reinterpret_cast<DrxTransform::CRotation*>(ser.pointer());
			...
			paramInfo.setValueCallback = [pRotation](const TMovieSystemValue& newValueT) -> bool
			{
				...
				{
					*pRotation = DrxTransform::CRotation(newValue);
					return true;
				}
		...
		else if (ser.type() == yasli::TypeID::get<DrxTransform::CAngle>() || ser.type() == yasli::TypeID::get<DrxTransform::CClampedAngle<>>())
		{
			CAnimEntityNode::SComponentPropertyParamInfo paramInfo;
			...
			DrxTransform::CAngle* pAngle = reinterpret_cast<DrxTransform::CAngle*>(ser.pointer());
			...
			paramInfo.setValueCallback = [pAngle](const TMovieSystemValue& newValueT) -> bool
			{
				...
				{
					*pAngle = DrxTransform::CAngle::FromDegrees(newValue);
					return true;
				}
	...
	YASLI_INLINE bool operator()(yasli::BlackBox& ser, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::ContainerInterface& ser, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::KeyValueInterface& ser, tukk name = "", tukk label = 0)  override { return false; }
	YASLI_INLINE bool operator()(yasli::PointerInterface& ser, tukk name = "", tukk label = 0)  override { return false; }
...
void CAnimEntityNode::UpdateDynamicParams_Editor()
{
	if (IEntity* pEntity = GetEntity())
	{
		...
		{
			tukk szComponentName = pComponent->GetClassDesc().GetName().c_str();
			...
			const sxema::CClassMemberDescArray& componentMembers = pComponent->GetClassDesc().GetMembers();
			for (const sxema::CClassMemberDesc& componentMember : componentMembers)
			{
				componentMember.GetTypeDesc().GetOperators().serialize(archive, reinterpret_cast<u8*>(pComponent) + componentMember.GetOffset(), componentMember.GetName(), componentMember.GetLabel());
			}
		}
	}
}
...
void CAnimEntityNode::FindDynamicPropertiesRec(IScriptTable* pScriptTable, const string& currentPath, u32 depth)
{
	// There might be infinite recursion in the tables, so we need to limit the max recursion depth
	u32k kMaxRecursionDepth = 5;
...
bool CAnimEntityNode::ObtainPropertyTypeInfo(tukk pKey, IScriptTable* pScriptTable, SScriptPropertyParamInfo& paramInfo)
{
	ScriptAnyValue value;
...
void CAnimEntityNode::AddPropertyToParamInfoMap(tukk pKey, const string& currentPath, SScriptPropertyParamInfo& paramInfo)
{
	// Strip variable name prefix
	tukk strippedName = pKey;
	...
	for (size_t i = 0; i < sizeof(s_VariablePrefixes) / sizeof(tukk); ++i)
	{
...
u32 CAnimEntityNode::GetParamCount() const
{
	return CAnimEntityNode::GetParamCountStatic() + m_entityProperties.size();
}

CAnimParamType CAnimEntityNode::GetParamType(u32 nIndex) const
{
	SParamInfo info;
...
i32 CAnimEntityNode::GetParamCountStatic()
{
	return g_nodeParams.size();
}

bool CAnimEntityNode::GetParamInfoStatic(i32 nIndex, SParamInfo& info)
{
	if (nIndex >= 0 && nIndex < (i32)g_nodeParams.size())
	{
...
{
	for (i32 i = 0; i < (i32)g_nodeParams.size(); i++)
	{
...
tukk CAnimEntityNode::GetParamName(const CAnimParamType& param) const
{
	SParamInfo info;
...
void CAnimEntityNode::SetEntityId(i32k entityId)
{
	m_EntityId = entityId;
}
...
void CAnimEntityNode::Animate(SAnimContext& animContext)
{
	...
	i32 animCharacterLayer = 0;
	i32 animationTrack = 0;
	...
	for (i32 paramIndex = 0; paramIndex < trackCount; paramIndex++)
	{
		...
		case eAnimParamType_Scale:
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_MOVIE, VALIDATOR_WARNING,
					           "An EntityNode <%s> gets an invalid scale (%f,%f,%f) from a TrackView track, so ignored.",
					           (tukk)GetName(), scale.x, scale.y, scale.z);
					scale = m_scale;
				}
		...
		case eAnimParamType_Event:
			if (!animContext.bResetting)
			{
				...
				SEventKey key;
				i32 entityKey = pEntityTrack->GetActiveKey(animContext.time, &key);
		...
		case eAnimParamType_AudioFile:
			{
				...
				if (!animContext.bResetting && !bMute)
				{
					...
					i32k audioFileKeyNum = pAudioFileTrack->GetActiveKey(animContext.time, &audioFileKey);
					if (pEntity && audioFileKeyNum >= 0 && audioFileKey.m_duration > SAnimTime(0) && !(audioFileKey.m_bNoTriggerInScrubbing && animContext.bSingleFrame))
					{
						...
						if (animContext.time <= audioFileKey.m_time + audioFileKey.m_duration)
						{
							if (audioFileInfo.audioKeyStart < audioFileKeyNum)
							{
								...
								{
									const DrxAudio::SPlayFileInfo audioPlayFileInfo(audioFileKey.m_audioFile, audioFileKey.m_bIsLocalized);
		...
		case eAnimParamType_AudioSwitch:
			{
				...
				if (!animContext.bResetting && !bMute)
				{
					...
					if (prevAudioSwitchKeyNum != newAudioSwitchKeyNum)
					{
						...
						{
							DrxAudio::ControlId audioSwitchId = audioSwitchKey.m_audioSwitchId;
							DrxAudio::SwitchStateId audioSwitchStateId = audioSwitchKey.m_audioSwitchStateId;
		...
		case eAnimParamType_DynamicResponseSignal:
			{
				if (!animContext.bResetting)
				{
					...
					i32 drsSignalKey = pDRSSignalTrack->GetActiveKey(animContext.time, &key);
					if (drsSignalKey != m_lastDrsSignalKey || key.m_time == animContext.time)
					{
						...
						if (drsSignalKey >= 0)
						{
							...
							if (!bNotTrigger && pEntity)
							{
								...
								const IEntityDynamicResponseComponent* pIEntityDrsProxy = drxcomponent_cast<IEntityDynamicResponseComponent*>(pEntity->CreateProxy(ENTITY_PROXY_DYNAMICRESPONSE));
		...
		case eAnimParamType_Mannequin:
			if (!animContext.bResetting)
			{
				...
				i32 key = pMannequinTrack->GetActiveKey(animContext.time, &manKey);
				...
				if (key >= 0)
				{
					...
					if (m_iCurMannequinKey != key)
					{
						...
						u32k valueName = CCrc32::ComputeLowercase(manKey.m_fragmentName);
							...
							if (pAnimChar)
							{
								...
								if (!tagName.empty())
								{
									...
									while (tagName.find("+") != string::npos)
									{
										...
										if (tagDefinition)
										{
											u32k tagCRC = CCrc32::ComputeLowercase(curTagName);
										...
										if (!found)
										{
											...
											if (tagDefinition)
											{
												i32k tagCRC = CCrc32::ComputeLowercase(curTagName);
												const TagID tagID = tagDefinition->Find(tagCRC);
												found = tagID != TAG_ID_INVALID;
												tagDefinition->Set(tagState, tagID, true);
											}
...
void CAnimEntityNode::PrepareAnimations()
{
	...
	for (i32 paramIndex = 0; paramIndex < trackCount; paramIndex++)
	{
		...
		if (pAnimations)
		{
			...
			case eAnimParamType_Animation:
				{
					i32 numKeys = pTrack->GetNumKeys();

					for (i32 i = 0; i < numKeys; i++)
					{
						SCharacterKey key;
						pTrack->GetKey(i, &key);
						i32 animId = pAnimations->GetAnimIDByName(key.m_animation);
...
IFacialAnimSequence* CAnimEntityNode::LoadFacialSequence(tukk sequenceName)
{
...
void CAnimEntityNode::Activate(bool bActivate)
{
	...
	else
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_MOVIE, VALIDATOR_ERROR, "Entity: %s. A trackview movie has called lua function 'OnPropertyChange' too "
...
{
	DRX_ASSERT_MESSAGE(m_activeAudioTriggers.empty(), "m_activeAudioTriggers is not empty during CAnimEntityNode::OnStart");
...
void CAnimEntityNode::UpdateEntityPosRotVel(const Vec3& targetPos, const Quat& targetRot, const bool initialState, i32k flags, SAnimTime fTime)
{
	IEntity* pEntity = GetEntity();
...
void CAnimEntityNode::ApplyEventKey(CEventTrack* track, i32 keyIndex, SEventKey& key)
{
	IEntity* pEntity = GetEntity();
	...
	if (key.m_event.size() > 0) // if there's an event
	{
		...
		if (pScriptProxy)
		{
			...
			{
				i32k count = pClass->GetEventCount();
			...
			case IEntityClass::EVT_VECTOR:
				{
					Vec3 vTemp(0, 0, 0);
					float x = 0, y = 0, z = 0;
					i32 res = sscanf(key.m_eventValue, "%f,%f,%f", &x, &y, &z);
					assert(res == 3);
					vTemp(x, y, z);
...
//////////////////////////////////////////////////////////////////////////
void CAnimEntityNode::ApplyAudioTriggerKey(DrxAudio::ControlId audioTriggerId, bool const bPlay /* = true */)
{
...
void CAnimEntityNode::OnEndAnimation(tukk sAnimation)
{
	IEntity* pEntity = GetEntity();
...
static i32 GetActiveKeys(i32 activeKeys[], SAnimTime ectime, CCharacterTrack* track)
{
	i32 numActiveKeys = 0;
...
static bool GetNearestKeys(i32 activeKeys[], SAnimTime ectime, CCharacterTrack* track, i32& numActiveKeys)
{
...
static bool HaveKeysChanged(i32 activeKeys[], i32 previousKeys[])
{
	return !((activeKeys[0] == previousKeys[0]) && (activeKeys[1] == previousKeys[1]));
}

void CAnimEntityNode::AnimateCharacterTrack(class CCharacterTrack* pTrack, SAnimContext& animContext, i32 layer, i32 trackIndex, SAnimState& animState, IEntity* pEntity, ICharacterInstance* pCharacter)
{
	...
	i32 activeKeys[2] = { -1, -1 };
	i32 numActiveKeys = GetActiveKeys(activeKeys, animContext.time, pTrack);
...
void CAnimEntityNode::AnimateExpressionTrack(CExprTrack* pTrack, SAnimContext& animContext)
{
	...
	SExprKey exprKey;
	i32 nKeys = pTrack->GetNumKeys();

	// we go through all the keys, since expressions may overlap
	for (i32 nKey = 0; nKey < nKeys; nKey++)
	{
		...
		DrxCharMorphParams morphParams;
...
void CAnimEntityNode::AnimateFacialSequence(CFaceSequenceTrack* pTrack, SAnimContext& animContext)
{
	...
	SFaceSequenceKey key;
	i32 nkey = pTrack->GetActiveKey(animContext.time, &key);
	...
	if (pLookAtEntity)
	{
		...
		else
		{
			...
			if (pLookAtChar)
			{
				...
				i16 nHeadBoneId = pLookAtChar->GetIDefaultSkeleton().GetJointIDByName(HEAD_BONE_NAME);
		...
		if (pCharacter)
		{
			...
			{
				u32 lookIKLayer = DEFAULT_LOOKIK_LAYER;
...
bool CAnimEntityNode::AnimateEntityProperty(IAnimTrack* pTrack, SAnimContext& animContext, tukk szName)
{
	if (IEntity* pEntity = GetEntity())
	{
...
bool CAnimEntityNode::AnimateLegacyScriptProperty(const SScriptPropertyParamInfo& param, IEntity* pEntity, IAnimTrack* pTrack, SAnimContext& animContext)
{
	...
	case eAnimValue_Bool:
		...
		{
			i32 boolIntValue = boolValue ? 1 : 0;
...
void CAnimEntityNode::Serialize(XmlNodeRef& xmlNode, bool bLoading, bool bLoadEmptyTracks)
{
	if (bLoading)
	{
		tukk name = xmlNode->getAttr("Name");
...
void CAnimEntityNode::ApplyAnimKey(i32 keyIndex, class CCharacterTrack* track, SAnimTime ectime,
																		 ICharacterInstance* pCharacter, i32 layer, i32 animIndex, bool bAnimEvents)
{
	SCharacterKey key;
...
void CAnimEntityNode::UpdateAnimTimeJumped(i32 keyIndex, class CCharacterTrack* track,
																						 SAnimTime ectime, ICharacterInstance* pCharacter, i32 layer, bool bAnimEvents, i32 trackIndex, SAnimState& animState)
{
...
void CAnimEntityNode::UpdateAnimRegular(i32 numActiveKeys, i32 activeKeys[], class CCharacterTrack* track, SAnimTime ectime, ICharacterInstance* pCharacter, i32 layer, bool bAnimEvents)
{
	// cross-fade
	if (numActiveKeys == 2)
	{
...
void CAnimEntityNode::UpdateAnimBlendGap(i32 activeKeys[], class CCharacterTrack* track, SAnimTime ectime, ICharacterInstance* pCharacter, i32 layer)
{
	f32 blendWeight = 0.0f;
	SCharacterKey key1, key2;
	track->GetKey(activeKeys[0], &key1);
...
bool CAnimEntityNode::CheckTimeJumpingOrOtherChanges(const SAnimContext& animContext, i32 activeKeys[], i32 numActiveKeys,
                                                     ICharacterInstance* pCharacter, i32 layer, i32 trackIndex, SAnimState& animState)
{
...
void CAnimEntityNode::PrecacheStatic(SAnimTime time)
{
	...
	for (i32 paramIndex = 0; paramIndex < trackCount; paramIndex++)
	{
		...
		if (paramType == eAnimParamType_AudioTrigger)
		{
			...
			i32 const numKeys = pTrack->GetNumKeys();
			for (i32 keyIndex = 0; keyIndex < numKeys; ++keyIndex)
			{
		...
		if (trackType == eAnimParamType_Animation)
		{
			...
			for (i32 i = 0; i < numKeys; i++)
			{
				...
				if (key.m_time < time)
				{
					...
					if (animID >= 0)
					{
						u32 animPathCRC = pAnimations->GetFilePathCRCByAnimID(animID);
...
void CAnimationCacher::AddToCache(u32 animPathCRC)
{
	u32 numAnims = m_cachedAnims.size();
	...
	if (gEnv->pCharacterUpr->CAF_AddRef(animPathCRC))
	{
		m_cachedAnims.push_back(animPathCRC);
	}
}

void CAnimationCacher::RemoveFromCache(u32 animPathCRC)
{
	u32 numAnims = m_cachedAnims.size();
	...
	for (u32 i = 0; i < numAnims; ++i)
	{
		if (m_cachedAnims[i] == animPathCRC)
		{
			gEnv->pCharacterUpr->CAF_Release(animPathCRC);

			for (u32 j = i; j < numAnims - 1; ++j)
			{
...
void CAnimationCacher::ClearCache()
{
	...
	for (u32 i = 0; i < numAnims; ++i)
	{
		gEnv->pCharacterUpr->CAF_Release(m_cachedAnims[i]);
	}
