...
#include <drx3D/Movie/AnimCameraNode.h>
#include <drx3D/Movie/SceneNode.h>
#include <drx3D/Movie/MaterialNode.h>
#include <drx3D/Movie/EventNode.h>
#include <drx3D/Movie/AnimPostFXNode.h>
...
#include <drx3D/Movie/AnimKey_impl.h>
#include <drx3D/Act/IViewSystem.h>

i32 CMovieSystem::m_mov_NoCutscenes = 0;
float CMovieSystem::m_mov_cameraPrecacheTime = 1.f;
#if !defined(_RELEASE)
i32 CMovieSystem::m_mov_debugCamShake = 0;
i32 CMovieSystem::m_mov_debugSequences = 0;
#endif
...
namespace
{
std::unordered_map<i32, string> g_animNodeEnumToStringMap;
std::unordered_map<i32, string> g_animNodeEnumToDefaultNameMap;
std::map<string, EAnimNodeType, stl::less_stricmp<string>> g_animNodeStringToEnumMap;

std::unordered_map<i32, string> g_animParamEnumToStringMap;
...
//////////////////////////////////////////////////////////////////////////
bool CMovieSystem::Load(tukk pszFile, tukk pszMission)
{
...
//////////////////////////////////////////////////////////////////////////
IAnimSequence* CMovieSystem::CreateSequence(tukk pSequenceName, bool bLoad, u32 id)
{
	if (!bLoad)
	{
		id = m_nextSequenceId++;
...
//////////////////////////////////////////////////////////////////////////
IAnimSequence* CMovieSystem::LoadSequence(tukk pszFilePath)
{
...
IAnimSequence* CMovieSystem::LoadSequence(XmlNodeRef& xmlNode, bool bLoadEmpty)
{
	...
	tukk pFullName = pSequence->GetName();
...
//////////////////////////////////////////////////////////////////////////
IAnimSequence* CMovieSystem::FindSequence(tukk pSequenceName) const
{
	...
	for (Sequences::const_iterator it = m_sequences.begin(); it != m_sequences.end(); ++it)
	{
		...
		tukk fullname = pCurrentSequence->GetName();
...
//////////////////////////////////////////////////////////////////////////
IAnimSequence* CMovieSystem::FindSequenceById(u32 id) const
{
...
//////////////////////////////////////////////////////////////////////////
IAnimSequence* CMovieSystem::FindSequenceByGUID(DrxGUID guid) const
{
	for (Sequences::const_iterator it = m_sequences.begin(); it != m_sequences.end(); ++it)
	{
		IAnimSequence* pCurrentSequence = *it;
...
IAnimSequence* CMovieSystem::GetSequence(i32 i) const
{
...
i32 CMovieSystem::GetNumSequences() const
{
	return m_sequences.size();
}

IAnimSequence* CMovieSystem::GetPlayingSequence(i32 i) const
{
...
i32 CMovieSystem::GetNumPlayingSequences() const
{
...
void CMovieSystem::PlaySequence(tukk pSequenceName, IAnimSequence* pParentSeq, bool bResetFx, bool bTrackedSequence, SAnimTime startTime, SAnimTime endTime)
{
...
bool CMovieSystem::StopSequence(tukk pSequenceName)
{
	IAnimSequence* pSequence = FindSequence(pSequenceName);
...
void CMovieSystem::UpdateInternal(const float deltaTime, const bool bPreUpdate)
{
	...
	for (size_t i = 0; i < numPlayingSequences; ++i)
	{
		...
		i32 nSeqFlags = playingSequence.sequence->GetFlags();
		...
		{
			i32 seqFlags = playingSequence.sequence->GetFlags();
	...
	for (i32 i = 0; i < (i32)stopSequences.size(); i++)
	{
...
void CMovieSystem::Serialize(XmlNodeRef& xmlNode, bool bLoading, bool bRemoveOldNodes, bool bLoadEmpty)
{
	if (bLoading)
	{
		...
		if (seqNode)
		{
			...
			for (i32 i = 0; i < seqNode->getChildCount(); i++)
			{
	...
	else
	{
		...
		for (i32 i = 0; i < GetNumSequences(); ++i)
		{
...
void CMovieSystem::SendGlobalEvent(tukk pszEvent)
{
	if (m_pUser)
	{
...
{
	i32 numSequences = gEnv->pMovieSystem->GetNumSequences();

	for (i32 i = 0; i < numSequences; i++)
	{
		IAnimSequence* pSeq = gEnv->pMovieSystem->GetSequence(i);

		if (pSeq)
		{
			DrxLogAlways("%s", pSeq->GetName());
		}
	}
}
#endif //#if !defined(_RELEASE)
...
{
	tukk sequenceName = pArgs->GetArg(1);
...
void CMovieSystem::ShowPlayedSequencesDebug()
{
	...
	for (Sequences::iterator it = m_sequences.begin(); it != m_sequences.end(); ++it)
	{
		...
		tukk szSeqName = currentSequence->GetName();
		...
		if (m_mov_debugSequences == 3)
		{
			...
			for (i32 i = 0; i < currentSequence->GetNodeCount(); ++i)
			{
				...
				tukk szNodeName = pAnimNode->GetName();
...
void CMovieSystem::GoToFrame(tukk seqName, float targetFrame)
{
	assert(seqName != NULL);
	     ...
	     it != m_playingSequences.end(); ++it)
	{
		...
		tukk fullname = ps.sequence->GetName();
...
void CMovieSystem::ControlCapture()
{
	...
	if (m_bStartCapture)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_MOVIE, VALIDATOR_WARNING, "StartCapture failed, can't find a valid Sequence to capture!");
		}
	}
	...
	if (m_bEndCapture)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_MOVIE, VALIDATOR_WARNING, "EndCapture failed, passed sequence doesn't seem to be valid!");
		}
	}
}

void CMovieSystem::SerializeNodeType(EAnimNodeType& animNodeType, XmlNodeRef& xmlNode, bool bLoading, const uint version, i32 flags)
{
	static tukk kType = "Type";

	if (bLoading)
	{
		// Old serialization values that are no longer
		// defined in IMovieSystem.h, but needed for conversion:
		static i32k kOldParticleNodeType = 0x18;
	...
	else
	{
		...
		tukk pTypeString = g_animNodeEnumToStringMap[animNodeType];
...
{
	static tukk kByNameAttrName = "paramIdIsName";
	static tukk kParamUserValue = "paramUserValue";
	...
	if (bLoading)
	{
		...
		{
			static tukk kParamType = "paramType";
	...
	else
	{
		...
		{
			pTypeString = "User";
			xmlNode->setAttr(kParamUserValue, (i32)animParamType.m_type);
		}
		else
		{
...
tukk CMovieSystem::GetParamTypeName(const CAnimParamType& animParamType)
{
	if (animParamType.m_type == eAnimParamType_ByString)
	{
...
ILightAnimWrapper* CMovieSystem::CreateLightAnimWrapper(tukk szName) const
{
...
CLightAnimWrapper::CLightAnimWrapper(tukk szName)
...
CLightAnimWrapper* CLightAnimWrapper::Create(tukk szName)
{
...
CLightAnimWrapper* CLightAnimWrapper::FindLightAnim(tukk name)
{
...
void CLightAnimWrapper::CacheLightAnim(tukk name, CLightAnimWrapper* p)
{
	ms_lightAnimWrapperCache.insert(LightAnimWrapperCache::value_type(string(name), p));
}

void CLightAnimWrapper::RemoveCachedLightAnim(tukk name)
{
	ms_lightAnimWrapperCache.erase(CONST_TEMP_STRING(name));
}

#ifdef MOVIESYSTEM_SUPPORT_EDITING
EAnimNodeType CMovieSystem::GetNodeTypeFromString(tukk pString) const
{
	return stl::find_in_map(g_animNodeStringToEnumMap, pString, eAnimNodeType_Invalid);
}

CAnimParamType CMovieSystem::GetParamTypeFromString(tukk pString) const
{
...
DynArray<EAnimNodeType> CMovieSystem::GetAllNodeTypes() const
{
	...
	for (std::unordered_map<i32, string>::const_iterator iter = g_animNodeEnumToDefaultNameMap.begin(); iter != g_animNodeEnumToDefaultNameMap.end(); ++iter)
	{
...
tukk CMovieSystem::GetDefaultNodeName(EAnimNodeType type) const
{
