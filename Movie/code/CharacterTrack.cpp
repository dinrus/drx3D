...
void CCharacterTrack::SerializeKey(SCharacterKey& key, XmlNodeRef& keyNode, bool bLoading)
{
	if (bLoading)
	{
		...
		drx_strcpy(key.m_animation, str);
...
float CCharacterTrack::GetKeyDuration(i32 key) const
{
	assert(key >= 0 && key < (i32)m_keys.size());
	const float EPSILON = 0.001f;
	if (m_keys[key].m_bLoop)
	{
		SAnimTime lastTime = m_timeRange.end;
		if (key + 1 < (i32)m_keys.size() && m_keys[key + 1].GetAnimDuration() > SAnimTime(0.0f))
		{
