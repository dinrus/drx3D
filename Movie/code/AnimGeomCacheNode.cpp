...
void AddSupportedParams(tukk sName, i32 paramType, EAnimValue valueType)
{
...
CAnimGeomCacheNode::CAnimGeomCacheNode(i32k id)
...
void CAnimGeomCacheNode::Animate(SAnimContext& animContext)
{
	...
	{
		u32k trackCount = NumTracks();

		for (u32 paramIndex = 0; paramIndex < trackCount; ++paramIndex)
		{
			...
			if (pTrack && paramType == eAnimParamType_TimeRanges)
			{
				...
				i32k currentKeyIndex = pTimeRangesTrack->GetActiveKeyIndexForTime(animContext.time);
...
u32 CAnimGeomCacheNode::GetParamCount() const
{
	return AnimGeomCacheNode::s_geomCacheNodeParams.size() + CAnimEntityNode::GetParamCount();
}

CAnimParamType CAnimGeomCacheNode::GetParamType(u32 nIndex) const
{
	u32k numOwnParams = (u32)AnimGeomCacheNode::s_geomCacheNodeParams.size();
	...
	if (pGeomCacheRenderNode)
	{
		...
		for (u32 paramIndex = 0; paramIndex < trackCount; ++paramIndex)
		{
			...
			if (pTrack && paramType == eAnimParamType_TimeRanges)
			{
				...
				i32k currentKeyIndex = pTimeRangesTrack->GetActiveKeyIndexForTime(startTime + SAnimTime(startPrecacheTime));
