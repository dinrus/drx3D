...
#include <drx3D/Movie/GotoTrack.h>
...
void AddSupportedParam(tukk sName, i32 paramId, EAnimValue valueType, i32 flags = 0)
{
...
CAnimSceneNode::CAnimSceneNode(i32k id) : CAnimNode(id)
{
	m_lastCameraKey = -1;
	m_lastEventKey = -1;
	m_lastConsoleKey = -1;
...
u32 CAnimSceneNode::GetParamCount() const
{
	return s_nodeParams.size();
}

CAnimParamType CAnimSceneNode::GetParamType(u32 nIndex) const
{
	if (nIndex < (i32)s_nodeParams.size())
	{
...
{
	for (i32 i = 0; i < (i32)s_nodeParams.size(); i++)
	{
...
void CAnimSceneNode::Activate(bool bActivate)
{
	...
	for (i32 paramIndex = 0; paramIndex < trackCount; paramIndex++)
	{
		...
		for (i32 currKey = 0; currKey < pSequenceTrack->GetNumKeys(); currKey++)
		{
			SSequenceKey key;
...
void CAnimSceneNode::Animate(SAnimContext& animContext)
{
	...
	tukk overrideCamName = gEnv->pMovieSystem->GetOverrideCamName();
	...
	if (overrideCamEntity)    // There is a valid overridden camera.
	{
		...
		{
			SCameraKey key;
			drx_strcpy(key.m_selection, overrideCamName);
	...
	{
		SCameraKey key;
		i32 cameraKey = cameraTrack->GetActiveKey(animContext.time, &key);
		if (cameraKey >= 0)
		{
	...
	{
		SConsoleKey key;
		i32 nConsoleKey = pConsoleTrack->GetActiveKey(animContext.time, &key);
	...
	{
		SSequenceKey key;
		i32 nSequenceKey = pSequenceTrack->GetActiveKey(animContext.time, &key);
		IAnimSequence* pSequence = gEnv->pMovieSystem->FindSequenceByGUID(key.m_sequenceGUID);
	...
	{
		SCaptureKey key;
		i32 nCaptureKey = pCaptureTrack->GetActiveKey(animContext.time, &key);
...
{
	SCameraKey nextKey;
	i32 nextCameraKeyNumber = m_currentCameraTrackKeyNumber + 1;
...
void CAnimSceneNode::ApplySequenceKey(IAnimTrack* pTrack, i32 nPrevKey, i32 nCurrKey, SSequenceKey& key, SAnimContext& animContext)
{
	if (nCurrKey >= 0 && (key.m_sequenceGUID != DrxGUID::Null()))
	{
		IAnimSequence* pSequence = gEnv->pMovieSystem->FindSequenceByGUID(key.m_sequenceGUID);
...
void CAnimSceneNode::ApplyGotoKey(CGotoTrack* poGotoTrack, SAnimContext& animContext)
{
	...
	i32 currentActiveKeyIndex = poGotoTrack->GetActiveKey(animContext.time, &discreteFloadKey);
	...
	for (uint trackIndex = 0; trackIndex < numTracks; ++trackIndex)
	{
		...
		else if (pAnimTrack->GetParameterType() == eAnimParamType_Camera)
		{
			...
			if (fPrecacheCameraTime > 0.f)
			{
				...
				SCameraKey key;
				i32 keyID = pCameraTrack->GetActiveKey(time + SAnimTime(fPrecacheCameraTime), &key);
				...
				if (time < key.m_time && (time + SAnimTime(fPrecacheCameraTime)) > key.m_time && key.m_time > m_lastPrecachePoint)
				{
					...
					if (pCameraEntity != NULL)
					{
						...
						{
							DrxWarning(VALIDATOR_MODULE_MOVIE, VALIDATOR_WARNING, "Could not find animation node for camera %s in sequence %s", key.m_selection, m_pSequence->GetName());
						}
					}
				}
			}
