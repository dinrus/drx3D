...
public:
	...
	template<typename T>
	void AddSupportedParam(tukk sName, EAnimValue eValueType, tukk sControlName, T defaultValue)
	{
		CAnimNode::SParamInfo param;
		param.name = sName;
		param.paramType = eAnimParamType_User + (i32)m_nodeParams.size();
...
CAnimPostFXNode::CAnimPostFXNode(i32k id, CFXNodeDescription* pDesc)
...
CAnimNode* CAnimPostFXNode::CreateNode(i32k id, EAnimNodeType nodeType)
{
	...
	if (bLoading)
	{
		...
		if (paramIdVersion <= 2)
		{
			...
			for (i32 i = 0; i < num; ++i)
			{
				...
				static u32k OLD_APARAM_USER = 100;
...
u32 CAnimPostFXNode::GetParamCount() const
{
	return m_pDescription->m_nodeParams.size();
}

CAnimParamType CAnimPostFXNode::GetParamType(u32 nIndex) const
{
