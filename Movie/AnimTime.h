...
struct SAnimTime
{
	static i32k numTicksPerSecond = 6000;
	...
	SAnimTime() : m_ticks(0) {}
	explicit SAnimTime(i32 ticks) : m_ticks(ticks) {}
	...
	static tukk GetFrameRateName(EFrameRate frameRate)
	{
		tukk frameRateNames[eFrameRate_Num] =
		{
			// Common
	...
	bool  Serialize(Serialization::IArchive& ar, tukk szName, tukk szLabel);

	//! Helper to serialize from ticks or old float time.
	void Serialize(XmlNodeRef keyNode, bool bLoading, tukk pName, tukk pLegacyName)
	{
		if (bLoading)
		{
			i32 ticks = 0;
	...
	i32            GetTicks() const              { return m_ticks; }
	void             SetTicks(i32 ticks)         { m_ticks = ticks; }

	void             SetTime(float time)           { m_ticks = static_cast<i32>(do_lround(static_cast<double>(time) * numTicksPerSecond)); }

	static SAnimTime Min()                         { SAnimTime minTime; minTime.m_ticks = std::numeric_limits<i32>::lowest(); return minTime; }
	static SAnimTime Max()                         { SAnimTime maxTime; maxTime.m_ticks = (std::numeric_limits<i32>::max)(); return maxTime; }
	...
	SAnimTime        operator+(SAnimTime r) const  { SAnimTime temp = *this; temp.m_ticks += r.m_ticks; return temp; }
	SAnimTime        operator*(SAnimTime r) const  { SAnimTime temp = *this; temp.m_ticks *= r.m_ticks; return temp; }
	SAnimTime        operator/(SAnimTime r) const  { SAnimTime temp; temp.m_ticks = static_cast<i32>((static_cast<int64>(m_ticks) * numTicksPerSecond) / r.m_ticks); return temp; }
	SAnimTime        operator%(SAnimTime r) const  { SAnimTime temp = *this; temp.m_ticks %= r.m_ticks; return temp; }
	SAnimTime        operator*(float r) const      { SAnimTime temp; temp.m_ticks = static_cast<i32>(do_lround(static_cast<double>(m_ticks) * r)); return temp; }
	SAnimTime        operator/(float r) const      { SAnimTime temp; temp.m_ticks = static_cast<i32>(do_lround(static_cast<double>(m_ticks) / r)); return temp; }
	...
	{
		i32k sign = drxmath::sign(m_ticks);
		i32k absTicks = abs(m_ticks);

		i32k framesMod = numTicksPerSecond / GetFrameRateValue(frameRate);
		i32k remainder = absTicks % framesMod;
	...
	{
		i32k framesMod = numTicksPerSecond / GetFrameRateValue(frameRate);
		i32k remainder = m_ticks % framesMod;
...
private:
	...
	friend bool         Serialize(Serialization::IArchive& ar, SAnimTime& animTime, tukk name, tukk label);
...
inline void STimeCode::FromTicks(i32 ticks)
{
	i32k ticksPerFrame = SAnimTime::numTicksPerSecond / fps;
...
inline i32 STimeCode::ToTicks()
{
	i32k tps = SAnimTime::numTicksPerSecond;
	i32k ticksPerFrame = SAnimTime::numTicksPerSecond / fps;
...
inline bool SAnimTime::Serialize(Serialization::IArchive& ar, tukk szName, tukk szLabel)
{
	if (!ar.isEdit())
	{
	...
	else
	{
		...
		i32k fps = GetFrameRateValue(context.fps);
		if (ar.isInput())
		{
			...
			case EDisplayMode::Frames:
				{
					i32k ticksPerFrame = numTicksPerSecond / fps;
					i32 frames = 0;
		...
		else
		{
			...
			case EDisplayMode::Frames:
				{
					i32k ticksPerFrame = numTicksPerSecond / fps;
					i32 frames = GetTicks() / ticksPerFrame;
					ar(frames, "frames", szLabel);
				}
				break;
			}
...
inline bool Serialize(Serialization::IArchive& ar, SAnimTime& animTime, tukk szName, tukk szLabel)
{
