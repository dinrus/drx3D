...
//! If you want to expand drx3DMovie to control new stuff this is probably the enum you want to change.
...
public:
	...
	//! Convert from old enum or i32.
	void operator=(i32k type)
	{
...
struct SAnimContext
{
	...
	EntityId       m_activeCameraEntity; //!< Used for editor to pass viewport camera to drx3DMovie.

	// TODO: Replace trackMask with something more type safe
	// TODO: Mask should be stored with dynamic length
	u32    trackMask;  //!< To update certain types of tracks only
...
struct SSequenceAudioTrigger
{
	SSequenceAudioTrigger()
		: m_onStopTrigger(DrxAudio::InvalidControlId)
		, m_onPauseTrigger(DrxAudio::InvalidControlId)
		, m_onResumeTrigger(DrxAudio::InvalidControlId)
	{}

	void Serialize(XmlNodeRef xmlNode, bool bLoading);
	void Serialize(Serialization::IArchive& ar);

	DrxAudio::ControlId m_onStopTrigger;
	DrxAudio::ControlId m_onPauseTrigger;
	DrxAudio::ControlId m_onResumeTrigger;
...
{
	eTDT_Void  = drx_variant::get_index<SMovieSystemVoid, TMovieSystemValue>::value,
	eTDT_Float = drx_variant::get_index<float, TMovieSystemValue>::value,
	eTDT_Vec3  = drx_variant::get_index<Vec3, TMovieSystemValue>::value,
	eTDT_Vec4  = drx_variant::get_index<Vec4, TMovieSystemValue>::value,
	eTDT_Quat  = drx_variant::get_index<Quat, TMovieSystemValue>::value,
	eTDT_Bool  = drx_variant::get_index<bool, TMovieSystemValue>::value,
};
...
struct IMovieUser
{
	...
	virtual void BeginCutScene(IAnimSequence* pSeq, u64 dwFlags, bool bResetFX) = 0;
	//! Called when movie system exits from cut-scene mode.
	virtual void EndCutScene(IAnimSequence* pSeq, u64 dwFlags) = 0;
	//! Called when movie system wants to send a global event.
	virtual void SendGlobalEvent(tukk pszEvent) = 0;
...
struct IAnimTrack : public _i_reference_target_t
{
	...
	virtual i32 GetSubTrackCount() const = 0;

	//! Retrieve pointer the specfied sub track.
	virtual IAnimTrack* GetSubTrack(i32 nIndex) const = 0;
	virtual tukk GetSubTrackName(i32 nIndex) const = 0;
	virtual void        SetSubTrackName(i32 nIndex, tukk name) = 0;
	//////////////////////////////////////////////////////////////////////////
	...
	// TODO: Mask should be stored with dynamic length.
	virtual bool IsMasked(u32k mask) const = 0;
	...
	virtual i32 CreateKey(SAnimTime time) = 0;

	//! Gets the key type as a string
	virtual tukk GetKeyType() const = 0;
	...
	virtual _smart_ptr<IAnimKeyWrapper> GetWrappedKey(i32 key) = 0;
	...
	virtual i32 NextKeyByTime(i32 key) const { if (key + 1 < GetNumKeys()) { return key + 1; } else { return -1; } }

	//! Get the animation layer index assigned. (only for character/look-at tracks ATM).
	virtual i32 GetAnimationLayerIndex() const { return -1; }

	//! Set the animation layer index. (only for character/look-at tracks ATM).
	virtual void SetAnimationLayerIndex(i32 index) {}
...
struct IAnimEntityNode
{
	...
	virtual void SetEntityId(i32k entityId) {}
...
public:
	...
	virtual CAnimParamType GetParamType(u32 nIndex) const = 0;
	...
	virtual tukk GetParamName(const CAnimParamType& paramType) const = 0;
	...
	virtual i32 GetTrackCount() const = 0;

	//! Return track assigned to the specified parameter.
	virtual IAnimTrack* GetTrackByIndex(i32 nIndex) const = 0;
	...
	virtual IAnimTrack* GetTrackForParameter(const CAnimParamType& paramType, u32 index) const = 0;

	//! Get the index of a given track among tracks with the same parameter type in this node.
	virtual u32 GetTrackParamIndex(const IAnimTrack* pTrack) const = 0;
...
struct ITrackEventListener
{
	...
	virtual void OnTrackEvent(IAnimSequence* pSequence, i32 reason, tukk event, uk pUserData) = 0;
	// </interfuscator:shuffle>
};
...
{
	static i32k kSequenceVersion = 4;
	...
	virtual i32 GetCutSceneFlags(const bool localFlags = false) const = 0;
	...
	virtual i32        GetNodeCount() const = 0;
	//! Get animation node at specified index.
	virtual IAnimNode* GetNode(i32 index) const = 0;
	...
	virtual IAnimNode* FindNodeByName(tukk sNodeName, const IAnimNode* pParentDirector) = 0;
	...
	virtual void Serialize(XmlNodeRef& xmlNode, bool bLoading, bool bLoadEmptyTracks = true, u32 overrideId = 0, bool bResetLightAnimSet = false) = 0;

	//! Adds/removes track events in sequence.
	virtual bool AddTrackEvent(tukk szEvent) = 0;
	virtual bool RemoveTrackEvent(tukk szEvent) = 0;
	virtual bool RenameTrackEvent(tukk szEvent, tukk szNewEvent) = 0;
	virtual bool MoveUpTrackEvent(tukk szEvent) = 0;
	virtual bool MoveDownTrackEvent(tukk szEvent) = 0;
	virtual void ClearTrackEvents() = 0;

	//! Gets the track events in the sequence.
	virtual i32 GetTrackEventsCount() const = 0;

	//! Gets the specified track event in the sequence.
	virtual char const* GetTrackEvent(i32 iIndex) const = 0;

	//! Called to trigger a track event.
	virtual void TriggerTrackEvent(tukk event, tukk param = NULL) = 0;
...
struct IMovieEngineModule : public Drx::IDefaultModule
{
	DRXINTERFACE_DECLARE_GUID(IMovieEngineModule, "271a9f97-7e6d-4cfa-b3ae-2a5c3227d302"_drx_guid);
};
//! \endcond
...
struct IMovieSystem
{
	...
	virtual bool           Load(tukk pszFile, tukk pszMission) = 0;

	virtual IAnimSequence* CreateSequence(tukk sequence, bool bLoad = false, u32 id = 0) = 0;
	...
	virtual IAnimSequence* FindSequence(tukk sequence) const = 0;
	virtual IAnimSequence* FindSequenceById(u32 id) const = 0;
	virtual IAnimSequence* FindSequenceByGUID(DrxGUID guid) const = 0;
	virtual IAnimSequence* GetSequence(i32 i) const = 0;
	virtual i32            GetNumSequences() const = 0;
	virtual IAnimSequence* GetPlayingSequence(i32 i) const = 0;
	virtual i32            GetNumPlayingSequences() const = 0;
	virtual bool           IsCutScenePlaying() const = 0;

	virtual u32         GrabNextSequenceId() = 0;
	...
	virtual void PlaySequence(tukk pSequenceName, IAnimSequence* pParentSeq, bool bResetFX, bool bTrackedSequence,
	...
	virtual bool StopSequence(tukk pSequenceName) = 0;
	...
	virtual void GoToFrame(tukk seqName, float targetFrame) = 0;

	//! Get the name of camera used for sequences instead of cameras specified in the director node.
	virtual tukk GetOverrideCamName() const = 0;
	...
	virtual ILightAnimWrapper* CreateLightAnimWrapper(tukk szName) const = 0;
	...
	virtual tukk GetParamTypeName(const CAnimParamType& animParamType) = 0;

#ifdef MOVIESYSTEM_SUPPORT_EDITING
	virtual EAnimNodeType  GetNodeTypeFromString(tukk pString) const = 0;
	virtual CAnimParamType GetParamTypeFromString(tukk pString) const = 0;

	//! Get all node types
	virtual DynArray<EAnimNodeType> GetAllNodeTypes() const = 0;

	// Get defualt node name for this type
	virtual tukk GetDefaultNodeName(EAnimNodeType type) const = 0;
#endif
...
inline void SSequenceAudioTrigger::Serialize(XmlNodeRef xmlNode, bool bLoading)
{
	if (bLoading)
	{
		if (xmlNode->getAttr("onStopAudioTrigger"))
		{
			...
			m_onStopTrigger = DrxAudio::StringToId(m_onStopTriggerName.c_str());
		}
		if (xmlNode->getAttr("onPauseAudioTrigger"))
		{
			m_onPauseTriggerName = xmlNode->getAttr("onPauseAudioTrigger");
			m_onPauseTrigger = DrxAudio::StringToId(m_onPauseTriggerName.c_str());
		}
		if (xmlNode->getAttr("onResumeAudioTrigger"))
		{
			m_onResumeTriggerName = xmlNode->getAttr("onResumeAudioTrigger");
			m_onResumeTrigger = DrxAudio::StringToId(m_onResumeTriggerName.c_str());
		}

	}
	else
	{
		if (m_onStopTrigger != DrxAudio::InvalidControlId)
		{
			xmlNode->setAttr("onStopAudioTrigger", m_onStopTriggerName.c_str());
		}
		if (m_onPauseTrigger != DrxAudio::InvalidControlId)
		{
			xmlNode->setAttr("onPauseAudioTrigger", m_onPauseTriggerName.c_str());
		}
		if (m_onResumeTrigger != DrxAudio::InvalidControlId)
		{
...
inline void SSequenceAudioTrigger::Serialize(Serialization::IArchive& ar)
{
	if (ar.isInput())
	{
		...
		{
			m_onStopTrigger = DrxAudio::StringToId(stopTriggerName.c_str());
		}
		...
		{
			m_onPauseTrigger = DrxAudio::StringToId(pauseTriggerName.c_str());
		}
		...
		{
			m_onResumeTrigger = DrxAudio::StringToId(resumeTriggerName.c_str());
		}
	}
	else
	{
