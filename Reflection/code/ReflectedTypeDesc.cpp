...
CReflectedTypeDesc::CReflectedTypeDesc(const DrxGUID& guid, const DrxTypeDesc& typeDesc)
	: m_typeDesc(typeDesc)
	, m_guid(guid)
{
	m_category = ETypeCategory::Unset;
	...
	else if (typeDesc.IsClass())
	{
		...
		m_classMembers.baseTypesByIndex = std::vector<DrxTypeId>();
...
bool CReflectedTypeDesc::AddBaseType(DrxTypeId typeId, const SSourceFileInfo& srcPos)
{
	DRX_ASSERT_MESSAGE(IsClass(), "Base can only be added to class types.");
	if (IsClass())
	{
		auto compare = [typeId](DrxTypeId id) -> bool
		{
			return (id == typeId);
		};

		DRX_ASSERT_MESSAGE(std::find_if(m_classMembers.baseTypesByIndex.begin(), m_classMembers.baseTypesByIndex.end(), compare) == m_classMembers.baseTypesByIndex.end(), "Type already registered as base.");
...
IFunctionDesc* CReflectedTypeDesc::AddFunction(const CMemberFunction& memberFunc, tukk szLabel, const DrxGUID& guid, const SSourceFileInfo& srcPos)
{
	DRX_ASSERT_MESSAGE(IsClass(), "Functions can only be added to class types.");
	if (IsClass())
	{
		...
		DRX_ASSERT_MESSAGE(isGuidUnused, "Function guid already used.");
		if (isGuidUnused)
		{
...
IVariableDesc* CReflectedTypeDesc::AddVariable(DrxTypeId typeId, ptrdiff_t offset, tukk szLabel, const DrxGUID& guid, const SSourceFileInfo& srcPos)
{
	DRX_ASSERT_MESSAGE(IsClass(), "Variables can only be added to class types.");
	if (IsClass())
	{
		...
		DRX_ASSERT_MESSAGE(std::find_if(m_classMembers.variablesByIndex.begin(), m_classMembers.variablesByIndex.end(), compare) == m_classMembers.variablesByIndex.end(), "Variable already registered.");
...
IEnumValueDesc* CReflectedTypeDesc::AddEnumValue(tukk szLabel, size_t value, tukk szDescription)
{
	DRX_ASSERT_MESSAGE(IsEnum(), "Enum values can only be added to enum types.");
	if (IsEnum())
	{
		// TODO: Decide if we should always check.
		...
		DRX_ASSERT_MESSAGE(std::find_if(m_enumMembers.valuesByIndex.begin(), m_enumMembers.valuesByIndex.end(), compare) == m_enumMembers.valuesByIndex.end(), "Name '%s' with value '%d' already exists.", szLabel, value);
		// ~TODO
...
bool CReflectedTypeDesc::AddConversionOperator(DrxTypeId typeId, Type::CConversionOperator conversionOperator)
{
	const bool isNotRegistered = (m_conversionsByTypeId.find(typeId) == m_conversionsByTypeId.end());
	DRX_ASSERT_MESSAGE(isNotRegistered, "Conversion already registered.");
	if (isNotRegistered)
	{
...
Type::CConversionOperator CReflectedTypeDesc::GetConversionOperator(DrxTypeId typeId)
{
