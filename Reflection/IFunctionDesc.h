...
enum class EFunctionParamFlags : u32
{
	Empty       = 0,
	IsReference = BIT(0),
	IsConst     = BIT(1),
...
struct IFunctionDesc : public IExtensibleDesc
{
	...
	virtual DrxTypeId                     GetReturnTypeId() const = 0;
...
{
	SFunctionParameterDesc(DrxTypeId _typeId, u32 _index)
		: typeId(_typeId)
		, index(_index)
	{}
...
public:
	...
	const DrxTypeId   GetReturnTypeId() const { return m_returnTypeId; }
...
struct ParamIterator
{
	...
	{
		params.emplace_back(Drx::TypeIdOf<typename FunctionDesc::template ParamType<INDEX>>(), INDEX);
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 0>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 0, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 1>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 1 && pParam0, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 2>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 2 && pParam0 && pParam1, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 3>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 3 && pParam0 && pParam1 && pParam2, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 4>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 4 && pParam0 && pParam1 && pParam2 && pParam3, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 5>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 5 && pParam0 && pParam1 && pParam2 && pParam3 && pParam4, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 6>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 6 && pParam0 && pParam1 && pParam2 && pParam3 && pParam4 && pParam5, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 7>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 7 && pParam0 && pParam1 && pParam2 && pParam3 && pParam4 && pParam5 && pParam6, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 8>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 8 && pParam0 && pParam1 && pParam2 && pParam3 && pParam4 && pParam5 && pParam6 && pParam7, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 9>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 9 && pParam0 && pParam1 && pParam2 && pParam3 && pParam4 && pParam5 && pParam6 && pParam7 && pParam8, "Parameter missmatch.");
...
struct SProxy<FUNCTION_DESC, RETURN_TYPE, 10>
{
	static void Execute(const CFunctionPtr& funcPtr, CFunctionExecutionContext& context)
	{
		...
		DRX_ASSERT_MESSAGE(context.GetParamCount() == 10 && pParam0 && pParam1 && pParam2 && pParam3 && pParam4 && pParam5 && pParam6 && pParam7 && pParam8 && pParam9, "Parameter missmatch.");
