...
public:
	...
	public:
		...
		constexpr Token(NodeType type, STxtview view) :
			type(type),
			view(view) {
		}
	...
	public:
		 ...
		 */
		STxt GetIndents(int8_t indent) const {
			return STxt(spacesPerIndent * indent, ' ');
		}
	...
	virtual void ParseString(Node &node, STxtview string) = 0;
	...
	void ParseStream(Node &node, std::basic_istream<_Elem> &stream) {
...
#ifndef DRX3D_BUILD_MSVC
			throw drx::Exc("Cannot dynamicly parse wide streams on GCC or Clang");
#else
		...
		STxt s(std::istreambuf_iterator<_Elem>(stream), {});
...
public:
	void ParseString(Node &node, STxtview string) override {
		T::Load(node, string);
	}
