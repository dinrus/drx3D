...
void Json::Load(Node &node, STxtview string) {
...
void Json::AddToken(STxtview view, std::vector<Token> &tokens) {
	if (view.length() != 0) {
		// Finds the node value type of the string and adds it to the tokens vector.
		if (view == "null") {
			tokens.emplace_back(NodeType::Null, STxtview());
...
void Json::Convert(Node &current, const std::vector<Token> &tokens, int32_t &k) {
	if (tokens[k] == Token(NodeType::Token, "{")) {
		...
		while (tokens[k] != Token(NodeType::Token, "}")) {
			...
			if (k + 2 >= tokens.size())
				throw drx::Exc("Missing end of {} array");
			if (tokens[k + 1].view != ":")
				throw drx::Exc("Missing object colon");
			k += 2;
...
#endif
				Convert(current.AddProperty(STxt(key)), tokens, k);
			if (tokens[k].view == ",")
				k++;
		}
		k++;
		...
		while (tokens[k] != Token(NodeType::Token, "]")) {
			if (k >= tokens.size())
				throw drx::Exc("Missing end of [] object");
