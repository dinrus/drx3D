...
namespace drx3d {
void Xml::Load(Node &node, STxtview string) {
	...
	for (const auto [index, c] : Enumerate(string)) {
		...
		if (quoteState == QuoteState::None) {
			if (tagState == TagState::Open && String::IsWhitespace(c)) {
				...
				AddToken(STxtview(string.data() + tokenStart, index - tokenStart), tokens);
				tokenStart = index + 1;
			} else if (c == '<' || c == '>' || (tagState == TagState::Open && (c == '?' || c == '!' || c == '=' || c == '/'))) {
				// Tokens used to read XML nodes.
				AddToken(STxtview(string.data() + tokenStart, index - tokenStart), tokens);
				tokens.emplace_back(NodeType::Token, STxtview(string.data() + index, 1));
				if (c == '<')
	...
	if (tokens.empty())
		throw drx::Exc("No tokens found in document");
...
void Xml::AddToken(STxtview view, std::vector<Token> &tokens) {
...
void Xml::Convert(Node &current, const std::vector<Token> &tokens, int32_t &k) {
	...
	if (tokens[k] == Token(NodeType::Token, "!") && (tokens[k + 1].view.find("--") == 0)) {
		k += 2;
		while (tokens[k + 1] != Token(NodeType::Token, ">") && (tokens[k].view.find("--") == STxt::npos))
			k++;
		k += 2;
	...
	while (tokens[k] != Token(NodeType::Token, ">")) {
		...
		if (tokens[k] == Token(NodeType::Token, "=")) {
			property.AddProperty(AttributePrefix + STxt(tokens[k - 1].view)) = tokens[k + 1].view.substr(1, tokens[k + 1].view.size() - 2);
			k++;
		}
		k++;
	}
...
Node &Xml::CreateProperty(Node &current, const STxt &name) {
...
void Xml::AppendData(const STxt &nodeName, const Node &node, std::ostream &stream, Format format, int32_t indent) {
