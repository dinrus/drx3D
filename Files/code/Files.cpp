...
Files::Files() {
	...
	if (std::filesystem::exists(DRX3D_RESOURCES_DEV))
		AddSearchPath(STxt(DRX3D_RESOURCES_DEV));
}
...
void Files::AddSearchPath(const STxt &path) {
...
void Files::RemoveSearchPath(const STxt &path) {
...
std::optional<STxt> Files::Read(const std::filesystem::path &path) {
	auto pathStr = path.string();
	std::replace(pathStr.begin(), pathStr.end(), '\\', '/');
	...
	return STxt(data.begin(), data.end());
}

std::vector<u8> Files::ReadBytes(const std::filesystem::path &path) {
	IFStream file(path);
	file >> std::noskipws;
	...
	std::copy(std::istream_iterator<u8>(file),
...
std::vector<STxt> Files::FilesInPath(const std::filesystem::path &path, bool recursive) {
	auto pathStr = path.string();
	std::replace(pathStr.begin(), pathStr.end(), '\\', '/');
	auto rc = PHYSFS_enumerateFiles(pathStr.c_str());

	std::vector<STxt> files;
...
std::istream &Files::SafeGetLine(std::istream &is, STxt &t) {
	t.clear();
