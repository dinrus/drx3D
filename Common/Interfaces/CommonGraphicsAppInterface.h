...
struct CommonGraphicsApp
{
	...
	virtual void dumpNextFrameToPng(tukk pngFilename) {}
	...
	virtual i32 getUpAxis() const = 0;

	virtual void swapBuffer() = 0;
	virtual void drawText(tukk txt, i32 posX, i32 posY)
	{
		float size = 1;
	...
	virtual void drawText(tukk txt, i32 posX, i32 posY, float size)
	{
		float colorRGBA[4] = { 0, 0, 0, 1 };
		drawText(txt, posX, posY, size, colorRGBA);
	}
	virtual void drawText(tukk txt, i32 posX, i32 posY, float size, float colorRGBA[4]) = 0;
	virtual void drawText3D(tukk txt, float posX, float posY, float posZ, float size) = 0;
	virtual void drawText3D(tukk txt, float position[3], float orientation[4], float color[4], float size, i32 optionFlag) = 0;
	virtual void drawTexturedRect(float x0, float y0, float x1, float y1, float color[4], float u0, float v0, float u1, float v1, i32 useRGBA) = 0;
	virtual i32 registerCubeShape(float halfExtentsX, float halfExtentsY, float halfExtentsZ, i32 textureIndex = -1, float textureScaling = 1) = 0;
	virtual i32 registerGraphicsUnitSphereShape(EnumSphereLevelOfDetail lod, i32 textureId = -1) = 0;

	virtual void registerGrid(i32 xres, i32 yres, float color0[4], float color1[4]) = 0;
	...
	//	void defaultKeyboardCallback(i32 key, i32 state)
	//	{
	//	}
