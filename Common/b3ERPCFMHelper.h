#ifndef D3_ERPCFM_HELPER
#define D3_ERPCFM_HELPER
...
public:
	 ...
	 *
	 * There is a mechanism to reduce joint ошибка: during each simulation step each joint applies a special force to bring its bodies back into correct alignment. This force is controlled by the error reduction parameter (ERP), which has a value between 0 and 1.
	 *
	 ...
	 * @return
	 */
	static Scalar getERP(Scalar timeStep, Scalar kSpring,
						   Scalar kDamper)
	{
	 ...
	 * @return
	 */
	static Scalar getCFM(Scalar avoidSingularity, Scalar timeStep, Scalar kSpring,
						   Scalar kDamper)
	{
		return Scalar(avoidSingularity) / (timeStep * kSpring + kDamper);
	}
};

#endif /* D3_ERPCFM_HELPER */
