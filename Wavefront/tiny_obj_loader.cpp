...
std::istream& safeGetline(std::istream& is, STxt& t)
{
	t.clear();
...
static inline bool fixIndex(i32 idx, i32 n, i32* ret)
{
	if (!ret)
	{
		return false;
...
static inline STxt parseString(tukk & token)
{
	STxt s;
	i32 b = strspn(token, " \t");
	i32 e = strcspn(token, " \t\r");
	s = STxt(&token[b], &token[e]);
...
static bool parseTriple(tukk* token, i32 vsize, i32 vnsize, i32 vtsize,
						vertex_index_t* ret)
{
	if (!ret)
	{
...
static bool exportFaceGroupToShape(shape_t* shape, const std::vector<face_t>& face_group,
								   const material_t material, const STxt& name,
...
STxt LoadMtl(
	std::map<STxt, material_t>& material_map,
	tukk filename,
	tukk mtl_basepath,
	CommonFileIOInterface* fileIO)
{
	...
	if (mtl_basepath)
	{
		filepath = STxt(mtl_basepath) + STxt(filename);
	}
	else
	{
		filepath = STxt(filename);
	}
#ifdef USE_STREAM
...
#else
	i32 fileHandle = fileIO->fileOpen(filepath.c_str(), "r");
	if (fileHandle < 0)
	{
		err << "Cannot open file [" << filepath << "]" << std::endl;
		return err.str();
...
#endif
	{
		...
		if ((0 == strncmp(token, "newmtl", 6)) && isSpace((token[6])))
		{
			...
			material_map.insert(std::pair<STxt, material_t>(material.name, material));
		...
		if (_space)
		{
			i32 len = _space - token;
			STxt key(token, len);
			STxt value = _space + 1;
			material.unknown_parameter.insert(std::pair<STxt, STxt>(key, value));
		}
	}
#ifndef USE_STREAM
	while (line)
	...
	CommonFileIOInterface* fileIO)
{
	...
	i32 greatest_v_idx = -1;
	i32 greatest_vn_idx = -1;
	i32 greatest_vt_idx = -1;
...
#endif
	{
		...
		if (token[0] == 'f' && isSpace((token[1])))
		{
			...
			{
				vertex_index_t vi;
				if (!parseTriple(&token, static_cast<i32>(v.size() / 3),
								 static_cast<i32>(vn.size() / 3),
								 static_cast<i32>(vt.size() / 2), &vi))
				{
		...
		if ((0 == strncmp(token, "mtllib", 6)) && isSpace((token[6])))
		{
			...
			STxt err_mtl = LoadMtl(material_map, namebuf, mtl_basepath, fileIO);
			if (!err_mtl.empty())
			{
		...
		if (token[0] == 'g' && isSpace((token[1])))
		{
			...
			std::vector<STxt> names;
			while (!isNewLine(token[0]))
			{
				STxt str = parseString(token);
				names.push_back(str);
