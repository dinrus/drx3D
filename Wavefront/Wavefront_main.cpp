	...
	bool verbose)
{
	...
	STxt err = bt_tinyobj::LoadObj(shapes, fullPath, prefix[index]);
	...
	if (verbose)
	{
		for (size_t i = 0; i < shapes.size(); i++)
		{
			...
			std::map<STxt, STxt>::iterator it(shapes[i].material.unknown_parameter.begin());
			std::map<STxt, STxt>::iterator itEnd(shapes[i].material.unknown_parameter.end());
			for (; it != itEnd; it++)
			{
