...
// See Also
//   IVehicleAnimation
typedef i32 TVehicleAnimStateId;
...
{
	i32 typeId;
	SVehicleMovementStatus(i32 type = eVMS_invalid) : typeId(type) {}
};
...
struct SVehicleImpls
{
	...
	i32    Count() const           { return vImpls.size(); }

	string GetAt(i32 idx) const
	{
		if (idx >= 0 && idx < (i32)vImpls.size())
			return vImpls[idx];
		return "";
	}
private:
...
struct SExhaustParams
{
	...
	virtual IVehicleHelper* GetHelper(i32 idx) const = 0;

	virtual tukk     GetStartEffect() const = 0;
	virtual tukk     GetStopEffect() const = 0;
	virtual tukk     GetRunEffect() const = 0;
	virtual tukk     GetBoostEffect() const = 0;
...
struct SEnvironmentLayer
{
	...
	virtual size_t          GetWheelCount(i32 group) const = 0;
	virtual i32             GetWheelAt(i32 group, i32 wheel) const = 0;

	virtual bool            IsGroupActive(i32 group) const = 0;
	virtual void            SetGroupActive(i32 group, bool active) = 0;
...
struct SEnvironmentParticles
{
	...
	virtual const SEnvironmentLayer& GetLayer(i32 idx) const = 0;
	virtual tukk              GetMFXRowName() const = 0;
};
...
struct SParticleParams
{
	...
	virtual const SDamageEffect*   GetDamageEffect(tukk pName) const = 0;
...
struct SVehicleSoundInfo
{
	...
	IVehicleHelper*     pHelper;
	DrxAudio::ControlId soundId;
	bool                isEngineSound;

	SVehicleSoundInfo() : isEngineSound(false), pHelper(NULL)
	{
		soundId = DrxAudio::InvalidControlId;
	}
};
typedef i32 TVehicleSoundEventId;
	...
	: public IVehicleObject
{
	...
	virtual i32  OnEvent(i32 eventType, SVehicleEventParams& eventParams) = 0;
...
struct IVehicle : public IGameObjectExtension
{
	...
	virtual void                        OnAction(const TVehicleActionId actionId, i32 activationMode, float value, EntityId callerId) = 0;
	...
	virtual void                        SetAmmoCount(IEntityClass* pAmmoType, i32 amount) = 0;
	...
	virtual i32                         IsUsable(EntityId userId) = 0;
	virtual bool                        OnUsed(EntityId userId, i32 index) = 0;

	virtual bool                        AddSeat(const SmartScriptTable& seatParams) = 0;
	virtual bool                        AddHelper(tukk name, Vec3 position, Vec3 direction, IVehiclePart* pPart) = 0;

	virtual i32                         GetPartCount() = 0;
	virtual IVehiclePart*               GetPart(u32 index) = 0;
	virtual void                        GetParts(IVehiclePart** parts, i32 nMax) = 0;

	virtual i32                         GetComponentCount() const = 0;
	virtual IVehicleComponent*          GetComponent(i32 index) = 0;
	virtual IVehicleComponent*          GetComponent(tukk name) = 0;

	virtual IVehicleAnimationComponent& GetAnimationComponent() = 0;

	virtual IVehicleAnimation*          GetAnimation(tukk name) = 0;

	virtual i32                         GetActionCount() = 0;
	virtual IVehicleAction*             GetAction(i32 index) = 0;

	// get the current movement controller
	virtual IMovementController* GetMovementController() = 0;

	virtual IFireController*     GetFireController(u32 controllerNum = 0) = 0;
	...
	virtual i32 GetWeaponCount() const = 0;

	// Get EntityId by weapon index
	virtual EntityId GetWeaponId(i32 index) const = 0;
	...
	virtual bool              HasHelper(tukk pName) = 0;

	virtual IVehiclePart*     GetPart(tukk name) = 0;
	...
	virtual i32           GetWheelCount() = 0;

	virtual IVehiclePart* GetWheelPart(i32 idx) = 0;
	...
	// See Also
	//   UnregisterVehicleEventListener
	virtual void RegisterVehicleEventListener(IVehicleEventListener* pEvenListener, tukk name) = 0;
	...
	virtual IVehicleHelper* GetHelper(tukk pName) = 0;
	...
	virtual void NeedsUpdate(i32 flags = 0, bool bThreadSafe = false) = 0;

	// Summary:
	//  Register Entity Timer. If timerId == -1, timerId will be assigned
	virtual i32 SetTimer(i32 timerId, i32 ms, IVehicleObject* pObject) = 0;

	// Summary:
	//  Kill Entity Timer
	virtual i32 KillTimer(i32 timerId) = 0;
	...
	virtual i32 GetSkipEntities(IPhysicalEntity** pSkipEnts, i32 nMaxSkip) = 0;

	// Summary:
	//  Returns vehicle modifications as a comma separated list (with no spaces)
	virtual tukk GetModification() const = 0;
	...
	//	the xml file
	virtual tukk GetActionMap() const = 0;

	// Returns collision damage multiplayer
	virtual float GetSelfCollisionMult(const Vec3& velocity, const Vec3& normal, i32 partId, EntityId colliderId) const = 0;
	...
	: public IMovementController
{
	...
	virtual i32 GetWheelContacts() const = 0;
	...
	virtual void OnAction(const TVehicleActionId actionId, i32 activationMode, float value) = 0;
	...
	//   called from a callback function in DinrusXPhys rather than from
	...
	virtual i32                    GetStatus(SVehicleMovementStatus* status) = 0;
	...
	virtual DrxCriticalSection*    GetNetworkLock() = 0;

	virtual void                   GetMemoryUsage(IDrxSizer* s) const = 0;

	virtual IEntityAudioComponent* GetAudioProxy() const                      { return nullptr; }
	virtual DrxAudio::ControlId    GetPrimaryWeaponAudioTrigger() const       { return DrxAudio::InvalidControlId; }
	virtual DrxAudio::ControlId    GetPrimaryWeaponAudioStopTrigger() const   { return DrxAudio::InvalidControlId; }
	virtual DrxAudio::ControlId    GetSecondaryWeaponAudioTrigger() const     { return DrxAudio::InvalidControlId; }
	virtual DrxAudio::ControlId    GetSecondaryWeaponAudioStopTrigger() const { return DrxAudio::InvalidControlId; }
};
...
{
	CDrxName moodName; //name of the soundmood to apply
...
struct IVehicleSeat
{
	...
	virtual tukk GetSeatName() const = 0;
	...
	virtual i32            GetCurrentTransition() const = 0;
	...
	virtual i32                     GetViewCount() const = 0;
	...
	virtual void                    OnCameraShake(float& angleAmount, float& shiftAmount, const Vec3& pos, tukk source) const = 0;
...
struct IVehicleWheel
{
	...
	virtual i32                     GetSlot() const = 0;
	virtual i32                     GetWheelIndex() const = 0;
	...
	: public IVehicleObject
{
	...
	virtual bool ChangeState(EVehiclePartState state, i32 flags = 0) = 0;
	...
	virtual i32              GetPhysId() = 0;
	virtual i32              GetSlot() = 0;
...
struct IVehicleComponent
{
	...
	virtual tukk GetComponentName() const = 0;
	...
	// See Also
	//   GetPart
	virtual u32  GetPartCount() const = 0;

	virtual IVehiclePart* GetPart(u32 index) const = 0;
...
struct IVehicleSystem
{
	...
	virtual IVehicle* CreateVehicle(u16 channelId, tukk name, tukk vehicleClass, const Vec3& pos, const Quat& rot, const Vec3& scale, EntityId id = 0) = 0;
	...
	virtual IVehicle*                        GetVehicleByChannelId(u16 channelId) = 0;

	virtual bool                             IsVehicleClass(tukk name) const = 0;
	...
	virtual bool                             GetOptionalScript(tukk vehicleName, tuk buf, size_t len) = 0;
...
struct IVehicleClient
{
	...
	virtual void OnAction(IVehicle* pVehicle, EntityId actorId, const ActionId& actionId, i32 activationMode, float value) = 0;
