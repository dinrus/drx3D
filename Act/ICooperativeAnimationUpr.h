...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Central manager for synchronized animations on multiple animated characters/actors
   -------------------------------------------------------------------------
   История:
...
/// Cooperative Animation Upr Common Interface
struct ICooperativeAnimationUpr
{
	virtual ~ICooperativeAnimationUpr(){}
...
public:
	...
	SCharacterParams(IAnimatedCharacter* actor, tukk signalName, bool allowHPhysics = false, float slidingDuration = animParamDefaultSlidingDuration) : deltaLocatorMovement(IDENTITY),
		collisionsDisabledWithFirstActor(0), animationState(AS_Unrequested), animFilepathCRC(0)
	{
		DRX_ASSERT_MESSAGE(actor != NULL, "Invalid parameter. The actor here may not be NULL");

		DRX_ASSERT_MESSAGE(signalName != NULL, "Invalid parameter. The signal name cannot be NULL");
		DRX_ASSERT_MESSAGE(strcmp(signalName, "") != 0, "Invalid parameter. The signal name cannot be empty");

		DRX_ASSERT_MESSAGE(slidingDuration > 0.0f, "Invalid parameter. The sliding duration cannot be <= 0.0f");
