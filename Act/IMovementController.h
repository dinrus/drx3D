...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Control pipe between AI and game objects.
...
#include <drx3D/AI/IAgent.h> // For enums
...
struct SExactPositioningTarget
{
	...
	{}
	u32              preparing              : 1;
	u32              activated              : 1; // == action entered
...
typedef u32 TExactPositioningQueryID;
...
public:
	ILINE CMovementRequest()
		...
		, m_desiredSpeed(0.0f)
		, m_targetSpeed(0.0f)
		, m_lookImportance(0.0f)
		, m_distanceToPathEnd(0.0f)
		, m_desiredLean(0.0f)
		, m_desiredPeekOver(0.0f)
		, m_pseudoSpeed(0.0f)
		, m_alertness(0)
		, m_alertnessLast(0)
		, m_context(0)
	...
	void SetAlertness(i32 alertness)
	{
		m_alertness = alertness;
	}

	ILINE i32 GetAlertness()
	{
		return m_alertness;
	}
	...
	u32k GetContext() const
	{
		DRX_ASSERT(HasContext());
		return m_context;
	}

	const Vec3& GetForcedNavigation() const
	{
		DRX_ASSERT(HasForcedNavigation());
	...
	{
		DRX_ASSERT(CheckFlag(eMRF_ActorTarget));
