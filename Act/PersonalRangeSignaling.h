...
public:
	...
	// Utils ---------------------------------------------------------
	bool           AddRangeSignal(float fRadius, float fBoundary, tukk sSignal, IAISignalExtraData* pData = NULL);
	bool           AddTargetRangeSignal(EntityId IdTarget, float fRadius, float fBoundary, tukk sSignal, IAISignalExtraData* pData = NULL);
	bool           AddAngleSignal(float fAngle, float fBoundary, tukk sSignal, IAISignalExtraData* pData = NULL);
...
private:
	...
	CRange*             SearchRange(tukk sSignal, const VecRanges& vecRangeList) const;
	CAngleAlert const*  GetAngleTo(const Vec3& vPos, bool bUseBoundary = false) const;
	CAngleAlert*        SearchAngle(tukk sSignal) const;
