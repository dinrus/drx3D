...
public:
	...
	struct SShake
	{
		...
		explicit SShake(i32 const shakeID)
			: updating(false)
			, flip(false)
			, doFlip(false)
			, groundOnly(false)
	...
	virtual void               SetViewShake(Ang3 shakeAngle, Vec3 shakeShift, float duration, float frequency, float randomness, i32 shakeID, bool bFlipVec = true, bool bUpdateOnly = false, bool bGroundOnly = false) override;
	virtual void               SetViewShakeEx(const SShakeParams& params) override;
	virtual void               StopShake(i32 shakeID) override;
...
protected:
	...
	Drx::Audio::DefaultComponents::CListenerComponent* m_pAudioListenerComponent;
