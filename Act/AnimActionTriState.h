...
public:
	...
	CAnimActionTriState(i32 priority, FragmentID fragmentID, IAnimatedCharacter& animChar, bool oneShot, float maxMiddleDuration = -1, bool skipIntro = false, IAnimActionTriStateListener* pListener = NULL);
	...
	// Initialize the location that is passed as parameter "TargetPos" to DrxMannequin
	void InitTargetLocation(const QuatT& targetLocation);

	// Clear the location that is passed as parameter "TargetPos" to DrxMannequin
...
private:
	bool TrySetTransitionFragment(u32 fragTagCRC);
