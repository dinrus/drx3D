...
struct Checkpoint
{
	...
	Checkpoint(bool bOutput, tukk msg = 0) : m_bOutput(bOutput), m_szMsg(msg)
	{
		Start();
	}
	...
	CTimeValue Check(tukk msg)
	{
		CTimeValue elapsed = End();
		if (m_bOutput && msg && CDrxActionCVars::Get().g_saveLoadExtendedLog != 0)
			DrxLog("Checkpoint %12s : %.4f ms", msg, elapsed.GetMilliSeconds());
		Start();
		return elapsed;
	}
