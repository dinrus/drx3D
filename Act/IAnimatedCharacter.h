...
struct SGroundAlignmentParams
{
	...
	ILINE void SetFlag(const EGroundAlignment flag, const bool set) { set ? flags |= ((u8)flag) : flags &= ~((u8)flag); }
...
struct IAnimatedCharacter : public IGameObjectExtension
{
	...
	virtual void                     PushForcedState(tukk state) = 0;
	virtual void                     ClearForcedStates() = 0;
	virtual void                     ChangeGraph(tukk graph, i32 layer) = 0;
	...
	virtual void                     SetShadowCharacterSlot(i32 id) = 0;

	virtual void                     SetAnimationPlayerProxy(CAnimationPlayerProxy* proxy, i32 layer) = 0;
	virtual CAnimationPlayerProxy*   GetAnimationPlayerProxy(i32 layer) = 0;
	...
	virtual void SetBlendWeightParam(const EMotionParamID motionParamID, const float value, u8k targetFlags = eBWPT_All) = 0;
	...
	virtual i32  GetCurrentStance() = 0;
	virtual bool InStanceTransition() = 0;
	virtual void RequestStance(i32 stanceID, tukk name) = 0;
	...
	virtual void                   RequestPhysicalColliderMode(EColliderMode mode, EColliderModeLayer layer, tukk tag = NULL) = 0;
	virtual void                   SetCharacterCollisionFlags(u32 flags) = 0;
	...
	virtual void                   SetMovementControlMethods(EMCMSlot slot, EMovementControlMethod horizontal, EMovementControlMethod vertical, tukk tag = NULL) = 0;
	...
	virtual i32  GetFacialAlertnessLevel() = 0;

	// look IK is allowed by default. use this to disallow it
	virtual void AllowLookIk(bool allow, i32 layer = -1) = 0;
...
#if DEBUG_VELOCITY()
	virtual void AddDebugVelocity(const QuatT& movement, const float frameTime, tukk szComment, const ColorF& colorF, const bool pastMovement = false) const = 0;
...
{
	DRXINTERFACE_DECLARE_GUID(IAnimationPoseModifierTorsoAim, "388374ea-ddf8-49ba-b8b7-5dfd824b2c3a"_drx_guid);
};
