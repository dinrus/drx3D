...
i32 CReader::MAX_NUM_LIVE_NODES = 64;

//////////////////////////////////////////////////////////////////////////
_smart_ptr<IGeneralMemoryHeap> CReader::CreateHeap()
{
	return DrxGetIMemoryUpr()->CreateGeneralExpandingMemoryHeap(8 * 1024 * 1024, 0, "Load heap");
}
...
CReader::~CReader()
{
	...
	//	DrxLog(" Binary SaveGame Reader: max live nodes active in the reader: %d", m_maxNumActiveNodes );
}
...
const CNodeLiveReader& CReader::ActivateLiveNodeFromCompact(NodeGlobalID nodeId)
{
	...
	bool found = false;
	u32 size = m_liveNodes.size();
	for (i32 i = m_firstFreeLiveNode + 1; i < size; ++i)
	{
...
FlatAddr CReader::ReadFromBuffer(FlatAddr addr, u8*& rdata, u32 len)
{
...
void CReader::ReadDataFromFile(IPlatformOS::ISaveReaderPtr& pOSSaveReader, uk pDst, u32 numBytes)
{
	if (!CDrxActionCVars::Get().g_XMLCPBUseExtraZLibCompression)
	{
...
// TODO: extract all zlib code into a separate object
void CReader::ReadDataFromZLibBuffer(IPlatformOS::ISaveReaderPtr& pOSSaveReader, u8*& pDst, u32& numBytesToRead)
{
	...
	u32 bytesAvailableInBuffer = m_ZLibBufferSizeWithData - m_ZLibBufferSizeAlreadyRead;
	u32 bytesToCopy = min(bytesAvailableInBuffer, numBytesToRead);
...
// TODO: remove all those pOSSaveReader parameter chains and make it a member, passing it along is not needed anymore
void CReader::ReadDataFromFileInternal(IPlatformOS::ISaveReaderPtr& pOSSaveReader, uk pDst, u32 numBytes)
{
...
bool CReader::ReadBinaryFile(tukk pFileName)
{
	IPlatformOS::ISaveReaderPtr pOSSaveReader = gEnv->pSystem->GetPlatformOS()->SaveGetReader(pFileName);
	if (!m_pZLibBuffer)
		m_pZLibBuffer = new u8[XMLCPB_ZLIB_BUFFER_SIZE];
	if (!m_pZLibCompressedBuffer)
		m_pZLibCompressedBuffer = new u8[XMLCPB_ZLIB_BUFFER_SIZE];
	...
	if (!m_errorReading)
	{
		...
		if (!m_errorReading)
		{
			...
			pOSSaveReader->Seek(-i32(sizeof(fileHeader)), IPlatformOS::ISaveReader::ESM_END);
			...
			{
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "XMLCPB ERROR: file type signature not correct. Savegame File corrupted!");
				m_errorReading = true;
			}
	...
	DrxLog("[LOAD GAME] --Binary saveload: reading done--");

	if (m_errorReading)
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "XMLCPB ERROR: while reading the file: '%s'", pFileName);
...
bool CReader::CheckFileCorruption(IPlatformOS::ISaveReaderPtr& pOSSaveReader, const SFileHeader& fileHeader, u32 totalSize)
{
	bool error = false;

	if (totalSize <= sizeof(fileHeader))
	{
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "XMLCPB ERROR: size check failed. savegame File corrupted! (probably not fully saved)");
		error = true;
		return error;
	}

	u32 sizeToCheck = totalSize - sizeof(fileHeader);

	tuk pBuf = new char[sizeToCheck];
	pOSSaveReader->Seek(0, IPlatformOS::ISaveReader::ESM_BEGIN);
	...
	pZLib->MD5Update(&context, (u8k*) pBuf, sizeToCheck);

	SFileHeader tempFileHeader = fileHeader;
	for (u32 i = 0; i < SFileHeader::MD5_SIGNATURE_SIZE; ++i)
		tempFileHeader.m_MD5Signature[i] = 0;                                                        // the original signature is always calculated with this zeroed.
	pZLib->MD5Update(&context, (u8k*)(&tempFileHeader), sizeof(tempFileHeader));
	pZLib->MD5Final(&context, (u8*) MD5signature);
	...
	for (u32 i = 0; i < SFileHeader::MD5_SIGNATURE_SIZE; ++i)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "XMLCPB ERROR: md5 check failed. savegame File corrupted!");
			error = true;
			break;
		}
	}
...
void CReader::ReadDataFromMemory(u8k* pData, u32 dataSize, uk pSrc, u32 numBytes, u32& outReadLoc)
{
	assert(pData);
	assert(pSrc);
	...
	{
		i32k dataRemaining = (dataSize - numBytes + outReadLoc);
...
bool CReader::ReadBinaryMemory(u8k* pData, u32 uSize)
{
	...
	if (m_errorReading)
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "XMLCPB ERROR: while reading from memory location");
