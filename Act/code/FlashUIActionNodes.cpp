...
// -------------------------------------------------------------------------
//  Имя файла:   FlashUIActionNodes.cpp
//  Version:     v1.00
...
#include <drx3D/Act/FlashUIActionNodes.h>
#include <drx3D/Act/FlashUIBaseNode.h>
#include <drx3D/Act/FlashUI.h>

//--------------------------------------------------------------------------------------------
CFlashUIActionBaseNode::CFlashUIActionBaseNode(tukk name)
	: m_pAction(NULL)
{
	if (gEnv->pFlashUI)
		gEnv->pFlashUI->GetUIActionUpr()->AddListener(this, name);
}
...
void CFlashUIActionBaseNode::UpdateAction(tukk sName, bool bStrict)
{
	if (gEnv->pFlashUI)
	...
	case eFE_Initialize:
		...
		if (m_pAction)
		{
			...
			if (GetPortBool(pActInfo, eI_UseAsState))
				gEnv->pFlashUI->GetUIActionUpr()->EnableAction(m_pAction, false);
		}
		break;
	case eFE_Update:
...
void CFlashUIEndActionNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_Activate:
		if (gEnv->pFlashUI && m_pAction && IsPortActive(pActInfo, eI_OnActionEnd))
		{
			...
			gEnv->pFlashUI->GetUIActionUpr()->EndAction(m_pAction, args);
...
void CFlashUIActionNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_Activate:
		...
		if (gEnv->pFlashUI && m_pAction)
		{
			...
			{
				SUIArguments args(GetPortString(pActInfo, eI_Args).c_str(), true);
				gEnv->pFlashUI->GetUIActionUpr()->StartAction(m_pAction, args);
