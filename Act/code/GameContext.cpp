...
#include <drx3D/Act/VoiceController.h>
...
#include <drx3D/Act/CET_ClientConnections.h>

#include <drx3D/Act/NetDebug.h>
...
DrxLockT<DRXLOCK_RECURSIVE> SScopedTickerLock::s_tickerMutex;
...
CGameContext::CGameContext(CDrxAction* pFramework, CScriptRMI* pScriptRMI, CActionGame* pGame) :
...
#endif
{
	DRX_ASSERT(s_pGameContext == NULL);
	s_pGameContext = this;
	...
	if (!CDrxActionCVars::Get().g_syncClassRegistry)
	{
		...
		{
			m_classRegistry.RegisterClassName(pClass->GetName(), ~u16(0));
		}
	}
}
...
void CGameContext::SetContextInfo(unsigned flags, u16 port, tukk connectionString)
{
	m_flags = flags;
	m_port = port;
...
void CGameContext::AddLoadLevelTasks(IContextEstablisher* pEst, bool isServer, i32 flags, bool** ppLoadingStarted, i32 establishedToken, bool bChannelIsMigrating)
{
...
void CGameContext::AddLoadingCompleteTasks(IContextEstablisher* pEst, i32 flags, bool* pLoadingStarted, bool bChannelIsMigrating)
{
	...
	if (!bChannelIsMigrating)
	{
		...
		{
			if (CGameClientNub* pGCN = CDrxAction::GetDrxAction()->GetGameClientNub())
			{
...
#define NEW_CLIENT_WAIT_LENGTH (5.f)
	...
	CCET_WaitForPendingConnections(CActionGame* cag, CDrxAction* cca) : m_state(EW_INITIAL_TICK), m_cag(cag), m_cca(cca), m_start(0.f), m_prevNumConnections(0), m_waitLen(DEFAULT_WAIT_LENGTH) {}
	...
	EContextEstablishTaskResult OnStep(SContextEstablishState& state)
	{
		if (m_cag->GetServerNetNub())
		{
			...
			case EW_WAIT_FOR_PENDING:
				DrxLog("CEst -- Waiting until we have pending connections!");
				if (havePendingConnections)
				{
					DrxLog("CEst -- We now have pending connections!");
			...
			case EW_WAIT_UNTIL_CONNECTED:
				{
					...
					i32 numChannels = m_cag->GetServerNetNub()->GetNumChannels();
					...
					//DrxLog("CEst -- num channels %d, expecting %d!", numChannels, numExpected);

					if (numChannels >= numExpected && !havePendingConnections)
					{
						DrxLog("CEst -- We have NO pending connections remaining!");
						//m_start = gEnv->pTimer->GetAsyncTime();
						return eCETR_Ok;
					}
					break;
			...
			if (diff < m_waitLen)
				...
				DrxLog("CEst -- Waited more than %f seconds, continuing...", m_waitLen);
		}

		DrxLog("CEst -- No pending connections, we have a go!");
		return eCETR_Ok;
	}
...
private:
	...
	CDrxAction*  m_cca;
	CTimeValue   m_start;
	i32          m_prevNumConnections;
	float        m_waitLen;
};

void AddWaitForPendingConnections(IContextEstablisher* pEst, EContextViewState state, CActionGame* cag, CDrxAction* cca)
{
...
bool CGameContext::InitGlobalEstablishmentTasks(IContextEstablisher* pEst, i32 establishedToken)
{
...
bool CGameContext::InitChannelEstablishmentTasks(IContextEstablisher* pEst, INetChannel* pChannel, i32 establishedSerial)
{
	...
	i32 flags = (isLevelLoaded ? eEF_LevelLoaded : 0) | (isReset ? 0 : eEF_LoadNewLevel);
	...
	if (isClient)
	{
		if (isReset)
		{
			for (i32 i = eCVS_EstablishContext; i < eCVS_PostSpawnEntities; i++)
...
void CGameContext::ResetMap(bool isServer)
{
	...
	if (CGameServerNub* pGSN = CDrxAction::GetDrxAction()->GetGameServerNub())
...
/*
   ...
   IPlayerProfileUpr* pPPMgr = m_pFramework->GetIPlayerProfileUpr();
   if (pPPMgr)
   {
   ...
   if (pActionMap == 0)
   {
    ...
    IActionMapUpr * pActionMapMan = m_pFramework->GetIActionMapUpr();
    if (pActionMapMan)
    {
...
ESynchObjectResult CGameContext::SynchObject(EntityId entityId, NetworkAspectType nAspect, u8 profile, TSerialize serialize, bool verboseLogging)
{
	...
	{
		if (verboseLogging)
			GameWarning("CGameContext::SynchObject: game fails to serialize aspect %d on profile %d", BitIndex(nAspect), i32(profile));
		NET_PROFILE_COUNT_READ_BITS(false);
		return eSOR_Failed;
	}
...
public:
	...
	void UpdateState(u32 nFromSeq, ENetSendableStateUpdate)
	{
	}
...
{
	CDrxAction::GetDrxAction()->GetIActorSystem()->SetDemoPlaybackMappedOriginalServerPlayer(id);
}
...
INetAtSyncItem* CGameContext::HandleRMI(bool bClient, EntityId objID, u8 funcID, TSerialize ser, INetChannel* pChannel)
{
...
bool CGameContext::OnBeforeSpawn(SEntitySpawnParams& params)
{
	if (m_isInLevelLoad && !gEnv->IsEditor() && gEnv->bMultiplayer)
	{
		...
		if (params.sLayerName && !strnicmp(params.sLayerName, prefix, prefixLen))
		{
			...
			{
				tukk currentType = pGameRules->GetClass()->GetName();
				tukk gameType = params.sLayerName + prefixLen;
...
void CGameContext::OnSpawn(IEntity* pEntity, SEntitySpawnParams& params)
{
	...
	DrxLogAlways("CGameContext: Spawn Entity");
	DrxLogAlways("  name       : %s", params.sName);
	DrxLogAlways("  id         : 0x%.4x", params.id);
	DrxLogAlways("  flags      : 0x%.8x", params.nFlags);
	DrxLogAlways("  pos        : %.1f %.1f %.1f", params.vPosition.x, params.vPosition.y, params.vPosition.z);
	DrxLogAlways("  cls name   : %s", params.pClass->GetName());
	DrxLogAlways("  cls flags  : 0x%.8x", params.pClass->GetFlags());
	DrxLogAlways("  channel    : %d", channelId);
	DrxLogAlways("  net aspects: 0x%.8x", aspects);
#endif
	...
	if (m_isInLevelLoad && gEnv->bMultiplayer)
	{
		...
		{
			//DrxLog("Forcibly binding %s to network", params.sName);
...
bool CGameContext::OnRemove(IEntity* pEntity)
{
	...
	if (gEnv->IsEditor() || CDrxAction::GetDrxAction()->IsGameSessionMigrating())
	{
		if (bound)
		{
...
void CGameContext::OnReused(IEntity* pEntity, SEntitySpawnParams& params)
{
	...
	if (0 == m_removeObjectUnlocks && gEnv->bMultiplayer && !CDrxAction::GetDrxAction()->IsGameSessionMigrating())
	{
...
void CGameContext::OnCollision(const EventPhys* pEvent, uk )
{
...
bool CGameContext::RegisterClassName(const string& name, u16 id)
{
	return m_classRegistry.RegisterClassName(name, id);
}

u32 CGameContext::GetClassesHash()
{
...
bool CGameContext::ClassIdFromName(u16& id, const string& name) const
{
	return m_classRegistry.ClassIdFromName(id, name);
}

bool CGameContext::ClassNameFromId(string& name, u16 id) const
{
...
{
	if (CDrxAction::GetDrxAction()->GetILevelSystem()->IsLevelLoaded())
	...
	while (HasContextFlag(eGSF_Server))
	{
		...
		if (HasContextFlag(eGSF_DemoRecorder))
		{
			...
			CDrxAction::GetDrxAction()->OnActionEvent(SActionEvent(eAE_demoRecorderCreated, 0));
			break;
		}
...
bool CGameContext::Update()
{
	...
	/*
	   else if (m_bScheduledLevelLoad)
	   {
	    ...
	    m_pFramework->GetIActionMapUpr()->Enable(false); // diable action processing during loading
	    ...
	    m_pFramework->GetIActionMapUpr()->Enable(true);
	...
	if (0 == (m_broadcastActionEventInGame -= (m_broadcastActionEventInGame != -1)))
		CDrxAction::GetDrxAction()->OnActionEvent(eAE_inGame);
...
bool CGameContext::OnBeforeVarChange(ICVar* pVar, tukk sNewValue)
{
	...
	if (!allowChange)
	{
		DrxLog("[CVARS]: [CHANGED] CGameContext::OnBeforeVarChange(): variable [%s] is VF_NET_SYNCED with a value of [%s]; CLIENT changing attempting to set to [%s] - IGNORING",
		       pVar->GetName(),
		       pVar->GetString(),
		       sNewValue);
	}
...
IHostMigrationEventListener::EHostMigrationReturn CGameContext::OnInitiate(SHostMigrationInfo& hostMigrationInfo, HMStateType& state)
{
	...
	{
		DrxLogAlways("[Host Migration]: no game rules found - aborting");
...
IHostMigrationEventListener::EHostMigrationReturn CGameContext::OnDemoteToClient(SHostMigrationInfo& hostMigrationInfo, HMStateType& state)
{
	...
	DrxLogAlways("[Host Migration]: CGameContext::OnDemoteToClient() started");

	if (HasContextFlag(eGSF_Server) == true)
	{
		m_flags &= ~eGSF_Server;

		CDrxAction::GetDrxAction()->GetIGameSessionHandler()->EndSession();
	}

	DrxLogAlways("[Host Migration]: CGameContext::OnDemoteToClient() finished");
	return IHostMigrationEventListener::Listener_Done;
}
...
IHostMigrationEventListener::EHostMigrationReturn CGameContext::OnPromoteToServer(SHostMigrationInfo& hostMigrationInfo, HMStateType& state)
{
	...
	DrxLogAlways("[Host Migration]: CGameContext::OnPromoteToServer() started");
	m_flags |= eGSF_Server;

	ICVar* pCVar = gEnv->pConsole->GetCVar("sv_requireinputdevice");
	if (pCVar)
	{
		tukk requiredInputDevice = pCVar->GetString();
	...
	DrxLogAlways("[Host Migration]: CGameContext::OnPromoteToServer() finished");
	return IHostMigrationEventListener::Listener_Done;
}
...
static XmlNodeRef Child(XmlNodeRef& from, tukk name)
{
...
{
	DrxLogAlways("Game reconfiguration: %s", pNetChannel->GetName());
}
...
{
	i32 tick = gEnv->pPhysicalWorld->GetiPhysicsTime();
...
void CGameContext::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
#ifndef OLD_VOICE_SYSTEM_DEPRECATED
	if (m_pVoiceController)
...
static ILINE bool IsDX11()
{
	...
	return true;//renderType == eRT_DX11;	// marcio: in this context, we assume DX11 for Drxsis2, so immersiveness can work
}
...
string CGameContext::GetConnectionString(DrxFixedStringT<HOST_MIGRATION_MAX_PLAYER_NAME_SIZE>* pNameOverride, bool fake) const
{
	...
	else
	{
		...
		if (IPlayerProfileUpr* pProfileUpr = CDrxAction::GetDrxAction()->GetIPlayerProfileUpr())
		{
			IPlatformOS::TUserName userName;
			IPlatformOS* pOS = GetISystem()->GetPlatformOS();
			u32 userIndex = pProfileUpr->GetExclusiveControllerDeviceIndex();
			...
			else if (CDrxActionCVars::Get().useCurrentUserNameAsDefault != 0)
			{
	...
	drx_sprintf(buf, "%d", (i32)time(NULL));
	constr = buf + constr;
	i32 len = constr.length();
	i32 r = drx_random(0, 19) + CLAMP(64 - len, 0, 64);
	for (i32 i = 0; i < r || (constr.length() & 7); i++)
	{
		char c = drx_random('a', 'z');
		constr = c + constr;
	}
	len = constr.length();
	...
	string temp = out;
	DRX_ASSERT(FromHexStr(temp));
	DRX_ASSERT(temp == constr);
	return out;
}
...
void CGameContext::PlaybackBreakage(i32 breakId, INetBreakagePlaybackPtr pBreakage)
{
	if (m_pBreakReplicator.get())
		m_pBreakReplicator->PlaybackBreakage(breakId, pBreakage);
}

uk CGameContext::ReceiveSimpleBreakage(TSerialize ser)
{
	if (m_pBreakReplicator.get())
...
void CGameContext::PlaybackSimpleBreakage(uk userData, INetBreakageSimplePlaybackPtr pBreakage)
{
	if (m_pBreakReplicator.get())
