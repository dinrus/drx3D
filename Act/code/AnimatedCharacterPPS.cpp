...
bool CAnimatedCharacter::EvaluateSimpleMovementConditions() const
{
	...
	if (m_isPlayer && GetEntityId() == CDrxAction::GetDrxAction()->GetClientActorId())
		return false;
...
void CAnimatedCharacter::UpdateSimpleMovementConditions()
{
	    ...
	    !m_bSimpleMovementSetOnce) // Loading a singleplayer map in game takes too long between character reset and update.
	{
		...
		//DrxLogAlways("AC[%s]: simplified movement %s!", name, (m_simplifyMovement ? "enabled" : "disabled"));
		...
		if ((pPhysEnt != NULL) && (pPhysEnt->GetType() == PE_LIVING))
		{
			...
			if (m_simplifyMovement || m_forceDisableSlidingContactEvents)
			{
				...
				//DrxLogAlways("AC[%s]: events disabled!", name);
			}
			else
			{
				pf.flagsOR = lef_report_sliding_contacts;
				//DrxLogAlways("AC[%s]: events enabled!", name);
			}
...
void CAnimatedCharacter::UpdateTime()
{
	...
	DRX_ASSERT(NumberValid(frameTime));
...
void CAnimatedCharacter::AcquireRequestedBehaviourMovement()
{
	...
	DRX_ASSERT(m_moveRequest.velocity.IsValid());
	DRX_ASSERT(m_moveRequest.rotation.IsValid());
	...
	case eCMT_Normal:
		DRX_ASSERT(m_requestedEntityMovementType != RequestedEntMovementType_Impulse);
	...
	case eCMT_Impulse:
		DRX_ASSERT(m_requestedEntityMovementType != RequestedEntMovementType_Absolute);
		m_requestedEntityMovementType = RequestedEntMovementType_Impulse;
		DRX_ASSERT(m_curFrameTime > 0.0f);
		if (m_curFrameTime > 0.0f)
		{
	...
	DRX_ASSERT(m_requestedEntityMovement.IsValid());
...
void CAnimatedCharacter::PostProcessingUpdate()
{
	...
	if (IActor* pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(pEntity->GetId()))
	{
	...
	if (allowLandBob && m_landBobParams.IsValid())
	{
		...
		if (m_landBobTime > 0.0f)
		{
			...
			DRX_ASSERT_MESSAGE((landingBobFactor >= 0.0f) && (landingBobFactor <= 1.0f), "Invalid value for m_landingBobTimeFactor cvar, must be 0..1");
...
QuatT CAnimatedCharacter::CalculateDesiredAnimMovement() const
{
	...
	if (m_pSkeletonAnim != NULL)
	{
		...
		if (CAnimationGraphCVars::Get().m_useMovementPrediction)
		{
			...
			else
			{
				...
				assetAnimMovement.t = assetAnimMovement.q * futureAssetAnimMovement.t; // request the 'future' translation, properly adjusted for the fact that the rotation we got back from drxanimation is relative to the rotation of the current frame
			}
		}
		else
		{
	...
	//DRX_ASSERT((GetMCMH() != eMCM_Animation) || (m_colliderMode != eColliderMode_Pushable));
	...
	DRX_ASSERT(desiredAnimMovement.IsValid());
	return desiredAnimMovement;
}
...
QuatT CAnimatedCharacter::CalculateWantedEntityMovement(const QuatT& desiredAnimMovement) const
{
	...
	DRX_ASSERT(wantedEntMovement.IsValid());
	...
	if (DebugVelocitiesEnabled())
	{
		...
		char buffer[256];
		tukk const mcmHTag = m_currentMovementControlMethodTags[eMCMComponent_Horizontal];
		tukk const mcmVTag = m_currentMovementControlMethodTags[eMCMComponent_Vertical];
		drx_sprintf(buffer, "Wanted: Hor:%s%s%s(%s) Ver:%s%s(%s)", szMcmH, m_moveOverride_useAnimXY ? "+ANMXY" : "", m_moveOverride_useAnimRot ? "+ANMQ" : "", mcmHTag, szMcmV, m_moveOverride_useAnimZ ? "+ANMZ" : "", mcmVTag);
...
void CAnimatedCharacter::RequestPhysicalEntityMovement(const QuatT& wantedEntMovement)
{
	...
	if (pParent != NULL)
	{
		...
		if (GetMCMH() != eMCM_Entity)
		{
			...
			DRX_ASSERT(wantedEntLocationLocal.IsValid());
			Matrix34 wantedEntLocationLocalMat(wantedEntLocationLocal);
			DRX_ASSERT(wantedEntLocationLocalMat.IsValid());
	...
	{
		{
			    ...
			    ((GetMCMH() == eMCM_Animation || GetMCMH() == eMCM_AnimationHCollision) && (GetMCMV() == eMCM_Animation)))
			{
				...
				if ((curMoveVeloHash != m_prevMoveVeloHash) || (move.iJump != m_prevMoveJump) || (actualMoveVeloHash != curMoveVeloHash) || RecentQuickLoad())
				{
					...
					if (!move.dir.IsZero() && !wantedEntMovement.t.IsZero())
						DRX_ASSERT(m_colliderModeLayers[eColliderModeLayer_ForceSleep] == eColliderMode_Undefined);
			...
			else
			{
				if (m_requestedEntityMovementType != RequestedEntMovementType_Undefined) // Can be underfined right after start, if there is no requested movement yet.
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "AnimatedCharacter::UpdatePhysicalEntityMovement() - Undefined movement type %d, expected absolute(1) or impulse(2).", m_requestedEntityMovementType);
		...
		if (!NoMovementOverride())
		{
			...
			DRX_ASSERT(newRotation.IsValid());
...
void CAnimatedCharacter::UpdatePhysicalColliderMode()
{
	...
	DRX_ASSERT(m_colliderMode != eColliderMode_Undefined);
	...
	u32k characterGeomCollTypeFlags = m_characterCollisionFlags;
	...
	else if (m_colliderMode == eColliderMode_Pushable)
	{
		...
		bool bIsClient = GetEntityId() == CDrxAction::GetDrxAction()->GetClientActorId();
		pf.flagsOR = pef_pushable_by_players | (lef_loosen_stuck_checks & - (i32)!bIsClient);
		pf.flagsAND = ~(pef_ignore_areas | (lef_loosen_stuck_checks & - (i32)bIsClient));
	}
	...
	{
		DRX_ASSERT(!"ColliderMode not implemented!");
		return;
	}
...
void CAnimatedCharacter::GetCurrentEntityLocation()
{
	...
	DRX_ASSERT(m_entLocation.IsValid());
...
QuatT CAnimatedCharacter::CalculateProceduralLeaning()
{
	...
	{
		i32 j = (m_reqLocalEntAxxNextIndex + i) % NumAxxSamples;
	...
	DRX_ASSERT(prevSmoothedActualEntVelo.IsValid());
	...
	DRX_ASSERT(smoothedActualEntVeloL.IsValid());
	...
	DRX_ASSERT(avgAxxScaled.IsValid());
	...
	DRX_ASSERT(m_avgLocalEntAxx.IsValid());
