...
// -------------------------------------------------------------------------
//  Имя файла:   FlashUIDisplayNodes.cpp
//  Version:     v1.00
...
#include <drx3D/Act/FlashUIDisplayNodes.h>
...
void CFlashUIDisplayNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	else if (event == eFE_Activate)
	{
		...
		i32k instanceId = GetPortInt(pActInfo, eI_InstanceID);
...
void CFlashUIDisplayConfigNode::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig<i32>("layer",           "layer of the element"),
		{ 0 }
	};
	...
	static const SOutputPortConfig out_config[] = {
		...
		OutputPortConfig<i32>("layer",              "layer of the element"),
		{ 0 }
	};
...
void CFlashUIDisplayConfigNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	else if (event == eFE_Activate)
	{
		...
		if (IsPortActive(pActInfo, eI_Set))
		{
			...
			i32k layer = GetPortInt(pActInfo, eI_Layer);
			const uint64 flags = GetFlags(pActInfo);

			SPerInstanceCall3<uint64, float, i32> caller;
...
void CFlashUIDisplayConfigNode::SetToInstance(IUIElement* pInstance, uint64 flags, float alpha, i32 layer)
{
...
void CFlashUIConstraintsNode::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig_Void("set",      "Set configuration"),
		InputPortConfig_Void("get",      "Get configuration"),
		InputPortConfig<i32>("type",     1,                                             _HELP("Set positioning type (fixed=fixed position, dynamic=fit screen but keep aspect ratio, fullscreen=scale to fullscreen) "),0, _UICONFIG("enum_int:Fixed=0,Fullscreen=1,Dynamic=2")),
		InputPortConfig<i32>("left",     "Set left position (fixed mode)"),
		InputPortConfig<i32>("top",      "Set top position (fixed mode)"),
		InputPortConfig<i32>("width",    "Set width (fixed mode)"),
		InputPortConfig<i32>("height",   "Set height (fixed mode)"),
		InputPortConfig<bool>("scale",   "Set scale"),
		InputPortConfig<i32>("hAlign",   1,                                             _HELP("Set horizontal align (dynamic mode)"),                                                                  0, _UICONFIG("enum_int:Left=0,Middle=1,Right=2")),
		InputPortConfig<i32>("vAlign",   1,                                             _HELP("Set vertical align (dynamic mode)"),                                                                    0, _UICONFIG("enum_int:Top=0,Middle=1,Bottom=2")),
	...
	static const SOutputPortConfig out_config[] = {
		OutputPortConfig_Void("OnSet",     _HELP("Triggered on set")),
		OutputPortConfig_Void("OnGet",     _HELP("Triggered on get")),
		OutputPortConfig<i32>("type",      _HELP("Get positioning type (0=fixed position, 1=scaled to fit screen)")),
		OutputPortConfig<i32>("left",      "Get left position"),
		OutputPortConfig<i32>("top",       "Get top position"),
		OutputPortConfig<i32>("width",     "Get width"),
		OutputPortConfig<i32>("height",    "Get height"),
		OutputPortConfig<bool>("scale",    "Get scale"),
		OutputPortConfig<i32>("hAlign",    "Get horizontal align (0:Left, 1:Middle, 2:Right)"),
		OutputPortConfig<i32>("vAlign",    "Get vertical align (0:Bottom, 1:Middle 2:Top"),
...
void CFlashUIConstraintsNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	else if (event == eFE_Activate)
	{
		...
		if (IsPortActive(pActInfo, eI_Get))
		{
			...
			ActivateOutput(pActInfo, eO_PosType, (i32) constraints.eType);
			...
			ActivateOutput(pActInfo, eO_HAlign, (i32) constraints.eHAlign);
			ActivateOutput(pActInfo, eO_VAlign, (i32) constraints.eVAlign);
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			...
			InputPortConfig<string>("FunctionName", _HELP("This function will be called in ActionScript (signature: myFunc = function(i32, i32))")),
	...
	bool RayIntersectMesh(IRenderMesh* pMesh, IMaterial* pMaterial, IUIElement* pElement, const Ray& ray, Vec3& hitpos, Vec3& p0, Vec3& p1, Vec3& p2, Vec2& uv0, Vec2& uv1, Vec2& uv2)
	{
		...
		i32k numIndices = pMesh->GetIndicesCount();
		i32k numVertices = pMesh->GetVerticesCount();
					...
					if (type == PHYS_FOREIGN_ID_ENTITY)
					{
						...
						
						{
							...
							if (pElement && pRenderNode)
							{
								...
								i32 iX = int_round(uv.x * (float)width);
								i32 iY = int_round(uv.y * (float)height);
