...
//------------------------------------------------------------------------
bool CVehiclePartSubPartWheel::Init(IVehicle* pVehicle, const CVehicleParams& table, IVehiclePart* parent, CVehicle::SPartInitInfo& initInfo, i32 partType)
{
...
//------------------------------------------------------------------------
bool CVehiclePartSubPartWheel::ChangeState(EVehiclePartState state, i32 flags)
{
	...
	if (state == eVGS_Default)
	{
		if (IPhysicalEntity* pPhysics = m_pVehicle->GetEntity()->GetPhysics())
		{
			...
			if (pPhysics->GetParams(&params_car))
			{
				...
				{
					DRX_ASSERT_MESSAGE(false, "Wheel index exceeds number of wheels");
				}
			}
		}
	}
...
void CVehiclePartSubPartWheel::Physicalize()
{
	...
	if (IsDebugParts())
		DrxLog("[CVehicleSubPartWheel]: <%s> got physId %i", GetName(), m_physId);
#endif
}
