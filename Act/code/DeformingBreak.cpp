...
#include <drx3D/Act/DeformingBreak.h>
...
void LogDeformPhysicalEntity(tukk from, IPhysicalEntity* pEnt, const Vec3& p, const Vec3& n, float energy)
{
	...
	if (CNetworkCVars::Get().BreakageLog)
	{
		...
		DrxLog("[brk]    selector is %s", CObjectSelector::GetDescription(pEnt).c_str());
...
void SDeformingBreak::AddSendables(INetSendableSink* pSink, i32 brkId)
{
...
i32 CDeformingBreak::GetVirtualId(IPhysicalEntity* pEnt)
{
...
CObjectSelector CDeformingBreak::GetObjectSelector(i32 idx)
{
	if (idx == 1)
	{
...
void CDeformingBreak::PreparePlaybackForEvent(i32 event)
{
	m_pPhysEnt = 0;
