...
void CNetDebug::DebugAspectsChange(IEntity* pEntity, u8 aspects)
{
	...
	tukk sClassName = pEntity->GetClass()->GetName();
	tukk sEntityName = pEntity->GetName();
...
void CNetDebug::DebugRMI(tukk szDescription, size_t nSize)
{
	if (!m_varDebugRMI)
		return;
...
tukk CNetDebug::IdxToAspectString(i32 idx)
{
	switch (idx)
	{
	case 0:
...
i32 CNetDebug::AspectToIdx(EEntityAspects aspect)
{
	switch (aspect)
	{
	case eEA_Script:
...
void CNetDebug::UpdateAspectsDebugEntity(const DebugAspectsContext& ctx, DebugAspectsEntity& ent, u8 aspects)
{
...
bool CNetDebug::ProcessFilter(tukk szClass, tukk szEntity)
{
	tukk szFilterClass = gEnv->pConsole->GetCVar("g_debugAspectFilterClass")->GetString();
	tukk szFilterEntity = gEnv->pConsole->GetCVar("g_debugAspectFilterEntity")->GetString();
		...
		for (TDebugEntities::iterator iter = pCtx->entities.begin(); iter != pCtx->entities.end(); ++iter)
		{
			...
			for (i32 i = 0; i < MAX_ASPECTS; ++i)
			{
				if (pEnt->entityTTL[i] > 0)
				{
					...
					{
						drx_sprintf(msg, "There are more messages to track. Please use filter.");
						color[3] = color[0] = 1;
						IRenderAuxText::Draw2dLabel(xPos, yPos, 1.2f, &color[0], false, "%s", msg);
					}
					else
					{
						drx_sprintf(msg, "Class [%s]: entity [%s] aspect [%s], total [%u], rate [%.2f]",
...
void CNetDebug::DrawRMI()
{
	...
	for (TDebugRMI::iterator it = m_rmi.begin(); it != m_rmi.end(); ++it)
	{
		...
		if (yPos < cellSize * maxRow)
		{
			drx_sprintf(msg, "[%s]: invoke [%u] total [%" PRISIZE_T "], peak [%.2f], rate [%.2f]",
			            pEnt->sDesc.c_str(),
			            pEnt->invokeCount,
			            pEnt->totalSize,
			            pEnt->peakRate,
