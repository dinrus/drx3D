...
static u32k INVALID_FACIAL_CHANNEL_ID = ~0;
...
tukk CDialogActorContext::GetSoundKey(const string& soundName)
{
	stack_string const sLocalizationFolder(PathUtil::GetLocalizationFolder() + DRX_NATIVE_PATH_SEPSTR "dialog" DRX_NATIVE_PATH_SEPSTR);
...
// returns ptr to static string buffer.
tukk CDialogActorContext::FullSoundName(const string& soundName, bool bWav)
{
	stack_string const sLocalizationFolder(PathUtil::GetLocalizationFolder() + DRX_NATIVE_PATH_SEPSTR "dialog" DRX_NATIVE_PATH_SEPSTR);

	// check if it already starts with full path
	if (DrxStringUtils::stristr(soundName.c_str(), sLocalizationFolder.c_str()) == soundName.c_str())
	{
	...
	: m_pCurLine(0)
{
	...
	tukk debugName = pEntity ? pEntity->GetName() : "<no entity>";
	m_pIActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(m_entityID);
	m_bIsLocalPlayer = m_pIActor != 0 && CDrxAction::GetDrxAction()->GetClientActor() == m_pIActor;
	m_SpeechAuxObject = DrxAudio::InvalidAuxObjectId;
...
{
	if (m_SpeechAuxObject != DrxAudio::InvalidAuxObjectId)
	{
...
bool CDialogActorContext::Update(float dt)
{
	...
	do
	{
		...
		case eDAC_ScheduleSoundPlay:
			{
				bAdvance = true;
				const bool bHasSound = m_pCurLine->m_audioID != DrxAudio::InvalidControlId;
				if (bHasSound)
				{
					if (m_bSoundScheduled == false)
					{
						...
						if (m_SpeechAuxObject == DrxAudio::InvalidAuxObjectId)
						{
							...
							pActorAudioProxy->AddAsListenerToAudioAuxObject(m_SpeechAuxObject, &CDialogActorContext::OnAudioTriggerFinished, DrxAudio::ESystemEvents::TriggerFinished);
						}
						UpdateAuxProxyPosition();

						DrxAudio::SRequestUserData const userData(DrxAudio::ERequestFlags::DoneCallbackOnExternalThread, nullptr, (uk )CDialogActorContext::GetClassIdentifier(), reinterpret_cast<uk>(static_cast<intptr_t>(m_ContextID)));
		...
		case eDAC_SoundFacial:
			{
				...
				//	{
				//		m_soundLength = drx_random(2.0f, 6.0f);
...
void CDialogActorContext::UpdateAuxProxyPosition()
{
	...
	if (m_VoiceAttachmentIndex == DIALOG_VOICE_POSITION_NEEDS_UPDATE && pCharacter)
	{
		...
		if (pAttachmentUpr)
		{
			m_VoiceAttachmentIndex = pAttachmentUpr->GetIndexByName("voice");  //First prio: get the "voice" attachment position
	...
	{
		const IAttachmentUpr* pAttachmentUpr = pCharacter->GetIAttachmentUpr();
		const IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByIndex(m_VoiceAttachmentIndex);
		if (pAttachment)
		{
...
////////////////////////////////////////////////////////////////////////////
bool CDialogActorContext::DoAnimActionAG(IEntity* pEntity, tukk sAction)
{
...
////////////////////////////////////////////////////////////////////////////
bool CDialogActorContext::DoAnimActionEP(IEntity* pEntity, tukk sAction)
{
...
////////////////////////////////////////////////////////////////////////////
void CDialogActorContext::SetOutput(tukk output, tukk value)
{
}
...
////////////////////////////////////////////////////////////////////////////
bool CDialogActorContext::ExecuteAI(i32& goalPipeID, tukk signalText, IAISignalExtraData* pExtraData, bool bRegisterAsListener)
{
...
{
	return m_bSoundStarted && m_pCurLine && m_pCurLine->m_audioID != DrxAudio::InvalidControlId; //m_soundID != INVALID_SOUNDID && m_bSoundStarted;
}
...
tukk EventName(EGoalPipeEvent event)
{
	switch (event)
	{
	case ePN_Deselected:
...
// IGoalPipeListener
////////////////////////////////////////////////////////////////////////////
void CDialogActorContext::OnGoalPipeEvent(IPipeUser* pPipeUser, EGoalPipeEvent event, i32 goalPipeId, bool& unregisterListenerAfterEvent)
{
...
////////////////////////////////////////////////////////////////////////////
void CDialogActorContext::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
////////////////////////////////////////////////////////////////////////////
void CDialogActorContext::OnAudioTriggerFinished(DrxAudio::SRequestInfo const* const pAudioRequestInfo)
{
...
////////////////////////////////////////////////////////////////////////////
CDialogActorContext* CDialogActorContext::GetDialogActorContextByAudioCallbackData(DrxAudio::SRequestInfo const* const pAudioRequestInfo)
{
