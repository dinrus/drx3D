...
i32k CNodeLiveWriter::INITIAL_SIZE_CHILDREN_VECTOR = 4;
i32k CNodeLiveWriter::INITIAL_SIZE_ATTRS_VECTOR = 4;
...
CNodeLiveWriter::CNodeLiveWriter(CWriter& Writer, NodeLiveID ID, StringID IDTag, u32 safecheckID)
	: m_Writer(Writer)
	, m_ID(ID)
{
...
CNodeLiveWriterRef CNodeLiveWriter::AddChildNode(tukk pChildName)
{
...
void CNodeLiveWriter::Done()
{
	...
	i32 numChildren = m_children.size();
...
void CNodeLiveWriter::AddAttr(tukk pAttrName, u8k* data, u32 len, bool needInmediateCopy)
{
	assert(data && len > 0);
	assert(!m_done && m_valid);
...
tukk CNodeLiveWriter::GetTag() const
{
...
void CNodeLiveWriter::Reuse(StringID IDTag, u32 safecheckID)
{
	m_IDTag = IDTag;
...
//                              if bit6 ==1 then:
//                                 bits 0-5 is the high bits. the next 2 bytes are the lower bits, stored as a single u16
//
//
// [...]									-> attr data
...
void CNodeLiveWriter::Compact()
{
...
#ifdef XMLCPB_COLLECT_STATS
	u32 startSize = writeBuffer.GetDataSize();
#endif

	NodeGlobalID lastGlobalId = m_Writer.GetLastUsedGlobalId();
	u32 numChildBlocks = m_globalIdLastChildInBlock.size();

	assert(m_IDTag <= MAX_TAGID);
	u16 nodeHeader = m_IDTag;
	...
	u8 attrSetLow8Bits = 0;
	if (numAttrs > 0)
	{
		nodeHeader |= FLN_HASATTRS;

		u16 attrHeaders[MAX_NUM_ATTRS];
		for (i32 a = 0; a < numAttrs; ++a)
		...
		u32 highBitsAttrSetId = (attrSetId8Bits << (HIGHPART_ATTRID_BIT_POSITION - 8)) & HIGHPART_ATTRID_MASK;
		nodeHeader |= highBitsAttrSetId;
	}

	// amount of children is specified in a few bits in the header, when that is possible
	{
		u32 val = m_totalAmountChildren;
	...
	if (m_totalAmountChildren > MAX_NUMBER_OF_CHILDREN_IN_8BITS)  // when the number of children does not fit in 8 bits
	{
		...
		u8 childrenLow = m_totalAmountChildren & 255;
		u8 childrenHigh = m_totalAmountChildren >> 8;
	...
	else if (m_totalAmountChildren > CHILDREN_HEADER_MAXNUM)  // when the number of children does not fit in the header, but it does fit in 8 bits
	{
		...
		u32 children = m_totalAmountChildren - CHILDREN_HEADER_MAXNUM; // dirty trick: subtracts the value that could fit in the header, to increase a bit the range of valid values for the all-in-single-byte attempt.
		...
		if (children <= CHILDANDDISTINONEBYTE_MAX_AMOUNT_CHILDREN && !childrenRightBefore)
		{
			...
			u32 distNodes = (lastGlobalId - m_globalIdLastChildInBlock[0]) - 1; // dirty trick: -1 because, as it cant never be 0, by doing that we can extend the range a little bit
			if (distNodes <= CHILDANDDISTINONEBYTE_MAX_DIST)
			{
				u8 val = children | (distNodes << CHILDANDDISTINONEBYTE_CHILD_BITS) | FL_CHILDREN_NUMBER_AND_BLOCKDIST_IN_ONE_BYTE;
	...
	if (!childrenRightBefore && !childrenAndDistStoredInFirstByte)
	{
		...
		if (numChildBlocks == 1)
		{
			u32 dist = lastGlobalId - m_globalIdLastChildInBlock[0];
			...
			{
				u8 byteHigh = CHILDBLOCKS_USING_MORE_THAN_8BITS | (dist >> 8);
				u8 byteLow = dist & 255;
		...
		{
			for (i32 i = 0; i < numChildBlocks; ++i)
			{
				u32 dist = lastGlobalId - m_globalIdLastChildInBlock[i];
				u8 byteHigh = CHILDBLOCKS_USING_24BITS | (dist >> 16);
				u16 wordLow = dist & 65535;
				writeBuffer.AddData(&byteHigh, sizeof(byteHigh));
...
#ifdef XMLCPB_COLLECT_STATS
	u32 endSize = writeBuffer.GetDataSize();
...
tukk CNodeLiveWriter::ReadAttrStr(tukk pAttrName)
{
	for (i32 i = 0; i < m_attrs.size(); ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
// used only for debug purposes
bool CNodeLiveWriter::HaveAttr(tukk pAttrName)
{
	for (i32 i = 0; i < m_attrs.size(); ++i)
	{
		if (strcmp(m_attrs[i].GetName(), pAttrName) == 0)
...
void CNodeLiveWriter::CheckHardcodedLimits()
{
	if (!m_Writer.HasInternalError())
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "XMLCPB ERROR: IDTag overflow: %u / %d", m_IDTag, MAX_TAGID);
			ShowExtendedErrorInfo();
			m_Writer.NotifyInternalError();
		}
		else if (numAttrs > MAX_NUM_ATTRS)
		{
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "XMLCPB ERROR: NumAttrs overflow: %u / %d", numAttrs, MAX_NUM_ATTRS);
			ShowExtendedErrorInfo();
			m_Writer.NotifyInternalError();
		}
		else if (m_totalAmountChildren > MAX_NUMBER_OF_CHILDREN_IN_16BITS)
		{
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "XMLCPB ERROR: NumChildren overflow: %u / %d", m_totalAmountChildren, MAX_NUMBER_OF_CHILDREN_IN_16BITS);
			ShowExtendedErrorInfo();
			m_Writer.NotifyInternalError();
		}
		else
		{
			for (u32 a = 0; a < numAttrs; ++a)
			{
...
void CNodeLiveWriter::ShowExtendedErrorInfo() const
{
	...
	string buf;
	for (i32 i = nodesVec.size() - 1; i >= 0; --i)
	{
	...
	DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "Node: %s", buf.c_str());
}
#endif
