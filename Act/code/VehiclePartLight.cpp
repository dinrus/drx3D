...
namespace
{
SVehicleLightParamsConstPtr LoadVehicleLightParams(ISharedParamsUpr* pSharedParamsUpr, tukk inputLightType)
{
	...
	i32k numberOfLights = lightsListNode->getChildCount();

	for (i32 i = 0; i < numberOfLights; ++i)
	{
		...
		tukk type = lightParamsNode->getAttr("type");

		if (!pSharedParamsUpr->Get(type))
		{
			...
			SVehicleLightParamsConstPtr pNewVehicleLightParams = CastSharedParamsPtr<SVehicleLightParams>(pSharedParamsUpr->Register(type, lightParams));
...
//------------------------------------------------------------------------
bool CVehiclePartLight::Init(IVehicle* pVehicle, const CVehicleParams& table, IVehiclePart* parent, CVehicle::SPartInitInfo& initInfo, i32 partType)
{
	...
	if (CVehicleParams lightTable = table.findChild("Light"))
	{
		...
		SVehicleLightParamsConstPtr pVehicleLightParams = CastSharedParamsPtr<SVehicleLightParams>(pSharedParamsUpr->Get(m_lightType.c_str()));

		if (!pVehicleLightParams)
		{
			pVehicleLightParams = LoadVehicleLightParams(pSharedParamsUpr, m_lightType.c_str());
		}
		...
		if (pVehicleLightParams)
		{
			...
			if (pVehicleLightParams->castShadows > 0)
			{
				...
				i32 configSpec = (pLightSpec != NULL) ? pLightSpec->GetIVal() : gEnv->pSystem->GetConfigSpec(true);
			...
			{
				m_pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(pVehicleLightParams->material.c_str());
			}

			if (pVehicleLightParams->flare.empty() == false && gEnv->pRenderer)
			{
				i32 lensOpticsID = 0;
				if (gEnv->pOpticsUpr->Load(pVehicleLightParams->flare.c_str(), lensOpticsID))
				{
					m_light.SetLensOpticsElement(gEnv->pOpticsUpr->GetOptics(lensOpticsID));

					i32 modularAngle = ((i32)pVehicleLightParams->flareFOV) % 360;
					if (modularAngle == 0)
						m_light.m_LensOpticsFrustumAngle = 255;
					else
						m_light.m_LensOpticsFrustumAngle = (u8)(pVehicleLightParams->flareFOV * (255.0f / 360.0f));
				}
...
void CVehiclePartLight::PostInit()
{
	...
	for (i32 i = 0, nComps = m_pVehicle->GetComponentCount(); i < nComps; ++i)
	{
		IVehicleComponent* pComponent = m_pVehicle->GetComponent(i);

		for (i32 j = 0, nParts = pComponent->GetPartCount(); j < nParts; ++j)
		{
