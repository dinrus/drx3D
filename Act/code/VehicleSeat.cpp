...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Implements a standard seat for vehicles
...
CDrxAction* CVehicleSeat::m_pGameFramework = NULL;
...
CVehicleSeat::~CVehicleSeat()
{
	DRX_ASSERT_MESSAGE(!m_passengerId, "Passenger should be long gone by now.");
	...
	for (TVehicleSeatActionVector::iterator ite = m_seatActions.begin(); ite != m_seatActions.end(); ++ite)
	{
		...
		DRX_ASSERT(seatActionData.pSeatAction);
...
{
template<typename T>
T GetAttrWithDefault(const CVehicleParams& seatTable, tukk name, const T& def)
{
	T value = def;
...
bool CVehicleSeat::Init(IVehicle* pVehicle, TVehicleSeatId seatId, const CVehicleParams& seatTable)
{
	...
	if (!m_pGameFramework)
	{
		m_pGameFramework = CDrxAction::GetDrxAction();
	}
	...
	{
		i32 viewCount = viewsTable.getChildCount();
	...
	if (CVehicleParams soundsTable = seatTable.findChild("Sounds"))
	{
		...
		tukk moodName = soundsTable.getAttr("moodName");
...
void CVehicleSeat::OnSpawnComplete()
{
	...
	if (gEnv->bMultiplayer && gEnv->bServer && needsSynch && !IsDemoPlayback() && !(m_pVehicle->GetEntity()->GetFlags() & ENTITY_FLAG_CLIENT_ONLY))
	{
		...
		if (!params.pClass)
		{
			DrxFatalError("VehicleSeatSerializer not registered in the entity system!");
		}

		static_cast<CVehicleSystem*>(CDrxAction::GetDrxAction()->GetIVehicleSystem())->SetInitializingSeat(this);

		IEntity* pSerializerEntity = gEnv->pEntitySystem->SpawnEntity(params);

		static_cast<CVehicleSystem*>(CDrxAction::GetDrxAction()->GetIVehicleSystem())->SetInitializingSeat(0);
...
bool CVehicleSeat::InitSeatActions(const CVehicleParams& seatTable)
{
	if (IsDriver())
	{
		...
		{
			DRX_ASSERT(0 && "[CVehicleSeat::InitSeatActions] failed to init movement action");
	...
	i32 i = 0;
	i32 c = seatActionsTable.getChildCount();
...
bool CVehicleSeat::Enter(EntityId actorId, bool isTransitionEnabled)
{
	...
	if (gEnv->bMultiplayer && !CDrxActionCVars::Get().g_multiplayerEnableVehicles)
		return false;
	...
	if (doTransition)
	{
		...
		i32 seatID = pSeatToEnter->GetSeatId();
	...
	DrxLog("[VEHICLE] [%s] ENTERING VEHICLE: Vehicle[%u] Passenger[%u]", gEnv->bServer ? "SERVER" : "CLIENT", m_pVehicle->GetEntityId(), actorId);
...
bool CVehicleSeat::EnterRemotely(EntityId actorId)
{
	...
	IActor* pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(actorId);
...
bool CVehicleSeat::SitDown()
{
	...
	{
		pAISystem->GetSmartObjectUpr()->AddSmartObjectState(pActor->GetEntity(), "InVehicle");
	}
	...
	CDrxAction::GetDrxAction()->GetIGameplayRecorder()->Event(pActor->GetEntity(), GameplayEvent(eGE_EnteredVehicle, 0, 0, (uk )(EXPAND_PTR)m_pVehicle->GetEntityId()));
...
bool CVehicleSeat::QueueTransition()
{
	...
	{
		for (i32 i = 0, nWait = info.waitFor.size(); i < nWait; ++i)
		{
			CVehicleSeat* pSeat = (CVehicleSeat*)m_pVehicle->GetSeatById(info.waitFor[i]);
			DRX_ASSERT(pSeat);

			i32 trans = pSeat->GetCurrentTransition();
...
bool CVehicleSeat::StandUp()
{
	...
	DrxLog("Making %s leave %s", actorTextDescription.c_str(), vehicleTextDescription.c_str());
	...
	if (pAISystem && pAISystem->IsEnabled())
	{
		pAISystem->GetSmartObjectUpr()->RemoveSmartObjectState(pActor->GetEntity(), "InVehicle");
	}
	...
	DrxLog("[VEHICLE] [%s] EXITING VEHICLE:  Vehicle[%u] Passenger[%u]", gEnv->bServer ? "SERVER" : "CLIENT", m_pVehicle->GetEntityId(), pActor->GetEntityId());
	...
	CDrxAction::GetDrxAction()->GetIGameplayRecorder()->Event(pActor->GetEntity(), GameplayEvent(eGE_LeftVehicle, 0, 0, (uk )(EXPAND_PTR)m_pVehicle->GetEntityId()));
...
IActor* CVehicleSeat::GetPassengerActor(bool remoteUser /*=false*/) const
{
	...
	return CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(id);
}
...
//------------------------------------------------------------------------
i32 CVehicleSeat::GetSeatActionId(IVehicleSeatAction* pAction) const
{
	i32 action = 0;
...
//------------------------------------------------------------------------
const IVehicleSeatAction* CVehicleSeat::GetSeatActionById(i32 id) const
{
	if (id >= 0)
	{
		i32 action = 0;
...
bool CVehicleSeat::IsFree(IActor* pInActor)
{
	...
	IActor* pCurrentActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(m_passengerId);
	if (!pCurrentActor)
		return true;
...
//------------------------------------------------------------------------
void CVehicleSeat::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
	...
	if (pSeat->IsGunner() && VehicleCVars().v_driverControlledMountedGuns)
	{
		...
		if (pMC && pTurretAction)
		{
			...
			else
			{
				...
				for (i32 i = 0, count = m_pVehicle->GetEntity()->GetChildCount(); i < count && nskip < 9; ++i)
				{
				...
				i32 hits = gEnv->pPhysicalWorld->RayWorldIntersection(info.weaponPosition, info.aimDirection * 200.f, ent_all,
	...
	else
	{
		if (IActor* pActor = m_pGameFramework->GetIActorSystem()->GetActor(m_passengerId))
		{
			...
			if (ICharacterInstance* pCharInstance = pEntity->GetCharacter(0))
			{
				...
				i16 headId = rIDefaultSkeleton.GetJointIDByName("Bip01 Head");
				if (headId > -1)
				{
...
void CVehicleSeat::UpdateSounds(float deltaTime)
{
	...
	tukk moodName = params.moodName.c_str();
...
void CVehicleSeat::NetSetPassengerId(EntityId passengerId)
{
	...
	{
		//DrxLog("%s::NetSetPassenger: <%s> %i -> %i", m_pVehicle->GetEntity()->GetClass()->GetName(), m_name.c_str(), m_passengerId, passengerId);
		...
		if (exitId)
		{
			...
			{
				DrxLog("[VEHICLE] [%s] SERIALISING EXIT: Vehicle[%u] Passenger[%u]", gEnv->bServer ? "SERVER" : "CLIENT", m_pVehicle->GetEntityId(), m_passengerId);
		...
		if (enterId)
		{
			...
			{
				DrxLog("[VEHICLE] [%s] SERIALISING ENTR: Vehicle[%d] Passenger[%d]", gEnv->bServer ? "SERVER" : "CLIENT", m_pVehicle->GetEntityId(), passengerId);
...
void CVehicleSeat::Serialize(TSerialize ser, EEntityAspects aspects)
{
	if (ser.GetSerializationTarget() != eST_Network)
	{
		...
		else if (ser.IsReading())
		{
			...
			i32 transitionType = 0, queueTransitionType = 0;
		...
		for (TVehicleSeatActionVector::iterator ite = m_seatActions.begin(); ite != m_seatActions.end(); ++ite)
		{
			drx_sprintf(pSeatActionString, "seatAction_%d", ++i);
...
{
	IActor* pActor = m_passengerId ? CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(m_passengerId) : 0;
...
//------------------------------------------------------------------------
float CVehicleSeat::ProcessPassengerDamage(float actorHealth, float damage, i32 hitTypeId, bool explosion)
{
...
void CVehicleSeat::OnPassengerDeath()
{
	...
	if (m_transitionType == eVT_RemoteUsage)
	{
		DRX_ASSERT(0 && "OnPassengerDeath called on remotely used seat");
		return;
	}
	    ...
	    VehicleCVars().v_ragdollPassengers == 1 || m_pVehicle->IsDestroyed() || pActor->IsPlayer())
	{
		...
		if (VehicleCVars().v_debugdraw)
			DrxLog("unlinked %s", pActor->GetEntity()->GetName());
#endif
	}
	else
	{
...
#if ENABLE_VEHICLE_DEBUG
		if (VehicleCVars().v_debugdraw)
			DrxLog("%s: setting transition type eVT_Dying", pActor->GetEntity()->GetName());
#endif
	}
...
#if ENABLE_VEHICLE_DEBUG
		if (VehicleCVars().v_debugdraw)
			DrxLog("hiding %s during destruction of vehicle %s", pActor->GetEntity()->GetName(), m_pVehicle->GetEntity()->GetName());
#endif
...
#if ENABLE_VEHICLE_DEBUG
		if (VehicleCVars().v_debugdraw)
			DrxLog("%s: setting ragdoll physicalization profle", pActor->GetEntity()->GetName());
#endif
...
//------------------------------------------------------------------------
void CVehicleSeat::OnCameraShake(float& angleAmount, float& shiftAmount, const Vec3& pos, tukk source) const
{
...
bool CVehicleSeat::TestExitPosition(IActor* pActor, Vec3& testPos, EntityId* pBlockingEntity)
{
	...
	if (m_pVehicle->GetStatus().speed > 1.0f)
	{
		...
		if (IPhysicalEntity* pEntityPhysics = pEntity->GetPhysics())
		{
			if (pEntityPhysics->GetStatus(&dyn))
			{
				...
				{
					//DrxLog("Disallowing vehicle exit due to sliding");
					return false;
				}
			}
		}
	...
	i32 nskip = m_pVehicle->GetSkipEntities(pSkipEnts, 10);
...
//------------------------------------------------------------------------
void CVehicleSeat::GetMemoryUsage(IDrxSizer* s) const
{
