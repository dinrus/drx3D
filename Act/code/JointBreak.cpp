...
void SJointBreak::AddSendables(INetSendableSink* pSink, i32 brkId)
{
...
static u32k JointBreakPBTFlags = ePBTF_ChainBreaking;
	#else
static u32k JointBreakPBTFlags = 0;
	#endif
...
i32 CJointBreak::GetVirtualId(IPhysicalEntity* pEnt)
{
	...
	else if (iForeignData == PHYS_FOREIGN_ID_STATIC && pForeignData)
	{
		...
		u32k hash = CObjectSelector::CalculateHash(rn);
		for (size_t i = 0; i < m_states.size(); i++)
...
CObjectSelector CJointBreak::GetObjectSelector(i32 idx)
{
	...
	/*
	   if (!hasJointBreaks)
	   {
	    ...
	    if (CNetworkCVars::Get().BreakageLog)
	      DrxLogAlways("[brk] CJointBreak::BeginPlayback: apply impulse to simulate joint break on object %s", sel.GetDescription().c_str());
...
void CJointBreak::PatchRecording(DynArray<SProceduralSpawnRec>& spawnRecs, DynArray<SJointBreakRec>& jointBreaks)
{
	...
	std::map<i32, i32> rootEntitiesAndBreaks;
	...
	for (std::map<i32, i32>::iterator it = rootEntitiesAndBreaks.begin(); it != rootEntitiesAndBreaks.end(); ++it)
	{
		if (!it->second)
		{
			SJointBreakRec jb;
