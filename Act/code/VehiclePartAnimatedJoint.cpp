...
//------------------------------------------------------------------------
bool CVehiclePartAnimatedJoint::Init(IVehicle* pVehicle, const CVehicleParams& table, IVehiclePart* parent, CVehicle::SPartInitInfo& initInfo, i32 partType)
{
...
void CVehiclePartAnimatedJoint::InitGeometry(const CVehicleParams& table)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "CVehiclePartAnimatedJoint '%s' : Parent part required.", GetName());
		...
		if (m_pCharInstance = m_pAnimatedPart->GetEntity()->GetCharacter(m_pAnimatedPart->GetSlot()))
		{
			...
			if (m_pSharedParameters->m_useOption >= 0 && m_pSharedParameters->m_useOption <= MAX_OPTIONAL_PARTS)
			{
				...
				if (m_pSharedParameters->m_useOption > 0)
				{
					...
					drx_sprintf(jointName, "%s_option_%i", GetName(), m_pSharedParameters->m_useOption);

					i32 jointId = rIDefaultSkeleton.GetJointIDByName(jointName);
					if (jointId != -1)
						m_jointId = jointId;
				}

				// remove unused statobjs
				for (i32 i = 0; i <= MAX_OPTIONAL_PARTS; ++i)
				{
					if (i == m_pSharedParameters->m_useOption)
						continue;

					if (i == 0)
						drx_sprintf(jointName, "%s", GetName());
					else
						drx_sprintf(jointName, "%s_option_%i", GetName(), i);
					...
					if (jointId > -1)
					{
						...
						for (i32 n = 0, count = rIDefaultSkeleton.GetJointCount(); n < count; ++n)
						{
...
#ifdef VEH_USE_RPM_JOINT
				DrxFixedStringT<32> name = GetName();
...
void CVehiclePartAnimatedJoint::SetMoveable(bool allowTranslationMovement)
{
	...
	DrxCreateClassInstanceForInterface(drxiidof<IAnimationOperatorQueue>(), m_operatorQueue);
	m_pVehicle->SetObjectUpdate(this, IVehicle::eVOU_PassengerUpdate);
}

//------------------------------------------------------------------------
bool CVehiclePartAnimatedJoint::ChangeState(EVehiclePartState state, i32 flags)
{
	...
	if (m_pCharInstance && m_jointId > -1)
	{
		...
		if (!m_isMoveable)
		{
			...
			if (pSkeletonAnim && (forced || pSkeletonAnim->GetNumAnimsInFIFO(0)))
			{
				...
				DRX_ASSERT(relJointQuat.IsValid());
...
void CVehiclePartAnimatedJoint::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
