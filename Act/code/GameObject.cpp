...
i32 CGameObject::m_nAddingExtension = 0;
...
tukk CGameObject::UpdateNames[eUS_COUNT_STATES] =
{
...
tukk CGameObject::EventNames[eUSE_COUNT_EVENTS] =
{
	"BecomeVisible",
	"BecomeClose",
	"BecomeFarAway",
...
static i32 g_TextModeY = 0;
static float g_y;
static CTimeValue g_lastUpdate;
static i32 g_showUpdateState = 0;
...
static i32 g_forceFastUpdate = 0;
static i32 g_visibilityTimeout = 0;
...
bool CGameObject::IsProbablyVisible()
{
			...
			if (fDiff > g_visibilityTimeoutTime)
			{
				if (m_bVisible)
				{
					if (g_visibilityTimeout == 2)
						DrxLogAlways("Entity %s not visible for %f seconds - visibility timeout", GetEntity()->GetName(), fDiff);
				}
				m_bVisible = false;
				return (false);
			}
	...
	{
		if (g_visibilityTimeout == 2)
			DrxLogAlways("Entity %s is now visible again after visibility timeout", GetEntity()->GetName());
	}
	m_bVisible = true;
...
//------------------------------------------------------------------------
bool CGameObject::ShouldUpdateSlot(const SExtension* pExt, u32 slot, u32 slotbit, bool checkAIDisable)
{
	if (checkAIDisable)
...
void CGameObject::OnInitEvent()
{
	...
	if (!m_extensions.empty())
	{
		...
		{
			tukk szExtensionName = pGOS->GetName(iter->id);
...
void CGameObject::DebugUpdateState()
{
	...
	if (g_showUpdateState == 2)
	{
		...
		for (TExtensions::iterator iter = m_extensions.begin(); iter != m_extensions.end(); ++iter)
		{
			...
			for (i32 slot = 0; slot < MAX_UPDATE_SLOTS_PER_EXTENSION; slot++)
			{
				pOut = buf;
				pOut += sprintf(pOut, "%s[%d]: en:%u", m_pGOS->GetName(iter->id), slot, iter->updateEnables[slot]);

				if (iter->forceEnables[slot])
					pOut += sprintf(pOut, " force:%d", i32(iter->forceEnables[slot]));
...
void CGameObject::Update(SEntityUpdateContext& ctx)
{
	...
	DRX_ASSERT(numRemovingExtensions == NOT_IN_UPDATE_MODE);
	DRX_ASSERT(updatingEntity == 0);
	...
	{
#ifdef _DEBUG
		tukk name = pGameObjectSystem->GetName(iter->id);
#endif
		u32 slotbit = 1;
	 ...
	 * ADD EXTENSIONS
	 */
	for (i32 i = 0; i < m_nAddingExtension; i++)
	...
	DRX_ASSERT(numRemovingExtensions != NOT_IN_UPDATE_MODE);
	DRX_ASSERT(updatingEntity == GetEntityId());
	numRemovingExtensions = NOT_IN_UPDATE_MODE;
	updatingEntity = 0;
}

void CGameObject::ForceUpdateExtension(IGameObjectExtension* pExt, i32 slot)
{
...
//------------------------------------------------------------------------
IGameObjectExtension* CGameObject::GetExtensionWithRMIBase(ukk pBase)
{
	for (TExtensions::iterator iter = m_extensions.begin(); iter != m_extensions.end(); ++iter)
	{
...
bool CGameObject::CaptureProfileUpr(IGameObjectProfileUpr* pPM)
{
	return m_pNetEntity->CaptureProfileUpr(pPM);
}
...
void CGameObject::ReleaseProfileUpr(IGameObjectProfileUpr* pPM)
{
	m_pNetEntity->ReleaseProfileUpr(pPM);
}

void CGameObject::ClearProfileUpr()
{
	m_pNetEntity->ClearProfileUpr();
}

bool CGameObject::HasProfileUpr()
{
	return m_pNetEntity->HasProfileUpr();
}
...
//------------------------------------------------------------------------
void CGameObject::OnAction(const ActionId& actionId, i32 activationMode, float value)
{
...
static tukk AspectProfileSerializationName(i32 i)
{
...
{
	DRX_ASSERT(ser.GetSerializationTarget() != eST_Network);

	if (ser.BeginOptionalGroup("GameObject", true))
	{
		i32 updateState = eUS_INVALID;
		if (ser.IsWriting())
			updateState = (i32) m_updateState;
		ser.Value("updateState", updateState);
		if (ser.IsReading())
			m_updateState = (EUpdateState) updateState;

		u8k profileDefault = 255;
		DrxFixedStringT<32> name;
		IGameObjectSystem* pGameObjectSystem = m_pGOS;
		std::vector<DrxFixedStringT<32>> activatedExtensions;
		...
		if (ser.IsWriting())
		{
			...
			if (extensionsCount > 1) // when we have many extensions, sort by serialization priority
			{
				...
				sortingExtensionsList.resize(m_extensions.size());
				{
					...
					for (u32 i = 0; iterS != m_extensions.end(); ++iterS, ++iterD, ++i)
					{
			...
			u8 profile = profileDefault;
			if (m_pNetEntity->HasProfileUpr())
			{
		...
		else //reading
		{
			...
			ser.Value("activatedExtensions", activatedExtensions);
			for (std::vector<DrxFixedStringT<32>>::iterator iter = activatedExtensions.begin(); iter != activatedExtensions.end(); ++iter)
...
//------------------------------------------------------------------------
bool CGameObject::NetSerializeEntity(TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags)
{
...
//------------------------------------------------------------------------
void CGameObject::InitClient(i32 channelId)
{
	for (TExtensions::iterator iter = m_extensions.begin(); iter != m_extensions.end(); ++iter)
...
//------------------------------------------------------------------------
void CGameObject::PostInitClient(i32 channelId)
{
	for (TExtensions::iterator iter = m_extensions.begin(); iter != m_extensions.end(); ++iter)
...
//------------------------------------------------------------------------
template<class T>
ILINE bool CGameObject::DoGetSetExtensionParams(tukk extension, SmartScriptTable params)
{
	IGameObjectSystem* pGameObjectSystem = m_pGOS;
	SExtension ext;
...
IGameObjectExtension* CGameObject::QueryExtension(tukk extension) const
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
...
bool CGameObject::GetExtensionParams(tukk extension, SmartScriptTable params)
{
	return DoGetSetExtensionParams<CSerializeScriptTableWriterImpl>(extension, params);
}

bool CGameObject::SetExtensionParams(tukk extension, SmartScriptTable params)
{
	return DoGetSetExtensionParams<CSerializeScriptTableReaderImpl>(extension, params);
}

//------------------------------------------------------------------------
IGameObjectExtension* CGameObject::ChangeExtension(tukk name, EChangeExtension change)
{
		...
		if (ext.id != IGameObjectSystem::InvalidExtensionID)
		{
			...
			if (change == eCE_Activate || change == eCE_Acquire)
			{
				...
				else
				{
					...
					if (ext.pExtension)
					{
						...
						{
							DRX_ASSERT(m_nAddingExtension < MAX_ADDING_EXTENSIONS);
							PREFAST_ASSUME(m_nAddingExtension < MAX_ADDING_EXTENSIONS);
							if (m_nAddingExtension == MAX_ADDING_EXTENSIONS)
								DrxFatalError("Too many extensions added in a single frame");
			...
			else // change == eCE_Deactivate || change == eCE_Release
			{
				...
				if (iter == m_extensions.end() || iter->id != ext.id)
				{
					...
					{
						DRX_ASSERT(0);
						DrxFatalError("Attempt to release a non-existant extension %s", name);
					}
				}
				else
				{
					...
					if (!iter->refCount && !iter->activated && !iter->sticky)
					{
						if (updatingEntity == GetEntityId() && numRemovingExtensions != NOT_IN_UPDATE_MODE)
						{
							if (numRemovingExtensions == MAX_REMOVING_EXTENSIONS)
								DrxFatalError("Trying to removing too many game object extensions in one frame");
...
void CGameObject::FlushExtensions(bool includeStickyBits)
{
	...
	DRX_ASSERT(pActivatedExtensions->empty());
	for (TExtensions::iterator iter = m_extensions.begin(); iter != m_extensions.end(); ++iter)
	{
	...
	{
		//DRX_ASSERT( m_extensions.empty() );

		// [10/12/2009 evgeny] The only way to ensure that "0 == m_pProfileUpr" in all cases
		// is to release the profile manager here explicitly.
		// It's basically just zeroing the pointer, so not memory leak is expected.
		m_pNetEntity->ClearProfileUpr();

		//DRX_ASSERT(0 == m_pActionDelegate);
		//DRX_ASSERT(0 == m_pViewDelegate);
	}

	inFlush = wasInFlush;
}

u8 CGameObject::GetUpdateSlotEnables(IGameObjectExtension* pExtension, i32 slot)
{
...
void CGameObject::EnableUpdateSlot(IGameObjectExtension* pExtension, i32 slot)
{
	SExtension* pExt = GetExtensionInfo(pExtension);
	if (pExt)
	{
		DRX_ASSERT_TRACE(255 != pExt->updateEnables[slot], ("Already got 255 reasons for slot %d of '%s' to be enabled", slot, GetEntity()->GetEntityTextDescription().c_str()));
		++pExt->updateEnables[slot];
	}
	EvaluateUpdateActivation();
}

void CGameObject::DisableUpdateSlot(IGameObjectExtension* pExtension, i32 slot)
{
	SExtension* pExt = GetExtensionInfo(pExtension);
	if (pExt)
	{
...
void CGameObject::SendEvent(const SGameObjectEvent& event)
{
	...
	if (event.flags & eGOEF_ToScriptSystem)
	{
		...
		{
			tukk eventName = m_pGOS->GetEventName(event.event);
	...
	if (event.flags & eGOEF_ToExtensions)
	{
		...
		{
			if (CDrxActionCVars::Get().g_handleEvents == 1)
			{
...
void CGameObject::SetChannelId(u16 id)
{
...
u16 CGameObject::GetChannelId() const
{
...
void CGameObject::SetUpdateSlotEnableCondition(IGameObjectExtension* pExtension, i32 slot, EUpdateEnableCondition condition)
{
...
bool CGameObject::ShouldUpdate()
{
	...
	{
		u32 slotbit = 1;
		for (i32 slot = 0; slot < 4 && !shouldBeActivated; slot++)
		{
...
void CGameObject::EvaluateUpdateActivation()
{
	...
	{
		u32 slotbit = 1;
		for (u32 slot = 0; slot < 4 && !shouldBeActivated != 0; slot++)
		{
			shouldBeActivated |= ShouldUpdateSlot(&*iter, slot, slotbit, checkAIDisableOnSlots);
			slotbit <<= 1;
		}
...
void CGameObject::SetPhysicsDisable(bool disablePhysics)
{
	if (disablePhysics != m_bPhysicsDisabled)
	{
		...
		//DrxLogAlways("[gobj] %s physics on %s", disablePhysics? "disable" : "enable", GetEntity()->GetName());
...
/*
   ...
   {
   //		DrxLogAlways("%s is now %svisible", m_pEntity->GetName(), isVisible? "" : "in");
...
u8 CGameObject::GetAspectProfile(EEntityAspects aspect)
{
	return m_pNetEntity->GetAspectProfile(aspect);
}

bool CGameObject::SetAspectProfile(EEntityAspects aspect, u8 profile, bool fromNetwork)
{
...
u8 CGameObject::GetDefaultProfile(EEntityAspects aspect)
{
...
void CGameObject::Pulse(u32 pulse)
{
...
void CGameObject::GetMemoryUsage(IDrxSizer* s) const
{
	{
		SIZER_SUBCOMPONENT_NAME(s, "GameObject");
		s->AddObject(this, sizeof(*this));
		s->AddObject(m_extensions);
		for (i32 i = 0; i < CGameObject::MAX_ADDING_EXTENSIONS; ++i)
...
i32 CGameObject::GetPredictionHandle()
{
	return m_predictionHandle;
}

void CGameObject::RegisterAsValidated(IGameObject* pGO, i32 predictionHandle)
{
	if (!pGO)
		return;
...
#if GAME_OBJECT_SUPPORTS_CUSTOM_USER_DATA
uk CGameObject::GetUserData() const
{
	return m_pUserData;
}

void CGameObject::SetUserData(uk ptr)
{
	m_pUserData = ptr;
}
#endif

void CGameObject::RegisterExtForEvents(IGameObjectExtension* piExtention, i32k* pEvents, i32k numEvents)
{
...
void CGameObject::UnRegisterExtForEvents(IGameObjectExtension* piExtention, i32k* pEvents, i32k numEvents)
{
	SExtension* pExtension = GetExtensionInfo(piExtention);
	if (pExtension)
	{
...
void CGameObject::OnNetworkedEntityTransformChanged(EntityTransformationFlagsMask transformReasons)
{
	if (gEnv->bMultiplayer && (m_pEntity->GetFlags() & (ENTITY_FLAG_CLIENT_ONLY | ENTITY_FLAG_SERVER_ONLY)) == 0 && gEnv->pNetContext)
	{
		...
		// disabled ... see OnSpawn for reasoning
		if (doAspectUpdate)
		{
