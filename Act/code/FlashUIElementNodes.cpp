...
// -------------------------------------------------------------------------
//  Имя файла:   FlashUIElementNodes.cpp
//  Version:     v1.00
...
void CFlashUIVariableBaseNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	else if (event == eFE_Activate)
	{
		...
		i32k instanceId = GetPortInt(pActInfo, GetInputPort(eI_InstanceID));
...
void CFlashUIGotoAndPlayBaseNode::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig<i32>("FrameId",     -1,                                              "Frame Number, -1 = use FrameName"),
...
void CFlashUIGotoAndPlayBaseNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	else if (event == eFE_Activate)
	{
		...
		if (GetElement())
		{
			i32k frameId = GetPortInt(pActInfo, GetInputPort(eI_FrameNum));
			...
			{
				SPerInstanceCall2<i32, tukk > caller;
...
////////////////////////////////////////////////////////////////////////////
void CFlashUIGotoAndPlayBaseNode::GotoAndPlay(IUIElement* pElement, i32 frameId, tukk frameName)
{
...
////////////////////////////////////////////////////////////////////////////
void CFlashUIGotoAndPlayBaseNode::GotoAndStop(IUIElement* pElement, i32 frameId, tukk frameName)
{
	IFlashVariableObject* pMc = pElement->GetMovieClip(GetObjectDesc(), GetTmplDesc());
	if (pMc)
	{
...
CFlashUIEventNode::CFlashUIEventNode(IUIElement* pUIElement, string sCategory, const SUIEventDesc* pEventDesc)
	...
	, m_iCurrInstanceId(-1)
{
	DRX_ASSERT_MESSAGE(pEventDesc, "NULL pointer passed!");
	...
	m_outPorts.push_back(OutputPortConfig<i32>("instanceID", "The instance ID of the element that raised the event"));
...
void CFlashUIEventNode::FlushNextEvent(SActivationInfo* pActInfo)
{
	if (m_events.size() > 0)
	{
		const std::pair<SUIArguments, i32>& data = m_events.get();
		const SUIArguments& args = data.first;
		bool bTriggerEvent = true;
		i32k checkValue = GetPortInt(pActInfo, eI_CheckPort);

		if (checkValue >= 0)
		{
			bTriggerEvent = false;
			DRX_ASSERT_MESSAGE(checkValue < args.GetArgCount(), "Port does not exist!");
		...
		if (bTriggerEvent)
		{
			i32 end = m_eventDesc.InputParams.Params.size();
			string val;

			i32 i = 0;
			for (; i < end; ++i)
			{
				DRX_ASSERT_MESSAGE(i < args.GetArgCount(), "UIEvent received wrong number of arguments!");
...
{
	DRX_ASSERT_MESSAGE(pFuncDesc, "NULL pointer passed!");
...
void CFlashUIElementListenerNode::GetConfiguration(SFlowNodeConfig& config)
{
	...
	static const SOutputPortConfig out_config[] = {
		OutputPortConfig<i32>("instanceID",          "Instance ID for the element that fired the event"),
...
void CFlashUIElementListenerNode::FlushNextEvent(SActivationInfo* pActInfo)
{
	if (m_events.size() > 0)
	{
		const std::pair<EOutputs, i32>& data = m_events.get();
