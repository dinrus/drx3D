...
#include <drx3D/Act/DeformingBreak.h> // for debug function
...
i32 CActionGame::g_procedural_breaking = 0;
i32 CActionGame::g_joint_breaking = 1;
float CActionGame::g_tree_cut_reuse_dist = 0;
i32 CActionGame::g_no_secondary_breaking = 0;
i32 CActionGame::g_no_breaking_by_objects = 0;
i32 CActionGame::g_breakage_mem_limit = 0;
i32 CActionGame::g_breakage_debug = 0;
i32 CActionGame::s_waterMaterialId = -1;
float g_breakageFadeTime = 0.f;
float g_breakageFadeDelay = 0.f;
float g_breakageMinAxisInertia = 0.01f;
i32 g_breakageNoDebrisCollisions = 0;
float g_glassForceTimeout = 0.f;
float g_glassForceTimeoutSpread = 0.f;
i32 g_glassNoDecals = 0;
i32 g_glassAutoShatter = 0;
i32 g_glassAutoShatterOnExplosions = 0;
i32 g_glassMaxPanesToBreakPerFrame = 0;
float CActionGame::g_glassAutoShatterMinArea = 0.f;
i32 g_glassSystemEnable = 1;
/*
   ============================================================================================================================================
   ...
   ============================================================================================================================================
 */
i32 g_breakageTreeMax = 0;
i32 g_breakageTreeInc = 0;
i32 g_breakageTreeDec = 0;
i32 g_breakageTreeIncGlass = 0;
i32 g_waterHitOnly = 0;
#if BREAKAGE_THROTTLE_LOGGING
	#define ThrottleLogAlways DrxLogAlways
#else
	...
	, m_pendingPlaneBreaks(10)
{
	...
	m_pGameContext = new CGameContext(CDrxAction::GetDrxAction(), pScriptRMI, this);
	m_pGameTokenSystem = CDrxAction::GetDrxAction()->GetIGameTokenSystem();
...
{
#ifndef _RELEASE
	DrxLog("Destroying CActionGame instance %p (level=\"%s\")", this, GetLevelName().c_str());
	...
	{
		IGameSessionHandler* pGameSessionHandler = CDrxAction::GetDrxAction()->GetIGameSessionHandler();
	...
	if (pDrxAction)
		pDrxAction->GetIGameRulesSystem()->DestroyGameRules();
	...
	if (!gEnv->IsDedicated())
	{
		if (ICVar* pDefaultGameRulesCVar = gEnv->pConsole->GetCVar("sv_gamerulesdefault"))
		{
			...
			tukk szDefaultGameRules = pDefaultGameRulesCVar->GetString();
	...
	gEnv->bServer = false;
	{
		const IGameSessionHandler* pGameSessionHandler = CDrxAction::GetDrxAction()->GetIGameSessionHandler();
...
{
	UnloadPhysicsData();
	CDrxAction::GetDrxAction()->GetILevelSystem()->UnLoadLevel();
}
...
ILINE bool CActionGame::AllowProceduralBreaking(u8 proceduralBreakType)
{
	// bit 1 : test state
	// bit 2 : and (true), or (false)
	// bit 3 : current state
	// bit 4 : skip flag
	static u16k truth = 0xf0b2;
...
bool CActionGame::Init(const SGameStartParams* pGameStartParams)
{
	...
	DRX_ASSERT(m_pGameServerNub == NULL);
	DRX_ASSERT(m_pServerNub == NULL);
	...
	else
	{
		...
		{
			tukk pLevelName = pLobby->GetDinrusXLevelNameHint();
			if (strlen(pLevelName) != 0)
			{
				m_pGameContext->SetLevelName(pLevelName);
			}
			tukk pRulesName = pLobby->GetDinrusXRulesNameHint();
	...
	tukk configName = isMultiplayer ? "multiplayer" : "singleplayer";
	...
	CDrxAction::GetDrxAction()->AllowSave(true);
	CDrxAction::GetDrxAction()->AllowLoad(true);
...
void CActionGame::ServerInit(const SGameStartParams* pGameStartParams, bool* io_ok, bool* io_hasPbSvStarted)
{
	...
	DRX_ASSERT(m_pGameContext->GetServerPort() != 0);

#ifdef  __WITH_PB__
	if (CDrxAction::GetDrxAction()->IsPbSvEnabled() && gEnv->bMultiplayer)
	{
	...
	{
		drx_sprintf(address, "%s:%u", LOCAL_CONNECTION_STRING, pGameStartParams->port);
	}
	else
	{
		...
		{
			drx_sprintf(address, "%s:%u", pCVar->GetString(), pGameStartParams->port);
		}
		else
		{
			drx_sprintf(address, "0.0.0.0:%u", pGameStartParams->port);
		}
	}
...
void CActionGame::ClientInit(const SGameStartParams* pGameStartParams, bool* io_ok, bool* io_hasPbSvStarted, bool* io_requireBlockingConnection)
{
...
#ifdef __WITH_PB__
	if (hasPbSvStarted || CDrxAction::GetDrxAction()->IsPbClEnabled() && gEnv->bMultiplayer)
		gEnv->pNetwork->StartupPunkBuster(false);
#endif

	m_pGameClientNub = new CGameClientNub(CDrxAction::GetDrxAction());
	...
	{
		if (pGameStartParams->session != DrxSessionInvalidHandle)
		{
...
void CActionGame::LogModeInformation(const bool isMultiplayer, tukk hostname) const
{
	assert(gEnv->pSystem);

	if (gEnv->IsEditor())
	{
		DrxLogAlways("Starting in Editor mode");
	}
	else if (gEnv->IsDedicated())
	{
		DrxLogAlways("Starting in Dedicated server mode");
	}
	else if (!isMultiplayer)
	{
		DrxLogAlways("Starting in Singleplayer mode");
	}
	else if (IsServer())
	{
		DrxLogAlways("Starting in Server mode");
	}
	else if (m_pGameContext->HasContextFlag(eGSF_Client))
	{
		DrxLogAlways("Starting in Client mode");
		...
		DrxLogAlways("ServerName: %s", address.c_str());
	}
}
...
void CActionGame::UpdateImmersiveness()
{
	...
	m_proceduralBreakFlags =
	  (i32)/*(!procMP) **/ ePBF_ObeyCVar +
	  (i32)procMP * ePBF_AllowGlass +
	  (i32)(immMP || !procMP) * ePBF_DefaultAllow;
	if (!m_pGameContext->HasContextFlag(eGSF_Server))
...
{
	return CDrxAction::GetDrxAction()->IsGameStarted() && !CDrxAction::GetDrxAction()->IsGamePaused();
}

CActionGame::eInitTaskState CActionGame::NonBlockingConnect(BlockingConditionFunction condition, bool requireClientChannel, tukk conditionText)
{
	bool done = false;
...
bool CActionGame::BlockingConnect(BlockingConditionFunction condition, bool requireClientChannel, tukk conditionText)
{
	...
	{
		float numSecondsTaken = (pTimer->GetAsyncTime() - startTime).GetSeconds();
		DrxLog("BlockingConnect: Failed to achieve condition '%s' (tried for %.2f seconds)", conditionText, numSecondsTaken);
	}
...
bool CActionGame::ChangeGameContext(const SGameContextParams* pGameContextParams)
{
	if (!IsServer())
	{
		...
		DRX_ASSERT(!"Can't ChangeGameContext() on client");
		return false;
	}

	DRX_ASSERT(pGameContextParams);
...
IActor* CActionGame::GetClientActor()
{
	...
	return CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(playerId);
}
...
bool CActionGame::Update()
{
	if (m_initState == eIS_InitDone)
	{
		...
		{
			s_waterMaterialId = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr()->GetSurfaceTypeByName("mat_water")->GetId();
		}
...
bool CActionGame::IsStale()
{
	if (m_pGameClientNub && m_pClientNub)
		if (!m_pClientNub->IsConnecting() && !m_pGameClientNub->GetGameClientChannel() && !CDrxAction::GetDrxAction()->IsGameSessionMigrating())
			return true;
...
IHostMigrationEventListener::EHostMigrationReturn CActionGame::OnInitiate(SHostMigrationInfo& hostMigrationInfo, HMStateType& state)
{
	...
	DrxLogAlways("[Host Migration]: CActionGame::OnInitiate() started");
	...
	if (pGameClientChannel != NULL)
	{
		...
		{
			DrxLogAlways("[Host Migration]: no actor found - aborting");
			return IHostMigrationEventListener::Listener_Terminate;
		}
	}

	DrxLogAlways("[Host Migration]: CActionGame::OnInitiate() finished");
	return IHostMigrationEventListener::Listener_Done;
}
...
IHostMigrationEventListener::EHostMigrationReturn CActionGame::OnDisconnectClient(SHostMigrationInfo& hostMigrationInfo, HMStateType& state)
{
	...
	{
	case eDS_Disconnect:
		DrxLogAlways("[Host Migration]: CActionGame::OnDisconnectClient() started");
		// Do the disconnect
		if (m_pGameClientNub)
		{
		...
		{
			DrxLog("[Host Migration]: CActionGame::OnDisconnectClient() - m_pClientNub is NULL");
		}
	...
	case eDS_Disconnecting:
		DrxLogAlways("[Host Migration]: CActionGame::OnDisconnectClient() waiting");
	...
	case eDS_Disconnected: // Intentional fall-through
		if (CDrxAction::GetDrxAction()->GetIMaterialEffects())
		{
			...
			CDrxAction::GetDrxAction()->GetIMaterialEffects()->ClearDelayedEffects();
		}

		DrxLogAlways("[Host Migration]: CActionGame::OnDisconnectClient() finished");
	default:
		done = true;
		break;
	}
...
IHostMigrationEventListener::EHostMigrationReturn CActionGame::OnDemoteToClient(SHostMigrationInfo& hostMigrationInfo, HMStateType& state)
{
	...
	DrxLogAlways("[Host Migration]: CActionGame::OnDemoteToClient() started");
	...
	DrxLogAlways("[Host Migration]: CActionGame::OnDemoteToClient() finished");
	return IHostMigrationEventListener::Listener_Done;
}
...
IHostMigrationEventListener::EHostMigrationReturn CActionGame::OnPromoteToServer(SHostMigrationInfo& hostMigrationInfo, HMStateType& state)
{
	...
	DrxLogAlways("[Host Migration]: CActionGame::OnPromoteToServer() started");
	...
	i32 maxPlayers = pCVar->GetIVal();

	// Set the server name
	IPlayerProfileUpr* pPlayerProfileUpr = gEnv->pGameFramework->GetIPlayerProfileUpr();
	DrxFixedStringT<128> serverName(s_this->m_pNetwork->GetHostName());
	serverName.append(" ");
	serverName.append(pPlayerProfileUpr->GetCurrentProfile(pPlayerProfileUpr->GetCurrentUser())->GetName());
	...
	if (pCVar && pCVar->GetString())
	{
		drx_sprintf(address, "%s:%u", pCVar->GetString(), s_this->m_pGameContext->GetServerPort());
	}
	else
	{
		drx_sprintf(address, "0.0.0.0:%u", s_this->m_pGameContext->GetServerPort());
	}
	...
	{
		// Failed
		DrxLogAlways("Host migration error: unable to create server");
		return IHostMigrationEventListener::Listener_Terminate;
	}

	DrxLogAlways("[Host Migration]: CActionGame::OnPromoteToServer() finished");
#ifndef _RELEASE
	m_timeToPromoteToServer = 0.f;
#endif
	return IHostMigrationEventListener::Listener_Done;
...
IHostMigrationEventListener::EHostMigrationReturn CActionGame::OnReconnectClient(SHostMigrationInfo& hostMigrationInfo, HMStateType& state)
{
	...
	case eRS_Reconnect:
		{
			DrxLogAlways("[Host Migration]: CActionGame::OnReconnectClient() started");
			...
			if (s_this->m_pClientNub)
			{
				...
				DrxLogAlways("[Host Migration]: Will use %s as connection address", address.c_str());
	...
	case eRS_Reconnecting:
		DrxLogAlways("[Host Migration]: CActionGame::OnReconnectClient() waiting");
	...
	case eRS_Terminated:
		// Failed
		DrxLogAlways("Host migration error: unable to reconnect client");
		return IHostMigrationEventListener::Listener_Terminate;

	case eRS_Reconnected: // Intentional fall-through
		DrxLogAlways("[Host Migration]: CActionGame::OnReconnectClient() finished");
	default:
...
IHostMigrationEventListener::EHostMigrationReturn CActionGame::OnFinalise(SHostMigrationInfo& hostMigrationInfo, HMStateType& state)
{
	...
	DrxLogAlways("[Host Migration]: CActionGame::OnFinalise() started");
	...
	DrxLogAlways("[Host Migration]: CActionGame::OnFinalise() finished");
	return IHostMigrationEventListener::Listener_Done;
}
...
void CActionGame::AddGlobalPhysicsCallback(i32 event, void (* proc)(const EventPhys*, uk ), uk userdata)
{
	i32 idx = (event & (0xff << 8)) != 0;
...
void CActionGame::RemoveGlobalPhysicsCallback(i32 event, void (* proc)(const EventPhys*, uk ), uk userdata)
{
	i32 idx = (event & (0xff << 8)) != 0;
	if (event & eEPE_OnCollisionLogged || event & eEPE_OnCollisionImmediate)
...
i32 CActionGame::OnBBoxOverlap(const EventPhys* pEvent)
{
	...
	if (pOverlap->iForeignData[1] == PHYS_FOREIGN_ID_STATIC)
	{
		...
		if (rn != NULL && eERType_Vegetation == rn->GetRenderNodeType())
		{
			...
			while (phys != 0)
			{
				...
				//DrxLogAlways("colldyn: %d collstat: %d", sr.nCollDyn, sr.nCollStat);
				idx++;
			...
			if (hit_veg && pEnt)
			{
				...
				if (play_sound)
				{
					IMaterialEffects* pMaterialEffects = CDrxAction::GetDrxAction()->GetIMaterialEffects();
...
i32 CActionGame::OnCollisionLogged(const EventPhys* pEvent)
{
...
bool CActionGame::ProcessHitpoints(const Vec3& pt, IPhysicalEntity* pent, i32 partid, ISurfaceType* pMat, i32 iDamage)
{
	if (m_bLoading)
		return true;
	i32 i, imin, id;
	...
	SEntityHits* phits;
	pe_status_pos sp;
	std::map<i32, SEntityHits*>::iterator iter;
	...
	else
	{
		...
		phits->hitpoints = (i32)physParams.hit_points;
		phits->maxdmg = (i32)physParams.hit_maxdmg;
		phits->lifeTime = physParams.hit_lifetime;
		m_mapEntHits.insert(std::pair<i32, SEntityHits*>(id, phits));
	}
	...
	if (phits->nHits == 0 || (phits->pHits[imin] - ptloc).len2() > sqr(phits->hitRadius) && phits->nHits < phits->nMaxHits)
	{
		if (phits->nHitsAlloc == phits->nHits)
		{
			...
			i32* ns = phits->pnHits;
			memcpy(phits->pnHits = new i32[phits->nHitsAlloc], ns, phits->nHits * sizeof(i32));
...
SBreakEvent& CActionGame::StoreBreakEvent(const SBreakEvent& breakEvent)
{
	...
	CDrxAction::GetDrxAction()->OnBreakEvent(static_cast<u16>(m_breakEvents.size() - 1));

	return m_breakEvents.back();
}

void CActionGame::PerformPlaneBreak(const EventPhysCollision& epc, SBreakEvent* pRecordedEvent, i32 flags, CDelayedPlaneBreak* pDelayedTask)
{
	// TODO: improve
	...
	ISurfaceType* pMat = pSurfaceTypeUpr->GetSurfaceType(epc.idmat[1]);
	if (!pMat)
		return;
	...
	if (pb2d)
	{
		timeout = pb2d->destroy_timeout + pb2d->destroy_timeout_spread * drx_random(-1.0f, 1.0f);
	...
	{
		timeout = g_glassForceTimeout + drx_random(0.0f, 1.0f) * g_glassForceTimeoutSpread;
	}
	...
	if (pStatObj)
	{
		...
		i32k breakEventSize = s_this->m_breakEvents.size();
		...
		if (!pDelayedTask)
		{
			...
			if (!pRecordedEvent || pRecordedEvent->iState == eBES_Generated)
			{
				if (!(pStatObj->GetFlags() & STATIC_OBJECT_GENERATED))
				{
					...
					for (i32 i = 0; i < s_this->m_brokenObjs.size(); i++)
					{
		...
		i32 bIsExplosion = iszero(epc.idmat[0] + 1);  // idmat==-1 is an explosion
		u32 processFlags = 0;
		if (pDelayedTask)
		{
		...
		IBreakableUpr* pBreakableUpr = gEnv->pEntitySystem->GetBreakableUpr();
		SProcessImpactIn in;
		{
			in.pthit = epc.pt;
		...
		{
			IBreakableGlassSystem* pGlassSys = CDrxAction::GetDrxAction()->GetIBreakableGlassSystem();
		...
		else
		{
			...
			result = pBreakableUpr->ProcessPlaneImpact(in, out);
		    ...
		    result == eProcessImpact_DelayedMeshOnly)
		{
			...
			for (i32 i = s_this->m_pendingPlaneBreaks.size() - 1; i >= 0; i--)
			{
				...
				else
				{
					...
					dpbTrg = dpb;
					dpbTrg.m_iBreakEvent = (i32)(&be - &s_this->m_breakEvents[0]);
					dpbTrg.m_epc = epc;
					dpbTrg.m_islandIn.pStatObj->AddRef();
					dpbTrg.m_idx = i;
					dpbTrg.m_count = s_this->m_pendingPlaneBreaks.size();
					gEnv->p3DEngine->GetDeferredPhysicsEventUpr()->DispatchDeferredEvent(&dpbTrg);
					return;
				}
			}
			if (gEnv->bMultiplayer)
			{
				...
				in.bDelay = false;
				pBreakableUpr->ProcessPlaneImpact(in, out);
		...
		//DrxLogAlways("CActionGame::PerformPlaneBreak() - ProcessImpact returned pStatObjNew: 0x%p\n  pStatObjAux: 0x%p", pStatObjNew, pStatObjAux);
...
ForceObjUpdate:
		...
		if (g_glassNoDecals == 0 && !bIsExplosion && pb2d && pb2d->crack_decal_scale && r > 0)
		{
			...
			dcl.fAngle = drx_random(0.0f, 2.0f * gf_PI);
			dcl.bSkipOverlappingTest = true;
			dcl.bForceSingleOwner = true;
			dcl.bDeferred = false;
			drx_strcpy(dcl.szMaterialName, pb2d->crack_decal_mtl);
...
void CActionGame::OnCollisionLogged_Breakable(const EventPhys* pEvent)
{
	...
	ISurfaceTypeUpr* pSurfaceTypeUpr = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr();
	ISurfaceType* pMat = pSurfaceTypeUpr->GetSurfaceType(pCEvent->idmat[1]), * pMat0;
		        ...
		        s_this->ProcessHitpoints(pCEvent->pt, pCEvent->pEntity[1], pCEvent->partid[1], pMat, FtoI(min(1E6f, hitenergy / energy))))))))
		{
			PerformPlaneBreak(*pCEvent, NULL, 0, 0);
		         ...
		         (!(pMat->GetHitpoints()) ||
		          s_this->ProcessHitpoints(pCEvent->pt, pCEvent->pEntity[1], pCEvent->partid[1], pMat, FtoI(min(1E6f, hitenergy / energy)))))
		{
			if (!(pMat0 = pSurfaceTypeUpr->GetSurfaceType(pCEvent->idmat[0])) || !(pMat0->GetFlags() & SURFACE_TYPE_NO_COLLIDE))
			{
				...
				if (pCEvent->pEntity[1]->GetStatus(&sp) && (!s_this->m_bLoading || !pCEvent->pEntity[1]->GetParams(&psj)))
				{
					...
					if (bBreak)
					{
						...
						i32 flags = s_this->m_bLoading ? 0 : 2;
...
void CActionGame::OnCollisionLogged_MaterialFX(const EventPhys* pEvent)
{
	...
	std::map<i32, SEntityCollHist*>::iterator iter;
	...
	else if (s_this->m_pFreeCHSlot0->pnext != s_this->m_pFreeCHSlot0)
	{
		...
		pech->mass = 0;
		s_this->m_mapECH.insert(std::pair<i32, SEntityCollHist*>(id, pech));
	}
	...
	i32k debug = CMaterialEffectsCVars::Get().mfx_Debug & 0x1;
	...
	if (!backface && impactVelSquared > (partImpThresh * partImpThresh))
	{
		...
		i32k defaultSurfaceIndex = pMaterialEffects->GetDefaultSurfaceIndex();
		...
		for (i32 k = 0; k < MAX_CACHED_EFFECTS; k++)
		{
		...
		if (effectId == InvalidEffectId)
		{
			tukk pSrcArchetype = (pEntitySrc && pEntitySrc->GetArchetype()) ? pEntitySrc->GetArchetype()->GetName() : 0;
			tukk pTrgArchetype = (pEntityTrg && pEntityTrg->GetArchetype()) ? pEntityTrg->GetArchetype()->GetName() : 0;
		...
		if (effectId != InvalidEffectId)
		{
			...
			if (debug != 0)
			{
				...
				ISurfaceTypeUpr* pSurfaceTypeUpr = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr();
				DrxLogAlways("[MFX] Running effect for:");
				if (pEntitySrc)
				{
					tukk pSrcName = pEntitySrc->GetName();
					tukk pSrcClass = pEntitySrc->GetClass()->GetName();
					tukk pSrcArchetype = pEntitySrc->GetArchetype() ? pEntitySrc->GetArchetype()->GetName() : "<none>";
					DrxLogAlways("      : SrcClass=%s SrcName=%s Arch=%s", pSrcClass, pSrcName, pSrcArchetype);
				}
				if (pEntityTrg)
				{
					tukk pTrgName = pEntityTrg->GetName();
					tukk pTrgClass = pEntityTrg->GetClass()->GetName();
					tukk pTrgArchetype = pEntityTrg->GetArchetype() ? pEntityTrg->GetArchetype()->GetName() : "<none>";
					DrxLogAlways("      : TrgClass=%s TrgName=%s Arch=%s", pTrgClass, pTrgName, pTrgArchetype);
				}
				DrxLogAlways("      : Mat0=%s", pSurfaceTypeUpr->GetSurfaceType(pCEvent->idmat[0])->GetName());
				DrxLogAlways("      : Mat1=%s", pSurfaceTypeUpr->GetSurfaceType(pCEvent->idmat[1])->GetName());
				DrxLogAlways("impact-speed=%f fx-threshold=%f mass=%f speed=%f", sqrtf(impactVelSquared), partImpThresh, finalparam, adjustedRelativeVelocity);
			}
		}
		else
		{
			if (debug != 0)
			{
				...
				ISurfaceTypeUpr* pSurfaceTypeUpr = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr();
				DrxLogAlways("[MFX] Couldn't find effect for any combination of:");
				if (pEntitySrc)
				{
					tukk pSrcName = pEntitySrc->GetName();
					tukk pSrcClass = pEntitySrc->GetClass()->GetName();
	...
	else
	{
		/*
		   ...
		   ISurfaceTypeUpr *pSurfaceTypeUpr = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr();
		   DrxLogAlways("discarded:");
		   if (pEntitySrc)
		    DrxLogAlways("      : SrcClass: %s SrcName: %s", pEntitySrc->GetClass()->GetName(), pEntitySrc->GetName());
		   if (pEntityTrg)
		    DrxLogAlways("      : TrgClass: %s TrgName: %s", pEntityTrg->GetClass()->GetName(), pEntityTrg->GetName());
		   DrxLogAlways("      : Mat0: %s", pSurfaceTypeUpr->GetSurfaceType(pCEvent->idmat[0])->GetName());
		   DrxLogAlways("      : Mat1: %s", pSurfaceTypeUpr->GetSurfaceType(pCEvent->idmat[1])->GetName());
		   if (backface)
		    DrxLogAlways("backface collision");
		   DrxLogAlways("impact speed squared: %f", impactVelSquared);
		   }
		 */
	}
	...
	//		DrxLogAlways("      : Mat0: %s", pSurfaceTypeUpr->GetSurfaceType(pCEvent->idmat[0])->GetName());
	//		DrxLogAlways("      : Mat1: %s", pSurfaceTypeUpr->GetSurfaceType(pCEvent->idmat[1])->GetName());
	//		DrxLogAlways("rolling speed: %f, rolling time: %f", sqrtf(pech->velRoll2), pech->timeRolling);
	//	}
	//	else
	//	{
	//	}
...
i32 CActionGame::OnPostStepLogged(const EventPhys* pEvent)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
...
void CActionGame::OnPostStepLogged_MaterialFX(const EventPhys* pEvent)
{
	...
	Vec3 pos0 = CDrxAction::GetDrxAction()->GetISystem()->GetViewCamera().GetPosition();

	if ((pPSEvent->pos - pos0).len2() < sqr(maxSoundDist * 1.4f))
	{
		i32 id = s_this->m_pPhysicalWorld->GetPhysicalEntityId(pPSEvent->pEntity);
		std::map<i32, SEntityCollHist*>::iterator iter;
		...
		if ((iter = s_this->m_mapECH.find(id)) != s_this->m_mapECH.end())
		{
			...
			else
			{
				...
				if (pech->velRoll2 > 0.1f)
				{
					...
					//DrxLog("roll %.2f",sqrt_tpl(pech->velRoll2));
					pech->velRoll2 = 0;
...
i32 CActionGame::OnStateChangeLogged(const EventPhys* pEvent)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
	...
	{
		//DrxLogAlways("[0] = %d, [1] = %d", pStateChange->iSimClass[0], pStateChange->iSimClass[1]);
...
void CActionGame::OnStateChangeLogged_MaterialFX(const EventPhys* pEvent)
{
	...
	{
		i32 id = s_this->m_pPhysicalWorld->GetPhysicalEntityId(pSCEvent->pEntity);
		std::map<i32, SEntityCollHist*>::iterator iter;
		if ((iter = s_this->m_mapECH.find(id)) != s_this->m_mapECH.end())
		{
...
i32 CActionGame::OnCreatePhysicalEntityLogged(const EventPhys* pEvent)
{
	...
	DrxComment("CPE: %s", pEntity ? pEntity->GetName() : "Vegetation/Brush Object");
...
void CActionGame::RegisterEntsForBreakageReuse(IPhysicalEntity* pPhysEnt, i32 partid, IPhysicalEntity* pPhysEntNew, float h, float size)
{
	IEntity* pEntity;
...
i32 CActionGame::ReuseBrokenTrees(const EventPhysCollision* pCEvent, float size, i32 flags)
{
	if (g_tree_cut_reuse_dist <= 0 || gEnv->bMultiplayer)
		return 0;
	IRenderNode* pVeg;
		...
		if (rec && (pentSrc = (IEntity*)(pPhysEntSrc = rec->pPhysEntSrc)->GetForeignData(PHYS_FOREIGN_ID_ENTITY)))
		{
			...
			do
			{
				...
				if (epp.type == PE_STATIC)
				{
					...
					(drxcomponent_cast<IEntitySubstitutionComponent*>(pentClone->CreateProxy(ENTITY_PROXY_SUBSTITUTION)))->SetSubstitute(pVeg);
					pVeg->SetPhysics(0);
...
i32 CActionGame::OnUpdateMeshLogged(const EventPhys* pEvent)
{
...
void CActionGame::FreeBrokenMeshesForEntity(IPhysicalEntity* pent)
{
	...
	if (pent)
	{
		i32 id = gEnv->pPhysicalWorld->GetPhysicalEntityId(pent);
		std::map<i32, SBrokenMeshSize>::iterator iter = m_mapBrokenMeshes.lower_bound(id << 8), nextIter;
		while (iter != m_mapBrokenMeshes.end() && (iter->first >> 8) == id)
		{
			DRX_ASSERT(pent == iter->second.pent);
			++(nextIter = iter);
			if (FreeBrokenMesh(pent, iter->second))
			{
				m_totBreakageSize -= iter->second.size;
				DrxLog("Removed broken mesh for entity: %p; new total size %dKb", pent, m_totBreakageSize);
...
i32 CActionGame::OnPhysEntityDeleted(const EventPhys* pEvent)
{
...
static void FreeSlotsAndFoilage(IEntity* pEntity)
{
	...
	for (i32 i = pEntity->GetSlotCount() - 1; i >= 0; i--)
		pEntity->FreeSlot(i);
}

i32 CActionGame::FreeBrokenMesh(IPhysicalEntity* pent, SBrokenMeshSize& bm)
{
	if (!pent || pent != bm.pent)
		return 0;
	IStatObj::SSubObject* pSubObj;
	IStatObj* pStatObj;
	i32 ifd = pent->GetiForeignData();
	uk pfd = pent->GetForeignData(ifd);
	...
	if (bm.timeout > 0 && bm.fractureFX && (pFractureFx = gEnv->pParticleUpr->FindEffect(bm.fractureFX)))
...
void CActionGame::RegisterBrokenMesh(IPhysicalEntity* pPhysEnt, IGeometry* pPhysGeom, i32 partid, IStatObj* pStatObj, IGeometry* pSkel,
                                     float timeout, tukk fractureFX)
{
	if (pPhysEnt && (g_breakage_mem_limit > 0 || timeout > 0.0f))
	{
		...
		if (pPhysGeom)
		{
			...
			if (uk pPlane = pPhysGeom->GetForeignData(1))
			{
				IBreakableUpr* pBreakableUpr = gEnv->pEntitySystem->GetBreakableUpr();
				pBreakableUpr->GetPlaneMemoryStatistics(pPlane, &sizer);
			}
		}
		...
		i32 id = gEnv->pPhysicalWorld->GetPhysicalEntityId(pPhysEnt) << 8 | partid & 255;
		std::map<i32, SBrokenMeshSize>::iterator iter, iterNext;
		...
		if ((iter = m_mapBrokenMeshes.find(id)) != m_mapBrokenMeshes.end())
		{
			if (iter->second.size == sizer.m_size)
				return;
			m_totBreakageSize -= iter->second.size;
			DrxLog("Updated breakable mesh %s: %dKb->%" PRISIZE_T "Kb, new total size %" PRISIZE_T "Kb", GetStatObjName(pStatObjNamer), iter->second.size, sizer.m_size, m_totBreakageSize + sizer.m_size);
			m_mapBrokenMeshes.erase(iter);
		}
		...
		m_mapBrokenMeshes.insert(std::pair<i32, SBrokenMeshSize>(id, SBrokenMeshSize(pPhysEnt, sizer.m_size, partid, timeout, fractureFX)));
		...
		i32 idRndFrame = gEnv->pRenderer->GetFrameID();
		while (m_totBreakageSize * g_breakage_mem_limit > sqr(g_breakage_mem_limit))
		{
			...
			for (iter = m_mapBrokenMeshes.begin(); iter != m_mapBrokenMeshes.end(); iter = iterNext)
				    ...
				    ((ifd = pent->GetiForeignData()) == PHYS_FOREIGN_ID_ENTITY || ifd == PHYS_FOREIGN_ID_STATIC) && (pfd = pent->GetForeignData(ifd)))
				{
					...
					if (ifd == PHYS_FOREIGN_ID_STATIC)
						...
						continue;
					i32 lastDrawFrame = pRndNode->GetDrawFrame();
					i32 bVisible = isneg(idRndFrame - lastDrawFrame - 10) || pent == pPhysEnt;
				...
				else
				{
					...
					DrxLog("Removing obsolete breakable mesh record of %dKb", iter->second.size);
					m_totBreakageSize -= iter->second.size;
					m_mapBrokenMeshes.erase(iter);
				}
			...
			m_totBreakageSize -= iter->second.size;
			m_mapBrokenMeshes.erase(iter);
			DrxLog("Breakable memory limit breached - removing a mesh; new total size %dKb", m_totBreakageSize);
		}
...
{
	std::map<i32, SBrokenMeshSize>::iterator iter;
	i32 sizes[16] = { 0 }, keys[16] = { 0 }, nTop = 0, i;
	pe_status_pos sp;
...
void CActionGame::AddBroken2DChunkId(i32 id)
{
...
void CActionGame::UpdateBrokenMeshes(float dt)
{
	...
	for (i32 i = m_pendingPlaneBreaks.size() - 1; i >= 0; i--)
	...
	if (dt)
		for (iter = m_mapBrokenMeshes.begin(); iter != m_mapBrokenMeshes.end(); iter = iterNext)
			if (++(iterNext = iter), iter->second.timeout > 0 && (iter->second.timeout -= dt) <= 0.0)
			{
				...
				m_totBreakageSize -= iter->second.size;
				DrxLog("Removing broken mesh on timeout; new total size %dKb", m_totBreakageSize);
				m_mapBrokenMeshes.erase(iter);
			}
}

void CActionGame::RemoveEntFromBreakageReuse(IPhysicalEntity* pEntity, i32 bRemoveOnlyIfSecondary)
{
...
i32 CActionGame::OnRemovePhysicalEntityPartsLogged(const EventPhys* pEvent)
{
...
i32 CActionGame::OnCollisionImmediate(const EventPhys* pEvent)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
	...
	ISurfaceTypeUpr* pSurfaceTypeUpr = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr();
	ISurfaceType* pMat = pSurfaceTypeUpr->GetSurfaceType(pCollision->idmat[1]);
	float energy, hitenergy;
...
i32 CActionGame::OnPostStepImmediate(const EventPhys* pEvent)
{
...
i32 CActionGame::OnStateChangeImmediate(const EventPhys* pEvent)
{
...
i32 CActionGame::OnCreatePhysicalEntityImmediate(const EventPhys* pEvent)
{
...
{
	std::map<EntityId, i32>::iterator iter;
	EntityId id = 0;
	if ((iter = m_entPieceIdx.find(idSrc)) == m_entPieceIdx.end())
		iter = m_entPieceIdx.insert(std::pair<EntityId, i32>(idSrc, -1)).first;
...
i32 CActionGame::OnUpdateMeshImmediate(const EventPhys* pEvent)
{
...
void CActionGame::HideBrokenObjectsByIndex(u16* pObjectIndicies, i32 iNumObjectIndices)
{
	for (i32 k = iNumObjectIndices - 1; k >= 0; k--)
	{
		i32 iEventIndex = pObjectIndicies[k];

		i32 i = m_breakEvents[iEventIndex].iBrokenObjectIndex;

		if (i >= 0)
		{
			if (m_brokenObjs[i].itype == PHYS_FOREIGN_ID_STATIC)
			{
				//DrxLogAlways("Hide & remove decals from original pBrush 0x%p at index %d", m_brokenObjs[i].pBrush, i);
			...
			{
				DrxLogAlways("Hide & remove decals from ENTITY 0x%08X at index %d", m_brokenObjs[i].idEnt, i);
...
void CActionGame::UnhideBrokenObjectsByIndex(u16* pObjectIndicies, i32 iNumObjectIndices)
{
	for (i32 k = iNumObjectIndices - 1; k >= 0; k--)
	{
		i32 iEventIndex = pObjectIndicies[k];
...
void CActionGame::ApplyBreakToClonedObjectFromEvent(const SRenderNodeCloneLookup& renderNodeLookup, i32 brokenObjIndex, i32 i)
{
	...
	i32k kNumNodes = renderNodeLookup.iNumPairs;

	for (i32 a = 0; a < kNumNodes; a++)
	{
...
void CActionGame::ApplySingleProceduralBreakFromEventIndex(u16 uBreakEventIndex, const SRenderNodeCloneLookup& renderNodeLookup)
{
	i32 index = m_breakEvents[uBreakEventIndex].iBrokenObjectIndex;
	if (index >= 0)
	{
...
void CActionGame::ApplyBreaksUntilTimeToObjectList(i32 iFirstBreakEventIndex, const SRenderNodeCloneLookup& renderNodeLookup)
{
	i32 i = 0;
	...
	for (i32 q = 0; q < renderNodeLookup.iNumPairs; q++)
	{
	...
	{
		i32 index = m_breakEvents[i].iBrokenObjectIndex;
		if (index >= 0)
		{
...
void CActionGame::CloneBrokenObjectsByIndex(u16* pBreakEventIndices, i32& iNumBreakEventIndices, IRenderNode** outClonedNodes, i32& iNumClonedNodes, SRenderNodeCloneLookup& nodeLookup)
{
	//Remove duplicates
	i32 iNumIndices = iNumBreakEventIndices;
	//DrxLogAlways("Searching for duplicate indices");
	for (i32 i = iNumIndices - 1; i >= 0; i--)
	{
		i32k iSearchIndex = m_breakEvents[pBreakEventIndices[i]].iBrokenObjectIndex;

		for (i32 j = iNumIndices - 1; j >= 0; j--)
		{
			if (m_breakEvents[pBreakEventIndices[j]].iBrokenObjectIndex == iSearchIndex && i != j)
			{
				//DrxLogAlways("  Duplicate index found: %d, removing one", iSearchIndex);
	...
	i32 iLocalNumClonedNodes = iNumClonedNodes;

	for (i32 k = iNumIndices - 1; k >= 0; k--)
	{
		i32 iEventIndex = pBreakEventIndices[k];

		i32 i = m_breakEvents[iEventIndex].iBrokenObjectIndex;

		if (i >= 0)
		{
			//DrxLogAlways("  Event %d has a valid broken object index of %d", iEventIndex, i);
			if (m_brokenObjs[i].itype == PHYS_FOREIGN_ID_ENTITY)
			{
				...
				//        if(pOriginalEntity)
				//        {
				//          //DrxLogAlways("PR:   Found entity with ID 0x%08X, cloning...", originalBrokenId);
				//
				...
				//
				//          IBreakableUpr * pBreakableMgr = gEnv->pEntitySystem->GetBreakableUpr();
				//
			...
			else
			{
				...
				if (iRenderNodeType == eERType_Brush)
				{
					...
					//DrxLogAlways("    iRenderNodeType eERType_Brush - adding");
		...
		{
			//DrxLogAlways("  Event %d has an INVALID BROKEN OBJECT INDEX", iEventIndex);
		}
	}
...
{
	//
	if (IBreakableUpr* pBreakUpr = gEnv->pEntitySystem->GetBreakableUpr())
	{
		pBreakUpr->ResetBrokenObjects();
	}

	// Even if cvar off, allow to reset in case it *was* enabled at some point
	if (IBreakableGlassSystem* pGlassSystem = CDrxAction::GetDrxAction()->GetIBreakableGlassSystem())
	{
		pGlassSystem->ResetAll();
	}

	//
	for (i32 i = m_brokenObjs.size() - 1; i >= 0; i--)
	{
		if (bRestoreBroken)
		{
			if (m_brokenObjs[i].itype == PHYS_FOREIGN_ID_ENTITY)
			{
				...
				if (pEnt)
				{
					...
					{
						for (i32 j = pEnt->GetSlotCount() - 1; j >= 0; j--)
							pEnt->FreeSlot(j);
...
void CActionGame::OnEditorSetGameMode(bool bGameMode)
{
	...
	CDrxAction* const pDrxAction = CDrxAction::GetDrxAction();
	pDrxAction->AllowSave(true);
	pDrxAction->AllowLoad(true);
	...
	CDialogSystem* pDS = pDrxAction->GetDialogSystem();
	if (pDS)
	{
		pDS->Reset(false);
	...
	pDrxAction->GetPersistantDebug()->Reset();
}
...
void CActionGame::Serialize(TSerialize ser)
{
	...
	if (ser.IsReading())
	{
		for (i32 i = 0; i < MAX_CACHED_EFFECTS; ++i)
...
void CDrxAction::Serialize(TSerialize ser)
{
	if (m_pGame)
		m_pGame->Serialize(ser);
}

void CDrxAction::FlushBreakableObjects()
{
	if (m_pGame)
		m_pGame->FlushBreakableObjects();
}

void CDrxAction::ClearBreakHistory()
{
	if (m_pGame)
...
void CActionGame::GetMemoryUsage(IDrxSizer* s) const
{
	{
...
{
	m_pGameStats = new CGameStats(CDrxAction::GetDrxAction());
}
