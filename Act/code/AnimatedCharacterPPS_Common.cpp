...
{
	DRX_ASSERT(origin.IsValid());
	DRX_ASSERT(offset.IsValid());
	QuatT destination(origin.q * offset.q, origin.t + offset.t);
	destination.q.Normalize();
	DRX_ASSERT(destination.IsValid());
	return destination;
}
...
QuatT CombineHVComponents2D(const QuatT& cmpH, const QuatT& cmpV)
{
	...
	DRX_ASSERT(cmpH.IsValid());
	DRX_ASSERT(cmpV.IsValid());
...
QuatT CAnimatedCharacter::MergeMCM(const QuatT& ent, const QuatT& anim, bool flat) const
{
	...
	DRX_ASSERT(ent.IsValid());
	DRX_ASSERT(anim.IsValid());

	EMovementControlMethod mcmh = GetMCMH();
	EMovementControlMethod mcmv = GetMCMV();

	DRX_ASSERT(mcmh >= 0 && mcmh < eMCM_COUNT);
	DRX_ASSERT(mcmv >= 0 && mcmv < eMCM_COUNT);
...
void CAnimatedCharacter::SetMovementControlMethods(EMCMSlot slot, EMovementControlMethod horizontal, EMovementControlMethod vertical, tukk tag)
{
	DRX_ASSERT((u32)eMCMSlotStack_Begin <= (u32)slot);
	DRX_ASSERT((u32)slot < (u32)eMCMSlotStack_End);
	DRX_ASSERT(vertical != eMCM_AnimationHCollision);
