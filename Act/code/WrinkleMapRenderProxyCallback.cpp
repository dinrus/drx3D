...
#include <drx3D/Act/WrinkleMapRenderProxyCallback.h>
...
bool HasWrinkleBlending(IMaterial& material)
{
	...
	for (i32 i = 0; i < pShaderGen->m_BitMask.size(); ++i)
	{
...
IMaterial* FindFirstMaterialWithWrinkleBlending(IMaterial& material)
{
	...
	for (i32 i = 0; i < material.GetSubMtlCount(); ++i)
	{
...
IMaterial* FindFirstMaterialWithWrinkleBlending(ICharacterInstance& instance)
{
	...
	IAttachmentUpr* pAttachmentUpr = instance.GetIAttachmentUpr();
	if (!pAttachmentUpr)
		return NULL;

	uint attachmentCount = pAttachmentUpr->GetAttachmentCount();
...
DRXREGISTER_CLASS(CWrinkleMapShaderParamCallback)
DRXREGISTER_CLASS(CWrinkleMapShaderParamCallbackUI)
...
void CWrinkleMapShaderParamCallback::SetupBoneWrinkleMapInfo()
{
	IDefaultSkeleton& rIDefaultSkeleton = m_pCharacterInstance->GetIDefaultSkeleton();
	{
		...
		{
			char acTmp[32];
			drx_sprintf(acTmp, "%s_%02d", WRINKLE_BONE_PREFIX, i + 1);
  ...
  IShaderPublicParams* pParams, IMaterial* pMaterial)
{
	...
	if (pParams && m_pCharacterInstance && !m_WrinkleBoneInfo.empty())
	{
		...
		if (m_bWrinklesEnabled && m_pCharacterInstance->GetISkeletonPose())
		{
			...
			static tukk wrinkleMask[3] = { WRINKLE_MASK_NAME0, WRINKLE_MASK_NAME1, WRINKLE_MASK_NAME2 };
			...
			for (uint i = 0; i < 3; ++i)
			{
				...
				if (pParams->GetParamBySemantic(m_eSemantic[i]) == 0)
				{
					...
					drx_strcpy(newParam.m_Name, wrinkleMask[i]);
