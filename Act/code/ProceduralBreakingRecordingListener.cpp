...
i32 CProceduralBreakingRecordingListener::GetEntRef(EntityId id)
{
	if (!id)
	{
		DRX_ASSERT(false);
...
void CProceduralBreakingRecordingListener::GotJointBreak(i32 idxRef, i32 id, i32 epicenter)
{
...
i32 CProceduralBreakingRecordingListener::GotBreakOp(ENetBreakOperation op, i32 idxRef, i32 partid, EntityId to)
{
	DRX_ASSERT(op >= eNBO_Create && op < eNBO_NUM_OPERATIONS);
...
void CProceduralBreakingRecordingListener::FinishBreakOp(ENetBreakOperation op, i32 idxRef, EntityId to, i32 virtId)
{
}
...
void CProceduralBreakingRecordingListener::Complete()
{
	...
	char message[256];
	drx_sprintf(message, "recording %p order=%d", this, m_orderId);
	...
	m_orderId = -1;
	//	CDrxAction::GetDrxAction()->GetGameContext()->GetNetContext()->LogBreak(desc);
}
