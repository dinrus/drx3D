...
#include <drx3D/Act/FlashUIBaseNode.h>
#include <drx3D/Act/FlashUI.h>

i32 CUIFGStackMan::m_Size = 0;
bool CUIFGStackMan::m_bEnabled = true;
#ifdef ENABLE_UISTACK_DEBUGGING
std::map<i32, tukk > CUIFGStackMan::m_debugInfo;
#endif
...
{
	return InputPortConfig<i32>("instanceID", -1, "Instance ID for the element, -1 = all instances (lazy init), -2 = all initialized instances");
}
...
// ---------------------------------------------------------------
IUIElement* CFlashUIBaseNode::GetInstance(i32 instanceID)
{
	if (instanceID < 0)
		instanceID = 0;
...
void CFlashUIBaseNodeDynPorts::AddParamInputPorts(const SUIEventDesc::SEvtParams& eventDesc, std::vector<SInputPortConfig>& ports)
{
	for (TUIParams::const_iterator iter = eventDesc.Params.begin(); iter != eventDesc.Params.end(); ++iter)
	{
		...
		case SUIParameterDesc::eUIPT_Int:
			ports.push_back(InputPortConfig<i32>(iter->sDisplayName, iter->sDesc));
			break;
...
void CFlashUIBaseNodeDynPorts::AddParamOutputPorts(const SUIEventDesc::SEvtParams& eventDesc, std::vector<SOutputPortConfig>& ports)
{
	for (TUIParams::const_iterator iter = eventDesc.Params.begin(); iter != eventDesc.Params.end(); ++iter)
	{
		...
		case SUIParameterDesc::eUIPT_Bool:
			...
			break;
		case SUIParameterDesc::eUIPT_Int:
			ports.push_back(OutputPortConfig<i32>(iter->sDisplayName, iter->sDesc));
			break;
		case SUIParameterDesc::eUIPT_Float:
...
void CFlashUIBaseNodeDynPorts::AddCheckPorts(const SUIEventDesc::SEvtParams& eventDesc, std::vector<SInputPortConfig>& ports)
{
	...
	ports.push_back(InputPortConfig<i32>("Port", -1, _HELP("If a port is selected, the node will only trigger if the selected \"Port\" has \"Idx\" value"), 0, _UICONFIG(m_enumStr.c_str())));
	ports.push_back(InputPortConfig<string>("Idx", "Value that is compated with current value of output on selected \"Port\""));
}

// ---------------------------------------------------------------
void CFlashUIBaseNodeDynPorts::GetDynInput(SUIArguments& args, const SUIParameterDesc& desc, SActivationInfo* pActInfo, i32 port)
{
	switch (desc.eType)
	{
	case SUIParameterDesc::eUIPT_Bool:
...
// ---------------------------------------------------------------
void CFlashUIBaseNodeDynPorts::ActivateDynOutput(const TUIData& arg, const SUIParameterDesc& desc, SActivationInfo* pActInfo, i32 port)
{
	switch (desc.eType)
	{
	case SUIParameterDesc::eUIPT_Bool:
