...
#include <drx3D/Act/FlowVehicleSeat.h>
...
void CFlowVehicleSeat::GetConfiguration(SFlowNodeConfig& nodeConfig)
{
	...
	static const SInputPortConfig pInConfig[] =
	{
		...
		InputPortConfig<i32>("LockType", 0,                                                           _HELP("Who to lock the vehicle for"), NULL,_UICONFIG("enum_int:All=0,Players=1,AI=2")),
		{ 0 }
	};
			...
			if (pVehicle)
			{
				...
				if (pSeat)
				{
					...
					if (bLock)
					{
						...
						i32 intLockType = GetPortInt(pActInfo, eIn_LockType);
						switch (intLockType)
						{
						case 0:
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eINP_Trigger))
				{
					...
					if (pActInfo->pEntity)
					{
						...
						CVehicle* pVehicle = static_cast<CVehicle*>(CDrxAction::GetDrxAction()->GetIVehicleSystem()->GetVehicle(vehicleId));
...
public:
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			{
				IVehicle* pVehicle = pActInfo->pEntity ? CDrxAction::GetDrxAction()->GetIVehicleSystem()->GetVehicle(pActInfo->pEntity->GetId()) : NULL;

				if (pVehicle)
				{
					i32 lockFor = GetPortInt(pActInfo, eINP_LockFor);
	...
	void Lock(IVehicle* pVehicle, bool lock, i32 lockFor)
	{
		...
		for (u32 i = 0; i < pVehicle->GetSeatCount(); ++i)
		{
