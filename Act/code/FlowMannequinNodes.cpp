...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			...
			InputPortConfig<i32>("Priority",              _HELP("Priority, higher number = higher priority")),
		...
		case eFE_Activate:
			{
				...
				if (IsPortActive(pActInfo, EIP_Play))
				{
					if (pAnimChar && pAnimChar->GetActionController())
					{
						...
						i32k fragCRC = CCrc32::ComputeLowercase(fragName);
						...
						i32k priority = GetPortInt(pActInfo, EIP_Prio);
		...
		case eFE_Activate:
			...
			if (IsPortActive(pActInfo, EIP_Enslave) || IsPortActive(pActInfo, EIP_UnEnslave))
			{
				...
				if (pAnimChar && pAnimChar->GetActionController())
				{
					...
					{
						IAnimationDatabaseUpr& dbUpr = gEnv->pGameFramework->GetMannequinInterface().GetAnimationDatabaseUpr();
						u32 db_crc32 = CCrc32::ComputeLowercase(GetPortString(pActInfo, EIP_DB));
						const IAnimationDatabase* db = dbUpr.FindDatabase(db_crc32);
					...
					{
						DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "No GameObject or Animated character found for the slave");
						ActivateOutput(pActInfo, EOP_Fail, 1);
					}
				}
				else
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "No GameObject or AnimatedCharacter found");
					ActivateOutput(pActInfo, EOP_Fail, 1);
				}
			}
...
{
public:
	PROCEDURAL_CONTEXT(CFlowGraphEventsProceduralContext, "FlowGraphEvents", "33445592-5e89-47fb-8008-b3f89cac0c37"_drx_guid);
...
DRXREGISTER_CLASS(CFlowGraphEventsProceduralContext);
