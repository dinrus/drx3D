...
#include <drx3D/Act/VehicleViewBase.h>
...
CVehicleViewBase::CVehicleViewBase()
	...
	, m_pitchDownActionOnBorderAAM(0)
	, m_pitchVal(0.0f)
	, m_yawVal(0.0f)
	, m_hidePlayer(false)
	, m_isEnabled(true)
...
void CVehicleViewBase::OnStartUsing(EntityId playerId)
{
	...
	//DrxLog("StartUsing: %s %s", m_pSeat->GetName().c_str(), IsThirdPerson()?"(tp)":"");
	...
	for (i32 i = 0; i < m_hideParts.size(); ++i)
	{
...
{
	//if (VehicleCVars().v_debugdraw == eVDB_View)
	//DrxLog("StopUsing: %s %s", m_pSeat->GetName().c_str(), IsThirdPerson()?"(tp)":"");
...
//------------------------------------------------------------------------
void CVehicleViewBase::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
...
void CVehicleViewBase::Update(const float frameTime)
{
	...
	IActor* pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(m_pSeat->GetPassenger());
	/*
	   if (false && pActor)
