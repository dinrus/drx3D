...
#define LAYERSWITCHNODE_SEP           DRX_NATIVE_PATH_SEPSTR
	...
	, IEntityLayerSetUpdateListener
{
	...
	typedef std::vector<tukk >   TSzNames;
	typedef DrxStackStringT<char, 256> TPathString;
...
public:
	...
	virtual ~CLayerSetSwitchNode()
	{
		if (gEnv->IsEditor() && m_bRegistered)
		{
			...
			{
				ILevelSystem* pLevelSystem = pDrxAction->GetILevelSystem();
...
private:
		...
		case eFE_Activate:
			{
				...
				{
					DRX_PROFILE_REGION(PROFILE_GAME, __FUNCTION__);
	...
	// IEntityLayerSetUpdateListener
	virtual void LayerEnablingEvent(tukk szLayerName, bool bEnabled, bool bSerialized) override
	{
	...
	bool ParseXml(TStringNames& outStringNames, const string& layerSetName) const
	{
		...
		if (xmlRoot)
		{
			...
			for (i32 i = 0; i < childCount; ++i)
			{
				...
				{
					tukk const szName = childNode->getAttr("Name");
					if (szName && *szName)
					{
	...
	static void FindAllFilesInFolder(TStringNames& outNames, tukk szFolderName)
	{
	...
	void RefreshLayerSetsUIEnum() const
	{
		if (CDrxAction* pDrxAction = CDrxAction::GetDrxAction())
		{
			...
			{
				if (IGameToEditorInterface* pGameToEditor = pDrxAction->GetIGameToEditor())
				{
					...
					pGameToEditor->SetUIEnums(LAYERSWITCHNODE_UICONFIG_NAME, &fileSzNames[0], static_cast<i32>(fileSzNames.size()));
				}
			}
		}
	}
