...
// -------------------------------------------------------------------------
//  Имя файла:   NetMsgDispatcher.h
//  Описание: Central Hub for bidirectional communication of simple messages
...
// -------------------------------------------------------------------------
//  История: Dario Sancho. 15.10.2014. Created
//
////////////////////////////////////////////////////////////////////////////

#include <drx3D/Act/StdAfx.h>
#include <drx3D/Act/NetMsgDispatcher.h>
#include <drx3D/CoreX/Renderer/IRenderer.h> // for debug render
...
// Local CVARS
// ------------------------------------------------------------------------
i32 CNetMessageDistpatcher::CVars::net_netMsgDispatcherDebug = 0;
i32 CNetMessageDistpatcher::CVars::net_netMsgDispatcherLogging = 0;
i32 CNetMessageDistpatcher::CVars::net_netMsgDispatcherWarnThreshold = 10;
...
{
	if (CNetMessageDistpatcher* pNetMsgDispatcher = CDrxAction::GetDrxAction()->GetNetMessageDispatcher())
	{
...
{
	{
		DrxAutoLock<DrxCriticalSectionNonRecursive> lock(m_criticalSectionReceivedQueue);
...
{
	u32 cntDispatchedMsgs = 0;

	DRX_ASSERT_MESSAGE(msg.tgt != SNetMsgData::eT_Invalid, "Invalid message target");
	...
	case SNetMsgData::eL_Client:
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "[CNetMessageDistpatcher::DispatchNetMessages] ClientNub == NULL");
			}
		}
		break;
	...
	case SNetMsgData::eL_DedicatedServer:
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "[CNetMessageDistpatcher::DispatchNetMessages] pServerNub == NULL");
			}
		}
		break;
	default:
...
// ------------------------------------------------------------------------
void CNetMessageDistpatcher::RegisterListener(INetMsgListener* pListener, tukk name)
{
...
// ------------------------------------------------------------------------
/* static */ tukk CNetMessageDistpatcher::DbgTranslateLocationToString()
{
...
// ------------------------------------------------------------------------
/* static */ void CNetMessageDistpatcher::Log(tukk szMessage)
{
...
// ------------------------------------------------------------------------
void CNetMessageDistpatcher::CNetStats::Add(ETypes type, u32 value)
{
	if (CVars::net_netMsgDispatcherDebug)
	{
		switch (type)
