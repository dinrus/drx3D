...
bool CClientClock::OnSyncTimeMessage(const SSyncTimeClient& messageParams)
{
	...
	//DrxLogAlways("Ping: %f", halfRoundTripTime.GetMilliSeconds());
	m_pings += 1;
	...
	else if (m_syncState == eSS_Refining)
	{
		...
		//DrxLogAlways("Diff avg: %f", avgDiff);

		if (m_pings > 32)
		{
			//DrxLogAlways("Offset time: %f", m_OffsetTime.GetMilliSeconds());
...
void CClientClock::SendSyncPing(i32k id, const CTimeValue currentTime, const CTimeValue serverTime /*= CTimeValue()*/)
{
...
#endif
{
	...
	CDrxAction::GetDrxAction()->OnActionEvent(SActionEvent(eAE_channelCreated, 0));
}

CGameClientChannel::~CGameClientChannel()
{
	CDrxAction::GetDrxAction()->OnActionEvent(SActionEvent(eAE_channelDestroyed, 0));
	...
	if (!CDrxAction::GetDrxAction()->IsGameSessionMigrating())
	{
		for (std::map<string, string>::iterator it = m_originalCVarValues.begin(); it != m_originalCVarValues.end(); ++it)
		{
			if (ICVar* pVar = gEnv->pConsole->GetCVar(it->first.c_str()))
			{
				...
				{
					DrxLog("Restore cvar '%s' to '%s'", it->first.c_str(), it->second.c_str());
				}
...
{
	delete this;
	DrxLogAlways("CGameClientChannel::Release");
}
...
void CGameClientChannel::OnDisconnect(EDisconnectionCause cause, tukk description)
{
	//DrxLogAlways("CGameClientChannel::OnDisconnect(%d, %s)", cause, description?описание:"");
	CDrxAction::GetDrxAction()->OnActionEvent(SActionEvent(eAE_disconnected, i32(cause), description));
...
NET_IMPLEMENT_SIMPLE_ATSYNC_MESSAGE(CGameClientChannel, RegisterEntityClassHash, eNRT_ReliableUnordered, eMPF_BlocksStateChange)
{
	...
	u32 localCrc = pGameContext->GetClassesHash();
...
NET_IMPLEMENT_SIMPLE_ATSYNC_MESSAGE(CGameClientChannel, SetGameType, eNRT_ReliableOrdered, eMPF_BlocksStateChange)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "No GameRules");
	}
	...
	if (!bFromDemoSystem)
	{
		DrxLogAlways("Game rules class: %s", rulesClass.c_str());
	...
	else
	{
		...
		//
		CDrxAction::GetDrxAction()->GetIGameRulesSystem()->CreateGameRules(rulesClass.c_str()); // we don't do context establishment tasks when playing back demo
		ok = CDrxAction::GetDrxAction()->GetILevelSystem()->LoadLevel(levelName.c_str()) != 0;
		//
...
NET_IMPLEMENT_IMMEDIATE_MESSAGE(CGameClientChannel, DefaultSpawn, eNRT_UnreliableOrdered, eMPF_BlocksStateChange | eMPF_DecodeInSync)
{
	...
	u16 channelId = bFromDemoSystem ? (param.nChannelId != 0 ? -1 : 0) : param.nChannelId;
...
NET_IMPLEMENT_SIMPLE_ATSYNC_MESSAGE(CGameClientChannel, SetBatchConsoleVariables, eNRT_ReliableUnordered, eMPF_BlocksStateChange)
{
	...
	for (i32 i = 0; i < param.actual && res; ++i)
...
void CGameClientChannel::SetPlayerId(EntityId id)
{
	...
	if (id)
	{
		...
		if (pEntity)
		{
			IGameObject* pGameObject = CDrxAction::GetDrxAction()->GetGameObject(id);
...
void CGameClientChannel::CallOnSetPlayerId()
{
	...
	if (IActor* pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(GetPlayerId()))
...
bool CGameClientChannel::HookCreateActor(IEntity* pEntity, IGameObject* pGameObject, uk pUserData)
{
	...
	if (pGameObject)
	{
		pGameObject->SetChannelId(*static_cast<u16*>(pUserData));
	}
	return true;
}
bool CGameClientChannel::SetConsoleVar(const string& key, const string& val)
{
	...
	if (!pVar)
	{
		DrxLog("Server sets console variable '%s' to '%s'", key.c_str(), val.c_str());
		DrxLog("   cvar not found, ignoring");
		return true;
	}
	...
	{
		DrxLog("Server sets console variable '%s' to '%s'", key.c_str(), val.c_str());
		DrxLog("   cvar not synchronized, disconnecting");
		return false;
	}
