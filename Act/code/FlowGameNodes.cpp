...
inline IActor* GetAIActor(IFlowNode::SActivationInfo* pActInfo)
{
	if (!pActInfo->pEntity)
		return 0;
	return CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(pActInfo->pEntity->GetId());
}
...
public:
	...
	{
		EntityId playerId = CDrxAction::GetDrxAction()->GetClientEntityId();
...
void SendFlowHitToEntity(EntityId targetId, EntityId shooterId, i32 damage, const Vec3& pos)
{
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			...
			InputPortConfig<i32>("damage",         0,                                                       _HELP("Amount of damage to exert when [Trigger] is activated"),                                                                           _HELP("Damage")),
			InputPortConfig<i32>("damageRelative", 0,                                                       _HELP("Amount of damage to exert when [Trigger] is activated. Is relative to the maximun health of the actor. (100 = full max damage)"),  _HELP("DamageRelative")),
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			if (IsPortActive(pActInfo, INP_TRIGGER))
			{
				...
				if (pActor)
				{
					i32 damage = GetPortInt(pActInfo, INP_DAMAGE);
					if (damage == 0)
						damage = (GetPortInt(pActInfo, INP_RELATIVEDAMAGE) * i32(pActor->GetMaxHealth())) / 100;
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] = {
			InputPortConfig_Void("Trigger",        _HELP("Trigger this port to actually damage the actor")),
			InputPortConfig<i32>("damage",         0,                                                       _HELP("Amount of damage to exert when [Trigger] is activated"),                                                                                                                                                             _HELP("Damage")),
			InputPortConfig<i32>("damageRelative", 0,                                                       _HELP("Amount of damage to exert when [Trigger] is activated. Is relative to the maximun health of the entity. (100 = full max damage). /nwarning: This will only works for entities with a GetMaxHealth() lua function"),  _HELP("DamageRelative")),
			InputPortConfig<Vec3>("Position",      Vec3(ZERO),                                              _HELP("Position of damage")),
			{ 0 }
		};
		config.pInputPorts = inputs;
		...
		case eFE_Activate:
			if (IsPortActive(pActInfo, INP_TRIGGER))
			{
				...
				if (pEntity)
				{
					...
					if (damage == 0)
					{
						i32 damageRelative = GetPortInt(pActInfo, INP_RELATIVEDAMAGE);
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig outputs[] = {
			OutputPortConfig<i32>("Health", _HELP("Current health of entity")),
			{ 0 }
		};
		config.pInputPorts = inputs;
		config.pOutputPorts = outputs;
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			if (IsPortActive(pActInfo, 0))
			{
				...
				{
					DrxLogAlways("CFlowActorCheckHealth - No Entity or Entity not an Actor!");
				}
			}
		}
	}
...
public:
	...
	void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (eFE_Activate == event && IsPortActive(pActInfo, 0))
		{
			...
			u32 eventId = CDrxAction::GetDrxAction()->GetIGameObjectSystem()->GetEventID(eventName.c_str());
			SGameObjectEvent evt(eventId, eGOEF_ToAll, IGameObjectSystem::InvalidExtensionID, (uk ) (GetPortString(pActInfo, 2).c_str()));
			if (pActInfo->pEntity == 0)
			{
				// broadcast to all gameobject events
				CDrxAction::GetDrxAction()->GetIGameObjectSystem()->BroadcastEvent(evt);
			}
			else
			{
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			if (IsPortActive(pActInfo, 0))
			{
				...
				if (strcmp(mapname, "") != 0)
					pLevelInfo = CDrxAction::GetDrxAction()->GetILevelSystem()->GetLevelInfo(mapname);

				if (pLevelInfo)
				{
					size_t numGameRules = pLevelInfo->GetGameRulesCount();
					std::vector<tukk > gameRules(numGameRules);
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			if (IsPortActive(pActInfo, 0))
			{
				...
				bool bResult = CDrxAction::GetDrxAction()->GetILevelSystem()->GetCurrentLevel()->IsOfType(levelType);
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo *pActInfo)
	{
		...
		case eFE_Activate:
		{
			...
			if (IsPortActive(pActInfo, IN_PAUSE))
			{
				...
				if (bAffectActionmap) { gEnv->pGameFramework->GetIActionMapUpr()->Enable(false, true); }
			}
			else if (IsPortActive(pActInfo, IN_UNPAUSE))
			{
				...
				if (bAffectActionmap) { gEnv->pGameFramework->GetIActionMapUpr()->Enable(true); }
			}
