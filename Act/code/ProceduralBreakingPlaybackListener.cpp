...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  network breakability: plazback of procedural breaks
   -------------------------------------------------------------------------
   История:
...
void CProceduralBreakingPlaybackListener::UpdateJointBreaks()
{
	...
	if (CNetworkCVars::Get().BreakageLog)
		DrxLog("[brk] playback frame %d", m_playbackFrame);
	#endif
	...
	for (size_t i = 0; i < jbs.size(); ++i)
	{
		...
		tukk desc = CObjectSelector::GetDescription(jbs[i].idxRef).c_str();

	#if BREAK_HIERARCHICAL_TRACKING
		if (CNetworkCVars::Get().BreakageLog)
			DrxLog("[brk] break joint %d on ref %d (%s) [reached joint frame %d]", jbs[i].id, jbs[i].idxRef, desc, i32(jbs[i].frame));
	#else
		if (CNetworkCVars::Get().BreakageLog)
			DrxLog("[brk] break joint %d on ref %d (%s)", jbs[i].id, jbs[i].idxRef, desc);
	#endif
...
{
	char message[256];
	drx_sprintf(message, "begin playback %d", m_pStream->m_magicId);
...
i32 CProceduralBreakingPlaybackListener::GotBreakOp(ENetBreakOperation op, i32 idxRef, i32 partid, EntityId to)
{
...
void CProceduralBreakingPlaybackListener::FinishBreakOp(ENetBreakOperation op, i32 idxRef, EntityId to, i32 virtId)
{
	#if BREAK_HIERARCHICAL_TRACKING
...
{
	char message[256];
	drx_sprintf(message, "end playback %d", m_pStream->m_magicId);
	DumpSpawnRecs(m_pStream->m_spawnRecs, m_pStream->m_jointBreaks, message);
}
...
bool CProceduralBreakingPlaybackStream::SetMagicId(i32 id)
{
	if (m_magicId >= 0 || id < 0)
		return false;
	m_magicId = id;
...
{
	DRX_ASSERT(rec->op >= eNBO_Create && rec->op < eNBO_NUM_OPERATIONS);
