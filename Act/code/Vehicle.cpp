...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Implements vehicle functionality
...
i32 CVehicle::s_repairHitTypeId = 0;
i32 CVehicle::s_disableCollisionsHitTypeId = 0;
i32 CVehicle::s_collisionHitTypeId = 0;
i32 CVehicle::s_normalHitTypeId = 0;
i32 CVehicle::s_fireHitTypeId = 0;
i32 CVehicle::s_punishHitTypeId = 0;
i32 CVehicle::s_vehicleDestructionTypeId = 0;
...
//------------------------------------------------------------------------
u32 CVehicle::GetPartChildrenCount(IVehiclePart* pParentPart)
{
...
CVehicle::~CVehicle()
{
	...
	if (pVehicleSystem->GetCurrentClientVehicle() == this)
	{
		...
		DrxAutoCriticalSection lock(pVehicleSystem->GetCurrentVehicleLock());
	...
	if (SpawnAndDeleteEntities(true))
	{
		...
		i32 nChildren = GetEntity()->GetChildCount();
	...
	CDrxAction::GetDrxAction()->GetIVehicleSystem()->RemoveVehicle(GetEntityId());
...
{
//------------------------------------------------------------------------
XmlNodeRef LoadXmlForClass(tukk className, const string& xmlDir)
{
	...
	if (0 == xmlData)
	{
		...
		if ((handle = gEnv->pDrxPak->FindFirst(xmlDir + "*.xml", &fd)) != -1)
		{
			do
			{
				...
				res = gEnv->pDrxPak->FindNext(handle, &fd);
			}
			while (res >= 0);
...
{
	DrxLog("Init vehicle: %s", pGameObject->GetEntity()->GetEntityTextDescription().c_str());
	INDENT_LOG_DURING_SCOPE();

	SetGameObject(pGameObject);

	IEntity* pEntity = GetEntity();
	DRX_ASSERT(pEntity == pGameObject->GetEntity());
	    ...
	    || s_normalHitTypeId == 0 || s_fireHitTypeId == 0 || s_punishHitTypeId == 0)
	{
		...
		DRX_ASSERT_MESSAGE(pGR, "No valid game rules set!");
	}

	if (gEnv->bMultiplayer && (CDrxActionCVars::Get().g_multiplayerEnableVehicles == 0))
	{
	...
	DRX_ASSERT(m_pIEntityAudioComponent && "no sound proxy on entity");

	m_pVehicleSystem = CDrxAction::GetDrxAction()->GetIVehicleSystem();
	m_engineSlotBySpeed = true;

	m_parts.clear();
	m_seats.clear();

	CDrxAction::GetDrxAction()->GetIVehicleSystem()->AddVehicle(GetEntityId(), this);
	CDrxAction::GetDrxAction()->GetVehicleScriptBind()->AttachTo(this);
	...
	if (CVehicleParams partsTable = vehicleParams.findChild("Parts"))
		...
		DrxLog("%s: No Parts table found!", pEntity->GetName());
	...
	{
		i32 componentCount = componentsTable.getChildCount();
	...
	{
		i32 i = 0;
		i32 c = animationsTable.getChildCount();
	...
	{
		i32 i = 0;
		i32 c = movementsTable.getChildCount();
	...
	{
		i32 c = seatGroupsTable.getChildCount();
		i32 i = 0;
	...
	if (CVehicleParams inventoryTable = vehicleParams.findChild("Inventory"))
	{
		...
		{
			i32 i = 0;
			i32 c = ammosTable.getChildCount();
	...
	//////////////////////////////////////////////////////////////////////////
	{
		i32 viewDistRatio = DEFAULT_VEHICLE_VIEW_DIST_RATIO;
	...
	for (TVehicleSeatVector::iterator it = m_seats.begin(); it != m_seats.end(); ++it)
		CDrxAction::GetDrxAction()->GetVehicleSeatScriptBind()->AttachTo(this, GetSeatId(it->second));
...
void CVehicle::InitPaint(const CVehicleParams& xmlContent)
{
	...
	if (IScriptTable* pScriptTable = GetEntity()->GetScriptTable())
	{
		...
		{
			i32 i = 0;
			i32 c = paintsTable.getChildCount();

			for (; i < c; i++)
			{
				if (CVehicleParams paintRef = paintsTable.getChild(i))
				{
					tukk paintName = paintRef.getAttr("name");
					if (!stricmp(paintName, m_paintName.c_str()))
					{
						tukk paintMaterial = paintRef.getAttr("material");
						if (paintMaterial && paintMaterial[0])
						{
							m_pPaintMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(paintMaterial, false);
							if (m_pPaintMaterial)
								GetEntity()->SetMaterial(m_pPaintMaterial);
							else
								DrxLog("[CVehicle::Paint] <%s> Material %s for Paint %s not found", GetEntity()->GetName(), paintMaterial, m_paintName.c_str());
						}

						tukk destroyedMaterialName = paintRef.getAttr("materialDestroyed");
						if (destroyedMaterialName && destroyedMaterialName[0])
						{
							m_pDestroyedMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(destroyedMaterialName, false);
						}
...
bool CVehicle::InitActions(const CVehicleParams& vehicleTable)
{
	...
	i32 c = actionsTable.getChildCount();
	m_actions.reserve(c);
			...
			if (actionRef.haveAttr("class"))
			{
				...
				if (IVehicleAction* pAction = m_pVehicleSystem->CreateVehicleAction(className))
				{
					if (pAction->Init(this, actionRef))
					{
						...
						{
							i32 numActivations = activationsTable.getChildCount();
							actionInfo.activations.resize(numActivations);

							for (i32 n = 0; n < numActivations; n++)
							{
					...
					else
					{
						...
						pAction->Release();
						DrxLog("Vehicle %s has an action defined (probably an 'Enter' action) that refers to a seat that doesn't exist.", vehicleTable.getAttr("name"));
					}
				}
			}
		}
...
void CVehicle::PostInit(IGameObject* pGameObject)
{
	...
	//  for (i32 i=0; i<MAX_UPDATE_SLOTS_PER_EXTENSION; ++i)
	...
	if (!CDrxAction::GetDrxAction()->IsEditing())
		NeedsUpdate();
#if ENABLE_VEHICLE_DEBUG
	...
	if (VehicleCVars().v_debug_mem > 0)
	{
		...
		i32 vehicleSize = pSizer->GetTotalSize();
		DrxLog("Vehicle initialized: <%s> takes %" PRISIZE_T " bytes.", GetEntity()->GetName(), pSizer->GetTotalSize());
...
//------------------------------------------------------------------------
void CVehicle::PostInitClient(i32 channelId)
{
...
bool CVehicle::ReloadExtension(IGameObject* pGameObject, const SEntitySpawnParams& params)
{
	...
	DRX_ASSERT_MESSAGE(false, "CVehicle::ReloadExtension not implemented");
	...
	if (VehicleCVars().v_debugdraw == eVDB_PhysParts || VehicleCVars().v_debugdraw == eVDB_PhysPartsExt)
	{
		...
		if (VehicleCVars().v_debugdraw == eVDB_PhysParts)
		{
			for (TVehiclePartVector::const_iterator ite = m_parts.begin(); ite != m_parts.end(); ++ite)
			{
				...
				DrxLog("Part <%-25s>: slot %2i, physId %8i, status %i", part->GetName(), part->GetSlot(), part->GetPhysId(), status);
			}
		}
		else
		{
			...
			for (i32 part = 0; part < numParts; ++part)
			{
				...
				if (pPhysics->GetStatus(&status))
				{
					...
					DrxLog("Part [%i]: partId %i, min: (%.2f, %.2f, %.2f), max: (%.2f, %.2f, %.2f)", part, status.partid, min.x, min.y, min.z, max.x, max.y, max.z);

					{
						// quadratic, but well, we're debugging
						u32 numJoints = rIDefaultSkeleton.GetJointCount();
						for (u32 i = 0; i < numJoints; ++i)
						{
							if (pSkeletonPose->GetPhysIdOnJoint(i) == status.partid)
								DrxLog("partId %i matches Joint <%s>", status.partid, rIDefaultSkeleton.GetJointNameByID(i));
						}
					}
...
//------------------------------------------------------------------------
void CVehicle::SetAmmoCapacity(IEntityClass* pAmmoType, i32 capacity)
{
	if (m_pInventory)
...
//------------------------------------------------------------------------
void CVehicle::SetAmmoCount(IEntityClass* pAmmoType, i32 amount)
{
	if (m_pInventory)
	{
		i32 oldAmount = m_pInventory->GetAmmoCount(pAmmoType);
		...
		if (oldAmount == 0 && amount != 0 && gEnv->IsClient())
		{
			...
			{
				i32 weaponCount = GetWeaponCount();
				for (i32 i = 0; i < weaponCount; ++i)
				{
...
//------------------------------------------------------------------------
i32 CVehicle::GetAmmoCount(IEntityClass* pAmmoType) const
{
	if (m_pInventory)
...
//------------------------------------------------------------------------
void CVehicle::SendWeaponSetup(i32 where, i32 channelId)
{
	...
	for (TVehicleSeatVector::iterator it = m_seats.begin(); it != m_seats.end(); ++it)
	{
		...
		for (TVehicleSeatActionVector::const_iterator ait = seatActions.begin(), aitEnd = seatActions.end(); ait != aitEnd; ++ait)
		{
			...
			if (CVehicleSeatActionWeapons* weapAction = CAST_VEHICLEOBJECT(CVehicleSeatActionWeapons, pSeatAction))
			{
				...
				i32 n = weapAction->GetWeaponCount();
				for (i32 w = 0; w < n; w++)
...
void CVehicle::ProcessEvent(const SEntityEvent& entityEvent)
{
	...
	case ENTITY_EVENT_TIMER:
		OnTimer((i32)entityEvent.nParam[0]);
		break;
...
void CVehicle::OnMaterialLayerChanged(const SEntityEvent& event)
{
	...
	if (frozen != prev)
	{
		...
		for (i32 i = 0; i < n; ++i)
		{
			...
			{
				if (IActor* pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(pChild->GetId()))
					if (pActor->IsPlayer()) // don't freeze players inside vehicles
						continue;

				//u8 mask = pIEntityRender->GetMaterialLayersMask();
...
//------------------------------------------------------------------------
i32 CVehicle::SetTimer(i32 timerId, i32 ms, IVehicleObject* pObject)
{
	if (timerId == -1)
	{
		if (!m_timers.empty())
	...
	if (m_timers.insert(std::make_pair(timerId, pObject)).second)
	{
		...
		//DrxLog("vehicle <%s> setting timer %i, %i ms", GetEntity()->GetName(), timerId, ms);
...
//------------------------------------------------------------------------
i32 CVehicle::KillTimer(i32 timerId)
{
	if (timerId != -1)
	{
...
//------------------------------------------------------------------------
void CVehicle::OnTimer(i32 timerId)
{
...
void CVehicle::Reset(bool enterGame)
{
	...
	if (enterGame)
	{
		...
		DrxAudio::ControlId const triggerId = DrxAudio::StringToId("ENGINE_ON");
	...
	else
	{
		...
		// Temp Code, testing only
		DrxAudio::ControlId const triggerId = DrxAudio::StringToId("ENGINE_OFF");
		m_pIEntityAudioComponent->ExecuteTrigger(triggerId);
	}
...
//------------------------------------------------------------------------
void CVehicle::Update(SEntityUpdateContext& ctx, i32 slot)
{
...
{
	for (u32 updatePolicy = 0; updatePolicy <= eVUS_Last; ++updatePolicy)
	{
		for (TVehicleObjectUpdateInfoList::iterator ite = m_objectsToUpdate.begin(), end = m_objectsToUpdate.end(); ite != end; ++ite)
		{
			SObjectUpdateInfo& updateInfo = *ite;
...
//------------------------------------------------------------------------
void CVehicle::NeedsUpdate(i32 flags, bool bThreadSafe /*=false*/)
{
	m_bNeedsUpdate = true;
	...
	if (flags & eVUF_AwakePhysics)
	{
		if (IPhysicalEntity* pPE = GetEntity()->GetPhysics())
		{
			...
			pPE->Action(&awakeParams, (i32)bThreadSafe);
		}
	}
}

//------------------------------------------------------------------------
void CVehicle::CheckDisableUpdate(i32 slot)
{
	if (m_bNeedsUpdate)
		return;
	...
	for (i32 i = eVUS_Visible - 1; i >= 0; --i)
	{
...
void CVehicle::DebugDraw(const float frameTime)
{
	...
	if (cvars.v_draw_helpers)
	{
		...
		CPersistantDebug* pDB = CDrxAction::GetDrxAction()->GetPersistantDebug();
		DrxFixedStringT<32> dbgName;
...
void CVehicle::DebugDrawClientPredict()
{
	if (IRenderAuxGeom* pRenderAuxGeom = gEnv->pAuxGeomRenderer)
	{
		DrxAutoCriticalSection lk(m_debugDrawLock);
...
void CVehicle::TestClientPrediction(const float deltaTime)
{
	...
	i32 predictSamples = VehicleCVars().v_testClientPredict;
		...
		if (pPhysics)
		{
			if (m_pMovement)
			{
				...
				if (m_predictionHistory.size() >= predictSamples)
				{
					...
					DrxAutoCriticalSection debuglk(m_debugDrawLock);
...
void CVehicle::UpdateNetwork(const float deltaTime)
{
	...
	if (m_hasAuthority && !gEnv->bServer && VehicleCVars().v_serverControlled)
	{
		...
		{
			DrxAutoCriticalSection netlk(*m_pMovement->GetNetworkLock());
#if ENABLE_VEHICLE_DEBUG
			DrxAutoCriticalSection debuglk(m_debugDrawLock);
#endif
			...
			if (pPhysics->GetStatus(&netPos))
			{
				...
				if (VehicleCVars().v_clientPredict)
				{
					...
					for (i32 i = m_predictionHistory.size() - 1; i >= 0; --i)
					{
...
void CVehicle::ReplayPredictionHistory(const float remainderTime)
{
	...
	i32 physicsTime = gEnv->pPhysicalWorld->GetiPhysicsTime();  // Save the physics time
...
//------------------------------------------------------------------------
IFireController* CVehicle::GetFireController(u32 controllerNum)
{
...
//------------------------------------------------------------------------
bool CVehicle::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags)
{
	if (m_pMovement)
...
//------------------------------------------------------------------------
void CVehicle::OnAction(const TVehicleActionId actionId, i32 activationMode, float value, EntityId callerId)
{
	if (CVehicleSeat* pCurrentSeat = (CVehicleSeat*)GetSeatForPassenger(callerId))
	{
		...
		if (performAction && activationMode == eAAM_OnPress)
		{
			...
			{
				i32 seatIndex = actionId - eVAI_ChangeSeat;
				...
				{
					if (IActor* pUser = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(callerId))
					{
...
void CVehicle::OnHit(const HitInfo& hitInfo, IVehicleComponent* pHitComponent)
{
	...
	if (damage > 0.f)
	{
		...
		else if (IActor* pActor = CDrxAction::GetDrxAction()->GetClientActor())
		{
			if (IsPlayerDriving())
			{
	...
	{
		IGameRules* pGR = CDrxAction::GetDrxAction()->GetIGameRulesSystem()->GetCurrentGameRules();
		tukk pHitClass = pGR->GetHitType(hitInfo.type);
		DrxLog("============================================================");
		DrxLog("<%s> OnHit ('%s'): localPos %.2f %.2f %.2f, radius %.2f", GetEntity()->GetName(), pHitClass, localPos.x, localPos.y, localPos.z, radius);
	}
#endif
	...
	{
		IActor* pShooter = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(hitInfo.shooterId);
...
bool CVehicle::IsActionUsable(const SVehicleActionInfo& actionInfo, const SMovementState* movementState)
{
	     ...
	     ite != end; ++ite)
	{
		...
		if (activationInfo.m_type == SActivationInfo::eAT_OnUsed)
		{
			...
			i32 hit = Intersect::Lineseg_AABB(lineSeg, bounds, intersectOut);
...
//------------------------------------------------------------------------
i32 CVehicle::IsUsable(EntityId userId)
{
	...
	DRX_ASSERT(pController);
	SMovementState movementState;
	pController->GetMovementState(movementState);

	i32k actionsUsable = IsFlipped() ? 0 : 1;
	...
	if (m_usesVehicleActionToEnter)
	{
		...
		for (TVehicleActionVector::iterator ite = m_actions.begin(), end = m_actions.end(); ite != end; ++ite)
		{
			...
			if ((actionsUsable ^ actionInfo.useWhenFlipped) && IsActionUsable(actionInfo, &movementState))
			{
				...
				{
					i32 ret = (actionIndex << 16) + eventParams.iParam;
					return ret;
				}
			}
...
//------------------------------------------------------------------------
bool CVehicle::OnUsed(EntityId userId, i32 index)
{
	if (index <= 0)
		return false;

	IActor* pUser = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(userId);
	if (!pUser)
		return false;
	...
	if (gEnv->bServer)
	{
		...
		DrxLog("[VEHICLE] %s is using vehicle %s...", userDesc.c_str(), vehicleDesc.c_str());
		INDENT_LOG_DURING_SCOPE();

		if (m_usesVehicleActionToEnter)
		{
			i32 actionIndex = (index & 0xffff0000) >> 16;
			i32 param = index & 0x0000ffff;
...
IVehiclePart* CVehicle::AddPart(const CVehicleParams& partParams, IVehiclePart* parent, SPartInitInfo& initInfo)
{
	...
	if (IVehiclePart* pPart = m_pVehicleSystem->CreateVehiclePart(partClass))
	{
		...
		{
			DrxLog("Vehicle error: part <%s> failed to initialize on vehicle <%s>", partName.c_str(), GetEntity()->GetName());
...
bool CVehicle::AddSeat(const SmartScriptTable& seatTable)
{
	...
	{
		CDrxAction::GetDrxAction()->GetVehicleSeatScriptBind()->AttachTo(this, seatId);
	}
...
//------------------------------------------------------------------------
IVehiclePart* CVehicle::GetPart(tukk name)
{
	if (0 == name)
		return NULL;
...
//------------------------------------------------------------------------
IVehicleAnimation* CVehicle::GetAnimation(tukk name)
{
	if (0 == name)
		return NULL;
...
//------------------------------------------------------------------------
bool CVehicle::AddHelper(tukk pName, Vec3 position, Vec3 direction, IVehiclePart* pPart)
{
	...
	if (position.IsZero() && (direction.IsZero() || direction.IsEquivalent(FORWARD_DIRECTION)))
	{
		if (CVehiclePartAnimated* pAnimatedPart = GetVehiclePartAnimated())
		{
			      ...
			      pAnimatedPart->GetEntity()->GetCharacter(pAnimatedPart->GetSlot()))
			{
				...
				i16 id = rIDefaultSkeleton.GetJointIDByName(pName);
				if (id > -1)
				{
...
void CVehicle::InitHelpers(const CVehicleParams& table)
{
	...
	i32 i = 0;
	i32 c = helpersTable.getChildCount();
	...
	for (; i < c; i++)
	{
		if (CVehicleParams helperRef = helpersTable.getChild(i))
		{
			...
			if (IVehiclePart* pPart = GetPart(helperRef.getAttr("part")))
				...
				DrxLog("Error adding helper <%s> on vehicle <%s>", helperRef.getAttr("part"), GetEntity()->GetName());
		}
	}
...
void CVehicle::CreateAIHidespots()
{
	...
	for (TVehicleHelperMap::iterator it = m_helpers.begin(); it != endIte; ++it)
	{
		...
		if (helperName.substr(0, 3) == "ai_")
		{
			...
			if (!pEntityClass)
			{
				DrxLog("Entity class 'AIAnchor' not found (entity name: %s)", GetEntity()->GetName());
				continue;
			}
...
//------------------------------------------------------------------------
bool CVehicle::HasHelper(tukk pName)
{
...
//------------------------------------------------------------------------
IVehicleComponent* CVehicle::GetComponent(tukk name)
{
	if (0 == name)
		return NULL;
...
bool CVehicle::SetMovement(const string& movementName, const CVehicleParams& table)
{
	...
	{
		DrxLogAlways("CVehicle::SetMovemement() failed to init movement <%s> on vehicle of class <%s>.", movementName.c_str(), GetEntity()->GetClass()->GetName());
		SAFE_RELEASE(m_pMovement);
...
//------------------------------------------------------------------------
float CVehicle::GetSelfCollisionMult(const Vec3& velocity, const Vec3& normal, i32 partId, EntityId colliderId) const
{
	float mult = 1.f;
	bool wheel = false;
	...
	if (colliderId == 0 && zspeed > m_damageParams.groundCollisionMinSpeed)
	{
		...
		if (VehicleCVars().v_debugCollisionDamage)
			DrxLog("[coll] %s [part %i], zspeed: %.1f [min: %.1f, max: %.1f]", GetEntity()->GetName(), partId, zspeed, m_damageParams.groundCollisionMinSpeed, m_damageParams.groundCollisionMaxSpeed);
#endif
...
#if ENABLE_VEHICLE_DEBUG
		if (VehicleCVars().v_debugCollisionDamage)
			DrxLog("[coll] %s ratio %.2f, groundmult %.2f", GetEntity()->GetName(), speedratio, groundMult);
#endif
	...
	if (wheel)
	{
		...
		{
			if (VehicleCVars().v_debugCollisionDamage)
				DrxLog("[coll] %s, wheelMult %.2f", GetEntity()->GetName(), wheelMult);
		}
#endif
...
#if ENABLE_VEHICLE_DEBUG
					if (VehicleCVars().v_debugCollisionDamage)
						DrxLog("[coll] above destruction speed: speed %.2f, threshold %.2f, mult %.2f", velocity.GetLength(), m_damageParams.vehicleCollisionDestructionSpeed, mult);
#endif
				}
			}
...
bool CVehicle::InitParticles(const CVehicleParams& table)
{
		...
		if (exhaustParams.helpers.size() != 0)
		{
			...
			if (CVehicleParams pTable = exhaustTable.findChild("EngineStart"))
			{
				...
				if (!exhaustParams.startEffect.empty())
					m_pParticleEffects.push_back(gEnv->pParticleUpr->FindEffect(exhaustParams.startEffect, "CVehicle::InitParticles"));
			}
			...
			if (CVehicleParams pTable = exhaustTable.findChild("EngineStop"))
			{
				...
				if (!exhaustParams.stopEffect.empty())
					m_pParticleEffects.push_back(gEnv->pParticleUpr->FindEffect(exhaustParams.stopEffect, "CVehicle::InitParticles"));
			}
			...
			if (CVehicleParams pTable = exhaustTable.findChild("EngineRunning"))
			{
				...
				if (!exhaustParams.runEffect.empty())
					m_pParticleEffects.push_back(gEnv->pParticleUpr->FindEffect(exhaustParams.runEffect, "CVehicle::InitParticles"));

				if (!exhaustParams.boostEffect.empty())
					m_pParticleEffects.push_back(gEnv->pParticleUpr->FindEffect(exhaustParams.boostEffect, "CVehicle::InitParticles"));
	...
	{
		i32 i = 0;
		i32 c = damageEffectsTable.getChildCount();
		...
		for (; i < c; i++)
		{
			if (CVehicleParams damageEffectRef = damageEffectsTable.getChild(i))
			{
				...
				{
					tukk pEffectName = damageEffectRef.getAttr("name");
...
#if ENABLE_VEHICLE_DEBUG
					if (VehicleCVars().v_debugdraw == eVDB_Damage)
						DrxLog("Adding damage effect: name %s, effect %s", damageEffectsTable.getAttr("name"), damageEffect.effectName.c_str());
#endif

					m_particleParams.damageEffects.insert(TDamageEffectMap::value_type(pEffectName, damageEffect));

					// preload
					m_pParticleEffects.push_back(gEnv->pParticleUpr->FindEffect(damageEffect.effectName.c_str(), "CVehicle::InitParticles"));
				}
			}
		}
	}
	...
	{
		i32 cnt = envTable.getChildCount();
		...
		for (i32 i = 1; i <= cnt; ++i)
		{
			...
			{
				i32 n = 0;
				i32 cEmitters = emitterTable.getChildCount();
			...
			if (CVehicleParams wheelsTable = layerTable.findChild("Wheels"))
			{
				...
				else
				{
					...
					{
						i32 n = 0;
						i32 cWheelGroups = groupsTable.getChildCount();
						...
						for (; n < cWheelGroups; n++)
						{
							if (CVehicleParams wheelGroupRef = groupsTable.getChild(n))
							{
								...
								{
									i32 k = 0;
									i32 cWheels = wheelArrayRef.getChildCount();
...
//------------------------------------------------------------------------
TVehicleSeatId CVehicle::GetSeatId(tukk pSeatName)
{
	i32 seatId = InvalidVehicleSeatId + 1;
...
//------------------------------------------------------------------------
void CVehicle::ChangeSeat(EntityId actorId, i32 seatChoice, TVehicleSeatId newSeatId)
{
	if (IsFlipped())
...
//------------------------------------------------------------------------
void CVehicle::RegisterVehicleEventListener(IVehicleEventListener* pEventListener, tukk name)
{
...
const CVehicleSeatActionWeapons* CVehicle::GetCurrentSeatActionWeapons(EntityId passengerId, bool secondary) const
{
	...
	{
		i32k transition = pSeat->GetCurrentTransition();
...
bool CVehicle::HasFriendlyPassenger(IEntity* pPlayer)
{
	...
	for (i32 i = FirstVehicleSeatId, nSeatCount = GetSeatCount(); i <= nSeatCount; ++i)
	{
...
//------------------------------------------------------------------------
IVehiclePart* CVehicle::GetPart(u32 index)
{
...
//------------------------------------------------------------------------
void CVehicle::GetParts(IVehiclePart** parts, i32 nMax)
{
	for (i32 i = 0; i < m_parts.size() && i < nMax; ++i)
		parts[i] = m_parts[i].second;
}

//------------------------------------------------------------------------
IVehiclePart* CVehicle::GetWheelPart(i32 idx)
{
	DRX_ASSERT(idx >= 0 && idx < GetWheelCount());
...
//------------------------------------------------------------------------
IVehicleAction* CVehicle::GetAction(i32 index)
{
...
//------------------------------------------------------------------------
IVehicleHelper* CVehicle::GetHelper(tukk pName)
{
	if (!pName)
		return NULL;
...
//------------------------------------------------------------------------
i32 CVehicle::GetComponentCount() const
{
	return (i32)m_components.size();
}

//------------------------------------------------------------------------
IVehicleComponent* CVehicle::GetComponent(i32 index)
{
...
//------------------------------------------------------------------------
tukk CVehicle::GetSoundName(tukk eventName, bool isEngineSound)
{
	return "";
}
...
//	{
//		tukk name = GetSoundName(info.name.c_str(), info.isEngineSound);
//		Matrix34 tm;
...
//------------------------------------------------------------------------
void CVehicle::SetSoundParam(TVehicleSoundEventId eventId, tukk param, float value, bool start /*=true*/)
{
...
void CVehicle::StopSound(TVehicleSoundEventId eventId)
{
	...
	pInfo->soundId = DrxAudio::InvalidControlId;
}
...
void CVehicle::StartAbandonedTimer(bool force, float timer)
{
	...
	pEntity->SetTimer(eVT_Abandoned, (i32)(max(mintime + 0.5f, timer) * 1000.0f));

	pEntity->KillTimer(eVT_AbandonedSound);
	pEntity->SetTimer(eVT_AbandonedSound, (i32)(max(0.0f, ((timer + 0.5f) - mintime)) * 1000.0f)); // warn sound
}
...
void CVehicle::CheckFlippedStatus(const float deltaTime)
{
	...
	if (flipping || unflipping)
	{
		...
		if (VehicleCVars().v_debugdraw)
			DrxLog("%s: %s (%.2f)", GetEntity()->GetName(), flipping ? "flipping" : "unflipping", m_status.flipped);
#endif
...
void CVehicle::ProcessFlipped()
{
	...
	if (!ai)
	{
		...
		else
		{
			...
			i32 count = gEnv->pEntitySystem->QueryProximity(query);
...
//------------------------------------------------------------------------
i32 CVehicle::GetNextPhysicsSlot(bool high) const
{
	...
	i32 idMax = (high) ? EntityPhysicsUtils::PARTID_MAX_SLOTS - 10 : -1;

	// get next physid not belonging to CGA range
	for (TVehiclePartVector::const_iterator ite = m_parts.begin(); ite != m_parts.end(); ++ite)
	{
		i32 physId = ite->second->GetPhysId();
...
float CVehicle::GetAltitude()
{
	if (iszero(m_status.altitude))
	{
		...
		if (i32 hitCount = gEnv->pPhysicalWorld->RayWorldIntersection(worldPos, Vec3(0.0f, 0.0f, -m_status.altitude),
...
//------------------------------------------------------------------------
i32 CVehicle::GetWeaponCount() const
{
	i32 count = 0;
...
//------------------------------------------------------------------------
EntityId CVehicle::GetWeaponId(i32 index) const
{
	i32 count = 0;
	...
	for (TVehicleSeatVector::const_iterator it = m_seats.begin(); it != itSeatsEnd; ++it)
	{
		...
		for (TVehicleSeatActionVector::const_iterator itActions = seatActions.begin(), itActionsEnd = seatActions.end(); itActions != itActionsEnd; ++itActions)
		{
			if (CVehicleSeatActionWeapons* pAction = CAST_VEHICLEOBJECT(CVehicleSeatActionWeapons, itActions->pSeatAction))
			{
				for (i32 i = 0; i < pAction->GetWeaponCount(); ++i)
				{
					if (count++ == index)
...
//------------------------------------------------------------------------
void CVehicle::GetMemoryUsage(IDrxSizer* s) const
{
...
IMPLEMENT_RMI(CVehicle, SvRequestUse)
{
	...
	if (IActor* pUser = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(params.actorId))
	{
		if (!pUser->IsClient())
		{
...
IMPLEMENT_RMI(CVehicle, ClRequestComplete)
{
	if (IActor* pUser = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(params.actorId))
	{
		DRX_ASSERT(pUser->IsStillWaitingOnServerUseResponse());
...
{
	i32 nseats = (i32)params.seats.size();
	for (i32 s = 0; s < nseats; s++)
	{
		...
		for (TVehicleSeatActionVector::const_iterator ait = seatActions.begin(), aitEnd = seatActions.end(); ait != aitEnd; ++ait)
		{
			...
			if (CVehicleSeatActionWeapons* weapAction = CAST_VEHICLEOBJECT(CVehicleSeatActionWeapons, pSeatAction))
			{
				DRX_ASSERT(iaction < params.seats[s].seatactions.size());
				i32 nweapons = (i32)params.seats[s].seatactions[iaction].weapons.size();

				for (i32 w = 0; w < nweapons; w++)
				{
...
IMPLEMENT_RMI(CVehicle, ClRespawnWeapon)
{
	...
	DRX_ASSERT_MESSAGE(false, "CVehicle::ClRespawnWeapon - Unable to find the correct seat weapon action to place the newly spawned weapon");
	DrxLog("CVehicle::ClRespawnWeapon - Unable to find the correct seat weapon action to place the newly spawned weapon");
	return true;
}
...
bool CVehicle::GetExitPositionForActor(IActor* pActor, Vec3& pos, bool extended /*=false*/)
{
	...
	for (i32 i = 0; i < m_pVehicle->GetSeatCount(); ++i)
	{
...
void CVehicle::ExitVehicleAtPosition(EntityId passengerId, const Vec3& pos)
{
	...
	{
		if (IActor* pUser = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(passengerId))
		{
			if (!pUser->IsStillWaitingOnServerUseResponse())
			{
				pUser->SetStillWaitingOnServerUseResponse(true);
...
i32 CVehicle::GetSkipEntities(IPhysicalEntity** pSkipEnts, i32 nMaxSkip)
{
	i32 nskip = 0;
	...
	for (i32 i = 0, count = m_pVehicle->GetEntity()->GetChildCount(); i < count && nskip < nMaxSkip; ++i)
	{
...
bool CVehicle::ExitSphereTest(IPhysicalEntity** pSkipEnts, i32 nSkip, Vec3 startPos, Vec3 testPos, EntityId* pBlockingEntity)
{
	...
	{
		CPersistantDebug* pPersistantDebug = CDrxAction::GetDrxAction()->GetPersistantDebug();
...
{
	//DrxLog("Starting extended vehicle exit check");
	static i32 numChecks = 4;
	...
	static i32k maxSkip = 15;
	IPhysicalEntity* pSkipEnts[maxSkip];
	i32 numSkip = GetSkipEntities(pSkipEnts, maxSkip - numChecks);
	...
	for (i32 check = 0; check < numChecks; ++check)
	{
		IActor* pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(blockerId);
		if (!pActor)
			break;
...
tukk CVehicle::GetModification() const
{
