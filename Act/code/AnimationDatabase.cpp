...
i32 CAnimationDatabase::s_mnAllowEditableDatabasesInPureGame = 0;
...
float AppendLayers(SFragmentData& fragmentData, const CFragment& fragment, u8 partID, const IAnimationSet* animSet, float startTime, float startOffset, bool isBlend, bool& isOneShot)
{
	...
	u32 numALayers = fragment.m_animLayers.size();
	u32 numPLayers = fragment.m_procLayers.size();
	...
	for (u32 i = 0; i < numALayers; i++)
	{
		u32k oldLength = fragmentData.animLayers[i].size();
		u32k fragLength = fragment.m_animLayers[i].size();
		const bool hadEntry = !fragmentData.animLayers[i].empty();
		const bool hasNewEntry = (fragLength > 0);
		float lastDuration = 0.0f;
		i32 startIdx = oldLength;
		...
		u32 installFragLength = isBlend ? max(fragLength, 1u) : fragLength;

		fragmentData.animLayers[i].resize(startIdx + installFragLength);
		for (u32 k = 0; k < installFragLength; k++)
		{
			...
			if (animSet)
			{
				...
				{
					i32 animID = animSet->GetAnimIDByCRC(animClip.animation.animRef.crc);
					if (0 <= animID)
					{
						animDuration = ((animSet->GetDuration_sec(animID) - animClip.blend.startTime) / animClip.animation.playbackSpeed);
						u32k flags = animSet->GetAnimationFlags(animID);
	...
	for (u32 i = numALayers; i < fragmentData.animLayers.size(); i++)
	{
		u32k oldLength = fragmentData.animLayers[i].size();
		const bool hadEntry = !fragmentData.animLayers[i].empty();
	...
	{
		u32k oldLength = fragmentData.procLayers[i].size();
		u32k fragLength = fragment.m_procLayers[i].size();
		...
		u32 installProcLength = isBlend ? max(fragLength, 1u) : fragLength;

		fragmentData.procLayers[i].resize(startIdx + installProcLength);

		float procTotalTime = 0.0f;
		for (u32 k = 0; k < installProcLength; k++)
		{
			const bool firstClip = (k == 0);
	...
	//--- Ensure that any dangling entries from the previous blend get assigned to last part
	for (u32 i = numPLayers; i < fragmentData.procLayers.size(); i++)
	{
		u32k oldLength = fragmentData.procLayers[i].size();
...
float AppendBlend(SFragmentData& outFragmentData, const SBlendQueryResult& blend, const IAnimationSet* inAnimSet, u8 partID, float& timeOffset, float& timeTally, u32& retFlags)
{
...
u32 CAnimationDatabase::Query(SFragmentData& outFragmentData, const SBlendQuery& inBlendQuery, u32 inOptionIdx, const IAnimationSet* inAnimSet, SFragmentSelection* outFragSelection) const
{
	u32 retFlags = 0;
...
{
	u32 totalAnimClips = 0;
	u32 totalProcClips = 0;
	u32k numFragmentTypes = m_fragmentList.size();
	for (u32 i = 0; i < numFragmentTypes; i++)
	{
		const TFragmentOptionList& optionList = tagSetList.m_values[i];
		u32 numOptions = optionList.size();
		for (u32 o = 0; o < numOptions; o++)
		{
			u32 numAnimLayers = optionList[o].fragment->m_animLayers.size();
			u32 numProcLayers = optionList[o].fragment->m_procLayers.size();

			for (u32 l = 0; l < numAnimLayers; l++)
			{
...
void CAnimationDatabase::SetEntry(FragmentID fragmentID, const SFragTagState& tags, u32 optionIdx, const CFragment& fragment)
{
	u32k numFragmentTypes = m_fragmentList.size();
	if (fragmentID < numFragmentTypes)
	{
		...
		if (pOptionList)
		{
			DRX_ASSERT(optionIdx < (u32)pOptionList->size());
...
bool CAnimationDatabase::DeleteEntry(FragmentID fragmentID, const SFragTagState& tags, u32 optionIdx)
{
	u32k numFragmentTypes = m_fragmentList.size();
	if (fragmentID < numFragmentTypes)
	{
		...
		i32 idx = fragmentEntry.tagSetList.FindIdx(tags);

		if (idx < 0)
		{
			DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "[AnimDatabase:DeleteEntry] Invalid fragment tags: %d passed to %s", fragmentID, m_filename.c_str());
			return false;
		}
		...
		else
		{
			...
			if (optionIdx < (u32)optionList.size())
			{
			...
			{
				DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "AnimDatabase: Invalid option index: %u for fragment id: %d passed to %s", optionIdx, fragmentID, m_filename.c_str());
				return false;
			}
		}
	...
	{
		DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, "AnimDatabase: Invalid fragment id: %d passed to %s", fragmentID, m_filename.c_str());
	}

	return false;
}

u32 CAnimationDatabase::AddEntry(FragmentID fragmentID, const SFragTagState& tags, const CFragment& fragment)
{
	u32k numFragmentTypes = m_fragmentList.size();
	if (fragmentID < numFragmentTypes)
	{
		...
		u32k newOption = pOptionList->size() - 1;
...
void CAnimationDatabase::CompressFragmentID(FragmentID fragID)
{
	...
	if (pEntry)
	{
		...
		u32k numUsed = pEntry->tagSetList.Size();
		if (numUsed > 0)
		{
...
{
	u32k numFragments = m_pFragDef->GetNum();
...
#if 0
			u32 cost = numUsed * 8 * 2;
			u32 minmem = pEntry->compiledList->GetKeySize();
			u32 newCost = numUsed * minmem;
...
#if 0
	{
		...
		for (u32 i = 0; i < numIts; i++)
		{
			for (u32 f = 0; f < numFragments; f++)
			{
		...
		u32 ticks = (u32)(updateEnd.QuadPart - updateStart.QuadPart);
		DrxLogAlways("Database %s Fragments: %d Time: %d Hash: %d", GetFilename(), numFragments, ticks, hash);

		DrxLogAlways("Compressing %s Num FragIDs: %d Num Frags: %d old cost: %d new cost: %d saving: %d", GetFilename(), numFragments, totalEntries, totalMem, totalNewMem, totalMem - totalNewMem);
		...
		for (u32 f = 0; f < numFragments; f++)
		{
			...
			{
				DRX_ASSERT(pOptionList->size() == pOptionListNew->size());
			}
			else
			{
				DRX_ASSERT(pOptionList == pOptionListNew);
			}
		}
	}
#endif //0
...
/*
   bool ValidateBlend(const IAnimationSet *animSet, const SAnimBlend &animBlend, AnimID anim1, tukk animName1, AnimID anim2, tukk animName2, const TagState &tags, MannErrorCallback errorCallback, uk errorCallbackContext)
   {
   if (!animBlend.transition.animRef.IsEmpty())
   {
    i32 animID = animSet->GetAnimIDByCRC(animBlend.transition.animRef.crc);
    if (animID == -1)
    {
      if (errorCallback)
      {
        ...
        drx_sprintf(mannErrorReport.error, "Animation not found: %s in blend from %s to %s", animBlend.transition.animRef.GetAnimName(), animName1, animName2);
      ...
      DrxLog("Animation not found: %s in blend from %s to %s", animBlend.transition.animRef.GetAnimName(), animName1, animName2);
      return false;
    }
   }
   return true;
...
{
	u32k numFragments = m_fragmentList.size();
	for (u32 i = 0; i < numFragments; i++)
	{
		...
		{
			u32k numTagVars = entry.compiledList->Size();
			for (u32 t = 0; t < numTagVars; t++)
			{
...
bool CAnimationDatabase::ValidateFragment(const CFragment* pFragment, const IAnimationSet* animSet, SMannequinErrorReport& errorReport, MannErrorCallback errorCallback, uk errorCallbackContext) const
{
	bool noErrors = true;

	u32k numLayers = pFragment->m_animLayers.size();
	for (u32 l = 0; l < numLayers; l++)
	{
		const TAnimClipSequence& animLayer = pFragment->m_animLayers[l];
		u32k numClips = animLayer.size();
		for (u32 c = 0; c < numClips; c++)
		{
			...
			if (!animClip.animation.IsEmpty())
			{
				...
				if (animID == -1)
				{
					...
					if (errorCallback)
					{
						drx_sprintf(errorReport.error, "Animation not found: %s in layer: %u", animClip.animation.animRef.c_str(), l);

						(*errorCallback)(errorReport, errorCallbackContext);
					}
					DrxLog("Animation not found: %s in layer: %u", animClip.animation.animRef.c_str(), l);
				}
			}
		}
	}
...
bool CAnimationDatabase::ValidateAnimations(FragmentID fragID, u32 tagSetID, u32 numOptions, const SFragTagState& tagState, const IAnimationSet* animSet, MannErrorCallback errorCallback, uk errorCallbackContext) const
{
	bool noErrors = true;
...
bool CAnimationDatabase::IsDuplicate(const CFragment* pFragmentA, const CFragment* pFragmentB) const
{
	...
	u32k numAnimLayersA = pFragmentA->m_animLayers.size();
	u32k numAnimLayersB = pFragmentB->m_animLayers.size();

	u32k numProcLayersA = pFragmentA->m_procLayers.size();
	u32k numProcLayersB = pFragmentB->m_procLayers.size();
...
bool CAnimationDatabase::Validate(const IAnimationSet* animSet, MannErrorCallback errorCallback, MannErrorCallback warningCallback, uk errorCallbackContext) const
{
	bool noErrors = true;
	u32k numFragments = m_fragmentList.size();
	for (u32 i = 0; i < numFragments; i++)
	{
		u32k numTagSets = GetTotalTagSets(i);
		for (u32 k = 0; k < numTagSets; k++)
		{
			SFragTagState tags;
			u32 numOptions = GetTagSetInfo(i, k, tags);
			...
			if (warningCallback)
			{
				for (u32 m = 0; m < numTagSets; m++)
				{
					if (k != m)
					{
						SFragTagState compareTags;
						u32 compareNumOptions = GetTagSetInfo(i, m, compareTags);
						...
						if (compareNumOptions == numOptions)
						{
							    ...
							    && ((pFragTagDef == NULL) || pFragTagDef->Contains(tags.fragmentTags, compareTags.fragmentTags)))
							{
								...
								for (u32 n = 0; n < compareNumOptions && allOptionsAreDuplicated; n++)
								{
									const CFragment* childFragment = GetEntry(i, k, n);

									bool foundDuplicateOption = false;
									for (u32 p = 0; (p < compareNumOptions) && !foundDuplicateOption; p++)
									{
								...
								if (allOptionsAreDuplicated)
								{
									...
									DrxStackStringT<char, 1024> sFragTagList;
									...
									drx_sprintf(mannErrorReport.error, "Fragment is duplicate of a parent fragment! Parent Tags - Global: '%s' Fragment: '%s'", sTagList.c_str(), sFragTagList.c_str());
									mannErrorReport.tags = tags;
	...
	if (animSet)
	{
		...
		for (TFragmentBlendDatabase::const_iterator iter = m_fragmentBlendDB.begin(); iter != m_fragmentBlendDB.end(); ++iter)
		{
			...
			u32k numVariants = variantList.size();
...
void CAnimationDatabase::EnumerateFragmentAnimAssets(const CFragment* pFragment, const IAnimationSet* animSet, SAnimAssetReport& assetReport, MannAssetCallback assetCallback, uk callbackContext) const
{
	u32k numLayers = pFragment->m_animLayers.size();
	for (u32 l = 0; l < numLayers; l++)
	{
...
void CAnimationDatabase::EnumerateAnimAssets(const IAnimationSet* animSet, MannAssetCallback assetCallback, uk callbackContext) const
{
	SAnimAssetReport mannAssetReport;

	u32k numFragments = m_fragmentList.size();
	for (u32 fragID = 0; fragID < numFragments; fragID++)
	{
		u32k numTagSets = GetTotalTagSets(fragID);
		for (u32 k = 0; k < numTagSets; k++)
		{
			SFragTagState tags;
			u32 numOptions = GetTagSetInfo(fragID, k, tags);
...
{
	u32 bits1 = m_tagDefs.RateTagState(lhs.tagsFrom.globalTags) + (m_pFragTagDefsFrom ? m_pFragTagDefsFrom->RateTagState(lhs.tagsFrom.fragmentTags) : 0);
	u32 bits2 = m_tagDefs.RateTagState(rhs.tagsFrom.globalTags) + (m_pFragTagDefsFrom ? m_pFragTagDefsFrom->RateTagState(rhs.tagsFrom.fragmentTags) : 0);
...
SFragmentBlendUid CAnimationDatabase::AddBlendInternal(FragmentID fragmentIDFrom, FragmentID fragmentIDTo, const SFragTagState& tagsFrom, const SFragTagState& tagsTo, const SFragmentBlend& fragBlend)
{
	...
	u32k numVariants = blendEntry.variantList.size();
...
u32 CAnimationDatabase::GetNumBlends(FragmentID fragmentIDFrom, FragmentID fragmentIDTo, const SFragTagState& tagFrom, const SFragTagState& tagTo) const
{
...
const SFragmentBlend* CAnimationDatabase::GetBlend(FragmentID fragmentIDFrom, FragmentID fragmentIDTo, const SFragTagState& tagFrom, const SFragTagState& tagTo, u32 blendNum) const
{
	const SFragmentBlendVariant* variant = GetVariant(fragmentIDFrom, fragmentIDTo, tagFrom, tagTo);
...
void CAnimationDatabase::DeleteBlend(FragmentID fragmentIDFrom, FragmentID fragmentIDTo, const SFragTagState& tagFrom, const SFragTagState& tagTo, SFragmentBlendUid blendUid)
{
	...
	if (variant)
	{
		u32k blendNum = variant->FindBlendIndexForUid(blendUid);
...
tukk CAnimationDatabase::FindSubADBFilenameForIDInternal(FragmentID fragmentID, const SSubADB& pSubADB) const
{
	// Search FragIDs
	...
	{
		tukk fName = FindSubADBFilenameForIDInternal(fragmentID, *itSubADB);
...
tukk CAnimationDatabase::FindSubADBFilenameForID(FragmentID fragmentID) const
{
...
const CAnimationDatabase::SSubADB* CAnimationDatabase::FindSubADB(const TSubADBList& subAdbList, tukk szSubADBFilename, bool recursive)
{
...
CAnimationDatabase::SSubADB* CAnimationDatabase::FindSubADB(tukk szSubADBFilename, bool recursive)
{
	return const_cast<SSubADB*>(FindSubADB(m_subADBs, szSubADBFilename, recursive));
}

const CAnimationDatabase::SSubADB* CAnimationDatabase::FindSubADB(tukk szSubADBFilename, bool recursive) const
{
...
CAnimationDatabase::SFragmentBlendVariant* CAnimationDatabase::GetVariant(FragmentID fragmentIDFrom, FragmentID fragmentIDTo, const SFragTagState& tagFrom, const SFragTagState& tagTo) const
{
	...
	if (iter != m_fragmentBlendDB.end())
	{
		...
		u32k numVariants = entry.variantList.size();
		for (u32 i = 0; i < numVariants; i++)
		{
...
void CAnimationDatabase::FindBestBlendInVariant(const SFragmentBlendVariant& variant, const SBlendQuery& blendQuery, SBlendQueryResult& result) const
{
	...
	{
		u32k blendIdx = variant.FindBlendIndexForUid(blendQuery.forceBlendUid);
