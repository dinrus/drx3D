...
// -------------------------------------------------------------------------
//  Имя файла:   StatisticsHelpers.cpp
//  Version:     v1.00
...
#include <drx3D/Act/StatisticsHelpers.h>
...
{
	i32 eventCount = pGameStats->GetEventCount();
	i32 stateCount = pGameStats->GetStateCount();
...
void CStatsTracker::GetMemoryStatistics(IDrxSizer* pSizer)
{
	pSizer->Add(*this);
...
void CStatsContainer::GetMemoryStatistics(IDrxSizer* pSizer)
{
	pSizer->Add(*this);
...
void SDeadStatNode::GetMemoryStatistics(IDrxSizer* pSizer) const
{
...
void SScopeData::GetMemoryStatistics(IDrxSizer* pSizer) const
{
	pSizer->Add(*this);
...
{
	DRX_ASSERT(scopeDescs);
	DRX_ASSERT(numScopes > 0);
...
const SGameScopeDesc* CGameScopes::GetScopeDesc(tukk scriptName) const
{
...
CStatsTracker* CGameScopes::PushGameScope(size_t scopeID, u32 timestamp, CGameStatistics* gameStatistics)
{
	DRX_ASSERT(scopeID < m_scopeRegistry.size());
	if (scopeID >= m_scopeRegistry.size())
		return 0;

	// Check that same scope isn't already on the stack
	size_t pos = FindScopePos(scopeID);
	DRX_ASSERT_MESSAGE(pos == m_scopeStack.size(), "Same scope can't be pushed twice");
...
bool CGameScopes::ValidateScopes(const SGameScopeDesc scopeDescs[], size_t numScopes)
{
	...
	std::set<tukk , stl::less_strcmp<tukk >> scriptNames;
...
void CGameScopes::InsertScopesInRegistry(const SGameScopeDesc scopeDescs[], size_t numScopes)
{
	for (size_t i = 0; i != numScopes; ++i)
	{
		DRX_ASSERT(m_scopeRegistry[scopeDescs[i].statID].scriptName.empty());
...
void CGameScopes::GetMemoryStatistics(IDrxSizer* pSizer) const
{
...
bool CStatRegistry::Register(const SGameStatDesc statDescs[], size_t numStats)
{
	...
	{
		// Check for overwrite
		DRX_ASSERT(m_statRegistry[statDescs[i].statID].scriptName.empty());
...
size_t CStatRegistry::Register(tukk scriptName, tukk serializeName)
{
...
size_t CStatRegistry::GetID(tukk scriptName) const
{
...
void CStatRegistry::GetMemoryStatistics(IDrxSizer* pSizer) const
{
...
bool CElemRegistry::Register(const SGameElementDesc elemDescs[], size_t numElems)
{
	...
	{
		// Check for overwrite
		DRX_ASSERT(m_elemRegistry[elemDescs[i].statID].scriptName.empty());
...
size_t CElemRegistry::GetID(tukk scriptName) const
{
	DRX_ASSERT(scriptName);
...
void CElemRegistry::GetMemoryStatistics(IDrxSizer* pSizer) const
{
