...
#include <drx3D/Act/FlowVehicleMovement.h>
...
void CFlowVehicleMovement::ZeroMass(bool enable)
{
	...
	if (enable)
	{
		...
		i32 count = pPhysics->GetStatus(&nparts);
	...
	{
		for (i32 i = 0, n = m_partMass.size(); i < n; ++i)
		{
...
void CFlowVehicleMovement::Serialize(SActivationInfo* pActivationInfo, TSerialize ser)
{
	...
	for (i32 i = 0; i < count; ++i)
	{
		drx_sprintf(buf, "partid_%i", i);
		ser.Value(buf, m_partMass[i].partid);

		drx_sprintf(buf, "mass_%i", i);
