...
void CPersistantDebug::Begin(tukk name, bool clear)
{
	if (clear)
...
void CPersistantDebug::AddText(float x, float y, float size, ColorF clr, float timeout, tukk fmt, ...)
{
	char buffer[4096];
	va_list args;
	va_start(args, fmt);
...
void CPersistantDebug::AddText3D(const Vec3& pos, float size, ColorF clr, float timeout, tukk fmt, ...)
{
	char buffer[4096];
	va_list args;
	va_start(args, fmt);
...
void CPersistantDebug::Add2DText(tukk text, float size, ColorF clr, float timeout)
{
	if (text == 0 || *text == '\0')
		return;
...
{
	i32 iScreenWidth  = gEnv->pRenderer->GetOverlayWidth();
	i32 iScreenHeight = gEnv->pRenderer->GetOverlayHeight();
...
void CPersistantDebug::Update(float frameTime)
{
	...
	static i32k flags3D = e_Mode3D | e_AlphaBlended | e_DrawInFrontOff | e_FillModeSolid | e_CullModeBack | e_DepthWriteOn | e_DepthTestOn;
	static i32k flags2D = e_Mode2D | e_AlphaBlended;
	...
	for (MapListObj::iterator iterMap = m_objects.begin(); iterMap != m_objects.end(); ++iterMap)
	{
		...
		for (ListObj::iterator iterList = iterMap->second.begin(); iterList != iterMap->second.end(); ++iterList)
		{
			...
			case eOT_Text3D:
				{
					if ((u8)(clr.a * 255) == 0) // Font renderer ignores the color when alpha==0
						continue;
			...
			case eOT_Disc:
				{
					...
					i32 steps = (i32)(10 * iterList->radius2);
...
void CPersistantDebug::OnWriteToConsole(tukk sText, bool bNewLine)
{
	if (sText)
	{
