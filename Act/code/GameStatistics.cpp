...
#include <drx3D/Act/ScriptBind_GameStatistics.h>
...
static i32k MAX_SAVE_ATTEMPTS = 100;
...
CGameStatistics::~CGameStatistics()
{
	DRX_ASSERT_MESSAGE(!m_gameScopes.GetStackSize(), "Stack should be empty now");
	if (m_gameScopes.GetStackSize())
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "CGameStatistics::~CGameStatistics() Stack is not empty");
	}
...
size_t CGameStatistics::RegisterGameEvent(tukk scriptName, tukk serializeName)
{
...
size_t CGameStatistics::GetEventID(tukk scriptName) const
{
...
size_t CGameStatistics::GetEventIDBySerializeName(tukk serializeName) const
{
...
size_t CGameStatistics::RegisterGameState(tukk scriptName, tukk serializeName)
{
...
size_t CGameStatistics::GetStateID(tukk scriptName) const
{
...
{
	DRX_ASSERT(m_gameScopes.GetStackSize());
	if (!m_gameScopes.GetStackSize())
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "CGameStatistics::PopGameScope() called when no scopes on the stack, ignoring.");
		return false;
	}
	...
	DRX_ASSERT(checkScopeID == INVALID_STAT_ID || checkScopeID == locator.scopeID);
	if (checkScopeID != INVALID_STAT_ID && checkScopeID != locator.scopeID)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR,
...
void CGameStatistics::PopScopesUntil(size_t scopeID)
{
	...
	while (pos < m_gameScopes.GetStackSize())
	{
		...
		if (desc && desc->scriptName)
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Normalizing scope state, popping %s", desc->scriptName.c_str());
		PopGameScope(INVALID_STAT_ID);
	}
}
...
{
	DRX_ASSERT(depth < m_gameScopes.GetStackSize());
...
size_t CGameStatistics::GetScopeID(tukk scriptName) const
{
	DRX_ASSERT(scriptName);
...
XmlNodeRef CGameStatistics::CreateStatXMLNode(tukk tag)
{
...
{
	const SGameElementDesc* desc = m_elemRegistry.GetDesc(locator.elemID);
	DRX_ASSERT(desc && (desc->locatorID == locator.locatorType));
	DRX_ASSERT(!locator.isScope());

	if (!desc || locator.isScope() || desc->locatorID != locator.locatorType)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "CGameStatistics::AddGameElement() Invalid locator");
		return false;
	}

	size_t stackPos = m_gameScopes.FindScopePos(locator.scopeID);

	DRX_ASSERT_MESSAGE(stackPos != m_gameScopes.GetStackSize(), "Elements can be added only to the scopes on the stack");
	if (stackPos == m_gameScopes.GetStackSize())
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "CGameStatistics::AddGameElement() attempt to add element to non existing scope");
		return false;
	}
...
void CGameStatistics::RemoveElement(const SNodeLocator& locator)
{
	...
	DRX_ASSERT_MESSAGE(stackPos != m_gameScopes.GetStackSize(), "No such scope on the stack");
	if (stackPos == m_gameScopes.GetStackSize())
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "CGameStatistics::RemoveElement() attempt to remove element from non existing scope");
		return;
	}
...
size_t CGameStatistics::GetElementID(tukk scriptName) const
{
...
{
	DRX_ASSERT(locator.scopeID < m_gameScopes.GetRegisteredCount());

	if (locator.isScope())
	{
		DRX_ASSERT(locator.elemID == INVALID_STAT_ID);
	...
	{
		DRX_ASSERT(m_elemRegistry.GetDesc(locator.elemID));
		DRX_ASSERT(m_elemRegistry.GetDesc(locator.elemID)->locatorID == locator.locatorType);
...
{
	// Allow reset only
	DRX_ASSERT(!m_gsCallback || !pCallback);
...
void CGameStatistics::GetMemoryStatistics(IDrxSizer* pSizer) const
{
...
tukk CGameStatistics::GetSerializeName(const SNodeLocator& locator) const
{
...
void CGameStatistics::SaveDeadNodesRec(SDeadStatNode* node)
{
	...
	tukk serializeName = GetSerializeName(node->locator);
...
void CGameStatistics::SaveScopesRec(size_t scopePos)
{
	...
	tukk sname = GetSerializeName(sd.locator);
	     ...
	     ++it)
	{
		...
		tukk elName = GetSerializeName(it->first);
...
void CGameStatistics::NotifyVisitNode(const SNodeLocator& locator, tukk serializeName, IStatsContainer& container, EStatNodeState state)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
...
void CGameStatistics::NotifyLeaveNode(const SNodeLocator& locator, tukk serializeName, IStatsContainer& container, EStatNodeState state)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
