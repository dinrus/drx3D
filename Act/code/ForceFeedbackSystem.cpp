...
public:
	static void Init()
	{
		assert(s_pDebugHistoryUpr == NULL);

		if (s_pDebugHistoryUpr == NULL)
		{
			s_pDebugHistoryUpr = CDrxAction::GetDrxAction()->CreateDebugHistoryUpr();
		}
	}

	static void Shutdown()
	{
		SAFE_RELEASE(s_pDebugHistoryUpr);
	}
	...
	static void DebugFFOutput(bool debugEnabled, float motorA, float motorB, const SFFTriggerOutputData& triggers)
	{
		...
		if (!debugEnabled)
		{
			if (s_pDebugHistoryUpr)
			{
				s_pDebugHistoryUpr->Clear();
			}
			return;
		}
		...
		if (s_pDebugHistoryUpr)
		{
			s_pDebugHistoryUpr->LayoutHelper("FFMotorA_HighFreq", NULL, true, -20, 20, 0, 1, 0.0f, 0.0f);
			s_pDebugHistoryUpr->LayoutHelper("FFMotorB_LowFreq", NULL, true, -20, 20, 0, 1, 1.0f, 0.0f);
			s_pDebugHistoryUpr->LayoutHelper("FFLeftTriggerGain", NULL, true, 0, 1, 0, 1.f, 2.0f, 0.0f);
			s_pDebugHistoryUpr->LayoutHelper("FFRightTriggerGain", NULL, true, 0, 1, 0, 1.f, 2.5f, 0.0f);
			s_pDebugHistoryUpr->LayoutHelper("FFLeftTriggerEnv", NULL, true, 0, 2000, 0, 2000.f, 3.0f, 0.0f);
			s_pDebugHistoryUpr->LayoutHelper("FFRightTriggerEnv", NULL, true, 0, 2000, 0, 2000.f, 3.5f, 0.0f);

			IDebugHistory* pDHMotorA = s_pDebugHistoryUpr->GetHistory("FFMotorA_HighFreq");
			if (pDHMotorA != NULL)
			{
				pDHMotorA->AddValue(motorA);
			}

			IDebugHistory* pDHMotorB = s_pDebugHistoryUpr->GetHistory("FFMotorB_LowFreq");
			if (pDHMotorB != NULL)
			{
				pDHMotorB->AddValue(motorB);
			}

			IDebugHistory* pDHLeftTriggerGain = s_pDebugHistoryUpr->GetHistory("FFLeftTriggerGain");
			if (pDHLeftTriggerGain != NULL)
			{
				pDHLeftTriggerGain->AddValue(triggers.leftGain);
			}

			IDebugHistory* pDHRightTriggerGain = s_pDebugHistoryUpr->GetHistory("FFRightTriggerGain");
			if (pDHRightTriggerGain != NULL)
			{
				pDHRightTriggerGain->AddValue(triggers.rightGain);
			}

			IDebugHistory* pDHLeftTriggerEnv = s_pDebugHistoryUpr->GetHistory("FFLeftTriggerEnv");
			if (pDHLeftTriggerEnv != NULL)
			{
				pDHLeftTriggerEnv->AddValue(triggers.leftEnv);
			}

			IDebugHistory* pDHRightTriggerEnv = s_pDebugHistoryUpr->GetHistory("FFRightTriggerEnv");
...
private:
	...
	static IDebugHistoryUpr* s_pDebugHistoryUpr;
};

IDebugHistoryUpr* CForceFeedBackSystemDebug::s_pDebugHistoryUpr = NULL;
#else
class CForceFeedBackSystemDebug
{
public:
...
void CForceFeedBackSystem::FFBInternalId::Set(tukk _name)
{
	assert(_name);
	name = _name;
	id = DrxStringUtils::CalculateHash(_name);
}

tukk CForceFeedBackSystem::FFBInternalId::GetDebugName() const
{
	return name.c_str();
}
...
CForceFeedBackSystem::FFBInternalId& CForceFeedBackSystem::FFBInternalId::GetIdForName(tukk name)
{
	static FFBInternalId cachedId;

	cachedId.id = DrxStringUtils::CalculateHash(name);
...
void CForceFeedBackSystem::PlayForceFeedbackEffect(ForceFeedbackFxId id, const SForceFeedbackRuntimeParams& runtimeParams)
{
	...
	i32k activeEffectSize = m_activeEffects.size();
	bool freeSlotsAvailable = (activeEffectSize < (i32)MAX_FORCE_FEEDBACK_EFFECTS);

	if (freeSlotsAvailable)
	{
		i32k testId = (i32)id;
		bool validId = ((testId >= 0) && (testId < (i32)(m_effects.size())));
...
ForceFeedbackFxId CForceFeedBackSystem::GetEffectIdByName(tukk effectName) const
{
...
{
	tukk xmlDataFile = "Libs/GameForceFeedback/ForceFeedbackEffects.xml";
	...
	for (i32 i = 0; i < childCount; ++i)
	{
		...
		tukk childTag = childNode->getTag();
...
{
	i32k patterCount = patternsNode->getChildCount();

	m_patters.reserve(patterCount);

	TSamplesBuffer samplesBuffer;
	i32k maxSampleCount = FFSYSTEM_MAX_PATTERN_SAMPLES / 2;
	float readValues[maxSampleCount];

	for (i32 i = 0; i < patterCount; ++i)
	{
		XmlNodeRef childPatternNode = patternsNode->getChild(i);

		tukk customPatternName = childPatternNode->getAttr("name");
		...
		i32 samplesFound = ParseSampleBuffer(samplesBuffer, &readValues[0], maxSampleCount);
...
{
	i32k envelopesCount = envelopesNode->getChildCount();

	m_envelopes.reserve(envelopesCount);

	TSamplesBuffer samplesBuffer;
	i32k maxSampleCount = FFSYSTEM_MAX_ENVELOPE_SAMPLES / 2;
	float readValues[maxSampleCount];

	for (i32 i = 0; i < envelopesCount; ++i)
	{
		XmlNodeRef envelopeChildNode = envelopesNode->getChild(i);

		tukk customEnvelopeName = envelopeChildNode->getAttr("name");
...
void CForceFeedBackSystem::LoadEffects(XmlNodeRef& effectsNode)
{
	...
	for (i32 i = 0; i < effectsCount; ++i)
	{
		...
		{
			SEffect newEffect;
			i32k effectDataCount = childEffectNode->getChildCount();

			tukk effectName = childEffectNode->getAttr("name");
			...
			for (i32 j = 0; j < effectDataCount; ++j)
			{
				XmlNodeRef motorNode = childEffectNode->getChild(j);

				tukk motorTag = motorNode->getTag();
			...
			m_effectToIndexMap.insert(TEffectToIndexMap::value_type(internalId, ((i32)m_effects.size() - 1)));
		}
	}
}

i32 CForceFeedBackSystem::ParseSampleBuffer(const TSamplesBuffer& buffer, float* outputValues, i32k maxOutputValues)
{
	i32 tokenStart = 0;
	i32 tokenEnd = 0;
	i32 samplesFound = 0;

	for (i32 i = 0; i < maxOutputValues; ++i)
	{
		tokenEnd = buffer.find(",", tokenStart);
		if (tokenEnd != buffer.npos)
		{
			i32k charCount = tokenEnd - tokenStart;
			samplesFound++;
		...
		{
			//Last token
			i32k charCount = buffer.length() - tokenStart;
			samplesFound++;
			outputValues[i] = (float)atof(buffer.substr(tokenStart, charCount).c_str());
			break;
		}
...
void CForceFeedBackSystem::DistributeSamples(const float* sampleInput, i32k sampleInputCount, u16* sampleOutput, i32k sampleOutputCount)
{
	i32k sampleDistributionStep = (sampleOutputCount / sampleInputCount);
	i32k sampleIterations = ((sampleInputCount % 2) == 0) ? (sampleInputCount / 2) : (sampleInputCount / 2) + 1;

	i32 lastStartOffsetIdx = 0;
	i32 lastEndOffsetIdx = (sampleOutputCount - 1);

	for (i32 i = 0; i < sampleIterations; ++i)
	{
		i32k startOffsetIdx = sampleDistributionStep * i;
		i32k endOffsetIdx = sampleOutputCount - 1 - startOffsetIdx;

		DRX_ASSERT((startOffsetIdx >= 0) && (startOffsetIdx < sampleOutputCount));
		DRX_ASSERT((endOffsetIdx >= 0) && (endOffsetIdx < sampleOutputCount));

		sampleOutput[startOffsetIdx] = (u16)(clamp_tpl(sampleInput[i], 0.0f, 1.0f) * 65535.0f);
		sampleOutput[endOffsetIdx] = (u16)(clamp_tpl(sampleInput[sampleInputCount - 1 - i], 0.0f, 1.0f) * 65535.0f);

		// Fill values in between, left side
		if (lastStartOffsetIdx < startOffsetIdx)
		{
			i32k startValue = sampleOutput[lastStartOffsetIdx];
			i32k step = (sampleOutput[startOffsetIdx] - sampleOutput[lastStartOffsetIdx]) / (startOffsetIdx - lastStartOffsetIdx);
			for (i32 j = lastStartOffsetIdx; j < startOffsetIdx; ++j)
			{
		...
		{
			i32k startValue = sampleOutput[endOffsetIdx];
			i32k step = (sampleOutput[lastEndOffsetIdx] - sampleOutput[endOffsetIdx]) / (lastEndOffsetIdx - endOffsetIdx);
			for (i32 j = endOffsetIdx; j < lastEndOffsetIdx; ++j)
			{
		...
		if (i == (sampleIterations - 1))
		{
			...
			{
				i32k startValue = sampleOutput[startOffsetIdx];
				i32k step = (sampleOutput[endOffsetIdx] - sampleOutput[startOffsetIdx]) / (endOffsetIdx - startOffsetIdx);
				for (i32 j = startOffsetIdx; j < endOffsetIdx; ++j)
				{
...
{
	CForceFeedBackSystem* pFFS = static_cast<CForceFeedBackSystem*>(CDrxAction::GetDrxAction()->GetIForceFeedbackSystem());
...
void CForceFeedBackSystem::EnumerateEffects(IFFSPopulateCallBack* pCallBack)
{
	...
	{
		tukk const pName = iter->c_str();
		pCallBack->AddFFSEffectName(pName);
		++iter;
	}
}

i32 CForceFeedBackSystem::GetEffectNamesCount() const
{
...
void CForceFeedBackSystem::SuppressEffects(bool bSuppressEffects)
{
	...
	DrxLog("[FFB] ForceFeedback EffectLock now: %d was %d", m_effectLock, bSuppressEffects ? m_effectLock - 1 : m_effectLock + 1);
}
