...
{
	DRX_ASSERT(desiredLocalLocation.IsValid());
...
void CAnimatedCharacter::SetBlendWeightParam(const EMotionParamID motionParamID, const float value, u8k targetFlags /* = eBWPT_All*/)
{
	...
	{
		DrxLogAlways("[DinrusXAnimation] ERROR: motionParam that was sent: %d is not within allowed range of blendweights %d to %d", motionParamID, eMotionParamID_BlendWeight, eMotionParamID_BlendWeight_Last);
	}
}
