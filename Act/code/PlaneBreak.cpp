...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  network breakability: Plane breaks
   -------------------------------------------------------------------------
   История:
...
#include <drx3D/Act/StdAfx.h>
	...
	#include <drx3D/Act/PlaneBreak.h>
...
void LogPlaneBreak(tukk from, const SBreakEvent& evt)
{
	/*
	   if (!pEnt)
	    return;
	   ...
	   {
	    DrxLog("[brk] DeformPhysicalEntity on %s @ (%.8f,%.8f,%.8f); n=(%.8f,%.8f,%.8f); energy=%.8f", from, p.x, p.y, p.z, n.x, n.y, n.z, energy);
	    CObjectSelector sel(pEnt);
	    DrxLog("[brk]    selector is %s", sel.GetDescription().c_str());
	    ...
	    case PHYS_FOREIGN_ID_STATIC:
	      ...
	      {
	        DrxLog("[brk]    name is %s", pRN->GetName());
	        DrxLog("[brk]    entity class name is %s", pRN->GetEntityClassName());
	        DrxLog("[brk]    debug string is %s", pRN->GetDebugString().c_str());
	      }
	      break;
	    case PHYS_FOREIGN_ID_ENTITY:
	      if (IEntity * pEntity = (IEntity*)pEnt->GetForeignData(PHYS_FOREIGN_ID_ENTITY))
	      {
	        DrxLog("[brk]    name is %s", pEntity->GetName());
	        Matrix34 m = pEntity->GetWorldTM();
	        DrxLog("[brk]    world tm:");
	        for (i32 i=0; i<3; i++)
	        {
	          Vec4 row = m.GetRow4(i);
	          DrxLog("[brk]       %+12.8f %+12.8f %+12.8f %+12.8f", row[0], row[1], row[2], row[3]);
	        }
	      }
	      break;
	    }
...
void CPlaneBreak::AbsorbStep()
{
	...
	DRX_ASSERT(m_absorbIdx < m_bes.size());
	if (m_absorbIdx >= m_bes.size())
		return;
...
i32 CPlaneBreak::GetVirtualId(IPhysicalEntity* pEnt)
{
	i32 iForeignData = pEnt->GetiForeignData();
	uk pForeignData = pEnt->GetForeignData(iForeignData);
...
CObjectSelector CPlaneBreak::GetObjectSelector(i32 idx)
{
	if (idx == 1)
	{
...
void CPlaneBreak::PreparePlaybackForEvent(i32 event)
{
	m_pPhysEnt = 0;
