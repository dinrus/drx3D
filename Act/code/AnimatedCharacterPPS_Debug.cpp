...
{
	CDebugVelocity() {}
	CDebugVelocity(const QuatT& movement, const float frameTime, tukk szComment, const ColorF& colorF, const bool pastMovement = false)
...
void CDebugVelocity::Draw(const Vec3& origin, const float textXPos, const float textYPosIn) const
{
	...
	u32k colorB = m_colorF.pack_abgr8888();
...
//--------------------------------------------------------------------------------
void CAnimatedCharacter::AddDebugVelocity(const QuatT& movement, const float frameTime, tukk szComment, const ColorF& colorF, const bool pastMovement) const
{
	static i32 s_debugVelocityRenderFrameId = -1;

	if (!DebugVelocitiesEnabled())
		return;

	// Empty list of velocities when frame changes
	i32 renderFrameId = gEnv->pRenderer->GetFrameID();
...
static void DrawDebugVelocities(const Vec3& entityPos)
{
	...
	for (i32 i = 0; i < s_debugVelocities.size(); ++i)
	{
		if (i > 0)
...
#ifdef DEBUGHISTORY
void CAnimatedCharacter::LayoutHelper(tukk id, tukk name, bool visible, float minout, float maxout, float minin, float maxin, float x, float y, float w /*=1.0f*/, float h /*=1.0f*/)
{
	if (!visible)
	{
		m_debugHistoryUpr->RemoveHistory(id);
		return;
	}

	IDebugHistory* pDH = m_debugHistoryUpr->GetHistory(id);
	if (pDH != NULL)
		return;

	pDH = m_debugHistoryUpr->CreateHistory(id, name);
...
void CAnimatedCharacter::SetupDebugHistories()
{
	...
	/*
	   ...
	   {
	   m_pEntityMoveSpeed = m_debugHistoryUpr->CreateHistory("EntityMoveSpeed");
	   ...
	   {
	   m_pEntityPhysSpeed = m_debugHistoryUpr->CreateHistory("EntityPhysSpeed");
	   ...
	   {
	   m_pACRequestSpeed = m_debugHistoryUpr->CreateHistory("ACRequestSpeed");
	...
	/*
	   LayoutUpr.getNode("root").addHorizontal("");
	   LayoutUpr.getNode("last").add(DH);
	   LayoutUpr.getNode("last").add(DH);
	   LayoutUpr.getNode("last").add(DH);
	   LayoutUpr.getNode("last").add(DH);
	   LayoutUpr.getNode("root").addHorizontal("middle").addVertical("");
	   LayoutUpr.getNode("last").add(DH);
	   LayoutUpr.getNode("last").add(DH);
	   LayoutUpr.getNode("last").add(DH);
	   LayoutUpr.getNode("root").addHorizontal("");
	   ...
	   LayoutUpr.getNode("last").add(DH);
	   LayoutUpr.getNode("last").add(DH);
	   LayoutUpr.getNode("middle").addRows(2);
	   LayoutUpr.getNode("middle.row1").addCols(2);
	   LayoutUpr.getNode("middle.row1.col0").add(DH);
...
void CAnimatedCharacter::DebugGraphQT(const QuatT& m, tukk tx, tukk ty, tukk rz, tukk tz /* = UNDEFINED */) const
{
#ifndef DEBUGHISTORY
	return;
#else
...
void CAnimatedCharacter::DebugHistory_AddValue(tukk id, float x) const
{
#ifndef DEBUGHISTORY
	return;
#else
...
bool CAnimatedCharacter::DebugFilter() const
{
	...
	tukk entityfilter = CAnimationGraphCVars::Get().m_pDebugFilter->GetString();
	tukk entityname = GetEntity()->GetName();
...
void CAnimatedCharacter::DebugBlendWeights(const ISkeletonAnim* pSkeletonAnim)
{
	/*
	   ...
	   u32 numAnimsLayer0 = pSkeleton->GetNumAnimsInFIFO(0);
	   if (numAnimsLayer0 == 1)
	   {
...
QuatT GetDebugEntityLocation(tukk name, const QuatT& _default)
{
#ifdef _DEBUG
...
#ifdef _DEBUG
bool RunExtractCombineTest(bool flat)
{
	i32 errors = 0;
...
#ifdef _DEBUG
bool RunQuatConcatTest()
{
	i32 validConcatCountWorst = 10000;
	i32 validConcatCountBest = 0;
	bool allValid = true;
	...
	for (i32 i = 0; i < 100; i++)
	{
		...
		for (i32 j = 0; j < 1000; j++)
		{
			 ...
			 */
			q.v.x = drx_random(-1.0f, 1.0f);
			q.v.y = drx_random(-1.0f, 1.0f);
			q.v.z = drx_random(-1.0f, 1.0f);
			q.w = drx_random(-1.0f, 1.0f);
			q.Normalize();
			DRX_ASSERT(q.IsValid());

			Quat qInv = q.GetInverted();
			DRX_ASSERT(qInv.IsValid());
	...
	if (!allValid)
		DrxLog("RunQuatConcatTest: valid consecutive concatenations - worst case %d, best case %d.", validConcatCountWorst, validConcatCountBest);
...
bool RunQuatPredicateTest()
{
	...
	{
		Quat q;
		q.v = Vec3(0.0f, 0.0f, drx_random(0.0f, 1.0f));
		q.w = drx_random(0.0f, 1.0f);
