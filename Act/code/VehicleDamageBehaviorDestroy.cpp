...
void CVehicleDamageBehaviorDestroy::SetDestroyed(bool isDestroyed, EntityId shooterId)
{
	...
	{
		DrxLog("%s being driven by %s has been destroyed!", m_pVehicle->GetEntity()->GetEntityTextDescription().c_str(), m_pVehicle->GetDriver() ? m_pVehicle->GetDriver()->GetEntity()->GetEntityTextDescription().c_str() : "nobody");
		...
		if (pAISystem && pAISystem->IsEnabled() && pAISystem->GetSmartObjectUpr())
		{
			gEnv->pAISystem->GetSmartObjectUpr()->SetSmartObjectState(m_pVehicle->GetEntity(), "Dead");
		}
