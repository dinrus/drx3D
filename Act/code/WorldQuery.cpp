	...
	: m_physicalEntityInFrontOf(-100)
{
	...
	m_pViewSystem = CDrxAction::GetDrxAction()->GetIViewSystem();
	m_rayHitAny = false;
...
{
#if WORLDQUERY_USE_DEFERRED_LINETESTS
	for (i32 i = 0; i < kMaxQueuedRays; ++i)
	{
...
bool CWorldQuery::Init(IGameObject* pGameObject)
{
	...
	m_pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(GetEntityId());
	if (!m_pActor)
	{
...
bool CWorldQuery::ReloadExtension(IGameObject* pGameObject, const SEntitySpawnParams& params)
{
	...
	DRX_ASSERT_MESSAGE(false, "CWorldQuery::ReloadExtension not implemented");
...
void CWorldQuery::Update(SEntityUpdateContext& ctx, i32 slot)
{
	//m_validQueries = 0;
...
void CWorldQuery::UpdateRaycastQuery()
{
	...
	static i32k obj_types = ent_all; // ent_terrain|ent_static|ent_rigid|ent_sleeping_rigid|ent_living;
	static u32k flags = rwi_pierceability(PIERCE_GLASS) | rwi_colltype_any;

#if WORLDQUERY_USE_DEFERRED_LINETESTS
	IPhysicalEntity* skipEntities[1];
	skipEntities[0] = pPhysEnt;

	i32 raySlot = GetRaySlot();
	DRX_ASSERT(raySlot != -1);

	m_queuedRays[raySlot].rayId = CDrxAction::GetDrxAction()->GetPhysicQueues().GetRayCaster().Queue(
	...
	if (timeSinceLastDeferred > k_maxTimeRaycastStaysValid)
	{
		...
		m_rayHitAny = false;
		m_lookAtEntityId = 0;
		//DrxLogAlways("CWorldQuery::UpdateRaycastQuery() - frameStartTime=%f; it's been too long (%f) since the last deferred raycast result, assuming hit nothing\n", frameStartTime, timeSinceLastDeferred);
	}
	...
	//	if (m_rayHitAny)
	//		DrxLogAlways( "HIT: terrain:%i distance:%f (%f,%f,%f)", m_rayHit.bTerrain, m_rayHit.dist, m_rayHit.pt.x, m_rayHit.pt.y, m_rayHit.pt.z );
...
void CWorldQuery::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void CWorldQuery::UpdatePhysicalEntitiesAround()
{
	...
	i32 numEntities = gEnv->pPhysicalWorld->GetEntitiesInBox(m_worldPosition - checkOffset, m_worldPosition + checkOffset, ppList, ent_static | ent_sleeping_rigid | ent_rigid | ent_living);
...
{
	i32k objectTypes = ent_static | ent_sleeping_rigid | ent_rigid | ent_living;
	u32k flags = ent_all;

	ray_hit rayHitResult;
	i32k numberOfHits = gEnv->pPhysicalWorld->RayWorldIntersection(m_worldPosition, m_proximityRadius * m_dir, objectTypes, flags, &rayHitResult, 1, (IPhysicalEntity*)0);
...
{
	i32 raySlot = GetSlotForRay(rayID);
	DRX_ASSERT(raySlot != -1);
	...
	if (rayHitSucced)
	{
		...
		i32 partId = result.hits[0].partid;
		if (partId && pEntity)
		{
...
i32 CWorldQuery::GetSlotForRay(const QueuedRayID& rayId) const
{
	for (i32 i = 0; i < kMaxQueuedRays; ++i)
	{
