...
/********************************************************************
   ...
   - 2:12:2005           Now talks directly to DinrusAction interfaces (craig tiller)
...
CAIProxy::CAIProxy(IGameObject* pGameObject) :
	...
	m_currentWeaponId(0),
	m_currentWeaponFiremodeIndex(std::numeric_limits<i32>::max()),
	m_pIActor(0),
	m_shotBulletCount(0),
	m_lastShotTime(0.0f),
...
CAIProxy::~CAIProxy()
{
	...
	CDrxAction::GetDrxAction()->GetAIProxyUpr()->OnAIProxyDestroyed(this);
}
...
tukk CAIProxy::GetVoiceLibraryName(const bool useForcedDefaultName /* = false */) const
{
...
tukk CAIProxy::GetCommunicationConfigName() const
{
...
tukk CAIProxy::GetNavigationTypeName() const
{
...
IWeapon* CAIProxy::GetCurrentWeapon(EntityId& currentWeaponId) const
{
	...
	IItem* pItem = CDrxAction::GetDrxAction()->GetIItemSystem()->GetItem(m_currentWeaponId);
...
{
	DRX_ASSERT(prefGrenadeType > eRGT_INVALID && prefGrenadeType < eRGT_COUNT);
	...
	if (itemEntity)
	{
		if (IItem* pItem = CDrxAction::GetDrxAction()->GetIItemSystem()->GetItem(itemEntity))
		{
			if (pSecondaryWeaponId)
...
IFireController* CAIProxy::GetFireController(u32 controllerNum)
{
	IVehicleSystem* pVehicleSystem = CDrxAction::GetDrxAction()->GetIVehicleSystem();
...
//
//----------------------------------------------------------------------------------------------------------
i32 CAIProxy::Update(SOBJECTSTATE& state, bool bFullUpdate)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	if (pMC)
	{
		...
		i32 alertness = GetAlertnessState();
		...
		else
		{
			/*
			   ...
			   {
			   i32 num = min((i32) state.predictedCharacterStates.nStates, (i32) SPredictedCharacterStates::maxStates);
			   SPredictedCharacterStates predStates;
			   predStates.nStates = num;
			   pe_status_pos status;
			   GetPhysics()->GetStatus(&status);
			   for (i32 iPred = 0 ; iPred != num ; ++iPred)
			   {
...
void CAIProxy::UpdateShooting(const SOBJECTSTATE& state, bool isAlive)
{
	...
	if (pWeapon)
	{
		...
		pWeapon->Query(eWQ_Raise_Weapon, (ukk )&raise);
	}
	...
	if (fireSec && !m_firingSecondary && hasFireControl)
	{
		...
		{
			u32 index = state.secondaryIndex;
...
void CAIProxy::UpdateCurrentWeapon()
{
	...
	if (pActor)
	{
		...
		if (IVehicle* pVehicle = pActor->GetLinkedVehicle())
		{
			...
			{
				pItem = CDrxAction::GetDrxAction()->GetIItemSystem()->GetItem(weaponInfo.entityId);
	...
	{
		i32k currentWeaponFiremodeIndex = pCurrentWeapon ? pCurrentWeapon->GetCurrentFireMode() : -1;
...
//
//----------------------------------------------------------------------------------------------------------
void CAIProxy::DebugDraw(i32 iParam)
{
	if (!m_pAIHandler)
		return;
	...
	if (iParam == 1)
	{
		...
		tukk szCurrentBehaviour = 0;
		tukk szPreviousBehaviour = 0;
		tukk szFirstBehaviour = 0;
		...
		tukk szCurrentCharacter = m_pAIHandler->m_sCharacterName.c_str();
		tukk szPreviousCharacter = m_pAIHandler->m_sPrevCharacterName.c_str();
		tukk szFirstCharacter = m_pAIHandler->m_sFirstCharacterName.c_str();
...
void CAIProxy::Reset(EObjectResetType type)
{
	...
	if (m_pGameObject)
	{
		...
		CDrxAction::GetDrxAction()->GetRangeSignaling()->OnProxyReset(m_pGameObject->GetEntityId());
		CDrxAction::GetDrxAction()->GetSignalTimer()->OnProxyReset(m_pGameObject->GetEntityId());
	}
...
//
//----------------------------------------------------------------------------------------------------------
CTimeValue CAIProxy::GetEstimatedAGAnimationLength(EAIAGInput input, tukk value)
{
	if (m_pAIHandler)
...
//
//----------------------------------------------------------------------------------------------------------
bool CAIProxy::SetAGInput(EAIAGInput input, tukk value, const bool isUrgent)
{
...
//
//----------------------------------------------------------------------------------------------------------
bool CAIProxy::IsSignalAnimationPlayed(tukk value)
{
...
//
//----------------------------------------------------------------------------------------------------------
bool CAIProxy::IsActionAnimationStarted(tukk value)
{
...
//
//----------------------------------------------------------------------------------------------------------
void CAIProxy::PlayAnimationAction(const IAIAction* pAction, i32 goalPipeId)
{
	DRX_ASSERT(pAction->GetAnimationName() != NULL && pAction->GetAnimationName()[0] != 0);

	if (m_animActionGoalPipeId != 0)
		gEnv->pAISystem->GetAIActionUpr()->AbortAIAction(m_pGameObject->GetEntity(), m_animActionGoalPipeId);
...
void CAIProxy::AnimationActionDone(bool succeeded)
{
	...
	{
		i32 tmp = m_animActionGoalPipeId;
		m_animActionGoalPipeId = 0;
		if (succeeded)
			gEnv->pAISystem->GetAIActionUpr()->FinishAIAction(m_pGameObject->GetEntity(), tmp);
		else
			gEnv->pAISystem->GetAIActionUpr()->AbortAIAction(m_pGameObject->GetEntity(), tmp);
	}
}
...
void CAIProxy::UpdateMind(SOBJECTSTATE& state)
{
	...
	if (m_pGameObject->GetEntity()->GetAI()->IsUpdatedOnce())
	{
		while (!state.vSignals.empty())
		{
			...
			{
				tukk szText = sstruct.strText;
				u32k crc = sstruct.m_nCrcText;
...
{
	m_pGameObject = CDrxAction::GetDrxAction()->GetGameObject(params.id);
...
//
//----------------------------------------------------------------------------------------------------------
void CAIProxy::SendSignal(i32 signalID, tukk szText, IEntity* pSender, const IAISignalExtraData* pData, u32 crc /* = 0u */)
{
	IF_UNLIKELY (crc == 0u)
	{
...
//
//----------------------------------------------------------------------------------------------------------
i32 CAIProxy::GetAlertnessState() const
{
...
//
//----------------------------------------------------------------------------------------------------------
void CAIProxy::SetAlertnessState(i32 alertness)
{
	if (m_pAIHandler)
...
//
//----------------------------------------------------------------------------------------------------------
bool CAIProxy::SetCharacter(tukk character, tukk behavior)
{
	if (!m_pAIHandler)
		return true;
...
//
//----------------------------------------------------------------------------------------------------------
void CAIProxy::GetReadabilityBlockingParams(tukk text, float& time, i32& id)
{
	time = 0;
	id = 0;
}
...
//
//---------------------------------------------------------------------------------------------------------------
bool CAIProxy::PredictProjectileHit(float vel, Vec3& posOut, Vec3& dirOut, float& speedOut, Vec3* pTrajectoryPositions, u32* trajectorySizeInOut, Vec3* pTrajectoryVelocities)
{
	EntityId currentWeaponId = 0;
...
bool CAIProxy::PredictProjectileHit(const Vec3& throwDir, float vel, Vec3& posOut, float& speedOut, ERequestedGrenadeType prefGrenadeType,
                                    Vec3* pTrajectoryPositions, u32* trajectorySizeInOut, Vec3* pTrajectoryVelocities)
{
...
//
//---------------------------------------------------------------------------------------------------------------
tukk CAIProxy::GetCurrentBehaviorName() const
{
	tukk szName = NULL;
...
//
//---------------------------------------------------------------------------------------------------------------
tukk CAIProxy::GetPreviousBehaviorName() const
{
	tukk szName = NULL;
...
float CAIProxy::GetActorHealth() const
{
	...
	const IVehicleSystem* pVehicleSystem = CDrxAction::GetDrxAction()->GetIVehicleSystem();
...
//
//---------------------------------------------------------------------------------------------------------------
i32 CAIProxy::GetActorArmor() const
{
	const IActor* pActor = GetActor();
	if (pActor != NULL)
...
//
//---------------------------------------------------------------------------------------------------------------
i32 CAIProxy::GetActorMaxArmor() const
{
	const IActor* pActor = GetActor();
	if (pActor != NULL)
...
//
//----------------------------------------------------------------------------------------------------------
void CAIProxy::SetBehaviour(tukk szBehavior, const IAISignalExtraData* pData)
{
	if (m_pAIHandler)
	{
...
{
	// (MATT) Surely this is in DrxCommon somewhere {2009/01/27}
	if (x < k)
	{
		x = x / k;
		return A + (B - A) * x;
...
{
	IItem* pItem = CDrxAction::GetDrxAction()->GetIItemSystem()->GetItem(entityId);
