...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Vehicle part class that spawns an entity and attaches it to the vehicle.
...
//------------------------------------------------------------------------
bool CVehiclePartEntity::Init(IVehicle* pVehicle, const CVehicleParams& table, IVehiclePart* parent, CVehicle::SPartInitInfo& initInfo, i32 partType)
{
			...
			if (params.iParam == m_entityNetId && params.entityId != 0)
			{
				...
				if (m_entityId == 0)
				{
					if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(params.entityId))
					{
						...
						DrxLog("VehiclePartEntity: Successfully attached part %d to entity %u", m_index, m_entityId);
					}
				}
				else
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CVehiclePartEntity::OnVehicleEvent trying to attach an entity but part already has one");
				}
			}
...
void CVehiclePartEntity::Serialize(TSerialize serializer, EEntityAspects aspects)
{
	...
	{
		serializer.Value("netId", m_entityNetId, 'u16');
	}
...
//------------------------------------------------------------------------
void CVehiclePartEntity::GetMemoryUsage(IDrxSizer* pSizer) const
{
	pSizer->Add(*this);
