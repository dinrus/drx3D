...
// Error logging
	...
	#define LOG_GLASS_WARNING(...) DrxLogAlways("[BreakGlassSystem Warning]: " __VA_ARGS__)
#else
...
void CBreakableGlassSystem::Update(const float frameTime)
{
	...
	i32k numPlanes = m_glassPlanes.Count();
...
void CBreakableGlassSystem::OnEnabledCVarChange(ICVar* pCVar)
{
	if (pCVar)
	{
		...
		{
			if (CBreakableGlassSystem* pGlassSystem = static_cast<CBreakableGlassSystem*>(pDrxAction->GetIBreakableGlassSystem()))
			{
...
//--------------------------------------------------------------------------------------------------
i32 CBreakableGlassSystem::HandleImpact(const EventPhys* pPhysEvent)
{
	if (CDrxAction* pDrxAction = CDrxAction::GetDrxAction())
	{
		if (CBreakableGlassSystem* pGlassSystem = static_cast<CBreakableGlassSystem*>(pDrxAction->GetIBreakableGlassSystem()))
			...
			if (pGlassSystem->m_enabled)
			{
				if (const EventPhysCollision* pCollEvent = static_cast<const EventPhysCollision*>(pPhysEvent))
				{
					...
					if (pPhysEnt && pCollEvent->iForeignData[physFrag] == PHYS_FOREIGN_ID_BREAKABLE_GLASS_FRAGMENT)
					{
						...
						i32k nonFragType = pCollEvent->iForeignData[1 - physFrag];
					...
					else if (pCollEvent->iForeignData[PHYSEVENT_COLLIDEE] == PHYS_FOREIGN_ID_BREAKABLE_GLASS)
					{
						...
						i32k numPlanes = glassPlanes.Count();
						...
						i32k targetId = pRenderNode ? pRenderNode->GetId() : numPlanes;
...
// Note: We're currently duplicating some work done in DinrusAction, so should be reading that in.
...
bool CBreakableGlassSystem::BreakGlassObject(const EventPhysCollision& physEvent, const bool forceGlassBreak)
{
	...
	if (ISurfaceType* pMat = pSurfaceTypeUpr->GetSurfaceType(initParams.surfaceTypeId))
	{
		if (ISurfaceType::SBreakable2DParams* pBreakableParams = pMat->GetBreakable2DParams())
		{
			initParams.pShatterEffect = pBreakableParams->full_fracture_fx ? gEnv->pParticleUpr->FindEffect(pBreakableParams->full_fracture_fx) : NULL;
		}
	}
	    ...
	    ValidateExtractedOutline(physData, initParams))
	{
		...
		dupeEvent.pForeignData[PHYSEVENT_COLLIDEE] = (uk )pRenderNode;
...
//--------------------------------------------------------------------------------------------------
void CBreakableGlassSystem::GetMemoryUsage(IDrxSizer* pSizer) const
{
	// System usage
...
// Note 2: We're currently duplicating some work done in DinrusAction, so should be reading that in
//--------------------------------------------------------------------------------------------------
bool CBreakableGlassSystem::ExtractPhysDataFromEvent(const EventPhysCollision& physEvent, SBreakableGlassPhysData& data, SBreakableGlassInitParams& initParams)
{
	if (IPhysicalEntity* pPhysEntity = physEvent.pEntity[PHYSEVENT_COLLIDEE])
	{
		...
		i32k entType = pPhysEntity->GetiForeignData();
		i32k entPart = physEvent.partid[PHYSEVENT_COLLIDEE];
				...
				if (IRenderNode* pBrush = (IRenderNode*)physEvent.pForeignData[PHYSEVENT_COLLIDEE])
				{
					...
					if (pStatObj && pStatObj->GetFlags() & STATIC_OBJECT_COMPOUND)
					{
						if (IStatObj::SSubObject* pSubObj = pStatObj->GetSubObject(entPart))
						{
							...
							i32k subMtlCount = pRenderMat->GetSubMtlCount();
...
//--------------------------------------------------------------------------------------------------
bool CBreakableGlassSystem::ValidatePhysMesh(mesh_data* pPhysMesh, i32k thinAxis)
{
...
//--------------------------------------------------------------------------------------------------
bool CBreakableGlassSystem::ExtractPhysMesh(mesh_data* pPhysMesh, i32k thinAxis, const primitives::box& bbox, TGlassDefFragmentArray& fragOutline)
{
	// Calculate thin axis
	...
	i32 xAxis = thinAxis == 0 ? 2 : (thinAxis == 1 ? 0 : 0);
	i32 yAxis = thinAxis == 0 ? 1 : (thinAxis == 1 ? 2 : 1);
	...
	else
	{
		...
		for (i32 i = 0; i < fragTriCount + 2; ++i)
		{
			fragOutline.push_back();
		}

		// Add initial triangle
		i32 triIndex = fragTris[0] * 3;
		...
		for (i32 j = 1; j < fragTriCount; ++j)
		{
			...
			uint insertInd = 0;
			i32 indMin = -1, indMax = -1;

			for (i32 k = 0; k < ptCount; ++k)
			{
				for (i32 l = 0; l < 3; ++l)
				{
	...
	i32k fragOutlineSize = fragOutline.size();

	for (i32 i = 0; i < fragOutlineSize; ++i)
	{
...
//--------------------------------------------------------------------------------------------------
void CBreakableGlassSystem::MergeFragmentTriangles(mesh_data* pPhysMesh, i32k tri, const Vec3& thinRow, TGlassFragmentIndexArray& touchedTris, TGlassFragmentIndexArray& fragTris)
{
	// Check for limits
	if (touchedTris.size() < touchedTris.max_size())
	{
		...
		if (index == -1)
		{
			...
			if (thinRow.Dot(pPhysMesh->pNormals[tri]) >= flatThresh)
			{
				...
				for (i32 i = 0; i < 3; ++i)
				{
					i32k buddy = pPhysMesh->pTopology[tri].ibuddy[i];
					if (buddy >= 0)
					{
...
//--------------------------------------------------------------------------------------------------
void CBreakableGlassSystem::ExtractUVCoords(IStatObj* const pStatObj, const primitives::box& bbox, i32k thinAxis, SBreakableGlassPhysData& data)
{
	...
	if (pMesh)
	{
		DRX_ASSERT(pMesh->m_pPositionsF16 == 0);
		...
		if (pTangents && pUV)
		{
			...
			DRX_ASSERT(numVerts == pMesh->GetTexCoordCount());
...
{
#ifndef RELEASE
	DRX_ASSERT_MESSAGE(m_enabled || m_glassPlanes.Count() == 0, "Breakable glass system is disabled, should not have any active planes.");
#endif
}//-------------------------------------------------------------------------------------------------
