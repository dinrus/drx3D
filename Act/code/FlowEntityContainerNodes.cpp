...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		{
			OutputPortConfig<i32>("ContainerSize", _HELP("Container size after the operation completed")),
			{ 0 }
		};
		...
		case eFE_Activate:
			{
				...
				if (CDrxAction* pDrxAction = CDrxAction::GetDrxAction())
				{
					...
					{
						size_t groupSize = containerUpr.AddEntity(containerId, GetPortEntityId(pActInfo, eI_InputEntityId));
						ActivateOutput(pActInfo, eO_ContainerSize, (i32)groupSize);
					}
					if (IsPortActive(pActInfo, eI_RemoveEntity))
					{
						size_t groupSize = containerUpr.RemoveEntity(containerId, GetPortEntityId(pActInfo, eI_InputEntityId));
						ActivateOutput(pActInfo, eO_ContainerSize, (i32)groupSize);
					}
					if (IsPortActive(pActInfo, eI_AddContainerOfEntities))
					{
						size_t groupSize = containerUpr.AddContainerOfEntitiesToContainer(containerId, GetPortEntityId(pActInfo, eI_SrcContainerId));
						ActivateOutput(pActInfo, eO_ContainerSize, (i32)groupSize);
					}
					if (IsPortActive(pActInfo, eI_RemoveContainerOfEntities))
					{
						size_t groupSize = containerUpr.RemoveContainerOfEntitiesFromContainer(containerId, GetPortEntityId(pActInfo, eI_SrcContainerId));
						ActivateOutput(pActInfo, eO_ContainerSize, (i32)groupSize);
					}
				}
			}
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig outputs[] =
		{
			OutputPortConfig<i32>("GroupSize", _HELP("Size of the destination container after the merge is done")),
			{ 0 }
		};
		...
		case eFE_Activate:
			{
				...
				if (IsPortActive(pActInfo, eI_Copy) && pActInfo->pEntity)
				{
					...
					size_t dstSize = containerUpr.MergeContainers(GetPortEntityId(pActInfo, eI_SrcContainerId), dstContainerId, false);
					ActivateOutput(pActInfo, eO_ContainerSize, (i32)dstSize);
				}
				else if (IsPortActive(pActInfo, eI_Merge) && pActInfo->pEntity)
				{
					EntityId dstContainerId = pActInfo->pEntity->GetId();
					size_t dstSize = containerUpr.MergeContainers(GetPortEntityId(pActInfo, eI_SrcContainerId), dstContainerId, true);
					ActivateOutput(pActInfo, eO_ContainerSize, (i32)dstSize);
				}
			}
			break;
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eI_ClearContainer) && pActInfo->pEntity)
				{
					EntityId containerId = pActInfo->pEntity->GetId();
					CEntityContainerMgr& containerUpr = CDrxAction::GetDrxAction()->GetEntityContainerMgr();
					containerUpr.ClearContainer(containerId);
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] =
		{
			...
			InputPortConfig<i32>(   "NumEntitiesFrame", 1,    _HELP("Number of entities to unhide per frame. Set to -1 to unhide all at once, otherwise tweak according to the cost of the entities in the container for performance resaons."), _HELP("Num Unhide Entities Per Frame")),
		...
		case eFE_Activate:
			{
				...
				{
					containerUpr.SetHideEntities(containerId, true);
				...
				if (IsPortActive(pActInfo, eI_UnhideEntities))
				{
					...
					containerUpr.GetEntitiesInContainer(containerId, m_entitiesToUnhide);
					if (m_numEntitiesToUnhidePerFrame <= 0)
					{
						// un-hide all entities immediately
						containerUpr.SetHideEntities(containerId, false);
				...
				{
					containerUpr.RemoveEntitiesFromTheGame(containerId);
				...
				if (IsPortActive(pActInfo, eI_RunModule))
				{
					...
					bool bSuccess = containerUpr.RunModule(containerId, moduleName.c_str());
		...
		case eFE_Update:
			{
				...
				{
					// hide entities
					i32 lowIndex = m_numEntitiesToUnhidePerFrame >= m_entitiesToUnhide.size() ? 0 : m_entitiesToUnhide.size() - m_numEntitiesToUnhidePerFrame;
					for (i32 i = m_entitiesToUnhide.size() - 1; i >= lowIndex; --i)
					{
...
private:
	...
	i32 m_numEntitiesToUnhidePerFrame;
};
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig outputs[] =
		{
			OutputPortConfig<i32>("ContainerSize", _HELP("Container size")),
			{ 0 }
		};
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eI_DoQuery) && pActInfo->pEntity)
				{
					EntityId containerId = pActInfo->pEntity->GetId();
					CEntityContainerMgr& containerUpr = CDrxAction::GetDrxAction()->GetEntityContainerMgr();
					size_t size = containerUpr.GetContainerSize(containerId);
					ActivateOutput(pActInfo, eO_ContainerSize, (i32)size);
				}
			}
			break;
		}
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig outputs[] =
		{
			OutputPortConfig<u32>("RandomEntityId", _HELP("A random element from the container")),
			{ 0 }
		};
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eI_DoQuery) && pActInfo->pEntity)
				{
					EntityId containerId = pActInfo->pEntity->GetId();
					CEntityContainerMgr& containerUpr = CDrxAction::GetDrxAction()->GetEntityContainerMgr();
					EntityId entityId = containerUpr.GetRandomEntity(containerId);
					ActivateOutput(pActInfo, eO_RandomEntityId, (i32)entityId);
				}
			}
			break;
		}
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig outputs[] =
		{
			OutputPortConfig<i32>("ContainerCount", _HELP("Number of containers that the entity belongs to")),
			OutputPortConfig<i32>("ContainerId1",   _HELP("First Container Id that the entity belongs to")),
			OutputPortConfig<i32>("ContainerId2",   _HELP("Second Container Id that the entity belongs to")),
			OutputPortConfig<i32>("ContainerId3",   _HELP("Third Container Id that the entity belongs to")),
			OutputPortConfig<i32>("ContainerId4",   _HELP("Fourth Container Id that the entity belongs to")),
			{ 0 }
		};
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eI_DoQuery) && pActInfo->pEntity)
				{
					...
					containerUpr.GetEntitysContainerIds(entityId, groupIdArray);
					...
					ActivateOutput(pActInfo, eO_ContainerCount, (i32)numElements);
				}
			}
			break;
		}
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eI_DoQuery) && pActInfo->pEntity)
				{
					...
					containerUpr.GetEntitysContainerIds(entityId, containerIdArray);
...
private:
	...
	{
		if (CDrxAction::GetDrxAction())
		{
			CEntityContainerMgr& containerUpr = CDrxAction::GetDrxAction()->GetEntityContainerMgr();
			containerUpr.UnregisterListener(this);
		}
	}
	...
	void RegisterListener(SActivationInfo* pActInfo)
	{
		if (pActInfo && pActInfo->pEntity)
		{
			...
			containerUpr.RegisterListener(this, listenerName.c_str(), false);
		}
	}
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] =
		{
			...
			InputPortConfig<i32>("Index", 0, _HELP("Index of the entity to query inside a group [index: from 1 to num entities in the group]")),
			{ 0 }
		};
		static const SOutputPortConfig outputs[] =
		{
		...
		case eFE_Activate:
			{
				if (IsPortActive(pActInfo, eI_DoQuery) && pActInfo->pEntity)
				{
					u32k index = std::max(0, GetPortInt(pActInfo, eI_Index));

					CEntityContainerMgr& containerUpr = CDrxAction::GetDrxAction()->GetEntityContainerMgr();
					const EntityId containerId = pActInfo->pEntity->GetId();
					CEntityContainer::TEntitiesInContainer containerIdArray;

					containerUpr.GetEntitiesInContainer(containerId, containerIdArray);
...
public:
	...
	virtual void OnGroupChange(IEntityContainerListener::SChangeInfo& info)
	{
		if (m_actInfo.pEntity)
		{
			...
			{
				CEntityContainerMgr& containerUpr = CDrxAction::GetDrxAction()->GetEntityContainerMgr();
				size_t size = containerUpr.GetContainerSize(info.containerId);
				ActivateOutput(&m_actInfo, eO_ContainerSize, (i32)size);
				switch (info.eventType)
				{
				case IEntityContainerListener::eET_EntityAdded:
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig inputs[] =
		{
			...
			InputPortConfig<i32>("Filters", _HELP("Filters Set"), 0, _UICONFIG("enum_int:None=0,Closest=1,Farthest=2,Random=3")),
			...
			InputPortConfig<i32>("OutputSize", -1, _HELP("How many elements will be outputted into OutputContainerId (-1: ALL)")),
		...
		case eFE_Activate:
			{
				...
				if (IsPortActive(pActInfo, eI_Apply))
				{
					...
					containerUpr.GetEntitiesInContainer(containerId, ids);

					// Read node common params
					i32 filterType = GetPortInt(pActInfo, eI_Filters);
					i32k kSelectedOutputSize = GetPortInt(pActInfo, eI_OutputSize);
					const EntityId kOutputContainerId = GetPortEntityId(pActInfo, eI_OutputContainerId);

					const bool bOutputToContainer = containerUpr.IsContainer(kOutputContainerId);
					u32k kNumFgOutputs = eO_LAST_MEMBER_OUTPUT - eO_FIRST_MEMBER_OUTPUT + 1; // number of fixed result outputs in FG node
					u32k kElementsToOutputByNode = std::min(static_cast<u32>(ids.size()), kNumFgOutputs);

					u32 numElementsToOutputToContainer = UpdateNumElementsToOutputToGroup(kSelectedOutputSize, ids);
					...
					if (filterType == eFT_Closest || filterType == eFT_Furthest)
					{
						...
						u32k numElementsForPartialSort = std::max(numElementsToOutputToContainer, kElementsToOutputByNode);
						...
						if (!ids.empty())
						{
							...
							if (filterType == eFT_Furthest)
							{
								...
								std::partial_sort(ids.begin(), ids.begin() + std::min(numElementsForPartialSort, static_cast<u32>(ids.size())), ids.end(), comp);
							}
							else
							{
					...
					if (bOutputToContainer)
					{
						...
						if (clearGroup)
						{
							containerUpr.ClearContainer(kOutputContainerId);
						}

						const bool splitGroup = GetPortBool(pActInfo, eI_SplitContainer);
						for (size_t i = 0; i < numElementsToOutputToContainer; ++i)
						{
							containerUpr.AddEntity(kOutputContainerId, ids[i].id);
							if (splitGroup)
							{
								containerUpr.RemoveEntity(containerId, ids[i].id);
							}
						}
					}
					...
					u32k kOutputSize = std::min(kElementsToOutputByNode, static_cast<u32>(ids.size()));
					if (!ids.empty())
					{
						// Active outputs
					...
					for (u32 i = kOutputSize; i < kNumFgOutputs; ++i)
	...
	inline u32 UpdateNumElementsToOutputToGroup(i32 outputSize, CEntityContainer::TEntitiesInContainer ids)
	{
		...
		return (outputSize < 0) ? ids.size() : std::min(static_cast<u32>(ids.size()), static_cast<u32>(outputSize));
	}
