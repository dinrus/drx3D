...
#include <drx3D/Act/VehicleViewThirdPerson_Bone.h>

tukk CVehicleViewThirdPersonBone::m_name = "ThirdPersonBone";
...
bool CVehicleViewThirdPersonBone::Init(IVehicleSeat* pSeat, const CVehicleParams& table)
{
	...
	if (ICharacterInstance* pCharacterInstance = m_pVehicle->GetEntity()->GetCharacter(0))
	{
		IDefaultSkeleton& rIDefaultSkeleton = pCharacterInstance->GetIDefaultSkeleton();
		{
			...
			{
				tukk boneName = table.getAttr("DirectionBone");
...
void CVehicleViewThirdPersonBone::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
	//ToDo: Zoom
}
