...
bool CEntityContainerMgr::CreateContainer(EntityId containerId, tukk szContainerName /*= nullptr */)
{
...
EntityId CEntityContainerMgr::GetRandomEntity(EntityId containerId) const
{
	...
	{
		const CEntityContainer::TEntitiesInContainer& entities = pContainer->GetEntities();
		u32k idx = drx_random((size_t)0, entities.size() - 1); // returns size_t between 0 and size-1
...
bool CEntityContainerMgr::RunModule(EntityId containerId, tukk moduleName)
{
	IFlowGraphModule* pModule = gEnv->pFlowSystem->GetIModuleUpr()->GetModule(moduleName);
	if (!pModule)
	{
		return false;
	}
...
void CEntityContainerMgr::DebugRender(EntityId containerId, const Vec3& srcPos)
{
	...
	if (pContainer && pAux)
	{
		...
		for (auto const& entity : entitiesInContainer)
		{
			...
			tukk szName = pEntity ? pEntity->GetName() : nullptr;
