...
#include <drx3D/Act/BreakReplicator_Simple.h>
 ...
 */
	...
	#define DEBUG_ASSERT(x) { if (!(x)) { i32* p = 0; *p = 42; } } // crash on purpose!
#else
...
struct EmittedProduct
{
	...
	i32              partidSrc; // The part id. Note that a gib can contain more than one part. At the moment we just store the part that has the smallest index
	Vec3             initialVel;
	Vec3             initialAngVel;
};
...
static tukk BreakStreamGetModeName(u32 mode)
{
	enum EMode {k_invalid = 0, k_recording, k_playing, k_finished, k_numModes};
	#if DEBUG_NET_BREAKAGE
	static tukk names[BreakStream::k_numModes + 1] = {
		"invalid",
		"k_recording",
		"k_playing",
		"k_finished",
	...
	return names[min((u32)mode, (u32)BreakStream::k_numModes)];
	#endif
	return "";
}

static tukk BreakStreamGetTypeName(u32 type)
{
	#if DEBUG_NET_BREAKAGE
	static tukk names[BreakStream::k_numTypes + 1] = {
		"unknown",
		"PartBreak",
		"PlaneBreak",
		"DeformBreak",
	...
	return names[min((u32)type, (u32)BreakStream::k_numTypes)];
	#endif
	return "";
}
...
public:
	...
	u8           m_numProducts;
	u8           m_numJointsBroken;
	u8           m_numProductsCollected;
};
...
void CObjectIdentifier::SerializeWith(CBitArray& array, bool includeObjCenter)
{
	...
	i32 bStatic = iszero((i32)m_objType - (i32)k_static_entity);
...
IPhysicalEntity* CObjectIdentifier::FindPhysicalEntity()
{
	...
	if (m_objType == k_static_entity)
	{
		if (m_objHash)
		{
			...
			i32 idx = CBreakReplicator::Get()->GetIdentifier(this, NULL, true);
			if (idx >= 0)
			{
...
{
	i32k n = m_numFramesLeft ? m_numFramesLeft - 1 : 0;
...
void BreakStream::LogHostMigrationInfo()
{
	...
	LOGBREAK("HOSTMIG for stream: %d", m_breakIdx);
	{
		...
		OnSend(desc);
		INetContext* pNetContext = CDrxAction::GetDrxAction()->GetNetContext();
		if (pNetContext)
		{
...
void PartBreak::SerializeWith(CBitArray& array)
{
	...
	for (i32 i = 0; i < m_numJointsBroken; i++)
	{
...
bool PlaneBreak::IsRenderMeshReady()
{
	if (m_pStatObj == NULL)
	{
		i32 iForeign = m_pent->GetiForeignData();
...
void DeformBreak::OnSend(SNetBreakDescription& desc)
{
	...
	if (m_mode == k_playing)
	{
		...
		for (i32 i = 0; i < m_spawnedEntities.size(); i++)
		{
...
i32 CBreakReplicator::m_accurateWorldPosNumBits = 0;
i32 CBreakReplicator::m_inaccurateWorldPosNumBits = 0;
	...
	, m_timeSinceLevelLoaded(-1.0f)
{
	...
	m_inaccurateWorldPosNumBits = IntegerLog2_RoundUp(u32(max / WORLD_POS_SAMPLE_DISTANCE_APPART_INACCURATE));
	m_accurateWorldPosNumBits = IntegerLog2_RoundUp(u32(max / WORLD_POS_SAMPLE_DISTANCE_APPART_ACCURATE));

	DrxLog("[CBreakReplicator] Inaccurate world pos using range 0-%f with %d bits", max, m_inaccurateWorldPosNumBits);
	DrxLog("[CBreakReplicator] Accurate world pos using range 0-%f with %d bits", max, m_accurateWorldPosNumBits);
}
...
{
	for (i32 i = 0; i < m_numPendingStreams; i++)
	{
...
i32 CBreakReplicator::PushBackNewStream(const EventPhysMono* pMono, BreakStream* pStream)
{
	// Insert into the active list
	i32k breakIdx = m_streams.size();
	...
	i32 idx = GetIdentifier(&pStream->m_identifier, pMono->pEntity, true);
	DEBUG_ASSERT(idx >= 0);
	...
	i32 subIndex = 0;
	i32 numStreams = m_streams.size();
	for (i32 i = numStreams - 1; i >= 0; --i)
	{
...
i32 CBreakReplicator::GetBreakIndex(const EventPhysMono* pMono, bool add)
{
	...
	i32k lastBreak = m_streams.size();
	i32 i = m_activeStartIdx;
	while (i < lastBreak)
	{
...
i32 CBreakReplicator::GetIdentifier(CObjectIdentifier* identifier, const IPhysicalEntity* pent, bool add)
{
	// Have we already identified this object before?
	...
	{
		for (i32 i = 0; i < m_identifiers.size(); i++)  // ToDo: make me faster!
		{
...
bool CBreakReplicator::OnCreatePhysEntityPart(const EventPhysCreateEntityPart* pEvent)
{
	...
	i32 breakIdx = GetBreakIndex(pEvent, m_bDefineProtocolMode_server);
	if (breakIdx >= 0)
	{
...
bool CBreakReplicator::OnRemovePhysEntityParts(const EventPhysRemoveEntityParts* pEvent)
{
	...
	CDrxAction::GetDrxAction()->OnPartRemoveEvent(oldSize);
...
i32 CBreakReplicator::OnRevealEntityPart(const EventPhys* pEvent)
{
...
bool CBreakReplicator::OnJointBroken(const EventPhysJointBroken* pEvent)
{
	...
	i32 breakIdx = GetBreakIndex(&mono, m_bDefineProtocolMode_server);
	if (breakIdx >= 0)
	{
		BreakStream* pStream = m_streams[breakIdx];
...
void CBreakReplicator::OnEndFrame()
{
	...
	for (i = 0; i < (i32k)m_entitiesToRemove.size(); i++)
	{
...
void CBreakReplicator::OnSpawn(IEntity* pEntity, IPhysicalEntity* pPhysEntity, IPhysicalEntity* pSrcPhysEntity)
{
	...
	if (pPhysEntity == pSrcPhysEntity)
	{
		...
		{
			u32 objHash = CObjectSelector::CalculateHash(pRenderNode, true);
			if (objHash)
			{
				for (i32 i = 0; i < m_identifiers.size(); i++)  // ToDo: make me faster!
				{
	...
	i32 breakIdx = GetBreakIndex(&mono, false);
	if (breakIdx >= 0)
	{
		BreakStream* pStream = m_streams[breakIdx];
...
void CBreakReplicator::OnRemove(IEntity* pEntity)
{
	...
	for (i32 i = 0; i < m_identifiers.size(); i++)
	{
...
void CBreakReplicator::RemoveEntity(IEntity* pEntity)
{
	...
	DRX_ASSERT(pEntity->GetFlags() & (ENTITY_FLAG_CLIENT_ONLY | ENTITY_FLAG_SERVER_ONLY));
	gEnv->pEntitySystem->RemoveEntity(pEntity->GetId());
	#endif
}

void CBreakReplicator::SerialisePosition(CBitArray& array, Vec3& pos, float range, i32 numBits)
{
	if (array.IsReading())
	{
...
void CBreakReplicator::SendPayloads()
{
	...
	while (i < lastBreak)
	{
		...
		if (pStream->m_mode == BreakStream::k_recording && pStream->CanSendPayload())
		{
			...
			if (pDrxAction != NULL)
			{
				INetContext* pNetContext = pDrxAction->GetNetContext();
				if (pNetContext != NULL)
				{
					pNetContext->LogBreak(desc);
				}
...
void CBreakReplicator::PushPlayback(BreakStream* pStream)
{
	...
	if (m_numPendingStreams < NET_MAX_PENDING_BREAKAGE_STREAMS)
	{
		...
		i32 i = m_numPendingStreams;
		i32k breakIdx = pStream->m_breakIdx;
...
i32 CBreakReplicator::PlaybackStream(BreakStream* pStream)
{
	...
	if (bStartStream)
	{
		...
		i32k initialSize = m_streams.size();
		if (m_streams.size() <= pStream->m_breakIdx)
		{
			m_streams.resize(pStream->m_breakIdx + 32);
			i32k newSize = m_streams.size();
			for (i32 j = initialSize; j < newSize; j++)
			{
		...
		m_activeStartIdx = min(m_activeStartIdx, (i32)pStream->m_breakIdx);
		pStream->Playback();
	}
...
void CBreakReplicator::PlaybackBreakage()
{
	...
	i32 n = 0;
	i32k numPending = m_numPendingStreams;
	i32 numPlayed = 0;
	...
	for (; p < numPending && numPlayed < BS_MAX_NUM_TO_PLAYBACK; p++)
	{
		...
		if (pStream->m_invalidFindCount < NET_MAX_NUM_FRAMES_TO_FIND_ENTITY)
		{
			...
			else
			{
				...
				for (i32 i = m_streams.size() - 1; i >= 0; --i)
				{
					if (m_streams[i]->IsIdentifierEqual(pStream->m_identifier))
					{
			...
			i32 bPlayed = (pent) ? PlaybackStream(pStream) : 0;
			numPlayed += bPlayed;
			if (bPlayed == 0)
			{
				// Couldn't start, we need to keep the stream pending
...
uk CBreakReplicator::SerialiseBreakage(TSerialize ser, BreakStream* pStream)
{
	CBitArray array(&ser);
...
void CBreakReplicator::OnBrokeSomething(const SBreakEvent& be, EventPhysMono* pMono, bool isPlane)
{
	...
	else
	{
		...
		CBreakReplicator::SendSvRecvGlassBreakWith(glassBreak, CDrxAction::GetDrxAction()->GetClientChannel());
	}
}

// Static callbacks from the physics events
i32 CBreakReplicator::OnJointBroken(const EventPhys* pEvent)
{
	m_pThis->OnJointBroken(static_cast<const EventPhysJointBroken*>(pEvent));
	return 1;
}

i32 CBreakReplicator::OnCreatePhysEntityPart(const EventPhys* pEvent)
{
	m_pThis->OnCreatePhysEntityPart(static_cast<const EventPhysCreateEntityPart*>(pEvent));
	return 1;
}

i32 CBreakReplicator::OnRemovePhysEntityParts(const EventPhys* pEvent)
{
