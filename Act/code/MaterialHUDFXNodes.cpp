...
// -------------------------------------------------------------------------
//  Имя файла:   MaterialHUDFXNodes.cpp
//  Version:     v1.00
...
public:
	...
	//When activated, just notify the MaterialFGUpr that the effect ended
	//See MaterialFGUpr.cpp
	void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		switch (event)
		{
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Integer",     0,                            _HELP("INTEGER parameter")),
	...
	static IFlashPlayer* GetFlashPlayerFromMaterialIncludingSubMaterials(IMaterial* pMaterial)
	{
		...
		for (i32 i = 0; i != subMtlCount; ++i)
		{
			...
			if (pFlashPlayer)
			{
				if (pRetFlashPlayer)
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "GetFlashPlayerFromMaterialIncludingSubMaterials: Multiple flash players detected, returning last sub material one");
				}
...
public:
	...
	void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_ports[] = {
			...
			InputPortConfig<i32>("FrameNumber",   -1,                                   _HELP("Number of flash frame to goto (If not using FrameName)")),
			{ 0 }
		};
								...
								if (!objectPath.empty())
								{
									...
									if (pFlashObject)
									{
										...
										if (!frameName.empty())
										{
											...
											if (!bResult)
											{
												DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CMaterialFlashGotoAndPlayNode::ProcessEvent: Failed on object: %s with framename: %s", objectPath.c_str(), frameName.c_str());
											}
										}
										else // Use number
										{
											i32k iFrameNumber = GetPortInt(pActInfo, EIP_FrameNumber);
											bool bResult = pFlashObject->GotoAndPlay(iFrameNumber);
											if (!bResult)
											{
												DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CMaterialFlashGotoAndPlayNode::ProcessEvent: Failed on object: %s with framenumber: %d", objectPath.c_str(), iFrameNumber);
											}
										}
									...
									{
										DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CMaterialFlashGotoAndPlayNode::ProcessEvent: Failed finding object: %s", objectPath.c_str());
									}
								}
								else
								{
									DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CMaterialFlashGotoAndPlayNode::ProcessEvent: ObjectPath can't be empty");
								}
							}
						}
					}
...
private:
	...
	// IFSCommandHandler
	void HandleFSCommand(tukk pCommand, tukk pArgs, uk pUserData)
	{
		if (IsActive())
		{
