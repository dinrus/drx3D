...
#include <drx3D/Act/ProceduralClipsPositioning.h>
...
struct SPositionAdjuster
{
	...
	void Update(IEntity& entity, float timePassed)
	{
		...
		//DrxLog("Moving %s from (%f, %f, %f) to (%f, %f, %f) delta (%f, %f, %f) timeDelta: %f time: %f timeTgt: %f",
		//			entity.GetName(),
...
public:
	PROCEDURAL_CONTEXT(CProceduralContext_AdjustPos, "AdjustPosContext", "c6c08712-1421-4854-adc5-6ab6422834bd"_drx_guid);
	...
	virtual void Update(float timePassed) override
	{
		if (m_enabled)
		{
			...
			u32k numAdjusts = m_posAdjusts.size();
			for (u32 i = 0; i < numAdjusts; i++)
			{
	...
	void StartCollisionCheck(const SCollisionAdjustParams& params, ActionScopes installedScopeMask)
	{
		...
		{
			u32 numScopes = m_actionController->GetTotalScopes();

			for (i32 i = 0; i < numScopes; i++)
			{
...
private:
		...
		if (!m_slaveScopeIds.empty())
		{
			...
			if (!slaveProceduralContexts.empty())
			{
				...
				if (!move.IsZero())
				{
					...
					i32 numSlaveContexts = slaveProceduralContexts.size();

					for (i32 i = 0; i < numSlaveContexts; i++)
					{
	...
	SPosAdjust& GetOrAddPosAdjustForScope(i32 scopeID)
	{
	...
	SPosAdjust* GetPosAdjustForScope(i32 scopeID)
	{
		u32k numAdjusts = m_posAdjusts.size();
		for (u32 i = 0; i < numAdjusts; i++)
		{
	...
	std::vector<i32>        m_slaveScopeIds;
	bool                    m_enabled;
...
DRXREGISTER_CLASS(CProceduralContext_AdjustPos);
...
public:
	...
	virtual void OnEnter(float blendTime, float duration, const SPositionAdjustParams& params)
	{
		...
		if (isRootEntity && hasParam)
		{
			...
			//DrxLog("Init (%f, %f, %f) Target  (%f, %f, %f) Delta: (%f, %f, %f)", entityLoc.t.x, entityLoc.t.y, entityLoc.t.z, m_posAdjuster.m_targetLoc.t.x, m_posAdjuster.m_targetLoc.t.y, m_posAdjuster.m_targetLoc.t.z, m_posAdjuster.m_delta.x, m_posAdjuster.m_delta.y, m_posAdjuster.m_delta.z);
		}
		else
		{
			...
			if (!hasParam)
			{
				DrxLog("Failed to init PositionAdjust due to missing parameter: TargetPos");
			}
		}
	}
...
public:
	...
	virtual void OnEnter(float blendTime, float duration, const SPositionAdjustWithCollisionCheck& params)
	{
		...
		if (isRootEntity)
		{
			...
			tukk pParamName = params.paramName.empty() ? "TargetPos" : params.paramName.c_str();

			u32 paramNameCRC = CCrc32::ComputeLowercase(pParamName);
			...
			{
				DrxLog("Failed to init PositionAdjust due to missing parameter: %s", pParamName);
			}
		}
	}
...
public:
	...
	void UpdateDeltas(QuatT& animStartLoc)
	{
		...
		//DrxLog("Init %s (%f, %f, %f) Target  (%f, %f, %f) Delta: (%f, %f, %f)", m_scope->GetEntity().GetName(), entityLoc.t.x, entityLoc.t.y, entityLoc.t.z, m_posAdjuster.m_targetLoc.t.x, m_posAdjuster.m_targetLoc.t.y, m_posAdjuster.m_targetLoc.t.z, m_posAdjuster.m_delta.x, m_posAdjuster.m_delta.y, m_posAdjuster.m_delta.z);
...
public:
	...
	void UpdateDeltas()
	{
		...
		if (pAnim && (pAnim->GetTransitionWeight() > 0.0f))
		{
			...
			// TODO Jean: hook this up when DinrusXAnimation integration is done
...
CProceduralClipPosAdjustAlignEnd::CProceduralClipPosAdjustAlignEnd() : m_targetAccuracy(0.0f)
{
}
...
public:
	...
	virtual void OnEnter(float blendTime, float duration, const SProceduralClipPosAdjustTargetLocatorParams& params)
	{
		...
		i32 scopeID = contDef.m_scopeIDs.Find(params.targetScopeName.crc);
		...
		if (scope && scope->GetCharInst())
		{
			...
			i32 jointID = rIDefaultSkeleton.GetJointIDByName(params.targetJointName.c_str());
			if (jointID >= 0)
			{
		...
		else
		{
			...
			if (!hasParam)
			{
				DrxLog("Failed to init PositionAdjust due to missing target object or joint: %s", params.targetJointName.c_str());
			}
		}
	}
