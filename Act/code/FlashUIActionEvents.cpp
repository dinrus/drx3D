...
// -------------------------------------------------------------------------
//  Имя файла:   FlashUIActionEvents.cpp
//  Version:     v1.00
	...
	, m_pUIFunctions(NULL)
{
	...
	DRX_ASSERT_MESSAGE(m_pGameFramework, "IGameFramework is not initialized, crash can follow!");

	m_pLevelSystem = m_pGameFramework ? m_pGameFramework->GetILevelSystem() : NULL;
	DRX_ASSERT_MESSAGE(m_pLevelSystem, "ILevelSystem is not initialized, crash can follow!");

	DRX_ASSERT_MESSAGE(gEnv->pFlashUI, "No FlashUI extension available!");
	if (gEnv->pFlashUI)
	{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
void CFlashUIActionEvents::OnLoadingProgress(ILevelInfo* pLevel, i32 progressAmount)
{
	if (gEnv->IsEditor()) return;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
void CFlashUIActionEvents::OnLoadingError(ILevelInfo* pLevel, tukk error)
{
	if (gEnv->IsEditor()) return;
...
// @param1: Server
//
void CFlashUIActionEvents::OnConnect(tukk server)
{
	if (gEnv->IsEditor()) return;
...
// @param1: Error
//
void CFlashUIActionEvents::OnDisconnect(tukk error)
{
	if (gEnv->IsEditor()) return;
...
void CFlashUIActionEvents::OnUnloadAllElements(const SUIArguments& elements)
{
	...
	for (i32 i = 0; i < gEnv->pFlashUI->GetUIElementCount(); ++i)
	{
		...
		for (i32 k = 0; k < elements.GetArgCount(); ++k)
		{
			string element;
