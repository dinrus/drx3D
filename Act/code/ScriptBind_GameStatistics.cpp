...
void CScriptBind_GameStatistics::RegisterGlobals()
{
	...
	for (size_t i = 0; i != numEventTypes; ++i)
		gEnv->pScriptSystem->SetGlobalValue(m_pGS->GetEventDesc(i)->scriptName, static_cast<i32>(i));

	size_t numStateTypes = m_pGS->GetStateCount();
	for (size_t i = 0; i != numStateTypes; ++i)
		gEnv->pScriptSystem->SetGlobalValue(m_pGS->GetStateDesc(i)->scriptName, static_cast<i32>(i));

	size_t numScopeTypes = m_pGS->GetScopeCount();
	for (size_t i = 0; i != numScopeTypes; ++i)
		gEnv->pScriptSystem->SetGlobalValue(m_pGS->GetScopeDesc(i)->scriptName, static_cast<i32>(i));
	...
	for (size_t i = 0; i != numElemTypes; ++i)
	{
		...
		gEnv->pScriptSystem->SetGlobalValue(desc->scriptName, static_cast<i32>(i));
		gEnv->pScriptSystem->SetGlobalValue(desc->locatorName, static_cast<i32>(desc->locatorID));
	}
}

IGameStatistics* CScriptBind_GameStatistics::GetStats(IFunctionHandler* pH)
{
	if (uk pThis = pH->GetThis())
		return static_cast<IGameStatistics*>(pThis);
	return 0;
}

bool CScriptBind_GameStatistics::BindTracker(IScriptTable* pT, tukk name, IStatsTracker* tracker)
{
...
bool CScriptBind_GameStatistics::UnbindTracker(IScriptTable* pT, tukk name, IStatsTracker* tracker)
{
#if STATISTICS_CHECK_BINDING
	...
	if (pT)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[Telemetry] Error unbinding tracker");
			return false;
		}
	}
	return false;
...
i32 CScriptBind_GameStatistics::PushGameScope(IFunctionHandler* pH, i32 scopeID)
{
...
i32 CScriptBind_GameStatistics::PopGameScope(IFunctionHandler* pH)
{
	if (GetStats(pH) != m_pGS)
	{
...
i32 CScriptBind_GameStatistics::CurrentScope(IFunctionHandler* pH)
{
	i32 scope = m_pGS->GetScopeStackSize() != 0 ? (i32)m_pGS->GetScopeID() : -1;
...
i32 CScriptBind_GameStatistics::AddGameElement(IFunctionHandler* pH)
{
	if (GetStats(pH) != m_pGS)
	{
	...
	else
	{
		...
		else
		{
			...
			u32 lv = locVal.GetType() == EScriptAnyType::Number ? static_cast<u32>(locVal.GetNumber()) : static_cast<u32>(locVal.GetScriptHandle().n);
...
i32 CScriptBind_GameStatistics::RemoveGameElement(IFunctionHandler* pH)
{
	if (GetStats(pH) != m_pGS)
	{
...
i32 CScriptBind_GameStatistics::Event(IFunctionHandler* pH)
{
#if STATS_MODE_CVAR
	...
	if (!tracker)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[Telemetry] Event() tracker is nil");
	}
#if STATISTICS_CHECK_BINDING
	else if (m_boundTrackers.find(tracker) == m_boundTrackers.end())
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[Telemetry] Event() tracker is not bound");
	}
#endif
...
i32 CScriptBind_GameStatistics::StateValue(IFunctionHandler* pH)
{
#if STATS_MODE_CVAR
	if (CDrxActionCVars::Get().g_statisticsMode != 2)
		return pH->EndFunction();
	...
	if (!tracker)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[Telemetry] StateValue() tracker is nil");
	}
#if STATISTICS_CHECK_BINDING
	else if (m_boundTrackers.find(tracker) == m_boundTrackers.end())
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[Telemetry] StateValue() tracker is not bound");
	}
#endif
	else if (pH->GetParamCount() != 2)
	{
...
void SaveLuaToXML(XmlNodeRef node, tukk name, const SmartScriptTable& table)
{
