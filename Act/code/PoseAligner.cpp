...
bool CContactRaycast::Update(Vec3& position, Vec3& normal)
{
	...
	if (IActor* pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(m_pEntity ? m_pEntity->GetId() : -1))
	{
		// Always include self + any other optional entities actor would like to ignore
		i32 nSkip = pActor->GetPhysicalSkipEntities(pEntityPhysics ? &pSkipEntities[1] : pSkipEntities, pEntityPhysics ? MAX_SKIP_ENT - 1 : MAX_SKIP_ENT);
		if (pEntityPhysics)
		{
			++nSkip;
		}
...
bool CChain::SetupTargetPoseModifiers(const QuatT& location, const Vec3& limitOffset, ISkeletonAnim& skeletonAnim)
{
	...
	if (!m_pPoseAlignerChain)
		DrxCreateClassInstanceForInterface<IAnimationPoseAlignerChain>(drxiidof<IAnimationPoseAlignerChain>(), m_pPoseAlignerChain);
	if (!m_pPoseAlignerChain)
		return false;
...
bool CPose::Initialize(IEntity& entity, ICharacterInstance* pCharacter, i32 rootJointIndex)
{
	Clear();
...
void CPose::DrawDebug(const QuatT& location, const Vec3& groundNormal)
{
	...
	IRenderAuxText::DrawLabelF(location * m_pSkeletonPose->GetAbsJointByID(m_rootJointIndex).t, 1.5f,
	                           ...
	                           m_blendWeight);
	gEnv->pRenderer->GetIRenderAuxGeom()->DrawSphere(location.t, 0.125f, ColorB(u8((1.0f - m_blendWeight) * 255.0f), 0xff, 0xff, 0xff), true);
}
