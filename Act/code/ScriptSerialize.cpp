  ...
  i32 param)
{
	...
	case eSST_Void:
		break;
	default:
		DRX_ASSERT(!"need to add the type enum to this switch");
	}
	return true;
}
...
void CScriptSerializeAny::PushFuncParam(IScriptSystem* pSS) const
{
	...
	case eSST_Float:
		...
		break;
	case eSST_Int8:
		pSS->PushFuncParam((i32)*Ptr<int8>());
		break;
	case eSST_Int16:
		pSS->PushFuncParam((i32)*Ptr<i16>());
		break;
	case eSST_Int32:
		pSS->PushFuncParam((i32)*Ptr<i32>());
		break;
	case eSST_StringTable:
	case eSST_String:
...
void CScriptSerializeAny::SerializeWith(TSerialize ser, tukk name)
{
#define SER_NAME(n) (name ? name : n)
	...
	case eSST_Int8:
		...
		break;
	case eSST_Int16:
		ser.Value(SER_NAME("i16"), *Ptr<i16>());
		break;
	case eSST_Int32:
		ser.Value(SER_NAME("i32"), *Ptr<i32>());
		break;
	case eSST_String:
...
bool CScriptSerializeAny::CopyFromTable(SmartScriptTable& tbl, tukk name)
{
	bool ok = false;
...
void CScriptSerializeAny::CopyToTable(SmartScriptTable& tbl, tukk name)
{
	switch (m_type)
	{
	case eSST_Bool:
	...
	case eSST_Float:
		...
		break;
	case eSST_Int8:
		tbl->SetValue(name, (i32)*Ptr<int8>());
		break;
	case eSST_Int16:
		tbl->SetValue(name, (i32)*Ptr<i16>());
		break;
	case eSST_Int32:
		tbl->SetValue(name, (i32)*Ptr<i32>());
		break;
	case eSST_StringTable:
	case eSST_String:
...
string CScriptSerializeAny::DebugInfo() const
{
	...
	case eSST_Float:
		...
		break;
	case eSST_Int8:
		output.Format("%i", (i32)*Ptr<int8>());
		break;
	case eSST_Int16:
		output.Format("%i", (i32)*Ptr<i16>());
		break;
	case eSST_Int32:
		output.Format("%i", (i32)*Ptr<i32>());
		break;
	case eSST_StringTable:
	case eSST_String:
		output = *Ptr<string>();
	...
	case eSST_EntityId:
		{
			...
			tukk name = pEntity ? pEntity->GetName() : "<no such entity>";
			output.Format("entity[%u]:%s", id, name);
		}
		break;
	case eSST_Vec3:
		{
...
CScriptRMISerialize::CScriptRMISerialize(tukk format)
{
	while (*format)
	{
...
bool CScriptSerialize::ReadValue(tukk name, EScriptSerializeType type, TSerialize ser, IScriptTable* pTable)
{
	switch (type)
	{
	case eSST_Bool:
	...
	case eSST_Vec3:
		...
		break;
	default:
		DRX_ASSERT(!"type not added to ReadValue");
		return false;
	}
	return true;
}

bool CScriptSerialize::WriteValue(tukk name, EScriptSerializeType type, TSerialize ser, IScriptTable* pTable)
{
	bool ok = false;
	switch (type)
	{
	...
	default:
		DRX_ASSERT(!"type not added to WriteValue");
		ok = false;
	}
	return ok;
}
