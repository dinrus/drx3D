...
// -------------------------------------------------------------------------
//  Имя файла:   FlowNetMsgNodes.cpp
//  Описание: FG nodes to send bidirectional client <--> server simple messages
// -------------------------------------------------------------------------
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Target",     0,                                                            _HELP("Default (client sends msg to server and vice versa) - All (default + client messages are loopback to itself)"),_HELP("Target"), _UICONFIG("enum_int:Default=0,All=1")), // the values needs to coincide with SNetMsgData::ETarget
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		...
		case eFE_Activate:
			if (IsPortActive(pActInfo, IN_SEND))
			{
				...
				i32k iTarget = GetPortInt(pActInfo, IN_TARGET);
				SNetMsgData::ETarget target = SNetMsgData::ConvertTargetIntToEnum(iTarget);
				DRX_ASSERT_MESSAGE(target != SNetMsgData::eT_Invalid, "CFlowNode_MsgSender: IN_TARGET input got converted to an invalid Enum");
