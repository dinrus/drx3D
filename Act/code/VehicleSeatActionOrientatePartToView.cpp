...
bool CVehicleSeatActionOrientatePartToView::Init(IVehicle* pVehicle, IVehicleSeat* pSeat, const CVehicleParams& table)
{
	...
	if (partsTable)
	{
		...
		for (i32 partIdx = 0; partIdx < partCount; ++partIdx)
		{
			...
			{
				tukk controlledPartName = controlledPart.getAttr("name");
...
void CVehicleSeatActionOrientatePartToView::Update(const float deltaTime)
{
	...
	{
		IVehiclePart* pPart = m_pVehicle->GetPart((u32)partCit->partIndex);
...
void CVehicleSeatActionOrientatePartToView::GetMemoryUsage(IDrxSizer* s) const
{
