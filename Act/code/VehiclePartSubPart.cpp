...
//------------------------------------------------------------------------
bool CVehiclePartSubPart::Init(IVehicle* pVehicle, const CVehicleParams& table, IVehiclePart* pParent, CVehicle::SPartInitInfo& initInfo, i32 partType)
{
	...
	if (CVehicleParams subPartTable = table.findChild("SubPart"))
	{
		...
		if (!fileName.empty())
		{
			...
			for (EVehiclePartState state = eVGS_Default; state != eVGS_Last; state = static_cast<EVehiclePartState>(state + 1))
			{
				...
				IF_UNLIKELY (!gEnv->pDrxPak->IsFileExist(stateFileName))
				{
					continue;
				}
...
//------------------------------------------------------------------------
bool CVehiclePartSubPart::ChangeState(EVehiclePartState state, i32 flags)
{
	...
	for (TStateGeomVector::const_iterator iStateGeom = m_stateGeoms.begin(), iEnd = m_stateGeoms.end(); iStateGeom != iEnd; ++iStateGeom)
	{
		if (iStateGeom->state == state)
		{
			if (IStatObj* pStatObj = iStateGeom->pStatObj)
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CVehiclePartSubPart::ChangeState() : Failed to load geometry '%s'", pStatObj->GetFilePath());
...
//------------------------------------------------------------------------
void CVehiclePartSubPart::GetMemoryUsage(IDrxSizer* pSizer) const
{
	pSizer->Add(*this);
...
//------------------------------------------------------------------------
void CVehiclePartSubPart::GetMemoryUsageInternal(IDrxSizer* pSizer) const
{
