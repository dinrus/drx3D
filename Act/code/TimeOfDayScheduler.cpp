...
// -------------------------------------------------------------------------
//  Имя файла:   TimeOfDayScheduler.cpp
//  Version:     v1.00
...
CTimeOfDayScheduler::TimeOfDayTimerId CTimeOfDayScheduler::AddTimer(float time, CTimeOfDayScheduler::TimeOfDayTimerCallback callback, uk pUserData)
{
	assert(callback != 0);
	if (callback == 0)
...
uk CTimeOfDayScheduler::RemoveTimer(TimeOfDayTimerId id)
{
	TEntries::iterator iter = std::find(m_entries.begin(), m_entries.end(), id);
	if (iter == m_entries.end())
		return 0;
	uk pUserData = iter->pUserData;
...
void CTimeOfDayScheduler::Update()
{
	...
	//DrxLogAlways("CTOD: lastTime=%f curTime=%f", lastTime, curTime);
	...
	while (iter != iterEnd)
	{
		...
		if (entry.time > maxTime)
			break;
		// DrxLogAlways("Adding: %d time=%f", entry.id, entry.time);
	...
	if (bWrap) // process interval [0, curTime]
	{
		...
		while (iter != iterEnd)
		{
			...
			if (entry.time > curTime)
				break;
			// DrxLogAlways("Adding[wrap]: %d time=%f", entry.id, entry.time);
			processingEntries.push_back(entry);
			++iter;
		}
	}
...
public:
	...
	void ResetTimer()
	{
		...
		{
			CDrxAction::GetDrxAction()->GetTimeOfDayScheduler()->RemoveTimer(m_timerId);
			m_timerId = CTimeOfDayScheduler::InvalidTimerId;
		}
	}
	...
	void RegisterTimer(SActivationInfo* pActInfo)
	{
		...
		m_timerId = CDrxAction::GetDrxAction()->GetTimeOfDayScheduler()->AddTimer(time, OnTODCallback, (uk )this);
	}
...
protected:
	static void OnTODCallback(CTimeOfDayScheduler::TimeOfDayTimerId timerId, uk pUserData, float curTime)
	{
