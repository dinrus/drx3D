...
static tukk RUN_SPEED_UICONFIG = "enum_int:VerySlow=0,Walk=1,Run=2,Sprint=3";
static tukk STANCE_UICONFIG = "enum_int:Prone=0,Crouch=1,Combat=2,CombatAlerted=3,Relaxed=4,Stealth=5";
static tukk FORCE_UICONFIG = "enum_int:No=0,IgnoreAll=1";
static tukk SECONDARY_UICONFIG = "enum_int:No=0,Any=1,Left=2,Right=3";
...
public:
	...
	bool         Execute(SActivationInfo* pActInfo, tukk pSignalText, IAISignalExtraData* pData = NULL, i32 senderId = 0);
...
protected:
	...
	virtual bool ExecuteOnAI(SActivationInfo* pActInfo, tukk pSignalText,
		IAISignalExtraData* pData, IEntity* pEntity, IEntity* pSender);

	// you might want to override this method
	virtual bool ExecuteOnEntity(SActivationInfo* pActInfo, tukk pSignalText,
		IAISignalExtraData* pData, IEntity* pEntity, IEntity* pSender);

	// Utility function to set an AI's speed.
	void SetSpeed(IAIObject* pAI, i32 iSpeed);

	// Utility function to set an AI's stance.
	void SetStance(IAIObject* pAI, i32 stance);
...
//
//-------------------------------------------------------------------------------------------------------------
template<bool TBlocking> void CFlowNode_AIBase<TBlocking >::OnGoalPipeEvent(IPipeUser* pPipeUser, EGoalPipeEvent event, i32 goalPipeId, bool& unregisterListenerAfterEvent)
{
...
//
//-------------------------------------------------------------------------------------------------------------
template<bool TBlocking> bool CFlowNode_AIBase<TBlocking >::Execute(SActivationInfo* pActInfo, tukk pSignalText, IAISignalExtraData* pData, i32 senderId)
{
	UnregisterEvents();
...
//
//-------------------------------------------------------------------------------------------------------------
template<bool TBlocking> bool CFlowNode_AIBase<TBlocking >::ExecuteOnAI(SActivationInfo* pActInfo, tukk pSignalText,
	IAISignalExtraData* pData, IEntity* pEntity, IEntity* pSender)
{
	IAIObject* pAI = pEntity->GetAI();
	DRX_ASSERT(pAI);
...
//
//-------------------------------------------------------------------------------------------------------------
template<bool TBlocking> bool CFlowNode_AIBase<TBlocking >::ExecuteOnEntity(SActivationInfo* pActInfo, tukk pSignalText,
	IAISignalExtraData* pData, IEntity* pEntity, IEntity* pSender)
{
...
template<bool TBlocking> void CFlowNode_AIBase<TBlocking >::SetSpeed(IAIObject* pAI, i32 iSpeed)
{
	IAIActor* pAIActor = pAI->CastToIAIActor();
	if (!pAIActor)
		return;
...
template<bool TBlocking> void CFlowNode_AIBase<TBlocking >::SetStance(IAIObject* pAI, i32 stance)
{
	IAIActor* pAIActor = pAI->CastToIAIActor();
	if (!pAIActor)
		return;
...
template<bool TBlocking> void CFlowNode_AIForceableBase<TBlocking >::Serialize(IFlowNode::SActivationInfo* pActInfo, TSerialize ser)
{
	...
	i32 lastForceMethod = eNoForce;

	if (ser.IsWriting())
	{
		lastForceMethod = (i32) m_LastForceMethod;
	}
...
void CFlowNode_AIEnterVehicle::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig<i32>("Force",          eNoForce,                                                 _HELP("ForceExecution method"),  _HELP("Force"),                                     _UICONFIG(FORCE_UICONFIG)),
		{ 0 }
	};
...
void CFlowNode_AIEnterVehicle::DoProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	IVehicle* pVehicle = CDrxAction::GetDrxAction()->GetIVehicleSystem()->GetVehicle(vehicleId);
	if (!pVehicle || pVehicle->IsCrewHostile(pActInfo->pEntity->GetId()))
	{
		DrxLog("Actor %s failed to enter vehicle (IsCrewHostile returned true)", pActInfo->pEntity->GetName());
	...
	IActor* pActor = pEntity ? CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(pEntity->GetId()) : 0;
...
void CFlowNode_AIVehicleFollowPath::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		...
		InputPortConfig<i32>("Loops",            0,                                    _HELP("Number of times to loop around the path (-1 = forever)")),
		...
		InputPortConfig<i32>("Force",            0,                                    _HELP("ForceExecution method"),                                  _HELP("Force"), _UICONFIG(FORCE_UICONFIG)),
		{ 0 }
	};
	static const SOutputPortConfig out_config[] = {
...
void CFlowNode_AIVehicleFollowPath::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	if (m_EntityId && event == eFE_Activate && IsPortActive(pActInfo, 7))
	{
		...
		if (pEntity)
		{
			...
			{
				u16 nType = pAI->GetAIType();
				if (nType != AIOBJECT_VEHICLE)
				{
					DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "AI:VehicleFollowPath node should only be used to control vehicle entities");
					return;
				}
...
void CFlowNode_AIVehicleFollowPath::DoProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	i32 loops = GetPortInt(pActInfo, 5);
...
void CFlowNode_AIVehicleStickPath::OnEntityEvent(IEntity* pEntity, const SEntityEvent& event)
{
	...
	{
		tukk eventName = (tukk)event.nParam[0];
...
protected:
	i32                     m_numPassengers;
	std::map<i32, EntityId> m_mapPassengers;

	void         UnloadSeat(IVehicle* pVehicle, i32 seatId);
	virtual void OnGoalPipeEvent(IPipeUser* pPipeUser, EGoalPipeEvent event, i32 goalPipeId, bool& unregisterListenerAfterEvent);
	virtual void UnregisterEvents();
...
void CFlowNode_AIUnload::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig in_config[] = {
		InputPortConfig_Void("sink",   _HELP("for synchronization only"), _HELP("Sync")),
		InputPortConfig_Void("Cancel", _HELP("cancels execution")),
		InputPortConfig<i32>("Seat",   _HELP("seat(s) to be unloaded"),   NULL,          _UICONFIG("enum_int:All=0,AllExceptDriver=-1,AllExceptGunner=-2,Driver=1,Gunner=2,Seat03=3,Seat04=4,Seat05=5,Seat06=6,Seat07=7,Seat08=8,Seat09=9,Seat10=10,Seat11=11")),
		{ 0 }
	};
	static const SOutputPortConfig out_config[] = {
		OutputPortConfig<EntityId>("done",    _HELP("action done")),
...
{
	DRX_ASSERT(0); // This function should not be called!
}
	...
	else
	{
		...
		if (pEntity)
		{
			...
			else
			{
				...
				i32 seatsToUnload = GetPortInt(pActInfo, 2);
				i32 numSeats = pVehicle->GetSeatCount();
				switch (seatsToUnload)
				{
				case -2: // all except the gunner
					UnloadSeat(pVehicle, 1);
					for (i32 i = 3; i <= numSeats; ++i)
						UnloadSeat(pVehicle, i);
					break;
				case -1: // all except the driver
					for (i32 i = 2; i <= numSeats; ++i)
						UnloadSeat(pVehicle, i);
					break;
				case 0: // all
...
//
//-------------------------------------------------------------------------------------------------------------
void CFlowNode_AIUnload::UnloadSeat(IVehicle* pVehicle, i32 seatId)
{
			...
			if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(passengerId))
				if (IAIObject* pAI = pEntity->GetAI())
					if (IAIActor* pAIActor = pAI->CastToIAIActor())
						...
						{
							i32 goalPipeId = gEnv->pAISystem->AllocGoalPipeId();
...
//
//-------------------------------------------------------------------------------------------------------------
void CFlowNode_AIUnload::OnGoalPipeEvent(IPipeUser* pPipeUser, EGoalPipeEvent event, i32 goalPipeId, bool& unregisterListenerAfterEvent)
{
	std::map<i32, EntityId>::iterator it = m_mapPassengers.find(goalPipeId);
...
{
	std::map<i32, EntityId>::iterator it, itEnd = m_mapPassengers.end();
