...
static i32k MANNEQUIN_NAVSO_EXACTPOS_PRIORITY = 4; // currently equal to PP_UrgentAction, just above normal actions, but underneath hit death, etc.
static i32k MANNEQUIN_EXACTPOS_PRIORITY = 3;       // currently equal to PP_Action, just above movement but underneath urgent actions, hit death, etc.
...
CAIHandler::CAIHandler(IGameObject* pGameObject)
	...
	m_FaceUpr(pGameObject->GetEntity()),
...
//
//------------------------------------------------------------------------------
tukk CAIHandler::GetInitialCharacterName()
{
	tukk szAICharacterName = 0;
...
//
//------------------------------------------------------------------------------
tukk CAIHandler::GetInitialBehaviorName()
{
...
void CAIHandler::SetInitialBehaviorAndCharacter()
{
	...
	if (!isPlayer)
	{
...
#ifdef USE_DEPRECATED_AI_CHARACTER_SYSTEM
		tukk szDefaultCharacter = GetInitialCharacterName();
		...
		tukk szDefaultBehavior = GetInitialBehaviorName();
...
void CAIHandler::AIMind(SOBJECTSTATE& state)
{
	...
	case AITARGET_VISUAL:
		...
		{
			DRX_PROFILE_REGION(PROFILE_AI, "AI_OnObjectSeen");
		...
		else if (state.eTargetThreat == AITHREAT_AGGRESSIVE)
		{
			if (!pExtraData)
				pExtraData = gEnv->pAISystem->CreateSignalExtraData();
			pExtraData->iValue = (i32)state.eTargetStuntReaction;
			state.eTargetStuntReaction = AITSR_NONE;

			DRX_PROFILE_REGION(PROFILE_AI, "AI_OnPlayerSeen2");
			value = &state.fDistanceFromTarget;
			...
			if (targetIsCloaked)
			{
				tukk cloakedTargetSeenSignal = "OnCloakedTargetSeen";
...
//
//------------------------------------------------------------------------------
void CAIHandler::AISignal(i32 signalID, tukk signalText, u32 crc, IEntity* pSender, const IAISignalExtraData* pData)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
#ifdef USE_DEPRECATED_AI_CHARACTER_SYSTEM
	if (tukk szNextBehavior = CheckAndGetBehaviorTransition(signalText))
	{
...
//
//------------------------------------------------------------------------------
tukk CAIHandler::CheckAndGetBehaviorTransition(tukk szSignalText) const
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	tukk szBehaviorName = 0;
	tukk szNextBehaviorName = 0;
	...
	if (*m_pBehavior && *m_pCharacter)
	{
		...
		if (bTable || bDefaultTable)
		{
			    ...
			    (bDefaultTable && (pCharacterDefaultTable->GetValue(szSignalText, szNextBehaviorName) || pCharacterDefaultTable->GetValue("_ANY_", szNextBehaviorName))))
			{
				...
				{
					DRX_PROFILE_REGION(PROFILE_AI, "Logging of the character change");
	...
	{
		DRX_PROFILE_REGION(PROFILE_AI, "Logging of DEFAULT character change");
		if (m_pEntity && m_pEntity->GetName())
		{
...
//
//------------------------------------------------------------------------------
bool CAIHandler::SetCharacter(tukk szCharacter, ESetFlags setFlags)
{
...
//
//------------------------------------------------------------------------------
void CAIHandler::SetBehavior(tukk szNextBehaviorName, const IAISignalExtraData* pData, ESetFlags setFlags)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	i32 alertness = m_CurrentAlertness;
	i32 exclusive = 0;
	...
	if (aiObject)
	{
		if (IAIActorProxy* proxy = aiObject->GetProxy())
		{
			if (IAIActor* actor = aiObject->CastToIAIActor())
			{
				...
				tukk currentName = proxy->GetCurrentBehaviorName();
				tukk previousName = proxy->GetPreviousBehaviorName();
...
//
//-------------------------------------------------------------------------------------------------
void CAIHandler::FindOrLoadBehavior(tukk szBehaviorName, SmartScriptTable& pBehaviorTable)
{
	...
	if (!m_pBehaviorTable->GetValue(szBehaviorName, pBehaviorTable))
	{
		...
		DRX_PROFILE_REGION(PROFILE_AI, "On-DemandBehaviourLoading");
		tukk szAIBehaviorFileName = GetBehaviorFileName(szBehaviorName);
...
//
//------------------------------------------------------------------------------
tukk CAIHandler::GetBehaviorFileName(tukk szBehaviorName)
{
	tukk szFileName = 0;
...
//
//------------------------------------------------------------------------------
bool CAIHandler::CallScript(IScriptTable* scriptTable, tukk funcName, float* pValue, IEntity* pSender, const IAISignalExtraData* pData)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	if (scriptTable)
	{
		...
		if (bFound)
		{
			...
			//			str+= funcName;
			//		DRX_PROFILE_REGION(PROFILE_AI, "Calling behavior signal");
			//		DRX_PROFILE_REGION(PROFILE_AI, str.c_str());
			...
			//drx_sprintf(m_szSignalName,"AISIGNAL: %s",funcName);
			DRX_PROFILE_REGION(PROFILE_AI, "AISIGNAL");

#ifdef AI_LOG_SIGNALS
			static ICVar* pCVarAILogging = NULL;
			CTimeValue start;
			uint64 pagefaults = 0;
			if (CDrxActionCVars::Get().aiLogSignals)
			{
				if (!pCVarAILogging)
				...
				if (pCVarAILogging && pCVarAILogging->GetIVal())
				{
					...
					gEnv->pSystem->GetIMemoryUpr()->GetProcessMemInfo(memCounters);
			...
			if (CDrxActionCVars::Get().aiLogSignals)
			{
				...
				if (start.GetMilliSeconds() > CDrxActionCVars::Get().aiMaxSignalDuration)
				{
					tuk behaviour = "";
					scriptTable->GetValue("Name", behaviour);
					...
					char buffer[256];
					drx_sprintf(buffer, "%s.%s,%g,%d,%s", behaviour, funcName, start.GetMilliSeconds(), (i32)pagefaults, m_pEntity->GetName());
...
bool CAIHandler::GetMostLikelyTable(IScriptTable* table, SmartScriptTable& destTable)
{
	...
	i32 probability = drx_random(0, maxProb - 1);
...
void CAIHandler::SetAlertness(i32 value, bool triggerEvent /*=false*/)
{
	assert(value <= 2);
	...
	IActor* const pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(m_pEntity->GetId());
	if (pActor)
	{
		if (switchToWeaponAlerted)
		{
			SEntityEvent radarEvent(ENTITY_EVENT_ADD_TO_RADAR);
	...
	CAIFaceUpr::e_ExpressionEvent expression(CAIFaceUpr::EE_NONE);
	tukk sStates = "";
	switch (m_CurrentAlertness)
	{
	case 0: // Idle
		sStates = "Idle-Alerted,Combat";
		expression = CAIFaceUpr::EE_IDLE;
		break;
	case 1: // Alerted
		sStates = "Alerted-Idle,Combat";
		expression = CAIFaceUpr::EE_ALERT;
		break;
	case 2: // Combat
		sStates = "Combat-Idle,Alerted";
		expression = CAIFaceUpr::EE_COMBAT;
		break;
	}
	MakeFace(expression);

	if (sStates)
		gEnv->pAISystem->GetSmartObjectUpr()->ModifySmartObjectStates(m_pEntity, sStates);
}
		...
		// Safety check to handle invalid coverlocations
		{
			...
			if (isInCoverStance)
			{
				if (state.coverRequest.coverLocationRequest == eCoverLocationRequest_Set)
				{
					...
					DRX_ASSERT(coverLocation.IsValid());

					const float TOO_FAR_AWAY = 50.0f;
					if (coverLocation.t.GetSquaredDistance2D(m_pEntity->GetPos()) >= sqr(TOO_FAR_AWAY))
					{
						DRX_ASSERT_MESSAGE(false, "Cover stance requested but CoverLocation is too far away from the entity");
...
// ----------------------------------------------------------------------------
CAnimActionExactPositioning* CAIHandler::CreateExactPositioningAction(bool isOneShot, const float loopDuration, tukk szFragmentID, bool isNavigationalSO, const QuatT& exactStartLocation)
{
	IActor* pActor = GetActor();
	if (!pActor)
		return NULL;
	...
	u32k valueCRC = CCrc32::ComputeLowercase(szFragmentID);

	bool isDefaultValue = true;
	if (isOneShot)
	{
		static u32k noneCRC = CCrc32::ComputeLowercase("none");
		isDefaultValue = (valueCRC == noneCRC);
	}
	else
	{
		static u32k idleCRC = CCrc32::ComputeLowercase("idle");
		isDefaultValue = (valueCRC == idleCRC);
	}

	if (!valueCRC)
	{
		DRX_ASSERT(false); // no signal or action was selected, or we're extremely unlucky and hit a 0 CRC
		return NULL;
	}

	DRX_ASSERT(!isDefaultValue);
	if (isDefaultValue)
		return NULL;
...
//
//------------------------------------------------------------------------------
CTimeValue CAIHandler::GetEstimatedAGAnimationLength(EAIAGInput input, tukk value)
{
	CTimeValue length;
	...
	for (i32 layerIndex = 0; pQuery = pAGState->CreateExistanceQuery(layerIndex); layerIndex++)
	{
...
//
//------------------------------------------------------------------------------
bool CAIHandler::SetAGInput(EAIAGInput input, tukk value, const bool isUrgent)
{
	IActionController* pActionController = GetActionController();
	if (pActionController)
...
bool CAIHandler::IsSignalAnimationPlayed(tukk value)
{
...
bool CAIHandler::IsActionAnimationStarted(tukk value)
{
...
//
//------------------------------------------------------------------------------
bool CAIHandler::FindOrLoadTable(IScriptTable* pGlobalTable, tukk szTableName, SmartScriptTable& tableOut)
{
...
//
//------------------------------------------------------------------------------
void CAIHandler::MakeFace(CAIFaceUpr::e_ExpressionEvent expression)
{
	m_FaceUpr.SetExpression(expression);
}

//
//------------------------------------------------------------------------------
/*static*/ tukk CAIHandler::GetAGInputName(EAIAGInput input)
{
	switch (input)
	{
	case AIAG_SIGNAL:
	...
	default:
		{
			DRX_ASSERT_MESSAGE(false, "Invalid input name");
			return NULL;
		}
	}
}
...
//
//------------------------------------------------------------------------------
void CAIHandler::DoReadibilityPackForAIObjectsOfType(u16 nType, tukk szText, float fResponseDelay)
{
	...
	i32 count = pAISystem->GetGroupCount(groupId, IAISystem::GROUP_ENABLED, nType);
	...
	if (nearestInGroup)
	{
		...
		pData->fValue = fResponseDelay > 0.0f ? fResponseDelay : drx_random(2.5f, 4.f); // Delay
...
void CAIHandler::OnAnimActionTriStateMiddle(CAnimActionTriState* pActionTriState)
{
	...
	if (pAction->GetType() == CAnimActionAIAction::EAT_Looping)
	{
		DRX_ASSERT(m_currentActionAnimName == value);
		DRX_ASSERT(m_playingActionAnimation);
	...
	{
		DRX_ASSERT(m_currentSignalAnimName == value);
		DRX_ASSERT(m_playingSignalAnimation);
...
//
//------------------------------------------------------------------------------
bool CAIHandler::SetMannequinAGInput(IActionController* pActionController, EAIAGInput input, tukk value, const bool isUrgent)
{
	u32k valueCRC = CCrc32::ComputeLowercase(value);
	...
	if (input == AIAG_ACTION)
	{
		...
		else
		{
			if (!isIdleValue)
			{
				...
				//}
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CAIHandler::SetAGInput: Unable to find looping animation '%s'", value);
			}
	...
	else if (input == AIAG_SIGNAL)
	{
		...
		else
		{
			if (!isNoneValue)
			{
				...
				//}
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CAIHandler::SetAGInput: Unable to find one-shot animation '%s'", value);
			}
