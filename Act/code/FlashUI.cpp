...
#include <drx3D/Act/FlashUIAction.h>
#include <drx3D/Act/FlashUIElementNodes.h>
#include <drx3D/Act/FlashUIEventNodes.h>
#include <drx3D/Act/FlashUIActionEvents.h>
...
//------------------------------------------------------------------------------------
string CreateDisplayName(tukk format, ...)
{
	va_list args;
	va_start(args, format);
	char result[1024];
	drx_vsprintf(result, format, args);
	va_end(args);
	return result;
}
...
void RenderStackDebugInfo(bool, tukk , i32);
#endif
//-------------------------------------------------------------------
DRXREGISTER_SINGLETON_CLASS(CFlashUI)

CFlashUI::CFlashUI()
	: m_pUIActionUpr(new CUIActionUpr())
	, m_iWidth(1)
	, m_iHeight(1)
...
{
	DRX_ASSERT_MESSAGE(gEnv->pHardwareMouse && gEnv->pInput, "Unable to register as input listener!");
	if (gEnv->pHardwareMouse && gEnv->pInput)
	{
	...
	if (gEnv->pGameFramework)
	{
		...
		DRX_ASSERT_MESSAGE(gEnv->pGameFramework->GetILevelSystem(), "Unable to register as levelsystem listener!");
	...
	DRX_ASSERT_MESSAGE(gEnv->pSystem, "Unable to register as system listener!");
	if (gEnv->pSystem)
	{
...
//------------------------------------------------------------------------------------
void CFlashUI::OnLevelNotFound(tukk levelName)
{
	StopRenderThread();
...
//------------------------------------------------------------------------------------
void CFlashUI::OnLoadingError(ILevelInfo* pLevel, tukk error)
{
	StopRenderThread();
...
//------------------------------------------------------------------------------------
void CFlashUI::OnLoadingProgress(ILevelInfo* pLevel, i32 progressAmount)
{
	if (m_systemState == eSS_Loading)
	{
		if (CV_gfx_loadtimethread == 0 && m_bLoadtimeThread == false)
		{
			u32 nRecursionLevel = 0;
...
void CFlashUI::UpdateFG()
{
	...
	DRX_ASSERT_MESSAGE(!isUpdating, "Recursive loop: trying to update UIAction FlowGraphs within update loop!");
	if (!isUpdating)
	{
		isUpdating = true;
		i32 loops = 0;
		do
		{
			m_pUIActionUpr->Update();// preupdate
			...
			m_pUIActionUpr->Update();// post update
			DEBUG_ADD_STACK("PostUpdate", loops);

			if (++loops > 255)
			{
				DRX_ASSERT_MESSAGE(false, "stack constantly gets new events within this update loop, should not happen!");
...
{
	DRX_ASSERT_MESSAGE(gEnv->IsEditor(), "Only allowed for editor!");
	if (gEnv->IsEditor())
	{
...
//------------------------------------------------------------------------------------
void CFlashUI::RegisterModule(IUIModule* pModule, tukk name)
{
	const bool ok = m_modules.Add(pModule, name);
	DRX_ASSERT_MESSAGE(ok, "Module already registered!");
}

//------------------------------------------------------------------------------------
void CFlashUI::UnregisterModule(IUIModule* pModule)
{
	DRX_ASSERT_MESSAGE(m_modules.Contains(pModule), "Module was never registered or already unregistered!");
...
//-------------------------------------------------------------------
void CFlashUI::OnHardwareMouseEvent(i32 iX, i32 iY, EHARDWAREMOUSEEVENT eHardwareMouseEvent, i32 wheelDelta)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
...
void CFlashUI::CreateMouseClick(IUIElement::EControllerInputState state)
{
	...
	SendFlashMouseEvent(evt, (i32) fX, (i32) fY, 0, 0.f, true);
}

//-------------------------------------------------------------------
void CFlashUI::SendFlashMouseEvent(SFlashCursorEvent::ECursorState evt, i32 iX, i32 iY, i32 iButton /*= 0*/, float fWheel /*= 0.f*/, bool bFromController /*= false*/)
{
	const TSortedElementList& sortedElements = GetSortedElements();
...
//-------------------------------------------------------------------
bool CFlashUI::DisplayVirtualKeyboard(u32 flags, tukk title, tukk initialInput, i32 maxInputLength, IVirtualKeyboardEvents* pInCallback)
{
	IPlatformOS* pPlatformOS = gEnv->pSystem->GetPlatformOS();
	if (pPlatformOS && gEnv->pGameFramework)
	{
		u32 controllerIdx = gEnv->pGameFramework->GetIPlayerProfileUpr()->GetExclusiveControllerDeviceIndex();
...
//-------------------------------------------------------------------
IUIActionUpr* CFlashUI::GetUIActionUpr() const
{
	return m_pUIActionUpr;
}
...
static void CreateMCFunctionNodes(std::vector<CFlashUiFlowNodeFactory*>& nodelist, IUIElement* pElement, Item* pItem, const string& path, bool fill = false)
{
	...
	if (fill)
	{
		i32k fctcount = SUIGetDesc<eUOT_Functions, Item, i32>::GetCount(pItem);
		for (i32 i = 0; i < fctcount; ++i)
		{
			const SUIEventDesc* pDesc = SUIGetDesc<eUOT_Functions, Item, i32>::GetDesc(pItem, i);
	...
	i32k subcount = SUIGetDesc<Type, Item, i32>::GetCount(pItem);
	for (i32 i = 0; i < subcount; ++i)
	{
		const SUIMovieClipDesc* pDesc = SUIGetDesc<Type, Item, i32>::GetDesc(pItem, i);
		string newpath = path;
		newpath += ":";
...
//-------------------------------------------------------------------
bool CFlashUI::LoadActionFromFile(tukk sFileName, IUIAction::EUIActionType type)
{
	switch (type)
	{
...
//-------------------------------------------------------------------
bool CFlashUI::LoadFGActionFromFile(tukk sFileName)
{
...
//-------------------------------------------------------------------
bool CFlashUI::LoadLuaActionFromFile(tukk sFileName)
{
	IUIAction* pAction = GetUIAction(PathUtil::GetFileName(sFileName));
	if (!pAction)
	{
...
//-------------------------------------------------------------------
void CFlashUI::LoadFromFile(tukk pFolderName, tukk pSearch, bool (CFlashUI::* fhFileLoader)(tukk))
{
	...
	if (handle != -1)
	{
		...
		do
		{
			drx_strcpy(filename, pFolderName);
			drx_strcat(filename, "/");
...
//-------------------------------------------------------------------
void CFlashUI::PreloadTextures(tukk pLevelName)
{
	LOADING_TIME_PROFILE_SECTION;
	...
	if (!gEnv->pDrxPak->IsFileExist(file))
		return;
...
void CFlashUI::PreloadTexturesFromNode(const XmlNodeRef& node)
{
	if (node)
	{
		...
		{
			tukk pFile = node->getChild(i)->getAttr("file");
			if (pFile)
			{
...
bool CFlashUI::PreloadTexture(tukk pFileName)
{
...
//-------------------------------------------------------------------
bool CFlashUI::UseSharedRT(tukk instanceStr, bool defVal) const
{
...
//-------------------------------------------------------------------
void CFlashUI::GetMemoryStatistics(IDrxSizer* s) const
{
	...
	s->Add(*m_pUIActionUpr);
	s->Add(*m_pFlashUIActionEvents);
}

//-------------------------------------------------------------------
IUIEventSystem* CFlashUI::CreateEventSystem(tukk sName, IUIEventSystem::EEventSystemType eType)
{
...
//------------------------------------------------------------------------------------
IUIEventSystem* CFlashUI::GetEventSystem(tukk name, IUIEventSystem::EEventSystemType eType)
{
	TUIEventSystemMap* pMap = GetEventSystemMap(eType);
...
TUIEventSystemMap* CFlashUI::GetEventSystemMap(IUIEventSystem::EEventSystemType eType)
{
	...
	case IUIEventSystem::eEST_UI_TO_SYSTEM:
		...
		break;
	default:
		DRX_ASSERT_MESSAGE(false, "Invalid IUIEventSystem::EEventSystemType type");
	}
	return pMap;
}
...
//-------------------------------------------------------------------
std::pair<string, i32> CFlashUI::GetUIIdentifiersByInstanceStr(tukk sUIInstanceStr) const
{
	std::pair<string, i32> result = std::make_pair<string, i32>("", 0);

	DRX_ASSERT(sUIInstanceStr != nullptr);
	...
	char name[_MAX_PATH];
	drx_strcpy(name, tmpName.c_str());

	tukk pExt = PathUtil::GetExt(sUIInstanceStr);
	if (*pExt != '\0' && strcmpi(pExt, "ui"))
		return result;

	uint instanceId = 0;

	i32 str_length = strlen(name);
	i32 index = 0;
	i32 id_index = -1;
	while (name[index] != '\0')
	{
...
std::pair<IUIElement*, IUIElement*> CFlashUI::GetUIElementsByInstanceStr(tukk sUIInstanceStr) const
{
	std::pair<IUIElement*, IUIElement*> result = std::make_pair<IUIElement*, IUIElement*>(nullptr, nullptr);
	std::pair<string, i32> identifiers = GetUIIdentifiersByInstanceStr(sUIInstanceStr);
...
IUIElement* CFlashUI::GetUIElementByInstanceStr(tukk sUIInstanceStr) const
{
...
//------------------------------------------------------------------------------------
void CFlashUI::GetScreenSize(i32& width, i32& height)
{
	DRX_ASSERT_MESSAGE(gEnv->IsEditor(), "Only allowed for editor!");
	if (m_ScreenSizeCB)
	{
...
//------------------------------------------------------------------------------------
void CFlashUI::LogUIAction(ELogEventLevel level, tukk format, ...)
{
#if defined(UIACTION_LOGGING)
	...
	else
	{
		if (CheckFilter(tmp))
		{
			DrxLogAlways("$7[UIAction] $1%s", tmp);
		}
	}
#endif
}
...
bool CFlashUI::CheckFilter(const string& str)
{
	...
	if (CV_gfx_uiaction_log_filter)
	{
		...
		i32k filter_count = filterlist.GetArgCount();
		for (i32 i = 0; i < filter_count; ++i)
		{
			string filterEntry;
...
//------------------------------------------------------------------------------------
#if !defined (_RELEASE) || defined(ENABLE_UISTACK_DEBUGGING)
void Draw2dLabel(float x, float y, float fontSize, const float* pColor, tukk pText)
{
...
void DrawTextLabelEx(float x, float y, float size, const float* pColor, tukk pFormat, ...) PRINTF_PARAMS(5, 6);
void DrawTextLabelEx(float x, float y, float size, const float* pColor, tukk pFormat, ...)
{
	char buffer[512];
...
void RenderDebugInfo()
{
	if (gEnv->pFlashUI && gEnv->pSystem)
	{
		...
		if (CFlashUI::CV_gfx_debugdraw & 1)
		{
			...
			for (i32 i = 0; i < gEnv->pFlashUI->GetUIElementCount(); ++i)
			{
				...
				DrawTextLabel(col6, py, color, "%4d KB", static_cast<u32>(pSizer->GetTotalSize() >> 10));
				pSizer->Release();
				py += dy;
		...
		if (CFlashUI::CV_gfx_debugdraw & 2)
		{
			...
			for (i32 i = 0; i < gEnv->pFlashUI->GetUIActionCount(); ++i)
			{
			...
			for (i32 i = 0; i < 2; ++i)
			{
				for (std::list<IUIAction*>::iterator it = actions[i].begin(); it != actions[i].end(); ++it)
				{
					...
					DrawTextLabel(col4, py, color, "%4d KB", static_cast<u32>(pSizer->GetTotalSize() >> 10));
					pSizer->Release();
					py += dy;
				}
				py += dy_space;
...
struct SStackInfo
{
	...
	SStackInfo(tukk str, float t) : info(str), time(t), index(-1), type(eDefault), id(-1) {}
	SStackInfo(tukk str, float t, i32 i, EType tp, i32 idx) : info(str), time(t), index(i), type(tp), id(idx) {}
	string info;
	float  time;
	i32    index;
	EType  type;
...
void RenderStackDebugInfo(bool render, tukk label, i32 loop)
{
	if (CFlashUI::CV_gfx_debugdraw & 4)
	{
		static bool empty = true;
		static std::deque<SStackInfo> stacklist;
		static std::vector<i32> currStack;
		static std::map<i32, tukk > lastStack;

		const float time = gEnv->pTimer->GetAsyncCurTime();

		const std::map<i32, tukk >& stack = CUIFGStackMan::GetStack();
		if (lastStack != stack)
		{
			lastStack = stack;
			...
			if (!stack.empty() || !currStack.empty())
			{
				...
				i32 removed = 0;
				for (std::vector<i32>::iterator it = currStack.begin(); it != currStack.end(); )
				{
				...
				i32 stackNum = 0;
				for (std::map<i32, tukk >::const_iterator it = stack.begin(), end = stack.end(); it != end; ++it)
				{
		...
		float py = 0;
		float px = 0;
		i32k count = stacklist.size();
		i32 round = 0;

		colorWhite[3] = 1.f;
		for (i32 i = 0; i <= count / itemsPerRow; ++i)
		{
