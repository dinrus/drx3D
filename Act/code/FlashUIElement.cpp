...
#include <drx3D/Act/FlashUIElement.h>
...
struct SUIElementSerializer
{
	...
	{
		DRX_ASSERT_MESSAGE(pElement, "NULL pointer passed!");
		DRX_ASSERT_MESSAGE(xmlNode != 0, "XmlNode is invalid");
	...
	static void ReadElementFlag(const XmlNodeRef& node, CFlashUIElement* pElement, tukk sFlag, IUIElement::EFlashUIFlags eFlag)
	{
		bool xmlflag = false;
	...
	static IUIElement::SUIConstraints::EPositionType GetPositionType(tukk mode)
	{
		if (mode != NULL)
		{
	...
	static IUIElement::SUIConstraints::EPositionAlign GetAlignType(tukk type)
	{
		if (type != NULL)
		{
	...
	static SUIParameterDesc::EUIParameterType GetParamType(tukk type)
	{
		if (type != NULL)
		{
			if (!strcmpi('bool', type))
				return SUIParameterDesc::eUIPT_Bool;
			else if (!strcmpi("i32", type))
	...
	static D* AddToList(T& list, tukk name, tukk displname, tukk desc, SUIParameterDesc::EUIParameterType eType, const SUIParameterDesc* pParent)
	{
	...
	template<class D, class T>
	static void ReadParamNodes(const XmlNodeRef& node, T& list, tukk sParamName, bool bSetParent = true, const SUIParameterDesc* pParent = NULL, tukk nodename = NULL)
	{
		if (node)
		{
			for (i32 i = 0; i < node->getChildCount(); ++i)
			{
				...
				tukk displName = paramNode->getAttr("name");
				tukk varName = paramNode->getAttr(sParamName);
				varName = strcmp(varName, "") ? varName : displName;
				tukk root = strstr(varName, "_root.");
				if (root) varName = root + 6;
				tukk desc = paramNode->getAttr("desc");
				tukk type = paramNode->getAttr("type");
	...
	template<class T>
	static void ReadEventNodes(const XmlNodeRef& node, T& list, tukk sEventName, const SUIParameterDesc* pParent = NULL, tukk nodename = NULL)
	{
		if (node)
		{
			for (i32 i = 0; i < node->getChildCount(); ++i)
			{
				...
				tukk displName = eventNode->getAttr("name");
				tukk eventName = eventNode->getAttr(sEventName);
				eventName = strcmp(eventName, "") ? eventName : displName;
				tukk root = strstr(eventName, "_root.");
				if (root) eventName = root + 6;
				tukk desc = eventNode->getAttr("desc");
				bool IsDyn = false;
				eventNode->getAttr("dynamic", IsDyn);
				tukk dynName = eventNode->getAttr("dynName");
				tukk dynDesc = eventNode->getAttr("dynDesc");
	...
	template<class T>
	static bool IsUnique(const T& list, tukk displayName, tukk flashName)
	{
...
template<>
SUIParameterDesc* SUIElementSerializer::AddToList<SUIParameterDesc, TUIParams>(TUIParams& list, tukk name, tukk displname, tukk desc, SUIParameterDesc::EUIParameterType eType, const SUIParameterDesc* pParent)
{
...
template<>
bool SUIElementSerializer::IsUnique<TUIParams>(const TUIParams& list, tukk displayName, tukk flashName)
{
...
CFlashUIElement::~CFlashUIElement()
{
	...
	DRX_ASSERT_MESSAGE(m_variableObjects.empty(), "Variable objects not cleared!");
	DRX_ASSERT_MESSAGE(!m_pFlashplayer, "Flash player not correct unloaded!");
	DRX_ASSERT_MESSAGE(m_pBootStrapper == NULL, "Flash bootstrapper not correct unloaded!");

#if !defined (_RELEASE)
	bool ok = stl::find_and_erase(s_ElementDebugList, this);
	DRX_ASSERT_MESSAGE(ok, "UIElement was not registered to debug list!!");
#endif
...
//------------------------------------------------------------------------------------
tukk CFlashUIElement::GetStringBuffer(tukk str)
{
	if (m_pBaseInstance)
...
//------------------------------------------------------------------------------------
void CFlashUIElement::SetFlashFile(tukk sFlashFile)
{
...
bool CFlashUIElement::Init(bool bLoadAsset)
{
	...
	assert(pRoot);
	m_variableObjects[CDrxName(m_pRoot->sName)].pObj = pRoot;
	m_variableObjects[CDrxName(m_pRoot->sName)].pParent = NULL;
	m_variableObjectsLookup[(const SUIParameterDesc*)NULL][m_pRoot] = &m_variableObjects[CDrxName(m_pRoot->sName)];
	if (!HasExtTexture())
	{
		UpdateViewPort();
	}

	UIACTION_LOG("%s (%i): UIElement invoke \"drx_onSetup\"", GetName(), m_iInstanceID);
	if (m_pFlashplayer->IsAvailable("drx_onSetup"))
	{
		if (gEnv->IsEditor())
		{
			...
			case IFlashUI::ePUI_Durango:
				m_pFlashplayer->Invoke1("drx_onSetup", FLASH_PLATFORM_DURANGO);
				break;
			case IFlashUI::ePUI_Orbis:
				m_pFlashplayer->Invoke1("drx_onSetup", FLASH_PLATFORM_ORBIS);
				break;
			case IFlashUI::ePUI_Android:
				m_pFlashplayer->Invoke1("drx_onSetup", FLASH_PLATFORM_ANDROID);
				break;
			default:
				m_pFlashplayer->Invoke1("drx_onSetup", FLASH_PLATFORM_PC);
			}
		}
		else
		{
...
void CFlashUIElement::Unload(bool bAllInstances)
{
	...
	if (m_pFlashplayer)
	{
		...
		for (i32 i = m_firstDynamicDisplObjIndex; i < m_displayObjects.size(); ++i)
...
void CFlashUIElement::DestroyBootStrapper()
{
	...
	DRX_ASSERT_MESSAGE(!HasExtTexture(), "Can't destory Bootstrapper while still used by dynamic textures");
	SAFE_RELEASE(m_pBootStrapper);
...
{
	DRX_ASSERT_MESSAGE(bIsLoading, "only loading supported!");
...
void CFlashUIElement::RequestHide()
{
	...
	if (m_pFlashplayer && m_pFlashplayer->IsAvailable("drx_requestHide"))
	{
		UIACTION_LOG("%s (%i): UIElement invoke \"drx_requestHide\"", GetName(), m_iInstanceID);
		m_pFlashplayer->Invoke0("drx_requestHide");
	}
...
void CFlashUIElement::SetVisible(bool bVisible)
{
	...
	if (m_pFlashplayer)
	{
		if (m_bVisible && m_pFlashplayer->IsAvailable("drx_onShow"))
		{
			UIACTION_LOG("%s (%i): UIElement invoke \"drx_onShow\"", GetName(), m_iInstanceID);
			m_pFlashplayer->Invoke0("drx_onShow");
		}
		else if (m_pFlashplayer->IsAvailable("drx_onHide"))
		{
			UIACTION_LOG("%s (%i): UIElement invoke \"drx_onHide\"", GetName(), m_iInstanceID);
			m_pFlashplayer->Invoke0("drx_onHide");
		}
...
//------------------------------------------------------------------------------------
void CFlashUIElement::SetLayer(i32 iLayer)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
...
{
	DRX_ASSERT_MESSAGE((flag & eFUI_NOT_CHANGEABLE) == 0, "Not allowed to set non changeable flag during runtime!");
...
bool CFlashUIElement::DefaultInfoCheck(SFlashObjectInfo*& pInfo, const SUIParameterDesc* pDesc, const SUIMovieClipDesc* pTmplDesc)
{
	...
	DRX_ASSERT_MESSAGE(pDesc, "NULL pointer passed!");
	if (!pDesc)
		return false;

	const SUIMovieClipDesc* pParentDesc = pTmplDesc ? pTmplDesc : m_pRoot;
	DRX_ASSERT_MESSAGE(pParentDesc, "Invalid parent passed!");
	if (!pParentDesc)
		return false;
...
//------------------------------------------------------------------------------------
bool CFlashUIElement::CallFunction(tukk pFctName, const SUIArguments& args, TUIData* pDataRes, tukk pTmplName)
{
...
//------------------------------------------------------------------------------------
IFlashVariableObject* CFlashUIElement::GetMovieClip(tukk movieClipName, tukk pTmplName)
{
...
//------------------------------------------------------------------------------------
IFlashVariableObject* CFlashUIElement::CreateMovieClip(const SUIMovieClipDesc*& pNewInstanceDesc, tukk movieClipTemplate, tukk mcParentName, tukk mcInstanceName)
{
...
//------------------------------------------------------------------------------------
IFlashVariableObject* CFlashUIElement::CreateMovieClip(const SUIMovieClipDesc*& pNewInstanceDesc, const SUIMovieClipDesc* pMovieClipTemplateDesc, const SUIMovieClipDesc* pParentMC, tukk mcInstanceName)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
	...
	if (pParent)
	{
		...
		if (pNewMc)
		{
			...
			SFlashObjectInfo* pNewInfo = &m_variableObjects[CDrxName(instanceName.c_str())];
...
//------------------------------------------------------------------------------------
void CFlashUIElement::RemoveMovieClip(tukk movieClipName)
{
...
void CFlashUIElement::RemoveMovieClip(IFlashVariableObject* flashVarObj)
{
	...
	DRX_ASSERT_MESSAGE(false, "The given IFlashVariableObject was not created thru the UISystem!");
}

//------------------------------------------------------------------------------------
bool CFlashUIElement::SetVariable(tukk pVarName, const TUIData& value, tukk pTmplName)
{
...
//------------------------------------------------------------------------------------
bool CFlashUIElement::GetVariable(tukk pVarName, TUIData& valueOut, tukk pTmplName)
{
	const SUIParameterDesc* pVarDesc = GetOrCreateVariableDesc(pVarName);
	const SUIMovieClipDesc* pTmplDesc = pTmplName ? GetMovieClipDesc(pTmplName) : NULL;
...
//------------------------------------------------------------------------------------
bool CFlashUIElement::CreateVariable(const SUIParameterDesc*& pNewDesc, tukk varName, const TUIData& value, tukk pTmplName)
{
	const SUIMovieClipDesc* pTmplDesc = pTmplName ? GetMovieClipDesc(pTmplName) : NULL;
...
//------------------------------------------------------------------------------------
bool CFlashUIElement::CreateVariable(const SUIParameterDesc*& pNewDesc, tukk varName, const TUIData& value, const SUIMovieClipDesc* pTmplDesc)
{
	bool bExist;
...
//------------------------------------------------------------------------------------
bool CFlashUIElement::SetArray(tukk pArrayName, const SUIArguments& values, tukk pTmplName)
{
...
bool CFlashUIElement::SetArray(const SUIParameterDesc* pArrayDesc, const SUIArguments& values, const SUIMovieClipDesc* pTmplDesc)
{
	...
	{
		i32k valueCount = values.GetArgCount();
...
//------------------------------------------------------------------------------------
bool CFlashUIElement::GetArray(tukk pArrayName, SUIArguments& valuesOut, tukk pTmplName)
{
	const SUIParameterDesc* pArrayDesc = GetOrCreateArrayDesc(pArrayName);
	const SUIMovieClipDesc* pTmplDesc = pTmplName ? GetMovieClipDesc(pTmplName) : NULL;
...
bool CFlashUIElement::GetArray(const SUIParameterDesc* pArrayDesc, SUIArguments& valuesOut, const SUIMovieClipDesc* pTmplDesc)
{
	...
	if (pInfo->pObj->IsArray())
	{
		i32k size = pInfo->pObj->GetArraySize();
		SFlashVarValue val = SFlashVarValue::CreateUndefined();
...
//------------------------------------------------------------------------------------
bool CFlashUIElement::CreateArray(const SUIParameterDesc*& pNewDesc, tukk arrayName, const SUIArguments& values, tukk pTmplName)
{
	const SUIMovieClipDesc* pTmplDesc = pTmplName ? GetMovieClipDesc(pTmplName) : NULL;
...
//------------------------------------------------------------------------------------
bool CFlashUIElement::CreateArray(const SUIParameterDesc*& pNewDesc, tukk arrayName, const SUIArguments& values, const SUIMovieClipDesc* pTmplDesc)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
	...
	const SUIMovieClipDesc* pParentDesc = pTmplDesc ? pTmplDesc : m_pRoot;
	DRX_ASSERT_MESSAGE(pParentDesc && arrayName, "Invalid parent passed!");
	if (!pParentDesc)
		return false;
	...
	if (pParent)
	{
		...
		DRX_ASSERT_MESSAGE(ok, "Failed to create new flash array!");
...
//------------------------------------------------------------------------------------
void CFlashUIElement::LoadTexIntoMc(tukk movieClip, ITexture* pTexture, tukk pTmplName)
{
...
//------------------------------------------------------------------------------------
void CFlashUIElement::UnloadTexFromMc(tukk movieClip, ITexture* pTexture, tukk pTmplName)
{
	const SUIParameterDesc* pMovieClipDesc = GetOrCreateMovieClipDesc(movieClip);
	const SUIMovieClipDesc* pTmplDesc = pTmplName ? GetMovieClipDesc(pTmplName) : NULL;
...
{
	DRX_ASSERT_MESSAGE(pDynTexture, "NULL pointer passed!");
	if (pDynTexture)
	{
		...
		DRX_ASSERT_MESSAGE(ok, "Texture already registered!");
...
void CFlashUIElement::RemoveTexture(IDynTextureSource* pDynTexture)
{
	...
	if (pDynTexture)
	{
		...
		DRX_ASSERT_MESSAGE(ok, "Texture was never registered or already unregistered!");
	}
}

//------------------------------------------------------------------------------------
void CFlashUIElement::SendCursorEvent(SFlashCursorEvent::ECursorState evt, i32 iX, i32 iY, i32 iButton /*= 0*/, float fWheel /*= 0.f*/)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
	...
	if (HasExtTexture() && gEnv->pRenderer)
	{
		i32 x = 0;
		i32 y = 0;
		i32 width  = gEnv->pRenderer->GetOverlayWidth();
		i32 height = gEnv->pRenderer->GetOverlayHeight();

		float fX = (float) iX / (float) width;
		float fY = (float) iY / (float) height;
		float aspect;
		m_pFlashplayer->GetViewport(x, y, width, height, aspect);
		iX = (i32) (fX * (float)width);
		iY = (i32) (fY * (float)height);
	}
...
void CFlashUIElement::SendControllerEvent(EControllerInputEvent event, EControllerInputState state, float value)
{
	...
	if (m_pFlashplayer->IsAvailable("drx_onController"))
	{
		HideCursor();

		SFlashVarValue args[] = { SFlashVarValue((i32) event), SFlashVarValue(state == eCIS_OnRelease), SFlashVarValue(value) };
		m_pFlashplayer->Invoke("drx_onController", args, 3);
		m_pFlashplayer->Advance(0);
	}
	ForceLazyUpdateInl();
}
...
//-------------------------------------------------------------------
void CFlashUIElement::GetMemoryUsage(IDrxSizer* s) const
{
...
//------------------------------------------------------------------------------------
void CFlashUIElement::HandleFSCommand(tukk sCommand, tukk sArgs, uk pUserData)
{
	DRX_PROFILE_FUNCTION(PROFILE_ACTION);
...
//------------------------------------------------------------------------------------
bool CFlashUIElement::HandleInternalCommand(tukk sCommand, const SUIArguments& args)
{
	if (strcmp(sCommand, "drx_hideElement") == 0)
	{
		UIACTION_LOG("%s (%i): UIElement received \"drx_hideElement\" from ActionScript", GetName(), m_iInstanceID);
		SetVisible(false);
		return true;
	}
	if (strcmp(sCommand, "drx_virtualKeyboard") == 0)
	{
		UIACTION_LOG("%s (%i): UIElement received \"drx_virtualKeyboard\" from ActionScript with args %s", GetName(), m_iInstanceID, args.GetAsString());
		string mode;
		string utf8Title;
		...
		if (ok && utf8Title[0] == '@') // this is a localization label (label name stored in utf8Title)
		{
			...
			if (gEnv->pSystem->GetLocalizationUpr()->LocalizeLabel(utf8Title.c_str(), utf8Localized))
			{
				utf8Title.swap(utf8Localized);
			}
		}

		DRX_ASSERT_MESSAGE(ok, "drx_virtualKeyboard received with wrong arguments!");
		if (ok)
		{
			u32 flags = IPlatformOS::KbdFlag_Default;
		...
		{
			UIACTION_ERROR("%s (%i): drx_virtualKeyboard received with wrong arguments! Args: %s", GetName(), m_iInstanceID, args.GetAsString());
		}
		return true;
	}
	if (strcmp(sCommand, "drx_imeFocus") == 0)
	{
		UIACTION_LOG("%s (%i): UIElement received \"drx_imeFocus\" from ActionScript", GetName(), m_iInstanceID);
...
void CFlashUIElement::KeyboardCancelled()
{
	...
	if (m_pFlashplayer->IsAvailable("drx_onVirtualKeyboard"))
	{
		SUIArguments args;
		args.AddArgument(false);
		args.AddArgument("");
		m_pFlashplayer->Invoke("drx_onVirtualKeyboard", args.GetAsList(), args.GetArgCount());
	}
}

//------------------------------------------------------------------------------------
void CFlashUIElement::KeyboardFinished(tukk pInString)
{
	if (!m_pFlashplayer)
		return;
...
//------------------------------------------------------------------------------------
const SUIParameterDesc* CFlashUIElement::GetOrCreateVariableDesc(tukk pVarName, bool* bExist /*= NULL*/)
{
...
//------------------------------------------------------------------------------------
const SUIParameterDesc* CFlashUIElement::GetOrCreateArrayDesc(tukk pArrayName, bool* bExist /*= NULL*/)
{
...
//------------------------------------------------------------------------------------
const SUIMovieClipDesc* CFlashUIElement::GetOrCreateMovieClipDesc(tukk pMovieClipName, bool* bExist /*= NULL*/)
{
...
//------------------------------------------------------------------------------------
const SUIMovieClipDesc* CFlashUIElement::GetOrCreateMovieClipTmplDesc(tukk pMovieClipTmplName)
{
...
//------------------------------------------------------------------------------------
const SUIEventDesc* CFlashUIElement::GetOrCreateFunctionDesc(tukk pFunctionName)
{
...
CFlashUIElement::SFlashObjectInfo* CFlashUIElement::GetFlashVarObj(const SUIParameterDesc* pDesc, const SUIMovieClipDesc* pTmplDesc)
{
	...
	if (pDesc && !pVarInfo)
	{
		...
		TVarMap::iterator it = m_variableObjects.find(CDrxName(sInstance.c_str()));
		...
		else
		{
			...
			i32k count = path.GetArgCount();
			IFlashVariableObject* pParent = m_variableObjects[CDrxName(m_pRoot->sName)].pObj;
			IFlashVariableObject* pVar = NULL;
			for (i32 i = 1; i < count && pParent; ++i)
			{
				string member;
				...
				it = m_variableObjects.find(CDrxName(currPathToObj.c_str()));
				if (it != m_variableObjects.end())
				{
				...
				else
				{
					...
					if (pVar)
					{
						pCurrVarInfo = &m_variableObjects[CDrxName(currPathToObj.c_str())];
...
void CFlashUIElement::RemoveFlashVarObj(const SUIParameterDesc* pDesc)
{
	...
	tukk sInstanceStr = instance.c_str();
	i32 lenInstance = instance.length();
	for (TVarMap::iterator it = m_variableObjects.begin(); it != m_variableObjects.end(); ++it)
	{
		tukk instStr = it->first.c_str();
		i32 len = it->first.length();
	...
	DRX_ASSERT_MESSAGE(!iterToDelete.empty(), "The given ParameterDescription is not valid!");
	for (TDelList::iterator it = iterToDelete.begin(); it != iterToDelete.end(); ++it)
	{
		...
		if (it->second.first)
		{
			...
			DRX_ASSERT_MESSAGE(ok, "no lookup for this object!");
		}
...
//------------------------------------------------------------------------------------
void CFlashUIElement::AddEventListener(IUIElementEventListener* pListener, tukk name)
{
...
void CFlashUIElement::UpdateViewPort()
{
	...
	i32 posX, posY, width, height, dx, dy;
	...
	else if (m_constraints.eType == SUIConstraints::ePT_Dynamic)
	{
		if (m_constraints.bScale)
		{
			...
			width = fitToHeight ? (i32) (dy * hudAspectRatio) : dx;
			height = !fitToHeight ? (i32) (dx / hudAspectRatio) : dy;
			;
			...
			case SUIConstraints::ePA_Mid:
				posX = fitToHeight ? (i32) ((dx - dy * hudAspectRatio) / 2.f) : 0;
				break;
			case SUIConstraints::ePA_Upper:
				posX = fitToHeight ? (i32) (dx - dy * hudAspectRatio) : 0;
				break;
			}
			...
			case SUIConstraints::ePA_Lower:
				...
				break;
			case SUIConstraints::ePA_Mid:
				posY = !fitToHeight ? (i32) ((dy - dx / hudAspectRatio) / 2.f) : 0;
				break;
			case SUIConstraints::ePA_Upper:
				posY = !fitToHeight ? (i32) (dy - dx / hudAspectRatio) : 0;
				break;
			}
...
//------------------------------------------------------------------------------------
void CFlashUIElement::GetViewPort(i32& x, i32& y, i32& width, i32& height, float& aspectRatio)
{
	if (m_pFlashplayer)
	{
