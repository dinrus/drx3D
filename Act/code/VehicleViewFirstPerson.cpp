...
tukk CVehicleViewFirstPerson::m_name = "FirstPerson";
...
void CVehicleViewFirstPerson::OnStartUsing(EntityId passengerId)
{
	...
	if (!m_sCharacterBoneName.empty())
	{
		...
		if (pCharacter)
		{
			...
			u32 id = rIDefaultSkeleton.GetJointIDByName(m_sCharacterBoneName);
...
void CVehicleViewFirstPerson::OnStopUsing()
{
	...
	{
		if (IItem* pItem = CDrxAction::GetDrxAction()->GetIItemSystem()->GetItem(weaponId))
		{
...
void CVehicleViewFirstPerson::UpdateView(SViewParams& viewParams, EntityId playerId)
{
	...
	IActor* pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(playerId);
...
Quat CVehicleViewFirstPerson::GetVehicleRotGoal()
{
	...
	else
	{
		...
		if (!m_sCharacterBoneName.empty())
		{
			...
			{
				IDefaultSkeleton& rIDefaultSkeleton = pCharacter->GetIDefaultSkeleton();
				u32k id = rIDefaultSkeleton.GetJointIDByName(m_sCharacterBoneName);
...
void CVehicleViewFirstPerson::HideEntitySlots(IEntity* pEnt, bool hide)
{
	...
	if (hide)
	{
		for (i32 i = 0; i < pEnt->GetSlotCount(); ++i)
		{
			if (pEnt->IsSlotValid(i) && pEnt->GetSlotFlags(i) & ENTITY_SLOT_RENDER)
			{
				...
				m_slotFlags.insert(std::pair<EntityId, i32>(pEnt->GetId(), i));
			}
		}

		// hide all children
		for (i32 i = 0; i < pEnt->GetChildCount(); ++i)
...
//------------------------------------------------------------------------
void CVehicleViewFirstPerson::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
