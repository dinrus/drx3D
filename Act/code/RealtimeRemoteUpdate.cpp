...
/*************************************************************************
   ...
   $DateTime$
   Описание:	This is the source file for the module Realtime remote
...
// Should CERTAINLY be moved to DrxCommon.
...
//////////////////////////////////////////////////////////////////////////
void CRealtimeRemoteUpdateListener::OnNotificationNetworkReceive(ukk pBuffer, size_t length)
{
...
void CRealtimeRemoteUpdateListener::LoadMaterials(XmlNodeRef& root)
{
	...
	for (i32 i = 0; i < root->getChildCount(); i++)
	{
		...
		{
			tukk mtlName = mtlNode->getAttr("name");
			gEnv->p3DEngine->GetMaterialUpr()->LoadMaterialFromXml(mtlName, mtlNode);
		}
	}
}
//////////////////////////////////////////////////////////////////////////
void CRealtimeRemoteUpdateListener::LoadConsoleVariables(XmlNodeRef& root)
{
	...
	// Remove Entities with ID`s from the list.
	for (i32 i = 0; i < root->getNumAttributes(); ++i)
	{
		root->getAttributeByIndex(i, (tukk*)&szKey, (tukk*)&szValue);
...
void CRealtimeRemoteUpdateListener::LoadParticles(XmlNodeRef& root)
{
	...
	for (nCurrentChild = 0; nCurrentChild < nNumberOfChildren; ++nCurrentChild)
	{
		...
		gEnv->pParticleUpr->LoadLibrary((tukk)strLibraryName, oLibrary, true);
	}
}
//////////////////////////////////////////////////////////////////////////
void CRealtimeRemoteUpdateListener::LoadTerrainLayer(XmlNodeRef& root, u8* uchData)
{
	i32 texId(0);
	i32 posx(0), posy(0);
	i32 w(0), h(0);
...
void CRealtimeRemoteUpdateListener::Update()
{
	while (!m_ProcessingQueue.empty())
	{
		...
		tuk const szBuffer = (tuk)&(pCurrentBuffer->front());
		const size_t nStringSize = strlen(szBuffer) + 1;
		u8* const chBinaryBuffer = (u8*)(szBuffer + nStringSize);
		...
		if (oSyncType.compare("EngineTerrainData") == 0)
		{
			...
			if (nBinaryDataSize > 0)
			{
				if (ITerrain* piTerrain = gEnv->p3DEngine->GetITerrain())
				{
					...
					u8* szData = new u8[nBinaryDataSize];
					...
					piTerrain->SetCompiledData((u8*)szData + sizeof(SHotUpdateInfo), nBinaryDataSize - sizeof(SHotUpdateInfo), &pStatObjTable, &pMatTable, true, pExportInfo);
		...
		else if (oSyncType.compare("EngineIndoorData") == 0)
		{
			if (nBinaryDataSize > 0)
			{
				if (IVisAreaUpr* piIVisAreaUpr = gEnv->p3DEngine->GetIVisAreaUpr())
				{
					size_t nUncompressedBinarySize(nBinaryDataSize);
					u8* szData = new u8[nBinaryDataSize];
					...
					if (piIVisAreaUpr)
						piIVisAreaUpr->SetCompiledData((u8*)szData + sizeof(SHotUpdateInfo), nBinaryDataSize - sizeof(SHotUpdateInfo), &pStatObjTable, &pMatTable, true, pExportInfo);
		...
		else if (oSyncType.compare("Particle.Library") == 0)
		{
			...
			if (oChildRootNode)
			{
				...
				gEnv->pParticleUpr->LoadEffect(szEffectName, oEffectNode, true);
			}
		}
		...
		else if (oSyncType.compare("GeometryList") == 0)
		{
			...
			tukk szAttributeValue(NULL);
			tukk szAttributeName(NULL);
