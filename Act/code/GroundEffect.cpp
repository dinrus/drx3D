...
CGroundEffect::~CGroundEffect()
{
	...
	if (m_raycastID != 0)
	{
		CDrxAction::GetDrxAction()->GetPhysicQueues().GetRayCaster().Cancel(m_raycastID);
		m_raycastID = 0;
	}
}
...
{
	DRX_ASSERT(speed >= 0.f);

	m_interpSpeed = speed;
}

void CGroundEffect::SetFlags(i32 flags)
{
	m_flags = flags;
	m_active = false;
}

i32 CGroundEffect::GetFlags() const
{
	return m_flags;
}

bool CGroundEffect::SetParticleEffect(tukk pName)
{
	if (pName)
	{
		m_pParticleEffect = gEnv->pParticleUpr->FindEffect(pName);
	}
	else
	{
		m_pParticleEffect = NULL;
	...
	if (DebugOutput())
	{
		DrxLog("[GroundEffect] <%s> set particle effect to (%s)", m_pEntity->GetName(), m_pParticleEffect->GetName());
	}

	return true;
}

void CGroundEffect::SetInteraction(tukk pName)
{
	m_interaction = pName;
}
...
void CGroundEffect::Update()
{
	if (!m_stopped)
	{
		...
		i32 objTypes = ent_static | ent_terrain | ent_rigid | ent_sleeping_rigid;

		i32 flags = rwi_colltype_any | rwi_ignore_noncolliding | rwi_stop_at_pierceable;
		...
		if (DeferredRayCasts())
		{
			if (m_raycastID == 0)
			{
				...
				m_raycastID = CDrxAction::GetDrxAction()->GetPhysicQueues().GetRayCaster().Queue(RayCastRequest::HighPriority, RayCastRequest(rayPos, Vec3(0.0f, 0.0f, -m_height), objTypes, flags, &skipList[0], skipList.size()), functor(*this, &CGroundEffect::OnRayCastDataReceived));
			}
		}
		else
		{
		...
		if ((m_rayWorldIntersectSurfaceIdx != m_surfaceIdx) && !m_interaction.empty())
		{
			...
			if (DebugOutput())
			{
				tukk pMaterialName = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceType(m_rayWorldIntersectSurfaceIdx)->GetName();

				DrxLog("[GroundEffect] <%s> GetEffectString for matId %i (%s) returned <%s>", m_pEntity->GetName(), m_rayWorldIntersectSurfaceIdx, pMaterialName, pEffectName ? pEffectName : "null");
			}
		...
		if ((m_active != prevActive) || newEffect)
		{
			if (m_active && m_pParticleEffect)
			{
				...
				if (DebugOutput())
				{
					DrxLog("[GroundEffect] <%s> effect %s loaded to slot %i", m_pEntity->GetName(), m_pParticleEffect->GetName(), m_slot);
				}
			}
			else
			{
				if (DebugOutput())
				{
					DrxLog("[GroundEffect] <%s> slot %i freed", m_pEntity->GetName(), m_slot);
				}
		...
		if (DebugOutput())
		{
			...
			tukk pEffectName = m_pParticleEffect ? m_pParticleEffect->GetName() : "";
...
{
	DRX_ASSERT_MESSAGE(rayID == m_raycastID, "CGroundEffect: Received raycast data with mismatching id");
