...
tukk CPersistantDebug::entityTagsContext = "PersistantDebugEntities";
const float CPersistantDebug::kUnlimitedTime = -1.0f;

void CPersistantDebug::AddEntityTag(const SEntityTagParams& params, tukk tagContext)
{
	// Create tag
	SEntityTag tag;
	tag.params = params;
	...
	{
		obj->timeRemaining = tag.totalTime < 0 ? kUnlimitedTime : max(obj->timeRemaining, tag.totalTime);
		obj->totalTime = obj->timeRemaining;
		i32 size = max(i32(obj->columns.size()), params.column);
	...
	if (m_pETLog->GetIVal() > 0)
	{
		...
		if (ent)
		{
			DrxLog("[Entity Tag] %s added tag: %s", ent->GetName(), params.text.c_str());

			if (m_pETLog->GetIVal() > 1)
			{
				char text[256];
				drx_sprintf(text, "[Entity Tag] %s", params.text.c_str());
...
void CPersistantDebug::ClearStaticTag(EntityId entityId, tukk staticId)
{
	SObj* obj = FindObj(entityId);
	if (obj && staticId)
	{
...
void CPersistantDebug::ClearTagContext(tukk tagContext)
{
	if (!tagContext)
		return;
...
void CPersistantDebug::ClearTagContext(tukk tagContext, EntityId entityId)
{
	if (!tagContext)
		return;
...
void CPersistantDebug::UpdateTags(float frameTime, SObj& obj, bool doFirstPass)
{
	if (!doFirstPass)
	{
		...
		for (i32 i = 0; i < obj.columns.size(); ++i)
	...
	for (TListTag::iterator iterList = obj.tags.begin(); iterList != obj.tags.end(); ++iterList)
	{
		...
		if (doFirstPass)
		{
			i32 pos(iterList->params.column - 1);
		...
		else
		{
			...
			{
				i32 centerLine = obj.columns.size() / 2;
				if (iterList->params.column <= centerLine)
					for (i32 i = iterList->params.column - 1; i < centerLine; ++i)
						screenPos.x -= obj.columns[i].width;
				else
					for (i32 i = centerLine; i < iterList->params.column - 1; ++i)
						screenPos.x += obj.columns[i].width;
