...
#include <drx3D/plugins/DrxDefaultEntities/DefaultComponents/Audio/ListenerComponent.h>
...
{
	if (!Drx::pCamShakeMult)
	{
		Drx::pCamShakeMult = gEnv->pConsole->GetCVar("c_shakeMult");
	}
	if (!Drx::pHmdTrackingOrigin)
	{
		Drx::pHmdTrackingOrigin = gEnv->pConsole->GetCVar("hmd_tracking_origin");
	}
...
void CView::Update(float frameTime, bool isActive)
{
	...
	if (pLinkedTo || pEntity)
	{
		...
		if (pLinkedTo || m_linkedEntityCallback)
		{
			...
			if (!m_viewParams.position.IsValid())
			{
				...
				DRX_ASSERT_MESSAGE(0, "Camera position is invalid, reverting to old position");
			}
			if (!m_viewParams.rotation.IsValid())
			{
				m_viewParams.rotation = m_viewParams.GetRotationLast();
				DRX_ASSERT_MESSAGE(0, "Camera rotation is invalid, reverting to old rotation");
			}
		}
		else
		{
		...
		if (pHmdUpr)
		{
			pHmdDevice = pHmdUpr->GetHmdDevice();
			if (pHmdDevice)
			{
		...
		if (pLinkedTo)
		{
			...
			{
				i32k slotIndex = 0;
				u32 entityFlags = pLinkedToEntity->GetSlotFlags(slotIndex);
		...
		else if (pHmdDevice && bHmdTrackingEnabled)
		{
			...
			if (Drx::pHmdTrackingOrigin && Drx::pHmdTrackingOrigin->GetIVal() == (i32)EHmdTrackingOrigin::Seated)
			{
	...
	else
	{
		...
		if (!pDrxAction->IsGameSessionMigrating())    // If we're host migrating, leave the camera where it was
		{
			...
			if (!pDrxAction->GetIGameSessionHandler()->IsMidGameLeaving())
			{
...
//------------------------------------------------------------------------
void CView::SetViewShake(Ang3 shakeAngle, Vec3 shakeShift, float duration, float frequency, float randomness, i32 shakeID, bool bFlipVec, bool bUpdateOnly, bool bGroundOnly)
{
	SShakeParams params;
...
void CView::SetViewShakeEx(const SShakeParams& params)
{
	...
	i32 shakes(m_shakes.size());
	SShake* pSetShake(nullptr);

	for (i32 i = 0; i < shakes; ++i)
	{
...
{
	DRX_ASSERT_MESSAGE(scale == 1.0f || m_scale == 1.0f, "Attempting to CView::SetScale but has already been set!");
	m_scale = scale;
}

void CView::SetZoomedScale(const float scale)
{
	DRX_ASSERT_MESSAGE(scale == 1.0f || m_zoomedScale == 1.0f, "Attempting to CView::SetZoomedScale but has already been set!");
	m_zoomedScale = scale;
}
...
{
	float shakeMult(Drx::pCamShakeMult->GetFVal());
...
void CView::GetRandomQuat(Quat& quat, SShake* pShake)
{
	...
	quat *= Quat::CreateRotationXYZ(Ang3(drx_random(-r, r), drx_random(-r, r), drx_random(-r, r)));
}
...
void CView::GetRandomVector(Vec3& vec, SShake* pShake)
{
	...
	len /= 3.f;
	float r = len * randomAmt;
	vec += Vec3(drx_random(-r, r), drx_random(-r, r), drx_random(-r, r));
}
...
void CView::ProcessShakeNormal_DoShaking(SShake* pShake, float frameTime)
{
	...
	if (pShake->nextShake <= 0.0f)
	{
		...
		len /= 3.0f;
		float r = len * randomAmt;
		pShake->goalShake *= Quat::CreateRotationXYZ(Ang3(drx_random(-r, r), drx_random(-r, r), drx_random(-r, r)));

		//translational randomization
		len = fabs(pShake->amountVector.x) + fabs(pShake->amountVector.y) + fabs(pShake->amountVector.z);
		len /= 3.0f;
		r = len * randomAmt;
		pShake->goalShakeVector += Vec3(drx_random(-r, r), drx_random(-r, r), drx_random(-r, r));
...
//------------------------------------------------------------------------
void CView::StopShake(i32 shakeID)
{
	u32 num = m_shakes.size();
	for (u32 i = 0; i < num; ++i)
	{
...
void CView::GetMemoryUsage(IDrxSizer* s) const
{
...
void CView::CreateAudioListener()
{
	...
	if (m_pAudioListenerEntity == nullptr && pIEntity != nullptr)
	{
		...
		if (m_pAudioListenerEntity != nullptr)
		{
			...
			m_pAudioListenerComponent = m_pAudioListenerEntity->GetOrCreateComponent<Drx::Audio::DefaultComponents::CListenerComponent>();
			DRX_ASSERT(m_pAudioListenerComponent != nullptr);
		...
		{
			DrxFatalError("<Audio>: AudioListenerEntity creation failed in CView::CreateAudioListener!");
		}
	}
