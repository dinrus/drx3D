...
static i32k RTT_LOOPS = 2; // how many round trips to include in ping averaging
...
CPhysicsSync::CPhysicsSync(CGameChannel* pParent)
	...
	, m_catchup(false)
{
	m_pDebugHistory = CDrxAction::GetDrxAction()->CreateDebugHistoryUpr();
}
...
bool CPhysicsSync::OnPacketHeader(CTimeValue tm)
{
	...
	i32 curTimestamp = m_pWorld->GetiPhysicsTime();
	i32 translatedTimestamp = -1;
...
emergency_reset:
	...
	translatedTimestamp = i32(m_physEpochTimestamp + m_physEstimatedLocalLaggedTime.GetSeconds() / timeGranularity + 0.5);
	if (translatedTimestamp >= curTimestamp)
	{
		m_catchup = false;
		translatedTimestamp = curTimestamp;
		if (translatedTimestamp > curTimestamp)
			DrxLog("[phys] time compression occurred (%d ticks)", curTimestamp - translatedTimestamp);
	}
	else
	{
	...
	//DRX_ASSERT(translatedTimestamp >= 0);
	if (translatedTimestamp < 0)
		translatedTimestamp = 0;
	DRX_ASSERT(translatedTimestamp <= curTimestamp);

	if (!resetEstimate && (curTimestamp - translatedTimestamp) * timeGranularity > 0.25f + m_pingEstimate.GetSeconds())
	{
		DrxLog("[phys] way out of sync (%.2f seconds)... performing emergency reset", float((curTimestamp - translatedTimestamp) * timeGranularity));
		m_catchup = false;
		resetEstimate = true;
	...
	if (!resetEstimate)
	{
		...
		i32 deltaTimestamp = curTimestamp - m_physEpochTimestamp;
		m_physEpochTimestamp = curTimestamp;
		m_epochWhen = gEnv->pTimer->GetAsyncTime();
...
bool CPhysicsSync::OnPacketFooter()
{
	...
	{
		pDbg = CDrxAction::GetDrxAction()->GetIPersistantDebug();
	}
