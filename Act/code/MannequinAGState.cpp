...
// ----------------------------------------------------------------------------
void CMannequinAGState::SetAnimatedCharacter(class CAnimatedCharacter* animatedCharacter, i32 layerIndex, IAnimationGraphState* parentLayerState)
{
...
// ----------------------------------------------------------------------------
bool CMannequinAGState::SetInput(InputID iid, tukk value, TAnimationGraphQueryID* pQueryID /*= 0 */)
{
	return SetInputInternal(iid, value, pQueryID, false);
}

// ----------------------------------------------------------------------------
bool CMannequinAGState::SetInputOptional(InputID iid, tukk value, TAnimationGraphQueryID* pQueryID /*= 0 */)
{
	return SetInputInternal(iid, value, pQueryID, true);
}

// ----------------------------------------------------------------------------
void CMannequinAGState::GetInput(InputID iid, tuk value) const
{
	GetInput(iid, value, 0);
}

// ----------------------------------------------------------------------------
void CMannequinAGState::GetInput(InputID iid, tuk value, i32 layerIndex) const
{
	value[0] = '\0';

	DRX_ASSERT(layerIndex == 0);
	if (layerIndex != 0)
		return;
...
// ----------------------------------------------------------------------------
tukk CMannequinAGState::GetInputName(InputID iid) const
{
...
// ----------------------------------------------------------------------------
tukk CMannequinAGState::GetVariationInputName(InputID) const
{
	// Only called from CAnimationGraphStates::RebindInputs
	return NULL;
}
...
// ----------------------------------------------------------------------------
void CMannequinAGState::AddListener(tukk name, IAnimationGraphStateListener* pListener)
{
	SListener listener;
	drx_strcpy(listener.name, name);
...
// ----------------------------------------------------------------------------
AnimationGraphInputID CMannequinAGState::GetInputId(tukk input)
{
...
// ----------------------------------------------------------------------------
AnimationGraphInputID CMannequinAGState::GetVariationInputId(tukk variationInputName) const
{
	DRX_ASSERT(false);
	return InputID(-1);
}
...
// ----------------------------------------------------------------------------
tukk CMannequinAGState::GetCurrentStateName()
{
	tukk result = "<unknown>";
	...
	for (i32 scopeIndex = 0; scopeIndex < pActionController->GetTotalScopes(); ++scopeIndex)
	{
...
// ----------------------------------------------------------------------------
IAnimationGraphExistanceQuery* CMannequinAGState::CreateExistanceQuery(i32 layer)
{
	DRX_ASSERT(layer == 0);
	if (layer != 0)
		return NULL;
...
// ----------------------------------------------------------------------------
void CMannequinAGState::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
...
// ----------------------------------------------------------------------------
bool CMannequinAGState::SetInputInternal(InputID iid, tukk value, TAnimationGraphQueryID* pQueryID, bool optional)
{
	switch (iid)
	{
	case eSIID_Action:
	...
	default:
		{
			DRX_ASSERT(false); // trying to set input with non-existing input id
			return false;
		}
	}
	;
...
// ----------------------------------------------------------------------------
bool CMannequinAGState::SetActionOrSignalInput(_smart_ptr<CAnimActionAGAction>& pAction, TKeyValue& currentValue, InputID inputID, EAIActionType actionType, tukk defaultValue, u32 defaultValueCRC, tukk value, TAnimationGraphQueryID* pQueryID, bool optional)
{
	IActionController* pActionController = GetActionController();
	if (!pActionController)
		return false;
	...
	i32k MANNEQUIN_PRIORITY = 2; // currently equal to PP_Action, just above movement, but underneath urgent actions, hit death, etc.
	...
	static u32 idleCRC = CCrc32::ComputeLowercase("idle");
	static u32 noneCRC = CCrc32::ComputeLowercase("none");
	...
	else
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CMannequinAGState::SetInput: Unable to find animation '%s'", value);
		}
...
{
	DRX_ASSERT(m_pCallerOfSendEvent == NULL); // paranoia: check for weird re-entry case
...
void CMannequinAGState::ResetInputValues()
{
	...
	for (size_t i = 0; i < eSIID_COUNT; i++)
	{
		SendEvent_ChangedInput(static_cast<MannequinAG::CMannequinAGState::InputID>(i), false);
	}
...
// ----------------------------------------------------------------------------
CAnimActionAGAction::CAnimActionAGAction(i32 priority, FragmentID fragmentID, CMannequinAGState& mannequinAGState, EAIActionType type, const string& value, u32 valueCRC, TAnimationGraphQueryID queryID, bool skipIntro)
...
{
	for (i32 i = 0; i < m_leaveQueryIDs.size(); ++i)
	{
