...
//------------------------------------------------------------------------
bool CVehiclePartTread::Init(IVehicle* pVehicle, const CVehicleParams& table, IVehiclePart* parent, CVehicle::SPartInitInfo& initInfo, i32 partType)
{
	...
	if (subTable.haveAttr("materialName"))
	{
...

		{
			...
			if (pMaterial)
			{
				...
				{
					for (i32 i = 0; i < pMaterial->GetSubMtlCount(); ++i)
					{
		...
		{
			// clone
			IMaterial* pCloned = pMaterial->GetMaterialUpr()->CloneMultiMaterial(pMaterial, subMtlName);
			if (pCloned)
			{
	...
	{
		tukk boneName = rIDefaultSkeleton.GetJointNameByID(i);
		if (0 != boneName)
		{
			// extract wheel name
			...
			{
				drx_strcpy(wheelName, boneName, len);
...
//------------------------------------------------------------------------
bool CVehiclePartTread::ChangeState(EVehiclePartState state, i32 flags /* =0 */)
{
