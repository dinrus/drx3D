...
public:
	...
	static i32k NORMAL_ACTION_PRIORITY = 3;   // currently equal to PP_Action, just above movement, but underneath urgent actions, hit death, etc.
	static i32k URGENT_ACTION_PRIORITY = 4;   // currently equal to PP_Urgent_Action, just above normal actions, but underneath hit death, etc.
...
private:
	...
	void QueueAction(const UpdateContext& context)
	{
		...
		IActor* actor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(context.entityId);
