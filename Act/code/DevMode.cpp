...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:	Вспомогательный класс для реализации функциональности
                CDrxAction в режиме "только разработчик".
...
void CDevMode::GotoTagPoint(i32 i)
{
...
void CDevMode::SaveTagPoint(i32 i)
{
	std::vector<STagFileEntry> tags = LoadTagFile();
	if (tags.size() <= size_t(i))
		tags.resize(i + 1);
...
void CDevMode::OnGameplayCommand(tukk cmd)
{
	SUIArguments args;
	args.SetDelimiter(":");
...
bool CDevMode::OnInputEvent(const SInputEvent& evt)
{
	...
	bool handled = false;
	bool canCheat = CDrxAction::GetDrxAction()->CanCheat();
	...
	if (canCheat)
	{
		...
		else if (!handled && (evt.state == eIS_Pressed) && !(evt.modifiers & eMM_Modifiers))
		{
			...
			{
				CDrxAction::GetDrxAction()->GetIItemSystem()->GetIEquipmentUpr()->GiveEquipmentPack(pActor, "Player_Default", true, true);
			}
...
{
	DrxSetFileAttributes(TagFileName().c_str(), FILE_ATTRIBUTE_NORMAL);
...
void CDevMode::GotoSpecialSpawnPoint(i32 i)
{
	char cmd[256];
	drx_sprintf(cmd, "#g_localActor:SpawnAtSpawnPoint(\"SpawnPoint%d\")", i);
