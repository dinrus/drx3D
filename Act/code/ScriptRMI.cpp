...
{
	/*
	   DrxAutoCriticalSection lkDispatch(m_dispatchMutex);
...
{
	DrxAutoLock<DrxCriticalSection> lock(m_dispatchMutex);
	m_entities.clear();
...
i32 CScriptRMI::ExposeClass(IFunctionHandler* pFH)
{
  ...
  tukk name)
{
	...
	while (methods->MoveNext(iter))
	{
		if (iter.sKey)
		{
			...
			DRX_ASSERT(format.length() <= MaxRMIParameters);
			drx_strcpy(info.format, format.c_str());
			info.funcID = funcID;
...
bool CScriptRMI::BuildSynchTable(SmartScriptTable vars, SmartScriptTable cls, tukk name)
{
		...
		if (iter.sKey)
		{
			...
			if (len > MaxSynchedPropertyNameLength)
			{
				...
				pSS->RaiseError("Synched var name '%s' too long (max is %d)",
				                iter.sKey, (i32)MaxSynchedPropertyNameLength);
				return false;
			}
			SSynchedPropertyInfo info;
			drx_strcpy(info.name, iter.sKey);
...
i32 CScriptRMI::SerializeFunction(IFunctionHandler* pH, uk pBuffer, i32 nSize)
{
	SmartScriptTable serTable;
	ScriptHandle hdl;
	...
	i32 nProperties = nSize / sizeof(SSynchedPropertyInfo);

	if (p->ser.IsReading())
	{
		for (i32 i = 0; i < nProperties; i++)
...
bool CScriptRMI::ValidateDispatchTable(tukk clazz, SmartScriptTable dispatch, SmartScriptTable methods, bool bServerTable)
{
	DrxAutoCriticalSection lkDispatch(m_dispatchMutex);
	...
	if (iterN == m_entityClassToEntityTypeID.end() || iterN->first != clazz)
	{
		...
		DRX_ASSERT(iterN->second == m_dispatch.size());
	...
	while (dispatch->MoveNext(iter))
	{
		...
		else
		{
			...
			tukk funcData = iter.value.GetString();
			tukk colon = strchr(funcData, ':');
			if (colon == NULL)
			{
				dispatch->EndIteration(iter);
				return false;
...
public:
	...
	bool Prologue(EntityId objID, bool bClient, u8 funcID)
	{
	...
	void Prologue(tukk function, tukk format)
	{
		drx_strcpy(m_function, function);
		m_format = format;
	}
	...
	bool        PerformCall(EntityId objID, CScriptRMISerialize* pSer, bool bClient, INetContext* pNetContext, INetChannel* pChannel)
	{
		...
		u16 channelId = pGameServerChannel->GetChannelId();
...
// handle a call
INetAtSyncItem* CScriptRMI::HandleRMI(bool bClient, EntityId objID, u8 funcID, TSerialize ser, INetChannel* pChannel)
{
	if (!m_pParent)
	{
...
void CScriptRMI::OnInitClient(u16 channelId, IEntity* pEntity)
{
...
void CScriptRMI::OnPostInitClient(u16 channelId, IEntity* pEntity)
{
	SmartScriptTable server;
	SmartScriptTable entity = pEntity->GetScriptTable();
...
void CScriptRMI::AddProxyTable(IScriptTable* pEntityTable,
                               ...
                               tukk name, SmartScriptTable dispatchTable)
{
...
void CScriptRMI::DispatchRMI(INetChannel* pChannel, _smart_ptr<CScriptMessage> pMsg, bool bClient)
{
	...
	else
	{
		...
		{
			DRX_ASSERT(!"This should never happen");
			return;
		}
...
// send a call
i32 CScriptRMI::ProxyFunction(IFunctionHandler* pH, uk pBuffer, i32 nSize)
{
	if (!m_pThis->m_pParent)
	{
	...
	i32 formatStart = 2;
	if (u32 flagsMask = flags.n & (eDF_ToClientOnChannel | eDF_ToClientOnOtherChannels))
	{
	...
	CDrxAction* pFramework = m_pThis->m_pParent->GetFramework();
	...
	if (gatherDebugInfo)
	{
		...
		DrxLogAlways("[rmi] %s(%s) [%s] on entity[%d] %s",
		             pH->GetFuncName(), funcInfo.c_str(), dispatchInfo.c_str(), (i32)id.n, name);
	}

	return pH->EndFunction(true);
}

i32 CScriptRMI::SynchedNewIndexFunction(IFunctionHandler* pH)
{
	if (!m_pThis->m_pParent)
	{
...
void CScriptRMI::GetMemoryStatistics(IDrxSizer* s)
{
	DrxAutoCriticalSection lk(m_dispatchMutex);
