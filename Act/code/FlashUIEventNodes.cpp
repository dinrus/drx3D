...
// -------------------------------------------------------------------------
//  Имя файла:   FlashUIEventNodes.cpp
//  Version:     v1.00
...
CFlashUIEventSystemFunctionNode::CFlashUIEventSystemFunctionNode(IUIEventSystem* pSystem, string sCategory, const SUIEventDesc* pEventDesc)
	...
	, m_pSystem(pSystem)
{
	DRX_ASSERT_MESSAGE(m_pSystem && pEventDesc, "NULL pointer passed!");
...
void CFlashUIEventSystemFunctionNode::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_Activate:
		if (IsPortActive(pActInfo, eI_Send))
		{
			...
			i32 end = m_eventDesc.OutputParams.Params.size();
			i32 i = 0;
			for (; i < end; ++i)
			{
				DRX_ASSERT_MESSAGE(i < res.GetArgCount(), "UIEvent returns wrong number of arguments!");
