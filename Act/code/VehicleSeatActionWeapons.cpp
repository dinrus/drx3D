...
{
	if (IGameObject* pGameObject = CDrxAction::GetDrxAction()->GetGameObject(weaponId))
	{
...
bool CVehicleSeatActionWeapons::Init(IVehicle* pVehicle, IVehicleSeat* pSeat, const CVehicleParams& table)
{
	...
	i32 i = 0;
	i32 c = weaponsArrayTable.getChildCount();
	m_weapons.reserve(c);
	...
	for (; i < c; i++)
	{
		if (CVehicleParams weaponTable = weaponsArrayTable.getChild(i))
		{
			...
			if (!className.empty())
			{
				...
				{
					DrxLog("[CVehicleSeatActionWeapons]: EntityClass %s not found!", className.c_str());
					continue;
				}
				...
				{
					i32 k = 0;
					i32 cHelpers = helpersTable.getChildCount();
					helpers.reserve(c);
				...
				if (CVehicleParams actionsTable = weaponTable.findChild("Actions"))
				{
					...
					i32 k = 0;
					i32 numActionsTables = actionsTable.getChildCount();
					...
					for (; k < numActionsTables; k++)
					{
						if (CVehicleParams actionTable = actionsTable.getChild(k))
						{
							...
							{
								i32 n = 0;
								i32 numAnimations = animsTable.getChildCount();
	...
	if (tukk attackInput = weaponsParamsTable.getAttr("attackInput"))
	{
...
IEntity* CVehicleSeatActionWeapons::SpawnWeapon(SVehicleWeapon& weapon, IEntity* pVehicleEntity, const string& partName, i32 weaponIndex)
{
	...
	if ((isServer || bClientOnly) && static_cast<CVehicle*>(m_pVehicle)->SpawnAndDeleteEntities())
	{
		...
		drx_sprintf(pWeaponName, "%s_Weapon_%s_%s_%d",
...
{
	DRX_ASSERT(weapon.weaponEntityId);
...
void CVehicleSeatActionWeapons::StartUsing(EntityId passengerId)
{
	...
	{
		DRX_ASSERT(!"error: another passenger is already linked to this weapon seat action");
	}
...
//------------------------------------------------------------------------
void CVehicleSeatActionWeapons::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
...
void CVehicleSeatActionWeapons::DoUpdate(float frameTime)
{
	...
	for (i32 i = 0; i < m_weapons.size(); ++i)
	{
...
//------------------------------------------------------------------------
SVehicleWeapon& CVehicleSeatActionWeapons::GetWeaponInfo(i32 weaponIndex)
{
...
{
	return CDrxAction::GetDrxAction()->GetIItemSystem()->GetItem(vehicleWeapon.weaponEntityId);
}
...
IVehicleHelper* CVehicleSeatActionWeapons::GetHelper(SVehicleWeapon& vehicleWeapon)
{
	...
	if (!helpers.empty())
	{
		...
		if (pWeapon)
		{
			...
			if (pFireMode)
			{
				i32 index = pFireMode->GetCurrentBarrel();
...
void CVehicleSeatActionWeapons::Serialize(TSerialize ser, EEntityAspects aspects)
{
	...
	{
		char buffer[16];
		i32 n = m_weapons.size();

		for (i32 i = 0; i < n; ++i)
		{
			drx_sprintf(buffer, "weapon_%d", i);
...
void CVehicleSeatActionWeapons::StartFire()
{
	...
	if (!m_isUsingShootingByUpdate)
	{
		...
		if (pWeapon)
		{
			...
			DrxAudio::IAudioSystem const* const pIAudioSystem = gEnv->pAudioSystem;
			...
			if (pMovement != nullptr && pWeapon->CanFire())
			{
				...
				{
					DrxAudio::ControlId audioControlID = DrxAudio::InvalidControlId;
...
//------------------------------------------------------------------------
u32 CVehicleSeatActionWeapons::GetWeaponCount() const
{
...
//------------------------------------------------------------------------
EntityId CVehicleSeatActionWeapons::GetWeaponEntityId(u32 index) const
{
...
//------------------------------------------------------------------------
IItem* CVehicleSeatActionWeapons::GetIItem(u32 index)
{
	if (index < m_weapons.size())
	{
...
//------------------------------------------------------------------------
IWeapon* CVehicleSeatActionWeapons::GetIWeapon(u32 index)
{
	if (index < m_weapons.size())
	{
...
//------------------------------------------------------------------------
void CVehicleSeatActionWeapons::ClSetupWeapon(u32 index, EntityId weaponId)
{
	IItem* pItem = CDrxAction::GetDrxAction()->GetIItemSystem()->GetItem(weaponId);
	if (!pItem)
		return;
...
//------------------------------------------------------------------------
i32 CVehicleSeatActionWeapons::GetSkipEntities(IPhysicalEntity** pSkipEnts, i32 nMaxSkip)
{
	i32 nSkip = 0;
	...
	i32 count = pVehicleEntity->GetChildCount();

	for (i32 c = 0; c < count && nSkip < nMaxSkip; ++c)
...
bool CVehicleSeatActionWeapons::GetProbableHit(EntityId weaponId, const IFireMode* pFireMode, Vec3& hitPos)
{
	...
	i32 nSkip = GetSkipEntities(pSkipEntities, 10);
	...
	if (event == eVE_Destroyed)
	{
		...
		for (TVehicleWeaponVector::iterator ite = m_weapons.begin(); ite != m_weapons.end(); ++ite)
		{
			if (IEntity* pEntity = GetEntity(*ite))
			{
				...
				{
					pAISystem->GetSmartObjectUpr()->SetSmartObjectState(pEntity, "Busy");
				}
				pEntity->Hide(true);
			}
		}
	...
	else if (event == eVE_WeaponRemoved)
	{
		...
		for (; ite != itEnd; ++ite)
		{
			if (ite->weaponEntityId == params.entityId)
			{
				...
				if (childId)
				{
					if (IGameObject* pGameObject = CDrxAction::GetDrxAction()->GetGameObject(childId))
					{
...
void CVehicleSeatActionWeapons::OnWeaponRespawned(i32 weaponIndex, EntityId newWeaponEntityId)
{
	DRX_ASSERT_MESSAGE(weaponIndex < m_weapons.size(), "CVehicleSeatActionWeapons::WeaponRespawned - Invalid weaponIndex!");
...
void CVehicleSeatActionWeapons::GetMemoryUsage(IDrxSizer* s) const
{
	s->Add(*this);
