...
//  Описание: A bunch of FlowNodes that are AI-Sequence-compatible and can only be instantiated from within such one.
...
//////////////////////////////////////////////////////////////////////////
//
// DrxAction_AIActionSequenceFlowNodeBase
//
//////////////////////////////////////////////////////////////////////////
class DrxAction_AIActionSequenceFlowNodeBase    // prefixed with "DinrusAction" to prevent naming clashes in monolithic builds
...
DrxAction_AIActionSequenceFlowNodeBase::DrxAction_AIActionSequenceFlowNodeBase()
{
	m_stopFlowGraphNodeFromGettingUpdatedWhenSequenceActionFinishesOrCancels = false;
}

void DrxAction_AIActionSequenceFlowNodeBase::FinishSequenceActionAndActivateOutputPort(i32 port)
{
...
void DrxAction_AIActionSequenceFlowNodeBase::CancelSequenceAndActivateOutputPort(i32 port)
{
...
void DrxAction_AIActionSequenceFlowNodeBase::SequenceActionFinishedOrCanceled()
{
...
class CFlowNode_AISequenceAction_ApproachAndEnterVehicle
	: public DrxAction_AIActionSequenceFlowNodeBase
...
void CFlowNode_AISequenceAction_ApproachAndEnterVehicle::GetConfiguration(SFlowNodeConfig& config)
{
	static const SInputPortConfig inputPortConfig[] =
	{
		...
		InputPortConfig<i32>("Speed",         _HELP("Speed to approach the vehicle with"),       NULL,      _UICONFIG("enum_int:Walk=0,Run=1,Sprint=2")),
		InputPortConfig<i32>("Stance",        _HELP("Stance while approaching the vehicle"),     NULL,      _UICONFIG("enum_int:Relaxed=0,Alert=1,Combat=2")),
	...
	case eFE_Activate:
		{
			...
			if (IsPortActive(pActInfo, InputPort_Start))
			{
				...
				{
					gEnv->pAISystem->GetSequenceUpr()->RequestActionStart(assignedSequenceId, pActInfo->myID);
...
void CFlowNode_AISequenceAction_ApproachAndEnterVehicle::GetMemoryUsage(IDrxSizer* sizer) const
{
	sizer->Add(*this);
}
	...
	case AIActionSequence::StartAction:
		{
			...
			if (!pVehicle)
			{
				DrxLog("Actor %s failed to enter vehicle (specified vehicle not found or its crew is hostile towards the actor returned true)", m_actInfo.pEntity->GetName());
				CancelSequenceAndActivateOutputPort(OutputPort_Done);
				return;
			}
			...
			if (!pSeat)
			{
				DrxLog("Actor %s failed to enter vehicle (bad seat number provided: %i)", m_actInfo.pEntity->GetName(), m_seatNumber);
				CancelSequenceAndActivateOutputPort(OutputPort_Done);
				return;
			}
			...
			if (!pEnterHelper)
			{
				DrxLog("Actor %s failed to enter vehicle (vehicle has no enter-helper)", m_actInfo.pEntity->GetName());
				CancelSequenceAndActivateOutputPort(OutputPort_Done);
				return;
			}
			...
			{
				DRX_ASSERT_MESSAGE(0, "no compatible entity was provided");
				DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_WARNING, "Actor %s failed to enter vehicle (no compatible entity was provided)", m_actInfo.pEntity->GetName());
				CancelSequenceAndActivateOutputPort(OutputPort_Done);
			}
		}
		break;
...
void CFlowNode_AISequenceAction_VehicleRotateTurret::GetMemoryUsage(IDrxSizer* sizer) const
{
	sizer->Add(*this);
}
	...
	case AIActionSequence::StartAction:
		{
			...
			if (!pVehicle)
			{
				DRX_ASSERT_MESSAGE(0, "agent is not linked to a vehicle");
				DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_WARNING, "Actor %s is not linked to a vehicle", m_actInfo.pEntity->GetName());
				CancelSequenceAndActivateOutputPort(OutputPort_Done);
				return;
			}
			...
			if (!pSeat)
			{
				DRX_ASSERT_MESSAGE(0, "agent is not sitting in the vehicle it is linked to");
				DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_WARNING, "Actor %s is not sitting in the vehicle it is linked to", m_actInfo.pEntity->GetName());
				CancelSequenceAndActivateOutputPort(OutputPort_Done);
				return;
			}
			...
			if (!m_pActionRotateTurret)
			{
				DRX_ASSERT_MESSAGE(0, "a CVehicleSeatActionRotateTurret is not provided by the vehicle or someone else in the vehicle has reserved that action already.");
				DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_WARNING, "Actor %s could not find a CVehicleSeatActionRotateTurret or that action is already reserved by someone else", m_actInfo.pEntity->GetName());
				CancelSequenceAndActivateOutputPort(OutputPort_Done);
				return;
			}
