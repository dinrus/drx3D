...
#include <drx3D/Act/IDrxMannequinEditor.h>
...
public:
	PROCEDURAL_CONTEXT(CParticleEffectContext, "ParticleEffectContext", "9ced3080-5f88-45a3-f527-229c51b542a8"_drx_guid);
...
DRXREGISTER_CLASS(CParticleEffectContext);
...
CParticleEffectContext::~CParticleEffectContext()
{
	...
	for (u32 i = 0u, sz = m_stopRequests.size(); i < sz; ++i)
	{
...
void CParticleEffectContext::StartEffect(SParticleEffectClipData& data, const SPlayParticleEffectParams& params, EntityId entityId, ICharacterInstance* pCharacterInstance, IActionController& pActionController)
{
	...
	tukk const szEffectName = params.effectName.c_str();
	if (IParticleEffect* const pEffect = gEnv->pParticleUpr->FindEffect(szEffectName, "Particle.SpawnEffect"))
	{
		IAttachment* pAttachment = NULL;

		if (pCharacterInstance)
		{
			IAttachmentUpr& attachmentUpr = *pCharacterInstance->GetIAttachmentUpr();
			tukk szNewAttachmentName = NULL;
			u32k jointCrc = params.jointName.crc;
			i32 attachmentJointId = jointCrc ? pCharacterInstance->GetIDefaultSkeleton().GetJointIDByCRC32(jointCrc) : -1;
			...
			else
			{
				...
				pAttachment = attachmentUpr.GetInterfaceByNameCRC(params.attachmentName.crc);
			...
			if (szNewAttachmentName && attachmentJointId >= 0)
			{
				...
				static u16 s_clonedAttachmentCount = 0;
				++s_clonedAttachmentCount;
				DrxFixedStringT<64> attachmentCloneName;
				attachmentCloneName.Format("%s%s%u", szNewAttachmentName, "FXClone", s_clonedAttachmentCount);

				tukk const pBoneName = pCharacterInstance->GetIDefaultSkeleton().GetJointNameByID(attachmentJointId);
				const IAttachment* const pOriginalAttachment = pAttachment;
				pAttachment = attachmentUpr.CreateAttachment(attachmentCloneName.c_str(), CA_BONE, pBoneName);
				...
				else
				{
					...
					DRX_ASSERT(params.cloneAttachment);
		...
		if (data.pEmitter)
		{
			...
			u32k numListeners = mannequinInterface.GetNumMannequinGameListeners();
			for (u32 itListeners = 0; itListeners < numListeners; ++itListeners)
			{
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CParticleEffectContext: could not load requested effect %s", szEffectName);
	}
}
...
void CParticleEffectContext::ShutdownEffect(SStopRequest& request)
{
	...
	if (request.data.pEmitter)
	{
		...
		{
			IAttachmentUpr& attachmentUpr = *request.pCharInst->GetIAttachmentUpr();
			if (IAttachment* pAttachment = attachmentUpr.GetInterfaceByNameCRC(request.data.ownedAttachmentNameCRC))
			{
