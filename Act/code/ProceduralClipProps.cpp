...
public:
	...
	virtual void OnEnter(float blendTime, float duration, const SProceduralClipAttachPropParams& params)
	{
		if (m_charInstance)
		{
			IAttachmentUpr* pAttachmentUpr = m_charInstance->GetIAttachmentUpr();

			if (IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByNameCRC(params.attachmentName.crc))
			{
				m_attachmentCRC = params.attachmentName.crc;

				tukk fileExt = PathUtil::GetExt(params.objectFilename.c_str());
				...
				{
					if (IAttachment* pAttach = pAttachmentUpr->GetInterfaceByNameCRC(params.attachmentName.crc))
					{
						if (IAttachmentSkin* pSkinAttach = pAttach->GetIAttachmentSkin())
						{
							if (ISkin* pSkin = gEnv->pCharacterUpr->LoadModelSKIN(params.objectFilename.c_str(), 0))
							{
				...
				else if (m_pAttachedCharInst = gEnv->pCharacterUpr->CreateInstance(params.objectFilename.c_str()))
				{
	...
	virtual void OnExit(float blendTime)
	{
		...
		{
			if (IAttachment* pAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByNameCRC(m_attachmentCRC))
			{
...
private:
	u32                         m_attachmentCRC; //TODO: This should be available via m_pParams pointer but currently is cleared by the time OnExit is called. Remove once fixed by Tom
...
public:
	...
	virtual void OnEnter(float blendTime, float duration, const SProceduralClipAttachEntityParams& params)
	{
		if (m_charInstance)
		{
			if (IAttachment* pAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByNameCRC(params.attachmentName.crc))
			{
...
public:
	...
	virtual void OnEnter(float blendTime, float duration, const SSwapAttachmentParams& params)
	{
		...
		if (m_charInstance)
		{
			if (IAttachment* pNewAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByNameCRC(params.attachmentNameNew.crc))
			{
				if (IAttachment* pOldAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByNameCRC(params.attachmentNameOld.crc))
				{
	...
	virtual void OnExit(float blendTime)
	{
		...
		{
			if (IAttachment* pNewAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByNameCRC(m_newAttachmentCRC))
			{
				if (m_oldAttachmentCRC != 0)
				{
					if (IAttachment* pOldAttachment = m_charInstance->GetIAttachmentUpr()->GetInterfaceByNameCRC(m_oldAttachmentCRC))
					{
...
private:
	u32 m_newAttachmentCRC;
	u32 m_oldAttachmentCRC;
};
