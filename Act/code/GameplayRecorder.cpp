...
#include <drx3D/Act/IGameStateRecorder.h>
#include <drx3D/Act/DrxActionCVars.h>

//------------------------------------------------------------------------
CGameplayRecorder::CGameplayRecorder(CDrxAction* pGameFramework)
...
void CGameplayRecorder::Event(IEntity* pEntity, const GameplayEvent& event)
{
	...
	if (CDrxActionCVars::Get().g_debug_stats)
	{
		DrxLog("GamePlay event %u, %p, value:%f extra:%p desc:%s", event.event, pEntity, event.value, event.extra, event.description ? event.description : "");
	}
...
{
	IGameplayRecorder* pRecorder = CDrxAction::GetDrxAction()->GetIGameplayRecorder();

	{
		int64 t = gEnv->pTimer->GetFrameStartTime().GetValue();
		IMetadataPtr pMetadata;
		pMetadata->SetTag(eDT_frame);
		pMetadata->SetValue(eBT_i64, (u8*)&t, 8);
	...
	while (!it->IsEnd())
	{
		...
		if (IActor* pActor = pActorSystem->GetActor(pEntity->GetId()))
		{
			...
			pActorMetadata->AddField(eDT_name, eBT_str, (u8*)sActorName.data(), static_cast<u8>(sActorName.size()));
			Vec3 pos = pActor->GetEntity()->GetWorldPos();
			pActorMetadata->AddField(eDT_position, eBT_vec3, (u8*)&pos, sizeof(pos));
			if (pActor != NULL)
			{
				u8 health = pActor->GetHealthAsRoundedPercentage();
				pActorMetadata->AddField(eDT_health, eBT_i08, &health, 1);
				u8 armor = pActor->GetArmor();
				pActorMetadata->AddField(eDT_armor, eBT_i08, &armor, 1);
				u8 isgod = pActor->IsGod() ? 0xff : 0x00;
				pActorMetadata->AddField(eDT_isgod, eBT_i08, &isgod, 1);
				//u8 third = pOldActor->IsThirdPerson() ? 0xff : 0x00;
				...
				if (IItem* pItem = pActor->GetCurrentItem())
				{
					...
					pItemMetadata->AddField(eDT_name, eBT_str, (u8*)name.data(), static_cast<u8>(name.size()));
					if (IWeapon* pWeapon = pItem->GetIWeapon())
					{
						...
						pWeaponMetadata->AddField(eDT_firemode, eBT_str, (u8*)fire.data(), static_cast<u8>(fire.size()));
						if (pFireMode->GetAmmoType())
						{
							string ammt = pFireMode->GetAmmoType()->GetName();
							pWeaponMetadata->AddField(eDT_ammotype, eBT_str, (u8*)ammt.data(), static_cast<u8>(ammt.size()));
						}
						u32 ammo = pFireMode->GetAmmoCount();
						pWeaponMetadata->AddField(eDT_ammocount, eBT_u32, (u8*)&ammo, sizeof(ammo));
		...
		else
		{
			...
			pEntityMetadata->AddField(eDT_name, eBT_str, (u8*)name.data(), static_cast<u8>(name.size()));
			string type = pEntity->GetClass()->GetName();
			pEntityMetadata->AddField(eDT_type, eBT_str, (u8*)type.data(), static_cast<u8>(type.size()));
			Vec3 pos = pEntity->GetWorldPos();
			pEntityMetadata->AddField(eDT_position, eBT_vec3, (u8*)&pos, sizeof(pos));
	...
	case eDT_entity:
		{
			...
			DrxLog("entity [%s] [%s] [%f,%f,%f]", name.c_str(), type.c_str(), pos.x, pos.y, pos.z);
		}
		break;
	...
	case eDT_actor:
		{
			...
			if (const string* temp = boost::get<const string>(&data.GetField(eDT_item).GetField(eDT_weapon).GetField(eDT_firemode).GetValue()))
				firemode = *temp;
			DrxLog("actor [%s] [%s] [%s] [%f,%f,%f]", name.c_str(), item.c_str(), firemode.c_str(), pos.x, pos.y, pos.z);
		}
		break;
	}
}
...
IGameStateRecorder* CGameplayRecorder::EnableGameStateRecorder(bool bEnable, IGameplayListener* pL, bool bRecording)
{
	...
	if (!m_pGameStateRecorder)
	{
		if (auto* pGame = gEnv->pGameFramework->GetIGame())
			m_pGameStateRecorder = pGame->CreateGameStateRecorder(pL);//pTestUpr);
	}
...
////////////////////////////////////////////////////////////////////////////
void CGameplayRecorder::GetMemoryStatistics(IDrxSizer* s)
{
