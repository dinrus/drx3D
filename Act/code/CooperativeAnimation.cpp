...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  Синхронизированное воспроизведение анимации с несколькими
   акторами, включая точное позиционировнаие и "слайдинг" акторов
   в эту позицию.
   -------------------------------------------------------------------------
   История:
...
#include <drx3D/Act/CooperativeAnimation.h>
...
#define _LIBCPP_ENABLE_CXX17_REMOVED_BINDERS 1

#include <functional>

const static tuk COOP_ANIM_NAME = "CoopAnimation";
const static i32 MY_ANIM = 54545;
...
//	return (it == itEnd);
return true; // Версия clang 15 не находит std::mem_fun_ref !!!
}
...
bool CCooperativeAnimation::Update(float dt)
{
	...
	// DEBUG OUTPUT
	if (CDrxActionCVars::Get().co_coopAnimDebug)
	{
...
i32 MovementCallback(ICharacterInstance* pCharacter, uk pEntity)
{
...
bool CCooperativeAnimation::Init(TCharacterParams& characterParams, const SCooperativeAnimParams& generalParams)
{
	...
	m_bPauseAG = generalParams.bLooping || (CDrxActionCVars::Get().co_usenewcoopanimsystem != 0); // false will use an old AG based method which needs special setup in each character's AG
	...
	for (; it != iend; ++it)
	{
		...
		{
			DrxLogAlways("Cooperative Animation Upr was passed an invalid actor");
			return false;
		}

		if (chrParams.sSignalName == "")
		{
			DrxLogAlways("Cooperative Animation Upr was not passed a correct signal name");
			return false;
		}
		...
		if (m_bPauseAG)
		{
			...
			DRX_ASSERT(pAnimChar && pAnimChar->GetEntity());

			ICharacterInstance* pICharacter = pAnimChar->GetEntity()->GetCharacter(0);
			DRX_ASSERT(pICharacter);

			IAnimationSet* pAnimSet = pICharacter->GetIAnimationSet();
			i32k animId = pAnimSet->GetAnimIDByName(chrParams.sSignalName.c_str());
			if (animId < 0)
			{
				GameWarning("Cooperative Animation Upr was passed a non-existing anim (%s) for actor %s", chrParams.sSignalName.c_str(), chrParams.pActor->GetEntity()->GetName());
				return false;
			}
			...
			DRX_ASSERT(chrParams.animFilepathCRC != 0);
		}
	}
	...
	if (CDrxActionCVars::Get().co_slideWhileStreaming)
	{
...
bool CCooperativeAnimation::InitForOne(const SCharacterParams& params, const SCooperativeAnimParams& generalParams)
{
	...
	{
		DrxLogAlways("Cooperative Animation Upr was not passed a correct animation or signal name");
		return false;
	}
	...
	if (m_bPauseAG)
	{
		...
		IAnimationSet* pAnimSet = pICharacter->GetIAnimationSet();
		i32k animId = pAnimSet->GetAnimIDByName(characterParams.sSignalName.c_str());
		if (animId < 0)
		{
			GameWarning("Cooperative Animation Upr was passed a non-existing anim (%s) for actor %s", characterParams.sSignalName.c_str(), characterParams.pActor->GetEntity()->GetName());
			return false;
		}

		// Cache FilePathCRC of this anim for efficient streaming queries
		characterParams.animFilepathCRC = pAnimSet->GetFilePathCRCByAnimID(animId);
		DRX_ASSERT(characterParams.animFilepathCRC != 0);
	}
...
bool CCooperativeAnimation::StartAnimation(SCharacterParams& params, i32 characterIndex)
{
	DRX_ASSERT(params.pActor);
	IAnimatedCharacter* pAnimChar = params.pActor;
	DRX_ASSERT(pAnimChar);

	IAnimationGraphState* pGraphState = pAnimChar->GetAnimationGraphState();
	DRX_ASSERT_MESSAGE(pGraphState, "Cannot retrieve animation graph state");
	if (!pGraphState)
		return false;
	...
	else
	{
		...
		tukk cSignal = "coopanimation";
		tukk cOutPut = pGraphState->QueryOutput("CoopAnim");
		...
		{
			DRX_ASSERT_MESSAGE(0, "Could not set animation graph input 'coopanimation'.");
			DrxLogAlways("Cooperative Animation start failed - animation graph not prepared?");
			return false;
		}
		...
		{
			DRX_ASSERT_MESSAGE(0, "Could not set animation graph variation 'CoopAnimation'.");
			DrxLogAlways("Cooperative Animation start failed - animation graph not prepared?");
			return false;
		}
		...
		{
			DRX_ASSERT_MESSAGE(0, "Could not retrieve correct output from AG");
			DrxLogAlways("Cooperative Animation - state teleport failed? (Could not retrieve correct output from AG graph state %s)", pGraphState->GetCurrentStateName());
...
bool CCooperativeAnimation::AnimationsFinished()
{
	...
	TCharacterParams::iterator iend = m_paramsList.end();
	SCharacterParams* params;
	i32 finishedCount = m_paramsList.size();
	for (; it != iend; ++it)
	{
		// cast into the correct class
		params = &(*it);  // looks overly complicated, but is needed to compile for console
		...
		else
		{
			...
			{
				tukk stateName = params->pActor->GetAnimationGraphState()->GetCurrentStateName();
...
void CCooperativeAnimation::PlayAndSlideCharacters(float dt, bool& bAnimFailure, bool& bAllStarted, bool& bAllDone)
{
	...
	for (i32 i = 0; it != iend; ++it, ++i)
	{
		...
		if (!animJustStarted || CDrxActionCVars::Get().co_slideWhileStreaming)
		{
	...
	default:
		{
			...
			{
				DRX_ASSERT_MESSAGE(0, "Possibly an invalid quaternion, or only z deviation in animations.");
			}
...
QuatT CCooperativeAnimation::GetStartOffset(SCharacterParams& params)
{
	...
	if (!params.pActor)
	{
		DRX_ASSERT_MESSAGE(params.pActor, "No valid Actor received!");
		return retVal;
	}

	ICharacterInstance* pCharacter = params.pActor->GetEntity()->GetCharacter(0);
	DRX_ASSERT(pCharacter != NULL);
	...
	i32 id = pAnimSet->GetAnimIDByName(animation0.c_str());
	if (id < 0)
		return retVal;

	u32 flags = pAnimSet->GetAnimationFlags(id);
	...
	{
		DRX_ASSERT_MESSAGE(animDuration > params.fSlidingDuration, "Incorrect parameter: Sliding Duration longer than animation.");
		DrxLogAlways("Warning: sliding duration longer than actual animation. Will adjust given parameter.");
...
void CCooperativeAnimation::CleanupForFinishedCharacter(SCharacterParams& params)
{
	...
	const ICooperativeAnimationUpr* const pCAUpr = gEnv->pGameFramework->GetICooperativeAnimationUpr();
	DRX_ASSERT(pCAUpr);

	DRX_ASSERT_MESSAGE(!pCAUpr->IsActorBusy(params.pActor, this), "Cleaning up for a character that's already playing a second animation");
	...
	if (m_bPauseAG && pAC->GetAnimationGraphState())
	{
		...
		{
			const bool bReleased = gEnv->pCharacterUpr->CAF_Release(params.animFilepathCRC);
			DRX_ASSERT(bReleased);
...
float CCooperativeAnimation::GetAnimationNormalizedTime(const EntityId entID) const
{
	...
	i32 iAnimCount = pISkeletonAnim->GetNumAnimsInFIFO(0);

	for (i32 i = 0; i < iAnimCount; ++i)
	{
	...
	for (; it != iend; ++it)
	{
		...
		if (params->animationState != SCharacterParams::AS_Ready)
		{
			...
			case SCharacterParams::AS_Unrequested:
				{
					// Request
					const bool bRefAdded = gEnv->pCharacterUpr->CAF_AddRef(params->animFilepathCRC);
					DRX_ASSERT(bRefAdded);
			...
			case SCharacterParams::AS_NotReady:
				{
					...
					if (gEnv->pCharacterUpr->CAF_IsLoaded(params->animFilepathCRC))
					{
