...
u32k MAX_ALLOWED_QUEUE_SIZE = 10;
...
u32 CActionController::s_blendChannelCRCs[MANN_NUMBER_BLEND_CHANNELS];

#if DRXMANNEQUIN_WARN_ABOUT_VALIDITY()
i32 CActionController::s_mnFatalErrorOnInvalidCharInst = 0;
i32 CActionController::s_mnFatalErrorOnInvalidEntity = 0;
#endif
...
i32 CActionController::s_mnDebugFragments = 0;
...
void CActionController::RegisterCVars()
{
	if (!s_registeredCVars)
	{
		...
		REGISTER_COMMAND("mn_dump", ::DumpSequence, VF_CHEAT, "Dumps specified entity's DrxMannequin history in the path specified by mn_sequence_path");
...
{
	u32k numControllers = s_actionControllers.size();
	...
	if (numControllers > 0)
	{
		for (TActionControllerList::iterator iter = s_actionControllers.begin(); iter != s_actionControllers.end(); ++iter)
		{
			...
			DrxLogAlways("ActionController not released - Owner Controller Def: %s", pAC->GetContext().controllerDef.m_filename.c_str());
		}
		DrxFatalError("ActionControllers (%u) not released at shutdown", numControllers);
	}
}
...
#ifndef _RELEASE
void CActionController::ChangeDebug(tukk entName)
{
...
void BuildFilename(stack_string& filename, tukk entityName)
{
	char dateTime[128];
	time_t ltime;
	time(&ltime);
...
void CActionController::DumpSequence(tukk entName, float dumpTime)
{
	CActionController* debugAC = NULL;
	for (TActionControllerList::iterator iter = s_actionControllers.begin(); iter != s_actionControllers.end(); ++iter)
	{
...
CActionController::CActionController(IEntity* pEntity, SAnimationContext& context)
	...
	, m_scopeArray(static_cast<CActionScope*>(DrxModuleMalloc(sizeof(CActionScope) * m_scopeCount)))
...
{
	u32k numScopeContexts = context.controllerDef.m_scopeContexts.GetNum();
...
CActionController::~CActionController()
{
	...
	{
		u32k numOwningControllers = m_owningControllers.size();
		for (u32 i = 0; i < numOwningControllers; i++)
		{
	...
	u32 numProcContexts = m_procContexts.size();
	for (u32 i = 0; i < numProcContexts; i++)
	{
...
bool CActionController::CanInstall(const IAction& action, TagID subContext, const ActionScopes& scopeMask, float timeStep, float& timeTillInstall) const
{
	...
	for (u32 i=0; i<m_scopeCount; i++, scopeFlag <<= 1)
	{
		if (scopeFlag & expandedScopeMask)
		{
			const CActionScope& scope = m_scopeArray[i];
			{
				...
				else
				{
					...
					u32k fragCRC = m_context.controllerDef.m_fragmentIDs.GetTagCRC(action.GetFragmentID());
...
{
	u32k numScopes = GetTotalScopes();
	const ActionScopes mask = GetActiveScopeMask() & scopeMask;

	u32 installedContexts = 0;
...
void CActionController::Install(IAction& action, float timeRemaining)
{
	...
	u32 optionIdx = action.GetOptionIdx();
	...
	for (u32 i=0; i<m_scopeCount; i++, scopeFlag <<= 1)
	{
		...
		if (scopeFlag & filteredScope)
		{
			...
			if (pSlaveAC != NULL)
			{
				...
				u32 fragCRC = m_context.controllerDef.m_fragmentIDs.GetTagCRC(fragmentID);
	...
	for (u32 i=0; i<m_scopeCount; i++, scopeFlag <<= 1)
	{
		if (scopeFlag & scopeMask)
		{
			...
			if (hasFound)
			{
				...
				for (u32 p = 0; p < SFragmentData::PART_TOTAL; p++)
				{
...
void CActionController::ReleaseScopes()
{
	...
	for (u32 i = 0; i < m_queuedActions.size(); ++i)
	{
...
void CActionController::ReleaseScopeContexts()
{
	...
	u32k numContexts = m_context.controllerDef.m_scopeContexts.GetNum();
...
void CActionController::FlushScope(u32 scopeID, ActionScopes scopeFlag, EFlushMethod flushMethod)
{
...
void CActionController::SetScopeContext(u32 scopeContextID, IEntity& entity, ICharacterInstance* pCharacter, const IAnimationDatabase* animDatabase)
{
	if (scopeContextID >= (u32)m_context.controllerDef.m_scopeContexts.GetNum())
	{
		DrxFatalError("[SetScopeContext] Invalid scope context id %u used for %s", scopeContextID, m_cachedEntity->GetName());
	}
	...
	if ((pCharacter != scopeContext.pCharInst) || (animDatabase != scopeContext.pDatabase))
	{
		...
		// DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_INFO, "***SetScopeContext %2d on entity %s (id=%d; %p) to entity=%s (id=%8d; %p), char=%x", scopeContextID, GetSafeEntityName(), m_entityId, m_cachedEntity, entity.GetName(), entity.GetId(), &entity, pCharacter);
		...
		DRX_ASSERT(scopeContextIsValid);
...
void CActionController::ClearScopeContext(u32 scopeContextID, bool flushAnimations)
{
	if (scopeContextID >= (u32)m_context.controllerDef.m_scopeContexts.GetNum())
	{
		DrxFatalError("[ClearScopeContext] Invalid scope context id %u used for %s", scopeContextID, m_cachedEntity->GetName());
	}
...
IScope* CActionController::GetScope(u32 scopeID)
{
...
const IScope* CActionController::GetScope(u32 scopeID) const
{
	DRX_ASSERT_MESSAGE((scopeID < m_scopeCount), "Invalid scope id");
...
{
	for (u32 i = 0, scopeFlag = 1; i < m_scopeCount; i++, scopeFlag <<= 1)
	{
		CActionScope& scope = m_scopeArray[i];

		if (scope.m_pAction)
		{
			DRX_ASSERT(scope.m_pAction->m_installedScopeMask & scopeFlag);
		}
	}
}
#endif //_DEBUG
...
void CActionController::ResolveActionStates()
{
	...
	for (u32 i = 0; i < m_endedActions.size(); i++)
	{
	...
	//--- Now install actions
	for (u32 i = 0; i < m_triggeredActions.size(); i++)
	{
...
bool CActionController::ResolveActionInstallations(float timePassed)
{
	...
	u32k numActions = m_queuedActions.size();
	...
	for (u32 i = 0; i < m_scopeCount; i++)
	{
		    ...
		    && ((pExitingAction->m_flags & IAction::TransitionPending) == 0)) // No blend off if there is a fragment we are waiting to blend to already
		{
			...
			if (rootScope.CanInstall(priority, FRAGMENT_ID_INVALID, SFragTagState(), false, timeLeft) && (timePassed >= timeLeft))
			{
				...
				ActionScopes scopeFlag = 1;
				for (u32 s=0; s<m_scopeCount; s++, scopeFlag<<=1)
				{
...
{
	i32 numTooManyActions = m_queuedActions.size() - MAX_ALLOWED_QUEUE_SIZE;

	if (numTooManyActions > 0)
	{
		// print the remaining action queue before shrinking it
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "--- Mannequin Error, attempting to queue too many actions on entity '%s' (queue size = %" PRISIZE_T ", but only up to %u allowed) ---", GetSafeEntityName(), m_queuedActions.size(), MAX_ALLOWED_QUEUE_SIZE);
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "    %" PRISIZE_T " actions in queue:", m_queuedActions.size());
		for (size_t k = 0; k < m_queuedActions.size(); k++)
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "    #%" PRISIZE_T ": %s", k, m_queuedActions[k]->GetName());
		}
		...
		do
		{
			...
			for (i32 i = (i32)m_queuedActions.size() - 1; i >= 0; --i)
			{
				IActionPtr pOtherAction = m_queuedActions[i];
				const bool isInterruptable = ((pOtherAction->GetFlags() & IAction::Interruptable) != 0);
				if (!isInterruptable)
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Ditching other action '%s' (%p) (at queue index %d)", pOtherAction->GetName(), pOtherAction.get(), i);
			...
			{
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Can't remove anything");
				break;
			}
		}
	...
	if ((m_flags & AC_PausedUpdate) == 0)
	{
		...
		for (u32 i = 0; i < m_scopeCount; i++)
		{
			...
			if (pScopeAction && (pScopeAction->m_rootScope == &scope))
			{
				...
				if (pInst)
				{
					for (u32 ch = 0; ch < MANN_NUMBER_BLEND_CHANNELS; ch++)
					{
		...
		DRX_ASSERT(s_tickedActions.empty());
		u32 numIts;
		u32k MAX_ITERATIONS = 5;
		...
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Error, cannot resolve action changes in %u iterations for '%s'", MAX_ITERATIONS, GetSafeEntityName());
		}
		else if (numIts > 0)
		{
...
void CActionController::SetSlaveController(IActionController& target, u32 targetContext, bool enslave, const IAnimationDatabase* piOptionTargetDatabase)
{
	const bool doFullEnslavement = !piOptionTargetDatabase;

	if (targetContext >= m_context.controllerDef.m_scopeContexts.GetNum())
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "SetSlaveController: invalid scope context index!");
		return;
	}
	...
	if (enslave)
	{
		...
		DRX_ASSERT(targetController.GetEntityId());
		...
		if (targetController.GetEntityId())
		{
			...
			else
			{
				...
				DRX_ASSERT(piOptionTargetDatabase);
				...
				for (u32 i=0; i<targetController.m_scopeCount; i++, scopeMask <<= 1)
				{
...
{
	u32k numScopeContexts = m_context.controllerDef.m_scopeContexts.GetNum();
	for (u32 i = 0; i < numScopeContexts; i++)
	{
		SScopeContext& scopeContext = m_scopeContexts[i];
...
void CActionController::SynchTagsToSlave(SScopeContext& scopeContext, bool enable)
{
	...
	if (pTargetController)
	{
		...
		for (u32 i = 0; i < tagDefSlave.GetNum(); i++)
		{
...
void CActionController::Reset()
{
	...
	u32k numScopeContexts = (u32)m_context.controllerDef.m_scopeContexts.GetNum();
	for (u32 itContext = 0; itContext < numScopeContexts; ++itContext)
	{
...
void CActionController::Flush()
{
	...
	{
		DrxFatalError("Flushing the action controller in the middle of an update, this is bad!");
	}
	UpdateValidity();
	ActionScopes scopeFlag = 1;
	for (u32 i=0; i<m_scopeCount; i++, scopeFlag <<= 1)
	...
	u32k numQueuedActions = m_queuedActions.size();
	for (u32 i = 0; i < numQueuedActions; i++)
	{
...
{
	u32k numListeners = m_listeners.size();
	...
	DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Attempting to remove an unknown listener!");
}
...
void CActionController::EndAction(IAction& action, EFlushMethod flushMethod)
{
	for (u32 i = 0; i < m_scopeCount; i++)
	{
		DRX_ASSERT(m_scopeArray[i].m_pAction != &action);
	}
...
ActionScopes CActionController::EndActionsOnScope(ActionScopes scopeMask, IAction* pPendingAction, bool blendOut, EFlushMethod flushMethod)
{
	...
	//--- Clean up actions
	for (u32 i = 0; i < deleteActionList.size(); i++)
	{
...
{
	i32k priority = action.GetPriority();
...
{
	DRX_ASSERT(action.GetStatus() == IAction::Installed);
...
void CActionController::DumpHistory(tukk filename, float timeDelta) const
{
	...
	for (u32 i = 0; i < TOTAL_HISTORY_SLOTS; i++)
	{
		u32 slotID = (i + m_historySlot + 1) % TOTAL_HISTORY_SLOTS;
	...
	if (!success)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Error saving DrxMannequin history to '%s'", filename);
	}
}
...
void CActionController::DebugDraw() const
{
	...
	DrxStackStringT<char, 1024> sTagState;
	...
	for (u32 i = 0; i < m_context.subStates.size(); i++)
	{
		DrxStackStringT<char, 1024> sSubState;
	...
	for (u32 i = 0; i < m_scopeCount; i++)
	{
		DrxStackStringT<char, 1024> sMessage;
		const CActionScope& scope = m_scopeArray[i];

		FragmentID fragID = scope.GetLastFragmentID();
		tukk fragName = (fragID != FRAGMENT_ID_INVALID) ? controllerDef.m_fragmentIDs.GetTagName(fragID) : "NoFragment";
		if (scope.HasFragment())
		{
			...
			if (pFragTags && (scope.m_lastFragSelection.tagState.fragmentTags != TAG_STATE_EMPTY))
			{
				...
				{
					DrxStackStringT<char, 1024> sLastOptionIdx;
		...
		else
		{
			...
			if (scope.m_pAction)
			{
				...
				DrxStackStringT<char, 1024> sScopeContext;
		...
		DrxStackStringT<char, 1024> sExitingAction;
		if (scope.m_pExitingAction)
		...
		if (scope.m_scopeContext.pCharInst)
		{
			...
			for (u32 l = 0; l < scope.m_numLayers; l++)
			{
				...
				if (sequencer.sequence.size() > 0)
				{
					...
					for (u32 k = 0; k < sequencer.sequence.size(); k++)
					{
						tukk animName = sequencer.sequence[k].animation.animRef.c_str();
						i32 letterCount = animName ? strlen(animName) : 0;
		...
		u32k numProcLayers = scope.m_procSequencers.size();
		for (u32 l = 0; l < numProcLayers; l++)
		{
			...
			{
				float xpos = XPOS_SEQUENCE;
				for (u32 k = 0; k < procSeq.sequence.size(); k++)
				{
					...
					if (!isNone)
					{
						tukk typeName = mannequin::FindProcClipTypeName(procEntry.typeNameHash);
						i32 letterCount = typeName ? strlen(typeName) : 0;
						float* colourA = FONT_COLOUR;
						if (k == procSeq.pos)
						{
						...
						DrxStackStringT<char, 1024> sTypename(typeName);
						...
						if (!infoString.IsEmpty())
							sTypename += "(" + DrxStackStringT<char, 1024>(infoString.c_str()) + ")";
	...
	for (u32 i = 0; i < m_queuedActions.size(); i++)
	{
		...
		tukk fragName = (fragID != FRAGMENT_ID_INVALID) ? m_context.controllerDef.m_fragmentIDs.GetTagName(fragID) : "NoFragment";
...
IProceduralContext* CActionController::FindOrCreateProceduralContext(const DrxClassID& contextId)
{
...
IProceduralContext* CActionController::CreateProceduralContext(const DrxClassID& contextId)
{
	...
	DrxCreateClassInstance<IProceduralContext>(contextId, newProcContext.pContext);
...
IProceduralContext* CActionController::FindProceduralContext(const DrxClassID& contextId)
{
...
const IProceduralContext* CActionController::FindProceduralContext(const DrxClassID& contextId) const
{
	for(auto& proceduralContext : m_procContexts)
	{
		if (proceduralContext.contextId.hipart == contextId.hipart
...
bool CActionController::IsActionPending(u32 userToken) const
{
	for (u32 i = 0; i < m_queuedActions.size(); i++)
	{
		const IAction& action = *m_queuedActions[i];
...
void CActionController::Resume(u32 resumeFlags)
{
	if (GetFlag(AC_PausedUpdate))
	{
...
ActionScopes CActionController::FlushScopesByScopeContext(u32 scopeContextID, EFlushMethod flushMethod)
{
...
void CActionController::UpdateValidity()
{
	...
	u32k numScopeContexts = m_context.controllerDef.m_scopeContexts.GetNum();
	u32 scopeContextsToFlush = 0;
	for (u32 scopeContextID = 0; scopeContextID < numScopeContexts; scopeContextID++)
	{
...
bool CActionController::UpdateScopeContextValidity(u32 scopeContextID)
{
	SScopeContext& scopeContext = m_scopeContexts[scopeContextID];
	...
	if (hasInvalidEntity)
	{
		...
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[CActionController::UpdateScopeContextValidity] Dangling Entity %p (expected %p for id=%u) in context '%s'", (uk )scopeContext.pCachedEntity, (uk )expectedEntity, scopeContext.entityId, m_context.controllerDef.m_scopeContexts.GetTagName(scopeContextID));
		DRX_ASSERT_MESSAGE(0,"[CActionController::UpdateScopeContextValidity] Dangling Entity");
#endif // !DRXMANNEQUIN_WARN_ABOUT_VALIDITY()
	...
	{
#if DRXMANNEQUIN_WARN_ABOUT_VALIDITY()
		tukk entityName = (scopeContext.pCachedEntity ? scopeContext.pCachedEntity->GetName() : "<NULL>");
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[CActionController::UpdateScopeContextValidity] Dangling Char Inst in entity '%s' (id=%d) in context '%s'", entityName, scopeContext.entityId, m_context.controllerDef.m_scopeContexts.GetTagName(scopeContextID));
		DRX_ASSERT_MESSAGE(0,"[CActionController::UpdateScopeContextValidity] Dangling Char Inst in entity ");
#endif // !DRXMANNEQUIN_WARN_ABOUT_VALIDITY()
...
bool CActionController::UpdateRootEntityValidity()
{
	...
	{
#if DRXMANNEQUIN_WARN_ABOUT_VALIDITY()
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[CActionController::UpdateRootEntityValidity] Dangling Entity %p (expected %p for id=%u) in actioncontroller for '%s'", (uk )m_cachedEntity, (uk )expectedEntity, m_entityId, m_context.controllerDef.m_filename.c_str());
		DRX_ASSERT_MESSAGE(0,"[CActionController::UpdateRootEntityValidity] Dangling Entity");
#endif // !DRXMANNEQUIN_WARN_ABOUT_VALIDITY()
		m_entityId = 0;
		m_cachedEntity = NULL;
	}
...
QuatT CActionController::ExtractLocalAnimLocation(FragmentID fragID, TagState fragTags, u32 scopeID, u32 optionIdx)
{
	...
	if (IScope* pScope = GetScope(scopeID))
	{
		...
		if (pAnimationSet)
		{
			...
			{
				i32 animID = pAnimationSet->GetAnimIDByCRC(fragData.animLayers[0][0].animation.animRef.crc);
				if (animID >= 0)
				{
