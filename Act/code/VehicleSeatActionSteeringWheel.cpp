...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Implements a seat action for the steering wheel

   -------------------------------------------------------------------------
   История:
   - 28:11:2005: Created by Mathieu Pinard
   - 14:06:2012: Re-factored to work with DrxMannequin by Benito Gangoso Rodriguez
...
CVehicleSeatActionSteeringWheel::CVehicleSeatActionSteeringWheel()
	: m_isBeingUsed(false)
	, m_isUsedSerialization(false)
	, m_steeringActions(ZERO)
	, m_steeringValues(ZERO)
	, m_wheelInvRotationMax(0.0f)
	, m_currentSteering(0.0f)
	, m_jitterOffset(0.0f)
	, m_jitterSuspensionResponse(0.0f)
{
}
...
bool CVehicleSeatActionSteeringWheel::Init(IVehicle* pVehicle, IVehicleSeat* pSeat, const CVehicleParams& table)
{
	...
	ISharedParamsUpr* pSharedParamsUpr = gEnv->pGameFramework->GetISharedParamsUpr();
	...
	if (!m_pSharedParams)
	{
		...
		m_pSharedParams = CastSharedParamsPtr<SSharedParams>(pSharedParamsUpr->Register(sharedParamsName, params));
	}
...
//------------------------------------------------------------------------
void CVehicleSeatActionSteeringWheel::OnAction(const TVehicleActionId actionId, i32 activationMode, float value)
{
	if (m_pSharedParams->steeringClass == eSC_Generic)
	{
