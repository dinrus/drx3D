...
bool CInteractor::Init(IGameObject* pGameObject)
{
	...
	m_pQuery->SetProximityRadius(CDrxActionCVars::Get().playerInteractorRadius);

	m_pGameRules = CDrxAction::GetDrxAction()->GetIGameRulesSystem()->GetCurrentGameRulesEntity()->GetScriptTable();
	...
	m_queryMethods = (m_funcAreUsable) ? "m" : "rb";  //m is optimized for Drxsis2, "rb" is Drxsis1 compatible
...
bool CInteractor::ReloadExtension(IGameObject* pGameObject, const SEntitySpawnParams& params)
{
	...
	DRX_ASSERT_MESSAGE(false, "CInteractor::ReloadExtension not implemented");
...
void CInteractor::Update(SEntityUpdateContext&, i32)
{
	...
	const IActor* const pActor = CDrxAction::GetDrxAction()->GetIActorSystem()->GetActor(myId);
	if (pActor && !pActor->IsInteracting())
	{
		...
		{
			if ((CDrxActionCVars::Get().playerInteractorRadius != m_lastRadius) && m_pQuery)
			{
				m_lastRadius = CDrxActionCVars::Get().playerInteractorRadius;
...
void CInteractor::PerformQueries(EntityId& id, i32& idx)
{
...
bool CInteractor::PerformMergedQuery(SQueryResult& r, float minDot)
{
	...
	{
		//	DrxLog ("[INTERACTION] Dot winner = '%s', box winner = '%s'!", pNearestDotEntity ? pNearestDotEntity->GetClass()->GetName() : "N/A", pNearestBoxEntity ? pNearestBoxEntity->GetClass()->GetName() : "N/A");
...
i32 CInteractor::PerformUsableTest(IEntity* pEntity) const
{
...
void CInteractor::PerformUsableTestOnEntities(TQueryVector& queryList)
{
	...
	if (m_areUsableEntityList)
	{
		...
		for (TQueryVector::const_iterator elementCit = queryList.begin(); elementCit != endCit; ++elementCit)
		{
			...
			DRX_ASSERT(element.first);
			DRX_ASSERT(element.first->GetScriptTable());
		...
		{
			i32k resultCount = scriptResults->Count();
			i32k queryCount = queryList.size();

			DRX_ASSERT(queryCount <= resultCount);

			if (queryCount <= resultCount)
			{
				for (i32 i = 0; i < queryCount; ++i)
				{
...
bool CInteractor::PerformUsableTestAndCompleteIds(IEntity* pEntity, SQueryResult& r) const
{
	...
	r.entityId = 0;
	r.slotIdx = 0;
	if (i32 usableIdx = PerformUsableTest(pEntity))
	{
...
void CInteractor::UpdateTimers(EntityId newOverId, i32 usableIdx)
{
...
bool CInteractor::NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags)
{
	return true;
}
...
void CInteractor::GetMemoryUsage(IDrxSizer* pSizer) const
{
