...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config) override
	{
		static const SInputPortConfig in_config[] = {
			InputPortConfig<i32>("CharacterSlot",     _HELP("Slot in which the character is loaded or -1 for all loaded characters.")),
	...
	template<typename TCustomArgs, bool(CEntityAttachmentControlNode::* TFunc)(i32 slotId, ICharacterInstance*, const string&, const TCustomArgs&)>
	void ForRequiredCharacters(SActivationInfo* const pActInfo, const string& attachmentName, const TCustomArgs& customArguments)
	{
		i32k characterSlot = GetPortInt(pActInfo, eIP_CharacterSlot);
		if (characterSlot >= 0)
		{
		...
		{
			i32k slotCount = pActInfo->pEntity->GetSlotCount();
			for (i32 slotId = 0; slotId != slotCount; ++slotId)
			{
				ICharacterInstance* pCharacterInstance = pActInfo->pEntity->GetCharacter(characterSlot);
	...
	typedef VectorMap<i32, IAttachmentObject*> TSlotAttachmentCache;
	...
	bool UpdateAttachmentObject(i32 slotId, ICharacterInstance* pCharacterInstance, const string& attachmentName, const SUpdateAttachmentObjectArgs& args)
	{
		bool bSuccess = false;
		IAttachmentUpr* const pAttachmentUpr = pCharacterInstance->GetIAttachmentUpr();
		IAttachment* const pAttachment = pAttachmentUpr ? pAttachmentUpr->GetInterfaceByName(attachmentName.c_str()) : nullptr;
		if (pAttachment)
		{
			...
			case CA_BONE:
				{
					...
					if (bIsCdf)
					{
						if (const _smart_ptr<ICharacterInstance> pCharacterInstance = gEnv->pCharacterUpr->CreateInstance(args.objectName.c_str()))
						{
			...
			case CA_SKIN:
				{
					ISkin* pSkin = gEnv->pCharacterUpr->LoadModelSKIN(args.objectName.c_str(), 0);
					if (pSkin)
					{
	...
	bool UpdateAttachmentMaterial(i32 slotId, ICharacterInstance* pCharacterInstance, const string& attachmentName, const SUpdateAttachmentMaterialArgs& args)
	{
		bool bSuccess = false;
		...
		if (!pAttachmentObject)
		{
			...
			if (pAttachmentUpr && !pAttachmentObject)
			{
				GameWarning("[CharacterAttachmentControl] At least one character instance didn't have the binding we were looking for :\n\tAttachmentName=%s", attachmentName.c_str());
			}
		}
		...
		if (pAttachmentObject)
		{
			IMaterialUpr* const pMaterialUpr = gEnv->p3DEngine->GetMaterialUpr();
			IMaterial* const pMaterial = pMaterialUpr->LoadMaterial(args.materialName.c_str());
			if (pMaterial)
			{
	...
	bool ChangeAttachmentVisibility(i32 slotId, ICharacterInstance* pCharacterInstance, const string& attachmentName, const bool& bVisible)
	{
		bool bSuccess = false;
		IAttachmentUpr* const pAttachmentUpr = pCharacterInstance->GetIAttachmentUpr();
		IAttachment* const pAttachment = pAttachmentUpr ? pAttachmentUpr->GetInterfaceByName(attachmentName.c_str()) : nullptr;
...
public:
	...
	CEntityAttachmentExNodeRegistry& GetRegistry()
	{
		...
		{
			pRegistry = &pDrxAction->GetEntityAttachmentExNodeRegistry();
		}
	...
	template<bool(CEntityAttachmentExNode::* TFunc)(ICharacterInstance*, IAttachmentUpr*, IAttachment*)>
	bool ForRequiredCharacter()
	{
		...
		if (pCharacterInstance)
		{
			...
			if (pAttachmentUpr)
			{
				IAttachment* pAttachment = pAttachmentUpr->GetInterfaceByName(m_currentAttachmentName.c_str());
				bSuccess = (this->*TFunc)(pCharacterInstance, pAttachmentUpr, pAttachment);
			}
			else
			{
	...
	virtual void GetConfiguration(SFlowNodeConfig& config) override
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("JointId",            INVALID_JOINT_ID,                                                                                                                                                             _HELP("Id of the joint to attach to (only used if the joint name is left empty).")),
	...
	static i32 GetDesiredJointId(SActivationInfo* pActInfo, ICharacterInstance* pCharacterInstance)
	{
	...
	static string GetDesiredJointName(SActivationInfo* pActInfo, ICharacterInstance* pCharacterInstance)
	{
		...
		{
			i32 jointId = GetPortInt(pActInfo, eIP_JointId);
			if (jointId >= 0)
			{
				const IDefaultSkeleton& defaultSkeleton = pCharacterInstance->GetIDefaultSkeleton();
	...
	bool Attach(ICharacterInstance* pCharacterInstance, IAttachmentUpr* pAttachmentUpr, IAttachment* pOldAttachment)
	{
		bool bSuccess = false;
		...
		if (targetId != INVALID_ENTITYID)
		{
			...
			if (pFinalAttachment)
			{
				...
				const IDefaultSkeleton& defaultSkeleton = pCharacterInstance->GetIDefaultSkeleton();
				i32k oldJointId = static_cast<i32>(pFinalAttachment->GetJointID());
				i32k desiredJointId = GetDesiredJointId(&m_actInfo, pCharacterInstance);
				if (oldJointId != desiredJointId)
				{
					...
					{
						pAttachmentUpr->RemoveAttachmentByInterface(pFinalAttachment);
			...
			if (!bAbort)
			{
				...
				if (!pFinalAttachment)
				{
					...
					if (pFinalAttachment = pAttachmentUpr->CreateAttachment(m_currentAttachmentName.c_str(), CA_BONE, desiredJointName))
					{
				...
				if (pFinalAttachment && (CA_BONE == pFinalAttachment->GetType()))
				{
					...
					UpdateOffset(pCharacterInstance, pAttachmentUpr, pFinalAttachment);
	...
	bool Detach(ICharacterInstance* pCharacterInstance, IAttachmentUpr* pAttachmentUpr, IAttachment* pAttachment)
	{
		bool bSuccess = false;
	...
	bool Show(ICharacterInstance* pCharacterInstance, IAttachmentUpr* pAttachmentUpr, IAttachment* pAttachment)
	{
		bool bSuccess = false;
		if (pAttachment)
		{
	...
	bool Hide(ICharacterInstance* pCharacterInstance, IAttachmentUpr* pAttachmentUpr, IAttachment* pAttachment)
	{
		bool bSuccess = false;
		if (pAttachment)
		{
	...
	bool UpdateOffset(ICharacterInstance* pCharacterInstance, IAttachmentUpr* pAttachmentUpr, IAttachment* pAttachment)
	{
		bool bSuccess = false;
		if (pAttachment)
		{
...
private:
	...
	static u32   s_instanceCounter;
};

u32 CEntityAttachmentExNode::s_instanceCounter = 0;
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config) override
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("LinkIndex",      _HELP("Index of the link you want the details of. Should be in the range [0, LinkCount[")),
			...
			InputPortConfig<i32>("NameIndex",      0,                                                                                                                      _HELP("When multiple links have the same name, you can access the ones after the first one by increasing this value.")),
		...
		static const SOutputPortConfig out_config[] = {
			...
			OutputPortConfig<i32>("LinkCount",       _HELP("Number of links available on the selected entity.")),
		...
		case eFE_Activate:
			{
				...
				if (bGetCount || bGetByIndex || bGetByName || bGetNameCount)
				{
					...
					if (pActInfo->pEntity)
					{
						...
						if (bGetByIndex)
						{
							i32 index = GetPortInt(pActInfo, eIP_Index);
						...
						if (bGetByName)
						{
							i32 nameIndex = GetPortInt(pActInfo, eIP_NameIndex);
							bool bFound = false;
							const IEntityLink* pLink = pFirstLink;
							while (pLink)
							{
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config) override
	{
		static const SInputPortConfig in_config[] = {
			...
			InputPortConfig<i32>("Index",       0,                                                                                                                                                                                        _HELP("When multiple links have the same name, you can access the ones after the first one by increasing this value. This is only used for Remove and Rename commands. -1 means all of them.")),
		...
		case eFE_Activate:
			{
				if (IEntity* pEntity = pActInfo->pEntity)
				{
					...
					if (bAdd || bRemove || bRename)
					{
						...
						i32k linkIndex = GetPortInt(pActInfo, eIP_Index);
	...
	static bool DoRename(IEntity& entity, const string& linkName, i32 linkIndex, EntityId linkTargetId, const string& newLinkName)
	{
		bool bResult = false;
		if (linkIndex >= 0)
		{
	...
	static bool DoRemove(IEntity& entity, const string& linkName, i32 linkIndex, EntityId linkTargetId)
	{
		bool bResult = false;
		if (linkIndex >= 0)
		{
	...
	static IEntityLink* FindLink(IEntity& entity, const string& linkName, i32k linkIndex, EntityId linkTargetId)
	{
		IEntityLink* pLink = entity.GetEntityLinks();
		i32 nameIndex = linkIndex;
		while (pLink)
		{
