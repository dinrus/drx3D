...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Manage the activation and lifetime of multiple effects.
...
struct IGroundEffect
{
	...
	virtual bool SetParticleEffect(tukk name) = 0;
	...
	virtual void SetInteraction(tukk name) = 0;
...
struct IEffectSystem
{
	...
	virtual EffectId       GetEffectId(tukk name) = 0;

	virtual void           Activate(const EffectId& eid) = 0;
	virtual bool           BindEffect(tukk name, IEffect* pEffect) = 0;
