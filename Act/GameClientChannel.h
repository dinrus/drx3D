...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Implements a client-side IGameChannel interface.
...
{
	SGameTypeParams() {}
	SGameTypeParams(u16 rules, const string& level, bool imm) : rulesClass(rules), levelName(level), immersive(imm) {}
...
struct SEntityClassRegistration : public ISerializable
{
	...
	SEntityClassRegistration(u16 i, const string& n) : id(i), name(n)
	{
	}
...
struct SEntityClassHashRegistration : public ISerializable
{
	...
	SEntityClassHashRegistration(u32 _crc) : crc(_crc)
	{
	}
...
struct SClientBatchConsoleVariablesParams : public ISerializable
{
	...
	bool Add(const string& k, const string& v)
	{
		if (batchLimit == actual)
		{
			...
			if (s < dataLimit)
			{
				...
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "[SClientBatchConsoleVariablesParams]: data limit isn't used effectively, left [%d], try to add more vars in batch.", left);
			}
#endif
			return false;
		}
		...
		{
#if VARS_BATCH_PROFILE
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "[SClientBatchConsoleVariablesParams]: exceeded data limit at count [%d]", actual);
#endif
			return false;
		}
	...
	i32                          actual;
	static i32k             dataLimit = 1200;
	static i32k             batchLimit = 50;
...
{
	SSyncTimeClient() {};
	SSyncTimeClient(i32 id, int64 originTime, int64 serverTimeGuess, int64 serverTimeActual)
	{
		this->id = id;
...
private:
	void SendSyncPing(i32k id, const CTimeValue currentTime, const CTimeValue serverTime = CTimeValue());
...
public:
	...
	virtual void OnDisconnect(EDisconnectionCause cause, tukk description) override;
	// ~IGameChannel
