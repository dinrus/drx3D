...
struct SNetMsgData : public ISerializable
{
	...
	static ETarget ConvertTargetIntToEnum(i32 i)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_ERROR, "Invalid conversion from i32 to ETarget");
			return eT_Invalid;
		}
	}

	tukk DbgTranslateTargetToString() const
	{
	...
	tukk DbgTranslateSourceToString() const
	{
...
public:
	...
	void RegisterListener(INetMsgListener* pListener, tukk name = NULL);
	...
	static tukk            DbgTranslateLocationToString();
	static void                   Log(tukk szMessage);
	...
	class CNetStats
	{
		struct SStats
		{
			...
			void Add(u32 value) { m_total += value; m_cntInFrame += value; m_max = m_cntInFrame > m_max ? m_cntInFrame : m_max; }

			u32 m_cntInFrame, m_max;
			uint64 m_total;
		};
	...
	public:
		...
		void          Add(ETypes type, u32 value);
...
private:
	struct CVars
	{
		static i32  net_netMsgDispatcherDebug;
		static i32  net_netMsgDispatcherLogging;
		static i32  net_netMsgDispatcherWarnThreshold;
	...
	DrxCriticalSectionNonRecursive m_criticalSectionReceivedQueue;
	CNetStats                      m_stats;
};
