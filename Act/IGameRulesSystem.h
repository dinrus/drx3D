...
struct HitTypeInfo
{
	HitTypeInfo()
		: m_flags(0) {}
	HitTypeInfo(tukk pName, i32 flags = 0)
		: m_name(pName)
		, m_flags(flags) {}
...
struct HitInfo
{
	...
	float    angle;
	i32      material;   // material id of the surface which got hit
	i32      type;       // type id of the hit, see IGameRules::GetHitTypeId for more information
	i32      bulletType; //type of bullet, if hit was of type bullet
	...
	i32  penetrationCount;    // number of surfaces the bullet has penetrated
	...
	HitInfo(EntityId shtId, EntityId trgId, EntityId wpnId, float dmg, float rd, i32 mat, i32 part, i32 typ)
		: shooterId(shtId),
		targetId(trgId),
		weaponId(wpnId),
		projectileId(0),
	...
	HitInfo(EntityId shtId, EntityId trgId, EntityId wpnId, float dmg, float rd, i32 mat, i32 part, i32 typ, const Vec3& p, const Vec3& d, const Vec3& n)
		: shooterId(shtId),
		targetId(trgId),
		weaponId(wpnId),
		projectileId(0),
	...
	void SerializeWith(TSerialize ser)
	{
		...
		ser.Value("projectileId", projectileId, 'u32'); //'eid');
		...
		if (IsPartIDInvalid())
		{
			DrxFatalError("HitInfo::SerializeWith - partId %i is out of range.", partId);
		}
#endif
		...
		ser.Value("weaponClass", weaponClassId, 'u16');
		ser.Value("penetrationCount", penetrationCount, 'u8');
...
struct ExplosionInfo
{
	...
	i32   firstPassPhysicsEntities; // Specify which physics types to hit on the first pass. defaults to 'ent_living', set to zero to disable
	...
	ExplosionInfo(EntityId shtId, EntityId wpnId, EntityId prjId, float dmg, const Vec3& p, const Vec3& d, float minR, float r,
	              float minPhysR, float physR, float a, float press, float holesize, i32 typ)
		: shooterId(shtId),
		weaponId(wpnId),
		projectileId(prjId),
		projectileClassId(~u16(0)),
	...
	void SetEffect(tukk effectName, float scale, float maxblurdistance, float blindAmount = 0.0f, float flashbangScale = 1.0f)
	{
		effect_name = effectName;
		effect_scale = scale;
		if (!effect_name.empty())
			pParticleEffect = gEnv->pParticleUpr->FindEffect(effect_name.c_str());
		this->maxblurdistance = maxblurdistance;
		this->blindAmount = blindAmount;
		this->flashbangScale = flashbangScale;
	}

	void SetEffectName(tukk effectName)
	{
		this->effect_name = effectName;
	}

	void SetEffectClass(tukk cls)
	{
		effect_class = cls;
	}
	...
	void SerializeWith(TSerialize ser)
	{
		...
		{
			DRX_ASSERT_MESSAGE(angle == 0.0f, "Non zero explosion angles are not supported in multiplayer");
		}
...
struct IGameRules : public IGameObjectExtension
{
	...
	virtual bool ShouldKeepClient(i32 channelId, EDisconnectionCause cause, tukk desc) const = 0;
	...
	virtual void PrecacheLevelResource(tukk resourceName, EGameResourceType resourceType) = 0;
	...
	virtual XmlNodeRef FindPrecachedXmlFile(tukk sFilename) = 0;

	// client notification
	virtual void OnDisconnect(EDisconnectionCause cause, tukk desc) = 0; // notification to the client that he has been disconnected

	// Summary
	//   Notifies the server when a client is connecting
	virtual bool OnClientConnect(i32 channelId, bool isReset) = 0;

	// Summary
	//   Notifies the server when a client is disconnecting
	virtual void OnClientDisconnect(i32 channelId, EDisconnectionCause cause, tukk desc, bool keepClient) = 0;

	// Summary
	//   Notifies the server when a client has entered the current game
	virtual bool OnClientEnteredGame(i32 channelId, bool isReset) = 0;
	...
	virtual void SendTextMessage(ETextMessageType type, tukk msg, u32 to = eRMI_ToAllClients, i32 channelId = -1,
	                             tukk p0 = 0, tukk p1 = 0, tukk p2 = 0, tukk p3 = 0) = 0;
	...
	//   msg - the message to be sent
	virtual void SendChatMessage(EChatMessageType type, EntityId sourceId, EntityId targetId, tukk msg) = 0;
	...
	// See Also
	//   HitInfo, GetHitType
	virtual i32 GetHitTypeId(u32k crc) const = 0;
	virtual i32 GetHitTypeId(tukk type) const = 0;
	...
	virtual tukk GetHitType(i32 id) const = 0;
	...
	virtual EntityId SetChannelForMigratingPlayer(tukk name, u16 channelID) = 0;
	...
	virtual tukk GetTeamName(i32 teamId) const = 0;
};
...
struct IGameRulesSystem
{
	...
	virtual bool RegisterGameRules(tukk pRulesName, tukk pExtensionName, bool bUseScript = true) = 0;
	...
	virtual bool CreateGameRules(tukk pRulesName) = 0;
	...
	virtual void AddGameRulesAlias(tukk gamerules, tukk alias) = 0;

	// Summary
	//   Adds a default level location for the specified game rules. Level system will look up levels here.
	virtual void AddGameRulesLevelLocation(tukk gamerules, tukk mapLocation) = 0;

	// Summary
	//	 Returns the ith map location for the specified game rules
	virtual tukk GetGameRulesLevelLocation(tukk gamerules, i32 i) = 0;

	// Sumarry
	//	 Returns the correct gamerules name from an alias
	virtual tukk GetGameRulesName(tukk alias) const = 0;

	// Summary
	//   Determines if the specified GameRules has been registered
	// Parameters
	//   pRulesName - Name of the GameRules
	virtual bool HaveGameRules(tukk pRulesName) = 0;
