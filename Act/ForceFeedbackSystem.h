...
private:
	...
	struct FFBInternalId
	{
		...
		static FFBInternalId& GetIdForName(tukk name);
	...
	struct SPattern
	{
		float SamplePattern(float time) const
		{
			...
			i32 sampleIdx1 = (i32)fSampleIdx;
			assert((sampleIdx1 >= 0) && (sampleIdx1 < FFSYSTEM_MAX_PATTERN_SAMPLES));
			i32 sampleIdx2 = (sampleIdx1 >= (FFSYSTEM_MAX_PATTERN_SAMPLES - 1)) ? 0 : sampleIdx1 + 1;
		...
		ILINE float SampleToFloat(i32 idx) const
		{
		...
		{
			memset(m_patternSamples, 0, sizeof(u16) * FFSYSTEM_MAX_PATTERN_SAMPLES);
		}
		...
		u16        m_patternSamples[FFSYSTEM_MAX_PATTERN_SAMPLES];
	...
	struct SEnvelope
	{
		...
		ILINE void ResetToDefault()
		{
			memset(m_envelopeSamples, 0, sizeof(u16) * FFSYSTEM_MAX_ENVELOPE_SAMPLES);
		}
		...
		u16        m_envelopeSamples[FFSYSTEM_MAX_ENVELOPE_SAMPLES];
	...
	typedef DrxFixedStringT<256>       TSamplesBuffer;
...
public:
	...
	virtual ForceFeedbackFxId GetEffectIdByName(tukk effectName) const override;
	...
	virtual i32               GetEffectNamesCount() const override;
...
private:
	...
	i32              ParseSampleBuffer(const TSamplesBuffer& buffer, float* outputValues, i32k maxOutputValues);
	void             DistributeSamples(const float* sampleInput, i32k sampleInputCount, u16* sampleOutput, i32k sampleOutputCount);
