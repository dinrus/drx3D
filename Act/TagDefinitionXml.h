...
{
	void SetFilename(tukk const filename)
	{
		assert(filename);
		assert(filename[0]);
	...
	STagDefinitionImportsInfo& AddImport(tukk const filename)
	{
		assert(filename);
		assert(filename[0]);
	...
	{
		for (i32 filterIndex = m_filter.size() - 1; filterIndex >= 0; --filterIndex)
		{
			u32 mappedTagID = (modifiedTagDef.Find(originalTagDef.GetTagCRC(m_filter[filterIndex])));
			if (mappedTagID != TAG_ID_INVALID)
			{
				if (m_filter[filterIndex] != mappedTagID)
				{
					//DrxLog("[TAGDEF]: MapTags() (%s): mapping tag %i [%s] to %i [%s]", m_filename, m_filter[filterIndex], originalTagDef.GetTagName(m_filter[filterIndex]), mappedTagID, modifiedTagDef.GetTagName(mappedTagID));
				}
				m_filter[filterIndex] = mappedTagID;
			}
			else
			{
				//DrxLog("[TAGDEF]: MapTags() (%s): cannot map tag %i [%s] - will be erased", m_filename, m_filter[filterIndex], originalTagDef.GetTagName(m_filter[filterIndex]));
				m_filter.erase(m_filter.begin() + filterIndex);
			}
		}

		for (i32 importsIndex = m_imports.size() - 1; importsIndex >= 0; --importsIndex)
		{
...
namespace mannequin
{
bool                       LoadTagDefinition(tukk const filename, CTagDefinition& tagDefinitionOut, bool isTags);

XmlNodeRef                 SaveTagDefinition(const CTagDefinition& tagDefinition);

STagDefinitionImportsInfo& GetDefaultImportsInfo(tukk const filename);
