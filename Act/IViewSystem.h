...
struct SViewParams
{
	...
	void SetViewID(u8 id, bool blend = true)
	{
		viewID = id;
		if (!blend)
			viewIDLast = id;
...
struct IView
{
	...
	virtual void               SetViewShake(Ang3 shakeAngle, Vec3 shakeShift, float duration, float frequency, float randomness, i32 shakeID, bool bFlipVec = true, bool bUpdateOnly = false, bool bGroundOnly = false) = 0;
	virtual void               SetViewShakeEx(const SShakeParams& params) = 0;
	virtual void               StopShake(i32 shakeID) = 0;
...
struct IViewSystem
{
	...
	virtual void   RemoveView(u32 viewId) = 0;

	virtual void   SetActiveView(IView* pView) = 0;
	virtual void   SetActiveView(u32 viewId) = 0;

	//utility functions
	virtual IView*       GetView(u32 viewId) const = 0;
	virtual IView*       GetActiveView() const = 0;
	virtual bool         IsClientActorViewActive() const = 0;

	virtual u32 GetViewId(IView* pView) const = 0;
	virtual u32 GetActiveViewId() const = 0;

	virtual IView*       GetViewByEntityId(u32 id, bool forceCreate = false) = 0;
	...
	virtual float GetDefaultZNear() const = 0;
