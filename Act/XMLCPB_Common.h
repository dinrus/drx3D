...
//#define XMLCPB_COLLECT_STATS  // remove comment for some manual profiling
	...
	#if DRX_PLATFORM_WINDOWS // consoles dont need it because they already have a built-in integrity check. Also, in the way it is implemented atm, it could have a noticeable performance hit on consoles.
...
typedef u32 AttrSetID; // an "AttrSet" defines the datatype + tagId of all the attributes of a node.
typedef u32 StringID;
typedef u32 NodeLiveID;
typedef u32 NodeGlobalID; // identifies each "real" node, as opposed to "live" nodes. The actual value is just the index of the node in the buffer that stores all of them
typedef u32 FlatAddr;     // is just a 32 bits offset into data buffer.
typedef u16 FlatAddr16;   // is just a 16 bits offset into data buffer.

static u32 const XMLCPB_INVALID_ID = 0xffffffff;
static u32 const XMLCPB_ROOTNODE_ID = 0;
static u32 const XMLCPB_INVALID_FLATADDR = 0xffffffff;
static u32 const XMLCPB_INVALID_SAFECHECK_ID = 0xffffffff;
static u32 const XMLCPB_ZLIB_BUFFER_SIZE = 32 * 1024;        // size for the buffer used in the zlib compression
...
struct SZLibBlockHeader
{
	...
	u32 m_compressedSize;  // when it is = NO_ZLIB_USED, the data is raw, without zlib compression (this should happen only very rarely)
...
struct STypeInfo
{
	...
	tukk   pName; // used only for profiling / debuging / error visualization
};

eAttrDataType                 GetBasicDataType(eAttrDataType type);
u32                        GetDataTypeSize(eAttrDataType type);
tukk                   GetDataTypeName(eAttrDataType type);
void                          InitializeDataTypeInfo();
tukk                   GetConstantString(u32 ind);
...
template<class T> inline bool IsFlagActive(T val, u32 flag)
{
	return (val & flag) != 0;
}
