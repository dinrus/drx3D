...
struct IItem : public IGameObjectExtension
{
	...
	virtual void     EnableUpdate(bool enable, i32 slot = -1) = 0;
	virtual void     RequireUpdate(i32 slot) = 0;

	//-------------------- ACTIONS --------------------
	virtual void ForcePendingActions(u8 blockedActions = 0) = 0;
	...
	virtual void OnHit(float damage, i32 hitType) = 0;
	...
	virtual void         PickUp(EntityId picker, bool sound, bool select = true, bool keepHistory = true, tukk setup = NULL) = 0;
	...
	virtual const string GetAttachedAccessoriesString(tukk separator = ",") = 0;

	virtual void         SetHand(i32 hand) = 0;
	...
	virtual void        SetSubContextID(i32 tagContext) = 0;
	virtual i32         GetSubContextID() = 0;
