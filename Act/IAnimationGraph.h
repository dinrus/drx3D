...
u32k INVALID_ANIMATION_TOKEN = 0xffffffff;
...
extern tuk g_szMCMString[eMCM_COUNT];
...
extern tuk g_szColliderModeString[eColliderMode_COUNT];
...
extern tuk g_szColliderModeLayerString[eColliderModeLayer_COUNT];
...
typedef u8 AnimationGraphInputID;
...
public:
	...
	virtual bool StartAnimation(IEntity* entity, tukk szAnimName0, const DrxCharAnimationParams& Params, float speedMultiplier = 1.0f)
	{
	...
	virtual float GetTopAnimExpectedSecondsLeft(IEntity* entity, i32 layer)
	{
		ICharacterInstance* pICharacter = entity->GetCharacter(0);
		if (!pICharacter)
			return -1.f;
	...
	virtual float GetTopAnimNormalizedTime(IEntity* entity, i32 layer)
	{
		ICharacterInstance* pICharacter = entity->GetCharacter(0);
		if (!pICharacter)
			return -1.f;
	...
	virtual const CAnimation* GetTopAnimation(IEntity* entity, i32 layer)
	{
		ICharacterInstance* pICharacter = entity->GetCharacter(0);
		if (pICharacter)
		{
...
struct SAnimationTarget
{
	...
	u32         preparing              : 1;
	u32         activated              : 1;
	u32         doingSomething         : 1;
	mutable u32 allowActivation        : 1;
	mutable u32 notAiControlledAnymore : 1;
	u32         isNavigationalSO       : 1;
	float          maxRadius;
	Vec3           position;
	float          startWidth;
	float          positionWidth;
...
public:
	...
	{
		for (i32 index = 0; index < m_numParams; ++index)
		{
...
private:
	...
	u8 m_motionParameterID[MAX_PREDICTION_PARAMS];
	u8 m_numParams;
};
...
struct IAnimationGraphAuxillaryInputs
{
	...
	virtual void                  SetInput(InputID, i32) = 0;
	virtual void                  SetInput(InputID, tukk ) = 0;

	template<class T>
	inline void SetInput(tukk name, T value);
};
...
public:
	...
	template<class T>
	inline bool SetInput(tukk name, T value, TAnimationGraphQueryID* pQueryID = 0)
	{
	...
	inline bool SetInputOptional(tukk name, tukk value, TAnimationGraphQueryID* pQueryID = 0)
	{
		return SetInputOptional(GetInputId(name), value, pQueryID);
	}
	inline void QueryChangeInput(tukk name, TAnimationGraphQueryID* pQueryID)
	{
		QueryChangeInput(GetInputId(name), pQueryID);
	}
	inline void LockInput(tukk name, bool locked)
	{
	...
	virtual bool        SetInput(InputID, i32, TAnimationGraphQueryID * pQueryID = 0) = 0;
	virtual bool        SetInput(InputID, tukk , TAnimationGraphQueryID * pQueryID = 0) = 0;
	// SetInputOptional is same as SetInput except that it will not set the default input value in case a non-existing value is passed
	virtual bool        SetInputOptional(InputID, tukk , TAnimationGraphQueryID * pQueryID = 0) = 0;
	virtual void        ClearInput(InputID) = 0;
	virtual void        LockInput(InputID, bool locked) = 0;

	virtual bool        SetVariationInput(tukk name, tukk value) = 0;
	virtual bool        SetVariationInput(InputID inputID, tukk value) = 0;
	virtual tukk GetVariationInput(InputID inputID) const = 0;

	// assert all equal, use any (except if signalled, then return the one not equal to default, or return default of all default)
	virtual void GetInput(InputID, tuk) const = 0;

	// get input from specific layer if layer index is valid for this state
	virtual void GetInput(InputID, tuk, i32 layerIndex) const = 0;

	// AND all layers
	virtual bool IsDefaultInputValue(InputID) const = 0;

	// returns NULL if InputID is out of range
	virtual tukk GetInputName(InputID) const = 0;
	virtual tukk GetVariationInputName(InputID) const = 0;
	...
	virtual void AddListener(tukk name, IAnimationGraphStateListener* pListener) = 0;
	...
	virtual void SetAnimatedCharacter(class CAnimatedCharacter* animatedCharacter, i32 layerIndex, IAnimationGraphState* parentLayerState) = 0;
	...
	virtual void PushForcedState(tukk state, TAnimationGraphQueryID* pQueryID = 0) = 0;
	...
	virtual InputID GetInputId(tukk input) = 0;
	virtual InputID GetVariationInputId(tukk variationInputName) const = 0;
	...
	virtual tukk GetCurrentStateName() = 0;
	...
	//virtual void SetOutput( i32 id ) = 0;
	//virtual void ClearOutput( i32 id ) = 0;
	...
	virtual tukk QueryOutput(tukk name) = 0;

	// Don't expose (only used on specific layer in AGParams)
	//virtual void SetCurrentStructure( const CDrxName& ) = 0;
	//virtual const CDrxName& GetCurrentStructure() = 0;

	// don't expose (exact positioning uses fullbody layer only, and AGAnimation operates on specific layer already)
	//virtual u32 GetCurrentToken() = 0;
	...
	virtual IAnimationGraphExistanceQuery* CreateExistanceQuery(i32 layer) = 0;
	...
	//virtual u16 GetBlendSpaceWeightFlags() = 0;
	//virtual void  SetBlendSpaceWeightFlags(u16 flags) = 0;
...
template<class T>
inline void IAnimationGraphAuxillaryInputs::SetInput(tukk name, T value)
{
...
struct IAnimationGraphStateListener
{
	...
	virtual void SetOutput(tukk output, tukk value) = 0;
