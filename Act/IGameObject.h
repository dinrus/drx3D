...
static i32k MAX_UPDATE_SLOTS_PER_EXTENSION = 5;
...
protected:
	...
	public:
		...
		static CRMIBodyImpl* Create(const SGameObjectExtensionRMI* method, EntityId id, const T& params, IRMIListener* pListener, i32 userId, EntityId dependentId)
		{
	...
	private:
		...
		CRMIBodyImpl(const SGameObjectExtensionRMI* method, EntityId id, const T& params, IRMIListener* pListener, i32 userId, EntityId dependentId) :
...
public:
	...
	virtual IGameObjectExtension* QueryExtension(tukk extension) const = 0;
	virtual IGameObjectExtension* QueryExtension(IGameObjectSystem::ExtensionID id) const = 0;

	// set extension parameters
	virtual bool                  SetExtensionParams(tukk extension, SmartScriptTable params) = 0;
	// get extension parameters
	virtual bool                  GetExtensionParams(tukk extension, SmartScriptTable params) = 0;
	...
	virtual void                  ForceUpdateExtension(IGameObjectExtension* pGOE, i32 slot) = 0;
	...
	virtual IGameObjectExtension* GetExtensionWithRMIBase(ukk pBase) = 0;
	virtual void                  EnablePrePhysicsUpdate(EPrePhysicsUpdate updateRule) = 0;
	virtual void                  Pulse(u32 pulse) = 0;
	virtual void                  RegisterAsPredicted() = 0;
	virtual void                  RegisterAsValidated(IGameObject* pGO, i32 predictionHandle) = 0;
	virtual i32                   GetPredictionHandle() = 0;

	virtual void                  RegisterExtForEvents(IGameObjectExtension* piExtention, i32k* pEvents, i32k numEvents) = 0;
	virtual void                  UnRegisterExtForEvents(IGameObjectExtension* piExtention, i32k* pEvents, i32k numEvents) = 0;

	virtual void                  ChangedNetworkState(NetworkAspectType aspects) = 0;

	// enable/disable sending physics events to this game object
	virtual void EnablePhysicsEvent(bool enable, i32 events) = 0;
	virtual bool WantsPhysicsEvent(i32 events) = 0;
	...
	void InvokeRMIWithDependentObject(const MI method, const T& params, unsigned where, EntityId ent, i32 channel = -1)
	{
		InvokeRMI_Primitive(method, params, where, 0, 0, channel, ent);
	}

	template<class MI, class T>
	void InvokeRMI(const MI method, const T& params, unsigned where, i32 channel = -1)
	{
		InvokeRMI_Primitive(method, params, where, 0, 0, channel, 0);
	}

	template<class MI, class T>
	void InvokeRMI_Primitive(const MI method, const T& params, unsigned where, IRMIListener* pListener, i32 userId, i32 channel, EntityId dependentId)
	{
	...
	ILINE bool                  ActivateExtension(tukk extension)   { return ChangeExtension(extension, eCE_Activate) != 0; }
	// turn an extension off
	ILINE void                  DeactivateExtension(tukk extension) { ChangeExtension(extension, eCE_Deactivate); }
	// forcefully get a pointer to an extension (may instantiate if needed)
	ILINE IGameObjectExtension* AcquireExtension(tukk extension)    { return ChangeExtension(extension, eCE_Acquire); }
	// release a previously acquired extension
	ILINE void                  ReleaseExtension(tukk extension)    { ChangeExtension(extension, eCE_Release); }
	...
	virtual void                       EnableUpdateSlot(IGameObjectExtension* pExtension, i32 slot) = 0;
	virtual void                       DisableUpdateSlot(IGameObjectExtension* pExtension, i32 slot) = 0;
	virtual u8                      GetUpdateSlotEnables(IGameObjectExtension* pExtension, i32 slot) = 0;
	...
	virtual void                       SetUpdateSlotEnableCondition(IGameObjectExtension* pExtension, i32 slot, EUpdateEnableCondition condition) = 0;
	...
	virtual void  SetUserData(uk ptr) = 0;
#endif
...
private:
	...
	virtual IGameObjectExtension* ChangeExtension(tukk extension, EChangeExtension change) = 0;
};
		...
		if (IGameObject* pGameObject = gEnv->pGameFramework->GetGameObject(m_id))
		{
			...
			else
			{
				DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_ERROR,
					...
					(u32)(TRUNCATE_PTR)m_pRMI->pBase, pGameObject->GetEntity()->GetName(),
					m_pRMI->pMsgDef->description);
			}
		}
		else
		{
			drx_sprintf(msg, "Entity %u for RMI %s not found", m_id, m_pRMI->pMsgDef->description);
		}
...
{
public:
	static void GetGameObjectExtensionRMIData(uk * ppRMI, size_t* nCount)
	{
		*ppRMI = ms_statics.m_vMessages;
		*nCount = ms_statics.m_nMessages;
	}

	ukk GetRMIBase() const
	{
...
protected:
	...
	static void ActivateOutputPort(EntityId id, i32 port, const TFlowInputData& data)
	{
		SEntityEvent evnt;
	...
	static const SGameObjectExtensionRMI* Helper_AddMessage(SGameObjectExtensionRMI::DecoderFunction decoder, tukk description, ERMIAttachmentType attach, bool isServerCall, ENetReliabilityType reliability, bool lowDelay)
	{
		if (ms_statics.m_nMessages >= MAX_STATIC_MESSAGES)
		{
			// Assert or DrxFatalError here uses gEnv, which is not yet initialized.
			__debugbreak();
			((void (*)())NULL)();
			return NULL;
		}
...
// Declare game object as deprecated except for the legacy modules that expose functionality for it
	...
	&& eDrxModule != eDrxM_FlowGraph && eDrxModule != eDrxM_Editor && eDrxModule != eDrxM_Movie     \
	&& eDrxModule != eDrxM_Legacy && eDrxModule != eDrxM_Schematyc2)

	#define DRX_DEPRECATED_GAMEOBJECT DRX_DEPRECATED("(v5.4) IGameObjectExtension has been replaced with IEntityComponent, and will be removed in a future update.")

#else
	#define DRX_DEPRECATED_GAMEOBJECT
#endif
...
{
	DRX_DEPRECATED_GAMEOBJECT IGameObjectExtension() : m_pGameObject(0) {}
	...
	//               initialization
	virtual void InitClient(i32 channelId) = 0;
	...
	//   ISerialize
	virtual void FullSerialize(TSerialize ser) = 0;
	virtual bool NetSerialize(TSerialize ser, EEntityAspects aspect, u8 profile, i32 pflags) override = 0;
	...
	virtual void Update(SEntityUpdateContext& ctx, i32 updateSlot) = 0;
	...
	virtual void SetChannelId(u16 id) = 0;
	...
	virtual ukk GetRMIBase() const = 0;
...
#define CHANGED_NETWORK_STATE(object, aspects)     do { /* IEntity * pEntity = object->GetGameObject()->GetEntity(); DrxLogAlways("%s changed aspect %x (%s %d)", pEntity ? pEntity->GetName() : "NULL", aspects, __FILE__, __LINE__); */ object->GetGameObject()->ChangedNetworkState(aspects); } while (0)
#define CHANGED_NETWORK_STATE_GO(object, aspects)  do { /* IEntity * pEntity = object->GetEntity(); DrxLogAlways("%s changed aspect %x (%s %d)", pEntity ? pEntity->GetName() : "NULL", aspects, __FILE__, __LINE__); */ object->ChangedNetworkState(aspects); } while (0)
#define CHANGED_NETWORK_STATE_REF(object, aspects) do { /* IEntity * pEntity = object.GetGameObject()->GetEntity(); DrxLogAlways("%s changed aspect %x (%s %d)", pEntity ? pEntity->GetName() : "NULL", aspects, __FILE__, __LINE__); */ object.GetGameObject()->ChangedNetworkState(aspects); } while (0)
