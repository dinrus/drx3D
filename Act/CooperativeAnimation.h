...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: synchronized animation playing on multiple actors
...
public:
	...
	ILINE bool IsUsingActor(const IAnimatedCharacter* pActor) const { DRX_ASSERT(pActor); return pActor && IsUsingActor(pActor->GetEntityId()); }
...
private:
	...
	bool StartAnimation(SCharacterParams& params, i32 characterIndex);
