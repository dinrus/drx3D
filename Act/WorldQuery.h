...
private:
	...
	struct SRayInfo
	{
		...
		void Reset()
		{
			if (rayId != 0)
			{
				CDrxAction::GetDrxAction()->GetPhysicQueues().GetRayCaster().Cancel(rayId);
				rayId = 0;
			}
			counter = 0;
		}
...
public:
	...
	ILINE const EntityId* ProximityQuery(i32& numberOfEntities)
	{
	...
	ILINE const EntityId* GetEntitiesAround(i32& num)
	{
	...
	ILINE IPhysicalEntity* const* GetPhysicalEntitiesAround(i32& num)
	{
...
private:
...
#if WORLDQUERY_USE_DEFERRED_LINETESTS
	i32 GetRaySlot();
	i32 GetSlotForRay(const QueuedRayID& rayId) const;

	static i32k kMaxQueuedRays = 6;
	...
	i32                           m_physicalEntityInFrontOf;

	ILINE void ValidateQuery(EWorldQuery query)
	{
		u32 queryMask = 1u << query;
