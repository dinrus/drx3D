...
typedef boost::mpl::vector<int8, u8, i16, u16, i32, u32, int64, uint64, f32, f64, string, Vec3, Matrix34, Quat, QuatT> TBasicTypes;
typedef boost::make_variant_over<TBasicTypes>::type                                                                                 TBasicType;

template<typename T>
static inline T ComposeValue(u8k* data, size_t size)
{
	DRX_ASSERT(sizeof(T) == size);
	T t = T();
	memcpy(&t, data, size);
	return t;
}
...
template<>
inline string ComposeValue<string>(u8k* data, size_t size)
{
	return string((tukk)data, size);
}
...
static inline TBasicType ComposeValue(const IMetadata* metadata)
{
	...
	case eBT_i08:
		...
		break;
	case eBT_u08:
		v = TBasicType(ComposeValue<u8>(dt, sz));
		break;
	case eBT_i16:
		v = TBasicType(ComposeValue<i16>(dt, sz));
		break;
	case eBT_u16:
		v = TBasicType(ComposeValue<u16>(dt, sz));
		break;
	case eBT_i32:
		v = TBasicType(ComposeValue<i32>(dt, sz));
		break;
	case eBT_u32:
		v = TBasicType(ComposeValue<u32>(dt, sz));
		break;
	case eBT_i64:
...
public:
	...
	const CCompositeData& GetField(u32 tag, size_t count = 1) const
	{
		size_t n = 0;
