...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Implements a seat action to control vehicle weapons
...
public:
	...
	virtual void OnFireModeChanged(IWeapon* pWeapon, i32 currentFireMode) override                     {}
	...
	u32    GetWeaponCount() const;
	u32    GetCurrentWeapon() const { return m_weaponIndex; }

	EntityId        GetCurrentWeaponEntityId() const;
	EntityId        GetWeaponEntityId(u32 index) const;
	void            ClSetupWeapon(u32 index, EntityId weaponId);

	IItem*          GetIItem(u32 index);
	IWeapon*        GetIWeapon(u32 index);
	...
	void OnWeaponRespawned(i32 weaponIndex, EntityId newWeaponEntityId);
	void OnVehicleActionControllerDeleted();

protected:

	IEntity*        SpawnWeapon(SVehicleWeapon& weapon, IEntity* pVehicleEntity, const string& partName, i32 weaponIndex);
	void            SetupWeapon(SVehicleWeapon& weapon);

	SVehicleWeapon& GetVehicleWeapon();
	SVehicleWeapon& GetWeaponInfo(i32 weaponIndex);
...
protected:
	...
	i32                  m_lastActionActivationMode;
	i32                  m_weaponIndex;
