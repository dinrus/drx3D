...
struct IFireMode
{
	...
	virtual void Update(float frameTime, u32 frameId) = 0;
	...
	virtual tukk   GetCrosshair() const { return ""; };
	virtual bool          CanOverheat() const = 0;
	virtual Vec3          ApplySpread(const Vec3& dir, float spread, i32 quadrant = -1) const = 0;
	...
	virtual void          SetProjectileLaunchParams(const SProjectileLaunchParams& launchParams) { DRX_ASSERT_MESSAGE(0, "Firemode does not handle launch params"); }

	virtual void          NetShoot(const Vec3& hit, i32 predictionHandle) = 0;
	virtual void          NetShootEx(const Vec3& pos, const Vec3& dir, const Vec3& vel, const Vec3& hit, float extra, i32 predictionHandle) = 0;
	...
	virtual i32           GetCurrentBarrel() const = 0;
...
struct IZoomMode
{
	...
	virtual bool       StartZoom(bool stayZoomed = false, bool fullZoomOut = true, i32 zoomStep = 1) = 0;
	virtual void       StopZoom() = 0;
	virtual void       ExitZoom(bool force = false) = 0;

	virtual i32        GetCurrentStep() const = 0;
	virtual float      GetZoomFoVScale(i32 step) const = 0;
	...
	virtual i32        GetMaxZoomSteps() const = 0;
...
struct IWeaponEventListener
{
	...
	virtual void OnFireModeChanged(IWeapon* pWeapon, i32 currentFireMode) = 0;
...
struct IWeapon
{
	...
	virtual void                  OnAction(EntityId actorId, const ActionId& actionId, i32 activationMode, float value) = 0;

	virtual void                  SetFiringLocator(IWeaponFiringLocator* pLocator) = 0;
	virtual IWeaponFiringLocator* GetFiringLocator() const = 0;

	virtual void                  AddEventListener(IWeaponEventListener* pListener, tukk who) = 0;
	...
	// See Also
	//   CanZoom
	virtual void StartZoom(EntityId shooterId, i32 zoomed = 1) = 0;
	...
	virtual i32 GetNumOfFireModes() const = 0;
	...
	virtual bool                      HasAttachmentAtHelper(tukk helper) = 0;
	virtual void                      GetAttachmentsAtHelper(tukk helper, CDrxFixedStringListT<5, 30>& rAttachments) = 0;
	...
	virtual bool Query(EWeaponQuery query, ukk param = NULL) = 0;
	...
	virtual void       RequestShoot(IEntityClass* pAmmoType, const Vec3& pos, const Vec3& dir, const Vec3& vel, const Vec3& hit, float extra, i32 predictionHandle, bool forceExtended) = 0;
	...
	virtual i32          GetInventoryAmmoCount(IEntityClass* pAmmoType) const = 0;
	virtual void         SetInventoryAmmoCount(IEntityClass* pAmmoType, i32 count) = 0;

	virtual i32          GetMaxZoomSteps() = 0;
	...
	//virtual void AdjustPosition(float time, bool zoomIn, i32 currentStep) = 0;
