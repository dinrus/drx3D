...
// Entity Group Upr. Used for managing groups of entities
...
public:
	...
	bool     CreateContainer(EntityId containerId, tukk szContainerName = nullptr);             // szContainerName is stored as tukk  , so do not pass temporary strings
	...
	// Group Action
	bool RunModule(EntityId containerId, tukk moduleName); // Executes a Flowgraph module for each entity in the selected container
	...
	void RegisterListener(IEntityContainerListener* pListener, tukk szName, bool bStaticName = true) { m_listeners.Add(pListener, szName, bStaticName); }
