...
public:
	...
	inline IUIElement*      GetInstance(i32 instanceID);
...
private:
	...
	inline const typename SUIDescTypeOf<Type>::TType * UpdateObjectDescInt(Item * parent, const SUIArguments &path, i32 depth)
	{
		if (!parent)
			return NULL;
		...
		if (depth < path.GetArgCount() - 1)
			return UpdateObjectDescInt<eUOT_MovieClip>(SUIGetDesc<ParentType, Item, tukk >::GetDesc(parent, name.c_str()), path, depth + 1);
		return SUIGetDesc<Type, Item, tukk >::GetDesc(parent, name.c_str());
	}
...
public:
	...
	tukk          GetCategory() const                { return m_category.c_str(); }
	virtual void         GetMemoryUsage(IDrxSizer* s) const { s->Add(*this); }
...
protected:
	...
	void GetDynInput(SUIArguments& args, const SUIParameterDesc& desc, SActivationInfo* pActInfo, i32 port);
	void ActivateDynOutput(const TUIData& arg, const SUIParameterDesc& desc, SActivationInfo* pActInfo, i32 port);
...
{
public:
	CFlashUiFlowNodeFactory(tukk sClassName, IFlowNodePtr pFlowNode) : m_sClassName(sClassName), m_pFlowNode(pFlowNode) {}
	IFlowNodePtr Create(IFlowNode::SActivationInfo* pActInfo) { return m_pFlowNode->Clone(pActInfo); }
	void         GetMemoryUsage(IDrxSizer* s) const           { SIZER_SUBCOMPONENT_NAME(s, "CFlashUiFlowNodeFactory"); }
	void         Reset()                                      {}

	tukk  GetNodeTypeName() const { return m_sClassName; }
...
{
public:
	static i32  GetTotalStackSize() { return m_Size; }
#ifdef ENABLE_UISTACK_DEBUGGING
	static void Add(i32 count, tukk dgbInfo, i32 id)
	{
		m_Size += count;
		for (i32 i = 0; i < count; ++i)
			m_debugInfo[id] = dgbInfo;
	}
	static void Remove(i32 count, i32 id)
	{
		m_Size -= count;
		for (i32 i = 0; i < count; ++i)
		{
	...
	static const std::map<i32, tukk >& GetStack()                { return m_debugInfo; }
	static i32                               GetNextId()               { static i32 id = 0; return id++; }
#else
	static void                              Add(i32 count = 1)        { m_Size += count; }
	static void                              Remove(i32 count = 1)     { m_Size -= count; assert(m_Size >= 0); }
#endif
...
#ifdef ENABLE_UISTACK_DEBUGGING
	static std::map<i32, tukk > m_debugInfo;
#endif
};
...
public:
...
#ifdef ENABLE_UISTACK_DEBUGGING
	inline void push(const T& val, tukk fmt, ...)
	{
	...
	inline i32      size() const { return m_Impl.size(); }
...
private:
	...
	{
		DRX_ASSERT_MESSAGE(m_isUIAction != -1, "Stack was not intialized");
...
private:
	std::deque<T>                     m_Impl;
#ifdef ENABLE_UISTACK_DEBUGGING
	std::list<std::pair<string, i32>> m_debugInfo;
#endif
	i32                               m_isUIAction;
	IFlowGraph*                       m_pGraph;
