...
public:
	...
	bool GetMetaData(tukk fileName, SCheckpointData& metaData, bool bRepairId = true);
	...
	virtual bool LoadGame(tukk fileName);

	//save a game checkpoint with the specified filename
	virtual bool SaveGame(EntityId checkpointId, tukk fileName);
	...
	bool SaveXMLNode(XmlNodeRef node, tukk identifier);

	// use this to load XMLNodes during OnLoad
	XmlNodeRef LoadXMLNode(tukk identifier);
...
private:
	...
	void WriteXML(XmlNodeRef data, tukk fileName);
	...
	bool LoadCheckpointMap(const SCheckpointData& metaData, DrxFixedStringT<32>& curLevelName);
	...
	XmlNodeRef ReadXML(tukk fileName);
	...
	static void ParseSavedFiles(std::vector<string>& saveNames, tukk pLevelName = NULL);
	...
	bool RepairEntityId(EntityId& id, tukk pEntityName);

	//this removes sub-directories from a level name
	void RepairLevelName(DrxFixedStringT<32>& levelName);
