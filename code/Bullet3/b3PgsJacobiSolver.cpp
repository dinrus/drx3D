//enable D3_SOLVER_DEBUG if you experience solver crashes
//#define D3_SOLVER_DEBUG
...
static b3Vec3 getVelocityInLocalPoint(b3RigidBodyData* rb, const b3Vec3& rel_pos)
{
...
{
	b3Vec3 m_positionWorldOnA;
	b3Vec3 m_positionWorldOnB;
	b3Vec3 m_normalWorldOnB;
	...
	b3Vec3 m_lateralFrictionDir1;
	b3Vec3 m_lateralFrictionDir2;
	...
	b3Vec3 getPositionWorldOnA()
	{
		return m_positionWorldOnA;
	}
	b3Vec3 getPositionWorldOnB()
	{
...
void getContactPoint(b3Contact4* contact, i32 contactIndex, b3ContactPoint& pointOut)
{
	...
	b3Vec3 normalOnB = contact->m_worldNormalOnB;
...
void b3PgsJacobiSolver::solveContacts(i32 numBodies, b3RigidBodyData* bodies, b3InertiaData* inertias, i32 numContacts, b3Contact4* contacts, i32 numConstraints, b3TypedConstraint** constraints)
{
...
i32 b3PgsJacobiSolver::b3RandInt2(i32 n)
{
...
void b3PgsJacobiSolver::initSolverBody(i32 bodyIndex, b3SolverBody* solverBody, b3RigidBodyData* rb)
{
...
void b3PgsJacobiSolver::setupFrictionConstraint(b3RigidBodyData* bodies, b3InertiaData* inertias, b3SolverConstraint& solverConstraint, const b3Vec3& normalAxis, i32 solverBodyIdA, i32 solverBodyIdB, b3ContactPoint& cp, const b3Vec3& rel_pos1, const b3Vec3& rel_pos2, b3RigidBodyData* colObj0, b3RigidBodyData* colObj1, b3Scalar relaxation, b3Scalar desiredVelocity, b3Scalar cfmSlip)
{
	...
	{
		b3Vec3 ftorqueAxis1 = rel_pos1.cross(solverConstraint.m_contactNormal);
	...
	{
		b3Vec3 ftorqueAxis1 = rel_pos2.cross(-solverConstraint.m_contactNormal);
...
b3SolverConstraint& b3PgsJacobiSolver::addFrictionConstraint(b3RigidBodyData* bodies, b3InertiaData* inertias, const b3Vec3& normalAxis, i32 solverBodyIdA, i32 solverBodyIdB, i32 frictionIndex, b3ContactPoint& cp, const b3Vec3& rel_pos1, const b3Vec3& rel_pos2, b3RigidBodyData* colObj0, b3RigidBodyData* colObj1, b3Scalar relaxation, b3Scalar desiredVelocity, b3Scalar cfmSlip)
{
...
void b3PgsJacobiSolver::setupRollingFrictionConstraint(b3RigidBodyData* bodies, b3InertiaData* inertias, b3SolverConstraint& solverConstraint, const b3Vec3& normalAxis1, i32 solverBodyIdA, i32 solverBodyIdB,
...
{
	b3Vec3 normalAxis = b3MakeVector3(0, 0, 0);
	...
	{
		b3Vec3 ftorqueAxis1 = -normalAxis1;
		solverConstraint.m_relpos1CrossNormal = ftorqueAxis1;
		solverConstraint.m_angularComponentA = body0 ? getInvInertiaTensorWorld(&inertias[solverBodyA.m_originalBodyIndex]) * ftorqueAxis1 : b3MakeVector3(0, 0, 0);
	}
	{
	...
	{
		b3Vec3 iMJaA = body0 ? getInvInertiaTensorWorld(&inertias[solverBodyA.m_originalBodyIndex]) * solverConstraint.m_relpos1CrossNormal : b3MakeVector3(0, 0, 0);
		b3Vec3 iMJaB = body1 ? getInvInertiaTensorWorld(&inertias[solverBodyB.m_originalBodyIndex]) * solverConstraint.m_relpos2CrossNormal : b3MakeVector3(0, 0, 0);
		b3Scalar sum = 0;
...
b3SolverConstraint& b3PgsJacobiSolver::addRollingFrictionConstraint(b3RigidBodyData* bodies, b3InertiaData* inertias, const b3Vec3& normalAxis, i32 solverBodyIdA, i32 solverBodyIdB, i32 frictionIndex, b3ContactPoint& cp, const b3Vec3& rel_pos1, const b3Vec3& rel_pos2, b3RigidBodyData* colObj0, b3RigidBodyData* colObj1, b3Scalar relaxation, b3Scalar desiredVelocity, b3Scalar cfmSlip)
{
...
i32 b3PgsJacobiSolver::getOrInitSolverBody(i32 bodyIndex, b3RigidBodyData* bodies, b3InertiaData* inertias)
{
	//drx3DAssert(bodyIndex< m_tmpSolverBodyPool.size());
	...
	{
		drx3DAssert(m_bodyCount[bodyIndex] > 0);
...
void b3PgsJacobiSolver::setupContactConstraint(b3RigidBodyData* bodies, b3InertiaData* inertias, b3SolverConstraint& solverConstraint,
											   ...
											   b3Vec3& vel, b3Scalar& rel_vel, b3Scalar& relaxation,
											   b3Vec3& rel_pos1, b3Vec3& rel_pos2)
{
	const b3Vec3& pos1 = cp.getPositionWorldOnA();
	const b3Vec3& pos2 = cp.getPositionWorldOnB();
	...
	//			b3Vec3 rel_pos1 = pos1 - colObj0->getWorldTransform().getOrigin();
	//			b3Vec3 rel_pos2 = pos2 - colObj1->getWorldTransform().getOrigin();
	rel_pos1 = pos1 - bodyA->getWorldTransform().getOrigin();
	rel_pos2 = pos2 - bodyB->getWorldTransform().getOrigin();

	relaxation = 1.f;

	b3Vec3 torqueAxis0 = rel_pos1.cross(cp.m_normalWorldOnB);
	...
	b3Vec3 torqueAxis1 = rel_pos2.cross(cp.m_normalWorldOnB);
...

	{
		...
		//			b3Vec3 vel2 = rb1 ? rb1->getVelocityInLocalPoint(rel_pos2) : b3Vec3(0,0,0);
		vel = vel1 - vel2;
	...
	if (infoGlobal.m_solverMode & D3_SOLVER_USE_WARMSTARTING)
	{
													 ...
													 b3ContactPoint& cp, const b3ContactSolverInfo& infoGlobal)
{
	...
	if ((infoGlobal.m_solverMode & D3_SOLVER_USE_2_FRICTION_DIRECTIONS))
	{
...
void b3PgsJacobiSolver::convertContact(b3RigidBodyData* bodies, b3InertiaData* inertias, b3Contact4* manifold, const b3ContactSolverInfo& infoGlobal)
{
	...
	i32 solverBodyIdA = getOrInitSolverBody(manifold->getBodyA(), bodies, inertias);
	i32 solverBodyIdB = getOrInitSolverBody(manifold->getBodyB(), bodies, inertias);
	...
	i32 numContacts = getNumContacts(manifold);
	for (i32 j = 0; j < numContacts; j++)
	{
		...
		if (cp.getDistance() <= getContactProcessingThreshold(manifold))
		{
			...
			///You can also enable two friction directions, using the D3_SOLVER_USE_2_FRICTION_DIRECTIONS.
			///In that case, the second friction direction will be orthogonal to both contact normal and first friction direction.
			///
			///If you choose D3_SOLVER_DISABLE_VELOCITY_DEPENDENT_FRICTION_DIRECTION, then the friction will be independent from the relative projected velocity.
			///
			...
			///
			if (!(infoGlobal.m_solverMode & D3_SOLVER_ENABLE_FRICTION_DIRECTION_CACHING) || !cp.m_lateralFrictionInitialized)
			{
				cp.m_lateralFrictionDir1 = vel - cp.m_normalWorldOnB * rel_vel;
				b3Scalar lat_rel_vel = cp.m_lateralFrictionDir1.length2();
				if (!(infoGlobal.m_solverMode & D3_SOLVER_DISABLE_VELOCITY_DEPENDENT_FRICTION_DIRECTION) && lat_rel_vel > D3_EPSILON)
				{
...
b3Scalar b3PgsJacobiSolver::solveGroupCacheFriendlySetup(b3RigidBodyData* bodies, b3InertiaData* inertias, i32 numBodies, b3Contact4* manifoldPtr, i32 numManifolds, b3TypedConstraint** constraints, i32 numConstraints, const b3ContactSolverInfo& infoGlobal)
{
	D3_PROFILE("solveGroupCacheFriendlySetup");
	...
	{
		i32 bodyIndexA = constraints[i]->getRigidBodyA();
		i32 bodyIndexB = constraints[i]->getRigidBodyB();
		if (m_usePgs)
		{
	...
	{
		{
			...
			for (i = 0; i < numConstraints; i++)
			{
				...
				{
					drx3DAssert(currentRow < totalNumRows);
					...
					i32 solverBodyIdA = getOrInitSolverBody(constraint->getRigidBodyA(), bodies, inertias);
					i32 solverBodyIdB = getOrInitSolverBody(constraint->getRigidBodyB(), bodies, inertias);
					...
					drx3DAssert(info2.rowskip * sizeof(b3Scalar) == sizeof(b3SolverConstraint));
...
b3Scalar b3PgsJacobiSolver::solveSingleIteration(i32 iteration, b3TypedConstraint** constraints, i32 numConstraints, const b3ContactSolverInfo& infoGlobal)
{
	i32 numNonContactPool = m_tmpSolverNonContactConstraintPool.size();
	i32 numConstraintPool = m_tmpSolverContactConstraintPool.size();
	i32 numFrictionPool = m_tmpSolverContactFrictionConstraintPool.size();

	if (infoGlobal.m_solverMode & D3_SOLVER_RANDMIZE_ORDER)
	{
	...
	if (infoGlobal.m_solverMode & D3_SOLVER_SIMD)
	{
		...
		if (iteration < infoGlobal.m_numIterations)
		{
			...
			if (infoGlobal.m_solverMode & D3_SOLVER_INTERLEAVE_CONTACT_AND_FRICTION_CONSTRAINTS)
			{
				...
				i32 multiplier = (infoGlobal.m_solverMode & D3_SOLVER_USE_2_FRICTION_DIRECTIONS) ? 2 : 1;
			...
			else  //D3_SOLVER_INTERLEAVE_CONTACT_AND_FRICTION_CONSTRAINTS
			{
	...
	else
	{
		...
		if (iteration < infoGlobal.m_numIterations)
		{
			...
			i32 numFrictionPoolConstraints = m_tmpSolverContactFrictionConstraintPool.size();
			for (i32 j = 0; j < numFrictionPoolConstraints; j++)
			{
				b3SolverConstraint& solveManifold = m_tmpSolverContactFrictionConstraintPool[m_orderFrictionConstraintPool[j]];
				b3Scalar totalImpulse = m_tmpSolverContactConstraintPool[solveManifold.m_frictionIndex].m_appliedImpulse;
...
void b3PgsJacobiSolver::solveGroupCacheFriendlySplitImpulseIterations(b3TypedConstraint** constraints, i32 numConstraints, const b3ContactSolverInfo& infoGlobal)
{
	i32 iteration;
...
b3Scalar b3PgsJacobiSolver::solveGroupCacheFriendlyIterations(b3TypedConstraint** constraints, i32 numConstraints, const b3ContactSolverInfo& infoGlobal)
{
	D3_PROFILE("solveGroupCacheFriendlyIterations");
...
{
	D3_PROFILE("averaging");
	//average the velocities
	i32 numBodies = m_bodyCount.size();
	...
	for (i32 i = 0; i < m_tmpSolverBodyPool.size(); i++)
	{
		...
		{
			i32 orgBodyIndex = m_tmpSolverBodyPool[i].m_originalBodyIndex;
	...
	for (i32 i = 0; i < m_tmpSolverBodyPool.size(); i++)
	{
		...
		if (!m_tmpSolverBodyPool[i].m_invMass.isZero())
		{
			drx3DAssert(m_bodyCount[orgBodyIndex] == m_bodyCountCheck[orgBodyIndex]);
...
b3Scalar b3PgsJacobiSolver::solveGroupCacheFriendlyFinish(b3RigidBodyData* bodies, b3InertiaData* inertias, i32 numBodies, const b3ContactSolverInfo& infoGlobal)
{
	...
	{
		D3_PROFILE("write back velocities and transforms");
		for (i = 0; i < m_tmpSolverBodyPool.size(); i++)
		{
			...
			if (body->m_invMass)
			{
				...
				else
				{
					...
					b3Vec3 deltaLinVel = m_deltaLinearVelocities[bodyIndex] * factor;
					b3Vec3 deltaAngVel = m_deltaAngularVelocities[bodyIndex] * factor;
					//printf("body %d\n",bodyIndex);
