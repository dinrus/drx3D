...
public:
	...
	public:
		...
		i32 compare(const Rational64& b) const;

		b3Scalar toScalar() const
		{
			return sign * ((m_denominator == 0) ? D3_INFINITY : (b3Scalar)m_numerator / m_denominator);
		}
	};
	...
	public:
		...
		i32 compare(const Rational128& b) const;

		i32 compare(Int64_t b) const;

		b3Scalar toScalar() const
		{
			return sign * ((denominator.getSign() == 0) ? D3_INFINITY : numerator.toScalar() / denominator.toScalar());
		}
	};
	...
	public:
		...
		void print()
		{
			drx3DPrintf("V%d (%d, %d, %d)", point.index, point.x, point.y, point.z);
		}
		...
		void receiveNearbyFaces(Vertex* src)
		{
			...
			{
				drx3DAssert(f->nearbyVertex == src);
				f->nearbyVertex = this;
			}
	...
	public:
		...
		void link(Edge* n)
		{
			drx3DAssert(reverse->target == n->reverse->target);
			next = n;
			n->prev = this;
		}
		...
		void print()
		{
			drx3DPrintf("E%p : %d -> %d,  n=%p p=%p   (0 %d\t%d\t%d) -> (%d %d %d)", this, reverse->target->point.index, target->point.index, next, prev,
...
private:
	...
	public:
		...
		PoolArray(i32 size) : size(size), next(NULL)
		{
	...
	public:
		...
		void setArraySize(i32 arraySize)
		{
	...
	void removeEdgePair(Edge* edge)
	{
		...
		drx3DAssert(edge->target && r->target);
	...
	void computeInternal(i32 start, i32 end, IntermediateHull& result);

	bool mergeProjection(IntermediateHull& h0, IntermediateHull& h1, Vertex*& c0, Vertex*& c1);

	void merge(IntermediateHull& h0, IntermediateHull& h1);

	b3Vec3 toBtVector(const Point32& v);

	b3Vec3 getBtNormal(Face* face);
...
public:
	...
	void compute(ukk coords, bool doubleCoords, i32 stride, i32 count);

	b3Vec3 getCoordinates(const Vertex* v);
...
i32 b3ConvexHullInternal::Rational64::compare(const Rational64& b) const
{
	if (sign != b.sign)
	{
		return sign - b.sign;
...
i32 b3ConvexHullInternal::Rational128::compare(const Rational128& b) const
{
	if (sign != b.sign)
	{
		return sign - b.sign;
...
i32 b3ConvexHullInternal::Rational128::compare(Int64_t b) const
{
	if (isInt64)
	{
...
bool b3ConvexHullInternal::mergeProjection(IntermediateHull& h0, IntermediateHull& h1, Vertex*& c0, Vertex*& c1)
{
	...
	{
		drx3DAssert(v0->point.z < v1->point.z);
		Vertex* v1p = v1->prev;
		if (v1p == v1)
		{
			c0 = v0;
			if (v1->edges)
			{
				drx3DAssert(v1->edges->next == v1->edges);
				v1 = v1->edges->target;
				drx3DAssert(v1->edges->next == v1->edges);
			}
			c1 = v1;
...
void b3ConvexHullInternal::computeInternal(i32 start, i32 end, IntermediateHull& result)
{
	i32 n = end - start;
	switch (n)
	{
		...
		case 2:
		{
			...
			if (v->point != w->point)
			{
				...
				if ((dx == 0) && (dy == 0))
				{
					...
					drx3DAssert(v->point.z < w->point.z);
					v->next = v;
					v->prev = v;
					result.minXy = v;
					result.maxXy = v;
...
{
	drx3DAssert(prev->reverse->target == next->reverse->target);
	if (prev->next == next)
	{
		if (prev->prev == next)
		{
...
#ifdef DEBUG_CONVEX_HULL
	drx3DPrintf("find max edge for %d\n", start->point.index);
#endif
	Edge* e = start->edges;
	if (e)
	{
...
#ifdef DEBUG_CONVEX_HULL
				drx3DPrintf("      Angle is %f (%d) for ", (float)b3Atan(cot.toScalar()), (i32)cot.isNaN());
				e->print();
#endif
				if (cot.isNaN())
				{
					drx3DAssert(ccw ? (t.dot(s) < 0) : (t.dot(s) > 0));
				}
				else
				{
					i32 cmp;
...
void b3ConvexHullInternal::findEdgeForCoplanarFaces(Vertex* c0, Vertex* c1, Edge*& e0, Edge*& e1, Vertex* stop0, Vertex* stop1)
{
	...
	drx3DAssert(!start1 || (start1->target->point.dot(normal) == dist));
	Point64 perp = s.cross(normal);
	drx3DAssert(!perp.isZero());

#ifdef DEBUG_CONVEX_HULL
	drx3DPrintf("   Advancing %d %d  (%p %p, %d %d)\n", c0->point.index, c1->point.index, start0, start1, start0 ? start0->target->point.index : -1, start1 ? start1->target->point.index : -1);
#endif
	...
	if (e0)
	{
		while (e0->target != stop0)
		{
			...
			drx3DAssert(e->target->point.dot(normal) == dist);
			if (e->copy == mergeStamp)
			{
				break;
			}
		...
		while (true)
		{
			...
			if (e1 && (e1->target != stop1))
			{
				...
				if (f1->copy > mergeStamp)
				{
					...
					{
						drx3DAssert((e1 == start1) && (d1.dot(normal) < 0));
					}
				}
			}
		...
		while (true)
		{
			...
			if (e0 && (e0->target != stop0))
			{
				...
				if (f0->copy > mergeStamp)
				{
					...
					{
						drx3DAssert((e0 == start0) && (d0.dot(normal) < 0));
					}
				}
			}
...
#ifdef DEBUG_CONVEX_HULL
	drx3DPrintf("   Advanced edges to %d %d\n", et0.index, et1.index);
#endif
}
...
void b3ConvexHullInternal::merge(IntermediateHull& h0, IntermediateHull& h1)
{
	...
	while (true)
	{
		...
		{
			i32 cmp = !min0 ? 1 : !min1 ? -1 : minCot0.compare(minCot1);
#ifdef DEBUG_CONVEX_HULL
			drx3DPrintf("    -> Result %d\n", cmp);
#endif
...
#ifdef DEBUG_CONVEX_HULL
			drx3DPrintf("   Found min edges to %d %d\n", e0 ? e0->target->point.index : -1, e1 ? e1->target->point.index : -1);
#endif
...
void b3ConvexHullInternal::compute(ukk coords, bool doubleCoords, i32 stride, i32 count)
{
	b3Vec3 min = b3MakeVector3(b3Scalar(1e30), b3Scalar(1e30), b3Scalar(1e30)), max = b3MakeVector3(b3Scalar(-1e30), b3Scalar(-1e30), b3Scalar(-1e30));
	tukk ptr = (tukk)coords;
	if (doubleCoords)
	{
		for (i32 i = 0; i < count; i++)
		{
			const double* v = (const double*)ptr;
			b3Vec3 p = b3MakeVector3((b3Scalar)v[0], (b3Scalar)v[1], (b3Scalar)v[2]);
			ptr += stride;
			min.setMin(p);
			max.setMax(p);
		}
	...
	else
	{
		for (i32 i = 0; i < count; i++)
		{
			...
			b3Vec3 p = b3MakeVector3(v[0], v[1], v[2]);
			ptr += stride;
			min.setMin(p);
			max.setMax(p);
		}
...
#ifdef DEBUG_CONVEX_HULL
	drx3DPrintf("max. edges %d (3v = %d)", maxUsedEdgePairs, 3 * count);
#endif
}

b3Vec3 b3ConvexHullInternal::toBtVector(const Point32& v)
{
	b3Vec3 p;
...
b3Vec3 b3ConvexHullInternal::getBtNormal(Face* face)
{
	return toBtVector(face->dir0).cross(toBtVector(face->dir1)).normalized();
}

b3Vec3 b3ConvexHullInternal::getCoordinates(const Vertex* v)
{
	b3Vec3 p;
	p[medAxis] = v->xvalue();
	p[maxAxis] = v->yvalue();
...
b3Scalar b3ConvexHullInternal::shrink(b3Scalar amount, b3Scalar clampAmount)
{
	...
	i32 faceCount = faces.size();

	if (clampAmount > 0)
	{
		b3Scalar minDist = D3_INFINITY;
		for (i32 i = 0; i < faceCount; i++)
		{
			b3Vec3 normal = getBtNormal(faces[i]);
		...
		amount = d3Min(amount, minDist * clampAmount);
	}

	u32 seed = 243703;
	for (i32 i = 0; i < faceCount; i++, seed = 1664525 * seed + 1013904223)
	{
...
{
	b3Vec3 origShift = getBtNormal(face) * -amount;
	if (scaling[0] != 0)
	{
	...
	drx3DAssert(shiftedDot <= origDot);
...
#ifdef DEBUG_CONVEX_HULL
	drx3DPrintf("Start edge is ");
	startEdge->print();
	drx3DPrintf(", normal is (%lld %lld %lld), shifted dot is %lld\n", normal.x, normal.y, normal.z, shiftedDot);
#endif
	Rational128 optDot = face->nearbyVertex->dot(normal);
	i32 cmp = optDot.compare(shiftedDot);
	...
	if (cmp >= 0)
	{
		...
		do
		{
			...
			drx3DAssert(dot.compare(origDot) <= 0);
#ifdef DEBUG_CONVEX_HULL
			drx3DPrintf("Moving downwards, edge is ");
			e->print();
			drx3DPrintf(", dot is %f (%f %lld)\n", (float)dot.toScalar(), (float)optDot.toScalar(), shiftedDot);
#endif
			if (dot.compare(optDot) < 0)
			{
				i32 c = dot.compare(shiftedDot);
				optDot = dot;
				e = e->reverse;
				startEdge = e;
				if (c < 0)
	...
	else
	{
		...
		do
		{
			...
			drx3DAssert(dot.compare(origDot) <= 0);
#ifdef DEBUG_CONVEX_HULL
			drx3DPrintf("Moving upwards, edge is ");
			e->print();
			drx3DPrintf(", dot is %f (%f %lld)\n", (float)dot.toScalar(), (float)optDot.toScalar(), shiftedDot);
#endif
			if (dot.compare(optDot) > 0)
...
#ifdef SHOW_ITERATIONS
	drx3DPrintf("Needed %d iterations to find initial intersection\n", n);
#endif
...
#ifdef DEBUG_CONVEX_HULL
			drx3DPrintf("Checking for outwards edge, current edge is ");
			e->print();
			drx3DPrintf("\n");
#endif
		}
#ifdef SHOW_ITERATIONS
		drx3DPrintf("Needed %d iterations to check for complete containment\n", n);
#endif
	}
...
#endif
#ifdef DEBUG_CONVEX_HULL
		drx3DPrintf("Intersecting edge is ");
		intersection->print();
		drx3DPrintf("\n");
#endif
		if (cmp == 0)
...
#ifdef SHOW_ITERATIONS
			drx3DPrintf("Needed %d iterations to advance intersection\n", n);
#endif
		}

#ifdef DEBUG_CONVEX_HULL
		drx3DPrintf("Advanced intersecting edge to ");
		intersection->print();
		drx3DPrintf(", cmp = %d\n", cmp);
#endif
...
#endif
			e = e->reverse->prev;
			drx3DAssert(e != intersection->reverse);
			cmp = e->target->dot(normal).compare(shiftedDot);
#ifdef DEBUG_CONVEX_HULL
			drx3DPrintf("Testing edge ");
			e->print();
			drx3DPrintf(" -> cmp = %d\n", cmp);
#endif
			if (cmp >= 0)
...
#ifdef SHOW_ITERATIONS
		drx3DPrintf("Needed %d iterations to find other intersection of face\n", n);
#endif
...
#ifdef DEBUG_CONVEX_HULL
			drx3DPrintf("1: Removed part contains (%d %d %d)\n", removed->point.x, removed->point.y, removed->point.z);
#endif
		...
		if (prevFaceEdge)
		{
			...
			else if (faceEdge != prevFaceEdge->reverse)
			{
				...
				while (faceEdge->next != prevFaceEdge->reverse)
				{
					...
					stack.push_back(removed);
#ifdef DEBUG_CONVEX_HULL
					drx3DPrintf("2: Removed part contains (%d %d %d)\n", removed->point.x, removed->point.y, removed->point.z);
#endif
				}
				stack.push_back(NULL);
			}
...
#ifdef SHOW_ITERATIONS
	drx3DPrintf("Needed %d iterations to process all intersections\n", m);
#endif
	...
	else if (firstFaceEdge != faceEdge->reverse)
	{
		...
		while (firstFaceEdge->next != faceEdge->reverse)
		{
			...
			stack.push_back(removed);
#ifdef DEBUG_CONVEX_HULL
			drx3DPrintf("3: Removed part contains (%d %d %d)\n", removed->point.x, removed->point.y, removed->point.z);
#endif
		}
		stack.push_back(NULL);
	}

	drx3DAssert(stack.size() > 0);
	vertexList = stack[0];

#ifdef DEBUG_CONVEX_HULL
	drx3DPrintf("Removing part\n");
#endif
#ifdef SHOW_ITERATIONS
	n = 0;
#endif
...
#ifdef SHOW_ITERATIONS
	drx3DPrintf("Needed %d iterations to remove part\n", n);
#endif
...
static i32 getVertexCopy(b3ConvexHullInternal::Vertex* vertex, b3AlignedObjectArray<b3ConvexHullInternal::Vertex*>& vertices)
{
	i32 index = vertex->copy;
	if (index < 0)
	{
		index = vertices.size();
		vertex->copy = index;
...
#ifdef DEBUG_CONVEX_HULL
		drx3DPrintf("Vertex %d gets index *%d\n", vertex->point.index, index);
#endif
	}
	return index;
}

b3Scalar b3ConvexHullComputer::compute(ukk coords, bool doubleCoords, i32 stride, i32 count, b3Scalar shrink, b3Scalar shrinkClamp)
{
	if (count <= 0)
	{
		vertices.clear();
...
#ifdef DEBUG_CONVEX_HULL
					drx3DPrintf("      CREATE: Vertex *%d has edge to *%d\n", copied, c->getTargetVertex());
#endif
				}
				if (prevCopy >= 0)
				{
	...
	for (i32 i = 0; i < copied; i++)
	{
		...
		if (firstEdge)
		{
			...
			do
			{
				...
				{
#ifdef DEBUG_CONVEX_HULL
					drx3DPrintf("Vertex *%d has edge to *%d\n", i, edges[e->copy].getTargetVertex());
#endif
