																												  ...
																												  m_largeAabbs(context, queue)
{
	...
	//
	const char CL_PROGRAM_PATH[] = "drx3D/Physics/Collision/BroadPhase/kernels/parallelLinearBvh.cl";

	tukk kernelSource = parallelLinearBvhCL;  //parallelLinearBvhCL.h
	cl_int error;
	...
	drx3DAssert(m_parallelLinearBvhProgram);
	...
	drx3DAssert(m_separateAabbsKernel);
	...
	drx3DAssert(m_findAllNodesMergedAabbKernel);
	...
	drx3DAssert(m_assignMortonCodesAndAabbIndiciesKernel);
	...
	drx3DAssert(m_computeAdjacentPairCommonPrefixKernel);
	...
	drx3DAssert(m_buildBinaryRadixTreeLeafNodesKernel);
	...
	drx3DAssert(m_buildBinaryRadixTreeInternalNodesKernel);
	...
	drx3DAssert(m_findDistanceFromRootKernel);
	...
	drx3DAssert(m_buildBinaryRadixTreeAabbsRecursiveKernel);
	...
	drx3DAssert(m_findLeafIndexRangesKernel);
	...
	drx3DAssert(m_plbvhCalculateOverlappingPairsKernel);
	...
	drx3DAssert(m_plbvhRayTraverseKernel);
	...
	drx3DAssert(m_plbvhLargeAabbAabbTestKernel);
	...
	drx3DAssert(m_plbvhLargeAabbRayTestKernel);
}
...
void b3GpuParallelLinearBvh::build(const b3OpenCLArray<b3SapAabb>& worldSpaceAabbs, const b3OpenCLArray<i32>& smallAabbIndices,
								   const b3OpenCLArray<i32>& largeAabbIndices)
{
	D3_PROFILE("b3ParallelLinearBvh::build()");

	i32 numLargeAabbs = largeAabbIndices.size();
	i32 numSmallAabbs = smallAabbIndices.size();
	...
	{
		D3_PROFILE("Separate large and small AABBs");
	...
	//
	i32 numLeaves = numSmallAabbs;  //Number of leaves in the BVH == Number of rigid bodies with small AABBs
	i32 numInternalNodes = numLeaves - 1;
	...
	{
		D3_PROFILE("Assign morton codes");
	...
	//
	{
		D3_PROFILE("Sort leaves by morton codes");
	...
	{
		D3_PROFILE("m_findLeafIndexRangesKernel");
...
{
	i32 maxPairs = out_overlappingPairs.size();
	b3OpenCLArray<i32>& numPairsGpu = m_temp;
	...
	{
		D3_PROFILE("PLBVH small-small AABB test");

		i32 numQueryAabbs = m_leafNodeAabbs.size();
	...
	i32 numLargeAabbRigids = m_largeAabbs.size();
	if (numLargeAabbRigids > 0 && m_leafNodeAabbs.size() > 0)
	{
		D3_PROFILE("PLBVH large-small AABB test");
...
void b3GpuParallelLinearBvh::testRaysAgainstBvhAabbs(const b3OpenCLArray<b3RayInfo>& rays,
													 b3OpenCLArray<i32>& out_numRayRigidPairs, b3OpenCLArray<b3Int2>& out_rayRigidPairs)
{
	D3_PROFILE("PLBVH testRaysAgainstBvhAabbs()");

	i32 numRays = rays.size();
	i32 maxRayRigidPairs = out_rayRigidPairs.size();
	...
	{
		D3_PROFILE("PLBVH ray test small AABB");
	...
	{
		D3_PROFILE("PLBVH ray test large AABB");
	...
	if (numRayRigidPairs > maxRayRigidPairs)
		drx3DError("Error running out of rayRigid pairs: numRayRigidPairs = %d, maxRayRigidPairs = %d.\n", numRayRigidPairs, maxRayRigidPairs);
}

void b3GpuParallelLinearBvh::constructBinaryRadixTree()
{
	D3_PROFILE("b3GpuParallelLinearBvh::constructBinaryRadixTree()");

	i32 numLeaves = m_leafNodeAabbs.size();
	i32 numInternalNodes = numLeaves - 1;
	...
	{
		D3_PROFILE("m_computeAdjacentPairCommonPrefixKernel");
	...
	{
		D3_PROFILE("m_buildBinaryRadixTreeLeafNodesKernel");
	...
	{
		D3_PROFILE("m_buildBinaryRadixTreeInternalNodesKernel");
	...
	{
		D3_PROFILE("m_findDistanceFromRootKernel");
	...
	{
		D3_PROFILE("m_buildBinaryRadixTreeAabbsRecursiveKernel");

		i32 maxDistanceFromRoot = -1;
		{
			D3_PROFILE("copy maxDistanceFromRoot to CPU");
			m_maxDistanceFromRoot.copyToHostPointer(&maxDistanceFromRoot, 1);
			clFinish(m_queue);
		}

		for (i32 distanceFromRoot = maxDistanceFromRoot; distanceFromRoot >= 0; --distanceFromRoot)
		{
			b3BufferInfoCL bufferInfo[] =
				{
					b3BufferInfoCL(m_distanceFromRoot.getBufferCL()),
