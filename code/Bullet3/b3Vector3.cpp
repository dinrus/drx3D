...
long b3_maxdot_large(const float *vv, const float *vec, u64 count, float *dotResult);
long b3_maxdot_large(const float *vv, const float *vec, u64 count, float *dotResult)
{
	const float4 *vertices = (const float4 *)vv;
	static u8k indexTable[16] = {(u8)-1, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0};
	float4 dotMax = b3Assign128(-D3_INFINITY, -D3_INFINITY, -D3_INFINITY, -D3_INFINITY);
...
long b3_mindot_large(const float *vv, const float *vec, u64 count, float *dotResult);
...
long b3_mindot_large(const float *vv, const float *vec, u64 count, float *dotResult)
{
	...
	float4 dotmin = b3Assign128(D3_INFINITY, D3_INFINITY, D3_INFINITY, D3_INFINITY);
	float4 vvec = _mm_loadu_ps(vec);
	float4 vHi = b3CastiTo128f(_mm_shuffle_epi32(b3CastfTo128i(vvec), 0xaa));  /// zzzz
	float4 vLo = _mm_movelh_ps(vvec, vvec);                                    /// xyxy
...
static long b3_maxdot_large_v0(const float *vv, const float *vec, u64 count, float *dotResult);
static long b3_maxdot_large_v1(const float *vv, const float *vec, u64 count, float *dotResult);
static long b3_maxdot_large_sel(const float *vv, const float *vec, u64 count, float *dotResult);
static long b3_mindot_large_v0(const float *vv, const float *vec, u64 count, float *dotResult);
static long b3_mindot_large_v1(const float *vv, const float *vec, u64 count, float *dotResult);
static long b3_mindot_large_sel(const float *vv, const float *vec, u64 count, float *dotResult);

long (*b3_maxdot_large)(const float *vv, const float *vec, u64 count, float *dotResult) = b3_maxdot_large_sel;
long (*b3_mindot_large)(const float *vv, const float *vec, u64 count, float *dotResult) = b3_mindot_large_sel;

extern "C"
{
	i32 _get_cpu_capabilities(void);
}
...
long b3_maxdot_large_v0(const float *vv, const float *vec, u64 count, float *dotResult)
{
	u64 i = 0;
	...
	float32x2_t dotMaxLo = (float32x2_t){-D3_INFINITY, -D3_INFINITY};
	float32x2_t dotMaxHi = (float32x2_t){-D3_INFINITY, -D3_INFINITY};
...
long b3_maxdot_large_v1(const float *vv, const float *vec, u64 count, float *dotResult)
{
	float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
	...
	float32x4_t maxDot = (float32x4_t){-D3_INFINITY, -D3_INFINITY, -D3_INFINITY, -D3_INFINITY};
...
long b3_mindot_large_v0(const float *vv, const float *vec, u64 count, float *dotResult)
{
	u64 i = 0;
	float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
	float32x2_t vLo = vget_low_f32(vvec);
	float32x2_t vHi = vdup_lane_f32(vget_high_f32(vvec), 0);
	float32x2_t dotMinLo = (float32x2_t){D3_INFINITY, D3_INFINITY};
	float32x2_t dotMinHi = (float32x2_t){D3_INFINITY, D3_INFINITY};
	uint32x2_t indexLo = (uint32x2_t){0, 1};
	uint32x2_t indexHi = (uint32x2_t){2, 3};
	uint32x2_t iLo = (uint32x2_t){-1, -1};
	uint32x2_t iHi = (uint32x2_t){-1, -1};
...
long b3_mindot_large_v1(const float *vv, const float *vec, u64 count, float *dotResult)
{
	float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
	float32x4_t vLo = vcombine_f32(vget_low_f32(vvec), vget_low_f32(vvec));
	float32x4_t vHi = vdupq_lane_f32(vget_high_f32(vvec), 0);
	...
	uint32x4_t local_index = (uint32x4_t){0, 1, 2, 3};
	uint32x4_t index = (uint32x4_t){-1, -1, -1, -1};
	float32x4_t minDot = (float32x4_t){D3_INFINITY, D3_INFINITY, D3_INFINITY, D3_INFINITY};
