	...
	: m_data(0), m_planeBodyIndex(-1), m_static0Index(-1), m_context(ctx), m_device(device), m_queue(queue)
{
	...
	m_data->m_uniqueEdgesGPU = new b3OpenCLArray<b3Vec3>(ctx, queue, config.m_maxConvexUniqueEdges, true);
	m_data->m_uniqueEdges.reserve(config.m_maxConvexUniqueEdges);

	m_data->m_convexVerticesGPU = new b3OpenCLArray<b3Vec3>(ctx, queue, config.m_maxConvexVertices, true);
	m_data->m_convexVertices.reserve(config.m_maxConvexVertices);

	m_data->m_convexIndicesGPU = new b3OpenCLArray<i32>(ctx, queue, config.m_maxConvexIndices, true);
	m_data->m_convexIndices.reserve(config.m_maxConvexIndices);

	m_data->m_worldVertsB1GPU = new b3OpenCLArray<b3Vec3>(ctx, queue, config.m_maxConvexBodies * config.m_maxVerticesPerFace);
	m_data->m_clippingFacesOutGPU = new b3OpenCLArray<b3Int4>(ctx, queue, config.m_maxConvexBodies);
	m_data->m_worldNormalsAGPU = new b3OpenCLArray<b3Vec3>(ctx, queue, config.m_maxConvexBodies);
	m_data->m_worldVertsA1GPU = new b3OpenCLArray<b3Vec3>(ctx, queue, config.m_maxConvexBodies * config.m_maxVerticesPerFace);
	m_data->m_worldVertsB2GPU = new b3OpenCLArray<b3Vec3>(ctx, queue, config.m_maxConvexBodies * config.m_maxVerticesPerFace);
...
b3GpuNarrowPhase::~b3GpuNarrowPhase()
{
	...
	for (i32 i = 0; i < m_data->m_bvhData.size(); i++)
	{
		delete m_data->m_bvhData[i];
	}
	for (i32 i = 0; i < m_data->m_meshInterfaces.size(); i++)
	{
...
i32 b3GpuNarrowPhase::allocateCollidable()
{
...
i32 b3GpuNarrowPhase::registerSphereShape(float radius)
{
	...
	{
		b3SapAabb aabb;
		b3Vec3 myAabbMin = b3MakeVector3(-radius, -radius, -radius);
		b3Vec3 myAabbMax = b3MakeVector3(radius, radius, radius);
...
i32 b3GpuNarrowPhase::registerFace(const b3Vec3& faceNormal, float faceConstant)
{
...
i32 b3GpuNarrowPhase::registerPlaneShape(const b3Vec3& planeNormal, float planeConstant)
{
	i32 collidableIndex = allocateCollidable();
	if (collidableIndex < 0)
		return collidableIndex;
...
i32 b3GpuNarrowPhase::registerConvexHullShapeInternal(b3ConvexUtility* convexPtr, b3Collidable& col)
{
...
i32 b3GpuNarrowPhase::registerConvexHullShape(const float* vertices, i32 strideInBytes, i32 numVertices, const float* scaling)
{
...
i32 b3GpuNarrowPhase::registerConvexHullShape(b3ConvexUtility* utilPtr)
{
	i32 collidableIndex = allocateCollidable();
	if (collidableIndex < 0)
		return collidableIndex;
...
i32 b3GpuNarrowPhase::registerCompoundShape(b3AlignedObjectArray<b3GpuChildShape>* childShapes)
{
	i32 collidableIndex = allocateCollidable();
	if (collidableIndex < 0)
		return collidableIndex;
	...
	{
		drx3DAssert(col.m_shapeIndex + childShapes->size() < m_data->m_config.m_maxCompoundChildShapes);
		for (i32 i = 0; i < childShapes->size(); i++)
		{
	...
	for (i32 i = 0; i < childShapes->size(); i++)
	{
		i32 childColIndex = childShapes->at(i).m_shapeIndex;
		//b3Collidable& childCol = getCollidableCpu(childColIndex);
		b3SapAabb aabbLoc = m_data->m_localShapeAABBCPU->at(childColIndex);

		b3Vec3 childLocalAabbMin = b3MakeVector3(aabbLoc.m_min[0], aabbLoc.m_min[1], aabbLoc.m_min[2]);
		b3Vec3 childLocalAabbMax = b3MakeVector3(aabbLoc.m_max[0], aabbLoc.m_max[1], aabbLoc.m_max[2]);
		b3Vec3 aMin, aMax;
		b3Scalar margin(0.f);
		b3Transform childTr;
		childTr.setIdentity();
		...
		childTr.setRotation(b3Quat(childShapes->at(i).m_childOrientation));
	...
	i32 numNodes = childShapes->size();

	for (i32 i = 0; i < numNodes; i++)
	{
		b3QuantizedBvhNode node;
		b3Vec3 aabbMin, aabbMax;
		aabbMin = (b3Vec3&)childLocalAabbs[i].m_min;
		aabbMax = (b3Vec3&)childLocalAabbs[i].m_max;
	...
	i32 numSubTrees = bvh->getSubtreeInfoArray().size();
	...
	i32 numNewNodes = bvh->getQuantizedNodeArray().size();

	for (i32 i = 0; i < numNewNodes - 1; i++)
	{
		if (bvh->getQuantizedNodeArray()[i].isLeafNode())
		{
			i32 orgIndex = bvh->getQuantizedNodeArray()[i].getTriangleIndex();

			b3Vec3 nodeMinVec = bvh->unQuantize(bvh->getQuantizedNodeArray()[i].m_quantizedAabbMin);
			b3Vec3 nodeMaxVec = bvh->unQuantize(bvh->getQuantizedNodeArray()[i].m_quantizedAabbMax);
	...
	i32 numNewSubtrees = bvh->getSubtreeInfoArray().size();
	m_data->m_subTreesCPU.reserve(m_data->m_subTreesCPU.size() + numNewSubtrees);
	for (i32 i = 0; i < numNewSubtrees; i++)
	{
		m_data->m_subTreesCPU.push_back(bvh->getSubtreeInfoArray()[i]);
	}
	i32 numNewTreeNodes = bvh->getQuantizedNodeArray().size();

	for (i32 i = 0; i < numNewTreeNodes; i++)
	{
...
i32 b3GpuNarrowPhase::registerConcaveMesh(b3AlignedObjectArray<b3Vec3>* vertices, b3AlignedObjectArray<i32>* indices, const float* scaling1)
{
	b3Vec3 scaling = b3MakeVector3(scaling1[0], scaling1[1], scaling1[2]);
	...
	{
		b3Vec3 vtx(vertices->at(i) * scaling);
	...
	mesh.m_vertexBase = (u8k*)&vertices->at(0).x;
	mesh.m_vertexStride = sizeof(b3Vec3);
	mesh.m_triangleIndexStride = 3 * sizeof(i32);  // or sizeof(i32)
	mesh.m_triangleIndexBase = (u8k*)&indices->at(0);

	meshInterface->addIndexedMesh(mesh);
	bvh->build(meshInterface, useQuantizedAabbCompression, (b3Vec3&)aabb.m_min, (b3Vec3&)aabb.m_max);
	m_data->m_bvhData.push_back(bvh);
	i32 numNodes = bvh->getQuantizedNodeArray().size();
...
i32 b3GpuNarrowPhase::registerConcaveMeshShape(b3AlignedObjectArray<b3Vec3>* vertices, b3AlignedObjectArray<i32>* indices, b3Collidable& col, const float* scaling1)
{
	b3Vec3 scaling = b3MakeVector3(scaling1[0], scaling1[1], scaling1[2]);
	...
	for (i32 i = 0; i < convex.m_numFaces; i++)
	{
		if (i % 256 == 0)
		{
			//printf("i=%d out of %d", i,convex.m_numFaces);
		}
		b3Vec3 vert0(vertices->at(indices->at(i * 3)) * scaling);
		b3Vec3 vert1(vertices->at(indices->at(i * 3 + 1)) * scaling);
		b3Vec3 vert2(vertices->at(indices->at(i * 3 + 2)) * scaling);

		b3Vec3 normal = ((vert1 - vert0).cross(vert2 - vert0)).normalize();
...
i32 b3GpuNarrowPhase::getNumBodiesGpu() const
{
...
i32 b3GpuNarrowPhase::getNumBodyInertiasGpu() const
{
	return m_data->m_inertiaBufferGPU->size();
}

b3Collidable& b3GpuNarrowPhase::getCollidableCpu(i32 collidableIndex)
{
	return m_data->m_collidablesCPU[collidableIndex];
}

const b3Collidable& b3GpuNarrowPhase::getCollidableCpu(i32 collidableIndex) const
{
	return m_data->m_collidablesCPU[collidableIndex];
}
...
i32 b3GpuNarrowPhase::getNumCollidablesGpu() const
{
	return m_data->m_collidablesGPU->size();
}

i32 b3GpuNarrowPhase::getNumContactsGpu() const
{
...
void b3GpuNarrowPhase::computeContacts(cl_mem broadphasePairs, i32 numBroadphasePairs, cl_mem aabbsWorldSpace, i32 numObjects)
{
	...
	//i32 curSize = m_data->m_pBufContactBuffersGPU[m_data->m_currentContactBuffer]->size();

	i32 maxTriConvexPairCapacity = m_data->m_config.m_maxTriConvexPairCapacity;
	i32 numTriConvexPairsOut = 0;
...
const b3SapAabb& b3GpuNarrowPhase::getLocalSpaceAabb(i32 collidableIndex) const
{
	return m_data->m_localShapeAABBCPU->at(collidableIndex);
}

i32 b3GpuNarrowPhase::registerRigidBody(i32 collidableIndex, float mass, const float* position, const float* orientation, const float* aabbMinPtr, const float* aabbMaxPtr, bool writeToGpu)
{
	b3Vec3 aabbMin = b3MakeVector3(aabbMinPtr[0], aabbMinPtr[1], aabbMinPtr[2]);
	b3Vec3 aabbMax = b3MakeVector3(aabbMaxPtr[0], aabbMaxPtr[1], aabbMaxPtr[2]);

	if (m_data->m_numAcceleratedRigidBodies >= (m_data->m_config.m_maxConvexBodies))
	{
		drx3DError("registerRigidBody: exceeding the number of rigid bodies, %d > %d \n", m_data->m_numAcceleratedRigidBodies, m_data->m_config.m_maxConvexBodies);
		return -1;
	}
	...
	if (mass == 0.f)
	{
		...
		shapeInfo.m_initInvInertia.setVal(0, 0, 0, 0, 0, 0, 0, 0, 0);
		shapeInfo.m_invInertiaWorld.setVal(0, 0, 0, 0, 0, 0, 0, 0, 0);
	}
	else
	{
		drx3DAssert(body.m_collidableIdx >= 0);

		//approximate using the aabb of the shape

		//Aabb aabb = (*m_data->m_shapePointers)[shapeIndex]->m_aabb;
		b3Vec3 halfExtents = (aabbMax - aabbMin);  //*0.5f;//fake larger inertia makes demos more stable ;-)
		...
		localInertia.setVal((mass / 12.0f) * (ly * ly + lz * lz),
							  (mass / 12.0f) * (lx * lx + lz * lz),
							  (mass / 12.0f) * (lx * lx + ly * ly));

		b3Vec3 invLocalInertia;
		...
		shapeInfo.m_initInvInertia.setVal(
...
i32 b3GpuNarrowPhase::getNumRigidBodies() const
{
	return m_data->m_numAcceleratedRigidBodies;
}
...
void b3GpuNarrowPhase::setObjectTransformCpu(float* position, float* orientation, i32 bodyIndex)
{
	if (bodyIndex >= 0 && bodyIndex < m_data->m_bodyBufferCPU->size())
	{
		...
		m_data->m_bodyBufferCPU->at(bodyIndex).m_quat.setVal(orientation[0], orientation[1], orientation[2], orientation[3]);
	}
	else
	{
		drx3DWarning("setObjectVelocityCpu out of range.\n");
	}
}
void b3GpuNarrowPhase::setObjectVelocityCpu(float* linVel, float* angVel, i32 bodyIndex)
{
	if (bodyIndex >= 0 && bodyIndex < m_data->m_bodyBufferCPU->size())
	{
...
bool b3GpuNarrowPhase::getObjectTransformFromCpu(float* position, float* orientation, i32 bodyIndex) const
{
	if (bodyIndex >= 0 && bodyIndex < m_data->m_bodyBufferCPU->size())
	{
	...
	drx3DWarning("getObjectTransformFromCpu out of range.\n");
	return false;
}
