...
void bFile::parseHeader()
{
	...
	char header[D3_SIZEOFBLENDERHEADER + 1];
	memcpy(header, blenderBuf, D3_SIZEOFBLENDERHEADER);
	header[D3_SIZEOFBLENDERHEADER] = '\0';

	if (strncmp(header, m_headerString, 6) != 0)
	{
		memcpy(header, m_headerString, D3_SIZEOFBLENDERHEADER);
		return;
	}
...
void bFile::preSwap()
{
	...
	{
		// one behind
		if (dataChunk.code == D3_SDNA || dataChunk.code == D3_DNA1 || dataChunk.code == D3_TYPE || dataChunk.code == D3_TLEN || dataChunk.code == D3_STRC)
		{
			swapDNA(dataPtr);
			break;
		}
...
#define b3SetEle(value, current, type, cast, size, ptr) \
	...
	for (i32 i = 0; i < arrayLen; i++)
	{
		...
		b3GetEle(value, old, "i32", i32, sizeof(i32), oldPtr);
		b3SetEle(value, cur, "i32", i32, sizeof(i32), curData);
		b3GetEle(value, old, "long", i32, sizeof(i32), oldPtr);
		b3SetEle(value, cur, "long", i32, sizeof(i32), curData);
	...
	if (VOID_IS_8)
	{
		if (varies)
		{
			...
			if (swap)
			{
				...
				D3_SWITCH_INT(chunk.len);
				D3_SWITCH_INT(chunk.dna_nr);
				D3_SWITCH_INT(chunk.nr);
			}
