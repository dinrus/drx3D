#include <drx3D/Physics/Collision/Gimpact/gim_tri_collision.h>
...
public:
	...
	Vec4 tu_plane;
	Vec4 tv_plane;
	Vec3 closest_point_u;
	Vec3 closest_point_v;
	Vec3 edge_edge_dir;
	Vec3 distances;
	GREAL du[4];
	GREAL du0du1;
	...
	Vec3 contact_points[MAX_TRI_CLIPPING];
	...
	*/
	SIMD_FORCE_INLINE GUINT clip_triangle(
		const Vec4 &tri_plane,
		const Vec3 *tripoints,
		const Vec3 *srcpoints,
		Vec3 *clip_points)
	{
		// edge 0
	...
	SIMD_FORCE_INLINE void sort_isect(
		GREAL &isect0, GREAL &isect1, GUINT &e0, GUINT &e1, Vec3 &vec0, Vec3 &vec1)
	{
		if (isect1 < isect0)
		{
			//swap
	...
	*/
	SIMD_FORCE_INLINE GUINT cross_line_intersection_test()
	{
		...
		//proyected vertices
		Vec3 up_e0 = tu_vertices[tu_e0].lerp(tu_vertices[(tu_e0 + 1) % 3], tu_scale_e0);
		Vec3 up_e1 = tu_vertices[tu_e1].lerp(tu_vertices[(tu_e1 + 1) % 3], tu_scale_e1);

		Vec3 vp_e0 = tv_vertices[tv_e0].lerp(tv_vertices[(tv_e0 + 1) % 3], tv_scale_e0);
		Vec3 vp_e1 = tv_vertices[tv_e1].lerp(tv_vertices[(tv_e1 + 1) % 3], tv_scale_e1);
