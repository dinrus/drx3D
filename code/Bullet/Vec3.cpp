...
#if defined DRX3D_USE_SIMD_VECTOR3
...
long _maxdot_large(const float *vv, const float *vec, u64 count, float *dotResult);
long _maxdot_large(const float *vv, const float *vec, u64 count, float *dotResult)
{
	const float4 *vertices = (const float4 *)vv;
	static u8k indexTable[16] = {(u8)-1, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0};
	float4 dotMax = Assign128(-DRX3D_INFINITY, -DRX3D_INFINITY, -DRX3D_INFINITY, -DRX3D_INFINITY);
	float4 vvec = _mm_loadu_ps(vec);
	float4 vHi = CastiTo128f(_mm_shuffle_epi32(CastfTo128i(vvec), 0xaa));  /// zzzz
...
#else
		{
			for (u32 i = 0; i < localCount / 4; i++, index++)
			{  // do four dot products at a time. Carefully avoid touching the w element.
				float4 v0 = vertices[0];
				float4 v1 = vertices[1];
				float4 v2 = vertices[2];
...
long _mindot_large(const float *vv, const float *vec, u64 count, float *dotResult);

long _mindot_large(const float *vv, const float *vec, u64 count, float *dotResult)
{
	const float4 *vertices = (const float4 *)vv;
	static u8k indexTable[16] = {(u8)-1, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0};
	float4 dotmin = Assign128(DRX3D_INFINITY, DRX3D_INFINITY, DRX3D_INFINITY, DRX3D_INFINITY);
	float4 vvec = _mm_loadu_ps(vec);
	float4 vHi = CastiTo128f(_mm_shuffle_epi32(CastfTo128i(vvec), 0xaa));  /// zzzz
	float4 vLo = _mm_movelh_ps(vvec, vvec);                                    /// xyxy
...
static long _maxdot_large_v0(const float *vv, const float *vec, u64 count, float *dotResult);
static long _maxdot_large_v1(const float *vv, const float *vec, u64 count, float *dotResult);
static long _maxdot_large_sel(const float *vv, const float *vec, u64 count, float *dotResult);
static long _mindot_large_v0(const float *vv, const float *vec, u64 count, float *dotResult);
static long _mindot_large_v1(const float *vv, const float *vec, u64 count, float *dotResult);
static long _mindot_large_sel(const float *vv, const float *vec, u64 count, float *dotResult);

long (*_maxdot_large)(const float *vv, const float *vec, u64 count, float *dotResult) = _maxdot_large_sel;
long (*_mindot_large)(const float *vv, const float *vec, u64 count, float *dotResult) = _mindot_large_sel;

static inline uint32_t GetCpuCapabilities(void)
{
...
#else
//support 64bit arm
#define vld1q_f32_aligned_postincrement(_ptr) ({ float32x4_t _r = ((float32x4_t*)(_ptr))[0]; (_ptr) = (const float*) ((tukk)(_ptr) + 16L); /*return*/ _r; })
#endif

long _maxdot_large_v0(const float *vv, const float *vec, u64 count, float *dotResult)
{
	u64 i = 0;
	...
	float32x2_t dotMaxLo = (float32x2_t){-DRX3D_INFINITY, -DRX3D_INFINITY};
	float32x2_t dotMaxHi = (float32x2_t){-DRX3D_INFINITY, -DRX3D_INFINITY};
...
long _maxdot_large_v1(const float *vv, const float *vec, u64 count, float *dotResult)
{
	float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
	...
	float32x4_t maxDot = (float32x4_t){-DRX3D_INFINITY, -DRX3D_INFINITY, -DRX3D_INFINITY, -DRX3D_INFINITY};
...
long _mindot_large_v0(const float *vv, const float *vec, u64 count, float *dotResult)
{
	u64 i = 0;
	float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
	float32x2_t vLo = vget_low_f32(vvec);
	float32x2_t vHi = vdup_lane_f32(vget_high_f32(vvec), 0);
	float32x2_t dotMinLo = (float32x2_t){DRX3D_INFINITY, DRX3D_INFINITY};
	float32x2_t dotMinHi = (float32x2_t){DRX3D_INFINITY, DRX3D_INFINITY};
	uint32x2_t indexLo = (uint32x2_t){0, 1};
	uint32x2_t indexHi = (uint32x2_t){2, 3};
	uint32x2_t iLo = (uint32x2_t){static_cast<uint32_t>(-1), static_cast<uint32_t>(-1)};
	uint32x2_t iHi = (uint32x2_t){static_cast<uint32_t>(-1), static_cast<uint32_t>(-1)};
...
long _mindot_large_v1(const float *vv, const float *vec, u64 count, float *dotResult)
{
	float32x4_t vvec = vld1q_f32_aligned_postincrement(vec);
	float32x4_t vLo = vcombine_f32(vget_low_f32(vvec), vget_low_f32(vvec));
	float32x4_t vHi = vdupq_lane_f32(vget_high_f32(vvec), 0);
	...
	uint32x4_t local_index = (uint32x4_t){0, 1, 2, 3};
	uint32x4_t index = (uint32x4_t){static_cast<uint32_t>(-1), static_cast<uint32_t>(-1), static_cast<uint32_t>(-1), static_cast<uint32_t>(-1)};
	float32x4_t minDot = (float32x4_t){DRX3D_INFINITY, DRX3D_INFINITY, DRX3D_INFINITY, DRX3D_INFINITY};
