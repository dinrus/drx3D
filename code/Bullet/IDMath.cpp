...
idScalar maxAbs(const vecx &v)
{
	...
	for (i32 i = 0; i < v.size(); i++)
	{
		const idScalar tmp = DRX3D_ID_FABS(v(i));
		if (tmp > result)
		{
			result = tmp;
		}
...
#if (defined DRX3D_ID_HAVE_MAT3X)
idScalar maxAbsMat3x(const mat3x &m)
{
	...
	for (idArrayIdx col = 0; col < m.cols(); col++)
	{
		...
		{
			result = DRX3D_ID_MAX(result, std::fabs(m(row, col)));
		}
	}
	return result;
}
...
void mul(const mat33 &a, const mat3x &b, mat3x *result)
{
	...
	{
		drx3d_id_error_message("size missmatch. b.cols()= %d, result->cols()= %d\n",
							static_cast<i32>(b.cols()), static_cast<i32>(result->cols()));
		abort();
	}
...
void add(const mat3x &a, const mat3x &b, mat3x *result)
{
	if (a.cols() != b.cols())
	{
		drx3d_id_error_message("size missmatch. a.cols()= %d, b.cols()= %d\n",
							static_cast<i32>(a.cols()), static_cast<i32>(b.cols()));
		abort();
	}
	for (idArrayIdx col = 0; col < b.cols(); col++)
	{
...
{
	mat33 T;
	const idScalar cos_alpha = DRX3D_ID_COS(alpha);
	const idScalar sin_alpha = DRX3D_ID_SIN(alpha);
	// [1  0 0]
	// [0  c s]
	// [0 -s c]
	T(0, 0) = 1.0;
...
{
	mat33 T;
	const idScalar cos_beta = DRX3D_ID_COS(beta);
	const idScalar sin_beta = DRX3D_ID_SIN(beta);
	// [c 0 -s]
	// [0 1  0]
	// [s 0  c]
	T(0, 0) = cos_beta;
...
{
	mat33 T;
	const idScalar cos_gamma = DRX3D_ID_COS(gamma);
	const idScalar sin_gamma = DRX3D_ID_SIN(gamma);
	// [ c s 0]
	// [-s c 0]
	// [ 0 0 1]
	T(0, 0) = cos_gamma;
...
{
	const idScalar sa = DRX3D_ID_SIN(alpha);
	const idScalar ca = DRX3D_ID_COS(alpha);
	const idScalar st = DRX3D_ID_SIN(theta);
	const idScalar ct = DRX3D_ID_COS(theta);
...
{
	const idScalar c = DRX3D_ID_COS(angle);
	const idScalar s = -DRX3D_ID_SIN(angle);
...
bool isValidInertiaMatrix(const mat33 &I, i32k index, bool has_fixed_joint)
{
	...
	if (!has_fixed_joint)
	{
		...
		{
			drx3d_id_error_message("invalid inertia tensor for body %d, I(0,0) + I(1,1) < I(2,2)\n", index);
			drx3d_id_error_message(
				"matrix is:\n"
				"[%.20e %.20e %.20e;\n"
				"%.20e %.20e %.20e;\n"
		...
		{
			drx3d_id_error_message("invalid inertia tensor for body %d, I(1,1) + I(2,2) < I(0,0)\n", index);
			drx3d_id_error_message(
				"matrix is:\n"
				"[%.20e %.20e %.20e;\n"
				"%.20e %.20e %.20e;\n"
	...
	for (i32 i = 0; i < 3; i++)
	{
		if (I(i, i) < 0)
		{  // accept zero
			drx3d_id_error_message("invalid inertia tensor, I(%d,%d)= %e <0\n", i, i, I(i, i));
			return false;
		}
	}
	// check symmetry
	if (DRX3D_ID_FABS(I(1, 0) - I(0, 1)) > kIsZero)
	{
		drx3d_id_error_message(
	...
	if (DRX3D_ID_FABS(I(2, 0) - I(0, 2)) > kIsZero)
	{
		drx3d_id_error_message(
	...
	if (DRX3D_ID_FABS(I(1, 2) - I(2, 1)) > kIsZero)
	{
		drx3d_id_error_message("invalid inertia tensor body %d I(1,2)!=I(2,1). I(1,2)-I(2,1)= %e\n", index,
							I(1, 2) - I(2, 1));
		return false;
	}
	return true;
...
bool isValidTransform2Matrix(const mat33 &m)
{
#define print_mat(x)                                                                                   \
	drx3d_id_error_message("matrix is [%e, %e, %e; %e, %e, %e; %e, %e, %e]\n", x(0, 0), x(0, 1), x(0, 2), \
	...
	for (i32 i = 0; i < 3; i++)
	{
		const idScalar length_minus_1 =
			DRX3D_ID_FABS(m(0, i) * m(0, i) + m(1, i) * m(1, i) + m(2, i) * m(2, i) - 1.0);
	...
	if (DRX3D_ID_FABS(m(0, 0) * m(0, 1) + m(1, 0) * m(1, 1) + m(2, 0) * m(2, 1)) > kAxisLengthEpsilon)
	{
		drx3d_id_error_message("Not a valid rotation matrix (columns 0 and 1 not orthogonal)\n");
		print_mat(m);
		return false;
	}
	if (DRX3D_ID_FABS(m(0, 0) * m(0, 2) + m(1, 0) * m(1, 2) + m(2, 0) * m(2, 2)) > kAxisLengthEpsilon)
	{
		drx3d_id_error_message("Not a valid rotation matrix (columns 0 and 2 not orthogonal)\n");
		print_mat(m);
		return false;
	}
	if (DRX3D_ID_FABS(m(0, 1) * m(0, 2) + m(1, 1) * m(1, 2) + m(2, 1) * m(2, 2)) > kAxisLengthEpsilon)
	{
		drx3d_id_error_message("Not a valid rotation matrix (columns 0 and 2 not orthogonal)\n");
		print_mat(m);
		return false;
	...
	if (determinant(m) <= 0)
	{
		drx3d_id_error_message("Not a valid rotation matrix (determinant <=0)\n");
		print_mat(m);
		return false;
	}
	return true;
...
{
	return DRX3D_ID_FABS(vector(0) * vector(0) + vector(1) * vector(1) + vector(2) * vector(2) - 1.0) <
		   kIsZero;
}
...
{
	vec3 rpy;
	rpy(2) = DRX3D_ID_ATAN2(-rot(1, 0), rot(0, 0));
	rpy(0) = DRX3D_ID_ATAN2(-rot(2, 0), rot(2, 2));
	rpy(1) = DRX3D_ID_ATAN2(rot(2, 0), DRX3D_ID_COS(rpy(2)) * rot(0, 0) - DRX3D_ID_SIN(rpy(0)) * rot(1, 0));
	return rpy;
}
}  // namespace drx3d_inverse
