...
public:
	...
	public:
		...
		Scalar toScalar() const
		{
			return sign * ((m_denominator == 0) ? SIMD_INFINITY : (Scalar)m_numerator / m_denominator);
		}
	};
	...
	public:
		...
		Scalar xvalue() const
		{
			return (point.index >= 0) ? Scalar(point.x) : point128.xvalue();
		}

		Scalar yvalue() const
		{
			return (point.index >= 0) ? Scalar(point.y) : point128.yvalue();
		}

		Scalar zvalue() const
		{
			return (point.index >= 0) ? Scalar(point.z) : point128.zvalue();
		}
...
private:
	...
	public:
		...
		{
			array = (T*)AlignedAlloc(sizeof(T) * size, 16);
		}
	...
	public:
		...
		T* newObject()
		{
			...
			if (!o)
			{
				...
				{
					p = new (AlignedAlloc(sizeof(PoolArray<T>), 16)) PoolArray<T>(arraySize);
					p->next = arrays;
					arrays = p;
				}
				o = p->init();
	...
	Vec3 toBtVector(const Point32& v);

	Vec3 getBtNormal(Face* face);

	bool shiftFace(Face* face, Scalar amount, AlignedObjectArray<Vertex*> stack);
...
public:
	...
	Vec3 getCoordinates(const Vertex* v);

	Scalar shrink(Scalar amount, Scalar clampAmount);
};
...
i32 ConvexHullInternal::Rational64::compare(const Rational64& b) const
{
	if (sign != b.sign)
	{
		return sign - b.sign;
...
i32 ConvexHullInternal::Rational128::compare(const Rational128& b) const
{
	if (sign != b.sign)
	{
		return sign - b.sign;
...
i32 ConvexHullInternal::Rational128::compare(int64_t b) const
{
	if (isInt64)
	{
...
ConvexHullInternal::Edge* ConvexHullInternal::newEdgePair(Vertex* from, Vertex* to)
{
	Assert(from && to);
...
bool ConvexHullInternal::mergeProjection(IntermediateHull& h0, IntermediateHull& h1, Vertex*& c0, Vertex*& c1)
{
	Vertex* v0 = h0.maxYx;
	Vertex* v1 = h1.minYx;
...
void ConvexHullInternal::computeInternal(i32 start, i32 end, IntermediateHull& result)
{
	i32 n = end - start;
	switch (n)
	{
...
void ConvexHullInternal::Vertex::printGraph()
{
	print();
	printf("\nEdges\n");
	Edge* e = edges;
...
ConvexHullInternal::Orientation ConvexHullInternal::getOrientation(const Edge* prev, const Edge* next, const Point32& s, const Point32& t)
{
...
ConvexHullInternal::Edge* ConvexHullInternal::findMaxAngle(bool ccw, const Vertex* start, const Point32& s, const Point64& rxs, const Point64& sxrxs, Rational64& minCot)
{
	Edge* minEdge = NULL;
...
#ifdef DEBUG_CONVEX_HULL
				printf("      Angle is %f (%d) for ", (float)Atan(cot.toScalar()), (i32)cot.isNaN());
				e->print();
#endif
				if (cot.isNaN())
				{
...
void ConvexHullInternal::findEdgeForCoplanarFaces(Vertex* c0, Vertex* c1, Edge*& e0, Edge*& e1, Vertex* stop0, Vertex* stop1)
{
	Edge* start0 = e0;
	Edge* start1 = e1;
...
void ConvexHullInternal::merge(IntermediateHull& h0, IntermediateHull& h1)
{
	if (!h1.maxXy)
	{
		return;
...
{
public:
	bool operator()(const ConvexHullInternal::Point32& p, const ConvexHullInternal::Point32& q) const
	{
		return (p.y < q.y) || ((p.y == q.y) && ((p.x < q.x) || ((p.x == q.x) && (p.z < q.z))));
	}
};

void ConvexHullInternal::compute(ukk coords, bool doubleCoords, i32 stride, i32 count)
{
	Vec3 min(Scalar(1e30), Scalar(1e30), Scalar(1e30)), max(Scalar(-1e30), Scalar(-1e30), Scalar(-1e30));
	...
	if (doubleCoords)
	{
		for (i32 i = 0; i < count; i++)
		{
			...
			Vec3 p((Scalar)v[0], (Scalar)v[1], (Scalar)v[2]);
			ptr += stride;
			min.setMin(p);
			max.setMax(p);
		}
...
Vec3 ConvexHullInternal::toBtVector(const Point32& v)
{
	Vec3 p;
	p[medAxis] = Scalar(v.x);
	p[maxAxis] = Scalar(v.y);
	p[minAxis] = Scalar(v.z);
	return p * scaling;
}

Vec3 ConvexHullInternal::getBtNormal(Face* face)
{
	return toBtVector(face->dir0).cross(toBtVector(face->dir1)).normalized();
}

Vec3 ConvexHullInternal::getCoordinates(const Vertex* v)
{
	Vec3 p;
	p[medAxis] = v->xvalue();
	p[maxAxis] = v->yvalue();
...
Scalar ConvexHullInternal::shrink(Scalar amount, Scalar clampAmount)
{
	if (!vertexList)
	{
		return 0;
	...
	if (clampAmount > 0)
	{
		Scalar minDist = SIMD_INFINITY;
		for (i32 i = 0; i < faceCount; i++)
		{
			Vec3 normal = getBtNormal(faces[i]);
			Scalar dist = normal.dot(toBtVector(faces[i]->origin) - hullCenter);
			if (dist < minDist)
			{
				minDist = dist;
			}
...
bool ConvexHullInternal::shiftFace(Face* face, Scalar amount, AlignedObjectArray<Vertex*> stack)
{
	Vec3 origShift = getBtNormal(face) * -amount;
	if (scaling[0] != 0)
	{
...
static i32 getVertexCopy(ConvexHullInternal::Vertex* vertex, AlignedObjectArray<ConvexHullInternal::Vertex*>& vertices)
{
...
Scalar ConvexHullComputer::compute(ukk coords, bool doubleCoords, i32 stride, i32 count, Scalar shrink, Scalar shrinkClamp)
{
	if (count <= 0)
	{
		vertices.clear();
	...
	AlignedObjectArray<ConvexHullInternal::Vertex*> oldVertices;
