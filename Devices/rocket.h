...
i32 main() {
    ...
    // Подключаем первый обработчик нашего сигнала
    my_signal.connect([]() {
        std::cout << "First handler called!" << std::endl;
    });

    // Подключаем второй обработчик сигнала альтернативным синтаксисом
    my_signal += []() {
...
i32 main() {
    rocket::signal<void(STxt)> my_signal;

    my_signal.connect([](const STxt& argument) {
...
class Subject {
public:
    void setName(const STxt& newName) {
        if (name != newName) {
            name = newName;
            nameChanged(newName);
        }
...
public:
    rocket::signal<void(STxt)> nameChanged;
...
public:
    ...
    void onNameChanged(const STxt& name) {
...
struct ILogger {
    virtual void logMessage(const STxt& message) = 0;
};

struct ConsoleLogger : ILogger {
    void logMessage(const STxt& message) override {
...
struct App {
    ...
    rocket::signal<void(STxt)> onSuccess;
};
...
i32 main() {
    ...
    // function to a signal with an `i32` argument and `i32` return value.
...
namespace rocket
{
	...
	{
		tukk what() const ROCKET_NOEXCEPT override
		{
	...
	optional() ROCKET_NOEXCEPT = default;
		...
		private:
		uk storage() ROCKET_NOEXCEPT
		{
			return static_cast<uk> ( &buffer );
		}
...
*/
	...
	{
		u64 addref() ROCKET_NOEXCEPT
		{
			return ++count;
		}

		u64 release() ROCKET_NOEXCEPT
		{
			return --count;
		}

		u64 get() const ROCKET_NOEXCEPT
		{
			return count;
		}
	...
	class stable_list final
	{
		...
		public:
			...
			:
				...
				iterator_base operator ++ ( i32 ) ROCKET_NOEXCEPT
				{
	...
	namespace detail
	{
		...
		virtual ~connection_base() ROCKET_NOEXCEPT = default;
			...
			bool is_connected() const ROCKET_NOEXCEPT
			{
				return static_cast<intrusive_ptr<connection_base>  const&> ( prev ).get() != nullptr;
			}
			...
			void disconnect() ROCKET_NOEXCEPT
			{
				...
				if ( prev != nullptr )
				{
					...
					// leave the next link alive so we can still traverse through the connections
					// if the slot gets disconnected during signal emit.
					static_cast<intrusive_ptr<connection_base> &> ( prev ) = nullptr;
				}
			}
			...
			bool is_blocked() const ROCKET_NOEXCEPT
			{
				return ( *static_cast<u64 const *> ( &block_count ) ) > 0;
			}
		...
		{
			connection_scope ( uk base, thread_local_data* th ) ROCKET_NOEXCEPT
	...
	:
		...
		explicit connection ( uk base ) ROCKET_NOEXCEPT
	...
	namespace detail
	{
		...
		struct timer_queue final
		{
			...
			template < class Rep = u64, class Period = std::milli >
			connection set_interval ( slot_type slot, std::chrono::duration<Rep, Period> const& interval )
			{
				...
				return connection{ static_cast<uk> ( base ) };
			}

			template < auto Method, class Rep = u64, class Period = std::milli >
			...
			template < class Instance, class Class, class R, class Rep = u64, class Period = std::milli >
			...
			template < auto Method, class Instance, class Rep = u64, class Period = std::milli >
					...
					while ( current != end )
					{
							...
							ROCKET_LIKELY
						{
							...
							timed_connection* conn = static_cast <
									timed_connection* > ( static_cast<uk> ( current ) );
		...
		// make_tuple(i32 const&) => tuple<i32>
		// make_tuple(i32&) => tuple<i32&>
	...
	inline connection set_interval ( std::function < void() > slot, u64 interval_ms )
	{
		return detail::get_timer_queue()->template set_interval<> ( std::move ( slot ), std::chrono::milliseconds ( interval_ms ) );
	}

	template <auto Method>
	inline connection set_interval ( u64 interval_ms )
	{
	...
	inline connection set_interval ( Instance& object, R ( Class::*method ) (), u64 interval_ms )
	{
	...
	inline connection set_interval ( Instance& object, u64 interval_ms )
	{
		...
		ValueCollector collector{};
		{
			...
			while ( current != end )
			{
					...
					ROCKET_LIKELY
				{
					...
					functional_connection* > ( static_cast<uk> ( current ) );
