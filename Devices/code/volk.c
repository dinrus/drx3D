...
__declspec(dllimport) i32 __stdcall FreeLibrary(HMODULE);
#endif

static uk loadedModule = NULL;
static VkInstance loadedInstance = VK_NULL_HANDLE;
static VkDevice loadedDevice = VK_NULL_HANDLE;

static void volkGenLoadLoader(uk context, PFN_vkVoidFunction (*load)(uk , tukk ));
static void volkGenLoadInstance(uk context, PFN_vkVoidFunction (*load)(uk , tukk ));
static void volkGenLoadDevice(uk context, PFN_vkVoidFunction (*load)(uk , tukk ));
static void volkGenLoadDeviceTable(struct VolkDeviceTable* table, uk context, PFN_vkVoidFunction (*load)(uk , tukk ));

static PFN_vkVoidFunction vkGetInstanceProcAddrStub(uk context, tukk name)
{
	return vkGetInstanceProcAddr((VkInstance)context, name);
}

static PFN_vkVoidFunction vkGetDeviceProcAddrStub(uk context, tukk name)
{
	return vkGetDeviceProcAddr((VkDevice)context, name);
}

static PFN_vkVoidFunction nullProcAddrStub(uk context, tukk name)
{
	(void)context;
	(void)name;
	return NULL;
...
VkResult volkInitialize(void)
{
	...
	uk module = dlopen("libvulkan.dylib", RTLD_NOW | RTLD_LOCAL);
	if (!module)
...
#else
	uk module = dlopen("libvulkan.so.1", RTLD_NOW | RTLD_LOCAL);
	if (!module)
