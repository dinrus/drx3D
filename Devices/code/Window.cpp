...
void CallbackDrop(GLFWwindow *glfwWindow, int32_t count, tukk* paths) {
	auto window = static_cast<Window *>(glfwGetWindowUserPointer(glfwWindow));
	std::vector<STxt> files(static_cast<uint32_t>(count));
...
Window::Window(std::size_t id) :
	...
	if (!window) {
		glfwTerminate();
		throw drx::Exc("GLFW failed to create the window");
	}
...
void Window::SetTitle(const STxt &title) {
	this->title = title;
...
STxt Window::GetClipboard() const {
	return glfwGetClipboardString(window);
}

void Window::SetClipboard(const STxt &string) const {
...
int32_t OverlappingArea(Vector2i l1, Vector2i r1, Vector2i l2, Vector2i r2) {
	i32 area1 = std::abs(l1.x - r1.x) * std::abs(l1.y - r1.y);

	i32 area2 = std::abs(l2.x - r2.x) * std::abs(l2.y - r2.y);

	i32 areaI = (std::min(r1.x, r2.x) - std::max(l1.x, l2.x)) *
