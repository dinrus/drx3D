...
#include <drx3D/Devices/Cursor.h>
...
public:
	 ...
	 */
	const STxt &GetTitle() const { return title; }

	/**
	 * Sets window title.
	 * @param title The new title.
	 */
	void SetTitle(const STxt &title);
	 ...
	 */
	STxt GetClipboard() const;

	/**
	 * Sets the clipboard to the specified string.
	 * @param string The string to set as the clipboard.
	 */
	void SetClipboard(const STxt &string) const;
	...
	static STxt ToString(Key key);

	DRX3D_NO_EXPORT GLFWwindow *GetWindow() const { return window; }
	 ...
	 * @return The rocket::signal.
	 */
	rocket::signal<void(STxt)> &OnTitle() { return onTitle; }
	 ...
	 * @return The delegate.
	 */
	rocket::signal<void(std::vector<STxt>)> &OnDrop() { return onDrop; }
...
private:
	...
	friend void CallbackDrop(GLFWwindow *glfwWindow, int32_t count, tukk* paths);
	...
	rocket::signal<void(STxt)> onTitle;
	...
	rocket::signal<void(std::vector<STxt>)> onDrop;
...
template<>
struct enum_range<drx3d::Key> {
	inline constexpr static i32 min = -1;
	inline constexpr static i32 max = 400;
};
}
