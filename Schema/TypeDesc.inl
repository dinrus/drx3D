...
inline void ArrayToString(IString& output, const CArrayTypeDesc& typeDesc, const void* pInput)
{
	...
	{
		u32k size = (*arrayOperators.size)(pInput);
...
public:
	...
	inline void Serialize(Serialization::IArchive& archive)
	{
		for (const CClassBaseDesc& base : m_typeDesc.GetBases())
		{
			...
			{
				(*typeOperators.serialize)(archive, static_cast<u8*>(m_pValue) + base.GetOffset(), baseTypeDesc.GetName().c_str(), baseTypeDesc.GetLabel());
			}
		}
		...
		for (const CClassMemberDesc& memberDesc : m_typeDesc.GetMembers())
		{
			...
			if (typeOperators.serialize)
			{
				(*typeOperators.serialize)(archive, static_cast<u8*>(m_pValue) + memberDesc.GetOffset(), memberDesc.GetName(), memberDesc.GetLabel());
			}
		}
	}
...
inline void ClassToString(IString& output, const CClassDesc& typeDesc, const void* pInput)
{
	...
	for (const CClassBaseDesc& base : typeDesc.GetBases())
	{
		...
		{
			CStackString temp;
			(*typeOperators.toString)(temp, static_cast<u8k*>(pInput) + base.GetOffset());
	...
	for (const CClassMemberDesc& memberDesc : typeDesc.GetMembers())
	{
		...
		{
			CStackString temp;
			(*typeOperators.toString)(temp, static_cast<u8k*>(pInput) + memberDesc.GetOffset());
...
inline void CCommonTypeDesc::SetGUID(const DrxGUID& guid)
{
	m_guid = guid;
}

inline const DrxGUID& CCommonTypeDesc::GetGUID() const
{
	return m_guid;
}
...
inline CClassMemberDesc::CClassMemberDesc(const CCommonTypeDesc& typeDesc, ptrdiff_t offset, u32 id, const char* szName, const char* szLabel, const char* szDescription, Utils::IDefaultValuePtr&& pDefaultValue)
	: m_typeDesc(typeDesc)
	, m_offset(offset)
	, m_id(id)
	, m_szName(szName)
...
inline u32 CClassMemberDesc::GetId() const
{
	return m_id;
}
...
inline const CClassBaseDesc* CClassDesc::FindBaseByTypeID(const DrxGUID& typeId) const
{
	for (const CClassBaseDesc& base : m_bases)
	{
...
inline const CClassMemberDesc* CClassDesc::FindMemberById(u32 id) const
{
	for (const CClassMemberDesc& member : m_members)
	{
		if (member.GetId() == id)
...
// Convert member id 
inline u32 CClassDesc::FindMemberIndexById(u32 id) const
{
...
inline CClassMemberDesc& CClassDesc::AddMember(const CCommonTypeDesc& typeDesc, ptrdiff_t offset, u32 id, const char* szName, const char* szLabel, const char* szDescription, Utils::IDefaultValuePtr&& pDefaultValue)
{
	CClassMemberDesc* pNullMember = nullptr;

	const bool bDuplicateOffset = FindMemberByOffset(offset) != nullptr;
	DRX_ASSERT(!bDuplicateOffset);
	if (bDuplicateOffset)
	{
...
template<typename TYPE> inline DrxGUID GetTypeGUID()
{
	SXEMA_VERIFY_TYPE_IS_REFLECTED(TYPE);
