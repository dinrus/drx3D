...
// #SchematycTODO : Merge with DrxFlags?
...
public:
	...
	void Serialize(Serialization::IArchive& archive)
	{
		...
		{
			m_value = m_base;
			for (i32 flagIdx = 0, flagCount = enumDescription.count(); flagIdx < flagCount; ++flagIdx)
			{
				i32k flag = enumDescription.valueByIndex(flagIdx);
...
template<typename VALUE_TYPE> inline bool Serialize(Serialization::IArchive& archive, CEnumFlags<VALUE_TYPE>& value, tukk szName, tukk szLabel)
{
