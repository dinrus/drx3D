...
public:
	...
	static CAnyValuePtr                         MakeShared(const CCommonTypeDesc& typeDesc, ukk pValue);
...
inline uk CAnyValue::GetValue()
{
	return this + 1;
}

inline ukk CAnyValue::GetValue() const
{
	return this + 1;
}
...
inline CAnyValuePtr CAnyValue::MakeShared(const CCommonTypeDesc& typeDesc, ukk pValue)
{
	...
	SXEMA_CORE_ASSERT_MESSAGE(typeDesc.GetOperators().copyConstruct, "Type must provide copy constructor!");
	if (typeDesc.GetOperators().copyConstruct)
	{
		SXEMA_CORE_ASSERT(pValue);
		if (pValue)
		{
...
inline CAnyValuePtr CAnyValue::MakeSharedDefault(const CCommonTypeDesc& typeDesc)
{
	...
	SXEMA_CORE_ASSERT_MESSAGE(typeDesc.GetOperators().defaultConstruct, "Type must provide default constructor!");
...
inline CAnyValue::CAnyValue(const CCommonTypeDesc& typeDesc, ukk pValue)
	: m_typeDesc(typeDesc)
{
...
inline CAnyRef::CAnyRef(const CCommonTypeDesc& typeDesc, uk pValue)
	: m_pTypeDesc(&typeDesc)
	, m_pValue(pValue)
{
	SXEMA_CORE_ASSERT(IsValid());
}
...
inline uk CAnyRef::GetValue() const
{
	SXEMA_CORE_ASSERT(IsValid());
	return m_pValue;
}
...
inline CAnyConstRef::CAnyConstRef(const CCommonTypeDesc& typeDesc, ukk pValue)
	: m_pTypeDesc(&typeDesc)
	, m_pValue(pValue)
{
	SXEMA_CORE_ASSERT(IsValid());
...
inline ukk CAnyConstRef::GetValue() const
{
	return m_pValue;
}
...
inline CAnyPtr::CAnyPtr(const CCommonTypeDesc& typeDesc, uk pValue)
...
{
	SXEMA_CORE_ASSERT(m_ref.IsValid());
	return &m_ref;
}
...
inline CAnyConstPtr::CAnyConstPtr(const CCommonTypeDesc& typeDesc, ukk pValue)
	: m_ref(typeDesc, pValue)
{}
...
inline bool CopyAssign(const CAnyRef& lhs, const CAnyConstRef& rhs)
{
	...
	SXEMA_CORE_ASSERT_MESSAGE(typeDesc.GetOperators().copyAssign, "Type must provide copy assignment operator!");
	if (typeDesc.GetOperators().copyAssign)
	{
		ukk pRHSValue = Helpers::DynamicCast(rhs.GetTypeDesc(), rhs.GetValue(), typeDesc);
		SXEMA_CORE_ASSERT_MESSAGE(pRHSValue, "Type mismatch!");
		if (pRHSValue)
		{
...
{
	const CCommonTypeDesc& typeDesc = lhs.GetTypeDesc();
	SXEMA_CORE_ASSERT_MESSAGE(typeDesc.GetOperators().equals, "Type must provide equals operator!");
...
inline bool ToString(IString& output, const CAnyConstRef& input)
{
	...
	SXEMA_CORE_ASSERT_MESSAGE(typeDesc.GetOperators().toString, "Type must provide 'ToString' method!");
...
inline bool Serialize(Serialization::IArchive& archive, const CAnyRef& value, tukk szName, tukk szLabel)
{
	...
	SXEMA_CORE_ASSERT_MESSAGE(typeDesc.GetOperators().toString, "Type must provide 'Serialize' method!");
...
{
	const CCommonTypeDesc& typeDesc = value.GetTypeDesc();
	SXEMA_CORE_ASSERT_MESSAGE(typeDesc.GetOperators().serialize, "Type must provide 'Serialize' method!");
	if (typeDesc.GetOperators().serialize)
	{
		return (*typeDesc.GetOperators().serialize)(archive, value.GetValue(), szName, szLabel);
	}
