...
#define SXEMA_MAKE_ENV_FUNCTION(functionPtr, guid, name) sxema::EnvFunction::MakeShared(functionPtr, guid, name, SXEMA_SOURCE_FILE_INFO)
...
typedef void (* StubPtr)(const SBinding&, CRuntimeParamMap& params, uk pObject);
...
struct SBinding
{
	...
	u32                 inputCount;
	u32                 inputs[MaxParams];
	u32                 outputCount;
	u32                 outputs[MaxParams];
};
...
inline void BindInput(SBinding& binding, u32 paramIdx, u32 id, tukk szName, tukk szDescription, const CAnyValuePtr& pData)
{
	// #SchematycTODO : Make sure id is unique and name is valid!!!

	SXEMA_CORE_ASSERT_FATAL(paramIdx < MaxParams);
	SXEMA_CORE_ASSERT_FATAL(szName);

	SParamBinding& param = binding.params[paramIdx];
	SXEMA_CORE_ASSERT_FATAL(param.flags.Check(EParamFlags::AvailableInput));
...
inline void BindOutput(SBinding& binding, u32 paramIdx, u32 id, tukk szName, tukk szDescription)
{
	// #SchematycTODO : Make sure id is unique and name is valid!!!
	...
	SParamBinding& param = binding.params[paramIdx];
	SXEMA_CORE_ASSERT_FATAL(param.flags.Check(EParamFlags::AvailableOutput));
...
{
	for (u32 paramIdx = 0; paramIdx < MaxParams; ++paramIdx)
	{
...
template<typename TYPE> struct SParamReader
{
	...
	static inline ReturnType ReadParam(const SBinding& binding, u32 paramIdx, CRuntimeParamMap& params)
	{
		const SParamBinding& param = binding.params[paramIdx];
		SXEMA_CORE_ASSERT_FATAL(param.flags.Check(EParamFlags::BoundToInput));
...
template<typename TYPE> struct SParamReader<TYPE&>
{
	...
	{
		const SParamBinding& param = binding.params[paramIdx];
		SXEMA_CORE_ASSERT_FATAL(param.flags.Check(EParamFlags::BoundToOutput));
...
template<typename TYPE> inline typename SParamReader<TYPE>::ReturnType ReadParam(const SBinding& binding, u32 paramIdx, CRuntimeParamMap& params)
{
	return SParamReader<TYPE>::ReadParam(binding, paramIdx, params);
}

template<typename TYPE> inline void WriteParam(const SBinding& binding, u32 paramIdx, CRuntimeParamMap& params, const TYPE& value)
{
	const SParamBinding& param = binding.params[paramIdx];
	SXEMA_CORE_ASSERT_FATAL(param.flags.Check(EParamFlags::BoundToOutput));
...
struct SBinder<void (*)()>
{
	...
	static void Stub(const SBinding& binding, CRuntimeParamMap& params, uk pObject)
	{
...
public:
	...
	template<typename FUNCTION_PTR_TYPE> inline CEnvFunction(FUNCTION_PTR_TYPE pFunction, const DrxGUID& guid, tukk szName, const SSourceFileInfo& sourceFileInfo)
	...
	virtual u32 GetInputId(u32 inputIdx) const override
	{
	...
	virtual u32 GetOutputId(u32 outputIdx) const override
	{
	...
	virtual void Execute(CRuntimeParamMap& params, uk pObject) const override
	{
		if (m_binding.pStub)
		{
	...
	inline void BindInput(u32 paramIdx, u32 id, tukk szName, tukk szDescription = nullptr)
	{
		EnvFunctionUtils::BindInput(m_binding, paramIdx, id, szName, szDescription, CAnyValuePtr());
	}

	template<typename TYPE> inline void BindInput(u32 paramIdx, u32 id, tukk szName, tukk szDescription, const TYPE& value)
	{
		EnvFunctionUtils::BindInput(m_binding, paramIdx, id, szName, szDescription, CAnyValue::MakeShared(value));
	}

	inline void BindOutput(u32 paramIdx, u32 id, tukk szName, tukk szDescription = nullptr)
	{
...
template<typename FUNCTION_PTR_TYPE> inline std::shared_ptr<CEnvFunction> MakeShared(FUNCTION_PTR_TYPE pFunction, const DrxGUID& guid, tukk szName, const SSourceFileInfo& sourceFileInfo)
{
