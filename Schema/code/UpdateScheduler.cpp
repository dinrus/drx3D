...
CUpdateScheduler::SSlot::SSlot(u32 _firstBucketIdx, UpdateFrequency _frequency, UpdatePriority _priority)
...
void CUpdateScheduler::CBucket::Update(const SUpdateContext(&frequencyUpdateContexts)[EUpdateFrequency::Count], UpdatePriority beginPriority, UpdatePriority endPriority)
{
	...
	u32k endPos = m_observers.size() - m_dirtyCount;
...
bool CUpdateScheduler::Connect(const SUpdateParams& params)
{
	...
	if ((params.frequency >= EUpdateFrequency::EveryFrame) && (params.frequency < EUpdateFrequency::Count))
	{
		...
		u32k bucketStride = u32(1) << params.frequency;
		u32 firstBucketIdx = 0;
		if (params.frequency > EUpdateFrequency::EveryFrame)
		{
			u32 bucketGroupSize[BucketCount] = {};
			for (u32 bucketIdx = 0; bucketIdx < BucketCount; ++bucketIdx)
			{
				bucketGroupSize[bucketIdx % bucketStride] += m_buckets[bucketIdx].GetSize();
			}
			for (u32 bucketGroupIdx = 1; bucketGroupIdx < bucketStride; ++bucketGroupIdx)
			{
		...
		m_slots.push_back(SSlot(static_cast<u32>(firstBucketIdx), params.frequency, params.priority));
		m_slots.back().connection.Connect(params.scope, SXEMA_MEMBER_DELEGATE(&CUpdateScheduler::Disconnect, *this));
...
void CUpdateScheduler::Disconnect(CConnectionScope& scope)
{
	if (scope.HasConnections())
	{
		...
		if (itSlot != m_slots.end())
		{
			...
			u32k bucketStride = u32(1) << itSlot->frequency;
...
bool CUpdateScheduler::Update(UpdatePriority beginPriority, UpdatePriority endPriority)
{
	...
	if (m_bInFrame)
	{
		...
		{
			u32k frameCount = u32(1) << frequency;
			for (u32 frameIdx = 0; frameIdx < frameCount; ++frameIdx)
			{
