...
// #sxema : Ditch visit status support or implement correct visitor status handling!!!
...
// #SchematycTODO : No need to pass element type, this can be retrieved using ELEMENT::ElementType.
template<typename ELEMENT> void VisitScriptElementsInClass(const std::function<EVisitStatus(const ELEMENT&)>& visitor, EScriptElementType elementType, EDomainScope scope, DrxGUID scopeGUID)
{
...
template<typename ELEMENT> void VisitScriptElementsInModule(const std::function<EVisitStatus(const ELEMENT&)>& visitor, EScriptElementType elementType, DrxGUID scopeGUID)
{
	const IScriptElement* pScriptElement = gEnv->pSchematyc->GetScriptRegistry().GetElement(scopeGUID);
	if (pScriptElement)
	{
...
template<typename ELEMENT> void VisitScriptElements(const std::function<EVisitStatus(const ELEMENT&)>& visitor, EScriptElementType type, const DrxGUID& scopeGUID)
{
...
template<typename ELEMENT> void VisitEnclosedElememts(const DrxGUID& scopeGUID, const std::function<void(const ELEMENT&)>& visitor)
{
	const IScriptElement* pScriptScope = gEnv->pSchematyc->GetScriptRegistry().GetElement(scopeGUID);
	if (pScriptScope)
	{
...
template<typename ELEMENT> void VisitAccesibleElememts(const DrxGUID& scopeGUID, const std::function<void(const ELEMENT&)>& visitor)
{
	const IScriptElement* pScriptScope = gEnv->pSchematyc->GetScriptRegistry().GetElement(scopeGUID);
	if (pScriptScope)
	{
...
CScriptView::CScriptView(const DrxGUID& scopeGUID)
	: m_scopeGUID(scopeGUID)
{}

const DrxGUID& CScriptView::GetScopeGUID() const
{
	return m_scopeGUID;
}
...
const IScriptStateMachine* CScriptView::GetScriptStateMachine(const DrxGUID& guid) const
{
	return nullptr;
}

const IScriptComponentInstance* CScriptView::GetScriptComponentInstance(const DrxGUID& guid) const
{
...
const IScriptActionInstance* CScriptView::GetScriptActionInstance(const DrxGUID& guid) const
{
	return nullptr;
}

const IScriptElement* CScriptView::GetScriptElement(const DrxGUID& guid) const
{
