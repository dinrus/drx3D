...
CObject::STimer::STimer(CObject* _pObject, const DrxGUID& _guid, TimerFlags _flags)
	: pObject(_pObject)
	, guid(_guid)
	, flags(_flags)
{}
...
CObject::CObject(IEntity& entity, ObjectId id, uk pCustomData)
	: m_id(id)
...
bool CObject::Init(DrxGUID classGUID, const IObjectPropertiesPtr& pProperties)
{
	m_pProperties = pProperties;

	CRuntimeClassConstPtr pClass = CCore::GetInstance().GetRuntimeRegistryImpl().GetClassImpl(classGUID);
	DRX_ASSERT_MESSAGE(pClass, "sxema class '%s' couldn't be found in class registry.");
	if (!pClass)
	{
		return false;
	}
...
{
	DRX_ASSERT_MESSAGE(m_pClass, "Runtime class of sxema Object must be not null.");
	return *m_pClass;
}

tukk CObject::GetScriptFile() const
{
	const IScriptElement* pElement = CCore::GetInstance().GetScriptRegistry().GetElement(m_pClass->GetGUID());
	DRX_ASSERT_MESSAGE(pElement, "Script Element not found!");
	if (pElement && pElement->GetType() == EScriptElementType::Class || pElement->GetType() == EScriptElementType::Module)
	{
		...
		DRX_ASSERT_MESSAGE(pScript, "Script Element doesn't have a script.");
		if (pScript)
		{
...
uk CObject::GetCustomData() const
{
...
void CObject::ProcessSignal(const SObjectSignal& signal)
{
	...
	else
	{
		...
		u32k stateMachineCount = m_stateMachines.size();
		for (u32 stateMachineIdx = 0; stateMachineIdx < stateMachineCount; ++stateMachineIdx)
		{
		...
		SXEMA_CORE_ASSERT(m_bQueueSignals);
		m_bQueueSignals = false;
		ProcessSignalQueue();
	}
}
...
void CObject::Dump(IObjectDump& dump, const ObjectDumpFlags& flags) const
{
	if (flags.Check(EObjectDumpFlags::States))
	{
		...
		for (u32 stateMachineIdx = 0, stateMachineCount = m_stateMachines.size(); stateMachineIdx < stateMachineCount; ++stateMachineIdx)
		{
			...
			u32k stateIdx = m_stateMachines[stateMachineIdx].stateIdx;
	...
	if (flags.Check(EObjectDumpFlags::Timers))
	{
		...
		for (u32 timerIdx = 0, timerCount = m_timers.size(); timerIdx < timerCount; ++timerIdx)
		{
...
bool CObject::ExecuteFunction(u32 functionIdx, CRuntimeParamMap& params)
{
...
bool CObject::StartAction(u32 actionIdx, CRuntimeParamMap& params)
{
...
IEntityComponent* CObject::GetComponent(u32 componentIdx)
{
...
{
	u32k graphCount = m_pClass->GetGraphCount();
	...
	if (m_pProperties)
	{
		for (const SRuntimeClassVariable& variable : m_pClass->GetVariables())
		{
			if (variable.bPublic)
			{
				...
				{
					SXEMA_CORE_ERROR("Failed to initialize variable: class = %s, variable = %s", m_pClass->GetName(), variable.name.c_str());
					//return false;
...
bool CObject::CreateStateMachines()
{
	...
	const RuntimeClassStates& classStates = m_pClass->GetStates();
	u32k stateCount = classStates.size();
	m_states.resize(stateCount);

	ITimerSystem& timerSystem = gEnv->pSchematyc->GetTimerSystem();
	for (u32 stateIdx = 0; stateIdx < stateCount; ++stateIdx)
	{
		const SRuntimeClassState& classState = classStates[stateIdx];
		SState& state = m_states[stateIdx];

		u32k stateTimerCount = classState.timers.size();
		...
		for (const SRuntimeClassStateTimer& classStateTimer : classState.timers)
		{
			...
			timer.id = timerSystem.CreateTimer(timerParams, SXEMA_MEMBER_DELEGATE(&STimer::Activate, timer));
		}
	}
...
bool CObject::CreateComponents()
{
	...
	u32k componentCount = classComponentInstances.size();
	...
	if (IEntity* pEntity = GetEntity())
	{
		for (auto componentIt = m_components.begin(), end = m_components.end(); componentIt != end; ++componentIt)
		{
			...
			EntityComponentFlags flags(EEntityComponentFlags::sxema);
...
bool CObject::CreateActions()
{
	...
	u32k actionCount = actionDescs.size();
...
void CObject::RegisterForUpdate()
{
	...
	{
		SUpdateParams updateParams(SXEMA_MEMBER_DELEGATE(&CObject::Update, *this), m_connectionScope);
...
void CObject::ExecuteSignalReceivers(const SObjectSignal& signal)
{
	for (const SRuntimeClassSignalReceiver& classSignalReceiver : m_pClass->GetSignalReceivers())
	{
		...
		if (classSignalReceiver.signalGUID == signal.typeGUID)
		{
			...
			{
				SXEMA_CORE_CRITICAL_ERROR("Signal sender guid expected!");
			}
		}
...
void CObject::StartStateTimers(u32 stateMachineIdx)
{
	u32k stateIdx = m_stateMachines[stateMachineIdx].stateIdx;
	if (stateIdx != InvalidIdx)
	{
...
void CObject::StopStateTimers(u32 stateMachineIdx)
{
	u32k stateIdx = m_stateMachines[stateMachineIdx].stateIdx;
	if (stateIdx != InvalidIdx)
	{
...
void CObject::ExecuteStateSignalReceivers(u32 stateMachineIdx, const SObjectSignal& signal)
{
	u32k stateIdx = m_stateMachines[stateMachineIdx].stateIdx;
	if (stateIdx != InvalidIdx)
	{
...
bool CObject::EvaluateStateTransitions(u32 stateMachineIdx, const SObjectSignal& signal)
{
	u32k stateIdx = m_stateMachines[stateMachineIdx].stateIdx;
	if (stateIdx != InvalidIdx)
	{
...
void CObject::ChangeState(u32 stateMachineIdx, u32 stateIdx)
{
...
void CObject::ExecuteFunction(u32 graphIdx, CRuntimeParamMap& params, SRuntimeActivationParams activationParams)
{
