...
typedef u16 RuntimeGraphNodeIdx;
...
typedef u8 RuntimeGraphPortIdx;
...
enum class ERuntimeStatus : u8 // #SchematycTODO : Simplify this enumeration by using flags?
{
	Continue = 0,
	ContinueAndRepeat,
	Break,
...
public:
	...
	inline CRuntimeGraphNode(const DrxGUID& guid, tukk szName, RuntimeGraphNodeCallbackPtr pCallback, RuntimeGraphPortIdx inputCount, RuntimeGraphPortIdx outputCount)
		: m_guid(guid)
		, m_name(szName)
	...
	inline void SetDataPos(u32 dataPos)
	{
		m_dataPos = dataPos;
	}

	inline u32 GetDataPos() const
	{
		return m_dataPos;
	}
	...
	inline bool ConfigureInput(RuntimeGraphPortIdx inputIdx, u32 dataPos, RuntimeGraphNodeIdx linkNodeIdx, RuntimeGraphPortIdx linkPortIdx, u8 flags)
	{
		// Validate parameters.
	...
	inline bool ConfigureOutput(RuntimeGraphPortIdx outputIdx, u32 dataPos, RuntimeGraphNodeIdx linkNodeIdx, RuntimeGraphPortIdx linkPortIdx, u8 flags)
	{
		// Validate parameters.
...
public:
	...
	inline CRuntimeGraph(const DrxGUID& guid, tukk szName)
		: m_guid(guid)
		, m_name(szName)
	{}
	...
	inline bool AddNode(const DrxGUID& guid, tukk szName, RuntimeGraphNodeCallbackPtr pCallback, RuntimeGraphPortIdx inputCount, RuntimeGraphPortIdx outputCount)
	{
...
public:
	...
	bool Execute(uk pObject, CRuntimeParamMap& params, SRuntimeActivationParams activationParams);
...
private:
	...
	ERuntimeStatus PullNodesRecursive(uk pObject, CRuntimeParamMap& params, const CRuntimeGraphNode& node);
...
struct SRuntimeContext
{
	inline SRuntimeContext(uk _pObject, CRuntimeParamMap& _params, const CRuntimeGraphInstance& _graph, CRuntimeGraphNodeInstance& _node)
		: pObject(_pObject)
		, params(_params)
		, graph(_graph)
		, node(_node)
...
inline bool CRuntimeGraphInstance::Execute(uk pObject, CRuntimeParamMap& params, SRuntimeActivationParams activationParams)
{
...
inline ERuntimeStatus CRuntimeGraphInstance::PullNodesRecursive(uk pObject, CRuntimeParamMap& params, const CRuntimeGraphNode& node)
{
	const SRuntimeGraphPort* pInputs = node.GetInputs();
	for (u32 inputIdx = 0, inputCount = node.GetInputCount(); inputIdx < inputCount; ++inputIdx)
	{
