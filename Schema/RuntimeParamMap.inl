...
inline void CRuntimeParamMap::ReserveInputs(u32 capacity)
{
...
inline void CRuntimeParamMap::ReserveOutputs(u32 capacity)
{
...
inline bool FromInputClass(const CClassDesc& typeDesc, CRuntimeParamMap& params, const void* pInput)
{
	...
	{
		if (!params.BindInput(CUniqueId::FromUInt32(memberDesc.GetId()), CAnyConstPtr(memberDesc.GetTypeDesc(), reinterpret_cast<u8k*>(pInput) + memberDesc.GetOffset())))
		{
			return false;
		}
	}
...
inline bool ToInputClass(const CClassDesc& typeDesc, void* pOutput, const CRuntimeParamMap& params)
{
	for (const CClassMemberDesc& memberDesc : typeDesc.GetMembers())
	{
		if (!params.GetInput(CUniqueId::FromUInt32(memberDesc.GetId()), CAnyRef(memberDesc.GetTypeDesc(), reinterpret_cast<u8*>(pOutput) + memberDesc.GetOffset())))
		{
			return false;
		}
	}
