...
{
public:
	idScalar& operator()(i32 i) { return m_data[i]; }
	const idScalar& operator()(i32 i) const { return m_data[i]; }
	i32k size() const { return 3; }
...
{
public:
	idScalar& operator()(i32 i, i32 j) { return m_data[3 * i + j]; }
	const idScalar& operator()(i32 i, i32 j) const { return m_data[3 * i + j]; }
...
public:
	...
	matxx(i32 rows, i32 cols) : m_rows(rows), m_cols(cols)
	{
		m_data = static_cast<idScalar*>(idMalloc(sizeof(idScalar) * rows * cols));
	}
	~matxx() { idFree(m_data); }
	idScalar& operator()(i32 row, i32 col) { return m_data[row * m_cols + col]; }
	const idScalar& operator()(i32 row, i32 col) const { return m_data[row * m_cols + col]; }
	i32k& rows() const { return m_rows; }
	i32k& cols() const { return m_cols; }
...
public:
	...
	mat3x(i32 rows, i32 cols) : m_cols(cols)
	{
		allocate();
	};
	...
	// avoid operators that would allocate -- use functions sub/add/mul in IDMath.h instead
private:
...
inline const vecx& vecx::operator=(const vecx& rhs)
{
	if (size() != rhs.size())
	{
		drx3d_id_error_message("size missmatch, size()= %d but rhs.size()= %d\n", size(), rhs.size());
		abort();
	}
	if (&rhs != this)
	{
