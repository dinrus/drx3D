...
Rigidbody::~Rigidbody() {
	if (auto body = RigidBody::upcast(this->body); body && body->getMotionState())
...
void Rigidbody::Start() {
	...
	auto motionState = new DefaultMotionState(worldTransform);
	RigidBody::RigidBodyConstructionInfo cInfo(mass, motionState, shape.get(), localInertia);

	rigidBody = std::make_unique<RigidBody>(cInfo);
