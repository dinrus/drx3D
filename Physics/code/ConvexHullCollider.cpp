...
CollisionShape *ConvexHullCollider::GetCollisionShape() const {
	return shape.get();
}
...
void ConvexHullCollider::SetPointCount(const std::vector<float> &pointCloud) {
	...
	shape = std::make_unique<ConvexHullShape>(pointCloud.data(), static_cast<int32_t>(pointCloud.size() / 3), static_cast<int32_t>(sizeof(float)));
