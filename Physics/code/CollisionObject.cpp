...
void CollisionObject::SetChildTransform(Collider *child, const Transform &transform) {
	auto compoundShape = dynamic_cast<::CompoundShape *>(shape.get());
	if (!compoundShape)
		return;
...
void CollisionObject::AddChild(Collider *child) {
	if (auto compoundShape = dynamic_cast<CompoundShape *>(shape.get())) {
...
void CollisionObject::CreateShape(bool forceSingle) {
	...
	if (!dynamic_cast<::CompoundShape *>(shape.get()))
		shape.release();

	auto compoundShape = new ::CompoundShape();
