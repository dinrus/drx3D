#ifndef D3_FIND_CONCAVE_SEPARATING_AXIS_H
#define D3_FIND_CONCAVE_SEPARATING_AXIS_H

#define D3_TRIANGLE_NUM_CONVEX_FACES 5
						  ...
						  float* dmin)
{
	...
	posB.w = 0.f;
	/*
	static i32 maxFaceVertex = 0;

	i32 curFaceVertexAB = hullA->m_numFaces*hullB->m_numVertices;
								  ...
								  bool searchAllEdgeEdge)
{
	...
	i32 curEdgeEdge = 0;
	// Test edges
	static i32 maxEdgeTests = 0;
	i32 curEdgeTests = hullA->m_numUniqueEdges * hullB->m_numUniqueEdges;
							   ...
							   __global b3Int4* clippingFaces, i32 pairIndex)
{
	...
	i32 numVerticesA = facesA[hullA->m_faceOffset + closestFaceA].m_numIndices;
...
__kernel void b3FindConcaveSeparatingAxisKernel(__global b3Int4* concavePairs,
												...
												__global b3Vec3* worldVertsA1Out,
												__global b3Vec3* worldNormalsA1Out,
												__global b3Vec3* worldVertsB1Out,
												__global i32* hasSeparatingNormals,
												...
												i32 pairIdx)
{
	...
	*/
	...
	i32 numActualConcaveConvexTests = 0;
	...
	if (overlap)
	{
		...
		b3GpuFace facesA[D3_TRIANGLE_NUM_CONVEX_FACES];
		i32 indicesA[3 + 3 + 2 + 2 + 2];
		i32 curUsedIndices = 0;
		i32 fidx = 0;
		...
		convexPolyhedronA.m_numFaces = D3_TRIANGLE_NUM_CONVEX_FACES;
		...
		{
			i32 compoundChild = concavePairs[pairIdx].w;
			i32 childShapeIndexB = compoundChild;  //collidables[collidableIndexB].m_shapeIndex+compoundChild;
...
#endif  //D3_FIND_CONCAVE_SEPARATING_AXIS_H
