...
static tukk satKernelsCL =
	...
	"	i32			m_subTreeOffset;\n"
	"} b3BvhInfo;\n"
	"i32	getTriangleIndex(const b3QuantizedBvhNode* rootNode)\n"
	"{\n"
	"	u32 x=0;\n"
	...
	"}\n"
	"i32	getTriangleIndexGlobal(__global const b3QuantizedBvhNode* rootNode)\n"
	"{\n"
	"	u32 x=0;\n"
	"	u32 y = (~(x&0))<<(31-MAX_NUM_PARTS_IN_BITS);\n"
	"	// Get only the lower bits where the triangle index is stored\n"
	"	return (rootNode->m_escapeIndexOrTriangleIndex&~(y));\n"
	"}\n"
	"i32 isLeafNode(const b3QuantizedBvhNode* rootNode)\n"
	"{\n"
	"	//skipindex is negative (internal node), triangleindex >=0 (leafnode)\n"
	"	return (rootNode->m_escapeIndexOrTriangleIndex >= 0)? 1 : 0;\n"
	"}\n"
	"i32 isLeafNodeGlobal(__global const b3QuantizedBvhNode* rootNode)\n"
	"{\n"
	"	//skipindex is negative (internal node), triangleindex >=0 (leafnode)\n"
	"	return (rootNode->m_escapeIndexOrTriangleIndex >= 0)? 1 : 0;\n"
	"}\n"
	"	\n"
	"i32 getEscapeIndex(const b3QuantizedBvhNode* rootNode)\n"
	"{\n"
	"	return -rootNode->m_escapeIndexOrTriangleIndex;\n"
	"}\n"
	"i32 getEscapeIndexGlobal(__global const b3QuantizedBvhNode* rootNode)\n"
	"{\n"
	"	return -rootNode->m_escapeIndexOrTriangleIndex;\n"
	"}\n"
	"typedef struct\n"
	...
	"																					__global  float4* gpuCompoundSepNormalsOut,\n"
	"																					__global  i32* gpuHasCompoundSepNormalsOut,\n"
	...
	"	__global const GpuChildShape* gpuChildShapes,\n"
	"	__global  int4* gpuCompoundPairsOut,\n"
	...
	"			i32 numSubTreesA = bvhInfos[bvhA].m_numSubTrees;\n"
	"			i32 subTreesOffsetA = bvhInfos[bvhA].m_subTreeOffset;\n"
	"			i32 subTreesOffsetB = bvhInfos[bvhB].m_subTreeOffset;\n"
	"			i32 numSubTreesB = bvhInfos[bvhB].m_numSubTrees;\n"
	"			\n"
	"			float4 posA = rigidBodies[bodyIndexA].m_pos;\n"
	...
	"					{\n"
	"						\n"
	"						i32 startNodeIndexA = subtreeA.m_rootNodeIndex+bvhInfos[bvhA].m_nodeOffset;\n"
	"						i32 endNodeIndexA = startNodeIndexA+subtreeA.m_subtreeSize;\n"
	"						i32 startNodeIndexB = subtreeB.m_rootNodeIndex+bvhInfos[bvhB].m_nodeOffset;\n"
	"						i32 endNodeIndexB = startNodeIndexB+subtreeB.m_subtreeSize;\n"
	"						b3Int2 nodeStack[D3_MAX_STACK_DEPTH];\n"
	"						b3Int2 node0;\n"
	"						node0.x = startNodeIndexA;\n"
	"						node0.y = startNodeIndexB;\n"
	"						i32 maxStackDepth = D3_MAX_STACK_DEPTH;\n"
	"						i32 depth=0;\n"
	...
	"								{\n"
	"									//printf(\"Ошибка: traversal exceeded maxStackDepth\");\n"
	"									continue;\n"
	"								}\n"
	"								if(isInternalA)\n"
	"								{\n"
	...
	"									i32 nodeArightChild = isNodeALeftChildLeaf? node.x+2 : node.x+1 + getEscapeIndexGlobal(&quantizedNodes[node.x+1]);\n"
	"									if(isInternalB)\n"
	"									{					\n"
	"										i32 nodeBleftChild = node.y+1;\n"
	"										bool isNodeBLeftChildLeaf = isLeafNodeGlobal(&quantizedNodes[node.y+1]);\n"
	"										i32 nodeBrightChild = isNodeBLeftChildLeaf? node.y+2 : node.y+1 + getEscapeIndexGlobal(&quantizedNodes[node.y+1]);\n"
	...
	"									else\n"
	"									{\n"
	"										i32 compoundPairIdx = atomic_inc(numCompoundPairsOut);\n"
	"										if (compoundPairIdx<maxNumCompoundPairsCapacity)\n"
	"										{\n"
	"											i32 childShapeIndexA = getTriangleIndexGlobal(&quantizedNodes[node.x]);\n"
	"											i32 childShapeIndexB = getTriangleIndexGlobal(&quantizedNodes[node.y]);\n"
	...
	"	}//(i<numPairs)\n"
	"}\n"
	"inline i32	findClippingFaces(const float4 separatingNormal,\n"
	...
	"		i32 numVertices = polyB.m_numIndices;\n"
	...
	"																					__global i32* concaveHasSeparatingNormals,\n"
