...
static tukk satClipKernelsCL =
	...
	"i32 clipFaceGlobal(__global const float4* pVtxIn, i32 numVertsIn, float4 planeNormalWS,float planeEqWS, __global float4* ppVtxOut)\n"
	"{\n"
	"	\n"
	"	i32 ve;\n"
	"	float ds, de;\n"
	...
	"	return numVertsOut;\n"
	"}\n"
	"i32 clipFaceAgainstHull(const float4 separatingNormal, __global const b3ConvexPolyhedronData_t* hullA,  \n"
	"	const float4 posA, const Quat ornA, float4* worldVertsB1, i32 numWorldVertsB1,\n"
	...
	"	return numContactsOut;\n"
	"}\n"
	"i32 clipFaceAgainstHullLocalA(const float4 separatingNormal, const b3ConvexPolyhedronData_t* hullA,  \n"
	"	const float4 posA, const Quat ornA, float4* worldVertsB1, i32 numWorldVertsB1,\n"
	"	float4* worldVertsB2, i32 capacityWorldVertsB2,\n"
	"	const float minDist, float maxDist,\n"
	...
	"	return numContactsOut;\n"
	"}\n"
	"i32	clipHullAgainstHull(const float4 separatingNormal,\n"
	"	__global const b3ConvexPolyhedronData_t* hullA, __global const b3ConvexPolyhedronData_t* hullB, \n"
	"	const float4 posA, const Quat ornA,const float4 posB, const Quat ornB, \n"
	...
	"	return numContactsOut;\n"
	"}\n"
	"i32	clipHullAgainstHullLocalA(const float4 separatingNormal,\n"
	...
	"	return numContactsOut;\n"
	"}\n"
	"#define PARALLEL_SUM(v, n) for(i32 j=1; j<n; j++) v[0] += v[j];\n"
	"#define PARALLEL_DO(execution, n) for(i32 ie=0; ie<n; ie++){execution;}\n"
	"#define REDUCE_MAX(v, n) {i32 i=0; for(i32 offset=0; offset<n; offset++) v[i] = (v[i].y > v[i+offset].y)? v[i]: v[i+offset]; }\n"
	"#define REDUCE_MIN(v, n) {i32 i=0; for(i32 offset=0; offset<n; offset++) v[i] = (v[i].y < v[i+offset].y)? v[i]: v[i+offset]; }\n"
	"i32 extractManifoldSequentialGlobal(__global const float4* p, i32 nPoints, float4 nearNormal, int4* contactIdx)\n"
	"{\n"
	"	if( nPoints == 0 )\n"
	"        return 0;\n"
	"    \n"
	...
	"    \n"
	"}\n"
	"i32 extractManifoldSequentialGlobalFake(__global const float4* p, i32 nPoints, float4 nearNormal, i32* contactIdx)\n"
	"{\n"
	"    contactIdx[0] = 0;\n"
	"    contactIdx[1] = 1;\n"
	"    contactIdx[2] = 2;\n"
	...
	"    \n"
	"}\n"
	"i32 extractManifoldSequential(const float4* p, i32 nPoints, float4 nearNormal, i32* contactIdx)\n"
	"{\n"
	"	if( nPoints == 0 ) return 0;\n"
	...
	"					}\n"
	"					idx[0] = (i32)a[0].x & 0xff;\n"
	"					idx[1] = (i32)a[0].y & 0xff;\n"
	"					idx[2] = (i32)a[0].z & 0xff;\n"
	"					idx[3] = (i32)a[0].w & 0xff;\n"
	"				}\n"
	"			}\n"
	"			{\n"
	"				float2 h[64];\n"
	...
	"																	__global i32k* contactCounts,\n"
	"																	__global i32k* contactOffsets,\n"
	...
	"																	)\n"
	"{\n"
	"	i32 idx = get_global_id(0);\n"
	"	\n"
	"	if (idx<numPairs)\n"
	"	{\n"
	"		float4 normal = separatingNormalsWorld[idx];\n"
	"		i32 nPoints = contactCounts[idx];\n"
	...
	"		}\n"
	"		i32 contactIdx[4];// = {-1,-1,-1,-1};\n"
	"		contactIdx[0] = -1;\n"
	"		contactIdx[1] = -1;\n"
	"		contactIdx[2] = -1;\n"
	"		contactIdx[3] = -1;\n"
	"		i32 nContacts = extractManifoldSequential(localPoints, nPoints, normal, contactIdx);\n"
	"		i32 dstIdx;\n"
	...
	"			\n"
	"		\n"
	"			i32 numLocalContactsOut = clipHullAgainstHull(separatingNormals[i],\n"
	...
	"				contactIdx[3] = -1;\n"
	"		\n"
	"				i32 nReducedContacts = extractManifoldSequential(pointsIn, nPoints, normal, contactIdx);\n"
	"		\n"
	"				\n"
	"				i32 mprContactIndex = pairs[pairIndex].z;\n"
	"				i32 dstIdx = mprContactIndex;\n"
	"				if (dstIdx<0)\n"
	"				{\n"
	...
	"																					__global i32k* gpuHasCompoundSepNormalsOut,\n"
	"																					__global struct b3Contact4Data* restrict globalContactsOut,\n"
	"																					counter32_t nGlobalContactsOut,\n"
	...
	"			return;\n"
	"		i32 bodyIndexA = concavePairsIn[i].x;\n"
	"		i32 bodyIndexB = concavePairsIn[i].y;\n"
	"		i32 f = concavePairsIn[i].z;\n"
	"		i32 childShapeIndexA = f;\n"
	"		\n"
	"		i32 collidableIndexA = rigidBodies[bodyIndexA].m_collidableIdx;\n"
	"		i32 collidableIndexB = rigidBodies[bodyIndexB].m_collidableIdx;\n"
	"		\n"
	...
	"		\n"
	"		\n"
	"		i32 numLocalContactsOut = clipHullAgainstHullLocalA(sepAxis,\n"
	...
	"				c->m_restituitionCoeffCmp = (0.f*0xffff);c->m_frictionCoeffCmp = (0.7f*0xffff);\n"
	"				c->m_batchIdx = pairIndex;\n"
	"				i32 bodyA = concavePairsIn[pairIndex].x;\n"
	"				i32 bodyB = concavePairsIn[pairIndex].y;\n"
	"				c->m_bodyAPtrAndSignBit = rigidBodies[bodyA].m_invMass==0?-bodyA:bodyA;\n"
	"				c->m_bodyBPtrAndSignBit = rigidBodies[bodyB].m_invMass==0?-bodyB:bodyB;\n"
	"				c->m_childIndexA = childShapeIndexA;\n"
	"				c->m_childIndexB = childShapeIndexB;\n"
	...
	"	}//	if (i<numPairs)\n"
	"}\n"
	"i32	findClippingFaces(const float4 separatingNormal,\n"
	"                      __global const b3ConvexPolyhedronData_t* hullA, __global const b3ConvexPolyhedronData_t* hullB,\n"
	"                      const float4 posA, const Quat ornA,const float4 posB, const Quat ornB,\n"
	...
	"                      __global const b3GpuFace_t* faces,\n"
	"                      __global i32k* indices,\n"
	"                      __global int4* clippingFaces, i32 pairIndex)\n"
	"{\n"
	"	i32 numContactsOut = 0;\n"
	"	i32 numWorldVertsB1= 0;\n"
	"    \n"
	...
	"	}\n"
	"    \n"
	"    i32 numVerticesA = faces[hullA->m_faceOffset+closestFaceA].m_numIndices;\n"
	"	for(i32 e0=0;e0<numVerticesA;e0++)\n"
	"	{\n"
	...
	"	return numContactsOut;\n"
	"}\n"
	"i32 clipFaces(__global float4* worldVertsA1,\n"
	"              __global float4* worldNormalsA1,\n"
	"              __global float4* worldVertsB1,\n"
	"              __global float4* worldVertsB2, \n"
	"              i32 capacityWorldVertsB2,\n"
	"              const float minDist, float maxDist,\n"
	...
	"            \n"
	"            \n"
	"			i32 numLocalContactsOut = findClippingFaces(separatingNormals[i],\n"
	"                                                        &convexShapes[shapeIndexA], &convexShapes[shapeIndexB],\n"
	"                                                        rigidBodies[bodyIndexA].m_pos,rigidBodies[bodyIndexA].m_quat,\n"
	"                                                        rigidBodies[bodyIndexB].m_pos,rigidBodies[bodyIndexB].m_quat,\n"
	"                                                        worldVertsA1,\n"
