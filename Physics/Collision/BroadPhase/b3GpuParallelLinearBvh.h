#ifndef D3_GPU_PARALLEL_LINEAR_BVH_H
#define D3_GPU_PARALLEL_LINEAR_BVH_H
...
#include <drx3D/Physics/Collision/BroadPhase/kernels/parallelLinearBvhKernels.h>
...
class b3GpuParallelLinearBvh
{
	...
	//1 element
	b3OpenCLArray<i32> m_rootNodeIndex;        //Most significant bit(0x80000000) is set to indicate internal node
	b3OpenCLArray<i32> m_maxDistanceFromRoot;  //d3Max number of internal nodes between an internal node and the root node
	b3OpenCLArray<i32> m_temp;                 //Used to hold the number of pairs in calculateOverlappingPairs()
	...
	b3OpenCLArray<i32> m_internalNodeParentNodes;         //For parent node index, msb(0x80000000) is not set since it is always internal

	//1 element per internal node; for binary radix tree construction
	b3OpenCLArray<b3Int64> m_commonPrefixes;
	b3OpenCLArray<i32> m_commonPrefixLengths;
	b3OpenCLArray<i32> m_distanceFromRoot;  //Number of internal nodes between this node and the root

	//1 element per leaf node (leaf nodes only include small AABBs)
	b3OpenCLArray<i32> m_leafNodeParentNodes;                //For parent node index, msb(0x80000000) is not set since it is always internal
...
public:
	...
	void build(const b3OpenCLArray<b3SapAabb>& worldSpaceAabbs, const b3OpenCLArray<i32>& smallAabbIndices,
