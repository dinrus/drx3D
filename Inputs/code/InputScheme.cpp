...
InputAxis *InputScheme::GetAxis(const STxt &name) {
...
InputAxis *InputScheme::AddAxis(const STxt &name, std::unique_ptr<InputAxis> &&axis) {
	return axes.emplace(name, std::move(axis)).first->second.get();
}

void InputScheme::RemoveAxis(const STxt &name) {
	if (auto it = axes.find(name); it != axes.end())
		axes.erase(it);
}

InputButton *InputScheme::GetButton(const STxt &name) {
...
InputButton *InputScheme::AddButton(const STxt &name, std::unique_ptr<InputButton> &&button) {
	return buttons.emplace(name, std::move(button)).first->second.get();
}

void InputScheme::RemoveButton(const STxt &name) {
