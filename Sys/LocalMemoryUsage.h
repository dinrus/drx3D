...
private:
	...
	struct SResource
	{
		...
		static i32         m_arrMemoryUsage[LOCALMEMORY_SECTOR_PER_PASS];             // Memory usage of the resource in sectors (last row)
		static i32         m_arrPieces[LOCALMEMORY_SECTOR_PER_PASS];                  // Pieces nr of the resource in sectors (last row)
	...
	struct STextureInfo : public SResource
	{
		...
		//i32 m_size;																			// For later use - Used memory
		//i32 m_xSize;																		// For later use - Texture x size
		//i32 m_ySize;																		// For later use - Texture y size
		//i32 m_numMips;																	// For later use - Number of mip maps
	...
	{
		i32    m_streamableContentMemoryUsage;            // Total streamable memory usage
		...
		/*
		    i32 m_lodNr;																			// For later use
		    i32 m_vertices;
		    i32 m_meshSize;
		    i32 m_physProxySize;
		    i32 m_physPrimitives;
		    i32 m_indices;
		    i32 m_indicesPerLod[MAX_LODS];
		 */
	...
	struct SSector
	{
		...
		i32  m_memoryUsage_Textures;                      // Sum of texture memory usage

		i32  m_memoryUsage_Geometry;                      // Sum of geometry memory usage
	};
	...
	i32          sys_LocalMemoryTextureLimit;                // Config: texture limit for streaming
	i32          sys_LocalMemoryGeometryLimit;               // Config: stat object geometry limit for streaming
	i32          sys_LocalMemoryTextureStreamingSpeedLimit;  // Config: texture streaming speed limit (approx)
	i32          sys_LocalMemoryGeometryStreamingSpeedLimit; // Config: stat object geometry streaming speed limit (approx)
	...
	i32          sys_LocalMemoryObjectAlpha;            // Config: Color alpha of the debug object
	...
	i32          sys_LocalMemoryDiagramAlpha;                  // Config: Color alpha of the diagram

	i32          sys_LocalMemoryDrawText;               // Config: If != 0, it will draw the numeric values
	i32          sys_LocalMemoryLogText;                // Config: If != 0, it will log the numeric values

	i32          sys_LocalMemoryOptimalMSecPerSec;      // Config: Optimal calculation time (MSec) per secundum
	i32          sys_LocalMemoryMaxMSecBetweenCalls;    // Config: Maximal time difference (MSec) between calls
...
private:
	...
	void CheckCharacterP1(ICharacterInstance* pCharacter, IRenderNode* pRenderNode, IMaterial* pSlotMat, AABB bounding, float maxViewDist, float scale, i32 nMaxDepth);
