...
struct HmdTrackingState
{
	...
	bool CheckStatusFlags(u32 checked, u32 wanted) const { return (statusFlags & checked) == wanted; }
	bool CheckStatusFlags(u32 checkedAndWanted) const             { return CheckStatusFlags(checkedAndWanted, checkedAndWanted); }
...
struct IHmdController
{
	...
	//! \return Tracking state in the Hmd's local tracking space using DRXENGINE's coordinate system.
	...
	virtual u32 GetCaps(EHmdController id) const = 0;

	//! \returns the hardware device handle(if applicable) of the selected controller
	virtual uk GetDeviceHandle(EHmdController id) const { return nullptr; };
...
public:
	...
	virtual HMDCameraSetup GetHMDCameraSetup(i32 nEye, float projRatio, float fnear) const = 0;
	...
	virtual i32                     GetControllerCount() const = 0;
	...
	virtual void GetPreferredRenderResolution(u32& width, u32& height) = 0;
