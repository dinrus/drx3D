...
struct CDREnd
{
	...
	u32 lSignature;         // end of central dir signature    4 bytes  (0x06054b50)
	u16 nDisk;              // number of this disk             2 bytes
	u16 nCDRStartDisk;      // number of the disk with the start of the central directory  2 bytes
	u16 numEntriesOnDisk;   // total number of entries in the central directory on this disk  2 bytes
	u16 numEntriesTotal;    // total number of entries in the central directory           2 bytes
	u32 lCDRSize;           // size of the central directory   4 bytes
	u32 lCDROffset;         // offset of start of central directory with respect to the starting disk number        4 bytes
	u16 nCommentLength;     // .ZIP file comment length        2 bytes
...
//Header for HEADERS_ENCRYPTED_DRXCUSTOM technique. Paired with a DrxSignedCDRHeader to allow for signing as well as encryption.
//i.e. the comment section for a file that uses this technique needs the following in order:
//DrxCustomExtendedHeader, DrxSignedCDRHeader, DrxCustomEncryptionHeader
struct DrxCustomEncryptionHeader
{
	// Note: Not initialized for performance reasons
	// cppcheck-suppress uninitMemberVar
	DrxCustomEncryptionHeader() {}

	u32        nHeaderSize;                                               // Size of the extended header.
	u8 CDR_IV[RSA_KEY_MESSAGE_LENGTH];                            //Initial Vector is actually BLOCK_CIPHER_KEY_LENGTH bytes in length, but is encrypted as a RSA_KEY_MESSAGE_LENGTH byte message.
	u8 keys_table[BLOCK_CIPHER_NUM_KEYS][RSA_KEY_MESSAGE_LENGTH]; //As above, actually BLOCK_CIPHER_KEY_LENGTH but encrypted.
} PACK_GCC;
...
//DrxCustomExtendedHeader, DrxSignedCDRHeader
struct DrxSignedCDRHeader
{
	// Note: Not initialized for performance reasons
	// cppcheck-suppress uninitMemberVar
	DrxSignedCDRHeader() {}
	...
	u32        nHeaderSize; // Size of the extended header.
	u8 CDR_signed[RSA_KEY_MESSAGE_LENGTH];
} PACK_GCC;

//Stores type of encryption and signing
struct DrxCustomExtendedHeader
{
	u32 nHeaderSize; // Size of the extended header.
	u16 nEncryption; // Matches one of EHeaderEncryptionType: 0 = No encryption/extension
	u16 nSigning;    // Matches one of EHeaderSignatureType: 0 = No signing

	DrxCustomExtendedHeader() : nHeaderSize(0), nEncryption(0) {}
} PACK_GCC;
...
struct CDRFileHeader
{
	...
	u32         lSignature;     // central file header signature   4 bytes  (0x02014b50)
	u16         nVersionMadeBy; // version made by                 2 bytes
	u16         nVersionNeeded; // version needed to extract       2 bytes
	u16         nFlags;         // general purpose bit flag        2 bytes
	u16         nMethod;        // compression method              2 bytes
	u16         nLastModTime;   // last mod file time              2 bytes
	u16         nLastModDate;   // last mod file date              2 bytes
	DataDescriptor desc;
	u16         nFileNameLength;    // file name length                2 bytes
	u16         nExtraFieldLength;  // extra field length              2 bytes
	u16         nFileCommentLength; // file comment length             2 bytes
	u16         nDiskNumberStart;   // disk number start               2 bytes
	u16         nAttrInternal;      // internal file attributes        2 bytes
	u32         lAttrExternal;      // external file attributes        4 bytes
	...
	u32 lLocalHeaderOffset;    // relative offset of local header 4 bytes
...
struct LocalFileHeader
{
	...
	u32         lSignature;     // local file header signature     4 bytes  (0x04034b50)
	u16         nVersionNeeded; // version needed to extract       2 bytes
	u16         nFlags;         // general purpose bit flag        2 bytes
	u16         nMethod;        // compression method              2 bytes
	u16         nLastModTime;   // last mod file time              2 bytes
