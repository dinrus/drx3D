...
public:
	...
	void DumpMemoryCoverage()
	{
		...
		uk * pMemoryBlocks = 0;                           // linked list of memory blocks (to free them)

		size_t nUnits = 0;
		u32 dwAllocCnt = 0, dwFreeCnt = 0;
		...
		while (nCurrentUnitSize >= nMinMemoryPerUnit)
		{
			...
			for (;; )
			{
				uk * pMem = (uk *)::malloc(nCurrentUnitSize - nMallocOverhead);
			...
			//			drx_sprintf(str,"CMemoryFragmentationProfiler nCurrentUnitSize= %7d KB unit size => %6d more units allocated, resulting in %5d MB\n",
		...
		while (pMemoryBlocks)
		{
			uk pNext = *pMemoryBlocks;

			::free(pMemoryBlocks);
			++dwFreeCnt;

			pMemoryBlocks = (uk *)pNext;
		}

		//		_heapmin();

		DrxLog("CMemoryFragmentationProfiler  Y=%d, available memory=%d MB, used memory=%d MB",
...
private: // ------------------------------------------------------------------
	...
	void Init()
	{
		...
		if (out)
		{
			...
			for (i32 y = 0; y < m_nLineCount; y++)                   // amount of lines
				for (i32 x = 0; x < m_nPixelsPerLine; x++)
				{
					...
					if (nAddress == 0x30000000
					    ...
					    || nAddress == 0x38000000
					    || nAddress == 0x39000000)
						putc((u8)100, out);   // blue   DLL start
					else
						putc((u8)0, out);     // black
	...
	u32        m_dwLine;                               // [0..m_nLineCount-1], m_nLineCount means bitmap is full, 0xffffffff means not initialized yet
