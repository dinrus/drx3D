...
public:
	GMemorySTLAlloc(GMemoryHeap* pHeap = GMemory::GetGlobalHeap(), u32 SID = GStat_Default_Mem) throw()
		: m_pHeap(pHeap)
		, m_SID(SID)
	{
	...
	pointer allocate(size_type n, ukk pHint = 0)
	{
		T* p = (T*) GHEAP_ALLOC(m_pHeap, n * sizeof(T), m_SID);
		return p;
	}

	void deallocate(uk p, size_type n)
	{
		if (p)
