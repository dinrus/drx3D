...
enum EVarFlags : u32
{
	...
	VF_COPYNAME                = 0x00008000,      //!< Otherwise the tukk  to the name will be stored without copying the memory.
...
struct IKeyBindDumpSink
{
	...
	virtual void OnKeyBindFound(tukk sBind, tukk sCommand) = 0;
	// </interfuscator:shuffle>
};
...
struct IOutputPrintSink
{
	...
	virtual void Print(tukk inszText) = 0;
	// </interfuscator:shuffle>
};
...
struct IConsoleVarSink
{
	...
	virtual bool OnBeforeVarChange(ICVar* pVar, tukk sNewValue) = 0;
...
struct IConsoleCmdArgs
{
	...
	virtual tukk GetArg(i32 nIndex) const = 0;

	//! Gets complete command line.
	virtual tukk GetCommandLine() const = 0;
	// </interfuscator:shuffle>
};
...
struct IConsoleArgumentAutoComplete
{
	...
	virtual tukk GetValue(i32 nIndex) const = 0;
	// </interfuscator:shuffle>
};
...
struct IManagedConsoleCommandListener
{
	...
	virtual void OnManagedConsoleCommandEvent(tukk commandName, IConsoleCmdArgs* consoleCommandArguments) = 0;
};
...
struct IConsole
{
	...
	//! \see ICVar.
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleVariable.cpp
	virtual void UnregisterVariable(tukk sVarName, bool bDelete = false) = 0;

	//! Set the y coordinate where the console will stop to scroll when is dropped.
	//! \param value Y in screen coordinates.
	virtual void SetScrollMax(i32 value) = 0;
	...
	virtual void ShowConsole(bool show, i32k iRequestScrollMax = -1) = 0;
	...
	virtual void DumpCVars(ICVarDumpSink* pCallback, u32 nFlagsFilter = 0) = 0;
	...
	virtual void CreateKeyBind(tukk sCmd, tukk sRes) = 0;
	...
	//! \param pImage Background image.
	virtual void SetLoadingImage(tukk szFilename) = 0;
	...
	virtual bool GetLineNo(i32k indwLineNo, tuk outszBuffer, i32k indwBufferSize) const = 0;

	//! \return Current number of lines in the console.
	virtual i32 GetLineCount() const = 0;
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleVariable.cpp
	virtual ICVar* GetCVar(tukk name) = 0;
	...
	virtual tuk GetVariable(tukk szVarName, tukk szFileName, tukk def_val) = 0;
	...
	//! \param def_val default value (if the variable is not found into the file).
	//! \return the variable value.
	virtual float GetVariable(tukk szVarName, tukk szFileName, float def_val) = 0;

	//! Print a string in the console and go to the new line.
	//! \param s The string to print.
	virtual void PrintLine(tukk s) = 0;

	//! Append a string in the last console line.
	//! \param s The string to print.
	virtual void PrintLinePlus(tukk s) = 0;
	...
	virtual void RegisterListener(IManagedConsoleCommandListener* pListener, tukk name) = 0;
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleCommand.cpp
	virtual void RemoveCommand(tukk sName) = 0;
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleCommand.cpp
	virtual void ExecuteString(tukk command, const bool bSilentMode = false, const bool bDeferExecution = false) = 0;

	//! Print a message into the log and abort the execution of the application.
	//! \param message error string to print in the log.
	virtual void Exit(tukk command, ...) PRINTF_PARAMS(2, 3) = 0;
	...
	virtual size_t      GetSortedVars(tukk* pszArray, size_t numItems, tukk szPrefix = 0, i32 nListTypes = 0) const = 0;
	virtual tukk AutoComplete(tukk substr) = 0;
	virtual tukk AutoCompletePrev(tukk substr) = 0;
	virtual tukk ProcessCompletion(tukk szInputBuffer) = 0;
	virtual void        RegisterAutoComplete(tukk sVarOrCommand, IConsoleArgumentAutoComplete* pArgAutoComplete) = 0;
	virtual void        UnRegisterAutoComplete(tukk sVarOrCommand) = 0;
	...
	virtual void ResetProgressBar(i32 nProgressRange) = 0;

	//! Function related to progress bar.
	virtual void TickProgressBar() = 0;

	//! \param szLine Must not be 0.
	virtual void SetInputLine(tukk szLine) = 0;
	...
	virtual tukk FindKeyBind(tukk sCmd) const = 0;
	...
	virtual tuk  GetCheatVarAt(u32 nOffset) = 0;
	...
	virtual tukk GetHistoryElement(const bool bUpOrDown) = 0;

	//! \param szCommand Must not be 0.
	virtual void                   AddCommandToHistory(tukk szCommand) = 0;
	...
	virtual void                   LoadConfigVar(tukk sVariable, tukk sValue) = 0;
	...
	virtual void                   LoadConfigCommand(tukk szCommand, tukk szArguments = nullptr) = 0;
	...
	virtual void SetClientDataProbeString(tukk pName, tukk pValue) = 0;
#endif
...
public:
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleCommand.cpp
	virtual void AddCommand(tukk sCommand, ConsoleCommandFunc func, i32 nFlags = 0, tukk sHelp = NULL, bool bIsManaged = false) = 0;
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleCommand.cpp
	virtual void AddCommand(tukk sName, tukk sScriptFunc, i32 nFlags = 0, tukk sHelp = NULL) = 0;
	...
	virtual ICVar* RegisterString(tukk sName, tukk sValue, i32 nFlags, tukk help = "", ConsoleVarFunc pChangeFunc = 0) = 0;

	//! Create a new console variable that store the value in a i32.
	//! \param sName Console variable name.
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleVariable.cpp
	virtual ICVar* RegisterInt(tukk sName, i32 iValue, i32 nFlags, tukk help = "", ConsoleVarFunc pChangeFunc = 0) = 0;
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleVariable.cpp
	virtual ICVar* RegisterInt64(tukk sName, int64 iValue, i32 nFlags, tukk help = "", ConsoleVarFunc pChangeFunc = 0) = 0;
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleVariable.cpp
	virtual ICVar* RegisterFloat(tukk sName, float fValue, i32 nFlags, tukk help = "", ConsoleVarFunc pChangeFunc = 0) = 0;
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleVariable.cpp
	virtual ICVar* Register(tukk name, float* src, float defaultvalue, i32 nFlags = 0, tukk help = "", ConsoleVarFunc pChangeFunc = 0, bool allowModify = true) = 0;
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleVariable.cpp
	virtual ICVar* Register(tukk name, i32* src, i32 defaultvalue, i32 nFlags = 0, tukk help = "", ConsoleVarFunc pChangeFunc = 0, bool allowModify = true) = 0;
	...
	//! \par Example
	//! \include DinrusXSys/Examples/ConsoleVariable.cpp
	virtual ICVar* Register(tukk name, tukk* src, tukk defaultvalue, i32 nFlags = 0, tukk help = "", ConsoleVarFunc pChangeFunc = 0, bool allowModify = true) = 0;
...
struct IRemoteConsole
{
	...
	virtual void AddLogMessage(tukk log) = 0;
	virtual void AddLogWarning(tukk log) = 0;
	virtual void AddLogError(tukk log) = 0;

	virtual void Update() = 0;

	virtual void RegisterListener(IRemoteConsoleListener* pListener, tukk name) = 0;
...
struct ICVar
{
	...
	virtual void Set(i32k i) = 0;

	//! Clear the specified bits in the flag field.
	virtual void ClearFlags(i32 flags) = 0;
	...
	virtual i32 SetFlags(i32 flags) = 0;
	...
	virtual i32 GetRealIVal() const = 0;
	...
	virtual void SetDataProbeString(tukk pDataProbeString) = 0;
#endif
};
