...
/*************************************************************************
   -------------------------------------------------------------------------
   Описание:  Реализация дистанционной командной системы (сервер)
   -------------------------------------------------------------------------
   История:
...
CRemoteCommandServer::WrappedCommand::WrappedCommand(IRemoteCommand* pCommand, u32k commandId)
	: m_pCommand(pCommand)
	, m_refCount(1)
...
CRemoteCommandServer::Endpoint::Endpoint(CRemoteCommandUpr* pUpr, class CRemoteCommandServer* pServer, IServiceNetworkConnection* pConnection)
...
tukk CRemoteCommandServer::Endpoint::GetClassName(u32k classId) const
{
...
IRemoteCommand* CRemoteCommandServer::Endpoint::CreateObject(u32k classId) const
{
	// class index is out of bounds
	if (classId >= m_pLocalClassFactories.size())
	{
...
void CRemoteCommandServer::Endpoint::Execute()
{
	...
	while (!m_pCommandsToExecute.empty())
	{
		...
		DRX_ASSERT(pCommand->GetId() > idOfLastExecutedCommand);
	...
	if (idOfLastExecutedCommand != 0)
	{
		...
		DRX_ASSERT(idOfLastExecutedCommand > m_lastExecutedCommand);
...
bool CRemoteCommandServer::Endpoint::Update()
{
			...
			case PackedHeader::eCommand_ClassList:
				{
					...
					u32k firstCommandID = packetHeader.count;
					...
					GetUpr()->BuildClassMapping(classNames, m_pLocalClassFactories);
					break;
				}
			...
			case PackedHeader::eCommand_Command:
				{
					...
					u32k numCommands = packetHeader.count;
					for (u32 i = 0; i < numCommands; ++i)
					{
						...
						u32k offset = reader.GetOffset();
						u32k endOffset = offset + header.size; // here is where we can skip
						...
						u32k expectedNextCommand = m_lastReceivedCommand + 1;
						...
						else
						{
							...
							DRX_ASSERT(header.commandId == expectedNextCommand);
						...
						DRX_ASSERT(reader.GetOffset() <= endOffset);
	...
	u32k snapshotLastExecutedCommand = m_lastExecutedCommand;
...
CRemoteCommandServer::CRemoteCommandServer(CRemoteCommandUpr* pUpr, IServiceNetworkListener* pListener)
	: m_pUpr(pUpr)
	...
	, m_bIsSuppressed(false)
{
	...
	if (!gEnv->pThreadUpr->SpawnThread(this, "RemoteCommandServer"))
	{
		DrxFatalError("Error spawning \"RemoteCommandServer\" thread.");
	}
}
...
void CRemoteCommandServer::ProcessRawMessageAsync(IServiceNetworkMessage* pMessage, IServiceNetworkConnection* pConnection)
{
	...
	DrxAutoLock<DrxMutex> lock(m_rawMessagesLock);
...
{
	if (DrxInterlockedIncrement(&m_suppressionCounter) > 0)
	{
...
{
	if (DrxInterlockedDecrement(&m_suppressionCounter) == 0)
	{
		m_bIsSuppressed = false;
	}
}
	...
	while (!m_bCloseThread)
	{
		// Accept new connections
		{
			...
			if (NULL != pNewConnection)
			{
				...
				Endpoint* pEndPoint = new Endpoint(GetUpr(), this, pNewConnection);
