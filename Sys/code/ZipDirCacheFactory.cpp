...
#include <drx3D/Sys/ZipDirCacheFactory.h>
...
ZipDir::CachePtr ZipDir::CacheFactory::New(tukk szFile, IDrxPak::EInMemoryPakLocation eInMemLocal)
{
	m_szFilename = szFile;
	...
	if (eInMemLocal != IDrxPak::eInMemoryPakLocale_Unload)
	{
		if (m_fileExt.m_file)
		{
	...
	if (m_fileExt.m_file)
	{
		...
		if (!m_fileExt.OpenUnbuffered(szFile))
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "Failed to open '%s' for unbuffered IO.", szFile);
#endif
...
ZipDir::CachePtr ZipDir::CacheFactory::New(tukk szFile, IDrxPak::EInMemoryPakLocation eInMemLocale, FILE* pPakFileRef, i32 nAssetOffset, i32 nAssetLength, ZipDir::FileEntry* pPakEntry)
{
	m_szFilename = szFile;
	    ...
	    pPakEntry->desc.lSizeUncompressed == pPakEntry->desc.lSizeCompressed)
	{
		...
		if (eInMemLocale != IDrxPak::eInMemoryPakLocale_Unload)
		{
...
ZipDir::CachePtr ZipDir::CacheFactory::New(tukk szFile, IDrxPak::EInMemoryPakLocation eInMemLocale, AAssetUpr* pAssetUpr)
{
	m_szFilename = szFile;

	Clear();

	AAsset* pAsset = AAssetUpr_open(pAssetUpr, szFile, AASSET_MODE_RANDOM);
	i32 fd = -1;
	...
	if (pAsset)
	{
		off_t offset = 0;
		off_t length = 0;
		i32 fd = AAsset_openFileDescriptor(pAsset, &offset, &length);
		AAsset_close(pAsset);
		...
		if (fd >= 0)
		{
			...
			m_fileExt.m_nAssetOffset = (i32) offset;
			m_fileExt.m_nAssetLength = (i32) length;
...
ZipDir::CachePtr ZipDir::CacheFactory::New(tukk szFile, IMemoryBlock* pData)
{
	m_szFilename = szFile;
...
ZipDir::CacheRWPtr ZipDir::CacheFactory::NewRW(tukk szFileName)
{
...
bool ZipDir::CacheFactory::Prepare()
{
	...
	//Pak may be encrypted with DrxCustom technique and/or signed. Being signed is compatible (in principle) with the earlier encryption methods.
	...
	if (m_CDREnd.nCommentLength >= sizeof(m_headerExtended))
	{
		...
		u16 expectedCommentLength = sizeof(m_headerExtended);
		...
		else
		{
			...
			case HEADERS_ENCRYPTED_STREAMCIPHER_KEYTABLE:
				expectedCommentLength += sizeof(ZipFile::DrxCustomEncryptionHeader);
				break;
			default:
		...
		case HEADERS_CDR_SIGNED:
			expectedCommentLength += sizeof(ZipFile::DrxSignedCDRHeader);
			break;
		default:
			// Unexpected technique
...
inline uint64 GenPakFileOffsetOnMedia(tukk szFile)
{
...
ZipDir::CachePtr ZipDir::CacheFactory::MakeCache(tukk szFile)
{
	if (!Prepare())
		return NULL;
	...
	if (m_bBuildOptimizedFileEntry)
	{
		if (m_optimizedFileEntries.size() > 0)
		{
			...
			for (i32 i = 0, num = m_optimizedFileEntries.size(); i < num - 1; i++)
			{
				if (m_optimizedFileEntries[i].nNameOffset == m_optimizedFileEntries[i + 1].nNameOffset)
				{
					// Duplicate CRC found, Fatal Error.
					DrxFatalError("Duplicate CRC32 of filenames in pak file %s", m_szFilename.c_str());
				}
			}
#endif //#ifndef OPTIMIZED_READONLY_ZIP_ENTRY
	...
	tuk pZipPath = ((tuk)(pCacheInstance + 1)) + nSizeRequired;
...
bool ZipDir::CacheFactory::FindCDREnd()
{
	...
	u32 nOldBufPos = (u32)nFileSize;
	// start scanning well before the end of the file to avoid reading beyond the end

	u32 nScanPos = nOldBufPos - sizeof(CDREnd);

	m_CDREnd.lSignature = 0; // invalid signature as the flag of not-found CDR End structure
	while (true)
	{
		u32 nNewBufPos;             // the new buf pos
		tuk pWindow = &pReservedBuffer[0]; // the window pointer into which data will be read (takes into account the possible tail-of-CDREnd)
		...
		if (nFileSize > (sizeof(CDREnd) + 0xFFFF))
		{
			...
			if (nNewBufPos < (u32)(nFileSize - sizeof(CDREnd) - 0xFFFF))
			{
				nNewBufPos = (u32)(nFileSize - sizeof(CDREnd) - 0xFFFF);
			}
		}
...
bool ZipDir::CacheFactory::BuildFileEntryMap()
{
	...
	tukk pEndOfData = &pBuffer[0] + m_CDREnd.lCDRSize, * pFileName;

	while ((pFileName = (tukk)(pFile + 1)) <= pEndOfData)
	{
		...
		tukk pEndOfRecord = (pFileName + pFile->nFileNameLength + pFile->nExtraFieldLength + pFile->nFileCommentLength);
		...
		tukk pExtraField = (pFileName + pFile->nFileNameLength);
		tukk pExtraEnd = pExtraField + pFile->nExtraFieldLength;
		while (pExtraField < pExtraEnd)
		{
			tukk pAttrData = pExtraField + sizeof(ExtraFieldHeader);
		...
		{
			// Add this file entry.
			tuk str = const_cast<tuk>(pFileName);
			for (i32 i = 0; i < pFile->nFileNameLength; i++)
			{
				str[i] = ::tolower(str[i]);
#if DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID || DRX_PLATFORM_APPLE  // On linux and Mac always forward slashes.
				if (str[i] == '\\')
					str[i] = '/';
#else
				if (str[i] == '/')
...
void ZipDir::CacheFactory::AddFileEntry(tuk strFilePath, const ZipFile::CDRFileHeader* pFileHeader, const SExtraZipFileData& extra)
{
	...
	if (m_bBuildOptimizedFileEntry)
	{
		u32 nHash = ZipDir::FileNameHash(strFilePath);
...
void ZipDir::CacheFactory::InitDataOffset(FileEntry& fileEntry, const ZipFile::CDRFileHeader* pFileHeader)
{
	...
	else
	{
		...
		if (memicmp((tukk)&pBuffer[sizeof(LocalFileHeader)], (tukk)pFileHeader + 1, pFileHeader->nFileNameLength))
		{
...
void ZipDir::CacheFactory::Validate(const FileEntry& fileEntry)
{
	...
	tuk pUncompressed = &pBuffer[fileEntry.desc.lSizeCompressed];
	tuk pCompressed = &pBuffer[0];
	...
	if (fileEntry.nMethod == METHOD_STORE_AND_STREAMCIPHER_KEYTABLE || fileEntry.nMethod == METHOD_DEFLATE_AND_STREAMCIPHER_KEYTABLE)
	{
		...
		i32 nKeyIndex = ZipEncrypt::GetEncryptionKeyIndex(&fileEntry);
		ZipEncrypt::GetEncryptionInitialVector(&fileEntry, IV);
		if (!ZipEncrypt::DecryptBufferWithStreamCipher((u8*)pUncompressed, fileEntry.desc.lSizeCompressed, m_block_cipher_keys_table[nKeyIndex], IV))
		{
			// Decryption failed
	...
	{
	#if !defined(_RELEASE)
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "Tried to verify a file from an archive using an unsupported method");
	#endif                                                            //!_RELEASE
		THROW_ZIPDIR_ERROR(ZD_ERROR_CORRUPTED_DATA, "Data is corrupt"); //Don't give a clue about the encryption support
	}

	u64 nDestSize = fileEntry.desc.lSizeUncompressed;
	i32 nError = Z_OK;
...
tuk ZipDir::CacheFactory::GetFilePath(tukk pFileName, u16 nFileNameLength)
{
	...
	for (i32 i = 0; i < nFileNameLength; i++)
	{
...
void ZipDir::CacheFactory::Seek(u32 nPos, i32 nOrigin)  // throw
{
...
bool ZipDir::CacheFactory::Read(uk pDest, unsigned nSize)  // throw
{
...
u64 ZipDir::CacheFactory::GetReferenceCRCForPak()
{
	u64 result = 0;
	...
	//
	//	for (size_t i=0; i<DRX_ARRAY_COUNT(refs); i++)
	//	{
...
bool ZipDir::CacheFactory::ReadHeaderData(uk pDest, unsigned nSize)  // throw
{
	if (!Read(pDest, nSize))
		return false;
	...
	case HEADERS_ENCRYPTED_STREAMCIPHER_KEYTABLE:
		// Decrypt CDR
		if (!ZipEncrypt::DecryptBufferWithStreamCipher((u8*)pDest, nSize, m_block_cipher_keys_table[0], m_block_cipher_cdr_initial_vector))
		{
			// Decryption of CDR failed.
	#if !defined(_RELEASE)
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "Failed to decrypt pak header");
	#endif //!_RELEASE
			return false;
		}
		break;
	...
	default:
#if !defined(_RELEASE)
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "Attempting to load encrypted pak by unsupported method, or unencrypted pak when support is disabled");
#endif
		return false;
	}
	...
	case HEADERS_CDR_SIGNED:
#ifdef SUPPORT_RSA_PAK_SIGNING
		{
			...
			tukk szPathSep = strrchr(m_szFilename.c_str(), '\\');
			tukk szPathSepFS = strrchr(m_szFilename.c_str(), '/');
			...
			u8k* dataToVerify[2];
			dataToVerify[0] = (u8k*)pDest;
			dataToVerify[1] = (u8k*)szPathSep;
			u32 sizesToVerify[2];
		...
		#if !defined(_RELEASE)
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "Failed to verify RSA signature of pak header");
		#endif //!_RELEASE
...
#else
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "[ZipDir] HEADERS_CDR_SIGNED not yet supported");
#endif
		break;
	...
	default:
#if !defined(_RELEASE)
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "Unsupported pak signature, or use of unsigned pak when support is disabled.");
#endif  //!_RELEASE
		return false;
		break;
	}
...
u64 ZipDir::GetStreamCipherKey(const FileEntry* inEntry)
{
	// take crc 32, salt it and mod with a large prime to chew it up in a non obvious way
	// do same with file header offset
	// xor them together
	u32 key = (((inEntry->desc.lCRC32 ^ 0x9b7c9df2) % 3302203733) ^ ((inEntry->nFileDataOffset ^ 0xce30acdf) % 30829));
	//DrxLog("got key %x from crc %x file data offset %x",key,inEntry->desc.lCRC32,inEntry->nFileDataOffset);
	return key;
}
#endif
...
bool ZipDir::CacheFactory::DecryptKeysTable()
{
	...
	for (i32 i = 0; i < BLOCK_CIPHER_NUM_KEYS; i++)
	{
		i32 stat = 0;
		len = sizeof(tmp);
