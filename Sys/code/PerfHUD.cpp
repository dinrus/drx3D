...
DRXREGISTER_SINGLETON_CLASS(CPerfHUD)
CPerfHUD::CPerfHUD() :
...
{
	m_widgets.reserve(IDrxPerfHUDWidget::eWidget_Num);
}
...
void CPerfHUD::Destroy()
{
	...
	IMiniGUIPtr pGUI;
	if (DrxCreateClassInstanceForInterface(drxiidof<IMiniGUI>(), pGUI))
	{
		pGUI->RemoveAllCtrl();
	}
}
...
i32 CPerfHUD::m_sys_perfhud = 0;
i32 CPerfHUD::m_sys_perfhud_pause = 0;
...
void CPerfHUD::LoadBudgets()
{
	...
	if (budgets)
	{
		i32k nWidgets = m_widgets.size();

		for (i32 i = 0; i < nWidgets; i++)
		{
...
//////////////////////////////////////////////////////////////////////////
void CPerfHUD::SaveStats(tukk filename)
{
...
void CPerfHUD::AddWidget(IDrxPerfHUDWidget* pWidget)
{
	assert(pWidget);
	static i32 s_widgetUID = IDrxPerfHUDWidget::eWidget_Num;
...
void CPerfHUD::RemoveWidget(IDrxPerfHUDWidget* pWidget)
{
	...
	case eHudInFocus:
		{
			...
			i32 nRootMenus = m_rootMenus.size();
			for (i32 i = 0; i < nRootMenus; i++)
			{
...
void CPerfHUD::InitUI(IMiniGUI* pGUI)
{
	...
	{
		gEnv->pParticleUpr->CreatePerfHUDWidget();
	}
	if (gEnv->pDrxPak)
	{
		gEnv->pDrxPak->CreatePerfHUDWidget();
	}
...
IMiniCtrl* CPerfHUD::CreateMenu(tukk name, IMiniCtrl* pParent)
{
	assert(name);
...
//////////////////////////////////////////////////////////////////////////
bool CPerfHUD::CreateCVarMenuItem(IMiniCtrl* pMenu, tukk name, tukk controlVar, float controlVarOff, float controlVarOn)
{
...
//////////////////////////////////////////////////////////////////////////
bool CPerfHUD::CreateCallbackMenuItem(IMiniCtrl* pMenu, tukk name, ClickCallback clickCallback, uk pCallbackData)
{
...
//////////////////////////////////////////////////////////////////////////
IMiniInfoBox* CPerfHUD::CreateInfoMenuItem(IMiniCtrl* pMenu, tukk name, RenderCallback renderCallback, const Rect& rect, bool onAtStart)
{
	assert(pMenu->GetType() == eCtrlType_Menu);
	...
	if (DrxCreateClassInstanceForInterface(drxiidof<IMiniGUI>(), pGUI))
	{
		...
		i32 infoFlags = eCtrl_Moveable | eCtrl_CloseButton;
...
//////////////////////////////////////////////////////////////////////////
IMiniTable* CPerfHUD::CreateTableMenuItem(IMiniCtrl* pMenu, tukk name)
{
	assert(pMenu && name);
	assert(pMenu->GetType() == eCtrlType_Menu);
...
IMiniCtrl* CPerfHUD::GetMenu(tukk name)
{
	i32 nRootMenus = m_rootMenus.size();
...
{
	//DrxLog( "Button Clicked = %d",cmd.nCtrlID );
}
...
bool CPerfHUD::OnInputEvent(const SInputEvent& rInputEvent)
{
	...
	{
		i32 frameNum = gEnv->pRenderer->GetFrameID(false);
		...
		else if (rInputEvent.state == eIS_Down)
		{
			...
			if (m_triggersDownStartTime > 0.f && gEnv->pTimer->GetAsyncCurTime() - m_triggersDownStartTime > activateTime)
			{
				...
				tukk hudStateStr = NULL;

				switch (m_hudState)
				{
				case eHudInFocus:
					hudStateStr = "DrxPerfHUD Edit Mode";
					break;

				case eHudOutOfFocus:
					hudStateStr = "DrxPerfHUD Game Mode";
					break;

				case eHudOff:
					hudStateStr = "DrxPerfHUD Off";
					break;

				default:
					hudStateStr = "DrxPerfHud unknown";
					break;
				}
...
void CPerfHUD::SetNextState()
{
	if (m_sys_perfhud != eHudOff)
	{
		...
		//DrxLogAlways("Setting new HUD state: %d", m_hudState);
...
void CPerfHUD::ResetCallback(uk data, bool status)
{
...
void CPerfHUD::ReloadBudgetsCallback(uk data, bool status)
{
	IDrxPerfHUDPtr pPerfHUD;
	if (DrxCreateClassInstanceForInterface(drxiidof<IDrxPerfHUD>(), pPerfHUD))
	{
...
void CPerfHUD::SaveStatsCallback(uk data, bool status)
{
	IDrxPerfHUDPtr pPerfHUD;
	if (DrxCreateClassInstanceForInterface(drxiidof<IDrxPerfHUD>(), pPerfHUD))
	{
...
void CPerfHUD::EnableWidget(IDrxPerfHUDWidget::EWidgetID id, i32 mode)
{
	i32k nWidgets = m_widgets.size();
...
void CPerfHUD::DisableWidget(IDrxPerfHUDWidget::EWidgetID id)
{
	i32k nWidgets = m_widgets.size();
...
//////////////////////////////////////////////////////////////////////////
void CPerfHUD::AddWarning(float duration, tukk fmt, va_list argList)
{
	if (m_hudState != eHudOff)
	{
		...
		for (i32 i = 0; i < nWidgets; i++)
		{
			if (m_widgets[i]->m_id == IDrxPerfHUDWidget::eWidget_Warnings)
			{
...
const std::vector<IDrxPerfHUD::PerfBucket>* CPerfHUD::GetFpsBuckets(float& totalTime) const
{
	i32k nWidgets = m_widgets.size();

	for (i32 i = 0; i < nWidgets; i++)
	{
		if (m_widgets[i]->m_id == IDrxPerfHUDWidget::eWidget_FpsBuckets)
		{
...
i32 CFpsWidget::m_cvarPerfHudFpsExclusive = 0;

CFpsWidget::CFpsWidget(IMiniCtrl* pParentMenu, IDrxPerfHUD* pPerfHud) : IDrxPerfHUDWidget(eWidget_FpsBuckets)
{
	m_fpsBucketSize = 5.f;
	m_fpsBudget = 30.f;
	m_dpBudget = 2500.f;
...
void CFpsWidget::LoadBudgets(XmlNodeRef PerfXML)
{
	if (PerfXML)
	{
		...
		if ((xmlNode = PerfXML->findChild("fpsBucketValues")))
		{
			...
			u32 nBuckets = xmlNode->getChildCount();
			...
			for (u32 i = 0; i < nBuckets; i++)
			{
				...
				IDrxPerfHUD::PerfBucket bucket(target);
		...
		else if ((xmlNode = PerfXML->findChild("fpsBucketMax")))
		{
			...
			for (u32 i = 0; i < NUM_FPS_BUCKETS_DEFAULT; i++)
			{
				IDrxPerfHUD::PerfBucket bucket(targetFPS);
		...
		else if ((xmlNode = PerfXML->findChild("drawPrimBucketMax")))
		{
			...
			for (u32 i = 0; i < NUM_FPS_BUCKETS_DEFAULT; i++)
			{
				IDrxPerfHUD::PerfBucket bucket(nDPs);
...
void CFpsWidget::UpdateBuckets(PerfBucketsStat& bucketStat, float frameTime, tukk name, float stat)
{
	u32k numBuckets = bucketStat.buckets.size();
	...
	if (bucketStat.totalTime > 0.f)
	{
		...
		drx_sprintf(entryBuffer, "%s: %.2f", name, stat);
		...
		if (m_cvarPerfHudFpsExclusive)
		{
			for (u32 i = 0; i < numBuckets; i++)
			{
				...
				if (i > 0)
				{
					...
					drx_sprintf(entryBuffer, "%.2f%%%% of time %.1f -> %.1f FPS", percentAtTarget, bucketStat.buckets[i].target, bucketStat.buckets[i - 1].target);
				}
				else
				{
					float percentAtTarget = 100.f * (bucketStat.buckets[i].timeAtTarget / bucketStat.totalTime);
					drx_sprintf(entryBuffer, "%.2f%%%% of time >= %.1f FPS", percentAtTarget, bucketStat.buckets[i].target);
				}
				m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_NORM, CPerfHUD::TEXT_SIZE_NORM);
			}
		}
		else //render inclusive stats
		{
			...
			{
				float percentAtTarget = 100.f * (bucketStat.buckets[i].timeAtTarget / bucketStat.totalTime);
				drx_sprintf(entryBuffer, "%.2f%%%% of time %s %.1f", percentAtTarget, LESS_THAN ? "<=" : ">=", bucketStat.buckets[i].target);
				m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_NORM, CPerfHUD::TEXT_SIZE_NORM);
			}
		}
	}
...
void CFpsWidget::Init()
{
	...
	for (u32 i = 0; i < BUCKET_TYPE_NUM; i++)
	{
...
void CFpsWidget::Reset()
{
	for (u32 i = 0; i < BUCKET_TYPE_NUM; i++)
	{
		...
		u32k nBuckets = stat.buckets.size();
...
void CFpsWidget::ResetCallback(uk data, bool status)
{
	assert(data);
...
void CFpsWidget::SaveStats(XmlNodeRef statsXML)
{
	if (statsXML)
	{
		...
		for (i32 i = 0; i < BUCKET_TYPE_NUM; i++)
		{
			...
			if (perfBucket.totalTime > 0.f)
			{
				if ((fpsNode = statsXML->newChild(perfBucketTypeStr[i])))
				{
					...
					u32k numBuckets = perfBucket.buckets.size();

					for (u32 j = 0; j < numBuckets; j++)
					{
...
const std::vector<IDrxPerfHUD::PerfBucket>* CFpsWidget::GetFpsBuckets(float& totalTime) const
{
...
CRenderStatsWidget::CRenderStatsWidget(IMiniCtrl* pParentMenu, IDrxPerfHUD* pPerfHud) : IDrxPerfHUDWidget(eWidget_RenderStats)
{
	m_pPerfHUD = pPerfHud;
	m_fpsBudget = 30.f;
	m_dpBudget = 2000;
	...
	if (buildFile.Open("./BuildName.txt", "rb", IDrxPak::FOPEN_ONDISK | IDrxPak::FOPEN_HINT_QUIET))
	{
...
void CRenderStatsWidget::Update()
{
	...
	//drx_sprintf(entryBuffer, "%s - %s - Build: %d", pRenderType, buildType, m_buildNum);
	//m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_NORM, CPerfHUD::TEXT_SIZE_NORM);
	...
	drx_sprintf(entryBuffer, "FPS: %.2f (%.2f)", m_runtimeData.fps, m_fpsBudget);
	...
	else
	{
		...
		DrxPerfHUDWarning(1.f, "FPS Too Low: %.2f", m_runtimeData.fps);
	...
	if (gpuTime > 0.f)
	{
		...
		drx_sprintf(entryBuffer, "GPU FPS: %.2f (%.2f)", gpuFPS, m_fpsBudget);
		...
		{
			m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
			DrxPerfHUDWarning(1.f, "GPU FPS Too Low: %.2f", gpuFPS);
		}
	}
	...
	if (bHDRModeEnabled)
	{
		...
		{
			DrxPerfHUDWarning(1.f, "HDR Disabled");
		}
	}
	...
	if (!gEnv->IsEditor())
	{
		...
		{
			//drx_sprintf(entryBuffer, "Render Thread Enabled");
			//m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_NORM, CPerfHUD::TEXT_SIZE_NORM);
			m_runtimeData.renderThreadEnabled = true;
		}
		else
		{
			drx_sprintf(entryBuffer, "Render Thread Disabled");
			m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
			m_runtimeData.renderThreadEnabled = false;
			DrxPerfHUDWarning(1.f, "Render Thread Disabled");
		}
	}
	...
	drx_sprintf(entryBuffer, "Draw Calls: %u (%u)", m_runtimeData.nDrawPrims, m_dpBudget);
	...
	{
		m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
		DrxPerfHUDWarning(1.f, "Too Many Draw Calls: %u", m_runtimeData.nDrawPrims);
	}

	drx_sprintf(entryBuffer, "Num Tris: %u (%u)", m_runtimeData.nPolys, m_polyBudget);
	...
	{
		m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
		DrxPerfHUDWarning(1.f, "Too Many Tris: %d", m_runtimeData.nPolys);
	}
	...
	drx_sprintf(entryBuffer, "Num Post Effects: %u (%u)", m_runtimeData.nPostEffects, m_postEffectBudget);
	...
	{
		m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
		DrxPerfHUDWarning(1.f, "Too Many Post Effects: %d", m_runtimeData.nPostEffects);
	}
	...
	u32k nDynamicLights = pLights->Count();

	if (nDynamicLights)
	{
		u32 nShadowCastingLights = 0;

		for (u32 i = 0; i < nDynamicLights; i++)
		{
		...
		drx_sprintf(entryBuffer, "Num Forward Lights: %u (~)", nDynamicLights);
		m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_NORM, CPerfHUD::TEXT_SIZE_NORM);

		if (nShadowCastingLights)
		{
			drx_sprintf(entryBuffer, "Num Forward Shadow Casting Lights: %u (%u)", nShadowCastingLights, m_shadowCastBudget);
			...
			{
				m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
				DrxPerfHUDWarning(1.f, "Too Many Fwd Shadow Casting Lights: %d", nShadowCastingLights);
			}
		}
	...
	// Particles
	//
	IParticleUpr* pParticleMgr = gEnv->p3DEngine->GetParticleUpr();
	...
	if (pParticleMgr)
	{
		...
		m_runtimeData.nParticles = (u32)particleCounts.particles.rendered;

		drx_sprintf(entryBuffer, "Num Particles Rendered: %d (%u)", (i32)particleCounts.particles.rendered, m_particlesBudget);
		...
		{
			m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
			DrxPerfHUDWarning(1.f, "Too Many Particles: %d", (i32)particleCounts.particles.rendered);
		}
	}
	...
	if (gEnv->p3DEngine)
	{
		...
		drx_sprintf(entryBuffer, "Mesh Required: %.2f (%dMB)", fMeshRequiredMB, objStats.nMeshPoolSize);
		...
		{
			m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
			DrxPerfHUDWarning(1.f, "Too Many Geometry: %.2fMB", fMeshRequiredMB);
		}
	}
	...
	if (gEnv->pRenderer)
	{
		...
		drx_sprintf(entryBuffer, "Textures Required: %.2f (%" PRISIZE_T "MB)", fTexRequiredMB, textureStats.nMaxPoolSize / (1024 * 1024));
		...
		{
			m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
			DrxPerfHUDWarning(1.f, "Too Many Textures: %.2fMB", fTexRequiredMB);
		}
	}
}
...
CStreamingStatsWidget::CStreamingStatsWidget(IMiniCtrl* pParentMenu, IDrxPerfHUD* pPerfHud) : IDrxPerfHUDWidget(eWidget_StreamingStats)
{
	m_pPerfHUD = pPerfHud;
...
CWarningsWidget::CWarningsWidget(IMiniCtrl* pParentMenu, IDrxPerfHUD* pPerfHud) : IDrxPerfHUDWidget(eWidget_Warnings)
{
...
void CWarningsWidget::Update()
{
	...
	assert(DrxGetCurrentThreadId() == m_nMainThreadId);
	...
	i32 nWarnings = m_warnings.size();
	m_pInfoBox->ClearEntries();

	//display warnings
	for (i32 i = 0; i < nWarnings; i++)
	{
...
void CWarningsWidget::SaveStats(XmlNodeRef statsXML)
{
	if (statsXML)
	{
		i32k nWarnings = m_warnings.size();
...
void CWarningsWidget::AddWarningV(float duration, tukk fmt, va_list argList)
{
	char warningText[WARNING_LENGTH];

	drx_vsprintf(warningText, fmt, argList);

	AddWarning(duration, warningText);
}

void CWarningsWidget::AddWarning(float duration, tukk warning)
{
	if (DrxGetCurrentThreadId() == m_nMainThreadId)
	{
		...
		for (size_t i = 0; i < nWarnings; i++)
		{
			if (strncmp(m_warnings[i].text, warning, nCompareLen) == 0)
			{
				...
				drx_strcpy(m_warnings[i].text, warning);
				bNewWarning = false;
				break;
			}
		}
		...
		if (bNewWarning)
		{
			...
			drx_strcpy(newWarning.text, warning);
...
CRenderBatchWidget::CRenderBatchWidget(IMiniCtrl* pParentMenu, IDrxPerfHUD* pPerfHud) : IDrxPerfHUDWidget(eWidget_RenderBatchStats)
{
...
void CRenderBatchWidget::Enable(i32 mode)
{
	...
	if (m_displayMode != newMode)
	{
		...
		default:
			DrxLogAlways("[Render Batch Stats] Attempting to set incorrect display mode set: %d", mode);
			break;
		}
	}
...
void CRenderBatchWidget::Update()
{
	...
	case DISPLAY_MODE_GPU_TIMES:
		...
		break;
	default:
		DrxLogAlways("[Render Batch Stats]Incorrect Display mode set: %d", m_displayMode);
		break;
	}
}
...
void CRenderBatchWidget::Update_ModeBatchStats()
{
	...
	//mesh totals
	static tukk strMeshTotals = "TOTAL (Mesh)";
	BatchInfoPerPass totalsMesh;
	totalsMesh.col.set(255, 255, 255, 255);
	totalsMesh.name = strMeshTotals;

	i32 nDPs = gEnv->pRenderer->GetCurrentNumberOfDrawCalls();

	//scene totals
	static tukk strSceneTotals = "TOTAL (Scene)";
	...
	for (; pItor != pEnd; ++pItor)
	{
		...
		u32 nDrawcalls = drawInfo.nShadows + drawInfo.nZpass + drawInfo.nGeneral + drawInfo.nTransparent + drawInfo.nMisc;

		tukk pRenderNodeName = drawInfo.meshName;
		tukk pNameShort = strrchr(pRenderNodeName, '/');
	...
	if (gEnv->pConsole->GetCVar("e_Particles")->GetIVal() != 0)
	{
		...
		gEnv->pParticleUpr->GetCounts(CurCounts);

		static tukk s_strParticles = "PARTICLES";

		static BatchInfoPerPass s_particlesBatch;

		s_particlesBatch.Reset();
		s_particlesBatch.name = s_strParticles;
		s_particlesBatch.nBatches = (u16)CurCounts.emitters.rendered;
	...
	if (unknownDPs != 0)
	{
		static tukk s_strUnknown = "Unknown";
	...
	if (i32 nBatches = sortedBatchList.size())
	{
		...
		for (i32 i = 0; i < nBatches; i++)
		{
			...
			i32 nInstances = max(1, (i32)batch->nInstances);
	...
	#endif//RELEASE (DO_RENDERSTATS - not available in DinrusSystem)
}
