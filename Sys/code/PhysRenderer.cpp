...
void CPhysRenderer::Init()
{
	...
	aray.dir.Set(0, 0, 1);
	aray.origin.zero();
	m_pRayGeom = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::ray::type, &aray);
...
void CPhysRenderer::DrawGeometry(IGeometry* pGeom, geom_world_data* pgwd, i32 idxColor, i32 bSlowFadein, const Vec3& sweepDir, const ColorF& color)
{
...
void CPhysRenderer::DrawFrame(const Vec3& pnt, const Vec3* axes, const float scale, const Vec3* limits, i32k axes_locked)
{
...
void CPhysRenderer::DrawLine(const Vec3& pt0, const Vec3& pt1, i32 idxColor, i32 bSlowFadein)
{
	m_pRay->origin = pt0;
	m_pRay->dir = pt1 - pt0;
	i32 bPeaked = isneg(max(1e-6f - m_rayPeakTime, m_rayPeakTime - gEnv->pTimer->TicksToSeconds(bSlowFadein >> 1) * 1000));
...
void CPhysRenderer::DrawEntityHelpers(IPhysicalWorld* pWorld, i32 entityId, i32 iDrawHelpers)
{
	pWorld->DrawEntityHelperInformation(this, entityId, iDrawHelpers);
}

void CPhysRenderer::DrawEntityHelpers(IPhysicalEntity* pEntity, i32 helperFlags)
{
	IPhysicalWorld* pWorld = pEntity->GetWorld();
	i32 entityId = pWorld->GetPhysicalEntityId(pEntity);
	pWorld->DrawEntityHelperInformation(this, entityId, helperFlags);
}

inline float getheight(primitives::heightfield* phf, i32 ix, i32 iy)
{
...
void CPhysRenderer::DrawGeometry(IGeometry* pGeom, geom_world_data* pgwd, const ColorB& clr, const Vec3& sweepDir)
{
	...
	if (fAlpha < 0.99) clrNew.set(clr.r, clr.g, clr.b, (u8)(fAlpha * (float)clr.a));
...
void CPhysRenderer::DrawGeometry(i32 itype, ukk pGeomData, geom_world_data* pgwd, const ColorB& clr0, const Vec3& sweepDir)
{
	Matrix33 R = Matrix33::CreateIdentity();
	...
	case GEOM_CYLINDER:
		{
			...
			{
				assert(pgwd);
				i32 sg0, sg1, sgax = isneg(axes[2] * sweepDir);
				pt[0] = pt[2] = center + axes[0] * (pcyl->r * scale);
