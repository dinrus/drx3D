...
////////////////////////////////////////////////////////////////////////////
void CStroboscope::StartProfiling(float deltaStart /* = 0*/, float duration /* = -1*/, i32 throttle /* = 100*/, const SThreadInfo::TThreadIds threadIds /* = SThreadInfo::TThreadIds()*/)
{
	if (DrxInterlockedCompareExchange(&m_started, 1, 0) == 0)
	{
		...
		m_sampling.Clear();
		m_run = true;
		if (!gEnv->pThreadUpr->SpawnThread(this, "Stroboscope"))
		{
			DrxFatalError("Error spawning \"Stroboscope\" thread.");
		}
	}
}
...
void CStroboscope::ThreadEntry()
{
	...
	DrxInterlockedCompareExchange(&m_started, 0, 1);
}
...
void CStroboscope::ProfileThreads()
{
	...
	curr = prev = start = DrxGetTicks();

	i32 frameId = -1;
	m_sampling.StartFrame = -1;
	while (m_run)
	{
		DrxSleep(100 / m_throttle);
...
////////////////////////////////////////////////////////////////////////////
bool CStroboscope::SampleThreads(const SThreadInfo::TThreads& threads, float delta, i32 frameId)
{
	// sample threads
	i32 activeThreads = 0;
	for (i32 i = 0, end = threads.size(); i < end; ++i)
	{
...
////////////////////////////////////////////////////////////////////////////
bool CStroboscope::SampleThread(const SThreadInfo::SThreadHandle& thread, float delta, i32 frameId)
{
	#if !DRX_PLATFORM_LINUX && !DRX_PLATFORM_ANDROID && !DRX_PLATFORM_APPLE && !DRX_PLATFORM_ORBIS// No SuspendThread / ResumeThread with pthreads natively
	...
	if (m_result.Valid)
	{
		...
		for (SStrobosopeSamplingData::TThreadResult::const_iterator thrd = m_sampling.Threads.begin(); thrd != m_sampling.Threads.end(); ++thrd)
		{
			...
			for (SStrobosopeSamplingData::TFrameCallstacks::const_iterator it = threadRes.Callstacks.begin(), end = threadRes.Callstacks.end(); it != end; ++it)
			{
				...
				{
					for (i32 i = 0; i < callIt->first.Depth; ++i)
						addresses.insert(callIt->first.Stack[i]);
				}
			}

			typedef std::map<uk , i32> TSymbolTable;
...
struct SSleepyResultFile
{
	SSleepyResultFile()
	{
		pArchive = gEnv->pDrxPak->OpenArchive("stroboscope.sleepy", IDrxArchive::FLAGS_RELATIVE_PATHS_ONLY | IDrxArchive::FLAGS_CREATE_NEW);
	}
	...
	~SSleepyResultFile()
	{
		if (IsValid())
		{
			for (std::map<string, string>::iterator it = files.begin(); it != files.end(); ++it)
				pArchive->UpdateFile(it->first.c_str(), (uk )it->second.c_str(), it->second.length());
		}
	}

	bool IsValid() const { return pArchive.get() != NULL; }
	void WriteLineToFile(const string& file, tukk format, ...)
	{
		va_list args;
...
private:
	_smart_ptr<IDrxArchive>  pArchive;
...
void CStroboscope::DumpOutputResult()
{
	...
	if (res.Valid)
	{
		...
		{
			string out;
			drx_sprintf(tmp, "%d %u %.6f", it->FrameId, it->ThreadId, it->Spend);
			out = tmp;
		...
		// Version info
		file.WriteLineToFile("Version 0.84.DRX required", "0.84.DRX");
	}
	else
	{
...
void CStroboscope::StartProfilerCmd(IConsoleCmdArgs* pArgs)
{
	...
	{
		tukk arg = pArgs->GetArg(2);
		startDelay = (float)atof(arg);
	}

	if (pArgs->GetArgCount() > 3)
	{
		tukk arg = pArgs->GetArg(3);
		throttle = atoi(arg);
	}

	for (i32 i = 4, end = pArgs->GetArgCount(); i < end; ++i)
	{
		tuk stopAt;
		tukk arg = pArgs->GetArg(i);
