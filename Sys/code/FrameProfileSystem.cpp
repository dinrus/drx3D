...
// -------------------------------------------------------------------------
//  Имя файла:   FrameProfileSystem.cpp
//  Version:     v1.00
...
DrxCriticalSection CFrameProfileSystem::m_staticProfilersLock;
	...
	, m_nLastPageFaultCount(0)
{
	...
	for (i32 i = 0; i < DRX_ARRAY_COUNT(m_subsystems); i++)
	{
	...
	#if defined(JOBMANAGER_SUPPORT_FRAMEPROFILER)
	m_ThreadFrameStats = new JobUpr::CWorkerFrameStats(0);
	m_BlockingFrameStats = new JobUpr::CWorkerFrameStats(0);
	#endif
}
...
void CFrameProfileSystem::Init()
{
	...
	REGISTER_INT_CB("profile", 0, 0, 
		...
		[](ICVar* pEnable)
		{
			...
			i32 profValue = pEnable->GetIVal();
			bool bEnable  = profValue != 0;
			bool bDisplay = profValue > 0;
			i32 dispNum   = abs(profValue);
...
//////////////////////////////////////////////////////////////////////////
void CFrameProfileSystem::SetProfiling(bool on, bool display, tuk prefix, ISystem* pSystem)
{
	Enable(on, display);
	m_pPrefix = prefix;
	...
	{
		m_nCurSample = 0;
		DrxLogAlways("Profiling data started (%s), prefix = \"%s\"", display ? "display only" : "tracing", prefix);
	...
	{
		DrxLogAlways("Profiling data finished");
		{
	#if DRX_PLATFORM_WINDOWS
			// find the "frameprofileXX" filename for the file
			char outfilename[32] = "frameprofile.dat";
			// while there is such file already
			for (i32 i = 0; (GetFileAttributes(outfilename) != INVALID_FILE_ATTRIBUTES) && i < 1000; ++i)
				drx_sprintf(outfilename, "frameprofile%02d.dat", i);

			FILE* f = fopen(outfilename, "wb");
			if (!f)
			{
				DrxFatalError("Could not write profiling data to file!");
			}
			else
			{
				i32 i;
				...
				for (i = 0; i < (i32)m_pProfilers->size(); i++)
				{
				...
				i32 version = 2;                            // bump this if any of the format below changes
				fwrite(&version, sizeof(i32), 1, f);

				i32 numSamples = m_nCurSample;
				fwrite(&numSamples, sizeof(i32), 1, f); // number of samples per group (everything little endian)
				i32 mapsize = numProf + 1;              // Plus 1 global.
				fwrite(&mapsize, sizeof(i32), 1, f);

				// Write global profiler.
				fwrite("__frametime", strlen("__frametime") + 1, 1, f);
				i32 len = (i32)m_frameTimeOfflineHistory.m_selfTime.size();
				assert(len == numSamples);
				for (i = 0; i < len; i++)
				{
					fwrite(&m_frameTimeOfflineHistory.m_selfTime[i], 1, sizeof(i32), f);
				...
				for (i = 0; i < (i32)m_pProfilers->size(); i++)
				{
					...
					tukk name = GetFullName(pProfiler);
					//i32 slen = strlen(name)+1;
					fwrite(name, strlen(name) + 1, 1, f);

					len = (i32)pProfiler->m_pOfflineHistory->m_selfTime.size();
					assert(len == numSamples);
					for (i32 i = 0; i < len; i++)
					{
						fwrite(&pProfiler->m_pOfflineHistory->m_selfTime[i], 1, sizeof(i32), f);
				...
				;
				fclose(f);
				DrxLogAlways("Profiling data saved to file '%s'", outfilename);
			};
	#endif
		};
...
//////////////////////////////////////////////////////////////////////////
CFrameProfiler* CFrameProfileSystem::GetProfiler(i32 index) const
{
	assert(index >= 0 && index < (i32)m_profilers.size());
...
{
	DrxAutoCriticalSection lock(m_profilersLock);
	...
	pProfiler->m_colorIdentifier = (i32)pProfiler->m_description & PROFILE_TYPE_CHECK_MASK;
...
static i32 nMAX_THREADED_PROFILERS = 256;
...
void CFrameProfileSystem::SProfilerThreads::Reset()
{
	...
	if (!m_pReservedProfilers)
	{
		...
		for (i32 i = 0; i < nMAX_THREADED_PROFILERS; i++)
		{
...
void CFrameProfileSystem::StartProfilerSection(CFrameProfilerSection* pSection)
{
	...
	// Platform profiling
	::DrxProfile::detail::PushProfilingMarker(pSection->m_pFrameProfiler->m_colorIdentifier, pSection->m_pFrameProfiler->m_name);
	...
	pSection->m_startTime = DrxGetTicks();
}
...
void CFrameProfileSystem::EndProfilerSection(CFrameProfilerSection* pSection)
{
	...
	// Platform profiling
	DrxProfile::detail::PopProfilingMarker(pSection->m_pFrameProfiler->m_colorIdentifier, pSection->m_pFrameProfiler->m_name);
...
void CFrameProfileSystem::AccumulateProfilerSection(CFrameProfilerSection* pSection)
{
	...
	if (s_pFrameProfileSystem->profile_callstack)
	{
		if (pProfiler == s_pFrameProfileSystem->m_pGraphProfiler)
		{
			...
			DrxLogAlways("Function Profiler: %s  (time=%.2fms)", s_pFrameProfileSystem->GetFullName(pSection->m_pFrameProfiler), fMillis);
...
{
	DrxAutoCriticalSection lock(m_staticProfilersLock);
	...
	if (pSection->m_pFrameProfiler)
	{
		...
		s_pFrameProfileSystem->m_ProfilerThreads.PushSection(pSection, nThreadId);
		pSection->m_excludeTime = 0;
		pSection->m_startTime = DrxMemoryGetAllocatedSize();
	}
}
...
void CFrameProfileSystem::EndMemoryProfilerSection(CFrameProfilerSection* pSection)
{
	...
	int64 endTime = DrxMemoryGetAllocatedSize();
	TThreadId nThreadId = GetCurrentThreadId();
	CFrameProfiler* pProfiler = pSection->m_pFrameProfiler;
	if (!pProfiler)
		return;
	...
	if (0 == _stricmp(pProfiler->m_name, "DrxMalloc") ||
	    0 == _stricmp(pProfiler->m_name, "DrxRealloc") ||
	    0 == _stricmp(pProfiler->m_name, "DrxFree"))
	{
		selfTime = 0;
		totalTime = 0;
	}
...
void CFrameProfileSystem::EndCustomSection(CCustomProfilerSection* pSection)
{
	...
	i32 total = *pSection->m_pValue;
	i32 self = total - pSection->m_excludeValue;
...
tukk CFrameProfileSystem::GetFullName(CFrameProfiler* pProfiler)
{
	static char sNameBuffer[256];
	drx_sprintf(sNameBuffer, pProfiler->m_name);
...
//////////////////////////////////////////////////////////////////////////
tukk CFrameProfileSystem::GetModuleName(CFrameProfiler* pProfiler)
{
	return m_subsystems[pProfiler->m_subsystem].name;
}

tukk CFrameProfileSystem::GetModuleName(i32 num) const
{
...
i32k CFrameProfileSystem::GetModuleCount(void) const
{
	return (i32)PROFILE_LAST_SUBSYSTEM;
}

const float CFrameProfileSystem::GetOverBudgetRatio(i32 modulenumber) const
{
...
void CFrameProfileSystem::EndFrame()
{
	...
	if (!m_bNoPsapiDll)
	{
		...
		if (pfGetProcessMemoryInfo)
		{
			...
			m_nPagesFaultsLastFrame = (i32)(pc.PageFaultCount - m_nLastPageFaultCount);
			m_nLastPageFaultCount = pc.PageFaultCount;
			static float fLastPFTime = 0;
			static i32 nPFCounter = 0;
	...
	i32 nWeightMode = pVarMode ? pVarMode->GetIVal() : 0;
	...
	for (auto const pFrameProfiler : * m_pProfilers)
	{
		...
		if (pFrameProfiler != nullptr && (!m_bSubsystemFilterEnabled || (m_bSubsystemFilterEnabled && pFrameProfiler->m_subsystem == (u8)m_subsystemFilter)))
		{
			//filter stall profilers
			if ((i32)m_displayQuantity == STALL_TIME)
				continue;

			// Skip profiler markers
			u32 profilerType = pFrameProfiler->m_description & EProfileDescription::TYPE_MASK;
...
void CFrameProfileSystem::UpdateOfflineHistory(CFrameProfiler* pProfiler)
{
	...
	i32 prevCont = (i32)pProfiler->m_pOfflineHistory->m_selfTime.size();
	i32 newCount = m_nCurSample + 1;
	pProfiler->m_pOfflineHistory->m_selfTime.resize(newCount);
	pProfiler->m_pOfflineHistory->m_count.resize(newCount);

	u32 micros = FtoI(gEnv->pTimer->TicksToSeconds(pProfiler->m_selfTime) * 1e6f);
	u16 count = pProfiler->m_count;
	for (i32 i = prevCont; i < newCount; i++)
	{
...
void CFrameProfileSystem::AddPeak(SPeakRecord& peak)
{
	...
	if (gEnv->IsDedicated())
		GetISystem()->GetILog()->Log("Peak: name:'%s' val:%.2f avg:%.2f cnt:%d",
		                             (tukk)GetFullName(peak.pProfiler),
		                             peak.peakValue,
		                             peak.averageValue,
		                             peak.count);
...
//////////////////////////////////////////////////////////////////////////
void CFrameProfileSystem::SetSubsystemFilterThread(tukk sFilterThreadName)
{
	if (sFilterThreadName[0] != 0)
	{
		s_nFilterThreadId = gEnv->pThreadUpr->GetThreadId(sFilterThreadName);
	}
	else
	{
		s_nFilterThreadId = 0;
...
//////////////////////////////////////////////////////////////////////////
void CFrameProfileSystem::SetSubsystemFilter(tukk szFilterName)
{
	bool bFound = false;
	for (i32 i = 0; i < PROFILE_LAST_SUBSYSTEM; i++)
	{
			...
			case eKI_Up:
				{
					...
					i32 off = (event.keyId == eKI_Up) ? -1 : 1;
...
tukk CFrameProfileSystem::GetProfilerThreadName(CFrameProfiler* pProfiler) const
{
	if (pProfiler->m_threadId == GetMainThreadId())
	{
		return "Main";
	...
	{
		drx_sprintf(sThreadNameBuffer, "Job x%d", pProfiler->m_activeThreads);
	...
	{
		drx_sprintf(sThreadNameBuffer, "@%" PRI_THREADID, pProfiler->m_threadId);
	}
	return sThreadNameBuffer;
}
...
CFrameProfiler* CFrameProfileSystem::GetProfiler(i32 index) const                                  { return nullptr; }
void            CFrameProfileSystem::StartFrame()                                                  {}
void            CFrameProfileSystem::EndFrame()                                                    {}
void            CFrameProfileSystem::OnSliceAndSleep()                                             {}
void            CFrameProfileSystem::AddPeaksListener(IFrameProfilePeakCallback* pPeakCallback)    {}
