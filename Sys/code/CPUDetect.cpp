...
bool HasCPUID()
{
#if (DRX_PLATFORM_WINDOWS && DRX_PLATFORM_32BIT) || (DRX_PLATFORM_LINUX && DRX_PLATFORM_32BIT)
	_asm
	{
		pushfd                // save EFLAGS
...
#elif (DRX_PLATFORM_WINDOWS && DRX_PLATFORM_64BIT) || (DRX_PLATFORM_LINUX && DRX_PLATFORM_64BIT) || DRX_PLATFORM_MAC || DRX_PLATFORM_DURANGO || DRX_PLATFORM_ORBIS
...
#if DRX_PLATFORM_MAC || DRX_PLATFORM_LINUX || DRX_PLATFORM_ORBIS
static inline void __cpuid(i32 CPUInfo[4], i32 InfoType)
{
	asm  ("cpuid" : "=a" (*CPUInfo), "=b" (*(CPUInfo + 1)), "=c" (*(CPUInfo + 2)), "=d" (*(CPUInfo + 3)) : "a" (InfoType));
}
#endif // DRX_PLATFORM_MAC

bool IsAMD()
{
#if DRX_PLATFORM_WINDOWS || DRX_PLATFORM_LINUX || DRX_PLATFORM_MAC || DRX_PLATFORM_DURANGO || DRX_PLATFORM_ORBIS
	i32 CPUInfo[4];
	__cpuid(CPUInfo, 0x00000000);

	char szCPU[13];
	memset(szCPU, 0, sizeof(szCPU));
	*(i32*)&szCPU[0] = CPUInfo[1];
	*(i32*)&szCPU[4] = CPUInfo[3];
	*(i32*)&szCPU[8] = CPUInfo[2];
...
bool IsIntel()
{
#if DRX_PLATFORM_WINDOWS || DRX_PLATFORM_LINUX || DRX_PLATFORM_MAC
	i32 CPUInfo[4];
	__cpuid(CPUInfo, 0x00000000);
...
{
#if (DRX_PLATFORM_WINDOWS && DRX_PLATFORM_32BIT) || (DRX_PLATFORM_LINUX && DRX_PLATFORM_32BIT) || DRX_PLATFORM_DURANGO || DRX_PLATFORM_ORBIS
	i32 CPUInfo[4];
	...
	if (CPUInfo[3] & 0x20000000)    // Bit 29 in edx is set if 64-bit address extension is supported
		...
		return false;
#elif (DRX_PLATFORM_WINDOWS && DRX_PLATFORM_64BIT) || (DRX_PLATFORM_LINUX && DRX_PLATFORM_64BIT) || DRX_PLATFORM_MAC
	return true;
#else
	return false;
#endif
...
u8 GetAPIC_ID()
{
	...
	return (u8) ((CPUInfo[1] & 0xFF000000) >> 24);
#else
	return 0;
#endif
}

void GetCPUName(tuk pName)
{
#if DRX_PLATFORM_WINDOWS || DRX_PLATFORM_LINUX || DRX_PLATFORM_MAC || DRX_PLATFORM_DURANGO || DRX_PLATFORM_ORBIS
	if (pName)
	{
		...
		if (CPUInfo[0] >= 0x80000004)
		{
			...
			((i32*)pName)[0] = CPUInfo[0];
			((i32*)pName)[1] = CPUInfo[1];
			((i32*)pName)[2] = CPUInfo[2];
			((i32*)pName)[3] = CPUInfo[3];

			__cpuid(CPUInfo, 0x80000003);
			((i32*)pName)[4] = CPUInfo[0];
			((i32*)pName)[5] = CPUInfo[1];
			((i32*)pName)[6] = CPUInfo[2];
			((i32*)pName)[7] = CPUInfo[3];

			__cpuid(CPUInfo, 0x80000004);
			((i32*)pName)[8] = CPUInfo[0];
			((i32*)pName)[9] = CPUInfo[1];
			((i32*)pName)[10] = CPUInfo[2];
			((i32*)pName)[11] = CPUInfo[3];
		}
		else
			pName[0] = '\0';
	}
...
void GetCPUSteppingModelFamily(i32& stepping, i32& model, i32& family)
{
#if DRX_PLATFORM_WINDOWS || DRX_PLATFORM_LINUX || DRX_PLATFORM_MAC || DRX_PLATFORM_DURANGO || DRX_PLATFORM_ORBIS
	i32 CPUInfo[4];
	__cpuid(CPUInfo, 0x00000001);
...
#if DRX_PLATFORM_WINDOWS || DRX_PLATFORM_LINUX || DRX_PLATFORM_MAC || DRX_PLATFORM_DURANGO || DRX_PLATFORM_ORBIS
static u64 __stdcall DetectProcessor(uk arg)
{
	...
	u64 cache_ecx = 0;
	u64 cache_edx = 0;
	#if DRX_PLATFORM_WINDOWS && DRX_PLATFORM_X86
	u64 features_ebx = 0;
	u64 features_ecx = 0;
	u8 cyrix_flag = 0;
	#endif
	...
	if (HasCPUID())
	{
		if (IsAMD() && Has64bitExtension())
		{
			...
			drx_strcpy(p->mSerialNumber, "");
			GetCPUSteppingModelFamily(p->mStepping, p->mModel, p->mFamily);
			drx_strcpy(p->mVendor, "AMD");
			GetCPUName(p->mCpuType);
			drx_strcpy(p->mFpuType, HasFPUOnChip() ? "On-Chip" : "Unknown");
			p->mbPhysical = true;
...
end_fpu_type:
	...
	#else // DRX_PLATFORM_WINDOWS && DRX_PLATFORM_X86
	cpu_type = 0xF;
	fpu_type = 3;
	signature = 0;
	#endif // DRX_PLATFORM_WINDOWS && DRX_PLATFORM_X86
	p->mFamily = cpu_type;
	...
	drx_strcpy(p->mCpuType, sCpuType.c_str());
	drx_strcpy(p->mFpuType, fpu_string);
	drx_strcpy(p->mVendor, vendor_string);
...
#endif // DRX_PLATFORM_WINDOWS || DRX_PLATFORM_LINUX || DRX_PLATFORM_MAC || DRX_PLATFORM_DURANGO || DRX_PLATFORM_ORBIS

#if DRX_PLATFORM_MAC || DRX_PLATFORM_LINUX
static uk DetectProcessorThreadProc(uk pData)
{
...
/*
   ...
   #else // DRX_PLATFORM_WINDOWS && DRX_PLATFORM_64BIT
   i32 GetPhysicalCpuCount()
   {
   return 1;
   }
   #endif // DRX_PLATFORM_WINDOWS && DRX_PLATFORM_64BIT
 */

#if DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID
// collection of functions to read from /proc/cpuinfo

static bool proc_read_str(tuk buffer, tuk output, size_t output_length)
{
	...
	{
		buffer += 2;
		drx_strcpy(output, output_length, buffer);
		i32k len = strlen(output);
...
static bool proc_read_int(tuk buffer, i32& output)
{
	if (!buffer)
	{
		return false;
...
void CCpuFeatures::Detect(void)
{
	...
	for (u8 c = 0; c < m_NumSystemProcessors; c++)
	{
	...
	/*    m_NumLogicalProcessors = m_NumSystemProcessors = sysconf(_SC_NPROCESSORS_CONF);
	   ...
	   for( u8 c = 0; c < m_NumAvailProcessors; c++ )
	   {
	     pthread_attr_init(&attr);
	     CPU_ZERO(&cpuset);
	     CPU_SET(c+1,&cpuset);
	     pthread_attr_setaffinity_np(&attr,sizeof(cpuset),&cpuset);
	     pthread_create(&threads[c],&attr,&DetectProcessorThreadProc,(uk )&m_Cpu[c]);
	   }
	   for( u8 c = 0; c < m_NumSystemProcessors; c++ )
	   {
	...
	if (!cpu_info)
	{
		...
		DrxLogAlways("Could not open /proc/cpuinfo, defaulting values to 1.");
	}
	else
	{
		i32 nCores = 0;
		...
		while (!feof(cpu_info))
		{
			...
			else if (strncmp("fpu", buffer, (index = strlen("fpu"))) == 0)
			{
				...
				if (buffer[index] == ':')
				{
					...
					{
						drx_strcpy(m_Cpu[nCpu].mFpuType, "On-Chip");
					}
					else
					{
						drx_strcpy(m_Cpu[nCpu].mFpuType, "Unkown");
					}
				}
			}
	...
	{
		DrxLogAlways("Failed to detect the number of available processors, defaulting to 1");
		m_NumSystemProcessors = 1;
	}
	...
	{
		DrxLogAlways("Failed to detect the number of available logical processors, defaulting to 1");
	...
	{
		DrxLogAlways("Failed to detect cpu frequency , defaulting to 0");
		cpu_freq = 0;
	}
	...
	#endif
	for (i32 c = 0; c < m_NumAvailProcessors; c++)
	{
		m_Cpu[c] = cpuInfo;
	}

#elif DRX_PLATFORM_DURANGO || DRX_PLATFORM_ORBIS
...
#if DRX_PLATFORM_WINDOWS
	DrxLogAlways("Total number of logical processors: %d", m_NumSystemProcessors);
	DrxLogAlways("Number of available logical processors: %d", m_NumAvailProcessors);
	...
	DrxLogAlways("Total number of system cores: %d", m_NumSystemProcessors);
	DrxLogAlways("Number of cores available to process: %d", m_NumAvailProcessors);

#else
	DrxLogAlways("Number of system processors: %d", m_NumSystemProcessors);
	DrxLogAlways("Number of available processors: %d", m_NumAvailProcessors);
#endif

	if (m_NumAvailProcessors > MAX_CPU)
		m_NumAvailProcessors = MAX_CPU;

	for (i32 i = 0; i < m_NumAvailProcessors; i++)
	{
		SCpu* p = &m_Cpu[i];
		if (i == 0)
		...
		DrxLogAlways(" ");
		DrxLogAlways("Processor %d:", i);
		DrxLogAlways("  CPU: %s %s", p->mVendor, p->mCpuType);
		DrxLogAlways("  Family: %d, Model: %d, Stepping: %d, FPU: %s", p->mFamily, p->mModel, p->mStepping, p->mFpuType);
		string sFeatures;
		...
		DrxLogAlways("  Features: %s", sFeatures.c_str());
	}

	DrxLogAlways(" ");

	//m_NumPhysicsProcessors = m_NumSystemProcessors;
	for (i32 i = m_NumPhysicsProcessors = 0; i < m_NumAvailProcessors; i++)
