...
#include <drx/Core/lib/z/zlib.h> // declaration of Z_OK for ZipRawDecompress
...
//////////////////////////////////////////////////////////////////////////
tuk ZipDir::CacheRW::UnifyPath(tuk const str, tukk pPath)
{
	assert(str);
	tukk src = pPath;
	tuk trg = str;
...
bool ZipDir::CacheRW::WriteCompressedData(tukk data, size_t size, bool encrypt)
{
	if (size == 0)
		return true;
	...
	#else
		#if !defined(_RELEASE)
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "Attempt to use XTEA pak encryption when it is disabled.");
		#endif
		return false;
	#endif
	}
...
//////////////////////////////////////////////////////////////////////////
tuk ZipDir::CacheRW::AllocPath(tukk pPath)
{
	char str[_MAX_PATH];
	tuk temp = UnifyPath(str, pPath);
...
ZipDir::ErrorEnum ZipDir::CacheRW::UpdateFile(tukk szRelativePathSrc, uk pUncompressed, unsigned nSize, unsigned nCompressionMethod, i32 nCompressionLevel)
{
	char str[_MAX_PATH];
	tuk szRelativePath = UnifyPath(str, szRelativePathSrc);
	...
	// chunks
	const u64 maxChunk = 1 << 20;
	u64 sizeLeft = nSizeCompressed;
	u64 sizeToWrite;
	tuk ptr = (tuk) pCompressed;
...
ZipDir::ErrorEnum ZipDir::CacheRW::StartContinuousFileUpdate(tukk szRelativePathSrc, unsigned nSize)
{
	char str[_MAX_PATH];
	tuk szRelativePath = UnifyPath(str, szRelativePathSrc);
...
// adds a directory (creates several nested directories if needed)
ZipDir::ErrorEnum ZipDir::CacheRW::UpdateFileContinuousSegment(tukk szRelativePathSrc, unsigned nSize, uk pUncompressed, unsigned nSegmentSize, unsigned nOverwriteSeekPos)
{
	char str[_MAX_PATH];
	tuk szRelativePath = UnifyPath(str, szRelativePathSrc);
	...
	if (!WriteCompressedData((tuk)pUncompressed, nSegmentSize, encrypt))
	{
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR,
		           "Cannot write to zip file!! error = (%d): %s", errno, strerror(errno));
		return ZD_ERROR_IO_FAILED;
...
ZipDir::ErrorEnum ZipDir::CacheRW::UpdateFileCRC(tukk szRelativePathSrc, unsigned dwCRC32)
{
	char str[_MAX_PATH];
	tuk szRelativePath = UnifyPath(str, szRelativePathSrc);
...
ZipDir::ErrorEnum ZipDir::CacheRW::RemoveFile(tukk szRelativePathSrc)
{
	char str[_MAX_PATH];
	tuk szRelativePath = UnifyPath(str, szRelativePathSrc);

	// find the last slash in the path
	tukk pSlash = max(strrchr(szRelativePath, '/'), strrchr(szRelativePath, '\\'));

	tukk pFileName; // the name of the file to delete
...
ZipDir::ErrorEnum ZipDir::CacheRW::RemoveDir(tukk szRelativePathSrc)
{
	char str[_MAX_PATH];
	tuk szRelativePath = UnifyPath(str, szRelativePathSrc);
	...
	tukk pDirName; // the name of the dir to delete
...
ZipDir::ErrorEnum ZipDir::CacheRW::ReadFile(FileEntry* pFileEntry, uk pCompressed, uk pUncompressed)
{
	if (!pFileEntry)
...
ZipDir::FileEntry* ZipDir::CacheRW::FindFile(tukk szPathSrc, bool bFullInfo)
{
	char str[_MAX_PATH];
	tuk szPath = UnifyPath(str, szPathSrc);
...
uk ZipDir::CacheRW::AllocAndReadFile(FileEntry* pFileEntry)
{
	if (!pFileEntry)
		return NULL;

	uk pData = m_pHeap->TempAlloc(pFileEntry->desc.lSizeUncompressed, "ZipDir::CacheRW::AllocAndReadFile");
	if (pData)
	{
...
bool ZipDir::CacheRW::WriteCDR(FILE* fTarget)
{
	...
	uk pCDR = m_pHeap->TempAlloc(nSizeCDR, "ZipDir::CacheRW::WriteCDR");
	...
	if (m_encryptedHeaders == HEADERS_ENCRYPTED_TEA)
	{
		...
		ZipDir::Encrypt((tuk)pCDR, nSizeCDR - sizeof(ZipFile::CDREnd));
	#else
		#if !defined(_RELEASE)
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "Attempt to use XTEA pak encryption when it is disabled.");
		#endif
...
string ZipDir::CacheRW::GetRandomName(i32 nAttempt)
{
	if (nAttempt)
	{
		char szBuf[8];
...
{
	for (i32 nAttempt = 0; nAttempt < 32; ++nAttempt)
	{
		string strNewFilePath = m_strFilePath + "$" + GetRandomName(nAttempt);
		if (DrxGetFileAttributes(strNewFilePath.c_str()) != ~0)
...
bool ZipDir::CacheRW::RelinkZip(FILE* fTmp)
{
	...
	for (FileRecordList::iterator it = arrFiles.begin(); it != arrFiles.end(); ++it)
	{
		...
		if (it->pFileEntry->IsEncrypted())
		{
			...
			{
				u8 IV[ZipFile::BLOCK_CIPHER_KEY_LENGTH]; //16 byte
				i32 nKeyIndex = ZipEncrypt::GetEncryptionKeyIndex(it->pFileEntry);
				ZipEncrypt::GetEncryptionInitialVector(it->pFileEntry, IV);
				if (!ZipEncrypt::DecryptBufferWithStreamCipher((u8*)(tuk)pFile->GetData(), it->pFileEntry->desc.lSizeCompressed, m_block_cipher_keys_table[nKeyIndex], IV))
				{
					// Decryption of CDR failed.
					return false;
				}
			...
			{
				ZipDir::Decrypt((tuk)pFile->GetData(), it->pFileEntry->desc.lSizeCompressed)
			}
	#endif //SUPPORT_XTEA_PAK_ENCRYPTION
	#if !defined(OPTIMIZED_READONLY_ZIP_ENTRY) && defined(SUPPORT_STREAMCIPHER_PAK_ENCRYPTION)
			else if (it->pFileEntry->nMethod == METHOD_DEFLATE_AND_STREAMCIPHER)
			{
				ZipDir::StreamCipher((tuk)pFile->GetData(), it->pFileEntry);
			}
	#endif //!OPTIMIZED_READONLY_ZIP_ENTRY && SUPPORT_STREAMCIPHER_PAK_ENCRYPTION
			else
			{
	...
	u32 lOldCDROffset = m_lCDROffset;
	// the file data has now been written out. Now write the CDR
	#if DRX_PLATFORM_WINDOWS
	m_lCDROffset = (u32)_ftelli64(fTmp);
	#elif DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID
	m_lCDROffset = (u32)ftello(fTmp);
	#else
...
bool ZipDir::CacheRW::WriteZipFiles(std::vector<FileDataRecordPtr>& queFiles, FILE* fTmp)
{
	...
	#if DRX_PLATFORM_WINDOWS
		(*it)->pFileEntry->nFileHeaderOffset = (u64)_ftelli64(fTmp);
	#elif DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID
		(*it)->pFileEntry->nFileHeaderOffset = (u64)ftello(fTmp);
	#else
	...
	#if DRX_PLATFORM_WINDOWS
		assert((*it)->pFileEntry->nEOFOffset == (u64)_ftelli64(fTmp));
	#elif DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID
		assert((*it)->pFileEntry->nEOFOffset == (u64)ftello(fTmp));
	#else
