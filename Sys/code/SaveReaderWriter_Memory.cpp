...
// -------------------------------------------------------------------------
//  Имя файла:   SaveReaderWriter_Memory.cpp
//  Created:     30/4/2010 by Ian Masters.
//  Описание: Simple in-memory file system for saving/loading files.
// -------------------------------------------------------------------------
//  История:
//
//  TODO:
...
#include <drx3D/Sys/PatternMatcher.h>
...
public:
	CMemoryFile(tukk fileName, u32 userIndex)
		: m_user(userIndex)
		, m_filePath(fileName)
		, m_filePtr(0)
		, m_bDirty(true)
	...
	void FillFindData(_finddata_t& fd)
	{
		...
		fd.size = GetLength();
		drx_strcpy(fd.name, GetFilePath());
	}
...
class CMemoryFileStandard : public CMemoryFile
{
	...
	std::vector<u8, stl::STLGlobalAllocator<u8>> m_fileBytes;

public:

	CMemoryFileStandard(tukk fileName, u32 userIndex) : CMemoryFile(fileName, userIndex)
	{
	...
	virtual void Truncate()
	{
		...
		time_t unixtime;
		time(&unixtime);
		// Borrowed from UnixTimeToFileTime in LocalizedStringUpr.cpp
#ifndef Int32x32To64
	...
	virtual ukk GetFileContents() const
	{
...
public:
	...
	virtual IMemoryFilePtr CreateFile(tukk fileName, u32 userIndex)
	{
		Lock();
	...
	virtual IMemoryFilePtr GetFile(tukk fileName, u32 user)
	{
		Lock();
	...
	virtual i32 GetNumFiles(u32 user)
	{
		Lock();
		i32 count = 0;
		for (FileList::const_iterator it = m_files.begin(); it != m_files.end(); ++it)
	...
	virtual i32 GetNumDirtyFiles(u32 user)
	{
		Lock();
		i32 dirty = 0;
		for (FileList::const_iterator it = m_files.begin(); it != m_files.end(); ++it)
	...
	virtual intptr_t FindFirst(u32 userIndex, tukk filePattern, _finddata_t* fd)
	{
		Lock();
		intptr_t result = -1;
		...
		for (find->it = m_files.begin(); find->it != m_files.end(); ++find->it)
		{
			...
			{
				i32 match = PatternMatch(file->m_filePath, filePattern);
				if (match)
				{
	...
	virtual i32 FindNext(intptr_t handle, _finddata_t* fd)
	{
		...
		for (++find->it; find->it != m_files.end(); ++find->it)
		{
			...
			{
				i32 match = PatternMatch(file->m_filePath, find->filePattern);
				if (match)
				{
					file->FillFindData(*fd);
					result = 0;
	...
	virtual void Format(u32 user)
	{
		...
		Unlock();
		DrxLog("[CSaveWriter_Memory:%x] Formatted", user);
	}
	...
	virtual bool DebugSave(u32 user, IPlatformOS::SDebugDump& dump)
	{
	...
	static void CorrectUserName(string& fileName)
	{
		...
		static tukk key = "%user%/profiles/";
		size_t offset = path.find(key);
		if (offset != string::npos)
			fileName.insert(offset + strlen(key), "default/");
	}

	virtual bool DebugLoad(u32 user, tukk fileName)
	{
	...
	virtual void DumpFiles(tukk label)
	{
#ifndef _RELEASE
		Lock();
		DrxLog("[IMemoryFileSystem] Dumping files - %s", label);
		for (FileList::const_iterator it = m_files.begin(); it != m_files.end(); ++it)
		{
			const CMemoryFile* file = static_cast<CMemoryFile*>(it->get());
			DrxLog("[%u] %s, %s", file->m_user, file->GetFilePath(), file->m_bDirty ? "Dirty" : "Clean");
		}
		Unlock();
#endif // _RELEASE
	}
...
CSaveWriter_Memory::CSaveWriter_Memory(const IMemoryFileSystemPtr& pFileSystem, tukk fileName, u32 userIndex)
	...
	, m_bClosed(false)
{
	...
	pFileSystem->Unlock();
	DrxLog("[CSaveWriter_Memory:%x] \"%s\" opened for write", userIndex, fileName);
}
...
IPlatformOS::EFileOperationCode CSaveWriter_Memory::AppendBytes(ukk data, size_t length)
{
	CMemoryFile* file = static_cast<CMemoryFile*>(m_file.get());
	return file->AppendBytes(data, length);
}

void CSaveWriter_Memory::GetMemoryUsage(IDrxSizer* pSizer) const
{
	pSizer->Add(*this);
...
CSaveReader_Memory::CSaveReader_Memory(const IMemoryFileSystemPtr& pFileSystem, tukk fileName, u32 userIndex)
	: m_pFileSystem(pFileSystem)
	, m_eLastError(IPlatformOS::eFOC_Success)
{
	//assert(userIndex != IPlatformOS::Unknown_User);
	...
	if (m_file != NULL)
	{
		...
		DrxLog("[CSaveReader_Memory:%x] \"%s\" opened for read", userIndex, fileName);
	}
	else
...
IPlatformOS::EFileOperationCode CSaveReader_Memory::ReadBytes(uk data, size_t numBytes)
{
	...
	memcpy(data, &static_cast<tukk >(m_file->GetFileContents())[filePos], count);
...
void CSaveReader_Memory::GetMemoryUsage(IDrxSizer* pSizer) const
{
	pSizer->Add(*this);
	m_file->GetMemoryUsage(pSizer);
}

CFileFinderMemory::CFileFinderMemory(const IMemoryFileSystemPtr& pFileSystem, u32 user)
	: m_pFileSystem(pFileSystem)
	, m_user(user)
{
	if (m_pFileSystem)
...
intptr_t CFileFinderMemory::FindFirst(tukk filePattern, _finddata_t* fd)
{
	return m_pFileSystem->FindFirst(m_user, filePattern, fd);
}

i32 CFileFinderMemory::FindNext(intptr_t handle, _finddata_t* fd)
{
	return m_pFileSystem->FindNext(handle, fd);
}

i32 CFileFinderMemory::FindClose(intptr_t handle)
{
	return m_pFileSystem->FindClose(handle);
}

IPlatformOS::IFileFinder::EFileState CFileFinderMemory::FileExists(tukk path)
{
	IMemoryFilePtr file = m_pFileSystem->GetFile(path, m_user);
	return file ? eFS_File : eFS_NotExist;
}

void CFileFinderMemory::GetMemoryUsage(IDrxSizer* pSizer) const
{
	pSizer->Add(*this);
}
