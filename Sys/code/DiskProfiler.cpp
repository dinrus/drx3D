...
#include <drx3D/Sys/DiskProfilerWindowsSpecific.h>
...
i32 CDiskProfiler::profile_disk = 0;
i32 CDiskProfiler::profile_disk_max_items = 10000;
i32 CDiskProfiler::profile_disk_max_draw_items = 2000;
float CDiskProfiler::profile_disk_timeframe = 5;    // max time for profiling timeframe
i32 CDiskProfiler::profile_disk_type_filter = -1;
i32 CDiskProfiler::profile_disk_budget = -1;
...
bool CDiskProfiler::RegisterStatistics(SDiskProfileStatistics* pStatistics)
{
	...
	{
		assert(pStatistics);
		DrxAutoCriticalSection lock(m_csLock);
		...
		if ((i32)m_statistics.size() < profile_disk_max_items)
...
{
	i32k width  = gEnv->pRenderer->GetOverlayWidth();
	i32k height = gEnv->pRenderer->GetOverlayHeight();
	...
	i32 szy = 20; // vertical text labels' step

	i32 labelHeight = m_nHeightOffset - szy;  // height for current label in screen stack

	i32 startFileLabel = 0;
	// display IO statistics
	{
		...
		if (!firstMeasure)
		{
			...
			if (profile_disk_budget > 0 && profile_disk_budget < (i32)fThp)
			{
	...
	for (Statistics::reverse_iterator it = m_statistics.rbegin(); it != m_statistics.rend(); ++it)
	{
		...
		u32k nColorId = profile_disk != 3 ? (*it)->m_threadId : (*it)->m_nTaskType;
	...
	for (ThreadColorMap::const_iterator i = m_threadsColorLegend.begin(); i != m_threadsColorLegend.end(); ++i)
	{
		...
		if (profile_disk != 3)
			IRenderAuxText::Draw2dLabel(5, labelHeight, fTextSize, fThreadColor, false, "Thread: %s", gEnv->pThreadUpr->GetThreadName(i->first));
		else
		{
...
void CDiskProfiler::Update()
{
	...
	if (profile_disk > 0)
	{
		...
		for (u32 i = 0; i < m_statistics.size(); )
		{
...
{
	return m_bEnabled && (i32)m_statistics.size() < profile_disk_max_items;
}

void CDiskProfiler::SetTaskType(const threadID nThreadId, u32k nType /*= eStreamTaskTypeCount*/)
{
	DrxAutoCriticalSection lock(m_csLock);
