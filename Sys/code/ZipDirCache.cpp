...
DrxCriticalSection ZipDir::Cache::CacheData::m_csCacheIOLock;
#endif

// initializes the instance structure
void ZipDir::Cache::Construct(CZipFile& fNew, CMTSafeHeap* pHeap, size_t nDataSizeIn, u32 nFactoryFlags, size_t nAllocatedSize)
{
	m_pRefCount = new i32;
	*m_pRefCount = 0;
...
void ZipDir::Cache::PreloadToMemory(IMemoryBlock* pMemoryBlock)
{
	...
	DrxAutoCriticalSection lock(m_pCacheData->m_csCacheIOLock);
...
ZipDir::CachePtr ZipDir::NewCache(tukk szFileName, CMTSafeHeap* pHeap, InitMethodEnum nInitMethod)
{
	// try .. catch(Error)
	CacheFactory factory(pHeap, nInitMethod);
	return factory.New(szFileName, (nInitMethod == ZD_INIT_VALIDATE_IN_MEMORY) ? IDrxPak::eInMemoryPakLocale_GPU : IDrxPak::eInMemoryPakLocale_Unload);
}
...
{
	bool operator()(const ZipDir::FileEntry& f1, u32 nCrc32) const
	{
		return f1.nNameOffset < nCrc32;
	}
	bool operator()(u32 nCrc32, const ZipDir::FileEntry& f1) const
	{
...
ZipDir::FileEntry* ZipDir::Cache::FindFile(tukk szPath, bool bRefresh)
{
	if (m_nCacheFactoryFlags & CacheFactory::FLAGS_FILENAMES_AS_CRC32)
	{
		uk pDataStart = (this + 1);
		FileEntry* pFirstEntry = reinterpret_cast<FileEntry*>(pDataStart);
		FileEntry* pLastEntry = pFirstEntry + (m_nDataSize / sizeof(FileEntry));

		u32 nNameAsCrc32 = ZipDir::FileNameHash(szPath);
...
ZipDir::ErrorEnum ZipDir::Cache::ReadFile(FileEntry* pFileEntry, uk pCompressed, uk pUncompressed, const bool decompress, int64 nDataOffset, int64 nDataReadSize, const bool decrypt)
{
	if (!pFileEntry)
	...
	{
	#if !defined(_RELEASE)
		DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "Tried to load a non-encrypted file when support for non-encrypted paks is disabled");
	#endif //!_RELEASE
	...
	uk pBuffer = pCompressed; // the buffer where the compressed data will go
	...
	{
		DrxAutoCriticalSection lock(m_pCacheData->m_csCacheIOLock); // guarantees that fseek() and fread() will be executed together
		DRX_PROFILE_REGION(PROFILE_SYSTEM, "ZipDir_Cache_ReadFile");

		{
			...
			if (nSeekRes)
			{
				...
				i32 errnoVal = 0;
#endif
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "ZipDir::FSeek failed (%p (%i, %i, %lli), %lli, %i, %p) = %u, %i",
				           &m_zipFile, ZipDir::FError(&m_zipFile), ZipDir::FEof(&m_zipFile), ZipDir::FTell(&m_zipFile),
				           nFileOffset + pFileEntry->nFileDataOffset, SEEK_SET, pFileEntry, (i32)GetLastError(), errnoVal);
		...
		}
		{
			...
			if (nReadRes != 1)
			{
				...
				i32 errnoVal = 0;
#endif
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "ZipDir::FRead failed (%p (%i, %i, %lli), %p, %i, %i, %p) = %lli, %u, %i",
				           &m_zipFile, ZipDir::FError(&m_zipFile), ZipDir::FEof(&m_zipFile), ZipDir::FTell(&m_zipFile),
				           pBuffer, (i32)nFileReadSize, 1, pFileEntry, nReadRes, (i32)GetLastError(), errnoVal);
				return ZD_ERROR_IO_FAILED;
			}
		}
	}
	...
	if (decrypt && pFileEntry->IsEncrypted())
	{
		...
		else if (pFileEntry->nMethod == METHOD_STORE_AND_STREAMCIPHER_KEYTABLE || pFileEntry->nMethod == METHOD_DEFLATE_AND_STREAMCIPHER_KEYTABLE)
		{
			...
			i32 nKeyIndex = ZipEncrypt::GetEncryptionKeyIndex(pFileEntry);
			ZipEncrypt::GetEncryptionInitialVector(pFileEntry, IV);
			if (!ZipEncrypt::DecryptBufferWithStreamCipher((u8*)pBuffer, pFileEntry->desc.lSizeCompressed, m_block_cipher_keys_table[nKeyIndex], IV))
			{
		...
		else if (pFileEntry->nMethod == METHOD_DEFLATE_AND_ENCRYPT)
		{
			...
			ZipDir::Decrypt((tuk)pBuffer, pFileEntry->desc.lSizeCompressed);
		}
		...
		{
			ZipDir::StreamCipher((tuk)pBuffer, pFileEntry);
		}
#endif
		else
		{
...
#if !defined(_RELEASE)
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "Tried to load a file from an archive using an unsupported method");
#endif  //!_RELEASE
			return ZD_ERROR_CORRUPTED_DATA;
		}
	}
	...
	if (pUncompressed && decompress)
	{
		...
		{
			u32 computedCRC32 = crc32(0, (u8*)pUncompressed, pFileEntry->desc.lSizeUncompressed);
	...
	#if !defined(_RELEASE)
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "ZipDir::Cache::ReadFile mismatch detected for file %s: Generated CRC = 0x%8X, Loaded CRC = 0x%8X", GetFileEntryName(pFileEntry), computedCRC32, pFileEntry->desc.lCRC32);
	#endif //!_RELEASE
...
ZipDir::ErrorEnum ZipDir::Cache::ReadFileStreaming(FileEntry* pFileEntry, uk pOut, int64 nDataOffset, int64 nDataReadSize)
{
	if (!pFileEntry)
		return ZD_ERROR_INVALID_CALL;
	...
	if (!m_zipFile.IsInMemory() && g_cvars.pakVars.nUncachedStreamReads)
	{
		...
		if (m_zipFile.m_unbufferedFile != INVALID_HANDLE_VALUE)
		{
			...
			while (nBufferOffs < (ptrdiff_t)nDataReadSize)
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR_DBGBRK, "ZipDir::ReadFileStreaming failed (%p %p) = %i",
				...
				memcpy(reinterpret_cast<tuk>(pOut) + dstLeft, reinterpret_cast<tuk>(m_zipFile.m_pReadTarget) + srcLeft, dstRight - dstLeft);
...
ZipDir::ErrorEnum ZipDir::Cache::DecompressFile(FileEntry* pFileEntry, uk pCompressed, uk pUncompressed, DrxCriticalSection& csDecmopressLock)
{
	...
	u64 nSizeUncompressed = pFileEntry->desc.lSizeUncompressed;

	SmartPtr pBufferDestroyer(m_pCacheData->m_pHeap);

	uk pBuffer = pCompressed;
...
uk ZipDir::Cache::AllocAndReadFile(FileEntry* pFileEntry)
{
	if (!pFileEntry)
		return NULL;

	uk pData = m_pCacheData->m_pHeap->TempAlloc(pFileEntry->desc.lSizeUncompressed, "ZipDir::Cache::AllocAndReadFile");
	if (pData)
	{
...
void ZipDir::Cache::Free(uk pData)
{
	m_pCacheData->m_pHeap->FreeTemporary(pData);
}
...
ZipDir::ErrorEnum ZipDir::Cache::Refresh(FileEntry* pFileEntry)
{
	...
	DrxAutoCriticalSection lock(m_pCacheData->m_csCacheIOLock); // Not thread safe without this
	return ZipDir::Refresh(&m_zipFile, pFileEntry);
}

//////////////////////////////////////////////////////////////////////////
u32 ZipDir::Cache::GetFileDataOffset(FileEntry* pFileEntry)
{
	if (pFileEntry->nFileDataOffset == pFileEntry->INVALID_DATA_OFFSET)
	{
		DrxAutoCriticalSection lock(m_pCacheData->m_csCacheIOLock); // Not thread safe without this
...
bool ZipDir::Cache::ReOpen(tukk filePath)
{
	DrxAutoCriticalSection lock(m_pCacheData->m_csCacheIOLock);
	...
	if (newFile)
	{
		...
		if (!m_zipFile.OpenUnbuffered(filePath))
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "Failed to reopen '%s' for unbuffered IO.", filePath);
#endif
...
//////////////////////////////////////////////////////////////////////////
tukk ZipDir::Cache::GetFileEntryName(FileEntry* pFileEntry)
{
	if (m_pRootData)
...
void ZipDir::Cache::ValidateFilenameHash(u32 nHash, tukk filename, tukk pakFile)
{
	static std::map<u32, SValidateHashEntry*> validateHashMap;
	...
	if (hashEntry)
	{
		...
		{
			DrxFatalError("Filename Hash function collision, Files from different pak files generate same hash code [0x%X]\r\nFile1: (%s) %s\r\nFile2: (%s) %s",
...
//////////////////////////////////////////////////////////////////////////
int64 ZipDir::FSeek(CZipFile* file, int64 origin, i32 command)
{
	if (file->IsInMemory())
	{
		int64 retCode = -1;
...
int64 ZipDir::FRead(CZipFile* file, uk data, size_t elementSize, size_t count)
{
	if (file->IsInMemory())
	{
...
int64 ZipDir::FTell(CZipFile* file)
{
	...
	else
	{
		...
		if (file->InPakFileOrAsset() || file->IsAssetFile())
		{
			u32 offset = file->GetFileDataOffset();
			FILE* pFile = NULL;
			u32 length = 0;
			...
			{
	#if !defined(_RELEASE)
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "File position indicator out of range.");
	#endif //!_RELEASE
				return -1;
			}
			return relPos;
...
i32 ZipDir::FError(CZipFile* zipFile)
{
...
i32 ZipDir::FEof(CZipFile* zipFile)
{
	if (zipFile->IsInMemory())
