...
public:
	...
	void Reset()
	{
		...
		for (i32 i = 0; i < eMAX_THREADS_TO_PROFILE; ++i)
		{
			m_threadNames[i].clear();
		}
	}

	ILINE u32 GetThreadIndexByID(threadID threadID)
	{
		for (i32 i = 0; i < eMAX_THREADS_TO_PROFILE; ++i)
		{
			if (m_threadInfo[i] == 0)
		...
		u32k counter = DrxInterlockedIncrement(&m_threadCounter) - 1;     //count to index
		m_threadInfo[counter] = threadID;
		m_threadNames[counter] = gEnv->pThreadUpr->GetThreadName(threadID);

		return counter;
	}

	ILINE tukk GetThreadNameByIndex(u32 threadIndex)
	{
		assert(threadIndex < m_threadCounter);
		return m_threadNames[threadIndex].c_str();
	}
	
	ILINE threadID GetThreadIdByIndex(u32 threadIndex)
	{
		assert(threadIndex < m_threadCounter);
...
private:
	...
	DrxFixedStringT<32> m_threadNames[eMAX_THREADS_TO_PROFILE];
	i32                 m_threadCounter;
};
...
i32 CBootProfiler::CV_sys_bp_enabled = 1;
i32 CBootProfiler::CV_sys_bp_level_load = 1;
i32 CBootProfiler::CV_sys_bp_frames_worker_thread = 0;
i32 CBootProfiler::CV_sys_bp_frames = 0;
i32 CBootProfiler::CV_sys_bp_frames_sample_period = 0;
i32 CBootProfiler::CV_sys_bp_frames_sample_period_rnd = 0;
...
public:
	...
	ILINE CBootProfilerRecord(tukk label, LARGE_INTEGER timestamp, u32 threadIndex, tukk args, CBootProfilerSession* pSession,EProfileDescription type)
		:	m_label(label)
	...
	void Print(FILE* file, tuk buf, size_t buf_size, size_t depth, LARGE_INTEGER stopTime, LARGE_INTEGER frequency, tukk threadName, const float timeThreshold)
	{
...

		{
			...
			drx_sprintf(buf, buf_size, "%s<block name=\"%s\" totalTimeMS=\"%f\" startTime=\"%" PRIu64 "\" stopTime=\"%" PRIu64 "\" args=\"%s\"> \n",
		...
		drx_sprintf(buf, buf_size, "%s</block>\n", tabs.c_str());
		fprintf(file, "%s", buf);
	}
};
...
public:
	...
	{
		m_baseAddr = (CBootProfilerRecord*)DrxModuleMemalign(eNUM_RECORDS_PER_POOL * sizeof(CBootProfilerRecord), 16);
	}

	~CRecordPool()
	{
		DrxModuleMemalignFree(m_baseAddr);
		delete m_next;
	}
...
public:
	CBootProfilerSession(tukk szName);
	~CBootProfilerSession();

	void                 Start();
	void                 Stop();

	CBootProfilerRecord* StartBlock(tukk name, tukk args, u32k threadIndex, EProfileDescription type);
	...
	struct DRX_ALIGN(CACHE_LINE_SIZE_BP_INTERNAL) SThreadEntry
	{
		union
		{
			struct
			{
			 ...
			 char m_padding[CACHE_LINE_SIZE_BP_INTERNAL];
		};
	};
...
CBootProfilerSession::CBootProfilerSession(tukk szName) : m_name(szName)
{
...
{
	for (u32 i = 0; i < eMAX_THREADS_TO_PROFILE; ++i)
	{
...
CBootProfilerRecord* CBootProfilerSession::StartBlock(tukk name, tukk args, u32k threadIndex, EProfileDescription type)
{
	...
	u32 profilerType = type & EProfileDescription::TYPE_MASK;
	...
	struct BootProfilerEventSerializerToJSON
	{
		...
		void Serialize(Serialization::IArchive& ar)
		{
			...
			if (type == EventType::THREAD_NAME)
			{
				...
				string threadName = GetISystem()->GetIThreadUpr()->GetThreadName(threadId);
			...
			else if (type == EventType::THREAD_SORT)
			{
				...
				ar(string("M"), "ph");
				ar(SSerializeIntArg{(u32)counter++,"sort_index"},"args");
				return;
			}
...

			{
				...
				u32 profilerType = pRecord->m_type & EProfileDescription::TYPE_MASK; //FUNCTIONENTRY,REGION,SECTION
	...
	void Serialize(Serialization::IArchive& ar)
	{
		...
		for (size_t i = 0; i < numThreads; ++i)
		{
			...
			if (pRoot)
			{
				...
				tukk threadName = gThreadsInterface.GetThreadNameByIndex(i);
				if (!threadName)
					threadName = "UNKNOWN";
...
{
	static tukk szTestResults = "%USER%/TestResults";
...
static void SaveProfileSessionToXML(CBootProfilerSession* pSession, const float funcMinTimeThreshold)
{
	...
	gEnv->pDrxPak->AdjustFileName(filePath.c_str(), path, IDrxPak::FLAGS_PATH_REAL | IDrxPak::FLAGS_FOR_WRITING);
	gEnv->pDrxPak->MakeDir(szTestResults);
	...
	char buf[512];
	u32k buf_size = sizeof(buf);

	drx_sprintf(buf, buf_size, "<root>\n");
	fprintf(pFile, "%s", buf);
	...
	for (size_t i = 0; i < numThreads; ++i)
	{
		...
		if (pRoot)
		{
			...
			drx_sprintf(buf, buf_size, "\t<thread name=\"%s\" totalTimeMS=\"%f\" startTime=\"%" PRIu64 "\" stopTime=\"%" PRIu64 "\" totalBlocks=\"%u\"> \n", threadName, time,
			...
			drx_sprintf(buf, buf_size, "\t</thread>\n");
			fprintf(pFile, "%s", buf);
		}
	}
...
// start session
void CBootProfiler::StartSession(tukk sessionName)
{
	...
	{
		DrxLogAlways("BootProfiler: failed to start session '%s' as another one is active '%s'", sessionName, m_pCurrentSession->GetName());
		return;
	}
...
CBootProfilerRecord* CBootProfiler::StartBlock(tukk name, tukk args, EProfileDescription type)
{
	if (CBootProfilerSession* pSession = m_pCurrentSession)
	{
		const threadID curThread = DrxGetCurrentThreadId();
		u32k threadIndex = gThreadsInterface.GetThreadIndexByID(curThread);
...
void CBootProfiler::StartFrame(tukk name)
{
	static i32 prev_CV_sys_bp_frames = CV_sys_bp_frames;
	...
	if (CV_sys_bp_frames_threshold != 0.0f)
	{
		if (m_pCurrentSession)
		{
			DrxLogAlways("BootProfiler: failed to start session 'frame_threshold' as another one is active '%s'", m_pCurrentSession->GetName());
		}
		else
		{
...
void CBootProfiler::StopFrame()
{
	...
	if (CV_sys_bp_frames_threshold > 0.0f || bDisablingThresholdMode)
	{
		...
		{
			i32k nextOffset = drx_random(0, CV_sys_bp_frames_sample_period_rnd);
			m_countdownToNextSaveSesssion = drxmath::clamp(CV_sys_bp_frames_sample_period + nextOffset, 0, std::numeric_limits<i32>::max());
		}
		...
		if ((m_pCurrentSession->GetTotalTime() >= CV_sys_bp_frames_threshold) && !bDisablingThresholdMode && bShouldCollectResults)
		{
			...
			static i32 frameNumber = 0;
			i32k frameNum = gEnv->pRenderer ? gEnv->pRenderer->GetFrameID(false) : ++frameNumber;

			DrxFixedStringT<32> sessionName;
...
void CBootProfiler::RegisterCVars()
{
	...
	{
		gEnv->pThreadUpr->SpawnThread(this, "BootProfiler");
	}
}
