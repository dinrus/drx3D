  ...
  bool inShareChildren)
{
	...
	if (m_patchXML)
	{
		...
		if (newNode)
		{
			...
			i32 numAttr = inOrig->getNumAttributes();

			for (i32 i = 0; i < numAttr; i++)
			{
				tukk pKey, * pValue;
...
void CXMLPatcher::PatchFail(
  tukk pInReason)
{
	DrxLogAlways("Failed to apply data patch for file '%s' - reason '%s'", m_pFileBeingPatched, pInReason);
}
  ...
  tukk pInFileToPatch)
{
	...
	if (m_patchXML)
	{
		for (i32 i = 0, m = m_patchXML->getChildCount(); i < m; i++)
		{
			XmlNodeRef child = m_patchXML->getChild(i);

			if (child->isTag("patch"))
			{
				tukk pForFile = child->getAttr("forfile");

				if (pForFile && DrxStringUtils::stristr(pForFile, pInFileToPatch) != 0)
				{
					result = child;
					break;
				}
  ...
  const XmlNodeRef& inPatch)
{
	...
	for (i32 i = 0, m = inPatch->getChildCount(); i < m; i++)
	{
		...
		i32 maxChildren = result->getChildCount();
  ...
  tukk pInXMLFileName)
{
	...
	if (m_patchXML)
	{
		...
		if (patchForFile)
		{
			...
			DrxLog("Applying game data patch to %s", pInXMLFileName);
	...
	if (inA != inB)
	{
		...
		if (stricmp(inA->getTag(), inB->getTag()) == 0)
		{
			if (inA->getNumAttributes() == inB->getNumAttributes())
			{
				...
				for (i32 i = 0, m = inA->getNumAttributes(); i < m; i++)
				{
					tukk pAKey, * pBKey;
					tukk pAValue, * pBValue;
  ...
  DrxFixedStringT<512>* ioTempString)
{
	...
	inIndent = min(inIndent, i32(sizeof(k_lotsOfTabs) - 1));
	...
	for (i32 i = 0, m = inNode->getNumAttributes(); i < m; i++)
	{
		tukk pKey, * pVal;
	...
	for (i32 i = 0, m = inNode->getChildCount(); i < m; i++)
	{
  ...
  const XmlNodeRef& inAfter)
{
	...
	{
		DrxLog("Dumping before and after data files for '%s'", pInXMLFileName);
		...
		if (pOrigFileName = strrchr(pInXMLFileName, '/'))
		{
			...
			DrxFixedStringT<128> newFileName(pOrigFileName);
		...
		{
			DrxLog("Couldn't determine file name for path '%s' can't output diffs", pInXMLFileName);
		}
	}
}
  ...
  const XmlNodeRef& inNode)
{
	...
	IDrxPak* pIPak = GetISystem()->GetIPak();
	FILE* pFile = pIPak->FOpen(pInFilePath, "wb");

	if (pFile)
	{
		DrxFixedStringT<512> tempStr;
