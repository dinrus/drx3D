...
// -------------------------------------------------------------------------
//  Имя файла:   SystemScheduler.cpp
//  Version:     v1.00
//  Created:     18:11:2010 by Sergey Rustamov.
//  Описание: Implementation of the CSystemScheduler class
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
extern "C" void SliceAndSleep(tukk pFunc, i32 line)
{
...
void CSystemScheduler::SliceAndSleep(tukk sliceName, i32 line)
{
	#if defined(MAP_LOADING_SLICING)
	if (!gEnv->IsDedicated())
		return;
	...
	if (doSleep)
	{
		if (m_svSliceLoadLogging->GetIVal())
		{
			...
			if (diff > sliceBudget)
				DrxLogAlways("[SliceAndSleep]: Interval between slice [%s:%i] and [%s:%i] was [%f] out of budget [%f]", m_pLastSliceName, m_lastSliceLine, sliceName, line, diff, sliceBudget);
		}
...
void CSystemScheduler::SchedulingModeUpdate()
{
	...
	if (i32 scheduling = m_svSchedulingMode->GetIVal())
	{
