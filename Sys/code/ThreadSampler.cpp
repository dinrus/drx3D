...
#include <drx3D/Sys/ThreadSampler.h>
...
i32k FIRST_IOCTL_INDEX = 0x800;
i32k FILE_DEVICE_myDrv = 0x00008000;

	#ifndef METHOD_BUFFERED
i32k METHOD_BUFFERED = 0;
	#endif

	#ifndef FILE_ANY_ACCESS
i32k FILE_ANY_ACCESS = 0;
	#endif
...
void CWinThreadSampler::LoadDriver()
{
	...
	sch = OpenSCUpr(NULL, NULL, SC_MANAGER_ALL_ACCESS);
	...
	if (schService != 0)
	{
		...
		if (hRes != S_OK)
		{
			...
			DrxLog("Failed to load sampling driver");
		}
...
//============================================================
void CWinThreadSampler::GetDriverFileName(tuk fname)
{
	tuk dummy;
...
void CWinThreadSampler::Tick()
{
	...
	m_snapshotInfo.m_procNumCtxtSwitches[0] = (i32)csw;
...
//============================================================
void CWinThreadSampler::SetThreadName(tukk name)
{
...
//============================================================
void CWinThreadSampler::SetThreadNameEx(DWORD threadId, tukk name)
{
	/*
...
//============================================================
void CWinThreadSampler::CreateSpanListForThread(TProcessID processId, threadID threadId, TTDSpanList& spanList, u32 width, u32 scale, u32* totalTime,
                                                i32* ProcessorId, u32* color)
{
...
//============================================================
//tukk CWinThreadSampler::GetThreadName()
//============================================================
tukk CWinThreadSampler::GetThreadName(threadID threadId)
{
	if (tukk threadName = gEnv->pThreadUpr->GetThreadName(threadId))
		return threadName;
...
void CWinThreadSampler::EnumerateProcesses()
{
	...
	if (Process32First(hSnapshoot, &pe32))
	{
		do
		{
			...
			drx_strcpy(item.exeName, pe32.szExeFile);
...
void CWinThreadSampler::MergeCPUData(TSamplesCPU* CPUData)
{
	...
	qsort((uk )&samples.driverData.samples[0], DRIVERSAMPLESCOUNT, sizeof(TSample), sort_function);
}
	#endif // TIMESAMPLER
