...
void CMiniInfoBox::OnPaint(CDrawContext& dc)
{
	...
	// Draw entries.
	for (i32 i = 0, num = (i32)m_entries.size(); i < num; i++)
	{
...
void CMiniInfoBox::AddEntry(tukk str, ColorB col, float textSize)
{
	SInfoEntry info;

	info.color = col;
	info.textSize = textSize;
	drx_strcpy(info.text, str);
...
void CMiniInfoBox::AutoResize()
{
	...
	for (i32 i = 0, num = (i32)m_entries.size(); i < num; i++)
	{
		...
		u32 strLength = strlen(info.text);
