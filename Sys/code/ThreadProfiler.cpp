...
//  Описание: Rendering of ThreadProfile.
// -------------------------------------------------------------------------
//
////////////////////////////////////////////////////////////////////////////
	...
	, m_pSampler(nullptr)
{
	...
	REGISTER_CVAR(profile_threads, 0, 0,
	              ...
	              "The C++ function DrxThreadSetName(threadid,\"Name\") needs to be called on thread creation.\n"
...
static void RenderThreadSpan(SThreadProfilerRenderInfo& ri)
{
	...
	for (i32 i = 0; i < (i32)spans.size(); i++)
	{
...
void CThreadProfiler::Render()
{
	...
	i32 width = gEnv->pRenderer->GetWidth();
	i32 height = gEnv->pRenderer->GetHeight();
	...
	i32 nContextSwitchesTotal = 0;
	for (u32 cpu = 0; cpu < snapshotInfo->m_procNumCtxtSwitches.size(); cpu++)
	{
	...
	i32 numThreads = m_pSampler->GetNumThreads();
	...
	i32 nShowThreads = numThreads;
	#if DRX_PLATFORM_WINDOWS
	nShowThreads += 1;
	#endif

	m_nDisplayedThreads = 0;
	for (i32 ti = 0; ti < nShowThreads; ti++)
	{
		threadID threadId = -1;
		if (ti < numThreads)
		{
		...
		u32 span_color = 0;
		spans.resize(0);
		m_pSampler->CreateSpanListForThread(processId, threadId, spans, (u32)(width - graph_offset), 2, &totalTime, &processor, &span_color);
		...
		if (threadId != -1)
		{
			...
			drx_strcpy(str, m_pSampler->GetThreadName(threadId));
			if (str[0] == 0)
				drx_sprintf(str, "  * %x", threadId);
			ri.sThreadName = str;
		}
		else
		{
