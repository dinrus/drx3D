...
//////////////////////////////////////////////////////////////////////////
		#if defined(INCLUDE_SCALEFORM_SDK) && !DRX_PLATFORM_ORBIS
...
public:
	...
	virtual bool Lock(u32 offset, u32 length, LockRange& lr)
	{
...
public:
	...
	virtual ISoundDelegate* CreateSound(u32 numChannels, u32 sampleRate, u32 lengthInBytes)
	{
		return NULL;
		/*if (!m_pFMOD)
		   return 0;
...
//DrxVideoSoundSystem::IPlayerDelegate* CSoundSystem::CreateVideoSoundSystem(DrxVideoSoundSystem::IAllocatorDelegate* pAllocator) const
//{
//	FMOD::System* pFMOD = (FMOD::System*) (m_pAudioDevice ? m_pAudioDevice->GetSoundLibrary() : 0);
//	return pFMOD && pAllocator ? DrxVideoSoundSystem::PlayerDelegate::CreateInstance(pAllocator, pFMOD) : 0;
//}

		#endif // #if defined(INCLUDE_SCALEFORM_SDK) && !DRX_PLATFORM_ORBIS

using namespace DrxVideoSoundSystem;

class GFxVideoDrxSoundSystemImpl;
class GFxVideoDrxSound;
...
class GFxVideoDrxSoundSystemImpl : public GNewOverrideBase<GStat_Video_Mem>
{
public:
	GFxVideoDrxSoundSystemImpl(GMemoryHeap* pHeap);
	~GFxVideoDrxSoundSystemImpl();

	void   AttachSound(GFxVideoDrxSound* pSound);
	void   DetachSound(GFxVideoDrxSound* pSound);
...
private:
	static u32k UpdateThreadStackSize = 16 * 1024;

	struct AllocatorDelegate : public IAllocatorDelegate
	{
	public:
		uk Allocate(size_t size)
		{
			++m_numAllocs;
...
private:
	static i32 UpdateFunc(GThread*, uk h);
	float      UpdateAuxStreams();

private:
	GMemoryHeap*                m_pHeap;
	GArrayDH<GFxVideoDrxSound*> m_sounds;
	GLock                       m_soundsLock;
...
class GFxVideoDrxSound : public GFxVideoSound
{
public:
...
public:
	GFxVideoDrxSound(GMemoryHeap* pHeap, GFxVideoDrxSoundSystemImpl* pSoundSystem, GFxVideoPlayer::SoundTrack type);
	virtual ~GFxVideoDrxSound();
...
private:
	...
	static u32k AuxSoundReadBufLenMs = 300;
	static u32k AuxSoundLenMs = AuxSoundReadBufLenMs * 15 / 10; // = AuxSoundReadBufLenMs * 5

private:
	uint64       GetTotalBytesReadInTicks() const          { return BytesToTicks(m_totalBytesRead); }
	uint64       BytesToTicks(u32 bytes) const    { return uint64(VideoSoundTimer::TicksPerSecond()) * bytes / (m_sampleRate * m_numChannels * sizeof(SoundSampleType)); }
	u32 DistToFillBuffPos(u32 pos) const { return (pos > m_fillPosition) ? (m_fillPosition + m_soundLength) - pos : m_fillPosition - pos; }

	u32 ReadAndFillSound(bool fillup);
...
private:
	GMemoryHeap*                m_pHeap;
	GFxVideoDrxSoundSystemImpl* m_pSoundSystem;
...
GFxVideoDrxSoundSystemImpl::GFxVideoDrxSoundSystemImpl(GMemoryHeap* pHeap)
	: m_pHeap(pHeap)
	, m_sounds(pHeap)
	, m_soundsLock()
	, m_pPlayerDelegate(0)
...
GFxVideoDrxSoundSystemImpl::~GFxVideoDrxSoundSystemImpl()
{
...
uint64 GFxVideoDrxSoundSystemImpl::GetSafeStartTick() const
{
	return VideoSoundTimer::GetTick();
}

void GFxVideoDrxSoundSystemImpl::AttachSound(GFxVideoDrxSound* pSound)
{
	tukk threadName = "DrxGFxVideo_SoundUpdate";
	assert(pSound);
	...
	if (!m_pUpdateThread)
	{
		...
		if (pThreadConfig == pThreadConfigMngr->GetDefaultThreadConfig())
			DrxGFxLog::GetAccess().LogWarning("Unable to find thread config for '%s' . Using global default config.", threadName);
...
void GFxVideoDrxSoundSystemImpl::DetachSound(GFxVideoDrxSound* pSound)
{
	assert(pSound);
	GLock::Locker lock(&m_soundsLock);
...
i32 GFxVideoDrxSoundSystemImpl::UpdateFunc(GThread*, uk h)
{
	GFxVideoDrxSoundSystemImpl* pThis = (GFxVideoDrxSoundSystemImpl*) h;
	u32 waitTime = 2000; // initial wait will be interrupted by pulsed event sent by first AttachSound() call, i.e. won't cause issue with timer wrap around detection on Xbox 360
	while (true)
	{
		pThis->m_event.Wait(waitTime);
		if (pThis->m_stopThread)
			break;
		waitTime = (u32) (pThis->UpdateAuxStreams() * 1000);
	}
	return 0;
}

float GFxVideoDrxSoundSystemImpl::UpdateAuxStreams()
{
	GLock::Locker lock(&m_soundsLock);
...
GFxVideoDrxSound::GFxVideoDrxSound(GMemoryHeap* pHeap, GFxVideoDrxSoundSystemImpl* pSoundSystem, GFxVideoPlayer::SoundTrack type)
	: GFxVideoSound()
	, m_pHeap(pHeap)
...
GFxVideoDrxSound::~GFxVideoDrxSound()
{
}

void GFxVideoDrxSound::ReleaseBuffers()
{
...
void GFxVideoDrxSound::UpdateVolume(bool muteMainTrack)
{
...
float GFxVideoDrxSound::Update(bool muteMainTrack)
{
	...
	if (distToRefill < m_bufferSize / 3)
	{
		...
		u32k recvBytes = ReadAndFillSound(true); // m_starved; // always refill to inject zeros to reduce clicks should we starve in the near future
...
bool GFxVideoDrxSound::CreateOutput(UInt32 numChannels, UInt32 sampleRate)
{
	...
	m_pInterleavedBuffer = (u8*) GHEAP_ALLOC(m_pHeap, m_bufferSize, GStat_Video_Mem);
	if (!m_pInterleavedBuffer)
	{
		ReleaseBuffers();
		return false;
	...
	if (!m_pSound)
	{
		DrxGFxLog::GetAccess().LogError("Failed to create video sound output. Video will not play! Is the dummy sound system active?");
		ReleaseBuffers();
		return false;
	}
...
void GFxVideoDrxSound::DestroyOutput()
{
	GLock::Locker lock(&m_lock);
...
void GFxVideoDrxSound::Start(PCMStream* pStream)
{
	{
		GLock::Locker lock(&m_lock);
...
void GFxVideoDrxSound::Pause(bool pause)
{
	GLock::Locker lock(&m_lock);
...
void GFxVideoDrxSound::GetTime(UInt64* pCount, UInt64* pUnit) const
{
	GLock::Locker lock(&m_lock);
...
void GFxVideoDrxSound::ClearSoundBuffer()
{
	assert(m_pSound);
...
u32 GFxVideoDrxSound::ReadAndFillSound(bool fillup)
{
	assert(m_pSound);
	assert(m_pInterleavedBuffer);

	u32k recvBytes = GetPCMData();
	if (fillup && recvBytes < m_bufferSize)
		GMemUtil::Set(&m_pInterleavedBuffer[recvBytes], 0, m_bufferSize - recvBytes);

	u32k copySize = !fillup ? recvBytes : m_bufferSize;
	if (copySize > 0)
	{
		ISoundDelegate::LockRange lr = { 0 };
...
u32 GFxVideoDrxSound::GetPCMData()
{
	u32k numChannels = m_numChannels;
	u32k recvNumSamples = m_pPCMStream->GetDataSInt16(numChannels, m_pChannelBuffer, m_bufferSamples);

	static u32k chMap[MaxChannels] = { 0, 1, 4, 5, 2, 3, 6, 7 };
	...
	for (u32 ch = 0; ch < numChannels; ++ch)
	{
		...
		for (u32 i = 0; i < recvNumSamples; ++i, ++pSrc, pDst += numChannels)
			*pDst = *pSrc;
	}
...
//////////////////////////////////////////////////////////////////////////
// GFxVideoDrxSoundSystem

GFxVideoDrxSoundSystem::GFxVideoDrxSoundSystem(GMemoryHeap* pHeap)
	: GFxVideoSoundSystem(pHeap)
	, m_pImpl(GHEAP_NEW(GetHeap()) GFxVideoDrxSoundSystemImpl(GetHeap()))
{
}

GFxVideoDrxSoundSystem::~GFxVideoDrxSoundSystem()
{
	delete m_pImpl;
}

GFxVideoSound* GFxVideoDrxSoundSystem::Create(GFxVideoPlayer::SoundTrack type)
{
	VideoSoundTimer::InitFrequency();
	return m_pImpl ? (GHEAP_NEW(GetHeap()) GFxVideoDrxSound(GetHeap(), m_pImpl, type)) : 0;
}

	#endif // #if defined(USE_GFX_VIDEO)

void GFxVideoDrxSoundSystem::InitCVars()
{
	#if defined(_DEBUG)
	{
