...
{
public:
	COverloadDG(COverloadSceneUpr* pOSM) : m_pOSM(pOSM) {}

	virtual SDescription GetDescription() const
	{
		return SDescription('L', "overload scene manager", "['/Overload/' (i32 enabled) (float targetFrameRate) (float fbScale)"
...
// Desc: Constructor
//--------------------------------------------------------------------------------------------------
COverloadSceneUpr::COverloadSceneUpr()
{
	InitialiseCVars();
...
static void cmd_setFBScale(IConsoleCmdArgs* pParams)
{
	...
	i32 nWidth  = gEnv->pRenderer->GetWidth();
	i32 nHeight = gEnv->pRenderer->GetHeight();

	gEnv->pLog->LogWithType(ILog::eInputResponse, "Current Viewport Resolution: %dx%d",
	                        i32(nWidth * newScale.x), i32(nHeight * newScale.y));
}
...
//--------------------------------------------------------------------------------------------------
void COverloadSceneUpr::InitialiseCVars()
{
#if DEBUG_OVERLOAD_SCENE_MANAGER
...
//--------------------------------------------------------------------------------------------------
void COverloadSceneUpr::Reset()
{
	gEnv->pStatoscope->AddUserMarker("Overload", "OverloadSceneUpr::Reset()");
...
void COverloadSceneUpr::ResetDefaultValues()
{
	m_fbScale = 1.0f;
	...
	for (i32 i = 0; i < osm_historyLength; i++)
	{
...
//       overrides.
//--------------------------------------------------------------------------------------------------
void COverloadSceneUpr::OverrideScale(float frameScale, float dt)
{
	gEnv->pStatoscope->AddUserMarker("Overload", "OverloadSceneUpr::OverrideScale()");
...
void COverloadSceneUpr::ResetScale(float dt)
{
	gEnv->pStatoscope->AddUserMarker("Overload", "OverloadSceneUpr::ResetScale()");
...
//--------------------------------------------------------------------------------------------------
void COverloadSceneUpr::UpdateStats()
{
	m_currentFrameStat++;
	...
	{
		float curTime = gEnv->pTimer->GetCurrTime();
		currentStats.gpuFrameRate = 25.0f + sinf(curTime) * drx_random(0.0f, 10.0f);
	}
#endif
...
void COverloadSceneUpr::CalculateSmoothedStats()
{
...
float COverloadSceneUpr::CalcFBScale()
{
	float curTime = gEnv->pTimer->GetCurrTime();
...
void COverloadSceneUpr::ResizeFB()
{
	...
	if (osm_stress == 2)
	{
		m_fbAutoScale = drx_random(osm_fbMinScale, 1.0f);
	}
#endif
...
//--------------------------------------------------------------------------------------------------
void COverloadSceneUpr::DebugDrawDisplay()
{
	if (osm_debug)
	{
		// Display title
		...
		IRenderAuxText::Draw2dLabel(textPos.x, textPos.y, textSize, &textCol.r, false, "Overload Scene Upr debug view");
...
//--------------------------------------------------------------------------------------------------
void COverloadSceneUpr::DebugDrawGraphs()
{
	if (m_pDebugHistoryUpr == NULL)
	{
		...
		m_pDebugHistory = m_pDebugHistoryUpr->CreateHistory("vehicle-graph", "");
