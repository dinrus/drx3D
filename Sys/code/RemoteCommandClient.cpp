...
CRemoteCommandClient::Command* CRemoteCommandClient::Command::Compile(const IRemoteCommand& cmd, u32k commandId, u32k classId)
{
	...
	u32k commandDataStart = writer.GetSize();
	cmd.SaveToStream(writer);
	u32k commandDataEnd = writer.GetSize();
	...
	// TODO: consider writing the size directly
	{
		...
		u32k dataSize = commandDataEnd - commandDataStart;
...
CRemoteCommandClient::Connection::Connection(CRemoteCommandUpr* pUpr, IServiceNetworkConnection* pConnection, u32 currentCommandId)
...
void CRemoteCommandClient::Connection::AddToSendQueue(Command* pCommand)
{
	...
	{
		DrxAutoLock<DrxMutex> lock(m_commandAccessMutex);
...
bool CRemoteCommandClient::Connection::Update()
{
	...
	u32 newLastExecutedCommand = m_lastExecutedCommand;
	u32 newLastReceivedCommand = m_lastReceivedCommand;
		...
		// Deserialize the message
		{
			...
			if (response.magic == PackedHeader::kMagic)
			{
				if (response.msgType == PackedHeader::eCommand_ACK)
				{
					...
					newLastExecutedCommand = max<u32>(newLastExecutedCommand, response.lastCommandExecuted);
					newLastReceivedCommand = max<u32>(newLastReceivedCommand, response.lastCommandReceived);
	...
	// (Re)Send the commands
	{
		...
		u32k maxCommandIdToSend = m_lastExecutedCommand + kCommandSendLead;
		...
		for (;; )
		{
			...
			u32 packetDataSizeSoFar = 0;
			{
				     ...
				     it != m_pCommands.end(); ++it)
				{
					...
					if (commandRef->m_lastSentTime < cutoffTime)
					{
						...
						u32k commandDataSize = commandRef->m_pCommand->GetMessage()->GetSize();
			...
			u32k messageDataSize = packetDataSizeSoFar + PackedHeader::kSerializationSize;
...
CRemoteCommandClient::CRemoteCommandClient(CRemoteCommandUpr* pUpr)
	: m_pUpr(pUpr)
	, m_commandId(0)
	, m_bCloseThread(false)
{
	if (!gEnv->pThreadUpr->SpawnThread(this, "RemoteCommandClient"))
	{
		DrxFatalError("Error spawning \"RemoteCommandClient\" thread.");
	}
}
...
IRemoteCommandConnection* CRemoteCommandClient::ConnectToServer(const class ServiceNetworkAddress& serverAddress)
{
	...
	u32k firstCommandId = m_commandId;

	// Create a wrapping class and add it to the connection list
	Connection* pConnection = new Connection(GetUpr(), pNetConnection, firstCommandId);
...
bool CRemoteCommandClient::Schedule(const IRemoteCommand& command)
{
	...
	u32 classId = 0;
	if (!GetUpr()->FindClassId(command.GetClass(), classId))
	{
	...
	u32k commandId = DrxInterlockedIncrement(( i32*) &m_commandId);
