...
void CManualFrameStepController::GenerateRequest(u8k numFrames)
{
	if (gEnv->bServer)
	{
...
void CManualFrameStepController::ProcessCommand(u8k numFrames)
{
	switch (numFrames)
	{
	case kDisableMask:
...
void CManualFrameStepController::NetSyncClients(const SNetMessage& netMessage)
{
	if (gEnv->bMultiplayer && gEnv->bServer)
	{
		if (IGameServerNub* pGameServerNub = gEnv->pGameFramework->GetIGameServerNub())
		{
			...
			DrxLog("[ManualFrameStepController] Client Sync sent. Frames = %u", netMessage.numFrames);
		}
	}
}
	...
	case eKI_Left:
		{
			...
			if (IsEnabled() && eIS_Pressed == inputEvent.state)
			{
				...
				tukk szFileFormat = pCaptureFormat ? pCaptureFormat->GetString() : "jpg";

				const ICVar* const pUserPrefix = gEnv->pConsole->GetCVar("capture_file_prefix");
				tukk szUserPrefix = pUserPrefix ? pUserPrefix->GetString() : "";
				tukk szRealPrefix = szUserPrefix[0] ? szUserPrefix : "Frame";
				...
				tukk szAdjustedFramesFolder = gEnv->pDrxPak->AdjustFileName(framesFolder.c_str(), buffer, IDrxPak::FLAGS_FOR_WRITING);

				char szBuffer[MAX_PATH + 256];
				drx_sprintf(szBuffer, "explorer.exe %s\\%s", szAdjustedFramesFolder, lastFrameName.c_str());
