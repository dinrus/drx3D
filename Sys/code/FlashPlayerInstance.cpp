...
// flash player implementation via Scaleform's GFx
	...
	#include <drx3D/Sys/GTexture_Impl.h>
		...
		#include <drx3D/Sys/GFxAmpServer.h>
	#endif

	#include <drx3D/CoreX/String/StringUtils.h>              // stristr()

	#if defined(ENABLE_FLASH_INFO)
static i32 ms_sys_flash_info = 0;
	#endif

	#if DRX_PLATFORM_WINDOWS || DRX_PLATFORM_ORBIS || DRX_PLATFORM_DURANGO || DRX_PLATFORM_MAC || DRX_PLATFORM_LINUX
...
struct SFlashPeak
{
	SFlashPeak(float timeRecorded, tukk pDisplayInfo)
...
inline static tukk GetFlashFunctionName(EFlashFunctionID functionID)
{
	switch (functionID)
	{
	case eFncAdvance:
...
template<typename T, u32 HistogramSize>
struct SFlashProfilerHistogram
{
	SFlashProfilerHistogram()
		: m_curIdx(0)
	{
		for (u32 i(0); i < HistogramSize; ++i)
			m_hist[i] = 0;
	}
...
struct SFlashProfilerData
{
	...
	typedef SFlashProfilerHistogram<u32, HistogramSize> TotalCallHistogram;
	typedef SFlashProfilerHistogram<float, HistogramSize>        TotalTimeHistogram;
	typedef SFlashProfilerHistogram<u32, HistogramSize> DrawCallHistogram;
	typedef SFlashProfilerHistogram<u32, HistogramSize> TriCountHistogram;
	typedef SFlashProfilerHistogram<u32, HistogramSize> LocklessDisparityHistogram;
	...
	void UpdateDrawStats(u32 idx, bool write)
	{
		if (write)
		{
	...
	void AddTotalCalls(EFlashFunctionID functionID, u32 amount)
	{
		if (!ms_histWriteProtected)
	...
	void AddDrawCalls(u32 drawCalls, u32 idx)
	{
		m_curDrawCalls[idx] += drawCalls;
	}

	void AddTriCount(u32 triCount, u32 idx)
	{
...
public:
	...
	void Enable(bool enabled, i32 curFrame)
	{
		if (!enabled && m_file)
		{
			fclose(m_file);
		...
		{
			char frameMsg[512];
			drx_sprintf(frameMsg, ">>>>>>>> Frame: %.10d <<<<<<<<", curFrame);
			fwrite("\n", 1, 1, m_file);
...
{
public:
	CFlashFunctionProfiler(float peakTolerance, EFlashFunctionID functionID, i32 numFuncArgs, const SPODVariant* pFuncArgs,
	                       tukk pCustomFuncName, SFlashProfilerData* pProfilerData, tukk pFlashFilePath)
		: m_funcTime(0)
...
private:
	void FormatFlashVarValue(const SFlashVarValue* pArg, tuk pBuf, size_t sizeBuf) const
	{
		switch (pArg->GetType())
		{
		case SFlashVarValue::eBool:
			drx_sprintf(pBuf, sizeBuf, "%s", pArg->GetBool() ? "true" : "false");
			break;
		case SFlashVarValue::eInt:
			drx_sprintf(pBuf, sizeBuf, "%d", pArg->GetInt());
			break;
		case SFlashVarValue::eUInt:
			drx_sprintf(pBuf, sizeBuf, "%u", pArg->GetUInt());
			break;
		case SFlashVarValue::eDouble:
			drx_sprintf(pBuf, sizeBuf, "%lf", pArg->GetDouble());
			break;
		case SFlashVarValue::eFloat:
			drx_sprintf(pBuf, sizeBuf, "%f", pArg->GetFloat());
			break;
		case SFlashVarValue::eConstStrPtr:
			drx_sprintf(pBuf, sizeBuf, "\"%s\"", pArg->GetConstStrPtr());
			break;
		case SFlashVarValue::eConstWstrPtr:
			drx_sprintf(pBuf, sizeBuf, "\"%ls\"", pArg->GetConstWstrPtr());
			break;
		case SFlashVarValue::eNull:
			drx_sprintf(pBuf, sizeBuf, "null");
			break;
		case SFlashVarValue::eObject:
			drx_sprintf(pBuf, sizeBuf, "obj");
			break;
		case SFlashVarValue::eUndefined:
			drx_sprintf(pBuf, sizeBuf, "???");
			break;
		default:
			assert(0);
			drx_sprintf(pBuf, sizeBuf, "???");
	...
	void FormatMsg(float funcTimeMs, tuk msgBuf, i32 sizeMsgBuf) const
	{
		char funcArgList[1024];
		drx_strcpy(funcArgList, "");

		for (i32 i = 0; i < m_numFuncArgs; ++i)
		{
			char curArg[512];
			switch (m_pFuncArgs[i].type)
			{
			case SPODVariant::eBool:
				drx_sprintf(curArg, "%s", m_pFuncArgs[i].data.b ? "true" : "false");
				break;
			case SPODVariant::eInt:
				drx_sprintf(curArg, "%d", m_pFuncArgs[i].data.i);
				break;
			case SPODVariant::eUInt:
				drx_sprintf(curArg, "%u", m_pFuncArgs[i].data.ui);
				break;
			case SPODVariant::eDouble:
				drx_sprintf(curArg, "%lf", m_pFuncArgs[i].data.d);
				break;
			case SPODVariant::eFloat:
				drx_sprintf(curArg, "%f", m_pFuncArgs[i].data.f);
				break;
			case SPODVariant::eConstStrPtr:
				drx_sprintf(curArg, "\"%s\"", m_pFuncArgs[i].data.pStr);
				break;
			case SPODVariant::eConstWstrPtr:
				drx_sprintf(curArg, "\"%ls\"", m_pFuncArgs[i].data.pWstr);
				break;
			case SPODVariant::eConstVoidPtr:
				drx_sprintf(curArg, "0x%p", m_pFuncArgs[i].data.pVoid);
				break;
			case SPODVariant::eFlashVariableArrayType:
				{
					tukk pFlashVarTypeName("FVAT_???");
					...
					drx_sprintf(curArg, "%s", pFlashVarTypeName);
					break;
				}
			...
			case SPODVariant::eFlashVarValueList:
				{
					...
					u32k numArgs = m_pFuncArgs[i].data.fvvl.numArgs;
					for (u32 n = 0; n < numArgs; ++n)
					{
						char tmpArg[512];
						FormatFlashVarValue(&pArgs[n], tmpArg, sizeof(tmpArg));

						drx_strcat(curArg, tmpArg);
			...
			drx_strcat(funcArgList, curArg);

			if (i < m_numFuncArgs - 1)
			{
				drx_strcat(funcArgList, ", ");
			}
		}

		drx_sprintf(msgBuf, sizeMsgBuf, "%.2f %s(%s) - %s", funcTimeMs, m_pCustomFuncName, funcArgList, m_pFlashFilePath);
	}
...
private:
	static u32 ms_peakFuncExcludeMask;
...
private:
	...
	tukk         m_pCustomFuncName;
	SFlashProfilerData* m_pProfilerData;
	tukk         m_pFlashFilePath;
};

u32 CFlashFunctionProfiler::ms_peakFuncExcludeMask(0);
...
public:
	...
	{
		DRX_PROFILE_PUSH_MARKER("Flash");
	}

	~CFlashFunctionProfilerLight()
	{
		u32 delta = (u32) (FlashTimer::GetTicks() - m_startTick); // 32 bit should be enough for tick delta

		#if defined(FPL_THREAD_SAFE_COUNTING)
		while (true)
		{
			u32k curAccum = ms_deltaTicksAccum;
			IF (curAccum == (u32) DrxInterlockedCompareExchange(alias_cast< LONG*>(&ms_deltaTicksAccum), (LONG) (curAccum + delta), (LONG) curAccum), 1)
				break;
		}
		#else
		ms_deltaTicksAccum += delta;
		#endif
		DRX_PROFILE_POP_MARKER("Flash");
	}
	...
	static void Update()
	{
		...
		{
			u32k curAccum = ms_deltaTicksAccum;
			IF (curAccum == (u32) DrxInterlockedCompareExchange(alias_cast< LONG*>(&ms_deltaTicksAccum), 0, (LONG) curAccum), 1)
				break;
		}
		#else
...
private:
	static  u32 ms_deltaTicksAccum;
	static u32k    HIST_SIZE = 32;
	static u32          ms_histIdx;
...
private:
 ...
 u32 CFlashFunctionProfilerLight::ms_deltaTicksAccum = 0;
u32 CFlashFunctionProfilerLight::ms_histIdx = 0;
...
{
public:
	CFlashLogContext(DrxGFxLog* pLog, tukk pFlashContext)
		: m_pLog(pLog)
		, m_pPrevLogContext(0)
	{
...
private:
	DrxGFxLog*  m_pLog;
	tukk m_pPrevLogContext;
};

		#define SET_LOG_CONTEXT(filePath)      CFlashLogContext logContext(&DrxGFxLog::GetAccess(), filePath->c_str());
		#define SET_LOG_CONTEXT_CSTR(filePath) CFlashLogContext logContext(&DrxGFxLog::GetAccess(), filePath);
	...
	#define SYNC_THREADS DrxAutoCriticalSection theLock(*m_lock.get());
...
static DrxCriticalSection s_displaySync;
...
public:
	...
	virtual bool                  HasMember(tukk pMemberName) const;
	virtual bool                  SetMember(tukk pMemberName, const SFlashVarValue& value);
	virtual bool                  SetMember(tukk pMemberName, const IFlashVariableObject* pVarObj);
	virtual bool                  GetMember(tukk pMemberName, SFlashVarValue& value) const;
	virtual bool                  GetMember(tukk pMemberName, IFlashVariableObject*& pVarObj) const;
	virtual void                  VisitMembers(ObjectVisitor* pVisitor) const;
	virtual bool                  DeleteMember(tukk pMemberName);
	virtual bool                  Invoke(tukk pMethodName, const SFlashVarValue* pArgs, u32 numArgs, SFlashVarValue* pResult = 0);
	virtual bool                  Invoke(tukk szMethodName, const IFlashVariableObject** pArgs, u32 numArgs, SFlashVarValue* pResult = 0);

	virtual u32          GetArraySize() const;
	virtual bool                  SetArraySize(u32 size);
	virtual bool                  SetElement(u32 idx, const SFlashVarValue& value);
	virtual bool                  SetElement(u32 idx, const IFlashVariableObject* pVarObj);
	virtual bool                  GetElement(u32 idx, SFlashVarValue& value) const;
	virtual bool                  GetElement(u32 idx, IFlashVariableObject*& pVarObj) const;
	...
	virtual bool                  RemoveElements(u32 idx, i32 count = -1);
	...
	virtual bool                  SetText(tukk pText);
	virtual bool                  SetText(const wchar_t* pText);
	virtual bool                  SetTextHTML(tukk pHtml);
	...
	virtual bool                  CreateEmptyMovieClip(IFlashVariableObject*& pVarObjMC, tukk pInstanceName, i32 depth = -1);
	virtual bool                  AttachMovie(IFlashVariableObject*& pVarObjMC, tukk pSymbolName, tukk pInstanceName, i32 depth = -1, const IFlashVariableObject* pInitObj = 0);
	virtual bool                  GotoAndPlay(tukk pFrame);
	virtual bool                  GotoAndStop(tukk pFrame);
	virtual bool                  GotoAndPlay(u32 frame);
	virtual bool                  GotoAndStop(u32 frame);
	virtual bool                  SetVisible(bool visible);

public:
	CFlashVariableObject(const GFxValue& value, const stringConstPtr& refFilePath, const DrxCriticalSectionPtr& lock);
...
CFlashVariableObject::CFlashVariableObject(const GFxValue& value, const stringConstPtr& refFilePath, const DrxCriticalSectionPtr& lock)
	: m_value(value)
...
bool CFlashVariableObject::HasMember(tukk pMemberName) const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::SetMember(tukk pMemberName, const SFlashVarValue& value)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::SetMember(tukk pMemberName, const IFlashVariableObject* pVarObj)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::GetMember(tukk pMemberName, SFlashVarValue& value) const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::GetMember(tukk pMemberName, IFlashVariableObject*& pVarObj) const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::Invoke(tukk pMethodName, const SFlashVarValue* pArgs, u32 numArgs, SFlashVarValue* pResult)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::Invoke(tukk szMethodName, const IFlashVariableObject** pArgs, u32 numArgs, SFlashVarValue* pResult)
{
	FLASH_PROFILER_LIGHT;
...
void CFlashVariableObject::VisitMembers(ObjectVisitor* pVisitor) const
{
	...
	if (m_value.IsObject() && pVisitor)
	{
		struct VisitorAdaptor : public GFxValue::ObjectVisitor
		{
			...
			virtual void Visit(tukk name, const GFxValue& val)
			{
...
bool CFlashVariableObject::DeleteMember(tukk pMemberName)
{
	FLASH_PROFILER_LIGHT;
...
u32 CFlashVariableObject::GetArraySize() const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::SetArraySize(u32 size)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::SetElement(u32 idx, const SFlashVarValue& value)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::SetElement(u32 idx, const IFlashVariableObject* pVarObj)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::GetElement(u32 idx, SFlashVarValue& value) const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::GetElement(u32 idx, IFlashVariableObject*& pVarObj) const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::RemoveElements(u32 idx, i32 count)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::GetColorTransform(SFlashCxform& cx) const
{
	...
	if (m_value.IsDisplayObject())
	{
		...
		if (res)
		{
			...
			cx.add.r = static_cast<u8>(cxform.M_[0][1]);
			cx.mul.g = cxform.M_[1][0];
			cx.add.g = static_cast<u8>(cxform.M_[1][1]);
			cx.mul.b = cxform.M_[2][0];
			cx.add.b = static_cast<u8>(cxform.M_[2][1]);
			cx.mul.a = cxform.M_[3][0];
			cx.add.a = static_cast<u8>(cxform.M_[3][1]);
		}
	}
	return res;
}

bool CFlashVariableObject::SetText(tukk pText)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::SetTextHTML(tukk pHtml)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::CreateEmptyMovieClip(IFlashVariableObject*& pVarObjMC, tukk pInstanceName, i32 depth)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::AttachMovie(IFlashVariableObject*& pVarObjMC, tukk pSymbolName, tukk pInstanceName, i32 depth, const IFlashVariableObject* pInitObj)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::GotoAndPlay(tukk pFrame)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::GotoAndStop(tukk pFrame)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::GotoAndPlay(u32 frame)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashVariableObject::GotoAndStop(u32 frame)
{
	FLASH_PROFILER_LIGHT;
...
public:
	...
	virtual void          Release();
	virtual bool          Load(tukk pFilePath);
	...
	virtual std::shared_ptr<IFlashPlayer> CreatePlayerInstance(u32 options = IFlashPlayer::DEFAULT, u32 cat = IFlashPlayer::eCat_Default);
...
bool CFlashPlayerBootStrapper::Load(tukk pFilePath)
{
...
std::shared_ptr<IFlashPlayer> CFlashPlayerBootStrapper::CreatePlayerInstance(u32 options, u32 cat)
{
	return m_pMovieDef.GetPtr() != 0 ? CFlashPlayer::CreateBootstrapped(m_pMovieDef, options, cat) : 0;
}

tukk CFlashPlayerBootStrapper::GetFilePath() const
{
	return m_pMovieDef.GetPtr() != 0 ? m_pMovieDef->GetFileURL() : 0;
}

size_t CFlashPlayerBootStrapper::GetMetadata(tuk pBuff, u32 buffSize) const
{
	return m_pMovieDef.GetPtr() != 0 ? m_pMovieDef->GetMetadata(pBuff, buffSize) : 0;
}

bool CFlashPlayerBootStrapper::HasMetadata(tukk pTag) const
{
	bool bResult = false;
	const size_t buffSize = GetMetadata(0, 0);
	if (buffSize)
	{
		tuk pMetaData = new char[buffSize];
		if (pMetaData)
		{
			GetMetadata(pMetaData, buffSize);
			bResult = DrxStringUtils::stristr(pMetaData, pTag) != 0;
		}
		delete[] pMetaData;
	}
...
i32 CFlashPlayer::ms_sys_flash(1);
i32 CFlashPlayer::ms_sys_flash_edgeaa(1);
	#if defined(ENABLE_FLASH_INFO)
//i32 CFlashPlayer::ms_sys_flash_info(0);
float CFlashPlayer::ms_sys_flash_info_peak_tolerance(5.0f);
float CFlashPlayer::ms_sys_flash_info_histo_scale(1.0f);
	#endif
i32 CFlashPlayer::ms_sys_flash_log_options(0);
float CFlashPlayer::ms_sys_flash_curve_tess_error(1.0f);
i32 CFlashPlayer::ms_sys_flash_warning_level(1);
i32 CFlashPlayer::ms_sys_flash_static_pool_size(0);
i32 CFlashPlayer::ms_sys_flash_address_space_kb(DEFAULT_VA_SPACE_IN_KB);
i32 CFlashPlayer::ms_sys_flash_allow_mesh_cache_reset(1);
i32 CFlashPlayer::ms_sys_flash_reset_mesh_cache(0);
i32 CFlashPlayer::ms_sys_flash_check_filemodtime(0);
i32 CFlashPlayer::ms_sys_flash_mipmaps(0);
...
CFlashPlayer::CFlashPlayer()
	...
	, m_pProfilerData(0)
	#endif
	, m_lock(new DrxCriticalSection) // Level heap note: this new doesn't go into the GFx mem pool! Then again the counter object of the shared ptr doesn't neither.
	, m_retValRefHolder()
	, m_cmdBuf()
...
CFlashPlayer::~CFlashPlayer()
{
	...
	// release ref counts of all shared resources
	{
		...
		if (extRefs > 0)
		{
			DrxGFxLog::GetAccess().LogError("Releasing flash player object while still holding %d associated flash variable object reference(s)! Enforce breaking into the debugger...", (i32) extRefs);
			//__debugbreak();
		}
	#endif
...
void CFlashPlayer::InitCVars()
{
	...
	REGISTER_CVAR2("sys_flash_warning_level", &ms_sys_flash_warning_level, 1,
	               0, "Sets verbosity level for DinrusX related warnings...\n"
	                  "0) Omit warning\n"
	                  "1) Log warning\n"
	...
	#if !defined(_RELEASE) || DRX_PLATFORM_WINDOWS || DRX_PLATFORM_APPLE || DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID
	...
	#endif
	GFxVideoDrxSoundSystem::InitCVars();
	GFxMemoryArenaWrapper::InitCVars();
}

i32 CFlashPlayer::GetWarningLevel()
{
...
bool CFlashPlayer::ConstructInternal(tukk pFilePath, GFxMovieDef* pMovieDef, u32 options, u32 cat)
{
	...
	*m_filePath = pFilePath;
	...
	m_pMovieView->SetFSCommandHandler(&DrxGFxFSCommandHandler::GetAccess());
	m_pMovieView->SetExternalInterface(&DrxGFxExternalInterface::GetAccess());
	m_pMovieView->SetUserEventHandler(&DrxGFxUserEventHandler::GetAccess());

	// enable mouse support
	m_pMovieView->SetMouseCursorCount((options & ENABLE_MOUSE_SUPPORT) ? 1 : 0);

	// cache constant data for fast lock free lookup
	*const_cast<u32*>(&m_frameCount) = m_pMovieView->GetFrameCount();
	*const_cast<float*>(&m_frameRate) = m_pMovieView->GetFrameRate();
	*const_cast<i32*>(&m_width) = (i32) m_pMovieDef->GetWidth();
	*const_cast<i32*>(&m_height) = (i32) m_pMovieDef->GetHeight();

	return true;
}
bool CFlashPlayer::Load(tukk pFilePath, u32 options, u32 cat)
{
	...
	{
#endif
		i32k warningLevel = GetWarningLevel();
		if (warningLevel)
		{
			...
			if (!movieInfo.IsStripped())
			{
				...
				{
				case 1:
					DrxGFxLog::GetAccess().LogWarning("Trying to load non-stripped flash movie! Use gfxexport to strip movies and generate optimized assets.");
					break;
				case 2:
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "!Trying to load non-stripped flash movie! "
	...
	else
	{
		...
		{
			DrxGFxLog::GetAccess().LogError("Error: GFxLoader failed to open '%s'", pFilePath);
			return false;
		}
	...
	if (isVideoFile)
	{
		*const_cast<i32*>(&m_width) = vidWidth;
		*const_cast<i32*>(&m_height) = vidHeight;
...
bool CFlashPlayer::Bootstrap(GFxMovieDef* pMovieDef, u32 options, u32 cat)
{
	if (!pMovieDef)
		return false;
	...
	tukk pFilePath = pMovieDef->GetFileURL();
...
std::shared_ptr<CFlashPlayer> CFlashPlayer::CreateBootstrapped(GFxMovieDef* pMovieDef, u32 options, u32 cat)
{
...
void CFlashPlayer::SetViewport(i32 x0, i32 y0, i32 width, i32 height, float aspectRatio)
{
	FLASH_PROFILER_LIGHT;
...
void CFlashPlayer::GetViewport(i32& x0, i32& y0, i32& width, i32& height, float& aspectRatio) const
{
	FLASH_PROFILER_LIGHT;
...
void CFlashPlayer::SetScissorRect(i32 x0, i32 y0, i32 width, i32 height)
{
	FLASH_PROFILER_LIGHT;
...
void CFlashPlayer::GetScissorRect(i32& x0, i32& y0, i32& width, i32& height) const
{
	FLASH_PROFILER_LIGHT;
...
void CFlashPlayer::RenderPlaybackLocklessCallback(i32 cbIdx, EFrameType ft, bool finalPlayback)
{
	FLASH_PROFILER_LIGHT;
...
void CFlashPlayer::SetClearFlags(u32 clearFlags, ColorF clearColor)
{
...
void CFlashPlayer::GotoFrame(u32 frameNumber)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::GotoLabeledFrame(tukk pLabel, i32 offset)
{
	FLASH_PROFILER_LIGHT;
...
u32 CFlashPlayer::GetCurrentFrame() const
{
	FLASH_PROFILER_LIGHT;
...
void CFlashPlayer::SetFSCommandHandler(IFSCommandHandler* pHandler, uk pUserData)
{
...
void CFlashPlayer::SetExternalInterfaceHandler(IExternalInterfaceHandler* pHandler, uk pUserData)
{
	//SYNC_THREADS; //don't sync (handler needs to reside in same thread as flash player instance)
...
void CFlashPlayer::SendCharEvent(const SFlashCharEvent& charEvent)
{
	...
	if ((gEnv->pInput->GetModifiers() & eMM_Ctrl) != 0)
	{
		...
		SFlashKeyEvent keyEvent(SFlashKeyEvent::eKeyDown, code, SFlashKeyEvent::eCtrlPressed, static_cast<u8>(code), code);
...
bool CFlashPlayer::SetOverrideTexture(tukk pResourceName, ITexture* pTexture, bool resize)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::SetVariable(tukk pPathToVar, const SFlashVarValue& value)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::SetVariable(tukk pPathToVar, const IFlashVariableObject* pVarObj)
{
	FLASH_PROFILER_LIGHT;
	...
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
	FLASH_PROFILE_FUNC_2ARG(eFncSetVar, false, "SetVariable", pPathToVar, (uk ) pVarObj);
	SET_LOG_CONTEXT(m_filePath);
	bool res(false);
	if (m_pMovieView)
...
bool CFlashPlayer::GetVariable(tukk pPathToVar, SFlashVarValue& value) const
{
	FLASH_PROFILER_LIGHT;
	...
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
	FLASH_PROFILE_FUNC_2ARG(eFncGetVar, false, "GetVariable", pPathToVar, (uk ) &value);
	SET_LOG_CONTEXT(m_filePath);
	m_retValRefHolder = GFxValue();
	bool res(false);
	if (m_pMovieView)
...
bool CFlashPlayer::GetVariable(tukk pPathToVar, IFlashVariableObject*& pVarObj) const
{
	FLASH_PROFILER_LIGHT;
	...
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
	FLASH_PROFILE_FUNC_2ARG(eFncGetVar, false, "GetVariable", pPathToVar, (uk ) pVarObj);
	SET_LOG_CONTEXT(m_filePath);
	pVarObj = 0;
	if (m_pMovieView)
	{
...
bool CFlashPlayer::IsAvailable(tukk pPathToVar) const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::SetVariableArray(EFlashVariableArrayType type, tukk pPathToVar, u32 index, ukk pData, u32 count)
{
	FLASH_PROFILER_LIGHT;
...
u32 CFlashPlayer::GetVariableArraySize(tukk pPathToVar) const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::GetVariableArray(EFlashVariableArrayType type, tukk pPathToVar, u32 index, uk pData, u32 count) const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::Invoke(tukk pMethodName, const SFlashVarValue* pArgs, u32 numArgs, SFlashVarValue* pResult)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::CreateString(tukk pString, IFlashVariableObject*& pVarObj)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::CreateObject(tukk pClassName, const SFlashVarValue* pArgs, u32 numArgs, IFlashVariableObject*& pVarObj)
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::CreateFunction(IFlashVariableObject*& pFuncVarObj, IActionScriptFunction* pFunc, uk pUserData)
{
	FLASH_PROFILER_LIGHT;
...
u32 CFlashPlayer::GetFrameCount() const
{
	return m_frameCount;
}
...
i32 CFlashPlayer::GetWidth() const
{
	return m_width;
}

i32 CFlashPlayer::GetHeight() const
{
	return m_height;
}

size_t CFlashPlayer::GetMetadata(tuk pBuff, u32 buffSize) const
{
	FLASH_PROFILER_LIGHT;
...
bool CFlashPlayer::HasMetadata(tukk pTag) const
{
	FLASH_PROFILER_LIGHT;
...
tukk CFlashPlayer::GetFilePath() const
{
...
{
	DrxGFxTranslator::GetAccess().SetDirty(false);
}

void CFlashPlayer::ScreenToClient(i32& x, i32& y) const
{
	FLASH_PROFILER_LIGHT;
...
void CFlashPlayer::ClientToScreen(i32& x, i32& y) const
{
	FLASH_PROFILER_LIGHT;
...
void CFlashPlayer::DelegateFSCommandCallback(tukk pCommand, tukk pArgs)
{
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
...
void CFlashPlayer::DelegateExternalInterfaceCallback(tukk pMethodName, const GFxValue* pArgs, UInt numArgs)
{
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
	if (m_pMovieView)
	{
...
{
	u32k renderFlags(m_renderConfig.GetRenderFlags());

	u32 newRenderFlags(renderFlags);
...
static void Draw2dLabel(float x, float y, float fontSize, const float* pColor, tukk pText)
{
...
static void DrawText(float x, float y, const float* pColor, tukk pFormat, ...)
{
	char buffer[512];
...
static void DrawTextNoFormat(float x, float y, float* const pColor, tukk pText)
{
...
{
	#if defined(ENABLE_FLASH_INFO)
	DrxAutoCriticalSection lock(GetList().GetLock());

	IRenderer* pRenderer = gEnv->pRenderer;
	assert(pRenderer);

	i32k curFrameID = pRenderer->GetFrameID(false);
	...
	static u32 s_lastNumTexturesTotal = GTextureXRenderBase::GetNumTexturesTotal();

	if (ms_sys_flash_info)
	{
		IF (ms_sys_flash_info == 3, 0)
		{
			i32 fontCacheTexId = GTextureXRenderBase::GetFontCacheTextureID();
		...
		IF (ms_sys_flash_info == 4 || ms_sys_flash_info == 5, 0)
		{
			...
			{
				PrintHeapInfo(float x, float y, float limitY, u32 startIdx, bool logToConsole)
		...
		if (pGFxHeap)
		{
			GPtr<GFxMeshCacheUpr> pMeshCacheMgr = pLoader ? pLoader->GetMeshCacheUpr() : 0;
			GMemoryHeap* pHeap = pMeshCacheMgr ? pMeshCacheMgr->GetHeap() : 0;
			size_t memMeshCache = pHeap ? pHeap->GetTotalUsedSpace() : 0;

			GPtr<GFxFontCacheUpr> pFontCacheMgr = pLoader ? pLoader->GetFontCacheUpr() : 0;
			pHeap = pFontCacheMgr ? pFontCacheMgr->GetHeap() : 0;
			size_t memFontCache = pHeap ? pHeap->GetTotalUsedSpace() : 0;

			GSysAllocDrxMem::Stats memAllocStats = CSharedFlashPlayerResources::GetAccess().GetSysAllocStats();
			...
			u32 numTexturesTotal = GTextureXRenderBase::GetNumTexturesTotal() + GTextureXRenderBase::GetNumTexturesTempRT();
		...
		if (CFlashInfoInput::Get()->ShouldSort())
		{
			...
			while (needSorting)
			{
				...
				{
					tukk pFileNameCur = pNode->m_pHandle->m_filePath->c_str();
					tukk pFileNameNext = pNext->m_pHandle->m_filePath->c_str();
		...
		u32 numFlashFiles(0), numActiveFlashFiles(0);
		...
		while (pCurNode != &GetListRoot())
		{
			...
			tukk pDisabled = !disabled ? "" : "[disabled] ";
			...
			if (pFlashPlayer)
			{
				tukk pFileName = pFlashPlayer->m_filePath->c_str();
				if (SFlashProfilerData::ShowStrippedPath())
				...
				if (ms_sys_flash_info != 2)
				{
					...
					tukk pLocklessDisparityMsg = "";
					tukk pLocklessDisparityMsgCol = "";
				...
				{
					u32 dcCur = 0, dcAvg = 0, triCur = 0, triAvg = 0;
					if (pFlashProfilerData)
					{
			...
			if (ms_sys_flash_info == 1 && pFlashProfilerData && pFlashProfilerData->IsViewExpanded())
			{
				...
				for (u32 i(0); i < eNumFncIDs; ++i)
				{
					...
					u32 numAvgCalls(pFlashProfilerData->GetTotalCallsHisto((EFlashFunctionID)i).GetAvg());
					...
					if (!isMTRenderer)
					{
						u8 histogram[SFlashProfilerData::HistogramSize];
						for (u32 j(0); j < SFlashProfilerData::HistogramSize; ++j)
							histogram[j] = 255 - (u32) clamp_tpl(pFlashProfilerData->GetTotalTimeHisto((EFlashFunctionID)i).GetAt(j) * histoScale, 0.0f, 255.0f);

						ColorF histogramCol(0, 1, 0, 1);
						pRenderer->Graph(histogram, (i32) (xAdj + 440), (i32) (yAdj + ypos), SFlashProfilerData::HistogramSize, 14, pFlashProfilerData->GetTotalTimeHisto((EFlashFunctionID)i).GetCurIdx(), 2, 0, histogramCol, 1);
					}
		...
		tukk pExludeMaskStr(CV_sys_flash_info_peak_exclude ? CV_sys_flash_info_peak_exclude->GetString() : 0);
		if (pExludeMaskStr)
		{
			for (u32 i(0); i < eNumFncIDs; ++i)
			{
				EFlashFunctionID funcID((EFlashFunctionID) i);
				tukk pFlashFuncName(GetFlashFunctionName(funcID));
		...
		if (pFlashRenderer)
		{
			...
			i32 nTempRenderTargets = tempRenderTargets.size();
			...
			if (nTempRenderTargets > 0)
			{
				...
				for (i32 i = 0; i < nTempRenderTargets; i++)
				{
	...
	else
	{
		...
		#if defined(USE_GFX_VIDEO)
		{
			i32k numVideoInstances = GFxVideoWrapper::GetNumVideoInstances();
			if (numVideoInstances)
			{
				...
				{
					static const float color[4] = { 1, 1, 1, 1 };
					tukk videoCol = ((curFrameID / 10) & 1) != 0 ? "$8" : "$4";
	...
	bool leaksDetected = false;
	{
		...
		if (pNode != &GetListRoot())
		{
			...
			for (; pNode != &GetListRoot(); pNode = pNode->m_pNext)
			{
				...
				tukk pFilePath = pPlayer->m_filePath->c_str();
				pLog->Log("  ptr = 0x%p [%s]", pPlayer, pFilePath ? pFilePath : "#NULL#");
			}
		}
	}
	{
		DrxAutoCriticalSection lock(CFlashVariableObject::GetList().GetLock());
		...
		if (pNode != &CFlashVariableObject::GetListRoot())
		{
			...
			for (; pNode != &CFlashVariableObject::GetListRoot(); pNode = pNode->m_pNext)
			{
				...
				tukk pRefFilePath = refFilePath->c_str();
				pLog->Log("  ptr = 0x%p, ref = 0x%p [%s]", pVar, refFilePath.get(), pRefFilePath ? pRefFilePath : "#NULL#");
			}
		}
	}
	{
		DrxAutoCriticalSection lock(CFlashPlayerBootStrapper::GetList().GetLock());
		...
		if (pNode != &CFlashPlayerBootStrapper::GetListRoot())
		{
			...
			for (; pNode != &CFlashPlayerBootStrapper::GetListRoot(); pNode = pNode->m_pNext)
			{
				...
				tukk pFilePath = pBootStrapper->GetFilePath();
	...
	while (!m_cancelRequestSent)
	{
		...
		if (!m_cancelRequestSent)
		{
			...
			if (CFlashPlayer::AllowMeshCacheReset())
			{
				GFxLoader2* pLoader = CSharedFlashPlayerResources::GetAccess().GetLoader(true);
				if (pLoader)
					pLoader->GetMeshCacheUpr()->ClearCache();
			}
			s_displaySync.Unlock();
		}
		m_awakeThread.Reset();
...
u32 CFlashPlayer::GetLogOptions()
{
