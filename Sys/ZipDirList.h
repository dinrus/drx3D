...
struct FileDataRecord : public FileRecord
{
	...
	void AddRef()  { DrxInterlockedIncrement(&m_nRefCount); }
	void Release() { if (DrxInterlockedDecrement(&m_nRefCount) <= 0) Delete(); }
...
public:
	...
	size_t MakeZipCDR(u32 lCDROffset, uk p, bool encryptedFlag) const;
