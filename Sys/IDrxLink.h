...
namespace DrxLinkService
{
	namespace UriEncoding
	{
		inline string Encode(tukk str, size_t length);
		inline string Decode(tukk str, size_t length);
	}

	enum class ELinkType
	{
		Commands = 0,		// 'DrxLink://<processName>/exec?<commands>'
	...
	// CDrxLink
	//
	// Format: DrxLink://<processName>/<additionalInfo>/.../<linkType>?<query1>&<query2>& ...
	//
	//	For instance 'DrxLink://game_1/exec?cmd1=mission+tutorial&cmd2=missionSetQuest+5'
	//
	// Syntax: 
...
#define DRXLINK_SCHEME_NAME "drxlink"
#define DRXLINK_SCHEME_BEGIN DRXLINK_SCHEME_NAME "://"
#define DRXLINK_MAX_MESSAGE_LENGTH 4096
	...
	public:
		...
		CDrxLink(tukk szUri = DRXLINK_SCHEME_BEGIN)
			: m_uri(szUri)
		...
		tukk GetScheme() const
		{
			return m_scheme.c_str();
		}

		tukk GetAuthority() const
		{
	...
	class CDrxLinkUriFactory
	{
	public:
		CDrxLinkUriFactory(tukk szProcessName, ELinkType linkType = ELinkType::Commands)
			: m_szProcessName(szProcessName)
			, m_linkType(linkType)
			, m_commandStringsLength(0)
		{}

		void AddCommand(tukk szCommand, size_t length)
		{
		...
		static string GetUri(tukk szProcessName, std::vector<string> commands)
		{
			CDrxLinkUriFactory uriFactory(szProcessName);
			for (const string& command : commands)
				uriFactory.AddCommand(command, command.length());
			return uriFactory.GetUri();
		}

		static string GetUri(tukk szProcessName, const string& command)
		{
			CDrxLinkUriFactory uriFactory(szProcessName);
			uriFactory.AddCommand(command, command.length());
			return uriFactory.GetUri();
		...
		static string GetUriV(tukk szProcessName, tukk szCommand, ...)
		{
			va_list args;
			va_start(args, szCommand);
			char szBuffer[DRXLINK_MAX_MESSAGE_LENGTH];
			drx_vsprintf(szBuffer, szCommand, args);
			va_end(args);

			return CDrxLinkUriFactory::GetUri(szProcessName, szBuffer);
		}
		...
		string GetUri() const
		{
			...
			uri.Format("%s://%s/%s?", DRXLINK_SCHEME_NAME, m_szProcessName, GetType());
	...
	private:
		tukk GetType() const
		{
			static tukk linkTypes[] =
			{
				"exec"
			};

			static_assert(static_cast<size_t>(ELinkType::Count) == (sizeof(linkTypes) / sizeof(tukk)), "linkTypes size mismatch!");

			return linkTypes[static_cast<u32>(m_linkType)];
		}

		std::vector<string> m_commands;
		tukk m_szProcessName;
		u32 m_commandStringsLength;
		ELinkType m_linkType;
	};

	namespace UriEncoding
	{
		static tukk s_szAsHex[] =
		{
		...
		string Encode(tukk szBuffer, size_t length)
		{
		...
		string Decode(tukk szBuffer, size_t length)
		{
			...
			for (size_t i = 0; i < length; ++i)
			{
				...
				case '%':
					ch = szBuffer[++i];
					result = ((isdigit(ch) ? ch - '0' : 10 + DrxStringUtils::toLowerAscii(ch) - 'a') & 0xF) << 4;
					ch = szBuffer[++i];
					result += ((isdigit(ch) ? ch - '0' : 10 + DrxStringUtils::toLowerAscii(ch) - 'a') & 0xF);
