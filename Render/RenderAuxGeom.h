...
public:
	...
	void                DrawPoint(const Vec3& v, const ColorB& col, u8 size = 1) override;
	void                DrawPoints(const Vec3* v, u32 numPoints, const ColorB& col, u8 size = 1) override;
	void                DrawPoints(const Vec3* v, u32 numPoints, const ColorB* col, u8 size = 1) override;

	void                DrawLine(const Vec3& v0, const ColorB& colV0, const Vec3& v1, const ColorB& colV1, float thickness = 1.0f) override;
	void                DrawLines(const Vec3* v, u32 numPoints, const ColorB& col, float thickness = 1.0f) override;
	void                DrawLines(const Vec3* v, u32 numPoints, const ColorB* col, float thickness = 1.0f) override;
	void                DrawLines(const Vec3* v, u32k* packedColorARGB8888, u32 numPoints, float thickness = 1.0f, bool alphaFlag = true) override;
	void                DrawLines(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB& col, float thickness = 1.0f) override;
	void                DrawLines(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB* col, float thickness = 1.0f) override;
	void                DrawLineStrip(const Vec3* v, u32 numPoints, const ColorB* col, float thickness = 1.0f) override;
	void                DrawPolyline(const Vec3* v, u32 numPoints, bool closed, const ColorB& col, float thickness = 1.0f) override;
	void                DrawPolyline(const Vec3* v, u32 numPoints, bool closed, const ColorB* col, float thickness = 1.0f) override;
	...
	void                DrawTriangles(const Vec3* v, u32 numPoints, const ColorB& col) override;
	void                DrawTriangles(const Vec3* v, u32 numPoints, const ColorB* col) override;
	void                DrawTriangles(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB& col) override;
	void                DrawTriangles(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB* col) override;

	void                DrawBuffer(const SAuxVertex* inVertices, u32 numVertices, bool textured) override;
	SAuxVertex*         BeginDrawBuffer(u32 maxVertices, bool textured) override;
	void                EndDrawBuffer(u32 numVertices) override;

	void                DrawAABB(const AABB& aabb, bool bSolid, const ColorB& col, const EBoundingBoxDrawStyle& bbDrawStyle) override;
	void                DrawAABBs(const AABB* aabbs, u32 aabbCount, bool bSolid, const ColorB& col, const EBoundingBoxDrawStyle& bbDrawStyle) override;
	...
	void                RenderTextQueued(Vec3 pos, const SDrawTextInfo& ti, tukk text) override;

	i32                 SetTexture(i32 texID) override;
	i32                 PushMatrix(const Matrix34& mat) override;
	Matrix34*           GetMatrix() override;
	void                SetMatrixIndex(i32 matID) override;
...
public:
	...
	struct SAuxDrawObjParams
	{
		...
		SAuxDrawObjParams(const Matrix34& matWorld, u32k& color, float size, bool shaded)
			: m_matWorld(matWorld)
			, m_color(color)
			, m_size(size)
			, m_shaded(shaded)
	...
	struct SAuxPushBufferEntry
	{
		...
		SAuxPushBufferEntry(u32 numVertices, u32 numIndices, u32 vertexOffs, u32 indexOffs, u32 transMatrixIdx, i32 worldMatrixIdx, const SAuxGeomRenderFlags& renderFlags, i32 texID = -1, const SDisplayContextKey& displayContextKey = {})
		...
		SAuxPushBufferEntry(u32 drawParamOffs, u32 transMatrixIdx, i32 worldMatrixIdx, const SAuxGeomRenderFlags& renderFlags, i32 texID = -1, const SDisplayContextKey& displayContextKey = {})
			: m_numVertices(0)
			, m_numIndices(0)
			...
			, m_displayContextKey(displayContextKey)
		{
			DRX_ASSERT(e_Obj == GetPrimType(m_renderFlags));
		}

		bool GetDrawParamOffs(u32& drawParamOffs) const
		{
		...
		u32                  m_vertexOffs;
		u32                  m_indexOffs;
		i32                     m_transMatrixIdx;
		i32                     m_worldMatrixIdx;
		i32                     m_textureID;
	...
	public:
		...
		i32                     m_curTransMatIdx;
		i32                     m_curWorldMatIdx;
		i32                     m_textureID;
		uint                    m_uCount;
		bool                    m_isUsed = false;
...
public:
	...
	static u8           GetPointSize(const SAuxGeomRenderFlags& renderFlags);
...
private:
	...
	u32 CreatePointRenderFlags(u8 size);
	u32 CreateLineRenderFlags(bool indexed);
	u32 CreateLineStripRenderFlag(bool indexed);
	u32 CreateTriangleRenderFlags(bool indexed, bool textured = false);
	u32 CreateObjectRenderFlags(const EAuxDrawObjType& objType);

	void   DrawThickLine(const Vec3& v0, const ColorB& colV0, const Vec3& v1, const ColorB& colV1, float thickness);
	void   DrawThickLineStrip(const Vec3* v, u32 numPoints, const ColorB& col, float thickness);

	void   AddPushBufferEntry(u32 numVertices, u32 numIndices, const SAuxGeomRenderFlags& renderFlags);

	void   AddPrimitive(SAuxVertex*& pVertices, u32 numVertices, const SAuxGeomRenderFlags& renderFlags);
	void   AddIndexedPrimitive(SAuxVertex*& pVertices, u32 numVertices, vtx_idx*& pIndices, u32 numIndices, const SAuxGeomRenderFlags& renderFlags);
...
protected:
	i32 GetTransMatrixIndex() const
	{
		return m_rawData->m_curTransMatIdx;
	}

	i32 GetWorldMatrixIndex() const
	{
...
inline u32 CAuxGeomCB::CreatePointRenderFlags(u8 size)
{
	return(m_rawData->m_curRenderFlags.m_renderFlags | (e_PtList << e_PrimTypeShift) | size);
}

inline u32 CAuxGeomCB::CreateLineRenderFlags(bool indexed)
{
	if (false != indexed)
	{
...
inline u32 CAuxGeomCB::CreateLineStripRenderFlag(bool indexed)
{
	if (false != indexed)
	{
...
inline u32 CAuxGeomCB::CreateTriangleRenderFlags(bool indexed, bool textured)
{
	u32 idx = indexed ? e_TriListInd : e_TriList;
	u32 tex = textured ? 1 : 0;

	return m_rawData->m_curRenderFlags.m_renderFlags | (idx << e_PrimTypeShift) | tex;
}

inline u32 CAuxGeomCB::CreateObjectRenderFlags(const EAuxDrawObjType& objType)
{
...
{
	u32 primType((renderFlags.m_renderFlags & e_PrimTypeMask) >> e_PrimTypeShift);
	switch (primType)
	{
	case e_PtList:
		{
...
inline bool CAuxGeomCB::IsThickLine(const SAuxGeomRenderFlags& renderFlags)
{
	...
	//DRX_ASSERT(e_LineList == primType);
...
inline CAuxGeomCB::EAuxDrawObjType CAuxGeomCB::GetAuxObjType(const SAuxGeomRenderFlags& renderFlags)
{
	...
	u32 objType((renderFlags.m_renderFlags & e_PrivateRenderflagsMask));
	switch (objType)
	{
	case eDOT_Sphere:
	default:
		{
			DRX_ASSERT(eDOT_Sphere == objType);
			return(eDOT_Sphere);
		}
	case eDOT_Cone:
		{
			DRX_ASSERT(eDOT_Cone == objType);
			return(eDOT_Cone);
		}
	case eDOT_Cylinder:
		{
			DRX_ASSERT(eDOT_Cylinder == objType);
			return(eDOT_Cylinder);
		}
	}
}

inline u8 CAuxGeomCB::GetPointSize(const SAuxGeomRenderFlags& renderFlags)
{
	EPrimType primType(GetPrimType(renderFlags));
	DRX_ASSERT(e_PtList == primType);
...
public:
	...
	void                DrawAABBs(const AABB* aabbs, u32 aabbCount, bool bSolid, const ColorB& col, const EBoundingBoxDrawStyle& bbDrawStyle) final       {}
