...
public:
	  ...
	  REFIID riid) : m_RefCount(1), Handable(this)
	{
		...
		{
			Desc.Width = Align(Desc.Width, std::min<UINT64>(DRX_PLATFORM_ALIGNMENT, std::max<UINT64>(DRX_PLATFORM_ALIGNMENT, Desc.Alignment)));
...
#define BroadcastWithFail(func)                 \
  ...
  return S_OK;
	...
	virtual HRESULT STDMETHODCALLTYPE QueryInterface(
	  /* [in] */ REFIID riid,
	  /* [iid_is][out] */ _COM_Outptrvoid __RPC_FAR* __RPC_FAR* ppvObject) final
	{
		abort();
	}
	...
	{
		Broadcast(Release());
		ULONG count = DrxInterlockedDecrement(&m_RefCount);
	...
	virtual HRESULT STDMETHODCALLTYPE GetDevice(
	  REFIID riid,
	  _COM_Outptr_optvoid** ppvDevice) final
	{
	  ...
	  _Outptr_opt_result_bytebuffer_(_Inexpressible_("Dependent on resource"))  void** ppData) final
	{
		...
		int count = DrxInterlockedIncrement(&m_MapCount);
		if (count == 1)
		{
	  ...
	  _In_opt_ const D3D12_RANGE* pWrittenRange) final
	{
		...
		int count = DrxInterlockedDecrement(&m_MapCount);
		if (count == 0)
		{
//			__debugbreak();
			delete [] m_Buffer;
		...
		_In_ const UINT numDataBlocks)
	{
		...
		for (int i = 0; i < numTargets; ++i)
		{
			u8k* pInDataBlock = reinterpret_cast<u8k*>(pInData) + ((Range->End - Range->Begin) * (i % numDataBlocks));

			Pick(i, Map(Subresource, &sNoRead, &pOutData));
			StreamBufferData(reinterpret_cast<u8*>(pOutData) + Range->Begin, pInDataBlock, Range->End - Range->Begin);
		...
		_In_ const UINT numDataBlocks)
	{
		...
		{
			u8* pOutDataBlock = reinterpret_cast<u8*>(pOutData) + ((Range->End - Range->Begin) * (i));

			Pick(0, Map(Subresource, Range, &pInData));
			FetchBufferData(pOutDataBlock, reinterpret_cast<u8*>(pInData) + Range->Begin, Range->End - Range->Begin);
