...
class CDrxDX12Device : public CDrxDX12Object<ID3D11Device1ToImplement>
{
public:
	DX12_OBJECT(CDrxDX12Device, CDrxDX12Object<ID3D11Device1ToImplement> );

	static CDrxDX12Device* Create(CDrxDX12GIAdapter* pAdapter, D3D_FEATURE_LEVEL* pFeatureLevel);

	CDrxDX12Device(NDrxDX12::CDevice* device);

	NDrxDX12::CDevice*     GetDX12Device() const                 { return m_pDevice; }

	ID3D12Device*          GetD3D12Device() const                { return m_pDevice->GetD3D12Device(); }

	CDrxDX12DeviceContext* GetDeviceContext() const              { return m_pMainContext; }
	CDrxDX12DeviceContext* GetDeviceContext(UINT nodeMask) const { return m_pNodeContexts[nodeMask]; }
	...
	VIRTUALGFX HRESULT STDMETHODCALLTYPE OpenSharedResource(
	  ...
	  _Out_optvoid** ppResource) FINALGFX;
	...
	VIRTUALGFX HRESULT STDMETHODCALLTYPE OpenSharedResource1(
	  ...
	  _In_ REFIID returnedInterface,
	  /* [annotation] */
	  _COM_Outptrvoid** ppResource) FINALGFX;
...
private:
	DX12_PTR(NDrxDX12::CDevice) m_pDevice;
	...
	DX12_PTR(CDrxDX12DeviceContext) m_pMainContext;
	std::vector<DX12_PTR(CDrxDX12DeviceContext)> m_pNodeContexts;
};
