...
#include <drx3D/Render/DX12/Misc/SDrxDX11PipelineState.hpp>

#include <drx3D/Render/DX12/API/DX12Base.hpp>
#include <drx3D/Render/DX12/API/DX12CommandScheduler.hpp>
#include <drx3D/Render/DX12/API/DX12CommandList.hpp>

#include <drx3D/Render/DX12/Device/CDrxDX12Device.hpp>

class CDrxDX12DeviceContext : public CDrxDX12Object<ID3D11DeviceContext1ToImplement>
{
	friend class CSubmissionQueue_DX11;
	friend class CDeviceObjectFactory;

public:
	DX12_OBJECT(CDrxDX12DeviceContext, CDrxDX12Object<ID3D11DeviceContext1ToImplement> );

	static CDrxDX12DeviceContext* Create(CDrxDX12Device* pDevice, UINT nodeMask, bool isDeferred);

	CDrxDX12DeviceContext(CDrxDX12Device* pDevice, UINT nodeMask, bool isDeferred);

	virtual ~CDrxDX12DeviceContext();
	...
	ILINE bool RecreateCommandListPool(i32 nPoolId)
	{
		return m_Scheduler.RecreateCommandListPool(nPoolId, m_nodeMask);
	}

	void CeaseCommandQueueCallback(i32 nPoolId);
	void ResumeCommandQueueCallback(i32 nPoolId);

	ILINE NDrxDX12::CCommandScheduler& GetDX12Scheduler() { return m_Scheduler; }

	ILINE NDrxDX12::CCommandListPool& GetCoreGraphicsCommandListPool() {
		return m_Scheduler.GetCommandListPool(CMDQUEUE_GRAPHICS); }
	ILINE NDrxDX12::CCommandList* GetCoreGraphicsCommandList() const {
		return m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS); }

	ILINE CDrxDX12Device*    GetDevice     () const { return m_pDevice; }
	ILINE NDrxDX12::CDevice* GetDX12Device () const { return m_pDevice->GetDX12Device(); }
	ILINE ID3D12Device*      GetD3D12Device() const { return m_pDevice->GetD3D12Device(); }

	ILINE CDrxDX12DeviceContext* SetNode(UINT nodeMask) const
	{
		return GetDevice()->GetDeviceContext(nodeMask);
	}

	void Finish(NDrxDX12::CSwapChain* pDX12SwapChain);
	void Finish();
	...
	VIRTUALGFX HRESULT STDMETHODCALLTYPE GetPrivateData(
	  ...
	  _Out_writes_bytes_opt_(*pDataSize)  uk pData) FINALGFX;

	VIRTUALGFX HRESULT STDMETHODCALLTYPE SetPrivateData(
	  _In_ REFGUID guid,
	  _In_ UINT DataSize,
	  _In_reads_bytes_opt_(DataSize)  ukk pData) FINALGFX;
	...
	VIRTUALGFX void STDMETHODCALLTYPE UpdateSubresource1(ID3D11Resource* pDstResource, UINT DstSubresource, const D3D11_BOX* pDstBox, ukk pSrcData, UINT SrcRowPitch, UINT SrcDepthPitch, UINT CopyFlags) FINALGFX;
	...
	HRESULT STDMETHODCALLTYPE MappedWriteToSubresource(
	  ...
	  _In_ SIZE_T Size,
	  _In_ D3D11_MAP MapType,
	  _In_reads_bytes_(Size) ukk pData,
	...
	HRESULT STDMETHODCALLTYPE MappedReadFromSubresource(
	  ...
	  _In_ SIZE_T Size,
	  _In_ D3D11_MAP MapType,
	  _Out_writes_bytes_(Size) uk pData,
	  _In_ const UINT numDataBlocks);
	...
	ILINE void InsertTimestamp(INT index, NDrxDX12::CCommandList* pCommandList)
	{
	...
	ILINE void QueryTimestamp(INT index, uk mem)
	{
		QueryTimestamp(m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS), index, mem);
	}

	ILINE void QueryTimestamps(INT firstIndex, INT numIndices, uk mem)
	{
		u32 outIndex = 0;
		for (INT index = firstIndex; index < firstIndex + numIndices; ++index)
		{
			QueryTimestamp(m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS), index, (tuk)mem + outIndex * sizeof(UINT64));
			++outIndex;
		}
	}
	...
	ILINE void QueryTimestampBuffer(ID3D11Buffer** ppTimestampBuffer)
	{
		...
		{
			*ppTimestampBuffer = CDrxDX12Buffer::Create(m_pDevice, m_TimestampDownloadBuffer, D3D12_RESOURCE_STATE_GENERIC_READ);
		}
	}
...
private:
	...
	void SetConstantBuffers1(NDrxDX12::EShaderStage shaderStage, UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppConstantBuffers, const UINT* pFirstConstant, const UINT* pNumConstants, bool bGfx);

	bool               m_bDeferred;
	UINT               m_nodeMask;

	CDrxDX12Device*    m_pDevice;
	NDrxDX12::CDevice* m_pDX12Device;

	NDrxDX12::CCommandScheduler& m_Scheduler;

	bool m_bCmdListBegins[CMDQUEUE_NUM];

	DX12_PTR(NDrxDX12::CPSO) m_pCurrentPSO;
	DX12_PTR(NDrxDX12::CRootSignature) m_pCurrentRootSignature;

	SDrxDX11PipelineState m_GraphicsState;
	SDrxDX11PipelineState m_ComputeState;

public:
	UINT TimestampIndex(NDrxDX12::CCommandList* pCmdList);
	void InsertTimestamp(NDrxDX12::CCommandList* pCmdList, UINT index);
	void ResolveTimestamps(NDrxDX12::CCommandList* pCmdList);
	void QueryTimestamp(NDrxDX12::CCommandList* pCmdList, UINT index, uk mem);

	UINT OcclusionIndex(NDrxDX12::CCommandList* pCmdList, bool counter);
	void InsertOcclusionStart(NDrxDX12::CCommandList* pCmdList, UINT index, bool counter);
	void InsertOcclusionStop(NDrxDX12::CCommandList* pCmdList, UINT index, bool counter);
	void ResolveOcclusion(NDrxDX12::CCommandList* pCmdList, UINT index, uk mem);
...
private:
	...
	uk                      m_TimestampMemory;
	uk                      m_OcclusionMemory;
	bool                       m_TimestampMapValid;
	bool                       m_OcclusionMapValid;
	NDrxDX12::CDescriptorHeap* m_pResourceHeap;
	NDrxDX12::CDescriptorHeap* m_pSamplerHeap;

	NDrxDX12::CQueryHeap       m_TimestampHeap;
	NDrxDX12::CQueryHeap       m_OcclusionHeap;
