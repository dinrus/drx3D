...
template<typename T>
class CDrxDX12DeviceChild : public CDrxDX12Object<T>
{
public:
	DX12_OBJECT(CDrxDX12DeviceChild, CDrxDX12Object<T> );
		...
		if (m_pChild)
		{
			if (pData)
			{
				if (guid == WKPDID_D3DDebugObjectName)
				{
					...
					size_t len = strlen((tuk)pData);
					MultiByteToWideChar(0, 0, (tuk)pData, len, objectname, len);
...
protected:
	CDrxDX12DeviceChild(CDrxDX12Device* pDevice, ID3D12DeviceChild* pChild)
	{
	#if !defined(RELEASE)
		m_pChild = pChild;
	#endif
...
private:
	#if !defined(RELEASE)
	typedef std::unordered_map<GUID, std::pair<UINT, uk>> TDatas;
	TDatas             m_Data;
