...
#include <drx3D/Render/DX12CommandListFence.hpp>
#include <drx3D/Render/DX12PSO.hpp>
#include <drx3D/Render/DX12DescriptorHeap.hpp>
#include <drx3D/Render/DX12QueryHeap.hpp>
#include <drx3D/Render/DX12View.hpp>
...
#define CMDLIST_GRAPHICS      NDrxDX12::CCommandListPool::m_MapQueueType[CMDQUEUE_GRAPHICS]
#define CMDLIST_COMPUTE       NDrxDX12::CCommandListPool::m_MapQueueType[CMDQUEUE_COMPUTE]
#define CMDLIST_COPY          NDrxDX12::CCommandListPool::m_MapQueueType[CMDQUEUE_COPY]
...
{
public:
	CCommandListPool(CDevice* device, CCommandListFenceSet& rCmdFence, i32 nPoolFenceId);
	~CCommandListPool();
	...
	void AcquireCommandLists(u32 numCLs, DX12_PTR(CCommandList) * results) threadsafe;
	void ForfeitCommandLists(u32 numCLs, DX12_PTR(CCommandList) * results, bool bWait = false) threadsafe;
	...
	ILINE NDrxDX12::CAsyncCommandQueue& GetAsyncCommandQueue() threadsafe
	{
	...
	ILINE i32 GetFenceID() threadsafe_const
	{
...
public:
	...
	static bool IsInOrder(i32 nPoolFenceId)
	{
		return
	...
	ILINE void WaitForFenceOnGPU(const UINT64 fenceValue, i32k id) threadsafe
	{
	...
	ILINE void WaitForFenceOnCPU(const UINT64 fenceValue, i32k id) threadsafe_const
	{
		if (!m_rCmdFences.IsCompleted(fenceValue, id))
		{
	#ifdef DX12_STATS
	...
	ILINE bool IsCompletedPerspCPU(const UINT64 fenceValue, i32k id) threadsafe_const
	{
...
private:
	...
	NDrxDX12::CAsyncCommandQueue m_AsyncCommandQueue;
...
public:
	...
	template<class T>
	ILINE void MaxResourceFenceValue(const T& pResource, i32k type)
	{
	...
	template<class T>
	ILINE UINT64 SetResourceFenceValue(T& pResource, i32k type) const
	{
		return pResource.SetFenceValue(m_CurrentFenceValue, m_rPool.GetFenceID(), type);
	}

	template<class T>
	ILINE void TrackResourceUsage(T& pResource, D3D12_RESOURCE_STATES stateUsage, i32 cmdBlocker = CMDTYPE_WRITE, i32 cmdUsage = CMDTYPE_READ)
	{
	...
	template<class T>
	ILINE void TrackResourceUsage(T& pResource, const CView& view, D3D12_RESOURCE_STATES stateUsage, i32 cmdBlocker = CMDTYPE_WRITE, i32 cmdUsage = CMDTYPE_READ)
	{
		MaxResourceFenceValue(pResource, cmdBlocker);
	...
	ILINE void SetGraphics32BitConstants(UINT RootParameterIndex, UINT Num32BitValuesToSet, ukk pSrcData, UINT DestOffsetIn32BitValues)
	{
		m_pCmdList->SetGraphicsRoot32BitConstants(RootParameterIndex, Num32BitValuesToSet, pSrcData, DestOffsetIn32BitValues);
		m_nCommands += CLCOUNT_SETIO;
	}

	ILINE void SetCompute32BitConstants(UINT RootParameterIndex, UINT Num32BitValuesToSet, ukk pSrcData, UINT DestOffsetIn32BitValues)
	{
	...
	void         UpdateSubresourceRegion(CResource& pResource, UINT subResource, const D3D12_BOX* box, ukk data, UINT rowPitch, UINT depthPitch);
