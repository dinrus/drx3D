...
#include <drx3D/Render/DX12SwapChain.hpp>
...
public:
	...
	i32                   SelectQueueForTransitionBarrier(i32 altQueue, i32 desiredQueue, D3D12_RESOURCE_STATES desiredState) const;
	...
	UINT64 SetFenceValue(UINT64 fenceValue, i32k id, i32k type) threadsafe
	{
	...
	ILINE UINT64 GetFenceValue(i32k id, i32k type) threadsafe_const
	{
		return m_FenceValues[type][id];
	}

	ILINE const FVAL64 (&GetFenceValues(i32k type) threadsafe_const)[CMDQUEUE_NUM]
	{
	...
	ILINE void MaxFenceValues(UINT64 (&RESTRICT_REFERENCE rFenceValues)[CMDTYPE_NUM][CMDQUEUE_NUM], const UINT64 fenceValueClamp, i32k nPoolFenceId, i32k type, const FVAL64 (&RESTRICT_REFERENCE rCompletedFenceValues)[CMDQUEUE_NUM]) threadsafe_const
	{
	...
	ILINE bool IsUsed(CCommandListPool& pCmdListPool, i32k type = CMDTYPE_ANY) threadsafe_const
	{
		return !pCmdListPool.IsCompleted<FVAL64, bPerspGPU>(m_FenceValues[type]);
	}

	template<const bool bBlockCPU>
	ILINE void WaitForUnused(CCommandListPool& pCmdListPool, i32k type = CMDTYPE_ANY) threadsafe
	{
		std::integral_constant<bool, bBlockCPU> tag;
		WaitForUnusedImpl(pCmdListPool, type, tag);
	}

	ILINE void WaitForUnusedImpl(CCommandListPool& pCmdListPool, i32k type, std::integral_constant<bool, false> tag) threadsafe
	{
		pCmdListPool.WaitForFenceOnGPU(m_FenceValues[type]);
	}

	ILINE void WaitForUnusedImpl(CCommandListPool& pCmdListPool, i32k type, std::integral_constant<bool, true> tag) threadsafe
	{
	...
	HRESULT MappedWriteToSubresource(UINT Subresource, const D3D12_RANGE* Range, ukk pInData);
	HRESULT MappedReadFromSubresource(UINT Subresource, const D3D12_RANGE* Range, uk pOutData);
