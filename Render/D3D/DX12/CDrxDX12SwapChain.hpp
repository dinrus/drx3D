...
class CDrxDX12SwapChain : public CDrxDX12GIObject<IDXGISwapChain3ToImplement>
{
public:
	DX12_OBJECT(CDrxDX12SwapChain, CDrxDX12GIObject<IDXGISwapChain3ToImplement>);

	static CDrxDX12SwapChain* Create(CDrxDX12Device* pDevice, IDXGIFactory4ToCall* factory, DXGI_SWAP_CHAIN_DESC* pDesc);
	static CDrxDX12SwapChain* Create(CDrxDX12Device* pDevice, IDXGISwapChain1ToCall* swapchain, DXGI_SWAP_CHAIN_DESC1* pDesc);

	virtual ~CDrxDX12SwapChain();

	ILINE CDrxDX12Device* GetDevice() const
	{
		return m_pDevice;
	}
	...
	ILINE NDrxDX12::CSwapChain* GetDX12SwapChain()
	{
	...
	VIRTUALGFX HRESULT STDMETHODCALLTYPE GetDevice(
		_In_ REFIID riid,
		_Outvoid** ppDevice) FINALGFX { abort(); return S_OK; }
	...
	{
		DX12_FUNC_LOG
		return m_pDX12SwapChain->SetFullscreenState(Fullscreen, pTarget ? reinterpret_cast<CDrxDX12GIOutput*>(pTarget)->GetDXGIOutput() : nullptr);
	}
	...
	VIRTUALGFX HRESULT STDMETHODCALLTYPE GetCoreWindow(
	  ...
	  _In_ REFIID refiid,
	  /* [annotation][out] */
	  _COM_Outptrvoid** ppUnk) FINALGFX
	{
		DX12_FUNC_LOG
...
protected:
	CDrxDX12SwapChain(CDrxDX12Device* pDevice, NDrxDX12::CSwapChain* pSwapChain);

private:
	CDrxDX12Device*       m_pDevice;

	NDrxDX12::CSwapChain* m_pDX12SwapChain;

	std::vector<CDrxDX12Texture2D*> m_pBuffers;
};
