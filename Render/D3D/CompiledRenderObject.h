...
template<class T, class I = i32, class STORE = NArray::SmallDynStorage<>>
	...
	void push_back(const T& value)
	{
		...
		if (index >= eCachedItems)
		{
			if (m_size - eCachedItems > m_capacity)
			{
				...
				m_array = (T*)DrxModuleRealloc(m_array, m_capacity * sizeof(T));
			}
...
public:
	...
	{
		u32                 m_sortValue;       //!< Encoded sort value, keeping info about material.
		u32                 m_nBatchFlags;     //!< see EBatchFlags, batch flags describe on what passes this object will be used (transparent,z-prepass,etc...)
		u32                 m_objSort;         //!< Custom object sorting value.
		u32                 m_nRenderList : 8; //!< Defines in which render list this compiled object belongs to, see ERenderListID
	...
	i32          m_compiledReadyMask;
	i32          m_lastCompiledFrame;
...
public:
	/////////////////////////////////////////////////////////////////////////////
	// Packed parameters
	u32 m_StencilRef           : 8; //!< Stencil ref value
	u32 m_nNumVertexStreams  : 8; //!< Number of vertex streams specified
	u32 m_nLastVertexStreamSlot  : 8; //!< Highest vertex stream slot
	u32 m_bOwnPerInstanceCB    : 1; //!< True if object owns its own per instance constant buffer, and is not sharing it with other compiled object
	u32 m_bRenderNearest       : 1; //!< True if object needs to be rendered with nearest camera
	u32 m_bIncomplete          : 1; //!< True if compilation failed
	u32 m_bHasTessellation     : 1; //!< True if tessellation is enabled
	u32 m_bSharedWithShadow    : 1; //!< True if objects is shared between shadow and general pass
	u32 m_bDynamicInstancingPossible : 1; //!< True if this render object can be dynamically instanced with other compiled render object
	u32 m_bCustomRenderElement       : 1; //!< When dealing with not known render element, will cause Draw be redirected to the RenderElement itself
	/////////////////////////////////////////////////////////////////////////////
	...
	i32                    m_TessellationPatchIDOffset; // for tessellation only
...
public:
	...
	void DrawToCommandList(const SGraphicsPipelinePassContext& RESTRICT_REFERENCE passContext, CDeviceCommandList* commandList, CConstantBuffer* pDynamicInstancingBuffer = nullptr, u32 dynamicInstancingCount = 1) const;
...
private:
	...
	void UpdatePerDrawCB(uk pData, size_t size);
...
private:
	static CRenderObjectsPools* s_pPools;
	static DrxCriticalSectionNonRecursive m_drawCallInfoLock;
};