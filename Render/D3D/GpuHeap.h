...
protected:
	...
	virtual uk MapBlock(uint32_t memoryType, TBlockHandle blockHandle) = 0;

	// Unmaps a block from CPU memory view.
	// Note: Will not be called recursively.
	virtual void UnmapBlock(uint32_t memoryType, TBlockHandle blockHandle, uk pAddress) = 0;
...
public:
	...
	typedef void (* TAllocationCallback)(uk pContext, THandle handle, uint32_t memoryType, TBlockHandle blockHandle, uint32_t blockOffset, uint32_t allocationSize, uk pMappedAddress);
	typedef void (* TSummaryCallback)(uk pContext, uint32_t memoryType, uint64_t totalApiAllocated, uint64_t totalClientAllocated, uint32_t totalClientHandles, uint32_t cpuSideOverhead);
	uint32_t Walk(uk pContext, TSummaryCallback pfnSummary, TAllocationCallback pfnAllocation = nullptr, uint32_t memoryTypeMask = 0xFFFFFFFFU) const;
