...
#include <drx3D/Render/RenderPipeline.h> // EShapeMeshType
...
private:
		...
		, m_blockPack(0, 0)
	{
		...
		{
			for (i32 j = 0; j < MAX_REND_RECURSION_LEVELS; ++j)
			{
...
private:
	u32 m_nVisAreasGIRef[RT_COMMAND_BUF_COUNT][MAX_REND_RECURSION_LEVELS];
	...
	CShader*           m_pShader;
	CDrxNameTSCRC      m_pDeferredDecalTechName;
	CDrxNameTSCRC      m_pLightVolumeTechName;
	CDrxNameTSCRC      m_pTechName;
	CDrxNameTSCRC      m_pAmbientOutdoorTechName;
	CDrxNameTSCRC      m_pCubemapsTechName;
	CDrxNameTSCRC      m_pCubemapsVolumeTechName;
	CDrxNameTSCRC      m_pReflectionTechName;
	CDrxNameTSCRC      m_pDebugTechName;
	CDrxNameR          m_pParamLightPos;
	CDrxNameR          m_pParamLightDiffuse;
	CDrxNameR          m_pParamLightProjMatrix;
	CDrxNameR          m_pGeneralParams;
	CDrxNameR          m_pParamAmbient;
	CDrxNameR          m_pParamAmbientGround;
	CDrxNameR          m_pParamAmbientHeight;
	CDrxNameR          m_pAttenParams;

	CDrxNameR          m_pParamDecalTS;
	CDrxNameR          m_pParamDecalDiffuse;
	CDrxNameR          m_pParamDecalSpecular;
	CDrxNameR          m_pParamDecalMipLevels;
	CDrxNameR          m_pClipVolumeParams;
	...
	DRX_ALIGN(16) Vec4 m_vClipVolumeParams[MAX_DEFERRED_CLIP_VOLUMES];
...
public:
	...
	void Clear();
	void FreeMemory();
	bool AllocateGroup(i32* pOffsetX, i32* pOffsetY, i32 nSizeX, i32 nSizeY);

	i32              m_nSize;
	static i32k MAX_BLOCKS = 128;
	u32           m_arrAllocatedBlocks[MAX_BLOCKS];

#ifdef _DEBUG
	u32 m_nTotalWaste;
	struct SShadowMapBlock
	{
		u16 m_nX1, m_nX2, m_nY1, m_nY2;
	...
	void  _AddDebugBlock(i32 x, i32 y, i32 sizeX, i32 sizeY);
