...
struct SCommandBuffer
{
	...
	DRX_ALIGN(64) char m_buffer[COMMAND_BUFFER_SIZE];
...
struct SBindOffsetCB : public SJob
{
	...
	size_t Process()
	{
		...
		;
	#else
		DrxFatalError("D3D11.1 API functions are not supported on this platform");
	#endif
...
static void ExecuteAsyncDIPJob(char* iter, char* end, volatile int* finished)
{
	...
	DrxInterlockedDecrement(finished);
}
#endif
}
...
inline void CSubmissionQueue_DX11::BindOffsetConstantBuffersAsync(SHADER_TYPE type)
{
	...
	while (m_CB[type].dirty1)
	{
		...
		pJob->Offsets = (u32*)s_CommandBuffer.Allocate(sizeof(m_CB[type].offsets[0]) * count);
		pJob->Counts = (u32*)s_CommandBuffer.Allocate(sizeof(m_CB[type].sizes[0]) * count);
		for (size_t i = 0; i < count; ++i)
		{
...
inline void CSubmissionQueue_DX11::BindUAVsAsync(SHADER_TYPE type)
{
	...
	for (size_t j = 0; j < MAX_UAV_DIRTY; ++j)
	{
		while (m_UAV[type].dirty[j])
		{
			...
			pJob->counts = (u32*)s_CommandBuffer.Allocate(sizeof(m_UAV[type].counts[0]) * count);
			for (size_t i = 0; i < count; ++i)
			{
...
inline void CSubmissionQueue_DX11::BindIAAsync()
{
	...
	while (m_VBs.dirty)
	{
		...
		pJob->Strides = (u32*)s_CommandBuffer.Allocate(sizeof(m_VBs.strides[0]) * count);
		pJob->Offsets = (u32*)s_CommandBuffer.Allocate(sizeof(m_VBs.offsets[0]) * count);
		for (size_t i = 0; i < count; ++i)
		{
...
inline void CSubmissionQueue_DX11::CommitDeviceStatesAsync()
{
	...
	#if defined(BUFFER_ENABLE_DIRECT_ACCESS) && (DRX_RENDERER_DIRECT3D == 110) && !DRX_RENDERER_GNM
	{
		DETAILED_PROFILE_MARKER("InvalidateBuffers");
		u32k threadid = gRenDev->m_RP.m_nProcessThreadID;
...
inline void CSubmissionQueue_DX11::SubmitAsyncDipJob(char* start, char* end, volatile int* nSyncState)
{
	...
	u32 nFreeQueueSlots = std::abs((int)(*(volatile u32*)(&m_nPull) - m_nPush));
	if (nFreeQueueSlots * 4 > nQueueSize * 3)
	{
		DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_WARNING, "AsyncDip Queue overflow detected; Please increase the queue size to prevent performance issues");
		while (nFreeQueueSlots * 4 > nQueueSize * 3)
		{
			DrxSleep(0);
			nFreeQueueSlots = std::abs((int)(*(volatile u32*)(&m_nPull) - m_nPush));
		}
	}

	// write job params to queue and flush the data
	u32 nPushSlot = m_nPush & (nQueueSize - 1);
	...
	do
	{
		nCurrentState.nValue = *const_cast<volatile u32*>(&gEnv->mAsyncDipState.nValue);
		...
		if (DrxInterlockedCompareExchange((volatile long*)&gEnv->mAsyncDipState.nValue, nNewState.nValue, nCurrentState.nValue) == nCurrentState.nValue)
		{
...
inline void CSubmissionQueue_DX11::ExecuteAsyncDIP()
{
	while (true)
	{
		...
		assert((*(volatile u32*)(&m_nPush) != m_nPull));

		// Execute next Job
		u32 nPullSlot = m_nPull & (nQueueSize - 1);
