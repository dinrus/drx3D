...
struct DRX_ALIGN(16) SStateBlend
{
	uint64 nHashVal;
	...
	{
		u32 hashLow = InDesc.AlphaToCoverageEnable |
		...
		u32 hashHigh = InDesc.RenderTarget[0].RenderTargetWriteMask |
...
struct DRX_ALIGN(16) SStateRaster
{
	uint64 nValuesHash;
	u32 nHashVal;
	...
	static u32 GetHash(const D3D11_RASTERIZER_DESC& InDesc)
	{
		u32 nHash;
...
inline u32 sStencilState(const D3D11_DEPTH_STENCILOP_DESC& Desc)
{
	u32 nST = (Desc.StencilFailOp << 0) |
		(Desc.StencilDepthFailOp << 4) |
		(Desc.StencilPassOp << 8) |
		(Desc.StencilFunc << 12);
	return nST;
}
struct DRX_ALIGN(16) SStateDepth
{
	uint64 nHashVal;
...
class CDeviceStatesUprDX11
{
public:
	CDeviceStatesUprDX11() {}
	~CDeviceStatesUprDX11() {}

	void ShutDown();
	void ReleaseUnusedStates(u32 currentFrameID);

	static CDeviceStatesUprDX11* GetInstance();

	u32 GetOrCreateBlendState(const D3D11_BLEND_DESC& desc);
	u32 GetOrCreateRasterState(const D3D11_RASTERIZER_DESC& rasterizerDec, const bool bAllowMSAA = true);
	u32 GetOrCreateDepthState(const D3D11_DEPTH_STENCIL_DESC& desc);
