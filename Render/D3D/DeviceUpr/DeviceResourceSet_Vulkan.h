...
static NDrxVulkan::CDevice* GetDevice()
{
	return GetDeviceObjectFactory().GetVKDevice();
}

static NDrxVulkan::CCommandScheduler* GetScheduler()
{
...
protected:
	...
	static VkDescriptorType GetDescriptorType(SResourceBindPoint bindPoint)
	{
		...
		{
			if (u8(bindPoint.flags & SResourceBindPoint::EFlags::IsTexture))
			{
				return VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
			}
			else
			{
				return u8(bindPoint.flags & SResourceBindPoint::EFlags::IsStructured)
...
public:
	...
	const std::vector<u8>& GetEncodedLayout()    const { return *GetDeviceObjectFactory().LookupResourceLayoutEncoding(m_hash); }



protected:
	std::vector<u8>        EncodeDescriptorSet(const VectorMap<SResourceBindPoint, SResourceBinding>& resources);
