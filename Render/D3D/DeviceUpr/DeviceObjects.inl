...
template <class Impl> inline u32 CDeviceTimestampGroup_Base<Impl>::IssueTimestamp()
{
...
template <class Impl> inline float CDeviceTimestampGroup_Base<Impl>::GetTimeMS(u32 timestamp0, u32 timestamp1)
{
...
inline void CDeviceObjectFactory::ExtractBasePointer(D3DBuffer* buffer, D3D11_MAP mode, u8*& base_ptr)
{
#if BUFFER_ENABLE_DIRECT_ACCESS
	#if DRX_RENDERER_GNM
		base_ptr = buffer->GnmGetBaseAddress();
	#elif DRX_PLATFORM_ORBIS
		base_ptr = (u8*)buffer->GetData();
		...
		assert(hr == S_OK);
		base_ptr = reinterpret_cast<u8*>(data);
...
inline void CDeviceObjectFactory::ReleaseBasePointer(D3DBuffer* buffer)
{
	...
	#elif defined(DRX_RENDERER_VULKAN)
		buffer->Unmap();
	#endif
#endif
}

inline u8 CDeviceObjectFactory::MarkReadRange(D3DBuffer* buffer, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode)
{
#if (DRX_RENDERER_DIRECT3D >= 120)
...
inline u8 CDeviceObjectFactory::MarkWriteRange(D3DBuffer* buffer, buffer_size_t offset, buffer_size_t size, u8 marker)
{
#if (DRX_RENDERER_DIRECT3D >= 120)
