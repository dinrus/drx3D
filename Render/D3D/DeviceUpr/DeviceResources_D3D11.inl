	...
	if (pView.m_Desc.eViewType == SResourceView::eShaderResourceView)
	{
		...
		if (pView.m_Desc.bMultisample && m_eTT == eTT_2D)
		{
			...
			if (nSliceAvailable > 1)
			{
				...
				DRX_ASSERT(srvDesc.Texture2DMSArray.ArraySize == ~0 || (srvDesc.Texture2DMSArray.ArraySize > 0 && srvDesc.Texture2DMSArray.ArraySize <= nSliceAvailable));
			}
		}
		else if (m_bFilterable)
		{
			...
			DRX_ASSERT(srvDesc.Texture1D.MipLevels == ~0 || (srvDesc.Texture1D.MipLevels > 0 && srvDesc.Texture1D.MipLevels <= nMipAvailable));
			...
			if (nSliceAvailable > 1)
			{
				...
				DRX_ASSERT(srvDesc.Texture1DArray.ArraySize == ~0 || (srvDesc.Texture1DArray.ArraySize > 0 && srvDesc.Texture1DArray.ArraySize <= nSliceAvailable));
			}
		}
		else
		{
			...
			if (nElemsAvailable > 0)
			{
				...
				DRX_ASSERT(srvDesc.Buffer.NumElements > 0 && srvDesc.Buffer.NumElements <= nElemsAvailable);
			}
		}
	...
	else // SResourceView::eRenderTargetView || SResourceView::eDepthStencilView || SResourceView::eUnorderedAccessView)
	{
		...
		if (pView.m_Desc.bMultisample && m_eTT == eTT_2D && pView.m_Desc.eViewType != SResourceView::eUnorderedAccessView)
		{
			...
			if (nSliceAvailable > 1)
			{
				...
				DRX_ASSERT(rtvDesc.Texture2DMSArray.ArraySize == ~0 || (rtvDesc.Texture2DMSArray.ArraySize > 0 && rtvDesc.Texture2DMSArray.ArraySize <= nSliceAvailable));
			}
		}
		else if (m_bFilterable)
		{
			...
			if (nSliceAvailable > 1 || m_eTT == eTT_3D || m_eTT == eTT_Cube || m_eTT == eTT_CubeArray)
			{
				...
				DRX_ASSERT(rtvDesc.Texture1DArray.ArraySize == ~0 || (rtvDesc.Texture1DArray.ArraySize > 0 && rtvDesc.Texture1DArray.ArraySize <= nSliceAvailable));
			}
		}
		else
		{
			...
			if (nElemsAvailable > 0)
			{
				...
				DRX_ASSERT(rtvDesc.Buffer.NumElements > 0 && rtvDesc.Buffer.NumElements <= nElemsAvailable);
			}
		}
...
template<typename T>
static inline u32 ConvertFromDX11Flags(const T& desc)
{
	// *INDENT-OFF*
	return
...
CDeviceResource::ESubstitutionResult CDeviceResource::SubstituteUsedResource()
{
	...
	const auto& fenceUpr = rFactory.GetDX11Scheduler()->GetFenceUpr();

	// NOTE: Poor man's resource tracking (take current time as last-used moment)
	HRESULT hResult = rFactory.GetDX11Device()->SubstituteUsedCommittedResource(fenceUpr.GetCurrentValues(), &m_pNativeResource);
...
void CDeviceTexture::UploadFromStagingResource(u32k nSubRes, StagingHook cbTransfer)
{
	D3DResource* pStagingResource;
...
void CDeviceTexture::UploadFromStagingResource(u32k nSubRes)
{
...
#if DEVRES_USE_PINNING && !DRX_PLATFORM_DURANGO
