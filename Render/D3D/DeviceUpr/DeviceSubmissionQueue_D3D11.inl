...
#ifndef _DeviceUprInline_H_
#define _DeviceUprInline_H_

#include "../Common/DevBuffer.h" // CConstantBuffer

#if DEVICE_MANAGER_IMMEDIATE_STATE_WRITE
	#include "../../DriverD3D.h"
	#include "DeviceWrapper_D3D11.h" // CDrxDeviceContextWrapper
#endif

//===============================================================================================================
//
//  Inline implementation for CDeviceUpr
//
//===============================================================================================================

namespace DevManUtil
{
#if DRX_PLATFORM_ORBIS || DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID || DRX_PLATFORM_APPLE // BitScanReverse/BitScanForward are not avilable on orbis
static inline u32 bsr(u32 input)
{
...
static inline u32 bsf(u32 input)
{
	signed mask = iszero((signed)input) - 1;
...
inline void CSubmissionQueue_DX11::BindConstantBuffer(SHADER_TYPE type, const CConstantBuffer* pBuffer, u32 slot)
{
#if CONSTANT_BUFFER_ENABLE_DIRECT_ACCESS == 1
	BindConstantBuffer(type, pBuffer, slot, 0, 0);
	return;
#endif
	u32 _offset = 0, _size = 0;
...
inline void CSubmissionQueue_DX11::BindConstantBuffer(SHADER_TYPE type, const CConstantBuffer* pBuffer, u32 slot, buffer_size_t offset, buffer_size_t size)
{
...
#if DRX_RENDER_DIRECT3D && (DRX_RENDERER_DIRECT3D < 111)
	assert(_offset == 0);
	BindConstantBuffer(type, pBuffer, slot);
#elif DEVICE_MANAGER_IMMEDIATE_STATE_WRITE
	CDrxDeviceContextWrapper& rDeviceContext = gcpRendD3D->GetDeviceContext_Unsynchronized();
	u32 offsetArray[1] = { static_cast<u32>(_offset) };
	u32 sizeArray[1] = { static_cast<u32>(_size) };
#if DEVICE_MANAGER_USE_TYPE_DELEGATES
...
inline void CSubmissionQueue_DX11::BindCBs(SHADER_TYPE type, const CConstantBuffer* const* Buffer, u32 start_slot, u32 count)
{
...
inline void CSubmissionQueue_DX11::BindConstantBuffer(SHADER_TYPE type, D3DBuffer* pBuffer, u32 slot)
{
#if DEVICE_MANAGER_IMMEDIATE_STATE_WRITE
	CDrxDeviceContextWrapper& rDeviceContext = gcpRendD3D->GetDeviceContext_Unsynchronized();
#if DEVICE_MANAGER_USE_TYPE_DELEGATES
...
inline void CSubmissionQueue_DX11::BindConstantBuffer(SHADER_TYPE type, D3DBuffer* pBuffer, u32 slot, u32 offset, u32 size)
{
	u32 _offset = 0, _size = 0;
...
inline void CSubmissionQueue_DX11::BindCBs(SHADER_TYPE type, D3DBuffer* const* Buffer, u32 start_slot, u32 count)
{
	for (unsigned i = 0; i < count; ++i)
	{
		BindConstantBuffer(type, Buffer[i], start_slot + i);
...
inline void CSubmissionQueue_DX11::BindSRV(SHADER_TYPE type, D3DShaderResource* SRV, u32 slot)
{
#if DEVICE_MANAGER_IMMEDIATE_STATE_WRITE
	CDrxDeviceContextWrapper& rDeviceContext = gcpRendD3D->GetDeviceContext_Unsynchronized();
#if DEVICE_MANAGER_USE_TYPE_DELEGATES
...
inline void CSubmissionQueue_DX11::BindSRV(SHADER_TYPE type, D3DShaderResource* const* SRV, u32 start_slot, u32 count)
{
	for (unsigned i = 0; i < count; ++i)
	{
...
inline void CSubmissionQueue_DX11::BindUAV(SHADER_TYPE type, D3DUAV* UAV, u32 count, u32 slot)
{
#if DEVICE_MANAGER_IMMEDIATE_STATE_WRITE
	CDrxDeviceContextWrapper& rDeviceContext = gcpRendD3D->GetDeviceContext_Unsynchronized();
	switch (type)
...
inline void CSubmissionQueue_DX11::BindUAV(SHADER_TYPE type, D3DUAV* const* UAV, u32k* counts, u32 start_slot, u32 count)
{
	for (unsigned i = 0; i < count; ++i)
	{
...
inline void CSubmissionQueue_DX11::BindSampler(SHADER_TYPE type, D3DSamplerState* Sampler, u32 slot)
{
#if DEVICE_MANAGER_IMMEDIATE_STATE_WRITE
	CDrxDeviceContextWrapper& rDeviceContext = gcpRendD3D->GetDeviceContext_Unsynchronized();
#if DEVICE_MANAGER_USE_TYPE_DELEGATES
...
inline void CSubmissionQueue_DX11::BindSampler(SHADER_TYPE type, D3DSamplerState* const* Samplers, u32 start_slot, u32 count)
{
	for (unsigned i = 0; i < count; ++i)
	{
...
inline void CSubmissionQueue_DX11::SetDepthStencilState(ID3D11DepthStencilState* dss, u32 stencilref)
{
#if DEVICE_MANAGER_IMMEDIATE_STATE_WRITE
	CDrxDeviceContextWrapper& rDeviceContext = gcpRendD3D->GetDeviceContext_Unsynchronized();
...
inline void CSubmissionQueue_DX11::SetBlendState(ID3D11BlendState* pBlendState, float* BlendFactor, u32 SampleMask)
{
#if DEVICE_MANAGER_IMMEDIATE_STATE_WRITE
	CDrxDeviceContextWrapper& rDeviceContext = gcpRendD3D->GetDeviceContext_Unsynchronized();
...
#if !defined(RENDERER_ENABLE_LEGACY_PIPELINE) && (!defined(DRX_RENDERER_DIRECT3D) || defined(DRX_RENDERER_GNM))
#if !DEVICE_MANAGER_IMMEDIATE_STATE_WRITE
