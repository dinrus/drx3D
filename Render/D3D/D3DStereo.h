...
private:
	STxt          profileMarkerTag;
	SDisplayContextKey   displayContextKey;
	SDisplayContextKey   previousDisplayContextKey;

	CStereoRenderingScope(STxt &&profileMarkerTag, SDisplayContextKey displayContextKey);
...
public:
	...
	void              ReleaseBuffers();
	void              OnResolutionChanged(i32 newWidth, i32 newHeight);
	void              CalculateResolution(i32 requestedWidth, i32 requestedHeight, i32* pRenderWidth, i32 *pRenderHeight);
	void              OnStereoModeChanged();

	SStereoRenderContext PrepareStereoRenderingContext(i32 nFlags, const SRenderingPassInfo& passInfo) const;

	void              ProcessScene(i32 sceneFlags, const SRenderingPassInfo& passInfo);
	...
	_smart_ptr<CTexture> WrapD3DRenderTarget(D3DTexture* d3dTexture, u32 width, u32 height, DXGI_FORMAT format, tukk name, bool shaderResourceView);
...
public:
	...
	Vec2_tpl<i32>      GetOverlayResolution() const override final { return { m_headlockedQuadCamera.GetViewSurfaceX(), m_headlockedQuadCamera.GetViewSurfaceZ() }; }
	// ~IStereoRenderer Interface
...
private:
	...
	CCamera       PrepareCamera(i32 nEye, const CCamera& currentCamera);
	...
	void          RecreateDisplayContext(std::pair<std::shared_ptr<CCustomRenderDisplayContext>, SDisplayContextKey> &target, STxt name, const ColorF &clearColor, std::vector<_smart_ptr<CTexture>> &&swapChain);
