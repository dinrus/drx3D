...
// -------------------------------------------------------------------------
//  Имя файла:   GLPlatform.hpp
//  Version:     v1.00
//  Created:     27/03/2014 by Valerio Guagliumi.
//  Описание: Platform specific DXGL requirements implementation
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
#if 1 // DRX_RENDERER_OPENGL
	#include <drx3D/Render/GLDrxPlatform.hpp>
#elif DRX_PLATFORM_WINDOWS
	#include <drx3D/Render/Standalone/GLWinPlatform.hpp>
#elif DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID
	#include <drx3D/Render/Standalone/GLLinuxPlatform.hpp>
#else
...
#if defined(_MSC_VER)
	#define DXGL_WARNING(_Format, ...) NDrxOpenGL::LogMessage(NDrxOpenGL::eLS_Warning, DXGL_LOG_MSG(_Format), __VA_ARGS__)
	#define DXGL_ERROR(_Format, ...)   NDrxOpenGL::LogMessage(NDrxOpenGL::eLS_Error, DXGL_LOG_MSG(_Format), __VA_ARGS__)
	#define DXGL_INFO(_Format, ...)    NDrxOpenGL::LogMessage(NDrxOpenGL::eLS_Info, DXGL_LOG_MSG(_Format), __VA_ARGS__)
	#define DXGL_FATAL(_Format, ...)   NDrxOpenGL::LogMessage(NDrxOpenGL::eLS_Fatal, DXGL_LOG_MSG(_Format), __VA_ARGS__)
#else
	#define DXGL_WARNING(_Format, ...) NDrxOpenGL::LogMessage(NDrxOpenGL::eLS_Warning, DXGL_LOG_MSG(_Format), ## __VA_ARGS__, __LINE__)
	#define DXGL_ERROR(_Format, ...)   NDrxOpenGL::LogMessage(NDrxOpenGL::eLS_Error, DXGL_LOG_MSG(_Format), ## __VA_ARGS__, __LINE__)
	#define DXGL_INFO(_Format, ...)    NDrxOpenGL::LogMessage(NDrxOpenGL::eLS_Info, DXGL_LOG_MSG(_Format), ## __VA_ARGS__, __LINE__)
	#define DXGL_FATAL(_Format, ...)   NDrxOpenGL::LogMessage(NDrxOpenGL::eLS_Fatal, DXGL_LOG_MSG(_Format), ## __VA_ARGS__, __LINE__)
#endif

#define DXGL_NOT_IMPLEMENTED NDrxOpenGL::BreakUnique(__FILE__, __LINE__);
#define DXGL_TODO(_DESC)

#define DXGL_ARRAY_SIZE(_Array) (DRX_ARRAY_COUNT(_Array))
...
#define DXGL_DECLARE_REF_COUNTED(_Qualifier, _TypeName) \
  ...
  _Qualifier _TypeName: public NDrxOpenGL::SRefCounted < _TypeName >
...
struct SSpinlockBitMaskWord
{
	...
	bool Set(u32 uIndex, bool bFlag)
	{
		while (true)
		{
			Word kOld(m_aMask);
...
template<u32 uNumWords, typename BitMaskWord>
...
struct SBitMaskImpl
{
	...
	{
		for (u32 uPosition = 0; uPosition < uNumWords; ++uPosition)
			pMask->m_akWords[uPosition].Set((Word)0);
	}

	static void SetOne(Type* pMask, Word kUsedMask)
	{
		for (u32 uPosition = 0; uPosition < uNumWords - 1; ++uPosition)
			pMask->m_akWords[uPosition].Set(~(Word)0);
	...
	{
		for (u32 uIndex = 0; uIndex < uNumWords; ++uIndex)
...
template<u32 uSize, typename BitMaskWord>
struct SBitMask : SBitMaskImpl<(uSize + BitMaskWord::BIT_SIZE - 1) / BitMaskWord::BIT_SIZE, BitMaskWord>::Type
{
	typedef SBitMaskImpl<(uSize + BitMaskWord::BIT_SIZE - 1) / BitMaskWord::BIT_SIZE, BitMaskWord> Impl;
	typedef typename BitMaskWord::Word                                                             Word;
	enum { USED_MASK = ~(~(Word)0 << uSize % BitMaskWord::BIT_SIZE) };
...
inline uk GetTLSValue(uk pTLSHandle)
{
#if DRX_PLATFORM_WINDOWS
...
inline void SetTLSValue(uk pTLSHandle, uk pValue)
{
#if DRX_PLATFORM_WINDOWS
...
struct SList
{
	...
	SList()
	{
		DrxInitializeSListHead(m_kHeader);
	}

	void Push(TListEntry& kEntry)
	{
		DrxInterlockedPushEntrySList(m_kHeader, kEntry);
	}

	TListEntry* Pop()
	{
		return alias_cast<TListEntry*>(DrxInterlockedPopEntrySList(m_kHeader));
	}
...
struct SScopedProfileLabel
{
	...
	SScopedProfileLabel(tukk szName)
		: m_szName(szName)
	{
...
#if DXGL_PROFILING
	#define DXGL_SCOPED_PROFILE(_Name) NDrxOpenGL::SScopedProfileLabel kDXGLScopedProfileLabel(_Name);
#else
