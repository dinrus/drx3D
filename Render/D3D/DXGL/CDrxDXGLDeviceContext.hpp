...
// -------------------------------------------------------------------------
//  Имя файла:   CDrxDXGLDeviceContext.hpp
//  Version:     v1.00
//  Created:     20/02/2013 by Valerio Guagliumi.
//  Описание: Declaration of the DXGL wrapper for ID3D11DeviceContext
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////

#ifndef __DRXDXGLDEVICECONTEXT__
#define __DRXDXGLDEVICECONTEXT__
...
class CDrxDXGLBlendState;
class CDrxDXGLBuffer;
class CDrxDXGLDepthStencilState;
class CDrxDXGLDepthStencilView;
class CDrxDXGLInputLayout;
class CDrxDXGLQuery;
class CDrxDXGLRasterizerState;
class CDrxDXGLRenderTargetView;
class CDrxDXGLSamplerState;
class CDrxDXGLShader;
class CDrxDXGLShaderResourceView;
class CDrxDXGLUnorderedAccessView;

class CDrxDXGLDeviceContext
#if DXGL_VIRTUAL_DEVICE_AND_CONTEXT && !DXGL_FULL_EMULATION
	: public ID3D11DeviceContext
#else
	: public CDrxDXGLDeviceChild
#endif
{
public:
	DXGL_IMPLEMENT_INTERFACE(CDrxDXGLDeviceContext, D3D11DeviceContext)

	CDrxDXGLDeviceContext();
	virtual ~CDrxDXGLDeviceContext();

	bool                  Initialize(CDrxDXGLDevice* pDevice);
	void                  Shutdown();

	NDrxOpenGL::CContext* GetGLContext();
	...
	HRESULT STDMETHODCALLTYPE                   GetData(ID3D11Asynchronous* pAsync, uk pData, UINT DataSize, UINT GetDataFlags);
	...
	void STDMETHODCALLTYPE                      UpdateSubresource(ID3D11Resource* pDstResource, UINT DstSubresource, const D3D11_BOX* pDstBox, ukk pSrcData, UINT SrcRowPitch, UINT SrcDepthPitch);
...
protected:
	static _smart_ptr<CDrxDXGLBlendState>        CreateDefaultBlendState(CDrxDXGLDevice* pDevice);
	static _smart_ptr<CDrxDXGLDepthStencilState> CreateDefaultDepthStencilState(CDrxDXGLDevice* pDevice);
	static _smart_ptr<CDrxDXGLRasterizerState>   CreateDefaultRasterizerState(CDrxDXGLDevice* pDevice);
	static _smart_ptr<CDrxDXGLSamplerState>      CreateDefaultSamplerState(CDrxDXGLDevice* pDevice);

	struct SStage
	{
		_smart_ptr<CDrxDXGLShader>              m_spShader;
		_smart_ptr<CDrxDXGLSamplerState>        m_aspSamplerStates[D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT];
		_smart_ptr<CDrxDXGLShaderResourceView>  m_aspShaderResourceViews[D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT];
		_smart_ptr<CDrxDXGLUnorderedAccessView> m_aspUnorderedAccessViews[D3D11_1_UAV_SLOT_COUNT];
		_smart_ptr<CDrxDXGLBuffer>              m_aspConstantBuffers[D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT];
		u32 m_auConstantBufferOffsets[D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT];
		u32 m_auConstantBufferSizes[D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT];
	};

	void SetShaderResources(u32 uStage, UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView* const* ppShaderResourceViews);
	void SetUnorderedAccessViews(u32 uStage, UINT StartSlot, UINT NumViews, ID3D11UnorderedAccessView* const* ppUnorderedAccessViews);
	void SetShader(u32 uStage, CDrxDXGLShader* pShader, ID3D11ClassInstance* const* ppClassInstances, UINT NumClassInstances);
	void SetSamplers(u32 uStage, UINT StartSlot, UINT NumSamplers, ID3D11SamplerState* const* ppSamplers);
	void SetConstantBuffers(u32 uStage, UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppConstantBuffers, const UINT* pFirstConstant = NULL, const UINT* pNumConstants = NULL);

	void GetShaderResources(u32 uStage, UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView** ppShaderResourceViews);
	void GetUnorderedAccesses(u32 uStage, UINT StartSlot, UINT NumViews, ID3D11UnorderedAccessView** ppUnorderedAccessViews);
	void GetShader(u32 uStage, CDrxDXGLShader** pShader, ID3D11ClassInstance** ppClassInstances, UINT* NumClassInstances);
	void GetSamplers(u32 uStage, UINT StartSlot, UINT NumSamplers, ID3D11SamplerState** ppSamplers);
	void GetConstantBuffers(u32 uStage, UINT StartSlot, UINT NumBuffers, ID3D11Buffer** ppConstantBuffers, UINT* pFirstConstant = NULL, UINT* pNumConstants = NULL);
protected:
	NDrxOpenGL::CContext*                 m_pContext;
	_smart_ptr<CDrxDXGLBlendState>        m_spBlendState;
	_smart_ptr<CDrxDXGLDepthStencilState> m_spDepthStencilState;
	_smart_ptr<CDrxDXGLRasterizerState>   m_spRasterizerState;
	...
	_smart_ptr<CDrxDXGLBuffer>            m_aspVertexBuffers[D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT];
	u32                                m_auVertexBufferStrides[D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT];
	u32                                m_auVertexBufferOffsets[D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT];
	_smart_ptr<CDrxDXGLInputLayout>       m_spInputLayout;
	_smart_ptr<CDrxDXGLBuffer>            m_spIndexBuffer;
	DXGI_FORMAT                           m_eIndexBufferFormat;
	u32                                m_uIndexBufferOffset;
	D3D11_PRIMITIVE_TOPOLOGY              m_ePrimitiveTopology;
	_smart_ptr<CDrxDXGLRenderTargetView>  m_aspRenderTargetViews[D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT];
	_smart_ptr<CDrxDXGLDepthStencilView>  m_spDepthStencilView;
	u32                                m_uNumViewports;
	D3D11_VIEWPORT                        m_akViewports[D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE];
	u32                                m_uNumScissorRects;
	D3D11_RECT                            m_akScissorRects[D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE];
	_smart_ptr<CDrxDXGLQuery>             m_spPredicate;
	bool m_bPredicateValue;
	_smart_ptr<CDrxDXGLBuffer>            m_aspStreamOutputBuffers[D3D11_SO_BUFFER_SLOT_COUNT];
	u32                                m_auStreamOutputBufferOffsets[D3D11_SO_BUFFER_SLOT_COUNT];

	_smart_ptr<CDrxDXGLBlendState>        m_spDefaultBlendState;
	_smart_ptr<CDrxDXGLDepthStencilState> m_spDefaultDepthStencilState;
	_smart_ptr<CDrxDXGLRasterizerState>   m_spDefaultRasterizerState;
	_smart_ptr<CDrxDXGLSamplerState>      m_spDefaultSamplerState;
};

#endif //__DRXDXGLDEVICECONTEXT__
