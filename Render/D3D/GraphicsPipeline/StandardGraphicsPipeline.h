...
public:
	...
	void GeneratePerViewConstantBuffer(const SRenderViewInfo* pViewInfo, i32 viewInfoCount, CConstantBufferPtr pPerViewBuffer,const SRenderViewport* pCustomViewport = nullptr);
	...
	void   ExecutePostAA();
	void   ExecuteAnisotropicVerticalBlur(CTexture* pTex, i32 nAmount, float fScale, float fDistribution, bool bAlphaOnly);

	u32 IncrementNumInvalidDrawcalls(i32 count) { return DrxInterlockedAdd(( i32*)&m_numInvalidDrawcalls, count); }
	u32 GetNumInvalidDrawcalls() const          { return m_numInvalidDrawcalls;   }
	...
	u32 GetRenderFlags() const { return m_renderingFlags; }
