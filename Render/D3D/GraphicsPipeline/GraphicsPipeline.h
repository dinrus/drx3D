...
#include <drx3D/Render/RenderPassScheduler.h>
...
public:
	...
	CGraphicsPipelineStage* GetStage(u32 stageID)
	{
	...
	template<class T> T* GetOrCreateUtilityPass()
	{
		...
		auto& cache = m_utilityPassCaches[u32(id)];
...
protected:
	template<class T> void RegisterStage(T*& pPipelineStage, u32 stageID)
	{
		assert(stageID < m_pipelineStages.size());
		assert(m_pipelineStages[stageID] == nullptr);
	...
	template<class T, u32 stageID> void RegisterSceneStage(T*& pPipelineStage)
	{
	...
	void ResizeStages(i32 renderWidth, i32 renderHeight)
	{
		for (auto pStage : m_pipelineStages)
		{
			if (pStage)
...
private:
	std::array<SUtilityPassCache, u32(IUtilityRenderPass::EPassId::MaxPassCount)> m_utilityPassCaches;
