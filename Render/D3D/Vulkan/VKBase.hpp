...
#include <drx3D/Plugins/concqueue/concqueue.hpp>
	...
	#define VK_LOG(cond, ...) \
	  do { if (cond) DrxLog("Vulkan Log: " __VA_ARGS__); } while (0)
	#define VK_ERROR(...) \
	  do { DrxLog("Vulkan Error: " __VA_ARGS__); } while (0)
	#define VK_WARNING(...) \
	  do { DrxLog("Vulkan Warning: " __VA_ARGS__); } while (0)
	#define VK_FUNC_LOG() \
	  do { if (VK_FUNCTION_LOGGING) DrxLog("Vulkan function call: %s", __FUNC__); } while (0)

	#define VK_ASSERT(cond, ...) \
	  DRX_ASSERT_MESSAGE(cond, "VK_ASSERT " __VA_ARGS__)
...
struct SSurfaceCreationInfo
{
	...
	HWND      windowHandle;
	HINSTANCE appHandle;
#elif USE_SDL2_VIDEO && DRX_PLATFORM_ANDROID
	SDL_Window* pWindow;
...
public:
	...
	{
		return (ULONG)DrxInterlockedIncrement(&m_refCount);
	}

	virtual ULONG Release() final threadsafe
	{
		i32k newRefCount = DrxInterlockedDecrement(&m_refCount);
		if (newRefCount == 0)
		{
			Destroy();
		}
	...
	virtual HRESULT QueryInterface(REFIID riid, uk * ppvObject) final
	{
		*ppvObject = nullptr;
		return E_FAIL;
	}
...
public:
	...
	HRESULT SetPrivateData(REFGUID, UINT, ukk ); // Legacy, to be removed!
