...
#include <drx3D/Render/D3D/Vulkan/VKCommandListFence.hpp>
...
#define CMDLIST_GRAPHICS      NDrxVulkan::CCommandListPool::m_MapQueueType[CMDQUEUE_GRAPHICS]
#define CMDLIST_COMPUTE       NDrxVulkan::CCommandListPool::m_MapQueueType[CMDQUEUE_COMPUTE]
#define CMDLIST_COPY          NDrxVulkan::CCommandListPool::m_MapQueueType[CMDQUEUE_COPY]
...
public:
	...
	void AcquireCommandLists(u32 numCLs, VK_PTR(CCommandList)* results) threadsafe;
	void ForfeitCommandLists(u32 numCLs, VK_PTR(CCommandList)* results, bool bWait = false) threadsafe;
	...
	ILINE u32 GetVkQueueFamily() threadsafe_const
	{
		return m_nPoolFamily;
	}

	ILINE u32 GetVkQueueIndex() threadsafe_const
	{
		return m_nPoolIndex;
	}
	...
	ILINE NDrxVulkan::CAsyncCommandQueue& GetAsyncCommandQueue() threadsafe
	{
...
public:
	...
	ILINE void WaitForFenceOnGPU(const UINT64 fenceValue, i32k id, INT& numPendingWSemaphores, std::array<VkSemaphore, 64>& PendingWSemaphoreHeap) threadsafe
	{
	...
	ILINE void WaitForFenceOnGPU(const VkSemaphore semaphoreObject, i32k id, INT& numPendingWSemaphores, std::array<VkSemaphore, 64>& PendingWSemaphoreHeap) threadsafe
	{
		// Under Vulkan it's not possible to Reset(0 GPU-only semaphores from the CPU-side, or through a command-list command except vkQueueSubmit().
		// Which means we are forced to pass through all "waits" even though we know they are nops.
...
private:
	...
	VkQueue                        m_CmdQueue;
	NDrxVulkan::CAsyncCommandQueue m_AsyncCommandQueue;
...
public:
	...
	static std::pair<u32, u32> m_MapQueueBits[1U << 4];
	static u32 m_MapQueueIndices[32];
};
