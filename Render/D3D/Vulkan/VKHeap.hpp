...
#include <drx3D/Render/D3D/GpuHeap.h>
...
private:
	...
	static uk VKAPI_PTR CpuMalloc(uk pUserData, size_t size, size_t alignment, VkSystemAllocationScope scope);
	static uk VKAPI_PTR CpuRealloc(uk pUserData, uk pOriginal, size_t size, size_t alignment, VkSystemAllocationScope scope);
	static void VKAPI_PTR  CpuFree(uk pUserData, uk pMemory);
	static void VKAPI_PTR  CpuForeignMalloc(uk pUserData, size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope);
	static void VKAPI_PTR  CpuForeignFree(uk pUserData, size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope);

#if !defined(_RELEASE)
	static const size_t        kNumPools = VK_SYSTEM_ALLOCATION_SCOPE_RANGE_SIZE + VK_INTERNAL_ALLOCATION_TYPE_RANGE_SIZE;
	 struct SPoolStats m_stats[kNumPools];

	// Access stats structure for some pool.
	 SPoolStats& AtStats(VkSystemAllocationScope scope) { return m_stats[scope]; }
	 SPoolStats& AtStats(VkInternalAllocationType type) { return m_stats[type + VK_SYSTEM_ALLOCATION_SCOPE_RANGE_SIZE]; }
#endif
};
...
public:
	...
	{
		return CGpuHeap::GetAdjustedSize(static_cast<u32>(requirements.size), static_cast<u32>(requirements.alignment));
	}

	// Maps memory for CPU access
	uk Map(CMemoryHandle& handle)
	{
...
private:
	...
	virtual uk        MapBlock(uint32_t memoryType, TBlockHandle blockHandle) override;
	virtual void         UnmapBlock(uint32_t memoryType, TBlockHandle blockHandle, uk pAddress) override;

	static tukk   FormatSize(char buffer[10], uint64_t bytes);
