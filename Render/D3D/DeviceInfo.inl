...
#if !DRX_RENDERER_OPENGL && !DRX_RENDERER_VULKAN && !DRX_RENDERER_GNM
#if DRX_RENDERER_DIRECT3D >= 120
	ZeroStruct(m_D3D120aOptions);
	ZeroStruct(m_D3D120bOptions);
	#if DRX_RENDERER_DIRECT3D >= 121
...
bool GetForcedFeatureLevel(D3D_FEATURE_LEVEL* pForcedFeatureLevel)
{
	...
	FeatureLevel* pLevel;
	for (pLevel = s_aLevels; pLevel < s_aLevels + DRX_ARRAY_COUNT(s_aLevels); ++pLevel)
	{
	...
	DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_WARNING, "Invalid value for d3d11_forcedFeatureLevel %s - using available feature level", szForcedName);
	return false;
}
...
bool DeviceInfo::CreateDevice(int zbpp, OnCreateDeviceCallback pCreateDeviceCallback, CreateWindowCallback pCreateWindowCallback)
{
...
#if !defined(_RELEASE)
			D3D_FEATURE_LEVEL eForcedFeatureLevel(DRX_RENDERER_DIRECT3D_FL);
				...
				if (pAdapter)
				{
					...
					if (SUCCEEDED(hr) && pDevice && pContext)
					{
						...
						if (!m_D3D110aOptions.MapNoOverwriteOnDynamicConstantBuffer)
							DrxFatalError("D3D11.1 feature 'MapNoOverwriteOnDynamicConstantBuffer' is required!");
#endif
	...
	{
		DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_ERROR, "DeviceInfo::CreateDevice() failed");
		Release();
		return false;
	}
...
void DeviceInfo::PushSystemEvent(ESystemEvent event, UINT_PTR wParam, UINT_PTR lParam)
{
	...
	#endif
	DrxAutoCriticalSection lock(m_msgQueueLock);
