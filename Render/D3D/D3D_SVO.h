...
public:
	...
	i32                  GetIntegratioMode() const;
	i32                  GetIntegratioMode(bool& bSpecTracingInUse) const;
	...
	static bool          SetShaderParameters(float*& pSrc, u32 paramType, UFloat4* sData);
...
protected:
	...
	static CTexture*       GetGBuffer(i32 nId);
	...
	void                   CheckCreateUpdateRT(_smart_ptr<CTexture>& pTex, i32 nWidth, i32 nHeight, ETEX_Format eTF, ETEX_Type eTT, i32 nTexFlags, tukk szName);

	void                   UpdateGpuVoxParams(I3DEngine::SSvoNodeInfo& nodeInfo);
	void                   ExecuteComputeShader(tukk szTechFinalName, CSvoComputePass& rp, i32* nNodesForUpdateStartIndex, i32 nObjPassId, PodArray<I3DEngine::SSvoNodeInfo>& arrNodesForUpdate);
	uint64                 GetRunTimeFlags(bool bDiffuseMode = true, bool bPixelShader = true);
	template<class T> void SetupSvoTexturesForRead(I3DEngine::SSvoStaticTexInfo& texInfo, T& rp, i32 nStage, i32 nStageOpa = 0, i32 nStageNorm = 0);
	void                   SetupNodesForUpdate(i32& nNodesForUpdateStartIndex, PodArray<I3DEngine::SSvoNodeInfo>& arrNodesForUpdate, CSvoComputePass& rp);
	...
	static i32k                  SVO_MAX_NODE_GROUPS = 4;
	float                             m_arrNodesForUpdate[SVO_MAX_NODE_GROUPS][4][4];
	i32                               m_nCurPropagationPassID;
	...
	#define INIT_ALL_SVO_CVARS                                      \
	  INIT_SVO_CVAR(i32, e_svoDVR);                                 \
	  INIT_SVO_CVAR(i32, e_svoEnabled);                             \
	  INIT_SVO_CVAR(i32, e_svoRender);                              \
	  INIT_SVO_CVAR(i32, e_svoTI_ResScaleBase);                     \
	  INIT_SVO_CVAR(i32, e_svoTI_ResScaleAir);                      \
	  INIT_SVO_CVAR(i32, e_svoTI_Active);                           \
	  INIT_SVO_CVAR(i32, e_svoTI_IntegrationMode);                  \
	  ...
	  INIT_SVO_CVAR(i32, e_svoTI_NumberOfBounces);                  \
	  ...
	  INIT_SVO_CVAR(i32, e_svoTI_LowSpecMode);                      \
	  INIT_SVO_CVAR(i32, e_svoTI_HalfresKernelPrimary);             \
	  INIT_SVO_CVAR(i32, e_svoTI_HalfresKernelSecondary);           \
	  INIT_SVO_CVAR(i32, e_svoVoxelPoolResolution);                 \
	  INIT_SVO_CVAR(i32, e_svoTI_Apply);                            \
	  INIT_SVO_CVAR(float, e_svoTI_Diffuse_Spr);                    \
	  INIT_SVO_CVAR(i32, e_svoTI_Diffuse_Cache);                    \
	  INIT_SVO_CVAR(i32, e_svoTI_SpecularFromDiffuse);              \
	  INIT_SVO_CVAR(i32, e_svoTI_DynLights);                        \
	  INIT_SVO_CVAR(i32, e_svoTI_ShadowsFromSun);                   \
	  INIT_SVO_CVAR(i32, e_svoTI_ShadowsFromHeightmap);             \
	  INIT_SVO_CVAR(i32, e_svoTI_Troposphere_Active);               \
	  ...
	  INIT_SVO_CVAR(i32, e_svoTI_SunRSMInject);                     \
	  INIT_SVO_CVAR(i32, e_svoDispatchX);                           \
	  INIT_SVO_CVAR(i32, e_svoDispatchY);                           \
	  INIT_SVO_CVAR(i32, e_svoDebug);                               \
	  INIT_SVO_CVAR(i32, e_svoVoxGenRes);                           \
	  INIT_SVO_CVAR(float, e_svoDVR_DistRatio);                     \
	  INIT_SVO_CVAR(i32, e_svoTI_GsmCascadeLod);                    \
	  ...
	  INIT_SVO_CVAR(i32, e_svoTI_DualTracing);                      \
	  INIT_SVO_CVAR(i32, e_svoTI_AnalyticalOccluders);              \
	  INIT_SVO_CVAR(i32, e_svoTI_RsmUseColors);                     \
	  INIT_SVO_CVAR(float, e_svoTI_AnalyticalOccludersRange);       \
	  INIT_SVO_CVAR(float, e_svoTI_AnalyticalOccludersSoftness);    \
	  INIT_SVO_CVAR(i32, e_svoTI_AnalyticalGI);                     \
	  INIT_SVO_CVAR(i32, e_svoTI_TraceVoxels);                      \
	  INIT_SVO_CVAR(i32, e_svoTI_AsyncCompute);                     \
