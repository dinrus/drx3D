...
static u32k kCalcBoundsBlocks = 16u;
...
template<class T>
using TDynArrayAligned = DynArray<T, uint, NArray::FastDynStorage<NAlloc::AllocAlign<NAlloc::ModuleAlloc, DRX_PLATFORM_ALIGNMENT>>>;

class CParticleComponentRuntime : public IParticleComponentRuntime
{
public:
	static i32k kThreadsInBlock;
	...
	void                SetUprSlot(i32 i) { m_parameters->managerSlot = i; }

	void                SetLifeTime(float lifeTime) { m_parameters->lifeTime = lifeTime; }

	i32                 GetNumParticles()           { return m_parameters->numParticles; };
	...
	void SetBoundsFromUpr(const SReadbackData* pData);
	void SetCounterFromUpr(u32k* pCounter);
...
private:
	...
	gpu::CStructuredResource<i32, gpu::BufferFlagsReadWrite> m_blockSums;
	gpu::CStructuredResource<i32, gpu::BufferFlagsReadWrite> m_killList;
