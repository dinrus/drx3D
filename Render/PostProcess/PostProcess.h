...
* 23/02/2005: Re-factored/Converted to drx3D 2.0
...
public:
	...
	virtual void SetParamString(tukk pszParam)                       {}
	...
	virtual tukk GetParamString() const { return 0; }
...
{
public:
	CParamInt(i32 nParam, bool bSmoothTransition)
	{
		for (i32 i = 0; i < RT_COMMAND_BUF_COUNT; ++i)
		{
...
public:
	...
	CParamTexture(i32 nInit, bool bSmoothTransition)
	{
		Reset();
	}
	...
	// Create texture
	i32          Create(tukk pszFileName);
	// Release resources
	virtual void Release();

	virtual void SetParamString(tukk pParam)
	{
		Create(pParam);
	}

	virtual tukk GetParamString() const;
	const CTexture*     GetParamTexture() const
	{
		i32k threadID = gRenDev->m_pRT ? gRenDev->m_pRT->GetThreadList() : 0;
...
public:
	...
	virtual i32  Initialize()      { return 1; }
...
typedef std::map<u32, CEffectParam*> KeyEffectMap;
...
public:
	...
	// Get post effect ID
	EPostEffectID GetEffectID(tukk pEffectName);
	...
	CEffectParam* GetByName(tukk pszParam);

	// Given a string returns containing value if exists, else returns 0
	float GetByNameF(tukk pszParam);
	Vec4  GetByNameVec4(tukk pszParam);
	...
	void RegisterParam(tukk pszName, CEffectParam*& pParam, const T& pParamVal, bool bSmoothTransition = true)
	{
	...
	static i32 SortEffectsByID(const CPostEffect* p1, const CPostEffect* p2);

	// Get techniques list
	CPostEffectVec& GetActiveEffects(i32 threadID)
	{
...
private:
	...
	u32 GetCRC(tukk pszName);
	// Used only when creating the crc table
	u32 CRC32Reflect(u32 ref, char ch);
	// zero out the cache
	void   ClearCache()
	{
#ifndef _RELEASE
...
protected:
	...
	u32 m_nCRC32Table[256];  // Lookup table array
...
#define AddEffect(ef)                                  PostEffectMgr()->RegisterEffect(DrxAlignedNew<ef>())
#define AddParamBool(szName, pParam, val)              PostEffectMgr()->RegisterParam<CParamBool, bool>((szName), (pParam), val)
#define AddParamInt(szName, pParam, val)               PostEffectMgr()->RegisterParam<CParamInt, i32>((szName), (pParam), val)
...
#define AddParamTex(szName, pParam, val)               PostEffectMgr()->RegisterParam<CParamTexture, i32>((szName), (pParam), val)
