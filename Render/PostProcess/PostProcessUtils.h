...
struct SPostEffectsUtils
{
	...
	static bool GetOrCreateRenderTarget(tukk szTexName, CTexture*& pTex, i32 iWidth, i32 iHeight, const ColorF& cClear, bool bUseAlpha, bool bMipMaps = 0, ETEX_Format pTexFormat = eTF_R8G8B8A8, i32 nCustomID = -1, i32 nFlags = 0);
	static bool GetOrCreateDepthStencil(tukk szTexName, CTexture*& pTex, i32 iWidth, i32 iHeight, const ColorF& cClear, bool bUseAlpha, bool bMipMaps = 0, ETEX_Format pTexFormat = eTF_R8G8B8A8, i32 nCustomID = -1, i32 nFlags = 0);
	...
	static bool ShBeginPass(CShader* pShader, const CDrxNameTSCRC& TechName, u32 nFlags = 0);

	// Ends render pass utility
	static void ShEndPass();

	// Set vertex shader constant utility
	static void ShSetParamVS(const CDrxNameR& pParamName, const Vec4& pParam);

	// Set pixel shader constant utility
	static void ShSetParamPS(const CDrxNameR& pParamName, const Vec4& pParam);

	static void GetFullScreenTri(SVF_P3F_C4B_T2F pResult[3], i32 nTexWidth, i32 nTexHeight, float z = 0, const RECT* pSrcRegion = NULL);
	static void GetFullScreenTriWPOS(SVF_P3F_T2F_T3F pResult[3], i32 nTexWidth, i32 nTexHeight, float z = 0, const RECT* pSrcRegion = NULL);

	static void GetFullScreenQuad(SVF_P3F_C4B_T2F pResult[4], i32 nTexWidth, i32 nTexHeight, float z = 0, const RECT* pSrcRegion = NULL);
	static void GetFullScreenQuadWPOS(SVF_P3F_T2F_T3F pResult[4], i32 nTexWidth, i32 nTexHeight, float z = 0, const RECT* pSrcRegion = NULL);

	// Draws fullscreen aligned triangle
	static void DrawFullScreenTri(i32 nTexWidth, i32 nTexHeight, float z = 0, const RECT* pSrcRegion = NULL);
	static void DrawFullScreenTriWPOS(i32 nTexWidth, i32 nTexHeight, float z = 0, const RECT* pSrcRegion = NULL);

	// Deprecated - use DrawQuadFS. Draws screen aligned quad
	static void DrawScreenQuad(i32 nTexWidth, i32 nTexHeight, float x0 = 0, float y0 = 0, float x1 = 1, float y1 = 1);

	//  Deprecated: Only used in GammaCorrection technique - Draws a generic, non-screen-aligned quad
	static void DrawQuad(i32 nTexWidth, i32 nTexHeight,
	...
	// Sets a texture
	static void SetTexture(CTexture* pTex, i32 nStage, i32 nFilter = FILTER_LINEAR, ESamplerAddressMode eMode = eSamplerAddressMode_Clamp, bool bSRGBLookup = false, DWORD dwBorderColor = 0);
	...
	static float srandf()
	{
		return drx_random(-1.0f, 1.0f);
	}

	// Returns a quasi-random sequence of values; for 2d data (2, 3) is the recommended base
	static float HaltonSequence(i32 index, i32 primeBase)
	{
		float invBase = 1.0f / (float)primeBase;
		float f = invBase;
		float result = 0;

		for (i32 i = index; i > 0; i /= primeBase, f *= invBase)
	...
	static i32 GetClosestPow2Size(i32 size)
	{
		float fPower = floorf(logf((float)size) / logf(2.0f));
		i32 nResize = i32(powf(2.0f, fPower));
	...
	{
		i32 eye = static_cast<i32>(pRenderView->GetCurrentEye());
...
public:
	...
	static i32            m_iFrameCounter;
	static i32            m_nColorMatrixFrameID;
