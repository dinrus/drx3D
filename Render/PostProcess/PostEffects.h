...
#include <drx3D/Render/PostProcessUtils.h>
...
struct SObjMotionBlurParams
{
	...
	SObjMotionBlurParams(CRenderObject* pObj, const Matrix34A& objToWorld, u32 frameUpdateID)
...
public:
	...
	virtual i32             Initialize();
	virtual i32             CreateResources();
...
private:
	...
	CDrxNameTSCRC m_pMotionBlurDofTechName;
	CDrxNameTSCRC m_pRefDofMBNormalize;

	CDrxNameTSCRC m_pMotionBlurTechName;
	CDrxNameTSCRC m_pDofTechName;
	CDrxNameTSCRC m_pDownscaleDofTechName;
	CDrxNameTSCRC m_pCompositeDofTechName;

	CDrxNameTSCRC m_pPackVelocities;
	CDrxNameTSCRC m_pDownscaleVelocity;

	CDrxNameR     m_pDofFocusParam0Name;
	CDrxNameR     m_pDofFocusParam1Name;
	CDrxNameR     m_pDirBlurParamName;
	CDrxNameR     m_pRadBlurParamName;
	CDrxNameR     m_pViewProjPrevParam;
	CDrxNameR     m_pMotionBlurParamName;

	// bool, i32, float, float, bool, texture, float, float, i32, float, float
...
private:
	...
	u32 m_nVisSampleCount;

	// i32, float, float, float, vec4
...
private:
	...
	Matrix44 ComputeCurrentView(i32 iViewportWidth, i32 iViewportHeight);

	// Spawn a particle
	void SpawnParticle(SRainDrop*& pParticle, i32 iRTWidth, i32 iRTHeight);
	// Update all particles
	void UpdateParticles(i32 iRTWidth, i32 iRTHeight);
	// Generate rain drops map
	void RainDropsMapGen();
	// Draw the raindrops
	void DrawRaindrops(i32 iViewportWidth, i32 iViewportHeight, i32 iRTWidth, i32 iRTHeight);
	// Apply the blur and movement to it
	void ApplyExtinction(CTexture*& rptexPrevRT, i32 iViewportWidth, i32 iViewportHeight, i32 iRTWidth, i32 iRTHeight);
	...
	i32              m_nAliveDrops;
	static i32k m_nMaxDropsCount = 100;
};
...
private:
	...
	CDrxNameTSCRC         m_pszAmbientTechName;
	CDrxNameTSCRC         m_pszSoundHintsTechName;
	CDrxNameTSCRC         m_pszTechFinalComposition;
	CDrxNameTSCRC         m_pszTechNameGhosting;
	CDrxNameR             m_pszParamNameVS;
	CDrxNameR             m_pszParamNamePS;
};
...
private:
	...
	CDrxNameTSCRC m_deferredSilhouettesOptimisedTech;
	CDrxNameR     m_vsParamName;
	CDrxNameR     m_psParamName;
...
public:
	...
	virtual i32         CreateResources() { return 1; }
	virtual void        Release() {}
...
public:
	...
	static i16            s_nFlashWidthMax;
	static i16            s_nFlashHeightMax;
...
private:
	...
	void FinalPass();
	void ReleaseFlashPlayerRef(u32k nThreadID);
...
private:
	...
	CDrxNameTSCRC m_pGeneralTechName;
	CDrxNameTSCRC m_pDownsampleTechName;
	CDrxNameTSCRC m_pUpdateBloomTechName;
	CDrxNameR     m_pHudParamName;
	CDrxNameR     m_pHudEffectsParamName;
	CDrxNameR     m_pMatViewProjParamName;
	CDrxNameR     m_pHudTexCoordParamName;
	CDrxNameR     m_pHudOverrideColorMultParamName;

	float         m_maxParallax;
	i32         m_nFlashUpdateFrameID;
	float         m_accumulatedFrameTime;

#ifndef _RELEASE
	CDrxNameTSCRC m_pTexToTexTechName;
	bool          m_bDebugView;
...
private:
	...
	CDrxNameTSCRC m_techName;
	CDrxNameR     m_paramName;
...
private:
	...
	CDrxNameTSCRC m_shaderTechName;
	CDrxNameR     m_paramName;
	CDrxNameR     m_viewProjMatrixParamName;
...
private:
	...
	void RenderGroup(u8 groupId);
	void RenderMeshes(u8 groupId, float screenRect[4], ERenderMeshMode renderMeshMode = eRMM_Default);
	void RenderDepth(u8 groupId, float screenRect[4], bool bCustomRender = false);
	void AlphaCorrection();
	void GammaCorrection(float screenRect[4]);
	void RenderSilhouettes(u8 groupId, float screenRect[4]);
	...
	CDrxNameTSCRC m_gammaCorrectionTechName;
	CDrxNameTSCRC m_alphaCorrectionTechName;
	CDrxNameTSCRC m_texToTexTechName;
	CDrxNameTSCRC m_customRenderTechName;
	CDrxNameTSCRC m_combineSilhouettesTechName;
	CDrxNameTSCRC m_silhouetteTechName;
	...
	u8         m_groupCount;
	u8         m_post3DRendererflags;
	u8         m_deferDisableFrameCountDown;
};
