...
#include <drx3D/Render/Textures/PowerOf2BlockPacker.h> // CPowerOf2BlockPacker
...
constexpr u32 kMaxShadowPassesNum = sizeof(u32) * CHAR_BIT - 1; // reserve first bit for main view
...
struct ShadowMapFrustum : public CMultiThreadRefCount
{
	...
	struct ShadowCacheData
	{
		...
		void Reset(u32 cacheGeneration)
		{
		...
		u32                           mObjectsRendered;
		u8                            mGeneration;

		static i32k                 MAX_TRAVERSAL_PATH_LENGTH = 32;
		u8                            mOctreePath[MAX_TRAVERSAL_PATH_LENGTH];
		u8                            mOctreePathNodeProcessed[MAX_TRAVERSAL_PATH_LENGTH];
	};

public:
	eFrustumType m_eFrustumType; // NOTE: adjust constructor if you add any variable before this
	u32       m_cachedShadowFrameId = 0;
	...
	TRect_tpl<u32> shadowPoolPack[OMNI_SIDES_NUM];

	u8             nShadowPoolUpdateRate;                                   // For time-sliced updates: Update rate in frames count

	// Only one side in case of directional lights
	std::bitset<OMNI_SIDES_NUM> nOmniFrustumMask;                              // Mask of enabled sides
	u32                      nSideSampleMask = 0;                           // Mask of sides that participate in shadow casting
	u8                       nSideDrawnOnFrame[OMNI_SIDES_NUM] = { 0 };     // Congruence class of FrameID upon which side was last rendererd (modulo 255).
	...
	std::atomic<u32>&       GetSideSampleMask()       { return *reinterpret_cast<std::atomic<u32>*>(&this->nSideSampleMask); }
	const std::atomic<u32>& GetSideSampleMask() const { return *reinterpret_cast<const std::atomic<u32>*>(&this->nSideSampleMask); }
	std::atomic<u32>&       GetOnePassCastersCount() const  { return *reinterpret_cast<std::atomic<u32>*>(&this->onePassCastersNum); }
	...
	mutable i32                     onePassCastersNum = 0;        // Contains number of casters if one-pass octree traversal is used for this frustum
	...
	i32                             nShadowMapLod;                // currently use as GSMLod, can be used as cubemap side, -1 means this variable is not used
	i32                             nShadowCacheLod;
	u32                          m_Flags;
...
public:
	...
	void GetSideViewport(i32 nSide, i32* pViewport) const
	{
		if (bUseShadowsPool)
		{
	...
	void GetTexOffset(i32 nSide, float* pOffset, float* pScale) const
	{
		if (bUseShadowsPool)
		{
	...
	bool isSideOutdated(i32 side) const
	{
		return nOutdatedSideMask[side];
	}

	bool isSideInvalidated(i32 side) const
	{
		return nSideInvalidatedMask[side];
	}

	bool ShouldCacheSideHint(i32 side) const
	{
		return nSideCacheMask[side];
	}

	bool ShouldSampleSide(i32 side) const
	{
	...
	bool ShouldUpdateSide(i32 side) const
	{
		return !nSideCacheMask[side] && ShouldSampleSide(side);
	}

	void MarkSideAsRendered(i32 side, u8 frameID8)
	{
	...
	void MarkShadowGenMaskForSide(i32 side)
	{
	...
	ILINE bool IntersectAABB(const AABB& bbox, bool* pAllIn, i32 side = -1) const
	{
	...
	void DrawFrustum(IRenderer* pRend, i32 nFrames = 1)
	{
		...
		const uint nColorCount = DRX_ARRAY_COUNT(cCascadeColors);
	...
	i32 GetCasterNum() const
	{
		return onePassCastersNum;
	}

	i32 GetNumSides() const
	{
		return bOmniDirectionalShadow ? OMNI_SIDES_NUM : 1;
	}

	const CCamera& GetCamera(i32 side) const
	{
	...
	void SortRenderItemsForFrustumAsync(i32 side, struct SRendItem* pFirst, size_t nNumRendItems);

	// Reserves a shadowpool slot
	void                         PrepareForShadowPool(u32 frameID, u32& numShadowPoolAllocsThisFrame, CPowerOf2BlockPacker& blockPack, TArray<SShadowAllocData>& shadowPoolAlloc, const SRenderLight& light, u32 timeSlicedShadowUpdatesLimit = ~0, u32* timeSlicedShadowsUpdated = nullptr);
	// For time-sliced updates: Returns a mask of per-side flags that hint whether or not the side should be updated
	std::bitset<6>               GenerateTimeSlicedUpdateCacheMask(u32 frameID) const;
	...
	{
		DRX_ASSERT(IsCached() && nShadowCacheLod>=0 && nShadowCacheLod < MAX_GSM_CACHED_LODS_NUM);
	...
	{
		DRX_ASSERT(this->IsCached());
		if (pNode)
		{
			DRX_ASSERT(nShadowCacheLod >= 0 && nShadowCacheLod < MAX_GSM_CACHED_LODS_NUM);
...
struct ShadowFrustumMGPUCache : public ISyncMainWithRenderListener
{
	...
	void Init()
	{
		...
		for (i32 i = 0; i < m_staticShadowMapFrustums.size(); ++i)
		{
	...
	virtual void SyncMainWithRender()
	{
		...
		i32k nFullUpdateMask = (1 << gEnv->pRenderer->GetActiveGPUCount()) - 1;
...
struct SShadowFrustumToRender
{
	...
	SShadowFrustumToRender(ShadowMapFrustum* _pFrustum, SRenderLight* _pLight, i32 _nLightID, IRenderViewPtr _pShadowsView)
		: pFrustum(_pFrustum)
		, pLight(_pLight)
		, nLightID(_nLightID)
		, pShadowsView(std::move(_pShadowsView))
	{
		...
		DRX_ASSERT(pFrustum->pDepthTex == nullptr);
	}
};
