...
#include <drx3D/Render/LightVolumeBuffer.h>
#include <drx3D/Render/ParticleBuffer.h>

#include <drx3D/Render/Shaders/CShader.h>         // CShaderMan
#include <drx3D/Render/Shaders/ShaderResources.h> // CShaderResources
//====================================================================
...
struct DRX_ALIGN(32) SRendItem
{
	...
	static u32 EncodeObjFlagsSortingValue(CRenderObject* pObj)
	{
		return (pObj->m_ObjFlags & ~0xFFFF) + (pObj->m_nSort & 0xFFFF);
	}

	static bool TestObjFlagsSortingValue(u32 nFlag, u32 sortingValue)
	{
		return (nFlag & sortingValue) != 0;
	}

	static inline void ExtractShaderItem(u32 nVal, SShaderItem& sh)
	{
		...
		u32 nTechnique = (nVal & 0x3f);
		if (nTechnique == 0x3f)
			nTechnique = -1;
		sh.m_nTechnique = nTechnique;
		i32 nRes = (nVal >> 6) & (MAX_REND_SHADER_RES - 1);
		sh.m_pShaderResources = (IRenderShaderResources*)((nRes) ? CShader::s_ShaderResources_known[nRes] : nullptr);
	}

	static inline u32 PackShaderItem(const SShaderItem& shaderItem)
	{
		u32 nResID = shaderItem.m_pShaderResources ? ((CShaderResources*)(shaderItem.m_pShaderResources))->m_Id : 0;
		u32 nShaderId = ((CShader*)shaderItem.m_pShader)->mfGetID();
		assert(nResID < CShader::s_ShaderResources_known.size());
		assert(nShaderId != 0);
		u32 value = (nResID << 6) | (nShaderId << 20) | (shaderItem.m_nTechnique & 0x3f);
		return value;
	}

	// Sort by SortVal member of RI
	static void mfSortPreprocess(SRendItem* First, i32 Num);
	// Sort by distance
	static void mfSortByDist(SRendItem* First, i32 Num, bool bDecals, bool InvertedOrder = false);
	// Sort by light
	static void mfSortByLight(SRendItem* First, i32 Num, bool bSort, const bool bIgnoreRePtr, bool bSortDecals);
	// Special sorting for ZPass (compromise between depth and batching)
	static void mfSortForZPass(SRendItem* First, i32 Num);
};
