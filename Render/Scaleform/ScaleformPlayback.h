...
struct SSF_GlobalDrawParams
{
	...
	struct OutputParams : private NoCopy
	{
		...
		Matrix44 oldViewMat;
		i32 oldViewportWidth;
		i32 oldViewportHeight;
	...
	struct _D3DRectangle : public ::D3DRectangle {
		_D3DRectangle() {}
		_D3DRectangle(i32 l, i32 t, i32 r, i32 b) { left = l; top = t; right = r; bottom = b; }
	} scissor;
...
public:
	...
	virtual i32 PushTempRenderTarget(const RectF& frameRect, u32 targetW, u32 targetH, bool wantClear = false, bool wantStencil = false) override;
	...
	virtual void DrawIndexedTriList(i32 baseVertexIndex, i32 minVertexIndex, i32 numVertices, i32 startIndex, i32 triangleCount) override;
	virtual void DrawLineStrip(i32 baseVertexIndex, i32 lineCount) override;
	...
	virtual void DrawBitmaps(const DeviceData* pBitmaps, i32 startIndex, i32 count, ITexture* pTi, const Matrix23& m) override;
...
public:
	...
	virtual DeviceData* CreateDeviceData(ukk pVertices, i32 numVertices, VertexFormat vf, bool bTemp = false) override;
	virtual DeviceData* CreateDeviceData(ukk pIndices, i32 numIndices, IndexFormat idxf, bool bTemp = false) override;
	virtual DeviceData* CreateDeviceData(const BitmapDesc* pBitmapList, i32 numBitmaps, bool bTemp = false) override;
	virtual void ReleaseDeviceData(DeviceData* pData) override;

	// IFlashPlayer
	virtual void SetClearFlags(u32 clearFlags, ColorF clearColor = Clr_Transparent) override;
	...
	virtual void SetThreadIDs(u32 mainThreadID, u32 renderThreadID) override;
...
private:
	...
	void ApplyStencilMask(SSF_GlobalDrawParams::ESFMaskOp maskOp, u32 stencilRef);
	void ApplyBlendMode(BlendType blendMode);
	void ApplyColor(const ColorF& src);
	void ApplyTextureInfo(u32 texSlot, const FillTexture* pFill = 0);
	void ApplyCxform();
...
public:
	...
	virtual void PopRenderTarget() override {}
	virtual i32 PushTempRenderTarget(const RectF&, u32, u32, bool, bool) override { return 0; }
	...
	virtual void DrawIndexedTriList(i32, i32, i32, i32, i32) override {}
	virtual void DrawLineStrip(i32, i32) override {}
	...
	virtual void DrawBitmaps(const DeviceData*, i32, i32, ITexture*, const Matrix23&) override {}
...
public:
	...
	virtual DeviceData* CreateDeviceData(ukk pVertices, i32 numVertices, VertexFormat vf, bool bTemp = false) override { return nullptr; }
	virtual DeviceData* CreateDeviceData(ukk pIndices, i32 numIndices, IndexFormat idxf, bool bTemp = false) override { return nullptr; }
	virtual DeviceData* CreateDeviceData(const BitmapDesc* pBitmapList, i32 numBitmaps, bool bTemp = false) override { return nullptr; }
	virtual void ReleaseDeviceData(DeviceData* pData) override {}
		...
		while (pBuffer->GetReadPos() < cmdBufSize)
		{
			...
			_PLAYBACK_CMD_SUFFIX
				...
				case GRCBA_PushTempRenderTarget:
				{
					...
					u32 targetW = pBuffer->Read<u32>();
					u32 targetH = pBuffer->Read<u32>();
					i32* pRT = pBuffer->Read<i32*>();
					bool wS = pBuffer->Read<bool>();
					i32 pTempRT = pRenderer->T::PushTempRenderTarget(frameRect, targetW, targetH, true, wS);
					*pRT = pTempRT;
					break;
				}
				...
				{
					i32 baseVertexIndex = pBuffer->Read<i32>();
					i32 minVertexIndex = pBuffer->Read<i32>();
					i32 numVertices = pBuffer->Read<i32>();
					i32 startIndex = pBuffer->Read<i32>();
					i32 triangleCount = pBuffer->Read<i32>();
				...
				{
					i32 baseVertexIndex = pBuffer->Read<i32>();
					i32 lineCount = pBuffer->Read<i32>();
