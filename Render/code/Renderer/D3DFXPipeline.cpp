...
#include <drx3D/Render/D3D/DeviceUpr/DeviceObjects.h> // SInputLayout
...
static i32 sTexLimitRes(u32 nSrcsize, u32 nDstSize)
{
	while (true)
	{
	...
	const SRenderingPassInfo& passInfo)
{
	...
	i32 nThreadList = m_pRT->GetThreadList();

	u32 nPrFlags = pRT->m_nFlags;
	...
	u32 nWidth  = pRT->m_nWidth;
	u32 nHeight = pRT->m_nHeight;

	if (pRT->m_nIDInPool >= 0)
	{
		assert((i32)CRendererResources::s_CustomRT_2D.Num() > pRT->m_nIDInPool);
		if ((i32)CRendererResources::s_CustomRT_2D.Num() <= pRT->m_nIDInPool)
			return false;
		...
		if (nWidth == -1)
			nWidth = u32(sTexLimitRes(CRendererResources::s_renderWidth, u32(CRendererResources::s_resourceWidth)) * fSizeScale);
		if (nHeight == -1)
			nHeight = u32(sTexLimitRes(CRendererResources::s_renderHeight, u32(CRendererResources::s_resourceHeight)) * fSizeScale);

		ETEX_Format eTF = pRT->m_eTF;
		if (!pEnvTex->m_pTex || pEnvTex->m_pTex->GetFormat() != eTF)
		{
			char name[128];
			drx_sprintf(name, "$RT_ENV_2D_%d", m_TexGenID++);
			i32 flags = FT_NOMIPS | FT_STATE_CLAMP | FT_DONT_STREAM | FT_USAGE_RENDERTARGET;
		...
		DRX_ASSERT(nWidth  > 0 && nWidth  <= CRendererResources::s_resourceWidth);
		DRX_ASSERT(nHeight > 0 && nHeight <= CRendererResources::s_resourceHeight);
	...
	if (pEnvTex && (!pEnvTex->m_pTex || pEnvTex->m_pTex->GetFormat() != eTF))
	{
		...
		char name[128];
		drx_sprintf(name, "$RT_2D_%d", m_TexGenID++);
		i32 flags = FT_NOMIPS | FT_STATE_CLAMP | FT_DONT_STREAM;
		pEnvTex->m_pTex = new SDynTexture(nWidth, nHeight, Clr_Transparent, eTF, eTT_2D, flags, name);
	...
	case eRTUpdate_WaterReflect:
	{
		...
		i32 nVisibleWaterPixelsCount = eng->GetOceanVisiblePixelsCount() / 2;            // bug in occlusion query returns 2x more
		i32 nPixRatioThreshold       = (i32)(CRendererResources::s_renderArea * CRenderer::CV_r_waterreflections_min_visible_pixels_update);

		static i32 nVisWaterPixCountPrev = nVisibleWaterPixelsCount;
	...
	if (nPrFlags & FRT_CAMERA_REFLECTED_WATERPLANE)
	{
		...
		tmp_cam.SetFrustum((i32)(pEnvTex->m_pTex->GetWidth() * tmp_cam.GetProjRatio()), pEnvTex->m_pTex->GetHeight(), tmp_cam.GetFov(), fDistOffset, fMaxDist);
	...
	else if (nPrFlags & FRT_CAMERA_REFLECTED_PLANE)
	{
		...
		if (pRes && pRes->m_pCamera)
		{
			...
			//tmp_cam.SetFrustum((i32)(Tex->GetWidth()*tmp_cam.GetProjRatio()), Tex->GetHeight(), tmp_cam.GetFov(), fMinDist, tmp_cam.GetFarPlane());
		...
		else
		{
			...
			assert(Tex);
			tmp_cam.SetFrustum((i32)(Tex->GetWidth() * tmp_cam.GetProjRatio()), Tex->GetHeight(), tmp_cam.GetFov(), fMinDist, fMaxDist);       //tmp_cam.GetFarPlane());
			bMirror       = true;
			bUseClipPlane = true;
		}
	...
	float fAnisoScale = 1.0f;
	{
		...
		i32 nRenderPassFlags = (gRenDev->EF_GetRenderQuality()) ? SRenderingPassInfo::TERRAIN : 0;

		i32 nReflQuality = (bOceanRefl) ? (i32)CV_r_waterreflections_quality : (i32)CV_r_reflections_quality;
		...
		i32 nRFlags = SHDF_ALLOWHDR | SHDF_NO_DRAWNEAR | SHDF_FORWARD_MINIMAL;
