...
#include <drx3D/Render/ShadowMap.h>
...
#include <drx3D/Render/Bloom.h>
#include <drx3D/Render/HeightMapAO.h>
#include <drx3D/Render/ScreenSpaceObscurance.h>
#include <drx3D/Render/ScreenSpaceReflections.h>
#include <drx3D/Render/ScreenSpaceSSS.h>
#include <drx3D/Render/VolumetricFog.h>
#include <drx3D/Render/Fog.h>
#include <drx3D/Render/VolumetricClouds.h>
#include <drx3D/Render/Water.h>
#include <drx3D/Render/MotionBlur.h>
#include <drx3D/Render/DepthOfField.h>
#include <drx3D/Render/SunShafts.h>
#include <drx3D/Render/ToneMapping.h>
#include <drx3D/Render/PostAA.h>
#include <drx3D/Render/ComputeSkinning.h>
#include <drx3D/Render/GpuParticles.h>
#include <drx3D/Render/ClipVolumes.h>
#include <drx3D/Render/DeferredDecals.h>
#include <drx3D/Render/ShadowMask.h>
#include <drx3D/Render/TiledShading.h>
#include <drx3D/Render/ColorGrading.h>
#include <drx3D/Render/WaterRipples.h>
#include <drx3D/Render/LensOptics.h>
#include <drx3D/Render/PostEffects.h>
#include <drx3D/Render/Rain.h>
#include <drx3D/Render/Snow.h>
#include <drx3D/Render/MobileComposition.h>
#include <drx3D/Render/OmniCamera.h>
#include <drx3D/Render/TiledLightVolumes.h>
#include <drx3D/Render/DebugRenderTargets.h>

#include <drx3D/Render/DepthReadback.h>
...
#include <drx3D/Render/DriverD3D.h> //for gcpRendD3D
...
void CStandardGraphicsPipeline::Init()
{
	...
	{
		m_defaultMaterialBindPoints.SetConstantBuffer(eConstantBufferShaderSlot_PerMaterial, CDeviceBufferUpr::GetNullConstantBuffer(), EShaderStage_AllWithoutCompute);
	...
	{
		m_defaultDrawExtraRL.SetConstantBuffer(eConstantBufferShaderSlot_SkinQuat    , CDeviceBufferUpr::GetNullConstantBuffer(), EShaderStage_Vertex);
		m_defaultDrawExtraRL.SetConstantBuffer(eConstantBufferShaderSlot_SkinQuatPrev, CDeviceBufferUpr::GetNullConstantBuffer(), EShaderStage_Vertex);
		m_defaultDrawExtraRL.SetConstantBuffer(eConstantBufferShaderSlot_PerGroup    , CDeviceBufferUpr::GetNullConstantBuffer(), EShaderStage_Vertex | EShaderStage_Pixel | EShaderStage_Hull);
		...
		m_defaultDrawExtraRL.SetBuffer(EReservedTextureSlot_SkinExtraWeights , CDeviceBufferUpr::GetNullBufferStructured(), EDefaultResourceViews::Default, EShaderStage_Vertex);
		m_defaultDrawExtraRL.SetBuffer(EReservedTextureSlot_ComputeSkinVerts , CDeviceBufferUpr::GetNullBufferStructured(), EDefaultResourceViews::Default, EShaderStage_Vertex);
		m_defaultDrawExtraRL.SetBuffer(EReservedTextureSlot_GpuParticleStream, CDeviceBufferUpr::GetNullBufferStructured(), EDefaultResourceViews::Default, EShaderStage_Vertex);
		m_defaultDrawExtraRL.SetBuffer(EReservedTextureSlot_AdjacencyInfo    , CDeviceBufferUpr::GetNullBufferTyped()     , EDefaultResourceViews::Default, EShaderStage_Domain);
...
//////////////////////////////////////////////////////////////////////////
void CStandardGraphicsPipeline::Resize(i32 renderWidth, i32 renderHeight)
{
	if (CVrProjectionUpr::IsMultiResEnabledStatic())
	{
		CVrProjectionUpr::Instance()->Configure(SRenderViewport(0, 0, renderWidth, renderHeight), false);
	}
...
{
	const CRenderView* pRenderView = GetCurrentRenderView();
	for (i32 i = 0; i < pRenderView->GetViewInfoCount(); i++)
	{
...
void CStandardGraphicsPipeline::GeneratePerViewConstantBuffer(const SRenderViewInfo* pViewInfo, i32 viewInfoCount, CConstantBufferPtr pPerViewBuffer,const SRenderViewport* pCustomViewport)
{
	const CRenderView* pRenderView = GetCurrentRenderView();
	DRX_ASSERT(pRenderView);
	...
	DrxStackAllocWithSizeVectorCleared(HLSL_PerViewGlobalConstantBuffer, viewInfoCount, bufferData, CDeviceBufferUpr::AlignBufferSizeForStreaming);

	const SRenderGlobalFogDescription& globalFog = pRenderView->GetGlobalFog();

	for (i32 i = 0; i < viewInfoCount; ++i)
	{
		DRX_ASSERT(pViewInfo[i].pCamera);
...
bool CStandardGraphicsPipeline::FillCommonScenePassStates(const SGraphicsPipelineStateDescription& inputDesc, CDeviceGraphicsPSODesc& psoDesc)
{
	...
	u8k renderState = inputDesc.renderState;
	...
	psoDesc.m_ShaderFlags_RT |= CVrProjectionUpr::Instance()->GetRTFlags();
...
void CStandardGraphicsPipeline::ExecuteAnisotropicVerticalBlur(CTexture* pTex, i32 nAmount, float fScale, float fDistribution, bool bAlphaOnly)
{
...
void CStandardGraphicsPipeline::Execute()
{
	...
	pCmdList->BeginResourceTransitions(DRX_ARRAY_COUNT(pTextures), pTextures, eResTransition_TextureRead);
	...
	// Depth downsampling
	{
		...
		if (CVrProjectionUpr::IsMultiResEnabledStatic())
			CVrProjectionUpr::Instance()->ExecuteFlattenDepth(pSourceDepth, CVrProjectionUpr::Instance()->GetZTargetFlattened());
	...
	{
		m_pSceneForwardStage->ExecuteTransparentLoRes(1 + drxmath::clamp<i32>(CRenderer::CV_r_ParticlesHalfResAmount, 0, 1));
	}
