...
#include <drx3D/Render/CDrxDX12RenderTargetView.hpp>
...
CDrxDX12RenderTargetView* CDrxDX12RenderTargetView::Create(CDrxDX12Device* pDevice, ID3D11Resource* pResource11, const D3D11_RENDER_TARGET_VIEW_DESC* pDesc)
{
		...
		case eDX12RT_Buffer:
			{
				...
				(reinterpret_cast<CDrxDX12Buffer*>(pResource11))->GetDesc(&desc);
		...
		case eDX12RT_Texture3D:
			{
				...
				(reinterpret_cast<CDrxDX12Texture3D*>(pResource11))->GetDesc(&desc);
	...
	CDrxDX12RenderTargetView* result;

	if (pDesc)
	{
		result = DX12_NEW_RAW(CDrxDX12RenderTargetView(pResource11, desc11, pResource12, desc12));
	}
	else
	{
		result = DX12_NEW_RAW(CDrxDX12RenderTargetView(pResource11, desc11, pResource12));
	}
...
CDrxDX12RenderTargetView::CDrxDX12RenderTargetView(ID3D11Resource* pResource11, const D3D11_RENDER_TARGET_VIEW_DESC& rDesc11, ID3D12Resource* pResource12, const D3D12_RENDER_TARGET_VIEW_DESC& rDesc12)
	: Super(pResource11, NDrxDX12::EVT_RenderTargetView)
	, m_Desc11(rDesc11)
{
	m_DX12View.GetRTVDesc() = rDesc12;
}

CDrxDX12RenderTargetView::CDrxDX12RenderTargetView(ID3D11Resource* pResource11, const D3D11_RENDER_TARGET_VIEW_DESC& rDesc11, ID3D12Resource* pResource12)
	: Super(pResource11, NDrxDX12::EVT_RenderTargetView)
	, m_Desc11(rDesc11)
{
