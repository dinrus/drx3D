...
CResource::~CResource()
{
	...
	if (bResuable && (m_eInitialState != m_eCurrentState))
	{
		...
		uVals[CMDQUEUE_GRAPHICS] = std::max(fVals[CMDQUEUE_COMPUTE].load(), pCmdScheduler.GetFenceUpr().GetCurrentValue(CMDQUEUE_GRAPHICS));
	...
	if (m_CpuAccessibleData)
	{
		for (auto& subresourceData : m_CpuAccessibleData->m_SubResourceData)
		{
			...
			{
				DrxModuleMemalignFree((uk )subresourceData.Data.pData);
			}
		}
...
void CResource::DiscardInitialData()
{
	if (m_InitialData)
	{
		...
		{
			DrxModuleMemalignFree((uk )m_InitialData->m_SubResourceData[i].pData);
		}
...
CResource::SCpuAccessibleData::SubresourceData& CResource::GetOrCreateCpuAccessibleData(UINT Subresource, bool bAllocate)
{
	...
	if (!result.Data.pData)
	{
		...
		if (bAllocate)
		{
			...
			result.Data.pData = DrxModuleMemalign(size_t(requiredSize), DRX_PLATFORM_ALIGNMENT);
		}
		else
		{
...
D3D12_RESOURCE_STATES CResource::TransitionBarrier(CCommandList* pCmdList, const CView& view, D3D12_RESOURCE_STATES desiredState)
{
	...
	for (i32 slice = slices.start; slice < slices.end; ++slice)
	{
		for (i32 mip = mips.start; mip < mips.end; ++mip)
		{
			for (i32 plane = 0; plane < GetPlaneCount(); ++plane)
			{
...
i32 CResource::SelectQueueForTransitionBarrier(i32 altQueue, i32 desiredQueue, D3D12_RESOURCE_STATES desiredState) const
{
	...
	while (mergedState)
	{
		...
		u32 z = countTrailingZeros32(u32(mergedState));
	...
	else
	{
		...
		else
		{
			...
			for (i32 slice = slices.start; slice < slices.end; ++slice)
			{
				for (i32 mip = mips.start; mip < mips.end; ++mip)
				{
					for (i32 plane = 0; plane < m_PlaneCount; ++plane)
					{
						u32 subresource = desc.CalcSubresource(mip, slice, plane);
...
{
	DRX_ASSERT(!HasSubresourceTransitionBarriers());
	DRX_ASSERT(m_SubresourceStates.empty());

	u32 subresourceCount = CD3DX12_RESOURCE_DESC(m_Desc).Subresources(GetPlaneCount());
...
void CResource::DisableSubresourceTransitionBarriers(CCommandList* pCmdList, D3D12_RESOURCE_STATES desiredCommonState)
{
	...
	for (i32 subresource = 0; subresource < m_SubresourceStates.size(); ++subresource)
	{
		auto subresourceState = m_SubresourceStates[subresource];
...
//---------------------------------------------------------------------------------------------------------------------
HRESULT CResource::MappedWriteToSubresource(UINT Subresource, const D3D12_RANGE* Range, ukk pInData)
{
	...
	StreamBufferData(reinterpret_cast<u8*>(pOutData) + Range->Begin, pInData, Range->End - Range->Begin);
	GetD3D12Resource()->Unmap(Subresource, Range);

	return S_OK;
}

HRESULT CResource::MappedReadFromSubresource(UINT Subresource, const D3D12_RANGE* Range, uk pOutData)
{
	...
	FetchBufferData(pOutData, reinterpret_cast<u8*>(pInData) + Range->Begin, Range->End - Range->Begin);
