...
#error StagingPool is a requirement for DX12
	...
	#include <drx3D/Render/DX12/API/Redirections/D3D12Device.inl"
#endif
...
{
	auto pEmulatedDevice = reinterpret_cast<CDrxDX12Device*>(pDevice);

	m_pDX12Device    = pEmulatedDevice->GetDX12Device();
	m_pDX12Scheduler = &m_pDX12Device->GetScheduler();

	m_pDX12Scheduler->AddQueueEventCallback(m_pCoreCommandList.get(), lambdaCeaseCallback, NDrxDX12::CCommandScheduler::CMDQUEUE_EVENT_CEASE);
	m_pDX12Scheduler->AddQueueEventCallback(m_pCoreCommandList.get(), lambdaResumeCallback, NDrxDX12::CCommandScheduler::CMDQUEUE_EVENT_RESUME);
...
CDeviceCommandListUPtr CDeviceObjectFactory::AcquireCommandList(EQueueType eQueueType /*= eQueue_Graphics*/)
{
	...
	NDrxDX12::CCommandListPool& pQueue = m_pDX12Scheduler->GetCommandListPool(eQueueType);
...
D3DResource* CDeviceObjectFactory::AllocateStagingResource(D3DResource* pForTex, bool bUpload, uk & pMappedAddress)
{
...
#if 0
	IDrxDX12Resource* dx12Resource = DX12_EXTRACT_IDRXDX12RESOURCE(pForTex);
	...
	{
		pStagingResource = CDrxDX12Buffer::Create(gcpRendD3D->GetDevice_Unsynchronized().GetRealDevice(), stagingResource, initialState);
...
void CDeviceObjectFactory::ReleaseStagingResource(D3DResource* pStagingTex)
{
	if (pStagingTex)
	{
		...
		{
			IDrxDX12Resource* dx12Resource = DX12_EXTRACT_IDRXDX12RESOURCE(pStagingTex);
			NDrxDX12::CResource& rResource = dx12Resource->GetDX12Resource();
	...
	, ukk pData)
{
	...
	if ((nUsage & USAGE_CPU_WRITE))
		BufDesc.ByteWidth = CDeviceBufferUpr::AlignElementCountForStreaming(nSize, elemSize) * elemSize;
