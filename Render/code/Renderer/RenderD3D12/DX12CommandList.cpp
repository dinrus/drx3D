...
#include <drx3D/Render/DX12/GI/CDrxDX12SwapChain.hpp>
...
bool CCommandList::Init(UINT64 currentFenceValue)
{
	...
	if (!m_pCmdList)
	{
		...
		else
		{
			...
			pCmdList->QueryInterface(__uuidof(ID3D12GraphicsCommandList1), (uk *)&pCmdList1);
			...
			pCmdList->QueryInterface(__uuidof(ID3D12GraphicsCommandList1), (uk *)&pCmdList2);
...
void CCommandList::ResourceBarrier(UINT NumBarriers, const D3D12_RESOURCE_BARRIER* pBarriers)
{
	if (DX12_BARRIER_FUSION && CRenderer::CV_r_D3D12BatchResourceBarriers)
	{
		...
		{
			DRX_ASSERT(m_CurrentNumPendingBarriers + NumBarriers < 256);
...
//---------------------------------------------------------------------------------------------------------------------
void CCommandList::UpdateSubresourceRegion(CResource& rResource, UINT subResource, const D3D12_BOX* box, ukk data, UINT rowPitch, UINT slicePitch)
{
...
//---------------------------------------------------------------------------------------------------------------------
CCommandListPool::CCommandListPool(CDevice* device, CCommandListFenceSet& rCmdFence, i32 nPoolFenceId)
	: m_pDevice(device)
...
void CCommandListPool::ScheduleCommandLists()
{
	...
	for (u32 t = 0; t < m_LiveCommandLists.size(); ++t)
	{
...
{
	static DrxCriticalSectionNonRecursive csThreadSafeScope;
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(csThreadSafeScope);
...
void CCommandListPool::AcquireCommandLists(u32 numCLs, DX12_PTR(CCommandList)* results) threadsafe
{
	static DrxCriticalSectionNonRecursive csThreadSafeScope;
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(csThreadSafeScope);
	...
	for (u32 i = 0; i < numCLs; ++i)
		CreateOrReuseCommandList(results[i]);
}

void CCommandListPool::ForfeitCommandLists(u32 numCLs, DX12_PTR(CCommandList)* results, bool bWait) threadsafe
{
	static DrxCriticalSectionNonRecursive csThreadSafeScope;
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(csThreadSafeScope);
