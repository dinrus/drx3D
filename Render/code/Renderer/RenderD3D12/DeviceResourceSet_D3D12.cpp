...
bool CDeviceResourceSet_DX12::UpdateImpl(const CDeviceResourceSetDesc& desc, CDeviceResourceSetDesc::EDirtyFlags dirtyFlags)
{
	...
	DrxStackAllocatorWithSizeVector(D3D12_CPU_DESCRIPTOR_HANDLE, descriptorCount, descriptorMem, NoAlign);
	...
	if (bSuccess)
	{
		DRX_ASSERT(descriptors.size() == descriptorCount);
...
bool CDeviceResourceLayout_DX12::Init(const SDeviceResourceLayoutDesc& desc)
{
	...
	for (auto& itLayoutBinding : desc.m_resourceBindings)
	{
		...
		{
			i32 descriptorCount = 0;
			i32 descriptorRangeStart = resourceMappings.m_DescRangeCursor;
			...
			for (auto itResource : itLayoutBinding.second)
			{
				...
				{
					const NDrxDX12::CSamplerState& samplerState = GET_DX12_SAMPLERSTATE(resourceBinding.samplerState)->GetDX12SamplerState();
					...
					DRX_ASSERT(resourceMappings.m_NumStaticSamplers <= DRX_ARRAY_COUNT(resourceMappings.m_StaticSamplers));
				}
				else
				{
					...
					DRX_ASSERT(resourceMappings.m_DescRangeCursor < DRX_ARRAY_COUNT(resourceMappings.m_DescRanges));
			...
			if (descriptorCount > 0)
			{
				...
				DRX_ASSERT(resourceMappings.m_NumRootParameters <= DRX_ARRAY_COUNT(resourceMappings.m_RootParameters));
			}
		}
		break;
...
{
	i32k descriptorCount = descriptors.size();
	...
	for (const auto& it : resourceDesc.GetResources())
	{
		...
		case SResourceBindPoint::ESlotType::ConstantBuffer:
		{
			...
			CDrxDX12Buffer* constantBuffer = reinterpret_cast<CDrxDX12Buffer*>(resource.pConstantBuffer->GetD3D(&start, &length));

			{
				D3D12_CONSTANT_BUFFER_VIEW_DESC cbvDesc;

				const NDrxDX12::CView& pCBV = constantBuffer->GetDX12View();
				...
				cbvDesc.SizeInBytes = min(cbvDesc.SizeInBytes, D3D12_REQ_CONSTANT_BUFFER_ELEMENT_COUNT * NDrxDX12::CONSTANT_BUFFER_ELEMENT_SIZE);
		...
		case SResourceBindPoint::ESlotType::TextureAndBuffer:
		{
			...
			CDrxDX12ShaderResourceView* pSrv = GET_DX12_SHADER_VIEW(pResource, resource.view);
		...
		case SResourceBindPoint::ESlotType::UnorderedAccessView:
		{
			...
			CDrxDX12UnorderedAccessView* pUav = GET_DX12_UNORDERED_VIEW(pResource, resource.view);
	...
	CDescriptorBlock& descriptorBlock) const
{
	...
	// 64 descriptors
	static constexpr auto chunkSize = DRX_ARRAY_COUNT(DescriptorRangesOfOnes);

	DRX_ASSERT(descriptorBlock.GetCapacity() - descriptorBlock.GetCursor() == descriptors.size());
