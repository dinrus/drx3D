...
#include <drx3D/Render/CDrxDX12Texture2D.hpp>
...
CDrxDX12Texture2D* CDrxDX12Texture2D::Create(CDrxDX12Device* pDevice)
{
	...
	CDrxDX12Texture2D* result = DX12_NEW_RAW(CDrxDX12Texture2D(pDevice, desc11, nullptr, D3D12_RESOURCE_STATE_COMMON, CD3DX12_RESOURCE_DESC(desc12), NULL));

	return result;
}

CDrxDX12Texture2D* CDrxDX12Texture2D::Create(CDrxDX12Device* pDevice, CDrxDX12SwapChain* pSwapChain, ID3D12Resource* pResource)
{
	...
	CDrxDX12Texture2D* result = DX12_NEW_RAW(CDrxDX12Texture2D(pDevice, desc11, pResource, D3D12_RESOURCE_STATE_PRESENT, CD3DX12_RESOURCE_DESC(desc12), NULL));
	if (pSwapChain)
...
CDrxDX12Texture2D* CDrxDX12Texture2D::Create(CDrxDX12Device* pDevice, const FLOAT cClearValue[4], const D3D11_TEXTURE2D_DESC* pDesc, const D3D11_SUBRESOURCE_DATA* pInitialData)
{
	...
	auto result = DX12_NEW_RAW(CDrxDX12Texture2D(pDevice, *pDesc, resource, resourceUsage, CD3DX12_RESOURCE_DESC(resource->GetDesc()), pInitialData, desc12.DepthOrArraySize * desc12.MipLevels));
	resource->Release();
...
CDrxDX12Texture2D::CDrxDX12Texture2D(CDrxDX12Device* pDevice, const D3D11_TEXTURE2D_DESC& desc11, ID3D12Resource* pResource, D3D12_RESOURCE_STATES eInitialState, const CD3DX12_RESOURCE_DESC& desc12, const D3D11_SUBRESOURCE_DATA* pInitialData, size_t numInitialData)
