...
CDeviceGraphicsPSO::EInitResult CDeviceGraphicsPSO_DX12::Init(const CDeviceGraphicsPSODesc& psoDesc)
{
	...
	// blend state
	{
		...
		for (i32 i = 0; i < DRX_ARRAY_COUNT(blendDesc.RenderTarget); ++i)
		{
			u32 RenderTargetWriteMask = i < psoInitParams.m_Desc.NumRenderTargets ? DeviceFormats::GetWriteMask(psoInitParams.m_Desc.RTVFormats[i]) : 0;
	...
	auto extractShaderBytecode = [&](EHWShaderClass shaderClass)
	{
		...
		{
			return reinterpret_cast<CDrxDX12Shader*>(hwShaders[shaderClass].pDeviceShader)->GetD3D12ShaderBytecode();
		}
		else
		{
...
bool CDeviceComputePSO_DX12::Init(const CDeviceComputePSODesc& psoDesc)
{
	...
	{
		psoInitParams.m_Desc.CS = static_cast<CDrxDX12Shader*>(hwShaders[eHWSC_Compute].pDeviceShader)->GetD3D12ShaderBytecode();
	}
