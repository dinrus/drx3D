...
#include <drx3D/Render/DX12CommandScheduler.hpp>
#include <drx3D/Render/DX12Resource.hpp>
...
auto GetDebugName = [](ID3D12Resource* pO) -> tukk { UINT len = 511; static char name[512] = "unknown"; pO->GetPrivateData(WKPDID_D3DDebugObjectName, &len, name); name[len] = '\0'; return name; };
#else
auto GetDebugName = [](ID3D12Resource* pO) -> tukk { return "unknown"; };
#endif

//---------------------------------------------------------------------------------------------------------------------
CDevice* CDevice::Create(CDrxDX12GIAdapter* pAdapter, D3D_FEATURE_LEVEL* pFeatureLevel)
{
...
//	  (D3D12CreateDevice(pDXGIAdapter, level = D3D_FEATURE_LEVEL_11_2, IID_GFX_ARGS(&pDevice12)) == S_OK) ||
	...
	if (CRenderer::CV_r_StereoEnableMgpu && i32(nodeCount) > 1)
	{
		nodeMask = (1UL << 2) - 1UL;
		switch (nodeCount)
		{
...
{
	return CRenderer::CV_r_StereoEnableMgpu && (i32(m_nodeCount) > 1);
}
	...
	, m_Scheduler(this, nodeMask)
{
	...
	m_pDevice->QueryInterface(__uuidof(ID3D12Device1), (uk *)&pDevice1);
	...
	m_pDevice->QueryInterface(__uuidof(ID3D12Device2), (uk *)&pDevice2);
...
{
	DrxAutoLock<DrxCriticalSectionNonRecursive> m_DescriptorAllocatorTheadSafeScope(m_DescriptorAllocatorTheadSafeScope);
...
//---------------------------------------------------------------------------------------------------------------------
DrxCriticalSectionNonRecursive CDevice::m_SamplerThreadSafeScope;
DrxCriticalSectionNonRecursive CDevice::m_ShaderResourceThreadSafeScope;
DrxCriticalSectionNonRecursive CDevice::m_UnorderedAccessThreadSafeScope;
DrxCriticalSectionNonRecursive CDevice::m_DepthStencilThreadSafeScope;
DrxCriticalSectionNonRecursive CDevice::m_RenderTargetThreadSafeScope;
DrxCriticalSectionNonRecursive CDevice::m_DescriptorAllocatorTheadSafeScope;

DrxCriticalSectionNonRecursive CDevice::m_ReleaseHeapTheadSafeScope;
DrxCriticalSectionNonRecursive CDevice::m_RecycleHeapTheadSafeScope;

//---------------------------------------------------------------------------------------------------------------------
void CDevice::InvalidateSampler(const D3D12_SAMPLER_DESC* pDesc)
{
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(m_SamplerThreadSafeScope);
...
{
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(m_ShaderResourceThreadSafeScope);
...
{
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(m_UnorderedAccessThreadSafeScope);
...
{
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(m_DepthStencilThreadSafeScope);
...
{
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(m_RenderTargetThreadSafeScope);
...
HRESULT STDMETHODCALLTYPE CDevice::CreateOrReuseCommittedResource(
	...
	_COM_Outptr_optvoid** ppvResource) threadsafe
{
	struct
	{
	...
	uk ptr1 = ((tuk)&hashableBlob.sResourceDesc.Dimension) + sizeof(hashableBlob.sResourceDesc.Dimension);
	ZeroMemory(ptr1, offsetof(D3D12_RESOURCE_DESC, Alignment) - sizeof(hashableBlob.sResourceDesc.Dimension));
	uk ptr2 = ((tuk)&hashableBlob.sResourceDesc.Flags) + sizeof(hashableBlob.sResourceDesc.Flags);
	...
	{
		DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(m_RecycleHeapTheadSafeScope);
...
void CDevice::FlushReleaseHeap(const UINT64 (&completedFenceValues)[CMDQUEUE_NUM], const UINT64(&pruneFenceValues)[CMDQUEUE_NUM]) threadsafe
{
	...
	{
		DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(m_ReleaseHeapTheadSafeScope);
	...
	if (CRendererCVars::CV_r_ShowVideoMemoryStats)
	{
		...
		IRenderAuxText::Draw2dLabel(colPos, float(rowPos += rowHeight), fontSize, Col_Blue, false, "Fences: Current %lli, Submitted %lli, Completed %lli",
			fenceUpr.GetCurrentValue(CMDQUEUE_GRAPHICS), fenceUpr.GetSubmittedValue(CMDQUEUE_GRAPHICS), fenceUpr.GetLastCompletedFenceValue(CMDQUEUE_GRAPHICS));
		...
		for (it = m_RecycleHeap.begin(); it != m_RecycleHeap.end(); it = nx)
		{
			...
			{
				u32 size = CTexture::TextureDataSize(u32(sDesc.Width), 1, 1, sDesc.MipLevels, sDesc.DepthOrArraySize, DeviceFormats::ConvertToTexFormat(sDesc.Format));
				recycleSize += counter * size;
				recycleNums += counter;
				...
				IRenderAuxText::Draw2dLabel /* Ex */(colPos, float(rowPos += rowHeightSmall), fontSizeSmall, Col_Yellow, false /* eDrawText_Monospace */,
					...
					sHeapFlags, sDesc.Flags, sHeapProperties.CPUPageProperty, sHeapProperties.Type,
					u32(sDesc.Width), sDesc.DepthOrArraySize, sDesc.MipLevels);
			}
			else if (sDesc.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE2D)
			{
				u32 size = CTexture::TextureDataSize(u32(sDesc.Width), sDesc.Height, 1, sDesc.MipLevels, sDesc.DepthOrArraySize, DeviceFormats::ConvertToTexFormat(sDesc.Format));
				recycleSize += counter * size;
				recycleNums += counter;
				...
				IRenderAuxText::Draw2dLabel /* Ex */(colPos, float(rowPos += rowHeightSmall), fontSizeSmall, Col_Yellow, false /* eDrawText_Monospace */,
					...
					sHeapFlags, sDesc.Flags, sHeapProperties.CPUPageProperty, sHeapProperties.Type,
					u32(sDesc.Width), sDesc.Height, sDesc.DepthOrArraySize, sDesc.MipLevels);
			}
			else if (sDesc.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE3D)
			{
				u32 size = CTexture::TextureDataSize(u32(sDesc.Width), sDesc.Height, sDesc.DepthOrArraySize, sDesc.MipLevels, 1, DeviceFormats::ConvertToTexFormat(sDesc.Format));
				recycleSize += counter * size;
				recycleNums += counter;
