...
CDrxDX12DeviceContext* CDrxDX12DeviceContext::Create(CDrxDX12Device* pDevice, UINT nodeMask, bool isDeferred)
{
	return DX12_NEW_RAW(CDrxDX12DeviceContext(pDevice, nodeMask, isDeferred));
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

static auto lambdaCeaseCallback = [](uk ctx, uint nPoolId)
{
	reinterpret_cast<CDrxDX12DeviceContext*>(ctx)->CeaseCommandQueueCallback(nPoolId);
};

static auto lambdaResumeCallback = [](uk ctx, uint nPoolId)
{
	reinterpret_cast<CDrxDX12DeviceContext*>(ctx)->ResumeCommandQueueCallback(nPoolId);
};

CDrxDX12DeviceContext::CDrxDX12DeviceContext(CDrxDX12Device* pDevice, UINT nodeMask, bool bDeferred)
	: Super()
	...
	, m_OcclusionHeap(pDevice->GetDX12Device())
{
	...
	m_Scheduler.AddQueueEventCallback(this, lambdaCeaseCallback, NDrxDX12::CCommandScheduler::CMDQUEUE_EVENT_CEASE);
	m_Scheduler.AddQueueEventCallback(this, lambdaResumeCallback, NDrxDX12::CCommandScheduler::CMDQUEUE_EVENT_RESUME);
}

CDrxDX12DeviceContext::~CDrxDX12DeviceContext()
{
	DX12_FUNC_LOG
...
bool CDrxDX12DeviceContext::PrepareGraphicsPSO()
{
	...
	{
		NDrxDX12::CDevice* device = m_pDevice->GetDX12Device();

		DX12_PTR(NDrxDX12::CRootSignature) newRootSignature;
		DX12_PTR(NDrxDX12::CGraphicsPSO) newPSO;

		NDrxDX12::CRootSignature::SInitParams rsParams;
		NDrxDX12::CGraphicsPSO::SInitParams psoParams;
...
bool CDrxDX12DeviceContext::PrepareComputePSO()
{
	...
	if (stateFlags & EPSPB_PipelineState)
	{
		...
		DX12_PTR(NDrxDX12::CRootSignature) newRootSignature;
		DX12_PTR(NDrxDX12::CComputePSO) newPSO;

		NDrxDX12::CRootSignature::SInitParams rsParams;
		NDrxDX12::CComputePSO::SInitParams psoParams;
...
void CDrxDX12DeviceContext::PrepareGraphicsFF()
{
	const UINT& stateFlags = m_GraphicsState.m_StateFlags;
	...
	if (stateFlags & EPSPB_VertexBuffers)
	{
		...
		{
			CDrxDX12Buffer* buffer = m_GraphicsState.InputAssembler.VertexBuffers.Get(i);
			if (buffer)
			{
...
void CDrxDX12DeviceContext::PrepareIO(bool bGfx)
{
...
bool CDrxDX12DeviceContext::PrepareGraphicsState()
{
...
bool CDrxDX12DeviceContext::PrepareComputeState()
{
...
void CDrxDX12DeviceContext::BindResources(bool bGfx)
{
	ID3D12Device* pDevice12 = GetD3D12Device();
	const SDrxDX11PipelineState& rState = bGfx ? m_GraphicsState : m_ComputeState;
	NDrxDX12::CCommandList* pCmdList = bGfx ? m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS) : m_Scheduler.GetCommandList(CMDQUEUE_COMPUTE);
	const NDrxDX12::SResourceMappings& mappings = m_pCurrentRootSignature->GetResourceMappings();
	...
	if (stateFlags & (EPSPB_ConstantBuffers | EPSPB_Resources))
	{
		for (UINT r = 0, n = mappings.m_NumRESs; r < n; ++r)
		{
			...
			const NDrxDX12::EShaderStage i = resInput.ShaderStage;
			if (resInput.ViewType == D3D12_DESCRIPTOR_RANGE_TYPE_CBV)
			{
				CDrxDX12Buffer* buffer = rState.Stages[i].ConstantBufferViews.Get(resInput.ShaderSlot);
			...
			{
				CDrxDX12ShaderResourceView* resource = rState.Stages[i].ShaderResourceViews.Get(resInput.ShaderSlot);
			...
			{
				CDrxDX12UnorderedAccessView* resource = rState.Stages[i].UnorderedAccessViews.Get(resInput.ShaderSlot);
	...
	if (stateFlags & EPSPB_Samplers)
	{
		for (UINT s = 0, n = mappings.m_NumSMPs; s < n; ++s)
		{
			...
			const NDrxDX12::EShaderStage i = smpInput.ShaderStage;
			DX12_ASSERT(smpInput.ViewType == D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER, "");
			{
				CDrxDX12SamplerState* sampler = rState.Stages[i].SamplerState.Get(smpInput.ShaderSlot);
...
void CDrxDX12DeviceContext::BindOutputViews()
{
	const SDrxDX11PipelineState& rState = m_GraphicsState;
	const NDrxDX12::CCommandList* pCmdList = m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS);

	const NDrxDX12::CView* dsv = NULL;
	const NDrxDX12::CView* rtv[D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT];
	size_t numRTVs = 0;

	// Get current depth stencil views
	{
		CDrxDX12DepthStencilView* view = rState.OutputMerger.DepthStencilView;
		if (view)
		{
	...
	{
		CDrxDX12RenderTargetView* view = rState.OutputMerger.RenderTargetViews.Get(i);
		if (view)
		{
...
namespace NDrxDX12 {
tukk StateToString(D3D12_RESOURCE_STATES state);
}

void CDrxDX12DeviceContext::DebugPrintResources(bool bGfx)
{
	ID3D12Device* pDevice12 = GetD3D12Device();
	const SDrxDX11PipelineState& rState = bGfx ? m_GraphicsState : m_ComputeState;
	const NDrxDX12::CCommandList* pCmdList = bGfx ? m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS) : m_Scheduler.GetCommandList(CMDQUEUE_COMPUTE);
	const NDrxDX12::SResourceMappings& mappings = m_pCurrentRootSignature->GetResourceMappings();
	...
	for (UINT r = 0, n = mappings.m_NumRESs; r < n; ++r)
	{
		...
		if (resInput.ViewType == D3D12_DESCRIPTOR_RANGE_TYPE_CBV)
		{
			...
			DX12_LOG(g_nPrintDX12, " %s: C %2d -> %2d %s [%s]",
			         i == NDrxDX12::ESS_Compute ? "Every    shader stage" :
			         i == NDrxDX12::ESS_Vertex ? "Vertex   shader stage" :
			         i == NDrxDX12::ESS_Hull ? "Hull     shader stage" :
			         i == NDrxDX12::ESS_Domain ? "Domain   shader stage" :
			         i == NDrxDX12::ESS_Geometry ? "Geometry shader stage" :
			         i == NDrxDX12::ESS_Pixel ? "Pixel    shader stage" : "Unknown  shader stage",
			         resInput.ShaderSlot,
			         uResourceDTOffset++,
			         buffer ? buffer->GetName().c_str() : "nullptr",
			         buffer ? NDrxDX12::StateToString(buffer->GetDX12Resource().GetState()) : "-");
		}
		else if (resInput.ViewType == D3D12_DESCRIPTOR_RANGE_TYPE_SRV)
		{
			CDrxDX12ShaderResourceView* resource = rState.Stages[i].ShaderResourceViews.Get(resInput.ShaderSlot);
			...
			DX12_LOG(g_nPrintDX12, " %s: T %2d -> %2d %s [%s]",
			         ...
			         resource ? NDrxDX12::StateToString(resource->GetDX12Resource().GetState()) : "-");
		}
		else if (resInput.ViewType == D3D12_DESCRIPTOR_RANGE_TYPE_UAV)
		{
			CDrxDX12UnorderedAccessView* resource = rState.Stages[i].UnorderedAccessViews.Get(resInput.ShaderSlot);
...
void CDrxDX12DeviceContext::CeaseCommandQueueCallback(i32 nPoolId)
{
...
void CDrxDX12DeviceContext::ResumeCommandQueueCallback(i32 nPoolId)
{
...
void CDrxDX12DeviceContext::Finish(NDrxDX12::CSwapChain* pDX12SwapChain)
{
...
void CDrxDX12DeviceContext::Finish()
{
...
UINT CDrxDX12DeviceContext::TimestampIndex(NDrxDX12::CCommandList* pCmdList)
{
...
void CDrxDX12DeviceContext::InsertTimestamp(NDrxDX12::CCommandList* pCmdList, UINT index)
{
	pCmdList->EndQuery(m_TimestampHeap, D3D12_QUERY_TYPE_TIMESTAMP, index);
}

void CDrxDX12DeviceContext::ResolveTimestamps(NDrxDX12::CCommandList* pCmdList)
{
	if (m_TimestampMemory)
	{
...
void CDrxDX12DeviceContext::QueryTimestamp(NDrxDX12::CCommandList* pCmdList, UINT index, uk mem)
{
	if (mem)
	{
		if (!m_TimestampMemory)
		...
		memcpy(mem, (tuk)m_TimestampMemory + index * sizeof(UINT64), sizeof(UINT64));
	}
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

UINT CDrxDX12DeviceContext::OcclusionIndex(NDrxDX12::CCommandList* pCmdList, bool counter)
{
...
void CDrxDX12DeviceContext::InsertOcclusionStart(NDrxDX12::CCommandList* pCmdList, UINT index, bool counter)
{
	pCmdList->BeginQuery(m_OcclusionHeap, counter ? D3D12_QUERY_TYPE_OCCLUSION : D3D12_QUERY_TYPE_BINARY_OCCLUSION, index);
}

void CDrxDX12DeviceContext::InsertOcclusionStop(NDrxDX12::CCommandList* pCmdList, UINT index, bool counter)
{
...
void CDrxDX12DeviceContext::ResolveOcclusion(NDrxDX12::CCommandList* pCmdList, UINT index, uk mem)
{
	if (mem)
	{
		...
		memcpy(mem, (tuk)m_OcclusionMemory + index * sizeof(UINT64), sizeof(UINT64));
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GetDevice(
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::GetPrivateData(
  _In_ REFGUID guid,
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::SetPrivateData(
  _In_ REFGUID guid,
  _In_ UINT DataSize,
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::SetPrivateDataInterface(
  _In_ REFGUID guid,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::VSSetConstantBuffers(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::PSSetShaderResources(
  ...
  _In_reads_opt_(NumViews)  ID3D11ShaderResourceView* const* ppShaderResourceViews)
{
	...
	{
		CDrxDX12ShaderResourceView* srv = reinterpret_cast<CDrxDX12ShaderResourceView*>(*ppShaderResourceViews);
		if (m_GraphicsState.Stages[NDrxDX12::ESS_Pixel].ShaderResourceViews.Set(i, srv) && srv)
		{
			NDrxDX12::CResource& resource = srv->GetDX12Resource();
			{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::PSSetShader(
  _In_opt_ ID3D11PixelShader* pPixelShader,
  _In_reads_opt_(NumClassInstances)  ID3D11ClassInstance* const* ppClassInstances,
  UINT NumClassInstances)
{
	DX12_FUNC_LOG
	m_GraphicsState.Stages[NDrxDX12::ESS_Pixel].Shader.Set(reinterpret_cast<CDrxDX12Shader*>(pPixelShader));
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::PSSetSamplers(
  ...
  _In_reads_opt_(NumSamplers)  ID3D11SamplerState* const* ppSamplers)
{
	...
	{
		m_GraphicsState.Stages[NDrxDX12::ESS_Pixel].SamplerState.Set(i, reinterpret_cast<CDrxDX12SamplerState*>(*ppSamplers));
	}
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::VSSetShader(
  _In_opt_ ID3D11VertexShader* pVertexShader,
  _In_reads_opt_(NumClassInstances)  ID3D11ClassInstance* const* ppClassInstances,
  UINT NumClassInstances)
{
	DX12_FUNC_LOG
	m_GraphicsState.Stages[NDrxDX12::ESS_Vertex].Shader.Set(reinterpret_cast<CDrxDX12Shader*>(pVertexShader));
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::DrawIndexed(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::Draw(
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::Map(
  ...
  _Out_ D3D11_MAPPED_SUBRESOURCE* pMappedResource)
{
	...
	IDrxDX12Resource* dx12Resource = DX12_EXTRACT_IDRXDX12RESOURCE(pResource);
	NDrxDX12::CResource& resource = dx12Resource->GetDX12Resource();
	...
	case D3D11_MAP_READ:
		...
		{
			DrxCriticalSectionNonRecursive safe;
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::Unmap(
  _In_ ID3D11Resource* pResource,
  _In_ UINT Subresource)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::PSSetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _In_reads_opt_(NumBuffers)  ID3D11Buffer* const* ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::IASetInputLayout(
  _In_opt_ ID3D11InputLayout* pInputLayout)
{
	DX12_FUNC_LOG

	m_GraphicsState.InputAssembler.InputLayout.Set(reinterpret_cast<CDrxDX12InputLayout*>(pInputLayout));
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::IASetVertexBuffers(
  ...
  _In_reads_opt_(NumBuffers)  const UINT* pOffsets)
{
	...
	{
		CDrxDX12Buffer* vb = reinterpret_cast<CDrxDX12Buffer*>(*ppVertexBuffers);
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::IASetIndexBuffer(
  ...
  _In_ UINT Offset)
{
	...
	CDrxDX12Buffer* ib = reinterpret_cast<CDrxDX12Buffer*>(pIndexBuffer);
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DrawIndexedInstanced(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DrawInstanced(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GSSetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _In_reads_opt_(NumBuffers)  ID3D11Buffer* const* ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GSSetShader(
  _In_opt_ ID3D11GeometryShader* pShader,
  _In_reads_opt_(NumClassInstances)  ID3D11ClassInstance* const* ppClassInstances,
  UINT NumClassInstances)
{
	DX12_FUNC_LOG
	m_GraphicsState.Stages[NDrxDX12::ESS_Geometry].Shader.Set(reinterpret_cast<CDrxDX12Shader*>(pShader));
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::IASetPrimitiveTopology(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::VSSetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _In_reads_opt_(NumViews)  ID3D11ShaderResourceView* const* ppShaderResourceViews)
{
	...
	{
		CDrxDX12ShaderResourceView* srv = reinterpret_cast<CDrxDX12ShaderResourceView*>(*ppShaderResourceViews);
		if (m_GraphicsState.Stages[NDrxDX12::ESS_Vertex].ShaderResourceViews.Set(i, srv) && srv)
		{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::VSSetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _In_reads_opt_(NumSamplers)  ID3D11SamplerState* const* ppSamplers)
{
	...
	for (UINT i = StartSlot, S = StartSlot + NumSamplers; i < S; ++i, ++ppSamplers)
	{
		m_GraphicsState.Stages[NDrxDX12::ESS_Vertex].SamplerState.Set(i, reinterpret_cast<CDrxDX12SamplerState*>(*ppSamplers));
	}
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::Begin(
  _In_ ID3D11Asynchronous* pAsync)
{
	DX12_FUNC_LOG
	auto pQuery = reinterpret_cast<CDrxDX12Query*>(pAsync);
	...
	else if (desc.Query == D3D11_QUERY_OCCLUSION || desc.Query == D3D11_QUERY_OCCLUSION_PREDICATE)
	{
		...
		auto pOcclusionQuery = reinterpret_cast<CDrxDX12ResourceQuery*>(pAsync);
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::End(
  _In_ ID3D11Asynchronous* pAsync)
{
	DX12_FUNC_LOG
	auto pQuery = reinterpret_cast<CDrxDX12Query*>(pAsync);
	...
	if (desc.Query == D3D11_QUERY_EVENT)
	{
		...
		auto pEventQuery = reinterpret_cast<CDrxDX12EventQuery*>(pAsync);
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::GetData(
  _In_ ID3D11Asynchronous* pAsync,
  _Out_writes_bytes_opt_(DataSize)  uk pData,
  _In_ UINT DataSize,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::SetPredication(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GSSetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _In_reads_opt_(NumViews)  ID3D11ShaderResourceView* const* ppShaderResourceViews)
{
	...
	{
		CDrxDX12ShaderResourceView* srv = reinterpret_cast<CDrxDX12ShaderResourceView*>(*ppShaderResourceViews);
		if (m_GraphicsState.Stages[NDrxDX12::ESS_Geometry].ShaderResourceViews.Set(i, srv) && srv)
		{
			srv->BeginResourceStateTransition(m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS));
		}
	}
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GSSetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _In_reads_opt_(NumSamplers)  ID3D11SamplerState* const* ppSamplers)
{
	...
	for (UINT i = StartSlot, S = StartSlot + NumSamplers; i < S; ++i, ++ppSamplers)
	{
		m_GraphicsState.Stages[NDrxDX12::ESS_Geometry].SamplerState.Set(i, reinterpret_cast<CDrxDX12SamplerState*>(*ppSamplers));
	}
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::OMSetRenderTargets(
  ...
  _In_opt_ ID3D11DepthStencilView* pDepthStencilView)
{
	...
	{
		CDrxDX12RenderTargetView* rtv = reinterpret_cast<CDrxDX12RenderTargetView*>(ppRenderTargetViews[i]);
	...
	{
		CDrxDX12DepthStencilView* dsv = reinterpret_cast<CDrxDX12DepthStencilView*>(pDepthStencilView);
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::OMSetRenderTargetsAndUnorderedAccessViews(
  _In_ UINT NumRTVs,
  ...
  _In_reads_opt_(NumUAVs)  const UINT* pUAVInitialCounts)
{
	...
	{
		CDrxDX12UnorderedAccessView* uav = reinterpret_cast<CDrxDX12UnorderedAccessView*>(*ppUnorderedAccessViews);
		if (m_GraphicsState.Stages[NDrxDX12::ESS_Pixel].UnorderedAccessViews.Set(i, uav) && uav)
		{
			NDrxDX12::CResource& resource = uav->GetDX12Resource();
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::OMSetBlendState(
  ...
  _In_ UINT SampleMask)
{
	...
	m_GraphicsState.OutputMerger.BlendState.Set(reinterpret_cast<CDrxDX12BlendState*>(pBlendState));
	m_GraphicsState.OutputMerger.SampleMask.Set(SampleMask);
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::OMSetDepthStencilState(
  _In_opt_ ID3D11DepthStencilState* pDepthStencilState,
  _In_ UINT StencilRef)
{
	DX12_FUNC_LOG

	m_GraphicsState.Rasterizer.DepthStencilState.Set(reinterpret_cast<CDrxDX12DepthStencilState*>(pDepthStencilState));
	m_GraphicsState.OutputMerger.StencilRef.Set(StencilRef);
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::SOSetTargets(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DrawAuto()
{
	DX12_FUNC_LOG
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::DrawIndexedInstancedIndirect(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DrawInstancedIndirect(
  _In_ ID3D11Buffer* pBufferForArgs,
  _In_ UINT AlignedByteOffsetForArgs)
{
	DX12_FUNC_LOG
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::Dispatch(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DispatchIndirect(
  _In_ ID3D11Buffer* pBufferForArgs,
  _In_ UINT AlignedByteOffsetForArgs)
{
	DX12_FUNC_LOG
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::RSSetState(
  _In_opt_ ID3D11RasterizerState* pRasterizerState)
{
	DX12_FUNC_LOG

	m_GraphicsState.Rasterizer.RasterizerState.Set(reinterpret_cast<CDrxDX12RasterizerState*>(pRasterizerState));
	if (pRasterizerState)
	{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::RSSetViewports(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::RSSetScissorRects(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CopySubresourceRegion(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CopySubresourcesRegion(
  _In_ ID3D11Resource* pDstResource,
  _In_ UINT DstSubresource,
  _In_ UINT DstX,
  _In_ UINT DstY,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CopyResource(
  _In_ ID3D11Resource* pDstResource,
  _In_ ID3D11Resource* pSrcResource)
{
	DX12_FUNC_LOG
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::UploadResource(
  _In_ IDrxDX12Resource* pDstResource,
  ...
  _In_reads_(NumInitialData)  const D3D11_SUBRESOURCE_DATA* pInitialData)
{
	...
	NDrxDX12::CResource& dstResource = pDstResource->GetDX12Resource();
	NDrxDX12::CResource::SInitialData* id = dstResource.GetOrCreateInitialData();
	...
	for (UINT i = 0; i < NumInitialData; ++i)
	{
		...
		initialData.pData = DrxModuleMemalign(size_t(curSize), DRX_PLATFORM_ALIGNMENT);
		memset((tuk)(initialData.pData) + cpySize, 0, size_t(curSize - cpySize));
		memcpy(const_cast<uk>(initialData.pData), pInitialData[i].pSysMem, size_t(cpySize));
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::UploadResource(
  _In_ NDrxDX12::CResource* pDstResource,
  _In_ const NDrxDX12::CResource::SInitialData* pSrcData)
{
	DX12_FUNC_LOG
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::UpdateSubresource(
  _In_ ID3D11Resource* pDstResource,
  _In_ UINT DstSubresource,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CopyStructureCount(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::ClearRenderTargetView(
  _In_ ID3D11RenderTargetView* pRenderTargetView,
  _In_ const FLOAT ColorRGBA[4])
{
	DX12_FUNC_LOG

	NDrxDX12::CView* view = DX12_EXTRACT_DX12VIEW(pRenderTargetView);
	CDrxDX12RenderTargetView* rtv = reinterpret_cast<CDrxDX12RenderTargetView*>(pRenderTargetView);
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::ClearUnorderedAccessViewUint(
  _In_ ID3D11UnorderedAccessView* pUnorderedAccessView,
  _In_ const UINT Values[4])
{
	DX12_FUNC_LOG

	NDrxDX12::CView* view = DX12_EXTRACT_DX12VIEW(pUnorderedAccessView);
	CDrxDX12UnorderedAccessView* uav = reinterpret_cast<CDrxDX12UnorderedAccessView*>(pUnorderedAccessView);
	DX12_LOG(g_nPrintDX12, "Clearing unordered access view [i32]: %p %s", pUnorderedAccessView, uav->GetResourceName().c_str());

	m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS)->ClearUnorderedAccessView(*view, Values);
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::ClearUnorderedAccessViewFloat(
  _In_ ID3D11UnorderedAccessView* pUnorderedAccessView,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::ClearDepthStencilView(
  ...
  _In_ UINT8 Stencil)
{
	...
	NDrxDX12::CView* view = DX12_EXTRACT_DX12VIEW(pDepthStencilView);
	CDrxDX12DepthStencilView* dsv = reinterpret_cast<CDrxDX12DepthStencilView*>(pDepthStencilView);
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GenerateMips(
  _In_ ID3D11ShaderResourceView* pShaderResourceView)
{
	DX12_FUNC_LOG
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::SetResourceMinLOD(
  _In_ ID3D11Resource* pResource,
  FLOAT MinLOD)
{
	DX12_FUNC_LOG
...
FLOAT STDMETHODCALLTYPE CDrxDX12DeviceContext::GetResourceMinLOD(
  _In_ ID3D11Resource* pResource)
{
	DX12_FUNC_LOG
	return 0.0f;
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::ResolveSubresource(
  _In_ ID3D11Resource* pDstResource,
  _In_ UINT DstSubresource,
  _In_ ID3D11Resource* pSrcResource,
  _In_ UINT SrcSubresource,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::HSSetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _In_reads_opt_(NumViews)  ID3D11ShaderResourceView* const* ppShaderResourceViews)
{
	...
	{
		CDrxDX12ShaderResourceView* srv = reinterpret_cast<CDrxDX12ShaderResourceView*>(*ppShaderResourceViews);
		if (m_GraphicsState.Stages[NDrxDX12::ESS_Hull].ShaderResourceViews.Set(i, srv) && srv)
		{
			srv->BeginResourceStateTransition(m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS));
		}
	}
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::HSSetShader(
  _In_opt_ ID3D11HullShader* pHullShader,
  _In_reads_opt_(NumClassInstances)  ID3D11ClassInstance* const* ppClassInstances,
  UINT NumClassInstances)
{
	DX12_FUNC_LOG
	m_GraphicsState.Stages[NDrxDX12::ESS_Hull].Shader.Set(reinterpret_cast<CDrxDX12Shader*>(pHullShader));
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::HSSetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _In_reads_opt_(NumSamplers)  ID3D11SamplerState* const* ppSamplers)
{
	...
	for (UINT i = StartSlot, S = StartSlot + NumSamplers; i < S; ++i, ++ppSamplers)
	{
		m_GraphicsState.Stages[NDrxDX12::ESS_Hull].SamplerState.Set(i, reinterpret_cast<CDrxDX12SamplerState*>(*ppSamplers));
	}
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::HSSetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _In_reads_opt_(NumBuffers)  ID3D11Buffer* const* ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DSSetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _In_reads_opt_(NumViews)  ID3D11ShaderResourceView* const* ppShaderResourceViews)
{
	...
	{
		CDrxDX12ShaderResourceView* srv = reinterpret_cast<CDrxDX12ShaderResourceView*>(*ppShaderResourceViews);
		if (m_GraphicsState.Stages[NDrxDX12::ESS_Domain].ShaderResourceViews.Set(i, srv) && srv)
		{
			srv->BeginResourceStateTransition(m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS));
		}
	}
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DSSetShader(
  _In_opt_ ID3D11DomainShader* pDomainShader,
  _In_reads_opt_(NumClassInstances)  ID3D11ClassInstance* const* ppClassInstances,
  UINT NumClassInstances)
{
	DX12_FUNC_LOG
	m_GraphicsState.Stages[NDrxDX12::ESS_Domain].Shader.Set(reinterpret_cast<CDrxDX12Shader*>(pDomainShader));
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::DSSetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _In_reads_opt_(NumSamplers)  ID3D11SamplerState* const* ppSamplers)
{
	...
	for (UINT i = StartSlot, S = StartSlot + NumSamplers; i < S; ++i, ++ppSamplers)
	{
		m_GraphicsState.Stages[NDrxDX12::ESS_Domain].SamplerState.Set(i, reinterpret_cast<CDrxDX12SamplerState*>(*ppSamplers));
	}
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::DSSetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _In_reads_opt_(NumBuffers)  ID3D11Buffer* const* ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSSetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _In_reads_opt_(NumViews)  ID3D11ShaderResourceView* const* ppShaderResourceViews)
{
	...
	{
		CDrxDX12ShaderResourceView* srv = reinterpret_cast<CDrxDX12ShaderResourceView*>(*ppShaderResourceViews);
		if (m_ComputeState.Stages[NDrxDX12::ESS_Compute].ShaderResourceViews.Set(i, srv) && srv)
		{
			NDrxDX12::CResource& resource = srv->GetDX12Resource();
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSSetUnorderedAccessViews(
  ...
  _In_reads_opt_(NumUAVs)  const UINT* pUAVInitialCounts)
{
	...
	{
		CDrxDX12UnorderedAccessView* uav = reinterpret_cast<CDrxDX12UnorderedAccessView*>(*ppUnorderedAccessViews);
		if (m_ComputeState.Stages[NDrxDX12::ESS_Compute].UnorderedAccessViews.Set(i, uav) && uav)
		{
			NDrxDX12::CResource& resource = uav->GetDX12Resource();
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSSetShader(
  _In_opt_ ID3D11ComputeShader* pComputeShader,
  _In_reads_opt_(NumClassInstances)  ID3D11ClassInstance* const* ppClassInstances,
  UINT NumClassInstances)
{
	DX12_FUNC_LOG
	m_ComputeState.Stages[NDrxDX12::ESS_Compute].Shader.Set(reinterpret_cast<CDrxDX12Shader*>(pComputeShader));
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSSetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _In_reads_opt_(NumSamplers)  ID3D11SamplerState* const* ppSamplers)
{
	...
	for (UINT i = StartSlot, S = StartSlot + NumSamplers; i < S; ++i, ++ppSamplers)
	{
		m_ComputeState.Stages[NDrxDX12::ESS_Compute].SamplerState.Set(i, reinterpret_cast<CDrxDX12SamplerState*>(*ppSamplers));
	}
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSSetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _In_reads_opt_(NumBuffers)  ID3D11Buffer* const* ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::VSGetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::PSGetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::PSGetShader(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::PSGetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::VSGetShader(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::PSGetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _Out_writes_opt_(NumBuffers)  ID3D11Buffer** ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::IAGetInputLayout(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::IAGetVertexBuffers(
  _In_range_(0, D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumBuffers,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::IAGetIndexBuffer(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GSGetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _Out_writes_opt_(NumBuffers)  ID3D11Buffer** ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GSGetShader(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::IAGetPrimitiveTopology(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::VSGetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _Out_writes_opt_(NumViews)  ID3D11ShaderResourceView** ppShaderResourceViews)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::VSGetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _Out_writes_opt_(NumSamplers)  ID3D11SamplerState** ppSamplers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GetPredication(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GSGetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _Out_writes_opt_(NumViews)  ID3D11ShaderResourceView** ppShaderResourceViews)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::GSGetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _Out_writes_opt_(NumSamplers)  ID3D11SamplerState** ppSamplers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::OMGetRenderTargets(
  _In_range_(0, D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT)  UINT NumViews,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::OMGetRenderTargetsAndUnorderedAccessViews(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::OMGetBlendState(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::OMGetDepthStencilState(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::SOGetTargets(
  _In_range_(0, D3D11_SO_BUFFER_SLOT_COUNT)  UINT NumBuffers,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::RSGetState(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::RSGetViewports(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::RSGetScissorRects(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::HSGetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _Out_writes_opt_(NumViews)  ID3D11ShaderResourceView** ppShaderResourceViews)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::HSGetShader(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::HSGetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _Out_writes_opt_(NumSamplers)  ID3D11SamplerState** ppSamplers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::HSGetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _Out_writes_opt_(NumBuffers)  ID3D11Buffer** ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DSGetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _Out_writes_opt_(NumViews)  ID3D11ShaderResourceView** ppShaderResourceViews)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DSGetShader(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DSGetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _Out_writes_opt_(NumSamplers)  ID3D11SamplerState** ppSamplers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::DSGetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _Out_writes_opt_(NumBuffers)  ID3D11Buffer** ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSGetShaderResources(
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot)  UINT NumViews,
  _Out_writes_opt_(NumViews)  ID3D11ShaderResourceView** ppShaderResourceViews)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSGetUnorderedAccessViews(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSGetShader(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSGetSamplers(
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot)  UINT NumSamplers,
  _Out_writes_opt_(NumSamplers)  ID3D11SamplerState** ppSamplers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CSGetConstantBuffers(
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)  UINT StartSlot,
  _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot)  UINT NumBuffers,
  _Out_writes_opt_(NumBuffers)  ID3D11Buffer** ppConstantBuffers)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::ClearState()
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::Flush()
{
	DX12_FUNC_LOG
...
D3D11_DEVICE_CONTEXT_TYPE STDMETHODCALLTYPE CDrxDX12DeviceContext::GetType()
{
	DX12_FUNC_LOG
	return D3D11_DEVICE_CONTEXT_IMMEDIATE;
}

UINT STDMETHODCALLTYPE CDrxDX12DeviceContext::GetContextFlags()
{
	DX12_FUNC_LOG
	return 0;
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::ExecuteCommandList(
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::FinishCommandList(
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CopySubresourceRegion1(
	_In_ ID3D11Resource* pDstResource,
	_In_ UINT DstSubresource,
	_In_ UINT DstX,
	_In_ UINT DstY,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CopySubresourcesRegion1(
  _In_ ID3D11Resource* pDstResource,
  _In_ UINT DstSubresource,
  _In_ UINT DstX,
  _In_ UINT DstY,
  ...
  _In_ UINT NumSubresources)
{
	...
	IDrxDX12Resource* dstDX12Resource = DX12_EXTRACT_IDRXDX12RESOURCE(pDstResource);
	IDrxDX12Resource* srcDX12Resource = DX12_EXTRACT_IDRXDX12RESOURCE(pSrcResource);
	NDrxDX12::CResource& dstResource = dstDX12Resource->GetDX12Resource(); dstResource.VerifyBackBuffer();
	NDrxDX12::CResource& srcResource = srcDX12Resource->GetDX12Resource(); srcResource.VerifyBackBuffer();
	...
	i32k copyQueue = bCpy ? CMDQUEUE_COPY : CMDQUEUE_COMPUTE;
	i32 activeQueue = CMDQUEUE_GRAPHICS;
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CopyResource1(
  _In_ ID3D11Resource* pDstResource,
  _In_ ID3D11Resource* pSrcResource,
  _In_ UINT CopyFlags)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::UpdateSubresource1(
  _In_ ID3D11Resource* pDstResource,
  _In_ UINT DstSubresource,
  _In_opt_ const D3D11_BOX* pDstBox,
  _In_ ukk pSrcData,
  ...
  _In_ UINT CopyFlags)
{
	...
	IDrxDX12Resource* dx12Resource = DX12_EXTRACT_IDRXDX12RESOURCE(pDstResource);
	NDrxDX12::CResource& resource = dx12Resource->GetDX12Resource(); resource.VerifyBackBuffer();
	...
	if (!bGfx && (CMDLIST_COPY != CMDLIST_TYPE_DIRECT))
	{
		...
		const NDrxDX12::NODE64& uploadMasks = resource.GetNodeMasks();
...
void CDrxDX12DeviceContext::UpdateSubresources(
  _In_ NDrxDX12::CResource* pDstResource,
  ...
  _In_reads_(NumInitialData) D3D12_SUBRESOURCE_DATA* pSrcData)
{
	...
	NDrxDX12::CResource& resource = *pDstResource;

	// TODO: copy command on the swap chain are special (can't execute on any other queue), make an API for that
	/*const*/ bool bGfx = (resource.IsBackBuffer());

	NDrxDX12::CCommandListPool& rCmdListPool = bGfx ? m_Scheduler.GetCommandListPool(CMDQUEUE_GRAPHICS) : m_Scheduler.GetCommandListPool(CMDQUEUE_COPY);
	NDrxDX12::CCommandList* pCmdList = bGfx ? m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS) : m_Scheduler.GetCommandList(CMDQUEUE_COPY);
...
void CDrxDX12DeviceContext::DiscardResource(ID3D11Resource* pResource)
{
	DX12_FUNC_LOG
...
void CDrxDX12DeviceContext::DiscardView(ID3D11View* pResourceView)
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void CDrxDX12DeviceContext::VSSetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppConstantBuffers, const UINT* pFirstConstant, const UINT* pNumConstants)
{
	DX12_FUNC_LOG

	  SetConstantBuffers1(NDrxDX12::ESS_Vertex, StartSlot, NumBuffers, ppConstantBuffers, pFirstConstant, pNumConstants, true);
}

void CDrxDX12DeviceContext::HSSetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppConstantBuffers, const UINT* pFirstConstant, const UINT* pNumConstants)
{
	DX12_FUNC_LOG

	  SetConstantBuffers1(NDrxDX12::ESS_Hull, StartSlot, NumBuffers, ppConstantBuffers, pFirstConstant, pNumConstants, true);
}

void CDrxDX12DeviceContext::DSSetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppConstantBuffers, const UINT* pFirstConstant, const UINT* pNumConstants)
{
	DX12_FUNC_LOG

	  SetConstantBuffers1(NDrxDX12::ESS_Domain, StartSlot, NumBuffers, ppConstantBuffers, pFirstConstant, pNumConstants, true);
}

void CDrxDX12DeviceContext::GSSetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppConstantBuffers, const UINT* pFirstConstant, const UINT* pNumConstants)
{
	DX12_FUNC_LOG

	  SetConstantBuffers1(NDrxDX12::ESS_Geometry, StartSlot, NumBuffers, ppConstantBuffers, pFirstConstant, pNumConstants, true);
}

void CDrxDX12DeviceContext::PSSetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppConstantBuffers, const UINT* pFirstConstant, const UINT* pNumConstants)
{
	DX12_FUNC_LOG

	  SetConstantBuffers1(NDrxDX12::ESS_Pixel, StartSlot, NumBuffers, ppConstantBuffers, pFirstConstant, pNumConstants, true);
}

void CDrxDX12DeviceContext::CSSetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppConstantBuffers, const UINT* pFirstConstant, const UINT* pNumConstants)
{
	DX12_FUNC_LOG

	  SetConstantBuffers1(NDrxDX12::ESS_Compute, StartSlot, NumBuffers, ppConstantBuffers, pFirstConstant, pNumConstants, false);
}

void CDrxDX12DeviceContext::SetConstantBuffers1(NDrxDX12::EShaderStage shaderStage, UINT StartSlot, UINT NumBuffers, ID3D11Buffer* const* ppConstantBuffers, const UINT* pFirstConstant, const UINT* pNumConstants, bool bGfx)
{
	SDrxDX11PipelineState& rState = bGfx ? m_GraphicsState : m_ComputeState;
	NDrxDX12::CCommandList* pCmdList = bGfx ? m_Scheduler.GetCommandList(CMDQUEUE_GRAPHICS) : m_Scheduler.GetCommandList(CMDQUEUE_COMPUTE);

	for (UINT i = StartSlot, S = StartSlot + NumBuffers, j = 0; i < S; ++i, ++ppConstantBuffers, ++j)
	{
		CDrxDX12Buffer* cb = reinterpret_cast<CDrxDX12Buffer*>(*ppConstantBuffers);

		TRange<UINT> bindRange;
		if (pFirstConstant)
		{
			bindRange.start = pFirstConstant[j] * NDrxDX12::CONSTANT_BUFFER_ELEMENT_SIZE;
			bindRange.end = bindRange.start + pNumConstants[j] * NDrxDX12::CONSTANT_BUFFER_ELEMENT_SIZE;
		}
...
void CDrxDX12DeviceContext::VSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer** ppConstantBuffers, UINT* pFirstConstant, UINT* pNumConstants)
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void CDrxDX12DeviceContext::HSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer** ppConstantBuffers, UINT* pFirstConstant, UINT* pNumConstants)
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void CDrxDX12DeviceContext::DSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer** ppConstantBuffers, UINT* pFirstConstant, UINT* pNumConstants)
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void CDrxDX12DeviceContext::GSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer** ppConstantBuffers, UINT* pFirstConstant, UINT* pNumConstants)
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void CDrxDX12DeviceContext::PSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer** ppConstantBuffers, UINT* pFirstConstant, UINT* pNumConstants)
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void CDrxDX12DeviceContext::CSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer** ppConstantBuffers, UINT* pFirstConstant, UINT* pNumConstants)
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void CDrxDX12DeviceContext::SwapDeviceContextState(ID3DDeviceContextState* pState, ID3DDeviceContextState** ppPreviousState)
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void CDrxDX12DeviceContext::ClearView(ID3D11View* pView, const FLOAT Color[4], const D3D11_RECT* pRect, UINT NumRects)
{
	DX12_FUNC_LOG
	  DX12_LOG(g_nPrintDX12, "Clearing view: %p", pView);

	NDrxDX12::CView* view = DX12_EXTRACT_DX12VIEW(pView);
...
void CDrxDX12DeviceContext::DiscardView1(ID3D11View* pResourceView, const D3D11_RECT* pRects, UINT NumRects)
{
	DX12_FUNC_LOG
	  DX12_NOT_IMPLEMENTED
}

void STDMETHODCALLTYPE CDrxDX12DeviceContext::ClearRectsRenderTargetView(
  _In_ ID3D11RenderTargetView* pRenderTargetView,
  _In_ const FLOAT ColorRGBA[4],
  _In_ UINT NumRects,
  _In_reads_opt_(NumRects)  const D3D11_RECT* pRects)
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::ClearRectsUnorderedAccessViewUint(
  _In_ ID3D11UnorderedAccessView* pUnorderedAccessView,
  _In_ const UINT Values[4],
  _In_ UINT NumRects,
  _In_reads_opt_(NumRects)  const D3D11_RECT* pRects)
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::ClearRectsUnorderedAccessViewFloat(
  _In_ ID3D11UnorderedAccessView* pUnorderedAccessView,
  _In_ const FLOAT Values[4],
  _In_ UINT NumRects,
  _In_reads_opt_(NumRects)  const D3D11_RECT* pRects)
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::ClearRectsDepthStencilView(
  _In_ ID3D11DepthStencilView* pDepthStencilView,
  _In_ UINT ClearFlags,        // DX11 and DX12 clear flags are identical
  _In_ FLOAT Depth,
  _In_ UINT8 Stencil,
  ...
  _In_reads_opt_(NumRects)  const D3D11_RECT* pRects)
{
	...
	#include <drx3D/Render/D3D/DX12/API/Redirections/D3D12Device.inl"
#endif

void STDMETHODCALLTYPE CDrxDX12DeviceContext::CopyResourceOvercross(
	_In_ ID3D11Resource* pDstResource,
	_In_ ID3D11Resource* pSrcResource)
{
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::CopyResourceOvercross1(
	_In_ ID3D11Resource* pDstResource,
	_In_ ID3D11Resource* pSrcResource,
	_In_ UINT CopyFlags)
{
	...
	{
		IDrxDX12Resource* srcDX12ResourceL = DX12_EXTRACT_IDRXDX12RESOURCE(pSrcResource);
		IDrxDX12Resource* srcDX12ResourceR = DX12_EXTRACT_IDRXDX12RESOURCE(pDstResource);
		NDrxDX12::CResource& srcResourceL = srcDX12ResourceL->GetDX12Resource(); srcResourceL.VerifyBackBuffer();
		NDrxDX12::CResource& srcResourceR = srcDX12ResourceR->GetDX12Resource(); srcResourceR.VerifyBackBuffer();
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::JoinSubresourceRegion(
	_In_ ID3D11Resource* pDstResource,
	_In_ UINT DstSubresource,
	_In_ UINT DstX,
	_In_ UINT DstY,
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::JoinSubresourceRegion1(
	_In_ ID3D11Resource* pDstResource,
	_In_ UINT DstSubresource,
	_In_ UINT DstX,
	_In_ UINT DstY,
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::CopyStagingResource(
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::MapStagingResource(
  _In_ ID3D11Resource* pStagingResource,
  _In_ BOOL Upload,
  _Outvoid** ppStagingMemory)
{
	DX12_FUNC_LOG
...
void STDMETHODCALLTYPE CDrxDX12DeviceContext::UnmapStagingResource(
  _In_ ID3D11Resource* pStagingResource,
  _In_ BOOL Upload)
{
	DX12_FUNC_LOG
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::TestStagingResource(
  _In_ ID3D11Resource* pStagingResource)
{
	DX12_FUNC_LOG
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::WaitStagingResource(
  _In_ ID3D11Resource* pStagingResource)
{
	DX12_FUNC_LOG
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::MappedWriteToSubresource(
	_In_ ID3D11Resource* pResource,
	_In_ UINT Subresource,
	_In_ SIZE_T Offset,
	_In_ SIZE_T Size,
	_In_ D3D11_MAP MapType,
	_In_reads_bytes_(Size) ukk pInData,
...
HRESULT STDMETHODCALLTYPE CDrxDX12DeviceContext::MappedReadFromSubresource(
	_In_ ID3D11Resource* pResource,
	_In_ UINT Subresource,
	_In_ SIZE_T Offset,
	_In_ SIZE_T Size,
	_In_ D3D11_MAP MapType,
	_Out_writes_bytes_(Size) uk pOutData,
	_In_ const UINT numDataBlocks)
{
	IDrxDX12Resource* dx12Resource = DX12_EXTRACT_IDRXDX12RESOURCE(pResource);
	NDrxDX12::CResource& resource = dx12Resource->GetDX12Resource();
...
void CDrxDX12DeviceContext::ResetCachedState(bool bGraphics, bool bCompute)
{
