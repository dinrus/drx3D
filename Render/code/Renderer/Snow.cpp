...
void CSnowStage::Init()
{
	...
	DRX_ASSERT(m_pSnowFrostBumpTex == nullptr);
...
void CSnowStage::ResizeResource(i32 resourceWidth, i32 resourceHeight)
{
...
void CSnowStage::Resize(i32 renderWidth, i32 renderHeight)
{
...
void CSnowStage::ExecuteDeferredSnowGBuffer()
{
	...
	{
		static CDrxNameTSCRC techName("Snow");
		...
		i32k stencilState = STENC_FUNC(FSS_STENCFUNC_EQUAL) |
			STENCOP_FAIL(FSS_STENCOP_KEEP) |
			STENCOP_ZFAIL(FSS_STENCOP_KEEP) |
			STENCOP_PASS(FSS_STENCOP_KEEP);
		u8k stencilRef = BIT_STENCIL_RESERVED;
		u8k stencilReadMask = BIT_STENCIL_RESERVED;
	...
	static CDrxNameR paramName("g_SnowVolumeParams");
	...
	static CDrxNameR param1Name("g_SnowMultipliers");
	...
	{
		static CDrxNameR param3Name("g_SnowOcc_TransMat");
...
void CSnowStage::ExecuteDeferredSnowDisplacement()
{
	...
	static CDrxNameR param5Name("g_CameraMatrix");
...

	{
		...
		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName = "ParallaxMapPrepass";
			pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
			pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
...

	{
		...
		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName = "ParallaxMapMin";
			pass.SetTechnique(CShaderMan::s_ShaderDeferredSnow, techName, 0);
			pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
	...
	// Iteratively apply displacement to maximize quality and minimize sample count.
	{
		...
		static CDrxNameTSCRC techName = "ParallaxMapApply";
		static CDrxNameR paramsName("g_DisplacementParams");
...
bool CSnowStage::GenerateSnowClusterVertex()
{
	...
	i32 iRTWidth  = CRendererResources::s_renderWidth;
	i32 iRTHeight = CRendererResources::s_renderHeight;
	    ...
	    || ((m_nFlakesPerCluster != snowVolParams.m_nSnowFlakeCount) && 0 < snowVolParams.m_nSnowFlakeCount))
	{
		...
		for (i32 p = 0; p < m_nSnowFlakeVertCount; p += 6)
		{
			Vec3 vPosition = Vec3(drx_random(-10.0f, 10.0f), drx_random(-10.0f, 10.0f), drx_random(-10.0f, 10.0f));
			float fSize = fUserSize + fUserSizeVar * drx_random(-1.0f, 1.0f);
			float fRandPhase = drx_random(0.0f, 10.0f);
		...
		DRX_ASSERT(m_snowFlakeVertexBuffer != ~0u);
...
{
	auto numClusters = max<i32>(1, CRenderer::CV_r_snowFlakeClusters);
	if (numClusters != m_nNumClusters)
	{
		DRX_ASSERT(m_nNumClusters == m_snowClusterArray.size());

		m_snowClusterArray.resize(numClusters);

		// create and initialize render primitives.
		for (i32 index = m_nNumClusters; index < numClusters; ++index)
		{
	...
	for (auto& cluster : m_snowClusterArray)
	{
		...
		if (fabs(fCurrLifeTime) > 1.0f || cluster.m_fLifeTime < 0 || fClusterDist > 30.0f)
		{
			if (bAllowSpawn)
			{
				...
				cluster.m_pPos.x += drx_random(-1.0f, 1.0f) * 15.0f;
				cluster.m_pPos.y += drx_random(-1.0f, 1.0f) * 15.0f;
				cluster.m_pPos.z += drx_random(-1.0f, 1.0f) * 5.0f + 4.0f;

				cluster.m_pPosPrev = cluster.m_pPos;
				cluster.m_fLifeTime = (pNewDrop.m_fLifeTime / std::max(1.0f, m_SnowVolParams.m_fSnowFallGravityScale) + pNewDrop.m_fLifeTimeVar * drx_random(-1.0f, 1.0f));

				// Increase/decrease weight randomly
				cluster.m_fWeight = clamp_tpl<float>(pNewDrop.m_fWeight + pNewDrop.m_fWeightVar * drx_random(-1.0f, 1.0f), 0.1f, 1.0f);
...
void CSnowStage::RenderSnowClusters()
{
	...
	u32 renderState = GS_BLSRC_SRCALPHA | GS_BLDST_ONEMINUSSRCALPHA | GS_DEPTHFUNC_LEQUAL;
	...
	static CDrxNameTSCRC tech0Name("SceneSnow");
	...
	for (auto& cluster : m_snowClusterArray)
	{
		...
		// update constant buffer
		{
			...
			auto constants = pPrim->GetConstantUpr().BeginTypedConstantUpdate<SSnowClusterCB>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex | EShaderStage_Pixel);
			...
			pPrim->GetConstantUpr().EndTypedConstantUpdate(constants);
		}
...
void CSnowStage::ExecuteHalfResComposite()
{
	...
	if (pass.IsDirty())
	{
		static CDrxNameTSCRC techName = "SnowHalfResComposite";
...
void CSnowStage::GetScissorRegion(const Vec3& cameraOrigin, const Vec3& vCenter, float fRadius, i32& sX, i32& sY, i32& sWidth, i32& sHeight) const
{
	CD3D9Renderer* const RESTRICT_POINTER rd = gcpRendD3D;
	...
	sX = (i32)(vMin.x * fWidth);
	sY = (i32)((1.0f - vMax.y) * fHeight);
	sWidth = (i32)ceilf((vMax.x - vMin.x) * fWidth);
	sHeight = (i32)ceilf((vMax.y - vMin.y) * fHeight);
}
