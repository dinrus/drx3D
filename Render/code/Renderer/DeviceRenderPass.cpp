...
#include <drx3D/Render/DeviceRenderPass.h>
...
bool CDeviceRenderPassDesc::SetResources(const CDeviceRenderPassDesc& other)
{
	...
	{
		const SResourceBindPoint bindPoint(SResourceBindPoint::ESlotType::TextureAndBuffer, u8(slot), EShaderStage_Pixel);
		bResourcesInvalidated |= UpdateResource(bindPoint, m_renderTargets[slot], other.m_renderTargets[slot]);
	}

	{
		const SResourceBindPoint bindPoint(SResourceBindPoint::ESlotType::TextureAndBuffer, u8(-1), EShaderStage_Pixel);
	...
	{
		const SResourceBindPoint bindPoint(SResourceBindPoint::ESlotType::UnorderedAccessView, u8(slot), EShaderStage_Pixel);
...
CDeviceRenderPassDesc::CDeviceRenderPassDesc(const CDeviceRenderPassDesc& other, uk pInvalidateCallbackOwner, const SResourceBinding::InvalidateCallbackFunction& invalidateCallback)
{
...
bool CDeviceRenderPassDesc::GetDeviceRendertargetViews(std::array<D3DSurface*, MaxRendertargetCount>& views, i32& viewCount) const
{
	viewCount = 0;
...
bool CDeviceRenderPassDesc::SetRenderTarget(u32 slot, CTexture* pTexture, ResourceViewHandle hView)
{
	DRX_ASSERT(slot < MaxRendertargetCount);
...
{
	DRX_ASSERT(!pTexture || !m_renderTargets[0].pTexture || (
...
bool CDeviceRenderPassDesc::SetOutputUAV(u32 slot, CGpuBuffer* pBuffer)
{
	DRX_ASSERT(slot < MaxOutputUAVCount);
...
bool CDeviceRenderPassDesc::OnResourceInvalidated(uk pThis, SResourceBindPoint bindPoint, UResourceReference pResource, u32 flags) threadsafe
{
	...
	{
		UResourceReference pFoundResource = (uk )nullptr;

		if (bindPoint.slotType == SResourceBindPoint::ESlotType::UnorderedAccessView)
		{
			pFoundResource = DrxInterlockedCompareExchangePointer((uk *)&pPass->m_outputUAVs[bindPoint.slotNumber], nullptr, pResource.pAnonymous);
		}
		else if (bindPoint.slotType == SResourceBindPoint::ESlotType::TextureAndBuffer)
		{
			if ((int8)bindPoint.slotNumber >= 0)
				pFoundResource = DrxInterlockedCompareExchangePointer((uk *)&pPass->m_renderTargets[bindPoint.slotNumber], nullptr, pResource.pAnonymous);
			else
				pFoundResource = DrxInterlockedCompareExchangePointer((uk *)&pPass->m_depthTarget, nullptr, pResource.pAnonymous);
		}
...
bool CDeviceRenderPass_Base::Update(const CDeviceRenderPassDesc& passDesc)
{
	...
	DRX_ASSERT(m_targetFormats == currentFormats);
#endif
...
{
	DRX_ASSERT(psoDesc.m_pRenderPass != nullptr);
...
const CDeviceInputStream* CDeviceObjectFactory::CreateVertexStreamSet(u32 numStreams, const SStreamInfo* streams)
{
