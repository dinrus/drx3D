...
#if defined(AMD_LIQUID_VR) && !DRX_RENDERER_OPENGL
	#ifdef _M_AMD64
...
CStereoRenderingScope::CStereoRenderingScope(STxt &&tag, SDisplayContextKey displayContextKey)
	...
	, m_bPreviousCameraValid(false)
{
	...
	if (_hack_bUseMGPUAffinity)
	{
		...
		{
			pFnEnable(TRUE);
			DrxLogAlways("AMD LIQUID VR library loaded");
		}
		else
		{
			DrxLogAlways("AMD LIQUID VR library failed to loaded");
		}
	}
#endif
}
...
void CD3DStereoRenderer::InitDeviceAfterD3D()
{
	...
	g_pMgpu = NULL;
#if (DRX_RENDERER_DIRECT3D >= 110) && (DRX_RENDERER_DIRECT3D < 120) && defined(AMD_LIQUID_VR)
...
void CD3DStereoRenderer::PrepareStereo()
{
	...
	if (m_device != EStereoDevice::STEREO_DEVICE_NONE && m_mode != EStereoMode::STEREO_MODE_NO_STEREO)
	{
		...
		auto device = gEnv->pSystem->GetHmdUpr()->GetHmdDevice();
...
void CD3DStereoRenderer::RecreateDisplayContext(std::pair<std::shared_ptr<CCustomRenderDisplayContext>, SDisplayContextKey> &target, STxt name, const ColorF &clearColor, std::vector<_smart_ptr<CTexture>> &&swapChain)
{
...
void CD3DStereoRenderer::CreateEyeDisplayContext(CCamera::EEye eEye, std::vector<_smart_ptr<CTexture>> &&swapChain)
{
	...
	const STxt name = (eEye == CCamera::EEye::eEye_Left ? "LeftEye-SwapChain" : "RightEye-SwapChain");
...
void CD3DStereoRenderer::CreateVrQuadLayerDisplayContext(RenderLayer::EQuadLayers id, std::vector<_smart_ptr<CTexture>> &&swapChain)
{
	...
	const STxt name = STxt("Quad") + number + STxt("-SwapChain");
...
CCamera CD3DStereoRenderer::PrepareCamera(i32 nEye, const CCamera& currentCamera)
{
	...
	if (renderToHmd)
	{
		if (IHmdUpr* pHmdUpr = gEnv->pSystem->GetHmdUpr())
		{
			...
			const HMDCameraSetup cameraSetup = pHmdUpr->GetHMDCameraSetup(nEye, projRatio, n);
...
void CD3DStereoRenderer::ProcessScene(i32 sceneFlags, const SRenderingPassInfo& passInfo)
{
...
SStereoRenderContext CD3DStereoRenderer::PrepareStereoRenderingContext(i32 nFlags, const SRenderingPassInfo& passInfo) const
{
	DRX_ASSERT(gRenDev->m_pRT->IsMainThread());
	...
	if (pHmdUpr)
	{
		...
		IHmdDevice* pHmdDevice = pHmdUpr->GetHmdDevice();
		if (pHmdDevice)
...
void CD3DStereoRenderer::RenderScene(const SStereoRenderContext &context)
{
	...
	DRX_ASSERT(context.flags & (SHDF_STEREO_LEFT_EYE | SHDF_STEREO_RIGHT_EYE));
	...
	DRX_ASSERT_MESSAGE(!m_framePrepared, "Frame already prepared.");
	...
	{
		i32k sceneFlagsLeft = context.flags & ~SHDF_STEREO_RIGHT_EYE;
		i32 sceneFlagsRight = (context.flags & ~SHDF_STEREO_LEFT_EYE) | SHDF_NO_SHADOWGEN;
...
std::pair<EHmdSocialScreen, EHmdSocialScreenAspectMode> CD3DStereoRenderer::GetSocialScreenType() const
{
	...
	if (pHmdSocialScreenCVar->GetIVal() >= -1 && pHmdSocialScreenCVar->GetIVal() < static_cast<i32>(EHmdSocialScreen::FirstInvalidIndex))
...
void CD3DStereoRenderer::RenderSocialScreen(CTexture* pColorTarget, const std::pair<EHmdSocialScreen, EHmdSocialScreenAspectMode> &socialScreenType, const Vec2i &displayResolutions)
{
	...
	static const CDrxNameTSCRC techniqueNameSimple("TextureToTexture");
	static const CDrxNameTSCRC techniqueNameRegion("TextureToTextureReg");
	static const CDrxNameR vsRegionParamName("texToTexParamsTC");
...
void CD3DStereoRenderer::OnResolutionChanged(i32 newWidth, i32 newHeight)
{
...
void CD3DStereoRenderer::CalculateResolution(i32 requestedWidth, i32 requestedHeight, i32* pDisplayWidth, i32 *pDisplayHeight)
{
	switch (m_output)
	{
		...
		case EStereoOutput::STEREO_OUTPUT_HMD:
			if (IHmdDevice* pHmdDevice = gEnv->pSystem->GetHmdUpr()->GetHmdDevice())
			{
				...
				i32k screenWidth  = (i32)floor(float(deviceInfo.screenWidth / 2) * resolutionScale);
				i32k screenHeight = (i32)floor(float(deviceInfo.screenHeight   ) * resolutionScale);
...
void CD3DStereoRenderer::OnHmdDeviceChanged(IHmdDevice* pHmdDevice)
{
	...
	if (m_output == EStereoOutput::STEREO_OUTPUT_HMD && gEnv->pSystem->GetHmdUpr()->GetHmdDevice() == nullptr)
	{
		m_output = EStereoOutput::STEREO_OUTPUT_STANDARD;
		m_device = EStereoDevice::STEREO_DEVICE_NONE;
...
IHmdRenderer* CD3DStereoRenderer::CreateHmdRenderer(IHmdDevice& device)
{
	...
	case eHmdClass_Oculus:
		return new CD3DOculusRenderer(static_cast<DrxVR::Oculus::IOculusDevice*>(&device), gcpRendD3D, this);
	case eHmdClass_OpenVR:
		return new CD3DOpenVRRenderer(static_cast<DrxVR::OpenVR::IOpenVRDevice*>(&device), gcpRendD3D, this);
	case eHmdClass_Osvr:
		return new CD3DOsvrRenderer(static_cast<DrxVR::Osvr::IOsvrDevice*>(&device), gcpRendD3D, this);
	default:
...
//////////////////////////////////////////////////////////////////////////
_smart_ptr<CTexture> CD3DStereoRenderer::WrapD3DRenderTarget(D3DTexture* d3dTexture, u32 width, u32 height, DXGI_FORMAT format, tukk name, bool shaderResourceView)
{
