...
#include <drx3D/CoreX/Renderer/CREBreakableGlass.h>
#include <drx3D/Render/D3DREBreakableGlassBuffer.h>

#include <drx3D/Render/RendElements/Utils/PolygonMath2D.h>
#include <drx3D/Render/RendElements/Utils/SpatialHashGrid.h>
...
// Error logging
#ifndef RELEASE
	#define LOG_GLASS_ERROR(str) DrxLogAlways("[BreakGlassSystem Error]: %s", str)
#else
...
bool CREBreakableGlass::mfDraw(CShader* ef, SShaderPass* sfm)
{
	...
	if (pBufferId && bufferMan.RT_IsBufferValid(*pBufferId, bufferType))
	{
		...
		u32k drawFrame = pRenDev->GetFrameID(false);
		u32k updateFrame = m_geomUpdateFrame;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::RT_UpdateBuffers(i32k subFragIndex)
{
	...
	if (geomRebuilt && m_dirtyGeomBufferCount > 0)
	{
		...
		u32k bufferId = isFrag ? m_fragGeomBufferIds[subFragIndex].m_geomBufferId : m_geomBufferId;
		    ...
		    crackGeom.m_verts.Count() >= 3 && crackGeom.m_inds.Count() >= 3)
		{
			...
			bufferMan.RT_UpdateVertexBuffer(bufferId, bufferType, &planeGeom.m_verts[0], (u32)planeGeom.m_verts.Count());
			bufferMan.RT_UpdateIndexBuffer(bufferId, bufferType, &planeGeom.m_inds[0], (u32)planeGeom.m_inds.Count());
			bufferMan.RT_UpdateTangentBuffer(bufferId, bufferType, &planeGeom.m_tans[0], (u32)planeGeom.m_tans.Count());
			...
			if (!isFrag)
			{
				...
				bufferMan.RT_UpdateVertexBuffer(bufferId, bufferType, &crackGeom.m_verts[0], (u32)crackGeom.m_verts.Count());
				bufferMan.RT_UpdateIndexBuffer(bufferId, bufferType, &crackGeom.m_inds[0], (u32)crackGeom.m_inds.Count());
				bufferMan.RT_UpdateTangentBuffer(bufferId, bufferType, &crackGeom.m_tans[0], (u32)crackGeom.m_tans.Count());
			}
		}
		else
		{
...
void CREBreakableGlass::UpdateImpactShaderConstants()
{
	...
	float minImpactSpeed = GLASSCFG_MIN_DRX3D_SPEED;
	...
	float invImpactScale = __fres(impactScale);
	...
	while (m_numDecalPSConsts < m_numDecalImpacts && m_numDecalImpacts <= GLASSCFG_MAX_NUM_PHYS_FRAGMENTS)
	{
		...
		invDecalScale = __fres(invDecalScale);
...
void CREBreakableGlass::DebugDraw(const bool wireframe, const bool data)
{
	...
	if (wireframe)
	{
		...
		//      IRenderAuxText::DrawLabelExF(fragCenter, 1.2f, fragFontCol, false, true, "A:%s\nL:%s\nF:%s", activeBin, looseBin, freeBin);
	}
	...
	if (data)
	{
		...
		if (!m_impactLineList.IsEmpty())
		{
			...
			DRX_ASSERT(m_impactLineList.Count() % 2 == 0);// Invalid number of verts
...
void CREBreakableGlass::DrawFragmentDebug(IRenderAuxGeom* const pRenderer, SAuxGeomRenderFlags& flags)
{
	...
	u32k invalidState = ~m_fragsActive | m_fragsLoose;
	u32 fragBit = 1;
	...
	for (uint i = 0; i < numFrags; ++i, fragBit <<= 1)
	{
		...
		for (i32 j = 0; j < frag.m_outlinePts.Count(); ++j)
		{
			i32k next = (j + 1) % frag.m_outlinePts.Count();
		...
		// Triangle outlines
		//    for (i32 j = 0; j < frag.m_triInds.Count(); j+=3)
		//    {
		...
		// //
		// //       IRenderAuxText::DrawLabelExF(triCenter, 0.5f, triFontCol, false, true, "%i", j);
		//    }
		...
		//    fragCenter = m_params.matrix.TransformPoint(fragCenter);
		//
		//    IRenderAuxText::DrawLabelExF(fragCenter, 0.8f, fragFontCol, false, true, "%i", i);
	}

	// Transform and draw
	if (!temp.IsEmpty())
	{
		for (i32 i = 0; i < temp.Count(); ++i)
		{
			temp[i].z = -0.005f;
		}
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::DrawFragmentDebug(const uint fragIndex, const Matrix34& matrix, u8k buffId, const float alpha)
{
	PodArray<Vec3> temp;
	...
	if (fragIndex < (uint)m_frags.size())
	{
		...
		const float fragFontCol[4] = { 1.0f, 0.0f, 1.0f, 1.0f };
		IRenderAuxText::DrawLabelExF(fragCenter, 0.5f, fragFontCol, false, true, "B:%i, F:%i", buffId, fragIndex);
	}

	if (!temp.IsEmpty())
	{
		// Transform
		i32k numPts = temp.Count();
...
void CREBreakableGlass::DrawFragmentConnectionDebug(IRenderAuxGeom* const pRenderer, SAuxGeomRenderFlags& flags)
{
	...
	PodArray<i32> anchorFrags, stableFrags;
	u8k numAnchors = m_glassParams.numAnchors;

	// Find all anchored fragments
	for (i32 i = 0; i < numAnchors; ++i)
	{
		...
		Vec3 a(Vec3Constants<float>::fVec3_Zero);
		Vec3 b(Vec3Constants<float>::fVec3_Zero);
		sincos_tpl(0.0f, &a.x, &a.y);

		const float angleInc = gf_PI2 / (float)numSides;
		float angle = angleInc;

		for (i32 j = 0; j < numSides; ++j, angle += angleInc)
		{
		...
		//    IRenderAuxText::DrawLabelExF(m_params.matrix.TransformPoint(center), 0.8f, fontCol, false, true, "%i", i);
	}
	...
	for (uint i = 0; i < numFrags; ++i, fragBit <<= 1)
	{
		...
		for (i32 j = 0; j < frag.m_outConnections.Count(); ++j)
		{
			// Get end frag center
			i32 m = frag.m_outConnections[j];
	...
	{
		for (i32 i = 0; i < anchorLines.Count(); ++i)
		{
	...
	{
		for (i32 i = 0; i < connLinesA.Count(); ++i)
		{
	...
	{
		for (i32 i = 0; i < connLinesB.Count(); ++i)
		{
