...
/*
	...
	static CDrxNameTSCRC pTechName("VolumetricScattering");
	u32 nPasses;
	...
	static CDrxNameR pParam0Name("VolumetricScattering");
	static CDrxNameR pParam1Name("VolumetricScatteringColor");

	static CDrxNameR pParam2Name("PI_volScatterParamsVS");
	static CDrxNameR pParam3Name("PI_volScatterParamsPS");
	...
	CDrxNameTSCRC pTechName0("VolumetricScatteringFinal");
...
/*
	...
	if (HasModelsToRender() && IsActive())
	{
		...
		for (u8 groupId = 0; groupId < m_groupCount; groupId++)
		{
			RenderGroup(groupId);
		}
	}
...
/*
	...
	u8k zbufferRenderTile = 2; // Use zbuffer render tile, because in stereo the 1st 2 are used for the eyes

	CClearSurfacePass::Execute(m_pFlashRT, Clr_Transparent);
*/
}

void CPost3DRenderer::RenderGroup(u8 groupId)
{
...
void CPost3DRenderer::RenderMeshes(u8 groupId, float screenRect[4], ERenderMeshMode renderMeshMode)
{
	// OLD PIPELINE
	ASSERT_LEGACY_PIPELINE
}
...
void CPost3DRenderer::RenderSilhouettes(u8 groupId, float screenRect[4])
{
