...
void CFogStage::ResizeResource(i32 resourceWidth, i32 resourceHeight)
{
...
void CFogStage::Resize(i32 renderWidth, i32 renderHeight)
{
#if defined(VOLUMETRIC_FOG_SHADOWS)
	// Recreate render targets if quality was changed
	i32k resolutionScale = (CRenderer::CV_r_FogShadows == 1) ? 2 : 4;

	i32k width  = renderWidth  / resolutionScale;
	i32k height = renderHeight / resolutionScale;
...
void CFogStage::OnCVarsChanged(const CCVarUpdateRecorder& cvarUpdater)
{
	...
	if (auto pVar = cvarUpdater.GetCVar("r_FogShadows"))
	{
		...
		// Recreate render targets if quality was changed
		i32k resolutionScale = (pVar->intValue == 1 ? 2 : 4);
...
void CFogStage::Execute()
{
	...
	i32 width  = RenderView()->GetViewport().width;
	i32 height = RenderView()->GetViewport().height;
	...
	// render fog
	{
		...
		i32k nSvoGiTexId = (pSR && pSR->GetTroposphereMinRT()) ? pSR->GetTroposphereMinRT()->GetTextureID() : 0;
#else
		i32k nSvoGiTexId = 0;
#endif
		i32k viewInfoCount = GetStdGraphicsPipeline().GetViewInfoCount();
		...
		if (m_passFog.IsDirty(inputFlag, nSvoGiTexId, RenderView()->GetDepthTarget()->GetTextureID()))
		{
			...
			static CDrxNameTSCRC techName("FogPass");
			...
			u32 nRS = GS_BLSRC_ONE | GS_BLDST_SRCALPHA | (bFogDepthTest ? GS_DEPTHFUNC_LEQUAL : GS_NODEPTHTEST);
		...
		static CDrxNameR szHDRParams("HDRParams2");
		...
		if (bVolFogShadow)
		{
			...
			static CDrxNameR volFogShadowDarkeningN("volFogShadowDarkening");
			...
			static CDrxNameR volFogShadowDarkeningSunAmbN("volFogShadowDarkeningSunAmb");
			...
			static CDrxNameR volFogShadowRangeN("volFogShadowRange");
			...
			static CDrxNameR volFogShadowBufSampleOffsetsN("volFogShadowBufSampleOffsets");
			m_passFog.SetConstantArray(volFogShadowBufSampleOffsetsN, sampleOffsets, 5, eHWSC_Pixel);
		}
#endif

#if defined(FEATURE_SVO_GI)
		static CDrxNameR sSVO_AirTextureScale("SVO_AirTextureScale");
	...
	if (bLightning)
	{
		if (m_passLightning.IsDirty())
		{
			...
			static CDrxNameTSCRC techName("FogPassWithLightning");
		...
		static CDrxNameR Param1Name("LightningPos");
		...
		static CDrxNameR Param2Name("LightningColSize");
...
void CFogStage::ExecuteVolumetricFogShadow()
{
	...
	// ray cast into sun shadow maps
	{
		...
		if (m_passVolFogShadowRaycast.IsDirty(inputFlag, CRenderer::CV_r_FogShadowsMode))
		{
			...
			static CDrxNameTSCRC TechName0("FogPassVolShadowsInterleavePass");
			static CDrxNameTSCRC TechName1("MultiGSMShadowedFog");
		...
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("TexGen0"), shadowSamplingInfo.shadowTexGen[0]);
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("TexGen1"), shadowSamplingInfo.shadowTexGen[1]);
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("TexGen2"), shadowSamplingInfo.shadowTexGen[2]);
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("TexGen3"), shadowSamplingInfo.shadowTexGen[3]);
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("vInvShadowMapSize"), shadowSamplingInfo.invShadowMapSize);
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("fDepthTestBias"), shadowSamplingInfo.depthTestBias);
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("fOneDivFarDist"), shadowSamplingInfo.oneDivFarDist);
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("fKernelRadius"), shadowSamplingInfo.kernelRadius);
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("CloudShadowParams"), shadowSamplingInfo.cloudShadowParams);
		m_passVolFogShadowRaycast.SetConstant(CDrxNameR("CloudShadowAnimParams"), shadowSamplingInfo.cloudShadowAnimParams);
		m_passVolFogShadowRaycast.SetConstantArray(CDrxNameR("irreg_kernel_2d"), shadowSamplingInfo.irregKernel2d, 8);
	...
	// horizontal blur
	{
		...
		{
			static CDrxNameTSCRC TechName("FogPassVolShadowsGatherPass");
		...
		for (i32 x = -4, index = 0; x < 4; ++x, ++index)
		{
	...
	// vertical blur
	{
		...
		for (i32 y = -4, index = 0; y < 4; ++y, ++index)
		{
