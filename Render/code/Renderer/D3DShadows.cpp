...
BOOL InflateRect(LPRECT lprc, i32 dx, i32 dy)
{
	if (!lprc)
		return FALSE;
...
namespace
{
DrxCriticalSection g_cDynTexLock;
}
...
void CD3D9Renderer::EF_PrepareShadowGenRenderList(const SRenderingPassInfo& passInfo)
{
	...
	i32 NumDynLights = arrDynLights.size();
	i32 NumDeferLights = arrDeferLights.size();
...

	{
		...
		for (u32 nDynLightID = 0; itr != arrDynLights.end(); ++itr, ++nDynLightID)
		{
...

	{
		...
		for (u32 nDeferLightID = NumDynLights; itr != arrDeferLights.end(); ++itr, ++nDeferLightID)
	...
	if (haveSun)
	{
		...
		for (u32 i = 0; i < nFrustumCount; ++i)
		{
			ShadowMapFrustum* pCurFrustum = arrCustomFrustums[i];
			IRenderViewPtr pShadowView = pCurFrustum->pOnePassShadowView;

			DRX_ASSERT(pCurFrustum && pCurFrustum->m_eFrustumType == ShadowMapFrustum::e_PerObject);
	...
	for (i32 slice = 0; slice < pRenderView->m_shadows.m_frustumsPerTiledShadingSlice.size(); ++slice)
	{
...
bool CD3D9Renderer::EF_PrepareShadowGenForLight(CRenderView* pRenderView, SRenderLight* pLight, i32 nLightID)
{
	assert((u32) nLightID < (MAX_REND_LIGHTS + MAX_DEFERRED_LIGHTS));
	if ((u32) nLightID >= (MAX_REND_LIGHTS + MAX_DEFERRED_LIGHTS))
	{
	...
	i32 nCurLOD = 0;//max(nFrustumLOD, nCaster);

	for (i32 nCaster = 0; *ppSMFrustumList && nCaster != MAX_GSM_LODS_NUM; ++ppSMFrustumList, ++nCaster)
	{
...
void CD3D9Renderer::PrepareShadowPool(CRenderView* pRenderView) const
{
	...
	i32 nDLights = pRenderView->GetDynamicLightsCount();
...
bool CD3D9Renderer::PrepareShadowGenForFrustum(CRenderView* pRenderView, ShadowMapFrustum* pCurFrustum, const SRenderLight* pLight, i32 nLightID, i32 nLOD)
{
...
CShadowUtils::SShadowsSetupInfo CD3D9Renderer::ConfigShadowTexgen(CRenderView* pRenderView, const ShadowMapFrustum* pFr, i32 nFrustNum, bool bScreenToLocalBasis)
{
