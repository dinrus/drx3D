...
/*
	...
	if ((SRendItem::BatchFlags(EFSLIST_GENERAL) | SRendItem::BatchFlags(EFSLIST_TRANSP)) & FB_CUSTOM_RENDER)
	{
		// could render directly to frame buffer ? save 1 resolve - no glow though
		{
			...
			static CDrxNameTSCRC pTech1Name("BinocularView");
		...
		{
			static CDrxNameTSCRC pTechSilhouettesName("BinocularViewSilhouettes");
...
/*
	...
	static CDrxNameTSCRC pTechName("AlienInterference");
	...
	static CDrxNameR pParamAlienInterferenceName("AlienInterferenceTint");
...
/*
	...
	if (fAmount <= 0.02f)
	{
		m_fRandOffset = drx_random(0.0f, 1.0f);
		return;
	}
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////
	// display frost
	static CDrxNameTSCRC pTechName("ScreenFrost");
	PostProcessUtils().ShBeginPass(CShaderMan::s_shPostEffectsGame, pTechName, FEF_DONTSETSTATES);

	gcpRendD3D->FX_SetState(GS_NODEPTHTEST);

	static CDrxNameR pParam0Name("screenFrostParamsVS");
	static CDrxNameR pParam1Name("screenFrostParamsPS");
...
void CRainDrops::SpawnParticle(SRainDrop*& pParticle, i32 iRTWidth, i32 iRTHeight)
{
	...
	if (drx_random(0.0f, 1.0f) > 0.5f && fabs(PostProcessUtils().m_pTimer->GetCurrTime() - fLastSpawnTime) > m_pSpawnTimeDistance->GetParam())
	{
		pParticle->m_pPos.x = drx_random(0.0f, 1.0f);
		pParticle->m_pPos.y = drx_random(0.0f, 1.0f);

		pParticle->m_fLifeTime = pNewDrop.m_fLifeTime + pNewDrop.m_fLifeTimeVar * drx_random(-1.0f, 1.0f);
		//pParticle->m_fSize = (pNewDrop.m_fSize + pNewDrop.m_fSizeVar * drx_random(-1.0f, 1.0f)) * 2.5f;
		//pNewDrop.m_fSize + pNewDrop.m_fSizeVar
		pParticle->m_fSize = 1.0f / (10.0f * (fUserSize + 0.5f * fUserSizeVar * drx_random(-1.0f, 1.0f)));
...
void CRainDrops::UpdateParticles(i32 iRTWidth, i32 iRTHeight)
{
	ASSERT_LEGACY_PIPELINE
/*
	...
	m_nAliveDrops = 0;
	static i32 s_nPrevAliveDrops = 0;
	...
	for (pItor = m_pDropsLst.begin(); pItor != pItorEnd; ++pItor)
	{
		...
		// add gravity
		pCurr->m_pPos.y += m_pVelocityProj.y * drx_random(-0.2f, 1.0f);
		...
		pCurr->m_pPos.x += m_pVelocityProj.x * drx_random(-0.2f, 1.0f);
		pCurr->m_pPos.x += fCurrFrameTime * std::min(pCurr->m_fWeight, 0.25f * pCurr->m_fSize) * fGravity * drx_random(-1.0f, 1.0f);

		// Increase/decrease weight randomly
		pCurr->m_fWeight = clamp_tpl<float>(pCurr->m_fWeight + fCurrFrameTime * pCurr->m_fWeightVar * drx_random(-4.0f, 4.0f), 0.0f, 1.0f);
	}
...
Matrix44 CRainDrops::ComputeCurrentView(i32 iViewportWidth, i32 iViewportHeight)
{
	ASSERT_LEGACY_PIPELINE
	return Matrix44();
...
void CRainDrops::Render()
{
	...
	i32 iViewportX, iViewportY, iViewportWidth, iViewportHeight;
	...
	u16 uPrevDytex = (m_uCurrentDytex + 1) % 2;
	...
	assert(rpCurrDytex);
	i32 iRTWidth = rpCurrDytex->GetWidth();
	i32 iRTHeight = rpCurrDytex->GetHeight();
...
void CRainDrops::DrawRaindrops(i32 iViewportWidth, i32 iViewportHeight, i32 iRTWidth, i32 iRTHeight)
{
	ASSERT_LEGACY_PIPELINE
		/*
	...
	//if( fDot >= - 0.25)
	{
		...
		static CDrxNameTSCRC pTech0Name("RainDropsGen");
		...
		for (pItor = m_pDropsLst.begin(); pItor != pItorEnd; ++pItor)
		{
			...
			static CDrxNameR pParam0Name("vRainParams");
...
void CRainDrops::ApplyExtinction(CTexture*& rptexPrevRT, i32 iViewportWidth, i32 iViewportHeight, i32 iRTWidth, i32 iRTHeight)
{
	ASSERT_LEGACY_PIPELINE
		/*
...
/*
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////
	static CDrxNameTSCRC pTechName("FlashBang");
...
/*
	...
	{
		m_cornerGlowCountdownTimer = drx_random(0.0f, maxGlowRandTime);
		m_cornerGlowStrength = minCornerGlow + LERP(minCornerGlow, maxCornerGlow, drx_random(0.0f, 1.0f));
	}
	...
	if (pRenderData.pRenderElement && pShaderTech)
	{
		...
		// Render state
		i32 nRenderState = GS_NODEPTHTEST | GS_BLSRC_SRCALPHA | GS_BLDST_ONEMINUSSRCALPHA;
...
/*
	...
	static CDrxNameTSCRC pTechName("ScreenBlood");
