...
#include <drx3D/Render/StdAfx.h>
	...
	#include <drx3D/Render/BenchmarkRendererSensor.h>
...
static BenchmarkFramework::IBenchmarkFramework* GetFrameworkInstance()
{
	if (!s_instance)
	{
		HMODULE hlib = DrxLoadLibrary("BenchmarkFramework.dll");
		if (hlib)
		{
			FUNCTIONPOINTER_GetGlobalBenchmarkFrameworkInstance pf = (FUNCTIONPOINTER_GetGlobalBenchmarkFrameworkInstance)DrxGetProcAddress(hlib, "GetGlobalBenchmarkFrameworkInstance");
			if (pf)
			{
				s_instance = pf();
			}
...

{
	if (GetFrameworkInstance())
		GetFrameworkInstance()->getRendererSensorUpr()->setBenchmarkRendererSensor(this);
}
...
BenchmarkRendererSensor::~BenchmarkRendererSensor()
{
	...
	if (GetFrameworkInstance())
	{
		GetFrameworkInstance()->getRendererSensorUpr()->setBenchmarkRendererSensor(0);
	}
	SAFE_DELETE(m_timer);
	s_instance = 0;
}
...
/*void BenchmarkRendererSensor::setMatrixTextureResolution(i32 w, i32 h)
   {
   if(gRenDev->m_pRT->IsRenderThread())
   {
    m_matrixTexWidth = w;
    ...
    CommandQueue& queue = m_commandQueue[gRenDev->m_pRT->m_nCurThreadFill];
    byte* ptr = queue.pushCommand(SetMatrixTextureResolution, sizeof(i32) * 2);
    queue.pushData(ptr, w);
    queue.pushData(ptr, h);
   }

   }*/
void BenchmarkRendererSensor::setFrameSampling(i32 interval, i32 flags)
{
	if (gRenDev->m_pRT->IsRenderThread())
	{
	...
	{
		CommandQueue& queue = m_commandQueue[gRenDev->m_pRT->m_nCurThreadFill];
		byte* ptr = queue.pushCommand(SetFrameSampling, sizeof(i32) * 2);
...
/*void BenchmarkRendererSensor::setMatrixDimensions(i32 rows, i32 columns)
   {
   ...
   if(gRenDev->m_pRT->IsRenderThread())
   {
    ...
    CommandQueue& queue = m_commandQueue[gRenDev->m_pRT->m_nCurThreadFill];
    byte* ptr = queue.pushCommand(SetMatrixDimensions, sizeof(i32) * 2);
...
void BenchmarkRendererSensor::overrideMonitorWindowSize(bool override, i32 width, i32 height, bool fullscreen)
{
	if (gRenDev->m_pRT->IsRenderThread())
	{
	...
	{
		CommandQueue& queue = m_commandQueue[gRenDev->m_pRT->m_nCurThreadFill];
		byte* ptr = queue.pushCommand(ForceWindowSize, sizeof(i32) * 4);
...
void BenchmarkRendererSensor::enableFlicker(bool enable, BenchmarkFramework::FlickerFunction func)
{
	...
	{
		CommandQueue& queue = m_commandQueue[gRenDev->m_pRT->m_nCurThreadFill];
		byte* ptr = queue.pushCommand(SetupFlicker, sizeof(i32) + sizeof(BenchmarkFramework::FlickerFunction));
...
void BenchmarkRendererSensor::processCommands()
{
	...
	while (!queue.isQueueEmpty())
	{
		...
		case ForceWindowSize:
			{
				bool enable = queue.popData<i32>() > 0;
				i32 width = queue.popData<i32>();
				i32 height = queue.popData<i32>();
				bool enableFullscreen = queue.popData<i32>() != 0;
				overrideMonitorWindowSize(enable, width, height, enableFullscreen);
			}
			break;
		case SetupFlicker:
			{
				i32 enable = queue.popData<i32>();
		...
		/*case SetMatrixTextureResolution:
		   ...
		   {
		   i32 rows = queue.popData<i32>();
		   i32 columns = queue.popData<i32>();
		   setMatrixDimensions(rows, columns);
		   }
		   break;*/
		case SetFrameSampling:
			{
				i32 interval = queue.popData<i32>();
				i32 flags = queue.popData<i32>();
...
void BenchmarkRendererSensor::forceFlush(CDrxDeviceWrapper& device, CDrxDeviceContextWrapper& context)
{
	context.Flush();
	...
	do
	{
		hr = context.GetData(query, (uk )&data, sizeof(BOOL), 0);
	}
	while (hr == S_FALSE);
...
/*void BenchmarkRendererSensor::copyFrameToMatrix(CTexture* texture)
   ...
   i32 vpW = m_matrixTexWidth / m_columns;
   i32 vpH = m_matrixTexHeight / m_rows;

   i32 indX = m_currentIndex  % m_columns;
   i32 indY = m_currentIndex / m_columns;
   ...
   i32 width = m_renderer->GetWidth();
   i32 height = m_renderer->GetHeight();
...
void BenchmarkRendererSensor::afterSwapBuffers(CDrxDeviceWrapper& device, CDrxDeviceContextWrapper& context)
{
