...
void CSceneGBufferStage::ExecuteSceneOpaque()
{
	...
	i32 numItems_Nearest = pRenderView->GetRenderItems(EFSLIST_NEAREST_OBJECTS).size();
	i32 velocityEnd_Nearest = pRenderView->FindRenderListSplit(EFSLIST_NEAREST_OBJECTS, FOB_HAS_PREVMATRIX);

	i32 numItems_General = pRenderView->GetRenderItems(EFSLIST_GENERAL).size();
	i32 velocityEnd_General = pRenderView->FindRenderListSplit(EFSLIST_GENERAL, FOB_HAS_PREVMATRIX);
	i32 numItems_Skin = pRenderView->GetRenderItems(EFSLIST_SKIN).size();
	i32 velocityEnd_Skin = pRenderView->FindRenderListSplit(EFSLIST_SKIN, FOB_HAS_PREVMATRIX);
...
void CSceneGBufferStage::ExecuteLinearizeDepth()
{
	...
	{
		static CDrxNameTSCRC techLinearizeDepth("LinearizeDepth");
	...
	static CDrxNameR paramName("NearProjection");
...
void CSceneGBufferStage::ExecuteGBufferVisualization()
{
	...
	if (CRenderer::CV_r_DeferredShadingDebugGBuffer)
	{
		...
		static CDrxNameTSCRC tech("DebugGBuffer");
		...
		static CDrxNameR paramName("DebugViewMode");
...
void CSceneGBufferStage::Execute()
{
...

	{
		...
		if (CVrProjectionUpr::Instance()->GetProjectionType() == CVrProjectionUpr::eVrProjection_LensMatched)
		{
			...
			CVrProjectionUpr::Instance()->ExecuteLensMatchedOctagon(pRenderView->GetDepthTarget());
		}
		else
		{
