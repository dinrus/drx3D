...
{
	for (u32 i = 0; i < kNumPendingFrames; i++)
	{
		m_frameData[i].m_timestampGroup.Init();
	}
}

void CRenderPipelineProfiler::BeginFrame(i32k frameID)
{
...
void CRenderPipelineProfiler::EndFrame()
{
	...
	// Backup current CPU and GPU counters
	{
		...
		u32k fillThreadID = (u32)gRenDev->GetMainThreadID();
		SFrameData& frameData = *m_frameDataRT;
	...
	i32 prevFrameIndex = -1;
	for (i32 i = 1; i < kNumPendingFrames; ++i)
	{
		i32 tryFrameIndex = (m_frameDataIndex + (kNumPendingFrames - i)) % kNumPendingFrames;

		DRX_ASSERT((m_frameData + tryFrameIndex) != m_frameDataRT);
	...
	if (prevFrameIndex >= 0)
	{
		DRX_ASSERT(m_frameData[prevFrameIndex].m_frameID != 0);
...
bool CRenderPipelineProfiler::FilterLabel(tukk name)
{
	return
...
u32 CRenderPipelineProfiler::InsertSection(tukk name, u32 profileSectionFlags)
{
	DRX_ASSERT(!m_recordData || (profileSectionFlags & eProfileSectionFlags_RootElement) || (m_stack.size() > 0));
	...
	u32k sectionPos = frameData.m_numSections++;
	SProfilerSection& section = frameData.m_sections[sectionPos];

	drx_strcpy(section.name, name);
	...
	string path = "";
	for (i32 i = 0; i < m_stack.size(); i++)
	{
		path += "\\";
		path += frameData.m_sections[m_stack[i]].name;
	}
	section.path = CDrxNameTSCRC(path);

	return sectionPos;
}

void CRenderPipelineProfiler::BeginSection(tukk name)
{
	InsertSection(name, 0);
}

void CRenderPipelineProfiler::EndSection(tukk name)
{
	if (!m_recordData || FilterLabel(name))
		return;
...
u32 CRenderPipelineProfiler::InsertMultithreadedSection(tukk name)
{
	u32 index = InsertSection(name, eProfileSectionFlags_MultithreadedSection);
	EndSection(name);
	return index;
}

void CRenderPipelineProfiler::UpdateMultithreadedSection(u32 index, bool bSectionStart, i32 numDIPs, i32 numPolys, bool bIssueTimestamp, CTimeValue deltaTimestamp, CDeviceCommandList* pCommandList)
{
...

	{
...
#if defined(ENABLE_PROFILING_CODE)
		DrxInterlockedAdd(&section.numDIPs, numDIPs);
		DrxInterlockedAdd(&section.numPolys, numPolys);
#endif
		...
		if (bIssueTimestamp)
		{
			static DrxCriticalSection s_lock;
			AUTO_LOCK(s_lock);
...
void CRenderPipelineProfiler::UpdateSectionTimesAndStats(u32 frameDataIndex)
{
	SFrameData& frameData = m_frameData[frameDataIndex];
	DRX_ASSERT(&frameData != m_frameDataRT);

	// This has been updated previously (frame-stats is repeated because of GPU delay)
	if (frameData.m_updated)
		return;

	for (u32 i = 0, n = frameData.m_numSections; i < n; ++i)
	{
	...
	i32 drawcallSum[16] = { 0 };
	i32 polygonSum[16] = { 0 };
	i32 curRecLevel = frameData.m_numSections > 0 ? frameData.m_sections[frameData.m_numSections - 1].recLevel : 0;

	for (i32 i = frameData.m_numSections - 1; i >= 0; --i)
	{
		SProfilerSection& section = frameData.m_sections[i];

		if (section.recLevel >= static_cast<int8>(DRX_ARRAY_COUNT(drawcallSum)))
		{
			assert(0);
			continue;
		}
		...
		{
			for (u32 j = curRecLevel; j < DRX_ARRAY_COUNT(drawcallSum); j++)
			{
...
void CRenderPipelineProfiler::UpdateThreadTimings(u32 frameDataIndex)
{
	SFrameData& frameData = m_frameData[frameDataIndex];
	DRX_ASSERT(&frameData != m_frameDataRT);
...
{
	static i32 s_frameCounter = 0;
	i32k kUpdateFrequency = 60;

	u32k processThreadID = (u32)gRenDev->GetRenderThreadID();
	u32k fillThreadID = (u32)gRenDev->GetMainThreadID();
	...
	for (u32 i = 0, n = currData.m_numSections; i < n; ++i)
	{
...
void CRenderPipelineProfiler::UpdateStats(u32 frameDataIndex)
{
	RPProfilerStats* pBasicStats = m_basicStats[gRenDev->GetRenderThreadID()];
	DynArray<RPProfilerDetailedStats> &detailedStats = m_detailedStats[gRenDev->GetRenderThreadID()];
...
void CRenderPipelineProfiler::DisplayDetailedPassStats(u32 frameDataIndex)
{
#ifndef _RELEASE
	...
	SFrameData& frameData = m_frameData[frameDataIndex];
	DRX_ASSERT(&frameData != m_frameDataRT);
	u32 elemsPerColumn = (pDC->GetDisplayResolution()[1] - 60) / 16;
	...
	i32 frameDist = m_frameDataIndex - frameDataIndex;
	...
	// Reset the used flag
	for (std::multimap<CDrxNameTSCRC, SStaticElementInfo>::iterator it = m_staticNameList.begin(); it != m_staticNameList.end(); ++it)
	{
	...
	for (u32 i = 0, n = frameData.m_numSections; i < n; ++i)
	{
		...
		std::multimap<CDrxNameTSCRC, SStaticElementInfo>::iterator it = m_staticNameList.lower_bound(section.path);
		...
		float xpos = 20.0f + ((i32)(it->second.nPos / elemsPerColumn)) * 600.0f;
		...
		IRenderAuxText::Draw2dLabel(xpos + max((i32)(abs(section.recLevel) - 2), 0) * 15.0f, ypos, 1.5f, &color.r, false, "%s", section.name);
...
void DrawText(float x, float y, float size, ColorF color, tukk text)
{
	// TODO: relative/normalized coordinate system in screen-space
...
void DrawText(float x, float y, float size, ColorF color, tukk format, va_list args)
{
	char buffer[512];
...
void DrawTable(float x, float y, float width, i32 numColumns, tukk title)
{
...
void DrawTableColumn(float tableX, float tableY, i32 columnIndex, tukk columnText, ...)
{
	va_list args;
...
void DrawTableBar(float x, float tableY, i32 columnIndex, float percentage, ColorF color)
{
...
void CRenderPipelineProfiler::DisplayOverviewStats(u32 frameDataIndex)
{
#ifndef _RELEASE
	if (gEnv->pConsole->IsOpened())
		return;
	...
	// GPU times
	{
		...
		i32 numColumns = sizeof(statsGroups) / sizeof(StatsGroup);
