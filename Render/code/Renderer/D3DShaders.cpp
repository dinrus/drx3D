...
void CShaderMan::mfCheckObjectDependParams(std::vector<SCGParam>& PNoObj, std::vector<SCGParam>& PObj, EHWShaderClass eSH, CShader* pFXShader)
{
	...
	for (i = 0; i < PNoObj.size(); i++)
	{
		...
		{
			i32 nType = prNoObj->m_eCGParamType & 0xff;
...
void CHWShader_D3D::mfLogShaderCacheMiss(SHWSInstance* pInst)
{
	...
	char nameCache[256];
	drx_strcpy(nameCache, GetName());
	tuk s = strchr(nameCache, '(');
	if (s)
		s[0] = 0;
	string sNew;
	...
	{
		DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_WARNING, "[SHADERS] GCM Global Cache Miss: %s\n", sNew.c_str());
	}
	...
	CDrxNameTSCRC drxName(sEntry);

	// do we already contain this entry (vec is sorted so lower bound gives us already the good position to insert)
	CShaderMan::ShaderCacheMissesVec::iterator it = std::lower_bound(Man.m_ShaderCacheMisses.begin(), Man.m_ShaderCacheMisses.end(), drxName);
	if (it == Man.m_ShaderCacheMisses.end() || drxName != (*it))
	{
		Man.m_ShaderCacheMisses.insert(it, drxName);
