		...
		for (auto& re : lensOpticsElements)
		{
...

			{
				...
				if (pLight->m_LensOpticsFrustumAngle != 0)
				{
					...
					{
						i32 angle = (i32)((float)pLight->m_LensOpticsFrustumAngle * (360.0f / 255.0f));
				...
				if (pRootElem->ProcessAll(m_passLensOptics, prePasses, flareLight, viewInfo, viewInfoCount, false, bUpdateOcclusion))
				{
					if (bUpdateOcclusion)
					{
						m_softOcclusionUpr.AddSoftOcclusionQuery(pOcc, pLight->GetPosition());
					}
				}
...
void CLensOpticsStage::UpdateOcclusionQueries(SRenderViewInfo* pViewInfo, i32 viewInfoCount)
{
	PROFILE_LABEL_SCOPE("Soft Occlusion Query");

	CFlareSoftOcclusionQuery::BatchReadResults(); // copy to system memory previous frame

	m_softOcclusionUpr.Update(pViewInfo, viewInfoCount);

	CFlareSoftOcclusionQuery::ReadbackSoftOcclQuery(); // update current frame to staging buffer
	for (i32 i = 0, iSize(m_softOcclusionUpr.GetSize()); i < iSize; ++i)
	{
		CFlareSoftOcclusionQuery* pSoftOcclusion = m_softOcclusionUpr.GetSoftOcclusionQuery(i);
		if (pSoftOcclusion == NULL)
			continue;
		pSoftOcclusion->UpdateCachedResults();
	}

	m_softOcclusionUpr.Reset();
}