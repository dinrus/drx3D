...
#include <drx3D/Render/StdAfx.h>
	...
	#include <drx3D/Render/inspectable.h>
...
CSmartGlassContext::~CSmartGlassContext()
{
	...
	if (m_pDrxTexture)
	{
		delete m_pDrxTexture->m_pDevTexture;
		m_pDrxTexture->m_pDevTexture = m_pPrevDeviceTexture;
		m_pPrevDeviceTexture = NULL;

		m_pDrxTexture->m_pDeviceRTV = m_pPrevRenderTargetView;
		m_pPrevRenderTargetView = NULL;

		SAFE_RELEASE(m_pDrxTexture);
	}
...
void CSmartGlassContext::InitializeDevice(D3DDevice* pD3DDevice, D3DDeviceContext* pD3DDeviceContext)
{
	...
	pIns->QueryInterface(IID_ISmartGlassDirectSurfaceNative, (uk *)&pDirectSurfaceNative);

	m_pSmartGlassDirectSurfaceNative = pDirectSurfaceNative;
	m_screenHeight = (i32)pDirectSurface->NativeBounds.Height;
	m_screenWidth = (i32)pDirectSurface->NativeBounds.Width;
...
void CSmartGlassContext::CreateSwapChain()
{
	...
	m_pDrxTexture = CTexture::GetOrCreateTextureObject("$SmartGlass", (u32)m_viewport.Width, (u32)m_viewport.Height, 1, eTT_2D, FT_DONT_RELEASE | FT_DONT_STREAM | FT_USAGE_RENDERTARGET, eTF_Unknown, TO_SMARTGLASS);
	bool bRes = m_pDrxTexture->CreateRenderTarget(eTF_R8G8B8A8, Clr_Empty); //
	m_pPrevRenderTargetView = reinterpret_cast<D3DSurface*>(m_pDrxTexture->m_pDeviceRTV);
	m_pPrevDeviceTexture = m_pDrxTexture->m_pDevTexture;
	m_pDrxTexture->m_pDeviceRTV = m_pRenderTargetView;
	m_pDrxTexture->m_pDevTexture = new CDeviceTexture(backBuffer.Get());
...
{
	gcpRendD3D->FX_PushRenderTarget(0, m_pDrxTexture, NULL);
	if (pTexBackbuffer)
	{
...
void CSmartGlassContext::SendInputEvents()
{
	...
	for (InputEventVec::iterator it = events.begin(); it != end; ++it)
	{
		...
		default:
			{
				DrxFatalError("Unexpected SmartGlass input event type.");
			} break;
		}
	}
