...
bool CSunShafts::IsVisible()
{
	...
	if (CRenderer::CV_r_sunshafts && IsActive() && !gRenDev->IsCustomRenderModeEnabled(eRMF_THERMALVISION | eRMF_SONARVISION))
	{
		...
		u32 nCamVisAreaFlags = gRenDev->m_p3DEngineCommon.m_pCamVisAreaInfo.nFlags;
	...
	if (CRenderer::CV_r_sunshafts > 1 && m_bShaftsEnabled)
	{
		...
		bool bSunShaftsVisible = ((i32)m_nVisSampleCount > (CRendererResources::s_ptexBackBuffer->GetWidth() * CRendererResources::s_ptexBackBuffer->GetHeight() / 100));   // || !bReady;
		...
		if (!pOcclQuery->GetDrawFrame() || pOcclQuery->IsReady())
		{
			...
			static CDrxNameTSCRC pTechName("OcclCheckTechnique");
...
/*
	...
	static CDrxNameR pParam1Name("SunShafts_SunPos");
	...
	static CDrxNameR pParam2Name("PI_sunShaftsParams");
	...
	static CDrxNameTSCRC pTech0Name("SunShaftsMaskGen");
	...
	static CDrxNameR pParam3Name("texToTexParams0");
	static CDrxNameR pParam4Name("texToTexParams1");
	...
	static CDrxNameTSCRC pTech1Name("SunShaftsGen");
	...
	static CDrxNameR pParam0Name("SunShafts_ViewProj");
...
/*
	...
	u32 nWidth = CRendererResources::s_ptexBackBufferScaled[1]->GetWidth();
	u32 nHeight = CRendererResources::s_ptexBackBufferScaled[1]->GetHeight();
	...
	static CDrxNameTSCRC pTech2Name("SunShaftsDisplay");
	...
	static CDrxNameR pParam5Name("SunShafts_SunCol");
