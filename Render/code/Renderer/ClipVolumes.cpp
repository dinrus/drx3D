...
void CClipVolumesStage::Init()
{
	...
	for (i32 i = 0; i < MaxDeferredClipVolumes; ++i)
	{
...
#ifdef FEATURE_RENDER_CLIPVOLUME_GEOMETRY_SHADER
	u32k flags = FT_NOMIPS | FT_DONT_STREAM | FT_USAGE_DEPTHSTENCIL;
#else
	u32k flags = FT_NOMIPS | FT_DONT_STREAM | FT_USAGE_RENDERTARGET;
#endif
	DRX_ASSERT(m_pClipVolumeStencilVolumeTex == nullptr);
...
void CClipVolumesStage::GenerateClipVolumeInfo()
{
	...
	// Outdoor
	{
		u32 nFlags = IClipVolume::eClipVolumeConnectedToOutdoor | IClipVolume::eClipVolumeAffectedBySun;
	...
	for (const auto& volume : clipVolumes)
	{
		...
		u32k paramIndex = volume.nStencilRef;
		DRX_ASSERT(paramIndex > STENCIL_VALUE_OUTDOORS && paramIndex < MaxDeferredClipVolumes);
...

	{
		...
		DrxStackAllocWithSizeVector(SClipVolumeInfo, MaxDeferredClipVolumes, clipVolumeInfo, CDeviceBufferUpr::AlignBufferSizeForStreaming);

		for (u32 volumeIndex = 0; volumeIndex < MaxDeferredClipVolumes /* numVolumes */; ++volumeIndex)
...
void CClipVolumesStage::Prepare()
{
	...
	u8k StencilReadWriteMask = 0xFF & ~BIT_STENCIL_RESERVED;

	static CDrxNameTSCRC techStencil = "ClipVolumeStencil";
	static CDrxNameTSCRC techPortalBlend = "ClipVolumeBlendValue";
...
#ifdef FEATURE_RENDER_CLIPVOLUME_GEOMETRY_SHADER
	static CDrxNameTSCRC techStencilVolFog = "ClipVolumeStencilVolFog";
	...
	i32k volFogDepthNum = CVolumetricFogStage::GetVolumeTextureDepthSize();
	const float fVolFogDepthNum = static_cast<float>(volFogDepthNum);
	i32k maxDepthSliceNum = 64; // this must exactly match to ClipVolumeVolFogGS shader's limitation.
#endif

	// Create stencil and blend pass primitives
	for (i32 i = clipVolumes.size() - 1; i >= 0; --i)
	{
		...
		if (volume.m_pRenderMesh && volume.nStencilRef < MaxDeferredClipVolumes)
		{
			...
			DRX_ASSERT(((volume.nStencilRef) & (BIT_STENCIL_RESERVED | BIT_STENCIL_INSIDE_CLIPVOLUME)) == 0);
			i32k stencilRef = ~(volume.nStencilRef) & ~(BIT_STENCIL_RESERVED | BIT_STENCIL_INSIDE_CLIPVOLUME);
			...
			if (pRenderMesh = reinterpret_cast<CRenderMesh*>(volume.m_pRenderMesh.get()))
			{
				...
				if (hVertexStream != ~0u && hIndexStream != ~0u)
				{
					...
					auto& constantUpr = primInit.GetConstantUpr();

					auto constants = constantUpr.BeginTypedConstantUpdate<SPrimitiveConstants>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex);
					...
					if (gcpRendD3D->m_bVolumetricFogEnabled)
					{
						...
						i32 minDepthIndex = static_cast<i32>(fMinDepthIndex);
						...
						if (minDepthIndex < volFogDepthNum)
						{
							...
							auto constants = constantUpr.BeginTypedConstantUpdate<SPrimitiveVolFogConstants>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Geometry | EShaderStage_Vertex);
							constants->transformMatrix = Matrix44(volume.mWorldTM.GetTransposed()) * viewInfo[0].cameraProjMatrix;
							...
							i32 maxDepthIndex = static_cast<i32>(fMaxDepthIndex);
			...
			{
				i32k stencilTestRef = BIT_STENCIL_INSIDE_CLIPVOLUME | volume.nStencilRef;

				CRenderPrimitive& primBlend = m_blendPrimitives[i];
				primBlend.SetTechnique(CShaderMan::s_shDeferredShading, techPortalBlend, CVrProjectionUpr::Instance()->GetRTFlags());
				...
				auto& constantUpr = primBlend.GetConstantUpr();
				auto constants = constantUpr.BeginTypedConstantUpdate<SPrimitiveConstants>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Pixel | EShaderStage_Vertex);
...
void CClipVolumesStage::Execute()
{
	...
	{
		static CDrxNameTSCRC techResolveStencil("ResolveStencil");
...
void CClipVolumesStage::PrepareVolumetricFog()
{
	if (gcpRendD3D->m_bVolumetricFogEnabled)
	{
		...
		i32k scaledWidth = CVolumetricFogStage::GetVolumeTextureSize(renderWidth, CRenderer::CV_r_VolumetricFogTexScale);
		i32k scaledHeight = CVolumetricFogStage::GetVolumeTextureSize(renderHeight, CRenderer::CV_r_VolumetricFogTexScale);
		i32k depth = CVolumetricFogStage::GetVolumeTextureDepthSize();
...
#ifdef FEATURE_RENDER_CLIPVOLUME_GEOMETRY_SHADER
		DRX_ASSERT(m_pClipVolumeStencilVolumeTex);
		    ...
		    || m_depthTargetArrayVolFog.size() != depth)
		{
			...
			{
				for (i32 i = 0; i < m_jitteredDepthPassArray.size(); ++i)
				{
			...
			u32k commonFlags = FT_NOMIPS | FT_DONT_STREAM | FT_DONT_RELEASE;
			...
			if (m_pClipVolumeStencilVolumeTex)
			{
				...
				for (i32 i = 0; i < m_depthTargetArrayVolFog.size(); ++i)
				{
			...
			i32k safeMargin = 2; // workaround: shader isn't activated in initial several frames.
		    ...
		    || m_depthTargetArrayVolFog.size() != depth)
		{
			...
			{
				for (i32 i = 0; i < m_volumetricStencilPassArray.size(); ++i)
				{
			...
			{
				for (i32 i = 0; i < m_resolveVolumetricStencilPassArray.size(); ++i)
				{
...
void CClipVolumesStage::ExecuteVolumetricFog()
{
	...
	if (rd->m_bVolumetricFogEnabled && bFogEnabled)
	{
		...
		if (m_cleared > 0)
		{
			...
			for (i32 i = 0; i < maxDSVCount; ++i)
			{
				// separate DSV
				auto& depthTargetView = m_depthTargetArrayVolFog[i];
				auto& pass = m_jitteredDepthPassArray[i];

				static CDrxNameTSCRC shaderName("StoreJitteringDepthToClipVolumeDepth");
				...
				static CDrxNameR paramDepth("ParamDepth");
		...
		// Resolve stencil
		static CDrxNameTSCRC techResolveStencil("ResolveStencilVolFog");
		for (i32 i = 0; i < maxDSVCount; ++i)
		{
