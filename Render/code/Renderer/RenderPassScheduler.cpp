...
void CRenderPassScheduler::InsertResourceTransitions(CPrimitiveRenderPass* pPass)
{
	...
	u32 curRenderPassId = (i32)m_renderPasses.size() - 1;

	for (u32 i = 0; i < CDeviceRenderPassDesc::MaxRendertargetCount; i++)
	{
		...
		if (pTex)
		{
			...
			if (resState.passID != -1 && resState.state != eState_RenderTargetWrite)
			{
				...
				assert(renderPass.numTransitions < DRX_ARRAY_COUNT(renderPass.transitions));
			}
	...
	for (u32 i = 0; i < pPass->m_compiledPrimitives.size(); i++)
	{
...
void CRenderPassScheduler::Execute()
{
	...
	{
		//for (u32 i = 0; i < pass.numTransitions; i++)
		//{
