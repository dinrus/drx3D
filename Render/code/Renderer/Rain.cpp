...
void CRainStage::Init()
{
	...
	{
		i32k vertexStride = sizeof(SVF_P3F_C4B_T2F);
		i32k totalVertexCount = (2 * (m_slices + 1)) + (2 * (m_slices + 1)) + (2 * (m_slices + 1));

		m_rainVertexBuffer = gRenDev->m_DevBufMan.Create(BBT_VERTEX_BUFFER, BU_STATIC, totalVertexCount * vertexStride);
		DRX_ASSERT(m_rainVertexBuffer != ~0u);
...
void CRainStage::ExecuteDeferredRainGBuffer()
{
	...
	{
		static CDrxNameTSCRC techName("DeferredRainGBuffer");
	...
	static CDrxNameR puddleParamName0("g_RainPuddleParams0");
	...
	static CDrxNameR puddleParamName1("g_RainPuddleParams1");
	...
	// Volume
	static CDrxNameR volumeParamName("g_RainVolumeParams");
	...
	static CDrxNameR colorMulParamName("g_RainColorMultipliers");
	...
	if (shouldApplyOcclusion)
	{
		...
		static CDrxNameR occTransMatParamName("g_RainOcc_TransMat");
		...
		static CDrxNameR windParamName("g_RainOcc_WindOffs");
...
void CRainStage::Execute()
{
	...
	if (shouldApplyOcclusion)
	{
...

		{
			...
			if (pass.IsDirty())
			{
				static CDrxNameTSCRC pSceneRainOccAccTechName("SceneRainOccAccumulate");
			...
			static CDrxNameR pRainOccParamName("sceneRainOccMtx");
	...
	static CDrxNameTSCRC pSceneRainTechName("SceneRain");

	const float fAmount = rainVolParams.fCurrentAmount * CRenderer::CV_r_rainamount;
	i32k nLayers = clamp_tpl(i32(fAmount + 0.0001f), 1, m_maxRainLayers);
	...
	for (i32 nCurLayer = 0; nCurLayer < nLayers; ++nCurLayer)
	{
		...
		{
			auto constants = prim.GetConstantUpr().BeginTypedConstantUpdate<SSceneRainCB>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex | EShaderStage_Pixel);
			...
			prim.GetConstantUpr().EndTypedConstantUpdate(constants);
		}
...
void CRainStage::ExecuteRainOcclusionGen()
{
	...
	// render occluders to rain occlusion texture
	{
		...
		static CDrxNameTSCRC techName("RainOcclusion");
		...
		for (auto& it : arrOccluders)
		{
			...
			if (pRenderMesh)
			{
				...
				if (hVertexStream != ~0u && hIndexStream != ~0u)
				{
					...
					DRX_ASSERT(m_rainOccluderPrimitives[index]);
					...
					// update constant buffer
					{
						auto constants = prim.GetConstantUpr().BeginTypedConstantUpdate<SRainOccluderCB>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex);
