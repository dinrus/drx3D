...
#include <drx3D/Render/DevicePSO_Vulkan.h>
#include <drx3D/Render/DeviceCommandListCommon_Vulkan.h>	
...
static auto lambdaCeaseCallback = [](uk cmd, uint nPoolId)
{
	reinterpret_cast<CDeviceCommandListImpl*>(cmd)->CeaseCommandListEvent(nPoolId);
};

static auto lambdaResumeCallback = [](uk cmd, uint nPoolId)
{
...
void CDeviceObjectFactory::AssignDevice(D3DDevice* pDevice)
{
	...
	m_pVKScheduler->AddQueueEventCallback(m_pCoreCommandList.get(), lambdaCeaseCallback, NDrxVulkan::CCommandScheduler::CMDQUEUE_EVENT_CEASE);
	m_pVKScheduler->AddQueueEventCallback(m_pCoreCommandList.get(), lambdaResumeCallback, NDrxVulkan::CCommandScheduler::CMDQUEUE_EVENT_RESUME);
...
const std::vector<u8>* CDeviceObjectFactory::LookupResourceLayoutEncoding(uint64 layoutHash)
{
...
void CDeviceObjectFactory::RegisterEncodedResourceLayout(uint64 layoutHash, std::vector<u8>&& encodedLayout)
{
	if (m_encodedResourceLayouts.find(layoutHash) != m_encodedResourceLayouts.end())
		DrxFatalError("An encoded resource layout with provided hash is already available!");
...
u32 CDeviceObjectFactory::GetEncodedResourceLayoutSize(const std::vector<u8>& encodedLayout)
{
	u32 encodedResourceLayoutSize = 0;

	if (encodedLayout.size() > 0)
	{
		encodedResourceLayoutSize++;
		u8 numberOfResourceLayout = encodedLayout[0];
...
CDeviceCommandListUPtr CDeviceObjectFactory::AcquireCommandList(EQueueType eQueueType /*= eQueue_Graphics*/)
{
	...
	NDrxVulkan::CCommandListPool& pQueue = m_pVKScheduler->GetCommandListPool(eQueueType);
...
std::vector<CDeviceCommandListUPtr> CDeviceObjectFactory::AcquireCommandLists(u32 listCount, EQueueType eQueueType /*= eQueue_Graphics*/)
{
	// In theory this whole function need to be atomic, instead it voids the core command-list(s),
	// Synchronization between different threads acquiring command-lists is deferred to the higher level.
	NDrxVulkan::CCommandListPool& pQueue = m_pVKScheduler->GetCommandListPool(eQueueType);
	...
	for (u32 n = 0; n < listCount; n += 256U)
	{
		u32k chunkCount = std::min(listCount - n, 256U);
		pQueue.AcquireCommandLists(chunkCount, pCLs);

		for (u32 b = 0; b < chunkCount; ++b)
		{
...
void CDeviceObjectFactory::ForfeitCommandLists(std::vector<CDeviceCommandListUPtr> pCommandLists, EQueueType eQueueType /*= eQueue_Graphics*/)
{
	...
	u32k listCount = pCommandLists.size();
	VK_PTR(CCommandList) pCLs[256];
...
HRESULT CDeviceObjectFactory::SyncFence(DeviceFenceHandle query, bool block, bool flush)
{
	...
	if (handle)
	{
		...
		if (hr != S_OK)
		{
			...
			DRX_ASSERT(!flush || !gRenDev->m_pRT || gRenDev->m_pRT->IsRenderThread());
...
void CDeviceObjectFactory::AllocateNullResources()
{
	...
	if (!m_NullResources[D3D11_RESOURCE_DIMENSION_BUFFER])
	{
		...
		{
			pNullBuffer->CMemoryResource::AddFlags(NDrxVulkan::kResourceFlagNull);
		...
		m_NullBufferViewStructured = new NDrxVulkan::CBufferView(pNullBuffer, 0, 16, VK_FORMAT_UNDEFINED);
		m_NullBufferViewTyped = new NDrxVulkan::CBufferView(pNullBuffer, 0, 16, VK_FORMAT_R32G32B32A32_SFLOAT);
	}
	else
	{
	...
	for (i32 dimension = D3D11_RESOURCE_DIMENSION_TEXTURE1D; dimension <= D3D11_RESOURCE_DIMENSION_TEXTURE3D; ++dimension)
	{
		if (!m_NullResources[dimension])
		{
			...
			{
				pNullImage->CMemoryResource::AddFlags(NDrxVulkan::kResourceFlagNull);
...
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CDeviceObjectFactory::UpdateDeferredUploads");
	AUTO_LOCK_T(DrxCriticalSectionNonRecursive, m_deferredUploadCS);
...
D3DResource* CDeviceObjectFactory::AllocateStagingResource(D3DResource* pForTex, bool bUpload, uk & pMappedAddress)
{
...
static EHeapType ConfigureUsage(VkImageCreateInfo& info, u32 nUsage)
{
...
static EHeapType ConfigureUsage(VkBufferCreateInfo& info, u32 nUsage)
{
...
{
	DRX_ASSERT(!(info.arrayLayers > 1 && info.extent.depth > 1)); // arrays of volumes not supported
...
void CDeviceObjectFactory::SelectStagingLayout(const NDrxVulkan::CImageResource* pImage, u32 subResource, SResourceMemoryMapping& result)
{
	u32k mip = subResource % pImage->GetMipCount();
	u32k slice = subResource / pImage->GetMipCount();
	VK_ASSERT(mip < pImage->GetMipCount() && slice < pImage->GetSliceCount() && "Bad subresource selected");

	u32k width = std::max(pImage->GetWidth() >> mip, 1U);
	u32k height = std::max(pImage->GetHeight() >> mip, 1U);
	u32k depth = std::max(pImage->GetDepth() >> mip, 1U);

	u32 blockWidth = 1, blockHeight = 1, blockBytes;
	...
	u32k widthInBlocks = (width + blockWidth - 1U) / blockWidth;
	u32k heightInBlocks = (height + blockHeight - 1U) / blockHeight;
...
HRESULT CDeviceObjectFactory::Create2DTexture(u32 nWidth, u32 nHeight, u32 nMips, u32 nArraySize, u32 nUsage, const ColorF& cClearValue, D3DFormat Format, LPDEVICETEXTURE* ppDevTexture, const STexturePayload* pTI, i32 nESRAMOffset)
{
	VkImageCreateInfo info;
...
HRESULT CDeviceObjectFactory::CreateCubeTexture(u32 nSize, u32 nMips, u32 nArraySize, u32 nUsage, const ColorF& cClearValue, D3DFormat Format, LPDEVICETEXTURE* ppDevTexture, const STexturePayload* pTI)
{
	VkImageCreateInfo info;
...
HRESULT CDeviceObjectFactory::CreateVolumeTexture(u32 nWidth, u32 nHeight, u32 nDepth, u32 nMips, u32 nUsage, const ColorF& cClearValue, D3DFormat Format, LPDEVICETEXTURE* ppDevTexture, const STexturePayload* pTI)
{
	VkImageCreateInfo info;
...
HRESULT CDeviceObjectFactory::CreateBuffer(buffer_size_t nSize, buffer_size_t elemSize, u32 nUsage, u32 nBindFlags, D3DBuffer** ppBuff, ukk pData)
{
	VkBufferCreateInfo info;
...
HRESULT CDeviceObjectFactory::InvalidateCpuCache(uk buffer_ptr, size_t size, size_t offset)
{
	return S_OK;
}

HRESULT CDeviceObjectFactory::InvalidateGpuCache(D3DBuffer* buffer, uk buffer_ptr, buffer_size_t size, buffer_size_t offset)
{
	return S_OK;
}
#endif

void CDeviceObjectFactory::InvalidateBuffer(D3DBuffer* buffer, uk base_ptr, buffer_size_t offset, buffer_size_t size, u32 id)
{
}

//=============================================================================

u8* CDeviceObjectFactory::Map(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode)
{
	...
	return reinterpret_cast<u8*>(buffer->Map());
}

void CDeviceObjectFactory::Unmap(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode)
{
	VK_ASSERT(mode == D3D11_MAP_WRITE_NO_OVERWRITE || mode == D3D11_MAP_READ); // NOTE: access to resources in use by the gpu MUST be synchronized externally
	buffer->Unmap();
}
...
void CDeviceObjectFactory::UploadContents(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode, ukk pInDataCPU, uk pOutDataGPU, UINT numDataBlocks)
{
	VK_ASSERT(mode == D3D11_MAP_WRITE_NO_OVERWRITE); // only no-overwrite supported currently

	if (!bDirectAccess)
	{
		u8k* pInData = reinterpret_cast<u8k*>(pInDataCPU);
		u8* pOutData = CDeviceObjectFactory::Map(buffer, subresource, offset, 0, mode) + offset;
...
template<const bool bDirectAccess>
void CDeviceObjectFactory::DownloadContents(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode, uk pOutDataCPU, ukk pInDataGPU, UINT numDataBlocks)
{
	...
	u8* pInData = CDeviceObjectFactory::Map(buffer, subresource, offset, size, mode) + offset;
...
template
void CDeviceObjectFactory::UploadContents<true>(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode, ukk pInDataCPU, uk pOutDataGPU, UINT numDataBlocks);
template
void CDeviceObjectFactory::UploadContents<false>(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode, ukk pInDataCPU, uk pOutDataGPU, UINT numDataBlocks);

template
void CDeviceObjectFactory::DownloadContents<true>(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode, uk pOutDataCPU, ukk pInDataGPU, UINT numDataBlocks);
template
void CDeviceObjectFactory::DownloadContents<false>(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode, uk pOutDataCPU, ukk pInDataGPU, UINT numDataBlocks);

// Shader API
static NDrxVulkan::CShader* CreateShader(NDrxVulkan::CDevice* pDevice, ukk pShaderBytecode, size_t bytecodeLength)
{
	auto pShader = new NDrxVulkan::CShader(pDevice);
...
ID3D11VertexShader* CDeviceObjectFactory::CreateVertexShader(ukk pData, size_t bytes)
{
	return CreateShader(GetVKDevice(), pData, bytes);
}

ID3D11PixelShader* CDeviceObjectFactory::CreatePixelShader(ukk pData, size_t bytes)
{
	return CreateShader(GetVKDevice(), pData, bytes);
}

ID3D11GeometryShader* CDeviceObjectFactory::CreateGeometryShader(ukk pData, size_t bytes)
{
	return CreateShader(GetVKDevice(), pData, bytes);
}

ID3D11HullShader* CDeviceObjectFactory::CreateHullShader(ukk pData, size_t bytes)
{
	return CreateShader(GetVKDevice(), pData, bytes);
}

ID3D11DomainShader* CDeviceObjectFactory::CreateDomainShader(ukk pData, size_t bytes)
{
	return CreateShader(GetVKDevice(), pData, bytes);
}

ID3D11ComputeShader* CDeviceObjectFactory::CreateComputeShader(ukk pData, size_t bytes)
{
	return CreateShader(GetVKDevice(), pData, bytes);
}
...
{
	std::unique_ptr<D3DOcclusionQuery> pResult(new NDrxVulkan::COcclusionQuery(GetVKDevice()->GetOcclusionQueries()));
