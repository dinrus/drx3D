...
{
	DRX_ASSERT(m_numTimestamps < kMaxTimestamps);
...
float CDeviceTimestampGroup::GetTimeMS(u32 timestamp0, u32 timestamp1)
{
...
void CDeviceCommandListImpl::RequestTransition(NDrxVulkan::CBufferResource* pResource, VkAccessFlags desiredAccess) const
{
	pResource->TrackAnnouncedAccess(desiredAccess);
...
{
	NDrxVulkan::CCommandList* pCommandListVK = GetVKCommandList();
...
{
	u32 toBind = bindings.validMask;

	while (toBind != 0)
	{
		u32 start = countTrailingZeros32(toBind);
		u32 curBit = (1 << start);
		...
		u32 end = countTrailingZeros32(curBit);
		toBind &= ~u32(curBit - 1);
...
void CDeviceGraphicsCommandInterfaceImpl::PrepareRenderPassForUseImpl(CDeviceRenderPass& renderPass) const
{
	...
	for (u32 i = 0; i < renderPass.m_RenderTargetCount; ++i)
	{
...
void CDeviceGraphicsCommandInterfaceImpl::PrepareInlineConstantBufferForUseImpl(u32 bindSlot, CConstantBuffer* pBuffer, EConstantBufferShaderSlot shaderSlot, EHWShaderClass shaderClass) const
{
...
void CDeviceGraphicsCommandInterfaceImpl::PrepareInlineConstantBufferForUseImpl(u32 bindSlot, CConstantBuffer* pBuffer, EConstantBufferShaderSlot shaderSlot, EShaderStage shaderStages) const
{
	...
	DRX_ASSERT(pActualBuffer->AsDynamicOffsetBuffer() != nullptr); // Needs to be a buffer with dynamic offsets
...
void CDeviceGraphicsCommandInterfaceImpl::BeginRenderPassImpl(const CDeviceRenderPass& renderPass, const D3DRectangle& renderArea)
{
	...
	{
		min(renderPass.m_frameBufferExtent.width,  u32(renderArea.right - renderArea.left)),
		min(renderPass.m_frameBufferExtent.height, u32(renderArea.bottom - renderArea.top))
	};
...
void CDeviceGraphicsCommandInterfaceImpl::SetResourcesImpl(u32 bindSlot, const CDeviceResourceSet* pResources)
{
	...
	DRX_ASSERT(pVkResources->GetVKDescriptorSet() != VK_NULL_HANDLE);
...
void CDeviceGraphicsCommandInterfaceImpl::SetInlineConstantBufferImpl(u32 bindSlot, const CConstantBuffer* pBuffer, EConstantBufferShaderSlot shaderSlot, EShaderStage shaderStages)
{
	...
	NDrxVulkan::CBufferResource* pVkBuffer = pBuffer->GetD3D(&offset, &size);
	...
	DRX_ASSERT(dynamicDescriptorSet != VK_NULL_HANDLE);
...
void CDeviceGraphicsCommandInterfaceImpl::SetVertexBuffersImpl(u32 numStreams, u32 lastStreamSlot, const CDeviceInputStream* vertexStreams)
{
	...
	for (i32 i = 0; i <= lastStreamSlot + 1; ++i)
		buffers[i] = VK_NULL_HANDLE;

	for (u32 i = 0; i < numStreams; i++)
	{
		const CDeviceInputStream& vertexStream = vertexStreams[i];
		i32k curSlot = vertexStream.nSlot;
	...
	for (i32 rangeStart = 0, rangeEnd = 1; rangeStart <= lastStreamSlot; rangeStart = rangeEnd = rangeEnd + 1)
	{
...
{
	DRX_ASSERT(NumRects == 0); // currently not supported on Vulkan

	CImageResource* pImage = pView->GetResource();
	VK_ASSERT(pImage && pImage->GetFlag(NDrxVulkan::kImageFlagColorAttachment) && "The image was not created as a color-target");
...
void CDeviceGraphicsCommandInterfaceImpl::ClearSurfaceImpl(D3DDepthSurface* pView, i32 clearFlags, float depth, u8 stencil, u32 numRects, const D3D11_RECT* pRects)
{
	...
	CImageResource* pImage = pView->GetResource();
	VK_ASSERT(pImage && pImage->GetFlag(NDrxVulkan::kImageFlagDepthAttachment) && "The image was not created as a depth-target");
...
void CDeviceGraphicsCommandInterfaceImpl::BindNullVertexBuffers()
{
	...
	for (i32 i = 0; i < DRX_ARRAY_COUNT(buffers); ++i)
	{
		buffers[i] = nullBuffer;
		offsets[i] = 0;
	}

	vkCmdBindVertexBuffers(GetVKCommandList()->GetVkCommandList(), 0, DRX_ARRAY_COUNT(buffers), buffers, offsets);
}
...
void CDeviceComputeCommandInterfaceImpl::PrepareInlineConstantBufferForUseImpl(u32 bindSlot, CConstantBuffer* pBuffer, EConstantBufferShaderSlot shaderSlots, EShaderStage shaderStages) const
{
	buffer_size_t offset, bytes;
	CBufferResource* const pActualBuffer = pBuffer->GetD3D(&offset, &bytes);
	RequestTransition(pActualBuffer, VK_ACCESS_UNIFORM_READ_BIT);
	DRX_ASSERT(pActualBuffer->AsDynamicOffsetBuffer() != nullptr);
}
...
void CDeviceComputeCommandInterfaceImpl::SetResourcesImpl(u32 bindSlot, const CDeviceResourceSet* pResources)
{
	...
	DRX_ASSERT(descriptorSetHandle != VK_NULL_HANDLE);
...
void CDeviceComputeCommandInterfaceImpl::SetInlineConstantBufferImpl(u32 bindSlot, const CConstantBuffer* pBuffer, EConstantBufferShaderSlot shaderSlot)
{
	...
	buffer_size_t offset, size;
	NDrxVulkan::CBufferResource* pVkBuffer = pBuffer->GetD3D(&offset, &size);
	DRX_ASSERT(pVkBuffer->AsDynamicOffsetBuffer() != nullptr); // Needs to be a buffer with dynamic offsets
...
void CDeviceComputeCommandInterfaceImpl::ClearUAVImpl(D3DUAV* pView, const FLOAT Values[4], UINT NumRects, const D3D11_RECT* pRects)
{
	...
	else
	{
		...
		vkCmdFillBuffer(GetVKCommandList()->GetVkCommandList(), info.buffer, info.offset, info.range, reinterpret_cast<u32k*>(Values)[0]);
		GetVKCommandList()->m_nCommands += CLCOUNT_CLEAR;
	}
}
...
void CDeviceComputeCommandInterfaceImpl::ClearUAVImpl(D3DUAV* pView, const UINT Values[4], UINT NumRects, const D3D11_RECT* pRects)
{
	...
	if (pImage)
	{
		...
		VkClearColorValue clearValue;
		clearValue.u32[0] = Values[0];
		clearValue.u32[1] = Values[1];
		clearValue.u32[2] = Values[2];
		clearValue.u32[3] = Values[3];
...
bool CDeviceCopyCommandInterfaceImpl::FillBuffer(ukk pSrc, CBufferResource* pDst, const SResourceMemoryMapping& mapping)
{
...
CBufferResource* CDeviceCopyCommandInterfaceImpl::UploadBuffer(ukk pSrc, CBufferResource* pDst, const SResourceMemoryMapping& mapping, bool bAllowGpu)
{
	const uint32_t srcBytes = mapping.MemoryLayout.volumeStride;
	const uint32_t dstOffset = mapping.ResourceOffset.Left;
...
void CDeviceCopyCommandInterfaceImpl::UploadImage(ukk pSrc, CImageResource* pDst, const SResourceMemoryMapping& mapping, bool bExt)
{
	...
	u32k srcBytes = mapping.MemoryLayout.volumeStride;
	if (GetDevice()->CreateOrReuseStagingResource(pDst, srcBytes, &pStaging, true) != VK_SUCCESS)
	{
		VK_ASSERT(false && "Upload buffer skipped: Cannot create staging buffer");
		return;
...
void CDeviceCopyCommandInterfaceImpl::DownloadImage(NDrxVulkan::CImageResource* pSrc, NDrxVulkan::CBufferResource* pDst, const SResourceMemoryMapping& mapping, bool bExt)
{
	VK_ASSERT(mapping.MemoryLayout.rowStride % mapping.MemoryLayout.typeStride == 0 && "Texel rows must be aligned to the texel size");
	VK_ASSERT(mapping.MemoryLayout.planeStride % mapping.MemoryLayout.rowStride == 0 && "Texel planes must be aligned to the texel rows");
	VK_ASSERT(mapping.MemoryLayout.volumeStride % mapping.MemoryLayout.planeStride == 0 && "Texel volumes must be aligned to the texel planes");
