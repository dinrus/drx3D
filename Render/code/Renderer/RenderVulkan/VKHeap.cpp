...
#include <drx3D/Render/VKHeap.hpp>
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
uk VKAPI_CALL NDrxVulkan::CHostAllocator::CpuMalloc(uk pUserData, size_t size, size_t alignment, VkSystemAllocationScope scope)
{
	uk const pResult = DrxModuleMemalign(size, alignment);
#if !defined(_RELEASE)
	if (pResult)
	{
		const size_t actualSize = DrxModuleMemSize(pResult, alignment);
		 SPoolStats& stats = static_cast<CHostAllocator*>(pUserData)->AtStats(scope);
		DrxInterlockedAdd(&stats.numAllocations, 1);
		DrxInterlockedAdd(&stats.bytesAllocated, actualSize);
	}
#endif
	return pResult;
}

uk VKAPI_CALL NDrxVulkan::CHostAllocator::CpuRealloc(uk pUserData, uk pOriginal, size_t size, size_t alignment, VkSystemAllocationScope scope)
{
#if !defined(_RELEASE)
	 SPoolStats& stats = static_cast<CHostAllocator*>(pUserData)->AtStats(scope);
	if (pOriginal)
	{
		const size_t previousSize = DrxModuleMemSize(pOriginal, alignment);
		DrxInterlockedAdd(&stats.numDeallocations, 1);
		DrxInterlockedAdd(&stats.bytesDeallocated, previousSize);
	}
#endif
	uk const pResult = DrxModuleReallocAlign(pOriginal, size, alignment);
#if !defined(_RELEASE)
	if (pResult)
	{
		const size_t actualSize = DrxModuleMemSize(pResult, alignment);
...
void VKAPI_CALL NDrxVulkan::CHostAllocator::CpuFree(uk pUserData, uk pMemory)
{
#if !defined(_RELEASE)
	if (pMemory)
	{
		const size_t previousSize = DrxModuleMemSize(pMemory, 1);
...
#endif
	DrxModuleMemalignFree(pMemory);
}

void VKAPI_CALL NDrxVulkan::CHostAllocator::CpuForeignMalloc(uk pUserData, size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope)
{
#if !defined(_RELEASE)
	if (size)
	{
		 SPoolStats& stats = static_cast<CHostAllocator*>(pUserData)->AtStats(type);
		DrxInterlockedAdd(&stats.numAllocations, 1);
		DrxInterlockedAdd(&stats.bytesAllocated, size);
	}
#endif
}

void VKAPI_CALL NDrxVulkan::CHostAllocator::CpuForeignFree(uk pUserData, size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope)
{
#if !defined(_RELEASE)
	if (size)
	{
		 SPoolStats& stats = static_cast<CHostAllocator*>(pUserData)->AtStats(type);
		DrxInterlockedAdd(&stats.numDeallocations, 1);
		DrxInterlockedAdd(&stats.bytesDeallocated, size);
	}
#endif
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void NDrxVulkan::CHeap::Init(VkPhysicalDevice physicalDevice, VkDevice device)
{
...
tukk NDrxVulkan::CHeap::FormatSize(char buffer[10], uint64_t bytes)
{
	tukk szQuantity = "B";
	...
	drx_sprintf(buffer, 10, "%u.%02u%s", bytes, postFix, szQuantity);
	return buffer;
};

void NDrxVulkan::CHeap::LogHeapConfiguration() const
{
	VkDeviceSize totalBytes = 0;
	char buffer[10];
	for (uint32_t i = 0; i < m_heapProperties.memoryHeapCount; ++i)
	{
		DrxLog("Vulkan memory heap %u (%s, %s)", i,
	...
	DrxLog("Vulkan total GPU visible memory: %s", FormatSize(buffer, totalBytes));
}

namespace Detail
{
// Converts a C++ lambda to a C style callback (with an opaque first parameter of uk )
template<typename TFunctor>
struct DeduceCFunctor
{
	template<typename TResult, typename ... TArgs>
	static TResult WrappedCallback(uk pContext, TArgs ... args)
	{
...
// The returned free function pointer has a leading uk parameter to refer to the lambda context.
...
void NDrxVulkan::CHeap::LogHeapSummary() const
{
	struct STypeSummary
	{
		uint64_t committed;
	...
	for (uint32_t heapIndex = 0; heapIndex < m_heapProperties.memoryHeapCount; ++heapIndex)
	{
		...
		DrxLog("Vulkan memory heap %u (%s %s) summary", heapIndex,
			FormatSize(buffer[0], heap.size),
			(heap.flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) ? "GPU" : "CPU");
		DrxLog("| pool# | page | coh ||   commit   | allocated | blocks |");
		for (uint32_t typeIndex = 0; typeIndex < m_heapProperties.memoryTypeCount; ++typeIndex)
		{
			...
			DrxLog("|  %03u  |  %s  |  %c  || %10s | %9s | %6u |", typeIndex,
	...
	DrxLog("Vulkan total GPU committed %s, allocated %s", FormatSize(buffer[0], gpuCommitted), FormatSize(buffer[1], gpuAllocated));
}

NDrxVulkan::CMemoryHandle NDrxVulkan::CHeap::Allocate(const VkMemoryRequirements& requirements, EHeapType heapHint)
{
...
NDrxVulkan::CHeap::TBlockHandle NDrxVulkan::CHeap::AllocateBlock(uint32_t memoryType, uint32_t bytes, uint32_t align)
{
...
void NDrxVulkan::CHeap::DeallocateBlock(uint32_t memoryType, TBlockHandle blockHandle, uint32_t bytes)
{
...
uk NDrxVulkan::CHeap::MapBlock(uint32_t memoryType, TBlockHandle blockHandle)
{
...
void NDrxVulkan::CHeap::UnmapBlock(uint32_t memoryType, TBlockHandle blockHandle, uk pAddress)
{
	(void)pAddress;
