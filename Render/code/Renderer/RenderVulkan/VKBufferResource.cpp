...
//---------------------------------------------------------------------------------------------------------------------
tukk AccessToString(VkAccessFlags accessFlags);
...
{
	DRX_ASSERT(layout != VK_NULL_HANDLE && m_hVkBufferResource);
	...
	DRX_ASSERT(bufferSize >= 256 && bufferSize <= 32768);
#ifndef _RELEASE
	if (!m_bufferSizeLog2) { m_bufferSizeLog2 =  u32(IntegerLog2(NextPower2(bufferSize))); }
	DRX_ASSERT(              m_bufferSizeLog2 == u32(IntegerLog2(NextPower2(bufferSize))));
#endif // !_RELEASE
