...
CDeviceGraphicsPSO::EInitResult CDeviceGraphicsPSO_Vulkan::Init(const CDeviceGraphicsPSODesc& psoDesc)
{
	...
	for (EHWShaderClass shaderClass = eHWSC_Vertex; shaderClass < eHWSC_NumGfx; shaderClass = EHWShaderClass(shaderClass + 1))
	{
		...
		case eHWSC_Hull:
			...
			break;
		default:
			DRX_ASSERT_MESSAGE(0, "Shader class not supported");
			return EInitResult::Failure;
		}
		...
		shaderStageCreateInfos[validShaderCount].module = reinterpret_cast<NDrxVulkan::CShader*>(hwShaders[shaderClass].pDeviceShader)->GetVulkanShader();
	...
	if (psoDesc.m_VertexFormat != EDefaultInputLayouts::Empty)
	{
		...
		if (CHWShader_D3D::SHWSInstance* pVsInstance = reinterpret_cast<CHWShader_D3D::SHWSInstance*>(hwShaders[eHWSC_Vertex].pHwShaderInstance))
		{
			...
			u32k declarationCount = inputLayout.m_Declaration.size();
				...
				for (const auto& vsInputElement : pVsInstance->m_VSInputStreams)
				{
						...
						vsInputElement.semanticIndex == declInputElement.SemanticIndex)
					{
						...
						for (; itBinding != vertexInputBindingDescriptions.end(); ++itBinding)
						{
							...
							{
								DRX_ASSERT_MESSAGE(itBinding->inputRate == (declInputElement.InputSlotClass == D3D11_INPUT_PER_VERTEX_DATA ? VK_VERTEX_INPUT_RATE_VERTEX : VK_VERTEX_INPUT_RATE_INSTANCE), "Mismatching input rate not supported");
								break;
							}
						}
						...
						{
							DRX_ASSERT_MESSAGE(declInputElement.InputSlotClass == D3D11_INPUT_PER_VERTEX_DATA || declInputElement.InstanceDataStepRate == 1, "Data step rate not supported");
	...
	// primitive topology
	{
		...
		for (i32 i = 0; i < DRX_ARRAY_COUNT(topologyTypes); ++i)
		{
		...
		{
			DRX_ASSERT_MESSAGE(0, "Primitive type not supported");
			return EInitResult::Failure;
		}
	}
	...
	i32 validBlendAttachmentStateCount = 0;
...
bool CDeviceComputePSO_Vulkan::Init(const CDeviceComputePSODesc& psoDesc)
{
	...
	computePipelineCreateInfo.stage.module = reinterpret_cast<NDrxVulkan::CShader*>(hwShaders[eHWSC_Compute].pDeviceShader)->GetVulkanShader();
	if(CRendererCVars::CV_r_VkShaderCompiler && strcmp(CRendererCVars::CV_r_VkShaderCompiler->GetString(), STR_VK_SHADER_COMPILER_HLSLCC) == 0)
