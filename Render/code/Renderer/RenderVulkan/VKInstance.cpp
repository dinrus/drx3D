...
bool CreateSDLWindowContext(SDL_Window*& kWindowContext, tukk szTitle, u32 uWidth, u32 uHeight, bool bFullScreen)
{
	u32 uWindowFlags = 0;
	if (bFullScreen)
	{
...
bool CInstance::CreateSDLWindow(tukk szTitle, u32 uWidth, u32 uHeight, bool bFullScreen, HWND* pHandle)
{
...
bool CInstance::Initialize(tukk appName, uint32_t appVersion, tukk engineName, uint32_t engineVersion)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_ERROR, "Vulkan instance initialization failed! Please make sure your GPU supports Vulkan.");
		return false;
	}
...
VkResult CInstance::InitializeInstanceExtensions(tukk layerName, std::vector<VkExtensionProperties>& extensions)
{
...
VkResult CInstance::InitializeInstance(tukk appName, uint32_t appVersion, tukk engineName, uint32_t engineVersion)
{
	...
	if (res == VK_ERROR_LAYER_NOT_PRESENT)
	{
		...
		if (res == VK_SUCCESS)
		{
			...
			DrxLogAlways("vkCreateInstance: Discarded %" PRISIZE_T " layers during Vulkan initialization", m_enabledInstanceLayers.size());
		}
	}
...
VkResult CInstance::InitializePhysicalDeviceExtensions(VkPhysicalDevice& device, tukk layerName, std::vector<VkExtensionProperties>& extensions)
{
	VkExtensionProperties* extensionsList;
	uint32_t extensionCount;
	VkResult res;
...
VKAPI_ATTR VkBool32 VKAPI_CALL CInstance::DebugLayerCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objType, uint64_t obj,
	size_t location, int32_t code, tukk layerPrefix, tukk msg, uk userData) 
{
	if (flags & (VK_DEBUG_REPORT_WARNING_BIT_EXT | VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT | VK_DEBUG_REPORT_ERROR_BIT_EXT))
	{
		DrxLog("[Vulkan DebugLayer, %s, %d]: %s", layerPrefix, code, msg);
	}
	return VK_FALSE;
}
...
VkResult CInstance::InitializeDebugLayerCallback()
{
	if (CRendererCVars::CV_r_EnableDebugLayer)
	{
		...
		DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_WARNING, "Failed to initialize debug layer report callback");
