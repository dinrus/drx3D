...
bool CCommandList::Init(UINT64 currentFenceValue)
{
	...
	u32 nCmdListFamily = m_rPool.GetVkQueueFamily();
...
void CCommandList::ResourceBarrier(UINT NumBarriers, const VkMemoryBarrier* pBarriers)
{
	if (VK_BARRIER_FUSION && CRenderer::CV_r_VkBatchResourceBarriers)
	{
		...
		{
			DRX_ASSERT(m_CurrentNumPendingMBarriers + NumBarriers < 256);
...
void CCommandList::ResourceBarrier(UINT NumBarriers, const VkBufferMemoryBarrier* pBarriers)
{
	if (VK_BARRIER_FUSION && CRenderer::CV_r_VkBatchResourceBarriers)
	{
		...
		if (CRenderer::CV_r_VkBatchResourceBarriers > 1)
		{
			DRX_ASSERT(m_CurrentNumPendingBBarriers + NumBarriers < 256);
			for (UINT i = 0; i < NumBarriers; ++i)
			{
...
void CCommandList::ResourceBarrier(UINT NumBarriers, const VkImageMemoryBarrier* pBarriers)
{
	if (VK_BARRIER_FUSION && CRenderer::CV_r_VkBatchResourceBarriers)
	{
		...
		if (CRenderer::CV_r_VkBatchResourceBarriers > 1)
		{
			DRX_ASSERT(m_CurrentNumPendingIBarriers + NumBarriers < 256);
			for (UINT i = 0; i < NumBarriers; ++i)
			{
				UINT j = 0;
...
std::pair<u32, u32> CCommandListPool::m_MapQueueBits[1U << 4] =
{
	std::make_pair(0, 0)
};

u32 CCommandListPool::m_MapQueueIndices[32] =
{
	0
};
...
void CCommandListPool::Configure()
{
	...
	{
		for (i32 f = 0, n = pInfo->queueFamilyProperties.size(); f < n; ++f)
		{
			i32 queueFlags = pInfo->queueFamilyProperties[f].queueFlags;
			i32 queueCount = pInfo->queueFamilyProperties[f].queueCount;
		...
		for (i32 i = 0x0; i <= 0x3; ++i)
		for (i32 f = 0x0; f <= 0xF; ++f)
		for (i32 n = 0x0; n <= 0x3; ++n)
		{
			i32 queueFlags = f & ~(1U << n);
...
void CCommandListPool::AcquireCommandLists(u32 numCLs, VK_PTR(CCommandList)* results) threadsafe
{
	static DrxCriticalSectionNonRecursive csThreadSafeScope;
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(csThreadSafeScope);
...
void CCommandListPool::ForfeitCommandLists(u32 numCLs, VK_PTR(CCommandList)* results, bool bWait) threadsafe
{
	static DrxCriticalSectionNonRecursive csThreadSafeScope;
	DrxAutoLock<DrxCriticalSectionNonRecursive> lThreadSafeScope(csThreadSafeScope);
