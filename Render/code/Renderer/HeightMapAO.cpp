...
void CHeightMapAOStage::Execute()
{
	...
	DRX_ASSERT(!m_bHeightMapAOExecuted);
	...
	if (m_pHeightMapFrustum)
	{
		...
		i32k resolutionIndex = clamp_tpl(CRenderer::CV_r_HeightMapAO - 1, 0, 2);
		...
		// Generate occlusion
		{
			...
			{
				static CDrxNameTSCRC techSampling("HeightMapAOPass");
			...
			static CDrxNameR nameParams("HMAO_Params");
			static CDrxNameR nameTexToWorldT("HMAO_TexToWorldTranslation");
			static CDrxNameR nameTexToWorldS("HMAO_TexToWorldScale");
			static CDrxNameR nameTransform("HMAO_Transform");
		...
		// Depth aware blur
		{
			...
			u32 clipVolumeCount = RenderView()->GetClipVolumes().size();
			...
			if (m_passSmoothing.IsDirty(resolutionIndex, clipVolumeCount > 0 ? 1 : 0))
			{
				...
				static CDrxNameTSCRC techSmoothing("HMAO_Blur");
			...
			static CDrxNameR namePixelOffset("PixelOffset");
			static CDrxNameR nameClipVolumeData("HMAO_ClipVolumeData");
