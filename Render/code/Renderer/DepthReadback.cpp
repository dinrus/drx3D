...
void CDepthReadbackStage::Init()
{
	...
	{
		DrxFatalError("Cannot create depth readback resources");
	}
...
void CDepthReadbackStage::Execute()
{
	...
	u32k sourceWidth = pSource->GetWidth();
	u32k sourceHeight = pSource->GetHeight();
	i32k sourceTexId = pSource->GetID();
	...
	if (bNeedRecreateResources)
	{
		...
		{
			DRX_ASSERT(false && "Unable to create resources, this should only be possible in response to CVar changes (ie, developer mode)");
			return;
		}
	}
...
CTexture* CDepthReadbackStage::GetInputTexture(EConfigurationFlags& flags)
{
	...
	const bool bMultiResEnabled = CVrProjectionUpr::IsMultiResEnabledStatic();
	...
	i32 allFlags = bMultiResEnabled ? kMultiRes : kNone;
	...
	return
	  ...
	  : bMultiResEnabled ? CVrProjectionUpr::Instance()->GetZTargetFlattened()
	  : CRendererResources::s_ptexLinearDepth;
}

bool CDepthReadbackStage::CreateResources(u32 sourceWidth, u32 sourceHeight)
{
	DRX_ASSERT(sourceWidth != 0 && sourceHeight != 0 && "Bad input size requested");
	bool bFailed = false;

	// Find number of downsample passes needed for source resolution
	u32k downSampleX = sourceWidth > CULL_SIZEX ? IntegerLog2((sourceWidth - 1) / CULL_SIZEX) : 0;
	u32k downSampleY = sourceHeight > CULL_SIZEY ? IntegerLog2((sourceHeight - 1) / CULL_SIZEY) : 0;
	u32k maxDownsamplePasses = kMaxDownsamplePasses;
	...
	for (u32 i = 0; i < m_downsamplePassCount && !bFailed; ++i)
	{
		u32k downsampleIndex = i + (kMaxDownsamplePasses - m_downsamplePassCount);
		CTexture* const pTarget = CRendererResources::s_ptexLinearDepthDownSample[downsampleIndex];
		DRX_ASSERT(pTarget && "Z Downsample target should already exist");

		// Ensure the intermediate textures are never larger than the source image
		u32k width = CULL_SIZEX << std::min(m_downsamplePassCount - i, downSampleX);
		u32k height = CULL_SIZEY << std::min(m_downsamplePassCount - i, downSampleY);

		if (!pTarget->GetDevTexture() || pTarget->GetWidth() != width || pTarget->GetHeight() != height)
		{
			u32k downsampleFlags = FT_DONT_STREAM | FT_DONT_RELEASE;
	...
	for (u32 i = m_downsamplePassCount; i < kMaxDownsamplePasses; ++i)
	{
		u32k downsampleIndex = i - m_downsamplePassCount;
		CTexture* const pTarget = CRendererResources::s_ptexLinearDepthDownSample[downsampleIndex];
		DRX_ASSERT(pTarget && "Z Downsample target should already exist");
	...
	for (u32 i = 0; i < kMaxReadbackPasses && !bFailed; ++i)
	{
		u32k readbackIndex = i;
		CTexture* const pTarget = CRendererResources::s_ptexLinearDepthReadBack[readbackIndex];
		DRX_ASSERT(pTarget && "Z Readback target should already exist");
		...
		{
			u32k readbackFlags = FT_DONT_STREAM | FT_DONT_RELEASE | FT_STAGE_READBACK;
...
{
	static const CDrxNameTSCRC techniqueNameSimple("TextureToTextureResampled");
	static const CDrxNameTSCRC techniqueNameRegion("TextureToTextureResampledReg");
	...
	for (u32 i = kMaxDownsamplePasses - m_downsamplePassCount; i < kMaxDownsamplePasses; ++i)
	{
	...
	for (u32 i = 0; i < kMaxReadbackPasses; ++i)
	{
...
{
	static const CDrxNameR psParam0Name("texToTexParams0");
	static const CDrxNameR psParam1Name("texToTexParams1");
	static const CDrxNameR vsParamName("texToTexParamsTC");
	...
	// Issue readback
	u32k readbackIndex = m_readbackIndex % kMaxReadbackPasses;
	++m_readbackIndex;
	CTexture* const pTarget = CRendererResources::s_ptexLinearDepthReadBack[readbackIndex];
...
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CDepthReadbackStage::ReadbackLatestData");

	// Determine the last completed readback.
	u32k oldestReadback = m_readbackIndex % kMaxReadbackPasses;
	u32 receivableIndex = kMaxReadbackPasses;
	for (u32 i = 0; i < kMaxReadbackPasses; ++i)
	{
		u32k readbackIndex = (oldestReadback - i - 1) % kMaxReadbackPasses;
	...
	u32k reservedIndex = m_lastPinned < kMaxResults ? 1 - m_lastPinned : m_lastResult < kMaxResults ? m_lastResult : 0;
	DRX_ASSERT(reservedIndex < kMaxResults && reservedIndex != m_lastPinned && (kMaxResults == 3 ? reservedIndex != m_lastResult : true));
	...
	DRX_ASSERT(readback.bIssued == true);

	const auto readbackData = [&readback, &result](uk pData, u32 rowPitch, u32 slicePitch) -> bool
	{
		DRX_ASSERT(rowPitch == CULL_SIZEX * sizeof(float) && slicePitch >= CULL_SIZEX * CULL_SIZEY * sizeof(float));

		const float* const pDepths = static_cast<const float*>(pData);
		if (readback.flags & kNativeDepth)
		{
			for (u32 i = 0; i < CULL_SIZEX * CULL_SIZEY; ++i)
			{
