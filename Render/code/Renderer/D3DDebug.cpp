...
bool CD3DDebug::Init(ID3D11Device* pD3DDevice)
{
	...
	if (FAILED(pD3DDevice->QueryInterface(__uuidof(ID3D11InfoQueue), (uk *)&m_pd3dDebugQueue)))
		return false;
...
void CD3DDebug::Update(ESeverityCombination muteSeverity, tukk strMuteMsgList, tukk strBreakOnMsgList)
{
	if (!m_pd3dDebugQueue)
		return;
	...
	i32k nVal = atoi(strBreakOnMsgList);
	if (nVal == -1)
	{
		// Break on all errors
...
UINT CD3DDebug::ParseIDs(tukk strMsgIDList, D3D11_MESSAGE_ID arrMsgList[MAX_NUM_DEBUG_MSG_IDS]) const
{
	const size_t nLen = strlen(strMsgIDList);
	UINT nIDs = 0;
	tukk pStart = strMsgIDList;
	tuk pEnd;
	UINT nValID = 1;
