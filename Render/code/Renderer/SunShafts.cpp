...
i32 CSunShaftsStage::GetDownscaledTargetsIndex()
{
...
void CSunShaftsStage::Execute()
{
	...
	i32k downscaledSourceIndex = GetDownscaledTargetsIndex();

	// Generate mask for sun shafts
	{
		if (m_passShaftsMask.IsDirty())
		{
			static CDrxNameTSCRC techMaskGen("SunShaftsMaskGen");
	...
	// Apply local radial blur to mask
	{
		...
		i32 viewInfoCount = GetGraphicsPipeline().GenerateViewInfo(viewInfo);
		...
		// Pass 1
		{
			...
			{
				static CDrxNameTSCRC techShaftsGen("SunShaftsGen");
				uint64 rtMask = g_HWSR_MaskBit[HWSR_SAMPLE0];
