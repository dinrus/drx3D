...
void CBloomStage::Execute()
{
	...
	static CDrxNameTSCRC techBloomGaussian("HDRBloomGaussian");
	static CDrxNameR hdrParams0Name("HDRParams0");

	i32 width = CRendererResources::s_ptexHDRFinalBloom->GetWidth();
	i32 height = CRendererResources::s_ptexHDRFinalBloom->GetHeight();

	// Note: Just scaling the sampling offsets depending on the resolution is not very accurate but works acceptably
	i32 widthHalfRes = (CRendererResources::s_ptexHDRTarget->GetWidth() + 1) / 2;
	i32 widthQuarterRes = (widthHalfRes + 1) / 2;
