...
#include <DinrusXNetwork/INetwork.h>
...
IStatoscopeDataGroup::SDescription CDetailedRenderTimesDG::GetDescription() const
{
	return IStatoscopeDataGroup::SDescription('I', "Detailed Rendering Timers",
		"['/RenderTimes/$' (float GPUTime) (float CPUTime) (i32 DIPs) (i32 Polygons)]");
}
...
void CDetailedRenderTimesDG::Write(IStatoscopeFrameRecord& fr)
{
	...
	i32 curRecLevel = 1;
	char buf[1024];
	std::stack<i32> recLevelStack;
	recLevelStack.push(0);
	i32 disambigCounter = 0;
	for (auto it = m_stats->begin(); it != m_stats->end(); ++it)
	{
		...
		{
			drx_sprintf(&buf[recLevelStack.top()], 1024 - recLevelStack.top(), "%s%d/", st.name, ++disambigCounter);
		}
		else
		{
			disambigCounter = 0;
			drx_sprintf(&buf[recLevelStack.top()], 1024-recLevelStack.top(), "%s/", st.name);
		}
...
u32 CDetailedRenderTimesDG::PrepareToWrite()
{
...
void CGraphicsDG::ResetGPUUsageHistory()
{
	...
	for (u32 i = 0; i < GPU_HISTORY_LENGTH; i++)
	{
...
void CGraphicsDG::TrackGPUUsage(float gpuLoad, float frameTimeMs, i32 totalDPs)
{
	...
	i32 currentFrame = gEnv->pRenderer->GetFrameID(false);
...
IStatoscopeDataGroup::SDescription CGraphicsDG::GetDescription() const
{
	return IStatoscopeDataGroup::SDescription('g', "graphics",
	                                          "['/Graphics/' (float GPUUsageInPercent) (float GPUFrameLengthInMS) (i32 numTris) (i32 numDrawCalls) "
	                                          "(i32 numShadowDrawCalls) (i32 numGeneralDrawCalls) (i32 numTransparentDrawCalls) (i32 numTotalDrawCalls) "
	                                          "(i32 numPostEffects) (i32 numForwardLights) (i32 numForwardShadowCastingLights)]");
}
...
void CGraphicsDG::Write(IStatoscopeFrameRecord& fr)
{
	...
	i32 numTris, numShadowVolPolys;
	m_pRenderer->GetPolyCount(numTris, numShadowVolPolys);
	fr.AddValue(numTris);

	i32 numDrawCalls, numShadowDrawCalls, numGeneralDrawCalls, numTransparentDrawCalls;
	...
	i32 nDynamicLights = (i32)pLights->Count();
	i32 nShadowCastingLights = 0;
	...
	for (i32 i = 0; i < nDynamicLights; i++)
	{
...
IStatoscopeDataGroup::SDescription CPerformanceOverviewDG::GetDescription() const
{
	return IStatoscopeDataGroup::SDescription('O', "frame performance overview",
	                                          ...
	                                          "(i32 numDrawCalls) (float FrameScaleFactor)]");
}
...
void CPerformanceOverviewDG::Write(IStatoscopeFrameRecord& fr)
{
	...
	i32 numDrawCalls, numShadowDrawCalls;
