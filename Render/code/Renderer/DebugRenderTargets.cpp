...
tukk CDebugRenderTargetsStage::showRenderTargetHelp =
...
void CDebugRenderTargetsStage::Execute()
{
	if (CRendererCVars::CV_r_DeferredShadingDebugGBuffer >= 1 && CRendererCVars::CV_r_DeferredShadingDebugGBuffer <= 9)
	{
		...
		// *INDENT-OFF*
		struct { tukk name; tukk desc; } helpText[9] =
		{
			{ "Normals",                        "" },
			{ "Smoothness",                     "" },
			{ "Reflectance",                    "" },
...
void CDebugRenderTargetsStage::ExecuteShowTargets()
{
	if (!m_renderTargetList.empty())
	{
		...
		i32k tileCount = min(i32(m_renderTargetList.size()), m_columnCount * m_columnCount);

		for (i32 i = m_debugPrimitives.size(); i < tileCount; ++i)
		{
		...
		for (size_t i = 0; i < tileCount; ++i)
		{
			...
			if (rtInfo.mip != -1 || rtInfo.slice != -1)
			{
				...
				i32 firstSlice = clamp_tpl(rtInfo.slice, 0, texLayout.m_nArraySize - 1);
				i32 sliceCount = clamp_tpl(rtInfo.slice < 0 ? texLayout.m_nArraySize : 1, 1, i32(texLayout.m_nArraySize));

				i32 firstMip = clamp_tpl(rtInfo.mip, 0, texLayout.m_nMips - 1);
				i32 mipCount = clamp_tpl(rtInfo.mip < 0 ? texLayout.m_nMips : 1, 1, i32(texLayout.m_nMips));
			...
			if (prim.Compile(m_debugPass) == CRenderPrimitive::eDirty_None)
			{
				...
				auto constants = prim.GetConstantUpr().BeginTypedConstantUpdate<SDebugRenderTargetConstants>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex | EShaderStage_Pixel);
				...
				IRenderAuxText::WriteXY((i32)(curX * 800 + 2), (i32)((curY + tileH) * 600 - 15), 1, 1, 1, 1, 1, 1, "Fmt: %s, Type: %s", rtInfo.pTexture->GetFormatName(), CTexture::NameForTextureType(rtInfo.pTexture->GetTextureType()));
				IRenderAuxText::WriteXY((i32)(curX * 800 + 2), (i32)((curY + tileH) * 600 + 1),  1, 1, 1, 1, 1, 1, "%s   %d x %d",      rtInfo.pTexture->GetName(),       rtInfo.pTexture->GetWidth(), rtInfo.pTexture->GetHeight());
			}
		}
