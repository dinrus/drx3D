...
void CMotionBlurStage::Execute()
{
	...
	i32 vpX = viewport.x, vpY=viewport.y, vpWidth=viewport.width, vpHeight=viewport.height;
	...
	static CDrxNameR motionBlurParamName("vMotionBlurParams");

	{
		...
		{
			static CDrxNameTSCRC techPackVelocities("PackVelocities");
		...
		static CDrxNameR viewProjPrevName("mViewProjPrev");
		static CDrxNameR dirBlurName("vDirectionalBlur");
		static CDrxNameR radBlurName("vRadBlurParam");
...

	{
		...
		if (m_passMotionBlur.IsDirty(CRenderer::CV_r_MotionBlurQuality, bGatherDofEnabled))
		{
			...
			static CDrxNameTSCRC techMotionBlur("MotionBlur");
