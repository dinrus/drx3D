...
bool CWaterStage::UpdateCausticsGrid(N3DEngineCommon::SCausticInfo& causticInfo, bool& bVertexUpdated, CRenderer* pRenderer)
{
	...
	u32k nCausticMeshWidth = clamp_tpl(CRenderer::CV_r_watervolumecausticsdensity, 16, 255);
	u32k nCausticMeshHeight = clamp_tpl(CRenderer::CV_r_watervolumecausticsdensity, 16, 255);
	...
	if ((!causticInfo.m_pCausticQuadMesh || causticInfo.m_nCausticMeshWidth != nCausticMeshWidth || causticInfo.m_nCausticMeshHeight != nCausticMeshHeight))
	{
		...
		u32k nCausticVertexCount = (nCausticMeshWidth + 1) * (nCausticMeshHeight + 1);
		u32k nCausticIndexCount = (nCausticMeshWidth * nCausticMeshHeight) * 6;
		...
		// Fill vertex buffer.
		for (u32 y = 0; y <= nCausticMeshHeight; ++y)
		{
			for (u32 x = 0; x <= nCausticMeshWidth; ++x)
			{
...
void CWaterStage::Init()
{
	...
	//DRX_ASSERT(CTexture::IsTextureExist(CRendererResources::s_ptexWaterCaustics[0])); // this can fail because CRendererResources::s_ptexWaterCaustics[0] is null when launching the engine with r_watervolumecaustics=0.
	DRX_ASSERT(CTexture::IsTextureExist(CRendererResources::s_ptexWaterVolumeRefl[0]));
	...
	DRX_ASSERT(pDummyRenderTarget && pDummyRenderTarget->GetTextureDstFormat() == eTF_R8G8B8A8);
...
void CWaterStage::Resize(i32 renderWidth, i32 renderHeight)
{
	u32k flags = FT_NOMIPS | FT_DONT_STREAM | FT_USAGE_RENDERTARGET; 
...
void CWaterStage::ExecuteWaterVolumeCaustics()
{
	...
	u32k nBatchMask = pRenderView->GetBatchFlags(EFSLIST_WATER);
...
void CWaterStage::ExecuteDeferredWaterVolumeCaustics()
{
	...
	if (pass.IsDirty())
	{
		static CDrxNameTSCRC techName = "WaterVolumeCaustics";
	...
	static CDrxNameR paramLightView("mLightView");
...
void CWaterStage::ExecuteDeferredOceanCaustics()
{
	...
	DRX_ASSERT(viewInfoCount > 0);

	static u8k stencilReadWriteMask = 0xFF & ~BIT_STENCIL_RESERVED;
	u8 stencilRef = 0xFF;
	...
	if (CRenderer::CV_r_watercausticsdeferred == 2)
	{
		...
		static CDrxNameTSCRC techName("DeferredOceanCausticsStencil");
		...
		DRX_ASSERT(rd->m_nStencilMaskRef > 0 && rd->m_nStencilMaskRef <= STENC_MAX_REF);
		...
		static i32k stencilStateBackFace = STENC_FUNC(FSS_STENCFUNC_GEQUAL) |
		...
		{
			auto& constantUpr = backFacePrim.GetConstantUpr();

			auto constants = constantUpr.BeginTypedConstantUpdate<water::SPrimitiveConstants>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex);
		...
		static i32k stencilStateFrontFace = STENC_FUNC(FSS_STENCFUNC_GEQUAL) |
		                                           STENCOP_FAIL(FSS_STENCOP_KEEP) |
	...
	// Deferred ocean caustic pass
	{
		...
		{
			static CDrxNameTSCRC techName = "General";
			pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
			pass.SetTechnique(CShaderMan::s_ShaderDeferredCaustics, techName, 0);
		...
		static i32k stencilStateTest = STENC_FUNC(FSS_STENCFUNC_EQUAL) |
		                                      STENCOP_FAIL(FSS_STENCOP_KEEP) |
		...
		static CDrxNameR nameCausticParams("vCausticParams");
		...
		static CDrxNameR nameAnimParams("vAnimParams");
		...
		static CDrxNameR nameLightView("mLightView");
...
void CWaterStage::Execute()
{
	...
	DRX_ASSERT(pRenderView->GetShaderRenderingFlags() & (SHDF_ALLOW_WATER | SHDF_ALLOWPOSTPROCESS));
...
bool CWaterStage::CreatePipelineStates(u32 passMask, DevicePipelineStatesArray& stateArray, const SGraphicsPipelineStateDescription& stateDesc, CGraphicsPipelineStateLocalCache* pStateCache)
{
...
bool CWaterStage::PrepareDefaultPerInstanceResources()
{
	...
	// default textures for water volume
	{
		...
		if (!pRenderer->m_bPauseTimer)
		{
			...
			DRX_ASSERT(elapsedTime >= 0.0f);
			const float AnimTexFlipTime = 0.05f;
			m_rainRippleTexIndex = (u32)(elapsedTime / AnimTexFlipTime) % m_pRainRippleTex.size();
		}

		DRX_ASSERT(m_rainRippleTexIndex < m_pRainRippleTex.size());
...
bool CWaterStage::SetAndBuildPerPassResources(bool bOnInit, EPass passId)
{
	...
	CD3D9Renderer* RESTRICT_POINTER pRenderer = gcpRendD3D;
	i32k frameID = (pRenderView) ? pRenderView->GetFrameId() : gRenDev->GetRenderFrameID();
	...
	i32k currWaterVolID = GetCurrentFrameID(frameID);
	i32k prevWaterVolID = GetPreviousFrameID(frameID);
	...
	// Tiled shading resources
	{
		...
		if (bOnInit)
		{
			resources.SetBuffer(17, CDeviceBufferUpr::GetNullBufferTyped(), EDefaultResourceViews::Default, EShaderStage_AllWithoutCompute);
			resources.SetBuffer(18, CDeviceBufferUpr::GetNullBufferStructured(), EDefaultResourceViews::Default, EShaderStage_AllWithoutCompute);
		//	resources.SetBuffer(19, CDeviceBufferUpr::GetNullBufferStructured(), EDefaultResourceViews::Default, EShaderStage_AllWithoutCompute);
	...
	DRX_ASSERT(!resources.HasChangedBindPoints()); // Cannot change resource layout after init. It is baked into the shaders
...
void CWaterStage::UpdatePerPassResources(EPass passId)
{
	...
	// update per pass constant buffer.
	{
		...
		DRX_ASSERT(pWaterRipplesStage);
		DRX_ASSERT(pVolFogStage);

		DrxStackAllocWithSize(water::SPerPassConstantBuffer, cb, CDeviceBufferUpr::AlignBufferSizeForStreaming);
		...
		DRX_ASSERT(m_pPerPassCB[passId]);
...
void CWaterStage::ExecuteWaterNormalGen()
{
	...
	DRX_ASSERT(RenderView()->GetCurrentEye() == CCamera::eEye_Left);

	i32k nGridSize = 64;

	i32k frameID = SPostEffectsUtils::m_iFrameCounter % 2;
	...
	// spawn water simulation job for next frame.
	{
		...
		if (m_frameIdWaterSim != nCurFrameID)
		{
			...
			if (CTexture::IsTextureExist(pTexture))
			{
				...
				uk pRawPtr = nullptr;
				WaterSimMgr()->Update((i32)nCurFrameID, fUpdateTime, true, pRawPtr);

				Vec4* pDispGrid = WaterSimMgr()->GetDisplaceGrid();

				u32k pitch = 4 * sizeof(f32) * nGridSize;
				u32k width = nGridSize;
				u32k height = nGridSize;

				DRX_PROFILE_REGION_WAITING(PROFILE_RENDERER, "CWaterStage: update subresource");

				CDeviceTexture * pDevTex = pTexture->GetDevTexture();
				pDevTex->UploadFromStagingResource(0, [=](uk pData, u32 rowPitch, u32 slicePitch)
				{
					drxMemcpy(pData, pDispGrid, 4 * width * height * sizeof(f32));
					return true;
				});
			}
	...
	// generate water normal from the simulation result.
	{
		...
		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName("WaterVolumesNormalGen");
			pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
			pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
		...
		static CDrxNameR pParamName("waterVolumesParams");
...
void CWaterStage::ExecuteWaterVolumeCausticsGen(N3DEngineCommon::SCausticInfo& causticInfo)
{
	...
	DRX_ASSERT(pRenderView);
	DRX_ASSERT(pRenderView->GetCurrentEye() == CCamera::eEye_Left);
	...
	// Dilate caustics gen texture.
	{
		...
		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName("WaterCausticsInfoDilate");
	...
	{
		{
			...
			if (bVertexUpdated || prim.IsDirty())
			{
				...
				if (hVertexStream != ~0u && hIndexStream != ~0u)
				{
					...
					static CDrxNameTSCRC techName("WaterCausticsGen");
...
void CWaterStage::ExecuteReflection()
{
	...
	const bool bReverseDepth = true;
	i32k frameID = pRenderView->GetFrameId();

	const auto renderList = EFSLIST_WATER;
	u32k batchMask = pRenderView->GetBatchFlags(renderList);
	    ...
	    && CTexture::IsTextureExist(CRendererResources::s_ptexWaterVolumeRefl[0]))
	{
		...
		i32k nWidth = i32(pCurrWaterVolRefl->GetWidth() * (downscaleFactor.x));
		i32k nHeight = i32(pCurrWaterVolRefl->GetHeight() * (downscaleFactor.y));
...
i32 CWaterStage::GetCurrentFrameID(i32k frameID) const
{
	return (frameID % 2);
}

i32 CWaterStage::GetPreviousFrameID(i32k frameID) const
{
	return (frameID + 1) % 2;
}
