...
i32 CSceneRenderPass::s_recursionCounter = 0;
...
void CSceneRenderPass::SetupPassContext(u32 stageID, u32 stagePassID, EShaderTechniqueID technique, u32 filter, ERenderListID renderList, u32 excludeFilter, bool drawCompiledRenderObject)
{
...
{
	DRX_ASSERT_MESSAGE(!!pResourceLayout, "Layout to be set turns out to be invalid!");
...
void CSceneRenderPass::ExchangeRenderTarget(u32 slot, CTexture* pNewColorTarget, ResourceViewHandle hRenderTargetView)
{
	DRX_ASSERT(!pNewColorTarget || pNewColorTarget->GetDevTexture());
...
{
	DRX_ASSERT(!pNewDepthTarget || pNewDepthTarget->GetDevTexture());
...
void CSceneRenderPass::PrepareRenderPassForUse(CDeviceCommandListRef RESTRICT_REFERENCE commandList)
{
	...
	if (m_passFlags & ePassFlags_VrProjectionPass)
	{
		if (CVrProjectionUpr::IsMultiResEnabledStatic())
		{
			...
			CVrProjectionUpr::Instance()->PrepareProjectionParameters(commandList, GetViewport(false));
			CVrProjectionUpr::Instance()->PrepareProjectionParameters(commandList, GetViewport(true));
		}
	}
}

void CSceneRenderPass::ResolvePass(CDeviceCommandListRef RESTRICT_REFERENCE commandList, const std::vector<TRect_tpl<u16>>& screenBounds) const
{
	CDeviceGraphicsCommandInterface* pCommandInterface = commandList.GetGraphicsInterface();
	...
	for (const auto &bounds : screenBounds)
	{
		...
		SResourceRegionMapping mapping =
		{
			...
			region,   // dst position
			{
				static_cast<UINT>(std::max<i32>(0, std::min<i32>(w, textureWidth - x))),
				static_cast<UINT>(std::max<i32>(0, std::min<i32>(h, textureHeight - y))),
				1, 1
			},    // size
...
void CSceneRenderPass::BeginRenderPass(CDeviceCommandListRef RESTRICT_REFERENCE commandList, bool bNearest) const
{
	...
	if (m_passFlags & ePassFlags_VrProjectionPass)
	{
		bViewportSet = CVrProjectionUpr::Instance()->SetRenderingState(commandList, viewport,
...
inline void DebugDrawRenderResolve(const std::vector<TRect_tpl<u16>> &ns, const std::size_t count)
{
...
void CSceneRenderPass::DrawRenderItems(CRenderView* pRenderView, ERenderListID list, i32 listStart, i32 listEnd)
{
	DRX_ASSERT(s_recursionCounter == 1);
	DRX_ASSERT(list != EFSLIST_TRANSP_BW && list != EFSLIST_TRANSP_AW && list != EFSLIST_TRANSP_NEAREST);

	u32k nBatchFlags = pRenderView->GetBatchFlags(list);
...
void CSceneRenderPass::DrawTransparentRenderItems(CRenderView* pRenderView, ERenderListID list)
{
	...
	{
		passContext.rendItems = { 0, static_cast<i32>(pRenderView->GetRenderItems(list).size()) };
		passContext.renderItemGroup = m_numRenderItemGroups++;
	...
	else
	{
		if (CRendererCVars::CV_r_RefractionPartialResolveMode == 0)
		{
			...
			resolvePass.resolveScreenBounds.push_back(TRect_tpl<u16>{ static_cast<u16>(vp.x), static_cast<u16>(vp.y), static_cast<u16>(vp.width), static_cast<u16>(vp.height) });
