...
void CToneMappingStage::Execute()
{
	...
	i32 featureMask = ((i32)bSunShafts << 1) | ((i32)bColorGrading << 2) | ((i32)bBloomEnabled << 3) |
	...
	if (m_passToneMapping.IsDirty(featureMask, pSunShaftsTex->GetTextureID(), pColorChartTex->GetTextureID()))
	{
		...
		static CDrxNameTSCRC techToneMapping("HDRFinalPass");
	...
	static CDrxNameR eyeAdaptationName("HDREyeAdaptation");
	static CDrxNameR filmCurveName("HDRFilmCurve");
	static CDrxNameR colorBalanceName("HDRColorBalance");
	static CDrxNameR bloomColorName("HDRBloomColor");
	static CDrxNameR shaftsSunColName("SunShafts_SunCol");
...
void CToneMappingStage::ExecuteDebug()
{
	...
	i32 featureMask = ((CRenderer::CV_r_HDRDebug & 0xF) << 9);
	...
	if (m_passToneMapping.IsDirty(featureMask))
	{
		...
		static CDrxNameTSCRC techToneMapping("HDRFinalDebugPass");
...
//	ASSERT_LEGACY_PIPELINE
	...
	{
		uint64 rtMask = 0;
		static CDrxNameTSCRC techToneMapping("HDRFinalPassFixedExposure");
...
void CToneMappingStage::DisplayDebugInfo()
{
	...
	DRX_ASSERT(pSrcDevTex);

	// Read back data
	const auto readbackData = [&fLuminance, &fIlluminance](uk pData, u32 rowPitch, u32 slicePitch) -> bool
	{
		DrxHalf* pDataHalf = (DrxHalf*)pData;
		fLuminance = DrxConvertHalfToFloat(pDataHalf[0]);
		fIlluminance = DrxConvertHalfToFloat(pDataHalf[1]);
		return true;
	};
	...
	u32 illum = (u32)(fIlluminance * RENDERER_LIGHT_UNIT_SCALE);
