...
#include <drx3D/Render/NullD3D11Device.h>
...
void CD3D9Renderer::DisplaySplash()
{
	...
	HBITMAP hImage = (HBITMAP)LoadImage(DrxGetCurrentModule(), "splash.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
...
static DrxCriticalSection gs_contextLock;
...
SDisplayContextKey CD3D9Renderer::CreateSwapChainBackedContext(const SDisplayContextDescription& desc) threadsafe
{
	...
	i32k width = desc.screenResolution.x;
	i32k height = desc.screenResolution.y;
	const STxt name = desc.type == IRenderer::eViewportType_Default ? "MainView-SwapChain" : "SecView-SwapChain";
	...
	pDC->m_desc.superSamplingFactor.x = std::max<i32>(1, desc.superSamplingFactor.x);
	pDC->m_desc.superSamplingFactor.y = std::max<i32>(1, desc.superSamplingFactor.y);
...
void CD3D9Renderer::ResizeContext(CRenderDisplayContext *pDC, i32 width, i32 height) threadsafe
{
...
void CD3D9Renderer::ResizeContext(const SDisplayContextKey& key, i32 width, i32 height) threadsafe
{
...
RectI CD3D9Renderer::GetDefaultContextWindowCoordinates()
{
	...
	return RectI
	{
		rcClient.left,
		rcClient.top,
		static_cast<i32>(m_pBaseDisplayContext->GetDisplayResolution().x),
		static_cast<i32>(m_pBaseDisplayContext->GetDisplayResolution().y)
	};
}
#endif
...
bool CD3D9Renderer::ChangeRenderResolution(i32 nNewRenderWidth, i32 nNewRenderHeight, CRenderView* pRenderView)
{
	if (m_bDeviceLost)
		return true;
...
bool CD3D9Renderer::ChangeOutputResolution(i32 nNewOutputWidth, i32 nNewOutputHeight, CRenderOutput* pRenderOutput)
{
	if (m_bDeviceLost)
		return true;
...
bool CD3D9Renderer::ChangeDisplayResolution(i32 nNewDisplayWidth, i32 nNewDisplayHeight, i32 nNewColDepth, i32 nNewRefreshHZ, EWindowState previousWindowState, bool bForceReset, CRenderDisplayContext* pDC)
{
...
// 		OnD3D11PostCreateDevice(m_devInfo.Device());
	...
	if (pDrxFont)
	{
		IFFont* pFont = pDrxFont->GetFont("default");
	}
...
bool CD3D9Renderer::ChangeDisplayResolution(i32 nNewDisplayWidth, i32 nNewDisplayHeight, i32 nNewColDepth, i32 nNewRefreshHZ, EWindowState previousWindowState, bool bForceReset, const SDisplayContextKey& displayContextKey)
{
	if (m_bDeviceLost)
		return true;
...
//-----------------------------------------------------------------------------
HRESULT CD3D9Renderer::AdjustWindowForChange(i32k displayWidth, i32k displayHeight, EWindowState previousWindowState)
{
	...
	else if (m_windowState == EWindowState::BorderlessWindow || m_windowState == EWindowState::BorderlessFullscreen)
	{
		...
		i32k x = monitorBounds.x + (monitorBounds.w - displayWidth) / 2;
		i32k y = monitorBounds.y + (monitorBounds.h - displayHeight) / 2;
	...
	else
	{
		...
		i32k x = windowRect.left;
		i32k y = windowRect.top;
		...
		i32k width = windowRect.right - windowRect.left;
		i32k height = windowRect.bottom - windowRect.top;
...
i32 CD3D9Renderer::EnumDisplayFormats(SDispFormat* formats)
{
#if DRX_PLATFORM_WINDOWS || DRX_RENDERER_OPENGL
...
{
#if defined(USE_SDL2_VIDEO) && (DRX_RENDERER_OPENGL || DRX_RENDERER_OPENGLES)
	DXGLDestroySDLWindow(m_hWnd);
#elif defined(USE_SDL2_VIDEO) && (DRX_RENDERER_VULKAN)
...
void CD3D9Renderer::RT_ShutDown(u32 nFlags)
{
	CVrProjectionUpr::Reset();
...
#if !DRX_PLATFORM_ORBIS && !DRX_RENDERER_OPENGL
...
HWND CD3D9Renderer::CreateWindowCallback()
{
	...
	i32k displayWidth  = pDC->GetDisplayResolution()[0];
	i32k displayHeight = pDC->GetDisplayResolution()[1];

	return gcpRendD3D->SetWindow(displayWidth, displayHeight) ? gcpRendD3D->m_hWnd : 0;
}
#endif

bool CD3D9Renderer::SetWindow(i32 width, i32 height)
{
	LOADING_TIME_PROFILE_SECTION;

	iSystem->RegisterWindowMessageHandler(this);

#if USE_SDL2_VIDEO && (DRX_RENDERER_OPENGL || DRX_RENDERER_OPENGLES)
	DXGLCreateSDLWindow(m_WinTitle, width, height, IsFullscreen(), &m_hWnd);
#elif USE_SDL2_VIDEO && (DRX_RENDERER_VULKAN)
	...
	i32 x = monitorInfo.rcMonitor.left;
	i32 y = monitorInfo.rcMonitor.top;
	i32k monitorWidth = monitorInfo.rcMonitor.right - monitorInfo.rcMonitor.left;
	i32k monitorHeight = monitorInfo.rcMonitor.bottom - monitorInfo.rcMonitor.top;
	...
	if (IsEditorMode())
	{
		...
		{
			DrxFatalError("Cannot Register Window Class %s", wc.lpszClassName);
			return false;
		}
	...
	else
	{
		if (!m_hWnd)
		{
			LPCWSTR pClassName = L"DrxENGINE";
			...
			{
				DrxFatalError("Cannot Register Launcher Window Class");
				return false;
			}
			...
			{
				DrxFatalError("Expected an UNICODE window for launcher");
				return false;
			}
...
#if DRX_PLATFORM_WINDOWS || DRX_PLATFORM_ANDROID || DRX_PLATFORM_LINUX
	m_VSync = !IsEditorMode() ? CV_r_vsync : 0;
...
bool CD3D9Renderer::SetWindowIcon(tukk path)
{
#if DRX_PLATFORM_WINDOWS
	if (IsEditorMode())
	{
...
WIN_HWND CD3D9Renderer::Init(i32 x, i32 y, i32 width, i32 height, u32 colorBits, i32 depthBits, i32 stencilBits, WIN_HWND Glhwnd, bool bReInit, bool bShaderCacheGen)
{
	LOADING_TIME_PROFILE_SECTION;
...
#if (DRX_RENDERER_OPENGL || DRX_RENDERER_OPENGLES) && !DXGL_FULL_EMULATION
	#if OGL_SINGLE_CONTEXT
	DXGLInitialize(0);
	#else
	...
	iLog->Log("Dinrus Direct3D driver version %4.2f (%s <%s>)", VERSION_D3D, __DATE__, __TIME__);

	tukk sGameName = gEnv->pSystem->GetIProjectUpr()->GetCurrentProjectName();
	drx_strcpy(m_WinTitle, sGameName);
	...
	if (!m_bShaderCacheGen)
	{
		...
		while (true)
		{
			...
			if (!bRes)
			{
				DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_ERROR, "Rendering device creation failed!");
				ShutDown(true);
				return 0;
			}
...
#if defined(SUPPORT_DEVICE_INFO)
		iLog->Log(" ****** DrxRender InfoDump ******");
		iLog->Log(" Driver описание: %S", m_devInfo.AdapterDesc().Description);
#endif

#if DRX_RENDERER_DIRECT3D
		iLog->Log(" API Version: Direct3D %d.%1d", (DRX_RENDERER_DIRECT3D / 10), (DRX_RENDERER_DIRECT3D % 10));
#endif
		...
		CVrProjectionUpr::Instance()->Init(this);
	...
	#elif (DRX_RENDERER_DIRECT3D >= 112)
		GetDevice().GetImmediateContext2(&pContext);
	#elif (DRX_RENDERER_DIRECT3D >= 111)
...
#define QUALITY_VAR(name) { ICVar* pVar = ConsoleRegistrationHelper::Register("q_Shader" # name, &m_cEF.m_ShaderProfiles[(i32)eST_ ## name].m_iShaderProfileQuality, 1, \
...
#undef QUALITY_VAR
	...
	if (!g_shaderGeneralHeap)
		g_shaderGeneralHeap = DrxGetIMemoryUpr()->CreateGeneralExpandingMemoryHeap(4 * 1024 * 1024, 0, "Shader General");
...
void CD3D9Renderer::InitNVAPI()
{
	...
	// enumerate displays
	for (i32 i = 0; i < NVAPI_MAX_DISPLAYS; ++i)
	{
...
bool CD3D9Renderer::CreateDeviceDurango()
{
	...
	CDrxDX12Device* pDX12Device = nullptr;
	...
	if (pD3D11CD)
	{
...
#if defined(DX11_ALLOW_D3D_DEBUG_RUNTIME)
		u32k debugRTFlag = gcpRendD3D->CV_r_EnableDebugLayer ? D3D11_CREATE_DEVICE_VALIDATED | D3D11_CREATE_DEVICE_DEBUG : 0;
#else
		u32k debugRTFlag = 0;
#endif

		D3D_DRIVER_TYPE driverType = D3D_DRIVER_TYPE_HARDWARE;
		u32 creationFlags = debugRTFlag
		...
		D3D_FEATURE_LEVEL arrFeatureLevels[] = { DRX_RENDERER_DIRECT3D_FL };
		D3D_FEATURE_LEVEL* pFeatureLevels(arrFeatureLevels);
		u32 uNumFeatureLevels(sizeof(arrFeatureLevels) / sizeof(D3D_FEATURE_LEVEL));
		...
		{
#if (DRX_RENDERER_DIRECT3D >= 120)
			pD3D12Device = (pDX12Device = reinterpret_cast<CDrxDX12Device*>(pD3D11Device))->GetD3D12Device();
#endif
		}
	}
	...
	{
		DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_ERROR, "XBOX::CreateDevice() failed");
		return false;
	}
...
#if defined(DEVICE_SUPPORTS_PERFORMANCE_DEVICE)
	{
		...
		hr = GetDevice().QueryInterface(__uuidof(ID3DXboxPerformanceDevice), (uk *)&pPerformanceDevice);
		assert(hr == S_OK);

		hr = GetDeviceContext().QueryInterface(__uuidof(ID3DXboxPerformanceContext), (uk *)&pPerformanceDeviceContext);
		assert(hr == S_OK);
...
#ifdef USE_PIX_DURANGO
	HRESULT res = GetDeviceContext().QueryInterface(__uuidof(ID3DUserDefinedAnnotation), reinterpret_cast<uk *>(&m_pPixPerf));
...
#elif DRX_PLATFORM_IOS || DRX_PLATFORM_ANDROID
...
#elif DRX_PLATFORM_WINDOWS || DRX_PLATFORM_APPLE || DRX_PLATFORM_LINUX
bool CD3D9Renderer::CreateDeviceDesktop()
{
	...
	m_adapterInfo.name = DrxStringUtils::WStrToUTF8(desc.Description).c_str();
...
bool CD3D9Renderer::CreateDeviceOrbis()
{
	...
	DXOrbis::CreateCDrxDXOrbisRenderDevice();
	DXOrbis::CreateCDrxDXOrbisSwapChain();
...
#if (DRX_RENDERER_DIRECT3D >= 110) && NTDDI_WIN10 && (WDK_NTDDI_VERSION >= NTDDI_WIN10) && defined(SUPPORT_DEVICE_INFO)
	#include <drx3D/Render/dxgi1_4.h>
#endif
...
void CD3D9Renderer::GetVideoMemoryUsageStats(size_t& vidMemUsedThisFrame, size_t& vidMemUsedRecently, bool bGetPoolsSizes)
{
	...
	else
	{
		...
		pAdapter->AddRef();
	#else
		rd->m_devInfo.Adapter()->QueryInterface(__uuidof(IDXGIAdapter3), (uk *)&pAdapter);
	#endif
...
HRESULT CALLBACK CD3D9Renderer::OnD3D11CreateDevice(D3DDevice* pd3dDevice)
{
...
#if defined(SUPPORT_DEVICE_INFO)
	rd->BindContextToThread(DrxGetCurrentThreadId());
	...
	#endif
	iLog->Log("D3D Adapter: Описание: %ls", rd->m_devInfo.AdapterDesc().Description);
	...
	rd->m_nGPUs = min(rd->m_nGPUs, (u32)MAX_GPU_NUM);

#endif
	rd->m_adapterInfo.nNodeCount = max(rd->m_nGPUs, rd->m_adapterInfo.nNodeCount);
	DrxLogAlways("Active GPUs: %i", rd->m_nGPUs);
	...
	if (CRenderer::CV_r_TexturesStreamPoolSize <= 0)
		CRenderer::CV_r_TexturesStreamPoolSize = (i32)(rd->m_MaxTextureMemory / 1024.0f / 1024.0f * 0.75f);
	...
	case WM_SYSKEYDOWN:
		{
			...
			if (bAlt)
			{
				if (wParam == VK_RETURN) // ALT+ENTER
				{
					...
					{
						if (m_CVWindowType->GetIVal() == static_cast<i32>(EWindowState::Fullscreen))
						{
							m_CVWindowType->Set(static_cast<i32>(EWindowState::Windowed));
						}
						else
						{
							m_CVWindowType->Set(static_cast<i32>(EWindowState::Fullscreen));
						}
...
#include <drx3D/Render/DeviceInfo.inl"
...
void EnableCloseButton(uk hWnd, bool enabled)
{
	...
	if (hWnd)
	{
		...
		if (hMenu)
		{
			u32k flags = enabled ? MF_ENABLED : (MF_DISABLED | MF_GRAYED);
