...
#if !DRX_PLATFORM_ORBIS && !DRX_RENDERER_OPENGL && !DRX_RENDERER_OPENGLES && !DRX_RENDERER_VULKAN
...
static DrxCriticalSection g_cAILock;
...
DrxEvent SShaderAsyncInfo::s_RequestEv;

i32 CHWShader_D3D::s_nDevicePSDataSize = 0;
i32 CHWShader_D3D::s_nDeviceVSDataSize = 0;

namespace
{
	inline i32 GetCurrentFrameID()
	{
...
public:
	CSpinLock()
	{
		...
		while (DrxInterlockedCompareExchange(&s_locked, 1L, 0L) == 1L)
			DrxSleep(0);
#endif
	}
	...
	~CSpinLock()
	{
		...
		while (DrxInterlockedCompareExchange(&s_locked, 0L, 1L) == 0L);
#endif
	}
...
private:
 ...
 LONG CSpinLock::s_locked = 0L;

 i32 SShaderAsyncInfo::s_nPendingAsyncShaders = 0;
i32 SShaderAsyncInfo::s_nPendingAsyncShadersFXC = 0;

//==============================================================================================================

bool CHWShader_D3D::mfAddFXSampler(SHWSInstance* pInst, SShaderFXParams& FXParams, SFXSampler* pr, tukk ParamName, SCGBind* pBind, CShader* ef, EHWShaderClass eSHClass)
{
	assert(pBind);
	if (!pBind)
		return false;
	...
	{
		for (u32 i = 0; i < pParams->size() - nOffs; i++)
		{
...
bool CHWShader_D3D::mfAddFXTexture(SHWSInstance* pInst, SShaderFXParams& FXParams, SFXTexture* pr, tukk ParamName, SCGBind* pBind, CShader* ef, EHWShaderClass eSHClass)
{
	assert(pBind);
	if (!pBind)
		return false;
...
void CHWShader_D3D::mfAddFXParameter(SHWSInstance* pInst, SParamsGroup& OutParams, SShaderFXParams& FXParams, SFXParam* pr, tukk ParamName, SCGBind* pBind, CShader* ef, bool bInstParam, EHWShaderClass eSHClass)
{
	SCGParam CGpr;
	...
	i32 nComps = 0;
	i32 nParams = pBind->m_nParameters;
	if (!pr->m_Semantic.empty())
		nComps = pr->m_nComps;
	else
	{
		for (i32 i = 0; i < pr->m_nComps; i++)
		{
			DrxFixedStringT<128> cur;
...
bool CHWShader_D3D::mfAddFXParameter(SHWSInstance* pInst, SParamsGroup& OutParams, SShaderFXParams& FXParams, tukk param, SCGBind* bn, bool bInstParam, EHWShaderClass eSHClass, CShader* pFXShader)
{
...
i32 CGBindCallback(const VOID* arg1, const VOID* arg2)
{
...
tukk szNamesCB[CB_NUM] = { "PER_BATCH", "PER_MATERIAL" };

void CHWShader_D3D::mfCreateBinds(std::vector<SCGBind> &binds, ukk pConstantTable, std::size_t nSize)
{
	u32 i;
	...
	for (u32 n = 0; n < Desc.ConstantBuffers; n++)
	{
...
void CHWShader_D3D::mfGatherFXParameters(SHWSInstance* pInst, std::vector<SCGBind>& BindVars, CHWShader_D3D* pSH, i32 nFlags, CShader* pFXShader)
{
	...
	if (Group.Params[0].size() > 0)
	{
		...
		pInst->m_nParams[0] = CGParamUpr::GetParametersGroup(Group, 0);
	}
	if (Group.Params[1].size() > 0)
	{
		qsort(&Group.Params[1][0], Group.Params[1].size(), sizeof(SCGParam), CGBindCallback);
		pInst->m_nParams[1] = CGParamUpr::GetParametersGroup(Group, 1);
	}
}
...
void CHWShader_D3D::mfUpdateFXVertexFormat(SHWSInstance* pInst, CShader* pSH)
{
	...
	if (pSH)
	{
		...
		bool bCurrent = false;
		for (u32 i = 0; i < pSH->m_HWTechniques.Num(); i++)
		{
...
InputLayoutHandle CHWShader_D3D::mfVertexFormat(bool& bUseTangents, bool& bUseLM, bool& bUseHWSkin)
{
	...
	for (i = 0; i < m_Insts.size(); i++)
	{
		...
		eVFormat = (InputLayoutHandle)max((u32)eVFormat, (u32)pInst->m_nVertexFormat);
...
InputLayoutHandle CHWShader_D3D::mfVertexFormat(SHWSInstance* pInst, CHWShader_D3D* pSH, D3DBlob* pShader, uk pConstantTable)
{
	...
	uk pData = pShader->GetBufferPointer();
	uk pShaderReflBuf = pConstantTable;
	...
	for (u32 i = 0; i < Desc.InputParameters; i++)
	{
		...
		{
			DRX_ASSERT_TRACE(0, ("Invalid SemanticName %s", IDesc.SemanticName));
		}
...
static bool sGetMask(tuk str, SShaderGen* pGen, uint64& nMask)
{
	u32 i;
...
void CHWShader::mfValidateTokenData(CResFile* pRes)
{
	...
	for (u32 i = 0; i < Dir->size(); i++)
	{
		...
		if (pDE->GetFlags() & RF_RES_$TOKENS)
		{
			...
			if (nL * sizeof(u32) > nSize)
			{
				bTokenValid = false;
				break;
			}

			i32 nTableSize = nSize - (4 + nL * sizeof(u32));
			if (nTableSize < 0)
			{
				bTokenValid = false;
				break;
	...
	if (!bTokenValid)
		DrxFatalError("Invalid token data in shader cache file");
#endif
}
...
void CHWShader::mfValidateDirEntries(CResFile* pRF)
{
	...
	for (auto it1 = dirEntries->begin(); it1 != dirEntries->end(); it1++)
	{
		for (auto it2 = it1 + 1; it2 != dirEntries->end(); ++it2)
		{
			...
			{
				DrxFatalError("Duplicate dir entry in shader cache file");
			}
		}
	}
#endif
...
bool CHWShader::mfWriteoutTokensToCache()
{
	...
	char szName[256];
#if defined(__GNUC__)
	drx_sprintf(szName, "$MAP_%llx", m_nMaskGenFX);
#else
	drx_sprintf(szName, "$MAP_%I64x", m_nMaskGenFX);
#endif

	auto cache = AcquireDiskCache(cacheSource::user);
	DRX_ASSERT_MESSAGE(cache && cache->m_pRes, "Could not acquire user disk cache");
	if (cache && cache->m_pRes)
	{
		...
		pOE->pData = const_cast<tuk>(m_CachedTokens.data());
...
bool CHWShader_D3D::mfStoreCacheTokenMap(const FXShaderToken& Table, const TArray<u32>& SHData)
{
	TArray<byte> Data;
	...
	if (CParserBin::m_bEndians)
	{
		...
		Data.Copy((byte*)&nSizeEnd, sizeof(u32));
		for (u32 i = 0; i < nSize; i++)
		{
			u32 nToken = SHData[i];
			SwapEndian(nToken, eBigEndian);
			Data.Copy((byte*)&nToken, sizeof(u32));
		}
	}
	else
	{
		Data.Copy((byte*)&nSize, sizeof(u32));
		Data.Copy((byte*)&SHData[0], nSize * sizeof(u32));
	}
	for (auto T : Table)
	{
		if (CParserBin::m_bEndians)
...
bool CHWShader_D3D::mfGetCacheTokenMap(FXShaderToken& Table, TArray<u32>& SHData)
{
	...
	{
		memcpy(&SHData[0], &pData[4], nL * sizeof(u32));
	}
	pData += 4 + nL * sizeof(u32);
	nSize -= 4 + nL * sizeof(u32);
	i32 nOffs = 0;

	while (nOffs < nSize)
	{
		tuk pStr = (tuk)&pData[nOffs + sizeof(DWORD)];
		DWORD nToken;
		LoadUnaligned(pData + nOffs, nToken);
		if (CParserBin::m_bEndians)
			SwapEndian(nToken, eBigEndian);
		i32 nLen = strlen(pStr) + 1;
		STokenD TD;
		TD.Token = nToken;
		TD.SToken = pStr;
		Table.push_back(TD);
...
bool CHWShader_D3D::mfGenerateScript(CShader* pSH, SHWSInstance* pInst, std::vector<SCGBind>& InstBindVars, u32 nFlags, TArray<char>& sNewScr)
{
	u32 nSFlags = m_Flags;
	FXShaderToken Table;
	TArray<u32> SHData;
	...
	{
		i32 nLights = pInst->m_Ident.m_LightMask & 0xf;
		if (nLights)
		...
		for (i32 i = 0; i < 4; i++)
		{
			i32 nLightType = (pInst->m_Ident.m_LightMask >> (SLMF_LTYPE_SHIFT + i * SLMF_LTYPE_BITS)) & SLMF_TYPE_MASK;
	...
	{
		CParserBin::AddDefineToken(eT__LT_LIGHTS, NewTokens);
		i32 nLightType = (pInst->m_Ident.m_LightMask >> SLMF_LTYPE_SHIFT) & SLMF_TYPE_MASK;
		if (nLightType == SLMF_PROJECTED)
			CParserBin::AddDefineToken(eT__LT_HASPROJ, eT_1, NewTokens);
	}
	...
	{
		u32k tcProjMask = HWMD_TEXCOORD_PROJ;
		u32k tcMatrixMask = HWMD_TEXCOORD_MATRIX;
		u32k tcGenObjectLinearMask = HWMD_TEXCOORD_GEN_OBJECT_LINEAR;
	...
	{
		i32 nMDV = pInst->m_Ident.m_MDVMask & 0x0fffffff;
		i32 nType = nMDV & 0xf;
		if (nType)
	...
	if (nSFlags & HWSG_FP_EMULATION)
	{
		...
		{
			i32 nStreams = pInst->m_Ident.m_LightMask & 0xff;
	...
	i32 nT = NewTokens.size();
	NewTokens.resize(nT + SHData.size());
	memcpy(&NewTokens[nT], &SHData[0], SHData.size() * sizeof(u32));
...
bool CHWShader_D3D::AutoGenMultiresGS(TArray<char>& sNewScr, CShader *pSH)
{
	...
	if (szStart)
	{
		...
		{
#if !defined(_RELEASE)
			DRX_ASSERT_MESSAGE(false, "Cannot generate a GS for a VS with no SV_POSITION output");
#endif
			return false;
		}
		...
		szNewS.Copy(szStrStart, u32(szStrEnd-szStrStart)+1);
		...
		memcpy(&NewTokens[nT], &m_TokenData[0], m_TokenData.size() * sizeof(u32));
...
{
	i32 nPos = Parser.FindToken(0, Parser.m_Tokens.size() - 1, eT_cbuffer);
	while (nPos >= 0)
	{
		u32 nName = Parser.m_Tokens[nPos + 1];
		if (nName == eT_PER_BATCH || nName == eT_PER_INSTANCE)
		{
			i32 nPosEnd = Parser.FindToken(nPos + 3, Parser.m_Tokens.size() - 1, eT_br_cv_2);
			assert(nPosEnd >= 0);
			i32 nPosN = Parser.FindToken(nPos + 1, Parser.m_Tokens.size() - 1, eT_br_cv_1);
			assert(nPosN >= 0);
			nPosN++;
			while (nPosN < nPosEnd)
			{
				u32 nT = Parser.m_Tokens[nPosN + 1];
				i32 nPosCode = Parser.FindToken(nPosEnd + 1, Parser.m_Tokens.size() - 1, nT);
				if (nPosCode < 0)
				{
					assert(nPosN > 0 && nPosN < (i32)Parser.m_Tokens.size());
					if (InstBindVars.size())
					{
						size_t i = 0;
						CDrxNameR nm(Parser.GetString(nT));
	...
	/*i32 nStart = 0;
	   while (true)
	   {
	   u32 nName = sFindVar(Parser, nStart);
	   if (nStart < 0)
	    break;
...
#if DRX_RENDERER_VULKAN
STxt resourceTypeToString(SResourceBindPoint::ESlotType type)
{
	switch (type)
	{
	...
	case SResourceBindPoint::ESlotType::UnorderedAccessView:
		return "u";
	default:
		DRX_ASSERT_MESSAGE(false, "Type is not defined.");
		return "";
	};
};
...
struct ResourceSetBindingInfo
{
	...
	ResourceSetBindingInfo(u8 s = (u8)~0u, u8 b = (u8)~0u, SResourceBindPoint::ESlotType t = SResourceBindPoint::ESlotType::ConstantBuffer)
...
u32 AddNewTableEntry(FXShaderToken& tokenTable, tukk  newTokenStr)
{
	u32 nToken = CParserBin::GetCRC32(newTokenStr);
...
RegisterRanges ExtractRegisterRanges(const std::vector<u8>& RsrcLayoutEncoding)
{
...
void CHWShader_D3D::AddResourceLayoutToBinScript(CParserBin & Parser, SHWSInstance* pInst, FXShaderToken* Table)
{
	...
	std::map<STxt, ResourceSetBindingInfo> vkResourceMapping;
	const std::vector<u8>* LayoutEncoding = GetDeviceObjectFactory().LookupResourceLayoutEncoding(pInst->m_Ident.m_pipelineState.VULKAN.resourceLayoutHash);
	if (LayoutEncoding)
	{
		...
		for (setIndex = 0; setIndex < (uint32_t)registerRanges.size(); setIndex++)
		{
			...
			for (size_t i = 0; i < registerRanges[setIndex].size(); i++)
			{
				...
				for (uint32_t registerIdxOffset = 0; registerIdxOffset < rangeDesc.count; ++registerIdxOffset)
				{
					...
					{
						STxt resourceRegisterName = resourceTypeToString(rangeDesc.type);
	...
	i32 nCur = 0;
	i32 nSize = Parser.m_Tokens.size();
	u32k* pTokens = &Parser.m_Tokens[0];
	std::map<u32, u32> alreadyProcessed;
	while (true)
	{
		...
		if (nCur < 0)
			break;
		i32 OpenBacketToken = Parser.FindToken(nCur, nSize - 1, eT_br_rnd_1);
		i32 CloseBacketToken = Parser.FindToken(nCur, nSize - 1, eT_br_rnd_2);
		...
		u32 newToken = nToken;
		tukk registerName = Parser.GetString(nToken, *Table, false);
		auto resource = vkResourceMapping.find(registerName);
		if (resource != vkResourceMapping.end())
		{
			STxt newTokenStr = resource->second.ToString();
...
void CHWShader_D3D::CorrectScriptEnums(CParserBin& Parser, SHWSInstance* pInst, std::vector<SCGBind>& InstBindVars, const FXShaderToken& Table)
{
	...
	i32 nInstParam = 0;
	u32k Toks[] = { eT_TEXCOORDN, eT_TEXCOORDN_centroid, eT_unknown };
	...
	while (true)
	{
		...
		if (nCur < 0)
			break;
		i32 nLastStr = Parser.FindToken(nCur, nSize - 1, eT_br_cv_2);
		assert(nLastStr >= 0);
		if (nLastStr < 0)
			break;
		bool bNested = false;
		...
		u32 nName = pTokens[nCur + 1];
		i32 n = 0;
		while (nCur < nLastStr)
		{
			i32 nTN = Parser.FindToken(nCur, nLastStr, Toks);
			if (nTN < 0)
			{
				nCur = nLastStr + 1;
				break;
			...
			if (pTokens[nTN - 2] == eT_br_sq_2)
			{
				...
				if ((u32) nArrSize > 15)
				{
					tukk szArrSize = Parser.GetString(pTokens[nTN - 3], Table);
					nArrSize = szArrSize ? atoi(szArrSize) : 0;
				}
				assert(pTokens[nTN - 4] == eT_br_sq_1);
				//tukk szName = Parser.GetString(pTokens[nTN-5], *Table);
				nTokName = pTokens[nTN - 5];
			}
			else
			{
				u32 nType = pTokens[nTN - 3];
...
static i32 sFetchInst(u32& nCur, u32* pTokens, u32 nT, std::vector<u32>& Parameter)
{
	while (true)
	{
...
bool CHWShader_D3D::ConvertBinScriptToASCII(CParserBin& Parser, SHWSInstance* pInst, std::vector<SCGBind>& InstBindVars, const FXShaderToken& Table, TArray<char>& Text)
{
	...
	for (i = 0; i < nT; i++)
	{
		...
		if (nToken == eT_fetchinst)
		{
			char str[512];
			i++;
			std::vector<u32> ParamDst, ParamSrc;
			TArray<char> sParamDstFull, sParamDstName, sParamSrc;
			i32 nDst = sFetchInst(i, &Parser.m_Tokens[0], Parser.m_Tokens.size(), ParamDst);
			assert(Parser.m_Tokens[i] == eT_eq);
			if (Parser.m_Tokens[i] != eT_eq)
			{
				// Should never happen
				i32 n = CParserBin::FindToken(i, Parser.m_Tokens.size() - 1, &Parser.m_Tokens[0], eT_semicolumn);
				if (n > 0)
					i = n + 1;
				continue;
			}
			i++;
			i32 nSrc = sFetchInst(i, &Parser.m_Tokens[0], Parser.m_Tokens.size(), ParamSrc);
		...
		tukk szStr = CParserBin::GetString(nToken, Table, false);
		assert(szStr);
		if (!szStr || !szStr[0])
		{
			assert(0);
		...
		{
#if defined(_DEBUG) && !DRX_PLATFORM_ORBIS
			i32 n = 0;
			while (szStr[n])
			{
				char c = szStr[n++];
...
i32 OrigToBase64Size(i32 orig_size)
{
	return ((orig_size + 2) / 3) * 4;
}

static u8k BASE64_TABLE[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

void Base64EncodeBuffer(ukk orig_buf, i32 orig_buf_size, uk b64_buf)
{
	u8k* p = (u8k*)orig_buf;
	u8* q = (u8*)b64_buf;
	i32 w;

	for (i32 i = 0, e = orig_buf_size / 3; i < e; ++i, p += 3, q += 4)
	{
...
bool CHWShader_D3D::AddResourceLayoutToScriptHeader(SHWSInstance* pInst, tukk szProfile, tukk pFunCDrxName, TArray<char>& Scr)
{
#if DRX_RENDERER_VULKAN
	...
	{
		i32k TempBufferSize = 4096;
		i32 bin[TempBufferSize];
		i32 pointer = 0;
		...
		bin[pointer++] = strlen(pFunCDrxName);
		bin[pointer + ((strlen(pFunCDrxName) + 3) / 4) - 1] = 0;
		memcpy(&bin[pointer], pFunCDrxName, strlen(pFunCDrxName));
		pointer += (strlen(pFunCDrxName) + 3) / 4;

		DRX_ASSERT(TempBufferSize - pointer - pEncodedLayout->size() > 0);
		memcpy(&bin[pointer], pEncodedLayout->data(), pEncodedLayout->size());
		pointer += (pEncodedLayout->size() + 3) / 4;

		// vertex input описание: currently disabled.
		bin[pointer++] = 0;
		bin[pointer++] = 0;
...
void CHWShader_D3D::mfGetSrcFileName(tuk srcName, i32 nSize)
{
	if (!srcName || nSize <= 0)
		return;
	if (!m_NameSourceFX.empty())
	{
		drx_strcpy(srcName, nSize, m_NameSourceFX.c_str());
		return;
	}
	drx_strcpy(srcName, nSize, gRenDev->m_cEF.m_HWPath);
	if (m_eSHClass == eHWSC_Vertex)
		drx_strcat(srcName, nSize, "Declarations/CGVShaders/");
	else if (m_eSHClass == eHWSC_Pixel)
		drx_strcat(srcName, nSize, "Declarations/CGPShaders/");
	else
		drx_strcat(srcName, nSize, "Declarations/CGGShaders/");
	drx_strcat(srcName, nSize, GetName());
	drx_strcat(srcName, nSize, ".drxcg");
}

void CHWShader_D3D::mfGenName(SHWSInstance* pInst, tuk dstname, i32 nSize, byte bType)
{
	if (bType)
...
void CHWShader_D3D::mfGetDstFileName(SHWSInstance* pInst, tuk dstname, i32 nSize, byte bType)
{
	drx_strcpy(dstname, nSize, gRenDev->m_cEF.m_ShadersCache);

	if (m_eSHClass == eHWSC_Vertex)
	{
		if (bType == 1 || bType == 4)
			drx_strcat(dstname, nSize, "CGVShaders/Debug/");
		else if (bType == 0)
			drx_strcat(dstname, nSize, "CGVShaders/");
		else if (bType == 2 || bType == 3)
			drx_strcat(dstname, nSize, "CGVShaders/Pending/");
	}
	else if (m_eSHClass == eHWSC_Pixel)
	{
		if (bType == 1 || bType == 4)
			drx_strcat(dstname, nSize, "CGPShaders/Debug/");
		else if (bType == 0)
			drx_strcat(dstname, nSize, "CGPShaders/");
		else if (bType == 2 || bType == 3)
			drx_strcat(dstname, nSize, "CGPShaders/Pending/");
	}
	else if (m_eSHClass == eHWSC_Geometry)
	{
		if (bType == 1 || bType == 4)
			drx_strcat(dstname, nSize, "CGGShaders/Debug/");
		else if (bType == 0)
			drx_strcat(dstname, nSize, "CGGShaders/");
		else if (bType == 2 || bType == 3)
			drx_strcat(dstname, nSize, "CGGShaders/Pending/");
	}
	else if (m_eSHClass == eHWSC_Hull)
	{
		if (bType == 1 || bType == 4)
			drx_strcat(dstname, nSize, "CGHShaders/Debug/");
		else if (bType == 0)
			drx_strcat(dstname, nSize, "CGHShaders/");
		else if (bType == 2 || bType == 3)
			drx_strcat(dstname, nSize, "CGHShaders/Pending/");
	}
	else if (m_eSHClass == eHWSC_Domain)
	{
		if (bType == 1 || bType == 4)
			drx_strcat(dstname, nSize, "CGDShaders/Debug/");
		else if (bType == 0)
			drx_strcat(dstname, nSize, "CGDShaders/");
		else if (bType == 2 || bType == 3)
			drx_strcat(dstname, nSize, "CGDShaders/Pending/");
	}
	else if (m_eSHClass == eHWSC_Compute)
	{
		if (bType == 1 || bType == 4)
			drx_strcat(dstname, nSize, "CGCShaders/Debug/");
		else if (bType == 0)
			drx_strcat(dstname, nSize, "CGCShaders/");
		else if (bType == 2 || bType == 3)
			drx_strcat(dstname, nSize, "CGCShaders/Pending/");
	}

	drx_strcat(dstname, nSize, GetName());

	if (bType == 2)
		drx_strcat(dstname, nSize, "_out");

	if (bType == 0)
	{
		tuk s = strchr(dstname, '(');
		if (s)
			s[0] = 0;
	}
	...
	drx_strcat(dstname, nSize, szGenName);
}

//========================================================================================================
// Binary cache support

SDiskShaderCache::SDiskShaderCache(tukk name, cacheSource cacheType) : cacheType(cacheType)
{
	OpenCacheFile(name, cacheType);
}

SDiskShaderCache::SDiskShaderCache(recreateUserCacheTag, tukk name, std::uint32_t CRC32, float cacheVer) : cacheType(cacheSource::user)
{
	...
	bool result = rfUser->mfOpen(RA_CREATE | (CParserBin::m_bEndians ? RA_ENDIANS : 0), &gRenDev->m_cEF.m_ResLookupDataMan[static_cast<i32>(cacheType)]) > 0;
	if (!result)
	{
		rfUser->mfClose();
		SAFE_DELETE(rfUser);
...
std::pair<std::unique_ptr<byte[]>, u32> SDiskShaderCache::DecompressResource(CResFileOpenScope &scope, size_t offset, size_t size) const
{
	...
	// Read resource
	gEnv->pDrxPak->FSeek(handle->mfGetHandle(), offset, SEEK_SET);
	std::unique_ptr<byte[]> dataSource = std::unique_ptr<byte[]>(new byte[size]);
	const auto readBytes = gEnv->pDrxPak->FReadRaw(dataSource.get(), size, 1, handle->mfGetHandle());
	gEnv->pDrxPak->FSeek(handle->mfGetHandle(), 0, SEEK_SET);
	...
	case RESVERSION_LZSS:
		decompressedSize = *reinterpret_cast<u32*>(buf);
		if (size >= 10000000)
			return ReturnType{};
		if (swapEndian)
		...
		if (!pData)
		{
			DrxWarning(EValidatorModule::VALIDATOR_MODULE_RENDERER, EValidatorSeverity::VALIDATOR_ERROR, "FileRead - Allocation fault");
			return ReturnType{};
		}
		if (!Decodem(&buf[4], pData.get(), size - 4, decompressedSize))
		{
			DrxWarning(EValidatorModule::VALIDATOR_MODULE_RENDERER, EValidatorSeverity::VALIDATOR_ERROR, "FileRead - Decodem fault");
			return ReturnType{};
		}
	...
	case RESVERSION_LZMA:
		...
		{
			DrxWarning(EValidatorModule::VALIDATOR_MODULE_RENDERER, EValidatorSeverity::VALIDATOR_ERROR, "FileRead - data error");
			return ReturnType{};
		}
		decompressedSize = static_cast<u32>(outSize64);
		if (decompressedSize != 0)
		{
			pData = std::unique_ptr<byte[]>(new byte[decompressedSize]);
			if (!pData)
			{
				DrxWarning(EValidatorModule::VALIDATOR_MODULE_RENDERER, EValidatorSeverity::VALIDATOR_ERROR, "FileRead - can't allocate");
				return ReturnType{};
			}
			...
			if (res != 0)
			{
				DrxWarning(EValidatorModule::VALIDATOR_MODULE_RENDERER, EValidatorSeverity::VALIDATOR_ERROR, "FileRead - LzmaDecoder error");
				return ReturnType{};
			}
		}
	...
	case RESVERSION_DEBUG:
		...
		{
			DrxWarning(EValidatorModule::VALIDATOR_MODULE_RENDERER, EValidatorSeverity::VALIDATOR_ERROR, "FileRead - Corrupt DirEntiy size");
			return ReturnType{};
		}
		pData = std::unique_ptr<byte[]>(new byte[decompressedSize]);
...
void SDiskShaderCache::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
{
	bool bValid = true;
	i32 nRes = pRF->mfOpen(RA_READ | (CParserBin::m_bEndians ? RA_ENDIANS : 0), &gRenDev->m_cEF.m_ResLookupDataMan[static_cast<i32>(cacheType)], nullptr);
	if (nRes == 0)
	{
		pRF->mfClose();
		bValid = false;
	...
	else if (nRes > 0)
	{
		if (cacheType == cacheSource::user)
		{
			...
			if (bValid)
			{
				i32 nAcc = CRenderer::CV_r_shadersAllowCompilation != 0 ? (RA_READ | RA_WRITE) : RA_READ;
				if (!pRF->mfOpen(nAcc | (CParserBin::m_bEndians ? RA_ENDIANS : 0), &gRenDev->m_cEF.m_ResLookupDataMan[static_cast<i32>(cacheType)]))
				{
					pRF->mfClose();
					bValid = false;
				}
...
bool SDiskShaderCache::OpenCacheFile(tukk szName, cacheSource src)
{
	bool bValidRO = false;
...
SDeviceShaderEntry CHWShader_D3D::mfGetCacheItem(CShader* pFX, tukk name, SDiskShaderCache *cache, u32& nFlags)
{
	if (!cache || !cache->m_pRes)
		return {};
	...
	if (!rfOpenGuard.open(RA_READ | (CParserBin::m_bEndians ? RA_ENDIANS : 0), &gRenDev->m_cEF.m_ResLookupDataMan[static_cast<i32>(cache->GetType())], nullptr))
		return {};
...
bool CHWShader_D3D::mfAddCacheItem(SDiskShaderCache* pCache, SShaderCacheHeaderItem* pItem, const byte* pData, i32 nLen, bool bFlush, CDrxNameTSCRC Name)
{
	...
	if (CRenderer::CV_r_shadersdebug == 3 || CRenderer::CV_r_shadersdebug == 4)
		DrxLog("---Cache: StoredToGlobal %s': 0x%x", pCache->m_pRes->mfGetFileName(), Name.get());

	pItem->m_CRC32 = CCrc32::Compute(pData, nLen);
	//DrxLog("Size: %d: CRC: %x", nLen, pItem->m_CRC32);
	...
	rfOpenGuard.open(RA_WRITE | RA_READ | (CParserBin::m_bEndians ? RA_ENDIANS : 0), &gRenDev->m_cEF.m_ResLookupDataMan[static_cast<i32>(pCache->GetType())], nullptr);
	pCache->m_pRes->mfFileAdd(&de);
	SDirEntryOpen* pOE = pCache->m_pRes->mfOpenEntry(de.GetName());
	if (!pOE)
	{
		DRX_ASSERT_MESSAGE(false, "CHWShader_D3D::mfAddCacheItem(): Couldn't add/open cache entry");
		delete[] pNew;
		return false;
	}
...
bool CHWShader_D3D::mfAddEmptyCombination(uint64 nRT, uint64 nGL, u32 nLT, const SCacheCombination& cmbSaved)
{
...
bool SDiskShaderCache::mfOptimiseCacheFile(SOptimiseStats* pStats)
{
	...
	for (i = 0; i < Data.size(); i++)
	{
		...
		i32 nSizeComp = Data[i].nSizeComp;
		i32 nSizeDecomp = Data[i].nSizeDecomp;
	...
	if (nOutFiles != Data.size() || CRenderer::CV_r_shaderscachedeterministic)
	{
		...
		m_pRes->mfClose();
		m_pRes->mfOpen(RA_CREATE | (CParserBin::m_bEndians ? RA_ENDIANS : 0), &gRenDev->m_cEF.m_ResLookupDataMan[static_cast<i32>(GetType())]);
		...
		{
			DRX_ASSERT_MESSAGE(pLookupCache == NULL, "Losing ShaderIdents by recreating lookupdata cache");
		...
		for (i = 0; i < Data.size(); i++)
		{
			...
			else
			{
				...
				if (pD->pData)
				{
					...
					u32 nSize = pD->nSizeDecomp;
					memcpy(pData, &nSize, sizeof(u32));
					memcpy(pData+sizeof(u32), pD->pData, pD->nSizeComp);
					pOE->pData = pData;
					SAFE_DELETE_ARRAY(pD->pData);
				}
			}
...
i32 __cdecl sSort(const VOID* arg1, const VOID* arg2)
{
...
byte* CHWShader_D3D::mfBindsToCache(SHWSInstance* pInst, std::vector<SCGBind>* Binds, i32 nParams, byte* pP)
{
	i32 i;
	for (i = 0; i < nParams; i++)
	{
		...
		i32 len = strlen(cgb->m_Name.c_str()) + 1;
...
const byte* CHWShader_D3D::mfBindsFromCache(std::vector<SCGBind>& Binds, i32 nParams, const byte* pP)
{
...
byte* CHWShader::mfIgnoreBindsFromCache(i32 nParams, byte* pP)
{
	i32 i;
	for (i = 0; i < nParams; i++)
	{
...
bool CHWShader_D3D::mfUploadHW(SHWSInstance* pInst, const byte* pBuf, u32 nSize, CShader* pSH, u32 nFlags)
{
	PROFILE_FRAME(Shader_mfUploadHW);

	tukk sHwShaderName = _HELP("Vertex Shader");
	if (m_eSHClass == eHWSC_Pixel)
	...
	DRX_ASSERT_MESSAGE(pInst->m_Handle.m_pShader && pInst->m_Handle.m_pShader->GetHandle(), "Shader creation failed");
	...
	if (pInst->m_Handle.m_pShader->GetHandle())
	{
		...
		char name[1024];
		drx_sprintf(name, "%s_%s(LT%x)@(RT%llx)(MD%x)(MDV%x)(GL%llx)(PSS%llx)", pSH->GetName(), m_EntryFunc.c_str(), pInst->m_Ident.m_LightMask, pInst->m_Ident.m_RTMask, pInst->m_Ident.m_MDMask, pInst->m_Ident.m_MDVMask, pInst->m_Ident.m_GLMask, pInst->m_Ident.m_pipelineState.opaque);
		((CDrxDXOrbisShader*)pInst->m_Handle.m_pShader->GetHandle())->RegisterWithGPUDebugger(name);
	#endif

		// Assign name to Shader for enhanced debugging
	#if !defined(RELEASE) && (DRX_PLATFORM_WINDOWS || DRX_PLATFORM_ORBIS)
		char name[1024];
		sprintf(name, "%s_%s(LT%x)@(RT%llx)(MD%x)(MDV%x)(GL%llx)(PSS%llx)", pSH->GetName(), m_EntryFunc.c_str(), pInst->m_Ident.m_LightMask, pInst->m_Ident.m_RTMask, pInst->m_Ident.m_MDMask, pInst->m_Ident.m_MDVMask, pInst->m_Ident.m_GLMask, pInst->m_Ident.m_pipelineState.opaque);
				...
				reinterpret_cast<NDrxVulkan::CShader*>(pInst->m_Handle.m_pShader->GetHandle())->DebugSetName(name);
			#endif
		#elif DRX_PLATFORM_ORBIS
					((CDrxDXOrbisShader*)pInst->m_Handle.m_pShader->GetHandle())->DebugSetName(name);
		#endif
	#endif
	}
...
bool CHWShader_D3D::mfUploadHW(D3DBlob* pShader, SHWSInstance* pInst, CShader* pSH, u32 nFlags)
{
	bool bResult = true;
	...
	if (pShader)
	{
		...
		{
			bResult = mfUploadHW(pInst, (byte*)pCode, (u32)pShader->GetBufferSize(), pSH, nFlags);
			if (m_eSHClass == eHWSC_Vertex)
			{
				size_t nSize = pShader->GetBufferSize();
...
bool CHWShader_D3D::mfActivateCacheItem(CShader* pSH, const SDeviceShaderEntry* cacheEntry, u32 nFlags)
{
	bool bResult = true;
...
bool CHWShader_D3D::mfCreateCacheItem(SHWSInstance* pInst, CShader *ef, std::vector<SCGBind>& InstBinds, byte* pData, i32 nLen, bool bShaderThread)
{
	auto cache = AcquireDiskCache(cacheSource::user);
	if (!cache->m_pRes)
		return false;
	...
	i32 nNewSize = (h.m_nInstBinds) * sizeof(SShaderCacheHeaderItemVar) + nLen;
	...
	CDrxNameTSCRC nm = CDrxNameTSCRC(name);
	bool bRes = mfAddCacheItem(cache, &h, pNewData, (i32)(pP - pNewData), false, nm);
...
void CHWShader_D3D::mfSaveCGFile(tukk scr, tukk path)
{
	...
	{
#if defined(__GNUC__)
		drx_sprintf(name, "%s/%s(LT%x)@(RT%llx)(MD%x)(MDV%x)(GL%llx)(PSS%llx).cg", path, GetName(), m_pCurInst->m_Ident.m_LightMask, m_pCurInst->m_Ident.m_RTMask, m_pCurInst->m_Ident.m_MDMask, m_pCurInst->m_Ident.m_MDVMask, m_pCurInst->m_Ident.m_GLMask, m_pCurInst->m_Ident.m_pipelineState.opaque);
#else
		drx_sprintf(name, "%s/%s(LT%x)/(RT%I64x)(MD%x)(MDV%x)(GL%I64x)(PSS%llx).cg", path, GetName(), m_pCurInst->m_Ident.m_LightMask, m_pCurInst->m_Ident.m_RTMask, m_pCurInst->m_Ident.m_MDMask, m_pCurInst->m_Ident.m_MDVMask, m_pCurInst->m_Ident.m_GLMask, m_pCurInst->m_Ident.m_pipelineState.opaque);
#endif
	}
	else
	{
#if defined(__GNUC__)
		drx_sprintf(name, "Shaders/Cache/D3D10/fxerror/%s(GL%llx)@(LT%x)(RT%llx)@(MD%x)(MDV%x)(PSS%llx).cg", GetName(), m_pCurInst->m_Ident.m_GLMask, m_pCurInst->m_Ident.m_LightMask, m_pCurInst->m_Ident.m_RTMask, m_pCurInst->m_Ident.m_MDMask, m_pCurInst->m_Ident.m_MDVMask, m_pCurInst->m_Ident.m_pipelineState.opaque);
#else
		drx_sprintf(name, "FXError/%s(GL%I64x)/(LT%x)(RT%I64x)/(MD%x)(MDV%x)(PSS%llx).cg", GetName(), m_pCurInst->m_Ident.m_GLMask, m_pCurInst->m_Ident.m_LightMask, m_pCurInst->m_Ident.m_RTMask, m_pCurInst->m_Ident.m_MDMask, m_pCurInst->m_Ident.m_MDVMask, m_pCurInst->m_Ident.m_pipelineState.opaque);
#endif
	}

	FILE* fp;

	fp = gEnv->pDrxPak->FOpen(name, "w");

	if (fp)
	{
		size_t len = strlen(scr);
		gEnv->pDrxPak->FWrite(scr, len, fp);
		gEnv->pDrxPak->FClose(fp);
	}
}

void CHWShader_D3D::mfOutputCompilerError(string& strErr, tukk szSrc)
{
		...
		if (pSH)
		{
			...
			if (pInst)
			{
				if (pInst->m_pAsync != pAI)
					DrxFatalError("Shader instance async info doesn't match queued async info.");
...
void CHWShader::mfFlushPendedShadersWait(i32 nMaxAllowed)
{
...
Start:
		while (true)
		{
			if (SShaderAsyncInfo::s_nPendingAsyncShaders <= 0)
				break;
			i32 n = (i32)iTimer->GetAsyncCurTime();
			if (!(n % 2))
				iLog->Update();
			if (!(n % 8))
			{
...
i32 CHWShader_D3D::mfAsyncCompileReady(SHWSInstance* pInst)
{
	//SHWSInstance *pInst = m_pCurInst;
	//assert(pInst->m_pAsync);
	if (!pInst->m_pAsync)
	...
	CShader* pSH = pAsync->m_pFXShader;
	{
		...
		gEnv->pDrxPak->AdjustFileName(nmDst, nameSrc, 0);
...
bool CHWShader_D3D::mfRequestAsync(CShader* pSH, SHWSInstance* pInst, std::vector<SCGBind>& InstBindVars, tukk prog_text, tukk szProfile, tukk szEntry)
{
#ifdef SHADER_ASYNC_COMPILATION
	...
	// Generate request line text to store on the shaderlist for next shader cache gen
	{
		...
		drx_strcpy(szShaderGenName, GetName());
		tuk s = strchr(szShaderGenName, '(');
		if (s)
			s[0] = 0;
		string RequestLine;
		SShaderCombIdent Ident = pInst->m_Ident;
...
tukk GetShaderlistName(u32 nPlatform)
{
	...
	default:
		DrxFatalError("Unexpected Shader Platform/No platform specified");
...
bool CHWShader_D3D::mfCompileHLSL_Int(CShader* pSH, tuk prog_text, D3DBlob** ppShader, uk * ppConstantTable, D3DBlob** ppErrorMsgs, string& strErr, std::vector<SCGBind>& InstBindVars)
{
	HRESULT hr = S_OK;
	SHWSInstance* pInst = m_pCurInst;
	tukk szProfile = mfProfileString(pInst->m_eClass);
	tukk pFunCDrxName = m_EntryFunc.c_str();
	...
	{
		DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_WARNING,"Cannot compile %s(LT%x)/(RT%I64x)(MD%x)(MDV%x)(GL%I64x)(PSS%llx).cg. Resource layout missing!",
	...
	if (CRenderer::CV_r_shadersasynccompiling && !(m_Flags & HWSG_SYNC))
	{
		return mfRequestAsync(pSH, pInst, InstBindVars, prog_text, szProfile, pFunCDrxName);
	}
	else if (CRenderer::CV_r_shadersremotecompiler)
	{
		...
		std::vector<u8> Data;
		if (NRemoteCompiler::ESOK != NRemoteCompiler::CShaderSrv::Instance().Compile(Data, szProfile, prog_text, pFunCDrxName, sCompiler.c_str(), RequestLine.c_str()))
		{
			string sErrorText;
		...
		*ppShader = (D3DBlob*) pShader;
		...
		if (bReflect)
		{
			...
			{
				ID3D11ShaderReflection* pShaderReflection = (ID3D11ShaderReflection*)pShaderReflBuf;
				*ppConstantTable = (uk )pShaderReflection;
			}
			else
			{
				assert(0);
	...
	else
	{
		...
		#endif
		u32 nFlags = D3D10_SHADER_PACK_MATRIX_ROW_MAJOR | D3D10_SHADER_ENABLE_BACKWARDS_COMPATIBILITY;
		if (CRenderer::CV_r_shadersdebug == 3 || CRenderer::CV_r_shadersdebug == 4)
			nFlags |= D3D10_SHADER_DEBUG | D3D10_SHADER_SKIP_OPTIMIZATION;

		hr = D3DCompile(prog_text, strlen(prog_text), GetName(), NULL, NULL, pFunCDrxName, szProfile, nFlags, 0, (ID3DBlob**) ppShader, (ID3DBlob**) ppErrorMsgs);
		if (FAILED(hr) || !*ppShader)
		{
			if (ppErrorMsgs && *ppErrorMsgs)
			{
				tukk err = (tukk)ppErrorMsgs[0]->GetBufferPointer();
				strErr += err;
			}
			else
			{
		...
		else
		{
			...
			UINT nSize = (u32)ppShader[0]->GetBufferSize();
...
#endif // #if DRX_PLATFORM_WINDOWS || DRX_PLATFORM_DURANGO

	return false;
}

D3DBlob* CHWShader_D3D::mfCompileHLSL(CShader* pSH, tuk prog_text, uk * ppConstantTable, D3DBlob** ppErrorMsgs, u32 nFlags, std::vector<SCGBind>& InstBindVars)
{
	//	LOADING_TIME_PROFILE_SECTION(iSystem);
...
void CHWShader_D3D::mfPrepareShaderDebugInfo(SHWSInstance* pInst, tukk szAsm, std::vector<SCGBind>& InstBindVars, uk pConstantTable)
{
	if (szAsm)
	{
		tuk szInst = strstr((tuk)szAsm, "pproximately ");
		if (szInst)
	...
	if (CRenderer::CV_r_shadersdebug)
	{
...

		{
			...
			statusdst = gEnv->pDrxPak->FOpen(szName.c_str(), "wb");
		}

		if (statusdst)
		{
			gEnv->pDrxPak->FPrintf(statusdst, "\n// %s %s\n\n", "%STARTSHADER", mfProfileString(pInst->m_eClass));
			if (m_eSHClass == eHWSC_Vertex)
			{
				for (u32 i = 0; i < (u32)InstBindVars.size(); i++)
				{
					SCGBind* pBind = &InstBindVars[i];
					gEnv->pDrxPak->FPrintf(statusdst, "//   %s %s %d %d\n", "%%", pBind->m_Name.c_str(), pBind->m_nParameters, pBind->m_dwBind);
				}
			}
			gEnv->pDrxPak->FPrintf(statusdst, "%s", szAsm);
			gEnv->pDrxPak->FPrintf(statusdst, "\n// %s\n", "%ENDSHADER");
			gEnv->pDrxPak->FClose(statusdst);
		}
		pInst->m_Handle = {};
	}
}
...
{
	i32 nConsts = 0;
	i32 nParams = pInst->m_pBindVars.size();
	for (i32 i = 0; i < nParams; i++)
	{
		SCGBind* pB = &pInst->m_pBindVars[i];
	...
	char szGenName[512];
	drx_strcpy(szGenName, GetName());
	tuk s = strchr(szGenName, '(');
	if (s)
		s[0] = 0;
	if (CRenderer::CV_r_shadersdebug == 2)
	{
		...
		DrxLog(" Compile (FX:0x%I64x, GL:0x%I64x) %s (%d instructions, %d tempregs, %d/%d constants) ... ", m_nMaskGenFX, pInst->m_Ident.m_GLMask, pName.c_str(), pInst->m_nInstructions, pInst->m_nTempRegs, nParams, nConsts);
		i32 nSize = strlen(szGenName);
		mfGenName(pInst, &szGenName[nSize], 512 - nSize, 1);
		DrxLog("           --- Cache entry: %s", szGenName);
	}
	else
	{
		i32 nSize = strlen(szGenName);
		mfGenName(pInst, &szGenName[nSize], 512 - nSize, 1);
		DrxLog(" Compile %s (%d instructions, %d tempregs, %d/%d constants) ... ", szGenName, pInst->m_nInstructions, pInst->m_nTempRegs, nParams, nConsts);
	}
...
bool CHWShader_D3D::mfCreateShaderEnv(i32 nThread, SHWSInstance* pInst, D3DBlob* pShader, uk & pConstantTable, D3DBlob*& pErrorMsgs, std::vector<SCGBind>& InstBindVars, CHWShader_D3D* pSH, bool bShaderThread, CShader* pFXShader, i32 nCombination, tukk src)
{
	...
	if (pShader)
		pSH->mfCreateCacheItem(pInst, pFXShader, InstBindVars, (byte*)pShader->GetBufferPointer(), (u32)pShader->GetBufferSize(), bShaderThread);
	else
...
bool CHWShader_D3D::mfActivate(CShader* pSH, u32 nFlags)
{
	...
	if (mfIsValid(pInst, true) == ED3DShError_NotCompiled)
	{
		...
		pInst->m_DeviceObjectID = CDrxNameTSCRC{ name }.get();
		...
		if (it == devCache.end())
		{
			...
			for (const auto &cacheType : cacheTypes)
			{
				...
				if (newEntry)
				{
					DrxLog("CHWShader_D3D::mfActivate(): Shader '%s' not found in device cache but found in disk cache.", m_Name.c_str());
...
void CAsyncShaderTask::InsertPendingShader(SShaderAsyncInfo* pAsync)
{
	...
	DrxInterlockedIncrement(&SShaderAsyncInfo::s_nPendingAsyncShaders);
}
...
void CAsyncShaderTask::FlushPendingShaders()
{
	...
	for (pAI = m_flush_list.m_Next; pAI != &m_flush_list; pAI = pAINext)
	{
		...
		DrxInterlockedDecrement(&SShaderAsyncInfo::s_nPendingAsyncShaders);
		{
			AUTO_LOCK(g_cAILock);
...
bool CAsyncShaderTask::CompileAsyncShader(SShaderAsyncInfo* pAsync)
{
	...
	if (CRenderer::CV_r_shadersremotecompiler)
	{
		...
		if (bReflect)
		{
			...
			hr = D3DReflect(pBuf, nSize, IID_ID3D11ShaderReflection, (uk *)&pShaderReflection);
			if (SUCCEEDED(hr))
			{
				pAsync->m_pConstants = (uk )pShaderReflection;
			}
		}
	...
	else
	{
		...
		tukk Name = pAsync->m_pShader ? pAsync->m_pShader->GetName() : "Unknown";
		HRESULT hr = S_OK;
		...
		if (FAILED(hr) || !pAsync->m_pDevShader)
		{
			if (pAsync->m_pErrors)
			{
				tukk err = (tukk)pAsync->m_pErrors->GetBufferPointer();
				pAsync->m_Errors += err;
			}
			else
			{
		...
		else
		{
			...
			hr = D3DReflect(pData, nSize, IID_ID3D11ShaderReflection, (uk *)&pShaderReflection);
			if (SUCCEEDED(hr))
			{
				pAsync->m_pConstants = (uk )pShaderReflection;
				bResult = PostCompile(pAsync);
...
bool CHWShader_D3D::Export(CShader *pSH, SShaderSerializeContext& SC)
{
	...
	if (bOutputTokens)
	{
		...
		for (const auto &T : Table)
		{
			...
			u32 tokenStrIdx = SC.AddString(T.SToken.c_str());
...
CHWShader* CHWShader::Import(SShaderSerializeContext& SC, i32 nOffs, u32 CRC32, CShader* pSH)
{
	if (nOffs < 0)
		return NULL;
	...
	tukk szName = sString(pSHW->m_nsName, SC.Strings);
	tukk szNameSource = sString(pSHW->m_nsNameSourceFX, SC.Strings);
	tukk szNameEntry = sString(pSHW->m_nsEntryFunc, SC.Strings);

	TArray<u32> SHData;
	SHData.resize(pSHW->m_nTokens);
	memcpy(&SHData[0], pData, pSHW->m_nTokens * sizeof(u32));
...
tukk CHWShader_D3D::mfGetActivatedCombinations(bool bForLevel)
{
...
tukk CHWShader::GetCurrentShaderCombinations(bool bForLevel) threadsafe
{
	...
	if (pRL)
	{
		...
		for (itor = pRL->m_RMap.begin(); itor != pRL->m_RMap.end(); itor++)
		{
			...
			if (!vs)
				continue;
			tukk szCombs = vs->mfGetActivatedCombinations(bForLevel);
			if (!szCombs)
				continue;
	...
	if (pRL)
	{
		...
		for (itor = pRL->m_RMap.begin(); itor != pRL->m_RMap.end(); itor++)
		{
			...
			if (!ps)
				continue;
			tukk szCombs = ps->mfGetActivatedCombinations(bForLevel);
			if (!szCombs)
				continue;
			Combinations.Copy(szCombs, strlen(szCombs));
			delete[] szCombs;
