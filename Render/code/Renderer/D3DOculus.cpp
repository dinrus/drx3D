...
#include <drx3D/Render/D3DOculus.h>
...
#include <DinrusXSys/VR/IHMDDevice.h>
	...
	#include <IBenchmarkRendererSensorUpr.h>
		...
		#include <drx3D/Render/DX12/Resource/Texture/CDrxDX12Texture2D.hpp>
			...
			#include <drx3D/Render/DX12/API/Workarounds/OculusMultiGPU.inl"
		#endif
	#endif

CD3DOculusRenderer::SLayersUpr::SLayersUpr()
{
	...
	m_swapChainInfoEyeLeft.eye = DrxVR::Oculus::SHmdSwapChainInfo::eye_t::left;
	...
	m_swapChainInfoEyeRight.eye = DrxVR::Oculus::SHmdSwapChainInfo::eye_t::right;
	...
	for (u32 i = RenderLayer::eSceneLayers_0; i < RenderLayer::eSceneLayers_Total; ++i)
	{
...
void CD3DOculusRenderer::SLayersUpr::UpdateSwapChainData(CD3DStereoRenderer* pStereoRenderer, const STextureSwapChainRenderData* scene3DRenderData, const STextureSwapChainRenderData* quadRenderData)
{
...
DrxVR::Oculus::SHmdSubmitFrameData CD3DOculusRenderer::SLayersUpr::ConstructFrameData()
{
	DrxVR::Oculus::SHmdSubmitFrameData data;
...
CD3DOculusRenderer::CD3DOculusRenderer(DrxVR::Oculus::IOculusDevice* oculusDevice, CD3D9Renderer* renderer, CD3DStereoRenderer* stereoRenderer)
...
bool CD3DOculusRenderer::Initialize(i32 initialWidth, i32 initialeight)
{
	...
	DrxVR::Oculus::TextureDesc eyeTextureDesc;
	eyeTextureDesc.width = m_eyeWidth;
	eyeTextureDesc.height = m_eyeHeight;
	eyeTextureDesc.format = (u32)DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;

	DrxVR::Oculus::TextureDesc quadTextureDesc;
	quadTextureDesc.width = m_eyeWidth;
	quadTextureDesc.height = m_eyeHeight;
	quadTextureDesc.format = (u32)DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;

	DrxVR::Oculus::TextureDesc mirrorTextureDesc;
	mirrorTextureDesc.width = m_eyeWidth * 2;
	mirrorTextureDesc.height = m_eyeHeight;
	mirrorTextureDesc.format = (u32)DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;
	...
	for (u32 quad = 0; quad < RenderLayer::eQuadLayers_Total; ++quad)
	{
		std::vector<_smart_ptr<CTexture>> swapChain;
...
bool CD3DOculusRenderer::InitializeTextureSwapSet(ID3D11Device* pD3d11Device, EEyeType eye, STextureSwapChainRenderData& eyeRenderData, DrxVR::Oculus::TextureDesc desc, const STxt& name)
{
#if (DRX_RENDERER_DIRECT3D >= 120)
	...
	u32 numTextures = eyeRenderData.vrTextureSet.numTextures;
	if (numTextures == 0)
	{
	...
	for (u32 t = 0; t < numTextures; ++t)
	{
		...
		if (m_pRenderer->GetDevice().GetNodeCount() == 1)
		{
			...
			#if (DRX_RENDERER_DIRECT3D >= 120)
			D3DTexture* pTex = CDrxDX12Texture2D::Create((CDrxDX12Device*)pD3d11Device, nullptr, static_cast<ID3D12Resource*>(eyeRenderData.vrTextureSet.pTextures[t]));
			#else
...
bool CD3DOculusRenderer::InitializeTextureSwapSet(ID3D11Device* pD3d11Device, EEyeType eye, DrxVR::Oculus::TextureDesc desc, const STxt& name)
{
	return InitializeTextureSwapSet(pD3d11Device, eye, m_scene3DRenderData[eye], desc, name);
}

bool CD3DOculusRenderer::InitializeQuadTextureSwapSet(ID3D11Device* d3dDevice, RenderLayer::EQuadLayers id, DrxVR::Oculus::TextureDesc desc, const STxt& name)
{
	return InitializeTextureSwapSet(d3dDevice, static_cast<EEyeType>(-1), m_quadLayerRenderData[id], desc, name);
}

bool CD3DOculusRenderer::InitializeMirrorTexture(ID3D11Device* pD3d11Device, DrxVR::Oculus::TextureDesc desc, const STxt& name)
{
	#if (DRX_RENDERER_DIRECT3D >= 120)
	ID3D12CommandQueue* pD3d12Queue = GetDeviceObjectFactory().GetNativeCoreCommandQueue();
	...
	{
		#if (DRX_RENDERER_DIRECT3D >= 120)
		D3DTexture* pTex = CDrxDX12Texture2D::Create((CDrxDX12Device*)pD3d11Device, nullptr, static_cast<ID3D12Resource*>(m_mirrorData.vrMirrorTexture.pTexture));
		#else
...
void CD3DOculusRenderer::OnResolutionChanged(i32 newWidth, i32 newHeight)
{
...
void CD3DOculusRenderer::PrepareFrame(uint64_t frameId)
{
	...
	if (m_pOculusDevice->BeginFrame(frameId) == DrxVR::Oculus::OculusStatus::DeviceLost)
		m_deviceLostFlag = true;
}
...
void CD3DOculusRenderer::SubmitFrame()
{
	...
	m_layerUpr.UpdateSwapChainData(m_pStereoRenderer, m_scene3DRenderData, m_quadLayerRenderData);
	DrxVR::Oculus::SHmdSubmitFrameData data = m_layerUpr.ConstructFrameData();
...
#endif
	{
		...
		CDrxDX12RenderTargetView* lRV = (CDrxDX12RenderTargetView*)leftDc->GetCurrentBackBuffer()->GetSurface(-1, 0);
		CDrxDX12RenderTargetView* rRV = (CDrxDX12RenderTargetView*)rightDc->GetCurrentBackBuffer()->GetSurface(-1, 0);
		...
		for (u32 i = 0; i < RenderLayer::eQuadLayers_Total; ++i)
		{
			i32k idx = m_pOculusDevice->GetCurrentSwapChainIndex(m_quadLayerRenderData[i].vrTextureSet.pDeviceTextureSwapChain);
			if (m_quadLayerRenderData[i].textures[idx])
			{
				CDrxDX12RenderTargetView* qRV = (CDrxDX12RenderTargetView*)m_quadLayerRenderData[i].textures[idx]->GetSurface(-1, 0);
				if (qRV)
				{
	...
	if (m_pOculusDevice->SubmitFrame(data) == DrxVR::Oculus::OculusStatus::DeviceLost)
		m_deviceLostFlag = true;
...
RenderLayer::CProperties* CD3DOculusRenderer::GetQuadLayerProperties(RenderLayer::EQuadLayers id)
{
	...
	{
		return &(m_layerUpr.m_quadLayerProperties[id]);
	}
	return nullptr;
}
...
RenderLayer::CProperties* CD3DOculusRenderer::GetSceneLayerProperties(RenderLayer::ESceneLayers id)
{
	if (id < RenderLayer::eQuadLayers_Total)
	{
		return &(m_layerUpr.m_scene3DLayerProperties[id]);
	}
	return nullptr;
}
