...
static i32 snSPInfo = 0;
static i32 snSPInfoMax = -1;

static float sfTableTCFloat[129];
static i32 snAtlasSize = 0;
...
void sSetViewPort(i32 nCurX, i32 nCurY, i32 nSpriteRes)
{
...
void sFlushSprites(SSpriteGenInfo* pSGI, i32 nCurX, i32 nCurY, CTexture* pSrcTex, i32 nTexSizeInt, i32 nTexHeightFinal)
{
	...
	while (true)
	{
		...
		/*if ((i32)pSP->m_vPos[0] == 1049 && (i32)pSP->m_vPos[1] == 2069 && (i32)pSP->m_vPos[2] == 248)
		   {
		   i32 nnn = 0;
		   }*/
		...
		*pSG->ppTexture = pNewTex;
		...
		pSP->m_pTex = pNewTex;
		u32k dwWidth = pT->GetWidth();
		u32k dwHeight = pT->GetHeight();
		...
		v[1] = fHalfY;
		v[2] = fSizeX;
		static CDrxNameR vPixelOffsetName("vPixelOffset");
...
static void sRT_AddEf(CRenderElement* pRE, SShaderItem& SH, CRenderObject* pObj, i32 nList, i32 nAW, const SRenderingPassInfo& passInfo)
{
	...
	u32 nBatchFlags = rd->EF_BatchFlags(SH, pObj, pRE, passInfo, nAW);
...
{
	CD3D9Renderer* rd = gcpRendD3D;
	i32 nThreadID = rd->m_RP.m_nProcessThreadID;
...
void CD3D9Renderer::MakeSprites(TArray<SSpriteGenInfo>& SGI, const SRenderingPassInfo& passInfoOriginal)
{
	...
	if (!CTexture::IsTextureExist(pTempTex))
		return;
	i32 nWidth = GetWidth();
	i32 nHeight = GetHeight();
	assert(pTempTex && nWidth == pTempTex->GetWidth() && nHeight == pTempTex->GetHeight());
	i32 nSpriteResFinal = CV_r_VegetationSpritesTexRes;
	i32 nSpriteResInt = nSpriteResFinal << 1;
	i32 nSprX = nWidth / nSpriteResInt;
	i32 nSprY = nHeight / nSpriteResInt;
	i32 nCurX = 0;
	i32 nCurY = 0;
	...
	i32 nTech = m_RP.m_nShaderTechnique;
	...
	//i32 nPrevStr = CV_r_texturesstreamingsync;
	i32 nPrevAsync = CV_r_shadersasynccompiling;
	//CV_r_texturesstreamingsync = 1;
	CV_r_shadersasynccompiling = 0;
	i32 nPrevZpass = CV_r_usezpass;
	CV_r_usezpass = 0;
	...
	i32 vX, vY, vWidth, vHeight;
	GetViewport(&vX, &vY, &vWidth, &vHeight);

	CCamera origCam = GetCamera();
	u32 saveFlags = m_RP.m_TI[nThreadID].m_PersFlags;
	u32 saveFlags2 = m_RP.m_PersFlags2;
...
void CD3D9Renderer::GenerateObjSprites(PodArray<struct SVegetationSpriteInfo>* pList, const SRenderingPassInfo& passInfo)
{
	...
	u32k dwSunSkyRel = (u32)(256.0f * pEngine->GetSunRel());
	assert(dwSunSkyRel <= 256);                                                                     // 0..256
	u32k dwSkySunRel = 256 - dwSunSkyRel;
	...
	i32 nPolygonMode = CV_r_wireframe;
	i32 nZPass = CV_r_usezpass;
#ifndef CONSOLE_CONST_CVAR_MODE
	CV_r_wireframe = 0;
#endif
	CV_r_usezpass = 0;

	static u32 s_dwOldMakeSpriteCalls = 0;

	i32 e_debug_mask = 0;//pVarDebugMask->GetIVal();
	...
	i32 nTexHeightFinal = CV_r_VegetationSpritesTexRes;
	i32 nSizeSprX = nTexHeightFinal;
#ifdef SPRITES_2_TEXTURES
	nSizeSprX <<= 1;
#endif
	...
	for (i = 0; i < pList->Count(); i++)
	{
		...
		if (!bLightInfoIsUpToDate || !bStreamingStateIsUpToDate || !pTex || !pTex->_IsValid() || (pTex->m_nHeight != nTexHeightFinal) || CV_r_VegetationSpritesGenAlways)
		{
			...
			{
				i32 nReadyLod = vi.pStatInstGroup->pStatObj->FindHighestLOD(0);
				if (nReadyLod >= 0)
			...
			if (!pTex || (pTex->m_nHeight != nTexHeightFinal))
			{
...
#ifdef _DEBUG
				char Name[128];
				drx_sprintf(Name, "$SpriteTex_(%1.f %1.f %1.f)_%d", pSP->m_vPos.x, pSP->m_vPos.y, pSP->m_vPos.z, vi.ucSlotId);
				szName = Name;
#else
				szName = "$SpriteTex";
#endif
				...
				i32 nTexX = nTexHeightFinal / nDownScaleX;
#ifdef SPRITES_2_TEXTURES
				nTexX <<= 1;
#endif
			...
			{
				tukk pGroupFilePath = vi.pStatInstGroup->szFileName;
...
void CD3D9Renderer::ObjSpritesFlush(SVF_P3F_C4B_T2F* pVerts, u16* pInds, i32 nSprites, uk & pCurVB, SShaderTechnique* pTech, bool bZ)
{
	assert(pVerts && pInds);
	assert(pTech);
	if (!pTech)
	...
	for (u32 i = 0; i < pTech->m_Passes.Num(); i++, pPass++)
	{
...
void CD3D9Renderer::DrawObjSprites(PodArray<SVegetationSpriteInfo>* pList, SSpriteInfo* pSPInfo, i32k nSPI, bool bZ, bool bShadows)
{
	u32 i;
	...
	if (bShadows)
	{
		i32 nColorMask = ~(1 << GS_COLMASK_SHIFT) & GS_COLMASK_MASK;
	...
	static CDrxNameR TexAtlasSizeName("TexAtlasSize");
	...
	i32k nMemV = (i32)(128.0f * 1024.0f * (1 - fFracMem));
	i32k nMemI = (i32)(128.0f * 1024.0f * fFracMem);
	assert(nMemI + nMemV <= 128 * 1024);
	i32k nMaxVerts = nMemV / sizeof(SVF_P3F_C4B_T2F);
	i32k nMaxSprites = nMaxVerts / 4;
	i32k nMaxInds = nMemI / sizeof(short);
	if (!m_pSpriteVerts)
	{
		byte* pMem = (byte*)DrxModuleMemalign(128 * 1028, 128);
		m_pSpriteVerts = (SVF_P3F_C4B_T2F*)pMem;
		m_pSpriteInds = (u16*)(pMem + nMemV + 256);
	}
	...
	for (i = 0; (i32)i < nSPI; i++)
	{
		...
		if (pPrevTex != pTex || pPrevTerrainTexInfo != pSP->m_pTerrainTexInfo)
		{
			...
			if (!bShadows)
			{
				...
				if (m_RP.m_nPassGroupID == EFSLIST_GENERAL && pSP->m_pTerrainTexInfo)
				{
					...
					static CDrxNameR SpritesOutdoorAOVertInfoName("SpritesOutdoorAOVertInfo");
		...
		// hash up this cell
		u32 cellHash = 0;
		cellHash |= ((u32)(TRUNCATE_PTR)pSP->m_pTex) << 16; // store texture low-end 16 bits, enough to give us uniquely which texture we're using.
		cellHash |= ((u32)pSP->m_ucTexCoordMinX) << 8;      // store min texture co-ords
		cellHash |= ((u32)pSP->m_ucTexCoordMinY);
		...
		i32 nIOffs = nO * 6;
		u16* __restrict pInds = &m_pSpriteInds[nIOffs];
		pInds[0] = nOffs;
		pInds[1] = nOffs + 1;
		pInds[3] = nOffs + 1;
		pInds[2] = nOffs + 2;
...
void CD3D9Renderer::DrawObjSprites(PodArray<SVegetationSpriteInfo>* pList)
{
	...
	if (m_RP.m_nBatchFilter & FB_Z)
	{
		...
		{
			static CDrxNameTSCRC TechName("General_Z");
