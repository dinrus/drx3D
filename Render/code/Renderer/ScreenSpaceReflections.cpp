...
void CScreenSpaceReflectionsStage::Execute()
{
	...
	i32k frameID = SPostEffectsUtils::m_iFrameCounter;
	Matrix44 mViewport(0.5f, 0, 0, 0,
	                   0, -0.5f, 0, 0,
	                   0, 0, 1.0f, 0,
	                   0.5f, 0.5f, 0, 1.0f);
	u32 numGPUs = rd->GetActiveGPUCount();
	Matrix44 mViewProjPrev = m_prevViewProj[max((frameID - (i32)numGPUs) % MAX_GPU_NUM, 0)] * mViewport;
...

	{
		...
		{
			static CDrxNameTSCRC techRaytrace("SSR_Raytrace");
		...
		static CDrxNameR viewProjprevName("g_mViewProjPrev");
		static CDrxNameR ssrParamsName("g_mSSRParams"); // we need to tell the shader to read from a depth buffer with twice the size of the output in halfres mode
		Vec4 ssrParams(
...

	{
		...
		{
			static CDrxNameTSCRC techComposition("SSReflection_Comp");
