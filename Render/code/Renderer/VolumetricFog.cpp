...
#include <drx3D/Render/VolumetricFog.h>
...
{
	u32   miscFlag; // 1: volumeType, 8: stencilRef, 1: affectThisAreaOnly, 22: reserved
	Vec3     fogColor;
...
u32 vfGetDownscaledShadowMapSize(u32& shadowMapTempSize, const ICVar* pCVarShadowMaxTexRes)
{
	u32 shadowMapSize = 0;
	u32 tempSize = 0;
...
i32 vfGetShadowCascadeNum()
{
	...
	i32 num = (CRenderer::CV_r_VolumetricFogDownscaledSunShadow == 1) ?
...
i32 CVolumetricFogStage::GetVolumeTextureDepthSize()
{
	i32 d = CRenderer::CV_r_VolumetricFogTexDepth;
	d = (d < 4) ? 4 : d;
...
i32 CVolumetricFogStage::GetVolumeTextureSize(i32 size, i32 scale)
{
	scale = max(scale, 2);
...
void CVolumetricFogStage::Init()
{
	if (!m_lightCullInfoBuf.IsAvailable())
	{
		...
		u32k stride = sizeof(CTiledLightVolumesStage::STiledLightCullInfo);
	...
	{
		u32k stride = sizeof(CTiledLightVolumesStage::STiledLightShadeInfo);
		const DXGI_FORMAT format = DXGI_FORMAT_UNKNOWN;
	...
	{
		u32k stride = sizeof(SFogVolumeCullInfo);
		const DXGI_FORMAT format = DXGI_FORMAT_UNKNOWN;
	...
	{
		u32k stride = sizeof(SFogVolumeInjectInfo);
		const DXGI_FORMAT format = DXGI_FORMAT_UNKNOWN;
	...
	u32k commonFlags = FT_NOMIPS | FT_DONT_STREAM;
	u32k uavFlags = commonFlags | FT_USAGE_UNORDERED_ACCESS;
	u32k rtFlags = commonFlags | FT_USAGE_RENDERTARGET;
	u32k uavRtFlags = commonFlags | FT_USAGE_UNORDERED_ACCESS | FT_USAGE_RENDERTARGET;
	u32k dsFlags = commonFlags | FT_USAGE_DEPTHSTENCIL;
	...
	DRX_ASSERT(m_pVolFogBufDensityColor == nullptr);
	...
	// create scene render pass and resources.
	{
		...
		DRX_ASSERT(m_pSceneRenderResourceLayout);
...
void CVolumetricFogStage::Update()
{
	...
	i32k renderWidth  = pRenderView->GetRenderResolution()[0];
	i32k renderHeight = pRenderView->GetRenderResolution()[1];

	// size for view frustum aligned volume texture.
	i32k scaledWidth  = GetVolumeTextureSize(renderWidth , CRenderer::CV_r_VolumetricFogTexScale);
	i32k scaledHeight = GetVolumeTextureSize(renderHeight, CRenderer::CV_r_VolumetricFogTexScale);
	i32k depth = GetVolumeTextureDepthSize();
	...
	i32k depthTempWidth = (scaledWidth << 1);
	i32k depthTempHeight = (CRendererResources::s_ptexLinearDepthScaled[0]->GetHeight() >> 1);
	...
	u32 shadowMapTempWidth = 0;
	u32k shadowMapWidth = vfGetDownscaledShadowMapSize(shadowMapTempWidth, pCVarShadowMaxTexRes);
	DRX_ASSERT(CRenderer::CV_r_shadowtexformat >= 0 && CRenderer::CV_r_shadowtexformat < 3);
	const ETEX_Format fmtShadow = (CRenderer::CV_r_shadowtexformat == 1) ? eTF_D16 : eTF_D32F;

	auto createBuffer = [=](CGpuBuffer& buffer, u32 elementNum) -> bool
	{
		u32k tileSizeX = 4;
		u32k tileSizeY = 4;
		u32k tileSizeZ = 4;
		u32k dispatchSizeX = (scaledWidth / tileSizeX) + (scaledWidth % tileSizeX > 0 ? 1 : 0);
		u32k dispatchSizeY = (scaledHeight / tileSizeY) + (scaledHeight % tileSizeY > 0 ? 1 : 0);
		u32k dispatchSizeZ = (depth / tileSizeZ) + (depth % tileSizeZ > 0 ? 1 : 0);
		u32k num = dispatchSizeX * dispatchSizeY * dispatchSizeZ * elementNum;
	...
	auto createTexture3D = [=](CTexture* pTex, i32 flags, ETEX_Format texFormat) -> bool
	{
		bool bCreate = false;
		        ...
		        || pTex->Invalidate(scaledWidth, scaledHeight, texFormat)))
		{
			...
			{
				DrxFatalError("Couldn't allocate texture.");
			}
			else
			{
				bCreate = true;
	...
	auto createTexture2D = [](CTexture* pTex, i32 w, i32 h, i32 flags, ETEX_Format texFormat) -> bool
	{
		bool bCreate = false;
	...
	DRX_ASSERT(m_pVolFogBufDensityColor);
	DRX_ASSERT(m_pVolFogBufDensityColor->GetDstFormat() == fmtDensityColor);
	createTexture3D(m_pVolFogBufDensityColor, uavRtFlags, fmtDensityColor);

	DRX_ASSERT(m_pVolFogBufDensity);
	DRX_ASSERT(m_pVolFogBufDensity->GetDstFormat() == fmtDensity);
	createTexture3D(m_pVolFogBufDensity, uavRtFlags, fmtDensity);

	DRX_ASSERT(m_pVolFogBufEmissive);
	DRX_ASSERT(m_pVolFogBufEmissive->GetDstFormat() == fmtEmissive);
	createTexture3D(m_pVolFogBufEmissive, uavRtFlags, fmtEmissive);

	DRX_ASSERT(m_pInscatteringVolume);
	...
	if (CRenderer::CV_r_VolumetricFogDownscaledSunShadow > 0)
	{
		...
		{
			DRX_ASSERT(m_pDownscaledShadowTemp);
	...
	DRX_ASSERT(CRendererResources::s_ptexVolumetricFog);
...
void CVolumetricFogStage::BindVolumetricFogResources(RenderPassType& pass, i32 startTexSlot, i32 trilinearClampSamplerSlot)
{
	if (startTexSlot >= 0)
	{
...
template
void      CVolumetricFogStage::BindVolumetricFogResources(CFullscreenPass& pass, i32 startTexSlot, i32 trilinearClampSamplerSlot);
template
void      CVolumetricFogStage::BindVolumetricFogResources(CComputeRenderPass& pass, i32 startTexSlot, i32 trilinearClampSamplerSlot);
...
bool CVolumetricFogStage::PreparePerPassResources(bool bOnInit)
{
	...
	// Constant buffers
	{
		if (!bOnInit)
		{
			DrxStackAllocWithSize(SVolFogSceneRenderPassConstantBuffer, cb, CDeviceBufferUpr::AlignBufferSizeForStreaming);

			DRX_ASSERT(m_pSceneRenderPassCB && RenderView());
		...
		{
			pPerPassCB = CDeviceBufferUpr::GetNullConstantBuffer();
			pPerViewCB = CDeviceBufferUpr::GetNullConstantBuffer();
		}
		else
		{
	...
	DRX_ASSERT(!m_sceneRenderPassResources.HasChangedBindPoints()); // Cannot change resource layout after init. It is baked into the shaders
...
void CVolumetricFogStage::ExecuteVolumetricFog(const SScopedComputeCommandList& commandList)
{
	...
	i32k VolumetricFogBlurInscattering = 1;
	if (VolumetricFogBlurInscattering > 0)
	{
		i32 maxBlurCount = VolumetricFogBlurInscattering;
		maxBlurCount = maxBlurCount <= 4 ? maxBlurCount : 4;
		for (i32 count = 0; count < maxBlurCount; ++count)
		{
#ifdef ENABLE_VOLFOG_TEX_FORMAT_RGBA16F
...
u32 CVolumetricFogStage::GetTemporalBufferId() const
{
	...
	for (i32 i = 0; i < cascadeNum; ++i)
	{
		// downscale full resolution to half resolution.
		{
				...
				m_resourceFrameID))
			{
				...
				static CDrxNameTSCRC techName("RenderDownscaledShadowMap");
		...
		if (CRenderer::CV_r_VolumetricFogDownscaledSunShadowRatio != 0)
		{
			...
			if (pass.IsDirty(CRenderer::CV_r_VolumetricFogDownscaledSunShadowRatio,
				...
				m_resourceFrameID))
			{
				static CDrxNameTSCRC shaderName0("DownscaleShadowMap2");
				static CDrxNameTSCRC shaderName1("DownscaleShadowMap4");
...
void CVolumetricFogStage::GenerateLightList()
{
	...
	DrxStackAllocWithSizeVectorCleared(CTiledLightVolumesStage::STiledLightCullInfo, MaxNumTileLights, tileLightsCull, CDeviceBufferUpr::AlignBufferSizeForStreaming);
	DrxStackAllocWithSizeVectorCleared(CTiledLightVolumesStage::STiledLightShadeInfo, MaxNumTileLights, tileLightsShade, CDeviceBufferUpr::AlignBufferSizeForStreaming);

	u32 numTileLights = 0;
	u32 numRenderLights = 0;
	u32 numValidRenderLights = 0;
	...
	for (u32 lightListIdx = 0; lightListIdx < lightArraySize; ++lightListIdx)
	{
		...
		for (u32 lightIdx = 0; itr != stp; ++itr, ++lightIdx)
		{
			...
			if (lightListIdx == 0)
			{
...

				{
					...
					i32 arrayIndex = tiledLights->InsertTexture(pSpecTexture, mipFactor, tiledLights->m_specularProbeAtlas, -1);
					if (arrayIndex < 0)
						continue;  // Skip light
			...
			else
			{
				...
				if (renderLight.m_Flags & DLF_PROJECT)
				{
...

					{
						...
						i32 arrayIndex = tiledLights->InsertTexture(pProjTexture, mipFactor, tiledLights->m_spotTexAtlas, -1);
						if (arrayIndex < 0)
							continue;  // Skip light
				...
				{
					i32 numDLights = pRenderView->GetDynamicLightsCount();
					i32 frustumIdx = lightIdx + numDLights;
					...
					if (!SMFrustums.empty())
					{
						...
						i32 numSides = firstFrustum.bOmniDirectionalShadow ? 6 : 1;
						...
						for (i32 side = 0; side < numSides; ++side)
						{
	...
	const size_t tileLightsCullUploadSize = CDeviceBufferUpr::AlignBufferSizeForStreaming(sizeof(CTiledLightVolumesStage::STiledLightCullInfo) * std::min(MaxNumTileLights, Align(numTileLights, 64) + 64));
	const size_t tileLightsShadeUploadSize = CDeviceBufferUpr::AlignBufferSizeForStreaming(sizeof(CTiledLightVolumesStage::STiledLightShadeInfo) * std::min(MaxNumTileLights, Align(numTileLights, 64) + 64));
...
void CVolumetricFogStage::ExecuteBuildLightListGrid(const SScopedComputeCommandList& commandList)
{
	...
	i32k nScreenWidth = m_pInscatteringVolume->GetWidth();
	i32k nScreenHeight = m_pInscatteringVolume->GetHeight();
	i32k volumeDepth = m_pInscatteringVolume->GetDepth();
	...
	{
		static CDrxNameTSCRC techName("BuildLightListGrid");
		pass.SetTechnique(pShader, techName, 0);
	...
	static CDrxNameR paramScreenSize("ScreenSize");
	...
	static CDrxNameR paramFrustumTL("FrustumTL");
	...
	static CDrxNameR paramFrustumTR("FrustumTR");
	...
	static CDrxNameR paramFrustumBL("FrustumBL");
	...
	u32k tileSizeY = 4;
	u32k tileSizeZ = 4;
	u32k dispatchSizeX = (nScreenWidth / tileSizeX) + (nScreenWidth % tileSizeX > 0 ? 1 : 0);
	u32k dispatchSizeY = (nScreenHeight / tileSizeY) + (nScreenHeight % tileSizeY > 0 ? 1 : 0);
	u32k dispatchSizeZ = (volumeDepth / tileSizeZ) + (volumeDepth % tileSizeZ > 0 ? 1 : 0);

	static CDrxNameR paramDispatchSize("DispatchSize");
...
void CVolumetricFogStage::ExecuteDownscaledDepth(const SScopedComputeCommandList& commandList)
{
	...
	{
		i32k nScreenWidth = m_pMaxDepthTemp->GetWidth();
		i32k nScreenHeight = m_pMaxDepthTemp->GetHeight();
		i32k nSrcTexWidth = CRendererResources::s_ptexLinearDepthScaled[0]->GetWidth();
		i32k nSrcTexHeight = CRendererResources::s_ptexLinearDepthScaled[0]->GetHeight();

		auto& pass = m_passDownscaleDepthHorizontal;

		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName("StoreDownscaledMaxDepthHorizontal");
			pass.SetTechnique(pShader, techName, 0);
		...
		static CDrxNameR paramDispatchParams("maxDepthDispatchParams");
	...
	{
		i32k nScreenWidth = m_pMaxDepth->GetWidth();
		i32k nScreenHeight = m_pMaxDepth->GetHeight();
		i32k nSrcTexWidth = m_pMaxDepthTemp->GetWidth();
		i32k nSrcTexHeight = m_pMaxDepthTemp->GetHeight();

		auto& pass = m_passDownscaleDepthVertical;

		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName("StoreDownscaledMaxDepthVertical");
			pass.SetTechnique(pShader, techName, 0);
...
void CVolumetricFogStage::GenerateFogVolumeList()
{
	...
	// NOTE: Get aligned stack-space (pointer and size aligned to manager's alignment requirement)
	DrxStackAllocWithSizeVector(SFogVolumeCullInfo, CRenderView::MaxFogVolumeNum, cullInfoArray, CDeviceBufferUpr::AlignBufferSizeForStreaming);
	DrxStackAllocWithSizeVector(SFogVolumeInjectInfo, CRenderView::MaxFogVolumeNum, injectInfoArray, CDeviceBufferUpr::AlignBufferSizeForStreaming);

	u32 numFogVol = 0;

	for (u32 type = 0; type < IFogVolumeRenderNode::eFogVolumeType_Count; ++type)
	{
	...
	const size_t cullInfoArrayUploadSize = CDeviceBufferUpr::AlignBufferSizeForStreaming(sizeof(SFogVolumeCullInfo) * m_numFogVolumes);
	const size_t injectInfoArrayUploadSize = CDeviceBufferUpr::AlignBufferSizeForStreaming(sizeof(SFogVolumeInjectInfo) * m_numFogVolumes);
...
void CVolumetricFogStage::ExecuteInjectFogDensity(const SScopedComputeCommandList& commandList)
{
	...
	u32k nScreenWidth = m_pVolFogBufDensity->GetWidth();
	u32k nScreenHeight = m_pVolFogBufDensity->GetHeight();
	u32k volumeDepth = m_pVolFogBufDensity->GetDepth();
	...
	if (pass.IsDirty())
	{
		static CDrxNameTSCRC techName("InjectFogDensity");
		pass.SetTechnique(pShader, techName, 0);
	...
	static CDrxNameR paramInjectExponentialHeightFogParams("InjectFogDensityParams");
...
bool CVolumetricFogStage::ReplaceShadowMapWithStaticShadowMap(CShadowUtils::SShadowCascades& shadowCascades, u32 shadowCascadeSlot) const
{
	CRenderView* pRenderView = RenderView();
	DRX_ASSERT(pRenderView);
	DRX_ASSERT(shadowCascadeSlot < CShadowUtils::MaxCascadesNum);
...
void CVolumetricFogStage::ExecuteInjectInscatteringLight(const SScopedComputeCommandList& commandList)
{
	...
	u32k nScreenWidth = m_pInscatteringVolume->GetWidth();
	u32k nScreenHeight = m_pInscatteringVolume->GetHeight();
	u32k volumeDepth = m_pInscatteringVolume->GetDepth();
	...
	{
		i32k staticShadowMapSlot = 3; // explicitly set to 4th cascade.
	...
	if (pass.IsDirty(inputFlag,
		...
		m_resourceFrameID))
	{
		static CDrxNameTSCRC techName("InjectVolumetricInscattering");
		pass.SetTechnique(pShader, techName, rtMask);
	...
	static CDrxNameR paramSunDir("vfSunDir");
	...
	pass.SetConstant(CDrxNameR("TexGen0"), shadowSamplingInfo.shadowTexGen[0]);
	pass.SetConstant(CDrxNameR("TexGen1"), shadowSamplingInfo.shadowTexGen[1]);
	pass.SetConstant(CDrxNameR("TexGen2"), shadowSamplingInfo.shadowTexGen[2]);
	pass.SetConstant(CDrxNameR("TexGen3"), shadowSamplingInfo.shadowTexGen[3]);
	pass.SetConstant(CDrxNameR("vInvShadowMapSize"), shadowSamplingInfo.invShadowMapSize);
	pass.SetConstant(CDrxNameR("fDepthTestBias"), shadowSamplingInfo.depthTestBias);
	pass.SetConstant(CDrxNameR("fOneDivFarDist"), shadowSamplingInfo.oneDivFarDist);
	pass.SetConstant(CDrxNameR("fKernelRadius"), shadowSamplingInfo.kernelRadius);
	pass.SetConstant(CDrxNameR("CloudShadowParams"), shadowSamplingInfo.cloudShadowParams);
	pass.SetConstant(CDrxNameR("CloudShadowAnimParams"), shadowSamplingInfo.cloudShadowAnimParams);
	pass.SetConstantArray(CDrxNameR("irreg_kernel_2d"), shadowSamplingInfo.irregKernel2d, 8);
	...
	static CDrxNameR paramSunColor("SunColor");
	...
	static CDrxNameR paramFogColor("ExponentialHeightFogColor");
	...
	static CDrxNameR paramInjectInscattering("InjectInscatteringParams");
...
void CVolumetricFogStage::ExecuteBlurDensityVolume(const SScopedComputeCommandList& commandList)
{
	...
	u32k nScreenHeight = m_pVolFogBufDensity->GetHeight();
	u32k volumeDepth = m_pVolFogBufDensity->GetDepth();
	u32k bufferId = GetTemporalBufferId();
...

	{
		...
		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName("BlurHorizontalDensityVolume");
			pass.SetTechnique(pShader, techName, 0);
...

	{
		...
		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName("BlurVerticalDensityVolume");
			pass.SetTechnique(pShader, techName, 0);
...
void CVolumetricFogStage::ExecuteBlurInscatterVolume(const SScopedComputeCommandList& commandList)
{
...

	{
		...
		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName("BlurHorizontalInscatteringVolume");
			pass.SetTechnique(pShader, techName, 0);
...

	{
		...
		if (pass.IsDirty())
		{
			static CDrxNameTSCRC techName("BlurVerticalInscatteringVolume");
			pass.SetTechnique(pShader, techName, 0);
...
void CVolumetricFogStage::ExecuteTemporalReprojection(const SScopedComputeCommandList& commandList)
{
	...
	i32k activeGpuCount = gRenDev->GetActiveGPUCount();
	...
	// temporal reprojection
	{
		...
		if (pass.IsDirty(CRenderer::CV_r_VolumetricFogReprojectionMode))
		{
			...
			static CDrxNameTSCRC techName("ReprojectVolumetricFog");
			pass.SetTechnique(pShader, techName, rtMask);
		...
		static CDrxNameR param1("PrevViewProjMatrix");
...
void CVolumetricFogStage::ExecuteRaymarchVolumetricFog(const SScopedComputeCommandList& commandList)
{
	...
	if (pass.IsDirty())
	{
		static CDrxNameTSCRC techName("RaymarchVolumetricFog");
		pass.SetTechnique(pShader, techName, 0);
