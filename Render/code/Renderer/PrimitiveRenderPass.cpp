...
#include <drx3D/CoreX/Renderer/RendElement.h>
...
i32 CRenderPrimitive::s_nPrimitiveGeometryCacheUsers = 0;
...
CRenderPrimitive::CRenderPrimitive(CRenderPrimitive&& other)
	...
	, m_constantUpr(std::move(other.m_constantUpr))
...
void CRenderPrimitive::AllocateTypedConstantBuffer(EConstantBufferShaderSlot shaderSlot, i32 size, EShaderStage shaderStages)
{
...
{
	DRX_ASSERT_MESSAGE(targetPass.GetRenderPass() && targetPass.GetRenderPass()->IsValid(), 
	...
	if ((dirtyMask != eDirty_None) || (m_currentPsoUpdateCount != m_pPipelineState->GetUpdateCount()))
	{
		...
		if (dirtyMask & eDirty_Geometry)
		{
			...
			if (m_primitiveType != ePrim_Custom)
			{
				...
				if (CVrProjectionUpr::Instance()->GetProjectionType() == CVrProjectionUpr::eVrProjection_LensMatched && primitiveType == ePrim_Triangle)
		...
		{
			m_constantUpr.ReleaseShaderReflection();
			...
			if (m_flags & eFlags_ReflectShaderConstants_Mask)
			{
				...
				m_constantUpr.AllocateShaderReflection(m_pShader, m_techniqueName, m_rtMask, stages);
			}
			...
			for (auto& cb : m_constantUpr.GetBuffers())
		...
		if (dirtyMask & (eDirty_Technique | eDirty_RenderState | eDirty_ResourceLayout | eDirty_RenderPass | eDirty_Topology))
		{
			...
			if (m_flags & eFlags_ReflectShaderConstants_Mask)
				m_constantUpr.InitShaderReflection(*m_pPipelineState);

			const auto& cBuffers = m_constantUpr.GetBuffers();
...
void CRenderPrimitive::AddPrimitiveGeometryCacheUser()
{
	if (s_nPrimitiveGeometryCacheUsers == 0)
	{
		...
		// ePrim_Triangle
		{
			...
			DrxStackAllocWithSizeVector(SVF_P3F_C4B_T2F, 3, fullscreenTriVertices, CDeviceBufferUpr::AlignBufferSizeForStreaming);
		...
		// ePrim_FullscreenQuad
		{
			...
			// NOTE: Get aligned stack-space (pointer and size aligned to manager's alignment requirement)
			DrxStackAllocWithSizeVectorCleared(SVF_P3F_C4B_T2F, 4, fullscreenQuadVertices, CDeviceBufferUpr::AlignBufferSizeForStreaming);
		...
		// Projector Primitives
		{
			...
			for (i32 i = ePrim_Projector; i <= ePrim_Projector2; ++i)
			{
			...
			for (i32 i = ePrim_ClipProjector; i <= ePrim_ClipProjector2; ++i)
			{
...
void CPrimitiveRenderPass::SetTargetClearMask(u32 clearMask)
{
...
{
	DRX_ASSERT(m_bAddingPrimitives); // Need to call 'BeginAddingPrimitives' first
...
{
	DRX_ASSERT(m_bAddingPrimitives); // Need to call 'BeginAddingPrimitives' first
	DRX_ASSERT(pPrimitive->m_pPipelineState->IsValid());
	DRX_ASSERT(pPrimitive->m_pResourceLayout);
	DRX_ASSERT(!pPrimitive->m_pResources || pPrimitive->m_pResources->IsValid());
...
void CPrimitiveRenderPass::Compile()
{
	...
	if (bOutputsDirty)
	{
		...
		if (outputUAVs[0].IsValid())
		{
			...
			for (i32 i = 0; i < outputUAVs.size(); ++i)
			{
			...
			if (m_outputResources.HasChanged())
			{
				...
				DRX_ASSERT(m_pOutputResourceSet->IsValid());
			}
			...
			if (m_outputNULLResources.HasChanged())
			{
				...
				DRX_ASSERT(m_pOutputNULLResourceSet->IsValid());
			}
		}
	}
}
...
{
	u32 firstBindSlot = m_pOutputResourceSet ? 1 : 0;
	...
	{
		u32 bindSlot = firstBindSlot;

		DRX_ASSERT(pPrimitive->m_pPipelineState->IsValid());
		DRX_ASSERT(pPrimitive->m_pResourceLayout);
		DRX_ASSERT_MESSAGE(pPrimitive->m_type == SCompiledRenderPrimitive::eType_RenderPrimitive ? !static_cast<CRenderPrimitive*>(pPrimitive)->IsDirty() : true, "Render primitive of the current render pass is dirty!");

		if (pPrimitive->m_pResources)
		{
			DRX_ASSERT(pPrimitive->m_pResources->IsValid());
			pCommandInterface->PrepareResourcesForUse(bindSlot++, pPrimitive->m_pResources.get());
		}

		u32 cbBindSlot = bindSlot;
...
void CPrimitiveRenderPass::Execute()
{
	...
	if (!CVrProjectionUpr::Instance()->SetRenderingState(
		commandList,
		m_viewport,
	...
	{
		CVrProjectionUpr::Instance()->RestoreState(commandList);
	}
