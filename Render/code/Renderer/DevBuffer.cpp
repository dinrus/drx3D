...
#include <drx3D/CoreX/BitFiddling.h> // IntegerLog2_RoundUp
...
struct SStatoscopeTimer
{
	...
	~SStatoscopeTimer()
	{
		* value += DrxGetTicks() - start;
	}
};

SStatoscopeData& GetStatoscopeData(u32 nIndex);
...
BufferInvalidationsT& GetBufferInvalidations(u32 threadid);
...
static tukk ConstantToString(BUFFER_USAGE usage)
{
	switch (usage)
	{
	case BU_IMMUTABLE:
...
static tukk ConstantToString(BUFFER_BIND_TYPE type)
{
	switch (type)
	{
...
static inline i32 _GetThreadID()
{
...
class CPartitionTable
{
	...
	typedef std::vector<u32> key_storage_t;
...
public:
	...
	~CPartitionTable()
	{
		for (size_t i = 0; i < (m_capacity >> TableShift); ++i)
			DrxModuleMemalignFree(m_storage[i]);
		realloc(m_storage, 0);
	}

	u32 Capacity() const { return m_capacity; }
	u32 Count() const    { return m_partition; }
	...
	u32 Allocate()
	{
		...
		IF (m_partition + 1 >= m_capacity, 0)
		{
			...
			if ((m_capacity >> TableShift) > MaxTables)
				DrxFatalError("ran out of number of items, please increase MaxTables enum");
			m_storage[old_capacity >> TableShift] = (T*)(DrxModuleMemalign(TableSize * sizeof(T), 16));
			m_table.resize(m_capacity, u32());
			m_remap_table.resize(m_capacity, u32());
			std::iota(m_table.begin() + old_capacity, m_table.end(), old_capacity);
		}
		u32 storage_index = m_table[key = m_partition++];
	...
	void Free(item_handle_t key)
	{
		...
		u32 roster_index = m_remap_table[key];
		(&(this->operator[](key)))->~T();
...
struct SBufferPoolItem
{
	...
	SBufferPoolItem(size_t handle)
		...
		, m_marked(0)
	{
		//DrxLogAlways("ITEM %4d created", handle);
	}

	~SBufferPoolItem()
	{
		//DrxLogAlways("ITEM %4d destroyed", m_handle);
#if DEVBUFFERMAN_DEBUG
		m_offset = ~0u;
		m_bank = ~0u;
		m_base_ptr = (u8*)-1;
	...
	void Relocate(SBufferPoolItem& item)
	{
		...
		if (m_defrag)
		{
			...
			m_defrag_allocator->ChangeContext(m_defrag_handle, reinterpret_cast<uk>(static_cast<uintptr_t>(item.m_handle)));
			m_defrag_allocator->ChangeContext(item.m_defrag_handle, reinterpret_cast<uk>(static_cast<uintptr_t>(m_handle)));
		}
...
public:
	...
	uk BeginRead   (CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset) { DrxFatalError("Attempting to use unimplemented CDefaultUpdater"); return nullptr; }
	uk BeginWrite  (CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset) { DrxFatalError("Attempting to use unimplemented CDefaultUpdater"); return nullptr; }
	void  EndReadWrite(CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset) { DrxFatalError("Attempting to use unimplemented CDefaultUpdater"); }

	void  RegisterBank(SBufferPoolBank* bank)                       {}
	void  UnregisterBank(SBufferPoolBank* bank)                     {}

	void  Read (CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset,       uk dst) { DrxFatalError("Attempting to use unimplemented CDefaultUpdater"); }
	void  Write(CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset, ukk src) { DrxFatalError("Attempting to use unimplemented CDefaultUpdater"); }
	  ...
	  , buffer_size_t src_offset)
	{
		...
		DrxFatalError("Defragmentation not stageable on directx 11.0! Turn off the defragmenter.");
#endif
...
public:
	...
	bool CreateResources()
	{
		...
		{
			DrxLogAlways("CTransientStagingBufferPool::CreateResources: could not create staging buffer");
			FreeResources();
			return false;
		}
	...
	uk BeginRead(CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset)
	{
		MEMORY_SCOPE_CHECK_HEAP();
	...
	uk BeginWrite(CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset)
	{
		MEMORY_SCOPE_CHECK_HEAP();
		DEVBUFFERMAN_ASSERT(buffer && size);
		DEVBUFFERMAN_ASSERT(size <= s_PoolConfig.m_pool_bank_size);
	...
	void Read(CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset, uk dst)
	{
		MEMORY_SCOPE_CHECK_HEAP();
		DEVBUFFERMAN_ASSERT(buffer && size);
		DEVBUFFERMAN_ASSERT(size <= s_PoolConfig.m_pool_bank_size);
	...
	void Write(CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset, ukk src)
	{
		MEMORY_SCOPE_CHECK_HEAP();
		DEVBUFFERMAN_ASSERT(buffer && size);
		DEVBUFFERMAN_ASSERT(size <= s_PoolConfig.m_pool_bank_size);
...
public:
	...
	bool CreateResources()
	{
		...
		if (!(m_resources.m_staging_buffers[SStagingResources::WRITE] = CDeviceBuffer::Create(LayoutW, nullptr)) ||
			!(m_resources.m_staging_buffers[SStagingResources::READ ] = CDeviceBuffer::Create(LayoutR, nullptr)))
		{
			DrxLogAlways("SStaticBufferPool::CreateResources: could not create staging buffer");
			FreeResources();
			return false;
		}
...
public:
	...
	void Read(CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset, uk dst)
	{
		...
		uk base_ptr = BeginRead(buffer, size, offset);
	...
	void Write(CDeviceBuffer* buffer, buffer_size_t size, buffer_size_t offset, ukk src)
	{
		...
		uk base_ptr = BeginWrite(buffer, size, offset);
		...
		, buffer_size_t src_offset)
	{
		...
		uk src_ptr = BeginRead (src_buffer, src_size, src_offset);
		uk dst_ptr = BeginWrite(dst_buffer, dst_size, dst_offset);
...
struct CDynamicDefragAllocator
{
	...
	{
		MEMORY_SCOPE_CHECK_HEAP();
		if (m_defrag_allocator = DrxGetIMemoryUpr()->CreateDefragAllocator())
		{
	...
	item_handle_t Allocate(buffer_size_t size, SBufferPoolItem*& item)
	{
		...
		item->m_size = size;
		item->m_offset = (u32)m_defrag_allocator->WeakPin(hdl);
		item->m_defrag_allocator = m_defrag_allocator;
		item->m_defrag_handle = hdl;
		item->m_defrag = true;
		m_defrag_allocator->ChangeContext(hdl, reinterpret_cast<uk>(static_cast<uintptr_t>(item_hdl)));
		return item_hdl;
	}
	...
	void Update(u32 inflight, u32 frame_id, bool allow_defragmentation)
	{
...
struct CPartitionAllocator
{
...
#if (DRX_RENDERER_DIRECT3D >= 111) && (DRX_RENDERER_DIRECT3D < 120)
	u32              m_map_count = 0;
#endif
	...
	CPartitionAllocator(
		CDeviceBuffer* buffer, uk base_ptr, size_t page_size, size_t bucket_size)
		: m_buffer(buffer)
		, m_base_ptr(base_ptr)
		, m_page_size((u32)page_size)
		, m_bucket_size((u32)bucket_size)
		, m_partition(0)
	...
	uk          base_ptr() const { return m_base_ptr; }
...
struct CConstantBufferAllocator
{
	...
	typedef std::pair<CPartitionAllocator*, u16> RetiredSlot;
	...
	{
		DRX_PROFILE_FUNCTION(PROFILE_RENDERER);
		DRX_ASSERT(cbuffer && cbuffer->m_size && "Bad allocation request");
		const unsigned size = cbuffer->m_size;
		const unsigned nsize = NextPower2(size);
		const unsigned bucket = IntegerLog2(nsize) - 8;
		DRX_ASSERT(bucket < DRX_ARRAY_COUNT(m_page_buckets) && "Bad allocation size");
		bool failed = false;
retry:
		for (size_t i = m_page_buckets[bucket].size(); i > 0; --i)
		{
			...
			if (key != ~0u)
			{
				...
				cbuffer->m_allocator = reinterpret_cast<uk>(m_page_buckets[bucket][i - 1]);
				return true;
			}
		}
		if (!failed)
		{
			...
			{
				DrxLogAlways("Failed to create constant buffer pool");
				return false;
			}
	...
	void Free(CConstantBuffer* cbuffer)
	{
		...
		m_retired_slots[m_frameid].push_back(
		  std::make_pair(allocator, (u16)(cbuffer->m_offset >> (bucket + 8))));
	}

	void Update(u32 frame_id, DeviceFenceHandle fence, bool allow_defragmentation)
	{
		FlushRetiredSlots();
...
public:
	...
	virtual void          Sync()                                                                      {}
	virtual void          Update(u32 frameId, DeviceFenceHandle fence, bool allow_defragmentation) {}
	virtual void          ReleaseEmptyBanks()                                                         {}
	virtual uk         BeginRead(SBufferPoolItem* item)                                            { return NULL; }
	virtual uk         BeginWrite(SBufferPoolItem* item)                                           { return NULL; }
	virtual void          EndReadWrite(SBufferPoolItem* item, bool requires_flush)                    {}
	virtual void          Write(SBufferPoolItem* item, ukk src, buffer_size_t size, buffer_size_t offset) { __debugbreak(); }
	  ...
	  , private IDefragAllocatorPolicy
{
	...
	void ProcessPendingMove(SPendingMove& move, bool block)
	{
		...
		if (done)
		{
			...
			{
				u32 nSrcBank = nDecOffs / s_PoolConfig.m_pool_bank_size;
	...
	SBufferPoolBank* CreateBank()
	{
		...
		{
			DB_MEMREPLAY_SCOPE(EMemReplayAllocClass::C_UserPointer, EMemReplayUserPointerClass::C_DrxMalloc);

			const SBufferLayout Layout =
			{
				DXGI_FORMAT_UNKNOWN, s_PoolConfig.m_pool_bank_size, 1,
				CDeviceObjectFactory::USAGE_HIFREQ_HEAP |
#if !BUFFER_USE_STAGED_UPDATES && (DRX_PLATFORM_CONSOLE) // UMA
				CDeviceObjectFactory::USAGE_DIRECT_ACCESS |
#endif
			...
			if (!(buffer = CDeviceBuffer::Create(Layout, nullptr)))
			{
				DrxLogAlways("SBufferPoolImpl::Allocate: could not allocate additional bank of size %" PRISIZE_T, s_PoolConfig.m_pool_bank_size);
				return nullptr;
			}
		}
	...
	void PrintDebugStats()
	{
		...
		DrxLogAlways("SBufferPoolImpl Stats : %04" PRISIZE_T " num_banks %06" PRISIZE_T " allocations"
	...
	bool RecreateBank(SBufferPoolBank* bank)
	{
		FUNCTION_PROFILER_RENDERER();
		{
			...
			{
				DrxLogAlways("SBufferPoolImpl::Allocate: could not re-allocate freed bank of size %" PRISIZE_T, s_PoolConfig.m_pool_bank_size);
				return false;
			}
		}
#if !BUFFER_USE_STAGED_UPDATES
	...
	{
#if DRX_PLATFORM_WINDOWS && DRX_PLATFORM_64BIT // Workaround for Win64, using a C-cast here breaks Orbis
	#pragma warning( push )
	...
	void Relocate(u32 userMoveId, uk pContext, UINT_PTR newOffset, UINT_PTR oldOffset, UINT_PTR size) final
	{
		...
		SBufferPoolBank* bank = &m_bank_table[m_banks[item.m_bank]];
		u8* old_offset = bank->m_base_ptr + item.m_offset;
	...
	void CancelCopy(u32 userMoveId, uk pContext, bool bSync) final
	{
	...
	void SyncCopy(uk pContext, UINT_PTR dstOffset, UINT_PTR srcOffset, UINT_PTR size) final { __debugbreak(); }
...
freestanding:
			...
			if (!(buffer = CDeviceBuffer::Create(Layout, nullptr)))
			{
				DrxLogAlways("SBufferPoolImpl::Allocate: could not allocate buffer of size %" PRISIZE_T, size);
...
retry:
		...
		{
			DrxLogAlways("SBufferPoolImpl::Allocate: could not allocate pool item of size %" PRISIZE_T, size);
		...
		{
#ifndef _RELEASE
			DrxLogAlways("SBufferPoolImpl::Allocate: WARNING: "
	...
	bool CreateResources(bool enable_defragging, bool best_fit) final
	{
		...
		{
			DrxLogAlways("buffer pool allocator failed to create resources");
			return false;
		}
		if (!m_updater.CreateResources())
		{
			DrxLogAlways("Buffer pool updater failed to create resources");
			return false;
		}
		...
		for (size_t i = 0; i < s_PoolConfig.m_pool_max_moves_per_update; ++i)
		{
			...
			{
				DrxLogAlways("Could not create buffer pool copy gpu fence");
				return false;
			}
			if (GetDeviceObjectFactory().CreateFence(m_pending_moves[i].m_relocate_fence) != S_OK)
			{
				DrxLogAlways("Could not create buffer pool relocate fence");
				return false;
			}
		}
		return true;
	...
	void Sync() final
	{
		...
		for (i32 i = 0; i < SPoolConfig::POOL_FRAME_QUERY_COUNT; ++i)
		{
	...
	//////////////////////////////////////////////////////////////////////////////////////
	void Update(u32 frame_id, DeviceFenceHandle fence, bool allow_defragmentation) final
	{
		...
		for (u32 i = m_current_frame; i < m_current_frame + SPoolConfig::POOL_FRAME_QUERY_COUNT; ++i)
		{
		...
		m_allocator.Update(min(inflight, (u32)s_PoolConfig.m_pool_max_moves_per_update)
		                   , frame_id, allow_defragmentation);
	}

	//////////////////////////////////////////////////////////////////////////////////////////////
	// Buffer IO methods
	uk BeginRead(SBufferPoolItem* item) final
	{
...
private:
	SBufferPoolItem* GetWritableItem(SBufferPoolItem* item)
	{
		...
		IF(item->m_bank != ~0u && item->m_used /*&& GetDeviceObjectFactory().SyncFence(m_current_fence, false, false) != S_OK*/, 0)
		{
			...
			if (handle == ~0u)
			{
				DrxLogAlways("failed to allocate new slot on write");
				return NULL;
			}
	...
	void WriteStaged(SBufferPoolItem* item_, ukk src, buffer_size_t size, buffer_size_t offset)
	{
		GetDeviceObjectFactory().SyncToGPU();
...
#if !BUFFER_USE_STAGED_UPDATES
	void WriteUnstaged(SBufferPoolItem* item_, ukk src, buffer_size_t size, buffer_size_t offset)
	{
		GetDeviceObjectFactory().SyncToGPU();
		SBufferPoolItem* item = item_;
		u8* item_base_ptr = nullptr;
...
public:
	uk BeginWrite(SBufferPoolItem* item) final
	{
		GetDeviceObjectFactory().SyncToGPU();
	...
	void Write(SBufferPoolItem* item, ukk src, buffer_size_t size, buffer_size_t offset) final
	{
		...
		for (buffer_size_t cursor = 0; cursor < size; )
		{
			...
			m_updater.Write(item->m_buffer, sz, item->m_offset + offset + of, reinterpret_cast<u8k*>(src) + of);
...
public:
	...
	item_handle_t Allocate(buffer_size_t size, bool /*bUsePool*/) final
	{
		...
		assert(!(uintptr_t(item->m_offset  ) & (DRX_PLATFORM_ALIGNMENT - 1ULL)));
		assert(!(uintptr_t(item->m_base_ptr) & (DRX_PLATFORM_ALIGNMENT - 1ULL)));
	...
	bool CreateResources(bool, bool) final
	{
		...
		{
			DrxLogAlways("CTransientBufferPool::CreateResources: could not allocate backing buffer of size %" PRISIZE_T, s_PoolConfig.m_transient_pool_size);
			return false;
		}
	...
	void Sync() final                            {}
	void Update(u32 frameId, DeviceFenceHandle fence, bool allow_defragmentation) final
	{
	...
	uk BeginRead(SBufferPoolItem* item) final { return NULL; }
	uk BeginWrite(SBufferPoolItem* item) final
	{
		MEMORY_SCOPE_CHECK_HEAP();
		uk ret;
		...
		ret = reinterpret_cast<u8*>(item->m_base_ptr) + item->m_offset;
	#else
	...
	void Write(SBufferPoolItem* item, ukk src, buffer_size_t size, buffer_size_t offset) final
	{
		...
		size = min(item->m_size, Align(size, DRX_PLATFORM_ALIGNMENT));

	#if BUFFER_ENABLE_DIRECT_ACCESS
		CDeviceObjectFactory::UploadContents<true>(item->m_buffer->GetBuffer(), 0, item->m_offset + offset, size, m_map_type, src, reinterpret_cast<u8*>(item->m_base_ptr) + item->m_offset);
	#else
...
public:
		...
		, m_updater(resources)
	{
		...
		{
			DrxLogAlways("DEVBUFFER WARNING: could not create free standing buffer");
		}
	}
	...
	item_handle_t Allocate(buffer_size_t size, bool /*bUsePool*/) final
	{
		...
		{
			DrxFatalError("free standing buffer allocated twice?!");
			return ~0u;
		}
	...
	bool CreateResources(bool, bool) final
	{
		...
		if (!(m_backing_buffer.m_buffer = CDeviceBuffer::Create(Layout, nullptr)))
		{
			DrxLogAlways("FreeStandingBuffer::CreateResources: could not allocate backing buffer of size %" PRISIZE_T, m_allocation_size);
			return false;
		}
...
class CDescriptorPool
{
	struct SDescriptorBlockList
	{
		...
		std::vector<NDrxDX12::CDescriptorBlock> blocks;
...
public:
	...
	SDescriptorBlock* Allocate(buffer_size_t size)
	{
		...
		NDrxDX12::CDescriptorBlock& block = blockList.blocks[itemHandle];
		if (block.GetCapacity() == 0)
		{
			NDrxDX12::CDevice* pDevice = reinterpret_cast<CDrxDX12Device*>(gcpRendD3D->GetDevice().GetRealDevice())->GetDX12Device();
	...
	void Update(u32 frameId, DeviceFenceHandle fence)
	{
		for (i32 i = 0; i < SPoolConfig::POOL_FRAME_QUERY_COUNT; ++i)
		{
...
public:
	...
	SDescriptorSet* Allocate(const VkDescriptorSetLayout& descriptorSetLayout)
	{
		...
		NDrxVulkan::CDevice* pDevice = GetDeviceObjectFactory().GetVKDevice();
		...
		DRX_ASSERT_MESSAGE(result == VK_SUCCESS, "Vulkan fails to provide a descriptor set!");
...
struct SPoolUpr
{
	...
	bool CreatePool(BUFFER_BIND_TYPE type, BUFFER_USAGE usage, bool enable_defragging, bool best_fit, SBufferPool* pool)
	{
		...
		{
			DrxLogAlways("SPoolUpr::Initialize: could not initialize buffer pool of type '%s|%s'"
	...
	bool Initialize()
	{
		...
		for (size_t i = 0; i < SPoolConfig::POOL_FRAME_QUERY_COUNT; ++i)
			...
			{
				DrxLogAlways("SPoolUpr::Initialize: could not create per-frame gpu fence");
				goto error;
			}
		...
		if (!success)
		{
			DrxLogAlways("SPoolUpr::Initialize: could not initialize a buffer pool");
			goto error;
		}
	...
	bool Shutdown()
	{
		...
		for (size_t i = 0; i < BBT_MAX; ++i)
			for (size_t j = 0; j < BU_MAX; ++j)
			{
				...
				{
					DrxLogAlways("SPoolUpr::Initialize: could not shutdown buffer pool of type '%s|%s'"
		...
		for (size_t i = 0; i < SPoolConfig::POOL_FRAME_QUERY_COUNT; ++i)
		{
			...
			{
				DrxLogAlways("SPoolUpr::Initialize: could not releasefence");
				success = false;
			}
...
public:
	...
	{
		SREC_AUTO_LOCK(s_PoolUpr.m_lock);
		LARGE_INTEGER freq;
		QueryPerformanceFrequency(&freq);
		double rfreq = 1. / (double)freq.QuadPart;

		fr.AddValue(s_PoolUpr.m_sdata[1].m_written_bytes / 1024.f);
		fr.AddValue(s_PoolUpr.m_sdata[1].m_read_bytes / 1024.f);
		fr.AddValue((float)(s_PoolUpr.m_sdata[1].m_creator_time * rfreq));
		fr.AddValue((float)(s_PoolUpr.m_sdata[1].m_io_time * rfreq) * 1000.f);
		fr.AddValue((float)(s_PoolUpr.m_sdata[1].m_cpu_flush_time * rfreq) * 1000.f);
		fr.AddValue((float)(s_PoolUpr.m_sdata[1].m_gpu_flush_time * rfreq) * 1000.f);

	#if CONSTANT_BUFFER_ENABLE_DIRECT_ACCESS
		size_t cbsize =
		  s_PoolUpr.m_constant_allocator[0].m_pages +
		  s_PoolUpr.m_constant_allocator[1].m_pages;
		cbsize *= s_PoolConfig.m_pool_bank_size;
		fr.AddValue((i32)(cbsize >> 10));
	#else
		fr.AddValue(0);
	#endif

		memset(&s_PoolUpr.m_sdata[1], 0, sizeof(s_PoolUpr.m_sdata[1]));
	}
};
...
public:
	virtual SDescription GetDescription() const
	{
		return SDescription('D', "devbuffers", "['/DevBuffers/$' "
		                                       "(i32 poolAllocatedSize) "
		                                       "(i32 poolNumBanks) "
		                                       "(i32 poolNumAllocs) "
		                                       "(i32 poolFreeBlocks) "
		                                       "(i32 poolMovingBlocks) "
		                                       "(float poolUsageMB) "
		                                       "(float poolFreeMB) "
		                                       "(float poolFrag) "
		                                       "]");
	...
	virtual void Write(IStatoscopeFrameRecord& fr)
	{
		for (i32 i = 0; i < BU_MAX; ++i)
			for (i32 j = 0; j < BBT_MAX; ++j)
			{
				...
				fr.AddValue((i32)stats.bank_size * (i32)stats.num_banks);
				fr.AddValue((i32)stats.num_banks);
				fr.AddValue((i32)stats.allocator_stats.nInUseBlocks);
				fr.AddValue((i32)stats.allocator_stats.nFreeBlocks);
				fr.AddValue((i32)stats.allocator_stats.nMovingBlocks);
...
//////////////////////////////////////////////////////////////////////////////////////
CDeviceBufferUpr::CDeviceBufferUpr()
{
}
...
//////////////////////////////////////////////////////////////////////////
CDeviceBufferUpr* CDeviceBufferUpr::Instance()
{
	return &gcpRendD3D->m_DevBufMan;
}

//////////////////////////////////////////////////////////////////////////////////////
void CDeviceBufferUpr::LockDevMan()
{
	MEMORY_SCOPE_CHECK_HEAP();
	s_PoolUpr.m_lock.Lock();
}

//////////////////////////////////////////////////////////////////////////////////////
void CDeviceBufferUpr::UnlockDevMan()
{
	MEMORY_SCOPE_CHECK_HEAP();
	s_PoolUpr.m_lock.Unlock();
}

//////////////////////////////////////////////////////////////////////////////////////
bool CDeviceBufferUpr::Init()
{
	LOADING_TIME_PROFILE_SECTION;
	MEMORY_SCOPE_CHECK_HEAP();
	SREC_AUTO_LOCK(s_PoolUpr.m_lock);
	if (s_PoolUpr.m_initialized == true)
		return true;

	// Initialize the pool manager
	if (!s_PoolUpr.Initialize())
	{
		DrxFatalError("CDeviceBufferUpr::Init(): pool manager failed to initialize");
		return false;
	}
...
//////////////////////////////////////////////////////////////////////////////////////
bool CDeviceBufferUpr::Shutdown()
{
	MEMORY_SCOPE_CHECK_HEAP();
	SREC_AUTO_LOCK(s_PoolUpr.m_lock);
	if (s_PoolUpr.m_initialized == false)
		return true;

	// Initialize the pool manager
	if (!s_PoolUpr.Shutdown())
	{
		DrxFatalError("CDeviceBufferUpr::Init(): pool manager failed during shutdown");
		return false;
	}
...
//////////////////////////////////////////////////////////////////////////////////////
void CDeviceBufferUpr::Sync(u32 frameId)
{
	FUNCTION_PROFILER_RENDERER();
	MEMORY_SCOPE_CHECK_HEAP();
	SREC_AUTO_LOCK(s_PoolUpr.m_lock);
	...
	for (i32 i = 0; i < SPoolConfig::POOL_FRAME_QUERY_COUNT; ++i)
		GetDeviceObjectFactory().SyncFence(s_PoolUpr.m_fences[i], true);

	for (size_t i = 0; i < BBT_MAX; ++i)
		for (size_t j = 0; j < BU_MAX; ++j)
		{
			IF (s_PoolUpr.m_pools[i][j] == NULL, 0)
				continue;
			SREC_AUTO_LOCK(s_PoolUpr.m_pools[i][j]->m_lock);
			s_PoolUpr.m_pools[i][j]->Sync();
		}

#if CONSTANT_BUFFER_ENABLE_DIRECT_ACCESS
	s_PoolUpr.m_constant_allocator[0].FlushRetiredSlots();
	s_PoolUpr.m_constant_allocator[1].FlushRetiredSlots();
#endif
	...
	GetDeviceObjectFactory().IssueFence(s_PoolUpr.m_fences[frameId & SPoolConfig::POOL_FRAME_QUERY_MASK]);
}

//////////////////////////////////////////////////////////////////////////////////////
void CDeviceBufferUpr::ReleaseEmptyBanks(u32 frameId)
{
	FUNCTION_PROFILER_RENDERER();
	MEMORY_SCOPE_CHECK_HEAP();
	SREC_AUTO_LOCK(s_PoolUpr.m_lock);
	...
	for (size_t i = 0; i < BBT_MAX; ++i)
		for (size_t j = 0; j < BU_MAX; ++j)
		{
			IF (s_PoolUpr.m_pools[i][j] == NULL, 0)
				continue;
			SREC_AUTO_LOCK(s_PoolUpr.m_pools[i][j]->m_lock);
			s_PoolUpr.m_pools[i][j]->ReleaseEmptyBanks();
		}

	// Release empty constant buffers
#if CONSTANT_BUFFER_ENABLE_DIRECT_ACCESS
	for (size_t i = 0; i < 2; ++i)
		s_PoolUpr.m_constant_allocator[i].ReleaseEmptyBanks();
#endif
...
//////////////////////////////////////////////////////////////////////////////////////
void CDeviceBufferUpr::Update(u32 frameId, bool called_during_loading)
{
	FUNCTION_PROFILER_RENDERER();
...
#if ENABLE_STATOSCOPE
	s_PoolUpr.m_sdata[1].m_written_bytes += s_PoolUpr.m_sdata[0].m_written_bytes;
	s_PoolUpr.m_sdata[1].m_read_bytes += s_PoolUpr.m_sdata[0].m_read_bytes;
	s_PoolUpr.m_sdata[1].m_creator_time += s_PoolUpr.m_sdata[0].m_creator_time;
	s_PoolUpr.m_sdata[1].m_io_time += s_PoolUpr.m_sdata[0].m_io_time;
	s_PoolUpr.m_sdata[1].m_cpu_flush_time += s_PoolUpr.m_sdata[0].m_cpu_flush_time;
	s_PoolUpr.m_sdata[1].m_gpu_flush_time += s_PoolUpr.m_sdata[0].m_gpu_flush_time;
	memset(&s_PoolUpr.m_sdata[0], 0, sizeof(s_PoolUpr.m_sdata[0]));
#endif

	GetDeviceObjectFactory().SyncFence(s_PoolUpr.m_fences[frameId & SPoolConfig::POOL_FRAME_QUERY_MASK], true);
	...
	for (size_t i = 0; i < BBT_MAX; ++i)
		for (size_t j = 0; j < BU_MAX; ++j)
		{
			IF (s_PoolUpr.m_pools[i][j] == NULL, 0)
				continue;
			SREC_AUTO_LOCK(s_PoolUpr.m_pools[i][j]->m_lock);
			s_PoolUpr.m_pools[i][j]->Update(frameId
			                                    , s_PoolUpr.m_fences[frameId & SPoolConfig::POOL_FRAME_QUERY_MASK]
	...
	for (size_t i = 0; i < 2; ++i)
	{
		s_PoolUpr.m_constant_allocator[i].Update(frameId
		                                             , s_PoolUpr.m_fences[frameId & SPoolConfig::POOL_FRAME_QUERY_MASK]
		                                             , called_during_loading == false);
	}
#endif
...
#if (DRX_RENDERER_DIRECT3D >= 120) || (DRX_RENDERER_VULKAN >= 10)
	s_PoolUpr.m_ResourceDescriptorPool.Update(frameId,
	                                              s_PoolUpr.m_fences[frameId & SPoolConfig::POOL_FRAME_QUERY_MASK]);
#endif
...
//////////////////////////////////////////////////////////////////////////////////////
CConstantBuffer* CDeviceBufferUpr::CreateConstantBufferRaw(buffer_size_t size, bool dynamic, bool ts)
{
	STATOSCOPE_TIMER(s_PoolUpr.m_sdata[0].m_creator_time);
	size = (max(size, buffer_size_t(1u)) + (255)) & ~(255);
	CConditonalDevManLock lock(this, ts);
#if DEVBUFFERMAN_CCONSTANTBUFFER_POOL
	CConstantBuffer* buffer = &s_PoolUpr.m_constant_buffers[ts][s_PoolUpr.m_constant_buffers[ts].Allocate()];
#else
...
CConstantBufferPtr CDeviceBufferUpr::GetNullConstantBuffer()
{
	return s_PoolUpr.m_pNullConstantBuffer;
}

CGpuBuffer* CDeviceBufferUpr::GetNullBufferTyped()
{
	return &s_PoolUpr.m_nullBufferTyped;
}

CGpuBuffer* CDeviceBufferUpr::GetNullBufferStructured()
{
	return &s_PoolUpr.m_nullBufferStructured;
}


#if (DRX_RENDERER_DIRECT3D >= 120)
SDescriptorBlock* CDeviceBufferUpr::CreateDescriptorBlock(buffer_size_t size)
{
	return s_PoolUpr.m_ResourceDescriptorPool.Allocate(size);
}

void CDeviceBufferUpr::ReleaseDescriptorBlock(SDescriptorBlock* pBlock)
{
	DRX_ASSERT(pBlock != NULL);
	s_PoolUpr.m_ResourceDescriptorPool.Free(pBlock);
}

#else
SDescriptorBlock* CDeviceBufferUpr::CreateDescriptorBlock(buffer_size_t size)               { return NULL; }
void              CDeviceBufferUpr::ReleaseDescriptorBlock(SDescriptorBlock* pBlock) {}
#endif

#if (DRX_RENDERER_VULKAN >= 10)
SDescriptorSet* CDeviceBufferUpr::AllocateDescriptorSet(const VkDescriptorSetLayout& descriptorSetLayout)
{
	return s_PoolUpr.m_ResourceDescriptorPool.Allocate(descriptorSetLayout);
}

void CDeviceBufferUpr::ReleaseDescriptorSet(SDescriptorSet *pDescriptorSet)
{
	DRX_ASSERT(pDescriptorSet != NULL);
	s_PoolUpr.m_ResourceDescriptorPool.Free(pDescriptorSet);
}
#endif

// PPOL_ALIGNMENT is 128 in general, which is a bit much, we use the smallest alignment necessary for fast mem-copies
#if 0
buffer_size_t CDeviceBufferUpr::GetBufferAlignmentForStreaming()
{
...
//////////////////////////////////////////////////////////////////////////////////////
buffer_handle_t CDeviceBufferUpr::Create_Locked(BUFFER_BIND_TYPE type, BUFFER_USAGE usage, buffer_size_t size, bool bUsePool)
{
	MEMORY_SCOPE_CHECK_HEAP();
	DEVBUFFERMAN_ASSERT((type >= BBT_VERTEX_BUFFER && type < BBT_MAX));
	DEVBUFFERMAN_ASSERT((usage >= BU_IMMUTABLE && usage < BU_MAX));
	DEVBUFFERMAN_ASSERT(s_PoolUpr.m_pools[type][usage] != NULL);

	// Workaround for NVIDIA SLI issues with latest drivers. GFE should disable the cvar below when fixed
	// Disabled for now
#if DRX_PLATFORM_WINDOWS
	if (s_PoolUpr.m_buffer_creators[type][usage])
	{
		...
		{
			SBufferPool* pool = s_PoolUpr.m_buffer_creators[type][usage](
			  s_PoolUpr.m_staging_resources[usage],
			  size
			  );
	...
	item_handle_t item_handle = s_PoolUpr.m_pools[type][usage]->Allocate(size, bUsePool);
	return item_handle == ~0u ? (buffer_handle_t) ~0u : (buffer_handle_t)s_PoolUpr.m_pools[type][usage]->Resolve(item_handle);
}

buffer_handle_t CDeviceBufferUpr::Create(BUFFER_BIND_TYPE type, BUFFER_USAGE usage, buffer_size_t size, bool bUsePool)
{
	DRX_PROFILE_FUNCTION(PROFILE_RENDERER);
	STATOSCOPE_TIMER(s_PoolUpr.m_sdata[0].m_creator_time);
	if (!s_PoolUpr.m_pools[type][usage])
		return ~0u;
#if DRX_PLATFORM_WINDOWS
	SRecursiveSpinLocker __lock(&s_PoolUpr.m_lock);
#endif
	SREC_AUTO_LOCK(s_PoolUpr.m_pools[type][usage]->m_lock);
	return Create_Locked(type, usage, size, bUsePool);
}

//////////////////////////////////////////////////////////////////////////////////////
void CDeviceBufferUpr::Destroy_Locked(buffer_handle_t handle)
{
	DRX_PROFILE_FUNCTION(PROFILE_RENDERER);
	MEMORY_SCOPE_CHECK_HEAP();
...
void CDeviceBufferUpr::Destroy(buffer_handle_t handle)
{
	STATOSCOPE_TIMER(s_PoolUpr.m_sdata[0].m_creator_time);
#if DRX_PLATFORM_WINDOWS
	SRecursiveSpinLocker __lock(&s_PoolUpr.m_lock);
...
//////////////////////////////////////////////////////////////////////////////////////
void CDeviceBufferUpr::PinItem_Locked(buffer_handle_t handle)
{
...
void CDeviceBufferUpr::PinItem(buffer_handle_t handle)
{
	STATOSCOPE_TIMER(s_PoolUpr.m_sdata[0].m_creator_time);
#if DRX_PLATFORM_WINDOWS
	SRecursiveSpinLocker __lock(&s_PoolUpr.m_lock);
...
//////////////////////////////////////////////////////////////////////////////////////
void CDeviceBufferUpr::UnpinItem_Locked(buffer_handle_t handle)
{
	DEVBUFFERMAN_ASSERT(handle != ~0u);
	SBufferPoolItem& item = *reinterpret_cast<SBufferPoolItem*>(handle);
	IF (item.m_bank != ~0u, 1)
...
void CDeviceBufferUpr::UnpinItem(buffer_handle_t handle)
{
	STATOSCOPE_TIMER(s_PoolUpr.m_sdata[0].m_creator_time);
#if DRX_PLATFORM_WINDOWS
	SRecursiveSpinLocker __lock(&s_PoolUpr.m_lock);
...
//////////////////////////////////////////////////////////////////////////////////////
uk CDeviceBufferUpr::BeginRead_Locked(buffer_handle_t handle)
{
	STATOSCOPE_TIMER(s_PoolUpr.m_sdata[0].m_io_time);
...
uk CDeviceBufferUpr::BeginRead(buffer_handle_t handle)
{
	DRX_PROFILE_FUNCTION(PROFILE_RENDERER);
#if DRX_PLATFORM_WINDOWS
	SRecursiveSpinLocker __lock(&s_PoolUpr.m_lock);
...
//////////////////////////////////////////////////////////////////////////////////////
buffer_size_t CDeviceBufferUpr::Size_Locked(buffer_handle_t handle)
{
	SBufferPoolItem& item = *reinterpret_cast<SBufferPoolItem*>(handle);
	return item.m_size;
}
buffer_size_t CDeviceBufferUpr::Size(buffer_handle_t handle)
{
	return Size_Locked(handle);
}

//////////////////////////////////////////////////////////////////////////////////////
uk CDeviceBufferUpr::BeginWrite_Locked(buffer_handle_t handle)
{
	STATOSCOPE_TIMER(s_PoolUpr.m_sdata[0].m_io_time);
...
uk CDeviceBufferUpr::BeginWrite(buffer_handle_t handle)
{
#if DRX_PLATFORM_WINDOWS
	SRecursiveSpinLocker __lock(&s_PoolUpr.m_lock);
#endif
...
//////////////////////////////////////////////////////////////////////////////////////
void CDeviceBufferUpr::EndReadWrite_Locked(buffer_handle_t handle)
{
	STATOSCOPE_TIMER(s_PoolUpr.m_sdata[0].m_io_time);
	STATOSCOPE_IO_WRITTEN(Size(handle));
	DRX_PROFILE_FUNCTION(PROFILE_RENDERER);
...
void CDeviceBufferUpr::EndReadWrite(buffer_handle_t handle)
{
#if DRX_PLATFORM_WINDOWS
	SRecursiveSpinLocker __lock(&s_PoolUpr.m_lock);
#endif
...
//////////////////////////////////////////////////////////////////////////////////////
bool CDeviceBufferUpr::UpdateBuffer_Locked(
  buffer_handle_t handle, ukk src, buffer_size_t size, buffer_size_t offset)
{
	STATOSCOPE_TIMER(s_PoolUpr.m_sdata[0].m_io_time);
	STATOSCOPE_IO_WRITTEN(Size(handle));
	DRX_PROFILE_FUNCTION(PROFILE_RENDERER);
...
bool CDeviceBufferUpr::UpdateBuffer
  (buffer_handle_t handle, ukk src, buffer_size_t size, buffer_size_t offset)
{
#if DRX_PLATFORM_WINDOWS
	SRecursiveSpinLocker __lock(&s_PoolUpr.m_lock);
...
//////////////////////////////////////////////////////////////////////////////////////
D3DBuffer* CDeviceBufferUpr::GetD3D(buffer_handle_t handle, buffer_size_t* offset)
{
	MEMORY_SCOPE_CHECK_HEAP();
	DEVBUFFERMAN_ASSERT(handle != 0);
	SBufferPoolItem& item = *reinterpret_cast<SBufferPoolItem*>(handle);
...
//////////////////////////////////////////////////////////////////////////////////////
D3DBuffer* CDeviceBufferUpr::GetD3D(buffer_handle_t handle, buffer_size_t* offset, buffer_size_t* size)
{
	MEMORY_SCOPE_CHECK_HEAP();
	DEVBUFFERMAN_ASSERT(handle != 0);
	SBufferPoolItem& item = *reinterpret_cast<SBufferPoolItem*>(handle);
...
//////////////////////////////////////////////////////////////////////////////////////
D3DVertexBuffer* CDeviceBufferUpr::GetD3DVB(buffer_handle_t handle, buffer_size_t* offset)
{
	MEMORY_SCOPE_CHECK_HEAP();
...
//////////////////////////////////////////////////////////////////////////////////////
D3DIndexBuffer* CDeviceBufferUpr::GetD3DIB(buffer_handle_t handle, buffer_size_t* offset)
{
	MEMORY_SCOPE_CHECK_HEAP();
...
//////////////////////////////////////////////////////////////////////////////////////
bool CDeviceBufferUpr::GetStats(BUFFER_BIND_TYPE type, BUFFER_USAGE usage, SDeviceBufferPoolStats& stats)
{
	DRX_PROFILE_FUNCTION(PROFILE_RENDERER);
	...
	if (!s_PoolUpr.m_pools[type][usage])
		return false;
	SREC_AUTO_LOCK(s_PoolUpr.m_pools[type][usage]->m_lock);
	return s_PoolUpr.m_pools[type][usage]->GetStats(stats);
}
...
SStatoscopeData& GetStatoscopeData(u32 nIndex)
{
	return s_PoolUpr.m_sdata[nIndex];
}
#endif

//////////////////////////////////////////////////////////////////////////////////////
CConstantBuffer::CConstantBuffer(u32 handle)
	: m_buffer()
#if CONSTANT_BUFFER_ENABLE_DIRECT_ACCESS
	, m_allocator()
#endif
...
void CConstantBuffer::ReturnToPool()
{
	...
	if (m_used)
	{
		s_PoolUpr.m_constant_allocator[m_lock].Free(this);
		m_used = 0;
	}
#endif
...
#if DEVBUFFERMAN_CCONSTANTBUFFER_POOL
	if (m_handle != ~0u)
		s_PoolUpr.m_constant_buffers[m_lock].Free(m_handle);
#else
	delete this;
#endif
}
...
uk CConstantBuffer::BeginWrite()
{
	...
	if (s_PoolUpr.m_constant_allocator[m_lock].Allocate(this))
	{
		m_used = 1;
		...
		return (uk )((uintptr_t)base_ptr + m_offset);
	}
#else
	if (!m_used)
	{
		...
		{
			DrxLogAlways("CConstantBuffer::BeginWrite: could not allocate buffer of size %" PRISIZE_T, m_size);
			return nullptr;
		}
...
void CConstantBuffer::EndWrite(bool requires_flush)
{
	...
	else
	{
		...
		delete[] alias_cast<tuk>(m_base_ptr);
		m_base_ptr = nullptr;
	}
#endif
	m_nUpdCount++;
...
//////////////////////////////////////////////////////////////////////////////////////
bool CConstantBuffer::UpdateBuffer(ukk src, buffer_size_t size, buffer_size_t offset /*= 0*/, i32 numDataBlocks /*= 1*/)
{
	...
	if (s_PoolUpr.m_constant_allocator[m_lock].Allocate(this))
	{
		...
		CDeviceObjectFactory::UploadContents<true>(m_buffer->GetBuffer(), 0, m_offset + offset, size, D3D11_MAP_WRITE_NO_OVERWRITE_CB, src, reinterpret_cast<u8*>(base_ptr) + m_offset + offset, numDataBlocks);
...
SResourceView SResourceView::ShaderResourceRawView(DXGI_FORMAT nFormat, i32 nFirstElement, i32 nElementCount, i32 nFlags)
{
	SResourceView result(0);

	result.m_Desc.eViewType = eShaderResourceView;
	result.m_Desc.nFormat = nFormat;
	result.m_Desc.nOffsetBits = IntegerLog2_RoundUp(u32(nFirstElement + 1));
	...
	DRX_ASSERT(nFirstElement == UINT(result.m_Desc.nOffsetAndSize >> (46 - result.m_Desc.nOffsetBits)));
	DRX_ASSERT(nElementCount == UINT(result.m_Desc.nOffsetAndSize & MASK64(46 - result.m_Desc.nOffsetBits)));

	return result;
}

SResourceView SResourceView::RenderTargetRawView(DXGI_FORMAT nFormat, i32 nFirstElement, i32 nElementCount)
{
	SResourceView result(0);
...
SResourceView SResourceView::DepthStencilRawView(DXGI_FORMAT nFormat, i32 nFirstElement, i32 nElementCount, i32 nFlags)
{
	SResourceView result(0);
...
SResourceView SResourceView::UnorderedAccessRawView(DXGI_FORMAT nFormat, i32 nFirstElement, i32 nElementCount, i32 nFlags)
{
	SResourceView result(0);
...
//		DrxInterlockedAdd(&CGpuBuffer::s_nStatsCurManagedNonStreamedBufMem, m_nBaseSize);
	}
...
void CGpuBuffer::Create(u32 elementCount, u32 elementSize, DXGI_FORMAT elementFormat, u32 eFlags, ukk pData)
{
	Release();
...
CDeviceBuffer* CGpuBuffer::AllocateDeviceBuffer(ukk pInitialData) const
{
	const SBufferLayout Layout =
	{
		m_eFormat,
...
void CGpuBuffer::UpdateBufferContent(ukk pData, u32 nSize)
{
	DRX_ASSERT(!m_bLocked);
...
uk CGpuBuffer::Lock()
{
	DRX_ASSERT(m_eFlags & CDeviceObjectFactory::USAGE_CPU_WRITE);
	DRX_ASSERT(!m_bLocked);
...
//////////////////////////////////////////////////////////////////////////
void CGraphicsDeviceConstantBuffer::SetData(u8k* data, size_t size)
{
	DrxAutoCriticalSection lock(s_accessLock);
	m_size = size;
	m_data.resize(0);
...
CConstantBufferPtr CGraphicsDeviceConstantBuffer::GetConstantBuffer()
{
	...
	if (m_bDirty && m_data.size() > 0)
	{
		...
		const buffer_size_t dataSize = Align(m_data.size(), DRX_PLATFORM_ALIGNMENT);
