...
// -------------------------------------------------------------------------
//  Имя файла:   GLShader.cpp
//  Version:     v1.00
//  Created:     15/05/2013 by Valerio Guagliumi.
//  Описание: Implements the shader related functions
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
#include <drx3D/Render/hlslcc_bin.hpp>
...
// Shaders generated with HLSL2GLSL are simplified using glslOptimizer to exploit its function inlining and simplify instrumentation.
	#include <drx3D/Render/Tools/glslOptimizer/src/glsl/glsl_optimizer.h>
struct SSimplifiedShader
{
	...
	tukk Simplify(tukk szSource, NDrxOpenGL::EShaderType eType)
	{
		struct SAutoContext
		{
		...
		switch (eType)
		{
		case NDrxOpenGL::eST_Vertex:
			eOptType = kGlslOptShaderVertex;
			break;
		case NDrxOpenGL::eST_Fragment:
...
SSource::SSource(tukk pData, u32 uDataSize)
...
struct SDXBCInfo
{
	u32 m_uMajorVersion, m_uMinorVersion;
};
...
struct SDXBCParseContext : SDXBCInputBuffer
{
	...
	SDXBCParseContext(u8k* pBegin, u8k* pEnd, SDXBCInfo* pInfo)
		: SDXBCInputBuffer(pBegin, pEnd)
		, m_pInfo(pInfo)
	{
	}

	SDXBCParseContext(const SDXBCParseContext& kOther, u32 uOffset)
	...
	bool ReadString(tuk pBuffer, u32 uBufSize)
	{
		u32 uSize((u32)strlen(alias_cast<tukk >(m_pIter)) + 1);
		if (uSize > uBufSize)
			return false;
...
void SShaderSource::SetData(tukk pData, u32 uDataSize)
{
	delete[] m_pData;
	if (uDataSize > 0)
	{
		tuk pBuffer = new char[uDataSize];
		NDrxOpenGL::Memcpy(pBuffer, pData, uDataSize);
		m_pData = pBuffer;
	}
	else
	{
...
bool InitializeShaderReflectionConstBuffer(SShaderReflectionConstBuffer* pConstBuffer, SDXBCParseContext* pContext)
{
	...
	u32 uNumVariables, uPosVariables;
...
bool InitializeShaderReflectionParameters(SShaderReflection::TParameters& kParameters, SDXBCParseContext* pContext)
{
	...
	if (uNumElements > 0)
	{
		u32 uPrevNumElements(kParameters.size());
...
bool InitializeShaderReflection(SShaderReflection* pReflection, ukk pvData, u32 uSize)
{
	enum { CHUNKS_POSITION = sizeof(u32) * 7 };
	SDXBCInfo kInfo;
	u8k* puData(alias_cast<u8k*>(pvData));
	...
	u32 uOffset;
	for (u32 uChunk = 0; uChunk < uNumChunks; ++uChunk)
	{
		u32 uChunkBegin;
		...
		case FOURCC_RDEF:
			{
				u32 uNumConstBuffers, uPosConstBuffers, uNumResources, uPosResources;
				...
				if (!kConstBufferContext.SeekAbs(uPosConstBuffers))
					return false;
				u32 uConstBuffer;
				u32 uPrevNumConstBuffers(pReflection->m_kConstantBuffers.size());
				...
				if (!kResourceContext.SeekAbs(uPosResources))
					return false;
				u32 uResource;
				u32 uPrevNumResources(pReflection->m_kResources.size());
		...
		case FOURCC_GLSL:
			...
			uOffset = (u32)(kChunkContext.m_pIter - kContext.m_pBegin);
...
bool ConvertDXBCToGLSL(ukk pvSourceData, size_t uSourceSize, SGLSLConversionOutput* pOutput)
{
	#if DXGL_GLSL_FROM_HLSLCROSSCOMPILER
	u32 uFlags(HLSLCC_FLAG_UNIFORM_BUFFER_OBJECT);
	...
	if (TranslateHLSLFromMem(static_cast<tukk >(pvSourceData), uSourceSize, uFlags, eLanguage, &kExtensions, &pOutput->m_kGLSLShader) != 1)
	{
...
bool InitializeTranslatedShaderSource(SShaderSource* pSource, ukk pvDXBCData, size_t uDXBCSize, const SGLSLConversionOutput& kConversionOutput)
{
	#if DXGL_GLSL_FROM_HLSLCROSSCOMPILER
	u8k* puDXBC(static_cast<u8k*>(pvDXBCData));
	...
	u8* puCombined(static_cast<u8*>(Malloc(uCombinedSize)));
	pSource->m_pData = alias_cast<tukk >(puCombined);
	pSource->m_uDataSize = (u32)uCombinedSize;
...
bool InitializeShaderReflectionFromInput(SShaderReflection* pReflection, ukk pvInputData)
{
	enum { SIZE_POSITION = 6 };
	u32 uInputDataSize(alias_cast<u32k*>(pvInputData)[SIZE_POSITION]);
#if DXGL_INPUT_GLSL
...
bool ReplaceEntries(const STxt* stringsToSearch, const STxt* stringsToReplace, size_t count, STxt& out, const GLchar* in, size_t inStrLen)
{
	out = STxt(in, 0, inStrLen);
	bool linesChanged = false;
	for (i32 j = 0; j < count; ++j)
	{
		const STxt& searchStr = stringsToSearch[j];
		const STxt& replaceString = stringsToReplace[j];
...
bool CompileShaderFromSources(GLuint* puName, const GLchar* aszSources[], GLint aiSourceLengths[], u32 uNumSources, GLenum eGLShaderType)
{
	...
	const STxt searchStrings[] = { "ivec4(4290838528, 4290838528, 4290838528, 0)", "4294967232", "4290772992", "2147483648", "4294967295" };
	const STxt replaceStrings[] = { "vec4(0.000000000, 0.00000000, 0.0000000,0.0)", "65535", "65535", "65535", "65535" };


	STxt sources[4];
	const GLchar* sourcesC[4];
	GLint srcLengths[4];
	for (i32 i = 0; i < uNumSources; ++i)
	{

		const GLchar* src = aszSources[i];

		bool linesChanged = ReplaceEntries(searchStrings, replaceStrings, sizeof(searchStrings) / sizeof(STxt), sources[i], src, aiSourceLengths[i]);
...
bool FindShaderSymbolByType(u32k* puSymbols, u32 uNumSymbols, SYMBOL_TYPE eType, u32* puID, u32* puValue)
{
	u32k* puSymbolsEnd = puSymbols + uNumSymbols * 3;
	for (u32k* puSymbol = puSymbols; puSymbol != puSymbolsEnd; puSymbol += 3)
	{
...
bool FindShaderSymbolByTypeAndID(u32k* puSymbols, u32 uNumSymbols, SYMBOL_TYPE eType, u32 uID, u32* puValue)
{
	u32k* puSymbolsEnd = puSymbols + uNumSymbols * 3;
	for (u32k* puSymbol = puSymbols; puSymbol != puSymbolsEnd; puSymbol += 3)
	{
...
template<u32 uMaxDigits>
u32 WriteShaderDecimal(tuk acOutput, u32 uValue)
{
	char acDigits[uMaxDigits];
	tuk pcDigit(acDigits + uMaxDigits - 1);
	while (true)
	{
		u32 uNext(uValue / 10);
		u32 uDigit(uValue % 10);
		*pcDigit = '0' + uDigit;
	...
	u32 uNumDigits((u32)(acDigits + uMaxDigits - pcDigit));
	NDrxOpenGL::Memcpy(acOutput, pcDigit, uNumDigits);
	return uNumDigits;
}
...
void BuildVersionString(SPipelineCompilationBuffer* pBuffer, CDevice* pDevice)
{
	...
	tukk szVersion = "#version 450\n";
#elif DXGL_REQUIRED_VERSION >= DXGL_VERSION_44
	tukk szVersion = "#version 440\n";
...
#elif DXGL_REQUIRED_VERSION >= DXGL_VERSION_43
	tukk szVersion = "#version 430\n";
#elif DXGL_REQUIRED_VERSION >= DXGL_VERSION_42
	tukk szVersion = "#version 420\n";
#elif DXGL_REQUIRED_VERSION >= DXGL_VERSION_41
	tukk szVersion = "#version 410\n";
#elif DXGLES_REQUIRED_VERSION >= DXGLES_VERSION_31
	tukk szVersion = "#version 310 es\n";
#elif DXGLES_REQUIRED_VERSION >= DXGLES_VERSION_30
	tukk szVersion = "#version 300 es\n";
#else
	#error "Shading language revision not defined for this GL version"
#endif
	pBuffer->m_iVersionStringLength = min(strlen(szVersion), sizeof(pBuffer->m_acVersionString));
	NDrxOpenGL::Memcpy(pBuffer->m_acVersionString, szVersion, pBuffer->m_iVersionStringLength);
}

void BuildShaderImportHeader(SPipelineCompilationBuffer* pBuffer, u32k* auImports, u32 uNumImports, const SPipelineConfiguration& kConfiguration, EShaderType eStage)
{
	...
	{
		pBuffer->m_acImportsHeader = static_cast<tuk>(Realloc(pBuffer->m_acImportsHeader, IMPORT_STRIDE * uNumImports));
		memset(pBuffer->m_acImportsHeader + IMPORT_STRIDE * pBuffer->m_uImportsHeaderCapacity, (i32)' ', IMPORT_STRIDE * (uNumImports - pBuffer->m_uImportsHeaderCapacity));
		for (u32 uImport = pBuffer->m_uImportsHeaderCapacity; uImport < uNumImports; ++uImport)
		{
			tuk acIter(pBuffer->m_acImportsHeader + IMPORT_STRIDE * uImport);
			NDrxOpenGL::Memcpy(acIter, acImportPrefix, IMPORT_INDEX_OFFSET);
	...
	for (u32 uImport = 0; uImport < uNumImports; ++uImport)
	{
		SYMBOL_TYPE eType((SYMBOL_TYPE)auImports[uImport * 3]);
		u32 uID(auImports[uImport * 3 + 1]);
		u32 uDefaultValue(auImports[uImport * 3 + 2]);

		u32 uReqValue = uDefaultValue;
		switch (eType)
		{
		...
		case SYMBOL_TESSELLATOR_OUTPUT_PRIMITIVE:
			if (kConfiguration.m_apShaders[eST_TessControl])
			{
				...
				u32k* auLinkExports(alias_cast<u32k*>(pLinkShader->m_kSource.m_pData + pLinkShader->m_kReflection.m_uExportsOffset));
				u32 uNumLinkExports(pLinkShader->m_kReflection.m_uExportsSize);
		...
		{
			tuk acIter(pBuffer->m_acImportsHeader + IMPORT_STRIDE * uImport + IMPORT_VALUE_OFFSET);
			u32 uNumDigits(WriteShaderDecimal<IMPORT_VALUE_MAX_DIGITS>(acIter, uReqValue));
...
void DumpShaderSources(const GLchar** aszSources, const GLint* aiSourceLengths, u32 uNumSources, u32 uHash)
{
	STraceFile kTraceFile;
	char acBuffer[32];
	drx_sprintf(acBuffer, "0x%08X.txt", uHash);
	if (!kTraceFile.Open(acBuffer, false))
	{
		DXGL_ERROR("Could not open file \"%s\" for writing", acBuffer);
		return;
	}
	for (u32 uSource = 0; uSource < uNumSources; ++uSource)
...
bool CompilePipeline(SPipeline* pPipeline, SPipelineCompilationBuffer* pBuffer, CDevice* pDevice)
{
	...
	for (u32 uShader = 0; uShader < eST_NUM; ++uShader)
	{
		...
		if (IsPipelineStageUsed(pPipeline->m_kConfiguration.m_eMode, (EShaderType)uShader) && pShader != NULL)
		{
			...
			u32 uGLSLOffset(pShader->m_kReflection.m_uGLSLSourceOffset);
			u32 uSymbolsOffset(pShader->m_kReflection.m_uSymbolsOffset);
			SSource kGLSLSource(pShader->m_kSource.m_pData + uGLSLOffset, pShader->m_kSource.m_uDataSize - uGLSLOffset);
			u32k* auImports(alias_cast<u32k*>(pShader->m_kSource.m_pData + pShader->m_kReflection.m_uImportsOffset));
			u32 uNumImports(pShader->m_kReflection.m_uImportsSize);
...
bool InitializeShader(SShader* pShader, ukk pvSourceData, size_t uSourceSize, const GLchar* szSourceOverride)
{
#if DXGL_INPUT_GLSL
	pShader->m_kSource.SetData(reinterpret_cast<tukk >(pvSourceData), uSourceSize);
#else
	...
	if (!InitializeShaderReflection(&pShader->m_kReflection, pShader->m_kSource.m_pData, pShader->m_kSource.m_uDataSize) ||
	    (uGLSLOffset = pShader->m_kReflection.m_uGLSLSourceOffset) >= (u32)uSourceSize)
	{
	...
	{
		tuk szGLSL = (tuk)(pShader->m_kSource.m_pData + pShader->m_kReflection.m_uGLSLSourceOffset);
	...
	u32 uOffset = 0;
	for (u32 uStep = 0; uStep < pTracingInfo->ui32NumTraceSteps; ++uStep)
	{
		const StepTraceInfo* pStep(pTracingInfo->psTraceSteps + uStep);
		for (u32 uVariable = 0; uVariable < pStep->ui32NumVariables; ++uVariable)
...
bool BindUniformBufferToUnit(GLuint uProgramName, u32 uUnit, tukk szName)
{
...
bool BindStorageBufferToUnit(GLuint uProgramName, u32 uUnit, tukk szName)
{
...
bool BindUniformToUnit(GLuint uProgramName, u32 uUnit, tukk szName)
{
...
u32 FindUnusedShaderStorageBufferUnit(SPipeline* pPipeline, CDevice* pDevice)
{
	...
	u32 uNumSupportedStorageSlots((u32)pDevice->GetAdapter()->m_kCapabilities.m_akResourceUnits[eRUT_StorageBuffer].m_aiMaxTotal);
	for (u32 uSlot = 0; uSlot < uNumSupportedStorageSlots; ++uSlot)
	{
...
{
	u32 uUnusedUnit = FindUnusedShaderStorageBufferUnit(pPipeline, pDevice);
...
bool GetEmbeddedName(char (&acBuffer)[uSize], tukk szGLSL, u32 uNameMask)
{
	u32 uNameOffset = uNameMask >> 12;
	u32 uNameSize = uNameMask & 0x3FF;
	...
	NDrxOpenGL::Memcpy(acBuffer, szGLSL + uNameOffset, uNameSize);
...
inline u32 DecodeTextureIndex(u32 uSamplerField)     { return uSamplerField >> 22; }
inline u32 DecodeSamplerIndex(u32 uSamplerField)     { return (uSamplerField >> 12) & 0x3FF; }
inline u32 DecodeTextureUnitIndex(u32 uSamplerField) { return (uSamplerField >> 2) & 0x3FF; }
inline bool   DecodeNormalSample(u32 uSamplerField)     { return ((uSamplerField >> 1) & 0x1) == 1; }
inline bool   DecodeCompareSample(u32 uSamplerField)    { return (uSamplerField & 0x1) == 1; }
inline u32 DecodeResourceIndex(u32 uResourceField)   { return uResourceField; }

template<u32(* DecodeFunc)(u32), u32 uStride>
void GetResourceIndexRange(SResourceIndexRange* pRange, u32k* puResources, u32 uNumResources)
{
	pRange->m_uMin = 0xFFFFFFFF;
	pRange->m_uMax = 0;
	if (uNumResources > 0)
	{
		u32k* puResourcesEnd = puResources + uStride * uNumResources;
		for (u32k* puResource = puResources; puResource < puResourcesEnd; puResource += uStride)
		{
			u32 uIndex = DecodeFunc(*puResource);
...
struct SPipelineResourceRequirements
{
	struct SResourceType
	{
		...
		u32 m_uNumUnits;
		u32 m_uCompatiblePartitionID; // INVALID_PARTITION_ID if no partition is compatible
...
void GetPipelineResourceRequirements(SPipelineResourceRequirements* pRequirements, const SPipeline* pPipeline, CContext* pContext)
{
	...
	for (u32 uShader = 0; uShader < eST_NUM; ++uShader)
	{
		...
		if (IsPipelineStageUsed(pPipeline->m_kConfiguration.m_eMode, (EShaderType)uShader) && pShader != NULL)
		{
			...
			GetResourceIndexRange<DecodeTextureUnitIndex, ENCODED_SAMPLER_STRIDE>(
			  ...
			  reinterpret_cast<u32k*>(pShader->m_kSource.m_pData + kReflection.m_uSamplersOffset),
			...
			GetResourceIndexRange<DecodeResourceIndex, ENCODED_RESOURCE_STRIDE>(
			  ...
			  reinterpret_cast<u32k*>(pShader->m_kSource.m_pData + kReflection.m_uUniformBuffersOffset),
			...
			GetResourceIndexRange<DecodeResourceIndex, ENCODED_RESOURCE_STRIDE>(
			  ...
			  reinterpret_cast<u32k*>(pShader->m_kSource.m_pData + kReflection.m_uStorageBuffersOffset),
			...
			GetResourceIndexRange<DecodeResourceIndex, ENCODED_RESOURCE_STRIDE>(
			  ...
			  reinterpret_cast<u32k*>(pShader->m_kSource.m_pData + kReflection.m_uImagesOffset),
	...
	for (u32 uResourceType = 0; uResourceType < eRUT_NUM; ++uResourceType)
	{
		u32 uNumPartitions = pDevice->GetNumResourceUnitPartitions((EResourceUnitType)uResourceType);
		for (u32 uPartition = 0; uPartition < uNumPartitions; ++uPartition)
		{
  ...
  const SIndexPartition* pPartition)
{
	...
	u32 uNextTextureUnit = uMapPosition;
	SUnitMap::SElement* pMapElementOut = pMap->m_akUnits + uMapPosition;
	for (u32 uElement = 0; uElement < uNumSamplers; ++uElement)
	{
		u32 uSamplerField = pSamplers[uElement * 3 + 0];
		u32 uEmbeddedNormalName = pSamplers[uElement * 3 + 1];
		u32 uEmbeddedCompareName = pSamplers[uElement * 3 + 2];

		u32 uTextureIndex = DecodeTextureIndex(uSamplerField);
		u32 uSamplerIndex = DecodeSamplerIndex(uSamplerField);
		u32 uTextureUnitIndex = DecodeTextureUnitIndex(uSamplerField);
		...
		u32k uTextureSlot = TextureSlot(eStage, uTextureIndex);
		u32k uSamplerSlot = SamplerSlot(eStage, uSamplerIndex);
		u32k uTextureUnit = pPartition ? pPartition->m_akStages[eStage](uTextureUnitIndex) : uMapPosition + uElement;
...
template<bool pfBindResourceToUnit(GLuint uProgramName, u32 uUnit, tukk szName), u32 pfIndexToSlot(EShaderType eStage, u32 uIndex), u32k slotLimit, u32k unitLimit>
...
{
	char acNameBuffer[MAX_REFLECT_STRING_LENGTH];
	u32 uNextResourceUnit = uMapPosition;
	SUnitMap::SElement* pMapElementOut = pMap->m_akUnits + uMapPosition;
	for (u32 uElement = 0; uElement < uNumResources; ++uElement)
	{
		u32 uResourceField = pResources[uElement * 2 + 0];
		u32 uResourceName = pResources[uElement * 2 + 1];

		u32 uResourceIndex = DecodeResourceIndex(uResourceField);

		u32 uResourceSlot = pfIndexToSlot(eStage, uResourceIndex);
		u32 uResourceUnit = pPartition ? pPartition->m_akStages[eStage](uResourceIndex) : uMapPosition + uElement;
...
bool InitializePipelineResources(SPipeline* pPipeline, CContext* pContext)
{
	...
	u32 auUnitMapPositions[eRUT_NUM];
	...
	for (u32 uShader = 0; uShader < eST_NUM; ++uShader)
	{
		...
		GLuint uProgramName = GetProgramName(pPipeline, pShader);
		tukk szGLSL = pShader->m_kSource.m_pData + pShader->m_kReflection.m_uGLSLSourceOffset;

		// Initialize texture unit map and bind texture units to shader uniforms
		u32 uNumSamplers(pShader->m_kReflection.m_uNumSamplers);
		if (uNumSamplers > 0)
		{
			u32k* pSamplers(reinterpret_cast<u32k*>(pShader->m_kSource.m_pData + pShader->m_kReflection.m_uSamplersOffset));
		...
		u32 uNumUniformBuffers(pShader->m_kReflection.m_uNumUniformBuffers);
		if (uNumUniformBuffers > 0)
		{
			u32k* pUniformBuffers(reinterpret_cast<u32k*>(pShader->m_kSource.m_pData + pShader->m_kReflection.m_uUniformBuffersOffset));
		...
		u32 uNumStorageBuffers(pShader->m_kReflection.m_uNumStorageBuffers);
		if (uNumStorageBuffers > 0)
		{
			u32k* pStorageBuffers(reinterpret_cast<u32k*>(pShader->m_kSource.m_pData + pShader->m_kReflection.m_uStorageBuffersOffset));
		...
		u32 uNumImages(pShader->m_kReflection.m_uNumImages);
		if (uNumImages > 0)
		{
			u32k* pImages(reinterpret_cast<u32k*>(pShader->m_kSource.m_pData + pShader->m_kReflection.m_uImagesOffset));
...
SUnitMap::SUnitMap(u16 uNumUnits)
	...
	: m_uNumUnits(kOther.m_uNumUnits)
{
	...
	else
	{
		...
		NDrxOpenGL::Memcpy(m_akUnits, kOther.m_akUnits, sizeof(SElement) * m_uNumUnits);
	}
}
...
bool PushInputLayoutAttribute(
  ...
  u32 auSlotOffsets[D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT],
  ...
  bool bInteger)
{
	...
	// the input layout
	NDrxOpenGL::EGIFormat eGIFormat(GetGIFormat(kDesc.Format));
	const NDrxOpenGL::SGIFormatInfo* pFormatInfo(NULL);
	...
	u32& uSlotOffset(auSlotOffsets[kDesc.InputSlot]);
