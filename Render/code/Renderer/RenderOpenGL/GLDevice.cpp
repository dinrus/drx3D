...
// -------------------------------------------------------------------------
//  Имя файла:   GLDevice.cpp
//  Version:     v1.00
//  Created:     03/05/2013 by Valerio Guagliumi.
//  Описание: Implementation of the type CDevice and the functions to
...
#include <drx3D/Render/GLResource.hpp>
	...
	#include <drx3D/Render/AppleGPUInfoUtils.h>
#endif
...
#if defined(DEBUG) && !DRX_PLATFORM_MAC
...
SMainWindow SMainWindow::ms_kInstance = { NULL, 0, 0, "", true };
	...
	#endif
void DXGL_DEBUG_CALLBACK_CONVENTION DebugCallback(GLenum eSource, GLenum eType, GLuint uId, GLenum eSeverity, GLsizei uLength, const GLchar* szMessage, uk pUserParam);
  ...
  const TNativeDisplay& kDefaultNativeDisplay)
{
	...
	#if DRX_PLATFORM_WINDOWS && !defined(USE_SDL2_VIDEO)
...
bool InitializeSDLAttributes()
{
	...
	#endif
	#if DRX_RENDERER_OPENGLES && !defined(DXGL_ANDROID_GL)
	iResult = SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_ES);
	#else
	{
...
bool CreateSDLWindowContext(TWindowContext& kWindowContext, tukk szTitle, u32 uWidth, u32 uHeight, bool bFullScreen)
{
	u32 uWindowFlags = SDL_WINDOW_OPENGL;
	if (bFullScreen)
	{
...
bool SetWindowPixelFormat(const TWindowContext& kWindowContext, const SPixelFormatSpec* pPixelFormatSpec)
{
	...
	{
		i32 aiAttributes[128];
		i32* pAttrCursor = aiAttributes;
...
bool ResizeWindowContext(TNativeDisplay kNativeDisplay, u32 uWidth, u32 uHeight)
{
	HWND kWindowHandle(WindowFromDC(kNativeDisplay));
...
SOutput* GetWindowOutput(SAdapter* pAdapter, const TNativeDisplay& kNativeDisplay)
{
	...
	i32 iWindowCenterX((kWindowRect.left + kWindowRect.right) / 2);
	i32 iWindowCenterY((kWindowRect.top + kWindowRect.bottom) / 2);
	...
	for (uOutput = 0; uOutput < pAdapter->m_kOutputs.size(); ++uOutput)
	{
		...
		i32 iDistX(iWindowCenterX - (kDisplayRect.left + kDisplayRect.right) / 2);
		i32 iDistY(iWindowCenterY - (kDisplayRect.top + kDisplayRect.bottom) / 2);
		u32 uCenterDistSqr(iDistX * iDistX + iDistY * iDistY);
...
EGIFormat SDLPixelFormatToGIFormat(u32 uPixelFormat)
{
	switch (uPixelFormat)
	{
...
u32 GIFormatToSDLPixelFormat(EGIFormat eGIFormat)
{
	switch (eGIFormat)
	{
...
#if defined(USE_SDL2_VIDEO) || DRX_PLATFORM_WINDOWS
...
#endif //defined(USE_SDL2_VIDEO) || DRX_PLATFORM_WINDOWS
...
u32 CDevice::ms_uNumContextsPerDevice = 16;
#else
u32 CDevice::ms_uNumContextsPerDevice = 1;
#endif
...
#if !DXGL_FULL_EMULATION
void CDevice::Configure(u32 uNumSharedContexts)
{
	ms_uNumContextsPerDevice = min((u32)MAX_NUM_CONTEXT_PER_DEVICE, 1 + uNumSharedContexts);
}
#endif //!DXGL_FULL_EMULATION

#if defined(USE_SDL2_VIDEO)

bool CDevice::CreateSDLWindow(tukk szTitle, u32 uWidth, u32 uHeight, bool bFullScreen, HWND* pHandle)
{
	#if defined(DXGL_SINGLEWINDOW)
...
CContext* CDevice::AllocateContext()
{
	...
	if (pContext == NULL)
	{
		DXGL_ERROR("CDevice::AllocateContext failed - no free context available. Please increase the number of contexts at initialization time (currently %d).", (i32)m_kContexts.size());
	}
	return pContext;
}
...
NDrxOpenGL::CContext* CDevice::GetCurrentContext()
{
	return static_cast<CContext*>(GetTLSValue(m_pCurrentContextTLS));
}

u32 CDevice::GetMaxContextCount()
{
...
#else
	for (u32 uContext = 0; uContext < m_kContexts.size(); ++uContext)
  ...
  const TNativeDisplay& kNativeDisplay)
{
...
#elif DRX_PLATFORM_WINDOWS && !defined(USE_SDL2_VIDEO)
	i32 aiAttributes[9] =
	{
	...
	for (u32 uContext = 0; uContext < ms_uNumContextsPerDevice; ++uContext)
	{
#if DXGL_USE_ES_EMULATOR
	...
	if (SupportDebugOutput())
	{
		...
		for (u32 uContext = 0; uContext < kRenderingContexts.size(); ++uContext)
		{
			...
			for (u32 uSeverityLevel = 0; uSeverityLevel < DXGL_ARRAY_SIZE(aeSeverityLevels); ++uSeverityLevel)
...
tukk GetResourceUnitTypeName(EResourceUnitType eUnitType)
{
	switch (eUnitType)
	{
	case eRUT_Texture:
...
{
	u32 uTotBelowLimit = 0;
	u32 uTotAvailable = kCapabilities.m_aiMaxTotal;

	u32 uTotUsed = 0;
	for (u32 uStage = 0; uStage < eST_NUM; ++uStage)
	{
	...
	{
		u32 uTotRemaining = uTotAvailable - uTotUsed;
		u32 uTotAssigned = 0;
		for (u32 uStage = 0; uStage < eST_NUM; ++uStage)
		{
			u32 uBelowLimit = kCapabilities.m_aiMaxPerStage[uStage] - kPartition.m_akStages[uStage].m_uCount;
			if (uBelowLimit > 0)
			{
				u32 uAssigned = min(uTotRemaining - uTotAssigned, max(1u, uTotRemaining * uBelowLimit / uTotBelowLimit));
	...
	for (u32 uStage = 0, uFirstSlot = 0; uStage < eST_NUM; ++uStage)
	{
...
{
#if defined(USE_SDL2_VIDEO)
	kNativeDisplay = reinterpret_cast<const NDrxOpenGL::TNativeDisplay>(kWindowHandle);
	return true;
#elif DRX_PLATFORM_WINDOWS
...
bool QueryInternalFormatSupport(GLenum eTarget, GLenum eInternalFormat, GLenum eQueryName, u32 uFlag, u32& uMask)
{
...
void QueryInternalFormatTexSupport(GLenum eTarget, GLenum eInternalFormat, u32 uTexFlag, u32& uMask)
{
...
bool DetectIfCopyImageWorksOnCubeMapFaces()
{
	...
	for (u32 uPixel = 0; uPixel < DXGL_ARRAY_SIZE(auInput); ++uPixel)
	...
	bool bCopied(true);
	for (u32 uFace = 0; uFace < 6; ++uFace)
	{
		u32 uX((uFace % 3) * 4), uY((uFace / 3) * 4);
		...
		for (u32 uRow = 0; uRow < 4; ++uRow)
			NDrxOpenGL::Memcpy(auOutput + (uRow + uY) * 4 * 3 + uX, auFace + uRow * 4, 4 * sizeof(GLuint));
	}
...
template<u32k uPerStageImplementationLimit, u32k uTotalImplementationLimit>
...
bool DetectFeaturesAndCapabilities(TFeatures& kFeatures, SCapabilities& kCapabilities)
{
	...
	for (u32 uGIFormat = 0; uGIFormat < eGIF_NUM; ++uGIFormat)
...
EDriverVendor DetectDriverVendor(tukk szVendorName)
{
	struct
	{
	...
	for (u32 uVendor = 0; uVendor < DXGL_ARRAY_SIZE(akKnownVendors); ++uVendor)
	{
...
bool GetEntryPoint(FunctionPtr& pEntryPoint, tukk szName)
{
...
#elif DRX_PLATFORM_ANDROID && DRX_RENDERER_OPENGLES
...
#endif //DXGL_EXTENSION_LOADER
		{
			spAdapter->m_strRenderer = reinterpret_cast<tukk >(glGetString(GL_RENDERER));
			spAdapter->m_strVendor = reinterpret_cast<tukk >(glGetString(GL_VENDOR));
			spAdapter->m_strVersion = reinterpret_cast<tukk >(glGetString(GL_VERSION));
...
{
#if defined(USE_SDL2_VIDEO)
	i32 uNumDisplays = SDL_GetNumVideoDisplays();
	if (uNumDisplays <= 0)
	{
	...
	u32 uDisplay;
	for (uDisplay = 0; uDisplay < (u32)uNumDisplays; ++uDisplay)
	{
		i32 uNumDisplayModes = SDL_GetNumDisplayModes(uDisplay);
		...
		char acBuffer[32];
		drx_sprintf(acBuffer, "SDL Screen %d", uDisplay);
		spOutput->m_strDeviceName = acBuffer;
		spOutput->m_strDeviceID = spOutput->m_strDeviceName;
		u32 uDisplayMode;
		for (uDisplayMode = 0; uDisplayMode < (u32)uNumDisplayModes; ++uDisplayMode)
		{
...
void DXGL_DEBUG_CALLBACK_CONVENTION DebugCallback(GLenum eSource, GLenum eType, GLuint uId, GLenum eSeverity, GLsizei uLength, tukk szMessage, uk pUserParam)
{
	...
	NDrxOpenGL::LogMessage(eLogSeverity, "[GL_DEBUG] %s [source=0x%X, type=0x%X, id=0x%X]", szMessage, (u32)eSource, (u32)eType, (u32)uId);
}
...
void CallTracePrintf(tukk szFormat, ...)
{
	...
	drx_vsprintf(acBuffer, szFormat, kArgs);
	va_end(kArgs);
