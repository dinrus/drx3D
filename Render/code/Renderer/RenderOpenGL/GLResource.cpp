...
// -------------------------------------------------------------------------
//  Имя файла:   GLResource.cpp
//  Version:     v1.00
//  Created:     30/04/2013 by Valerio Guagliumi.
//  Описание: Implements the resource related functions
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
#include <drx3D/Render/GLDevice.hpp>
...
u32 GetRowPitch(u32 uWidth, u32 uRowBytes, const SGIFormatInfo* pFormatInfo)
{
	u32 uNumElementsPerRow(uRowBytes * pFormatInfo->m_pTexture->m_uBlockWidth / pFormatInfo->m_pTexture->m_uNumBlockBytes);
	return uNumElementsPerRow == uWidth ? 0 : uNumElementsPerRow;
}

u32 GetImagePitch(u32 uHeight, u32 uImageBytes, u32 uRowBytes)
{
	u32 uNumRowsPerImage(uImageBytes / uRowBytes);
...
{
	u32 uSubresource(D3D11CalcSubresource(kSubID.m_iMipLevel, kSubID.m_uElement, pTexture->m_uNumMipLevels));
...
struct SDefaultTex1DBase : SDefaultTexBase, STex1DBase
{
	...
	static void ApplyUnpackState(CContext* pContext, STexSize kSize, u32 uLogDataAlignment, u32 uRowPitch, u32, const SGIFormatInfo* pFormat)
	{
		pContext->SetUnpackAlignment(std::min<i32>(MAX_UNPACK_ALIGNMENT, 1 << uLogDataAlignment));
	}

	static void ApplyPackState(CContext* pContext, STexSize kSize, u32 uLogDataAlignment, u32 uRowPitch, u32, const SGIFormatInfo* pFormat)
	{
		pContext->SetPackAlignment(std::min<i32>(MAX_PACK_ALIGNMENT, 1 << uLogDataAlignment));
	}
};
...
struct SDefaultTex2DBase : SDefaultTexBase, STex2DBase
{
	...
	static void ApplyUnpackState(CContext* pContext, STexSize kSize, u32 uLogDataAlignment, u32 uRowPitch, u32, const SGIFormatInfo* pFormat)
	{
		...
		pContext->SetUnpackAlignment(std::min<i32>(MAX_UNPACK_ALIGNMENT, 1 << std::min<i32>(uLogDataAlignment, countTrailingZeros32(uRowPitch))));
	}

	static void ApplyPackState(CContext* pContext, STexSize kSize, u32 uLogDataAlignment, u32 uRowPitch, u32, const SGIFormatInfo* pFormat)
	{
		pContext->SetPackRowLength(GetRowPitch(kSize.x, uRowPitch, pFormat));
		pContext->SetPackAlignment(std::min<i32>(MAX_PACK_ALIGNMENT, 1 << std::min<i32>(uLogDataAlignment, countTrailingZeros32(uRowPitch))));
	}
...
struct SDefaultTex3DBase : SDefaultTexBase, STex3DBase
{
	...
	static void ApplyUnpackState(CContext* pContext, STexSize kSize, u32 uLogDataAlignment, u32 uRowPitch, u32 uImagePitch, const SGIFormatInfo* pFormat)
	{
		pContext->SetUnpackRowLength(GetRowPitch(kSize.x, uRowPitch, pFormat));
		pContext->SetUnpackImageHeight(GetImagePitch(kSize.y, uImagePitch, uRowPitch));
		pContext->SetUnpackAlignment(std::min<i32>(MAX_UNPACK_ALIGNMENT, 1 << std::min<i32>(uLogDataAlignment, std::min<i32>(countTrailingZeros32(uRowPitch), countTrailingZeros32(uImagePitch)))));
	}

	static void ApplyPackState(CContext* pContext, STexSize kSize, u32 uLogDataAlignment, u32 uRowPitch, u32 uImagePitch, const SGIFormatInfo* pFormat)
	{
		pContext->SetPackRowLength(GetRowPitch(kSize.x, uRowPitch, pFormat));
		pContext->SetPackImageHeight(GetImagePitch(kSize.y, uImagePitch, uRowPitch));
		pContext->SetPackAlignment(std::min<i32>(MAX_PACK_ALIGNMENT, 1 << std::min<i32>(uLogDataAlignment, std::min<i32>(countTrailingZeros32(uRowPitch), countTrailingZeros32(uImagePitch)))));
	}
...
struct STexUncompressed
{
	...
	static bool GetPackedRange(const STexBox& kPixels, STexBox* pPackedRange, const SGIFormatInfo* pFormat)
	{
		u32 uPixelBytes(pFormat->m_pTexture->m_uNumBlockBytes);
...
{
	static void GetSubTargetAndLayer(STexture* pTexture, u32 uElement, GLenum& eTarget, GLint& iLayer)
	{
		eTarget = pTexture->m_eTarget;
		iLayer = (GLint)uElement;
	}

	static u32 GetNumLayers(STexture* pTexture)
	{
...
void TestUploadBounds(STexture* pTexture, GLenum eTarget, GLint iLevel, STexBox kBox, const SGIFormatInfo* pFormat, const GLvoid* pData, u32 uSrcRowPitch, u32 uSrcDepthPitch)
{
	static u32 s_uPageSize = GetPageSize();
	static std::map<u32, tuk> s_kBuffers;
	static u32 s_uNumBufferPages = 0;
	...
	u32 uRequiredPages(2 + (uSize + s_uPageSize) / s_uPageSize);

	tuk pBuffer;
	std::map<u32, tuk>::iterator kFound(s_kBuffers.find(uRequiredPages));
	if (kFound != s_kBuffers.end())
		pBuffer = kFound->second;
	else
	{
	#if DRX_PLATFORM_WINDOWS
		pBuffer = reinterpret_cast<tuk>(VirtualAlloc(NULL, uRequiredPages * s_uPageSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
		DWORD uUnused;
	...
	#else
		pBuffer = reinterpret_cast<tuk>(mmap(NULL, uRequiredPages * s_uPageSize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE, -1, 0));
...
template<typename Interface>
void TestUploadBounds(STexture*, GLenum, GLint, STexBox, const SGIFormatInfo*, const GLvoid*, u32, u32) {}
...
{
	static void InitializeStorage(STexture* pTexture, u32, const SGIFormatInfo* pFormat, CContext* pContext)
	{
	...
	static void UploadImage(STexture* pTexture, STexSubresourceID kSubID, STexBox kBox, ukk pSrcData, u32 uSrcRowPitch, u32 uSrcDepthPitch, CContext* pContext, const SGIFormatInfo* pFormat)
	{
		GLenum eSubTarget;
		GLint iLayer;
		...
		Interface::ApplyUnpackState(pContext, kBox.m_kSize, countTrailingZeros32((u32)reinterpret_cast<uintptr_t>(pSrcData)), uSrcRowPitch, uSrcDepthPitch, pFormat);
	...
	static void DownloadImage(STexture* pTexture, STexSubresourceID kSubID, STexBox kBox, uk pDstData, u32 uDstRowPitch, u32 uDstDepthPitch, CContext* pContext, const SGIFormatInfo* pFormat)
	{
		Interface::ApplyPackState(pContext, kBox.m_kSize, countTrailingZeros32((u32)reinterpret_cast<uintptr_t>(pDstData)), uDstRowPitch, uDstDepthPitch, pFormat);
	...
	static void Map(STexture* pTexture, STexSubresourceID kSubID, bool bDownload, SMappedSubTexture& kMappedSubTex, CContext* pContext, const SGIFormatInfo* pFormat)
	{
		...
		kMappedSubTex.m_pBuffer = static_cast<u8*>(Memalign(kPackedLayout.m_uTextureSize, MIN_MAPPED_RESOURCE_ALIGNMENT));
...
template<typename Interface>
u32 GetSystemMemoryTextureOffset(STexture* pTexture, const SGIFormatInfo* pFormat, STexSubresourceID kID)
{
	u32 uOffset(0);
	u32 uTotSize(0);
	u32 uLevel;
...
struct SStagingTexImpl : Interface
{
	static void AllocateResource(STexture* pTexture, CDevice* pDevice)
	{
		...
		u32 uNumPixelBuffers(pTexture->m_uNumMipLevels * pTexture->m_uNumElements);
		pTexture->m_akPixelBuffers = new CResourceName[uNumPixelBuffers];
		for (u32 uPixelBuffer = 0; uPixelBuffer < uNumPixelBuffers; ++uPixelBuffer)
		{
	...
	static void InitializeStorage(STexture* pTexture, u32 uCPUAccess, const SGIFormatInfo* pFormat, CContext* pContext)
	{
#if DXGL_USE_PBO_FOR_STAGING_TEXTURES
		GLenum eUsage;
		...
		u32 uMappedSize(GetSystemMemoryTextureOffset<Interface>(pTexture, pFormat, kEndID));
		pTexture->m_pSystemMemoryCopy = static_cast<u8*>(Memalign(uMappedSize, MIN_MAPPED_RESOURCE_ALIGNMENT));
#endif
	}
	...
	static void UploadImage(STexture* pTexture, STexSubresourceID kSubID, STexBox kBox, ukk pSrcData, u32 uSrcRowPitch, u32 uSrcDepthPitch, CContext* pContext, const SGIFormatInfo* pFormat)
	{
		...
		u8* puDstData(static_cast<u8*>(pvMappedData) + kPackedRange.m_kOffset.x);
#else
		u8* puDstData(
		...
		u8k* puSrcData(reinterpret_cast<u8k*>(pSrcData));
...
#if DXGL_USE_PBO_FOR_STAGING_TEXTURES
	static void DownloadImage(STexture* pTexture, STexSubresourceID kSubID, STexBox kBox, ukk pDstData, u32 uSrcRowPitch, u32 uSrcDepthPitch, CContext* pContext, const SGIFormatInfo* pFormat)
	{
		DXGL_NOT_IMPLEMENTED
	}
#endif //DXGL_USE_PBO_FOR_STAGING_TEXTURES
	...
	static void Map(STexture* pTexture, STexSubresourceID kSubID, bool bDownload, SMappedSubTexture& kMappedSubTex, CContext* pContext, const SGIFormatInfo* pFormat)
	{
		...
		GLint iPixelBufferName(pTexture->m_akPixelBuffers[uSubResource].GetName());
		if (bDownload)
			kMappedSubTex.m_pBuffer = static_cast<u8*>(glMapNamedBufferRangeEXT(iPixelBufferName, 0, kPackedLayout.m_uTextureSize, GL_MAP_READ_BIT));
		else
			kMappedSubTex.m_pBuffer = static_cast<u8*>(glMapNamedBufferRangeEXT(iPixelBufferName, 0, kPackedLayout.m_uTextureSize, GL_MAP_WRITE_BIT));
#else
...
inline STexSubresourceID GetTexSubresourceID(STexture* pTexture, u32 uSubresource)
{
...
void UpdateTexSubresource(SResource* pResource, u32 uSubresource, const D3D11_BOX* pDstBox, ukk pSrcData, u32 uSrcRowPitch, u32 uSrcDepthPitch, CContext* pContext)
{
...
template<typename Impl>
bool MapTexSubresource(SResource* pResource, u32 uSubresource, D3D11_MAP eMapType, u32 uMapFlags, D3D11_MAPPED_SUBRESOURCE* pMappedResource, CContext* pContext)
{
...
template<typename Impl>
u32 LocateTexPackedData(STexture* pTexture, STexSubresourceID kSubID, STexPos kOffset, SMappedSubTexture& kDataLocation)
{
	const SGIFormatInfo* pFormat(GetGIFormatInfo(pTexture->m_eFormat));
...
template<typename Impl>
void InitializeTexture(STexture* pTexture, const D3D11_SUBRESOURCE_DATA* pInitialData, u32 uCPUAccess, CContext* pContext, const SGIFormatInfo* pFormatInfo)
{
...
GLenum SFrameBufferConfiguration::AttachmentIndexToID(u32 uIndex)
{
...
u32 SFrameBufferConfiguration::AttachmentIDToIndex(GLenum eID)
{
...
CResourceName SResourceNamePool::Create(GLuint uName)
{
	...
	u32 uNewBlockSize(m_pBlocksHead == NULL ? MIN_BLOCK_SIZE : alias_cast<TBlockHeader*>(m_pBlocksHead)->m_uSize * GROWTH_FACTOR);
...
STexture::STexture(GLsizei iWidth, GLsizei iHeight, GLsizei iDepth, GLenum eTarget, EGIFormat eFormat, u32 uNumMipLevels, u32 uNumElements)
	: m_iWidth(iWidth)
	, m_iHeight(iHeight)
	, m_iDepth(iDepth)
	, m_eTarget(eTarget)
...
#if DXGL_SUPPORT_APITRACE && DRX_PLATFORM_WINDOWS

	#if DRX_PLATFORM_WINDOWS
u32 SDynamicBufferCopy::ms_uPageSize = 0;
...
u8* SDynamicBufferCopy::Allocate(u32 uSize)
{
	m_uSize = uSize;
	#if DRX_PLATFORM_WINDOWS
	if (ms_uPageSize == 0)
	...
	m_pData = reinterpret_cast<u8*>(VirtualAlloc(NULL, m_uNumPages * ms_uPageSize, MEM_COMMIT | MEM_RESERVE | MEM_WRITE_WATCH, PAGE_READWRITE));
	m_apDirtyPages = new uk [m_uNumPages];
	return m_pData;
	#else
...
bool SDynamicBufferCopy::GetDirtyRegion(u32& uOffset, u32& uSize)
{
	#if DRX_PLATFORM_WINDOWS
	ULONG uGranularity;
	...
	u32 uMin(0xFFFFFFFF), uMax(0);
	u32 uChange;
	for (uChange = 0; uChange < uCount; ++uChange)
	{
		assert(m_apDirtyPages[uChange] >= m_pData);
		u32 uOffset((u32)(reinterpret_cast<tuk>(m_apDirtyPages[uChange]) - reinterpret_cast<tuk>(m_pData)));
...
SShaderViewPtr SBuffer::CreateShaderResourceView(EGIFormat eFormat, u32 uFirstElement, u32 uNumElements, u32 uFlags, CContext* pContext)
{
...
SShaderViewPtr SBuffer::CreateUnorderedAccessView(EGIFormat eFormat, u32 uFirstElement, u32 uNumElements, u32 uFlags, CContext* pContext)
{
	DXGL_TODO("Consider uFlags as well if required")
...
SShaderBufferViewPtr SBuffer::CreateBufferView(u32 uFirstElement, u32 uNumElements, CContext* pContext)
{
...
bool SOcclusionQuery::GetData(uk pData, u32 uDataSize, bool bFlush)
{
...
u32 SOcclusionQuery::GetDataSize()
{
	return (u32)sizeof(UINT64);
}
...
bool SFenceSync::GetData(uk pData, u32 uDataSize, bool bFlush)
{
	assert(m_pFence != NULL);
...
u32 SFenceSync::GetDataSize()
{
	return (u32)sizeof(BOOL);
}

#if DXGL_SUPPORT_TIMER_QUERIES

bool                                   STimestampDisjointQuery::GetData(uk pData, u32 uDataSize, bool bFlush)
{
...
u32 STimestampDisjointQuery::GetDataSize()
{
	return (u32)sizeof(D3D11_QUERY_DATA_TIMESTAMP_DISJOINT);
}
...
bool STimestampQuery::GetData(uk pData, u32 uDataSize, bool bFlush)
{
	GLint iAvailable;
...
u32 STimestampQuery::GetDataSize()
{
	return (u32)sizeof(UINT64);
}
...
void SDefaultFrameBufferTexture::UpdateSubresource(SResource* pResource, u32 uSubresource, const D3D11_BOX* pDstBox, ukk pSrcData, u32 uSrcRowPitch, u32 uSrcDepthPitch, CContext* pContext)
{
...
bool SDefaultFrameBufferTexture::MapSubresource(SResource* pResource, u32 uSubresource, D3D11_MAP MapType, UINT MapFlags, D3D11_MAPPED_SUBRESOURCE* pMappedResource, CContext* pContext)
{
	SDefaultFrameBufferTexture* pDefaultFrameBufferTexture = static_cast<SDefaultFrameBufferTexture*>(pResource);
...
void SDefaultFrameBufferTexture::UnmapSubresource(SResource* pResource, u32 uSubresource, CContext* pContext)
{
	SDefaultFrameBufferTexture* pDefaultFrameBufferTexture = static_cast<SDefaultFrameBufferTexture*>(pResource);
...
u32 SDefaultFrameBufferTexture::LocatePackedData(STexture* pTexture, STexSubresourceID kSubID, STexPos kOffset, SMappedSubTexture& kDataLocation)
{
	DXGL_TODO(
	  "Currently only the texture FBO case is handled so we have to make sure that the texture "
	  "exists during this function call. The default framebuffer case should be supported too.");
...
SInitialDataCopy::SInitialDataCopy(const D3D11_SUBRESOURCE_DATA* akSubresourceData, u32 uNumSubresources)
	: m_uNumSubresources(uNumSubresources)
{
	...
	{
		u32 uDataSize = uNumSubresources * sizeof(*m_akSubresourceData);
...
{
	for (u32 uSubresource = 0; uSubresource < m_uNumSubresources; ++uSubresource)
		Free(const_cast<uk>(m_akSubresourceData[uSubresource].pSysMem));
...
SInitialDataCopyPtr CreateTexture1DInitialDataCopy(const D3D11_TEXTURE1D_DESC& kDesc, const D3D11_SUBRESOURCE_DATA* pInitialData)
{
	...
	u32 uNumSubresources = kDesc.MipLevels * kDesc.ArraySize;
	SInitialDataCopyPtr spInitialDataCopy = new SInitialDataCopy(pInitialData, uNumSubresources);

	for (u32 uSubresource = 0; uSubresource < uNumSubresources; ++uSubresource)
	{
		u32 uMipLevel = uSubresource % kDesc.MipLevels;
		u32 uWidth = kDesc.Width >> uMipLevel;

		u32 uBlockWidth = pFormatInfo->m_pTexture->m_uBlockWidth;
		u32 uBlockSize = pFormatInfo->m_pTexture->m_uNumBlockBytes;

		u32 uNumBlocks = (uWidth + uBlockWidth - 1) / uBlockWidth;

		u32 uRowSize = uBlockSize * uNumBlocks;

		uk pDataCopy = Malloc(uRowSize);
...
SInitialDataCopyPtr CreateTexture2DInitialDataCopy(const D3D11_TEXTURE2D_DESC& kDesc, const D3D11_SUBRESOURCE_DATA* pInitialData)
{
	...
	for (u32 uSubresource = 0; uSubresource < uNumSubresources; ++uSubresource)
	{
		u32 uMipLevel = uSubresource % kDesc.MipLevels;
		u32 uWidth = kDesc.Width >> uMipLevel;
		u32 uHeight = kDesc.Height >> uMipLevel;

		u32 uBlockWidth = pFormatInfo->m_pTexture->m_uBlockWidth;
		u32 uBlockHeight = pFormatInfo->m_pTexture->m_uBlockHeight;
		u32 uBlockSize = pFormatInfo->m_pTexture->m_uNumBlockBytes;

		u32 uNumBlocks = (uWidth + uBlockWidth - 1) / uBlockWidth;
		u32 uNumRows = (uHeight + uBlockHeight - 1) / uBlockHeight;

		u32 uRowPitch = pInitialData[uSubresource].SysMemPitch;

		u32 uRowSize = uBlockSize * uNumBlocks;
		u32 uAllocSize = uRowPitch * uNumRows;
		u32 uUsedSize = uAllocSize - uRowPitch + uRowSize;

		uk pDataCopy = Malloc(uAllocSize);
...
SInitialDataCopyPtr CreateTexture3DInitialDataCopy(const D3D11_TEXTURE3D_DESC& kDesc, const D3D11_SUBRESOURCE_DATA* pInitialData)
{
	...
	u32 uNumSubresources = kDesc.MipLevels;
	SInitialDataCopyPtr spInitialDataCopy = new SInitialDataCopy(pInitialData, uNumSubresources);

	for (u32 uSubresource = 0; uSubresource < uNumSubresources; ++uSubresource)
	{
		u32 uMipLevel = uSubresource;
		u32 uWidth = kDesc.Width >> uMipLevel;
		u32 uHeight = kDesc.Height >> uMipLevel;
		u32 uDepth = kDesc.Depth >> uMipLevel;

		u32 uBlockWidth = pFormatInfo->m_pTexture->m_uBlockWidth;
		u32 uBlockHeight = pFormatInfo->m_pTexture->m_uBlockHeight;
		u32 uBlockDepth = pFormatInfo->m_pTexture->m_uBlockDepth;
		u32 uBlockSize = pFormatInfo->m_pTexture->m_uNumBlockBytes;

		u32 uNumBlocks = (uWidth + uBlockWidth - 1) / uBlockWidth;
		u32 uNumRows = (uHeight + uBlockHeight - 1) / uBlockHeight;
		u32 uNumSlices = (uDepth + uBlockDepth - 1) / uBlockDepth;

		u32 uRowPitch = pInitialData[uSubresource].SysMemPitch;
		u32 uSlicePitch = pInitialData[uSubresource].SysMemSlicePitch;

		u32 uRowSize = uBlockSize * uNumBlocks;
		u32 uSliceSize = uRowPitch * uNumRows;
		u32 uAllocSize = uSlicePitch * uNumSlices;
		u32 uUsedSize = uAllocSize - uSlicePitch + uSliceSize;
...
SInitialDataCopyPtr CreateBufferInitialDataCopy(const D3D11_BUFFER_DESC& kDesc, const D3D11_SUBRESOURCE_DATA* pInitialData)
{
	...
	uk pDataCopy = Malloc(kDesc.ByteWidth);
...
STexturePtr CreateTexture1D(const D3D11_TEXTURE1D_DESC& kDesc, const D3D11_SUBRESOURCE_DATA* pInitialData, CContext* pContext)
{
	...
	u32 uNumElements(kDesc.ArraySize);
...
void InitializeTexture2D(STexture* pTexture, bool bArray, bool bStaging, const D3D11_SUBRESOURCE_DATA* pInitialData, u32 uCPUAccess, CContext* pContext, const SGIFormatInfo* pFormatInfo)
{
	if (bStaging)
	{
		if (pFormatInfo->m_pTexture->m_bCompressed)
...
void glNamedBufferSubDataAsync(GLuint uBuffer, GLintptr iOffset, GLsizeiptr iSize, const GLvoid* pvData)
{
	...
	while (iSize > 0)
	{
		u32 uUploadSize((u32)min(iSize, (GLintptr)MAX_UPLOAD_SIZE));
		glNamedBufferSubDataEXT(uBuffer, iOffset, iSize, pvData);
		iOffset += uUploadSize;
		iSize -= uUploadSize;
		pvData = static_cast<u8k*>(pvData) + uUploadSize;
	}
#else
	glNamedBufferSubDataEXT(uBuffer, iOffset, iSize, pvData);
#endif
...
{
	static void UpdateBufferSubresource(SResource* pResource, u32 uSubresource, const D3D11_BOX* pDstBox, ukk pSrcData, u32, u32, CContext* pContext)
	{
...
{
	static bool MapBufferRange(SBuffer* pBuffer, size_t uOffset, size_t uSize, D3D11_MAP eMapType, u32 uMapFlags, D3D11_MAPPED_SUBRESOURCE* pMappedResource, CContext* pContext)
	{
	...
	static bool MapBuffer(SResource* pResource, u32 uSubresource, D3D11_MAP eMapType, u32 uMapFlags, D3D11_MAPPED_SUBRESOURCE* pMappedResource, CContext* pContext)
	{
	...
	static void UpdateBufferSubresource(SResource* pResource, u32 uSubresource, const D3D11_BOX* pDstBox, ukk pSrcData, u32, u32, CContext* pContext)
	{
		...
		NDrxOpenGL::Memcpy(pBuffer->m_pSystemMemoryCopy, static_cast<u8k*>(pSrcData) + uDstOffset, uDstSize);
	}
};

void MapBufferSystemMemoryRange(SBuffer* pBuffer, u32 uOffset, D3D11_MAPPED_SUBRESOURCE* pMappedResource)
{
...
void UpdateBufferSystemMemory(SBuffer* pBuffer, const D3D11_BOX* pDstBox, ukk pSrcData)
{
	if (pDstBox != NULL)
		NDrxOpenGL::Memcpy(pBuffer->m_pSystemMemoryCopy + pDstBox->left, pSrcData, pDstBox->right - pDstBox->left);
	else
		NDrxOpenGL::Memcpy(pBuffer->m_pSystemMemoryCopy, pSrcData, pBuffer->m_uSize);
}

struct SStagingBufferImpl
{
	static bool MapBufferRange(SBuffer* pBuffer, size_t uOffset, size_t uSize, D3D11_MAP, u32, D3D11_MAPPED_SUBRESOURCE* pMappedResource, CContext*)
	{
	...
	static bool MapBuffer(SResource* pResource, u32, D3D11_MAP, u32, D3D11_MAPPED_SUBRESOURCE* pMappedResource, CContext*)
	{
	...
	static void UpdateBufferSubresource(SResource* pResource, u32, const D3D11_BOX* pDstBox, ukk pSrcData, u32, u32, CContext*)
	{
...
{
	static bool MapBufferRange(SBuffer* pBuffer, size_t uOffset, size_t uSize, D3D11_MAP eMap, u32, D3D11_MAPPED_SUBRESOURCE* pMappedResource, CContext*)
	{
		DXGL_SCOPED_PROFILE("SStagingBufferImpl::MapBufferRange")
		MapBufferSystemMemoryRange(pBuffer, uOffset, pMappedResource);
	...
	static bool MapBuffer(SResource* pResource, u32, D3D11_MAP eMap, u32, D3D11_MAPPED_SUBRESOURCE* pMappedResource, CContext*)
	{
		DXGL_SCOPED_PROFILE("SStagingBufferImpl::MapBuffer")
		SBuffer * pBuffer(static_cast<SBuffer*>(pResource));
...
SBufferPtr CreateBuffer(const D3D11_BUFFER_DESC& kDesc, const D3D11_SUBRESOURCE_DATA* pInitialData, CContext* pContext)
{
	...
	if (bAllocateSystemMemory)
		spBuffer->m_pSystemMemoryCopy = static_cast<u8*>(Memalign(kDesc.ByteWidth, MIN_MAPPED_RESOURCE_ALIGNMENT));

	if (spBuffer->m_pSystemMemoryCopy != NULL && pInitialData != NULL)
		NDrxOpenGL::Memcpy(spBuffer->m_pSystemMemoryCopy, pInitialData->pSysMem, kDesc.ByteWidth);
  ...
  STexSize kBoxSize, CContext* pContext)
{
	...
	u32 uPackedSize(pSrcTexture->m_pfLocatePackedDataFunc(pSrcTexture, kSrcSubID, kSrcPos, kDataLocation));
...
void CopyVideoToSystemTextureBox(
  ...
  STexSize kBoxSize, CContext* pContext)
{
	u32 uDstSubResource(D3D11CalcSubresource(kDstSubID.m_iMipLevel, kDstSubID.m_uElement, pDstTexture->m_uNumMipLevels));
...
void CopySystemToVideoTextureBox(
  ...
  STexSize kBoxSize, CContext* pContext)
{
	u32 uSrcSubResource(D3D11CalcSubresource(kSrcSubID.m_iMipLevel, kSrcSubID.m_uElement, pSrcTexture->m_uNumMipLevels));
...
void CopySubTextureImpl(
  STexture* pDstTexture, u32 uDstSubresource, u32 uDstX, u32 uDstY, u32 uDstZ,
  STexture* pSrcTexture, u32 uSrcSubresource, const D3D11_BOX* pSrcBox, CContext* pContext)
{
...
void CopySubBufferInternal(SBuffer* pDstBuffer, SBuffer* pSrcBuffer, u32 uDstOffset, u32 uSrcOffset, u32 uSize)
{
	if (pSrcBuffer->m_pSystemMemoryCopy != NULL && pDstBuffer->m_pSystemMemoryCopy != NULL)
		NDrxOpenGL::Memcpy(pDstBuffer->m_pSystemMemoryCopy + uDstOffset, pSrcBuffer->m_pSystemMemoryCopy + uSrcOffset, uSize);
	...
	else if (pSrcBuffer->m_kName.IsValid() && pSrcBuffer->m_pSystemMemoryCopy == NULL)
	{
		...
		NDrxOpenGL::Memcpy(pDstBuffer->m_pSystemMemoryCopy + uDstOffset, pvData, uSize);
...
void CopySubBuffer(
  SBuffer* pDstBuffer, u32 uDstSubresource, u32 uDstX, u32, u32,
  SBuffer* pSrcBuffer, u32 uSrcSubresource, const D3D11_BOX* pSrcBox, CContext* pContext)
{
	DXGL_SCOPED_PROFILE("CopySubBuffer")

	u32 uSrcBegin, uSrcEnd;
	if (pSrcBox != NULL)
	{
