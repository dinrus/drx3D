...
//  Описание: Implements the resource view related functions
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
CResourceName CreateTextureBuffer(SBuffer* pBuffer, const SGIFormatInfo* pFormatInfo, u32 uFirstElement, u32 uNumElements, CContext* pContext)
{
#if DXGL_SUPPORT_TEXTURE_BUFFERS
	u32 uElementSize(pFormatInfo->m_pTexture->m_uNumBlockBytes);
	u32 uViewSize(uNumElements * uElementSize);
...
bool SShaderTextureBasedView::BindTextureUnit(SSamplerState*, STextureUnitContext&, CContext*)
{
...
bool SShaderTextureBufferView::BindTextureUnit(SSamplerState* pSamplerState, STextureUnitContext& kContext, CContext* pContext)
{
...
bool SShaderTextureView::BindTextureUnit(SSamplerState* pSamplerState, STextureUnitContext& kContext, CContext* pContext)
{
	...
	if (!m_bTextureOwned)
	{
		if (m_kConfiguration.m_eTarget == m_pTexture->m_eTarget)
		{
			...
			if (m_pTexture->m_pBoundModifier == NULL)
			{
				if (m_pTexture->m_kCache != m_kViewState)
				{
					m_kViewState.Apply(m_pTexture->m_kName.GetName(), m_kConfiguration.m_eTarget);
...
SOutputMergerView::~SOutputMergerView()
{
	...
	for (uContext = 0; uContext < DXGL_ARRAY_SIZE(m_kContextMap); ++uContext)
	{
...
#endif
		{
			...
			{
				if (--kFrameBufferRefIter->m_uNumBindings == 0)
				{
...
bool SDefaultFrameBufferShaderTextureView::BindTextureUnit(SSamplerState* pSamplerState, STextureUnitContext& kContext, CContext* pContext)
{
	...
	return SShaderTextureBasedView::BindTextureUnit(pSamplerState, kContext, pContext);
}
...
struct SShaderResourceViewImpl : SResourceViewBaseImpl<D3D11_SHADER_RESOURCE_VIEW_DESC, SShaderView>
{
	enum EViewDimension
	{
	...
	static SResourceViewBaseImpl::TViewPtr GetView(STexture* pTexture, DXGI_FORMAT eDXGIFormat, GLenum eTarget, u32 uMinLevel, u32 uNumLevels, u32 uMinElement, u32 uNumElements, CContext* pContext)
	{
	...
	static TViewPtr GetViewMip(STexture* pTexture, const DimDesc& kDimDesc, DXGI_FORMAT eDXGIFormat, GLenum eTarget, u32 uMinElement, u32 uNumElements, CContext* pContext)
	{
		u32 uNumMipLevels(kDimDesc.MipLevels == -1 ? (pTexture->m_uNumMipLevels - kDimDesc.MostDetailedMip) : kDimDesc.MipLevels);
		return GetView(pTexture, eDXGIFormat, eTarget, kDimDesc.MostDetailedMip, uNumMipLevels, uMinElement, uNumElements, pContext);
	}

	template<typename DimDesc>
	static TViewPtr GetViewLayers(STexture* pTexture, const DimDesc& kDimDesc, DXGI_FORMAT eDXGIFormat, u32 uMinLevel, u32 uNumLevels, GLenum eTarget, CContext* pContext)
	{
		u32 uNumElements(kDimDesc.ArraySize == -1 ? (pTexture->m_uNumElements - kDimDesc.FirstArraySlice) : kDimDesc.ArraySize);
...
struct SUnorderedAccessViewImpl : SResourceViewBaseImpl<D3D11_UNORDERED_ACCESS_VIEW_DESC, SShaderView>
{
	enum EViewDimension
	{
	...
	static SResourceViewBaseImpl::TViewPtr GetView(STexture* pTexture, DXGI_FORMAT eDXGIFormat, GLenum eTarget, u32 uMipSlice, u32 uMinElement, u32 uNumElements, CContext* pContext)
	{
	...
	template<typename DimDesc>
	static TViewPtr GetViewLayers(STexture* pTexture, const DimDesc& kDimDesc, DXGI_FORMAT eDXGIFormat, u32 uMipSlice, GLenum eTarget, CContext* pContext)
	{
		u32 uNumElements(kDimDesc.ArraySize == -1 ? (pTexture->m_uNumElements - kDimDesc.FirstArraySlice) : kDimDesc.ArraySize);
...
struct SOutputMergerViewImpl : SResourceViewBaseImpl<ViewDesc, SOutputMergerView>
{
	static typename SOutputMergerViewImpl::TViewPtr GetView(STexture* pTexture, DXGI_FORMAT eDXGIFormat, GLenum, u32 uMipLevel, u32, u32 uMinElement, u32 uNumElements, CContext* pContext)
	{
	...
	template<typename DimDesc>
	static typename SOutputMergerViewImpl::TViewPtr GetViewMip(STexture* pTexture, const DimDesc& kDimDesc, DXGI_FORMAT eDXGIFormat, GLenum eTarget, u32 uMinElement, u32 uNumElements, CContext* pContext)
	{
		return GetView(pTexture, eDXGIFormat, eTarget, kDimDesc.MipSlice, 1, uMinElement, uNumElements, pContext);
	}

	template<typename DimDesc>
	static typename SOutputMergerViewImpl::TViewPtr GetViewLayers(STexture* pTexture, const DimDesc& kDimDesc, DXGI_FORMAT eDXGIFormat, GLenum eTarget, u32 uMinLevel, u32 uNumLevels, CContext* pContext)
	{
		u32 uNumElements(kDimDesc.ArraySize == -1 ? (pTexture->m_uNumElements - kDimDesc.FirstArraySlice) : kDimDesc.ArraySize);
		return GetView(pTexture, eDXGIFormat, eTarget, uMinLevel, uNumLevels, kDimDesc.FirstArraySlice, uNumElements, pContext);
	}
...
struct SRenderTargetViewImpl : SOutputMergerViewImpl<D3D11_RENDER_TARGET_VIEW_DESC>
{
	enum EViewDimension
	{
...
struct SDepthStencilViewImpl : SOutputMergerViewImpl<D3D11_DEPTH_STENCIL_VIEW_DESC>
{
	enum EViewDimension
	{
...
{
	if ((u32)kViewDesc.ViewDimension == (u32)Impl::DIMENSION_TEXTURE3D)
...
{
	if ((u32)kViewDesc.ViewDimension == (u32)Impl::DIMENSION_BUFFER)
...
{
	if ((u32)kViewDesc.ViewDimension == (u32)Impl::DIMENSION_BUFFEREX)
