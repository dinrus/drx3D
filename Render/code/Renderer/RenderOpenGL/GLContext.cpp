...
// -------------------------------------------------------------------------
//  Имя файла:   GLContext.cpp
//  Version:     v1.00
//  Created:     25/04/2013 by Valerio Guagliumi.
//  Описание: Implementation of the type CContext.
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
i32 SGlobalConfig::iStreamingConstantBuffersMode = 0;
i32 SGlobalConfig::iStreamingConstantBuffersPersistentMap = 0;
i32 SGlobalConfig::iStreamingConstantBuffersGranularity = 0;
i32 SGlobalConfig::iStreamingConstantBuffersGrowth = 0;
i32 SGlobalConfig::iStreamingConstantBuffersMaxUnits = 0;
#endif
i32 SGlobalConfig::iMinFramePoolSize = 0;
i32 SGlobalConfig::iMaxFramePoolSize = 0;
i32 SGlobalConfig::iBufferUploadMode = 0;
#if DXGL_ENABLE_SHADER_TRACING
i32 SGlobalConfig::iShaderTracingMode = 0;
i32 SGlobalConfig::iShaderTracingHash = 0;
i32 SGlobalConfig::iVertexTracingID = 0;
i32 SGlobalConfig::iPixelTracingX = 0;
i32 SGlobalConfig::iPixelTracingY = 0;
i32 SGlobalConfig::iComputeTracingWorkGroupIDX = 0;
i32 SGlobalConfig::iComputeTracingWorkGroupIDY = 0;
i32 SGlobalConfig::iComputeTracingWorkGroupIDZ = 0;
i32 SGlobalConfig::iComputeTracingLocalInvocationIDX = 0;
i32 SGlobalConfig::iComputeTracingLocalInvocationIDY = 0;
i32 SGlobalConfig::iComputeTracingLocalInvocationIDZ = 0;
#endif
...
inline void GetStateVari(GLenum eTarget, u32 uIndex, GLint* pData)     { glGetIntegeri_v(eTarget, uIndex, pData); }
inline void GetStateVari(GLenum eTarget, u32 uIndex, GLenum* pData)    { glGetIntegeri_v(eTarget, uIndex, reinterpret_cast<GLint*>(pData)); }
#if DXGL_SUPPORT_INDEXED_BOOL_STATE
inline void GetStateVari(GLenum eTarget, u32 uIndex, GLboolean* pData) { glGetBooleani_v(eTarget, uIndex, pData); }
#endif //DXGL_SUPPORT_INDEXED_BOOL_STATE
#if DXGL_SUPPORT_INDEXED_FLOAT_STATE
inline void GetStateVari(GLenum eTarget, u32 uIndex, GLfloat* pData)  { glGetFloati_v(eTarget, uIndex, pData); }
inline void GetStateVari(GLenum eTarget, u32 uIndex, GLdouble* pData) { glGetDoublei_v(eTarget, uIndex, pData); }
...
struct SFrameBufferCache
{
	struct SHashCompare
	{
		...
		{
			return (size_t)NDrxOpenGL::GetCRC32(kConfig.m_akAttachments, sizeof(kConfig.m_akAttachments));
		}
...
struct SPipelineCache
{
	struct SHashCompare
	{
		...
		{
			return (size_t)
			       NDrxOpenGL::GetCRC32(reinterpret_cast<tukk >(&kConfig.m_apShaders), sizeof(kConfig.m_apShaders))
#if DXGL_ENABLE_SHADER_TRACING
			       ^ kConfig.m_eStageTracing
#endif  //DXGL_ENABLE_SHADER_TRACING
#if !DXGL_SUPPORT_DEPTH_CLAMP
			       ^ ((u32)kConfig.m_bEmulateDepthClamp)
#endif  //!DXGL_SUPPORT_DEPTH_CLAMP
			       ^ ((u32)kConfig.m_eMode) << 2;
		}

		bool operator()(const SPipelineConfiguration& kLeft, const SPipelineConfiguration& kRight) const
		{
			i32 iCmp(memcmp(kLeft.m_apShaders, kRight.m_apShaders, sizeof(kLeft.m_apShaders)));
			if (iCmp != 0)
				return false;

#if DXGL_ENABLE_SHADER_TRACING
			iCmp = (i32)kRight.m_eStageTracing - (i32)kLeft.m_eStageTracing;
			if (iCmp != 0)
				return false;
#endif  //DXGL_ENABLE_SHADER_TRACING

			iCmp = (i32)kRight.m_eMode - (i32)kLeft.m_eMode;
			if (iCmp != 0)
				return false;

#if !DXGL_SUPPORT_DEPTH_CLAMP
			iCmp = (i32)kRight.m_bEmulateDepthClamp - (i32)kLeft.m_bEmulateDepthClamp;
			if (iCmp != 0)
				return false;
#endif  //!DXGL_SUPPORT_DEPTH_CLAMP
...
struct SUnitMapCache
{
	struct SHashCompare
	{
		...
		{
			return (size_t)(kMap->m_uNumUnits ^ NDrxOpenGL::GetCRC32(kMap->m_akUnits, sizeof(kMap->m_akUnits[0]) * kMap->m_uNumUnits));
		}
...
u32 DumpShaderTrace(u32k* puValues, const SShaderTraceIndex& kIndex, u32 uStride, u32 uSize, u32 uCapacity)
{
	if (uSize > uCapacity)
	{
		DXGL_WARNING(
	...
	u32 uNumElements((uSize + uStride - 1) / uStride);
	if (uNumElements > 0)
	{
		...
		drx_sprintf(acFileName, "shader_trace_%d.txt", uTraceID);
		kTraceFile.Open(acFileName, false);

		for (u32 uElement = 0; uElement < uNumElements; ++uElement)
		{
			u32 uStep;
			u32k* puValue(puValues + uElement * uStride);
			u32k* puEnd(puValue + uStride);
			...
			while (puValue < puEnd && (uStep = *(puValue++)) < kIndex.m_kTraceStepSizes.size())
			{
				...
				u32 uStepVariablesBegin = kIndex.m_kTraceStepOffsets.at(uStep);

				for (u32 uVariable = 0; puValue < puEnd && uVariable < kIndex.m_kTraceStepSizes.at(uStep); ++uVariable)
				{
					...
					case TRACE_VARIABLE_SINT:
						kTraceFile.Printf("\t%c%d.%c = %i [0x%08X]\n", cPrefix, pVariable->ui8Index, cComponent, *reinterpret_cast<i32k*>(puValue), *puValue);
						++puValue;
						break;
...
{
	u32 m_auValues[SShaderTraceBufferCommon::CAPACITY];
};

template<typename StageHeader>
void BeginTraceInternal(u32 uBufferName, const StageHeader& kStageHeader, u32 uStride)
{
...
template<typename StageHeader>
u32 EndTraceInternal(u32 uBufferName, EShaderType eShaderType, const SShaderTraceIndex& kIndex)
{
	...
	u32 uNumTraces = DumpShaderTrace(pBuffer->m_auValues, kIndex, pBuffer->m_uStride, pBuffer->m_uSize, pBuffer->m_uCapacity);
	glUnmapNamedBufferEXT(uBufferName);
...
SStreamingBufferContext::~SStreamingBufferContext()
{
	for (u32 uFramePool = 0; uFramePool < m_kFramePools.size(); ++uFramePool)
...
void SStreamingBufferContext::SwitchFrame(CDevice* pDevice)
{
	...
	if (m_spCurrentFrame == NULL || m_uPreviousFramesNum < MAX_PREVIOUS_FRAMES)
	{
		...
		{
			u32 uSize(min((u32)(SGlobalConfig::iMinFramePoolSize << m_kFramePools.size()), (u32)SGlobalConfig::iMaxFramePoolSize));
			...
			for (u32 uElement = 0; uElement < uLast; ++uElement)
...
void SStreamingBufferContext::UpdateStreamingSizes(CDevice* pDevice)
{
	...
	for (SStreamingBuffer* pBuffer = m_akStreamingBuffers; pBuffer < pBuffersEnd; ++pBuffer)
	{
		...
		{
			u32 uGranularity((u32)SGlobalConfig::iStreamingConstantBuffersGranularity);
			u32 uMinUnits((pBuffer->m_uRequestedSectionSize + uGranularity - 1) / uGranularity);
			u32 uOldUnits(pBuffer->m_uSectionCapacity / uGranularity);
			u32 uDesiredUnits(uOldUnits);
			while (uDesiredUnits < uMinUnits)
			{
				uDesiredUnits = max(1u, uDesiredUnits * (u32)SGlobalConfig::iStreamingConstantBuffersGrowth);
			}
			u32 uMissingUnits(uDesiredUnits - uOldUnits);
...
void SStreamingBufferContext::UploadAndBindUniformData(CContext* pContext, const SConstantBufferSlot& kSlot, u32 uUnit)
{
	u32 uStreamingAlignment(pContext->GetDevice()->GetAdapter()->m_kCapabilities.m_iUniformBufferOffsetAlignment);
	#if !OGL_SINGLE_CONTEXT
	u32 uContextIndex(pContext->GetIndex());
	#endif
	...
	if (kSlot.m_pBuffer != NULL)
	{
		...
		u32 uStreamingSize(kSlot.m_kRange.m_uSize + uStreamingAlignment - 1 - (kSlot.m_kRange.m_uSize - 1) % uStreamingAlignment);
	#if OGL_SINGLE_CONTEXT
		...
		if (pBuffer->m_bStreaming)
		{
			...
			if (bDataDirty || bCopyDirty)
			{
				if (pStreaming->m_uNextPosition + uStreamingSize < pStreaming->m_uEndPosition)
				{
					...
					uk pSrcData(pBuffer->m_pSystemMemoryCopy + kSlot.m_kRange.m_uOffset);
					size_t uSize(kSlot.m_kRange.m_uSize);

	#if DXGL_SUPPORT_BUFFER_STORAGE
					if (pStreaming->m_pMappedData != NULL)
					{
						GLvoid* pData(static_cast<u8*>(pStreaming->m_pMappedData) + pStreaming->m_uNextPosition);
						NDrxOpenGL::Memcpy(pData, pSrcData, uSize);
						m_bFlushNeeded = true;
					}
					else
	#endif
...
CContext::CContext(CDevice* pDevice, const TRenderingContext& kRenderingContext, const TWindowContext& kDefaultWindowContext, u32 uIndex)
	: m_uIndex(uIndex)
	...
	, m_pUnitMapCache(new SUnitMapCache)
{
	...
	for (u32 uUnitType = 0; uUnitType < eRUT_NUM; ++uUnitType)
...
bool CContext::Initialize()
{
	...
	drx_sprintf(acTraceFileName, "Calls_%d", m_uIndex);
	...
	u32 uNumTextureUnits(min((u32)DXGL_ARRAY_SIZE(m_kStateCache.m_akTextureUnits), (u32)kCapabilities.m_akResourceUnits[eRUT_Texture].m_aiMaxTotal));
#if DXGL_SUPPORT_SHADER_IMAGES
	u32 uNumImageUnits(min((u32)DXGL_ARRAY_SIZE(m_kStateCache.m_akImageUnits), (u32)kCapabilities.m_akResourceUnits[eRUT_Image].m_aiMaxTotal));
#endif //DXGL_SUPPORT_SHADER_IMAGES

	for (u32 uUnit = 0; uUnit < uNumTextureUnits; ++uUnit)
		GetTextureUnitCache(uUnit, m_kStateCache.m_akTextureUnits[uUnit]);
#if DXGL_SUPPORT_SHADER_IMAGES
	for (u32 uUnit = 0; uUnit < uNumImageUnits; ++uUnit)
...
bool CContext::GetBlendCache(SBlendCache& kCache)
{
	...
	for (u32 uOverriddenTarget = 1; uOverriddenTarget < DXGL_ARRAY_SIZE(kCache.m_kTargets); ++uOverriddenTarget)
...
bool CContext::GetTextureUnitCache(u32 uUnit, STextureUnitCache& kCache)
{
...
bool CContext::GetImageUnitCache(u32 uUnit, SImageUnitCache& kCache)
{
...
bool CContext::GetInputAssemblerCache(SInputAssemblerCache& kCache)
{
	...
	{
		DXGL_WARNING("Currently %d vertex attribute slots supported at most, additional attributes will not be used", (i32)sizeof(kCache.m_uVertexAttribsEnabled) * 8);
	...
	{
		for (u32 uSlot = 0; uSlot < iNumVertexAttribs; ++uSlot)
		{
		...
		for (u32 uBinding = 0; uBinding < iNumVertexAttribBindings; ++uBinding)
		{
...
bool CContext::GetImplicitStateCache(SImplicitStateCache& kCache)
{
	...
	for (u32 uViewport = 0; uViewport < DXGL_NUM_SUPPORTED_VIEWPORTS; ++uViewport)
	{
	...
	for (u32 uBufferBinding = 0; uBufferBinding < eBB_NUM; ++uBufferBinding)
	{
	...
	u32 uNumUniformBuffers = kCapabilities.m_akResourceUnits[eRUT_UniformBuffer].m_aiMaxTotal;
	for (u32 uIndex = 0; uIndex < uNumUniformBuffers; ++uIndex)
	{
		GLuint uBufferBound(0);
	...
	u32 uNumStorageBuffers = kCapabilities.m_akResourceUnits[eRUT_StorageBuffer].m_aiMaxTotal;
	for (u32 uIndex = 0; uIndex < uNumStorageBuffers; ++uIndex)
	{
		GLuint uBufferBound(0);
...
template<typename TSetBlendState>
void SetTargetsBlendState(STargetBlendState* pRTCache, const STargetBlendState* pRTState, u32 uNumTargets)
{
	u32 uTarget;
...
void CContext::FlushTextureUnits()
{
	...
	if (m_abResourceUnitsDirty[eRUT_Texture] && pTextureUnitMap != NULL)
	{
		...
		u32 uNumMapElements(pTextureUnitMap->m_uNumUnits);

		for (u32 uMapElement = 0; uMapElement < uNumMapElements; ++uMapElement)
		{
			SUnitMap::SElement kMapElement(pTextureUnitMap->m_akUnits[uMapElement]);
			u32 uUnit(kMapElement.GetTextureUnit());
			u32 uTextureSlot(kMapElement.GetTextureSlot());
			...
			{
				u32 uSamplerSlot(kMapElement.GetSamplerSlot());
...
void CContext::FlushUniformBufferUnits()
{
	...
	{
		u32 uNumUnits = pUniformBufferUnitMap->m_uNumUnits;
		...
		for (const SUnitMap::SElement* pUnit = pUnitsBegin; pUnit < pUnitsEnd; ++pUnit)
		{
			...
			u16 uSlot = kUnit.GetResourceSlot();
			u16 uUnit = kUnit.GetResourceUnit();
...
void CContext::FlushStorageBufferUnits()
{
	...
	{
		u32 uNumUnits = pStorageBufferUnitMap->m_uNumUnits;
...
void CContext::FlushImageUnits()
{
	...
	{
		u32 uNumUnits = pImageUnitMap->m_uNumUnits;
...
void CContext::FlushInputAssemblerStateVab()
{
	...
	if (m_bInputLayoutDirty)
	{
		if (m_pInputLayout != NULL)
		{
			...
			for (u32 uSlot = 0; uSlot < m_pInputLayout->m_akVertexAttribFormats.size(); ++uSlot)
			{
			...
			{
				const GLint iNumVertexAttribBindings(m_pDevice->GetAdapter()->m_kCapabilities.m_iMaxVertexAttribBindings);
				for (u32 uBindingSlot = 0; uBindingSlot < iNumVertexAttribBindings; ++uBindingSlot)
				{
			...
			{
				u32k uMaxChangedVertexAttribIndex = m_pDevice->GetAdapter()->m_kCapabilities.m_iMaxVertexAttribs;

				for (u32 uAttribIndex = 0; uAttribIndex < uMaxChangedVertexAttribIndex; ++uAttribIndex)
				{
					u32k uAttribEnabledBit = 1 << uAttribIndex;
			...
			// now update the format and bindings for *all* enabled vertex attribs (and not just those that we might just have enabled)
			{
				...
				for (u32 uAttribIndex = 0; uAttribIndex < iNumVertexAttribs; ++uAttribIndex)
				{
	...
	if (m_bInputAssemblerSlotsDirty)
	{
		...
		for (i32 uSlot = 0; uSlot < MAX_VERTEX_ATTRIB_BINDINGS; ++uSlot)
		{
...
void CContext::SetVertexOffset(u32 uVertexOffset)
{
...
void CContext::BindUniformBuffer(const TIndexedBufferBinding& kBinding, u32 uUnit)
{
...
void CContext::BindStorageBuffer(const TIndexedBufferBinding& kBinding, u32 uUnit)
{
...
void CContext::BindImage(const CResourceName& kName, SShaderImageViewConfiguration kConfiguration, u32 uUnit)
{
...
void CContext::SetViewports(u32 uNumViewports, const D3D11_VIEWPORT* pViewports)
{
...
void CContext::SetScissorRects(u32 uNumRects, const D3D11_RECT* pRects)
{
...
u32 MatchColorAttachmentIndex(SOutputMergerView* pView, SFrameBufferConfiguration& kFrameBufferConfig)
{
	u32 uAttachment(0);
...
SFrameBufferPtr GetCompatibleColorAttachmentFrameBuffer(SOutputMergerView* pOMView, u32& uAttachment, CContext* pContext)
{
...
void CContext::ClearRenderTarget(SOutputMergerView* pRenderTargetView, const float afColor[4])
{
	...
	u32 uAttachment(MatchColorAttachmentIndex(pRenderTargetView, m_kFrameBufferConfig));
	if (uAttachment != SFrameBufferConfiguration::MAX_COLOR_ATTACHMENTS)
		FlushFrameBufferState();
	else
	{
	...
	u32 uDrawBufferIndex((u32)(SFrameBufferConfiguration::AttachmentIndexToID(uAttachment) - GL_COLOR_ATTACHMENT0));
...
void CContext::ClearDepthStencil(SOutputMergerView* pDepthStencilView, bool bClearDepth, bool bClearStencil, float fDepthValue, u8 uStencilValue)
{
...
void CContext::SetRenderTargets(u32 uNumRTViews, SOutputMergerView** apRenderTargetViews, SOutputMergerView* pDepthStencilView)
{
	...
	if (pDepthStencilView != NULL)
	{
		...
		u32 uAttachmentIndex(SFrameBufferConfiguration::MAX_ATTACHMENTS);
...
void CContext::SetShader(SShader* pShader, u32 uStage)
{
...
void CContext::SetShaderTexture(SShaderTextureBasedView* pView, u32 uStage, u32 uIndex)
{
	u32 uSlot = TextureSlot((EShaderType)uStage, uIndex);
...
void CContext::SetShaderImage(SShaderImageView* pView, u32 uStage, u32 uIndex)
{
	u32 uSlot = ImageSlot((EShaderType)uStage, uIndex);
...
void CContext::SetShaderBuffer(SShaderBufferView* pView, u32 uStage, u32 uIndex)
{
	u32 uSlot = StorageBufferSlot((EShaderType)uStage, uIndex);
...
void CContext::SetShaderResourceView(SShaderView* pView, u32 uStage, u32 uSlot)
{
	if (pView == NULL)
	{
#if DXGL_SUPPORT_SHADER_STORAGE_BLOCKS
...
void CContext::SetUnorderedAccessView(SShaderView* pView, u32 uStage, u32 uSlot)
{
...
void CContext::SetSampler(SSamplerState* pSampler, u32 uStage, u32 uIndex)
{
	u32 uSlot = SamplerSlot((EShaderType)uStage, uIndex);
...
void CContext::SetConstantBuffer(SBuffer* pConstantBuffer, SBufferRange kRange, u32 uStage, u32 uIndex)
{
	u32 uSlot = ConstantBufferSlot((EShaderType)uStage, uIndex);
...
void CContext::SetVertexBuffer(u32 uSlot, SBuffer* pVertexBuffer, u32 uStride, u32 uOffset)
{
	SInputAssemblerSlot& kSlot(m_akInputAssemblerSlots[uSlot]);
...
void CContext::PrepareTraceHeader(u32 uFirstVertex, u32 uFirstIndex)
{
	switch (m_eStageTracing)
	{
	case eST_Vertex:
		{
			u32 uIndex = uFirstIndex + m_kStageTracingInfo.m_kVertex.m_uVertexIndex;
			...
			if (kIndexBuffer.IsValid())
			{
				...
				else
				{
					...
					case GL_UNSIGNED_INT:
						uVertexID = (u32) * static_cast<GLuint*>(pIndex);
						break;
					case GL_UNSIGNED_SHORT:
						uVertexID = (u32) * static_cast<GLushort*>(pIndex);
						break;
					case GL_UNSIGNED_BYTE:
						uVertexID = (u32) * static_cast<GLubyte*>(pIndex);
						break;
					default:
	...
	case eST_Fragment:
		{
			u32 uFragmentX = m_kStageTracingInfo.m_kFragment.m_uFragmentCoordX;
			u32 uFragmentY = m_kStageTracingInfo.m_kFragment.m_uFragmentCoordY;
...
void CContext::FlushShaderTracingState()
{
	...
	{
		u32 uCurrentConfigHash(m_kPipelineConfiguration.m_apShaders[m_eStageTracing]->m_kReflection.m_uInputHash);
...
void CContext::BeginTrace(u32 uFirstVertex, u32 uFirstIndex)
{
	enum
	{
	...
	{
		u32 uCurrentPipelineHash(m_spPipeline->m_kConfiguration.m_apShaders[m_eStageTracing]->m_kReflection.m_uInputHash);
		...
		u32 uTraceBufferUnit = m_spPipeline->m_uTraceBufferUnit;
...
void CContext::DrawIndexed(u32 uIndexCount, u32 uStartIndexLocation, u32 uBaseVertexLocation)
{
...
void CContext::Draw(u32 uVertexCount, u32 uStartVertexLocation)
{
...
void CContext::DrawIndexedInstanced(u32 uIndexCountPerInstance, u32 uInstanceCount, u32 uStartIndexLocation, u32 uBaseVertexLocation, u32 uStartInstanceLocation)
{
...
void CContext::DrawInstanced(u32 uVertexCountPerInstance, u32 uInstanceCount, u32 uStartVertexLocation, u32 uStartInstanceLocation)
{
...
void CContext::Dispatch(u32 uGroupX, u32 uGroupY, u32 uGroupZ)
{
...
void CContext::DispatchIndirect(u32 uIndirectOffset)
{
...
SPipelinePtr CContext::AllocatePipeline(const SPipelineConfiguration& kConfiguration)
{
	...
	for (u32 uShader = 0; uShader < eST_NUM; ++uShader)
	{
  ...
  GLbitfield uMask, GLenum eFilter)
{
	...
	if (bColor)
	{
		u32 uSrcAttachment, uDstAttachment;
...
void CContext::TogglePixelTracing(bool bEnable, u32 uShaderHash, u32 uPixelX, u32 uPixelY)
{
	if (bEnable)
	{
...
void CContext::ToggleVertexTracing(bool bEnable, u32 uShaderHash, u32 uVertexIndex)
{
	if (bEnable)
	{
...
void CContext::CallTraceWrite(tukk szTrace)
{
