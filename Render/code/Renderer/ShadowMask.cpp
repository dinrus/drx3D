...
#include <drx3D/Render/ShadowMask.h>
#include <drx3D/Render/DriverD3D.h>

#include <drx3D/Render/PostProcess/PostProcessUtils.h>
...
class CSunShadows
{
	...
	static i32k MaxCustomFrustums = 64;
...
public:
	...
	i32  PreparePrimitives(CPrimitiveRenderPass& sliceGenPass, i32& firstUnusedStencilValue, bool renderCloudShadows, bool renderScreenSpaceShadows, bool renderSvoShadows,
...
private:
	void PrepareCascadePrimitivesNoBlending(CPrimitiveRenderPass& sliceGenPass, i32& firstUnusedStencilValue, const SCascadePrimitiveContext& context);
	void PrepareCascadePrimitivesWithBlending(CPrimitiveRenderPass& sliceGenPass, i32& firstUnusedStencilValue, const SCascadePrimitiveContext& context);
	void PrepareCascadePrimitivesWithPrepass(CPrimitiveRenderPass& sliceGenPass, CPrimitiveRenderPass* pDebugCascadesPass, i32& firstUnusedStencilValue, const SCascadePrimitiveContext& context);

	void PreparePerObjectPrimitives(CRenderPrimitive& primStencil0, CRenderPrimitive& primStencil1, CRenderPrimitive& primSampling, i32& firstUnusedStencilValue, const SCustomPrimitiveContext& context);
	...
	bool PrepareDebugPrimitive(CPrimitiveRenderPass& debugPass, CRenderPrimitive& primitive, const ShadowMapFrustum* pFrustum, i32 stencilRef) const;
...
public:
	...
	void InitPrimitives();
	void ResetPrimitives();
	i32  PreparePrimitives(std::vector<CPrimitiveRenderPass>& maskGenPasses, i32& firstUnusedStencilValue, bool usingScreenSpaceShadows, CRenderView* pRenderView, uint64 qualityFlags);
	
	void OnCVarsChanged(const CCVarUpdateRecorder& cvarUpdater);

private:
	i32                    PreparePrimitivesForLight(const CRenderView* pRenderView, CPrimitiveRenderPass& sliceGenPass, i32& firstUnusedStencilValue, bool usingScreenSpaceShadows, SShadowFrustumToRender* pFrustumToRender, uint64 qualityFlags, i32& volumePrimitiveCount, i32& quadPrimitiveCount);
	void                   PrepareConstantBuffersForPrimitives(SLocalLightPrimitives& primitives, SShadowFrustumToRender* pFrustumToRender, i32 side, bool bVolumePrimitive) const;
...
void CShadowMaskStage::Prepare()
{
	...
	for (i32 shadowMaskSlice = 0; shadowMaskSlice < slices; ++shadowMaskSlice)
	{
	...
	i32 firstUnusedStencilValue = rd->m_nStencilMaskRef;
	...
	DRX_ASSERT(rd->m_nStencilMaskRef == STENCIL_VALUE_OUTDOORS+1);
	DRX_ASSERT(rd->m_nStencilMaskRef + firstUnusedStencilValue < STENC_MAX_REF);
...
void CShadowMaskStage::Execute()
{
	...
	if (m_localLightPrimitives > 0)
	{
		...
		for (i32 shadowMaskSlice = m_sunShadowPrimitives > 0 ? 1 : 0; shadowMaskSlice < m_maskGenPasses.size(); ++shadowMaskSlice)
		{
...
i32 CSunShadows::PreparePrimitives(CPrimitiveRenderPass& sliceGenPass, i32& firstUnusedStencilValue, bool renderCloudShadows, bool renderScreenSpaceShadows, bool renderSvoShadows,
                                   bool useTexelRelativeBias, bool extendLastCachedCascade, CPrimitiveRenderPass* pDebugCascadesPass, CRenderView* pRenderView, uint64 qualityFlags)
{
	...
	i32k previousPrimitiveCount = sliceGenPass.GetPrimitiveCount();
	...
	if (bPrepareCustomPrimitives)
	{
		...
		for (auto& pFrustumToRender : pRenderView->GetShadowFrustumsByType(CRenderView::eShadowFrustumRenderType_Custom))
		{
			...
			{
				DRX_ASSERT(pFrustum->m_eFrustumType == ShadowMapFrustum::e_PerObject);
...
void CSunShadows::PrepareCascadePrimitivesWithPrepass(CPrimitiveRenderPass& sliceGenPass, CPrimitiveRenderPass* pDebugCascadesPass, i32& firstUnusedStencilValue, const SCascadePrimitiveContext& context)
{
	const SRenderViewInfo& viewInfo = shadowMaskStage.m_viewInfo[0];

	// stencil primitives first
	for (i32 i = context.frustums.size() - 1; i >= 0; --i)
	{
		static CDrxNameTSCRC techStencil = "ClipVolumeStencil";
		CShader* pShader = CShaderMan::s_shDeferredShading;

		ShadowMapFrustum* pFrustum = context.frustums[i]->pFrustum;
		DRX_ASSERT(pFrustum->nShadowMapLod >= 0 && pFrustum->nShadowMapLod < MAX_GSM_LODS_NUM);

		i32k stencilRef = firstUnusedStencilValue + i;
	...
	for (i32 i = 0; i < context.frustums.size(); ++i)
	{
		static CDrxNameTSCRC techSampling = "ShadowMask";
...
void CSunShadows::PrepareCascadePrimitivesNoBlending(CPrimitiveRenderPass& sliceGenPass, i32& firstUnusedStencilValue, const SCascadePrimitiveContext& context)
{
	static CDrxNameTSCRC techSampling = "ShadowMaskVolume";
	CShader* pShader = CShaderMan::s_ShaderShadowMaskGen;

	const SRenderViewInfo& viewInfo = shadowMaskStage.m_viewInfo[0];
	i32k maxStencilValue = firstUnusedStencilValue + context.frustums.size() - 1;
	const bool bReverseDepth = (viewInfo.flags & SRenderViewInfo::eFlags_ReverseDepth) != 0;

	for (i32 i = 0; i < context.frustums.size(); ++i)
	{
		const bool bScreenspaceShadows = i == 0 && context.renderScreenspaceShadows;
		i32k stencilRef = maxStencilValue - i;
...
void CSunShadows::PrepareCascadePrimitivesWithBlending(CPrimitiveRenderPass& sliceGenPass, i32& firstUnusedStencilValue, const SCascadePrimitiveContext& context)
{
	static CDrxNameTSCRC techSampling = "ShadowMaskVolume";
	CShader* pShader = CShaderMan::s_ShaderShadowMaskGen;

	const SRenderViewInfo& viewInfo = shadowMaskStage.m_viewInfo[0];
	i32k maxStencilValue = firstUnusedStencilValue + 2 * context.frustums.size() - 1;
	const bool bReverseDepth = (viewInfo.flags & SRenderViewInfo::eFlags_ReverseDepth) != 0;
	i32k gsDepthFunc = bReverseDepth ? GS_DEPTHFUNC_LEQUAL : GS_DEPTHFUNC_GEQUAL;

	for (i32 i = 0; i < context.frustums.size(); ++i)
	{
		const bool bScreenspaceShadows = i == 0 && context.renderScreenspaceShadows;
		i32k stencilRef = maxStencilValue - 2 * i;
...
void CSunShadows::PrepareNearestPrimitive(CRenderPrimitive& primitive, ShadowMapFrustum* pFrustum, uint64 rtFlags)
{
	...
	if (m_nearestZ != CRendererCVars::CV_r_DrawNearZRange)
	{
		...
		DrxStackAllocWithSizeVector(SVF_P3F, 3, fullscreenTriVertices, CDeviceBufferUpr::AlignBufferSizeForStreaming);
...
void CSunShadows::PreparePerObjectPrimitives(CRenderPrimitive& primStencil0, CRenderPrimitive& primStencil1, CRenderPrimitive& primSampling, i32& firstUnusedStencilValue, const SCustomPrimitiveContext& context)
{
	i32k stencilRef = firstUnusedStencilValue;
	const SRenderViewInfo& viewInfo = shadowMaskStage.m_viewInfo[0];
	const bool bReverseDepth = (viewInfo.flags & SRenderViewInfo::eFlags_ReverseDepth) != 0;
	i32k gsDepthTest = (bReverseDepth ? GS_DEPTHFUNC_GEQUAL : GS_DEPTHFUNC_LEQUAL);
...
{
	auto constants = primitive.GetConstantUpr().BeginTypedConstantUpdate<STypedConstants>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex | EShaderStage_Pixel);
	...
	primitive.GetConstantUpr().EndTypedConstantUpdate(constants);
}
...
void CSunShadows::PrepareConstantBuffers(CRenderPrimitive& primitive, ShadowMapFrustum* pFrustum, ShadowMapFrustum* pVolumeProvider, bool bScaledVolume) const
{
	...
	auto& constantUpr = primitive.GetConstantUpr();

	// assign per view constant buffer
	constantUpr.SetTypedConstantBuffer(eConstantBufferShaderSlot_PerView, shadowMaskStage.m_pPerViewConstantBuffer, EShaderStage_Vertex | EShaderStage_Pixel);

	// update typed constants
	auto constants = constantUpr.BeginTypedConstantUpdate<STypedConstants>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex | EShaderStage_Pixel);

	for (i32 i = 0; i < shadowMaskStage.m_viewInfoCount; ++i)
	{
	...
	constantUpr.EndTypedConstantUpdate(constants);
}
...
void CSunShadows::PrepareCustomShadowsPrimitive(CRenderPrimitive& primitive, _smart_ptr<CTexture>& pCloudShadowTex) const
{
	...
	CShader* pShader = CShaderMan::s_ShaderShadowMaskGen;
	static CDrxNameTSCRC techCustomShadows = "CustomShadows";
	...
	// per view constant buffer
	constantUpr.SetTypedConstantBuffer(eConstantBufferShaderSlot_PerView, shadowMaskStage.m_pPerViewConstantBuffer);

	// cloud shadow specific constants
	SRenderViewShaderConstants& PF = shadowMaskStage.RenderView()->GetShaderConstants();
	auto constants = constantUpr.BeginTypedConstantUpdate<SCloudShadowConstants>(eConstantBufferShaderSlot_PerPrimitive);
...
bool CSunShadows::PrepareDebugPrimitive(CPrimitiveRenderPass& debugPass, CRenderPrimitive& primitive, const ShadowMapFrustum* pFrustum, i32 stencilRef) const
{
	u32k StencilStateTest =
	  STENC_FUNC(FSS_STENCFUNC_EQUAL) |
	  STENCOP_FAIL(FSS_STENCOP_KEEP) |
	  STENCOP_ZFAIL(FSS_STENCOP_KEEP) |
	...
	i32k cascadeColorCount = DRX_ARRAY_COUNT(cascadeColors);

	static CDrxNameTSCRC DebugTech = "DebugShadowCascades";
	static CDrxNameR CascadeColorParam("DebugCascadeColor");
	...
	{
		auto& constantUpr = primitive.GetConstantUpr();
		DRX_ASSERT(constantUpr.IsShaderReflectionValid());

		constantUpr.BeginNamedConstantUpdate();
		constantUpr.SetNamedConstant(CascadeColorParam, cascadeColors[pFrustum->nShadowMapLod % cascadeColorCount], eHWSC_Pixel);
		constantUpr.EndNamedConstantUpdate(&debugPass.GetViewport());
...
i32 CLocalLightShadows::PreparePrimitives(std::vector<CPrimitiveRenderPass>& maskGenPasses, i32& firstUnusedStencilValue, bool usingScreenSpaceShadows, CRenderView* pRenderView, uint64 qualityFlags)
{
	// sort shadow frustums into shadow mask slices such that frustums in each slice don't overlap in screen space
	u32k maxSliceCount = shadowMaskStage.m_pShadowMaskRT->StreamGetNumSlices();
	auto &frustumsPerSlice = pRenderView->m_shadows.m_frustumsPerTiledShadingSlice;

	// prepare primitives for each shadow mask slice
	i32 volumePrimitiveCount = 0;
	i32 quadPrimitiveCount = 0;
	for (i32 slice = 0; slice < frustumsPerSlice.size(); ++slice)
	{
	...
	SRenderStatistics::Write().m_NumShadowMaskChannels = (maxSliceCount << 16) | ((u32)frustumsPerSlice.size() & 0xFFFF);
#endif
	return (volumePrimitiveCount + quadPrimitiveCount) * 3;
}

i32 CLocalLightShadows::PreparePrimitivesForLight(const CRenderView* pRenderView, CPrimitiveRenderPass& sliceGenPass, i32& firstUnusedStencilValue, bool usingScreenSpaceShadows, SShadowFrustumToRender* pFrustumToRender, uint64 qualityFlags, i32& volumePrimitiveCount, i32& quadPrimitiveCount)
{
	CD3D9Renderer* const __restrict rd = gcpRendD3D;
	...
	DRX_ASSERT(meshType >= SHAPE_PROJECTOR && meshType <= SHAPE_PROJECTOR2);
	...
	i32k nSides = pFrustum->GetNumSides();
	...
	/* TheoM TODO: scissor
	   ...
	   {
	   EF_Scissor(true,
	    static_cast<i32>(lightRect.x * m_RP.m_CurDownscaleFactor.x),
	    static_cast<i32>(lightRect.y * m_RP.m_CurDownscaleFactor.y),
	    static_cast<i32>(lightRect.z * m_RP.m_CurDownscaleFactor.x + 1),
	    static_cast<i32>(lightRect.w * m_RP.m_CurDownscaleFactor.y + 1));
	   }
	 */

	CRenderPrimitive* stencilPrimitives[6 * 2], * samplingPrimitives[6];
	i32 numStencilPrimitives = 0, numSamplingPrimitives = 0;

	for (i32 nS = 0; nS < nSides; nS++)
	{
		if (pFrustum->ShouldSampleSide(nS))
		{
			...
			i32k stencilRef = firstUnusedStencilValue + nSides - nS;

			static CDrxNameTSCRC techStencil = "LightVolumeStencil";
			static CDrxNameTSCRC techSamplingQuad = "ShadowMaskGen";
			static CDrxNameTSCRC techSamplingVolume = "ShadowMaskGenVolume";

			CShader* pShader = CShaderMan::s_shDeferredShading;
			CDrxNameTSCRC& techSampling = bUseLightVolumes ? techSamplingVolume : techSamplingQuad;
			...
			i32k gsDepthFunc = bReverseDepth ? GS_DEPTHFUNC_GEQUAL : GS_DEPTHFUNC_LEQUAL;
			i32k gsDepthTest = bUseLightVolumes ? 0 : GS_NODEPTHTEST;
	...
	for (i32 i = 0; i < numStencilPrimitives; ++i)
		sliceGenPass.AddPrimitive(stencilPrimitives[i]);

	for (i32 i = 0; i < numSamplingPrimitives; ++i)
...
void CLocalLightShadows::PrepareConstantBuffersForPrimitives(SLocalLightPrimitives& primitives, SShadowFrustumToRender* pFrustumToRender, i32 side, bool bVolumePrimitive) const
{
	SRenderViewShaderConstants& PF = shadowMaskStage.RenderView()->GetShaderConstants();
	...
	i32k nShadowAtlasRes = pVar->GetIVal();
	...
	auto& constantUpr = primitives.sampling.GetConstantUpr();
