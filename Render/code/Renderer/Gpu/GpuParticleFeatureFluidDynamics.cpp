...
void CFeatureMotionFluidDynamics::Update(const gpu_pfx2::SUpdateContext& context, CDeviceCommandListRef RESTRICT_REFERENCE commandList)
{
	...
	DRX_ASSERT(p.numSpawnParticles < 512);
	gpu_physics::SFluidBody bodies[512];
	for (i32 i = 0; i < p.numSpawnParticles; ++i)
	{
		float randX = drx_random(-1.0f, 1.0f);
		float randY = drx_random(-1.0f, 1.0f);
		float randZ = drx_random(-1.0f, 1.0f);
