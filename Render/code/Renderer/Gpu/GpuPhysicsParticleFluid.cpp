...
#include <drx3D/Render/GpuPhysicsParticleFluid.h>

#include <DinrusXPhys/physinterface.h>

namespace gpu_physics
{
void SetupAdjacency(i32* adjescency, i32 resX, i32 resY)
{
...
CParticleFluidSimulation::CParticleFluidSimulation(i32k maxBodies)
	: m_maxBodies(maxBodies)
{
	...
	m_passCalcLambda.SetTechnique(pShader, CDrxNameTSCRC("CalcLambda"), 0);
	m_passPredictDensity.SetTechnique(pShader, CDrxNameTSCRC("PredictDensity"), 0);
	m_passCorrectDensityError.SetTechnique(pShader, CDrxNameTSCRC("CorrectDensityError"), 0);
	m_passCorrectDivergenceError.SetTechnique(pShader, CDrxNameTSCRC("CorrectDivergenceError"), 0);
	m_passPositionUpdate.SetTechnique(pShader, CDrxNameTSCRC("PositionUpdate"), 0);
	m_passBodiesInject.SetTechnique(pShader, CDrxNameTSCRC("BodiesInject"), 0);
	m_passClearGrid.SetTechnique(pShader, CDrxNameTSCRC("ClearGrid"), 0);
	m_passAssignAndCount.SetTechnique(pShader, CDrxNameTSCRC("AssignAndCount"), 0);
	m_passPrefixSumBlocks.SetTechnique(pShader, CDrxNameTSCRC("PrefixSumBlocks"), 0);
	m_passBuildGridIndices.SetTechnique(pShader, CDrxNameTSCRC("BuildGridIndices"), 0);
	m_passRearrangeParticles.SetTechnique(pShader, CDrxNameTSCRC("RearrangeParticles"), 0);
	m_passEvolveExternalParticles.SetTechnique(pShader, CDrxNameTSCRC("EvolveExternalParticles"), 0);
	m_passCollisionsScreenSpace.SetTechnique(pShader, CDrxNameTSCRC("CollisionScreenSpace"), 0);
...
void CParticleFluidSimulation::CreateResources()
{
	...
	i32k totalGridSize = m_params->gridSizeX * m_params->gridSizeY * m_params->gridSizeZ;
...
void CParticleFluidSimulation::RetrieveCounter()
{
	...
	i32k counter = m_pData->bodies.RetrieveCounter();
  ...
  i32k cellBlock)
{
	...
	{
		m_passBodiesInject.SetDispatchSize((i32)ceil((float)m_params->numberBodiesInject / cellBlock), 1, 1);
...
void CParticleFluidSimulation::EvolveParticles(CDeviceCommandListRef RESTRICT_REFERENCE commandList, CGpuBuffer& defaultParticleBuffer, i32 numParticles)
{
	i32k blocks = gpu::GetNumberOfBlocksForArbitaryNumberOfThreads(numParticles, kThreadsInBlock);
...
void CParticleFluidSimulation::FluidCollisions(CDeviceCommandListRef RESTRICT_REFERENCE commandList, CConstantBufferPtr parameterBuffer, i32 constantBufferSlot)
{
...
void CParticleFluidSimulation::IntegrateBodies(CDeviceCommandListRef RESTRICT_REFERENCE commandList, i32k blocks)
{
...
#if 0
	for (i32 i = 0; i < m_params->numberOfIterations; ++i)
	{
		{
...
void CParticleFluidSimulation::InternalInjectBodies(const EBodyType type, const SBodyBase* b, i32k numBodies)
{
...
void CParticleFluidSimulation::DebugDraw()
{
	if (CRenderer::CV_r_GpuPhysicsFluidDynamicsDebug)
	{
		...
		for (i32 i = 0; i < m_params->numberOfBodies; ++i)
