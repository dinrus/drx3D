...
//   To achieve this, instead of taking a uk handle, we should pack mapped address + handle into one 64-bit field (ie, assume 4K aligned mapped addresses, and allow max 4K handles)
...
#if defined(_DEBUG)
	#define HEAP_CHECK 1
	#define HEAP_ASSERT(x) DRX_ASSERT(x)
#else
	#define HEAP_CHECK 0
...
struct SHugePage : SLinkedPage<1>
{
	...
	uk GetAddress() const
	{
		return reinterpret_cast<uk>(static_cast<size_t>(address) << kAlignShift);
	}

	bool SetAddress(uk pAddress)
	{
...
static TPageId GetPageId(const UPage* pPages, const SLinkedPage<N>* pPage)
{
	...
	HEAP_ASSERT(((tuk)pPage - (tuk)pPages) % sizeof(UPage) == 0 && result < kMaxPages);
...
void MakeMemReplayEvent(const SHugePage& page, uint32_t offset, bool bMapPersistent, bool bAllocate, uint32_t bytes, uint32_t align)
{
	if (bMapPersistent)
	{
		uk pPersistent = page.GetAddress();
		if (pPersistent)
		{
...
CGpuHeap::~CGpuHeap()
{
	...
	if (bFreeAllHandles)
	{
		const auto pfnFree = [](uk pContext, THandle handle, uint32_t memoryType, TBlockHandle blockHandle, uint32_t blockOffset, uint32_t allocationSize, uk pMappedAddress)
		{
	...
	else
	{
		for (uint32_t i = 0; i < m_numHeaps; ++i)
		{
			m_pHeaps[i].hugeBlocks.Walk(m_pPages, [this, i](SHugePage& page) -> bool
			{
				if (page.mapped)
				{
				  uk const pAddress = page.GetAddress();
				  if (pAddress)
				  {
...
CGpuHeap::THandle CGpuHeap::Allocate(uint32_t memoryType, uint32_t bytes, uint32_t align)
{
	...
	DrxAutoCriticalSectionNoRecursive lock(m_lock);
...
CGpuHeap::THandle CGpuHeap::AllocateHuge(uint32_t memoryType, uint32_t bytes, uint32_t align, bool bCommit)
{
	...
	if (m_bMapPersistent)
	{
		uk pAddress = MapBlock(memoryType, blockHandle);
...
uk CGpuHeap::Map(THandle handle)
{
	if (!handle)
	{
		return nullptr;
	...
	if (!page.mapped)
	{
		...
		uk const pAddress = MapBlock(memoryType, blockHandle);
	...
	return static_cast<tuk>(pAddress) + (pAddress ? offset : 0U);
}
...
uint32_t CGpuHeap::Walk(uk pContext, TSummaryCallback pfnSummary, TAllocationCallback pfnAllocation, uint32_t memoryTypeMask) const
{
	DrxAutoCriticalSectionNoRecursive lock(m_lock);
	...
	for (uint32_t memoryType = 0U; memoryType < m_numHeaps; ++memoryType)
	{
		...
		const auto invokeCallback = [&](THandle handle)
		{
			...
			if (pfnAllocation)
			{
				...
				tuk const pAddress = static_cast<tuk>(pHugePage->GetAddress());
