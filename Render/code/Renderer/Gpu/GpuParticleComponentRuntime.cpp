...
#include <drx3D/Render/D3D/Gpu/GpuMergeSort.h>
#include <drx3D/Render/CompiledRenderObject.h>

#include <drx3D/Render/D3D/Gpu/Physics/GpuPhysicsParticleFluid.h>

#include <drx3D/CoreX/BitFiddling.h>

namespace gpu_pfx2
{
i32k CParticleComponentRuntime::kThreadsInBlock = 1024;
...
void CParticleComponentRuntime::SetBoundsFromUpr(const SReadbackData* pData)
{
...
void CParticleComponentRuntime::SetCounterFromUpr(u32k* pCounter)
{
...
void CParticleComponentRuntime::AddParticles(SUpdateContext& context)
{
	...
	DRX_ASSERT(m_parameters->numNewBorns <= m_params.maxNewBorns);
	DRX_ASSERT(m_parameters->numParticles + m_parameters->numNewBorns <= m_params.maxParticles);
	i32 maxNewBorns = min(m_parameters->numNewBorns, m_params.maxParticles - m_parameters->numParticles);
...
void CParticleComponentRuntime::UpdateNewBorns(const SUpdateContext& context, CDeviceCommandListRef RESTRICT_REFERENCE commandList)
{
	...
	for (i32 i = 0; i < eFeatureInitializationSrvSlot_COUNT; ++i)
	{
	...
	i32k blocks = gpu::GetNumberOfBlocksForArbitaryNumberOfThreads(
...
void CParticleComponentRuntime::UpdateFeatures(const SUpdateContext& context, CDeviceCommandListRef RESTRICT_REFERENCE commandList)
{
	...
	for (i32 i = 0; i < eFeatureUpdateSrvSlot_COUNT; ++i)
	{
	...
	for (i32 i = 0; i < eConstantBufferSlot_COUNT; ++i)
	{
...
void CParticleComponentRuntime::Sort(const SUpdateContext& context, CDeviceCommandListRef RESTRICT_REFERENCE commandList)
{
	...
	i32k processSlots = NextPower2(GetNumParticles());
	i32k blocks = gpu::GetNumberOfBlocksForArbitaryNumberOfThreads(processSlots, kThreadsInBlock);
...
void CParticleComponentRuntime::UpdatePasses()
{
	if (m_initializationShaderFlags != m_previousInitializationShaderFlags)
	{
		...
		m_passFeatureInitialization.SetTechnique(m_pInitShader, CDrxNameTSCRC("FeatureInitialization"), 0);
	...
	if (m_updateShaderFlags != m_previousUpdateShaderShaderFlags)
	{
		...
		m_passFeatureUpdate.SetTechnique(m_pUpdateShader, CDrxNameTSCRC("FeatureUpdate"), 0);
...
{
	DRX_ASSERT(m_parameters->numKilled <= m_parameters->numParticles);
...
void CParticleComponentRuntime::InitializePasses()
{
	...
	m_passCalcBounds.SetTechnique(CShaderMan::s_ShaderGpuParticles, CDrxNameTSCRC("CalculateBounds"), 0);
	m_passSwapToEnd.SetTechnique(CShaderMan::s_ShaderGpuParticles, CDrxNameTSCRC("SwapToEnd"), 0);
	m_passFillKillList.SetTechnique(CShaderMan::s_ShaderGpuParticles, CDrxNameTSCRC("FillKillList"), 0);
	m_passPrepareSort.SetTechnique(CShaderMan::s_ShaderGpuParticles, CDrxNameTSCRC("PrepareSort"), 0);
	m_passReorderParticles.SetTechnique(CShaderMan::s_ShaderGpuParticles, CDrxNameTSCRC("ReorderParticles"), 0);
}
