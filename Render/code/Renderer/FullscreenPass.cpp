...
void CFullscreenPass::BeginConstantUpdate()
{
	...
	{
		UpdatePrimitive();
		m_primitive.GetConstantUpr().BeginNamedConstantUpdate();

		m_bPendingConstantUpdate = true;
	}
}

static void GetFullScreenTriWPOS(const SRenderViewInfo& viewInfo, SVF_P3F_T2F_T3F pResult[3], i32 nTexWidth, i32 nTexHeight, float z, const D3DViewPort* pSrcRegion)
{
...
static void GetFullScreenQuadWPOS(const SRenderViewInfo& viewInfo, SVF_P3F_T2F_T3F pResult[4], i32 nTexWidth, i32 nTexHeight, float z, const D3DViewPort* pSrcRegion)
{
	Vec3 vLT = viewInfo.m_frustumCorners[SRenderViewInfo::eFrustum_LT];
	Vec3 vLB = viewInfo.m_frustumCorners[SRenderViewInfo::eFrustum_LB];
	Vec3 vRT = viewInfo.m_frustumCorners[SRenderViewInfo::eFrustum_RT];
...
{
	// update viewport
	i32 nRenderTargetWidth  = 0;
	i32 nRenderTargetHeight = 0;
	{
		DRX_ASSERT(m_renderPassDesc.GetRenderTargets()[0].pTexture || m_renderPassDesc.GetDepthTarget().pTexture);
	...
	if (m_bRequirePerViewCB)
	{
		...
		i32 viewInfoCount = gcpRendD3D.GetGraphicsPipeline().GetViewInfoCount();
	...
	if (m_bRequireWorldPos)
	{
		...
		bool bUseQuad = CVrProjectionUpr::Instance()->GetProjectionType() == CVrProjectionUpr::eVrProjection_LensMatched;
		const SRenderViewInfo& viewInfo = gcpRendD3D.GetGraphicsPipeline().GetCurrentViewInfo(CCamera::eEye_Left);
		...
		if (bUseQuad)
		{
			...
			CDeviceBufferUpr::Instance()->UpdateBuffer(m_vertexBuffer, fullscreenQuadWPOSVertices, 4 * sizeof(SVF_P3F_T2F_T3F));
		}
		else
		{
			...
			CDeviceBufferUpr::Instance()->UpdateBuffer(m_vertexBuffer, fullscreenTriWPOSVertices, 3 * sizeof(SVF_P3F_T2F_T3F));
		}
...
bool CFullscreenPass::Execute()
{
	if (m_bPendingConstantUpdate)
	{
		...
		m_primitive.GetConstantUpr().EndNamedConstantUpdate(&GetViewport());
		m_bPendingConstantUpdate = false;
	}
	else
	{
	...
	{
		DRX_ASSERT(!m_primitive.IsDirty()); // Primitive modified AFTER call to BeginConstantUpdate
