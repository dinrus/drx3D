...
struct DecalSortComparison
{
	...
	{
		u32 normalMap0 = (decal0.nFlags & DECAL_HAS_NORMAL_MAP);
		u32 normalMap1 = (decal1.nFlags & DECAL_HAS_NORMAL_MAP);
...
void CDeferredDecalsStage::ResizeDecalBuffers(size_t requiredDecalCount)
{
	...
	if (allocatedDecalCount < requiredDecalCount)
	{
		...
		for (i32 i = allocatedDecalCount; i < requiredDecalCount; ++i)
		{
	...
	else if (allocatedDecalCount > requiredDecalCount && allocatedDecalCount > MaxPersistentDecals)
	{
		...
		for (i32 i = allocatedDecalCount; i > desiredDecalCount; --i)
		{
...
void CDeferredDecalsStage::SetupDecalPrimitive(const SDeferredDecal& decal, CRenderPrimitive& primitive, _smart_ptr<IRenderShaderResources>& pShaderResources)
{
	...
	static CDrxNameTSCRC techDefDecalVolume = "DeferredDecalVolumeMerged";
	...
	// Update constants
	{
		...
		auto constants = constantUpr.BeginTypedConstantUpdate<SDecalConstants>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Pixel | EShaderStage_Vertex);
		...
		for (u32 i = 0, si = shaderParams.size(); i < si; ++i)
		{
			tukk name = shaderParams[i].m_Name;
	...
	i32 renderState = GS_BLSRC_SRCALPHA | GS_BLDST_ONEMINUSSRCALPHA;
...
void CDeferredDecalsStage::Execute()
{
	...
	for (u32 i = 0, count = deferredDecals.size(); i < count; i++)
	{
