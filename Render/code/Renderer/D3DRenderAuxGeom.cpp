...
void CRenderAuxGeomD3D::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
{
	for (u32 i(0); i < e_auxObjNumLOD; ++i)
	{
...
i32 CRenderAuxGeomD3D::GetDeviceDataSize()
{
	i32 nSize = 0;
...
static void CreateSphere(AuxObjVertexBuffer& vb, AuxObjIndexBuffer& ib, float radius, u32 rings, u32 sections)
{
	// calc required number of vertices/indices/triangles to build a sphere for the given parameters
	u32 numVertices((rings - 1) * (sections + 1) + 2);
	u32 numTriangles((rings - 2) * sections * 2 + 2 * sections);
...
static void CreateCone(AuxObjVertexBuffer& vb, AuxObjIndexBuffer& ib, float radius, float height, u32 sections)
{
	// calc required number of vertices/indices/triangles to build a cone for the given parameters
	u32 numVertices(2 * (sections + 1) + 2);
	u32 numTriangles(2 * sections);
	u32 numIndices(numTriangles * 3);
...
static void CreateCylinder(AuxObjVertexBuffer& vb, AuxObjIndexBuffer& ib, float radius, float height, u32 sections)
{
	// calc required number of vertices/indices/triangles to build a cylinder for the given parameters
	u32 numVertices(4 * (sections + 1) + 2);
	u32 numTriangles(4 * sections);
	u32 numIndices(numTriangles * 3);
	...
	// side
	{
		...
		// build faces
		for (u16 i(0); i < sections; ++i, vIdx += 2)
		{
...
{
	SSphereMeshCreateFunc(float radius, u32 rings, u32 sections)
		: m_radius(radius)
		, m_rings(rings)
		, m_sections(sections)
	{
...
{
	SConeMeshCreateFunc(float radius, float height, u32 sections)
		: m_radius(radius)
		, m_height(height)
		, m_sections(sections)
	{
...
{
	SCylinderMeshCreateFunc(float radius, float height, u32 sections)
		: m_radius(radius)
		, m_height(height)
		, m_sections(sections)
	{
...
i32 CRenderAuxGeomD3D::SDrawObjMesh::GetDeviceDataSize() const
{
	i32 nSize = 0;
...
template<class T = vtx_idx, i32 size = sizeof(T)*CHAR_BIT> struct indexbuffer_type;
...
CRenderAuxGeomD3D::CBufferUpr::~CBufferUpr()
{
	if( vbAux != ~0u ) gcpRendD3D->m_DevBufMan.Destroy(vbAux);
	if( ibAux != ~0u ) gcpRendD3D->m_DevBufMan.Destroy(ibAux);
}

buffer_handle_t CRenderAuxGeomD3D::CBufferUpr::update(BUFFER_BIND_TYPE type, ukk data, size_t size)
{
...
buffer_handle_t CRenderAuxGeomD3D::CBufferUpr::fill(buffer_handle_t buf, BUFFER_BIND_TYPE type, ukk data, size_t size)
{
...
//////////////////////////////////////////////////////////////////////////
CDeviceGraphicsPSOPtr CRenderAuxGeomD3D::GetGraphicsPSO(const SAuxGeomRenderFlags& flags, const CDrxNameTSCRC& techique, ERenderPrimitiveType topology, InputLayoutHandle format)
{
	...
	u32 gsFunc = bReverseDepth ? GS_DEPTHFUNC_GEQUAL : GS_DEPTHFUNC_LEQUAL;
...
//////////////////////////////////////////////////////////////////////////
void CRenderAuxGeomD3D::DrawAuxPrimitives(const CAuxGeomCB::SAuxGeomCBRawData& rawData,CAuxGeomCB::AuxSortedPushBuffer::const_iterator itBegin, CAuxGeomCB::AuxSortedPushBuffer::const_iterator itEnd, const Matrix44& mViewProj, const SRenderViewport& vp, i32 texID)
{
	DrxStackAllocWithSize(HLSL_AuxGeomObjectConstantBuffer, cbObj, CDeviceBufferUpr::AlignBufferSizeForStreaming);
	DrxStackAllocWithSize(HLSL_AuxGeomConstantBuffer, cbPrimObj, CDeviceBufferUpr::AlignBufferSizeForStreaming);
	...
	static CDrxNameTSCRC tGeomText("AuxText");
	static CDrxNameTSCRC tGeom("AuxGeometry");
	static CDrxNameTSCRC tGeomTexture("AuxGeometryTexture");
	static CDrxNameTSCRC tGeomThickLines("AuxGeometryThickLines");
	static CDrxNameTSCRC tGeomObject("AuxGeometryObj");

	CDrxNameTSCRC technique;
	if (bDrawObjects)
	{
	...
	for (auto it = itBegin; it != itEnd; ++it)
	{
		...
		if (bDrawObjects)
		{
			...
			// DRX_ASSERT than all objects in this batch are of same type
			DRX_ASSERT(CAuxGeomCB::GetAuxObjType(entry.m_renderFlags) == objType);
			DRX_ASSERT(rawData.m_auxDrawObjParamBuffer.size() > 0);
			...
			float scale;
			DRX_ASSERT(fabs(v0.w - v0.w) < 1e-4);
			if (fabs(v0.w) < 1e-2)
				scale = 0.5f;
			else
			...
			u32 lodLevel((u32)((scale / 0.5f) * (e_auxObjNumLOD - 1)));
			...
			DRX_ASSERT(lodLevel >= 0 && lodLevel < e_auxObjNumLOD);
...
void CRenderAuxGeomD3D::RT_Flush(const SAuxGeomCBRawDataPackagedConst& data)
{
	...
	DRX_ASSERT(m_renderer.m_pRT->IsRenderThread());

	DRX_ASSERT(data.m_pData);
	...
	if (!m_pAuxGeomShader)
	{
		...
		DRX_ASSERT(m_pAuxGeomShader != nullptr);
	}
...
inline const Matrix44A& CRenderAuxGeomD3D::GetAuxOrthoMatrix(i32 idx) const
{
	DRX_ASSERT(m_pCurCBRawData && idx >= 0 && idx < (i32)m_pCurCBRawData->m_auxOrthoMatrices.size());
	return m_pCurCBRawData->m_auxOrthoMatrices[idx];
}

inline const Matrix34A& CRenderAuxGeomD3D::GetAuxWorldMatrix(i32 idx) const
{
	DRX_ASSERT(m_pCurCBRawData && idx >= 0 && idx < (i32)m_pCurCBRawData->m_auxWorldMatrices.size());
...
bool CRenderAuxGeomD3D::CAuxDeviceResourceSetCacheForTexture::OnTextureInvalidated(uk pListener, SResourceBindPoint bindPoint, UResourceReference resource, u32 invalidationFlags)
{
...
CDeviceResourceSetPtr CRenderAuxGeomD3D::CAuxDeviceResourceSetCacheForTexture::GetOrCreateResourceSet(i32 textureId)
{
...
void CAuxGeomCBCollector::GetMemoryUsage(IDrxSizer* pSizer) const
{
	m_rwGlobal.RLock();
...
CAuxGeomCB* CAuxGeomCBCollector::Get(i32 jobID)
{
	threadID tid = DrxGetCurrentThreadId();
...
void CAuxGeomCBCollector::SThread::GetMemoryUsage(IDrxSizer* pSizer) const
{
	m_rwlLocal.RLock();
...
CAuxGeomCB* CAuxGeomCBCollector::SThread::Get(i32 jobID, threadID tid)
{
	m_rwlLocal.RLock();
