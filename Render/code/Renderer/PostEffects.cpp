...
const CEffectParam* CPostEffectContext::GetEffectParamByName(tukk pszParam) const
{
	DRX_ASSERT(m_pPostEffectsMgr);
...
bool CPostEffectStage::Execute()
{
	...
	for (CPostEffectItor pItor = pPostMgr->GetEffects().begin(), pItorEnd = pPostMgr->GetEffects().end(); pItor != pItorEnd; ++pItor)
	{
		...
		if (pCurrEffect->Preprocess(viewInfo))
		{
			...
			u32 nRenderFlags = pCurrEffect->GetRenderFlags();
			...
			for (u32 i = 0, nNumEffects = activeEffects.size(); i < nNumEffects; ++i)
			{
	...
	if (CRenderer::CV_r_AntialiasingModeDebug > 0)
	{
		...
		{
			static CDrxNameTSCRC pszTechName("DebugPostAA");
		...
		static CDrxNameR pszDebugParams("vDebugParams");
			...
			if (pEffectsParamsUpdated)
			{
				if (!pEffectsParamsUpdated->empty())
				{
					...
					for (StringEffectMapItor pItor = pEffectsParamsUpdated->begin(); pItor != pEnd; ++pItor)
					{
						...
						for (u32 p = 0, nNumParams = activeParams.size(); p < nNumParams; ++p)
						{
...
void CUnderwaterGodRaysPass::Execute(const CPostEffectContext& context)
{
	...
	static CDrxNameR param0Name("PB_GodRaysParamsVS");
	static CDrxNameR param1Name("PB_GodRaysParamsPS");
	...
	// render god-rays into screen.
	{
		...
		{
			static CDrxNameTSCRC techName("UnderwaterGodRaysFinal");
			pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
...
{
	DRX_ASSERT(m_pWaterDropletsBumpTex == nullptr);
...
void CWaterDropletsPass::Execute(const CPostEffectContext& context)
{
	...
	if (pass.IsDirty(pDstTex->GetID(), pSrcTex->GetID()))
	{
		static CDrxNameTSCRC techName("WaterDroplets");
		pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
...
{
	DRX_ASSERT(m_pWaterFlowBumpTex == nullptr);
...
void CWaterFlowPass::Execute(const CPostEffectContext& context)
{
	...
	if (pass.IsDirty(pDstTex->GetID(), pSrcTex->GetID()))
	{
		static CDrxNameTSCRC techName("WaterFlow");
		pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
		pass.SetTechnique(CShaderMan::s_shPostEffectsGame, techName, 0);
		pass.SetState(GS_NODEPTHTEST);
...
void CSharpeningPass::Execute(const CPostEffectContext& context)
{
	...
	if (pass.IsDirty(pDstTex->GetID(), pSrcTex->GetID()))
	{
		static CDrxNameTSCRC techName("CA_Sharpening");
		pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
		pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
		pass.SetTechnique(CShaderMan::s_shPostEffects, techName, 0);
		pass.SetState(GS_NODEPTHTEST);
	...
	static CDrxNameR pParamName("psParams");
...
void CBlurringPass::Execute(const CPostEffectContext& context)
{
	...
	if (pass.IsDirty(pDstTex->GetID(), pSrcTex->GetID()))
	{
		static CDrxNameTSCRC techName("BlurInterpolation");
		pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
		pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
		pass.SetTechnique(CShaderMan::s_shPostEffects, techName, 0);
		pass.SetState(GS_NODEPTHTEST);
...
void CUberGamePostEffectPass::Execute(const CPostEffectContext& context)
{
	...
	if (pPostEffect)
	{
		...
		uint64 rtMask = 0;
		u8 postEffectMask = gamePostEffect.m_nCurrPostEffectsMask;
		...
		{
			static CDrxNameTSCRC techName("UberGamePostProcess");
			pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
			pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
		...
		static CDrxNameR pParamNamePS0("UberPostParams0");
		static CDrxNameR pParamNamePS1("UberPostParams1");
		static CDrxNameR pParamNamePS2("UberPostParams2");
		static CDrxNameR pParamNamePS3("UberPostParams3");
		static CDrxNameR pParamNamePS4("UberPostParams4");
		static CDrxNameR pParamNamePS5("UberPostParams5");
		...
		Vec4 vParamsPS[6] =
		{
			...
			Vec4(drx_random(0.0f,                           1.0f),                                                     drx_random(0.0f,                                  1.0f),                                                                                                   drx_random(0.0f,                                           1.0f),drx_random(0.0f, 1.0f)),
			Vec4(0,                                         0,                                                         0,                                                0),
		};
...
void CFlashBangPass::Execute(const CPostEffectContext& context)
{
	...
	if (pPostEffect)
	{
		...
		if (!postEffect.m_fSpawnTime)
		{
			...
			i32k flags = FT_USAGE_RENDERTARGET | FT_NOMIPS;
		...
		{
			static CDrxNameTSCRC techName("FlashBang");
			pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
			pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
			pass.SetTechnique(CShaderMan::s_shPostEffectsGame, techName, 0);
			pass.SetState(GS_NODEPTHTEST);
		...
		static CDrxNameR pParamName("vFlashBangParams");
		pass.SetConstant(pParamName, vParams);
...
void CPostStereoPass::Execute(const CPostEffectContext& context)
{
	...
	DRX_ASSERT(pTmpMaskTex);
	DRX_ASSERT(pTmpMaskTex->GetWidth() == pSrcBackBufferTexture->GetWidth());
	DRX_ASSERT(pTmpMaskTex->GetHeight() == pSrcBackBufferTexture->GetHeight());
	DRX_ASSERT(pTmpMaskTex->GetDstFormat() == pSrcBackBufferTexture->GetDstFormat());
...

	{
		...
		{
			static CDrxNameTSCRC techName("StereoNearMask");
			pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
			pass.SetTechnique(CShaderMan::s_shPostEffects, techName, 0);

			i32 nRS = GS_DEPTHFUNC_LEQUAL;
...

	{
		...
		{
			static CDrxNameTSCRC techName("PostStereo");
			pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
			pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
			pass.SetTechnique(CShaderMan::s_shPostEffects, techName, 0);
			pass.SetState(GS_NODEPTHTEST);
...
{
	DRX_ASSERT(m_pNoiseTex == nullptr);
...
void CKillCameraPass::Execute(const CPostEffectContext& context)
{
	...
	if (pPostEffect)
	{
		...
		if (pass.IsDirty(pDsttex->GetID(), pSrcTex->GetID()))
		{
			static CDrxNameTSCRC techName("KillCameraFilter");
			pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
			pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
			pass.SetTechnique(CShaderMan::s_shPostEffectsGame, techName, 0);
		...
		// update constant buffer
		{
			...
			pParams[0].x = drx_random(0, 1023) / (float)width;
			pParams[0].y = drx_random(0, 1023) / (float)height;
...
void CScreenBloodPass::Execute(const CPostEffectContext& context)
{
	...
	{
		static CDrxNameTSCRC techName("ScreenBlood");
		pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_ReflectShaderConstants_PS);
		pass.SetPrimitiveType(CRenderPrimitive::ePrim_ProceduralTriangle);
		pass.SetTechnique(CShaderMan::s_shPostEffectsGame, techName, 0);
...
void CScreenFaderPass::Execute(const CPostEffectContext& context)
{
	...
	static CDrxNameR paramName("texToTexParams2");
...
void CHudSilhouettesPass::ExecuteDeferredSilhouettesOptimised(const CPostEffectContext& context, float fBlendParam, float fType, float fFillStrength)
{
	...
	if (bHasSilhouettesToRender)
	{
		...
		if (pass.IsDirty(pDstTex->GetID()))
		{
			static CDrxNameTSCRC techName("DeferredSilhouettesOptimised");
			pass.SetTechnique(CShaderMan::s_shPostEffectsGame, techName, 0);
		...
		static CDrxNameR vsParamName("vsParams");
		...
		static CDrxNameR psParamName("psParams");
...
void CHud3DPass::ExecuteFlashUpdate(const CPostEffectContext& context, CHud3D& hud3d)
{
	...
	u32k nThreadID = gRenDev->GetRenderThreadID();

	u32 nRECount = hud3d.m_pRenderData[nThreadID].size();
	...
	if (nRECount || bForceRefresh) //&& m_nFlashUpdateFrameID != rd->GetFrameID(false) )
	{
		...
		if ((context.GetRenderView()->GetFrameId() % max(1, (i32)CRenderer::CV_r_PostProcessHUD3DCache)) != 0)
		{
			if (!bForceRefresh)
			{
		...
		i32k rtWidth  = std::min<i32>(SHudData::s_nFlashWidthMax , hud3d.m_pHUD_RT->GetWidth());
		i32k rtHeight = std::min<i32>(SHudData::s_nFlashHeightMax, hud3d.m_pHUD_RT->GetHeight());
		...
		for (u32 r = 0; r < nRECount; ++r)
		{
			...
			if (pData.pFlashPlayer && pPrevDiffuse != pDiffuse)
			{
				...
				i32k width  = std::min(pData.pFlashPlayer->GetWidth (), rtWidth);
				i32k height = std::min(pData.pFlashPlayer->GetHeight(), rtHeight);
...
void CHud3DPass::ExecuteDownsampleHud4x4(const CPostEffectContext& context, class CHud3D& hud3d, CTexture* pDstRT)
{
	...
	// tessellation and skinning aren't supported.
	{
		...
		u32 nRECount = renderDataArray.size();
		u32 index = 0;
...
void CHud3DPass::ExecuteFinalPass(const CPostEffectContext& context, CTexture* pOutputRT, CTexture* pOutputDS, CHud3D& hud3d)
{
	...
	u32 hudEffectParamCount = 1; // Default to only pass 1 vecs
	...
	if (interferenceStrength > 0.0f)
	{
		...
		{
			hud3d.m_interferenceRandNums.x = drx_random(-1.0f, 1.0f);
			hud3d.m_interferenceRandNums.y = drx_random(-1.0f, 1.0f);
			hud3d.m_interferenceRandNums.z = drx_random(0.0f, 1.0f);
			hud3d.m_interferenceRandNums.w = drx_random(-1.0f, 1.0f);
	...
	// render HUD meshes. tessellation and skinning aren't supported.
	{
		...
		for (u32 r = 0; r < nRECount; ++r)
		{
			...
			if (SetVertex(prim, pData))
			{
				...
				i32 nRenderState = GS_BLSRC_ONE | GS_BLDST_ONEMINUSSRCALPHA;
...
void CHud3DPass::SetShaderParams(
  ...
  CRenderPrimitive::ConstantUpr& constantUpr,
  ...
  const class CHud3D& hud3d) const
{
	...
	constantUpr.SetNamedConstantArray(hud3d.m_pMatViewProjParamName, (Vec4*)mViewProj.GetData(), 4, eHWSC_Vertex);
	...
	{
		constantUpr.SetNamedConstant(hud3d.m_pHudTexCoordParamName, vHudTexCoordParams, eHWSC_Vertex);
	}

	if (shaderStages & EShaderStage_Pixel)
	{
		constantUpr.SetNamedConstant(hud3d.m_pHudTexCoordParamName, vHudTexCoordParams, eHWSC_Pixel);
		...
		constantUpr.SetNamedConstant(hud3d.m_pHudParamName, vHudParams, eHWSC_Pixel);
	}
}
