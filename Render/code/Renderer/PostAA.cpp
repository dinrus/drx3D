...
void CPostAAStage::CalculateJitterOffsets(i32 renderWidth, i32 renderHeight, CRenderView* pRenderView)
{
	pRenderView->m_vProjMatrixSubPixoffset = Vec2(0.0f, 0.0f);

	// TODO: Support temporal AA in the editor
	u32 aaMode = CRenderer::FX_GetAntialiasingType();
	...
	{
		i32 jitterPattern = CRenderer::CV_r_AntialiasingTAAPattern;
		if (jitterPattern == 1)
		{
		...
		i32k nSampleID = SPostEffectsUtils::m_iFrameCounter;
...
void CPostAAStage::ApplySMAA(CTexture*& pCurrRT)
{
	...
	// Pass 1: Edge Detection
	{
		...
		{
			static CDrxNameTSCRC techEdgeDetection("LumaEdgeDetectionSMAA");
	...
	// Pass 2: Generate blend weight map
	{
		...
		{
			static CDrxNameTSCRC techBlendWeights("BlendWeightSMAA");
	...
	// Final Pass: Blend neighborhood pixels
	{
		...
		{
			static CDrxNameTSCRC techNeighborhoodBlending("NeighborhoodBlendingSMAA");
...
void CPostAAStage::ApplyTemporalAA(CTexture*& pCurrRT, CTexture*& pMgpuRT, u32 aaMode)
{
	CD3D9Renderer* pRenderer = gcpRendD3D;
	...
	DRX_ASSERT_MESSAGE(pDestRT && pPrevRT, "PostAA rendertargets do not exist!");
	DRX_ASSERT_MESSAGE(pCurrRT->GetFlags() & FT_USAGE_ALLOWREADSRGB, "PostAA: Expected sRGB target.");
	if (!pDestRT || !pPrevRT)
		return;
	...
	{
		static CDrxNameTSCRC techTemporalAA("PostAA");
...
void CPostAAStage::DoFinalComposition(CTexture*& pCurrRT, CTexture* pDestRT, u32 aaMode)
{
	PROFILE_LABEL_SCOPE("FLARES, GRAIN");

	CTexture* pTexLensOptics = CRendererResources::s_ptexSceneTargetR11G11B10F[0];
	DRX_ASSERT(pCurrRT != pDestRT);
	...
	{
		static CDrxNameTSCRC techComposition("PostAAComposites");
	...
	{
		static CDrxNameR paramsName("vParams");
		static CDrxNameR lensOpticsParamsName("vLensOpticsParams");
		static CDrxNameR hdrParamsName("HDRParams");
		static CDrxNameR hdrEyeAdaptationName("HDREyeAdaptation");
...
void CPostAAStage::Resize(i32 renderWidth, i32 renderHeight)
{
...
CTexture* CPostAAStage::GetAARenderTarget(const CRenderView* pRenderView, bool bCurrentFrame) const
{
	...
	i32 index = (bCurrentFrame ? SPostEffectsUtils::m_iFrameCounter : (SPostEffectsUtils::m_iFrameCounter + 1)) % 2;

	DRX_ASSERT(eye == CCamera::eEye_Left || eye == CCamera::eEye_Right);
