...
#include <drx3D/Render/DevicePSO.h>

static u8 g_StencilFuncLookup[8] =
{
...
void CDeviceGraphicsPSODesc::FillDescs(D3D11_RASTERIZER_DESC& rasterizerDesc, D3D11_BLEND_DESC& blendDesc, D3D11_DEPTH_STENCIL_DESC& depthStencilDesc) const
{
	...
	u32k renderState = m_RenderState;
...

	{
		for (u32 i = 0; i < 4; ++i)
		{
			u32 mask = 0xfffffff0 | (ColorMasks[(renderState & GS_COLMASK_MASK) >> GS_COLMASK_SHIFT][i]);
			mask = (~mask) & 0xf;
...
u32 CDeviceGraphicsPSODesc::CombineVertexStreamMasks(u32 fromShader, u32 fromObject) const
{
	u32 result = fromShader;
...
CDeviceComputePSODesc::CDeviceComputePSODesc(CDeviceResourceLayoutPtr pResourceLayout, ::CShader* pShader, const CDrxNameTSCRC& technique, uint64 rtFlags, u32 mdFlags, u32 mdvFlags)
{
	InitWithDefaults();
...
bool CDeviceGraphicsPSO::ValidateShadersAndTopologyCombination(const CDeviceGraphicsPSODesc& psoDesc, const std::array<uk , eHWSC_Num>& hwShaderInstances)
{
