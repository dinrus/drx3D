...
#include <drx3D/Render/D3D/GraphicsPipeline/Common/GraphicsPipelineStateSet.h>
...
bool SResourceBinding::IsValid() const
{
	...
	default: DRX_ASSERT(false);         return false;
	}
}
...
bool SResourceBinding::IsVolatile() const
{
	...
	default: DRX_ASSERT(false);         return true;
	}
}
...
const std::pair<SResourceView, CDeviceResourceView*>* SResourceBinding::GetDeviceResourceViewInfo() const
{
	...
	default:                      DRX_ASSERT(false);                           break;
	}
...
DXGI_FORMAT SResourceBinding::GetResourceFormat() const
{
	if (fastCompare)
	{
		...
		DRX_ASSERT(view == EDefaultResourceViews::Default || view == EDefaultResourceViews::RasterizerTarget); // only safe with default views
		DRX_ASSERT(type == EResourceType::Texture && pTexture); // TODO implement for buffers
...
SResourceBindPoint::SResourceBindPoint(const SResourceBinding& resource, u8 _slotNumber, EShaderStage shaderStages)
{
	...
	{
		DRX_ASSERT(resource.view == EDefaultResourceViews::Default || resource.pTexture->GetDevTexture());
	...
	{
		DRX_ASSERT(resource.view == EDefaultResourceViews::Default || resource.pBuffer->GetDevBuffer());
...
SResourceBindPoint::SResourceBindPoint(ESlotType _type, u8 _slotNumber, EShaderStage _shaderStages, EFlags _flags)
	: slotType(_type)
	, flags(_flags)
	, slotNumber(_slotNumber)
...
CDeviceResourceSetDesc::CDeviceResourceSetDesc(const CDeviceResourceSetDesc& other, uk pInvalidateCallbackOwner, const SResourceBinding::InvalidateCallbackFunction& invalidateCallback)
{
...
bool CDeviceResourceSetDesc::OnResourceInvalidated(uk pThis, SResourceBindPoint bindPoint, UResourceReference pResource, u32 flags) threadsafe
{
	...
	if (flags & eResourceDestroyed)
	{
		...
		if (it != pSet->m_resources.end())
		{
			...
			UResourceReference pFoundResource = DrxInterlockedCompareExchangePointer((uk *)&existingBinding.fastCompare, nullptr, pResource.pAnonymous);
...
void SDeviceResourceLayoutDesc::SetResourceSet(u32 bindSlot, const CDeviceResourceSetDesc& resourceSet)
{
...
void SDeviceResourceLayoutDesc::SetConstantBuffer(u32 bindSlot, EConstantBufferShaderSlot shaderSlot, ::EShaderStage shaderStages)
{
...
bool SDeviceResourceLayoutDesc::IsValid() const
{
	...
	auto GetBindPointName = [](SResourceBindPoint bindPoint)
	{
		...
		drx_sprintf(buffer, "%c%d", slotPrefix[min((size_t)bindPoint.slotType, DRX_ARRAY_COUNT(slotPrefix) - 1)], bindPoint.slotNumber);

		return buffer;
	};

	std::set<u32> usedLayoutBindSlots;
	std::map<i16, SResourceBinding> usedShaderBindSlotsS[(u8)SResourceBindPoint::ESlotType::Count][eHWSC_Num]; // used slot numbers per slot type and shader stage
	std::map<i16, SResourceBinding> usedShaderBindSlotsL[(u8)SResourceBindPoint::ESlotType::Count][EResourceLayoutSlot_Max + 1];

	auto validateLayoutSlot = [&](u32 layoutSlot)
	{
		if (usedLayoutBindSlots.insert(layoutSlot).second == false)
		{
			DRX_ASSERT_TRACE(false, ("Invalid Resource Layout: Multiple resources on layout (CPU side) slot %d", layoutSlot));
			return false;
		}
	...
	auto validateResourceBindPoint = [&](u8 layoutSlot, SResourceBindPoint bindPoint, const SResourceBinding& resource)
	{
		...
		for (EHWShaderClass shaderClass = eHWSC_Vertex; validShaderStages; shaderClass = EHWShaderClass(shaderClass + 1), validShaderStages >>= 1)
		{
			if (validShaderStages & 1)
			{
				...
				auto insertResultS = usedShaderBindSlotsS[u8(bindPoint.slotType)][shaderClass].insert(std::make_pair(bindPoint.slotNumber, resource));
				if (insertResultS.second == false)
				{
					auto& existingResource = insertResultS.first->second;
					auto& currentResource = resource;

					DRX_ASSERT_TRACE(false, ("Invalid Resource Layout : Multiple resources bound to shader slot %s across multiple layoutSlots: A: %s - B: %s",
		...
		auto insertResultL = usedShaderBindSlotsL[u8(bindPoint.slotType)][layoutSlot].insert(std::make_pair(bindPoint.slotNumber, resource));
		if (insertResultL.second == false)
		{
			auto& existingResource = insertResultL.first->second;
			auto& currentResource = resource;

			DRX_ASSERT_TRACE(false, ("Invalid Resource Layout : Multiple resources bound to shader slot %s within the same layoutSlot: A: %s - B: %s",
				GetBindPointName(bindPoint), GetResourceName(existingResource), GetResourceName(currentResource)));
	...
	// Make sure there are no 'holes' in the used binding slots
	{
		...
		for (auto slot : usedLayoutBindSlots)
		{
			...
			{
				DRX_ASSERT_MESSAGE(false, "Invalid Resource Layout: gap in layout (CPU side) slots");
				return false;
			}
