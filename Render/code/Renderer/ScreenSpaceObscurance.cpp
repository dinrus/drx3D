...
void CScreenSpaceObscuranceStage::Execute()
{
	...
	// Obscurance generation
	{
		...
		static CDrxNameTSCRC techSampling("SSDO_Sampling");
	...
	{
		i32k sizeX = CRendererResources::s_ptexLinearDepth->GetWidth();
		i32k sizeY = CRendererResources::s_ptexLinearDepth->GetHeight();
		i32k srcSizeX = CRendererResources__s_ptexSceneSpecular->GetWidth();
		i32k srcSizeY = CRendererResources__s_ptexSceneSpecular->GetHeight();

		if (m_passFilter.IsDirty(bLowResOutput))
		{
			static CDrxNameTSCRC techFilter("SSDO_Filter");
		...
		static CDrxNameR sourceTexSizeName("SSDO_SourceTexSize");
		static CDrxNameR blurOffsetName("SSDO_BlurOffset");
		static CDrxNameR blurKernelName("SSDO_BlurKernel");
