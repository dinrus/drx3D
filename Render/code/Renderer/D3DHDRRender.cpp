...
#include <drx3D/Render/D3D/GraphicsPipeline/Bloom.h>
...
void CHDRPostProcess::AddRenderTarget(u32 nWidth, u32 nHeight, const ColorF& cClear, ETEX_Format Format, float fPriority, tukk szName, CTexture** pStorage, u32 nFlags, i32 nCustomID, bool bDynamicTex)
{
...
void CTexture::GenerateHDRMaps()
{
	...
	i32k width  = r->GetWidth(),  width_r2  = (width  + 1) / 2, width_r4  = (width_r2  + 1) / 2, width_r8  = (width_r4  + 1) / 2, width_r16  = (width_r8  + 1) / 2;
	i32k height = r->GetHeight(), height_r2 = (height + 1) / 2, height_r4 = (height_r2 + 1) / 2, height_r8 = (height_r4 + 1) / 2, height_r16 = (height_r8 + 1) / 2;
	...
	u32 nHDRTargetFlags = FT_DONT_RELEASE | (CRenderer::CV_r_msaa ? FT_USAGE_MSAA : 0);
	u32 nHDRTargetFlagsUAV = nHDRTargetFlags | (CRenderer::CV_r_msaa ? 0 : FT_USAGE_UNORDERED_ACCESS);  // UAV required for tiled deferred shading
