...
#include <drx3D/Render/D3D11/DeviceObjects_D3D11.h>
...
uint8_t SInputLayoutCompositionDescriptor::GenerateShaderMask(const InputLayoutHandle VertexFormat, ID3D11ShaderReflection* pShaderReflection)
{
	...
	std::vector<tukk > reflectedNames;
	...
	for (u32 i=0; i<Desc.InputParameters; i++)
	{
		...
		auto it = std::lower_bound(reflectedNames.begin(), reflectedNames.end(), Sig.SemanticName, [](tukk const lhs, tukk const rhs)
		{
	...
	for (i32 i = 0; layout_it != layoutDescriptor->m_Declaration.cend() && vs_it != reflectedNames.cend(); ++i, ++layout_it)
	{
		i32 compResult;
...
VertexDecls[EDefaultInputLayouts::PreAllocated] =
{
	...
	// Base stream
	{ DRX_ARRAY_COUNT(VertexDecl_P3F_C4B_T2F     ), VertexDecl_P3F_C4B_T2F      },
	{ DRX_ARRAY_COUNT(VertexDecl_P3S_C4B_T2S     ), VertexDecl_P3S_C4B_T2S      },
	{ DRX_ARRAY_COUNT(VertexDecl_P3S_N4B_C4B_T2S ), VertexDecl_P3S_N4B_C4B_T2S  },

	{ DRX_ARRAY_COUNT(VertexDecl_P3F_C4B_T4B_N3F2), VertexDecl_P3F_C4B_T4B_N3F2 },
	{ DRX_ARRAY_COUNT(VertexDecl_TP3F_C4B_T2F    ), VertexDecl_TP3F_C4B_T2F     },
	{ DRX_ARRAY_COUNT(VertexDecl_P3F_T3F         ), VertexDecl_P3F_T3F          },
	{ DRX_ARRAY_COUNT(VertexDecl_P3F_T2F_T3F     ), VertexDecl_P3F_T2F_T3F      },

	{ DRX_ARRAY_COUNT(VertexDecl_P3F             ), VertexDecl_P3F              },

	{ DRX_ARRAY_COUNT(VertexDecl_P2S_N4B_C4B_T1F ), VertexDecl_P2S_N4B_C4B_T1F  },
	{ DRX_ARRAY_COUNT(VertexDecl_P3F_C4B_T2S     ), VertexDecl_P3F_C4B_T2S      },

	// Additional streams
	{ DRX_ARRAY_COUNT(VertexDecl_T4F_B4F         ), VertexDecl_T4F_B4F          },
	{ DRX_ARRAY_COUNT(VertexDecl_T4S_B4S         ), VertexDecl_T4S_B4S          },
	{ DRX_ARRAY_COUNT(VertexDecl_Q4F             ), VertexDecl_Q4F              },
	{ DRX_ARRAY_COUNT(VertexDecl_Q4S             ), VertexDecl_Q4S              },
	{ DRX_ARRAY_COUNT(VertexDecl_N3F             ), VertexDecl_N3F              },
	{ DRX_ARRAY_COUNT(VertexDecl_W4B_I4S         ), VertexDecl_W4B_I4S          },
	{ DRX_ARRAY_COUNT(VertexDecl_V3F             ), VertexDecl_V3F              },
	{ DRX_ARRAY_COUNT(VertexDecl_W2F             ), VertexDecl_W2F              },

	{ DRX_ARRAY_COUNT(VertexDecl_V4Fi            ), VertexDecl_V4Fi             }
};
...
const SInputLayout* CDeviceObjectFactory::GetInputLayoutDescriptor(const InputLayoutHandle VertexFormat)
{
	...
	if (idx >= size)
	{
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_RENDERER, EValidatorSeverity::VALIDATOR_ERROR, "GetInputLayoutDescriptor(): Supplied handle is neither a predefined format nor was created via CreateCustomVertexFormat()!");
		return nullptr;
	}
...
const CDeviceObjectFactory::SInputLayoutPair* CDeviceObjectFactory::GetOrCreateInputLayout(const SShaderBlob* pVertexShader, i32 StreamMask, const InputLayoutHandle VertexFormat)
{
	DRX_ASSERT(pVertexShader);
	if (!pVertexShader)
		return nullptr;
...
SInputLayout CDeviceObjectFactory::CreateInputLayoutForPermutation(const SShaderBlob* m_pConsumingVertexShader, const SInputLayoutCompositionDescriptor &compositionDescription, i32 StreamMask, const InputLayoutHandle VertexFormat)
{
	bool bInstanced = (StreamMask & VSM_INSTANCED) != 0;

	const auto* layoutDescriptor = GetInputLayoutDescriptor(VertexFormat);
	DRX_ASSERT(layoutDescriptor);
	if (!layoutDescriptor)
		return SInputLayout({});
	...
	for (i32 i=0; i<layoutDescriptor->m_Declaration.size(); ++i)
...
{
	DrxAutoCriticalSectionNoRecursive lock(m_RenderPassCacheLock);
...
bool CDeviceObjectFactory::OnRenderPassInvalidated(uk pRenderPass, SResourceBindPoint bindPoint, UResourceReference pResource, u32 flags)
{
...
void CDeviceObjectFactory::ReloadPipelineStates(i32 currentFrameID)
{
...
void CDeviceObjectFactory::TrimPipelineStates(i32 currentFrameID, i32 trimBeforeFrameID)
{
	...
	CDeviceStatesUprDX11::GetInstance()->ReleaseUnusedStates(0 /* kill them all */);
#endif
}
