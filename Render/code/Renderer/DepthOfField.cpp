...
void CDepthOfFieldStage::Execute()
{
	...
	static CDrxNameR dofFocusParam0Name("vDofParamsFocus0");
	static CDrxNameR dofFocusParam1Name("vDofParamsFocus1");
	static CDrxNameR dofTapsName("g_Taps");

	{
		// 1st downscale stage
		{
			...
			{
				static CDrxNameTSCRC techNameDownscale("DownscaleDof");
		...
		// 2nd downscale stage (tile min CoC)
		{
			...
			for (u32 i = 1; i < MIN_DOF_COC_K; i++)
			{
				if (m_passTileMinCoC[i].IsDirty())
				{
					static CDrxNameTSCRC techNameTileMinCoC("TileMinCoC");
...

	{
		// 1st gather pass
		{
			...
			{
				static CDrxNameTSCRC techDOF("Dof");
		...
		// Final composition
		{
			...
			{
				static CDrxNameTSCRC techCompositeDof("CompositeDof");
