...
#include <drx3D/CoreX/BitFiddling.h>    // ConvertBlock3DcToDXT5()
...
static bool DecompressSubresourcePayload(const STextureLayout& pLayout, const ETEX_TileMode eSrcTileMode, ukk pData[], STexturePayload& pPayload)
{
	...
	i32k nSlices = pLayout.m_nArraySize;
	i32k nMips = pLayout.m_nMips;
	...
	ukk* pDataCursor = pData;

	PREFAST_ASSUME(*pDataCursor != nullptr);

	for (i32 nSlice = 0, nSubresource = 0; (nSlice < nSlices); ++nSlice)
	{
		i32 w = pLayout.m_nWidth;
		i32 h = pLayout.m_nHeight;
		i32 d = pLayout.m_nDepth;

		for (i32 nMip = 0; (nMip < nMips) && (w | h | d); ++nMip, ++nSubresource)
		{
			if (!w) w = 1;
			if (!h) h = 1;
			if (!d) d = 1;
...
byte* CTexture::Convert(byte* pSrc, i32 nWidth, i32 nHeight, i32 nMips, ETEX_Format eTFSrc, ETEX_Format eTFDst, i32 nOutMips, i32& nOutSize, bool bLinear)
{
	nOutSize = 0;
	byte* pDst = NULL;
		...
		if (eTFDst == eTF_BC3)
		{
			...
			for (i = 0; i < nOutMips; i++)
			{
...

				{
					...
					for (u32 n = 0; n < outSize / 16; n++)     // for each block
					{
						u8* pSrcBlock = &src[n * 16];
						u8* pDstBlock = &pDst[nOffsDST + n * 16];
...
D3DSurface* CTexture::GetSurface(i32 nCMSide, i32 nLevel)
{
	if (!m_pDevTexture)
		return NULL;
	...
	i32k nMipLevel = (m_eFlags & FT_FORCE_MIPS) ? min((i32)(m_nMips - 1), nLevel) : 0;
	i32k nSlice = (m_eTT == eTT_Cube || m_eTT == eTT_CubeArray ? nCMSide : 0);
	i32k nSliceCount = (m_eTT == eTT_Cube || m_eTT == eTT_CubeArray ? 1 : -1);
...
D3DSurface* CTexture::GetSurface(i32 nCMSide, i32 nLevel) const
{
	if (!m_pDevTexture)
		return NULL;
...
bool CTexture::Resolve(i32 nTarget, bool bUseViewportSize)
{
	if (m_bResolved)
		return true;
...
bool CTexture::CreateDeviceTexture(ukk pData[])
{
	DRX_ASSERT(m_eDstFormat != eTF_Unknown);
	DRX_ASSERT(!pData || !*pData || m_eSrcTileMode != eTM_Unspecified);
...
bool CTexture::RT_CreateDeviceTexture(ukk pData[])
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CTexture::RT_CreateDeviceTexture");
...
#if DRX_RENDERER_VULKAN || DRX_PLATFORM_ORBIS
...

	{
		m_nDevTextureSize = m_nPersistentSize = m_pDevTexture->GetDeviceSize();
		 ...
		 size_t* pTexMem = &CTexture::s_nStatsCurManagedNonStreamedTexMem;
		if (IsDynamic())
			pTexMem = &CTexture::s_nStatsCurDynamicTexMem;
		
		DrxInterlockedAdd(pTexMem, m_nDevTextureSize);
	}
...
void CTexture::ReleaseDeviceTexture(bool bKeepLastMips, bool bFromUnload) threadsafe
{
	...
	if (!m_bNoTexture)
	{
		...
		if (m_pFileTexMips)
		{
			...
			if (bKeepLastMips)
			{
				i32k nLastMipsStart = m_nMips - m_CacheFileHeader.m_nMipsPersistent;
				i32 nSides = StreamGetNumSlices();
				for (i32 nS = 0; nS < nSides; nS++)
				{
					for (i32 i = 0; i < nLastMipsStart; i++)
					{
...
bool SSamplerState::SetFilterMode(i32 nFilter)
{
	IF (nFilter < 0, 0)
...
bool SSamplerState::SetDefaultFilterMode(i32 nFilter)
{
...
void CTexture::UpdateTextureRegion(byte* pSrcData, i32 nX, i32 nY, i32 nZ, i32 USize, i32 VSize, i32 ZSize, ETEX_Format eSrcFormat)
{
	...
	{
		i32 nSize = TextureDataSize(USize, VSize, ZSize, GetNumMips(), 1, eSrcFormat);
		std::shared_ptr<std::vector<u8>> tempData = std::make_shared<std::vector<u8>>(pSrcData,pSrcData+nSize);
...
void CTexture::RT_UpdateTextureRegion(byte* pSrcData, i32 nX, i32 nY, i32 nZ, i32 USize, i32 VSize, i32 ZSize, ETEX_Format eSrcFormat)
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CTexture::RT_UpdateTextureRegion");
	...
	DRX_ASSERT(m_eSrcTileMode == eTM_None || m_eSrcTileMode == eTM_Unspecified);
	DRX_ASSERT(m_nArraySize == 1);
	for (i32 i = 0; i < m_nMips; i++)
	{
		if (!USize)
			USize = 1;
		if (!VSize)
...
bool CTexture::Clear(const ColorF& color)
{
	...
	gRenDev->ExecuteRenderThreadCommand([=]
	{
		...
		if (pSelf->GetFlags() & FT_USAGE_DEPTHSTENCIL)
			CClearSurfacePass::Execute(pSelf, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, color.r, (u8)color.g); // NOTE: normalized depth in color.r and unnormalized stencil in color.g
...
static void DrawSceneToCubeSide(CRenderOutputPtr pRenderOutput, const Vec3& Pos, i32 tex_size, i32 side)
{
	DRX_ASSERT(gRenDev->m_pRT->IsMainThread());
	if (!iSystem)
		return;
	...
	i32 nRFlags = SHDF_CUBEMAPGEN | SHDF_ALLOWPOSTPROCESS | SHDF_ALLOWHDR | SHDF_ZPASS | SHDF_NOASYNC | SHDF_ALLOW_AO;
	u32 nRenderPassFlags = SRenderingPassInfo::DEFAULT_FLAGS | SRenderingPassInfo::CUBEMAP_GEN;
...
{
	template<typename T>
	SCvarOverrideHelper(tukk cvarName, T value)
	{
	...
	void ApplyValue()
	{
		if (m_pCVar)
		{
			...
			{
			case CVAR_INT:
				m_previousValue.emplace<i32>(m_pCVar->GetIVal());
				m_pCVar->Set(stl::get<i32>(m_desiredValue));
				break;
			case CVAR_FLOAT:
	...
	void RestoreValue()
	{
		if (m_pCVar)
		{
			...
			{
			case CVAR_INT:
				m_pCVar->Set(stl::get<i32>(m_previousValue));
				break;
			case CVAR_FLOAT:
...
private:
	DrxVariant<i32, float> m_desiredValue;
	DrxVariant<i32, float> m_previousValue;
	ICVar*                 m_pCVar;
};
...
DynArray<std::uint16_t> CTexture::RenderEnvironmentCMHDR(std::size_t size, const Vec3& Pos)
{
	...
	for (i32 nSide = 0; nSide < 6; nSide++)
	{
		i32 waitFrames = max(0, CRendererCVars::CV_r_CubemapGenerationTimeout);
		while (waitFrames-->0)
		{
		...
		if (waitFrames<0)
		{
			DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_WARNING, 
		...
		gcpRendD3D->ExecuteRenderThreadCommand([&]
		{
			...
			pDstDevTex->DownloadToStagingResource(0, [&](uk pData, u32 rowPitch, u32 slicePitch)
			{
				const auto* pTarg = reinterpret_cast<const std::uint16_t*>(pData);
				u32k nLineStride = CTexture::TextureDataSize(size, 1, 1, 1, 1, eTF_R16G16B16A16F) / sizeof(*pTarg);

				// Copy vertically flipped image
				for (u32 nLine = 0; nLine < size; ++nLine)
				{
...
static i32 __cdecl RTCallback(const VOID* arg1, const VOID* arg2)
{
	CTexture* p1 = *(CTexture**)arg1;
	CTexture* p2 = *(CTexture**)arg2;

	// show big textures first
	i32 nSize1 = p1->GetDataSize();
	i32 nSize2 = p2->GetDataSize();
	if (nSize1 > nSize2)
		return -1;
...
void CD3D9Renderer::DrawAllDynTextures(tukk szFilter, const bool bLogNames, const bool bOnlyIfUsedThisFrame)
{
#ifndef _RELEASE
	char name[256]; //, nm[256];
	drx_strcpy(name, szFilter);
	strlwr(name);
	TArray<CTexture*> UsedRT;
	i32 nMaxCount = CV_r_ShowDynTexturesMaxCount;
	...
	else
	{
		...
		for (it = pRL->m_RMap.begin(); it != pRL->m_RMap.end(); it++)
		{
			...
			{
				char nameBuffer[128];
				drx_strcpy(nameBuffer, tp->GetName());
				strlwr(nameBuffer);
				if (DrxStringUtils::MatchWildcard(nameBuffer, name))
					UsedRT.AddElem(tp);
			}
		}
	#if 0
	...
	x = 0;
	y = 0;
	for (u32 i = 0; i < UsedRT.Num(); i++)
	{
		//SetState(GS_NODEPTHTEST);
		CTexture* tp = UsedRT[i];
		i32 nSavedAccessFrameID = tp->m_nAccessFrameID;
		...
		tukk pTexName = tp->GetName();
		char nameBuffer[128];
		memset(nameBuffer, 0, sizeof nameBuffer);
		for (i32 iStr = 0, jStr = 0; pTexName[iStr] && jStr < sizeof nameBuffer - 1; ++iStr)
		{
			if (pTexName[iStr] == '$')
			{
				nameBuffer[jStr] = '$';
...
bool CFlashTextureSourceBase::Update()
{
	...
	i32k rtWidth  = GetWidth();
	i32k rtHeight = GetHeight();
...
void CFlashTextureSourceSharedRT::ProbeDepthStencilSurfaceCreation(i32 width, i32 height)
{
...
void CTexture::CopySliceChain(CDeviceTexture* const pDstDevTex, i32 nDstNumMips, i32 nDstSliceOffset, i32 nDstMipOffset,
...
#if DRX_PLATFORM_DURANGO && (DRX_RENDERER_DIRECT3D >= 110) && (DRX_RENDERER_DIRECT3D < 120)  && defined(DEVICE_SUPPORTS_PERFORMANCE_DEVICE)
	else if (!gcpRendD3D->m_pRT->IsRenderThread(true))
	{
		// We can use the move engine!

		DrxAutoLock<DrxCriticalSection> dmaLock(GetDeviceObjectFactory().m_dma1Lock);
		ID3D11DmaEngineContextX* pDMA = GetDeviceObjectFactory().m_pDMA1;

		for (i32 nMip = 0; nMip < nNumMips; ++nMip)
		{
...
i16 CTexture::StreamCalculateMipsSignedFP(float fMipFactor) const
{
	assert(IsStreamed());
	...
	i32 nMip = static_cast<i32>(floorf(fMip * 256.0f));
	i32k nNewMip = min(nMip, (m_nMips - m_CacheFileHeader.m_nMipsPersistent) << 8);
	return (i16)nNewMip;
}

float CTexture::StreamCalculateMipFactor(i16 nMipsSigned) const
{
