...
#include <drx3D/Render/VolumetricClouds.h>
...
struct VCCloudRenderContext
{
	...
	i32     cloudVolumeTexId;
	i32     cloudNoiseTexId;
	i32     edgeNoiseTexId;
	i32     fullscreenWidth;
	i32     fullscreenHeight;
	i32     screenWidth;
	i32     screenHeight;
	i32     screenDepth;
	bool      stereoReproj;
...
namespace
{
const static i32 VCVerticalShadowTexSize = 8;
...
const static i32 VCFrameCountCycle = 1024;
...
i32 VCGetDownscaleRatio()
{
	return clamp_tpl<i32>(CRenderer::CV_r_VolumetricClouds, 1, 2); // half and quarter res are available.
}

i32 VCGetDownscaledResolution(i32 resolution, i32 downscaleRatio)
{
	for (i32 i = 0; i < downscaleRatio; ++i)
	{
...
i32 VCGetCloudShadowTexSize()
{
	i32 size = clamp_tpl<i32>(CRenderer::CV_r_VolumetricCloudsShadowResolution, 16, 512);
	size = size - (size % 16); // keep multiple of 16.
	return size;
}

i32 VCGetCloudRaymarchStepNum()
{
	i32 num = clamp_tpl<i32>(CRenderer::CV_r_VolumetricCloudsRaymarchStepNum, 16, 256);
...
{
	i32k shadowTexSize = VCGetCloudShadowTexSize();
...
CVolumetricCloudsStage::CVolumetricCloudsStage()
	...
	, m_tick(-1)
{
	for (i32 e = 0; e < MaxEyeNum; ++e)
	{
		m_nUpdateFrameID[e] = -1;

		for (i32 i = 0; i < MaxFrameNum; ++i)
		{
...
void CVolumetricCloudsStage::Update()
{
	...
	i32k screenWidth  = CRendererResources::s_renderWidth;
	i32k screenHeight = CRendererResources::s_renderHeight;
	...
	{
		i32k shadowTexSize = VCGetCloudShadowTexSize();
		i32k w = shadowTexSize;
		i32k h = shadowTexSize;
		i32k d = VCVerticalShadowTexSize;
		const ETEX_Format shadowTexFormat = eTF_R11G11B10F;
		i32k nRTFlags = FT_NOMIPS | FT_USAGE_UNORDERED_ACCESS;

		i32k width = CRendererResources::s_ptexVolCloudShadow->GetWidth();
		i32k height = CRendererResources::s_ptexVolCloudShadow->GetHeight();
	...
	i32k scale = VCGetDownscaleRatio();
	i32k scaledWidth = VCGetDownscaledResolution(screenWidth, scale);
	i32k scaledHeight = VCGetDownscaledResolution(screenHeight, scale);
	i32k depth = VCGetCloudRaymarchStepNum();
	u32k rtFlags = FT_NOMIPS | FT_USAGE_UNORDERED_ACCESS | FT_USAGE_RENDERTARGET;

	bool bReset = false;

	const auto createTexture2D = [=](CTexture* pTex, ETEX_Format texFormat, u32 flags, bool cond) -> bool
	{
		if (pTex != nullptr
...
void CVolumetricCloudsStage::ExecuteShadowGen()
{
	...
	i32k currEye = static_cast<i32>(RenderView()->GetCurrentEye());
	...
	if (bLeftEye || bRightEye)
	{
		...
		i32k currentFrameIndex = GetCurrentFrameIndex();

		DRX_ASSERT(viewInfoCount == 1 || viewInfoCount == 2);
...
void CVolumetricCloudsStage::Execute()
{
	...
	{
		i32k curEye = static_cast<i32>(RenderView()->GetCurrentEye());
		i32k gpuCount = rd->GetActiveGPUCount();
		auto* pVolFogStage = rd->GetGraphicsPipeline().GetVolumetricFogStage();

		CShader* pShader = CShaderMan::s_ShaderClouds;

		i32k bufferIndex = GetBufferIndex(gpuCount, context.bStereoMultiGPURendering);
		i32k previousFrameIndex = GetPreviousFrameIndex(gpuCount, context.bStereoMultiGPURendering);

		DRX_ASSERT(curEye == 0 || curEye == 1);
		...
		DRX_ASSERT(prevScaledDepth->GetWidth() == context.scaledZTarget->GetWidth());
		DRX_ASSERT(prevScaledDepth->GetHeight() == context.scaledZTarget->GetHeight());
		...
		// apply temporal re-projection to clouds texture for max-depth.
		{
			...
			{
				static CDrxNameTSCRC shaderName = "ReprojectClouds";
		...
		// upscale and blend it with scene.
		{
			...
			if (pass.IsDirty(inputFlag, CRenderer::CV_r_VolumetricClouds, currMinTex->GetID(), currMaxTex->GetID(), zTarget->GetID(), context.scaledZTarget->GetID()))
			{
				...
				static CDrxNameTSCRC shaderName = "CloudsBlend";
				pass.SetPrimitiveFlags(CRenderPrimitive::eFlags_None);
				pass.SetTechnique(pShader, shaderName, rtMask);
...
void CVolumetricCloudsStage::ExecuteVolumetricCloudShadowGen()
{
	...
	i32k widthVolShadowTex = CRendererResources::s_ptexVolCloudShadow->GetWidth();
	i32k heightVolShadowTex = CRendererResources::s_ptexVolCloudShadow->GetHeight();
	i32k depthVolShadowTex = CRendererResources::s_ptexVolCloudShadow->GetDepth();
	...
	if (pass.IsDirty(inputFlag, cloudVolumeTex->GetID(), cloudNoiseTex->GetID(), cloudEdgeNoiseTex->GetID()))
	{
		...
		static CDrxNameTSCRC shaderName = "CloudShadowGen";
		pass.SetTechnique(pShader, shaderName, rtMask);
	...
	u32k nTileSizeY = 16;
	u32k nTileSizeZ = 1;
	u32k dispatchSizeX = (widthVolShadowTex / nTileSizeX) + ((widthVolShadowTex % nTileSizeX) > 0 ? 1 : 0);
	u32k dispatchSizeY = (heightVolShadowTex / nTileSizeY) + ((heightVolShadowTex % nTileSizeY) > 0 ? 1 : 0);
	u32k dispatchSizeZ = (depthVolShadowTex / nTileSizeZ) + ((depthVolShadowTex % nTileSizeZ) > 0 ? 1 : 0);
...
void CVolumetricCloudsStage::GenerateCloudShadowGenShaderParam(const Vec3& texSize)
{
	...
	DRX_ASSERT(m_pCloudShadowConstantBuffer->m_size >= sizeof(SCloudShadowShaderParam));
...
void CVolumetricCloudsStage::ExecuteComputeDensityAndShadow(const VCCloudRenderContext& context)
{
	if (CRenderer::CV_r_VolumetricCloudsPipeline != 0) // Inject clouds density and shadow to frustum aligned volume texture.
	{
		...
		CD3D9Renderer* const __restrict rd = gcpRendD3D;
		i32k currentRenderEye = static_cast<i32>(RenderView()->GetCurrentEye());
		...
		{
			static CDrxNameTSCRC shaderName = "InjectCloudDensityAndShadow";
			pass.SetTechnique(pShader, shaderName, rtMask);
		...
		i32 screenWidth = context.screenWidth;
		i32 screenHeight = context.screenHeight;
		...
		if (CVrProjectionUpr::IsMultiResEnabledStatic())
		{
			...
			auto constantBuffer = CVrProjectionUpr::Instance()->GetProjectionConstantBuffer(screenWidth, screenHeight);
		...
		u32k nTileSizeY = 8;
		u32k nTileSizeZ = 1;
		u32k dispatchSizeX = gridWidth / nTileSizeX + (gridWidth % nTileSizeX > 0 ? 1 : 0);
		u32k dispatchSizeY = gridHeight / nTileSizeY + (gridHeight % nTileSizeY > 0 ? 1 : 0);
		u32k dispatchSizeZ = 1;
...
void CVolumetricCloudsStage::ExecuteRenderClouds(const VCCloudRenderContext& context)
{
	...
	if (pass.IsDirty(rtMask, inputFlag, cloudVolumeTex->GetID()))
	{
		...
		static CDrxNameTSCRC shaderName0 = "RenderCloud_Monolithic";
		static CDrxNameTSCRC shaderName1 = "RenderCloud";
		CDrxNameTSCRC& shaderName = bMono ? shaderName0 : shaderName1;
		pass.SetTechnique(pShader, shaderName, rtMask);
...
void CVolumetricCloudsStage::GenerateCloudShaderParam(::VCCloudRenderContext& context)
{
	...
	DRX_ASSERT(pVolFogStage);

	SRenderViewInfo viewInfos[2];
	const size_t viewInfoCount = GetGraphicsPipeline().GenerateViewInfo(viewInfos);

	i32k width  = CRendererResources::s_renderWidth;
	i32k height = CRendererResources::s_renderHeight;
	const bool bStereo = rd->IsStereoEnabled();
	...
	u32k downscaleIndex = max(0, scale - 1);
	...
	DRX_ASSERT(!context.bStereoMultiGPURendering || (context.bStereoMultiGPURendering && (gpuCount == 2)));

	// update constant buffer for cloud rendering and upscaling.
	{
		DrxStackAllocWithSizeVectorCleared(SVolumetricCloudsShaderParam, viewInfoCount, bufferData, CDeviceBufferUpr::AlignBufferSizeForStreaming);
		...
		for (i32 i = 0; i < viewInfoCount; ++i)
		{
			...
			i32k iFrameCount = m_tick % VCFrameCountCycle;
		...
		DRX_ASSERT(m_pRenderCloudConstantBuffer->m_size >= size);
	...
	{
		DrxStackAllocWithSizeVectorCleared(SReprojectionParam, viewInfoCount, bufferData, CDeviceBufferUpr::AlignBufferSizeForStreaming);
		i32k previousFrameIndex = GetPreviousFrameIndex(gpuCount, context.bStereoMultiGPURendering);
		...
		for (i32 i = 0; i < viewInfoCount; ++i)
		{
			...
			i32k eyeIndex = context.bStereoMultiGPURendering ? i : curEye;
		...
		DRX_ASSERT(m_pReprojectionConstantBuffer->m_size >= size);
		m_pReprojectionConstantBuffer->UpdateBuffer(&bufferData[0], size, 0, viewInfoCount);
	}
}

i32 CVolumetricCloudsStage::GetBufferIndex(i32k gpuCount, bool bStereoMultiGPURendering) const
{
	i32 bufferIndex;
	if (bStereoMultiGPURendering)
	{
...
i32 CVolumetricCloudsStage::GetCurrentFrameIndex() const
{
	return m_tick % MaxFrameNum;
}

i32 CVolumetricCloudsStage::GetPreviousFrameIndex(i32k gpuCount, bool bStereoMultiGPURendering) const
{
	i32 frameIndex;
	if (bStereoMultiGPURendering)
	{
...
bool CVolumetricCloudsStage::AreTexturesValid() const
{
	...
	i32k eyeNum = bStereo ? MaxEyeNum : 1;
	for (i32 e = 0; e < eyeNum; ++e)
	{
		for (i32 i = 0; i < 2; ++i)
		{
...
void CVolumetricCloudsStage::GenerateCloudBlockerList()
{
	...
	DRX_ASSERT(numBlocker <= CRenderView::MaxCloudBlockerWorldSpaceNum);
...
void CVolumetricCloudsStage::GenerateCloudBlockerSSList()
{
	...
	auto numBlocker = blockers.size();
	DRX_ASSERT(numBlocker <= CRenderView::MaxCloudBlockerScreenSpaceNum);
