...
void CSvoRenderer::UpdateCompute(CRenderView* pRenderView)
{
	...
	static i32 nTI_Compute_FrameId = -1;
	...
	if (GetIntegratioMode())
	{
		...
		for (i32 n = 0; n < m_arrNodeInfo.Count(); n++)
		{
	...
	i32 nPrevAsync = CRenderer::CV_r_shadersasynccompiling;
	...
	// clear pass
	{
		...
		for (i32 nNodesForUpdateStartIndex = 0; nNodesForUpdateStartIndex < m_arrNodesForUpdateIncr.Count(); )
	...
	static i32 nLightsDynamicCountPrevFrame = 0;
	...
	if ((e_svoTI_DynLights && (m_arrLightsDynamic.Count() || nLightsDynamicCountPrevFrame)) || e_svoTI_SunRSMInject)
	{
		...
		for (i32 nNodesForUpdateStartIndex = 0; nNodesForUpdateStartIndex < m_arrNodesForUpdateNear.Count(); )
...
void CSvoRenderer::ExecuteComputeShader(tukk szTechFinalName, CSvoComputePass& rp, i32* pnNodesForUpdateStartIndex, i32 nObjPassId, PodArray<I3DEngine::SSvoNodeInfo>& arrNodesForUpdate)
{
	#ifdef FEATURE_SVO_GI_ALLOW_HQ
...
CTexture* CSvoRenderer::GetGBuffer(i32 nId) // simplify branch compatibility
{
	CTexture* pRes;
...
void CSvoRenderer::TropospherePass()
{
	...
	tukk szTechFinalName = "RenderAtmosphere";
...
void CSvoRenderer::TraceSunShadowsPass()
{
	...
	tukk szTechFinalName = "TraceSunShadows";
...
void CSvoRenderer::ConeTracePass(SSvoTargetsSet* pTS)
{
	...
	tukk szTechFinalName = "ConeTracePass";
...
void CSvoRenderer::SetupCommonConstants(SSvoTargetsSet* pTS, T& rp, CTexture* pRT)
{
	...
	{
		static CDrxNameR paramName("SVO_ReprojectionMatrix");

		static i32 nReprojFrameId = -1;
	...
	{
		static CDrxNameR paramName("SVO_FrameIdByte");
	...
	{
		static CDrxNameR paramName("SVO_CamPos");
		Vec4 vData(gEnv->pSystem->GetViewCamera().GetPosition(), 0);
		rp.SetConstantArray(paramName, (Vec4*)&vData, 1);
	}

	{
		static CDrxNameR paramName("SVO_ViewProj");
		static CDrxNameR paramNamePrev("SVO_ViewProjPrev");
	...
	if (pRT)
	{
		i32 nTargetSize = pRT->GetWidth() + pRT->GetHeight() + i32(e_svoTI_SkyColorMultiplier > 0) + e_svoTI_Diffuse_Cache + e_svoTI_ShadowsFromSun;
		...
		static CDrxNameR paramName("SVO_TargetResScale");
	...
	{
		static CDrxNameR paramName("SVO_helperInfo");
		rp.SetConstantArray(paramName, (Vec4*)&m_texInfo.helperInfo, 1);
	}

	{
		static CDrxNameR paramName0("SVO_FrustumVerticesCam0");
		static CDrxNameR paramName1("SVO_FrustumVerticesCam1");
		static CDrxNameR paramName2("SVO_FrustumVerticesCam2");
		static CDrxNameR paramName3("SVO_FrustumVerticesCam3");
		Vec3 pvViewFrust[8];
	...
	{
		static CDrxNameR paramName("SVO_AnalyticalOccluders");
	...
	{
		static CDrxNameR paramName("SVO_PostOccluders");
	...
	{
		static CDrxNameR paramName("SVO_CloudShadowParams");
		CD3D9Renderer* const __restrict r = gcpRendD3D;
		SRenderViewShaderConstants& PF = pRenderView->GetShaderConstants();
		Vec4 vData;
	...
	if (pTS)
	{
		static CDrxNameR paramName("SVO_SrcPixSize");
		Vec4 vData(0, 0, 0, 0);
...

	{
		...
		static CDrxNameR paramName("SVO_DepthTargetRes");
...
void CSvoRenderer::DemosaicPass(SSvoTargetsSet* pTS)
{
	...
	tukk szTechFinalName = "DemosaicPass";
...
{
	i32k nLightGroupsNum = 2;

	static CDrxNameR paramNamesLightPos[nLightGroupsNum] =
	{
		CDrxNameR("SVO_LightPos0"),
		CDrxNameR("SVO_LightPos1"),
	};
	static CDrxNameR paramNamesLightDir[nLightGroupsNum] =
	{
		CDrxNameR("SVO_LightDir0"),
		CDrxNameR("SVO_LightDir1"),
	};
	static CDrxNameR paramNamesLightCol[nLightGroupsNum] =
	{
		CDrxNameR("SVO_LightCol0"),
		CDrxNameR("SVO_LightCol1"),
	};

	for (i32 g = 0; g < nLightGroupsNum; g++)
	{
		Vec4 LightPos[4];
		Vec4 LightDir[4];
		Vec4 LightCol[4];
...
void CSvoRenderer::SetupNodesForUpdate(i32& nNodesForUpdateStartIndex, PodArray<I3DEngine::SSvoNodeInfo>& arrNodesForUpdate, CSvoComputePass& rp)
{
	static CDrxNameR paramNames[SVO_MAX_NODE_GROUPS] =
	{
		CDrxNameR("SVO_NodesForUpdate0"),
		CDrxNameR("SVO_NodesForUpdate1"),
		CDrxNameR("SVO_NodesForUpdate2"),
		CDrxNameR("SVO_NodesForUpdate3"),
	};

	for (i32 g = 0; g < SVO_MAX_NODE_GROUPS; g++)
	{
		Vec4 matVal[4];
		...
		for (i32 x = 0; x < 4; x++)
		{
			for (i32 y = 0; y < 4; y++)
			{
				i32 nId = nNodesForUpdateStartIndex + g * 16 + x * 4 + y;
...
template<class T>
void CSvoRenderer::SetupSvoTexturesForRead(I3DEngine::SSvoStaticTexInfo& texInfo, T& rp, i32 nStage, i32 nStageOpa, i32 nStageNorm)
{
...
{
	auto screenResolution = Vec2i(CRendererResources::s_renderWidth, CRendererResources::s_renderHeight);
	i32 nWidth = screenResolution.x;
	i32 nHeight = screenResolution.y;

	i32 nVoxProxyViewportDim = e_svoVoxGenRes;

	i32 nResScaleBase = SATURATEB(e_svoTI_ResScaleBase + e_svoTI_LowSpecMode);
	i32 nDownscaleAir = SATURATEB(e_svoTI_ResScaleAir + e_svoTI_LowSpecMode);

	i32 nInW = (nWidth / nResScaleBase);
	i32 nInH = (nHeight / nResScaleBase);
	i32 nAirW = (nWidth / nDownscaleAir);
	i32 nAirH = (nHeight / nDownscaleAir);
...
inline Vec3 SVO_StringToVector(tukk str)
{
	Vec3 vTemp(0, 0, 0);
	float x, y, z;
...
bool CSvoRenderer::SetShaderParameters(float*& pSrc, u32 paramType, UFloat4* sData)
{
	bool bRes = true;
	...
	case ECGP_PB_SvoParams4:
		{
			...
			float fModeFin = 0;
			i32 nModeGI = CSvoRenderer::GetInstance()->GetIntegratioMode();
...
void CSvoRenderer::DebugDrawStats(const RPProfilerStats* pBasicStats, float& ypos, const float ystep, float xposms)
{
	...
	#define SVO_Draw2dLabel(labelName)                                                                                                             \
	  IRenderAuxText::Draw2dLabel(60, ypos += ystep, 2, &color.r, false, (tukk)(((tukk)( # labelName)) + 10));                       \
...
i32 CSvoRenderer::GetIntegratioMode() const
{
	return e_svoTI_IntegrationMode;
}

i32 CSvoRenderer::GetIntegratioMode(bool& bSpecTracingInUse) const
{
...
void CSvoRenderer::UpscalePass(SSvoTargetsSet* pTS)
{
	...
	tukk szTechFinalName = "UpScalePass";
...
{
	i32k rsmDepthTexSlot = 29;
	i32k rsmColorTexSlot = 30;
	i32k rsmNormlTexSlot = 31;
...
{
	static CDrxNameR lightProjParamName("SVO_RsmSunShadowProj");
	static CDrxNameR rsmSunColparamName("SVO_RsmSunCol");
	static CDrxNameR rsmSunDirparamName("SVO_RsmSunDir");
	Matrix44A shadowMat;
...
void CSvoRenderer::BindTiledLights(PodArray<I3DEngine::SLightTI>& lightsTI, T& rp)
{
	...
	for (i32 l = 0; l < lightsTI.Count(); l++)
	{
		...
		for (u32 lightIdx = 0; lightIdx <= 255 && tiledLightShadeInfo[lightIdx].posRad != Vec4(0, 0, 0, 0); ++lightIdx)
		{
...
void CSvoRenderer::CheckCreateUpdateRT(_smart_ptr<CTexture>& pTex, i32 nWidth, i32 nHeight, ETEX_Format eTF, ETEX_Type eTT, i32 nTexFlags, tukk szName)
{
