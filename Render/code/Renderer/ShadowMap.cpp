...
void CShadowMapStage::Init()
{
	// init per pass resource set template
	{
		...
		m_perPassResources.SetConstantBuffer(eConstantBufferShaderSlot_PerPass, CDeviceBufferUpr::GetNullConstantBuffer(), shaderStages);
		m_perPassResources.SetConstantBuffer(eConstantBufferShaderSlot_PerView, CDeviceBufferUpr::GetNullConstantBuffer(), shaderStages);
...
void CShadowMapStage::ReAllocateResources()
{
	...
	i32k shadowPoolSize = pShadowsPoolSizeCVar ? pShadowsPoolSizeCVar->GetIVal() : 2048;
	...
	if (!CTexture::IsTextureExist(CRendererResources::s_ptexRT_ShadowPool))
	{
		...
		static i32 reallocationCount = 0;
	...
	// allocate shadow maps for dynamic frustums
	{
		...
		if (SDynTexture_Shadow::s_RootShadow.m_NextShadow == &SDynTexture_Shadow::s_RootShadow)
		{
			for (i32 i = 0; i < MAX_GSM_LODS_NUM; ++i)
			{
				...
				char name[64];
				drx_sprintf(name, "$Dyn_%s_2D_%s_%d", "ShadowRT", CTexture::NameForTextureFormat(texFormat), pNewShadow->m_nUniqueID);
...
void CShadowMapStage::Update()
{
...

	{
			      ...
			      frustumType  = CRenderView::eShadowFrustumRenderType(frustumType + 1))
		{
			...
			{
				DRX_ASSERT(pRenderView->GetFrameId() == pFrustumToRender->pShadowsView->GetFrameId());
...
void CShadowMapStage::PrepareShadowPasses(SShadowFrustumToRender& frustumToRender, CRenderView::eShadowFrustumRenderType frustumRenderType)
{
	...
	for (i32 side = 0; side < nSides; side++)
	{
		...
		if (pFrustum->ShouldSampleSide(side))
		{
			...
			if (PrepareOutputsForPass(frustumToRender, side, curPass))
			{
				...
				drx_strcpy(curPass.m_ProfileLabel, profileLabel);
...
void CShadowMapStage::PreparePassIDForFrustum(const SShadowFrustumToRender& frustumToRender, CRenderView::eShadowFrustumRenderType frustumRenderType, EPass& passID, ProfileLabel& profileLabel) const
{
	...
	if (CSvoRenderer::GetRsmColorMap(frustum, true) && CSvoRenderer::GetRsmNormlMap(frustum, true))
	{
		if (frustumRenderType == CRenderView::eShadowFrustumRenderType_SunDynamic)
		{
			...
			drx_sprintf(profileLabel, "SUN FRUSTUM (RSM) %i", frustum.nShadowMapLod);
		}
		else
		{
			passID = ePass_LocalLightRSM;
			drx_sprintf(profileLabel, "LOCAL LIGHT (RSM) %s", frustumToRender.pLight->m_sName);
		}
	}
	else
#endif
		...
		case CRenderView::eShadowFrustumRenderType_SunDynamic:
			{
				...
				tukk szLabel = frustum.m_eFrustumType == ShadowMapFrustum::e_GsmDynamicDistance ? "SUN DISTANCE FRUSTUM %i" : "SUN FRUSTUM %i";
				drx_sprintf(profileLabel, szLabel, frustum.nShadowMapLod);
			}
			break;
		case CRenderView::eShadowFrustumRenderType_SunCached:
			{
				passID = ePass_DirectionalLightCached;
				drx_sprintf(profileLabel, "SUN CACHED %i", frustum.nShadowMapLod);
			}
			break;
		case CRenderView::eShadowFrustumRenderType_HeightmapAO:
			{
				passID = ePass_DirectionalLightCached;
				drx_sprintf(profileLabel, "HEIGHTMAP AO");
			}
			break;
		case CRenderView::eShadowFrustumRenderType_Custom:
			{
				...
				{
					drx_sprintf(profileLabel, "SUN NEAREST");
				}
				else
				{
					drx_sprintf(profileLabel, "SUN PER OBJECT");
				}
			}
			break;
		case CRenderView::eShadowFrustumRenderType_LocalLight:
			{
				...
				drx_sprintf(profileLabel, "LOCAL LIGHT %s", frustumToRender.pLight->m_sName);
			}
			break;
		}
	}
...
bool CShadowMapStage::PrepareOutputsForPass(const SShadowFrustumToRender& frustumToRender, i32 nSide, CShadowMapPass& targetPass) const
{
	const ShadowMapFrustum& frustum = *frustumToRender.pFrustum;
...
{
	DRX_ASSERT(frustum.IsCached() || frustum.m_eFrustumType == ShadowMapFrustum::eFrustumType::e_GsmDynamicDistance);
	...
	if (frustum.IsCached())
	{
		...
		{
			i32 nCachedMapIndex = frustum.nShadowMapLod - (CRendererCVars::CV_r_ShadowsCache - 1);
			DRX_ASSERT(nCachedMapIndex >= 0 && nCachedMapIndex < DRX_ARRAY_COUNT(CRendererResources::s_ptexCachedShadowMap));

			pDepthTarget = CRendererResources::s_ptexCachedShadowMap[clamp_tpl(nCachedMapIndex, 0, i32(DRX_ARRAY_COUNT(CRendererResources::s_ptexCachedShadowMap) - 1))];
		}
		else // e_HeightMapAO
		{
...
void CShadowMapStage::PrepareShadowPassForFrustum(const SShadowFrustumToRender& frustumToRender, i32 nSide, CShadowMapPass& targetPass) const
{
...
void CShadowMapStage::CShadowMapPassGroup::Init(CShadowMapStage* pStage, i32 nSize, CTexture* pDepthTarget, CTexture* pColorTarget0, CTexture* pColorTarget1)
{
	m_Passes.clear();
...
bool CShadowMapStage::CShadowMapPass::PrepareResources(const CRenderView* pMainView)
{
	...
	DRX_ASSERT(!m_perPassResources.HasChangedBindPoints()); // Cannot change resource layout after init. It is baked into the shaders
	m_pPerPassResourceSet->Update(m_perPassResources);
	DRX_ASSERT(m_pPerPassResourceSet->IsValid());
...
void CShadowMapStage::CShadowMapPass::PreRender()
{
	...
	{
		DRX_ASSERT(m_pClearDepthMapProvider);
...
void CShadowMapStage::CopyShadowMap(const CShadowMapPass& sourcePass, CShadowMapPass& targetPass)
{
	...
	DRX_ASSERT(pSrc->m_eFrustumType == ShadowMapFrustum::e_GsmCached);
	DRX_ASSERT(pDst->m_eFrustumType == ShadowMapFrustum::e_GsmDynamicDistance);
	DRX_ASSERT(pSrc->nShadowMapLod == pDst->nShadowMapLod);
	...
	if (bEmptySrcFrustum || !renderItems.empty())
	{
		...
		{
			static CDrxNameTSCRC tech("ReprojectShadowMap");
			...
			static CDrxNameR paramReprojMatDstToSrc("g_mReprojDstToSrc");
			...
			static CDrxNameR paramReprojMatSrcToDst("g_mReprojSrcToDst");
		...
		pDst->shadowPoolPack[0] = { 
			0, 0, 
			static_cast<u32>(pDst->nTextureWidth), static_cast<u32>(pDst->nTextureHeight) 
		};
	}
	else
	{
		...
		Vec4 crop = Vec4(
		  crop.x = fSnap * i32(srcClipPosTL.x / fSnap),
		  crop.y = fSnap * i32(srcClipPosTL.y / fSnap),
		...
		pDst->shadowPoolPack[0].Min = {
			static_cast<u32>((crop.x * 0.5f + 0.5f) * pSrc->pDepthTex->GetWidth() + 0.5f),
			static_cast<u32>((-(crop.y + crop.w) * 0.5f + 0.5f) * pSrc->pDepthTex->GetHeight() + 0.5f)
		};
		pDst->shadowPoolPack[0].Max = pDst->shadowPoolPack[0].Min + Vec2_tpl<u32>{
			static_cast<u32>(pDst->nTextureWidth),
			static_cast<u32>(pDst->nTextureHeight)
		};
	...
	if (shadowMapPasses[ePass_LocalLight].GetCount() > 0 || shadowMapPasses[ePass_LocalLightRSM].GetCount() > 0)
	{
		...
		for (const auto& pass : passes)
		{
			for (const auto& localLightPass : shadowMapPasses[pass])
			{
				...
				DRX_ASSERT(localLightPass.GetPassDesc().GetDepthTarget().pTexture == CRendererResources::s_ptexRT_ShadowPool);
				DRX_ASSERT(localLightPass.m_clearMode == CShadowMapPass::eClearMode_FillRect);
...
void CShadowMapStage::Execute()
{
	...
	i32k nThreadID = gRenDev->GetRenderThreadID();
