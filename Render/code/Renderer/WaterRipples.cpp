...
{
	DRX_ASSERT(m_vertexBuffer == ~0u);
	m_vertexBuffer = gRenDev->m_DevBufMan.Create(BBT_VERTEX_BUFFER, BU_DYNAMIC, sTotalVertexCount * sVertexStride);

	i32k flags = FT_FORCE_MIPS | FT_DONT_STREAM | FT_USAGE_RENDERTARGET;

	DRX_ASSERT(m_pTexWaterRipplesDDN == nullptr);
	...
	DRX_ASSERT(m_pTempTexture == nullptr);
...
{
	DRX_ASSERT(m_pTexWaterRipplesDDN);
...
bool CWaterRipplesStage::RefreshParameters()
{
	...
	u32k gpuCount = gRenDev->GetActiveGPUCount();
...
void CWaterRipplesStage::ExecuteWaterRipples(CTexture* pTargetTex, const D3DViewPort& viewport)
{
...

	{
		...
		DRX_ASSERT(waterRipples.size() <= SWaterRippleInfo::MaxWaterRipplesInScene);
		DrxStackAllocWithSizeVector(SVertex, sTotalVertexCount, ripplesVertices, CDeviceBufferUpr::AlignBufferSizeForStreaming);

		for (i32 i = 0; i < waterRipples.size(); ++i)
		{
...
void CWaterRipplesStage::UpdateAndDrawDebugInfo()
{
	...
	DRX_ASSERT(m_debugRippleInfos.size() <= maxRippleDebugCount);
