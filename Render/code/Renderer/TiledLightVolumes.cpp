...
#include <drx3D/Render/TiledLightVolumes.h>
...
u32k LightTileSizeX = 8;
u32k LightTileSizeY = 8;

u32k AtlasArrayDim = 64;
u32k SpotTexSize = 512;
u32k DiffuseProbeSize = 32;
u32k SpecProbeSize = 256;
u32k ShadowSampleCount = 16;
...
struct HLSL_VolumeLightListGenConstants
{
	...
	u32   lightIndexOffset, numVertices;
	Vec4     screenScale;
	Vec4     viewerPos;
	Vec4     worldBasisX;
	Vec4     worldBasisY;
...
CTiledLightVolumesStage::CTiledLightVolumesStage()
{
	...
	tileLightsCull = reinterpret_cast<STiledLightCullInfo*>(DrxModuleMemalign(sizeof(STiledLightCullInfo) * MaxNumTileLights, CDeviceBufferUpr::GetBufferAlignmentForStreaming()));
	tileLightsShade = reinterpret_cast<STiledLightShadeInfo*>(DrxModuleMemalign(sizeof(STiledLightShadeInfo) * MaxNumTileLights, CDeviceBufferUpr::GetBufferAlignmentForStreaming()));
...
constexpr u8 CTiledLightVolumesStage::AtlasItem::highestMip;
constexpr float CTiledLightVolumesStage::AtlasItem::mipFactorMinSize;

constexpr u16 CTiledLightVolumesStage::STiledLightShadeInfo::resNoIndex;
constexpr u8  CTiledLightVolumesStage::STiledLightShadeInfo::resMipLimit;

void CTiledLightVolumesStage::Init()
{
	// Cubemap Array(s) ==================================================================

#if DRX_RENDERER_OPENGLES || DRX_PLATFORM_ANDROID
	...
	if (!m_specularProbeAtlas.texArray)
	{
		...
		if (m_specularProbeAtlas.texArray->GetFlags() & FT_FAILED)
			DrxFatalError("Couldn't allocate specular probe texture atlas");
	...
	if (!m_diffuseProbeAtlas.texArray)
	{
		...
		if (m_diffuseProbeAtlas.texArray->GetFlags() & FT_FAILED)
			DrxFatalError("Couldn't allocate diffuse probe texture atlas");
	...
	if (!m_spotTexAtlas.texArray)
	{
		...
		if (m_spotTexAtlas.texArray->GetFlags() & FT_FAILED)
			DrxFatalError("Couldn't allocate spot texture atlas");
	...
	// Create geometry for light volumes
	{
		...
		for (u32 i = 0; i < eVolumeType_Count; i++)
		{
			switch (i)
			{
			...
			assert(vertices.size() < DRX_ARRAY_COUNT(vertexData));
			...
			for (u32 i = 0; i < 256; i++)
			{
				for (u32 j = 0; j < volumeMesh.numIndices; j++)
				{
					indices[i * volumeMesh.numIndices + j] = indices[j] + i * volumeMesh.numVertices;
				}
			}

			volumeMesh.indexBuffer  = pRenderer->m_DevBufMan.Create(BBT_INDEX_BUFFER , BU_STATIC, volumeMesh.numIndices  * sizeof(u16) * 256);
			...
			for (u32 i = 0; i < volumeMesh.numVertices; i++)
			{
	...
	for (u32 i = 0; i < DRX_ARRAY_COUNT(m_volumePasses); i++)
	{
...
void CTiledLightVolumesStage::Resize(i32 renderWidth, i32 renderHeight)
{
	// Tiled Light Lists =================================================================

	i32 gridWidth = renderWidth;
	i32 gridHeight = renderHeight;
	
	if (CVrProjectionUpr::IsMultiResEnabledStatic())
		CVrProjectionUpr::Instance()->GetProjectionSize(renderWidth, renderHeight, gridWidth, gridHeight);
	
	u32k dispatchSizeX = gridWidth  / LightTileSizeX + (gridWidth  % LightTileSizeX > 0 ? 1 : 0);
	u32k dispatchSizeY = gridHeight / LightTileSizeY + (gridHeight % LightTileSizeY > 0 ? 1 : 0);
...
void CTiledLightVolumesStage::Clear()
{
	...
	for (u32 i = 0, si = m_specularProbeAtlas.items.size(); i < si; ++i)
		m_specularProbeAtlas.items[i] = AtlasItem();
	for (u32 i = 0, si = m_diffuseProbeAtlas.items.size(); i < si; ++i)
		m_diffuseProbeAtlas.items[i] = AtlasItem();
	for (u32 i = 0, si = m_spotTexAtlas.items.size(); i < si; ++i)
...
void CTiledLightVolumesStage::GenerateLightVolumeInfo()
{
	...
	u32k kNumPasses = eVolumeType_Count * 2;
	u16 volumeLightIndices[kNumPasses][MaxNumTileLights];
	...
	i32 screenWidth = GetViewport().width;
	i32 screenHeight = GetViewport().height;
	...
	u32 numLights = GetValidLightCount();
	for (u32 lightIdx = 0; lightIdx < numLights; ++lightIdx)
	{
	...
	for (u32 i = 0; i < kNumPasses; i++)
	{
		...
		for (u32 j = 0; j < m_numVolumesPerPass[i]; j++)
		{
...
i32 CTiledLightVolumesStage::InsertTexture(CTexture* pTexInput, float mipFactor, TextureAtlas& atlas, i32 arrayIndex)
{
	...
	i32k updateID = pTexInput->GetUpdateFrameID();
	...
	if (arrayIndex < 0)
	{
		...
		for (u32 i = 0, si = atlas.items.size(); i < si; ++i)
		{
	...
	if (arrayIndex < 0)
	{
		u32 minIndex = 0, minValue = 0xFFFFFFFF;
		for (i32 i = 0, si = atlas.items.size(); i < si; ++i)
		{
			u32 accessFrameID = atlas.items[i].accessFrameID;
	...
	item.lowestRenderableMip  = std::min<i32>(item.invalid ? item.highestMip : item.lowestRenderableMip, pTexInput->IsStreamed() ? pTexInput->StreamGetLoadedMip() : 0);
		...
		pTexInput->GetDstFormat() != atlas.texArray->GetDstFormat())
	{
		...
		{
			iLog->LogError("TiledShading: Unsupported texture format: %s (W:%i H:%i F:%s), it has to be equal to the tile-atlas (F:%s), please change the texture's preset by re-exporting with DrxTif",
...
void CTiledLightVolumesStage::UploadTextures(TextureAtlas& atlas)
{
	...
	// Check if texture is already in atlas
	for (u32 arrayIndex = 0, si = atlas.items.size(); arrayIndex < si; ++arrayIndex)
	{
		AtlasItem& item = atlas.items[arrayIndex];
		...
		i32k availableMip = pTexUpload->IsStreamed() ? std::max(0, pTexUpload->StreamGetLoadedMip       (                       )) : 0;
		i32k requestedMip = pTexUpload->IsStreamed() ? std::max(0, pTexUpload->StreamCalculateMipsSigned(item.mipFactorRequested)) : 0;
		...
		if (item.lowestTransferedMip > availableMip)
		{
			...
			// Update atlas
			u32k numMisMips = availableMip;
			u32k numSrcMips = (u32)pTexUpload->GetNumMips();
			u32k numDstMips = (u32)atlas.texArray->GetNumMips();
			u32k numSkpMips = IntegerLog2(std::max((u32)(pTexUpload->GetWidth() >> numMisMips) / atlas.texArray->GetWidth(), 1U));
			u32k numFaces = atlas.texArray->GetTexType() == eTT_CubeArray ? 6 : 1;
...
void CTiledLightVolumesStage::GenerateLightList()
{
	...
	u32k maxSliceCount = CRendererResources::s_ptexShadowMask->StreamGetNumSlices();	// Should be set to same texture as CShadowMaskStage::m_pShadowMaskRT
		...
		for (u32 lightIdx = 0; itr != stp; ++itr, ++lightIdx)
		{
			...
			if (lightListIdx == 0)
			{
...

				{
					...
					i32 arrayIndex = InsertTexture(pSpecTexture, mipFactor, m_specularProbeAtlas, -1);
					if (arrayIndex < 0)
						continue;  // Skip light
			...
			else
			{
				...
				if (renderLight.m_Flags & DLF_PROJECT)
				{
...

					{
						...
						i32 arrayIndex = InsertTexture(pProjTexture, mipFactor, m_spotTexAtlas, -1);
						if (arrayIndex < 0)
							continue;  // Skip light
	...
	const size_t tileLightsCullUploadSize = CDeviceBufferUpr::AlignBufferSizeForStreaming(sizeof(STiledLightCullInfo) * std::min(MaxNumTileLights, Align(numTileLights, 64) + 64));
	const size_t tileLightsShadeUploadSize = CDeviceBufferUpr::AlignBufferSizeForStreaming(sizeof(STiledLightShadeInfo) * std::min(MaxNumTileLights, Align(numTileLights, 64) + 64));
	...
	if (CRenderer::CV_r_DeferredShadingTiledDebug == 1)
	{
		...
		for (u32 i = 0; i < numTileLights; i++)
		{
			...
			i32 lastAccess = pItem ? pItem->accessFrameID : 0;
			i32 lastUpdate = pItem ? pItem->updateFrameID : 0;
			tukk pName = pItem && pItem->texture ? pItem->texture->GetName() : "-";
			i32 streamed0 = 0; // 32x32 non-streamed full-resolution
			i32 streamed1 = pItem && pItem->texture && pItem->texture->IsStreamed() ? std::max(0, pItem->texture->StreamGetLoadedMip()) : 0;
...
void CTiledLightVolumesStage::ExecuteVolumeListGen(u32 dispatchSizeX, u32 dispatchSizeY)
{
	CD3D9Renderer* pRenderer = gcpRendD3D;
...

	{
		...
		static CDrxNameTSCRC techCopy("CopyToDeviceDepth");
	...
	
	{
		...
		u32 curIndex = 0;
		for (u32 pass = 0; pass < eVolumeType_Count * 2; pass++)
		{
			u32 volumeType = pass % eVolumeType_Count;
			...
			if (m_numVolumesPerPass[pass] > 0)
			{
				...
				static CDrxNameTSCRC techLightVolume("VolumeLightListGen");
				...
				u32 numIndices = volumeMesh.numIndices;
				u32 numVertices = volumeMesh.numVertices;
				u32 numInstances = m_numVolumesPerPass[pass];
				...
				{
					auto constants = primitive.GetConstantUpr().BeginTypedConstantUpdate<HLSL_VolumeLightListGenConstants>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex | EShaderStage_Pixel);
...
void CTiledLightVolumesStage::Execute()
{
	...
	i32 screenWidth  = GetViewport().width;
	i32 screenHeight = GetViewport().height;
	i32 gridWidth  = screenWidth;
	i32 gridHeight = screenHeight;

	if (CVrProjectionUpr::IsMultiResEnabledStatic())
		CVrProjectionUpr::Instance()->GetProjectionSize(screenWidth, screenHeight, gridWidth, gridHeight);

	u32 dispatchSizeX = gridWidth  / LightTileSizeX + (gridWidth  % LightTileSizeX > 0 ? 1 : 0);
	u32 dispatchSizeY = gridHeight / LightTileSizeY + (gridHeight % LightTileSizeY > 0 ? 1 : 0);
...
u32 CTiledLightVolumesStage::GetLightShadeIndexBySpecularTextureId(i32 textureId) const
{
