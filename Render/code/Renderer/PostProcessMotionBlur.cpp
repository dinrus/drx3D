...
bool CMotionBlur::GetPrevObjToWorldMat(CRenderObject* pObj, Matrix44A& res)
{
	...
	{
		u32 nThreadID = gRenDev->GetRenderThreadID();
		SRenderObjData* const __restrict pOD = pObj->GetObjData();

		const uintptr_t ObjID = pOD ? pOD->m_uniqueObjectId : 0;
		u32k nFrameID = gRenDev->GetRenderFrameID();
		u32k nObjFrameReadID = (nFrameID - 1) % 3;
...
{
	SMotionBlurDiscardCheck(u32 nDiscardThreshold, u32 nFrameID) : m_nDiscardThreshold(nDiscardThreshold), m_nFrameID(nFrameID){}
...
void CMotionBlur::OnBeginFrame(const SRenderingPassInfo& passInfo)
{
	...
	u32k nDiscardThreshold = 60;
	u32k nFrameID = gRenDev->GetMainFrameID();
	u32k nObjFrameReadID = nFrameID % 3;
...
void CMotionBlur::InsertNewElements()
{
	...
	u32k nFrameID = gRenDev->GetRenderFrameID();
	u32k nObjFrameWriteID = (nFrameID - 1) % 3;
...
void CMotionBlur::FreeData()
{
	...
	for (size_t i = 0; i < DRX_ARRAY_COUNT(m_pOMBData); ++i)
	{
	...
	if (CRenderer::CV_r_MotionBlur && !gRenDev->m_nDisableTemporalEffects)
	{
...

		{
			...
			if (CRenderer::CV_r_MotionBlur && !gRenDev->m_nDisableTemporalEffects)
			{
				...
				static CDrxNameTSCRC techVelocityTileGen("VelocityTileGen");
				static CDrxNameTSCRC techTileNeighborhood("VelocityTileNeighborhood");
	...
	if (bSpriteDofEnabled)
	{
...

		{
			...
			GetUtils().StretchRect(CRendererResources::s_ptexHDRTarget, pSceneHalfRes, false, false, false, true); // 0.25
			{
				...
				u32k nWidth = CRendererResources::s_ptexHDRDofLayers[0]->GetWidth();
				u32k nHeight = CRendererResources::s_ptexHDRDofLayers[0]->GetHeight();
				u32k nBokehQuadsCount = nWidth * nHeight * 3 * 2;
				...
				static CDrxNameTSCRC pszTech("DofBokehRef");
				...
				i32 nRenderState = GS_NODEPTHTEST | GS_BLSRC_ONE | GS_BLDST_ONE;
				...
				static CDrxNameR pAspectRatioName("vAspectRatio");
