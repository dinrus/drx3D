...
#include <drx3D/Render/D3DPostProcess.h>  // CMotionBlur::GetPrevObjToWorldMat
...
#include <drx3D/Render/D3D/GraphicsPipeline/SceneGBuffer.h>
...
DrxCriticalSectionNonRecursive CCompiledRenderObject::m_drawCallInfoLock;
...
CConstantBufferPtr CRenderObjectsPools::AllocatePerDrawConstantBuffer()
{
	if (!m_freeConstantBuffers.empty())
	{
		...
		DRX_ASSERT_MESSAGE(!ptr || (ptr->m_nRefCount == 1), "Someones hold a ref-count, can't reuse ConstantBuffer!");
		DRX_ASSERT_MESSAGE(ptr && !ptr->IsNullBuffer(), "Invalid cached pointer");
		return ptr;
	}
...
void CRenderObjectsPools::FreePerDrawConstantBuffer(CConstantBufferPtr&& buffer)
{
	...
	{
		// Constant buffer can still be temporary used DRX_ASSERT(buffer->m_nRefCount == 1 && "Attempt to free a buffer that is still used elsewhere");
...
CCompiledRenderObject::~CCompiledRenderObject()
{
	...
	{
		DRX_ASSERT_MESSAGE(m_perDrawCB, "CompiledRenderObject tagged as owning a buffer, but no buffer present");
		if (m_perDrawCB)
		{
	...
	for (i32 i = 0; i < eStage_SCENE_NUM; i++)
	{
...
void CCompiledRenderObject::UpdatePerDrawCB(uk pData, size_t size)
{
	if (!m_perDrawCB)
	{
		DRX_ASSERT_MESSAGE(!m_bOwnPerInstanceCB, "CompiledRenderObject tagged as owning a buffer, but no buffer present");
		if (!(m_perDrawCB = s_pPools->AllocatePerDrawConstantBuffer()))
		{
			DRX_ASSERT_MESSAGE(false, "Warning! Running out of memory for ConstantBuffers!");
			return;
		}
		m_bOwnPerInstanceCB = true;
	}

	DRX_ASSERT_MESSAGE(!m_perDrawCB->IsNullBuffer(), "Not allowed to write into the Null resource!");
...
void CCompiledRenderObject::CompilePerDrawCB(CRenderObject* pRenderObject)
{
	...
	{
		DRX_ASSERT(pRenderObject->m_bPermanent);
	...
	// silhouette color
	u32k params = pRenderObject->m_data.m_nHUDSilhouetteParams;
		...
		(nMaskGenFX & 0x400000000ULL && pRenderObject->m_ObjFlags & FOB_BLEND_WITH_TERRAIN_COLOR))
	{
		...
		DrxStackAllocWithSize(HLSL_PerDrawConstantBuffer_TeVe, cb, CDeviceBufferUpr::AlignBufferSizeForStreaming);
	...
	else if ((pRenderObject->m_ObjFlags & FOB_SKINNED) || bHasWrinkleBending)
	{
		...
		// NOTE: Get aligned stack-space (pointer and size aligned to manager's alignment requirement)
		DrxStackAllocWithSizeCleared(HLSL_PerDrawConstantBuffer_Skin, cb, CDeviceBufferUpr::AlignBufferSizeForStreaming);
	...
	{
		// NOTE: Get aligned stack-space (pointer and size aligned to manager's alignment requirement)
		DrxStackAllocWithSize(HLSL_PerDrawConstantBuffer_Base, cb, CDeviceBufferUpr::AlignBufferSizeForStreaming);
		...
		u32 ambColorPacked =
			((static_cast<u32>(ambColor.r * 255.0f) & 0xFF) << 24)
			| ((static_cast<u32>(ambColor.g * 255.0f) & 0xFF) << 16)
			| ((static_cast<u32>(ambColor.b * 255.0f) & 0xFF) << 8)
			| (static_cast<u32>(ambColor.a * 255.0f) & 0xFF);
		cb->CD_CustomData2.y = alias_cast<float>(ambColorPacked);

		UpdatePerDrawCB(cb, sizeof(HLSL_PerDrawConstantBuffer_Base));
	}

	DrxInterlockedExchangeOr(( LONG*)&m_compiledFlags, eObjCompilationOption_PerInstanceConstantBuffer);
}
...
void CCompiledRenderObject::CompilePerInstanceCB(CRenderObject* pRenderObject, bool bForce)
{
	...
	DrxStackAllocWithSizeVector(SPerInstanceShaderData, m_perDrawInstances, tempInstanceBuffer, CDeviceBufferUpr::AlignBufferSizeForStreaming); //May crash without m_perDrawInstances+1
	if (nSrcInsts == 0)
	{
	...
	assert(sizeof(SPerInstanceShaderData) == Align(sizeof(SPerInstanceShaderData), DRX_PLATFORM_ALIGNMENT));
...
void CCompiledRenderObject::CompilePerDrawExtraResources(CRenderObject* pRenderObject)
{
	...
	if (!m_bHasTessellation && !pRenderObject->m_data.m_pSkinningData) // only needed for skinning and tessellation at the moment
	{
		...
		DrxInterlockedExchangeOr(( LONG*)&m_compiledFlags, eObjCompilationOption_PerInstanceExtraResources);
		return;
	}
...
{
	AUTO_LOCK_T(DrxCriticalSectionNonRecursive, m_drawCallInfoLock);
...
bool CCompiledRenderObject::Compile(const EObjectCompilationOptions& compilationOptions, const AABB &localAABB, CRenderView *pRenderView)
{
	...
	//{ char buf[1024]; drx_sprintf(buf,"compiled: %p : frame(%d) \r\n", pRenderObject, nFrameId); OutputDebugString(buf); }
	...
	if (compilationOptions & eObjCompilationOption_InputStreams)
	{
		...
		DrxInterlockedExchangeOr(( LONG*)&m_compiledFlags, eObjCompilationOption_InputStreams);
	}
	...
	// Stencil ref value
	u8 stencilRef = 0; // @TODO: get from CRNTmpData::SRNUserData::m_pClipVolume::GetStencilRef
	...
	if (compilationOptions & eObjCompilationOption_PipelineState)
	{
		...
		DrxInterlockedExchangeOr(( LONG*)&m_compiledFlags, eObjCompilationOption_PipelineState);
	}
...
void CCompiledRenderObject::DrawToCommandList(const SGraphicsPipelinePassContext& RESTRICT_REFERENCE passContext, 
                                              ...
                                              u32 dynamicInstancingCount) const
{
	...
	{
		DrxInterlockedIncrement(&SRenderStatistics::Write().m_nIncompleteCompiledObjects);
	}
#endif
...

	{
		...
		if (m_vertexStreamSet)
		{
			...
			{
				DrxInterlockedIncrement(&SRenderStatistics::Write().m_NumRendSkinnedObjects);
			}
		}
...
CPermanentRenderObject::~CPermanentRenderObject()
{
	...
	for (i32 i = 0, num = m_permanentRenderItems[eRenderPass_Shadows].size(); i < num; i++)
	{
	...
	for (i32 i = 0, num = m_permanentRenderItems[eRenderPass_General].size(); i < num; i++)
	{
