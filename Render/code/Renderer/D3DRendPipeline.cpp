...
#include <drx3D/Render/BenchmarkCustom/BenchmarkRendererSensor.h>
#endif
...
#include <drx3D/Render/D3D/GraphicsPipeline/Common/GraphicsPipelineStage.h>
#include <drx3D/Render/D3D/GraphicsPipeline/Common/SceneRenderPass.h>
...
#include <drx3D/Render/D3D/GraphicsPipeline/PostAA.h>
#include <drx3D/Render/D3D/GraphicsPipeline/DepthReadback.h>
...
bool CD3D9Renderer::FX_HDRScene(CRenderView *pRenderView, bool bClear)
{
	if (pRenderView->AllowsHDRRendering())
	{
		...
		i32k shaderRenderingFlags = pRenderView->GetShaderRenderingFlags();
...
i32 CD3D9Renderer::EF_Preprocess(SRendItem* ri, u32 nums, u32 nume, const SRenderingPassInfo& passInfo) PREFAST_SUPPRESS_WARNING(6262)
{
	u32 i, j;
	CShader* Shader;
	...
	for (i = nums; i < nume; i++)
	{
		...
		if (nTech < 0)
			nTech = 0;
		if (nTech < (i32)Shader->m_HWTechniques.Num())
		{
...
void CD3D9Renderer::RT_UpdateSkinningConstantBuffers(CRenderView* pRenderView)
{
	...
	unsigned poolId = pRenderView->GetSkinningDataPools().poolIndex % DRX_ARRAY_COUNT(m_SkinningDataPool);
	for (util::list<SCharacterInstanceCB>* iter = m_CharCBActiveList[poolId].next; iter != &m_CharCBActiveList[poolId]; iter = iter->next)
	{
		...
		size_t boneQuatsSSize   = Align(pSkinningData->nNumBones        * sizeof(DualQuat                       ), DRX_PLATFORM_ALIGNMENT);
		size_t activeMorphsSize = Align(pSkinningData->nNumActiveMorphs * sizeof(compute_skinning::SActiveMorphs), DRX_PLATFORM_ALIGNMENT);
	...
	{
		i32 committed      = DrxInterlockedCompareExchange((LONG*)&m_CharCBAllocated, 0, 0);
		i32 totalRequested = m_CharCBFrameRequired[poolId] + m_CharCBFrameRequired[(poolId - 1) % 3];
		...
		if (totalRequested * 4 > committed * 3 && m_CharCBFreeList.empty() == false)
		{
			...
			DrxInterlockedDecrement(&m_CharCBAllocated);
		}
	}
}

uk CD3D9Renderer::FX_AllocateCharInstCB(SSkinningData* pSkinningData, u32 frameId)
{
	...
	if (cb == NULL)
	{
		...
		DrxInterlockedIncrement(&m_CharCBAllocated);
	}
	cb->updated = false;
	...
	DrxInterlockedIncrement(&m_CharCBFrameRequired[frameId % 3]);
	return cb;
}

void CD3D9Renderer::FX_ClearCharInstCB(u32 frameId)
{
	PROFILE_FRAME(FX_ClearCharInstCB);
	u32 poolId = frameId % 3;
	WriteLock _lock(m_lockCharCB);
...
{
	u32k shaderRenderingFlags = pRenderView->GetShaderRenderingFlags();
	...
	static i32 lightVolumeOldFrameID = -1;
	i32k newFrameID = pRenderView->GetFrameId();
...
void CD3D9Renderer::RT_RenderScene(CRenderView* pRenderView)
{
	...
	u32 shaderRenderingFlags = pRenderView->GetShaderRenderingFlags();
	...
	if (!bRecurse)
	{
		...
		CVrProjectionUpr::Instance()->Configure(pRenderView->GetViewport(), bRightEye);
	}

	i32 nSaveDrawNear     = CV_r_nodrawnear;
	i32 nSaveDrawCaustics = CV_r_watercaustics;
	...
	{
		DRX_ASSERT(shaderRenderingFlags & SHDF_ALLOWHDR);
...
void CD3D9Renderer::SubmitRenderViewForRendering(i32 nFlags, const SRenderingPassInfo& passInfo)
{
	...
	if (passInfo.IsGeneralPass())
	{
		if ((nFlags & SHDF_ALLOWPOSTPROCESS))
		{
			...
			{
				i32k size = static_cast<i32>(postProcessRenderItems.size());
...
void CD3D9Renderer::EF_EndEf3D(i32k nFlags, i32k nPrecacheUpdateIdSlow, i32k nPrecacheUpdateIdFast, const SRenderingPassInfo& passInfo)
{
	ASSERT_IS_MAIN_THREAD(m_pRT)
	...
	i32 nAsyncShaders = CV_r_shadersasynccompiling;
...
void CD3D9Renderer::EF_InvokeShadowMapRenderJobs(const SRenderingPassInfo& passInfo, i32k nFlags)
{
	CRenderView* pRenderView = passInfo.GetRenderView();
	if (!pRenderView->IsRecursive())
	{
...
//////////////////////////////////////////////////////////////////////////
void CD3D9Renderer::RenderFrame(i32 nSceneRenderingFlags, const SRenderingPassInfo& passInfo)
{
	ASSERT_IS_MAIN_THREAD(m_pRT)
...

	{
		...
		{
			char nm[256];
			drx_strcpy(nm, CV_r_excludeshader->GetString());
			strlwr(nm);
...
bool CD3D9Renderer::StoreGBufferToAtlas(const RectI& rcDst, i32 nSrcWidth, i32 nSrcHeight, i32 nDstWidth, i32 nDstHeight, ITexture *pDataD, ITexture *pDataN)
{
	bool bRes = true;
...
///////////////////////////////////////////////////////////////////////////////
void CD3D9Renderer::WaitForParticleBuffer(i32 frameId)
{
