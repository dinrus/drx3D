...
#include <drx3D/Render/SDrxDX11PipelineState.hpp>

#include <drx3D/Render/DX12/API/DX12Shader.hpp>
#include <drx3D/Render/DX12/API/DX12RootSignature.hpp>
#include <drx3D/Render/DX12/Device/CDrxDX12DeviceContext.hpp>


const D3D12_SHADER_BYTECODE& SDrxDX11ShaderStageState::GetD3D12ShaderBytecode() const
{
...
//---------------------------------------------------------------------------------------------------------------------
static tukk TopologyToString(D3D11_PRIMITIVE_TOPOLOGY type)
{
	switch (type)
	{
...
static tukk TypeToString(UINT8 dim)
{
	switch (dim)
	{
...
static tukk DimensionToString(UINT8 dim)
{
	switch (dim)
	{
...
static tukk DimensionToString(const D3D12_SHADER_RESOURCE_VIEW_DESC& desc)
{
...
static tukk DimensionToString(const D3D12_UNORDERED_ACCESS_VIEW_DESC& desc)
{
	switch (desc.ViewDimension)
	{
	case D3D_SRV_DIMENSION_UNKNOWN:
...
//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11ShaderStageState::Init(SDrxDX11PipelineState* pParent)
{
...
void SDrxDX11ShaderStageState::DebugPrint()
{
	...
	const NDrxDX12::SResourceRanges& rr = Shader.Get()->GetDX12Shader()->GetResourceRanges();
	...
	if (rr.m_ConstantBuffers.m_TotalLength)
	{
		const NDrxDX12::SBindRanges::TRanges& ranges = rr.m_ConstantBuffers.m_Ranges;
	...
	{
		const NDrxDX12::SBindRanges::TRanges& ranges = rr.m_InputResources.m_Ranges;
		for (size_t ri = 0, S = ranges.size(); ri < S; ++ri)
		{
	...
	{
		const NDrxDX12::SBindRanges::TRanges& ranges = rr.m_OutputResources.m_Ranges;
		for (size_t ri = 0, S = ranges.size(); ri < S; ++ri)
		{
	...
	{
		const NDrxDX12::SBindRanges::TRanges& ranges = rr.m_Samplers.m_Ranges;
		for (size_t ri = 0, S = ranges.size(); ri < S; ++ri)
		{
...
//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11IAState::Init(SDrxDX11PipelineState* pParent)
{
...
//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11IAState::DebugPrint()
{
...
//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11RasterizerState::Init(SDrxDX11PipelineState* pParent)
{
...
//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11RasterizerState::DebugPrint()
{
...
//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11OutputMergerState::Init(SDrxDX11PipelineState* pParent)
{
...
//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11OutputMergerState::DebugPrint()
{
...
void SDrxDX11PipelineState::InitD3D12Descriptor(NDrxDX12::CComputePSO::SInitParams& params, UINT nodeMask)
{
	...
	desc.CS = Stages[NDrxDX12::ESS_Compute].GetD3D12ShaderBytecode();

	params.m_CS = Stages[NDrxDX12::ESS_Compute].Shader ? Stages[NDrxDX12::ESS_Compute].Shader->GetDX12Shader() : NULL;
}

//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11PipelineState::InitD3D12Descriptor(NDrxDX12::CGraphicsPSO::SInitParams& params, UINT nodeMask)
{
	auto& desc = params.m_Desc;
	ZeroMemory(&desc, sizeof(params.m_Desc));
	...
	desc.VS = Stages[NDrxDX12::ESS_Vertex].GetD3D12ShaderBytecode();
	desc.PS = Stages[NDrxDX12::ESS_Pixel].GetD3D12ShaderBytecode();
	desc.HS = Stages[NDrxDX12::ESS_Hull].GetD3D12ShaderBytecode();
	desc.DS = Stages[NDrxDX12::ESS_Domain].GetD3D12ShaderBytecode();
	desc.GS = Stages[NDrxDX12::ESS_Geometry].GetD3D12ShaderBytecode();

	params.m_VS = Stages[NDrxDX12::ESS_Vertex].Shader ? Stages[NDrxDX12::ESS_Vertex].Shader->GetDX12Shader() : NULL;
	params.m_HS = Stages[NDrxDX12::ESS_Hull].Shader ? Stages[NDrxDX12::ESS_Hull].Shader->GetDX12Shader() : NULL;
	params.m_DS = Stages[NDrxDX12::ESS_Domain].Shader ? Stages[NDrxDX12::ESS_Domain].Shader->GetDX12Shader() : NULL;
	params.m_GS = Stages[NDrxDX12::ESS_Geometry].Shader ? Stages[NDrxDX12::ESS_Geometry].Shader->GetDX12Shader() : NULL;
	params.m_PS = Stages[NDrxDX12::ESS_Pixel].Shader ? Stages[NDrxDX12::ESS_Pixel].Shader->GetDX12Shader() : NULL;
	...
	{
		const CDrxDX12InputLayout::TDescriptors& descriptors = InputAssembler.InputLayout->GetDescriptors();
...
//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11PipelineState::InitRootSignatureInitParams(NDrxDX12::CRootSignature::SInitParams& params)
{
	params.m_VS = Stages[NDrxDX12::ESS_Vertex].Shader ? Stages[NDrxDX12::ESS_Vertex].Shader->GetDX12Shader() : NULL;
	params.m_HS = Stages[NDrxDX12::ESS_Hull].Shader ? Stages[NDrxDX12::ESS_Hull].Shader->GetDX12Shader() : NULL;
	params.m_DS = Stages[NDrxDX12::ESS_Domain].Shader ? Stages[NDrxDX12::ESS_Domain].Shader->GetDX12Shader() : NULL;
...
//---------------------------------------------------------------------------------------------------------------------
void SDrxDX11PipelineState::DebugPrint()
{
	if (!g_nPrintDX12)
		return;
