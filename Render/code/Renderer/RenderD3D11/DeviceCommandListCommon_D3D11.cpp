...
#include <drx3D/Render/DeviceCommandListCommon_D3D11.h>
...
CDeviceTimestampGroup::~CDeviceTimestampGroup()
{
	...
	for (u32 i = 0; i < m_timestampQueries.size(); i++)
	{
...
{
	u8k InvalidShaderPointer = (u8)(uint64)CDeviceResourceSet_DX11::InvalidPointer;
	DRX_ASSERT((InvalidShaderPointer & 0x1) != 0);
...
static inline void BindGraphicsSRV(EHWShaderClass shaderClass, ID3D11ShaderResourceView* pSrv, u32 slot)
{
	CD3D9Renderer* const __restrict rd = gcpRendD3D;
	rd->m_DevMan.BindSRV(static_cast<CSubmissionQueue_DX11::SHADER_TYPE>(shaderClass), pSrv, slot);
}

static inline void BindGraphicsSampler(EHWShaderClass shaderClass, ID3D11SamplerState* pSamplerState, u32 slot)
{
	CD3D9Renderer* const __restrict rd = gcpRendD3D;
	rd->m_DevMan.BindSampler(static_cast<CSubmissionQueue_DX11::SHADER_TYPE>(shaderClass), pSamplerState, slot);
}

static inline void BindGraphicsConstantBuffer(EHWShaderClass shaderClass, D3DBuffer* pBuffer, u32 slot, u32 offset, u32 size)
{
	CD3D9Renderer* const __restrict rd = gcpRendD3D;
...
void CDeviceGraphicsCommandInterfaceImpl::SetPipelineStateImpl(const CDeviceGraphicsPSO* devicePSO)
{
	...
	const std::array<uk , eHWSC_Num>& shaders = pDevicePSO->m_pDeviceShaders;
...
void CDeviceGraphicsCommandInterfaceImpl::SetResources_RequestedByShaderOnly(const CDeviceResourceSet* pResources)
{
	...
	i32 validShaderStages = m_sharedState.validShaderStages;
	for (EHWShaderClass shaderClass = eHWSC_Vertex; validShaderStages; shaderClass = EHWShaderClass(shaderClass + 1), validShaderStages >>= 1)
	{
		if (validShaderStages & 1)
		{
			...
			{
				for (i32 i = 0; i < m_sharedState.numSRVs[shaderClass]; ++i)
				{
					u8 srvSlot = m_sharedState.srvs[shaderClass][i];
			...
			{
				for (i32 i = 0; i < m_sharedState.numSamplers[shaderClass]; ++i)
				{
					u8 samplerSlot = m_sharedState.samplers[shaderClass][i];
			...
			// Bind buffers
			for (i32 i = 0; i < pResourcesDX11->numCompiledBufferSRVs[shaderClass]; ++i)
			{
			...
			for (i32 i = 0; i < pResourcesDX11->numCompiledCBs[shaderClass]; ++i)
			{
			...
			// Bind UAVs
			for (i32 i = 0; i < pResourcesDX11->numCompiledUAVs; ++i)
			{
...
void CDeviceGraphicsCommandInterfaceImpl::SetResources_All(const CDeviceResourceSet* pResources)
{
	...
	{
		// Bind SRVs
		for (i32 slot = 0; slot < pResourcesDX11->compiledTextureSRVs[shaderClass].size(); ++slot)
		{
		...
		// Bind Samplers
		for (i32 slot = 0; slot < pResourcesDX11->compiledSamplers[shaderClass].size(); ++slot)
		{
...
void CDeviceGraphicsCommandInterfaceImpl::ApplyRasterizerState()
{
	if (m_graphicsState.custom.bRasterizerStateDirty)
	{
		...
		{
			auto rsDesc = CDeviceStatesUprDX11::GetInstance()->m_StatesRS[m_graphicsState.custom.rasterizerStateIndex].Desc;
			rsDesc.DepthBias = i32(m_graphicsState.custom.depthConstBias);
			...
			u32 newRasterizerStateIndex = CDeviceStatesUprDX11::GetInstance()->GetOrCreateRasterState(rsDesc);
			if (newRasterizerStateIndex != u32(-1))
			{
				pRasterizerState = CDeviceStatesUprDX11::GetInstance()->m_StatesRS[newRasterizerStateIndex].pState;
			}
		}
...
void CDeviceGraphicsCommandInterfaceImpl::SetInlineConstantBufferImpl(u32 bindSlot, const CConstantBuffer* pBuffer, EConstantBufferShaderSlot shaderSlot, EHWShaderClass shaderClass)
{
	DRX_ASSERT(bindSlot <= EResourceLayoutSlot_Max);
...
#else
	DRX_ASSERT(numRects == 0); // not supported on dx11
...
void CDeviceComputeCommandInterfaceImpl::SetPipelineStateImpl(const CDeviceComputePSO* pDevicePSO)
{
	...
	// Shader
	const std::array<uk , eHWSC_Num>& shaders = pDevicePsoDX11->m_pDeviceShaders;
...
void CDeviceComputeCommandInterfaceImpl::SetResourcesImpl(u32 bindSlot, const CDeviceResourceSet* pResources)
{
	...
	// Bind SRVs
	for (i32 slot = 0; slot < pResourcesDX11->compiledTextureSRVs[eHWSC_Compute].size(); ++slot)
	{
	...
	// Bind Samplers
	for (i32 slot = 0; slot < pResourcesDX11->compiledSamplers[eHWSC_Compute].size(); ++slot)
	{
	...
	// Bind buffers
	for (i32 i = 0; i < pResourcesDX11->numCompiledBufferSRVs[eHWSC_Compute]; ++i)
	{
	...
	// Bind constant buffers
	for (i32 i = 0; i < pResourcesDX11->numCompiledCBs[eHWSC_Compute]; ++i)
	{
...
void CDeviceComputeCommandInterfaceImpl::SetInlineConstantBufferImpl(u32 bindSlot, const CConstantBuffer* pBuffer, EConstantBufferShaderSlot shaderSlot)
{
	DRX_ASSERT(bindSlot <= EResourceLayoutSlot_Max);
...
void CDeviceComputeCommandInterfaceImpl::DispatchImpl(u32 X, u32 Y, u32 Z)
{
	...
	{
		for (i32 i = 0; i < m_computeState.custom.boundUAVs.size(); ++i)
		{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CDeviceNvidiaCommandInterfaceImpl::SetModifiedWModeImpl(bool enabled, u32 numViewports, const float* pA, const float* pB)
{
	CD3D9Renderer* const __restrict rd = gcpRendD3D;
...
void CDeviceCopyCommandInterfaceImpl::CopyImpl(CDeviceTexture* pSrc, CDeviceTexture* pDst, const SResourceRegionMapping& region)
{
	...
	if (pSrc->IsResolvable())
	{
		...
		DRX_ASSERT(region.DestinationOffset.Left == 0);
		DRX_ASSERT(region.DestinationOffset.Top == 0);
		DRX_ASSERT(region.DestinationOffset.Front == 0);
	}
...
{
	CD3D9Renderer* const __restrict rd = gcpRendD3D;
	DRX_ASSERT((region.Extent.Subresources == 1) && ((region.ResourceOffset.Left == 0) || (DRX_RENDERER_DIRECT3D >= 111))); // TODO: batch
...
#if (DRX_RENDERER_DIRECT3D >= 111)
	rd->GetDeviceContext().UpdateSubresource1(pDst->GetD3D(), region.ResourceOffset.Subresource, DRX_RENDERER_DIRECT3D >= 111 ? &box : nullptr, pSrc, region.MemoryLayout.rowStride, region.MemoryLayout.planeStride, region.Flags);
#else
	rd->GetDeviceContext().UpdateSubresource(pDst->GetD3D(), region.ResourceOffset.Subresource, DRX_RENDERER_DIRECT3D >= 111 ? &box : nullptr, pSrc, region.MemoryLayout.rowStride, region.MemoryLayout.planeStride);
#endif
}
...
{
	CD3D9Renderer* const __restrict rd = gcpRendD3D;
	DRX_ASSERT((region.Extent.Subresources == 1) && (DRX_RENDERER_DIRECT3D < 111)); // TODO: batch
	D3D11_BOX box = { region.ResourceOffset.Left, region.ResourceOffset.Top, region.ResourceOffset.Front, region.ResourceOffset.Left + region.Extent.Width, region.ResourceOffset.Top + region.Extent.Height, region.ResourceOffset.Front + region.Extent.Depth };
#if (DRX_RENDERER_DIRECT3D >= 111)
...
{
	CD3D9Renderer* const __restrict rd = gcpRendD3D;
	DRX_ASSERT(region.Extent.Subresources == 1); // TODO: batch
	D3D11_BOX box = { region.ResourceOffset.Left, region.ResourceOffset.Top, region.ResourceOffset.Front, region.ResourceOffset.Left + region.Extent.Width, region.ResourceOffset.Top + region.Extent.Height, region.ResourceOffset.Front + region.Extent.Depth };
#if (DRX_RENDERER_DIRECT3D >= 111)
