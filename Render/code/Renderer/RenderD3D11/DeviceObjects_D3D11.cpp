...
#include <drx3D/Render/DeviceCommandList_D3D11.h>
#include <drx3D/Render/DeviceRenderPass_D3D11.h>

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#define DX11_COMMANDLIST_REDUNDANT_STATE_FILTERING

void CDeviceStatesUprDX11::ShutDown()
{
...
CDeviceStatesUprDX11* CDeviceStatesUprDX11::GetInstance()
{
	static CDeviceStatesUprDX11 sStatesUpr;
	return &sStatesUpr;
}
...
void CDeviceStatesUprDX11::ReleaseUnusedStates(u32 currentFrameID) {
...
u32 CDeviceStatesUprDX11::GetOrCreateBlendState(const D3D11_BLEND_DESC& desc)
{
	u32 i;
	HRESULT hr = S_OK;
	uint64 nHash = SStateBlend::GetHash(desc);
	for (i = 0; i < (u32)m_StatesBL.size(); i++)
	{
	...
	return SUCCEEDED(hr) ? i : u32(-1);
}

u32 CDeviceStatesUprDX11::GetOrCreateRasterState(const D3D11_RASTERIZER_DESC& rasterizerDec, const bool bAllowMSAA)
{
	u32 i;
	HRESULT hr = S_OK;
	...
	u32 nHash = SStateRaster::GetHash(desc);
	uint64 nValuesHash = SStateRaster::GetValuesHash(desc);
	for (i = 0; i < (u32)m_StatesRS.size(); i++)
	{
...
u32 CDeviceStatesUprDX11::GetOrCreateDepthState(const D3D11_DEPTH_STENCIL_DESC& desc)
{
	u32 i;
	HRESULT hr = S_OK;
	uint64 nHash = SStateDepth::GetHash(desc);
	i32k kNumStates = (i32)m_StatesDP.size();
...
{
	DRX_ASSERT(!m_pDX11Device);
	DRX_ASSERT(!m_pDX11Scheduler);
	...
	m_pDX11Device = NDrxDX11::CDevice::Create(pDevice, D3D_FEATURE_LEVEL_11_0);
...
{
	CDeviceStatesUprDX11::GetInstance()->ShutDown();
}
...
#if !DRX_PLATFORM_ORBIS && !(DRX_PLATFORM_DURANGO && BUFFER_ENABLE_DIRECT_ACCESS)
...
HRESULT CDeviceObjectFactory::SyncFence(DeviceFenceHandle query, bool block, bool flush)
{
	...
	if (d3d_query)
	{
		DRX_ASSERT(!flush || !gRenDev->m_pRT || gRenDev->m_pRT->IsRenderThread()); // Can only flush from render thread

		BOOL bQuery = false;
		do
		{
			hr = gcpRendD3D->GetDeviceContext().GetData(d3d_query, (uk )&bQuery, sizeof(BOOL), flush ? 0 : D3D11_ASYNC_GETDATA_DONOTFLUSH);
		} while (block && hr != S_OK);
	}
	return hr;
}
...
CDeviceSamplerState* CDeviceObjectFactory::CreateSamplerState(const SSamplerState& pState)
{
	...
	ColorF col((u32)pState.m_dwBorderColor);
...
CDeviceInputLayout* CDeviceObjectFactory::CreateInputLayout(const SInputLayout& pLayout, const SShaderBlob* m_pConsumingVertexShader)
{
	...
	i32k   nSize = m_pConsumingVertexShader->m_nDataSize;
	ukk pVSData = m_pConsumingVertexShader->m_pShaderData;
...
void CDeviceObjectFactory::ReleaseStagingResource(D3DResource* pStagingRes)
{
	...
	GetDX11Device()->ReleaseLater(GetDX11Scheduler()->GetFenceUpr().GetCurrentValues(), pStagingRes, true);
	pStagingRes->Release();
}
#endif
...
{
	// NOTE: Poor man's resource tracking (take current time as last-used moment)
	GetDX11Device()->ReleaseLater(GetDX11Scheduler()->GetFenceUpr().GetCurrentValues(), pResource, false);
	pResource->Release();
}
...
{
	// NOTE: Poor man's resource tracking (take current time as last-used moment)
	GetDX11Device()->ReleaseLater(GetDX11Scheduler()->GetFenceUpr().GetCurrentValues(), pResource, true);
	pResource->Release();
}

//=============================================================================

HRESULT CDeviceObjectFactory::Create2DTexture(u32 nWidth, u32 nHeight, u32 nMips, u32 nArraySize, u32 nUsage, const ColorF& cClearValue, D3DFormat Format, LPDEVICETEXTURE* ppDevTexture, const STexturePayload* pPayload, i32 nESRAMOffset)
{
	HRESULT hr = S_OK;
...
HRESULT CDeviceObjectFactory::CreateCubeTexture(u32 nSize, u32 nMips, u32 nArraySize, u32 nUsage, const ColorF& cClearValue, D3DFormat Format, LPDEVICETEXTURE* ppDevTexture, const STexturePayload* pPayload)
{
	HRESULT hr = S_OK;
...
HRESULT CDeviceObjectFactory::Create2DTexture(const D3D11_TEXTURE2D_DESC& Desc, u32 eFlags, const ColorF& cClearValue, LPDEVICETEXTURE* ppDevTexture, const STexturePayload* pPayload)
{
...
HRESULT CDeviceObjectFactory::CreateCubeTexture(const D3D11_TEXTURE2D_DESC& Desc, u32 eFlags, const ColorF& cClearValue, LPDEVICETEXTURE* ppDevTexture, const STexturePayload* pPayload)
{
	D3D11_SUBRESOURCE_DATA SRD[g_nD3D10MaxSupportedSubres];
...
HRESULT CDeviceObjectFactory::CreateVolumeTexture(const D3D11_TEXTURE3D_DESC& Desc, u32 eFlags, const ColorF& cClearValue, LPDEVICETEXTURE* ppDevTexture, const STexturePayload* pPayload)
{
	D3D11_SUBRESOURCE_DATA SRD[g_nD3D10MaxSupportedSubres];
	...
	, ukk pData)
{
	...
	if ((nUsage & USAGE_CPU_WRITE))
		Desc.ByteWidth = CDeviceBufferUpr::AlignElementCountForStreaming(nSize, elemSize) * elemSize;

	Desc.BindFlags = ConvertToDX11BindFlags(nBindFlags);
	if (nBindFlags & (BIND_STREAM_OUTPUT | BIND_RENDER_TARGET | BIND_DEPTH_STENCIL))
	{
		DrxFatalError("trying to create (currently) unsupported buffer type");
	}

#if DRX_PLATFORM_DURANGO && BUFFER_USE_STAGED_UPDATES == 0
	if (nUsage & USAGE_DIRECT_ACCESS)
	{
	//	if (nUsage & HEAP_STAGING)
	//	{
	//		DrxFatalError("staging buffers not supported if BUFFER_USE_STAGED_UPDATES not defined");
	//	}
		...
		{
			DrxFatalError("Memory Allocation Size for Direct Video Memory Access must be a multiple of 4 KB but the supplied size is %u", nSize * elemSize);
		}
...
u8* CDeviceObjectFactory::Map(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode)
{
	...
	HRESULT hr = DXGLMapBufferRange(
		...
		, 0
		, &mapped_resource);
	mapped_resource.pData = reinterpret_cast<u8*>(mapped_resource.pData) - offset;
#else
	...
	HRESULT hr = gcpRendD3D->GetDeviceContext_ForMapAndUnmap().Map(
		...
		, &mapped_resource);
#endif
	return reinterpret_cast<u8*>(mapped_resource.pData);
}
...
void CDeviceObjectFactory::UploadContents(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode, ukk pInDataCPU, uk pOutDataGPU, UINT numDataBlocks)
{
	if (!bDirectAccess)
	{
		...
		u8* pOutData = CDeviceObjectFactory::Map(buffer, subresource, offset, size, mode) + offset;
...
void CDeviceObjectFactory::DownloadContents(D3DBuffer* buffer, u32 subresource, buffer_size_t offset, buffer_size_t size, D3D11_MAP mode, uk pOutDataCPU, ukk pInDataGPU, UINT numDataBlocks)
{
	if (!bDirectAccess)
	{
		u8* pOutData = reinterpret_cast<u8*>(pOutDataCPU);
		u8k* pInData = CDeviceObjectFactory::Map(buffer, subresource, offset, size, mode) + offset;
