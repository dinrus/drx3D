...
auto GetDebugName = [](ID3D11Resource* pO) -> tukk { UINT len = 511; static char name[512] = "unknown"; pO->GetPrivateData(WKPDID_D3DDebugObjectName, &len, name); name[len] = '\0'; return name; };
#else
auto GetDebugName = [](ID3D11Resource* pO) -> tukk { return "unknown"; };
#endif
	...
	_Inout_ ID3D11Resource** ppSubstituteResource) threadsafe
{
	...
	if (fenceUpr.IsCompleted(fenceValues[CMDQUEUE_IMMEDIATE], CMDQUEUE_IMMEDIATE))
	{
	...
	_COM_Outptr_optvoid** ppvResource) threadsafe
{
	...
	uk ptr1 = ((tuk)&hashableBlob.sHeapProperties.CPUPageProperty) + sizeof(hashableBlob.sHeapProperties.Type);
	ZeroMemory(ptr1, offsetof(D3D11_HEAP_PROPERTIES, CPUPageProperty) - sizeof(hashableBlob.sHeapProperties.Type));
	uk ptr2 = ((tuk)&hashableBlob.sHeapProperties.CPUPageProperty) + sizeof(hashableBlob.sHeapProperties.CPUPageProperty);
	...
	// Clear spaces from alignment of members
	uk ptr3 = ((tuk)&hashableBlob.sResourceDesc.Dimension) + sizeof(hashableBlob.sResourceDesc.Dimension);
	ZeroMemory(ptr3, offsetof(D3D11_RESOURCE_DESC, Alignment) - sizeof(hashableBlob.sResourceDesc.Dimension));
	uk ptr4 = ((tuk)&hashableBlob.sResourceDesc.Flags) + sizeof(hashableBlob.sResourceDesc.Flags);
...
void CDevice::FlushReleaseHeap(const UINT64 (&completedFenceValues)[CMDQUEUE_NUM], const UINT64 (&pruneFenceValues)[CMDQUEUE_NUM]) threadsafe
{
	...
	if (CRendererCVars::CV_r_ShowVideoMemoryStats)
	{
		...
		IRenderAuxText::Draw2dLabel(colPos, float(rowPos += rowHeight), fontSize, Col_Blue, false, "Fences: Current %lli, Submitted %lli, Completed %lli",
			fenceUpr.GetCurrentValue(CMDQUEUE_IMMEDIATE), fenceUpr.GetSubmittedValue(CMDQUEUE_IMMEDIATE), fenceUpr.GetLastCompletedFenceValue(CMDQUEUE_IMMEDIATE));
		...
		for (it = m_RecycleHeap.begin(); it != m_RecycleHeap.end(); it = nx)
		{
			...
			else if (Dimension == D3D11_RESOURCE_DIMENSION_TEXTURE1D)
			{
				...
				u32 size = CTexture::TextureDataSize(sDesc.Width, 1, 1, sDesc.MipLevels, sDesc.ArraySize, DeviceFormats::ConvertToTexFormat(sDesc.Format));
				recycleSize += counter * size;
				recycleNums += counter;
			...
			else if (Dimension == D3D11_RESOURCE_DIMENSION_TEXTURE2D)
			{
				...
				u32 size = CTexture::TextureDataSize(sDesc.Width, sDesc.Height, 1, sDesc.MipLevels, sDesc.ArraySize, DeviceFormats::ConvertToTexFormat(sDesc.Format));
				recycleSize += counter * size;
				recycleNums += counter;
			...
			else if (Dimension == D3D11_RESOURCE_DIMENSION_TEXTURE3D)
			{
				...
				u32 size = CTexture::TextureDataSize(sDesc.Width, sDesc.Height, sDesc.Depth, sDesc.MipLevels, 1, DeviceFormats::ConvertToTexFormat(sDesc.Format));
				recycleSize += counter * size;
				recycleNums += counter;
