...
DrxCriticalSection STexPoolItemHdr::s_sSyncLock;

bool STexPoolItem::IsStillUsedByGPU(u32 nTick)
{
...
void CTextureStreamPoolMgr::Flush()
{
	...
	{
		assert(m_nFreePoolEnd < DRX_ARRAY_COUNT(m_FreePool));
		uk p = m_FreePool[m_nFreePoolBegin];
...
STexPool* CTextureStreamPoolMgr::GetPool(const STextureLayout& pLayout)
{
	...
	TexturePoolKey poolKey((u16)pLayout.m_nWidth, (u16)pLayout.m_nHeight, d3dFmt, (u8)pLayout.m_eTT, (u8)pLayout.m_nMips, (u16)pLayout.m_nArraySize);
...
STexPoolItem* CTextureStreamPoolMgr::GetItem(const STextureLayout& pLayout, bool bShouldBeCreated, tukk sName, const STexturePayload* pPayload, bool bCanCreate, bool bWaitForIdle)
{
	...
	else
	{
		...
		{
			assert(m_nFreePoolBegin < (DRX_ARRAY_COUNT(m_FreePool)));
		...
		DrxInterlockedAdd(&m_nDeviceMemReserved, pPool->m_nDevTextureSize);
	...
	DrxInterlockedAdd(&m_nDeviceMemInUse, pIT->m_nDevTextureSize);
...
void CTextureStreamPoolMgr::ReleaseItem(STexPoolItem* pItem)
{
	...
	if (CRenderer::CV_r_logTexStreaming == 2)
	{
		tukk name = pItem->m_pTex ? pItem->m_pTex->GetSourceName() : "";
	...
	DrxInterlockedAdd(&m_nDeviceMemInUse, -(intptr_t)pItem->m_nDevTextureSize);
	pItem->m_pTex = NULL;
...
STexPool* CTextureStreamPoolMgr::GetOrCreatePool(i32 nWidth, i32 nHeight, i32 nMips, i32 nArraySize, D3DFormat eTF, ETEX_Type eTT)
{
	STexPool* pPool = NULL;

	assert((eTT != eTT_Cube && eTT != eTT_CubeArray) || !(nArraySize % 6));
	TexturePoolKey poolKey((u16)nWidth, (u16)nHeight, eTF, (u8)eTT, (u8)nMips, (u16)nArraySize);
...
void CTextureStreamPoolMgr::FlushFree()
{
	...
	while (pITH != &m_FreeTexPoolItems)
	{
		...
		assert(!pIT->m_pTex);
		DrxInterlockedAdd(&m_nDeviceMemReserved, -(intptr_t)pIT->m_nDevTextureSize);

#ifdef TEXSTRM_USE_FREEPOOL
		u32 nFreePoolSize = (m_nFreePoolEnd - m_nFreePoolBegin) % MaxFreePool;
...
void CTextureStreamPoolMgr::GarbageCollect(size_t* nCurTexPoolSize, size_t nLowerPoolLimit, i32 nMaxItemsToFree)
{
	...
	DrxInterlockedAdd(&m_nDeviceMemReserved, -nFreed);
...
void CTextureStreamPoolMgr::GetMemoryUsage(IDrxSizer* pSizer)
{
