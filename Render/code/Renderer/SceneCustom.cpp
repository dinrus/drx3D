	...
	// particle resources
	{
		if (bOnInit)
		{
			m_perPassResources.SetBuffer(
				EReservedTextureSlot_ParticlePositionStream, CDeviceBufferUpr::GetNullBufferStructured(),
				EDefaultResourceViews::Default, EShaderStage_AllWithoutCompute);
			m_perPassResources.SetBuffer(
				EReservedTextureSlot_ParticleAxesStream, CDeviceBufferUpr::GetNullBufferStructured(),
				EDefaultResourceViews::Default, EShaderStage_AllWithoutCompute);
			m_perPassResources.SetBuffer(
				EReservedTextureSlot_ParticleColorSTStream, CDeviceBufferUpr::GetNullBufferStructured(),
				EDefaultResourceViews::Default, EShaderStage_AllWithoutCompute);
		}
		else
		{
...
void CSceneCustomStage::ExecuteSelectionHighlight()
{
	...
	u32 numItems = pRenderView->GetRenderItems(EFSLIST_HIGHLIGHT).size();
	if (numItems == 0)
		return;
	...
	// NOTE: no more external state changes in here, everything should have been setup
	{
		...
		u32 startHighlight = pRenderView->FindRenderListSplit(highlightPredicate, EFSLIST_HIGHLIGHT, 0, numItems);
		u32 startSelected  = pRenderView->FindRenderListSplit(selectionPredicate, EFSLIST_HIGHLIGHT, 0, startHighlight);
	...
	static CDrxNameTSCRC techSilhouette("SelectionSilhouetteHighlight");
	...
	static CDrxNameR highlightColorName("highlightColor");
	static CDrxNameR selectionColorName("selectionColor");
	static CDrxNameR ghostAlphaName("ghostAlpha");
	static CDrxNameR outlineName("outlineWidth");
