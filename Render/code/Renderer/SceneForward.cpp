...
#include <drx3D/Render/SceneForward.h>

#include <drx3D/Render/DriverD3D.h>
#include <drx3D/Render/D3D_SVO.h>
#include <drx3D/Render/ReverseDepth.h>
#include <drx3D/Render/RendElements/Stars.h>

#include <drx3D/Render/D3D/GraphicsPipeline/Fog.h>
                                             ...
                                             std::function<void(CDeviceGraphicsPSODesc& psoDesc, const SGraphicsPipelineStateDescription& desc)> customState)
{
	...
	{
		i32k shaderFlags = ((CShader*)desc.shaderItem.m_pShader)->GetFlags();
		i32k shaderFlags2 = ((CShader*)desc.shaderItem.m_pShader)->GetFlags2();
...
bool CSceneForwardStage::PreparePerPassResources(bool bOnInit, bool bShadowMask, bool bFog)
{
	...
	if (!bOnInit)
	{
		...
		DrxStackAllocWithSize(SPerPassConstantBuffer, cb, CDeviceBufferUpr::AlignBufferSizeForStreaming);
	...
	for (u32 i = 0; i < DRX_ARRAY_COUNT(pResourceDescs); i++)
	{
		...
		// Textures
		{
			i32 nTerrainTex0 = 0, nTerrainTex1 = 0, nTerrainTex2 = 0;
		...
		// Particle resources
		{
			if (bOnInit)
			{
				pResources->SetBuffer(
					...
					CDeviceBufferUpr::GetNullBufferStructured(),
		...
		// Tiled shading resources
		{
			if (bOnInit)
			{
				pResources->SetBuffer(17, CDeviceBufferUpr::GetNullBufferTyped(), EDefaultResourceViews::Default, EShaderStage_AllWithoutCompute);
				pResources->SetBuffer(18, CDeviceBufferUpr::GetNullBufferStructured(), EDefaultResourceViews::Default, EShaderStage_AllWithoutCompute);
				pResources->SetBuffer(19, CDeviceBufferUpr::GetNullBufferStructured(), EDefaultResourceViews::Default, EShaderStage_AllWithoutCompute);
		...
		DRX_ASSERT(!pResources->HasChangedBindPoints()); // Cannot change resource layout after init. It is baked into the shaders
...
void CSceneForwardStage::ExecuteTransparentDepthFixup()
{
	...
	static CDrxNameTSCRC techName("TranspDepthFixupMerge");
...
void CSceneForwardStage::ExecuteTransparentLoRes(i32 subRes)
{
	CRenderView* pRenderView = RenderView();
...
void CSceneForwardStage::ExecuteMinimum(CTexture* pColorTex, CTexture* pDepthTex)
{
	...
	DRX_ASSERT(pRenderer->m_nGraphicsPipeline == 3);
...
void CSceneForwardStage::SetHDRSkyParameters()
{
	...
	// Day sky
	{
		static CDrxNameR mieName("SkyDome_PartialMieInScatteringConst");
		static CDrxNameR rayleighName("SkyDome_PartialRayleighInScatteringConst");
		static CDrxNameR sunDirectionName("SkyDome_SunDirection");
		static CDrxNameR phaseName("SkyDome_PhaseFunctionConstants");
	...
	// Night sky
	{
		static CDrxNameR nightColBaseName("SkyDome_NightSkyColBase");
		static CDrxNameR nightColDeltaName("SkyDome_NightSkyColDelta");
		static CDrxNameR nightZenithColShiftName("SkyDome_NightSkyZenithColShift");
	...
	// Moon
	{
		static CDrxNameR moonTexGenRightName("SkyDome_NightMoonTexGenRight");
		static CDrxNameR moonTexGenUpName("SkyDome_NightMoonTexGenUp");
		static CDrxNameR moonDirSizeName("SkyDome_NightMoonDirSize");
		static CDrxNameR moonColorName("SkyDome_NightMoonColor");
		static CDrxNameR moonInnerScaleName("SkyDome_NightMoonInnerCoronaColorScale");
		static CDrxNameR moonOuterScaleName("SkyDome_NightMoonOuterCoronaColorScale");
...
static void FillSkyTextureData(CTexture* pTexture, ukk pData, u32k width, u32k height, u32k pitch)
{
	assert(pTexture && pTexture->GetWidth() == width && pTexture->GetHeight() == height && pTexture->GetDevTexture());
	const SResourceMemoryAlignment layout = SResourceMemoryAlignment::Linear<DrxHalf4>(width, height);
	DRX_ASSERT(pitch == layout.rowStride);
...
void CSceneForwardStage::ExecuteSky(CTexture* pColorTex, CTexture* pDepthTex)
{
	...
	DRX_ASSERT(pColorTex->GetWidth() == viewport.Width && pColorTex->GetHeight() == viewport.Height);
	...
	if (m_skyPass.IsDirty(rtMask, timestamp, !m_pHDRSkyRE), pDepthTex->GetTextureID())
	{
		...
		static CDrxNameTSCRC techSkyPass("SkyPass");
	...
	if (m_pHDRSkyRE && m_pHDRSkyRE->m_pStars && m_pHDRSkyRE->m_pStars->m_pStarMesh && starIntensity > 1e-3f)
	{
		...
		i32k depthState = bReverseDepth ? GS_DEPTHFUNC_GEQUAL : GS_DEPTHFUNC_LEQUAL;

		static CDrxNameTSCRC techStars("Stars");
		...
		{
			static CDrxNameR nameMoonTexGenRight("SkyDome_NightMoonTexGenRight");
			static CDrxNameR nameMoonTexGenUp("SkyDome_NightMoonTexGenUp");
			static CDrxNameR nameMoonDirSize("SkyDome_NightMoonDirSize");
			static CDrxNameR nameStarSize("StarSize");
			static CDrxNameR nameStarIntensity("StarIntensity");

			m_starsPrimitive.GetConstantUpr().BeginNamedConstantUpdate();

			m_starsPrimitive.GetConstantUpr().SetNamedConstant(nameMoonTexGenRight, m_paramMoonTexGenRight, eHWSC_Vertex);
			m_starsPrimitive.GetConstantUpr().SetNamedConstant(nameMoonTexGenUp, m_paramMoonTexGenUp, eHWSC_Vertex);
			m_starsPrimitive.GetConstantUpr().SetNamedConstant(nameMoonDirSize, m_paramMoonDirSize, eHWSC_Vertex);
			...
			m_starsPrimitive.GetConstantUpr().SetNamedConstant(nameStarSize, paramStarSize, eHWSC_Vertex);
			Vec4 paramStarIntensity(starIntensity * min(1.0f, size), 0, 0, 0);
			m_starsPrimitive.GetConstantUpr().SetNamedConstant(nameStarIntensity, paramStarIntensity, eHWSC_Pixel);

			m_starsPrimitive.GetConstantUpr().EndNamedConstantUpdate(&m_starsPass.GetViewport());
