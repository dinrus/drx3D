...
#include <drx3D/CoreX/Renderer/RendElements/CREWaterVolume.h>
...
struct SCompiledWaterVolume : NoCopy
{
	...
	u32                    m_bCaustics        : 1; //!< True if this object can be rendered in caustics gen pass.
	u32                    m_reserved         : 29;
};
}
...
bool CREWaterVolume::Compile(CRenderObject* pObj,CRenderView *pRenderView, bool updateInstanceDataOnly)
{
	...
	DRX_ASSERT((!bFullscreen && compiledObj.m_indexStreamSet != nullptr) || (bFullscreen && compiledObj.m_indexStreamSet == nullptr));
  ...
  CRenderView *pRenderView) const
{
	...
	DrxStackAllocWithSize(watervolume::SPerDrawConstantBuffer, cb, CDeviceBufferUpr::AlignBufferSizeForStreaming);
...
void CREWaterVolume::UpdateVertex(watervolume::SCompiledWaterVolume& compiledObj, bool bFullscreen)
{
	...
	DRX_ASSERT((bFullscreen && !bDrawSurface) || (!bFullscreen && bDrawSurface));
	...
	if (!bFullscreen)
	{
		...
		DRX_ASSERT(m_vertexBuffer.handle != watervolume::invalidBufferHandle);
		DRX_ASSERT(gRenDev->m_DevBufMan.Size(m_vertexBuffer.handle) >= vertexBufferSize);
		DRX_ASSERT(m_pParams->m_pVertices!=nullptr);
		...
		DRX_ASSERT(m_indexBuffer.handle != watervolume::invalidBufferHandle);
		DRX_ASSERT(gRenDev->m_DevBufMan.Size(m_indexBuffer.handle) >= indexBufferSize);
		DRX_ASSERT(m_pParams->m_pIndices != nullptr);
	...
	else
	{
		    ...
		    || m_vertexBuffer.size != vertexBufferSize)
		{
			...
			// update vertex buffer once and keep it.
			{
				...
				DrxStackAllocWithSizeVector(SVF_P3F_C4B_T2F, vertexNum, fullscreenTriVertices, CDeviceBufferUpr::AlignBufferSizeForStreaming);
