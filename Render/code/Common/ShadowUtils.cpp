...
void CShadowUtils::GetProjectiveTexGen(const SRenderLight* pLight, i32 nFace, Matrix44A* mTexGen)
{
	assert(pLight != NULL);
	float fOffsetX = 0.5f;
	float fOffsetY = 0.5f;
...
void CShadowUtils::GetCubemapFrustumForLight(const SRenderLight* pLight, i32 nS, float fFov, Matrix44A* pmProj, Matrix44A* pmView, bool bProjLight)
{
...
void CShadowUtils::GetCubemapFrustum(EFrustum_Type eFrustumType, const ShadowMapFrustum* pFrust, i32 nS, Matrix44A* pmProj, Matrix44A* pmView, Matrix33* pmLightRot)
{
	//new cubemap calculation
...
{
	Vec3 out;
	out.x = in.x - (float)(i32)in.x;
	out.y = in.y - (float)(i32)in.y;
	out.z = in.z - (float)(i32)in.z;
...
{
	DRX_ASSERT(pRenderView != nullptr);
...
bool CShadowUtils::GetShadowCascades(SShadowCascades& shadowCascades, const CRenderView* pRenderView)
{
	...
	bool valid = true;
	i32k size = static_cast<i32>(shadowFrustumArray.size());
	i32k count = (size < MaxCascadesNum) ? size : MaxCascadesNum;
...
bool CShadowUtils::GetShadowCascadesSamplingInfo(SShadowCascadesSamplingInfo& samplingInfo, const CRenderView* pRenderView)
{
	...
	u32 nCascadeMask = 0;
	bool valid = true;
	const auto size = static_cast<i32>(shadowFrustumArray.size());
...
Matrix44 CShadowUtils::GetClipToTexSpaceMatrix(const ShadowMapFrustum* pFrustum, i32 nSide)
{
	...
	{
		i32k texWidth = max(pFrustum->pDepthTex->GetWidth(), 1);
		i32k texHeight = max(pFrustum->pDepthTex->GetHeight(), 1);
...
CShadowUtils::SShadowSamplingInfo CShadowUtils::GetDeferredShadowSamplingInfo(ShadowMapFrustum* pFr, i32 nSide, const CCamera& cam, const SRenderViewport& viewport, const Vec2& subpixelOffset)
{
...
Vec2& CPoissonDiskGen::GetSample(i32 ind)
{
	assert(ind < m_vSamples.size() && ind >= 0);
	return m_vSamples[ind];
}

CPoissonDiskGen& CPoissonDiskGen::GetGenForKernelSize(i32 size)
{
	if ((i32)s_kernelSizeGens.size() <= size)
...
void CPoissonDiskGen::RandomPoint(Vec2& p)
{
	...
	do
	{
		p.x = drx_random(-0.5f, 0.5f);
		p.y = drx_random(-0.5f, 0.5f);
	}
...
void CPoissonDiskGen::InitSamples()
{
	...
	for (i32 i = 1; i < (i32)m_vSamples.size(); i++)
	{
		float dmax = -1.0;

		for (i32 c = 0; c < i * nQ; c++)
		{
			Vec2 curSample;
	...
	for (i32 i = 0; i < (i32)m_vSamples.size(); i++)
	{
		m_vSamples[i] *= 2.0f;
		//DrxLogAlways("Sample %i: (%.6f, %.6f)\n", i, m_vSamples[i].x, m_vSamples[i].y);
	}
...
void CShadowUtils::GetIrregKernel(float sData[][4], i32 nSamplesNum)
{
#define PACKED_SAMPLES 1
	...
	for (i32 i = 0, nIdx = 0; i < nSamplesNum; i += 2, nIdx++)
	{
	...
	for (i32 i = 0, nIdx = 0; i < nSamplesNum; i++, nIdx++)
	{
		Vec2 vSample = pdg.GetSample(i);
		sData[nIdx][0] = vSample.x;
		sData[nIdx][1] = vSample.y;
...
void ShadowMapFrustum::GetMemoryUsage(IDrxSizer* pSizer) const
{
