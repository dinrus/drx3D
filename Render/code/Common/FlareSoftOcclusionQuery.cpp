...
u8 CFlareSoftOcclusionQuery::s_paletteRawCache[s_nIDMax * 4];
char CFlareSoftOcclusionQuery::s_idHashTable[s_nIDMax];
i32 CFlareSoftOcclusionQuery::s_idCount = 0;
i32 CFlareSoftOcclusionQuery::s_ringReadIdx = 1;
i32 CFlareSoftOcclusionQuery::s_ringWriteIdx = 0;
i32 CFlareSoftOcclusionQuery::s_ringSize = MAX_OCCLUSION_READBACK_TEXTURES;
...
void ReportIDPoolOverflow(i32 idCount, i32 idMax)
{
	iLog->Log("Number of soft occlusion queries [%d] exceeds current allowed range %d", idCount, idMax);
}

i32 CFlareSoftOcclusionQuery::GenID()
{
	i32 hashCode = s_idCount % s_nIDMax;
...
void CFlareSoftOcclusionQuery::ReleaseID(i32 id)
{
	if (id < s_nIDMax)
...
void CFlareSoftOcclusionQuery::BatchReadResults()
{
	...
	DRX_PROFILE_REGION_WAITING(PROFILE_RENDERER, "CFlareSoftOcclusionQuery::BatchReadResults");

	CRendererResources::s_ptexFlaresOcclusionRing[s_ringReadIdx]->GetDevTexture()->AccessCurrStagingResource(0, false, [=](uk pData, u32 rowPitch, u32 slicePitch)
	{
		u8* pTexBuf = reinterpret_cast<u8*>(pData);
		i32 validLineStrideBytes = s_nIDColMax * 4;
		for (i32 i = 0; i < s_nIDRowMax; i++)
		{
...
CSoftOcclusionUpr::CSoftOcclusionUpr()
	: m_nPos(0)
	, m_indexBuffer(~0u)
...
bool CSoftOcclusionUpr::PrepareOcclusionPrimitive(CRenderPrimitive& primitive, const CPrimitiveRenderPass& targetPass,const SRenderViewInfo& viewInfo)
{
	if (m_indexBuffer == ~0u || m_occlusionVertexBuffer == ~0u)
		return false;

	u32k vertexCount = GetSize() * 4;
	u32k indexCount = GetSize() * 3 * 2;

	// NOTE: Get aligned stack-space (pointer and size aligned to manager's alignment requirement)
	DrxStackAllocWithSizeVector(SVF_P3F_C4B_T2F, vertexCount, pDeviceVBAddr, CDeviceBufferUpr::AlignBufferSizeForStreaming);

	for (i32 i(0), iSoftOcclusionListSize(GetSize()); i < iSoftOcclusionListSize; ++i)
	{
	...
	static CDrxNameTSCRC techRenderPlane("RenderPlane");
...
void CSoftOcclusionUpr::Init()
{
	i32k maxIndexCount = CFlareSoftOcclusionQuery::s_nIDMax * 2 * 3;
	i32k maxVertexCount = CFlareSoftOcclusionQuery::s_nIDMax * 4;

	m_indexBuffer           = gcpRendD3D->m_DevBufMan.Create(BBT_INDEX_BUFFER, BU_STATIC,   maxIndexCount  * sizeof(u16));
	...
	if (m_indexBuffer != ~0u)
	{
		DrxStackAllocWithSizeVector(u16, maxIndexCount, pDeviceIBAddr, CDeviceBufferUpr::AlignBufferSizeForStreaming);

		for (i32 i = 0; i < CFlareSoftOcclusionQuery::s_nIDMax; ++i)
		{
			i32 offset0 = i * 6;
			i32 offset1 = i * 4;
		...
		gcpRendD3D->m_DevBufMan.UpdateBuffer(m_indexBuffer, pDeviceIBAddr, maxIndexCount * sizeof(u16));
	}
}

bool CSoftOcclusionUpr::PrepareGatherPrimitive(CRenderPrimitive& primitive, const CPrimitiveRenderPass& targetPass, SRenderViewInfo* pViewInfo, i32 viewInfoCount)
{
	...
	static CDrxNameTSCRC techGatherPlane("Gather");
	...
	{
		static CDrxNameR occlusionParamsName("occlusionParams");
		...
		constantUpr.SetNamedConstant(occlusionParamsName, occlusionSizeParams);
	}
...
CFlareSoftOcclusionQuery* CSoftOcclusionUpr::GetSoftOcclusionQuery(i32 nIndex) const
{
...
void CSoftOcclusionUpr::AddSoftOcclusionQuery(CFlareSoftOcclusionQuery* pQuery, const Vec3& vPos)
{
...
void CSoftOcclusionUpr::Reset()
{
	m_nPos = 0;
}

bool CSoftOcclusionUpr::Update(SRenderViewInfo* pViewInfo, i32 viewInfoCount)
{
	if (GetSize() > 0)
	{
