...
CRenderOutput::CRenderOutput(CTexture* pColorTexture, u32 clearFlags, ColorF clearColor, float clearDepth)
...
{
	i32 outputWidth  = pColorTexture->GetWidth();
	i32 outputHeight = pColorTexture->GetHeight();

	ChangeOutputResolution(outputWidth, outputHeight);
	SetViewport(SRenderViewport(0, 0, outputWidth, outputHeight));
}

CRenderOutput::CRenderOutput(CTexture* pColorTexture, CTexture* pDepthTexture, u32 clearFlags, ColorF clearColor, float clearDepth)
	: m_pColorTarget(pColorTexture)
...
CRenderOutput::CRenderOutput(SDynTexture* pDynTexture, i32 outputWidth, i32 outputHeight, bool useTemporaryDepthBuffer, u32 clearFlags, ColorF clearColor, float clearDepth)
...
{
	DRX_ASSERT(m_pDisplayContext);

	i32k outputWidth  = m_pDisplayContext->IsScalable() && CRendererCVars::CV_r_CustomResWidth  ? std::min(CRendererCVars::CV_r_CustomResMaxSize, CRendererCVars::CV_r_CustomResWidth ) : m_pDisplayContext->GetDisplayResolution()[0];
	i32k outputHeight = m_pDisplayContext->IsScalable() && CRendererCVars::CV_r_CustomResHeight ? std::min(CRendererCVars::CV_r_CustomResMaxSize, CRendererCVars::CV_r_CustomResHeight) : m_pDisplayContext->GetDisplayResolution()[1];
...
void CRenderOutput::BeginRendering(CRenderView* pRenderView, stl::optional<u32> overrideClearFlags)
{
	...
	{
		DRX_ASSERT(m_OutputWidth  == (m_pDisplayContext->IsScalable() && CRendererCVars::CV_r_CustomResWidth  ? std::min(CRendererCVars::CV_r_CustomResMaxSize, CRendererCVars::CV_r_CustomResWidth)  : m_pDisplayContext->GetDisplayResolution()[0]));
		DRX_ASSERT(m_OutputHeight == (m_pDisplayContext->IsScalable() && CRendererCVars::CV_r_CustomResHeight ? std::min(CRendererCVars::CV_r_CustomResMaxSize, CRendererCVars::CV_r_CustomResHeight) : m_pDisplayContext->GetDisplayResolution()[1]));
...
// 			m_pDisplayContext->SetLastCamera(CCamera::eEye_Right, pRenderView->GetCamera(CCamera::eEye_Right));
	...
	//////////////////////////////////////////////////////////////////////////
	u32 clearTargetFlag = overrideClearFlags.value_or(m_clearTargetFlag);
...
CTexture* CRenderOutput::GetDepthTarget() const
{
	...
	DRX_ASSERT(m_pDepthTarget || m_bUseTempDepthBuffer);
...
//////////////////////////////////////////////////////////////////////////
void CRenderOutput::InitializeOutputResolution(i32 outputWidth, i32 outputHeight)
{
	...
	if (m_pDisplayContext)
	{
			...
			0 != displayResolution[1])
		{
			...
			DRX_ASSERT((outputWidth  >= ((displayResolution[0] + 1) >> 1)) && (outputWidth  <= displayResolution[0]));
			DRX_ASSERT((outputHeight >= ((displayResolution[1] + 1) >> 1)) && (outputHeight <= displayResolution[1]));
		}
	}
...
void CRenderOutput::ChangeOutputResolution(i32 outputWidth, i32 outputHeight)
{
	InitializeOutputResolution(outputWidth, outputHeight);
...
void CRenderOutput::AllocateColorTarget()
{
	...
	u32k renderTargetFlags = FT_NOMIPS | /* FT_DONT_RELEASE | */FT_DONT_STREAM | FT_USAGE_RENDERTARGET;
	const STxt uniqueTexName = GenerateUniqueTextureName(CImageExtensionHelper::IsDynamicRange(eCFormat) ? "$HDR-Output" : "$LDR-Output", m_uniqueId, m_name);
...
void CRenderOutput::AllocateDepthTarget()
{
	...
	u32k renderTargetFlags = FT_NOMIPS | /* FT_DONT_RELEASE | */FT_DONT_STREAM | FT_USAGE_DEPTHSTENCIL;
	const STxt uniqueTexName = GenerateUniqueTextureName("$Z-Output", m_uniqueId, m_name);
