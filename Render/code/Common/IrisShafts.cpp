...
IrisShafts::IrisShafts(tukk name)
...
i32 IrisShafts::ComputeDynamicSmoothLevel(i32 maxLevel, float spanAngle, float threshold)
{
	return min(maxLevel, (i32)(spanAngle / threshold));
}

void IrisShafts::GenMesh()
{
	stable_rand::setSeed((i32)(m_nNoiseSeed * m_fConcentrationBoost));
	...
	std::vector<i32> randomTable;
	randomTable.resize(m_nComplexity);
	for (i32 i = 0; i < m_nComplexity; ++i)
		randomTable[i] = i;
	for (i32 i = 0; i < m_nComplexity; ++i)
		std::swap(randomTable[(i32)(stable_rand::randPositive() * m_nComplexity)], randomTable[(i32)(stable_rand::randPositive() * m_nComplexity)]);
	...
	for (i32 i = 0; i < m_nComplexity; i++)
	{
		...
		i32 dynSmoothLevel = ComputeDynamicSmoothLevel(m_nSmoothLevel, halfAngle * 2, 1);
		if (dynSmoothLevel <= 0)
			continue;
		...
		i32 generatedPolyonNum = (i32)(indices.size() + m_indices.size()) / 3;
		if (CRenderer::CV_r_FlaresIrisShaftMaxPolyNum != 0 && generatedPolyonNum > CRenderer::CV_r_FlaresIrisShaftMaxPolyNum)
			break;

		i32 indexOffset = m_vertices.size();
		m_vertices.insert(m_vertices.end(), vertices.begin(), vertices.end());

		for (i32 k = 0, iIndexSize(indices.size()); k < iIndexSize; ++k)
...
bool IrisShafts::PreparePrimitives(const SPreparePrimitivesContext& context)
{
	...
	static CDrxNameTSCRC techName("IrisShafts");
	...
	// update constants
	{
		auto constants = m_primitive.GetConstantUpr().BeginTypedConstantUpdate<SShaderParams>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex | EShaderStage_Pixel);
		...
		m_primitive.GetConstantUpr().EndTypedConstantUpdate(constants);
	}
