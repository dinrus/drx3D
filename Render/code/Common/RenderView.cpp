...
#include <drx3D/CoreX/Renderer/branchmask.h>
...
//////////////////////////////////////////////////////////////////////////
CRenderView::CRenderView(tukk name, EViewType type, CRenderView* pParentView, ShadowMapFrustum* pShadowFrustumOwner)
	...
	, m_viewInfoCount(1)
{
...

	{
		...
		{
			DrxModuleMemalignFree(m_tempRenderObjects.pRenderObjectsPool);
		...
		m_tempRenderObjects.pRenderObjectsPool = (CRenderObject*)DrxModuleMemalign(sizeof(CRenderObject) * (m_tempRenderObjects.numObjectsInPool) + sizeof(CRenderObject), 16);
		for (u32 j = 0; j < m_tempRenderObjects.numObjectsInPool; j++)
		{
...
{
	DRX_ASSERT(m_usageMode == IRenderView::eUsageModeReadingDone || 
	...
	{
		DRX_ASSERT(!m_jobstate_Write.IsRunning());
	...
	for (i32 i = 0; i < eDLT_NumLightTypes; i++)
	{
	...
	for (i32 i = 0; i < IFogVolumeRenderNode::eFogVolumeType_Count; ++i)
	{
		m_fogVolumes[i].clear();
	}

	for (i32 i = 0; i < CloudBlockerTypeNum; ++i)
	{
...
void CRenderView::SetFrameId(i32 frameId)
{
	m_frameId = frameId;
}

void CRenderView::SetCameras(const CCamera* pCameras, i32 cameraCount)
{
	DRX_ASSERT(cameraCount == 1 || cameraCount == 2);

	for (i32 i = 0; i < cameraCount; ++i)
	{
		DRX_ASSERT(pCameras[i].GetEye() == CCamera::eEye_Left || pCameras[i].GetEye() == CCamera::eEye_Right);
...
//////////////////////////////////////////////////////////////////////////
void CRenderView::SetPreviousFrameCameras(const CCamera* pCameras, i32 cameraCount)
{
	DRX_ASSERT(cameraCount == 1 || cameraCount == 2);
...
void CRenderView::CalculateViewInfo()
{
	...
	{
		u32 renderingFlags = m_shaderRenderingFlags;
		if ((renderingFlags & (SHDF_STEREO_LEFT_EYE | SHDF_STEREO_RIGHT_EYE)) == 0) // non-stereo case
			renderingFlags |= SHDF_STEREO_LEFT_EYE;

		u32 currentEyeFlag = eye == CCamera::eEye_Left ? SHDF_STEREO_LEFT_EYE : SHDF_STEREO_RIGHT_EYE;
...
void CRenderView::SwitchUsageMode(EUsageMode mode)
{
	...
	{
		DRX_ASSERT(m_usageMode == IRenderView::eUsageModeUndefined || m_usageMode == IRenderView::eUsageModeReadingDone);
		DRX_ASSERT(!m_jobstate_Write.IsRunning());
		DRX_ASSERT(!m_jobstate_Sort.IsRunning());
		...
		//if (m_viewType != eViewType_Shadow)
		{
			...
			m_jobstate_Write.RegisterPostJobCallback("JobRenderViewPostWrite", lambda_job_after_write, JobUpr::eRegularPriority, &m_jobstate_PostWrite);
		}
	...
	{
		DRX_ASSERT(m_usageMode == IRenderView::eUsageModeWriting);
	...
	if (mode == eUsageModeReading)
	{
		...
		//Job_PostWrite();
		DRX_ASSERT(m_bPostWriteExecuted);

		DRX_ASSERT(m_usageMode == IRenderView::eUsageModeWritingDone || m_usageMode == IRenderView::eUsageModeReadingDone);
	...
	{
		DRX_ASSERT(m_usageMode == IRenderView::eUsageModeReading);
		DRX_ASSERT(!m_jobstate_Write.IsRunning());
		DRX_ASSERT(!m_jobstate_PostWrite.IsRunning());
		DRX_ASSERT(!m_jobstate_Sort.IsRunning());
	...
	{
		DrxAutoLock<DrxCriticalSectionNonRecursive> lock(m_lock_UsageMode);
		m_usageMode = mode;
	}
}
...
RenderLightIndex CRenderView::AddDeferredLight(const SRenderLight& pDL, float fMult, const SRenderingPassInfo& passInfo)
{
	...
	if (pDL.GetLensOpticsElement() && !pDL.m_pSoftOccQuery)
	{
		...
		u8k numVisibilityFaders = 2; // For each flare type
	...
	if (CRenderer::CV_r_DeferredShadingSortLights == 1 && bSort && lightsNum)
	{
		...
		u32 lightArea = pDL.m_sWidth * pDL.m_sHeight;
		...
		while (startPoint <= endPoint)  //binary search for insertion point
		{
			...
			u32 compareLightArea = compareLight.m_sWidth * compareLight.m_sHeight;
...
//////////////////////////////////////////////////////////////////////////
u8 CRenderView::AddClipVolume(const IClipVolume* pClipVolume)
{
	if (m_clipVolumes.size() < CClipVolumesStage::MaxDeferredClipVolumes)
	{
		...
		volume.nStencilRef = u8(m_clipVolumes.size()); // the first clip volume ID is reserved for outdoors. See STENCIL_VALUE_OUTDOORS
...
void CRenderView::SetClipVolumeBlendInfo(const IClipVolume* pClipVolume, i32 blendInfoCount, IClipVolume** blendVolumes, Plane* blendPlanes)
{
	size_t nClipVolumeIndex = pClipVolume->GetStencilRef() - 1; // 0 is reserved for outdoor
	DRX_ASSERT(blendInfoCount <= SDeferredClipVolume::MaxBlendInfoCount);
	DRX_ASSERT(nClipVolumeIndex >= 0 && nClipVolumeIndex < m_clipVolumes.size() && m_clipVolumes[nClipVolumeIndex].nStencilRef == pClipVolume->GetStencilRef());
	...
	for (i32 i = 0; i < blendInfoCount; ++i)
	{
...
{
	DRX_ASSERT(volumeType < IFogVolumeRenderNode::eFogVolumeType_Count);
	return m_fogVolumes[volumeType];
}

//////////////////////////////////////////////////////////////////////////

void CRenderView::AddCloudBlocker(const Vec3& pos, const Vec3& param, i32 flags)
{
	if (flags == 0)
	{
...
const std::vector<SCloudBlocker>& CRenderView::GetCloudBlockers(u32 blockerType) const
{
	DRX_ASSERT(blockerType < CloudBlockerTypeNum);
...
void CRenderView::AddPolygon(const SRenderPolygonDescription& poly, const SRenderingPassInfo& passInfo)
{
	...
	i32 nIndex = m_numUsedClientPolygons++;
...
void CRenderView::InspectRenderOutput()
{
	if (CRenderOutput* pRenderOutput = m_pRenderOutput.get())
	{
		...
		i32 outputWidth  = (vp.width  - vp.x); DRX_ASSERT(pRenderOutput->GetOutputResolution()[0] == outputWidth);
		i32 outputHeight = (vp.height - vp.y); DRX_ASSERT(pRenderOutput->GetOutputResolution()[1] == outputHeight);

		// Calculate the rendering resolution based on inputs ///////////////////////////////////////////////////////////
		i32 renderWidth  = outputWidth;
		i32 renderHeight = outputHeight;

		if (CRenderDisplayContext* pDisplayContext = pRenderOutput->GetDisplayContext())
		{
			i32k nMaxResolutionX = std::max(std::min(CRendererCVars::CV_r_CustomResMaxSize, gcpRendD3D.m_MaxTextureSize), outputWidth);
			i32k nMaxResolutionY = std::max(std::min(CRendererCVars::CV_r_CustomResMaxSize, gcpRendD3D.m_MaxTextureSize), outputHeight);

			i32 nSSSamplesX = pDisplayContext->m_nSSSamplesX; do { renderWidth  = outputWidth  * nSSSamplesX; --nSSSamplesX; } while (renderWidth  > nMaxResolutionX);
			i32 nSSSamplesY = pDisplayContext->m_nSSSamplesY; do { renderHeight = outputHeight * nSSSamplesY; --nSSSamplesX; } while (renderHeight > nMaxResolutionY);
		}
...
void CRenderView::ChangeRenderResolution(uint32_t renderWidth, uint32_t renderHeight, bool bForce)
{
	...
	DRX_ASSERT((renderWidth  % GetOutputResolution()[0]) == 0);
	DRX_ASSERT((renderHeight % GetOutputResolution()[1]) == 0);

	// No changes do not need to resize
	if (m_RenderWidth  == renderWidth &&
	    m_RenderHeight == renderHeight && !bForce)
	{ 
		DRX_ASSERT(m_pDepthTarget->GetWidth() >= renderWidth && m_pDepthTarget->GetHeight() >= renderHeight);
		DRX_ASSERT(m_pColorTarget->GetWidth() >= renderWidth && m_pColorTarget->GetHeight() >= renderHeight);
		return;
	}
...
//////////////////////////////////////////////////////////////////////////
CRenderView::RenderItems& CRenderView::GetRenderItems(i32 nRenderList)
{
	DRX_ASSERT(m_usageMode != eUsageModeWriting || nRenderList == EFSLIST_PREPROCESS); // While writing we must not read back items.

	m_renderItems[nRenderList].CoalesceMemory();
	return m_renderItems[nRenderList];
}

u32 CRenderView::GetBatchFlags(i32 nRenderList) const
{
...
{
	i32k passId = IsShadowGenView() ? 1 : 0;
...
//////////////////////////////////////////////////////////////////////////
static inline u32 CalculateRenderItemBatchFlags(SShaderItem& SH, CRenderObject* pObj, CRenderElement* re, const SRenderingPassInfo& passInfo, i32 nAboveWater) threadsafe
{
	u32 nFlags = SH.m_nPreprocessFlags & FB_MASK;
	...
	u32 uTransparent = 0; //(bool)(fAlpha < 1.0f); Not supported in new rendering pipeline 
	const uint64 ObjFlags = pObj->m_ObjFlags;

	if (!passInfo.IsRecursivePass() && pTech)
	{
		DrxPrefetch(pTech->m_nTechnique);
		DrxPrefetch(pR);
		...
		u32k nMaterialLayers = pObj->m_nMaterialLayers;
		u32k DecalFlags = pS->m_Flags & EF_DECAL;
		...
		{
			u32k nResourcesNoDrawFlags = static_cast<CShaderResources*>(pR)->CShaderResources::GetMtlLayerNoDrawFlags();

			// if ((nMaterialLayers & MTL_LAYER_BLEND_DYNAMICFROZEN) && !(nResourcesNoDrawFlags & MTL_LAYER_FROZEN))
			u32 uMask = mask_nz_zr(nMaterialLayers & MTL_LAYER_BLEND_DYNAMICFROZEN, nResourcesNoDrawFlags & MTL_LAYER_FROZEN);
		...
		//if ( ((ObjFlags & (FOB_DECAL)) | DecalFlags) == 0 ) // put the mask below
		{
			...
			{
				u32 uMask = mask_zr_zr(ObjFlags & (FOB_DECAL), DecalFlags);
		...
		{
			u32k nVisionParams = (pOD && pOD->m_nVisionParams);
			if (gRenDev->m_nThermalVisionMode && (pR->HeatAmount_unnormalized() || nVisionParams))
				nFlags |= FB_CUSTOM_RENDER;

			u32k customvisions = CRenderer::CV_r_customvisions;
			u32k nHUDSilhouettesParams = (pOD && pOD->m_nHUDSilhouetteParams);
...

	{
		...
		u32k mask = mask_nz_zr((u32)CRenderer::CV_r_ZPassOnly, nFlags & (FB_TRANSPARENT));
	...
	u32k nCloakMask = mask_nz_zr(pObj->m_nMaterialLayers & MTL_LAYER_BLEND_CLOAK, (pR ? static_cast<CShaderResources*>(pR)->CShaderResources::GetMtlLayerNoDrawFlags() & MTL_LAYER_CLOAK : 0));

	i32 nShaderFlags = (SH.m_pShader ? SH.m_pShader->GetFlags() : 0);
...
///////////////////////////////////////////////////////////////////////////////
static inline void AddEf_HandleForceFlags(i32& nList, i32& nAW, u32& nBatchFlags, u32k nShaderFlags, u32k nShaderFlags2, CRenderObject* obj) threadsafe
{
	// Force rendering in last place
	// FIXME: If object is permanent this is wrong!
	// branch-less

	i32k drawlast = nz2mask(nShaderFlags2 & EF2_FORCE_DRAWLAST);
	i32k drawfrst = nz2one(nShaderFlags2 & EF2_FORCE_DRAWFIRST);
...

	{
		...
		u32 mb1 = nShaderFlags2 & EF2_FORCE_TRANSPASS;
		u32 mb2 = nShaderFlags2 & EF2_FORCE_GENERALPASS;
		u32 mb3 = nShaderFlags2 & EF2_FORCE_WATERPASS;
	...
	u32k afterhdr = nz2mask(nShaderFlags2 & EF2_AFTERHDRPOSTPROCESS);
	u32k afterldr = nz2mask(nShaderFlags2 & EF2_AFTERPOSTPROCESS) | (afterhdr & drawlast);
...
///////////////////////////////////////////////////////////////////////////////
void CRenderView::AddRenderObject(CRenderElement* re, SShaderItem& SH, CRenderObject* obj, const SRenderingPassInfo& passInfo, i32 nList, i32 nAW) threadsafe
{
	...
	u32k nMaterialLayers = obj->m_nMaterialLayers;
	...
	u32k nCloakRenderedMask = pShaderResources && mask_nz_zr(nMaterialLayers & MTL_LAYER_BLEND_CLOAK, pShaderResources ? static_cast<CShaderResources*>(pShaderResources)->CShaderResources::GetMtlLayerNoDrawFlags() & MTL_LAYER_CLOAK : 0);
	u32 nCloakLayerMask = nz2mask(nMaterialLayers & MTL_LAYER_BLEND_CLOAK);
	...
	u32 nBatchFlags = CalculateRenderItemBatchFlags(SH, obj, re, passInfo, nAW);
	...
	u32k nRenderlistsFlags = (FB_PREPROCESS | FB_MULTILAYERS | FB_TRANSPARENT);
	if (nBatchFlags & nRenderlistsFlags || nCloakLayerMask)
	{
		...
		u32 mx1 = (nShaderFlags & EF_REFRACTIVE) | nCloakLayerMask;
		u32 mx2 = mask_nz_zr(nBatchFlags & FB_TRANSPARENT, (nList ^ EFSLIST_GENERAL) | mx1);
		...
		|| passInfo.IsRecursivePass() /* account for recursive scene traversal done in forward fashion*/)
	{
		...
		if (pOD && pOD->m_FogVolumeContribIdx == (u16)-1)
		{
	...
	u32k nShaderFlags2 = pSH->m_Flags2;
	...
	u32k nForceFlags = (EF2_FORCE_DRAWLAST | EF2_FORCE_DRAWFIRST | EF2_FORCE_ZPASS | EF2_FORCE_TRANSPASS | EF2_FORCE_GENERALPASS | EF2_FORCE_DRAWAFTERWATER | EF2_FORCE_WATERPASS | EF2_AFTERHDRPOSTPROCESS | EF2_AFTERPOSTPROCESS);
...
//////////////////////////////////////////////////////////////////////////
void CRenderView::AddRenderItem(CRenderElement* pElem, CRenderObject* RESTRICT_POINTER pObj, const SShaderItem& shaderItem, u32 nList, u32 nBatchFlags, const SRenderingPassInfo& passInfo, 
								SRendItemSorter sorter, bool bShadowPass, bool bForceOpaqueForward) threadsafe
{
	...
	// Need to differentiate between something rendered with cloak layer material, and sorted with cloak.
	// e.g. ironsight glows on gun should be sorted with cloak to not write depth - can be inconsistent with no depth from gun.
	u32k nCloakRenderedMask = shaderItem.m_pShaderResources && mask_nz_zr(nMaterialLayers & MTL_LAYER_BLEND_CLOAK, static_cast<CShaderResources*>(shaderItem.m_pShaderResources)->CShaderResources::GetMtlLayerNoDrawFlags() & MTL_LAYER_CLOAK);
...
inline void UpdateRenderListBatchFlags( u32& listFlags, i32 newFlags) threadsafe
{
	if (bConcurrent)
	{
		DrxInterlockedExchangeOr(( LONG*)&listFlags, newFlags);
	}
	else
	{
		listFlags |= newFlags;
...
template<bool bConcurrent>
inline void CRenderView::AddRenderItemToRenderLists(const SRendItem& ri, i32 nRenderList, CRenderObject* RESTRICT_POINTER pObj, const SShaderItem& shaderItem) threadsafe
{
	u32k nBatchFlags = ri.nBatchFlags;
	...
	if (!IsShadowGenView())
	{
		...
		{
			i32k targetRenderList = bNearest ? EFSLIST_FORWARD_OPAQUE_NEAREST : EFSLIST_FORWARD_OPAQUE;
...
void CRenderView::ExpandPermanentRenderObjects()
{
	...
	i32 renderPassType = IsShadowGenView() ? CPermanentRenderObject::eRenderPass_Shadows : CPermanentRenderObject::eRenderPass_General;
	u32 passId = IsShadowGenView() ? 1 : 0;
	...
	for (auto record : m_permanentObjects)
	{
		...
		while (pRenderObject)
		{
				...
				for (size_t i = 0; i < numItems; i++)
				{
					...
					if (!pri.m_pCompiledObject)
					{
						...
						if (bRequireCompiledRenderObject)
						{
							...
							if (DrxInterlockedCompareExchangePointer((uk *)&pri.m_pCompiledObject, allocatedCompiledObject, nullptr) != nullptr)
							{
							...
							else 
							{
								...
								if (i == 0)
									DrxInterlockedCompareExchangePointer((uk *)&pRenderObject->m_pCompiledObject, pri.m_pCompiledObject, nullptr);
							}
						}
					}
			    ...
			    !pRenderObject->m_bAllCompiledValid)
			{
				...
				DrxInterlockedExchangeAnd(( LONG*)&pRenderObject->m_compiledReadyMask, ~passMask);      // This compiled masks invalid
				if (auto compiledObj = pRenderObject->m_pCompiledObject)
					DrxInterlockedExchangeAnd(( LONG*)&compiledObj->m_compiledFlags, ~compilationOptions);
...
{
	DRX_PROFILE_FUNCTION_ARG(PROFILE_RENDERER, m_name.c_str())
	...
	for (const auto &compilationData : m_permanentRenderObjectsToCompile)
	{
		...
		for (i32 i = 0, num = items.size(); i < num; i++)
		{
			auto& pri = items[i];
			if (!pri.m_pCompiledObject)
				continue;
		...
		if (bAllCompiled)
		{
			DrxInterlockedExchangeOr(( LONG*)&pRenderObject->m_compiledReadyMask, passMask);
		}
		else
		{
			DrxInterlockedExchangeAnd(( LONG*)&pRenderObject->m_passReadyMask, ~passMask); // reset passReadyMask if compilation failed
	...
	i32 numTempObjects = m_temporaryCompiledObjects.size();
	...
	DrxInterlockedAdd(&SRenderStatistics::Write().m_nModifiedCompiledObjects, numObjects);
	DrxInterlockedAdd(&SRenderStatistics::Write().m_nTempCompiledObjects, numTempObjects);
#endif
...
{
	for (i32 i = 0; i < m_numUsedClientPolygons; ++i)
	{
				...
				for (EEfResTextures texType = EFTT_DIFFUSE; texType < EFTT_MAX; texType = EEfResTextures(texType + 1))
				{
					if (SEfResTexture* pTex = pShaderResources->m_Textures[texType])
					{
						...
						if (pTex->m_Sampler.Update())
						{
							if (previousTex)
							{
								previousTex->RemoveInvalidateCallbacks((uk )pShaderResources);
							}
						}
					}
				}
...
CRenderView::ItemsRange CRenderView::GetShadowItemsRange(ShadowMapFrustum* pFrustum, i32 nFrustumSide)
{
...
CRenderView::RenderItems& CRenderView::GetShadowItems(ShadowMapFrustum* pFrustum, i32 nFrustumSide)
{
...
void CRenderView::Job_PostWrite()
{
	...
	DrxAutoLock<DrxCriticalSectionNonRecursive> lock(m_lock_PostWrite);
	...
	for (i32 renderList = 0; renderList < listCount; renderList++)
	{
		...
		if (!renderItems.empty())
		{
			...
			gEnv->pJobUpr->AddLambdaJob("SortRenderItems", lambda_job, JobUpr::eRegularPriority, &m_jobstate_Sort);
			//lambda_job();
		}
	}
...
struct SCompareRendItemSelectionPass
{
	bool operator()(const SRendItem& a, const SRendItem& b) const
	{
		...
		u8 bAHighlightSelect = (a.pCompiledObject->m_pRO->m_editorSelectionID & 0x3);
		u8 bBHighlightSelect = (b.pCompiledObject->m_pRO->m_editorSelectionID & 0x3);
...
void CRenderView::Job_SortRenderItemsInList(ERenderListID list)
{
	...
	if (m_viewType == eViewType_Shadow)
	{
		...
		if (m_shadows.m_pShadowFrustumOwner)
		{
			...
			DRX_ASSERT(side >= 0 && side < OMNI_SIDES_NUM);
...
i32 CRenderView::FindRenderListSplit(ERenderListID nList, u32 objFlag)
{
	FUNCTION_PROFILER_RENDERER();

	DRX_ASSERT_MESSAGE(CRenderer::CV_r_ZPassDepthSorting == 1, "RendItem sorting has been overwritten and are not sorted by ObjFlags, this function can't be used!");;
	DRX_ASSERT_MESSAGE(!(nList == EFSLIST_TRANSP_BW || nList == EFSLIST_TRANSP_AW || nList == EFSLIST_TRANSP_NEAREST || nList == EFSLIST_HALFRES_PARTICLES), "The requested list isn't sorted by ObjFlags!");
	DRX_ASSERT_MESSAGE(objFlag & FOB_MASK_AFFECTS_MERGING, "The requested objFlag isn't used for sorting!");

	// Binary search, assumes that list is sorted by objFlag
	auto& renderItems = GetRenderItems(nList);

	i32 first = 0;
	i32 last = renderItems.size() - 1;
...
{
	DRX_ASSERT(frustum.pShadowsView->GetFrameId() == GetFrameId());
...
//////////////////////////////////////////////////////////////////////////
CRenderView::ShadowFrustumsPtr& CRenderView::GetShadowFrustumsForLight(i32 lightId)
{
...
void CRenderView::CheckAndScheduleForUpdate(const SShaderItem& shaderItem) threadsafe
{
	...
	if (pSR && pShader)
	{
		...
		{
			if (DrxInterlockedExchange(( LONG*)&pSR->m_nUpdateFrameID, (u32)m_frameId) != (u32)m_frameId)
			{
...
//////////////////////////////////////////////////////////////////////////
u16 CRenderView::PushFogVolumeContribution(const ColorF& fogVolumeContrib, const SRenderingPassInfo& passInfo) threadsafe
{
	const size_t maxElems((1 << (sizeof(u16) * 8)) - 1);
	...
	assert(nIndex <= (u16)-1); // Beware! Casting from u32 to u16 may loose top bits
	return static_cast<u16>(nIndex);
}

//////////////////////////////////////////////////////////////////////////
void CRenderView::GetFogVolumeContribution(u16 idx, ColorF& rColor) const
{
...
struct SCompareFrustumsByLightIds
{
	bool operator()(const SShadowFrustumToRender& rA, const SShadowFrustumToRender& rB) const
	{
		...
		else
		{
			...
			{
				return i32(rA.pFrustum->m_eFrustumType) < i32(rB.pFrustum->m_eFrustumType);
			}
			else
			{
...
void CRenderView::SShadows::AddNearestCaster(CRenderObject* pObj, const SRenderingPassInfo& passInfo)
{
	if (pObj->m_pRenderNode)
	{
		...
		DRX_ASSERT(pObj->m_pRenderNode->GetRenderNodeType() != eERType_MovableBrush ||
	...
	if (pNearestFrustum)
	{
		...
		for (auto& fr : m_renderFrustums)
		{
			if (fr.pFrustum->m_eFrustumType == ShadowMapFrustum::e_GsmDynamic && (fr.pLight->m_Flags & DLF_SUN))
			{
				...
				DRX_ASSERT(pShadowsView->m_usageMode == IRenderView::eUsageModeReading);

				pShadowsView->m_shadows.m_nearestCasterBoxes.CoalesceMemory();
				for (i32 i = 0; i < pShadowsView->m_shadows.m_nearestCasterBoxes.size(); ++i)
...
TRect_tpl<u16> CRenderView::ComputeResolveViewport(const AABB &aabb, bool forceFullscreenUpdate) const
{
	// TODO: Re-evaluate camera selection for multiviewport rendering
	const auto& camera = GetCamera(GetCurrentEye());
	const auto& viewport = GetViewport();

	TRect_tpl<u16> resolveViewport;
	...
	{
		resolveViewport.Min.x = static_cast<u16>(viewport.x);
		resolveViewport.Min.y = static_cast<u16>(viewport.y);
		resolveViewport.Max.x = static_cast<u16>(viewport.width);
		resolveViewport.Max.y = static_cast<u16>(viewport.height);
	}
