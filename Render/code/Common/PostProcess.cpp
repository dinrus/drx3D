...
void CParamInt::SetParam(float fParam, bool bForceValue)
{
	...
	pThreadSafeData->nParam = static_cast<i32>(fParam);
	pThreadSafeData->bSetThisFrame = true;
}
...
static void SetPostEffectParamF(IConsoleCmdArgs* pArgs)
{
	...
	if (pArgs->GetArgCount() > 3)
	{
		i32k iForceValue = (i32)atoi(pArgs->GetArg(3));
		if (iForceValue != 0)
		{
			bForceValue = true;
		}
	...
	tukk szPostEffectParamName = pArgs->GetArg(1);
...
EPostEffectID CPostEffectsMgr::GetEffectID(tukk pEffectName)
{
...
u32 CPostEffectsMgr::CRC32Reflect(u32 ref, char ch)
{
	u32 value = 0;
...
u32 CPostEffectsMgr::GetCRC(tukk pszName)
{
	if (!pszName)
	{
	...
	tukk szPtr = pszName;
	u32 ulCRC = 0xffffffff; // Start out with all bits set high.
	u8 c;
...
CEffectParam* CPostEffectsMgr::GetByName(tukk pszParam)
{
	DRX_ASSERT_MESSAGE(pszParam, "mfGetByName: null FX name");

	u32 nCurrKey = GetCRC(pszParam);

	i32 nThreadID = gRenDev->m_pRT ? gRenDev->m_pRT->GetThreadList() : 0;
...
float CPostEffectsMgr::GetByNameF(tukk pszParam)
{
...
Vec4 CPostEffectsMgr::GetByNameVec4(tukk pszParam)
{
	CEffectParam* pParam = GetByName(pszParam);
	if (pParam)
	{
...
i32 CPostEffectsMgr::SortEffectsByID(const CPostEffect* p1, const CPostEffect* p2)
{
...
i32 CParamTexture::Create(tukk pszFileName)
{
	if (!pszFileName)
	{
		return 0;
	...
	pThreadSafeData->bSetThisFrame = true;
	DRX_ASSERT_MESSAGE(pThreadSafeData->pTexParam, "CParamTexture.Create: texture not found!");

	return 1;
}
tukk CParamTexture::GetParamString() const
{
	i32k threadID = gRenDev->m_pRT ? gRenDev->m_pRT->GetThreadList() : 0;
