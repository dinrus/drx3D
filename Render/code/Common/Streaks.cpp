...
void Streaks::Load(IXmlNode* pNode)
{
	...
	if (pStreakNode)
	{
		...
		i32 nStreakCount(m_nStreakCount);
...
void Streaks::UpdateMeshes()
{
	...
	m_indexBuffer = gcpRendD3D->m_DevBufMan.Create(BBT_INDEX_BUFFER, BU_STATIC, m_meshIndices.size() * sizeof(u16));
	gcpRendD3D->m_DevBufMan.UpdateBuffer(m_indexBuffer, &m_meshIndices[0], m_meshIndices.size() * sizeof(u16));
...
bool Streaks::PreparePrimitives(const SPreparePrimitivesContext& context)
{
	...
	// update shared constant buffer
	{
		...
		auto constants = firstPrimitive.GetConstantUpr().BeginTypedConstantUpdate<SShaderParams>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex | EShaderStage_Pixel);
		...
		firstPrimitive.GetConstantUpr().EndTypedConstantUpdate(constants);
	}
	...
	{
		static CDrxNameTSCRC techName("Streaks");
