...
SResourceContainer* CBaseResource::GetResourcesForClass(const CDrxNameTSCRC& className)
{
...
CBaseResource* CBaseResource::GetResource(const CDrxNameTSCRC& className, i32 nID, bool bAddRef)
{
	...
	i32 nIndex = RListIndexFromId(nID);

	//assert(pRL->m_RList.size() > nID);
	if (nIndex >= (i32)pRL->m_RList.size() || nIndex < 0)
		return NULL;
...
CBaseResource* CBaseResource::GetResource(const CDrxNameTSCRC& className, const CDrxNameTSCRC& Name, bool bAddRef)
{
	FUNCTION_PROFILER_RENDER_FLAT
	DrxAutoReadLock<DrxRWLock> lock(s_cResLock);
...
bool CBaseResource::Register(const CDrxNameTSCRC& className, const CDrxNameTSCRC& Name)
{
	DrxAutoWriteLock<DrxRWLock> lock(s_cResLock);
	...
	if (pRL->m_AvailableIDs.size())
	{
		...
		assert(nIndex < (i32)pRL->m_RList.size());
...
void SResourceBinding::AddInvalidateCallback(uk pCallbackOwner, SResourceBindPoint bindPoint, const SResourceBinding::InvalidateCallbackFunction& callback) threadsafe const
{
	switch (type)
	{
...
void SResourceBinding::RemoveInvalidateCallback(uk pCallbackOwner, SResourceBindPoint bindPoint) threadsafe const
{
	switch (type)
	{
		case EResourceType::ConstantBuffer:                                                                       break;
...
void CResourceBindingInvalidator::AddInvalidateCallback(uk listener, const SResourceBindPoint bindPoint, const SResourceBinding::InvalidateCallbackFunction& callback) threadsafe
{
#if !DRX_PLATFORM_ORBIS || defined(__GXX_RTTI)
	DRX_ASSERT(callback.target<SResourceBinding::InvalidateCallbackSignature*>() != nullptr);
#endif
...
#if !DRX_PLATFORM_ORBIS || defined(__GXX_RTTI)
	DRX_ASSERT(*callback.target<SResourceBinding::InvalidateCallbackSignature*>() == *bindpoint_it->callback.target<SResourceBinding::InvalidateCallbackSignature*>());
#endif
}

void CResourceBindingInvalidator::RemoveInvalidateCallbacks(uk listener, const SResourceBindPoint bindPoint) threadsafe
{
...
void CResourceBindingInvalidator::InvalidateDeviceResource(CGpuBuffer* pBuffer, u32 dirtyFlags) threadsafe
{
//	pBuffer->AddRef();
	InvalidateDeviceResource(UResourceReference(pBuffer), dirtyFlags);
	DRX_ASSERT_MESSAGE(!(dirtyFlags & eResourceDestroyed) || (CountInvalidateCallbacks() == 0), "CGpuBuffer %s is destroyd but the invalidation callbacks haven't been removed!", "Unknown" /*pBuffer->GetName()*/);
//	pBuffer->Release();
}

void CResourceBindingInvalidator::InvalidateDeviceResource(CTexture* pTexture, u32 dirtyFlags) threadsafe
{
	pTexture->AddRef();
	InvalidateDeviceResource(UResourceReference(pTexture), dirtyFlags);
	DRX_ASSERT_MESSAGE(!(dirtyFlags & eResourceDestroyed) || (CountInvalidateCallbacks() == 0), "CTexture %s is destroyd but the invalidation callbacks haven't been removed!", pTexture->GetName());
	pTexture->Release();
}
...
void CResourceBindingInvalidator::InvalidateDeviceResource(UResourceReference pResource, u32 dirtyFlags) threadsafe
{
	m_invalidationLock.WLock();
