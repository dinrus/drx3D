...
bool CREMeshImpl::mfUpdate(InputLayoutHandle eVertFormat, i32 Flags, bool bTessellation)
{
...
uk CREMeshImpl::mfGetPointer(ESrcPointer ePT, i32* Stride, EParamType Type, ESrcPointer Dst, i32 Flags)
{
...
bool CREMeshImpl::GetGeometryInfo(SGeometryInfo& geomInfo, bool bSupportTessellation)
{
	...
	if (!mfCheckUpdate(geomInfo.eVertFormat, VSM_MASK, (u16)gRenDev->GetRenderFrameID(), bSupportTessellation))
		return false;
...
CREMesh::~CREMesh()
{
	//i32 nFrameId = gEnv->pRenderer->GetFrameID(false); { char buf[1024]; drx_sprintf(buf,"~CREMesh: %p : frame(%d) \r\n", this, nFrameId); OutputDebugString(buf); }
}
