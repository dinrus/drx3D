...
#include <drx3D/Render/RendElements/ChromaticRing.h>
...
ChromaticRing::ChromaticRing(tukk name)
	: COpticsElement(name)
...
void ChromaticRing::Load(IXmlNode* pNode)
{
	...
	if (pCameraOrbsNode)
	{
		...
		i32 nPolyComplexity(m_nPolyComplexity);
...
bool ChromaticRing::PreparePrimitives(const SPreparePrimitivesContext& context)
{
	...
	for (i32 i = 0; i < DRX_ARRAY_COUNT(pPrimitives); ++i)
	{
		static CDrxNameTSCRC techChromaticRing("ChromaticRing");
