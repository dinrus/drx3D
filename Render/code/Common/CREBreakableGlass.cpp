...
#include <drx3D/Render/RendElements/PolygonMath2D.h>
#include <drx3D/Render/RendElements/SpatialHashGrid.h>
...
// Statics
CDrxNameR CREBreakableGlass::s_ImpactDecalParamName("");
...
// #ifndef RELEASE
//  #define GLASS_FUNC_PROFILER			DRX_PROFILE_FUNCTION(PROFILE_GLASS)
...
// Error logging
#ifndef RELEASE
	#define LOG_GLASS_ERROR(...) DrxLogAlways("[BreakGlassSystem Error]: " __VA_ARGS__)
#else
...
#endif
{
	...
	DRX_ASSERT(POLY_ARRAY_SIZE == sizeTestArray1.max_size());
	DRX_ASSERT(POLY_ARRAY_SIZE == sizeTestArray2.max_size());
#endif
	...
	SetHashGridSize(Vec2Constants<float>::fVec2_One);
...
bool CREBreakableGlass::InitialiseRenderElement(const SBreakableGlassInitParams& params)
{
	...
	if (m_pHashGrid)
	{
		DRX_ASSERT_MESSAGE(0, "Glass render element initialised twice.");
...
void CREBreakableGlass::Update(SBreakableGlassUpdateParams& params)
{
	...
	{
		u32k activeState = m_fragsActive & ~m_fragsFree;
		u32 fragBit = 1;
	...
	if (params.m_pDeadPhysFrags && !params.m_pDeadPhysFrags->empty())
	{
		...
		{
			u16k fragData = params.m_pDeadPhysFrags->at(i);
			u8k buffId = fragData & 0xFF;
			u8k fragId = (fragData >> 8) & 0xFF;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::PlayShatterEffect(u8k fragIndex)
{
	if (!s_pCVars || s_pCVars->m_particleFXEnable > 0)
	{
		if (m_glassParams.pShatterEffect && !m_impactParams.empty())
		{
			...
			i32k numFragPts = frag.m_outlinePts.Count();
			...
			if (numFragPts >= 3)
			{
				...
				m_glassParams.pShatterEffect->Spawn(ParticleLoc(worldPos, dir), &spawnParams);
			}
		}
	}
}//-------------------------------------------------------------------------------------------------
...
void CREBreakableGlass::ApplyImpactToGlass(const SGlassImpactParams& params)
{
	...
	if (!m_shattered)
	{
		...
		if (CheckForGlassBreak(breakThreshold, excessEnergy))
		{
			...
			if (currImpact.speed > GLASSCFG_MIN_DRX3D_SPEED)
			{
				currImpact.impulse = max(currImpact.impulse, GLASSCFG_PLANE_SPLIT_IMPULSE * GLASSCFG_MIN_DRX3D_HOLE_SCALE);
			}
			...
			if (ApplyImpactToFragments(currImpact))
			{
				...
				u32k stableFrags = m_fragsActive & ~(m_fragsLoose | m_fragsFree);
...
void CREBreakableGlass::ApplyExplosionToGlass(const SGlassImpactParams& params)
{
	...
	if (!m_shattered)
	{
		...
		u32k validState = m_fragsActive & ~(m_fragsLoose | m_fragsFree);
		u32 fragBit = 1;
...
//--------------------------------------------------------------------------------------------------
uint CREBreakableGlass::GenerateCracksFromImpact(u8k parentFragIndex, const SGlassImpactParams& impact)
{
	GLASS_FUNC_PROFILER
...
float CREBreakableGlass::GetImpactRadius(const SGlassImpactParams& impact)
{
	...
	const float minImpulse = GLASSCFG_PLANE_SPLIT_IMPULSE * GLASSCFG_MIN_DRX3D_HOLE_SCALE;
...
//--------------------------------------------------------------------------------------------------
float CREBreakableGlass::GetDecalRadius(u8k impactIndex)
{
	// Get impact size multipliers
	float minImpactSpeed = s_pCVars ? s_pCVars->m_minImpactSpeed : GLASSCFG_MIN_DRX3D_SPEED;
...
//--------------------------------------------------------------------------------------------------
bool CREBreakableGlass::FindImpactFragment(const SGlassImpactParams& impact, u8& fragIndex)
{
	GLASS_FUNC_PROFILER
	...
	if (m_pHashGrid)
	{
		...
		u32k cellIndex = m_pHashGrid->HashPosition(impact.x, impact.y);
		...
		if (impactFrags && !impactFrags->empty())
		{
			...
			u8k* pImpactFrags = impactFrags->begin();

			for (uint i = 0; i < numImpactFrags; ++i)
			{
				// Get hashed data
				u8k ownerFrag = pImpactFrags[i];
				DRX_ASSERT_MESSAGE(ownerFrag < GLASSCFG_FRAGMENT_ARRAY_SIZE, "Invalid frag index");
...
{
	DRX_ASSERT_MESSAGE(!m_freeFragIndices.empty(), "Tried to add glass fragment, but none free.");
	...
	if (!m_freeFragIndices.empty())
	{
		...
		// Re-initialise it
		m_lastFragBit = (u32)1 << m_lastFragIndex;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::RemoveFragment(u8k fragIndex)
{
	GLASS_FUNC_PROFILER

	u32k fragBit = (u32)1 << fragIndex;
	u32k validState = m_fragsActive & ~m_fragsFree;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::RebuildChangedFragment(u8k fragIndex)
{
	SGlassFragment& frag = m_frags[fragIndex];
	u32k fragBit = (u32)1 << fragIndex;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::GenerateSubFragments(u8k parentFragIndex, const SGlassImpactParams& impact)
{
	GLASS_FUNC_PROFILER
	...
	i32 numUsedCrackTrees = fragIntersectPts.size();
	DRX_ASSERT_MESSAGE(numUsedCrackTrees > 0, "Should always generate something, as it's forced");
	...
	for (i32 i = 0; i < numUsedCrackTrees; ++i)
	{
		// Resizing the vector, so need to re-grab parent fragment pointer
		SGlassFragment& parentFrag = m_frags[parentFragIndex];
		i32 parentOutlineSize = parentFrag.m_outlinePts.Count();

		// Calculate number of parent outline sections to add
		i32 nextTreeIndex = i + 1 < numUsedCrackTrees ? i + 1 : 0;
		i32 outlineCount = fragIntersectPts[nextTreeIndex] - fragIntersectPts[i];
		...
		i32k firstTreeSize = m_radialCrackTrees[i].Count();
		i32k secondTreeSize = m_radialCrackTrees[nextTreeIndex].Count() - 1;

		i32k fullFragSize = firstTreeSize + outlineCount + secondTreeSize;
		...
		if (fullFragSize >= 3 && fullFragSize <= POLY_ARRAY_SIZE)
		{
			...
			if (outlineCount > 0)
			{
				for (i32 j = 0, k = fragIntersectPts[i] + 1; j < outlineCount; ++j, ++k)
				{
			...
			i32k secondTreeOffset = firstTreeSize + outlineCount;

			for (i32 j = 0, k = secondTreeSize; j < secondTreeSize; ++j, --k)
			{
	...
	u8k* pStableSubFrags = stableSubFrags.begin();
	i32k numStableSubFrags = stableSubFrags.size();
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::GenerateSubFragmentCracks(u8k parentFragIndex, const SGlassImpactParams& impact, TRadialCrackArray& fragIntersectPts)
{
	GLASS_FUNC_PROFILER
	...
	i32k numFragPts = fragOutline.Count();

	// Generate cracks from current impact pt
	fragIntersectPts.clear();
	i32k numRadialCracks = GenerateCracksFromImpact(parentFragIndex, impact);
	...
	for (i32 i = 0; i < numRadialCracks; ++i)
	{
		...
		i32k treeSize = m_radialCrackTrees[i].Count();
		Vec2** pRadialPts = &m_radialCrackTrees[i][0];

		for (i32 j = 0; j < treeSize - 1; ++j)
		{
			...
			for (i32 k = 0; k < numFragPts; ++k)
			{
				i32k nextIndex = k + 1 < numFragPts ? k + 1 : 0;
			...
			DRX_ASSERT_MESSAGE(found, "Crack line leaves fragment but did not find intersection point.");
...
//--------------------------------------------------------------------------------------------------
bool CREBreakableGlass::CreateFragmentFromOutline(const Vec2* pOutline, i32k outlineSize, u8k parentFragIndex, const bool forceLoose)
{
	GLASS_FUNC_PROFILER
	bool success = false;
	...
	if (pOutline && outlineSize >= 3)
	{
		...
		if (outlineSize <= POLY_ARRAY_SIZE)
		{
			...
			if (pSubFrag)
			{
				u8k fragIndex = m_lastFragIndex;
				u32k fragBitIndex = m_lastFragBit;
				...
				i32k numPts = pSubFrag->m_outlinePts.Count();
				...
				if (pSubFrag->m_area > GLASSCFG_MIN_VALID_FRAG_AREA)
				{
					...
					u8k numFrags = (u8)m_totalNumFrags;
...
{
	i32k MinLooseFragDepth = 3;
	i32k LooseFragDepthRange = 3; // Range is 3 -> 6
	const float EdgeStableFragSizeMul = 1.0f;

	bool isLoose = false;
	i32k fragDepth = frag.m_depth;

	// Test depth test against randomised range
	if (fragDepth >= MinLooseFragDepth)
	{
		i32k looseFragDepth = drx_random(0, LooseFragDepthRange - 1) + MinLooseFragDepth;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::RemoveFragmentConnections(u8k fragIndex, PodArray<u8>& connections)
{
	GLASS_FUNC_PROFILER
	...
	while (!connections.IsEmpty())
	{
		...
		u8k neighbour = connections.back();
		connections.DeleteLast();

		// Delete from neighbour's outgoing connection list
		//	- Manually doing delete search code here as u8
		//		template gives ambiguous function signatures
		PodArray<u8>& neighbourOutConns = m_frags[neighbour].m_outConnections;
		u8k* pNeighbourOutConns = neighbourOutConns.begin();
		i32k numOutConns = neighbourOutConns.Count();

		for (i32 i = 0; i < numOutConns; ++i)
		{
		...
		PodArray<u8>& neighbourInConns = m_frags[neighbour].m_inConnections;
		u8k* pNeighbourInConns = neighbourInConns.begin();
		i32k numInConns = neighbourInConns.Count();

		for (i32 i = 0; i < numInConns; ++i)
		{
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::ConnectSubFragments(u8k* pSubFrags, u8k numSubFrags)
{
	if (pSubFrags && numSubFrags > 1)
	{
		// All solid fragments just created may be connected, always in a circular fashion
		for (i32 i = 0, j = 1; i < numSubFrags; ++i, ++j)
		{
			j = (j == numSubFrags) ? 0 : j;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::ReplaceFragmentConnections(u8k parentFragIndex, u8k* pSubFrags, u8k numSubFrags)
{
	// Check for potential new connections between parent neighbours and new sub-fragments
	if (pSubFrags && numSubFrags > 0)
	{
		PodArray<u8>& neighbours = m_frags[parentFragIndex].m_inConnections;
		u8k* pNeighbours = neighbours.begin();
		i32k numNeighbours = neighbours.Count();

		for (i32 i = 0; i < numNeighbours; ++i)
		{
			for (i32 j = 0; j < numSubFrags; ++j)
			{
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::ConnectFragmentPair(u8k fragAIndex, u8k fragBIndex)
{
	GLASS_FUNC_PROFILER
...
void CREBreakableGlass::FindLooseFragments(TGlassPhysFragmentArray* pPhysFrags, TGlassPhysFragmentInitArray* pPhysFragsInitData)
{
	...
	u32k stableState = m_fragsActive & ~(m_fragsLoose | m_fragsFree);
	...
	if (!m_shattered)
	{
		...
		for (uint i = 0; i < numAnchors; ++i)
		{
			...
			if (FindImpactFragment(anchorParams, anchorFrag))
			{
				...
				u32k anchorFragBit = (u32)1 << anchorFrag;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::TraverseStableConnection(u8k fragIndex, TFragIndexPodArray& stableFrags)
{
	GLASS_FUNC_PROFILER
	...
	if (stableFrags.find(fragIndex) < 0)
	{
		...
		PodArray<u8>& connections = m_frags[fragIndex].m_outConnections;
		u8* pConnections = connections.begin();
		i32k numConnections = connections.Count();
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::PrePhysicalizeLooseFragment(TGlassPhysFragmentArray* pPhysFrags, TGlassPhysFragmentInitArray* pPhysFragsInitData, u8k fragIndex, const bool dampenVel, const bool noVel)
{
	GLASS_FUNC_PROFILER
	...
	if (physFragSize >= minPhysFragSize)
	{
		...
		i32k numPhysFrags = pPhysFrags ? pPhysFrags->size() : 0;
		i32k numPhysFragsInitData = pPhysFragsInitData ? pPhysFragsInitData->size() : 0;
		DRX_ASSERT(numPhysFrags == numPhysFragsInitData);
#endif
		...
		else
		{
			...
			{
				// Simple piece drop
				physFragsInitData.m_impulse = Vec3Constants<float>::fVec3_Zero;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::HashFragment(u8k fragIndex, const bool remove)
{
#if GLASSCFG_USE_HASH_GRID
	// Get fragment
	const SGlassFragment& frag = m_frags[fragIndex];
	...
	// Hash polygon
	i32k numTriIndices = frag.m_triInds.Count();
	u8k* pIndices = frag.m_triInds.begin();

	for (i32 j = 0; j < numTriIndices; j += 3)
	{
...
struct STriEdge
{
	STriEdge(const Vec2& a, const Vec2& b)
	{
		...
		pt[0].x = (float)__fsel(abMin, a.x, b.x);
		pt[0].y = (float)__fsel(abMin, a.y, b.y);
		pt[1].x = (float)__fsel(abMax, a.x, b.x);
		pt[1].y = (float)__fsel(abMax, a.y, b.y);
...
void CREBreakableGlass::HashFragmentTriangle(const Vec2& a, const Vec2& b, const Vec2& c,
                                             u8k& triFrag, const bool removeTri)
{
#if GLASSCFG_USE_HASH_GRID
	GLASS_FUNC_PROFILER
                                                 ...
                                                 u8k& triFrag, const bool removeTri)
{
	...
	uint numYSteps = static_cast<i32>(fabs_tpl(b.height - m_hashCellHalfSize.y) * m_hashCellInvSize.y) + 1;
	...
	for (uint y = 0; y <= numYSteps; ++y, yOffset += m_hashCellSize.y)
	{
		...
		// Pre-hash first cell
		u32 cellOffset = m_pHashGrid->HashPosition(span.pt[0], yOffset);
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::DeactivateFragment(u8k index, u32k bit)
{
	// Mark as inactive
	m_fragsActive &= ~bit;
...
void CREBreakableGlass::RebuildAllFragmentGeom()
{
	...
	if (m_geomDirty && !m_geomRebuilt && m_dirtyGeomBufferCount == 0)
	{
		...
		m_numDecalImpacts = (u8)m_impactParams.size();

		// Rebuilt, so ready for buffer copy/update
		MemoryBarrier();
		m_geomDirty = false;
		u8 numDirtyBuffers = 1;
...
void CREBreakableGlass::ProcessFragmentGeomData()
{
	...
	for (uint i = 0; i < GLASSCFG_MAX_NUM_PHYS_FRAGMENTS; ++i)
	{
		u8k fragId = m_fragGeomBufferIds[i].m_fragId;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::BuildFragmentTriangleData(u8k fragIndex, uint& vertOffs, uint& indOffs, const EGlassSurfaceSide side, i32k subFragID)
{
	GLASS_FUNC_PROFILER

	// Get fragment data
	const PodArray<Vec2>& vertices = m_frags[fragIndex].m_outlinePts;
	const PodArray<u8>& indices = m_frags[fragIndex].m_triInds;

	i32k vertCount = vertices.Count();
	i32k indCount = indices.Count();
	...
	i32k vertOffset = vertOffs;
	i32k indOffset = indOffs;
	...
	u8k randIndex = fragIndex & 7; // Optimized "index % 8"
	...
	const Vec2& uvOffset = isPhysFrag ? m_frags[fragIndex].m_center : Vec2Constants<float>::fVec2_Zero;
...
//--------------------------------------------------------------------------------------------------
void CREBreakableGlass::BuildFragmentOutlineData(u8k fragIndex, uint& vertOffs, uint& indOffs, i32k subFragID)
{
	GLASS_FUNC_PROFILER
	...
	i32k numPts = pts.Count();

	i32k vertStep = 2;
	i32k indStep = 6;

	i32k vertCount = numPts * vertStep;
	i32k indCount = numPts * indStep;
	...
	for (i32 i = 0, j = 0; i < vertCount; i += vertStep, ++j)
	{
	...
	for (i32 i = 0, j = 0; i < indCount; i += indStep, j += vertStep)
	{
		// Get cyclic vert indices
		i32k currIndex = vertOffset + j;
		i32k nextIndex = vertOffset + (j + vertStep >= vertCount ? 0 : j + vertStep);

		// Create index data
		i32k offset = indOffset + i;
...
void CREBreakableGlass::GenerateDefaultPlaneGeom()
{
	...
	// Initialise fragment
	u8k noParent = (u8) - 1; // Forcing a value we *know* is invalid
...
void CREBreakableGlass::GenerateImpactGeom(const SGlassImpactParams& impact)
{
	...
	// Add new quad
	i32 offset = m_impactLineList.Count();
	...
	if (radius > 0.0f)
	{
		...
		i32k doubleNumSides = IMPACT_NUM_IMPULSE_SIDES * 2;
		...
		for (i32 i = 0; i < doubleNumSides; i += 2, angle += angleInc)
		{
