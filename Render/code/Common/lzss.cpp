...
i32 match_position, match_length,                          /* of longest match.  These are
...
void InsertNode(i32 r, u8 text_buf[])
...
void DeleteNode(i32 p)  /* deletes node p from tree */
{
	i32 q;
...
inline i32 getcm(i32& Count, i32 Size, u8*& getcmP)
{
	if (Count++ >= Size)
		return EOF;
	return (i32)*getcmP++;
...
inline void putcm(i32 c, u8*& putcmP)
{
	*putcmP++ = (u8)c;
};

inline bool putcm(i32 c, u8*& putcmP, u8* putcmpPEnd)
{
...
i32 Encodem(u8* getcmP, u8* putcmP, i32 Size)
{
	u8 text_buf[N_SIZE + F_UPPER - 1]; /* ring buffer of size N_SIZE, with extra F_UPPER-1 bytes to facilitate string comparison */
	i32 Count = 0;
	i32 i, c, len, r, s, last_match_length, code_buf_ptr;
	u8 code_buf[17], mask;
	u32 codesize = 0;
...
// Thread safe
bool Decodem(u8* getcmP, u8* putcmP, i32 Size, size_t outBufCapacity) /* Just the reverse of Encode(). */
{
	i32 i, j, k, r, c;
	u32 flags;
	i32 Count = 0;
	u8 text_buf[N_SIZE + F_UPPER - 1]; /* ring buffer of size N_SIZE, with extra F_UPPER-1 bytes to facilitate string comparison */
	u8* putcmpPEnd = putcmP + outBufCapacity;
