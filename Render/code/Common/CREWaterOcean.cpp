...
#include <drx3D/CoreX/Renderer/RendElements/CREWaterOcean.h>
...
u32k passIdUnderWater = CWaterStage::ePass_WaterSurface;
u32k passIdAboveWater = CWaterStage::ePass_CausticsGen;
...
static_assert(i32(ePerInstanceTexture_Count) == i32(CWaterStage::ePerInstanceTexture_Count), "Per instance texture count must be same in water stage to ensure using same resource layout.");
...
struct SCompiledWaterOcean : NoCopy
{
	...
	i32                     m_nVerticesCount;
	i32                     m_nStartIndex;
	i32                     m_nNumIndices;

	DevicePipelineStatesArray m_psoArray;

	u32                    m_bValid           : 1; //!< True if compilation succeeded.
	u32                    m_bHasTessellation : 1; //!< True if tessellation is enabled
	u32                    m_bAboveWater      : 1; //!< True if camera is above the water ocean.
	u32                    m_reserved         : 29;
};
	...
	, m_nIndexSizeof(0)
{
	...
	DRX_ASSERT(m_pRenderTarget->m_nIDInPool >= 0 && m_pRenderTarget->m_nIDInPool < 64);
	...
	{
		if ((i32)CRendererResources::s_CustomRT_2D.Num() <= m_pRenderTarget->m_nIDInPool)
...
bool CREWaterOcean::RequestVerticesBuffer(SVF_P3F_C4B_T2F** pOutputVertices, u8** pOutputIndices, u32 nVerticesCount, u32 nIndicesCount, u32 nIndexSizeof)
{
	...
	DRX_ASSERT(rd->m_pRT->IsMainThread());

	*pOutputVertices = new SVF_P3F_C4B_T2F[nVerticesCount];
	*pOutputIndices = new u8[nIndicesCount * nIndexSizeof];
...
bool CREWaterOcean::SubmitVerticesBuffer(u32 nVerticesCount, u32 nIndicesCount, u32 nIndexSizeof, SVF_P3F_C4B_T2F* pVertices, u8* pIndices)
{
...
void CREWaterOcean::Create(u32 nVerticesCount, SVF_P3F_C4B_T2F* pVertices, u32 nIndicesCount, ukk pIndices, u32 nIndexSizeof)
{
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////
	{
		u32 size = nVerticesCount * sizeof(SVF_P3F_C4B_T2F);
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////
	{
		u32k size = nIndicesCount * m_nIndexSizeof;
...
void CREWaterOcean::FrameUpdate()
{
	...
	if (CTexture::IsTextureExist(pTexture))
	{
		...
		DRX_PROFILE_REGION_WAITING(PROFILE_RENDERER, "update subresource");
...
bool CREWaterOcean::Compile(CRenderObject* pObj,CRenderView *pRenderView, bool updateInstanceDataOnly)
{
	...
	DRX_ASSERT(rd->m_pRT->IsRenderThread());
	...
	DRX_ASSERT(compiledObj.m_pMaterialResourceSet);
	DRX_ASSERT(compiledObj.m_pPerDrawCB);
	DRX_ASSERT(compiledObj.m_pPerDrawRS && compiledObj.m_pPerDrawRS->IsValid());
	DRX_ASSERT(compiledObj.m_vertexStreamSet);
	DRX_ASSERT(compiledObj.m_indexStreamSet);
...
void CREWaterOcean::DrawToCommandList(CRenderObject* pObj, const struct SGraphicsPipelinePassContext& ctx, CDeviceCommandList* commandList)
{
	...
	DRX_ASSERT(ctx.stageID == eStage_Water);
	DRX_ASSERT(ctx.passID == CWaterStage::ePass_WaterSurface || ctx.passID == CWaterStage::ePass_OceanMaskGen);
...
void CREWaterOcean::UpdatePerDrawCB(water::SCompiledWaterOcean& RESTRICT_REFERENCE compiledObj, const CRenderObject& renderObj) const
{
	...
	DrxStackAllocWithSize(water::SPerDrawConstantBuffer, cb, CDeviceBufferUpr::AlignBufferSizeForStreaming);
...
void CREWaterOcean::UpdateVertex(water::SCompiledWaterOcean& compiledObj, i32 primType)
{
