...
#include <drx3D/Eng3D/CGF/CGFContent.h>
...
namespace 
{
	...
	inline i32 GetCurrentThreadID()
	{
		DRX_ASSERT(gRenDev->m_pRT->IsMainThread() || gRenDev->m_pRT->IsRenderThread());
	...
	struct SMeshPool
	{
		...
		uk m_MeshDataMemory;
    uk m_MeshInstanceMemory;
	  DrxCriticalSection m_MeshPoolCS; 
	...
	{
		return AlignedMeshDataSize(nSize, std::min(nAlign, CDeviceBufferUpr::GetBufferAlignmentForStreaming()));
	}

	static uk AllocateMeshDataUnpooled(buffer_size_t nSize, buffer_size_t nAlign = MESH_DATA_DEFAULT_ALIGN)
	{
		return DrxModuleMemalign(nSize, std::min(nAlign, CDeviceBufferUpr::GetBufferAlignmentForStreaming()));
	}

	static void FreeMeshDataUnpooled(uk ptr)
	{
		if (ptr == NULL)
			return;
	...
	//////////////////////////////////////////////////////////////////////////
	static uk AllocateMeshData(buffer_size_t nSize, buffer_size_t nAlign = MESH_DATA_DEFAULT_ALIGN, bool bFlush = false)
	{
		...
		try_again:
			...
			uk ptr = s_MeshPool.m_MeshDataPool->Memalign(nAlign, nSize, "RENDERMESH_POOL");
			if (ptr)
			{
		...
		return DrxModuleMemalign(nSize, nAlign);
	}

	static void FreeMeshData(uk ptr)
	{
		if (ptr == NULL)
			return;
		{
	...
	{
		uk pStorage = AllocateMeshData(sizeof(Type) * nCount, std::max((size_t)MESH_DATA_DEFAULT_ALIGN, (size_t)__alignof(Type)));
		if (!pStorage)
			return NULL;
	...
	static bool IsPoolAllocated(uk ptr)
	{
	...
	static bool InitializePool()
	{
		if (gRenDev->CV_r_meshpoolsize > 0)
		{
			...
			{
				DrxFatalError("render meshpool already initialized");
				return false;
			}
			size_t poolSize = static_cast<size_t>(gRenDev->CV_r_meshpoolsize)*1024U;
			s_MeshPool.m_MeshDataMemory = DrxModuleMemalign(poolSize, 128u);
			if (!s_MeshPool.m_MeshDataMemory)
			{
				DrxFatalError("could not allocate render meshpool");
				return false;
			}

			// Initialize the actual pool
			s_MeshPool.m_MeshDataPool = gEnv->pSystem->GetIMemoryUpr()->CreateGeneralMemoryHeap(
		...
		if (gRenDev->CV_r_meshinstancepoolsize && !s_MeshPool.m_MeshInstancePool)
		{
			...
			s_MeshPool.m_MeshInstanceMemory = DrxModuleMemalign(poolSize, 128u);
			if (!s_MeshPool.m_MeshInstanceMemory) 
			{
				DrxFatalError("could not allocate render mesh instance pool");
				return false; 
			}
      
			s_MeshPool.m_MeshInstancePool = gEnv->pSystem->GetIMemoryUpr()->CreateGeneralMemoryHeap(
	...
	static void ShutdownPool()
	{
		...
		if (s_MeshPool.m_MeshDataMemory)
		{
			DrxModuleMemalignFree(s_MeshPool.m_MeshDataMemory);
    ...
    if (s_MeshPool.m_MeshInstanceMemory)
    {
      DrxModuleMemalignFree(s_MeshPool.m_MeshInstanceMemory);
      s_MeshPool.m_MeshInstanceMemory = NULL; 
    }
	}

  static uk AllocateMeshInstanceData(size_t size, size_t align)
  {
    if (s_MeshPool.m_MeshInstancePool)
    {
      if (uk ptr = s_MeshPool.m_MeshInstancePool->Memalign(align, size, "rendermesh instance data"))
      {
#       if !defined(_RELEASE)
		    AUTO_LOCK(s_MeshPool.m_MeshPoolCS);
...
#       endif
    ...
    return DrxModuleMemalign(size, align);
  }

  static void FreeMeshInstanceData(uk ptr)
  {
    size_t size = 0u;
...
#define alignup16(value)						alignup(16, value)
  ...
  static inline bool VidMemPushDown(uk pDst, ukk pSrc, size_t nSize, uk pDst1=NULL, ukk pSrc1=NULL, size_t nSize1=0, i32 cachePosStride = -1, uk pFP16Dst=NULL, u32 nVerts=0)
{
}
	#define ASSERT_LOCK
...
DrxCriticalSection CRenderMesh::m_sLinkLock;
...
i32 CRenderMesh::Release()
{
	long refCnt = DrxInterlockedDecrement(&m_nRefCounter);
# if !defined(_RELEASE)
	if (refCnt < 0)
	{
		DRX_ASSERT_MESSAGE(refCnt>=0, "CRenderMesh::Release() called so many times on rendermesh that refcount became negative");
	}
# endif
	if (refCnt == 0)
	{
...
#   if !defined(_RELEASE)
		...
		{
			DRX_ASSERT_MESSAGE(refCnt > 0, "CRenderMesh::Release() mesh already in the garbage list (double delete pending)");
		}
#   endif
		m_nFlags |= FRM_RELEASED;
		i32 nFrame = gRenDev->m_pRT ? GetCurrentFrameID() : gEnv->nMainFrameID;
...
#ifdef MESH_TESSELLATION_RENDERER
	m_adjBuffer.Create(0, sizeof(u16), DXGI_FORMAT_R16_UNORM, CDeviceObjectFactory::BIND_SHADER_RESOURCE, nullptr);
#endif
...
CRenderMesh::CRenderMesh (tukk szType, tukk szSourceName, bool bLock)
{
  memset(m_VBStream, 0x0, sizeof(m_VBStream));
...
void CRenderMesh::Cleanup()
{
	...
	for(i32 i=0; i<m_lstVertexContainerUsers.Count(); i++)
	{
...
//				FreeMeshDataUnpooled(m_CreatedBoneIndices[i].pExtraStream);
	...
	for (size_t i=0,end=m_RemappedBoneIndices.size(); i<end; ++i)
	{
		if (m_RemappedBoneIndices[i].refcount && m_RemappedBoneIndices[i].guid != ~0u) 
			DrxLogAlways("remapped bone indices with refcount '%u' still around for '%s 0x%p\n", m_RemappedBoneIndices[i].refcount, m_sSource.c_str(), this);
...
CRenderMesh::~CRenderMesh()
{
	...
	i32 nThreadID = gRenDev->GetRenderThreadID();
	for (i32 i = 0; i < RT_COMMAND_BUF_COUNT; SyncAsyncUpdate(i++))
		;

	// make sure no subset rendermesh job is still running which uses this mesh
	for(i32 j=0;j<RT_COMMAND_BUF_COUNT;++j)
	{
		...
		for( size_t i = 0 ; i < nNumSubSetRenderMeshJobs ; ++i )
		{
			...
			{
				gEnv->pJobUpr->WaitForJob(rSubSetJob.jobState);
...
SMeshStream* CRenderMesh::GetVertexStream(i32 nStream, u32 nFlags)
{
...
uk CRenderMesh::LockVB(i32 nStream, u32 nFlags, i32 nOffset, i32 nVerts, i32 *nStride, bool prefetchIB, bool inplaceCachePos)
{
...
# if !defined(_RELEASE)
	DRX_ASSERT_MESSAGE(m_nThreadAccessCounter, "rendermesh must be locked via LockForThreadAccess() before LockIB/VB is called");
#endif

	// NOTE: When called from a job thread we will use main thread ID
	i32k threadId = gRenDev->m_pRT->IsRenderThread() ? gRenDev->GetRenderThreadID() : gRenDev->GetMainThreadID();
	...
	assert(nVerts <= (i32)m_nVerts);
	if (nVerts > (i32)m_nVerts)
		nVerts = m_nVerts;
	if (nStride)
...
lSysCreate:
		...
		if (!MS->m_pUpdateData)
		{
			u32 nSize = GetStreamSize(nStream);
...
vtx_idx *CRenderMesh::LockIB(u32 nFlags, i32 nOffset, i32 nInds)
{
	FUNCTION_PROFILER_RENDERER();
...
lSysCreate:
		...
		{
			u32 nSize = m_nInds * sizeof(vtx_idx);
			pD = reinterpret_cast<byte*>(AllocateMeshData(nSize));
			if (!pD) return NULL;
...
ILINE void CRenderMesh::UnlockVB(i32 nStream)
{
  MEMORY_SCOPE_CHECK_HEAP();
	SREC_AUTO_LOCK(m_sResLock);
...
void CRenderMesh::UnlockStream(i32 nStream)
{
  MEMORY_SCOPE_CHECK_HEAP();
  UnlockVB(nStream);
	SREC_AUTO_LOCK(m_sResLock);
...
size_t CRenderMesh::SetMesh_Int(CMesh &mesh, i32 nSecColorsSetOffset, u32 flags, const Vec3 *pPosOffset)	
{
  ...
  u32 nVerts = mesh.GetVertexCount();
  u32 nInds = mesh.GetIndexCount();
  vtx_idx *pInds = NULL;
  ...
  //////////////////////////////////////////////////////////////////////////
  u32 numSubsets = mesh.GetSubSetCount();
...
size_t CRenderMesh::SetMesh(CMesh &mesh, i32 nSecColorsSetOffset, u32 flags, const Vec3 *pPosOffset, bool requiresLock)
{
  LOADING_TIME_PROFILE_SECTION;
  MEMORY_SCOPE_CHECK_HEAP();
...
void CRenderMesh::SetSkinningDataVegetation(struct SMeshBoneMapping_uint8 *pBoneMapping)
{
	...
	for (u32 i = 0; i < m_nVerts; i++)
	{
		// get bone IDs
		u16 b0 = pBoneMapping[i].boneIds[0];
		u16 b1 = pBoneMapping[i].boneIds[1];
		u16 b2 = pBoneMapping[i].boneIds[2];
		u16 b3 = pBoneMapping[i].boneIds[3];

		// get weights
		u8k w0 = pBoneMapping[i].weights[0];
		u8k w1 = pBoneMapping[i].weights[1];
		u8k w2 = pBoneMapping[i].weights[2];
		u8k w3 = pBoneMapping[i].weights[3];
...
void CRenderMesh::SetSkinningDataCharacter(CMesh& mesh, u32 flags, struct SMeshBoneMapping_u16 *pBoneMapping, struct SMeshBoneMapping_u16 *pExtraBoneMapping)
{
  MEMORY_SCOPE_CHECK_HEAP();
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMeshType, 0, this->GetTypeName());
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMesh, 0, this->GetSourceName());
  SVF_W4B_I4S *pSkinBuff = (SVF_W4B_I4S *)LockVB(VSF_HWSKIN_INFO, FSL_VIDEO_CREATE);
	...
	if( pSkinBuff == NULL ) 
	{
		DRX_ASSERT_MESSAGE(pSkinBuff, "Skinning buffer allocation failed");
		return; 
	}
	...
	if (pExtraBoneMapping && m_extraBonesBuffer.IsNullBuffer() && m_nVerts)
	{
		...
		{
			// get bone IDs
			u16 b0 = pExtraBoneMapping[i].boneIds[0];
			u16 b1 = pExtraBoneMapping[i].boneIds[1];
			u16 b2 = pExtraBoneMapping[i].boneIds[2];
			u16 b3 = pExtraBoneMapping[i].boneIds[3];

			// get weights
			u8k w0 = pExtraBoneMapping[i].weights[0];
			u8k w1 = pExtraBoneMapping[i].weights[1];
			u8k w2 = pExtraBoneMapping[i].weights[2];
			u8k w3 = pExtraBoneMapping[i].weights[3];
	...
	if (bCreateCSBuffers)
	{
		...
		{
			m_pExtraBoneMapping = reinterpret_cast<SMeshBoneMapping_u16*>(AllocateMeshDataUnpooled(sizeof(SMeshBoneMapping_u16) * m_nVerts));

			memcpy(m_pExtraBoneMapping, pExtraBoneMapping, sizeof(SMeshBoneMapping_u16) * m_nVerts);
		}
...
void CRenderMesh::ComputeSkinningCreateBindPoseAndMorphBuffers(CMesh& mesh)
{
	...
	std::vector<u32> buckets(m_nVerts, 0);
	std::vector<u32> bucketsCounter(m_nVerts, 0);
	std::vector<u32> workBuckets(m_nVerts, 0);

	for (u32 i=0; i<m_nInds; ++i)
		++buckets[mesh.m_pIndices[i]];

	bucketsCounter[0] = 0;
	for (u32 i=1; i<m_nVerts; ++i)
		bucketsCounter[i] = bucketsCounter[i-1] + buckets[i-1];

	std::vector<u32> adjTriangles;
	adjTriangles.resize(m_nInds);

	i32 k=0;
	for (u32 i=0; i<m_nInds; i+=3)
	{
		u32 idx0 = mesh.m_pIndices[i+0];
		u32 idx1 = mesh.m_pIndices[i+1];
		u32 idx2 = mesh.m_pIndices[i+2];
...
void CRenderMesh::ComputeSkinningCreateSkinningBuffers(const SVF_W4B_I4S* pBoneMapping, const SMeshBoneMapping_u16* pExtraBoneMapping)
{
	MEMORY_SCOPE_CHECK_HEAP();
	...
	for (u32 i=0; i<m_nVerts; ++i)
	{
		...
		for (u32 j=0; j<4; ++j)
		{
			u16k index = pBoneMapping[i].indices[j];
			u8k  weight = pBoneMapping[i].weights.bcolor[j];
			if (weight > 0)
			{
		...
		if (pExtraBoneMapping)
		{
			for (u32 j=0; j<4; ++j)
			{
				u16k index = pExtraBoneMapping[i].boneIds[j];
				u8k  weight = pExtraBoneMapping[i].weights[j];
				if (weight > 0)
				{
					skinningVector.push_back(compute_skinning::SSkinning());
					skinningVector.back().weightIndex = (weight << 24 | index);
...
IIndexedMesh *CRenderMesh::GetIndexedMesh(IIndexedMesh *pIdxMesh)
{
  ...
  for(i=0;i<(i32)m_Chunks.size();i++)
  {
		...
		i32k nMatFlags = m_Chunks[i].m_nMatFlags;
		i32k nPhysicalizeType = (nMatFlags & MTL_FLAG_NOPHYSICALIZE)
			? PHYS_GEOM_TYPE_NONE
...
void CRenderMesh::GenerateQTangents()
{
  ...
  // between here and DinrusXAnimation.
  SScopedMeshDataLock _lock(this);
  i32 srcStride = 0;
  uk pSrcD = LockVB(VSF_TANGENTS, FSL_READ, 0, 0, &srcStride);
  if (pSrcD)
  {
    i32 dstStride = 0;
    uk pDstD = LockVB(VSF_QTANGENTS, FSL_VIDEO_CREATE, 0, 0, &dstStride);
    assert(pDstD);
    if (pDstD)
    {
...
void CRenderMesh::CreateChunksSkinned()
{
  ...
  for (i32 i=0; i<arrSrcMats.size(); ++i)
  {
...
i32 CRenderMesh::GetRenderChunksCount(IMaterial * pMaterial, i32 & nRenderTrisCount)
{
  i32 nCount = 0;
  nRenderTrisCount = 0;

  CRenderer *rd = gRenDev;
  u32k ni = (u32)m_Chunks.size();
  for (u32 i=0; i<ni; i++)
  {
...
void CRenderMesh::CopyTo(IRenderMesh *_pDst, i32 nAppendVtx, bool bDynamic, bool fullCopy)
{
	ASSERT_IS_MAIN_THREAD(gRenDev->m_pRT);
	DRX_ASSERT(this != _pDst);
	...
	if (fullCopy)
	{
		...
		for (i = 0; i < VSF_NUM; i++)
		{
			uk pSrcD = LockVB(i, FSL_READ);
			if (pSrcD)
			{
				uk pDstD = pDst->LockVB(i, FSL_VIDEO_CREATE);
				assert(pDstD);
				if (pDstD)
					drxMemcpy(pDstD, pSrcD, GetStreamSize(i), MC_CPU_TO_GPU);
				pDst->UnlockVB(i);
			}
			UnlockVB(i);
		}
		...
		uk pSrcD = LockIB(FSL_READ);
		if (pSrcD)
		{
			uk pDstD = pDst->LockIB(FSL_VIDEO_CREATE);
			assert(pDstD);
			if (pDstD)
				drxMemcpy(pDstD, pSrcD, m_nInds * sizeof(vtx_idx), MC_CPU_TO_GPU);
			pDst->UnlockIB();
		}
...
void CRenderMesh::SetCustomTexID( i32 nCustomTID )
{
  MEMORY_SCOPE_CHECK_HEAP();
...
void CRenderMesh::SetChunk(i32 nIndex, CRenderChunk &inChunk)
{
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMeshType, 0, this->GetTypeName());
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMesh, 0, this->GetSourceName());
  ...
  pRenderChunk->nNumIndices		= std::max<u32>(inChunk.nNumIndices,0);
  pRenderChunk->nFirstVertId	= inChunk.nFirstVertId;
  pRenderChunk->nNumVerts			= std::max<u32>(inChunk.nNumVerts,0);
...
void CRenderMesh::SetChunk(IMaterial *pNewMat, i32 nFirstVertId, i32 nVertCount, i32 nFirstIndexId, i32 nIndexCount, float texelAreaDensity, i32 nIndex)
{
  CRenderChunk chunk;
...
bool CRenderMesh::CreateCachePos(byte *pSrc, u32 nStrideSrc, uint nFlags)
{
...
bool CRenderMesh::CreateCacheUVs(byte *pSrc, u32 nStrideSrc, uint nFlags)
{
...
byte *CRenderMesh::GetPosPtrNoCache(i32& nStride, u32 nFlags, i32 nOffset)
{
  i32 nStr = 0;
  byte *pData = NULL;
...
byte *CRenderMesh::GetPosPtr(i32& nStride, u32 nFlags, i32 nOffset)
{
...
vtx_idx *CRenderMesh::GetIndexPtr(u32 nFlags, i32 nOffset)
{
...
byte *CRenderMesh::GetColorPtr(i32& nStride, u32 nFlags, i32 nOffset)
{
...
byte *CRenderMesh::GetNormPtr(i32& nStride, u32 nFlags, i32 nOffset)
{
...
byte *CRenderMesh::GetUVPtrNoCache(i32& nStride, u32 nFlags, i32 nOffset)
{
...
byte *CRenderMesh::GetUVPtr(i32& nStride, u32 nFlags, i32 nOffset)
{
...
byte *CRenderMesh::GetTangentPtr(i32& nStride, u32 nFlags, i32 nOffset)
{
...
byte *CRenderMesh::GetQTangentPtr(i32& nStride, u32 nFlags, i32 nOffset)
{
...
byte *CRenderMesh::GetHWSkinPtr(i32& nStride, u32 nFlags, i32 nOffset, bool remapped)
{
...
byte *CRenderMesh::GetVelocityPtr(i32& nStride, u32 nFlags, i32 nOffset)
{
...
void CRenderMesh::RT_AllocationFailure(tukk sPurpose, u32 nSize)
{
  SREC_AUTO_LOCK(m_sResLock);
	Cleanup();
	m_nVerts = 0;
...
#if !defined(_RELEASE)
  DrxLogAlways("rendermesh '%s(%s)' suffered from a buffer allocation failure for \"%s\" size %u bytes on thread 0x%" PRI_THREADID, m_sSource.c_str(), m_sType.c_str(), sPurpose, nSize, DrxGetCurrentThreadId());
# endif
}
...
{
	inline bool operator()(i32k i1, i32k i2) const
	{
...
void CRenderMesh::BuildAdjacency(const VertexFormat *pVerts, u32 nVerts, const vtx_idx *pIndexBuffer, uint nTrgs, std::vector<VecUV> &pTxtAdjBuffer)
{
	...
	std::vector<i32> arrSortedVertIDs;
	...
	i32 *_iSortedVerts = &arrSortedVertIDs[0];
	for (i32 iv = 0; iv < nVerts; ++iv)
	...
	std::vector<i32> arrLinkToMaster;
	arrLinkToMaster.resize(nVerts);
	i32 nUniqueVertices = 0;
	for (i32 iv0 = 0; iv0 < nVerts; )
	{
		i32 iMaster = arrSortedVertIDs[iv0];
	...
	std::vector<i32> &arrConnectedTrianglesCount = arrSortedVertIDs;
	for (i32 i = 0; i < arrConnectedTrianglesCount.size(); ++i)
		arrConnectedTrianglesCount[i] = 0;
	i32 *_nConnectedTriangles = &arrConnectedTrianglesCount[0];
	for (i32 it = 0; it < nTrgs; ++it)
	{
		const vtx_idx *pTrg = &pIndexBuffer[it * 3];
		i32 iMasterVertex0 = arrLinkToMaster[pTrg[0]];
		i32 iMasterVertex1 = arrLinkToMaster[pTrg[1]];
		i32 iMasterVertex2 = arrLinkToMaster[pTrg[2]];
	...
	{
		const vtx_idx *pTrg = &pIndexBuffer[it * 3];
		for (i32 ie = 0; ie < 3; ++ie)
		{
			// fix the corner here
			{
				i32 ivCorner = pTrg[ie];
				pTxtAdjBuffer[it * 12 + 9 + ie] = pVerts[arrLinkToMaster[ivCorner]].st;
			}
			// proceed with fixing the edges
			i32 iv0 = pTrg[ie];
			i32 iMasterVertex0 = arrLinkToMaster[iv0];

			i32 iv1 = pTrg[(ie + 1) % 3];
			i32 iMasterVertex1 = arrLinkToMaster[iv1];
			...
			for (i32 i0 = arrFirstConnectedTriangle[iMasterVertex0]; i0 < arrFirstConnectedTriangle[iMasterVertex0 + 1]; ++i0)
			{
				i32 iOtherTriangle = arrConnectedTriangles[i0];
				...
				for (i32 ieOther = 0; ieOther < 3; ++ieOther)
				{
...
bool CRenderMesh::RT_CheckUpdate(CRenderMesh *pVContainer, InputLayoutHandle eVF, u32 nStreamMask, bool bTessellation)
{
...

	{
		...
		for (u32k& guidToDelete : pVContainer->m_DeletedBoneIndices)
		{
...
void CRenderMesh::ReleaseVB(i32 nStream)
{
  UnlockVB(nStream);
...
bool CRenderMesh::CreateVidVertices(i32 nVerts, InputLayoutHandle eVF, i32 nStream)
{
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMeshType, 0, this->GetTypeName());
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMesh, 0, this->GetSourceName());
	SCOPED_RENDERER_ALLOCATION_NAME_HINT( GetSourceName() );
  ...
  i32 nSize = GetStreamSize(nStream, nVerts);
...
bool CRenderMesh::UpdateVidVertices(i32 nStream)
{
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMeshType, 0, this->GetTypeName());
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMesh, 0, this->GetSourceName());
	...
	if (IBStream.m_nID != ~0u && pMS)
	{
		if (m_IBStream.m_pUpdateData)
		{
			...
			if ((m_eVF == EDefaultInputLayouts::P3S_C4B_T2S || m_eVF == EDefaultInputLayouts::P3F_C4B_T2F) && pVertexStream)
			{
				...
				i32 nVerts = _GetNumVerts();
				for (i32 n = 0; n < nTrgs; ++n)
				{
				...
				for (i32 iChunk = 0; iChunk < m_Chunks.size(); ++iChunk)
				{
...
void CRenderMesh::AddRenderElements(IMaterial *pIMatInfo, CRenderObject *pObj, const SRenderingPassInfo &passInfo, i32 nList, i32 nAW)
{
...
void CRenderMesh::AddRE(IMaterial* pMaterial, CRenderObject* obj, IShader* ef, const SRenderingPassInfo& passInfo, i32 nList, i32 nAW)
{
...
size_t CRenderMesh::GetMemoryUsage( IDrxSizer* pSizer, EMemoryUsageArgument nType ) const
{
	size_t nSize = 0;
	switch (nType)
	{
	...
	case MEM_USAGE_ONLY_STREAMS:
		...
		if (pSizer)
		{
			...
			pSizer->AddObject((uk )this, nSize);
		}
	...
	{
		nSize += sizeof(*this);
		for (i32 i=0; i<(i32)m_Chunks.capacity(); i++)
		{
			if (i < m_Chunks.size())
		...
		for (i32 i=0; i<(i32)m_ChunksSkinned.capacity(); i++)
		{
...
void CRenderMesh::GetMemoryUsage( IDrxSizer* pSizer ) const
{
...
i32 CRenderMesh::GetAllocatedBytes( bool bVideoMem ) const
{
  if (!bVideoMem)
    return Size(SIZE_ONLY_SYSTEM);
  else
...
i32 CRenderMesh::GetTextureMemoryUsage( const IMaterial* pMaterial, IDrxSizer* pSizer, bool bStreamedIn ) const
{
  ...
  for (i32 a=0; a<m_Chunks.size(); a++)
  {
    ...
    for (i32 i=0; i<EFTT_MAX; i++)
    {
      ...
      used.insert(pTexture);
			...
			i32 nTexSize = bStreamedIn ? pTexture->GetDeviceDataSize() : pTexture->GetDataSize();
...
void CRenderMesh::InitTriHash(IMaterial * pMaterial)
{
  ...
  {
    for (u32 ii=0; ii<(u32)m_Chunks.size(); ii++)
    {
      ...
      u32 nFirstIndex = pChunk->nFirstIndexId;
      u32 nLastIndex = pChunk->nFirstIndexId + pChunk->nNumIndices;

      for (u32 i = nFirstIndex; i < nLastIndex; i+=3)
      {
        i32 I0	=	pIndices[i+0];
        i32 I1	=	pIndices[i+1];
        i32 I2	=	pIndices[i+2];
        ...
        for(float x=aabbCell.min.x; x<aabbCell.max.x; x++)
        {
          for(float y=aabbCell.min.y; y<aabbCell.max.y; y++)
          {
            for(float z=aabbCell.min.z; z<aabbCell.max.z; z++)
            {
              ...
              i32 key = (i32)(x*256.f*256.f + y*256.f + z);
              PodArray<std::pair<i32,i32> > * pTris = &(*m_pTrisMap)[key];
              std::pair<i32,i32> t(i,pChunk->m_nMatID);
              if(pTris->Find(t)<0)
                pTris->Add(t);
            }
          }
...
const PodArray<std::pair<i32,i32> > * CRenderMesh::GetTrisForPosition(const Vec3 & vPos, IMaterial * pMaterial)
{
#ifdef RENDER_MESH_TRIANGLE_HASH_MAP_SUPPORT
  ...
  i32 key = (i32)(vCellMin.x*256.f*256.f + vCellMin.y*256.f + vCellMin.z);
...
void CRenderMesh::UpdateBBoxFromMesh()
{
  ...
  i32 nVertCount = _GetVertexContainer()->GetVerticesCount();
  i32 nPosStride=0;
  i32 nIndCount = GetIndicesCount();
  const byte * pPositions = GetPosPtr(nPosStride, FSL_READ);
  const vtx_idx * pIndices = GetIndexPtr(FSL_READ);
  ...
  for (i32 a=0; a<m_Chunks.size(); a++)
  {
    ...
    for (u32 i = nFirstIndex; i < nLastIndex; i++)
    {
      i32 I0	=	pIndices[i];
      if(I0 < nVertCount)
      {
...
static void BlendWeights(DualQuat& dq, const DualQuat aBoneLocs[], const JointIdType aBoneRemap[], u16k indices[], UCol weights)
{
...
struct PosNormData
{
	...
	ILINE void GetPosNorm(PosNorm& ran, i32 nV)
	{
		ran = aPosNorms[nV];
	#if SKIN_MORPHING
	...
	if (!ext)
	{
		...
		if (pInds && (aPos || aPosH) && bNormals)
		{
			...
			for (i32 i=0; i<nParts ; i++)
			{
				i32 aIndices[3];
				Vec3 aVec[3];
				for (i32 v = TriIndices(aIndices, i, eForm)-1; v >= 0; v--)
	...
	if (pInds && m_PosNorms.size())
	{
		...
		if (eForm == GeomForm_Vertices)
		{
			...
			else
			{
				for (auto& ran : points)
				{
					i32 nIndex = seed.GetRandom(0U, m_nInds - 1);
		...
		else
		{
			...
			else
			{
				...
				for (auto& ran : points)
				{
					...
					i32 aIndices[3];
					i32 nVerts = TriIndices(aIndices, nPart, eForm);
...
i32 CRenderChunk::Size() const
{
	size_t nSize = sizeof(*this);
	return static_cast<i32>(nSize);
}

void CRenderMesh::Size( u32 nFlags, IDrxSizer* pSizer ) const
{
	u32 i;
...
buffer_size_t CRenderMesh::Size(u32 nFlags) const
{
...
void CRenderMesh::FreeDeviceBuffers(bool bRestoreSys)
{
	...
	for (i = 0; i < VSF_NUM; i++)
	{
		if (_HasVBStream(i))
		{
			...
			{
				SScopedMeshDataLock _lock(this);
				uk pSrc = LockVB(i, FSL_READ | FSL_VIDEO);
				uk pDst = LockVB(i, FSL_SYSTEM_CREATE);
				drxMemcpy(pDst, pSrc, GetStreamSize(i));
			}
			ReleaseVB(i);
		}
	}
	...
	if (_HasIBStream())
	{
		...
		{
			SScopedMeshDataLock _lock(this);
			uk pSrc = LockIB(FSL_READ | FSL_VIDEO);
			uk pDst = LockIB(FSL_SYSTEM_CREATE);
			drxMemcpy(pDst, pSrc, m_nInds * sizeof(vtx_idx));
		}
		ReleaseIB();
	}
}

void CRenderMesh::FreeVB(i32 nStream)
{
...
//////////////////////////////////////////////////////////////////////////
void CRenderMesh::DebugDraw( const struct SGeometryDebugDrawInfo &info,u32 nVisibleChunksMask )
{
  MEMORY_SCOPE_CHECK_HEAP();
	...
	i32 oldIndex = pRenderAuxGeom->PushMatrix(info.tm);
	...
	u32 currentIndexBase = 0;
#endif

	i32k chunkCount = m_Chunks.size();
	for (i32 currentChunkIndex = 0; currentChunkIndex < chunkCount; ++currentChunkIndex)
	{
		...
		const vtx_idx *pIndices = GetIndexPtr(FSL_READ);
		u32k numVertices = GetVerticesCount();
		u32k indexStep = 3;
		u32 numIndices = pChunk->nNumIndices;

		// Make sure number of indices is a multiple of 3.
		u32 indexRemainder = numIndices % indexStep;
		if (indexRemainder != 0)
		{
		...
		u32k firstIndex = pChunk->nFirstIndexId;
		u32k lastIndex = firstIndex + numIndices;

		for (u32 i = firstIndex; i < lastIndex; i += indexStep)
		{
			i32 I0 = pIndices[ i ];
			i32 I1 = pIndices[ i + 1 ];
			i32 I2 = pIndices[ i + 2 ];

			assert( (u32)I0 < numVertices );
			assert( (u32)I1 < numVertices );
			assert( (u32)I2 < numVertices );
...
void CRenderMesh::PrintMeshLeaks()
{
  MEMORY_SCOPE_CHECK_HEAP();
	...
	for (util::list<CRenderMesh> *iter = CRenderMesh::s_MeshList.next; iter != &CRenderMesh::s_MeshList; iter = iter->next)
	{
		...
		DRX_ASSERT_MESSAGE(0, "CRenderMesh leak");
	}
}

bool CRenderMesh::ClearStaleMemory(bool bAcquireLock, i32 threadId)
{
  MEMORY_SCOPE_CHECK_HEAP();
  DRX_PROFILE_FUNCTION(PROFILE_RENDERER);
	bool cleared = false; 
...
#     if !defined(_RELEASE) && defined(RM_CATCH_EXCESSIVE_LOCKS)
			...
			{
				DrxError("data lock for mesh '%s:%s' held longer than 32 seconds", (pRM->m_sType?pRM->m_sType:"unknown"), (pRM->m_sSource?pRM->m_sSource:"unknown"));
...
void CRenderMesh::UpdateModifiedMeshes(bool bAcquireLock, i32 threadId)
{
	DRX_ASSERT(gRenDev->m_pRT->IsRenderThread());
...
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CRenderMesh::Tick");
...
void CRenderMesh::ShutDown()
{
	...
	DRX_ASSERT(&CRenderMesh::s_MeshList == CRenderMesh::s_MeshList.prev); // Active RenderMesh-list needs to be empty, or leaks occur!
	for (i32 i=0; i<DRX_ARRAY_COUNT(CRenderMesh::s_MeshGarbageList); ++i) // Make sure there are no pending deletes in garbage list
		DRX_ASSERT(&CRenderMesh::s_MeshGarbageList[i] == CRenderMesh::s_MeshGarbageList[i].prev);
...
//////////////////////////////////////////////////////////////////////////
void CRenderMesh::SetRenderChunks( CRenderChunk *pInputChunksArray,i32 nCount,bool bSubObjectChunks )
{
...
{
	u32 nFrameID = GetCurrentFrameID();
	...
	for (MeshSubSetIndices::iterator it = m_meshSubSetIndices.begin(); it != m_meshSubSetIndices.end(); )
	{
		...
		if (abs((i32)nFrameID - (i32)((CRenderMesh*)pRM)->m_nLastRenderFrameID) > DELETE_SUBSET_MESHES_AFTER_NOTUSED_FRAMES)
		{
...
void CRenderMesh::MarkRenderElementsDirty()
{
 ...
 i32* CRenderMesh::SetAsyncUpdateState()
{
	SREC_AUTO_LOCK(m_sResLock); 
	ASSERT_IS_MAIN_THREAD(gRenDev->m_pRT);
	i32 threadID = gRenDev->GetMainThreadID();
	if (m_asyncUpdateStateCounter[threadID] == 0)
	{
		m_asyncUpdateStateCounter[threadID] = 1;
		LockForThreadAccess();
	}
	DrxInterlockedIncrement(&m_asyncUpdateState[threadID]);
...
bool CRenderMesh::SyncAsyncUpdate(i32 threadID, bool block)
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_RENDERER, "CRenderMesh::SyncAsyncUpdate() sync");
		i32 iter = 0;
...
void CRenderMesh::CreateRemappedBoneIndicesPair(const uint pairGuid, const TRenderChunkArray& Chunks)
{
	...
	SVF_W4B_I4S* remappedIndices = nullptr;
	{
		...
		DRX_ASSERT_MESSAGE(stride == sizeof(SVF_W4B_I4S), "Stride doesn't match");
		DRX_ASSERT(remappedIndices && pIndicesWeights);
...
void CRenderMesh::CreateRemappedBoneIndicesPair(const DynArray<JointIdType> &arrRemapTable, const uint pairGuid, ukk tag)
{
	ASSERT_IS_MAIN_THREAD(gRenDev->m_pRT);
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CRenderMesh::CreateRemappedBoneIndicesPair");
...
void CRenderMesh::ReleaseRemappedBoneIndicesPair(const uint pairGuid)
{
	...
	DRX_ASSERT(gRenDev->m_pRT->IsRenderThread());
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CRenderMesh::ReleaseRemappedBoneIndicesPair");
	...
	if(!bFound)
	{
		for (size_t i=0,end=m_CreatedBoneIndices.size(); i<end; ++i)
		{
			if (m_CreatedBoneIndices[i].guid == pairGuid)
			{
				DRX_ASSERT_MESSAGE(m_CreatedBoneIndices[i].refcount > 0, "Bone indices pair over-released!");
...
uk CRenderMesh::operator new(size_t size )
{
	return AllocateMeshInstanceData(size, alignof(CRenderMesh));
}

void CRenderMesh::operator delete( uk ptr )
{
  FreeMeshInstanceData(ptr);
}

D3DBuffer* CRenderMesh::_GetD3DVB( i32 nStream, buffer_size_t* offs ) const
{
  if (SMeshStream* pMS = GetVertexStream(nStream))
  {
    IF (pMS->m_nID != ~0u, 1)
...
bool CRenderMesh::GetRemappedSkinningData(u32 guid, SStreamInfo& streamInfo)
{
	for (size_t i = 0, end = m_RemappedBoneIndices.size(); i < end; ++i)
	{
...
bool CRenderMesh::CheckStreams()
{
	...
	for (i32 nStream = 0; nStream < VSF_NUM; ++nStream)
	{
...
void CRenderMesh::Render(CRenderObject* pObj, const SRenderingPassInfo& passInfo)
{
...
#if !defined(_RELEASE)
	tukk szExcl = CRenderer::CV_r_excludemesh->GetString();
	if (szExcl[0] && m_sSource)
	{
		char szMesh[1024];
		drx_strcpy(szMesh, this->m_sSource);
		strlwr(szMesh);
		if (szExcl[0] == '!')
		{
	...
	//////////////////////////////////////////////////////////////////////////
	if (!m_meshSubSetIndices.empty() && abs((i32)m_nLastRenderFrameID - (i32)m_nLastSubsetGCRenderFrameID) > DELETE_SUBSET_MESHES_AFTER_NOTUSED_FRAMES)
	{
	...
	i32k nAW = (pObj->m_ObjFlags & FOB_AFTER_WATER) || (pObj->m_ObjFlags & FOB_NEAREST) ? 1 : 0;
...
void CRenderMesh::AddShadowPassMergedChunkIndicesAndVertices(CRenderChunk* pCurrentChunk, IMaterial* pMaterial, i32& rNumVertices, i32& rNumIndices)
{
	if (m_Chunks.size() == 0)
		return;
	...
	for (u32 i = (u32)(pCurrentChunk - &m_Chunks[0]) + 1; i < (u32)m_Chunks.size(); ++i)
	{
...
void CRenderMesh::ClearJobResources()
{
	for (i32 i = 0; i < RT_COMMAND_BUF_COUNT; ++i)
	{
		...
		{
			gEnv->pJobUpr->WaitForJob(m_meshSubSetRenderMeshJobs[i][j].jobState);
		}
...
{
	// Main thread
	u32k nThreadID = gRenDev->GetMainThreadID();
...
void SMeshSubSetIndicesJobEntry::CreateSubSetRenderMesh()
{
	...
	u32 nChunkCount              = renderChunks.size();

	SScopedMeshDataLock _lock(pSrcMesh);
	i32 nIndCount = pSrcMesh->GetIndicesCount();
	if (vtx_idx* pInds = pSrcMesh->GetIndexPtr(FSL_READ))
	{
		...
		for (u32 c = 0; c < nChunkCount; c++)
		{
			...
			if (!(m_nMeshSubSetMask & (hidemask1 << srcChunk.nSubObjectIndex)))
			{
				...
				newChunks.back().nNumVerts    = max((i32)srcChunk.nFirstVertId + (i32)srcChunk.nNumVerts - (i32)newChunks.back().nFirstVertId, (i32)newChunks.back().nNumVerts);
			}
		}
		_lock = {};
...
bool CRenderMesh::RayIntersectMesh(const Ray& ray, Vec3& hitpos, Vec3& p0, Vec3& p1, Vec3& p2, Vec2& uv0, Vec2& uv1, Vec2& uv2)
{
	...
	i32k numIndices = GetIndicesCount();
	i32k numVertices = GetVerticesCount();
	if (numIndices || numVertices)
	{
		...
		i32k nChunkCount = Chunks.size();
		for (i32 nChunkId = 0; nChunkId < nChunkCount; nChunkId++)
		{
			const CRenderChunk* pChunk = &Chunks[nChunkId];
			if ((pChunk->m_nMatFlags & MTL_FLAG_NODRAW))
				continue;

			i32 lastIndex = pChunk->nFirstIndexId + pChunk->nNumIndices;
			for (i32 i = pChunk->nFirstIndexId; i < lastIndex; i += 3)
			{
