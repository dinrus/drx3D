...
#include <drx3D/CoreX/Renderer/CREParticle.h>
#include <drx3D/Render/DriverD3D.h>

#if DRX_PLATFORM_DESKTOP || DRX_PLATFORM_CONSOLE
	#define PFX_32BIT_IB
#else
	#define PFX_16BIT_IB
#endif
...
void CParticleSubBuffer::Create(uint elemsCount, uint stride, DXGI_FORMAT format, u32 flags)
{
	m_flags = flags;
	if (IsStream(flags))
	{
...
void CParticleSubBuffer::Unlock(u32 size)
{
	if (m_pLockedData)
	{
		if (IsStream(m_flags))
...
void CParticleBufferSet::Lock(i32 frameId)
{
	assert(m_valid);
...
void CParticleBufferSet::Unlock(i32 frameId)
{
	assert(m_valid);
	assert(gRenDev->m_pRT->IsRenderThread());
...
void CParticleBufferSet::SetFence(i32 frameId)
{
...
void CParticleBufferSet::WaitForFence(i32 frameId)
{
#if BUFFER_ENABLE_DIRECT_ACCESS == 1
	assert(m_valid);
...
uint CParticleBufferSet::GetAllocId(i32 frameId) const
{
	const uint bindId = frameId % CREParticle::numBuffers;
	return bindId;
}
...
{
	DRX_ASSERT(m_subBufferVertices.m_pMemoryBase[allocId]);
...
{
	DRX_ASSERT(m_subBufferIndices.m_pMemoryBase[allocId]);
...
{
	DRX_ASSERT(m_subBufferPositions.m_pMemoryBase[allocId] &&
...
bool CParticleBufferSet::IsValid(i32 frameId) const
{
	const uint bindId = frameId % CREParticle::numBuffers;
	return
		m_valid &&
