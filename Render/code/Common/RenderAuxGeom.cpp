...
static inline u32 PackColor(const ColorB& col)
{
...
ScaleColor(ColorB color, float scale)
{
	...
	col *= scale;
	return(ColorB((u8) col.x, (u8)col.y, (u8) col.z, color.a));
}

static inline u32 AlphaFlags(const ColorB& colV)
{
...
SAuxGeomRenderFlags CAuxGeomCB::SetRenderFlags(const SAuxGeomRenderFlags& renderFlags)
{
	...
	DRX_ASSERT(0 == (renderFlags.m_renderFlags & ~e_PublicParamsMask));
...
void CAuxGeomCB::DrawPoint(const Vec3& v, const ColorB& col, u8 size)
{
	DRX_ASSERT(size > 0);
...
void CAuxGeomCB::DrawPoints(const Vec3* v, u32 numPoints, const ColorB& col, u8 size)
{
	DRX_ASSERT(size > 0);
	SAuxVertex* pVertices(nullptr);
	AddPrimitive(pVertices, numPoints, CreatePointRenderFlags(size));

	u32 color(PackColor(col));
...
void CAuxGeomCB::DrawLines(const Vec3* v, u32 numPoints, const ColorB& col, float thickness)
{
	DRX_ASSERT((numPoints >= 2) && (0 == (numPoints & 1)));
...
void CAuxGeomCB::DrawLines(const Vec3* v, u32k* packedColorARGB8888, u32 numPoints, float thickness, bool alphaFlag)
{
	DRX_ASSERT((numPoints >= 2) && (0 == (numPoints & 1)));
...
void CAuxGeomCB::DrawLines(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB& col, float thickness)
{
	DRX_ASSERT(numPoints >= 2);
	DRX_ASSERT((numIndices >= 2) && (0 == (numIndices & 1)));
...
void CAuxGeomCB::DrawLineStrip(const Vec3 * v, u32 numPoints, const ColorB* col, float thickness)
{
	DRX_ASSERT(numPoints >= 2);
...
void CAuxGeomCB::DrawPolyline(const Vec3* v, u32 numPoints, bool closed, const ColorB& col, float thickness)
{
	DRX_ASSERT(numPoints >= 2);
	DRX_ASSERT(!closed || numPoints >= 3);   // if "closed" then we need at least three vertices
...
{
	DRX_ASSERT(thickness > 0.0f);
	DRX_ASSERT(gRenDev != nullptr);
...
void CAuxGeomCB::DrawThickLineStrip(const Vec3 * v, u32 numPoints, const ColorB & col, float thickness)
{
	DRX_ASSERT(thickness > 0.0f);
	DRX_ASSERT(gRenDev != nullptr);
...
void CAuxGeomCB::DrawTriangles(const Vec3* v, u32 numPoints, const ColorB& col)
{
	DRX_ASSERT((numPoints >= 3) && (0 == (numPoints % 3)));
...
void CAuxGeomCB::DrawTriangles(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB& col)
{
	DRX_ASSERT(numPoints >= 3);
	DRX_ASSERT((numIndices >= 3) && (0 == (numIndices % 3)));
...
void CAuxGeomCB::DrawBuffer(const SAuxVertex* inVertices, u32 numVertices, bool textured)
{
	DRX_ASSERT((numVertices >= 3) && (0 == (numVertices % 3)));
...
SAuxVertex * CAuxGeomCB::BeginDrawBuffer(u32 maxVertices, bool textured)
{
	DRX_ASSERT_MESSAGE(!IsDrawBufferActive(), "Nested BeginDrawBuffer-EndDrawBuffer is not allowed.");
	...
	{
		DRX_ASSERT((maxVertices >= 3) && (0 == (maxVertices % 3)));
...
void CAuxGeomCB::EndDrawBuffer(u32 numVertices)
{
	DRX_ASSERT(IsDrawBufferActive());
	...
	if (IsActiveDrawBufferUsingScratch())
	{
		...
		DRX_ASSERT(numVertices <= m_maxScratchBufferVertices);
		SAuxVertex* bufVertices;
...
void CAuxGeomCB::DrawAABB(const AABB& aabb, bool bSolid, const ColorB& col, const EBoundingBoxDrawStyle& bbDrawStyle)
{
	...
	if (eBBD_Extremes_Color_Encoded == bbDrawStyle)
	{
		...
		u32 color(PackColor(col));
		u32 colorMin(PackColor(ColorB(15, 15, 15, col.a)));
		u32 colorMax(PackColor(ColorB(255, 255, 255, col.a)));
	...
	else
	{
		...
		else
		{
			...
			u32 colDown(PackColor(ScaleColor(col, 0.5f)));
			pVertices[0].xyz = xyz;
			...
			u32 colBack(PackColor(ScaleColor(col, 0.6f)));
			pVertices[8].xyz = xyz;
			...
			u32 colFront(PackColor(ScaleColor(col, 0.9f)));
			...
			u32 colLeft(PackColor(ScaleColor(col, 0.7f)));
			...
			u32 colRight(PackColor(ScaleColor(col, 0.8f)));
...
void CAuxGeomCB::DrawAABBs(const AABB* aabbs, u32 aabbCount, bool bSolid, const ColorB& col,
...
void CAuxGeomCB::DrawAABB(const AABB& aabb, const Matrix34& mat, bool bSolid, const ColorB& col, const EBoundingBoxDrawStyle& bbDrawStyle)
{
	SAuxVertex* pVertices(nullptr);
	vtx_idx* pIndices(nullptr);
	i32 oldMatrixIndex = PushMatrix(mat);
...
void CAuxGeomCB::RenderTextQueued(Vec3 pos, const SDrawTextInfo& ti, tukk text)
{
	if (!gEnv->IsDedicated())
	{
		...
		if (!pFont)
		{
			if (gEnv->pSystem && gEnv->pSystem->GetIDrxFont())
				pFont = gEnv->pSystem->GetIDrxFont()->GetFont("default");
...
i32 CAuxGeomCB::PushMatrix(const Matrix34& mat)
{
	i32 curIndex = m_rawData->m_curWorldMatIdx;
	m_rawData->m_curWorldMatIdx = m_rawData->m_auxWorldMatrices.size();
	m_rawData->m_auxWorldMatrices.push_back(mat);
	return curIndex;
}

i32 CAuxGeomCB::SetTexture(i32 texID)
{
	i32 prev = m_rawData->m_textureID;
...
void CAuxGeomCB::SetMatrixIndex(i32 matID)
{
...
void CAuxGeomCB::Submit(uint frames)
{
	...
	DRX_ASSERT(m_rawData->m_curTransMatIdx == -1);
...
{
	DRX_ASSERT(begin < end);
	DRX_ASSERT(end <= m_auxPushBuffer.size());
...
void CAuxGeomCB::AddPushBufferEntry(u32 numVertices, u32 numIndices, const SAuxGeomRenderFlags& renderFlags)
{
	DRX_ASSERT(numVertices > 0);

	AuxPushBuffer& auxPushBuffer(AccessData()->m_auxPushBuffer);

	EPrimType primType = GetPrimType(renderFlags);
	i32 textureID = IsTextured(renderFlags) ? AccessData()->m_textureID : -1;
...
void CAuxGeomCB::AddPrimitive(SAuxVertex*& pVertices, u32 numVertices, const SAuxGeomRenderFlags& renderFlags)
{
	DRX_ASSERT_MESSAGE(!IsDrawBufferActive(), "Adding primitive while BeginDrawBuffer will cause issue in aux drawing.");
	DRX_ASSERT(numVertices > 0);
	...
	DRX_ASSERT(auxVertexBuffer.size() > oldVBSize);
	pVertices = &auxVertexBuffer[oldVBSize];
}

void CAuxGeomCB::AddIndexedPrimitive(SAuxVertex*& pVertices, u32 numVertices, vtx_idx*& pIndices, u32 numIndices, const SAuxGeomRenderFlags& renderFlags)
{
	DRX_ASSERT_MESSAGE(!IsDrawBufferActive(), "Adding primitive while BeginDrawBuffer will cause issue in aux drawing.");
	DRX_ASSERT(numVertices > 0);
	DRX_ASSERT(numIndices > 0);
	...
	DRX_ASSERT(auxIndexBuffer.size() >= oldIBSize);
...
void CAuxGeomCB::SAuxGeomCBRawData::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
{
	DRX_ASSERT_MESSAGE(!enabled || !IsDrawBufferActive(), "a buffer cannot be re-enabled while it is active.");
	if(enabled)
