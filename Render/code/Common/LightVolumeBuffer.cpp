...
void CLightVolumeBuffer::UpdateContent()
{
	...
	DrxStackAllocWithSizeVectorCleared(SLightVolumeInfo, maxNumLightInfos, gpuStageInfos, CDeviceBufferUpr::AlignBufferSizeForStreaming);
	...
	const size_t gpuStageInfosUploadSize = CDeviceBufferUpr::AlignBufferSizeForStreaming(sizeof(SLightVolumeInfo) * currentRange.end);
	const size_t gpuStageRangesUploadSize = CDeviceBufferUpr::AlignBufferSizeForStreaming(sizeof(SLightVolumeRange) * maxNumVolumes); // TODO: Update smaller range: actualNumRanges
