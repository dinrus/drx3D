...
#include <drx3DMovie/IMovieSystem.h>
#include <drx3D/Eng3D/IIndexedMesh.h>
#include <drx3D/CoreX/BitFiddling.h>                              // IntegerLog2()
#include <drx3D/Eng3D/ImageExtensionHelper.h>                     // CImageExtensionHelper
#include <drx3D/Render/Textures/Image/CImage.h>
...
#include <drx3D/Render/RendElements/OpticsFactory.h>
...
#include <drx3D/Render/Shaders/ShaderPublicParams.h>
...
#if !DRX_RENDERER_OPENGL && !DRX_RENDERER_VULKAN && (DRX_RENDERER_DIRECT3D < 120) && !DRX_PLATFORM_DURANGO && DRX_PLATFORM_WINDOWS
...
namespace
{
	...
	{
		DrxCriticalSection& _lock;
		bool _bActive;
	public:
		CConditonalLock(DrxCriticalSection& lock, bool bActive)
...
u32 ColorMasks[(ColorMask::GS_NOCOLMASK_COUNT >> GS_COLMASK_SHIFT)][4] =
{
...
std::array<u32, (ColorMask::GS_NOCOLMASK_COUNT >> GS_COLMASK_SHIFT)> AvailableColorMasks =
{
	{
...
i32 CRenderer::m_iGeomInstancingThreshold = 0;      // 0 means not set yet
...
//#include <drx3D/Render/FrameProfilers-list.h>
//#undef PROFILER
...
#if !DRX_PLATFORM_64BIT && !DRX_PLATFORM_ANDROID
...
void CRenderer::PostInit()
{
	...
	if (gEnv->pDrxFont)
	{
		...
		if (!m_pDefaultFont)
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "Error getting default font");
	}

	// load all default textures
	if (!m_bShaderCacheGen && m_pTextureUpr)
	{
		...
		m_pTextureUpr->PreloadDefaultTextures();
	}
...
void CRenderer::StartRenderIntroMovies()
{
	...
	if (!gEnv->pDrxPak->OpenPack(PathUtil::GetGameFolder(), INTRO_MOVIES_PAK,
	  IDrxPak::FLAGS_PAK_IN_MEMORY))
		return;

	gEnv->pDrxPak->LoadPakToMemory(INTRO_MOVIES_PAK, IDrxPak::eInMemoryPakLocale_GPU);
#endif
...
void CRenderer::StopRenderIntroMovies(bool bWaitForFinished)
{
	...
	gEnv->pDrxPak->LoadPakToMemory(INTRO_MOVIES_PAK, IDrxPak::eInMemoryPakLocale_Unload);
	gEnv->pDrxPak->ClosePack(INTRO_MOVIES_PAK, 0);
#endif
}
...
void CRenderer::Release()
{
	...
	if (!m_DevBufMan.Shutdown())
		DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_ERROR_DBGBRK, "could not free all buffers from CDevBufferMan!");
...
//////////////////////////////////////////////////////////////////////
/*CImage *CRenderer::FindImage(tukk filename)
   {
...
//////////////////////////////////////////////////////////////////////
//void CRenderer::ShowFps(tukk command/* =NULL */)
/*{
   if (!command)
    return;
...
i32 CRenderer::GetFrameID(bool bIncludeRecursiveCalls /*=true*/)
{
...
//////////////////////////////////////////////////////////////////////////
bool CRenderer::SaveTga(u8* sourcedata, i32 sourceformat, i32 w, i32 h, tukk filename, bool flip) const
{
	//assert(0);
	...
	if (flip)
	{
		i32 size             = w * (sourceformat / 8);
		u8* tempw = new u8[size];
		u8* src1  = sourcedata;
		u8* src2  = sourcedata + (w * (sourceformat / 8)) * (h - 1);
		for (i32 k = 0; k < h / 2; k++)
		{
	...
	u8* oldsourcedata = sourcedata;

	if (sourceformat == FORMAT_8_BIT)
	{

		u8* desttemp = new u8[w * h * 3];
		memset(desttemp, 0, w * h * 3);

		u8* destptr = desttemp;
		u8* srcptr  = sourcedata;
	...
	u8* data   = new u8[w * h * (sourceformat >> 3)];
	u8* dest   = data;
	u8* source = sourcedata;

	//memcpy(dest,source,w*h*(sourceformat>>3));

	for (i32 ax = 0; ax < h; ax++)
	{
		for (i32 by = 0; by < w; by++)
		{
			u8 r, g, b, a;
			r = *source;
			source++;
...
//#include <drx3D/Render/Character/DrxModel.h>
...
static DrxCriticalSection gs_RenderViewLock;
static tukk cc_RenderViewName[IRenderView::eViewType_Count] = { "Normal View", "Recursive View", "Shadow View", "BillboardGen View" };
...
{
	for (i32 type = 0; type < IRenderView::eViewType_Count; ++type)
		m_pRenderViewPool[type].ShutDown();
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void CRenderer::InitSystemResources(i32 nFlags)
{
	LOADING_TIME_PROFILE_SECTION;
...
void CRenderer::FreeSystemResources(i32 nFlags)
{
	iLog->Log("*** Start clearing render resources ***");

	// validate flag combinations
	constexpr i32 requiredForSystem          = (FRR_SYSTEM | FRR_OBJECTS);
	constexpr i32 requiredForSystemResources = (FRR_SYSTEM_RESOURCES | FRR_DELETED_MESHES | FRR_FLUSH_TEXTURESTREAMING | FRR_RP_BUFFERS | FRR_POST_EFFECTS | FRR_OBJECTS | FRR_PERMANENT_RENDER_OBJECTS);
	constexpr i32 requiredForTextures        = (FRR_SYSTEM_RESOURCES | FRR_DELETED_MESHES | FRR_FLUSH_TEXTURESTREAMING | FRR_RP_BUFFERS | FRR_POST_EFFECTS | FRR_OBJECTS | FRR_PERMANENT_RENDER_OBJECTS | FRR_TEXTURES);

	DRX_ASSERT((nFlags & FRR_SYSTEM) == 0           || ((nFlags & requiredForSystem)          == requiredForSystem));
	DRX_ASSERT((nFlags & FRR_SYSTEM_RESOURCES) == 0 || ((nFlags & requiredForSystemResources) == requiredForSystemResources));
	DRX_ASSERT((nFlags & FRR_TEXTURES) == 0         || ((nFlags & requiredForTextures)        == requiredForTextures));
	...
	if (nFlags & FRR_SYSTEM_RESOURCES)
	{
		...
		DrxModuleMemalignFree(m_pSpriteVerts);
		m_pSpriteVerts = NULL;
		m_pSpriteInds  = NULL;
...
Vec2 CRenderer::SetViewportDownscale(float xscale, float yscale)
{
	...
	i32 xres = i32(fWidth  * xscale);
	i32 yres = i32(fHeight * yscale);
...
EScreenAspectRatio CRenderer::GetScreenAspect(i32 nWidth, i32 nHeight)
{
...
bool CRenderer::WriteTGA(byte* dat, i32 wdt, i32 hgt, tukk name, i32 src_bits_per_pixel, i32 dest_bits_per_pixel)
{
	return ::WriteTGA((byte*)dat, wdt, hgt, name, src_bits_per_pixel, dest_bits_per_pixel);
}

bool CRenderer::WriteDDS(byte* dat, i32 wdt, i32 hgt, i32 Size, tukk nam, ETEX_Format eFDst, i32 NumMips)
{
#if DRX_PLATFORM_WINDOWS
	bool bRet = true;
...
tukk CRenderer::EF_GetShaderMissLogPath()
{
	return m_cEF.m_ShaderCacheMissPath.c_str();
}

string* CRenderer::EF_GetShaderNames(i32& nNumShaders)
{
	nNumShaders = m_cEF.m_ShaderNames.size();
	return nNumShaders ? &m_cEF.m_ShaderNames[0] : NULL;
}

IShader* CRenderer::EF_LoadShader (tukk name, i32 flags, uint64 nMaskGen)
{
...
uint64 CRenderer::EF_GetRemapedShaderMaskGen(tukk name, uint64 nMaskGen, bool bFixup)
{
	return m_cEF.mfGetRemapedShaderMaskGen(name, nMaskGen, bFixup);
}

uint64 CRenderer::EF_GetShaderGlobalMaskGenFromString(tukk szShaderName, tukk szShaderGen, uint64 nMaskGen)
{
...
tukk CRenderer::EF_GetStringFromShaderGlobalMaskGen(tukk szShaderName, uint64 nMaskGen)
{
	if (!m_cEF.mfUsesGlobalFlags(szShaderName))
		return "\0";
...
SShaderItem CRenderer::EF_LoadShaderItem (tukk szName, bool bShare, i32 flags, SInputShaderResources* Res, uint64 nMaskGen, const SLoadShaderItemArgs* pArgs)
{
...
//////////////////////////////////////////////////////////////////////////
bool CRenderer::EF_ReloadFile_Request (tukk szFileName)
{
	// Replace .tif extensions with .dds extensions.
	char realName[MAX_PATH + 1];
	i32  nameLength = __min(strlen(szFileName), size_t(MAX_PATH));
	memcpy(realName, szFileName, nameLength);
	realName[nameLength] = 0;
	static tukk tifExtension    = ".tif"; // Must start with "." to distinguish from ".*tif"
	static tukk ddsExtension    = ".dds";
	static i32k   extensionLength = 4;
...
bool CRenderer::EF_ReloadFile (tukk szFileName)
{
	tukk szExtension = PathUtil::GetExt(szFileName);
...
#endif //defined(DRX_ENABLE_RC_HELPER)
...
void CRenderer::EF_ReloadShaderFiles (i32 nCategory)
{
...
_smart_ptr<IImageFile> CRenderer::EF_LoadImage(tukk szFileName, u32 nFlags)
{
...
bool CRenderer::WriteTIFToDisk(ukk pData, i32 width, i32 height, i32 bytesPerChannel, i32 numChannels, bool bFloat, tukk szPreset, tukk szFileName)
{
	return WriteTIF(pData, width, height, bytesPerChannel, numChannels, bFloat, szPreset, szFileName);
}

i32 CRenderer::EF_LoadLightmap (tukk name)
{
...
ITexture* CRenderer::EF_GetTextureByID(i32 Id)
{
	if (Id > 0)
	{
...
ITexture* CRenderer::EF_LoadTexture(tukk szName, u32k flags)
{
	if (szName)
	{
		INDENT_LOG_DURING_SCOPE(true, "While trying to load texture '%s' flags=0x%x...", nameTex, flags);

		tukk szExtension = PathUtil::GetExt(szName);
...
IDynTextureSource* CRenderer::EF_LoadDynTexture(tukk dynsourceName, bool sharedRT)
{
	if (sharedRT)
...
bool SShaderItem::Update()
{
	if (!m_pShader || !(m_pShader->GetFlags() & EF_LOADED))
		return false;
	if ((u32)m_nTechnique > 1000 && m_nTechnique != -1) // HACK HACK HACK
	{
		CDrxNameTSCRC Name(m_nTechnique);
		if (!gRenDev->m_cEF.mfUpdateTechnik(*this, Name))
			return false;
	}

	u32 nPreprocessFlags = PostLoad();
...
void CRenderer::EF_StartEf (const SRenderingPassInfo& passInfo)
{
	...
	i32 nR = passInfo.GetRecursiveLevel();
	assert(nR < 2);
	...
	{
		DrxLogAlways("nR (%d) >= MAX_REND_RECURSION_LEVELS (%d)\n", nR, MAX_REND_RECURSION_LEVELS);
...
void CRenderer::EF_SubmitWind(const SWindGrid* pWind)
{
	auto lambdaCallback = [=]
	{
		DRX_PROFILE_REGION(PROFILE_RENDERER, "CRenderer::EF_SubmitWind::lambda");
		...
		i32 nThreadID = m_pRT->m_nCurThreadProcess;
		pDevTex->UploadFromStagingResource(0, [=](uk pData, u32 rowPitch, u32 slicePitch)
		{
			drxMemcpy(pData, pWind->m_pData, CTexture::TextureDataSize(pWind->m_nWidth, pWind->m_nHeight, 1, 1, 1, eTF_R16G16F));
			return true;
		});
	};
...
void CRenderer::Logv(tukk format, ...)
{
	va_list argptr;
...
void CRenderer::LogStrv(tuk format, ...)
{
	va_list argptr;
...
void CRenderer::LogShv(tuk format, ...)
{
	va_list argptr;
...
void CRenderer::EF_CheckLightMaterial(SRenderLight* pLight, u16 nRenderLightID, const SRenderingPassInfo& passInfo)
{
	ASSERT_IS_MAIN_THREAD(m_pRT);
...

	{
		...
		if (pRendElemBase && !pRendElemBase->empty())
		{
			...
			i32k nList     = (pRE->mfGetType() != eDATA_LensOptics) ? EFSLIST_TRANSP_AW : EFSLIST_LENSOPTICS;

			const float fWaterLevel = gEnv->p3DEngine->GetWaterLevel();
			const float fCamZ       = passInfo.GetCamera().GetPosition().z;
			i32k nAW         = ((fCamZ - fWaterLevel) * (pLight->m_Origin.z - fWaterLevel) > 0) ? 1 : 0;
...
i32 CRenderer::EF_AddDeferredLight(const SRenderLight& pLight, float fMult, const SRenderingPassInfo& passInfo)
{
...
EShaderQuality CRenderer::EF_GetShaderQuality(EShaderType eST)
{
	...
	i32 nQuality        = (i32)pSP->GetShaderQuality();
...
#pragma warning( disable : 4312 )   // 'type cast' : conversion from 'i32' to 'uk ' of greater size
#endif

i32 CRenderer::CurThreadList()
{
...
namespace {
	...
	template<typename T>
	void WriteQueryResult(uk pOutput, u32 nOutputSize, const T& rQueryResult)
	{
#if !defined(_RELEASE)
		if (pOutput == NULL)
			DrxFatalError("No Output Storage Specified");
		if (sizeof(T) != nOutputSize)
			DrxFatalError("Insufficient storage for EF_Query Output");
#endif
	...
	template<typename T>
	T ReadQueryParameter(uk pInput, u32 nInputSize)
	{
#if !defined(_RELEASE)
		if (pInput == NULL)
			DrxFatalError("No Input Storage Specified");
		if (sizeof(T) != nInputSize)
			DrxFatalError("Insufficient storage for EF_Query Input");
#endif
		return *alias_cast<T*>(pInput);
	}
}

void CRenderer::EF_QueryImpl(ERenderQueryTypes eQuery, uk pInOut0, u32 nInOutSize0, uk pInOut1, u32 nInOutSize1)
{
	switch (eQuery)
	{
	case EFQ_DeleteMemoryArrayPtr:
	{
		tuk pPtr = ReadQueryParameter<tuk>(pInOut0, nInOutSize0);
		delete[] pPtr;
	}
	break;
	...
	case EFQ_Alloc_APIMesh:
	{
		...
		for (util::list<CRenderMesh>* iter = CRenderMesh::s_MeshList.next; iter != &CRenderMesh::s_MeshList; iter = iter->next)
		{
			...
			nSize += static_cast<u32>(pRM->Size(CRenderMesh::SIZE_VB));
			nSize += static_cast<u32>(pRM->Size(CRenderMesh::SIZE_IB));
		}
		WriteQueryResult(pInOut0, nInOutSize0, nSize);
	}
	break;
	...
	case EFQ_Alloc_Mesh_SysMem:
	{
		...
		{
			CRenderMesh* pRM = iter->item<& CRenderMesh::m_Chain>();
			nSize += static_cast<u32>(pRM->Size(CRenderMesh::SIZE_ONLY_SYSTEM));
		}
		WriteQueryResult(pInOut0, nInOutSize0, nSize);
	}
	break;
	...
	{
		u32 streamPoolSize = (u32)(CRenderer::GetTexturesStreamPoolSize() * 1024 * 1024);
	...
	case EFQ_AAMode:
	{
		u32k nMode = CRenderer::CV_r_AntialiasingMode;
			...
			if (stats->bComputeReuquiredTexturesPerFrame)
			{
				...
				if (pRL)
				{
					...
					for (itor = pRL->m_RMap.begin(); itor != pRL->m_RMap.end(); itor++)
					{
						...
						i32  nPersMip = tp->GetNumMips() - tp->GetNumPersistentMips();
						bool bStale   = CTexture::s_pTextureStreamer->StatsWouldUnload(tp);
						i32  nCurMip  = bStale ? nPersMip : tp->GetRequiredMip();
						if (tp->IsForceStreamHighRes())
							nCurMip = 0;
						i32 nMips = tp->GetNumMips();
						nCurMip = min(nCurMip, nPersMip);

						i32 nTexSize = tp->StreamComputeSysDataSize(nCurMip);
	...
	{
		u32 nParticleVerticePoolSize = CV_r_ParticleVerticePoolSize * sizeof(SVF_P3F_C4B_T4B_N3F2);
	...
	{
		u32 nParticleVerticePoolSize = CV_r_ParticleVerticePoolSize * 3 * sizeof(u16);
		WriteQueryResult(pInOut0, nInOutSize0, nParticleVerticePoolSize);
	}
	break;
	...
	{
		u32 nMaxPartCon = nMaxParticleContainer;
	...
	case EFQ_SetDynTexSourceSharedRTDim:
	{
		if (pInOut0)
		{
			i32 p0 = ReadQueryParameter<i32>(pInOut0, nInOutSize0);
			i32 p1 = ReadQueryParameter<i32>(pInOut1, nInOutSize1);
	...
	{
		u32k nThreadID = m_pRT->GetThreadList();
		u32 nReverseDepth   = 1;
	...
	case EFQ_GetLastD3DDebugMessage:
	{
		...
		{
		public:
			D3DDebugMessage(tukk pMsg) : m_msg(pMsg) {}

			virtual void        Release() override      { delete this; }
			virtual tukk GetMsg() const override { return m_msg.c_str(); }
...
_smart_ptr<IRenderMesh> CRenderer::CreateRenderMesh(tukk szType, tukk szSourceName, IRenderMesh::SInitParamerers* pInitParams, ERenderMeshType eBufType)
{
	if (pInitParams)
	{
...
_smart_ptr<IRenderMesh> CRenderer::CreateRenderMeshInitialized(
	...
	const PublicRenderPrimitiveType nPrimetiveType, tukk szType, tukk szSourceName, ERenderMeshType eBufType,
	i32 nMatInfoCount, i32 nClientTextureBindID,
	bool (* PrepareBufferCallback)(IRenderMesh*, bool),
	uk CustomData, bool bOnlyVideoBuffer, bool bPrecache,
	const SPipTangents* pTangents, bool bLockForThreadAcc, Vec3* pNormals)
{
	...
	if (CustomData)
		DrxFatalError("CRenderMesh::CustomData not supported anymore. Will be removed from interface");
...
i32 CRenderer::GetWhiteTextureId() const
{
	i32k textureId = (CRendererResources::s_ptexWhite) ? CRendererResources::s_ptexWhite->GetID() : -1;
	return textureId;
}
...
i32 CRenderer::GetOverlayWidth() const
{
...
i32 CRenderer::GetOverlayHeight() const
{
	if (GetIStereoRenderer()->GetStereoEnabled() && !GetIStereoRenderer()->IsMenuModeEnabled())
...
void CRenderer::SetTextureAlphaChannelFromRGB(byte* pMemBuffer, i32 nTexSize)
{
	// set alpha channel
	for (i32 y = 0; y < nTexSize; y++)
	for (i32 x = 0; x < nTexSize; x++)
	{
		i32 t = (x + nTexSize * y) * 4;
...
//=============================================================================
// Precaching
bool CRenderer::EF_PrecacheResource(IRenderMesh* _pPB, IMaterial* pMaterial, float fMipFactor, float fTimeToReady, i32 nFlags, i32 nUpdateId)
{
	i32 i;
	...
	for (i = 0; i < pPB->m_Chunks.size(); i++)
	{
		...
		for (i32 j = 0; j <= pSR->m_nLastTexture; j++)
		{
			if (!pSR->m_Textures[j])
				continue;
...
bool CRenderer::EF_PrecacheResource(SRenderLight* pLS, float fMipFactor, float fTimeToReady, i32 Flags, i32 nUpdateId)
{
...
void CRenderer::PrecacheTexture(ITexture* pTP, float fMipFactor, float fTimeToReady, i32 Flags, i32 nUpdateId, i32 nCounter)
{
	if (!CRenderer::CV_r_texturesstreaming)
		return;
...
bool CRenderer::EF_PrecacheResource(IShader* pSH, float fMipFactor, float fTimeToReady, i32 Flags)
{
	if (!CRenderer::CV_r_texturesstreaming)
		return true;
...
#include <drx3D/Render/../SDKs/squish-ccr/squish.h>
#include <drx3D/Render/../SDKs/squish-ccr/squish.inl"
...
static void DXTDecompressRow(SCompressRowData data)
{
	...
	u8* dst = ((u8*)data.destinationData) + (data.row * data.rowStride);
	u8* src = ((u8*)data.sourceData) + ((data.row >> 2) * data.blockStride);

	for (i32 x = 0; x < data.width; x += data.blockWidth)
	{
		u8 values[4][4][4];

		// decode
		data.pSqio->decoder((u8*)values, (uk )src, data.pSqio->flags);

		// transfer
		for (i32 by = 0; by < data.blockHeight; by += 1)
		{
			u8* bdst = ((u8*)dst) + (by * data.rowStride);

			for (i32 bx = 0; bx < data.blockWidth; bx += 1)
			{
...
static void DXTDecompressRowFloat(SCompressRowData data)
{
	...
	for (i32 x = 0; x < data.width; x += data.blockWidth)
	{
		...
		for (i32 by = 0; by < data.blockHeight; by += 1)
		{
			...
			for (i32 bx = 0; bx < data.blockWidth; bx += 1)
			{
				bdst[bx * data.pixelStride + 0] = data.sourceChannels <= 0 ? 0U : std::min((u8)255, (u8)floorf(values[by][bx][0] * LDR_UPPERNORM / HDR_UPPERNORM + 0.5f));
				bdst[bx * data.pixelStride + 1] = data.sourceChannels <= 1 ? bdst[bx * data.pixelStride + 0] : std::min((u8)255, (u8)floorf(values[by][bx][1] * LDR_UPPERNORM / HDR_UPPERNORM + 0.5f));
				bdst[bx * data.pixelStride + 2] = data.sourceChannels <= 1 ? bdst[bx * data.pixelStride + 0] : std::min((u8)255, (u8)floorf(values[by][bx][2] * LDR_UPPERNORM / HDR_UPPERNORM + 0.5f));
...
static void DXTCompressRow(SCompressRowData data)
{
	...
	u8* dst = ((u8*)data.destinationData) + ((data.row >> 2) * data.blockStride);
	u8* src = ((u8*)data.sourceData) + (data.row * data.rowStride);
	...
	for (i32 x = 0; x < data.width; x += data.blockWidth)
	{
		...
		for (i32 by = 0; by < data.blockHeight; by += 1)
		{
			u8* bsrc = ((u8*)src) + (by * data.rowStride);
		...
		// encode
		data.pSqio->encoder((float*)values, 0xFFFF, (uk )dst, data.pSqio->flags);
...
bool CRenderer::DXTDecompress(byte* sourceData, const size_t srcFileSize, byte* destinationData, i32 width, i32 height, i32 mips, ETEX_Format sourceFormat, bool bUseHW, i32 nDstBytesPerPix)
{
	FUNCTION_PROFILER_RENDERER();
	...
	data.pixelStride         = blockChannels * sizeof(u8);
	...
	if ((datatype == squish::sqio::DT_U8) && (nDstBytesPerPix == 4))
	{
		...
		for (i32 y = 0; y < height; y += blockHeight)
		{
			...
			compressJob.SetPriorityLevel(JobUpr::eStreamPriority);
			compressJob.Run();
#else
...
bool CRenderer::DXTCompress(byte* sourceData, i32 width, i32 height, ETEX_Format destinationFormat, bool bUseHW, bool bGenMips, i32 nSrcBytesPerPix, MIPDXTcallback callback)
{
	FUNCTION_PROFILER_RENDERER();
	...
	u8* destinationData = new u8[sqio.compressedsize];
	if (!destinationData)
		return false;
	...
	if ((datatype == squish::sqio::DT_U8) && (nSrcBytesPerPix == 4))
	{
		...
		for (i32 y = 0; y < height; y += blockHeight)
		{
			...
			compressJob.RegisterJobState(&jobState);
			compressJob.SetPriorityLevel(JobUpr::eHighPriority);
			compressJob.Run();
#else
			DXTCompressRow(data);
#endif
...
bool CRenderer::WriteJPG(byte* dat, i32 wdt, i32 hgt, tuk name, i32 src_bits_per_pixel, i32 nQuality)
{
	return ::WriteJPG(dat, wdt, hgt, name, src_bits_per_pixel, nQuality);
}

//////////////////////////////////////////////////////////////////////////
ITexture* CRenderer::CreateTexture(tukk name, i32 width, i32 height, i32 numMips, u8* pData, ETEX_Format eTF, i32 flags)
{
	char uniqueName[128];
	drx_sprintf(uniqueName, "%s%d", name, m_TexGenID++);
	return CTexture::GetOrCreate2DTexture(uniqueName, width, height, numMips, flags, pData, eTF);
}

//////////////////////////////////////////////////////////////////////////
ITexture* CRenderer::CreateTextureArray(tukk name, ETEX_Type eType, u32 nWidth, u32 nHeight, u32 nArraySize, i32 nMips, u32 nFlags, ETEX_Format eTF, i32 nCustomID)
{
	char uniqueName[128];
	drx_sprintf(uniqueName, "%s%d", name, m_TexGenID++);
...
//////////////////////////////////////////////////////////////////////////
void CRenderer::CopyTextureRegion(ITexture* pSrc, RectI srcRegion, ITexture* pDst, RectI dstRegion, ColorF& color, i32k renderStateFlags)
{
...
tukk CRenderer::GetTextureFormatName(ETEX_Format eTF)
{
	return CTexture::NameForTextureFormat(eTF);
}

i32 CRenderer::GetTextureFormatDataSize(i32 nWidth, i32 nHeight, i32 nDepth, i32 nMips, ETEX_Format eTF)
{
...
void CRenderer::GetPolyCount(i32& nPolygons, i32& nShadowPolys)
{
#if defined(ENABLE_PROFILING_CODE)
...
i32 CRenderer::GetNumGeomInstances()
{
#if defined(ENABLE_PROFILING_CODE)
...
i32 CRenderer::GetNumGeomInstanceDrawCalls()
{
#if defined(ENABLE_PROFILING_CODE)
...
i32 CRenderer::GetCurrentNumberOfDrawCalls()
{
#if defined(ENABLE_PROFILING_CODE)
...
void CRenderer::GetCurrentNumberOfDrawCalls(i32& nGeneral, i32& nShadowGen)
{
#if defined(ENABLE_PROFILING_CODE)
...
i32 CRenderer::GetCurrentNumberOfDrawCalls(u32k EFSListMask)
{
#if defined(ENABLE_PROFILING_CODE)
...
i32 SRenderStatistics::GetNumberOfDrawCalls(u32k EFSListMask) const
{
	i32 nDIPs = 0;
	for (u32 i = 0; i < EFSLIST_NUM; i++)
	{
		if ((1 << i) & EFSListMask)
		{
			nDIPs += m_nDIPs[i]; // +m_nAsynchDIPs[i]; impossible because of non-atomicity of stat-collection
...
i32 SRenderStatistics::GetNumberOfPolygons() const
{
	i32 nDIPs = 0;
	for (i32 i = 0; i < EFSLIST_NUM; i++)
	{
...
i32 SRenderStatistics::GetNumberOfPolygons(u32k EFSListMask) const
{
	i32 nDIPs = 0;
	for (u32 i = 0; i < EFSLIST_NUM; i++)
	{
...
void IRenderer::SDrawCallCountInfo::Update(CRenderObject* pObj, IRenderMesh* pRM, EShaderTechniqueID techniqueID)
{
	if (((IRenderNode*)pObj->m_pRenderNode))
	{
		...
		if (meshName[0] == '\0')
		{
			tukk pMeshName = pRM->GetSourceName();
			if (pMeshName)
			{
				...
				drx_strcpy(meshName, pMeshName);
			}

			tukk pTypeName = pRM->GetTypeName();
			if (pTypeName)
			{
				drx_strcpy(typeName, pTypeName);
			}
		}
...
void S3DEngineCommon::Update(const SRenderingPassInfo& passInfo)
{
	...
	{
		i32k nFrmID = passInfo.GetFrameID();
...
void S3DEngineCommon::UpdateRainOccInfo(const SRenderingPassInfo& passInfo)
{
	...
	u32k nMaxOccluders = 512 * (bSnowEnabled ? 3 : 2);
	...
	u32k numGPUs = gRenDev->GetActiveGPUCount();
				...
				if (pRndNode)
				{
					...
					if (pObj)
					{
						...
						if (pObj->GetFlags() & STATIC_OBJECT_COMPOUND)
						{
							...
							i32 nSubCount               = pObj->GetSubObjectCount();
							for (i32 nSubId = 0; nSubId < nSubCount && m_RainOccluders.m_nNumOccluders < occluderLimit; nSubId++)
							{
...
void CRenderer::GetMemoryUsage(IDrxSizer* pSizer)
{
...
{
	if (m_pTextureUpr)
		m_pTextureUpr->ReleaseDefaultTextures();
}

//////////////////////////////////////////////////////////////////////////
void CRenderer::PostShutDown()
{
	SAFE_DELETE(m_pTextureUpr);
}
...
bool CRenderer::IsCustomRenderModeEnabled(u32 nRenderModeMask)
{
...
#else
	DrxFatalError("The external ExecuteAsyncDIP functionality is only supported on Durango");
#endif // DRX_PLATFORM_DURANGO && DURANGO_ENABLE_ASYNC_DIPS == 1
}

//////////////////////////////////////////////////////////////////////////
void CRenderer::EF_SetPostEffectParam(tukk pParam, float fValue, bool bForceValue)
{
	DRX_ASSERT((pParam) && "mfSetParameter: null parameter");
	if (!pParam)
	{
		return;
	}
...
void CRenderer::EF_SetPostEffectParamVec4(tukk pParam, const Vec4& pValue, bool bForceValue)
{
	DRX_ASSERT((pParam) && "mfSetParameter: null parameter");
...
//////////////////////////////////////////////////////////////////////////
void CRenderer::EF_SetPostEffectParamString(tukk pParam, tukk pszArg)
{
	DRX_ASSERT((pParam && pszArg) && "mfSetParameter: null parameter");
...
//////////////////////////////////////////////////////////////////////////
void CRenderer::EF_GetPostEffectParam(tukk pParam, float& fValue)
{
	DRX_ASSERT((pParam) && "mfGetParameter: null parameter");
...
//////////////////////////////////////////////////////////////////////////
void CRenderer::EF_GetPostEffectParamVec4(tukk pParam, Vec4& pValue)
{
	DRX_ASSERT((pParam) && "mfGetParameter: null parameter");
...
//////////////////////////////////////////////////////////////////////////
void CRenderer::EF_GetPostEffectParamString(tukk pParam, tukk & pszArg)
{
	DRX_ASSERT((pParam && pszArg) && "mfGetParameter: null parameter");
...
//////////////////////////////////////////////////////////////////////////
i32 CRenderer::EF_GetPostEffectID(tukk pPostEffectName)
{
	DRX_ASSERT(pPostEffectName && "mfGetParameter: null parameter");
...
//////////////////////////////////////////////////////////////////////////
SSkinningData* CRenderer::EF_CreateSkinningData(IRenderView* pRenderView, u32 nNumBones, bool bNeedJobSyncVar)
{
	i32 nList = m_nPoolIndex % m_computeSkinningData.size();

	u32 nNeededSize = Align(sizeof(SSkinningData), 16);
	nNeededSize += Align(bNeedJobSyncVar ? sizeof(JobUpr::SJobState) : 0, 16);
	...
	pSkinningRenderData->pAsyncJobs = bNeedJobSyncVar ? alias_cast<JobUpr::SJobState*>(pData) : NULL;
	pData += Align(bNeedJobSyncVar ? sizeof(JobUpr::SJobState) : 0, 16);

	if (bNeedJobSyncVar)
	{
		new(pSkinningRenderData->pAsyncJobs) JobUpr::SJobState();
	}
...
SSkinningData* CRenderer::EF_CreateRemappedSkinningData(IRenderView* pRenderView, u32 nNumBones, SSkinningData* pSourceSkinningData, u32 nCustomDataSize, u32 pairGuid)
{
	...
	pSkinningRenderData->pCustomData = nCustomDataSize ? alias_cast<uk>(pData) : NULL;
...
{
	DRX_ASSERT(gcpRendD3D->m_pRT->IsMainThread()); // Compute skinning data pool is not thread safe currently.
	static_cast<CRenderView*>(pRenderView)->GetSkinningDataPools().pDataComputeSkinning->push_back(pData);
}

i32 CRenderer::GetTexturesStreamPoolSize()
{
	i32 poolSize = CV_r_TexturesStreamPoolSize + CV_r_TexturesStreamPoolSecondarySize;
...
{
	i32k currentPoolIndex  =  m_nPoolIndex %  DRX_ARRAY_COUNT(m_SkinningDataPool);
	i32k previousPoolIndex = (m_nPoolIndex + (DRX_ARRAY_COUNT(m_SkinningDataPool) - 1)) % DRX_ARRAY_COUNT(m_SkinningDataPool);
...
i32 CRenderer::EF_GetSkinningPoolID()
{
	return m_nPoolIndex;
}
...
void CRenderer::RefreshShaderResourceConstants(SShaderItem* pShaderItem, IMaterial* pMaterial)
{
	...
	ExecuteRenderThreadCommand(
		[=]
		{
			...
			{
				DRX_PROFILE_REGION(PROFILE_RENDERER, "CRenderer::RefreshShaderResourceConstants");
...
void CRenderer::ForceUpdateShaderItem(SShaderItem* pShaderItem, IMaterial* pMaterial)
{
	...
	ExecuteRenderThreadCommand( 
		[=]
		{
			if (pShaderItem->m_pShader && pShaderItem->m_pShaderResources)
			{
				DRX_PROFILE_REGION(PROFILE_RENDERER, "CRenderer::ForceUpdateShaderItem");
...
void CRenderer::FreePermanentRenderObjects(i32 bufferId)
{
	m_tempRenderObjects.m_persistentRenderObjectsToDelete[bufferId].CoalesceMemory();
	for (i32 i = 0, num = m_tempRenderObjects.m_persistentRenderObjectsToDelete[bufferId].size(); i < num; i++)
	{
...
void CRenderer::SetCloudShadowsParams(i32 nTexID, const Vec3& speed, float tiling, bool invert, float brightness)
{
...
void CRenderer::SetVolumetricCloudParams(i32 nTexID)
{
...
void CRenderer::SetVolumetricCloudNoiseTex(i32 cloudNoiseTexId, i32 edgeNoiseTexId)
{
...
void CRenderer::UpdateCachedShadowsLodCount(i32 nGsmLods) const
{
...
i32 CRenderer::GetDrawCallsPerNode(IRenderNode* pRenderNode)
{
...
void CRenderer::AddRecordedProfilingStats(const SProfilingStats& stats, ERenderListID renderList, bool bAsynchronous)
{
	...
	if (bAsynchronous)
	{
		DrxInterlockedAdd(&pipelineStats.m_nAsynchNumPSOSwitches        , stats.numPSOSwitches);
		DrxInterlockedAdd(&pipelineStats.m_nAsynchNumLayoutSwitches     , stats.numLayoutSwitches);
		DrxInterlockedAdd(&pipelineStats.m_nAsynchNumResourceSetSwitches, stats.numResourceSetSwitches);
		DrxInterlockedAdd(&pipelineStats.m_nAsynchNumInlineSets         , stats.numInlineSets);
		DrxInterlockedAdd(&pipelineStats.m_nAsynchNumTopologySets       , stats.numTopologySets);
		DrxInterlockedAdd(&pipelineStats.m_nAsynchDIPs[renderList]      , stats.numDIPs);
		DrxInterlockedAdd(&pipelineStats.m_nAsynchPolygons[renderList]  , stats.numPolygons);
	}
...
void CRenderer::ResumeRendererFromFrameEnd()
{
	...
	{
		m_mtxStopAtRenderFrameEnd.Unlock();
		DrxFatalError("Trying to resume render thread but render thread was not stopped before. Use StopRendererAtFrameEnd() prior using ResumeRendererFromFrameEnd().");
	}
	m_bStopRendererAtFrameEnd = false;
	m_mtxStopAtRenderFrameEnd.Unlock();
...
void CRenderer::RT_DelayedDeleteResources(bool bAllResources)
{
	...
	i32 buffer = bAllResources ? 0 : m_currentResourceDeleteBuffer;
	i32k bufferEnd = bAllResources ? RT_COMMAND_BUF_COUNT : buffer + 1;
...
alloc_info_struct* CRenderer::GetFreeChunk(i32 bytes_count, i32 nBufSize, PodArray<alloc_info_struct>& alloc_info, tukk szSource)
{
	i32 best_i = -1;
	i32 min_size = 10000000;

	// find best chunk
	for (i32 i = 0; i < alloc_info.Count(); i++)
	{
	...
	if (best_i >= 0)
	{
		...
		i32 bytes_free = alloc_info[best_i].bytes_num - bytes_count;
		if (bytes_free > 0)
		{
			// modify reused shunk
	...
	i32 piplevel = alloc_info.Count() ? (alloc_info.Last().ptr - alloc_info[0].ptr) + alloc_info.Last().bytes_num : 0;
...
bool CRenderer::ReleaseChunk(i32 p, PodArray<alloc_info_struct>& alloc_info)
{
	for (i32 i = 0; i < alloc_info.Count(); i++)
	{
		if (alloc_info[i].ptr == p)
		{
			...
			// merge unused chunks
			for (i32 s = 0; s < alloc_info.Count() - 1; s++)
			{
