...
//
//	File:Renderer.cpp
//  Описание: Abstract renderer API
//
//	История:
...
#include <drx3D/Render/D3D/CompiledRenderObject.h>
...
///////////////////////////////////////////////////////////////////////////////
static inline void AddEf_HandleForceFlags(i32& nList, i32& nAW, u32& nBatchFlags, u32k nShaderFlags, u32k nShaderFlags2, CRenderObject* obj)
{
	// Force rendering in last place
	// FIXME: If object is permanent this is wrong!
	// branchless

	i32k sort1 = nz2mask(nShaderFlags2 & EF2_FORCE_DRAWLAST);
	i32k sort2 = nz2one(nShaderFlags2 & EF2_FORCE_DRAWFIRST);
	...
	{
		u32 predicate = nz2mask(nShaderFlags2 & EF2_AFTERHDRPOSTPROCESS);

		u32k mask = nz2mask(nShaderFlags2 & EF2_FORCE_DRAWLAST);
...
///////////////////////////////////////////////////////////////////////////////
void CRenderer::EF_AddEf_NotVirtual(CRenderElement* re, SShaderItem& SH, CRenderObject* obj, const SRenderingPassInfo& passInfo, i32 nList, i32 nAW)
{
	...
	u32k nCloakRenderedMask = mask_nz_zr(nMaterialLayers & MTL_LAYER_BLEND_CLOAK, pShaderResources ? static_cast<CShaderResources*>(pShaderResources)->CShaderResources::GetMtlLayerNoDrawFlags() & MTL_LAYER_CLOAK : 0);
	...
	u32 nBatchFlags = EF_BatchFlags(SH, obj, re, passInfo, nAW);
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void CRenderer::EF_AddEf(CRenderElement* re, SShaderItem& pSH, CRenderObject* obj, const SRenderingPassInfo& passInfo, i32 nList, i32 nAW)
{
	EF_AddEf_NotVirtual(re, pSH, obj, passInfo, nList, nAW);
}

//////////////////////////////////////////////////////////////////////////
u16 CRenderer::PushFogVolumeContribution(const ColorF& fogVolumeContrib, const SRenderingPassInfo& passInfo)
{
...
//////////////////////////////////////////////////////////////////////////
void CRenderer::GetFogVolumeContribution(u16 idx, ColorF& rColor) const
{
...
//////////////////////////////////////////////////////////////////////////
u32 CRenderer::EF_BatchFlags(SShaderItem& SH, CRenderObject* pObj, CRenderElement* re, const SRenderingPassInfo& passInfo, i32 nAboveWater)
{
...

	{
		...
		u32k mask = mask_nz_zr((u32)CV_r_ZPassOnly, nFlags & (FB_TRANSPARENT));
...
///////////////////////////////////////////////////////////////////////////////
CRenderObject* CRenderer::EF_GetObject_Temp(i32 nThreadID)
{
	DrxAutoCriticalSection lock(m_renderObjectAccessLock);
...
void CMotionBlur::SetupObject(CRenderObject* pObj, const SRenderingPassInfo& passInfo)
{
	...
	u32 nThreadID = passInfo.ThreadID();
	...
	{
		u32k nFrameID = passInfo.GetMainFrameID();
		const uintptr_t ObjID = pOD ? pOD->m_uniqueObjectId : 0;
		u32k nObjFrameWriteID = (nFrameID) % 3;
		...
		if (it != m_pOMBData[nObjFrameWriteID].end())
		{
			...
			u32k nObjPrevFrameID = (nFrameID - 1) % 3;
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void SRendItem::mfSortPreprocess(SRendItem* First, i32 Num)
{
	std::sort(First, First + Num, SCompareItemPreprocess());
}

//////////////////////////////////////////////////////////////////////////
void SRendItem::mfSortForZPass(SRendItem* First, i32 Num)
{
	std::sort(First, First + Num, SCompareRendItemZPass());
}

//////////////////////////////////////////////////////////////////////////
void SRendItem::mfSortByLight(SRendItem* First, i32 Num, bool bSort, const bool bIgnoreRePtr, bool bSortDecals)
{
	if (bSort)
	{
		if (bIgnoreRePtr)
...
//////////////////////////////////////////////////////////////////////////
void SRendItem::mfSortByDist(SRendItem* First, i32 Num, bool bDecals, bool InvertedOrder)
{
	...
	if (!bDecals)
	{
		...
		i32k iPrefetchLoopLastIndex = min_branchless(8, Num);
		...
		for (i = 0; i < Num; i++)
		{
			...
			i32 iPrefetchIndex = min_branchless(i + 8, iLastValidIndex);
...
{
	assert(IsStreamed());
	u32k nMaxExtent = max(m_nWidth, m_nHeight);
