...
CDrxNameTSCRC CShaderMan::s_cNameHEAD;
...
SEfTexModPool::ModificatorList SEfTexModPool::s_pool;
 i32 SEfTexModPool::s_lockState = 0;

SEfTexModificator* SEfTexModPool::Add(SEfTexModificator& mod)
{
	mod.m_crc = CCrc32::Compute((tukk)&mod, sizeof(SEfTexModificator) - sizeof(u16) - sizeof(u32));
	...
	s_pool.insert(std::pair<u32, SEfTexModificator*>(mod.m_crc, pMod));
...
void SEfTexModPool::Update(SEfTexModificator*& pMod, SEfTexModificator& newMod)
{
	...
	if (pMod)
	{
		...
		else if (memcmp(pMod, &newMod, sizeof(SEfTexModificator) - sizeof(u16) - sizeof(u32)))            // Ignore reference count and crc
		{
			Remove_NoLock(pMod);
			pMod = Add(newMod);
		}
...
{
	DrxSpinLock(&s_lockState, 0, 1);
	// Locked
}

void SEfTexModPool::Unlock(void)
{
	DrxSpinLock(&s_lockState, 1, 0);
	// Unlocked
}
#endif
...
void CShader::GetMemoryUsage(IDrxSizer* pSizer) const
{
	pSizer->Add(*this);
...
#if DRX_PLATFORM_WINDOWS && DRX_PLATFORM_64BIT
	#pragma warning( push )               //AMD Port
	#pragma warning( disable : 4311 )     // I believe the i32 cast below is okay.
#endif
...
CShader& CShader::operator=(const CShader& src)
{
	...
	i32 Offs = (i32)(INT_PTR)&(((CShader*)0)->m_eSHDType);
	byte* d = (byte*)this;
	byte* s = (byte*)&src;
...
SShaderTechnique* CShader::GetTechnique(i32 nStartTechnique, i32 nRequestedTechnique, bool bSilent)
{
	...
	if (m_HWTechniques.Num())
	{
		...
		{
			assert(nStartTechnique < (i32)m_HWTechniques.Num());
			if (nStartTechnique < (i32)m_HWTechniques.Num())
			{
	...
	if (!pTech ||
	    ...
	    pTech->m_nTechnique[nRequestedTechnique] >= (i32)m_HWTechniques.Num())
	{
		if (!bSilent)
...
void CShaderResources::PostLoad(CShader* pSH)
{
	...
	if (pSH && (pSH->m_Flags & EF_SKY))
	{
		if (m_Textures[EFTT_DIFFUSE])
		{
			char sky[128];
			char path[1024];
			drx_strcpy(sky, m_Textures[EFTT_DIFFUSE]->m_Name.c_str());
			i32 size = strlen(sky);
			tukk ext = PathUtil::GetExt(sky);
			while (sky[size] != '_')
			{
				size--;
				if (!size)
...
SShaderTexSlots* CShader::GetUsedTextureSlots(i32 nTechnique)
{
...
CTexture* CShader::mfFindBaseTexture(TArray<SShaderPass>& Passes, i32* nPass, i32* nTU)
{
	return NULL;
}

ITexture* CShader::GetBaseTexture(i32* nPass, i32* nTU)
{
	for (u32 i = 0; i < m_HWTechniques.Num(); i++)
	{
...
u32 CShader::GetUsedTextureTypes(void)
{
...
void CShaderMan::mfCreateCommonGlobalFlags(tukk szName)
{
	// Create globals.txt

	assert(szName);
	u32 nCurrMaskCount = 0;
	tukk pszShaderExtPath = "Shaders/";

	struct _finddata_t fileinfo;
	intptr_t handle;
	char dirn[256];
	drx_strcpy(dirn, pszShaderExtPath);
	drx_strcat(dirn, "*.*");

	handle = gEnv->pDrxPak->FindFirst(dirn, &fileinfo);
	...
	do
	{
		...
		tukk pszExt = PathUtil::GetExt(fileinfo.name);
		if (stricmp(pszExt, "ext"))
			continue;

		char pszFileName[256];
		drx_sprintf(pszFileName, "%s%s", pszShaderExtPath, fileinfo.name);

		FILE* fp = gEnv->pDrxPak->FOpen(pszFileName, "r");
		if (fp)
		{
			gEnv->pDrxPak->FSeek(fp, 0, SEEK_END);
			i32 len = gEnv->pDrxPak->FTell(fp);
			tuk buf = new char[len + 1];
			gEnv->pDrxPak->FSeek(fp, 0, SEEK_SET);
			len = gEnv->pDrxPak->FRead(buf, len, fp);
			gEnv->pDrxPak->FClose(fp);
			buf[len] = 0;

			// Check if global flags are common
			tuk pCurrOffset = strstr(buf, "UsesCommonGlobalFlags");
			if (pCurrOffset)
			{
				// add shader to list
				...
				while (pCurrOffset = strstr(pCurrOffset, "Name"))
				{
					...
					char dummy[256] = "\n";
					char name[256] = "\n";
					i32 res = sscanf(pCurrOffset, "%255s %255s", dummy, name);         // Get flag name..
					assert(res);

					string pszNameFlag = name;
					i32 nSzSize = pszNameFlag.size();
...
void CShaderMan::mfSaveCommonGlobalFlagsToDisk(tukk szName, u32 nMaskCount)
{
	// Write all flags
	assert(nMaskCount);

	FILE* fp = gEnv->pDrxPak->FOpen(szName, "w");
	if (fp)
	{
		gEnv->pDrxPak->FPrintf(fp, "FX_CACHE_VER %f\n", FX_CACHE_VER);
		gEnv->pDrxPak->FPrintf(fp, "%s\n\n", m_pShadersRemapList.c_str());
...
void CShaderMan::mfInitCommonGlobalFlags(void)
{
	...
	FILE* fp = gEnv->pDrxPak->FOpen(pszGlobalsPath.c_str(), "r", IDrxPak::FOPEN_HINT_QUIET);
	if (fp)
	{
		char str[256] = "\n";
		char name[128] = "\n";

		gEnv->pDrxPak->FGets(str, 256, fp);
		if (strstr(str, "FX_CACHE_VER"))
		{
			float fCacheVer = 0.0f;
			i32 res = sscanf(str, "%127s %f", name, &fCacheVer);
			assert(res);
...
{
	m_ResLookupDataMan[static_cast<i32>(cacheSource::readonly)].Clear();
	string dirdatafilename("%ENGINE%/" + string(m_ShadersCache));
	dirdatafilename += "lookupdata.bin";
	m_ResLookupDataMan[static_cast<i32>(cacheSource::readonly)].LoadData(dirdatafilename.c_str(), CParserBin::m_bEndians, true);

	m_ResLookupDataMan[static_cast<i32>(cacheSource::user)].Clear();
	dirdatafilename = m_szUserPath + string(m_ShadersCache);
	dirdatafilename += "lookupdata.bin";
	m_ResLookupDataMan[static_cast<i32>(cacheSource::user)].LoadData(dirdatafilename.c_str(), CParserBin::m_bEndians, false);
}
...
void CShaderMan::mfInit(void)
{
	...
	s_cNameHEAD = CDrxNameTSCRC("HEAD");
	...
	if (!m_bInitialized)
	{
		...
		m_ShadersGamePath = gEnv->pDrxPak->GetGameFolder() + string("/GameShaders/HWScripts/");
		m_ShadersGameExtPath = gEnv->pDrxPak->GetGameFolder() + string("/GameShaders/");
		m_ShadersMergeCachePath = "Shaders/MergeCache/";
#if (DRX_PLATFORM_LINUX && DRX_PLATFORM_32BIT) || DRX_PLATFORM_ANDROID
		...
		gEnv->pDrxPak->OpenPack(PathUtil::GetGameFolder() + "/", "%ENGINEROOT%/Engine/Shaders.pak",
		                        IDrxPak::FLAGS_PAK_IN_MEMORY | IDrxPak::FLAGS_PATH_REAL);
		...
		if (!CRenderer::CV_r_shadersAllowCompilation)
		{
...

			{
				...
				//				"files are write protected / media is read only / windows user setting don't allow file changes","drx3D",MB_ICONEXCLAMATION|MB_OK);
...
{
	return true; //gEnv->pDrxPak->OpenPack(PathUtil::GetGameFolder() + "/", "Engine/ShaderCacheStartup.pak", IDrxPak::FLAGS_PAK_IN_MEMORY | IDrxPak::FLAGS_PATH_REAL);
}
...
void CShaderMan::UnloadShaderStartupCache()
{
	...
	//gEnv->pDrxPak->ClosePack("Engine/ShaderCacheStartup.pak");
}
...
void CShaderMan::ParseShaderProfile(tuk scr, SShaderProfile* pr)
{
	tuk name;
	long cmd;
	tuk params;
...
void CShaderMan::ParseShaderProfiles()
{
	...
	tuk scr = NULL;
	FILE* fp = gEnv->pDrxPak->FOpen("Shaders/ShaderProfiles.txt", "rb");
	if (fp)
	{
		gEnv->pDrxPak->FSeek(fp, 0, SEEK_END);
...
{
	assert((i32)eST < sizeof(m_cEF.m_ShaderProfiles) / sizeof(SShaderProfile));
...
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CShaderMan::RT_SetShaderQuality");

	eSQ = CLAMP(eSQ, eSQ_Low, eSQ_VeryHigh);
	if (eST == eST_All)
	{
		for (i32 i = 0; i < eST_Max; i++)
		{
...
static bool sLoadShader(tukk szName, CShader*& pStorage)
{
...
bool CShaderMan::mfGatherShadersList(tukk szPath, bool bCheckIncludes, bool bUpdateCRC, std::vector<string>* Names)
{
	struct _finddata_t fileinfo;
	intptr_t handle;
	char nmf[256];
...
void CShaderMan::mfGatherFilesList(tukk szPath, std::vector<CDrxNameR>& Names, i32 nLevel, bool bUseFilter, bool bMaterial)
{
	struct _finddata_t fileinfo;
	intptr_t handle;
	char nmf[256];
...
i32 CShaderMan::mfInitShadersList(std::vector<string>* Names)
{
...
void CShaderMan::mfPreloadShaderExts(void)
{
	...
	handle = gEnv->pDrxPak->FindFirst("Shaders/*.ext", &fileinfo);
	if (handle == -1)
		return;
	do
	{
...
CShader* CShaderMan::mfNewShader(tukk szName)
{
	CShader* pSH;

	CDrxNameTSCRC className = CShader::mfGetClassName();
	CDrxNameTSCRC Name = szName;
...
SEnvTexture* SHRenderTarget::GetEnv2D()
{
	...
	if (m_nIDInPool >= 0)
	{
		assert(m_nIDInPool < (i32)CRendererResources::s_CustomRT_2D.Num());
		if (m_nIDInPool < (i32)CRendererResources::s_CustomRT_2D.Num())
...
void SHRenderTarget::GetMemoryUsage(IDrxSizer* pSizer) const
{
	pSizer->Add(*this);
...
void SEfResTexture::Update(i32 nTSlot, u32& nMDMask)
{
	FUNCTION_PROFILER_RENDER_FLAT
...
void SEfResTexture::UpdateWithModifier(i32 nTSlot, u32& nMDMask)
{
	CRenderer* rd = gRenDev;
	i32 nFrameID = gRenDev->GetRenderFrameID();
	i32 recursion = 0;
		...
		case ETMM_Jitter:
			{
				...
				{
					pMod->m_CurrentJitter[0] = drx_random(0.0f, pMod->m_OscAmplitude[0]);
					pMod->m_LastTime[0] = pMod->m_OscPhase[0] + floor_tpl(Su);
				}
		...
		case ETMM_Jitter:
			{
				...
				{
					pMod->m_CurrentJitter[1] = drx_random(0.0f, pMod->m_OscAmplitude[1]);
					pMod->m_LastTime[1] = pMod->m_OscPhase[1] + floor_tpl(Sv);
				}
...
float CShaderMan::EvalWaveForm(SWaveForm* wf)
{
	...
	tukk sShaderName = "Unknown"; //gRenDev->m_RP.m_pShader->GetName();
	...
	case eWF_Sin:
		val = (i32)((animationTime * Freq + Phase) * (float)SStaticSinusTable::sSinTableCount);
...
float CShaderMan::EvalWaveForm(SWaveForm2* wf)
{
	...
	case eWF_Sin:
		val = (i32)((animationTime * wf->m_Freq + wf->m_Phase) * (float)SStaticSinusTable::sSinTableCount);
...
EHWShaderClass CHWShader::mfStringProfile(tukk profile)
{
...
EHWShaderClass CHWShader::mfStringClass(tukk szClass)
{
	EHWShaderClass Profile = eHWSC_Num;
...
tukk CHWShader::mfProfileString(EHWShaderClass eClass)
{
	tuk szProfile = "Unknown";
	switch (eClass)
	{
	case eHWSC_Vertex:
...
tukk CHWShader::mfClassString(EHWShaderClass eClass)
{
	tuk szClass = "Unknown";
	switch (eClass)
	{
...
void CShaderMan::FilterShaderCacheGenListForOrbis(FXShaderCacheCombinations& combinations)
{
	...
	static tukk const szFilter = REGISTER_STRING("r_ShadersOrbisFiltering", "L", VF_NULL, "")->GetString();
	for (tukk szFilterItem = szFilter; szFilterItem && *szFilterItem; ++szFilterItem)
	{
		switch (*szFilterItem)
		{
		case 'l':
	...
	for (auto& item : combinations)
	{
		...
		{
			u8 isa = item.second.Ident.m_pipelineState.GNM.GetISA(item.second.eCL);
			const auto insertIsa = [&result, &item, isa](u8 targetIsa)
			{
				...
				result[CDrxNameR(name.c_str())] = item.second;
			};
