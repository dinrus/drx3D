...
#include <drx3D/CoreX/Renderer/RendElements/CREFogVolume.h>
                          ...
                          CGraphicsPipelineStateLocalCache* pStateCache)
{
	...
	bFullyCompiled &= forwardStage.CreatePipelineState(_stateDesc, stateArray[u32(EPsoId::ViewerOustideVolume)], CSceneForwardStage::ePass_Forward, customRenderStateOutside);

	// rendered with no depth test, cull front.
	_stateDesc.technique = TTYPE_GENERAL;
	bFullyCompiled &= forwardStage.CreatePipelineState(_stateDesc, stateArray[u32(EPsoId::ViewerInsideVolume)], CSceneForwardStage::ePass_Forward, nullptr);
	...
	bFullyCompiled &= forwardStage.CreatePipelineState(_stateDesc, stateArray[u32(EPsoId::ViewerInsideVolumeNearCutoff)], CSceneForwardStage::ePass_Forward, customRenderStateNearCutoff);
...
bool CREFogVolume::Compile(CRenderObject* pObj,CRenderView *pRenderView, bool updateInstanceDataOnly)
{
	...
	DRX_ASSERT(compiledObj.m_pPerDrawRS);
...
void CREFogVolume::DrawToCommandList(CRenderObject* pObj, const struct SGraphicsPipelinePassContext& ctx, CDeviceCommandList* commandList)
{
	...
	u32 psoId = compiledObj.m_bViewerInsideVolume
	               ? (compiledObj.m_bNearCutoff ? u32(render_element::fogvolume::EPsoId::ViewerInsideVolumeNearCutoff) : u32(render_element::fogvolume::EPsoId::ViewerInsideVolume))
	               : u32(render_element::fogvolume::EPsoId::ViewerOustideVolume);
...
void CREFogVolume::UpdatePerDrawCB(render_element::fogvolume::SCompiledFogVolume& RESTRICT_REFERENCE compiledObj, const CRenderObject& renderObj) const
{
	...
	DrxStackAllocWithSize(render_element::fogvolume::SPerDrawConstantBuffer, cb, CDeviceBufferUpr::AlignBufferSizeForStreaming);
...
bool CREFogVolume::UpdateVertex(render_element::fogvolume::SCompiledFogVolume& RESTRICT_REFERENCE compiledObj)
{
	...
	if (m_handleIndexBuffer == render_element::fogvolume::invalidBufferHandle)
	{
		...
		if (m_handleIndexBuffer != render_element::fogvolume::invalidBufferHandle)
		{
			DRX_ASSERT(gRenDev->m_DevBufMan.Size(m_handleIndexBuffer) >= indexBufferSize);

			// NOTE: Get aligned stack-space (pointer and size aligned to manager's alignment requirement)
			DrxStackAllocWithSizeVector(u16, numIndices, volumeIndices, CDeviceBufferUpr::AlignBufferSizeForStreaming);

			static u16k bbIndices[numIndices] =
			{
				0, 1, 2, 0, 2, 3,
				7, 6, 5, 7, 5, 4,
				3, 2, 6, 3, 6, 7,
	...
	{
		DRX_ASSERT(gRenDev->m_DevBufMan.Size(m_handleVertexBuffer) >= vertexBufferSize);

		// update vertex buffer once and keep it.
		{
			// NOTE: Get aligned stack-space (pointer and size aligned to manager's alignment requirement)
			DrxStackAllocWithSizeVector(SVF_P3F_C4B_T2F, numVertices, volumeVertices, CDeviceBufferUpr::AlignBufferSizeForStreaming);
