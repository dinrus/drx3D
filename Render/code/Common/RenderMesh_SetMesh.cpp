...
static void transfer_writecombined(uk pDst, ukk pSrc, size_t size)
{
	drxMemcpy(pDst, pSrc, size, MC_CPU_TO_GPU);
}
...
template<EStreamIDs stream, size_t Size>
u32 CompactStream(u8 (&buffer)[Size], SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
{
...
{
template<typename T>
void CompactPositions(T* pVBuff, SSetMeshIntData& data, CMesh& mesh, const Vec3& posOffset, u32 beg, u32 end)
{
...
template<>
void CompactPositions(SVF_P3F_C4B_T2F* pVBuff, SSetMeshIntData& data, CMesh& mesh, const Vec3& posOffset, u32 beg, u32 end)
{
	if (mesh.m_pPositionsF16)
		for (size_t i = 0; i < end; ++i)
...
template<typename T>
void CompactUVs(T* pVBuff, SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
{
	if (mesh.m_pTexCoord)
		for (size_t i = 0; i < end; ++i)
...
template<>
void CompactUVs(SVF_P3F_C4B_T2F* pVBuff, SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
{
	if (mesh.m_pTexCoord)
		for (size_t i = 0; i < end; ++i)
...
{
	template<typename T>
	static void CompactPositions(T* pVBuff, SSetMeshIntData& data, CMesh& mesh, const Vec3& posOffset, u32 beg, u32 end)
	{
		return StreamCompactorDetail::CompactPositions<T>(pVBuff, data, mesh, posOffset, beg, end);
	}

	static void CompactNormals(SVF_P3S_N4B_C4B_T2S* pVBuff, SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
	{
		if (mesh.m_pNorms)
			for (size_t i = 0; i < end; ++i)
			{
	...
	template<typename T>
	static void CompactColors0(T* pVBuff, SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
	{
		if (mesh.m_pColor0)
			for (size_t i = 0; i < end; ++i)
			{
	...
	static void CompactColors1(SVF_P2S_N4B_C4B_T1F* pVBuff, SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
	{
		if (mesh.m_pColor1)
			for (size_t i = 0; i < end; ++i)
			{
	...
	template<typename T>
	static void CompactUVs(T* pVBuff, SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
	{
		StreamCompactorDetail::CompactUVs<T>(pVBuff, data, mesh, beg, end);
	}

	static u32 Compact(u8 (&buffer)[Size], SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
	{
		...
		if (data.m_pVBuff)
		{
			...
			if (data.m_pMesh->m_pPositions)
			{
				...
				amount = min((u32)(end - beg), (u32)(Size / sizeof(pVBuff[0])));
		...
		DrxFatalError("CRenderMesh::SetMesh_Impl: invalid vertex format for general stream");
		return 0;
	}
};
...
struct StreamCompactor<VSF_TANGENTS, Size>
{
	static u32 Compact(u8 (&buffer)[Size], SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
	{
		...
		u32 amount = min((u32)(end - beg), (u32)(Size / sizeof(pTBuff[0])));
...
struct StreamCompactor<VSF_QTANGENTS, Size>
{
	static u32 Compact(u8 (&buffer)[Size], SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
	{
		...
		u32 amount = min((u32)(end - beg), (u32)(Size / sizeof(pQTBuff[0])));
...
struct StreamCompactor<VSF_NORMALS, Size>
{
	static u32 Compact(u8 (&buffer)[Size], SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
	{
		...
		u32 amount = min((u32)(end - beg), (u32)(Size / sizeof(data.m_pNormalsBuff[0])));
...
struct StreamCompactor<VSF_VERTEX_VELOCITY, Size>
{
	static u32 Compact(u8 (&buffer)[Size], SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
	{
		...
		u32 amount = min((u32)(end - beg), (u32)(Size / sizeof(data.m_pVelocities[0])));
...
template<size_t Size>
u32 CompactIndices(u8 (&buffer)[Size], SSetMeshIntData& data, CMesh& mesh, u32 beg, u32 end)
{
	...
	u32 amount = min((u32)(end - beg), (u32)(Size / sizeof(mesh.m_pIndices[0])));
...
typedef DRX_ALIGN (128) u8 AlignedStagingBufferT[(8 << 10) + 128];
...
void CRenderMesh::SetMesh_IntImpl(SSetMeshIntData data)
{
	...
	//////////////////////////////////////////////////////////////////////////
	for (u32 iter = 0; iter < data.m_nVerts; iter += CompactStream<VSF_GENERAL>(stagingBuffer, data, mesh, iter, data.m_nVerts))
		;
	for (u32 iter = 0; iter < data.m_nVerts; iter += CompactStream<VSF_TANGENTS>(stagingBuffer, data, mesh, iter, data.m_nVerts))
		;
	for (u32 iter = 0; iter < data.m_nVerts; iter += CompactStream<VSF_QTANGENTS>(stagingBuffer, data, mesh, iter, data.m_nVerts))
		;
#if ENABLE_NORMALSTREAM_SUPPORT
	for (u32 iter = 0; iter < data.m_nVerts; iter += CompactStream<VSF_NORMALS>(stagingBuffer, data, mesh, iter, data.m_nVerts))
		;
#endif
	for (u32 iter = 0; iter < data.m_nVerts; iter += CompactStream<VSF_VERTEX_VELOCITY>(stagingBuffer, data, mesh, iter, data.m_nVerts))
		;
	for (u32 iter = 0; iter < data.m_nInds; iter += CompactIndices(stagingBuffer, data, mesh, iter, data.m_nInds))
		;
}
