...
#include <drx3D/Render/ShaderResources.h>
...
size_t CShaderResources::GetResourceMemoryUsage(IDrxSizer* pSizer)
{
...
{
	if (DrxInterlockedDecrement(&m_nRefCounter) <= 0) // is checked inside render thread
	{
...
{
	if (DrxInterlockedDecrement(&m_nRefCounter) <= 0)
	{
		delete this;
	}
}
...
void CShaderResources::SetInputLM(const CInputLightMaterial& lm)
{
	...
	memcpy(pDst, lm.m_Channels, min(i32(EFTT_MAX), REG_PM_DIFFUSE_COL / 2) * sizeof(lm.m_Channels[0]));
...
static tuk sSetParameterExp(tukk szExpr, Vec4& vVal, DynArray<SShaderParam>& Params, bool& bResult);

static tuk sSetOperand(tuk szExpr, Vec4& vVal, DynArray<SShaderParam>& Params, bool& bResult)
{
	char temp[256];
...
static bool sSetParameter(SFXParam* pPR, Vec4* Constants, DynArray<SShaderParam>& Params, i32 nOffs, EHWShaderClass eSHClass)
{
	u32 nFlags = pPR->GetParamFlags();
	bool bFound = false;
	for (i32 i = 0; i < 4; i++)
	{
		if (nFlags & PF_AUTOMERGED)
		{
			DrxFixedStringT<128> n;
			pPR->GetCompName(i, n);
			bool bF = SShaderParam::GetValue(n.c_str(), &Params, &Constants[pPR->m_nRegister[eSHClass] - nOffs][0], i);
			if (!bF)
			{
				DrxFixedStringT<128> v;
...
{
	u32 nMergeMaskPerShaderClass[eHWSC_Num] = { 2, 1, 2, 2, 2, 2 };
	u32 nMergeMask = nMergeMaskPerShaderClass[shaderClass];

	i32k numFXParams = FXParams.m_FXParams.size();
	for (i32 n = 0; n < numFXParams; n++)
	{
...
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CShaderResources::RT_UpdateConstants");
	//assert(gRenDev->m_pRT->IsRenderThread());
	...
	for (i32 i = 0; i < m_ShaderParams.size(); i++)
	{
		...
		for (i32 j = 0; j < PublicParams.size(); j++)
		{
	...
	for (i32 i = 0; i < (i32)pSH->m_HWTechniques.Num(); i++)
	{
		SShaderTechnique* pTech = pSH->m_HWTechniques[i];
		for (i32 j = 0; j < (i32)pTech->m_Passes.Num(); j++)
		{
	...
	for (EEfResTextures texType = EFTT_DIFFUSE; texType < EFTT_MAX; texType = EEfResTextures(texType + 1))
	{
		...
		if (m_Textures[texType])
		{
			...
			if (smp.m_pDynTexSource)
			{
				...
				{
					m_flags = flags;
					DRX_ASSERT(m_resources.HasChanged());
