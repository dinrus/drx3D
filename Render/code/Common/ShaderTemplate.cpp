...
u32 SShaderItem::PostLoad()
{
	u32 nPreprocessFlags = 0;
...
EEfResTextures CShaderMan::mfCheckTextureSlotName(tukk mapname)
{
...
CTexture* CShaderMan::mfCheckTemplateTexName(tukk mapname, ETEX_Type eTT)
{
...
tukk CShaderMan::mfTemplateTexIdToName(i32 Id)
{
	switch (Id)
	{
	case EFTT_DIFFUSE:
...
STexAnim* CShaderMan::mfReadTexSequence(tukk na, i32 Flags, bool bFindOnly)
{
	...
	i32 n;
	CTexture* tx, * tp;
	i32 startn, endn, nums;
	char name[_MAX_PATH];

	tx = NULL;

	drx_strcpy(name, na);
	tukk ext = PathUtil::GetExt(na);
...
i32 CShaderMan::mfReadTexSequence(STexSamplerRT* smp, tukk na, i32 Flags, bool bFindOnly)
{
	STexAnim* ta;
...
CTexture* CShaderMan::mfTryToLoadTexture(tukk nameTex, STexSamplerRT* smp, i32 Flags, bool bFindOnly)
{
	if (nameTex && strchr(nameTex, '#')) // test for " #" to skip max material names
	{
		i32 n = mfReadTexSequence(smp, nameTex, Flags, bFindOnly);
	}
...
CTexture* CShaderMan::mfFindResourceTexture(tukk nameTex, tukk path, i32 Flags, SEfResTexture* Tex)
{
...
CTexture* CShaderMan::mfLoadResourceTexture(tukk nameTex, tukk path, i32 Flags, SEfResTexture* Tex)
{
	mfSetResourceTexState(Tex);
...
bool CShaderMan::mfLoadResourceTexture(EEfResTextures Id, SInputShaderResources& RS, u32 CustomFlags, bool bReplaceMeOnFail)
{
	bool bReturn = false;
...
bool CShaderMan::mfLoadResourceTexture(EEfResTextures Id, CShaderResources& RS, u32 CustomFlags, bool bReplaceMeOnFail)
{
