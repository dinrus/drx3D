...
i32 CMotionBlur::Initialize()
{
	return 1;
}

i32 CMotionBlur::CreateResources()
{
...
i32 CDepthOfField::CreateResources()
{
...
i32 CRainDrops::CreateResources()
{
	Release();
	...
	for (i32 p = 0; p < m_nMaxDropsCount; p++)
	{
...
i32 CNightVision::CreateResources()
{
...
i32 CSonarVision::CreateResources()
{
	SAFE_RELEASE(m_pGradient);
	SAFE_RELEASE(m_pNoise);
...
i32 CThermalVision::CreateResources()
{
	Release();
...
bool CHudSilhouettes::Preprocess(const SRenderViewInfo& viewInfo)
{
	if ((CRenderer::CV_r_customvisions != 3) || (m_bSilhouettesOptimisedTechAvailable))
	{
		...
		u32 nBatchMask = pRenderView->GetBatchFlags(EFSLIST_GENERAL) | pRenderView->GetBatchFlags(EFSLIST_TRANSP_BW) | pRenderView->GetBatchFlags(EFSLIST_TRANSP_AW);
...
i32 CFilterKillCamera::Initialize()
{
...
bool CFilterKillCamera::Preprocess(const SRenderViewInfo& viewInfo)
{
	...
	{
		i32k mode = int_round(m_pMode->GetParam());
		if (mode != m_lastMode)
		{
			m_blindTimer = 0.0f;
			m_lastMode = mode;
...
i32 CNanoGlass::CreateResources()
{
	Release();
...
{
	// Main thread
	u32k nThreadID = passInfo.ThreadID();
...
{
	CRenderView* pRenderView = gcpRendD3D->GetGraphicsPipeline().GetCurrentRenderView();
	u32k batchMask = pRenderView->GetBatchFlags(EFSLIST_GENERAL)
...
void CMotionBlur::SetupObject(CRenderObject* pObj, const SRenderingPassInfo& passInfo)
{
	...
	if (pOD->m_pSkinningData)
	{
		...
		{
			gEnv->pJobUpr->WaitForJob(*pOD->m_pSkinningData->pAsyncJobs);
		}

		if (pOD->m_pSkinningData->pPreviousSkinningRenderData->pAsyncJobs)
		{
			gEnv->pJobUpr->WaitForJob(*pOD->m_pSkinningData->pPreviousSkinningRenderData->pAsyncJobs);
		}
	}
