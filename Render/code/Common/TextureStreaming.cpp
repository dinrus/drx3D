...
CTextureArrayAlloc<STexStreamOutState, CTexture::MaxStreamTasks> CTexture::s_StreamOutTasks;
 ...
 i32 CTexture::s_nBytesSubmittedToStreaming = { 0 };
 i32 CTexture::s_nMipsSubmittedToStreaming = { 0 };
 i32 CTexture::s_nNumStreamingRequests = { 0 };
i32 CTexture::s_nBytesRequiredNotSubmitted = 0;

#if !defined (_RELEASE) || defined(ENABLE_STATOSCOPE_RELEASE)
i32 CTexture::s_TextureUpdates = 0;
float CTexture::s_TextureUpdatesTime = 0.0f;
i32 CTexture::s_TexturesUpdatedRendered = 0;
float CTexture::s_TextureUpdatedRenderedTime = 0.0f;
i32 CTexture::s_StreamingRequestsCount = 0;
...
i32 CTexture::s_nTexturesDataBytesLoaded = 0;
 i32 CTexture::s_nTexturesDataBytesUploaded = 0;
i32 CTexture::s_nStatsAllocFails;
bool CTexture::s_bOutOfMemoryTotally;
 size_t CTexture::s_nStatsStreamPoolInUseMem;
 size_t CTexture::s_nStatsStreamPoolBoundMem;
 size_t CTexture::s_nStatsStreamPoolBoundPersMem;
 size_t CTexture::s_nStatsCurManagedNonStreamedTexMem = { 0 };
 size_t CTexture::s_nStatsCurDynamicTexMem = { 0 };
 size_t CTexture::s_nStatsStreamPoolWanted = { 0 };
...
DrxCriticalSection CTexture::s_streamFormatLock;
SStreamFormatCode CTexture::s_formatCodes[256];
u32 CTexture::s_nFormatCodes = 1;
...
void STexStreamInState::CopyMips()
{
	...
	if (!m_bAborted)
	{
		...
		{
			i32k nNewMipOffset = tp->m_nMinMipVidUploaded - m_nHigherUploadedMip;
			i32k nNumMips = tp->GetNumMips() - tp->m_nMinMipVidUploaded;
...
void CTexture::StreamReleaseMipsData(i32 nStartMip, i32 nEndMip)
{
	...
	for (i32 i = 0; i < nSides; i++)
		for (i32 j = nStartMip; j <= nEndMip; j++)
...
void STexStreamInState::Reset()
{
	...
	memset(&m_pTexture, 0, (tuk)(this + 1) - (tuk)&m_pTexture);
}
...
void STexStreamInState::StreamAsyncOnComplete(IReadStream* pStream, unsigned nError)
{
	...
	i32 nMip = (i32)pStream->GetUserData();
	...
	if (!nError && tp->m_pFileTexMips)
	{
...
#elif DRX_PLATFORM_DURANGO && (DRX_RENDERER_DIRECT3D >= 110) && (DRX_RENDERER_DIRECT3D < 120)
		ukk pTextureData = pStream->GetBuffer();
		...
		i32 nAbsMip = m_nHigherUploadedMip + nMip;
	...
	{
		i32 nChunkSize = tp->m_pFileTexMips->m_pMipHeader[nMip + m_nHigherUploadedMip].m_SideSize * tp->GetNumSides();
		DrxInterlockedAdd(&CTexture::s_nBytesSubmittedToStreaming, -nChunkSize);
	}
	DrxInterlockedDecrement(&CTexture::s_nMipsSubmittedToStreaming);
	assert(CTexture::s_nBytesSubmittedToStreaming >= 0);

	i32k nRef = DrxInterlockedDecrement(&m_nAsyncRefCount);
	...
	if (nRef == 0)
	{
		...
		if (!m_bAborted)
		{
			...
			if (pCmdList)
			{
				...
				for (i32 i = 0, c = m_nLowerUploadedMip - m_nHigherUploadedMip + 1; i != c; ++i)
	...
	if (!m_bAborted)
	{
...
#if !defined(TEXSTRM_ASYNC_UPLOAD) && !DRX_PLATFORM_DURANGO
		for (size_t i = 0, c = m_nLowerUploadedMip - m_nHigherUploadedMip + 1; i != c; ++i)
		{
		...
		if (!m_bValidLowMips)
		{
			...
			if (pCurItem)
			{
				...
				i32k nNumMips = pCurItem->m_pOwner->m_nMips;
				i32k nNewMipOffset = pNewPoolItem->m_pOwner->m_nMips - nNumMips;
		...
		if (pNewPoolItem)
		{
			...
			// bind new texture
			i32k nNewNumMips = m_nHigherUploadedMip;
...
i32 CTexture::StreamCalculateMipsSigned(float fMipFactor) const
{
	return StreamCalculateMipsSignedFP(fMipFactor) >> 8;
}

i32 CTexture::GetStreamableMipNumber() const
{
	assert(IsStreamed());
	return max(0, m_nMips - m_CacheFileHeader.m_nMipsPersistent);
}

bool CTexture::IsStreamedIn(i32k nMinPrecacheRoundIds[MAX_STREAM_PREDICTION_ZONES]) const
{
	if (IsStreamed())
	{
		for (i32 nZoneIdx = 0; nZoneIdx < MAX_STREAM_PREDICTION_ZONES; ++nZoneIdx)
		{
			if (m_streamRounds[nZoneIdx].nRoundUpdateId < nMinPrecacheRoundIds[nZoneIdx])
				return false;
		}

		i32 nMinMip = s_pTextureStreamer->GetMinStreamableMipWithSkip();
		return max(GetRequiredMip(), nMinMip) >= CTexture::GetMinLoadedMip();
	}
	return true;
}

i32 CTexture::GetStreamableMemoryUsage(i32 nStartMip) const
{
	assert(IsStreamed());
...
void CTexture::SetMinLoadedMip(i32 nMinMip)
{
#ifdef ENABLE_TEXTURE_STREAM_LISTENER
	if (m_nMinMipVidUploaded != nMinMip)
	{
		...
		if (pListener)
			pListener->OnTextureHasMip(this, min(nMinMip, (i32)m_nMips));
	}
#endif
...
const bool CTexture::IsParticularMipStreamed(float fMipFactor) const
{
	...
	i32 nMipClamp = (s_bStreamingFromHDD || bHighPriority) ? 0 : CRenderer::CV_r_TexturesStreamingMipClampDVD;
	i32k nMip = max(nMipClamp, CTexture::StreamCalculateMipsSigned(fMipFactor));
	return m_nMinMipVidUploaded <= nMip;
}

void CTexture::PrecacheAsynchronously(float fMipFactor, i32 nFlags, i32 nUpdateId, i32 nCounter)
{
	if (!IsStreamed())
		return;   // already done
...
void CTexture::StreamLoadFromCache(i32k Flags)
{
	if (IsUnloaded())
	{
...
bool CTexture::StreamPrepare(bool bFromLoad)
{
	...
	if (!m_pFileTexMips)
	{
		tukk szExt = PathUtil::GetExt(m_SrcName.c_str());
		if (szExt != 0 && (!stricmp(szExt, "tif") || !stricmp(szExt, "hdr")) && !gEnv->pDrxPak->IsFileExist(m_SrcName.c_str()))
		{
			m_SrcName = PathUtil::ReplaceExtension(m_SrcName, "dds");
			if (!gEnv->pDrxPak->IsFileExist(m_SrcName))
				return false;
		}
		...
		u32 nImageFlags = FIM_STREAM_PREPARE
...
bool CTexture::StreamPrepare(CImageFile* pIM)
{
	...
	i32 nWidth = pIM->mfGet_width();
	i32 nHeight = pIM->mfGet_height();
	i32 nDepth = pIM->mfGet_depth();
	i32 nMips = pIM->mfGet_numMips();
	ETEX_Type eTT = pIM->mfGet_NumSides() == 1 ? eTT_2D : eTT_Cube;
	i32 nSides = eTT != eTT_Cube ? 1 : 6;
	...
	i32 nMipsPersistent = max(pIM->mfGet_numPersistantMips(), DDSSplitted::GetNumLastMips(nWidth, nHeight, nMips, nSides, eSrcFormat, (m_eFlags & FT_ALPHA) ? FIM_ALPHA : 0));
	...
	fh.m_nSides = nSides;
	for (i32 iMip = 0; iMip < m_nMips; ++iMip)
	{
	...
	if (m_nMips > 1)
	{
		i32 nSyncStartMip = -1;
		i32 nSyncEndMip = -1;
		i32 nStartLowestM = max(0, m_nMips - m_CacheFileHeader.m_nMipsPersistent);
		...
		{
			nOffs = 0;
			for (i32 iMip = nSyncStartMip, nMipW = max(1, m_nWidth >> iMip), nMipH = max(1, m_nHeight >> iMip);
			     iMip <= nSyncEndMip;
			     ++iMip, nMipW = max(1, nMipW >> 1), nMipH = max(1, nMipH >> 1))
			{
				...
				i32 nSrcSideSize = CTexture::TextureDataSize(
					max(1, m_nWidth  >> iMip),
					max(1, m_nHeight >> iMip),
					max(1, m_nDepth  >> iMip),
					1, 1,
				...
				if (iSide == 0 || (m_eFlags & FT_REPLICATE_TO_ALL_SIDES) == 0)
				{
					...
					if (mp->DataArray)
					{
						drxMemcpy(&mp->DataArray[0], &buf[nOffs], nSrcSideSize);
					}
					nOffs += nSrcSideSize;
				}
				else if (iSide > 0)
				{
					if (m_eFlags & FT_REPLICATE_TO_ALL_SIDES)
					{
						...
						{
							drxMemcpy(&mp->DataArray[0], &mh.m_Mips[0].DataArray[0], nSrcSideSize);
						}
					}
					else
						assert(0);
	...
	// store file position on DVD
	{
		...
		{
			for (i32 i = 0; i < m_nMips - m_CacheFileHeader.m_nMipsPersistent; ++i)
			{
				i32k chunkNumber = i >= (m_nMips - m_CacheFileHeader.m_nMipsPersistent) ? 0 : m_nMips - i - m_CacheFileHeader.m_nMipsPersistent;
				...
				m_pFileTexMips->m_pMipHeader[i].m_eMediaType = gEnv->pDrxPak->GetFileMediaType(sLastChunkName);
			}
		}
	}
...
bool CTexture::StreamPrepare_Finalise(bool bFromLoad)
{
	...
	assert(m_pFileTexMips);
	for (i32 iSide = 0; iSide < m_CacheFileHeader.m_nSides; ++iSide)
	{
		for (i32 iMip = m_nMips - m_CacheFileHeader.m_nMipsPersistent; iMip < m_nMips; iMip++)
		{
			STexMipHeader& mh = m_pFileTexMips->m_pMipHeader[iMip];
			SMipData* mp = &mh.m_Mips[iSide];
			assert(mp->DataArray);
...
void CTexture::StreamValidateTexSize()
{
	...
	i32 nSizeManagedStreamed = 0;
	i32 nSizeManagedNonStreamed = 0;
	i32 nSizeDynamic = 0;
...
u8 CTexture::StreamComputeFormatCode(u32 nWidth, u32 nHeight, u32 nMips, ETEX_Format fmt)
{
	...
	u32 nMaxDim = 1 << (SStreamFormatCode::MaxMips - 1);

	// Determine how many missing tail mips there are
	u32 nFullMips = IntegerLog2(max(nWidth, nHeight)) + 1;
	u32 nTailMips = nFullMips - nMips;
	...
	SStreamFormatCodeKey key(nWidth, nHeight, fmt, (u8)nTailMips);

	DrxAutoLock<DrxCriticalSection> lock(s_streamFormatLock);
	...
	if (it == s_formatCodeMap.end())
	{
		...
		for (u32 nMip = nTailMips, nMipWidth = nWidth, nMipHeight = nHeight; nMip < SStreamFormatCode::MaxMips; ++nMip, nMipWidth = max(1u, nMipWidth >> 1), nMipHeight = max(1u, nMipHeight >> 1))
		{
			u32 nMip1Size = CTexture::TextureDataSize(nMipWidth, nMipHeight, 1, SStreamFormatCode::MaxMips - nMip, 1, fmt);
			...
			for (u32 nSlices = 1; nSlices <= 32; ++nSlices)
			{
				u32 nMipSize = CTexture::TextureDataSize(nMipWidth, nMipHeight, 1, SStreamFormatCode::MaxMips - nMip, nSlices, fmt);

				u32 nExpectedLinearSize = nMip1Size * nSlices;
				u32 nAlignedSlices = 1u << (32 - (nSlices > 1 ? countLeadingZeros32(nSlices - 1) : 32));
				u32 nExpectedPoTSize = nMip1Size * nAlignedSlices;
...
void CTexture::StreamUpdateStats()
{
	...
	{
		uk pBegunUsing[512];
		uk pStoppedUsing[512];

		i32 nBegun = 0, nStopped = 0;
		i32 curFrame = gRenDev->GetRenderFrameID();
		...
		for (std::vector<CTexture*>::const_iterator it = texs.begin(), itEnd = texs.end(); it != itEnd; ++it)
		{
			...
			if (curFrame - pTex->m_nAccessFrameID <= 2)
			{
				...
				if (!pTex->m_bUsedRecently)
				{
					...
					IF_UNLIKELY (nBegun == DRX_ARRAY_COUNT(pBegunUsing))
					{
			...
			else
			{
				if (pTex->m_bUsedRecently)
				{
					...
					IF_UNLIKELY (nStopped == DRX_ARRAY_COUNT(pStoppedUsing))
					{
...
bool CTexture::CanStreamInPlace(i32 nMip, STexPoolItem* pNewPoolItem)
{
...
bool CTexture::StartStreaming(CTexture* pTex, STexPoolItem* pNewPoolItem, i32k nStartMip, i32k nEndMip, i32k nActivateMip, EStreamTaskPriority estp)
{
	FUNCTION_PROFILER_RENDERER();
	...
	if (pTex->TryAddRef() > 0)
	{
		...
		if (pStreamState)
		{
			...
			i32 nDeltaMips = desc.nMips - pTex->m_nMips;
			...
			i32 nSides = pTex->GetNumSides();

#if DEVRES_USE_PINNING
			CDeviceTexture* pDevTex = pNewPoolItem->m_pDevTexture;
			tuk pPinnedBaseAddress = (tuk)pDevTex->Pin();
			...
			for (DDSSplitted::ChunkInfo* it = chunks, * itEnd = chunks + nNumChunks; it != itEnd; ++it)
			{
				...
				u32 nChunkMip = chunk.nMipLevel - nDeltaMips;
				u16 nMipIdx = nChunkMip - nStartMip;
			...
			if (nStreams)
			{
				...
				DrxInterlockedAdd(&s_nBytesSubmittedToStreaming, nSizeToSubmit);
				DrxInterlockedAdd(&s_nMipsSubmittedToStreaming, (i32)nStreams);
...
void CTexture::StreamUploadMips(i32 nStartMip, i32 nEndMip, STexPoolItem* pNewPoolItem)
{
	assert(pNewPoolItem);
	...
	if (s_bStreamDontKeepSystem && m_pFileTexMips && m_pFileTexMips->m_pPoolItem)
	{
		...
		i32 nSrcOffset = m_nMips - pSrcItem->m_pOwner->m_nMips;
		i32 nDstOffset = m_nMips - pNewPoolItem->m_pOwner->m_nMips;
...
void CTexture::InitStreaming()
{
	...
	SSysGlobEnv* pEnv = iSystem->GetGlobalEnvironment();

#if DRX_PLATFORM_DESKTOP
	if (CRenderer::CV_r_texturesstreaming)
	{
		i32 nMinTexStreamPool = 192;
		i32 nMaxTexStreamPool = 1536;
		...
		if (pICVarTexRes)
		{
			i32k valTexRes = pICVarTexRes->GetIVal();
			if (valTexRes == 0)
			{
...
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "CTexture::RT_FlushStreaming");
...
void CTexture::RT_FlushAllStreamingTasks(const bool bAbort /* = false*/)
{
...
#ifdef TEXSTRM_ASYNC_TEXCOPY
	{
		for (i32 i = 0; i < MaxStreamTasks; ++i)
		{
	...
	if (bAbort)
	{
		for (i32 i = 0; i < MaxStreamPrepTasks; ++i)
		{
...

	{
		for (i32 i = 0; i < MaxStreamTasks; ++i)
		{
			...
			if (ins.m_pTexture)
			{
				...
				for (i32 m = 0; m < (ins.m_nLowerUploadedMip - ins.m_nHigherUploadedMip) + 1; ++m)
				{
...
void CTexture::StreamState_Update()
{
	...
	for (u16 i = 0, c = s_StreamInTasks.GetNumLive(); i < MaxStreamTasks && c > 0; ++i)
	{
		...
		if (state.m_pTexture)
		{
			...
			else if (state.m_bAborted)
			{
				...
				for (i32 mi = 0, mc = state.m_nLowerUploadedMip - state.m_nHigherUploadedMip + 1; mi != mc; ++mi)
				{
...
void CTexture::StreamState_UpdatePrep()
{
	...
	for (u16 i = 0, c = s_StreamPrepTasks.GetNumLive(); i < MaxStreamPrepTasks && c > 0; ++i)
	{
...
STexStreamingInfo* CTexture::StreamState_AllocateInfo(i32 nMips)
{
	// Temporary - will be replaced by custom allocator later.
	STexStreamingInfo* pInfo = (STexStreamingInfo*)DrxModuleMalloc(sizeof(STexStreamingInfo));
