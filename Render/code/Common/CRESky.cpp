...
void CREHDRSky::GenerateSkyDomeTextures(i32 width, i32 height)
{
	SAFE_RELEASE(m_pSkyDomeTextureMie);
	SAFE_RELEASE(m_pSkyDomeTextureRayleigh);

	u32k creationFlags = FT_STATE_CLAMP | FT_NOMIPS | FT_DONT_STREAM;
	...
	, m_pShader(0)
{
	if (!LoadData())
	{
		DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_WARNING, "CStars: Failed to load data");
	}
}
...
{
	u32k c_fileTag(0x52415453);       // "STAR"
	u32k c_fileVersion(0x00010001);
...
void CStars::Render(bool bUseMoon)
{
	...
	if (m_pStarMesh && m_pShader && gRenDev->m_RP.m_nPassGroupID == EFSLIST_FORWARD_OPAQUE && (gRenDev->m_RP.m_nBatchFilter & FB_GENERAL) && starIntensity > 1e-3f)
	{
		...
		static CDrxNameTSCRC shaderTech("Stars");
		...
		i32 vpX(0), vpY(0), vpWidth(0), vpHeight(0);
		...
		static CDrxNameR vspnStarSize("StarSize");
		...
		static CDrxNameR pspnStarIntensity("StarIntensity");
		...
		i32 nRenderState = GS_BLSRC_ONE | GS_BLDST_ONE;
		...
		if (!FAILED(rd->FX_SetVertexDeclaration(0, EDefaultInputLayouts::P3S_C4B_T2S)))
		{
			...
			//uk pVB(m_pStarVB->GetStream(VSF_GENERAL, &offset));
