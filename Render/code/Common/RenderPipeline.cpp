...
struct SCompareRendItem
{
	bool operator()(const SRendItem& a, const SRendItem& b) const
	{
		i32 nMotionVectorsA = (a.ObjSort & FOB_HAS_PREVMATRIX);
		i32 nMotionVectorsB = (b.ObjSort & FOB_HAS_PREVMATRIX);
		if (nMotionVectorsA != nMotionVectorsB)
			return nMotionVectorsA > nMotionVectorsB;

		i32 nAlphaTestA = (a.ObjSort & FOB_ALPHATEST);
		i32 nAlphaTestB = (b.ObjSort & FOB_ALPHATEST);
...
struct SCompareRendItemZPass
{
	bool operator()(const SRendItem& a, const SRendItem& b) const
	{
		i32k layerSize = 50;  // Note: ObjSort contains round(entityDist * 2) for meshes
		...
		i32 depthLayerA = (a.ObjSort & 0xFFFF) / layerSize;
		i32 depthLayerB = (b.ObjSort & 0xFFFF) / layerSize;
