...
public:
	Verify()
	{
		for (i32 i = 0; s_TexSlotSemantics[i].def; i++)
		{
...
EEfResTextures FindTexSlot(tukk texSemantic)
{
	if (texSemantic)
	{
		for (i32 i = 0; s_TexSlotSemantics[i].semantic; i++)
		{
...
bool VerifyTexSuffix(EEfResTextures texSlot, tukk texPath)
{
	...
	return (strlen(texPath) > strlen(s_TexSlotSemantics[texSlot].suffix) && (DrxStringUtils::stristr(texPath, s_TexSlotSemantics[texSlot].suffix)));
}

bool VerifyTexSuffix(EEfResTextures texSlot, const string& texPath)
{
	assert((texSlot >= 0) && (texSlot < EFTT_MAX));
	return (texPath.size() > strlen(s_TexSlotSemantics[texSlot].suffix) && (DrxStringUtils::stristr(texPath, s_TexSlotSemantics[texSlot].suffix)));
}
