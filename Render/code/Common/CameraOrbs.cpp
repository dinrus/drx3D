...
CameraOrbs::CameraOrbs(tukk name, i32k numOrbs)
...
void CameraOrbs::Load(IXmlNode* pNode)
{
	...
	if (pAdvancedShading)
	{
		...
		i32 nAlpha((i32)(m_cAmbientDiffuse.a * 255.0f));
...
void CameraOrbs::ScatterOrbs()
{
	...
	for (u32 i = 0; i < m_OrbsList.size(); i++)
	{
...
bool CameraOrbs::PreparePrimitives(const SPreparePrimitivesContext& context)
{
	...
	static CDrxNameTSCRC techCameraOrbs("CameraOrbs");
	static CDrxNameTSCRC techCameraLens("CameraLens");
	...
	// update constants
	{
		auto constants = m_GlowPrimitive.GetConstantUpr().BeginTypedConstantUpdate<SShaderParams>(eConstantBufferShaderSlot_PerPrimitive, EShaderStage_Vertex | EShaderStage_Pixel);
		...
		m_GlowPrimitive.GetConstantUpr().EndTypedConstantUpdate(constants);
	}
