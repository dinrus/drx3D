...
#include <drx3D/Render/D3D/D3DDeferredShading.h>          // CDeferredShading
#include <drx3D/Render/D3D/D3DPostProcess.h>              // SD3DPostEffectsUtils

#include <drx3D/Render/RendElements/FlareSoftOcclusionQuery.h>       // CFlareSoftOcclusionQuery
...
public:
	...
	void AddRenderTarget(u32 nWidth, u32 nHeight, const ColorF& cClear, ETEX_Format Format, float fPriority, tukk szName, CTexture** pStorage, u32 nFlags = 0, i32 nCustomID = -1, bool bDynamicTex = 0);
...
void SRenderTargetPool::AddRenderTarget(u32 nWidth, u32 nHeight, const ColorF& cClear, ETEX_Format Format, float fPriority, tukk szName, CTexture** pStorage, u32 nFlags, i32 nCustomID, bool bDynamicTex)
{
...
bool SRenderTargetPool::CreateRenderTargetList()
{
	...
	for (u32 t = 0; t < m_pRenderTargets.size(); ++t)
	{
...
i32 CRendererResources::s_nCurLumTextureIndex;
...
void CRendererResources::LoadDefaultSystemTextures()
{
	...
	if (!m_bLoadedSystem)
	{
		...
		for (i32 t = 0; t < DRX_ARRAY_COUNT(texturesFromFile); ++t)
		{
			if (!texturesFromFile[t].pTexture)
			{
				...
				{
					DrxFatalError("Default system texture \"%s\" failed to load, please make sure the engine.pak or the engine folder is available.", texturesFromFile[t].szFileName);
				}

				if (!texturesFromFile[t].pTexture || !texturesFromFile[t].pTexture->IsLoaded() || !texturesFromFile[t].pTexture->GetDevTexture())
				{
					DrxFatalError("Can't open %s texture file.", texturesFromFile[t].szFileName);
				}
			}
		}
		...
		i32 nRTFlags = FT_DONT_RELEASE | FT_DONT_STREAM | FT_STATE_CLAMP | FT_USAGE_RENDERTARGET;
	...
	//	if (!s_ptexBackBuffer)
		{
			...
			{
				drx_sprintf(str, "$FlaresOcclusion_%d", i);
		...
		for (i = 0; i < 2; i++)
		{
			drx_sprintf(str, "$FromRE%d_FromContainer", i);
...
void CRendererResources::CreateSystemTargets(i32 resourceWidth, i32 resourceHeight)
{
	...
	DRX_ASSERT(resourceWidth);
	DRX_ASSERT(resourceHeight);
...
void CRendererResources::ResizeSystemTargets(i32 renderWidth, i32 renderHeight)
{
	i32 resourceWidth  = s_resourceWidth;
	i32 resourceHeight = s_resourceHeight;
...
void CRendererResources::CreateDepthMaps(i32 resourceWidth, i32 resourceHeight)
{
	...
	u32k nDSFlags = FT_DONT_STREAM | FT_DONT_RELEASE | FT_USAGE_DEPTHSTENCIL;
	u32k nRTFlags = FT_DONT_STREAM | FT_DONT_RELEASE | FT_USAGE_RENDERTARGET;
...
void CRendererResources::CreateSceneMaps(ETEX_Format eTF, i32 resourceWidth, i32 resourceHeight)
{
	i32k nWidth  = resourceWidth;
	i32k nHeight = resourceHeight;
	u32 nFlags = FT_DONT_STREAM | FT_USAGE_RENDERTARGET | FT_USAGE_UNORDERED_ACCESS;
...
void CRendererResources::CreateDeferredMaps(i32 resourceWidth, i32 resourceHeight)
{
	Vec2i resolution = Vec2i(resourceWidth, resourceHeight);

	static u32 nPrevLBuffersFmt = CRenderer::CV_r_DeferredShadingLBuffersFmt;
...
//		|| (gRenDev->IsStereoEnabled() && !s_ptexVelocityObjects[1]->GetDevTexture()))
	{
		...
		i32k width  = resolution.x, width_r2  = (width  + 1) / 2, width_r4  = (width_r2  + 1) / 2, width_r8  = (width_r4 +  1) / 2;
		i32k height = resolution.y, height_r2 = (height + 1) / 2, height_r4 = (height_r2 + 1) / 2, height_r8 = (height_r4 + 1) / 2;
	...
	// shadow mask
	{
		...
		{
			i32k nArraySize = gcpRendD3D->CV_r_ShadowCastingLightsMaxCount;
	...
	{
		i32k shift = clamp_tpl(3 - CRenderer::CV_r_HeightMapAO, 0, 2);

		i32 hmaoWidth = resolution.x;
		i32 hmaoHeight = resolution.y;
		...
		for (i32 i = 0; i < 2; ++i)
		{
			...
			{
				char buf[128];
				drx_sprintf(buf, "$HeightMapAO_%d", i);
...
void CRendererResources::CreateHDRMaps(i32 resourceWidth, i32 resourceHeight)
{
	i32 i;
	char szName[256];
	...
	i32k width = resourceWidth, width_r2 = (width + 1) / 2, width_r4 = (width_r2 + 1) / 2, width_r8 = (width_r4 + 1) / 2, width_r16 = (width_r8 + 1) / 2;
	i32k height = resourceHeight, height_r2 = (height + 1) / 2, height_r4 = (height_r2 + 1) / 2, height_r8 = (height_r4 + 1) / 2, height_r16 = (height_r8 + 1) / 2;

	pHDRPostProcess->ClearRenderTargetList();

	ETEX_Format nHDRFormat = CRenderer::CV_r_HDRTexFormat == 0 ? eTF_R11G11B10F : eTF_R16G16B16A16F;

	u32 nHDRTargetFlags = FT_DONT_RELEASE;
	u32 nHDRTargetFlagsUAV = nHDRTargetFlags | (FT_USAGE_UNORDERED_ACCESS);  // UAV required for tiled deferred shading
	...
	{
		drx_sprintf(szName, "$MinCoC_%d", i);
	...
	{
		i32 iSampleLen = 1 << (2 * i);
		drx_sprintf(szName, "$HDRToneMap_%d", i);
	...
	{
		drx_sprintf(szName, "$HDRMeasuredLum_%d", i);
...
bool CRendererResources::CreatePostFXMaps(i32 resourceWidth, i32 resourceHeight)
{
	...
	i32k width = resourceWidth, width_r2 = (width + 1) / 2, width_r4 = (width_r2 + 1) / 2, width_r8 = (width_r4 + 1) / 2;
	i32k height = resourceHeight, height_r2 = (height + 1) / 2, height_r4 = (height_r2 + 1) / 2, height_r8 = (height_r4 + 1) / 2;
		...
		bCreateCaustics)
	{
		...
		{
			i32k nCausticRes = clamp_tpl(CRenderer::CV_r_watervolumecausticsresolution, 256, 4096);
...
#if defined(VOLUMETRIC_FOG_SHADOWS)
		i32 fogShadowBufDiv = (CRenderer::CV_r_FogShadows == 2) ? 4 : 2;
		...
		char str[256];
		for (i32 i = 0, end = gcpRendD3D->GetActiveGPUCount() * MAX_FRAMES_IN_FLIGHT; i < end; i++)
		{
...
void CRendererResources::DestroyPostFXMaps()
{
	...
	for (i32 i = 0; i < MAX_OCCLUSION_READBACK_TEXTURES; i++)
...
{
	StaticArray<i32, MAX_GSM_LODS_NUM> nResolutions = gRenDev->GetCachedShadowsResolution();
	...
	i32k cachedShadowsStart = clamp_tpl(CRendererCVars::CV_r_ShadowsCache, 0, MAX_GSM_LODS_NUM - 1);

	i32 gsmCascadeCount = gEnv->pSystem->GetConfigSpec() == CONFIG_LOW_SPEC ? 4 : 5;
	if (ICVar* pGsmLodsVar = gEnv->pConsole->GetCVar("e_GsmLodsNum"))
		gsmCascadeCount = pGsmLodsVar->GetIVal();
	i32k cachedCascadesCount = cachedShadowsStart > 0 ? clamp_tpl(gsmCascadeCount - cachedShadowsStart + 1, 0, MAX_GSM_LODS_NUM) : 0;

	for (i32 i = 0; i < MAX_GSM_LODS_NUM; ++i)
	{
		...
		{
			char szName[32];
			drx_sprintf(szName, "CachedShadowMap_%d", i);
		...
		{
			DrxLog("Allocating shadow map cache %d x %d: %.2f MB", nResolutions[i], nResolutions[i], sqr(nResolutions[i]) * CTexture::BitsPerPixel(texFormat) / (1024.f * 1024.f * 8.f));
	...
	{
		i32k nTexRes = (i32)clamp_tpl(CRendererCVars::CV_r_HeightMapAOResolution, 0.f, 16384.f);
...
{
	i32k texResolution = CRendererCVars::CV_r_ShadowsNearestMapResolution;
...
i32 CRendererResources::s_resourceWidth = 0, CRendererResources::s_resourceHeight = 0;
i32 CRendererResources::s_renderWidth   = 0, CRendererResources::s_renderHeight   = 0;
i32 CRendererResources::s_renderMinDim  = 0, CRendererResources::s_renderArea     = 0;
i32 CRendererResources::s_outputWidth   = 0, CRendererResources::s_outputHeight   = 0;
i32 CRendererResources::s_displayWidth  = 0, CRendererResources::s_displayHeight  = 0;

void CRendererResources::OnRenderResolutionChanged(i32 renderWidth, i32 renderHeight)
{
...
void CRendererResources::OnOutputResolutionChanged(i32 outputWidth, i32 outputHeight)
{
...
void CRendererResources::OnDisplayResolutionChanged(i32 displayWidth, i32 displayHeight)
{
...
CTexture* CRendererResources::CreateDepthTarget(i32 nWidth, i32 nHeight, const ColorF& cClear, ETEX_Format eTF)
{
	char pName[128]; // Create unique names for every allocation, otherwise name-matches would occur in GetOrCreateDepthStencil()
	drx_sprintf(pName, "$DynDepthStencil_%8_x%d", m_DTallocs + 1, m_DTallocs + 2); ++m_DTallocs;
...
Ang3 sDeltAngles(Ang3& Ang0, Ang3& Ang1)
{
	...
	{
		float a0 = Ang0[i];
		a0 = (float)((360.0 / 65536) * ((i32)(a0 * (65536 / 360.0)) & 65535)); // angmod
		float a1 = Ang1[i];
		a1 = (float)((360.0 / 65536) * ((i32)(a1 * (65536 / 360.0)) & 65535));
		out[i] = a0 - a1;
	}
	return out;
}

SEnvTexture* CRendererResources::FindSuitableEnvTex(Vec3& Pos, Ang3& Angs, bool bMustExist, i32 RendFlags, bool bUseExistingREs, CShader* pSH, CShaderResources* pRes, CRenderObject* pObj, bool bReflect, CRenderElement* pRE, bool* bMustUpdate, const SRenderingPassInfo* pPassInfo)
{
	SEnvTexture* cm = NULL;
	...
	else
	{
		...
		{
			DRX_ASSERT(!pPassInfo);
			DRX_ASSERT(!gcpRendD3D->m_pRT->IsRenderThread());
