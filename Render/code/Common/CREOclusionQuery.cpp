...
u32 CREOcclusionQuery::m_nQueriesPerFrameCounter = 0;
u32 CREOcclusionQuery::m_nReadResultNowCounter = 0;
u32 CREOcclusionQuery::m_nReadResultTryCounter = 0;
...
bool CREOcclusionQuery::mfDraw(CShader* ef, SShaderPass* sfm)
{
	...
	if (!m_nDrawFrame) // only allow queries update, if finished already with previous query
	{
		...
		if (m_nOcclusionID)
		{
			...
			i32 nPersFlagsSave = gRenDev->m_renderThreadInfo[gRenDev->GetRenderThreadID()].m_PersFlags;
...
void CREOcclusionQuery::mfReadResult_Try(u32 nDefaultNumSamples)
{
...
bool CREOcclusionQuery::RT_ReadResult_Try(u32 nDefaultNumSamples)
{
	PROFILE_FRAME(CREOcclusionQuery::RT_ReadResult_Try);

	i32 nFrame = gcpRendD3D->GetRenderFrameID();
	...
	if (pVizQuery)
	{
		...
		if (hRes == S_OK)
		{
			m_nVisSamples = (i32)samplesPassed;
			m_nDrawFrame = 0;
