...
static void sParseTimeExpr(tukk szScr, tukk szAnnotations, SCGParam* vpp, i32 nComp, CShader* ef)
{
	if (szScr)
	{
		if (!vpp->m_pData)
		...
		char str[256];
		float f;
		i32 n = sscanf(szScr, "%255s %f", str, &f);
		if (n == 2)
...
static void sParseRuntimeShaderFlag(tukk szScr, tukk szAnnotations, SCGParam* vpp, i32 nComp, CShader* ef)
{
	if (szScr)
	{
		if (!vpp->m_pData)
		...
		str[0] = '\0';
		strval[0] = '\0';
		u32 n = sscanf(szScr, "%255s %255s", str, strval);
		if (n == 2)
		{
			bool bOK = false;
			if (strval[0] == '%')
...
tukk CShaderMan::mfGetShaderParamName(ECGParam ePR)
{
	i32 n = 0;
	tukk szName;
...
SParamDB* CShaderMan::mfGetShaderParamDB(tukk szSemantic)
{
	tukk szName;
	i32 n = 0;
	while (szName = sParams[n].szName)
	{
		i32 nLen = strlen(szName);
...
bool CShaderMan::mfParseParamComp(i32 comp, SCGParam* pCurParam, tukk szSemantic, tuk params, tukk szAnnotations, SShaderFXParams& FXParams, CShader* ef, u32 nParamFlags, EHWShaderClass eSHClass, bool bExpressionOperand)
{
	if (comp >= 4 || comp < -1)
		return false;
	if (!pCurParam)
	...
	if (!szSemantic || !szSemantic[0])
	{
		...
		if (!((nParamFlags >> comp) & PF_TWEAKABLE_0))
			pCurParam->m_eCGParamType = (ECGParam)((i32)pCurParam->m_eCGParamType | (i32)(ECGP_PB_Scalar << (comp * 8)));
		else
		{
			pCurParam->m_eCGParamType = (ECGParam)((i32)pCurParam->m_eCGParamType | (i32)(ECGP_PM_Tweakable << (comp * 8)));
			if (!bExpressionOperand)
			{
				pCurParam->m_eCGParamType = (ECGParam)((i32)pCurParam->m_eCGParamType | (i32)ECGP_PM_Tweakable);
	...
	while (szName = sParams[n].szName)
	{
		...
		if (!strnicmp(szName, szSemantic, nLen) || (sParams[n].szAliasName && !strnicmp(sParams[n].szAliasName, szSemantic, strlen(sParams[n].szAliasName))))
		{
			...
			if (comp > 0)
				pCurParam->m_eCGParamType = (ECGParam)((i32)pCurParam->m_eCGParamType | (i32)(sParams[n].eParamType << (comp * 8)));
			else
			...
			if (sParams[n].nFlags & PD_INDEXED)
			{
				if (szSemantic[nLen] == '[')
				{
					i32 nID = shGetInt(&szSemantic[nLen + 1]);
					assert(nID < 256);
					if (comp > 0)
						nID <<= (comp * 8);
					pCurParam->m_nID |= nID;
...
bool CShaderMan::mfParseCGParam(tuk scr, tukk szAnnotations, SShaderFXParams& FXParams, CShader* ef, std::vector<SCGParam>* pParams, i32 nComps, u32 nParamFlags, EHWShaderClass eSHClass, bool bExpressionOperand)
{
	tuk name;
	long cmd;
	tuk params;
...
bool CShaderMan::mfParseFXParameter(SShaderFXParams& FXParams, SFXParam* pr, tukk ParamName, CShader* ef, bool bInstParam, i32 nParams, std::vector<SCGParam>* pParams, EHWShaderClass eSHClass, bool bExpressionOperand)
{
	SCGParam CGpr;
	char scr[256];

	u32 nParamFlags = pr->GetParamFlags();

	DrxFixedStringT<512> Semantic = "Name=";
	Semantic += ParamName;
	Semantic += " ";
	i32 nComps = 0;
	...
	else
	{
		for (i32 i = 0; i < pr->m_nComps; i++)
		{
			...
			if (cur.empty())
				break;
			nComps++;
			if ((cur.at(0) == '-' && isdigit(u8(cur.at(1)))) || isdigit(u8(cur.at(0))))
				drx_sprintf(scr, "Comp = %s", cur.c_str());
			else
				drx_sprintf(scr, "Comp '%s'", cur.c_str());
			Semantic += scr;
		}
	...
	if (nComps)
	{
		...
		bool bRes = mfParseCGParam((tuk)Semantic.c_str(), pr->m_Annotations.c_str(), FXParams, ef, pParams, nComps, nParamFlags, eSHClass, bExpressionOperand);
		assert(bRes);
...
bool CShaderMan::mfParseFXSampler(SShaderFXParams& FXParams, SFXSampler* pr, tukk ParamName, CShader* ef, i32 nParams, std::vector<SCGSampler>* pParams, EHWShaderClass eSHClass)
{
	SCGSampler CGpr;
	...
	tukk szSemantic = pr->m_Semantic.c_str();
	tukk szName;
	i32 n = 0;
...
bool CShaderMan::mfParseFXTexture(SShaderFXParams& FXParams, SFXTexture* pr, tukk ParamName, CShader* ef, i32 nParams, std::vector<SCGTexture>* pParams, EHWShaderClass eSHClass)
{
	SCGTexture CGpr;
	...
	if (pr->m_Semantic.empty())
	{
		...
		{
			tukk nameTex = pr->m_szTexture.c_str();
...
//===========================================================================================
bool SShaderParam::GetValue(tukk szName, DynArrayRef<SShaderParam>* Params, float* v, i32 nID)
{
	bool bRes = false;
...
bool SShaderParam::GetValue(u8 eSemantic, DynArrayRef<SShaderParam>* Params, float* v, i32 nID)
{
	bool bRes = false;
	for (i32 i = 0; i < Params->size(); i++)
	{
