...
static i32 gaussian_use_last = 0;

#define m_nGridSize    (i32)g_nGridSize
#define m_nLogGridSize (i32)g_nGridLogSize // log2(64)
#define m_fG           (9.81f)
...
void WaterAsyncUpdate(CWaterSim*, i32, float, bool, uk );
DECLARE_JOB("WaterUpdate", TWaterUpdateJob, WaterAsyncUpdate);

class DRX_ALIGN(128) CWaterSim
{
...
public:
		  ...
		  , m_fChoppyWaveScale(400.0f)
	{
		...
		u32 nSize = m_nGridSize * m_nGridSize * sizeof(complexF);
	...
	{
		u32 nSize = m_nGridSize * m_nGridSize * sizeof(std::complex<float> );
		memset(&m_pFourierAmps[0], 0, nSize);
		memset(&m_pHeightField[0], 0, nSize);
		memset(&m_pDisplaceFieldX[0], 0, nSize);
		memset(&m_pDisplaceFieldY[0], 0, nSize);
	...
	virtual void SaveToDisk(tukk pszFileName)
	{
	}
	...
	float frand_gaussian(float m = 0.0f, float s = 1.0f)  // m- mean, s - standard deviation
	{
		...
		else
		{
			do
			{
				x1 = drx_random(-1.0f, 1.0f);
				x2 = drx_random(-1.0f, 1.0f);
	...
	inline i32 GetGridOffset(i32k& x, i32k& y) const
	{
	...
	i32 GetMirrowedGridOffset(i32k& x, i32k& y) const
	{
	...
	i32 GetGridOffsetWrapped(i32k& x, i32k& y) const
	{
	...
	void ComputeFFT1D(i32 nDir, float* pReal, float* pImag)
	{
	...
	void ComputeFFT2D(i32 nDir, complexF* const __restrict c)
	{
		// reference: "2 Dimensional FFT" - by Paul Bourke
		...
		// Transform the rows
		for (i32 y(0); y < m_nGridSize; ++y)
		{
			for (i32 x(0); x < m_nGridSize; ++x)
			{
				i32 nGridOffset = GetGridOffset(x, y);
	...
	{
		i32 nFrameID = pThreadInfo.nFrameID;
		...
		for (i32 y(0); y < fHalfY; ++y)
		{
			for (i32 x(0); x < m_nGridSize; ++x)
			{
				i32 offset = GetGridOffset(x, y);
				i32 offsetMirrowed = GetMirrowedGridOffset(x, y);
...
#if DRX_PLATFORM_ORBIS // Workaround for bug in Orbis maths library which means sinf becomes orders of magnitude slower for big numbers
	...
	void Update(i32 nFrameID, float fTime, bool bOnlyHeight)
	{
	...
	void SpawnUpdateJob(i32 nFrameID, float fTime, bool bOnlyHeight, uk pRawPtr)
	{
...
protected:
	...
	DRX_ALIGN(128) Vec4 m_pDisplaceGrid[2][g_nGridSize * g_nGridSize];
	// Simulation constants
	DRX_ALIGN(128) Vec4 m_pLUTK[g_nGridSize * g_nGridSize];                // pre-computed K table
	DRX_ALIGN(128) complexF m_pFourierAmps[m_nGridSize * m_nGridSize];     // Fourier amplitudes at time 0 (aka. H0)
	DRX_ALIGN(128) complexF m_pHeightField[m_nGridSize * m_nGridSize];     // Current Fourier amplitudes height field
	DRX_ALIGN(128) complexF m_pDisplaceFieldX[m_nGridSize * m_nGridSize];  // Current Fourier amplitudes displace field
	DRX_ALIGN(128) complexF m_pDisplaceFieldY[m_nGridSize * m_nGridSize];  // Current Fourier amplitudes displace field

	SWaterUpdateThreadInfo m_pThreadInfo[2];

	i32 m_nFrameID;
	u32 m_nWorkerThreadID;
	u32 m_nFillThreadID;
	DrxEvent m_Event;
	bool m_bQuit;
...
void WaterAsyncUpdate(CWaterSim* pWaterSim, i32 nFrameID, float fTime, bool bOnlyHeight, uk pRawPtr)
{
	if (pWaterSim == NULL)
		return;
...
void CWater::Create(float fA, float fWind, float fWorldSizeX, float fWorldSizeY)
{
	...
	Release();
	m_pWaterSim = DrxAlignedNew<CWaterSim>(); //::GetInstance();
...
void CWater::SaveToDisk(tukk pszFileName)
{
	assert(m_pWaterSim);
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////
void CWater::Update(i32 nFrameID, float fTime, bool bOnlyHeight, uk pRawPtr)
{
	if (m_pWaterSim)
...
Vec3 CWater::GetPositionAt(i32 x, i32 y) const
{
	if (m_pWaterSim)
...
float CWater::GetHeightAt(i32 x, i32 y) const
{
	if (m_pWaterSim)
...
void CWater::GetMemoryUsage(IDrxSizer* pSizer) const
{
