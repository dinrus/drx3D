...
inline void process_root(const node* r, roots_t &new_roots, const F &f) noexcept
{
	...
	for (auto &edge : r->out)
	{
		...
		DRX_ASSERT(edge->in_edges_count >= 0);
	}
}
...
std::size_t CRenderView::OptimizeTransparentRenderItemsResolves(STransparentSegments &segments, RenderItems &renderItems, std::size_t resolves_count) const
{
	...
	DRX_ASSERT_MESSAGE(CRendererCVars::CV_r_RefractionPartialResolveMode == 1 || CRendererCVars::CV_r_RefractionPartialResolveMode == 2,
	...
	else
	{
		...
		DRX_ASSERT(end == renderItems.size());
...
void CRenderView::StartOptimizeTransparentRenderItemsResolvesJob()
{
	...
	gEnv->pJobUpr->AddLambdaJob("OptimizeTransparentRenderItemsResolvesJob", [this]()
		{
		...
		JobUpr::eRegularPriority, &m_optimizeTransparentRenderItemsResolvesJobStatus);
}
