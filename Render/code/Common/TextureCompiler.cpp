...
#include <drx3D/Render/TextureCompiler.h>
  ...
  size_t inputFilenameSizeInBytes)
{
	...
	tukk ext = GetExtension(filename);
	...
	drx_strcpy(inputFilename, inputFilenameSizeInBytes, filename);
}
...
static tukk GetDelayedTexture(tukk szFile)
{
	tukk szImposter;
...
static tukk GetFailedTexture(tukk szFile)
{
	tukk szImposter;
...
static bool CopyDummy(tukk szImposter, tukk szSrcFile, tukk szDstFile, __int64 timedelta)
{
	string dummyPath = szImposter;
	bool success = true;
	IDrxPak::FileTime ft = 0;

	{
		FILE* pSrcFile = gEnv->pDrxPak->FOpen(szSrcFile, "rb");
		if (pSrcFile)
		{
			ft = gEnv->pDrxPak->GetModificationTime(pSrcFile);
			ft -= timedelta;

			gEnv->pDrxPak->FClose(pSrcFile);
		}
	}
	...
	FILE* pSrcFile = gEnv->pDrxPak->FOpen(dummyPath, "rb");
	...
	if (success)
	{
		...
		tuk buf = new char[chunkSize];
		size_t readBytes = 0;
		DWORD writtenBytes = 0;
		size_t totalBytes = 0;

		while (!gEnv->pDrxPak->FEof(pSrcFile))
		{
			readBytes = gEnv->pDrxPak->FReadRaw(buf, sizeof(char), chunkSize, pSrcFile);
...
static bool MoveAssetFile(tukk szSrcFile, tukk szDstFile)
{
	...
	if (dstFileAttributes != INVALID_FILE_ATTRIBUTES)
	{
		...
		{
			// CE-12815. Should be able to compile tiff to dds if .drxasset file is write protected.
			if (stricmp(PathUtil::GetExt(szDstFile), "drxasset") == 0)
			{
	...
	gEnv->pDrxPak->MakeDir(PathUtil::GetPathWithoutFilename(szDstFile));
	...
	for (i32 i = 0, stage = 0; i < 5; ++i)
	{
		...
		case 3:
			...
			// TODO: find a way not to generate drxasset with the Sandbox AssetGenerator until all asset files are copied.
...
static bool IsFileReadOnly(tukk szPath)
{
...
private:
	...
	static string GetTemporaryDirectoryPath()
	{
		...
		return gEnv->pDrxPak->AdjustFileName("%USER%/TextureCompiler", path, IDrxPak::FLAGS_PATH_REAL | IDrxPak::FLAGS_FOR_WRITING | IDrxPak::FLAGS_ADD_TRAILING_SLASH);
	}
	...
	static std::vector<SAsset> CreateTemp(const string& srcFile, const string& dstFile)
	{
		...
		gEnv->pDrxPak->MakeDir(tempDir.c_str());
		...
		for (const string& suffix : suffixes)
		{
			...
			// Try to update existing ".drxasset" instead of creating a new one to keep the asset GUID.
			const string dstDrxasset = dstPath + ".drxasset";
			const string tmpDrxasset = tmpPath + ".drxasset";
			gEnv->pDrxPak->CopyFileOnDisk(dstDrxasset.c_str(), tmpDrxasset.c_str(), false);
		}
	...
	static bool DeleteTemp(const string& filepath)
	{
		...
		return gEnv->pDrxPak->RemoveDir(tmpFolder.c_str(), true);
	}

	// Returns list of asset filenames without paths.
	static std::vector<string> GetAssetFiles(const string& srcFile)
	{
		// Try to read the file list form .drxasset
		const string drxasset = srcFile + ".drxasset";

		const XmlNodeRef pXml = (GetFileAttributes(drxasset) != INVALID_FILE_ATTRIBUTES) ? gEnv->pSystem->LoadXmlFromFile(drxasset) : nullptr;
		if (!pXml)
		{
		...
		if (files.empty())
		{
			iLog->LogError("Drxasset has no data files: %s", drxasset.c_str());
			files.push_back(PathUtil::GetFile(srcFile));
		}

		files.emplace_back(PathUtil::GetFile(drxasset));
		return files;
	}
	...
	static bool MoveAsset(const string& srcFile, const string& dstFile)
	{
		...
		DRX_ASSERT(stricmp(PathUtil::GetFile(srcFile), PathUtil::GetFile(dstFile)) == 0);
 ...
 */
bool CTextureCompiler::HasQueuedResourceCompiler(tukk szSrcFile, tukk szDstFile)
{
	DrxAutoReadLock<DrxRWLock> lock(m_rwLockWatch);
...
CResourceCompilerHelper::ERcCallResult CTextureCompiler::QueueResourceCompiler(tukk szSrcFile, tukk szDstFile, const bool bWindow, const bool bRefresh)
{
...
bool CTextureCompiler::AddToWatchList(tukk szDstFile, tukk szSrcFile)
{
	// check if to be generated from the same source
	...
	DrxAutoWriteLock<DrxRWLock> lock(m_rwLockWatch);
	...
	else
	{
		...
		{
			tukk szImposter = GetDelayedTexture(szDstFile);
			...
			if (strstr(szSrcFile, "_cm."))
			{
				...
				tukk szImposter = GetDelayedTexture(szDstDiff.c_str());
...
void CTextureCompiler::ForkOffResourceCompiler(tukk szSrcFile, tukk szDstFile, const bool bWindow, const bool bRefresh)
{
	DrxAutoWriteLock<DrxRWLock> lock(m_rwLockProcessing);
...
bool CTextureCompiler::IsFileOpened(tukk szPath)
{
	if (!gEnv->pDrxPak->IsFileExist(szPath, IDrxPak::eFileLocation_OnDisk))
	{
		return false;
	}

	FILE* pFile = gEnv->pDrxPak->FOpen(szPath, "r", IDrxPak::FOPEN_ONDISK | IDrxPak::FOPEN_LOCKED_OPEN);
	if (!pFile)
	{
		return true;
	}
...
void CTextureCompiler::GetNextItem(TProcItem* &item, i32 &pending)
{
	DrxAutoWriteLock<DrxRWLock> lockWatch(m_rwLockWatch);
	DrxAutoWriteLock<DrxRWLock> lockProcessing(m_rwLockProcessing);
	...
	{
		DRX_ASSERT_MESSAGE(foundrc != m_qProcessingList.end(), "Severe container-damage, should not happen.");
		return;
	}
...
{
	DrxAutoReadLock<DrxRWLock> lock(m_rwLockNotify);
...
void CTextureCompiler::NotifyCompilationStarted(TProcItem* item, i32 pending)
{
	DrxAutoReadLock<DrxRWLock> lock(m_rwLockNotify);
	for (IAsyncTextureCompileListener* notify : m_sNotifyList)
	{
...
void CTextureCompiler::NotifyCompilationQueueTriggered(i32 pending)
{
	DrxAutoReadLock<DrxRWLock> lock(m_rwLockNotify);
	for (IAsyncTextureCompileListener* notify : m_sNotifyList)
	{
  ...
  bool immediate)
{
	...
	gEnv->pDrxPak->AdjustFileName(sDestFile, sFullDestFilename, 0);

	EResult result = EResult::AlreadyCompiled;

	for (u32 dwIndex = 0;; ++dwIndex)    // check for all input files
	{
		...
		// Adjust filename so that it is global.
		gEnv->pDrxPak->AdjustFileName(sSrcFile, sFullSrcFilename, 0);
		...
		CDrxFile destinationFile, sourceFile;
		{
		...
		{
			IDrxPak::FileTime timeSrc = gEnv->pDrxPak->GetModificationTime(sourceFile.GetHandle());
			IDrxPak::FileTime timeDest = gEnv->pDrxPak->GetModificationTime(destinationFile.GetHandle());
		...
		if (bInvokeResourceCompiler)
		{
			...
			if(!processed)
			{
				drx_strcpy(processedFilename, processedFilenameSizeInBytes, originalFilename);
	...
	drx_strcpy(processedFilename, processedFilenameSizeInBytes, sDestFile);
...
{
	DrxAutoWriteLock<DrxRWLock> lock(m_rwLockNotify);
