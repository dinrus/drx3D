...
i32 SDynTexture::s_nMemoryOccupied;

u32 SDynTexture::s_iNumTextureBytesCheckedOut;
u32 SDynTexture::s_iNumTextureBytesCheckedIn;
...
i32 SDynTexture2::s_nMemoryOccupied[eTP_Max];

u32 SDynTexture::s_SuggestedDynTexAtlasCloudsMaxsize;
u32 SDynTexture::s_SuggestedDynTexAtlasSpritesMaxsize;
u32 SDynTexture::s_SuggestedTexAtlasSize;
u32 SDynTexture::s_SuggestedDynTexMaxSize;

u32 SDynTexture::s_CurDynTexAtlasCloudsMaxsize;
u32 SDynTexture::s_CurDynTexAtlasSpritesMaxsize;
u32 SDynTexture::s_CurTexAtlasSize;
u32 SDynTexture::s_CurDynTexMaxSize;
...
SDynTexture::SDynTexture(tukk szSource)
{
	m_nWidth = 0;
	m_nHeight = 0;
...
SDynTexture::SDynTexture(i32 nWidth, i32 nHeight, ColorF clearValue, ETEX_Format eTF, ETEX_Type eTT, i32 nTexFlags, tukk szSource)
{
	DRX_ASSERT(nTexFlags & (FT_USAGE_DEPTHSTENCIL | FT_USAGE_RENDERTARGET));
	m_nWidth = nWidth;
	m_nHeight = nHeight;
	m_nReqWidth = m_nWidth;
	m_nReqHeight = m_nHeight;
...
i32 SDynTexture::GetTextureID()
{
	return m_pTexture ? m_pTexture->GetTextureID() : 0;
}

bool SDynTexture::FreeTextures(bool bOldOnly, i32 nNeedSpace)
{
	bool bFreed = false;
	if (bOldOnly)
	{
		...
		while (nNeedSpace + s_nMemoryOccupied > (i32)SDynTexture::s_CurDynTexMaxSize * 1024 * 1024)
		{
		...
		if (nNeedSpace + s_nMemoryOccupied < (i32)SDynTexture::s_CurDynTexMaxSize * 1024 * 1024)
			return true;
	}
	if (!bFreed)
	...
	//shadows pools
	for (i32 nPool = SBP_D16; nPool < SBP_MAX; nPool++)
	{
		if (!bFreed && m_eTT == eTT_2D)
...
bool SDynTexture::Update(i32 nNewWidth, i32 nNewHeight)
{
...
bool SDynTexture::RT_Update(i32 nNewWidth, i32 nNewHeight)
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "SDynTexture::RT_Update");
	...
	if (!m_pTexture)
	{
		i32 nNeedSpace = CTexture::TextureDataSize(m_nWidth, m_nHeight, 1, 1, m_eTT == eTT_Cube ? 6 : 1, m_eTF);
		SDynTexture* pTX = SDynTexture::s_Root.m_Prev;
		if (nNeedSpace + s_nMemoryOccupied > (i32)SDynTexture::s_CurDynTexMaxSize * 1024 * 1024)
		{
			...
			if (!m_pTexture)
			{
				...
				if (!bFreed)
				{
					...
					i32 nFrame = gRenDev->GetRenderFrameID() - 1;
					while (nNeedSpace + s_nMemoryOccupied > (i32)SDynTexture::s_CurDynTexMaxSize * 1024 * 1024)
					{
						if (pTX == &SDynTexture::s_Root)
						{
...
void SDynTexture::GetImageRect(u32& nX, u32& nY, u32& nWidth, u32& nHeight)
{
	nX = 0;
	nY = 0;
	nWidth = m_nWidth;
...
bool SDynTexture::FreeAvailableDynamicRT(i32 nNeedSpace, TextureSet* pSet, bool bOldOnly)
{
	assert(s_iNumTextureBytesCheckedOut + s_iNumTextureBytesCheckedIn == s_nMemoryOccupied);

	i32 nSpace = s_nMemoryOccupied;
	i32 nFrame = gRenDev->GetRenderFrameID() - 400;
	while (nNeedSpace + nSpace > (i32)SDynTexture::s_CurDynTexMaxSize * 1024 * 1024)
	{
		...
		while (itor != pSet->end())
		{
			...
			while (itorss != pSubset->end())
			{
				...
				if (!bOldOnly || (pTex->m_nAccessFrameID < nFrame && pTex->m_nUpdateFrameID < nFrame))
				{
					...
					SAFE_RELEASE(pTex);
					if (nNeedSpace + nSpace < (i32)SDynTexture::s_CurDynTexMaxSize * 1024 * 1024)
						break;
				}
				else
					itorss++;
...
void SDynTexture::ReleaseDynamicRT(bool bForce)
{
	...
	if (bForce)
	{
		...
		assert(s_iNumTextureBytesCheckedOut + s_iNumTextureBytesCheckedIn == s_nMemoryOccupied);
		i32 refCount = m_pTexture->Release();
		assert(refCount <= 0);
		m_pTexture = NULL;
		Unlink();
		return;
...
CTexture* SDynTexture::CreateDynamicRT()
{
	...
	if (m_eTT == eTT_2D)
		drx_sprintf(name, "$Dyn_%s_2D_%s_%d", m_sSource, CTexture::NameForTextureFormat(m_eTF), gRenDev->m_TexGenID++);
	else
		drx_sprintf(name, "$Dyn_%s_Cube_%s_%d", m_sSource, CTexture::NameForTextureFormat(m_eTF), gRenDev->m_TexGenID++);
	...
	if (m_nTexFlags & FT_USAGE_DEPTHSTENCIL)
	{
		...
		CClearSurfacePass::Execute(pNewTexture, CLEAR_ZBUFFER | CLEAR_STENCIL, m_clearValue.r, u8(m_clearValue.g));
	}
	else
	{
...
{
	i32 nFrame = gRenDev->RT_GetCurrGpuID();
...
i32 SDynTexture2::GetTextureID()
{
	return m_pTexture ? m_pTexture->GetTextureID() : 0;
}

void SDynTexture2::GetImageRect(u32& nX, u32& nY, u32& nWidth, u32& nHeight)
{
	nX = 0;
	nY = 0;
	if (m_pTexture)
...
SDynTexture_Shadow::SDynTexture_Shadow(tukk szSource) : SDynTexture(szSource)
{
	m_nUniqueID = 0;
	m_NextShadow = NULL;
	m_PrevShadow = NULL;
...
SDynTexture_Shadow::SDynTexture_Shadow(i32 nWidth, i32 nHeight, ColorF clearValue, ETEX_Format eTF, ETEX_Type eTT, i32 nTexFlags, tukk szSource) : SDynTexture(nWidth, nHeight, clearValue, eTF, eTT, nTexFlags, szSource)
{
	if (gRenDev)
...
{
	DRX_PROFILE_REGION(PROFILE_RENDERER, "SDynTexture_Shadow::RT_EntityDelete");
...
SDynTexture_Shadow* SDynTexture_Shadow::GetForFrustum(const ShadowMapFrustum* pFrustum)
{
	...
	if (!pDynTX)
	{
		...
		DRX_ASSERT(pFrustum->nTextureWidth == pDynTX->m_nWidth && pFrustum->nTextureHeight == pDynTX->m_nHeight);
	}
...
SDynTextureArray::SDynTextureArray(i32 nWidth, i32 nHeight, i32 nArraySize, ETEX_Format eTF, i32 nTexFlags, tukk szSource)
{
	m_eTF = eTF;
	m_eTT = eTT_2D;
...
bool SDynTextureArray::Update(i32 nNewWidth, i32 nNewHeight)
{
	return
...
DrxCriticalSection SDynTexture2::s_texturePoolLock;
DrxRWLock SDynTexture2::s_memoryOccupiedLock;
SDynTexture2::TextureSet SDynTexture2::s_TexturePool[eTP_Max];

SDynTexture2::SDynTexture2(tukk szSource, ETexPool eTexPool) threadsafe
{
	m_nWidth = 0;
	m_nHeight = 0;
...
SDynTexture2::SDynTexture2(u32 nWidth, u32 nHeight, u32 nTexFlags, tukk szSource, ETexPool eTexPool) threadsafe
{
	m_nWidth = nWidth;
	m_nHeight = nHeight;
	...
	DrxAutoCriticalSection scopeLock(s_texturePoolLock);
...
i32 SDynTexture2::GetPoolMaxSize(ETexPool eTexPool)
{
...
void SDynTexture2::SetPoolMaxSize(ETexPool eTexPool, i32 nSize, bool bWarn)
{
	if (eTexPool == eTP_Clouds)
	{
		if (bWarn)
...
tukk SDynTexture2::GetPoolName(ETexPool eTexPool)
{
	if (eTexPool == eTP_Clouds)
		return "Clouds";
	if (eTexPool == eTP_Sprites)
...
void SDynTexture2::Init(ETexPool eTexPool)
{
	...
	{
		i32 nSize = CRenderer::CV_r_texatlassize;
		TArray<SDynTexture2*> Texs;
		i32 nMaxSize = GetPoolMaxSize(eTexPool);
		tukk szName = GetPoolName(eTexPool);
		nMaxSize *= 1024 * 1024;
		while (true)
		{
			ETEX_Format eTF = GetPoolTexFormat(eTexPool);
			i32 nNeedSpace = CTexture::TextureDataSize(nSize, nSize, 1, 1, 1, eTF);
			{
				DrxAutoReadLock<DrxRWLock> scopeLock(s_memoryOccupiedLock);
...
bool SDynTexture2::UpdateAtlasSize(i32 nNewWidth, i32 nNewHeight)
{
	if (!m_pOwner)
		return false;
	if (!m_pTexture)
	...
	if (m_pTexture->GetWidth() != nNewWidth || m_pTexture->GetHeight() != nNewHeight)
	{
		...
		i32 nBlockW = (m_nWidth + TEX_POOL_BLOCKSIZE - 1) / TEX_POOL_BLOCKSIZE;
		i32 nBlockH = (m_nHeight + TEX_POOL_BLOCKSIZE - 1) / TEX_POOL_BLOCKSIZE;
		m_pAllocator->RemoveBlock(m_nBlockID);
		assert(m_pAllocator && m_pAllocator->GetNumUsedBlocks() == 0);

		i32 nW = (nNewWidth + TEX_POOL_BLOCKSIZE - 1) / TEX_POOL_BLOCKSIZE;
		i32 nH = (nNewHeight + TEX_POOL_BLOCKSIZE - 1) / TEX_POOL_BLOCKSIZE;
		m_pAllocator->UpdateSize(nW, nH);
		m_nBlockID = m_pAllocator->AddBlock(nBlockW, nBlockH);
		
		{
			DrxAutoWriteLock<DrxRWLock> scopeLock(s_memoryOccupiedLock);
			s_nMemoryOccupied[m_eTexPool] -= CTexture::TextureDataSize(m_pTexture->GetWidth(), m_pTexture->GetHeight(), 1, 1, 1, m_pOwner->m_eTF);
		}
		SAFE_RELEASE(m_pTexture);

		char name[256];
		drx_sprintf(name, "$Dyn_2D_%s_%s_%d", CTexture::NameForTextureFormat(m_pOwner->m_eTF), GetPoolName(m_eTexPool), gRenDev->m_TexGenID++);
...
bool SDynTexture2::Update(i32 nNewWidth, i32 nNewHeight)
{
	i32 i;
	if (!m_pOwner)
		return false;
	...
	DrxAutoCriticalSection texPoolLock(m_pOwner->m_TexPoolsLock);
	...
	u32 nFrame = gRenDev->GetRenderFrameID();
	if (bRecreate)
	{
		i32 nSize = CRenderer::CV_r_texatlassize;
		if (nSize <= 512)
		...
		i32 nMaxSize = GetPoolMaxSize(m_eTexPool);
		i32 nNeedSpace = CTexture::TextureDataSize(nSize, nSize, 1, 1, 1, m_pOwner->m_eTF);
		if (nNeedSpace > nMaxSize * 1024 * 1024)
		{
			SetPoolMaxSize(m_eTexPool, nNeedSpace / (1024 * 1024), true);
		}

		i32 nBlockW = (nNewWidth + TEX_POOL_BLOCKSIZE - 1) / TEX_POOL_BLOCKSIZE;
		i32 nBlockH = (nNewHeight + TEX_POOL_BLOCKSIZE - 1) / TEX_POOL_BLOCKSIZE;
		Remove();
		SetUpdateMask();

		u32 nID = ~0;
		CPowerOf2BlockPacker* pPack = NULL;
		for (i = 0; i < (i32)m_pOwner->m_TexPools.size(); i++)
		{
		...
		if (i == m_pOwner->m_TexPools.size())
		{
			...
			if (nNeedSpace + memoryOccupied > nMaxSize * 1024 * 1024)
			{
				...
				for (pDT = m_pOwner->m_pRoot; pDT; pDT = pDT->m_Next)
				{
					...
					{
						i32 nEr = pDT->m_nWidth - m_nWidth + pDT->m_nHeight - m_nHeight;
						i32 fEr = nEr + (nFrame - pDT->m_nAccessFrame);
				...
				if (!pDT)
				{
					...
					if (!pPackBest || time.GetSeconds() + 0.5f > currentTime.GetSeconds())
					{
						...
						u32 nUsedBlocks = TEX_POOL_BLOCKSIZE * TEX_POOL_BLOCKSIZE + 1;
						pPackBest = NULL;
						for (i = 0; i < (i32)m_pOwner->m_TexPools.size(); i++)
						{
							CPowerOf2BlockPacker* pNextPack = m_pOwner->m_TexPools[i];
							i32 nBlocks = pNextPack->GetNumUsedBlocks();
							if (nBlocks < (i32)nUsedBlocks)
							{
								nUsedBlocks = nBlocks;
								pPackBest = pNextPack;
							}
			...
			{
				nStage |= 0x100;
				i32 n = (nSize + TEX_POOL_BLOCKSIZE - 1) / TEX_POOL_BLOCKSIZE;
...
void SDynTexture2::ShutDown() threadsafe
{
	...
	for (i = 0; i < eTP_Max; i++)
	{
		for (auto it = s_TexturePool[i].begin(); it != s_TexturePool[i].end(); it++)
		{
			...
			PREFAST_ASSUME(pF);
			SDynTexture2* pDT, * pNext;
			DrxAutoCriticalSection lock(pF->m_rootLock);
...
{
	DrxAutoCriticalSection texPoolLock(m_TexPoolsLock);
	...
	for (i = 0; i < m_TexPools.size(); i++)
	{
		...
		if (pP->m_pTexture)
		{
			i32 nSize = CTexture::TextureDataSize(pP->m_pTexture->GetWidth(), pP->m_pTexture->GetHeight(), 1, 1, 1, pP->m_pTexture->GetTextureDstFormat());
			{
				DrxAutoWriteLock<DrxRWLock> scopeLock(SDynTexture2::s_memoryOccupiedLock);
