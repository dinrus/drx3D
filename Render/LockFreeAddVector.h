...
struct lockfree_add_vector
{
	...
	{
		//DrxAutoLock<DrxCriticalSectionNonRecursive> lock_temp(m_temp_lock);

		i32 newIndex = DrxInterlockedIncrement(&m_lastIndex);
		i32 lastCapacity = m_capacity;
		...
		else
		{
			...
			DrxAutoLock<DrxCriticalSectionNonRecursive> lock(m_resize_lock);
...
private:
	...
	struct SMemoryBuffer
	{
		...
		SMemoryBuffer(size_t sz, T* pPrevBuffer, size_t prevSize)
		{
			...
			pBuffer = (T*)DrxModuleMemalign(alignedSize, align);
			size = sz;
			if (pPrevBuffer)
				drxMemcpy(pBuffer, pPrevBuffer, prevSize * sizeof(T));
			// Fill empty memory with 0
			memset(pBuffer + prevSize, 0, (sz - prevSize) * sizeof(T));
		}
		~SMemoryBuffer()
		{
			DrxModuleMemalignFree(pBuffer);
		}
	};
...
private:
	...
	DrxCriticalSectionNonRecursive m_resize_lock;

	DrxCriticalSectionNonRecursive m_temp_lock;
};
