...
struct SVertexInputStream
{
	...
	SVertexInputStream(tukk streamSemanticName, u8 streamSemanticIndex, u8 streamAttributeLocation)
	{
		DRX_ASSERT(strlen(streamSemanticName) < DRX_ARRAY_COUNT(semanticName));

		strncpy(semanticName, streamSemanticName, DRX_ARRAY_COUNT(semanticName));
...
struct SCGParam : SCGBind
{
	...
	const CDrxNameR GetParamCompName(i32 nComp) const
	{
		if (!m_pData)
			return CDrxNameR("None");
		return m_pData->m_CompNames[nComp];
	}

	i32 Size()
	{
		i32 nSize = sizeof(SCGParam);
		if (m_pData)
			nSize += m_pData->Size();
		return nSize;
	}
