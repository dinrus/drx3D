...
private:
	...
	static i32k         MAX_ORBS_NUMBER = 10000;
...
public:
	CameraOrbs(tukk name, i32k numOrbs = 100);
...
public:
	...
	i32  GetNumOrbs() const { return m_OrbsList.size(); }
	void SetNumOrbs(i32 n)
	{
		n = clamp_tpl<i32>(n, 0, MAX_ORBS_NUMBER);
		if (n != m_nNumOrbs)
		{
			m_nNumOrbs = n;
			if (m_nNumOrbs > 0)
	...
	i32  GetNoiseSeed() const { return m_iNoiseSeed; }
	void SetNoiseSeed(i32 s)
	{
		m_iNoiseSeed = s;
		m_meshDirty = true;
	}
