...
bool CVehiclePrototype::Init(CBaseObject* prev, const string& file)
{
	...
	if (name.GetLength() == 0)
	{
		DrxLog("Creation of VehiclePrototype failed, no filename given");
		return false;
	}
	...
	if (xmlDef == 0)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Loading definition file %s failed.. returning", VEHICLE_XML_DEF);
		return false;
	}
	...
	if (vehicleXml == 0)
	{
		DrxLog("Loading of %s failed, returning..", fullPath);
		return false;
	}

	if (!vehicleXml->haveAttr("name"))
	{
		DrxLog("%s has no 'name' attribute [file %s], returning..", name, fullPath);
		return false;
	}
	...
	if (!m_pVehicleData)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "VehicleXmlLoad for %s failed.", name);
		return false;
	}
...
//////////////////////////////////////////////////////////////////////////
void CVehiclePrototype::OnObjectEvent(CBaseObject* node, i32 event)
{
...
{
	i32 nChildCount = GetChildCount();
	VeedLog("[CVehiclePrototype]: deleting %i children..", nChildCount);

	for (i32 i = nChildCount - 1; i >= 0; --i)
	{
