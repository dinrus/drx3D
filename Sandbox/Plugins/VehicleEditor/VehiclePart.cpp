...
void CVehiclePart::DrawRotationLimits(DisplayContext& dc, IVariable* pSpeed, IVariable* pLimits, IVariable* pHelper, i32 axis)
{
	if (pSpeed)
	{
		float speed = 0;
...
//////////////////////////////////////////////////////////////////////////
void CVehiclePart::OnObjectEvent(CBaseObject* node, i32 event)
{
...
void CVehiclePart::UpdateVarFromObject()
{
	...
	if (!pEnt)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "[CVehiclePart::UpdateVariable] pEnt is null, returning");
		return;
	}

	IVariable* pVar = GetChildVar(m_pVar, "name");
	if (!pVar)
		DrxLog("ChildVar <name> not found in Part!");
	else
		pVar->Set(GetName());
