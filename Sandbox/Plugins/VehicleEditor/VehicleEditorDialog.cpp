...
class CVehicleEditorViewClass : public IViewPaneClass
{
	...
	virtual tukk    ClassName()       { return "Vehicle Editor"; };
	virtual tukk    Category()        { return "Game"; };
	//////////////////////////////////////////////////////////////////////////
	virtual CRuntimeClass* GetRuntimeClass() { return RUNTIME_CLASS(CVehicleEditorDialog); };
	virtual tukk    GetPaneTitle()    { return _T("Vehicle Editor"); };
...
//////////////////////////////////////////////////////////////////////////
void CVehicleEditorDialog::OnPrototypeEvent(CBaseObject* object, i32 event)
{
...
//////////////////////////////////////////////////////////////////////////
void CVehicleEditorDialog::OnEntityEvent(CBaseObject* object, i32 event)
{
...
bool CVehicleEditorDialog::ApplyToVehicle(string filename, bool mergeFile)
{
	...
	/*CPakFile pakFile;
	   ...
	   pakFile.UpdateFile( VEHICLE_XML_PATH + filename, (uk )((tukk)xmlString), xmlString.GetLength() );
	   }*/

	string absDir = PathUtil::GamePathToDrxPakPath(VEHICLE_XML_PATH);
	...
	{
		DrxLog("[CVehicleEditorDialog]: saved to %s.", targetFile);
	}
	else
	{
		DrxLog("[CVehicleEditorDialog]: not saved!", targetFile);
		return false;
	}
...
//////////////////////////////////////////////////////////////////////////
void CVehicleEditorDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CVehicleEditorDialog::OnFileSave()
{
	if (!m_pVehicle)
	{
		DrxLog("No active vehicle prototype, can't save to file");
		return;
	}
...
void CVehicleEditorDialog::OnFileSaveAs()
{
	if (!m_pVehicle)
	{
		DrxLog("No active vehicle prototype");
		return;
	}

	// string dir = Path::GetPath(filename);
	string filename("MyVehicle.xml");
	string path = PathUtil::GamePathToDrxPakPath(VEHICLE_XML_PATH);
	...
	if (CFileUtil::SelectSaveFile(VEED_FILE_FILTER, "xml", path, filename))
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "The new vehicle entity will be registered at next Editor/Game launch.");
		}
	}
}
...
void CVehicleEditorDialog::OnPropsSelChanged(CPropertyItem* pItem)
{
	...
	i32 varType = pItem->GetVariable()->GetDataType();
...
bool CVehicleEditorDialog::OpenVehicle(bool silent /*=false*/)
{
	...
	if (!obj->IsKindOf(RUNTIME_CLASS(CEntityObject)))
	{
		if (!silent)
		{
			DrxLog("Selected object %s is no vehicle entity.", obj->GetName());
		}
	...
	if (!pScript)
	{
		if (!silent)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "%s: Entity script is NULL!");
		}
	...
	if (-1 == sFile.MakeLower().Find("vehiclepool"))
	{
		if (!silent)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "%s doesn't seem to be a valid vehicle (got script: %s)", pEnt->GetName(), sFile);
		}
	...
	if (!pProt)
	{
		if (!silent)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Spawn of prototype failed, please inform Code!");
		}
