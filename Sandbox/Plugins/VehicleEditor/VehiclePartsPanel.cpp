...
BOOL CWheelMasterDialog::OnInitDialog()
{
	...
	{
		i32 idx = m_wheelList.AddString((*it)->GetName());
...
void CWheelMasterDialog::ApplyCheckBoxes()
{
	...
	for (i32 i = 0; i < NUM_BOXES && i < pItem->GetChildCount(); ++i)
	{
...
void CWheelMasterDialog::OnApplyWheels()
{
	...
	i32 count = m_wheelList.GetSelCount();

	if (count > 0)
	{
		i32* items = new i32[count];
		...
		for (i32 i = 0; i < count; ++i)
		{
			...
			if (!pSubVar)
			{
				DrxLog("Warning, SubPartWheel not found!");
				continue;
			}

			IVariable* pEditMaster = m_pVar->GetVariable(0);

			for (i32 i = 0; i < pEditMaster->GetNumVariables() && i < NUM_BOXES; ++i)
			{
				if (m_boxes[i] == TRUE)
				{
					CString val;
					...
					tukk varName = pEditMaster->GetVariable(i)->GetName();
					...
					{
						DrxLog("%s: Variable %s not found", pWheel->GetName(), varName);
					}
				}
			}
		}
...
void CWheelMasterDialog::OnToggleAll()
{
	...
	for (i32 i = 1; i < NUM_BOXES; ++i) // don't toggle axle box
	{
...
void CVehiclePartsPanel::OnHelperRename()
{
	...
	if (!pEdit)
	{
		DrxLog("[CVehiclePartsPanel] CEdit for treeCtrl is NULL");
		return;
	}
}
...
void CVehiclePartsPanel::ExpandProps(CPropertyItem* pItem, bool expand /*=true*/)
{
	...
	for (i32 i = 0; i < pItem->GetChildCount(); ++i)
	{
		...
		for (i32 j = 0; j < pChild->GetChildCount(); ++j)
		{
...
void CVehiclePartsPanel::ReloadTreeCtrl()
{
	if (m_pVehicle)
	{
		...
		i32 icon = IVeedObject::GetVeedObject(m_pVehicle)->GetIconIndex();
...
//////////////////////////////////////////////////////////////////////////////
i32 CVehiclePartsPanel::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CVehiclePartsPanel::CreateHelpersFromStatObj(IStatObj* pObj, CVehiclePart* pParent)
{
	...
	for (i32 j = 0; j < pObj->GetSubObjectCount(); ++j)
	{
		IStatObj::SSubObject* pSubObj = pObj->GetSubObject(j);

		//DrxLog("sub <%s> type %i", pSubObj->name, pSubObj->nType);
		if (pSubObj->nType == STATIC_SUB_OBJECT_DUMMY)
		{
			...
			{
				//DrxLog("helper %s already loaded from xml, skipping", pSubObj->name);
				continue;
			}
...
void CVehiclePartsPanel::ShowObject(TPartToTreeMap::iterator it, bool bShow)
{
	...
	else if (it->second.item == 0)
	{
		...
		i32 icon = IVeedObject::GetVeedObject(pObj)->GetIconIndex();
		...
		if (!it->second.item)
		{
			DrxLog("InsertItem for %s failed!", pObj->GetName());
		}
	}
}
...
void CVehiclePartsPanel::FillAssetHelpers()
{
	...
	for (i32 partIndex = 0; partIndex < pVehicle->GetPartCount(); partIndex++)
	{
		...
		if (isAnimatedPart)
		{
			...
			if (pPartEntity != NULL)
			{
				...
				if (pCharacter != NULL)
				{
					...
					for (i32 jointId = 0; jointId < numJoints; jointId++)
					{
...
void CVehiclePartsPanel::FillSeats()
{
	...
	if (pSeatsVar)
	{
		for (i32 i = 0; i < pSeatsVar->GetNumVariables(); ++i)
		{
...
//////////////////////////////////////////////////////////////////////////////
CVehicleWeapon* CVehiclePartsPanel::CreateWeapon(i32 weaponType, CVehicleSeat* pSeat, IVariable* pVar /*=0*/)
{
	CScopedSuspendUndo susp;
...
void CVehiclePartsPanel::FillComps(IVariablePtr pData)
{
	...
	if (pComps)
	{
		i32 numChildren = pComps->GetNumVariables();
...
//////////////////////////////////////////////////////////////////////////////
i32 CVehiclePartsPanel::FillParts(IVariablePtr pData)
{
	CScopedSuspendUndo susp;
...
void CVehiclePartsPanel::AddParts(IVariable* pParts, CBaseObject* pParent)
{
	...
	if (!pParts || !pParent)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "[CVehiclePartsPanel::AddParts] ERROR: called with NULL pointer!");
		return;
	}

	for (i32 i = 0; i < pParts->GetNumVariables(); ++i)
	{
		IVariable* pPartVar = pParts->GetVariable(i);

		if (!pPartVar)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[CVehiclePartsPanel::AddParts] ERROR: pPartVar is NULL!");
		}
		...
		i32 icon = IVeedObject::GetVeedObject(pPartObj)->GetIconIndex();
		...
		{
			i32 nOrigHelpers = pHelpers->GetNumVariables();

			for (i32 h = 0; h < nOrigHelpers; ++h)
			{
...
void CVehiclePartsPanel::FillHelpers(IVariablePtr pData)
{
	...
	{
		for (i32 i = 0; i < pHelpers->GetNumVariables(); ++i)
		{
			IVariable* pHelperVar = pHelpers->GetVariable(i);

			if (!pHelperVar)
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[CVehiclePartsPanel::AddHelpers] ERROR: pHelperVar is NULL!");
			}
...
//////////////////////////////////////////////////////////////////////////
void CVehiclePartsPanel::OnObjectEvent(CBaseObject* object, i32 event)
{
...
//////////////////////////////////////////////////////////////////////////
void CVehiclePartsPanel::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CVehiclePartsPanel::OnScaleHelpers()
{
	...
	if (scale <= 0.f)
	{
		DrxLog("[VehiclePartsPanel]: scale %f invalid", scale);
		return;
	}
