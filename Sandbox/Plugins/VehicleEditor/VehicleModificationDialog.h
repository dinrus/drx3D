...
BOOL CVehicleModificationDialog::OnInitDialog()
{
	...
	i32 width = wp.rcNormalPosition.right - wp.rcNormalPosition.left;
	i32 height = wp.rcNormalPosition.bottom - wp.rcNormalPosition.top;
...
void CVehicleModificationDialog::ReloadModList()
{
	if (m_pVar)
	{
		...
		for (i32 i = 0; i < m_pVar->GetNumVariables(); ++i)
		{
			...
			if (IVariable* pName = GetChildVar(pMod, "name"))
			{
				string name;
				pName->Get(name);
				i32 idx = m_modList.AddString(name);
...
{
	for (i32 i = 0; i < m_modList.GetCount(); ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
void CVehicleModificationDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
	...
	i32 Buttons[] = { IDC_VEEDMOD_CLOSE, IDC_VEEDMOD_SAVE };
	...
	for (i32 i = 0; i < DRX_ARRAY_COUNT(Buttons); ++i)
	{
		if (wnd = GetDlgItem(Buttons[i]))
		{
			WINDOWPLACEMENT wp2;
			wnd->GetWindowPlacement(&wp2);
			i32 dy = wp2.rcNormalPosition.bottom - wp2.rcNormalPosition.top;
