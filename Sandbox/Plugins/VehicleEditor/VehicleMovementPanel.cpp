...
void CVehicleMovementPanel::OnMovementTypeChange(IVariable* pVar)
{
	...
	if (IVariable* pParams = GetChildVar(pDefaults, "MovementParams"))
	{
		string type;
		pVar->Get(type);
		DrxLog("looking up defaults for movement type %s", type);
...
void CVehicleMovementPanel::UpdateVehiclePrototype(CVehiclePrototype* pProt)
{
...

	{
		...
		XmlNodeRef node;
		for (i32 i = 0; i < def->getChildCount(); ++i)
		{
			node = def->getChild(i);
...
i32 CVehicleMovementPanel::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CVehicleMovementPanel::OnSize(UINT nType, i32 cx, i32 cy)
{
