...
class CEquipPackEditorClass : public IViewPaneClass
{
	...
	virtual tukk    ClassName()       override { return "EquipPackEditor"; };
	...
	virtual tukk    GetPaneTitle()    override { return _T("Equip Pack Editor"); };
...
void CEquipPackDialog::UpdateEquipPacksList()
{
	...
	i32 nCurSel = m_EquipPacksList.FindStringExact(0, m_sCurrEquipPack);
	i32k nEquipPacks = m_pEquipPacks->Count();
	if (!nEquipPacks)
		nCurSel = -1;
...
void CEquipPackDialog::UpdateEquipPackParams()
{
	...
	i32 nItem = m_EquipPacksList.GetCurSel();
...
CVarBlockPtr CEquipPackDialog::CreateVarBlock(CEquipPack* pEquip)
{
	...
	i32 nCount = pVarBlock->GetNumVariables();
...
BOOL CEquipPackDialog::OnInitDialog()
{
	...
	if (iter)
	{
		while (iter->Next(item))
		{
			static i32k defaultAmount = 0;
			IVariablePtr pVar = new CVariable<i32>;
...
void CEquipPackDialog::OnBnClickedInsert()
{
	...
	i32 currSelected = m_AvailEquipList.GetCurSel();
