...
bool CEquipPackLib::LoadLibs(bool bExportToGame)
{
	...
	for (i32 iFile = 0; iFile < files.size(); ++iFile)
	{
		...
		// DrxLogAlways("Filename '%s'", filename);

		XmlNodeRef node = XmlHelpers::LoadXmlFromFile(filename);
		if (node == 0)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "CEquipPackLib:LoadLibs: Cannot load pack from file '%s'", filename.GetString());
			continue;
		}

		if (node->isTag("EquipPack") == false)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "CEquipPackLib:LoadLibs: File '%s' is not an equipment pack. Skipped.", filename.GetString());
			continue;
		}
...
void CEquipPackLib::Serialize(XmlNodeRef& xmlNode, bool bLoading, bool bResetWhenLoad)
{
	...
	if (bLoading)
	{
		...
		if (equipXMLNode)
		{
			for (i32 i = 0; i < equipXMLNode->getChildCount(); ++i)
			{
				...
				if (!node->getAttr("name", packName))
				{
					DrxLog("Warning: Unnamed EquipPack found !");
					packName = "Unnamed";
				...
				if (!pCurPack)
				{
					DrxLog("Warning: Unable to create EquipPack %s !", packName.GetString());
					continue;
				}
				pCurPack->Load(node);
