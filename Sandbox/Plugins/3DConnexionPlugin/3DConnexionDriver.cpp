...
//////////////////////////////////////////////////////////////////////////
bool C3DConnexionDriver::GetInputMessageData(uk message, long* returnVal)
{
	...
	else
	{
		if (event->header.dwType == RIM_TYPEHID)
		{
			...
			else if (pRawHid->bRawData[0] == 3) // Buttons (display most significant byte to least)
			{
				...
				buttons[0] = (u8)pRawHid->bRawData[1];
				buttons[1] = (u8)pRawHid->bRawData[2];
				buttons[2] = (u8)pRawHid->bRawData[3];

				DrxLog("Button mask: %.2x %.2x %.2x\n", (u8)pRawHid->bRawData[3], (u8)pRawHid->bRawData[2], (u8)pRawHid->bRawData[1]);
