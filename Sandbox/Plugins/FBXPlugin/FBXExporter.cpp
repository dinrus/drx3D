...
STxt GetFilePath(const STxt& filename)
{
	i32 pos = filename.find_last_of("/\\");
	if (pos > 0)
...
STxt GetFileName(const STxt& fullFilename)
{
	i32 pos = fullFilename.find_last_of("/\\");
	if (pos > 0)
...
tukk CFBXExporter::GetExtension() const
{
	return "fbx";
}

tukk CFBXExporter::GetShortDescription() const
{
	return "FBX format";
}

bool CFBXExporter::ExportToFile(tukk filename, const SExportData* pData)
{
	if (!m_pFBXManager)
	...
	for (i32 objectID = 0; objectID < pData->GetObjectCount(); ++objectID)
	{
	...
	STxt name = GetFileName(filename);
	...
	pDocInfo->mSubject = "Exported geometry from DinrusPro 3D Sandbox application.";
	pDocInfo->mAuthor = "DinrusPro 3D Sandbox FBX exporter.";
	pDocInfo->mRevision = "rev. 2.0";
	pDocInfo->mKeywords = "DinrusPro 3D Sandbox FBX export";
	pDocInfo->mComment = "";
	...
	i32 numObjects = pData->GetObjectCount();
	...
	// solve hierarchy
	for (i32 i = 0; i < m_nodes.size() && i < numObjects; ++i)
	{
	...
	if (m_settings.bAsciiFormat)
	{
		if (nFileFormat < 0 || nFileFormat >= m_pFBXManager->GetIOPluginRegistry()->GetWriterFormatCount())
		{
			...
			i32 lFormatIndex, lFormatCount = m_pFBXManager->GetIOPluginRegistry()->GetWriterFormatCount();
...
FbxMesh* CFBXExporter::CreateFBXMesh(const SExportObject* pObj)
{
	...
	i32 numColors = pObj->GetColorCount();
	...
	for (i32 j = 0; j < numMeshes; ++j)
	{
		...
		i32 polygonCount = pFBXMesh->GetPolygonCount();
		pMaterialElement->GetIndexArray().SetCount(polygonCount + numFaces);
		i32 materialIndex = m_meshMaterialIndices[pMesh];
...
FbxFileTexture* CFBXExporter::CreateFBXTexture(tukk pTypeName, tukk pName)
{
	STxt filename = pName;
...
FbxSurfaceMaterial* CFBXExporter::CreateFBXMaterial(const STxt& name, const SExportMesh* pMesh)
{
...
FbxNode* CFBXExporter::CreateFBXAnimNode(FbxScene* pScene, FbxAnimLayer* pAnimBaseLayer, const SExportObject* pObj)
{
	...
	if (pObj->entityType == Export::eCamera)
	{
		...
		if (pObj->cameraTargetNodeName != "")
		{
			for (i32 i = 0; i < m_nodes.size(); ++i)
			{
				...
				tukk nodeName = pCameraTargetNode->GetName();
	...
	{
		i32 animDataCount = pObj->GetEntityAnimationDataCount();

		for (i32 animDataIndex = 0; animDataIndex < animDataCount; ++animDataIndex)
		{
...
FbxNode* CFBXExporter::CreateFBXNode(const SExportObject* pObj)
{
	...
	{
		for (i32 i = 0; i < pObj->GetMeshCount(); ++i)
		{
			...
			STxt materialName = pObj->materialName;
...
void       CFBXExporter::FillAnimationData(SExportObject* pObject, FbxAnimLayer* pAnimLayer, FbxAnimCurve* pCurve, EAnimParamType paramType)
{
	if (pCurve)
	{
		for (i32 keyID = 0; keyID < pCurve->KeyGetCount(); ++keyID)
		{
...
bool CFBXExporter::ImportFromFile(tukk filename, SExportData* pData)
{
	...
	if (pRootNode)
	{
		for (i32 animStackID = 0; animStackID < pFBXScene->GetSrcObjectCount(FbxCriteria::ObjectType(FbxAnimStack::ClassId)); ++animStackID)
		{
			FbxAnimStack* pAnimStack = (FbxAnimStack*)pFBXScene->GetSrcObject(FbxCriteria::ObjectType(FbxAnimStack::ClassId), animStackID);
			i32k animLayersCount = pAnimStack->GetMemberCount(FbxCriteria::ObjectType(FbxAnimLayer::ClassId));

			for (i32 layerID = 0; layerID < animLayersCount; ++layerID)
			{
				FbxAnimLayer* pAnimLayer = (FbxAnimLayer*)pAnimStack->GetMember(FbxCriteria::ObjectType(FbxAnimLayer::ClassId), layerID);

				i32k nodeCount = pRootNode->GetChildCount();

				for (i32 nodeID = 0; nodeID < nodeCount; ++nodeID)
				{
					...
					if (pNode)
					{
						...
						if (pObject)
						{
							drx_strcpy(pObject->name, pNode->GetName());
