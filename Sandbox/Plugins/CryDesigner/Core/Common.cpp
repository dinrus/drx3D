...
BrushVec3 ScreenPos2WorldPos(const BrushVec3& screenPos)
{
	...
	i32 x = vp.left;
	i32 y = vp.top;
	i32 w = (vp.right - vp.left) + 1;
	i32 h = (vp.bottom - vp.top) + 1;
  ...
  BrushEdge3D& negativeEdge)
{
	...
	for (i32 a = 0; a < 2; ++a)
	{
		for (i32 k = 0; k < 3; ++k)
		{
			for (i32 i = 0, iSplitHelperEdgeCount(splitHelperEdges.size()); i < iSplitHelperEdgeCount; ++i)
			{
	...
	if (d0 > 0 && d1 < 0 || d0 < 0 && d1 > 0)
	{
		...
		for (i32 i = 0, iEdgeSize(splitHelperEdges.size()); i < iEdgeSize; ++i)
		{
...
{
	for (i32 i = nSegmentCount - 1; i >= 0; --i)
	{
...
bool DoesEquivalentExist(std::vector<BrushEdge3D>& edgeList, const BrushEdge3D& edge, i32* pOutIndex)
{
...
bool ComputePlane(const std::vector<Vertex>& vList, BrushPlane& outPlane)
{
	...
	i32 nMinimumIndex = -1;
	i32 iVListCount(vList.size());
	for (i32 i = 0; i < iVListCount; ++i)
	{
	...
	i32 nPrevIndex = ((nMinimumIndex - 1) + iVListCount) % iVListCount;
	i32 nNextIndex = (nMinimumIndex + 1) % iVListCount;
...
void Write2Buffer(std::vector<char>& buffer, ukk pData, i32 nDataSize)
{
	...
	if (pData == 0 || nDataSize == 0)
		return;
	for (i32 i = 0; i < nDataSize; ++i)
		buffer.push_back(((tuk)pData)[i]);
}
...
{
	i32 nShortestIndex = 0;
	BrushFloat fShortestDistance = edges[0].GetLength();

	for (i32 i = 1, iEdgeSize(edges.size()); i < iEdgeSize; ++i)
	{
...
bool PickPosFromWorld(IDisplayViewport* view, const CPoint& point, Vec3& outPickedPos)
{
	...
	i32k types = ent_all;
	i32k flags = (geom_colltype_ray | geom_colltype_obstruct) << rwi_colltype_bit | rwi_colltype_any | rwi_ignore_terrain_holes | rwi_stop_at_pierceable;
	ray_hit hit;
...
{
	for (i32 i = 0, iVertexCount(vertexList.size()); i < iVertexCount; ++i)
	{
