...
void UpdateMirroredPartWithPlane(Model* pModel, const BrushPlane& plane)
{
	...
	for (i32 i = 0, iPolygonSize(polygons.size()); i < iPolygonSize; ++i)
...
bool MakeListConsistingOfArc(const BrushVec2& vOutsideVertex, const BrushVec2& vBaseVertex0, const BrushVec2& vBaseVertex1, i32 nSegmentCount, std::vector<BrushVec2>& outVertexList)
{
	...
	for (i32 k = 0; k < 2; ++k)
	{
		...
		for (i32 i = 0, iArcVertexSize(arcTryVertexList[k].size()); i < iArcVertexSize; ++i)
		{
	...
	for (i32 i = 0, iCount(outVertexList.size()); i < iCount; ++i)
	{
...
{
	for (i32 i = 0, iSourcePolygonCount(sourcePolygons.size()); i < iSourcePolygonCount; ++i)
...
PolygonPtr Convert2ViewPolygon(IDisplayViewport* pView, const BrushMatrix34& worldTM, PolygonPtr pPolygon, PolygonPtr* pOutPolygon)
{
	...
	std::set<i32> verticesBehindCamera;
	...
	if (!verticesBehindCamera.empty())
	{
		...
		i32 nBackPolygonCount = backPolygons.size();
		...
		{
			pViewPolygon->Clear();
			for (i32 i = 0; i < nBackPolygonCount; ++i)
				pViewPolygon->Union(backPolygons[i]);
		}

		for (i32 i = 0, iVertexCount(pViewPolygon->GetVertexCount()); i < iVertexCount; ++i)
		{
...
{
	static i32k kMaxCount(10);
	static i32k kMaxCountToAvoidInfiniteLoop(100);
...
void ResetXForm(MainContext& mc, i32 nResetFlag)
{
...
void CreateMeshFacesFromPolygon(PolygonPtr pPolygon, FlexibleMesh& mesh, bool bCreateBackFaces)
{
	...
	i32 nMatID = pPolygon->GetSubMatID();
	i32 nSubsetNumber = mesh.AddMatID(nMatID);
	for (i32 k = 0, iFaceSize(mesh.faceList.size()); k < iFaceSize; ++k)
...
void TransformUVs(const TexInfo& texInfo, FlexibleMesh& mesh)
{
	...
	for (i32 i = 0, iCount(mesh.vertexList.size()); i < iCount; ++i)
...
void AssignMatIDToSelectedPolygons(i32 matID)
{
	...
	for (i32 i = 0, iCount(pSelect->GetCount()); i < iCount; ++i)
	{
...
std::vector<DesignerObject*> GetSelectedDesignerObjects()
{
	...
	for (i32 i = 0, iSelectionCount(pSelection->GetCount()); i < iSelectionCount; ++i)
	{
...
{
	for (i32 i = 0, iFaceCount(meshFaces.size()); i < iFaceCount; ++i)
	{
		std::vector<Vec3> f;
...
void MergeTwoObjects(const MainContext& mc0, const MainContext& mc1)
{
	...
	i32 nSubMatCount = mc0.pObject->GetMaterial()->GetSubMaterialCount();
	...
	for (i32 shelfID = eShelf_Base; shelfID < cShelfMax; ++shelfID)
	{
		...
		for (i32 i = 0, nPolygonSize(mc1.pModel->GetPolygonCount()); i < nPolygonSize; ++i)
		{
...
{
	for (i32 i = 0, vSize(vertices.size()); i < vSize; ++i)
	{
