...
void UVIsland::Serialize(XmlNodeRef& xmlNode, bool bLoading, bool bUndo, Model* pModel)
{
	...
	{
		for (i32 i = 0, iCount(m_Polygons.size()); i < iCount; ++i)
		{
			QString attr = QString("Polygon%1").arg(i);
...
const AABB& UVIsland::GetUVBoundBox() const
{
	...
	for (i32 i = 0, iPolygonCount(m_Polygons.size()); i < iPolygonCount; ++i)
	{
...
{
	for (i32 k = 0, iPolygonCount(GetCount()); k < iPolygonCount; ++k)
	{
...
void UVIsland::Normalize(const AABB& range)
{
	...
	{
		for (i32 i = 0, iVertexCount((*iPoly)->GetVertexCount()); i < iVertexCount; ++i)
		{
...
{
	for (i32 i = 0, iPolygonCount(GetCount()); i < iPolygonCount; ++i)
	{
...
void UVIsland::Clean(Model* pModel)
{
	...
	for (; ii != m_Polygons.end(); )
	{
		...
		for (i32 iShelf = eShelf_Base; iShelf < cShelfMax; ++iShelf)
		{
...
i32 UVIsland::GetSubMatID() const
{
	if (m_Polygons.empty())
		return 0;
...
bool UVIsland::HasSubMatID(i32 nSubMatID) const
{
	auto ii = m_Polygons.begin();
	for (; ii != m_Polygons.end(); ++ii)
	{
...
bool UVIsland::FindIsolatedAreas(std::vector<UVIslandPtr>& outIsolatedAreas)
{
	...
	i32 polygonCount = (i32)m_Polygons.size();
