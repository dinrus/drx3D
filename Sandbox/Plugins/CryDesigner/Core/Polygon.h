...
//! A polygon in DrxDesigner means both a non-closed path and a simple polygon, which is a flat shape consisting of straight, non-intersecting line segments that are joined pair-wise to form a closed path
...
public:
	...
	bool Intersect(
	  PolygonPtr BPolygon,
	  u8 includeCoEdgeFlags = eICEII_IncludeBothBoundary);
	...
	void SetSubMatID(i32 nSubMatID);

	//! Returns the sub material ID.
	//! \return the current sub material ID
	i32 GetSubMatID() const { return m_SubMatID; }
	...
	BrushEdge3D GetEdge(i32 nEdgeIndex) const;
	...
	i32 GetEdgeIndex(const BrushEdge3D& edge3D) const;

	//! Returns the edge index pair referred by the input index
	const IndexPair& GetEdgeIndexPair(i32 nEdgeIndex) const { return m_Edges[nEdgeIndex]; }
	...
	bool CheckFlags(i32 nFlags) const;

	//! Adds the new flags.
	i32 AddFlags(i32 nFlags) { return m_Flag |= nFlags; }

	//! Replaces the existing flag with the new one.
	void SetFlag(i32 nFlag) { m_Flag = nFlag; }

	//! Removes the specific flags.
	i32 RemoveFlags(i32 nFlags) { return m_Flag &= ~nFlags; }

	//! Returns the flag.
	i32 GetFlag() const { return m_Flag; }
	...
	const BrushVec3& GetPos(i32 nIndex) const { return m_Vertices[nIndex].pos; }

	//! Returns the uv of the vertex referred by the input index.
	const Vec2& GetUV(i32 nIndex) const { return m_Vertices[nIndex].uv; }

	//! Returns a vertex referred by the input index.
	const Vertex& GetVertex(i32 nIndex) const { return m_Vertices[nIndex]; }
	...
	i32 GetVertexCountAbovePlane(const BrushPlane& plane) const;
	...
	bool HasPosition(const BrushVec3& position, i32* pOutVertexIndex = NULL) const;
	...
	void RemoveEdge(i32 nEdgeIndex);
	...
	ETrackingResult TrackVertices(
	  ...
	  std::vector<i32>* pOutVertexIndices = NULL) const;
	...
	DrxGUID GetGUID() const { return m_GUID; }
...
private:
	...
	i32  LoadBinary(std::vector<char>& buffer);
	...
	static bool DoesIdenticalEdgeExist(
	  const std::vector<IndexPair>& edges,
	  const IndexPair& e, i32* pOutIndex = NULL);
	...
	bool QueryEdges(
	  const BrushVec3& vertex,
	  i32 vIndexInEdge,
	  std::set<i32>* pOutEdgeIndices = NULL) const;
	...
	BrushEdge GetEdge2D(i32 nEdgeIndex) const;
...
private:
	...
	void SetVertex_Basic(i32 nIndex, const Vertex& vertex)
	{
		InvalidateCacheData();
		m_Vertices[nIndex] = vertex;
	}
	void SetPos_Basic(i32 nIndex, const BrushVec3& pos)
	{
		InvalidateCacheData();
		m_Vertices[nIndex].pos = pos;
	}
	void SetUV_Basic(i32 nIndex, const Vec2& uv)
	{
		InvalidateCacheData();
	...
	void SwapEdgeIndex_Basic(i32 nIndex)
	{
		InvalidateCacheData();
		std::swap(m_Edges[nIndex].m_i[0], m_Edges[nIndex].m_i[1]);
	}
	bool CheckPrivateFlags(i32 nFlags) const;
	i32  AddPrivateFlags(i32 nFlags) const    { return m_PrivateFlag |= nFlags; }
	i32  RemovePrivateFlags(i32 nFlags) const { return m_PrivateFlag &= ~nFlags; }
