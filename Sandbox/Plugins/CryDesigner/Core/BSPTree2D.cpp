...
public:
	...
	i32                      GetCoincidentSize() const                  { return m_coEdge3DList.size(); }
	const BrushEdge3D& GetCoincidentEdge(i32 n) const             { return m_coEdge3DList[n]; }
...
EPointPosEnum BSPTree2DNode::IsVertexIn(const BrushVec3& vertex) const
{
	...
	else
	{
		...
		{
			for (i32 i = 0, iCoEdgeSize(m_coEdge3DList.size()); i < iCoEdgeSize; ++i)
			{
				BrushVec3 v;
...
void BSPTree2DNode::GetPartitions(const BrushEdge3D& inEdge, BSPTree2D::OutputEdges& outEdges) const
{
	...
	else
	{
		...
		for (i32 i = 0, iSize(m_coEdge3DList.size()); i < iSize; ++i)
		{
			BrushEdge3D outE;
		...
		{
			for (i32 i = 0, iSize(intersectionEdges.size()); i < iSize; ++i)
			{
			...
			for (i32 i = 0, iSize(resultEdges[nTurn].size()); i < iSize; ++i)
...
BSPTree2DNode* BSPTree2D::ConstructTree(const BrushPlane& plane, const std::vector<BrushEdge3D>& edgeList)
{
	...
	for (i32 i = 1, edgeLineSize(edgeList.size()); i < edgeLineSize; ++i)
	{
		BrushEdge3D posEdge;
		BrushEdge3D negEdge;
