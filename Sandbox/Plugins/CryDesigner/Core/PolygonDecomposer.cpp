...
bool PolygonDecomposer::TriangulatePolygon(PolygonPtr pPolygon, std::vector<PolygonPtr>& outTrianglePolygons)
{
	...
	for (i32 i = 0, iFaceList(mesh.faceList.size()); i < iFaceList; ++i)
	{
...
bool PolygonDecomposer::TriangulatePolygon(PolygonPtr pPolygon, FlexibleMesh& outMesh)
{
	...
	for (i32 i = 0, faceCount(outMesh.faceList.size()); i < faceCount; ++i)
...
bool PolygonDecomposer::DecomposeNonplanarQuad()
{
	...
	i32 v_order[] = { 0, 1, 2, 2, 3, 0 };
	i32 n_order[] = { 2, 0, 2, 2, 1, 2 };
...
bool PolygonDecomposer::Decompose()
{
	...
	for (i32 k = 0, iOuterPolygonsCount(outerPolygons.size()); k < iOuterPolygonsCount; ++k)
	{
		...
		if (m_bGenerateConvexes)
		{
			for (i32 i = 0, iVertexCount(m_VertexList.size()); i < iVertexCount; ++i)
				m_InitialEdgesSortedByEdge.insert(GetSortedEdgePair(i, (i + 1) % iVertexCount));
		}

		i32 iVertexSize(m_VertexList.size());
		...
		for (i32 i = 0, innerPolygonSize(innerPolygons.size()); i < innerPolygonSize; ++i)
		{
			if (!pOuterPolygon->IncludeAllEdges(innerPolygons[i]))
				continue;
			bHasInnerHull = true;
			i32 nBaseIndex = m_PointList.size();
			std::vector<Vertex> innerVertexList;
			FillVertexListFromPolygon(innerPolygons[i], innerVertexList);
			for (i32 k = 0, nVertexSize(innerVertexList.size()); k < nVertexSize; ++k)
			{
				i32 nPrevIndex = k == 0 ? nBaseIndex + nVertexSize - 1 : nBaseIndex + k - 1;
				i32 nNextIndex = k < nVertexSize - 1 ? m_PointList.size() + 1 : nBaseIndex;
			...
			if (m_bGenerateConvexes)
			{
				for (i32 i = 0, iVertexCount(innerVertexList.size()); i < iVertexCount; ++i)
	...
	i32 vertexCount(m_pOutData->vertexList.size());
...
{
	i32 iIndexCount(indexList.size());
	if (iIndexCount < 3)
		return false;
...
BSPTree2D* PolygonDecomposer::GenerateBSPTree(const IndexList& indexList) const
{
	...
	for (i32 i = 0, iIndexListCount(indexList.size()); i < iIndexListCount; ++i)
	{
		i32 nCurr = indexList[i];
		i32 nNext = indexList[(i + 1) % iIndexListCount];
...
{
	for (i32 i = 0, nIndexBuffSize(indexList.size()); i < nIndexBuffSize; ++i)
	{
		i32 nNext = (i + 1) % nIndexBuffSize;
		...
		if (DecomposerComp::IsEquivalent(v0.y, v1.y))
		{
			...
			if (outEdgeList.find(fY) != outEdgeList.end())
			{
				...
				for (i32 k = 0, edgeListCount(edgeList.size()); k < edgeListCount; ++k)
				{
...
void PolygonDecomposer::BuildPointSideList(const IndexList& indexList, const EdgeListMap& edgeListHavingSameY, std::vector<EDirection>& outPointSideList, std::map<PointComp, std::pair<i32, i32>>& outSortedPointsMap) const
{
	outPointSideList.resize(m_VertexList.size());

	i32 nLeftTopIndex = FindLeftTopVertexIndex(indexList);
	i32 nRightBottomIndex = FindRightBottomVertexIndex(indexList);
	...
	for (i32 i = 0, nIndexBuffSize(indexList.size()); i < nIndexBuffSize; ++i)
	{
		...
		i32 xPriority = outPointSideList[indexList[i]] == eDirection_Left ? 0 : 100;
		...
		if (ifY != edgeListHavingSameY.end())
		{
			...
			for (i32 k = 0, iIndexEdgeCount(indexEdgeList.size()); k < iIndexEdgeCount; ++k)
			{
		...
		outSortedPointsMap[pointComp] = std::pair<i32, i32>(indexList[i], i);
	}
...
bool PolygonDecomposer::TriangulateMonotonePolygon(const IndexList& indexList, std::vector<SMeshFace>& outFaceList) const
{
	...
	std::map<PointComp, std::pair<i32, i32>> yxSortedPoints;
	BuildPointSideList(indexList, edgeListHavingSameY, sideList, yxSortedPoints);

	std::stack<i32> vertexStack;
	...
	{
		i32 top = vertexStack.top();
		i32 current = ii->second.first;

		DESIGNER_ASSERT(vertexStack.size() >= 2);

		vertexStack.pop();
		i32 prev = vertexStack.top();
		...
		{
			i32 nBaseFaceIndex = outFaceList.size();
...
bool PolygonDecomposer::TriangulateConcave(const IndexList& indexList, std::vector<SMeshFace>& outFaceList)
{
	...
	for (i32 i = 0, iMonotonePiecesCount(monotonePieces.size()); i < iMonotonePiecesCount; ++i)
	{
...
bool PolygonDecomposer::IsCCW(i32 i0, i32 i1, i32 i2) const
{
...
bool PolygonDecomposer::IsCCW(const IndexList& indexList, i32 nCurr) const
{
	i32 nIndexListCount = indexList.size();
...
bool PolygonDecomposer::IsConvex(const IndexList& indexList, bool bGetPrevNextFromPointList) const
{
	...
	for (i32 i = 0; i < iIndexCount; ++i)
	{
		i32 nPrev = bGetPrevNextFromPointList ? m_PointList[indexList[i]].prev : indexList[((i - 1) + iIndexCount) % iIndexCount];
		i32 nNext = bGetPrevNextFromPointList ? m_PointList[indexList[i]].next : indexList[(i + 1) % iIndexCount];
...
BrushFloat PolygonDecomposer::Cosine(i32 i0, i32 i1, i32 i2) const
{
...
bool PolygonDecomposer::GetNextIndex(i32 nCurr, const IndexList& indices, i32& nOutNextIndex) const
{
	for (i32 i = 0, iSize(indices.size()); i < iSize; ++i)
	{
		if (indices[i] == nCurr)
		{
...
bool PolygonDecomposer::HasArea(i32 i0, i32 i1, i32 i2) const
{
...
void PolygonDecomposer::AddFace(i32 i0, i32 i1, i32 i2, const IndexList& indices, std::vector<SMeshFace>& outFaceList) const
{
	if (!HasArea(i0, i1, i2))
		return;

	bool bAddedSubTriangles = false;
	i32 faceIndex[3] = { i0, i1, i2 };
	for (i32 i = 0; i < 3; ++i)
	{
		i32 nPrev = faceIndex[((i - 1) + 3) % 3];
		i32 nCurr = faceIndex[i];
		i32 nNext = faceIndex[(i + 1) % 3];

		i32 nIndex = 0;
		i32 nPrevIndex = nNext;
...
void PolygonDecomposer::AddTriangle(i32 i0, i32 i1, i32 i2, std::vector<SMeshFace>& outFaceList) const
{
	if (!HasArea(i0, i1, i2))
		return;

	SMeshFace newFace;
	i32 nBaseVertexIndex = m_nBasedVertexIndex;
	i32 _i0 = i0 + nBaseVertexIndex;
	i32 _i1 = i1 + nBaseVertexIndex;
	i32 _i2 = i2 + nBaseVertexIndex;
	...
	if (m_bGenerateConvexes)
	{
		...
		triangle[1] = i1;
		triangle[2] = i2;
		i32 nConvexIndex = m_Convexes.size();
		m_Convexes.push_back(triangle);

		std::pair<i32, i32> e[3] = { GetSortedEdgePair(i0, i1), GetSortedEdgePair(i1, i2), GetSortedEdgePair(i2, i0) };
		for (i32 i = 0; i < 3; ++i)
		{
...
bool PolygonDecomposer::IsInside(BSPTree2D* pBSPTree, i32 i0, i32 i1) const
{
	if (!pBSPTree)
		return true;
...
bool PolygonDecomposer::IsOnEdge(BSPTree2D* pBSPTree, i32 i0, i32 i1) const
{
	if (!pBSPTree)
		return true;
...
bool PolygonDecomposer::HasAlreadyAdded(i32 i0, i32 i1, i32 i2, const std::vector<SMeshFace>& faceList) const
{
	for (i32 i = 0, iFaceCount(faceList.size()); i < iFaceCount; ++i)
	{
...
bool PolygonDecomposer::IsInsideEdge(i32 i0, i32 i1, i32 i2) const
{
...
bool PolygonDecomposer::IsDifferenceOne(i32 i0, i32 i1, const IndexList& indexList) const
{
	i32 nIndexCount = indexList.size();
	if (i0 == 0 && i1 == nIndexCount - 1 || i0 == nIndexCount - 1 && i1 == 0)
		return true;
	return std::abs(i0 - i1) == 1;
}

i32 PolygonDecomposer::FindLeftTopVertexIndex(const IndexList& indexList) const
{
	return FindExtreamVertexIndex<std::less<YXPair>>(indexList);
}

i32 PolygonDecomposer::FindRightBottomVertexIndex(const IndexList& indexList) const
{
	return FindExtreamVertexIndex<std::greater<YXPair>>(indexList);
}

template<class _Pr>
i32 PolygonDecomposer::FindExtreamVertexIndex(const IndexList& indexList) const
{
	std::map<YXPair, i32, _Pr> YXSortedList;
...
PolygonDecomposer::EDirection PolygonDecomposer::QueryPointSide(i32 nIndex, const IndexList& indexList, i32 nLeftTopIndex, i32 nRightBottomIndex) const
{
	i32 nBoundary[] = { nLeftTopIndex, nRightBottomIndex };
	PolygonDecomposer::EDirection side[] = { eDirection_Left, eDirection_Right };
	i32 nIndexBuffSize = indexList.size();

	for (i32 i = 0, iSize(sizeof(nBoundary) / sizeof(*nBoundary)); i < iSize; ++i)
	{
		i32 nTraceIndex = nBoundary[i];
...
i32 PolygonDecomposer::FindDirectlyLeftEdge(i32 nBeginIndex, const IndexList& edgeSearchList, const IndexList& indexList) const
{
	i32 nIndexCount = indexList.size();
	...
	for (i32 i = 0, iEdgeCount(edgeSearchList.size()); i < iEdgeCount; ++i)
	{
		i32 nIndex = indexList[edgeSearchList[i]];
		i32 nNextIndex = m_PointList[nIndex].next;
...
void PolygonDecomposer::EraseElement(i32 nIndex, IndexList& indexList) const
{
	for (i32 i = 0, indexCount(indexList.size()); i < indexCount; ++i)
	{
...
void PolygonDecomposer::AddDiagonalEdge(i32 i0, i32 i1, EdgeSet& diagonalSet) const
{
	i32 i0Prev = m_PointList[i0].prev;
	i32 i0Next = m_PointList[i0].next;
	i32 i1Prev = m_PointList[i1].prev;
	i32 i1Next = m_PointList[i1].next;
...
bool PolygonDecomposer::DecomposePolygonIntoMonotonePieces(const IndexList& indexList, std::vector<IndexList>& outMonatonePieces)
{
	...
	std::map<PointComp, i32, std::less<PointComp>> yMap;

	for (i32 i = 0, nIndexCount(indexList.size()); i < nIndexCount; ++i)
	{
		pointTypeList.push_back(QueryPointType(i, indexList));
		PointComp newPoint(m_PointList[indexList[i]].pos.y, (i32)((m_PointList[indexList[i]].pos.x) * 10000.0f));
	...
	for (; yIter != yMap.end(); ++yIter)
	{
		...
		i32 index = yIter->second;
		i32 prev = m_PointList[index].prev;
		...
		case ePointType_Split:
			{
				i32 nDirectlyLeftEdgeIndex = FindDirectlyLeftEdge(index, edgeSearchList, indexList);
...
{
	i32 iIndexListCount(indexList.size());

	EdgeSet handledEdgeSet;
	EdgeSet edgeSet;
	EdgeMap edgeMap;

	for (i32 i = 0; i < iIndexListCount; ++i)
	{
...
PolygonDecomposer::EPointType PolygonDecomposer::QueryPointType(i32 nIndex, const IndexList& indexList) const
{
	i32 iIndexCount(indexList.size());

	i32 nCurrIndex = indexList[nIndex];
	i32 nPrevIndex = m_PointList[nCurrIndex].prev;
	i32 nNextIndex = m_PointList[nCurrIndex].next;
...
PolygonDecomposer::EDirection PolygonDecomposer::QueryInteriorDirection(i32 nIndex, const IndexList& indexList) const
{
	i32 nIndexSize = indexList.size();
...
void PolygonDecomposer::RemoveIndexWithSameAdjacentPoint(IndexList& indexList) const
{
	...
	{
		i32 nCurr = *ii;
		i32 nNext = (ii + 1) == indexList.end() ? nStart : *(ii + 1);
...
void PolygonDecomposer::FindMatchedConnectedVertexIndices(i32 iV0, i32 iV1, const IndexList& indexList, i32& nOutIndex0, i32& nOutIndex1) const
{
	i32 i0 = -1, i1 = -1;
	for (i32 i = 0, iVertexCount(indexList.size()); i < iVertexCount; ++i)
	{
		i32 nNext = (i + 1) % iVertexCount;
...
bool PolygonDecomposer::MergeTwoConvexes(i32 iV0, i32 iV1, i32 iConvex0, i32 iConvex1, IndexList& outMergedPolygon)
{
	...
	for (i32 i = 0, iVertexCount(m_Convexes[iConvex0].size()); i < iVertexCount; ++i)
		outMergedPolygon.push_back(m_Convexes[iConvex0][(i + i01) % iVertexCount]);
	for (i32 i = 0, iVertexCount(m_Convexes[iConvex1].size()); i < iVertexCount - 2; ++i)
...
void PolygonDecomposer::CreateConvexes()
{
	...
	for (; ii != m_ConvexesSortedByEdge.end(); ++ii)
	{
		...
		i32 i0 = ii->first.first;
		i32 i1 = ii->first.second;
	...
	for (i32 i = 0, iConvexCount(m_Convexes.size()); i < iConvexCount; ++i)
	{
		if (m_Convexes[i].empty())
			continue;
		Convex convex;
		for (i32 k = 0, iVListCount(m_Convexes[i].size()); k < iVListCount; ++k)
