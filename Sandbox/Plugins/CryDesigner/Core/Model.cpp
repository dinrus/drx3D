...
Model& Model::operator=(const Model& model)
{
	...
	for (i32 shelfID = eShelf_Base; shelfID < cShelfMax; ++shelfID)
	{
		i32 nPolygonSize(model.m_Polygons[shelfID].size());
		m_Polygons[shelfID].reserve(nPolygonSize);
		for (i32 i = 0; i < nPolygonSize; ++i)
...
{
	for (i32 i = 0, iPolygonCount(GetPolygonCount()); i < iPolygonCount; ++i)
	{
  ...
  std::vector<BrushEdge3D>& outEdges) const
{
	...
	for (i32 i = 0, iPolygonCount(GetPolygonCount()); i < iPolygonCount; ++i)
	{
		...
		bFound = true;
		for (i32 k = 0, iEdgeCount(edgeIndices.size()); k < iEdgeCount; ++k)
...
void Model::QueryOpenPolygons(
  ...
  std::vector<PolygonPtr>& outPolygons) const
{
	for (i32 i = 0, iPolygonCount(m_Polygons[m_ShelfID].size()); i < iPolygonCount; ++i)
	{
...
bool Model::AddUnionPolygon(PolygonPtr pPolygon)
{
	...
	for (i32 i = 0, iPolygonSize(m_Polygons[m_ShelfID].size()); i < iPolygonSize; ++i)
	{
		if (m_Polygons[m_ShelfID][i]->IsOpen())
			continue;
...
bool Model::AddIntersectedPolygon(PolygonPtr pPolygon)
{
	...
	{
		for (i32 i = 0, iPolygonCount(intersectedPolygons.size()); i < iPolygonCount; ++i)
		{
  ...
  bool bApplyUnionToAdjancentPolygon)
{
	...
	i32 iOverlappedPolygonCount = overlappedPolygons.size();
	if (iOverlappedPolygonCount == 0)
	{
		if (!touchedPolygons.empty())
		{
			if (bApplyUnionToAdjancentPolygon)
			{
				...
				for (i32 i = 1, iTouchedPolygonCount(touchedPolygons.size()); i < iTouchedPolygonCount; ++i)
				{
	...
	{
		for (i32 i = 0; i < iOverlappedPolygonCount; ++i)
		{
			...
			{
				PolygonPtr pInputPolygon(pPolygon->Clone());
				for (i32 k = 0; k < iOverlappedPolygonCount; ++k)
				{
					if (i == k)
						continue;
...
bool Model::AddSplitPolygon(PolygonPtr pPolygon)
{
	...
	for (i32 i = 0, iSize(spannedPolygons.size()); i < iSize; ++i)
	{
	...
	for (i32 i = 0, iSize(subtractedPolygons.size()); i < iSize; ++i)
	{
	...
	for (i32 i = 0, iSize(intersectedPolygons.size()); i < iSize; ++i)
	{
		intersectedPolygons[i]->ResetUVs();
  ...
  bool bResetUV)
{
	...
	if (islands.size() >= 2)
	{
		for (i32 k = 0, iIslandCount(islands.size()); k < iIslandCount; ++k)
		{
...
bool Model::EraseEdge(const BrushEdge3D& edge)
{
	...
	if (neighbourPolygons.size() == 1 && neighbourPolygons[0]->IsOpen())
	{
		...
		if (dividedPolygons.empty())
			return true;
		for (i32 i = 0, iPolygonSize(dividedPolygons.size()); i < iPolygonSize; ++i)
	...
	else if (neighbourPolygons.size() == 2)
	{
		...
		else
		{
			...
			for (i32 k = 0, iUnconnectedSize(isolatedPaths.size()); k < iUnconnectedSize; ++k)
			{
				i32 edgeIndex;
				...
				if (isolatedPaths[k]->IsEdgeOverlappedByBoundary(edge, &edgeIndex))
				{
					...
					for (i32 i = 0, iSize(dividedPolygons.size()); i < iSize; ++i)
						AddPolygon(dividedPolygons[i]);
				}
				else
				{
...
bool Model::EraseVertex(const BrushVec3& position)
{
	...
	for (i32 k = 0, iQListCount(qlist.size()); k < iQListCount; ++k)
	{
		for (i32 a = 0, iMarkCount(qlist[k].m_MarkList.size()); a < iMarkCount; ++a)
...
{
	for (i32 i = 0, iCount(GetPolygonCount()); i < iCount; ++i)
	{
...
PolygonPtr Model::QueryPolygon(DrxGUID guid) const
{
	for (i32 i = eShelf_Base; i < cShelfMax; ++i)
	{
		for (i32 k = 0, iPolygonCount(m_Polygons[i].size()); k < iPolygonCount; ++k)
		{
...
PolygonPtr Model::GetPolygonPtr(i32 nIndex) const
{
	DESIGNER_ASSERT(m_ShelfID >= 0 && m_ShelfID < cShelfMax);
  ...
  i32& nOutIndex) const
{
	...
	nOutIndex = -1;
	for (i32 i = 0, polygonSize(m_Polygons[m_ShelfID].size()); i < polygonSize; ++i)
	{
  ...
  std::vector<PolygonPtr>& outPolygons) const
{
	...
	for (i32 i = 0, iPolygonCount(m_Polygons[m_ShelfID].size()); i < iPolygonCount; ++i)
	{
		...
		if (std::abs(pPolygon->GetPlane().Normal().Dot(pCandidatePolygon->GetPlane().Normal())) > kDesignerEpsilon)
			continue;
		for (i32 a = 0, iCandidateEdgeCount(pCandidatePolygon->GetEdgeCount()); a < iCandidateEdgeCount; ++a)
		{
  ...
  BrushVec3* pNormal) const
{
	...
	for (i32 i = 0, iPolygonCount(m_Polygons[m_ShelfID].size()); i < iPolygonCount; ++i)
	{
		...
		for (i32 a = 0, iVertexCount(pPolygon->GetVertexCount()); a < iVertexCount; ++a)
		{
  ...
  bool bUndo)
{
	if (bLoading)
	{
		i32 childNum = xmlNode->getChildCount();

		m_Polygons[0].clear();
		m_Polygons[1].clear();
		m_Polygons[0].reserve(childNum);

		for (i32 i = 0; i < childNum; ++i)
		{
			...
			if (!strcmp(pChildNode->getTag(), "Polygon") || !strcmp(pChildNode->getTag(), "Region"))
			{
				...
				static i32k kBackFaceFlag = BIT(2);
	...
	{
		for (i32 i = 0, iSize(m_Polygons[0].size()); i < iSize; ++i)
		{
			if (m_Polygons[0][i])
			{
...
PolygonPtr Model::GetPolygon(i32 nPolygonIndex) const
{
	DESIGNER_ASSERT(m_ShelfID >= 0 && m_ShelfID < cShelfMax);
...
i32 Model::GetPolygonIndex(PolygonPtr pPolygon) const
{
	DESIGNER_ASSERT(m_ShelfID >= 0 && m_ShelfID < cShelfMax);
  ...
  BrushFloat& outDistance) const
{
	...
	for (i32 i = 0, nPolygonSize(m_Polygons[m_ShelfID].size()); i < nPolygonSize; ++i)
	{
...
void Model::MovePolygonsBetweenShelves(
  ...
  ShelfID destShelfID)
{
	for (i32 i = 0, iPolygonCount(m_Polygons[sourceShelfID].size()); i < iPolygonCount; ++i)
...
bool Model::SplitPolygonsByOpenPolygon(PolygonPtr pOpenPolygon)
{
	...
	for (i32 i = 0, iSize(m_Polygons[m_ShelfID].size()); i < iSize; ++i)
	{
		if (m_Polygons[m_ShelfID][i]->IsOpen())
			continue;
	...
	for (; iPolygon != spannedPolygons.end(); ++iPolygon)
	{
		...
		for (i32 i = 0, iUnconnectedPolygonSize(isolatedPaths.size()); i < iUnconnectedPolygonSize; ++i)
		{
			...
			i32 nNewVtxIdx[2] = { -1, -1 };
			i32 nNewEdgeIdx[2] = { -1, -1 };

			std::set<i32> newEdgeIndexCandidates[2];
			...
			i32 iVertexSize = isolatedPaths[i]->GetVertexCount();
...
PolygonPtr Model::QueryEquivalentPolygon(
  ...
  i32* pOutPolygonIndex) const
{
	i32 iPolygonSize(m_Polygons[nShelfID].size());
  ...
  _smart_ptr<Model>& pOutBackPart) const
{
	...
	for (i32 i = 0, iPolygonSize(m_Polygons[m_ShelfID].size()); i < iPolygonSize; ++i)
	{
		...
		i32 vertexCount = m_Polygons[m_ShelfID][i]->GetVertexCountAbovePlane(clipPlane);
	...
	if (bFillFacet)
	{
		...
		{
			for (i32 i = 0, iPolygonSize(facetPolygons.size()); i < iPolygonSize; ++i)
			{
  ...
  std::vector<PolygonPtr>& outPolygons)
{
	...
	i32 ifacetPolygonsize(facetPolygons.size());

	for (i32 i = 0; i < ifacetPolygonsize; ++i)
	{
		...
		i32 nSmallestIndex = 0;
		i32 iPtSize(polygon.size());
		for (i32 k = 0; k < iPtSize; ++k)
		{
	...
	for (i32 i = 0; i < ifacetPolygonsize; ++i)
	{
		if (usedIndices.find(i) != usedIndices.end())
			continue;
		bool bInclude = false;
		for (i32 k = 0; k < ifacetPolygonsize; ++k)
		{
			if (i == k)
				continue;
			if (usedIndices.find(k) != usedIndices.end())
...
AABB Model::GetBoundBox(ShelfID nShelf)
{
	...
	for (i32 i = eShelf_Base; i < cShelfMax; ++i)
	{
		if (static_cast<ShelfID>(i) == nShelf || nShelf == eShelf_Any)
		{
			...
			for (i32 k = 0, iPolygonCount(GetPolygonCount()); k < iPolygonCount; ++k)
...
void Model::SetSubMatID(i32 nSubMatID)
{
	for (i32 i = 0, iPolygonCount(GetPolygonCount()); i < iPolygonCount; ++i)
	{
		PolygonPtr pPolygon = GetPolygon(i);
