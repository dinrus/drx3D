...
void HalfEdgeMesh::ConstructMesh(Model* pModel)
{
	...
	for (i32 id = eShelf_Base; id < cShelfMax; ++id)
	{
		...
		for (i32 i = 0, iPolygonCount(pModel->GetPolygonCount()); i < iPolygonCount; ++i)
		{
			...
			for (i32 k = 0, iConvexCount(pConvexList->GetConvexCount()); k < iConvexCount; ++k)
...
{
	i32 nStartEdgeIndex = (i32)m_Edges.size();
	i32 nFaceIndex = (i32)m_Faces.size();

	for (i32 i = 0, iVListCount(vConvex.size()); i < iVListCount; ++i)
	{
		i32 nVertexIndex = (i32)m_Vertices.size();
		i32 nEdgeIndex = (i32)m_Edges.size();
...
i32 HalfEdgeMesh::AddPos(const BrushVec3& vPos)
{
	for (i32 i = 0, iCount(m_Positions.size()); i < iCount; ++i)
	{
...
void HalfEdgeMesh::FindEachPairEdge()
{
	...
	for (i32 i = 0; i < nEdgeCount; ++i)
	{
		if (m_Edges[i].pair_edge != -1)
			continue;
		std::pair<i32, i32> e0(GetVertex(m_Edges[i].vertex).pos_index, GetVertex(m_Edges[m_Edges[i].next_edge].vertex).pos_index);
		for (i32 k = 0; k < nEdgeCount; ++k)
		{
			if (i == k || m_Edges[k].pair_edge != -1 || m_Edges[i].face == m_Edges[k].face)
				continue;
			std::pair<i32, i32> e1(GetVertex(m_Edges[k].vertex).pos_index, GetVertex(m_Edges[m_Edges[k].next_edge].vertex).pos_index);
...
BrushVec3 HalfEdgeMesh::GetFaceAveragePos(const HE_Face& f) const
{
	...
	i32 nStartingEdge = f.edge;
	i32 nEdge = nStartingEdge;
	i32 nCount = 0;
	do
	{
...
void HalfEdgeMesh::GetFacePosIndices(const HE_Face& f, std::vector<i32>& outPosIndices) const
{
	i32 nStartingEdge = f.edge;
	i32 nEdge = nStartingEdge;
...
void HalfEdgeMesh::CreateMeshFaces(std::vector<FlexibleMeshPtr>& outMeshes, bool bGenerateBackFaces, bool bEnableSmoothingSurface)
{
	...
	for (i32 i = 0; i < nFaceCount; ++i)
	{
		...
		for (i32 a = 0; a < 2; ++a)
		{
			...
			i32 nVertexOffset = m->vertexList.size();
			i32 iPosIndicesCount(vertices.size());

			for (i32 k = 0; k < iPosIndicesCount; ++k)
			{
			...
			for (i32 k = 0; k < iPosIndicesCount - 2; ++k)
			{
...
{
	i32 nPosCount = m_Positions.size();
	i32 nFaceCount = m_Faces.size();

	outNormals.resize(nPosCount);
	for (i32 i = 0; i < nPosCount; ++i)
...
Model* HalfEdgeMesh::CreateModel(UVIslandManager* pUVIslandMgr)
{
	...
	for (i32 i = 0; i < nFaceCount; ++i)
	{
		...
		for (i32 k = 0; k < he_vertices.size(); ++k)
		{
...
void FindPointBetweenEdge(const BrushEdge3D& edge, PolygonPtr pPolygon, std::vector<PolygonPtr>& neighbourPolygons, std::map<BrushFloat, BrushVec3>& outPoints)
{
	...
	for (i32 i = 0, iNeighbourPolygonCount(neighbourPolygons.size()); i < iNeighbourPolygonCount; ++i)
	{
		PolygonPtr pNeighbourPolygon = neighbourPolygons[i];
		if (pPolygon == pNeighbourPolygon || pNeighbourPolygon->IsOpen())
			continue;

		for (i32 k = 0, iEdgeCount(neighbourPolygons[i]->GetEdgeCount()); k < iEdgeCount; ++k)
		{
...
void HalfEdgeMesh::SolveTJunction(Model* pModel, PolygonPtr pPolygon, Convexes* pConvexes)
{
	...
	for (i32 i = 0, iConvexCount(pConvexes->GetConvexCount()); i < iConvexCount; ++i)
	{
		Convex updatedConvex;
		Convex& convex = pConvexes->GetConvex(i);
		i32 iVertexCount(convex.size());
		updatedConvex.reserve(iVertexCount);
		for (i32 k = 0; k < iVertexCount; ++k)
		{
...
bool HalfEdgeMesh::GetValenceCount(const HE_Vertex& v, i32& nOutValenceCount) const
{
