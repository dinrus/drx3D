...
void Polygon::Reset(const std::vector<Vertex>& vertices)
{
	...
	for (i32 i = 0, iVertexCount(vertices.size()); i < iVertexCount; ++i)
...
void Polygon::InitializeEdgesAndUpdate(bool bClosed)
{
	...
	for (i32 i = 0, iVertexSize(m_Vertices.size()); i < iVertexSize; ++i)
	{
		if (!bClosed)
		{
			if (i == iVertexSize - 1)
  ...
  std::vector<IndexPair>& destincationEdges)
{
	...
	for (i32 i = 0, iEdgeSize(sourceEdges.size()); i < iEdgeSize; ++i)
...
EIntersectionType Polygon::HasIntersection(PolygonPtr pPolygon)
{
	...
	for (i32 i = 0, iSize(pPolygon->m_Vertices.size()); i < iSize; ++i)
	{
	...
	for (i32 i = 0, iSize(pPolygon->GetEdgeCount()); i < iSize; ++i)
	{
	...
	if (bHaveTouched)
	{
		std::vector<std::pair<i32, i32>> vertexIndices;
		for (i32 i = 0, iVertexCount(m_Vertices.size()); i < iVertexCount; ++i)
		{
			i32 nVertexIndex = -1;
			if (!pPolygon->GetVertexIndex(m_Vertices[i].pos, nVertexIndex))
				continue;
			vertexIndices.push_back(std::pair<i32, i32>(i, nVertexIndex));
		}
		if (vertexIndices.size() == 1)
		{
			std::vector<i32> edgeIndices[2];
			...
			for (i32 i = 0, iEdgeIndexCount0(edgeIndices[0].size()); i < iEdgeIndexCount0; ++i)
			{
				BrushEdge3D e0 = GetEdge(edgeIndices[0][i]);
				for (i32 k = 0, iEdgeIndexCount1(edgeIndices[1].size()); k < iEdgeIndexCount1; ++k)
				{
  ...
  BrushEdge3D* pOutIntersectedEdge) const
{
	...
	for (i32 i = 0, iEdgeSize(m_Edges.size()); i < iEdgeSize; ++i)
	{
...
bool Polygon::IsEquivalent(const PolygonPtr& pPolygon) const
{
	...
	for (i32 i = 0, iEdgeSize(GetEdgeCount()); i < iEdgeSize; ++i)
	{
...
{
	for (i32 k = 0, iEdgeCount0(polygon->GetEdgeCount()); k < iEdgeCount0; ++k)
	{
		...
		for (i32 i = 0, iEdgeCount1(GetEdgeCount()); i < iEdgeCount1; ++i)
		{
...
bool Polygon::IncludeAllEdges(PolygonPtr pPolygon) const
{
	...
	for (i32 i = 0, iEdgeCount(pPolygon->GetEdgeCount()); i < iEdgeCount; ++i)
	{
		BrushEdge3D edge = pPolygon->GetEdge(i);
...
{
	for (i32 i = 0, iEdgeSize(edges.size()); i < iEdgeSize; ++i)
	{
  ...
  std::map<BrushFloat, BrushVec3>& outIntersections) const
{
	...
	for (i32 i = 0, iEdgeCount(m_Edges.size()); i < iEdgeCount; ++i)
	{
...
bool Polygon::QueryEdgesContainingVertex(
  const BrushVec3& pos,
  std::vector<i32>& outEdgeIndices) const
{
	bool bAdded = false;
	BrushVec2 ptPos = m_Plane.W2P(pos);
	for (i32 i = 0, iEdgeListCount(m_Edges.size()); i < iEdgeListCount; ++i)
	{
		BrushEdge edge(
		  m_Plane.W2P(m_Vertices[m_Edges[i].m_i[0]].pos),
		  m_Plane.W2P(m_Vertices[m_Edges[i].m_i[1]].pos));
...
bool Polygon::QueryEdges(
  const BrushVec3& pos,
  i32 vIndexInEdge,
  std::set<i32>* pOutEdgeIndices) const
{
...
bool Polygon::QueryAxisAlignedLines(std::vector<BrushLine>& outLines)
{
	...
	for (i32 i = 0, iEdgeSize(GetEdgeCount()); i < iEdgeSize; ++i)
	{
		...
		for (i32 k = 0; k < sizeof(vAxises) / sizeof(*vAxises); ++k)
		{
...
bool Polygon::ShouldOrderReverse(PolygonPtr BPolygon) const
{
	...
	for (i32 i = 0, iEdgeSize(BPolygon->GetEdgeCount()); i < iEdgeSize; ++i)
	{
		...
		for (i32 a = 0; a < 2; ++a)
		{
			...
			{
				for (i32 k = 0, nAEdgeSize(m_Edges.size()); k < nAEdgeSize; ++k)
				{
  ...
  std::vector<IndexPair>& edges) const
{
	...
	std::vector<i32> edgeListSecondDisconnect;
	for (i32 i = 0, iEdgeCount(edges.size()); i < iEdgeCount; ++i)
	{
		bool bFound = false;
		for (i32 k = 0; k < iEdgeCount; ++k)
	...
	{
		std::set<i32> usedIndices;
		for (i32 i = 0, iFirstCount(edgeListFirstDisconnect.size()); i < iFirstCount; ++i)
		{
			if (usedIndices.find(edgeListFirstDisconnect[i]) != usedIndices.end())
				continue;
			BrushFloat theLeastDistance = (BrushFloat)3e10;
			i32 theLeastIndex = -1;
			for (i32 k = 0, iSecondCount(edgeListSecondDisconnect.size()); k < iSecondCount; ++k)
			{
...
void Polygon::RemoveUnconnectedEdges(std::vector<IndexPair>& edges) const
{
	...
	while (bKeepProcessing)
	{
		...
		for (i32 i = 0, iEdgeCount(edges.size()); i < iEdgeCount; ++i)
		{
			i32 nCountWithConnectionFromBeginningToEnd = 0;
			i32 nCountWithConnectionFromEndToBeginning = 0;
			for (i32 k = 0; k < iEdgeCount; ++k)
			{
  ...
  u8 includeCoEdgeFlags)
{
	...
	if (IsOpen(vertices, edges))
	{
		SetEdgeList_Basic(edges);
		DeleteAllVertices_Basic();
		for (i32 i = 0, iVertexSize(vertices.size()); i < iVertexSize; ++i)
...
bool Polygon::Attach(PolygonPtr pPolygon)
{
	...
	i32 nBaseIndex = m_Vertices.size();

	for (i32 i = 0; i < pPolygon->m_Vertices.size(); ++i)
		AddVertex_Basic(pPolygon->GetVertex(i));

	for (i32 i = 0; i < pPolygon->m_Edges.size(); ++i)
  ...
  std::vector<IndexPair>& edges)
{
	...
	for (i32 i = 0, iVertexCount(vertices.size()); i < iVertexCount; ++i)
	{
		for (i32 a = 0; a < 3; ++a)
		{
			...
			for (i32 k = 0, iXYZListCount(xyzList[a].size()); k < iXYZListCount; ++k)
			{
	...
	for (i32 i = 0, iCount(vertices.size()); i < iCount; ++i)
		vertices[i].id = 0;
...
void Polygon::UpdatePrivateFlags() const
{
	...
	if (!IsOpen(m_Vertices, m_Edges))
	{
		...
		else if (!HasHoles())
		{
			...
			{
				AddPrivateFlags(eRPF_ConvexHull);
				for (i32 i = 0, iPolygonSize(polygon.size()); i < iPolygonSize; ++i)
				{
					i32 nNextI = (i + 1) % iPolygonSize;
					i32 nDoubleNextI = (i + 2) % iPolygonSize;
...
{
	for (i32 i = 0, iSize(GetEdgeCount()); i < iSize; ++i)
	{
		BrushEdge3D edge = GetEdge(i);
		if (edge.IsPoint())
			return false;
  ...
  u8 vertexID) const
{
	...
	for (i32 i = 0, iEdgeSize(m_Edges.size()); i < iEdgeSize; ++i)
	{
		...
		else if (clipObjective == eCO_Subtract)
		{
			if (cliptype == eCT_Positive)
			{
				for (i32 a = 0, iCount(outEdges.negList.size()); a < iCount; ++a)
		...
		for (i32 a = 0; a < validEdges.size(); ++a)
		{
			for (i32 k = 0, iPosEdgeSize(validEdges[a].size()); k < iPosEdgeSize; k++)
			{
				i32 index0(Designer::AddVertex(vertices, Vertex(validEdges[a][k].m_v[0], validEdges[a][k].m_data[0], vertexID)));
				i32 index1(Designer::AddVertex(vertices, Vertex(validEdges[a][k].m_v[1], validEdges[a][k].m_data[1], vertexID)));
...
bool Polygon::GetAdjacentEdgeIndexWithEdgeIndex(
  i32 edgeIndex,
  i32& outPrevEdgeIndex,
  i32& outNextEdgeIndex,
  const std::vector<Vertex>& vertices,
  const std::vector<IndexPair>& edges) const
{
	outPrevEdgeIndex = -1;
	...
	std::vector<i32> edgesPreviousFirstVertex;
	if (GetAdjacentPrevEdgeIndicesWithVertexIndex(edge.m_i[0], edgesPreviousFirstVertex, edges))
	{
		...
		else if (edgesPreviousFirstVertex.size() > 1)
		{
			...
			for (i32 i = 0, iEdgeCount(edgesPreviousFirstVertex.size()); i < iEdgeCount; ++i)
	...
	std::vector<i32> edgesNextSecondVertex;
	if (GetAdjacentNextEdgeIndicesWithVertexIndex(edge.m_i[1], edgesNextSecondVertex, edges))
	{
		...
		{
			EdgeIndexSet candidateSecondIndices;
			for (i32 i = 0, iEdgeCount(edgesNextSecondVertex.size()); i < iEdgeCount; ++i)
  ...
  const std::vector<IndexPair>& edges) const
{
	...
	if (IsOpen(vertices, edges))
	{
		...
		for (i32 i = 0, nEdgeSize(edges.size()); i < nEdgeSize; ++i)
		{
			bool bExist(false);
			for (i32 k = 0; k < nEdgeSize; ++k)
			{
				if (i == k)
					continue;
				if (edges[i].m_i[0] == edges[k].m_i[1])
	...
	while (outVertexList.size() < edges.size() &&
	       (counter++) < edges.size())
	{
		std::vector<i32> linkedNextEdges;
		...
		{
			EdgeIndexSet candidateSecondIndices;
			for (i32 i = 0, iEdgeCount(linkedNextEdges.size()); i < iEdgeCount; ++i)
  ...
  std::vector<IndexPair>& edges) const
{
	...
	for (i32 i = 0, iEdgeSize(edges.size()); i < iEdgeSize; ++i)
	{
		...
		i32 edgeIndexInEnds[2] = { i, i };
		for (i32 direction = 0; direction < 2; ++direction)
		{
			if (edgeLists[direction].empty())
				continue;
			i32 iEdgeListCount = edgeLists[direction].size();
			for (i32 k = 0; k < iEdgeListCount; ++k)
			{
				i32 edgeIndex(edgeLists[direction][k]);
...
{
	std::set<i32> removedEdgeIndices;
	for (i32 i = 0, iEdgeSize(edges.size()); i < iEdgeSize; ++i)
	{
	...
	auto ii = edges.begin();
	for (i32 counter = 0; ii != edges.end(); ++counter)
	{
...
void Polygon::SearchLinkedColinearEdges(
  ...
  const std::vector<Vertex>& vertices,
  const std::vector<IndexPair>& edges,
  std::vector<i32>& outLinkedEdges) const
{
	i32 outEdgeIndices[2];
	BrushLine edgeLine(GetLineFromEdge(edgeIndex, vertices, edges));
	i32 counter(0);
	i32 edgeSize(edges.size());
  ...
  std::vector<IndexPair>& edges) const
{
	...
	std::map<i32, i32> vertexMapper;
	newVertices.reserve(vertices.size());
	for (i32 i = 0, edgeSize(edges.size()); i < edgeSize; ++i)
	{
		for (i32 a = 0; a < 2; ++a)
		{
			if (vertexMapper.find(edges[i].m_i[a]) == vertexMapper.end())
			{
				i32 newIndex = newVertices.size();
...
i32 Polygon::LoadBinary(std::vector<char>& buffer)
{
	i32 nBufferPos = 0;
	i32 nVertexCount = 0;
	nBufferPos = ReadFromBuffer(buffer, nBufferPos, &nVertexCount, sizeof(i32));
	m_Vertices.clear();
	if (nVertexCount > 0)
	{
		std::vector<BrushVec3> positions(nVertexCount);
	...
	i32 nEdgeCount = 0;
	m_Edges.clear();
	nBufferPos = ReadFromBuffer(buffer, nBufferPos, &nEdgeCount, sizeof(i32));
	if (nEdgeCount > 0)
	{
		m_Edges.resize(nEdgeCount);
		nBufferPos = ReadFromBuffer(buffer, nBufferPos, &m_Edges[0], sizeof(IndexPair) * nEdgeCount);
	}
	nBufferPos = ReadFromBuffer(buffer, nBufferPos, &m_Flag, sizeof(u32));
	nBufferPos = ReadFromBuffer(buffer, nBufferPos, &m_SubMatID, sizeof(i32));
  ...
  bool bUndo)
{
	if (bLoading)
	{
		...
		{
			i32 nLength = strlen(srcString);
			if (nLength > 0)
			{
				std::vector<char> buffer;
				i32 nDestBufferLen = Base64::decodedsize_base64(nLength);
				buffer.resize(nDestBufferLen);
				Base64::decode_base64(&buffer[0], srcString, nLength, false);
				if (!buffer.empty())
				{
					i32 offset = LoadBinary(buffer);
					i32 decoded_offset = Base64::encodedsize_base64(offset + 1);
  ...
  bool bUndo)
{
	...
	for (i32 i = 0;; ++i)
	{
		...
		if (!xmlNode->getAttr(attribute, edgeindices))
			break;
		i32 e[2] = { (edgeindices & 0xFFFF0000) >> 16, edgeindices & 0x0000FFFF };
  ...
  bool bUndo)
{
	...
	for (i32 i = 0, iSize(m_Vertices.size()); i < iSize; ++i)
	{
		string attribute;
		attribute.Format("v%d", i);
	...
	for (i32 i = 0, iSize(m_Edges.size()); i < iSize; ++i)
	{
		string attribute;
		attribute.Format("e%d", i);
		i32 edgeindices((m_Edges[i].m_i[0] << 16) | (m_Edges[i].m_i[1]));
  ...
  const BrushVec3& projectionDir)
{
	...
	i32 numberOfVertices = (i32)m_Vertices.size();
	i32 numberOfEdges = (i32)m_Edges.size();

	newVertices.reserve(numberOfVertices);
	newEdges.reserve(numberOfEdges);

	std::map<i32, i32> indexMap;
	...
	for (i32 i = 0; i < numberOfEdges; ++i)
	{
		...
		i32 newIndex0 = iIndex0->second;
		i32 newIndex1 = iIndex1->second;
...
i32 Polygon::GetVertexCountAbovePlane(const BrushPlane& plane) const
{
	i32 countAbovePlane = 0;
	for (i32 i = 0, iVertexCount(GetVertexCount()); i < iVertexCount; ++i)
	{
...
BrushVec3 Polygon::GetCenterPosition() const
{
	...
	i32 iVertexSize(m_Vertices.size());
	for (i32 i = 0; i < iVertexSize; ++i)
	{
...
BrushVec3 Polygon::GetRepresentativePosition() const
{
	...
	{
		std::map<BrushFloat, i32, std::greater<BrushFloat>> verticesSortedByDistance;
		for (i32 i = 1, iVertexCount(m_Vertices.size()); i < iVertexCount; ++i)
		{
  ...
  bool bCheckBoundary)
{
	...
	if (bCheckBoundary)
	{
		...
		if (outPolygon.size() == 1 && !innerPolygons.empty())
		{
			...
			i32 innerPolygonSize(innerPolygons.size());
			for (i32 i = 0; i < innerPolygonSize; ++i)
			{
			...
			for (i32 i = 0; i < innerPolygonSize; ++i)
			{
				if (!outPolygon[0]->IncludeAllEdges(innerPolygons[i]))
					return false;
				for (i32 k = i + 1; k < innerPolygonSize; ++k)
				{
	...
	i32 iEdgeSize(m_Edges.size());

	edges.resize(iEdgeSize);
	lines.resize(iEdgeSize);

	for (i32 i = 0; i < iEdgeSize; ++i)
	{
	...
	for (i32 i = 0; i < iEdgeSize; ++i)
	{
		...
		if (bCheckBoundary)
		{
			...
			i32 nNextI = (i + 1) % iEdgeSize;
			i32 nPrevI = (i - 1) < 0 ? iEdgeSize - 1 : (i - 1);

			for (i32 k = 0; k < iEdgeSize; ++k)
			{
...
{
	i32 nPrevEdgeIndex = -1;
	i32 nNextEdgeIndex = -1;
  ...
  const BrushVec3& direction) const
{
	...
	for (i32 i = 0, iVertexSize(pPolygon->m_Vertices.size()); i < iVertexSize; ++i)
	{
...
bool Polygon::GetEdgeIndex(
  ...
  i32& nEdgeIndex)
{
	for (i32 i = 0, iEdgeSize(edgeList.size()); i < iEdgeSize; ++i)
	{
...
i32 Polygon::GetEdgeIndex(const BrushEdge3D& edge3D) const
{
	for (i32 i = 0, iEdgeSize(m_Edges.size()); i < iEdgeSize; ++i)
	{
  ...
  EdgeIndexSet* pOutNewEdgeIndices)
{
	...
	i32 nNearestEdgeIndex(-1);

	i32 iEdgeSize(GetEdgeCount());
	...
	i32 nNewVertexIndex(m_Vertices.size());
...
{
	for (i32 i = 0, iCount(m_Vertices.size()); i < iCount; ++i)
	{
  ...
  bool* bOutFirst) const
{
	...
	for (i32 i = 0, iEdgeSize(m_Edges.size()); i < iEdgeSize; ++i)
	{
		for (i32 k = 0; k < 2; ++k)
		{
			i32 index = m_Edges[i].m_i[k];
			const BrushVec3& vertex = GetPos(index);
			if (!Comparison::IsEquivalent(vertex, position))
				continue;

			std::vector<i32> linkedPrevEdgeIndices;
			std::vector<i32> linkedNextEdgeIndices;
...
i32 Polygon::AddEdge(const BrushEdge3D& edge)
{
...
bool Polygon::Concatenate(PolygonPtr pPolygon)
{
	...
	for (i32 i = 0, nVertexSize(pPolygon->m_Vertices.size()); i < nVertexSize; ++i)
	{
		...
		if (IsEndPoint(pPolygon->GetPos(i), &bThisFirst))
		{
			...
			if (!pPolygon->IsEndPoint(pPolygon->GetPos(i), &bTargetFirst))
				continue;
			for (i32 k = 0, nEdgeSize(pPolygon->m_Edges.size()); k < nEdgeSize; ++k)
			{
...
bool Polygon::GetFirstVertex(BrushVec3& outVertex) const
{
	...
	for (i32 i = 0, iVertexSize(m_Vertices.size()); i < iVertexSize; ++i)
	{
		std::vector<i32> adjacentPrevEdgeIndices;
		std::vector<i32> adjacentNextEdgeIndices;
...
void Polygon::Rearrange()
{
	...
	i32 iVertexSize(linkedVertices.size());
	std::vector<IndexPair> newEdges;
	for (i32 i = 0; i < iVertexSize - 1; ++i)
  ...
  std::vector<PolygonPtr>& outDividedPolygons) const
{
	...
	i32 nVertexSize = m_Vertices.size();
	exVertices.reserve(nVertexSize);
	for (i32 i = 0; i < nVertexSize; ++i)
...
bool Polygon::GetVertexIndex(
  ...
  i32& nOutIndex) const
{
	for (i32 i = 0, nVertexSize(m_Vertices.size()); i < nVertexSize; ++i)
	{
...
i32 Polygon::GetEdgeIndex(
  i32 nVertexIndex0,
  i32 nVertexIndex1) const
{
	for (i32 i = 0, iEdgeCount(m_Edges.size()); i < iEdgeCount; ++i)
	{
...
bool Polygon::GetAdjacentEdgesByEdgeIndex(
  i32 nEdgeIndex,
  i32* pOutPrevEdgeIndex,
  i32* pOutNextEdgeIndex) const
{
	i32 nPrevEdgeIndex = -1;
	i32 nNextEdgeIndex = -1;
  ...
  i32* pOutNextEdgeIndex) const
{
	...
	i32* const pPrevEdgeIndex = pOutPrevEdgeIndex ? pOutPrevEdgeIndex : &prevEdgeIndex;
	i32* const pNextEdgeIndex = pOutNextEdgeIndex ? pOutNextEdgeIndex : &nextEdgeIndex;

	*pPrevEdgeIndex = -1;
	*pNextEdgeIndex = -1;

	std::vector<i32> edgeIndices;
...
bool Polygon::IsCCW() const
{
	...
	if (!GetLinkedVertices(vertices))
		return false;
	i32 nVertexCount = vertices.size();
	if (nVertexCount < m_Vertices.size())
		return false;

	std::map<std::pair<BrushFloat, BrushFloat>, i32> sortedPoints;
	for (i32 i = 0; i < nVertexCount; ++i)
	{
	...
	i32 nLeftTopIndex = sortedPoints.begin()->second;
	i32 nPrev = ((nLeftTopIndex - 1) + nVertexCount) % nVertexCount;
	i32 nNext = (nLeftTopIndex + 1) % nVertexCount;
...
void Polygon::RemoveEdge(const BrushEdge3D& edge)
{
	...
	for (i32 i = 0, iSize(subtractedEdges.size()); i < iSize; ++i)
	{
		i32 nEdgeIdx0 = Designer::AddVertex(m_Vertices, Vertex(subtractedEdges[i].m_v[0]));
		i32 nEdgeIdx1 = Designer::AddVertex(m_Vertices, Vertex(subtractedEdges[i].m_v[1]));
...
void Polygon::RemoveEdge(i32 nEdgeIndex)
{
...
bool Polygon::FindFirstEdgeIndex(
  const std::set<i32>& edgeSet,
  i32& outEdgeIndex) const
{
	i32 iEdgeSize(m_Edges.size());
...
void Polygon::GetIsolatedPaths(std::vector<PolygonPtr>& outIsolatedPaths)
{
	...
	i32 nStartEdgeIndex = nEdgeIndex;
	...
	{
		std::vector<i32> nextEdgeIndices;
...
ETrackingResult Polygon::TrackVertices(
  ...
  std::vector<i32>* pOutVertexIndices) const
{
	i32 nCounter = 0;
	i32 outEdgeIndices[2];
	i32 nCurrentEdgeIdx = nStartEdgeIdx;
...
{
	i32 vertexCountAboveClipPlane = GetVertexCountAbovePlane(clipPlane);
	...
	if (pFrontPart->IsOpen() || pBackPart->IsOpen())
	{
		for (i32 i = 0, iBoundaryVertexCount(boundaryVertices.size()); i < iBoundaryVertexCount; ++i)
		{
		...
		if (GetPlane().IntersectionLine(clipPlane, intersectionLine))
		{
			...
			{
				for (i32 i = 0, iSortedGapEdgeCount(sortedGapEdges.size()); i < iSortedGapEdgeCount; ++i)
				{
					for (i32 k = 0; k < 2; ++k)
					{
						...
						if (bConnect0to0 && bConnect1to1 && bConnect0to1 && bConnect1to0)
						{
							for (i32 a = 0; a < 2; ++a)
							{
								...
								i32 nEdgeIndex = pPolygons[k]->AddEdge(candidateEdge);
								i32 nNextIndex = -1;
  ...
  std::vector<BrushEdge3D>& outGapEdges) const
{
	...
	for (i32 k = 0, iIntersectionSize(vList.size()); k < iIntersectionSize; ++k)
	{
  ...
  std::vector<BrushEdge3D>& outGapEdges) const
{
	...
	for (i32 k = 0, iIntersectionSize(edgeList.size()); k < iIntersectionSize; ++k)
	{
  ...
  EdgeQueryResult& outIntersectionEdges)
{
	...
	for (i32 i = 0, iEdgeCount(pViewPolygon->GetEdgeCount()); i < iEdgeCount; ++i)
	{
...
{
	for (i32 i = 0, iEdgeCount0(GetEdgeCount()); i < iEdgeCount0; ++i)
	{
		BrushEdge3D e0 = GetEdge(i).GetInverted();

		for (i32 k = 0, iEdgeCount1(pPolygon->GetEdgeCount()); k < iEdgeCount1; ++k)
		{
...
bool Polygon::FindOppositeEdge(
  ...
  BrushEdge3D& outEdge) const
{
	i32 nEdgeCount = GetEdgeCount();
	if (nEdgeCount == 4)
	{
		for (i32 a = 0; a < nEdgeCount; ++a)
		{
	...
	{
		for (i32 a = 0, iVertexCount(GetVertexCount()); a < iVertexCount; ++a)
		{
...
void Polygon::SetSubMatID(i32 nSubMatID)
{
	InvalidateTriangles();
...
bool Polygon::CheckFlags(i32 nFlags) const
{
...
bool Polygon::CheckPrivateFlags(i32 nFlags) const
{
