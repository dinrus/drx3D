...
public: //! Query methods
	...
	PolygonPtr QueryPolygon(DrxGUID guid) const;
	...
	PolygonPtr QueryEquivalentPolygon(
	  PolygonPtr pPolygon,
	  i32* pOutPolygonIndex = NULL) const;
...
public: //! Etc methods
	...
	PolygonPtr GetPolygon(i32 nPolygonIndex) const;
	...
	i32 GetPolygonIndex(PolygonPtr pPolygon) const;
	...
	i32 GetPolygonCount() const { return m_Polygons[m_ShelfID].size(); }

	//! Setter of the model flag. usually combination of items of EModelFlag.
	void SetFlag(i32 nFlag) { m_Flags = nFlag; }

	//! Getter of the model flag.
	i32 GetFlag() const { return m_Flags; }

	//! Adds a new model flag.
	void AddFlag(i32 nFlag) { m_Flags |= nFlag; }

	//! Checks if the input flags are set or not.
	bool CheckFlag(i32 nFlags) { return m_Flags & nFlags ? true : false; }

	//! Setter of the subdivision level.
	void SetSubdivisionLevel(u8 nLevel) { m_SubdivisionLevel = nLevel; }

	//! Getter of the subdivision level.
	u8 GetSubdivisionLevel() const { return m_SubdivisionLevel; }

	//! Returns the model DB instance.
	ModelDB* GetDB() const { return m_pDB; }

	//! Refresh the Model DB instance based on this model
	void ResetDB(i32 nFlag, ShelfID nValidShelfID = eShelf_Any);
...
private:
	...
	PolygonPtr GetPolygonPtr(i32 nIndex) const;
	bool       GetVisibleEdge(
	  const BrushEdge3D& edge,
	  const BrushPlane& plane,
