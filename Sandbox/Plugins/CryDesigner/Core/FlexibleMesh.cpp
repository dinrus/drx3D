...
i32 FlexibleMesh::FindMatIdFromSubsetNum(i32 nSubsetNum) const
{
...
i32 FlexibleMesh::AddMatID(i32 nMatID)
{
	i32 nSubsetNumber = 0;
...
void FlexibleMesh::Reserve(i32 nSize)
{
...
void FlexibleMesh::FillIndexedMesh(IIndexedMesh* pMesh) const
{
	...
	i32 vertexCount = vertexList.size();
	i32 faceCount = faceList.size();
...
{
	i32 nVertexOffset = vertexList.size();
	...
	for (i32 i = 0, iFaceCount(mesh.faceList.size()); i < iFaceCount; ++i)
	{
		SMeshFace f;
		for (i32 k = 0; k < 3; ++k)
			f.v[k] = mesh.faceList[i].v[k] + nVertexOffset;
		i32 nMatID = mesh.FindMatIdFromSubsetNum(mesh.faceList[i].nSubset);
