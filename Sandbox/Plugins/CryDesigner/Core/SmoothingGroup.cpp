...
bool SmoothingGroup::CalculateNormal(const BrushVec3& vPos, BrushVec3& vOutNormal) const
{
	...
	{
		for (i32 i = 0, iCount(qResult[0].m_MarkList.size()); i < iCount; ++i)
...
i32 SmoothingGroup::GetPolygonCount() const
{
	return m_Polygons.size();
}

PolygonPtr SmoothingGroup::GetPolygon(i32 nIndex) const
{
...
void SmoothingGroup::UpdateMesh(Model* pModel, bool bGenerateBackFaces)
{
	...
	for (i32 i = 0, iPolygonCount(GetPolygonCount()); i < iPolygonCount; ++i)
	{
		...
		for (i32 k = 0, iVertexCount(mesh.vertexList.size()); k < iVertexCount; ++k)
