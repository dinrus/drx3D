...
void UVIslandManager::Serialize(XmlNodeRef& xmlNode, bool bLoading, bool bUndo, Model* pModel)
{
	...
	{
		Clear();
		i32 childCount = xmlNode->getChildCount();
	...
	{
		for (i32 i = 0, iCount(m_UVIslands.size()); i < iCount; ++i)
		{
...
UVIslandManager& UVIslandManager::operator=(const UVIslandManager& uvIslandMgr)
{
	...
	for (i32 i = 0, iCount(uvIslandMgr.GetCount()); i < iCount; ++i)
...
void UVIslandManager::AddUVIsland(UVIslandPtr pUVIsland)
{
	...
	for (i32 i = 0, iPolygonCount(pUVIsland->GetCount()); i < iPolygonCount; ++i)
	{
		...
		{
			for (i32 k = 0, iIslandCount(owners.size()); k < iIslandCount; ++k)
...
bool UVIslandManager::HasSubMatID(i32 nSubMatID) const
{
	auto ii = m_UVIslands.begin();
...
UVIslandPtr UVIslandManager::FindUVIsland(const DrxGUID& guid) const
{
	auto ii = m_UVIslands.begin();
	for (; ii != m_UVIslands.end(); ++ii)
	{
...
{
	for (i32 i = 0, iCount(pUVIslandManager->GetCount()); i < iCount; ++i)
