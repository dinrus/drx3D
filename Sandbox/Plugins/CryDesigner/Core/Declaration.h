...
struct Vertex
{
	...
	Vertex(const BrushVec3& _pos, const Vec2& _uv, i32 _id = 0) : pos(_pos), uv(_uv), id(_id) {}
	Vertex(const BrushVec3& _pos, i32 _id) : pos(_pos), uv(Vec2(0, 0)), id(_id) {}
...
struct IndexPair
{
	...
	IndexPair(i32 index0, i32 index1)
	{
		m_i[0] = index0;
		m_i[1] = index1;
	}
...
typedef std::set<i32>               EdgeIndexSet;
typedef std::map<i32, EdgeIndexSet> EdgeMap;
}
