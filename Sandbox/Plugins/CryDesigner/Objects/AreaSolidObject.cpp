...
i32 AreaSolidObject::s_nGlobalFileAreaSolidID = 0;
...
void AreaSolidObject::SetAreaId(i32 nAreaId)
{
	m_areaId = nAreaId;
	UpdateGameArea();
}
...
void AreaSolidObject::UpdateGameArea()
{
	...
	for (auto const& polygon : polygonList)
	{
		...
		for (auto const& innerPolygon : innerPolygons)
		{
			...
			for (i32 a = 0, iEdgeCount(innerPolygon->GetEdgeCount()); a < iEdgeCount; ++a)
			{
...
void AreaSolidObject::InvalidateTM(i32 nWhyFlags)
{
...
void AreaSolidObject::Display(CObjectRenderHelper& objRenderHelper)
{
	...
	if (GetCompiler())
	{
		for (i32 i = 0, iCount(GetEntityCount()); i < GetEntityCount(); i++)
		{
