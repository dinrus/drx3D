...
void DesignerObject::DrawOpenPolygons(DisplayContext& dc)
{
	...
	i32 oldLineWidth = dc.GetLineWidth();
...
void DesignerObject::Serialize(CObjectArchive& ar)
{
	...
	if (ar.bLoading)
	{
		...
		if (GetCompiler())
		{
			i32 nRenderFlag = ERF_HAS_CASTSHADOWMAPS | ERF_CASTSHADOWMAPS;
			...
			i32 nViewDistRadio = kDefaultViewDist;
	...
	else
	{
		...
		if (m_pCompiler)
		{
			...
			if (GetCompiler()->IsValid() && !ar.bUndo)
			{
				...
				{
					u32 meshBufferSize = meshBuffer.size();
					std::vector<char> encodedStr(Base64::encodedsize_base64(meshBufferSize) + 1);
					i32 nReturnedSize = Base64::encode_base64(&encodedStr[0], &meshBuffer[0], meshBufferSize, true);
...
void DesignerObject::SetMaterial(IEditorMaterial* mtl)
{
	...
	if (pParentMat && pParentMat == GetMaterial())
	{
		...
		for (i32 i = 0, iSubMatCount(pParentMat->GetSubMaterialCount()); i < iSubMatCount; ++i)
		{
...
void DesignerObject::SetMaterialLayersMask(u32 nLayersMask)
{
	CBaseObject::SetMaterialLayersMask(nLayersMask);
	UpdateEngineNode();
}

void DesignerObject::SetMinSpec(u32 nSpec, bool bSetChildren)
{
...
void DesignerObject::InvalidateTM(i32 nWhyFlags)
{
...
{
	generatedFileName.Format("%%level%%/Brush/designer_%d.%s", m_nBrushUniqFileId, DRX_GEOMETRY_FILE_EXT);
}
...
void DesignerObjectFlags::Set()
{
	...
	i32 flags = m_pObj->GetCompiler()->GetRenderFlags();
	i32 statobjFlags = m_pObj->GetCompiler()->GetStaticObjFlags();
...
namespace
{
void ModifyFlag(i32& nFlags, i32 flag, i32 clearFlag, bool var)
{
	nFlags = (var) ? (nFlags | flag) : (nFlags & (~clearFlag));
}
void ModifyFlag(i32& nFlags, i32 flag, bool var)
{
...
{
	i32 nFlags = m_pObj->GetCompiler()->GetRenderFlags();
	i32 statobjFlags = m_pObj->GetCompiler()->GetStaticObjFlags();
