...
void DesignerBaseObject<T >::Validate()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Empty Designer Object: %s %s", GetName(),
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "This designer object consists of only edges not polygons: %s %s", GetName(),
		           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
	}
}
