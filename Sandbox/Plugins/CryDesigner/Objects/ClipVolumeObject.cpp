...
i32 ClipVolumeObject::s_nGlobalFileClipVolumeID = 0;
...
void ClipVolumeObject::EntityLinked(const string& name, DrxGUID targetEntityId)
{
	...
	if (m_pEntity)
	{
		if (IClipVolumeComponent* pClipVolumeProxy = static_cast<IClipVolumeComponent*>(m_pEntity->GetProxy(ENTITY_PROXY_CLIPVOLUME)))
		{
			if (IBSPTree3D* pBspTree = pClipVolumeProxy->GetBspTree())
			{
				...
				uk pTreeData = new u8[nBufferSize];
...
void ClipVolumeObject::InvalidateTM(i32 nWhyFlags)
{
