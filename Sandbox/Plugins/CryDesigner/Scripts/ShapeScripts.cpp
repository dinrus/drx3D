...
ElementsPtr AddPolygonsToDesignerObjects(const std::vector<PolygonPtr>& polygons)
{
	...
	for (i32 k = 0, iPolygonCount(polygons.size()); k < iPolygonCount; ++k)
...
ElementID PyDesignerAddSphere(pSPyWrappedProperty vPyCenter, float radius, i32 numSides)
{
...
ElementID PyDesignerAddCylinder(pSPyWrappedProperty vPyBottomCenter, float radius, float height, i32 numSides)
{
...
ElementID PyDesignerAddCone(pSPyWrappedProperty vPyBottomCenter, float radius, float height, i32 numSides)
{
	if (vPyBottomCenter->type != SPyWrappedProperty::eType_Vec3)
		throw std::logic_error("center is invalid data type.");
...
ElementID PyDesignerAddDisc(pSPyWrappedProperty vPyCenter, float radius, i32 numSides)
{
	if (vPyCenter->type != SPyWrappedProperty::eType_Vec3)
		throw std::logic_error("center is invalid data type.");
...
ElementID PyDesignerFinishPolygonAddition()
{
	...
	for (i32 i = 0, iVertexCountOutside(s_apc.verticesOutside.size()); i < iVertexCountOutside; ++i)
	{
	...
	for (i32 i = 0, iVertexSetCountInside(s_apc.insidevertexSet.size()); i < iVertexSetCountInside; ++i)
	{
		AddPolygonContext::VertexList& verticesInside(s_apc.insidevertexSet[i]);
		i32 nOffset = vList.size();
		for (i32 k = 0, iVertexCountInside(verticesInside.size()); k < iVertexCountInside; ++k)
		{
...
/*
   REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE( Designer::Script::PyDesignerAddBox,
                       ...
                       "i32 designer.add_box((float pos_x,float pos_y,float pos_z),(float width,float depth,float height))" );

   REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE( Designer::Script::PyDesignerAddSphere,
                       designer,
                       add_sphere,
                       "Adds a sphere in a designer object.",
                       "i32 designer.add_sphere((float center_x,float center_y,float center_z),float radius,i32 numSides)" );
   ...
   REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE( Designer::Script::PyDesignerAddCylinder,
                       ...
                       "i32 designer.add_cylinder((float bottomcenter_x,float bottomcenter_y,float bottomcenter_z),float radius,float height,i32 numSides)" );
   ...
   REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE( Designer::Script::PyDesignerAddCone,
                       ...
                       "i32 designer.add_cone((float bottomcenter_x,float bottomcenter_y,float bottomcenter_z),float radius,float height,i32 numSides)" );
   ...
   REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE( Designer::Script::PyDesignerAddRectangle,
                       ...
                       "i32 designer.add_rectangle((float min_x,float min_y,float min_z),(float max_x,float max_y,float max_z))" );
   ...
   REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE( Designer::Script::PyDesignerAddDisc,
                       ...
                       "i32 designer.add_disc((float center_x,float center_y,float center_z),float radius,float numSides)" );
   ...
   REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE( Designer::Script::PyDesignerAddStair,
                       ...
                       "i32 designer.add_stair((float boxmin_x,float boxmin_y,float boxmin_z),(float boxmax_x,float boxmax_y,float boxmax_z),float stepRise,bool bMirror,bool bRotationBy90Degree)" );
   ...
   REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE( Designer::Script::PyDesignerFinishPolygonAddition,
                       ...
                       "i32 designer.finish_polygon_addition()" );
