...
void OutputPolygonPythonCreationCode(Polygon* pPolygon)
{
	...
	for (i32 i = 0, iPolygonCount(outerPolygons.size()); i < iPolygonCount; ++i)
	{
	...
	{
		for (i32 i = 0, iPolygonCount(innerPolygons.size()); i < iPolygonCount; ++i)
		{
...
/*
   ...
   REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE( Designer::Script::PyDesignerSetEnv,
                       ...
                       "designer.set_env( str name, [bool || i32 || float || str || (float,float,float)] paramValue )" );
