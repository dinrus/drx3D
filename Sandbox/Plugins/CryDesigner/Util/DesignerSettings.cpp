...
{
public:
	UndoExclusiveMode(tukk undoDescription = NULL)
	{
	}

	i32         GetSize()        { return sizeof(*this); }
	tukk GetDescription() { return "Undo for Designer Exclusive Mode"; }
...
void DesignerExclusiveMode::EnableExclusiveMode(bool bEnable)
{
	...
	if (bEnable)
	{
		...
		if (pGroup->GetCount() == 0)
			return;
		for (i32 i = 0, iObjectCount(pGroup->GetCount()); i < iObjectCount; ++i)
		{
	...
	if (m_bExclusiveMode)
	{
		...
		for (i32 i = 0, iCount(allLightsUnderCamera.size()); i < iCount; ++i)
...
void DesignerExclusiveMode::SetObjectsFlagForExclusiveMode()
{
	...
	for (i32 i = 0, iObjectCount(objects.size()); i < iObjectCount; ++i)
	{
		m_ObjectHiddenFlagMap[objects[i]] = objects[i]->IsHidden();
		bool bSelected = false;
		for (i32 k = 0, iSelectionCount(pSelection->GetCount()); k < iSelectionCount; ++k)
		{
...
void DesignerExclusiveMode::SetTimeOfDayForExclusiveMode()
{
	char workingDirectory[_MAX_PATH] = { 0, };
	GetCurrentDirectory(_MAX_PATH, workingDirectory);
	string designerTimeOfDay = string(workingDirectory) + "\\Editor\\DrxDesigner_TimeOfDay.xml";
