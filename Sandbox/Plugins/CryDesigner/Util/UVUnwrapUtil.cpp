...
{
	i32 iVertexCount = polygon->GetVertexCount();
...
{
	i32 iUVIslandCount(uvIslands.size());
	i32 columnRowCount = static_cast<i32>(sqrt(iUVIslandCount));
	...
	for (i32 i = 0; i < iUVIslandCount; ++i)
	{
		...
		{
			for (i32 k = 0, iCount(pUVIsland->GetCount()); k < iCount; ++k)
		...
		i32 row = i / columnRowCount;
		i32 column = i % columnRowCount;
...
bool Project(const CCamera& camera, const Vec3& p, Vec3& outPos)
{
	...
	i32 pViewport[4] = { 0, 0, camera.GetViewSurfaceX(), camera.GetViewSurfaceZ() };
...
void MakeUVIslandsFromSelectedElements(std::vector<UVIslandPtr>& outUVIslands)
{
	...
	std::set<i32> usedPolygonIndices;

	i32 i = 0;
	i32 iPolygonCount = polygons.size();
