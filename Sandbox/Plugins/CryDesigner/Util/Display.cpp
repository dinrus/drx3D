...
void DisplayModel(DisplayContext& dc, Model* pModel, ExcludedEdgeManager* pExcludedEdgeMgr, ShelfID nShelf, i32k nLineThickness, const ColorB& lineColor)
{
...
void DisplaySubdividedMesh(DisplayContext& dc, Model* pModel)
{
	...
	for (i32 i = 0, iFaceCount(pSubdivisionResult->GetFaceCount()); i < iFaceCount; ++i)
	{
		const HE_Face& f = pSubdivisionResult->GetFace(i);
		std::vector<BrushVec3> posList;
		pSubdivisionResult->GetFacePositions(f, posList);
		for (i32 k = 0, iPosListCount(posList.size()); k < iPosListCount; ++k)
		{
...
{
	i32 oldThickness = dc.GetLineWidth();
	...
	for (i32 i = 0; i < iPolygonSize; ++i)
	{
		...
		for (i32 k = 0, iEdgeSize(pPolygon->GetEdgeCount()); k < iEdgeSize; ++k)
		{
			...
			{
				for (i32 i = 0, iVisibleSize(visibleParts.size()); i < iVisibleSize; ++i)
...
void DisplayPolygon(DisplayContext& dc, PolygonPtr polygon)
{
	...
	for (i32 i = 0, iEdgeCount(polygon->GetEdgeCount()); i < iEdgeCount; ++i)
	{
...
void DisplayVertexNormals(DisplayContext& dc, MainContext& mc)
{
	...
	for (i32 i = 0; i < md.m_nVertCount; ++i)
	{
...
void DisplayTriangulation(DisplayContext& dc, MainContext& mc)
{
	...
	for (i32 shelfID = eShelf_Base; shelfID < cShelfMax; ++shelfID)
	{
		...
		for (i32 i = 0, iPolygonCount(mc.pModel->GetPolygonCount()); i < iPolygonCount; ++i)
		{
			...
			for (i32 k = 0, iFaceCount(pTriangles->faceList.size()); k < iFaceCount; ++k)
			{
				Vec3 v[3] = {
