...
DesignerUndo::DesignerUndo(CBaseObject* pObj, const Model* pModel, tukk undoDescription) :
	...
	m_Tool(eDesigner_Invalid)
{
	...
	if (pModel)
	{
		if (undoDescription)
			...
			m_undoDescription = "DrxDesignerUndo";
...
CBaseObject* DesignerUndo::GetBaseObject(const DrxGUID& objGUID)
{
...
MainContext DesignerUndo::GetMainContext(const DrxGUID& objGUID)
{
...
Model* DesignerUndo::GetModel(const DrxGUID& objGUID)
{
	CBaseObject* pObj = GetBaseObject(objGUID);
	if (pObj == NULL)
		return NULL;
...
ModelCompiler* DesignerUndo::GetCompiler(const DrxGUID& objGUID)
{
	CBaseObject* pObj = GetBaseObject(objGUID);
	if (pObj == NULL)
		return NULL;
...
void DesignerUndo::RestoreEditTool(Model* pModel, DrxGUID objGUID, EDesignerTool tool)
{
...
UndoSelection::UndoSelection(ElementSet& selectionContext, CBaseObject* pObj, bool bRetoreToolAfterUndo, tukk undoDescription)
{
	...
	for (i32 i = 0, iSelectionCount(selectionContext.GetCount()); i < iSelectionCount; ++i)
	{
...
void UndoSelection::CopyElements(ElementSet& sourceElements, ElementSet& destElements)
{
	...
	for (i32 i = 0, iElementSize(destElements.GetCount()); i < iElementSize; ++i)
	{
...
void UndoSelection::ReplacePolygonsWithExistingPolygonsInModel(ElementSet& elements)
{
	...
	for (i32 i = 0, iCount(elements.GetCount()); i < iCount; ++i)
	{
	...
	for (i32 i = 0, iRemovedCount(removedElements.size()); i < iRemovedCount; ++i)
...
void UndoTextureMapping::RestoreTexInfo(const std::vector<ContextInfo>& contextList)
{
	...
	for (i32 i = 0, iContextCount(contextList.size()); i < iContextCount; ++i)
	{
