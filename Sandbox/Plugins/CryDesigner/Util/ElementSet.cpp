...
bool ElementSet::Add(ElementSet& elements)
{
	if (elements.IsEmpty())
		return false;
	for (i32 i = 0, iElementSize(elements.GetCount()); i < iElementSize; ++i)
	{
		bool bEquivalentExist = false;
		for (i32 k = 0, iSelectedElementSize(m_Elements.size()); k < iSelectedElementSize; ++k)
		{
...
bool ElementSet::PickFromModel(CBaseObject* pObject, Model* pModel, CViewport* viewport, CPoint point, i32 nFlag, bool bOnlyIncludeCube, BrushVec3* pOutPickedPos)
{
	...
	if (nFlag & ePF_Edge)
	{
		...
		if (!edges.empty())
		{
			...
			i32 nShortEdgeIndex = FindShortestEdge(nearestEdges);
...
{
	i32 nOldLineWidth = dc.GetLineWidth();
	...
	for (i32 i = 0, iElementCount(m_Elements.size()); i < iElementCount; ++i)
	{
	...
	for (i32 i = 0, iElementCount(m_Elements.size()); i < iElementCount; ++i)
	{
		...
		{
			for (i32 k = 0, iEdgeCount(m_Elements[i].m_pPolygon->GetEdgeCount()); k < iEdgeCount; ++k)
			{
...
{
	for (i32 i = 0, iCount(m_Elements.size()); i < iCount; ++i)
	{
...
BrushVec3 ElementSet::GetNormal(Model* pModel) const
{
	...
	for (i32 i = 0, iSize(m_Elements.size()); i < iSize; ++i)
	{
	...
	for (i32 i = 0, iQuerySize(queryResult.size()); i < iQuerySize; ++i)
	{
		for (i32 k = 0, iMarkListSize(queryResult[i].m_MarkList.size()); k < iMarkListSize; ++k)
		{
...
void ElementSet::Erase(i32 nElementFlags)
{
...
bool ElementSet::Has(const Element& elementInfo) const
{
	...
	for (i32 i = 0, iElementSize(m_Elements.size()); i < iElementSize; ++i)
	{
...
ModelDB::QueryResult ElementSet::QueryFromElements(Model* pModel) const
{
	...
	for (i32 i = 0, iSelectedSize(m_Elements.size()); i < iSelectedSize; ++i)
	{
		i32 iVertexSize(m_Elements[i].m_Vertices.size());
		for (i32 k = 0; k < iVertexSize; ++k)
  ...
  BrushVec3* pOutNormal) const
{
	...
	for (i32 a = 0, iPolygonCount(pModel->GetPolygonCount()); a < iPolygonCount; ++a)
	{
		PolygonPtr pPolygon = pModel->GetPolygon(a);
		if (pPolygon->CheckFlags(ePolyFlag_Hidden))
			continue;
		for (i32 i = 0, iVertexCount(pPolygon->GetVertexCount()); i < iVertexCount; ++i)
		{
...
PolygonPtr ElementSet::PickPolygonFromRepresentativeBox(CBaseObject* pObject, Model* pModel, CViewport* pView, CPoint point, BrushVec3& outPickedPos) const
{
	...
	for (i32 i = eShelf_Base; i < cShelfMax; ++i)
	{
		...
		for (i32 k = 0, iPolygonCount(pModel->GetPolygonCount()); k < iPolygonCount; ++k)
		{
...
i32 ElementSet::GetPolygonCount() const
{
	i32 nFaceCount = 0;
	for (i32 i = 0, iElementCount(m_Elements.size()); i < iElementCount; ++i)
	{
...
i32 ElementSet::GetEdgeCount() const
{
	i32 nEdgeCount = 0;
	for (i32 i = 0, iElementCount(m_Elements.size()); i < iElementCount; ++i)
	{
...
i32 ElementSet::GetVertexCount() const
{
	i32 nVertexCount = 0;
	for (i32 i = 0, iElementCount(m_Elements.size()); i < iElementCount; ++i)
	{
...
void ElementSet::ApplyOffset(const BrushVec3& vOffset)
{
	for (i32 i = 0, iElementCount(m_Elements.size()); i < iElementCount; ++i)
	{
		for (i32 k = 0, iVertexCount(m_Elements[i].m_Vertices.size()); k < iVertexCount; ++k)
...
{
	for (i32 i = 0, iCount(GetCount()); i < iCount; ++i)
	{
		Element& element = m_Elements[i];
		for (i32 k = 0, iVertexCount(element.m_Vertices.size()); k < iVertexCount; ++k)
