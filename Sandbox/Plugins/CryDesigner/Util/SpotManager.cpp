...
void SpotManager::DrawPolyline(DisplayContext& dc) const
{
	...
	for (i32 i = 0, iSpotSize(m_SpotList.size() - 1); i < iSpotSize; ++i)
	{
...
bool SpotManager::AddPolygonToDesignerFromSpotList(Model* pModel, const SpotList& spotList)
{
	...
	else if (firstSpot.IsAtEndPoint() && lastSpot.IsAtEndPoint())
	{
		...
		if (pPolygon0)
		{
			for (i32 i = 0, nSpotSize(copiedSpotList.size() - 1); i < nSpotSize; ++i)
			{
	...
	else if (firstSpot.IsAtEndPoint() || lastSpot.IsAtEndPoint())
	{
		if (!firstSpot.IsAtEndPoint())
		{
			...
			i32 iSize(copiedSpotList.size());
			for (i32 i = 0, iHalfSize(iSize / 2); i < iHalfSize; ++i)
...
{
	i32 iSpotSize(spotList.size());
	outVList.reserve(iSpotSize);
	for (i32 i = 0; i < iSpotSize; ++i)
...
void SpotManager::RegisterSpotList(Model* pModel, const SpotList& spotList)
{
	...
	{
		for (i32 i = 0, spotListCount(splittedSpotList.size()); i < spotListCount; ++i)
...
void SpotManager::SplitSpotList(Model* pModel, const SpotList& spotList, std::vector<SpotList>& outSpotLists)
{
	...
	for (i32 k = 0, iSpotListCount(spotList.size()); k < iSpotListCount - 1; ++k)
	{
		...
		for (i32 i = 0, iSpotPairCount(splittedSpotPairs.size()); i < iSpotPairCount; ++i)
		{
...
void SpotManager::SplitSpot(Model* pModel, const SpotPair& spotPair, SpotPairList& outSpotPairs)
{
	...
	for (i32 i = 0, iPolygonCount(pModel->GetPolygonCount()); i < iPolygonCount; ++i)
	{
		...
		i32 iSubtractedEdgeCount(subtractedEdges.size());

		for (i32 k = 0; k < iSubtractedEdgeCount; ++k)
		{
...
bool SpotManager::FindSpotNearAxisAlignedLine(IDisplayViewport* pViewport, PolygonPtr pPolygon, const BrushMatrix34& worldTM, Spot& outSpot)
{
	...
	for (i32 i = 0, iLineCount(axisAlignedLines.size()); i < iLineCount; ++i)
	{
		BrushVec2 vHitPos;
  ...
  bool bSearchAllShelves)
{
	...
	if (!bSuccessQuery || bKeepInitialPlane && !pPickedPolygon)
	{
		...
		for (i32 i = 0, iCount(penetratedOpenPolygons.size()); i < iCount; ++i)
		{
			...
			for (i32 k = 0, iVCount(vList.size()); k < iVCount; ++k)
			{
				if (k == 0)
	...
	for (i32 i = 0, iQueryResultsCount(queryResults.size()); i < iQueryResultsCount; ++i)
	...
	i32 nShortestIndex = FindShortestEdge(queryEdges);
...
void SpotManager::AddSpotToSpotList(const Spot& spot)
{
	...
	for (i32 i = 0, iSpotSize(GetSpotListCount()); i < iSpotSize; ++i)
	{
...
bool SpotManager::FindBestPlane(Model* pModel, const Spot& s0, const Spot& s1, BrushPlane& outPlane)
{
	...
	for (i32 i = 0, iCandidatePlaneCount(sizeof(candidatePlanes) / sizeof(*candidatePlanes)); i < iCandidatePlaneCount; ++i)
	{
...
{
	for (i32 i = 0, iPolygonSize(pModel->GetPolygonCount()); i < iPolygonSize; ++i)
	{
		PolygonPtr pPolygon = pModel->GetPolygon(i);
		if (pPolygon == NULL)
			continue;
