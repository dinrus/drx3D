	...
	m_fScale(fScale)
{
	...
	{
		for (i32 i = 0, iPerpendicularSize(perpendicularPolygons->size()); i < iPerpendicularSize; ++i)
		{
...
{
	for (i32 i = 0, iSize(m_EdgePlanePairs.size()); i < iSize; ++i)
	{
...
void ArgumentModel::UpdateWithScale(EBrushArgumentUpdate updateOp)
{
	...
	for (i32 i = 0, iEdgeSize(m_pInitialPolygon->GetEdgeCount()); i < iEdgeSize; ++i)
	{
...
void ArgumentModel::AddSidePolygons()
{
	...
	for (i32 a = 0, iPolygonCount(polygons.size()); a < iPolygonCount; ++a)
	{
		...
		for (i32 i = 0, iEdgeSize(pPolygon->GetEdgeCount()); i < iEdgeSize; ++i)
		{
...
void ArgumentModel::MoveVertices2Plane(const BrushPlane& targetPlane, const BrushVec3& dir)
{
	...
	{
		for (i32 i = 0, iVertexSize(m_pPolygon->GetVertexCount()); i < iVertexSize; ++i)
		{
...
{
	i32 iPolygonSize(m_pModel->GetPolygonCount());

	for (i32 i = 1; i < iPolygonSize; ++i)
