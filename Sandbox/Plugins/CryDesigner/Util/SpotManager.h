...
protected:
	...
	void             SetSpotProcessed(i32 nPos, bool bProcessed)     { m_SpotList[nPos].m_bProcessed = bProcessed; }
	void             SetPosState(i32 nPos, ESpotPositionState state) { m_SpotList[nPos].m_PosState = state; }

	void             SwapCurrentAndStartSpots()                      { std::swap(m_CurrentSpot, m_StartSpot); }

	i32              GetSpotListCount() const                        { return m_SpotList.size(); }
	const BrushVec3& GetSpotPos(i32 nPos) const                      { return m_SpotList[nPos].m_Pos; }
	void             AddSpotToSpotList(const Spot& spot);
	void             ReplaceSpotList(const SpotList& spotList);

	void             ClearSpotList()           { m_SpotList.clear(); }

	const Spot&     GetSpot(i32 nIndex) const { return m_SpotList[nIndex]; }
