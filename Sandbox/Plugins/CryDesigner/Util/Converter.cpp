...
bool Converter::CreateNewDesignerObject()
{
	...
	i32 iSizeOfDesignerObjects(pDesignerObjects.size());
	...
	for (i32 i = 0; i < iSizeOfDesignerObjects; ++i)
	{
		AABB aabb;
...
bool Converter::ConvertToDesignerObject()
{
	...
	for (i32 i = 0, iSizeOfSelectedObjects(pSelectedMeshes.size()); i < iSizeOfSelectedObjects; ++i)
...
{
	for (i32 i = 0, iSizeOfSelectedObjs(pSelectedMeshes.size()); i < iSizeOfSelectedObjs; ++i)
	{
...
void Converter::GetSelectedObjects(std::vector<SelectedMesh>& pObjects) const
{
	...
	for (i32 i = 0, iCount(pSelection->GetCount()); i < iCount; ++i)
	{
		...
		else if (pObj->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			...
			if (pEntity)
			{
				i32 nSlotCount = pEntity->GetSlotCount();
				i32 nCurSlot = 0;
...
bool Converter::ConvertMeshToBrushDesigner(IIndexedMesh* pMesh, Model* pModel)
{
	...
	i32 numVerts = pMesh->GetVertexCount();
	i32 numFaces = pMesh->GetIndexCount() / 3;
	...
	i32 nSubsetCount = pMesh->GetMesh()->m_subsets.size();
	for (i32 i = 0; i < nSubsetCount; ++i)
	{
		const SMeshSubset& subset = pMesh->GetMesh()->m_subsets[i];
		for (i32 k = 0; k < subset.nNumIndices; k += 3)
		{
			i32 i0 = indices[k + 0 + subset.nFirstIndexId];
			i32 i1 = indices[k + 1 + subset.nFirstIndexId];
			i32 i2 = indices[k + 2 + subset.nFirstIndexId];
...
bool Converter::ConvertSolidXMLToDesignerObject(XmlNodeRef pSolidNode, DesignerObject* pDesignerObject)
{
	...
	tukk tag = pSolidNode->getChild(0)->getTag();

	if (!stricmp(tag, "Face"))
	{
		i32 numFaces = pSolidNode->getChildCount();
	...
	else if (!stricmp(tag, "Polygon"))
	{
		...
		i32 numberOfChildren = pSolidNode->getChildCount();
...
{
	for (i32 i = 0, iPolySize(polygonList.size()); i < iPolySize; ++i)
	{
		const SolidPolygon& solidPolygon = polygonList[i];
		std::vector<BrushVec3> reorderedVertexList;
		for (i32 k = 0, iVIndexCount(solidPolygon.vIndexList.size()); k < iVIndexCount; ++k)
...
bool IsAdjacent(std::vector<PolygonPtr>& polygons, PolygonPtr polygon)
{
	...
	for (; ii != polygons.end(); ++ii)
	{
		if ((*ii)->GetSubMatID() != polygon->GetSubMatID())
			continue;
		for (i32 i = 0, iEdgeCount0(polygon->GetEdgeCount()); i < iEdgeCount0; ++i)
		{
			...
			for (i32 k = 0, iEdgeCount1((*ii)->GetEdgeCount()); k < iEdgeCount1; ++k)
			{
...
void Converter::GenerateUVIslands(Model* pModel)
{
	...
	{
		bool bAdded = false;
		for (i32 i = 0, iCount(pModel->GetPolygonCount()); i < iCount; ++i)
		{
		...
		if (!bAdded && !groupPolygons.empty())
		{
			...
			for (i32 i = 0, iCount(groupPolygons.size()); i < iCount; ++i)
