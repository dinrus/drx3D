...
class DesignerEditor_ClassDesc : public IClassDesc
{
	...
	virtual tukk    ClassName()       { return "EditTool.DesignerEditor"; };
	virtual tukk    Category()        { return "Brush"; };
...
void DesignerEditor::StartCreation(tukk szObjectType)
{
...
void DesignerEditor::Create(tukk szObjectType)
{
...
void DesignerEditor::BeginEdit()
{
	...
	else
	{
		...
		// at this point we are guaranteed that our selection is only composed of designer type objects
		for (i32 i = 0, totSelected = pSel->GetCount(); i < totSelected; ++i)
		{
...
void DesignerEditor::Display(DisplayContext& dc)
{
	...
	i32 vp_x = 0, vp_y = 0, vp_w = dc.GetWidth(), vp_h = dc.GetHeight();
...
bool DesignerEditor::OnKeyDown(CViewport* pView, u32 nChar, u32 nRepCnt, u32 nFlags)
{
	if (m_Tool == eDesigner_Invalid)
		return false;
  ...
  bool bAllowMultipleMode)
{
	...
	i32 nCombinationDesignerMode = m_Tool;
...
void DesignerEditor::OnManipulatorBegin(IDisplayViewport* view, ITransformManipulator* pManipulator, const Vec2i& point, i32 flags)
{
	if (pManipulator != m_pManipulator)
		return;
...
void DesignerEditor::UpdateTMManipulatorBasedOnElements(ElementSet* elements)
{
	...
	i32 iResultSize(0);
	for (i32 i = 0, iListSize(elements->GetCount()); i < iListSize; ++i)
	{
		...
		{
			for (i32 k = 0, iElementSize((*elements)[i].m_Vertices.size()); k < iElementSize; ++k)
			{
...
class CreateDesignerObjectTool_ClassDesc : public IClassDesc
{
	...
	tukk    ClassName()       { return "EditTool.CreateDesignerObjectTool"; };
