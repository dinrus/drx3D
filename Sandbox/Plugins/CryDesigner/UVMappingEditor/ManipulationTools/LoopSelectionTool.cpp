...
bool LoopSelectionTool::BorderSelection(UVElementSetPtr pUVElements)
{
	...
	i32 initialElementCount = pUVElements->GetCount();

	std::vector<UVElement> elements;
	for (i32 i = 0, iCount(pUVElements->GetCount()); i < iCount; ++i)
	{
...
std::vector<UVElement> LoopSelectionTool::BorderSelection(const std::vector<UVElement>& input)
{
	...
	for (i32 i = 0, iCount(input.size()); i < iCount; ++i)
	{
		...
		i32 prev = FindBestBorderUVEdge(uvElement.m_UVVertices[0].GetUV(), uvElement.m_UVVertices[1].GetUV(), pUVIslandPtr, prevConnectedUVEdges);
		if (prev != -1)
		{
		...
		i32 next = FindBestBorderUVEdge(uvElement.m_UVVertices[0].GetUV(), uvElement.m_UVVertices[1].GetUV(), pUVIslandPtr, nextConnectedUVEdges);
		if (next != -1)
		{
...
i32 LoopSelectionTool::FindBestBorderUVEdge(const Vec2& uv0, const Vec2& uv1, UVIslandPtr pUVIslandPtr, const std::vector<UVEdge>& candidateUVEdges)
{
	const float kThresholdAngle = 80.0f;

	Vec3 vUVEdgeDir = (uv1 - uv0).GetNormalized();
	for (i32 i = 0, iEdgeCount(candidateUVEdges.size()); i < iEdgeCount; ++i)
	{
...
std::vector<UVElement> LoopSelectionTool::LoopSelection(const std::vector<UVElement>& input)
{
	...
	for (i32 i = 0, iCount(input.size()); i < iCount; ++i)
	{
		...
		i32 prev = FindBestLoopUVEdge(uvElement.m_UVVertices[0].GetUV(), uvElement.m_UVVertices[1].GetUV(), pUVIslandPtr, prevConnectedUVEdges);
		if (prev != -1)
		{
			UVElement element(prevConnectedUVEdges[prev]);
			if (pUVElements->AddElement(element))
		...
		i32 next = FindBestLoopUVEdge(uvElement.m_UVVertices[0].GetUV(), uvElement.m_UVVertices[1].GetUV(), pUVIslandPtr, nextConnectedUVEdges);
		if (next != -1)
		{
			UVElement element(nextConnectedUVEdges[next]);
			if (pUVElements->AddElement(element))
...
i32 LoopSelectionTool::FindBestLoopUVEdge(const Vec2& uv0, const Vec2& uv1, UVIslandPtr pUVIslandPtr, const std::vector<UVEdge>& candidateUVEdges)
{
