...
void AddUVPolygon(std::vector<UVPolygon>& uvPolygons, PolygonPtr polygon, UVIslandPtr pUVIsland)
{
	...
	for (i32 i = 0, iVertexCount(polygon->GetVertexCount()); i < iVertexCount; ++i)
		uvs.push_back(UVVertex(pUVIsland, polygon, i));
	uvPolygons.push_back(UVPolygon(pUVIsland, uvs));
}

Vec3 SearchUtil::FindHitPointToUVPlane(QViewport* viewport, i32 x, i32 y)
{
	Ray ray;
...
bool SearchUtil::FindPolygons(const Ray& ray, UVIslandPtr pUVIsland, std::vector<UVPolygon>& outFoundPolygons)
{
	...
	bool bFound = false;
	for (i32 i = 0, iCount(pUVIsland->GetCount()); i < iCount; ++i)
	{
...
bool SearchUtil::FindPolygonsSharingEdge(const Vec2& uv0, const Vec2& uv1, UVIslandPtr pUVIsland, std::set<PolygonPtr>& outFoundPolygons)
{
	...
	for (i32 i = 0, iUVIslandCount(pUVIslandMgr->GetCount()); i < iUVIslandCount; ++i)
	{
		...
		for (i32 k = 0, iPolygonCount(pUVIsland->GetCount()); k < iPolygonCount; ++k)
		{
			...
			for (i32 a = 0, iEdgeCount(polygon->GetEdgeCount()); a < iEdgeCount; ++a)
			{
...
bool SearchUtil::FindUVVerticesFromPos(const BrushVec3& pos, i32 subMatId, std::vector<UVVertex>& outUVVertices)
{
	UVIslandManager* pUVIslandMgr = GetUVEditor()->GetUVIslandMgr();
	bool bFound = false;
...
bool SearchUtil::FindUVEdgesFromEdge(const BrushEdge3D& edge, i32 subMatId, std::vector<UVEdge>& outUVEdges)
{
	UVIslandManager* pUVIslandMgr = GetUVEditor()->GetUVIslandMgr();
	bool bFound = false;
	...
	for (i32 i = 0, iUVIslandCount(pUVIslandMgr->GetCount()); i < iUVIslandCount; ++i)
	{
		...
		for (i32 k = 0, iPolygonCount(pUVIsland->GetCount()); k < iPolygonCount; ++k)
		{
			...
			if (!GetUVEditor()->AllIslandsViewed() && polygon->GetSubMatID() != subMatId)
				continue;
			i32 edgeIndex = polygon->GetEdgeIndex(edge);
			if (edgeIndex == -1)
...
bool SearchUtil::FindUVPolygonsFromPolygon(PolygonPtr inPolygon, i32 subMatId, std::vector<UVPolygon>& outUVPolygons)
{
	UVIslandManager* pUVIslandMgr = GetUVEditor()->GetUVIslandMgr();
	bool bFound = false;
	...
	for (i32 i = 0, iUVIslandCount(pUVIslandMgr->GetCount()); i < iUVIslandCount; ++i)
	{
		...
		for (i32 k = 0, iPolygonCount(pUVIsland->GetCount()); k < iPolygonCount; ++k)
		{
			...
			if (polygon == inPolygon)
			{
				...
				for (i32 a = 0, iVertexCount(polygon->GetVertexCount()); a < iVertexCount; ++a)
  ...
  std::vector<UVEdge>& outNextConnectedUVEdges)
{
	...
	for (i32 i = 0, iPolygonCount(pUVIsland->GetCount()); i < iPolygonCount; ++i)
	{
...
PolygonPtr SearchUtil::FindPolygonContainingEdge(const Vec2& uv0, const Vec2& uv1, UVIslandPtr pUVIsland)
{
	for (i32 i = 0, iPolygonCount(pUVIsland->GetCount()); i < iPolygonCount; ++i)
	{
		...
		for (i32 k = 0, iEdgeCount(polygon->GetVertexCount()); k < iEdgeCount; ++k)
		{
			const IndexPair& edgeIndexPair = polygon->GetEdgeIndexPair(k);
			SBrushEdge<float> uvEdge(polygon->GetUV(edgeIndexPair.m_i[0]), polygon->GetUV(edgeIndexPair.m_i[1]));
...
void SyncSelection()
{
	...
	for (i32 i = 0, iCount(pUVElementSet->GetCount()); i < iCount; ++i)
	{
		...
		{
			for (i32 k = 0, iPolygonCount(element.m_pUVIsland->GetCount()); k < iPolygonCount; ++k)
