...
MainContext GetMainContext(DrxGUID objGuid)
{
	MainContext mc;
...
void UVIslandUndo::Record(DrxGUID& objGUID, UndoData& undoData)
{
...
void UVSelectionUndo::Apply(UVElementSetPtr pElementSet)
{
	...
	i32 nSubMatID = GetUVEditor()->GetSubMatID();
...
void UVSelectionUndo::RefreshUVIslands(UVElementSetPtr pElementSet)
{
	...
	for (i32 i = 0, iCount(pElementSet->GetCount()); i < iCount; ++i)
	{
		...
		if (pUVIsland)
		{
			...
			for (i32 k = 0, iVertexCount(element.m_UVVertices.size()); k < iVertexCount; ++k)
...
void UVMoveUndo::ApplyInvolvedPolygons(std::vector<PolygonPtr>& involvedPolygons)
{
	...
	i32 nSubMatID = GetUVEditor() ? GetUVEditor()->GetSubMatID() : 0;
...
void UVProjectionUndo::SavePolygons(std::vector<PolygonPtr>& polygons)
{
	...
	for (i32 i = 0, iCount(pSelectedElement->GetCount()); i < iCount; ++i)
	{
