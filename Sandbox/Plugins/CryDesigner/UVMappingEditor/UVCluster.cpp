...
void UVCluster::Update()
{
	...
	for (i32 k = 0, iElementCount(pUVElementSet->GetCount()); k < iElementCount; ++k)
	{
		...
		{
			for (i32 i = 0, iCount(element.m_UVVertices.size()); i < iCount; ++i)
			{
	...
	{
		for (i32 i = 0, iCount((*uvIsland)->GetCount()); i < iCount; ++i)
		{
...
void UVCluster::Flip(const Vec2& pivot, const Vec2& normal)
{
	...
	for (; iIsland != m_uvIslandCluster.end(); ++iIsland)
	{
		for (i32 i = 0, iPolygonCount((*iIsland)->GetCount()); i < iPolygonCount; ++i)
		{
...
void UVCluster::GetPolygons(std::set<PolygonPtr>& outPolygons)
{
	...
	{
		for (i32 i = 0, iCount((*iterUVIsland)->GetCount()); i < iCount; ++i)
...
void UVCluster::Shelve()
{
	...
	{
		for (i32 i = 0, iPolygonCount((*iUVIslandCluster)->GetCount()); i < iPolygonCount; ++i)
...
void UVCluster::FindSmoothingGroupAffected(std::set<SmoothingGroupPtr>& outAffectedSmoothingGroups)
{
	...
	{
		for (i32 i = 0, iPolygonCount((*iUVIsland)->GetCount()); i < iPolygonCount; ++i)
		{
