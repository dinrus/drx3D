...
void CubeUnwrappingTool::Enter()
{
	...
	constexpr i32 numberOfAxises = DRX_ARRAY_COUNT(axises);
	...
	for (i32 k = 0, iCount(pSelected->GetCount()); k < iCount; ++k)
	{
		...
		for (i32 i = 0; i < numberOfAxises; ++i)
		{
	...
	for (i32 i = 0; i < numberOfAxises; ++i)
	{
		...
		for (i32 k = 0, iCount(polygonSets[i].size()); k < iCount; ++k)
