...
{
	UVVertex() : vIndex(-1) {}
	UVVertex(UVIslandPtr _pUVIsland, PolygonPtr _pPolygon, i32 _vIndex) : pUVIsland(_pUVIsland), pPolygon(_pPolygon), vIndex(_vIndex) {}
...
{
public:
	static Vec3 FindHitPointToUVPlane(QViewport* viewport, i32 x, i32 y);
	...
	static bool FindUVVerticesFromPos(const BrushVec3& pos, i32 subMatId, std::vector<UVVertex>& outUVVertices);
	static bool FindUVEdgesFromEdge(const BrushEdge3D& edge, i32 subMatId, std::vector<UVEdge>& outUVEdges);
	static bool FindUVPolygonsFromPolygon(PolygonPtr inPolygon, i32 subMatId, std::vector<UVPolygon>& outUVPolygons);
...
{
public:
	UVMappingToolRegister(EUVMappingTool tool, EUVMappingToolGroup group, tukk name, tukk classname)
	{
...
static bool IsUVCCW(PolygonPtr polygon)
{
	...
	i32 previdx = ((idxWithMinimumX - 1) + vertices.size()) % vertices.size();
	i32 nextidx = (idxWithMinimumX + 1) % vertices.size();
...
typedef Designer::DesignerFactory<Designer::UVMapping::EUVMappingTool, Designer::UVMapping::BaseTool, i32> UVMappingToolFactory;
