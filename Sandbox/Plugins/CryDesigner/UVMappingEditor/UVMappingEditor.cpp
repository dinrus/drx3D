...
void UVMappingEditor::OrganizeToolbar(QBoxLayout* pTopLayout)
{
	...
	i32 comboBoxPos = UVMappingToolGroupMapper::the().GetToolList(eUVMappingToolGroup_Unwrapping).size();
	pLeftLayout->addWidget(m_pPlaneAxisComboBox, comboBoxPos / 3, comboBoxPos % 3, 1, 1);
	for (i32 i = 0, iCount(Serialization::getEnumDescription<EPlaneAxis>().count()); i < iCount; ++i)
	...
	m_pPivotComboBox->AddItem("Selection", QVariant((i32)ePivotType_Selection));
	m_pPivotComboBox->AddItem("Cursor", QVariant((i32)ePivotType_Cursor));
	...
	m_pRotationPushBox->setIcon(DrxIcon("icons:Designer/Designer_UV_Rotate_Camera.ico"));
...
void UVMappingEditor::RegisterMenuButtons(EUVMappingToolGroup what, QGridLayout* where, i32 columnNumber)
{
	...
	for (; iTool != tools.end(); ++iTool)
	{
		...
		tukk szName = GetUVMapptingToolDesc().name(tool);
...
void UVMappingEditor::RenderUnwrappedMesh(DisplayContext& dc)
{
	...
	for (i32 i = 0, iIslandCount(pUVIslandMgr->GetCount()); i < iIslandCount; ++i)
	{
		...
		for (i32 k = 0, iPolygonCount(pUVIsland->GetCount()); k < iPolygonCount; ++k)
		{
...
i32 UVMappingEditor::GetSubMatID() const
{
	i32 subMatID = m_pSubMatComboBox->GetSubMatID();
	if (subMatID == -1)
		return 0;
	return subMatID;
}

void UVMappingEditor::SetSubMatID(i32 nSubMatID)
{
...
{
	dc.SetColor(color);
	for (i32 i = 0, iCount(pUVElementSet->GetCount()); i < iCount; ++i)
	{
		...
		if (element.IsPolygon())
		{
			...
			for (i32 k = 0, iEdgeCount(polygon->GetEdgeCount()); k < iEdgeCount; ++k)
			{
		...
		{
			i32 iVertexCount = element.m_UVVertices.size();
			for (i32 k = 0; k < iVertexCount - 1; ++k)
			{
...
{
	i32k currentIndex = m_pPivotComboBox->GetCheckedItem();
...
void UVMappingEditor::ApplyPostProcess(i32 updateType)
{
	if (GetDesigner() == NULL)
		return;
...
void UVMappingEditor::OnGizmoLMBDown(i32 mode)
{
	if (GetCurrentTool())
		GetCurrentTool()->OnGizmoLMBDown(mode);
}

void UVMappingEditor::OnGizmoLMBUp(i32 mode)
{
	if (GetCurrentTool())
		GetCurrentTool()->OnGizmoLMBUp(mode);
}

void UVMappingEditor::OnTransformGizmo(i32 mode, const Vec3& offset)
{
	if (GetCurrentTool())
...
void UVMappingEditor::OnSelectAll()
{
	...
	if (pElementSet->IsEmpty())
	{
		...
		for (i32 i = 0, iCount(pUVIslandMgr->GetCount()); i < iCount; ++i)
		{
			UVIslandPtr pUVIsland = pUVIslandMgr->GetUVIsland(i);
...
void UVMappingEditor::UpdateSubMatComboBox()
{
	...
	if (GetUVIslandMgr())
	{
		for (i32 i = 0, iCount(m_pSubMatComboBox->GetItemCount()); i < iCount; ++i)
		{
...
void UVMappingEditor::UpdateSharedSelection()
{
	...
	for (i32 i = 0, iCount(m_ElementSet->GetCount()); i < iCount; ++i)
...
void UVMappingEditor::OnSubMatSelectionChanged(i32 nSelectedItem)
{
...
void UVMappingEditor::OnDesignerNotifyHandler(EDesignerNotify notification, uk param)
{
	switch (notification)
	{
...
void UVMappingEditor::RefreshPolygonsInUVIslands()
{
	...
	for (i32 i = 0, iUVIslandCount(pUVIslandMgr->GetCount()); i < iUVIslandCount; ++i)
	{
		UVIslandPtr pUVIsland = pUVIslandMgr->GetUVIsland(i);
