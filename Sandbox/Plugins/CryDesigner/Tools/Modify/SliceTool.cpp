...
void SliceTool::DrawOutlines(DisplayContext& dc)
{
	...
	for (i32 i = 0, iSize(m_RestTraverseLineSet.size()); i < iSize; ++i)
...
{
	for (i32 i = 0, iSize(lineList.size()); i < iSize; ++i)
...
void SliceTool::GenerateLoop(const BrushPlane& SlicePlane, TraverseLineList& outLineList) const
{
	...
	for (i32 i = 0, iPolygonSize(GetModel()->GetPolygonCount()); i < iPolygonSize; ++i)
	{
		...
		for (i32 i = 0, iEdgeSize(sortedIntersectedEdges.size()); i < iEdgeSize; ++i)
...
BrushVec3 SliceTool::GetLoopPivotPoint() const
{
	AABB aabb;
	aabb.Reset();
	for (i32 i = 0, iSize(m_MainTraverseLines.size()); i < iSize; ++i)
	{
...
void SliceTool::Divide()
{
	...
	for (i32 i = 0, iSize(traverseLineListSet.size()); i < iSize; ++i)
	{
		const TraverseLineList& lineList = traverseLineListSet[i];
		for (i32 k = 0, iLineListSize(lineList.size()); k < iLineListSize; ++k)
		{
...
void SliceTool::OnManipulatorDrag(IDisplayViewport* pView, ITransformManipulator* pManipulator, CPoint& p0, BrushVec3 value, i32  nFlags)
{
...
void SliceTool::OnManipulatorBegin(IDisplayViewport* pView, ITransformManipulator* pManipulator, CPoint& point, i32 flags)
{
