...
void MirrorTool::ApplyMirror()
{
	...
	*GetModel() = *backPart;
	...
	for (i32 i = 0, iPolygonSize(backPart->GetPolygonCount()); i < iPolygonSize; ++i)
...
void MirrorTool::RemoveEdgesOnMirrorPlane(Model* pModel)
{
	...
	for (i32 i = 0, iBorderEdgeCount(borderEdges.size()); i < iBorderEdgeCount; ++i)
	{
...
void MirrorTool::OnManipulatorBegin(IDisplayViewport* pView, ITransformManipulator* pManipulator, CPoint& point, i32 flags)
{
	if (GetModel()->CheckFlag(eModelFlag_Mirror))
	{
