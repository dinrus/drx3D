...
namespace Designer
{
i32 GetDifferentVerticesCount(const std::vector<BrushVec3>& vList)
{
	std::set<BrushVec3, Comparison::less_BrushVec3> vSet;
	for (i32 i = 0, iSize(vList.size()); i < iSize; ++i)
		vSet.insert(vList[i]);
	return vSet.size();
}
...
bool BevelTool::PP0_Initialize(bool bSpreadEdge)
{
	...
	for (i32 i = 0, iElementSize(pSelected->GetCount()); i < iElementSize; ++i)
	{
		if ((*pSelected)[i].IsEdge())
		{
			...
			for (i32 k = 0, iVertexSize((*pSelected)[i].m_Vertices.size()); k < iVertexSize; ++k)
				GetModel()->GetDB()->QueryAsVertex((*pSelected)[i].m_Vertices[k], queryResult);
		}
		else if ((*pSelected)[i].IsPolygon())
		{
			for (i32 k = 0, iEdgeSize((*pSelected)[i].m_pPolygon->GetEdgeCount()); k < iEdgeSize; ++k)
			{
	...
	for (i32 i = 0, iRemovedPolygonSize(removedPolygons.size()); i < iRemovedPolygonSize; ++i)
	{
...
bool BevelTool::OnLButtonDown(CViewport* view, UINT nFlags, CPoint point)
{
	...
	if (m_BevelMode == eBevelMode_Spread)
	{
		...
		for (i32 i = 0, iPolygonSize(m_ResultForSecondPhase.middlePhaseEdgePolygons.size()); i < iPolygonSize; ++i)
			GetModel()->RemovePolygon(m_ResultForSecondPhase.middlePhaseEdgePolygons[i]);

		for (i32 i = 0, iPolygonSize(m_ResultForSecondPhase.middlePhaseSidePolygons.size()); i < iPolygonSize; ++i)
			GetModel()->RemovePolygon(m_ResultForSecondPhase.middlePhaseSidePolygons[i]);

		for (i32 i = 0, iPolygonSize(m_ResultForSecondPhase.middlePhaseBottomPolygons.size()); i < iPolygonSize; ++i)
...
{
	i32 nEdgeCount0 = pPolygon0->GetVertexCount();
	i32 nEdgeCount1 = pPolygon1->GetVertexCount();

	if (nEdgeCount0 != nEdgeCount1)
		return false;

	for (i32 i = 0; i < nEdgeCount0; ++i)
	{
...
bool IsPolygonValid(PolygonPtr pPolygon, PolygonPtr pOriginalPolygon)
{
	if (pOriginalPolygon->HasHoles())
	{
		...
		for (i32 k = 0, iInnterPolygonCount(innerPolygons.size()); k < iInnterPolygonCount; ++k)
		{
...
bool BevelTool::PP1_PushEdgesAndVerticesOut(ResultForNextPhase& outResultForNextPhase, MappingInfo& outMappingInfo)
{
	...
	std::vector<i32> selectedElements;

	std::vector<PolygonPtr> updatedPolygons;

	for (i32 i = 0, iPolygonSize(m_OriginalPolygons.size()); i < iPolygonSize; ++i)
	{
		...
		std::set<i32> edgeSetMatchingSelectedElement;
		std::map<i32, i32> mapBetweenEdgeIndexAndElement;

		for (i32 k = 0, iElementSize(m_OriginalSelectedElements.GetCount()); k < iElementSize; ++k)
		{
	...
	for (i32 i = 0, iSize(outResultForNextPhase.middlePhaseSidePolygons.size()); i < iSize; ++i)
	{
		PolygonPtr pPolygon = outResultForNextPhase.middlePhaseSidePolygons[i];

		bool bTouched = false;
		for (i32 k = 0; k < iSize; ++k)
		{
	...
	for (i32 i = 0, iUpdatedPolygonCount(updatedPolygons.size()); i < iUpdatedPolygonCount; ++i)
...
{
	for (i32 a = 0, iEdgeSize(pEdgePolygon->GetEdgeCount()); a < iEdgeSize; ++a)
	{
		...
		for (i32 b = 0, iVertex2ApexCount(mappingInfo.mapSpreadedVertex2Apex.size()); b < iVertex2ApexCount; ++b)
		{
		...
		i32 nCount = GetEdgeCountHavingVertexInElementList(apexPos, m_OriginalSelectedElements);
		if (nCount < 3)
		{
...
void BevelTool::PP1_MakeApexPolygons(const MappingInfo& mappingInfo, ResultForNextPhase& outResultForNextPhase)
{
	...
	i32 iApexEdgeSize(apexEdges.size());
	...
	while (usedEdges.size() < iApexEdgeSize || usedEdges.size() == iApexEdgeSize && bFoundApexPolygon)
	{
		...
		for (i32 i = 0, iApexEdgeSize(apexEdges.size()); i < iApexEdgeSize; ++i)
		{
			if (i == nIndex)
				continue;
...
void BevelTool::PP0_SpreadEdges(i32 offset, bool bSpreadEdge)
{
	MODEL_SHELF_RECONSTRUCTOR(GetModel());
...
void BevelTool::PP0_SubdivideSpreadedEdge(i32 nSubdivideNum)
{
	MODEL_SHELF_RECONSTRUCTOR(GetModel());
	...
	if (nSubdivideNum == 1)
	{
		for (i32 i = 0, iSize(rp.middlePhaseEdgePolygons.size()); i < iSize; ++i)
			GetModel()->AddPolygon(rp.middlePhaseEdgePolygons[i]);

		for (i32 i = 0, iSidePolygonSize(rp.middlePhaseSidePolygons.size()); i < iSidePolygonSize; ++i)
		{
		...
		for (i32 i = 0, iFlatPolygonSize(rp.middlePhaseBottomPolygons.size()); i < iFlatPolygonSize; ++i)
	...
	for (i32 i = 0, iMiddleSidePolygonSize(rp.middlePhaseSidePolygons.size()); i < iMiddleSidePolygonSize; ++i)
	{
		if (GetModel()->QueryEquivalentPolygon(rp.middlePhaseSidePolygons[i]))
			continue;
	...
	for (i32 k = 0, iPolygonSize(rp.middlePhaseEdgePolygons.size()); k < iPolygonSize; ++k)
	{
		...
		for (i32 i = 0; i < 4; ++i)
		{
			...
			for (i32 a = 0, iSidePolygonSize(middleSidePolygons.size()); a < iSidePolygonSize; ++a)
			{
			...
			if (MakeListConsistingOfArc(vOutsideVtx, vBaseVtx0, vBaseVtx1, nSubdivideNum + 1, sidePoints[nCount]))
			{
				...
				for (i32 a = 1, iSize(sidePoints[nCount].size()); a < iSize; ++a)
				{
				...
				if (rp.mapBetweenEdgeIdToVertex.find(edgeId) != rp.mapBetweenEdgeIdToVertex.end())
				{
					...
					for (i32 a = 0, iSideVertexCount(sideVertices[nCount].size()); a < iSideVertexCount; ++a)
					{
						if (nCount == 0)
		...
		if (nCount == 2 && !sideVertices[0].empty() && sideVertices[0].size() == sideVertices[1].size())
		{
			...
			i32 nValidDivideNum = sideVertices[0].size();
			for (i32 i = 0; i < nValidDivideNum - 1; ++i)
			{
				std::vector<BrushVec3> vList;
...
i32 BevelTool::FindCorrespondingEdge(const BrushEdge3D& e, const std::vector<InfoForSubdivingApexPolygon>& infoForSubdividingApexPolygonList) const
{
	for (i32 i = 0, iCount(infoForSubdividingApexPolygonList.size()); i < iCount; ++i)
	{
...
void BevelTool::PP1_SubdivideApexPolygon(i32 nSubdivideNum, const std::vector<InfoForSubdivingApexPolygon>& infoForSubdividingApexPolygonList)
{
	ResultForNextPhase& rp = m_ResultForSecondPhase;

	for (i32 i = 0, iCount(rp.middlePhaseApexPolygons.size()); i < iCount; ++i)
	{
		...
		for (i32 k = 0, iVListCount(vList.size()); k < iVListCount; ++k)
		{
			BrushEdge3D e(vList[k].pos, vList[(k + 1) % iVListCount].pos);
			i32 nCorrespondingEdgeIndex = FindCorrespondingEdge(e, infoForSubdividingApexPolygonList);
		...
		if (!initialPolygons.empty())
		{
			...
			for (i32 k = 0, iInitialPolygonCount(initialPolygons.size()); k < iInitialPolygonCount; ++k)
...
std::vector<PolygonPtr> BevelTool::CreateFirstOddSubdividedApexPolygons(const std::vector<const InfoForSubdivingApexPolygon*>& subdividedEdges)
{
	...
	i32 nSubdivisionCount = subdividedEdges[0]->vIntermediate.size();
	i32 nMiddleVertexIndex = nSubdivisionCount / 2;

	BrushVec3 vCenter(0, 0, 0);
	i32 nSubdividedEdgeCount(subdividedEdges.size());
	for (i32 i = 0; i < nSubdividedEdgeCount; ++i)
...
std::vector<PolygonPtr> BevelTool::CreateFirstEvenSubdividedApexPolygons(const std::vector<const InfoForSubdivingApexPolygon*>& subdividedEdges)
{
	...
	std::vector<BrushVec3> vList;
	for (i32 i = 0, iEdgeCount(subdividedEdges.size()); i < iEdgeCount; ++i)
	{
		i32 nPrevIndex = i == 0 ? iEdgeCount - 1 : i - 1;
		i32 nCurrIndex = i;
...
bool BevelTool::OnMouseMove(CViewport* view, UINT nFlags, CPoint point)
{
	if (m_BevelMode != eBevelMode_Nothing)
	{
		i32 offset = point.y - m_nMousePrevY;
		if (m_BevelMode == eBevelMode_Spread)
...
bool BevelTool::OnKeyDown(CViewport* view, u32 nKeycode, u32 nRepCnt, u32 nFlags)
{
...
i32 BevelTool::GetEdgeCountHavingVertexInElementList(const BrushVec3& vertex, const ElementSet& elementList) const
{
	i32 nCount = 0;
	for (i32 i = 0, iElementCount(elementList.GetCount()); i < iElementCount; ++i)
	{
		const Element& elementInfo = elementList.Get(i);
		for (i32 k = 0, iVertexCount(elementInfo.m_Vertices.size()); k < iVertexCount; ++k)
		{
