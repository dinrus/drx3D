  ...
  bool bPathClosed)
{
	...
	i32 iPathCount = vPath.size();
	planeAtEveryIntersection.resize(iPathCount);

	i32 nPathCount = bPathClosed ? iPathCount : iPathCount - 2;
	for (i32 i = 0; i < nPathCount; ++i)
	{
  ...
  PolygonPtr pInitPolygon, bool bFlip)
{
	...
	BrushPlane plane;
	i32 id = DesignerSession::GetInstance()->GetCurrentSubMatID();
  ...
  const std::vector<PolygonPtr>& polygons)
{
	for (i32 i = 0, iPolygonCount(polygons.size()); i < iPolygonCount; ++i)
	{
		...
		for (i32 k = 0, iCandidateCount(candidatedPolygons.size()); k < iCandidateCount; ++k)
		{
		...
		for (i32 k = 0, iIntersectCount(intersectedPolygons.size()); k < iIntersectCount; ++k)
		{
  ...
  PolygonPtr pPathPolygon)
{
	...
	i32 nVertexCount = pInitProfilePolygon->GetVertexCount();
	i32 iEdgeCount = pInitProfilePolygon->GetEdgeCount();
	...
	i32 nPathCount = bClosed ? iPathCount : iPathCount - 1;
	for (i32 i = 0; i < nPathCount; ++i)
	{
		i32 nPathIndex = (nStartIndex + i) % iPathCount;
		i32 nNextPathIndex = (nPathIndex + 1) % iPathCount;
	...
	pSelected->Clear();
	for (i32 i = 0, iCount(newPolygons.size()); i < iCount; ++i)
