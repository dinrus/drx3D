...
void SubdivisionTool::Enter()
{
	...
	for (i32 i = 0, iSize(pSelected->GetCount()); i < iSize; ++i)
	{
...
void SubdivisionTool::Subdivide(i32 nLevel, bool bUpdateBrush)
{
...
void SubdivisionTool::HighlightEdgeGroup(tukk edgeGroupName)
{
	DesignerSession* pSession = DesignerSession::GetInstance();
	...
	for (i32 i = 0, iSize(m_HighlightedSharpEdges.size()); i < iSize; ++i)
...
void SubdivisionTool::DeleteEdgeTag(tukk name)
{
	if (name == NULL)
		return;
	EdgeSharpnessManager* pEdgeSharpnessMgr = GetModel()->GetEdgeSharpnessMgr();
...
void SubdivisionTool::Display(DisplayContext& dc)
{
	...
	for (i32 i = 0, iCount(m_HighlightedSharpEdges.size()); i < iCount; ++i)
...
void SubdivisionTool::DeleteAllUnused()
{
	...
	for (i32 i = 0, iCount(pEdgeSharpnessMgr->GetCount()); i < iCount; ++i)
	{
		...
		for (i32 k = 0, iEdgeCount(sharpness.edges.size()); k < iEdgeCount; ++k)
		{
	...
	for (i32 i = 0, iCount(deletedEdges.size()); i < iCount; ++i)
