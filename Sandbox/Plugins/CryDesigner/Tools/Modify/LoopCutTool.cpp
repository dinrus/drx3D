...
bool LoopCutTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
void LoopCutTool::UpdateLoops()
{
	...
	if (m_pSelectedPolygon->QueryNearestEdge(m_vPosOnSurface, nearestEdge, nearestPos))
	{
		...
		{
			for (i32 i = 0, iPolygonCount(neighborPolygons.size()); i < iPolygonCount; ++i)
			{
				...
				for (i32 k = 0, iPolygonCount(m_LoopPolygons.size()); k < iPolygonCount; ++k)
				{
		...
		if (m_LoopCutMode == eLoopCutMode_Divide)
		{
			for (i32 i = 0, eListCcount(eList.size()); i < eListCcount; ++i)
			{
				if (!eList[i].IsPoint())
				{
...
void LoopCutTool::Display(DisplayContext& dc)
{
	if (!m_LoopPolygons.empty())
	{
		...
		for (i32 i = 0, iCount(m_LoopPolygons.size()); i < iCount; ++i)
		{
			const std::vector<BrushEdge3D>& edges = m_LoopPolygons[i].edges;
			for (i32 k = 1, iEdgeCount(edges.size()); k < iEdgeCount - 1; ++k)
...
void LoopCutTool::FreezeLoops()
{
	...
	for (i32 i = 0, iLoopCount(m_LoopPolygons.size()); i < iLoopCount; ++i)
	{
		PolygonPtr pPolygon = m_LoopPolygons[i].pPolygon;

		i32 nInit = std::abs(m_NormalizedSlideOffset + 1.0) <= kDesignerEpsilon ? 1 : 0;
		i32 nEnd = std::abs(m_NormalizedSlideOffset - 1.0) <= kDesignerEpsilon ? 1 : 0;
		for (i32 k = nInit, iEdgeCount(m_LoopPolygons[i].edges.size()); k < iEdgeCount - 1 - nEnd; ++k)
		{
