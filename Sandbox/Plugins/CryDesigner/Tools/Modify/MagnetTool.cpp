...
void MagnetTool::PrepareChooseFirstPointStep()
{
	...
	for (i32 i = 0, iVertexCount(m_pSelectedPolygon->GetVertexCount()); i < iVertexCount; ++i)
	{
		AddVertexToList(
	...
	for (i32 i = 0, iEdgeCount(m_pSelectedPolygon->GetEdgeCount()); i < iEdgeCount; ++i)
	{
...
void MagnetTool::AlignSelectedPolygon()
{
	...
	{
		i32 iEdgeCount(edgeIndices.size());
...
bool MagnetTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
bool MagnetTool::OnMouseMove(CViewport* view, UINT nFlags, CPoint point)
{
	...
	if (m_Phase == eMTP_ChooseFirstPoint || m_Phase == eMTP_ChooseUpPoint)
	{
		...
		i32 nSelectedVertex = -1;
		for (i32 i = 0, iVertexCount(m_SourceVertices.size()); i < iVertexCount; ++i)
		{
