...
void OffsetTool::ApplyOffset(
  ...
  bool bBridgeEdges)
{
	for (i32 i = 0, iHoleCount(holes.size()); i < iHoleCount; ++i)
	...
	std::map<i32, BrushLine> correspondingLineMap;
	PolygonPtr pResizedPolygon = pOriginalPolygon->Clone();
	pResizedPolygon->Scale((BrushFloat)0.01);

	for (i32 i = 0, iCount(pResizedPolygon->GetVertexCount()); i < iCount; ++i)
	{
		i32 nCorrespondingIndex = 0;
	...
	for (i32 i = 0, iVertexCount(pScaledPolygon->GetVertexCount()); i < iVertexCount; ++i)
	{
		...
		{
			i32 nVertexIndexInSelectedPolygon = ii->first;
