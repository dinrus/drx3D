	...
	if (perpendicularPolygons.size() == m_PickedHolePolygon->GetEdgeCount() * 2)
	{
		...
		if (eFindOppositePolygon_Intersection == m_pHoleContainer->QueryOppositePolygon(m_PickedHolePolygon, eFOF_PushDirection, 0.0f, pOppositePolygon, fDistance))
		{
			...
			if (perpendicularPolygons.size() == perpendicularPolygonsFromOpposite.size())
			{
				...
				i32 iPolygonSize(perpendicularPolygons.size());
...
{
	for (i32 i = 0, iPolygonSize(polygonList.size()); i < iPolygonSize; ++i)
	{
...
bool FillTool::FillHoleBasedOnSelectedElements()
{
	...
	if (validEdgeList.empty())
	{
		...
		if (validVertexList.size() < 2)
			return false;
		for (i32 i = 0, iVertexSize(validVertexList.size()); i < iVertexSize; ++i)
	...
	std::set<i32> usedEdgeSet;

	linkedEdgeList.push_back(validEdgeList[0]);
	usedEdgeSet.insert(0);
	i32k nValidEdgeSize(validEdgeList.size());
	...
	for (i32 i = 0, iEdgeSize(linkedEdgeList.size()); i < iEdgeSize; ++i)
...
void FillTool::CompileHoles()
{
	...
	for (i32 i = 0, iPolygonSize(GetModel()->GetPolygonCount()); i < iPolygonSize; ++i)
	{
	...
	{
		for (i32 i = 0, iPolygonSize(polygonList[nSource].size()); i < iPolygonSize; ++i)
		{
	...
	for (i32 i = 0, iPolygonSize(polygonList[nTarget].size()); i < iPolygonSize; ++i)
	{
		std::vector<PolygonPtr> innerPolygons = polygonList[nTarget][i]->GetLoops()->GetHoleClones();
		for (i32 k = 0, innerPolygonSize(innerPolygons.size()); k < innerPolygonSize; ++k)
		{
			innerPolygons[k]->ReverseEdges();
			for (i32 a = 0; a < iPolygonSize; ++a)
			{
				if (a == i)
					continue;
