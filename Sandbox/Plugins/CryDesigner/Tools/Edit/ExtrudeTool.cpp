...
void ExtrudeTool::RaiseHeight(const CPoint& point, CViewport* view, i32 nFlags)
{
	...
	for (i32 i = 0, iVertexCount(pCapPolygon->GetVertexCount()); i < iVertexCount; ++i)
	{
...
void ExtrudeTool::UpdateModel(ExtrusionContext& ec)
{
	...
	{
		ec.pModel->SetShelf(eShelf_Base);
		for (i32 i = 0, iPolygonCount(ec.backupPolygons.size()); i < iPolygonCount; ++i)
			ec.pModel->RemovePolygon(ec.backupPolygons[i]);
		for (i32 i = 0, iPolygonCount(ec.mirroredBackupPolygons.size()); i < iPolygonCount; ++i)
	...
	{
		for (i32 i = 0, iSidePolygonSize(sidePolygons.size()); i < iSidePolygonSize; ++i)
		{
			...
			std::set<i32> removedBackUpPolygons;

			for (i32 k = 0, iBackupPolygonSize(ec.backupPolygons.size()); k < iBackupPolygonSize; ++k)
			{
