...
bool CurveTool::OnLButtonUp(CViewport* view, UINT nFlags, CPoint point)
{
	...
	else if (m_ArcState == eArcState_ControlMiddlePoint)
	{
		...
		SpotList spotList;
		i32 iSpotListSize(GetSpotListCount());
		for (i32 i = 0; i < iSpotListSize - 1; ++i)
		{
			...
			for (i32 k = 0, iSize(intersections.size()); k < iSize; ++k)
			{
...
void CurveTool::PrepareArcSpots(CViewport* view, UINT nFlags, CPoint point)
{
	...
	i32 nEdgeCount = m_CurveParameter.m_NumOfSubdivision;
...
bool CurveTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
