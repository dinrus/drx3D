...
bool CubeEditor::OnLButtonUp(CViewport* view, UINT nFlags, CPoint point)
{
	...
	for (i32 i = 0, iBrushAABBCount(m_BrushAABBs.size()); i < iBrushAABBCount; ++i)
	{
...
BrushVec3 CubeEditor::Snap(const BrushVec3& vPos) const
{
	...
	for (i32 i = 0; i < 3; ++i)
	{
		for (i32 k = 0, iCutUnitCount(fCutUnits.size()); k < iCutUnitCount; ++k)
		{
...
bool CubeEditor::OnMouseMove(CViewport* view, UINT nFlags, CPoint point)
{
	...
	if (nFlags & MK_LBUTTON)
	{
		if (m_DS.m_bPressingShift)
		{
			...
			i32 nNumber = fLength / fCubeSize;
			m_BrushAABBs.clear();
			for (i32 i = 0; i <= nNumber; ++i)
			{
...
{
	for (i32 i = 0, iBrushCount(m_BrushAABBs.size()); i < iBrushCount; ++i)
	{
...
AABB CubeEditor::GetBrushBox(const BrushVec3& vSnappedPos, const BrushVec3& vPickedPos, const BrushVec3& vNormal)
{
	...
	if (nElement != -1)
	{
		i32 nSign = vNormal[nElement] > 0 ? 1 : -1;
		if (GetEditMode() != eCubeEditorMode_Add)
			nSign = -nSign;
...
std::vector<PolygonPtr> CubeEditor::GetBrushPolygons(const AABB& aabb) const
{
	...
	if (!aabb.IsReset())
	{
		...
		i32 nMatID = DesignerSession::GetInstance()->GetCurrentSubMatID();
		for (i32 i = 0, iPolygonCount(brushPolygons.size()); i < iPolygonCount; ++i)
...
void CubeEditor::AddCube(const AABB& brushAABB)
{
	...
	for (i32 i = 0, iPolygonCount(brushPolygons.size()); i < iPolygonCount; ++i)
	{
		...
		if (!candidatePolygons.empty())
		{
			...
			if (!touchedPolygons.empty())
			{
				for (i32 k = 0, iTouchedPolygonCount(touchedPolygons.size()); k < iTouchedPolygonCount; ++k)
				{
					if (expandedBrushAABB.ContainsBox(touchedPolygons[k]->GetExpandedBoundBox()))
					{
						...
						for (i32 a = 0, iOutsidePolygonCount(outsidePolygons.size()); a < iOutsidePolygonCount; ++a)
...
void CubeEditor::RemoveCube(const AABB& brushAABB)
{
	...
	for (; ii != boundaryPolygons.end(); )
	{
		...
		for (i32 k = 0, iBrushPolygonCount(brushPolygons.size()); k < iBrushPolygonCount; ++k)
		{
	...
	for (auto ii = intersectedPolygons.begin(); ii != intersectedPolygons.end(); )
	{
		...
		{
			bool bTouched = false;
			for (i32 i = 0, iBrushPolygonCount(brushPolygons.size()); i < iBrushPolygonCount; ++i)
			{
		...
		else if (brushAABB.IsIntersectBox((*ii)->GetExpandedBoundBox()))
		{
			...
			{
				GetModel()->RemovePolygon(*ii);
				for (i32 i = 0, iPosCount(output.posList.size()); i < iPosCount; ++i)
	...
	for (i32 i = 0, iBrushPolygonCount(brushPolygons.size()); i < iBrushPolygonCount; ++i)
	{
		...
		{
			for (i32 k = 0, negListCount(output.negList.size()); k < negListCount; ++k)
	...
	for (i32 i = 0, iPolygonCount(model.GetPolygonCount()); i < iPolygonCount; ++i)
	{
...
void CubeEditor::PaintCube(const AABB& brushAABB)
{
	...
	i32 submatID = DesignerSession::GetInstance()->GetCurrentSubMatID();
	...
	Model designer[2];
	for (i32 i = 0, iCandidatePolygonCount(candidatePolygons.size()); i < iCandidatePolygonCount; ++i)
	{
		OutputPolygons output;
		...
		for (i32 k = 0, iPosPolygonCount(output.posList.size()); k < iPosPolygonCount; ++k)
			designer[0].AddUnionPolygon(output.posList[k]);

		i32 submatID = DesignerSession::GetInstance()->GetCurrentSubMatID();
		for (i32 k = 0, iPolygonCountInside(polygonsInside.size()); k < iPolygonCountInside; ++k)
		{
	...
	for (i32 i = 0; i < 2; ++i)
	{
		for (i32 k = 0, iPolygonCount(designer[i].GetPolygonCount()); k < iPolygonCount; ++k)
...
void CubeEditor::AddBrush(const AABB& aabb)
{
	...
	for (i32 i = 0, iCount(m_BrushAABBs.size()); i < iCount; ++i)
	{
