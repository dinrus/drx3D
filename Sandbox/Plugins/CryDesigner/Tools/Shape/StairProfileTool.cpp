...
void StairProfileTool::DrawCandidateStair(DisplayContext& dc, i32 nIndex, const ColorB& color)
{
	...
	i32 nSize = m_CandidateStairs[nIndex].size();
	if (nSize == 0)
		return;
...
bool StairProfileTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
bool StairProfileTool::OnLButtonDown(CViewport* view, UINT nFlags, CPoint point)
{
	...
	else if (m_SideStairMode == eSideStairMode_SelectDirection)
	{
		...
		i32 nStairSize = selectedStairs.size();
...
void StairProfileTool::CreateCandidates()
{
	...
	if (heightMode == eStairHeightCalculationWay_StepRise)
	{
		...
		i32 nElement = std::abs(vZ.Dot(vX3D)) > std::abs(vZ.Dot(vY3D)) ? 0 : 1;
