...
void PolylineTool::RegisterSpotListAfterBreaking()
{
	...
	for (i32 i = 0, iSpotSize(GetSpotListCount()); i < iSpotSize; ++i)
	{
		...
		if (spot.IsOnEdge())
		{
			...
			{
				SpotList spotList;
				for (i32 k = nFirstSpotIndexOnEdge; k <= i; ++k)
				{
...
void PolylineTool::RegisterEitherEndSpotList()
{
	...
	{
		for (i32 i = 1; i < GetSpotListCount(); ++i)
		{
	...
	{
		for (i32 i = GetSpotListCount() - 2; i >= 0; --i)
		{
			if (GetSpot(i).IsOnEdge())
			{
				SpotList spotList;
				for (i32 k = i; k < GetSpotListCount(); ++k)
					spotList.push_back(GetSpot(k));
				RegisterSpotList(GetModel(), spotList);
				break;
			}
...
void PolylineTool::AddPolygonWithCurrentSameAsFirst()
{
	...
	i32 nFirstSpotIndex = GetSpotListCount() - 1;
	...
	else
	{
		...
		i32 nSpotIndex = nFirstSpotIndex;
		do
		{
...
void PolylineTool::PutCurrentSpot()
{
	...
	if (GetSpotListCount() > 0 && GetModel())
	{
		...
		for (i32 i = 0, iSize(intersections.size()); i < iSize; ++i)
		{
...
bool PolylineTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
ELineState PolylineTool::GetAlienedPointWithAxis(const BrushVec3& v0, const BrushVec3& v1, const BrushPlane& plane, IDisplayViewport* view, std::vector<BrushEdge3D>* pAxisList, BrushVec3& outPos) const
{
	...
	{
		for (i32 i = 0, iSize(pAxisList->size()); i < iSize; ++i)
		{
...
bool PolylineTool::IntersectExisintingLines(const BrushVec3& v0, const BrushVec3& v1, i32* pOutSpotIndex) const
{
	...
	for (i32 i = 0, iSpotSize(GetSpotListCount() - 1); i < iSpotSize; ++i)
	{
