                            ...
                            OutputStairParameter& out)
{
	...
	i32 nCompleteStepNum = (i32)(fBoxHeight / fStepRise);
	...
	i32 nWidthSign = fBoxWidth > 0 ? 1 : -1;
	i32 nDepthSign = fBoxDepth > 0 ? 1 : -1;
	...
	for (i32 i = 0; i <= nCompleteStepNum; ++i)
	{
...
void StairTool::UpdateShape(bool bUpdateUIs)
{
	...
	for (i32 i = 0, iPolygonCount(output.polygons.size()); i < iPolygonCount; ++i)
	{
...
void StairTool::RegisterShape(PolygonPtr pFloorPolygon)
{
	...
	for (i32 k = 0, iCount(m_PolygonsNeedPostProcess.size()); k < iCount; ++k)
	{
