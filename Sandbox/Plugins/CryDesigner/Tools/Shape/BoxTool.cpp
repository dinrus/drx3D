...
void BoxTool::UpdateShape(bool bUpdateUIs)
{
	...
	for (i32 i = 0, iCount(polygonList.size()); i < iCount; ++i)
	{
...
void BoxTool::RegisterShape(PolygonPtr pFloorPolygon)
{
	...
	if (m_pCapPolygon)
	{
		...
		if (!IsSeparateStatus())
		{
			...
			for (i32 i = 0, iSidePolygonCount(sidePolygons.size()); i < iSidePolygonCount; ++i)
			{
				for (i32 k = 0; k < 2; ++k)
				{
			...
			for (i32 i = 0, iCount(intersectedSidePolygons.size()); i < iCount; ++i)
			{
				GetModel()->SetShelf(eShelf_Base);
			}
		}
