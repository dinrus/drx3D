...
bool SelectGrowTool::SelectAdjacentEdgesToPolygonVertex(CBaseObject* pObject, ElementSet* pSelected, PolygonPtr pPolygon, i32 index)
{
	i32 prevEdge, nextEdge;
...
bool SelectGrowTool::SelectAdjacentElements(MainContext& mc)
{
	...
	bool bAddedNewAdjacent = false;
	for (i32 i = 0, nPolygonCount(mc.pModel->GetPolygonCount()); i < nPolygonCount; ++i)
	{
		...
		for (i32 i = 0, iSelectionCount(initSelection.size()); i < iSelectionCount; ++i)
		{
			...
			else if (ele.IsVertex())
			{
					...
					&& pPolygon->GetAdjacentEdgesByVertexIndex(index, &prevEdge, &nextEdge))
				{
					if (prevEdge != -1)
					{
						...
						i32k otherIndex = edgePair.m_i[0] == index ? edgePair.m_i[1] : edgePair.m_i[0];
