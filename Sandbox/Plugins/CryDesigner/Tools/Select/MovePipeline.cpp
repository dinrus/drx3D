...
bool MovePipeline::VertexAdditionFirstPass()
{
	...
	for (i32 i = 0, iQuerySize(m_InitQueryResult.size()); i < iQuerySize; ++i)
	{
		ModelDB::Vertex& v = m_InitQueryResult[i];
		i32 vertexMarkListSize(v.m_MarkList.size());

		for (i32 k = 0; k < vertexMarkListSize; ++k)
		{
			...
			for (i32 a = 0; a < 2; ++a)
			{
				...
				for (i32 b = 0, iNewVertexSize(newVertices.size()); b < iNewVertexSize; ++b)
				{
	...
	for (i32 i = 0, iNewVertexSize(newVertices.size()); i < iNewVertexSize; ++i)
	{
		ModelDB::Vertex& v = newVertices[i];
		for (i32 k = 0, iMarkListSize(v.m_MarkList.size()); k < iMarkListSize; ++k)
		{
...
bool MovePipeline::VertexAdditionSecondPass()
{
	...
	{
		ModelDB::Vertex& v = m_InitQueryResult[i];
		i32 markListSize(v.m_MarkList.size());
		...
		for (i32 k = eShelf_Base; k < cShelfMax; ++k)
		{
...
bool MovePipeline::SubdivisionPass()
{
	...
	for (; ii != m_OrganizedQueryResult.end(); ++ii)
	{
		...
		i32 iQuerySize(queryInputs.size());
		...
		for (; i < iQuerySize; ++i)
		{
			i32 nQueryIndex(queryInputs[i].first);
		...
		for (i32 i = 0, iTrianglePolygonSize(trianglePolygons.size()); i < iTrianglePolygonSize; ++i)
	...
	for (i32 i = 0, iPolygonSize(addedPolygons.size()); i < iPolygonSize; ++i)
...
void MovePipeline::TransformationPass()
{
	...
	for (; ii != m_OrganizedQueryResult.end(); ++ii)
	{
		...
		for (i32 i = 0; i < iQuerySize; ++i)
		{
			i32 nQueryIndexInQueryInputs(queryInputs[i].first);
...
bool MovePipeline::MergeCoplanarPass()
{
	...
	for (; ii != m_SubdividedPolygons.end(); ++ii)
	{
		...
		i32 iPolygonSize(polygonList.size());
		if (iPolygonSize == 1)
			continue;
...
PolygonPtr MovePipeline::FindAdjacentPolygon(PolygonPtr pPolygon, const BrushVec3& vPos, i32& outAdjacentPolygonIndex)
{
	for (i32 a = 0, iPolygonSize(GetModel()->GetPolygonCount()); a < iPolygonSize; ++a)
	{
...
void MovePipeline::Initialize(const ElementSet& elements)
{
	...
	for (i32 i = 0, iQueryResult(m_QueryResult.size()); i < iQueryResult; ++i)
	{
		const ModelDB::Vertex& v = m_QueryResult[i];
		for (i32 k = 0, iMarkSize(v.m_MarkList.size()); k < iMarkSize; ++k)
		{
...
void MovePipeline::InitializeIsolated(ElementSet& elements)
{
	...
	std::map<PolygonPtr, std::vector<i32>> polygons;

	for (i32 i = 0, iElementCount(elements.GetCount()); i < iElementCount; ++i)
	{
		if (elements[i].IsPolygon())
		{
			for (i32 k = 0, iVertexCount(elements[i].m_pPolygon->GetVertexCount()); k < iVertexCount; ++k)
		...
		else if (elements[i].IsEdge())
		{
			...
			{
				for (i32 k = 0, iVertexCount((*ii)->GetVertexCount()); k < iVertexCount; ++k)
		...
		else if (elements[i].IsVertex())
		{
			...
			if (qr.empty())
				continue;
			for (i32 b = 0, iMarkCount(qr[0].m_MarkList.size()); b < iMarkCount; ++b)
			{
				PolygonPtr polygon = qr[0].m_MarkList[b].m_pPolygon;
				for (i32 k = 0, iVertexCount(polygon->GetVertexCount()); k < iVertexCount; ++k)
	...
	for (; ii != polygons.end(); ++ii)
	{
		...
		for (i32 i = 0, iVertexCount(ii->second.size()); i < iVertexCount; ++i)
		{
			ModelDB::Vertex v;
			ModelDB::Vertex* pV = &v;
			i32 vtxIndex = ii->second[i];

			BrushVec3 pos = polygon->GetPos(vtxIndex);
			for (i32 a = 0, iQueryResultCount(m_QueryResult.size()); a < iQueryResultCount; ++a)
			{
...
void MovePipeline::End()
{
	...
	{
		for (i32 i = 0, iQuerySize(m_QueryResult.size()); i < iQuerySize; ++i)
...
bool MovePipeline::GetAveragePos(BrushVec3& outAveragePos) const
{
	...
	i32 iQSize(m_IntermediateTransQueryPos.size());
	if (iQSize > 0)
	{
		for (i32 i = 0; i < iQSize; ++i)
...
void MovePipeline::SnappedToMirrorPlane()
{
	...
	i32 iIntermediateCount(m_IntermediateTransQueryPos.size());
	BrushVec3 mirrorNormal = GetModel()->GetMirrorPlane().Normal();
	i32 nFarthestPosIndex = -1;
	BrushFloat fFarthestDist = 0;
	for (i32 i = 0; i < iIntermediateCount; ++i)
	{
