...
void RingSelectionTool::SelectRing(MainContext& mc, const BrushEdge3D& inputEdge)
{
	...
	for (i32 i = 0, iLoopPolygonCount(loopPolygons.size()); i < iLoopPolygonCount; ++i)
	{
...
void SelectLoopFromOnePolygon(MainContext& mc, const std::vector<Vertex>& quad, PolygonPtr pQuadPolygon, i32 nEdgeIndex)
{
	...
	for (i32 c = 0; c < adjacentPolygons.size(); ++c)
	{
...
void RingSelectionTool::SelectFaceRing(MainContext& mc)
{
	...
	std::set<i32> usedQuads;
	i32 quadCount = quads.size();
