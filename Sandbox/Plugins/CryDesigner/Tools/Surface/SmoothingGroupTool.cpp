...
bool IsAdjacentWithSeedPolygons(PolygonPtr pPolygon, const std::set<PolygonPtr>& seedPolygons)
{
	...
	for (; ii != seedPolygons.end(); ++ii)
	{
		...
		i32 nEdgeCount = pPolygon->GetEdgeCount();
...
void SmoothingGroupTool::ApplyAutoSmooth()
{
	...
	while (1)
	{
		...
		{
			i32 nOffset = polygonsInGroup.size();
			for (i32 i = 0; i < iSelectedElementCount; ++i)
			{
			...
			else
			{
				...
				for (i32 i = 0, iPolygonCount(polygonsInGroup.size()); i < iPolygonCount; ++i)
...
bool SmoothingGroupTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
void SmoothingGroupTool::AddPolygonsToSmoothingGroup(tukk name)
{
	CUndo undo("Designer : Smoothing Group");
...
void SmoothingGroupTool::SelectPolygons(tukk name)
{
	DesignerSession* pSession = DesignerSession::GetInstance();
...
void SmoothingGroupTool::DeleteSmoothingGroup(tukk name)
{
	CUndo undo("Designer : Smoothing Group");
...
bool SmoothingGroupTool::RenameSmoothingGroup(tukk former_name, tukk name)
{
	SmoothingGroupManager* pSmoothingGroupMgr = GetModel()->GetSmoothingGroupMgr();
