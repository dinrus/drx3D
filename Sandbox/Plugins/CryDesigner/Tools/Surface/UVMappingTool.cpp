...
void UVMappingTool::Enter()
{
	...
	i32 nCurrentEditMode = GetIEditor()->GetEditMode();
...
bool UVMappingTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
bool UVMappingTool::QueryPolygon(const BrushRay& ray, i32& nOutPolygonIndex, bool& bOutNew) const
{
	bOutNew = false;
...
void UVMappingTool::ApplyTextureInfo(const TexInfo& texInfo, i32 nModifiedParts)
{
...
void UVMappingTool::SelectPolygonsBySubMatID(i32 subMatID)
{
	MODEL_SHELF_RECONSTRUCTOR(GetModel());
	...
	for (i32 i = 0, iPolygonCount(GetModel()->GetPolygonCount()); i < iPolygonCount; ++i)
	{
		GetModel()->SetShelf(eShelf_Base);
	...
	GetModel()->SetShelf(eShelf_Base);
	for (i32 i = 0, iSelectionCount(pSelected->GetCount()); i < iSelectionCount; ++i)
...
void UVMappingTool::SetTexInfoToPolygon(PolygonPtr pPolygon, const TexInfo& texInfo, i32 nModifiedParts)
{
...
void UVMappingTool::OnManipulatorDrag(IDisplayViewport* pView, ITransformManipulator* pManipulator, CPoint& p0, BrushVec3 value, i32 nFlags)
{
	...
	i32 editMode = GetIEditor()->GetEditMode();
	if (editMode == eEditModeMove)
	{
		Vec3 vTranslation = offsetTM.GetTranslation();
		BrushVec3 vNormal(0, 0, 0);

		for (i32 i = 0, iCount(m_MouseDownContext.m_UVInfos.size()); i < iCount; ++i)
		{
...
void UVMappingTool::OnManipulatorBegin(IDisplayViewport* pView, ITransformManipulator* pManipulator, CPoint& point, i32 flags)
{
	m_bHitGizmo = true;
...
void UVMappingTool::RecordTextureMappingUndo(tukk sUndoDescription) const
{
