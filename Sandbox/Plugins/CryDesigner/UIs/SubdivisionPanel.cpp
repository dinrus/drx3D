...
SubdivisionPanel::SubdivisionPanel(SubdivisionTool* pSubdivisionTool) : m_pSubdivisionTool(pSubdivisionTool)
{
	...
	m_pSubdivisionLevelSlider->setVal(pSubdivisionTool->GetModel()->GetSubdivisionLevel());
	...
	QObject::connect(m_pSemiSharpCreaseList, &QTreeWidget::itemDoubleClicked, this, [=](QTreeWidgetItem* item, i32 column){ OnItemDoubleClicked(item, column); });
	QObject::connect(m_pSemiSharpCreaseList, &QTreeWidget::itemChanged, this, [=](QTreeWidgetItem* item, i32 column){ OnItemChanged(item, column); });
	...
	QObject::connect(pSmoothingSurfaceButton, &QCheckBox::stateChanged, this, [ = ](i32 state)
		{
			if (state == Qt::Checked)
...
void SubdivisionPanel::Update()
{
	...
	if (!mc.pModel)
		return;
	m_pSubdivisionLevelSlider->setVal(mc.pModel->GetSubdivisionLevel());
}
...
void SubdivisionPanel::RefreshEdgeGroupList()
{
	...
	for (i32 i = 0, iCount(pEdgeSharpnessMgr->GetCount()); i < iCount; ++i)
	{
...
void SubdivisionPanel::OnItemDoubleClicked(QTreeWidgetItem* item, i32 column)
{
	if (item == NULL)
		return;
...
void SubdivisionPanel::OnItemChanged(QTreeWidgetItem* item, i32 column)
{
	if (item == NULL)
		return;
...
void SubdivisionPanel::OnApply()
{
	...
	m_pSubdivisionLevelSlider->setVal(0);
}
