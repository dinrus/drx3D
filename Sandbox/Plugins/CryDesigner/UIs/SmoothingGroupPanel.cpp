	...
	m_pSmoothingGroupTool(pSmoothingGroupTool)
{
	...
	QObject::connect(m_pSmoothingGroupList, &QTreeWidget::itemDoubleClicked, this, [=](QTreeWidgetItem* item, i32 column){ OnItemDoubleClicked(item, column); });
	QObject::connect(m_pSmoothingGroupList, &QTreeWidget::itemChanged, this, [=](QTreeWidgetItem* item, i32 column){ OnItemChanged(item, column); });
...
void SmoothingGroupPanel::OnItemDoubleClicked(QTreeWidgetItem* item, i32 column)
{
	if (item == NULL)
		return;
...
void SmoothingGroupPanel::OnItemChanged(QTreeWidgetItem* item, i32 column)
{
	if (item == NULL)
		return;
