...
QToolButton* DesignerPanel::AddButton(EDesignerTool tool, const WidgetContext& wc, i32 row, i32 column, bool bColumnSpan)
{
	tukk szToolName = Serialization::getEnumDescription<EDesignerTool>().name(tool);
...
void DesignerPanel::SetAttributeWidget(tukk name, IBasePanel* pPanel)
{
	RemoveAttributeWidget();
...
i32 DesignerPanel::ArrangeButtons(WidgetContext& wc, EToolGroup toolGroup, i32 stride, i32 offset, bool enforce)
{
	std::vector<EDesignerTool> tools = ToolGroupMapper::the().GetToolList(toolGroup);
	i32 iEnd(tools.size() + offset);
	for (i32 i = offset; i < iEnd; ++i)
	{
...
QWidget* DesignerPanel::CreateSelectionWidget()
{
	...
	i32 offset = ArrangeButtons(wc, eToolGroup_BasicSelection, s_stride, 0);
...
QWidget* DesignerPanel::CreateShapeWidget()
{
	...
	i32 offset = ArrangeButtons(wc, eToolGroup_Shape, s_stride, 0, true);
...
{
	i32 insertionIndex = pTab->insertTab(pTab->count(), new QWidget(pTab), "Extrude/Delete");
...
{
	i32 insertionIndex = pTab->insertTab(pTab->count(), new QWidget(pTab), "Tools");
	QWidget* pPaper = pTab->widget(insertionIndex);
...
{
	i32 insertionIndex = pTab->insertTab(pTab->count(), new QWidget(pTab), "Groups/UV");
	QWidget* pPaper = pTab->widget(insertionIndex);
	QGridLayout* pSurfaceGridLayout = new QGridLayout;
	i32 offset = ArrangeButtons(WidgetContext(pTab, pPaper, pSurfaceGridLayout), eToolGroup_Surface, 2, 0);
...
{
	i32 insertionIndex = pTab->insertTab(pTab->count(), new QWidget(pTab), "Transform");
	QWidget* pPaper = pTab->widget(insertionIndex);
...
void DesignerPanel::ShowTab(EDesignerTool tool)
{
	...
	i32 nTabIndex = menuButton.pTab->indexOf(menuButton.pPaper);
...
i32 DesignerPanel::GetSubMatID() const
{
...
void DesignerPanel::SetSubMatID(i32 nID)
{
	CBaseObject* pObject = DesignerSession::GetInstance()->GetBaseObject();
	m_pSubMatComboBox->SetSubMatID(pObject, nID);
}

void DesignerPanel::OnSubMatSelectionChanged(i32 nSelectedItem)
{
	if (m_pSubMatComboBox->GetCurrentIndex() != nSelectedItem)
	{
		m_pSubMatComboBox->SetCurrentIndex(nSelectedItem);
