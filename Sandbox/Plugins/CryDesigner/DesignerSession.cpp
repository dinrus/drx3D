	...
	GetIEditor()->GetObjectManager()->RegisterTypeConverter(RUNTIME_CLASS(CBrushObject), "Designer", [](CBaseObject* pObject)
		{
			...
			if (!converter.ConvertToDesignerObject())
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "The selected object(s) can't be converted.");
	  });
...
void DesignerSession::SetCurrentSubMatID(i32 idx)
{
	//move to session
...
i32 DesignerSession::GetCurrentSubMatID() const
{
...
void DesignerSession::UpdateSelectionMeshFromSelectedElements(MainContext& mc)
{
	...
	for (i32 k = 0, iElementSize(pSelectedElement->GetCount()); k < iElementSize; ++k)
	{
	...
	i32 renderFlag = mc.pCompiler->GetRenderFlags();
	i32 viewDist = mc.pCompiler->GetViewDistRatio();
	u32 minSpec = mc.pObject->GetMinSpec();
	u32 materialLayerMask = mc.pObject->GetMaterialLayersMask();
  ...
  bool bForce)
{
	...
	i32 renderFlag = pCompiler->GetRenderFlags();
	i32 viewDist = pCompiler->GetViewDistRatio();
	u32 minSpec = pObj->GetMinSpec();
	u32 materialLayerMask = pObj->GetMaterialLayersMask();
