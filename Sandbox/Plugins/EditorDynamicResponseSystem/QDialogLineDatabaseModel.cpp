...
//--------------------------------------------------------------------------------------------------
i32 QDialogLineDatabaseModel::rowCount(const QModelIndex& parent) const
{
...
//--------------------------------------------------------------------------------------------------
i32 QDialogLineDatabaseModel::columnCount(const QModelIndex& parent) const
{
	return (i32)EColumns::SIZE;
}

//--------------------------------------------------------------------------------------------------
QVariant QDialogLineDatabaseModel::data(const QModelIndex& index, i32 role) const
{
		...
		case Qt::EditRole:
			{
				if (ItemType(index) == EItemType::DIALOG_LINE)
				{
					...
					if (pDialogLineSet)
					{
						...
						if (pLine)
						{
							...
							case EColumns::PAUSE:
								return QtUtil::ToQStringSafe(DrxStringUtils::toString(pLine->GetPauseLength()));
							case EColumns::CUSTOM_DATA:
				...
				else
				{
					...
					if (pDialogLineSet)
					{
						...
						case EColumns::PRIORITY:
							return QtUtil::ToQStringSafe(DrxStringUtils::toString(pDialogLineSet->GetPriority()));
						case EColumns::FLAGS:
							{
								u32 lineFlags = pDialogLineSet->GetFlags();
							...
							break;
						case EColumns::MAX_QUEUING_DURATION:
							return QtUtil::ToQStringSafe(DrxStringUtils::toString(pDialogLineSet->GetMaxQueuingDuration()));
						case EColumns::SUBTITLE:
						...
						case EColumns::PAUSE:
							if (pDialogLineSet->GetLineCount() > 0)
							{
								return QtUtil::ToQStringSafe(DrxStringUtils::toString(pDialogLineSet->GetLineByIndex(0)->GetPauseLength()));
							}
							break;
						case EColumns::CUSTOM_DATA:
							if (pDialogLineSet->GetLineCount() > 0)
...
//--------------------------------------------------------------------------------------------------
bool QDialogLineDatabaseModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
...
//--------------------------------------------------------------------------------------------------
QVariant QDialogLineDatabaseModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
//--------------------------------------------------------------------------------------------------
bool QDialogLineDatabaseModel::insertRows(i32 row, i32 count, const QModelIndex& parent)
{
	...
	else
	{
		for (i32 i = 0; i < count; ++i)
		{
			...
			if (pLineSet)
			{
				...
				for (i32 i = 0; i < pLineSet->GetLineCount() - 1; ++i)
				{
...
//--------------------------------------------------------------------------------------------------
bool QDialogLineDatabaseModel::removeRows(i32 row, i32 count, const QModelIndex& parent)
{
	SLineItem* pParent = ItemFromIndex(parent);
	if (pParent)
	{
...
//--------------------------------------------------------------------------------------------------
QModelIndex QDialogLineDatabaseModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
	if ((row >= 0) && (column >= 0))
	{
...
//--------------------------------------------------------------------------------------------------
bool QDialogLineDatabaseModel::ExecuteScript(i32 row, i32 count)
{
	const uint lineCount = m_pDatabase->GetLineSetCount();
	if (row + count > lineCount)
	{
...
void QDialogLineDatabaseModel::ForceDataReload()
{
	...
	if (m_pDatabase)
	{
		u32k size = m_pDatabase->GetLineSetCount();
		for (u32 i = 0; i < size; ++i)
		{
			...
			if (pLineSet)
			{
				...
				u32k count = pLineSet->GetLineCount() - 1;
