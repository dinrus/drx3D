...
static i32 s_currentExpandLevel = 4;
...
public:
	...
	static string GetPathForSignal(ExplorerEntry* pEntry, const stack_string& signalName)
	{
		...
		for (i32 i = 0; i < pEntry->children.size(); i++)
		{
...
CDrsResponseEditorWindow::CDrsResponseEditorWindow()
{
	...
	{
		DRX_ASSERT(s_pPropertyTree == nullptr);
		DRX_ASSERT(s_pSubElementsTree == nullptr);
		...
		QObject::connect(pSignalViewTabs, &QTabWidget::currentChanged, this, [=](i32 index)
		{
		...
		{
			pSignalViewTabs->setCurrentIndex((i32)eCDS_eSignalsFromFile);
			s_CurrentlyDisplayedSubelements = eCDS_eSignalsFromFile;
...
void CDrsResponseEditorWindow::OnFileExplorerSelectionChanged()
{
	...
	i32k selectedSignals = s_RecentResponseSerializer.m_signalNames.size();
...
//--------------------------------------------------------------------------------------------------
bool CResponsesFileLoader::Load(tukk szFilename)
{
...
//--------------------------------------------------------------------------------------------------
bool CResponsesFileLoader::Save(tukk szFilename)
{
	m_currentFile = szFilename;
	s_pPropertyTree->apply(false);
	PathHelper::CreateDirectoryIfNotExisting(PathUtil::GetGameFolder() + DRX_NATIVE_PATH_SEPSTR + szFilename);
...
//--------------------------------------------------------------------------------------------------
void BaseResponseTreeWidget::FillWithSignals(DynArray<tukk >& elements)
{
	DynArray<signalNameWithCounter> filteredElements;
	for (DynArray<tukk >::iterator it = elements.begin(); it != elements.end(); ++it)
	{
	...
	if (filteredElements.empty())
	{
		...
		newItem->setText(1, DrxStringUtils::toString(0).c_str());
	...
	else
	{
		for (DynArray<signalNameWithCounter>::iterator it = filteredElements.begin(); it != filteredElements.end(); ++it)
		{
			...
			newItem->setText(1, DrxStringUtils::toString(it->counter).c_str());
...
RecentResponsesWidget::RecentResponsesWidget()
{
	...
	QObject::connect(m_pTree, &QTreeWidget::itemClicked, this, [=](QTreeWidgetItem* pItem, i32 column) { UpdateTrees(); });
	...
	QObject::connect(m_pCreateNewResponseButton, &QPushButton::clicked, this, [ = ]
	{
		if (!s_ResponseSerializer.m_signalNames.empty())
		{
		  ...
		  if (gEnv->pDynamicResponseSystem->GetResponseManager()->AddResponse(signalToCreate))
		  {
		    ...
		    {
		      DrxWarning(VALIDATOR_MODULE_DRS, VALIDATOR_WARNING, "(DRS) Failed to save file %s", outName.c_str());
		    }
		  }
		}
	});
...
CPropertyTreeWithOptions::CPropertyTreeWithOptions(QWidget* pParent) : QWidget(pParent)
{
	...
	QAction* pAction = pToolBar->addAction(DrxIcon("icons:General/File_Save.ico"), QString("Save current response (Ctrl+s)"));
	...
	QObject::connect(pAction, &QAction::triggered, this, [ = ]
	{
		if (s_CurrentlyDisplayedSubelements == eCDS_eSignalsFromFile)
		{
		  ...
		  i32 numSavedEntries = 0;
		  i32 numSelectedEntries = entries.size();
		  for (i32 i = 0; i < numSelectedEntries; i++)
		  {
		...
		else if (s_CurrentlyDisplayedSubelements == eCDS_eRecentResponses)
		{
		  for (const stack_string& signalName : s_ResponseSerializer.m_signalNames)
		  {
		    ...
		    {
		      PathHelper::CreateDirectoryIfNotExisting(PathUtil::GetGameFolder() + DRX_NATIVE_PATH_SEPSTR + pathForSignal);
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/Load_From_Disc.ico"), QString("Reload from File (Ctrl+l)"));
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/Visibility_False.ico"), QString("Show/hide Actions and Conditions (Ctrl+h)"));
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/Tick.ico"), QString("Evaluate Conditions (for current signal-sender) (Ctrl+e)"));
	pAction->setCheckable(true);
	pAction->setChecked(false);
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/Exit.ico"), QString("Reset DRS"));
	QObject::connect(pAction, &QAction::triggered, this, [ = ]
	{
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/Pointer_Up.ico"), QString("Collapse all (Ctrl+1)"));
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/Pointer_Up_Tinted.ico"), QString("collapse one level (Ctrl+2)"));
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/Tick_Box_Tinted.ico"), QString("collapse to default depth (Ctrl+3)"));
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/Pointer_Down_Tinted.ico"), QString("expand one level(Ctrl+4)"));
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/Pointer_Down.ico"), QString("Expand All (Ctrl+5)"));
	...
	m_pSendSignalButton = pToolBar->addAction(DrxIcon("icons:General/Arrow_Right.ico"), QString("Send Signal on Actor (Ctrl+B)"));
...
CPropertyTreeWithAutoupdateOption::CPropertyTreeWithAutoupdateOption(QWidget* pParent) : QWidget(pParent)
{
	...
	m_pAutoupdateButton = pToolBar->addAction(DrxIcon("icons:General/Reload.ico"), QString("Auto Update"));
	...
	QAction* pAction = pToolBar->addAction(DrxIcon("icons:General/File_Save.ico"), QString("Save current log to JSON"));
	QObject::connect(pAction, &QAction::triggered, this, [ = ]
	{
	...
	pAction = pToolBar->addAction(DrxIcon("icons:General/TrashCan.ico"), QString("Clear History"));
	QObject::connect(pAction, &QAction::triggered, this, [ = ]
	{
	...
	pMaxElements->setVal(128);
	pMaxElements->setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::Fixed);

	QObject::connect(pMaxElements, static_cast<void (QSpinBox::*)(i32)>(&QSpinBox::valueChanged), this, [=](i32 newValue)
	{
