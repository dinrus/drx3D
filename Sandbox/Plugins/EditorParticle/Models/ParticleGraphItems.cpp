...
CNodeItem::CNodeItem(pfx2::IParticleComponent& component, DrxGraphEditor::CNodeGraphViewModel& viewModel)
...
{
	DrxGraphEditor::CNodeWidget* pNode = new DrxGraphEditor::CNodeWidget(*this, view);
	...
	pNode->AddHeaderIcon(new CEmitterActiveIcon(*pNode), DrxGraphEditor::CNodeHeader::EIconSlot::Right);
	pNode->AddHeaderIcon(new CEmitterVisibleIcon(*pNode), DrxGraphEditor::CNodeHeader::EIconSlot::Right);
	pNode->AddHeaderIcon(new CSoloEmitterModeIcon(*pNode), DrxGraphEditor::CNodeHeader::EIconSlot::Right);
...
void CNodeItem::SetName(const QString& name)
{
	...
	if (oldName != newName)
	{
		...
		if (GetIEditor()->GetIUndoManager()->IsUndoRecording())
		{
			CUndo::Record(new DrxGraphEditor::CUndoNodeNameChange(*this, oldName.c_str()));
		}
		// ~TODO
	}
}
...
void CNodeItem::Serialize(Serialization::IArchive& archive)
{
	...
	u32k numFeatures = m_component.GetNumFeatures();
	...
	DRX_ASSERT(m_features.size() == movedFeatures);

	m_features.swap(features);
	m_pins.swap(pins);

	DRX_ASSERT(m_features.size() == numFeatures);
...
u32 CNodeItem::GetIndex() const
{
	...
	u32k numNodes = effect.GetNumComponents();
...
CFeatureItem* CNodeItem::AddFeature(u32 index, const pfx2::SParticleFeatureParams& featureParams)
{
...
void CNodeItem::RemoveFeature(u32 index)
{
	if (m_features.size() > index)
	{
		...
		if (CFeaturePinItem* pFeaturePinItem = pFeatureItem->GetPinItem())
		{
			...
			{
				CParticleGraphModel& model = static_cast<CParticleGraphModel&>(GetViewModel());
				for (DrxGraphEditor::CAbstractConnectionItem* pConnectionItem : pFeaturePinItem->GetConnectionItems())
				{
...
bool CNodeItem::MoveFeatureAtIndex(u32 featureIndex, u32 destIndex)
{
	u32k maxIndex = m_features.size() - 1;
...
bool CFeaturePinItem::CanConnect(const DrxGraphEditor::CAbstractPinItem* pOtherPin) const
{
	if (pOtherPin)
...
CFeatureItem::CFeatureItem(pfx2::IParticleFeature& feature, CNodeItem& node, DrxGraphEditor::CNodeGraphViewModel& viewModel)
...
u32 CFeatureItem::GetIndex() const
{
	pfx2::IParticleComponent& component = m_node.GetComponentInterface();

	for (u32 i = 0; i < component.GetNumFeatures(); ++i)
	{
	...
	DRX_ASSERT_MESSAGE(0, "Invalid feature item!");
	return ~0;
}
...
CConnectionItem::CConnectionItem(CBasePinItem& sourcePin, CBasePinItem& targetPin, DrxGraphEditor::CNodeGraphViewModel& viewModel)
