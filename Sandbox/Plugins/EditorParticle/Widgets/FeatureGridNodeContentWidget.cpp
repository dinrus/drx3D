...
CFeatureWidget::CFeatureWidget(CFeatureItem& feature, CFeatureGridNodeContentWidget& contentWidget, DrxGraphEditor::CNodeGraphView& view)
	...
	, m_contentWidget(contentWidget)
{
	...
	QGraphicsWidget* pIsEnabledWidget = DrxGraphEditor::AddWidget(view.scene(), m_pEnabledCheckbox);
	...
	//m_pTitle->setFont(DrxGraphEditor::CNodeStyle::GetContentTextFont());
	m_pTitle->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);

	QGraphicsWidget* pTitleWidget = DrxGraphEditor::AddWidget(view.scene(), m_pTitle);
	...
	{
		m_pPin = new DrxGraphEditor::CPinWidget(*pPinItem, contentWidget.GetNode(), view, false);
...
void CFeatureWidget::DeleteLater()
{
	...
	DrxGraphEditor::CAbstractNodeItem& nodeItem = m_contentWidget.GetNode().GetItem();
	nodeItem.SignalDeactivatedChanged.DisconnectObject(this);

	CNodeGraphViewGraphicsWidget::DeleteLater();
}

DrxGraphEditor::CAbstractNodeGraphViewModelItem* CFeatureWidget::GetAbstractItem() const
{
	return static_cast<DrxGraphEditor::CAbstractNodeGraphViewModelItem*>(&m_feature);
}
...
void CFeatureWidget::hoverEnterEvent(QGraphicsSceneHoverEvent* pEvent)
{
	DrxGraphEditor::SMouseInputEventArgs mouseArgs = DrxGraphEditor::SMouseInputEventArgs(
	  DrxGraphEditor::EMouseEventReason::HoverEnter,
...
{
	DrxGraphEditor::SMouseInputEventArgs mouseArgs = DrxGraphEditor::SMouseInputEventArgs(
	  DrxGraphEditor::EMouseEventReason::HoverMove,
	  Qt::MouseButton::NoButton, Qt::MouseButton::NoButton, pEvent->modifiers(),
	  pEvent->pos(), pEvent->scenePos(), pEvent->screenPos(),
	  pEvent->lastPos(), pEvent->lastScenePos(), pEvent->lastScreenPos());
...
{
	DrxGraphEditor::SMouseInputEventArgs mouseArgs = DrxGraphEditor::SMouseInputEventArgs(
	  DrxGraphEditor::EMouseEventReason::ButtonPress,
...
{
	DrxGraphEditor::SMouseInputEventArgs mouseArgs = DrxGraphEditor::SMouseInputEventArgs(
	  DrxGraphEditor::EMouseEventReason::ButtonRelease,
	  pEvent->button(), pEvent->buttons(), pEvent->modifiers(),
	  pEvent->pos(), pEvent->scenePos(), pEvent->screenPos(),
	  pEvent->lastPos(), pEvent->lastScenePos(), pEvent->lastScreenPos());
...
{
	DrxGraphEditor::SMouseInputEventArgs mouseArgs = DrxGraphEditor::SMouseInputEventArgs(
	  DrxGraphEditor::EMouseEventReason::ButtonPressAndMove,
	  pEvent->button(), pEvent->buttons(), pEvent->modifiers(),
	  pEvent->pos(), pEvent->scenePos(), pEvent->screenPos(),
	  pEvent->lastPos(), pEvent->lastScenePos(), pEvent->lastScreenPos());
...
DrxGraphEditor::CNodeWidget& CFeatureWidget::GetNodeWidget() const
{
...
CFeatureGridNodeContentWidget::CFeatureGridNodeContentWidget(DrxGraphEditor::CNodeWidget& node, CParentPinItem& parentPin, CChildPinItem& childPin, DrxGraphEditor::CNodeGraphView& view)
	...
	, m_grabbedFeatureIndex(~0)
{
	...
	m_pParentPin = new DrxGraphEditor::CPinWidget(parentPin, node, view);
	m_pins.push_back(m_pParentPin);
	pLayout->addItem(m_pParentPin);
	pLayout->setAlignment(m_pParentPin, Qt::AlignVCenter);

	m_pChildPin = new DrxGraphEditor::CPinWidget(childPin, node, view);
	...
	DRX_ASSERT(node.GetItem().Cast<CNodeItem>());
...
void CFeatureGridNodeContentWidget::BeginFeatureMove(CFeatureWidget& featureWidget)
{
	...
	for (auto& pair : m_featureWidgetsByItems)
	{
		...
		u32k index = pFeatureItem->GetIndex();
...
void CFeatureGridNodeContentWidget::MoveFeature(const QPointF& delta)
{
	if (delta.y() != 0.0)
	{
		...
		u32k orgFeatureIndex = m_pGrabbedFeature->GetItem().GetIndex();

		u32k numFeatures = m_floatingFeatures.size();
		for (u32 i = 0; i < numFeatures; ++i)
		{
			...
			if (targetSlot.geometry().intersects(featureGeom))
			{
				...
				{
					u32k targetIndex = targetSlot.GetIndex();
					u32k numSlots = m_featureSlots.size();
					for (u32 s = 0, f = 0; s < numSlots; ++s)
					{
						if (s != targetIndex)
						{
...
u32 CFeatureGridNodeContentWidget::EndFeatureMove()
{
	for (CFeatureWidget* pFeatureWidget : m_floatingFeatures)
	{
		u32k index = pFeatureWidget->GetItem().GetIndex();
	...
	u32k destIndex = m_grabbedFeatureIndex;
	u32k itemIndex = m_pGrabbedFeature->GetItem().GetIndex();
...
void CFeatureGridNodeContentWidget::OnFeatureAdded(CFeatureItem& feature)
{
	...
	if (DrxGraphEditor::CPinWidget* pPinWidget = pFeatureWidget->GetPinWidget())
...
void CFeatureGridNodeContentWidget::OnFeatureMoved(CFeatureItem& feature)
{
	...
	{
		CFeatureWidget* pFeatureWidget = result->second;
		u32k destIndex = feature.GetIndex();
