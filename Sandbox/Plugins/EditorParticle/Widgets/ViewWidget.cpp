...
CItemProperties::CItemProperties(DrxGraphEditor::GraphItemSet& selectedItems)
	...
	, m_isPushingUndo(true)
{
	...
	std::map<u32 /* name crc */, SNode> nodesByNameCrc;
	for (DrxGraphEditor::CAbstractNodeGraphViewModelItem* pAbstractItem : selectedItems)
	{
		if (CNodeItem* pNodeItem = pAbstractItem->Cast<CNodeItem>())
		{
			u32k nameCrc = CCrc32::Compute(pNodeItem->GetComponentInterface().GetName());
		...
		else if (CFeatureItem* pFeatureItem = pAbstractItem->Cast<CFeatureItem>())
		{
			...
			u32k nameCrc = CCrc32::Compute(nodeItem.GetComponentInterface().GetName());
...
{
	m_isPushingUndo = true;
	DrxGraphEditor::CUndoNodePropertiesChange* pUndoObject = new DrxGraphEditor::CUndoNodePropertiesChange(*m_pNodeItem);
...
{
	CFeatureWidget* pFeatureWidget = DrxGraphEditor::CNodeGraphViewGraphicsWidget::Cast<CFeatureWidget>(pSender);
	if (pFeatureWidget == nullptr)
		return;

	switch (args.GetReason())
	{
	case DrxGraphEditor::EMouseEventReason::ButtonPress:
		{
		...
		break;
	case DrxGraphEditor::EMouseEventReason::ButtonRelease:
		{
			...
			if (args.GetButton() == Qt::MouseButton::LeftButton)
			{
				...
				{
					DRX_ASSERT(m_pMovingFeatureWidget);
					if (m_pMovingFeatureWidget)
					{

						i32k destIndex = pFeatureWidget->GetContentWidget().EndFeatureMove();
		...
		break;
	case DrxGraphEditor::EMouseEventReason::ButtonPressAndMove:
		{
		...
		break;
	case DrxGraphEditor::EMouseEventReason::HoverMove:
		{
			pFeatureWidget->SetHighlighted(true);
		}
		break;
	case DrxGraphEditor::EMouseEventReason::HoverLeave:
		{
...
bool CGraphView::MoveFeatureToIndex(CFeatureWidget& featureWidget, u32 destIndex)
{
	CFeatureItem& moveFeatureItem = featureWidget.GetItem();
	CNodeItem& nodeItem = moveFeatureItem.GetNodeItem();

	u32k itemIndex = moveFeatureItem.GetIndex();
...
bool CGraphView::PopulateMenuWithFeatures(tukk szTitle, DrxGraphEditor::CAbstractNodeItem& node, QMenu& menu, u32 index)
{
	...
	u32k numParams = GetParticleSystem()->GetNumFeatureParams();
...
bool CGraphView::DeleteCustomItem(DrxGraphEditor::CAbstractNodeGraphViewModelItem& item)
{
...
void CGraphView::OnRemoveCustomItem(DrxGraphEditor::CAbstractNodeGraphViewModelItem& item)
{
	DeselectItem(item);
}
