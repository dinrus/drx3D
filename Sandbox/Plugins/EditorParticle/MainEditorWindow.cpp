...
#define APPLICATION_USER_DIRECTORY      "/DrxEngine"
#define APPLICATION_USER_STATE_FILENAME "EditorState.json"

namespace DrxParticleEditor {

static tukk const s_szEffectAssetName = "Effect";
static tukk const s_szCurveEditorPanelName = "Curve Editor";
static tukk const s_szInspectorName = "Inspector";
...
void CParticleEditor::InitMenu()
{
...

	{
		...
		m_pReloadEffectMenuAction = pFileMenu->CreateAction(DrxIcon("icons:General/Reload.ico"), tr("Reload Effect"), sec);
		...
		m_pShowEffectOptionsMenuAction = pFileMenu->CreateAction(DrxIcon("icons:General/Options.ico"), tr("Show Effect Options"), sec);
		...
		auto action = pFileMenu->CreateAction(DrxIcon("icons:General/Get_From_Selection.ico"), tr("Load from Selected Entity"), sec);
		connect(action, &QAction::triggered, this, &CParticleEditor::OnLoadFromSelectedEntity);

		action = pFileMenu->CreateAction(DrxIcon("icons:General/Assign_To_Selection.ico"), tr("Apply to Selected Entity"), sec);
		connect(action, &QAction::triggered, this, &CParticleEditor::OnApplyToSelectedEntity);

		sec = pFileMenu->GetNextEmptySection();

		action = pFileMenu->CreateAction(DrxIcon("icons:General/File_Import.ico"), tr("Import Selected Pfx1 Effect"), sec);
...
#define ADD_BUTTON(slot, name, shortcut, icon)                           \
  { QAction* pAction = pToolBar->addAction(DrxIcon(icon), QString());    \
...
bool LoadFile(std::vector<char>& content, tukk filename)
{
	FILE* f = gEnv->pDrxPak->FOpen(filename, "r");
	if (!f)
		return false;

	const size_t size = gEnv->pDrxPak->FGetSize(f);
...
void CParticleEditor::AssignToEntity(CBaseObject* pObject, const string& newAssetName)
{
	...
	{
		Drx::DefaultComponents::CParticleComponent* pParticleComponent = pEntity->GetOrCreateComponent<Drx::DefaultComponents::CParticleComponent>();
...
{
	DRX_ASSERT(pOutputName != nullptr);
...
void CParticleEditor::SaveEffect(CEditableAsset& editAsset)
{
	...
	DRX_ASSERT(pEffectAsset->GetAsset() && pEffectAsset->GetAsset()->GetFilesCount());
	...
	// Get effect dependency
	{
		...
		for (size_t component = 0, componentsCount = pEffect->GetNumComponents(); component < componentsCount; ++component)
		{
			...
			for (size_t feature = 0, featuresCount = pComponent->GetNumFeatures(); feature < featuresCount; ++feature)
			{
				...
				{
					tukk szResourceFilename = pFeature->GetResourceName(resource);
...
void CParticleEditor::OnReloadEffect()
{
	...
	DRX_ASSERT(pAsset && pAsset->GetFilesCount());
	...
	if (particleMgr)
	{
		if (CParticleItem* pSelectedItem = static_cast<CParticleItem*>(particleMgr->GetSelectedItem()))
		{
			if (pfx2::PParticleEffect pNewEffect = GetParticleSystem()->ConvertEffect(pSelectedItem->GetEffect(), true))
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Particle asset '%s' already exists.", assetFilePath.c_str());
				}

				const CParticlesType* const pParticlesType = (const CParticlesType*)CAssetManager::GetInstance()->FindAssetType("Particles");
				DRX_ASSERT(!strcmp(CParticlesType().GetTypeName(), "Particles"));
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "No particle selected for import in database view");
		}
	}
}
...
void CParticleEditor::OnLoadFromSelectedEntity()
{
	...
	for (i32 objectIndex = 0, numObjects = pGroup->GetCount(); objectIndex < numObjects; ++objectIndex)
	{
		...
		if (IEntity* pEntity = pEntityObject->GetIEntity())
		{
			for (i32 slotIndex = 0, n = pEntity->GetSlotCount(); slotIndex < n; ++slotIndex)
			{
