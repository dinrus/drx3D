...
{
	for (i32 i = 0; i < FbxMetaData::Units::eUnitSetting_COUNT; ++i)
	{
...
{
	return FbxTool::Axes::EAxis(((i32)axis + 1) % (i32)FbxTool::Axes::EAxis::COUNT);
}
...
void CGlobalImportSettings::SConversionSettings::Serialize(yasli::Archive& ar)
{
	...
	FbxMetaData::Units::ForallNames([&options](tukk szUnitName)
	{
		options.push_back(szUnitName);
	});
	options[0] += string(" (") + QtUtil::ToString(fileUnitSizeInCm) + string(")");

	Serialization::StringListValue dropDown(options, (i32)unit);
