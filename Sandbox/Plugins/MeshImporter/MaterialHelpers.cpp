...
void AutoAssignSubMaterialIDs(const std::vector<std::pair<i32, QString>>& knownMaterials, FbxTool::CScene* pScene)
{
	...
	std::vector<i32> takenIndices(numMaterials, 0);
...
void WriteMaterialMetaData(const FbxTool::CScene* pScene, std::vector<FbxMetaData::SMaterialMetaData>& materialMetaData)
{
	...
	for (i32 i = 0; i < pScene->GetMaterialCount(); ++i)
	{
		const FbxTool::SMaterial* const pMaterial = pScene->GetMaterialByIndex(i);
		assert(pMaterial);

		i32k subMaterial = pScene->GetMaterialSubMaterialIndex(pMaterial);
...
std::vector<std::pair<i32, QString>> GetSubMaterialList(const string& materialFilePath)
{
	...
	std::vector<std::pair<i32, QString>> loadedMaterialNames;
	...
	if (!materialFilePath.empty())
	{
		...
		{
			i32k numChildren = rootNode->getChildCount();
			for (i32 i = 0; i < numChildren; ++i)
			{
				tukk const szTag = rootNode->getChild(i)->getTag();
				if (strcmp(szTag, "SubMaterials") == 0)
				{
					i32k numSubmaterials = rootNode->getChild(i)->getChildCount();
					for (i32 j = 0; j < numSubmaterials; ++j)
					{
						XmlNodeRef subMaterialNode = rootNode->getChild(i)->getChild(j);
						tukk szName = subMaterialNode->getAttr("Name");
...
{	
	i32k sourceMaterialCount = pFbxScene->GetMaterialCount();
	...
	if (maxSubMatId > 0)
	{
		std::vector<i32> initSubMats;
		i32k targetMaterialCount = maxSubMatId + 1;
		initSubMats.resize(targetMaterialCount, 0);

		pParentMaterial->SetSubMaterialCount(targetMaterialCount);
		for (i32 i = 0; i < sourceMaterialCount; ++i)
		{
			FbxTool::CScene::ConstMaterialHandle pMaterial = pFbxScene->GetMaterialByIndex(i);
			i32k subMatId = pFbxScene->GetMaterialSubMaterialIndex(pMaterial);
