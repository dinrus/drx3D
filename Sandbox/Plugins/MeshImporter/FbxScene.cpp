...
static tukk const kDefaultNodeName = "<anonymous>";
static tukk const kDefaultMaterialName = "Default";
static tukk const kDefaultMappingName = "<anonymous>";

tukk MaterialPhysicalizeSettingToString(EMaterialPhysicalizeSetting ps)
{
	switch(ps)
	{
	...
	default:
		DRX_ASSERT(0 && "unkown material physicalize setting");
		return nullptr;
	}
}
...
static bool OnProgress(uk pArgs, float percentage, tukk pStatus)
{
	...
	{
		pContext->pCallbacks->OnProgressPercentage(i32(percentage * kImportPercentage + kInitializePercentage * 100.0f));
...
static void PostProcess(FbxScene* pScene, const SFileImportDescriptor& desc)
{
	...
	if (desc.bTriangulate)
	{
		if (desc.pCallbacks)
		{
			...
			desc.pCallbacks->OnProgressPercentage(i32(currentProgress * 100.0f));
		}
		...
		if (!toTriangulate.empty())
		{
			...
			{
				tukk szGeometryName = pGeometry->GetName();
	...
	if (desc.generateNormals != EGenerateNormals::Never)
	{
		...
		auto generator = [&toGenerate, bOverwrite](FbxMesh* pMesh, FbxNode* pNode)
		{
			if (!bOverwrite)
			{
				i32k numLayers = pMesh->GetLayerCount();
...
static tukk ToString(Axes::EAxis axis)
{
	switch (axis)
	{
...
static i32 GetSign(Axes::EAxis axis)
{
  ...
  Axes::EAxis& fwd, Axes::EAxis& up)
{
	...
	i32k upAxis = static_cast<i32>(fbxUpAxis) - 1;
	i32k parity = static_cast<i32>(fbxFwdAxis) - 1;
...
std::unique_ptr<CScene> CScene::ImportFileInternal(SFileImportDescriptor desc)
{
	...
	if (!pImporter->Initialize(desc.filePath.c_str(), -1, pIOSettings.get()))
	{
		...
		{
			tukk szPassword = desc.pCallbacks->OnPasswordRequired();
			if (szPassword)
			{
...
i32 CScene::GetMaterialCount() const
{
	const size_t numMats = GetMaterials().size();
	assert(numMats < INT_MAX);
	return static_cast<i32>(numMats);
}

i32 CScene::GetMeshCount() const
{
	const size_t numMeshes = GetMeshes().size();
	assert(numMeshes < INT_MAX);
	return static_cast<i32>(numMeshes);
}

i32 CScene::GetAnimationTakeCount() const
{
	return (i32)m_animationTakes.size();
}
...
i32 CScene::GetNodeCount() const
{
	return static_cast<i32>(m_nodes.size());
}
...
static bool GetFbxPropertyNamesFromChannelType(EMaterialChannelType channelType, std::vector<string>& properties)
{
	...
	case eMaterialChannelType_Specular:
		...
		return true;
	default:
		DRX_ASSERT(0 && "unknown channel type");
		return false;
	}
}
...
static void CreateMaterial(SMaterial* pResult, FbxSurfaceMaterial* pFbxMaterial)
{
	...
	for (i32 i = 0; i < eMaterialChannelType_COUNT; ++i)
	{
		...
		for (auto& propName : propertyNames)
		{
			...
			for (i32 j = 0; j < prop.GetSrcObjectCount(); ++j)
			{
			...
			for (i32 j = 0; j < prop.GetSrcObjectCount(); ++j)
			{
				...
				for (i32 srcObj = 0; srcObj < pFbxLayeredTexture->GetSrcObjectCount(); ++srcObj)
				{
...
static void CreateAllMaterials(FbxScene* pScene, std::vector<std::unique_ptr<SMaterial>>& materials)
{
	...
	for (i32 i = 0; i < sceneMaterials.Size(); ++i)
	{
		FbxSurfaceMaterial* pFbxMaterial = sceneMaterials[i];
		tukk const szName = pFbxMaterial->GetName();
...
static tukk GetNameOfAttribute(const FbxNodeAttribute* pAtt)
{
...
void CScene::InitializeNode(SNode* pNode, FbxNode* pFbxNode)
{
	...
	// Check attributes.
	for (i32 i = 0; i < pFbxNode->GetNodeAttributeCount(); ++i)
	{
...
void CScene::InitializeAnimations()
{
	...
	for (i32 i = 0; i < animStackNameArray.Size(); ++i)
	{
		...
		animTake.startFrame = (i32)start.GetFrameCount();
		animTake.endFrame = (i32)stop.GetFrameCount();
	...
	if (m_animationTakes.empty())
	{
		...
		defaultTake.name = "";
		defaultTake.startFrame = (i32)timeLineTimeSpan.GetStart().GetFrameCount();
		defaultTake.endFrame = (i32)timeLineTimeSpan.GetStop().GetFrameCount();
...
SNode* CScene::Initialize(const SFileImportDescriptor& desc)
{
	...
	i32k nodeCount = m_pScene->GetNodeCount();
	...
	while (!nodeStack.empty())
	{
		...
		for (i32 i = 0; i < pFbxNode->GetChildCount(); ++i)
		{
	...
	for (auto& pNodeIt : m_nodes)
	{
		...
		desc.pCallbacks->OnProgressPercentage((i32)progress);
	}
	...
	for (auto& meshIt : m_meshes)
	{
		...
		tukk szError = CreateEngineMesh(*pFbxMesh, displayMeshes, pFbxNode, m_materials, &stats.createEngineMesh);
		if (szError)
		{
...
AABB SMesh::ComputeAabb(const Matrix34& transform) const
{
	...
	for (const auto& displayMesh : displayMeshes)
	{
		...
		i32k faceCount = pMesh->GetFaceCount();
...
bool CScene::InitializeMesh(ICallbacks* pCallbacks, SMesh* pMesh, FbxNode* pFbxNode, SNode* pNode, FbxMesh* pFbxMesh, SCreateMeshStats* pStats)
{
	...
	i32k numMaterials = pFbxNode->GetMaterialCount();
	...
	if (pMaterials)
	{
		const FbxLayerElementArrayTemplate<i32>& materialIndices = pMaterials->GetIndexArray();
		i32k numMaterialIndices = materialIndices.GetCount();

		for (i32 i = 0; i < numMaterialIndices; ++i)
		{
			i32k materialIndex = materialIndices[i];
...
void CScene::SetNodeLod(const SNode* pNode, i32 lod)
{
	assert(pNode);
...
SMaterial* CScene::GetMaterialByName_Internal(tukk szName)
{
  ...
  std::vector<string>::const_iterator pathEnd)
{
	...
	for (i32 i = 0; i < pParent->numChildren; ++i)
	{
...
static tukk GetFormattedAxis(FbxTool::Axes::EAxis axis)
{
	switch (axis)
	{
