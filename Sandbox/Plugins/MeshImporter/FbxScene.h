...
struct SMaterial
{
	...
	static tukk DummyMaterialName() { return "<unassigned>"; }

	i32                id; // Uniquely identifies this material. In range [0, number of materials).

	tukk        szName;                  // Name of the material

	i32                numMeshesUsingIt;
...
struct SNode
{
	i32 id;    // Uniquely identifies this node. In range [0, number of nodes).
...
struct ICallbacks
{
	virtual void OnProgressMessage(tukk szMessage) = 0;
	virtual void OnProgressPercentage(i32 percentage) = 0;
	virtual void OnWarning(tukk szMessage) = 0;
	virtual void OnError(tukk szMessage) = 0;

	// In case the FBX file requires a password, this function will be called to retrieve it
	virtual tukk OnPasswordRequired()
	{
		return nullptr;
	}
};
...
public:
	...
	// The function will return nullptr if the file can't be loaded.
	static std::unique_ptr<CScene> ImportFile(tukk szPath)
	{
	...
	i32 GetAnimationTakeCount() const;

	const SNode* GetNodeByIndex(i32 index) const
	{
		return m_nodes[index].get();
	}

	const SAnimationTake* GetAnimationTakeByIndex(i32 index) const
	{
		return &m_animationTakes[index];
	}

	const SMaterial* GetMaterialByIndex(i32 index) const
	{
		assert(0 <= index && index < GetMaterialCount());
		return m_materials[index].get();
	}

	const SMaterial* GetMaterialByName(tukk szName) const
	{
	...
	i32 GetNodeLod(const SNode* pNode) const
	{
		assert(pNode);
		return m_nodeUserData[pNode->id].lod;
	}

	void SetNodeLod(const SNode* pNode, i32 lod);
	...
	void SetMaterialSubMaterialIndex(const SMaterial* pMaterial, i32 index)
	{
		assert(pMaterial);
	...
	i32 GetMaterialSubMaterialIndex(const SMaterial* pMaterial) const
	{
		assert(pMaterial);
...
private:
	SMaterial*       GetMaterialByName_Internal(tukk name);
private:
	SNode* NewNode();
	SMesh* NewMesh();
