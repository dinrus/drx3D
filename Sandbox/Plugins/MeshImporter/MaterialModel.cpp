...
#include <Drx3DEngine/CGF/DrxHeaders.h>  // MAX_SUB_MATERIALS
...
CMaterialModel::CMaterialModel()
	: m_pScene(nullptr)
{
	m_random.Seed((u32)time(nullptr));
}
...
void CMaterialModel::SetScene(FbxTool::CScene* pScene)
{
	...
	i32k numMats = pScene->GetMaterialCount();
	const auto& materials = pScene->GetMaterials();

	for (i32 i = 0; i < numMats; ++i)
	{
...
void CMaterialModel::ClearScene()
{
	...
	i32k lastRow = static_cast<i32>(m_materials.size() - 1);
...
QString CMaterialModel::GetSubMaterialName(i32 index) const
{
	QString name;
...
i32 CMaterialModel::rowCount(const QModelIndex& index) const
{
	return (i32)m_materials.size();
}

i32 CMaterialModel::columnCount(const QModelIndex& index) const
{
	return eColumnType_Count;
}

QVariant CMaterialModel::data(const QModelIndex& index, i32 role) const
{
	...
	case eColumnType_Physicalize:
		...
		{
			return (i32)Convert(AtIndex(index)->GetPhysicalizeSetting());
		}
		break;
...
QVariant CMaterialModel::headerData(i32 column, Qt::Orientation orientation, i32 role) const
{
	switch (column)
	{
	case eColumnType_Name:
...
bool CMaterialModel::setData(const QModelIndex& modelIndex, const QVariant& variant, i32 role)
{
	...
	{
		i32k editorIndex = variant.value<i32>();
		DRX_ASSERT(editorIndex >= 0);
		if (data(modelIndex, role).value<i32>() == editorIndex)
		{
			// Nothing changed.
			return false;
		}
	...
	{
		const auto physicalizeSetting = FbxTool::EMaterialPhysicalizeSetting(variant.value<i32>());
...
QVariant CMaterialModel::GetToolTipForColumn(i32 column)
{
	switch (column)
	{
	case eColumnType_Name:
...
QString CSortedMaterialModel::GetSubMaterialName(i32 index) const
{
