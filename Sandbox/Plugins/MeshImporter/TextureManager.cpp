...
i32 CTextureManager::GetTextureCount() const
{
	return m_textures.size();
}

CTextureManager::TextureHandle CTextureManager::GetTextureFromIndex(i32 index)
{
	DRX_ASSERT(index >= 0 && index < m_textures.size());
...
{
	for (i32 i = 0; i < GetTextureCount(); ++i)
	{
...
static tukk GetPresetFromChannelType(FbxTool::EMaterialChannelType channelType)
{
...
void CTextureManager::Init(const MeshImporter::CSceneManager& sceneManager)
{
	...
	for (i32 i = 0; i < pScene->GetMaterialCount(); ++i)
	{
		...
		for (i32 j = 0; j < FbxTool::eMaterialChannelType_COUNT; ++j)
		{
			...
			{
				string rcArguments;
				tukk szPreset = GetPresetFromChannelType(FbxTool::EMaterialChannelType(j));
				if (szPreset)
				{
