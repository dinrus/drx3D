...
#include <../../DrxEngine/Drx3DEngine/CGF/ChunkFile.h>
#include <../../DrxEngine/Drx3DEngine/MeshCompiler/MeshCompiler.h>
#include <../../DrxEngine/Drx3DEngine/MeshCompiler/TransformHelpers.h>
#include <Drx3DEngine/CGF/CGFContent.h>  // CGF_NODE_NAME_LOD_PREFIX
...
static tukk const kDefaultProxyMaterial = "%EDITOR%/Materials/areasolid";
...
void CMainDialog::OnViewportRender(const SRenderContext& rc)
{
	...
	// Update global scene rotation.
	{
		...
		tukk const errMsg = TransformHelpers::ComputeForwardUpAxesTransform(
...
public:
	...
	{
		i32k currentIndex = m_pShowMeshesMode->GetCheckedItem();
		DRX_ASSERT(currentIndex >= 0);  // Is single-selection?
...
void CMainDialog::MarkVisibleNodes()
{
	...
	i32k showMeshesMode = m_pShowMeshesModeWidget->GetShowMeshesMode();
	  ...
	  showMeshesMode == CShowMeshesModeWidget::eShowMeshesMode_SelectedNode)
	{
		...
		for (i32 i = 0; i < selectedIndices.size(); ++i)
		{
	...
	while (!elementStack.empty())
	{
		...
		{
			for (i32 j = 0; j < pNode->numChildren; ++j)
			{
...
void CMainDialog::RenderStaticMesh(const SRenderContext& rc)
{
	...
	for (size_t i = 0; i < m_pSceneModel->GetElementCount(); ++i)
	{
		...
		if (pRenderObj)
		{
			i32k flags = pRenderObj->GetFlags();
...
void CMainDialog::RenderSourceView(const SRenderContext& rc)
{
	...
	i32k nMaterials = pScene->GetMaterialCount();
	for (i32 i = 0; i < nMaterials; ++i)
	{
		tukk const szName = pScene->GetMaterials()[i]->szName;
		...
		i32k uberMaterialIdx = i / MAX_SUB_MATERIALS;
		i32k subMaterialIdx = i % MAX_SUB_MATERIALS;
	...
	std::vector<i32> visited(statObjs.size(), 0);
...
static QString PromptForImportFbxFile(QWidget* pParent, const QString& initialFile)
{
	...
	tukk const* const ppExtensions = FbxTool::GetSupportedFileExtensions(numExtensions);
...
bool ReadMetaDataFromFile(const QString& filePath, FbxMetaData::SMetaData& metaData)
{
	...
	if (cf.Read(QtUtil::ToString(filePath).c_str()))
	{
		...
		if (pChunk)
		{
			const string json((tukk)pChunk->data, (size_t)pChunk->size);
...
std::unique_ptr<CModelProperties::SSerializer> CreateSerializer(QAbstractItemModel* pModel, const QModelIndex& modelIndex)
{
	...
	if (data.isValid() && data.canConvert<uk>())
	{
		std::unique_ptr<Serialization::SStruct> sstruct((Serialization::SStruct*)data.value<uk>());
...
static bool FinalizeMesh(CMesh* pMesh, tukk & szError)
{
	...
	for (size_t i = 0; i < pMesh->m_subsets.size(); ++i)
	{
		...
		for (i32 j = subset.nFirstVertId; j < subset.nFirstVertId + subset.nNumVerts; ++j)
		{
...
void CMainDialog::AssignScene(const MeshImporter::SImportScenePayload* pPayload)
{
	...
	*m_pGlobalImportSettings = CGlobalImportSettings();
	...
	{
		IMaterialManager* const pMaterialManager = GetIEditor()->Get3DEngine()->GetMaterialManager();
		tukk const szDefaultMaterial = "%ENGINE%/EngineAssets/TextureMsg/DefaultSolids";
		TSmartPtr<IMaterial> const pReferenceMaterial = pMaterialManager->LoadMaterial(szDefaultMaterial);
		i32k materialCount = pActiveScene->GetMaterialCount();
		...
		i32k nUberMaterials = materialCount / MAX_SUB_MATERIALS + 1;

		m_uberMaterials.resize(nUberMaterials);

		for (i32 i = 0; i < nUberMaterials; ++i)
		{
		...
		for (i32 i = 0; i < materialCount; ++i)
		{
			i32k uberMaterial = i / MAX_SUB_MATERIALS;
			i32k subMaterial = i % MAX_SUB_MATERIALS;
	...
	// for (size_t i = 0; i < m_displayScene->statObjs.size(); ++i)
	concurrency::parallel_for(0, (i32)m_displayScene->statObjs.size(), [this, &pTimer, &tMeshCompiling](i32 i)
	{
	...
	i32k mapSize = pActiveScene->GetNodeCount() + 1;
...
tukk CMainDialog::GetDialogName() const
{
	return "Mesh";
}
...
void CMainDialog::SelectMaterialsOnMesh(const FbxTool::SMesh* pMesh)
{
	...
	for (i32 i = 0; i < pMesh->numMaterials; ++i)
	{
		...
		if (pMaterial)
		{
			...
			i32 numMaterials = pView->model()->rowCount(QModelIndex());
			for (i32 j = 0; j < numMaterials; ++j)
			{
...
void CMainDialog::SelectMeshesUsingMaterial(const FbxTool::SMaterial* pNeedle, const CSceneElementCommon* pHaystack)
{
	...
	i32k numChildren = pHaystack->GetNumChildren();
	for (i32 i = 0; i < numChildren; ++i)
	{
			...
			if (pSourceNodeElement->GetNode() && pSourceNodeElement->GetNode()->numMeshes > 0)
		{
			...
			i32 numMaterials = pMesh->numMaterials;
			for (i32 j = 0; j < numMaterials; ++j)
			{
...
bool CMainDialog::SaveCgf(const std::shared_ptr<QTemporaryDir>& pTempDir, const string& targetFilePath, const QString& sourceFilename)
	...
	ThreadingUtils::Async([saveState, pProxyTree, absOriginalFilePath, targetFilePath, pTempDir]()
	{
		...
		if (!ret.first)
		{
			...
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Copying source file to '%s' failed: '%s'",
...
bool CMainDialog::NeedAutoLod() const
{
	...
	DRX_ASSERT(!GetAutoLodNodes(GetScene()).empty());
...
{
	string geoName;
	i32    subObjectMeshCount;

	void ReadChunkfileContent(tukk szFilePath);
};

void SChunkFileContent::ReadChunkfileContent(tukk szFilePath)
{
...
static bool IsProxyFromName(tukk szName)
{
	tukk reservedNames[] = { CGF_NODE_NAME_PHYSICS_PROXY0, CGF_NODE_NAME_PHYSICS_PROXY1, CGF_NODE_NAME_PHYSICS_PROXY2 };
	for (tukk szProxyName : reservedNames)
	{
...
{
	i32k lodIndex = pScene->GetNodeLod(pNode);
	const bool isProxy = pScene->IsProxy(pNode);

	if (isProxy && (lodIndex > 0))
{
		DRX_ASSERT(0 && "'Physics Proxy' and 'LOD[1..n]' are mutually exclusive properties of node.");
}
...
void WriteNodeMetaData(FbxMetaData::SMetaData& metaData, const FbxTool::CScene* pScene)
{
	...
	for (i32 i = 0; i < pRoot->numChildren; ++i)
	{
		SStackItem item;
	...
	while (!stack.empty())
	{
		...
		for (i32 i = 0; i < item.pNode->numChildren; ++i)
		{
...
bool CMainDialog::CreateMetaData(FbxMetaData::SMetaData& metaData, const QString& sourceFilename, i32 flags)
{
	using namespace Private_MainDialog;
	...
	DRX_ASSERT(m_pGlobalImportSettings);
	...
	i32k numMaterials = pScene->GetMaterialCount();
  ...
  std::vector<string>::const_iterator pathEnd)
{
	...
	for (i32 i = 0; i < pParent->GetNumChildren(); ++i)
	{
...
void CMainDialog::ApplyMetaDataCommon(const FbxMetaData::SMetaData& metaData)
{
	...
	DRX_ASSERT(metaData.pEditorMetaData);
	if (metaData.pEditorMetaData)
	{
...
{
	DRX_ASSERT(metaData.nodeData.size() == 1);
...
void CMainDialog::CreateSkinMetaData(FbxMetaData::SMetaData& metaData)
{
	...
	DRX_ASSERT(m_pSceneUserData && m_pSceneUserData->GetSelectedSkin());
