...
static i32 s_idTaskProgressEvent = 0;
static i32 s_idTaskNotifyEvent = 0;
static i32 s_idTaskFinishedEvent = 0;
...
void CProgressNotificationStack::Remove(SNotification* pNotification)
{
	...
	DRX_ASSERT(it != m_notifications.end());
...
struct STaskNotifyEvent : public QEvent
{
	...
	STaskNotifyEvent(const CAsyncTaskBase* pTask, const QString& what, i32 type)
...
void CAsyncTaskHost::SetProgress(CAsyncTaskBase* pTask, const QString& what, i32 progress)
{
	if (m_pParent)
	{
...
void CAsyncTaskBase::UpdateProgress(const QString& what, i32 progress)
{
	assert(m_pHost);
...
void CAsyncTaskHostForDialog::OnTaskProgress(CAsyncTaskBase* pTask, const QString& message, i32 progress)
{
