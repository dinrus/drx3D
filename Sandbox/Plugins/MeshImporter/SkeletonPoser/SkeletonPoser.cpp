...
void ComputeAbsolutJointTransformationsRecursive(i32 joint, const IDefaultSkeleton& skel, IAnimationPoseData* pPoseData)
{
	i32k parent = skel.GetJointParentIDByID(joint);
	if (parent > -1)
	{
		pPoseData->SetJointAbsolute(joint, pPoseData->GetJointAbsolute(parent) * pPoseData->GetJointRelative(joint));
	}

	for (i32 i = 0, N = skel.GetJointChildrenCountByID(joint); i < N; ++i)
	{
		ComputeAbsolutJointTransformationsRecursive(skel.GetJointChildIDAtIndexByID(joint, i), skel, pPoseData);
	}
}

void ComputeAbsolutJointTransformations(const std::vector<i32>& dirtyRoots, const IDefaultSkeleton& skel, IAnimationPoseData* pPoseData)
{
	for (i32 dirtyRoot : dirtyRoots)
	{
...
bool CSkeletonPoseModifier::Execute(const SAnimationPoseModifierParams& params)
{
	...
	i32k jointCount = skel.GetJointCount();
	...
	for (auto& joint : validPosedJoints)
	{
		...
		i32 dirtyRoot = joint.m_id;
		i32 pathIt = skel.GetJointParentIDByID(joint.m_id);
