...
struct CSceneModelCommon::ISceneBuilder
{
	...
	virtual QModelIndex index(const CSceneModelCommon* pModel, i32 row, i32 column, const QModelIndex& parent) const = 0;
	virtual QModelIndex parent(const CSceneModelCommon* pModel, const QModelIndex& index) const = 0;
	virtual i32         rowCount(const CSceneModelCommon* pModel, const QModelIndex& index) const = 0;
};
...
struct CSceneModelCommon::CSceneModelWithPseudoRoot : CSceneModelCommon::ISceneBuilder
{
	...
	virtual QModelIndex index(const CSceneModelCommon* pModel, i32 row, i32 col, const QModelIndex& parentIdx) const override
	{
	...
	virtual i32 rowCount(const CSceneModelCommon* pModel, const QModelIndex& index) const override
	{
		if (pModel->IsSceneEmpty())
		{
			return 0;
...
{
public:
	CItemModelAttributeEnumFuncDynamic(tukk szName, std::function<QStringList()> populateFunc, Visibility visibility = CItemModelAttribute::Visible, bool filterable = true, QVariant defaultFilterValue = QVariant())
...
QVariant CSceneModelCommon::data(const QModelIndex& index, i32 role) const
{
...
QModelIndex CSceneModelCommon::index(i32 row, i32 column, const QModelIndex& parent) const
{
...
i32 CSceneModelCommon::rowCount(const QModelIndex& index) const
{
...
i32 CSceneModelCommon::GetElementCount() const
{
	return (i32)m_pScene->GetElementCount();
}

CSceneElementCommon* CSceneModelCommon::GetElementById(i32 id)
{
