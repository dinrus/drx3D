...
struct SCharacterGenerator : ICharacterGenerator
{
	SCharacterGenerator()
	{
		...
		m_pMaterialGenerator->sigMaterialGenerated.Connect(std::function<void(CMaterial*, uk )>([this](CMaterial* pMat, uk )
		{
			if (pMat)
			{
	...
	void AssignScene(const MeshImporter::SImportScenePayload* pUserData)
	{
		...
		AutoAssignSubMaterialIDs(std::vector<std::pair<i32, QString>>(), m_sceneManager.GetScene());
	...
	bool Import(const string& filePath)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, fileImporter.GetError().c_str());
			return false;
		}
	...
	string UpdateAnim(const string& takeName, i32 startFrame, i32 endFrame)
	{
		FbxMetaData::SMetaData metaData;
	...
	void CreateMetaDataCaf(FbxMetaData::SMetaData& metaData, const string& takeName, i32 startFrame, i32 endFrame) const
	{
		CreateMetaDataCommon(metaData);
	...
	void RcCaller_Assign(CRcInOrderCaller* pCaller, const QString& filePath, uk pUserData)
	{
		using namespace MeshImporter;
