...
static tukk strstri(tukk pString, tukk pSubstring)
{
	i32 i, j, k;
	for (i = 0; pString[i]; i++)
...
static void DrawDirectedConnection(IRenderAuxGeom* pAuxGeom, const Vec3& start, const Vec3& end, u8k opacity = 255)
{
...
static void DrawSkeleton(IRenderAuxGeom* pAuxGeom, IDefaultSkeleton* pDefaultSkeleton, ISkeletonPose* pPose, const QuatT& location, string filter, QuatT cameraFrame)
{
	...
	{
		u8k opacity = (filtered && Private_RenderHelpers::strstri(pDefaultSkeleton->GetJointNameByID(i), filter) == 0) ? 20 : 255;
