	...
	, m_pSceneUserData(nullptr)
{
	...
	m_pExportIcon->addPixmap(enabledIcon, DrxIcon::Normal, DrxIcon::On);
	m_pExportIcon->addPixmap(disabledIcon, DrxIcon::Normal, DrxIcon::Off);
...
static bool GetLodIndex(tukk const szName, tukk szPrefix, i32& lod)
{
	...
	i32k value = atoi(szName + lodNamePrefixLength);
...
{
	for (i32 i = 0; i < scene.GetElementCount(); ++i)
	{
...
void CSceneModel::SetScene(FbxTool::CScene* pScene, const FbxMetaData::SSceneUserSettings& userSettings)
{
	...
	for (i32 i = 0; i < GetElementCount(); ++i)
	{
...
{
	setData(index, (i32)value, eRoleType_Export);
}
...
static void ResetExportSettingsInSubtree(FbxTool::CScene* pScene, const FbxTool::SNode* pFbxNode)
{
	...
	for (i32 i = 0; i < pFbxNode->numChildren; ++i)
	{
...
void CSceneModel::ResetExportSettingsInSubtree(const QModelIndex& index)
{
	...
	DRX_ASSERT(IsSceneRoot(pSourceNodeElement));
...
i32 CSceneModel::columnCount(const QModelIndex& index) const
{
...
QVariant CSceneModel::GetSourceNodeData(CSceneElementSourceNode* pSelf, const QModelIndex& index, i32 role) const
{
...
QVariant CSceneModel::GetSkinData(CSceneElementSkin* pSelf, const QModelIndex& index, i32 role) const
{	
	switch (index.column())
	{
	case eColumnType_Name:
...
QVariant CSceneModel::data(const QModelIndex& index, i32 role) const
{
	if (!index.isValid())
	{
		return QVariant();
...
bool CSceneModel::SetDataSourceNodeElement(const QModelIndex& index, const QVariant& value, i32 role)
{
	...
	{
		GetScene()->SetNodeExportSetting(pNode, static_cast<FbxTool::ENodeExportSetting>(value.value<i32>()));
...
bool CSceneModel::SetDataSkinElement(const QModelIndex& index, const QVariant& value, i32 role)
{
	CSceneElementSkin* const pSkinElement = (CSceneElementSkin*)GetSceneElementFromModelIndex(index);

	DRX_ASSERT(pSkinElement->GetParent() && pSkinElement->GetParent()->GetType() == ESceneElementType::SourceNode);
	CSceneElementSourceNode* const pSourceNodeElement = (CSceneElementSourceNode*)pSkinElement->GetParent();
...
bool CSceneModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
	if (!index.isValid())
	{
		return false;
...
CItemModelAttribute* CSceneModel::GetColumnAttribute(i32 col) const
{
	switch(col)
	{
	case eColumnType_Name:
...
QVariant CSceneModel::headerData(i32 column, Qt::Orientation orientation, i32 role) const
{
	switch (column)
	{
	case eColumnType_Name:
...
QVariant CSceneModel::GetToolTipForColumn(i32 column)
{
	switch (column)
	{
	case eColumnType_Name:
