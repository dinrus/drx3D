...
static void Serialize(yasli::Archive& ar, SJointLimits& value)
{
	...
	char newMask = 0;
	for (i32 i = 0, N = DRX_ARRAY_COUNT(axisLimitNames); i < N; ++i)
	{
...
struct SerializerHelper : yasli::Serializer
{
	SerializerHelper(T& value)
		: yasli::Serializer(yasli::TypeID::get<T>(), static_cast<uk>(&value), sizeof(T), &SerializerHelper::YASLI_SERIALIZE_METHOD)
	{
		// Empty on purpose
	}

	static bool YASLI_SERIALIZE_METHOD(uk pObject, yasli::Archive& ar)
	{
...
#define DEFINE_YASLI_SERIALIZE_OVERRIDE(type)                                                               \
  namespace yasli                                                                                           \
  {                                                                                                         \
  static bool YASLI_SERIALIZE_OVERRIDE(Archive & ar, type & value, tukk szName, tukk szLabel) \
  {                                                                                                         \
...
{
template<typename T>
static tukk EnumToString(T value)
{
	auto& desc = yasli::getEnumDescription<T>();
	return desc.label(static_cast<i32>(value));
}

tukk ToString(EExportSetting value)
{
	return EnumToString(value);
}

tukk ToString(EPhysicalizeSetting value)
{
	return EnumToString(value);
}

tukk ToString(EGenerateNormals value)
{
	return EnumToString(value);
}
...
const UnitDesc& GetUnitDesc(EUnitSetting unit)
{
	...
	static_assert(DRX_ARRAY_COUNT(knownUnits) == eUnitSetting_COUNT,
	              "Size of array does not match index range. Forgot to update units?");
	assert(0 <= unit && unit < DRX_ARRAY_COUNT(knownUnits));
	return knownUnits[unit];
}

}   // unnamed namespace

tukk GetName(EUnitSetting unit)
{
	return GetUnitDesc(unit).szName;
}

tukk GetSymbol(EUnitSetting unit)
{
