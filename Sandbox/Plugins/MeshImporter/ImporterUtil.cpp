...
static QString GetTempAssetFolder()
{
	...
	static const QString folder = QtUtil::ToQString(gEnv->pDrxPak->AdjustFileName("%USER%/MeshImporter", path, IDrxPak::FLAGS_PATH_REAL | IDrxPak::FLAGS_FOR_WRITING | IDrxPak::FLAGS_ADD_TRAILING_SLASH));
	QDir().mkpath(folder);
	return folder;
}
