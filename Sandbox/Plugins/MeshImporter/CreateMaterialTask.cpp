...
} //endns Private_CreateMaterialTask
...
void CCreateMaterialTask::SetUserData(uk pUserData)
{
...
uk                 CCreateMaterialTask::GetUserData() { return m_pUserData; }
...
bool                  CCreateMaterialTask::InitializeTask()
{
	...
	i32k materialCount = pScene->GetMaterialCount();
...
bool CCreateMaterialTask::PerformTask()
{
	...
	// Convert textures to DrxTIF
	for (i32 i = 0; i < m_pTextureManager->GetTextureCount(); ++i)
	{
		...
		if (!rcSettings.empty())
		{
			TextureHelpers::CreateDrxTif(PathUtil::AddSlash(copyTo) + m_pTextureManager->GetTargetPath(tex), rcSettings);
		}
	}
