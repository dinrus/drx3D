...
tukk const s_defaultTake = "Default take";
...
{
	i32k takeCount = pScene->GetAnimationTakeCount();

	clips.clear();
	clips.reserve(takeCount);

	SAnimationClip clip;
	for (i32 i = 0; i < takeCount; ++i)
	{
...
string DottedTail(string str, i32 len = 20)
{
	i32k d = (i32)str.length() - len;
	if (d > 0)
	{
...
bool CDialogCAF::Serialize(Serialization::IArchive& ar, Private_DialogCAF::SAnimationClip& anim)
{
	...
	i32k clipCount = (i32)m_pSkeletonProperties->m_clips.size();
	...
	for (i32 i = 0; i < pScene->GetAnimationTakeCount(); ++i)
	{
...
void CDialogCAF::AssignMaterial(CMaterial* pMat, uk pUserData)
{
	using namespace Private_DialogCAF;
...
void CDialogCAF::UpdateAnimation(const Private_DialogCAF::SAnimationClip& clip)
{
	...
	DRX_ASSERT(m_pScene && GetSceneManager().GetScene());
...
tukk CDialogCAF::GetDialogName() const
{
	return "Animation";
}
...
i32 CDialogCAF::GetToolButtons() const
{
...
void CDialogCAF::RcCaller_Assign(CRcInOrderCaller* pCaller, const QString& filePath, uk pUserData)
{
	using namespace Private_DialogCAF;
...
void CDialogCAF::RcCaller_ChrAssign(CRcInOrderCaller* pCaller, const QString& filePath, uk pUserData)
{
	using namespace Private_DialogCAF;
	std::unique_ptr<SRcPayload> pPayload((SRcPayload*)pUserData);
...
void CDialogCAF::RcCaller_SkinAssign(CRcInOrderCaller* pCaller, const QString& filePath, uk pUserData)
{
	using namespace Private_DialogCAF;
	std::unique_ptr<SRcPayload> pPayload((SRcPayload*)pUserData);
...
{
	static tukk const szCdfContent =
	  "<CharacterDefinition>\n"
	  " <Model File=\"%1\"/>\n"
	  " %2"
	  "</CharacterDefinition>\n";

	static tukk const szSkinContent =
	  " <AttachmentList>\n"
...
void CDialogCAF::ReloadAnimationSet()
{
	...
	tukk const szFilenameCHRPARAMS = PathUtil::ReplaceExtension(GetSkeletonFilePath(), "chrparams").toLocal8Bit().constData();
...
void CDialogCAF::StartAnimation(const QString& animFilename)
{
	...
	i32k localAnimID = m_pScene->m_pCharInstance->GetIAnimationSet()->GetAnimIDByName(QtUtil::ToString(animFilename));
	if (localAnimID < 0)
	{
		return;
	}
...
bool CDialogCAF::IsCurrentScene(i32 sceneId) const
{
