...
struct SFatIndex
{
	i32 elements[eElement_COUNT];

	SFatIndex()
	{
		for (i32 i = 0; i < eElement_COUNT; ++i)
		{
			elements[i] = -1;
		}
	}
...
void SetElementIndices(
  ...
  i32 element, // in EElement
  const FbxLayerElement* pLayerElement,
  const FbxLayerElementArrayTemplate<i32>& indexArray,
  const FbxMesh& mesh)
{
	...
	{
		i32k nFaces = mesh.GetPolygonCount();
		...
		if (pLayerElement->GetReferenceMode() == FbxLayerElement::eDirect)
		{
			...
			{
				for (i32 j = 0; j < mesh.GetPolygonSize(i); ++j)
				{
...
i32 CompareLex(const Vec3& lhp, const Vec3& rhp)
{
...
i32 CompareLex(const SMeshTexCoord& lhp, const SMeshTexCoord& rhp)
{
	if (lhp == rhp)
	{
		return 0;
...
struct CompareTriangles
{
	...
	CompareTriangles(const SVertex* pVertices, i32k* pIndices)
		: pVertices(pVertices)
		, pIndices(pIndices)
	{}
...
{
	i32k nFaces = inputMesh.GetPolygonCount();
	i32 t0 = 0;
	for (i32 i = 0; i < nFaces; ++i)
	{
		i32k nFaceSize = inputMesh.GetPolygonSize(i);
		i32 t1 = t0 + 1;
		for (i32 j = 2; j < nFaceSize; ++j)
		{
			out.push_back(in[t0]);
			out.push_back(in[t1]);
...
void CreateDirectMesh(const FbxMesh& inputMesh, std::vector<SVertex>& vertices)
{
	...
	i32k nIndices = inputMesh.GetPolygonVertexCount();
	i32k nFaces = inputMesh.GetPolygonCount();
	...
	// Set fat indices.
	{
		...
		for (i32 i = 0; i < nFaces; ++i)
		{
			for (i32 j = 0; j < inputMesh.GetPolygonSize(i); ++j)
			{
	...
	i32k nTriIndices = fatIndices.size();

	// Create vertices.
	vertices.resize(nTriIndices);
	for (i32 i = 0; i < nTriIndices; ++i)
	{
		if (bHasMaterials)
		{
...
{
	i32k nMaterials = pNode->GetMaterialCount();

	// Maps old indices to new (translated) indices, such that map[old] == new.
	std::vector<i32> map(pNode->GetMaterialCount(), -1);
...
void CreateMesh(const SVertex* vertices, i32 nIndices, CMesh& outputMesh, SCreateEngineMeshStats* pStats)
{
	...
	std::vector<i32> sortedVertices(nIndices);
	for (i32 i = 0; i < nIndices; ++i)
	{
		sortedVertices[i] = i;
	}
	...
	std::vector<i32> indices(nIndices, -1);
	...
	std::vector<i32> tris(nIndices / 3);
	...
	std::vector<i32> sortedIndices(nIndices);
	for (size_t i = 0; i < tris.size(); ++i)
	{
	...
	// Collect subsets.
	startTime = pTimer->GetAsyncCurTime();
	std::vector<std::pair<i32, i32>> subsetIndexRanges;
	i32 firstSubsetIndex = 0;
	for (i32 i = 1; i < nIndices; ++i)
	{
	...
	i32k nSubsets = subsetIndexRanges.size();
	...
	for (size_t i = 0; i < nSubsets; ++i)
	{
		...
		{
			i32 index = sortedIndices[subset.nFirstIndexId + j];
	...
	{
		i32k nVertices = uniqueVertices.size();
  ...
  SCreateEngineMeshStats* pStats)
{
	...
	std::vector<std::pair<i32, i32>> subMeshes;
	...
	for (size_t i = 1; i < vertices.size(); ++i)
	{
		...
		{
			subMeshes.push_back(std::make_pair(firstVertex, (i32)(i)));
			firstVertex = i;
		}
	}
	subMeshes.push_back(std::make_pair(firstVertex, (i32)(vertices.size())));

	startTime = pTimer->GetAsyncCurTime();
	for (size_t i = 0; i < subMeshes.size(); ++i)
	{
		i32k nIndices = subMeshes[i].second - subMeshes[i].first;
