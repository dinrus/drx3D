...
void MarkSubtreeWithoutRoot(const FbxTool::SNode* pSubtreeRoot, std::vector<bool>& mark)
{
	...
	for (i32 i = 0; i < pSubtreeRoot->numChildren; ++i)
	{
	...
	while (!elementStack.empty())
	{
		...
		for (i32 i = 0; i < pElement->numChildren; ++i)
		{
...
public:
	...
	{
		i32k currentIndex = m_pPreviewMode->GetCheckedItem();
...
{
	for (i32 i = 0; i < pScene->GetElementCount(); ++i)
	{
...
std::unique_ptr<CModelProperties::SSerializer> CDialogCHR::CreateSerializer(CSceneElementCommon* pSceneElement)
{
	...
	struct SSerializer
	{
		...
		i32 GetIndexFromProxyMesh(const DynArray<CSceneElementCommon*>& proxyCandidateElements) const
		{
			...
			for (i32 i = 1; i < proxyCandidateElements.size(); ++i)
			{
		...
		struct SAxisConstraint
		{
			...
			SAxisConstraint(SSerializer* pSerializer, i32 axis)
			...
			void Serialize(Serialization::IArchive& ar)
			{
				static tukk limitLabels[] = { "^Min", "^Max" };
				static tukk limitNames[] = { "min", "max" };
				static tukk valueNames[] = { "minVal", "maxVal" };
				static tukk valueLabels[] = { "^", "^" };
		...
		void SerializeIkConstraints(Serialization::IArchive& ar)
		{
			...
			{
				for (i32 limit = 0; limit < 2; ++limit)
				{
					i32k axisLimit = 3 * limit + axis;
...
void CDialogCHR::UpdateCharacter()
{
	...
	// Initialize bones.
	u32k jointCount = m_pCharacter->m_pCharInstance->GetIDefaultSkeleton().GetJointCount();
...
i32 CDialogCHR::GetToolButtons() const
{
...
CMaterial* CDialogCHR::CreateSkeletonMaterial(const QString& materialFilePath)
{
	...
	i32k mtlFlags = GetScene()->GetMaterialCount() > 1 ? MTL_FLAG_MULTI_SUBMTL : 0;
...
void CDialogCHR::AssignScene(const SImportScenePayload* pUserData)
{
	...
	AutoAssignSubMaterialIDs(std::vector<std::pair<i32, QString>>(), GetScene());
...
{
	DRX_ASSERT(m_pSceneModel);
	m_pSceneModel->ClearScene();
}

tukk CDialogCHR::GetDialogName() const
{
	return "Skeleton";
}
...
void CDialogCHR::WriteJointPhysicsData(std::vector<FbxMetaData::SJointPhysicsData>& jointPhysicsData) const
{
	...
	DRX_ASSERT(pFbxScene);

	for (i32 i = 0; i < pFbxScene->GetNodeCount(); ++i)
	{
...
void CDialogCHR::ReadNodeMetaData(const std::vector<FbxMetaData::SNodeMeta>& nodeMetaData)
{
	...
	{
		DRX_ASSERT(!m_pSceneUserData->m_skeleton.GetExportRoot());
	...
	for (i32 i = 0; i < pScene->GetNodeCount(); ++i)
	{
...
void CDialogCHR::RenderJoints(const SRenderContext& rc, ICharacterInstance* pCharInstance)
{
	...
	else if (m_hitJointId != kInvalidJointId)
	{
		...
		tukk const jointName = defaultSkeleton.GetJointNameByID(m_hitJointId);
...
void CDialogCHR::SelectJoint(i32 jointId)
{
	tukk jointName = m_pCharacter->m_pCharInstance->GetIDefaultSkeleton().GetJointNameByID(jointId);
