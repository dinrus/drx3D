...
static void FillExtensionFilters(ExtensionFilterVector& extensionFilters, i32 fileFormatFlags)
{
...
QString CBaseDialog::ShowOpenFilePrompt(i32 fileFormatFlags)
{
	using namespace Private_DialogCommon;

	DRX_ASSERT(fileFormatFlags);
...
QString CBaseDialog::ShowSaveAsFilePrompt(i32 fileFormatFlags)
{
	using namespace Private_DialogCommon;
...
std::vector<string> GetAssetTypesFromFileFormatFlags(i32 flags)
{
...
static std::vector<string> GetExtensionsFromFileFormatFlags(i32 flags)
{
...
{	
	...
	if (IsAssetMetaDataFile(filePath))
	{
		...
		if (pAsset)
		{
			...
			i32k fmt = GetOpenFileFormatFlags() & ~eFileFormat_JSON;
