...
{
	DRX_ASSERT(cb->GetCurrentData().canConvert<i32>());
...
void CPhysProxiesControlsWidget::SetupUI()
{
	auto createVisibilityWidget = [](QWidget* pParent, bool canBeTransparent = true)
	{
		...
		cb->AddItem("Solid", (i32)CProxyGenerator::EVisibility::Solid);
		if (canBeTransparent)
		{
			cb->AddItem("Transparent", (i32)CProxyGenerator::EVisibility::Wireframe);
		}
		cb->AddItem("Hidden", (i32)CProxyGenerator::EVisibility::Hidden);
		return cb;
	};
