...
static IStatObj* SaveProxies(IStatObj* pStatObj, const SProxyTree* pProxyTree, const SProxyTree::SNode* pProxyTreeNode, i32& nProxies, i32 slotParent = 0)
{
	for (size_t i = 0; i < pProxyTreeNode->m_physGeometryCount; ++i)
	{
		...
		tukk name = slotParent >= 0 ? "$physics_proxy" : "";
...
SProxyTree::SProxyTree(const CProxyData* proxyData, const FbxTool::CScene* pFbxScene)
{
	...
	while (!Q.empty())
	{
		...
		for (i32 i = 0, N = proxyData->GetPhysProxiesCount(pFbxNode); i < N; ++i)
		{
