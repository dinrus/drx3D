...
const SPhysProxies* CProxyData::GetPhysProxiesByIndex(const FbxTool::SNode* pFbxNode, i32 id) const
{
	return m_physProxiesNodeMap[pFbxNode->id][id];
}

i32 CProxyData::GetPhysProxiesCount(const FbxTool::SNode* pFbxNode) const
{
...
uint64 CProxyData::HasPhysProxies(const FbxTool::SNode* pFbxNode) const
{
	...
	for (i32 i = 0; i < GetPhysProxiesCount(pFbxNode); ++i)
	{
		pProx = GetPhysProxiesByIndex(pFbxNode, i);
		isles |= pProx->params.islandMap;
	}
	i32 n = pProx ? pProx->pSrc->isleTriCount.size() : 1;
