...
void CMaterialElement::Serialize(Serialization::IArchive& ar)
{
	...
	if (ar.isEdit())
	{
		...
		for (i32 i = 0; i < m_pModel->rowCount(QModelIndex()); ++i)
		{
		...
		if (ar.isInput())
		{
			i32k nextSelection = dropDown.index();
...
i32 CMaterialElement::GetSubMaterialIndex() const
{
...
void CMaterialElement::SetSubMaterialIndex(i32 index)
{
