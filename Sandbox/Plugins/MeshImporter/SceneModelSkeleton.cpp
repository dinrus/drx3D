...
} //endns Private_SceneModelSkeleton
...
i32 CSceneModelSkeleton::columnCount(const QModelIndex& index) const
{
	return eColumnType_COUNT;
}

QVariant CSceneModelSkeleton::data(const QModelIndex& index, i32 role) const
{
	using namespace Private_SceneModelSkeleton;
...
QVariant CSceneModelSkeleton::headerData(i32 column, Qt::Orientation orientation, i32 role) const
{
	if (role == Qt::DisplayRole)
	{
		switch (column)
...
bool CSceneModelSkeleton::setData(const QModelIndex& modelIndex, const QVariant& value, i32 role)
{
...
CItemModelAttribute* CSceneModelSkeleton::GetColumnAttribute(i32 col) const
{
	switch (col)
	{
	case eColumnType_Name:
