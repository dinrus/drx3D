...
public:
	...
	{
		DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_WARNING, "%s", message.c_str());
	}
...
string ImportFbxFile(const string& inputFilePath, const string& outputDirectoryPath)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_ERROR, "Importing FBX file '%s' failed.", inputFilePath.c_str());
		return string();
	}
...
std::vector<const FbxTool::SNode*> FindAllSkinNodes(const FbxTool::CScene* pFbxScene)
{
	...
	for (i32 i = 0, N = pFbxScene->GetNodeCount(); i < N; ++i)
	{
		const FbxTool::SNode* const pFbxNode = pFbxScene->GetNodeByIndex(i);
		bool bHasSkin = false;
		for (i32 j = 0; !bHasSkin && j < pFbxNode->numMeshes; ++j)
		{
...
static std::vector<const FbxTool::SAnimationTake*> GetAnimationTakes(const FbxTool::CScene* pScene)
{
	...
	for (i32 i = 0, N = pScene->GetAnimationTakeCount(); i < N; ++i)
	{
...
static std::vector<std::pair<const FbxTool::SAnimationTake*, string>> GetNamedTakes(const std::vector<const FbxTool::SAnimationTake*>& takes)
{
	...
	std::vector<i32> count(namedTakes.size(), 0);
...
static std::vector<string> FindAllTextures(const FbxTool::CScene* pFbxScene)
{
	...
	for (i32 i = 0; i < pFbxScene->GetMaterialCount(); ++i)
	{
		const FbxTool::SMaterial* const pMaterial = pFbxScene->GetMaterialByIndex(i);

		for (i32 j = 0; j < FbxTool::eMaterialChannelType_COUNT; ++j)
		{
...
{
	for (i32 i = 0, N = pFbxScene->GetMaterialCount(); i < N; ++i)
	{
...
static void InitializeMaterial(CMaterial* pEditorMaterial, const FbxTool::SMaterial& fbxMaterial, const std::vector<std::pair<string, string>>& relTexs, const string& outDir)
{
	...
	constexpr tuk defaultTexture[FbxTool::eMaterialChannelType_COUNT]
	{
	...
	for (i32 i = 0; i < FbxTool::eMaterialChannelType_COUNT; ++i)
	{
		tukk szSemantic = GetTextureSemanticFromChannelType(FbxTool::EMaterialChannelType(i));
		if (!szSemantic)
		{
			continue;
		}
...
static void WriteCHRPARAMS(CAsset* pSkeletonAsset, CAssetImportContext& ctx)
{
	...
	if (!bWroteChrParams)
	{
		DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_WARNING, string().Format("Unable to write .chrparams file '%s'", PathUtil::ToUnixPath(chrParamsAbsFilePath).c_str()));
	}
}
...
std::vector<CAsset*> ImportDependency(const string& sourceFilePath, const string& outputDirectory, tukk assetType, CAssetImportContext& parentCtx)
{
...
} //endns Private_AssetImporterFBX
...
bool CAssetImporterFBX::Init(const string& inputFilePath)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Importing file '%s' failed.", inputFilePath.c_str());
		return false;
	}
...
FbxTool::CScene* CAssetImporterFBX::GetScene()
{
	...
	if (!m_pFbxScene)
	{
		...
		if (!m_pFbxScene)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Importing scene from failed.");
			return nullptr;
		}
...
void CAssetImporterFBX::MoveAsset(CAssetImportContext& ctx, const string& assetPath)
{
	...
	for (const auto& movedFilePath : movedFilePaths)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Cannot create asset '%s', because file '%s' is not writable.",
...
string CAssetImporterFBX::ImportMaterial(CAssetImportContext& ctx, const std::vector<std::pair<string, string>>& relTexs)
{
	...
	i32k materialCount = pFbxScene->GetMaterialCount();

	i32k mtlFlags = materialCount > 1 ? MTL_FLAG_MULTI_SUBMTL : 0;
...
void CAssetImporterFBX::ImportCharacterDefinition(CAssetImportContext& ctx, const std::vector<CAsset*> assets)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_ERROR, string().Format("Writing character definition file '%s' failed."),
