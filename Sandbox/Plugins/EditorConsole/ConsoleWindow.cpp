...
void CConsoleWindow::InitMenu()
{
	...
	AddToMenu(&items2[0], DRX_ARRAY_COUNT(items2));

	const CEditor::MenuItems itemsHelp[] = {
		CEditor::MenuItems::HelpMenu,
	};

	AddToMenu(&itemsHelp[0], DRX_ARRAY_COUNT(itemsHelp));
	...
	pAction->setIcon(DrxIcon(QStringLiteral("icons:ObjectTypes/Console_Commands.ico")));
	...
	pAction->setIcon(DrxIcon(QStringLiteral("icons:ObjectTypes/Console_Variables.ico")));
	menu->AddAction(pAction, 0, 0);
}
...
void CConsoleWindow::SetupUI()
{
	...
	i32 buttonSize = 20;
	DrxIcon iconBack;
	iconBack.addFile(QStringLiteral("icons:General/Arrow_Left.ico"), QSize(), DrxIcon::Normal, DrxIcon::Off);
	...
	DrxIcon iconForward;
	iconForward.addFile(QStringLiteral("icons:General/Arrow_Right.ico"), QSize(), DrxIcon::Normal, DrxIcon::Off);
	...
	DrxIcon iconClose;
	iconClose.addFile(QStringLiteral("icons:Window/Window_Close.ico"), QSize(), DrxIcon::Normal, DrxIcon::Off);
...
void CConsoleWindow::HandleInput(const QString& current)
{
	...
	i32 spacePos = query.indexOf(QChar(L' '));
...
void CConsoleWindow::HandleEnter()
{
	...
	i32 cutPos = current.indexOf(QStringLiteral(" "));
	if (cutPos > 0)
	{
...
void CConsoleWindow::MovePopup(bool force)
{
	...
	{
		i32 defaultPopupHeight = m_popup.GetDefaultHeight();
		i32 topMarginSize = 20;
		i32 inputEditBorderSize = 2;

		QPoint globalYpos = m_pInput->mapToGlobal(QPoint(0, 0));

		// Resize Popup Height
		i32 cutSizeY = -(globalYpos.y() - defaultPopupHeight - topMarginSize - inputEditBorderSize);
		...
		i32 newYPos = globalYpos.y() - m_popup.height() - inputEditBorderSize;
...
bool CConsoleWindow::event(QEvent* pEvent)
{
	...
	if (m_popup.isVisible())
	{
		...
		if (pWidget)
		{
			...
			for (i32 i = 0; i < childrenMain.count(); i++)
			{
...
void CConsoleWindow::OnWriteToConsole(tukk sText, bool bNewLine)
{
...
bool CConsoleWindow::OnSave()
{
	...
	if (!path.isEmpty())
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed to open path: %s", path);
			return true;
		}
