...
{
	//load icons
	m_icons[SAutoCompleteItem::eType_Int] = QPixmap(QStringLiteral(":/i32.png"));
...
QVariant CAutoCompletePopup::CAutoCompleteModel::data(const QModelIndex& index, i32 role) const
{
...
QModelIndex CAutoCompletePopup::CAutoCompleteModel::Find(const QString& name) const
{
	...
	{
		i32 result = name.compare(i->name, Qt::CaseInsensitive);
		i32 absolute = abs(result);
		if (absolute < best)
		{
			best = absolute;
			match = index;
...
void CAutoCompletePopup::Select(i32 direction, bool selectCurrent)
{
	QItemSelectionModel* pSelection = m_pItems->selectionModel();
	if (pSelection)
	{
		...
		if (index < 0) index = 0;
		i32 count = m_model.rowCount(*(QModelIndex*)NULL);
...
void CAutoCompletePopup::DisplayHistory()
{
	...
	if (pSelection)
	{
		i32 lastItemNumber = m_model.GetItemsCount();
		if (lastItemNumber > 0)
		{
