...
i32 MyCXTPReportInplaceList::OnMouseActivate(CWnd* /*pDesktopWnd*/, UINT /*nHitTest*/, UINT /*message*/)
{
...
i32 CDialogScriptViewView::OnCreate(LPCREATESTRUCT lpcs)
{
...
{
public:
	CDialogColumn(i32 nItemIndex, LPCTSTR strName, i32 nWidth, BOOL bAutoSize = TRUE, i32 nIconID = XTP_REPORT_NOICON, BOOL bSortable = TRUE, BOOL bVisible = TRUE)
...
DrxAudio::ControlId CDialogScriptView::ms_currentPlayLine = DrxAudio::InvalidControlId;
...
CDialogScriptView::CDialogScriptView()
{
	...
	DrxAudio::SCreateObjectData const objectData("Dialog Lines trigger preview", DrxAudio::EOcclusionType::Ignore);
	m_pIAudioObject = gEnv->pAudioSystem->CreateObject(objectData);
	gEnv->pAudioSystem->AddRequestListener(&CDialogScriptView::OnAudioTriggerFinished, m_pIAudioObject, DrxAudio::ESystemEvents::TriggerFinished);
}
...
void CDialogScriptView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	else
	{
		...
		if (m_pTargetRow && m_pSourceRow != m_pTargetRow)
		{
			...
			i32k from = pSource->GetIndex();
			i32k to = pTarget->GetIndex();
...
void CDialogScriptView::OnMouseMove(UINT nFlags, CPoint point)
{
	if (!m_bDragging)
	{
		...
		i32k dragX = ::GetSystemMetrics(SM_CXDRAG);
		i32k dragY = ::GetSystemMetrics(SM_CYDRAG);
...
void CDialogScriptView::Reload()
{
	...
	if (m_pScript)
	{
		for (i32 i = 0; i < m_pScript->GetNumLines(); ++i)
		{
...
void CDialogScriptView::OnDelLine()
{
	...
	CXTPReportRecords* pRecords = GetRecords();
	for (i32 i = 0; i < pSelRows->GetCount(); ++i)
	{
...
{
	if (ms_currentPlayLine != DrxAudio::InvalidControlId)
	{
		if (m_pIAudioObject != nullptr)
		{
			m_pIAudioObject->StopTrigger(ms_currentPlayLine);
		}
		ms_currentPlayLine = DrxAudio::InvalidControlId;
	}
}

void CDialogScriptView::PlayLine(i32 index)
{
	StopSound();
	...
	if (m_pScript != nullptr && m_pIAudioObject != nullptr)
	{
		...
		DrxAudio::ControlId const triggerId = DrxAudio::StringToId(pLine->m_audioTriggerName.c_str());
		m_pIAudioObject->ExecuteTrigger(triggerId);
		ms_currentPlayLine = triggerId;
	}
}

void CDialogScriptView::OnAudioTriggerFinished(DrxAudio::SRequestInfo const* const pAudioRequestInfo)
{
