...
public:
	...
	void                     PlayLine(i32 index);

	static void              OnAudioTriggerFinished(DrxAudio::SRequestInfo const* const pAudioRequestInfo);
	//static void OnPlayingLineFinished() { ms_currentPlayLine = DrxAudio::InvalidControlId; }

	//static void DialogTriggerFinishedCallback(TAudioObjectID const nObjectID, TAudioControlID const nTriggerID, uk const pCookie);
protected:
	class CPaintManager;
...
protected:
	...
	static DrxAudio::ControlId ms_currentPlayLine;
...
protected:
	...
	afx_msg void OnDestroy();
	afx_msg i32  OnCreate(LPCREATESTRUCT lpcs);
