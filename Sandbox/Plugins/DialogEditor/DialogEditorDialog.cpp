...
void CDialogFolderCtrl::Reload()
{
	...
	for (TEditorDialogScriptMap::const_iterator iter = scripts.begin(); iter != scripts.end(); ++iter)
	{
		...
		i32 midPos = 0;
		i32 pos = 0;
		i32 len = fullClassName.GetLength();
...
{
	u32 scAttr = SCC_FILE_ATTRIBUTE_NORMAL;
	...
	else
	{
		...
		if (pScript)
		{
			u32 scAttr = m_pDialog->GetManager()->GetFileAttrs(pScript, !bUseCached);
			{
...
{
	for (i32 i = 0; i < pRecords->GetCount(); ++i)
	{
...
void CDialogFolderCtrl::OnReportItemRClick(NMHDR* pNotifyStruct, LRESULT* /*result*/)
{
	...
	u32 nFileAttr = m_pDialog->GetManager()->GetFileAttrs(pScript, false);
	...
	i32 nMenuResult = menu.TrackPopupMenu(TPM_NONOTIFY | TPM_RETURNCMD | TPM_LEFTALIGN | TPM_RIGHTBUTTON, ptClick.x, ptClick.y, this, NULL);
...
class CDialogEditorDialogViewClass : public IViewPaneClass
{
	...
	virtual tukk    ClassName()       override { return DIALOG_EDITOR_NAME; };
	...
	virtual tukk    GetPaneTitle()    override { return _T(DIALOG_EDITOR_NAME); };
...
//////////////////////////////////////////////////////////////////////////
BOOL CDialogEditorDialog::OnCmdMsg(UINT nID, i32 nCode, uk pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
	BOOL res = FALSE;
...
void CDialogEditorDialog::OnEditorNotifyEvent(EEditorNotifyEvent event)
{
	...
	case eNotify_OnUpdateSequencer:             // Sent when editor needs to update the DrxMannequin sequencer view.
	case eNotify_OnUpdateSequencerKeys:         // Sent when editor needs to update keys in the DrxMannequin track view.
	case eNotify_OnUpdateSequencerKeySelection: // Sent when DrxMannequin sequencer view changes selection of keys.
		break;
...
void CDialogEditorDialog::OnAddDialog()
{
	...
	while (true)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, _T("Group/Name may not contain characters :./"));
		}
		else
		{
			CString id;
			...
			if (pNewScript != 0)
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, _T("Dialog with ID '%s' already exists."), id.GetString());
			}
			else
			{
	...
	if (QDialogButtonBox::StandardButton::Yes == CQuestionDialog::SQuestion(QObject::tr(""), QObject::tr(ask)))
	{
		if (GetIEditor()->IsSourceControlAvailable() && m_pDM->GetFileAttrs(pCurScript) & SCC_FILE_ATTRIBUTE_MANAGED)
		{
			...
			if (dlg.DoModal() == IDOK)
			{
				...
				DrxLogAlways("DialogEditor: Deleting file '%s' from SourceControl", fullPath);
...
void CDialogEditorDialog::OnRenameDialog()
{
	...
	while (true)
	{
		...
		else
		{
			...
			u32 scAttr = m_pDM->GetFileAttrs(pCurScript);
			string newName = id;
			...
			else
			{
				if (GetIEditor()->IsSourceControlAvailable() && (scAttr & SCC_FILE_ATTRIBUTE_MANAGED))
				{
					...
					DrxLogAlways("DialogEditor: Renaming '%s' to '%s' in SourceControl", oldPath, newPath);
...
CString GetSCStatusText(u32 scStatus)
{
...
tukk SCToName(CDialogEditorDialog::ESourceControlOp op)
{
	switch (op)
	{
...
bool CDialogEditorDialog::DoSourceControlOp(CEditorDialogScript* pScript, ESourceControlOp scOp)
{
	...
	{
		assert(false);
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "CDialogEditorDialog::DoSourceControlOp: Wrong edited item. Ask AlexL to fix this.");
		return false;
	}
	...
	DrxLogAlways("DialogEditor: Doing SC-Op: %s for %s", SCToName(scOp), path);
