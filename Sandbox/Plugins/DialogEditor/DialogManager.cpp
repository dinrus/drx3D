...
ILINE void bit_set(CEditorDialogScript::TActorID& n, i32 which)
{
	n |= (0x01 << which);
}

ILINE void bit_clear(CEditorDialogScript::TActorID& n, i32 which)
{
	n &= ~(0x01 << which);
}

ILINE bool is_bit_set(CEditorDialogScript::TActorID& n, i32 which)
{
	return (n & (0x01 << which)) != 0;
}

ILINE i32 bit_count(CEditorDialogScript::TActorID n)
{
...
i32 CEditorDialogScript::SActorSet::NumActors() const
{
...
bool CEditorDialogScript::AddLine(const SScriptLine& line)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "CEditorDialogScript::AddLine: Script='%s' Cannot add line because actorID %d is out of range [0..%d]", m_id.c_str(), line.m_actor, MAX_ACTORS - 1);
		return false;
	}
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "CEditorDialogScript::AddLine: Script='%s' Cannot add line because lookAtTargetID %d is out of range [0..%d]", m_id.c_str(), line.m_lookatActor, MAX_ACTORS - 1);
		return false;
	}
...
i32 CEditorDialogScript::GetNumRequiredActors() const
{
...
i32 CEditorDialogScript::GetNumLines() const
{
	return m_lines.size();
}
...
////////////////////////////////////////////////////////////////////////////
// Get a certain line
const CEditorDialogScript::SScriptLine* CEditorDialogScript::GetLine(i32 index) const
{
...
{
	DeleteAllScripts();
	SyncDrxActionScripts();
}

////////////////////////////////////////////////////////////////////////////
void CDialogManager::SyncDrxActionScripts()
{
...
bool CDialogManager::ReloadScripts()
{
	...
	for (i32 iFile = 0; iFile < files.size(); ++iFile)
	{
...
////////////////////////////////////////////////////////////////////////////
u32 CDialogManager::GetFileAttrs(CEditorDialogScript* pScript, bool bUpdateFromFile)
{
...
{
	u32 scAttr = GetFileAttrs(pScript, true);
...
bool CEditorDialogScriptSerializer::Load(XmlNodeRef node)
{
	...
	tukk storedId = node->getAttr("Name");
...
bool CEditorDialogScriptSerializer::Save(XmlNodeRef node)
{
	...
	i32 nLineCount = m_pScript->GetNumLines();
	for (i32 i = 0; i < nLineCount; ++i)
	{
