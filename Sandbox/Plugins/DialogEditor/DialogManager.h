...
protected:
	...
	u32         m_scFileAttributes; // source control
};
...
public:
	...
	// Delete a script.
	// if bDeleteFromDisk is true, script file will also be deleted from disk and DrxAction is synced.
	// if not deleted, manual delete + DrxAction sync has to be done
	bool DeleteScript(CEditorDialogScript* pScript, bool bDeleteFromDisk);

	// Rename a script
	// if bDeleteFromDisk is true, old script file will also be deleted from disk and DrxAction is synced.
	// if not deleted, manual delete + DrxAction sync has to be done
	...
	// Small kludge to make DrxAction aware of any changes [new script, delete script, rename script]
	void    SyncDrxActionScripts();

	u32  GetFileAttrs(CEditorDialogScript* pScript, bool bUpdateFromFile = true);
