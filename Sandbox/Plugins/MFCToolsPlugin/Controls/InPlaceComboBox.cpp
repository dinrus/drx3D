...
i32 CInPlaceCBListBox::SetCurSel(i32 nPos)
{
	i32 nRet = CListBox::SetCurSel(nPos);

	//
	// Check if we have autoscrolled
	//if( m_nLastTopIdx != GetTopIndex() )
	{
		i32 nDiff = m_nLastTopIdx - GetTopIndex();
...
void CInPlaceCBListBox::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	i32 nPos = point.y / GetItemHeight(0) + GetTopIndex();
	if (GetCurSel() != nPos)
	{
		SetCurSel(nPos);
	}
...
i32 CInPlaceCBListBox::GetBottomIndex()
{
	i32 nTop = GetTopIndex();
	CRect rc;
	GetClientRect(&rc);
	i32 nVisCount = 0;
	if (GetCount() > 0)
	{
		i32 nItemH = GetItemHeight(0);
		if (nItemH > 0)
...
void CInPlaceCBListBox::SetTopIdx(i32 nPos, BOOL bUpdateScrollbar)
{
	m_nLastTopIdx = nPos;
	SetTopIndex(nPos);
	if (bUpdateScrollbar)
...
void CInPlaceCBScrollBar::VScroll(UINT nSBCode, UINT nPos)
{
	...
	i32 nTop = m_pListBox->GetTopIndex();
	i32 nBottom = m_pListBox->GetBottomIndex();
...
i32 CInPlaceComboBox::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	rect.right -= m_nThumbWidth;
	i32 nListStyle = lpCreateStruct->style | LBS_DISABLENOSCROLL | LBS_HASSTRINGS | LBS_NOTIFY;
...
void CInPlaceComboBox::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CInPlaceComboBox::GetDropDownRect(CRect& rect)
{
	...
	i32 itemHeight = m_wndList.GetItemHeight(0);
	i32 nItems = m_wndList.GetCount();
	i32 nListBoxHeight = nItems > 0 ? 2 + nItems * itemHeight : 40; // 2 extra pixels for the frame

	i32 spaceAbove = max(0l, rcEdit.top - 1 - mi.rcWork.top);
	i32 spaceBelow = max(0l, mi.rcWork.bottom - rcEdit.bottom + 1);
	...
	i32 minWidth = m_minListWidth + m_nThumbWidth;
...
void CInPlaceComboBox::ResetListBoxHeight()
{
	/*
	   ...
	   i32 nItems = m_wndList.GetCount();
	   i32 nListBoxHeight = nItems > 0 ? nItems * m_nThumbWidth : DEFAULT_IPLISTBOX_HEIGHT;
...
//////////////////////////////////////////////////////////////////////////
void CInPlaceComboBox::FillSolidRect(HDC hdc, i32 x, i32 y, i32 cx, i32 cy, HBRUSH hBrush)
{
	::FillRect(hdc, CRect(x, y, x + cx, y + cy), hBrush);
}

//////////////////////////////////////////////////////////////////////////
void CInPlaceComboBox::DrawFrame(HDC hdc, LPRECT lprc, i32 nSize, HBRUSH hBrush)
{
...
void CInPlaceComboBox::OnPaint()
{
	...
	CBrush bkgBrush(GetXtremeColor(COLOR_WINDOW));
	...
	COLORREF clrBorders = GetXtremeColor(bSelected ? XPCOLOR_HIGHLIGHT_BORDER : COLOR_3DSHADOW);
	...
	i32 nWidth = max(2, rcBtn.Width() / 2 - 4);
	...
	XTPDrawHelpers()->Triangle(&dcMem, CPoint(pt.x - nWidth, pt.y - nWidth / 2), CPoint(pt.x + nWidth, pt.y - nWidth / 2),
	                           ...
	                           GetXtremeColor(!bEnabled ? XPCOLOR_DISABLED : bPressed ? XPCOLOR_PUSHED_TEXT :
	                                          bSelected ? XPCOLOR_HIGHLIGHT_TEXT : COLOR_BTNTEXT));

	CString str = GetSelectedString();
	dcMem.SetBkMode(TRANSPARENT);
	dcMem.SetTextColor(GetXtremeColor(COLOR_WINDOWTEXT));
	CRect rcText(rc);
	rcText.left += 6;
	rcText.right -= 6;
	dcMem.SelectObject(gMFCFonts.hSystemFontBold);
...
void CInPlaceComboBox::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (rectButton.PtInRect(point))
	{
		...
		if (abs(i32(t - m_dwLastCloseTime)) < 10)
		{
...
LRESULT CInPlaceComboBox::OnSelectionOk(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	SetCurSelToEdit(m_nCurrentSelection = i32(wParam));
...
LRESULT CInPlaceComboBox::OnEditKeyDown(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	i32 nSel = m_wndList.FindString(-1, str);
	if (nSel != LB_ERR)
...
void CInPlaceComboBox::SetCurSelToEdit(i32 nSelect)
{
	CString strText;
...
i32 CInPlaceComboBox::GetCount() const
{
	return m_wndList.GetCount();
}

i32 CInPlaceComboBox::SetCurSel(i32 nSelect, bool bSendSetData)
{
	if (nSelect >= m_wndList.GetCount() || nSelect < 0)
		return CB_ERR;

	i32 nRet = m_wndList.SetCurSel(nSelect);
...
void CInPlaceComboBox::SelectString(LPCTSTR pStrText)
{
	...
	i32 sel = m_wndList.FindString(-1, pStrText);
	if (sel != LB_ERR)
	{
		SetCurSel(sel, false);
	}
...
//////////////////////////////////////////////////////////////////////////
void CInPlaceComboBox::SelectString(i32 nSelectAfter, LPCTSTR pStrText)
{
	SetWindowText(pStrText);
	if (m_wndEdit)
	{
		m_wndEdit.SetWindowText(pStrText);
		i32 sel = m_wndList.FindString(nSelectAfter, pStrText);
		if (sel != LB_ERR)
		{
			SetCurSel(sel, false);
		}
...
i32 CInPlaceComboBox::AddString(LPCTSTR pStrText, DWORD nData)
{
	i32 nIndex = m_wndList.AddString(pStrText);

	CDC* dc = GetDC();
	HGDIOBJ hOldFont = dc->SelectObject(gMFCFonts.hSystemFont);
...
//////////////////////////////////////////////////////////////////////////
void CInPlaceComboBox::OnMove(i32 x, i32 y)
{
	CWnd::OnMove(x, y);
