...
i32 CPreviewModelCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CPreviewModelCtrl::LoadFile(tukk modelFile, bool changeCamera)
{
	m_bHaveAnythingToRender = false;
	if (!m_hWnd)
		return;
	...
	u32 isSKEL = stricmp(strFileExt, DRX_SKEL_FILE_EXT) == 0;
	u32 isSKIN = stricmp(strFileExt, DRX_SKIN_FILE_EXT) == 0;
	u32 isCDF = stricmp(strFileExt, DRX_CHARACTER_DEFINITION_FILE_EXT) == 0;
	u32 isCGA = stricmp(strFileExt, DRX_ANIM_GEOMETRY_FILE_EXT) == 0;
	u32 isCGF = stricmp(strFileExt, DRX_GEOMETRY_FILE_EXT) == 0;
	...
	if (isCGA)
	{
		...
		if (!m_pCharacter)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Loading of geometry object %s failed.", modelFile);
			if (m_nTimer != 0)
				KillTimer(m_nTimer);
			m_nTimer = 0;
			Invalidate();
	...
	if (isSKEL || isSKIN || isCDF)
	{
		...
		if (!m_pCharacter)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Loading of character %s failed.", modelFile);
			if (m_nTimer != 0)
				KillTimer(m_nTimer);
			m_nTimer = 0;
			Invalidate();
	...
	if (isCGF)
	{
		...
		if (!m_pObj)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Loading of geometry object %s failed.", (tukk)modelFile);
			if (m_nTimer != 0)
				KillTimer(m_nTimer);
			m_nTimer = 0;
			Invalidate();
...
void CPreviewModelCtrl::SetCamera(CCamera& cam)
{
	...
	CRect rc;
	GetClientRect(rc);
	i32 w = rc.Width() * m_tileSizeX;
	i32 h = rc.Height() * m_tileSizeY;
...
bool CPreviewModelCtrl::RenderInternal()
{
	...
	// save some cvars
	i32 showNormals = gEnv->pConsole->GetCVar("r_ShowNormals")->GetIVal();
	i32 showPhysics = gEnv->pConsole->GetCVar("p_draw_helpers")->GetIVal();
	i32 showInfo = gEnv->pConsole->GetCVar("r_displayInfo")->GetIVal();

	gEnv->pConsole->GetCVar("r_ShowNormals")->Set((i32)m_bShowNormals);
	gEnv->pConsole->GetCVar("p_draw_helpers")->Set((i32)m_bShowPhysics);
	gEnv->pConsole->GetCVar("r_displayInfo")->Set((i32)m_bShowRenderInfo);
...
void CPreviewModelCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CPreviewModelCtrl::GetImageOffscreen(CImageEx& image, const CSize& customSize)
{
	...
	u32* data = image.GetData();
	for (i32 row = 0; row < (height - 1) / 2; ++row)
	{
		for (i32 col = 0; col < width; ++col)
		{
			u32 pixelUp = data[row * width + col];
			u32 pixelDn = data[(height - row - 1) * width + col];
...
static i32 GetFaceCountRecursively(IStatObj* p)
{
	if (!p)
	{
		return 0;
	...
	for (i32 i = 0; i < p->GetSubObjectCount(); ++i)
	{
...
static i32 GetVertexCountRecursively(IStatObj* p)
{
	if (!p)
	{
		return 0;
...
static i32 GetMaxLodRecursively(IStatObj* p)
{
	if (!p)
	{
		return 0;
	...
	for (i32 i = 0; i < p->GetSubObjectCount(); ++i)
	{
		...
		if (pS)
		{
			i32k n2 = GetMaxLodRecursively(pS->pStatObj);
			n = (n < n2) ? n2 : n;
		}
	}
	return n;
...
i32 CPreviewModelCtrl::GetFaceCount()
{
	if (m_pObj)
	{
...
i32 CPreviewModelCtrl::GetVertexCount()
{
	if (m_pObj)
	{
...
i32 CPreviewModelCtrl::GetMaxLod()
{
	if (m_pObj)
	{
...
bool CPreviewModelCtrl::CheckVirtualKey(i32 virtualKey)
{
