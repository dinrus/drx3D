...
//////////////////////////////////////////////////////////////////////////
void CTimelineCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CTimelineCtrl::TimeToClient(float time)
{
	return m_grid.WorldToClient(Vec2(time, 0)).x;
}

//////////////////////////////////////////////////////////////////////////
float CTimelineCtrl::ClientToTime(i32 x)
{
...
void CTimelineCtrl::DrawTicks(CDC& dc)
{
	...
	for (i32 keyTimeIndex = 0; m_pKeyTimeSet && keyTimeIndex < m_pKeyTimeSet->GetKeyTimeCount(); ++keyTimeIndex)
	{
		i32 keyCountBound = __max(m_pKeyTimeSet->GetKeyCountBound(), 1);
		i32 keyCount = __min(m_pKeyTimeSet->GetKeyCount(keyTimeIndex), keyCountBound);
...
void CTimelineCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	i32 hitKeyTimeIndex = HitKeyTimes(point);
...
void CTimelineCtrl::SetTimeMarker(float fTime)
{
	...
	i32 x0 = TimeToClient(m_fTimeMarker);
	i32 x1 = TimeToClient(fTime);
...
//////////////////////////////////////////////////////////////////////////
i32 CTimelineCtrl::HitKeyTimes(CPoint point)
{
	i32k threshold = 3;

	i32 hitKeyTimeIndex = -1;
	for (i32 keyTimeIndex = 0; m_pKeyTimeSet && keyTimeIndex < m_pKeyTimeSet->GetKeyTimeCount(); ++keyTimeIndex)
	{
		float keyTime = (m_pKeyTimeSet ? m_pKeyTimeSet->GetKeyTime(keyTimeIndex) : 0.0f);
		i32 x = TimeToClient(keyTime);
...
void CTimelineCtrl::MoveSelectedKeyTimes(float scale, float offset)
{
	...
	if (m_pKeyTimeSet)
		m_pKeyTimeSet->MoveKeyTimes(i32(indices.size()), &indices[0], scale, offset, m_copyKeyTimes);
}
...
//////////////////////////////////////////////////////////////////////////
void CTimelineCtrl::SendNotifyEvent(i32 nEvent)
{
	NMHDR nmh;
	nmh.hwndFrom = m_hWnd;
...
void CTimelineCtrl::DrawSecondTicks(CDC& dc)
{
	...
	for (i32 gx = m_grid.firstGridLine.x; gx < m_grid.firstGridLine.x + m_grid.numGridLines.x + 1; gx++)
	{
		dc.SelectObject(ltgray);

		i32 x = m_grid.GetGridLineX(gx);
		if (x < 0)
			continue;
		...
		//if (gx % 10 == 0)
		{
			...
			drx_sprintf(str, "%g", t * m_fTicksTextScale);
...
public:
	...
	{
		dc.SelectObject(ltgray);
		dc.MoveTo(i32(rect.left + x), rect.bottom - 2);
		dc.LineTo(i32(rect.left + x), rect.bottom - 4);

		{
			char str[32];
			drx_sprintf(str, "%d", frameIndex);

			//t = t / pow(10,precision);
			dc.SelectObject(black);
			dc.MoveTo(i32(rect.left + x), rect.bottom - 2);
			dc.LineTo(i32(rect.left + x), rect.bottom - 14);
			dc.TextOut(i32(rect.left + x + 2), rect.top, str);
		}
	}
