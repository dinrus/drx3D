...
i32k kButtonWidth = 40;
const COLORREF kButtonTextColor = RGB(255, 255, 0);
i32k kTextLeftSpacing = 4;
i32k kTextFontSize = 70;
tukk kTextFontFace = "Arial";
...
void CImageHistogramCtrl::OnPaint()
{
	...
	i32 i = 0;
	i32 graphWidth = rcGraph.Width() != 0 ? abs(rcGraph.Width()) : 1;
	i32 graphHeight = abs(rcGraph.Height() - 2);
	i32 graphBottom = abs(rcGraph.bottom - 2);
	i32 crtX = 0;
	...
	{
		i32 lastHeight[kNumChannels] = { INT_MAX, INT_MAX, INT_MAX, INT_MAX };
		i32 heightR, heightG, heightB, heightA;
	...
	else if (m_drawMode == eDrawMode_SplitRGB)
	{
		...
		i32k aThirdOfNumColorLevels = kNumColorLevels / 3;
		i32k aThirdOfWidth = rcGraph.Width() / 3;
		CPen* pPen = NULL;
		float scale = 0, pos = 0;
		...
		for (size_t x = 0, xCount = abs(rcGraph.Width() - 2); x < xCount; ++x)
		{
			...
			i = (float)((i32)(pos * kNumColorLevels) % aThirdOfNumColorLevels) / aThirdOfNumColorLevels * kNumColorLevels;
			i = CLAMP(i, 0, kNumColorLevels - 1);
			scale = 0;
...
void CImageHistogramCtrl::ComputeStatisticsForChannel(i32 aIndex)
{
	i32 hits = 0;
	i32 total = 0;
...
{
	i32k kSize = kNumColorLevels * sizeof(UINT);
...
void CImageHistogramCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CImageHistogramCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	i32 index = mnu.TrackPopupMenu(TPM_RETURNCMD | TPM_NONOTIFY, pt.x, pt.y, this);
