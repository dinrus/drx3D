...
class CXTPReportRecordItemText_Custom : public CXTPReportRecordItemText
{
...
public:
	...
	void        SetIconIndex2(i32 nIconIndex) { m_nIconIndex2 = nIconIndex; };

	virtual i32 Draw(XTP_REPORTRECORDITEM_DRAWARGS* pDrawArgs)
	{
		i32 res = __super::Draw(pDrawArgs);
		if (m_nIconIndex2 != -1)
		{
...
CTreeItemRecord::CTreeItemRecord(bool bIsGroup, tukk name)
	: m_name(name),
	m_bIsGroup(bIsGroup),
	m_nHeight(-1),
	m_bDropTarget(false),
...
//////////////////////////////////////////////////////////////////////////
void CTreeItemRecord::SetName(tukk name)
{
	m_name = name;
	if (GetItemCount() > 0)
	{
...
//////////////////////////////////////////////////////////////////////////
void CTreeItemRecord::SetIcon(i32 nIconIndex)
{
	if (GetItemCount() > 0)
	{
...
//////////////////////////////////////////////////////////////////////////
void CTreeItemRecord::SetIcon2(i32 nIconIndex2)
{
	if (GetItemCount() > 0)
	{
		CXTPReportRecordItemText_Custom* pItem0 = (CXTPReportRecordItemText_Custom*)GetItem(0);
...
public:
	...
	virtual i32 GetRowHeight(CDC* pDC, CXTPReportRow* pRow)
	{
		...
		if (pRecord)
		{
			i32 nHeight = pRecord->GetItemHeight();
			if (nHeight > 0)
				return nHeight;
		}
...
CTreeCtrlReport::CTreeCtrlReport()
{
	...
	//i32 showHeader = regMgr.GetProfileInt(_T("FlowGraph"), _T("ComponentShowHeader"), 1);
...
void CTreeCtrlReport::OnMouseMove(UINT nFlags, CPoint point)
{
	if (!m_bDragging)
	{
		...
		i32k dragX = ::GetSystemMetrics(SM_CXDRAG) + 5;
		i32k dragY = ::GetSystemMetrics(SM_CYDRAG) + 5;
...
//////////////////////////////////////////////////////////////////////////
i32 CTreeCtrlReport::GetSelectedCount()
{
	return GetSelectedRows()->GetCount();
}

//////////////////////////////////////////////////////////////////////////
i32 CTreeCtrlReport::GetSelectedRecords(Records& items)
{
	CXTPReportSelectedRows* pRows = GetSelectedRows();
...
//////////////////////////////////////////////////////////////////////////
void CTreeCtrlReport::EnableAutoNameGrouping(bool bEnable, i32 nGroupIcon)
{
...
//////////////////////////////////////////////////////////////////////////
CTreeItemRecord* CTreeCtrlReport::CreateGroupRecord(tukk name, i32 nGroupIcon)
{
...
void CTreeCtrlReport::SetFilterText(const CString& filterText)
{
	...
	{
		CString token;
		for (i32 pos = 0; !(token = filterText.Tokenize(" ", pos)).IsEmpty(); )
		{
...
bool CTreeCtrlReport::FilterTest(const CString& filter, bool bAddOperation)
{
	...
	if (bAddOperation)
	{
		...
		{
			if (DrxStringUtils::stristr(filter, m_filterKeywords[key]) == 0)
				return false;
		}
		return true;
	}
...
//////////////////////////////////////////////////////////////////////////
i32 CTreeCtrlReport::UpdateFilterTextRecursive(CXTPReportRecords* pRecords, i32 level)
{
	i32 AnyVisible = 0;
	...
	for (i32 i = 0; i < num; i++)
	{
		...
		else if (OnFilterTest(pRecord))
		{
			...
			for (i32 k = 0, numChilds = pRecord->GetChildCount(); k < numChilds; k++)
			{
		...
		i32 AnyChildsVisible = UpdateFilterTextRecursive(pRecord->GetChilds(), level + 1);
...
void CTreeCtrlReport::RemoveItemsFromGroupsMapRecursive(CTreeItemRecord* pRec)
{
	...
	path = PathUtil::AddSlash(path.c_str());
...
bool CTreeCtrlReport::SelectRecordByPathRecursive(CTreeItemRecord* pRec, const std::vector<string>& rPathElements, u32 aCurrentElementIndex)
{
	if (rPathElements.size() <= aCurrentElementIndex)
		return false;

	if (rPathElements[aCurrentElementIndex] == pRec->GetName())
	{
		if (rPathElements.size() == aCurrentElementIndex + 1)
		{
			...
			for (i32 j = 0, nNumRows = pRows->GetCount(); j < nNumRows; j++)
			{
...
bool CTreeCtrlReport::SelectRecordByName(const CString& name, CXTPReportRow* pParent)
{
	...
	for (i32 i = 0; i < num; i++)
	{
		...
		if (stricmp(pRecord->GetName(), name) == 0)
		{
...
bool CTreeCtrlReport::SelectRecordByUserString(const CString& str, CXTPReportRow* pParent)
{
	...
	for (i32 i = 0; i < num; i++)
	{
		...
		tukk pString = pRecord->GetUserString();
		if (!pString)
		{
		...
		if (stricmp(pString, str) == 0)
		{
			...
			for (i32 j = 0, nNumRows = pCandiateRows->GetCount(); j < nNumRows; j++)
			{
...
bool CTreeCtrlReport::SelectRecordByPath(tukk pPath)
{
	std::vector<string> pathElements;

	SplitString(string(pPath), pathElements, '/');
...
void CTreeCtrlReport::SetPathSeparators(tukk pSeparators)
{
...
void CTreeCtrlReport::CalculateItemPath(CTreeItemRecord* const pRec, TreeItemPathString& rPath, bool bSlashPathSeparator)
{
	...
	while (pParent)
	{
		...
		if (bSlashPathSeparator)
			...
			path = PathUtil::AddBackslash(path.c_str());
