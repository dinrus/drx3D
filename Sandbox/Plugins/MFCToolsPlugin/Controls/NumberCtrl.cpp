...
void CNumberCtrl::Create(CWnd* parentWnd, CRect& rc, UINT nID, i32 nFlags)
{
	m_nFlags = nFlags;
...
void CNumberCtrl::Create(CWnd* parentWnd, UINT ctrlID, i32 flags)
{
	ASSERT(parentWnd);
	m_nFlags = flags;
	CRect rc;
...
i32 CNumberCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CNumberCtrl::GetBtnRect(i32 btn, CRect& rc)
{
	CRect rcw;
	GetClientRect(rcw);
...
i32 CNumberCtrl::GetBtn(CPoint point)
{
	for (i32 i = 0; i < 2; i++)
	{
		CRect rc;
...
void CNumberCtrl::SetBtnStatus(i32 s)
{
	m_btnStatus = s;
	RedrawWindow();
}
...
void CNumberCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_draggin)
	{
		...
		CPoint cp;
		GetCursorPos(&cp);
		i32 sX = GetSystemMetrics(SM_CXSCREEN);
		i32 sY = GetSystemMetrics(SM_CYSCREEN);
...
i32 CNumberCtrl::CalculateDecimalPlaces(double infNumber, i32 iniMaxPlaces)
{
	assert(iniMaxPlaces >= 0);

	char str[256], * _str = str;

	drx_sprintf(str, "%f", infNumber);
...
void CNumberCtrl::SetInternalPrecision(i32 iniDigits)
{
...
void CNumberCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
//////////////////////////////////////////////////////////////////////////
void CNumberCtrl::SetFloatFormatPrecision(i32 significantDigits)
{
