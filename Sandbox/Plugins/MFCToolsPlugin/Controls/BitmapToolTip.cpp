...
static i32k STATIC_TEXT_C_HEIGHT = 42;
static i32k HISTOGRAM_C_HEIGHT = 130;
...
tukk CBitmapToolTip::GetShowModeDescription(EShowMode eShowMode, bool bShowInOriginalSize) const
{
	switch (eShowMode)
	{
	case ESHOW_RGB:
...
void CBitmapToolTip::RefreshLayout()
{
	if (m_staticBitmap.m_hWnd)
	{
		...
		CRect rc(rcClient);
		i32 histoHeight = (m_bShowHistogram ? HISTOGRAM_C_HEIGHT : 0);
...
bool CBitmapToolTip::LoadImage(const CString& imageFilename)
{
	...
	if (CImageUtil::LoadImage(imageFilename, image))
	{
		...
		imginfo.Format(infoformat, image.GetWidth(), image.GetHeight(),
		               (tukk)image.GetFormatDescription(), pShowModeDescription);

		m_staticText.SetWindowText(imginfo);

		i32 w = image.GetWidth();
		i32 h = image.GetHeight();
		i32 multiplier = (m_eShowMode == ESHOW_RGB_ALPHA ? 2 : 1);
		i32 originalW = w * multiplier;
		i32 originalH = h;
		...
		{
			for (i32 h = 0; h < scaledImage.GetHeight(); h++)
				for (i32 w = 0; w < scaledImage.GetWidth(); w++)
				{
					i32 a = scaledImage.ValueAt(w, h) >> 24;
					scaledImage.ValueAt(w, h) = RGB(a, a, a);
				}
		}
		else if (m_eShowMode == ESHOW_RGB_ALPHA)
		{
			i32 halfWidth = scaledImage.GetWidth() / 2;
			for (i32 h = 0; h < scaledImage.GetHeight(); h++)
				for (i32 w = 0; w < halfWidth; w++)
				{
					i32 r = GetRValue(scaledImage.ValueAt(w, h));
					i32 g = GetGValue(scaledImage.ValueAt(w, h));
					i32 b = GetBValue(scaledImage.ValueAt(w, h));
					i32 a = scaledImage.ValueAt(w, h) >> 24;
...
//////////////////////////////////////////////////////////////////////////
i32 CBitmapToolTip::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
//////////////////////////////////////////////////////////////////////////
void CBitmapToolTip::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CBitmapToolTip::CorrectPosition()
{
	...
	bool bOffset = false;
	i32 cy = GetSystemMetrics(SM_CYCURSOR);
	i32 dy;
	CRect go = rc;
