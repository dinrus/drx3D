...
static i32k kCharacterHeightInItem = 14;
...
//////////////////////////////////////////////////////////////////////////
i32 CImageListCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
//////////////////////////////////////////////////////////////////////////
void CImageListCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CImageListCtrl::CalcLayout(bool bUpdateScrollBar)
{
	...
	if (nStyle & ILC_STYLE_HORZ)
	{
		if (bUpdateScrollBar)
		{
			// Set scroll info.
			i32 nPage = rc.Width();
			i32 w = m_items.size() * (m_itemSize.cx + m_borderSize.cx) + m_borderSize.cx;
		...
		// Horizontal Layout.
		i32 x = m_borderSize.cx - m_scrollOffset.x;
		i32 y = m_borderSize.cy;
	...
	{
		i32 nPageHorz = rc.Width();
		i32 nPageVert = rc.Height();
		...
		i32 nTextHeight = 12;
		i32 nItemWidth = m_itemSize.cx + m_borderSize.cx;
		i32 nItemHeight = m_itemSize.cy + m_borderSize.cy + nTextHeight;

		i32 nNumOfHorzItems = nPageHorz / nItemWidth;
		if (nNumOfHorzItems <= 0)
			nNumOfHorzItems = 1;
		i32 nNumOfVertItems = m_items.size() / nNumOfHorzItems;

		if (bUpdateScrollBar)
		{
			i32 w = nNumOfHorzItems * nItemWidth + m_borderSize.cx;
			i32 h = nNumOfVertItems * nItemHeight + m_borderSize.cy;
		...
		i32 x = m_borderSize.cx - m_scrollOffset.x;
		i32 y = m_borderSize.cy - m_scrollOffset.y;
		for (i32 itemCounter = 0; itemCounter < m_items.size(); ++itemCounter)
		{
...
bool CImageListCtrl::OnBeginDrag(UINT nFlags, CPoint point)
{
	...
	i32k kExtraItemHeight = 0;//kCharacterHeightInItem;

	i32 itemWidth = m_pSelectedItem->rect.Width();
	i32 itemHeight = m_pSelectedItem->rect.Height() + kExtraItemHeight;
	...
	i32 nIndex = m_DraggedImage.Add(&bitmap, RGB(255, 255, 255));
...
//////////////////////////////////////////////////////////////////////////
void CImageListCtrl::SelectItem(i32 nIndex, bool bSelect)
{
	i32 nStyle = GetStyle();
	if (!(nStyle & ILC_MULTI_SELECTION))
	{
...
void CImageListCtrl::OnUpdateItem(CImageListCtrlItem* pItem)
{
	...
	u32* pImageData = pItem->pImage->GetData();
	i32 w = pItem->pImage->GetWidth();
	i32 h = pItem->pImage->GetHeight();
