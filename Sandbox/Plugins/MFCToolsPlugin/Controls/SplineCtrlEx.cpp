...
static void SetKeyTangentType(ISplineInterpolator* pSpline, i32 key, ESplineKeyTangentType type)
{
	i32 flags = (pSpline->GetKeyFlags(key) & ~SPLINE_KEY_TANGENT_IN_MASK) & ~SPLINE_KEY_TANGENT_OUT_MASK;
...
protected:
	void AddSpline(ISplineInterpolator* pSpline)
	{
		...
		i32k numKeys = pSpline->GetKeyCount();
	...
	virtual i32         GetSize()        { return sizeof(*this); }
	virtual tukk GetDescription() { return "UndoSplineCtrlEx"; };
...
private:
	...
	{
	public:
		std::vector<i32>          keySelectionFlags;
...
public:
	...
	virtual bool IsSelectionChanged() const
	{
		...
		for (auto it = m_splineEntries.begin(); it != m_splineEntries.end(); ++it)
		{
			...
			for (i32 i = 0; i < pSpline->GetKeyCount(); ++i)
			{
...
//////////////////////////////////////////////////////////////////////////
i32 CSplineCtrlEx::GetKeyTimeCount() const
{
	UpdateKeyTimes();

	return i32(m_keyTimes.size());
}

//////////////////////////////////////////////////////////////////////////
float CSplineCtrlEx::GetKeyTime(i32 index) const
{
	UpdateKeyTimes();
...
//////////////////////////////////////////////////////////////////////////
bool CSplineCtrlEx::GetKeyTimeSelected(i32 index) const
{
	UpdateKeyTimes();
...
//////////////////////////////////////////////////////////////////////////
void CSplineCtrlEx::SetKeyTimeSelected(i32 index, bool selected)
{
	m_keyTimes[index].selected = selected;
}

//////////////////////////////////////////////////////////////////////////
i32 CSplineCtrlEx::GetKeyCount(i32 index) const
{
	UpdateKeyTimes();
...
//////////////////////////////////////////////////////////////////////////
i32 CSplineCtrlEx::GetKeyCountBound() const
{
	UpdateKeyTimes();
...
void CSplineCtrlEx::BeginEdittingKeyTimes()
{
	...
	for (i32 keyTimeIndex = 0; keyTimeIndex < i32(m_keyTimes.size()); ++keyTimeIndex)
...
//////////////////////////////////////////////////////////////////////////
void CSplineCtrlEx::MoveKeyTimes(i32 numChanges, i32* indices, float scale, float offset, bool copyKeys)
{
	if (CUndo::IsRecording())
	{
		...
		for (i32 splineIndex = 0; splineIndex < i32(m_splines.size()); ++splineIndex)
		{
	...
	{
	public:
		KeyChange(ISplineInterpolator* pSpline, i32 keyIndex, float oldTime, float newTime, i32 flags)
	...
	for (i32 changeIndex = 0; indices && changeIndex < numChanges; ++changeIndex)
	{
		i32 index = (indices ? indices[changeIndex] : 0);
		...
		for (i32 splineIndex = 0; splineIndex < i32(m_splines.size()); ++splineIndex)
		{
			...
			for (i32 keyIndex = 0; pSpline && keyIndex < pSpline->GetKeyCount(); ++keyIndex)
			{
	...
	if (copyKeys)
	{
		...
		{
			i32 keyIndex = (*keyToAdd).pSpline->InsertKey((*keyToAdd).oldTime, (*keyToAdd).value);
	...
	for (i32 splineIndex = 0; splineIndex < i32(m_splines.size()); ++splineIndex)
	{
		...
		pSpline->Update();
		for (i32 keyIndex = 0, keys = pSpline->GetKeyCount(); keyIndex <= keys; )
		{
			float keyTime = pSpline->GetKeyTime(keyIndex);
...
//////////////////////////////////////////////////////////////////////////
void CSplineCtrlEx::OnSize(UINT nType, i32 cx, i32 cy)
{
	...
	i32 oldW = oldRect.Width();
	i32 oldH = oldRect.Height();
...
//////////////////////////////////////////////////////////////////////////
float CSplineCtrlEx::XOfsToTime(i32 x)
{
...
//////////////////////////////////////////////////////////////////////////
CPoint CSplineCtrlEx::XOfsToPoint(i32 x, ISplineInterpolator* pSpline)
{
...
void CSplineCtrlEx::OnPaint()
{
...

	{
		...
		if (m_TimeUpdateRect != CRect(PaintDC.m_ps.rcPaint))
		{
			...
			//Draw Keys and Curve
			for (i32 i = 0; i < i32(m_splines.size()); ++i)
			{
...
public:
	...
	void operator()(i32 frameIndex, i32 x)
	{
...
void CSplineCtrlEx::DrawGrid(CDC* pDC)
{
	...
	//if (m_grid.pixelsPerGrid.y >= MIN_PIXEL_PER_GRID_Y)
	{
		...
		{
			i32 y = m_grid.GetGridLineY(gy);
			if (y < 0)
				continue;
			i32 py = m_rcSpline.bottom - (m_rcSpline.top + y);
...
void CSplineCtrlEx::DrawSpline(CDC* pDC, SSplineInfo& splineInfo, float startTime, float endTime)
{
	...
	i32 nTotalNumberOfDimensions(0);
	i32 nCurrentDimension(0);

	i32 left = TimeToXOfs(startTime);//rcClip.left;
	i32 right = TimeToXOfs(endTime);//rcClip.right;
...
//////////////////////////////////////////////////////////////////////////
void CSplineCtrlEx::DrawKeys(CDC* pDC, i32 splineIndex, float startTime, float endTime)
{
	...
	i32 lastKeyX = m_rcSpline.left - 100;
	...
	for (nCurrentDimension = 0; nCurrentDimension < nTotalNumberOfDimensions; nCurrentDimension++)
	{
		...
		i32 numKeys = pSpline->GetKeyCount();
		for (i = 0; i < numKeys; i++)
		{
...
bool CSplineCtrlEx::GetTangentHandlePts(CPoint&, CPoint&, CPoint&, i32, i32, i32)
{
	return false;
}

void CSplineCtrlEx::DrawTangentHandle(CDC* pDC, i32 nSpline, i32 nKey, i32 nDimension)
{
	// create and select a white pen
	CPen pen;
...
void CSplineCtrlEx::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_editMode == TrackingMode && GetNumSelected() == 1)
	{
		...
		for (i32 splineIndex = 0, endSpline = m_splines.size(); splineIndex < endSpline; ++splineIndex)
		{
			ISplineInterpolator* pSpline = m_splines[splineIndex].pSpline;
			for (i32 i = 0; i < pSpline->GetKeyCount(); i++)
			{
				for (i32 nCurrentDimension = 0; nCurrentDimension < pSpline->GetNumDimensions(); nCurrentDimension++)
				{
					if (pSpline->IsKeySelectedAtDimension(i, nCurrentDimension))
					{
...
//////////////////////////////////////////////////////////////////////////
bool CSplineCtrlEx::CheckVirtualKey(i32 virtualKey)
{
...
void CSplineCtrlEx::UpdateKeyTimes() const
{
	...
	for (i32 splineIndex = 0; splineIndex < i32(m_splines.size()); ++splineIndex)
	{
		...
		for (i32 keyIndex = 0; pSpline && keyIndex < pSpline->GetKeyCount(); ++keyIndex)
		{
			...
			i32 lower = 0;
			i32 upper = i32(m_keyTimes.size());
			while (lower < upper - 1)
			{
				i32 mid = ((lower + upper) >> 1);
				((m_keyTimes[mid].time >= value) ? upper : lower) = mid;
			}

			if ((lower >= i32(m_keyTimes.size()) || fabsf(m_keyTimes[lower].time - value) > threshold) &&
			    (upper >= i32(m_keyTimes.size()) || fabsf(m_keyTimes[upper].time - value) > threshold))
...
/////////////////////////////////////////////////////////////////////////////
void CSplineCtrlEx::SelectKey(ISplineInterpolator* pSpline, i32 nKey, i32 nDimension, bool bSelect)
{
	if (nKey >= 0)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CSplineCtrlEx::IsKeySelected(ISplineInterpolator* pSpline, i32 nKey, i32 nHitDimension) const
{
	if (pSpline && nKey >= 0)
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CSplineCtrlEx::GetNumSelected()
{
	i32 nSelected = 0;
	for (i32 splineIndex = 0, splineCount = m_splines.size(); splineIndex < splineCount; ++splineIndex)
	{
		if (ISplineInterpolator* pSpline = m_splines[splineIndex].pSpline)
		{
			for (i32 i = 0; i < (i32)pSpline->GetKeyCount(); i++)
			{
...
//////////////////////////////////////////////////////////////////////////
void CSplineCtrlEx::SetHorizontalExtent(i32 min, i32 max)
{
	/*
	   m_scrollMin.x = min;
	   m_scrollMax.x = max;
	   i32 width = max - min;
	   i32 nPage = m_rcClient.Width()/2;
	   i32 sx = width - nPage + m_rcSpline.left;
...
void CSplineCtrlEx::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	case SB_PAGELEFT:    // Scroll one page left.
		if (curpos > minpos)
			curpos = max(minpos, curpos - (i32)nPage);
		break;

	case SB_PAGERIGHT:      // Scroll one page right.
		if (curpos < maxpos)
			curpos = min(maxpos, curpos + (i32)nPage);
		break;
...
void CSplineCtrlEx::ScaleAmplitudeKeys(float time, float startValue, float offset)
{
	...
	for (i32 splineIndex = 0, splineCount = m_splines.size(); splineIndex < splineCount; ++splineIndex)
	{
		...
		i32 keyCount = (pSpline ? pSpline->GetKeyCount() : 0);
		i32 firstKeyIndex = keyCount;
		i32 lastKeyIndex = -1;
...
void CSplineCtrlEx::TimeScaleKeys(float time, float startTime, float endTime)
{
	...
	for (i32 splineIndex = 0, splineCount = m_splines.size(); splineIndex < splineCount; ++splineIndex)
	{
		...
		i32 keyCount = pSpline->GetKeyCount();
		...
		if (keyRangeMin <= keyRangeMax)
		{
			...
			i32 firstMovedKey = pSpline->GetKeyCount();
			for (i32 keyIndex = pSpline->GetKeyCount() - 1; keyIndex >= 0; --keyIndex)
			{
				if (pSpline->GetKeyTime(keyIndex) >= keyRangeMin)
					firstMovedKey = keyIndex;
			}

			i32 firstAffectedKey = max(0, firstMovedKey - 2);
			i32 lastAffectedKey = min(keyCount - 1, lastMovedKey + 2);
	...
	i32 rangeMin = TimeToXOfs(affectedRangeMin);
	i32 rangeMax = TimeToXOfs(affectedRangeMax);
...
//////////////////////////////////////////////////////////////////////////
void CSplineCtrlEx::RemoveKey(ISplineInterpolator* pSpline, i32 nKey)
{
...
void CSplineCtrlEx::RemoveSelectedKeys()
{
	...
	for (i32 splineIndex = 0, splineCount = m_splines.size(); splineIndex < splineCount; ++splineIndex)
	{
		...
		for (i32 i = 0; i < (i32)pSpline->GetKeyCount(); )
		{
			if (pSpline->IsKeySelectedAtAnyDimension(i))
				pSpline->RemoveKey(i);
			else
...
void CSplineCtrlEx::RemoveSelectedKeyTimesImpl()
{
	...
	if (numSelectedKeyTimes)
	{
		...
		for (i32 splineIndex = 0, end = m_splines.size(); splineIndex < end; ++splineIndex)
		{
			std::vector<KeyTime>::iterator itTime = m_keyTimes.begin(), endTime = m_keyTimes.end();
			for (i32 keyIndex = 0, endIndex = m_splines[splineIndex].pSpline->GetKeyCount(); keyIndex < endIndex; )
			{
				const float threshold = 0.01f;
...
void CSplineCtrlEx::RedrawWindowAroundMarker()
{
	...
	i32 keyTimeIndex = (itKeyTime != m_keyTimes.end() ? itKeyTime - m_keyTimes.begin() : m_keyTimes.size());
	i32 redrawRangeStart = (keyTimeIndex >= 2 ? TimeToXOfs(m_keyTimes[keyTimeIndex - 2].time) : m_rcSpline.left);
	i32 redrawRangeEnd = (keyTimeIndex < i32(m_keyTimes.size()) - 2 ? TimeToXOfs(m_keyTimes[keyTimeIndex + 2].time) : m_rcSpline.right);
...
{
	//float fSnapTime = i32((time * m_fGridTimeScale) + 0.5f) * (1.0f / m_fGridTimeScale);
	float fSnapTime = time;
	return fSnapTime;
}

//////////////////////////////////////////////////////////////////////////
i32 CSplineCtrlEx::InsertKey(ISplineInterpolator* pSpline, ISplineInterpolator* pDetailSpline, CPoint point)
{
	...
	i32 nKey = pSpline->InsertKey(time, currValue);  // TODO: Don't use FE specific snapping!
	if (m_defaultKeyTangentType != SPLINE_KEY_TANGENT_NONE)
	{
		SetKeyTangentType(pSpline, nKey, m_defaultKeyTangentType);
	}

	//i32 nKey = pSpline->InsertKeyFloat( time,val ); // TODO: Don't use FE specific snapping!
...
void CSplineCtrlEx::DuplicateSelectedKeys()
{
	...
	for (i32 splineIndex = 0, splineCount = m_splines.size(); splineIndex < splineCount; ++splineIndex)
	{
		...
		{
			i32 keyIndex = pSpline->InsertKey(SnapTimeToGridVertical((*keyToAdd).time), (*keyToAdd).value);
...
void CSplineCtrlEx::ZeroAll()
{
	...
	{
		ISplineInterpolator* pSpline = m_splines[splineIndex].pSpline;
		i32 keyIndex = (pSpline ? pSpline->FindKey(m_fTimeMarker, 0.015f) : -1);
	...
	{
		i32 keyIndex = ((*itSpline) ? (*itSpline)->FindKey(m_fTimeMarker, 0.015f) : -1);
...
void CSplineCtrlEx::KeyAll()
{
	...
	for (SplineContainer::iterator itSpline = splines.begin(); itSpline != splines.end(); ++itSpline)
	{
		...
		i32 keyIndex = (*itSpline)->InsertKeyFloat(SnapTimeToGridVertical(m_fTimeMarker), value);
		if (m_defaultKeyTangentType != SPLINE_KEY_TANGENT_NONE)
		{
...
//////////////////////////////////////////////////////////////////////////
void CSplineCtrlEx::SendNotifyEvent(i32 nEvent)
{
...
void CSplineCtrlEx::SelectRectangle(CRect rc, bool bSelect)
{
	...
	for (i32 splineIndex = 0, splineCount = m_splines.size(); splineIndex < splineCount; ++splineIndex)
	{
		...
		for (i32 i = 0; i < (i32)pSpline->GetKeyCount(); i++)
		{
			...
			i32 nTotalNumberOfDimensions(pSpline->GetNumDimensions());
			...
			for (i32 nCurrentDimension = 0; nCurrentDimension < nTotalNumberOfDimensions; nCurrentDimension++)
			{
				if (pDetailSpline)
				{
...
void CSplineCtrlEx::CopyKeys()
{
	...
	for (i = 0; i < (i32)pSpline->GetKeyCount(); i++)
	{
		if (!pSpline->IsKeySelectedAtAnyDimension(i))
			continue;
		float t = pSpline->GetKeyTime(i);
	...
	for (i = 0; i < (i32)pSpline->GetKeyCount(); i++)
	{
		...
		keyNode->setAttr("flags", (i32)pSpline->GetKeyFlags(i));
		keyNode->setAttr("in", tin);
		keyNode->setAttr("out", tout);

		for (i32 i = 0; i < pSpline->GetNumDimensions(); ++i)
		{
...
void CSplineCtrlEx::PasteKeys()
{
	...
	for (i = 0; i < rootNode->getChildCount(); i++)
	{
		...
		i32 nNumberOfChildXMLNodes(0);
		i32 nCurrentChildXMLNode(0);
		i32 nCurrentValue(0);
		...
		i32 key = pSpline->InsertKey(SnapTimeToGridVertical(t - minTime + fTime), afValue);
		if (key >= 0)
		{
...
//////////////////////////////////////////////////////////////////////////
void CSplineCtrlEx::ModifySelectedKeysFlags(i32 nRemoveFlags, i32 nAddFlags)
{
	...
	for (i32 splineIndex = 0, splineCount = m_splines.size(); splineIndex < splineCount; ++splineIndex)
	{
		...
		for (i32 i = 0; i < (i32)pSpline->GetKeyCount(); i++)
		{
			  ...
			  )
			{
				...
				{
					i32 flags = pSpline->GetKeyFlags(i);
					flags &= ~nRemoveFlags;
					flags |= nAddFlags;
...
void CSplineCtrlEx::FitSplineToViewHeight()
{
	...
	for (i32 i = 0; i < i32(m_splines.size()); ++i)
	{
		...
		for (i32 keyIndex = 0; pSpline && keyIndex < pSpline->GetKeyCount(); ++keyIndex)
		{
			...
			for (i32 d = 0, numDim = pSpline->GetNumDimensions(); d < numDim; d++)
			{
			...
			for (i32 i = 0; i < pSpline->GetKeyCount(); i++)
			{
				for (i32 nCurrentDimension = 0; nCurrentDimension < pSpline->GetNumDimensions(); nCurrentDimension++)
				{
					if (pSpline->IsKeySelectedAtDimension(i, nCurrentDimension))
					{
						...
						{
							i32 nextKey = previousKey ? i - 1 : i + 1;
	...
	else
	{
		for (i32 splineIndex = 0, endSpline = m_splines.size(); splineIndex < endSpline; ++splineIndex)
		{
			...
			if (fClosestKeyTime >= 0.0f)
			{
				...
				i32 dimensions = pSpline->GetNumDimensions();

				for (i32 i = 0; i < dimensions; i++)
				{
					float keyValue;
					i32 keyNum = pSpline->FindKey(fClosestKeyTime);
...
void CSplineCtrlEx::RemoveAllKeysButThis()
{
	...
	for (i32 splineIndex = 0, endSpline = m_splines.size(); splineIndex < endSpline; ++splineIndex)
	{
		...
		for (i32 i = pSpline->GetKeyCount(); i >= 0; i--)
		{
			bool saveKey = false;

			for (i32 nIndex = 0; nIndex < keys.size(); nIndex++)
			{
				if (keys[nIndex] == i)
					saveKey = true;
			}
