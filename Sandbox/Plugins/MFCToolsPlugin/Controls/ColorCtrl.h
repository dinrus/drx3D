...
public:
	...
	void     StartBlink(i32 iWho = CC_BLINK_BOTH, UINT nDelay = CC_BLINK_NOCHANGE);
	void     StopBlink(i32 iWho = CC_BLINK_BOTH);
...
template<class BASE_TYPE>
void CColorCtrl<BASE_TYPE >::StartBlink(i32 iWho, UINT nDelay)
{
...
template<class BASE_TYPE>
void CColorCtrl<BASE_TYPE >::StopBlink(i32 iWho)
{
	if (iWho & CC_BLINK_TEXT)
		m_fBlinkText = FALSE;
	if (iWho & CC_BLINK_BK)
...
template<class BASE_TYPE>
i32 CColorCtrl<BASE_TYPE >::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
template<class BASE_TYPE,
         ...
         i32 InitialDelay = CC_BLINK_NORMAL>
...
template<class BASE_TYPE>
void CColorPushButton<BASE_TYPE >::SetToolTip(tukk tooltipText)
{
	if (!m_tooltip)
	{
...
template<class BASE_TYPE>
void CColorPushButton<BASE_TYPE >::SetIcon(LPCSTR lpszIconResource, i32 nIconAlign, bool bDrawText)
{
	if (m_hIcon)
		DestroyIcon(m_hIcon);
...
template<class BASE_TYPE>
void CColorPushButton<BASE_TYPE >::SetIcon(HICON hIcon, i32 nIconAlign, bool bDrawText)
{
...
template<class BASE_TYPE>
i32 CColorPushButton<BASE_TYPE >::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (BASE_TYPE::OnCreate(lpCreateStruct) == -1)
		return -1;
	return 0;
...
void CColorPushButton<BASE_TYPE >::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	{
		// draw icon.
		i32 w = GetSystemMetrics(SM_CXICON);
		i32 h = GetSystemMetrics(SM_CYICON);
		i32 x;
		i32 y = rc.top + rc.Height() / 2 - h / 2;
