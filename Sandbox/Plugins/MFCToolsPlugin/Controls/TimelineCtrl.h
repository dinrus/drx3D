...
protected:
	...
	// Drawing functions
	float   ClientToTime(i32 x);
	i32     TimeToClient(float fTime);
	...
	i32     ToMillis(float time)      { return pos_directed_rounding(time * 1000.0f); };
	float   MillisToTime(i32 nMillis) { return nMillis / 1000.0f; }
