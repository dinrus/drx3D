...
public:
	 ...
	 */
	virtual CPropertyItem* AddVarBlock(CVarBlock* varBlock, tukk szCategory = NULL);
	 ...
	 */
	virtual CPropertyItem* AddVarBlockAt(CVarBlock* varBlock, tukk szCategory, CPropertyItem* pRoot, bool bFirstVarIsRoot = false);
	 ...
	 */
	void SetSplitter(i32 splitter) { m_splitter = splitter; };

	/** Get current value of splitter.
	 */
	i32 GetSplitter() const { return m_splitter; };
	...
	void SetItemHeight(i32 nItemHeight);

	//! Get height of item.
	i32            GetItemHeight(CPropertyItem* item) const;
	...
	void SetFontHeight(i32 nFontHeight)              { m_nFontHeight = nFontHeight; }
...
protected:
	...
	void         SendNotify(i32 code, CPropertyCtrlNotify& notify);
	void         DrawItem(CPropertyItem* item, CDC& dc, CRect& itemRect);
	i32          CalcOffset(CPropertyItem* item);
