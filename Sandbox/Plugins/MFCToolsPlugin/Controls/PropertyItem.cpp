...
{
public:
	CUndoVariableChange(IVariable* var, tukk undoDescription)
	{
...
static i32 NumPropertyTypes = sizeof(s_propertyTypeNames) / sizeof(s_propertyTypeNames[0]);

tukk DISPLAY_NAME_ATTR = "DisplayName";
tukk VALUE_ATTR = "Value";
tukk TYPE_ATTR = "Type";
tukk TIP_ATTR = "Tip";
tukk TIP_CVAR_ATTR = "TipCVar";
tukk FILEFILTER_ATTR = "FileFilters";
i32k FLOAT_NUM_DIGITS = 6;

static tukk PropertyTypeToResourceType(PropertyType type)
{
...
{
	inline void FormatFloatForUICString(CString& outstr, i32 significantDigits, double value)
	{
...
void CPropertyItem::ParseXmlNode(bool bRecursive /* =true  */)
{
	...
	m_image = -1;
	m_type = ePropertyInvalid;
	for (i32 i = 0; i < NumPropertyTypes; i++)
	{
	...
	if (m_type == ePropertyFloat || m_type == ePropertyInt)
	{
		...
		if (!m_node->getAttr("Precision", nPrecision))
			nPrecision = max(3 - i32(log(m_rangeMax - m_rangeMin) / log(10.f)), 0);
	...
	else if (bRecursive)
	{
		...
		m_bExpandable = false;
		// Create sub-nodes.
		for (i32 i = 0; i < m_node->getChildCount(); i++)
		{
			m_bIgnoreChildsUpdate = true;
...
void CPropertyItem::SetVariable(IVariable* var)
{
	...
	i32 dataType = m_pVariable->GetDataType();
	...
	if (!useExplicitStep)
	{
		...
		i32 nPrec = max(3 - i32(log(m_rangeMax - m_rangeMin) / log(10.f)), 0);
...
void CPropertyItem::CreateInPlaceControl(CWnd* pWndParent, CRect& ctrlRect)
{
	...
	case ePropertyAngle:
		{
			if (m_pEnumDBItem)
			{
				...
				i32 unsorted = m_pEnumDBItem->strings.size() && !strcmp(m_pEnumDBItem->strings[0], "#unsorted");
				...
				for (i32 i = unsorted; i < m_pEnumDBItem->strings.size(); i++)
	...
	case ePropertyString:
		if (m_pEnumDBItem)
		{
			...
			for (i32 i = 0; i < m_pEnumDBItem->strings.size(); i++)
				m_cCombo->AddString(m_pEnumDBItem->strings[i]);
			break;
		}
		else
	...
	case ePropertyVector4:
		{
			...
			for (i32 a = 0; a < numberOfElements; a++)
			{
...
void CPropertyItem::DrawValue(CDC* dc, CRect rect)
{
	...
	if (m_type == ePropertyBool)
	{
		i32 sz = rect.bottom - rect.top - 1;
		i32 borderwidth = (i32)(sz * 0.13f);                            //white frame border
		i32 borderoffset = (i32)(sz * 0.1f);                            //offset from the frameborder
	...
	else if (m_pVariable != 0 && m_pVariable->GetSpline(false) && m_pVariable->GetSpline(false)->GetKeyCount() > 0)
	{
		...
		i32 width = min(rect.Width() - 1, 128);
	...
	/*
	   ...
	   i32 splitPos = rc.left + pos * rc.Width();
...
//////////////////////////////////////////////////////////////////////////
tukk CPropertyItem::GetValue() const
{
	return m_value;
}

//////////////////////////////////////////////////////////////////////////
void CPropertyItem::SetValue(tukk sValue, bool bRecordUndo, bool bForceModified)
{
...
void CPropertyItem::VarToValue()
{
	...
	if (m_type == ePropertyColor)
	{
		...
		else
		{
			i32 col(0);
			m_pVariable->Get(col);
			m_value.Format("%d,%d,%d", GetRValue((u32)col), GetGValue((u32)col), GetBValue((u32)col));
		}
		return;
	}
...
//////////////////////////////////////////////////////////////////////////
CPropertyItem::TDValues* CPropertyItem::GetEnumValues(tukk strPropertyName)
{
...
void CPropertyItem::OnPsdEditButton()
{
	...
	{
		char buff[1024];
		drx_sprintf(buff, "Failed to find psd file for texture: '%s'", m_value);
	...
	if (pGetCustomItems != 0)
	{
		...
		if (bShowIt) // if func didn't veto, show the dialog
		{
			...
			if (pGetCustomItems->UseTree())
			{
				...
				tukk szSep = pGetCustomItems->GetTreeSeparator();
				if (szSep)
				{
					CString sep(szSep);
...
void CPropertyItem::OnLocalStringBrowseButton()
{
	...
	if (!pMgr)
		return;
	i32 nCount = pMgr->GetLocalizedStringCount();
	if (nCount <= 0)
		return;
...
void CPropertyItem::AddChildrenForPFProperties()
{
	...
	for (std::set<CString>::iterator it = setSelectedPathTypeNames.begin(); it != setSelectedPathTypeNames.end(); ++it)
	{
		...
		drx_sprintf(propertyN, "AgentType %d", N++);
...
void CPropertyItem::AddChildrenForAIEntityClasses()
{
	...
	{
		IVariable* pVar = new CVariable<CString>;
		drx_sprintf(propertyN, "Entity Class %d", N++);
		pVar->SetName(propertyN);
		pVar->Set(*it);
...
{
	for (i32 i = 0, num = m_controls.size(); i < num; i++)
	{
