...
public:
	 ...
	 */
	virtual void SetName(tukk sName) { m_name = sName; };
	 ...
	 */
	virtual void SetValue(tukk sValue, bool bRecordUndo = true, bool bForceModified = false);

	/** Returns current value of property item.
	 */
	virtual tukk GetValue() const;
	...
	i32 GetImage() const { return m_image; };
	 ...
	 */
	virtual void SetScriptDefault(tukk sScriptDefault) { m_strScriptDefault = sScriptDefault; };
	...
	i32            GetChildCount() const     { return m_childs.size(); };
	//! Get Child by id.
	CPropertyItem* GetChild(i32 index) const { return m_childs[index]; };
...
protected:
	...
	TDValues* GetEnumValues(tukk strPropertyName);
