...
public:
	...
	i32 GetGridLineX(i32 nGridLineX) const
	{
		return floor((nGridLineX * step.x - origin.x) * zoom.x + 0.5f);
	}
	i32 GetGridLineY(i32 nGridLineY) const
	{
		return floor((nGridLineY * step.y - origin.y) * zoom.y + 0.5f);
	}
	float GetGridLineXValue(i32 nGridLineX) const
	{
		return (nGridLineX * step.x);
	}
	float GetGridLineYValue(i32 nGridLineY) const
	{
