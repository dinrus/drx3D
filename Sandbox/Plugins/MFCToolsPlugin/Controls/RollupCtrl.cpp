...
//---------------------------------------------------------------------------
i32 CRollupCtrl::InsertPage(LPCTSTR caption, UINT nIDTemplate, CRuntimeClass* rtc, i32 idx)
{
...
//					  Make sure template had WS_CHILD style
//---------------------------------------------------------------------------
i32 CRollupCtrl::InsertPage(LPCTSTR caption, CDialog* pwndTemplate, BOOL bAutoDestroyTpl, i32 idx, BOOL bAutoExpand)
{
...
//					  Make sure template had WS_CHILD style
//---------------------------------------------------------------------------
i32 CRollupCtrl::_InsertPage(LPCTSTR caption, CDialog* pwndTemplate, i32 idx, BOOL bAutoDestroyTpl, BOOL bAutoExpand)
{
	...
	HFONT hfont = (HFONT)gMFCFonts.hSystemFont;
...
// Description		:
//---------------------------------------------------------------------------
void CRollupCtrl::RemovePage(i32 idx)
{
	if (!FindPage(idx))
		return;
...
//---------------------------------------------------------------------------
void CRollupCtrl::_RemovePage(i32 idx)
{
	// Find page
...
// Description		:
//---------------------------------------------------------------------------
void CRollupCtrl::ExpandPage(i32 idx, BOOL bExpand, BOOL bScroll, BOOL bFromUI)
{
	if (!FindPage(idx))
		return;
...
{
	//Expand-collapse All
	for (i32 i = 0; i < m_PageList.size(); i++)
...
// Description		:
//---------------------------------------------------------------------------
void CRollupCtrl::RenamePage(i32 idx, tukk caption)
{
	if (!FindPage(idx))
		return;
...
// Description		:
//---------------------------------------------------------------------------
void CRollupCtrl::EnablePage(i32 idx, BOOL bEnable)
{
	if (!FindPage(idx))
		return;
...
//---------------------------------------------------------------------------
void CRollupCtrl::ScrollToPage(i32 idx, BOOL bAtTheTop)
{
	if (!FindPage(idx))
		return;
...
//					  Return -1 if an error occurs
//---------------------------------------------------------------------------
i32 CRollupCtrl::MovePageAt(i32 id, i32 newidx)
{
	if (!FindPage(id)) return -1;
	i32 idx = FindPageIndex(id);
...
// Description		:
//---------------------------------------------------------------------------
BOOL CRollupCtrl::IsPageExpanded(i32 idx)
{
	if (!FindPage(idx))
		return FALSE;
...
// Description		:
//---------------------------------------------------------------------------
BOOL CRollupCtrl::IsPageEnabled(i32 idx)
{
	if (!FindPage(idx))
		return FALSE;
...
void CRollupCtrl::RecalLayout()
{
	...
	CRect r;
	GetClientRect(&r);
	i32 BottomPagePos = m_nStartYPos + m_nPageHeight;
...
//---------------------------------------------------------------------------
i32 CRollupCtrl::GetPageIdxFromButtonHWND(HWND hwnd)
{
...
// Description		: Return -1 if an error occurs
//---------------------------------------------------------------------------
RC_PAGEINFO* CRollupCtrl::GetPageInfo(i32 idx)
{
	if (!FindPage(idx))
...
LRESULT CALLBACK CRollupCtrl::DlgWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CTLCOLOR:
		return (LRESULT)_this->m_bkColor;
		break;
	}
...
// OnSize
//---------------------------------------------------------------------------
void CRollupCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CRollupCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if ((nFlags & MK_LBUTTON) && br.PtInRect(point))
	{
		...
		i32 SB_Size = ClientHeight - (((m_nPageHeight - r.Height()) * ClientHeight) / m_nPageHeight);
		i32 SB_Pos = -(m_nStartYPos * ClientHeight) / m_nPageHeight;
...
// OnMouseActivate
//---------------------------------------------------------------------------
i32 CRollupCtrl::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	//Timur
	//SetFocus();
	//Timur
...
RC_PAGEINFO* CRollupCtrl::FindPage(i32 id)
{
	for (i32 i = 0; i < m_PageList.size(); i++)
	{
...
i32 CRollupCtrl::FindPageIndex(i32 id)
{
	for (i32 i = 0; i < m_PageList.size(); i++)
	{
		if (m_PageList[i]->id == id)
