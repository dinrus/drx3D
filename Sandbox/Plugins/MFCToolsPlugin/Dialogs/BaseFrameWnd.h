...
public:
	...
	bool             AutoLoadFrameLayout(const CString& name, i32 nVersion = 0);
	// Every docking pane windows should be registered with this function
	CXTPDockingPane* CreateDockingPane(tukk sPaneTitle, CWnd* pWindow, UINT nID, CRect rc, XTPDockingPaneDirection direction, CXTPDockingPaneBase* pNeighbour = NULL);
...
protected:
	...
	i32                                m_frameLayoutVersion;

	std::vector<std::pair<i32, CWnd*>> m_dockingPaneWindows;
};
