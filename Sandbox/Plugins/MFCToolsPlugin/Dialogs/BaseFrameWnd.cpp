...
//////////////////////////////////////////////////////////////////////////
bool CBaseFrameWnd::AutoLoadFrameLayout(const CString& name, i32 nVersion)
{
...
bool CBaseFrameWnd::LoadFrameLayout()
{
	...
	i32 paneLayoutVersion = regMgr.GetProfileInt(CString(_T("FrameLayout_")) + m_frameLayoutKey, _T("LayoutVersion_"), 0);
...
LRESULT CBaseFrameWnd::OnDockingPaneNotify(WPARAM wParam, LPARAM lParam)
{
	if (wParam == XTP_DPN_SHOWWINDOW)
	{
		...
		{
			i32 id = pwndDockWindow->GetID();
			for (i32 i = 0; i < (i32)m_dockingPaneWindows.size(); i++)
			{
...
//////////////////////////////////////////////////////////////////////////
CXTPDockingPane* CBaseFrameWnd::CreateDockingPane(tukk sPaneTitle, CWnd* pWindow, UINT nID, CRect rc, XTPDockingPaneDirection direction, CXTPDockingPaneBase* pNeighbour /*= NULL */)
{
	CXTPDockingPane* pPane = GetDockingPaneManager()->CreatePane(nID, rc, direction, pNeighbour);
	pPane->SetTitle(sPaneTitle);
	m_dockingPaneWindows.push_back(std::pair<i32, CWnd*>(nID, pWindow));
	return pPane;
}
