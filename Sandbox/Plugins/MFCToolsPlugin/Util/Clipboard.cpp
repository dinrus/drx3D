...
XmlNodeRef CClipboard::Get() const
{
	...
	/*
	   ...
	   if (obj.IsDataAvailable(CF_TEXT)) {
	    ...
	    CDrxMemFile sf((BYTE*) ::GlobalLock(hmem), ::GlobalSize(hmem));
	    string buffer;
...
string CClipboard::GetString() const
{
	...
	if (OpenClipboard(NULL))
	{
		buffer = (tuk)GetClipboardData(CF_TEXT);
	}
	CloseClipboard();
...
bool CClipboard::GetImage(CImageEx& img)
{
	...
	u8* pSrc = (u8*)&pbi->bmiColors;
	u8* pDst = (u8*)img.GetData();

	if (pbi->bmiHeader.biCompression == BI_BITFIELDS)
		pSrc += 3 * sizeof(DWORD);

	i32 stSrc = (pbi->bmiHeader.biBitCount == 24) ? 3 : 4;

	for (i32 y = 0; y < pbi->bmiHeader.biHeight; y++)
		for (i32 x = 0; x < pbi->bmiHeader.biWidth; x++)
		{
