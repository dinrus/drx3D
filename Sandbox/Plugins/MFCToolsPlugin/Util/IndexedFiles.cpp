...
volatile i32 CIndexedFiles::s_bIndexingDone;
...
void CIndexedFiles::RemoveFile(const string& path)
{
	if (m_pathToIndex.find(path) == m_pathToIndex.end())
		return;
	std::map<string, i32>::iterator itr = m_pathToIndex.find(path);
	i32 index = itr->second;
