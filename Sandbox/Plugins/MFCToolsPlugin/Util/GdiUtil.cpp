...
bool CGdiCanvas::BlitTo(HDC hDestDC, i32 aDestX, i32 aDestY, i32 aDestWidth, i32 aDestHeight, i32 aSrcX, i32 aSrcY, i32 aRop)
{
	CDC dc;
	BITMAP bmpInfo;
...
bool CGdiCanvas::BlitTo(HWND hDestWnd, i32 aDestX, i32 aDestY, i32 aDestWidth, i32 aDestHeight, i32 aSrcX, i32 aSrcY, i32 aRop)
{
...
bool CGdiCanvas::GradientFillRect(CRect& rRect, COLORREF aStartColor, COLORREF aEndColor, i32 aFillType)
{
	return ::GradientFillRect(m_memDC.GetSafeHdc(), rRect, aStartColor, aEndColor, aFillType);
}

bool CGdiCanvas::BitBltWithAlpha(CDC& rBmpDC, i32 aDestX, i32 aDestY, i32 aDestWidth, i32 aDestHeight, i32 aSrcX, i32 aSrcY, i32 aSrcWidth, i32 aSrcHeight, BLENDFUNCTION* pBlendFunc)
{
	BLENDFUNCTION bf;
...
void CGdiCanvas::BreakTextByChars(tukk pText, string& rOutStr, const CRect& rRect)
{
	UINT i = 0, nSize = strlen(pText);
	CString chunk;
	CSize textSize;
	i32 rectWidth = rRect.Width();
...
bool CGdiCanvas::ComputeThumbsLayoutInfo(float aContainerWidth, float aThumbWidth, float aMargin, UINT aThumbCount, UINT& rThumbsPerRow, float& rNewMargin)
{
	...
	rThumbsPerRow = (i32) aContainerWidth / (aThumbWidth + aMargin * 2);
...
COLORREF CGdiCanvas::ScaleColor(COLORREF aColor, float aScale)
{
	...
	i32 r = GetRValue(aColor);
	i32 g = GetGValue(aColor);
	i32 b = GetBValue(aColor);
...
void CGdiCanvas::DrawBox(i32 aLeft, i32 aTop, i32 aWidth, i32 aHeight, COLORREF aFillColor1, COLORREF aFillColor2,
...
bool CAlphaBitmap::Load(tukk pFilename, bool bVerticalFlip)
{
	Free();
	...
	for (UINT y = 0; y < bmData.Height; ++y)
	{
		...
		{
			pPixel[0] = ((i32)pPixel[0] * pPixel[3] + 127) >> 8;
			pPixel[1] = ((i32)pPixel[1] * pPixel[3] + 127) >> 8;
			pPixel[2] = ((i32)pPixel[2] * pPixel[3] + 127) >> 8;
			pPixel += 4;
		}
	}
...
bool CAlphaBitmap::Save(tukk pFileName)
{
	BITMAP bp;
	...
	i32 imgSize = bp.bmWidthBytes * bp.bmHeight;
	_smart_ptr<IMemoryBlock> imgBuffer = gEnv->pDrxPak->PoolAllocMemoryBlock(imgSize, "");
	i32 returnedBytes = m_bmp.GetBitmapBits(imgSize, imgBuffer->GetData());
	if (returnedBytes == 0)
		return false;
	...
	wstring wideFileName = DrxStringUtils::UTF8ToWStr(pFileName);
...
bool CAlphaBitmap::GetCLSIDObjFromFileExt(tukk fileName, CLSID* pClsid) const
{
	if (pClsid == NULL)
		return false;
...
bool CAlphaBitmap::Create(uk pData, UINT aWidth, UINT aHeight, bool bVerticalFlip, bool bPremultiplyAlpha)
{
	Free();
...
i32 CAlphaBitmap::GetEncoderClsid(tukk format, CLSID* pClsid) const
{
	UINT num = 0;
	UINT size = 0;

	wstring wformat = DrxStringUtils::UTF8ToWStr(format);
...
bool GradientFillRect(HDC hDC, CRect& rRect, COLORREF aStartColor, COLORREF aEndColor, i32 aFillType)
{
	TRIVERTEX vert[2];
	GRADIENT_RECT gRect;
