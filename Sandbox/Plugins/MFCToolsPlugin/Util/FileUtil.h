...
public:
	...
	typedef Functor1<tukk >                       SelectSingleFileChangeCallback;
	...
	static bool ScanDirectory(tukk path, tukk fileSpec, FileArray& files, bool recursive = true, bool addDirAlso = false, ScanDirectoryUpdateCallBack updateCB = NULL, bool bSkipPaks = false) // for CString conversion
	{
	...
	static bool CompileLuaFile(tukk luaFilename);
	static bool ExtractFile(CString& file, bool bMsgBoxAskForExtraction = true, tukk pDestinationFilename = NULL);
	static void EditTextFile(tukk txtFile, i32 line = 0, ETextFileType fileType = FILE_TYPE_SCRIPT, bool bUseGameFolder = true);
	static void EditTextureFile(tukk txtureFile, bool bUseGameFolder);
	static bool EditMayaFile(tukk mayaFile, const bool bExtractFromPak, const bool bUseGameFolder);
	static bool EditFile(tukk filePath, const bool bExtrackFromPak, const bool bUseGameFolder);
	...
	static bool SelectSingleFile(ECustomFileType fileType, string& outputFile, tukk filter = "", tukk initialDir = "", tukk baseDir = "", const FileFilterCallback& fileFilterFunc = FileFilterCallback());
	...
	static bool SelectSingleFile(QWidget* pParent, ECustomFileType fileType, string& outputFile, tukk filter = "", tukk initialDir = "", tukk baseDir = "", const FileFilterCallback& fileFilterFunc = FileFilterCallback());
	...
	//! Display OpenFile dialog and allow to select multiple files.
	//! @return true if selected, false if canceled.
	static bool SelectMultipleFiles(ECustomFileType fileType, std::vector<string>& files, tukk filter = "", tukk initialDir = "", tukk baseDir = "", const FileFilterCallback& fileFilterFunc = FileFilterCallback()) // for CString conversion
	{
	...
	static bool SelectSaveFile(tukk fileFilter, tukk defaulExtension, tukk startFolder, string& fileName);
	...
	//! If yes file is deleted.
	//! @return True if file was deleted.
	static bool OverwriteFile(tukk filename) { return OverwriteFile(CString(filename)); } // for CString conversion
	...
	static bool CheckinFile(tukk filename);

	//! Adding the file to Source Control changelist without submitting.
	static bool AddFileToSourceControl(tukk filename);

	//! Creates this directory.
	static void CreateDirectory(tukk dir);

	//! Makes a backup file.
	static void BackupFile(tukk filename);
	...
	static void BackupFileDated(tukk filename, bool bUseBackupSubDirectory = false);

	// ! Added deltree as a copy from the function found in Drxpak.
	static bool Deltree(tukk szFolder, bool bRecurse);
	...
	static bool FileExists(tukk strFilePath, FileDesc* pDesc = NULL) { return FileExists(CString(strFilePath), pDesc); } // for CString conversion
	static bool PathExists(const CString& strPath);
	static bool PathExists(tukk strPath)                             { return PathExists(CString(strPath)); }
	static bool GetDiskFileSize(tukk pFilePath, uint64& rOutSize);
	...
	// Creates the entire path, if needed.
	static bool CreatePath(tukk strPath) { return CreatePath(CString(strPath)); }
	...
	// @param LPPROGRESS_ROUTINE pfnProgress - called by the system to notify of file copy progress
	// @param LPBOOL pbCancel - when the contents of this BOOL are set to TRUE, the system cancels the copy operation
	static ECopyTreeResult CopyFile(tukk strSourceFile, tukk strTargetFile, bool boConfirmOverwrite = false, LPPROGRESS_ROUTINE pfnProgress = NULL, LPBOOL pbCancel = NULL)
	{
	...
	struct ExtraMenuItems
	{
		...
		i32                  selectedIndexIfAny;

		ExtraMenuItems() : selectedIndexIfAny(-1) {}

		i32 AddItem(const CString& name)
		{
	...
	static void PopupMenu(tukk filename, tukk fullGamePath, CWnd* wnd = nullptr, bool* pIsSelected = nullptr, ExtraMenuItems* pItems = nullptr);
	...
	static u32 GetAttributes(tukk filename, bool bUseSourceControl = true);
...
{
public:
	CTempFileHelper(tukk pFileName);
	~CTempFileHelper();
