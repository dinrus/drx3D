...
void CopySourceControlPathToClipboard(tukk path)
{
...
bool GetLatestVersion(tukk path)
{
	if (GetIEditor()->IsSourceControlAvailable())
	{
...
bool AddToSourceControl(tukk path)
{
	if (GetIEditor()->IsSourceControlAvailable())
	{
		CSourceControlDescDlg dlg;
...
void SelectExtraItemFromPopup(CFileUtil::ExtraMenuItems* pItems, u32 id)
{
	if (!pItems)
	{
		return;
...
void OpenFromPopup(const CString& fullPath, u32 fileAttr)
{
...
void ExploreFromPopup(const CString& fullPath, const CString& filename, u32 fileAttr)
{
	if (fileAttr & SCC_FILE_ATTRIBUTE_INPAK)
	{
...
static std::unique_ptr<CDynamicPopupMenu> CreateDynamicPopupMenu(tukk pFilename, tukk pFullGamePath, bool* pIsSelected = nullptr, CFileUtil::ExtraMenuItems* pItems = nullptr, const std::function<void()>& createMessageBoxFunctor = std::function<void()>())
{
	...
	const CString fullPath(CString(currentDir.toStdString().c_str()) + CString("\\") + PathUtil::GamePathToDrxPakPath(path.GetString()).c_str());
	u32k fileAttr(GetIEditor()->IsSourceControlAvailable() ? CFileUtil::GetAttributes(path) : SCC_FILE_ATTRIBUTE_INVALID);
	...
	if (bHasExtraItems)
	{
		...
		for (size_t i = 0; i < pItems->names.size(); ++i)
		{
			...
			{
				root.Add<CFileUtil::ExtraMenuItems*, u32>(pItems->names[i], functor(&SelectExtraItemFromPopup), pItems, i);
			}
			root.AddSeparator();
		}
	}

	auto& openMenuItem = root.Add<const CString&, u32>(QObject::tr("Open").toStdString().c_str(), functor(&OpenFromPopup), fullPath, fileAttr);
	auto& exploreMenuItem = root.Add<const CString&, const CString&, u32>(QObject::tr("Explore").toStdString().c_str(), functor(&ExploreFromPopup), fullPath, filename, fileAttr);
	if (fileAttr & SCC_FILE_ATTRIBUTE_INPAK)
	{
...
void PyShowInExplorer(tukk path)
{
...
bool CFileUtil::CompileLuaFile(tukk luaFilename)
{
	...
	if (strlen(CompilerOutput) > 0)
	{
		...
		if (DrxMessageBox((CString("Error output from Lua compiler:\r\n") + CompilerOutput.GetString() +
		                      CString("\r\nDo you want to edit the file ?")), "Lua Compiler", eMB_YesCancel) == eQR_Yes)
		{
			i32 line = 0;
			string cmdLine = luaFile;
			i32 index = CompilerOutput.Find("at line");
			if (index >= 0)
			{
				tukk str = CompilerOutput;
...
//////////////////////////////////////////////////////////////////////////
bool CFileUtil::ExtractFile(CString& file, bool bMsgBoxAskForExtraction, tukk pDestinationFilename)
{
	CDrxFile cryfile;
	if (cryfile.Open(file, "rb"))
	{
		...
		if (cryfile.IsInPak())
		{
			...
			if (diskFile.Open(file, CFile::modeCreate | CFile::modeWrite))
			{
				...
				tuk data = new char[cryfile.GetLength()];
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Failed to create file %s on disk", (tukk)file);
			}
		}
		else
		{
...
//////////////////////////////////////////////////////////////////////////
void CFileUtil::EditTextFile(tukk txtFile, i32 line, ETextFileType fileType, bool bUseGameFolder)
{
	...
	if (line != 0)
	{
		cmd.Format("%s/%d/0", (tukk)file, line);
	}
	else
	{
		cmd = file;
...
//////////////////////////////////////////////////////////////////////////
void CFileUtil::EditTextureFile(tukk txtureFile, bool bUseGameFolder)
{
	...
	{
		//failed
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Failed to open texture editor %s with file %s", gEditorFilePreferences.textureEditor.c_str(), fullTexturePath.c_str());
	}
}

//////////////////////////////////////////////////////////////////////////
bool CFileUtil::EditMayaFile(tukk filepath, const bool bExtractFromPak, const bool bUseGameFolder)
{
	...
	if (bUseGameFolder)
	{
		...
		i32 nLength = sGameFolder.GetLength();
	...
	if ((DWORD_PTR)hInst <= 32)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Can't open the file. You can specify a source editor in Sandbox Preferences or create an association in Windows.");
	}
	return TRUE;
}

//////////////////////////////////////////////////////////////////////////
bool CFileUtil::EditFile(tukk filePath, const bool bExtrackFromPak, const bool bUseGameFolder)
{
...
void CFileUtil::FormatFilterString(CString& filter)
{
	...
	i32k numPipeChars = std::count(s.begin(), s.end(), '|');
	if (numPipeChars == 1)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CFileUtil::SelectSaveFile(tukk fileFilter, tukk defaulExtension, tukk startFolder, string& fileName)
{
...
//////////////////////////////////////////////////////////////////////////
bool CFileUtil::SelectSingleFile(QWidget* pParent, ECustomFileType fileType, string& outputFile, tukk filter, tukk initialDir, tukk baseDir, const FileFilterCallback& fileFilterFunc)
{
...
bool CFileUtil::SelectSingleFile(ECustomFileType fileType, string& outputFile, tukk filter, tukk initialDir, tukk baseDir, const FileFilterCallback& fileFilterFunc)
{
...
inline bool ScanDirectoryFiles(const CString& root, const CString& path, const CString& fileSpec, std::vector<CFileUtil::FileDesc>& files, bool bSkipPaks)
{
	...
	IDrxPak* pIPak = GetIEditor()->GetSystem()->GetIPak();
...
// Get directory contents.
//////////////////////////////////////////////////////////////////////////
inline i32 ScanDirectoryRecursive(const CString& root, const CString& path, const CString& fileSpec, std::vector<CFileUtil::FileDesc>& files, bool recursive, bool addDirAlso, CFileUtil::ScanDirectoryUpdateCallBack updateCB, bool bSkipPaks)
{
	bool anyFound = false;
	CString dir = PathUtil::AddBackslash((root + path).GetString());
	...
	if (recursive)
	{
		...
		if (fhandle != -1)
		{
			do
			{
				...
				// Scan directory.
				i32 result = ScanDirectoryRecursive(root, PathUtil::AddBackslash((path + fd.name).GetString()).c_str(), fileSpec, files, recursive, addDirAlso, updateCB, bSkipPaks);
				if (result == -1)
...
{
	// check if file exist.
	CString adjFileName = PathUtil::GamePathToDrxPakPath(filename.GetString(), true);
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Cannot Save File %s", adjFileName);
		return false;
	}
...
/*
   static bool CheckOutFile( tukk filename )
   {
...
//////////////////////////////////////////////////////////////////////////
bool CFileUtil::CheckoutFile(tukk filename)
{
	/*
	...
	u32 attr = CFileUtil::GetAttributes(filename);
	...
	else
	{
		...
		{
			CString str;
			str.Format("File %s is Read-Only, Overwrite?", (tukk)filename);
...
//////////////////////////////////////////////////////////////////////////
bool CFileUtil::CheckinFile(tukk filename)
{
	/*
	   SetEnvironmentVariable( "ssuser",gSettings.ssafeParams.user );
	   SetEnvironmentVariable( "ssdir",gSettings.ssafeParams.databasePath );
...
//////////////////////////////////////////////////////////////////////////
bool CFileUtil::AddFileToSourceControl(tukk filename)
{
	if (GetIEditor()->IsSourceControlAvailable())
	{
		u32 attr = CFileUtil::GetAttributes(filename);
...
static bool CheckAndCreateDirectory(tukk path)
{
...
//////////////////////////////////////////////////////////////////////////
void CFileUtil::CreateDirectory(tukk directory)
{
...
//////////////////////////////////////////////////////////////////////////
void CFileUtil::BackupFile(tukk filename)
{
...
//////////////////////////////////////////////////////////////////////////
void CFileUtil::BackupFileDated(tukk filename, bool bUseBackupSubDirectory /*=false*/)
{
	bool makeBackup = true;
	{
		// Check if backup needed.
...
//////////////////////////////////////////////////////////////////////////
bool CFileUtil::Deltree(tukk szFolder, bool bRecurse)
{
...
bool CFileUtil::GetDiskFileSize(tukk pFilePath, uint64& rOutSize)
{
	BOOL bOK;
	...
	bOK = ::GetFileAttributesEx(pFilePath, GetFileExInfoStandard, (uk )&fileInfo);
...
u32 CFileUtil::GetAttributes(tukk filename, bool bUseSourceControl /*= true*/)
{
...
std::vector<string> CFileUtil::PickTagsFromPath(const string& path)
{
	...
	while (tag != "")
	{
		i32k MIN_CHARS_AS_A_SEARCH_TERM = 3;
...
CTempFileHelper::CTempFileHelper(tukk pFileName)
