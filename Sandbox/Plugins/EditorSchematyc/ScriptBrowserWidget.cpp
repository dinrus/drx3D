...
#include <DrxSchematyc/SerializationUtils/IValidatorArchive.h>
...
namespace
{
static tukk g_szAddIcon = "icons:schematyc/add.png";
static tukk g_szRemoveIcon = "icons:General/Remove_Item.ico";
static tukk g_szHomeIcon = "icons:schematyc/home.png";
static tukk g_szBackIcon = "icons:schematyc/back.png";
static tukk g_szForwardIcon = "icons:schematyc/forward.png";
static tukk g_szFileNewIcon = "icons:schematyc/file_new.png";
static tukk g_szFileLocalIcon = "icons:schematyc/file_local.png";
static tukk g_szFileManagedIcon = "icons:schematyc/file_managed.png";
static tukk g_szFileCheckedOutIcon = "icons:schematyc/file_checked_out.png";
static tukk g_szFileAddIcon = "icons:schematyc/file_add.png";
static tukk g_szScriptRootIcon = "icons:schematyc/script_module.png";

const QSize g_defaultColumnSize[EScriptBrowserColumn::COUNT] = { QSize(100, 20), QSize(20, 20) };

tukk g_szClipboardPrefix = "[schematyc_xml]";
}
...
CScriptBrowserItem::CScriptBrowserItem(EScriptBrowserItemType type, tukk szName, tukk szIcon)
	: m_type(type)
	, m_name(szName)
	, m_iconName(szIcon)
...
CScriptBrowserItem::CScriptBrowserItem(EScriptBrowserItemType type, tukk szName, EFilterType filterType)
	: m_type(type)
	, m_name(szName)
...
CScriptBrowserItem::CScriptBrowserItem(EScriptBrowserItemType type, tukk szName, tukk szIcon, IScriptElement* pScriptElement)
	: m_type(type)
	, m_name(szName)
	, m_iconName(szIcon)
	, m_filterType(EFilterType::None)
...
void CScriptBrowserItem::SetName(tukk szName)
{
	if (m_pScriptElement)
	{
...
tukk CScriptBrowserItem::GetName() const
{
	return m_name.c_str();
}

tukk CScriptBrowserItem::GetPath() const
{
	return m_path.c_str();
}

tukk CScriptBrowserItem::GetIcon() const
{
	return m_iconName.c_str();
}

tukk CScriptBrowserItem::GetTooltip() const
{
	return m_tooltip.c_str();
}

tukk CScriptBrowserItem::GetSortString() const
{
	return m_sortString.c_str();
}

void CScriptBrowserItem::SetSortString(tukk szSort)
{
	m_sortString = szSort;
}

tukk CScriptBrowserItem::GetFilterString() const
{
	return m_filterString.c_str();
}

void CScriptBrowserItem::SetFilterString(tukk szFilter)
{
...
tukk CScriptBrowserItem::GetFileText() const
{
...
tukk CScriptBrowserItem::GetFileIcon() const
{
...
i32 CScriptBrowserItem::GetChildCount() const
{
	return static_cast<i32>(m_children.size());
}

i32 CScriptBrowserItem::GetChildIdx(CScriptBrowserItem* pChild)
{
	for (i32 childIdx = 0, childCount = m_children.size(); childIdx < childCount; ++childIdx)
	{
...
CScriptBrowserItem* CScriptBrowserItem::GetChildByIdx(i32 childIdx)
{
	return childIdx < m_children.size() ? m_children[childIdx].get() : nullptr;
}

CScriptBrowserItem* CScriptBrowserItem::GetChildByTypeAndName(EScriptBrowserItemType type, tukk szName)
{
	if (szName)
	{
...
void CScriptBrowserItem::Validate()
{
	if (m_pScriptElement)
	{
		...
		{
			auto collectWarnings = [this](EValidatorMessageType messageType, tukk szMessage)
			{
		...
		{
			auto collectErrors = [this](EValidatorMessageType messageType, tukk szMessage)
			{
...
CScriptBrowserModel::CScriptBrowserModel(DrxSchematycEditor::CMainWindow& editor, CAsset& asset, const DrxGUID& assetGUID)
...
QModelIndex CScriptBrowserModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
	if (parent.isValid())
	{
...
i32 CScriptBrowserModel::rowCount(const QModelIndex& parent) const
{
	if (parent.isValid())
	{
		CScriptBrowserItem* pParentItem = ItemFromIndex(parent);
...
i32 CScriptBrowserModel::columnCount(const QModelIndex& parent) const
{
...
QVariant CScriptBrowserModel::data(const QModelIndex& index, i32 role) const
{
	CScriptBrowserItem* pItem = ItemFromIndex(index);
	if (pItem)
	{
...
bool CScriptBrowserModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
	if (role == Qt::EditRole)
	{
		CScriptBrowserItem* pItem = ItemFromIndex(index);
		if (pItem)
		{
			tukk szNewName = value.toString().toStdString().c_str();
			...
			DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject(desc.c_str(), m_editor);
...
QVariant CScriptBrowserModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
QVariant CScriptBrowserModel::GetHeaderData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
QModelIndex CScriptBrowserModel::ItemToIndex(CScriptBrowserItem* pItem, i32 column) const
{
...
CScriptBrowserItem* CScriptBrowserModel::GetItemByGUID(const DrxGUID& guid)
{
	ItemsByGUID::iterator itItem = m_itemsByGUID.find(guid);
	return itItem != m_itemsByGUID.end() ? itItem->second : nullptr;
}

CScriptBrowserItem* CScriptBrowserModel::GetItemByFileName(tukk szFileName)
{
...
void CScriptBrowserModel::Populate()
{
	...
	DRX_ASSERT_MESSAGE(pScripElement, "Couldn't find Schematyc asset!");
	// ~TODO
	if (pScripElement)
	{
...
CScriptBrowserItem* CScriptBrowserModel::CreateScriptElementItem(IScriptElement& scriptElement, const ScriptBrowserItemFlags& flags, CScriptBrowserItem* pParentItem)
{
	...
	DRX_ASSERT_MESSAGE(scriptElement.GetType() == Schematyc::EScriptElementType::Class || scriptElement.GetType() == Schematyc::EScriptElementType::Module || pParentItem, "Error adding script element to model because of a missing parent element.");
	if (pParentItem)
	{
		// Create item.
		stack_string filter;
...
CScriptBrowserItem* CScriptBrowserModel::AddItem(CScriptBrowserItem& parentItem, const CScriptBrowserItemPtr& pItem)
{
	...
	i32k row = parentItem.GetChildCount();
...
CScriptBrowserItem* CScriptBrowserModel::GetFilter(tukk szName) const
{
...
bool CScriptBrowserModel::HasUnsavedChanged()
{
	...
	hasUnsavedChanges = [&hasUnsavedChanges](CScriptBrowserItem* pItem) -> bool
	{
		...
		{
			for (u32 i = pItem->GetChildCount(); i--; )
			{
...
CScriptBrowserWidget::CScriptBrowserWidget(DrxSchematycEditor::CMainWindow& editor)
	: QWidget(&editor)
	, m_editor(editor)
	...
	, m_pAsset(nullptr)
{
	...
	u32k behaviorFlags = QAdvancedTreeView::PreserveExpandedAfterReset | QAdvancedTreeView::PreserveSelectionAfterReset | QAdvancedTreeView::UseItemModelAttribute;
...
void CScriptBrowserWidget::SelectItem(const DrxGUID& guid)
{
	if (m_pModel)
	{
...
DrxGUID CScriptBrowserWidget::GetSelectedItemGUID() const
{
...
void CScriptBrowserWidget::OnAddItem(IScriptElement* pScriptElement, EScriptElementType elementType)
{
	if (m_pModel)
	{
		...
		case EScriptElementType::Enum:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Enumeration", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::Struct:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Struct", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::Signal:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Signal", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::Function:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Function Graph", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::Interface:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Interface", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::InterfaceFunction:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Interface Function", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::InterfaceTask:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Interface Task", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::StateMachine:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added State Machine", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::State:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added State", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::Variable:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Variable", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::Timer:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Timer", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::SignalReceiver:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Signal Receiver", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::InterfaceImpl:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Interface Implementation", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::ComponentInstance:
			{
				GetIEditor()->GetIUndoManager()->Begin();
				DrxSchematycEditor::CScriptUndoObject* pUndoObject = new DrxSchematycEditor::CScriptUndoObject("Added Component", m_editor);
				CUndo::Record(pUndoObject);
				GetIEditor()->GetIUndoManager()->Accept(pUndoObject->GetDescription());
		...
		case EScriptElementType::ActionInstance:
			{
				DRX_ASSERT_MESSAGE(false, "Actions not supported!");
				break;
			}
		}
	...
	if (m_pModel)
	{
		...
		if (pItem)
		{
			...
			if (pScriptElement)
			{
				...
				{
					DrxSchematycEditor::Utils::WriteToClipboard(xml->getXMLData()->GetString(), g_szClipboardPrefix);
				}
			}
		}
	}
	...
	if (m_pModel)
	{
		...
		if (pItem)
		{
			...
			if (pScriptElement)
			{
				...
				if (DrxSchematycEditor::Utils::ReadFromClipboard(clipboardText, g_szClipboardPrefix))
				{
...
void CScriptBrowserWidget::PopulateAddMenu(QMenu* pMenu, IScriptElement* pScriptScope)
{
	if (pScriptScope)
	{
		...
		/*if (ScriptBrowserUtils::CanAddScriptElement(EScriptElementType::Import, pScriptScope))
		   ...
		   pAddImportAction->setData(static_cast<i32>(EScriptElementType::Import));
		...
		/*if (ScriptBrowserUtils::CanAddScriptElement(EScriptElementType::Struct, pScriptScope))
		   ...
		   pAddStructAction->setData(static_cast<i32>(EScriptElementType::Struct));
		...
		/*if (ScriptBrowserUtils::CanAddScriptElement(EScriptElementType::InterfaceTask, pScriptScope))
		   ...
		   pAddInterfaceTask->setData(static_cast<i32>(EScriptElementType::InterfaceTask));
		...
		/*if (ScriptBrowserUtils::CanAddScriptElement(EScriptElementType::Property, pScriptScope))
		   ...
		   pAddPropertyAction->setData(static_cast<i32>(EScriptElementType::Property));
		...
		/*if (ScriptBrowserUtils::CanAddScriptElement(EScriptElementType::InterfaceImpl, pScriptScope))
		   ...
		   pAddInterfaceImplAction->setData(static_cast<i32>(EScriptElementType::InterfaceImpl));
