...
void CPreviewWidget::SetClass(const IScriptClass* pScriptClass)
{
	...
	if (pScriptClass)
	{
		DRX_ASSERT_MESSAGE(pScriptClass->GetType() == Schematyc::EScriptElementType::Class, "Unsupported element type.");
...
void CPreviewWidget::SetComponentInstance(const IScriptComponentInstance* pComponentInstance)
{
	...
	{
		m_componentInstanceGUID = DrxGUID();
	...
	if (pComponentInstance && pComponentInstance->HasTransform())
	{
		if (pComponentInstance->GetGUID() != m_componentInstanceGUID)
		{
			...
			auto onBeginDrag = [this](IDisplayViewport*, ITransformManipulator*, const Vec2i&, i32)
			{
			...
			auto onDrag = [this](IDisplayViewport*, ITransformManipulator*, const Vec2i&, const Vec3& offset, i32)
			{
...
XmlNodeRef CPreviewWidget::SaveSettings(tukk szName)
{
