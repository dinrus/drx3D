...
CPropertiesWidget::CPropertiesWidget(DrxGraphEditor::GraphItemSet& items, CMainWindow* pEditor)
	: m_pEditor(pEditor)
	, m_pPreview(nullptr)
	, m_isPushingUndo(false)
{
	...
	for (DrxGraphEditor::CAbstractNodeGraphViewModelItem* pAbstractItem : items)
	{
...
void CPropertiesWidget::OnContentDeleted(DrxGraphEditor::CAbstractNodeGraphViewModelItem* pDeletedItem)
{
	...
	auto iter = std::find_if(m_structs.begin(), m_structs.end(), [pDeletedItem](const Serialization::SStruct& a)
		{
			return a.cast<DrxGraphEditor::CAbstractNodeGraphViewModelItem>() == pDeletedItem;
	  });
	DRX_ASSERT(iter != m_structs.end());
...
CPropertiesWidget::~CPropertiesWidget()
{
	...
	auto iter = std::for_each(m_structs.begin(), m_structs.end(), [this](Serialization::SStruct& a)
		{
			DrxGraphEditor::CAbstractNodeGraphViewModelItem* pDeletedItem = a.cast<DrxGraphEditor::CAbstractNodeGraphViewModelItem>();
			if (pDeletedItem)
				pDeletedItem->SignalDeletion.DisconnectObject(this);
	  });
}
