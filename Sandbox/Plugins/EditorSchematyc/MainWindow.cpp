...
#include <DrxSchematyc/SerializationUtils/SerializationToString.h>
...
static tukk g_szSaveIcon = "icons:General/File_Save.ico";
static tukk g_szSettingsFolder = "schematyc";
static tukk g_szSettingsFile = "settings.json";
...
void CMainWindow::Show(const DrxGUID& elementGUID, const DrxGUID& detailGUID)
{
	if (m_pScriptBrowser)
	{
...
bool CMainWindow::RestoreUndo(const XmlNodeRef& input)
{
	...
	QPoint graphPos;
	DrxGraphEditor::GraphItemIds selectedItemIds;
	if (m_pGraphView)
	{
		graphPos = m_pGraphView->GetPosition();
		for (DrxGraphEditor::CAbstractNodeGraphViewModelItem* pItem : m_pGraphView->GetSelectedItems())
		{
...
{
	IDrxSchematycCore* pSchematycCore = gEnv->pSchematyc;
	Schematyc::IScriptRegistry& scriptRegistry = pSchematycCore->GetScriptRegistry();

	const size_t fileCount = pAsset->GetFilesCount();
	DRX_ASSERT_MESSAGE(fileCount > 0, "Schematyc Editor: Asset '%s' has no files.", pAsset->GetName());
...
bool CMainWindow::OnAboutToCloseAsset(string& reason) const
{
	...
	{
		DRX_ASSERT(GetAssetBeingEdited());
...
void CMainWindow::InitToolbar(QVBoxLayout* pWindowLayout)
{
...

	{
		...
		{
			QAction* pAction = pToolBar->addAction(DrxIcon("icons:General/File_Save.ico"), QString());
		...
		{
			m_pClearLogToolbarAction = pToolBar->addAction(DrxIcon("icons:schematyc/toolbar_clear_log.ico"), QString());
		...
		{
			m_pShowLogSettingsToolbarAction = pToolBar->addAction(DrxIcon("icons:schematyc/toolbar_log_settings.ico"), QString());
		...
		{
			m_pShowPreviewSettingsToolbarAction = pToolBar->addAction(DrxIcon("icons:schematyc/toolbar_preview_settings.ico"), QString());
...
void CMainWindow::SaveSettings()
{
	/*{
	   ...
	   gEnv->pDrxPak->MakeDir(szSettingsFolder.c_str());
	   }
	   {
	   const Schematyc::CStackString settingsFileName = GetSettingsFileName();
