...
public:
	...
	QModelIndex GetIndexForItem(const CAbstractVariablesModelItem& item, i32 column = 0) const
	{
	...
	, m_pDataModel(nullptr)
{
	...
	m_pVariablesList->setTreePosition(static_cast<i32>(CVariablesModel::Column_Name));
	m_pVariablesList->setEditTriggers(QTreeView::DoubleClicked | QTreeView::EditKeyPressed | QTreeView::SelectedClicked);
	m_pVariablesList->sortByColumn(static_cast<i32>(CVariablesModel::Column_Name), Qt::AscendingOrder);
...
void CVariablesWidget::SetModel(CAbstractVariablesModel* pModel)
{
	if (m_pModel != pModel)
	{
		...
		if (pModel)
		{
			...
			m_pFilterProxy->setFilterKeyColumn(static_cast<i32>(CVariablesModel::Column_Name));
...
void CVariablesWidget::OnSelectionChanged(const QItemSelection& selected, const QItemSelection& deselected)
{
	...
	if (selections.count())
	{
		...
		if (row != -1)
		{
			...
			{
				DrxSchematycEditor::CPropertiesWidget* pPropertiesWidget = nullptr /*new DrxSchematycEditor::CPropertiesWidget(*pItem)*/;
