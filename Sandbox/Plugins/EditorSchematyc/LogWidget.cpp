...
#include <DrxSystem/IDrxLink.h>
#include <drx3D/CoreX/Serialization/IArchiveHost.h>
#include <QAdvancedPropertyTree.h>
#include <DrxSchematyc/Script/IScriptRegistry.h>
#include <DrxSchematyc/Services/ILog.h>
#include <DrxSchematyc/Services/ILogRecorder.h>
...
void FormatMessage(const SLogSettings& settings, QString& output, const CLogMetaData* pMetaData, ELogMessageType messageType, tukk szMessage)
{
	...
	, bShowOrigin(false)
{
	...
	tukk szStreamName = log.GetStreamName(LogStreamId::Default);
...
//{
//	const IEntity* pEntity = gEnv->pEntitySystem->GetEntity(DrxSchematycEditor::Utils::GetSelectedEntityId());
...
void CLogWidget::OnLinkClicked(const QUrl& link)
{
	...
	DrxLinkUtils::ExecuteUri(qBytes.constData());
}
