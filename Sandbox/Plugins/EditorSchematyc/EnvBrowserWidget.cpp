...
namespace
{
tukk GetElementIcon(EEnvElementType elementType)
{
	return "";
}
}

CEnvBrowserItem::CEnvBrowserItem(const DrxGUID& guid, tukk szName, tukk szIcon)
	: m_guid(guid)
	, m_name(szName)
	, m_iconName(szIcon)
	, m_pParent(nullptr)
...
DrxGUID CEnvBrowserItem::GetGUID() const
{
	return m_guid;
}

tukk CEnvBrowserItem::GetName() const
{
	return m_name.c_str();
}

tukk CEnvBrowserItem::GetIcon() const
{
...
i32 CEnvBrowserItem::GetChildCount() const
{
	return static_cast<i32>(m_children.size());
}

i32 CEnvBrowserItem::GetChildIdx(CEnvBrowserItem* pChild)
{
...
CEnvBrowserItem* CEnvBrowserItem::GetChildByIdx(i32 childIdx)
{
...
public:
	...
	inline void SetFilterText(tukk szFilterText)
	{
...
private:
	...
	inline bool FilterItemByTextRecursive(CEnvBrowserItem* pItem) const   // #SchematycTODO : Keep an eye on performance of this test!
	{
		...
		for (i32 childIdx = 0, childCount = pItem->GetChildCount(); childIdx < childCount; ++childIdx)
		{
...
QModelIndex CEnvBrowserModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
...
i32 CEnvBrowserModel::rowCount(const QModelIndex& parent) const
{
	CEnvBrowserItem* pParentItem = ItemFromIndex(parent);
	if (pParentItem)
	{
...
i32 CEnvBrowserModel::columnCount(const QModelIndex& parent) const
{
...
QVariant CEnvBrowserModel::data(const QModelIndex& index, i32 role) const
{
	CEnvBrowserItem* pItem = ItemFromIndex(index);
	if (pItem)
	{
...
bool CEnvBrowserModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
	return false;
}

QVariant CEnvBrowserModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
QModelIndex CEnvBrowserModel::ItemToIndex(CEnvBrowserItem* pItem, i32 column) const
{
	if (pItem)
	{
		CEnvBrowserItem* pParentItem = pItem->GetParent();
...
CEnvBrowserItem* CEnvBrowserModel::ItemFromGUID(const DrxGUID& guid) const
{
...
void CEnvBrowserModel::Populate()
{
	auto visitEnvElement = [this](const IEnvElement& envElement) -> EVisitStatus
	{
		...
		const DrxGUID guid = envElement.GetGUID();
