...
QVariant CNodesDictionaryNodeEntry::GetColumnValue(i32 columnIndex) const
{
	switch (columnIndex)
	{
...
const QIcon* CNodesDictionaryNodeEntry::GetColumnIcon(i32 columnIndex) const
{
	switch (columnIndex)
	{
	case CNodesDictionary::eColumn_Name:
...
QVariant CNodesDictionaryCategoryEntry::GetColumnValue(i32 columnIndex) const
{
	switch (columnIndex)
	{
	case CNodesDictionary::eColumn_Name:
...
const CAbstractDictionaryEntry* CNodesDictionaryCategoryEntry::GetChildEntry(i32 index) const
{
	if (index < m_categories.size())
	{
		return static_cast<const CAbstractDictionaryEntry*>(m_categories[index]);
	}
	i32k nodeIndex = index - m_categories.size();
...
const CAbstractDictionaryEntry* CNodesDictionary::GetEntry(i32 index) const
{
	if (index < m_categories.size())
	{
		return static_cast<const CAbstractDictionaryEntry*>(m_categories[index]);
...
const CItemModelAttribute* CNodesDictionary::GetColumnAttribute(i32 index) const
{
...
public:
	...
	virtual bool AddCommand(const Schematyc::IScriptGraphNodeCreationCommandPtr& pCommand)
	{
		...
		if (pCommand)
		{
			tukk szBehavior = pCommand->GetBehavior();
			tukk szSubject = pCommand->GetSubject();
			tukk szDescription = pCommand->GetDescription();
			tukk szStyleId = pCommand->GetStyleId();
	...
	void AddRecursive(const QString& name, QStringList& categories, CNodesDictionaryCategoryEntry* pParentCategory, tukk szStyleId = nullptr)
	{
		const DrxGraphEditor::CNodeGraphViewStyle* pStyle = m_dictionary.m_pStyle;
		...
		if (categories.size() > 0)
		{
			...
			if (pStyle)
			{
				if (const DrxGraphEditor::CNodeWidgetStyle* pNodeStyle = pStyle->GetNodeWidgetStyle(szStyleId))
				{
...
{
	DrxGraphEditor::CNodeGraphViewStyle* pViewStyle = new DrxGraphEditor::CNodeGraphViewStyle("Schematyc");
	...
	for (u32 i = 0; i < dataTypesModel.GetTypeItemsCount(); ++i)
	{
