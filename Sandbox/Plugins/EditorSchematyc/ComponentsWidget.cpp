	...
	, m_pFilterProxy(nullptr)
{
	...
	m_pComponentsList->sortByColumn(static_cast<i32>(CComponentsModel::Column_Name), Qt::AscendingOrder);
...
void CComponentsWidget::SetModel(CAbstractComponentsModel* pModel)
{
	if (m_pModel != pModel)
	{
		...
		if (pModel)
		{
			...
			m_pFilterProxy->setFilterKeyColumn(static_cast<i32>(CComponentsModel::Column_Name));
