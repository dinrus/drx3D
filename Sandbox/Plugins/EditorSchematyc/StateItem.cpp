...
const DrxIcon*  CStateItem::GetIcon() const
{
	std::unique_ptr<DrxIcon> pIcon;
	if (pIcon.get() == nullptr)
	{
		pIcon = stl::make_unique<DrxIcon>("icons:schematyc/script_state.png");
	}

	return pIcon.get();
}

u32 CStateItem::GetNumChildItems() const
{
	return m_graphs.size() + m_states.size();
}

CAbstractObjectStructureModelItem* CStateItem::GetChildItemByIndex(u32 index) const
{
...
u32 CStateItem::GetIndex() const
{
	DRX_ASSERT(m_pParentItem);
	if (m_pParentItem)
	{
...
u32 CStateItem::GetChildItemIndex(const CAbstractObjectStructureModelItem& item) const
{
	switch (item.GetType())
	{
...
CGraphItem* CStateItem::CreateGraph(CGraphItem::EGraphType type)
{
	...
	case CGraphItem::eGraphType_SignalReceiver:
		...
		break;
	default:
		DRX_ASSERT_MESSAGE(false, "Invalid graph type.");
		break;
	}
...
DrxGUID CStateItem::GetGUID() const
{
