...
public:
				...
				case Role_Icon:
					{
						if (index.column() == Column_Name)
						{
							const DrxIcon* pIcon = pItem->GetIcon();
		...
		if (index.isValid())
		{
			...
			if (pItem)
			{
				...
				if (pParentItem)
				{
					...
					{
						for (i32 i = 0; i < pParentParentItem->GetNumChildItems(); ++i)
						{
					...
					{
						for (i32 i = 0; i < m_model.GetNumItems(); ++i)
						{
	...
	QModelIndex GetIndexForItem(const CAbstractObjectStructureModelItem& item, i32 column = 0) const
	{
		//u32k row = item.GetIndex();

		/*std::function<QModelIndex(const CAbstractObjectStructureModelItem&, i32)> GetIndexRecursive;
		   GetIndexRecursive = [&GetIndexRecursive, this](const CAbstractObjectStructureModelItem& item, i32 column) -> QModelIndex
		   {
		   const CAbstractObjectStructureModelItem* pParentItem = item.GetParentItem();
		   if (pParentItem)
		   {
		    u32k idx = item.GetIndex();
	...
	, m_pDataModel(nullptr)
{
	...
	m_pComponentsList->setTreePosition(static_cast<i32>(CObjectStructureModel::Column_Name));
	m_pComponentsList->setEditTriggers(QTreeView::DoubleClicked | QTreeView::EditKeyPressed);
	m_pComponentsList->sortByColumn(static_cast<i32>(CObjectStructureModel::Column_Name), Qt::AscendingOrder);
...
void CGraphsWidget::SetModel(CAbstractObjectStructureModel* pModel)
{
	if (m_pModel != pModel)
	{
		...
		if (pModel)
		{
			...
			m_pFilterProxy->setFilterKeyColumn(static_cast<i32>(CObjectStructureModel::Column_Name));
...
void CGraphsWidget::OnSelectionChanged(const QItemSelection& selected, const QItemSelection& deselected)
{
	...
	{
		i32 row = selectedIndexes.at(0).row();
...
void CGraphsWidget::PopulateContextMenuForItem(QMenu& menu, CStateItem& stateItem) const
{
	...
	}
	{
		...
		QObject::connect(pAction, &QAction::triggered, [this, &stateItem]()
			{
				DRX_ASSERT_MESSAGE(false, "Not yet implemented!");
		  });
	}
	{
