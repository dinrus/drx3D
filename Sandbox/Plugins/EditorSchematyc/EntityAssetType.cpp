		...
		if (pSchematycCore)
		{
			...
			if (result == QDialogButtonBox::Yes)
			{
				...
				if (pAsset)
				{
					...
					DRX_ASSERT_MESSAGE(pScript, "Selected script not found or of an invalid type.");
				}
			}
			else
			{
...
bool CEntityAssetType::OnCreate(CEditableAsset& editAsset, ukk /*pTypeSpecificParameter*/) const
{
	...
	if (baseClassId.guid != DrxGUID::Null() && baseClassId.domain != Schematyc::EDomain::Unknown)
	{
...
bool CEntityAssetType::RenameAsset(CAsset* pAsset, tukk szNewName) const
{
	if (pAsset)
	{
		...
		if (pScript)
		{
			DrxPathString prevName = pAsset->GetName();
			if (CAssetType::RenameAsset(pAsset, szNewName))
			{
				DrxPathString filePath = pScript->GetFilePath();
				DrxPathString path;
				DrxPathString file;
				DrxPathString ext;
...
string CEntityAssetType::GetObjectFilePath(const CAsset* pAsset) const
{
	...
	{
		const size_t fileCount = pAsset->GetFilesCount();
		DRX_ASSERT_MESSAGE(fileCount == 1, "Asset '%s' has an unexpected amount of script files.", pAsset->GetName());
		if (fileCount > 0)
		{
...
DrxIcon CEntityAssetType::GetIconInternal() const
{
	return DrxIcon("icons:schematyc/assettype_entity.ico");
}
...
Schematyc::IScript* CEntityAssetType::GetScript(const CAsset& asset) const
{
	...
	if (pSchematycCore)
	{
		...
		DRX_ASSERT_MESSAGE(fileCount == 1, "Asset '%s' has an unexpected amount of script files.", asset.GetName());
		if (fileCount > 0)
		{
