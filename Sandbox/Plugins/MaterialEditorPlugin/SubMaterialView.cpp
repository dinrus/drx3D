...
public:
	...
	{
		DRX_ASSERT(pMaterial && m_pMaterial);
		...
		{
			i32k count = m_pMaterial->GetSubMaterialCount();
	...
	inline CMaterial* GetMaterialForRow(i32 row) const
	{
		return m_bIsMultiMtl ? m_pMaterial->GetSubMaterial(row) : m_pMaterial;
	}

	//Use -1 for all rows
	void InvalidateRow(i32 row)
	{
		if (row == -1)
		{
			//invalidate all previews
			i32k rowCount = this->rowCount(QModelIndex());
			if (rowCount >= 0)
			{
				//invalidate it
		...
		else
		{
			...
			DRX_ASSERT(row < m_pPreviewPixmaps.size());
	...
	QModelIndex index(i32 row, i32 column, const QModelIndex& parent = QModelIndex()) const override
	{
		if (row >= 0 && column >= 0)
		{
	...
	{
		DRX_ASSERT(m_bIsMultiMtl && index.isValid());
		i32k row = index.row();
		CMaterial* material = GetMaterialForRow(row);
		DRX_ASSERT(material && material->IsPureChild());
...
void CSubMaterialView::OnMaterialForEditChanged(CMaterial* pEditorMaterial)
{
	...
	{
		DRX_ASSERT(pEditorMaterial == m_pModel->m_pMaterial);
	}
	else
	{
		...
		{
			i32k subMtlCount = m_pModel->m_pMaterial->GetSubMaterialCount();
			DRX_ASSERT(row < subMtlCount);
...
void CSubMaterialView::OnSelectionChanged(const QModelIndex& current, const QModelIndex& previous)
{
	...
	i32k row = current.row();
	if (m_pModel->m_pMaterial->IsMultiSubMaterial())
	{
		DRX_ASSERT(m_pModel->m_pMaterial->GetSubMaterialCount() > row);
