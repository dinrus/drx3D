...
//////////////////////////////////////////////////////////////////////////
bool CPickMaterialTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
...
void CPickMaterialTool::Display(DisplayContext& dc)
{
	...
	if (m_pHoveredMaterial)
	{
		...
		IRenderAuxText::Draw2dLabel(mousePoint.x + 12, mousePoint.y + 8, 1.2f, color, false, "%s", (tukk)m_displayString);
	}
...
bool CPickMaterialTool::OnMouseMove(CViewport* view, UINT nFlags, CPoint point)
{
	...
	i32 nPickObjectGroupFlag = CSurfaceInfoPicker::ePOG_All;
	if (nFlags & MK_SHIFT)
...
class CMaterialPickTool_ClassDesc : public IClassDesc
{
	...
	virtual tukk    ClassName()       { return "Material.PickTool"; };
	virtual tukk    Category()        { return "Material"; };
...
void CPickMaterialTool::OnPicked(IMaterial* pEditorMaterial)
{
	...
	if (pMtl)
	{
		...
		if (asset)
		{
			...
			DRX_ASSERT(editor->GetEditorName() == "Material Editor");
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Could not find asset for material %s", pMtl->GetFilename().c_str());
		}
	}
}
