...
void CMaterialEditor::InitMenuBar()
{
	...
	QAction* action = fileMenu->CreateAction(DrxIcon("icons:General/Picker.ico") ,tr("Pick Material From Scene"), 0, 1);
...
void CMaterialEditor::OnDataBaseItemEvent(IDataBaseItem* pItem, EDataBaseItemEvent event)
{
	...
	if(pItem == m_pMaterial.get())
	{
		...
		case EDB_ITEM_EVENT_DELETE:
			...
			/*DRX_ASSERT_MESSAGE(0, "Material was deleted by other means while it was being edited, this case is not well handled");
			TryCloseAsset();*/
			break;
		default:
			break;
...
bool CMaterialEditor::OnSaveAsset(CEditableAsset& editAsset)
{
	...
	if (ret)
	{
		...
		i32 textureCount = m_pMaterial->GetTextureDependencies(filenames);
...
void CMaterialEditor::FillMaterialMenu(CAbstractMenu* menu)
{
	...
	i32 sec = menu->GetNextEmptySection();
...
void CMaterialEditor::OnConvertToMultiMaterial()
{
	...
	DRX_ASSERT(m_pMaterial && !m_pMaterial->IsMultiSubMaterial());
...
void CMaterialEditor::OnSetSubMaterialSlotCount()
{
	...
	if (dialog.Execute())
	{
		...
		i32k newCount = dialog.GetValue();
		m_pMaterial->SetSubMaterialCount(newCount);

		for (i32 i = 0; i < newCount; i++)
		{
...
void CMaterialEditor::OnResetSubMaterial(i32 slot)
{
...
void CMaterialEditor::OnRemoveSubMaterial(i32 slot)
{
