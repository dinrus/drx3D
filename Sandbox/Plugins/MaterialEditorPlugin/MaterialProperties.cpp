...
void SerializeWordToDegree(Serialization::IArchive& ar, u16& word, tuk name, tuk label)
{
	float degree = Word2Degr(word);

	//round degree value
	degree = (float)((i32)(degree * 100 + 0.5f)) * 0.01f;
...
void CMaterialSerializer::Serialize(Serialization::IArchive& ar)
{
...

	{
		...
		*/
		...
		
		{
			...
			if (oldIndex == -1)
			{
				for (i32 i = 0; i < m_shaderStringList.size(); i++)
				{
...

	{
		...
		for (i32 i = 0; i < MTL_LAYER_MAX_SLOTS; i++)
		{
	...
	/*{
		...
		i32 mtlPropagationFlags = pMtl->GetPropagationFlags();
...
void CMaterialSerializer::SerializeTextureSlots(Serialization::IArchive& ar, bool& bHasDetailDecalOut)
{
	...
	i32k nTech = max(0, m_pMaterial->GetShaderItem().m_nTechnique);
	...
	for (EEfResTextures texId = EEfResTextures(0); texId < EFTT_MAX; texId = EEfResTextures(texId + 1))
	{
...

		{
			...
			//See MaterialHelpers.cpp (Drx3DEngine)
		...
		if (pTexModifier)
		{
			...
			pTexModifier->m_eTGType = u8(texGenType);
		}
		...
		if (texId == EFTT_DECAL_OVERLAY)
		{
			...
			//if (bIsDetailDecal)
			{
				...
				shaderResources.m_DetailDecalInfo.nBlending = (u8)int_round(opacity / fUint8RatioToFloat);

				float ssao = (float)shaderResources.m_DetailDecalInfo.nSSAOAmount * fUint8RatioToFloat;
				ar(ssao, "ssao", "SSAO Amount");
				shaderResources.m_DetailDecalInfo.nSSAOAmount = (u8)int_round(ssao / fUint8RatioToFloat);
				...
				if (ar.openBlock("top", "-Top Layer"))
				{
					...
					shaderResources.m_DetailDecalInfo.nDeformation[0] = (u8)int_round(deformation / fUint8RatioToFloat);
					...
					shaderResources.m_DetailDecalInfo.nThreshold[0] = (u8)int_round(sortingOffset / fUint8RatioToFloat);
				...
				if (ar.openBlock("bottom", "-Bottom Layer"))
				{
					...
					ar(deformation, "deformation", "Deformation");
					shaderResources.m_DetailDecalInfo.nDeformation[1] = (u8)int_round(deformation / fUint8RatioToFloat);

					float sortingOffset = (float)shaderResources.m_DetailDecalInfo.nThreshold[1] * fUint8RatioToFloat;
					ar(sortingOffset, "sortingOffset", "Sorting Offset");
					shaderResources.m_DetailDecalInfo.nThreshold[1] = (u8)int_round(sortingOffset / fUint8RatioToFloat);
		...
		if (pTexModifier)
		{
			...
			if (ar.openBlock("rotator", "-Rotator"))
			{
				...
				ar(type, "type", "Type");
				pTexModifier->m_eRotType = (u8)type;
...
template<typename T>
void SerializeShaderParam(Serialization::IArchive& ar, T& value, tukk name, const CMaterialSerializer::SShaderParamInfo& paramInfo)
{
...
void CMaterialSerializer::SerializeShaderParams(Serialization::IArchive& ar, bool bShaderChanged)
{
	...
	for (i32 i = 0; i < count; i++)
	{
		...
		case eType_BYTE:
		{
			i32 value = (i32)param.m_Value.m_Byte;
			SerializeShaderParam(ar, value, param.m_Name, info);
			if (value != (i32)param.m_Value.m_Byte)
			{
		...
		case eType_SHORT:
		{
			i32 value = (i32)param.m_Value.m_Short;
			SerializeShaderParam(ar, value, param.m_Name, info);
			if (value != (i32)param.m_Value.m_Short)
			{
				bShaderParamsChanged = true;
		...
		default:
			...
			//DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Unknown shader parameter type found in material %s: %s", m_pMaterial->GetName().c_str(), param.m_Name);
			break;
		}
	}
...
void CMaterialSerializer::SerializeShaderGenParams(Serialization::IArchive& ar)
{
	...
	i32k count = shaderGen->m_BitMask.size();
	if (count <= 0)
		return;
...
void CMaterialSerializer::PopulateStringLists()
{
	...
	if (pSurfaceTypeEnum)
	{
		...
		{
			tukk name = pSurfaceType->GetName();
