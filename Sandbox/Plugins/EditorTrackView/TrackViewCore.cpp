...
protected:
	void OnTextChanged(const QString& text)
	{
		if (!text.isEmpty())
		{
			...
			for (u32 i = CTrackViewPlugin::GetSequenceManager()->GetCount(); i--; )
			{
				DrxStackStringT<char, 256> seqName(CTrackViewPlugin::GetSequenceManager()->GetSequenceByIndex(i)->GetName());
				seqName.MakeLower();
...
CTrackViewSequence* CTrackViewCore::GetSequenceByGUID(DrxGUID sequenceGUID) const
{
...
void CTrackViewCore::OpenSequence(const DrxGUID sequenceGUID)
{
