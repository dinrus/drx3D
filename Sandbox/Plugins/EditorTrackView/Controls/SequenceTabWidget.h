...
protected:
	...
	virtual tukk GetComponentTitle() const override { return "Sequence Explorer"; }
...
private:
	...
	void            SetSequenceTime(const DrxGUID sequenceGUID, SAnimTime newTime);
	...
	CTimeline*                  GetDopeSheetFromSequenceGUID(const DrxGUID& guid) const;

	CCurveEditor*               GetActiveCurveEditor() const;
	CCurveEditor*               GetCurveEditorFromSequenceGUID(const DrxGUID& guid) const;

	DrxGUID                     GetSequenceGUIDFromTabIndex(i32 index) const;
