	...
	: CTrackViewCoreComponent(pTrackViewCore, true)
{
	...
	auto action = addAction(DrxIcon("icons:Trackview/Rewind_Backward_End.ico"), tr("Go to start of sequence"));
	connect(action, &QAction::triggered, this, &CTrackViewPlaybackControlsToolbar::OnGoToStart);

	QIcon* pPlayPauseIcon = new DrxIcon();
	...
	action = addAction(DrxIcon("icons:Trackview/Stop_Sequence.ico"), tr("Stop"));
	connect(action, &QAction::triggered, this, &CTrackViewPlaybackControlsToolbar::OnStop);

	action = addAction(DrxIcon("icons:Trackview/Rewind_Forward_End.ico"), tr("Go to end of sequence"));
	connect(action, &QAction::triggered, this, &CTrackViewPlaybackControlsToolbar::OnGoToEnd);

	m_pActionLoop = addAction(DrxIcon("icons:Trackview/trackview_time_loop.ico"), tr("Loop"));
	m_pActionLoop->setCheckable(true);
	connect(m_pActionLoop, &QAction::toggled, this, &CTrackViewPlaybackControlsToolbar::OnLoop);

	addSeparator();

	action = addAction(DrxIcon("icons:Trackview/trackview_time_limit_L_toolbar.ico"), tr("Set Playback Start"));
	connect(action, &QAction::triggered, this, &CTrackViewPlaybackControlsToolbar::OnSetStartMarker);

	action = addAction(DrxIcon("icons:Trackview/trackview_time_limit_R_toolbar.ico"), tr("Set Playback End"));
	connect(action, &QAction::triggered, this, &CTrackViewPlaybackControlsToolbar::OnSetEndMarker);

	addSeparator();

	m_pActionRecord = addAction(DrxIcon("icons:Trackview/trackview_time_record.ico"), tr("Record"));
...
{
	auto action = inOutMenu->CreateAction(DrxIcon("icons:Trackview/Play_Sequence.ico"), tr("Play"));
	connect(action, &QAction::triggered, [this]() { OnPlayPause(true); });

	action = inOutMenu->CreateAction(DrxIcon("icons:Trackview/trackview_time_pause.ico"), tr("Pause"));
	connect(action, &QAction::triggered, [this]() { OnPlayPause(false); });

	action = inOutMenu->CreateAction(DrxIcon("icons:Trackview/Stop_Sequence.ico"), tr("Stop"));
	connect(action, &QAction::triggered, this, &CTrackViewPlaybackControlsToolbar::OnStop);

	action = inOutMenu->CreateAction(DrxIcon("icons:Trackview/trackview_time_loop.ico"), tr("Loop"));
	...
	i32 sec = inOutMenu->GetNextEmptySection();

	action = inOutMenu->CreateAction(DrxIcon("icons:Trackview/trackview_time_limit_L_toolbar.ico"), tr("Set Playback Start"), sec);
	connect(action, &QAction::triggered, this, &CTrackViewPlaybackControlsToolbar::OnSetStartMarker);

	action = inOutMenu->CreateAction(DrxIcon("icons:Trackview/trackview_time_limit_R_toolbar.ico"), tr("Set Playback End"), sec);
	connect(action, &QAction::triggered, this, &CTrackViewPlaybackControlsToolbar::OnSetEndMarker);
	...
	action = inOutMenu->CreateAction(DrxIcon("icons:Trackview/trackview_time_record.ico"), tr("Record"), sec);
	action->setCheckable(true);
...
void CTrackViewPlaybackControlsToolbar::PopulateFramerateMenu(CAbstractMenu* inOutMenu, CTrackViewCore* pTrackViewCore)
{
	...
	{
		tukk szFrameRateName = SAnimTime::GetFrameRateName((SAnimTime::EFrameRate)i);
...
void CTrackViewPlaybackControlsToolbar::UpdateText()
{
	...
	i32k ticks = m_lastTime.GetTicks();
	i32k totalSeconds = (i32)ticks / SAnimTime::numTicksPerSecond;
	i32k totalRemainder = ticks % SAnimTime::numTicksPerSecond;
	i32k mins = (i32)totalSeconds / 60;
	i32k secs = totalSeconds % 60;
	...
	case SAnimTime::EDisplayMode::Frames:
		{
			i32k ticksPerFrame = SAnimTime::numTicksPerSecond / m_framerate;
			i32k totalFrames = ticks / ticksPerFrame;
	...
	case SAnimTime::EDisplayMode::Time:
		{
			i32k millisecs = int_round(((float)totalRemainder / SAnimTime::numTicksPerSecond) * 1000.f);
	...
	case SAnimTime::EDisplayMode::Timecode:
		{
			i32k ticksPerFrame = SAnimTime::numTicksPerSecond / m_framerate;
			i32k leftOverFrames = totalRemainder / (ticksPerFrame == 0 ? 1 : ticksPerFrame);
