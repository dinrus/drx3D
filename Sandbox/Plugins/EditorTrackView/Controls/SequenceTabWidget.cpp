	...
	, m_timeUnit(SAnimTime::EDisplayMode::Time)
{
	...
	connect(m_pDopesheetTabs, SIGNAL(tabBarClicked(i32)), this, SLOT(OnDopeSheetTabChanged(i32)));
	connect(m_pDopesheetTabs, SIGNAL(currentChanged(i32)), this, SLOT(OnDopeSheetTabChanged(i32)));
	connect(m_pDopesheetTabs, SIGNAL(tabCloseRequested(i32)), this, SLOT(OnDopeSheetTabClose(i32)));
...
void CTrackViewSequenceTabWidget::OnDopeSheetTabChanged(i32 index)
{
...
void CTrackViewSequenceTabWidget::OnDopeSheetTabClose(i32 index)
{
	DrxGUID guid = GetSequenceGUIDFromTabIndex(index);
...
DrxGUID CTrackViewSequenceTabWidget::GetSequenceGUIDFromTabIndex(i32 index) const
{
	return stl::find_in_map(m_tabToSequenceUIdMap, m_pDopesheetTabs->widget(index), DrxGUID::Null());
}

DrxGUID CTrackViewSequenceTabWidget::GetActiveSequenceGUID()
{
	return stl::find_in_map(m_tabToSequenceUIdMap, m_pDopesheetTabs->widget(m_pDopesheetTabs->currentIndex()), DrxGUID::Null());
}

CTrackViewSequenceTabWidget::SSequenceData* CTrackViewSequenceTabWidget::GetActiveSequenceData()
{
	const DrxGUID guid = GetActiveSequenceGUID();
	if (guid != DrxGUID::Null())
	{
...
void CTrackViewSequenceTabWidget::OpenSequenceTab(const DrxGUID sequenceGUID)
{
	...
	bool bOpen = false;
	i32k numTabs = m_pDopesheetTabs->count();
	for (i32 i = 0; i < numTabs; ++i)
	{
		const DrxGUID guid = GetSequenceGUIDFromTabIndex(i);
	...
	sequenceData.m_timelineContent.userSideLoad.resize(sizeof(DrxGUID));
	memcpy(&sequenceData.m_timelineContent.userSideLoad[0], &sequenceGUID, sizeof(DrxGUID));
	...
	QIcon addNodeIcon = DrxIcon("icons:General/Element_Add.ico");
...
void CTrackViewSequenceTabWidget::OnRenameNode()
{
	...
	{
		DrxLog("Problem with renaming node %s. Does a node with that name already exist?", newNameString);
	}
}
...
void CTrackViewSequenceTabWidget::DisableSelectedNodesAndTracks(bool disable)
{
	...
	for (i32 nodeNumber = 0; nodeNumber < selectedNodes.GetCount(); ++nodeNumber)
	{
	...
	for (i32 trackNumber = 0; trackNumber < selectedTracks.GetCount(); ++trackNumber)
	{
		...
		for (size_t i = 0; i < numDisplayedTracks; ++i)
		{
			...
			if (pNode && pNode->GetNodeType() == eTVNT_Track)
			{
				...
				if (!pTrack->IsCompoundTrack() && strcmp(pTrack->GetKeyType(), S2DBezierKey::GetType()) == 0)
				{
					...
					const DrxGUID trackGUID = pTrack->GetGUID();
					curve.userSideLoad.resize(sizeof(DrxGUID));
					memcpy(&curve.userSideLoad[0], &trackGUID, sizeof(DrxGUID));
...
void CTrackViewSequenceTabWidget::SetSequenceTime(const DrxGUID sequenceGUID, SAnimTime newTime)
{
...
void CTrackViewSequenceTabWidget::UpdateSequenceTabs()
{
	...
	i32 numTabs = m_pDopesheetTabs->count();
	for (i32 i = 0; i < numTabs; )
	{
		const DrxGUID guid = GetSequenceGUIDFromTabIndex(i);
		CTrackViewSequence* pSequence = CTrackViewPlugin::GetSequenceManager()->GetSequenceByGUID(guid);
		if (!pSequence)
...
{
	DrxGUID guid;
	memcpy(&guid, &pDopeSheet->Content()->userSideLoad[0], sizeof(DrxGUID));
	return CTrackViewPlugin::GetSequenceManager()->GetSequenceByGUID(guid);
}
...
DrxGUID CTrackViewSequenceTabWidget::GetGUIDFromDopeSheetTrack(const STimelineTrack* pDopeSheetTrack) const
{
...
{
	const DrxGUID guid = GetSequenceGUIDFromTabIndex(m_pDopesheetTabs->currentIndex());
	return GetDopeSheetFromSequenceGUID(guid);
}

CTimeline* CTrackViewSequenceTabWidget::GetDopeSheetFromSequenceGUID(const DrxGUID& guid) const
{
...
CCurveEditor* CTrackViewSequenceTabWidget::GetCurveEditorFromSequenceGUID(const DrxGUID& guid) const
{
	auto findIter = m_idToSequenceDataMap.find(guid);
	if (findIter != m_idToSequenceDataMap.end())
	{
...
void CTrackViewSequenceTabWidget::OnDopesheetTreeContextMenu(const QPoint& point)
{
	...
	if (pContent)
	{
		i32 tracksEnabledCount = 0;
		i32 tracksDisabledCount = 0;
		...
		i32 selectedObjectsCount = pSelectedObjects->GetCount();
...
void CTrackViewSequenceTabWidget::OnCurveEditorContentChanged()
{
...

	{
		...
		for (size_t i = 0; i < numCurves; ++i)
		{
			...
			DrxGUID guid;
			memcpy(&guid, &curve.userSideLoad[0], sizeof(DrxGUID));
...
void CTrackViewSequenceTabWidget::AddNodeToDopeSheet(CTrackViewNode* pNode)
{
	...
	const DrxGUID nodeGUID = pNode->GetGUID();
...
void CTrackViewSequenceTabWidget::OnNodeChanged(CTrackViewNode* pNode, ENodeChangeType type)
{
	...
	const DrxGUID sequenceGUID = pNode->GetSequence()->GetGUID();
	CTimeline* pDopeSheet = GetDopeSheetFromSequenceGUID(sequenceGUID);
	if (pDopeSheet)
	{
		pDopeSheet->ContentUpdated();
...
void CTrackViewSequenceTabWidget::OnNodeRenamed(CTrackViewNode* pNode, tukk pOldName)
{
...
void CTrackViewSequenceTabWidget::OnDopesheetPaste(SAnimTime time, STimelineTrack* pTrack)
{
	...
	if (!strcmp("Track view entity nodes", clip.GetTitle()))
	{
		...
		if (pDopesheet)
		{
			...
			{
				const DrxGUID currentTrackGUID = GetGUIDFromDopeSheetTrack(selectedTracks[0]);
	...
	else if (!strcmp("Track view keys", clip.GetTitle()))
	{
		...
		if (pDopesheet)
		{
			const DrxGUID currentTrackGUID = GetGUIDFromDopeSheetTrack(pTrack);
