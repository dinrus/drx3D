...
public:
	...
	void OnRowAdded()
	{
		i32k idx = m_sequence.GetTrackEventsCount() - 1;
	...
	void LoadEvents()
	{
		i32k numPrevRows = max(static_cast<i32>(m_dataRows.size()) - 1, 0);
		m_dataRows.clear();

		SRow row;
		for (i32 i = 0, e = m_sequence.GetTrackEventsCount(); i < e; ++i)
		{
			...
			u32k numNodes = nodeBundle.GetCount();
			for (u32 currentNode = 0; currentNode < numNodes; ++currentNode)
			{
				...
				u32k numTracks = tracks.GetCount();
				for (u32 currentTrack = 0; currentTrack < numTracks; ++currentTrack)
				{
					const CTrackViewTrack* pTrack = tracks.GetTrack(currentTrack);
					for (u32 currentKey = 0; currentKey < pTrack->GetKeyCount(); ++currentKey)
					{
...
public:
	...
	void OnTextChanged(const QString& text)
	{
		...
		if (!text.isEmpty())
		{
			...
			DrxStackStringT<char, 256> enteredName(byteArray.data());
			i32 eventNameCrc = CCrc32::ComputeLowercase(enteredName.c_str());

			for (i32 i = m_sequence.GetTrackEventsCount(); i--; )
			{
