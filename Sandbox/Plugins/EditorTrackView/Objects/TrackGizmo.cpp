...
#pragma message("TODO")
	u32 hideMask = 0; //gSettings.objectHideMask;
	...
	i32 nSubTracks = pTrack->GetChildCount();

	if (nSubTracks > 0)
	{
		for (i32 i = 0; i < nSubTracks; i++)
		{
...
void CTrackGizmo::DrawKeys(DisplayContext& dc, CTrackViewTrack* pTrack, CTrackViewTrack* pKeysTrack)
{
	...
	float sz = 0.2f;
	i32 nkeys = pKeysTrack->GetKeyCount();
