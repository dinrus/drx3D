...
void CTrackViewNode::ClearSelection()
{
	...
	u32k numChilds = GetChildCount();
	for (u32 childIndex = 0; childIndex < numChilds; ++childIndex)
	{
...
namespace
{
static i32 GetNodeOrder(EAnimNodeType nodeType)
{
	assert(nodeType < eAnimNodeType_Num);

	static i32 nodeOrder[eAnimNodeType_Num];
	...
	case eTVNT_AnimNode:
		{
			...
			i32k thisTypeOrder = GetNodeOrder(thisAnimNode.GetType());
			i32k otherTypeOrder = GetNodeOrder(otherAnimNode.GetType());
