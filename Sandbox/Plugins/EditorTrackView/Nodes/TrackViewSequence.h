...
struct ITrackViewSequenceListener
{
	...
	virtual void OnNodeRenamed(CTrackViewNode* pNode, tukk pOldName) {}
...
public:
	...
	virtual tukk        GetName() const override      { return m_pAnimSequence->GetName(); }
	virtual bool               SetName(tukk pName) override;
	...
	// Get DrxMovie sequence ID
	u32 GetDrxMovieId() const { return m_pAnimSequence->GetId(); }
	...
	i32         GetTrackEventsCount() const                                   { return m_pAnimSequence->GetTrackEventsCount(); }
	tukk GetTrackEvent(i32 index)                                      { return m_pAnimSequence->GetTrackEvent(index); }
	bool        AddTrackEvent(tukk szEvent)                            { return m_pAnimSequence->AddTrackEvent(szEvent); }
	bool        RemoveTrackEvent(tukk szEvent)                         { return m_pAnimSequence->RemoveTrackEvent(szEvent); }
	bool        RenameTrackEvent(tukk szEvent, tukk szNewEvent) { return m_pAnimSequence->RenameTrackEvent(szEvent, szNewEvent); }
	bool        MoveUpTrackEvent(tukk szEvent)                         { return m_pAnimSequence->MoveUpTrackEvent(szEvent); }
	bool        MoveDownTrackEvent(tukk szEvent)                       { return m_pAnimSequence->MoveDownTrackEvent(szEvent); }
	...
	void OnNodeRenamed(CTrackViewNode* pNode, tukk pOldName);

	// Get node from UID
	CTrackViewNode* GetNodeFromGUID(const DrxGUID guid) const;

	IAnimSequence*  GetIAnimSequence() const { return m_pAnimSequence; }
	virtual DrxGUID GetGUID() const override { return m_pAnimSequence->GetGUID(); }
...
private:
	...
	void                         UpdateLightAnimationRefs(tukk szOldName, tukk szNewName);
	...
	u32 m_notificationRecursionLevel;
	bool         m_bLoaded;
	...
	std::map<DrxGUID, CTrackViewNode*> m_uidToNodeMap;
};
