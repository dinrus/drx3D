...
{
	static tukk GetType() { return "SequenceKeyFacade"; }

	SSequenceKeyFacade()
		: SSequenceKey()
		, m_owningSequenceGUID(DrxGUID::Null())
	{}

	SSequenceKeyFacade(DrxGUID owningSequenceGUID)
		: SSequenceKey()
	...
	void Serialize(Serialization::IArchive& ar)
	{
		...
		if (pOwningSequence)
		{
			...
			for (uint i = 0; i < numSequences; ++i)
			{
				...
				{
					tukk pSequenceName = pSequence->GetName();
		...
		else if (ar.isInput())
		{
			...
			m_sequenceGUID = pSequence ? pSequence->GetGUID() : DrxGUID::Null();
		}
...
_smart_ptr<IAnimKeyWrapper> CTrackViewSequenceTrack::GetWrappedKey(i32 key)
{
	const DrxGUID sequenceGUID = GetSequence()->GetGUID();
...
tukk CTrackViewSequenceTrack::GetKeyType() const
{
