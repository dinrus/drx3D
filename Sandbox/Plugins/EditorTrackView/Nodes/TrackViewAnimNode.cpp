	...
	: CTrackViewNode(pParentNode), m_pAnimSequence(pSequence), m_pAnimNode(pAnimNode), m_pNodeAnimator(nullptr)
{
	if (pAnimNode)
	{
		...
		i32k nodeCount = pSequence->GetNodeCount();
		...
		// Search for tracks
		i32k trackCount = pAnimNode->GetTrackCount();
...
CTrackViewAnimNode* CTrackViewAnimNode::CreateSubNode(const string& name, const EAnimNodeType animNodeType, CEntityObject* pEntity)
{
	...
	// Create DrxMovie and TrackView node
...
CTrackViewTrack* CTrackViewAnimNode::CreateTrack(const CAnimParamType& paramType)
{
	...
	// Create DrxMovie and TrackView track
...
CTrackViewTrackBundle CTrackViewAnimNode::GetTracks(const bool bOnlySelected, const CAnimParamType& paramType)
{
	...
	for (auto iter = m_childNodes.begin(); iter != m_childNodes.end(); ++iter)
	{
		...
		if (pNode->GetNodeType() == eTVNT_Track)
		{
			...
			u32k subTrackCount = pTrack->GetChildCount();
...
CTrackViewTrack* CTrackViewAnimNode::GetTrackForParameter(const CAnimParamType& paramType, u32 index) const
{
	u32 currentIndex = 0;
	...
	for (auto iter = m_childNodes.begin(); iter != m_childNodes.end(); ++iter)
	{
		...
		if (pNode->GetNodeType() == eTVNT_Track)
		{
			...
			{
				u32 numChildTracks = pTrack->GetChildCount();
				for (u32 i = 0; i < numChildTracks; ++i)
				{
...
bool CTrackViewAnimNode::SetName(tukk pName)
{
...
CTrackViewAnimNodeBundle CTrackViewAnimNode::GetAnimNodesByName(tukk pName)
{
	CTrackViewAnimNodeBundle bundle;
...
tuk CTrackViewAnimNode::GetParamName(const CAnimParamType& paramType) const
{
	tukk pName = m_pAnimNode->GetParamName(paramType);
	return pName ? pName : "";
}
...
u32 CTrackViewAnimNode::GetParamCount() const
{
	return m_pAnimNode ? m_pAnimNode->GetParamCount() : 0;
}

CAnimParamType CTrackViewAnimNode::GetParamType(u32 index) const
{
	u32 paramCount = GetParamCount();
...
bool CTrackViewAnimNode::PasteNodesFromClipboard()
{
	...
	u32k numNodes = animNodesRoot->getChildCount();
...
{
	tukk pName = xmlNode->getAttr("Name");
	if (!pName)
	{
		return;
	}
	...
	i32k numChilds = xmlNode->getChildCount();
