...
CTrackViewTrack::CTrackViewTrack(IAnimTrack* pTrack, CTrackViewAnimNode* pTrackAnimNode,
                                 CTrackViewNode* pParentNode, bool bIsSubTrack, u32 subTrackIndex) : CTrackViewNode(pParentNode),
	m_pAnimTrack(pTrack), m_pTrackAnimNode(pTrackAnimNode), m_bIsSubTrack(bIsSubTrack), m_subTrackIndex(subTrackIndex)
{
	...
	u32k subTrackCount = m_pAnimTrack->GetSubTrackCount();
	for (u32 subTrackIndex = 0; subTrackIndex < subTrackCount; ++subTrackIndex)
	{
...
CTrackViewKeyHandle CTrackViewTrack::GetPrevKey(const SAnimTime time)
{
	...
	i32k numKeys = m_pAnimTrack->GetNumKeys();
...
CTrackViewKeyBundle CTrackViewTrack::GetKeys(const bool bOnlySelected, const SAnimTime t0, const SAnimTime t1)
{
	...
	i32k keyCount = m_pAnimTrack->GetNumKeys();
...
{
	i32k keyIndex = m_pAnimTrack->CreateKey(time);
...
CTrackViewKeyHandle CTrackViewTrack::GetKey(u32 index)
{
...
CTrackViewKeyConstHandle CTrackViewTrack::GetKey(u32 index) const
{
	if (index < GetKeyCount())
	{
...
CTrackViewKeyHandle CTrackViewTrack::GetKeyByTime(const SAnimTime time)
{
	if (m_bIsCompoundTrack)
	{
		...
		for (u32 i = 0; i < childCount; ++i)
		{
			...
			i32 keyIndex = pChildTrack->m_pAnimTrack->FindKey(time);
			if (keyIndex >= 0)
			{
	...
	i32 keyIndex = m_pAnimTrack->FindKey(time);
...
tukk CTrackViewTrack::GetName() const
{
...
void CTrackViewTrack::SetKey(u32 keyIndex, const STrackKey* pKey)
{
	m_pAnimTrack->SetKey(keyIndex, pKey);
	GetSequence()->OnNodeChanged(this, ITrackViewSequenceListener::eNodeChangeType_KeysChanged);
}

void CTrackViewTrack::GetKey(u32 keyIndex, STrackKey* pKey) const
{
	m_pAnimTrack->GetKey(keyIndex, pKey);
}

void CTrackViewTrack::SelectKey(u32 keyIndex, bool bSelect)
{
...
void CTrackViewTrack::RemoveKey(i32k index)
{
...
void CTrackViewTrack::SelectKeys(const bool bSelected)
{
	...
	else
	{
		// Affect sub tracks
		u32 childCount = GetChildCount();
		for (u32 childIndex = 0; childIndex < childCount; ++childCount)
		{
...
bool CTrackViewTrack::IsKeySelected(u32 keyIndex) const
{
	return (keyIndex < m_keySelectionStates.size()) ? m_keySelectionStates[keyIndex] : false;
}

CTrackViewKeyHandle CTrackViewTrack::GetSubTrackKeyHandle(u32 index) const
{
	...
	for (u32 childIndex = 0; childIndex < childCount; ++childIndex)
	{
		...
		u32k childKeyCount = pChildTrack->GetKeyCount();
...
void CTrackViewTrack::SetAnimationLayerIndex(i32k index)
{
	if (m_pAnimTrack)
	{
...
i32 CTrackViewTrack::GetAnimationLayerIndex() const
{
