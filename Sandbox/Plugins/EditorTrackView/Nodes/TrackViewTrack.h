...
public:
	...
	// Key getters
	virtual tukk              GetKeyType() const           { return m_pAnimTrack->GetKeyType(); }
	...
	// Mask
	bool IsMasked(u32 mask) const { return m_pAnimTrack->IsMasked(mask); }
	...
	void SetAnimationLayerIndex(i32k index);
	i32  GetAnimationLayerIndex() const;

	// Offset keys
	virtual void OffsetKeys(const TMovieSystemValue& offset);

	// Gets the wrapped key
	virtual _smart_ptr<IAnimKeyWrapper> GetWrappedKey(i32 key) { return m_pAnimTrack->GetWrappedKey(key); }

	// Returns true if the track has keys with duration
	virtual bool    KeysHaveDuration() const;

	virtual DrxGUID GetGUID() const override { return m_pAnimTrack->GetGUID(); }
