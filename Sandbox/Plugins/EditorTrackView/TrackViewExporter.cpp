...
bool CTrackViewExporter::ImportFromFile()
{
	...
	if (fileName.GetLength() > 0)
	{
		...
		tukk szExt = PathUtil::GetExt(fileName);
...
bool CTrackViewExporter::ReadFileIntoMemory(tukk fileName)
{
	tukk szExt = PathUtil::GetExt(fileName);
...
bool CTrackViewExporter::ImportFBXFile()
{
	...
	if (pSequence)
	{
		...
		i32k objectCount = m_data.GetObjectCount();

		uint objectsToImport = 0;
		QFBXImporterDlg fbxImporterDlg;
		for (i32 objectId = 0; objectId < objectCount; ++objectId)
		{
		...
		std::vector<i32> decisions;
		i32 result = QDialogButtonBox::No;
		for (i32 objectId = 0; objectId < objectCount; ++objectId)
		{
			...
			if (pObject)
			{
				...
				tukk szUpdatedNodeName = pObject->name;
				...
				i32 animationDataCount = pObject->GetEntityAnimationDataCount();
				for (i32 animDataId = 0; animDataId < animationDataCount; ++animDataId)
				{
		...
		for (uint objectId = 0; objectId < objectCount; ++objectId)
		{
			...
			if (pObject)
			{
				...
				else
				{
					...
					DrxLog("No matching node could be found for object %s. skipping...", szUpdatedNodeName);
					continue;
				}

				i32k animatonDataCount = pObject->GetEntityAnimationDataCount();

				// Add keys from the imported file to the selected tracks
				for (i32 animDataID = 0; animDataID < animatonDataCount; ++animDataID)
				{
					const Export::EntityAnimData* pAnimData = pObject->GetEntityAnimationData(animDataID);
	...
	if (fileName.GetLength() > 0)
	{
		...
		if (bResult)
		{
			...
			if (pExportManager)
			{
				...
				{
					DrxLog("Couldn't find suitable exporter for %s file format!", szExt);
				}
			}
		}
	}
...
bool CTrackViewExporter::AddSelectedEntityObjects()
{
	...
	u32k numSelectedNodes = selectedNodes.GetCount();

	for (u32 nodeNumber = 0; nodeNumber < numSelectedNodes; ++nodeNumber)
	{
		...
		if (pEntityObject)
		{
			...
			if (pEntityObject->IsKindOf(RUNTIME_CLASS(CCameraObjectTarget)))
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Please select camera object, to export its camera target.");
					return false;
				}
...
bool CTrackViewExporter::AddObject(CBaseObject* pBaseObj)
{
	...
	{
		for (i32 i = 0; i < pBaseObj->GetChildCount(); i++)
		{
	...
	/*
	   ...
	   m_objectMap[pBaseObj] = i32(m_data.m_objects.size()-1);
...
void CTrackViewExporter::AddEntityAnimationData(CBaseObject* pBaseObj)
{
	...
	if (pBaseObj->IsKindOf(RUNTIME_CLASS(CCameraObject)))
	{
		...
		if (pLookAt)
		{
			...
			{
				drx_strcpy(pObj->cameraTargetNodeName, pLookAt->GetName());
			}
		}
	...
	m_objectMap[pBaseObj] = i32(m_data.GetObjectCount() - 1);
}
...
{
	i32 keyCount = pTrack->GetKeyCount();
	...
	for (i32 keyNumber = 0; keyNumber < keyCount; keyNumber++)
	{
...
void CTrackViewExporter::ProcessEntityAnimationTrack(const CBaseObject* pBaseObj, SExportObject* pObj, EAnimParamType entityTrackParamType)
{
	...
	if (pEntityTrack->GetParameterType() == eAnimParamType_Rotation && bIsCameraObject)
	{
		...
		if (pParentTrack)
		{
			for (i32 trackID = 0; trackID < pParentTrack->GetChildCount(); ++trackID)
			{
		...
		if (bShakeTrackPresent)
		{
			...
			i32k fpsTimeInterval = SAnimTime::numTicksPerSecond / m_FBXBakedExportFramerate;

			pObj->m_entityAnimData.reserve(m_numberOfExportFrames * Export::kReserveCount);

			for (i32 frameID = 0; frameID < m_numberOfExportFrames; ++frameID)
			{
	...
	for (i32 trackNumber = 0; trackNumber < pEntityTrack->GetChildCount(); ++trackNumber)
	{
...
bool CTrackViewExporter::AddObjectsFromSequence(CTrackViewSequence* pSequence, XmlNodeRef seqNode)
{
	...
	u32k numAllNodes = allNodes.GetCount();

	for (u32 nodeID = 0; nodeID < numAllNodes; ++nodeID)
	{
		...
		if (pEntityObject)
		{
			...
			m_data.AddObject(pObj);
			m_objectMap[pEntityObject] = i32(m_data.GetObjectCount() - 1);
		}
	}
	...
	for (uint i = 0; i < numSequenceTracks; ++i)
	{
		...
		for (i32 keyIndex = 0; keyIndex < numKeys; ++keyIndex)
		{
			...
			if (sequenceKey.m_sequenceGUID != DrxGUID::Null())
			{
...
void CTrackViewExporter::FillAnimTimeNode(XmlNodeRef writeNode, CTrackViewAnimNode* pObjectNode, CTrackViewSequence* currentSequence)
{
	...
	u32k numAllTracks = allTracks.GetCount();
	...
	if (numAllTracks > 0)
	{
		...
		for (u32 trackID = 0; trackID < numAllTracks; ++trackID)
		{
...
{
	for (i32 objectID = 0; objectID < m_data.GetObjectCount(); ++objectID)
	{
...
bool CTrackViewExporter::AddCameraTargetObject(CBaseObjectPtr pLookAt)
{
	if (pLookAt && pLookAt->IsKindOf(RUNTIME_CLASS(CCameraObjectTarget)))
	{
		...
		m_objectMap[pLookAt] = i32(m_data.GetObjectCount() - 1);
...
bool CTrackViewExporter::ProcessObjectsForExport()
{
	...
	for (i32 frameID = startFrame; frameID <= m_numberOfExportFrames; ++frameID)
	{
		...
		for (size_t objectID = 0; objectID < m_data.GetObjectCount(); ++objectID)
		{
			...
			{
				DrxGUID camGUID = GetIEditor()->GetViewManager()->GetCameraObjectId();
...
void CTrackViewExporter::SolveHierarchy()
{
	for (TObjectMap::iterator it = m_objectMap.begin(); it != m_objectMap.end(); ++it)
	{
		...
		if (pObj && pObj->GetParent())
		{
			...
			{
				i32 indexOfParent = itFind->second;
...
{
	for (u32 trackId = 0; trackId < trackBundle.GetCount(); ++trackId)
	{
