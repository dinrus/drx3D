...
// Sequences
void PyTrackViewNewSequence(tukk name)
{
...
void PyTrackViewDeleteSequence(tukk name)
{
	CTrackViewSequenceManager* pSequenceManager = CTrackViewPlugin::GetSequenceManager();
	CTrackViewSequence* pSequence = pSequenceManager->GetSequenceByName(name);
	if (pSequence)
...
void PyTrackViewSetCurrentSequence(tukk name)
{
...
i32 PyTrackViewGetNumSequences()
{
	const CTrackViewSequenceManager* pSequenceManager = CTrackViewPlugin::GetSequenceManager();
	return pSequenceManager->GetCount();
}

string PyTrackViewGetSequenceName(u32 index)
{
...
boost::python::tuple PyTrackViewGetSequenceTimeRange(tukk name)
{
	const CTrackViewSequenceManager* pSequenceManager = CTrackViewPlugin::GetSequenceManager();
	CTrackViewSequence* pSequence = pSequenceManager->GetSequenceByName(name);
	if (!pSequence)
...
void PyTrackViewSetSequenceTimeRange(tukk name, float start, float end)
{
	const CTrackViewSequenceManager* pSequenceManager = CTrackViewPlugin::GetSequenceManager();
	CTrackViewSequence* pSequence = pSequenceManager->GetSequenceByName(name);
	if (!pSequence)
...
// Nodes
void PyTrackViewAddNode(tukk nodeTypeString, tukk nodeName)
{
...
CTrackViewAnimNode* GetNodeFromName(tukk nodeName, tukk parentDirectorName)
{
	CAnimationContext* pAnimationContext = CTrackViewPlugin::GetAnimationContext();
	CTrackViewSequence* pSequence = pAnimationContext->GetSequence();
	if (!pSequence)
...
void PyTrackViewDeleteNode(tukk nodeName, tukk parentDirectorName)
{
...
void PyTrackViewAddTrack(tukk paramName, tukk nodeName, tukk parentDirectorName)
{
	CTrackViewAnimNode* pNode = GetNodeFromName(nodeName, parentDirectorName);
	if (!pNode)
	{
	...
	i32k paramCount = pNode->GetParamCount();
	for (i32 i = 0; i < paramCount; ++i)
	{
		...
		{
			tukk name = pNode->GetParamName(paramType);
...
void PyTrackViewDeleteTrack(tukk paramName, u32 index, tukk nodeName, tukk parentDirectorName)
{
	CTrackViewAnimNode* pNode = GetNodeFromName(nodeName, parentDirectorName);
	if (!pNode)
	{
...
i32 PyTrackViewGetNumNodes(tukk parentDirectorName)
{
	CAnimationContext* pAnimationContext = CTrackViewPlugin::GetAnimationContext();
	CTrackViewSequence* pSequence = pAnimationContext->GetSequence();
	if (!pSequence)
...
string PyTrackViewGetNodeName(i32 index, tukk parentDirectorName)
{
	CAnimationContext* pAnimationContext = CTrackViewPlugin::GetAnimationContext();
	CTrackViewSequence* pSequence = pAnimationContext->GetSequence();
	if (!pSequence)
...
// Tracks
CTrackViewTrack* GetTrack(tukk paramName, u32 index, tukk nodeName, tukk parentDirectorName)
{
	CTrackViewAnimNode* pNode = GetNodeFromName(nodeName, parentDirectorName);
	if (!pNode)
	{
...
i32 PyTrackViewGetNumTrackKeys(tukk paramName, i32 trackIndex, tukk nodeName, tukk parentDirectorName)
{
	CTrackViewTrack* pTrack = GetTrack(paramName, trackIndex, nodeName, parentDirectorName);
	return (i32)GetKeyTimeSet(pTrack).size();
}

pSPyWrappedProperty PyTrackViewGetInterpolatedValue(tukk paramName, i32 trackIndex, float time, tukk nodeName, tukk parentDirectorName)
{
	CTrackViewTrack* pTrack = GetTrack(paramName, trackIndex, nodeName, parentDirectorName);
	...
	case eAnimValue_RGB:
		{
			...
			prop->property.colorValue.r = static_cast<i32>(clamp_tpl(value.x, 0.0f, 1.0f) * 255.0f);
			prop->property.colorValue.g = static_cast<i32>(clamp_tpl(value.y, 0.0f, 1.0f) * 255.0f);
			prop->property.colorValue.b = static_cast<i32>(clamp_tpl(value.z, 0.0f, 1.0f) * 255.0f);
		}
		break;
	default:
...
pSPyWrappedProperty PyTrackViewGetKeyValue(tukk paramName, i32 trackIndex, i32 keyIndex, tukk nodeName, tukk parentDirectorName)
{
	CTrackViewTrack* pTrack = GetTrack(paramName, trackIndex, nodeName, parentDirectorName);
...
REGISTER_PYTHON_COMMAND_WITH_EXAMPLE(PyTrackViewGetSequenceName, trackview, get_sequence_name,
                                     ...
                                     "trackview.get_sequence_name(i32 sequenceIndex)");
...
REGISTER_PYTHON_COMMAND_WITH_EXAMPLE(PyTrackViewDeleteTrack, trackview, delete_track,
                                     ...
                                     "trackview.delete_track(str paramType, i32 index, str nodeName, str parentDirectorName)");
...
REGISTER_PYTHON_COMMAND_WITH_EXAMPLE(PyTrackViewGetNodeName, trackview, get_node_name,
                                     "Gets the name of a sequence by its index.",
                                     "trackview.get_node_name(i32 nodeIndex, str parentDirectorName)");
...
REGISTER_PYTHON_COMMAND_WITH_EXAMPLE(PyTrackViewGetNumTrackKeys, trackview, get_num_track_keys,
                                     ...
                                     "trackview.get_num_track_keys(str paramName, i32 trackIndex, str nodeName, str parentDirectorName)");
REGISTER_PYTHON_COMMAND_WITH_EXAMPLE(PyTrackViewGetKeyValue, trackview, get_key_value,
                                     "Gets the value of the specified key",
                                     "trackview.get_num_track_keys(str paramName, i32 trackIndex, i32 keyIndex, str nodeName, str parentDirectorName)");
REGISTER_PYTHON_COMMAND_WITH_EXAMPLE(PyTrackViewGetInterpolatedValue, trackview, get_interpolated_value,
                                     ...
                                     "trackview.get_num_track_keys(str paramName, i32 trackIndex, float time, str nodeName, str parentDirectorName)");
