	...
	: CDockableEditor(pParent)
{
	...
	if (CTrackViewComponentsManager* pComponentsManager = m_pTrackViewCore->GetComponentsManager())
	{
		...
		i32k totalSize = pContentLayout->size().width();
		if (totalSize > 240)
		{
			QList<i32> sizes;
...
QVariantMap CTrackViewWindow::GetLayout() const
{
	...
	state.insert("unit", (i32)m_pTrackViewCore->GetCurrentDisplayMode());
...
void CTrackViewWindow::InitMenu()
{
...

	{
		...
		i32 sec = pFileMenu->GetNextEmptySection();
...

	{
		...
		i32k sec = pEditMenu->GetNextEmptySection();
	...
	pViewMenu->signalAboutToShow.Connect([pViewMenu, this]()
	{
		...
		i32 sec = pViewMenu->GetNextEmptySection();
	...
	pToolsMenu->signalAboutToShow.Connect([pToolsMenu, this]()
	{
		...
		i32 sec = pToolsMenu->GetNextEmptySection();
...
bool CTrackViewWindow::OnOpen()
{
	...
	if (dialog.exec())
	{
		for (const DrxGUID& seqGuid : dialog.GetSequenceGuids())
		{
