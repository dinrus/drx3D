...
{
public:
	CMovieCallback() : m_cameraObjectGUID(DrxGUID::Null()) {}
	void OnSetCamera(const SCameraParams& cameraParams)
	{
		...
		{
			m_cameraObjectGUID = DrxGUID::Null();
		}
	};
	...
	virtual DrxGUID GetActiveCameraObjectGUID() const
	{
...
void CAnimationContext::Update()
{
	...
	if ((GetIEditor()->GetViewManager()->GetCameraObjectId() != DrxGUID::Null()) && IsRecording())
	{
