...
CTrackViewSequence* CTrackViewSequenceManager::GetSequenceByGUID(DrxGUID guid) const
{
	for (auto iter = m_sequences.begin(); iter != m_sequences.end(); ++iter)
	{
		CTrackViewSequence* pSequence = (*iter).get();
...
CTrackViewSequence* CTrackViewSequenceManager::GetSequenceByIndex(u32 index) const
{
...
CTrackViewSequence* CTrackViewSequenceManager::CreateSequence(const string& name)
{
	...
	if (pGameEngine && pGameEngine->IsLevelLoaded() && !pExistingSequence)
	{
		...
		{
			DrxLog("$4Can't create new sequence object, do you have a valid layer selected?");
		}
	}
...
IAnimSequence* CTrackViewSequenceManager::OnCreateSequenceObject(const string& name)
{
	...
	IAnimSequence* pNewDrxMovieSequence = GetIEditor()->GetMovieSystem()->CreateSequence(name, false, ++m_nextSequenceId);
	CTrackViewSequence* pNewSequence = new CTrackViewSequence(pNewDrxMovieSequence);
