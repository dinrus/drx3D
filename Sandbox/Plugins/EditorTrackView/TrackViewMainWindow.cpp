...
#include <DrxMovie/AnimKey_impl.h>
...
CTrackViewMainWindow::CTrackViewMainWindow()
	: CDockableWindow()
	, m_activeSequenceUId(DrxGUID::Null())
...
void CTrackViewMainWindow::ShowNode(CTrackViewAnimNode* pNode)
{
	...
	DrxGUID sequenceGUID = pSequence->GetGUID();
...
void CTrackViewMainWindow::OnDopeSheetTabChanged(i32 index)
{
	UpdateActiveSequence();
}

void CTrackViewMainWindow::OnDopeSheetTabClose(i32 index)
{
...
void CTrackViewMainWindow::OnSequenceListItemPressed(QListWidgetItem* pItem)
{
	...
	const DrxGUID guid = *reinterpret_cast<const DrxGUID*>(guidByteArray.data());
...
{
	const QByteArray guidByteArray = pItem->data(Qt::UserRole).toByteArray();
	const DrxGUID sequenceGUID = *reinterpret_cast<const DrxGUID*>(guidByteArray.data());
	OpenSequenceTab(sequenceGUID);
}

void CTrackViewMainWindow::OpenSequenceTab(const DrxGUID sequenceGUID)
{
...
void CTrackViewMainWindow::UpdateSequenceList()
{
	...
	for (uint i = 0; i < numSequences; ++i)
	{
		...
		{
			const bool bNameMatchesFilter = DrxStringUtils::stristr(pSequence->GetName(), filterString) != nullptr;
		...
		const DrxGUID guid = pSequence->GetGUID();
		const QByteArray guidByteArray(reinterpret_cast<tukk >(&guid), sizeof(DrxGUID));
...
DrxGUID CTrackViewMainWindow::GetSequenceGUIDFromTabIndex(i32 index)
{
	return stl::find_in_map(m_tabToSequenceUIdMap, m_pDopesheetTabs->widget(index), DrxGUID::Null());
}

DrxGUID CTrackViewMainWindow::GetActiveSequenceGUID()
{
...
CTimeline* CTrackViewMainWindow::GetDopeSheetFromSequenceGUID(const DrxGUID& guid)
{
...
CCurveEditor* CTrackViewMainWindow::GetCurveEditorFromSequenceGUID(const DrxGUID& guid)
{
	auto findIter = m_idToSequenceDataMap.find(guid);
	if (findIter != m_idToSequenceDataMap.end())
	{
...
DrxGUID CTrackViewMainWindow::GetGUIDFromDopeSheetTrack(const STimelineTrack* pDopeSheetTrack) const
{
	Private_TrackViewWindow::STrackUserData userData;
...
{
	m_activeSequenceUId = pSequence ? pSequence->GetGUID() : DrxGUID::Null();
...
void CTrackViewMainWindow::OnTimeChanged(SAnimTime newTime)
{
	...
	if (pSequence)
	{
		const DrxGUID sequenceGUID = pSequence->GetGUID();
...
void CTrackViewMainWindow::OnNodeRenamed(CTrackViewNode* pNode, tukk pOldName)
{
