...
void CTrackViewSequenceDialog::UpdateSequenceList()
{
	...
	i32k numSequences = pSequenceManager->GetCount();
	for (i32 i = 0; i < numSequences; ++i)
	{
		CTrackViewSequence* pSequence = pSequenceManager->GetSequenceByIndex(i);
		if (!filterString.empty() && DrxStringUtils::stristr(pSequence->GetName(), filterString) == nullptr)
		{
			continue;
		}
...
void CTrackViewSequenceDialog::OnAccepted()
{
	...
	case OpenSequences:
		...
		{
			const DrxGUID guid = *reinterpret_cast<const DrxGUID*>(pItem->data(Qt::UserRole).toByteArray().data());
