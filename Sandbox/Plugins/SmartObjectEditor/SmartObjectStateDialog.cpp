...
HTREEITEM CSmartObjectStateDialog::AddItemInTreeCtrl(tukk name, const CString& location)
{
...
/*
   i32 CSmartObjectStateDialog::OnCompareItem( i32 id, LPCOMPAREITEMSTRUCT lpCompareItemStruct )
   {
   return stricmp( (LPCTSTR) lpCompareItemStruct->itemData1, (LPCTSTR) lpCompareItemStruct->itemData2 );
   }

   void CSmartObjectStateDialog::OnDrawItem( i32 id, LPDRAWITEMSTRUCT lpDrawItemStruct )
   {
...
/*
   ...
   if ( !m_bMultiple )
   {
    i32 nSel = m_wndStateList.GetCurSel();
    if ( nSel == LB_ERR)
      return;
   ...
   {
    // ignore arrow keys
    i32 newSelCount = m_wndStateList.GetSelCount();
    if ( newSelCount != m_selCount )
    {
      m_selCount = newSelCount;

      i32 nSel = m_wndStateList.GetCaretIndex();
      if ( nSel == LB_ERR)
        return;
      CString item;
