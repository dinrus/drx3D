...
i32 CSOLibrary::m_iNumEditors       = 0;
...
bool CSOLibrary::Load()
{
	...
	{
		m_bLoadNeeded = true;
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "CSOLibrary::Load() failed to load from %s", SMART_OBJECTS_XML);
		return false;
	}
...
CSOLibrary::CClassTemplateData* CSOLibrary::AddClassTemplate(tukk name)
{
...
public:
	...
	//////////////////////////////////////////////////////////////////////////
	// Exact positioning
	CVariableEnum<i32> approachSpeed;
	CVariableEnum<i32> approachStance;
	...
	//	CVarEnumList<i32> enumType;
	//	CVarEnumList<i32> enumBlendType;
	...
	bool GetConditionFromUI(SmartObjectCondition& condition) const
	{
		...
		condition.eActionType = (EActionType) (i32) actionType;
		actionName.Get(temp);
	...
	void SetConditionToUI(const SmartObjectCondition& condition, CPropertyCtrl* pPropertyCtrl)
	{
		...
		for (i32 i = 0; i < m_collapsedItems.size(); ++i)
...
private:
	...
	//////////////////////////////////////////////////////////////////////////
	void AddVariable(CVariableArray& varArray, CVariableBase& var, tukk varName, u8 dataType = IVariable::DT_SIMPLE, tukk description = NULL)
	{
	...
	//////////////////////////////////////////////////////////////////////////
	void AddVariable(CVarBlock* vars, CVariableBase& var, tukk varName, u8 dataType = IVariable::DT_SIMPLE, tukk description = NULL)
	{
		var.AddRef(); // Variables are local and must not be released by CVarBlock.
		if (varName)
			var.SetName(varName);
...
void CDragReportCtrl::DrawDropTarget()
{
	...
	{
		i32 sourceIdx = m_pSourceRow->GetIndex();
		i32 targetIdx = m_pTargetRow->GetIndex();
...
{
	i32 selCount = GetSelectedRows()->GetCount();
	...
	if (pRow || ptClient.y > GetReportRectangle().top)
	{
		...
		/*
		   ...
		   i32 nColumnCount = pColumns->GetCount();
		   for ( i32 i = 0; i < nColumnCount; ++i )
		   {
		...
		// track menu
		i32 nMenuResult = CXTPCommandBars::TrackPopupMenu(&menu, TPM_NONOTIFY | TPM_RETURNCMD | TPM_LEFTALIGN | TPM_RIGHTBUTTON, pos.x, pos.y, this, NULL);
...
class CSmartObjectsEditorViewClass : public IViewPaneClass
{
	...
	virtual tukk    ClassName()       override { return "Smart Objects Editor"; };
	virtual tukk    Category()        override { return "Game"; };
	virtual tukk    GetMenuPath()     override { return "Deprecated"; }
	virtual CRuntimeClass* GetRuntimeClass() override { return RUNTIME_CLASS(CSmartObjectsEditorDialog); };
	virtual tukk    GetPaneTitle()    override { return _T("Smart Objects Editor"); };
	...
	, m_pEditedEntry(NULL)
{
	...
	typedef Functor2<CBaseObject*, i32> EventCallback;
...
//////////////////////////////////////////////////////////////////////////
BOOL CSmartObjectsEditorDialog::OnCmdMsg(UINT nID, i32 nCode, uk pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
	BOOL res = FALSE;
...
BOOL CSmartObjectsEditorDialog::OnInitDialog()
{
	...
	public:
		...
		virtual i32 GetRowHeight(CDC* pDC, CXTPReportRow* pRow)
		{
...
void CSmartObjectsEditorDialog::ReloadEntries(bool bFromFile)
{
	...
	if (GetISystem()->GetIFlowSystem())
	{
		if (bFromFile)
		{
			...
			if (!CSOLibrary::Load())
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "CSmartObjectsEditorDialog::ReloadEntries() failed to load from %s", SMART_OBJECTS_XML);
				return;
			}
		}
	...
	if (m_pEditedEntry)
	{
		...
		if (pVar->GetDataType() == IVariable::DT_SOTEMPLATE)
		{
			...
			if (m_pEditedEntry->m_pCondition->iTemplateId != it->first)
			{
				...
				nmhdr.idFrom = IDC_REPORTCONTROL;
				nmhdr.code = XTP_NM_REPORT_SELCHANGED;
				PostMessage(WM_NOTIFY, (WPARAM) (i32) IDC_REPORTCONTROL, (LPARAM) &nmhdr);
			}
			return;
		}
...
void CSOLibrary::String2Classes(const string& sClass, SetStrings& classes)
{
	...
	i32 start = 0, end, length = sClass.length();
	string single;
	    ...
	    (pObject->GetType() & OBJTYPE_AIPOINT) && (!StrCmp(pObject->GetTypeName(), "AIAnchor") || !StrCmp(pObject->GetTypeName(), "SmartObject")))
	{
		...
		if (pVars)
		{
			...
			if (pVariable)
			{
				...
				pVariable->Get(value);
				if (!value.IsEmpty())
					CSOLibrary::String2Classes((tukk) value, classes);
			}
		}
...
void CSmartObjectsEditorDialog::SinkSelection()
{
	...
	i32 selCount = pSelection->GetCount();
	...
	else
	{
		...
		while (i--)
		{
			...
			if (classes.find((tukk) m_sEditedClass) != classes.end())
...
//////////////////////////////////////////////////////////////////////////
CSOLibrary::VectorEventData::iterator CSOLibrary::FindEvent(tukk name)
{
	Load();
...
//////////////////////////////////////////////////////////////////////////
void CSOLibrary::AddEvent(tukk name, tukk description)
{
	Load();
...
//////////////////////////////////////////////////////////////////////////
CSOLibrary::VectorStateData::iterator CSOLibrary::FindState(tukk name)
{
	Load();
...
//////////////////////////////////////////////////////////////////////////
void CSOLibrary::AddState(tukk name, tukk description, tukk location)
{
	Load();
...
//////////////////////////////////////////////////////////////////////////
CSOLibrary::VectorClassData::iterator CSOLibrary::FindClass(tukk name)
{
	Load();
...
//////////////////////////////////////////////////////////////////////////
void CSOLibrary::AddClass(tukk name, tukk description, tukk location, tukk templateName)
{
	Load();
...
//////////////////////////////////////////////////////////////////////////
CSOLibrary::CClassTemplateData const* CSOLibrary::FindClassTemplate(tukk name)
{
	if (!name || !*name)
		return NULL;
...
//////////////////////////////////////////////////////////////////////////
void CSmartObjectsEditorDialog::ModifyRuleOrder(i32 from, i32 to)
{
...
void CSmartObjectsEditorDialog::OnAddEntry()
{
	...
	if (sotd.DoModal() != IDOK)
		return;
	i32 templateId = sotd.GetSOTemplateId();
	...
	SmartObjectCondition condition =
	{
		...
		templateId,     //	i32		iTemplateId;

		0.0f,       //	float	fApproachSpeed;
		3,          //	i32		iApproachStance;
...
void CSmartObjectsEditorDialog::OnReportColumnRClick(NMHDR* pNotifyStruct, LRESULT* /*result*/)
{
	...
	// track menu
	i32 nMenuResult = CXTPCommandBars::TrackPopupMenu(&menu, TPM_NONOTIFY | TPM_RETURNCMD | TPM_LEFTALIGN | TPM_RIGHTBUTTON, ptClick.x, ptClick.y, this, NULL);
...
/*
   ...
   if ( pItemNotify->pRow->IsGroupRow() )
   {
    ...
    // track menu
    i32 nMenuResult = CXTPCommandBars::TrackPopupMenu( &menu, TPM_NONOTIFY | TPM_RETURNCMD | TPM_LEFTALIGN |TPM_RIGHTBUTTON, pItemNotify->pt.x, pItemNotify->pt.y, this, NULL);
...
bool CSOLibrary::Save()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "CSOLibrary::Save() failed to save in %s", SMART_OBJECTS_XML);
		return false;
	}
	DrxLog("CSOLibrary::Save() saved in %s", SMART_OBJECTS_XML);
	return true;
}
...
bool CSOLibrary::SaveToFile(tukk sFileName)
{
...
bool CSOLibrary::LoadFromFile(tukk sFileName)
{
