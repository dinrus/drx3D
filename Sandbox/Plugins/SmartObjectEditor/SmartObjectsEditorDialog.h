...
private:
	...
	static bool SaveToFile(tukk sFileName);
	static bool LoadFromFile(tukk sFileName);
...
protected:
	...
	static CClassTemplateData* AddClassTemplate(tukk name);
...
public:
	...
	static void                       AddEvent(tukk name, tukk description);
	static VectorEventData::iterator  FindEvent(tukk name);

	static void                       AddState(tukk name, tukk description, tukk location);
	static void                       AddAllStates(const string& sStates);
	static VectorStateData::iterator  FindState(tukk name);

	static CClassTemplateData const*  FindClassTemplate(tukk name);

	static void                       AddClass(tukk name, tukk description, tukk location, tukk templateName);
	static VectorClassData::iterator  FindClass(tukk name);
};
...
public:
	...
	void      OnObjectEventLegacy(CBaseObject* pObject, i32 eventType) { OnObjectEvent(CObjectEvent(static_cast<EObjectListenerEvent>(eventType), pObject)); }
	...
	void      ModifyRuleOrder(i32 from, i32 to);
