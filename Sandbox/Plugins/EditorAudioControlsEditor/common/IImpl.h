...
struct IImpl
{
	...
	virtual DrxIcon const& GetItemIcon(IItem const* const pIItem) const = 0;
	...
	CDrxSignal<void(ControlId const, SControlInfos&)> SignalGetConnectedSystemControls;
	...
	//! \param ControlId - Id of the selected middleware control.
	CDrxSignal<void(ControlId const, ControlId const)> SignalSelectConnectedSystemControl;
	...
	CDrxSignal<void(ExtensionFilterVector const&, QStringList const&, QString const&)> SignalImportFiles;
	...
	//! \param QString - Name of the target folder.
	CDrxSignal<void(FileImportInfos const&, QString const&)> SignalFilesDropped;
};
} //endns Impl
} //endns ACE
