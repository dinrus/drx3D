...
void CAudioControlsLoader::LoadControls(string const& folderPath)
{
	...
	intptr_t handle = pDrxPak->FindFirst(folderPath + s_controlsLevelsFolder + "*.*", &fd);
...
void CAudioControlsLoader::LoadAllLibrariesInFolder(string const& folderPath, string const& level)
{
	...
	if (handle != -1)
	{
		do
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[Audio Controls Editor] Failed parsing game sound file %s", filename);
			}
		}
		while (pDrxPak->FindNext(handle, &fd) >= 0);
...
//////////////////////////////////////////////////////////////////////////
void CAudioControlsLoader::LoadControlsLibrary(XmlNodeRef const pRoot, string const& filepath, string const& level, string const& filename, u32 const version)
{
...
//////////////////////////////////////////////////////////////////////////
CControl* CAudioControlsLoader::LoadControl(XmlNodeRef const pNode, Scope const scope, u32 const version, CAsset* const pParentItem)
{
	CControl* pControl = nullptr;

	if (pNode != nullptr)
	{
		bool const isInDefaultLibrary = (pParentItem->GetName().compareNoCase(DrxAudio::s_szDefaultLibraryName) == 0);
		QString pathName = "";
		...
		if (pFolderItem != nullptr)
		{
			...
			if (!((controlType == EAssetType::Switch) && ((name.compareNoCase("ObstrOcclCalcType") == 0) ||
			                                              ...
			                                              (name.compareNoCase(DrxAudio::s_szAbsoluteVelocityTrackingSwitchName) == 0) ||
			                                              (name.compareNoCase(DrxAudio::s_szRelativeVelocityTrackingSwitchName) == 0))) &&
			    !((controlType == EAssetType::Trigger) && ((name.compareNoCase(DrxAudio::s_szDoNothingTriggerName) == 0) ||
...
//////////////////////////////////////////////////////////////////////////
CControl* CAudioControlsLoader::LoadDefaultControl(XmlNodeRef const pNode, Scope const scope, u32 const version, CAsset* const pParentItem)
{
	CControl* pControl = nullptr;
		...
		else if ((controlType == EAssetType::Parameter) && (m_defaultParameterNames.find(name) != m_defaultParameterNames.end()))
		{
			...
			if (pControl == nullptr)
			{
				...
				if (pControl != nullptr)
				{
					...
					{
						pControl->SetName(DrxAudio::s_szAbsoluteVelocityParameterName);
					}
					else if (pControl->GetName().compareNoCase("object_doppler") == 0)
					{
						pControl->SetName(DrxAudio::s_szRelativeVelocityParameterName);
					}
...
void CAudioControlsLoader::LoadScopesImpl(string const& sLevelsFolder)
{
	...
	_finddata_t fd;
	IDrxPak* const pDrxPak = gEnv->pDrxPak;
	intptr_t const handle = pDrxPak->FindFirst(sLevelsFolder + "/*.*", &fd);
...
//////////////////////////////////////////////////////////////////////////
void CAudioControlsLoader::LoadPreloadConnections(XmlNodeRef const pNode, CControl* const pControl, u32 const version)
{
	...
	for (i32 i = 0; i < numChildren; ++i)
	{
		...
		i32 const numConnections = pGroupNode->getChildCount();
...
{
	if ((pFoldersNode != nullptr) && (library.GetName().compareNoCase(DrxAudio::s_szDefaultLibraryName) != 0))
	{
