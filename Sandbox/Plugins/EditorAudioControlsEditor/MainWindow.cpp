...
void CMainWindow::InitToolbar(QVBoxLayout* const pWindowLayout)
{
	...
	m_pSaveAction = m_pToolBar->addAction(DrxIcon("icons:General/File_Save.ico"), QString());
	...
	QAction* const pReloadAction = m_pToolBar->addAction(DrxIcon("icons:General/Reload.ico"), QString());
	...
	QAction* const pRefreshAudioSystemAction = m_pToolBar->addAction(DrxIcon("icons:Audio/Refresh_Audio.ico"), QString());
...
{
	DRX_ASSERT_MESSAGE(pSender != nullptr, "Dockable container is null pointer.");
...
{
	string levelPath = DrxAudio::s_szLevelsFolderName;
	levelPath += "/";
