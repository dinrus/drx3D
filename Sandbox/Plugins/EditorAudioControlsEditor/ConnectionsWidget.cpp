...
CConnectionsWidget::CConnectionsWidget(QWidget* const pParent)
	...
	, m_nameColumn(static_cast<i32>(CConnectionsModel::EColumns::Name))
{
	...
	m_pTreeView->header()->setSectionResizeMode(static_cast<i32>(CConnectionsModel::EColumns::Notification), QHeaderView::ResizeToContents);
...
void CConnectionsWidget::OnContextMenu(QPoint const& pos)
{
	...
	i32 const selectionCount = selection.count();
	...
	if (selectionCount > 0)
	{
		...
		if (selectionCount == 1)
		{
			ControlId const itemId = static_cast<ControlId>(selection[0].data(static_cast<i32>(ModelUtils::ERoles::Id)).toInt());
...
void CConnectionsWidget::OnConnectionAdded(ControlId const id)
{
	...
	{
		auto const& matches = m_pAttributeFilterProxyModel->match(m_pAttributeFilterProxyModel->index(0, 0, QModelIndex()), static_cast<i32>(ModelUtils::ERoles::Id), id, 1, Qt::MatchRecursive);
...
void CConnectionsWidget::RemoveSelectedConnection()
{
	if (m_pControl != nullptr)
	{
		...
		{
			i32 const numSelected = selectedIndexes.length();
			QString text;
			...
			if (messageBox->Execute() == QDialogButtonBox::Yes)
			{
				...
				{
					ControlId const id = static_cast<ControlId>(index.data(static_cast<i32>(ModelUtils::ERoles::Id)).toInt());
	...
	if (m_pControl != pControl)
	{
		...
		if ((m_pControl != nullptr) && restoreSelection)
		{
			...
			if (!selectedConnections.empty())
			{
				...
				{
					auto const& matches = m_pAttributeFilterProxyModel->match(m_pAttributeFilterProxyModel->index(0, 0, QModelIndex()), static_cast<i32>(ModelUtils::ERoles::Id), itemId, 1, Qt::MatchRecursive);
...
void CConnectionsWidget::ResizeColumns()
{
	...
	m_pTreeView->resizeColumnToContents(static_cast<i32>(CConnectionsModel::EColumns::Path));
}
