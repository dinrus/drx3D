	...
	, m_projectPath(projectPath)
{
	...
	string const locaFolder =
	  ...
	  DrxAudio::Impl::Wwise::s_szImplFolderName +
	  "/" +
	  DrxAudio::s_szAssetsFolderName;
...
{
	_finddata_t fd;
	intptr_t const handle = gEnv->pDrxPak->FindFirst(folderPath + "/*.*", &fd);
			...
			if ((name != ".") && (name != "..") && !name.empty())
			{
				if ((fd.attrib & _A_SUBDIR) == 0)
				{
					if ((name.find(".bnk") != string::npos) && (name.compareNoCase("Init.bnk") != 0))
					{
						...
						ControlId const id = DrxAudio::StringToId(fullname);
						...
						if (pItem == nullptr)
						{
							...
							EPakStatus const pakStatus = gEnv->pDrxPak->IsFileExist(fullname.c_str(), IDrxPak::eFileLocation_OnDisk) ? EPakStatus::OnDisk : EPakStatus::None;
...
void CProjectLoader::LoadFolder(string const& folderPath, string const& folderName, CItem& parent)
{
	...
	intptr_t const handle = pDrxPak->FindFirst(m_projectPath + "/" + folderPath + "/" + folderName + "/*.*", &fd);

	if (handle != -1)
	{
		EPakStatus const folderPakStatus = gEnv->pDrxPak->IsFileExist(fullFolderPath.c_str(), IDrxPak::eFileLocation_OnDisk) ? EPakStatus::OnDisk : EPakStatus::None;
		...
		do
		{
			...
			if ((name != ".") && (name != "..") && !name.empty())
			{
				...
				else
				{
					...
					EPakStatus const pakStatus = gEnv->pDrxPak->IsFileExist(fullPath.c_str(), IDrxPak::eFileLocation_OnDisk) ? EPakStatus::OnDisk : EPakStatus::None;
...
void CProjectLoader::LoadWorkUnitFile(const string& filePath, CItem& parent, EPakStatus const pakStatus)
{
	...
	if (pRoot != nullptr)
	{
		u32 const fileId = DrxAudio::StringToId(pRoot->getAttr("ID"));
		...
		if (m_filesLoaded.count(fileId) == 0)
		{
			...
			{
				u32 const parentDocumentId = DrxAudio::StringToId(pRoot->getAttr("RootDocumentID"));
				...
				if (m_items.count(parentDocumentId) == 0)
				{
					...
					{
						DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[Audio Controls Editor] [Wwise] Couldn't find Work Unit which file %s depends on. Are you missing wwise project files?.", filePath.c_str());
					}
				}
			}

			// Each files starts with the type of item and then the WorkUnit
			i32 const childCount = pRoot->getChildCount();
...
void CProjectLoader::LoadXml(XmlNodeRef const pRoot, CItem& parent, EPakStatus const pakStatus)
{
	if (pRoot != nullptr)
	{
		...
		if (type != EItemType::None)
		{
			...
			u32 const itemId = DrxAudio::StringToId(pRoot->getAttr("ID"));
		...
		{
			i32 const childCount = pChildren->getChildCount();
			for (i32 i = 0; i < childCount; ++i)
			{
...
CItem* CProjectLoader::CreateItem(const string& name, EItemType const type, CItem& parent, EPakStatus const pakStatus)
{
	...
	ControlId const id = DrxAudio::StringToId(fullPathName);
		...
		default:
			{
				...
				{
					pItem->SetRadius(m_eventsInfoMap[DrxAudio::StringToId(name.c_str())].maxRadius);
				}
			}
			break;
		}
...
void CProjectLoader::LoadEventsMetadata(const string& soundbanksPath)
{
	...
	if (pRootNode != nullptr)
	{
		...
		{
			i32 const numSoundBankNodes = pSoundBanksNode->getChildCount();

			for (i32 i = 0; i < numSoundBankNodes; ++i)
			{
				...
				if (pSoundBankNode != nullptr)
				{
					...
					{
						i32 const numEventNodes = pIncludedEventsNode->getChildCount();

						for (i32 j = 0; j < numEventNodes; ++j)
						{
							...
							if (pEventNode != nullptr)
							{
								...
								m_eventsInfoMap[DrxAudio::StringToId(pEventNode->getAttr("Name"))] = info;
							}
						}
					}
				}
...
void CProjectLoader::BuildFileCache(string const& folderPath)
{
	_finddata_t fd;
	IDrxPak* const pDrxPak = gEnv->pDrxPak;
	intptr_t const handle = pDrxPak->FindFirst(m_projectPath + "/" + folderPath + "/*.*", &fd);
		...
		do
		{
			...
			if ((name != ".") && (name != "..") && !name.empty())
			{
				...
				else
				{
					...
					if (pRoot != nullptr)
					{
						m_filesCache[DrxAudio::StringToId(pRoot->getAttr("ID"))] = path;
					}
				}
			}
		}
