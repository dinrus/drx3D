...
EItemType TagToType(char const* const szTag)
{
	...
	else if (_stricmp(szTag, DrxAudio::Impl::Fmod::s_szParameterTag) == 0)
	{
		type = EItemType::Parameter;
	}
	else if (_stricmp(szTag, DrxAudio::Impl::Fmod::s_szSnapshotTag) == 0)
	{
		type = EItemType::Snapshot;
	}
	else if (_stricmp(szTag, DrxAudio::Impl::Fmod::s_szFileTag) == 0)
	{
		type = EItemType::Bank;
	}
	else if (_stricmp(szTag, DrxAudio::Impl::Fmod::s_szBusTag) == 0)
	{
		type = EItemType::Return;
	}
	else if (_stricmp(szTag, DrxAudio::Impl::Fmod::s_szVcaTag) == 0)
	{
		type = EItemType::VCA;
	}
...
char const* TypeToTag(EItemType const type)
{
	...
	case EItemType::Parameter:
		szTag = DrxAudio::Impl::Fmod::s_szParameterTag;
		break;
	case EItemType::Snapshot:
		szTag = DrxAudio::Impl::Fmod::s_szSnapshotTag;
		break;
	case EItemType::Bank:
		szTag = DrxAudio::Impl::Fmod::s_szFileTag;
		break;
	case EItemType::Return:
		szTag = DrxAudio::Impl::Fmod::s_szBusTag;
		break;
	case EItemType::VCA:
		szTag = DrxAudio::Impl::Fmod::s_szVcaTag;
		break;
	default:
		szTag = nullptr;
		break;
...
CImpl::CImpl()
	...
	, m_assetsPath(AUDIO_SYSTEM_DATA_ROOT "/" + string(DrxAudio::Impl::Fmod::s_szImplFolderName) + "/" + string(DrxAudio::s_szAssetsFolderName))
	, m_szUserSettingsFile("%USER%/audiocontrolseditor_fmod.user")
{
	...
	m_implFolderName = DrxAudio::Impl::Fmod::s_szImplFolderName;
...
ConnectionPtr CImpl::CreateConnectionFromXMLNode(XmlNodeRef pNode, EAssetType const assetType)
{
	...
	if (pNode != nullptr)
	{
		...
		if (type != EItemType::None)
		{
			...
			string localizedAttribute = pNode->getAttr(DrxAudio::Impl::Fmod::s_szLocalizedAttribute);
#if defined (USE_BACKWARDS_COMPATIBILITY)
...
#endif      // USE_BACKWARDS_COMPATIBILITY
			bool const isLocalized = (localizedAttribute.compareNoCase(DrxAudio::Impl::Fmod::s_szTrueValue) == 0);
			...
			if ((pItem == nullptr) || (type != pItem->GetType()))
			{
				...
				string path = "";
				i32 const pos = name.find_last_of("/");
			...
			case EItemType::Event:
				{
					...
					if (actionType.compareNoCase(DrxAudio::Impl::Fmod::s_szStopValue) == 0)
					{
						eventActionType = CEventConnection::EActionType::Stop;
					}
					else if (actionType.compareNoCase(DrxAudio::Impl::Fmod::s_szPauseValue) == 0)
					{
						eventActionType = CEventConnection::EActionType::Pause;
					}
					else if (actionType.compareNoCase(DrxAudio::Impl::Fmod::s_szResumeValue) == 0)
					{
			...
			case EItemType::Snapshot:
				{
					...
					CSnapshotConnection::EActionType const snapshotActionType = (actionType.compareNoCase(DrxAudio::Impl::Fmod::s_szStopValue) == 0) ? CSnapshotConnection::EActionType::Stop : CSnapshotConnection::EActionType::Start;
			...
			case EItemType::VCA:
				{
					if ((assetType == EAssetType::Parameter) || (assetType == EAssetType::Environment))
					{
						float mult = DrxAudio::Impl::Fmod::s_defaultParamMultiplier;
						float shift = DrxAudio::Impl::Fmod::s_defaultParamShift;

						if (pNode->haveAttr(DrxAudio::Impl::Fmod::s_szMutiplierAttribute))
						{
							string const value = pNode->getAttr(DrxAudio::Impl::Fmod::s_szMutiplierAttribute);
...
#endif            // USE_BACKWARDS_COMPATIBILITY
						if (pNode->haveAttr(DrxAudio::Impl::Fmod::s_szShiftAttribute))
						{
							string const value = pNode->getAttr(DrxAudio::Impl::Fmod::s_szShiftAttribute);
					...
					{
						string valueString = pNode->getAttr(DrxAudio::Impl::Fmod::s_szValueAttribute);
#if defined (USE_BACKWARDS_COMPATIBILITY)
...
XmlNodeRef CImpl::CreateXMLNodeFromConnection(ConnectionPtr const pConnection, EAssetType const assetType)
{
	...
	if (pItem != nullptr)
	{
		...
		case EItemType::Event:
			{
				pNode->setAttr(DrxAudio::s_szNameAttribute, Utils::GetPathName(pItem, m_rootItem));
				...
				if (pEventConnection != nullptr)
				{
					...
					{
						pNode->setAttr(DrxAudio::s_szTypeAttribute, DrxAudio::Impl::Fmod::s_szStopValue);
					}
					else if (pEventConnection->GetActionType() == CEventConnection::EActionType::Pause)
					{
						pNode->setAttr(DrxAudio::s_szTypeAttribute, DrxAudio::Impl::Fmod::s_szPauseValue);
					}
					else if (pEventConnection->GetActionType() == CEventConnection::EActionType::Resume)
					{
						pNode->setAttr(DrxAudio::s_szTypeAttribute, DrxAudio::Impl::Fmod::s_szResumeValue);
					}
				}
			}
			break;
		...
		case EItemType::VCA:
			{
				...
				if (assetType == EAssetType::State)
				{
					...
					{
						pNode->setAttr(DrxAudio::Impl::Fmod::s_szValueAttribute, pStateConnection->GetValue());
					}
				}
				else if ((assetType == EAssetType::Parameter) || (assetType == EAssetType::Environment))
				{
					...
					if (pParamConnection->GetMultiplier() != DrxAudio::Impl::Fmod::s_defaultParamMultiplier)
					{
						pNode->setAttr(DrxAudio::Impl::Fmod::s_szMutiplierAttribute, pParamConnection->GetMultiplier());
					}

					if (pParamConnection->GetShift() != DrxAudio::Impl::Fmod::s_defaultParamShift)
					{
						pNode->setAttr(DrxAudio::Impl::Fmod::s_szShiftAttribute, pParamConnection->GetShift());
					}
				}
			}
			break;
		case EItemType::Bank:
			{
				...
				{
					pNode->setAttr(DrxAudio::Impl::Fmod::s_szLocalizedAttribute, DrxAudio::Impl::Fmod::s_szTrueValue);
				}
			}
			break;
		}
