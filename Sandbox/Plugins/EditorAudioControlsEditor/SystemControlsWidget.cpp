  ...
  Controls& outControls)
{
	...
	{
		QModelIndex const& nameColumnIndex = index.sibling(index.row(), static_cast<i32>(CSystemSourceModel::EColumns::Name));
		QVariant const internalPtr = nameColumnIndex.data(static_cast<i32>(ModelUtils::ERoles::InternalPointer));

		if (internalPtr.isValid())
		{
			QVariant const type = nameColumnIndex.data(static_cast<i32>(ModelUtils::ERoles::SortPriority));

			switch (type.toInt())
			{
			case static_cast<i32>(EAssetType::Library):
				outLibraries.push_back(reinterpret_cast<CLibrary*>(internalPtr.value<intptr_t>()));
				break;
			case static_cast<i32>(EAssetType::Folder):
	...
	, m_suppressRenaming(false)
{
	...
	m_pMountingProxyModel->SetHeaderDataCallbacks(static_cast<i32>(CSystemSourceModel::EColumns::Count), &CSystemSourceModel::GetHeaderData, Attributes::s_getAttributeRole);
	...
	m_pTreeView->header()->setSectionResizeMode(static_cast<i32>(CSystemSourceModel::EColumns::Notification), QHeaderView::ResizeToContents);
	m_pTreeView->header()->setSectionResizeMode(static_cast<i32>(CSystemSourceModel::EColumns::PakStatus), QHeaderView::ResizeToContents);
				...
				if (controls.size() == 1)
				{
					...
					else if (controlType == EAssetType::Preload)
					{
						if (pControl->GetScope() == GlobalScopeId && !pControl->IsAutoLoad())
						{
							...
							QObject::connect(pLoadAction, &QAction::triggered, [=]() { gEnv->pAudioSystem->PreloadSingleRequest(DrxAudio::StringToId(pControl->GetName()), false); });
							QObject::connect(pUnloadAction, &QAction::triggered, [=]() { gEnv->pAudioSystem->UnloadSingleRequest(DrxAudio::StringToId(pControl->GetName())); });
							pContextMenu->insertSeparator(pContextMenu->actions().at(0));
					...
					QObject::connect(pLoadAction, &QAction::triggered, [&]()
						{
							for (auto const pControl : controls)
							{
							  gEnv->pAudioSystem->PreloadSingleRequest(DrxAudio::StringToId(pControl->GetName()), false);
							}
					  });
					...
					QObject::connect(pUnloadAction, &QAction::triggered, [&]()
						{
							for (auto const pControl : controls)
							{
							  gEnv->pAudioSystem->UnloadSingleRequest(DrxAudio::StringToId(pControl->GetName()));
							}
					  });
...
{
	auto const& selection = m_pTreeView->selectionModel()->selectedRows(m_nameColumn);
	i32 const numSelected = selection.length();
...
//////////////////////////////////////////////////////////////////////////
void CSystemControlsWidget::SelectNewAsset(QModelIndex const& parent, i32 const row)
{
...
{
	ClearFilters();
	auto const& matches = m_pSystemFilterProxyModel->match(m_pSystemFilterProxyModel->index(0, 0, QModelIndex()), static_cast<i32>(ModelUtils::ERoles::Id), systemControlId, 1, Qt::MatchRecursive);
