...
CControl* CAssetsManager::CreateControl(string const& name, EAssetType const type, CAsset* const pParent /*= nullptr*/)
{
	...
	if ((pParent != nullptr) && !name.empty())
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[Audio Controls Editor] Controls must be created on either Libraries or Folders!");
		}
	}
	else
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[Audio Controls Editor] No valid parent or name to create control!");
	}
...
{
	m_scopes[DrxAudio::StringToId(name.c_str())] = SScopeInfo(name, isLocalOnly);
}

//////////////////////////////////////////////////////////////////////////
bool CAssetsManager::ScopeExists(string const& name) const
{
	return m_scopes.find(DrxAudio::StringToId(name.c_str())) != m_scopes.end();
}
...
{
	return DrxAudio::StringToId(name.c_str());
}
...
void CAssetsManager::Clear()
{
	...
	DRX_ASSERT_MESSAGE(m_controls.empty(), "m_controls is not empty.");
	DRX_ASSERT_MESSAGE(m_libraries.empty(), "m_systemLibraries is not empty.");
	ClearScopes();
	ClearDirtyFlags();
}
...
CAsset* CAssetsManager::CreateFolder(string const& name, CAsset* const pParent /*= nullptr*/)
{
	...
	if ((pParent != nullptr) && !name.empty())
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[Audio Controls Editor] Folders must be created on either Libraries or Folders!");
		}
	}
	else
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[Audio Controls Editor] No valid parent or name to create folder!");
	}
...
void CAssetsManager::UpdateFolderPaths()
{
	...
	if (g_pIImpl != nullptr)
	{
		...
		m_configFolderPath = implFolderPath + DrxAudio::s_szConfigFolderName + "/";
		m_assetFolderPath = implFolderPath + DrxAudio::s_szAssetsFolderName + "/";
	}
	else
	{
...
void CAssetsManager::UpdateAssetConnectionStates(CAsset* const pAsset)
{
	if (pAsset != nullptr)
	{
		...
		if ((assetType == EAssetType::Library) || (assetType == EAssetType::Folder) || (assetType == EAssetType::Switch))
		{
			...
			i32 const childCount = pAsset->ChildCount();
