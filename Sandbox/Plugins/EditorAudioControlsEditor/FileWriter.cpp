...
namespace ACE
{
u32 const CFileWriter::s_currentFileVersion = 3;
...
char const* TypeToTag(EAssetType const assetType)
{
	...
	case EAssetType::Parameter:
		szTag = DrxAudio::s_szParameterTag;
		break;
	case EAssetType::Trigger:
		szTag = DrxAudio::s_szTriggerTag;
		break;
	case EAssetType::Switch:
		szTag = DrxAudio::s_szSwitchTag;
		break;
	case EAssetType::State:
		szTag = DrxAudio::s_szStateTag;
		break;
	case EAssetType::Preload:
		szTag = DrxAudio::s_szPreloadRequestTag;
		break;
	case EAssetType::Environment:
		szTag = DrxAudio::s_szEnvironmentTag;
		break;
	default:
		szTag = nullptr;
		break;
	...
	if ((library.GetFlags() & EAssetFlags::IsModified) != 0)
	{
		...
		for (auto const& libraryPair : libraryXmlNodes)
		{
			...
			else
			{
				...
				libraryPath += DrxAudio::s_szLevelsFolderName;
			...
			if (libScope.isDirty)
			{
				XmlNodeRef pFileNode = GetISystem()->CreateXmlNode(DrxAudio::s_szRootNodeTag);
				pFileNode->setAttr(DrxAudio::s_szNameAttribute, library.GetName());
				pFileNode->setAttr(DrxAudio::s_szVersionAttribute, s_currentFileVersion);

				auto const numTypes = static_cast<i32>(EAssetType::NumTypes);

				for (i32 i = 0; i < numTypes; ++i)
				{
					if (i != static_cast<i32>(EAssetType::State))   // switch_states are written inside the switches
					{
				...
				// Editor data
				XmlNodeRef const pEditorData = pFileNode->createNode(DrxAudio::s_szEditorDataTag);
				...
				library.SetPakStatus(EPakStatus::OnDisk, gEnv->pDrxPak->IsFileExist(fullFilePath.c_str(), IDrxPak::eFileLocation_OnDisk));
			}
		}
	}
	else
...
void CFileWriter::WriteControlToXML(XmlNodeRef const pNode, CControl* const pControl, string const& path)
{
	...
	pChildNode->setAttr(DrxAudio::s_szNameAttribute, pControl->GetName());
	...
	if (type == EAssetType::Trigger)
	{
		...
		if (radius > 0.0f)
		{
			pChildNode->setAttr(DrxAudio::s_szRadiusAttribute, radius);
		}
	}
	...
	else if (type == EAssetType::Preload)
	{
		...
		{
			pChildNode->setAttr(DrxAudio::s_szTypeAttribute, DrxAudio::s_szDataLoadType);
		}
		...
		{
			XmlNodeRef const pFileNode = pChildNode->createNode(DrxAudio::s_szPlatformTag);
			pFileNode->setAttr(DrxAudio::s_szNameAttribute, g_platforms[i]);
...
//////////////////////////////////////////////////////////////////////////
void CFileWriter::WriteConnectionsToXML(XmlNodeRef const pNode, CControl* const pControl, i32 const platformIndex /*= -1*/)
{
	EAssetType const type = pControl->GetType();
		...
		if (pConnection != nullptr)
		{
			if ((type != EAssetType::Preload) || (pConnection->IsPlatformEnabled(static_cast<PlatformIndexType>(platformIndex))))
			{
				...
				if (pChild != nullptr)
				{
					...
					i32 const numNodeChilds = pNode->getChildCount();

					for (i32 j = 0; j < numNodeChilds; ++j)
					{
						XmlNodeRef const pTempNode = pNode->getChild(j);

						if ((pTempNode != nullptr) && (_stricmp(pTempNode->getTag(), pChild->getTag()) == 0))
						{
							i32 const numAttributes1 = pTempNode->getNumAttributes();
							i32 const numAttributes2 = pChild->getNumAttributes();
							...
							if (numAttributes1 == numAttributes2)
							{
								...
								for (i32 k = 0; k < numAttributes1; ++k)
								{
...
void CFileWriter::DeleteLibraryFile(string const& filepath)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[Audio Controls Editor] Failed to delete file %s", filepath);
	}
}
...
void CFileWriter::WriteFolderEditorData(CAsset const& library, XmlNodeRef const pParentNode) const
{
	...
	for (size_t i = 0; i < itemCount; ++i)
	{
		...
		if (pAsset->GetType() == EAssetType::Folder)
		{
			...
			{
				pFolderNode->setAttr(DrxAudio::s_szNameAttribute, pAsset->GetName());
	...
	for (size_t i = 0; i < itemCount; ++i)
	{
		...
		if (szTag != nullptr)
		{
			...
			if (pControlNode != nullptr)
			{
				...
				{
					pControlNode->setAttr(DrxAudio::s_szNameAttribute, asset.GetName());
