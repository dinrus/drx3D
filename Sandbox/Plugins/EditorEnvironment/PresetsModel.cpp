...
QModelIndex CPresetsModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
	if ((row < 0) || (column < 0))
		return QModelIndex();
...
i32 CPresetsModel::rowCount(const QModelIndex& parent) const
{
	CPresetsModelNode* pNode = GetEntryNode(parent);
	if (pNode == nullptr)
	{
	...
	return static_cast<i32>(pNode->children.size());
}

i32 CPresetsModel::columnCount(const QModelIndex& parent) const
{
	return eColumn_COUNT;
}

QVariant CPresetsModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
QVariant CPresetsModel::data(const QModelIndex& index, i32 role) const
{
	if (!index.isValid())
	{
		return QVariant();
			...
			else if (index.column() == eColumn_PakStatus)
			{
				if (pNode->type == CPresetsModelNode::EType_Leaf)
				{
					if (pNode->flags.AreAllFlagsActive(CPresetsModelNode::EFlags_InPak | CPresetsModelNode::EFlags_InFolder))
						return DrxIcon("icons:General/Pakfile_Folder.ico");
					if (pNode->flags.AreAnyFlagsActive(CPresetsModelNode::EFlags_InPak))
						return DrxIcon("icons:General/Pakfile.ico");
...
bool CPresetsModel::setData(const QModelIndex& index, const QVariant& value, i32 role /* = Qt::EditRole */)
{
	return false;
}
...
tukk CPresetsModel::GetHeaderSectionName(i32 section) const
{
	if (section == eColumn_Name)
	{
		return "Name";
...
void CPresetsModel::OnSignalBeginAddEntryNode(CPresetsModelNode* pEntryNode)
{
	...
	i32k newRowIndex = pEntryNode->pParent->children.size();
