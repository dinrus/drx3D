...
namespace
{
tukk szPresetsLibsPath = "libs/environmentpresets/";
...
void SavePresetsInNode(const QPresetsWidget* pPresetsWidget, ITimeOfDay* pTimeOfDay, CPresetsModelNode* pNode)
{
	if (pNode->type == CPresetsModelNode::EType_Leaf)
	{
		...
		{
			DrxLogAlways("ERROR: failed to save Environment Preset '%s'", pNode->path.c_str());
		}
	}
	else
	{
...
{
public:
	CPresetAsTempFileUndo(tukk name) : m_pTempFile(nullptr), m_presetName(name), m_bOk(false)
	{
	...
	tukk getPresetName() const { return m_presetName.c_str(); }
...
{
public:
	CPresetUndo(QPresetsWidget* pWidget, tukk name, tukk desc) : m_pUndoCommand(nullptr), m_description(desc)
	{
...
{
public:
	CPresetChangedUndoCommand(QPresetsWidget* pWidget, tukk newName, tukk oldName)
		: m_pPresetsWidget(pWidget), m_newName(newName), m_oldName(oldName)
	{
	}

	virtual i32         GetSize()        { return sizeof(*this); }
	virtual tukk GetDescription() { return "QPresetsWidget: Preset selected"; }
...
{
public:
	CPresetRemovedUndoCommand(QPresetsWidget* pWidget, tukk name)
		: CPresetAsTempFileUndo(name), m_pPresetsWidget(pWidget)
	{
	}

	virtual i32         GetSize()        { return sizeof(*this); }
	virtual tukk GetDescription() { return "QPresetsWidget: Preset removed"; }
	...
	virtual void        Undo(bool bUndo)
	{
		...
		if (pTimeOfDay)
		{
			tukk sPresetName = getPresetName();
			if (pTimeOfDay->AddNewPreset(sPresetName))
			{
				const STxt filename = m_pTempFile->fileName().toStdString();
...
{
public:
	CPresetResetUndoCommand(QPresetsWidget* pWidget, tukk name)
...
{
public:
	CPresetAddUndoCommand(QPresetsWidget* pWidget, const STxt& name, const STxt& selectedName)
		: m_pPresetsWidget(pWidget), m_presetName(name), m_selectedPresetName(selectedName)
	{
	}

	virtual i32         GetSize()        { return sizeof(*this); }
	virtual tukk GetDescription() { return "QPresetsWidget: Preset added"; }
	...
	virtual void        Undo(bool bUndo)
	{
		...
		if (pTimeOfDay)
		{
			tukk presetName = m_presetName.c_str();
			if (pTimeOfDay->RemovePreset(presetName))
			{
				tukk selectedName = m_selectedPresetName.c_str();
...
protected:
	QPresetsWidget*   m_pPresetsWidget;
	const STxt m_presetName;
	const STxt m_selectedPresetName;
};

class QPresetsWidget::CPresetAddExistingUndoCommand : public IUndoObject
{
public:
	CPresetAddExistingUndoCommand(QPresetsWidget* pWidget, const STxt& presetPath, const STxt& selectedName)
...
void QPresetsWidget::CreateUi()
{
	...
	i32k columnCount = m_presetsModel->columnCount(QModelIndex());
	for (i32 i = 1; i < columnCount; ++i)
	{
...
void QPresetsWidget::RefreshPresetList()
{
	...
	if (pTimeOfDay)
	{
		i32k nPresetCount = pTimeOfDay->GetPresetCount();
		...
		for (size_t i = 0; i < presets.size(); ++i)
		{
			...
			tukk szCurrentPath = preset.m_pName;
			while (true)
			{
				string name;
				string path;

				tukk szFolderName = szCurrentPath;
				tukk szFolderNameEnd = strchr(szCurrentPath, '/');
				...
				else
				{
					...
					tukk szPresetPath = pNewItem->path.c_str();
					pNewItem->flags.SetFlags(CPresetsModelNode::EFlags_InFolder, gEnv->pDrxPak->IsFileExist(szPresetPath, IDrxPak::eFileLocation_OnDisk));
					pNewItem->flags.SetFlags(CPresetsModelNode::EFlags_InPak, gEnv->pDrxPak->IsFileExist(szPresetPath, IDrxPak::eFileLocation_InPak));
...
void QPresetsWidget::AddNewPreset()
{
	...
	if (pTimeOfDay)
	{
		...
		if (!fileName.isEmpty())
		{
			...
			const STxt sPresetPath = fileName.toStdString();
...
void QPresetsWidget::ResetPreset(tukk szName)
{
	ITimeOfDay* pTimeOfDay = GetTimeOfDay();
	if (pTimeOfDay)
	{
...
void QPresetsWidget::RemovePreset(tukk szName)
{
	ITimeOfDay* pTimeOfDay = GetTimeOfDay();
	if (pTimeOfDay)
	{
...
void QPresetsWidget::OpenPreset(tukk szName)
{
	ITimeOfDay* pTimeOfDay = GetTimeOfDay();
	if (pTimeOfDay)
	{
		char fullPath[IDrxPak::g_nMaxPath];
		tukk szTemp = gEnv->pDrxPak->AdjustFileName(szPresetsLibsPath, fullPath, 0);
		...
		const STxt importPath = fileName.toStdString();
...
void QPresetsWidget::SavePresetAs(tukk szName)
{
	ITimeOfDay* pTimeOfDay = GetTimeOfDay();
	if (pTimeOfDay)
	{
		...
		const STxt exportPath = fileName.toStdString();
...
void QPresetsWidget::OnContextMenu(const QPoint& point)
{
	...
	if (index.isValid())
	{
		...
		if (pNode->type == CPresetsModelNode::EType_Leaf)
		{
			...
			QAction* savePresetAction = new QAction(DrxIcon("icons:General/File_Save.ico"), "Save", &menu);
	...
	QMenu* addMenu = menu.addMenu(DrxIcon("icons:General/Element_Add.ico"), "Add");
...
tukk QPresetsWidget::GetSelectedItem() const
{
