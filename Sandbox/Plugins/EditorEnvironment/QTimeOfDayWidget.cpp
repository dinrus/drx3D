...
i32k nRulerHeight = 30;
i32k nRulerGradientHeight = 14;
const float fToDParameterComareEpsilon = 0.0001f;
tukk sClipboardCurveContentMimeTypeFloat = "binary/curveeditorcontentfloat";
tukk sClipboardCurveContentMimeTypeColor = "binary/curveeditorcontentcolor";
...
{
	i32k nSeconds = QTime(0, 0, 0).secsTo(time);
...
{
	u32 hour = floor(fTime);
	u32 minute = floor((fTime - floor(fTime)) * nTimeLineMinutesScale + 0.5f);
	if (hour > 23)
	{
		hour = 23;
		minute = 59;
...
void UpdateCurveContentFromToDVar(i32 nVarId, i32 nSplineId, SCurveEditorContent* pContent)
{
	ITimeOfDay* pTimeOfDay = GetTimeOfDay();
...
void UpdateToDVarFromCurveContent(i32 nVarId, i32 nSplineId, const SCurveEditorContent* pContent)
{
	ITimeOfDay* pTimeOfDay = GetTimeOfDay();
...
QRgb ColorLinearToGamma(const Vec3& col)
{
	...
	return qRgb(i32(r), i32(g), i32(b));
}

void DrawGradient(ITimeOfDay* pTimeOfDay, i32 selectedParam, QPainter& painter, const QRect& rect, const Range& visibleRange)
{
	if (selectedParam >= 0 && selectedParam < pTimeOfDay->GetVariableCount())
	{
		...
		{
			i32k xMin = rect.left();
			i32k xMax = rect.right();
			i32k nRange = xMax - xMin;
			...
			else
			{
				for (size_t i = 0; i < nRange; ++i)
				{
					...
					i32k grayScaled = i32(normVal * 255.0f);
...
public:
	...
	virtual i32         GetSize()        { return sizeof(*this) + m_oldState.size() + m_newState.size(); }
	virtual tukk GetDescription() { return "QTimeOfDayWidget content changed"; }
	...
	i32 GetParamId() const { return m_paramId; }
protected:
...
void QTimeOfDayWidget::UpdateCurveContent()
{
	...
	{
		ITimeOfDay* pTimeOfDay = GetTimeOfDay();
		i32k paramID = m_selectedParamId;
		ITimeOfDay::SVariableInfo sVarInfo;
...
{
	i32k oldSelectedParam = m_selectedParamId;
...
void QTimeOfDayWidget::UndoEnd()
{
	...
	if (m_pUndoCommand)
	{
		...
		i32k paramId = m_pUndoCommand->GetParamId();
		if (paramId >= 0)
		{
			const STODParameter* pParam = m_groups.m_params[paramId];
			tukk paramName = pParam->GetName();
...
void QTimeOfDayWidget::OnCopyCurveContent()
{
	...
	if (pContent)
	{
		...
		tukk mimeType = (pParam->GetTODParamType() == eFloatType) ? sClipboardCurveContentMimeTypeFloat : sClipboardCurveContentMimeTypeColor;
...
void QTimeOfDayWidget::CreateUi()
{
...

	{
		...
		toolBar->addAction(DrxIcon("icons:CurveEditor/Copy_Curve.ico"), "Copy curve content", this, SLOT(OnCopyCurveContent()));
		toolBar->addAction(DrxIcon("icons:CurveEditor/Paste_Curve.ico"), "Paste curve content", this, SLOT(OnPasteCurveContent()));
...

		{
			...
			stopButton->setIcon(DrxIcon("icons:Trackview/Stop_Sequence.ico"));
			stopButton->setCheckable(true);
			stopButton->setChecked(!m_bIsPlaying);
			stopButton->setAutoExclusive(true);

			QToolButton* playButton = new QToolButton;
			playButton->setIcon(DrxIcon("icons:Trackview/Play_Sequence.ico"));
...
void QTimeOfDayWidget::LoadPropertiesTree()
{
	...
	ITimeOfDay* pTimeOfDay = gEnv->p3DEngine->GetTimeOfDay();
	i32k nTODParamCount = pTimeOfDay->GetVariableCount();

	std::map<string, size_t> tempMap;
	i32 nGroupId = 0;
	for (i32 varID = 0; varID < nTODParamCount; ++varID)
	{
		ITimeOfDay::SVariableInfo sVarInfo;
	...
	for (i32 varID = 0; varID < nTODParamCount; ++varID)
	{
		...
		if (pTimeOfDay->GetVariableInfo(varID, sVarInfo))
		{
			i32k nParamID = sVarInfo.nParamId;
			tukk sGroupName = sVarInfo.group;

			i32k groupId = tempMap[sGroupName];
	...
	for (STODParameterGroupSet::TPropertyGroupMap::iterator it = m_groups.m_propertyGroups.begin(); it != m_groups.m_propertyGroups.end(); ++it)
	{
		...
		for (size_t i = 0; i < nParamSize; ++i)
		{
			...
			i32k nParamId = pParam->GetParamID();
			...
			{
				assert(false);
				DrxLogAlways("ERROR: QTimeOfDayWidget::LoadPropertiesTree() Parameter mismatch");
			}
		}
	}
...
void QTimeOfDayWidget::UpdateValues()
{
	...
	//m_playSpeedEdit->setVal(advInfo.fAnimSpeed);
...
void QTimeOfDayWidget::UpdateSelectedParamId()
{
	...
	while (row && row->parent())
	{
		...
		{
			i32 index = row->parent()->childIndex(row);
...
{
	ITimeOfDay* pTimeOfDay = gEnv->p3DEngine->GetTimeOfDay();
	i32k nToDVarCount = pTimeOfDay->GetVariableCount();
	for (i32 varID = 0; varID < nToDVarCount; ++varID)
	{
		ITimeOfDay::SVariableInfo sVarInfo;
		if (pTimeOfDay->GetVariableInfo(varID, sVarInfo))
		{
