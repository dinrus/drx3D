			...
			IFacialEffector* RecurseToExpression(HTREEITEM item, bool forceOutputAll = false)
			{
				...
				else
				{
					if (sourceEffector->GetType() == EFE_TYPE_GROUP)
					{
						...
						for (i32 i = 0; i < sourceEffector->GetSubEffectorCount(); ++i)
						{
							...
							IFacialEffector* childEffector = RecurseToExpression(child, forceOutputAll);
							i32 subEffectorIndex = pEffector->GetSubEffectorCount();
...
HTREEITEM CExpressionsTreeCtrl::AddEffector(IFacialEffector* pEffector, IFacialEffCtrl* pCtrl, HTREEITEM hParent)
{
	...
	i32 nSubCount = pEffector->GetSubEffectorCount();
...
//////////////////////////////////////////////////////////////////////////
void CExpressionsTreeCtrl::OnFacialEdEvent(EFacialEdEvent event, IFacialEffector* pEffector, i32 nChannelCount, IFacialAnimChannel** ppChannels)
{
	if (m_bIgnoreEvent)
		return;
...
void CExpressionsTreeCtrl::HandleOrphans()
{
	...
	i32 orphanCount = FacialExpressionUtils::AddOrphansToGarbageFacialExpressionFolder(pLibrary);
	if (orphanCount)
	{
		string text;
...
//////////////////////////////////////////////////////////////////////////
void CExpressionsTabCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
//////////////////////////////////////////////////////////////////////////
void CFacialExpressionsDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
	__super::OnSize(nType, cx, cy);
...
void CFacialExpressionsDialog::OnNewFolder()
{
	...
	if (m_pContext->pSelectedEffector)
	{
		...
		if (dlg.exec() == QDialog::Accepted)
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Facial Effector with name %s already exist", (tukk)sName);
				return;
			}
...
//////////////////////////////////////////////////////////////////////////
void CFacialExpressionsDialog::OnNewExpression(i32 nType)
{
	if (!m_pContext)
		return;
	...
	if (m_pContext->pSelectedEffector)
	{
		...
		if (dlg.exec() == QDialog::Accepted)
		{
			...
			if (pEffector)
			{
				...
				{
					CString str;
					str.Format(_T("An expression by the name of %s is in the garbage - do you want to delete it and create a new one?"), (tukk)sName);
...
void CFacialExpressionsDialog::OnRename()
{
	...
	if (dlg.exec() == QDialog::Accepted)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Facial Expression with name %s already exist"), (tukk)sName);
			return;
		}
...
void CFacialExpressionsDialog::OnTreeRClick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	i32 cmd = menu.TrackPopupMenu(TPM_RETURNCMD | TPM_LEFTALIGN | TPM_LEFTBUTTON, p.x, p.y, this);
	switch (cmd)
	{
	case CMD_NEWFOLDER:
		OnNewFolder();
...
//////////////////////////////////////////////////////////////////////////
void CFacialExpressionsDialog::OnFacialEdEvent(EFacialEdEvent event, IFacialEffector* pEffector, i32 nChannelCount, IFacialAnimChannel** ppChannels)
{
	switch (event)
	{
	case EFD_EVENT_ADD:
...
void CFacialExpressionsDialog::OnBeginDrag(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	for (i32 i = 0; i < m_treeCtrl.GetSelectedCount(); ++i)
	{
		if (i > 0)
