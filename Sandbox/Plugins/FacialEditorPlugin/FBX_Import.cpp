...
#include <DrxAnimation/IFacialAnimation.h> // for handling the actual facial sequence and it's data
#include "Dialogs/GenericSelectItemDialog.h"       // access to the listbox dialog class

#ifdef DRX_ENABLE_FBX_SDK
...
FbxScene* LoadFBXFile(tukk filename)
{
	...
	if (!lImportStatus)
	{
		...
		DrxLog("[ERROR] Facial Editor: Could not import FBX file. Importer return error: '%s'", lImporter->GetStatus().GetErrorString());
		return NULL;
	}
...
FbxAnimStack* GetAnimStackToUse(FbxScene* lScene, CWnd* parent)
{
	...
	i32 numAnimStacks = lScene->GetSrcObjectCount(FbxCriteria::ObjectType(FbxAnimStack::ClassId));
	if (numAnimStacks <= 0)
	{
		CQuestionDialog::SWarning(QObject::tr(""), QObject::tr("FBX file has no animation. Aborting import."));
		DrxLog("[ERROR] Facial Editor: FBX file has no animation. Aborting import.");
		return NULL;
	}
	//	DrxLogAlways("Found %i AnimStacks", numAnimStacks);
	...
	if (numAnimStacks > 1)
	{
		...
		for (i32 n = 0; n < numAnimStacks; ++n)
		{
			...
			//		DrxLogAlways("AnimStack #%i: %s", n, pAnimStack->GetName());
		}
	...
	if (pAnimStack->GetMemberCount(FbxCriteria::ObjectType(FbxAnimLayer::ClassId)) > 1)
	{
		...
		DrxLog("[Warning] Facial Editor: Found more than one animation layer in FBX file. Only first layer will be imported!");
	}

	//DrxLogAlways("Using AnimStack %s", pAnimStack->GetName());
...
FbxNode* GetMeshToUse(FbxNode* lRootNode, CWnd* parent)
{
	...
	std::vector<CString> items;
	i32 childCount = lRootNode->GetChildCount();
	...
	if (items.size() < 0)
	{
		...
		DrxLog("[ERROR] Facial Editor: FBX file contains no meshes. Aborting import.");
		return NULL;
	}
...
#endif //DRX_ENABLE_FBX_SDK

//////////////////////////////////////////////////////////////////////////
void CFacialEditorDialog::LoadFBXToSequence(tukk filename)
{
...
bool CFacialEditorDialog::ImportFBXAsSequence(IFacialAnimSequence* newSequence, tukk filename)
{
#ifdef DRX_ENABLE_FBX_SDK
	...
	if (pMesh->GetDeformerCount(FbxDeformer::eBlendShape) <= 0 || pMesh->GetShapeCount() <= 0)
	{
		...
		DrxLog("[Error] Facial Editor FBX Importer: Found no deformer/morpher or no blendshapes in selected model. Aborting import");
		return false;
	}
	...
	//DrxLogAlways("Found Anim Stack to contain animation from %.2f to %.2f seconds (duration %.4f seconds)", secondStartTime, secondEndTime, duration);
	...
	i32 deformerCount = pMesh->GetDeformerCount(FbxDeformer::eBlendShape);
	for (i32 deformerIdx = 0; deformerIdx < deformerCount; ++deformerIdx)
	{
		FbxBlendShape* pDeformer = (FbxBlendShape*)pMesh->GetDeformer(deformerIdx, FbxDeformer::eBlendShape);
		i32 blendShapeChannelCount = pDeformer->GetBlendShapeChannelCount();
		for (i32 blendShapeChannelIdx = 0; blendShapeChannelIdx < blendShapeChannelCount; ++blendShapeChannelIdx)
		{
			...
			if (pChannel->GetTargetShapeCount() > 1 && !warningAboutMultiTargetsShowed)
			{
				...
				DrxLog("[WARNING] Facial Editor: Found one or more blend channels that contain more that one morph target shape.\nThis is not supported, only the first shape will be evaluated.");
				// only show this warning once per import, to avoid making the user click a hundred times
				warningAboutMultiTargetsShowed = true;
			}

			i32 keyCount = pAnimCurve->KeyGetCount();
			//DrxLogAlways("Evaluating blendchannel %s. Found %i keys", pChannel->GetName(), keyCount);
			...
			for (i32 keyIdx = 0; keyIdx < keyCount; ++keyIdx)
			{
	...
	if (channelNode->getChildCount() == 0)
	{
		...
		DrxLog("[WARNING] Facial Editor: Animation Take contained no keyframes for any blendshapes (only layer 0 was evaluated). Aborting Import");
		return false;
	}
...
#else //DRX_ENABLE_FBX_SDK
	DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "FBX SDK not supported.");
	return false;
#endif //DRX_ENABLE_FBX_SDK
}
