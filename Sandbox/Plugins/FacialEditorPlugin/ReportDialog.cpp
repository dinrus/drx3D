...
CReportDialog::CReportDialog(tukk title, CWnd* pParent)
	: CXTResizeDialog(IDD_ERROR_REPORT, pParent),
	m_title(title)
{
...
{
	if (!GetSafeHwnd())
		Create(IDD_ERROR_REPORT, AfxGetMainWnd());
	Load(report);
	ShowWindow(SW_SHOW);
}
...
void CReportDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
class StrLess : public std::less<tukk >
{
public:
...
{
	typedef std::map<tukk , i32, StrLess> ColumnMap;
	ColumnMap columnMap;
	...
	i32 numColumns = 0;
	for (i32 recordIndex = 0; report && recordIndex < report->GetRecordCount(); ++recordIndex)
	{
		IReportRecord* record = (report ? report->GetRecord(recordIndex) : 0);
		for (i32 fieldIndex = 0; record && fieldIndex < record->GetFieldCount(); ++fieldIndex)
		{
			tukk description = (record ? record->GetFieldDescription(fieldIndex) : 0);
	...
	{
		std::vector<tukk > columnDescriptions;
		...
		for (std::vector<tukk >::iterator column = columnDescriptions.begin(); column != columnDescriptions.end(); ++column)
...

	{
		...
		for (i32 recordIndex = 0; report && recordIndex < report->GetRecordCount(); ++recordIndex)
		{
			...
			{
				tukk description = (record ? record->GetFieldDescription(fieldIndex) : 0);
				tukk text = (record ? record->GetFieldText(fieldIndex) : 0);
				ColumnMap::iterator columnIterator = (description ? columnMap.find(description) : columnMap.end());
				i32 columnIndex = (columnIterator != columnMap.end() ? (*columnIterator).second : -1);
				if (columnIndex >= 0 && columnIndex < i32(items.size()) && text)
