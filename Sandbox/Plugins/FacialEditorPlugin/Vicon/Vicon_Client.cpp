...
u32 numJoints = sizeof(BipedSkeleton) / sizeof(SBipedJoint);

//////////////////////////////////////////////////////////////////////////
struct SViconJoint
{
	SViconJoint(tukk szVicon, tukk szBip, f32 six, f32 siy, f32 siz, i32 nParent)
	{
		if (szVicon && szBip)
		{
...
bool recieve(SOCKET Socket, tuk pBuffer, i32 BufferSize)
{
	tuk p = pBuffer;
	tuk e = pBuffer + BufferSize;
...
bool recieve(SOCKET Socket, i32& Val)
{
	return recieve(Socket, (tuk) &Val, sizeof(Val));
}

bool recieve(SOCKET Socket, u64& Val)
{
	return recieve(Socket, (tuk) &Val, sizeof(Val));
}
...
//////////////////////////////////////////////////////////////////////////
i32 Vicon_PostProcessCallback(ICharacterInstance* pInstance, uk pPlayer)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CViconClient::GetIDbyName(tukk strJointName)
{
	i32 idx = -1;
	for (u32 i = 0; i < numJoints; i++)
	{
		u32 res = stricmp(BipedSkeleton[i].m_strJointName, strJointName);
		if (res == 0)
		{
			idx = i;
			break;
...
void CViconClient::ExternalPostProcessing(ICharacterInstance* pInstance)
{
	...
	i32 nNameLen = strlen(sActorName);
	...
	if (pInstance)
	{
		...
		for (iBody = m_BodyChannels.begin(), iBodyData = m_bodyPositions.begin(); iBody != m_BodyChannels.end(); iBody++, iBodyData++)
		{
			...
			tukk joint_name = iBody->m_BipName.c_str();
			i32 idx = GetIDbyName(joint_name);
			...
			i32 ortho = m33.IsOrthonormalRH(0.01f);
			...
			i32 idxRFoot = GetIDbyName("Bip01 R Foot");
			i32 idxLFoot = GetIDbyName("Bip01 L Foot");
			if (idx == idxRFoot || idx == idxLFoot)
			{
				...
				IRenderAuxText::DrawLabel(vBPos, 1.0f, iBody->m_BipName.c_str());
			}
			g_pAuxGeom->DrawSphere(vBPos, 0.01f, ColorB(255, 0, 255));
			IRenderAuxText::DrawLabel(varrBPos[idx], 1.0f, iBody->Name.c_str());
			...
			u32 numViconJoints = sizeof(ViconSkeleton) / sizeof(SViconJoint) - 1;
			tukk VicName1 = iBody->Name.c_str();
			VicName1 = strstr(VicName1, ":") + 1; // skip useless stuff
			for (u32 i = 0; i < numViconJoints; i++)
			{
				tukk VicName2 = ViconSkeleton[i].sViconName.c_str();
				i32 eg = stricmp(VicName1, VicName2);
				if (eg)
					continue;
		...
		if (IsInitialized)
		{
			...
			i32 a_idxSrc1 = GetIDbyName("Bip01 Pelvis");
			i32 a_idxSrc2 = GetIDbyName("Bip01 Spine1");

			i32 b_idxSrc1 = GetIDbyName("Bip01 Spine1");
			i32 b_idxSrc2 = GetIDbyName("Bip01 Spine3");

			i32 c_idxSrc1 = GetIDbyName("Bip01 Spine3");
			i32 c_idxSrc2 = GetIDbyName("Bip01 Head");

			i32 idxSpine0 = GetIDbyName("Bip01 Spine");
			i32 idxSpine2 = GetIDbyName("Bip01 Spine2");
			i32 idxSpine3 = GetIDbyName("Bip01 Spine3");
			i32 idxNeck = GetIDbyName("Bip01 Neck");
			i32 idxNeck1 = GetIDbyName("Bip01 Neck1");

			i32 vidxNeck1 = rIDefaultSkeleton.GetJointIDByName("Bip01 Neck1");
			...
			i32 idxLFoot = GetIDbyName("Bip01 L Foot");
			assert(idxLFoot > -1);
			i32 idxLToe0 = GetIDbyName("Bip01 L Toe0");
			assert(idxLToe0 > -1);
			i32 idxLToe1 = GetIDbyName("Bip01 L Toe1");
			assert(idxLToe1 > -1);
			...
			i32 idxRFoot = GetIDbyName("Bip01 R Foot");
			assert(idxRFoot > -1);
			i32 idxRToe0 = GetIDbyName("Bip01 R Toe0");
			assert(idxRToe0 > -1);
			i32 idxRToe1 = GetIDbyName("Bip01 R Toe1");
			assert(idxRToe1 > -1);
			...
			{
				i32 p = BipedSkeleton[i].m_idxParent;
			...
			if (1)
			{
				i32 Pelvis = GetIDbyName("Bip01 Pelvis");
				assert(Pelvis > -1);

				i32 rThigh = GetIDbyName("Bip01 R Thigh");
				assert(rThigh > -1);
				i32 rCalf = GetIDbyName("Bip01 R Calf");
				assert(rCalf > -1);
				i32 rFoot = GetIDbyName("Bip01 R Foot");
				assert(rFoot > -1);
				i32 rToe0 = GetIDbyName("Bip01 R Toe0");
				assert(rToe0 > -1);
				FixInverseBending(Pelvis, rThigh, rCalf, rFoot, rToe0);

				i32 lThigh = GetIDbyName("Bip01 L Thigh");
				assert(lThigh > -1);
				i32 lCalf = GetIDbyName("Bip01 L Calf");
				assert(lCalf > -1);
				i32 lFoot = GetIDbyName("Bip01 L Foot");
				assert(lFoot > -1);
				i32 lToe0 = GetIDbyName("Bip01 L Toe0");
				assert(lToe0 > -1);
			...
			if (0)
			{
				...
				i32 rToe0 = GetIDbyName("Bip01 R Toe0");
				assert(rToe0 > -1);
				i32 rToe1 = GetIDbyName("Bip01 R Toe1");
				assert(rToe1 > -1);
				...
				i32 lToe0 = GetIDbyName("Bip01 L Toe0");
				assert(lToe0 > -1);
				i32 lToe1 = GetIDbyName("Bip01 L Toe1");
				assert(lToe1 > -1);
...

			{
				...
				f32 dot = cross | axisZ;
				if (dot <= 0)
					IRenderAuxText::Draw2dLabel(1, 200, 1.6f, fC1, false, "Inverse Bending on right leg");
			}

			{
				...
				f32 dot = cross | axisZ;
				if (dot <= 0)
					IRenderAuxText::Draw2dLabel(1, 220, 1.6f, fC1, false, "Inverse Bending on left leg");
			}
			...
			IRenderAuxText::Draw2dLabel(1, 300, 1.6f, fC1, false, "LToe1: %f %f %f", ltoe1.x, ltoe1.y, ltoe1.z);
			IRenderAuxText::Draw2dLabel(1, 320, 1.6f, fC1, false, "RToe1: %f %f %f", rtoe1.x, rtoe1.y, rtoe1.z);
			...
			{
				i32 idx = rIDefaultSkeleton.GetJointIDByName(BipedSkeleton[i].m_strJointName);
				if (idx < 0)
					continue;
				//	if (i>1)
			...
			g_pAuxGeom->SetRenderFlags(e_Def3DPublicRenderflags);
			u32 numViconJoints = sizeof(ViconSkeleton) / sizeof(SViconJoint) - 1;
			for (u32 i = 1; i < numViconJoints; i++)
			{
				i32 p = ViconSkeleton[i].m_ParentIdx;
				if (p < 0)
					continue;
...
//---              make sure we have no Hyper Extension or Inverse Bending                   ---
//----------------------------------------------------------------------------------------------
void CViconClient::FixInverseBending(i32 Pelvis, i32 Thigh, i32 Calf, i32 Foot, i32 Toe0)
{
...
//---              make sure we have no Hyper Extension or Inverse Bending                   ---
//----------------------------------------------------------------------------------------------
void CViconClient::TwoBoneSolver(Vec3d goal, i32 Pelvis, i32 Thigh, i32 Calf, i32 Foot, i32 Toe0, i32 Toe1)
{
...
//////////////////////////////////////////////////////////////////////////
MarkerData* CViconClient::FindMarker(tukk szName)
{
	...
	{
		tukk szSrc = iMarker->Name.c_str();
		if (stricmp(szSrc, szName) == 0)
		{
...
//////////////////////////////////////////////////////////////////////////
Vec3 CViconClient::GetTransformedMarkerPos(tukk szName)
{
...
bool CViconClient::Connect(IConsoleCmdArgs* pArgs)
{
	...
	i32 result = connect(m_SocketHandle, (struct sockaddr*) &Endpoint, sizeof(Endpoint));

	if (result == SOCKET_ERROR)
	{
		u32 nError = WSAGetLastError();
	...
	i32k bufferSize = 2040;
	char buff[bufferSize];
	tuk pBuff;

	pBuff = buff;

	*((i32*) pBuff) = ClientCodes::EInfo;
	pBuff += sizeof(i32);
	*((i32*) pBuff) = ClientCodes::ERequest;
	pBuff += sizeof(i32);
	...
	for (iInfo = m_info.begin(); iInfo != m_info.end(); iInfo++)
	{
		...
		{
			gEnv->pLog->LogError("<Vicon Client> Error recieving i32 s");
			PrintErrorDescription(WSAGetLastError());
			Disconnect();
			return false;
		}
	...
	for (iInfo = m_info.begin(); iInfo != m_info.end(); iInfo++)
	{
		...
		else if (iBody != m_BodyChannels.end())
		{
	...
	for (iBody = m_BodyChannels.begin(); iBody != m_BodyChannels.end(); iBody++, nCounter++)
	{
		...
		{
			tukk szSrc = iBody->Name.c_str();
			tukk szStart = strstr(szSrc, ":"); // skip useless stuff
			if (!szStart)
				szStart = szSrc;
			else
			{
				//gEnv->pLog->Log("<Vicon Client> %s %s %d",szSrc,szStart,szStart-szSrc);
				drx_strcpy(szTemp, szSrc, (size_t)(szStart - szSrc));
				szStart++; // skip ":"
					...
					if (pEntity)
					{
						...
						if (pCharacter)
						{
							...
							for (u32 i = 1; i < numJoints; i++)
							{
								...
								tukk strBipName = BipedSkeleton[i].m_strJointName;  //initialize this joint
								i32 idx = rIDefaultSkeleton.GetJointIDByName(strBipName);
								if (idx < 0)
									continue;
								...
								BipedSkeleton[i].m_idxParent = -1;
								i32 p = rIDefaultSkeleton.GetJointParentIDByID(idx);
								if (p > -1)
								{
									tukk pname = rIDefaultSkeleton.GetJointNameByID(p);
									i32 ParentID = GetIDbyName(pname);
									assert(ParentID > -1);
									if (p > -1)
			...
			if (stricmp(ViconSkeleton[i].sViconName.c_str(), szStart) == 0)
			{
...
void CViconClient::Update()
{
	...
	*((i32*) pBuff) = ClientCodes::ERequest;
	...
	i32 nSize = m_lstEntities.size();
	for (i32 k = 0; k < nSize; k++)
	{
		...
		i32 nNameLen = strlen(m_lstEntityNames[k].c_str());

		bool bCam = false;
		if (strnicmp(pEnt->GetName(), "camera", 6) == 0)
			bCam = true;
