...
static tukk boneNames[FORCED_ROTATION_ENTRY_COUNT] = { "Bip01 Neck", "eye_left_bone", "eye_right_bone" };
...
public:
		...
		m_pContext(0)
	{
		...
		for (i32 i = 0; i < FORCED_ROTATION_ENTRY_COUNT; ++i)
			m_forcedRotations[i].rotation.SetIdentity();

		for (i32 i = 0; i < StoredCameraIndexCount; ++i)
...
//////////////////////////////////////////////////////////////////////////
void CFacialPreviewDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
BOOL CFacialPreviewDialog::OnInitDialog()
{
	...
	for (i32 i = 0; i < m_pModelViewport->GetVarObject()->GetNumVariables(); ++i)
	{
...
void CFacialPreviewDialog::OnCenterOnHead()
{
	...
	{
		i16 id = m_pContext->pCharacter->GetIDefaultSkeleton().GetJointIDByName("Bip01 Head");
		if (id >= 0)
		{
...
//////////////////////////////////////////////////////////////////////////
static tukk eyeBones[2] = { "eye_left_bone", "eye_right_bone" };
void CModelViewportFE::Update()
{
	...
	if (m_bAnimateCamera)
	{
		...
		i32 positionSplineKeyCount = (pPositionSpline ? pPositionSpline->GetKeyCount() : 0);
		i32 orientationSplineKeyCount = (pOrientationSpline ? pOrientationSpline->GetKeyCount() : 0);
		...
		i32 fovSplineKeyCount = (pFOVSpline ? pFOVSpline->GetKeyCount() : 0);
	...
	if (pCharacter && pCharacter->GetISkeletonAnim())
	{
		...
		if (m_bShowEyeVectors)
		{
			...
			for (i32 boneIndex = 0; boneIndex < sizeof(eyeBones) / sizeof(eyeBones[0]); ++boneIndex)
			{
				tukk bone = eyeBones[boneIndex];
				i32 boneID = pCharacter->GetIDefaultSkeleton().GetJointIDByName(bone);
				if (boneID >= 0)
				{
...
//////////////////////////////////////////////////////////////////////////
void CFacialPreviewDialog::OnFacialEdEvent(EFacialEdEvent event, IFacialEffector* pEffector, i32 nChannelCount, IFacialAnimChannel** ppChannels)
{
	switch (event)
	{
...
void CFacialPreviewDialog::SetForcedEyeRotation(const Quat& rotation, EyeType eye)
{
	if (eye >= 0 && eye < 2)
	{
		i32k eyeEntries[] = { FORCED_ROTATION_ENTRY_EYE_LEFT, FORCED_ROTATION_ENTRY_EYE_RIGHT };
...
Vec3 CModelViewportFE::CalculateLookIKTarget()
{
	...
	{
		tukk bone = eyeBones[boneIndex];
		i32 boneID = GetCharacterBase()->GetIDefaultSkeleton().GetJointIDByName(bone);
		if (boneID >= 0)
		{
			Matrix34 abs34 = Matrix34(GetCharacterBase()->GetISkeletonPose()->GetAbsJointByID(boneID));
