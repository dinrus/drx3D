...
//////////////////////////////////////////////////////////////////////////
void CFacialSequenceDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CFacialSequenceDialog::Update()
{
	...
	//{
	//	char buffer[1024];
	//	drx_sprintf(buffer, "Getting time = %f\n", m_pContext->GetSequenceTime());
	...
	{
		//DrxLogAlways("Update(): fSoundTime = %f", fSoundTime);
...
BOOL CFacialSequenceDialog::OnInitDialog()
{
	...
	for (i32 i = 0; i < pSplineBar->GetControlCount(); ++i)
	{
	...
	// Initialize sequence controls.
	{
		...
		{
			ButtonDescription(CXTPToolBar* pToolbar, CXTPControlEdit*& control, i32 id, XTPControlType type, const string& text, i32 width, unsigned flags)
		...
		for (i32 i = 0; i < BUTTON_DESCRIPTION_COUNT; ++i)
		{
...

	{
		...
		{
		public:
			ButtonDescription(CXTPToolBar* pToolbar, i32 id, tukk caption, CXTPControlButton** button)
		...
		for (i32 buttonIndex = 0; buttonIndex < NUM_BUTTONS; ++buttonIndex)
		{
...
void CFacialSequenceDialog::OnTreeRClick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = TRUE;
	...
	CPoint pos;
	GetCursorPos(&pos);
	i32 cmd = menu.TrackPopupMenu(TPM_RETURNCMD | TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_NONOTIFY, pos.x, pos.y, this);
	switch (cmd)
	{
...
void CFacialSequenceDialog::OnMoveWaveforms(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	i32 soundEntryCount = (pSequence ? pSequence->GetSoundEntryCount() : 0);
	...
	std::vector<i32> unresolvedWaveforms(soundEntryCount);
	for (i32 i = 0; i < soundEntryCount; ++i)
		unresolvedWaveforms[i] = i;

	// Continue moving waveforms until there are no overlaps.
	i32 waveformToMovePositionInUnresolvedWaveforms = pNotification->waveformIndex;
	float deltaTime = pNotification->deltaTime;
	while (waveformToMovePositionInUnresolvedWaveforms >= 0)
	{
		i32 waveformToMoveIndex = unresolvedWaveforms[waveformToMovePositionInUnresolvedWaveforms];
		...
		for (i32 unresolvedIndex = 0, unresolvedCount = unresolvedWaveforms.size(); waveformToMovePositionInUnresolvedWaveforms < 0 && unresolvedIndex < unresolvedCount; ++unresolvedIndex)
		{
			i32 unresolvedWaveformIndex = unresolvedWaveforms[unresolvedIndex];
			...
			i32 direction = 0; // The first time we are stopped we are free to choose a direction in which to move.
			const float borderEpsilon = 0.001f;
			for (bool foundPlace = false; !foundPlace; )
			{
				foundPlace = true;
				for (i32 invalidRegionIndex = 0, invalidRegionCount = invalidRegionCentres.size(); invalidRegionIndex < invalidRegionCount; ++invalidRegionIndex)
				{
...
void CFacialSequenceDialog::OnEndWaveformChanges(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	GetIEditor()->GetIUndoManager()->Accept("Sound Time Changes");
	ReloadPhonemeCtrl();
}
...
void CFacialSequenceDialog::AddVertexDragChannel()
{
	...
	for (i32 i = 0; i < pSequence->GetChannelCount(); i++)
	{
		if ((pSequence->GetChannel(i)->GetFlags() & IFacialAnimChannel::FLAG_GROUP) &&
		    (stricmp(pSequence->GetChannel(i)->GetName(), "LipSync") == 0))
		{
	...
	for (SelectedTreeItemIterator itSel = BeginSelectedTreeItems(&m_channelsCtrl), end = EndSelectedTreeItems(&m_channelsCtrl); itSel != end; ++itSel)
	{
		for (TreeItemDataIterator<IFacialAnimChannel, RecursiveTreeItemIteratorTraits> it = BeginTreeItemDataRecursive<IFacialAnimChannel>(&m_channelsCtrl, *itSel), end = EndTreeItemDataRecursive<IFacialAnimChannel>(&m_channelsCtrl, *itSel); it != end; ++it)
		{
			...
			if (pChannel && pChannel->GetInterpolatorCount() > 1)
			{
				...
				for (i32 keyIndex = 0, keyCount = pLayerSpline->GetKeyCount(); keyIndex < keyCount; ++keyIndex)
				{
					float v;
				...
				for (i32 keyIndex = 0, keyCount = pSpline->GetKeyCount(); keyIndex < keyCount; ++keyIndex)
				{
					float v1, v2;
...
{
	static i32k shapeRange = 5;
	static i32k firstShapeKeyFrameOffset = -((shapeRange - 1) >> 1);
	static i32k lastShapeKeyFrameOffset = (shapeRange >> 1);

	float currentTime = (m_pContext ? m_pContext->GetSequenceTime() : 0.0f);
	i32 currentFrame = i32(FacialEditorSnapTimeToFrame(currentTime) * FACIAL_EDITOR_FPS + 0.5f);
	i32 rangeStart = currentFrame + firstShapeKeyFrameOffset;
	i32 rangeEnd = currentFrame + lastShapeKeyFrameOffset;
	...
	i32 firstExistingKey = -1, numExistingKeys = 0;
	if (pSpline)
	...
	if (numExistingKeys > 0)
	{
		...
		if (QDialogButtonBox::StandardButton::Yes != CQuestionDialog::SQuestion(QObject::tr(""), QObject::tr("The shape will overwrite existing keys.\r\nDo you want to continue?")))
		{
			return;
		}
	}

	{
		...
		i32 keyFrames[] = { rangeStart, currentFrame, rangeEnd };
...
{
	ShapeToAddEntry(IFacialAnimChannel* pChannel, i32 mainFrame) : pChannel(pChannel), mainFrame(mainFrame) {}
...
{
	KeyToAddEntry(IFacialAnimChannel* pChannel, i32 frame, float value)
...
void CFacialSequenceDialog::InsertVisimeSeriesIntoChannel()
{
	...
	QStringDialog dlg("Enter visime string:");
	string visimeString;
	if (dlg.exec() == QDialog::Accepted)
		visimeString = dlg.GetString();
	else
		return;
	...
	for (;; )
	{
		i32 token_start = visimeString.find_first_not_of(" \t", position);
		if (token_start == string::npos)
			break;
		i32 next_whitespace = visimeString.find_first_of(" \t", token_start);
		i32 token_end = (next_whitespace == string::npos ? i32(visimeString.size()) : next_whitespace);
	...
	for (i32 tokenIndex = 0, tokenCount = i32(tokens.size()); tokenIndex < tokenCount; ++tokenIndex)
	{
		string effectorName = "Visime_" + tokens[tokenIndex];

		// Search for this visime in the channels.
		IFacialAnimChannel* pVisimeChannel = 0;
		for (i32 channelIndex = 0, channelCount = (pSequence ? pSequence->GetChannelCount() : 0); channelIndex < channelCount; ++channelIndex)
		{
			IFacialAnimChannel* pChannel = (pSequence ? pSequence->GetChannel(channelIndex) : 0);
			tukk szEffectorName = (pChannel ? pChannel->GetEffectorName() : 0);
			if (stricmp((szEffectorName ? szEffectorName : ""), effectorName) == 0)
	...
	for (i32 shapeIndex = 0, shapeCount = i32(visimeChannels.size()); shapeIndex < shapeCount; ++shapeIndex)
	{
	...
	for (i32 shapeIndex = 0, shapeCount = i32(shapesToAdd.size()); shapeIndex < shapeCount; ++shapeIndex)
	{
		...
		i32 firstExistingKey = -1, numExistingKeys = 0;
		i32 rangeStart = shapesToAdd[shapeIndex].mainFrame + firstShapeKeyFrameOffset;
		i32 rangeEnd = shapesToAdd[shapeIndex].mainFrame + lastShapeKeyFrameOffset;
		if (pSpline)
			pSpline->FindKeysInRange(float(rangeStart) / FACIAL_EDITOR_FPS, float(rangeEnd) / FACIAL_EDITOR_FPS, firstExistingKey, numExistingKeys);
	...
	{
		if (QDialogButtonBox::StandardButton::Yes != CQuestionDialog::SQuestion(QObject::tr(""), QObject::tr("The shapes will overwrite existing keys.\r\nDo you want to continue?")))
		{
			return;
		}
	}
...

	{
		...
		float sequenceStart = (pSequence ? pSequence->GetTimeRange().start : 0.0f);
		float sequenceEnd = (pSequence ? pSequence->GetTimeRange().end : 0.0f);
		for (i32 keyIndex = 0, keyCount = i32(keysToAdd.size()); keyIndex < keyCount; ++keyIndex)
		{
		...
		if (!missingVisimes.empty())
		{
			...
			CQuestionDialog::SWarning(QObject::tr(""), QObject::tr(missingVisimeMessage));
		}
	}
}
...
IFacialAnimChannel* CFacialSequenceDialog::GetBakedLipSyncGroup()
{
	...
	{
		if ((pSequence->GetChannel(i)->GetFlags() & IFacialAnimChannel::FLAG_GROUP) &&
		    (stricmp(pSequence->GetChannel(i)->GetName(), "BakedLipSync") == 0))
		{
...
IFacialAnimChannel* CFacialSequenceDialog::GetChannelFromSpline(ISplineInterpolator* pSpline)
{
	...
	for (i32 channelIndex = 0; pSequence && channelIndex < pSequence->GetChannelCount(); ++channelIndex)
	{
		IFacialAnimChannel* pChannel = (pSequence ? pSequence->GetChannel(channelIndex) : 0);
		for (i32 i = 0, count = pChannel->GetInterpolatorCount(); i < count; ++i)
		{
...
{
	string text;
	text.Format("%d%%", i32((100 * m_waveCtrl.GetPlaybackSpeed()) + 0.5f));
	if (m_pSpeedEdit)
...
float CFacialSequenceDialog::ReadPlaybackSpeedFromToolbar()
{
	...
	if (m_pSpeedEdit)
	{
		...
		tukk buf = text.GetString();
		tuk end;
		i32 percentage = strtol(buf, &end, 10);
		if (buf != end)
...
void CFacialSequenceDialog::ReloadWaveCtrlSounds()
{
	...
	{
		i32 waveformIndex = m_waveCtrl.AddWaveform();
...
public:
	ChannelIndexPair() {}
	ChannelIndexPair(IFacialAnimChannel* pChannel, i32 index) : pChannel(pChannel), index(index) {}
	...
	if (pSequence)
	{
		...
		private:
			void Recurse(HTREEITEM item)
			{
				...
				{
					i32 index = (*stl::binary_find(channelIndexMap.begin(), channelIndexMap.end(), ChannelIndexPair(pChannel, 0))).index;
		...
		for (size_t i = 0; i < channelsToOutput.size(); ++i)
		{
			...
			{
				i32 parentIndex = (*stl::binary_find(sortedChannelIndexMap.begin(), sortedChannelIndexMap.end(), ChannelIndexPair(pChannel, 0))).index;
		...
		for (size_t i = 0; i < channelsToOutput.size(); ++i)
		{
			if (channelsToOutput[i])
			{
				...
				if (pOriginalSpline && pCloneSpline)
				{
					...
					for (i32 key = 0; key < pOriginalSpline->GetKeyCount(); ++key)
					{
						ISplineInterpolator::ValueType value;
						pOriginalSpline->GetKeyValue(key, value);
						i32 cloneKeyIndex = pCloneSpline->InsertKey(FacialEditorSnapTimeToFrame(pOriginalSpline->GetKeyTime(key)), value);
		...
		for (size_t i = 0; i < channelsToOutput.size(); ++i)
		{
			if (channelsToOutput[i])
			{
				...
				if (pChannel->GetParent())
				{
					i32 parentIndex = (*stl::binary_find(sortedChannelIndexMap.begin(), sortedChannelIndexMap.end(), ChannelIndexPair(pChannel->GetParent(), 0))).index;
...
//////////////////////////////////////////////////////////////////////////
i32 CFacialSequenceDialog::GetSplineCount() const
{
	IFacialAnimSequence* pSequence = (m_pContext ? m_pContext->GetSequence() : 0);
	i32 totalCurves = 0;
	for (i32 channelIndex = 0; pSequence && channelIndex < pSequence->GetChannelCount(); ++channelIndex)
...
//////////////////////////////////////////////////////////////////////////
i32 CFacialSequenceDialog::GetKeyCountAtTime(float time, float threshold) const
{
	//IFacialAnimSequence* pSequence = (m_pContext ? m_pContext->GetSequence() : 0);
	...
	//	for (i32 key = 0; pSpline && key < pSpline->GetKeyCount(); ++key)
	//	{
...
void CFacialSequenceDialog::OnNewFolder()
{
	...
	QStringDialog dlg("New Folder Name");
	if (dlg.exec() == QDialog::Accepted)
	{
...
void CFacialSequenceDialog::OnRenameFolder()
{
	...
	QStringDialog dlg("Change Folder Name");
...
//////////////////////////////////////////////////////////////////////////
void CFacialSequenceDialog::SelectChannels(i32 numChannels, IFacialAnimChannel** ppChannels, bool bExclusive)
{
	...
	for (i32 channelIndex = 0; channelIndex < numChannels; ++channelIndex)
	{
...
//////////////////////////////////////////////////////////////////////////
void CFacialSequenceDialog::OnFacialEdEvent(EFacialEdEvent event, IFacialEffector* pEffector, i32 nChannelCount, IFacialAnimChannel** ppChannels)
{
	if (!m_pContext)
		return;
...
void CFacialSequenceDialog::OnTimeChanged()
{
	...
	{
		CString timestr;
		i32 nmsec = fCurrentTime * 1000.0f;
		i32 nsec = nmsec / 1000;
		i32 mins = nsec / 60;
		i32 sec = nsec % 60;
		i32 ms = nmsec % 1000;
	...
	{
		CString frameString;
		frameString.Format("%03d", i32(fCurrentTime * FACIAL_EDITOR_FPS));
...
void CFacialSequenceDialog::OnPhonemesChange(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	for (i32 soundEntryIndex = 0, soundEntryCount = pSequence->GetSoundEntryCount(); soundEntryIndex < soundEntryCount; ++soundEntryIndex)
	{
		...
		for (i32 i = 0; i < m_phonemesCtrl.GetPhonemeCount(soundEntryIndex); i++)
		{
			CPhonemesCtrl::Phoneme& ctrlPh = m_phonemesCtrl.GetPhoneme(soundEntryIndex, i);
			IFacialSentence::Phoneme ph;
			drx_strcpy(ph.phoneme, ctrlPh.sPhoneme);
...
void CFacialSequenceDialog::OnPhonemesPreview(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	tukk phoneme = m_phonemesCtrl.GetMouseOverPhoneme();
	if (phoneme && *phoneme)
	{
...
void CFacialSequenceDialog::OnPhonemesBake(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	for (i32 phonemeIndex = 0, phonemeCount = (pPhonemeLib ? pPhonemeLib->GetPhonemeCount() : 0); phonemeIndex < phonemeCount; ++phonemeIndex)
	{
	...
	typedef std::map<string, i32, stl::less_stricmp<tukk >> PhonemeChannelMap;
	PhonemeChannelMap phonemeChannelMap;
	for (i32 phonemeIndex = 0, phonemeCount = i32(phonemeNames.size()); phonemeIndex < phonemeCount; ++phonemeIndex)
	{
		...
		if (pChannel)
		{
			...
			phonemeChannelMap.insert(std::make_pair(phonemeNames[phonemeIndex], i32(phonemes.size())));
		}
	...
	i32 frameCount = (m_pContext ? i32(m_pContext->GetTimelineLength() * FACIAL_EDITOR_FPS) : 0);
	for (i32 frameIndex = 0; frameIndex < frameCount; ++frameIndex)
	{
		...
		for (i32 splineIndex = 0, splineCount = (pPhonemeStrengthChannel ? pPhonemeStrengthChannel->GetInterpolatorCount() : 0); splineIndex < splineCount; ++splineIndex)
		{
		...
		std::vector<i32> lastKeyedFrames(phonemeNames.size());
		...
		for (i32 soundEntryIndex = 0, soundEntryCount = (pSequence ? pSequence->GetSoundEntryCount() : 0); soundEntryIndex < soundEntryCount; ++soundEntryIndex)
		{
			...
			if (time >= sentenceStart && time < sentenceEnd)
			{
				...
				i32 sampleCount = pSentence->Evaluate(time - sentenceStart, phonemeStrength, MAX_SAMPLES, samples);
				sampleCount = min(sampleCount, i32(MAX_SAMPLES));
				...
				for (i32 sampleIndex = 0; sampleIndex < sampleCount; ++sampleIndex)
				{
					...
					i32 channelIndex = (mapPosition != phonemeChannelMap.end() ? (*mapPosition).second : -1);
...
void CFacialSequenceDialog::ReloadPhonemeCtrl()
{
	...
	for (i32 soundEntryIndex = 0, soundEntryCount = pSequence->GetSoundEntryCount(); soundEntryIndex < soundEntryCount; ++soundEntryIndex)
	{
		...
		i32 sentenceIndex = m_phonemesCtrl.AddSentence();
		...
		i32 nWords = pSentence->GetWordCount();
		for (i = 0; i < nWords; i++)
		{
		...
		i32 nPhonemes = pSentence->GetPhonemeCount();
		for (i = 0; i < nPhonemes; i++)
		{
			...
			if (pSentence->GetPhoneme(i, ph))
			{
				...
				drx_strcpy(ctrlPh.sPhoneme, ph.phoneme);
...
//////////////////////////////////////////////////////////////////////////
void CFacialSequenceDialog::LipSync(i32 waveformIndex)
{
	IFacialAnimSequence* pSequence = m_pContext->GetSequence();
	if (!pSequence)
		return;
	...
	if (wavFile.IsEmpty())
		return;
	wavFile = PathUtil::GamePathToDrxPakPath(wavFile.GetString());
...
//////////////////////////////////////////////////////////////////////////
void CFacialSequenceDialog::RemoveSoundEntry(i32 waveformIndex)
{
	IFacialAnimSequence* pSequence = (m_pContext ? m_pContext->GetSequence() : 0);
	if (m_pContext && pSequence)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CFacialSequenceDialog::DoPhonemeExtraction(CString wavFile, CString strText, IFacialAnimSequence* pSequence, i32 soundEntryIndex)
{
	...
	{
		DrxLogAlways("Phoneme Extraction failed for file \"%s\"", wavFile.GetString());
		DrxLogAlways("  Used text was \"%s\"", strText);
		DrxLogAlways("  Error message was \"%s\"", analyzer.GetLastError());
	}
	...
	if (!extractedPhonemes)
	{
		if (analyzer.Analyze(wavFile, "", &pOuSentenece))
		{
			extractedPhonemes = true;
			phonemeText = "";
			DrxLogAlways("  Successfully extracted phonemes using empty text");
		}
		else
		{
			DrxLogAlways("  Also failed when using empty text: \"%s\"", analyzer.GetLastError());
		}
	}
	...
	pSentence->ClearAllPhonemes();
	for (i32 w = 0; w < (i32)pOuSentenece->nWordCount; w++)
	{
	...
	for (i32 p = 0; p < pOuSentenece->nPhonemeCount; p++)
	{
		...
		drx_strcpy(ph.phoneme, outPhoneme.sPhoneme);
