...
BOOL CFacialJoystickDialog::OnInitDialog()
{
	...
	{
	public:
		ButtonDescription(ButtonStyle style, i32 id, tukk caption, CXTPControl** controlPointer)
...
void CFacialJoystickDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CFacialJoystickDialog::OnFacialEdEvent(EFacialEdEvent event, IFacialEffector* pEffector, i32 nChannelCount, IFacialAnimChannel** ppChannels)
{
	switch (event)
	{
...
void CFacialJoystickDialog::OnUpdateSnapMarginSizeUI(CCmdUI* pCmdUI)
{
	...
	i32 snapMargin = m_ctrl.GetSnapMargin();
	if (m_snapMargins[m_displayedSnapMargin] != snapMargin)
	{
		...
		i32 sel = CB_ERR;
		for (i32 index = 0; index < i32(m_snapMargins.size()); ++index)
		{
...
void CFacialJoystickDialog::OnAction(JoystickAction action)
{
	...
	switch (action)
	{
	case JOYSTICK_ACTION_START:
		event = EFD_EVENT_START_EDITTING_JOYSTICKS;
		break;
	case JOYSTICK_ACTION_END:
		event = EFD_EVENT_STOP_EDITTING_JOYSTICKS;
		break;
	}
	if (m_pContext)
...
void CFacialJoystickDialog::ReadDisplayedSnapMargin()
{
	...
	{
		i32 selection = pControl->GetCurSel();
