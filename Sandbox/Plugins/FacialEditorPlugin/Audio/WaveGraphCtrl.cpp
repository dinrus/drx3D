...
//////////////////////////////////////////////////////////////////////////
i32 CWaveGraphCtrl::AddWaveform()
{
	i32 index = m_waveforms.size();
...
//////////////////////////////////////////////////////////////////////////
void CWaveGraphCtrl::DeleteWaveform(i32 index)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CWaveGraphCtrl::GetWaveformCount()
{
	return m_waveforms.size();
}

//////////////////////////////////////////////////////////////////////////
void CWaveGraphCtrl::SetWaveformTime(i32 index, float time)
{
	m_waveforms[index].time = time;
}

//////////////////////////////////////////////////////////////////////////
void CWaveGraphCtrl::LoadWaveformSound(i32 index, const CString& soundFile)
{
	m_bScrubbing = false;
...
//////////////////////////////////////////////////////////////////////////
float CWaveGraphCtrl::GetWaveformLength(i32 waveformIndex)
{
	return 0.0f;
	/*if (waveformIndex < 0 || waveformIndex >= i32(m_waveforms.size()))
	   return 0.0f;
...
//////////////////////////////////////////////////////////////////////////
void CWaveGraphCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
//////////////////////////////////////////////////////////////////////////
void CWaveGraphCtrl::OnFacialEdEvent(EFacialEdEvent event, IFacialEffector* pEffector, i32 nChannelCount, IFacialAnimChannel** ppChannels)
{
	switch (event)
	{
	case EFD_EVENT_ADD:
...
//////////////////////////////////////////////////////////////////////////
i32 CWaveGraphCtrl::HitTestWaveforms(const CPoint point)
{
	float time = ClientToWorld(point).x;
	i32 hitWaveform = -1;
	/*for (i32 waveFormIndex = 0, waveFormCount = m_waveforms.size(); waveFormIndex < waveFormCount; ++waveFormIndex)
	   {
...
//////////////////////////////////////////////////////////////////////////
void CWaveGraphCtrl::SendNotifyMessage(i32 code)
{
...
{
	//i32 cx = m_rcWaveGraph.Width();
	//i32 cy = m_rcWaveGraph.Height();
	...
	//	i32 minX = WorldToClient( Vec2(startTime,0) ).x;
	...
	//	u32 nSamplesPerSec = pWaveFileReader->GetSamplesPerSec();
	//	u32 nSamplesPerPixel = pWaveFileReader->GetSamplesPerSec() / m_grid.zoom.x;
	...
	//	i32 nWavLengthPixels = fPixelsPerSec * fWavLengthSec;
	...
	//	i32 maxX = minX + nWavLengthPixels;

	//	bool bFirst = true;
	//	for (i32 x = max(minX,(i32)rcClip.left), right = min(maxX, (i32)rcClip.right); x < right; x++)
	//	{
	...
	//		i32 nSample = v.x*nSamplesPerSec;
...
//////////////////////////////////////////////////////////////////////////
void CWaveGraphCtrl::SetWaveformTextString(i32 waveformIndex, const CString& text)
{
	if (waveformIndex < 0 || waveformIndex >= i32(m_waveforms.size()))
		return;
...
void CWaveGraphCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		i32 waveformIndex = HitTestWaveforms(point);
		if (waveformIndex >= 0)
		{
	...
	case eWaveDragMode:
		{
			...
			ASSERT(m_nWaveformBeingDragged >= 0 && m_nWaveformBeingDragged < i32(m_waveforms.size()));
...
//////////////////////////////////////////////////////////////////////////
void CWaveGraphCtrl::SetLeftOffset(i32 nLeft)
{
	m_nLeftOffset = nLeft;
	Invalidate();
}
...
//////////////////////////////////////////////////////////////////////////
void CWaveGraphCtrl::SetBottomWnd(CWnd* pWnd, i32 nHeight)
{
	m_pBottomWnd = pWnd;
...
struct WaveformSortPredicate
{
	...
	bool operator()(i32 left, i32 right) { return this->waveforms[left].time < this->waveforms[right].time; }
...
void CWaveGraphCtrl::StartSoundsAtTime(float fTime, bool bForceStart)
{
	...
	//std::vector<i32> sortedWaveformIndices(m_waveforms.size());
	//for (i32 i = 0, count = sortedWaveformIndices.size(); i < count; ++i)
	...
	//std::vector<i32> waveformsThatShouldBePlaying;
	...
	//if (!overlapSounds)
	//{
	//	i32 latestEntryIndex = -1;
	//	float latestStartTime = -FLT_MAX;
	//	for (i32 entryIndex = 0, entryCount = waveformsThatShouldBePlaying.size(); entryIndex < entryCount; ++entryIndex)
	//	{
	...
	//std::vector<i32> waveformsThatArePlaying;
	...
	//std::vector<i32> waveformsToStop, waveformsToStart, waveformsToUpdate;
	...
	//{
	//	i32 playingPosition = 0, toPlayPosition = 0, playingCount = waveformsThatArePlaying.size(), toPlayCount = waveformsThatShouldBePlaying.size();
	...
	//for (i32 entryIndex = 0, entryCount = waveformsToStart.size(); entryIndex < entryCount; ++entryIndex)
	//{
	...
	//for (i32 entryIndex = 0, entryCount = waveformsToStop.size(); entryIndex < entryCount; ++entryIndex)
	//{
	...
	//if (bForceStart)
	//{
	//	for (i32 entryIndex = 0, entryCount = waveformsToUpdate.size(); entryIndex < entryCount; ++entryIndex)
	//	{
