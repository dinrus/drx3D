...
// Desc: Initializes the Riff chunk for use
//--------------------------------------------------------------------------------------
VOID RiffChunkMemory::Initialize(FOURCC fccChunkId, const RiffChunkMemory* pParentChunk, tukk pFileData, u32 nFileSize)
{
	m_fccChunkId = fccChunkId;
	m_pParentChunk = pParentChunk;
...
{
	// Seek to the offset
	tukk pLocalPtr = m_pFileData + m_dwDataOffset + lOffset;
...
//--------------------------------------------------------------------------------------
HRESULT WaveFileMemory::Init(tukk pFileData, u32 nFileSize)
{
...
HRESULT WaveFileMemory::GetFormat(WAVEFORMATEXTENSIBLE* pwfxFormat) const
{
	...
	tuk buffer = new char[dwValidSize];
	...
	i32 len = MIN(dwValidSize, sizeof(WAVEFORMATEXTENSIBLE));
