...
void FacialExpressionUtils::FacialExpressionLibraryOrphanFinder::FindOrphans()
{
	...
	{
		for (i32 subEffectorIndex = 0, subEffectorCount = (*it).pEffector->GetSubEffectorCount(); subEffectorIndex < subEffectorCount; ++subEffectorIndex)
		{
...
i32 FacialExpressionUtils::AddOrphansToGarbageFacialExpressionFolder(IFacialEffectorsLibrary* pLibrary)
{
	i32 orphansAdded = 0;
...
i32 FacialExpressionUtils::DeleteOrphans(IFacialEffectorsLibrary* pLibrary)
{
	i32 orphansDeleted = 0;
	for (;; )
	{
		FacialExpressionLibraryOrphanFinder orphanFinder(pLibrary);
		i32 deletedThisPass = 0;
		for (FacialExpressionLibraryOrphanFinder::Iterator itOrphan = orphanFinder.Begin(), endOrphan = orphanFinder.End(); itOrphan != endOrphan; ++itOrphan)
		{
...
bool FacialExpressionUtils::IsExpressionInGarbage(IFacialEffectorsLibrary* pLibrary, IFacialEffector* pEffector)
{
	...
	if (pGarbageFolder && pEffector)
	{
		for (i32 i = 0, end = pGarbageFolder->GetSubEffectorCount(); i < end; ++i)
