...
// c3d stuff
//////////////////////////////////////////////////////////////////////////
void RPF(i32* offset, tuk type, tuk dim, FILE* infile)
{
	...
	//		T = parameter type: -1 = char, 1 = boolean, 2 = i32, 3 = float
...
struct tMarker
{
	...
	i32   nPos;   // in vicon channels vector
	bool  bExport;
	bool  bExport3D;
	bool  b3D;
	bool  bUseForPlaneFitting;
	i32   nJoystickIndex;
	i32   nJoystickIndex3D;

	void  Set(tukk szMarker, tukk szJoystick, tukk szJoystick3D = NULL)
	{
		drx_strcpy(sMarker, szMarker);
		drx_strcpy(sJoystick, szJoystick);
		if (szJoystick3D)
		{
			drx_strcpy(sJoystick3D, szJoystick3D);
			b3D = true;
		}
		else
		{
...
//////////////////////////////////////////////////////////////////////////
void CFacialEditorDialog::LoadC3DFile(tukk filename)
{
	...
	if (key1 != 0x50)
	{
		CQuestionDialog::SCritical(QObject::tr(""), QObject::tr("Error - The file is not a c3d file"));
		fclose(infile);
		return;
	}
	...
	i32 nFrames = nFrameEnd - nFrameStart;
	...
	i32 nCount = pJoySet->GetJoystickCount();
	for (i32 k1 = 0; k1 < nCount; k1++)
	{
		IJoystick* pJoy = pJoySet->GetJoystick(k1);
		if (!pJoy)
			continue;
		tukk szJoy = pJoy->GetName();
		for (i32 k = 0; k < FA_NUM_MARKERS; k++)
		{
			if (m_lstMarkers[k].b3D)
			{
				if (stricmp(m_lstMarkers[k].sJoystick3D, szJoy) == 0)
				{
					m_lstMarkers[k].nJoystickIndex3D = k1;
				}
			}

			if (stricmp(m_lstMarkers[k].sJoystick, szJoy) == 0)
			{
				m_lstMarkers[k].nJoystickIndex = k1;
				break;
			}
		} //k
		if (stricmp(szJoy, "Head") == 0)
		{
			m_pJoystickHead = pJoy;
		}
		else if (stricmp(szJoy, "Lean") == 0)
		{
	...
	for (i32 k = 0; k < FA_NUM_MARKERS; k++)
	{
		...
		m_lstMarkers[k].fMinX = 1.0f;
		m_lstMarkers[k].fMaxX = -1.0f;
		m_lstMarkers[k].fMinY = 1.0f;
		m_lstMarkers[k].fMaxY = -1.0f;
	...
	for (i32 i = 0; i < nrow; i++)
	{
		...
		for (i32 k = 0; k < FA_NUM_MARKERS; k++)
		{
			if (stricmp(m_lstMarkers[k].sMarker, gname) == 0)
			{
	...
	/*
	   ...
	   i32k nFramesExport=2;
	   float lstFramesExport[nFramesExport]={0,4.0f};
	   for (i32 j2=0;j2<nFramesExport;j2++)
	   {
	    std::vector<Vec3> pts;

	    for (i32 k=0;k<nMarkers;k++)
	    {
	      bool bSkip=false;
	      for (i32 j=0;j<lstNamesSkip.size();j++)
	      {
	        if (stricmp(lstNames[k].c_str(),lstNamesSkip[j].c_str())==0)
	        {
	          bSkip=true;
	          break;
	        }
	      ...
	      // Determine data offset based upon starting channel number
	      i32 nFrame=(i32)((lstFramesExport[j2]*fFreq)+0.5f);
	      i32 offset = 8*2*(k)+(nFrame*8*2*nMarkers);
	...
	for (i32 k = 0; k < FA_NUM_MARKERS; k++)
	{
		...
		C += dotproduct_matrix(vPos, vPos, tmp);
		org += vPos;
		nPt++;
	}
	//nPt=FA_NUM_MARKERS;
	...
	for (i32 j = 0; j < nFrames; j++)
	{
		...
		for (i32 k = 0; k < FA_NUM_MARKERS; k++)
		{
			...
			// Determine data offset based upon starting channel number
			i32 offset = 8 * 2 * (m_lstMarkers[k].nPos) + (j * 8 * 2 * nMarkers);
		...
		for (i32 k = 0; k < FA_NUM_MARKERS; k++)
		{
			...
			for (IJoystick::ChannelType axis = IJoystick::ChannelType(0); axis < 2; axis = IJoystick::ChannelType(axis + 1))
			{
				...
				//if (axis==0 && (stricmp(pJoystick->GetName(),"center_lip_upper")==0) || (stricmp(pJoystick->GetName(),"left_lip_top")==0))
