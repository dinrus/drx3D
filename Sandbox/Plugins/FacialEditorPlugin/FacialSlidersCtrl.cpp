...
//////////////////////////////////////////////////////////////////////////
void CFacialSlidersCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
{
	/*
	   for (i32 i = 0; i < m_sliders.size(); i++)
	   {
...
//////////////////////////////////////////////////////////////////////////
void CFacialSlidersCtrl::CreateSlider(i32 nIndex, IFacialEffector* pEffector)
{
	CRect rc(0, 0, 0, 0);
	CString str;
	...
	lvi.stateMask = 0;
	lvi.state = 0;
	lvi.pszText = const_cast<tuk>((tukk)str);
	if (m_bShowExpressions)
		lvi.iImage = 1;
	else
		lvi.iImage = 2;
	i32 id = InsertItem(&lvi);
...
void CFacialSlidersCtrl::CreateSliders()
{
	...
	i32 nSliders = (i32)effectors.size();
...
void CFacialSlidersCtrl::RecalcLayout()
{
	...
	i32 nSliderWidth = GetColumnWidth(COLUMN_SLIDER);
	i32 nTextrWidth = GetColumnWidth(COLUMN_NAME);
	i32 nNewTextWidth = rcClient.Width() - nSliderWidth - 4 - 45 - 50;
...
void CFacialSlidersCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	{
		SSliderInfo& si = m_sliders[i];
		if ((uk )si.pSlider == (uk )pScrollBar)
		{
			OnSliderChanged(i);
...
//////////////////////////////////////////////////////////////////////////
void CFacialSlidersCtrl::UpdateSliderUI(i32 nSlider)
{
...
//////////////////////////////////////////////////////////////////////////
void CFacialSlidersCtrl::OnSliderChanged(i32 nSlider)
{
...
//////////////////////////////////////////////////////////////////////////
void CFacialSlidersCtrl::SetRowColor(i32 iRow, COLORREF crText, COLORREF crBack)
{
	...
	i32k nTopIndex = m_pListCtrl->GetTopIndex();
	i32k nBotIndex = nTopIndex + m_pListCtrl->GetCountPerPage();
...
void CFacialSlidersCtrl::SetMorphWeight(IFacialEffector* pEffector, float fWeight)
{
	...
	{
		if (stricmp((*itSlider).pEffector->GetName(), pEffector->GetName()) == 0)
		{
...
//////////////////////////////////////////////////////////////////////////
void CFacialSlidersCtrl::OnFacialEdEvent(EFacialEdEvent event, IFacialEffector* pEffector, i32 nChannelCount, IFacialAnimChannel** ppChannels)
{
	switch (event)
	{
	case EFD_EVENT_ADD:
...
void CFacialSlidersCtrl::UpdateValuesFromSelected()
{
	...
	if (m_pContext->pSelectedEffector && m_pContext->pSelectedEffector->GetType() == EFE_TYPE_EXPRESSION)
	{
		...
		for (i32 i = 0; i < m_sliders.size(); i++)
		{
			...
			si.bEnabled = false;
			si.pSlider->SetPos(0);
			for (i32 j = 0; j < m_pContext->pSelectedEffector->GetSubEffectorCount(); j++)
			{
				IFacialEffCtrl* pController = m_pContext->pSelectedEffector->GetSubEffCtrl(j);
				if (pController->GetType() != IFacialEffCtrl::CTRL_LINEAR)
					continue;
				if (stricmp(pController->GetEffector()->GetName(), si.pEffector->GetName()) == 0)
				{
