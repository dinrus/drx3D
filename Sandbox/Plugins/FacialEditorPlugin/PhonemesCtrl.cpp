...
//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CPhonemesCtrl::TimeToClient(i32 time)
{
	return floor((time - m_fOrigin) * (m_fZoom) + 0.5f) + m_rcPhonemes.left;
}

//////////////////////////////////////////////////////////////////////////
i32 CPhonemesCtrl::ClientToTime(i32 x)
{
...
void CPhonemesCtrl::DrawPhonemes(CDC* pDC)
{
	...
	for (i32 sentenceIndex = 0, sentenceCount = m_sentences.size(); sentenceIndex < sentenceCount; ++sentenceIndex)
	{
		for (i32 i = 0; i < (i32)m_sentences[sentenceIndex].phonemes.size(); i++)
		{
			Phoneme& ph = GetPhoneme(sentenceIndex, i);

			i32 x = TimeToClient(ph.time0 + m_sentences[sentenceIndex].startTime * 1000.0f);
			i32 x1 = TimeToClient(ph.time1 + m_sentences[sentenceIndex].startTime * 1000.0f);
			...
			if (ph.intensity < 0.99f)
			{
				...
				if (ph.bActive)
					col = RGB(200, 100, 30);
				CBrush brush(col);
				i32 h = ph.intensity * (m_rcPhonemes.bottom - m_rcPhonemes.top);
...
void CPhonemesCtrl::DrawWords(CDC* pDC)
{
	...
	for (i32 sentenceIndex = 0, sentenceCount = m_sentences.size(); sentenceIndex < sentenceCount; ++sentenceIndex)
	{
		for (i32 i = 0; i < (i32)m_sentences[sentenceIndex].words.size(); i++)
		{
			Word& w = GetWord(sentenceIndex, i);

			i32 x = TimeToClient(w.time0 + m_sentences[sentenceIndex].startTime * 1000.0f);
			i32 x1 = TimeToClient(w.time1 + m_sentences[sentenceIndex].startTime * 1000.0f);
...
void CPhonemesCtrl::DrawTimeMarker(CDC* pDC)
{
	...
	i32 x = TimeToClient(m_fTimeMarker);
...
//////////////////////////////////////////////////////////////////////////
static i32k nInsertPhonemeId = 1000;
static i32k nRenamePhonemeId = 2000;
void CPhonemesCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	std::pair<i32, i32> phonemeId = PhonemeFromTime(ClientToTime(point.x));
	...
	{
		tukk sPhoneme = GetPhoneme(phonemeId.first, phonemeId.second).sPhoneme;
...
void CPhonemesCtrl::OnMenuSelect(UINT nItemID, UINT nFlags, HMENU hSysMenu)
{
	if (m_phonemePopupMenus.find(hSysMenu) != m_phonemePopupMenus.end())
	{
		...
		tukk phonemeString = (foundPhonemeInfo && phonemeInfo.ASCII ? phonemeInfo.ASCII : "");
...
//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::AddWord(i32 sentenceIndex, const Word& w)
{
...
//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::AddPhonemes(CMenu& menu, i32 nBaseId)
{
	IPhonemeLibrary* pPhonemeLib = GetPhonemeLib();
	if (!pPhonemeLib)
		return;

	for (i32 i = 0; i < pPhonemeLib->GetPhonemeCount(); i++)
	{
		SPhonemeInfo phonemeInfo;
...
void CPhonemesCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	std::pair<i32, i32> overPhoneme(-1, -1);
...
{
	i32 time = ClientToTime(point.x);
	i32 time0 = ClientToTime(point.x - 2);
	i32 time1 = ClientToTime(point.x + 2);
	...
	{
		i32 i;
		i32 num = GetPhonemeCount(sentenceIndex);
		for (i = 0; i < num; i++)
		{
			Phoneme& ph = GetPhoneme(sentenceIndex, i);
...
//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::SetPhonemeTime(i32 sentenceIndex, i32 index, i32 t0, i32 t1)
{
...
void CPhonemesCtrl::TrackPoint(CPoint point)
{
	...
	i32 nSentenceIndex = m_nHitSentence;
	i32 nPhoneme = m_nHitPhoneme;

	if (nPhoneme >= 0 && nPhoneme < GetPhonemeCount(nSentenceIndex))
	{
		i32 time0 = GetPhoneme(nSentenceIndex, nPhoneme).time0 + m_sentences[nSentenceIndex].startTime * 1000.0f;
		i32 time1 = GetPhoneme(nSentenceIndex, nPhoneme).time1 + m_sentences[nSentenceIndex].startTime * 1000.0f;
		...
		{
			i32 dt = ClientToTime(point.x) - ClientToTime(m_LButtonDown.x);
...
//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::RemovePhoneme(i32 sentenceIndex, i32 phonemeId)
{
...
//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::AddPhoneme(i32 sentenceIndex, Phoneme& phoneme)
{
...
//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::InsertPhoneme(CPoint point, i32 phonemeId)
{
	SPhonemeInfo phonemeInfo;
	...
	i32 sentenceIndex = -1;
	for (i32 i = 0, sentenceCount = m_sentences.size(); i < sentenceCount; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::RenamePhoneme(i32 sentenceIndex, i32 index, i32 phonemeId)
{
	SPhonemeInfo phonemeInfo;
	if (!GetPhonemeLib()->GetPhonemeInfo(phonemeId, phonemeInfo))
		return;
...
void CPhonemesCtrl::ClearSelection()
{
	for (i32 sentenceIndex = 0, sentenceCount = m_sentences.size(); sentenceIndex < sentenceCount; ++sentenceIndex)
	{
		for (i32 i = 0; i < GetPhonemeCount(sentenceIndex); i++)
		{
			Phoneme& ph = GetPhoneme(sentenceIndex, i);
			ph.bSelected = false;
		}
...
void CPhonemesCtrl::SetTimeMarker(float fTime)
{
	...
	// Erase old first.
	i32 pt0 = TimeToClient(m_fTimeMarker);
	i32 pt1 = TimeToClient(fTime);
...
void CPhonemesCtrl::UpdateCurrentActivePhoneme()
{
	...
	std::pair<i32, i32> id = PhonemeFromTime(m_fTimeMarker);
...

	{
		...
		if (!bNoChangeToWord)
		{
			for (i32 sentenceIndex = 0, sentenceCount = m_sentences.size(); sentenceIndex < sentenceCount; ++sentenceIndex)
			{
				i32 num = GetWordCount(sentenceIndex);
				for (i32 i = 0; i < num; i++)
				{
					Word& w = GetWord(sentenceIndex, i);
					if (w.bActive)
...
//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::SendNotifyEvent(i32 nEvent)
{
	NMHDR nmh;
	nmh.hwndFrom = m_hWnd;
...
//////////////////////////////////////////////////////////////////////////
std::pair<i32, i32> CPhonemesCtrl::PhonemeFromTime(i32 time)
{
	i32 i;
	for (i32 sentenceIndex = 0, sentenceCount = m_sentences.size(); sentenceIndex < sentenceCount; ++sentenceIndex)
	{
...
//////////////////////////////////////////////////////////////////////////
std::pair<i32, i32> CPhonemesCtrl::WordFromTime(i32 time)
{
	i32 i;
	for (i32 sentenceIndex = 0, sentenceCount = m_sentences.size(); sentenceIndex < sentenceCount; ++sentenceIndex)
	{
...
//////////////////////////////////////////////////////////////////////////
tukk CPhonemesCtrl::GetMouseOverPhoneme()
{
...
//////////////////////////////////////////////////////////////////////////
i32 CPhonemesCtrl::AddSentence()
{
	i32 sentenceIndex = m_sentences.size();
	m_sentences.resize(m_sentences.size() + 1);
	return sentenceIndex;
}

//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::DeleteSentence(i32 sentenceIndex)
{
	m_sentences.erase(m_sentences.begin() + sentenceIndex);
}

//////////////////////////////////////////////////////////////////////////
i32 CPhonemesCtrl::GetSentenceCount()
{
	return m_sentences.size();
}

//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::SetSentenceStartTime(i32 sentenceIndex, float startTime)
{
	m_sentences[sentenceIndex].startTime = startTime;
}

//////////////////////////////////////////////////////////////////////////
void CPhonemesCtrl::SetSentenceEndTime(i32 sentenceIndex, float endTime)
{
