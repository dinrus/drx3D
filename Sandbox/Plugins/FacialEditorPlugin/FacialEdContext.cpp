...
public:
	...
	MergeCollisionAction AskConfirmation(tukk szFilename)
	{
...
void CFacialEdContext::SetCharacter(ICharacterInstance* pChar)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Character has no facial Instance. Probably because there are no Facial Expressions in the chrparams file"));
		return;
	}
	if (pCharacter)
		pCharacter->Release();
...
void CFacialEdContext::LoadCharacter(const CString& filename)
{
	...
	if (!pChar)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Failed to load character %s"), (tukk)filename);
		return;
	}
	SetCharacter(pChar);
}
...
//////////////////////////////////////////////////////////////////////////
void CFacialEdContext::SendEvent(EFacialEdEvent event, IFacialEffector* pEffector, i32 nNumChannels, IFacialAnimChannel** ppChannels)
{
	if (m_bSupressEvents)
		return;
	for (i32 i = 0; i < (i32)m_listeners.size(); i++)
	{
...
void CFacialEdContext::SelectChannel(IFacialAnimChannel* pChannel)
{
	...
	{
		assert(pChannel != reinterpret_cast<IFacialAnimChannel*>(static_cast<uintptr_t>(0xCCCCCCCC)));
		assert(pChannel != reinterpret_cast<IFacialAnimChannel*>(static_cast<uintptr_t>(0xCDCDCDCD)));
		assert(pChannel != reinterpret_cast<IFacialAnimChannel*>(static_cast<uintptr_t>(0xFEFEFEFE)));
...
{
	tukk joystickFile = (m_pJoysticks ? m_pJoysticks->GetName() : 0);
...
void CFacialEdContext::CopyExpressionNameToClipboard(IFacialEffector* pEffector)
{
	...
	tukk szEffectName(pEffector->GetName());
	if (!szEffectName)
	{
	...
	BOOL bCliboardOpen(::OpenClipboard(NULL));
	if (!bCliboardOpen)
	{
		::GlobalFree(hMem);
...
void CFacialEdContext::PasteExpressionFromClipboard()
{
	...
	if (pEffector)
	{
		if (pEffector == pSelectedEffector)
		{
			...
			CQuestionDialog::SWarning(QObject::tr(""), QObject::tr("You can't copy an item and paste it as its own sub-item."));
			return;
		}
...
void CFacialEdContext::HandleChannelDropToSequence(IFacialAnimChannel* pParentChannel, COleDataObject* pDataObject)
{
	...
	if (pExistingChannel)
	{
		if (QDialogButtonBox::StandardButton::No == CQuestionDialog::SQuestion(QObject::tr(""), QObject::tr("This folder contains a channel with the same name - do you want to overwrite it?")))
		{
			pChannel = 0;
		}
	}
...
IFacialAnimChannel* CFacialEdContext::GetChannelFromDescriptor(const string& descriptor)
{
	...
	for (i32 i = 0, end = m_pCurrentSequence->GetChannelCount(); i < end; ++i)
	{
...
string CFacialEdContext::GetChannelDescriptorFromDataSource(COleDataObject* dataObject)
{
	...
	if (hData != NULL)
	{
		channelDescriptor = static_cast<tuk>(GlobalLock(hData));
		GlobalUnlock(hData);
	}
...
string CFacialEdContext::GetDescriptorForChannel(IFacialAnimChannel* pChannel)
{
	...
	while (pAncestor)
	{
		tukk szChannelName = pAncestor ? pAncestor->GetName() : 0;
...
string CFacialEdContext::GetExpressionNameFromDataSource(COleDataObject* dataObject)
{
	...
	if (hData != NULL)
	{
		expressionName = static_cast<tuk>(GlobalLock(hData));
		GlobalUnlock(hData);
	}
...
void CFacialEdContext::SetPoseFromExpression(IFacialEffector* pEffector, float weight, float time)
{
	...
	PoseExpressionMap poseExpressionMap;
	{
		...
		i32 subEffectorCount = (pEffector ? pEffector->GetSubEffectorCount() : 0);
		poseExpressionEntries.reserve(subEffectorCount);
		for (i32 subEffectorIndex = 0; subEffectorIndex < subEffectorCount; ++subEffectorIndex)
		{
	...
	i32 channelCount = (m_pCurrentSequence ? m_pCurrentSequence->GetChannelCount() : 0);
	for (i32 channelIndex = 0; channelIndex < channelCount; ++channelIndex)
	{
		...
		{
			tukk effectorName = (pChannel ? pChannel->GetEffectorName() : 0);
...
void CFacialEdContext::UpdateSelectedFromSliders()
{
	...
	i32 nMaxEffectors = (i32)effectors.size();
	...
	if (pSelectedEffector && pSelectedEffector->GetType() == EFE_TYPE_EXPRESSION)
	{
		...
		for (i32 i = 0; i < m_weights.size(); i++)
		{
			if (i >= nMaxEffectors)
				break;
			...
			if (fabs(fWeight) > 0.0001f)
			{
				...
				if (pLibEffector)
				{
					...
					i32 nLast = pCurrent->GetSubEffectorCount() - 1;
	...
	if (pSelectedEffector && pSelectedEffector->GetType() == EFE_TYPE_EXPRESSION)
	{
		...
		for (i32 channelIndex = 0; channelIndex < channelCount; ++channelIndex)
		{
			...
			if (pChannel && (pChannel->GetFlags() & nonExpressionChannelMask) == 0)
			{
				...
				tukk channelName = pChannel->GetName();
				...
				if (fabs(channelWeight) > minimumWeight)
				{
					...
					if (pLibEffector && (stricmp(pCurrent->GetName(), pLibEffector->GetName()) != 0))
					{
						...
						i32 subEffectorCount = pLibEffector->GetSubEffectorCount();
						if (subEffectorCount != 0)
						{
							for (i32 subEffector = 0; subEffector < subEffectorCount; ++subEffector)
							{
								IFacialEffector* pSubEffector = pLibEffector->GetSubEffector(subEffector);
								tukk subEffName = pSubEffector->GetName();
...
{
	projectFile = PathUtil::ToUnixPath(filename.GetString()).c_str();
	...
	if (!root)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Failed to load facial project %s"), (tukk)projectFile);
		return false;
	}
	...
	{
		bLibraryModfied = false;
		tukk str = nodeLibrary->getAttr("File");
		SAFE_RELEASE(pLibrary);
		IFacialEffectorsLibrary* pLib = GetISystem()->GetIAnimationSystem()->GetIFacialAnimation()->LoadEffectorsLibrary(str);
		if (!pLib)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Failed to load expressions library %s"), str);
			return false;
		}
		SetLibrary(pLib);
	}
	...
	if (nodeCharacter)
	{
		tukk str = nodeCharacter->getAttr("File");
		ICharacterInstance* pChar = GetISystem()->GetIAnimationSystem()->CreateInstance(str);
		if (!pChar)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Failed to load character %s"), str);
			return false;
		}
		SetCharacter(pChar);
	}
	...
	if (nodeSequence)
	{
		tukk str = nodeSequence->getAttr("File");

		if (!str)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Failed to load Sequence"), str);
			return false;
		}
	...
	if (nodeJoysticks)
	{
		tukk str = nodeJoysticks->getAttr("File");

		if (!str)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Failed to load Joysticks"), str);
			return false;
		}
...
bool CFacialEdContext::SaveProject(const CString& filename)
{
	...
	if (XmlHelpers::SaveXmlNode(root, projectFile))
	{
		bProjectModfied = false;
		return true;
	}
...
{
	CString file = PathUtil::ToUnixPath(filename.GetString()).c_str();
	...
	if (!pLib)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Failed to load expressions library %s"), (tukk)file);
		return false;
	}
	SetLibrary(pLib);
...
bool CFacialEdContext::SaveLibrary(const CString& filename, IFacialEffectorsLibrary* pLibrary)
{
	...
	string file = PathUtil::ToUnixPath(filename.GetString());

	if (pLibrary)
	{
		pLibrary->SetName(file.c_str());
		XmlNodeRef root = XmlHelpers::CreateXmlNode("ExpressionLibrary");
		pLibrary->Serialize(root, false);
		if (XmlHelpers::SaveXmlNode(root, PathUtil::GamePathToDrxPakPath(file)))
		{
			bLibraryModfied = false;
			return true;
		}
...
bool CFacialEdContext::ImportSequence(const CString& filename, CWnd* parentWindow)
{
	...
	if (!pImportSequence)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, _T("Failed to load sequence %s"), (tukk)file);
		return false;
	}
...
bool CFacialEdContext::SaveSequence(const CString& filename, IFacialAnimSequence* pSequence)
{
	...
	if (pSequence)
	{
		...
		if (XmlHelpers::SaveXmlNode(root, PathUtil::GamePathToDrxPakPath(file.GetString())))
		{
			if (undirtySequence)
				bSequenceModfied = false;
			return true;
...
void CFacialEdContext::SetSkeletonAnimation(const CString& filename, float startTime)
{
	if (m_pCurrentSequence)
	{
		...
		i32 skeletonAnimationEntry = (m_pCurrentSequence ? m_pCurrentSequence->GetSkeletonAnimationEntryCount() : 0);
		...
		if (pEntry && pCharacter)
		{
			...
			u32 animID = pAnimSet->GetAnimIDByName(filename.GetString());
			float endTime =
...
{
	for (i32 i = 0; i < pRootEffector->GetSubEffectorCount(); i++)
	{
...
public:
	...
	void operator()(IFacialEffector* pEffector)
	{
		...
		{
			tukk effectorName = (pEffector ? pEffector->GetName() : 0);
...
void CFacialEdContext::CheckMorphs(IMorphCheckHandler* handler)
{
	MorphSet morphSet;
	{
		...
		for (i32 morphTargetIndex = 0; pFacialModel && morphTargetIndex < pFacialModel->GetMorphTargetCount(); ++morphTargetIndex)
...
template<typename H> void CFacialEdContext::ForEachEffectorRecurse(H& handler, IFacialEffector* pEffector, EffectorSet& visitedEffectors)
{
	...
	{
		handler(pEffector);
		for (i32 subEffectorIndex = 0; pEffector && subEffectorIndex < pEffector->GetSubEffectorCount(); ++subEffectorIndex)
...
void CFacialEdContext::OnSequenceUndo(SequenceChangeType changeType)
{
	switch (changeType)
	{
	case FE_SEQ_CHANGE_TOTAL:
		SendEvent(EFD_EVENT_SEQUENCE_UNDO, 0);
		break;
	case FE_SEQ_CHANGE_SOUND_TIMES:
		SendEvent(EFD_EVENT_SEQUENCE_TIMES_CHANGE, 0);
		break;
	}
}
...
void CFacialEdContext::MoveToKey(MoveToKeyDirection direction)
{
	...
	for (i32 channelIndex = 0, end = (m_pCurrentSequence ? m_pCurrentSequence->GetChannelCount() : 0); channelIndex < end; ++channelIndex)
	{
		IFacialAnimChannel* pChannel = (m_pCurrentSequence ? m_pCurrentSequence->GetChannel(channelIndex) : 0);
...
void CFacialEdContext::ConvertSequenceToCorrectFrameRate(IFacialAnimSequence* pSequence)
{
	for (i32 channelIndex = 0, channelCount = (pSequence ? pSequence->GetChannelCount() : 0); channelIndex < channelCount; ++channelIndex)
	{
		...
		for (i32 keyIndex = 0, keyCount = (pSpline ? pSpline->GetKeyCount() : 0); keyIndex < keyCount; ++keyIndex)
		{
		...
		{
			i32 oldKeyCount = (pSpline ? pSpline->GetKeyCount() : 0);
			...
			for (i32 frameIndex = 0, frameCount = keys.size(); frameIndex < frameCount; ++frameIndex)
			{
				if (pSpline)
...
void CFacialEdContext::UpdateSkeletonAnimationStatus()
{
	if (pCharacter)
	{
		...
		for (i32 skeletonAnimationIndex = 0, skeletonAnimationCount = (m_pCurrentSequence ? m_pCurrentSequence->GetSkeletonAnimationEntryCount() : 0); skeletonAnimationIndex < skeletonAnimationCount; ++skeletonAnimationIndex)
		{
...
i32 CFacialVideoFrameReader::AddAVI(tukk filename, float time)
{
	...
	if (itAVI == m_avis.end())
	{
		...
		if (!loadedFile && pAVI)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Facial Editor: Unable to load video file: \"%s\".", (filename ? filename : "<MISSING FILENAME>"));
...
void CFacialVideoFrameReader::DeleteAVI(i32 index)
{
	if (index < 0 || index >= i32(m_avis.size()))
	{
		DrxLogAlways("CFacialVideoFrameReader::DeleteAVI(): Attempting to delete an AVI entry that doesn't exist.");
	}
	else
	{
...
i32 CFacialVideoFrameReader::GetLastLoadedAVIFrameFromTime(float time)
{
	if (m_lastLoadedIndex == -1)
		return -1;

	IAVI_Reader* pCurrentAVI = (*m_aviEntities[m_lastLoadedIndex].itAVI).second.pAVI;
	i32 fps = (pCurrentAVI ? pCurrentAVI->GetFPS() : 1);
	i32 frameCount = (pCurrentAVI ? pCurrentAVI->GetFrameCount() : 0);
	i32 frameIndex = i32(floor((time - m_aviEntities[m_lastLoadedIndex].time) * fps));
	if (pCurrentAVI && frameIndex >= 0 && frameIndex < frameCount)
		return frameIndex;
	return -1;
}

i32 CFacialVideoFrameReader::GetAVICount() const
{
	return i32(m_aviEntities.size());
}

i32 CFacialVideoFrameReader::GetWidth(float time)
{
	IAVI_Reader* pAVI = 0;
	i32 frame = -1;
...
i32 CFacialVideoFrameReader::GetHeight(float time)
{
	IAVI_Reader* pAVI = 0;
	i32 frame = -1;
	FindAVIFrame(time, pAVI, frame);
...
u8k* CFacialVideoFrameReader::GetFrame(float time)
{
	IAVI_Reader* pAVI = 0;
	i32 frame = -1;
	FindAVIFrame(time, pAVI, frame);
...
void CFacialVideoFrameReader::FindAVIFrame(float time, IAVI_Reader*& pAVI, i32& frame)
{
	pAVI = 0;
	frame = -1;
	for (i32 entityIndex = 0, entityCount = (m_aviEntities.size()); entityIndex < entityCount; ++entityIndex)
	{
		IAVI_Reader* pCurrentAVI = (*m_aviEntities[entityIndex].itAVI).second.pAVI;
		i32 fps = (pCurrentAVI ? pCurrentAVI->GetFPS() : 1);
		i32 frameCount = (pCurrentAVI ? pCurrentAVI->GetFrameCount() : 0);
		i32 frameIndex = i32(floor((time - m_aviEntities[entityIndex].time) * fps));
		if (pCurrentAVI && frameIndex >= 0 && frameIndex < frameCount)
		{
			pAVI = pCurrentAVI;
			frame = frameIndex;
