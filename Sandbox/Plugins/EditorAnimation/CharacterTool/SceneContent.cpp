...
static void SerializeSphericalAngle(IArchive& ar, float& radians, float range, tukk name, tukk label)
{
...
static tukk MakeStringPersistent(tukk str)
{
...
void BlendShapeSkin::Serialize(IArchive& ar)
{
	...
	{
		tukk name = ar.isEdit() ? MakeStringPersistent(params[i].name.c_str()) : params[i].name.c_str();
...
void BlendShapeOptions::Serialize(IArchive& ar)
{
	...
	{
		tukk name = ar.isEdit() ? MakeStringPersistent(skins[i].name.c_str()) : skins[i].name.c_str();
...
SceneContent::SceneContent()
	...
	, lookLayer(-1)
{
	::DrxCreateClassInstance<IAnimEventPlayer>(AnimEventPlayer_CharacterTool::GetCID(), animEventPlayer);
	if (animEventPlayer)
...
static bool SerializedContentChanged(std::vector<char>* buffer, const Serialization::SStruct& obj, i32 filter)
{
