	...
	, m_outlineModeUsed(OUTLINE_ONE_TREE)
{
	...
	locationLayout->setSpacing(2);
	{
		...
		m_locationButton->setIcon(DrxIcon());
		...
		m_followActions[FOLLOW_SELECTION] = m_followMenu->addAction(DrxIcon("icons:common/animation_selection.ico"), "Follow Selection", this, SLOT(OnFollowMenu()));
		m_followActions[FOLLOW_SELECTION]->setData(i32(FOLLOW_SELECTION));
		if (m_system->sourceAssetList)
		{
			m_followActions[FOLLOW_SOURCE_ASSET] = m_followMenu->addAction(DrxIcon("icons:common/animation_source_asset.ico"), "Import Assets", this, SLOT(OnFollowMenu()));
			;
			m_followActions[FOLLOW_SOURCE_ASSET]->setData(i32(FOLLOW_SOURCE_ASSET));
		}
		m_followMenu->addSeparator();
		m_followActions[FOLLOW_LOCK] = m_followMenu->addAction(DrxIcon("icons:General/Lock_True.ico"), "Lock", this, SLOT(OnFollowMenu()));
		m_followActions[FOLLOW_LOCK]->setData(i32(FOLLOW_LOCK));

		m_followButton = new QToolButton();
		m_followButton->setAutoRaise(true);
		m_followButton->setIcon(DrxIcon("icons:common/animation_selection.ico"));
		...
		m_settingActions[OUTLINE_ONE_TREE]->setData(i32(OUTLINE_ONE_TREE));
		m_settingActions[OUTLINE_ON_TOP] = m_settingsMenu->addAction("Outline: Top", this, SLOT(OnSettingMenu()));
		m_settingActions[OUTLINE_ON_TOP]->setData(i32(OUTLINE_ON_TOP));
		m_settingActions[OUTLINE_ON_LEFT] = m_settingsMenu->addAction("Outline: Left", this, SLOT(OnSettingMenu()));
		m_settingActions[OUTLINE_ON_LEFT]->setData(i32(OUTLINE_ON_LEFT));

		m_settingsButton = new QToolButton();
		m_settingsButton->setAutoRaise(true);
		m_settingsButton->setIcon(DrxIcon("icons:General/Options.ico"));
...
void PropertiesPanel::SetOutlineMode(OutlineMode outlineMode)
{
	...
	for (i32 i = 0; i < NUM_OUTLINE_MODES; ++i)
	{
		QAction* a = m_settingActions[i];
		if (!a)
			continue;
		a->setCheckable(true);
		a->setChecked(i == i32(m_outlineMode));
	}
}
...
void PropertiesPanel::UpdateUndoButtons()
{
	...
	{
		tukk actionText = undoActions[i].c_str();
		QAction* action = m_undoMenu->addAction(QString::fromLocal8Bit(actionText), this, SLOT(OnUndoMenu()));
		action->setData(i32(i + 1));
	}
}

void PropertiesPanel::Undo(i32 count)
{
	ExplorerEntries entries;
	GetActiveEntries(&entries, m_location.entries, m_system);
	if (entries.empty())
...
void PropertiesPanel::UpdateLocationBar()
{
	...
	if (m_location.entries.size() > 1)
	{
		...
		for (i32 i = 0; i < m_location.entries.size(); ++i)
		...
		char buf[64];
		drx_sprintf(buf, "%s%d Items", (atLeastOneEntryModified ? "*" : ""), (i32)m_location.entries.size());
		text = buf;
	...
	m_locationButton->setIcon(DrxIcon(icon));
	...
	m_followButton->setIcon(DrxIcon(followIcon));
	for (i32 mode = 0; mode < NUM_FOLLOW_MODES; ++mode)
	{
...
void PropertiesPanel::UpdateToolbar()
{
	...
	m_toolbar->clear();
	m_actionUndo = m_toolbar->addAction(DrxIcon("icons:General/History_Undo.ico"), "Undo", this, SLOT(OnUndo()));
	...
	m_actionRedo = m_toolbar->addAction(DrxIcon("icons:General/History_Redo.ico"), "Redo", this, SLOT(OnRedo()));
	...
	if (!actions.empty())
	{
		...
		for (size_t i = 0; i < actions.size(); ++i)
		{
			...
			if (action.func)
			{
				...
				QAction* qaction = new QAction(DrxIcon(QString(action.icon)), action.text, this);
...
void PropertiesPanel::OnSubselectionChanged(i32 changedLayer)
{
	...
	const vector<ukk>& handles = m_system->characterGizmoManager->Subselection(layer);
