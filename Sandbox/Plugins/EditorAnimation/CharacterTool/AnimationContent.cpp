...
void AnimationContent::ApplyToCharacter(bool* triggerPreview, ICharacterInstance* characterInstance, tukk animationPath, bool startingAnimation)
{
	...
	if (type == ANIMATION && (importState == IMPORTED || importState == WAITING_FOR_CHRPARAMS_RELOAD))
	{
		...
		if (ar.isEdit() && ar.isOutput())
		{
			if (auto entryBase = ar.context<Explorer::EntryBase>())
			{
				...
				{
					i32 dbaIndex = dbaTable->FindDBAForAnimation(item);
					if (dbaIndex >= 0)
					{
