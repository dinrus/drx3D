	...
	, m_isCurBoneFree(true)
{
	...
	EXPECTED(connect(m_scene.get(), SIGNAL(SignalElementsChanged(u32)), this, SLOT(OnSceneElementsChanged(u32))));
	EXPECTED(connect(m_scene.get(), SIGNAL(SignalElementContinuousChange(u32)), this, SLOT(OnSceneElementContinuousChange(u32))));
...
void ModeCharacter::EnterMode(const SModeContext& context)
{
	...
	EXPECTED(connect(m_system->characterGizmoManager.get(), SIGNAL(SignalSubselectionChanged(i32)), SLOT(OnSubselectionChanged(i32))));
	...
	m_actionMoveTool.reset(new QAction(DrxIcon("icons:Navigation/Basics_Move.ico"), "Move", 0));
	...
	m_actionRotateTool.reset(new QAction(DrxIcon("icons:Navigation/Basics_Rotate.ico"), "Rotate", 0));
	...
	m_actionScaleTool.reset(new QAction(DrxIcon("icons:Navigation/Basics_Scale.ico"), "Scale", 0));
...
void ModeCharacter::LeaveMode()
{
	...
	EXPECTED(disconnect(m_system->characterGizmoManager.get(), SIGNAL(SignalSubselectionChanged(i32)), this, SLOT(OnSubselectionChanged(i32))));
...
void ModeCharacter::OnSubselectionChanged(i32 layer)
{
	const vector<ukk>& items = m_system->characterGizmoManager->Subselection(GizmoLayer(layer));
...
void ModeCharacter::OnDisplayOptionsChanged()
{
	...
	constexpr i32 animationFlag = (1 << GIZMO_LAYER_ANIMATION);
...
void ModeCharacter::OnSceneSelectionChanged()
{
	...
	vector<ukk> selection;
	GetPropertySelection(&selection);

	for (i32 layer = 0; layer < GIZMO_LAYER_COUNT; ++layer)
...
void ModeCharacter::HandleSceneChange(i32 layerBits, bool continuous)
{
	u32 bitsLeft = layerBits;
	for (i32 layer = 0; bitsLeft != 0; bitsLeft = bitsLeft >> 1, ++layer)
	{
...
void ModeCharacter::OnSceneElementsChanged(u32 layerBits)
{
	HandleSceneChange(layerBits, false);
}

void ModeCharacter::OnSceneElementContinuousChange(u32 layerBits)
{
...
void ModeCharacter::GetPropertySelection(vector<ukk>* selection) const
{
	selection->clear();
...
void ModeCharacter::SetPropertySelection(const vector<ukk>& items)
{
}
...
struct SFakeSizer : public IDrxSizer 
{
	...
	virtual bool   AddObject(ukk ptr, size_t size, i32 nCount=1) { m_ptrLast = ptr; return true; }

	ukk m_ptrLast = nullptr;
};
...
bool ModeCharacter::OnViewportMouseProxy(const SMouseEvent& ev)
{
	...
	auto RayCheckRMesh = [&](IRenderMesh* pRM, IAttachmentSkin* pSkin = nullptr)
	{
		...
		for(i32 i = pRM->GetIndicesCount()-3; i >= 0; i-=3)
		{
			Vec3 pthit;
			if (Intersect::Ray_Triangle(wray, vtx[idx[i]],vtx[idx[i+1]],vtx[idx[i+2]], pthit) && (pthit-wray.origin)*wray.direction < mindist)
			{
				...
				i32 imax3 = idxmax3(hitvtx.weights.bcolor);
	...
	for(i32 i = pAtman->GetAttachmentCount()-1; i >= 0; i--)
	...
	if (ibone0 > 0)
	{
			...
			for(uint i = 0; i < cdf->attachments.size(); i++)
				if (!_stricoll(name, cdf->attachments[i].m_strSocketName.c_str())) 
				{
					...
					if (tree->selectByAddress(&cdf->attachments[i], false))
					{
						...
						std::vector<ukk> handles; handles.push_back(&cdf->attachments[i]);
		...
		{
			m_document->GetDisplayOptions()->physics.selectedBone = ibone0;
			strided_pointer<DrxBonePhysics> bonePhys;
			bonePhys.data = skel.GetJointPhysInfo(0);
			bonePhys.iStride = (i32)((INT_PTR)skel.GetJointPhysInfo(1) - (INT_PTR)bonePhys.data);
			...
			std::function<void(i32,const phys_geometry*,const phys_geometry*)> IterateChildren = [&](i32 ibone, const phys_geometry *pCheckVal, const phys_geometry *pMarkVal)
			{
				if (bonePhys[ibone].pPhysGeom != pCheckVal)
					return;
				bonePhys[ibone].pPhysGeom = const_cast<phys_geometry*>(pMarkVal);
				for(i32 i = skel.GetJointChildrenCountByID(ibone)-1; i >= 0; i--)
			...
			auto PickVertices = [&](IRenderMesh *pRM, const phys_geometry *pMarkVal, IAttachmentSkin *pSkin = nullptr)
			{
				...
				strided_pointer<SVF_W4B_I4S> skinData;
				skinData.data = (SVF_W4B_I4S*)pRM->GetHWSkinPtr(skinData.iStride, FSL_READ);
				for(i32 i = pRM->GetVerticesCount()-1, w; i >= 0; i--)
				{
					for(i32 j = w = 0; j < 4; j++)
			...
			if (vtxBone.size())
			{
				...
				i32 trisHull = gEnv->pPhysicalWorld->GetPhysUtils()->qhull(vtxBone.data(), vtxBone.size(), idxHull);
				...
				i32k primFlags[] = { mesh_approx_box, mesh_approx_capsule, mesh_approx_sphere };
				...
				if (tree->selectByAddress(&cdf->attachments.end()[-1], false))
				{
					tree->expandRow(tree->selectedRow());
					tree->ensureVisible(tree->selectedRow());
					std::vector<ukk> handles; handles.push_back(&cdf->attachments.end()[-1]);
					m_system->characterGizmoManager->SetSubselection(GIZMO_LAYER_CHARACTER, handles);
				}
				return true;
			}
...
void ModeCharacter::CommenceRagdollTest()
{
	...
	for(i32 i = 0; (pents[1] = skel.GetCharacterPhysics(i)); i++)
	{
	...
	for(i32 i = 0; i < tess; i++, r.set(r.x * cosa - r.y *sina, r.x * sina + r.y * cosa))
	{
		vtx[i] = r;	
	...
	DrxGetExecutableFolder(MAX_PATH, path);
	...
	for(i32 i = 0; i < DRX_ARRAY_COUNT(pents); i++)
	...
	if (!CreateProcess(nullptr, path, nullptr, nullptr, FALSE, 0, nullptr, nullptr, &si, &pi))
	{
		...
		DrxMessageBox(buf, "Error", eMB_Error);
	}
#endif
}
