...
public:
	...
	void                 LoadCharacter(tukk path);
	tukk          LoadedCharacterFilename() const { return m_loadedCharacterFilename.c_str(); }
	...
	void                            SetSelectedExplorerEntries(const ExplorerEntries& entries, i32 selectOptions);
	...
	void          TriggerAnimationPreview(i32 previewFlags);
	...
	tukk   PlaybackBlockReason() const { return m_playbackBlockReason; }
	void          Play();
	void          Pause();
...
public slots:
	...
	void       OnCharacterSavedAs(tukk oldName, tukk newName);
	void       OnCharacterDeleted(tukk filename);
	void       OnAnimationForceRecompile(tukk path);
...
protected slots:
	...
	void       OnExplorerEntrySavedAs(tukk oldPath, tukk newPath);
	void       OnCompressionMachineAnimationStarted();
private:
	static i32 AnimationEventCallback(ICharacterInstance* instance, uk userData);
	...
	void       CreateShaderParamCallback(tukk name, tukk UIname, u32 iIndex);
