...
static bool IsAimAnimation(ICharacterInstance* pCharacterInstance, tukk animationName)
{
	...
	i32k localAnimationId = pAnimationSet->GetAnimIDByName(animationName);
	if (localAnimationId == -1)
	{
		return false;
	}

	u32k animationFlags = pAnimationSet->GetAnimationFlags(localAnimationId);
...
void AddAnimationToAnimationSet(ICharacterInstance& characterInstance, tukk animationName, tukk animationPath)
{
	if (!animationName)
		return;
	if (!animationPath)
...
static bool SetAnimationReferenceByName(SAnimationReference* ref, ICharacterInstance* character, tukk animationName)
{
	...
	i32k localAnimationId = animationSet->GetAnimIDByName(animationName);
...
void SAnimationReference::reset(u32 pathCRC, tukk animationName, bool bIsRegularCaf)
{
...
{
public:
	BackgroundTaskCompressPreview(CompressionMachine* pEditor, i32 sessionIndex, tukk animationPath, tukk outputAnimationPath, const SAnimSettings& animSettings, bool ignorePresets)
		: m_pEditor(pEditor)
	...
	tukk Description() const override  { return m_description.c_str(); }
	tukk ErrorMessage() const override { return m_errorMessage.c_str(); }
	...
	ETaskResult Work() override
	{
		...
		char buffer[IDrxPak::g_nMaxPath] = { 0 };
		string filename = gEnv->pDrxPak->AdjustFileName(m_createdFile.c_str(), buffer, 0);
		m_outputFileSize = gEnv->pDrxPak->GetFileSizeOnDisk(filename.c_str());
		m_compressedCafSize = gEnv->pDrxPak->FGetSize(m_animationPath.c_str(), true);
...
struct CompressionMachine::AnimationSetExtender : IAnimationSetListener
{
	...
	void AddAnimation(tukk name, tukk path)
	{
...
static void FormatReferenceName(string* name, string* path, i32 index)
{
	if (name)
...
static void FormatPreviewName(string* name, string* path, i32 index)
{
	if (name)
...
void CompressionMachine::StartPreview(bool forceRecompile, bool expectToReloadChrparams)
{
	...
	for (size_t i = 0; i < m_playbackLayers.layers.size(); ++i)
	{
		...
		FormatPreviewName(&animation.compressedAnimationName, &animation.compressedAnimationPath, i32(i));
		if (m_uncompressedCharacter)
			FormatReferenceName(&animation.uncompressedAnimationName, &animation.uncompressedAnimationPath, i32(i));
		animation.type = animation.UNKNOWN;

		tukk extension = PathUtil::GetExt(animation.path.c_str());
		...
		else if (stricmp(extension, "caf") == 0)
		{
			...
			animation.hasSourceFile = gEnv->pDrxPak->IsFileExist(iCafPath.c_str(), IDrxPak::eFileLocation_OnDisk);
...
static void PlayLayer(ICharacterInstance* character, const PlaybackLayer& layer, tukk animationName,
                      float normalizedTime, float playbackSpeed, bool loop)
{
	ISkeletonAnim& skeletonAnim = *character->GetISkeletonAnim();
	i32 localAnimID = character->GetIAnimationSet()->GetAnimIDByName(animationName);
	if (localAnimID < 0)
		return;
...
void CompressionMachine::SetLoop(bool loop)
{
	if (m_loop != loop)
	{
		...
		if (IsPlaying())
		{
			i32 layer = 0; // TODO add layers supports
...
tukk CompressionMachine::AnimationPathConsideringPreview(tukk inputCaf) const
{
	for (size_t i = 0; i < m_animations.size(); ++i)
