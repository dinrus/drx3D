...
#include <drx3D/CoreX/Platform/platform.h>  // just for PathUtil::!
#include "SkeletonParameters.h"
#include <DrxSystem/File/DrxFile.h> // just for PathUtil::!
...
struct Substring
{
	...
	bool operator==(tukk str) const { return strncmp(str, start, end - start) == 0; }
	bool Empty() const                     { return start == end; }
};

static Substring GetLevel(tukk* outP)
{
	tukk p = *outP;
	while (*p != '/' && *p != '\0')
		++p;
...
static bool MatchesPathWildcardNormalized(tukk path, tukk wildcard)
{
	std::vector<std::pair<tukk , tukk >> border;
	border.push_back(std::make_pair(path, wildcard));

	while (!border.empty())
	{
		tukk p = border.front().first;
		tukk w = border.front().second;
		...
		while (true)
		{
			...
			for (tukk c = ws.start; c != ws.end; ++c)
			{
				if (*c != '*')
				{
...
static bool MatchesPathWildcard(tukk path, tukk wildcard)
{
...
bool AnimationSetFilter::Matches(tukk cafPath) const
{
...
bool SkeletonParameters::LoadFromXMLFile(tukk filename, bool* dataLost)
{
...
static void AddAnimation(const XmlNodeRef& dest, tukk name, tukk path, tukk flags = 0)
{
