...
static void DrawPhysicalEntities(IPhysicsDebugRenderer* pRenderer, ICharacterInstance* pCharacter, i32 iDrawHelpers)
{
	if (pCharacter == 0)
		return;
	...
	{
		u32 numAttachments = pIAttachmentManager->GetAttachmentCount();
...
static void DrawJoint(IRenderAuxGeom* pAuxGeom, const Vec3& start, const Vec3& end, u8k opacity = 255)
{
...
static void DrawFrame(IRenderAuxGeom* pAuxGeom, const QuatT& location, const float scale = 0.02f, u8k opacity = 255)
{
...
static void DrawSkeleton(IRenderAuxGeom* pAuxGeom, IDefaultSkeleton* pDefaultSkeleton, ISkeletonPose* pPose, const QuatT& location, string filter, QuatT cameraFrame)
{
	...
	u32 jointCount = pDefaultSkeleton->GetJointCount();
	bool filtered = !filter.empty();

	for (u32 i = 0; i < jointCount; ++i)
	{
		u8k opacity = (filtered && strstri(pDefaultSkeleton->GetJointNameByID(i), filter) == 0) ? 20 : 255;

		i32k parentIndex = pDefaultSkeleton->GetJointParentIDByID(i);
...
static tukk GetExt(tukk path)
{
	tukk ext = strchr(path, '.');
	if (!ext)
		return path + strlen(path);
	return ext + 1;
}

static MotionParameters QueryMotionParametersFromCharacterInstance(const ICharacterInstance* character, u32k layer)
{
	assert(character);
...
void CharacterDocument::ConnectExternalSignals()
{
	...
	EXPECTED(connect(m_system->animationList.get(), SIGNAL(SignalForceRecompile(tukk)), this, SLOT(OnAnimationForceRecompile(tukk))));
}
...
void CharacterDocument::Physicalize()
{
	...
	if (pMaterial)
	{
		...
		i32 numIds = pMaterial->FillSurfaceTypeIds(surfaceTypesId);
...
void CharacterDocument::LoadCharacter(tukk filename)
{
	...
	*m_loadedAnimationSetFilter = AnimationSetFilter();
	...
	if (m_compressedCharacter)
	{
		...
		{
			::DrxCreateClassInstanceForInterface<IAnimationGroundAlignment>(cryiidof<IAnimationGroundAlignment>(), m_groundAlignment);
		}
		...
		AnimationSetFilter filter;
		{
			...
			if (stricmp(PathUtil::GetExt(filename), "cga") == 0)
			{
				...
				if (gEnv->pDrxPak->IsFileExist(chrparamsFilenameForCga.c_str()))
			...
			else if (CharacterDefinition* definition = GetLoadedCharacterDefinition())
			{
				...
				if (gEnv->pDrxPak->IsFileExist(chrparamsFilenameForSkeleton.c_str()))
...
void CharacterDocument::OnCharacterSavedAs(tukk oldName, tukk newName)
{
...
void CharacterDocument::OnCharacterDeleted(tukk path)
{
...
void CharacterDocument::OnExplorerEntrySavedAs(tukk oldPath, tukk newPath)
{
...
void CharacterDocument::OnExplorerEntryModified(ExplorerEntryModifyEvent& ev)
{
	...
	if (m_system->animationList->OwnsAssetEntry(ev.entry) && ev.entry == GetActiveAnimationEntry())
	{
		...
		tukk animationPathConsideringCompression = m_compressionMachine->AnimationPathConsideringPreview(animation->path.c_str());
...
void CharacterDocument::TriggerAnimationPreview(i32 previewFlags)
{
...
void CharacterDocument::SetSelectedExplorerEntries(const ExplorerEntries& entries, i32 selectOptions)
{
...
static void DrawAnimationStateText(IRenderer* renderer, IRenderAuxGeom* pAuxRenderer,  const vector<StateText>& lines)
{
	...
	for (size_t i = 0; i < lines.size(); ++i)
	{
		...
		u32 color = l.type == l.PROGRESS ? 0xffffff00 :
...
i32 CharacterDocument::AnimationEventCallback(ICharacterInstance* instance, uk userData)
{
...
static void SetDesiredMotionParameters(ISkeletonAnim& skeletonAnimation, i32 layer, const MotionParameters& params)
{
	for (i32 i = 0; i < eMotionParamID_COUNT; ++i)
	{
		EMotionParamID id = EMotionParamID(i);
		float value = params.values[i];

		u32 numAnimsLayer0 = skeletonAnimation.GetNumAnimsInFIFO(layer);
		for (u32 a = 0; a < numAnimsLayer0; ++a)
		{
			i32 animCount = skeletonAnimation.GetNumAnimsInFIFO(layer);
			for (i32 i = 0; i < animCount; ++i)
			{
				...
				if (!sampler)
					continue;
				u32 endOfCycle = anim.GetEndOfCycle();
				for (u32 d = 0; d < sampler->m_numDimensions; d++)
				{
					if (sampler->m_MotionParameterID[d] == id)
					{
						u32 locked = sampler->m_MotionParameterFlags[d] & CA_Dim_LockedParameter;
						u32 init = sampler->m_MotionParameterFlags[d] & CA_Dim_Initialized;
...
static void SetCharacterAnimationNormalizedTime(ICharacterInstance* character, float normalizedTime, i32 activeLayer = -1)
{
	if (!character)
		return;
	ISkeletonAnim& skeletonAnimation = *character->GetISkeletonAnim();
	for (i32 layerId = 0; layerId < ISkeletonAnim::LayerCount; ++layerId)
	{
...
void CharacterDocument::PreRender(const SRenderContext& context)
{
	...
	if (pInstanceBase && !featureTestOverridesUpdate)
	{
		...
		u32 flags = m_compressedCharacter->GetCharEditMode() & ~CA_DrawSocketLocation;
		...
		{
			bool gotSlope = false;
			for (i32 layer = 0; layer < m_system->scene->layers.layers.size(); ++layer)
			{
		...
		auto updatePoseBlender = [&](IAnimationPoseBlenderDir* dir, const AimParameters& aimParameters, i32 layer, i32 fadeoutAngle)
		{
			if (dir)
			{
		...
		i32k activeLayerId = m_system->scene->layers.activeLayer;
		u32k activeLayerAnimationCount = skeletonAnim.GetNumAnimsInFIFO(activeLayerId);
		...
		if (m_bPaused)
		{
			...
			{
				i32k& layerId = it.layerId;
				u32k animationCount = skeletonAnim.GetNumAnimsInFIFO(layerId);
		...
		*m_viewportState = context.viewport->GetState();
		...
		if (m_AverageFrameTime > 0)
		{
			...
			i32 iOutOfBounds = pVars->iOutOfBounds;
			...
			if (pCharBasePhys)
				pCharBasePhys->SetParams(&pf);
			for (i32 i = 0; skeletonPose.GetCharacterPhysics(i); i++)
...
void CharacterDocument::DrawCharacter(ICharacterInstance* pInstanceBase, const SRenderContext& context)
{
	...
	if (m_displayOptions->skeleton.showJointNames)
	{
		ISkeletonPose& skeletonPose = *pInstanceBase->GetISkeletonPose();
		IDefaultSkeleton& defaultSkeleton = pInstanceBase->GetIDefaultSkeleton();
		u32 numJoints = defaultSkeleton.GetJointCount();
		bool filtered = !m_displayOptions->skeleton.jointFilter.empty();


		for (i32 j = 0; j < numJoints; j++)
		{
			tukk pJointName = defaultSkeleton.GetJointNameByID(j);
	...
	if (m_displayOptions->animation.showDccToolOrigin)
	{
		...
		i32k animCount = skeletonAnim.GetNumAnimsInFIFO(0);
		if (animCount > 0)
		{
	...
	u32 flags = pInstanceBase->GetCharEditMode() & ~(CA_BindPose | CA_AllowRedirection);
...
void CharacterDocument::CreateShaderParamCallbacks()
{
	...
	u32 iSize = _countof(g_shaderParamCallbacks);
...
void CharacterDocument::CreateShaderParamCallback(tukk name, tukk UIname, u32 iIndex)
{
#if 0
	if (DrxCreateClassInstance(name, m_pShaderParamCallbackArray[iIndex]))
	{
		...
		if (m_pShaderParamCallbackArray[iIndex]->Init())
		{
			...
			if (UIname != NULL)
			{
				...
				pVariable->Set(true);
				pVariable->SetUserData((uk ) &m_pShaderParamCallbackArray[iIndex]);
...
static void AllowAnimEventsToTriggerAgain(ICharacterInstance* characterInstance)
{
	...
	for (i32 layerIndex = 0; layerIndex < ISkeletonAnim::LayerCount; ++layerIndex)
	{
		i32k animCount = skeletonAnim.GetNumAnimsInFIFO(layerIndex);
...
void CharacterDocument::EnableMotionParameters()
{
	...
	i32k numLayers = m_system->scene->layers.layers.size();
	for (i32 layer = 0; layer < numLayers; ++layer)
	{
...
void CharacterDocument::UpdateBlendShapeParameterList()
{
	...
	i32 count = attachmentManager->GetAttachmentCount();
	for (i32 i = 0; i < count; ++i)
	{
		...
		if (!vertexFrames)
			continue;
		i32 numBlendShapes = vertexFrames->GetCount();
		if (numBlendShapes)
		{
			BlendShapeSkin skin;
			...
			for (i32 j = 0; j < numBlendShapes; ++j)
			{
...
void CharacterDocument::OnAnimationForceRecompile(tukk path)
{
	ExplorerEntries selectedEntries;
	GetSelectedExplorerEntries(&selectedEntries);
