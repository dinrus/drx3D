...
#include "../Drx3DEngine/CGF/CGFLoader.h"
...
struct UpdateAnimationSizesTask : public IBackgroundTask
{
	...
	ETaskResult Work() override
	{
		...
		{
			tukk animationPath = m_animationPaths[i].c_str();
			...
			u32 newSize = 0;
			if (FILE* f = gEnv->pDrxPak->FOpen(animationPath, "rb"))
			{
				m_newSizes[i] = gEnv->pDrxPak->FGetSize(f);
	...
	UpdateAnimationSizesTask(const vector<ExplorerEntryId>& entries, const vector<string>& animationPaths, i32 column, System* system, i32 subtree)
		: m_system(system)
		, m_entries(entries)
		, m_column(column)
	...
	UpdateAnimationSizesTask(const ExplorerEntryId& entryId, const string& animationPath, i32 column, System* system, i32 subtree)
		: m_system(system)
		, m_entries(1, entryId)
		, m_column(column)
...
static vector<string> LoadJointNames(tukk skeletonPath)
{
...
static bool LoadAnimEvents(ActionOutput* output, AnimEvents* animEvents, tukk animEventsFilename, tukk animationPath)
{
	animEvents->clear();
	...
	{
		i32 nodeCount = root->getChildCount();
	...
	if (animationNode)
	{
		for (i32 i = 0; i < animationNode->getChildCount(); ++i)
		{
...
static bool LoadAnimationEntry(SEntry<AnimationContent>* entry, SkeletonList* skeletonList, IAnimationSet* animationSet, const string& defaultSkeleton, tukk animEventsFilename)
{
	...
	else
	{
		...
		if (!gEnv->pDrxPak->IsFileExist(animationSettingsFilename.c_str()))
		{
...
static bool IsInternalAnimationName(tukk name, tukk animationPath)
{
	if (strncmp(name, "InternalPara", 12) == 0)
		return true;
	tukk editorPrefix = "animations/editor/";
	if (animationPath && _strnicmp(animationPath, editorPrefix, strlen(editorPrefix)) == 0)
		return true;
	return false;
}

static bool AnimationHasAudioEvents(tukk animationPath)
{
	...
	{
		i32 count = animEventList->GetCount();
...
static i32 GetAnimationPakState(tukk animationPath)
{
	i32 result = 0;
	tukk ext = PathUtil::GetExt(animationPath);
	if (stricmp(ext, "caf") == 0)
	{
		static tukk extensions[] = { "i_caf", "caf", "animsettings" };

		i32 numExtensions = sizeof extensions / sizeof extensions[0];
...
void AnimationList::Populate(ICharacterInstance* character, tukk defaultSkeletonAlias, const AnimationSetFilter& filter, tukk animEventsFilename)
{
	if (!character)
		return;
...
void AnimationList::SetExplorerData(ExplorerData* explorerData, i32 subtree)
{
	m_subtree = subtree;
	...
	i32 audioValueCount = sizeof(audioValues) / sizeof(audioValues[0]);
...
void AnimationList::ReloadAnimationList()
{
	...
	std::vector<std::pair<ExplorerEntryId, u32>> audioColumnValues;
	std::vector<std::pair<ExplorerEntryId, i32>> pakColumnValues;
	std::vector<std::pair<ExplorerEntryId, i32>> framesColumnValues;
	...
	i32 numAnimations = m_animationSet ? m_animationSet->GetAnimationCount() : 0;
	for (i32 i = 0; i < numAnimations; ++i)
	{
		tukk name = m_animationSet->GetNameByAnimID(i);
		tukk animationPath = m_animationSet->GetFilePathByID(i);
		if (IsInternalAnimationName(name, animationPath))
			continue;

		AnimationContent::Type type = AnimationContent::ANIMATION;
		i32 flags = m_animationSet->GetAnimationFlags(i);
		...
		i32 pakState = GetAnimationPakState(animationPath);
		pakColumnValues.push_back(std::make_pair(entryId, pakState));

		float durationSeconds = m_animationSet->GetDuration_sec(i);
		u32k animationSamplingFrequencyHz = 30;
		i32 frameCount = 1 + u32(durationSeconds * animationSamplingFrequencyHz + 0.5f);
...
void AnimationList::ScanForImportEntries(std::vector<std::pair<ExplorerEntryId, i32>>* pakColumnValues, bool resetFollows)
{
	typedef std::set<string, stl::less_stricmp<string>> UnusedAnimations;
	UnusedAnimations unusedAnimations;
	vector<u32> idsToRemove;
	...
	for (size_t i = 0; i < m_importEntries.size(); ++i)
	{
		...
		if (m_filter.Matches(animationPath.c_str()))
		{
			...
			if (isAdded)
			{
				...
				if (gEnv->pDrxPak->IsFileExist(animSettingsPath.c_str(), IDrxPak::eFileLocation_OnDisk) && m_system->animationCompressionManager)
				{
			...
			i32 pakState = GetAnimationPakState(animationPath.c_str());
			if (pakColumnValues)
	...
	{
		if (u32 id = m_animations.RemoveByPath(it->c_str()))
...
bool        AnimationList::LoadOrGetChangedEntry(u32 id)
{
...
SEntry<AnimationContent>* AnimationList::GetEntry(u32 id) const
{
	return m_animations.GetById(id);
}

bool AnimationList::IsNewAnimation(u32 id) const
{
	if (SEntry<AnimationContent>* entry = m_animations.GetById(id))
	{
...
SEntry<AnimationContent>* AnimationList::FindEntryByPath(tukk animationPath)
{
	return m_animations.GetByPath(animationPath);
}

u32 AnimationList::FindIdByAlias(tukk animationName)
{
...
bool AnimationList::ResaveAnimSettings(tukk filePath)
{
...

	{
		...
		tukk realPath = gEnv->pDrxPak->AdjustFileName(filePath, buffer, 0);
...
void AnimationList::CheckIfModified(u32 id, tukk reason, bool continuousChange)
{
	if (SEntry<AnimationContent>* entry = m_animations.GetById(id))
	{
		EntryModifiedEvent ev;
...
void AnimationList::OnAnimationSetAddAnimation(tukk animationPath, tukk name)
{
	...
	if (IsInternalAnimationName(name, animationPath))
		return;
	AnimationContent::Type type = AnimationContent::ANIMATION;
	i32 id = m_animationSet->GetAnimIDByName(name);
	if (id < 0)
		return;
	i32 flags = m_animationSet->GetAnimationFlags(id);
	if (flags & CA_ASSET_LMG)
	{
...
bool PatchAnimEvents(ActionOutput* output, tukk animEventsFilename, tukk animationPath, const AnimEvents& events)
{
	XmlNodeRef root = GetIEditor()->GetSystem()->LoadXmlFromFile(animEventsFilename);
	if (!root)
	{
	...
	if (needToSave)
	{
		char realPath[IDrxPak::g_nMaxPath];
		gEnv->pDrxPak->AdjustFileName(animEventsFilename, realPath, IDrxPak::FLAGS_FOR_WRITING);
		{
			string path;
			string filename;
...
static void CreateFolderForFile(tukk gameFilename)
{
	char buf[IDrxPak::g_nMaxPath];
	gEnv->pDrxPak->AdjustFileName(gameFilename, buf, IDrxPak::FLAGS_FOR_WRITING);
	QDir().mkpath(QString::fromLocal8Bit(PathUtil::ToUnixPath(PathUtil::GetParentDirectory(buf)).c_str()));
}

bool AnimationList::SaveAnimationEntry(ActionOutput* output, u32 id, bool notifyOfChange)
{
	if (SEntry<AnimationContent>* entry = m_animations.GetById(id))
	{
		if (!entry)
		...
		else
		{
			...
			if (entry->content.type == AnimationContent::BLEND_SPACE)
			{
				...
				gEnv->pDrxPak->AdjustFileName(entry->path.c_str(), path, 0);
		...
		i32 pakState = GetAnimationPakState(entry->path.c_str());
...
bool AnimationList::SaveEntry(ActionOutput* output, u32 id)
{
	return SaveAnimationEntry(output, id, true);
}

string AnimationList::GetSaveFilename(u32 id)
{
...
void AnimationList::RevertEntry(u32 id)
{
	if (SEntry<AnimationContent>* entry = m_animations.GetById(id))
	{
		entry->dataLostDuringLoading = !LoadAnimationEntry(entry, m_system->compressionSkeletonList, m_animationSet, m_defaultSkeletonAlias, m_animEventsFilename.c_str());
...
void AnimationList::UpdateAnimationEntryByPath(tukk filename)
{
...
static string StripExtension(tukk animationName)
{
	tukk dot = strrchr(animationName, '.');
	if (dot == 0)
		return animationName;
	return string(animationName, dot);
}

bool AnimationList::ImportAnimation(string* errorMessage, u32 id)
{
...
bool AnimationList::IsLoaded(u32 id) const
{
	SEntry<AnimationContent>* entry = GetEntry(id);
	if (!entry)
		return false;
...
void AnimationList::OnFileChange(tukk filename, EChangeType eType)
{
	const string originalExt = PathUtil::GetExt(filename);
	string animationPath;
	tukk pakStateFilename = "";
	    ...
	    originalExt == "animsettings")
	{
		...
		bool intermediateExists = gEnv->pDrxPak->IsFileExist(intermediatePath.c_str(), IDrxPak::eFileLocation_OnDisk);

		animationPath = PathUtil::ReplaceExtension(intermediatePath, "caf");
		bool animationExists = gEnv->pDrxPak->IsFileExist(animationPath.c_str());

		string animSettingsPath = PathUtil::ReplaceExtension(intermediatePath, "animsettings");
		bool animSettingsExists = gEnv->pDrxPak->IsFileExist(animSettingsPath.c_str(), IDrxPak::eFileLocation_OnDisk);
	...
	{
		bool fileExists = gEnv->pDrxPak->IsFileExist(filename, IDrxPak::eFileLocation_Any);
	...
	{
		i32 pakState = GetAnimationPakState(pakStateFilename);
		m_system->explorerData->SetEntryColumn(ExplorerEntryId(m_subtree, entry->id), m_explorerColumnPak, pakState, true);
		m_system->explorerData->SetEntryColumn(ExplorerEntryId(m_subtree, entry->id), m_explorerColumnNew, IsNewAnimation(entry->id), true);
	}
}

bool AnimationList::GetEntrySerializer(Serialization::SStruct* out, u32 id) const
{
...
static tukk EntryIconByContentType(AnimationContent::Type type, bool isAdditive)
{
	if (isAdditive)
...
static tukk GetContentTypeName(AnimationContent::Type type)
{
	switch (type)
	{
	case AnimationContent::BLEND_SPACE:
...
static void CollectExamplePaths(vector<string>* paths, AnimationList* animationList, SEntry<AnimationContent>* entry)
{
	if (entry->content.type == AnimationContent::BLEND_SPACE)
	{
		for (size_t i = 0; i < entry->content.blendSpace.m_examples.size(); ++i)
		{
			...
			u32 id = animationList->FindIdByAlias(example.animation.c_str());
			if (!id)
				continue;
...
void AnimationList::GetEntryActions(vector<ExplorerAction>* actions, u32 id, ExplorerData* explorerData)
{
...
i32 AnimationList::GetEntryCount() const
{
	return m_animations.Count();
}

u32 AnimationList::GetEntryIdByIndex(i32 index) const
{
...
void AnimationList::ActionExportHTR(ActionContext& x)
{
	...
	for (size_t i = 0; i < x.entries.size(); ++i)
	{
		...
		tukk animationPath = entry->path.c_str();
		tukk animationName = entry->name.c_str();
		...
		i32 animationId = animationSet->GetAnimIDByName(animationName);
		if (animationId < 0)
		{
...
dll_string AnimationAliasSelector(const SResourceSelectorContext& x, tukk previousValue, ICharacterInstance* character)
{
	if (!character)
	...
	i32 count = animationSet->GetAnimationCount();
	for (i32 i = 0; i < count; ++i)
	{
		tukk name = animationSet->GetNameByAnimID(i);
		tukk animationPath = animationSet->GetFilePathByID(i);
		if (IsInternalAnimationName(name, animationPath))
			continue;

		AnimationContent::Type type = AnimationContent::ANIMATION;

		i32 flags = animationSet->GetAnimationFlags(i);
		if (flags & CA_ASSET_LMG)
		{
		...
		tukk icon = EntryIconByContentType(type, isAdditive);
		tukk typeName = GetContentTypeName(type);

		dialog.AddRow(name, DrxIcon(icon));
