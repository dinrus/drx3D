...
static tukk ANIMATION_COMPRESSION_TEMP_ROOT = "Editor/Tmp/AnimationCompression/";

struct RCLogger : IResourceCompilerListener
{
	virtual void OnRCMessage(MessageSeverity severity, tukk text)
	{
...
bool CafCompressionHelper::CompressAnimation(const string& animationPath, string& outErrorMessage, bool* failReported)
{
	...
	if (gEnv->pDrxPak->IsFileExist(inputFilePath, IDrxPak::eFileLocation_OnDisk) == false)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CafCompressionHelper::CompressAnimationForPreview(string* outputCafPath, string* outErrorMessage, const string& animationPath, const SAnimSettings& animSettings, bool ignorePresets, i32 sessionIndex)
{
	if (!outErrorMessage)
		return false;
...
static bool GetFileTime(__time64_t* filetime, tukk filename)
{
	_finddata_t FindFileData;
	const intptr_t hFind = gEnv->pDrxPak->FindFirst(filename, &FindFileData, 0);

	if (hFind < 0)
		return false;

	gEnv->pDrxPak->FindClose(hFind);
...
static bool ModificationTimeEqual(tukk filename1, tukk filename2)
{
	__time64_t time1;
...
static string AnimationNameToPath(tukk animationName)
{
	return string(PathUtil::GetGameFolder()) + "/" + animationName;
}

bool CafCompressionHelper::CheckIfNeedUpdate(tukk animationName)
{
	string intermediatePath = PathUtil::ReplaceExtension(animationName, "i_caf");
	if (!gEnv->pDrxPak->IsFileExist(intermediatePath, IDrxPak::eFileLocation_OnDisk))
		return false;
	...
	if (!gEnv->pDrxPak->IsFileExist(settingsPath, IDrxPak::eFileLocation_OnDisk))
		return false;
	if (!gEnv->pDrxPak->IsFileExist(settingsDollarPath, IDrxPak::eFileLocation_OnDisk))
		return true;

	string animationPath = PathUtil::ReplaceExtension(intermediatePath, "caf");
	if (!gEnv->pDrxPak->IsFileExist(animationPath, IDrxPak::eFileLocation_OnDisk))
		return true;
...
bool CafCompressionHelper::MoveCompressionResult(string* outErrorMessage, tukk createdFile, tukk destination)
{
	if (createdFile[0] == '\0')
		return false;
...
void CafCompressionHelper::CleanUpCompressionResult(tukk createdFile)
{
