...
const AnimEventPreset* AnimEventPresetPanel::GetPresetByHotkey(i32 number) const
{
...
static void UpdateEventContentToColorMap(EventContentToColorMap* contentMap, const AnimEventPresets& presets)
{
	...
	for (size_t i = 0; i < presets.size(); ++i)
	{
		...
		QColor color = PresetColor(preset.colorHue);
		contentMap->push_back(std::make_pair(CCrc32::Compute((uk )serializedEvent.data(), serializedEvent.size()), ColorB(color.red(), color.green(), color.blue(), 255)));
	}
