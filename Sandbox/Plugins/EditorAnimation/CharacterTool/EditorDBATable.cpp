...
{
	string dbaTablePath = string(gEnv->pDrxPak->GetGameFolder()) + "\\Animations\\DBATable.json";
...
void EditorDBATable::UpdateMatchingAnimations()
{
	...
	{
		i32 dbaIndex = FindDBAForAnimation(filterItems[i]);
		if (dbaIndex == -1)
			continue;
...
i32 EditorDBATable::FindDBAForAnimation(const SAnimationFilterItem& animation) const
{
	for (size_t i = 0; i < m_entries.size(); ++i)
	{
...
const SEditorDBAEntry* EditorDBATable::GetEntryByIndex(i32 index) const
{
