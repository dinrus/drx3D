...
public:
	...
	bool CanSelect(tukk type, tukk path, i32 index) const override
	{
	...
	IExplorerEntryProvider* ReferenceTypeToProvider(tukk type) const
	{
	...
	static CExtensionFilter GetExtensionFilterForType(tukk type)
	{
	...
	static std::vector<string> GetAssetTypesForType(tukk type)
	{
	...
	tukk GetFileSelectorMaskForType(tukk type) const override { return GetMaskForType(type); }

	bool IsActive(tukk type, tukk path, i32 index) const override
	{
	...
	bool IsSelected(tukk type, tukk path, i32 index) const override
	{
	...
	bool IsModified(tukk type, tukk path, i32 index) const override
	{
	...
	bool Select(tukk type, tukk path, i32 index) const override
	{
		bool selected = false;
		if (strcmp(type, "Animation") == 0 || strcmp(type, "AnimationOrBSpace") == 0)
		{
	...
	bool CanPickFile(tukk type, i32 index) const override
	{
	...
	bool CanCreate(tukk type, i32 index) const override
	{
		return strcmp(type, "CharacterPhysics") == 0 || strcmp(type, "CharacterRig") == 0;
	}

	bool Create(tukk type, tukk path, i32 index) const override
	{
...
dll_string FileSelector(const SResourceSelectorContext& x, tukk previousValue)
{
...
static dll_string AnimationResourceSelector(const SResourceSelectorContext& x, tukk previousValue)
{
