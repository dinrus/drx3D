...
#include <drx3D/CoreX/Serialization/DrxExtensionImpl.h>
#include <drx3D/CoreX/Serialization/IArchiveHost.h>
#include <drx3D/CoreX/Serialization/DrxName.h>
...
// This function is a copy-paste from the exact same function in DrxAnimation.
...
struct ProxySet
{
	DynArray<CDrxName>&  names;
	CharacterDefinition* definition;
	ProxySet(DynArray<CDrxName>& names, CharacterDefinition* definition)
		: names(names)
		, definition(definition)
	{}
};

bool Serialize(Serialization::IArchive& ar, ProxySet& set, tukk name, tukk label)
{
	if (ar.isEdit() && set.definition)
	{
		if (ar.openBlock(name, label))
		{
			DynArray<CDrxName> allProxyNames;
			u32 numAttachments = set.definition->attachments.size();
			...
			for (u32 i = 0; i < numAttachments; i++)
			{
				...
				tukk strSocketName = att.m_strSocketName.c_str();
				if (att.m_attachmentType == CA_PROX)
					allProxyNames.push_back(CDrxName(strSocketName));
			}

			u32 numProxyNames = allProxyNames.size();
			DynArray<bool> arrUsedProxies(numProxyNames, false);

			arrUsedProxies.resize(numProxyNames);
			u32 numNames = set.names.size();
			for (u32 crcIndex = 0; crcIndex < numNames; crcIndex++)
			{
				u32 nCRC32lower = CCrc32::ComputeLowercase(set.names[crcIndex].c_str());
				for (u32 proxyIndex = 0; proxyIndex < numProxyNames; ++proxyIndex)
				{
					u32 nCRC32 = CCrc32::ComputeLowercase(allProxyNames[proxyIndex].c_str());
			...
			for (u32 proxyIndex = 0; proxyIndex < numProxyNames; ++proxyIndex)
			{
				tukk proxyName = allProxyNames[proxyIndex].c_str();
			...
			{
				i32 index = -1;
				for (i32 j = 0; j < set.names.size(); ++j)
	...
	void Serialize(Serialization::IArchive& ar)
	{
		...
		if (ct == SimulationParams::PENDULUM_CONE || ct == SimulationParams::PENDULUM_HINGE_PLANE || ct == SimulationParams::PENDULUM_HALF_CONE)
		{
			...
			if (!inJoint || p.m_useRedirect || hasGeometry)
			{
				...
				ar(Serialization::Range(p.m_nSimFPS, u8(10), u8(255)), "simulationFPS", "Simulation FPS");
		...
		if (ct == SimulationParams::TRANSLATIONAL_PROJECTION)
		{
			...
			if (p.m_nProjectionType)
			{
				...
				else
				{
					...
					u32 hasJointName = p.m_strDirTransJoint.length();
					if (hasJointName == 0)
			...
			if (inJoint)
			{
				...
				if (inJoint && pICharacterInstance)
				{
					...
					u32 numFunctions = pIAttachmentManager->GetProcFunctionCount();
					for (u32 i = 0; i < numFunctions; i++)
					{
						tukk pFunctionName = pIAttachmentManager->GetProcFunctionName(i);
						if (pFunctionName)
...
bool Serialize(Serialization::IArchive& ar, SimulationParams& params, tukk name, tukk label)
{
...
void SJointPhysics::Serialize(Serialization::IArchive& ar)
{
	...
	for (i32 setIndex = 0; setIndex < ePropertySetIndex_COUNT; ++setIndex)
	{
		...
		{
			DrxBonePhysics dummyBonePhysics;
			memset(&dummyBonePhysics, 0, sizeof(dummyBonePhysics));
			jointProperties[setIndex] = GetPhysInfoProperties_ROPE(dummyBonePhysics, (i32)type); // get the names & default values into the jointProperties
		}

		for (i32 propertyIndex = 1; propertyIndex < jointProperties[setIndex].size(); ++propertyIndex)
		{
...
void SJointPhysics::LoadFromXml(const XmlNodeRef& node)
{
	...
	DrxBonePhysics dummyBonePhysics;
	memset(&dummyBonePhysics, 0, sizeof(dummyBonePhysics));
	jointProperties[0] = jointProperties[1] = GetPhysInfoProperties_ROPE(dummyBonePhysics, (i32)type); // get the names & default values into the jointProperties

	char buf[32];
	for (i32 setIndex = 0; setIndex < ePropertySetIndex_COUNT; ++setIndex)
	{
		for (u32 propertyIndex = 1; propertyIndex < jointProperties[setIndex].size(); ++propertyIndex)
		{
			auto& property = jointProperties[setIndex][propertyIndex];

			drx_sprintf(buf, "lod%d_%s", setIndex, property.name);
			if (property.type == 0)
...
void CharacterAttachment::ChangeProxyType()
{
	...
	else
	{
		...
		i32 i;
		#define GSWITCH(a,b) ((i32)(a) * 16 + (i32)(b))
...
void CharacterAttachment::GenerateMesh()
{
	...
	if (m_proxySrc->m_hullMesh && m_meshSmooth > 0)
	{
		...
		if (i32 ngeoms = m_proxySrc->m_hullMesh->Proxify(pGeoms, &pp))
		{
			for(i32 i = 1; i < ngeoms; i++)
			...
			if (m_meshSmooth > 2)
			{
				...
				i32 nTris = pUtils->qhull(md.pVertices, md.nVertices, pTris);
...
void CharacterAttachment::UpdateMirrorInfo(i32 idBone, const IDefaultSkeleton& skel)
{
	m_boneTransMirror.SetIdentity();
	i32 idc = skel.GetJointChildIDAtIndexByID(idBone, 0);
	...
	tukk mirrorNames[] = { " l ", " r ",  "_l_", "_r_",  "#l ", "#r ",  "#l_", "#r_",  " l#", " r#",  "_l#", "_r#",  "left", "right" };
	string jname0 = m_strJointName, jname1; 
	jname0.MakeLower();
	for(i32 j = 0, id1 = -1; j < DRX_ARRAY_COUNT(mirrorNames) && id1 < 0; j++)
	{
		i32 l = strlen(mirrorNames[j]) - 1;
		...
		else if (tukk p = strstr(jname0, mirrorNames[j]))
...
void MirrorAttachment(const CharacterAttachment& att0, CharacterAttachment& att1)
{
	...
	i32 icorr = idxmax3(axProj.abs()), iter = 0;
	do 
	...
	for(i32 j = 0; j < 3; j++)
		att1.m_frame0[j] = ((i32)fabs(att1.m_frame0[j] * 10) * 0.1f) * sgnnz(att1.m_frame0[j]);
...
i32 CharacterDefinition::g_meshArchiveUsed = 0;
...
void CharacterAttachment::Serialize(Serialization::IArchive& ar)
{
	...
	if (m_attachmentType == CA_BONE)
	{
		...
		if (ar.isEdit())
		{
			tukk transformNamesBySpaces[] = { "transform_jj", "transform_jc", "transform_cj", "transform_cc" };
			tukk transformName = transformNamesBySpaces[(i32)m_rotationSpace * 2 + (i32)m_positionSpace];
		...
		i32 availableFlags = FLAGS_ATTACH_HIDE_ATTACHMENT | FLAGS_ATTACH_PHYSICALIZED_RAYS | FLAGS_ATTACH_PHYSICALIZED_COLLISIONS | FLAGS_ATTACH_EXCLUDE_FROM_NEAREST;
	...
	if (m_attachmentType == CA_SKIN)
	{
		...
		i32 availableFlags = FLAGS_ATTACH_HIDE_ATTACHMENT | FLAGS_ATTACH_EXCLUDE_FROM_NEAREST;
		{
	...
	if (m_attachmentType == CA_PROW)
	{
		...
		if (ct == RowSimulationParams::TRANSLATIONAL_PROJECTION)
		{
			...
			if (m_rowSimulationParams.m_nProjectionType)
			{
				...
				else
				{
					...
					u32 hasJointName = m_rowSimulationParams.m_strDirTransJoint.length();
					if (hasJointName == 0)
		...
		{
			ar(Serialization::Range(m_rowSimulationParams.m_nSimFPS, u8(10), u8(255)), "simulationFPS", "Simulation FPS");
	...
	if (m_attachmentType == CA_VCLOTH)
	{
		if (ar.openBlock("vcloth-parameter", "+Parameter"))
		{
			...
			if (ar.openBlock("debug", "+Debug"))
			{
				...
				ar(Serialization::Range(m_vclothParams.debugDrawCloth,0, std::numeric_limits<i32>::max()), "debugDrawCloth", "Draw Cloth");
				ar(Serialization::Range(m_vclothParams.debugDrawNndc,0, std::numeric_limits<i32>::max()), "debugDrawNNDC", "Draw Nearest Neighbor Distance Constraints");
				ar(Serialization::Range(m_vclothParams.debugPrint,0, std::numeric_limits<i32>::max()), "debugPrint", "Debug");
				ar.closeBlock();
			}
	...
	if (ICharacterInstance* characterInstance = ar.context<ICharacterInstance>())
	{
		...
		i32 id = defaultSkeleton.GetJointIDByName(m_strJointName.c_str());
...
void CharacterDefinition::Serialize(Serialization::IArchive& ar)
{
	...
	if (ar.isInput() && !modifiers)
		DrxCreateClassInstanceForInterface(cryiidof<IAnimationPoseModifierSetup>(), modifiers);
	if (modifiers)
		modifiers->Serialize(ar);
}

bool CharacterDefinition::LoadFromXmlFile(tukk filename)
{
...
bool CharacterDefinition::LoadFromXml(const XmlNodeRef& root)
{
	...
	struct tagname
	{
		...
		bool            m_dtext;
		bool            m_active;
		tagname(u32 crc32, AttachmentTypes type, bool dsetup, bool dtext, bool active)
		{
			m_crc32 = crc32;
			;
			m_type = type;
	...
	u32 numAttachments = attachments.size();
	for (u32 i = 0; i < numAttachments; i++)
	{
		...
		{
			u32 crc32 = CCrc32::ComputeLowercase(attachments[i].m_strSocketName.c_str());
	...
	for (u32 xmlnode = 0; xmlnode < numChilds; xmlnode++)
	{
		...
		if (strcmp(tag, "AttachmentList") == 0)
		{
			...
			u32 num = nodeAttachements->getChildCount();
			attachments.clear();
			...
			for (u32 i = 0; i < num; i++)
			{
				...
				nodeAttach->getAttr("PA_PendulumType", (i32&)ct);
				if (ct == SimulationParams::PENDULUM_CONE || ct == SimulationParams::PENDULUM_HINGE_PLANE || ct == SimulationParams::PENDULUM_HALF_CONE)
				{
					...
					u32 IsIdentical = stricmp(attach.m_simulationParams.m_strDirTransJoint.c_str(), attach.m_strJointName.c_str()) == 0;
				...
				tukk strProcFunction = nodeAttach->getAttr("ProcFunction");
	...
	//------------------------------------------------------------------------------------------------
	//------------------------------------------------------------------------------------------------
	u32 numDebugSetup = arrDebugSetup.size();
	for (u32 d = 0; d < numDebugSetup; d++)
	{
		u32 _crc32 = arrDebugSetup[d].m_crc32;
	...
	if (DrxCreateClassInstanceForInterface(cryiidof<IAnimationPoseModifierSetup>(), modifiers))
	{
...
XmlNodeRef CharacterDefinition::SaveToXml()
{
	...
	u32k numAttachments = nodeAttachments->getChildCount();
	...
	u32 numAttachmentsInList = attachments.size();
	for (u32 x = 0; x < numAttachmentsInList; x++)
	{

		DynArray<u8> arrProxies;
		arrProxies.resize(numAttachmentsInList);
		memset(&arrProxies[0], 0, numAttachmentsInList);
		for (u32 pn = 0; pn < attachments[x].m_simulationParams.m_arrProxyNames.size(); pn++)
		{
			tukk strProxyName = attachments[x].m_simulationParams.m_arrProxyNames[pn].c_str();
			for (u32 a = 0; a < numAttachmentsInList; a++)
			{
				tukk strSocketName = attachments[a].m_strSocketName.c_str();
		...
		for (u32 n = 0; n < numAttachmentsInList; n++)
		{
			if (arrProxies[n])
				attachments[x].m_simulationParams.m_arrProxyNames.push_back(CDrxName(attachments[n].m_strSocketName.c_str()));
		}

		memset(&arrProxies[0], 0, numAttachmentsInList);
		for (u32 pn = 0; pn < attachments[x].m_rowSimulationParams.m_arrProxyNames.size(); pn++)
		{
			tukk strProxyName = attachments[x].m_rowSimulationParams.m_arrProxyNames[pn].c_str();
			for (u32 a = 0; a < numAttachmentsInList; a++)
			{
				tukk strSocketName = attachments[a].m_strSocketName.c_str();
				if (attachments[a].m_attachmentType == CA_PROX && stricmp(strSocketName, strProxyName) == 0)
		...
		{
			if (arrProxies[n])
				attachments[x].m_rowSimulationParams.m_arrProxyNames.push_back(CDrxName(attachments[n].m_strSocketName.c_str()));
		}
	}
...
void CharacterDefinition::ExportPClothAttachment(const CharacterAttachment& attach, XmlNodeRef nodeAttachments)
{
	...
	nodeAttach->setAttr("ROW_ClampMode", (i32&)attach.m_rowSimulationParams.m_nClampMode);
	...
	if (ct == RowSimulationParams::TRANSLATIONAL_PROJECTION)
	{
		...
		{
			u32 IsIdentical = stricmp(attach.m_rowSimulationParams.m_strDirTransJoint.c_str(), attach.m_strJointName.c_str()) == 0;
		...
		u32 numProxiesName = std::min((u32)SimulationParams::MaxCollisionProxies, (u32)attach.m_rowSimulationParams.m_arrProxyNames.size());
		char proxytag[] = "ROW_Proxy00";
		for (u32 i = 0; i < numProxiesName; i++)
		{
			tukk pname = attach.m_rowSimulationParams.m_arrProxyNames[i].c_str();
...
void CharacterDefinition::ExportSimulation(const CharacterAttachment& attach, XmlNodeRef nodeAttach)
{
	...
	{
		u32 ct = attach.m_simulationParams.m_nClampType;
		if (ct == SimulationParams::PENDULUM_CONE || ct == SimulationParams::PENDULUM_HINGE_PLANE || ct == SimulationParams::PENDULUM_HALF_CONE)
		{
			nodeAttach->setAttr("PA_PendulumType", (i32&)attach.m_simulationParams.m_nClampType);
			...
			u32 numProxiesName = std::min((u32)SimulationParams::MaxCollisionProxies, (u32)attach.m_simulationParams.m_arrProxyNames.size());
			char proxytag[] = "PA_Proxy00";
			for (u32 i = 0; i < numProxiesName; i++)
			{
				tukk pname = attach.m_simulationParams.m_arrProxyNames[i].c_str();
				nodeAttach->setAttr(proxytag, pname);
				proxytag[9]++;
			}
		}
		...
		if (ct == SimulationParams::SPRING_ELLIPSOID)
		{
			nodeAttach->setAttr("SA_SpringType", (i32&)attach.m_simulationParams.m_nClampType);
		...
		if (ct == SimulationParams::TRANSLATIONAL_PROJECTION)
		{
			nodeAttach->setAttr("P_Projection", (i32&)attach.m_simulationParams.m_nClampType);
...
bool CharacterDefinition::Save(tukk filename)
{
...
static string NormalizeMaterialName(tukk materialName)
{
...
void CharacterDefinition::ApplyPhysAttachments(ICharacterInstance* pICharacterInstance)
{
	...
	IGeomManager *pGeoman = gEnv->pPhysicalWorld->GetGeomManager();
	for(i32 i = 0; i < skel.GetJointCount(); i++)
	...
	i32 numAttachments = attachments.size();
	for (i32 i = 0; i < numAttachments; ++i)
	{
		...
		if (attachment.m_attachmentType == CA_PROX && attachment.m_ProxyPurpose == CharacterAttachment::RAGDOLL)
		{
			...
			{
				DrxBonePhysics& phys = *skel.GetJointPhysInfo(skel.GetJointIDByName(attachment.m_strJointName));
	...
	i32 nRoots = 0, idummyRoot = -1;
	for(i32 i = 0, j; i < skel.GetJointCount(); i++)	if (skel.GetJointPhysGeom(i)) 
	{
	...
	if (nRoots > 1)
	{
		...
		DrxBonePhysics& phys = *skel.GetJointPhysInfo(idummyRoot);
		memset(&phys, 0, sizeof(DrxBonePhysics));
	...
	for (i32 i = 0, j; i < numAttachments; ++i)
	{	// apply 0-angles rotations last since they need full parent-child hierarchy with physicalization
		...
		{	
			i32 idx = skel.GetJointIDByName(att.m_strJointName);
...
void CharacterDefinition::ApplyToCharacter(bool* skinSetChanged, ICharacterInstance* pICharacterInstance, ICharacterManager* characterManager, bool showDebug, bool applyPhys)
{
	...
	vector<string> names;
	u32 attachmentCount = pIAttachmentManager->GetAttachmentCount();
	...
	u32 proxyCount = pIAttachmentManager->GetProxyCount();
	...
	{
		const CharacterAttachment& attachment = attachments[i];
		tukk strSocketName = attachment.m_strSocketName.c_str();
		i32 index = pIAttachmentManager->GetIndexByName(strSocketName);
		...
		if (pIAttachment == 0)
		{
			if (index < 0 && attachment.m_attachmentType == CA_PROX)
			{
				...
				if (pIProxy == 0)
				{
					u32 IsEmpty = attachment.m_strJointName.empty();
					if (IsEmpty == 0)
		...
		u32 flags = pIAttachment->GetFlags() & nMaskForDynamicFlags;
		...
		for (u32 i = 0; i < numAttachments; ++i)
		{
			...
			if (attachment.m_attachmentType == CA_BONE)
			{
				...
				if (pIAttachment)
				{
					...
					i32 id = rIDefaultSkeleton.GetJointIDByName(attachment.m_strJointName.c_str());
					if (id == -1)
						continue;
...
void CharacterDefinition::LoadPhysProxiesFromCharacter(ICharacterInstance *character)
{
	...
	for(i32 i = 0; i < skel.GetJointCount(); i++)
		if (const phys_geometry *pPhys = skel.GetJointPhysGeom(i))
		{
			...
			const DrxBonePhysics& bp = *skel.GetJointPhysInfo(i);
			for(i32 l = 0; l < 2; l++) for(i32 j = 0; j < 3; j++)
				att.m_limits[l][j] = (i32)(min(180.0f, max(-180.0f, RAD2DEG(bp.min[l * 3 + j] * (~bp.flags >> j & 1)))) + 180) - 180;
			...
			for(j = 0; j < 3; j++)
				att.m_frame0[j] = ((i32)fabs(att.m_frame0[j] * 10) * 0.1f) * sgnnz(att.m_frame0[j]);
...
void CharacterDefinition::SavePhysProxiesToCGF(tukk fname)
{
	IStatObj* pSkelObj = gEnv->p3DEngine->CreateStatObj();
	pSkelObj->FreeIndexedMesh();
	for(i32 i = 0; i < attachments.size(); i++)
	{
		const CharacterAttachment& att = attachments[i];
		if (att.m_attachmentType != CA_PROX || att.m_ProxyPurpose != CharacterAttachment::RAGDOLL)
			continue;
...
void CharacterDefinition::ApplyBoneAttachment(IAttachment* pIAttachment, ICharacterManager* characterManager, const CharacterAttachment& desc, ICharacterInstance* pICharacterInstance, bool showDebug) const
{
	...
	u32 type = pIAttachment->GetType();
	if (type == CA_BONE)
	{
		tukk strJointname = desc.m_strJointName.c_str();
		i32 nJointID1 = pIAttachment->GetJointID();
		i32 nJointID2 = pICharacterInstance->GetIDefaultSkeleton().GetJointIDByName(strJointname);
		...
		bAttachmentSortingRequired |= u8(ap.m_useRedirect) != desc.m_simulationParams.m_useRedirect;
...
void CharacterDefinition::ApplySkinAttachment(IAttachment* pIAttachment, ICharacterManager* characterManager, const CharacterAttachment& desc, ICharacterInstance* pICharacterInstance, bool* skinChanged) const
{
	    ...
	    || hadSoftwareSkinningEnabled != ((desc.m_nFlags & FLAGS_ATTACH_SW_SKINNING) != 0))
	{
		...
		{
			bool isSkin = stricmp(fileExt, DRX_SKIN_FILE_EXT) == 0;
			if (isSkin)
