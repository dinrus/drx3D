...
namespace Serialization
{
	...
	template<typename T, typename TContainer>
	inline bool Serialize(Serialization::IArchive& ar, CListSelectorDecorator<T, TContainer>& value, tukk name, tukk label)
	{
		if (ar.isEdit())
		{
			if (ar.isInput())
	...
	inline auto ListSelector(T& value, const TContainer& items, const StringList& labels, const T fallback) -> CListSelectorDecorator<T, TContainer>
	{
		...
		DRX_ASSERT(labels.size() == std::distance(std::begin(items), std::end(items)));
...
struct SMotionParametersPool
{
	...
	struct SSelector
	{
		SSelector(i32& value, bool onlyParamsWithExtractionFlag = false)
			: value(value)
	...
	std::vector<i32> allParams;
	Serialization::StringList allLabels;

	std::vector<i32> extractionFlaggedParams;
	Serialization::StringList extractionFlaggedLabels;
};

static bool Serialize(Serialization::IArchive& ar, SMotionParametersPool::SSelector& self, tukk name, tukk label)
{
	DRX_ASSERT(ar.context<SMotionParametersPool>());
	auto& pool = *ar.context<SMotionParametersPool>();

	const bool result = self.onlyParamsWithExtractionFlag
		? ar(Serialization::ListSelector(self.value, pool.extractionFlaggedParams, pool.extractionFlaggedLabels, i32(eMotionParamID_INVALID)), name, label)
		: ar(Serialization::ListSelector(self.value, pool.allParams, pool.allLabels, i32(eMotionParamID_INVALID)), name, label);
...
static tukk ParameterNameById(EMotionParamID id, bool additionalExtraction)
{
	SMotionParameterDetails details;
...
static EMotionParamID ParameterIdByName(tukk name, bool additionalExtraction)
{
	for (i32 i = 0; i < eMotionParamID_COUNT; ++i)
	{
		SMotionParameterDetails details;
...
void BlendSpaceAnnotation::Serialize(IArchive& ar)
{
	...
	std::vector<DrxGUID> exampleList;
	Serialization::StringList exampleLabels;
	if (ar.isEdit())
	{
		DRX_ASSERT(ar.context<BlendSpace>());
	...
	{
		ar(Serialization::ListSelector(guid, exampleList, exampleLabels, DrxGUID::Null()), "", "<");
	...
	if (ar.isEdit() && ar.isOutput())
	{
		...
		else
		{
			if (ICharacterInstance* characterInstance = ar.context<ICharacterInstance>())
			{
				...
				i32 animationId = animationSet->GetAnimIDByName(animation.c_str());
				if (animationId < 0)
				{
				...
				else
				{
					...
					{
						i32 flags = animationSet->GetAnimationFlags(animationId);
...
void BlendSpacePseudoExample::Serialize(IArchive& ar)
{
	...
	ar(Serialization::ListSelector(guid0, exampleList, exampleLabels, DrxGUID::Null()), "guid0", "^<");
	ar(weight0, "weight0", ">^");
	ar(Serialization::ListSelector(guid1, exampleList, exampleLabels, DrxGUID::Null()), "guid1", "^<");
	ar(weight1, "weight1", ">^");
}
...
bool BlendSpace::LoadFromXml(string& errorMessage, XmlNodeRef root, IAnimationSet* pAnimationSet)
{
	...
	for (u32 c = 0; c < numChilds; c++)
	{
		...
		if (strcmp(ListTag, "VEGPARAMS") == 0)
		{
			...
			if (nFlags)
				m_idleToMove = true; //this is a case for DrxMannequin
			continue;
		}
		...
		{
			i32 numDimensions = nodeList->getChildCount();
			...
			for (u32 d = 0; d < m_dimensions.size(); d++)
			{
				...
				if (strcmp(ExampleTag, "Param") == 0)
				{
					...
					tukk parameterName = nodeExample->getAttr("name");
		...
		{
			i32 numExtractionParams = nodeList->getChildCount();
			...
			for (u32 d = 0; d < numExtractionParams; d++)
			{
				XmlNodeRef nodeExample = nodeList->getChild(d);
				tukk ExampleTag = nodeExample->getTag();
				if (strcmp(ExampleTag, "Param") == 0)
		...
		if (strcmp(ListTag, "ExampleList") == 0)
		{
			i32 numExamples = nodeList->getChildCount();
		...
		if (strcmp(ListTag, "ExamplePseudo") == 0)
		{
			...
			for (u32 i = 0; i < num; i++)
			{
				...
				if (strcmp(ExampleTag, "Pseudo") == 0)
				{
					...
					const auto exampleIndex2Guid = [&](i32 index) -> DrxGUID
					{
						return (index >= 0 && size_t(index) < m_examples.size()) ? m_examples[index].runtimeGuid : DrxGUID::Null();
					};
...
XmlNodeRef BlendSpace::SaveToXml() const
{
	...
	u32 numDimensions = m_dimensions.size();
	...
	if (u32 numExtractionParams = m_additionalExtraction.size())
	{
	...
	i32 numExamples = m_examples.size();
	for (u32 i = 0; i < numExamples; ++i)
	{
	...
	if (u32 pseudoCount = m_pseudoExamples.size())
	{
		nodeList = root->newChild("ExamplePseudo");
		for (u32 i = 0; i < pseudoCount; ++i)
		{
			const auto exampleGuid2Index = [&](const DrxGUID& guid) -> i32
			{
				const auto itSearch = std::find_if(m_examples.begin(), m_examples.end(), [&](const auto& example) { return example.runtimeGuid == guid; });
				return (itSearch != m_examples.end()) ? i32(std::distance(m_examples.begin(), itSearch)) : i32(-1);
			};
	...
	if (u32k annotationCount = m_annotations.size())
	{
		const auto exampleGuid2Index = [&](const DrxGUID& guid) -> i32
		{
			const auto itSearch = std::find_if(m_examples.begin(), m_examples.end(), [&](const auto& example) { return example.runtimeGuid == guid; });
			...
			{
				return i32(std::distance(m_examples.begin(), itSearch));
			}
			...
			{
				return i32(m_examples.size() + std::distance(m_pseudoExamples.begin(), itSearchPseudo));
			}
		...
		for (u32 i = 0; i < annotationCount; ++i)
		{
			...
			DRX_ASSERT(face.exampleGuids.size() <= DRX_ARRAY_COUNT(facePointNames));
			for (u32 k = 0, pointCount = face.exampleGuids.size(); k < pointCount; ++k)
			{
	...
	if (u32 motionCombinationCount = m_motionCombinations.size())
	{
		nodeList = root->newChild("MotionCombination");
		for (u32 i = 0; i < motionCombinationCount; ++i)
		{
	...
	if (u32 jointCount = m_joints.size())
	{
	    ...
	    !m_virtualExamples3d.empty())
	{
		...
		if (!m_virtualExamples1d.empty())
		{
			for (u32 i = 0; i < m_virtualExamples1d.size(); ++i)
			{
		...
		{
			for (u32 i = 0; i < m_virtualExamples2d.size(); ++i)
			{
		...
		{
			for (u32 i = 0; i < m_virtualExamples3d.size(); ++i)
			{
...
void BlendSpace::Serialize(Serialization::IArchive& ar)
{
	...
	i32k maxBlendSpaceAnimations = 40;
...
XmlNodeRef CombinedBlendSpace::SaveToXml() const
{
	...
	if (u32 num = m_additionalExtraction.size())
	{
	...
	if (u32 num = m_motionCombinations.size())
	{
