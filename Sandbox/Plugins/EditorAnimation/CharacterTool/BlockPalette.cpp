...
static QRect AddElementToLayout(i32* currentTop, i32* currentLeft, i32 width, i32 elementWidth, i32 lineHeight, i32 padding)
{
...
static void CalculateLayout(BlockPaletteLayout* layout, const BlockPaletteContent& content, const QFont& font, i32 width)
{
	layout->items.clear();

	i32 padding = ELEMENT_PADDING;
	layout->height = padding;
	i32 currentLeft = padding;
	i32 margin = WIDGET_MARGIN;

	QFontMetrics metrics(font);
	i32 lineHeight = metrics.height() + margin * 2;
	if (lineHeight < 24)
		lineHeight = 24;
	...
	for (size_t i = 0; i < content.items.size(); ++i)
	{
		...
		SLayoutItem litem;
		i32 elementWidth = metrics.width(QString::fromLocal8Bit(item.name.c_str())) + 2 * ELEMENT_MARGIN;
...
static i32 HitItem(const SLayoutItems& items, const QPoint& pos)
{
...
static void HitItems(std::vector<i32>* selectedItems, const SLayoutItems& items, const QRect& rect)
{
	selectedItems->clear();
	for (size_t i = 0; i < items.size(); ++i)
	{
		...
		if (item.rect.intersects(rect))
			selectedItems->push_back(i32(i));
	}
}

static i32 FindDropIndex(const SLayoutItems& items, const QPoint& p, i32 excludingIndex)
{
	if (items.empty())
		return -1;
	typedef std::vector<std::pair<i32, i32>> DistanceToIndex;
	...
	for (size_t i = 0; i < items.size(); ++i)
	{
		...
		{
			distances.push_back(std::make_pair((p - r.topLeft()).manhattanLength(), i32(i)));
			distances.push_back(std::make_pair((p - r.bottomLeft()).manhattanLength(), i32(i)));
		}
		if (i + 1 < excludingIndex || i + 1 > excludingIndex + 1)
		{
			distances.push_back(std::make_pair((p - r.topRight()).manhattanLength(), i32(i + 1)));
			distances.push_back(std::make_pair((p - r.bottomRight()).manhattanLength(), i32(i + 1)));
		}
	}
	if (distances.empty())
		return -1;
...
static void DrawItem(QPainter& painter, const SLayoutItem& litem, const BlockPaletteItem& item, bool selected, bool hasFocus, const QPalette& palette, i32 hotkey)
{
...
struct BlockPalette::DragHandler : MouseHandler
{
	...
	void Finish() override
	{
		if (m_moved)
		{
			if (m_dropIndex != -1)
			{
				...
				{
					i32 index = m_palette->m_draggedItems[i];
				...
				{
					i32 oldIndex = m_palette->m_hotkeys[i];
	...
	void MouseReleaseEvent(QMouseEvent* ev) override
	{
		...
		{
			i32 itemIndex = HitItem(m_palette->m_layout->items, ev->pos());
			if (itemIndex >= 0)
	...
	void PaintOver(QPainter& painter) override
	{
		if (DragStarted())
		{
			...
			i32 index = m_palette->m_selectedItems[0];
...
void BlockPalette::paintEvent(QPaintEvent* ev)
{
	...
	{
		bool dragged = std::find(m_draggedItems.begin(), m_draggedItems.end(), i32(i)) != m_draggedItems.end();
		;
		if (dragged)
			continue;
		bool selected = std::find(m_selectedItems.begin(), m_selectedItems.end(), i32(i)) != m_selectedItems.end();
...
void BlockPalette::ItemClicked(i32 itemIndex)
{
...
void BlockPalette::AssignHotkey(i32 hotkey)
{
	if (size_t(hotkey) >= 10)
		return;
...
void BlockPalette::mousePressEvent(QMouseEvent* ev)
{
	...
	{
		i32 hitItem = HitItem(m_layout->items, ev->pos());
		if (hitItem >= 0)
		{
	...
	else if (ev->button() == Qt::RightButton)
	{
		...
		i32 selectionCount = m_selectedItems.size();
		...
		if (selectionCount == 1)
		{
			...
			for (i32 i = 0; i < 10; ++i)
			{
				...
				hotkeyMenu->addAction(text, this, SLOT(OnMenuAssignHotkey()), QKeySequence(shortcut))->setData(i32((i + 1) % 10));
			}
		}
		else
...
{
	i32 itemIndex = HitItem(m_layout->items, ev->pos());
	if (itemIndex >= 0)
	{
...
void BlockPalette::keyPressEvent(QKeyEvent* ev)
{
	if (!m_selectedItems.empty())
	{
		if ((ev->modifiers() & Qt::CTRL) != 0)
		{
			...
			{
				i32 keyIndex = i32(ev->key()) - Qt::Key_0;
...
const BlockPaletteItem* BlockPalette::GetItemByHotkey(i32 hotkey) const
{
	if (size_t(hotkey) >= m_hotkeys.size())
		return 0;
	i32 index = m_hotkeys[hotkey];
	if (size_t(index) >= m_content->items.size())
		return 0;
...
void BlockPalette::OnMenuAdd()
{
	...
	SignalChanged();
	m_selectedItems.clear();
	m_selectedItems.push_back(i32(m_content->items.size()) - 1);
	SignalSelectionChanged();
	UpdateLayout();
}
...
void BlockPalette::OnMenuDelete()
{
	...
	for (i32 i = i32(selectedItems.size()) - 1; i >= 0; --i)
	{
		i32 index = selectedItems[i];
