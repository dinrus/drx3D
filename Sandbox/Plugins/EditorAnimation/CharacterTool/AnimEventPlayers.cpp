...
void SerializeParameterAnimFXEvent(string& parameter, IArchive& ar)
{
	...
	i32 resultEventTypeUid = -1;
	i32 resultEventSubTypeUid = -1;

	u32 eventTypeUid(0u);
	u32 eventSubTypeUid(0u);
	sscanf_s(parameter.c_str(), "%u %u", &eventTypeUid, &eventSubTypeUid);

	i32k eventsCount = pAnimFXEvents->GetAnimFXEventsCount();
	eventList.reserve(eventsCount + 1);

	eventList.push_back("<unassigned>");

	SAnimFXEventInfo eventType;
	for (i32 eventIndex = 0; eventIndex < eventsCount; ++eventIndex)
	{
	...
	for (i32 eventIndex = 0; eventIndex < eventsCount; ++eventIndex)
	{
		...
		if (!strcmpi(eventTypeChoice.c_str(), eventType.szName))
		{
			...
			i32 eventParamCount = pAnimFXEvents->GetAnimFXEventParamCount(eventIndex);
			...
			if (eventParamCount)
			{
				...
				for (i32 eventParamIndex = 0; eventParamIndex < eventParamCount; eventParamIndex++)
				{
...
static void SpliterParameterValue(string* switchName, string* switchValue, tukk parameter)
{
	tukk sep = strchr(parameter, '=');
	if (!sep)
	{
...
static Vec3 GetBonePosition(ICharacterInstance* character, const QuatTS& physicalLocation, tukk boneName)
{
	i32 jointId = -1;
	if (boneName != '\0')
...
public:
	...
	DRXINTERFACE_END()
	CRYGENERATE_CLASS_GUID(AnimEventPlayer_AudioTranslationLayer, "AnimEventPlayer_AudioTranslationLayer", "a9fefa2d-fe04-dec4-a616-9d6e3ac635b0"_drx_guid)
	...
	{
		DrxAudio::SCreateObjectData const objectData("Character Tool", DrxAudio::EOcclusionType::Ignore);
	...
	bool Play(ICharacterInstance* character, const AnimEventInstance& event) override
	{
		if (!m_pIAudioObject)
			return false;
		tukk name = event.m_EventName ? event.m_EventName : "";
	...
	void SetRTPC(tukk name, float value)
	{
		DrxAudio::ControlId const parameterId = DrxAudio::StringToId(name);
		m_pIAudioObject->SetParameter(parameterId, value);
	}

	void SetSwitch(tukk name, tukk state)
	{
		DrxAudio::ControlId const switchId = DrxAudio::StringToId(name);
		DrxAudio::SwitchStateId const stateId = DrxAudio::StringToId(state);
		m_pIAudioObject->SetSwitchState(switchId, stateId);
	}

	void PlayTrigger(tukk trigger, Vec3 const& pos)
	{
		DrxAudio::ControlId const triggerId = DrxAudio::StringToId(trigger);
		m_pIAudioObject->SetTransformation(pos);
...
private:
	...
	DrxAudio::IListener*          m_pIAudioListener;
	string                        m_parameter;
...
public:
	...
	DRXINTERFACE_ADD(IAnimEventPlayer)
	DRXINTERFACE_END()
	CRYGENERATE_CLASS_GUID(AnimEventPlayerMaterialEffects, "AnimEventPlayer_MaterialEffects", "a9fffa2d-ae34-d6c4-a696-9d6e3ac732b0"_drx_guid)
	...
	bool Play(ICharacterInstance* character, const AnimEventInstance& event) override
	{
		...
		if (stricmp(name, "footstep") == 0)
		{
			...
			if (pMaterialEffects)
			{
				tukk effectName = (event.m_CustomParameter && event.m_CustomParameter[0]) ? event.m_CustomParameter : m_defaultFootstepEffect.c_str();
...
class AnimEventPlayerAnimFXEvents : public IAnimEventPlayer
{
	struct SAnimFXSource
	{
		...
		void Serialize(Serialization::IArchive& ar)
		{
			...
			{
				u32 index = 0;
				for (u32 i = 0; i < pAnimFXEvents->GetLibrariesCount(); ++i)
				{
					tukk soundLibraryName = pAnimFXEvents->GetLibraryName(i);
					soundFXLibsList.push_back(soundLibraryName);
				}

				i32 foundIdx = soundFXLibsList.find(m_soundFXLib.c_str());
				index = foundIdx == Serialization::StringList::npos ? 0 : u32(foundIdx);
...
public:
	...
	DRXINTERFACE_ADD(IAnimEventPlayer)
	DRXINTERFACE_END()
	CRYGENERATE_CLASS_GUID(AnimEventPlayerAnimFXEvents, "AnimEventPlayer_AnimFXEvents", "9688f293-04f3-400c-886c-2d31c5199481"_drx_guid)
	...
	bool Play(ICharacterInstance* character, const AnimEventInstance& event) override
	{
		...
		if (IMaterialEffects* pMaterialEffects = gEnv->pMaterialEffects)
		{
			...
			{
				pAnimFXEvents->GetAnimFXEventInfoById(u32(eventUid), eventType);
				...
				if (!effectFound)
				{
					for (const SAnimFXSource& animFxSource : m_animFxSources)
					{
						tukk szFullEventName = pAnimFXEvents->GetAnimFXEventParamName(eventUid, eventNameUid);
						if (!eventType.szName)
						{
...
public:
	...
	DRXINTERFACE_ADD(IAnimEventPlayer)
	DRXINTERFACE_END()
	CRYGENERATE_CLASS_GUID(AnimEventPlayer_Particles, "AnimEventPlayer_Particles", "a9fef72d-f304-dec4-a916-2a6e4ac635b0"_drx_guid)
...
AnimEventPlayer_CharacterTool::AnimEventPlayer_CharacterTool()
{
	...
	if (DrxCreateClassInstance<IAnimEventPlayer>(cryiidof<IAnimEventPlayerGame>(), defaultGamePlayer))
	{
	...
	if (::DrxCreateClassInstance<IAnimEventPlayer>(CharacterTool::AnimEventPlayer_AudioTranslationLayer::GetCID(), player))
	{
		m_list.push_back(player);
		m_names.push_back("Audio Translation Layer");
	}
	if (::DrxCreateClassInstance<IAnimEventPlayer>(CharacterTool::AnimEventPlayerMaterialEffects::GetCID(), player))
	{
		m_list.push_back(player);
		m_names.push_back("Material Effects");
	}
	if (::DrxCreateClassInstance<IAnimEventPlayer>(CharacterTool::AnimEventPlayer_Particles::GetCID(), player))
	{
		m_list.push_back(player);
		m_names.push_back("Particles");
	}
	if (::DrxCreateClassInstance<IAnimEventPlayer>(CharacterTool::AnimEventPlayerAnimFXEvents::GetCID(), player))
	{
		m_list.push_back(player);
