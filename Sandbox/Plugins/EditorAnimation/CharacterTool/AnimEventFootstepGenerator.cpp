...
typedef std::map<u32, std::vector<QuatT>> BoneSampleMap;

static bool SampleAnimation(BoneSampleMap* framesPerChannel, float* animationLength, string* errorMessage, const std::vector<u32>& bonesToTrack, tukk characterPath, tukk animationName)
{
	...
	*animationLength = -1.0f;
	...
	i32k animationId = animationSet->GetAnimIDByName(animationName);
	...
	*animationLength = skeletonAnimation.GetAnimFromFIFO(0, 0).GetExpectedTotalDurationSeconds();
	...
	u32k frameCount = u32((*animationLength * ANIMATION_FRAMERATE) + 0.5f);
...
{
public:
	FootstepGenerator(float fFootDownHeight, float fFootUpperLimit, u32 nFootBoneID, const AnimEvent& event, bool bShuffle)
	...
	u32 FootBoneID() const { return m_footBoneId; }
...
void FootstepGenerationParameters::SetLeftFootJoint(tukk joint)
{
	leftFootJoint = joint;
...
void FootstepGenerationParameters::SetRightFootJoint(tukk joint)
{
...
bool GenerateFootsteps(AnimationContent* content, string* errorMessage, ICharacterInstance* character, tukk animationName, const FootstepGenerationParameters& params)
{
	...
	tukk szFilePath = character->GetFilePath();

	// Sample the animation
	IDefaultSkeleton& skeleton = character->GetIDefaultSkeleton();
	float footHeight = params.footHeightMM * 0.001f;
	u32k leftFootIndex = skeleton.GetJointIDByName(params.leftFootJoint.c_str());
	u32k rightFootIndex = skeleton.GetJointIDByName(params.rightFootJoint.c_str());
