...
struct ActionCreator
{
	...
	QAction* operator()(tukk name, tukk slot)
	{
	...
	QAction* operator()(tukk name, tukk slot, const QVariant& actionData)
	{
		QAction* action = menu->addAction(name);
		action->setCheckable(checkable);
...
void CharacterToolForm::Initialize()
{
	...
	setCentralWidget(centralWidget);
	{
...

		{
			...
			m_displayParametersButton->setIcon(DrxIcon("icons:General/Options.ico"));
			...
			m_createProxyModeButton->setIcon(DrxIcon("icons:common/animation_skeleton.ico"));
...
static QDockWidget* CreateDockWidget(std::vector<QDockWidget*>* dockWidgets, tukk title, const QString& name, QWidget* widget, Qt::DockWidgetArea area, CharacterToolForm* window)
{
...
void CharacterToolForm::OnCharacterLoaded()
{
	...
	tukk filename = m_system->document->LoadedCharacterFilename();
...
void CharacterToolForm::InstallMode(IViewportMode* mode, ExplorerEntry* modeEntry)
{
	...
	else
	{
		...
		QAction* action = m_modeToolBar->addAction(DrxIcon("icons:Navigation/Basics_Move.ico"), QString());
		action->setEnabled(false);
		action->setPriority(QAction::LowPriority);
		action = m_modeToolBar->addAction(DrxIcon("icons:Navigation/Basics_Rotate.ico"), QString());
		action->setEnabled(false);
		action->setPriority(QAction::LowPriority);
		action = m_modeToolBar->addAction(DrxIcon("icons:Navigation/Basics_Scale.ico"), QString());
		action->setEnabled(false);
		action->setPriority(QAction::LowPriority);
	}
}
...
void CharacterToolForm::SaveLayoutToFile(tukk szFilePath)
{
...
void CharacterToolForm::LoadLayoutFromFile(tukk szFilePath)
{
...
void CharacterToolForm::OnFileRecentAboutToShow(QMenu* recentMenu)
{
	...
	for (size_t i = 0; i < m_recentCharacters.size(); ++i)
	{
		tukk fullPath = m_recentCharacters[i].c_str();
		string path;
		string file;
		PathUtil::Split(fullPath, path, file);
		QString text;
		if (i < 10 - 1)
			text.sprintf("&%d. %s\t%s", i32(i + 1), file.c_str(), path.c_str());
		else if (i == 10 - 1)
	...
	if (sender == m_splitViewport->CompressedViewport())
	{
		...
		if (ev->type() == QEvent::KeyPress)
		{
			...
			if (press->key() == QKeySequence(Qt::Key_F1))
			{
				static i32 index;
				char buf[128];
				drx_sprintf(buf, "Objects/%i/test.chr", index);
