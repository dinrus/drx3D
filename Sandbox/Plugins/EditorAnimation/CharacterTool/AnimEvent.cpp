...
bool IsAudioEventType(tukk type)
{
	static tukk audioEvents[] = { "sound", "foley", "footstep" };
	for (i32 i = 0; i < sizeof audioEvents / sizeof audioEvents[0]; ++i)
...
void AnimEvent::Serialize(IArchive& ar)
{
	...
	else
	{
		...
		if (player)
		{
			...
			for (i32 i = 0; i < customEventCount; ++i)
			{
		...
		i32 customIndex = -1;
		i32 usedParameters = typeChoice.index() == 0 ? ANIM_EVENT_USES_ALL_PARAMETERS : 0;
		tukk tooltip = "";
		for (i32 i = 0; i < customEventCount; ++i)
