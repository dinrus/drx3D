...
bool JointSelectionDialog::chooseJoint(string* name, IDefaultSkeleton* skeleton)
{
	...
	if (skeleton)
	{
		...
		i32 jointCount = skeleton->GetJointCount();
		...
		for (i32 i = 0; i < jointCount; ++i)
		{
			i32 parent = skeleton->GetJointParentIDByID(i);
...
// ---------------------------------------------------------------------------
dll_string JointNameSelector(const SResourceSelectorContext& x, tukk previousValue, ICharacterInstance* characterInstance)
{
