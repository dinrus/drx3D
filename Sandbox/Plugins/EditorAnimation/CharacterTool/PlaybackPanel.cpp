	...
	{
		{
...

			{
				...
				m_optionsButton = new QPushButton(DrxIcon("icons:General/Options.ico"), QString(), this);
				{
		...
		m_playIcon = DrxIcon("icons:common/animation_play.ico");
		m_pauseIcon = DrxIcon("icons:common/animation_pause.ico");
...
void PlaybackPanel::OnTimelineHotkey(i32 number)
{
...
static void SelectElementsById(STimelineTrack* track, const vector<i32>& userIds)
{
	for (size_t i = 0; i < track->elements.size(); ++i)
	{
...
void PlaybackPanel::OnTimeUnitsChanged(i32 index)
{
...
void PlaybackPanel::OnSpeedChanged(i32 index)
{
	float speed = 1.0f;
...
void PlaybackPanel::OnPlaybackStateChanged()
{
	...
	tukk tooltip = state == PLAYBACK_UNAVAILABLE ? m_system->document->PlaybackBlockReason() : "";
...
void PlaybackPanel::UpdateTimeUnitsUI(bool timeChanged, bool durationChanged)
{
	...
	i32 numFrames = i32(duration * FrameRate() + 0.5f);
	...
	if (durationChanged)
	{
		...
		if (duration >= 0.0f)
		{
			...
			if (m_timeUnits == TIME_IN_FRAMES)
				str.sprintf("/ %i ", i32(totalDisplay + 0.5));
			else
...
void PlaybackPanel::Serialize(Serialization::IArchive& ar)
{
	...
	{
		UpdateTimeUnitsUI(true, true);
		m_timeUnitsCombo->SetChecked(i32(m_timeUnits));
	}
}
...
void PlaybackPanel::WriteTimeline()
{
	...
	SEntry<AnimationContent>* animation = GetActiveAnimationEntry(m_system);
	i32 numEvents = animation ? animation->content.events.size() : 0;

	STimelineElements& elements = track.elements;
	vector<i32> selectedIds;
	{
		i32 newSelectedElementIndex = 0;
	...
	if (animation)
	{
		for (size_t i = 0; i < animation->content.events.size(); ++i)
		{
			...
			u32 hash = CCrc32::Compute(static_cast<uk>(e.userSideLoad.data()), e.userSideLoad.size());
			EventContentToColorMap::const_iterator it = std::lower_bound(eventContentToColor.begin(), eventContentToColor.end(), std::make_pair(hash, ColorB()),
			                                                             [&](const std::pair<u32, ColorB>& a, const std::pair<u32, ColorB>& b) { return a.first < b.first; }
			                                                             );
...
void PlaybackPanel::ReadTimeline()
{
	...
	for (i32 i = i32(removedElements.size() - 1); i >= 0; --i)
	{
...
void PlaybackPanel::PutAnimEvent(const AnimEventPreset& preset)
{
	...
	i32 index = animation->content.events.size();
	...
	vector<i32> selection(1, index + 1);
	SelectElementsById(m_timelineContent->track.tracks[0], selection);
	m_timeline->ContentUpdated();
	m_timeline->setFocus();
}

bool PlaybackPanel::HandleKeyEvent(i32 key)
{
...
void PlaybackPanel::OnSubselectionChanged(i32 layer)
{
	if (layer != GIZMO_LAYER_ANIMATION)
		return;
	const vector<ukk>& handles = m_system->characterGizmoManager->Subselection(GIZMO_LAYER_ANIMATION);
