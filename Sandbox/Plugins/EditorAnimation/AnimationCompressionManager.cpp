...
static bool IsFileExists(tukk filename)
{
...
static bool DeleteFileInResourceCacheFolder(tukk filename)
{
...
{
public:
	CBackgroundTaskCompressCAF(CAnimationCompressionManager* pManager, tukk animationName)
	...
	tukk AnimationName() const         { return m_animationName.c_str(); }
...
public:
	...
	tukk Description() const override { return "Update Local Animations"; }
	...
	ETaskResult Work() override
	{
		...
		if (CafCompressionHelper::CheckIfNeedUpdate(animationName.c_str()))
		{
			i32 index = m_totalFileCount - m_fileList.size();
...
{
public:
	tukk Description() const override { return "Clean up compiled animations"; }

	ETaskResult Work() override
	{
		std::vector<string> filenames;
		tukk const filePattern = "*.caf";
...
public:
	...
	tukk Description() const override { return "Reload CHRPARAMS"; }
...
void CAnimationCompressionManager::Start()
{
	...
	m_enabled = gEnv->pDrxPak->IsFileExist("Animations/skeletonList.xml");
	if (m_enabled)
	{
		tukk animationsRoot = "";
...
void CAnimationCompressionManager::QueueAnimationCompression(tukk animation)
{
...
void CAnimationCompressionManager::OnFileChange(tukk filename, EChangeType eType)
{
