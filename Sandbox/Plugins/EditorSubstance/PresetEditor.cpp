...
{
	static tukk Module = "EditorSubstance::PresetEditor";
	static tukk XRes = "ResolutionX";
	static tukk YRes = "ResolutionY";
	static tukk uniformRes = "ResolutionUniform";
}
	...
	, m_pPreset(nullptr)
{
	...
	m_pResUnified->setIcon(DrxIcon("icons:General/Uniform.ico"));
	...
	{
		QVariant data(i);
		STxt text = std::to_string(i32(pow(2, i)));
	...
	m_pComboYRes->signalCurrentIndexChanged.Connect([this](i32 index)
	{
		ResolutionChanged(m_pComboYRes, index);
	});
	m_pComboXRes->signalCurrentIndexChanged.Connect([this](i32 index)
	{
...
void CSubstancePresetEditor::ResolutionChanged(QMenuComboBox* sender, i32 index)
{
	if (m_pResUnified->isChecked())
	{
