...
namespace EditorSubstance
{
	...
	void OutputsWidget::SetResolution(const Vec2i& resolution)
	{
		...
		m_pComboXRes->SetText(std::to_string(i32(pow(2, resolution.x))).c_str());
		m_pComboYRes->SetText(std::to_string(i32(pow(2, resolution.y))).c_str());
	...
	void OutputsWidget::ResolutionChanged(QMenuComboBox* sender, i32 index)
	{
		if (!m_pResUnified->isChecked())
			return;
