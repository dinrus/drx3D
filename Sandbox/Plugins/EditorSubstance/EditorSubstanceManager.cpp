...
#include "DrxSerialization/STL.h"
#include "IEditor.h"
...
{
public:
	virtual bool AcceptFile(tukk szFilename, EChangeType changeType) override
	{
	...
	virtual bool ProcessFile(tukk szFilename, EChangeType changeType) override
	{
...
std::set<string> CManager::GetTexturePresetsForFile(const string& mask)
{
	...
	for (s = m_FilteredPresets.begin(); s != m_FilteredPresets.end(); ++s)
	{
		...
		if (DrxStringUtils::MatchWildcard(filename, suffix))
		{
