...
namespace EditorSubstance
{
	namespace Renderers
	{
		...
		u32 CCompressedRenderer::GetOutputCompressedFormat(const EditorSubstance::EPixelFormat& format, bool useSRGB)
		{
			u32 res = 0;
			switch (format)
			{
		...
		void CCompressedRenderer::AttachOutputUserData(const ISubstancePreset* preset, const SSubstanceOutput& output, SSubstanceRenderData& renderData, i32k& flags)
		{
		...
		void CCompressedRenderer::UpdateTexture(SubstanceAir::RenderResult* result, SGeneratedOutputData* data)
		{
			...
			i32 FIM_textureFlags = 0;
			i32 FT_textureFlags = 0;
			...
			i32 formatFlag = texture.pixelFormat & 0x1F;
			...
			u32 targetFlags = pTargetTexture->GetFlags();
			...
			if (sd.m_nFlags & FIM_NORMALMAP)
			{
				...
				{
					u8 *blocks = sd.m_pData[0];
					u8 *end = sd.m_pData[0] + TextureDataSize(sd.m_nWidth, sd.m_nHeight, sd.m_nDepth, sd.m_nMips, sd.m_eFormat);
					while (blocks < end)
					{
		...
		u32 CCompressedRenderer::TextureDataSize(u32 nWidth, u32 nHeight, u32 nDepth, u32 nMips, const ETEX_Format eTF) const
		{
			if (eTF == eTF_Unknown)
				return 0;
			...
			u32k nBytesPerBlock = bIsBlockCompressed ? CImageExtensionHelper::BytesPerBlock(eTF) : CImageExtensionHelper::BitsPerPixel(eTF) / 8;
			u32 nSize = 0;
