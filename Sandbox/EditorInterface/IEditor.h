...
enum EEditorNotifyEvent
{
	...
	eNotify_OnUpdateSequencer,             // Sent when editor needs to update the DrxMannequin sequencer view.
	eNotify_OnUpdateSequencerKeys,         // Sent when editor needs to update keys in the DrxMannequin track view.
	eNotify_OnUpdateSequencerKeySelection, // Sent when DrxMannequin sequencer view changes selection of keys.
...
struct IEditor
{
	...
	virtual void             ExecuteCommand(tukk sCommand, ...) = 0;
	...
	virtual IPane* CreateDockable(tukk className) = 0;
	...
	virtual CDrxEditDoc* GetDocument() const = 0;
	...
	virtual void             StartObjectCreation(tukk type, tukk file = nullptr) = 0;
	...
	virtual void              SetConsoleVar(tukk var, float value) = 0;
	virtual float             GetConsoleVar(tukk var) = 0;
	...
	virtual void AddNativeHandler(uintptr_t id, std::function<bool(uk message, long* retval)> callback) = 0;
	...
	virtual void RegisterURIListener(IUriEventListener* pListener, tukk szSchema) = 0;
	virtual void UnregisterURIListener(IUriEventListener* pListener, tukk szSchema) = 0;
	//~IUriEventListener interface

	virtual void  RequestScriptReload(const EReloadScriptsType& kind) = 0;

	virtual CWnd* OpenView(tukk sViewClassName) = 0;
	virtual CWnd* FindView(tukk sViewClassName) = 0;
	...
	virtual void              UpdateViews(i32 flags = 0xFFFFFFFF, AABB* updateRegion = NULL) = 0;
	...
	virtual void             RegisterDeprecatedPropertyEditor(i32 propertyType, std::function<bool(const string&, string&)>& handler) = 0;
	virtual bool             EditDeprecatedProperty(i32 propertyType, const string& oldValue, string& newValueOut) = 0;
	virtual bool             CanEditDeprecatedProperty(i32 propertyType) = 0;
...
struct IEditorNotifyListener
{
	...
	{
		if (m_bIsRegistered)
			DrxFatalError("Destroying registered IEditorNotifyListener");
	}
