...
public:
	...
	inline CBaseLibrary* FindLibrary(tukk  libraryName)
	{
		return FindLibrary(CString(libraryName));
	}
	inline CBaseLibrary* NewLibrary(tukk  libraryName)
	{
		return NewLibrary(CString(libraryName));
	}
	inline void          SelectLibrary(tukk  library, bool bForceSelect = false)
	{
		SelectLibrary(CString(library), bForceSelect);
	}
	inline bool			  SetItemName(CBaseLibraryItem* item, tukk groupName, tukk itemName)
	{
...
protected:
	...
	i32  GetComboBoxIndex(CBaseLibrary* pLibrary);
