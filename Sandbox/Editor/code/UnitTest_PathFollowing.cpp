...
u32 rcr_key_Z = 0, flip_Z = 2;
u32 rcr_key_H = 0, flip_H = 2;
u32 rcr_key_J = 0, flip_J = 1;
u32 rcr_key_T = 0, flip_T = 0;
...
extern i32 g_ShiftTarget;
extern i32 g_ShiftSource;
extern tuk g_NewName;

u32k LIMIT = 8 * 5;
Vec3 g_Path[LIMIT];

SPlayerControlMotionParams CModelViewportCE::EntityFollowing(u32& ypos)
{
...
f32 CModelViewportCE::PathCreation(f32 fDesiredSpeed)
{
	...
	/*	for (u32 i=0; i<LIMIT; i=i+5)
	   ...
	   u32 i0=i+0; if (i0>=LIMIT) i0-=LIMIT;
	   u32 i1=i+1; if (i1>=LIMIT) i1-=LIMIT;
	   u32 i2=i+2; if (i2>=LIMIT) i2-=LIMIT;//this is the first point on line-segment
	   u32 i3=i+3; if (i3>=LIMIT) i3-=LIMIT;
	   u32 i4=i+4; if (i4>=LIMIT) i4-=LIMIT;
	   ...
	   u32 t = u32(fabsf(radiants[i2])*0xff);
	   if (t>0xff)	t=0xff;
	   u8 c=0xff-t;
...
void CModelViewportCE::PathFollowing(const SPlayerControlMotionParams& mp, f32 fDesiredSpeed)
{
	...
	//---  camera control
	//------------------------------------------------------------------------
	u32 AttachedCamera = m_pCharPanel_Animation->GetAttachedCamera();
	if (AttachedCamera)
	{
		...
		tukk RootName = rIDefaultSkeleton.GetJointNameByID(0);
