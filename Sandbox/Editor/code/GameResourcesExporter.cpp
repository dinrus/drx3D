...
void CGameResourcesExporter::Save(const string& outputDirectory)
{
	...
	i32 numFiles = m_files.size();

	CLogFile::WriteLine("===========================================================================");
	DrxLog("Exporting Level %s resources, %d files", (tukk)GetIEditorImpl()->GetGameEngine()->GetLevelName(), numFiles);
	CLogFile::WriteLine("===========================================================================");
...
void CGameResourcesExporter::ExportPerLayerResourceList()
{
	...
	for (size_t i = 0; i < layers.size(); ++i)
	{
		...
		string listFilename;
		listFilename.Format("Layer_%s.txt", (tukk)pLayer->GetName());
