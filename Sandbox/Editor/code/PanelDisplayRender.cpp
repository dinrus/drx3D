...
void SerializeCVarBool(Serialization::IArchive& ar, tukk cvar, tukk display, i32 valueOn = 1, i32 valueOff = 0)
{
...
void SerializeCVarFloat(Serialization::IArchive& ar, tukk cvar, tukk display, float limitMin, float limitMax)
{
	ICVar* pVar = gEnv->pConsole->GetCVar(cvar);
	if (pVar)
	{
...
void SerializeCVarEnum(Serialization::IArchive& ar, tukk cvar, tukk display)
{
	ICVar* pVar = gEnv->pConsole->GetCVar(cvar);
	if (pVar)
	{
...
void SetCVarBool(tukk cvar, bool bEnable)
{
	ICVar* pVar = gEnv->pConsole->GetCVar(cvar);
	if (pVar)
	{
...
void SerializeMaskValue(Serialization::IArchive& ar, u32& value, u32 mask, tukk name)
{
...
void CPanelDisplayRender::Serialize(Serialization::IArchive& ar)
{
	...
	if (ar.openBlock("objtypes", "Hide by Category"))
	{
		...
		u32 m_mask = gViewportDebugPreferences.GetObjectHideMask();
		u32 prevMask = m_mask;
	...
	if (ar.openBlock("categories", "Render Types"))
	{
		...
		for (i32 i = 0; i < _countof(render_category_cvars) / 2; i++)
		{
			tukk cvar = render_category_cvars[i * 2];
			tukk display = render_category_cvars[i * 2 + 1];
	...
	{
		u32 flags = gViewportDebugPreferences.GetDebugFlags();
...
//////////////////////////////////////////////////////////////////////////
void CPanelDisplayRender::RegisterChangeCallback(tukk szVariableName, ConsoleVarFunc fnCallbackFunction)
{
	...
	if (pVar)
	{
		...
		{
			DRX_ASSERT_MESSAGE(0, "Attempting to register callback twice");
		}
	}
}

//////////////////////////////////////////////////////////////////////////
void CPanelDisplayRender::UnregisterChangeCallback(tukk szVariableName)
{
	ICVar* pVar = gEnv->pConsole->GetCVar(szVariableName);
	if (pVar)
	{
		if (m_varCallbackMap.find(pVar) != m_varCallbackMap.end())
		{
			...
			{
				DRX_ASSERT_MESSAGE(0, "Attempting to unregister callback twice");
			}
		}
		else
		{
			DRX_ASSERT_MESSAGE(0, "Attempting to unregister callback that doesn't exist");
		}
	}
}
...
void CPanelDisplayRender::SetObjectHideMask(u32 mask)
{
