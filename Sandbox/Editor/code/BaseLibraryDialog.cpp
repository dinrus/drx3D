...
//////////////////////////////////////////////////////////////////////////
void CBaseLibraryDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CBaseLibraryDialog::ReloadLibs()
{
	...
	for (i32 i = 0; i < m_pItemManager->GetLibraryCount(); i++)
	{
		...
		if (m_libraryCtrl)
		{
			...
			{
				i32 nWidth = (m_libraryCtrl.GetDC()->GetTextExtent(library)).cx + ::GetSystemMetrics(SM_CXVSCROLL) + 2 * ::GetSystemMetrics(SM_CXEDGE);
...
void CBaseLibraryDialog::OnReloadLib()
{
	...
	{
		string str;
		str.Format("Layer %s was modified.\nReloading layer will discard all modifications to this library!", (tukk)libname);
...
void CBaseLibraryDialog::LoadLibrary()
{
	...
	if (CFileUtil::SelectMultipleFiles(EFILE_TYPE_XML, files, "", PathUtil::GamePathToDrxPakPath(m_pItemManager->GetLibsPath().GetString()).c_str()))
	{
		i32 selectedFiles = files.size();
		if (selectedFiles > 0)
		{
			GetIEditorImpl()->GetIUndoManager()->Suspend();
			IDataBaseLibrary* matLib = nullptr;

			for (i32 i = 0; i < selectedFiles; ++i)
			{
