...
#include <DrxAudio/Dialog/IDialogSystem.h>
#include <DrxDynamicResponseSystem/IDynamicResponseSystem.h>
#include <DrxPhysics/IDeferredCollisionEvent.h>
...
#include <drx3D/Sys/IDrxPluginManager.h>
...
#include <../DrxAction/IGameRulesSystem.h>
#include <../DrxAction/ILevelSystem.h>

// Implementation of System Callback structure.
struct SSysUserCallback : public ISystemUserCallback
{
	SSysUserCallback(IInitializeUIInfo* logo) { m_pLogo = logo; };
	...
	struct SEditorSuspendUpdateScopeHelper
	{
		...
		{
			CDrxEditApp::GetInstance()->SuspendUpdate();
		}

		~SEditorSuspendUpdateScopeHelper()
		{
			CDrxEditApp::GetInstance()->ResumeUpdate();
		}
	};

	virtual EQuestionResult ShowMessage(tukk text, tukk caption, EMessageBox uType)
	{
...
struct SInGameEntitySystemListener : public IEntitySystemSink
{
	...
	~SInGameEntitySystemListener()
	{
		...
		for (std::set<i32>::iterator it = m_entities.begin(); it != m_entities.end(); ++it)
		{
	...
	{
		DRX_ASSERT_MESSAGE(false, "Editor should not be receiving entity reused events from IEntitySystemSink, investigate this.");
	}

private:
	// Ids of all spawned entities.
	std::set<i32> m_entities;
};
...
// Timur.
// This is FarDrx.exe authentication function, this code is not for public release!!
static void GameSystemAuthCheckFunction(uk data)
{
...
#define TEA_DECODE(src, trg, len, key) {                                                                                      \
  ...
  while (nlen--) {                                                                                                            \
	...
	i32 key1[4] = { 1178362782, 223786232, 371615531, 90884141 };
	TEA_DECODE((u32*)data, (u32*)data, 32, (u32*)key1);
	i32 key2[4] = { 89158165, 1389745433, 971685123, 785741042 };
	TEA_ENCODE((u32*)data, (u32*)data, 32, (u32*)key2);
}

static void KeepEditorActiveChanged(ICVar* pKeepEditorActive)
{
	i32k cvarKeepEditorActive = pKeepEditorActive->GetIVal();
	CDrxEditApp::GetInstance()->KeepEditorActive(cvarKeepEditorActive);
}
...
static i32 ed_killmemory_size;
static i32 ed_indexfiles;

void KillMemory(IConsoleCmdArgs* /* pArgs */)
{
	while (true)
	{
		i32k kLimit = 10000000;
		i32 size;
...
{
	m_pSystemUserCallback = new SSysUserCallback(logo);
	...
	if (sInCmdLine)
	{
		drx_strcpy(startupParams.szSystemCmdLine, sInCmdLine);
	...
	if (!DrxInitializeEngine(startupParams, true))
	{
		return false;
	}
...
void CGameEngine::InitAdditionalEngineComponents()
{
	...
	if (pPrefabManager)
	{
		...
		if (!bResult)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "CGameEngine::InitGame: Failed to init prefab events");
		}
	}
...
void CGameEngine::NotifyGameModeChange(bool bGameMode)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed configuring net context");
	}
}
  ...
  bool bReleaseResources)
{
	...
	m_bLevelLoaded = false;
	m_missionName = mission;
	DrxLog("Loading map '%s' into engine...", (tukk)m_levelPath);
	...
	{
		//DrxWarning( VALIDATOR_MODULE_GAME,VALIDATOR_WARNING,"Level Pack File %s Not Found",sPak1.c_str() );
	}
...
bool CGameEngine::LoadAI(const string& levelName, const string& missionName)
{
	...
	DrxLog("Loading AI data %s, %s", (tukk)levelName, (tukk)missionName);
	gEnv->pAISystem->FlushSystemNavigation();
	...
	DrxLog("Finished Loading AI data in %6.3f secs", m_pISystem->GetITimer()->GetAsyncCurTime() - fStartTime);
...
void CGameEngine::LoadAINavigationData()
{
	...
	DrxLog("Loading AI navigation data for Level:%s Mission:%s", (tukk)m_levelPath, (tukk)m_missionName);
	gEnv->pAISystem->FlushSystemNavigation();
	gEnv->pAISystem->LoadNavigationData(m_levelPath, m_missionName);
}

void CGameEngine::GenerateAiAll(u32 flags)
{
	if (!gEnv->pAISystem)
		return;
	...
	wait.Step(95);
	DrxLog("Validating SmartObjects");
	...
	// quick temp code to validate smart objects on level export
	{
		...
		DRX_ASSERT_MESSAGE(pClass != nullptr, "SmartObject class desc not found");
		...
		for (it = objects.begin(); it != itEnd; ++it)
		{
			...
			if (pBaseObject->GetClassDesc() == pClass)
			{
				...
				if (!gEnv->pAISystem->GetSmartObjectManager()->ValidateSOClassTemplate(pSOEntity->GetIEntity()))
				{
					...
					DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "SmartObject '%s' at (%.2f, %.2f, %.2f) is invalid!",
					           (tukk)pSOEntity->GetName(), pos.x, pos.y, pos.z);
				}
			}
		}
	}
...
void CGameEngine::GenerateAICoverSurfaces()
{
	...
	u32 surfaceObjectCount = surfaceObjects.size();
	...
	char fileName[1024];
	drx_sprintf(fileName, "%s\\cover%s.bai", (LPCTSTR)m_levelPath, (LPCTSTR)m_missionName);
...
void CGameEngine::SetPlayerEquipPack(tukk sEqipPackName)
{
...
void CGameEngine::SetCurrentMOD(tukk sMod)
{
	m_MOD = sMod;
}

tukk CGameEngine::GetCurrentMOD() const
{
	return m_MOD;
}
...
void CGameEngine::Update()
{
	...
	else
	{
		...
		Drx::IPluginManager* const pPluginManager = gEnv->pSystem->GetIPluginManager();
	...
	case eNotify_OnBeginSceneOpen:
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed spawning player or waiting for InGame");
			}
		}
		break;
