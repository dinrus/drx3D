...
#include <DrxAction/IMaterialEffects.h> // for footstep sound preview
...
void CModelViewportCE::AnimPreview_UnitTest(ICharacterInstance* pInstanceBase, const SRendParams& rRP, const SRenderingPassInfo& passInfo)
{
	...
	if (m_pCharPanel_Animation && m_pAnimationBrowserPanel)
	{
		...
		if (pSelectedSkel)
		{
			...
			i32 AnimEventCallback(ICharacterInstance * pInstance, uk pPlayer);
			pISkeletonAnim->SetEventCallback(AnimEventCallback, this);

			u32 IsParametricSampler = 0;
			for (i32 layer = 0; layer < ISkeletonAnim::LayerCount; ++layer)
			{
				u32 nAnimsInLayer = pISkeletonAnim->GetNumAnimsInFIFO(layer);
				if (nAnimsInLayer)
				{
			...
			u32 nAnimLayer0 = pISkeletonAnim->GetNumAnimsInFIFO(0);
			if (nAnimLayer0)
			{
				static u32 rcr_key_R = 0;
				rcr_key_R <<= 1;
				...
				if (mv_showStartLocation)
				{
					...
					tukk aname = pIAnimationSet->GetNameByAnimID(animation.GetAnimationId());
			...
			//------------------------------------------------------------------
			u32 nLayer = m_pCharPanel_Animation->GetLayer();
			u32 numAnimsLayer = pISkeletonAnim->GetNumAnimsInFIFO(nLayer);
			...
			for (u32 i = 0; i < ISkeletonAnim::LayerCount; i++)
			{
			...
			if (pIPoseBlenderAim)
			{
				...
				//		g_ypos+=10;
				u32 nAimIKLayer = (m_pCharPanel_Animation ? m_pCharPanel_Animation->GetAimIKLayerValue() : ISkeletonAnim::LayerCount);
				u32 nAimIK = m_pCharPanel_Animation->GetAimIK();
			...
			if (pIPoseBlenderLook)
			{
				...
				//	g_ypos+=10;
				u32 nLookIKLayer = (m_pCharPanel_Animation ? m_pCharPanel_Animation->GetLookIKLayerValue() : ISkeletonAnim::LayerCount);
				i32 LookIK = m_pCharPanel_Animation->GetLookIK();
	...
	i32k layerId = GetCurrentActiveAnimationLayer();
	...
	u32 visible = GetCamera().IsAABBVisible_E(taabb);
	if (visible)
	{
	...
	//-------------------------------------------------
	u32 numEntries = m_arrAnimatedCharacterPath.size();
	for (i32 i = (numEntries - 2); i > -1; i--)
	...
	u32 GroundAlign = (m_pCharPanel_Animation ? m_pCharPanel_Animation->GetGroundAlign() : 1);
	if (GroundAlign == 0)
	...
	{
		i32 mt = m_pCharPanel_Animation->GetUseMorphTargets();
	...
	if (m_pMotionAdaptorDlg)
	{
		for (u32 i = 0; i < ISkeletonAnim::LayerCount; i++)
		{
			u32 nAnimsInQueue = pInstanceBase->GetISkeletonAnim()->GetNumAnimsInFIFO(i);
			if (nAnimsInQueue)
			{
		...
		u32 numJoints = m_pMotionAdaptorDlg->m_dlgMain.GetViconJointNum();
		u32 numFrames = 0;
...
void CModelViewportCE::SkelExtensionsTest(ICharacterInstance* pInstanceBase)
{
	...
	static u32 lshift1 = 0;
	static u32 lshift2 = 0;
	static u32 lshift4 = 0;
	static u32 lshift8 = 0;
	static u32 shift = 0;
	...
	u32 chanage1 = (shift & 1) != (lshift1 & 1);
	if (chanage1)
	{
		lshift1 = shift;
	...
	u32 chanage2 = (shift & 2) != (lshift2 & 2);
	if (chanage2)
	{
		lshift2 = shift;
	...
	u32 chanage4 = (shift & 4) != (lshift4 & 4);
	if (chanage4)
	{
		lshift4 = shift;
	...
	u32 chanage8 = (shift & 8) != (lshift8 & 8);
	if (chanage8)
	{
		lshift8 = shift;
		...
		for (u32 a = 0; a < numAnimsLayer; a++)
		{
			...
			for (i32 i = 0; i < animCount; i++)
			{
				...
				for (u32 d = 0; d < lmg.m_numDimensions; d++)
				{
					...
					{
						u32 locked = lmg.m_MotionParameterFlags[d] & CA_Dim_LockedParameter;
						u32 init = lmg.m_MotionParameterFlags[d] & CA_Dim_Initialized;
...
void CModelViewportCE::AnimEventProcessing(ICharacterInstance* pInstance)
{
	...
	//u32 s0 = stricmp(g_LastAnimEvent.m_EventName, "sound") == 0;
	//u32 s1 = stricmp(g_LastAnimEvent.m_EventName, "sound_tp") == 0;
	...
	if (g_LastAnimEvent.m_EventName)
	{
		...
		i32 nWeaponIdx = rIDefaultSkeleton.GetJointIDByName("weapon_bone");
		i32 nSpine2Idx = rIDefaultSkeleton.GetJointIDByName("Bip01 Spine2");
		if (nWeaponIdx < 0)
			return;
		if (nSpine2Idx < 0)
			return;
...
//---------------------------------------------------------------------
//---------------------------------------------------------------------
u32 CModelViewportCE::UseHumanLimbIK(ICharacterInstance* pInstance, tukk strLimb)
{
	...
	i32 STRG = CheckVirtualKey(VK_CONTROL);
	i32 np1 = CheckVirtualKey(VK_NUMPAD1);
	i32 np2 = CheckVirtualKey(VK_NUMPAD2);
	i32 np3 = CheckVirtualKey(VK_NUMPAD3);
	i32 np4 = CheckVirtualKey(VK_NUMPAD4);
	i32 np5 = CheckVirtualKey(VK_NUMPAD5);
	i32 np6 = CheckVirtualKey(VK_NUMPAD6);
	i32 np7 = CheckVirtualKey(VK_NUMPAD7);
	i32 np8 = CheckVirtualKey(VK_NUMPAD8);
	i32 np9 = CheckVirtualKey(VK_NUMPAD9);
