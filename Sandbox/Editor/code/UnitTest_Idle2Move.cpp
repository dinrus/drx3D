...
void CModelViewportCE::Idle2Move_UnitTest(ICharacterInstance* pInstance, const SRendParams& rRP, const SRenderingPassInfo& passInfo)
{
	...
	i32 PostProcessCallback(ICharacterInstance * pInstance, uk pPlayer);
...
f32 CModelViewportCE::Idle2Move(IAnimationSet* pIAnimationSet)
{
	...
	u32 numAnimsLayer0 = pISkeletonAnim->GetNumAnimsInFIFO(0);
	if (numAnimsLayer0)
	...
	u32 isActivated = animationT.IsActivated();

	u32 nTransPossible = 0;
	...
	//	if (AttachedCamera)
	{
		...
		{
			u32 AimIKState = m_pCharPanel_Animation->GetAimIK();
			u32 AimIKLayer = m_pCharPanel_Animation->GetAimIKLayerValue();
