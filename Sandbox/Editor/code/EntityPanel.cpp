...
CEntityPanel::CEntityPanel(i32 idd, CWnd* pParent /*=NULL*/)
...
#pragma message("TODO")
	...
	/*if (m_entity)
	   ...
	   if (bundle.GetCount() > 0)
	   {
	    ...
	    for (u32 i = 0; i < bundle.GetCount(); ++i)
	    {
	    ...
	    CPoint p;
	    ::GetCursorPos(&p);
	    i32 chosen = menu.TrackPopupMenuEx( TPM_RETURNCMD|TPM_LEFTBUTTON|TPM_TOPALIGN|TPM_LEFTALIGN, p.x, p.y, this, NULL )-1;
	    if (chosen >= 0)
	    {
...
void CEntityPanel::OnBnClickedRemoveFlowGraph()
{
	if (m_entity)
	{
		if (m_entity->GetFlowGraph())
		{
			...
			CString str;
			str.Format("Remove Flow Graph for Entity %s?", (tukk)m_entity->GetName());
...
void CEntityEventsPanel::ReloadMethods()
{
	...
	{
		for (i32 i = 0; i < script->GetMethodCount(); i++)
		{
...
void CEntityEventsPanel::ReloadEvents()
{
	...
	for (i = 0; i < script->GetEventCount(); i++)
	{
		...
		for (i32 j = 0; j < m_entity->GetEventTargetCount(); j++)
		{
			CString targetName;
...
void CEntityEventsPanel::OnBnAddMission()
{
	...
	if (!m_currentSourceEvent.IsEmpty())
	{
		...
		CMenu menu;
		menu.CreatePopupMenu();
		for (i32 i = 0; i < script->GetEventCount(); i++)
		{
