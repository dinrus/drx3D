...
BOOL CTerrainDialog::OnInitDialog()
{
	...
	//////////////////////////////////////////////////////////////////////////
	char szCaption[128];
	drx_sprintf(szCaption, "Heightmap %" PRIu64 "x%" PRIu64, m_pHeightmap->GetWidth(), m_pHeightmap->GetHeight());
...
void CTerrainDialog::OnTerrainGenerate()
{
	...
	else
	{
		...
		cDialog.m_sldFrequency = (i32) (7.0f * 10); // Feature Size
		cDialog.m_sldFade = (i32) (0.46f * 10);     // Bumpiness
		cDialog.m_sldPasses = 8;                    // Detail (Passes)
		cDialog.m_sldRandomBase = 1;                // Variation
		cDialog.m_sldBlur = 0;
		cDialog.m_sldCover = 0;
		cDialog.m_sldSharpness = (i32) (0.999f * 1000);
		cDialog.m_sldFrequencyStep = (i32) (2.0f * 10);
	}
...
void CTerrainDialog::OnHeightmapShowLargePreview()
{
	...
	for (UINT y = 0; y < iHeight; ++y)
	{
		for (UINT x = 0; x < iWidth; ++x)
		{
			...
			u8 iColor = (u8)__min(pHeightmap[x + y * iWidth] / fMaxHeight * 255.0f, 255.0f);
...
void CTerrainDialog::OnExportTerrainArea()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "No Terrain Area is selected. Use \"Select Terrain\" tool to select the terrain area.");
...
void CTerrainDialog::OnResizeTerrain()
{
	...
	i32 resolution = result.heightmap.resolution;
