...
protected:
	tukk GetDescription() { return "Select database library."; };
...
static i32k LIBRARY_CB_WIDTH(150);
...
void CDatabaseFrameWnd::ReloadLibs()
{
	...
	bool bFound = false;
	for (i32 i = 0; i < m_pItemManager->GetLibraryCount(); i++)
	{
		...
		if (m_LibraryListComboBox)
		{
			i32 index = m_LibraryListComboBox.AddString(library);
			m_LibraryListComboBox.SetItemDataPtr(index, (CBaseLibrary*)m_pItemManager->GetLibrary(i));
			if (m_LibraryListComboBox.GetDC())
			{
				i32 nWidth = (m_LibraryListComboBox.GetDC()->GetTextExtent(library)).cx + ::GetSystemMetrics(SM_CXVSCROLL) + 2 * ::GetSystemMetrics(SM_CXEDGE);
...
i32 CDatabaseFrameWnd::GetComboBoxIndex(CBaseLibrary* pLibrary)
{
	i32 nCount = m_LibraryListComboBox.GetCount();
...
void CDatabaseFrameWnd::DeleteAllTreeItems()
{
	...
	for (i32 i = 0, iSize(allItems.size()); i < iSize; ++i)
	{
...
void CDatabaseFrameWnd::SortItems(SortRecursionType recursionType)
{
	struct Recurser
	{
		static void Sort(CTreeCtrl& treeCtrl, HTREEITEM hItem, i32 nLevels)
		{
			if (hItem)
			{
	...
	if ((i32)recursionType > 0)
		Recurser::Sort(GetTreeCtrl(), GetRootItem(), (i32)recursionType);
}
...
void CDatabaseFrameWnd::OnRemoveLibrary()
{
	...
	{
		CString ask;
		ask.Format("Save changes to the library %s?", (tukk)library);
...
void CDatabaseFrameWnd::OnRemoveItem()
{
	...
	if (m_cpoSelectedLibraryItems.empty())
	{
		if (m_pCurrentItem)
		{
			...
			CString str;
			str.Format(_T("Delete %s?"), (tukk)m_pCurrentItem->GetName());
...
void CDatabaseFrameWnd::LoadLibrary()
{
	...
	CString file;
	if (CFileUtil::SelectSingleFile(EFILE_TYPE_XML, file, "", PathUtil::GamePathToDrxPakPath(m_pItemManager->GetLibsPath().GetString()).c_str()))
	{
		if (!file.IsEmpty())
		{
...
bool CDatabaseFrameWnd::SetItemName(CBaseLibraryItem* item, const CString& groupName, const CString& itemName)
{
	...
	if (pOtherItem && pOtherItem != item)
	{
		...
		Warning("Duplicate Item Name %s", (tukk)name);
		return false;
	}
	else
	{
...
{
	for (i32 i = 0, iItemCount(m_pLibrary->GetItemCount()); i < iItemCount; ++i)
	{
...
{
	i32 nCurSel = m_LibraryListComboBox.GetCurSel();
