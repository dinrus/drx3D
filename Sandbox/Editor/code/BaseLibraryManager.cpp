...
//////////////////////////////////////////////////////////////////////////
IDataBaseItem* CBaseLibraryManager::FindItem(DrxGUID guid) const
{
	if (!m_bUniqGuidMap)
		return nullptr;
...
//////////////////////////////////////////////////////////////////////////
IDataBaseItem* CBaseLibraryManager::FindItemByName(tukk fullItemName)
{
	return FindItemByName(string(fullItemName));
}

//////////////////////////////////////////////////////////////////////////
IDataBaseItem* CBaseLibraryManager::LoadItemByName(tukk fullItemName)
{
...
IDataBaseLibrary* CBaseLibraryManager::LoadLibrary(const string& inFilename, bool bReload)
{
	...
	for (i32 i = 0; i < m_libs.size(); i++)
	{
		...
		{
			Error(_T("Loading Duplicate Library: %s"), (tukk)filename);
			return 0;
		}
	}
	...
	{
		Error(_T("Failed to Load Item Library: %s"), (tukk)filename);
		return 0;
	}
	if (FindLibrary(pLib->GetName()) != 0)
	{
		Error(_T("Loading Duplicate Library: %s"), (tukk)pLib->GetName());
		return 0;
	}
...
//////////////////////////////////////////////////////////////////////////
i32 CBaseLibraryManager::GetModifiedLibraryCount() const
{
	i32 count = 0;
	for (i32 i = 0; i < m_libs.size(); i++)
	{
	...
	for (i32 i = 0; i < m_libs.size(); i++)
	{
		if (stricmp(library, m_libs[i]->GetName()) == 0)
		{
			...
			if (!pLibrary->IsLevelLibrary())
			{
				...
				for (i32 j = 0; j < pLibrary->GetItemCount(); j++)
				{
...
//////////////////////////////////////////////////////////////////////////
IDataBaseLibrary* CBaseLibraryManager::GetLibrary(i32 index) const
{
...
void CBaseLibraryManager::Serialize(XmlNodeRef& node, bool bLoading)
{
	...
	if (bLoading)
	{
		...
		if (libs)
		{
			for (i32 i = 0; i < libs->getChildCount(); i++)
			{
...
string CBaseLibraryManager::MakeUniqItemName(const string& srcName)
{
	...
	{
		tukk name = pItem->GetName();
		if (strncmp(name, tpName, len) == 0)
		{
			i32 n = atoi(name + len) + 1;
			num = std::max(num, n);
		}
	}
	pEnum->Release();
	...
	{
		string str;
		str.Format("%s%d", (tukk)typeName, num);
		return str;
	}
	else
		return typeName;
...
//////////////////////////////////////////////////////////////////////////
void CBaseLibraryManager::RegisterItem(CBaseLibraryItem* pItem, DrxGUID newGuid)
{
	assert(pItem);
	...
	{
		bool bNewItem = true;
		DrxGUID oldGuid = pItem->GetGUID();
		if (oldGuid != DrxGUID::Null())
		{
			bNewItem = false;
			m_itemsGuidMap.erase(oldGuid);
			NotifyItemEvent(pItem, EDB_ITEM_EVENT_DELETE);
		}
		if (newGuid == DrxGUID::Null())
			return;
...
void CBaseLibraryManager::ReportDuplicateItem(CBaseLibraryItem* pItem, CBaseLibraryItem* pOldItem)
{
	...
	DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Item %s with duplicate GUID to loaded item %s ignored", (tukk)pItem->GetFullName(), (tukk)pOldItem->GetFullName());
}
