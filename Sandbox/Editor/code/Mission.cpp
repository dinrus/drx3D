...
namespace
{
tukk kTimeOfDayFile = "TimeOfDay.xml";
tukk kTimeOfDayRoot = "TimeOfDay";
tukk kEnvironmentFile = "Environment.xml";
tukk kEnvironmentRoot = "Environment";
};

//////////////////////////////////////////////////////////////////////////
CMission::CMission(CDrxEditDoc* doc)
{
	m_doc = doc;
...
void CMission::Serialize(CXmlArchive& ar, bool bParts)
{
	if (ar.bLoading)
	{
		...
		if (weapons)
		{
			...
			{
				CString weaponName;
				for (i32 i = 0; i < usedWeapons->getChildCount(); i++)
				{
	...
	else
	{
		...
		CString timeStr;
		i32 nHour = floor(m_time);
		i32 nMins = (m_time - floor(m_time)) * 60.0f;
		...
		for (i32 i = 0; i < m_usedWeapons.size(); i++)
		{
...
void CMission::Export(XmlNodeRef& root, XmlNodeRef& objectsNode)
{
	...
	//////////////////////////////////////////////////////////////////////////
	char path[1024];
	drx_strcpy(path, m_doc->GetPathName());
...
void CMission::ExportAnimations(XmlNodeRef& root)
{
	...
	for (i32 i = 0; i < AnimNode->getChildCount(); i++)
	{
