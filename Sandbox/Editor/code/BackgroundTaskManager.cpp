...
{
	{
		DrxAutoLock<DrxMutex> lock(m_lock);
...
void CTaskManager::Start(u32k threadCount /*=kDefaultThreadCount*/)
{
	m_bStop = false;
	...
	if (m_pThreads.empty())
	{
		// Always create one IO thread
		{
			...
			{
				DrxFatalError("Error spawning \"BackgroundTask_IO\" thread.");
			}
			m_pThreads.push_back(pThread);
		}

		// We also need at least one generic thread
		u32k numGenericThreads = max<u32>(threadCount, 1);
		for (u32 i = 0; i < numGenericThreads; ++i)
		{
			...
			{
				DrxFatalError("Error spawning \"BackgroundTask_%u\" thread.", i);
			}
			m_pThreads.push_back(pThread);
		}
	}
...
{
	DrxAutoLock<DrxMutex> lock(m_tasksLock);

	if (!m_scheduledTasks.empty())
	{
		u32k time = GetTickCount();
		while (!m_scheduledTasks.empty())
		{
			i32k delta = (i32)(time - m_scheduledTasks[0].time);
			if (delta > 0)
			{
...
void CTaskManager::Stop()
{
	if (!m_bStop)
	{
		...
		for (u32 i = 0; i < eTaskThreadMask_COUNT; ++i)
		{
			m_pendingTasks[i].Clear();
		}

		// kick all the threads to allow them to quit
		for (u32 j = 0; j < m_pThreads.size(); ++j)
		{
			for (u32 i = 0; i < eTaskThreadMask_COUNT; ++i)
			{
...
void CTaskManager::AddListener(IBackgroundTaskManagerListener* pListener, tukk name)
{
...
void CTaskManager::AddTask(IBackgroundTask* pTask, ETaskPriority priority, ETaskThreadMask threadMask)
{
	...
	STaskHandle handle;
	handle.id = DrxInterlockedIncrement(&m_nextTaskID);
...
void CTaskManager::ScheduleTask(IBackgroundTask* pTask, ETaskPriority priority, i32 delayMilliseconds, ETaskThreadMask threadMask)
{
	...
	task.handle.id = DrxInterlockedIncrement(&m_nextTaskID);
...
void CTaskManager::AddCompletedTask(const STaskHandle& handle, ETaskResult resultState)
{
	...
	DRX_ASSERT(handle.pTask->GetState() == eTaskState_Working);
	DRX_ASSERT(resultState != eTaskResult_Resume);
		...
		if (NULL != handle.pTask)
		{
			...
			else if (handle.state == eTaskResult_Failed)
			{
				if (description && description[0] != '\0' && !handle.pTask->FailReported())
				{
					...
					tukk errorMessage = handle.pTask->ErrorMessage();
