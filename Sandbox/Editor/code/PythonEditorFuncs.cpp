...
//////////////////////////////////////////////////////////////////////////
void PySetCVar(tukk pName, pSPyWrappedProperty pValue)
{
	ICVar* pCVar = GetIEditorImpl()->GetSystem()->GetIConsole()->GetCVar(pName);
	if (!pCVar)
	{
	...
	DrxLog("PySetCVar: %s set to %s", pName, pCVar->GetString());
}
...
//////////////////////////////////////////////////////////////////////////
tukk PyNewObject(tukk typeName, tukk fileName, tukk name, float x, float y, float z)
{
...
pPyGameObject PyCreateObject(tukk typeName, tukk fileName, tukk name, float x, float y, float z)
{
...
//////////////////////////////////////////////////////////////////////////
tukk PyNewObjectAtCursor(tukk typeName, tukk fileName, tukk name)
{
...
//////////////////////////////////////////////////////////////////////////
void PyStartObjectCreation(tukk typeName, tukk fileName)
{
	CUndo undo("Create new object");
...
//////////////////////////////////////////////////////////////////////////
void GetPythonArgumentsVector(tukk pArguments, std::vector<string>& inputArguments)
{
	if (pArguments == NULL)
		return;
...
//////////////////////////////////////////////////////////////////////////
void PyRunFileWithParameters(tukk pFile, tukk pArguments)
{
	string path;
	...
	{
		PyObject* pyFileObject = PyFile_FromString(const_cast<tuk>(path.GetString()), "r");

		if (pyFileObject)
		{
			std::vector<tukk > argv;
			...
			PySys_SetArgv(argv.size(), const_cast<tuk*>(&argv[0]));
...
void PyRunFile(tukk pFile)
{
	PyRunFileWithParameters(pFile, nullptr);
}

//////////////////////////////////////////////////////////////////////////
void PyExecuteCommand(tukk cmdline)
{
	GetIEditorImpl()->GetCommandManager()->Execute(cmdline);
}

//////////////////////////////////////////////////////////////////////////
void PyLog(tukk pMessage)
{
	if (strcmp(pMessage, "") != 0)
		DrxLogAlways(pMessage);
}

//////////////////////////////////////////////////////////////////////////
bool PyMessageBox(tukk pMessage)
{
...
bool PyMessageBoxYesNo(tukk pMessage)
{
...
bool PyMessageBoxOK(tukk pMessage)
{
...
SPyWrappedProperty PyEditBoxAndCheckSPyWrappedProperty(tukk pTitle)
{
	QStringDialog stringDialog(pTitle);
	...
	while (!isValidDataType && stringDialog.exec() == QDialog::Accepted)
	{
		...
		i32 countComa = 0;
		i32 countOpenRoundBraket = 0;
		i32 countCloseRoundBraket = 0;
		i32 countDots = 0;

		i32 posDots = 0;
		i32 posComa = 0;
		i32 posOpenRoundBraket = 0;
		i32 posCloseRoundBraket = 0;
		...
		else if (countDots == 0 && countComa == 0 && countOpenRoundBraket == 0 && countCloseRoundBraket == 0)
		{
			...
			else
			{
				...
				char tempString[255];
				drx_strcpy(tempString, stringDialog.GetString());

				for (i32 i = 0; i < stringDialog.GetString().GetLength(); i++)
				{
		...
		if (value.type == SPyWrappedProperty::eType_Vec3)
		{
			...
			i32 iStart = valueRed.Find("(");
			valueRed.Delete(0, iStart + 1);
			i32 iEnd = valueRed.Find(",");
		...
		else if (value.type == SPyWrappedProperty::eType_Color)
		{
			...
			iEnd = valueGreen.Find(",");
			valueGreen.Delete(iEnd, valueGreen.GetLength());
			i32 iValueGreen = atoi(valueGreen);

			string valueBlue = stringDialog.GetString();
			valueBlue.Delete(0, valueBlue.Find(",") + 1);
			valueBlue.Delete(0, valueBlue.Find(",") + 1);
			valueBlue.Delete(valueBlue.Find(")"), valueBlue.GetLength());
			i32 iValueBlue = atoi(valueBlue);
...
string PyComboBox(string title, std::vector<string> values, i32 selectedIdx = 0)
{
	string result;
...
static void PyDrawLabel(i32 x, i32 y, float size, float r, float g, float b, float a, tukk pLabel)
{
	if (!pLabel)
	{
...
tukk PyGetEditMode()
{
	i32 actualEditMode = GetIEditorImpl()->GetEditMode();
...
//////////////////////////////////////////////////////////////////////////
tukk PyGetPakFromFile(tukk filename)
{
	...
	tukk pArchPath = pIPak->GetFileArchivePath(pFile);
	pIPak->FClose(pFile);
	return pArchPath;
}
...
{
	u32 hideMask = gViewportDebugPreferences.GetObjectHideMask();
	gViewportDebugPreferences.SetObjectHideMask(~hideMask);
}

void PySetHideMask(tukk pName, bool bAdd)
{
	u32 hideMask = gViewportDebugPreferences.GetObjectHideMask();
	i32 hideType = 0;
...
void PySetSelectionMask(i32 mask)
{
	GetIEditorImpl()->GetObjectManager()->SetSelectionMask(mask);
}

void PySetCoordSys(i32 value)
{
...
REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE(PySetCVar, general, set_cvar,
                                          ...
                                          "general.set_cvar(str cvarName, [i32, float, string] cvarValue)");
...
REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE(PyDrawLabel, general, draw_label,
                                          ...
                                          "general.draw_label(i32 x, i32 y, float r, float g, float b, float a, str label)");
...
REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE(PyComboBox, general, combo_box,
                                          ...
                                          "general.combo_box(str title, [str] values, i32 selectedIndex)");
...
REGISTER_PYTHON_COMMAND_WITH_EXAMPLE(PySetSelectionMask, general, set_selection_mask,
                                     ...
                                     "general.set_selection_mask(i32 mask)");
REGISTER_PYTHON_COMMAND_WITH_EXAMPLE(PySetCoordSys, general, set_coord_sys,
                                     ...
                                     "general.set_coord_sys(i32 value)");
