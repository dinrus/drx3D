...
protected:
	virtual tukk GetDescription() { return m_description; };
...
//////////////////////////////////////////////////////////////////////////
IDataBaseItem* CBaseLibrary::GetItem(i32 index)
{
...
bool CBaseLibrary::SaveLibrary(tukk name)
{
	assert(name != NULL);
	if (name == NULL)
	{
		DrxFatalError("The library you are attempting to save has no name specified.");
		return false;
	}
	...
	if (!bRes)
	{
		...
		DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_ERROR, strMessage.c_str());
	}
	return bRes;
}
