...
void CConfigPanel::DisplayGroup(Config::CConfigGroup* pGroup, tukk szGroupName)
{
	...
	// Create the group container
	{
		...
		// Create the items
		u32 innerOffset = kGroupInnerTopMargin;
		u32k count = pGroup->GetVarCount();
		...
		case Config::IConfigVar::eType_FLOAT:
			{
				...
				float newValue = 0.0f;
				if (FromString(newValue, (tukk)text))
				{
		...
		case Config::IConfigVar::eType_STRING:
			{
				...
				const string newValue((tukk)text);
				pItem->m_pVar->Set(&newValue);
				OnConfigValueChanged(pItem->m_pVar);
...
void CConfigPanel::OnSize(UINT nType, i32 cx, i32 cy)
{
	...
	for (u32 i = 0; i < m_groups.size(); ++i)
	{
		...
		for (u32 i = 0; i < pGroup->m_items.size(); ++i)
		{
