...
void CTerrainTextureExport::PreviewToTile(u32& outX, u32& outY, u32 x, u32 y)
{
	u32 dwTileCountY = GetIEditorImpl()->GetTerrainManager()->GetRGBLayer()->GetTileCountY();
...
void CTerrainTextureExport::DrawPreview(CDC& dc)
{
	...
	CPen* prevPen = dc.SelectObject(&cGrayPen);
	{
		...
		for (i32 x = 0; x < dwTileCountX; x++)
			for (i32 y = 0; y < dwTileCountY; y++)
			{
				RECT rc = {
...
void CTerrainTextureExport::OnGenerate()
{
	...
	i32 skyQuality = ls->iHemiSamplQuality;
...
string CTerrainTextureExport::BrowseTerrainTexture(bool bIsSave)
{
	string filePath(GET_PERSONALIZATION_PROPERTY(CTerrainTextureExport, szFileProperty).toString().toLocal8Bit());
	if (filePath.empty())
		filePath = CFileUtil::FormatInitialFolderForFileDialog(PathUtil::GamePathToDrxPakPath("", true).c_str()) + "terraintex.bmp";
...
void CTerrainTextureExport::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	i32 x = (point.x - m_cx) * dwTileCountX / TERRAIN_PREVIEW_RESOLUTION;
	i32 y = (point.y - m_cy) * dwTileCountY / TERRAIN_PREVIEW_RESOLUTION;
...
void CTerrainTextureExport::OnChangeResolutionBtn()
{
	...
	u32 dwCurSize = pHeightmap->GetRGBLayer()->GetTileResolution(dwTileX, dwTileY);
	dlg.SetSize(dwCurSize);
	if (dlg.DoModal() != IDOK)
		return;

	u32 dwNewSize = dlg.GetSize();

	for (u32 y = rcSel.top; y <= rcSel.bottom; ++y)
		for (u32 x = rcSel.left; x <= rcSel.right; ++x)
		{
			PreviewToTile(dwTileX, dwTileY, x, y);

			u32 dwOldSize = pHeightmap->GetRGBLayer()->GetTileResolution(dwTileX, dwTileY);
			...
			i32 nTexSectorSize = p3DEngine->GetTerrainTextureNodeSizeMeters();
			u32 dwCountX = pHeightmap->GetRGBLayer()->GetTileCountX();
			u32 dwCountY = pHeightmap->GetRGBLayer()->GetTileCountY();

			if (!nTexSectorSize || !nTerrainSize || !dwCountX)
				continue;

			i32 numTexSectorsX = nTerrainSize / dwCountX / nTexSectorSize;
			i32 numTexSectorsY = nTerrainSize / dwCountY / nTexSectorSize;

			for (i32 iY = 0; iY < numTexSectorsY; ++iY)
				for (i32 iX = 0; iX < numTexSectorsX; ++iX)
