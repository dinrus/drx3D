...
public:
	...
	{
	public:
		virtual i32 GetPreviewHeight(CDC* pDC, CXTPReportRow* pRow, i32 nWidth)
		{
	...
	CTerrainLayerRecord(CLayer* pLayer, i32 nIndex)
	{
		m_pLayer = pLayer;
...
protected:
	virtual tukk GetDescription() { return "Terrain Layers"; };
	...
	, m_pAssignMaterialLink(NULL)
{
	...
	if (pTerrainManager)
	{
		for (i32 i = 0; i < pTerrainManager->GetLayerCount(); i++)
		{
...
void CTerrainTextureDialog::InitReportCtrl()
{
	...
	{
	protected:
		i32 DrawLink(i32* pnCurrDrawPos, XTP_REPORTRECORDITEM_DRAWARGS* pDrawArgs, CXTPReportHyperlink* pHyperlink, CString strText, CRect rcLink, i32 nFlag)
		{
			CFont* pFont = 0;
			CFont* pOldFont = 0;
			...
			i32 nRes = CXTPReportPaintManager::DrawLink(pnCurrDrawPos, pDrawArgs, pHyperlink, strText, rcLink, nFlag);
			if (pFont)
...
void CTerrainTextureDialog::ReloadLayerList()
{
	...
	m_wndReport.GetSelectedRows()->Clear();
	i32 count = m_wndReport.GetRows()->GetCount();
	...
	if (layers.size() == 1)
	{
		...
		/*
		   ...
		   pSelLayer->GetTextureWidth(),pSelLayer->GetTextureHeight(), pSelLayer->GetCurrentLayerId());
		            ...
		            i32 nNumSurfaceTypes = GetIEditorImpl()->GetTerrainManager()->GetSurfaceTypeCount();

		   string maskInfoText;
		   i32 maskRes = pSelLayer->GetMaskResolution();
...
void CTerrainTextureDialog::OnGeneratePreview()
{
	...
	i32 tflags = ETTG_ABGR | ETTG_USE_LIGHTMAPS;
	if (m_bUseLighting)
...
void CTerrainTextureDialog::OnFileExportLargePreview()
{
	...
	DrxLog("Exporting large terrain texture preview (%ix%i)...",
	...
	// Generate the terrain texture
	i32 tflags = ETTG_INVALIDATE_LAYERS | ETTG_STATOBJ_SHADOWS | ETTG_BAKELIGHTING;
	if (m_bUseLighting)
		tflags |= ETTG_LIGHTING;
	if (m_bShowWater)
		tflags |= ETTG_SHOW_WATER;
	...
	u32* pPixData = image.GetData(), * pPixDataEnd = &(image.GetData())[image.GetWidth() * image.GetHeight()];
	...
	string temp = (tukk)gEditorFilePreferences.strStandardTempDirectory;
...
/*
   bool CTerrainTextureDialog::GenerateSurface(DWORD *pSurface, UINT iWidth, UINT iHeight, i32 flags,CBitArray *pLightingBits, float **ppHeightmapData)
   {
   ////////////////////////////////////////////////////////////////////////
   ...
   ////////////////////////////////////////////////////////////////////////
   i32 numLayers = m_doc->GetLayerCount();
   for (i=iFirstUsedLayer; i<(i32) numLayers; i++)
   {
    ...
    // Status message
    drx_sprintf(szStatusBuffer, "Updating layer %i of %i...", i + 1, m_doc->GetLayerCount());
    ...
    drx_sprintf(szStatusBuffer, "Blending layer %i of %i...", i + 1, m_doc->GetLayerCount());
    GetIEditorImpl()->SetStatusText(szStatusBuffer);

    // Set the write pointer (will be incremented) for the surface data
    DWORD *pTex = pSurface;

    u32 layerWidth = layer->GetTextureWidth();
    u32 layerHeight = layer->GetTextureHeight();
 ...
 * pTex =  (((255 - iBlend) * (*pTex & 0x000000FF)	+  (crlfNewCol & 0x000000FF)        * iBlend) >> 8)      |
   ...
   i32 t1 = GetTickCount();
   DrxLog( "Texture surface layers blended in %dms",t1-t0 );
   ...
   //			m_doc->GetLighting(), pLightingBits,shadowMap );
    ...
    i32 t2 = GetTickCount();
    DrxLog( "Texture lighted in %dms",t2-t1 );
   }
   ...
   if (bShowWaterColor)
   {
    ...
    u32 layerWidth = water->GetTextureWidth();
    u32 layerHeight = water->GetTextureHeight();
 ...
 * pTex = water->GetTexturePixel( iTexX % layerWidth,layerY );
   ...
   if (bConvertToABGR)
   {
    ...
    for (iTexY=0; iTexY<(i32) iHeight; iTexY++)
    {
      for (iTexX=0; iTexX<(i32) iWidth; iTexX++)
      {
 ...
 * ppHeightmapData = pHeightmapData;
   ...
   i32 t2 = GetTickCount();
   DrxLog( "Texture surface generate in %dms",t2-t0 );
...
void CTerrainTextureDialog::OnImport()
{
	...
	if (!fileName.isEmpty())
	{
		DrxLog("Importing layer settings from %s", path.GetString());
...
void CTerrainTextureDialog::OnExport()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "No layers exist. You have to create some Terrain Layers first.");
	...
	if (!filePath.isEmpty())
	{
		DrxLog("Exporting layer settings to %s", path);
...
void CTerrainTextureDialog::GeneratePreviewImageList()
{
	...
	for (i32 i = 0; i < GetIEditorImpl()->GetTerrainManager()->GetLayerCount(); i++)
	{
		...
		i32 nRes = m_imageList.Add(&bmp, RGB(255, 0, 255));
	}
...
//////////////////////////////////////////////////////////////////////////
void CTerrainTextureDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CTerrainTextureDialog::OnEditorNotifyEvent(EEditorNotifyEvent event)
{
	...
	case eNotify_OnSelectionChange:
		{
			for (i32 i = 0, cnt = GetIEditorImpl()->GetTerrainManager()->GetLayerCount(); i < cnt; ++i)
			{
				CLayer* pLayer = GetIEditorImpl()->GetTerrainManager()->GetLayer(i);
