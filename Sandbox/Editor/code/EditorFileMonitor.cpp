...
//////////////////////////////////////////////////////////////////////////
bool CEditorFileMonitor::RegisterListener(IFileChangeListener* pListener, tukk sMonitorItem)
{
	return RegisterListener(pListener, sMonitorItem, "*");
}

//////////////////////////////////////////////////////////////////////////
static string CanonicalizePath(tukk szPath)
{
...
//////////////////////////////////////////////////////////////////////////
static string GetAbsolutePathOfProjectFolder(tukk szPath)
{
	tukk szProjectRoot = GetISystem()->GetIProjectManager()->GetCurrentProjectDirectoryAbsolute();
	DrxPathString path = PathUtil::Make(szProjectRoot, szPath);
...
void CEditorFileMonitor::MonitorDirectories()
{
	...
	{
		tukk sModPath = gEnv->pDrxPak->GetMod(index);
		if (!sModPath)
			break;
...
//////////////////////////////////////////////////////////////////////////
bool CEditorFileMonitor::RegisterListener(IFileChangeListener* pListener, tukk szFolderRelativeToGame, tukk sExtension)
{
...
//////////////////////////////////////////////////////////////////////////
static bool IsFilenameEndsWithDotDaeDotZip(tukk fln)
{
...
//////////////////////////////////////////////////////////////////////////
static bool RecompileColladaFile(tukk path)
{
...
//////////////////////////////////////////////////////////////////////////
static tukk AbsoluteToProjectPath(tukk szAbsolutePath)
{
	...
	if (drx_strnicmp(fixedPath.c_str(), projectPath.c_str(), projectPath.length()) == 0) // path starts with rootPathStr
	{
...
//////////////////////////////////////////////////////////////////////////
tukk GetPathRelativeToModFolder(tukk szAbsolutePath)
{
	...
	string modLocation;
	tukk modFolder = gameFolder.c_str();
	i32 modIndex = 0;
	do
	{
		if (_strnicmp(modFolder, szAbsolutePath, strlen(modFolder)) == 0)
		{
			tukk result = szAbsolutePath + strlen(modFolder);
			if (*result == '\\' || *result == '/')
				++result;
			return result;
		}

		modFolder = gEnv->pDrxPak->GetMod(modIndex);
		++modIndex;
	}
	while (modFolder != 0);
...
{
	CDrxEditApp* app = CDrxEditApp::GetInstance();
	...
	if (!filename.IsEmpty())
	{
		...
		else if (isCOLLADA)
		{
			...
			string cgfFileName;
			i32 nameLength = filename.GetLength();
			...
			{
				DrxLog("Recompile DAE file: %s", (LPCTSTR)filename);
		...
		else
		{
			...
			if (isLMG)
			{
				DrxLog("Reload blendspace file: %s", (LPCTSTR)strPath);
		...
		CDrxEditApp::GetInstance()->ForceNextIdleProcessing();
	}
}
