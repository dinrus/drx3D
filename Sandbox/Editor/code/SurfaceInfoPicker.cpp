                                    ...
                                    CBaseObject* pObject)
{
	...
	{
		for (i32 k = 0, nChildCount(pObject->GetChildCount()); k < nChildCount; ++k)
		{
...
bool CSurfaceInfoPicker::PickByAABB(const CPoint& point, i32 nFlag, IDisplayViewport* pView, CExcludedObjects* pExcludedObjects, std::vector<CBaseObjectPtr>* pOutObjects)
{
	...
	for (i32 i = 0, iCount(m_objects.size()); i < iCount; ++i)
	{
  ...
  SRayHitInfo& outHitInfo) const
{
	for (size_t i = 0; i < m_pObjects->size(); ++i)
	{
		...
		if (m_ExcludedObjects.Contains(object))
			continue;
		for (i32 k = 0, nChildCount(object->GetChildCount()); k < nChildCount; ++k)
		{
...
void CSurfaceInfoPicker::FindNearestInfoFromSolids(i32 nFlags,
                                                   const Vec3& vWorldRaySrc,
                                                   const Vec3& vWorldRayDir,
                                                   IMaterial** pOutLastMaterial,
                                                   SRayHitInfo& outHitInfo) const
...
void CSurfaceInfoPicker::FindNearestInfoFromTerrain(
  ...
  SRayHitInfo& outHitInfo) const
{
	i32 objTypes = ent_terrain;
	i32 flags = (geom_colltype_ray | geom_colltype14) << rwi_colltype_bit | rwi_colltype_any | rwi_ignore_terrain_holes | rwi_stop_at_pierceable;
	ray_hit hit;
	hit.pCollider = 0;
	i32 col = gEnv->pPhysicalWorld->RayWorldIntersection(vWorldRaySrc, vWorldRayDir, objTypes, flags, &hit, 1);
	if (col > 0 && hit.dist < outHitInfo.fDistance && hit.bTerrain)
	{
		...
		i32 nTerrainSurfaceType = hit.idmatOrg;
  ...
  SRayHitInfo& outHitInfo)
{
	...
	ray_hit hit;
	i32 col = GetIEditorImpl()->GetSystem()->GetIPhysicalWorld()->RayTraceEntity(physics, vWorldRaySrc, vWorldRayDir, &hit, 0, geom_collides);
	if (col <= 0)
		return false;
