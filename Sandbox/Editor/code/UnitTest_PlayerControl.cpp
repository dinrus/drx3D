...
void CModelViewportCE::PlayerControl_UnitTest(ICharacterInstance* pInstance, const SRendParams& rRP, const SRenderingPassInfo& passInfo)
{
	...
	i32 PostProcessCallback_PlayControl(ICharacterInstance * pInstance, uk pPlayer);
...
void CModelViewportCE::PlayerControlHuman(const Vec2& vViewDir2D, f32 fGroundRadian, f32 fGroundRadianMoveDir)
{
	...
	u32 num = sizeof(m_arrLTHUMB) / sizeof(Vec2);
	for (i32 i = (num - 2); i > -1; i--)
		m_arrLTHUMB[i + 1] = m_arrLTHUMB[i];
	m_arrLTHUMB[0] = m_LTHUMB;

	u32 index = 0;  //min(u32(0.10f/m_AverageFrameTime),num-1);
	...
	u32 LookIK = m_pCharPanel_Animation->GetLookIK();
	...
	u32 AimIK = m_pCharPanel_Animation->GetAimIK();
	...
	static u32 keyrcr_R = 0;
	keyrcr_R = (keyrcr_R << 1) | u32(m_RT > 0.8f);
	if ((keyrcr_R & 3) == 1)
...
//-------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------
u32 CModelViewportCE::UseFootIKNew(ICharacterInstance* pInstance)
{
	float color1[4] = { 1, 1, 1, 1 };
	IRenderAuxGeom* pAuxGeom = m_renderer->GetIRenderAuxGeom();
	ISkeletonAnim* pISkeletonAnim = pInstance->GetISkeletonAnim();
	...
	i32 LHeelIdx = rIDefaultSkeleton.GetJointIDByName("Bip01 L Heel");
	if (LHeelIdx < 0) return 0;
	i32 LToe0Idx = rIDefaultSkeleton.GetJointIDByName("Bip01 L Toe0");
	...
	i32 RHeelIdx = rIDefaultSkeleton.GetJointIDByName("Bip01 R Heel");
	if (RHeelIdx < 0) return 0;
	i32 RToe0Idx = rIDefaultSkeleton.GetJointIDByName("Bip01 R Toe0");
