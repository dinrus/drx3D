...
#include <DrxSandbox/DrxInterop.h>
...
static CDrxEditDoc * theDocument;
...
void CEditorImpl::ExecuteCommand(tukk sCommand, ...)
{
	char buffer[1024];
	va_list args;
	va_start(args, sCommand);
	drx_vsprintf(buffer, sCommand, args);
	va_end(args);
...
void CEditorImpl::WriteToConsole(tukk pszString)
{
...
CDrxEditDoc* CEditorImpl::GetDocument() const
{
	return theDocument;
}

void CEditorImpl::SetDocument(CDrxEditDoc* pDoc)
{
	theDocument = pDoc;
}
...
bool CEditorImpl::SaveDocumentBackup()
{
	...
	tukk const CRASH_FOLDER = "/.CRASH_BACKUP/";
	tukk const CRASH_FILE_PREFIX = "CRASH_BACKUP_";

	if (CDrxEditDoc* pDocument = GetDocument())
	{
			...
			if (sLevelFolder.GetLength() != 0)
			{
				...
				if (::CreateDirectory(backupFolder, NULL) || (GetLastError() == ERROR_ALREADY_EXISTS))
				{
					...
					if (::CreateDirectory(fullLayerPath.GetString(), NULL) || (GetLastError() == ERROR_ALREADY_EXISTS))
					{
						...
						drx_sprintf(szCrashBackupName, "%s%s%s", backupFolder.GetString(), CRASH_FILE_PREFIX, sLevelName.GetString());
...
{
	return DrxStringUtils::WStrToUTF8(m_masterCDFolder.c_str());
}
...
tukk CEditorImpl::GetUserFolder()
{
...
i32 CEditorImpl::GetEditMode()
{
	return m_currEditMode;
}

void CEditorImpl::SetEditMode(i32 editMode)
{
...
CBaseObject* CEditorImpl::NewObject(tukk type, tukk file /*=nullptr*/, bool bInteractive /*= false*/)
{
	...
	if (pObject)
	{
		if (bInteractive)
		{
			...
			i32 hideMask = gViewportDebugPreferences.GetObjectHideMask();
...
void CEditorImpl::StartObjectCreation(tukk type, tukk file)
{
...
i32 CEditorImpl::ClearSelection()
{
...
{
	return CDrxEditApp::GetInstance()->IsUpdateSuspended();
}
...
void CEditorImpl::UpdateViews(i32 flags, AABB* updateRegion)
{
...
CWnd* CEditorImpl::OpenView(tukk sViewClassName)
{
	return CTabPaneManager::GetInstance()->OpenMFCPane(sViewClassName);
}

CWnd* CEditorImpl::FindView(tukk sViewClassName)
{
	return CTabPaneManager::GetInstance()->FindMFCPane(sViewClassName);
}

IPane* CEditorImpl::CreateDockable(tukk className)
{
...
QWidget* CEditorImpl::CreatePreviewWidget(const QString& file, QWidget* pParent)
{
	...
	{
		QStringList result;
		result << QStringLiteral(DRX_SKEL_FILE_EXT)
	...
	else if (info.suffix() == DRX_MATERIAL_FILE_EXT)
	{
...
{
	return CDrxEditApp::GetInstance()->IsInTestMode();
}

bool CEditorImpl::IsInConsolewMode()
{
	return CDrxEditApp::GetInstance()->IsInConsoleMode();
}

bool CEditorImpl::IsInLevelLoadTestMode()
{
	return CDrxEditApp::GetInstance()->IsInLevelLoadTestMode();
}

bool CEditorImpl::IsInPreviewMode()
{
	return CDrxEditApp::GetInstance()->IsInPreviewMode();
}
...
void CEditorImpl::DetectVersion()
{
	...
	i32 verSize = GetFileVersionInfoSize(exe, &dwHandle);
	if (verSize > 0)
	{
...
void CEditorImpl::SetConsoleVar(tukk var, float value)
{
...
void CEditorImpl::SetConsoleStringVar(tukk var, tukk value)
{
	ICVar* ivar = GetSystem()->GetIConsole()->GetCVar(var);
	if (ivar)
		ivar->Set(value);
...
float CEditorImpl::GetConsoleVar(tukk var)
{
	ICVar* ivar = GetSystem()->GetIConsole()->GetCVar(var);
	if (ivar)
	{
...
{
	i32 hideMask = gViewportDebugPreferences.GetObjectHideMask();
	i32 maskChanges = m_objectHideMask ^ hideMask;
...
void CEditorImpl::GetMemoryUsage(IDrxSizer* pSizer)
{
...
void CEditorImpl::ReduceMemory()
{
	...
	if (hHeap)
	{
		...
		DrxLogAlways("Max Free Memory Block = %I64d Kb", maxsize / 1024);
	}
}
...
void CEditorImpl::RegisterURIListener(IUriEventListener* pListener, tukk szSchema)
{
...
void CEditorImpl::UnregisterURIListener(IUriEventListener* pListener, tukk szSchema)
{
	TURIListeners::iterator it = m_uriListeners.find(szSchema);
	if (it != m_uriListeners.end())
	{
...
void CEditorImpl::AddNativeHandler(uintptr_t id, std::function<bool(uk , long*)> callback)
{
...
{
	CDrxEditApp::GetInstance()->OnGroupMake();
}

void CEditorImpl::OnGroupAttach()
{
	CDrxEditApp::GetInstance()->OnGroupAttach();
}

void CEditorImpl::OnGroupDetach()
{
	CDrxEditApp::GetInstance()->OnGroupDetach();
}

void CEditorImpl::OnGroupDetachToRoot()
{
	CDrxEditApp::GetInstance()->OnGroupDetachToRoot();
}
...
{
	CDrxEditApp::GetInstance()->OnEditToolUnlink();
}
...
void CEditorImpl::RegisterDeprecatedPropertyEditor(i32 propertyType, std::function<bool(const string&, string&)>& handler)
{
	m_deprecatedPropertyEditors[propertyType] = handler;
}

bool CEditorImpl::EditDeprecatedProperty(i32 propertyType, const string& oldValue, string& newValueOut)
{
...
bool CEditorImpl::CanEditDeprecatedProperty(i32 propertyType)
{
	auto handler = m_deprecatedPropertyEditors[propertyType];
	if (handler)
		return true;
