...
namespace
{
i32 OnFactoryFilter(u32 code, struct _EXCEPTION_POINTERS* pExceptionPtrs)
{
  ...
  tukk szFilePath)
{
	...
	{
		DrxLog("Can't initialize plugin '%s'! Possible binary version incompatibility. Please reinstall this plugin.", szFilePath);
...
void ParseManifest(const string& manifestPath, SPlugin& plugin)
{
	...
	if (manifest)
	{
		...
		for (uint i = 0; i < numElements; ++i)
		{
			...
			{
				tukk dependencyName = element->getContent();
...
void SortPluginsByDependency(std::list<SPlugin>& plugins)
{
	...
	while (!plugins.empty())
	{
		...
		if (bCantReduce)
		{
			...
			{
				DrxLog("Can't load plugin DLL '%s' because of missing or cyclic dependencies", iter->m_path.GetString());
			}
			break;
		}
	}
...
bool CPluginManager::LoadPlugins(tukk pPathWithMask)
{
	LOADING_TIME_PROFILE_SECTION;
	...
	{
		DrxLog("Cannot find plugin directory '%s'", strPath.GetBuffer(1));
		return false;
	}
	...
	std::list<SPlugin> plugins;
	{
		...
		if (cDLLFiles.StartEnumeration(strPath.GetBuffer(1), strMask, &sFile))
		{
			do
			{
				...
				drx_strcpy(szFilePath, strPath.GetString());
				PathAddBackslash(szFilePath);
				drx_strcat(szFilePath, sFile.name);
	...
	for (auto iter = plugins.begin(); iter != plugins.end(); ++iter)
	{
		...
		i32k extensionPos = manifestPath.ReverseFind('.');
		if (extensionPos != -1)
		{
	...
	for (auto iter = plugins.begin(); iter != plugins.end(); ++iter)
	{
		...
		if (!hPlugin)
		{
			...
			DrxLog("Can't load plugin DLL '%s' message '%s' !", iter->m_path, szMessageBuffer);
			continue;
		}
		...
		if (!pfnFactory)
		{
			DrxLog("Cannot query plugin DLL '%s' factory pointer (is it a Sandbox plugin?)", iter->m_path);
			FreeLibrary(hPlugin);
			continue;
		}
		...
		if (!pPlugin)
		{
			...
			{
				DrxLog("Cannot create instance of plugin DLL '%s'! Version mismatch. Please update the plugin.", iter->m_path);
			}
			else
			{
				DrxLog("Cannot create instance of plugin DLL '%s'! Error code %u.", iter->m_path, sInitParam.outErrorCode);
			}
		...
		DrxLog("Successfully loaded plugin '%s', version '%i'",
