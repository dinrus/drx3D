...
bool CObjectCreateTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
	switch (event)
	{
	...
	default:
		if (m_createdObject)
		{
			...
			i32 res = m_createdObject->MouseCreateCallback(view, event, point, flags);
...
bool CObjectCreateTool::OnDragEvent(CViewport* view, EDragEvent eventId, QEvent* event, u32 flags)
{
	switch (eventId)
	{
	case eDragEnter:
	...
	case eDragMove:
		{
			if(IsCreating())
			{
				...
				DRX_ASSERT(m_createdObject);
				GetIEditorImpl()->GetIUndoManager()->Suspend();
				CPoint point(dragMove->pos().x(), dragMove->pos().y());
				i32 res = m_createdObject->MouseCreateCallback(view, eMouseMove, point, flags);
				GetIEditorImpl()->GetIUndoManager()->Resume();
				dragMove->accept();
				return true;
			}
...
bool CObjectCreateTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
void CObjectCreateTool::SelectObjectToCreate(tukk objectClassName, tukk filepath /*= nullptr*/)
{
...
void CObjectCreateTool::SelectObjectToCreate(CObjectClassDesc* objectType, tukk filepath)
{
	if (!CanStartCreation())
		CancelCreation();
