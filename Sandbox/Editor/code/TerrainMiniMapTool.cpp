...
protected:
	virtual tukk GetDescription() { return "MiniMap Params"; };
	...
	: m_bGenerationFinished(true)
{
	...
	m_path = PathUtil::GamePathToDrxPakPath(GetIEditorImpl()->GetGameEngine()->GetLevelPath().GetString());
	if (strstr(m_path, ":\\") == nullptr)
	{
		char rootpath[_MAX_PATH];
		GetCurrentDirectory(sizeof(rootpath), rootpath);
		m_path = PathUtil::Make(rootpath, PathUtil::GamePathToDrxPakPath(m_path.GetString()));
	}
...
void CTerrainMiniMapTool::SetResolution(i32 nResolution)
{
...
//////////////////////////////////////////////////////////////////////////
bool CTerrainMiniMapTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
...
void CTerrainMiniMapTool::SendParameters(uk data, u32 width, u32 height, f32 minx, f32 miny, f32 maxx, f32 maxy)
{
	CWaitCursor wait;
	...
	if (root)
	{
		for (i32 i = 0, nChilds = root->getChildCount(); i < nChilds; ++i)
		{
			...
			if (pVar)
			{
				...
				tukk pAttr = ChildNode->getAttr("value");
				string Value = pAttr;
				pVar->Set(Value.c_str());
			}
			else
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "[MiniMap: %s] Console variable %s does not exist", MAP_SCREENSHOT_SETTINGS, pTagName);
			}
		}
	}
	else
