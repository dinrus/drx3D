...
//#include "pure.h"
	...
	#include "DrxDevProjectSelectionDialog.cpp"
#endif

static tukk kAutoBackupFolder = "_autobackup";
static tukk kLastLoadPathFilename = "lastLoadPath.preset";

// List of files that need to be copied when saving to a different folder
static tukk kLevelSaveAsCopyList[] =
{
	"level.pak",
	"terraintexture.pak",
	"filelist.xml",
...
void SetGlobalLevelName(const string& szFilename)
{
	...
	if (sv_map)
	{
		sv_map->Set((tukk)szGameLevelName);
	}
}
...
void DisablePhysicsAndAISimulation()
{
	...
	{
		CDrxEditApp::GetInstance()->OnSwitchPhysics();
...
void CreateMovieSystemSequenceObjects()
{
	...
	if (pMs)
	{
		for (i32 k = 0; k < pMs->GetNumSequences(); ++k)
		{
...
void CreateLevelIfNeeded()
{
	...
	if (!pObjectLayerManager->HasLayers())
	{
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "The level doesn't have any layers. Creating a new layer as fallback. This is a critical error and may be a sign of data corruption.");
...
/////////////////////////////////////////////////////////////////////////////
// CDrxEditDoc construction/destruction

CDrxEditDoc::CDrxEditDoc()
...
void CDrxEditDoc::Save(CXmlArchive& xmlAr)
{
...
void CDrxEditDoc::Save(TDocMultiArchive& arrXmlAr)
{
	LOADING_TIME_PROFILE_SECTION;
	...
	if (arrXmlAr[DMAS_GENERAL] != NULL)
	{
		...
		(*arrXmlAr[DMAS_GENERAL]).root->setAttr("SandboxVersion", (tukk)GetIEditorImpl()->GetFileVersion().ToFullString());
...
void CDrxEditDoc::Load(CXmlArchive& xmlAr, const string& szFilename)
{
	TDocMultiArchive arrXmlAr;
	FillXmlArArray(arrXmlAr, &xmlAr);
	CDrxEditDoc::Load(arrXmlAr, szFilename);
}

void CDrxEditDoc::Load(TDocMultiArchive& arrXmlAr, const string& filename)
{
	using namespace Private_DrxEditDoc;
	...
	DrxLog("Loading from %s...", (tukk)filename);
...
void CDrxEditDoc::LoadShaderCache(TDocMultiArchive& arrXmlAr)
{
...
void CDrxEditDoc::ActivateMission(const string &currentMissionName)
{
	LOADING_TIME_PROFILE_SECTION_NAMED_ARGS("Activating Mission", currentMissionName.c_str());
	string str;
	str.Format("Activating Mission %s", (tukk)currentMissionName);
	...
	{
		auto getXMLAttribBool = [](XmlNodeRef pInputNode, tukk szLevel1, tukk szLevel2, bool bDefaultValue) -> bool
		{
...
string CDrxEditDoc::GetCurrentMissionName(TDocMultiArchive& arrXmlAr)
{
	string currentMissionName;
	SerializeMissions(arrXmlAr, currentMissionName, false);
...
void CDrxEditDoc::SerializeViewSettings(CXmlArchive& xmlAr)
{
...
void CDrxEditDoc::SerializeFogSettings(CXmlArchive& xmlAr)
{
	if (xmlAr.bLoading)
	{
...
void CDrxEditDoc::SerializeMissions(TDocMultiArchive& arrXmlAr, string& currentMissionName, bool bPartsInXml)
{
	...
	else
	{
		...
		for (i32 i = 0; i < m_missions.size(); i++)
		{
			CXmlArchive ar(*arrXmlAr[DMAS_GENERAL]);
...
void CDrxEditDoc::SerializeShaderCache(CXmlArchive& xmlAr)
{
	LOADING_TIME_PROFILE_SECTION;
	if (xmlAr.bLoading)
	{
	...
	else
	{
		...
		if (buf.GetLength() > 0)
		{
			xmlAr.pNamedData->AddDataBlock("ShaderCache", const_cast<tuk>(buf.GetBuffer()), buf.GetLength());
		}
	}
}

bool CDrxEditDoc::CanClose()
{
	if (!IsDocumentReady())
	{
...
BOOL CDrxEditDoc::DoSave(LPCTSTR lpszPathName, BOOL bReplace)
{
...
BOOL CDrxEditDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
...
BOOL CDrxEditDoc::BeforeOpenDocument(LPCTSTR lpszPathName, TOpenDocContext& context)
{
	...
	DrxLog("Opening document %s", (tukk)relativeLevelName);
...
BOOL CDrxEditDoc::DoOpenDocument(LPCTSTR lpszPathName, TOpenDocContext& context)
{
	...
	DrxLog("-----------------------------------------------------------");
	DrxLog("Successfully opened document %s", (tukk)levelPath);
	DrxLog("Level loading time: %.2f seconds", (loading_end_time - loading_start_time).GetSeconds());
	DrxLog("-----------------------------------------------------------");
...
BOOL CDrxEditDoc::OnSaveDocument(LPCTSTR lpszPathName)
{
...
BOOL CDrxEditDoc::BeforeSaveDocument(LPCTSTR lpszPathName, TSaveDocContext& context)
{

#if defined(IS_COMMERCIAL)
	CDrxDevProjectSelectionDialog ProjectSelection;
	INT_PTR iResult = ProjectSelection.DoModal();
	if (iResult == IDABORT || iResult == IDCANCEL)
	{
		DrxLog("You cancelled saving of level file \"%s\"", (tukk)lpszPathName);
		return FALSE;
	}
#endif
	...
	string levelPath = PathUtil::AbsolutePathToDrxPakPath(lpszPathName).c_str();
	if (levelPath.empty())
		return FALSE;

	DrxLog("Saving to %s...", (tukk)levelPath);
...
BOOL CDrxEditDoc::DoSaveDocument(LPCTSTR filename, TSaveDocContext& context)
{
	LOADING_TIME_PROFILE_SECTION;
	...
	if (bSaved)
	{
		...
		CDrxEditApp::GetInstance()->SaveTagLocations();

		bSaved = SaveLevel(PathUtil::AbsolutePathToDrxPakPath(filename).c_str());
...
BOOL CDrxEditDoc::AfterSaveDocument(LPCTSTR lpszPathName, TSaveDocContext& context, bool bShowPrompt)
{
	LOADING_TIME_PROFILE_SECTION;
...
bool CDrxEditDoc::SaveLevel(const string& filename)
{
	...
	const string oldLevelPath = PathUtil::AbsolutePathToDrxPakPath(GetPathName().GetString()).c_str();
	const string oldLevelFolder = PathUtil::GetPathWithoutFilename(oldLevelPath);

	if (oldLevelFolder != levelFolder)
	{
		LOADING_TIME_PROFILE_SECTION_NAMED("CDrxEditDoc::SaveLevel() level folder changed");
		...
		for (u32 i = 0; i < sizeof(kLevelSaveAsCopyList) / sizeof(tuk); ++i)
		{
	...
	DrxLog("Saving level file %s", filename.c_str());
	{
...
bool CDrxEditDoc::LoadLevel(TDocMultiArchive& arrXmlAr, const string& filename)
{
	LOADING_TIME_PROFILE_SECTION;
	...
	if (pszRelativePath[0] != '.') // if the level file is in the root path of mastercd.
		levelPath = PathUtil::GamePathToDrxPakPath(pszRelativePath.GetString());

	GetIEditorImpl()->GetGameEngine()->SetLevelPath(levelPath);
	OnStartLevelResourceList();

	// Load next level resource list.
	pIPak->GetResourceList(IDrxPak::RFOM_NextLevel)->Load(PathUtil::Make(levelFolder, "resourcelist.txt"));
	...
	pIPak->GetResourceList(IDrxPak::RFOM_NextLevel)->Clear();
...
QString CDrxEditDoc::GetLastLoadedLevelName()
{
...
void CDrxEditDoc::SetLastLoadedLevelName(tukk lastLoadedFileName)
{
...
void CDrxEditDoc::SaveAutoBackup(bool bForce)
{
	...
	intptr_t handle = gEnv->pDrxPak->FindFirst(autoBackupPath + "/*.*", &fileinfo);
	if (handle != -1)
	{
		do
		{
	...
	for (i32 i = i32(folders.size()) - gEditorFilePreferences.autoSaveMaxCount; i >= 0; --i)
	{
...
CMission* CDrxEditDoc::GetCurrentMission(bool bSkipLoadingAIWhenSyncingContent /* = false */)
{
	LOADING_TIME_PROFILE_SECTION;
	if (m_mission)
	{
...
void CDrxEditDoc::SetCurrentMission(CMission* mission)
{
...
void CDrxEditDoc::ClearMissions()
{
	for (i32 i = 0; i < m_missions.size(); i++)
	{
		delete m_missions[i];
...
bool CDrxEditDoc::IsLevelExported() const
{
	return m_boLevelExported;
}

void CDrxEditDoc::SetLevelExported(bool boExported)
{
	m_boLevelExported = boExported;
}

CMission* CDrxEditDoc::FindMission(const string& name) const
{
	for (i32 i = 0; i < m_missions.size(); i++)
	{
...
void CDrxEditDoc::AddMission(CMission* mission)
{
...
void CDrxEditDoc::RemoveMission(CMission* mission)
{
...
LightingSettings* CDrxEditDoc::GetLighting()
{
	return GetCurrentMission()->GetLighting();
}

void CDrxEditDoc::LogLoadTime(i32 time)
{
	char szExeFileName[_MAX_PATH];
	GetModuleFileName(DrxGetCurrentModule(), szExeFileName, sizeof(szExeFileName));
	...
	DrxLog("[LevelLoadTime] Level %s loaded in %d seconds", (tukk)level, time / 1000);
	...
	if (file)
	{
		...
		time = time / 1000;
		text.Format("\n[%s] Level %s loaded in %d seconds", (tukk)version, (tukk)level, time);
		fwrite((tukk)text, text.GetLength(), 1, file);
		fclose(file);
	}
}

void CDrxEditDoc::SetDocumentReady(bool bReady)
{
	m_bDocumentReady = bReady;
}

void CDrxEditDoc::GetMemoryUsage(IDrxSizer* pSizer)
{
	pSizer->Add(*this);
	GetIEditorImpl()->GetTerrainManager()->GetTerrainMemoryUsage(pSizer);
}

void CDrxEditDoc::RegisterConsoleVariables()
{
...
void CDrxEditDoc::OnValidateSurfaceTypesChanged(ICVar*)
{
	CSurfaceTypeValidator().Validate();
}

void CDrxEditDoc::OnStartLevelResourceList()
{
	...
	if (bFirstTime)
	{
		tukk pResFilename = gEnv->pDrxPak->GetResourceList(IDrxPak::RFOM_Level)->GetFirst();
		...
		while (pResFilename)
		{
			...
			pResFilename = gEnv->pDrxPak->GetResourceList(IDrxPak::RFOM_Level)->GetNext();
		}
...
void CDrxEditDoc::ForceSkyUpdate()
{
	LOADING_TIME_PROFILE_SECTION;
...
void CDrxEditDoc::InitEmptyLevel(i32 resolution, float unitSize, bool bUseTerrain)
{
	GetISystem()->SetSystemGlobalState(ESYSTEM_GLOBAL_STATE_LEVEL_LOAD_START_PREPARE);
...
void CDrxEditDoc::OnEnvironmentPropertyChanged(IVariable* pVar)
{
	if (pVar == NULL)
		return;
	...
	i32 nKey = (i32)(intptr_t)pVar->GetUserData();

	i32 nGroup = (nKey & 0xFFFF0000) >> 16;
	i32 nChild = (nKey & 0x0000FFFF);
...
string CDrxEditDoc::GetDrxIndexPath(const LPCTSTR levelFilePath)
{
...
BOOL CDrxEditDoc::LoadXmlArchiveArray(TDocMultiArchive& arrXmlAr, const string& relativeLevelName, const string& levelPath)
{
	IDrxPak* pIPak = GetIEditorImpl()->GetSystem()->GetIPak();
...
void CDrxEditDoc::ReleaseXmlArchiveArray(TDocMultiArchive& arrXmlAr)
{
	SAFE_DELETE(arrXmlAr[0]);
}

void CDrxEditDoc::SyncCurrentMissionContent(bool bRetrieve)
{
	GetCurrentMission()->SyncContent(bRetrieve, false);
}

void CDrxEditDoc::SetModifiedFlag(bool bModified)
{
	m_bModified = bModified;
}

bool CDrxEditDoc::IsModified() const
{
	return m_bModified;
}

string CDrxEditDoc::GetTitle() const
{
	return PathUtil::GetFileName(m_pathName).c_str();
}

void CDrxEditDoc::SetPathName(LPCTSTR lpszPathName, BOOL bAddToMRU /*= TRUE*/)
{
