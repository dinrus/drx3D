...
{
public:
	CSegmentSpinBox(CGotoPositionDialog* pParent, i32 rangeMin, i32 rangeMax, i32 initValue)
		: QSpinBox(pParent)
	{
		setAlignment(Qt::AlignRight | Qt::AlignTrailing | Qt::AlignVCenter);
...
void CGotoPositionDialog::OnPositionStringChanged()
{
	if (!m_pPositionString->text().isEmpty())
	{
		...
		m_pPositionBoxes[0]->setVal(value[0]);
		m_pPositionBoxes[1]->setVal(value[1]);
		m_pPositionBoxes[2]->setVal(value[2]);

		m_pRotationBoxes[0]->setVal(value[3]);
		m_pRotationBoxes[1]->setVal(value[4]);
		m_pRotationBoxes[2]->setVal(value[5]);
	}
}
