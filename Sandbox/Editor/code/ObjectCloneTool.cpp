...
void CObjectCloneTool::CloneSelection()
{
	...
	// Clone every object.
	for (i32 i = 0; i < currSelection->GetFilteredCount(); i++)
	{
...
bool CObjectCloneTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
	if (m_selection)
	{
		...
		if (event == eMouseMove)
		{
			...
			if (!selection->IsEmpty())
			{
				...
				i32 axis = GetIEditorImpl()->GetAxisConstrains();
...
bool CObjectCloneTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
