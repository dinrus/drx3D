...
struct CUndoTPElement
{
	...
	void AddSector(float fpx, float fpy, float radius)
	{
		...
		u32 dwMaxRes = pRGBLayer->CalcMaxLocalResolution(0, 0, 1, 1);

		u32 dwTileCountX = pRGBLayer->GetTileCountX();
		u32 dwTileCountY = pRGBLayer->GetTileCountY();
		...
		CRect recTiles = CRect(
		  (u32)floor(gx1 * dwTileCountX),
		  (u32)floor(gy1 * dwTileCountY),
		  (u32)ceil(gx2 * dwTileCountX),
		  (u32)ceil(gy2 * dwTileCountY));

		for (u32 dwTileY = recTiles.top; dwTileY < recTiles.bottom; ++dwTileY)
			for (u32 dwTileX = recTiles.left; dwTileX < recTiles.right; ++dwTileX)
			{
				u32 dwLocalSize = pRGBLayer->GetTileResolution(dwTileX, dwTileY);
				u32 dwSize = pRGBLayer->GetTileResolution(dwTileX, dwTileY) * pRGBLayer->GetTileCountX();

				u32 gwid = dwSize * radius * 4;
				if (gwid < 32)
					gwid = 32;
				else if (gwid < 64)
					gwid = 64;
				...
				CRect recSects = CRect(
				  ((i32)floor(x1 * dwSize / wid)) * wid,
				  ((i32)floor(y1 * dwSize / wid)) * wid,
				  ((i32)ceil(x2 * dwSize / wid)) * wid,
				  ((i32)ceil(y2 * dwSize / wid)) * wid);

				for (u32 sy = recSects.top; sy < recSects.bottom; sy += wid)
					for (u32 sx = recSects.left; sx < recSects.right; sx += wid)
					{
						bool bFind = false;
						for (i32 i = sects.size() - 1; i >= 0; i--)
						{
		...
		CRect reclids = CRect(
		  (u32)floor(gx1 * heightmap->GetWidth() / layerSize),
		  (u32)floor(gy1 * heightmap->GetHeight() / layerSize),
		  (u32)ceil(gx2 * heightmap->GetWidth() / layerSize),
		  (u32)ceil(gy2 * heightmap->GetHeight() / layerSize));

		for (u32 ly = reclids.top; ly < reclids.bottom; ly++)
			for (u32 lx = reclids.left; lx < reclids.right; lx++)
			{
				bool bFind = false;
				for (i32 i = layerIds.size() - 1; i >= 0; i--)
				{
...
protected:
	...
	virtual tukk GetDescription() { return "Terrain Painter Modify"; };
...
//////////////////////////////////////////////////////////////////////////
bool CTerrainTexturePainter::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
...
//////////////////////////////////////////////////////////////////////////
bool CTerrainTexturePainter::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
{
	for (i32 i = 0; i < GetIEditorImpl()->GetTerrainManager()->GetLayerCount(); i++)
	{
...
{
	i32 iTerrainSize = m_3DEngine->GetTerrainSize();                                      // in m
	i32 hmapWidth = m_heightmap->GetWidth();
	i32 hmapHeight = m_heightmap->GetHeight();

	i32 iX = (i32)((m_pointerPos.y * hmapWidth) / iTerrainSize);      // maybe +0.5f is needed
	i32 iY = (i32)((m_pointerPos.x * hmapHeight) / iTerrainSize);     // maybe +0.5f is needed

	if (iX >= 0 && iX < iTerrainSize)
		if (iY >= 0 && iY < iTerrainSize)
		{
			u32 dwLayerid = m_heightmap->GetLayerIdAt(iX, iY);
...
void CTerrainTexturePainter::Action_FixMaterialCount()
{
	...
	i32 nSectorSize = m_3DEngine->GetTerrainSectorSize();

	i32 nTerrainSize = m_3DEngine->GetTerrainSize();  // in m
	i32 hmapWidth = m_heightmap->GetWidth();
	i32 hmapHeight = m_heightmap->GetHeight();

	i32 iX = (i32)((m_pointerPos.y * hmapWidth) / nTerrainSize);
	i32 iY = (i32)((m_pointerPos.x * hmapHeight) / nTerrainSize);

	i32 iMinX = (iX / nSectorSize) * nSectorSize;
	i32 iMinY = (iY / nSectorSize) * nSectorSize;
	i32 iMaxX = iMinX + nSectorSize;
	i32 iMaxY = iMinY + nSectorSize;

	i32 UsageCounters[MAX_SURFACE_TYPE_ID_COUNT + 1];
	i32 iUsedSurfaceIDs = 0;
	...
	i32 iSelected = pSelectedLayer ? pSelectedLayer->GetCurrentLayerId() : -1;
	...
	while (iUsedSurfaceIDs >= 15)
	{
		...
		for (i32 i = 0; i < MAX_SURFACE_TYPE_ID_COUNT + 1; i++)
...
bool CTerrainTexturePainter::CheckSectorPalette(CLayer* pLayer, const Vec3& center)
{
	...
	if (m_brush.bDetailLayer && pLayer->GetSurfaceType())
	{
		...
		i32 nTerrainSizeInUnits = m_3DEngine->GetTerrainSize();
		i32 x = pos_directed_rounding(center.y * (float)m_heightmap->GetWidth() / nTerrainSizeInUnits);
		i32 y = pos_directed_rounding(center.x * (float)m_heightmap->GetHeight() / nTerrainSizeInUnits);

		float unitSize = m_heightmap->GetUnitSize();
		i32 r = pos_directed_rounding(m_brush.radius / unitSize);
...
void CTerrainTexturePainter::Serialize(Serialization::IArchive& ar)
{
	if (ar.openBlock("brush", "Brush"))
	{
		...
		if (pTerrainManager)
		{
			i32 layerCount = pTerrainManager->GetLayerCount();
...
void CTerrainTexturePainter::PaintLayer(CLayer* pLayer, const Vec3& center, bool bFlood)
{
	...
	//////////////////////////////////////////////////////////////////////////
	{
		i32 iTerrainSize = m_3DEngine->GetTerrainSize();                        // in meters
		i32 iTexSectorSize = m_3DEngine->GetTerrainTextureNodeSizeMeters();     // in meters
		...
		i32 iMinSecX = (i32)floor(vMin.x / (float)iTexSectorSize);
		i32 iMinSecY = (i32)floor(vMin.y / (float)iTexSectorSize);
		i32 iMaxSecX = (i32)ceil(vMax.x / (float)iTexSectorSize);
		i32 iMaxSecY = (i32)ceil(vMax.y / (float)iTexSectorSize);
		...
		for (i32 iY = iMinSecY; iY < iMaxSecY; ++iY)
			for (i32 iX = iMinSecX; iX < iMaxSecX; ++iX)
			{
	...
	// Build rectangle in heightmap coordinates.
	{
		...
		hmaprc.left = max((i32)floor(fX - fHMRadius), 0);
		hmaprc.top = max((i32)floor(fY - fHMRadius), 0);
		hmaprc.right = min((i32)ceil(fX + fHMRadius), hmapWidth);
		hmaprc.bottom = min((i32)ceil(fY + fHMRadius), hmapHeight);
...
class CTerrainTexturePainter_ClassDesc : public IClassDesc
{
	...
	virtual tukk ClassName() { return "EditTool.TerainPainter"; };
