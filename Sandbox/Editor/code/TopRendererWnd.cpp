...
static u32 sVegetationColors[16] =
{
	0xFFFF0000,
	0xFF00FF00,
	0xFF0000FF,
...
//////////////////////////////////////////////////////////////////////////
void CTopRendererWnd::UpdateContent(i32 flags)
{
	if (gViewportPreferences.mapViewportSwapXY)
		SetAxis(VPA_YX);
	else
...
//////////////////////////////////////////////////////////////////////////
void CTopRendererWnd::UpdateSurfaceTexture(i32 flags)
{
	////////////////////////////////////////////////////////////////////////
	...
	if (flags & eUpdateHeightmap)
	{
		...
		if (!bShowHeightmap)
		{
			...
			i32 flags = ETTG_LIGHTING | ETTG_FAST_LLIGHTING | ETTG_ABGR | ETTG_BAKELIGHTING;
			if (m_bShowWater)
		...
		{
			i32 cx = gViewportPreferences.mapViewportResolution;
			i32 cy = gViewportPreferences.mapViewportResolution;
...
void CTopRendererWnd::DrawStaticObjects()
{
	...
	i32 srcW = vegetationMap->GetNumSectors();
	i32 srcH = vegetationMap->GetNumSectors();
	...
	i32 trgW = rc.right - rc.left;
	i32 trgH = rc.bottom - rc.top;
	...
	u32* trg = m_vegetationTexture.GetData();
...
void CTopRendererWnd::Draw(CObjectRenderHelper& objRenderHelper)
{
	...
	if (!m_terrainTextureId)
	{
		...
		if (m_terrainTexture.IsValid())
			m_terrainTextureId = gEnv->pRenderer->UploadToVideoMemory((u8*)m_terrainTexture.GetData(), m_textureSize.cx, m_textureSize.cy, eTF_R8G8B8A8, eTF_R8G8B8A8, 0, 0, 0);
	}

	if (m_terrainTextureId && m_terrainTexture.IsValid())
		gEnv->pRenderer->UpdateTextureInVideoMemory(m_terrainTextureId, (u8*)m_terrainTexture.GetData(), 0, 0, m_textureSize.cx, m_textureSize.cy, eTF_R8G8B8A8);

	if (m_bShowStatObjects)
	{
		if (m_vegetationTexture.IsValid())
		{
			i32 w = m_vegetationTexture.GetWidth();
			i32 h = m_vegetationTexture.GetHeight();
			u32* tex = m_vegetationTexture.GetData();
			if (!m_vegetationTextureId)
			{
				m_vegetationTextureId = gEnv->pRenderer->UploadToVideoMemory((u8*)tex, w, h, eTF_R8G8B8A8, eTF_R8G8B8A8, 0, 0, FILTER_NONE);
			}
			else
			{
				i32 px = m_vegetationTexturePos.x;
				i32 py = m_vegetationTexturePos.y;
				gEnv->pRenderer->UpdateTextureInVideoMemory(m_vegetationTextureId, (u8*)tex, px, py, w, h, eTF_R8G8B8A8);
			}
