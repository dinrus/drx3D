...
struct IDataBaseManager
{
	...
	virtual IDataBaseItem*           FindItem(DrxGUID guid) const = 0;
	...
	virtual i32               GetLibraryCount() const = 0;
	//! Get Item library by index.
	virtual IDataBaseLibrary* GetLibrary(i32 index) const = 0;
	...
	{
		for (i32 j = 0; j < GetLibraryCount(); j++)
		{
			IDataBaseLibrary* lib = GetLibrary(j);

			for (i32 i = 0; i < lib->GetItemCount(); i++)
			{
