...
SArchiveContentInternal CPakFiles::GetContents(const QString& keyEnginePath) const
{
	...
	for (i32 i = 0; i < directoryList.size(); ++i)
	{
...
void CPakFiles::GetContentsInternal(const STxt& archiveEnginePathStr, const QString& fullLocalPath, SArchiveContentInternal& result, QStringList& directoryList) const
{
	static QDateTime fileTimeOrigin(QDate(1601, 1, 1), QTime(0, 0, 0, 0), Qt::UTC);

	STxt levelPakPath(GetIEditor()->GetLevelName());
	levelPakPath += "/level.pak";
	
	if (archiveEnginePathStr.find(levelPakPath) != STxt::npos) // don't index the level.pak
		return;
	...
	pIPak->ForEachArchiveFolderEntry(
	  ...
	  searchStr.c_str(),
	  [&](const IDrxPak::ArchiveEntryInfo& entry)
			{
