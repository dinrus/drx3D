...
const u64 MONITOR_BUFFER_SIZE = 257 * KILOBYTE;
/*
 ...
 */
const u64 MONITOR_BUFFER_ALIGNMENT = 128;
...
private:
	...
	static unsigned WINAPI RunAPI(uk parameter)
	{
...
private:
	...
	void HandleMonitorCompletions(DWORD count)
	{
		for (DWORD i = 0; i < count; ++i)
		{
			...
			auto completionKey = (i32)entry.lpCompletionKey;
		...
		case FILE_ACTION_RENAMED_OLD_NAME:
			{
				...
				DRX_ASSERT(!renameFromMap.contains(path));
		...
		case FILE_ACTION_RENAMED_NEW_NAME:
			{
				...
				DRX_ASSERT(it != renameFromMap.end());
...
void CActionMonitor::AddPath(u64 key, const QString& fullAbsolutePath)
{
	p->AddPath(key, fullAbsolutePath);
}

void CActionMonitor::Remove(u64 key)
{
	p->Remove(key);
}
