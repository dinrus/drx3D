...
i32k SNAPSHOT_UPDATE_TIME = 33; // [milliseconds]
...
void CWorker::StartFileMonitor(i32 handle, const SFileFilter& filter, const FileMonitorPtr& monitor)
{
	m_fileMonitors.Add(handle, filter, monitor);
	TriggerFileMonitorActivated(monitor, m_currentSnapshot);
}

void CWorker::StopFileMonitor(i32 handle)
{
	m_fileMonitors.Remove(handle);
}

void CWorker::StartSubTreeMonitor(i32 handle, const SFileFilter& filter, const SubTreeMonitorPtr& monitor)
{
	m_subTreeMonitors.Add(handle, filter, monitor);
	TriggerSubTreeMonitorActivated(monitor, m_currentSnapshot);
}

void CWorker::StopSubTreeMonitor(i32 handle)
{
