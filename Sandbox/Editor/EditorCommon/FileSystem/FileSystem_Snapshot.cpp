...
struct CSnapshot::Implementation
{
	...
	{
		DRX_ASSERT(filter.directoryTokens.isEmpty());
	...
	{
		DRX_ASSERT(1 == filter.directoryTokens.count());
	...
	{
		DRX_ASSERT(filter.fileExtensions.isEmpty() && filter.fileTokens.isEmpty());
	...
	{
		DRX_ASSERT(!filter.fileExtensions.isEmpty());
		QSet<ukk> found;
		auto indexedDirectory = reinterpret_cast<const Internal::SIndexedDirectory*>(directory.get());
	...
	{
		DRX_ASSERT(!filter.fileTokens.isEmpty());
		QSet<ukk> found;
		auto indexedDirectory = reinterpret_cast<const Internal::SIndexedDirectory*>(directory.get());
