...
struct CFileTreeModel::SImplementation
{
	...
	{
		DRX_ASSERT(!m_snapshot || update.from == m_snapshot);
	...
	void SetSubTreeRoot(const FileSystem::SSubTree& subTree)
	{
		...
		{
			DRX_ASSERT(m_directoryItems.contains(m_rootItem->directory));
			m_directoryItems.remove(m_rootItem->directory);
		}
		m_rootItem->directory = rootDir;
		DRX_ASSERT(!m_directoryItems.contains(rootDir));
	...
	void ApplySubTreeChangeRoot(const FileSystem::SSubTreeChange& rootChange)
	{
		...
		DRX_ASSERT(!m_directoryItems.contains(rootChange.to));
	...
	void SetSubTreeChildren(const FileSystem::SSubTree& subtree, SItem* item)
	{
		...
		{
			DRX_ASSERT(m_directoryItems.contains(subDirectoryItem->directory));
	...
	{
		DRX_ASSERT(m_directoryItems.contains(subtreeChange.from));
		DRX_ASSERT(!m_directoryItems.contains(subtreeChange.to));
		...
		if (subtreeChange.from->provider->fullName != subtreeChange.to->provider->fullName)
		{
			...
			m_pModel->dataChanged(index, index, QVector<i32>() << Qt::DisplayRole);
		}
	...
	{
		DRX_ASSERT(m_fileItems.contains(fileChange.from));
		DRX_ASSERT(!m_fileItems.contains(fileChange.to));
	...
	void RemoveDirectories(SItem* parent, const QSet<FileSystem::DirectoryPtr>& directories)
	{
		...
		i32 i = parent->directories.size() - 1;
		while (i >= 0)
		{
	...
	void RemoveDirectoryWithoutSignals(SItem* directoryItem)
	{
		...
		DRX_ASSERT(m_directoryItems.contains(directoryItem->directory));
	...
	void DropChildren(SItem* directoryItem)
	{
		...
		{
			DRX_ASSERT(m_fileItems.contains(fileItem->file));
	...
	{
		i32 row = parent->directories.size();
	...
	{
		DRX_ASSERT(directoryItem->directory);
		DRX_ASSERT(!m_directoryItems.contains(directoryItem->directory));
	...
	QVariant GetDirectoryData(const FileSystem::DirectoryPtr& pDirectory, i32 role, i32 column) const
	{
		if (!pDirectory)
		{
			return QVariant();
	...
	QVariant GetFileData(const FileSystem::FilePtr& pFile, i32 role, i32 column) const
	{
		if (!pFile)
		{
			return QVariant();
	...
	{
		DRX_ASSERT(fileType); // a filetype should always be present
		if (!fileType)
		{
			return QString();
		}
	...
	static DrxIcon GetFileTypeIcon(const SFileType* fileType)
	{
		DRX_ASSERT(fileType); // a filetype should always be present
		if (!fileType)
		{
		...
		return DrxIcon(fileType->iconPath);
	}
	...
	QModelIndex GetIndex(SItem* item) const
	{
		...
		auto internalPtr = *reinterpret_cast<uk *>(&item);
...
QModelIndex CFileTreeModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
	if (row < 0 || column < 0)
	{
		return QModelIndex();
	...
	auto internalPtr = static_cast<uk>(childItem);
...
i32 CFileTreeModel::rowCount(const QModelIndex& parent) const
{
...
i32 CFileTreeModel::columnCount(const QModelIndex& /*parent*/) const
{
	return eColumn_Count;
}

QVariant CFileTreeModel::data(const QModelIndex& index, i32 role) const
{
	if (!index.isValid())
	{
		return QVariant();
...
QVariant CFileTreeModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
