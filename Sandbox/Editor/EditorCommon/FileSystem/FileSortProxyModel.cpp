...
static inline QChar getNextChar(const QString& s, i32 index)
{
	return (index < s.length()) ? s.at(index) : QChar();
}

i32 naturalCompare(const QString& left, const QString& right)
{
	auto leftCount = left.count();
	auto rightCount = right.count();
	for (i32 iLeft = 0, iRight = 0; iLeft <= leftCount && iRight <= rightCount; ++iLeft, ++iRight)
	{
		...
		if (leftChar.isDigit() && rightChar.isDigit())
		{
			...
			i32 lookAheadLocationLeft = iLeft;
			i32 lookAheadLocationRight = iRight;
			i32 currentReturnValue = 0;
		...
		i32 r = QString::localeAwareCompare(leftChar, rightChar);
		if (r < 0)
		{
			return -1;
		}
...
} //endns Private_FileSortProxyModel
	...
	case CFileTreeModel::eColumn_Type:
		{
			...
			i32 typeDifference = QString::localeAwareCompare(leftType, rightType);
			if (typeDifference != 0)
			{
