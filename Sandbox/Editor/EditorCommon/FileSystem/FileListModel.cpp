...
{
	for (i32 i = 0; i < enginePaths.count(); i++)
	{
...
i32 CFileListModel::FindEntry(const QString& enginePath) const
{
...
i32 CFileListModel::EntriesCount() const
{
...
QString CFileListModel::GetEntry(i32 index) const
{
...
i32 CFileListModel::rowCount(const QModelIndex& /*parent*/) const
{
	return m_entries.size();
}

QVariant CFileListModel::data(const QModelIndex& index, i32 role) const
{
	if (!index.isValid())
	{
		return QVariant();
	...
	case Qt::DecorationRole:
			if(entry.IsFile())
			{
				...
				return DrxIcon(pType->iconPath);
			}
			else
			{
				return DrxIcon(SFileType::DirectoryType()->iconPath);
			}
...
QVariant CFileListModel::headerData(i32 /*section*/, Qt::Orientation orientation, i32 role) const
{
...
bool CFileListModel::canDropMimeData(const QMimeData *data, Qt::DropAction action, i32 row, i32 column, const QModelIndex &parent) const
{
...
bool CFileListModel::dropMimeData(const QMimeData *data, Qt::DropAction action, i32 row, i32 column, const QModelIndex &parent)
{
	i32 insertRow = -1;
