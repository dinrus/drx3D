...
public:
	...
	static SourceIndex CreateSourceChildIndex(
	  i32 sourceRow, i32 sourceColumn,
	  SourceModelMount mount,
	  const QModelIndex& parent,
	  const QAbstractItemModel* mainModel)
	{
		// TODO: remap column?
		DRX_ASSERT(sourceRow >= 0);
		DRX_ASSERT(sourceColumn >= 0);
	...
	bool IsMountedSourceMapped(const SourceModel* sourceModel, const SourceIndex& sourceIndex) const
	{
		if (sourceIndex.isValid())
		{
			...
			DRX_ASSERT(sourceIndex.model() == sourceModel);
	...
	SourceMapping GetMappingFromSource(const SourceIndex& sourceIndex) const
	{
		...
		DRX_ASSERT(it != m_sourceMapping.constEnd());
		return it;
	}
	...
	void EnsureMappingForSourceParent(const SourceIndex& sourceIndex) const
	{
		...
		if (!parentIndex.isValid())
		{
			...
			DRX_ASSERT(mit != m_sourceModelMounts.constEnd());
	...
	ProxyIndex CreateIndexFromMapping(i32 proxyRow, i32 proxyColumn, const SourceMapping& it) const
	{
	...
	SourceIndex GetMountedSourceIndex(const SourceModel* sourceModel, const SourceIndex& sourceIndex) const
	{
		if (sourceIndex.isValid())
		{
			DRX_ASSERT(sourceModel == sourceIndex.model());
			return sourceIndex;
		}
		DRX_ASSERT(nullptr != sourceModel);
		DRX_ASSERT(m_sourceModelMounts.contains(sourceModel));
	...
	ProxyIndex GetMountedProxyIndex(const SourceModel* sourceModel, const SourceIndex& sourceIndex) const
	{
		...
		DRX_ASSERT(mountedSourceIndex.isValid() || sourceModel == m_pModel->GetSourceModel());
	...
	{
		DRX_ASSERT(!m_sourceModelMounts.isEmpty());  // DO not call this unless you have a model mounted!
	...
	SourceModelMount CreateMountPoint(const SourceIndex& sourceIndex, SourceModel* sourceModel)
	{
		...
		DRX_ASSERT(!m_sourceModelMounts.contains(sourceModel)); // each submodel can only be mounted once!
	...
	{
		DRX_ASSERT(mount != m_sourceModelMounts.end());
	...
	void CollectSourceMappings(SourceMappingVector& sourceMappings, const SourceModel* sourceModel, const SourceIndex& sourceParent, i32 first, i32 end) const
	{
		for (i32 row = first; row < end; ++row)
		{
	...
	void MoveSourceMapping(const SourceMapping& it, const SourceIndex& newSourceIndex)
	{
		...
		{
			DRX_ASSERT(mapping.mount->sourceIndex == newSourceIndex);
		}
	}
	...
	template<typename F>
	void MoveSourceIndexes(const SourceMappingVector& sourceMappings, i32 rowDrift, F&& f)
	{
	...
	void DeepRemoveSourceMappings(const SourceModel* sourceModel, const SourceIndex& sourceParent, i32 first, i32 end)
	{
		for (i32 row = first; row < end; ++row)
		{
			auto sourceIndex = sourceModel->index(row, 0, sourceParent);
	...
	{
		DRX_ASSERT(SourceModelMount() != mount);
...
public:
	...
	virtual void onSourceRowsAboutToBeInserted(const SourceModel* sourceModel, const QModelIndex& sourceParent, i32 first, i32 last) override
	{
	...
	virtual void onSourceRowsAboutToBeRemoved(const SourceModel* sourceModel, const QModelIndex& sourceParent, i32 first, i32 last) override
	{
		auto proxyParent = GetMountedProxyIndex(sourceModel, sourceParent);
		SetBusy(true);
...
void CMountingProxyModel::SetHeaderDataCallbacks(i32 columnCount, const GetHeaderDataCallback& getHeaderData, i32 mergeValueRole, const SetHeaderDataCallback& setHeaderData)
{
	beginResetModel();
...
const QAbstractItemModel* CMountingProxyModel::GetSourceModel() const
{
	...
	{
		DRX_ASSERT(p->m_sourceMapping.isEmpty());
		return nullptr;
	}
...
CMountingProxyModel::SourceIndex CMountingProxyModel::MapToSource(const ProxyIndex& proxyIndex) const
{
	...
	auto it = p->GetMappingFromProxy(proxyIndex);
	auto sourceIndex = it.key();
	DRX_ASSERT(sourceIndex.row() == proxyIndex.row());
	DRX_ASSERT(sourceIndex.column() == 0);
...
CMountingProxyModel::ProxyIndex CMountingProxyModel::MapFromSource(const SourceIndex& sourceIndex) const
{
	...
	DRX_ASSERT(proxyRow == it.key().row());
...
QModelIndex CMountingProxyModel::index(i32 proxyRow, i32 proxyColumn, const QModelIndex& proxyParent) const
{
	if (proxyRow < 0 || proxyColumn < 0)
	{
		return QModelIndex();
...
QModelIndex CMountingProxyModel::sibling(i32 proxyRow, i32 proxyColumn, const QModelIndex& proxyIndex) const
{
...
i32 CMountingProxyModel::rowCount(const QModelIndex& parent) const
{
	...
	DRX_ASSERT(sourceParent.isValid());
	return sourceParent.model()->rowCount(sourceParent);
}

i32 CMountingProxyModel::columnCount(const QModelIndex&) const
{
...
QVariant CMountingProxyModel::data(const QModelIndex& proxyIndex, i32 role) const
{
	if (!proxyIndex.isValid())
	{
...
bool CMountingProxyModel::setData(const QModelIndex& proxyIndex, const QVariant& value, i32 role)
{
	if (!proxyIndex.isValid())
	{
...
QVariant CMountingProxyModel::headerData(i32 proxySection, Qt::Orientation orientation, i32 role) const
{
...
bool CMountingProxyModel::setHeaderData(i32 proxySection, Qt::Orientation orientation, const QVariant& value, i32 role)
{
	if (orientation == Qt::Horizontal)
	{
		if (role == p->m_columnMapping.GetValueRole())
...
QMap<i32, QVariant> CMountingProxyModel::itemData(const QModelIndex& proxyIndex) const
{
	if (!proxyIndex.isValid())
	{
...
bool CMountingProxyModel::setItemData(const QModelIndex& proxyIndex, const QMap<i32, QVariant>& roles)
{
	if (!proxyIndex.isValid())
	{
		return false;
...
bool CMountingProxyModel::canDropMimeData(const QMimeData* data, Qt::DropAction action, i32 proxyRow, i32 proxyColumn, const QModelIndex& proxyParent) const
{
...
bool CMountingProxyModel::dropMimeData(const QMimeData* data, Qt::DropAction action, i32 proxyRow, i32 proxyColumn, const QModelIndex& proxyParent)
{
	i32 sourceRow, sourceColumn;
	QModelIndex sourceParent;
	const SourceModel* sourceModel;
...
void CMountingProxyModel::sort(i32 proxyColumn, Qt::SortOrder order)
{
...
QHash<i32, QByteArray> CMountingProxyModel::roleNames() const
{
...
void CMountingProxyModel::ColumnsChanged(i32 columnCount)
{
	beginResetModel();
