...
struct QDeepFilterProxyModel::Implementation
{
	...
	void ConnectModel(QAbstractItemModel* sourceModel)
	{
		...
		m_connections << connect(sourceModel, &QAbstractItemModel::rowsMoved, m_pModel, [this](const QModelIndex& sourceParent, i32, i32, const QModelIndex& destinationParent)
		{
		...
		m_connections << connect(sourceModel, &QAbstractItemModel::dataChanged, m_pModel, [this](const QModelIndex& topLeft, const QModelIndex& bottomRight)
		{
			...
			DRX_ASSERT(sourceParent == bottomRight.parent());
			DRX_ASSERT(topLeft.row() <= bottomRight.row());
	...
	void UpdateSourceRows(const QModelIndex& sourceParent, i32 firstRow, i32 lastRow)
	{
		if (m_behavior == 0)
		{
	...
	const SFilterResult& FetchFilterResult(i32 sourceRow, const QModelIndex& sourceParent, BehaviorFlags behavior) const
	{
		auto pSourceModel = m_pModel->sourceModel();
		auto sourceIndex = pSourceModel->index(sourceRow, 0, sourceParent);
	...
	void UpdateFilterResult(SFilterResult& result, const QModelIndex& sourceIndex, const QModelIndex& sourceParent, BehaviorFlags behavior) const
	{
		...
		if (behavior.testFlag(AcceptIfParentMatches) && !result.wasTested(AcceptIfParentMatches))
		{
			...
			if (HasAcceptedParents(sourceParent))
			{
				...
				DRX_ASSERT(result.hasMatch(behavior));
				return;
			}
		}
	...
	bool HasAcceptedChildren(const QModelIndex& sourceIndex) const
	{
		...
		i32k childRows = sourceIndex.model()->rowCount(sourceIndex);
		if (childRows == 0)
		{
			return false;
		}
		for (i32 childRow = 0; childRow < childRows; ++childRow)
		{
...
QVariant QDeepFilterProxyModel::data(const QModelIndex& proxyIndex, i32 role) const
{
...
bool QDeepFilterProxyModel::filterAcceptsRow(i32 sourceRow, const QModelIndex& sourceParent) const
{
...
bool QDeepFilterProxyModel::rowMatchesFilter(i32 sourceRow, const QModelIndex& sourceParent) const
{
	if (p->m_searchTokens.size())
	{
		...
		{
			i32 columnCount = model->columnCount(sourceParent);
			for (i32 column = 0; column < columnCount; column++)
			{
