...
CMultiProxyColumnMapping::CMultiProxyColumnMapping(const ColumnValues& columnValues, i32 valueRole)
{
...
void CMultiProxyColumnMapping::Reset(const ColumnValues& columnValues, i32 valueRole)
{
...
void CMultiProxyColumnMapping::InsertColumns(i32 firstProxyColumn, const ColumnValues& columnValues)
{
	DRX_ASSERT(firstProxyColumn >= 0 && firstProxyColumn <= m_proxyColumns.size());
...
void CMultiProxyColumnMapping::RemoveColumns(i32 firstProxyColumn, i32 count)
{
	DRX_ASSERT(firstProxyColumn >= 0);
	DRX_ASSERT(count >= 0);
	DRX_ASSERT(firstProxyColumn + count < m_proxyColumns.size());
	if (0 <= count)
	{
