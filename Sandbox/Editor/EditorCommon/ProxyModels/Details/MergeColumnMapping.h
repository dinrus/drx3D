...
public:
	...
	MountDataAndNewColumns Mount(SourceModel* sourceModel) const
	{
		...
		if (!HasMapping())
		{
			...
			for (i32 i = 0; i < sourceColumnCount; i++)
			{
				result.mountData.m_sourceToProxy << i;
				result.mountData.m_proxyToSource[i] = i;
			}

			i32k addRefCount = std::min(ColumnCount(), sourceColumnCount);
			for (i32 i = 0; i < addRefCount; i++)
			{
				m_columns[i]->AddRef();
			}
	...
	void RemoveColumns(ProxyColumn proxyColumn, i32 columnCount)
	{
