	...
	: public CAbstractMultiProxyModelHelper
{
	...
	struct MountPoint
	{
		...
		i32                rootRowFirst; // first row index in the root node
		i32                rootRowLast;  // last row index in the root node
	};
...
public:
	Implementation(CMergingProxyModel* pModel)
		: m_pModel(pModel)
		, m_getHeaderDataCallback([](i32, Qt::Orientation, i32){ return QVariant(); })
		, m_setHeaderDataCallback([](i32, Qt::Orientation, const QVariant&, i32){ return false; })
	...
	{
		DRX_ASSERT(sourceIndex.isValid());
		DRX_ASSERT(0 == sourceIndex.column());
	...
	void MapDropCoordinatesToSource(i32 proxyRow, i32 proxyColumn, const QModelIndex& proxyParent,
	                                i32* sourceRow, i32* sourceColumn, QModelIndex* sourceParent, const SourceModel** sourceModel) const
	{
		*sourceRow = -1;
		*sourceColumn = -1;
	...
	void DeepRemoveSourceMappingForChildrenBetween(const SourceModel* sourceModel, const SourceIndex& sourceParent, i32 first, i32 last) const
	{
	...
	void CollectSourceMappingForChildenFrom(SourceMappingVector& storedSourceMappings, const SourceModel* sourceModel, const SourceIndex& sourceParent, i32 first) const
	{
		auto last = sourceModel->rowCount(sourceParent) - 1;
		for (auto row = first; row <= last; ++row)
		{
	...
	void MoveSourceIndexes(const SourceMappingVector& sourceMappings, const SourceModel* sourceModel, const SourceIndex& sourceParent, i32 rowDrift)
	{
		DRX_ASSERT(0 != rowDrift);
		if (rowDrift > 0)
		{
	...
	MappingIterator GetMappingFromProxy(const ProxyIndex& proxyIndex) const
	{
		...
		DRX_ASSERT(m_sourceMapping.contains(*it));
		return it;
	}

	i32 MapColumnFromProxy(const SourceModel* sourceModel, i32 proxyColumn) const
	{
	...
	i32 MapColumnFromSource(const SourceModel* sourceModel, i32 sourceColumn) const
	{
		auto mount = GetMountForSourceModel(sourceModel);
	...
	i32 MapChildRowFromSource(const SourceModel* sourceModel, const SourceIndex& sourceParent, i32 row) const
	{
		DRX_ASSERT(sourceModel);
		DRX_ASSERT(m_sourceModelMounts.contains(sourceModel));
		DRX_ASSERT(row >= 0);
		if (sourceParent.isValid())
		{
	...
	i32 MapRowFromSource(const SourceIndex& sourceIndex) const
	{
		DRX_ASSERT(sourceIndex.isValid());
	...
	SourceModelMount GetMountForSourceModel(const SourceModel* sourceModel)
	{
		...
		DRX_ASSERT(it != m_sourceModelMounts.end());
		return it;
	}
	...
	SourceModelMount GetMountFromProxyRow(i32 proxyRow) const
	{
		DRX_ASSERT(m_mountPoints.empty() || (*m_mountPoints.begin())->rootRowFirst == 0);
		DRX_ASSERT(proxyRow >= 0);
		auto it = std::upper_bound(m_mountPoints.begin(), m_mountPoints.end(), proxyRow, [](i32 row, const SourceModelMount& mount)
		{
	...
	SourceModelMount FindMountFromProxyColumn(i32 proxyColumn) const
	{
	...
	ProxyIndex CreateIndexFromMapping(i32 row, i32 column, MappingIterator it) const
	{
		if (row < 0 || column < 0)
		{
			return ProxyIndex();
	...
	void SetMountRowCount(const SourceModelMount& it, i32 rowCount)
	{
		i32 oldRowCount = 1 + it->rootRowLast - it->rootRowFirst;
		i32 rowDifference = rowCount - oldRowCount;
		if (0 == rowDifference)
		{
			return; // not changed
		}
	...
	void MoveMountsAfter(const SourceModelMount& it, i32 rowDifference)
	{
...
public:
	...
	virtual void onSourceDataChanged(const QModelIndex& sourceTopLeft, const QModelIndex& sourceBottomRight, const QVector<i32>& roles) override
	{
		DRX_ASSERT(sourceTopLeft.isValid());
		DRX_ASSERT(sourceBottomRight.isValid());
		DRX_ASSERT(sourceTopLeft.parent() == sourceBottomRight.parent());
		DRX_ASSERT(sourceTopLeft.column() <= sourceBottomRight.column());
		...
		QPair<i32, i32> minMaxProxyColumn = qMakePair(INT_MAX, -1);
	...
	virtual void onSourceRowsAboutToBeInserted(const SourceModel* sourceModel, const QModelIndex& sourceParent, i32 sourceFirst, i32 sourceLast) override
	{
		DRX_ASSERT(m_sourceModelMounts.contains(sourceModel));
		DRX_ASSERT(sourceFirst >= 0);
		DRX_ASSERT(sourceFirst <= sourceLast);
	...
	virtual void onSourceRowsInserted(const SourceModel* sourceModel, const QModelIndex& sourceParent, i32 first, i32 last) override
	{
	...
	virtual void onSourceRowsAboutToBeRemoved(const SourceModel* sourceModel, const QModelIndex& sourceParent, i32 sourceFirst, i32 sourceLast) override
	{
		DRX_ASSERT(m_sourceModelMounts.contains(sourceModel));
		DRX_ASSERT(sourceFirst >= 0);
		DRX_ASSERT(sourceFirst <= sourceLast);
	...
	virtual void onSourceRowsRemoved(const SourceModel* sourceModel, const QModelIndex& sourceParent, i32 first, i32 last) override
	{
		auto rowDrift = 1 + last - first;
		MoveSourceIndexes(m_storedSourceMappings, sourceModel, sourceParent, -rowDrift);
		m_storedSourceMappings.clear();
	...
	virtual void onSourceRowsAboutToBeMoved(
	  ...
	  const QModelIndex& sourceParent, i32 sourceStart, i32 sourceEnd,
	  const QModelIndex& destinationParent, i32 destinationRow) override
	{
		DRX_ASSERT(m_sourceModelMounts.contains(sourceModel));
		DRX_ASSERT(sourceStart >= 0);
		DRX_ASSERT(sourceStart <= sourceEnd);
		DRX_ASSERT(destinationRow >= 0);
...
i32 CMergingProxyModel::GetSourceModelCount() const
{
	return p->m_mountPoints.count();
}

const CMergingProxyModel::SourceModel* CMergingProxyModel::GetSourceModel(i32 index) const
{
...
void CMergingProxyModel::Mount(SourceModel* sourceModel, i32 index)
{
	...
	i32 firstRow = index <= 0 ? 0 : (index < p->m_mountPoints.size() ? p->m_mountPoints[index]->rootRowFirst : rowCount());
	if (0 < mountRows)
	{
		i32 lastRow = firstRow + mountRows - 1;
...
void CMergingProxyModel::SetHeaderDataCallbacks(i32 columnCount, const GetHeaderDataCallback& getHeaderData, i32 mergeValueRole, const SetHeaderDataCallback& setHeaderData)
{
	beginResetModel();
	...
	{
		p->m_setHeaderDataCallback = [](i32, Qt::Orientation, const QVariant&, i32){ return false; };
	}
...
void CMergingProxyModel::ColumnsChanged(i32 columnCount)
{
	beginResetModel();
...
QModelIndex CMergingProxyModel::index(i32 proxyRow, i32 proxyColumn, const QModelIndex& proxyParent) const
{
...
QModelIndex CMergingProxyModel::sibling(i32 proxyRow, i32 proxyColumn, const QModelIndex& proxyIndex) const
{
...
i32 CMergingProxyModel::rowCount(const QModelIndex& parent) const
{
	if (parent.isValid())
	{
		if (parent.column() != 0)
...
i32 CMergingProxyModel::columnCount(const QModelIndex&) const
{
...
QVariant CMergingProxyModel::data(const QModelIndex& proxyIndex, i32 role) const
{
	if (!proxyIndex.isValid())
	{
		if (p->m_mountPoints.isEmpty())
...
bool CMergingProxyModel::setData(const QModelIndex& proxyIndex, const QVariant& value, i32 role)
{
	if (!proxyIndex.isValid())
	{
		if (p->m_mountPoints.isEmpty())
...
QVariant CMergingProxyModel::headerData(i32 proxySection, Qt::Orientation orientation, i32 role) const
{
...
bool CMergingProxyModel::setHeaderData(i32 proxySection, Qt::Orientation orientation, const QVariant& value, i32 role)
{
	if (orientation == Qt::Horizontal)
	{
		if (role == p->m_columnMapping.GetValueRole())
...
QMap<i32, QVariant> CMergingProxyModel::itemData(const QModelIndex& proxyIndex) const
{
	if (!proxyIndex.isValid())
	{
		return QMap<i32, QVariant>();
	}
	auto sourceIndex = MapToSource(proxyIndex);
	return sourceIndex.model()->itemData(sourceIndex);
}

bool CMergingProxyModel::setItemData(const QModelIndex& proxyIndex, const QMap<i32, QVariant>& roles)
{
	if (!proxyIndex.isValid())
	{
		return false;
...
bool CMergingProxyModel::canDropMimeData(const QMimeData* data, Qt::DropAction action, i32 proxyRow, i32 proxyColumn, const QModelIndex& proxyParent) const
{
	i32 sourceRow, sourceColumn;
...
bool CMergingProxyModel::dropMimeData(const QMimeData* data, Qt::DropAction action, i32 proxyRow, i32 proxyColumn, const QModelIndex& proxyParent)
{
	i32 sourceRow, sourceColumn;
	QModelIndex sourceParent;
	const SourceModel* sourceModel;
...
void CMergingProxyModel::sort(i32 proxyColumn, Qt::SortOrder order)
{
...
QHash<i32, QByteArray> CMergingProxyModel::roleNames() const
{
	QHash<i32, QByteArray> result;
	foreach(const auto & mount, p->m_mountPoints)
	{
