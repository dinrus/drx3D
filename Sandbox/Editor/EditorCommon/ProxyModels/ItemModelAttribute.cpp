...
CItemModelAttributeEnum::CItemModelAttributeEnum(tukk szName, QStringList enumEntries, Visibility visibility, bool filterable, QVariant defaultFilterValue)
	: CItemModelAttribute(szName, eAttributeType_Enum, visibility, filterable, defaultFilterValue)
	, m_enumEntries(enumEntries)
{}

CItemModelAttributeEnum::CItemModelAttributeEnum(tukk szName, tuk* enumEntries, i32 enumEntryCount, Visibility visibility, bool filterable, QVariant defaultFilterValue)
	: CItemModelAttribute(szName, eAttributeType_Enum, visibility, filterable, defaultFilterValue)
{
	for (i32 i = 0; i < enumEntryCount; i++)
	{
		m_enumEntries += enumEntries[i];
	}
}

CItemModelAttributeEnum::CItemModelAttributeEnum(tukk szName, tuk commaSeparatedString, Visibility visibility, bool filterable, QVariant defaultFilterValue)
	: CItemModelAttribute(szName, eAttributeType_Enum, visibility, filterable, defaultFilterValue)
{
...
CItemModelAttributeEnumFunc::CItemModelAttributeEnumFunc(tukk szName, std::function<QStringList()> populateFunc, Visibility visibility /*= Visibility::Visible*/, bool filterable /*= true*/, QVariant defaultFilterValue /*= QVariant()*/)
...
public:
	QWidget* CreateEditWidget(std::shared_ptr<CAttributeFilter> filter) override
	{
		...
		if (currentValue.isValid())
			widget->setVal(currentValue.toInt());
		else
			widget->setVal(0);

		QWidget::connect(widget, &QNumericBox::valueSubmitted, [=]()
		{
			filter->SetFilterValue((i32)widget->value());
		});
...
public:
	QWidget* CreateEditWidget(std::shared_ptr<CAttributeFilter> filter) override
	{
		...
		if (currentValue.isValid())
			widget->setVal(currentValue.toDouble());
		else
			widget->setVal(0);
	...
	void UpdateWidget(QWidget* widget, const QVariant& value) override
	{
		QNumericBox* editWidget = qobject_cast<QNumericBox*>(widget);
		if (editWidget)
			editWidget->setVal(value.toDouble());
	}
};
...
public:
	...
	virtual QWidget* CreateEditWidget(std::shared_ptr<CAttributeFilter> filter) override
	{
		...
		QWidget::connect(widget, &QCheckBox::stateChanged, [=](i32 newState)
			{
				switch (newState)
				{
				case Qt::Unchecked:
...
public:
	...
	virtual bool    Match(const QVariant& value, const QVariant& filterValue) override
	{
		//TODO : handle do match by integer value if value is a number : implement attribute convert i32 to string and match string
