...
EDITOR_COMMON_API bool Remove(tukk szPath);
//! Removes the file at the given file path
EDITOR_COMMON_API bool RemoveFile(tukk szFilePath);
//! Moves the file at the given file path to the new file path allowing overwriting of the target.
EDITOR_COMMON_API bool MoveFileAllowOverwrite(tukk szOldFilePath, tukk szNewFilePath);
...
EDITOR_COMMON_API bool RemoveDirectory(tukk szPath, bool bRecursive = true);
...
//! \see IDrxPak::OpenPack()
EDITOR_COMMON_API void Unpak(tukk szArchivePath, tukk szDestPath, std::function<void(float)> progress);
...
EDITOR_COMMON_API QString ReplaceExtension(const QString& str, tukk ext);
...
EDITOR_COMMON_API string AbsoluteToRelativePath(const string& absolutePath, tukk dirPathRelativeTo);

//! DEPRECATED: Convert an absolute path to a patch that is relative to the project root directory
//! Prefer using ToGamePath which will always convert a path to a game path, which can be used in DrxPak methods as well
//! Example: <project_root>/<sys_game_folder>/Objects/box.cgf -> <sys_game_folder>/Objects/box.cgf
EDITOR_COMMON_API string AbsolutePathToDrxPakPath(const string& path);
...
//! DEPRECATED: Converts a path relative to the game's asset folder to a path that can be used in DrxPak.
//! This method should not really be used considering DrxPak itself will adjust assets-relative paths to be opened using the same implementation,
...
EDITOR_COMMON_API string GamePathToDrxPakPath(const string& path, bool bForWriting = false);
...
EDITOR_COMMON_API string ToGamePath(tukk path);

EDITOR_COMMON_API QString ToUnixPath(const QString& path);

//! Returns true if the file exists on disk (does not use DrxPak)
