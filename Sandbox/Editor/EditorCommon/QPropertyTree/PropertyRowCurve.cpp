...
#include <drx3D/CoreX/Serialization/yasli/TextOArchive.h>
...
void PropertyRowCurve::setValueAndContext(const Serialization::SStruct& ser, Serialization::IArchive& ar)
{
	...
	tukk szCurveDomain = ar.context<const char>();
...
bool PropertyRowCurve::assignTo(const Serialization::SStruct& ser) const
{
	...
	else
	{
		// Multiple curves
		auto multiSpline = (IMultiSplineEvaluator*)ser.pointer();
		i32 count = min<i32>(multiSpline->GetNumSplines(), curves().size());
...
void PropertyRowCurve::redraw(property_tree::IDrawContext& context)
{
	...
	auto setPoint = [=](QPointF& point, TDrawSpline& spline, i32 p)
	{
...
{
	DRX_ASSERT_MESSAGE(editorContent_, "Editor content not initialized");
	return editorContent_;
}
