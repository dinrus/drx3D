...
QString convertMFCToQtFileFilter(QString* defaultSuffix, tukk mfcFilter)
{
	...
	QString filter;
	for (i32 i = 0; i < i32(filterItems.size()) / 2; ++i)
	{
		i32 bracketPos = filterItems[i].indexOf('(');
...
Icon PropertyRowOutputFilePath::buttonIcon(const PropertyTree* tree, i32 index) const
{
