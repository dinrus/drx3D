...
bool PropertyRowResourceFilePath::onActivateButton(i32 buttonIndex, const PropertyActivationEvent& e)
{
	if (userReadOnly())
		return false;
	if (!GetIEditor())
	...
	setVal(filename, searchHandle(), typeId());
...
void PropertyRowResourceFilePath::setValueAndContext(const Serialization::SStruct& ser, Serialization::IArchive& ar)
{
	...
	setVal(value->path->c_str(), searchHandle(), typeId());
	flags_ = value->flags;
	handle_ = value->path;
}
...
void PropertyRowResourceFilePath::serializeValue(Serialization::IArchive& ar)
{
	...
	ar(path, "path");
	setVal(path, searchHandle(), typeId());

	ar(startFolder_, "startFolder");
}

Icon PropertyRowResourceFilePath::buttonIcon(const PropertyTree* tree, i32 index) const
{
