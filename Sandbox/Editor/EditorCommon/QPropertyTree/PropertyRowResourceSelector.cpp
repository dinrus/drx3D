...
#include <DrxAnimation/IDrxAnimation.h> // for ICharacterInstance in context
...
{
public:
	InplaceWidgetResource(PropertyRowResourceSelector* row, QPropertyTree* tree, i32 width)
		: InplaceWidget(tree)
		...
		, row_(row)
	{
		...
		connect(entry_.data(), &QLineEdit::textChanged, this, [this, tree] {
			...
			i32 contentWidth = min((i32)fm.width(entry_->text()) + 8, tree->width() - entry_->x());
	...
	uk actualWidget() { return entry_.data(); }
...
protected:
	void onEditingFinished()
	{
		if (initialValue_ != entry_->text() || row_->multiValue())
		{
			...
			row_->setVal(tree_, text.toUtf8().constData(), row_->searchHandle(), row_->typeId());
...
{
	DRX_ASSERT(GetIEditor() && GetIEditor()->GetResourceSelectorHost());
}

property_tree::InplaceWidget* PropertyRowResourceSelector::createWidget(PropertyTree* tree)
{
	i32 buttonsWidth = buttonCount() * 16;
	i32 iconSpace = buttonsWidth ? buttonsWidth + 2 : 0;
	i32 widgetWidth = max(16, widgetRect(tree).width() - buttonsWidth - 2);
...
void PropertyRowResourceSelector::setVal(PropertyTree* tree, tukk str, ukk handle, const yasli::TypeID& type)
{
	DRX_ASSERT(selector_);
	string value = value_;
...
bool PropertyRowResourceSelector::onDataDrop(PropertyTree* tree)
{
	...
	if (value() != filePath || multiValue())
	{
		...
		setVal(tree, filePath.toUtf8().constData(), searchHandle(), typeId());
...
i32 PropertyRowResourceSelector::buttonCount() const
{
	if (!provider_)
	{
		DRX_ASSERT(selector_);
...
bool PropertyRowResourceSelector::onActivateButton(i32 button, const PropertyActivationEvent& e)
{
	if (userReadOnly())
		return false;
...
public:
	...
	void SetValueChangedCallback(const std::function<void(tukk)>& onValueChanged)
	{
		m_onValueChanged = onValueChanged;
	}

private:
	std::function<void(tukk)> m_onValueChanged;
};

} //endns Private_ResourceSelectorHost
...
bool PropertyRowResourceSelector::pickResource(PropertyTree* tree)
{
	...
	auto setVal = [this, tree](tukk filename)
	{
		tree->model()->rowAboutToBeChanged(this);
		value_ = filename;
		tree->model()->rowChanged(this);
	...
	auto setValueCallback = [setVal](tukk filename)
	{
	...
	{
		setVal(filename.c_str());
	}

	return true;
}

QString convertMFCToQtFileFilter(QString* defaultSuffix, tukk mfcFilter);
...
void PropertyRowResourceSelector::setValueAndContext(const Serialization::SStruct& ser, IArchive& ar)
{
	...
	if (selector_ == nullptr || type_ != value->resourceType)
	{
		...
		DRX_ASSERT(selector_ != nullptr); //A default selector implementation should always be returned, even if no selector is registered
		tukk resourceIconPath = selector_->GetIconPath();
...
Icon PropertyRowResourceSelector::buttonIcon(const PropertyTree* tree, i32 index) const
{
	switch (index)
	{
	case BUTTON_PICK:
...
static tukk getFilenameFromPath(tukk path)
{
	tukk lastSep = strrchr(path, '/');
	if (!lastSep)
		return path;
	return lastSep + 1;
}
...
{
	i32 buttonCount = this->buttonCount();
	i32 offset = 0;
	...
	i32 iconSpace = offset ? offset + 2 : 0;
