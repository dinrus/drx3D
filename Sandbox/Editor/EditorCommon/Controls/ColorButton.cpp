...
void CColorButton::paintEvent(QPaintEvent* paintEvent)
{
	...
	{
		i32k size = 14;
		static i32 pixels[size * size];
		for (i32 i = 0; i < size * size; ++i)
			pixels[i] = ((i / size) / (size / 2) + (i % size) / (size / 2)) % 2 ? 0xffffffff : 0x000000ff;
		checkboardPattern = QImage((u8*)pixels, size, size, size * 4, QImage::Format_RGBA8888);
	}
...
void CColorButton::OnClick()
{
	...
	QObject::connect(picker, &QColorDialog::finished, [initialColor, picker, this](i32 result)
	{
		QVariant colorchanged = picker->property("colorChanged");
		if (DrxGetAsyncKeyState(VK_ESCAPE)) //TODO: avoid using this, an event filter could be good for instance
		{
...

	{
		...
		i32k screen = desktop->screenNumber(this);
