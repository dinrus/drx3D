...
void QPopupWidget::ShowAt(const QPoint& globalPos, Origin origin /* = Unknown*/)
{
	...
	i32 screenRight = screenRes.x() + screenRes.width();
	i32 screenBottom = screenRes.y() + screenRes.height();
...
void QPopupWidget::mouseMoveEvent(QMouseEvent* pEvent)
{
	...
	if (pEvent->buttons() & Qt::LeftButton && !m_mousePressPos.isNull())
	{
		...
		{
			i32 newLeft = currGeometry.left() - delta.x() * m_resizeConstraint.x();
			i32 newWidth = currGeometry.right() - newLeft;
		...
		{
			i32 newRight = currGeometry.right() + delta.x() * m_resizeConstraint.x();
		...
		{
			i32 newTop = currGeometry.top() + delta.y() * m_resizeConstraint.y();
			i32 newHeight = currGeometry.bottom() - newTop;
		...
		{
			i32 newBottom = currGeometry.bottom() - delta.y() * m_resizeConstraint.y();
