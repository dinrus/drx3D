...
{
	static i32k sWidthOffsetDefault = 36;
}
	...
	, m_widthOffset(Private_MenuComboBox::sWidthOffsetDefault)
{
	...
	QAction* showPopupAction = m_lineEdit->addAction(DrxIcon("icons:General/Pointer_Down.ico"), QLineEdit::TrailingPosition);
...
void QMenuComboBox::RemoveItem(i32 index)
{
...
QVariant QMenuComboBox::GetData(i32 index) const
{
	if (index < 0 || index >= m_data.size())
		return QVariant();
...
QString QMenuComboBox::GetItem(i32 index) const
{
	if (index < 0 || index >= m_data.size())
		return QString();
...
i32 QMenuComboBox::GetWidthOffset() const
{
	return m_widthOffset;
}

void QMenuComboBox::SetText(const QString& text, tukk separator /*= ", "*/)
{
	QStringList list = text.split(separator, QString::SkipEmptyParts);

	if (!m_multiSelect)
	{
		DRX_ASSERT(list.size() <= 1);
		if (list.size() == 1)
		{
...
void QMenuComboBox::SetChecked(i32 index, bool checked /*= true*/)
{
	i32k itemCount = GetItemCount();
...
void QMenuComboBox::SetWidthOffset(i32 widthOffset)
{
	m_widthOffset = widthOffset;
}

void QMenuComboBox::SetItemToolTip(i32 index, const QString& toolTip)
{
	if (index < 0 || index >= m_data.size())
	{
		return;
...
void QMenuComboBox::SetItemText(i32 index, const QString& text)
{
	if (index < 0 || index >= m_data.size())
	{
		return;
...
void QMenuComboBox::InternalSetChecked(i32 index, bool checked /*= true*/)
{
...
bool QMenuComboBox::IsChecked(i32 index) const
{
	if (index == -1)
	{
		if (m_emptySelect && !m_multiSelect)
...
QList<i32> QMenuComboBox::GetCheckedIndices() const
{
	QList<i32> ret;
	for (i32 i = 0; i < m_data.length(); i++)
	{
...
i32 QMenuComboBox::GetCheckedItem() const
{
...
void QMenuComboBox::ApplyDelta(i32 delta)
{
	if (m_multiSelect)
	{
		ShowPopup();
...
void QMenuComboBox::OnToggled(i32 index, bool checked)
{
	if (m_multiSelect)
	{
