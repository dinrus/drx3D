...
QSandboxTitleBar::QSandboxTitleBar(QWidget* parent, const QVariantMap& config) : QCustomTitleBar(parent), m_config(config)
{
	if (m_minimizeButton)
	{
		m_minimizeButton->setIcon(getIcon(m_config, SANDBOX_WRAPPER_MINIMIZE_ICON, DrxIcon()));
	}
	if (m_maximizeButton)
	{
		if (parentWidget()->windowState().testFlag(Qt::WindowMaximized))
		{
			m_maximizeButton->setIcon(getIcon(m_config, SANDBOX_WRAPPER_RESTORE_ICON, DrxIcon()));
		}
		else
		{
			m_maximizeButton->setIcon(getIcon(m_config, SANDBOX_WRAPPER_MAXIMIZE_ICON, DrxIcon()));
		}
	}
	m_closeButton->setIcon(getIcon(m_config, SANDBOX_WRAPPER_CLOSE_ICON, DrxIcon()));
}
