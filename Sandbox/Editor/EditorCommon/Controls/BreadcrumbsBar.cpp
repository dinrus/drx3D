...
CBreadcrumbsBar::CBreadcrumbsBar()
{
	...
	m_dropDownButton->setIcon(DrxIcon("icons:General/Pointer_Down.ico"));
...
{
	i32k widgetsCount = m_breadCrumbsLayout->count();
	for (i32 i = 0; i < m_breadcrumbs.count(); ++i)
	{
	...
	for (i32 i = m_breadcrumbs.count(); i < m_breadCrumbsLayout->count(); ++i) 
	{
		QLayoutItem* const pItem = m_breadCrumbsLayout->itemAt(i);
...
void CBreadcrumbsBar::UpdateVisibility()
{
	...
	i32 remainingSize = CBreadcrumbsBar::size().width();
	i32k spacing = m_breadCrumbsLayout->spacing();

	i32k buttonHeight = m_breadcrumbs[0].widget->size().height();
	m_dropDownButton->setMaximumSize(QSize(buttonHeight, buttonHeight));

	bool showDropDown = false;

	i32k count = m_breadcrumbs.count();
	for (i32 i = count - 1; i >= 0; i--)
	{
		auto& bc = m_breadcrumbs[i];
		DRX_ASSERT(bc.widget);
...
void CBreadcrumbsBar::OnBreadcrumbClicked(i32 index)
{
