...
void CVec3Widget::UpdateRow(yasli::TypeID type)
{
	...
	if (m_type != scaleType)
	{
		if (type == scaleType)
		{
			...
			pUniformScaleButton->setIcon(m_uniformScale ? DrxIcon("icons:General/Uniform.ico") : DrxIcon("icons:General/NonUniform.ico"));
...
void CVec3Widget::SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar)
{
	if (m_type != type)
	{
		UpdateRow(type);
...
void CVec3Widget::SetValue(const Vec3& value)
{
	...
	m_pXField->setVal(value.x);
	m_pYField->setVal(value.y);
	m_pZField->setVal(value.z);
	m_ignoreSignals = false;
}

void CVec3Widget::GetValue(uk valuePtr, const yasli::TypeID& type) const
{
	if (type == yasli::TypeID::get<Serialization::SPosition>())
	{
