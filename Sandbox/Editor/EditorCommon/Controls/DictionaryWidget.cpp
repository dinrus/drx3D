...
const CItemModelAttribute* CAbstractDictionary::GetColumnAttribute(i32 index) const
{
...
public:
	...
	virtual i32 CDictionaryModel::columnCount(const QModelIndex& parent) const override
	{
	...
	virtual QVariant CDictionaryModel::data(const QModelIndex& index, i32 role) const override
	{
		if (index.isValid())
		{
	...
	virtual QModelIndex CDictionaryModel::index(i32 row, i32 column, const QModelIndex& parent) const override
	{
		if (!parent.isValid())
		{
		...
		if (index.isValid())
		{
			...
			if (pEntry)
			{
				...
				if (pParentEntry)
				{
					...
					{
						for (i32 i = 0; i < pParentParentEntry->GetNumChildEntries(); ++i)
						{
					...
					{
						for (i32 i = 0; i < m_dictionary.GetNumEntries(); ++i)
						{
...
public:
	static QVariant GetHeaderDataFromDictionary(const CAbstractDictionary& dictionary, i32 section, Qt::Orientation orientation, i32 role)
	{
...
void CDictionaryWidget::AddDictionary(CAbstractDictionary& newDictionary)
{
	...
	i32k filterColumn = newDictionary.GetDefaultFilterColumn();
	if (filterColumn >= 0)
		m_pFilterProxy->setFilterKeyColumn(filterColumn);

	i32k sortColumn = newDictionary.GetDefaultSortColumn();
	if (sortColumn >= 0)
	{
...
QVariant CDictionaryWidget::GeneralHeaderDataCallback(std::vector<CItemModelAttribute*>& columns, i32 section, Qt::Orientation orientation, i32 role)
{
	DRX_ASSERT(section < columns.size());
