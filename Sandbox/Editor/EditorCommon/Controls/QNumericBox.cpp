...
public:
	...
	void        setCursorWidth(i32 width) { m_cursorWidth = width; }

	virtual i32 pixelMetric(PixelMetric metric, const QStyleOption* option = 0, const QWidget* widget = 0) const
	{
...
public:
	...
	virtual void mouseMoveEvent(QMouseEvent* e) override
	{
		if (!getEditMode() && QApplication::mouseButtons() == Qt::LeftButton)
		{
			...
			if (numbox && mouse != m_mouseDownLocation)
			{
				...
				numbox->setVal(numbox->value() + add);
	...
	void step(double multiplier)
	{
		...
		i32 revCursor = text().length() - cursorPosition();
		i32 cursor = stepCursor();

		if (-1 != cursor)
		{
			i32 digitStart = cursor;
			i32 digitLen = 1;
			bool valid = true;

			for (i32 i = 0; i < digitStart; ++i)
			{
				if (' ' == text().at(i))
				{
					continue;
			...
			QString addString;
			i32 digitEnd = digitStart + digitLen;
			...
			i32 decPoint = numString.indexOf('.');
			i32 precision = numString.length() - (decPoint + 1);
	...
	QRect stepCursorRect(i32 cursor)
	{
		i32 currentCursor = cursorPosition();
		if (m_lastText != text() || m_lastCursor != currentCursor)
		{
			i32 start = selectionStart();
			i32 length = selectedText().length();
...
void QNumericButton::timerEvent(QTimerEvent* e)
{
	...
	else if (e->timerId() == m_spinClickTimerId)
	{
		if (m_accelerated)
		{
			m_acceleration = m_acceleration + (i32)(m_effectiveSpinRepeatRate * 0.05);
	...
	, m_precision(6)
{
	...
	m_pUpButton->setIcon(DrxIcon("icons:General/Pointer_Up_Numeric.ico"));
	...
	m_pDownButton->setIcon(DrxIcon("icons:General/Pointer_Down_Numeric.ico"));
...
void QNumericBox::setVal(double value)
{
	value = clamp_tpl(value, m_min, m_max);

	QString outText = QString::number(value, 'f', m_precision);
	i32 revCursor = m_pEdit->text().length() - m_pEdit->cursorPosition();
	i32 decPoint = outText.lastIndexOf('.');

	if (-1 != decPoint)
	{
		for (i32 i = outText.length() - 1; i > decPoint; --i)
		{
			if ('0' == outText.at(i))
			{
				outText.remove(i, 1);
...
void QNumericBox::setPrecision(i32 precision)
{
...
void QNumericBox::step(double multiplier)
{
	...
	setVal(value() + m_step * multiplier);
}
...
void QNumericBox::paintEvent(QPaintEvent* e)
{
	...
	{
		QRect r = rect();
		QRect sliderOverlayRect(r.left(), r.top(), i32(r.width() * sliderPos), r.height());
