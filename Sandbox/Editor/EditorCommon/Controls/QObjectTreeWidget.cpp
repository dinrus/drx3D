...
QObjectTreeWidget::QObjectTreeWidget(QWidget* pParent /*= nullptr*/, tukk szRegExp /* = "[/\\\\.]"*/)
	: QWidget(pParent)
	, m_regExp(szRegExp)
	, m_bIsDragTracked(false)
{
	...
	{
		for (i32 i = indexes.count() - 1; i >= 0; --i)
		{
...
bool QObjectTreeWidget::PathExists(tukk path)
{
...
void QObjectTreeWidget::AddEntry(tukk path, tukk id, tukk sortStr/* = ""*/)
{
...
void QObjectTreeWidget::RemoveEntry(tukk path, tukk id)
{
	if (id)
	{
		TakeItemForId(id);
...
void QObjectTreeWidget::ChangeEntry(tukk path, tukk id, tukk sortStr/* = ""*/)
{
	RemoveEntry(path, id);
