	...
	, m_canClose(true)
{
	...
	}, [this](const QVariant& variant)
	{
		...
		if (!width.canConvert<i32>() || !height.canConvert<i32>())
		{
		  return;
		}
		...
		QTimer::singleShot(0, [size, self] { 
			if (self)
			{
				...
				for (i32 i = 0; i < dw->screenCount(); i++)
				{
	...
	}, [this](const QVariant& variant)
	{
		...
		if (!x.canConvert<i32>() || !y.canConvert<i32>())
		{
			return;
		}
...
bool CEditorDialog::nativeEvent(const QByteArray& eventType, uk message, long* result)
{
#if (defined(WIN32) || defined(WIN64))
