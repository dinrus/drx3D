	...
	, m_nodeWidgetStyle(nodeWidgetStyle)
{
	...
	for (i32 i = 0; i < StyleState_Count; ++i)
	{
...
void CNodeHeaderWidgetStyle::SetNodeIcon(const QIcon& icon)
{
	...
	m_nodeIconMenu = DrxIcon(m_nodeIcon.pixmap(m_nodeIconMenuSize), {
...
void CNodeHeaderWidgetStyle::SetNodeIconMenuColor(const QColor& color)
{
	...
	m_nodeIconMenu = DrxIcon(m_nodeIcon.pixmap(m_nodeIconMenuSize, QIcon::Normal), {
			{ QIcon::Mode::Normal, m_nodeIconMenuColor }
	  });
}
...
void CNodeHeaderWidgetStyle::SetNodeIconViewSize(const QSize& size)
{
	...
	for (i32 i = 0; i < StyleState_Count; ++i)
	{
		m_nodeIconViewPixmap[i] = DrxIcon(iconPixmap, {
...
{
	m_nodeIconViewPixmap[state] = DrxIcon(m_nodeIcon.pixmap(m_nodeIconViewSize, QIcon::Normal), {
