...
void CNodeGraphView::SetZoom(i32 percent)
{
...
void CNodeGraphView::FitSceneInView()
{
	...
	i32k percent(100 * std::min(sx, sy));
	i32k percentClamped(std::max(10, std::min(percent, 100)));
...
void CNodeGraphView::DeselectItem(CAbstractNodeGraphViewModelItem& item)
{
	...
	DRX_ASSERT(m_selectedItems.find(&item) == m_selectedItems.end());
}

void CNodeGraphView::SelectItems(const GraphItemSet& items)
{
	u32 numSelectedItems = 0;
...
void CNodeGraphView::DeselectWidget(CNodeGraphViewGraphicsWidget& itemWidget)
{
	...
	DRX_ASSERT(m_selectedWidgets.find(&itemWidget) == m_selectedWidgets.end());
	DRX_ASSERT(m_selectedItems.find(itemWidget.GetAbstractItem()) == m_selectedItems.end());
}
...
void CNodeGraphView::DeleteSelectedItems()
{
	if (m_selectedWidgets.size())
	{
		...
		for (auto itr = m_selectedItems.begin(); itr != m_selectedItems.end(); itr = m_selectedItems.begin())
		{
			...
			i32k type = pAbstractItem->GetType();
			switch (type)
			{
			case eItemType_Node:
				{
...
void CNodeGraphView::OnCreateConnection(CAbstractConnectionItem& connection)
{
	...
	DRX_ASSERT(pSourceNode && pTargetNode);
	...
	DRX_ASSERT(pSourceNodeContent && pTargetNodeContent);
	...
	DRX_ASSERT(pSourceConnectionPoint && pTargetConnectionPoint);
...
void CNodeGraphView::OnRemoveConnection(CAbstractConnectionItem& connection)
{
	...
	DRX_ASSERT_MESSAGE(false, "Item not found!!");
}
		...
		if (pNodeItem)
		{
			...
			if (pNodeItem && tryToConnect)
			{
				...
				if (m_pModel)
				{
					...
					DRX_ASSERT(m_pNewConnectionLine);
					if (m_pNewConnectionLine)
					{
...
void CNodeGraphView::SetAction(u32 action, bool abortCurrent)
{
	if (m_action != action)
	{
		if (abortCurrent)
		...
		DRX_ASSERT(abortCurrent == false || m_action == eAction_None);
...
void CNodeGraphView::ReloadItems()
{
	...
	for (i32 i = m_pModel->GetNodeItemCount(); i--; )
	{
		if (CAbstractNodeItem* pItem = m_pModel->GetNodeItemByIndex(i))
			AddNodeItem(*pItem);
	}

	for (i32 i = m_pModel->GetConnectionItemCount(); i--; )
	{
...
{
	i32k percent = m_currentScalePercent + (pEvent->delta() > 0 ? 10 : -10);
	SetZoom(percent);
}
...
bool CNodeGraphView::OnCopyEvent()
{
	if (m_pModel)
	{
		...
		if (pClipboardItems && pArchiveHost)
		{
			...
			tukk szEditorName = m_pModel->GetRuntimeContext().GetTypeName();
...
void CNodeGraphView::RegisterConnectionEventListener(DrxGraphEditor::IConnectionEventsListener* pListener)
{
	m_connectionEventListeners.insert(pListener);
}

void CNodeGraphView::RemoveConnectionEventListener(DrxGraphEditor::IConnectionEventsListener* pListener)
{
	m_connectionEventListeners.erase(pListener);
}

void CNodeGraphView::SendBeginCreateConnection(QWidget* pSender, DrxGraphEditor::SBeginCreateConnectionEventArgs& args)
{
	for (DrxGraphEditor::IConnectionEventsListener* pListener : m_connectionEventListeners)
	{
		pListener->OnBeginCreateConnection(pSender, args);
	}
}

void CNodeGraphView::SendEndCreateConnection(QWidget* pSender, DrxGraphEditor::SEndCreateConnectionEventArgs& args)
{
	for (DrxGraphEditor::IConnectionEventsListener* pListener : m_connectionEventListeners)
	{
