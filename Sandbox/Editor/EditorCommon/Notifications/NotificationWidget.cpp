...
CNotificationWidget::CNotificationWidget(i32 notificationId, QWidget* pParent /* = nullptr*/)
	...
	, m_bDisableTimerHide(false)
{
	...
	else
	{
		...
		case Internal::CNotification::Info:
			m_pIconLabel->setPixmap(DrxIcon("icons:Dialogs/dialog-question.ico").pixmap(iconSize, iconSize, QIcon::Normal, QIcon::On));
			break;
		case Internal::CNotification::Warning:
			m_pIconLabel->setPixmap(DrxIcon("icons:Dialogs/dialog-warning.ico").pixmap(iconSize, iconSize, QIcon::Normal, QIcon::On));
			break;
		case Internal::CNotification::Critical:
			m_pIconLabel->setPixmap(DrxIcon("icons:Dialogs/dialog-error.ico").pixmap(iconSize, iconSize, QIcon::Normal, QIcon::On));
			break;
		}
		pInfoLayout->addWidget(m_pIconLabel);
	}
...
void CNotificationWidget::TaskComplete()
{
	...
	if (m_pProgressBar)
		m_pProgressBar->setVal(m_pProgressBar->maximum());
...
{
	if (m_pProgressBar)
		m_pProgressBar->setVal(static_cast<i32>(value * 100.0f));
}
