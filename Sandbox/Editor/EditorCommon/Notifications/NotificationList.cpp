	...
	, m_combineIdx(-1)
{
	...
	connect(pNotificationCenter, &CNotificationCenter::NotificationAdded, this, static_cast<void (CNotificationList::*)(i32)>(&CNotificationList::AddNotification));
}
...
void CNotificationList::AddNotification(i32 notificationId)
{
	...
	pNotification->signalNotificationHide.Connect([this, pNotification]
	{
		...
		{
			i32 idx = m_notifications.indexOf(pNotification);
...
bool CNotificationList::CombineNotifications(i32 notificationId)
{
	CNotificationCenter* pNotificationCenter = static_cast<CNotificationCenter*>(GetIEditor()->GetNotificationCenter());
	...
	for (i32 i = m_notifications.size() - 1; i > m_combineIdx; --i)
	{
...
{
	QMenu menu;
	connect(menu.addAction(DrxIcon("icons:General/File_Copy.ico"), "Copy"), &QAction::triggered, [this, point]()
	{
