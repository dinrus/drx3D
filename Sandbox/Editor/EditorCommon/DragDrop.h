...
public:
	...
	QByteArray GetCustomData(tukk type) const;

	//! Sets a QByteArray data for a given key.
	void       SetCustomData(tukk type, const QByteArray& data);

	//! Returns true if mime data contains custom data for a given key
	bool       HasCustomData(tukk type) const;
	...
	template<typename T>
	void SetCustomData(tukk type, const T& data);
	...
	template<typename T>
	const T& GetCustomData(tukk type) const;
	...
	static QString GetMimeFormatForType(tukk type);
...
template<typename T>
void CDragDropData::SetCustomData(tukk type, const T& data)
{
	QByteArray byteArray((tuk)&data, sizeof(T));
	SetCustomData(type, byteArray);
}

template<typename T>
const T& CDragDropData::GetCustomData(tukk type) const
{
