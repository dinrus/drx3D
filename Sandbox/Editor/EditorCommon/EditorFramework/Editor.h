...
public:
	...
	virtual tukk GetEditorName() const = 0;
...
protected:
	...
	virtual bool OnOpenFile(const QString& path) { DRX_ASSERT(0); return false; }
	...
	void AddToMenu(const MenuItems* items, i32 count);
	//! Add an array of the predefined menu items to the menu
	...
	void AddToMenu(CAbstractMenu* pMenu, tukk command);
	//! Add an editor command to the menu
	void AddToMenu(tukk menuName, tukk command);

	//! Returns the default action for a command
	QAction* GetAction(tukk command);
	...
	CAbstractMenu* GetMenu(tukk menuName);
	...
	virtual void CreateDefaultLayout(CDockableContainer* sender) { DRX_ASSERT_MESSAGE(false, "Not implemented"); };
...
public:
	...
	virtual tukk GetPaneTitle() const final { return GetEditorName(); };
