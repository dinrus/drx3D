...
static tukk s_defaultPath = "Editor/Preferences";
static tukk s_fileSuffix = "pref";
...
void CPreferences::Load(const QString& path)
{
	...
	if (!file.open(QIODevice::ReadOnly))
	{
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_COMMENT, msg.toLocal8Bit());
		m_bIsLoading = false;
		return;
	}
...
void CPreferences::Reset(tukk path)
{
	if (m_bIsLoading)
		return;
...
std::vector<SPreferencePage*> CPreferences::GetPages(tukk path)
{
	auto ite = m_preferences.find(path);
	if (ite == m_preferences.end())
