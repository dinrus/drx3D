...
//////////////////////////////////////////////////////////////////////////
void DisplayContext::DrawPoint(const Vec3& p, i32 nSize)
{
...
void DisplayContext::DrawDottedLine(const Vec3& p1, const Vec3& p2, float interval)
{
	...
	i32 totsegments = length / interval;
...
//////////////////////////////////////////////////////////////////////////
void DisplayContext::DrawPolyLine(const Vec3* pnts, i32 numPoints, bool cycled)
{
	MAKE_SURE(numPoints >= 2, return );
	MAKE_SURE(pnts != 0, return );

	i32 numSegments = cycled ? numPoints : (numPoints - 1);
	Vec3 p1 = pnts[0];
	Vec3 p2;
...
//////////////////////////////////////////////////////////////////////////
void DisplayContext::DrawCircle(const Vec3& pos, float radius, i32 nUnchangedAxis)
{
	// Draw circle with default radius.
	Vec3 p0, p1;
...
void DisplayContext::DrawDisc(float radius, float angle)
{
	...
	i32 numsteps = angle / step;

	std::vector <Vec3> vertices;

	i32 numverts = numsteps * 3 + 3;
...
{
	InternalDrawLine(p1, ColorB(u8(col1.r * 255.0f), u8(col1.g * 255.0f), u8(col1.b * 255.0f), u8(col1.a * 255.0f)),
	                 p2, ColorB(u8(col2.r * 255.0f), u8(col2.g * 255.0f), u8(col2.b * 255.0f), u8(col2.a * 255.0f)));
}
...
void DisplayContext::PushMatrix(const Matrix34& tm)
{
	...
	{
		i32 prevMatrixIndex = m_currentMatrix++;
...
//////////////////////////////////////////////////////////////////////////
void DisplayContext::RenderObject(i32 objectType, const Vec3& pos, float scale, const SRenderingPassInfo& passInfo)
{
	Matrix34 tm;
	tm.SetIdentity();
...
//////////////////////////////////////////////////////////////////////////
void DisplayContext::RenderObject(i32 objectType, const Matrix34& tm, const SRenderingPassInfo& passInfo)
{
...
void DisplayContext::DrawTerrainLine(Vec3 worldPos1, Vec3 worldPos2)
{
	...
	i32 steps = i32((worldPos2 - worldPos1).GetLength() / 4);
	if (steps == 0) steps = 1;
...
//////////////////////////////////////////////////////////////////////////
void DisplayContext::DrawTextLabel(const Vec3& pos, float size, tukk text, const bool bCenter, i32 srcOffsetX, i32 scrOffsetY)
{
...
//////////////////////////////////////////////////////////////////////////
void DisplayContext::Draw2dTextLabel(float x, float y, float size, tukk text, bool bCenter)
{
...
void DisplayContext::DrawTextOn2DBox(const Vec3& pos, tukk text, float textScale, const ColorF& TextColor, const ColorF& TextBackColor)
{
	if (!m_width || !m_height || !camera)
		return;
	Vec3 worldPos = ToWorldPos(pos);
	i32 vx=0, vy=0, vw=m_width, vh=m_height;

	u32 backupstate = GetState();
	i32 backupThickness = i32(GetLineWidth());
...
//////////////////////////////////////////////////////////////////////////
u32 DisplayContext::GetState() const
{
...
u32 DisplayContext::SetState(u32 state)
{
	u32 old = m_renderState;
	m_renderState = state;
...
//! Set a new render state flags.
//! @param returns previous render state.
u32 DisplayContext::SetStateFlag(u32 state)
{
	u32 old = m_renderState;
	m_renderState |= state;
...
//! @param returns previous render state.
u32 DisplayContext::ClearStateFlag(u32 state)
{
	u32 old = m_renderState;
	m_renderState &= ~state;
	m_renderState = pRenderAuxGeom->GetRenderFlags().m_renderFlags;
...
i32 DisplayContext::SetFillMode(i32 nFillMode)
{
	i32 prevState = m_renderState;
	SAuxGeomRenderFlags renderFlags = m_renderState;
...
//////////////////////////////////////////////////////////////////////////
void DisplayContext::DrawTextureLabel(const Vec3& pos, i32 nWidth, i32 nHeight, i32 nTexId, i32 nTexIconFlags, i32 srcOffsetX, i32 srcOffsetY, float fDistanceScale, float distanceSquared)
{
...
void DisplayContext::Flush2D()
{
	...
	i32 nLabels = m_textureLabels.size();
	for (i32 i = 0; i < nLabels; i++)
	{
