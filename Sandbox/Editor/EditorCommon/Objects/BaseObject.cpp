...
bool DoesLineSegmentIntersectConvexHull(const Vec2& p1, const Vec2& p2, const Edge2D* edgelist, i32 edgeListCount)
{
	for (i32 i = 0; i < edgeListCount; ++i)
	{
...
bool IsIncludePointsInConvexHull(Edge2D* pEdgeArray0, i32 nEdgeArray0Size, Edge2D* pEdgeArray1, i32 nEdgeArray1Size)
{
	...
	bool bInside(true);
	for (i32 i = 0; i < nEdgeArray0Size; ++i)
	{
		const Vec2& v(pEdgeArray0[i].first);
		bInside = true;
		for (i32 k = 0; k < nEdgeArray1Size; ++k)
		{
...
void ModifyConvexEdgeDirection(Edge2D* pEdgeArray, i32 nEdgeArraySize)
{
	...
	if (vCross.z < 0)
	{
		for (i32 i = 0; i < nEdgeArraySize; ++i)
			std::swap(pEdgeArray[i].first, pEdgeArray[i].second);
	}
}

bool CalculateConvexEdgesForOBB(const CPoint* obb_p, i32k maxSizeOfEdgeList, Edge2D* edgeList, i32& edgeListCount)
{
	std::vector<Vec3> pointsForRegion1;
	pointsForRegion1.reserve(maxSizeOfEdgeList);
	for (i32 i = 0; i < maxSizeOfEdgeList; ++i)
...
{
public:
	CUndoBaseObject(CBaseObject* pObj, tukk undoDescription);
...
{
public:
	CUndoBaseObjectMinimal(CBaseObject* obj, tukk undoDescription, i32 flags);
...
public:
	...
	virtual tukk GetDescription() override { return "Set object's layer"; }

private:
	void SetLayer(const DrxGUID& guid)
	{
	...
	DrxGUID m_objGuid;
	DrxGUID m_oldLayerGuid;
	DrxGUID m_newLayerGuid;
};
...
private:
	...
	virtual i32         GetSize() { return sizeof(CUndoAttachBaseObject); }
	virtual tukk GetDescription() override { return "Attachment Changed"; }

	DrxGUID m_attachedObjectGUID;
	DrxGUID m_parentObjectGUID;
	DrxGUID m_oldLayerGUID;
	bool    m_bKeepPos;
	bool    m_bPlaceOnRoot;
	bool    m_bAttach;
...
private:
	...
	virtual i32         GetSize()  { return sizeof(CUndoLinkBaseObject); }
	virtual tukk GetDescription() override { return m_actionType == ActionType::Link ? "Objects linked" : "Objects unlinked"; }

	DrxGUID    m_objectGUID;
	DrxGUID    m_linkedObjectGUID;
	ActionType m_actionType;
};

//////////////////////////////////////////////////////////////////////////
CUndoBaseObject::CUndoBaseObject(CBaseObject* obj, tukk undoDescription)
{
...
//////////////////////////////////////////////////////////////////////////
tukk CUndoBaseObject::GetObjectName()
{
...
//////////////////////////////////////////////////////////////////////////
CUndoBaseObjectMinimal::CUndoBaseObjectMinimal(CBaseObject* pObj, tukk undoDescription, i32 flags)
{
	// Stores the current state of this object.
	assert(pObj != nullptr);
	m_undoDescription = undoDescription;
...
//////////////////////////////////////////////////////////////////////////
tukk CUndoBaseObjectMinimal::GetObjectName()
{
	CBaseObject* pObject = GetIEditor()->GetObjectManager()->FindObject(m_guid);
	if (!pObject)
		return "";
...
{
	u32 flags = eObjectUpdateFlags_Undo;
	if (!bUndo)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CBaseObject::SetPos(const Vec3& pos, i32 flags)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING,
		           "Object %s, SetPos called with invalid position: (%f,%f,%f) %s",
		           (tukk)GetName(), pos.x, pos.y, pos.z,
...
//////////////////////////////////////////////////////////////////////////
bool CBaseObject::SetRotation(const Quat& rotate, i32 flags)
{
	if (!IsRotatable())
	{
		return false;
...
//////////////////////////////////////////////////////////////////////////
bool CBaseObject::SetScale(const Vec3& scale, i32 flags)
{
	if (!IsScalable())
	{
		return false;
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Object %s, SetScale called with invalid scale: (%f,%f,%f) %s",
...
void CBaseObject::DrawDimensionsImpl(DisplayContext& dc, const AABB& localBoundBox, AABB* pMergedBoundBox)
{
	...
	i32k kElementSize(sizeof(basePoints) / sizeof(*basePoints));
	Vec3 axisDirections[kElementSize] = { Vec3(1, 1, 1), Vec3(1, -1, 1), Vec3(-1, -1, 1), Vec3(-1, 1, 1), Vec3(1, 1, -1), Vec3(1, -1, -1), Vec3(-1, -1, -1), Vec3(-1, 1, -1) };
	i32 nLoopCount = bHave2Axis ? (kElementSize / 2) : kElementSize;
	if (bHave2Axis)
	{
		for (i32 i = 0; i < nLoopCount; ++i)
	...
	i32 nNearestAxisIndex(0);
	bool bPrevVisible(camera.IsPointVisible(pivot));

	for (i32 i = 1; i < nLoopCount; ++i)
	{
	...
	u32 backupstate = dc.GetState();
	i32 backupThickness = dc.GetLineWidth();
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::DrawTextOn2DBox(DisplayContext& dc, const Vec3& pos, tukk text, float textScale, const ColorF& TextColor, const ColorF& TextBackColor)
{
	Vec3 worldPos = dc.ToWorldPos(pos);
	i32 vx=0, vy=0, vw=dc.GetWidth(), vh=dc.GetHeight();
...
void CBaseObject::DrawWarningIcons(DisplayContext& dc, const Vec3& pos)
{
	...
	if (gViewportPreferences.showIcons || gViewportPreferences.showSizeBasedIcons)
	{
		i32k warningIconSize = OBJECT_TEXTURE_ICON_SIZE / 2;
		i32k iconOffset = m_nTextureIcon ? (-OBJECT_TEXTURE_ICON_SIZE / 2) : 0;
...
//////////////////////////////////////////////////////////////////////////
i32 CBaseObject::MouseCreateCallback(IDisplayViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
...
//////////////////////////////////////////////////////////////////////////
bool CBaseObject::FilterByLayer(CBaseObject const& obj, uk pLayer)
{
...
void CBaseObject::Serialize(CObjectArchive& ar)
{
	...
	if (ar.bLoading)
	{
		...
		Quat quat = m_rotate;
		Ang3 angles(0, 0, 0);
		u32 nMinSpec = m_nMinSpec;

		COLORREF color = m_color;

		DrxGUID parentId = DrxGUID::Null();
		DrxGUID linkToId = DrxGUID::Null();
		DrxGUID idInPrefab = DrxGUID::Null();
		DrxGUID lookatId = DrxGUID::Null();
		...
		{
			// File Not found.
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Object %s have invalid position (%f,%f,%f) %s", (tukk)GetName(), pos.x, pos.y, pos.z,
			           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
		}
		//////////////////////////////////////////////////////////////////////////
	...
	else
	{
		...
		i32 flags = m_flags & OBJFLAG_PERSISTMASK;
		if (flags != 0)
		...
		if (m_nMinSpec != 0)
			xmlNode->setAttr("MinSpec", (u32)m_nMinSpec);

		if (m_bUseMaterialLayersMask)
		{
			u32 nMask = m_nMaterialLayersMask;
...
XmlNodeRef CBaseObject::Export(const string& levelPath, XmlNodeRef& xmlNode)
{
	...
	if (m_nMinSpec != 0)
		objNode->setAttr("MinSpec", (u32)m_nMinSpec);
...
//////////////////////////////////////////////////////////////////////////
CBaseObject* CBaseObject::FindObject(DrxGUID id) const
{
	return GetObjectManager()->FindObject(id);
}

//////////////////////////////////////////////////////////////////////////
void CBaseObject::StoreUndo(tukk UndoDescription, bool minimal, i32 flags)
{
...
{
	tukk className = m_classDesc->ClassName();
	tukk subClassName = strstr(className, "::");
	if (!subClassName)
		return className;
...
bool CBaseObject::HitTestRectBounds(HitContext& hc, const AABB& box)
{
	...
	static i32k kNumberOfBoundBoxPt(8);
...
bool CBaseObject::HitHelperAtTest(HitContext& hc, const Vec3& pos)
{
	...
	{
		i32 iconSize = OBJECT_TEXTURE_ICON_SIZE;
...
void CBaseObject::GetAllChildren(TBaseObjects& outAllChildren, CBaseObject* pObj) const
{
	...
	for (i32 i = 0, iChildCount(pBaseObj->GetChildCount()); i < iChildCount; ++i)
	{
...
void CBaseObject::GetAllPrefabFlagedChildren(std::vector<CBaseObject*>& outAllChildren, CBaseObject* pObj) const
{
	...
	for (i32 i = 0, iLinkedCount(pBaseObj->GetLinkedObjectCount()); i < iLinkedCount; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::InvalidateTM(i32 flags)
{
	...
	if (bMatrixWasValid)
	{
		...
		for (i32 i = 0; i < m_linkedObjects.size(); ++i)
		{
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::SetLocalTM(const Vec3& pos, const Quat& rotate, const Vec3& scale, i32 flags)
{
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::SetLocalTM(const Matrix34& tm, i32 flags)
{
	if (m_lookat)
	{
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::SetWorldPos(const Vec3& pos, i32 flags)
{
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::SetWorldTM(const Matrix34& tm, i32 flags)
{
	if (m_parent || m_pLinkedTo)
	{
		Matrix34 parentWorld = GetParentWorldTM();
...
void CBaseObject::Validate()
{
	...
	{
		// File Not found.
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Object %s have invalid scale (%f,%f,%f) %s", (tukk)GetName(), m_scale.x, m_scale.y, m_scale.z,
		           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
	} //////////////////////////////////////////////////////////////////////////

	if (GetMaterial() != NULL && GetMaterial()->IsDummy())
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Material: %s for object: %s not found %s", GetMaterial()->GetName(), (tukk)GetName(),
		           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
	}
...
void CBaseObject::PostClone(CBaseObject* pFromObject, CObjectCloneContext& ctx)
{
	...
	for (i32 i = 0; i < pFromObject->GetChildCount(); i++)
	{
	...
	for (i32 i = 0; i < pFromObject->GetLinkedObjectCount(); ++i)
	{
...
void CBaseObject::OnMtlResolved(u32 id, bool success, tukk orgName, tukk newName)
{
	if (success)
	{
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::SetMinSpec(u32 nSpec, bool bSetChildren)
{
	m_nMinSpec = nSpec;
	UpdateVisibility(!IsHidden());
...
void CBaseObject::OnContextMenu(CPopupMenuItem* menu)
{
	...
	transformMenu.Add("Clear Rotation", [ = ]
	{
		...
		Quat q(IDENTITY);
		for (i32 i = 0; i < pGroup->GetCount(); ++i)
		{
	...
	GetIEditor()->GetObjectManager()->IterateTypeConverters(GetRuntimeClass(), [menu, this, &bAddedSeparator](tukk szTargetName, std::function<void(CBaseObject* pObject)> conversionFunc)
	{
		if (!bAddedSeparator)
		{
			menu->AddSeparator();
...
void CBaseObject::LinkTo(CBaseObject* pLinkTo, bool bKeepPos/* = true*/)
{
	if (pLinkTo->IsDescendantOf(this))
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Cannot link %s to a direct descendant", pLinkTo->GetName().c_str());
		return;
	}
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::SetMaterialByName(tukk mtlName)
{
...
void CBaseObject::SerializeGeneralProperties(Serialization::IArchive& ar, bool bMultiEdit)
{
	...
	string name = (tukk)m_name;
	ar(name, "name", "Name");

	i32 abgr = m_color;
	ColorB color = ColorB((u32)abgr);
	ar(color, "color", "Color");

	string layer = (tukk)m_layer->GetFullName();
