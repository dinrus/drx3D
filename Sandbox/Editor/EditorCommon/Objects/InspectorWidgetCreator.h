...
public:
	...
	void AddPropertyTree(tukk szTitle, std::function<void(TObjectType* pObject, Serialization::IArchive& ar, bool bMultiEdit)> serializationFunc, bool bCollapsedByDefault = false)
	{
		uint64 id = stl::hash_strcmp<tukk >()(szTitle);
	...
	void AddPropertyTree(uint64 id, tukk szTitle, std::unique_ptr<QCollapsibleFrame>&& pFrame, CObjectPropertyWidget::TSerializationFunc serializationFunc, bool bCollapsedByDefault = false)
	{
		// Look if another object has already queued this widget
		auto it = std::lower_bound(m_queuedWidgets.begin(), m_queuedWidgets.end(), id, [](const SQueuedWidgetInfo& widgetInfo, const uint64 widgetId) -> bool { return widgetInfo.id < widgetId; });
		if (it != m_queuedWidgets.end() && it->id == id)
	...
	void AddWidgetsToInspector(CInspector& inspector)
	{
		...
		for (SQueuedWidgetInfo& queuedWidget : m_queuedWidgets)
		{
			...
			DRX_ASSERT(queuedWidget.createWidgetCallback);
...
protected:
	...
	void AddPropertyTreeInternal(uint64 id, tukk szTitle, std::unique_ptr<QCollapsibleFrame>&& pFrame, CObjectPropertyWidget::TSerializationFunc serializationFunc, bool bCollapsedByDefault = false)
	{
