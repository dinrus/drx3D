...
struct STimelineViewState
{
	...
	i32    ScrollOffset(float origin) const;
	i32    TimeToLayout(float time) const;
	float  LocalToTime(i32 x) const;
	i32    TimeToLocal(float time) const;
	float  LayoutToTime(i32 x) const;
};
...
public:
	...
	void            OnSplitterMoved(u32 newTreeWidth);
	void            OnLayoutChange();
...
signals:
	...
	void SignalPlay();
	void SignalNumberHotkey(i32 number);
...
protected slots:
	...
	void OnVerticalScroll(i32 value);
...
private:
	...
	void          UpdateTracksTimeMarkers(i32 width);
