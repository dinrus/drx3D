...
typedef std::pair<bool(CALLBACK*)(CBaseObject const&, uk ), uk> BaseObjectFilterFunctor;
...
public:
	...
	virtual DrxGUID GetFrom(const DrxGUID& loadedGuid) const = 0;
	DrxGUID         GetFor(CBaseObject*) const;
};

class CRandomUniqueGuidProvider : public IGuidProvider
{
public:
	virtual DrxGUID GetFrom(const DrxGUID&) const override;
};
...
public:
	...
	virtual i32 GetObjectCount() const = 0;
	...
	virtual void LinkSelectionTo(CBaseObject* pLinkTo, tukk szTargetName = "") = 0;
	//! Link single object to target
	virtual void Link(CBaseObject* pObject, CBaseObject* pLinkTo, tukk szTargetName = "") = 0;
	//! Link multiple objects to target
	virtual void Link(const std::vector<CBaseObject*> objects, CBaseObject* pLinkTo, tukk szTargetName = "") = 0;
	...
	//////////////////////////////////////////////////////////////////////////
	//! Find object by ID.
	virtual CBaseObject* FindObject(const DrxGUID& guid) const = 0;
	//////////////////////////////////////////////////////////////////////////
	//! Find object by name.
	// When runtime class is specified, returns valid object only if it belongs to specified runtime class.
	virtual CBaseObject* FindObject(tukk objectName, CRuntimeClass* pRuntimeClass = 0) const = 0;
	//////////////////////////////////////////////////////////////////////////
	...
	virtual i32  MoveObjects(const AABB& box, const Vec3& offset, i32 nRot = 0, bool isCopy = false) = 0;
	...
	virtual void FindObjectsInRect(CViewport* view, const CRect& rect, std::vector<DrxGUID>& guids) = 0;
	...
	virtual void Serialize(XmlNodeRef& rootNode, bool isLoading, i32 flags = SERIALIZE_ALL) = 0;
	...
	virtual void ChangeObjectId(const DrxGUID& oldId, const DrxGUID& newId) = 0;
	...
	virtual u32 ForceID() const = 0;
	virtual void   ForceID(u32 FID) = 0;

	//! Registers a method of converting one object type to another
	virtual void RegisterTypeConverter(CRuntimeClass* pSource, tukk szTargetName, std::function<void(CBaseObject* pObject)> conversionFunc) = 0;

	//! Goes through available converters for the source type
	virtual void IterateTypeConverters(CRuntimeClass* pSource, std::function<void(tukk szTargetName, std::function<void(CBaseObject*)> )> callback) = 0;
	...
	//////////////////////////////////////////////////////////////////////////
	virtual void StartObjectsLoading(i32 numObjects) = 0;
	...
	//////////////////////////////////////////////////////////////////////////
	virtual void FindAndRenameProperty2(tukk property2Name, const string& oldValue, const string& newValue) = 0;
	virtual void FindAndRenameProperty2If(tukk property2Name, const string& oldValue, const string& newValue, tukk otherProperty2Name, const string& otherValue) = 0;
	...
	virtual void SetSelectionMask(i32 mask) const = 0;
...
public:
	...
	CDrxSignal<void(CObjectEvent&)> signalObjectChanged;

	//! New method of determining when selection is changed
	//! Query IObjectManager::GetSelection to see what selection currently is
	CDrxSignal<void(const std::vector<CBaseObject*>&, const std::vector<CBaseObject*>&)>                 signalSelectionChanged;

	CDrxSignal<void(CBaseObject* pParent, const std::vector<CBaseObject*>& objects)>                     signalObjectsDetached;
	CDrxSignal<void(CBaseObject* pParent, const std::vector<CBaseObject*>& objects, bool keepTransform)> signalBeforeObjectsDetached;
	CDrxSignal<void(CBaseObject* pParent, const std::vector<CBaseObject*>& objects)>                     signalObjectsAttached;
	CDrxSignal<void(CBaseObject* pParent, const std::vector<CBaseObject*>& objects, bool keepTransform)> signalBeforeObjectsAttached;
	CDrxSignal<void(const CObjectLayer&, const std::vector<CBaseObject*>& objects)>                      signalBeforeObjectsDeleted;
	CDrxSignal<void(const CObjectLayer&, const std::vector<CBaseObject*>& objects)>                      signalObjectsDeleted;

	CDrxSignal<void(const std::vector<CBaseObject*>& objects)> signalBatchProcessStarted;
	CDrxSignal<void()> signalBatchProcessFinished;
};
