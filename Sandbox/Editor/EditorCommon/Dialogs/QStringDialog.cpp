...
void QStringDialog::SetString(tukk str)
{
	m_pEdit->setText(str); 
}
...
void QStringDialog::accept()
{
	...
	if (m_bFileNameLimitation)
	{
		...
		for (i32 i = 0, iCount(reservedCharacters.GetLength()); i < iCount; ++i)
		{
	...
	if (m_bFileNameAsciiOnly)
	{
		...
		{
			if (static_cast<u8>(str[i]) > 0x7F)
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "This string can't contain non-ASCII characters");
				return;
			}
		}
	}
