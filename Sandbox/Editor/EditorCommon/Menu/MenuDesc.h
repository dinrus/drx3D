...
struct SItem
{
	SItem(i32 priority, i32 section, const K& key)
		: m_pParent(nullptr)
		, m_priority(priority)
		, m_section(section)
		, m_key(key)
...
{
	SActionItem(QAction* pAction, i32 priority, i32 section, const K& key)
...
struct SMenuItem : SItem<K>
{
	...
	SMenuItem(tukk szName, i32 priority, i32 section, const K& key, ARGS... args)
		: SItem<K>(priority, section, key)
		, m_name(szName)
	{
...
template<typename K>
std::unique_ptr<SActionItem<K>> AddAction(const K& key, i32 section, i32 priority, QAction* pAction)
{
	return std::unique_ptr<SActionItem<K>>(new SActionItem<K>(pAction, priority, section, key));
}

template<typename K, typename... ARGS>
std::unique_ptr<SMenuItem<K>> AddMenu(const K& key, i32 section, i32 priority, tukk szName, ARGS... args)
{
...
public:
	...
	tukk GetMenuName(const K& key) const
	{
