...
public:
	...
	bool               HitTestLine(const Vec3& lineP1, const Vec3& lineP2, const CPoint& hitpoint, i32 pixelRadius, float* pToCameraDistance = 0) const override;
	...
	Vec3            WorldToView3D(const Vec3& worldPoint, i32 flags = 0) const override                                                                                      { return Vec3(0.0f, 0.0f, 0.0f); }
	...
	void            GetDimensions(i32* width, i32* height) const override;

	void            EnableXYViewport(bool bEnable) { m_bXYViewport = bEnable; }
	virtual Vec3	MapViewToCP(CPoint point, i32 axis, bool aSnapToTerrain = false, float aTerrainOffset = 0.f) { return Vec3(0, 0, 0); }
