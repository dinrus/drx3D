...
struct SCurveEditorCurve : ISplineEvaluator
{
	...
	bool Serialize(IArchive& ar, tukk name = "", tukk label = "")
	{
		ar(m_keys, "keys");
	...
	virtual i32 GetNumDimensions() const override
	{
		return m_bBezier2D ? 2 : 1;
	}
	virtual i32 GetKeyCount() const override
	{
		return m_keys.size();
	}
	virtual void GetKey(i32 i, KeyType& keyOut) const override
	{
