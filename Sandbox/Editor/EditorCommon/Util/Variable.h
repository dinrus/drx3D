...
typedef Functor4<u32, bool, tukk , tukk > TMissingAssetResolveCallback;

inline tukk to_c_str(tukk str)    { return str; }
inline tukk to_c_str(const string& str) { return str; }
...
struct EDITOR_COMMON_API IVariableContainer : public _i_reference_target_t
{
	...
	virtual i32 GetNumVariables() const = 0;

	//! Get variable at index
	virtual IVariable* GetVariable(i32 index) const = 0;
	...
	virtual IVariable* FindVariable(tukk name, bool bRecursive = false, bool bHumanName = false) const = 0;
...
struct EDITOR_COMMON_API IVariable : public IVariableContainer
{
	...
	virtual u8 GetDataType() const = 0;
	virtual void          SetDataType(u8 dataType) = 0;

	virtual void          SetUserData(uk pData) = 0;
	virtual uk         GetUserData() = 0;
	...
	inline void  SetDisplayValue(tukk value) { SetDisplayValue(string(value)); }
	...
	// Get methods.
	/////////////////////////////////////////////////////////////////////////////
	virtual void    Get(i32& value) const = 0;
...
public:
	...
	//! Get name of parameter.
	tukk GetName() const              { return to_c_str(m_name); };
	...
	void          SetDescription(tukk desc)    { m_description = desc; };
	//! Get name of parameter.
	tukk   GetDescription() const              { return to_c_str(m_description); };

	EType         GetType() const                     { return IVariable::UNKNOWN; };
	i32           GetSize() const                     { return sizeof(*this); };

	u8 GetDataType() const                 { return m_dataType; };
	void          SetDataType(u8 dataType) { m_dataType = dataType; }

	void          SetFlags(i32 flags)                 { m_flags = flags; }
	i32           GetFlags() const                    { return m_flags; }

	void          SetUserData(uk pData)            { m_pUserData = pData; }
	uk         GetUserData()                       { return m_pUserData; }

	//////////////////////////////////////////////////////////////////////////
	// Set methods.
	//////////////////////////////////////////////////////////////////////////
	void Set(i32 value)                        { assert(0); }
	...
	void Set(tukk value)                { assert(0); }
	void SetDisplayValue(const string& value) { Set(value); }
	void SetDisplayValue(tukk value) { SetDisplayValue(string(value)); } // for CString conversion

	//////////////////////////////////////////////////////////////////////////
	// Get methods.
	//////////////////////////////////////////////////////////////////////////
	void    Get(i32& value) const     { assert(0); }
	...
	virtual i32        GetNumVariables() const                                                                { return 0; }
	virtual IVariable* GetVariable(i32 index) const                                                           { return nullptr; }

	virtual bool       IsContainsVariable(IVariable* pVar, bool bRecursive = false) const                     { return false; }

	virtual IVariable* FindVariable(tukk name, bool bRecursive = false, bool bHumanName = false) const { return nullptr; }
...
public:
	...
	virtual i32   GetSize() const { return sizeof(CVariableArray); };
	...
	virtual void Set(tukk value) // for CString conversion
	{
		Set(string(value));
	}
	void OnSetValue(bool bRecursive)
	...
	void CopyValue(IVariable* fromVar)
	{
		...
		if (fromVar->GetType() != IVariable::ARRAY)
			return;
		i32 numSrc = fromVar->GetNumVariables();
		i32 numTrg = m_vars.size();
		for (i32 i = 0; i < numSrc && i < numTrg; i++)
		{
	...
	//////////////////////////////////////////////////////////////////////////
	virtual i32        GetNumVariables() const { return m_vars.size(); }

	virtual IVariable* GetVariable(i32 index) const
	{
		assert(index >= 0 && index < (i32)m_vars.size());
		return m_vars[index];
	}
	...
	virtual IVariable* FindVariable(tukk name, bool bRecursive, bool bHumanName) const;
...
{
//////////////////////////////////////////////////////////////////////////
template<i32 TypeID, bool IsStandart, bool IsInteger, bool IsSigned>
struct type_traits_base
{
	static i32  type()        { return TypeID; };
...
template<> struct type_traits<i32> : public type_traits_base<IVariable::INT, true, true, true> {};
...
struct type_convertor
{
	...
	void operator()(i32k& from, i32& to) const          { to = from; }
	void operator()(i32k& from, bool& to) const         { to = from != 0; }
	void operator()(i32k& from, float& to) const        { to = (float)from; }
	//////////////////////////////////////////////////////////////////////////
	void operator()(const bool& from, i32& to) const         { to = from; }
	void operator()(const bool& from, bool& to) const        { to = from; }
	void operator()(const bool& from, float& to) const       { to = from; }
	//////////////////////////////////////////////////////////////////////////
	void operator()(const float& from, i32& to) const        { to = (i32)from; }
	...
	void operator()(i32 value, string& to) const            { to.Format("%d", value); };
	void operator()(bool value, string& to) const           { to.Format("%d", (value) ? (i32)1 : (i32)0); };
	...
	void operator()(const string& from, i32& value) const   { value = atoi((tukk)from); };
	void operator()(const string& from, bool& value) const  { value = atoi((tukk)from) != 0; };
	void operator()(const string& from, float& value) const { value = (float)atof((tukk)from); };
	void operator()(const string& from, Vec2& value) const
	{
		value.x = 0;
		value.y = 0;
		sscanf((tukk)from, "%f,%f", &value.x, &value.y);
	};
	void operator()(const string& from, Vec3& value) const
	{
		value.Set(0, 0, 0);
		sscanf((tukk)from, "%f,%f,%f", &value.x, &value.y, &value.z);
	};
	void operator()(const string& from, Vec4& value) const
	{
		...
		value.z = 0;
		value.w = 0;
		sscanf((tukk)from, "%f,%f,%f,%f", &value.x, &value.y, &value.z, &value.w);
	};
	...
	{
		value.SetIdentity();
		sscanf((tukk)from, "%f,%f,%f,%f", &value.w, &value.v.x, &value.v.y, &value.v.z);
	};
};
...
inline bool compare(tukk s1, tukk s2)
{
...
public:
	...
	virtual i32   GetSize() const { return sizeof(T); };

	//////////////////////////////////////////////////////////////////////////
	// Set methods.
	//////////////////////////////////////////////////////////////////////////
	virtual void Set(i32 value)            { SetValue(value); }
	...
	virtual void Set(tukk value)    { SetValue(string(value)); }

	//////////////////////////////////////////////////////////////////////////
	// Get methods.
	//////////////////////////////////////////////////////////////////////////
	virtual void Get(i32& value) const     { GetValue(value); }
	...
	virtual void SetLimits(float fMin, float fMax, float fStep = 0.f, bool bHardMin = true, bool bHardMax = true)
	{
		DRX_ASSERT_MESSAGE(fMin <= fMax, "maximum value has to be bigger than minimum value!");
...
public:
	...
	//! Get name of parameter.
	virtual i32  GetSize() const { return sizeof(Self); };
	...
	//////////////////////////////////////////////////////////////////////////
	i32        GetNumVariables() const { return m_vars.size(); }
	IVariable* GetVariable(i32 index) const
	{
		assert(index >= 0 && index < (i32)m_vars.size());
		return m_vars[index];
	}
...
struct IVarEnumList : public _i_reference_target_t
{
	...
	virtual tukk GetItemName(uint index) = 0;
};
...
public:
	...
	virtual tukk GetItemName(uint index);
...
public:
	...
	void AddEnumItem(tukk name, const T& value)
	{
		if (GetFlags() & UI_USE_GLOBAL_ENUMS)  // don't think adding makes sense
			return;
		if (!m_enum)
...
public:
	...
	void         AddVariable(IVariable* pVar, tukk varName, u8 dataType = IVariable::DT_SIMPLE);
	// This used from smart variable pointer.
	void         AddVariable(CVariableBase& var, tukk varName, u8 dataType = IVariable::DT_SIMPLE);
	...
	//! Find variable by name.
	virtual IVariable* FindVariable(tukk name, bool bRecursive = true, bool bHumanName = false) const;
	...
	void ReserveNumVariables(i32 numVars);
	...
	IVariable*   FindChildVar(tukk name, IVariable* pParentVar) const;
...
public:
	...
	void       AddVariable(CVariableBase& var, const string& varName, VarOnSetCallback cb = NULL, u8 dataType = IVariable::DT_SIMPLE);
	void       AddVariable(CVariableBase& var, const string& varName, const string& varHumanName, VarOnSetCallback cb = NULL, u8 dataType = IVariable::DT_SIMPLE);
	void       AddVariable(CVariableArray& table, CVariableBase& var, const string& varName, const string& varHumanName, VarOnSetCallback cb = NULL, u8 dataType = IVariable::DT_SIMPLE);
