...
struct EDITOR_COMMON_API HeapCheck
{
	...
	static void Check(tukk file, i32 line);
};
...
{
inline XmlNodeRef CreateXmlNode(tukk sTag)
{
	return GetISystem()->CreateXmlNode(sTag);
}

inline bool SaveXmlNode(XmlNodeRef node, tukk filename)
{
	return node->saveToFile(filename);
}

inline XmlNodeRef LoadXmlFromFile(tukk fileName)
{
	return GetISystem()->LoadXmlFromFile(fileName);
}

inline XmlNodeRef LoadXmlFromBuffer(tukk buffer, size_t size)
{
...
// Tokenize string.
//////////////////////////////////////////////////////////////////////////
inline string TokenizeString(const string& str, tukk pszTokens, i32& iStart)
{
	assert(iStart >= 0);
	...
	tukk pszPlace = (tukk)str + iStart;
	tukk pszEnd = (tukk)str + str.GetLength();
	if (pszPlace < pszEnd)
	{
		i32 nIncluding = (i32)strspn(pszPlace, pszTokens);
		;

		if ((pszPlace + nIncluding) < pszEnd)
		{
			pszPlace += nIncluding;
			i32 nExcluding = (i32)strcspn(pszPlace, pszTokens);

			i32 iFrom = iStart + nIncluding;
			i32 nUntil = nExcluding;
...
inline void SplitString(const string& rSrcStr, std::vector<string>& rDestStrings, char aSeparator = ',')
{
	...
	while (true)
	{
		...
		if (-1 == crtPos)
		{
			...
			if (crtPos != lastPos)
			{
				rDestStrings.push_back(string((tukk)(rSrcStr.GetString() + lastPos), crtPos - lastPos));
			}
...
inline void SplitString(string& rSrcStr, std::vector<string>& rDestStrings, char aSeparator = ',')
{
	...
	while (true)
	{
		...
		if (-1 == crtPos)
		{
			...
			if (crtPos != lastPos)
			{
				rDestStrings.push_back(string((tukk)(rSrcStr.c_str() + lastPos), crtPos - lastPos));
			}
...
inline string FormatWithThousandsSeperator(u32k number)
{
	string string;

	string.Format("%d", number);

	for (i32 p = string.GetLength() - 3; p > 0; p -= 3)
		string.Insert(p, ',');
...
inline string ShortenStringWithEllipsis(string what, size_t maxCharCountIncludingEllipsis = 20, tukk szEllipsis = "...")
{
...
void EDITOR_COMMON_API FormatFloatForUI(string& str, i32 significantDigits, double value);

//////////////////////////////////////////////////////////////////////////
// Simply sub string searching case insensitive.
//////////////////////////////////////////////////////////////////////////
inline tukk strstri(tukk pString, tukk pSubstring)
{
	i32 i, j, k;
	for (i = 0; pString[i]; i++)
...
EDITOR_COMMON_API void ParsePropertyString(
  tukk szProperties,
  tukk szDelim,
  const std::function<void(tukk pKey, size_t keyLength, tukk pValue, size_t valueLength)>& fn);
