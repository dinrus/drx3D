...
{
	// Copy all variables.
	i32 numSrc = fromVarBlock->GetNumVariables();
	i32 numTrg = GetNumVariables();
...
//////////////////////////////////////////////////////////////////////////
void CVarBlock::AddVariable(IVariable* pVar, tukk varName, u8 dataType)
{
	if (varName)
...
//////////////////////////////////////////////////////////////////////////
void CVarBlock::AddVariable(CVariableBase& var, tukk varName, u8 dataType)
{
	if (varName)
		var.SetName(varName);
...
namespace
{
IVariable* FindVariable(tukk name, bool bRecursive, bool bHumanName, const std::vector<IVariablePtr>& vars)
{
	// Search all top level variables.
...
//////////////////////////////////////////////////////////////////////////
IVariable* CVarBlock::FindVariable(tukk name, bool bRecursive, bool bHumanName) const
{
	return ::FindVariable(name, bRecursive, bHumanName, m_vars);
}

//////////////////////////////////////////////////////////////////////////
IVariable* CVariableArray::FindVariable(tukk name, bool bRecursive, bool bHumanName) const
{
	return ::FindVariable(name, bRecursive, bHumanName, m_vars);
}
...
//////////////////////////////////////////////////////////////////////////
void CVarBlock::ReserveNumVariables(i32 numVars)
{
...
void CVarBlock::WireVar(IVariable* src, IVariable* trg, bool bWire)
{
	if (bWire)
		...
		src->Unwire(trg);
	i32 numSrcVars = src->GetNumVariables();
	if (numSrcVars > 0)
	{
		i32 numTrgVars = trg->GetNumVariables();
		for (i32 i = 0; i < numSrcVars && i < numTrgVars; i++)
		{
...
void CVarBlock::SetCallbackToVar(IVariable::OnSetCallback func, IVariable* pVar, bool bAdd)
{
	...
	i32 numVars = pVar->GetNumVariables();
	if (numVars > 0)
	{
...
{
	for (i32 i = 0; i < GetNumVariables(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CVarObject::AddVariable(CVariableBase& var, const string& varName, VarOnSetCallback cb, u8 dataType)
{
...
//////////////////////////////////////////////////////////////////////////
void CVarObject::AddVariable(CVariableBase& var, const string& varName, const string& varHumanName, VarOnSetCallback cb, u8 dataType)
{
	var.AddRef(); // Variables are local and must not be released by CVarBlock.
	var.SetName(varName);
...
//////////////////////////////////////////////////////////////////////////
void CVarObject::AddVariable(CVariableArray& table, CVariableBase& var, const string& varName, const string& varHumanName, VarOnSetCallback cb, u8 dataType)
{
	var.AddRef(); // Variables are local and must not be released by CVarBlock.
	var.SetName(varName);
	var.SetHumanName(varHumanName);
...
tukk CVarGlobalEnumList::GetItemName(uint index)
{
...
{
	template<class T>
	static void SerializeT(IVariable* pVar, Serialization::IArchive& ar, tukk name, tukk label)
	{
		if (pVar->GetEnumList())
		{
	...
	template<class T>
	static void SerializeNumericalT(IVariable* pVar, Serialization::IArchive& ar, tukk name, tukk label)
	{
		if (pVar->GetEnumList())
		{
			SerializeEnumT<T>(pVar, ar, name, label);
	...
	template<class T>
	static void SerializeEnumT(IVariable* pVar, Serialization::IArchive& ar, tukk name, tukk label)
	{
		T value;
		pVar->Get(value);
		...
		{
			tukk enumStr = varEnums->GetItemName(i);
			if (!enumStr)
				break;
			stringList.push_back(enumStr);
		}

		tukk enumValue = varEnums->ValueToName(value);
		i32 index = std::max(stringList.find(enumValue), 0);
	...
	static void SerializeString(IVariable* pVar, Serialization::IArchive& ar, tukk name, tukk label)
	{
		if (pVar->GetEnumList())
		{
		...
		string str;
		pVar->Get(str);
		string value = (tukk)str;

		u8 dataType = pVar->GetDataType();
	...
	static void SerializeVariable(IVariable* pVar, Serialization::IArchive& ar)
	{
		...
		{
			i32 numv = pVar->GetNumVariables();
			if (numv > 0)
			{
				if (ar.openBlock(pVar->GetName(), pVar->GetHumanName()))
				{
					for (i32 i = 0; i < numv; ++i)
		...
		else if (type == IVariable::INT)
		{
			...
			else if (dataType == IVariable::DT_UIENUM)
			{
				...
				if (m_pEnumDBItem)
				{
					...
					tukk enumValue = (LPCSTR)m_pEnumDBItem->ValueToName(svalue);
					i32 index = std::max(stringList.find(enumValue), 0);
					...
					if (ar.isInput())
					{
						...
						i32 inew = atoi((LPCSTR)enumValue);
						if (value != inew)
						{
							pVar->Set(inew);
						}
				...
				{
					SerializeNumericalT<i32>(pVar, ar, name, label);
				}
			}
			else
			{
