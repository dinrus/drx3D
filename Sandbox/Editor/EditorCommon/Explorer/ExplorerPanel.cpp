...
{
	i32 rowCount = model->rowCount(parent);
	...
	void Serialize(Serialization::IArchive& ar)
	{
		...
		if (ar.isOutput())
		{
			...
			while (!stack.empty())
			{
				...
				i32 rowCount = model->rowCount(index);
				for (i32 i = 0; i < rowCount; ++i)
				{
		...
		std::vector<i32> sectionsHidden;
		std::vector<i32> sectionsVisible;
		if (ar.isOutput())
		{
			for (i32 i = 0; i < treeView->model()->columnCount(); ++i)
		...
		if (ar.isInput())
		{
			i32 columnCount = treeView->model()->columnCount();
			for (i32 i = 0; i < sectionsHidden.size(); ++i)
			{
				i32 section = sectionsHidden[i];
				if (section >= 0 && section < columnCount)
					treeView->header()->hideSection(section);
			}
			for (i32 i = 0; i < sectionsVisible.size(); ++i)
			{
				i32 section = sectionsVisible[i];
				if (section >= 0 && section < columnCount)
...
bool Serialize(Serialization::IArchive& ar, QTreeView* treeView, tukk name, tukk label)
{
...
struct ColumnFilterOption
{
	...
	ColumnFilterOption(i32 column, const ExplorerColumn* columnData)
		: column(column)
	...
	bool Allow(const ExplorerEntry* entry) const
	{
		if (column < 0)
			return true;
		i32 val = entry->GetColumnValue(column);
...
struct FilterOptions
{
	...
	{
		for (i32 col = 0, count = explorerData.GetColumnCount(); col < count; col++)
		{
...
public:
	...
	bool matchFilter(i32 source_row, const QModelIndex& source_parent) const
	{
	...
	virtual bool filterAcceptsRow(i32 source_row, const QModelIndex& source_parent) const override
	{
...
private:
	...
	bool hasAcceptedChildrenCached(i32 source_row, const QModelIndex& source_parent) const
	{
		std::pair<QModelIndex, i32> indexId = std::make_pair(source_parent, source_row);
	...
	bool hasAcceptedChildren(i32 source_row, const QModelIndex& source_parent) const
	{
		QModelIndex item = sourceModel()->index(source_row, 0, source_parent);
		if (!item.isValid())
			return false;

		i32 childCount = sourceModel()->rowCount(item);
	...
	typedef map<std::pair<QModelIndex, i32>, bool> TAcceptCache;
	...
	, m_batchChangesRunning(0)
{
	...
	setLayout(layout);
	{
		...
		m_filterButton->setIcon(DrxIcon("icons:General/Options.ico"));
...
void ExplorerPanel::SetRootIndex(i32 rootIndex)
{
...
void ExplorerPanel::SetExpanded(const QModelIndex& index, bool bExpand, uint depth)
{
	...
	if (depth--)
	{
		...
		i32 numChildren = m_treeView->model()->rowCount(index);
...
void ExplorerPanel::UpdateRootMenu()
{
	...
	{
		i32k entryIndex = i32(m_rootMenuActions.size());
...
void ExplorerPanel::OnHeaderContextMenu(const QPoint& pos)
{
	...
	i32 columnCount = m_explorerData->GetColumnCount();
...
void ExplorerPanel::OnHeaderColumnToggle()
{
	...
	{
		i32 column = action->data().toInt();
		m_treeView->header()->setSectionHidden(column, !m_treeView->header()->isSectionHidden(column));
	}

	i32 numVisibleSections = 0;
	for (i32 i = 0; i < m_treeView->model()->columnCount(); ++i)
...
void ExplorerPanel::OnContextMenu(const QPoint& pos)
{
	...
	for (size_t i = 0; i < actions.size(); ++i)
	{
		...
		QAction* qaction = new QAction(DrxIcon(QString(action.icon)), action.text, &menu);
...
void ExplorerPanel::OnMenuPasteSelection()
{
	...
	i32 subtreeCount = m_explorerData->GetSubtreeCount();
	for (i32 subtree = 0; subtree < subtreeCount; ++subtree)
	{
...
ExplorerEntry* ExplorerPanel::GetEntryByPath(tukk szPath) const
{
	// TODO: This is super inefficient...
	for (i32 row = 0; row < m_filterModel->rowCount(); ++row)
	{
		for (i32 column = 0; column < m_filterModel->columnCount(); ++column)
		{
...
void ExplorerPanel::SetTreeViewModel(QAbstractItemModel* model)
{
	if (m_treeView->model() != model)
	{
		...
		for (i32 i = 1; i < m_explorerData->GetColumnCount(); ++i)
		{
		...
		for (i32 i = m_explorerData->GetColumnCount(); i < model->columnCount(); ++i)
			m_treeView->header()->hideSection(i);
	}
}
...
void ExplorerPanel::OnExplorerBeginBatchChange(i32 subtree)
{
	++m_batchChangesRunning;
}

void ExplorerPanel::OnExplorerEndBatchChange(i32 subtree)
{
	--m_batchChangesRunning;
