...
string EntryFormat::MakeFilename(tukk entryPath) const
{
...
bool JSONLoader::Load(EntryBase* entry, tukk filename)
{
	string filePath = string(gEnv->pDrxPak->GetGameFolder()) + "\\" + filename;
	filePath.replace('/', '\\');
...
bool JSONLoader::Save(EntryBase* entry, tukk filename)
{
	string filePath = string(gEnv->pDrxPak->GetGameFolder()) + "\\" + filename;
	filePath.replace('/', '\\');
...
EntryType* ExplorerFileList::AddSingleFileEntryType(EntryListBase* list, tukk path, tukk label, IEntryLoader* loader)
{
	EntryType entryType;
...
EntryType* ExplorerFileList::GetEntryTypeByExtension(tukk ext)
{
	for (size_t i = 0; i < m_entryTypes.size(); ++i)
...
const EntryType* ExplorerFileList::GetEntryTypeByExtension(tukk extension) const
{
	return const_cast<ExplorerFileList*>(this)->GetEntryTypeByExtension(extension);
}

EntryType* ExplorerFileList::GetEntryTypeByPath(tukk singleFilePath)
{
	for (size_t i = 0; i < m_entryTypes.size(); ++i)
		for (size_t j = 0; j < m_entryTypes[i].formats.size(); ++j)
...
const EntryType* ExplorerFileList::GetEntryTypeByPath(tukk path) const
{
	return const_cast<ExplorerFileList*>(this)->GetEntryTypeByPath(path);
}

bool ExplorerFileList::AddAndSaveEntry(tukk filename)
{
...
void ExplorerFileList::SetExplorerData(ExplorerData* explorerData, i32 subtree)
{
	...
	{
		// Matches the order of IDrxPak::EFileSearchLocation
		...
		i32 pakValueCount = sizeof(pakValues) / sizeof(pakValues[0]);
		m_explorerPakColumn = m_explorerData->AddColumn("Pak", ExplorerColumn::ICON, false, pakValues, pakValueCount);
	}
}

ExplorerEntryId ExplorerFileList::AddEntry(tukk filename, bool resetIfExists)
{
...
EntryBase* ExplorerFileList::GetEntryBaseByPath(tukk path)
{
	for (size_t i = 0; i < m_entryTypes.size(); ++i)
	{
...
i32 ExplorerFileList::GetEntryCount() const
{
	i32 result = 0;
	for (size_t i = 0; i < m_entryTypes.size(); ++i)
...
uint ExplorerFileList::GetEntryIdByIndex(i32 index) const
{
	i32 startRange = 0;
	for (size_t i = 0; i < m_entryTypes.size(); ++i)
	{
		i32 size = m_entryTypes[i].entryList->Count();
	...
	for (size_t j = 0; j < m_entryTypes.size(); ++j)
	{
		...
		for (size_t i = 0; i < format.entryList->Count(); ++i)
		{
			if (EntryBase* entry = format.entryList->GetBaseByIndex(i))
			{
				if (!entry->modified)
					continue;
				i32 errorCount = output->errorCount;
...
string ExplorerFileList::GetSaveFileExtension(tukk entryPath) const
{
...
bool ExplorerFileList::CanBeSaved(tukk entryPath) const
{
	string extension = PathUtil::GetExt(entryPath);
...
void ExplorerFileList::RevertEntry(uint id)
{
	...
	if (entry)
	{
		...
		for (size_t i = 0; i < entryType->formats.size(); ++i)
		{
			...
			string filename = format.MakeFilename(entry->path.c_str());
			bool fileExists = gEnv->pDrxPak->IsFileExist(entry->path.c_str());
			if (!format.loader)
				continue;
...
void ExplorerFileList::CheckIfModified(uint id, tukk reason, bool continuousChange)
{
...
void ExplorerFileList::OnFileChange(tukk filename, EChangeType eType)
{
...
tukk ExplorerFileList::CanonicalExtension() const
{
...
void ExplorerFileList::UpdateEntryPakState(const ExplorerEntryId& id, i32 state)
{
...
void ExplorerFileList::UpdateEntryPakState(const ExplorerEntryId& id, tukk filename, const EntryType* entryType)
{
	if (m_explorerData && m_explorerPakColumn >= 0)
	{
		i32 pakState = 0;
		if (entryType)
		{
			for (size_t i = 0; i < entryType->formats.size(); ++i)
			{
				tukk extension = entryType->formats[i].extension;
				if (!extension[0])
					continue;
...
i32 ExplorerFileList::GetFilePakState(tukk path)
{
	i32 result = 0;
	if (gEnv->pDrxPak->IsFileExist(path, IDrxPak::eFileLocation_OnDisk))
		result |= IDrxPak::eFileLocation_OnDisk;
	if (gEnv->pDrxPak->IsFileExist(path, IDrxPak::eFileLocation_InPak))
		result |= IDrxPak::eFileLocation_InPak;
	return result;
}
