...
struct CEngineFileDialog::Implementation
{
	...
	{
		m_ui.pLookInComboBox->signalCurrentIndexChanged.Connect([this](i32 index)
		{
	...
	void SetupExtensionFilters(const ExtensionFilterVector& extensionFilters)
	{
		...
		DRX_ASSERT(m_defaultExtension.isEmpty() || !IsWildcardExtension(m_defaultExtension));//default ext must be a valid extension
		...
		else
		{
			foreach(const CExtensionFilter &extensionFilter, extensionFilters)
			{
				...
				DRX_ASSERT(extensions.count() >= 1); // filter must not be empty
			...
			DRX_ASSERT(m_defaultExtension.isEmpty() || m_validExtensions.contains(m_defaultExtension));
		}
	...
	QStringList GetCurrentFileExtensions() const
	{
		...
		i32 index = menuComboBox->GetCheckedItem();
	...
	struct Ui
	{
		...
		void Setup(Models& models, CEngineFileDialog::Mode mode, bool allowCreateFolder, const QString& buttonLabel, QWidget* parent)
		{
			...
			backButton->setIcon(DrxIcon(":/icons/General/Arrow_Left.ico"));

			auto upButton = new QToolButton(parent);
			upButton->setText(tr("Up"));
			upButton->setIcon(DrxIcon(":/icons/General/Arrow_Up.ico"));
			...
			if (allowCreateFolder)
			{
				...
				createFolderButton->setIcon(DrxIcon(":/icons/General/Folder_Add.ico"));
			...
			for (i32 column = 1; column < directoryColumnCount; ++column)
			{
			...
			tab->setTabIcon(0, DrxIcon("icons:General/Folder.ico", colorMap));

			tab->addTab(pHistoryView, "");
			tab->setTabToolTip(1, "History");
			tab->setTabIcon(1, DrxIcon("icons:General/History_Undo_List.ico", colorMap));

			tab->addTab(pFavoritesView, "");
			tab->setTabToolTip(2, "Favorites");
			tab->setTabIcon(2, DrxIcon("icons:General/Favorites_Full.ico", colorMap));
			...
			filetypeComboBox->signalCurrentIndexChanged.Connect([&models, filenameLineEdit, filetypeComboBox](i32 index)
			{
	...
	void AddProperties()
	{
		...
		connect(m_models.pFavoritesModel, &QAbstractItemModel::rowsInserted, [this](const QModelIndex &parent, i32 first, i32 last)
		{
		...
		connect(m_models.pFavoritesModel, &QAbstractItemModel::rowsAboutToBeRemoved, [this](const QModelIndex &parent, i32 first, i32 last)
		{
			for (i32 i = first; i <= last; i++)
			{
				QString path = m_models.pFavoritesModel->GetEntry(i);
		...
		connect(m_models.pFavoritesModel, &QAbstractItemModel::rowsAboutToBeMoved, [this](const QModelIndex &parent, i32 start, i32 end, const QModelIndex &destination, i32 row)
		{
			i32 insertionIndex = 0;
			if (row > 0)
			{
