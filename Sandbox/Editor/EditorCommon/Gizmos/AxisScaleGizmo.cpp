...
void CAxisScaleGizmo::SetName(tukk sName)
{
	m_name = sName;
}

tukk CAxisScaleGizmo::GetName()
{
	return m_name.c_str();
}
...
bool CAxisScaleGizmo::MouseCallback(IDisplayViewport* view, EMouseEvent event, CPoint& point, i32 nFlags)
{
	if (GetFlag(EGIZMO_INTERACTING))
	{
		switch (event)
