...
namespace Loader_Private
{
	...
	uint64 GetTimestamp(tukk szAssetFilepath)
	{
		...
		AssetLoader::Predicate& predicate)
	{
		...
		const auto xmlCompletionHandler = [&items](tukk pBuffer, size_t numberOfBytes, uk pUserData) -> void
		{
			...
			if (!pAsset)
			{
				DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to load asset: \"%s\".", pAssetInfo->path.c_str());
				return;
			}
			items.Push(pAsset);
		};
		...
		while (iterator.hasNext())
		{
			...
			tukk szRelativeFilePath = filePath.c_str() + offsetToRelativePath;
			...
			{
				DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Failed to read asset file: \"%s\".", filePath.c_str());
				continue;
			}
			pAssetInfo.release();
		}
	...
	template<typename F>
	void EnumerateAssetFiles(tukk szAssetsRootPath, const bool bPakFilesOnly,  F f)
	{
		...
		tukk szAlias = pPak->GetAlias(szAssetsRootPath, false);
	...
	std::vector<CAsset*> LoadAssetsOsFileSystem(const std::vector<string>& assetRootPaths, AssetLoader::Predicate& predicate)
	{
		...
		{
			IDrxPak* const pPak = GetISystem()->GetIPak();
			tukk szAlias = pPak->GetAlias(path.c_str(), false);
			if (szAlias)
			{
	...
	std::vector<CAsset*> LoadAssetsPakFileSystem(const std::vector<string>& assetRootPaths, AssetLoader::Predicate& predicate)
	{
		...
		auto createAsset = [&buffer](tukk szFilename, uint64 timestamp) -> CAsset*
		{
			IDrxPak* const pPak = GetISystem()->GetIPak();
			...
			if (!file)
			{
				DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Failed to open asset file: \"%s\".", szFilename);
				return nullptr;
			}
		...
		{
			EnumerateAssetFiles(root, bPakFilesOnly, [&predicate, &assets, &createAsset](tukk szRelativeFilePath, uint64 timestamp)
			{
				if (!predicate(szRelativeFilePath, timestamp))
				{
					return;
				...
				if (!pAsset)
				{
					DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to load asset: \"%s\".", szRelativeFilePath);
					return;
				}
				assets.push_back(pAsset);
			});
