...
public:
	...
	{
		{
			...
			DRX_ASSERT(m_memoryUsageBytes >= freedSizeInBytes);
			DRX_ASSERT(m_openFilesCount);
	...
	virtual void WaitForIdle()
	{
		...
		m_condition.wait(lock, [this]()
		{
			...
			DRX_ASSERT((!m_openFilesCount) == (!m_memoryUsageBytes));
...
public:
	...
	{
		SCompletionData(CUniqueHandle&& file, size_t size, CompletionHandler completionHandler, uk pUserData)
			: file(std::move(file))
			, buffer(size)
...
public:
	...
	bool ReadFileAsync(tukk szFilePath, const CompletionHandler& completionHandler, uk pUserData) override
	{
		if (m_threads.empty())
		{
			return false;
