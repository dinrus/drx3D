...
static AssetLoader::SAssetMetadata GetMetadata(const CAsset& asset)
{
	...
	std::transform(dependencies.begin(), dependencies.end(), std::back_inserter(metadata.dependencies), [](const SAssetDependencyInfo& x) -> std::pair<string,i32>
	{
...
CAsset::CAsset(tukk type, const DrxGUID& guid, tukk name)
	: m_name(name)
	, m_guid(guid)
	, m_editor(nullptr)
{
...
void CAsset::SetName(tukk szName)
{
	m_name = szName;
}
...
void CAsset::SetMetadataFile(tukk szFilepath)
{
	m_metadataFile = PathUtil::ToUnixPath(szFilepath);

	tukk const szEngine = "%engine%";
	m_flags.readOnly = strnicmp(szEngine, szFilepath, strlen(szEngine)) == 0;
	m_folder.clear();
}

void CAsset::SetSourceFile(tukk szFilepath)
{
...
void CAsset::AddFile(const string& file)
{
	...
	if (file.empty())
	{
		DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_WARNING, string().Format("Ignoring addition of empty file '%s' to asset '%s'", file.c_str(), m_name.c_str()));
		return;
	}
	...
	if (it != m_files.end())
	{
		DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_WARNING, string().Format("Ignoring addition of duplicate file '%s' to asset '%s'", file.c_str(), m_name.c_str()));
		return;
	}
...
void CAsset::SetFiles(tukk szCommonPath, const std::vector<string>& filenames)
{
	m_files.clear();
...
void CAsset::NotifyChanged(i32 changeFlags /*= eAssetChangeFlags_All*/)
{
	signalChanged(*this, changeFlags);
	DRX_ASSERT(CAssetManager::GetInstance());
