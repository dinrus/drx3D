...
struct IUIContext
{
	...
	virtual CAsset* QueryNewAsset(const CAssetType& type, ukk pTypeSpecificParameter) = 0;
	virtual ~IUIContext() {}
};
...
public:
	...
	i32 GetAssetsCount() const { return m_assets.size(); }

	CAssetType* FindAssetType(tukk name);
	...
	CAsset* FindAssetForMetadata(tukk szFilePath) const;

	//! Finds an asset with matching file associated. Includes data and metadata files
	//! \param szFilePath Relative to the assets root directory.
	CAsset* FindAssetForFile(tukk szFilePath) const;
	...
	void MoveAssets(const std::vector<CAsset*>& assets, tukk szDestinationFolder) const;
	...
	bool RenameAsset(CAsset* pAsset, tukk szNewName);
	...
	std::vector<std::pair<CAsset*, i32>> GetReverseDependencies(const CAsset& asset) const;
	...
	tukk GetAliasName(tukk alias) const;

	//! Returns a list of known asset root aliases.
	std::vector<tukk > GetAliases() const;
	...
	void GenerateDrxassetsAsync(const std::function<void()>& finalize);
	...
	CDrxSignal<void()> signalBeforeAssetsUpdated;
	CDrxSignal<void()> signalAfterAssetsUpdated;

	//! Braces the insertion of new assets.
	CDrxSignal<void(const std::vector<CAsset*>&)> signalBeforeAssetsInserted;
	CDrxSignal<void(const std::vector<CAsset*>&)> signalAfterAssetsInserted;

	//! Braces the removing of existing assets.
	CDrxSignal<void(const std::vector<CAsset*>&)> signalBeforeAssetsRemoved;
	CDrxSignal<void()> signalAfterAssetsRemoved;

	//! Called when any asset changes. Equivalent to CAsset::signalChanged
	//! \sa AssetChangeFlags
	CDrxSignal<void(CAsset&, i32 /*changeFlags*/)> signalAssetChanged;

	CDrxSignal<void()> signalScanningCompleted;

	CDrxSignal<void(CAbstractMenu&, const std::vector<CAsset*>&, const std::shared_ptr<IUIContext>&)> signalContextMenuRequested;
...
private:
	...
	const static std::pair<tukk , tukk > m_knownAliases[];
