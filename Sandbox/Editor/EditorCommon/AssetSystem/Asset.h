...
struct SAssetDependencyInfo
{
	...
	SAssetDependencyInfo(const string& path, i32 usageCount) : path(path), usageCount(usageCount)
	{
	}

	string path;        //!< The path is relative to the assets root directory.
	i32  usageCount;  //!< The instance count for the path or 0 if such information is not available.
};
...
public:
	CAsset(tukk type, const DrxGUID& guid, tukk name);
	virtual ~CAsset();
	...
	tukk GetFile(size_t i) const { return m_files[i]; }
	...
	tukk GetSourceFile() const { return m_sourceFile; }
	...
	tukk GetMetadataFile() const { return m_metadataFile; }
	...
	//! \sa AssetChangeFlags
	CDrxSignal<void(CAsset&, i32 /*changeFlags*/)> signalChanged;
...
private:
	...
	void NotifyChanged(i32 changeFlags = eAssetChangeFlags_All);
