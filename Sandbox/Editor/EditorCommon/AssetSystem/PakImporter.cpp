...
void CPakImporter::OnFileChange(tukk szFilename, EChangeType changeType)
{
...
void CPakImporter::ProcessQueue()
{
	...
	if (!m_queue.empty())
	{
		i32k timeout = (pak == m_queue.back()) ? 500 : 0;
...
{
	typedef std::unique_ptr<IDrxPak::PakInfo, void(*)(IDrxPak::PakInfo*)> PakInfoPtr;
	PakInfoPtr pPakInfo(GetISystem()->GetIPak()->GetPakInfo(), [](IDrxPak::PakInfo* p)
	{
...
void CPakImporter::ImportAssets(const string& pak)
{
	...
	GenerateDrxassets(tempRoot.c_str(), [&notif, &progress](float v)
	{
	...
	{
		std::vector<CAsset*> assets = AssetLoader::CAssetFactory::LoadAssetsFromPakFile(cryassetPakPath, [](tukk szAssetRelativePath, uint64 timestamp)
		{
			return true;
		});
...
string CPakImporter::GetTemporaryDirectoryPath()
{
	...
	return gEnv->pDrxPak->AdjustFileName("%USER%/temp", path, IDrxPak::FLAGS_PATH_REAL | IDrxPak::FLAGS_FOR_WRITING | IDrxPak::FLAGS_ADD_TRAILING_SLASH);
}

// The pak should be opened.
void CPakImporter::Unpak(tukk szArchivePath, tukk szDestPath, std::function<void(float)> progress)
{
	...
	while (!stack.empty())
	{
		...
		GetISystem()->GetIPak()->ForEachArchiveFolderEntry(szArchivePath, mask, [szDestPath, &stack, &folder, &buffer, &progressValue, progress](const IDrxPak::ArchiveEntryInfo& entry)
		{
			...
			DrxPathString tempFilepath(PathUtil::Make(szDestPath, path));
...
void CPakImporter::GenerateDrxassets(tukk szPath, std::function<void(float)> progress)
{
	RCLogger rcLogger;
