...
public:
	...
	bool rowMatchesFilter(i32 sourceRow, const QModelIndex& sourceParent) const
	{
		if (m_bHideEngineFolder)
		{
			...
			QString path = index.data((i32)CAssetFoldersModel::Roles::FolderPathRole).toString();
			if (path.startsWith("%"))
			{
				return false;
			}
...
{
	DRX_ASSERT(folderIndex.model() == CAssetFoldersModel::GetInstance());
...
void CAssetFoldersView::SelectFolders(const QModelIndexList& foldersIndices)
{
	...
	{
		DRX_ASSERT(index.model() == CAssetFoldersModel::GetInstance());
		auto proxyModel = qobject_cast<QAbstractProxyModel*>(m_treeView->model());
...
void CAssetFoldersView::OnContextMenu(const QPoint &pos)
{
	...
	auto action = menu->addAction(DrxIcon("icons:General/Element_Add.ico"), tr("Create folder"));
	...
	if (CAssetFoldersModel::GetInstance()->IsEmptyFolder(folder))
	{
		...
		action = menu->addAction(DrxIcon("icons:General/Element_Remove.ico"), tr("Delete"));
...
void CAssetFoldersView::OnSelectionChanged()
{
	...
	if (selection.isEmpty())
	{
		...
		m_selectedFolders.push_back(assetsRootIndex.data((i32)CAssetFoldersModel::Roles::FolderPathRole).toString());
	}
	else
	{
		...
		{
			m_selectedFolders.push_back(index.data((i32)CAssetFoldersModel::Roles::FolderPathRole).toString());
		}
	}
