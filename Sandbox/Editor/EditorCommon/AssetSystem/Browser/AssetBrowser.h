...
public:
	...
	CAsset* QueryNewAsset(const CAssetType& type, ukk pTypeSpecificParameter);
...
private:
	...
	void AddViewModeButton(ViewMode viewMode, tukk szIconPath, tukk szToolTip, QMenu* pMenu = nullptr);
	QWidget* CreateAssetsViewSelector();

	void FillCreateAssetMenu(CAbstractMenu* menu, const QString& folder);

	void BeginCreateAsset(const CAssetType& type, ukk pTypeSpecificParameter);
	void EndCreateAsset();
	...
	i32							m_navigationIndex; //-1 is "all assets"
