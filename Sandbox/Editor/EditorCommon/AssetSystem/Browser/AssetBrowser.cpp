...
static i32 GetType(const QModelIndex& index)
{
	return (EAssetModelRowType)index.data((i32)CAssetModel::Roles::TypeCheckRole).toUInt();
}

static bool IsAsset(const QModelIndex& index)
{
	bool ok = false;
	return index.data((i32)CAssetModel::Roles::TypeCheckRole).toUInt(&ok) == eAssetModelRow_Asset && ok;
}

static bool IsFolder(const QModelIndex& index)
{
	bool ok = false;
	return index.data((i32)CAssetModel::Roles::TypeCheckRole).toUInt(&ok) == eAssetModelRow_Folder && ok;
}

static CAsset* ToAsset(const QModelIndex& index)
{
	return reinterpret_cast<CAsset*>(index.data((i32)CAssetModel::Roles::InternalPointerRole).value<intptr_t>());
}

static QString ToFolderPath(const QModelIndex& index)
{
	return index.data((i32)CAssetFoldersModel::Roles::FolderPathRole).toString();
}
...
protected:
	...
	virtual void scrollContentsBy(i32 dx, i32 dy) override
	{
...
private:
	void TouchVisibleAssetsBatched(i32 firstBatchRow)
	{
		...
		i32k batchSize = 1 << 3;
		i32k lastRow = model()->rowCount() - 1;
		i32k lastBatchRow = std::min(lastRow, firstBatchRow + batchSize);
		for (i32 i = lastBatchRow; i >= firstBatchRow; --i)
		{
			QModelIndex index = model()->index(i, (i32)eAssetColumns_Thumbnail);
			if (index.isValid())
			{
				CAsset* const pAsset = (CAsset*)(index.data((i32)CAssetModel::Roles::InternalPointerRole).value<intptr_t>());
...
class SortFilterProxyModel : public QAttributeFilterProxyModel
{
	...
	{
		EAssetModelRowType leftType = (EAssetModelRowType)left.data((i32)CAssetModel::Roles::TypeCheckRole).toUInt();
		EAssetModelRowType rightType = (EAssetModelRowType)right.data((i32)CAssetModel::Roles::TypeCheckRole).toUInt();
		...
		if (leftType == rightType)
		{
			...
			{
				return left.data((i32)CAssetModel::Roles::InternalPointerRole).value<intptr_t>() < right.data((i32)CAssetModel::Roles::InternalPointerRole).value<intptr_t>();
			}
			else
			{
	...
	bool rowMatchesFilter(i32 sourceRow, const QModelIndex& sourceParent) const
	{
		...
		EAssetModelRowType rowType = (EAssetModelRowType)index.data((i32)CAssetModel::Roles::TypeCheckRole).toUInt();
...
{
	DRX_ASSERT(CAssetManager::GetInstance()->GetAssetImporters().size() > 0);
...
public:
	...
	virtual CAsset* QueryNewAsset(const CAssetType& type, ukk pTypeSpecificParameter) override
	{
...
void CAssetBrowser::dropEvent(QDropEvent* pEvent)
{
	...
	if (!filePaths.empty())
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_WARNING, tr("Select a single target folder for import").toLocal8Bit().constData());
...
{
	DRX_ASSERT(m_foldersView && m_foldersView->GetInternalView());
	DRX_ASSERT(m_thumbnailView && m_thumbnailView->GetInternalView());
	DRX_ASSERT(m_detailsView);
...
void CAssetBrowser::InitViews(bool bHideEngineFolder)
{
	...
	m_backButton->setIcon(DrxIcon("icons:General/Arrow_Left.ico"));
	m_backButton->setToolTip(tr("Back"));
	m_backButton->setEnabled(false);
	connect(m_backButton, &QToolButton::clicked, this, &CAssetBrowser::OnNavBack);

	m_forwardButton = new QToolButton();
	m_forwardButton->setIcon(DrxIcon("icons:General/Arrow_Right.ico"));
...
void CAssetBrowser::InitMenus()
{
	...
	AddToMenu(&items[0], DRX_ARRAY_COUNT(items));
	...
	menuFile->signalAboutToShow.Connect([menuFile, this]()
	{
		...
		i32 section = menuFile->GetNextEmptySection();
	...
	menuEdit->signalAboutToShow.Connect([menuEdit, this]()
	{
		...
		if (!CAssetManager::GetInstance()->IsScanning())
		{
			connect(action, &QAction::triggered, []()
			{
				...
				CAssetManager::GetInstance()->GenerateDrxassetsAsync([pNotification]()
				{
				});
			});
		}
	...
	menuView->signalAboutToShow.Connect([menuView, this]()
	{
		...
		i32 sec = menuView->GetNextEmptySection();
...
void CAssetBrowser::InitDetailsView()
{
	...
	m_detailsView->sortByColumn((i32)eAssetColumns_Name, Qt::AscendingOrder);
	m_detailsView->setContextMenuPolicy(Qt::CustomContextMenu);
	m_detailsView->header()->setStretchLastSection(false);
	m_detailsView->header()->resizeSection((i32)eAssetColumns_Name, fontMetrics().width(QStringLiteral("wwwwwwwwwwwwwwwwwwwwwwwwww")));
	m_detailsView->header()->resizeSection((i32)eAssetColumns_Type, fontMetrics().width(QStringLiteral("wwwwwwwwwwwwwww")));
	m_detailsView->setTreePosition((i32)eAssetColumns_Name);
...
void CAssetBrowser::AddViewModeButton(ViewMode viewMode, tukk szIconPath, tukk szToolTip, QMenu* pMenu)
{
...
void CAssetBrowser::SelectAsset(tukk szPath) const
{
...
void CAssetBrowser::EditNewAsset()
{
	...
	i32k col = pView == m_thumbnailView->GetInternalView() ? eAssetColumns_Thumbnail : eAssetColumns_Name;
...
void CAssetBrowser::BeginCreateAsset(const CAssetType& type, ukk pTypeSpecificParameter)
{
	auto folderSelection = m_foldersView->GetSelectedFolders();
...
CAsset* CAssetBrowser::QueryNewAsset(const CAssetType& type, ukk pTypeSpecificParameter)
{
...
QVariantMap CAssetBrowser::GetLayout() const
{
	...
	state.insert("viewMode", (i32)m_viewMode);
...
void CAssetBrowser::SetViewMode(ViewMode viewMode)
{
	if (m_viewMode != viewMode)
	{
		...
		DRX_ASSERT(m_viewModeButtons->buttons().size() == ViewMode::Max);
		for (i32 i = 0; i < ViewMode::Max; i++)
		{
...
void CAssetBrowser::OnContextMenu()
{
	...
	if (assets.length() != 0)
	{
		...
		i32 section = abstractMenu.FindSectionByName("Assets");
	...
	if (folders.length() == 1)//only one folder selected
	{
		...
		i32 section = abstractMenu.GetNextEmptySection();
		abstractMenu.SetSectionName(section, "Folders");

		if (CAssetFoldersModel::GetInstance()->IsEmptyFolder(folder))
		{
			auto action = abstractMenu.CreateAction(DrxIcon("icons:General/Element_Remove.ico"), tr("Delete"));
	...
	if (assets.length() == 0 && folders.length() == 0 && !m_recursiveView)//nothing selected in recursive view
	{
		...
		if (selectedFolders.count() == 1 && !pModel->IsReadOnlyFolder(selectedFolders.first()))
		{
			...
			auto action = abstractMenu.CreateAction(DrxIcon("icons:General/Element_Add.ico"), tr("Create folder"));
...
void CAssetBrowser::OnImport()
{
	...
	static tukk const szRecentImportPathProperty = "RecentImportPath";
	...
	else
	{
		if (filePaths.size() > 1)
		{
			DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_ERROR, "Cannot import files.");
		}
		else
		{
			DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_ERROR, "Cannot import file '%s'.", filePaths.front().c_str());
		}
	}
}
...
{
	DRX_ASSERT(std::none_of(assets.begin(), assets.end(), [](CAsset* pAsset) { return !pAsset; }));
...
{
	i32k numFolders = selectedFolders.size();
	if (numFolders > 1)
	{
		m_breadcrumbs->hide();
