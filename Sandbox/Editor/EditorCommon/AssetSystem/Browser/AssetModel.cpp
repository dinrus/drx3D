...
void CAssetModel::BuildDetailAttributes()
{
	for (CAssetType* pAssetType : CAssetManager::GetInstance()->GetAssetTypes())
	{
		for (CItemModelAttribute* pAttribute : pAssetType->GetDetails())
		{
			if (!pAttribute)
			{
				DrxWarning(VALIDATOR_MODULE_ASSETS, VALIDATOR_ERROR, "Asset type '%s' has null attribute", pAssetType->GetTypeName());
				continue;
			}
...
void CAssetModel::AddPredefinedComputedColumns()
{
	...
	{
		DRX_ASSERT(&fileExtensionAttribute == pAttribute);
	...
	{
		DRX_ASSERT(&filesCountAttribute == pAttribute);
		return pAsset ? QVariant(pAsset->GetFilesCount()) : QVariant();
	});
}

const CItemModelAttribute* CAssetModel::GetColumnAttribute(i32 column)
{
	switch (column)
	{
	...
	i32k detailColumn = column - (i32)eAssetColumns_Details;
	if (detailColumn >= 0 && detailColumn < (i32)GetInstance()->m_detailAttributes.size())
	{
...
i32 CAssetModel::rowCount(const QModelIndex& parent) const
{
	if (!parent.isValid())
...
i32 CAssetModel::GetColumnCount()
{
...
bool CAssetModel::AddComputedColumn(const CItemModelAttribute* pAttribute, std::function<QVariant(const CAsset* pAsset, const CItemModelAttribute* pAttribute)> computeFn)
{
	...
	{
		DRX_ASSERT_MESSAGE(false, "Adding computed columns after CAssetModel::Init() is not allowed.");
		return false;
	}
...
i32 CAssetModel::columnCount(const QModelIndex& parent) const
{
...
QModelIndex CAssetModel::ToIndex(const CAsset& asset, i32 col) const
{
	...
	i32k row = (i32)std::distance(assets.begin(), it);

	return createIndex(row, col, (quintptr)&asset);
}

QVariant CAssetModel::data(const QModelIndex& index, i32 role) const
{
	if (!index.isValid())
	{
		return QVariant();
	...
	case (i32)Roles::TypeCheckRole:
		return (i32)eAssetModelRow_Asset;
	...
	if (role == Qt::DisplayRole)
	{
		i32k detailColumn = index.column() - (i32)eAssetColumns_Details;
		if (detailColumn >= 0 && detailColumn < m_detailAttributes.size())
		{
			...
			// For example, an invalid variant is greater than any variant storing an i32.
...
bool CAssetModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
	if (index.isValid() && (index.column() == eAssetColumns_Name || index.column() == eAssetColumns_Thumbnail) && role == Qt::EditRole)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Invalid asset name: %s", newName.c_str());
			return false;
		}
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Asset already exists: %s", newAssetFilepath.c_str());
			return false;
		}
...
QVariant CAssetModel::GetHeaderData(i32 section, Qt::Orientation orientation, i32 role)
{
...
QVariant CAssetModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
QModelIndex CAssetModel::index(i32 row, i32 column, const QModelIndex& parent /*= QModelIndex()*/) const
{
...
{
	DRX_ASSERT(assets.size());
	i32k assetCount = CAssetManager::GetInstance()->GetAssetsCount();
...
void CAssetModel::PostInsert(const std::vector<CAsset*>& assets)
{
	...
	DRX_ASSERT(assets.size() <= CAssetManager::GetInstance()->GetAssetsCount());
	DRX_ASSERT(std::equal(assets.rbegin(), assets.rend(), CAssetManager::GetInstance()->m_assets.rbegin()));
