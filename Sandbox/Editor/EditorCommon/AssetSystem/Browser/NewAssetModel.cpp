...
void CNewAssetModel::BeginCreateAsset(const string& path, const string& name, const CAssetType& type, ukk pTypeSpecificParameter)
{
	if (m_pRow)
	{
		return;
...
i32 CNewAssetModel::rowCount(const QModelIndex& parent) const
{
	return !parent.isValid() && m_pRow ? 1 : 0;
}

i32 CNewAssetModel::columnCount(const QModelIndex& parent) const
{
	return CAssetModel::GetColumnCount();
}

QVariant CNewAssetModel::data(const QModelIndex& index, i32 role) const
{
	if (!index.isValid())
	{
		return QVariant();
	...
	if (role == (i32)CAssetModel::Roles::TypeCheckRole)
	{
...
bool CNewAssetModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
...
QModelIndex CNewAssetModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
	if (!parent.isValid())
	{	
...
QVariant CNewAssetModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
