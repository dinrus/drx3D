...
public:
		...
		, m_parent(parent)
	{
		...
		m_connections.push_back(connect(sourceModel, &CAssetFoldersModel::rowsInserted, [this](const QModelIndex&, i32, i32) { UpdateFolders(); }));
	}
	...
	void UpdateFolders()
	{
		i32k folderCount = m_folders.count();
		if(folderCount > 0)
		{
			...
			if(m_parent->m_acceptedFolders.count() > 0)
			{
				for (auto& folder : m_parent->m_acceptedFolders)
				{
					...
					if (index.isValid())
					{
						...
						for (i32 i = 0; i < subFolderCount; i++)
	...
	QModelIndex mapFromSource(const QModelIndex &sourceIndex) const override
	{
		if(sourceIndex.isValid())
		{
			...
			for (i32 i = 0; i < m_folders.count(); i++)
			{
	...
	QVariant data(const QModelIndex& index, i32 role) const override
	{
		if (index.isValid())
		{
			...
			case (i32)CAssetModel::Roles::TypeCheckRole:
				return (i32)eAssetModelRow_Folder;
...
void CAssetFolderFilterModel::SetAcceptedFolders(const QStringList& folders)
{
	if (folders.size() == 1)
	{
		...
		const QString assetsRootPath = assetsRootIndex.data((i32)CAssetFoldersModel::Roles::FolderPathRole).toString();
