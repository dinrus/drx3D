...
public:
	...
	std::vector<CAsset*> GetDependentAssets() const
	{
		...
		{
			std::vector<std::pair<CAsset*,i32>> tmp = CAssetManager::GetInstance()->GetReverseDependencies(*pAsset);
...
protected:
	bool filterAcceptsRow(i32 sourceRow, const QModelIndex& sourceParent) const override
	{
	...
	{
		QVariant variant = index.data((i32)CAssetModel::Roles::InternalPointerRole);
		if (!variant.isValid())
		{
			return nullptr;
		}
...
static QAdvancedTreeView* CreateAssetsView(QAbstractItemModel* pModel, QAbstractItemView::SelectionMode selectionMode)
{
	...
	pView->sortByColumn((i32)eAssetColumns_Name, Qt::AscendingOrder);
	pView->header()->setStretchLastSection(false);
	pView->header()->resizeSection((i32)eAssetColumns_Name, pView->fontMetrics().width(QStringLiteral("wwwwwwwwwwwwwwwwwwwwwwwwww")));
	pView->header()->resizeSection((i32)eAssetColumns_Type, pView->fontMetrics().width(QStringLiteral("wwwwwwwwwwwwwww")));
	pView->setTreePosition((i32)eAssetColumns_Name);
...
static QWidget* CreateInfoBox(const QString& infoText)
{
	...
	i32k iconSize = 48;
	pIconLabel->setPixmap(DrxIcon("icons:Dialogs/dialog-warning.ico").pixmap(iconSize, iconSize, QIcon::Normal, QIcon::On));
