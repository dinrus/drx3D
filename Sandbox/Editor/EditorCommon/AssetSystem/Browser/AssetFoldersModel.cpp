	...
	: QAbstractItemModel(parent)
{
	...
	tukk gameFolder = gEnv->pConsole->GetCVar("sys_game_folder")->GetString();
...
i32 CAssetFoldersModel::rowCount(const QModelIndex& parent) const
{
	const Folder* folder = ToFolder(parent);
	return folder->m_subFolders.size();
}

QVariant CAssetFoldersModel::data(const QModelIndex& index, i32 role) const
{
	...
	if (folder && index.column() == 0)
	{
		...
		case Qt::DecorationRole:
			return DrxIcon("icons:General/Folder.ico");
		case (i32)CAssetModel::Roles::TypeCheckRole:
			return (i32)EAssetModelRowType::eAssetModelRow_Folder;
		case (i32)Roles::FolderPathRole:
			return GetPath(folder);//TODO: this could be cached for optimization
		case (i32)Roles::DisplayFolderPathRole:
			return GetPrettyPath(GetPath(folder));
		case (i32)CAssetModel::Roles::InternalPointerRole:
...
bool CAssetFoldersModel::setData(const QModelIndex& index, const QVariant& value, i32 role /*= Qt::EditRole*/)
{
	if (index.isValid() && index.column() == 0 && role == Qt::EditRole)
	{
		Folder* folder = ToFolder(index);
		DRX_ASSERT(folder->m_empty);
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Invalid folder name: %s", newName.toStdString().c_str());
			return false;
		}
		...
		DRX_ASSERT(m_addedFolders.removeOne(oldName));
		...
		dataChanged(index, index, QVector<i32>() << Qt::DisplayRole);
	}
	return false;
}

QVariant CAssetFoldersModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
QModelIndex CAssetFoldersModel::index(i32 row, i32 column, const QModelIndex& parent /*= QModelIndex()*/) const
{
	const Folder* folder = ToFolder(parent);
...
void CAssetFoldersModel::PreInsert(const std::vector<CAsset*>& assets)
{
	for (CAsset* pAsset : assets)
	{
		...
		DRX_ASSERT(parentFolders.size() && pNewFolder == parentFolders.back().get());
...
CAssetFoldersModel::Folder* CAssetFoldersModel::AddFolder(tukk path, bool empty)
{
...
CAssetFoldersModel::Folder* CAssetFoldersModel::CreateSubFolder(Folder* parent, tukk childName, bool empty)
{
...
const CAssetFoldersModel::Folder* CAssetFoldersModel::GetFolder(tukk path) const
{
	auto segments = PathUtil::SplitIntoSegments(path);
...
QModelIndex CAssetFoldersModel::FindIndexForFolder(const QString& folder, Roles role) const
{
	...
	matches = match(index(0, 0, QModelIndex()), (i32)role, adjustedFolder, 1, Qt::MatchExactly | Qt::MatchRecursive);
...
QModelIndex CAssetFoldersModel::ToIndex(const Folder* pFolder) const
{
	...
	matches = match(index(0, 0, QModelIndex()), (i32)CAssetModel::Roles::InternalPointerRole, reinterpret_cast<intptr_t>(pFolder), 1, Qt::MatchExactly | Qt::MatchRecursive);
...
void CAssetFoldersModel::CreateFolder(const QString& parentPath, const QString& folderName)
{
	...
	DRX_ASSERT(parentIndex.isValid());
...
void CAssetFoldersModel::DeleteFolder(const QString& folderPath)
{
	...
	if (folderIndex.isValid())
	{
		...
		DRX_ASSERT(m_addedFolders.removeOne(folderPath));
		endRemoveRows();
	}
}
...
const QString& CAssetFoldersModel::GetProjectAssetsFolderName() const 
{
	...
	DRX_ASSERT(m_root.m_subFolders.size() >= 0);
...
QString CAssetFoldersModel::GetPrettyPath(const QString& path) const
{
	...
	{
		i32k aliasEndIndex = path.indexOf('%', 1);
