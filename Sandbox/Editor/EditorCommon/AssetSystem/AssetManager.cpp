...
const std::pair<tukk , tukk > CAssetManager::m_knownAliases[] =
{
	{ "%engine%", "Engine" }
};
...
CAssetType* CAssetManager::FindAssetType(tukk name)
{
...
CAsset* CAssetManager::FindAssetForMetadata(tukk szFilePath) const
{
...
CAsset* CAssetManager::FindAssetForFile(tukk szFilePath) const
{
	MAKE_SURE(szFilePath, return nullptr);
	...
	for (size_t i = 0; i < count; )
	{
		...
		if (pOther)
		{
			if (pOther != assets[i])
			{
				...
				DRX_ASSERT(pOther->GetType() == assets[i]->GetType());
				// DRX_ASSERT(pOther->GetGUID() == assets[i]->GetGUID());  Re-enable this after stabilization.
				DRX_ASSERT(!stricmp(pOther->GetName(), assets[i]->GetName()));
...
void CAssetManager::MoveAssets(const std::vector<CAsset*>& assets, tukk szDestinationFolder) const
{
	for (CAsset* pAsset : assets)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Asset already exists: %s", newMetadataFile.c_str());
			continue;
		}
...
bool CAssetManager::RenameAsset(CAsset* pAsset, tukk szNewName)
{
	using namespace Private_AssetManager;
...
{
	DRX_ASSERT(pAsset && pAsset->GetType() && !pAsset->IsReadOnly());

	tukk szSourceFile = pAsset->GetSourceFile();
	...
	ext.MakeLower();
	tukk const szAssetType = pAsset->GetType()->GetTypeName();
...
void CAssetManager::AppendContextMenuActions(CAbstractMenu& menu, const std::vector<CAsset*>& assets, const std::shared_ptr<IUIContext>& context) const
{
	...
	{
		i32 section = menu.GetNextEmptySection();
...
void CAssetManager::UpdateAssetTypes()
{
	...
	// ... not have a leading dot, nor a trailing "cryasset".
	{
		...
		auto endsWithDrxasset = [](const string& s)
		{
		...
		for (CAssetType* pAssetType : m_assetTypes)
		{
			...
			tukk szExt = pAssetType->GetFileExtension();
			DRX_ASSERT_MESSAGE(szExt && *szExt, "File extension of type %s must not be empty", pAssetType->GetTypeName());
			DRX_ASSERT_MESSAGE(szExt[0] != '.', "File extension of type %s must not have leading dot", pAssetType->GetTypeName());
			DRX_ASSERT_MESSAGE(isLowerCase(szExt), "File extension of type %s must be all-lower case", pAssetType->GetTypeName());
			DRX_ASSERT_MESSAGE(!endsWithDrxasset(szExt), "File extension of type %s must be all-lower case", pAssetType->GetTypeName());
		}
		...
		{
			DRX_ASSERT_MESSAGE(false, "Two asset types cannot be registered with the same file extension : %s", (*it)->GetFileExtension());
		}
	}
	...
	// Type names must be unique among asset types.
	{
		...
		if (it != m_assetTypes.end())
		{
			DRX_ASSERT_MESSAGE(false, "Two asset types cannot be registered with the same type name : %s", (*it)->GetTypeName());
		}
	}

	// CAssetManager::FindAssetType() assumes types to be sorted by name.
	DRX_ASSERT(std::is_sorted(m_assetTypes.begin(), m_assetTypes.end(), [](const auto a, const auto b)
	{
		return strcmp(a->GetTypeName(), b->GetTypeName()) < 0;
	}));
...
std::vector<std::pair<CAsset*,i32>> CAssetManager::GetReverseDependencies(const CAsset& asset) const
{
	...
	std::vector<std::pair<CAsset*,i32>> dependencies;
...
std::vector<CAsset*> CAssetManager::GetReverseDependencies(const std::vector<CAsset*>& assets) const
{
	...
	for( CAsset* pAsset : ordered)
	{
		...
		std::vector<std::pair<CAsset*,i32>> tmp = GetReverseDependencies(*pAsset);
...
bool CAssetManager::HasAnyReverseDependencies(const std::vector<CAsset*>& assets) const
{
	return std::any_of(assets.cbegin(), assets.cend(), [&assets, this](CAsset* pAsset)
	{
		...
		std::vector<std::pair<CAsset*, i32>> dependencyInfos(GetReverseDependencies(*pAsset));
...
tukk CAssetManager::GetAliasName(tukk alias) const
{
...
std::vector<tukk > CAssetManager::GetAliases() const
{
	std::vector<tukk > aliases;
...
void CAssetManager::GenerateDrxassetsAsync(const std::function<void()>& finalize)
{
	using namespace AssetManagerHelpers;
	...
	ThreadingUtils::Async([this, finalize]()
	{
		...
		CAssetGenerator::GenerateDrxassets();
