...
void CAssetGenerator::OnFileChange(tukk szFilename, EChangeType changeType)
{
	...
	static tukk const update[] = { "mtl", "cdf" };
	tukk szExt = PathUtil::GetExt(szFilename);
	const bool updateExisting = std::any_of(std::begin(update), std::end(update), [szExt](tukk szUpdatable)
	{
		return stricmp(szExt, szUpdatable) == 0;
	});

	tukk const szAssetDirectory = GetIEditor()->GetProjectManager()->GetCurrentAssetDirectoryAbsolute();
	...
	m_fileQueue.ProcessItemUniqueAsync(filePath, [this, updateExisting](const string& path)
	{
		...
		GenerateDrxasset(path);
		return true;
	});
}

bool CAssetGenerator::GenerateDrxassets()
{
...
void CAssetGenerator::GenerateDrxasset(const string& filePath)
{
...
void CAssetGenerator::OnCompilationStarted(tukk szSource, tukk szTarget, i32 nPending)
{
...
void CAssetGenerator::OnCompilationFinished(tukk szSource, tukk szTarget, ERcExitCode eReturnCode)
{
}

void CAssetGenerator::OnCompilationQueueTriggered(i32 nPending)
{
	std::unique_lock<std::mutex> lock(m_textureCompilerMutex);
