...
{
	tukk szFileName = PathUtil::GetFile(oldFilepath.c_str());
...
static bool RenameAssetFile(const string& oldFilename, const string& newFilename)
{
	...
	DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to rename asset file %s", oldFilename.c_str());
	return false;
}
...
static bool CopyAssetFile(const string& oldFilename, const string& newFilename)
{
	...
	DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to copy asset file %s to %s", oldFilename.c_str(), newFilepath.c_str());
	return false;
}
...
QVariant CAssetType::GetVariantFromDetail(const string& detailValue, const CItemModelAttribute* pAttrib)
{
	...
	case eAttributeType_Enum:
		return v.value<i32>();
	default:
		DRX_ASSERT(0 && "Unknown attribute type");
		return v;
	}
}

bool CAssetType::Create(tukk szFilepath, ukk pTypeSpecificParameter) const
{
	...
	CAsset* const pNewAsset(new CAsset(GetTypeName(), DrxGUID::Create(), AssetLoader::GetAssetName(szFilepath)));
...
DrxIcon CAssetType::GetIcon() const
{
	return m_icon;
}

DrxIcon CAssetType::GetIconInternal() const
{
	return DrxIcon("icons:General/Placeholder.ico");
}
...
bool CAssetType::DeleteAssetFiles(const CAsset& asset, bool bDeleteSourceFile, size_t& numberOfFilesDeleted) const
{
	...
	if (asset.IsReadOnly())
	{
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to delete asset \"%s\": the asset is read only.", asset.GetName());
		return false;
	}
...

	{
		...
		for (const string& path : files)
		{
			...
			if (!fileInfo.exists())
			{
				if (GetISystem()->GetIPak()->IsFileExist(PathUtil::AbsolutePathToGamePath(path), IDrxPak::eFileLocation_InPak))
				{
					DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to delete asset. The file is in pak: \"%s\"", path.c_str());
					bReadOnly = true;
				}
				continue;
			}
			...
			{
				DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to delete asset. The file is read-only: \"%s\"", path.c_str());
				bReadOnly = true;
				continue;
			}
...
bool CAssetType::RenameAsset(CAsset* pAsset, tukk szNewName) const
{
	...
	{
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to rename asset \"%s\": the asset is read only.", pAsset->GetName());
		return false;
	}
	...
	bool bReadOnly = false;
	{
		...
		for (string& path : filepaths)
		{
			...
			if (!fileInfo.exists())
			{
				if (GetISystem()->GetIPak()->IsFileExist(PathUtil::AbsolutePathToGamePath(path), IDrxPak::eFileLocation_InPak))
				{
					DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to rename asset. The file is in pak: \"%s\"", path.c_str());
					bReadOnly = true;
				}
			}
			else if (!fileInfo.isWritable())
			{
				DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to rename asset. The file is read-only: \"%s\"", path.c_str());
				bReadOnly = true;
			}
		}
	}
	...
	{
		// Should newer happen.
		DRX_ASSERT_MESSAGE(0, "The asset name \"%s\" differs from the metadata filename \"%s\"", pAsset->GetName(), pAsset->GetMetadataFile());
		return false;
	}
...
bool CAssetType::MoveAsset(CAsset* pAsset, tukk szDestinationFolder, bool bMoveSourcefile) const
{
	if (pAsset->IsReadOnly())
	{
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to move asset \"%s\": the asset is read only.", pAsset->GetName());
		return false;
	}
	...
	bool bReadOnly = false;
	{
		...
		for (string& path : filepaths)
		{
			...
			if (!fileInfo.exists())
			{
				if (GetISystem()->GetIPak()->IsFileExist(PathUtil::AbsolutePathToGamePath(path), IDrxPak::eFileLocation_InPak))
				{
					DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to move asset. The file is in pak: \"%s\"", path.c_str());
					bReadOnly = true;
				}
			}
			else if (!fileInfo.isWritable())
			{
				DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to move asset. The file is read-only: \"%s\"", path.c_str());
				bReadOnly = true;
			}
		}
	}
	...
	{
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable create target directory: \"%s\"", szDestinationFolder);
		return false;
	}
...
bool CAssetType::CopyAsset(CAsset* pAsset, tukk szNewPath) const
{
	DRX_ASSERT(pAsset);
	...
	{
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable create target directory: \"%s\"", destinationFolder.c_str());
		return false;
	}

	// Create a new instance of asset with a unique guid.
	std::unique_ptr<CAsset> pNewAsset(new CAsset(GetTypeName(), DrxGUID::Create(), newName));
...
class CDrxAssetType : public CAssetType
{
public:
	DECLARE_ASSET_TYPE_DESC(CDrxAssetType);

	virtual tukk GetTypeName() const override { return "cryasset"; }
	virtual tukk GetUiTypeName() const override { return "cryasset"; }
	virtual tukk GetFileExtension() const override { return "unregistered"; }
...
REGISTER_ASSET_TYPE(CDrxAssetType)
