...
public:
	CAutoFileRecovery(const std::vector<string>& files)
	{
		...
		pDrxPak->MakeDir(tempPrefix.c_str());
		for (const string& file : files)
		{
			const string tempFilemane = PathUtil::Make(tempPrefix, DrxGUID::Create().ToString(), "tmp");
			if (!pDrxPak->CopyFileOnDisk(file.c_str(), tempFilemane.c_str(), false))
			{
				break;
			}
	...
	void Discard()
	{
		...
		{
			pDrxPak->RemoveFile(file.second.c_str());
		}
		m_files.clear();
	}
	...
	virtual ~CAutoFileRecovery()
	{
		...
		for (const auto& file : m_files)
		{
			pDrxPak->CopyFileOnDisk(file.second.c_str(), file.first.c_str(), false);
			pDrxPak->RemoveFile(file.second.c_str());
		}
	}
private:
	static string GetTemporaryDirectoryPath()
	{
		...
		return GetISystem()->GetIPak()->AdjustFileName("%USER%/temp", path, IDrxPak::FLAGS_PATH_REAL | IDrxPak::FLAGS_FOR_WRITING | IDrxPak::FLAGS_ADD_TRAILING_SLASH);
	}
...
public:
	...
	static std::vector<string> GetAssetFiles(const CAsset& asset, bool bIncludeSourceFile)
	{
		...
		{
			return !GetISystem()->GetIPak()->IsFileExist(filename.c_str(), IDrxPak::eFileLocation_OnDisk);
		}), files.end());
...
CAssetEditor* CAssetEditor::OpenAssetForEdit(tukk editorClassName, CAsset* asset)
{
	DRX_ASSERT(asset);
...
i32 CAssetEditor::GetNewableAssetCount() const
{
	i32 newableAssets = 0;
...
void CAssetEditor::SetAssetBeingEdited(CAsset* pAsset)
{
	...
	if (pAsset)
	{
		CAssetManager::GetInstance()->signalBeforeAssetsRemoved.Connect([this](const std::vector<CAsset*>& assets)
		{
			...
			{
			  OnCloseAsset();
			  DRX_ASSERT(GetAssetBeingEdited() != nullptr);
...
void CAssetEditor::OnAssetChanged(CAsset& asset, i32 changeFlags)
{
	DRX_ASSERT(&asset == m_assetBeingEdited);
...
{
	i32k newableAssets = GetNewableAssetCount();
...
{
	i32k newableAssetCount = GetNewableAssetCount();
...
bool CAssetEditor::OnSaveAs()
{
	...
	DRX_ASSERT(!m_supportedAssetTypes.empty());
...
bool CAssetEditor::InternalSaveAs(const string& newAssetPath)
{
	...
	{
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_ERROR, "Unable to copy asset: %s", m_assetBeingEdited->GetName());
		return false;
	}
...
bool CAssetEditor::SaveBackup(const string& backupFolder)
{
	...
	IDrxPak* const pDrxPak = GetISystem()->GetIPak();
	pDrxPak->MakeDir(backupFolder.c_str());
	const string assetsRoot = PathUtil::GetGameProjectAssetsPath();
	for (const auto& file : files)
	{
		DRX_ASSERT(strncmp(file.c_str(), assetsRoot.c_str(), assetsRoot.size()));
		DrxPathString destFile = PathUtil::Make(backupFolder.c_str(), file.c_str() + assetsRoot.size());
		pDrxPak->MakeDir(PathUtil::GetDirectory(destFile.c_str()));
