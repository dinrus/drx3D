...
public:
	...
	{
		DRX_ASSERT_MESSAGE(!name.empty(), "Argument name must not be empty!");
		DRX_ASSERT_MESSAGE(!description.empty(), "Argument description must not be empty!");
...
public:
	...
	public:
		...
		i32           GetArgCount() const
		{ return m_args.size(); }
		const string& GetArg(i32 i) const
		{
	...
	private:
		...
		u8    m_stringFlags; // This is needed to quote string parameters when logging a command.
	};
...
public:
	...
	static inline string ToString_(tukk val)       { return val; }
	static inline string ToString_(const string& val)     { return val; }
	template<typename T>
	static bool          FromString_(T& t, tukk s) { return ::FromString(t, s); }
	static inline bool   FromString_(tukk & val, tukk s)
	{ return (val = s) != 0; }
...
public:
	...
	struct UiInfo
	{
		...
		UiInfo(tukk text, tukk iconName, const CKeyboardShortcut& shortcut)
			: buttonText(text)
			, icon(iconName)
			, key(shortcut)
		{}
	...
	void         SetDescription(tukk description) { m_commandDescription.SetDescription(description); }
...
dll_string CCommand1<LIST(1, P) >::Execute(const CCommand::CArgs& args)
{
	...
	{
		DrxLogAlways("Cannot execute the command %s.%s! One argument required.", GetModule().c_str(), GetName().c_str());
		return "";
	}
	...
	if (ok)
		...
		DrxLogAlways("Cannot execute the command %s.%s(%s)! Invalid argument type.",
...
dll_string CCommand1wRet<LIST(1, P), RT >::Execute(const CCommand::CArgs& args)
{
	...
	if (args.GetArgCount() < 1)
	{
		DrxLogAlways("Cannot execute the command %s.%s! One argument required.", m_module.c_str(), m_name.c_str());
		return "";
	}
...
dll_string CCommand2<LIST(2, P) >::Execute(const CCommand::CArgs& args)
{
	...
	{
		DrxLogAlways("Cannot execute the command %s.%s! Two arguments required.", GetModule().c_str(), GetName().c_str());
		return "";
	}
	...
	if (ok)
		...
		DrxLogAlways("Cannot execute the command %s.%s(%s,%s)! Invalid argument type(s).",
...
dll_string CCommand3<LIST(3, P) >::Execute(const CCommand::CArgs& args)
{
	...
	{
		DrxLogAlways("Cannot execute the command %s.%s! Three arguments required.", GetModule().c_str(), GetName().c_str());
		return "";
	}
	...
	if (ok)
		...
		DrxLogAlways("Cannot execute the command %s.%s(%s,%s,%s)! Invalid argument type(s).",
...
dll_string CCommand4<LIST(4, P) >::Execute(const CCommand::CArgs& args)
{
	...
	{
		DrxLogAlways("Cannot execute the command %s.%s! Four arguments required.", GetModule().c_str(), GetName().c_str());
		return "";
	}
	...
	if (ok)
		...
		DrxLogAlways("Cannot execute the command %s.%s(%s,%s,%s,%s)! Invalid argument type(s).",
		             GetModule(), GetName(), args.GetArg(0).c_str(), args.GetArg(1).c_str(), args.GetArg(2).c_str(),
		             args.GetArg(3).c_str());
	return "";
}
...
dll_string CCommand5<LIST(5, P) >::Execute(const CCommand::CArgs& args)
{
	...
	{
		DrxLogAlways("Cannot execute the command %s.%s! Five arguments required.", GetModule().c_str(), GetName().c_str());
		return "";
	}
	...
	if (ok)
		...
		DrxLogAlways("Cannot execute the command %s.%s(%s,%s,%s,%s,%s)! Invalid argument type(s).",
		             GetModule(), GetName(), args.GetArg(0).c_str(), args.GetArg(1).c_str(), args.GetArg(2).c_str(),
...
//////////////////////////////////////////////////////////////////////////
inline string CommandString(tukk commandString)
{
	return string(commandString);
}

template<LIST(1, typename P)>
inline string CommandString(tukk commandString, P1 p1)
{
	return string(commandString) + " " + CCommand::ToString_(p1);
}

template<LIST(2, typename P)>
inline string CommandString(tukk commandString, P1 p1, P2 p2)
{
	return string(commandString) + " " + CCommand::ToString_(p1) + " " + CCommand::ToString_(p2);
}

template<LIST(3, typename P)>
inline string CommandString(tukk commandString, P1 p1, P2 p2, P3 p3)
{
	return string(commandString) + " " + CCommand::ToString_(p1) + " " + CCommand::ToString_(p2) + " " + CCommand::ToString_(p3);
}

template<LIST(4, typename P)>
inline string CommandString(tukk commandString, P1 p1, P2 p2, P3 p3, P4 p4)
{
	return string(commandString) + " " + CCommand::ToString_(p1) + " " + CCommand::ToString_(p2) + " " + CCommand::ToString_(p3) + " " + CCommand::ToString_(p4);
}

template<LIST(5, typename P)>
inline string CommandString(tukk commandString, P1 p1, P2 p2, P3 p3, P4 p4, P5 p5)
{
