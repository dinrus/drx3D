...
#include <DrxAnimation/DrxCharMorphParams.h>
...
CModelViewport::CModelViewport(tukk settingsPath)
{
	...
	m_LTHUMB = Vec2(0, 0);
	m_RTHUMB = Vec2(0, 0);
	u32 size = sizeof(m_arrLTHUMB) / sizeof(Vec2);
	...
	if (gEnv->pInput)
	{
		...
		u32 test = gEnv->pInput->HasInputDeviceOfType(eIDT_Gamepad);
	}
...
bool CModelViewport::IsPreviewableFileType(tukk szPath)
{
	tukk szExt = PathUtil::GetExt(szPath);

	for (u32 i = 0; i < DRX_ARRAY_COUNT(szSupportedExt); ++i)
	{
...
/*
	...
	i32 vbCount = vb->GetNumVariables();

	regMgr.WriteProfileInt(strSection, _T("iDebugOptionCount"), vbCount);

	char keyType[64], keyValue[64];
	for (i32 i = 0; i < vbCount; ++i)
	{
		IVariable* var = vb->GetVariable(i);
		IVariable::EType vType = var->GetType();
		drx_sprintf(keyType, "DebugOption_%s_type", var->GetName());
		drx_sprintf(keyValue, "DebugOption_%s_value", var->GetName());
		switch (vType)
		{
...
/*
	...
	for (i32 i = 0; i < vbCount; ++i)
	{
		...
		IVariable::EType vType = var->GetType();
		drx_sprintf(keyType, "DebugOption_%s_type", var->GetName());
		i32 iType = regMgr.GetProfileInt(strSection, _T(keyType), 0);
...
void CModelViewport::LoadObject(const string& fileName, float scale)
{
	...
	SetName(string("Model View - ") + (tukk)file);
	...
	{
		// Try Load character.
		u32 isSKEL = stricmp(PathUtil::GetExt(file), DRX_SKEL_FILE_EXT) == 0;
		u32 isSKIN = stricmp(PathUtil::GetExt(file), DRX_SKIN_FILE_EXT) == 0;
		u32 isCGA = stricmp(PathUtil::GetExt(file), DRX_ANIM_GEOMETRY_FILE_EXT) == 0;
		u32 isCDF = stricmp(PathUtil::GetExt(file), DRX_CHARACTER_DEFINITION_FILE_EXT) == 0;
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "!Preview of this file type not supported", "Preview Error", MB_OK | MB_ICONEXCLAMATION);
		return;
	}
...
void CModelViewport::LoadStaticObject(const string& file)
{
	...
	if (!m_object)
	{
		DrxLog("Loading of object failed.");
		return;
	}
	m_object->AddRef();
...
void CModelViewport::OnRender()
{
	...
	if (m_renderer)
	{
		...
		i32 w = rcClient.right - rcClient.left;
		i32 h = rcClient.bottom - rcClient.top;
...
//////////////////////////////////////////////////////////////////////////
bool CModelViewport::OnKeyDown(u32 nChar, u32 nRepCnt, u32 nFlags)
{
	if (GetCharacterBase())
	{
		i32 index = -1;
		...
		if (pAnimations)
		{
			u32 numAnims = pAnimations->GetAnimationCount();
			if (index >= 0 && index < numAnims)
			{
				tukk animName = pAnimations->GetNameByAnimID(index);
...
void CModelViewport::AttachObjectToBone(const string& model, const string& bone)
{
	...
	{
		string str;
		str.Format("Loading of weapon model %s failed.", (tukk)model);
...
void CModelViewport::Physicalize()
{
	...
	if (m_object)
	{
		...
		for (i32 nobj = 0; nobj < m_object->GetSubObjectCount(); nobj++)
		{
	...
	if (pMaterial)
	{
		...
		i32 numIds = pMaterial->GetMatInfo()->FillSurfaceTypeIds(surfaceTypesId);
...
void CModelViewport::DrawModel(const SRenderingPassInfo& passInfo)
{
	...
	if (m_pPhysicalEntity)
	{
		...
		ray_hit hit;
		hit.pCollider = 0;
		i32 flags = rwi_any_hit | rwi_stop_at_pierceable;
		i32 col = gEnv->pPhysicalWorld->RayWorldIntersection(raySrc, rayDir * 1000.0f, ent_static, flags, &hit, 1);
		if (col > 0)
		{
			i32 nMatId = hit.idmatOrg;
	...
	//-----------------------------------------------------------------------------
	//-----             Render Character (handled by DrxAnimation)            ----
	//-----------------------------------------------------------------------------
	if (GetCharacterBase())
...
//////////////////////////////////////////////////////////////////////////
void CModelViewport::PlayAnimation(tukk szName)
{
...
void CModelViewport::DrawFloorGrid(const Quat& m33, const Vec3& vPhysicalLocation, const Matrix33& rGridRot, bool bInstantSubmit)
{
	...
	// Draw grid.
	const size_t nLineVertices = ( ((i32)(XR / step) * 2 + 1) + ((i32)(YR / step) * 2 + 1) ) * 2;
