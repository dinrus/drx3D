...
public:
	...
	void UpdateLoadingPixmap()
	{
		...
		m_rotatedLoading = DrxIcon(rotatedPixmap);
	}
...
private:
	...
	DrxIcon m_loadingIcon;
	DrxIcon m_rotatedLoading;
};
	...
	, m_previewSizeButtons(nullptr)
{
	...
	if (showSizeButtons)
	{
		...
		for (i32 i = 0; i < m_numOfPreviewSizes; i++)
		{
...
void QThumbnailsView::SetSpacing(i32 space)
{
	m_listView->setSpacing(space);
}

i32 QThumbnailsView::GetSpacing() const
{
...
void QThumbnailsView::SetDataColumn(i32 column)
{
	m_listView->setModelColumn(column);
}

bool QThumbnailsView::SetDataAttribute(const CItemModelAttribute* attribute /*= &Attributes::s_thumbnailAttribute*/, i32 attributeRole /*= Attributes::s_getAttributeRole*/)
{
	//TODO : handle model changes after this is set
	DRX_ASSERT(attribute);

	auto pModel = m_listView->model();
	DRX_ASSERT(pModel);//attribute cannot be set before model

	i32k columnCount = pModel->columnCount(GetRootIndex());
...
void QThumbnailsView::ScrollToRow(const QModelIndex& indexInRow, QAbstractItemView::ScrollHint scrollHint)
{
	...
	{
		DRX_ASSERT(indexInRow.model() == m_listView->model());
...
void QThumbnailsView::AppendPreviewSizeActions(CAbstractMenu& menu)
{
	for (i32 i = 0; i < m_numOfPreviewSizes; ++i)
	{
		...
		QAction* pAction = menu.CreateAction(DrxIcon(icon), QString("%1").arg(size));
...
void QThumbnailsView::OnPreviewSizeButtonClicked(i32 value)
{
...
void QThumbnailsView::AddSizingButton(i32 size, i32 iconNumber)
{
