...
void QViewport::CreateGridLine(ColorB col, const float alpha, const float alphaFalloff, const float slide, const float halfSlide, const float maxSlide, const Vec3& stepDir, const Vec3& orthoDir)
{
	...
	u32k cPacked = col.pack_argb8888();
	u32k cEndPacked = colEnd.pack_argb8888();
...
void QViewport::CreateGridLines(const uint count, const uint interStepCount, const Vec3& stepDir, const float stepSize, const Vec3& orthoDir, const float offset)
{
	...
	for (i32 i = 0; i < count + 2; i++)
	{
		...
		for (i32 d = 1; d < interStepCount; d++)
		{
...
void QViewport::DrawGrid()
{
	...
	m_pAuxGeom->DrawLines( &m_gridLineVertices[0], &m_gridLineVerticesColor[0], (u32)m_gridLineVertices.size() );
}
...
void QViewport::DrawOrigin(i32k left, i32k top, const float scale, const Matrix34 cameraTM)
{
	IRenderAuxGeom* aux = m_pAuxGeom;
...
QViewport::QViewport(SSysGlobEnv* env, QWidget* parent, i32 supersamplingFactor)
	: QWidget(parent)
...
bool QViewport::ScreenToWorldRay(Ray* ray, i32 x, i32 y)
{
	if (!m_env->pRenderer)
		return false;
...
QPoint QViewport::ProjectToScreen(const Vec3& wp)
{
	...
	{
		return QPoint((i32)out.x,(i32)out.y);
	}
...
i32 QViewport::Width() const
{
	return QtUtil::PixelScale(this, rect().width());
}

i32 QViewport::Height() const
{
...
void QViewport::IEditorEventFromQViewportEvent(const SMouseEvent& qEvt, CPoint& p, EMouseEvent& evt, i32& flags)
{
	p.x = qEvt.x;
	p.y = qEvt.y;
	flags = 0;
