...
} //endns PrivateDragDrop

QString CDragDropData::GetMimeFormatForType(tukk type)
{
...
const CDragDropData* CDragDropData::FromMimeDataSafe(const QMimeData* mimeData)
{
	...
	DRX_ASSERT_MESSAGE(result, "The QMimeData is not of type CDragDropData");
	return result;
}

QByteArray CDragDropData::GetCustomData(tukk type) const
{
...
void CDragDropData::SetCustomData(tukk type, const QByteArray& data)
{
	QMimeData::setData(GetMimeFormatForType(type), data);
}

bool CDragDropData::HasCustomData(tukk type) const
{
