...
#include <drx3D/CoreX/Math/Bezier_impl.h>
...
#include <DrxPhysics/polynomial.h>
...
{
i32k kDefaultRulerHeight = 16;
i32k kRulerShadowHeight = 6;
i32k kRulerMarkHeight = 8;
...
struct CCurveEditor::SPanHandler : public CCurveEditor::SMouseHandler
{
	...
	void mousePressEvent(QMouseEvent* pEvent) override
	{
		m_startPoint = QPoint(i32(pEvent->x()), i32(pEvent->y()));
	...
	void mouseMoveEvent(QMouseEvent* pEvent) override
	{
		...
		i32k pixelDeltaX = pEvent->x() - m_startPoint.x();
		i32k pixelDeltaY = pEvent->y() - m_startPoint.y();
...
struct CCurveEditor::SZoomHandler : public CCurveEditor::SMouseHandler
{
	...
	void mousePressEvent(QMouseEvent* pEvent) override
	{
		m_lastPoint = QPoint(i32(pEvent->x()), i32(pEvent->y()));
	...
	void mouseMoveEvent(QMouseEvent* pEvent) override
	{
		...
		i32k pixelDeltaX = pEvent->x() - m_lastPoint.x();
		i32k pixelDeltaY = -(pEvent->y() - m_lastPoint.y());
		m_lastPoint = QPoint(i32(pEvent->x()), i32(pEvent->y()));
...
void CCurveEditor::ZoomToTimeRange(const float start, const float end)
{
	...
	if (i32 width = GetCurveArea().width())
	{
...
void CCurveEditor::ZoomToValueRange(const float min, const float max)
{
	...
	if (i32 height = GetCurveArea().height())
	{
		// Adjust zoom and translation depending on m_fitMargin
...
void CCurveEditor::DrawGrid(QPainter& painter)
{
	...
	i32k height = curveRect.height();
	i32k width = curveRect.width();

	i32 verticalRulerPrecision;
	...
	char format[16];
	drx_sprintf(format, "%%.%df", verticalRulerPrecision);
	...
	for (const STick& tick : verticalTicks)
	{
		if (!tick.m_bTenth)
		{
			i32k y = height - tick.m_position + curveRect.top();
...
void CCurveEditor::SetRulerHeight(i32 height)
{
	m_rulerHeight = height;
	update();
}

void CCurveEditor::SetRulerTicksYOffset(i32 offset)
{
...
void CCurveEditor::SelectInRect(const QRect& rect, bool bToggleSelected, bool bDeselect)
{
	...
	ForEachKey(*m_pContent, [&](SCurveEditorCurve& curve, SCurveEditorKey& key)
	{
		...
		bool bKeyFoundInRect = rect.contains((i32)screenPoint.x, (i32)screenPoint.y);
...
{
	for (i32 i = 0; i < m_pContent->m_curves.size(); ++i)
...
CCurveEditor::TCurveReferences CCurveEditor::GetPrioritizedCurves(bool backwards)
{
	...
	{
		i32 s = (m_curveFocusIndex + (backwards ? count - 1 - i : i)) % count;
...
void CCurveEditor::OnFitCurvesVertically()
{
	...
	if (m_pContent)
	{
		...
		const float pivot = (0.5f - m_translation.y) / m_zoom.y;
		i32k height = GetCurveArea().height();
		if (height > 0)
		{
			m_zoom.y /= 1.0f + 2.0f * (m_fitMargin / height);
		}
...
{
	pToolBar->addAction(DrxIcon("icons:CurveEditor/auto.ico"), "Set in and out tangents to auto", this, SLOT(OnSetSelectedKeysTangentAuto()));
	pToolBar->addSeparator();
	pToolBar->addAction(DrxIcon("icons:CurveEditor/zero_in.ico"), "Set in tangent to zero", this, SLOT(OnSetSelectedKeysInTangentZero()));
	if (m_bAllowDiscontinuous)
		pToolBar->addAction(DrxIcon("icons:CurveEditor/step_in.ico"), "Set in tangent to step", this, SLOT(OnSetSelectedKeysInTangentStep()));
	pToolBar->addAction(DrxIcon("icons:CurveEditor/linear_in.ico"), "Set in tangent to linear", this, SLOT(OnSetSelectedKeysInTangentLinear()));
	pToolBar->addSeparator();
	pToolBar->addAction(DrxIcon("icons:CurveEditor/zero_out.ico"), "Set out tangent to zero", this, SLOT(OnSetSelectedKeysOutTangentZero()));
	if (m_bAllowDiscontinuous)
		pToolBar->addAction(DrxIcon("icons:CurveEditor/step_out.ico"), "Set out tangent to step", this, SLOT(OnSetSelectedKeysOutTangentStep()));
	pToolBar->addAction(DrxIcon("icons:CurveEditor/linear_out.ico"), "Set out tangent to linear", this, SLOT(OnSetSelectedKeysOutTangentLinear()));
	pToolBar->addSeparator();
	pToolBar->addAction(DrxIcon("icons:CurveEditor/fit_horizontal.ico"), "Fit curves horizontally", this, SLOT(OnFitCurvesHorizontally()));
	pToolBar->addAction(DrxIcon("icons:CurveEditor/fit_vertical.ico"), "Fit curves vertically", this, SLOT(OnFitCurvesVertically()));
	pToolBar->addSeparator();
	pToolBar->addAction(DrxIcon("icons:CurveEditor/break.ico"), "Break tangents", this, SLOT(OnBreakTangents()));
	pToolBar->addAction(DrxIcon("icons:CurveEditor/unify.ico"), "Unify tangents", this, SLOT(OnUnifyTangents()));
}
