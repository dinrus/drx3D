...
i32 STimelineViewState::ScrollOffset(float origin) const
{
	return i32((origin / visibleDistance + 0.5f) * widthPixels);
}

i32 STimelineViewState::TimeToLayout(float time) const
{
	return i32((time / visibleDistance) * widthPixels + 0.5f);
}

i32 STimelineViewState::TimeToLocal(float time) const
{
	return TimeToLayout(time) + treeWidth + scrollPixels.x();
}

float STimelineViewState::LayoutToTime(i32 x) const
{
	return (float(x) - 0.5f) / widthPixels * visibleDistance;
}

float STimelineViewState::LocalToTime(i32 x) const
{
...
SElementLayout& AddElementToTrackLayout(STimelineTrack& track, STrackLayout& trackLayout, const STimelineElement& element,
                                        const STimelineViewState& viewState, uint keySize, i32 treeWidth, i32& currentTop, size_t elementIndex)
{
	...
	{
		i32k left = (viewState.TimeToLayout(element.start.ToFloat()) - keySize / 2);
		i32k top = currentTop + ((track.height - keySize) / 2);
		elementl.rect = QRect(left, top, keySize, keySize);
	}
	else
	{
		i32k left = viewState.TimeToLayout(element.start.ToFloat());
		i32k right = viewState.TimeToLayout(element.end.ToFloat());
...
void AddCompoundElementsToTrackLayout(STimelineTrack& track, STimelineLayout* layout, const STimelineViewState& viewState, i32 trackId, uint keySize, i32 treeWidth, i32& currentTop)
{
...
bool FilterTracks(const STimelineTrack& track, std::unordered_set<const STimelineTrack*>& invisibleTracks, tukk filterString)
{
	bool bAnyChildVisible = false;
	const bool bNameMatchesFilter = DrxStringUtils::stristr(track.name, filterString) != nullptr;
...
void CalculateTrackLayout(STimelineLayout* layout, i32& currentTop, i32 currentIndent, STimelineTrack& parentTrack, const STimelineViewState& viewState,
                          float thumbTime, uint keySize, i32 treeWidth, const std::unordered_set<const STimelineTrack*>& invisibleTracks)
{
	const size_t numTracks = parentTrack.tracks.size();
	for (size_t i = 0; i < numTracks; ++i)
	{
		...
		{
			i32k trackLayoutId = layout->tracks.size() - 1;
		...
		i32 left = viewState.TimeToLayout(track.startTime.ToFloat());
		i32 right = viewState.TimeToLayout(track.endTime.ToFloat());
...
void ApplyPushOut(STimelineLayout* layout, uint keySize)
{
	...
	for (size_t i = 0; i < numLayoutTracks; ++i)
	{
		...
		{
			track.elements[j].rect.translate(QPoint(0, i32(pushOutScale * track.elements[j].pushOutDistance * PUSH_OUT_DISTANCE)));
		}
	}
}
...
void UpdateScrollPixels(STimelineLayout* layout, STimelineViewState& viewState)
{
	if (viewState.visibleDistance != 0)
	{
		...
		viewState.maxScrollX = i32(viewState.widthPixels * totalDuration.ToFloat() / viewState.visibleDistance) - viewState.widthPixels;
	}
	else
	{
...
void CalculateLayout(STimelineLayout* layout, STimelineContent& content, STimelineViewState& viewState, const QLineEdit* pFilterLineEdit, float thumbTime, uint keySize, bool treeVisible, bool mainTrackTimeRange)
{
	...
	i32 currentTop = RULER_HEIGHT + VERTICAL_PADDING;
	i32k treeWidth = treeVisible ? viewState.treeWidth : 0;
...
{
	auto findIter = std::upper_bound(tracks.begin(), tracks.end(), point.y(), [&](i32 y, const STrackLayout& track)
		{
...
void UpdateTruncatedDurations(STimelineTrack* track)
{
	...
	std::vector<i32> sortedTrackIndices(numElements);
	for (i32 i = 0; i < numElements; ++i)
		sortedTrackIndices[i] = i;
	std::sort(sortedTrackIndices.begin(), sortedTrackIndices.end(), [&](i32 a, i32 b) { return track->elements[a].start < track->elements[b].start; });
...
void UpdateClipLinesCache(STracksRenderCache& renderCache, const SElementLayout& element, const QRect& rect, const STimelineViewState& viewState)
{
	...
	i32 len = viewState.TimeToLayout(duration);
...
void UpdateTracksRenderCache(STracksRenderCache& renderCache, QPainter& painter, const STimelineLayout& layout, const STimelineViewState& viewState, const QRect& trackRect, float timeUnitScale, bool drawMarkers, bool bShowText, i32 scrollOffset)
{
	LOADING_TIME_PROFILE_SECTION
	...
	i32k trackAreaLeft = viewState.LocalToLayout(QPoint(viewState.treeWidth, 0)).x();
	i32k trackAreaRight = trackAreaLeft + trackRect.width();
	...
	i32k highMarkersMod = TRACK_MARK_HEIGHT;
	i32k lowMarkersMod = highMarkersMod / 2;
	...
	{
		const STrackLayout& track = tracks[j];
		u32k numElements = track.elements.size();
	...
	for (i32 i = PASS_BACKGROUND; i <= PASS_MAIN; ++i)
	{
			...
			case PASS_BACKGROUND:
				{
					...
					i32k lineY = track.rect.bottom() + 1;
					...
					if (drawMarkers && (track.pTimelineTrack->caps & STimelineTrack::CAP_DESCRIPTION_TRACK) == 0)
					{
						...
						i32k height = rect.height();
						i32k top = rect.top();

						i32k lowY = top + height - lowMarkersMod;
						i32k highY = top + height - highMarkersMod;

						if (renderCache.pTimeMarkers)
						{
							for (const DrawingPrimitives::CRuler::STick& tick : * renderCache.pTimeMarkers)
							{
								i32k x = tick.position + rect.left();
						...
						else
						{
							...
							{
								i32k x = tick.m_position + rect.left();
					...
					if ((track.pTimelineTrack->caps & STimelineTrack::CAP_TOGGLE_TRACK) != 0)
					{
						...
						u32k drawStart = track.pTimelineTrack->toggleDefaultState ? 0 : 1;
						for (u32 i = drawStart; i <= numElements; i += 2)
						{
			...
			default:
				{
					for (auto itr = sortedElements.begin(); itr != sortedElements.end(); ++itr)
					{
						...
						case STimelineElement::KEY:
							{
								DrxIconColorMap cryIconColorMap;
								cryIconColorMap[QIcon::Normal] = QColor(element.color.r, element.color.g, element.color.b, element.color.a);

								if (element.IsHighlighted())
								{
									QPixmap pixmap = DrxIcon("icons:Trackview/Trackview_Key_Highlight.ico", std::move(cryIconColorMap)).pixmap(rect.width(), rect.height());
									renderCache.highlightedKeyIcons.emplace_back(QRenderIcon(rect, std::move(pixmap)));
								}
								else if (element.IsSelected())
								{
									QPixmap pixmap = DrxIcon("icons:Trackview/Trackview_Key_Active.ico", std::move(cryIconColorMap)).pixmap(rect.width(), rect.height());
									renderCache.selectedKeyIcons.emplace_back(QRenderIcon(rect, std::move(pixmap)));
								}
								else
								{
									QPixmap pixmap = DrxIcon("icons:Trackview/Trackview_Key_normal.ico", std::move(cryIconColorMap)).pixmap(rect.width(), rect.height());
...
void DrawSelectionLines(QPainter& painter, const STimelineViewState& viewState, const Range& visibleRange, STimelineContent& content, i32 rulerPrecision, i32 width, i32 height, float time, float timeUnitScale, bool hasFocus)
{
	LOADING_TIME_PROFILE_SECTION
	...
	for (auto iter = times.begin(); iter != times.end(); ++iter)
	{
		...
		{
			i32k indicatorX = i32(viewState.TimeToLocal(iter->ToFloat()) + 0.5f);
			painter.drawLine(QPoint(indicatorX, 0), QPoint(indicatorX, height));
		}
	}
}

void UpdateTreeRenderCache(STreeRenderCache& renderCache, const QRect& treeRect, QWidget* timeline, const STimelineContent& content, const STrackLayouts& tracks, const STimelineViewState& viewState, i32 scrollOffset)
{
	LOADING_TIME_PROFILE_SECTION
	...
	for (size_t i = 0; i < tracks.size(); ++i)
	{
		...
		i32k branchLeft = TREE_LEFT_MARGIN + track.indent * TREE_INDENT_MULTIPLIER;
		...
		i32k iconLeft = branchLeft + TREE_BRANCH_INDICATOR_SIZE + 4;
		i32k textLeft = track.pTimelineTrack->HasIcon() ? (iconLeft + track.rect.height() + 2) : iconLeft;
		i32k textWidth = std::max(treeRect.width() - textLeft - 4, 0);
		...
		if (track.pTimelineTrack->HasIcon())
		{
			...
			if (track.pTimelineTrack->selected)
			{
				...
				renderCache.icons.emplace_back(QRenderIcon(iconRect, DrxIcon(QString(track.pTimelineTrack->icon), cryIconColorMap).pixmap(iconRect.width(), iconRect.height())));
			}
			else
				renderCache.icons.emplace_back(QRenderIcon(iconRect, DrxIcon(QString(track.pTimelineTrack->icon)).pixmap(iconRect.width(), iconRect.height())));
		}
...
struct CTimeline::SSelectionHandler : SMouseHandler
{
	...
	{
		i32k scroll = m_timeline->m_scrollBar ? m_timeline->m_scrollBar->value() : 0;
...
struct CTimeline::SMoveHandler : SMouseHandler
{
	...
	void mouseMoveEvent(QMouseEvent* ev) override
	{
		...
		i32k scroll = m_timeline->m_scrollBar ? m_timeline->m_scrollBar->value() : 0;
		const QPoint currentPos(ev->pos().x(), ev->pos().y() + scroll);
		i32 delta = m_timeline->m_viewState.LocalToLayout(currentPos).x() - m_startPoint.x();
...
struct CTimeline::SPanHandler : SMouseHandler
{
	...
	void mousePressEvent(QMouseEvent* ev) override
	{
		m_startPoint = QPoint(i32(ev->x()), i32(ev->y()));
	...
	void mouseMoveEvent(QMouseEvent* ev) override
	{
		const QPoint pos(i32(ev->x()), i32(ev->y()));
		...
		{
			m_timeline->m_scrollBar->setVal(m_startScroll - (i32)posDif.y());
		}
...
struct CTimeline::SScrubHandler : SMouseHandler
{
	...
	void SetThumbPositionX(i32 positionX)
	{
		i32 nThumbEndPadding = (THUMB_WIDTH / 2) + 2;
	...
	void mousePressEvent(QMouseEvent* ev) override
	{
		...
		i32 thumbPositionX = m_timeline->m_viewState.TimeToLayout(m_timeline->m_time.ToFloat());
...
struct CTimeline::SSplitterHandler : SMouseHandler
{
	...
	void mouseMoveEvent(QMouseEvent* ev) override
	{
		...
		uint treeWidth = clamp_tpl<i32>(ev->pos().x(), SPLITTER_WIDTH, m_timeline->width()) + m_offset;
...
struct CTimeline::STreeMouseHandler : SMouseHandler
{
	...
	void mousePressEvent(QMouseEvent* ev) override
	{
		...
		{
			i32k left = TREE_LEFT_MARGIN + pTrackLayout->indent * TREE_INDENT_MULTIPLIER;
			i32k right = left + TREE_BRANCH_INDICATOR_SIZE;
...
void CTimeline::paintEvent(QPaintEvent* ev)
{
	...
	{
		i32k tracksWidth = width() - m_viewState.treeWidth - SPLITTER_WIDTH;
	...
	for (auto& element : m_headerElements)
	{
		if (element.visible && visibleRange.IsInside(element.time.ToFloat()))
		{
			...
			i32 timePx = m_viewState.TimeToLocal(element.time.ToFloat());
			...
			{
				i32k vmid = rect.top() + rect.height() / 2;
	...
	if (m_treeVisible)
	{
		if (m_pContent)
		{
			...
			i32 searchHeight = m_searchWidget ? m_searchWidget->height() + 1 : 0;
...
void CTimeline::UpdateTracksTimeMarkers(i32 width)
{
...
bool CTimeline::HandleKeyEvent(i32 k)
{
	QKeySequence key(k);
...
void CTimeline::mousePressEvent(QMouseEvent* ev)
{
	...
	i32k scroll = m_scrollBar ? m_scrollBar->value() : 0;
...
void CTimeline::wheelEvent(QWheelEvent* ev)
{
	...
	{
		i32 pixelDelta = ev->pixelDelta().manhattanLength();
...
void CTimeline::ZoomStep(i32 steps)
{
...
void CTimeline::focusTrack(STimelineTrack* pTrack)
{
	...
	if (m_scrollBar)
	{
		for (STrackLayout& layout : m_layout->tracks)
		{
			...
			{
				m_scrollBar->setVal(layout.rect.top() - (RULER_HEIGHT + VERTICAL_PADDING));
				break;
			}
		}
	}
...
void CTimeline::UpdateLayout(bool forceClamp)
{
	...
	if (m_verticalScrollbarVisible)
	{
		if (!m_scrollBar)
		{
			...
			connect(m_scrollBar, SIGNAL(valueChanged(i32)), this, SLOT(OnVerticalScroll(i32)));
		}
	...
	if (m_scrollBar)
	{
		i32k timelineTrackCount = m_layout->tracks.size();
		i32k timelineHeight = rect().height();
		i32k scrollBarRange = m_layout->size.height() - timelineHeight;
		i32k singleStep = scrollBarRange / (timelineTrackCount > 0 ? timelineTrackCount : 1);
	...
	if (m_treeVisible)
	{
		i32 searchAreaWidth = m_viewState.treeWidth - SPLITTER_WIDTH - m_cornerWidgetWidth;
		i32 searchAreaHeight = RULER_HEIGHT + VERTICAL_PADDING;
...
void CTimeline::OnVerticalScroll(i32 value)
{
	InvalidateTree();
	InvalidateTracks();
	update();
...
STrackLayout* CTimeline::GetTrackLayoutFromPos(const QPoint& pos) const
{
	...
	auto findIter = std::upper_bound(tracks.begin(), tracks.end(), pos.y(), [&](i32 y, const STrackLayout& track)
	{
		return y < track.rect.bottom();
	});
...
void CTimeline::OnSplitterMoved(u32 newTreeWidth)
{
