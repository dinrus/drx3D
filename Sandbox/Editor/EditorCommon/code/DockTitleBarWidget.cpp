...
QSize CDockWidgetTitleButton::sizeHint() const
{
	...
	i32 size = 2 * style()->pixelMetric(QStyle::PM_DockWidgetTitleBarButtonMargin, 0, this);
	if (!icon().isNull())
	{
		i32 iconSize = style()->pixelMetric(QStyle::PM_SmallIconSize, 0, this);
...
void CDockWidgetTitleButton::paintEvent(QPaintEvent* ev)
{
	...
	i32 size = style()->pixelMetric(QStyle::PM_SmallIconSize, 0, this);
...
QAbstractButton* CDockTitleBarWidget::AddButton(const QIcon& icon, tukk tooltip)
{
...
CCustomDockWidget::CCustomDockWidget(tukk title, QWidget* parent, Qt::WindowFlags flags)
	: QDockWidget(title, parent, flags)
{
	...
	{
		const auto closeButton = titleBar->AddButton(DrxIcon("icons:Window/Window_Close.ico"), "Close");
	...
	{
		const auto floatButton = titleBar->AddButton(DrxIcon("icons:Window/Window_Maximize.ico"), "Toggle Floating");
