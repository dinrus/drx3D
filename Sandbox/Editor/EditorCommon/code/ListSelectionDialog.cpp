...
void ListSelectionDialog::SetColumnText(i32 column, tukk text)
{
	if (column >= m_model->columnCount())
	{
		i32 oldColumnCount = m_model->columnCount();
		m_model->setColumnCount(column + 1);
		for (i32 i = oldColumnCount; i <= column; ++i)
		{
#if QT_VERSION >= 0x50000
...
void ListSelectionDialog::SetColumnWidth(i32 column, i32 width)
{
	if (column >= m_model->columnCount())
		return;
...
void ListSelectionDialog::AddRow(tukk name)
{
	AddRow(name, DrxIcon());
}

void ListSelectionDialog::AddRow(tukk name, QIcon& icon)
{
...
void ListSelectionDialog::AddRowColumn(tukk text)
{
	i32 itemCount = m_model->rowCount(QModelIndex());
	if (itemCount == 0)
		return;
...
tukk ListSelectionDialog::ChooseItem(tukk currentItem)
{
