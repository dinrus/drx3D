...
bool IsModifierKeyDown(i32 modifier)
{
...
EDITOR_COMMON_API void OpenInExplorer(tukk path)
{
	...
	if (!args.isEmpty())
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Unable to open path: %s", path);
#else
...
EDITOR_COMMON_API void OpenFileForEdit(tukk filePath)
{
	...
	if(info.exists())
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "File does not exist: %s", filePath);
#else
	#error //Need platform specific implementation
#endif
}
...
EDITOR_COMMON_API QAction* AddActionFromPath(const QString& menuPath, QMenu* parentMenu)
{
	...
	for (i32 i = 0; i < splitPath.size() - 1; i++)
	{
		bool bFound = false;
...
namespace Private_QtUtil
{
	...
	{
		DRX_ASSERT(pProxyModel != pSourceModel);
		...
		{
			DRX_ASSERT(!nextChildStack.empty());
			...
			{
				DRX_ASSERT(proxy.type == SProxyVariant::Type::Merging);

				i32& nextItem = nextChildStack.back();
