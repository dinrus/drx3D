...
void QPrecisionSlider::mousePressEvent(QMouseEvent* evt)
{
	...
	{
		setVal(minimum() + maximum() * evt->x() / width());
		evt->accept();
	}
...
QMenuLabelSeparator::QMenuLabelSeparator(tukk text /*= ""*/)
...
QLoading::QLoading(QWidget* parent /*= nullptr*/)
	...
	, m_delta(16)
{
	m_loadingImage = DrxIcon("icons:Dialogs/dialog_loading.ico").pixmap(size() /*, QIcon::Normal, QIcon::On*/);
	m_doneImage = DrxIcon("icons:General/Tick.ico").pixmap(size() /*, QIcon::Normal, QIcon::On*/);
...
void QLoading::paintEvent(QPaintEvent* pEvent)
{
	...
	i32 pixMapSize = width() > height() ? height() : width();
