...
private:
	virtual i32 rowCount(const QModelIndex &parent = QModelIndex()) const override { return m_filtersName.count(); }
	...
	virtual bool setData(const QModelIndex &index, const QVariant &value, i32 role = Qt::EditRole)
	{
		if(index.isValid())
		{
	...
	: m_filterPanel(parent)
{
	...
	saveButton->setIcon(DrxIcon("icons:General/Save_as.ico"));
	...
	loadButton->setIcon(DrxIcon("icons:General/Save_To_Disc.ico"));
	...
	deleteButton->setIcon(DrxIcon("icons:General/Close.ico"));
...
public:
	...
	void        Remove();
	void        OnSelectAttribute(i32 index);
...
QFilteringPanel::CFilterWidget::CFilterWidget(QFilteringPanel* parent)
	...
	, m_pParent(parent)
{
	DRX_ASSERT(parent->GetAttributes().size() > 0);
	...
	m_pOperatorsComboBox->signalCurrentIndexChanged.Connect([&](i32 index)
	{
	...
	pRemoveButton->setIcon(DrxIcon("icons:General/Element_Remove.ico"));
	...
	m_pInvertButton->setIcon(DrxIcon("icons:General/Invert.ico"));
...
void QFilteringPanel::CFilterWidget::OnSelectAttribute(i32 index)
{
...
QFilteringPanel::QFilteringPanel(tukk uniqueName, QAttributeFilterProxyModel* pModel, QWidget* pParent)
	: QWidget(pParent)
	, m_pModel(nullptr)
	, m_timerMsec(0)
	, m_timer(nullptr)
	...
	, m_savedFiltersWidget(nullptr)
{
	...
	m_addFilterButton->setIcon(DrxIcon("icons:General/Element_Add.ico"));
	...
	m_saveLoadButton->setIcon(DrxIcon("icons:General/Save_as.ico"));
	...
	m_optionsButton->setIcon(DrxIcon("icons:General/Filter.ico"));
...
void QFilteringPanel::FillMenu(CAbstractMenu* pMenu, QString& submenuName)
{
	...
	i32 section = pMenu->IsEmpty() ? CAbstractMenu::eSections_Default : pMenu->GetNextEmptySection();
...
void QFilteringPanel::SaveFilter(tukk filterName)
{
	auto savedFiltersVariant = GetIEditor()->GetPersonalizationManager()->GetProjectProperty("QFilteringPanel", m_uniqueName);
...
void QFilteringPanel::RenameFilter(tukk filterName, tukk filterNewName)
{
	auto savedFiltersVariant = GetIEditor()->GetPersonalizationManager()->GetProjectProperty("QFilteringPanel", m_uniqueName);
...
void QFilteringPanel::DeleteFilter(tukk filterName)
{
	auto savedFiltersVariant = GetIEditor()->GetPersonalizationManager()->GetProjectProperty("QFilteringPanel", m_uniqueName);
