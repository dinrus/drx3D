	...
	, m_displayClearButton(false)
{
	...
	m_action = addAction(DrxIcon(QPixmap("icons:General/Search.ico")), QLineEdit::TrailingPosition);//TODO : make a real delete icon !!
...
void QSearchBox::OnTextChanged()
{
	...
	{
		m_action->setIcon(DrxIcon(QPixmap("icons:General/Search.ico")));
		m_displayClearButton = false;
	}
	else if (!text().isEmpty() && !m_displayClearButton)
	{
		m_action->setIcon(DrxIcon(QPixmap("icons:General/Close.ico")));
