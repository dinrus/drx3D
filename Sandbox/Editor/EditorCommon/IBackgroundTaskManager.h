...
struct IBackgroundTaskManagerListener
{
	...
	virtual void OnBackgroundTaskAdded(tukk description) = 0;
	virtual void OnBackgroundTaskCompleted(ETaskResult taskResult, tukk description) = 0;
...
struct IBackgroundTaskManager
{
	...
	virtual void ScheduleTask(IBackgroundTask* pTask, ETaskPriority priority, i32 delayMilliseconds, ETaskThreadMask threadMask) = 0;

	virtual void AddListener(IBackgroundTaskManagerListener* pListener, tukk name) = 0;
