...
void fillRoundRectangle(QPainter& p, const QBrush& brush, const QRect& _r, const QColor& border, i32 radius)
{
...
void QDrawContext::drawColor(const Rect& rect, const Color& treeColor)
{
	...
	{
		i32 size = 12;		
		static vector<i32> pixels(size * size);
		for (i32 i = 0; i < pixels.size(); ++i)
			pixels[i] = ((i / size) / (size / 2) + (i % size) / (size / 2)) % 2 ? 0xffffffff : 0x000000ff;
		checkboardPattern = QImage((u8*)pixels.data(), size, size, size * 4, QImage::Format_RGBA8888);
	}
...
void QDrawContext::drawComboBox(const Rect& rect, tukk text, bool disabled)
{
...
void QDrawContext::drawGroupBox(const Rect& rect, i32 headerHeight)
{
...
void QDrawContext::drawLabel(tukk text, Font font, const Rect& rect, bool selected, bool disabled)
{
...
void QDrawContext::drawNumberEntry(tukk text, const Rect& rect, i32 flags, double minValue, double maxValue)
{
...
void QDrawContext::drawIcon(const Rect& rect, const Icon& icon, IconEffect effect)
{
	...
	if (!image)
		return;
	i32 x = rect.left() + (rect.width() - image->width()) / 2;
	i32 y = rect.top() + (rect.height() - image->height()) / 2;
	// TODO effect
...
void QDrawContext::drawControlButton(const Rect& rect, tukk text, i32 buttonFlags, property_tree::Font font_name, const Color* colorOverride)
{
	drawButton(rect, text, buttonFlags, font_name, colorOverride);
}

void QDrawContext::drawButton(const Rect& rect, tukk text, i32 buttonFlags, property_tree::Font font_name, const Color* colorOverride)
{
	...
	if (colorOverride) {
		...
		for (i32 i = 0; i < sizeof(groups) / sizeof(groups[0]); ++i)
...
void QDrawContext::drawButtonWithIcon(const Icon& icon, const Rect& rect, tukk text, i32 buttonFlags, property_tree::Font font)
{
	bool pressed = (buttonFlags & BUTTON_PRESSED) != 0;
	bool focused = (buttonFlags & BUTTON_FOCUSED) != 0;
	bool center = (buttonFlags & BUTTON_CENTER_TEXT) != 0;
...
void QDrawContext::drawValueText(bool highlighted, tukk text)
{
	QColor textColor;
...
void QDrawContext::drawEntry(const Rect& rect, tukk text, bool pathEllipsis, bool grayBackground, i32 trailingOffset)
{
...
void QDrawContext::drawValidators(PropertyRow* row, const Rect& totalRect)
{
	...
	i32k padding = tree->_defaultRowHeight() * 0.1f;
	i32 offset = padding;
	auto drawFunc = [&](PropertyRow* row) {
		...
		if (const ValidatorEntry* validatorEntries = tree->_validatorBlock()->getEntry(row->validatorIndex(), row->validatorCount())) {
			for (i32 i = 0; i < row->validatorCount(); ++i) {
				...
				tukk text = validatorEntry->message.c_str();
				...
				i32 textHeight = max(tree->_defaultRowHeight(),
