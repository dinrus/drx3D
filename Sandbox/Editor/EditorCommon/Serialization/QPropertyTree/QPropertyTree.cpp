...
static i32 translateKey(i32 qtKey)
{
	switch (qtKey) {
...
static i32 translateModifiers(i32 qtModifiers)
{
	i32 result = 0;
...
void DragWindow::move(i32 deltaX, i32 deltaY)
{
...
struct DrawVisitor
{
	DrawVisitor(QPainter& painter, const QRect& area, i32 scrollOffset, bool selectionPass)
		: area_(area)
		, painter_(painter)
		, offset_(0)
...
void DragWindow::drawRow(QPainter& p)
{
	...
	i32 offsetX = -leftTop.x() - tree_->config().tabSize + 3;
	i32 offsetY = -leftTop.y() + 3;
...
, dragDropMimeData_(0)
{
	...
	connect(scrollBar_, SIGNAL(valueChanged(i32)), this, SLOT(onScroll(i32)));
...
void QPropertyTree::updateHeights(bool recalculateTextSize)
{
	...
	defaultRowHeight_ = max(16, i32(fm.lineSpacing() * editsScale_)); // to fit at least 16x16 icons
	...
	i32 scrollBarW = scrollbarVisible ? scrollBar_->width() : 0;
	area_ = fromQRect(widgetRect.adjusted(2, 2, -2 - scrollBarW, -2));

	i32 filterAreaHeight = 0;
	if (filterMode_)
	{
	...
	i32 lb = style_->compact ? 0 : 4;
	i32 rb = widgetRect.right() - lb - scrollBarW - 2;
	i32 availableWidth = widgetRect.width() - 4 - scrollBarW;
	...
	i32	contentHeight = totalHeight + filterAreaHeight + 4;
	if (sizeToContent_) {
...
{
	i32 pageSize = rect().height() - (filterMode_ ? filterEntry_->height() + 2 : 0) - 4;
...
void QPropertyTree::onScroll(i32 pos)
{
...
bool QPropertyTree::canBePasted(tukk destinationType)
{
	SharedPtr<PropertyRow> source;
	if (!propertyRowFromClipboard(source, model()->constStrings()))
		return false;
...
void QPropertyTree::startFilter(tukk filter)
{
	setFilterMode(true);
...
struct FilterVisitor
{
	...
	ScanResult operator()(PropertyRow* row, PropertyTree* _tree)
	{
		...
		tukk label = row->labelUndecorated();
...
void QPropertyTree::onFilterChanged(const QString& text)
{
	...
	tukk filterStr = filterMode_ ? arr.data() : "";
...
void QPropertyTree::drawFilteredString(QPainter& p, tukk text, RowFilter::Type type, const QFont* font, const QRect& rect, const QColor& textColor, bool pathEllipsis, bool center) const
{
	...
	if (filterMode_) {
		...
		if (matched && hiStart != hiEnd) {
			...
			QColor highlightColor, highlightBorderColor;
			{
				...
				if (h < 0)
					h += 360;
				highlightColor.setHsv(h, min(255, i32(s * 1.33f)), v, 255);
				highlightBorderColor.setHsv(h, s * 0.5f, v, 255);
			}

			i32 left = i32(boxFull.left() + boxStart.width()) - 1;
			i32 top = i32(boxFull.top());
			i32 right = i32(boxFull.left() + boxEnd.width());
			i32 bottom = i32(boxFull.top() + boxEnd.height());
...
void QPropertyTree::_drawRowValue(QPainter& p, tukk text, const QFont* font, const QRect& rect, const QColor& textColor, bool pathEllipsis, bool center) const
{
...
void QPropertyTree::paintEvent(QPaintEvent* ev)
{
	...
	i32 clientWidth = clientRect.width();
	i32 clientHeight = clientRect.height();
...
void QPropertyTree::resizeEvent(QResizeEvent* ev)
{
	...
	if (treeStyle().propertySplitter) {
		i32k splitterBorder = 2 * defaultRowHeight_;
		i32 propertySplitterPos = propertySplitterPos_;
		...
		if (ev->oldSize().width() < 0 && ev->size().width() > 0) {
			...
			propertySplitterPos = i32(float(propertySplitterPos) * scale);
		}
...
void QPropertyTree::keyPressEvent(QKeyEvent* ev)
{
	...
	if (ev->key() == Qt::Key_Up){
		i32 y = model()->root()->verticalIndex(this, model()->focusedRow());
		if (filterMode_ && y == 0) {
			setFilterMode(true);
			update();
			return;
...
void QPropertyTree::wheelEvent(QWheelEvent* ev) 
{
	...
	if (ev->modifiers() & Qt::CTRL)	{
		...
		if (scrollBar_->isVisible() && scrollBar_->isEnabled())
			scrollBar_->setVal(scrollBar_->value() + -ev->delta());
	}
}
