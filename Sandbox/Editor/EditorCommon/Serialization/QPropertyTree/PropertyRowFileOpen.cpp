...
static tukk getExtension(tukk path)
{
	tukk ext = strrchr(path, '.');
	if (!ext)
		return path + strlen(path);
	return ext;
}

static yasli::string removeExtension(tukk filename)
{
	tukk ext = getExtension(filename);
	return yasli::string(filename, ext);
}

yasli::string extractExtensionFromFilter(tukk mask)
{
	tukk start = strchr(mask, '(');
	tukk end = strchr(mask, ')');
	if (!start || !end)
		return yasli::string();
	++start;
	tukk ext = strchr(mask, '.');
	if (!ext)
		return yasli::string();
	if (ext > end)
		return yasli::string();
