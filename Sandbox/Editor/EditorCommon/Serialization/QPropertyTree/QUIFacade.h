...
{
public:
	QtMenu(QPropertyTreeMenu* menu, QPropertyTree* tree, tukk text)
	: menu_(menu)
	, tree_(tree)
	, text_(text)
	{
	...
	IMenu* addMenu(tukk text) override{ 
	...
	IMenu* findMenu(tukk text) override{
		for (size_t i = 0; i < menus_.size(); ++i)
	...
	IMenuAction* addAction(const Icon& icon, tukk text, i32 flags = 0) override{
...
public:
	...
	i32 textWidth(tukk text, Font font) override;
	i32 textHeight(i32 width, tukk text, Font font) override;
