...
TreePath PropertyTreeModel::pathFromRow(PropertyRow* row)
{
	...
	if(row)
		while(row->parent()){
            i32 childIndex = row->parent()->childIndex(row);
...
PropertyRow* PropertyTreeModel::rowFromPath(const TreePath& path)
{
	...
	for(it = path.begin(); it != path.end(); ++it){
		i32 index = it->index;
		if(index < i32(row->count()) && index >= 0){
...
void PropertyTreeModel::callRowCallback(PropertyRow* row)
{
	...
	while (true) {
		...
		if (callback) {
			auto applyFunc = [=](uk arg, const TypeID& type) {
...
bool PropertyTreeModel::defaultTypeRegistered(tukk typeName) const
{
	return defaultTypes_.find(typeName) != defaultTypes_.end();
}

void PropertyTreeModel::addDefaultType(PropertyRow* row, tukk typeName)
{
	YASLI_ESCAPE(typeName != 0, return);
	defaultTypes_[typeName] = row;
}

PropertyRow* PropertyTreeModel::defaultType(tukk typeName) const
{
...
const PropertyDefaultDerivedTypeValue* PropertyTreeModel::defaultType(const TypeID& baseType, i32 derivedIndex) const
{
...
bool PropertyTreeModel::defaultTypeRegistered(const TypeID& baseType, tukk derivedRegisteredName) const
{
...
bool YASLI_SERIALIZE_OVERRIDE(yasli::Archive& ar, TreeSelection& value, tukk name, tukk label)
{
