...
struct ClassMenuItemAdderRowContainer : ClassMenuItemAdder
{
	...
	void addAction(property_tree::IMenu& menu, tukk text, i32 index) override
	{
...
void PropertyRowContainer::redraw(IDrawContext& context)
{
	...
	tukk text = multiValue() ? "..." : buttonLabel_;
	using namespace property_tree; // BUTTON_
	i32 buttonFlags = (context.pressed ? BUTTON_PRESSED : 0) |
...
{
	tree->model()->rowAboutToBeChanged(container);
	i32 index = container->childIndex(element);
...
{
	sprintf_s(buttonLabel_, "%i", (i32)count());
...
{
	char buf[32] = { 0 };
	sprintf_s(buf, "%d", (i32)children_.size());
	return yasli::string(buf);
}

tukk PropertyRowContainer::typeNameForFilter(PropertyTree* tree) const 
{
...
i32 PropertyRowContainer::widgetSizeMin(const PropertyTree* tree) const
{
	return inlined_ ? 0 : (userWidgetSize() >=0 ? userWidgetSize() : i32(tree->_defaultRowHeight() * 1.7f));
}
