...
bool PropertyTree::onRowKeyDown(PropertyRow* row, const KeyEvent* ev)
{
	...
	i32 x = parentRow->horizontalIndex(this, focusedRow);
	i32 y = model()->root()->verticalIndex(this, parentRow);
...
struct FirstIssueVisitor
{
	...
	ScanResult operator()(PropertyRow* row, PropertyTree* tree, i32)
	{
		...
		if (row->validatorCount()) {
			if (const ValidatorEntry* validatorEntries = tree->_validatorBlock()->getEntry(row->validatorIndex(), row->validatorCount())) {
				for (i32 i = 0; i < row->validatorCount(); ++i) {
...
void PropertyTree::jumpToNextHiddenValidatorIssue(bool isError, PropertyRow* start)
{
	...
	for (i32 i = (i32)parents.size()-1; i >= 0; --i) {
...
static void rowsInBetween(vector<PropertyRow*>* rows, PropertyRow* a, PropertyRow* b)
{
	...
	i32 maxDepth = min((i32)pathA.size(), (i32)pathB.size());
	for (i32 i = 0; i < maxDepth; ++i) {
		PropertyRow* parentA = pathA[(i32)pathA.size() - 1 - i];
		PropertyRow* parentB = pathB[(i32)pathB.size() - 1 - i];
		if (parentA != parentB) {
			i32 indexA = commonParent->childIndex(parentA);
			i32 indexB = commonParent->childIndex(parentB);
			i32 minIndex = min(indexA, indexB);
			i32 maxIndex = max(indexA, indexB);
			for (i32 j = minIndex; j <= maxIndex; ++j)
...
i32 PropertyTree::revertObjects(vector<uk> objectAddresses)
{
	i32 result = 0;
...
bool PropertyTree::revertObject(uk objectAddress)
{
...
struct ValidatorVisitor
{
	...
	ScanResult operator()(PropertyRow* row, PropertyTree* tree, i32)
	{
		ukk rowHandle = row->searchHandle();
		i32 index = 0;
		i32 count = 0;
...
static yasli::string quoteIfNeeded(tukk str)
{
	if (!str)
		return yasli::string();
	if (strchr(str, ' ') != 0) {
...
void PropertyTree::setExpandLevels(i32 levels)
{
...
i32 PropertyTree::selectedRowCount() const
{
	return (i32)model()->selection().size();
}

PropertyRow* PropertyTree::selectedRowByIndex(i32 index)
{
	const PropertyTreeModel::Selection &sel = model()->selection();
...
bool PropertyTree::selectByAddress(ukk addr, bool keepSelectionIfChildSelected)
{
	return selectByAddresses(vector<ukk>(1, addr), keepSelectionIfChildSelected);
}

bool PropertyTree::selectByAddresses(const vector<ukk>& addresses, bool keepSelectionIfChildSelected)
{
...
bool PropertyTree::selectByAddresses(ukk const* addresses, size_t addressCount, bool keepSelectionIfChildSelected)
{
	bool result = false;
	if (model()->root()) {
...
void PropertyTree::RowFilter::parse(tukk filter)
{
...
bool PropertyTree::RowFilter::match(tukk textOriginal, Type type, size_t* matchStart, size_t* matchEnd) const
{
	...
	for (size_t i = 0; i < numSubstrings; ++i) {
		tukk substr = strstr(startPos, substrings[i].c_str());
		if (!substr){
			return false;
		}
...
{
	i32 splitterStart = propertySplitterPos_ - treeStyle().propertySplitterHalfWidth;
	i32 splitterWidth = 1 + treeStyle().propertySplitterHalfWidth * 2;
	return Rect(splitterStart, area_.top(), splitterWidth, area_.height());
}

void PropertyTree::setPropertySplitterPos(i32 pos)
{
