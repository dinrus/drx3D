...
void ClassMenuItemAdder::generateMenu(IMenu& createItem, const StringList& comboStrings)
{
	...
	for (it = comboStrings.begin(); it != comboStrings.end(); ++it) {
		...
		for(i32 level = 0; level < i32(path.size()); ++level){
			tukk leaf = path[level].c_str();
...
void ClassMenuItemAdder::addAction(IMenu& menu, tukk text, i32 index)
{
	menu.addAction(text, MENU_DISABLED);
}

IMenu* ClassMenuItemAdder::addMenu(IMenu& menu, tukk text)
{
...
void PropertyRowPointer::setDerivedType(tukk typeName, yasli::ClassFactoryBase* factory)
{
	if (!factory) {
...
yasli::string PropertyRowPointer::generateLabel() const
{
	...
	if(!derivedTypeName_.empty()){
		tukk textStart = derivedTypeName_.c_str();
		if (factory_) {
		...
		tukk p = textStart + strlen(textStart);
		while(p > textStart){
			if(*(p - 1) == '\\')
				break;
			--p;
...
struct ClassMenuItemAdderRowPointer : ClassMenuItemAdder{
	...
	void addAction(property_tree::IMenu& menu, tukk text, i32 index)
	{
...
i32 PropertyRowPointer::widgetSizeMin(const PropertyTree* tree) const
{
	if (userWidgetSize() >= 0)
	{
	...
	STxt text = generateLabel();
	return tree->ui()->textWidth(text.c_str(), font) + 18;
}

static Color parseColorString(tukk str)
{
	u32 color = 0;
...
void PropertyRowPointer::setValueAndContext(const yasli::PointerInterface& ptr, yasli::Archive& ar)
{
	...
	tukk colorString = factory_->findAnnotation(ptr.registeredTypeName(), "color");
	if (colorString[0] != '\0')
