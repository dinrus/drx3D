...
bool PropertyRowField::onActivate(const PropertyActivationEvent& e)
{
	...
	if (e.reason == e.REASON_PRESS)	{
		...
		i32 buttonIndex = hitButton(e.tree, e.clickPoint);
		if (buttonIndex != -1)
...
Icon PropertyRowField::buttonIcon(const PropertyTree* tree, i32 index) const
{
	return Icon();
}

i32 PropertyRowField::widgetSizeMin(const PropertyTree* tree) const
{ 
	if (userWidgetSize() >= 0)
...
i32 PropertyRowField::hitButton(const PropertyTree* tree, const Point& point) const
{
	i32 buttonCount = this->buttonCount();
	...
	if (buttonsRect.contains(point)) {
		i32 buttonIndex = (point.x() - buttonsRect.x) / BUTTON_SIZE;
