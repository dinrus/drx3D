...
public:
	...
	void setExpandLevels(i32 levels) { expandLevels_ = levels; }
	i32 expandLevels() const{ return expandLevels_; }
	...
	bool defaultTypeRegistered(tukk typeName) const;
	void addDefaultType(PropertyRow* propertyRow, tukk typeName);
	PropertyRow* defaultType(tukk typeName) const;

	bool defaultTypeRegistered(const yasli::TypeID& baseType, tukk derivedRegisteredName) const;
	void addDefaultType(const yasli::TypeID& baseType, const PropertyDefaultDerivedTypeValue& value);
	const PropertyDefaultDerivedTypeValue* defaultType(const yasli::TypeID& baseType, i32 index) const;
...
bool YASLI_SERIALIZE_OVERRIDE(yasli::Archive& ar, TreeSelection &selection, tukk name, tukk label);
