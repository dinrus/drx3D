...
public:
	...
	i32 revertObjects(vector<uk> objectAddresses);
	bool revertObject(uk objectAddress);
	...
	void setExpandLevels(i32 levels);
	...
	void setSliderUpdateDelay(i32 delayMS) { config_.sliderUpdateDelay = delayMS; }
	...
	void setFilter(i32 filter) { config_.filter = filter; }

	// Can be used to select once serialized object(s) in the tree.
	bool selectByAddress(ukk , bool keepSelectionIfChildSelected = false);
	bool selectByAddresses(ukk const* addresses, size_t addressCount, bool keepSelectionIfChildSelected);
	bool selectByAddresses(const vector<ukk>& addresses, bool keepSelectionIfChildSelected);
	// Can be used to query information about selection in the tree.
	bool setSelectedRow(PropertyRow* row);
	PropertyRow* selectedRow();
	i32 selectedRowCount() const;
	PropertyRow* selectedRowByIndex(i32 index);
...
public:
	// internal use
	i32 leftBorder() const { return leftBorder_; }
	i32 rightBorder() const { return rightBorder_; }
	...
	i32 _defaultRowHeight() const { return defaultRowHeight_; }
	i32 getPropertySplitterPos() const { return propertySplitterPos_; }
	void setPropertySplitterPos(i32 pos);
	...
	struct PROPERTY_TREE_API RowFilter {
		...
		void parse(tukk filter);
		bool match(tukk text, Type type, size_t* matchStart, size_t* matchEnd) const;
...
protected:
	...
	virtual bool canBePasted(tukk destinationType) = 0;
	...
	virtual void startFilter(tukk text) = 0;
...
protected:
	...
	i32 defaultRowHeight_;
	i32 leftBorder_;
	i32 rightBorder_;
	i32 propertySplitterPos_;
	Point size_;
	Point offset_;
	Rect area_;
