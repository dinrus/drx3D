...
inline unsigned calcHash(tukk str, unsigned hash = 5381)
{
	while(*str)
		hash = hash*33 + (u8)*str++;
	return hash;
...
void PropertyRow::setNames(tukk name, tukk label, tukk typeName)
{
	name_ = name;
	YASLI_ASSERT(name);
	label_ = label ? label : "";
...
PropertyRow* PropertyRow::childByIndex(i32 index)
{
	if(index >= 0 && index < i32(children_.size()))
		return children_[index];
	else
		return 0;
}

const PropertyRow* PropertyRow::childByIndex(i32 index) const
{
	if(index >= 0 && index < i32(children_.size()))
		return children_[index];
	else
...
struct SetExpandedOp {
    ...
    ScanResult operator()(PropertyRow* row, PropertyTree* tree, i32 index)
    {
...
void PropertyRow::setLabel(tukk label) 
{
	if (!label)
		label = "";
	if (label_ != label) {
...
void PropertyRow::setTooltip(tukk tooltip)
{
	tooltip_ = tooltip;
}

bool PropertyRow::setValidatorEntry(i32 index, i32 count)
{
...
void PropertyRow::updateLabel(const PropertyTree* tree, i32 index, bool parentHidesNonInlineChildren)
{
	if (!labelChanged_) {
		if (pulledUp_)
...
void PropertyRow::parseControlCodes(const PropertyTree* tree, tukk ptr, bool changeLabel)
{
	if (changeLabel) {
		userFullRow_ = false;
		pulledUp_ = false;
	...
	if (isContainer()) {
		...
		i32 numChildren = (i32)container->count();
...
tukk PropertyRow::typeNameForFilter(PropertyTree* tree) const
{
	return typeName();
}

void PropertyRow::updateTextSizeInitial(const PropertyTree* tree, i32 index, bool fontChanged)
{
	char containerLabel[16] = "";
	tukk text = rowText(containerLabel, tree, index);
...
void PropertyRow::calculateMinimalSize(const PropertyTree* tree, i32 posX, i32 availableWidth, bool force, i32* _extraSizeRemainder, i32* _extraSize, i32 index)
{
	...
	if(isRoot())
		...
		if(nonPulled->isRoot() || (tree->treeStyle().compact && nonPulled->parent()->isRoot()))
			...
			plusSize_ = i32(tree->treeStyle().firstLevelIndent * tree->_defaultRowHeight());
	...
	if (tree->treeStyle().propertySplitter)
	{
		...
		i32 splitterStart = tree->getPropertySplitterPos() - tree->treeStyle().propertySplitterHalfWidth;
		i32 splitterWidth = 1 + tree->treeStyle().propertySplitterHalfWidth * 2;
		i32 splitterEnd = splitterStart + splitterWidth;
		...
		if (!pulledUp())
		{
			...
			if (hasTitleIcon_)
			{
				i32 iconSize = tree->_defaultRowHeight();
				posX += iconSize;
			}
			textPos_ = posX;
			...
			i32 minWidgetSize = widgetSizeMin(tree);
		...
		i32 height = isRoot() ? 0 : tree->_defaultRowHeight() + floorHeight();
		size_.setY(height);
		i32 pulledCount = 0;
		i32 lastIndex = 0;
	...
	i32 extraSizeStorage = 0;
	i32& extraSize = !pulledUp() || !_extraSize ? extraSizeStorage : *_extraSize;

	i32 validatorIconsWidth = 0;
	if (validatorHasErrors_)
		validatorIconsWidth += tree->_defaultRowHeight();
	if (validatorHasWarnings_)
		validatorIconsWidth += tree->_defaultRowHeight();

	i32 freePulledChildren = 0;
	if(!pulledUp()){
		i32 minTextSize = 0;
		i32 totalMinimalWidth = 0;
	...
	if(widgetPlace == WIDGET_VALUE || widgetPlace == WIDGET_AFTER_PULLED || (freePulledChildren > 0)){
		...
		if(!pulledUp() && extraSize > 0){
			...
			if(!isFullRow(tree))
			{
				...
				i32 maxX = rightAlignment ? textSize_ + TEXT_VALUE_SPACING: posX;
				i32 newX = max(tree->leftBorder() + xround((tree->rightBorder() - tree->leftBorder())* (1.f - tree->treeStyle().valueColumnWidth)), maxX);
	...
	if (widgetPlace == WIDGET_VALUE || widgetPlace == WIDGET_INSTEAD_OF_TEXT) {
		...
		if (widgetPlace != WIDGET_INSTEAD_OF_TEXT)
		widgetPos_ = posX;
		DEBUG_TRACE_ROW("textSize: %i widgetPos: %i", i32(textSize_), i32(widgetPos_));
		posX += widgetSize_;
	}

	size_.setX(textSize_ + (textSize_ ? TEXT_VALUE_SPACING : 0) + widgetSize_ + validatorIconsWidth);
	
	i32 childrenLeft = nonPulled->pos_.x();
	if (parent() != 0){
		if (parent()->parent() == 0) {
			if (!tree->treeStyle().doNotIndentSecondLevel)
				childrenLeft += i32(tree->treeStyle().firstLevelIndent * tree->_defaultRowHeight());
		}
		else
			childrenLeft += i32(tree->treeStyle().levelIndent * tree->_defaultRowHeight());
	}

	i32 checkBoxChildren = 0;
	for (i32 i = 0; i < numChildren; ++i) {
		PropertyRow* row = children_[i];
		if(!row->visible(tree)) {
	...
	if (tree->packCheckboxes() || userPackCheckboxes_) {
		if (expanded() && checkBoxChildren > 0 && hasVisibleChildren(tree)) {
			i32 widthTotal = tree->rightBorder() - 16 - childrenLeft - plusSize_;
			i32 widthNextToLastCheckbox = 0;
			i32 left = childrenLeft + plusSize_;
			...
			for (i32 i = 0; i < numChildren; ++i) {
				...
				if(!row->pulledUp()){
					i32 checkboxWidth = row->textSize_ + tree->_defaultRowHeight()/* + TEXT_VALUE_SPACING*/;
			...
			if (hasChanges) {
				for (i32 i = 0; i < checkboxesToRealign.size(); ++i) {
	...
	if (!pulledUp() && !pulledBefore() && (validatorCount_ != 0 || hasPulled_)) {
		i32 padding = i32(0.1f * tree->_defaultRowHeight());
		auto calculateValidatorHeight = [&](PropertyRow* row) {
			...
			if (entries) {
				for (i32 i = 0; i < row->validatorCount_; ++i) {
					i32 startPos = pos_.x() + plusSize_;
					i32 height = tree->ui()->textHeight(availableWidth - startPos - tree->_defaultRowHeight() - padding * 2, entries[i].message.c_str(), property_tree::FONT_NORMAL);
...
void PropertyRow::adjustVerticalPosition(const PropertyTree* tree, i32& totalHeight)
{
	i32 defaultRowHeight = tree->_defaultRowHeight();
	pos_.setY(totalHeight);
	i32 rowHeight = size_.y() + i32(defaultRowHeight * (tree->treeStyle().rowSpacing - 1.0f) + 0.5f);
	...
	i32 delta = totalHeight - pos_.y();
	if (delta > USHRT_MAX)
		delta = USHRT_MAX;
	heightIncludingChildren_ = delta;
}

void PropertyRow::setTextSize(const PropertyTree* tree, i32 index, float mult)
{
	updateTextSizeInitial(tree, index, false);

	textSize_ = i32(textSizeInitial_ * mult);
...
void PropertyRow::calcPulledRows(i32* minTextSize, i32* freePulledChildren, i32* minimalWidth, const PropertyTree *tree, i32 index) 
{
	updateTextSizeInitial(tree, index, false);
...
PropertyRow* PropertyRow::find(tukk name, tukk nameAlt, tukk typeName)
{
	iterator it;
	for(it = children_.begin(); it != children_.end(); ++it){
		PropertyRow* row = *it;
...
PropertyRow* PropertyRow::findFromIndex(i32* outIndex, tukk name, tukk typeName, i32 startIndex) const
{
	i32 numChildren = (i32)children_.size();
	startIndex = min(startIndex, numChildren);

	for (i32 i = startIndex; i < numChildren; ++i) {
		PropertyRow* row = children_[i];
		if(((row->name() == name) || strcmp(row->name(), name) == 0) &&
...
const PropertyRow* PropertyRow::find(tukk name, tukk nameAlt, tukk typeName) const
{
...
void PropertyRow::drawRow(IDrawContext& context, const PropertyTree* tree, i32 index, bool selectionPass)
{
	Rect rowRect = rect();
	Rect selectionRect;
	...
	else
	{
		if (tree->treeStyle().propertySplitter)
		{
			...
			{
				i32 splitterHeight = heightIncludingChildren();
	...
	if (selectionPass) {
		...
		i32 lineSize = widgetPos_ - textPos_ - 2;
...
void PropertyRow::intersect(const PropertyRow* row)
{
	...
	i32 indexSource = 0;
	for(i32 i = 0; i < i32(children_.size()); ++i)
	{
...
tukk PropertyRow::rowText(char (&containerLabelBuffer)[16], const PropertyTree* tree, i32 index) const
{
...
PropertyRow* PropertyRow::findByAddress(ukk addr)
{
...
ukk PropertyRow::searchHandle() const
{
...
i32 PropertyRow::verticalIndex(PropertyTree* tree, PropertyRow* row)
{
...
struct RowByVerticalIndexOp{
    ...
    RowByVerticalIndexOp(i32 index) : row_(0), index_(index) {}
...
PropertyRow* PropertyRow::rowByVerticalIndex(PropertyTree* tree, i32 index)
{
...
i32 PropertyRow::horizontalIndex(PropertyTree* tree, PropertyRow* row)
{
	if(row == this)
		return 0;
...
struct RowByHorizontalIndexOp{
    ...
    RowByHorizontalIndexOp(i32 index) : row_(0), index_(index), pulledBefore_(index < 0) {}
...
PropertyRow* PropertyRow::rowByHorizontalIndex(PropertyTree* tree, i32 index)
{
	if(!index)
		return this;
...
{
	i32 rowHeight = tree->_defaultRowHeight();
	i32 width = validatorHasErrors_ && !expanded_ ? rowHeight : 0;
	i32 normalX = pos_.x() + size_.x() - width;
	i32 minimalX = max(widgetPos_ + widgetSize_, textPos_ + textSize_);
...
Rect PropertyRow::validatorWarningIconRect(const PropertyTree* tree) const
{
	...
	i32 width = validatorHasWarnings_ && !expanded_ ? r.height() : 0;
...
void PropertyRow::setTitleIcon(tukk icon)
{
	if (icon)
	{
...
i32 RowWidthCache::getOrUpdate(const PropertyTree* tree, const PropertyRow* rowForValue, i32 extraSpace)
{
	yasli::string value = rowForValue->valueAsString();
	const Font font = rowForValue->rowFont(tree);
	u32 newHash = calculateHash(value.c_str());
