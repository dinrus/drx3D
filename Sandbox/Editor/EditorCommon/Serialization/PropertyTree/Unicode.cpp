...
yasli::string fromWideChar(const wchar_t* wstr)
{
  ...
  // Therefore we avoid calling any wchar_t functions in Qt.
	...
	u32k codepage = CP_ACP;
#else
	u32k codepage = CP_UTF8;
#endif
	i32 len = WideCharToMultiByte(codepage, 0, wstr, -1, NULL, 0, 0, 0);
	tuk buf = (tuk)alloca(len);
	if(len > 1){
...
yasli::wstring toWideChar(tukk str)
{
#ifdef _WIN32
#ifdef WW_DISABLE_UTF8
	u32k codepage = CP_ACP;
	...
	u32k codepage = CP_UTF8;
#endif
    i32 len = MultiByteToWideChar(codepage, 0, str, -1, NULL, 0);
