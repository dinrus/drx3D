...
public:
	...
	void populateComboBox(std::vector<STxt>* values, i32* selectedIndex, PropertyTree* tree) override
	{
		values->assign(stringList_.begin(), stringList_.end());
		*selectedIndex = stringList_.find(value_.c_str());
	}

	bool onComboBoxSelected(tukk newValue, PropertyTree* tree) override
	{
		if (value_ != newValue) {
