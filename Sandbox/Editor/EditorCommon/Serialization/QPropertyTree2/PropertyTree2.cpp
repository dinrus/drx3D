	...
	, m_autoExpandDepth(1)
{
	...
	closeSearch->setIcon(DrxIcon("icons:General/Close.ico"));
	...
	searchNext->setIcon(DrxIcon("icons:General/Arrow_Down.ico"));
	...
	searchPrev->setIcon(DrxIcon("icons:General/Arrow_Up.ico"));
...
void QPropertyTree2::attach(const yasli::Serializers& serializers)
{
	...
	for(i32 i = 0; i < serializers.size(); i++)
		m_models.emplace_back(new PropertyTree2::CRowModel(nullptr));
	revert();
}
...
void QPropertyTree2::revert()
{
	...
	if (!m_attached.empty())
	{
		for(i32 i = 0; i < m_attached.size(); i++)
		{
...
void QPropertyTree2::CopyRowValueToModels(const PropertyTree2::CRowModel* row)
{
	...
	for (auto& model : m_models)
	{
		...
		for (auto it = parents.rbegin(); it != parents.rend(); ++it)
		{
			...
			DRX_ASSERT(foundIt != current->GetChildren().end()); //Since the models have been intersected, the equivalent row must exist
			current = *foundIt;
		}
		...
		DRX_ASSERT(matchingRowIt != current->GetChildren().end()); //Since the models have been intersected, the equivalent row must exist
...
void QPropertyTree2::SetSplitterPosition(i32 pos)
{
...
i32 QPropertyTree2::GetSplitterPosition() const
{
	...
	return i32(m_splitterRatio * float(width()));
}
