...
public:
	...
	CRowModel(tukk name, tukk label, CRowModel* parent);

	//! Does not set the widget
	CRowModel(tukk name, tukk label, CRowModel* parent, const yasli::TypeID& type);
	...
	void SetTooltip(tukk tooltip) { m_tooltip = tooltip; }
...
private:
	...
	Flags m_flags;
	i32 m_index; //cached index in parent, in order to speed up calculations
};
