...
#include <drx3D/CoreX/Serialization/Callback.h>
...
SERIALIZATION_ENUM_DECLARE(ECountry, : u8,
                           Albania,
                           Belarus,
                           Croatia,
                           Denmark,
...
struct MultiEditTest2
{
	...
	string                         memberString;
	bool                           memberBool;
	std::vector<i32>               intVector { 0, 1, 2, 3, 4 };
	std::shared_ptr<DynCreateBase> memberSharedPtr;
	ECountry                       country;
	ColorB                         color;
...
struct TestStruct
{
	...
	i32                                         intStaticArray[5] { 0, 1, 2, 3, 4 };
	std::map<string, i32>                       strintmap;
	...
	void                                        Serialize(Serialization::IArchive& ar)
	{
		ar(memberInt, "i32", "Member Int (tooltip)");
