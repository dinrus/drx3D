...
struct IPropertyTreeWidget
{
	...
	virtual void SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar) = 0;

	//!Gets the value from the widget into one of the viable types
	virtual void GetValue(uk valuePtr, const yasli::TypeID& type) const = 0;

	template<typename T>
	void SetValue(const T& valueRef, const yasli::Archive& ar) { SetValue((uk )&valueRef, yasli::TypeID::get<T>(), ar); }
	...
	virtual void SetMultiEditValue() { DRX_ASSERT(false); /* not implemented */ }
	...
	virtual void MoveChild(i32 oldIndex, i32 newIndex) {}
	...
	CDrxSignal<void(const PropertyTree2::CRowModel*)> signalChanged;
	CDrxSignal<void(const PropertyTree2::CRowModel*)> signalContinuousChanged;
...
public:
	...
	{
		Factory(tukk key, CPropertyTreeWidgetFactory& factory)
		{
...
public:
	void Add(tukk name, FactoryBase* factory)
	{
		m_factories[name] = factory;
	}

	Product* Create(tukk typeName) const
	{
	...
	bool IsRegistered(tukk typeName) const
	{
		auto it = m_factories.find(typeName);
...
private:
	...
	typedef std::map<tukk , FactoryBase*, LessStrCmpPR> Factories;
