...
namespace PropertyTree2
{
	...
	public:
		...
		virtual void SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar) override;
		virtual void GetValue(uk valuePtr, const yasli::TypeID& type) const override;
		virtual void Serialize(Serialization::IArchive& ar) final {}
	...
	public:
		...
		virtual void MoveChild(i32 oldIndex, i32 newIndex) final;
	...
	private:
		...
		struct Action
		{
			Action(ActionType t = ActionType::None, i32 a1 = -1, i32 a2 = -1) : type(t), arg1(a1), arg2(a2) {}
			ActionType type;
			i32 arg1;
			i32 arg2; //some actions use two indices, such as move(from,to)
	...
	template<typename T>
	void CNumberWidget<T>::SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar)
	{
		...
		else if (type == yasli::TypeID::get<yasli::RangeDecorator<T>>())
		{
			...
			setVal(m_previousValue);
		}
	}


	template<typename T>
	void CNumberWidget<T>::GetValue(uk valuePtr, const yasli::TypeID& type) const
	{
