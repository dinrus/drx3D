...
#include <drx3D/CoreX/Serialization/yasli/decorators/Button.h>
#include <drx3D/CoreX/Serialization/yasli/decorators/Range.h>
#include <drx3D/CoreX/Serialization/ColorImpl.h>
...
namespace PropertyTree2
{
	...
	void CTextWidget::SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar)
	{
	...
	void CTextWidget::GetValue(uk valuePtr, const yasli::TypeID& type) const
	{
		if (type == yasli::TypeID::get<yasli::StringInterface>())
		{
			yasli::StringInterface* si = (yasli::StringInterface*)valuePtr;
	...
	void CBoolWidget::SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar)
	{
	...
	void CBoolWidget::GetValue(uk valuePtr, const yasli::TypeID& type) const
	{
		if (type == yasli::TypeID::get<bool>())
		{
	...
	void CButtonWidget::SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar)
	{
	...
	void CButtonWidget::GetValue(uk valuePtr, const yasli::TypeID& type) const
	{
		//Do nothing
	}
	...
	public:
		...
		void SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar) final
		{
	...
	public:
		...
		void GetValue(uk valuePtr, const yasli::TypeID& type) const final
		{
			if (type == yasli::TypeID::get<yasli::Button>())
			{
				yasli::Button* button = (yasli::Button*)valuePtr;
	...
	void CPtrTypeSelectWidget::SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar)
	{
	...
	void CPtrTypeSelectWidget::GetValue(uk valuePtr, const yasli::TypeID& type) const
	{
		if (type == yasli::TypeID::get<yasli::PointerInterface>())
		{
			yasli::PointerInterface* ptr = (yasli::PointerInterface*)valuePtr;
			...
			bool ok = false;
			i32 selectedFactoryIndex = GetCurrentData().toInt(&ok);
			DRX_ASSERT(ok);
	...
	void CPtrTypeSelectWidget::SetupCombo()
	{
		...
		tukk nullLabel = m_factory->nullLabel();
	...
	CArrayWidget::CArrayWidget()
	{
		...
		m_addElementButton->setIcon(DrxIcon("icons:General/Element_Add.ico"));
	...
	void CArrayWidget::SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar)
	{
		DRX_ASSERT(type == yasli::TypeID::get<yasli::ContainerInterface>());
	...
	void CArrayWidget::GetValue(uk valuePtr, const yasli::TypeID& type) const
	{
		DRX_ASSERT(type == yasli::TypeID::get<yasli::ContainerInterface>());
		yasli::ContainerInterface* pContainer = (yasli::ContainerInterface*)valuePtr;
	...
	void CArrayWidget::MoveChild(i32 oldIndex, i32 newIndex)
	{
	...
	{
		DRX_ASSERT(row->GetParent() == m_rowModel);
	...
	{
		m_action = { ActionType::InsertItem, (i32)m_rowModel->GetChildren().size(), -1 };
		OnChanged();
	}
	...
	void CArrayWidget::Serialize(Serialization::IArchive& ar)
	{
		...
		else
		{
			...
			{
				i32 size = m_rowModel->GetChildren().size();
				ar(size, "size", "Size");
			}
		}
	}
	...
	void CStringListWidget::SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar)
	{
	...
	void CStringListWidget::GetValue(uk valuePtr, const yasli::TypeID& type) const
	{
		if (type == yasli::TypeID::get<yasli::StringListValue>())
		{
	...
	void PropertyTree2::CStringListWidget::SetupCombo(const List& list)
	{
		...
		{
			for (i32 i = 0; i < stringList.size(); i++)
			{
	...
	void CColorwidget::SetValue(uk valuePtr, const yasli::TypeID& type, const yasli::Archive& ar)
	{
		m_multiEdit = false;
		if (type == yasli::TypeID::get<SerializableColor_tpl<u8>>())
		{
			const SerializableColor_tpl<u8>& value = *(SerializableColor_tpl<u8>*)valuePtr;
			SetColor(value);
			SetHasAlpha(true);
		}
	...
	void CColorwidget::GetValue(uk valuePtr, const yasli::TypeID& type) const
	{
		if (type == yasli::TypeID::get<SerializableColor_tpl<u8>>())
		{
			ColorB& value = *(SerializableColor_tpl<u8>*)valuePtr;
			value = GetColorB();
		}
		else if (type == yasli::TypeID::get<SerializableColor_tpl<float>>())
		{
...
REGISTER_PROPERTY_WIDGET_TEMPLATE(SerializableColor_tpl<u8>, ColorB, PropertyTree2::CColorwidget);
