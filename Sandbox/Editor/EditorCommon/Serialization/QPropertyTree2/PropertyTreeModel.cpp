...
namespace PropertyTree2
{
	...
	CRowModel::CRowModel(tukk name, tukk label, CRowModel* parent)
		: m_name(name)
		, m_label(label)
		, m_widget(nullptr)
		, m_parent(parent)
	...
	CRowModel::CRowModel(tukk name, tukk label, CRowModel* parent, const yasli::TypeID& type)
		: m_name(name)
		, m_label(label)
		, m_type(type)
		, m_widget(nullptr)
	...
	void CRowModel::SetWidgetAndType(const yasli::TypeID& type)
	{
		if (type && !m_widget)
		{
			...
			DRX_ASSERT_MESSAGE(m_widget, "No widget registered for type %s", m_type.name());
	...
	void CRowModel::MarkDirty(DirtyFlag flag /*= DirtyFlag::Dirty*/)
	{
		...
		while (parent)
		{
			...
			case DirtyFlag::NotVisited:
				DRX_ASSERT(false); //This should not happen !
