...
namespace PropertyTree2
{
	...
	CFormWidget::CFormWidget(QPropertyTree2* parentTree, const CRowModel* row, i32 nesting)
		: m_rowModel(row)
	...
	void CFormWidget::SetupWidgets()
	{
		...
		i32k count = m_rowModel->GetChildren().size();
		...
		for (i32 i = 0; i < count; i++)
		{
			...
			else if(child->HasChildren())
			{
				...
				if (inlinedChild->GetFlags() & CRowModel::Inline)
				{
					...
					i32 index = 0;
					i32k count = child->GetChildren().size();
	...
	void CFormWidget::SetSpacing(i32 value)
	{
		if(value != m_spacing)
		{
			m_spacing = value;
	...
	void CFormWidget::SetMinRowHeight(i32 value)
	{
	...
	void CFormWidget::SetIconSize(i32 value)
	{
		if(value != m_iconSize)
		{
			m_iconSize = value;
	...
	void CFormWidget::SetGroupBorderWidth(i32 value)
	{
	...
	{
		DRX_ASSERT(row && row->GetParent() == m_rowModel && row->HasVisibleChildren());
	...
	void CFormWidget::SetNestingIndent(i32 value)
	{
	...
	void CFormWidget::paintEvent(QPaintEvent* event)
	{
		...
		if (m_dragInsertIndex >= 0)
		{
			...
			DRX_ASSERT(index >= 0 && index < m_rows.size());
		...
		i32k flags = Qt::AlignVCenter | Qt::TextForceLeftToRight | Qt::TextSingleLine;
		i32k leftMargin = contentsMargins().left();
		i32k textIndent = leftMargin + m_groupBorderWidth + m_nesting * m_nestingIndent + m_iconSize + m_spacing;

		i32k count = m_rows.size();
		for (i32 i = 0; i < count; i++)
		{
	...
	void CFormWidget::mousePressEvent(QMouseEvent* ev)
	{
		...
		i32k splitterPos = GetSplitterPosition();
	...
	void CFormWidget::dropEvent(QDropEvent* ev)
	{
		...
		if (data->HasCustomData("PropertyRow"))
		{
			...
			if (draggedRow->GetParent() == m_rowModel && m_allowReorderChildren)
			{
				i32k insertIndex = InsertionIndexAtPos(ev->pos());

				const auto w = m_rowModel->GetPropertyTreeWidget();
				DRX_ASSERT(w && w->CanMoveChildren());
	...
	void CFormWidget::UpdateMinimumSize()
	{
		...
		i32 height = contentsMargins().top() + contentsMargins().bottom();

		i32 widgetsMinWidth = 0;
		i32 nestedFormsMinWidth = 0;

		i32k childCount = m_rows.size();
		if (childCount > 0)
		...
		{
			const auto& row = m_rows[i];
			i32k groupPadding = row.HasChildren() ? m_groupBorderWidth * 2 : 0;
		...
		i32k labelsMinWidth = 20;
		i32 width = contentsMargins().left() + contentsMargins().right() + labelsMinWidth + widgetsMinWidth;
	...
	void CFormWidget::DoLayout(const QSize& size)
	{
		...
		i32 height = contentsMargins().top();

		//Left and right limit for content
		i32k left = contentsMargins().left();
		i32k right = size.width() - contentsMargins().right();

		//Left and right limit for widgets (right side of the tree)
		i32k wLeft = GetSplitterPosition();
		i32k wRight = right - (m_allowReorderChildren ? m_iconSize + m_spacing : 0);
		...
		for (i32 i = 0; i < childCount; i++)
		{
			...
			if (widget != nullptr)
			{
				...
				i32k widgetWidth = expandsHorizontal ? wRight - wLeft - groupPadding : min(sizeHint.width(), wRight - wLeft - groupPadding);
				i32k rowHeight = max(m_minRowHeight, sizeHint.height()) + 2*groupPadding;
			...
			{
				i32k rowHeight = m_minRowHeight + 2*groupPadding;
				row.rowRect = QRect(left, height, right - left, rowHeight);
			}
			...
			if (row.childContainer && row.IsExpanded())
			{
				...
				i32k rowHeight = row.childContainer->minimumSizeHint().height();
		...
		{
			UpdateMinimumSize();
			DRX_ASSERT(height - m_spacing <= minimumSize().height());
		}
	}
	...
	i32 CFormWidget::GetSplitterPosition() const
	{
	...
	{
		DRX_ASSERT(row && row->GetParent() == m_rowModel && !row->IsHidden());
	...
	i32 CFormWidget::InsertionIndexAtPos(const QPoint& position, i32 tolerancePx)
	{
	...
	i32 CFormWidget::RowIndex(const FormRow* row)
	{
		return i32(row - &(*m_rows.begin()));
	}

	void CFormWidget::OnActiveRowChanged(const CRowModel* newActiveRow)
	{
		DRX_ASSERT(!newActiveRow || !newActiveRow->IsRoot());
