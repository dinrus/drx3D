...
public:
	...
	virtual void  Execute(tukk command) = 0;
	virtual float GetAsFloat(tukk variable) = 0;
};

struct SPythonCommand
{
	SPythonCommand(tukk moduleName, tukk functionName, tukk description, tukk example, std::function<void()> registerFunc)
		: m_module(moduleName)
		, m_name(functionName)
