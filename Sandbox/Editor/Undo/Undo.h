...
public:
	...
	// Undo last operation.
	void Undo(i32 numSteps = 1);

	//! Redo last undo.
	void Redo(i32 numSteps = 1);
	...
	i32 GetUndoStackLen() const;
	//! Returns length of redo stack.
	i32 GetRedoStackLen() const;
	...
	void ClearRedoStack(i32 num);

	//! Clears num undo objects in undo stack
	void ClearUndoStack(i32 num);
