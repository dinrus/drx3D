...
public:
	...
	{
		for (i32 i = m_undoObjects.size() - 1; i >= 0; i--)
		{
	...
	virtual void Redo()
	{
		for (i32 i = 0; i < m_undoObjects.size(); i++)
		{
...
public:
	...
	virtual void Undo(bool bUndo)
	{
		for (i32 i = m_undoSteps.size() - 1; i >= 0; i--)
		{
	...
	virtual void Redo()
	{
		for (i32 i = 0; i < m_undoSteps.size(); i++)
		{
			m_undoSteps[i]->Redo();
		}
	}
...
{
	//DrxLog( "<Undo> Begin SuspendCount=%d",m_suspendCount );
	//if (m_bSuperRecording)
	//DrxLog( "<Undo> Begin (Inside SuperSuper)" );
...
//////////////////////////////////////////////////////////////////////////
void CUndoManager::Accept(tukk szName)
{
	//DrxLog( "<Undo> Accept, Suspend Count=%d",m_suspendCount );
	if (m_bUndoing || m_bRedoing) // If Undoing or redoing now, ignore this calls.
		return;
	...
	if (!m_currentUndo->IsEmpty())
	{
		...
		else
		{
			...
			signalBufferChanged(m_currentUndo, CUndoManager::eCommandChangeType_Insert, (i32)m_undoStack.size() - 1);
			
			QString logLine = name + ": " + m_currentUndo->GetObjectNames();
			//DrxLog(logLine.toStdString().c_str());
		}
		//DrxLog( "Undo Object Accepted (Undo:%d,Redo:%d, Size=%dKb)",m_undoStack.size(),m_redoStack.size(),GetDatabaseSize()/1024 );
...
void CUndoManager::Cancel()
{
	...
	if (!m_currentUndo->IsEmpty())
	{
		...
		Restore(true);
		DrxLog("Undo Object Canceled (Undo:%d,Redo:%d)", m_undoStack.size(), m_redoStack.size());
	}
...
//////////////////////////////////////////////////////////////////////////
void CUndoManager::Redo(i32 numSteps)
{
	...
	{
		DrxLog("Cannot Redo while Recording");
		return;
	}
	...
	if (!m_redoStack.empty())
	{
		...
		while (numSteps-- > 0 && !m_redoStack.empty())
		{
			...
			signalBufferChanged(redo, eCommandChangeType_Move, (i32)m_undoStack.size() - 1);
		}
		Resume();
	}
	if (m_suspendCount == 0)
		GetIEditorImpl()->UpdateViews(eUpdateObjects);
	DrxLog("Redo (Undo:%d,Redo:%d)", m_undoStack.size(), m_redoStack.size());
...
//////////////////////////////////////////////////////////////////////////
void CUndoManager::Undo(i32 numSteps)
{
	GetIEditorImpl()->Notify(eNotify_OnBeginUndoRedo);
	...
	if (m_bRecording || m_bSuperRecording)
	{
		DrxLog("Cannot Undo while Recording");
		return;
	}
	...
	if (!m_undoStack.empty())
	{
		...
		while (numSteps-- > 0 && !m_undoStack.empty())
		{
			...
			signalBufferChanged(undo, eCommandChangeType_Move, (i32)m_undoStack.size() - 1);
		}
		Resume();
	}
	// Update viewports.
	if (m_suspendCount == 0)
		GetIEditorImpl()->UpdateViews(eUpdateObjects);
	DrxLog("Undo (Undo:%d,Redo:%d)", m_undoStack.size(), m_redoStack.size());
	GetIEditorImpl()->GetObjectManager()->InvalidateVisibleList();
...
{
	//DrxLog( "<Undo> RecordUndo Name=%s",obj->GetDescription() );
	...
	if (m_bRecording && (m_suspendCount == 0))
	{
		...
		//DrxLog( "Undo Object Added: %s",obj->GetDescription() );
	}
	else
	{
...
{
	i32 redoCount = m_redoStack.size();
...
{
	i32 undoCount = m_undoStack.size();
	ClearUndoStack(undoCount);
}

//////////////////////////////////////////////////////////////////////////
void CUndoManager::ClearUndoStack(i32 num)
{
	i32 i = num;
...
//////////////////////////////////////////////////////////////////////////
void CUndoManager::ClearRedoStack(i32 num)
{
	i32 i = num;
...
{
	m_suspendCount++;
	//DrxLog( "<Undo> Suspend %d",m_suspendCount );
}
...
void CUndoManager::Resume()
{
	...
	//DrxLog( "<Undo> Resume %d",m_suspendCount );
}

//////////////////////////////////////////////////////////////////////////
void CUndoManager::SuperBegin()
{
	//DrxLog( "<Undo> SuperBegin (SuspendCount%d)",m_suspendCount );
	if (m_bUndoing || m_bRedoing) // If Undoing or redoing now, ignore this calls.
		return;
...
void CUndoManager::SuperAccept(const string& name)
{
	...
	if (!m_superUndo->IsEmpty())
	{
		...
		signalBufferChanged(m_superUndo, eCommandChangeType_Insert, (i32)m_undoStack.size() - 1);
	}
	else
	{
		// If no any object was recorded, Cancel undo operation.
	...
	//DrxLog( "Undo Object Accepted (Undo:%d,Redo:%d)",m_undoStack.size(),m_redoStack.size() );
	m_bSuperRecording = false;
	m_superUndo = 0;
...
//////////////////////////////////////////////////////////////////////////
i32 CUndoManager::GetUndoStackLen() const
{
	return m_undoStack.size();
}

//////////////////////////////////////////////////////////////////////////
i32 CUndoManager::GetRedoStackLen() const
{
