...
public:
	...
	void                         ExecuteCommand(tukk sCommand, ...);
	...
	void                         SetDocument(CDrxEditDoc* pDoc);
	void                         CloseDocument();
	CDrxEditDoc*                 GetDocument() const;
	...
	void                         WriteToConsole(tukk pszString);

	void                         SetConsoleVar(tukk var, float value);
	void                         SetConsoleStringVar(tukk var, tukk value);

	float                        GetConsoleVar(tukk var);
	...
	virtual tukk GetLevelName() override;
	virtual tukk GetLevelPath() override;
	...
	virtual CBaseObject*            NewObject(tukk type, tukk file = nullptr, bool bInteractive = false) override;

	void                            DeleteObject(CBaseObject* obj);
	CBaseObject*                    CloneObject(CBaseObject* obj);
	void                            StartObjectCreation(tukk type, tukk file = nullptr) override;
	...
	void                             UpdateViews(i32 flags = 0xFFFFFFFF, AABB* updateRegion = NULL);
	void                             ResetViews();
	...
	bool                             AddToolbarItem(u8 iId, IUIEvent* pIHandler);
	void                             SetDataModified();

	void                             SetEditMode(i32 editMode);
	i32                              GetEditMode();
	...
	CWnd*                            OpenView(tukk sViewClassName) override;
	CWnd*                            FindView(tukk sViewClassName) override;
	IPane*                           CreateDockable(tukk className) override;
	...
	bool                             CloseView(tukk sViewClassName);
	...
	virtual void RegisterURIListener(IUriEventListener* pListener, tukk szSchema);
	virtual void UnregisterURIListener(IUriEventListener* pListener, tukk szSchema);
	//~IUriEventListener interface

	virtual void           RequestScriptReload(const EReloadScriptsType& kind);

	void                   AddNativeHandler(uintptr_t id, std::function<bool(uk , long*)> callback);
	...
	virtual void             RegisterDeprecatedPropertyEditor(i32 propertyType, std::function<bool(const string& /*oldValue*/, string& /*newValueOut*/)>& handler) override;
	bool                     EditDeprecatedProperty(i32 propertyType, const string& oldValue, string& newValueOut);
	bool                     CanEditDeprecatedProperty(i32 propertyType);
...
protected:
	...
	std::map<i32, std::function<bool(const string&, string&)>> m_deprecatedPropertyEditors;
	...
	i32  doc_use_subfolder_for_crash_backup;
	bool m_bInitialized;
