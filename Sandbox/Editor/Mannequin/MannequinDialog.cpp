...
i32k CMannequinDialog::s_minPanelSize = 5;
static tukk kMannequin_setkeyproperty = "e_mannequin_setkeyproperty";
...
void SetMannequinDialogKeyPropertyCmd(IConsoleCmdArgs* pArgs)
{
	...
	tukk propertyName = pArgs->GetArg(1);
	tukk propertyValue = pArgs->GetArg(2);
...
class CMannequinPaneClass : public IViewPaneClass
{
	...
	virtual tukk    ClassName()       { return MANNEQUIN_EDITOR_TOOL_NAME; };
	virtual tukk    Category()        { return "Animation"; };
	virtual tukk    GetMenuPath()     { return "Animation"; }
	//////////////////////////////////////////////////////////////////////////
	virtual CRuntimeClass* GetRuntimeClass() { return RUNTIME_CLASS(CMannequinDialog); };
	virtual tukk    GetPaneTitle()    { return _T(MANNEQUIN_EDITOR_TOOL_NAME); };
...
//////////////////////////////////////////////////////////////////////////
bool CMannequinDialog::LoadPreviewFile(tukk filename, XmlNodeRef& xmlSequenceNode)
{
	...
	if (xmlData && (strcmp(xmlData->getTag(), "MannequinPreview") == 0))
	{
		...
		if (!xmlControllerDef)
		{
			DrxLog("[CMannequinDialog::LoadPreviewFile] <ControllerDef> missing in %s", filename);
			return false;
		}
		...
		if (!controllerDef)
		{
			DrxLog("[CMannequinDialog::LoadPreviewFile] Invalid controllerDef file %s", xmlControllerDef->getAttr("filename"));
			return false;
		}

		u32k numContexts = xmlContexts ? xmlContexts->getChildCount() : 0;
		u32k numScopes = controllerDef->m_scopeIDs.GetNum();
		...
		for (u32 i = 0; i < numContexts; i++)
		{
			...
			for (u32 j = 0; j < eMEM_Max; j++)
			{
				contextDef.viewData[j].enabled = contextDef.startActive;
			}
			tukk databaseFilename = xmlContext->getAttr("database");
			...
			contextDef.dataID = i;
			tukk contextData = xmlContext->getAttr("context");
			...
			else
			{
				...
				{
					DrxLog("[CMannequinDialog::LoadPreviewFile] Invalid preview file %s, couldn't find context scope %s in controllerDef %s", filename, xmlContext->getAttr("context"), xmlControllerDef->getAttr("filename"));
					return false;
				}
			}
			contextDef.changeCount = 0;
			tukk tagList = xmlContext->getAttr("tags");
			controllerDef->m_tags.TagListToFlags(tagList, contextDef.tags);
			contextDef.fragTags = xmlContext->getAttr("fragtags");
			tukk modelFName = xmlContext->getAttr("model");

			tukk controllerDefFilename = xmlContext->getAttr("controllerDef");
			if (controllerDefFilename && controllerDefFilename[0])
			{
				...
				{
					DrxLog("[CMannequinDialog::LoadPreviewFile] Invalid controllerDef file %s", controllerDefFilename);
					return false;
				}
			}
			tukk slaveDatabaseFilename = xmlContext->getAttr("slaveDatabase");
			if (slaveDatabaseFilename && slaveDatabaseFilename[0])
			{
				...
				{
					DrxLog("[CMannequinDialog::LoadPreviewFile] Invalid slave database file %s", slaveDatabaseFilename);
					return false;
				}
			}
			...
			{
				tukk ext = PathUtil::GetExt(modelFName);
				...
				tukk attachment = xmlContext->getAttr("attachment");
				contextDef.attachment = attachment;

				tukk attachmentContext = xmlContext->getAttr("attachmentContext");
				contextDef.attachmentContext = attachmentContext;

				tukk attachmentHelper = xmlContext->getAttr("attachmentHelper");
		...
		if (xmlBackgroundNode)
		{
			...
			{
				u32k numProps = xmlBackgroundProps->getChildCount();
...
//////////////////////////////////////////////////////////////////////////
bool CMannequinDialog::SavePreviewFile(tukk filename)
{
	IMannequin& mannequinSys = gEnv->pGameFramework->GetMannequinInterface();
	...
	for (u32 i = 0; i < numContexts; i++)
	{
		...
		tukk contextDataName = (contextDef.contextID == CONTEXT_DATA_NONE) ? "" : pControllerDef->m_scopeContexts.GetTagName(contextDef.contextID);
		contextDefNode->setAttr("context", contextDataName);

		DrxStackStringT<char, 512> sTags;
	...
	if (m_contexts.backGroundObjects)
	{
		...
		u32 numProps = m_contexts.backgroundProps.size();
		if (numProps > 0)
		{
...
void CMannequinDialog::ClearContextData(const EMannequinEditorMode editorMode, i32k scopeContextID)
{
	m_contexts.viewData[editorMode].m_pActionController->ClearScopeContext(scopeContextID);

	u32k numScopes = m_contexts.m_scopeData.size();
	SScopeContextData* pCurData = NULL;
	for (u32 s = 0; s < numScopes; s++)
	{
	...
	if (pCurData)
	{
		...
		{
			u32 parentContextDataID = 0;

			u32 parentContextID = 0;
...
bool CMannequinDialog::EnableContextData(const EMannequinEditorMode editorMode, i32k scopeContextDataID)
{
	u32k numContexts = m_contexts.m_contextData.size();
		...
		if (contextData.dataID == scopeContextDataID)
		{
			if (!contextData.viewData[editorMode].enabled)
			{
				...
				if (contextData.attachment.length() > 0)
				{
					...
					{
						u32k parentContextID = m_contexts.m_controllerDef->m_scopeContexts.Find(contextData.attachmentContext.c_str());
...
bool CMannequinDialog::InitialiseToPreviewFile(tukk previewFile)
{
	...
	if (loadedSetup)
	{
		...
		tukk PAGE_POSTFIX[eMEM_Max] = { "", "_Prv", "_Trn" };

		for (u32 em = 0; em < eMEM_Max; em++)
		{
			...
			assert(pEntityClass);
			for (u32 i = 0; i < m_contexts.m_contextData.size(); i++)
			{
			...
			u32k numContextData = m_contexts.m_contextData.size();
			u32 installedContexts = 0;
			for (u32 i = 0; i < numContextData; i++)
			{
				SScopeContextData& contextData = m_contexts.m_contextData[i];
				u32k contextFlag = (contextData.contextID == CONTEXT_DATA_NONE) ? 0 : (1 << contextData.contextID);
				u32k numScopes = m_contexts.m_scopeData.size();
				for (u32 s = 0; s < numScopes; s++)
				{
			...
			if (m_contexts.backGroundObjects)
			{
				...
				ar.ResolveObjects();
				i32k numObjects = ar.GetLoadedObjectsCount();
					...
					if (pObj)
					{
						...
						if (!pObj->GetParent())
						{
							...
							if (pObj->IsKindOf(RUNTIME_CLASS(CEntityObject)))
							{
								...
								u32k numProps = m_contexts.backgroundProps.size();
								for (u32 i = 0; i < numProps; i++)
								{
...
void CMannequinDialog::LoadNewPreviewFile(tukk previewFile)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Can't load preview file '%s' while game mode simulation is active!", previewFile);
		return;
	}
...
BOOL CMannequinDialog::OnInitDialog()
{
	...
	i32 tagsPanelIndex = m_wndFragmentsRollup.InsertPage("No Fragment Selected", m_wndTagsPanel);
	RC_PAGEINFO* pTagsPanelInfo = m_wndFragmentsRollup.GetPageInfo(tagsPanelIndex);
	LOGFONT logFont;
	memset(&logFont, 0, sizeof(logFont));
	logFont.lfHeight = 16;
	logFont.lfWeight = FW_BOLD;
	drx_strcpy(logFont.lfFaceName, "MS Shell Dlg 2");
...

	{
		...
		tukk szDefaultPreviewFile = NULL;
		...
		LoadNewPreviewFile(PathUtil::GamePathToDrxPakPath(szDefaultPreviewFile));
...
{
	string xmlLocation = gEnv->pDrxPak->GetAlias("%USER%");
...
void CMannequinDialog::OnMoveLocator(EMannequinEditorMode editorMode, u32 refID, tukk locatorName, const QuatT& transform)
{
...
void CMannequinDialog::EditFragmentByKey(const CFragmentKey& key, i32 contextID)
{
...
void CMannequinDialog::EditFragment(FragmentID fragID, const SFragTagState& tagState, i32 contextID)
{
...
void CMannequinDialog::EditTransitionByKey(const struct CFragmentKey& key, i32 contextID)
{
...
void CMannequinDialog::OnSave()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_COMMENT, "There are no modified files");
	}
	else
	{
...
void OnErrorCallback(const SMannequinErrorReport& errorReport, CMannErrorRecord::ESeverity errorSeverity, uk _context)
{
...
void ErrorCallback(const SMannequinErrorReport& errorReport, uk _context)
{
	OnErrorCallback(errorReport, CMannErrorRecord::ESEVERITY_ERROR, _context);
}

void WarningCallback(const SMannequinErrorReport& errorReport, uk _context)
{
...
//////////////////////////////////////////////////////////////////////////
void CMannequinDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CMannequinDialog::FindClipReferences(const CClipKey& key)
{
	...
	i32 startIndex = icafName.ReverseFind('/') + 1;
...
void CMannequinDialog::UpdateForFragment()
{
	...
	else
	{
		...
		tukk fragmentName = m_contexts.m_controllerDef->m_fragmentIDs.GetTagName(fragmentIDs[0]);
		CString title;
		if (!tags.empty())
		...
		title = "";
		tukk prevFragmentName = nullptr;
		for (size_t i = 0; i < fragmentIDs.size() && i < tags.size() && i < 3; ++i)
		{
			if (i != 0)
				title += ", ";

			tukk fragmentName = m_contexts.m_controllerDef->m_fragmentIDs.GetTagName(fragmentIDs[i]);
...
void CMannequinDialog::OnMenuLoadPreviewFile()
{
	...
	tukk setupFileFilter = "Setup XML Files (*.xml)|*.xml";
...
{
	DRX_ASSERT(FragmentBrowser());
	i32k scopeContextId = FragmentBrowser()->GetScopeContextId();
...
{
	for (u32 editorModeId = 0; editorModeId < eMEM_Max; ++editorModeId)
	{
...
//////////////////////////////////////////////////////////////////////////
void CMannequinDialog::SetKeyProperty(tukk propertyName, tukk propertyValue)
{
	if (m_wndFragmentEditorPage.KeyProperties())
	{
...
void CMannequinDialog::SetTagStateOnCtrl()
{
	...
	const std::vector<FragmentID>& fragmentIDs = FragmentBrowser()->GetFragmentIDs();
	DRX_ASSERT(fragmentIDs.size());
	DRX_ASSERT(m_tagControlsVec.size() == fragmentIDs.size());
	DRX_ASSERT(m_fragTagControlsVec.size() == fragmentIDs.size());
...
void OnAnimAssetReport(const SAnimAssetReport& animAssetReport, uk const pCallbackData)
{
	...
	DynArray<i32> subAnimIDs;
	pCollector->animSet.GetSubAnimations(subAnimIDs, animAssetReport.animID);
	u32k numSubAnims = subAnimIDs.size();
	for (u32 itSubAnims = 0; itSubAnims < numSubAnims; ++itSubAnims)
	{
		i32k subAnimID = subAnimIDs[itSubAnims];
...
void ListUsedAnimations(FILE* pFile, const SMannequinContexts& contexts)
{
	...
	for (size_t contextDataIndex = 0; contextDataIndex < scopeContextDataCount; ++contextDataIndex)
	{
		...
		for (size_t i = 0; i < animAssetCollector.assetList.size(); ++i)
		{
			...
			tukk const modelPath = pCharacterInstance->GetIDefaultSkeleton().GetModelFilePath();
			fprintf(pFile, "%s,", modelPath);

			tukk const animEventsFile = pCharacterInstance->GetModelAnimEventDatabase();
			fprintf(pFile, "%s,", animEventsFile ? animEventsFile : "");

			tukk const animationDatabasePath = pAnimationDatabase->GetFilename();
			fprintf(pFile, "%s,", animationDatabasePath);

			tukk const fragmentName = scopeContextData.database->GetFragmentDefs().GetTagName(animAssetReport.fragID);
			...
			tukk const fragmentNameTo = animAssetReport.fragIDTo != FRAGMENT_ID_INVALID ? scopeContextData.database->GetFragmentDefs().GetTagName(animAssetReport.fragIDTo) : "";
...
void CMannequinDialog::OnToolsListUsedAnimations()
{
	...
	FILE* pFile = fopen(static_cast<tukk >(outputFilename), "wt");
	if (pFile == NULL)
	{
		return;
	}
...
void CMannequinDialog::EnableMenuCommand(u32 commandId, bool enable)
{
