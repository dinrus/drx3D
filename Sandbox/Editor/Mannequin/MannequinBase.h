...
u32k HISTORY_ITEM_INVALID = 0xffffffff;
u32k CONTEXT_DATA_NONE = 0xffffffff;
...
struct SScopeContextData
{
	...
	bool CreateCharInstance(tukk modelName)
	{
		for (u32 i = 0; i < eMEM_Max; i++)
		{
	...
	bool CreateStatObj(tukk modelName)
	{
		for (u32 i = 0; i < eMEM_Max; i++)
		{
			SScopeContextViewData& data = viewData[i];
...
struct SMannequinContextViewData
{
	...
	std::vector<i32>                backgroundProps;
};
...
struct SMannequinContexts
{
	...
	SScopeContextData*       GetContextDataForID(const SFragTagState& fragTagState, FragmentID fragID, u32 contextID, EMannequinEditorMode editorMode);
	SScopeContextData*       GetContextDataForID(TagState state, u32 contextID, EMannequinEditorMode editorMode);

	const SScopeContextData* GetContextDataForID(TagState state, u32 contextID, EMannequinEditorMode editorMode) const
	{
...
public:
	...
	void LoadSequence(tukk sequence);
	void LoadSequence(XmlNodeRef root);
	void LoadSequence(i32k scopeContextIdx, const FragmentID fromID, const FragmentID toID, const SFragTagState fromFragTag, const SFragTagState toFragTag, const SFragmentBlendUid blendUid, const TagState& tagState);
	void SaveSequence(tukk filename);
	...
	struct SHistoryItem
	{
		...
		SHistoryItem(tukk _paramName, const SMannParameter& _mannParam)
			:
			time(-1.0f),
			type(Param),
			loadedScopeMask(0),
...
void                     GetFragmentFromClipTracks(CFragment& fragment, CSequencerNode* animNode, u32 historyNode = HISTORY_ITEM_INVALID, float fragStartTime = 0.0f, i32 fragPart = 0);
...
void                     FlagsToTagList(tuk tagList, u32 bufferSize, const SFragTagState& tagState, FragmentID fragID, const SControllerDef& def, tukk emptyString = NULL);
...
void                     LoadSplitterFromXml(const XmlNodeRef& xmlLayout, const CString& name, CSplitterWnd& splitter, i32 min = 0);
...
public:
	...
	{
		for (u32 i = 0; i < eMotionParamID_COUNT; i++)
		{
			m_locked[i] = false;
		}
	}
