...
static i32k TRANSITION_CHAR_BUFFER_SIZE = 512;
};
...
BOOL CTransitionEditorPage::OnInitDialog()
{
	...
	tukk SEQUENCE_NAME = "Transition Preview";
...
void CTransitionEditorPage::OnPlayMenu(CPoint pos)
{
	...
	i32 nScales = sizeof(Scales) / sizeof(SScales);
	CMenu menu;
	menu.CreatePopupMenu();
	for (i32 i = 0; i < nScales; i++)
	{
		i32 flags = 0;
	...
	i32 cmd = CXTPCommandBars::TrackPopupMenu(&menu,
...
//////////////////////////////////////////////////////////////////////////
float CTransitionEditorPage::PopulateClipTracks(CSequencerNode* node, i32k scopeID)
{
	struct SSelectedKey
	{
		SSelectedKey(ESequencerParamType trackParamType, i32 layerTrackIdx, i32 keyIdx)
		...
		i32                 m_layerTrackIdx; // index of track of type m_trackParamType (indexing of each type is separated)
		i32                 m_keyIdx;
	};

	typedef std::vector<SSelectedKey> TSelectedKeys;
	TSelectedKeys selectedKeys;
	i32 layerTrackCount[SEQUENCER_PARAM_TOTAL];
	...
	for (u32 l = 0; l < node->GetTrackCount(); )
	{
		...
		case SEQUENCER_PARAM_PROCLAYER:
			for (i32 keyIdx = track->GetNumKeys() - 1; keyIdx >= 0; --keyIdx)
			{
	...
	i32k numPropKeysInitial = propsTrack->GetNumKeys();
	for (i32 i = numPropKeysInitial - 1; i >= 0; i--)
	{
	...
	u32k contextID = scope->GetContextID();
	...
	for (u32 h = 0; h < m_fragmentHistory->m_history.m_items.size(); h++)
	{
		...
		else if (item.type == CFragmentHistory::SHistoryItem::Fragment)
		{
			...
			{
				i32k numFragKeys = fragTrack->GetNumKeys();
				for (i32 i = 0; i < numFragKeys; i++)
				{
				...
				{
					float nextTime = -1.0f;
					for (u32 n = h + 1; n < m_fragmentHistory->m_history.m_items.size(); n++)
					{
					...
					if (rootScope != scopeID)
					{
						if (additionalTags == TAG_STATE_EMPTY)
						{
							...
							u32k numScopes = pActionController->GetTotalScopes();
							u32k contextID = pActionController->GetScope(scopeID)->GetContextID();
							for (u32 i = 0; (i < scopeID) && primaryInstall; i++)
							{
								u32k altContextID = pActionController->GetScope(i)->GetContextID();
					...
					if (pSeqTrack)
					{
						i32k numKeys = pSeqTrack->GetNumKeys();
						if (numKeys > 0)
						{
					...
					{
						u32k numClips = fragData.animLayers[0].size();
					...
					{
						u32k numClips = fragData.procLayers[0].size();
						for (u32 c = 0; c < numClips; c++)
						{
					...
					if (blend1.pFragmentBlend)
					{
						...
						i32k id = propsTrack->CreateKey(transitionTime);
						...
						i32k idSelect = propsTrack->CreateKey(blendSelectTime);
						...
						i32k idStart = propsTrack->CreateKey(cycleIncrement + lastTime + lastFragmentDuration + transitionStartTime);
	...
	{
		i32 layerTrackCount = 0;
		for (i32 l = 0; l < node->GetTrackCount(); ++l)
		{
...
void CTransitionEditorPage::PopulateAllClipTracks()
{
	...
	u32 numScopes = m_contexts->m_scopeData.size();
	float maxTime = TRANSITION_MIN_TIME_RANGE;
	...
	u32k numHistoryItems = history.m_items.size();
	for (u32 h = 0; h < numHistoryItems; h++)
	{
...
{
	for (u32 h = 0; h < m_contextHistory->m_history.m_items.size(); h++)
	{
...
void CTransitionEditorPage::SetUIFromHistory()
{
	...
	if (m_contexts->m_controllerDef)
	{
		...
		u32k numHistoryItems = m_fragmentHistory->m_history.m_items.size();
		for (u32 h = 0; h < numHistoryItems; h++)
		{
			...
			if (item.type == CFragmentHistory::SHistoryItem::Tag)
			{
				...
				i32 newKeyID = tagTrack->CreateKey(time);
				CTagKey newKey;
				newKey.m_time = time;
			...
			{
				float time = item.time - m_fragmentHistory->m_history.m_firstTime;
				i32 newKeyID = paramTrack->CreateKey(time);
				CParamKey newKey;
				newKey.m_time = time;
...
void CTransitionEditorPage::OnUpdateTV(bool forceUpdate)
{
	...
	for (u32 i = 0; i < numScopes; i++)
	{
		u32 clipChangeCount = 0;
		u32 fragChangeCount = 0;
		u32 propChangeCount = 0;
		...
		{
			u32k numTracks = scopeData.animNode[eMEM_TransitionEditor]->GetTrackCount();
		...
		{
			std::vector<i32> overriddenBlends;

			i32k numKeys = pPropsTrack->GetNumKeys();
				...
				if (blend)
				{
					...
					if (propChangeCount)
					{
						if (key.overrideProps)
						{
							...
							{
								blendCopy.startTime = key.overrideStartTime - (float)(i32)key.overrideStartTime;
							}
							else
							{
								blendCopy.startTime = min(0.f, key.overrideStartTime);
							}
							DrxLogAlways("Set times from GUI: %f %f", blendCopy.selectTime, blendCopy.startTime);
						}
						else if (key.prop == CTransitionPropertyKey::eTP_Select)
						{
							...
							if (key.tranFlags & SFragmentBlend::Cyclic)
							{
								...
								DrxLogAlways("Pre-clip select time: %f", blendCopy.selectTime);
								blendCopy.selectTime -= (float)(i32)blendCopy.selectTime;
								DrxLogAlways("Set select time from key %f (%f - %f) / %f", blendCopy.selectTime, key.m_time, key.prevClipStart, key.prevClipDuration);
							}
							else
							{
								blendCopy.selectTime = key.m_time - key.prevClipStart;
								DrxLogAlways("Set select time from key %f (%f - %f)", blendCopy.selectTime, key.m_time, key.prevClipStart);
							}
						}
						else if (key.prop == CTransitionPropertyKey::eTP_Start)
						{
							...
							if (key.tranFlags & SFragmentBlend::Cyclic)
							{
								...
								DrxLogAlways("Pre-clip start time: %f", blendCopy.startTime);
								blendCopy.startTime -= (float)(i32)blendCopy.startTime;
								DrxLogAlways("Set start time from key %f (%f - %f) / %f", blendCopy.startTime, key.refTime, key.m_time, key.prevClipDuration);
							}
							else
							{
								blendCopy.startTime = min(0.0f, key.m_time - key.refTime);
								DrxLogAlways("Set start time from key %f (%f - %f)", blendCopy.startTime, key.m_time, key.refTime);
							}
						}
					}
		...
		{
			i32k numKeys = pFragTrack->GetNumKeys();
			float lastTime = 0.0f;
			for (u32 k = 0; k < numKeys; k++)
			{
				CFragmentKey fragKey;
				...
				{
					DRX_ASSERT(fragKey.context);

					bool alreadyDoneTransition = false;
					if (fragKey.sharedID != 0)
					{
						for (u32 s = 0, mask = 1; s < i; s++, mask <<= 1)
						{
					...
					i32 prevKeyID = pFragTrack->GetPrecedingFragmentKey(k, true);
					...
					if (pFragmentBlend)
					{
						...
						if (hasPrevClip && !alreadyDoneTransition)
						{
							...
							if (fragKey.tranFlags & SFragmentBlend::Cyclic)
							{
								clipSelectTime = (fragKey.tranSelectTime - fragKey.tranLastClipEffectiveStart) / fragKey.tranLastClipDuration;
								clipStartTime = (tranStartTime - fragKey.tranLastClipEffectiveStart) / fragKey.tranLastClipDuration;
								clipSelectTime -= (float)(i32)clipSelectTime;
								clipStartTime -= (float)(i32)clipStartTime;
							}
							else
							{
	...
	u32k numNodes = m_sequence->GetNodeCount();
	for (u32 i = 0; i < numNodes; i++)
	{
		CSequencerNode* seqNode = m_sequence->GetNode(i);
		u32k numTracks = seqNode->GetTrackCount();
		for (u32 t = 0; t < numTracks; t++)
		{
...
void CTransitionEditorPage::LoadSequence(i32k scopeContextIdx, const FragmentID fromID, const FragmentID toID, const SFragTagState fromFragTag, const SFragTagState toFragTag, const SFragmentBlendUid blendUid)
{
	m_fromID = fromID;
	m_toID = toID;
...
void CTransitionEditorPage::PopulateTagList()
{
	...
	u32k numScopes = m_contexts->m_controllerDef->m_scopeDef.size();
	...
	const CFragmentHistory& history = m_fragmentHistory->m_history;
	u32k numItems = history.m_items.size();
...
void CTransitionEditorPage::OnInternalVariableChange(IVariable* pVar)
{
	if (!m_bRefreshingTagCtrl)
	{
		...
		u32k numContexts = m_contexts->m_controllerDef->m_scopeContexts.GetNum();
