...
void CFragmentBrowser::SetContext(SMannequinContexts& context)
{
	...
	u32k numContexts = context.m_contextData.size();
...
void CFragmentBrowser::RebuildAll()
{
	...
	for (u32 i = 0; i < m_fragmentData.size(); i++)
	{
...
HTREEITEM CFragmentBrowser::FindFragmentItem(FragmentID fragmentID, const SFragTagState& tagState, u32 option) const
{
	for (u32 i = 0; i < m_fragmentData.size(); i++)
	{
...
bool CFragmentBrowser::SelectFragments(const std::vector<FragmentID> fragmentIDs, const std::vector<SFragTagState>& tagStates, const std::vector<u32>& options)
{
	bool bRet = false;
	std::vector<HTREEITEM> newBoldItems;
	std::vector<FragmentID> newFragmentIDs;
	std::vector<SFragTagState> newTagStates;
	std::vector<u32> newOptions;

	m_TreeCtrl.SelectAll(FALSE);
	for (size_t i = 0; i < fragmentIDs.size(); ++i)
	{
		SFragTagState bestTagState;
		u32 numOptions = m_animDB->FindBestMatchingTag(SFragmentQuery(fragmentIDs[i], tagStates[i]), &bestTagState);
		if (numOptions > 0)
		{
			u32 option = options[i] % numOptions;
...
bool CFragmentBrowser::SelectFragment(FragmentID fragmentID, const SFragTagState& tagState, u32 option)
{
...
IAnimationDatabase* CFragmentBrowser::FindHostDatabase(u32 contextID, const SFragTagState& tagState) const
{
...
bool CFragmentBrowser::SetTagState(const std::vector<SFragTagState>& newTagsVec)
{
	...
	for (i32 i = m_options.size() - 2; i >= 0; --i)
	{
		inds.push_back(i);
	...
	if (m_rightDrag)
	{
		...
		if (hitDest != NULL)
		{
			...
			if (fragmentDataFrom)
			{
				...
				IMannequin& mannequinSys = gEnv->pGameFramework->GetMannequinInterface();
				u32 newOption = mannequinSys.GetMannequinEditorManager()->AddFragmentEntry(m_animDB, fragID, newTagState, *cloneFrag);
...
void CFragmentBrowser::OnTVRightClick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	CPoint p;
	::GetCursorPos(&p);
	i32 res = ::TrackPopupMenuEx(menu.GetSafeHmenu(), TPM_LEFTBUTTON | TPM_RETURNCMD, p.x, p.y, GetSafeHwnd(), NULL);
...
void CFragmentBrowser::OnMenuCopy()
{
	...
	if (!pFragmentDataCopy || pFragmentDataCopy->tagSet)
	{
		...
		u32k numTagSets = m_animDB->GetTotalTagSets(fragmentID);
		for (u32 itTagSets = 0; itTagSets < numTagSets; ++itTagSets)
		{
			SFragTagState entryTagState;
			u32k numOptions = m_animDB->GetTagSetInfo(fragmentID, itTagSets, entryTagState);
	...
	if (!m_copiedItems.empty())
	{
		...
		for (TCopiedItems::const_iterator itCopiedItems = m_copiedItems.begin(); itCopiedItems != m_copiedItems.end(); ++itCopiedItems)
		{
			if (destFragmentID != FRAGMENT_ID_INVALID)
			{
				...
				u32k newOption = mannequinSys.GetMannequinEditorManager()->AddFragmentEntry(m_animDB, destFragmentID, insertionTagState, itCopiedItems->fragment);
...
void CFragmentBrowser::SetEditItem(const std::vector<HTREEITEM>& boldItems, std::vector<FragmentID> fragmentIDs, const std::vector<SFragTagState>& tagStates, const std::vector<u32>& options)
{
...
void CFragmentBrowser::SetADBFileNameTextToCurrent()
{
	...
	if (rootItem)
	{
		...
		tukk filename = m_animDB == NULL ? "" : m_animDB->FindSubADBFilenameForID(m_animDB->GetFragmentID(IDName.GetString()));
...
void CFragmentBrowser::SetScopeContext(i32 scopeContextID)
{
	const bool rangeValid = (0 <= scopeContextID) && (scopeContextID < m_context->m_contextData.size());
	i32k modifiedScopeContextID = rangeValid ? scopeContextID : -1;
...
void CFragmentBrowser::OnDeleteBtn()
{
	...
	{
		CString message;
		tukk fragmentName = m_animDB->GetFragmentDefs().GetTagName(selectedFragmentId);
		message.Format("Delete the fragment '%s [%s]'?", fragmentName, GetSelectedNodeText().GetBuffer());

		if (MessageBox(message.GetBuffer(), "DrxMannequin", MB_YESNO | MB_ICONQUESTION) == IDYES)
		{
...
{
	//--- Remove old data
	i32k numFragDatas = m_fragmentData.size();
	for (i32 i = numFragDatas - 1; i >= 0; i--)
	{
		STreeFragmentData* treeFragData = m_fragmentData[i];
		if (treeFragData->fragID == fragID)
		{
...
HTREEITEM CFragmentBrowser::FindInChildren(tukk szTagStr, HTREEITEM hCurrItem)
{
...
{
	typedef std::pair<i32, string> TTagPair;
...
void CFragmentBrowser::BuildFragment(FragmentID fragID)
{
	...
	u32k numFilters = m_filters.size();

	//--- Insert new data
	char szTagStateBuff[512];
	u32k numTagSets = m_animDB->GetTotalTagSets(fragID);
	for (u32 t = 0; t < numTagSets; t++)
	{
		SFragTagState tagState;
		u32 numOptions = m_animDB->GetTagSetInfo(fragID, t, tagState);
		...
		if (m_filterAnimClipText.GetLength() > 0)
		{
			for (u32 o = 0; o < numOptions; o++)
			{
				...
				for (u32 ali = 0; ali < numALayers; ali++)
				{
		...
		for (u32 o = 0; o < numOptions; o++)
		{
			...
			{
				drx_sprintf(subFragNameBuffer, "Option %d", o + 1);
...
u32 CFragmentBrowser::AddNewFragment(FragmentID fragID, const SFragTagState& newTagState, const string& sAnimName)
{
	IMannequin& mannequinSys = gEnv->pGameFramework->GetMannequinInterface();

	ActionScopes scopeMask = m_context->m_controllerDef->GetScopeMask(fragID, newTagState);
	u32 numScopes = m_context->m_controllerDef->m_scopeDef.size();
	u32 newOption = OPTION_IDX_INVALID;

	// Add fragment on root scope (only)
	for (u32 itScopes = 0; itScopes < numScopes; ++itScopes)
	{
...
void CFragmentBrowser::OnNewBtn()
{
	if (HTREEITEM item = m_TreeCtrl.GetSelectedItem())
	{
		...
		IMannequin& mannequinSys = gEnv->pGameFramework->GetMannequinInterface();
		u32 newOption = mannequinSys.GetMannequinEditorManager()->AddFragmentEntry(m_animDB, fragID, newTagState, CFragment());
...
void CFragmentBrowser::OnDeleteDefinitionBtn()
{
	...
	// Warn and bail out if a sub ADB rule uses this fragmentID
	{
		...
		tukk fragmentDefFilename = m_animDB->GetFragmentDefs().GetFilename();
		...
		for (const IAnimationDatabase* pDatabase : databases)
		{
			...
			tukk databaseFragmentDefFilename = pDatabase->GetFragmentDefs().GetFilename();
		...
		if (!foundFragmentIDRules.empty())
		{
			...
			MessageBox(message.GetBuffer(), "DrxMannequin", MB_OK | MB_ICONINFORMATION);
			return;
		}
	}
...
FragmentID CFragmentBrowser::GetValidFragIDForAnim(const CPoint& point, COleDataObject* pDataObject)
{
	...
	string animName = static_cast<tuk>(GlobalLock(hData));
	GlobalUnlock(hData);
	...
	for (; itCtx != m_context->m_contextData.end(); ++itCtx)
	{
		...
		i32 nAnimID = ctxData.animSet->GetAnimIDByName(animName);
		if (-1 != nAnimID)
		{
			break;
		}
