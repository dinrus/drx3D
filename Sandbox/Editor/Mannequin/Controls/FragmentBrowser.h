...
public:
	...
	void SetScopeContext(i32 scopeContextID);
	bool SelectFragments(const std::vector<FragmentID> fragmentIDs, const std::vector<SFragTagState>& tagStates, const std::vector<u32>& options);
	bool SelectFragment(FragmentID fragmentID, const SFragTagState& tagState, u32 option = 0);
...
protected:
	...
	void         SetEditItem(const std::vector<HTREEITEM>& boldItems, std::vector<FragmentID> fragmentIDs, const std::vector<SFragTagState>& tagStates, const std::vector<u32>& options);
	void         SetDatabase(IAnimationDatabase* animDB);

	HTREEITEM    FindFragmentItem(FragmentID fragmentID, const SFragTagState& tagState, u32 option) const;

	void         CleanFragmentID(FragmentID fragID);
	void         CleanFragment(FragmentID fragID);
	void         BuildFragment(FragmentID fragID);
	HTREEITEM    FindInChildren(tukk szTagStr, HTREEITEM parent);
	...
	IAnimationDatabase* FindHostDatabase(u32 contextID, const SFragTagState& tagState) const;
	...
	u32       AddNewFragment(FragmentID fragID, const SFragTagState& newTagState, const string& sAnimName);
private:
