...
BOOL CTransitionBrowser::OnInitDialog()
{
	...
	// This is a bit nasty - as tree control is created at runtime we can't set the tab order correctly through DrxEdit.rc
...
void CTransitionBrowser::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CTransitionBrowser::Refresh()
{
	...
	if (m_animDB)
	{
		...
		for (SEditorFragmentBlendID::TEditorFragmentBlendIDArray::const_iterator iterBlend = blendIDs.begin(), end = blendIDs.end(); iterBlend != end; ++iterBlend)
		{
			...
			const CString cFragFromLower = DrxStringUtils::toLower(cFragFrom.GetString());
			if (m_filterFragmentIDTextFrom[0] != 0 && cFragFromLower.Find(m_filterFragmentIDTextFrom) < 0)
				continue;

			const CString cFragToLower = DrxStringUtils::toLower(cFragTo.GetString());
			...
			{
				u32k numBlends = m_animDB->GetNumBlends(
				  (*iterBlend).fragFrom,
				  (*iterBlend).fragTo,
				...
				if (numFromFilters > 0)
				{
					...
					sTagState.MakeLower();
					for (u32 f = 0; f < numFromFilters; f++)
					{
				...
				for (i32 blendIdx = 0; blendIdx < numBlends; ++blendIdx)
				{
...
void CTransitionBrowserTreeCtrl::SetBoldRecursive(CXTPReportRecords* pRecords, const TTransitionBrowserRecordDataCollection& recordDataCollection, const BOOL bBold)
{
	...
	for (i32 recordIndex = 0; recordIndex < pRecords->GetCount(); recordIndex++) // GetCount changes whenever a row gets expanded!
	{
		...
		{
			for (i32 itemIndex = 0; itemIndex < pRecord->GetItemCount(); ++itemIndex)
			{
...
void SetBoldIncludingParents(CXTPReportRecord* pRecord, BOOL bBold = TRUE)
{
	while (pRecord)
	{
		for (i32 i = 0; i < pRecord->GetItemCount(); ++i)
		{
...
void CTransitionBrowser::SetScopeContextDataIdx(i32 scopeContextDataIdx)
{
...
void CTransitionBrowser::DeleteSelected()
{
	...
	{
		i32k result = MessageBox("Are you sure you want to delete the selected transition?", "Delete Transition", MB_YESNO | MB_ICONQUESTION);
		if (IDYES != result)
			return;
...
{
	fragSearchStr = DrxStringUtils::toLower(fragSearchStr.GetString());
...
{
	tagSearchStr = DrxStringUtils::toLower(tagSearchStr.GetString());
...
{
	animSearchStr = DrxStringUtils::toLower(animSearchStr.GetString());
...
bool CTransitionBrowser::FilterByAnimClip(const FragmentID fragId, const SFragTagState& tagState, const CString& filter)
{
	...
	for (u32 tagId = 0; tagId < numTagSets; ++tagId)
	{
		SFragTagState tagSetState;
		u32 numOptions = m_animDB->GetTagSetInfo(fragId, tagId, tagSetState);

		if (tagSetState != tagState)
			continue;

		for (u32 optionId = 0; optionId < numOptions; ++optionId)
		{
			const CFragment* sourceFragment = m_animDB->GetEntry(fragId, tagId, optionId);

			if (sourceFragment)
			{
				u32 numAnimLayers = sourceFragment->m_animLayers.size();
				for (u32 layerId = 0; layerId < numAnimLayers; ++layerId)
				{
					const TAnimClipSequence& animLayer = sourceFragment->m_animLayers[layerId];
					u32k numClips = animLayer.size();
					for (u32 clipId = 0; clipId < numClips; ++clipId)
					{
...
/*
   ...
   //////////////////////////////////////////////////////////////////////////
   void CTransitionBrowserTreeCtrl::PrintRows(tukk szWhen)
   {
   DrxWarning(
...
{
	CXTPReportRows* pRows = GetRows();
	for (i32 i = 0; i < pRows->GetCount(); i++) // GetCount changes whenever a row gets expanded!
	{
		CXTPReportRow* pRow = pRows->GetAt(i);
		CTransitionBrowserRecord* pCurrentRecord = RowToRecord(pRow);
...
void CTransitionBrowserTreeCtrl::SelectItem(const TTransitionID& transitionID)
{
	...
	for (i32 i = 0, nNumRecords = pRecords->GetCount(); (i < nNumRecords) && (pRecord == NULL); ++i)
	{
...
CTransitionBrowserTreeCtrl::CRecord* CTransitionBrowserTreeCtrl::SearchRecordForTransition(CRecord* pRecord, const TTransitionID& transitionID)
{
	...
	else
	{
		...
		{
			for (i32 i = 0, numChildren = pRecord->GetChildCount(); (i < numChildren) && (pFound == NULL); ++i)
			{
...
void CTransitionBrowserRecord::CreateItems()
{
	...
	i32k nIcon = m_data.m_bFolder ? IMAGE_FOLDER : IMAGE_ITEM;
	SetIcon(nIcon);
