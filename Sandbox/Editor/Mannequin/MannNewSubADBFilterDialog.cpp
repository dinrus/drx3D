...
namespace
{
tukk adb_noDB = "<no animation database (adb)>";
tukk adb_DBext = ".adb";
};
...
void CMannNewSubADBFilterDialog::PopulateFragIDList()
{
	...
	if (m_pData)
	{
		i32 nItem = m_fragUsedIDList.GetItemCount();
	...
	if (m_animDB)
	{
		...
		i32 nItem = m_fragIDList.GetItemCount();
...
void CMannNewSubADBFilterDialog::OnOk()
{
	...
	m_pData->vFragIDs.clear();
	i32k nItems = m_fragUsedIDList.GetItemCount();
