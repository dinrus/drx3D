...
{
	char szBuf[128];
	drx_sprintf(szBuf, "%g, %g, %g", a.x, a.y, a.z);
	return szBuf;
}
}
...
void CMannContextEditorDialog::EnableControls()
{
	...
	i32k index = isValidRowSelected ? pRow->GetIndex() - pRow->GetParentRow()->GetIndex() - 1 : 0;
...
//////////////////////////////////////////////////////////////////////////
void CMannContextEditorDialog::SetFocusedRecord(u32k dataID)
{
	if (CXTPReportRow* pRow = m_wndReport.GetFocusedRow())
	{
...
u32k NUM_WHITELISTED_OBJECTS = ARRAYSIZE(OBJECT_WHITELIST);

bool IsObjectWhiteListed(tukk objectType)
{
	for (u32 i = 0; i < NUM_WHITELISTED_OBJECTS; i++)
	{
...
tukk ENTITY_WHITELIST[] =
{
	"RigidBodyEx",
	"Light",
	"Switch"
};
u32k NUM_WHITELISTED_ENTITIES = ARRAYSIZE(ENTITY_WHITELIST);

bool IsEntityWhiteListed(tukk entityClass)
{
	for (u32 i = 0; i < NUM_WHITELISTED_ENTITIES; i++)
	{
...
void StripBackgroundXML(XmlNodeRef xmlRoot)
{
	if (xmlRoot)
	{
		u32 numChildren = xmlRoot->getChildCount();
		for (i32 i = numChildren - 1; i >= 0; i--)
		{
			XmlNodeRef xmlObject = xmlRoot->getChild(i);
			tukk type = xmlObject->getAttr("Type");
			...
			{
				tukk entClass = xmlObject->getAttr("EntityClass");
...
{
	tukk setupFileFilter = "Select an object group file (*.grp)|*.grp";
	CString filename;
	...
	if (xmlData)
	{
		...
		u32k numObjects = xmlData->getChildCount();
		...
		{
			i32 curRoot = dlgImportBackground.GetCurrentRoot();
