...
BOOL CMannTagEditorDialog::OnInitDialog()
{
	...
	tukk fragmentName = m_animDB->GetFragmentDefs().GetTagName(m_fragmentID);
...
void CMannTagEditorDialog::AddFragADBInternal(const CString& ADBDisplayName, const CString& normalizedFilename)
{
	...
	i32 nIndex = m_FragFileComboBox.AddString(ADBDisplayName);
...
void CMannTagEditorDialog::InitialiseFragmentADBs()
{
	...
	tukk filename = m_animDB->FindSubADBFilenameForID(m_fragmentID);
...
void CMannTagEditorDialog::InitialiseFragmentTagsRec(const CString& baseDir)
{
	...
	intptr_t handle = pDrxPak->FindFirst(baseDir + "*.xml", &fd);
	if (handle != -1)
	{
		do
		{
...
{
	i32 nIndex = m_FragFileComboBox.GetCurSel();
	if (nIndex == CB_ERR)
		return CString();
...
void CMannTagEditorDialog::ProcessFragmentADBChanges()
{
	...
	DRX_ASSERT(pMannequinEditorManager);

	tukk const currentSubADBFragmentFilterFilename = m_animDB->FindSubADBFilenameForID(m_fragmentID);
...
{
	i32k selIndex = m_tagDefComboBox.GetCurSel();
	CString displayName;
...
void CMannTagEditorDialog::AddTagDefInternal(const CString& tagDefDisplayName, const CString& normalizedFilename)
{
	...
	i32 nIndex = m_tagDefComboBox.AddString(tagDefDisplayName);
...
{
	i32 nIndex = m_tagDefComboBox.GetCurSel();
...
void CMannTagEditorDialog::SelectTagDefByFilename(const CString& filename)
{
	...
	for (size_t i = 0; i < entriesCount; ++i)
	{
		...
		{
			i32 nIndex = m_tagDefComboBox.FindStringExact(-1, entry.displayName);
			if (nIndex != CB_ERR)
...
{
	tukk fragName = (m_fragmentID != FRAGMENT_ID_INVALID) ? m_animDB->GetFragmentDefs().GetTagName(m_fragmentID) : "NoFragment";
