...
static i32k MARGIN_FOR_MAGNET_SNAPPING = 10;
...
//////////////////////////////////////////////////////////////////////////
i32 CSequencerDopeSheetBase::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CSequencerDopeSheetBase::DrawSelectedKeyIndicators(CDC* dc)
{
	...
	for (i32 i = 0; i < selectedKeys.keys.size(); ++i)
	{
		CSequencerTrack* pTrack = selectedKeys.keys[i].pTrack;
		i32 keyIndex = selectedKeys.keys[i].nKey;
		i32 x = TimeToClient(pTrack->GetKeyTime(keyIndex));
...
void CSequencerDopeSheetBase::ComputeFrameSteps(const Range& VisRange)
{
	...
	{
		nBIntermediateTicks = i32 (nBIntermediateTicks / 2.0f);
	}
...
void CSequencerDopeSheetBase::DrawTimeLineInSeconds(CDC* dc, CRect& rc, COLORREF& lineCol, COLORREF& textCol, double step)
{
	...
	for (double t = TickSnap(timeRange.start); t <= timeRange.end + step; t += step)
	{
		...
		i32 k = pos_directed_rounding(st * m_ticksStep);
		if (k % nNumberTicks == 0)
		{
			dc->SelectObject(black);
			dc->LineTo(x, rc.bottom - 14);
			char str[32];
			drx_sprintf(str, "%g", st);
			dc->TextOut(x + 2, rc.top, str);
			dc->SelectObject(ltgray);
		}
		else
...
void CSequencerDopeSheetBase::DrawTimeline(CDC* dc, const CRect& rcUpdate)
{
	...
	{
		i32 x = TimeToClient(m_currentTime);
...
//////////////////////////////////////////////////////////////////////////
i32 CSequencerDopeSheetBase::TimeToClient(float time) const
{
	i32 x = m_leftOffset - m_scrollOffset.x + time * m_timeScale;
	return x;
}
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::RedrawItem(i32 item)
{
	CRect rc;
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::OnSize(UINT nType, i32 cx, i32 cy)
{
...
{
	i32 x = point.x - m_leftOffset + m_scrollOffset.x;
...
//////////////////////////////////////////////////////////////////////////
const CSequencerDopeSheetBase::Item& CSequencerDopeSheetBase::GetItem(i32 item) const
{
	return m_tracks[item];
}

//////////////////////////////////////////////////////////////////////////
CSequencerTrack* CSequencerDopeSheetBase::GetTrack(i32 item) const
{
...
//////////////////////////////////////////////////////////////////////////
CSequencerNode* CSequencerDopeSheetBase::GetNode(i32 item) const
{
	if (item < 0 || item >= GetCount())
		return 0;
...
//////////////////////////////////////////////////////////////////////////
i32 CSequencerDopeSheetBase::FirstKeyFromPoint(CPoint point, bool exact) const
{
	return -1;
}

//////////////////////////////////////////////////////////////////////////
i32 CSequencerDopeSheetBase::LastKeyFromPoint(CPoint point, bool exact) const
{
	return -1;
}
...
void CSequencerDopeSheetBase::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	i32 key = LastKeyFromPoint(point);
	if (key >= 0)
	{
		...
		i32 secondarySelKey;
		i32 secondarySelection = track->FindSecondarySelectionPt(secondarySelKey, t1, t2);
		...
		if (secondarySelection)
		{
			...
			i32k xPointNew = TimeToClient(track->GetSecondaryTime(key, secondarySelection));
			point.x = xPointNew;
			m_mouseDownPos = point;
			m_grabOffset = 0;
		...
		i32 nNodeCount = m_pSequence->GetNodeCount();
		for (i32 nodeID = 0; nodeID < nNodeCount; ++nodeID)
		{
			CSequencerNode* pAnimNode = m_pSequence->GetNode(nodeID);

			i32 nTrackCount = pAnimNode->GetTrackCount();
			for (i32 trackID = 0; trackID < nTrackCount; ++trackID)
			{
...
void CSequencerDopeSheetBase::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	else if (m_mouseMode == MOUSE_MODE_MOVE)
	{
		CSequencerUtils::SelectedKeys selectedKeys;
		CSequencerUtils::GetSelectedKeys(m_pSequence, selectedKeys);
		for (i32 i = 0; i < (i32)selectedKeys.keys.size(); ++i)
		{
...
bool CSequencerDopeSheetBase::IsPointValidForFragmentInPreviewDrop(const CPoint& point, COleDataObject* pDataObject) const
{
	...
	i32 key = LastKeyFromPoint(point, true);
	if (key >= 0)
	{
		return false;
	}
	...
	u32 nScopeID = pFragTrack->GetScopeData().scopeID;
...
bool CSequencerDopeSheetBase::CreatePointForFragmentInPreviewDrop(const CPoint& point, COleDataObject* pDataObject)
{
	...
	if (nullptr != pTrack)
	{
		...
		{
			RecordTrackUndo(GetItem(item));
			i32 keyID = pTrack->CreateKey(keyTime);
		...
		if (keyCreated)
		{
			...
			for (i32 trackItem = 0; trackItem < GetCount(); ++trackItem)
			{
			...
			{
				RecordTrackUndo(GetItem(item));
				i32 keyID = pTagTrack->CreateKey(keyTime);
...
bool CSequencerDopeSheetBase::IsPointValidForAnimationInLayerDrop(const CPoint& point, COleDataObject* pDataObject) const
{
	...
	string sAnimName = static_cast<tuk>(GlobalLock(hData));
	GlobalUnlock(hData);
	...
	i32 key = FirstKeyFromPoint(point, true);
	if (key >= 0)
		return false;
	...
	i32 nAnimID = pClipTrack->GetContext()->animSet->GetAnimIDByName(sAnimName);
	if (-1 == nAnimID)
	{
		return false;
	}
...
void CSequencerDopeSheetBase::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	if (track)
	{
		...
		{
			RecordTrackUndo(GetItem(item));
			i32 keyID = track->CreateKey(keyTime);
...
void CSequencerDopeSheetBase::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if (m_rcTimeline.PtInRect(point))
	{
		// Clicked inside timeline.
		// adjust markers.
		i32 nMarkerStart = TimeToClient(m_timeMarked.start);
		i32 nMarkerEnd = TimeToClient(m_timeMarked.end);
	...
	if (key >= 0)
	{
		...
		CPoint p;
		::GetCursorPos(&p);
		i32 cmd = menu.TrackPopupMenu(TPM_NONOTIFY | TPM_RETURNCMD | TPM_LEFTALIGN | TPM_LEFTBUTTON, p.x, p.y, this, NULL);
		...
		else if (cmd == CHK_OR_ADD_SOURCE_CONTROL)
		{
			...
			if (GetIEditorImpl()->IsSourceControlAvailable())
			{
				...
				for (i32 idx = 0, idxEnd = vExists.size(); idx < idxEnd; ++idx)
				{
					...
					else if (tryToGetLatestFiles)
					{
						...
						u32k attr = GetIEditorImpl()->GetSourceControl()->GetFileAttributesA(gamePath);
...
{
	u32k attr = CFileUtil::GetAttributes(filename);
...
void CSequencerDopeSheetBase::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	if (m_bCursorWasInKey == false && clip.GetTitle() == "Track view keys")
	{
...

		{
			...
			{
				i32 nNumTracksToPaste = copyNode->getChildCount();
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::DrawTrack(i32 item, CDC* dc, CRect& rcItem)
{
}
...
void CSequencerDopeSheetBase::SelectAllKeysWithinTimeFrame(const CRect& rc)
{
	...
	for (i32 k = 0; k < m_pSequence->GetNodeCount(); ++k)
	{
		CSequencerNode* node = m_pSequence->GetNode(k);

		for (i32 i = 0; i < node->GetTrackCount(); i++)
		{
...
bool CSequencerDopeSheetBase::SelectFirstKey(const ESequencerParamType type)
{
	...
	for (i32 i = 0; i < GetCount(); ++i)
	{
		...
		if (pTrack)
		{
			...
			if (isOfRequiredType)
			{
				i32k numKeys = pTrack->GetNumKeys();
				if (0 < numKeys)
				{
...
void CSequencerDopeSheetBase::DelSelectedKeys(bool bPrompt, bool bAllowUndo, bool bIgnorePermission)
{
	...
	for (i32 k = (i32)selectedKeys.keys.size() - 1; k >= 0; --k)
	{
		CSequencerUtils::SelectedKey& skey = selectedKeys.keys[k];
...
void CSequencerDopeSheetBase::CloneSelectedKeys()
{
	...
	for (size_t k = 0; k < selectedKeyTimes.size(); ++k)
	{
		...
		i32 keyIndex = skey.pTrack->FindKey(selectedKeyTimes[k]);
		assert(keyIndex > -1);
		if (keyIndex <= -1)
			continue;
		...
		if (skey.pTrack->IsKeySelected(keyIndex))
		{
			...
			i32 newKey = skey.pTrack->CloneKey(keyIndex);
...
void CSequencerDopeSheetBase::SetCurrTime(float time, bool bForce)
{
	...
	if (bChange || bForce)
	{
		i32 x1 = TimeToClient(m_currentTime);
		i32 x2 = TimeToClient(time);
		m_currentTime = time;
...
bool CSequencerDopeSheetBase::CopyPasteKeys()
{
	...
	if (!pCurrTrack)
		return false;
	for (i32 i = 0; i < (i32)vecKeysToCopy.size(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::SetKeyInfo(CSequencerTrack* track, i32 key, bool openWindow)
{
	NotifyKeySelectionUpdate();
}

//////////////////////////////////////////////////////////////////////////
bool CSequencerDopeSheetBase::FindSingleSelectedKey(CSequencerTrack*& selTrack, i32& selKey)
{
	selTrack = 0;
	selKey = 0;
	CSequencerUtils::SelectedKeys selectedKeys;
...
//////////////////////////////////////////////////////////////////////////
i32 CSequencerDopeSheetBase::GetItemRect(i32 item, CRect& rect) const
{
	if (item < 0 || item >= GetCount())
		return -1;

	i32 y = -m_scrollOffset.y;

	i32 x = 0;
	for (i32 i = 0, num = (i32)m_tracks.size(); i < num && i < item; i++)
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CSequencerDopeSheetBase::ItemFromPoint(CPoint pnt) const
{
	CRect rc;
	i32 num = GetCount();
	for (i32 i = 0; i < num; i++)
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::SetHorizontalExtent(i32 min, i32 max)
{
	m_scrollMin = min;
	m_scrollMax = max;
	m_itemWidth = max - min;
	i32 nPage = m_rcClient.Width() / 2;
	i32 sx = m_itemWidth - nPage + m_leftOffset;
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::UpdateAnimation(i32 item)
{
	m_changeCount++;
...
bool CSequencerDopeSheetBase::CopyKeys(bool bPromptAllowed, bool bUseClipboard, bool bCopyTrack)
{
	...
	i32 iSelTrackCount = (i32)selectedTracks.tracks.size();
	...
	if (bCopySelected)
	{
		...
		for (i32 k = 0; k < iSelKeyCount; ++k)
		{
			...
			if (pCopyFromTrack != skey.pTrack)
			{
				...
				i32 trackOffset = 0;
				for (i32 trackID = 0; trackID < skey.pNode->GetTrackCount(); ++trackID)
				{
	...
	else
	{
		...
		bool bFoundStart = false;
		float fAbsoluteStartTime = 0;
		for (i32 i = 0; i < iSelTrackCount; ++i)
		{
...
void CSequencerDopeSheetBase::StartDraggingKeys(CPoint point)
{
	...
	i32 mouseOverTrackID = 0;
	i32 firstKeyTrackID = 0;
	bool bMouseOverFound = false;
	bool bFirstKeyFound = false;

	for (i32 trackID = 0; trackID < mouseOverNode->GetTrackCount(); ++trackID)
	{
...
void CSequencerDopeSheetBase::DeserializeTracks(const XmlNodeRef& source)
{
	...
	for (i32 nodeID = 0; nodeID < nNodeCount; ++nodeID)
	{
		...
		i32 xmlNodeCount = animXmlNode->getChildCount();
		i32 nTrackCount = pAnimNode->GetTrackCount();
		for (i32 trackID = 0; trackID < nTrackCount; ++trackID)
		{
			CSequencerTrack* pAnimTrack = pAnimNode->GetTrackByIndex(trackID);
			...
			for (i32 keyID = nKeyCount - 1; keyID >= 0; --keyID)
			{
...
bool CSequencerDopeSheetBase::PasteKeys(CSequencerNode* pAnimNode, CSequencerTrack* pAnimTrack, float fTimeOffset)
{
	...
	if (!pAnimNode)
	{
		...
		i32 nPasteToItem = ItemFromPoint(m_mouseOverPos);
		...
		if (nPasteToItem >= 0)
		{
			...
			{
				i32 hoverTrackID = 0;
				i32 trackCount = pAnimNode->GetTrackCount();
				for (i32 trackID = 0; trackID < trackCount; ++trackID)
				{
				...
				i32 offsetTrackID = CLAMP(hoverTrackID - m_mouseMoveStartTrackOffset, 0, trackCount - 1);
...

	{
		...
		i32 numOfMouseOverType = 0;
		for (i32 i = 0; i < copyNode->getChildCount(); i++)
		{
		...
		for (i32 i = 0; i < copyNode->getChildCount(); i++)
		{
			...
			i32 mouseOverTrackId = 0;
			i32 numOfTrackType = 0;
			bool trackFound = false;
			for (i32 j = 0; j < pAnimNode->GetTrackCount(); ++j)
			{
			...
			i32 trackId = paramIndex + mouseOverTrackId;
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::ShowKeyTooltip(CSequencerTrack* pTrack, i32 nKey, i32 secondarySelection, CPoint point)
{
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::InvalidateItem(i32 item)
{
	CRect rc;
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::SelectItem(i32 item)
{
	if (item >= 0 && item < GetCount())
	{
...
//////////////////////////////////////////////////////////////////////////
bool CSequencerDopeSheetBase::IsSelectedItem(i32 item)
{
	if (item >= 0 && item < GetCount())
	{
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::OffsetKey(CSequencerTrack* track, i32k keyIndex, const float timeOffset) const
{
...
bool CSequencerDopeSheetBase::IsOkToAddKeyHere(const CSequencerTrack* pTrack, float time) const
{
	...
	for (i32 i = 0; i < pTrack->GetNumKeys(); ++i)
	{
...
void CSequencerDopeSheetBase::MouseMoveMove(CPoint point, UINT nFlags)
{
	...
	i32k key = LastKeyFromPoint(m_mouseDownPos);
	if ((key >= 0) && (!m_secondarySelection))
	{
		i32 item = ItemFromPoint(m_mouseDownPos);
		CSequencerTrack* track = GetTrack(item);
		node = GetNode(item);
...
i32 CSequencerDopeSheetBase::GetAboveKey(CSequencerTrack*& aboveTrack)
{
...
i32 CSequencerDopeSheetBase::GetBelowKey(CSequencerTrack*& belowTrack)
{
	CSequencerTrack* track;
	i32 keyIndex;
	if (FindSingleSelectedKey(track, keyIndex) == false)
...
i32 CSequencerDopeSheetBase::GetRightKey(CSequencerTrack*& track)
{
	i32 keyIndex;
	if (FindSingleSelectedKey(track, keyIndex) == false)
		return -1;
...
i32 CSequencerDopeSheetBase::GetLeftKey(CSequencerTrack*& track)
{
	i32 keyIndex;
	if (FindSingleSelectedKey(track, keyIndex) == false)
		return -1;
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::ShowKeyPropertyCtrlOnSpot(i32 x, i32 y, bool bMultipleKeysSelected, bool bKeyChangeInSameTrack)
{
...
//////////////////////////////////////////////////////////////////////////
void CSequencerDopeSheetBase::SetScrollOffset(i32 hpos)
{
