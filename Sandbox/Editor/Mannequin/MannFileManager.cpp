...
public:
	CXTPMannFileRecord(const SFileEntry& fileEntry)
		...
		, m_bReadOnly(false)
	{
		m_fullFileName = PathUtil::GamePathToDrxPakPath(m_fileEntry.filename.GetString(), true);
	...
	void              UpdateState()
	{
		i32 nIcon = STATUSICON_OK;
		u32 attr = CFileUtil::GetAttributes(m_fullFileName);
	...
	void CheckOut()
	{
		...
		else
		{
			...
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, message);
		}
	}
...
void CMannFileManager::OnUndoSelection()
{
	...
	u32k numDels = undoSelection.size();
	for (i32 i = 0; i < numDels; i++)
	{
...
void CMannFileManager::OnSaveFiles()
{
	...
	if (filesInPaks.GetLength() > 0)
	{
		...
		if (CQuestionDialog::SQuestion("DrxMannequin", message.GetBuffer()) != QDialogButtonBox::Yes)
			return;
	}
