...
private:
	...
	CSmartVariable<i32>         mv_jointMask;
...
bool SynchFlag(i32& dest, i32 flag, IVariable* pVar, CSmartVariable<bool>& flagVar)
{
...
void CAnimClipUIControls::OnUIChange(IVariable* pVar, SelectedKeys& selectedKeys)
{
	if (!m_settingUp)
	{
		for (i32 keyIndex = 0, num = (i32)selectedKeys.keys.size(); keyIndex < num; keyIndex++)
		{
...
{
	bool bAssigned = false;
	u32k numScopes = m_parent->m_contexts->m_scopeData.size();
	i32k numSelKeys = selectedKeys.keys.size();
...
void CTransitionPropertyUIControls::OnUIChange(IVariable* pVar, SelectedKeys& selectedKeys)
{
	...
	std::set<u32> sharedKeys;
	i32k numSelKeys = (i32)selectedKeys.keys.size();
	for (i32 keyIndex = 0; keyIndex < numSelKeys; keyIndex++)
	{
		SelectedKey& selectedKey = selectedKeys.keys[keyIndex];
	...
	i32k newNumSelKeys = (i32)selKeys.size();
	for (i32 keyIndex = 0; keyIndex < newNumSelKeys; keyIndex++)
	{
...
private:
	...
	CSmartVariable<i32>         mv_fragmentOptionIdx;
	...
	if ((numSelKeys > 0) && isSame)
	{
		...
		if (paramType == SEQUENCER_PARAM_FRAGMENTID)
		{
			...
			for (u32 i = 0; i < numFrags; i++)
			{
				...
				{
					tukk szFragment = fragDefs.GetTagName(i);
...
{
	DRX_ASSERT(pVariable);

	i32k childCount = pVariable->GetNumVariables();
...
bool CProcClipUIControls::OnKeySelectionChange(SelectedKeys& selectedKeys)
{
	...
	if (selectedKeys.keys.size() == 1)
	{
		...
		if (paramType == SEQUENCER_PARAM_PROCLAYER)
		{
			...
			{
				tukk typeName = proceduralClipFactory.GetTypeNameByIndex(i);
			...
			{
				tukk const keyTypeName = proceduralClipFactory.FindTypeName(key.typeNameHash);
...
void CProcClipUIControls::OnProceduralParamsVariableChanged(IVariable* pVar)
{
	...
	for (i32 keyIndex = 0, num = (i32)selectedKeys.keys.size(); keyIndex < num; keyIndex++)
	{
		...
		if (paramType == SEQUENCER_PARAM_PROCLAYER)
		{
			...
			{
				DRX_ASSERT(mv_params == m_pVarBlock->FindVariable("Params"));
...
bool CTagStateUIControls::OnKeySelectionChange(SelectedKeys& selectedKeys)
{
	...
	if (selectedKeys.keys.size() == 1)
	{
		...
		if (paramType == SEQUENCER_PARAM_TAGS)
		{
			...
			//u32k numTags = m_parent->m_context.m_controllerDef->m_tags.GetNum();
...
public:
	static u32k      NUM_PARAMS_EXPOSED = 7;
...
protected:
	virtual void OnCreateVars()
	{
		...
		for (u32 i = 0; i < NUM_PARAMS_EXPOSED; i++)
		{
...
bool CMannParamUIControls::OnKeySelectionChange(SelectedKeys& selectedKeys)
{
	...
	if (selectedKeys.keys.size() == 1)
	{
		...
		if (paramType == SEQUENCER_PARAM_PARAMS)
		{
			...
			{
				tukk const paramName = MannUtils::GetParamNameByIndex(i);
...
void CMannKeyPropertiesDlgFE::OnKeySelectionChange()
{
	...
	if (selectedKeys.keys.size() > 0)
	{
		...
		for (i32 i = 0; i < (i32)m_keyControls.size(); i++)
		{
