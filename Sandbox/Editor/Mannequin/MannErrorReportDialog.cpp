...
static i32 __stdcall CompareItems(LPARAM p1, LPARAM p2, LPARAM sort)
{
...
CString CMannErrorRecord::GetErrorText()
{
	...
	str.Format("[%2d]\t%s", count, (tukk)error);
	str.TrimRight();
...
public:
	...
	CMannErrorMessageRecord(CMannErrorRecord& err, SMannequinContexts* contexts)
	{
		...
		tukk errorString[] = { "None", "Fragment", "Transition" };
		...
		tukk fragName = (err.fragmentID == FRAGMENT_ID_INVALID) ? ((err.type == SMannequinErrorReport::Blend) ? "Any" : "None") : contexts->m_controllerDef->m_fragmentIDs.GetTagName(err.fragmentID);
		char tagList[512];
...
//		return guid;
//	}
//	virtual tukk ClassName() { return "Error Report"; };
//	virtual tukk Category() { return "Editor"; };
//	//////////////////////////////////////////////////////////////////////////
//	virtual CRuntimeClass* GetRuntimeClass() { return RUNTIME_CLASS(CMannErrorReportDialog); };
//	virtual tukk GetPaneTitle() { return _T("Error Report"); };
...
//	{
//		/*
//		CDrxMemFile memFile( new BYTE[256], 256, 256 );
...
BOOL CMannErrorReportDialog::OnInitDialog()
{
	...
	{
		CDrxMemFile memFile(pbtData, nSize);
...
static volatile i32 TOP_CONTROL_OFFSET = 35;
...
//////////////////////////////////////////////////////////////////////////
void CMannErrorReportDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CMannErrorReportDialog::ReloadErrors()
{
	...
	for (i32 i = 0; i < m_errorRecords.size(); i++)
	{
...
void CMannErrorReportDialog::OnSelectObjects()
{
	/*
	   ...
	   while (pos)
	   {
	   i32 nItem = m_errors.GetNextSelectedItem(pos);
...
{
	i32k numRecords = m_wndReport.GetRecords()->GetCount();
...
void CMannErrorReportDialog::OnDeleteFragments()
{
	...
	if (numToDelete > 0)
	{
		...
		drx_sprintf(msg, "Delete all %d selected fragments?", numToDelete);
		i32 nCode = MessageBox(
		  msg,
		  "Delete Fragments?",
		...
		if (nCode == IDOK)
		{
			...
			i32 numRecordsToProcess = recordsToDelete.size();

			for (u32 i = 0; i < numRecordsToProcess; i++)
			{
				CMannErrorRecord& errorRecord = *recordsToDelete[i];
				u32k numScopeDatas = m_contexts->m_contextData.size();
				for (u32 s = 0; s < numScopeDatas; s++)
				{
...
void CMannErrorReportDialog::OnReportColumnRClick(NMHDR* pNotifyStruct, LRESULT* /*result*/)
{
	...
	for (i32 nColumn = 0; nColumn < nColumnCount; nColumn++)
	{
...
void CMannErrorReportDialog::CopyToClipboard()
{
	...
	for (i32 i = 0; i < selRows->GetCount(); i++)
	{
...
void CMannErrorReportDialog::SendInMail()
{
	...
	std::vector<tukk > who;
	CString subject;
	subject.Format("Level %s Error Report", (tukk)GetIEditorImpl()->GetGameEngine()->GetLevelName());
...
void CMannErrorReportDialog::OpenInExcel()
{
	...
	CString filename;
	filename.Format("ErrorList_%s.csv", (tukk)levelName);
	...
	if (f)
	{
		...
		{
			CMannErrorRecord& err = m_errorRecords[i];
			CString text = (tukk)err.GetErrorText();
			text.Replace(',', '.');
			text.Replace('\t', ',');
			fprintf(f, "%s\n", (LPCTSTR)text);
		}
	...
	{
		Warning("Failed to save %s", (tukk)filename);
	}
}
