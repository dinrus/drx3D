...
i32k PLAYER_ACTION_PRIORITY = 10;

//////////////////////////////////////////////////////////////////////////

CFragmentPlayback::CFragmentPlayback(ActionScopes scopeMask, u32 flags)
...
void CFragmentPlayback::SetFragment(FragmentID fragmentID, TagState fragTags, u32 option, float maxTime)
{
	m_timeSinceInstall = 0.0f;
	m_maxTime = maxTime;
...
void CFragmentPlayback::OnSequenceFinished(i32 layer, u32 scopeID)
{
...
{
	u32k numActions = m_actions.size();
...
void CFragmentSequencePlayback::SetTime(float time, bool bForce)
{
	...
	if (bForce || (fabs_tpl(timeDiff) >= MIN_TIME_CHANGE_EPSILON))
	{
		...
		u32k totalScopes = m_actionController.GetTotalScopes();
	...
	u32k newNumActions = m_actions.size();
	for (u32 i = 0; i < newNumActions; i++)
	{
