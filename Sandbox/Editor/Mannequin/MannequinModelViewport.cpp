...
#include "../../DrxEngine/DrxEntitySystem/RenderProxy.h"
...
//////////////////////////////////////////////////////////////////////////
i32 CMannequinModelViewport::OnPostStepLogged(const EventPhys* pEvent)
{
...
void CMannequinModelViewport::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		u32k numLocators = m_locators.size();
		for (u32 i = 0; i < numLocators; ++i)
		{
...
void CMannequinModelViewport::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	else
	{
		if (m_draggingLocator && m_selectedLocator < m_locators.size())
		{
			...
			i32 axis = locator.m_AxisHelper.GetHighlightAxis();
...
void CMannequinModelViewport::UpdatePropEntities(SMannequinContexts* pContexts, SMannequinContexts::SProp& prop)
{
	for (u32 em = 0; em < eMEM_Max; em++)
	{
		...
		u32k numBGObjects = backgroundObjects.size();
		for (u32 i = 0; i < numBGObjects; i++)
		{
...
bool CMannequinModelViewport::HitTest(HitContext& hc, const bool bIsClick)
{
	...
	if (pBestObject)
	{
		...
		{
			CString initialProp = "";
			for (u32 i = 0; i < pContexts->backgroundProps.size(); i++)
			{
...
void CMannequinModelViewport::OnRender()
{
	...
	i32 mode = (m_locMode == LM_Translate) ? CAxisHelper::MOVE_FLAG : CAxisHelper::ROTATE_FLAG;
	u32k numLocators = m_locators.size();
	for (u32 i = 0; i < numLocators; i++)
	{
		SLocator& locator = m_locators[i];
	...
	if (mv_AttachCamera)
	{
		i32k screenWidth  = std::max(pAuxGeom->GetCamera().GetViewSurfaceX(), 1);
		i32k screenHeight = std::max(pAuxGeom->GetCamera().GetViewSurfaceZ(), 1);
	...
	{
		tukk currentPropName = "Click to Assign Prop";
		...
		if (m_pHoverBaseObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			...
			if (pEntObject->GetIEntity())
			{
				...
				CMannequinDialog* pMannequinDialog = CMannequinDialog::GetCurrentInstance();
				SMannequinContexts* pContexts = pMannequinDialog->Contexts();
				u32k numProps = pContexts->backgroundProps.size();
...
void CMannequinModelViewport::SetPlayerPos()
{
	...
	u32 numSample6 = m_arrAnimatedCharacterPath.size();
	for (u32 i = 0; i < numSample6; i++)
	...
	u32 numSample7 = m_arrRunStrafeSmoothing.size();
	for (u32 i = 0; i < numSample7; i++)
	{
...
void CMannequinModelViewport::AddLocator(u32 refID, tukk name, const QuatT& transform, IEntity* pEntity, i16 refJointId, IAttachment* pAttachment, u32 paramCRC, string helperName)
{
	u32k numLocators = m_locators.size();
...
void CMannequinModelViewport::UpdateAnimation(float timePassed)
{
	...
	CMannequinDialog* pMannequinDialog = CMannequinDialog::GetCurrentInstance();
	SMannequinContexts* pContexts = pMannequinDialog->Contexts();
	u32k numContextDatums = pContexts->m_contextData.size();
	for (u32 i = 0; i < numContextDatums; i++)
	{
	...
	u32 numChars = m_entityList.size();
...
void CMannequinModelViewport::DrawEntityAndChildren(CEntityObject* pEntityObject, const SRendParams& rp, const SRenderingPassInfo& passInfo)
{
	...
	if (pEntity)
	{
		...
		i32k childEntityCount = pEntity->GetChildCount();
		for (i32 idx = 0; idx < childEntityCount; ++idx)
		{
	...
	i32k childObjectCount = pEntityObject->GetChildCount();
	for (i32 idx = 0; idx < childObjectCount; ++idx)
	{
...
void CMannequinModelViewport::DrawCharacter(ICharacterInstance* pInstance, const SRendParams& rRP, const SRenderingPassInfo& passInfo)
{
	...
	u32k numChars = m_entityList.size();
	for (u32 i = 0; i < numChars; i++)
	{
		IEntity* pEntity = m_entityList[i].GetEntity();
		if (pEntity && !pEntity->IsHidden())
...
void CMannequinModelViewport::DrawCharacter(IEntity* pEntity, ICharacterInstance* pInstance, IStatObj* pStatObj, const SRendParams& rRP, const SRenderingPassInfo& passInfo)
{
	...
	if (pInstance)
	{
		...
		if (m_showSceneRoots)
		{
			i32 numAnims = pInstance->GetISkeletonAnim()->GetNumAnimsInFIFO(0);
			if (numAnims > 0)
			{
