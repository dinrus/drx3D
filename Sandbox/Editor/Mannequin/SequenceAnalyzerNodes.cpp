...
i32k MI_SET_CONTEXT_NONE = 10000;
i32k MI_SET_CONTEXT_BASE = 10001;
...
i32 CRootNode::GetParamCount() const
{
	return NUM_GLOBAL_PARAM_IDS;
}

bool CRootNode::GetParamInfo(i32 nIndex, SParamInfo& info) const
{
...
void CScopeNode::InsertMenuOptions(CMenu& menu)
{
	...
	u32 contextID = m_pScopeData->contextID;
	u32 numContextDatas = m_pScopeData->mannContexts->m_contextData.size();
	u32 numMatchingContexts = 0;
	for (u32 i = 0; i < numContextDatas; i++)
	{
	...
	i32k flags = (numMatchingContexts > 1) ? 0 : MF_DISABLED;
...
void CScopeNode::OnMenuOption(i32 menuOption)
{
	...
	{
		i32 scopeContext = menuOption - MI_SET_CONTEXT_BASE;
...
i32 CScopeNode::GetParamCount() const
{
	return NUM_SCOPE_PARAM_IDS;
}

bool CScopeNode::GetParamInfo(i32 nIndex, SParamInfo& info) const
{
...
void CScopeNode::UpdateMutedLayerMasks(u32 mutedAnimLayerMask, u32 mutedProcLayerMask)
{
