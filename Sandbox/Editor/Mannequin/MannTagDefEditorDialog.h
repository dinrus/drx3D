...
protected:
	...
	void         CreateToolbarButton(CInPlaceButtonPtr& button, tukk text, CRect& rc, i32 index, i32 nID, CStatic* parent, CInPlaceButton::OnClick onClick);
	void         PopulateTagDefList();
	void         AddTagDefToList(CTagDefinition* pTagDef, bool select = false);
	HTREEITEM    AddGroupToTree(tukk szGroup, TagGroupID groupID, bool select = false);
	HTREEITEM    AddTagToTree(tukk szTag, TagID tagID, HTREEITEM parent, bool select = false);
