...
public:
	...
	void                     LoadNewPreviewFile(tukk previewFile);

	bool                     EnableContextData(const EMannequinEditorMode editorMode, i32k scopeContext);
	void                     ClearContextData(const EMannequinEditorMode editorMode, i32k scopeContextID);

	void                     EditFragment(FragmentID fragID, const SFragTagState& tagState, i32 contextID = -1);
	void                     EditFragmentByKey(const struct CFragmentKey& key, i32 contextID = -1);
	...
	void                     EditTransitionByKey(const struct CFragmentKey& key, i32 contextID = -1);

	bool                     CheckChangedData();

	void                     OnMoveLocator(EMannequinEditorMode editorMode, u32 refID, tukk locatorName, const QuatT& transform);

	void                     SetKeyProperty(tukk propertyName, tukk propertyValue);
...
protected:
	...
	bool         SavePreviewFile(tukk filename);
	bool         LoadPreviewFile(tukk filename, XmlNodeRef& xmlSequenceNode);

	void         EnableContextEditor(bool enable);

	bool         InitialiseToPreviewFile(tukk previewFile);
...
private:
	...
	void         EnableMenuCommand(u32 commandId, bool enable);
