...
i32k kIconFromParamID[SEQUENCER_PARAM_TOTAL] = { 0, 0, 14, 3, 10, 13 };
i32k kIconFromNodeType[SEQUENCER_NODE_TOTAL] = { 1, 1, 1, 1, 1, 2 };
...
{
	i32k topRowIndex = (m_sequence == seq) ? GetTopRowIndex() : 0;
	...
	//DrxLog("CMannNodesCtrl::SetSequence %" PRIi64 " %" PRIi64 " %" PRIi64 " %" PRIi64 " %" PRIi64 " %" PRIi64 , time1, time2, time3, time4, time5, time6);
...
void CMannNodesCtrl::Reload()
{
	...
	//DrxLog("CMannNodesCtrl::Reload %" PRIi64 " %" PRIi64 " %" PRIi64 , time1, time2, time3);
}
...
void CMannNodesCtrl::FillTracks(CRecord* pNodeRecord, CSequencerNode* node)
{
	...
	for (i32 i = 0; i < node->GetTrackCount(); i++)
	{
		...
		i32 nImage = GetIconIndexForParam(type);
		...
		if (track)
		{
			if ((type == SEQUENCER_PARAM_ANIMLAYER) || (type == SEQUENCER_PARAM_PROCLAYER))
			{
				...
				i32 nMuteIcon = GetIconIndexForMute(pTrackRecord);
...
//////////////////////////////////////////////////////////////////////////
i32 CMannNodesCtrl::GetIconIndexForParam(ESequencerParamType type) const
{
	return kIconFromParamID[type];
}

//////////////////////////////////////////////////////////////////////////
i32 CMannNodesCtrl::GetIconIndexForNode(ESequencerNodeType type) const
{
	return kIconFromNodeType[type];
}

//////////////////////////////////////////////////////////////////////////
i32 CMannNodesCtrl::GetIconIndexForMute(SItemInfo* pItem) const
{
	i32 icon = MUTE_TRACK_ICON_UNMUTED;
...
void CMannNodesCtrl::FillNodes(CRecord* pRecord)
{
	...
	for (i32 i = 0; i < seq->GetNodeCount(); i++)
	{
		CSequencerNode* node = seq->GetNode(i);

		i32 nNodeImage = GetIconIndexForNode(node->GetType());
		...
		if (node)
		{
			...
			i32 nMuteIcon = GetIconIndexForMute(pNodeRecord);
...
void CMannNodesCtrl::OnSelectionChanged()
{
	...
	for (i32 i = 0; i < m_sequence->GetNodeCount(); i++)
	{
		CSequencerNode* node = m_sequence->GetNode(i);
		for (i32 t = 0; t < node->GetTrackCount(); t++)
		{
	...
	for (i32 i = 0; i < nCount; i++)
	{
		...
		for (i32 i = 0; i < m_keysCtrl->GetCount(); i++)
		{
			if (pRecord->track)
...
void CMannNodesCtrl::SyncKeyCtrl()
{
	...
	i32 nNumRows = pRows->GetCount();
	for (i32 i = GetTopRowIndex(); i < nNumRows; i++)
	{
		...
		i32k nItemHeight = pRecord->GetItemHeight();
...
bool CMannNodesCtrl::IsNodeExpanded(CSequencerNode* node)
{
	...
	i32k nNumRows = pRows->GetCount();
	for (i32 i = 0; i < nNumRows; ++i)
	{
		CXTPReportRow* pRow = pRows->GetAt(i);
...
//////////////////////////////////////////////////////////////////////////
void CMannNodesCtrl::SelectNode(tukk sName)
{
	CXTPReportRows* pRows = GetRows();
	for (i32 i = 0, nNumRows = pRows->GetCount(); i < nNumRows; i++)
	{
...
void CMannNodesCtrl::OnNMLclick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (pRow)
	{
		...
		for (u32 i = 0; i < pRow->GetRecord()->GetItemCount(); ++i)
		{
...
void CMannNodesCtrl::OnNMRclick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	i32 cmd = ShowPopupMenu(point, pItemInfo, column);
	...
	if (cmd >= eMenuItem_AddLayer_First && cmd < eMenuItem_AddLayer_Last)
	{
		if (pItemInfo)
		{
			...
			if (node)
			{
				i32 paramIndex = cmd - eMenuItem_AddLayer_First;
...
bool CMannNodesCtrl::OnBeginDragAndDrop(CXTPReportSelectedRows* pRows, CPoint pt)
{
	...
	for (i32 i = 0; i < pRows->GetCount(); ++i)
	{
...
void CMannNodesCtrl::OnDragAndDrop(CXTPReportSelectedRows* pRows, CPoint absoluteCursorPos)
{
	...
	if (reportArea.PtInRect(p))
	{
		...
		if (pTargetRow)
		{
			...
			{
				tukk srcFirstNodeName = pFirstRecordSrc->node->GetName();
...
//////////////////////////////////////////////////////////////////////////
bool CMannNodesCtrl::HasNode(tukk name) const
{
	if (name == NULL)
		return false;
...
void CMannNodesCtrl::AddTrack(i32 paramIndex, CSequencerNode* node)
{
	CSequencerNode::SParamInfo paramInfo;
...
void CMannNodesCtrl::ShowHideTrack(CSequencerNode* node, i32 trackIndex)
{
...
void CMannNodesCtrl::MuteNodesRecursive(SItemInfo* pItemInfo, bool bMute)
{
	if (pItemInfo)
	{
		if (pItemInfo->track)
		{
			...
			{
				u32 mutedAnimLayerMask;
				u32 mutedProcLayerMask;
		...
		i32 numRecords = pItemInfo->GetChildCount();
...
void CMannNodesCtrl::MuteAllNodes()
{
	...
	if (pRecords)
	{
		i32 numRecords = pRecords->GetCount();
		for (i32 i = 0; i < numRecords; i++)
		{
...
void CMannNodesCtrl::GenerateMuteMasks(SItemInfo* pItem, u32& mutedAnimLayerMask, u32& mutedProcLayerMask)
{
	...
	if (pItem)
	{
		...
		i32 numRecords = pItem->GetChildCount();
...
i32 CMannNodesCtrl::ShowPopupMenu(CPoint point, const SItemInfo* pItemInfo, u32 column)
{
	// Create pop up menu.
...
i32 CMannNodesCtrl::ShowPopupMenuNode(CPoint point, const SItemInfo* pItemInfo)
{
	CMenu menu;
	CMenu menuAddTrack;
	...
	if (GetSelectedCount() == 1)
	{
		...
		if (onValidItem)
		{
			...
			i32k validParamCount = pItemInfo->node->GetParamCount();
			for (i32 i = 0; i < validParamCount; ++i)
			{
				CSequencerNode::SParamInfo paramInfo;
		...
		if (onValidItem)
		{
			...
			{
				CSequencerNode::SParamInfo paramInfo;
				for (i32 i = 0; i < pItemInfo->node->GetTrackCount(); i++)
				{
	...
	i32 ret = CXTPCommandBars::TrackPopupMenu(&menu, TPM_NONOTIFY | TPM_RETURNCMD | TPM_LEFTALIGN | TPM_RIGHTBUTTON,
...
i32 CMannNodesCtrl::ShowPopupMenuMute(CPoint point, const SItemInfo* pItemInfo)
{
	CMenu menu;
...
void CMannNodesCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CMannNodesCtrl::RestoreVerticalScrollPos(float fScrollPos)
{
	i32 sbMin = 0, sbMax = 0;
	GetScrollRange(SB_VERT, &sbMin, &sbMax);
	i32 newScrollPos = pos_directed_rounding(fScrollPos * (sbMax - sbMin) + sbMin);
...
{
	// List`s which tracks can be added to animation node.
	u32k validParamCount = u32 (pItemInfo->node->GetParamCount());
	...
	i32 keyID = pTrack->CreateKey(0.0f);
