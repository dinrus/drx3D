...
//////////////////////////////////////////////////////////////////////////
void CMannTagDefEditorDialog::CreateToolbarButton(CInPlaceButtonPtr& button, tukk text, CRect& rc, i32 index, i32 nID, CStatic* parent, CInPlaceButton::OnClick onClick)
{
...
BOOL CMannTagDefEditorDialog::OnInitDialog()
{
	...
	if (!m_initialTagFilename.IsEmpty())
	{
		...
		i32k itemIdx = m_tagDefList.FindItem(&nfo);
		if (itemIdx >= 0)
		{
...
void CMannTagDefEditorDialog::OnOKButton()
{
	...
	// Do renaming here
	for (u32 index = 0; index < m_tagDefLocalCopy.size(); ++index)
	{
		const CTagDefinition* pOriginal = m_tagDefLocalCopy[index].m_pOriginal;

		for (u32 renameIndex = 0; renameIndex < m_tagDefLocalCopy[index].m_renameInfo.size(); ++renameIndex)
		{
			...
			if (renameInfo.m_isGroup)
			{
				...
				if (id != GROUP_ID_NONE)
				{
					//DrxLog("[TAGDEF]: OnOKButton() (%s): renaming tag group [%s] to [%s]", pOriginal->GetFilename(), pOriginal->GetGroupName(id), renameInfo.m_newName);
			...
			else
			{
				...
				{
					//DrxLog("[TAGDEF]: OnOKButton() (%s): renaming tag [%s] to [%s]", pOriginal->GetFilename(), pOriginal->GetGroupName(id), renameInfo.m_newName);
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "The tag definition you have created exceeds the memory available! This tag definition has not been saved as it will cause data loss.");
	}
}
...
void CMannTagDefEditorDialog::OnNewTagButton()
{
	...
	if (dialog.exec() == QDialog::Accepted)
	{
		...
		tukk const szGroup = groupID != TAG_ID_INVALID ? m_selectedTagDef->GetGroupName(groupID) : NULL;
		...
		if (!success)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Cannot add new tag: please check with a programmer to increase the tag definition size if necessary.");
			return;
		}
...
void CMannTagDefEditorDialog::OnDeleteTagButton()
{
	...
	tuk buffer[4096];
	IMannequinEditorManager* pMgr = gEnv->pGameFramework->GetMannequinInterface().GetMannequinEditorManager();
	pMgr->GetAffectedFragmentsString(pTagDefPair->m_pOriginal, tagID, (tuk)buffer, sizeof(buffer));
...
{
	return (MessageBox(text.GetBuffer(), "DrxMannequin", MB_YESNO | MB_ICONQUESTION) == IDYES);
}
...
void CMannTagDefEditorDialog::PopulateTagDefListRec(const CString& baseDir)
{
	...
	intptr_t handle = pDrxPak->FindFirst(baseDir + "*", &fd);
	if (handle != -1)
	{
		do
		{
				...
				if (filename.Right(filter.GetLength()).CompareNoCase(filter) == 0)
				{
					if (XmlNodeRef root = GetISystem()->LoadXmlFromFile(filename))
					{
						...
						if (tag == "TagDefinition")
						{
							...
							{
								DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "CMannTagDefEditorDialog: Failed to load existing tag definition file '%s'", filename);
							}
						}
					}
				}
...
{
	for (i32 index = m_tagDefList.GetItemCount() - 1; index >= 0; --index)
	{
...
{
	i32 nItem = m_tagDefList.GetItemCount();
...
void CMannTagDefEditorDialog::PopulateTagDefTree()
{
	...
	i32 nGroups = m_selectedTagDef->GetNumGroups();
	for (i32 i = 0; i < nGroups; i++)
		AddGroupToTree(m_selectedTagDef->GetGroupName(i), i);

	// Create the tag nodes
	i32 nTags = m_selectedTagDef->GetNum();
	for (i32 i = 0; i < nTags; i++)
	{
...
//////////////////////////////////////////////////////////////////////////
HTREEITEM CMannTagDefEditorDialog::AddGroupToTree(tukk szGroup, TagGroupID groupID, bool select)
{
...
//////////////////////////////////////////////////////////////////////////
HTREEITEM CMannTagDefEditorDialog::AddTagToTree(tukk szTag, TagID tagID, HTREEITEM parent, bool select)
{
...
void CMannTagDefEditorDialog::OnTagTreeSelChanged(NMHDR*, LRESULT*)
{
	...
	if (HTREEITEM item = m_tagsTree->GetSelectedItem())
	{
		...
		{
			u32 priority = m_selectedTagDef->GetPriority(id);
...
bool CMannTagDefEditorDialog::ValidateTagName(const CString& name)
{
	...
	if (name.IsEmpty())
	{
		...
		MessageBox(errorMessage.GetBuffer(), "DrxMannequin", MB_OK | MB_ICONERROR);
		return false;
	}
