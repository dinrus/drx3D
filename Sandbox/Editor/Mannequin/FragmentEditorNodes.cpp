...
u32 CFragmentNode::GetChangeCount() const
{
	if (m_pScopeData->context[eMEM_FragmentEditor])
	{
...
u32 CFragmentNode::GetNumTracksOfParamID(ESequencerParamType paramID) const
{
	u32 numTracksOfType = 0;
...
i32k MI_ENABLE_SCOPE = 10000;
i32k MI_DISABLE_SCOPE = 10001;

void CFragmentNode::InsertMenuOptions(CMenu& menu)
{
	menu.AppendMenu(MF_SEPARATOR, 0, "");

	if (m_isSequenceNode)
	{
		menu.AppendMenu(MF_STRING, MI_ENABLE_SCOPE, "Include Scope");
	}
	else
	{
		menu.AppendMenu(MF_STRING, MI_DISABLE_SCOPE, "Exclude Scope");
	}
}

void CFragmentNode::ClearMenuOptions(CMenu& menu)
{
}

void CFragmentNode::OnMenuOption(i32 menuOption)
{
	if ((menuOption == MI_ENABLE_SCOPE) || (menuOption == MI_DISABLE_SCOPE))
	{
		...
		const ActionScopes scopeFlag = BIT64(m_pScopeData->scopeID);
...
i32 CFragmentNode::GetParamCount() const
{
	return NUM_PARAM_IDS;
}

bool CFragmentNode::GetParamInfo(i32 nIndex, SParamInfo& info) const
{
...
void CFragmentNode::UpdateMutedLayerMasks(u32 mutedAnimLayerMask, u32 mutedProcLayerMask)
{
...
i32 CFERootNode::GetParamCount() const
{
	return NUM_FE_ROOT_PARAM_IDS;
}

bool CFERootNode::GetParamInfo(i32 nIndex, SParamInfo& info) const
{
