...
tukk noDB = "<no animation database (adb)>";
tukk noControllerDef = "<no controller def>";
};
...
void CMannNewContextDialog::PopulateControls()
{
	...
	// Context combos
	for (i32 i = 0; i < pControllerDef->m_scopeContexts.GetNum(); i++)
	{
	...
	tukk noAttachmentContext = "<no attachment context>";
...
void CMannNewContextDialog::OnOk()
{
	...
	// some of m_pData members are of type string (DrxString<char>) so need to be retrieved via a temporary
	CString tempString;
	...
	else
	{
		...
		tukk idname = pControllerDef->m_scopeContexts.GetTagName(m_pData->contextID);
	...
	if (tempString[0])
	{
		tukk ext = PathUtil::GetExt(tempString);
	...
	i32k dbSel = m_databaseCombo.GetCurSel();
	CString dbString;
	...
	i32k attCont = m_attachmentContextCombo.GetCurSel();
	...
	i32k dbSlaveSel = m_slaveDatabaseCombo.GetCurSel();
	...
	i32k dbSlaveContDefSel = m_slaveControllerDefCombo.GetCurSel();
	...
	if (m_mode != eContextDialog_Edit)
	{
		...
		i32 newDataID = 0;
		for (i32 i = 0; i < pContexts->m_contextData.size(); i++)
		{
			i32k currDataID = pContexts->m_contextData[i].dataID;
...
void CMannNewContextDialog::PopulateDatabaseCombo()
{
	...
	i32k numEntries = m_databaseCombo.GetCount();
	assert(numEntries == 0);
	for (i32 i = 0; i < databases.size(); i++)
	{
...
void CMannNewContextDialog::PopulateControllerDefCombo()
{
	...
	i32k numEntries = m_slaveControllerDefCombo.GetCount();
	assert(numEntries == 0);
	for (i32 i = 0; i < controllerDefs.size(); i++)
	{
