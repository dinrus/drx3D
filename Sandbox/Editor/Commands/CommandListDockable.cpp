...
tukk CCommandListModel::s_ColumnNames[] = { QT_TR_NOOP("Command"), QT_TR_NOOP("Description") };
...
QVariant CCommandListModel::data(const QModelIndex& index, i32 role) const
{
	...
	case eCommandListDepth_Command:
		{
			...
			if (role == Qt::DisplayRole || role == CCommandListModel::SortRole)
			{
				...
				case eCommandListColumn_Description:
					return CommandModel::data(this->index(index.row(), 0, parent), static_cast<i32>(CommandModel::Roles::CommandDescriptionRole));
	...
	case eCommandListDepth_Argument:
		{
			...
			else if (role == static_cast<i32>(CommandModel::Roles::SearchRole))
			{
				return QVariant();
			}
		}
...
QVariant CCommandListModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
QModelIndex CCommandListModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
...
i32 CCommandListModel::rowCount(const QModelIndex& parent) const
{
	const auto depth = GetIndexDepth(parent);
	if (depth == eCommandListDepth_Command)
	{
...
public:
	...
	{
		DRX_ASSERT(GetIndexDepth(left) == GetIndexDepth(right));
	...
	: CDockableWidget(pParent)
{
	...
	pFilterProxy->setFilterRole(static_cast<i32>(CommandModel::Roles::SearchRole));
