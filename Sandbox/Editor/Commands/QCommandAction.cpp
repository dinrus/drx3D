...
QCommandAction::QCommandAction(const QString& actionName, const QString& actionText, QObject* parent, tukk command /*=0*/)
...
QCommandAction::QCommandAction(const QString& actionName, tukk command, QObject* parent)
	: QAction(actionName, parent)
{
...
void QCommandAction::SetCommand(tukk command)
{
...
void QCommandAction::operator=(const UiInfo& info)
{
	...
	if (!info.icon || *info.icon)
		setIcon(DrxIcon(QtUtil::ToQString(info.icon)));
}
