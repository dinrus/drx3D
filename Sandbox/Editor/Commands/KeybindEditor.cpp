...
void PySetKeybind(tukk command, tukk shortcut)
{
...
void PyResetKeybind(tukk command)
{
	GetIEditorImpl()->GetCommandManager()->ResetShortcut(command);
}

void PyAddCustomCommand(tukk uiName = nullptr, tukk command = nullptr, tukk shortcut = nullptr)
{
...
void PyRemoveCustomCommand(tukk command)
{
...
QVariant CKeybindEditor::KeybindModel::data(const QModelIndex& index, i32 role /* = Qt::DisplayRole */) const
{
...
bool CKeybindEditor::KeybindModel::setData(const QModelIndex& index, const QVariant& value, i32 role /*= Qt::EditRole*/)
{
...
class CKeybindEditor::KeybindItemDelegate : public QStyledItemDelegate
{
	...
	public:
			...
			, m_lineEdit(new QLineEdit(this))
		{
			...
			plusButton->setIcon(DrxIcon(QPixmap(":icons/General/Plus.ico")));
			...
			minusButton->setIcon(DrxIcon(QPixmap(":icons/General/Minus.ico")));
			...
			clearButton->setIcon(DrxIcon(QPixmap(":icons/General/Close.ico")));
	...
	, m_treeView(new QAdvancedTreeView())
{
	...
	proxy->setFilterRole((i32)CommandModel::Roles::SearchRole);
...
void CKeybindEditor::OnAddCustomCommand() const
{
	...
	QModelIndexList list = model->match(model->index(0, 0, QModelIndex()), (i32)CommandModel::Roles::CommandPointerRole, QVariant::fromValue((CCommand*)customCommand), 1, Qt::MatchRecursive | Qt::MatchExactly);
	if (!list.isEmpty())
	{
...
void CKeybindEditor::SaveKeybindsToFile(tukk filename)
{
	QFile file(filename);
	if (!file.open(QIODevice::WriteOnly))
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed to open path: %s", filename);
		return;
	}
...
void CKeybindEditor::LoadKeybindsFromFile(tukk filename)
{
	QFile file(filename);
	if (!file.open(QIODevice::ReadOnly))
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_COMMENT, "Failed to open path: %s", filename);
		return;
	}
