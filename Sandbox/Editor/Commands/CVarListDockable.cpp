...
tukk CCVarModel::s_szColumnNames[] = { QT_TR_NOOP(""), QT_TR_NOOP("Variable"), QT_TR_NOOP("Value"), QT_TR_NOOP("Description"), QT_TR_NOOP("Type") };
enum ECVarColumn
{
			...
			case CCVarModel::Int:
				{
					...
					pSpinBox->setMaximum(std::numeric_limits<i32>::max());
					pSpinBox->setMinimum(std::numeric_limits<i32>::lowest());
			...
			case CCVarModel::Float:
				{
					...
					pSpinBox->setVal(value.toDouble());
					return;
				}
...
{
	DRX_ASSERT(idx.row() < m_cvars.size());
	tukk cvarName = m_cvars[idx.row()].c_str();
	return gEnv->pConsole->GetCVar(cvarName);
}

QVariant CCVarModel::data(const QModelIndex& index, i32 role /*= Qt::DisplayRole*/) const
{
	DRX_ASSERT(index.row() < m_cvars.size());
	...
	if (role == Qt::DisplayRole || role == Qt::ToolTipRole || role == Roles::SortRole)
	{
		...
		case eCVarListColumn_Description:
			{
				tukk szHelpText = pCVar->GetHelp();
				if (role == Qt::ToolTipRole)
					return pCVar->GetHelp();

				QString helpText = szHelpText;
				i32k linebreak = helpText.indexOf('\n');
				if (linebreak > 0)
				{
...
QVariant CCVarModel::headerData(i32 section, Qt::Orientation orientation, i32 role /*= Qt::DisplayRole*/) const
{
...
QModelIndex CCVarModel::index(i32 row, i32 column, const QModelIndex& parent /*= QModelIndex()*/) const
{
...
bool CCVarModel::setData(const QModelIndex& index, const QVariant& value, i32 role /*= Qt::EditRole*/)
{
	if (index.isValid())
	{
		if (role == Qt::EditRole)
...
i32 CCVarModel::rowCount(const QModelIndex& parent /*= QModelIndex()*/) const
{
...
bool CCVarModel::OnBeforeVarChange(ICVar* pVar, tukk sNewValue)
{
	return true;
}
	...
	: m_pModel(new CCVarModel())
{
	...
	m_pFilterProxy->setSortRole(static_cast<i32>(CCVarModel::Roles::SortRole));
	...
	: CEditorDialog("CVar Browser")
{
	...
	pCVarBrowser->signalOnDoubleClick.Connect([this](tukk cvarName)
	{
...
void CCVarListDockable::LoadCVarListFromFile(tukk szFilename)
{
	QFile file(szFilename);
	if (!file.open(QIODevice::ReadOnly))
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed to open path: %s", szFilename);
		return;
	}
...
void CCVarListDockable::SaveCVarListToFile(tukk szFilename)
{
	QFile file(szFilename);
