...
struct IInitializeUIInfo
{
	virtual void SetInfoText(tukk text) = 0;
};

//! This class serves as a high-level wrapper for DrxEngine game.
...
public:
	...
	void         SetPlayerEquipPack(tukk sEqipPackName);
	...
	void                       SetCurrentMOD(tukk sMod);
	//! Returns game's current Mod name.
	tukk                GetCurrentMOD() const;
	//! Called every frame.
	void                       Update();
...
private:
	...
	struct SSysUserCallback*   m_pSystemUserCallback;
