...
/*
   ...
   CSmartVariableEnum<i32> tokenType;
...
BOOL CGameTokenDialog::OnInitDialog()
{
	...
	/*
	   ...
	   i32 HSplitter = AfxGetApp()->GetProfileInt("Dialogs\\GameTokensEditor","HSplitter",200 );
	   i32 VSplitter = AfxGetApp()->GetProfileInt("Dialogs\\GameTokensEditor","VSplitter",200 );
...
void CGameTokenDialog::UpdateSelectedItemInReport()
{
	...
	// find row.
	for (i32 i = 0; i < m_wndReport.GetRows()->GetCount(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CGameTokenDialog::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CGameTokenDialog::UpdateField(i32 nControlId, const CString& strValue, bool bBoolValue)
{
	if (GetSelectedGameToken())
	{
...
void CGameTokenDialog::OnTokenTypeChange()
{
	...
	if (m_bSkipUpdateItems)
		return;
	i32 nIndex = m_tokenTypeCombo.GetCurSel();
	if (nIndex == CB_ERR)
		return;
