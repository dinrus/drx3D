...
CExportManager::CExportManager() :
	...
	m_pBaseObj(0),
	m_fScale(100.0f), // this scale is used by DrxEngine RC
...
IExporter* CExportManager::GetExporterForExtension(tukk szExtension) const
{
...
void CExportManager::AddMesh(SExportObject* pObj, const IIndexedMesh* pIndMesh, Matrix34A* pTm)
{
	...
	i32 newOffsetIndex = pObj->GetVertexCount();
	...
	for (i32 v = 0; v < meshDesc.m_nCoorCount; ++v)
	{
		Export::UV tc;
	...
	{
		for (i32 i = 0; i < pIndMesh->GetSubSetCount(); ++i)
		{
			...
			i32 nTris = sms.nNumIndices / 3;
			pMesh->m_faces.reserve(nTris);
			for (i32 f = 0; f < nTris; ++f)
			{
				Export::Face face;
	...
	else
	{
		...
		if (meshDesc.m_nFaceCount == 0 && meshDesc.m_nIndexCount != 0 && meshDesc.m_pIndices != 0)
		{
			...
			i32 nTris = meshDesc.m_nIndexCount / 3;
			pMesh->m_faces.reserve(nTris);
			for (i32 f = 0; f < nTris; ++f)
			{
				Export::Face face;
		...
		else
		{
			...
			for (i32 f = 0; f < meshDesc.m_nFaceCount; ++f)
			{
				Export::Face face;
...
bool CExportManager::AddCharacter(SExportObject* pObj, ICharacterInstance* pCharacter, Matrix34A* pTm)
{
	...
	if (pSkeletonPose)
	{
		u32 nJointCount = rIDefaultSkeleton.GetJointCount();
		for (u32 i = 0; i < nJointCount; ++i)
		{
	...
	{
		u32 nAttachmentCount = AttachmentManager->GetAttachmentCount();
		for (u32 i = 0; i < nAttachmentCount; ++i)
		{
...
{
	for (i32 objectID = 0; objectID < m_data.m_objects.size(); ++objectID)
	{
...
bool CExportManager::AddSelectedObjects()
{
	...
	i32 numObjects = pSelection->GetCount();
...
bool CExportManager::AddTerrain()
{
	...
	i32 nHeightmapWidth = heightmap.GetWidth();
	i32 nHeightmapHeight = heightmap.GetHeight();
	...
	if ((i32)rc.bottom - rc.top <= 0 && (i32)rc.right - rc.left <= 0)
		return false;
	...
	i32 nExportWidth = rc.right - rc.left;
	i32 nExportHeight = rc.bottom - rc.top;
	...
	for (i32 y = 0; y < nExportHeight; ++y)
	{
		for (i32 x = 0; x < nExportWidth; ++x)
		{
			i32 nExtendedWidth = nExportWidth + 1;
			Export::Face face;
	...
	drx_strcpy(material.name, "Sandbox_Terrain");
	...
	/*
	   ...
	   drx_strcpy(szInfoPath, pszFileName);
	   PathRemoveExtension(szInfoPath);
	   drx_strcat(szInfoPath,".inf");
...
bool CExportManager::ExportToFile(tukk filename, bool bClearDataAfterExport)
{
	bool bRet = false;
	...
	for (i32 i = 0; i < m_exporters.size(); ++i)
	{
...
bool CExportManager::Export(tukk defaultName, tukk defaultExt, tukk defaultPath, bool isSelectedObjects, bool isSelectedRegionObjects, bool isTerrain, bool isOccluder, bool bAnimationExport)
{
...
bool CExportManager::ImportFromFile(tukk filename)
{
	bool bRet = false;
	string ext = PathUtil::GetExt(filename);
...
bool CExportManager::ExportSingleStatObj(IStatObj* pStatObj, tukk filename)
{
