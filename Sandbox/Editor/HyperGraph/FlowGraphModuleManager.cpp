...
#include <DrxFlowGraph/IFlowGraphDebugger.h>
...
{
	for (i32 i = 0; i < m_FlowGraphs.size(); ++i)
	{
...
bool CEditorFlowGraphModuleManager::SaveModuleGraph(CHyperFlowGraph* pFG)
{
	...
	tukk filename = gEnv->pFlowSystem->GetIModuleManager()->GetModulePath(pFG->GetName());
	...
	if (pModuleManager)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed to save modified flowgraph module '%s' to '%s'", pFG->GetName(), filename);
		}
	}
	return false;
}
...
bool CEditorFlowGraphModuleManager::NewModule(string& filename, bool bGlobal /*=true*/, CHyperGraph** pHyperGraph)
{
	...
	DrxPathString path(PathUtil::Make(
	...
	if (IFlowGraphModule* pModule = pModuleManager->GetModule(moduleName))
	{
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, msg);
		return false;
	}
...
//////////////////////////////////////////////////////////////////////////
IFlowGraphPtr CEditorFlowGraphModuleManager::GetModuleFlowGraph(i32 index) const
{
...
//////////////////////////////////////////////////////////////////////////
IFlowGraphPtr CEditorFlowGraphModuleManager::GetModuleFlowGraph(tukk name) const
{
	assert(name != NULL);
	if (name == NULL)
		return NULL;
...
void CEditorFlowGraphModuleManager::DeleteModuleFlowGraph(CHyperFlowGraph* pGraph)
{
	...
	{
		tukk moduleGraphName = pGraph->GetName();
		...
		if (CFlowGraphManager* pFlowGraphManager = GetIEditorImpl()->GetFlowGraphManager())
		{
			...
			tukk callerNodeName = pModuleManager->GetCallerNodeName(moduleGraphName);
...
//////////////////////////////////////////////////////////////////////////
void CEditorFlowGraphModuleManager::CreateModuleNodes(tukk moduleName)
{
	IFlowGraphModuleManager* pModuleManager = gEnv->pFlowSystem->GetIModuleManager();
