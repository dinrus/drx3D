...
#include <DrxSandbox/DrxFunction.h>
...
void WriteFlownodeToDocumentation(IFlowNodeData* pFlowNodeData, const IFlowNodeTypeIterator::SNodeType& nodeType)
{
	...
	i32 iInputs = pFlowNodeData->GetNumInputPorts();
	for (i32 input = 0; input < iInputs; ++input)
	{
	...
	i32 iOutputs = pFlowNodeData->GetNumOutputPorts();
	for (i32 output = 0; output < iOutputs; ++output)
	{
...
void CFlowGraphManager::RegisterGraph(CHyperFlowGraph* pGraph)
{
	...
	{
		DrxLogAlways("CFlowGraphManager::RegisterGraph: OLD Graph 0x%p", pGraph);
		assert(false);
	}
	else
	{
#ifdef FG_INTENSIVE_DEBUG
		DrxLogAlways("CFlowGraphManager::RegisterGraph: NEW Graph 0x%p", pGraph);
#endif
...
{
#ifdef FG_INTENSIVE_DEBUG
	DrxLogAlways("CFlowGraphManager::UnregisterGraph: Graph 0x%p", pGraph);
#endif
...
//////////////////////////////////////////////////////////////////////////
CHyperNode* CFlowGraphManager::CreateNode(CHyperGraph* pGraph, tukk sNodeClass, HyperNodeID nodeId, const Gdiplus::PointF& pos, CBaseObject* pObj, bool bAllowMissing)
{
...
void CFlowGraphManager::ReloadGraphs()
{
	...
	// restore graphs
	for (i32 i = 0; i < m_CurrentFgUndos.size(); ++i)
	{
...
void CFlowGraphManager::UpdateNodePrototypeWithoutReload(tukk nodeClassName)
{
...
float GetValue(const CString& s, tukk token)
{
	float fValue = 0.0f;
	i32 pos = s.Find(token);
	if (pos >= 0)
	{
		// fill in Enum Pairs
		pos = s.Find('=', pos + 1);
...
float GetValue(tukk s, tukk token)
{
...
UIEnumType ParseUIConfig(tukk sUIConfig, std::map<string, string>& outEnumPairs, Vec3& uiValueRanges)
{
...
void FillEnumVar(CVariableEnum<T>* pVar, const std::map<string, string>& nameValueMap)
{
	...
	for (std::map<string, string>::const_iterator iter = nameValueMap.begin(); iter != nameValueMap.end(); ++iter)
	{
		...
		//i32 val = atoi (iter->second.GetString());
		pVar->AddEnumItem(iter->first, val);
	}
}

//////////////////////////////////////////////////////////////////////////
IVariable* CFlowGraphManager::MakeInVar(const SInputPortConfig* pPortConfig, u32 portId, CFlowNode* pFlowNode)
{
	i32 type = pPortConfig->defaultData.GetType();
	...
	if (pPortConfig->sUIConfig != 0)
	{
		...
		case eUI_Int:
			{
				CVariableFlowNodeEnum<i32>* pEnumVar = new CVariableFlowNodeEnum<i32>;
				FillEnumVar<i32>(pEnumVar, enumPairs);
				pVar = pEnumVar;
			}
			break;
		case eUI_Float:
		...
		while (pPortConfig->name)
		{
			...
			if (port.pVar)
			{
				...
				case eFDT_Bool:
					...
					break;
				case eFDT_Int:
					port.pVar->Set(*pPortConfig->defaultData.GetPtr<i32>());
					break;
				case eFDT_Float:
					port.pVar->Set(*pPortConfig->defaultData.GetPtr<float>());
					break;
				case eFDT_EntityId:
					port.pVar->Set((i32)*pPortConfig->defaultData.GetPtr<EntityId>());
...
//////////////////////////////////////////////////////////////////////////
IVariable* CFlowGraphManager::MakeSimpleVarFromFlowType(i32 type)
{
	switch (type)
	{
	case eFDT_Void:
	...
	case eFDT_Bool:
		...
		break;
	case eFDT_Int:
		return new CVariableFlowNode<i32>;
		break;
	case eFDT_Float:
...
//////////////////////////////////////////////////////////////////////////
CHyperFlowGraph* CFlowGraphManager::CreateGraphForEntity(CEntityObject* pEntity, tukk sGroupName)
{
	CHyperFlowGraph* pFlowGraph = new CHyperFlowGraph(this, sGroupName, pEntity);
	pFlowGraph->SetName(pEntity->GetName());
#ifdef FG_INTENSIVE_DEBUG
	DrxLogAlways("CFlowGraphManager::CreateGraphForEntity: graph=0x%p entity=0x%p guid=%s", pFlowGraph, pEntity, GuidUtil::ToString(pEntity->GetId()));
#endif
	return pFlowGraph;
}
...
CHyperFlowGraph* CFlowGraphManager::CreateEditorGraphForModule(IFlowGraphModule* pModule)
{
	...
	if (pModule)
	{
		...
		i32k moduleFolderLength = moduleFolder.GetLength();
		i32 offset = groupName.Find(moduleFolder);
		i32 offset2 = groupName.Find("\\", offset + moduleFolderLength);
...
CHyperFlowGraph* CFlowGraphManager::FindGraphByName(tukk flowGraphName)
{
	for (i32 i = 0; i < m_graphs.size(); ++i)
	{
...
namespace
{
void PyOpenFlowGraphView(tukk flowgraph)
{
...
void PyOpenFlowGraphViewAndSelect(tukk flowgraph, tukk entityName)
{
	CHyperFlowGraph* pFlowGraph = GetIEditorImpl()->GetFlowGraphManager()->FindGraphByName(flowgraph);
