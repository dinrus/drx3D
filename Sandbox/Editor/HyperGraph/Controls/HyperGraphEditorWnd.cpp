...
#include <DrxAction/ICustomActions.h>
...
namespace
{
i32k FlowGraphLayoutVersion = 0x0004;   // bump this up on every substantial pane layout change
}
...
namespace {
tukk g_EntitiesFolderName = "Entities";
tukk g_AIActionsFolderName = "AI Actions";
tukk g_UIActionsFolderName = "UI Actions";
tukk g_CustomActionsFolderName = "Custom Actions";
tukk g_FGModulesFolderName = "FG Modules";
tukk g_MaterialFXFolderName = "Material FX";
...
class CHyperGraphViewClass : public IViewPaneClass
{
	...
	virtual tukk    ClassName()       override { return "Flow Graph"; };
	...
	virtual tukk    GetPaneTitle()    override { return _T("Flow Graph"); };
...
//////////////////////////////////////////////////////////////////////////
BOOL CHyperGraphDialog::OnCmdMsg(UINT nID, i32 nCode, uk pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
	BOOL res = FALSE;
	if (m_view.m_hWnd)
	{
...
BOOL CHyperGraphDialog::OnInitDialog()
{
	...
	i32 paneLayoutVersion = regMgr.GetProfileInt(_T("FlowGraph"), _T("FlowGraphLayoutVersion"), 0);
...
void CHyperGraphDialog::OnViewSelectionChange()
{
	...
	i32 h = m_pWndProps->GetVisibleHeight();
...
void CHyperGraphDialog::SaveGraph(CHyperGraph* pGraph /*, bool bCopyToFile*/)
{
	...
	else
	{
		if (pGraph->IsFlowGraph())
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed to save graph '%s' to '%s'", pGraph->GetName(), filename);
			}
		}
...
void CHyperGraphDialog::OnFileNewUIAction()
{
	...
	{
		IUIAction* pUIAction = gEnv->pFlashUI->GetUIAction(PathUtil::GetFileName((tukk)filename));
		if (pUIAction)
		{
			CFlowGraphManager* pManager = GetIEditorImpl()->GetFlowGraphManager();
...
void CHyperGraphDialog::OnFileNewCustomAction()
{
	...
	if (GetIEditorImpl()->GetCustomActionManager()->NewCustomAction(filename))
	{
		...
		DRX_ASSERT(pCustomActionManager != NULL);
		if (!pCustomActionManager)
			return;

		ICustomAction* pCustomAction = pCustomActionManager->GetCustomActionFromLibrary(PathUtil::GetFileName((tukk)filename));
		if (pCustomAction)
		{
...
void CHyperGraphDialog::OnGraphsRightClick(NMHDR* pNMHDR, LRESULT* pResult)
{
	*pResult = TRUE;
	...
	{
		i32 nCmd = menu.TrackPopupMenu(TPM_RETURNCMD | TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_NONOTIFY, screenPoint.x, screenPoint.y, this);
	...
	*pResult = 0;
	...
	if (GetCursorPos(&screenPoint))
	{
		...
		if (hCurrentItem && (TVHT_ONITEM & uFlags))
		{
			...
			{
				i32k nCmd = menu.TrackPopupMenu(TPM_RETURNCMD | TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_NONOTIFY, screenPoint.x, screenPoint.y, this);
...
i32 CHyperGraphDialog::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
...
CXTPControlStatic* FindInfoControl(CXTPControl* pControl)
{
	...
	for (i32 i = 0; i < pCommandBar->GetControls()->GetCount(); i++)
	{
...
i32 CalcWidth(CListBox* pListBox)
{
	i32 nMaxTextWidth = 0;
	i32 nScrollWidth = 0;
	...
	if (pListBox->GetCount() > 1 && ((i32)scrollInfo.nMax >= (i32)scrollInfo.nPage))
	{
	...
	if (pDC)
	{
		...
		CString Text;
		i32k nItems = pListBox->GetCount();
		for (i32 i = 0; i < nItems; ++i)
		{
			pListBox->GetText(i, Text);
			i32k nTextWidth = pDC->GetTextExtent(Text).cx;
	...
	if (pControlListBox)
	{
		...
		if (forward)
		{
			...
			while (iterStart != iterEnd)
			{
				...
				i32 index = pListBox->AddString(prettyName);
		...
		i32 width = CalcWidth(pListBox) + 4;
...
bool CHyperGraphDialog::CreateContextMenu(CMenu& roMenu, TDTreeItems& rchSelectedItems)
{
	...
	i32 nTotalTreeItems(0);
	i32 nCurrentTreeItem(0);
...
//////////////////////////////////////////////////////////////////////////
void CHyperGraphDialog::ProcessMenu(i32 nMenuOption, TDTreeItems& rchSelectedItem)
{
	...
	i32 nTotalSelectedItems(0);
	i32 nCurrentSelectedItem(0);
		...
		case ID_GRAPHS_REMOVE_GRAPH:
			{
				...
				if (nCurrentSelectedItem == nTotalSelectedItems - 1 && !cSelectedGraphs.empty())
				{
					...
					{
						str += (tukk)cSelectedGraphs[i]->GetName();
						str += "\n";
					}
...
void CHyperGraphDialog::ProcessBreakpointMenu(i32 nMenuOption, HTREEITEM hItem)
{
	switch (nMenuOption)
	{
