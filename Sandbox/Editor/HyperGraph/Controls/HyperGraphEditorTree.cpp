...
struct SHGTCReloadItem
{
	...
	SHGTCReloadItem(CString name, i32 image, HTREEITEM parent, CHyperFlowGraph* pFlowGraph)
...
i32 CALLBACK SortAlphabeticallyWithFoldersFirstCB(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
...
void CHyperGraphsTreeCtrl::UpdateGraphLabel(CHyperGraph* pGraph)
{
	if (pGraph && !m_bIgnoreReloads)
	{
		...
		if (pGraph->IsFlowGraph())
		{
			...
			i32 img = bIsDisabled ? eFGI_Disabled : pFlowGraph->GetIFlowGraph()->GetType();
			if (bHasError)
...
void CHyperGraphsTreeCtrl::AddGraphItemToTree(bool isRebuild, CString graphName, CHyperFlowGraph* pFlowGraph, EFlowGraphImage eType, HTREEITEM hParentfolder /*= NULL*/)
{
	...
	i32 img = pFlowGraph->IsEnabled() ? (eType + (pFlowGraph->HasError() ? eFGI_ErrorOffset : 0)) : eFGI_Disabled;
