...
void CHGGraphPropsPanel::Init(i32 nID)
{
	...
	m_varMultiPlayer->AddEnumItem("ServerOnly", (i32) CHyperFlowGraph::eMPT_ServerOnly);
	m_varMultiPlayer->AddEnumItem("ClientOnly", (i32) CHyperFlowGraph::eMPT_ClientOnly);
	m_varMultiPlayer->AddEnumItem("ClientServer", (i32) CHyperFlowGraph::eMPT_ClientServer);
...
void CHGGraphPropsPanel::ResizeProps(bool bHide)
{
	...
	i32 h = m_graphProps.GetVisibleHeight();
...
void CHGGraphPropsPanel::OnVarChange(IVariable* pVar)
{
	...
	CString val;
	pVar->Get(val);
	DrxLogAlways("Var %s changed to %s", pVar->GetName(), val.GetString());
#endif
	m_pGraph->SetEnabled(m_varEnabled);
	m_pGraph->SetMPType((CHyperFlowGraph::EMultiPlayerType) (i32) m_varMultiPlayer);
...
void CHGSelNodeInfoPanel::Init(i32 nID)
{
	m_pGroup = m_pTaskPanel->AddGroup(nID);
