...
static i32k FLOWGRAPH_ICON = 0;
static i32k FLOWNODE_ICON = 1;
static i32k BREAKPOINT_ICON = 2;
static i32k OUTPUT_PORT_ICON = 3;
static i32k INPUT_PORT_ICON = 4;
static i32k DISABLED_BREAKPOINT = 5;
static i32k TRACEPOINT = 6;
...
BOOL CBreakpointsTreeCtrl::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	...
	if (m_pFlowGraphDebugger)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Could not register as Flowgraph Debugger listener! [CBreakpointsTreeCtrl::Create]");
		}
		...
		if (hGraphItem)
		{
			...
			if (nodeItem)
			{
				...
				if (!portItem)
				{
					if (pPort)
					{
						i32k icon = breakpoint.addr.isOutput ? OUTPUT_PORT_ICON : INPUT_PORT_ICON;
...
void CBreakpointsTreeCtrl::OnEnableBreakpoint(const SBreakPoint& breakpoint, bool enable)
{
	...
	if (hPortItem)
	{
		i32k defaultIcon = breakpoint.addr.isOutput ? OUTPUT_PORT_ICON : INPUT_PORT_ICON;
		i32 icon = defaultIcon;
...
void CBreakpointsTreeCtrl::OnEnableTracepoint(const STracePoint& tracepoint, bool enable)
{
	...
	{
		i32k defaultIcon = tracepoint.addr.isOutput ? OUTPUT_PORT_ICON : INPUT_PORT_ICON;
		i32k icon = enable ? TRACEPOINT : defaultIcon;
		SetItemImage(hPortItem, icon, icon);
	}
}
