...
public:
	...
	bool GetNodesInRect(const CRect& viewRect, std::multimap<i32, CHyperNode*>& nodes, bool bFullInside = false);
	...
	void        SetComponentViewMask(u32 mask) { m_componentViewMask = mask; }
...
private:
	...
	Gdiplus::RectF DrawArrow(Gdiplus::Graphics& gr, CHyperEdge* pEdge, bool helper, i32 selection = 0);
	...
	void PopulateSubClassMenu(SClassMenuGroup& classMenu, std::vector<CString>& classes, i32 baseIndex);
	...
	bool HitTestEdge(CPoint pnt, CHyperEdge*& pEdge, i32& nIndex);
...
private:
	...
	struct TDraggingBorderNodeInfo
	{
		...
		i32             m_border; // actually is an EHyperNodeSubObjectId, but is managed as i32 in many other places, so
