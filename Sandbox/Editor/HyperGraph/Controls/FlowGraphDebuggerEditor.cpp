...
#include <DrxGame/IGameFramework.h>  //Pause game
#include <DrxInput/IHardwareMouse.h> //SetGameMode
...
bool CFlowGraphDebuggerEditor::Init()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Flowgraph Manager doesn't exist [CFlowgraphDebugger::Init]");
		return false;
	}
	...
	if (gEnv->pFlowSystem)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Could not register as Flowgraph Debugger listener! [CFlowgraphDebugger::Init]");
		}
	}
	else
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Flowgraph System doesn't exist [CFlowgraphDebugger::Init]");
		return false;
	}
...
bool CFlowGraphDebuggerEditor::Shutdown()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Flowgraph Manager doesn't exist [CFlowgraphDebugger::~CFlowgraphDebugger]");
		bError = true;
	}
	...
	if (pGraph)
	{
		if (CFlowNode* pNode = pGraph->FindFlowNode(tracepoint.addr.node))
		{
			...
			if (pPort)
			{
				...
				DrxLog("$2[TRACEPOINT HIT - FrameID: %d] GRAPH: %s (ID: %d) - NODE: %s (ID: %d) - PORT: %s - VALUE: %s", gEnv->pRenderer->GetFrameID(), pGraph->GetName(), pGraph->GetIFlowGraph()->GetGraphId(), pNode->GetClassName(), pNode->GetId(), pNode->GetPortName(*pPort), val.c_str());
			}
		}
	}
}
