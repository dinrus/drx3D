...
//////////////////////////////////////////////////////////////////////////
void CFlowGraphEditModuleDlg::OnCommand_ReorderInput(i32 oldPos, i32 newPos)
{
...
{
	i32 selected = m_outputListCtrl.GetSelCount();
	CArray<i32, i32> listSelections;
	listSelections.SetSize(selected);
...
//////////////////////////////////////////////////////////////////////////
void CFlowGraphEditModuleDlg::OnCommand_ReorderOutput(i32 oldPos, i32 newPos)
{
...
void CFlowGraphNewModuleInputDlg::OnOK()
{
	if (m_pPort)
	{
		...
		{
			DrxMessageBox("The port can not have type Void.", "Error Invalid Port Type", eMB_Error);
			return;
		}

		if (dialogName.empty())
		{
			DrxMessageBox("The port name can not be empty.", "Error Invalid Port Name", eMB_Error);
			return;
		}
		...
		{
			DrxMessageBox("The port name must start with an A-Z letter, ':' or '_'.", "Error Invalid Port Name", eMB_Error);
			return;
		}
		...
		if (m_pPort->input)
		{
			...
			{
				DrxMessageBox("Invalid port name! A default port with that name already exists.", "Error Invalid Port Name", eMB_Error);
				return;
			}
		}
		else
		...
		for (TPorts::const_iterator it = m_pOtherPorts->begin(), end = m_pOtherPorts->end(); it != end; ++it)
		{
			...
			{
				DrxMessageBox("Invalid port name! A port with that name already exists.", "Error Invalid Port Name", eMB_Error);
				return;
			}
		}
