...
void CFlowGraphTokensCtrl::Init(i32 nID)
{
...
CSmartVariableEnum<CString> CFlowGraphTokensCtrl::CreateVariable(tukk name, EFlowDataTypes type)
{
...
{
	i32 selected = m_tokenListCtrl.GetSelCount();
	std::vector<i32> listSelections(selected);
...
BOOL CFlowGraphNewTokenDlg::OnInitDialog()
{
	...
	for (i32 i = 1; i < sizeof(typeNames) / sizeof(CString); ++i)
	{
...
void CFlowGraphNewTokenDlg::OnOK()
{
	...
	if (m_pTokenData)
	{
		...
		if (m_pTokenData->name == newTokenName || !DoesTokenExist(newTokenName))
		{
			...
			for (i32 i = 0; i < sizeof(typeNames) / sizeof(CString); ++i)
			{
