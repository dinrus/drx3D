...
void CFlowGraphSearchOptions::Serialize(bool bLoading)
{
	...
	else
	{
		...
		i32 count = regMgr.GetProfileInt(strSection, _T("Count"), 0);
...
void AFXAPI _DDX_CBString(CDataExchange* pDX, i32 nIDC, CString& value)
{
	HWND hWndCtrl = pDX->PrepareCtrl(nIDC);
	if (pDX->m_bSaveAndValidate)
	{
		// Get current edit item text (or drop list static)
		i32 nLen = ::GetWindowTextLength(hWndCtrl);
...
void AddComboHistory(CComboBox& cmb, CString strText, CStringList& lstHistory)
{
	...
	i32 nIndex = cmb.FindString(-1, strText);
	if (nIndex == -1)
	{
...
public:
	CSearchResult(CHyperFlowGraph* pGraph, HyperNodeID nodeId, CString context)
	{
		...
		i32 hyPos = context.Find(delimiter);
		if (hyPos >= 0)
		{
			hyPos += delimiter.GetLength();
			i32 len = context.GetLength() - hyPos;
...
u32 Option2Cat(CFlowGraphSearchOptions::EFindSpecial o)
{
	switch (o)
	{
