...
//////////////////////////////////////////////////////////////////////////
void CHyperGraphView::OnSize(UINT nType, i32 cx, i32 cy)
{
...
//////////////////////////////////////////////////////////////////////////
bool CHyperGraphView::HitTestEdge(CPoint pnt, CHyperEdge*& pOutEdge, i32& nIndex)
{
	if (!m_pGraph)
		return false;
...
void CHyperGraphView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (pNode)
	{
		i32 objectClicked = pNode->GetObjectAt(GraphicsHelper(this), ViewToLocal(point));

		CHyperNodePort* pPort = NULL;
		static i32k WAS_A_PORT = -10000;
		static i32k WAS_A_SELECT = -10001;
		...
		else if (strcmp(pNode->GetClassName(), CBlackBoxNode::GetClassType()) == 0)
		{
			...
			else if (objectClicked == -1)
			{
				...
				if (pBB->IsMinimized() == false)
				{
					...
					{
						i32 clicked = ((*nodes)[i])->GetObjectAt(GraphicsHelper(this), ViewToLocal(point));
						if (clicked > 0)
						{
							pNode = (*nodes)[i];
		...
		case WAS_A_PORT:
			...
			else
			{
				...
				CPoint pt;
				GetCursorPos(&pt);
				i32 nEdge = menu.TrackPopupMenuEx(TPM_RETURNCMD, pt.x, pt.y, this, NULL);
				if (nEdge)
				{
					nEdge--;
...
//////////////////////////////////////////////////////////////////////////
bool CHyperGraphView::CheckVirtualKey(i32 virtualKey)
{
	...
	case SelectMode:
		{
			if (!m_rcSelect.IsRectEmpty())
			{
				...
				SStartNodeOperationBatching startBatching(this);
				std::multimap<i32, CHyperNode*> nodes;
				GetNodesInRect(m_rcSelect, nodes, true);
				for (std::multimap<i32, CHyperNode*>::reverse_iterator iter = nodes.rbegin(); iter != nodes.rend(); ++iter)
				{
	...
	case MoveMode:
		{
			...
			SStartNodeOperationBatching startBatching(this);
			for (i32 i = 0, iNodeCount(apNodes.size()); i < iNodeCount; ++i)
	...
	case PortDragMode:
		{
			...
			if (pTrgNode)
			{
				i32 objectClicked = pTrgNode->GetObjectAt(GraphicsHelper(this), ViewToLocal(point));
				if (objectClicked >= eSOID_ShiftFirstOutputPort && objectClicked <= eSOID_ShiftLastOutputPort)
				{
	...
	if (m_tooltip.m_hWnd)
	{
		if (pNode && pNode->IsTooltipShowable() && gFlowGraphGeneralPreferences.showToolTip)
		{
			...
			if (pPort) // Port tooltip.
			{
				...
				tukk desc = pPort->pVar->GetDescription();
				if (desc && *desc)
					tip.Format("[%s] %s", (tukk)type, desc);
				else
					tip.Format("[%s] %s", (tukk)type, (tukk)pNode->GetDescription());
			}
			else // Node tooltip.
			{
...
BOOL CHyperGraphView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	if (m_mode == NothingMode)
	{
		...
		if (pNode)
		{
			i32 object = pNode->GetObjectAt(GraphicsHelper(this), ViewToLocal(point));
...
void CHyperGraphView::DrawEdges(Gdiplus::Graphics& gr, const CRect& rc)
{
	...
	for (std::vector<CHyperEdge*>::const_iterator it = edges.begin(), end = edges.end(); it != end; ++it)
	{
		...
		// Draw arrow.
		i32 selection = 0;
		selection |= (i32)(gFlowGraphGeneralPreferences.highlightEdges && nodeIn->IsSelected());
		selection |= (i32)(gFlowGraphGeneralPreferences.highlightEdges && nodeOut->IsSelected()) << 1;
		if (selection == 0)
		...
		i32 customSelMode = pEdge->GetCustomSelectionMode();
...
void CHyperGraphView::DrawNodes(Gdiplus::Graphics& gr, const CRect& rc)
{
	...
	for (std::multimap<i32, CHyperNode*>::iterator iter = nodes.begin(); iter != nodes.end(); ++iter)
	{
		CHyperNode* pNode = iter->second;
...
void CHyperGraphView::SetGraph(CHyperGraph* pGraph)
{
	...
	if (m_pGraph)
	{
		// DrxLogAlways("CHyperGraphView: (1) Removing as listener from 0x%p before switching to 0x%p", m_pGraph, pGraph);
		m_pGraph->RemoveListener(this);
		// DrxLogAlways("CHyperGraphView: (2) Removing as listener from 0x%p before switching to 0x%p", m_pGraph, pGraph);
	}
...
void CHyperGraphView::ShowAndSelectNode(CHyperNode* pToNode, bool bSelect)
{
	...
	//else
	{
		...
		else
		{
			...
			i32 maxWH = max(rcGraph.Width(), rcGraph.Height());
			if (maxWH > 0)
			{
...
//////////////////////////////////////////////////////////////////////////
Gdiplus::RectF CHyperGraphView::DrawArrow(Gdiplus::Graphics& gr, CHyperEdge* pEdge, bool helper, i32 selection)
{
	DRX_PROFILE_FUNCTION(PROFILE_EDITOR);
...
//////////////////////////////////////////////////////////////////////////
bool CHyperGraphView::GetNodesInRect(const CRect& viewRect, std::multimap<i32, CHyperNode*>& nodes, bool bFullInside)
{
	if (!m_pGraph)
		return false;
	bool bFirst = true;
...
void CHyperGraphView::OnToggleMinimizeNode(CHyperNode* pNode)
{
	...
	if (!bMinimize)
	{
		for (i32 i = 0; i < pNode->GetInputs()->size(); i++)
		{
	...
	if (!bMinimize)
	{
		for (i32 i = 0; i < pNode->GetOutputs()->size(); i++)
		{
...
{ 
	...
	if (cmd >= BASE_OUTPUTS_CMD && cmd < BASE_OUTPUTS_CMD + 1000)
	{
		if (pNode)
		{
			CUndo undo("Graph Port Visibilty");
			pNode->RecordUndo();
			i32 nPort = cmd - BASE_OUTPUTS_CMD;
...
{
public:
	ViewNodeFilter(u32 mask) : mask(mask) {}
...
//////////////////////////////////////////////////////////////////////////
void CHyperGraphView::PopulateSubClassMenu(SClassMenuGroup& classMenu, std::vector<CString>& classes, i32 baseIndex)
{
...
void CHyperGraphView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	m_scrollOffset.x += si.nPos - min(i32(si.nMax - si.nPage), max(i32(si.nMin), curPos));
	SetScrollExtents(false);
	Invalidate();
...
void CHyperGraphView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	m_scrollOffset.y += si.nPos - min(i32(si.nMax - si.nPage), max(i32(si.nMin), curPos));
	SetScrollExtents(false);
	Invalidate();
...
void CHyperGraphView::RenameNode(CHyperNode* pNode)
{
	...
	i32 nEditFlags = WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL;
	if (pNode->IsEditorSpecialNode())
	{
...
void CHyperGraphView::PasteLinks(CHyperNode* pNode, bool isInput)
{
	...
	if (edgesXml)
	{
		...
		for (i32 i = 0; i < edgesXml->getChildCount(); i++)
		{
...
void CHyperGraphView::ShowEditPort(CHyperNode* pNode, CHyperNodePort* pPort)
{
	...
	if (pVarBlock)
	{
		...
		i32 w = RightBottom.x - LeftTop.x - 2;
		i32 h = MAX(m_editParamCtrl.GetVisibleHeight(), RightBottom.y - LeftTop.y);
