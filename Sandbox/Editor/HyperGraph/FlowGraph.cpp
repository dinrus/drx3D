...
public:
	CUndoFlowGraph(CHyperFlowGraph* pGraph)
	{
		...
		DrxLogAlways("CUndoFlowGraph 0x%p:: Serializing from graph 0x%p", this, pGraph);
#endif
	}
...
protected:
	...
	virtual tukk GetDescription() { return "FlowGraph Undo"; };
	...
	virtual void        Undo(bool bUndo)
	{
		...
		{
#ifdef FG_INTENSIVE_DEBUG
			DrxLogAlways("CUndoFlowGraph 0x%p::Undo 1: Serializing to graph 0x%p", this, pGraph);
#endif
...
#ifdef FG_INTENSIVE_DEBUG
		DrxLogAlways("CUndoFlowGraph 0x%p::Undo 2: Serializing to graph 0x%p", this, pGraph);
#endif
	...
	virtual void Redo()
	{
		if (m_redo)
		{
			...
			{
#ifdef FG_INTENSIVE_DEBUG
				DrxLogAlways("CUndoFlowGraph 0x%p::Redo: Serializing to graph 0x%p", this, pGraph);
#endif
				GetIEditorImpl()->GetPrefabManager()->SetSkipPrefabUpdate(true);
			...
			{
				DrxLogAlways("CUndoFlowGraph 0x%p: Got 0 graph", this);
			}
		}
	}
private:
...
CHyperFlowGraph::CHyperFlowGraph(CHyperGraphManager* pManager, tukk sGroupName/* = "" */, CEntityObject* pEntity /*= nullptr*/)
...
{
#ifdef FG_INTENSIVE_DEBUG
	DrxLogAlways("CHyperFlowGraph: Creating 0x%p", this);
#endif
...
{
#ifdef FG_INTENSIVE_DEBUG
	DrxLogAlways("CHyperFlowGraph: About to delete 1 0x%p", this);
#endif
...
#ifdef FG_INTENSIVE_DEBUG
	DrxLogAlways("CHyperFlowGraph: About to delete 2 0x%p", this);
#endif
}
...
bool CHyperFlowGraph::OnFlowNodeActivation(IFlowGraphPtr pFlowGraph, TFlowNodeId srcNode, TFlowPortId srcPort, TFlowNodeId toNode, TFlowPortId toPort, tukk value)
{
...
{
	{
		...
		if (bLoading)
		{
			...
			tukk mpTypeAttr = node->getAttr("MultiPlayer");
			if (mpTypeAttr)
			{
...
bool CHyperFlowGraph::Migrate(XmlNodeRef& node)
{
	...
	if (report.size() > 0)
	{
		for (i32 i = 0; i < report.size(); ++i)
		{
			const CFlowGraphMigrationHelper::ReportEntry& entry = report[i];
			DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, entry.description);
			DrxLogAlways("CHyperFlowGraph::Migrate: FG '%s': %s", GetName(), entry.description.GetString());
		}
	}
	return bChanged;
}
...
bool CHyperFlowGraph::CanConnectPorts(CHyperNode* pSrcNode, CHyperNodePort* pSrcPort, CHyperNode* pTrgNode, CHyperNodePort* pTrgPort, CHyperEdge* pExistingEdge)
{
	...
	if (bCanConnect && m_pGameFlowGraph)
	{
		...
		if (srcConfig.pOutputPorts && trgConfig.pInputPorts)
		{
			...
			i32 srcType = pSrcPortConfig->type;
			i32 trgType = pTrgPortConfig->defaultData.GetType();
...
void CHyperFlowGraph::ExtractObjectsPrefabIdToGlobalIdMappingFromPrefab(CObjectArchive& archive)
{
	if (CEntityObject* pEntity = GetEntity())
	{
		if (CPrefabObject* pPrefabObject = (CPrefabObject*)pEntity->GetPrefab())
		{
			...
			for (i32 i = 0, count = childs.size(); i < count; ++i)
