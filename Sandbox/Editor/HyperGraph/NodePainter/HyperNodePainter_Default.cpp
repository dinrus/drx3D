...
static i32k NumPortColors = sizeof(PortTypeToColor) / sizeof(*PortTypeToColor);
...
struct SAssets
{
		...
		brushNodeTitleError(NODE_TITLE_ERROR_COLOR)
	{
		...
		for (i32 i = 0; i < NumPortColors; i++)
		{
			brushPortFill[i] = new Gdiplus::SolidBrush(PortTypeToColor[i] | (i32)GET_TRANSPARENCY << Gdiplus::Color::AlphaShift);
		}
	}
	~SAssets()
	{
	...
	void Update()
	{
		...
		for (i32 i = 0; i < NumPortColors; i++)
		{
			brushPortFill[i]->SetColor(PortTypeToColor[i] | (i32)GET_TRANSPARENCY << Gdiplus::Color::AlphaShift);
		}
	}
