...
#include <DrxAISystem/ICommunicationManager.h>
...
IEntity* GetRefEntityByUIConfig(CFlowNodeGetCustomItemsBase* pGetCustomItems, const string& refEntity)
{
	...
	const string& uiConfig = pGetCustomItems->GetUIConfig();
	i32 pos = uiConfig.Find(refEntity);
	if (pos >= 0)
	{
...
IEntity* GetNodeEntity(IVariable* pVar, CFlowNodeGetCustomItemsBase*& pGetCustomItems)
{
	...
	DRX_ASSERT(pGetCustomItems != NULL);
...
bool ChooseCharacterAnimation(IVariable* pVar, std::vector<SItem>& outItems, string& outText)
{
	...
	u32k numAnims = pAnimSet->GetAnimationCount();
	for (i32 i = 0; i < numAnims; ++i)
	{
		tukk pName = pAnimSet->GetNameByAnimID(i);
		if (pName != 0)
		{
...
bool ChooseBone(IVariable* pVar, std::vector<SItem>& outItems, string& outText)
{
	...
	{
		tukk pName = rIDefaultSkeleton.GetJointNameByID(i);
		if (pName != 0)
		{
			outItems.push_back(SItem(pName));
		}
...
bool ChooseAttachment(IVariable* pVar, std::vector<SItem>& outItems, string& outText)
{
	...
	i32k numAttachment = pMgr->GetAttachmentCount();
	for (u32 i = 0; i < numAttachment; ++i)
	{
...
bool FragmentIDContainsLoopingFragments(const IActionController* pActionController, FragmentID fragmentID)
{
	...
	u32k numTagSets = pDatabase->GetTotalTagSets(fragmentID);
	for (u32 tagSetIndex = 0; tagSetIndex < numTagSets; ++tagSetIndex)
	{
		SFragTagState tagSetTagState;
		u32 numOptions = pDatabase->GetTagSetInfo(fragmentID, tagSetIndex, tagSetTagState);
		if (tagSetTagState.fragmentTags == TAG_STATE_EMPTY)
		{
			for (u32 optionIndex = 0; optionIndex < numOptions; ++optionIndex)
			{
...
bool IsFragmentIDAllowedForMethod(const IActionController* pActionController, FragmentID fragmentID, u32 method)
{
	...
	if (gEnv->pGameFramework)
	{
		...
		if (pActionController)
		{
			...
			for (FragmentID fragmentID = 0; fragmentID < fragmentIDs.GetNum(); ++fragmentID)
			{
				...
				{
					tukk szFragmentName = fragmentIDs.GetTagName(fragmentID);
...
bool ChooseEntityProperties(IVariable* pVar, std::vector<SItem>& outItems, string& outText)
{
	...
	if (pObj)
	{
		...
		if (pEntityObject->GetProperties())
		{
			...
			for (i32 i = 0; i < pEntityProperties->GetNumVariables(); i++)
			{
				IVariable* currVar = pEntityProperties->GetVariable(i);

				if (currVar->GetNumVariables() > 0)
				{
					for (i32 j = 0; j < currVar->GetNumVariables(); j++)
					{
			...
			if (pEntityObject->GetProperties2())
			{
				...
				for (i32 i = 0; i < pEntityProperties2->GetNumVariables(); i++)
				{
...
//////////////////////////////////////////////////////////////////////////
bool GetMatParams(IEntity* pEntity, i32 slot, i32 subMtlId, DynArrayRef<SShaderParam>** outParams)
{
...
//////////////////////////////////////////////////////////////////////////
void FillOutParamsByShaderParams(std::vector<SItem>& outItems, DynArrayRef<SShaderParam>* params, i32 limit)
{
	if (limit == 1)
	{
...
{
	u32 formationNameCount = 0;
	tukk formationNames[32];
	gEnv->pAISystem->EnumerateFormationNames(32, formationNames, &formationNameCount);
	for (u32 i = 0; i < formationNameCount; ++i)
	{
...
{
	i32k count = gEnv->pFlashUI->GetUIActionCount();
	for (i32 i = 0; i < count; ++i)
	{
...
void FillUISubItems(std::vector<SItem>& outItems, Item* pItem, const string& itemname)
{
	...
	{
		i32k count = SUIGetDesc<T1, Item, i32>::GetCount(pItem);
		for (i32 i = 0; i < count; ++i)
		{
			const typename SUIDescTypeOf<T1>::TType * pDesc = SUIGetDesc<T1, Item, i32>::GetDesc(pItem, i);
			string desc;
	...
	i32k subcount = SUIGetDesc<T2, Item, i32>::GetCount(pItem);
	for (i32 i = 0; i < subcount; ++i)
	{
		const typename SUIDescTypeOf<T2>::TType * pDesc = SUIGetDesc<T2, Item, i32>::GetDesc(pItem, i);
		string subitemname;
...
bool ChooseUIMovieClips(IVariable* pVar, std::vector<SItem>& outItems, string& outText)
{
	...
	if (gEnv->pFlashUI)
	{
		...
		{
			IUIElement* pElement = gEnv->pFlashUI->GetUIElement(i);
			i32k count = pElement->GetMovieClipCount();
			if (!pElement->IsValid()) continue;
...
bool ChooseUITemplate(IVariable* pVar, std::vector<SItem>& outItems, string& outText)
{
	...
	if (gEnv->pFlashUI)
	{
		...
		{
			IUIElement* pElement = gEnv->pFlashUI->GetUIElement(i);
			i32k count = pElement->GetMovieClipTmplCount();
			if (!pElement->IsValid()) continue;
...
struct CFlowNodeCustomItemsImpl : public CFlowNodeGetCustomItemsBase
{
	...
	CFlowNodeCustomItemsImpl(const TGetItemsFct& fct, bool tree, tukk sep) : m_fct(fct), m_tree(tree), m_sep(sep) {}
	...
	virtual tukk GetTreeSeparator()                                                            { return m_sep; }
	TGetItemsFct m_fct;
	bool         m_tree;
	tukk  m_sep;
};
...
static i32k prefix_dataType_numEntries = sizeof(prefix_dataType_table) / sizeof(FlowGraphVariables::MapEntry);

namespace FlowGraphVariables
{
const MapEntry* FindEntry(tukk sPrefix)
{
	for (i32 i = 0; i < prefix_dataType_numEntries; ++i)
	{
...
CUIEnumsDatabase_SEnum* CVariableFlowNodeDynamicEnum::GetSEnum()
{
	...
	assert(pGetCustomItems != 0);
	// DrxLogAlways("CVariableFlowNodeDynamicEnum::GetSEnum 0x%p pGetCustomItems=0x%p FlowNode=0x%p", this, pGetCustomItems, pGetCustomItems->GetFlowNode());
...
CUIEnumsDatabase_SEnum* CVariableFlowNodeDefinedEnum::GetSEnum()
{
	...
	assert(pGetCustomItems != 0);
	// DrxLogAlways("CVariableFlowNodeDefinedEnum::GetSEnum 0x%p pGetCustomItems=0x%p FlowNode=0x%p", this, pGetCustomItems, pGetCustomItems->GetFlowNode());
	...
	if (pCFlowNode)
	{
		...
		if (pIFlowNode)
		{
			...
			u32k portId = m_portId - ((config.nFlags & EFLN_TARGET_ENTITY) ? 1 : 0);
...
{
	m_pDynEnum = pDynEnum;
	// DrxLogAlways("CDynamicEnumList::ctor 0x%p", this);
}

//////////////////////////////////////////////////////////////////////////
CVariableFlowNodeCustomEnumBase::CDynamicEnumList::~CDynamicEnumList()
{
	// DrxLogAlways("CDynamicEnumList::dtor 0x%p", this);
}

//! Get the name of specified value in enumeration.
tukk CVariableFlowNodeCustomEnumBase::CDynamicEnumList::GetItemName(uint index)
{
