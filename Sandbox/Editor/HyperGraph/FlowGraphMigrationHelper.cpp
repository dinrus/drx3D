...
struct CInputValueCondition : public CFlowGraphMigrationHelper::ICondition
{
	...
	bool Fulfilled(XmlNodeRef node, CFlowGraphMigrationHelper::NodeEntry& entry)
	{
		...
		for (i32 j = 0; j < inputsXml->getNumAttributes(); j++)
		{
			tukk key;
			tukk value;
...
CFlowGraphMigrationHelper::Substitute(XmlNodeRef node)
{
	if (!m_bLoaded)
	{
		if (gEnv->pDrxPak->IsFileExist(DEPRECATIONS_FILE_PATH))
		{
	...
	if (nodesXml)
	{
		HyperNodeID nodeId;
		CString nodeclass;
		for (i32 i = 0; i < nodesXml->getChildCount(); i++)
		{
					...
					for (i32 j = 0; j < inputsXml->getNumAttributes(); j++)
					{
						...
						if (success)
						{
							...
							if (portIter != nodeEntry.inputPortMappings.end())
							{
								...
								// DrxLogAlways("PortEntry: key=%s value=%s newval=%s", key,value,portEntry.newName.GetString());
...
void CFlowGraphMigrationHelper::AddEntry(XmlNodeRef node)
{
	...
	for (i32 i = 0; i < node->getChildCount(); i++)
	{
		...
		{
			// collect new inputs
			for (i32 j = 0; j < portNode->getNumAttributes(); j++)
			{
				tukk key;
				tukk value;
