...
//////////////////////////////////////////////////////////////////////////
void CFlowNode::SetName(tukk sName)
{
...
//////////////////////////////////////////////////////////////////////////
void CFlowNode::DebugPortActivation(TFlowPortId port, tukk value,  bool bIsInitializationStep)
{
...
{
	tukk value = stl::find_in_map(m_portActivationMap, pp.nPortIndex, NULL);
...
void CFlowNode::SetInputs(bool bActivate, bool bForceResetEntities)
{
	...
	for (i32 i = startPort; i < m_inputs.size(); i++)
	{
		IVariable* pVar = m_inputs[i].pVar;
		i32 type = pVar->GetType();
...
//////////////////////////////////////////////////////////////////////////
tukk CFlowNode::GetDescription() const
{
...
//////////////////////////////////////////////////////////////////////////
tukk CFlowNode::GetUIClassName() const
{
...
void CFlowNode::Serialize(XmlNodeRef& node, bool bLoading, CObjectArchive* ar)
{
	...
	if (bLoading)
	{
			...
			for (i32 i = 0; i < m_inputs.size(); i++)
			{
				...
				if (pVar->GetType() != IVariable::UNKNOWN)
				{
					if (portsNode->haveAttr(pVar->GetName()) == false)
					{
						...
						tukk sVarName = pVar->GetName();
						tukk sSpecial = strchr(sVarName, '_');
						if (sSpecial)
							sVarName = sSpecial + 1;
						tukk val = portsNode->getAttr(sVarName);
						if (val[0])
						{
							pVar->Set(val);
	#ifdef FG_WARN_ABOUT_STRIPPED_PORTNAMES
							DrxLogAlways("CFlowNode::Serialize: FG '%s': Value of Deprecated Variable %s -> %s successfully resolved.",
							             GetGraph()->GetName(), sVarName, pVar->GetName());
							DrxLogAlways(" --> To get rid of this warning re-save flowgraph!", sVarName);
	#endif
						}
	#ifdef FG_WARN_ABOUT_STRIPPED_PORTNAMES
						else
						{
							DrxLogAlways("CFlowNode::Serialize: FG '%s': Can't resolve value for '%s' <may not be severe>", GetGraph()->GetName(), sVarName);
						}
	#endif
					}
				}
		...
		if (CheckFlag(EHYPER_NODE_ENTITY))
		{
			...
			else
			{
				if (node->getAttr("EntityGUID", m_entityGuid))
				{
					...
					else
					{
						...
						{
							SetEntity(NULL);
							DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_WARNING, "FlowGraph %s Node %d (Class %s) targets unknown entity %s",
			...
			{
				tukk sEntity = node->getAttr("GraphEntity");
				i32 index = atoi(node->getAttr("GraphEntity"));
				if (index == 0)
				{
					SetFlag(EHYPER_NODE_GRAPH_ENTITY, true);
	...
	else // saving
	{
		...
		else
		{
			...
			if (m_entityGuid != DrxGUID::Null())
			{
				...
				{
					DrxGUID guidInPrefab = m_pEntity->GetIdInPrefab();
...
tukk CFlowNode::GetCategoryName() const
{
	u32k cat = GetCategory();
	switch (cat)
	{
	case EFLN_APPROVED:
		return _T("Release");
...
CVarBlock* CFlowNode::GetInputsVarBlock()
{
	...
	for (i32 i = 0; i < pVarBlock->GetVarsCount(); ++i)
	{
