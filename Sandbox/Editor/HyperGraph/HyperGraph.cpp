...
/*
   ...
   protected:
   virtual tukk GetDescription() { return "HyperNodeUndoCreate"; };
...
bool CHyperGraphSerializer::Serialize(XmlNodeRef& node, bool bLoading, bool bLoadEdges, bool bIsPaste)
{
	...
	else
	{
		...
		if (nodesXml)
		{
			...
			HyperNodeID nodeId;
			i32 nodeCount = nodesXml->getChildCount();
		...
		if (bLoadEdges)
		{
			...
			if (edgesXml)
			{
				...
				bool bEnabled;
				i32 edgeCount = edgesXml->getChildCount();
				for (i32 i = 0; i < edgeCount; i++)
				{
					...
					if (!pNodeIn || !pNodeOut)
					{
						...
						DrxWarning(VALIDATOR_MODULE_FLOWGRAPH, VALIDATOR_ERROR, "Loading Graph '%s': Can't connect edge <%d,%s> -> <%d,%s> because %s",
...
//////////////////////////////////////////////////////////////////////////
i32 CHyperGraph::GetMissingNodesCount() const
{
	return m_iMissingNodes;
}

//////////////////////////////////////////////////////////////////////////
i32 CHyperGraph::GetMissingPortsCount() const
{
...
//////////////////////////////////////////////////////////////////////////
IHyperNode* CHyperGraph::CreateNode(tukk sNodeClass, Gdiplus::PointF& pos)
{
	CHyperNode* pNode = NULL;
	if (strcmp(sNodeClass, "selected_entity") == 0)
	{
		const CSelectionGroup* pSelection = GetIEditorImpl()->GetSelection();
		for (i32 o = 0; o < pSelection->GetCount(); ++o)
		{
			CBaseObject* pObject = pSelection->GetObject(o);
			if (pObject && pObject->IsPartOfPrefab())
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Cannot create flownode for '%s' as it is part of a Prefab.", pObject->GetName().c_str());
				return nullptr;
			}
...
bool CHyperGraph::CanConnectPorts(CHyperNode* pSrcNode, CHyperNodePort* pSrcPort, CHyperNode* pTrgNode, CHyperNodePort* pTrgPort, CHyperEdge* pExistingEdge)
{
	...
	if (!pSrcPort)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Unable to connect output of node '%s' ('%s')", pSrcNode->GetName(), pSrcNode->GetDescription());
		return false;
	}
...
//////////////////////////////////////////////////////////////////////////
bool CHyperGraph::Save(tukk filename)
{
	bool bWasModified = IsModified();

	m_filename = PathUtil::GamePathToDrxPakPath(filename);
	m_filename = PathUtil::AbsolutePathToDrxPakPath(m_filename.GetString());
...
//////////////////////////////////////////////////////////////////////////
bool CHyperGraph::Load(tukk filename)
{
...
//////////////////////////////////////////////////////////////////////////
bool CHyperGraph::Import(tukk filename)
{
	XmlNodeRef graphNode = XmlHelpers::LoadXmlFromFile(filename);
	if (graphNode != NULL && graphNode->isTag("Graph"))
	{
