...
struct MapEntry
{
	tukk                  sPrefix;   // prefix string (including '_' character)
...
public:
	...
	{
		// DrxLogAlways("CFlowNodeGetCustomItemsBase::ctor 0x%p", this);
	}

	virtual ~CFlowNodeGetCustomItemsBase()
	{
		// DrxLogAlways("CFlowNodeGetCustomItemsBase::dtor 0x%p", this);
	}
	...
	void SetUIConfig(tukk sUIConfig)
	{
		m_config = sUIConfig;
	}
...
template<> void CVariableFlowNode<Vec3 >::SetDisplayValue(const string& value)
{
	...
	{
		float r, g, b;
		i32 res = sscanf(value, "%f,%f,%f", &r, &g, &b);
		if (res == 3)
		{
			r /= 255.0f;
			g /= 255.0f;
...
{
public:
	CVariableFlowNodeDefinedEnum(const string& refFormatString, u32 portId)
		: CVariableFlowNodeCustomEnumBase()
	{
		m_refFormatString = refFormatString;
		m_portId = portId;
