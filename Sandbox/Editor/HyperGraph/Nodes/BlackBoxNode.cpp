...
void CBlackBoxNode::Serialize(XmlNodeRef& node, bool bLoading, CObjectArchive* ar)
{
	...
	if (bLoading)
	{
		...
		if (pMyEditorFG)
		{
			...
			{
				for (i32 i = 0; i < children->getNumAttributes(); ++i)
				{
					tukk key = NULL;
					tukk value = NULL;
	...
	else
	{
		...
		for (i32 i = 0; i < m_nodesIDs.size(); ++i)
		{
...
i32 CBlackBoxNode::GetObjectAt(Gdiplus::Graphics* pGr, Gdiplus::PointF point)
{
	i32 obj = __super::GetObjectAt(pGr, point);
...
CHyperNodePort* CBlackBoxNode::GetPortAtPoint(Gdiplus::PointF point)
{
	...
	for (; it != end; ++it)
	{
		i32 dX = (pos.X + it->second.X) - point.X;
		i32 dY = (pos.Y + it->second.Y) - point.Y;
