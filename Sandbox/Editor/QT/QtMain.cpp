...
static tukk styleSheetPath = "Editor/Styles/stylesheet.qss";
...
void EnableXTDarkSkin()
{
	// Enable dark theme in XT Toolikit
	{
		...
		//CXTPPaintManager::SetCustomTheme(new CDinrusPro 3DTheme());
		BOOL bLoaded = XTPSkinManager()->LoadSkin(PathUtil::Make(PathUtil::GetEnginePath(), "Editor\\Styles\\DrxDark.cjstyles").c_str());
...
void SetVisualStyle()
{
	...
	DrxIcon::SetDefaultTint(QIcon::Normal, GetStyleHelper()->iconTint());
	DrxIcon::SetDefaultTint(QIcon::Disabled, GetStyleHelper()->disabledIconTint());
	DrxIcon::SetDefaultTint(QIcon::Active, GetStyleHelper()->activeIconTint());
	DrxIcon::SetDefaultTint(QIcon::Selected, GetStyleHelper()->iconTint());
}
...
public:
	...
	{
		setWindowIcon(DrxIcon("icons:editor_icon.ico"));
...
i32 main(i32 argc, tuk argv[])
{
	...
	QCoreApplication::setOrganizationName("DinrusPro 3D");
	...
	// initialize MFC
	if (!AfxWinInit(DrxGetCurrentModule(), NULL, GetCommandLineA(), 0))
	{
		return 1;
	}

	bool bDevMode = false;
	i32 waitForSeconds = 0; // Time in seconds that editor waits on startup.
	// Qt initialization
	{
		QStringList arglist;
		...
		i32 waitIndex = arglist.indexOf("-waitfordebugger");
		if (waitIndex >= 0 && arglist.size() > ++waitIndex)
		{
			waitForSeconds = max(0, arglist[waitIndex].toInt());
		}

		i32 dpiIndex = arglist.indexOf("-nodpiscaling");
		if (dpiIndex == -1)
		{
	...
	if (!watcher->addPath(PathUtil::Make(PathUtil::GetEnginePath(), styleSheetPath).c_str()))
	{
		delete watcher;
		watcher = nullptr;
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed to add qss stylesheet to watcher");
	}
	else
	{
