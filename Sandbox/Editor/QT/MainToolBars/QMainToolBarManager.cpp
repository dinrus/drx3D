...
static tukk s_defaultPath = "Editor/ToolBars";
static tukk s_actionPropertyName = "cvarDescValue";
static tukk s_actionCVarBitFlagName = "cvarDescIsBitFlag";
static i32k s_version = 2;

#define ADD_TO_VARIANT_MAP(var, map) map[ # var] = var

QMainToolBarManager::QCommandDesc::QCommandDesc(const QVariantMap& variantMap, i32 version)
...
QMainToolBarManager::QCVarDesc::QCVarDesc(const QVariantMap& variantMap, i32 version)
	: name(variantMap["name"].toString())
	, iconPath(variantMap["iconPath"].toString())
...
QMainToolBarManager::QToolBarDesc::QToolBarDesc(const QVariantList& commandList, i32 version)
{
...
std::shared_ptr<QMainToolBarManager::QItemDesc> QMainToolBarManager::QToolBarDesc::CreateItem(const QVariant& itemVariant, i32 version)
{
...
i32 QMainToolBarManager::QToolBarDesc::IndexOfItem(std::shared_ptr<QItemDesc> pItem)
{
	return items.indexOf(pItem);
}

void QMainToolBarManager::QToolBarDesc::MoveItem(i32 currIdx, i32 idx)
{
...
i32 QMainToolBarManager::QToolBarDesc::IndexOfCommand(const CCommand* pCommand)
{
...
void QMainToolBarManager::QToolBarDesc::InsertItem(const QVariant& itemVariant, i32 idx)
{
	InsertItem(CreateItem(itemVariant, s_version), idx);
}

void QMainToolBarManager::QToolBarDesc::InsertItem(std::shared_ptr<QItemDesc> pItem, i32 idx)
{
	if (idx < 0)
	{
		items.push_back(pItem);
...
void QMainToolBarManager::QToolBarDesc::InsertCommand(const CCommand* pCommand, i32 idx)
{
	i32 currIdx = IndexOfCommand(pCommand);
...
void QMainToolBarManager::QToolBarDesc::InsertCVar(const QString& cvarName, i32 idx)
{
...
void QMainToolBarManager::QToolBarDesc::InsertSeparator(i32 idx)
{
...
void QMainToolBarManager::QToolBarDesc::RemoveItem(i32 idx)
{
	items.remove(idx);
}
...
void QMainToolBarManager::SaveToolBar(const QString& name) const
{
	...
	if (!file.open(QIODevice::WriteOnly))
	{
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, msg.toLocal8Bit());
		return;
	}
...
void QMainToolBarManager::LoadToolBarsFromDir(const QString& dirPath)
{
	...
	for (const QFileInfo& fileInfo : infoList)
	{
		...
		i32 loadedVersion = variantMap["version"].toInt();
		...
		case QItemDesc::Command:
			{
				...
				if (!pAction)
				{
					DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Couldn't create tool bar button for command: %s", pCommandDesc->GetCommand().toStdString().c_str());
					continue;
				}
				...
				if (!iconPath.isEmpty())
				{
					...
					pAction->setIcon(DrxIcon(iconPath));
				}
		...
		case QItemDesc::CVar:
			{
				...
				DrxIcon icon(iconPath.isEmpty() ? "icons:General/File.ico" : iconPath);
				...
				connect(pAction, &QAction::triggered, [this, pAction, pCVarDesc, pCVar](bool bChecked)
				{
					...
					case CVAR_INT:
					{
						i32 currValue = pCVar->GetIVal();
						i32 newValue = variant.toInt();
...
void QMainToolBarManager::OnCVarChanged(ICVar* pCVar)
{
	...
	case CVAR_INT:
		for (QAction* pAction : actions)
		{
			...
			i32 value = pAction->property(s_actionPropertyName).toInt();
