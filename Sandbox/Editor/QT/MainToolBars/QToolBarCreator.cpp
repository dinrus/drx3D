...
static tukk s_dragType = "CEToolBarItem";
static tukk s_dragListType = "CECommandListItem";
...
Qt::ItemFlags QToolBarCreator::DropCommandModel::flags(const QModelIndex& index) const
{
	...
	QVariant commandVar = index.model()->data(index, (i32)CommandModel::Roles::CommandPointerRole);
...
protected:
	virtual void paintEvent(QPaintEvent* pEvent) override
	{
		...
		if (m_bShowDropTarget)
		{
			...
			i32 spacing = (pixmap.width() + layout()->spacing()) / 2;
...
void QToolBarCreator::QDropContainer::AddItem(const QVariant& itemVariant, i32 idx /* = -1*/)
{
...
void QToolBarCreator::QDropContainer::AddCommand(const CCommand* pCommand, i32 idx /* = -1*/)
{
...
void QToolBarCreator::QDropContainer::AddSeparator(i32 sourceIdx /* = -1*/, i32 targetIdx /* = -1*/)
{
	if (sourceIdx > -1)
...
void QToolBarCreator::QDropContainer::AddCVar(const QString& cvarName, i32 idx /* = -1*/)
{
	if (cvarName.isEmpty())
	{
...
void QToolBarCreator::QDropContainer::RemoveItemAt(i32 idx)
{
...
i32 QToolBarCreator::QDropContainer::GetIndexFromMouseCoord(const QPoint& globalPos)
{
...
void QToolBarCreator::QDropContainer::SetSelectedActionIcon(tukk szPath)
{
	i32 idx = m_pCurrentToolBarDesc->IndexOfItem(m_pSelectedItem);
	if (idx < 0)
		return;
	...
	toolBarActions[idx]->setIcon(DrxIcon(szPath));
}
...
void QToolBarCreator::QDropContainer::dropEvent(QDropEvent* pEvent)
{
	...
	if (pEvent->mimeData()->hasFormat(s_dragType))
	{
		...
		i32 oldIdx = m_pCurrentToolBarDesc->IndexOfItem(m_pSelectedItem);
		i32 newIdx = GetIndexFromMouseCoord(mapToGlobal(pEvent->pos()));
		if (oldIdx == newIdx)
			return;
	...
	if (pEvent->mimeData()->hasFormat(s_dragListType))
	{
		...
		i32 oldIdx = m_pCurrentToolBarDesc->IndexOfCommand(pCommand);
		i32 newIdx = GetIndexFromMouseCoord(mapToGlobal(pEvent->pos()));
	...
	, m_pSelectedItem(nullptr)
{
	...
	pAddToolBar->setIcon(DrxIcon("icons:General/Element_Add.ico"));
	...
	pRemoveToolBar->setIcon(DrxIcon("icons:General/Element_Remove.ico"));
	...
	pRenameToolBar->setIcon(DrxIcon("icons:General/Edit.ico"));
	...
	m_pCVarBrowserButton->setIcon(DrxIcon("icons:General/Folder.ico"));
	...
	m_pIconBrowserButton->setIcon(DrxIcon("icons:General/Folder.ico"));
...
void QToolBarCreator::IconSelected(tukk szIconPath)
{
...
void QToolBarCreator::SetCVarName(tukk szCVarName)
{
...
void QToolBarCreator::SetCVarValue(tukk cvarValue)
{
	ICVar* pCVar = gEnv->pConsole->GetCVar(m_pCVarInput->text().toLocal8Bit());
	if (!pCVar)
	{
...
void QToolBarCreator::SetCommandName(tukk commandName)
{
...
void QToolBarCreator::SetIconPath(tukk szIconPath)
{
...
void QToolBarCreator::CurrentItemChanged(i32 index)
{
