...
protected:
	...
	void AddAction(tukk str, i32 value)
	{
		string command;
...
public:
	QSelectionMaskComboBox()
	{
		...
		connect(this, SIGNAL(activated(i32)), this, SLOT(OnSelection(i32)));
	}
...
private:
	void OnSelectionMaskChanged(i32 mask)
	{
...
private:
	virtual void OnEditorNotifyEvent(EEditorNotifyEvent event) override
	{
		...
		{
			i32k value = GetIEditorImpl()->GetReferenceCoordSys();
			for (i32 i = 0; i < count(); i++)
			{
...
protected:
	...
	virtual void OnPopulateMenu() override
	{
		...
		const float steps = DRX_ARRAY_COUNT(anglesArray);
...
protected:
	...
	virtual void OnPopulateMenu() override
	{
		...
		i32k steps = DRX_ARRAY_COUNT(scalesArray);
		for (i32 i = 0; i < steps; i++)
		{
			string str;
...
protected:
	...
	virtual void OnPopulateMenu() override
	{
		...
		for (size_t i = 0; i < agentTypeCount; ++i)
		{
			tukk szAgentTypeName = pManager->GetNavigationAgentTypeName(i);
