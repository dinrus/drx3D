...
ResultT GetCountRecursively(IStatObj* pObject, CountCurrentObjectT countCurrentObject, AccumulateSubObjectT accumulateSubObject)
{
	...
	for (i32 i = 0; i < pObject->GetSubObjectCount(); ++i)
	{
...
IParticleEffect* FindValidParticleEffect(IParticleEffect* pParentEffect, const QString& effectName)
{
	...
	for (i32 i = 0; i < pParentEffect->GetChildCount(); ++i)
	{
...
void QPreviewWidget::LoadFile(const QString& modelFile, const bool bChangeCamera)
{
	...
	const auto isSKEL = strFileExt.compare(QLatin1String(DRX_SKEL_FILE_EXT), Qt::CaseInsensitive) == 0;
	const auto isSKIN = strFileExt.compare(QLatin1String(DRX_SKIN_FILE_EXT), Qt::CaseInsensitive) == 0;
	const auto isCDF = strFileExt.compare(QLatin1String(DRX_CHARACTER_DEFINITION_FILE_EXT), Qt::CaseInsensitive) == 0;
	const auto isCGA = strFileExt.compare(QLatin1String(DRX_ANIM_GEOMETRY_FILE_EXT), Qt::CaseInsensitive) == 0;
	const auto isCGF = strFileExt.compare(QLatin1String(DRX_GEOMETRY_FILE_EXT), Qt::CaseInsensitive) == 0;
...
void QPreviewWidget::resizeEvent(QResizeEvent *ev)
{
	...
	i32 cx = ev->size().width() * devicePixelRatioF();
	i32 cy = ev->size().height() * devicePixelRatioF();
	if (cx == 0 || cy == 0)
		return;
...
void QPreviewWidget::SavePreview(tukk outFile)
{
	if (m_pCurrentMaterial)
	{
...
QImage QPreviewWidget::GetImage()
{
	...
	m_pRenderer->ReadFrameBuffer((u32*)img.bits(), clientWidth, clientHeight);
...
void QPreviewWidget::DrawBackground()
{
	...
	i32k clientWidth = width();
	i32k clientHeight = height();
