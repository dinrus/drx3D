...
public:
		...
		, m_parent(pParent)
	{
		...
		m_speedctrl->setVal(pViewport->GetCameraMoveSpeedIncrements());

		QObject::connect(m_speedctrl, &QPrecisionSlider::OnSliderPress, [=](i32 value)
		{
			m_timer->stop();
		});
		QObject::connect(m_speedctrl, &QPrecisionSlider::OnSliderRelease, [=](i32 value)
		{
			m_timer->start(s_timerValue);
		});
		QObject::connect(m_speedctrl, &QPrecisionSlider::OnSliderChanged, [=](i32 value)
		{
	...
	void ShowWithTimer(i32 increments)
	{
		m_speedctrl->setVal(increments);
...
private:
	...
	static i32k  s_timerValue;
};

i32k CameraSpeedPopup::s_timerValue = 2000;
...
protected:
	QWidget* createWidget(QWidget* parent) override
	{
		...
		i32 numsteps = log(9999) / log(1.1) + 0.5;
		...
		speedctrl->setVal(m_pViewport->GetCameraMoveSpeedIncrements());

		QObject::connect(speedctrl, &QPrecisionSlider::OnSliderChanged, [=](i32 value)
		{
...
QAction* QTerrainSnappingMenu::AddCommandAction(tukk szCommand, QViewportHeader* pViewportHeader)
{
...
void QTerrainSnappingMenu::RefreshParentToolButton()
{
	...
	if (GetIEditorImpl()->IsSnapToTerrainEnabled())
	{
		...
		{
			m_pParentToolButton->setIcon(DrxIcon("icons:common/viewport-snap-terrain-normal.ico"));
			return;
		}
		m_pParentToolButton->setIcon(DrxIcon("icons:common/viewport-snap-terrain.ico"));
	}
	else if (GetIEditorImpl()->IsSnapToGeometryEnabled())
	{
		...
		if (GetIEditorImpl()->IsSnapToNormalEnabled())
		{
			m_pParentToolButton->setIcon(DrxIcon("icons:common/viewport-snap-geometry-normal.ico"));
			return;
		}
		m_pParentToolButton->setIcon(DrxIcon("icons:common/viewport-snap-geometry.ico"));
	}
	else
	{
		m_pParentToolButton->setIcon(DrxIcon("icons:common/viewport-snap-none.ico"));
	}
}
	...
	, m_moduleName("QViewportHeader")
{
	...
	m_pivotSnapping->setIcon(DrxIcon("icons:Viewport/viewport-snap-pivot.ico"));
	...
	pTerrainSnapping->setIcon(DrxIcon("icons:common/viewport-snap-terrain.ico"));
	...
	m_currentToolExit->setIcon(DrxIcon("icons:General/Close.ico"));
	...
	if (pViewport->IsRenderViewport())
	{
		...
		connect(this, &QViewportHeader::cameraSpeedChanged, [=](float speed, i32 increments)
		{
...
void QViewportHeader::SetViewportFOV(float fov)
{
	if (m_viewport && m_viewport->IsRenderViewport())
	{
		...
		string fovText;
		fovText.Format("FOV:%d", (i32)fov);

		// if viewport camera is active, make selected fov new default
		if (GetIEditor()->GetViewManager()->GetCameraObjectId() == DrxGUID::Null())
		{
...
//////////////////////////////////////////////////////////////////////////
void QViewportHeader::AddAspectRatioMenus(CPopupMenuItem& menu, Functor2<u32, u32> setAspectRatioFunc,
                                          const std::vector<string>& customPresets)
{
	static const std::pair<u32, u32> ratios[] = {
		std::pair<u32, u32>(16, 9),
		std::pair<u32, u32>(16, 10),
		std::pair<u32, u32>(4,  3),
		std::pair<u32, u32>(5,  4)
	};
...
void QViewportHeader::AddResolutionMenus(CPopupMenuItem& menu, Functor2<i32, i32> resizeFunc, const std::vector<string>& customPresets)
{
	...
	{
		string text;
		i32 d = gcd(resolutions[i].width, resolutions[i].height);
		i32 ratio_num = resolutions[i].width / d;
		i32 ratio_denom = resolutions[i].height / d;
...
void QViewportHeader::OnMenuResolutionCustom()
{
	...
	if (dialog.exec() == QDialog::Accepted)
	{
		...
		// Update the custom presets.
		string text;
		i32 d = gcd(currentWidth, currentHeight);
		i32 ratio_num = currentWidth / d;
		i32 ratio_denom = currentHeight / d;
...
void QViewportHeader::OnCycleDisplayInfo(QAction* cycleDisplayInfo)
{
	...
	{
	case 0:
		cycleDisplayInfo->setIcon(DrxIcon("icons:Viewport/viewport-displayinfo.ico"));
		break;
	case 1:
		cycleDisplayInfo->setIcon(DrxIcon("icons:Viewport/viewport-displayinfo_2.ico"));
		break;
	case 2:
		cycleDisplayInfo->setIcon(DrxIcon("icons:Viewport/viewport-displayinfo_3.ico"));
		break;
	case 3:
		cycleDisplayInfo->setIcon(DrxIcon("icons:Viewport/viewport-displayinfo_1.ico"));
		break;
	default:
		break;
	}
...
void QViewportHeader::InputNamesToSearchFromSelection()
{
	...
	string names;
	for (i32 i = 0; i < pSelected->GetCount(); ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
QToolButton* QViewportHeader::AddToolButtonForCommand(tukk szCommand, QLayout* pLayout)
{
	QAction* pAction = GetIEditorImpl()->GetICommandManager()->GetAction(szCommand);
...
{
	IObjectManager* pObjMgr = GetIEditorImpl()->GetObjectManager();
	i32 objCount = pObjMgr->GetObjectCount();
...
void QViewportHeader::LoadCustomPresets(tukk keyName, std::vector<string>& outCustompresets)
{
...
void QViewportHeader::SaveCustomPresets(tukk keyName, const std::vector<string>& custompresets)
{
...
void QViewportHeader::UpdateCustomPresets(const string& text, std::vector<string>& custompresets)
{
	...
	for (i32 k = 0, m = 1; k < backup.size() && custompresets.size() <= MAX_NUM_CUSTOM_PRESETS; ++k)
	{
		if (backup[k] != text)
		{
...
namespace
{
	...
	{
		CDrxEditApp::GetInstance()->ToggleHelpersDisplay();
	}
