...
protected:
	...
	QAction* AddCommandAction(tukk szCommand, QViewportHeader* pViewportHeader);
...
public:
	...
	void                       AddAspectRatioMenus(CPopupMenuItem& menu, Functor2<u32, u32> setAspectRatioFunc, const std::vector<string>& customPresets);
	void                       AddResolutionMenus(CPopupMenuItem& menu, Functor2<i32, i32> resizeFunc, const std::vector<string>& customPresets);
	static void                UpdateCustomPresets(const string& text, std::vector<string>& custompresets);

	static void                LoadCustomPresets(tukk keyName, std::vector<string>& outCustompresets);
	static void                SaveCustomPresets(tukk keyName, const std::vector<string>& custompresets);
...
Q_SIGNALS:
	void cameraSpeedChanged(float speed, i32 increments);
...
private:
	...
	QToolButton* AddToolButtonForCommand(tukk szCommand, QLayout* pLayout);
...
private:
	...
	static i32k      MAX_NUM_CUSTOM_PRESETS = 10;
