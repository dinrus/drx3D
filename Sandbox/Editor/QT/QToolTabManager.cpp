...
#include <drx3D/Sys/UserAnalytics/IUserAnalytics.h>
...
static tukk szAppDataLayoutDir = "Layouts";
static tukk szDefaultLayoutDir = "Editor/Layouts";
static tukk szDefaultLayout = "Editor/Layouts/Default Layout.json";

void PyLoadLayoutFromFile(tukk fullFilename)
{
	CTabPaneManager::GetInstance()->LoadLayout(fullFilename);
}

void PySaveLayoutToFile(tukk fullFilename)
{
...
QString CTabPaneManager::CreateObjectName(tukk title)
{
	QString s(title);
	QString result = s;
...
//////////////////////////////////////////////////////////////////////////
QTabPane* CTabPaneManager::CreateTabPane(tukk paneClassName, tukk title, i32 nOverrideDockDirection, bool bLoadLayoutPersonalization)
{
	...
	i32 dockDir = IViewPaneClass::DOCK_FLOAT;
...
IPane* CTabPaneManager::CreatePane(tukk paneClassName, tukk title /*= 0*/, i32 nOverrideDockDirection /*= -1*/)
{
...
//////////////////////////////////////////////////////////////////////////
CWnd* CTabPaneManager::OpenMFCPane(tukk sPaneClassName)
{
...
CWnd* CTabPaneManager::FindMFCPane(tukk sPaneClassName)
{
	QTabPane* tool = FindTabPaneByClass(sPaneClassName);
	if (tool)
	{
...
IPane* CTabPaneManager::OpenOrCreatePane(tukk sPaneClassName)
{
	QTabPane* tool = FindTabPaneByClass(sPaneClassName);
...
bool CTabPaneManager::CloseTabPane(QTabPane* tool)
{
	...
	DRX_ASSERT(bDeleted); //if this is false we have tried to delete the tool twice, or delete a tool that wasn't registered
	if (tool && bDeleted)
	{
		// User Analytics - do not move or modify
...
//////////////////////////////////////////////////////////////////////////
QTabPane* CTabPaneManager::FindTabPaneByTitle(tukk title)
{
	QList<QTabPane*> tools = FindTabPanes();
	for (i32 i = 0; i < tools.count(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
QTabPane* CTabPaneManager::FindTabPaneByCategory(tukk sPaneCategory)
{
	QList<QTabPane*> tools = FindTabPanes();
	for (i32 i = 0; i < tools.count(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
QTabPane* CTabPaneManager::FindTabPaneByClass(tukk paneClassName)
{
...
IPane* CTabPaneManager::FindPaneByClass(tukk paneClassName)
{
...
IPane* CTabPaneManager::FindPaneByTitle(tukk title)
{
...
bool CTabPaneManager::LoadLayout(tukk filePath)
{
...
void CTabPaneManager::SaveLayoutToFile(tukk fullFilename)
{
	QFile file(fullFilename);
	if (!file.open(QIODevice::WriteOnly))
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed to open path: %s", fullFilename);
		return;
	}
...
bool CTabPaneManager::LoadLayoutFromFile(tukk fullFilename)
{
	LOADING_TIME_PROFILE_SECTION_ARGS(fullFilename);
	QFile file(fullFilename);
	if (!file.open(QIODevice::ReadOnly))
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_COMMENT, "Failed to open path: %s", fullFilename);
		return false;
	}
...
//////////////////////////////////////////////////////////////////////////
void CTabPaneManager::PushUserEvent(tukk szEventName, tukk szTitle, ukk pAddress)
{
	UserAnalytics::Attributes attributes;
	attributes.AddAttribute("title", szTitle);
	char addressBuffer[32];
	drx_sprintf(addressBuffer, "0x%p", pAddress);
