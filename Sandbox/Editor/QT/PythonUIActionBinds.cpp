...
namespace
{
CDrxEditApp* GetEditApp() { return &theApp; }
}
...
public:
	...
	UiActionCommandModuleDescription(tukk name, tukk uiName, tukk description)
		: CCommandModuleDescription(name, uiName, description)
	{}

	virtual string FormatCommandForUI(tukk command) const override
	{
...
namespace
{
void PyOpenViewPane(tukk viewClassName)
{
	GetIEditorImpl()->OpenView(viewClassName);
}

std::vector<STxt> PyGetViewPaneClassNames()
{
...
struct SAudioGeneralPreferences : public SPreferencePage
{
	...
	virtual bool Serialize(yasli::Archive& ar) override
	{
		...
		if (ar.isInput())
		{
			if (bMuteAudio)
			{
				gEnv->pAudioSystem->ExecuteTrigger(DrxAudio::MuteAllTriggerId);
			}
			else
			{
				gEnv->pAudioSystem->ExecuteTrigger(DrxAudio::UnmuteAllTriggerId);
			}
		}
...
{
	DrxLogAlways("<Audio> Executed \"Stop All Sounds\" command.");
...
{
	DrxAudio::SRequestUserData const data(DrxAudio::ERequestFlags::ExecuteBlocking);
