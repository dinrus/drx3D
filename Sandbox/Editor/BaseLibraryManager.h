...
public:
	...
	virtual IDataBaseItem*           FindItem(DrxGUID guid) const;
	...
	virtual IDataBaseItem*           FindItemByName(tukk fullItemName);
	virtual IDataBaseItem*           LoadItemByName(tukk fullItemName);
	...
	inline IDataBaseLibrary*  AddLibrary(tukk library, bool bSetFullFilename = false) { return AddLibrary(string(library), bSetFullFilename); } // for CString conversion
	virtual void              DeleteLibrary(const string& library);
	inline void				  DeleteLibrary(tukk library) { return DeleteLibrary(string(library)); } // for CString conversion
	//! Get number of libraries.
	virtual i32               GetLibraryCount() const { return m_libs.size(); };
	//! Get number of modified libraries.
	virtual i32               GetModifiedLibraryCount() const;

	//! Get Item library by index.
	virtual IDataBaseLibrary* GetLibrary(i32 index) const;
	...
	//! Find Items Library by name.
	inline IDataBaseLibrary* FindLibrary(tukk library) { return FindLibrary(string(library)); } // for CString conversion

	//! Load Items library.
	virtual IDataBaseLibrary* LoadLibrary(const string& filename, bool bReload = false);

	//! Load Items library.
	inline IDataBaseLibrary* LoadLibrary(tukk filename, bool bReload = false) { return LoadLibrary(string(filename), bReload); } // for CString conversion
	...
	//////////////////////////////////////////////////////////////////////////
	virtual void RegisterItem(CBaseLibraryItem* pItem, DrxGUID newGuid);
...
protected:
	...
	// GUID to item map.
	typedef std::map<DrxGUID, _smart_ptr<CBaseLibraryItem>> ItemsGUIDMap;
