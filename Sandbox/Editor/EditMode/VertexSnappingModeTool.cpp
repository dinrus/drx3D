...
struct VertexSnappingCVars
{
	...
	static i32 ed_vert_snapping_show_spatial_partition;
};

i32 VertexSnappingCVars::ed_vert_snapping_show_spatial_partition = 0;

class CVertexSnappingModeToolClassDesc : public IClassDesc
{
	ESystemClassID SystemClassID()   { return ESYSTEM_CLASS_EDITTOOL; }
	tukk    ClassName()       { return "EditTool.VertexSnappingMode"; };
	tukk    Category()        { return "Select"; };
...
bool CVertexSnappingModeTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
...
bool CVertexSnappingModeTool::HitTest(CViewport* view, const CPoint& point, CBaseObject* pExcludedObj, Vec3* outHitPos, CBaseObjectPtr& pHitObject, std::vector<CBaseObjectPtr>& outObjects)
{
	...
	i32 nPickFlag = CSurfaceInfoPicker::ePOG_BrushObject |
	...
	for (i32 i = 0, iCount(penetratedObjects.size()); i < iCount; ++i)
	{
	...
	for (ii = outObjects.begin(); ii != outObjects.end(); ++ii)
	{
		if (VertexSnappingCVars::ed_vert_snapping_show_spatial_partition)
		{
			...
			for (i32 i = nIndex; i < m_DebugBoxes.size(); ++i)
...
void CVertexSnappingModeTool::Display(struct DisplayContext& dc)
{
	...
	if (VertexSnappingCVars::ed_vert_snapping_show_spatial_partition)
	{
		...
		for (i32 i = 0, iCount(m_DebugBoxes.size()); i < iCount; ++i)
		{
...
void CVertexSnappingModeTool::DrawVertexCubes(DisplayContext& dc, const Matrix34& tm, IStatObj* pStatObj)
{
	...
	if (pIndexedMesh)
	{
		...
		for (i32 k = 0; k < md.m_nVertCount; ++k)
		{
			Vec3 vPos(0, 0, 0);
			if (md.m_pVerts)
				vPos = md.m_pVerts[k];
	...
	for (i32 i = 0, iSubStatObjNum(pStatObj->GetSubObjectCount()); i < iSubStatObjNum; ++i)
	{
