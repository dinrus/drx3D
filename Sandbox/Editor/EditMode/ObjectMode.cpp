	...
	, m_visibilityDirty(true)
{
	...
	i32 totCount = pSelection->GetCount();

	// register to all selected objects on initialization
	for (i32 i = 0; i < totCount; ++i)
	{
...
void CObjectManipulatorOwner::ObjectUpdateCallback(CBaseObject* pObj, i32 evt)
{
	...
	: m_objectManipulatorOwner(this)
{
	...
	m_MouseOverObject = DrxGUID::Null();
...
CObjectMode::~CObjectMode()
{
	...
	if (m_MouseOverObject != DrxGUID::Null())
	{
...
void CObjectMode::DrawSelectionPreview(struct DisplayContext& dc, CBaseObject* drawObject)
{
	...
	if (drawObject->GetParent())
		selColor.a = (u8)(gViewportSelectionPreferences.childObjectGeomAlpha * 255);
	...
	if (drawObject->GetChildCount() > 0)
	{
		...
		for (i32 gNo = 0; gNo < drawObject->GetChildCount(); ++gNo)
		{
...
void CObjectMode::DisplaySelectionPreview(struct DisplayContext& dc)
{
	...
	if (GetCommandMode() == SelectMode)
	{
		if (rc.Width() > 1 && rc.Height() > 1)
		{
			...
			i32 childNo = 0;
			for (i32 objNo = 0; objNo < m_PreviewGUIDs.size(); ++objNo)
			{
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
	switch (event)
	{
	case eMouseLDown:
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::OnKeyUp(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
	return false;
}

//////////////////////////////////////////////////////////////////////////
bool CObjectMode::OnLButtonDown(CViewport* view, i32 nFlags, CPoint point)
{
	...
	//drx_sprintf(szNewStatusText, "X:%g Y:%g Z:%g",pos.x,pos.y,pos.z );
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::OnLButtonUp(CViewport* view, i32 nFlags, CPoint point)
{
	if (GetIEditorImpl()->IsInGameMode())
	{
		// Ignore clicks while in game.
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::OnLButtonDblClk(CViewport* view, i32 nFlags, CPoint point)
{
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::OnRButtonDown(CViewport* view, i32 nFlags, CPoint point)
{
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::OnRButtonUp(CViewport* view, i32 nFlags, CPoint point)
{
	if (m_openContext)
	{
		m_openContext = false;
		...
		i32 halfLength = gViewportPreferences.dragSquareSize / 2;
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::OnMButtonDown(CViewport* view, i32 nFlags, CPoint point)
{
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::CheckVirtualKey(i32 virtualKey)
{
...
//////////////////////////////////////////////////////////////////////////
bool CObjectMode::OnMouseMove(CViewport* view, i32 nFlags, CPoint point)
{
	if (GetIEditorImpl()->IsInGameMode())
	{
	...
	i32 coordSys = GetIEditorImpl()->GetReferenceCoordSys();
	...
	if (GetCommandMode() == MoveMode)
	{
		...
		i32 selectionFlags = CSelectionGroup::eMS_None;
		if (GetIEditorImpl()->IsSnapToTerrainEnabled() && view->GetAxisConstrain() != AXIS_Z)
...
void CObjectMode::SetObjectCursor(CViewport* view, CBaseObject* hitObj, IObjectManager::ESelectOp selectMode)
{
	...
	if (m_MouseOverObject != DrxGUID::Null() && !m_suspendHighlightChange)
	{
		pMouseOverObject = GetIEditorImpl()->GetObjectManager()->FindObject(m_MouseOverObject);
	}
...
void CObjectMode::CheckDeepSelection(HitContext& hitContext, CViewport* pWnd)
{
	...
	if (m_pDeepSelection->GetCandidateObjectCount() > 1)
	{
		...
		if (m_pDeepSelection->GetMode() == CDeepSelection::DSM_POP)
		{
			...
			for (i32 i = 0; i < m_pDeepSelection->GetCandidateObjectCount(); ++i)
			{
			...
			CPoint p;
			::GetCursorPos(&p);
			i32 nSelect = popUpDeepSelect.TrackPopupMenu(TPM_NONOTIFY | TPM_RETURNCMD | TPM_CENTERALIGN, p.x, p.y, CWnd::FromHandle((HWND)pWnd->GetSafeHwnd()), NULL);
		...
		{
			i32 selPos = m_pDeepSelection->GetCurrentSelectPos();
...
Vec3& CObjectMode::GetScale(const CViewport* view, const CPoint& point, Vec3& OutScale)
{
	...
	i32 axisConstrain = view->GetAxisConstrain();
...
void CObjectMode::OnManipulatorBeginDrag(IDisplayViewport* view, ITransformManipulator* pManipulator, const Vec2i& point, i32 flags)
{
...
void CObjectMode::OnManipulatorDrag(IDisplayViewport* view, ITransformManipulator* pManipulator, const Vec2i& point, const Vec3& value, i32 flags)
{
