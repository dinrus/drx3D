...
//  Copyright (C), DinrusPro 3D, 1999-2012
// -------------------------------------------------------------------------
...
public:
	...
	void                      ExcludeHitTest(i32 except);
	...
	i32 CollectCandidate(float fMinDistance, float fRange);
	//! Return the candidate object in index position, then it is to be current
	//! selection position.
	CBaseObject* GetCandidateObject(i32 index);
	//! Return the current selection position that is update in "GetCandidateObject"
	//! function call.
	inline i32 GetCurrentSelectPos() const     { return m_CurrentSelectedPos; }
	//! Return the number of objects in the deep selection range.
	inline i32 GetCandidateObjectCount() const { return m_CandidateObjectCount; }
...
private:
	...
	i32                       m_CandidateObjectCount;
	i32                       m_CurrentSelectedPos;
};
