...
protected:
	virtual tukk GetDescription() { return "Prefab's Open/Close"; };
...
CUndoAddObjectsToPrefab::CUndoAddObjectsToPrefab(CPrefabObject* prefabObj, TBaseObjects& objects)
{
	...
	for (i32 i = 0; i < objects.size(); i++)
	{
		...
		{
			// Find later in array.
			for (i32 j = i + 1; j < objects.size(); j++)
			{
	...
	for (size_t i = 0, count = objects.size(); i < count; ++i)
	{
		...
		addedObject.m_objectParent = objects[i]->GetParent() ? objects[i]->GetParent()->GetId() : DrxGUID::Null();
...
void CUndoAddObjectsToPrefab::Undo(bool bUndo)
{
	...
	for (i32 i = m_addedObjects.size() - 1; i >= 0; --i)
	{
		...
		if (CBaseObject* pMember = pObjMan->FindObject(m_addedObjects[i].m_object))
		{
			...
			// Restore child links
			if (i32k childsCount = m_addedObjects[i].m_objectsChilds.size())
			{
				for (i32 j = 0; j < childsCount; ++j)
				{
...
{
	for (i32 i = 0, count = m_addedObjects.size(); i < count; ++i)
	{
...
void CPrefabManager::OpenSelected()
{
	...
	i32k count = pSelection->GetCount();
	if (!count)
		return;
...
void CPrefabManager::AddSelectionToPrefab(CPrefabObject* pPrefab)
{
	...
	for (i32 i = 0, count = objects.size(); i < count; ++i)
	{
...
void CPrefabManager::ExtractObjectsFromPrefabs(std::vector<CBaseObject*>& childObjects)
{
	...
	for (CBaseObject* pChild : childObjects)
	{
		...
		for (i32 i = 0, count = pPrefab->GetChildCount(); i < count; ++i)
		{
...
void CPrefabManager::CloneObjectsFromPrefabs(std::vector<CBaseObject*>& childObjects)
{
	...
	for (i32 i = 0, childCount = childObjects.size(); i < childCount; ++i)
	{
...
bool CPrefabManager::OpenPrefabs(std::vector<CPrefabObject*>& prefabObjects)
{
	...
	for (i32 i = 0, iCount(prefabObjects.size()); i < iCount; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CPrefabManager::GetPrefabInstanceCount(CPrefabItem* pPrefabItem)
{
	i32 instanceCount = 0;
	...
	if (pPrefabItem)
	{
		...
		for (i32 i = 0, prefabsFound(prefabObjects.size()); i < prefabsFound; ++i)
		{
