...
bool CPrefabEvents::Init()
{
	...
	DRX_ASSERT(pFlowGraphManager != NULL);
...
{
	DRX_ASSERT(pPrefabObj != NULL);
	DRX_ASSERT(pNode != NULL);
	CPrefabItem* pPrefabItem = pPrefabObj->GetPrefabItem();
	DRX_ASSERT(pPrefabItem != NULL);
	...
	case EHG_NODE_PROPERTIES_CHANGED:
		{
			...
			if (bResult)
			{
				DRX_ASSERT(pInstanceEventsData);
	...
	if (eventId == CUSTOMEVENTID_INVALID)   // Try to create event id
	{
		if (!eventName.IsEmpty())   // Has an event name so assign an id
		{
			...
			if (iEventIndex == -1)   // Didn't exist so need to attempt to get a free one
			{
				...
				for (i32 i = 1; i <= CUSTOMEVENTS_PREFABS_MAXNPERINSTANCE; ++i)
				{
			...
			if (iEventIndex != -1)
			{
				...
				DRX_ASSERT(pCustomEventManager != NULL);
  ...
  bool bRemoveDataOnNode)
{
	...
	if (bNodeRemoved)
	{
		...
		if (eventSourceNodeHelper.IsEmpty())
		{
			...
			DRX_ASSERT(eventSourcesDataIter != eventSourcesData.end());
...
void CPrefabEvents::UnregisterPrefabInstanceEvents(CFlowNode* pNode)
{
	...
	for (i32 i = 1; i <= CUSTOMEVENTS_PREFABS_MAXNPERINSTANCE; ++i)
	{
		...
		DRX_ASSERT(pEventTriggerInputPort && pEventTriggerOutputPort && pEventNamePort && pEventIdPort);
		if (pEventTriggerInputPort && pEventTriggerOutputPort && pEventNamePort && pEventIdPort)
		{
			i32 iCustomEventId = (i32)CUSTOMEVENTID_INVALID;
			pEventIdPort->pVar->Get(iCustomEventId);

			if (iCustomEventId != (i32)CUSTOMEVENTID_INVALID)
			{
				// Get rid of the event id set on node itself in case this node is going to be reused from a change
				pEventIdPort->pVar->Set((i32)CUSTOMEVENTID_INVALID);
			}
		}
	}
}
...
{
	DRX_ASSERT(pNode != NULL);
	DRX_ASSERT(ppPrefabName != NULL);
	DRX_ASSERT(ppPrefabInstanceName != NULL);
	DRX_ASSERT(ppInstanceEventsData != NULL);
  ...
  SPrefabEventSourceData** ppEventSourceData)
{
	...
	DRX_ASSERT(ppPrefabName != NULL);
	DRX_ASSERT(ppPrefabInstanceName != NULL);
	DRX_ASSERT(ppEventName != NULL);
	DRX_ASSERT(ppInstancesData != NULL);
	DRX_ASSERT(ppInstanceEventsData != NULL);
	DRX_ASSERT(ppEventSourceData != NULL);
...
bool CPrefabEvents::SetPrefabIdentifiersToEventNode(CFlowNode* pNode, const string& prefabName, const string& prefabInstanceName)
{
	if (pNode)
	{
		...
		DRX_ASSERT(pPrefabNamePort && pPrefabInstancePort);
...
//////////////////////////////////////////////////////////////////////////
bool CPrefabEvents::SetPrefabEventIndexToEventSourceNode(CFlowNode* pNode, i32k iEventIndex)
{
	DRX_ASSERT(pNode != NULL);
...
//////////////////////////////////////////////////////////////////////////
bool CPrefabEvents::GetPrefabEventIndexFromEventSourceNode(CFlowNode* pNode, i32& iEventIndex)
{
	DRX_ASSERT(pNode != NULL);
	CHyperNodePort* pPort = pNode->FindPort("EventIndex", true);
	DRX_ASSERT(pPort != NULL);
			...
			if (pEventTriggerInputPort && pEventTriggerOutputPort && pEventNamePort && pEventIdPort)
			{
				...
				if (instanceEventsData)
				{
					...
					if (bResult)
					{
						...
						pEventIdPort->pVar->Set((i32)pEventSourceData->m_eventId);
...
//////////////////////////////////////////////////////////////////////////
bool CPrefabEvents::GetPrefabEventSourceData(TPrefabEventSourceData& eventsSourceData, i32k iEventIndex, SPrefabEventSourceData** ppSourceData, const string** ppEventName)
{
