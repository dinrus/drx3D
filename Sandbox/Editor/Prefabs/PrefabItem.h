...
struct SObjectIdMapping
{
	DrxGUID from;
	DrxGUID to;
	SObjectIdMapping() : from(DrxGUID::Null()), to(DrxGUID::Null()) {}
	SObjectIdMapping(DrxGUID src, DrxGUID dst) : from(src), to(dst) {}
};
...
public:
	...
	XmlNodeRef FindObjectByGuid(const DrxGUID& guid, bool fowardSearch = true);
...
private:
	...
	CBaseObjectPtr FindObjectByPrefabGuid(const std::vector<CBaseObject*>& objects, DrxGUID guidInPrefab);
	...
	DrxGUID        ResolveID(const TObjectIdMapping& prefabIdToGuidMapping, DrxGUID id, bool prefabIdToGuidDirection);
	...
	void           RemoveAllChildsOf(DrxGUID guid);
