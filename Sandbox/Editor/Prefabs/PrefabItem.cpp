...
void CPrefabItem::Serialize(SerializeContext& ctx)
{
	...
	if (ctx.bLoading)
	{
		...
		if (objects)
		{
			...
			for (i32 i = 0, count = objects->getChildCount(); i < count; ++i)
			{
				...
				while (!groupObjects.empty())
				{
					...
					for (i32 j = 0, childCount = group->getChildCount(); j < childCount; ++j)
					{
			...
			for (i32 i = 0, count = objectsToReattach.size(); i < count; ++i)
			{
...
void CPrefabItem::UpdateFromPrefabObject(CPrefabObject* pPrefabObject, const SObjectChangedContext& context)
{
	...
	if (!pPrefabObject || pPrefabObject->IsModifyInProgress()
		||!pPrefabManager || context.m_modifiedObjectGuidInPrefab == DrxGUID::Null() || pPrefabManager->ShouldSkipPrefabUpdate())
	{
		return;
	}
...
void CPrefabItem::UpdateObjects()
{
	...
	for (i32 i = 0, iObjListSize(pPrefabObjects.size()); i < iObjListSize; ++i)
	{
		...
		else if (context.m_operation == eOCOT_Delete)
		{
			if (XmlNodeRef object = FindObjectByGuid(pObj->GetIdInPrefab(), true))
			{
				...
				else
				{
					...
					for (i32 i = 0, count = m_objectsNode->getChildCount(); i < count; ++i)
					{
						DrxGUID parentGuid = DrxGUID::Null();
	...
	else if (CBaseObject* const pObj = objectsInPrefabAsFlatSelection.GetObjectByGuidInPrefab(context.m_modifiedObjectGuidInPrefab))
	{
		if (context.m_operation == eOCOT_Modify)
		{
			if (XmlNodeRef changedObject = FindObjectByGuid(pObj->GetIdInPrefab(), false))
			{
				...
				for (i32 j = 0, count = guidMapping.size(); j < count; ++j)
		...
		else if (context.m_operation == eOCOT_ModifyTransform)
		{
			...
			{
				DrxGUID parentGUID = DrxGUID::Null();
				DrxGUID linkedToGUID = DrxGUID::Null();
...
//////////////////////////////////////////////////////////////////////////
XmlNodeRef CPrefabItem::FindObjectByGuid(const DrxGUID& guid, bool forwardSearch)
{
	...
	{
		for (i32 i = m_objectsNode->getChildCount() - 1; i >= 0; --i)
		{
			m_objectsNode->getChild(i)->getAttr("Id", objectId);
			if (objectId == guid)
				return m_objectsNode->getChild(i);
...
//////////////////////////////////////////////////////////////////////////
CBaseObjectPtr CPrefabItem::FindObjectByPrefabGuid(const std::vector<CBaseObject*>& objects, DrxGUID guidInPrefab)
{
...
void CPrefabItem::ExtractObjectsPrefabIDtoGuidMapping(CSelectionGroup& objects, TObjectIdMapping& mapping)
{
	...
	for (i32 i = 0, count = objects.GetCount(); i < count; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
DrxGUID CPrefabItem::ResolveID(const TObjectIdMapping& prefabIdToGuidMapping, DrxGUID id, bool prefabIdToGuidDirection)
{
	if (prefabIdToGuidDirection)
	{
		for (i32 j = 0, count = prefabIdToGuidMapping.size(); j < count; ++j)
		{
...
void CPrefabItem::RemapIDsInNodeAndChildren(XmlNodeRef objectNode, const TObjectIdMapping& mapping, bool prefabIdToGuidDirection)
{
	...
	for (i32 i = 0; i < objectNode->getChildCount(); ++i)
	{
		...
		while (!objects.empty())
		{
			...
			for (i32 j = 0, childCount = object->getChildCount(); j < childCount; ++j)
			{
...
//////////////////////////////////////////////////////////////////////////
void CPrefabItem::RemoveAllChildsOf(DrxGUID guid)
{
	std::deque<XmlNodeRef> childrenToRemove;
	std::deque<DrxGUID> guidsToCheckAgainst;
	DrxGUID parentId = DrxGUID::Null();
	DrxGUID linkedToId = DrxGUID::Null();
	...
	for (i32 i = 0, count = guidsToCheckAgainst.size(); i < count; ++i)
	{
