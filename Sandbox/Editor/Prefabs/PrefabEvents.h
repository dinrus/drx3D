...
#include <DrxAction/ICustomEvents.h>
...
protected:
	...
	struct SPrefabEventSourceData
	{
		...
		i32               m_iEventIndex; // To remember port order on instances
	...
	bool SetPrefabEventIndexToEventSourceNode(CFlowNode* pNode, i32k iEventIndex);

	// Retrieves custom event index (Used to preserve port order on instances) from event source node
	bool GetPrefabEventIndexFromEventSourceNode(CFlowNode* pNode, i32& iEventIndex);
	...
	bool GetPrefabEventSourceData(TPrefabEventSourceData& eventsSourceData, i32k iEventIndex, SPrefabEventSourceData** ppSourceData, const string** ppEventName);
