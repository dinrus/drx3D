...
///////////////////////////////////////////////////////////////////////////////////
bool CubemapUtils::GenCubemapWithPathAndSize(string& filename, i32k size, const bool dds)
{
...
///////////////////////////////////////////////////////////////////////////////////
bool CubemapUtils::GenCubemapWithObjectPathAndSize(string& filename, CBaseObject* pObject, i32k size, const bool dds)
{
	if (!pObject)
	{
	...
	i32 auxOn = gEnv->pConsole->GetCVar("r_auxGeom")->GetIVal();
	if(auxOn)
	...
	i32 nCMSufixCheck = texname.Find("_cm");
...
void CubemapUtils::GenHDRCubemapTiff(const string& fileName, std::size_t nDstSize, Vec3& pos)
{
	...
	DRX_ASSERT_MESSAGE(vecData.size() == srcSideSize * sides, "TIFF data does not match expected size");
	...
	for (i32 side = 0; side < sides; ++side)
	{
		for (u32 y = 0; y < nDstSize; ++y)
		{
			DrxHalf4* pSrcSide = (DrxHalf4*)&vecData[side * srcSideSize];
			DrxHalf4* pDst = (DrxHalf4*)&img.ValueAt(side * dstPitch, y);
			for (u32 x = 0; x < nDstSize; ++x)
			{
				Vec4 cResampledColor(0.f, 0.f, 0.f, 0.f);

				// resample the image at the original size
				for (u32 yres = 0; yres < 4; ++yres)
				{
					for (u32 xres = 0; xres < 4; ++xres)
					{
						const DrxHalf4& pSrc = pSrcSide[(y * 4 + yres) * nSrcSize + (x * 4 + xres)];
						cResampledColor += Vec4(DrxConvertHalfToFloat(pSrc.x), DrxConvertHalfToFloat(pSrc.y), DrxConvertHalfToFloat(pSrc.z), DrxConvertHalfToFloat(pSrc.w));
					}
				}
				...
				*pDst++ = DrxHalf4(cResampledColor.x, cResampledColor.y, cResampledColor.z, 1.0f);
			}
		}
	}
