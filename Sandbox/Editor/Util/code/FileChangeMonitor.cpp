...
namespace FileChangeMonitor
{
	...
	static u32k defaultIgnoreTimeMsec = 500;
	...
	static u32k maxFileChanges = 5;
	// timer ID
	static u32k timerId = 1;
	// buffer size used with ReadDirectoryChangesW to get file notifications in OVERLAPPED mode
	static u32k fileChangeNotificationBufferSize = 4096;
};
...
{
public:
	static tukk GetClassName()
	{
		return "DrxFileChangeMonitor";
	}
...
public:
	...
	void AddIgnoreFileMask(tukk pMask)
	{
		string maskLower = pMask;

		maskLower.MakeLower();

		DrxAutoLock<DrxMutex> lock(m_ignoreMasksMutex);
		m_ignoreMasks[pMask] = SIgnoreFileMask();
		m_ignoreMasks[pMask].mask = pMask;
	}

	void RemoveIgnoreFileMask(tukk pMask, i32 aAfterDelayMsec)
	{
		string maskLower = pMask;
		...
		if (GetOverlappedResult(pFolder->hFolder, &pFolder->overlapped, &byteCount, TRUE))
		{
			...
			while (lpInfo)
			{
				...
				if (lpInfo->FileNameLength)
				{
					...
					string filename = PathUtil::Make(pFolder->folderPath, string(DrxStringUtils::WStrToUTF8(wcFileName)));
	...
	{
		DrxAutoLock<DrxMutex> lock(m_fileChangesMutex);
	...
	static tukk GetActionName(DWORD action)
	{
		switch (action)
		{
		case FILE_ACTION_ADDED:
	...
	bool AddChangedFile(tukk pFilename, DWORD action)
	{
		DrxAutoLock<DrxMutex> lock(m_fileChangesMutex);
		...
		tuk strCanonicalPath = &canonicalPath[0];
		...
		if (!PathCanonicalize(strCanonicalPath, pathWithBackslashes.GetString()))
		{
			...
			drx_strcpy(strCanonicalPath, canonicalPath.size(), pFilename);
		}

		const string filename = PathUtil::ToUnixPath(const_cast<tukk >(strCanonicalPath)).c_str();
		...
		{
			tukk szActionName = GetActionName(action);
	...
	{
		for (i32 i = 0, iCount = m_monitorItems.size(); i < iCount; ++i)
		{
	...
	void EnqueuePendingItem(tukk pItem)
	{
		m_pendingItems.push(pItem);
	}

	void AddPendingItem(tukk pItem)
	{
	...
	void ThreadEntry()
	{
		while (true)
		{
			...
			{
				i32 triggeredHandleIndex = dwWaitStatus - WAIT_OBJECT_0;
...
protected:
	...
	DrxMT::queue<string>               m_pendingItems;
	...
	DrxMutex                            m_fileChangesMutex;
	DrxMutex                            m_ignoreMasksMutex;
...
void CFileChangeMonitor::Initialize()
{
	...
	{
		DrxFatalError("Error spawning \"FileChangeMonitor\" thread.");
	}
}

void CFileChangeMonitor::AddIgnoreFileMask(tukk pMask)
{
	if (m_pThread)
	{
...
void CFileChangeMonitor::RemoveIgnoreFileMask(tukk pMask, i32 aAfterDelayMsec)
{
	if (m_pThread)
	{
