...
bool CArcBall3D::ArcControl(const Matrix34& reference, const Ray& ray, u32 mouseleft)
{
	RotControl <<= 1;
...
u32 CArcBall3D::IntersectSphereLineSegment(const Sphere& sphere, const Vec3& LineStart, const Vec3& LineEnd, Vec3& I)
{
...
void CArcBall3D::DrawSphere(const Matrix34& reference, const CCamera& cam, IRenderAuxGeom* pRenderer)
{
	...
	if ((RotControl & 3) == 3)
	{
		...
		for (v = 0; v < 100; ++v)
		{
			...
			CBuffer[v + 2].b = u8((1.0f - t1) * CBuffer[1].b + t1 * CBuffer[102].b);
			CBuffer[v + 2].g = u8((1.0f - t1) * CBuffer[1].g + t1 * CBuffer[102].g);
			CBuffer[v + 2].r = u8((1.0f - t1) * CBuffer[1].r + t1 * CBuffer[102].r);
		}
	...
	if (AxisSnap)
	{
		...
		for (v = 0; v < 100; ++v)
		{
			...
			f32 t1 = (1.0f / 101.0f * v);
			CBuffer[v + 2].r = u8((1.0f - t1) * c0.r + t1 * c1.r);
			CBuffer[v + 2].g = u8((1.0f - t1) * c0.g + t1 * c1.g);
			CBuffer[v + 2].b = u8((1.0f - t1) * c0.b + t1 * c1.b);
		}
