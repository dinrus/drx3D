...
public:
	...
	{
		if (splitAxis == 0)  return CKDTree::eSA_X;
		if (splitAxis == 1)  return CKDTree::eSA_Y;
		if (splitAxis == 2)  return CKDTree::eSA_Z;
	...
	KDTreeNode* GetChild(u32 nIndex) const
	{
		if (nIndex > 1)
			return NULL;
		return pChildren[nIndex];
	}
	void SetChild(u32 nIndex, KDTreeNode* pNode)
	{
		if (nIndex > 1)
			return;
		if (pChildren[nIndex])
	...
	void SetVertexIndexBuffer(std::vector<u32>& vertexInfos)
	{
		nVertexIndexBufferSize = (u32)vertexInfos.size();
		...
		{
			pVertexIndices = new u32[nVertexIndexBufferSize];
			memcpy(pVertexIndices, &vertexInfos[0], sizeof(u32) * nVertexIndexBufferSize);
		}
	}
	u32 GetVertexIndex(u32 nIndex) const
	{
	...
	u32 GetObjIndex(u32 nIndex) const
	{
		if (GetVertexBufferSize() == 1)
...
private:
	union
	{
		...
		u32  oneIndex;       // Leaf
		u32* pVertexIndices; // Leaf : high 8bits - object index, low 24bits - vertex index
	};
	union
	{
		u32 splitAxis;              // Interior
		u32 nVertexIndexBufferSize; // Leaf
	};
	AABB        boundbox;     // Both
...
bool SearchForBestSplitPos(CKDTree::ESplitAxis axis, const std::vector<CKDTree::SStatObj>& statObjList, std::vector<u32>& indices, float& outBestSplitPos)
{
	...
	i32 nSizeOfIndices(indices.size());

	for (i32 i = 0; i < nSizeOfIndices; ++i)
	{
		i32 nObjIndex = (indices[i] & 0xFF000000) >> 24;
		i32 nVertexIndex = (indices[i] & 0xFFFFFF);
...
{
	AABB                aboveBoundbox;
	std::vector<u32> aboveIndices;
	AABB                belowBoundbox;
	std::vector<u32> belowIndices;
};

bool SplitNode(const std::vector<CKDTree::SStatObj>& statObjList, const AABB& boundbox, const std::vector<u32>& indices, CKDTree::ESplitAxis splitAxis, float splitPos, SSplitInfo& outInfo)
{
...
bool CKDTree::Build(IStatObj* pStatObj)
{
	...
	for (i32 i = 0, iStatObjSize(m_StatObjectList.size()); i < iStatObjSize; ++i)
	{
		...
		for (i32 k = 0; k < meshDesc.m_nVertCount; ++k)
		{
...
void CKDTree::BuildRecursively(KDTreeNode* pNode, const AABB& boundbox, std::vector<u32>& indices) const
{
...
void CKDTree::ConstructStatObjList(IStatObj* pStatObj, const Matrix34& matParent)
{
	if (pStatObj == NULL)
		return;
	for (i32 i = 0, nChildObjSize(pStatObj->GetSubObjectCount()); i < nChildObjSize; ++i)
	{
...
bool CKDTree::FindNearestVertexRecursively(KDTreeNode* pNode, const Vec3& raySrc, const Vec3& rayDir, float vVertexBoxSize, const Vec3& localCameraPos, Vec3& outPos, Vec3& vOutHitPosOnCube) const
{
	...
	if (pNode->IsLeaf())
	{
		...
		u32 nVBuffSize = pNode->GetVertexBufferSize();
		if (nVBuffSize == 0)
			return false;

		float fNearestDist = 3e10f;

		for (u32 i = 0; i < nVBuffSize; ++i)
		{
			u32 nVertexIndex = pNode->GetVertexIndex(i);
			u32 nObjIndex = pNode->GetObjIndex(i);
