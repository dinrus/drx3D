...
struct DrxLogPythonOutput : public PyScript::IPyScriptListener
{

	virtual void OnStdOut(tukk pString) override
	{
		Log(pString);
	}

	virtual void OnStdErr(tukk pString) override
	{
		Warning(pString);
	}
...
void SPyWrappedProperty::SetProperties(IVariable* pVar)
{
	...
	case IVariable::VECTOR:
		...
		if (pVar->GetDataType() == IVariable::DT_COLOR)
		{
			...
			this->property.colorValue.r = (i32)GetRValue(col);
			this->property.colorValue.g = (i32)GetGValue(col);
			this->property.colorValue.b = (i32)GetBValue(col);
		}
		else
		{
...
PyGameLayer::PyGameLayer(uk layerPtr)
{
	m_layerPtr = layerPtr;
...
PyGameMaterial::PyGameMaterial(uk pMat)
{
	m_matPtr = pMat;
	...
	for (i32 i = 0; i < pMaterial->GetSubMaterialCount(); i++)
	{
...
PyGameSubMaterial::PyGameSubMaterial(uk pMat, i32 id)
{
	m_matPtr = pMat;
	...
	for (iter1 = tMap1.begin(); iter1 != tMap1.end(); ++iter1)
	{
		...
		value1->property.colorValue.r = (i32)GetRValue(color);
		value1->property.colorValue.g = (i32)GetGValue(color);
		value1->property.colorValue.b = (i32)GetBValue(color);
	...
	{
		for (i32 i = 0; i < varBlockPublic->GetNumVariables(); i++)
		{
	...
	{
		for (i32 i = 0; i < varBlockGen->GetNumVariables(); i++)
		{
...
void PyGameSubMaterial::UpdateSubMaterial()
{
	...
	for (i32 i = 0; i < m_matTextures.size(); i++)
	{
	...
	if (varBlockPublic != NULL && !varBlockPublic->IsEmpty())
	{
		for (i32 i = 0; i < varBlockPublic->GetNumVariables(); i++)
		{
			...
			for (i32 j = 0; j < shdResources.m_ShaderParams.size(); j++)
			{
...
PyGameTexture::PyGameTexture(uk pTex)
{
	m_texPtr = pTex;
...
PyGameObject::PyGameObject(uk objPtr)
{
	m_objPtr = objPtr;
...
PyGamePrefab::PyGamePrefab(uk prefabPtr, pSPyWrappedClass sharedPtr)
{
...
PyGameGroup::PyGameGroup(uk groupPtr, pSPyWrappedClass sharedPtr)
{
	m_groupPtr = groupPtr;
...
PyGameCamera::PyGameCamera(uk cameraPtr, pSPyWrappedClass sharedPtr)
{
...
PyGameBrush::PyGameBrush(uk brushPtr, pSPyWrappedClass sharedPtr)
{
	m_brushPtr = brushPtr;
...
PyGameEntity::PyGameEntity(uk entityPtr, pSPyWrappedClass sharedPtr)
{
	...
	{
		for (i32 i = 0; i < varBlockProps1->GetNumVariables(); i++)
		{
			IVariable* pVar = varBlockProps1->GetVariable(i);

			if (pVar->GetNumVariables() > 0)
			{
				for (i32 j = 0; j < pVar->GetNumVariables(); j++)
				{
	...
	{
		for (i32 i = 0; i < varBlockProps2->GetNumVariables(); i++)
		{
...
PyGameVegetationInstance::PyGameVegetationInstance(uk vegPtr)
{
	if (vegPtr == NULL)
	{
		m_vegPtr = NULL;
...
PyGameVegetation::PyGameVegetation(uk vegPtr)
{
	if (vegPtr == NULL)
	{
		m_vegPtr = NULL;
...
class CDrxStringToPythonConverter
{
public:
...
public:
	...
	static uk convertible(PyObject* pObject)
	{
	...
	{
		tukk pString = PyString_AsString(pObject);
		if (!pString)
		{
			boost::python::throw_error_already_set();
		}

		uk pStorage = ((boost::python::converter::rvalue_from_python_storage<string>*)pData)->storage.bytes;
...
{
public:
	static uk convertible(PyObject* pObj)
	{
		try
		{
	...
	static void construct_obj(PyObject* pObj, boost::python::converter::rvalue_from_python_stage1_data* pData)
	{
		try
		{
			uk storage = ((boost::python::converter::rvalue_from_python_storage<SPyWrappedProperty>*)pData)->storage.bytes;
	...
	static void construct_ptr(PyObject* pObj, boost::python::converter::rvalue_from_python_stage1_data* pData)
	{
		try
		{
			...
			uk storage = ((boost::python::converter::rvalue_from_python_storage<pSPyWrappedProperty>*)pData)->storage.bytes;
...
private:
	static SPyWrappedProperty construct(PyObject* pObj)
	{
		...
		{
			value.property.intValue = (i32)PyInt_AS_LONG(pObj);
		...
		else if (PyTuple_Check(pObj))
		{
			...
			else if (PyTuple_Size(pObj) == 3 && PyInt_Check(PyTuple_GetItem(pObj, 0)) && PyInt_Check(PyTuple_GetItem(pObj, 1)) && PyInt_Check(PyTuple_GetItem(pObj, 2)))
			{
				...
				value.property.colorValue.r = (i32)PyInt_AS_LONG(PyTuple_GetItem(pObj, 0));
				value.property.colorValue.g = (i32)PyInt_AS_LONG(PyTuple_GetItem(pObj, 1));
				value.property.colorValue.b = (i32)PyInt_AS_LONG(PyTuple_GetItem(pObj, 2));
			}
			else if (PyTuple_Size(pObj) == 2 && PyInt_Check(PyTuple_GetItem(pObj, 0)) && PyInt_Check(PyTuple_GetItem(pObj, 1)))
			{
				...
				value.property.timeValue.hour = (i32)PyInt_AS_LONG(PyTuple_GetItem(pObj, 0));
				value.property.timeValue.min = (i32)PyInt_AS_LONG(PyTuple_GetItem(pObj, 1));
			}
		}
		return value;
	}
...
{
public:
	static uk convertible(PyObject* pObjPtr)
	{
	...
	static void construct(PyObject* pObjPtr, boost::python::converter::rvalue_from_python_stage1_data* pData)
	{
		...
		uk storage = ((boost::python::converter::rvalue_from_python_storage<pSPyWrappedClass>*)pData)->storage.bytes;
...
// Converter for Python list <-> std::vector<STxt>
class CStdStringVectorToPythonConverter
{
public:
	static PyObject* convert(std::vector<STxt> const& stringVector)
	{
		boost::python::list pyList;

		for (i32 i = 0; i < stringVector.size(); i++)
		{
...
public:
	static uk convertible(PyObject* pObjPtr)
	{
		...
		{
			for (i32 i = 0; i < PyList_GET_SIZE(pObjPtr); i++)
			{
	...
	static void construct(PyObject* pObjPtr, boost::python::converter::rvalue_from_python_stage1_data* pData)
	{
		...
		uk storage = (
		  (boost::python::converter::rvalue_from_python_storage<std::vector<STxt>>*)
		  pData)->storage.bytes;

		new(storage) std::vector<STxt>(value);
...
public:
	static uk convertible(PyObject* pObjPtr)
	{
		...
		tuk value = (tuk)PyString_AsString(pObjPtr);

		// Check for a standard GUID len of 38 and that the string starts and ends with curly braces.
		if (strlen(value) != 38 ||
		    strcmp((tukk)value[0], "{") != 0 ||
		    strcmp((tukk)value[strlen(value) - 1], "}") != 0)
		{
			return NULL;
		}
	...
	static void construct(PyObject* pObjPtr, boost::python::converter::rvalue_from_python_stage1_data* pData)
	{
		...
		if (PyString_Check(pObjPtr))
		{
			guid = static_cast<GUID>(GuidUtil::FromString((tukk)PyString_AsString));
		}
...
void InitializePyConverters()
{
	...
	boost::python::converter::registry::push_back(
	  ...
	  boost::python::type_id<std::vector<STxt>>());
...
typedef std::function<void (tukk output)> TRedirectWriteFunc;
...
void InitCppClasses()
{
	...
	class_<std::vector<i32>>("int_vector")
	.def(vector_indexing_suite<std::vector<i32>>());
...
tukk GetAsString(tukk varName)
{
	try
	{
		return GetPyValue<tukk >(varName);
	}
...
DrxLogPythonOutput* s_pLogOutput;
...
{
	s_pLogOutput = new DrxLogPythonOutput();
	...
	Py_SetPythonHome((tuk)pythonPath.c_str());
