...
struct SEditorPaintBrush
{
	...
	u32        m_dwLayerIdMask;    // reference Value for the mask, 0xffffffff if not used
...
public:
	...
	//   fpx - 0..1 in the whole terrain (used for the mask)
	//   fpy - 0..1 in the whole terrain (used for the mask)
	void PaintBrushWithPattern(const float fpx, const float fpy, CImageEx& outImage, u32k dwOffsetX, u32k dwOffsetY,
	...
	ColorF ReadColorFBilinear(float x, float y, const ColorB* pImg, i32 imgSizeW, i32 imgSizeH);
	ColorF ReadColorF(i32 x, i32 y, const ColorB* pImg, i32 imgSizeW, i32 imgSizeH);

	//
	void FillWithPattern(CImageEx& outImage, u32k dwOffsetX, u32k dwOffsetY, const CImageEx& imgPattern);
