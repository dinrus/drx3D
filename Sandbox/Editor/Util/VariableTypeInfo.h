...
inline string SpacedName(tukk sName)
{
	// Split name with spaces.
	string sSpacedName = sName;
	for (i32 i = 1; i < sSpacedName.GetLength(); i++)
	{
...
public:
	...
	static IVariable* Create(CTypeInfo::CVarInfo const& VarInfo, uk pBaseAddress, ukk pBaseAddressDefault);
	...
	static EType      GetType(CTypeInfo const& TypeInfo)
	{
		...
		if (TypeInfo.HasSubVars())
		{
			...
			return BOOL;
		else if (TypeInfo.IsType<i32>() || TypeInfo.IsType<uint>())
			return INT;
	...
	CVariableTypeInfo(CTypeInfo::CVarInfo const& VarInfo, uk pAddress, ukk pAddressDefault, EType eType)
	...
	virtual i32   GetSize() const { return m_pTypeInfo->Size; }
	...
	virtual void Get(i32& value) const
	{
		m_pTypeInfo->ToValue(m_pData, value);
	}
...
protected:
	...
	uk                      m_pData;        // Existing address in memory. Directly modified.
	ukk                m_pDefaultData; // Address of default data for this var.
...
class CVariableTypeInfoEnum : public CVariableTypeInfo
{
	struct CTypeInfoEnumList : IVarEnumList
	{
		...
		virtual tukk GetItemName(uint index) { return TypeInfo.EnumElem(index); }
	};

public:

	// Constructor.
	CVariableTypeInfoEnum(CTypeInfo::CVarInfo const& VarInfo, uk pAddress, ukk pAddressDefault, IVarEnumList* pEnumList = 0)
...
public:
	...
	// Constructor.
	CVariableTypeInfoSpline(CTypeInfo::CVarInfo const& VarInfo, uk pAddress, ukk pAddressDefault, ISplineInterpolator* pSpline)
...
public:
	...
	// Constructor.
	CVariableTypeInfoStruct(CTypeInfo::CVarInfo const& VarInfo, uk pAddress, ukk pAddressDefault)
		: CVariableTypeInfo(VarInfo, pAddress, pAddressDefault, ARRAY)
	{
		ProcessSubStruct(VarInfo, pAddress, pAddressDefault);
	}

	void ProcessSubStruct(CTypeInfo::CVarInfo const& VarInfo, uk pAddress, ukk pAddressDefault)
	{
	...
	{
		return (tukk)m_pTypeInfo->ToString(m_pData);
	}
	...
	IVariable* GetVariable(i32 index) const { return m_Vars[index]; }
...
IVariable* CVariableTypeInfo::Create(CTypeInfo::CVarInfo const& VarInfo, uk pAddress, ukk pAddressDefault)
{
