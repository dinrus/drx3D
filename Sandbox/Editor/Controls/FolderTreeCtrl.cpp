...
{
	const CString filePath = PathUtil::ToUnixPath(PathUtil::AbsolutePathToDrxPakPath(rChange.filename.GetString())).c_str();
	for (auto item = m_folders.begin(), end = m_folders.end(); item != end; ++item)
	{
...
CString CFolderTreeCtrl::CalculateFolderFullPath(const std::vector<string>& splittedFolder, i32 idx)
{
	CString path;
	for (i32 segIdx = 0; segIdx <= idx; ++segIdx)
	{
		if (segIdx != 0)
			path.Append("/");
...
CFolderTreeCtrl::CTreeItem* CFolderTreeCtrl::CreateFolderItems(const CString& folder)
{
	...
	if (!m_bFlatStyle)
	{
		...
		CString fullpath;
		i32k splittedFoldersCount = splittedFolder.size();
		for (i32 idx = 0; idx < splittedFoldersCount; ++idx)
		{
...
CFolderTreeCtrl::CTreeItem::CTreeItem(CFolderTreeCtrl& folderTreeCtrl, CFolderTreeCtrl::CTreeItem* parent,
                                      const CString& name, const CString& path, i32k image)
...
CFolderTreeCtrl::CTreeItem* CFolderTreeCtrl::CTreeItem::AddChild(const CString& name, const CString& path, i32k image)
{
