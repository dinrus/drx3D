	...
	: QWidget(parent)
{
	...
	searchButton->setIcon(DrxIcon("icons:General/Search.ico"));
...
void CActiveLayerWidget::UpdateLabel()
{
	...
	m_label->setText(QString(tr("Active Layer: %1")).arg(activeLayer ? (tukk)activeLayer->GetName() : "None"));
}
	...
	, m_treeView(nullptr)
{
	...
	m_pAttributeFilterProxyModel->setFilterKeyColumn((i32)eLayerColumns_Name);
	...
	delegate->SetColumnBehavior((i32)eLayerColumns_Visible, QAdvancedItemDelegate::BehaviorFlags(QAdvancedItemDelegate::DragCheck | QAdvancedItemDelegate::OverrideCheckIcon));
	delegate->SetColumnBehavior((i32)eLayerColumns_Frozen, QAdvancedItemDelegate::BehaviorFlags(QAdvancedItemDelegate::DragCheck | QAdvancedItemDelegate::OverrideCheckIcon));
	delegate->SetColumnBehavior((i32)eLayerColumns_Color, QAdvancedItemDelegate::BehaviorFlags(QAdvancedItemDelegate::DragCheck | QAdvancedItemDelegate::OverrideCheckIcon | QAdvancedItemDelegate::OverrideDrawRect));
...
void CLevelExplorer::InitMenuBar()
{
	...
	auto action = fileMenu->CreateAction(DrxIcon("icons:General/File_New.ico"), tr("New Layer"));
	connect(action, &QAction::triggered, [&]() { OnNewLayer(nullptr); });

	action = fileMenu->CreateAction(DrxIcon("icons:General/Folder_Add.ico"), tr("New Folder"));
	connect(action, &QAction::triggered, [&]() { OnNewFolder(nullptr); });

	i32 sec = fileMenu->GetNextEmptySection();
...
QVariantMap CLevelExplorer::GetLayout() const
{
	...
	state.insert("model", (i32)m_modelType);
...
void CLevelExplorer::OnContextMenu(const QPoint& pos) const
{
	...
	if (layerFolders.size() == 1)
	{
		...
		auto action = menu->addAction(DrxIcon("icons:General/File_New.ico"), tr("New Layer"));
		connect(action, &QAction::triggered, [&]() { OnNewLayer(layerFolders[0]); });

		action = menu->addAction(DrxIcon("icons:General/Folder_Add.ico"), tr("New Folder"));
	...
	if (layers.size())
	{
		...
		if (layers.size() == 1)
		{
			...
			connect(action, &QAction::triggered, [&]()
			{
				QtUtil::OpenInExplorer((tukk)layer->GetLayerFilepath());
			});
...
void CLevelExplorer::OnClick(const QModelIndex& index)
{
	if (index.isValid())
	{
		...
		case Qt::KeyboardModifier::AltModifier:
			{
				auto type = index.data((i32)CLevelModel::Roles::TypeCheckRole);
				if (type.toInt() == ELevelElementType::eLevelElement_Layer)
				{
					CObjectLayer* pLayer = reinterpret_cast<CObjectLayer*>(index.data((i32)CLevelLayerModel::Roles::InternalPointerRole).value<intptr_t>());
		...
		case Qt::KeyboardModifier::ControlModifier:
			{
				...
				{
					CBaseObject* pObject = reinterpret_cast<CBaseObject*>(index.data((i32)CLevelLayerModel::Roles::InternalPointerRole).value<intptr_t>());
					if (pObject)
					{
...
bool CLevelExplorer::OnDelete()
{
	...
	{
		QVariant type = index.data((i32)CLevelModel::Roles::TypeCheckRole);
		QVariant internalPtrVar = index.data((i32)CLevelModel::Roles::InternalPointerRole);
		switch (type.toInt())
		{
		case ELevelElementType::eLevelElement_Layer:
			{
...
QModelIndex CLevelExplorer::FindLayerIndexInModel(const CObjectLayer* pLayer) const
{
	...
	for (i32 i = 0; i < count; i++)
	{
		...
		const QVariant internalPtrVar = index.data((i32)CLevelModel::Roles::InternalPointerRole);
	...
	for (i32 i = nestingChain.size() - 2; i >= 0; --i)
	{
		auto pChainChild = nestingChain[i];
		for (i32 childIndex = 0; childIndex < pCurrentLayer->GetChildCount(); ++childIndex)
		{
...
QModelIndex CLevelExplorer::FindIndexByInternalId(intptr_t id) const
{
	...
	auto matchingList = model->match(model->index(0, 0, QModelIndex()), (i32)CLevelModel::Roles::InternalPointerRole, id, 1, Qt::MatchRecursive | Qt::MatchExactly);
...
void CLevelExplorer::OnSelectionChanged(const QItemSelection& selected, const QItemSelection& deselected)
{
	...
	//validate that selections is synced properly, objects can be selected in the manager but not in the table selection (happens for instance if you search/filter and then select)
	{
		...
		LevelModelsUtil::ProcessIndexList(selection, objects, layers, layerFolders);
		i32k objManSelectionCount = objManSelection->GetCount();

		if (objManSelectionCount != objects.size())
		{
			for (i32 i = 0; i < objManSelectionCount; i++)
			{
...
{
	m_treeView->edit(index.sibling(index.row(), (i32)eLayerColumns_Name));
}
...
void CLevelExplorer::OnHeaderSectionCountChanged()
{
	...
	// to reset them
	m_treeView->header()->resizeSection((i32)eLayerColumns_Color, 10);
	m_treeView->header()->resizeSection((i32)eLayerColumns_Visible, 25);
	m_treeView->header()->resizeSection((i32)eLayerColumns_Frozen, 25);
}
...
void CLevelExplorer::OnUnfreezeAllInLayer(CObjectLayer* layer) const
{
	...
	for (i32 i = 0; i < layer->GetChildCount(); i++)
	{
