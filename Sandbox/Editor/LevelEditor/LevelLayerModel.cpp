...
QVariant CLevelLayerModel::GetHeaderData(i32 section, Qt::Orientation orientation, i32 role)
{
	...
	if (role == Qt::DecorationRole)
	{
		if (section == eObjectColumns_Visible)
			return DrxIcon("icons:General/Visibility_True.ico");
		if (section == eObjectColumns_Frozen)
			return DrxIcon("icons:Navigation/Basics_Select_False.ico");
	}
...
i32 CLevelLayerModel::rowCount(const QModelIndex& parent) const
{
...
QVariant CLevelLayerModel::data(const QModelIndex& index, i32 role) const
{
	if (!index.isValid())
	{
		return QVariant();
	...
	switch (role)
	{
	case (i32)CLevelModel::Roles::TypeCheckRole:
		return (i32)eLevelElement_Object;
	case (i32)Roles::InternalPointerRole:
	...
	case Qt::EditRole:
		{
			...
			if (pObject)
			{
				...
				case eObjectColumns_TypeDesc:
					return (tukk)pObject->GetTypeDescription();
				case eObjectColumns_DefaultMaterial:
				...
				case eObjectColumns_InstanceCount:
					{
						i32 count = GetInstancesCount(pObject);
						return (count > 0) ? count : QVariant();
					}
				case eObjectColumns_LODCount:
					{
						i32 lods = GETLODNumber(pObject);
				...
				case eObjectColumns_AI_GroupId:
					{
						i32 group = GetAIGroupID(pObject);
	...
	case Qt::DecorationRole:
		if (index.column() == eObjectColumns_Name)
		{
			...
			if (pLinkedObject)
				return DrxIcon("icons:ObjectTypes/Link.ico").pixmap(16, 16, QIcon::Active, pObject->IsSelected() ? QIcon::On : QIcon::Off);

			CObjectClassDesc* const classDesc = pObject->GetClassDesc();
			if(classDesc == nullptr)
				return DrxIcon("icons:General/Placeholder.ico").pixmap(16, 16, QIcon::Active, pObject->IsSelected() ? QIcon::On : QIcon::Off);
	...
	case QAdvancedItemDelegate::s_IconOverrideRole:
		{
			...
			case eObjectColumns_Visible:
				if (pObject->IsVisible())
				{
					...
					return DrxIcon("icons:General/Visibility_False.ico");
			case eObjectColumns_Frozen:
				if (pObject->IsFrozen())
					return DrxIcon("icons:General/editable_false.ico");
				else
					return DrxIcon("icons:General/editable_true.ico");
			default:
				break;
			}
			break;
...
QVariant CLevelLayerModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
QModelIndex CLevelLayerModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
	...
	if (CBaseObject* pParent = ObjectFromIndex(parent))
	{
		...
		else if (pParent->GetLinkedObjectCount())
		{
			...
			i32 idx = row - pParent->GetChildCount();
...
bool CLevelLayerModel::canDropMimeData(const QMimeData* pData, Qt::DropAction action, i32 row, i32 column, const QModelIndex& parent) const
{
...
bool CLevelLayerModel::dropMimeData(const QMimeData* pData, Qt::DropAction action, i32 row, i32 column, const QModelIndex& parent)
{
	if (action != Qt::MoveAction)
	{
		return false;
...
bool CLevelLayerModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
	CBaseObject* pObject = ObjectFromIndex(index);
	if (pObject)
	{
...
i32 CLevelLayerModel::RowFromObject(const CBaseObject* pObject) const
{
	if (!pObject)
	{
...
bool CLevelLayerModel::Filter(CBaseObject const& obj, uk pLayer)
{
...
void CLevelLayerModel::RemoveObjects(i32 row, i32 count /* = 1 */)
{
...
void CLevelLayerModel::OnObjectEvent(CObjectEvent& eventObj)
{
	...
	switch (eventObj.m_type)
	{
		case OBJECT_ON_ADD:
		{
			...
			i32 irow = RowFromObject(pObject);
		...
		{
			// we need to be slightly careful here because objects get named before being added, so we might try to access an invalid index.
			QVector<i32> updateRoles(1, Qt::DisplayRole);
		...
		case OBJECT_ON_UNSELECT:
		{
			...
			QVector<i32> updateRoles(1, Qt::DecorationRole);
			NotifyUpdateObject(pObject, updateRoles);
		}
		break;
		default:
...
void CLevelLayerModel::OnBeforeObjectsAttached(CBaseObject* pParent, const std::vector<CBaseObject*>& objects, bool keepTransform)
{
	...
	i32 row = RowFromObject(objects[0]);
	if (row == -1)
	{
		return;
	}
	...
	DRX_ASSERT_MESSAGE(pFutureLayer == m_pLayer, "Layer Model: Attaching children between layers is unsupported");

	QModelIndex parentIdx = IndexFromObject(pParent);
	i32 parentRow = pParent->GetChildCount();
...
void CLevelLayerModel::OnObjectsAttached(CBaseObject* pParent, const std::vector<CBaseObject*>& objects)
{
	...
	DRX_ASSERT_MESSAGE(objects[0]->GetParent()->GetLayer() == m_pLayer, "Layer Model: Attaching children between layers is unsupported");
...
void CLevelLayerModel::OnSelectionChanged()
{
	...
	for(i32 i = 0, count = pSelection->GetCount(); i < count; ++i)
	{
...
void CLevelLayerModel::OnUnLink(CBaseObject* pObject)
{
	...
	if (pObject->GetLayer() == m_pLayer)
	{
		i32k row = RowFromObject(pObject);
		if (row < 0)
		{
			AddObject(pObject);
		}
	...
	for (auto i = 0; i < pObject->GetLinkedObjectCount(); ++i)
	{
		...
		if (pLinkedObject->GetLayer() == m_pLayer)
		{
			i32k row = RowFromObject(pLinkedObject);
			if (row < 0)
			{
...
tukk CLevelLayerModel::GetMaterialName(CBaseObject* pObject, bool bUseCustomMaterial) const
{
...
tukk CLevelLayerModel::GetObjectBreakability(CBaseObject* pObject) const
{
...
void CLevelLayerModel::GetMaterialBreakability(std::set<string>* breakTypes, CMaterial* pMaterial) const
{
	if (pMaterial)
	{
		...
		if (pSurfaceManager)
		{
			...
			i32 subMaterialCount = pMaterial->GetSubMaterialCount();
...
tukk CLevelLayerModel::GetSmartObject(CBaseObject* pObject) const
{
	if (pObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
	{
		CEntityObject* pEntity = (CEntityObject*)pObject;
		return (tukk)pEntity->GetSmartObjectClass();
	}
	return "";
}

tukk CLevelLayerModel::GetFlowGraphNames(CBaseObject* pObject) const
{
...
tukk CLevelLayerModel::GetGeometryFile(CBaseObject* pObject) const
{
	if (pObject->IsKindOf(RUNTIME_CLASS(CBrushObject)))
	{
	...
	else if (pObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
	{
		...
		if (pEngineNode)
		{
			...
			if (pEntityCharacter)
			{
				...
				{
					u32 numInstances = gEnv->pCharacterManager->GetNumInstancesPerModel(defaultSkeleton);
					if (numInstances > 0)
					{
...
i32 CLevelLayerModel::GetAIGroupID(CBaseObject* pObject) const
{
...
void CLevelLayerModel::NotifyUpdateObject(CBaseObject* pObject, const QVector<i32>& updateRoles)
{
