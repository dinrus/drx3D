...
//////////////////////////////////////////////////////////////////////////
static void ToggleInt(i32* variable)
{
	*variable = !*variable;
}
//////////////////////////////////////////////////////////////////////////
static CPopupMenuItem& AddCheckbox(CPopupMenuItem& menu, tukk text, bool* variable, bool* disableVariableIfOn = 0)
{
	return menu.Add(text, functor(ToggleBool), variable, disableVariableIfOn).Check(*variable);
}

//////////////////////////////////////////////////////////////////////////
static CPopupMenuItem& AddCheckbox(CPopupMenuItem& menu, tukk text, i32* variable)
{
...
namespace Private_AssetDrag
{
	static bool TryAssetDragCreate(const CDragDropData* pDragDropData, string& className)
	{
		...
		tukk szClassName = pAsset->GetType()->GetObjectClassName();
		if (!szClassName)
		{
			return false;
		}
...
bool CLevelEditorViewport::DragEvent(EDragEvent eventId, QEvent* event, i32 flags)
{
...
bool CLevelEditorViewport::AssetDragCreate(EDragEvent eventId, QEvent* event, i32 flags)
{
...
void CLevelEditorViewport::OnEditorNotifyEvent(EEditorNotifyEvent event)
{
	...
	if(event == eNotify_OnEndNewScene)
	{
		...
		viewTM.SetTranslation(Vec3(sx * pHmap->GetUnitSize(), sy * pHmap->GetUnitSize(), pHmap->GetXY(static_cast<u32>(sx), static_cast<u32>(sy)) + 2.0f));
		SetViewTM(viewTM);
	}
}
...
void CLevelEditorViewport::AddCameraMenuItems(CPopupMenuItem& menu)
{
	...
	if (numCameras > 0)
	{
		for (i32 i = 0; i < cameraObjects.size(); ++i)
		{
	...
	if (m_cameraObjectId != DrxGUID::Null() || bIsDelegateActive)
	{
		menu.AddSeparator();
...
struct CVarAutoSetAndRestore
{
	...
	CVarAutoSetAndRestore(tukk cvarName, T newValue)
	{
...
template<>
i32 CVarAutoSetAndRestore<i32>::GetCVarValue()
{
...
void CLevelEditorViewport::OnRender()
{
	...
	i32 w = m_currentResolution.width;
	i32 h = m_currentResolution.height;
	...
	if (cameraObject)
	{
		...
		if (cameraObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			...
			{
				if (Drx::DefaultComponents::CCameraComponent* pCameraComponent = pCameraEntity->GetComponent<Drx::DefaultComponents::CCameraComponent>())
				{
					...
					m_Camera.m_bOmniCamera = pCameraComponent->GetType() == Drx::DefaultComponents::ECameraType::Omnidirectional;
				}
				else
				{
	...
	i32k renderFlags = (GetType() == ET_ViewportCamera) ? 0 : SHDF_SECONDARY_VIEWPORT;
	...
	if (m_Camera.m_bOmniCamera)
	{
		...
		CVarAutoSetAndRestore<i32> intCvars[] = { CVarAutoSetAndRestore<i32>("r_motionblur", 0),
			CVarAutoSetAndRestore<i32>("r_HDRVignetting", 0),
			CVarAutoSetAndRestore<i32>("r_AntialiasingMode", 0),
			CVarAutoSetAndRestore<i32>("e_clouds", 0),
			CVarAutoSetAndRestore<i32>("r_sunshafts", 0),
			CVarAutoSetAndRestore<i32>("e_CoverageBuffer", 0),
			CVarAutoSetAndRestore<i32>("e_StatObjBufferRenderTasks", 0) };
...
void CLevelEditorViewport::RenderSnappingGrid()
{
		...
		// Draw the translation grid.
	{
		...
		i32k MIN_STEP_COUNT = 5;
		i32k MAX_STEP_COUNT = 300;
		i32 nSteps = std::min(std::max(pos_directed_rounding(size / step), MIN_STEP_COUNT), MAX_STEP_COUNT);
		size = nSteps * step;
		for (i32 i = -nSteps; i <= nSteps; ++i)
		{
			// Draw u lines.
		...
		// Draw the rotation grid.
	{
		...
		if (nAxis == AXIS_X || nAxis == AXIS_Y || nAxis == AXIS_Z)
		{
			...
			i32 nSteps = pos_directed_rounding(180.0f / step);
...
void CLevelEditorViewport::OnMenuCreateCameraFromCurrentView()
{
	...
	DrxGUID cameraComponentGUID = Schematyc::GetTypeGUID<Drx::DefaultComponents::CCameraComponent>();
	...
	if (CEntityObject* pNewCameraObject = static_cast<CEntityObject*>(pObjMgr->NewObject("EntityWithComponent", nullptr, cameraComponentGUID.ToString())))
	{
		...
		auto* pCameraComponent = pNewCameraObject->GetIEntity()->GetComponent<Drx::DefaultComponents::CCameraComponent>();
		DRX_ASSERT(pCameraComponent != nullptr);
		if(pCameraComponent != nullptr)
		{
			pCameraComponent->SetFieldOfView(DrxTransform::CAngle::FromRadians(GetFOV()));
		}
...
void CLevelEditorViewport::SetFOV(float fov)
{
	if (CEntityObject* pCameraObject = static_cast<CEntityObject*>(GetCameraObject()))
	{
		...
		else if (IEntity* pCameraEntity = pCameraObject->GetIEntity())
		{
			if (Drx::DefaultComponents::CCameraComponent* pCameraComponent = pCameraEntity->GetComponent<Drx::DefaultComponents::CCameraComponent>())
			{
				pCameraComponent->SetFieldOfView(DrxTransform::CAngle::FromRadians(fov));
			}
		}
	}
...
void CLevelEditorViewport::SetFOVDeg(float fov)
{
	...
	if (GetIEditor()->GetViewportManager()->GetCameraObjectId() == DrxGUID::Null())
...
Vec3 CLevelEditorViewport::ViewToWorld(POINT vp, bool* collideWithTerrain, bool onlyTerrain, bool bSkipVegetation, bool bTestRenderMesh) const
{
	...
	const CSelectionGroup* sel = GetIEditorImpl()->GetSelection();
	m_numSkipEnts = 0;
	for (i32 i = 0; i < sel->GetCount() && m_numSkipEnts < 32; i++)
	{
		m_pSkipEnts[m_numSkipEnts++] = sel->GetObject(i)->GetCollisionEntity();
	}

	i32 col = 0;
	i32k queryFlags = (onlyTerrain || GetIEditor()->IsSnapToTerrainEnabled()) ? ent_terrain : ent_all;
...
Vec3 CLevelEditorViewport::ViewToWorldNormal(POINT vp, bool onlyTerrain, bool bTestRenderMesh)
{
	...
	i32 col = 1;
	i32k queryFlags = (onlyTerrain || GetIEditor()->IsSnapToGeometryEnabled()) ? ent_terrain : ent_terrain | ent_static;
	while (col)
	{
		hit.pCollider = 0;
		col = world->RayWorldIntersection(vPos, vDir, queryFlags, flags, &hit, 1, m_pSkipEnts, m_numSkipEnts);
...
bool CLevelEditorViewport::MouseCallback(EMouseEvent event, CPoint& point, i32 flags)
{
