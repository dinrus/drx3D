...
QMimeData* GetDragDropData(const QModelIndexList& list)
{
	...
	{
		stream << (i32)eLevelElement_Layer;
		stream << (i32)size;
	...
	{
		stream << (i32)eLevelElement_Object;
		stream << (i32)size;
...
//See IEntitySystem.h
QString CLevelModel::LayerSpecToString(i32 spec)
{
	QString str;
...
i32 CLevelModel::LayerSpecToInt(const QString& specStr)
{
	i32 spec;
...
CItemModelAttribute* CLevelModel::GetColumnAttribute(i32 column) const
{
	switch (column)
	{
...
//////////////////////////////////////////////////////////
QVariant CLevelModel::data(const QModelIndex& index, i32 role) const
{
	if (!index.isValid())
	{
		return QVariant();
	...
	if (role == (i32)CLevelModel::Roles::TypeCheckRole)
	{
		return (i32)eLevelElement_Layer;
	}

	CObjectLayer* pLayer = LayerFromIndex(index);
	if (pLayer)
	{
		if (role == (i32)Roles::InternalPointerRole)
		{
		...
		case ELayerColumns::eLayerColumns_Name:
			...
			case Qt::DisplayRole:
			case Qt::EditRole:
				return (tukk)pLayer->GetName();
			case Qt::ToolTipRole:
				{
					if (pLayer->GetLayerType() == eObjectLayerType_Layer)
					{
						if (GetIEditorImpl()->GetObjectManager()->GetLayersManager()->GetCurrentLayer() == pLayer)
							return QString("%1 (Active Layer)").arg((tukk)pLayer->GetName());
						else
							return (tukk)pLayer->GetName();
					}
			...
			case Qt::DecorationRole:
				...
				{
					if (GetIEditorImpl()->GetObjectManager()->GetLayersManager()->GetCurrentLayer() == pLayer)
						return DrxIcon("icons:General/layer_active.ico").pixmap(16, 16, QIcon::Normal, QIcon::On);
					else
						return DrxIcon("icons:General/layer.ico").pixmap(16, 16);
				}
				else if (pLayer->GetLayerType() == eObjectLayerType_Folder)
				{
					...
					if(currentLayer && currentLayer->IsChildOf(pLayer))
						return DrxIcon("icons:General/Folder_Tree_Active.ico").pixmap(16, 16);
					else
						return DrxIcon("icons:General/Folder_Tree.ico").pixmap(16, 16);
				} 
				else if (pLayer->GetLayerType() == eObjectLayerType_Terrain)
				{
					return DrxIcon("icons:Tools/tools_terrain-editor.ico").pixmap(16, 16);
				}
			}
			break;
...
//////////////////////////////////////////////////////////
bool CLevelModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
	CObjectLayer* pLayer = LayerFromIndex(index);
	if (pLayer)
	{
		...
		case ELayerColumns::eLayerColumns_Name:
			if (role == Qt::EditRole)
			{
				...
				tukk szName = newName.c_str();
				if (szName == pLayer->GetName())
					return false;

				if (!IsNameValid(szName))
				{
					DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Layer name '%s' is already in use", szName);
					return false;
				}
...
//////////////////////////////////////////////////////////
QVariant CLevelModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
//////////////////////////////////////////////////////////
QModelIndex CLevelModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
	if ((row >= 0) && (column >= 0))
	{
...
//////////////////////////////////////////////////////////
bool CLevelModel::dropMimeData(const QMimeData* pData, Qt::DropAction action, i32 row, i32, const QModelIndex& parent)
{
...
//////////////////////////////////////////////////////////
bool CLevelModel::canDropMimeData(const QMimeData* pData, Qt::DropAction action, i32 row, i32 column, const QModelIndex& parent) const
{
	if (action != Qt::MoveAction)
	{
		return false;
...
//////////////////////////////////////////////////////////
i32 CLevelModel::rowCount(const QModelIndex& parent) const
{
...
QModelIndex CLevelModel::IndexFromLayer(const CObjectLayer* pLayer) const
{
	if (pLayer)
	{
		...
		if (pParent)
		{
			i32k size = pParent->GetChildCount();
...
i32 CLevelModel::RootRowIfInserted(const CObjectLayer* pLayer) const
{
...
//////////////////////////////////////////////////////////
bool CLevelModel::IsNameValid(tukk szName)
{
	...
	case CLayerChangeEvent::LE_BEFORE_ADD:
		{
			...
			{
				i32 row = RootRowIfInserted(event.m_layer);
