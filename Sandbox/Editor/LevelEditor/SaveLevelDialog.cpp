...
struct CSaveLevelDialog::Implementation
{
	...
	struct Ui
	{
		void Setup(const Models& models, QWidget* parent, const QString& title)
		{
			...
			if (!dir.mkpath(levelDir))
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "No levels folder found in current project");
			...
			for (i32 i = 1; i < models.pLevelFilter->columnCount(); ++i)
			{
	...
	bool CheckUserLevelPath(const QString& userPath)
	{
		...
		if (!DrxStringUtils::IsValidFileName(filename.c_str()))
		{
