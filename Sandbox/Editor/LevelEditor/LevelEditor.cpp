...
std::vector<string> CollectDrxLevels()
{
	...
	levels.erase(std::remove_if(levels.begin(), levels.end(), [](const string& x)
	{
		return stricmp(PathUtil::GetExt(x.c_str()), "cry") != 0
			|| GetISystem()->GetIPak()->IsFileExist(PathUtil::ReplaceExtension(x, "level"), IDrxPak::eFileLocation_OnDisk);
	}), levels.end());
...
bool UnpakDrxFile(IDrxPak* pDrxPak, const string& cryFilename, const string& destinationFolder, std::function<void(float)> progress)
{
	// Relative to the project root directory.
	const string cryPakPath = PathUtil::Make(PathUtil::GetGameFolder(), cryFilename);
	if (!pDrxPak->OpenPack(cryPakPath))
	{
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_WARNING, "Could not open pak %s", cryFilename.c_str());
		return false;
	}

	PathUtil::Unpak(cryPakPath, destinationFolder, std::move(progress));
	pDrxPak->ClosePack(cryPakPath);
	return true;
}
...
void UpdateLevels(const std::vector<string>& levels)
{
	...
	for (size_t i = 0, n = levels.size(); i < n; ++i)
	{
		...
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_COMMENT, "Updating level: %s", oldFilePath.c_str());

		tukk const szOldFilename = PathUtil::GetFile(oldFilePath.c_str());
		...
		if (!UnpakDrxFile(pDrxPak, oldFilePath, levelFolder, updateProgressBar) || !pLevelEditor->ConvertEditorXmlToLevelAssetType(levelFolder, szOldFilename))
		{
			DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_WARNING, "Could not import level %s", szOldFilename);
			continue;
		}
	...
	if (countOfImported)
	{
		DrxWarning(EValidatorModule::VALIDATOR_MODULE_EDITOR, EValidatorSeverity::VALIDATOR_WARNING, "%i levels have been updated to be compatible with the new version of CRYENGINE.\n", countOfImported);
	}
}
...
void CLevelEditor::OnEditorNotifyEvent(EEditorNotifyEvent event)
{
	...
	else if (event == eNotify_OnMainFrameCreated)
	{
		ThreadingUtils::AsyncQueue([this]()
		{
			...
			std::vector<string> levels = CollectDrxLevels();
			if (levels.empty())
			{
				return;
			}
...
void CLevelEditor::CreateRecentFilesMenu(QMenu* pRecentFilesMenu)
{
	...
	for (i32 i = 0; i < recentPaths.size(); ++i)
	{
...
bool CLevelEditor::ConvertEditorXmlToLevelAssetType(const string& levelFolder, tukk const szName) const
{
	...
	if (PathUtil::MoveFileAllowOverwrite(oldFilename, newFilename))
	{
		...
		CAsset asset(pType->GetTypeName(), DrxGUID::Create(), PathUtil::GetFile(cryassetPath));
...
bool CLevelEditor::OnOpen()
{
	...
	if (levelOpenDialog.exec() == QDialog::Accepted)
	{
		...
		CDrxEditApp::GetInstance()->LoadLevel(filename.c_str());
	}
	return true;
}
...
bool CLevelEditor::OnSaveAs()
{
	...
	if (levelSaveDialog.exec() == QDialog::Accepted)
	{
		...
		SaveDrxassetFile(GetIEditorImpl()->GetDocument()->GetPathName());
	}
	return true;
}
...
bool CLevelEditor::OnDelete()
{
	...
	string strAsk;
	i32 nSelectedObjects = GetIEditorImpl()->GetObjectManager()->GetSelection()->GetCount();
...
{
	CDrxEditApp::GetInstance()->OnEditDuplicate();
	return true;
}
...
void CLevelEditor::SaveDrxassetFile(const string& levelPath)
{
...
bool CLevelEditor::OnPaste()
{
	...
	archive.LoadInCurrentLayer(true);
	{
		...
		for (i32 i = 0, nObj = selGroup->GetCount(); i < nObj; ++i)
		{
...
{
	char buffer[31];
	drx_sprintf(buffer, "level.snap_to_grid '%i'", bEnable);
...
{
	char buffer[32];
	drx_sprintf(buffer, "level.snap_to_angle '%i'", bEnable);
	CommandEvent(buffer).SendToKeyboardFocus();
}
...
{
	char buffer[33];
	drx_sprintf(buffer, "level.snap_to_vertex '%i'", bEnable);
	CommandEvent(buffer).SendToKeyboardFocus();
}
...
{
	char buffer[32];
	drx_sprintf(buffer, "level.snap_to_pivot '%i'", bEnable);
	CommandEvent(buffer).SendToKeyboardFocus();
}
...
{
	char buffer[34];
	drx_sprintf(buffer, "level.snap_to_terrain '%i'", bEnable);
	CommandEvent(buffer).SendToKeyboardFocus();
}
...
{
	char buffer[35];
	drx_sprintf(buffer, "level.snap_to_geometry '%i'", bEnable);
	CommandEvent(buffer).SendToKeyboardFocus();
}
...
{
	char buffer[41];
	drx_sprintf(buffer, "level.snap_to_surface_normal '%i'", bEnable);
	CommandEvent(buffer).SendToKeyboardFocus();
}
