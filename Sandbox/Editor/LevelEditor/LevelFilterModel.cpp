...
QVariant CLevelFilterModel::data(const QModelIndex& index, i32 role) const
{
	switch (role)
	{
	case Qt::DecorationRole:
		if (IsIndexLevel(index))
		{
			...
			QIcon icon = DrxIcon(kLevelFileIconPath);
			return icon;
		}
		return QSortFilterProxyModel::data(index, role);
...
bool CLevelFilterModel::filterAcceptsRow(i32 sourceRow, const QModelIndex& sourceParent) const
{
...
bool CLevelFilterModel::IsIndexLevelOrContainsLevels(const QModelIndex& index, const QString& path) const
{
	...
	for (i32 i = 0, n = sourceModel()->rowCount(index); i < n; ++i)
	{
