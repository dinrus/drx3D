...
void CLensFlareLightEntityTree::OnLensFlareChangeItem(CLensFlareItem* pLensFlareItem)
{
	...
	for (i32 i = 0, iEntitySize(lightEntities.size()); i < iEntitySize; ++i)
...
void CLensFlareLightEntityTree::OnObjectEvent(CBaseObject* pObject, i32 nEvent)
{
...
void CLensFlareLightEntityTree::OnTvnItemDoubleClicked(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (pSelection)
	{
		i32 iSelectionCount(pSelection->GetCount());
