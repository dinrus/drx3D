...
public:
	...
	CLensFlareElement::LensFlareElementPtr InsertAtomicElement(i32 nIndex, EFlareType flareType, IOpticsElementBasePtr pParent);
	...
	CLensFlareElement::LensFlareElementPtr GetElement(i32 nIndex) const
	{
	...
	void                                   UpdateProperty();
	void                                   UpdateClipboard(tukk type, bool bPasteAtSameLevel);
...
protected:
	...
	XmlNodeRef                             CreateXML(tukk type) const;
	bool                                   GetClipboardList(tukk type, std::vector<LensFlareUtil::SClipboardData>& outList) const;

	CLensFlareElement::LensFlareElementPtr FindRootElement() const;
	i32                                    FindElementIndexByOptics(IOpticsElementBasePtr pOptics) const;
...
private:
	...
	HTREEITEM InsertElementToTree(i32 nIndex, HTREEITEM hParent);
