...
{
	for (i32 i = 0, iElementSize(m_LensFlareElements.size()); i < iElementSize; ++i)
	{
...
i32 CLensFlareElementTree::FindElementIndexByOptics(IOpticsElementBasePtr pOptics) const
{
	for (i32 i = 0, iElementSize(m_LensFlareElements.size()); i < iElementSize; ++i)
	{
...
void CLensFlareElementTree::AddLensElementRecursively(HTREEITEM hParent, IOpticsElementBasePtr pOptics)
{
	...
	i32 nIndex = FindElementIndexByOptics(pOptics);
	if (nIndex == -1)
		return;
...
CLensFlareElement::LensFlareElementPtr CLensFlareElementTree::InsertAtomicElement(i32 nIndex, EFlareType flareType, IOpticsElementBasePtr pParent)
{
	if (pParent == NULL)
		return NULL;
...
bool CLensFlareElementTree::AddElement(EFlareType flareType)
{
	...
	if (!LensFlareUtil::IsGroup(pSelectedElement->GetOpticsType()))
	{
		...
		i32 nIndex = LensFlareUtil::FindOpticsIndexUnderParentOptics(pSelectedElement->GetOpticsElement(), pParentElement->GetOpticsElement());
		if (nIndex != -1)
...
void CLensFlareElementTree::UpdateLensFlareElementsRecusively(IOpticsElementBasePtr pOptics)
{
	...
	for (i32 i = 0, iCount(pOptics->GetElementCount()); i < iCount; i++)
		UpdateLensFlareElementsRecusively(pOptics->GetElementAt(i));
}

HTREEITEM CLensFlareElementTree::InsertElementToTree(i32 nIndex, HTREEITEM hParent)
{
...
CLensFlareElement::LensFlareElementPtr CLensFlareElementTree::GetLensFlareElement(HTREEITEM hItem) const
{
	if (hItem == NULL)
		return NULL;
	i32 nItemIndex = (i32)GetItemData(hItem);
...
void CLensFlareElementTree::SelectTreeItemByLensFlareElement(const CLensFlareElement* pElement)
{
	...
	while (hVisibleItem)
	{
		i32 nItemIndex = (i32)GetItemData(hVisibleItem);
...
void CLensFlareElementTree::OnTvnEndlabeleditTree(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	else
	{
		...
		if (pLensFlareElement && pLensFlareElement->GetName(prevName))
		{
			...
			i32 offset = parentName.ReverseFind('.');
			if (offset == -1)
				parentName = "";
			else
...
XmlNodeRef CLensFlareElementTree::CreateXML(tukk type) const
{
...
void CLensFlareElementTree::UpdateClipboard(tukk type, bool bPasteAtSameLevel)
{
	std::vector<LensFlareUtil::SClipboardData> clipboardData;
	if (GetClipboardList(type, clipboardData))
...
bool CLensFlareElementTree::GetClipboardList(tukk type, std::vector<LensFlareUtil::SClipboardData>& outList) const
{
...
void CLensFlareElementTree::Paste(XmlNodeRef xmlNode)
{
	...
	for (i32 i = 0, iChildCount(xmlNode->getChildCount()); i < iChildCount; ++i)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "[%s] lens item can be pasted into the same item", clipboardData.m_LensFlareFullPath);
			continue;
		}
		...
		{
			i32 nInsertedPos = LensFlareUtil::FindOpticsIndexUnderParentOptics(pSelectedOptics, pParentOptics);
...
void CLensFlareElementTree::GatherItemsRecusively(HTREEITEM hItem, CLensFlareElement::LensFlareElementList& outList)
{
	...
	i32 nIndex = (i32)GetItemData(hItem);
...
void CLensFlareElementTree::DeleteOnesInElementList(CLensFlareElement::LensFlareElementList& elementList)
{
	...
	{
		bool bErased(false);
		for (i32 i = 0, elementListSize(elementList.size()); i < elementListSize; ++i)
		{
			if (elementList[i] == *ii)
			{
...
HTREEITEM CLensFlareElementTree::GetTreeItemByOpticsElement(const IOpticsElementBasePtr pOptics) const
{
	...
	while (hItem)
	{
		CLensFlareElement::LensFlareElementPtr pElement = GetElement((i32)GetItemData(hItem));
		if (pElement)
		{
...
void CLensFlareElementTree::CallChangeListeners()
{
	...
	for (i32 i = 0, iSize(m_LensFlaresElementListeners.size()); i < iSize; ++i)
