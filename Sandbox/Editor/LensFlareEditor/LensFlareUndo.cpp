...
void RestoreLensFlareItem(CLensFlareItem* pLensFlareItem, IOpticsElementBasePtr pOptics, tukk flarePathName)
{
...
void ActivateLensFlareItem(CLensFlareItem* pLensFlareItem, bool bRestoreSelectInfo, tukk selectedFlareItemName)
{
	CLensFlareEditor* pEditor = CLensFlareEditor::GetLensFlareEditor();
	if (pEditor == NULL)
		return;
...
CUndoLensFlareItem::CUndoLensFlareItem(CLensFlareItem* pLensFlareItem, tukk undoDescription)
{
	if (pLensFlareItem)
	{
...
CUndoRenameLensFlareItem::CUndoRenameLensFlareItem(tukk oldFullName, tukk newFullName, bool bRefreshItemTreeWhenUndo, bool bRefreshItemTreeWhenRedo)
{
...
void CUndoRenameLensFlareItem::Rename(const SUndoDataStruct& data, bool bRefreshItemTree)
{
	...
	i32 nDotPos = data.m_oldFullItemName.Find(".");
	if (nDotPos == -1)
		return;
...
CUndoLensFlareElementSelection::CUndoLensFlareElementSelection(CLensFlareItem* pLensFlareItem, tukk flareTreeItemFullName, tukk undoDescription)
{
	if (pLensFlareItem)
	{
...
CUndoLensFlareItemSelectionChange::CUndoLensFlareItemSelectionChange(tukk fullLensFlareItemName, tukk undoDescription)
{
