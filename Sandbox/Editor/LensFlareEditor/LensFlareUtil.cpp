...
#include "Objects/EntityObject.h"
#include <../../DrxPlugins/DrxDefaultEntities/Module/DefaultComponents/Lights/ILightComponent.h>
...
bool CreateXmlData(IOpticsElementBasePtr pOptics, XmlNodeRef& pOutNode)
{
	...
	for (i32 i = 0, iElementCount(pOptics->GetElementCount()); i < iElementCount; ++i)
	{
...
void SetVariablesTemplateFromOptics(IOpticsElementBasePtr pOptics, CVarBlockPtr& pRootVar, std::vector<IVariable::OnSetCallback>& funcs)
{
	...
	for (i32 i = 0, iNumVariables(pRootVar->GetNumVariables()); i < iNumVariables; ++i)
	{
		IVariable* pVariable = pRootVar->GetVariable(i);
		if (pVariable == NULL)
			continue;
		for (i32 k = 0, iNumVariables2(pVariable->GetNumVariables()); k < iNumVariables2; ++k)
		{
			IVariable* pChildVariable(pVariable->GetVariable(k));
			if (pChildVariable == NULL)
				continue;
			for (i32 i = 0, iSize(funcs.size()); i < iSize; ++i)
...
extern void SetVariablesTemplateFromOptics(IOpticsElementBasePtr pOptics, CVarBlockPtr& pRootVar)
{
	...
	for (i32 i = 0; i < paramGroups.size(); ++i)
	{
		...
		if (displayGroupName == "Common")
		{
			...
			i32 nTypeIndex = (i32)pOptics->GetType();
		...
		for (i32 k = 0; k < pGroup->GetVariableCount(); ++k)
		{
			...
			if (pFuncVar->paramType == e_FLOAT)
			{
				...
				floatVar->SetUserData((uk )(intptr_t)MakeFuncKey(i, k));
			}
			else if (pFuncVar->paramType == e_INT)
			{
				...
				intVar->SetUserData((uk )(intptr_t)MakeFuncKey(i, k));
			}
			else if (pFuncVar->paramType == e_BOOL)
			{
				...
				boolVar->SetUserData((uk )(intptr_t)MakeFuncKey(i, k));
			}
			else if (pFuncVar->paramType == e_VEC2)
			{
				...
				vec2Var->SetUserData((uk )(intptr_t)MakeFuncKey(i, k));
			}
			else if (pFuncVar->paramType == e_VEC3)
			{
				...
				vec3Var->SetUserData((uk )(intptr_t)MakeFuncKey(i, k));
			}
			else if (pFuncVar->paramType == e_VEC4)
			{
				...
				vec4Var->SetUserData((uk )(intptr_t)MakeFuncKey(i, k));
			}
			else if (pFuncVar->paramType == e_COLOR)
			{
				...
				colorVar->SetUserData((uk )(intptr_t)MakeFuncKey(i, k));

				CSmartVariable<i32> alphaVar;
				...
				alphaVar->SetUserData((uk )(intptr_t)MakeFuncKey(i, k));
			}
			...
			else if (pFuncVar->paramType == e_TEXTURE2D || pFuncVar->paramType == e_TEXTURE3D || pFuncVar->paramType == e_TEXTURE_CUBE)
			{
				...
				textureVar->SetUserData((uk )(intptr_t)MakeFuncKey(i, k));
			}
		}
	}
}
...
void AddOptics(IOpticsElementBasePtr pParentOptics, XmlNodeRef& xmlNode)
{
	...
	for (i32 i = 0, iChildCount(xmlNode->getChildCount()); i < iChildCount; ++i)
...
void CopyVariable(IFuncVariable* pSrcVar, IFuncVariable* pDestVar)
{
	...
	if (pSrcVar->paramType == e_FLOAT)
	{
		...
		pDestVar->InvokeSetter((uk )&value);
	}
...
void CopyOpticsAboutSameOpticsType(IOpticsElementBasePtr pSrcOptics, IOpticsElementBasePtr pDestOptics)
{
	...
	for (i32 i = 0, iVarGroupSize(srcVarGroups.size()); i < iVarGroupSize; ++i)
	{
		...
		for (i32 k = 0, iVarSize(srcVarGroup.GetVariableCount()); k < iVarSize; ++k)
		{
...
bool FindGroupByName(DynArray<FuncVariableGroup>& groupList, const string& name, i32& outIndex)
{
	for (i32 i = 0, iVarGroupSize(groupList.size()); i < iVarGroupSize; ++i)
	{
...
void CopyOpticsAboutDifferentOpticsType(IOpticsElementBasePtr pSrcOptics, IOpticsElementBasePtr pDestOptics)
{
	...
	for (i32 i = 0, iVarGroupSize(destVarGroups.size()); i < iVarGroupSize; ++i)
	{
		FuncVariableGroup destVarGroup = destVarGroups[i];
		i32 nIndex = 0;
		...
		for (i32 k = 0, iVarSize(destVarGroup.GetVariableCount()); k < iVarSize; ++k)
		{
			IFuncVariable* pDestVar = destVarGroup.GetVariable(k);
...
void CopyOptics(IOpticsElementBasePtr pSrcOptics, IOpticsElementBasePtr pDestOptics, bool bReculsiveCopy)
{
	...
	if (bReculsiveCopy)
	{
		...
		for (i32 i = 0, iChildCount(pSrcOptics->GetElementCount()); i < iChildCount; ++i)
		{
...
void GetSelectedLightEntities(std::vector<CEntityObject*>& outLightEntities)
{
	...
	if (pSelectionGroup == NULL)
		return;
	i32 nSelectionCount = pSelectionGroup->GetCount();
...
void ApplyOpticsToSelectedEntityWithComponent(const string& opticsFullName, IOpticsElementBasePtr pOptics)
{
	...
	i32k selectionCount = pSelectionGroup->GetCount();
	for (i32 i = 0; i < selectionCount; ++i)
	{
		...
		if (const IEntity* pEntity = pSelectedObj->GetIEntity())
		{
			...
			IEntityComponent* pLightComponent = pEntity->GetComponentByTypeId("0A86908D-642F-4590-ACEF-484E8E39F31B"_drx_guid);
			pLightComponent = pLightComponent == nullptr ? pEntity->GetComponentByTypeId("07D0CAD1-8E79-4177-9ADD-A2464A009FA5"_drx_guid) : pLightComponent;

			if (pLightComponent)
			{
				static_cast<Drx::DefaultComponents::ILightComponent*>(pLightComponent)->SetOptics(opticsFullName);
			}
		}
	}
...
IOpticsElementBasePtr FindOptics(IOpticsElementBasePtr pStartOptics, const string& name)
{
	...
	for (i32 i = 0, iElementCount(pStartOptics->GetElementCount()); i < iElementCount; ++i)
	{
...
void RemoveOptics(IOpticsElementBasePtr pOptics)
{
	...
	for (i32 i = 0, iElementCount(pParent->GetElementCount()); i < iElementCount; ++i)
	{
...
void ChangeOpticsRootName(IOpticsElementBasePtr pOptics, const string& newRootName)
{
	...
	for (i32 i = 0, iElementSize(pOptics->GetElementCount()); i < iElementSize; ++i)
...
XmlNodeRef CreateXMLFromClipboardData(const string& type, const string& groupName, bool bPasteAtSameLevel, std::vector<SClipboardData>& dataList)
{
	...
	for (i32 i = 0, iDataSize(dataList.size()); i < iDataSize; ++i)
	{
...
IFuncVariable* GetFuncVariable(IOpticsElementBasePtr pOptics, i32 nFuncKey)
{
	if (pOptics == NULL)
		return NULL;

	i32 nGroupIndex = (nFuncKey & 0xFFFF0000) >> 16;
	i32 nVarIndex = (nFuncKey & 0x0000FFFF);
...
void GetLightEntityObjects(std::vector<CEntityObject*>& outEntityLights)
{
	...
	for (i32 i = 0, iObjectSize(pEntityObjects.size()); i < iObjectSize; ++i)
	{
...
void OutputOpticsDebug(IOpticsElementBasePtr pOptics)
{
	...
	for (i32 i = 0, iGroupCount(groupArray.size()); i < iGroupCount; ++i)
	{
		...
		for (i32 k = 0, iParamCount(pGroup->GetVariableCount()); k < iParamCount; ++k)
		{
	...
	for (i32 i = 0, iChildCount(pOptics->GetElementCount()); i < iChildCount; ++i)
	{
...
void FillParams(XmlNodeRef& paramNode, IOpticsElementBase* pOptics)
{
	if (pOptics == NULL)
		return;
	DynArray<FuncVariableGroup> groupArray(pOptics->GetEditorParamGroups());
	for (i32 i = 0, iGroupCount(paramNode->getChildCount()); i < iGroupCount; ++i)
	{
		...
		for (i32 k = 0, iParamCount(pGroup->GetVariableCount()); k < iParamCount; ++k)
		{
			...
			else if (pVar->paramType == e_VEC4)
			{
				...
				if (groupNode->getAttr(pVar->name.c_str(), &strVec4Value))
				{
					...
					pVar->InvokeSetter((uk )&vec4Value);
				}
			}
			else if (pVar->paramType == e_COLOR)
			{
...
void BeginDragDrop(CTreeCtrl& treeCtrl, const CPoint& startPos, XmlNodeRef xmlData)
{
	...
	if (LensFlareUtil::CreateDragImage(treeCtrl, dragInfo.m_hDraggedItem, dragInfo.m_Image))
	{
		...
		if (dragInfo.m_Image.GetImageInfo(0, &imageInfo))
		{
			...
			i32 half_width = (imageInfo.rcImage.right - imageInfo.rcImage.left) / 2;
			i32 half_height = (imageInfo.rcImage.bottom - imageInfo.rcImage.top) / 2;
...
i32 FindOpticsIndexUnderParentOptics(IOpticsElementBasePtr pOptics, IOpticsElementBasePtr pParentOptics)
{
	for (i32 i = 0, iElementSize(pParentOptics->GetElementCount()); i < iElementSize; ++i)
	{
