...
tukk CLensFlareEditor::s_pLensFlareEditorClassName = "Lens Flare Editor";
...
static tukk LENS_FLARES_CLASSNAME("LENS_FLARE_EDITOR");
...
class CLensFlareEditorClass : public IViewPaneClass
{
	...
	virtual tukk    ClassName()       override { return CLensFlareEditor::s_pLensFlareEditorClassName; };
	virtual tukk    Category()        override { return "Database"; };
	...
	virtual tukk    GetPaneTitle()    override { return CLensFlareEditor::s_pLensFlareEditorClassName; };
...
{
	for (i32 i = 0, iItemSize(m_AllLensFlareTreeItems.size()); i < iItemSize; ++i)
	{
...
{
	for (i32 i = 0, iSize(m_AllLensFlareTreeItems.size()); i < iSize; ++i)
	{
...
XmlNodeRef CLensFlareEditor::CreateXML(tukk type) const
{
...
void CLensFlareEditor::UpdateClipboard(tukk type) const
{
	std::vector<LensFlareUtil::SClipboardData> clipboardDataList;
	string groupName;
	if (!GetClipboardDataList(clipboardDataList, groupName))
...
void CLensFlareEditor::OnAssignFlareToLightEntities()
{
	...
	for (i32 i = 0, iEntityCount(entityList.size()); i < iEntityCount; ++i)
...
void CLensFlareEditor::OnSelectAssignedObjects()
{
	...
	for (i32 i = 0, iAssignedObjectsSize(assignedObjects.size()); i < iAssignedObjectsSize; ++i)
...
void CLensFlareEditor::OnGetFlareFromSelection()
{
	...
	i32 nDotPosition = fullFlareName.Find(".");
	if (nDotPosition == -1)
		return;
...
void CLensFlareEditor::ChangeGroupName(HTREEITEM hItem, const string& newGroupName)
{
	...
	i32 nPos = fullChangedGroupName.ReverseFind('.');
	if (nPos == -1)
	...
	for (i32 i = 0, iItemCount(pLensFlareLibrary->GetItemCount()); i < iItemCount; ++i)
	{
	...
	for (i32 i = 0, iItemCount(ItemsWillBeChanged.size()); i < iItemCount; ++i)
	{
...
void CLensFlareEditor::OnRemoveItem()
{
	...
	if (pSelectedLensFlareItem == NULL)
	{
		...
		{
			string deleteMsgStr;
			deleteMsgStr.Format(_T("Delete %s?"), (tukk)fullLensFlareItemName.GetBuffer());
			if (MessageBox(deleteMsgStr, _T("Delete Confirmation"), MB_YESNO | MB_ICONQUESTION) == IDYES)
			{
				...
				if (pLensFlareLibrary)
				{
					...
					for (i32 i = 0, iDeleteItemSize(deletedItems.size()); i < iDeleteItemSize; ++i)
					{
	...
	{
		string deleteMsgStr;
		deleteMsgStr.Format(_T("Delete %s?"), (tukk)pSelectedLensFlareItem->GetName());
...
void CLensFlareEditor::OnAddLibrary()
{
	...
	dlg.SetCheckCallback([this](const string& library) -> bool
	{
		...
		{
		  DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, string("Library '") + library + "' already exists.");
		  return false;
		}
		return true;
	});
...
void CLensFlareEditor::OnSize(UINT nType, i32 cx, i32 cy)
{
...
void CLensFlareEditor::OnReloadLib()
{
	...
	if (pLibrary == NULL)
		return;
	for (i32 i = 0, iItemCount(pLibrary->GetItemCount()); i < iItemCount; ++i)
	{
...
{
	for (i32 i = 0, iSize(m_LensFlareChangeItemListenerList.size()); i < iSize; ++i)
	{
