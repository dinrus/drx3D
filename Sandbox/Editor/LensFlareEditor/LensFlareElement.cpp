...
void CLensFlareElement::OnInternalVariableChange(IVariable* pVar)
{
	...
	IFuncVariable* pFuncVar = LensFlareUtil::GetFuncVariable(pOptics, (i32)(intptr_t)pVar->GetUserData());
	if (pFuncVar == NULL)
		return;
	...
	case IVariable::INT:
		{
			...
			if (pFuncVar->paramType == e_COLOR)
			{
				...
				pFuncVar->InvokeSetter((uk )&color);
			}
	...
	case IVariable::STRING:
		{
			...
			if (pFuncVar->paramType == e_TEXTURE2D || pFuncVar->paramType == e_TEXTURE3D || pFuncVar->paramType == e_TEXTURE_CUBE)
			{
				...
				pFuncVar->InvokeSetter((uk )pTexture);
				if (pTexture)
				{
					pTexture->Release();
				}
