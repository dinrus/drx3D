...
//  Copyright (C), DinrusPro 3D Studios, 2012.
// -------------------------------------------------------------------------
...
inline void AddVariable(CVariableBase& varArray, CVariableBase& var, tukk varName, tukk humanVarName, tukk description, char dataType = IVariable::DT_SIMPLE)
{
	if (varName)
		var.SetName(varName);
	if (humanVarName)
...
inline void AddVariable(CVarBlock* vars, CVariableBase& var, tukk varName, tukk humanVarName, tukk description, u8 dataType = IVariable::DT_SIMPLE)
{
	if (varName)
		var.SetName(varName);
	if (humanVarName)
...
{
	i32 nLastDotPos = fullItemName.ReverseFind('.');
	if (nLastDotPos == -1)
		return fullItemName;

	string name = GetGroupNameFromName(fullItemName);

	i32 nFirstDotPos = name.Find('.');
	if (nFirstDotPos == -1)
		return name;
...
inline i32 MakeFuncKey(i32 nGroupIndex, i32 nVarIndex)
{
...
inline bool FindGroup(tukk groupName, IOpticsElementBase* pOptics, i32& nOutGroupIndex)
{
	if (groupName == NULL)
		return NULL;

	DynArray<FuncVariableGroup> groupArray = pOptics->GetEditorParamGroups();
	for (i32 i = 0, iCount(groupArray.size()); i < iCount; ++i)
	{
