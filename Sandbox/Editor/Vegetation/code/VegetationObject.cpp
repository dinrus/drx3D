...
CVegetationObject::CVegetationObject(i32 id)
{
	m_id = id;
...
void CVegetationObject::Serialize(XmlNodeRef& node, bool bLoading)
{
	if (bLoading)
	{
		...
		fileName = PathUtil::ToUnixPath((tukk)fileName).c_str();
	...
	else
	{
		...
		if (m_pVarObject != nullptr)
		{
			for (i32 i = 0; i < m_pVarObject->GetNumVariables(); ++i)
			{
		...
		if (!m_terrainLayers.empty())
		{
			...
			for (i32 i = 0; i < m_terrainLayers.size(); i++)
			{
...
void CVegetationObject::UpdateTerrainLayerVariables()
{
	...
	for (i32 i = 0; i < surfaceTypeCount; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CVegetationObject::GetTextureMemUsage(IDrxSizer* pSizer)
{
	i32 nSize = 0;
...
//////////////////////////////////////////////////////////////////////////
void CVegetationObject::SetNumInstances(i32 numInstances)
{
...
boost::python::list PyGetVegetation(const string& vegetationName = "", bool loadedOnly = false)
{
	...
	if (vegetationName.IsEmpty())
	{
		...
		if (pSel->GetCount() == 0)
		{
			for (i32 i = 0; i < pVegMap->GetObjectCount(); i++)
			{
