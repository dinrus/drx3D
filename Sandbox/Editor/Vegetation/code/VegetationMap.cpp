...
namespace
{
tukk kVegetationMapFile = "VegetationMap.dat";
tukk kVegetationMapFileOld = "VegetationMap.xml";   //TODO: Remove support after January 2012 (support old extension at least for convertion levels time)
i32k kMaxMapSize = 1024;
i32k kMinMaskValue = 32;
};
...
protected:
	virtual tukk GetDescription() { return "Vegetation Modify"; };
...
protected:
	virtual tukk GetDescription() { return "Vegetation Create"; };
...
protected:
	virtual tukk GetDescription() { return "Vegetation Object Create"; }
...
float CVegetationMap::GenerateRotation(CVegetationObject* pObject, const Vec3& vPos)
{
	...
	i32k nRotationRange = pObject->GetRotationRangeToTerrainNormal();
...
//////////////////////////////////////////////////////////////////////////
void CVegetationMap::Allocate(i32 nMapSize, bool bKeepData)
{
	CXmlArchive ar("Temp");
	if (bKeepData)
	{
	...
	i32 sz = sizeof(SectorInfo) * m_numSectors * m_numSectors;
...
//////////////////////////////////////////////////////////////////////////
i32 CVegetationMap::GetSize() const
{
...
//////////////////////////////////////////////////////////////////////////
inline CVegetationMap::SectorInfo* CVegetationMap::GetVegSector(i32 x, i32 y)
{
...
{
	i32 x = i32(worldPos.x * m_worldToSector);
	i32 y = i32(worldPos.y * m_worldToSector);
...
//////////////////////////////////////////////////////////////////////////
void CVegetationMap::RemoveDuplVegetation(i32 x1, i32 y1, i32 x2, i32 y2)
{
	if (x1 == -1)
	{
		x1 = 0;
...
CVegetationInstance* CVegetationMap::GetNearestInstance(const Vec3& pos, float radius)
{
	...
	for (i32 y = sy1; y <= sy2; y++)
	{
		for (i32 x = sx1; x <= sx2; x++)
		{
...
void CVegetationMap::GetObjectInstances(float x1, float y1, float x2, float y2, std::vector<CVegetationInstance*>& instances)
{
	...
	// check all sectors intersected by radius.
	i32 sx1 = i32(x1 * m_worldToSector);
	sx1 = max(sx1, 0);
	i32 sx2 = i32(x2 * m_worldToSector);
	sx2 = min(sx2, m_numSectors - 1);
	i32 sy1 = i32(y1 * m_worldToSector);
	sy1 = max(sy1, 0);
	i32 sy2 = i32(y2 * m_worldToSector);
	sy2 = min(sy2, m_numSectors - 1);
	for (i32 y = sy1; y <= sy2; y++)
	{
		for (i32 x = sx1; x <= sx2; x++)
...
void CVegetationMap::PaintBrush(CRect& rc, bool bCircle, CVegetationObject* object, Vec3* pPos)
{
	...
	i32 mapSize = m_numSectors * m_sectorSize;
	...
	i32 area = rc.Width() * rc.Height();
	i32 count = area / (density * density);
	...
	for (i32 i = 0; i < count; i++)
	{
		...
		Vec3 p(x1 + drx_random(0.0f, 2.0f) * width2, y1 + drx_random(0.0f, 2.0f) * height2, 0);
		...
		// swap x/y
		i32 hy = i32(p.x / unitSize);
		i32 hx = i32(p.y / unitSize);
...
float CVegetationMap::CalcHeightOnBrushes(const Vec3& p, const Vec3& posUpper, bool& bHitBrush)
{
	...
	i32 col = 0;
	Vec3 vDir(0, 0, -1);
	i32 flags = rwi_stop_at_pierceable | rwi_ignore_terrain_holes;
	for (i32 chcnt = 0; chcnt < 3; chcnt++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CVegetationMap::PaintBrightness(float x, float y, float w, float h, u8 brightness, u8 brightness_shadowmap)
{
	// Find sector range from world positions.
	i32 startSectorX = i32(x * m_worldToSector);
	i32 startSectorY = i32(y * m_worldToSector);
	i32 endSectorX = i32((x + w) * m_worldToSector);
	i32 endSectorY = i32((y + h) * m_worldToSector);
	...
	for (i32 sy = startSectorY; sy <= endSectorY; sy++)
	{
		for (i32 sx = startSectorX; sx <= endSectorX; sx++)
		{
...
void CVegetationMap::ClearMask(const string& maskFile)
{
	...
	{
		string str;
		str.Format("Error loading mask file %s", (tukk)maskFile);
		AfxGetMainWnd()->MessageBox(str, "Warning", MB_OK | MB_ICONEXCLAMATION);
		return;
	}
	i32 layerSize = m_numSectors;
...
{
	i32 id(GenerateVegetationObjectId());
...
//////////////////////////////////////////////////////////////////////////
CVegetationObject* CVegetationMap::GetObjectById(i32 id) const
{
...
std::vector<CVegetationObject*> CVegetationMap::GetSelectedObjects() const
{
	...
	for (i32 i = 0; i < GetObjectCount(); ++i)
	{
...
void CVegetationMap::Serialize(CXmlArchive& xmlAr)
{
	if (xmlAr.bLoading)
	{
		...
		if (mainNode && !stricmp(mainNode->getTag(), "VegetationMap"))
		{
			...
			if (objects)
			{
				i32 numObjects = objects->getChildCount();
...
void CVegetationMap::SerializeInstances(CXmlArchive& xmlAr, CRect* saveRect)
{
	if (xmlAr.bLoading)
	{
		...
		if (m_nVersion == 1)
		{
			...
			i32 numInst = arraySize / sizeof(SVegInst_V1_0);
			for (i32 i = 0; i < numInst; i++)
			{
		...
		else if (m_nVersion == 2)
		{
			...
			i32 numInst = arraySize / sizeof(SVegInst_V2_0);
			for (i32 i = 0; i < numInst; i++)
			{
				if (pVegInst[i].objectIndex >= numObjects)
				{
		...
		else
		{
			...
			i32 numInst = arraySize / sizeof(SVegInst);
			for (i32 i = 0; i < numInst; i++)
			{
				if (pVegInst[i].objectIndex < numObjects)
				{
	...
	else
	{
		...
		if (m_numInstances == 0)
			return;
		i32 arraySize = sizeof(SVegInst) * m_numInstances;
...
void CVegetationMap::ImportSegment(CMemoryBlock& mem, const Vec3& vOfs)
{
	...
	typedef std::map<i32, DrxGUID>  TIntGuidMap;
	typedef std::pair<i32, DrxGUID> TIntGuidPair;

	typedef std::map<i32, i32>   TIntIntMap;
	typedef std::pair<i32, i32>  TIntIntPair;

	bool bIsVegetationObjectInfoSaved = false;
	u8* p = (u8*)mem.GetBuffer();
	i32 curr = 0;

	if (mem.GetSize() > sizeof(i32))
	{
		i32 bckwCompatabiliyFlag = 0;// = 0xFFFFFFFF;
		// flag for backward compatibility
		memcpy(&bckwCompatabiliyFlag, &p[curr], sizeof(i32));
		curr += sizeof(i32);
	...
	if (bIsVegetationObjectInfoSaved)
	{
		...
		i32 numMapObjects = 0;
		// num objects
		memcpy(&numMapObjects, &p[curr], sizeof(i32));
		curr += sizeof(i32);

		for (i32 i = 0; i < numMapObjects; i++)
		{
			TIntGuidPair tPair;
			//object index
			memcpy(&tPair.first, &p[curr], sizeof(i32));
			curr += sizeof(i32);
		...
		// instances
		memcpy(&iInstances, &p[curr], sizeof(i32)); // num of instances
		curr += sizeof(i32);
	}

	SVegInst* array = bIsVegetationObjectInfoSaved ? (SVegInst*) &p[curr] : (SVegInst*) mem.GetBuffer();
	i32 numInst = bIsVegetationObjectInfoSaved ? iInstances : mem.GetSize() / sizeof(SVegInst);
	i32 numObjects = m_objects.size();
	for (i32 i = 0; i < numInst; i++)
	{
		i32 objectIndex = array[i].objectIndex;
...
void CVegetationMap::ExportSegment(CMemoryBlock& mem, const AABB& bb, const Vec3& vOfs)
{
	...
	i32 sizeOfMem = 0;
	sizeOfMem += sizeof(i32); // flag for backward compatibility
	sizeOfMem += sizeof(i32); // num of objects

	TIntGuidMap::iterator it;

	for (it = VegetationIndexGuidMap.begin(); it != VegetationIndexGuidMap.end(); ++it)
	{
		sizeOfMem += sizeof(i32);  //object index
		sizeOfMem += sizeof(GUID); // size of GUID;
	}

	sizeOfMem += sizeof(i32);                   // num of instances
	sizeOfMem += sizeof(SVegInst) * iInstances; // instances

	i32 numElems = VegetationIndexGuidMap.size();

	CMemoryBlock tmp;
	tmp.Allocate(sizeOfMem);
	u8* p = (u8*)tmp.GetBuffer();
	i32 curr = 0;

	i32 bckwCompatabiliyFlag = 0xFFFFFFFF;
	memcpy(&p[curr], &bckwCompatabiliyFlag, sizeof(i32)); // flag for backward compatibility
	curr += sizeof(i32);

	memcpy(&p[curr], &numElems, sizeof(i32)); // num of objects
	curr += sizeof(i32);

	for (it = VegetationIndexGuidMap.begin(); it != VegetationIndexGuidMap.end(); ++it)
	{
		memcpy(&p[curr], &it->first, sizeof(i32));
		curr += sizeof(i32); //object index
	...
	// save instances
	memcpy(&p[curr], &iInstances, sizeof(i32)); // num of instances
	curr += sizeof(i32);
...
void CVegetationMap::LoadOldStuff(CXmlArchive& xmlAr)
{
	...
	if (xmlAr.pNamedData->GetDataBlock("StatObjectsArray", pData, nSize))
	{
		...
		Vec3 pos;
		u8* pMap = (u8*)pData;
		for (i32 y = 0; y < 2048; ++y)
		{
			for (i32 x = 0; x < 2048; ++x)
			{
				i32 i = x + 2048 * y;
				if (!pMap[i])
					continue;

				u32 objIndex = pMap[i] - 1;
...
void CVegetationMap::GenerateShadowMap(CByteImage& shadowmap, float shadowAmmount, const Vec3& sunVector)
{
	...
	i32 width = shadowmap.GetWidth();
	i32 height = shadowmap.GetHeight();

	//@FIXME: Hardcoded.
	i32 sectorSizeInMeters = 64;

	float unitSize = pHeightmap->GetUnitSize();
	i32 numSectors = (pHeightmap->GetWidth() * unitSize) / sectorSizeInMeters;

	i32 sectorSize = shadowmap.GetWidth() / numSectors;
	i32 sectorSize2 = sectorSize * 2;
	assert(sectorSize > 0);

	u32 shadowValue = shadowAmmount;

	bool bProgress = width >= 2048;
	CWaitProgress wait("Calculating Objects Shadows", bProgress);

	u8* sectorImage = (u8*)malloc(sectorSize * sectorSize * 3);
	u8* sectorImage2 = (u8*)malloc(sectorSize2 * sectorSize2 * 3);

	for (i32 y = 0; y < numSectors; y++)
	{
		if (bProgress)
		{
		...
		for (i32 x = 0; x < numSectors; x++)
		{
			...
			// Scale sector image down and store into the shadow map.
			{
				...
				i32 x1 = x * sectorSize;
				i32 y1 = y * sectorSize;
				for (i32 j = 0; j < sectorSize; j++)
				{
					i32 sx1 = x1 + (sectorSize - j - 1);
					for (i32 i = 0; i < sectorSize; i++)
					{
...
//////////////////////////////////////////////////////////////////////////
i32 CVegetationMap::ExportObject(CVegetationObject* object, XmlNodeRef& node, CRect* saveRect)
{
	i32 numSaved = 0;
	assert(object != 0);
			...
			for (CVegetationInstance* obj = m_sectors[i].first; obj; obj = obj->next)
			{
				if (obj->object == object)
				{
					...
					if (obj->brightness != 255)
						inst->setAttr("Brightness", (i32)obj->brightness);
					if (obj->angle != 0)
						inst->setAttr("Angle", (i32)obj->angle);
				}
			}
		}
	}
...
void CVegetationMap::ImportObject(XmlNodeRef& node, const Vec3& offset)
{
		...
		if (pOldObject->GetGUID() == object->GetGUID())
		{
			...
			/*
			   ...
			   msg.Format( _T("Vegetation Object %s %s already exists in the level!.\r\nOverride existing object?"),
			              GuidUtil::ToString(pOldObject->GetGUID()),(tukk)pOldObject->GetFileName() );
	...
	if (instancesNode)
	{
		i32 numChilds = instancesNode->getChildCount();
...
void CVegetationMap::ImportObjectsFromXml(const string& filename)
{
	...
	for (i32 i = 0; i < pXmlRoot->getChildCount(); ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
void CVegetationMap::DrawToTexture(u32* texture, i32 texWidth, i32 texHeight, i32 srcX, i32 srcY)
{
	assert(texture != 0);

	for (i32 y = 0; y < texHeight; y++)
	{
		i32 trgYOfs = y * texWidth;
...
//////////////////////////////////////////////////////////////////////////
i32 CVegetationMap::WorldToSector(float worldCoord) const
{
	return i32(worldCoord * m_worldToSector);
}
...
void CVegetationMap::ExportBlock(const CRect& subRc, CXmlArchive& ar)
{
	...
	for (i32 i = 0; i < GetObjectCount(); i++)
	{
		...
		i32 numSaved = ExportObject(pObject, vegObjNode, &rect);
		if (numSaved > 0)
		{
...
void CVegetationMap::ImportBlock(CXmlArchive& ar, CPoint placeOffset)
{
	...
	for (i32 i = 0; i < mainNode->getChildCount(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CVegetationMap::RepositionArea(const AABB& box, const Vec3& offset, i32 nRot, bool isCopy)
{
	...
	i32 px1 = i32((x1 + offset.x) * m_worldToSector);
	i32 px2 = i32((x2 + offset.x) * m_worldToSector);
	i32 py1 = i32((y1 + offset.y) * m_worldToSector);
	i32 py2 = i32((y2 + offset.y) * m_worldToSector);

	if (nRot == 1 || nRot == 3)
	{
		px1 = i32((dst.x - (box.max.y - box.min.y)) * m_worldToSector);
		px2 = i32((dst.x + (box.max.y - box.min.y)) * m_worldToSector);
		py1 = i32((dst.y - (box.max.x - box.min.x)) * m_worldToSector);
		py2 = i32((dst.y + (box.max.x - box.min.x)) * m_worldToSector);
	}
...
//////////////////////////////////////////////////////////////////////////
i32 CVegetationMap::GetTexureMemoryUsage(bool bOnlySelectedObjects)
{
	...
	i32 nSize = pSizer->GetTotalSize();
	pSizer->Release();
	return nSize;
}

//////////////////////////////////////////////////////////////////////////
i32 CVegetationMap::GetSpritesMemoryUsage(bool bOnlySelectedObjects)
{
	i32 nSize = 0;
...
//////////////////////////////////////////////////////////////////////////
void CVegetationMap::GetMemoryUsage(IDrxSizer* pSizer)
{
	pSizer->Add(*this);
...
//////////////////////////////////////////////////////////////////////////
i32 CVegetationMap::GenerateVegetationObjectId()
{
	// Generate New id.
...
void CVegetationMap::GenerateBillboards(IConsoleCmdArgs*)
{
	...
	for (i32 i = 0; i < vegMap->GetObjectCount(); i++)
	{
	...
	u32 nRenderingFlags = SHDF_ALLOWHDR | SHDF_ALLOWPOSTPROCESS;
	...
	i32 nSpriteResFinal = 128;
	i32 nSpriteResInt = nSpriteResFinal << 1;
	i32 nLine = (i32)sqrtf(FAR_TEX_COUNT);
	i32 nAtlasRes = nLine * nSpriteResFinal;
	...
	i32 nProducedTexturesCounter = 0;
...
bool CVegetationMap::SaveBillboardTIFF(const CString& fileName, ITexture *pTexture, tukk szPreset, bool bConvertToSRGB)
{
	i32 nWidth = pTexture->GetWidth();
	i32 nHeight = pTexture->GetHeight();
	i32 nPitch = nWidth * 4;

	u8 *pSrcData = new u8[nPitch * nHeight];
	pTexture->GetData32(0, 0, pSrcData, eTF_B8G8R8A8);

	if (bConvertToSRGB)
	{
		byte *pSrc = pSrcData;
		for (i32 i = 0; i < nWidth * nHeight; i++)
		{
			byte r = pSrc[0];
			byte g = pSrc[1];
			byte b = pSrc[2];
...
bool CVegetationMap::IsAreaEmpty(const AABB& bbox)
{
	...
	for (i32 vegNo = 0; vegNo < foundVegetationIstances.size(); ++vegNo)
	{
