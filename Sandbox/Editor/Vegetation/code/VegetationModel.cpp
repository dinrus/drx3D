...
CVegetationModel::CVegetationModelItem::CVegetationModelItem(const QString& name, i32 row)
	: name(name)
	, row(row)
...
CVegetationModel::CVegetationModelItem::CVegetationModelItem(i32 r, CVegetationObject* pVegetationObject, CVegetationModelItem* pParent)
...
i32 CVegetationModel::CVegetationModelItem::rowCount() const
{
...
QModelIndex CVegetationModel::index(i32 row, i32 column, const QModelIndex& parent) const
{
...
i32 CVegetationModel::rowCount(const QModelIndex& parent) const
{
	if (!parent.isValid())
	{
...
i32 CVegetationModel::columnCount(const QModelIndex& parent) const
{
	return static_cast<i32>(Column::Count);
}

QVariant CVegetationModel::data(const QModelIndex& index, i32 role) const
{
	if (!index.isValid())
	{
		return QVariant();
...
QVariant CVegetationModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
...
bool CVegetationModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
	if (!index.isValid())
	{
		return false;
...
bool CVegetationModel::canDropMimeData(const QMimeData* pData, Qt::DropAction action, i32 row, i32 column, const QModelIndex& parent) const
{
...
bool CVegetationModel::dropMimeData(const QMimeData* pData, Qt::DropAction action, i32 row, i32, const QModelIndex& parent)
{
	if (action != Qt::MoveAction)
	{
		return true;
...
{
	QSet<i32> encodedObjectIds;
	QSet<i32> encodedGroupIndexes;
...
{
	for (i32 i = 0; i < m_pVegetationMap->GetObjectCount(); ++i)
	{
...
void CVegetationModel::AddGroup()
{
	...
	// in the range of u32 are used the pattern is extended with an
...
QModelIndexList CVegetationModel::FindVegetationObjects(const QVector<CVegetationInstance*> selectedInstances)
{
	...
	i32k groupCount = rowCount();

	for (i32 group = 0; group < groupCount; ++group)
	{
		i32k objectCount = rowCount(index(group, 0));
		QModelIndex groupIdx = index(group, 0);

		for (i32 item = 0; item < objectCount; ++item)
		{
...
void CVegetationModel::RemoveVegetationObjectsFromParent(CVegetationModelItem* pParentGroup, const QSet<CVegetationModelItem*>& removeObjects)
{
	...
	for (i32 i = groupObjects.size() - 1; i >= 0; --i)
	{
...
{
	for (i32 i = m_groups.size() - 1; i >= 0; --i)
	{
...
void CVegetationModel::UpdateVegetationObject(CVegetationObject* pVegetationObject)
{
	...
	if (pVegetationObject)
	{
		...
		dataChanged(topLeftIndex, topLeftIndex.sibling(topLeftIndex.row(), static_cast<i32>(Column::Count)));
	}
}
...
QVariant CVegetationModel::GetGroupData(CVegetationModelItem* pGroup, Column column, i32 role) const
{
	if (!pGroup->IsGroup())
	{
		return QVariant();
...
QVariant CVegetationModel::GetVegetationObjectData(CVegetationModelItem* pVegetationObjectItem, Column column, i32 role) const
{
	...
	case Qt::DisplayRole:
		{
			...
			case Column::Textsize:
				{
					i32 textSize = pVegetationObject->GetTextureMemUsage();
...
std::vector<CVegetationModel::VegetationModelItemPtr> CVegetationModel::DeserializeDropGroupData(const CVegetationDragDropData* pDragDropData)
{
	...
	QVector<i32> groupIndexes;
	stream >> groupIndexes;
	std::sort(groupIndexes.begin(), groupIndexes.end(), std::greater<i32>());
...
void CVegetationModel::DropVegetationObjects(std::vector<VegetationModelItemPtr>& movedVegetationObjectItems, i32 row, const QModelIndex& parent)
{
...
void CVegetationModel::DropGroups(std::vector<VegetationModelItemPtr>& movedGroupItems, i32 row, const QModelIndex& parent)
{
...
void CVegetationModel::ValidateChildRows(CVegetationModelItem* pGroup, i32 startIdx) const
{
	ValidateRows(pGroup->children, startIdx);
}

void CVegetationModel::ValidateRows(std::vector<VegetationModelItemPtr>& items, i32 startIdx) const
{
	for (i32 i = startIdx; i < items.size(); ++i)
	{
		items[i]->row = i;
	}
}
...
void CVegetationModel::SetVisibility(CVegetationModelItem* pItem, CVegetationModelItem::EVisibility visibility)
{
	...
	if (pVegetationObject)
	{
		DRX_ASSERT(visibility != CVegetationModelItem::EVisibility::PartiallyVisible);
...
void CVegetationModel::SetVisibilityRecursively(CVegetationModelItem* pItem, CVegetationModelItem::EVisibility visibility)
{
	...
	auto column = static_cast<i32>(Column::VisibleAndObject);
