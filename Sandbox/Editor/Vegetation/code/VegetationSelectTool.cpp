...
class CVegetationSelectTool_ClassDesc : public IClassDesc
{
	...
	virtual tukk ClassName() { return "EditTool.VegetationSelect"; };
...
bool CVegetationSelectTool::MouseCallback(CViewport* pView, EMouseEvent event, CPoint& point, i32 flags)
{
...
void CVegetationSelectTool::OnManipulatorDrag(IDisplayViewport* pView, ITransformManipulator* pManipulator, const Vec2i& point0, const Vec3& value, i32 flags)
{
...
bool CVegetationSelectTool::OnKeyDown(CViewport* view, u32 key, u32 repCnt, u32 nFlags)
{
	bool processed = false;
	switch (key)
	{
	...
	case Qt::Key_Escape:
		{
			if (!m_selectedThings.empty())
			{
				i32 numSelected = m_selectedThings.size();
...
{
	for (i32 i = 0; i < m_selectedThings.size(); ++i)
	{
...
void CVegetationSelectTool::SelectThing(CVegetationInstance* pVegetationInstance, bool showManipulator)
{
	...
	{
		pVegetationInstance->pRenderNode->SetEditorObjectId(drx_random_uint32());
