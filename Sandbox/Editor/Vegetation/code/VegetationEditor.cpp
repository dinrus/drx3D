...
void ImportObjectsFromXml(tukk xmlFile)
{
...
void ExportObjectsToXml(tukk xmlFile)
{
	auto pVegetationMap = GetIEditorImpl()->GetVegetationMap();
...
struct CVegetationEditor::SImplementation : public IEditorNotifyListener
{
	...
	struct SActions
	{
		...
		void SetupFileMenu(CAbstractMenu* pFileMenu)
		{
			...
			pAddNewGroupAction->setIcon(DrxIcon("icons:Vegetation/Tools_Add_Vegetation_Category.ico"));
			toolBarActions.push_back(pAddNewGroupAction);

			pAddNewVegetationObjectAction = pFileMenu->CreateAction(tr("Add Object"));
			pAddNewVegetationObjectAction->setIcon(DrxIcon("icons:Vegetation/Tools_Add_Vegetation_Object.ico"));
			toolBarActions.push_back(pAddNewVegetationObjectAction);

			i32k sec = pFileMenu->GetNextEmptySection();
		...
		void SetupEditMenu(CAbstractMenu* pEditMenu)
		{
			...
			pRemoveAction->setIcon(DrxIcon("icons:Vegetation/Tools_Clear_Vegetation.ico"));
			...
			pCloneVegetationObjectAction->setIcon(DrxIcon("icons:Vegetation/Tools_Duplicate_Vegetation_Object.ico"));
	...
	struct SUi
	{
		...
		static CVegetationTreeView* CreateVegetationTreeView(QAbstractItemModel* pModel, QWidget* pParent)
		{
			...
			for (i32 i = 1; i < pModel->columnCount(); ++i)
			{
	...
	void SetupControls()
	{
		...
		{
			i32 selectionCount = m_ui.pVegetationTreeView->selectionModel()->selectedRows().size();
		...
		QObject::connect(pSelectionModel, &QItemSelectionModel::selectionChanged, [this, pSelectionModel]
		{
			...
			for (const auto& currentIndex : currentSelection)
			{
			  if (m_pVegetationModel->IsGroup(currentIndex))
			  {
			    ...
			    for (i32 i = 0; i < pGroupItem->rowCount(); ++i)
			    {
	...
	{
		QObject::connect(m_pVegetationModel, &CVegetationModel::InfoDataChanged, [this](i32 objectCount, i32 instanceCount, i32 textureUsage)
		{
	...
	void SetupFileActions()
	{
		...
		QObject::connect(m_actions.pExportAction, &QAction::triggered, [this]
		{
			...
			{
			  DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "You need to select object(s) to export");
			}
	...
	void UpdatePaintBrushSerializer()
	{
		...
		{
			DRX_ASSERT(m_serializers.empty());
			AddVegetationSerializer();
		}
	...
	void AddVegetationObject()
	{
		...
		if (pGroupItem)
		{
			for (i32 i = oldObjectCount; i < pGroupItem->rowCount(); ++i)
			{
