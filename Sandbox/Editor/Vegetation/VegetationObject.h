...
struct SANDBOX_API CVegetationInstance
{
	...
	u8                brightness;             //!< Brightness of this object.
	float                angle;                  //!< Instance rotation.
	float                angleX;                 //!< Angle X for rotation
	float                angleY;                 //!< Angle Y for rotation

	i32                  m_refCount; //!< Number of references to this vegetation instance.
...
public:
	...
	CVegetationObject(i32 id);
	virtual ~CVegetationObject();

	void Serialize(XmlNodeRef& node, bool bLoading);

	// Member access
	tukk GetFileName() const { mv_fileName->Get(m_fileNameTmp); return m_fileNameTmp.GetString(); };
	...
	//! Get object's group.
	tukk GetGroup() const { return m_group; };
	...
	void SetNumInstances(i32 numInstances);
	...
	i32   GetNumInstances()       { return m_numInstances; };
	...
	i32   GetRotationRangeToTerrainNormal() const { return mv_rotationRangeToTerrainNormal; }
	...
	void SetId(i32 nId) { m_id = nId; SetEngineParams(); };
	...
	i32                GetTextureMemUsage(IDrxSizer* pSizer = NULL);
...
protected:
	...
	CSmartVariableEnum<i32> mv_hideable;
	CSmartVariableEnum<i32> mv_playerHideable;
	...
	CSmartVariable<i32>     mv_rotationRangeToTerrainNormal;
	...
	CSmartVariableEnum<i32> mv_growOn;

	CSmartVariable<bool>    mv_layerFrozen;
	CSmartVariable<bool>    mv_layerWet;

	CSmartVariableEnum<i32> mv_minSpec;
...
inline float CVegetationObject::CalcVariableSize() const
{
	...
	{
		float fval = mv_sizevar * drx_random(-1.0f, 1.0f);
		if (fval >= 0)
		{
