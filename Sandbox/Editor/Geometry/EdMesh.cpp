...
{
public:
	CUndoEdMesh(CEdMesh* pEdMesh, i32 nCopyFlags, tukk undoDescription)
	{
...
//////////////////////////////////////////////////////////////////////////
CEdMesh* CEdMesh::LoadMesh(tukk filename)
{
...
//////////////////////////////////////////////////////////////////////////
bool CEdMesh::IsSameObject(tukk filename)
{
...
//////////////////////////////////////////////////////////////////////////
bool CEdMesh::StartSubObjSelection(const Matrix34& nodeWorldTM, i32 elemType, i32 nFlags)
{
...
void CEdMesh::Display(DisplayContext& dc)
{
	...
	if (m_pStatObj)
	{
		i32 nStatObjFlags = m_pStatObj->GetFlags();
		if (g_SubObjSelOptions.displayType == SO_DISPLAY_GEOMETRY)
	...
	for (i32 i = 0; i < triMesh.GetFacesCount(); i++)
	{
	...
	if (g_SubObjSelOptions.displayType == SO_DISPLAY_FLAT)
	{
		...
		for (i32 i = 0; i < triMesh.GetFacesCount(); i++)
		{
			...
			if (triMesh.selectionType != SO_ELEM_FACE || !triMesh.faceSel[i])
			{
				...
				dt = max(0.4f, dt);
				dt = min(1.0f, dt);
				col.r = i32(faceColor.r * dt);
				col.g = i32(faceColor.g * dt);
				col.b = i32(faceColor.b * dt);
				col.a = faceColor.a;
	...
	if (triMesh.selectionType == SO_ELEM_VERTEX || triMesh.StreamHaveSelection(CTriMesh::VERTICES))
	{
		...
		for (i32 i = 0; i < triMesh.GetVertexCount(); i++)
		{
			bool bSelected = triMesh.vertSel[i] || triMesh.pWeights[i] != 0;
			if (bSelected)
			{
				i32 clr = (triMesh.pWeights[i] + fClrAdd) * 255;
	...
	if (triMesh.selectionType == SO_ELEM_EDGE || triMesh.StreamHaveSelection(CTriMesh::EDGES))
	{
		...
		// Draw selected edges.
		for (i32 i = 0; i < triMesh.GetEdgeCount(); i++)
		{
...
bool CEdMesh::SelectSubObjElements(SSubObjHitTestEnvironment& env, SSubObjHitTestResult& result)
{
	...
	if (env.bSelectOnHit && !result.elems.empty())
	{
		...
		{
			// Select on hit.
			for (i32 i = 0, num = result.elems.size(); i < num; i++)
			{
				i32 elem = result.elems[i];
...
bool CEdMesh::HitTest(HitContext& hit)
{
	...
	i32 nHitTestWhat = (hit.nSubObjFlags & SO_HIT_ELEM_ALL);
	if (nHitTestWhat == 0)
	{
...
bool CEdMesh::GetSelectionReferenceFrame(Matrix34& refFrame)
{
	...
	if (triMesh.selectionType == SO_ELEM_VERTEX)
	{
		...
		i32 numNormals = 0;
		i32 nFaces = triMesh.GetFacesCount();
		for (i32 i = 0; i < triMesh.GetVertexCount(); i++)
		{
...
//////////////////////////////////////////////////////////////////////////
void CEdMesh::CopyToMesh(CTriMesh& toMesh, i32 nCopyFlags)
{
	if (!m_pSubObjCache)
		return;
...
//////////////////////////////////////////////////////////////////////////
void CEdMesh::CopyFromMesh(CTriMesh& fromMesh, i32 nCopyFlags, bool bUndo)
{
	if (m_pSubObjCache)
	{
...
//////////////////////////////////////////////////////////////////////////
void CEdMesh::SaveToCGF(tukk sFilename, CPakFile* pPakFile, IMaterial* pMaterial)
{
	if (m_pStatObj)
	{
		...
		else
		{
			...
			if (m_pStatObj->SaveToCGF(sFilename, &pChunkFile))
			{
				...
				pPakFile->UpdateFile(sFilename, pMemFile, nFileSize, true, IDrxArchive::LEVEL_FASTER);
				pChunkFile->Release();
			}
		}
...
//////////////////////////////////////////////////////////////////////////
CEdMesh* CEdMesh::CreateMesh(tukk name)
{
...
void CEdMesh::SetWorldTM(const Matrix34& worldTM)
{
	...
	//////////////////////////////////////////////////////////////////////////
	CTriMesh& triMesh = *m_pSubObjCache->pTriMesh;
	i32 nVerts = triMesh.GetVertexCount();
