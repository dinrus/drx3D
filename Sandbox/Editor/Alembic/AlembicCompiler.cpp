...
private:
	static string GetTemporaryDirectoryPath()
	{
		...
		return GetISystem()->GetIPak()->AdjustFileName("%USER%/temp/AlembicCompiler", path, IDrxPak::FLAGS_PATH_REAL | IDrxPak::FLAGS_FOR_WRITING | IDrxPak::FLAGS_ADD_TRAILING_SLASH);
	}
	...
	static std::vector<SAsset> CreateTemp(const string& dstFile)
	{
		...
		DRX_ASSERT(!GetISystem()->GetIPak()->IsAbsPath(relPath));
		...
		for (const string& suffix : suffixes)
		{
			...
			const string dstDrxasset = dstPath + ".cryasset";
			const string tmpDrxasset = tmpPath + ".cryasset";
			CopyFileA(dstDrxasset.c_str(), tmpDrxasset.c_str(), false);
		}
...
bool CAlembicCompiler::CompileAlembic(string& fileName, const string& fullPath, bool showDialog)
{
	...
	if (!showDialog || dialog.DoModal() == IDOK)
	{
		...
		const CString vertexIndexFormat = (sizeof(vtx_idx) == sizeof(u16)) ? "u16" : "u32";

		string targetPath = PathUtil::ReplaceExtension(rcFilePath.GetString(), DRX_GEOM_CACHE_FILE_EXT);
		...
		CResourceCompilerDialog rcDialog(rcFilePath, additionalSettings, [&](const CResourceCompilerHelper::ERcCallResult result)
		{
			...
			fileName = PathUtil::ReplaceExtension(fileName.GetString(), DRX_GEOM_CACHE_FILE_EXT);
			bResult = true;
		});
