...
public:
	...
	CMaterial*   LoadMaterial(tukk sMaterialName, bool bMakeIfNotFound = true);
	virtual void OnRequestMaterial(IMaterial* pMaterial);
	// Creates a new material from a xml node.
	CMaterial*   CreateMaterial(const string& sMaterialName, XmlNodeRef& node = XmlNodeRef(), i32 nMtlFlags = 0, u64 nLoadingFlags = 0);
	CMaterial*   CreateMaterial(tukk sMaterialName, XmlNodeRef& node = XmlNodeRef(), i32 nMtlFlags = 0, u64 nLoadingFlags = 0);

	// Duplicate material and do nothing more.
	CMaterial* DuplicateMaterial(tukk newName, CMaterial* pOriginal);

	//! Export property manager to game.
	void         Export(XmlNodeRef& node);
	i32          ExportLib(CMaterialLibrary* pLib, XmlNodeRef& libNode);
	...
	void SetHighlightMask(i32 highlightMask);
	i32  GetHighlightMask() const { return m_highlightMask; }
	...
	virtual IMaterial* OnLoadMaterial(tukk sMtlName, bool bForceCreation = false, u64 nLoadingFlags = 0);
	...
	CMaterial* SelectNewMaterial(i32 nMtlFlags, tukk sStartPath = NULL);
...
protected:
	...
	static bool               SelectSaveMaterial(string& itemName, tukk defaultStartPath);
	...
	void                      RemoveFromHighlighting(CMaterial* pMaterial, i32 mask);
	i32                       GetHighlightFlags(CMaterial* pMaterial) const;
