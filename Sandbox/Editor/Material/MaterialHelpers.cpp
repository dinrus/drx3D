...
{
//////////////////////////////////////////////////////////////////////////
static void ParsePublicParamsScript(tukk sUIScript, IVariable* pVar)
{
...
CVarBlock* GetPublicVars(SInputShaderResources& pShaderResources)
{
	...
	for (i32 i = 0; i < pShaderResources.m_ShaderParams.size(); i++)
	{
		...
		{
		case eType_BYTE:
			pIVar = new CVariable<i32>(pParam->m_Value.m_Byte);
			break;
		case eType_SHORT:
			pIVar = new CVariable<i32>(pParam->m_Value.m_Short);
			break;
		case eType_INT:
			pIVar = new CVariable<i32>(pParam->m_Value.m_Int);
			break;
		case eType_FLOAT:
...
void SetPublicVars(CVarBlock* pPublicVars, SInputShaderResources& pInputShaderResources)
{
	...
	i32 numVars = pPublicVars->GetNumVariables();
	...
	for (i32 i = 0; i < numVars; i++)
	{
		...
		for (i32 j = 0; j < pInputShaderResources.m_ShaderParams.size(); j++)
		{
