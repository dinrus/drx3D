...
std::vector<STxt> PyGetSubMaterial(tukk pMaterialPath)
{
	...
	for (i32 i = 0; i < pMaterial->GetSubMaterialCount(); i++)
	{
		...
		{
			result.push_back(static_cast<STxt>(materialPath + "\\" + pMaterial->GetSubMaterial(i)->GetName()));
		}
	}
	return result;
}

CMaterial* TryLoadingMaterial(tukk pPathAndMaterialName)
{
...
std::deque<string> PreparePropertyPath(tukk pPathAndPropertyName)
{
...
string TryConvertingETEX_TypeToCString(u8k& texTypeName)
{
...
string TryConvertingTexFilterToCString(i32k& iFilterName)
{
	switch (iFilterName)
	{
	case FILTER_NONE:
...
string TryConvertingETexGenTypeToCString(u8k& texGenType)
{
	switch (texGenType)
	{
	case ETG_Stream:
...
string TryConvertingETexModRotateTypeToCString(u8k& rotateType)
{
	switch (rotateType)
	{
	case ETMR_NoChange:
...
string TryConvertingETexModMoveTypeToCString(u8k& oscillatorType)
{
...
string ParseUINameFromPublicParamsScript(tukk sUIScript)
{
...
std::map<string, float> ParseValidRangeFromPublicParamsScript(tukk sUIScript)
{
	string uiscript = sUIScript;
...
bool ValidateProperty(CMaterial* pMaterial, const std::deque<string>& splittedPropertyPathParam, const SPyWrappedProperty& value)
{
	...
	i32k iMinColorValue = 0;
	i32k iMaxColorValue = 255;
	...
	{
		for (i32 i = 0; i < pMaterial->GetShaderGenParamsVars()->GetNumVariables(); i++)
		{
...
SPyWrappedProperty PyGetProperty(tukk pPathAndMaterialName, tukk pPathAndPropertyName)
{
	...
	else if (categoryName == "Lighting Settings")
	{
		...
		else if (propertyName == "Glossiness")
		{
			...
			value.property.intValue = (i32)value.property.floatValue;
		}
...
void PySetProperty(tukk pPathAndMaterialName, tukk pPathAndPropertyName, const SPyWrappedProperty& value)
{
	CMaterial* pMaterial = TryLoadingMaterial(pPathAndMaterialName);
	std::deque<string> splittedPropertyPath = PreparePropertyPath(pPathAndPropertyName);
	std::deque<string> splittedPropertyPathCategory = splittedPropertyPath;
	...
	else if (categoryName == "Advanced")
	{
		...
		{
			pMaterial->GetShaderResources().m_FurAmount = static_cast<u8>(value.property.floatValue * 255.0f);
		}
		else if (propertyName == "Voxel Coverage")
		{
			pMaterial->GetShaderResources().m_VoxelCoverage = static_cast<u8>(value.property.floatValue * 255.0f);
		}
		else if (propertyName == "Heat Amount")
		{
			pMaterial->GetShaderResources().m_HeatAmount = static_cast<u8>((value.property.floatValue / 4.0f) * 255.0f);
		}
		else if (propertyName == "Cloak Amount")
		{
			pMaterial->GetShaderResources().m_CloakAmount = static_cast<u8>((value.property.floatValue * 255.0f));
		}
		else if (propertyName == "Link to Material")
		{
			...
			if (propertyName == ParseUINameFromPublicParamsScript(shaderParams[i].m_Script))
			{
				...
				else if (shaderParams[i].m_Type == eType_FCOLOR)
				{
					ColorF colorLinear = CMFCUtils::ColorGammaToLinear(
					  RGB(
					    (u8)value.property.colorValue.r,
					    (u8)value.property.colorValue.g,
					    (u8)value.property.colorValue.b
					    ));
...
REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE(PySetProperty, material, set_property,
                                          ...
                                          "material.set_property(str materialPath/materialName, str propertyPath/propertyName, [ str | (i32, i32, i32) | (float, float, float) | i32 | float | bool ] value)");
