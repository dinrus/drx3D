...
public:
	...
	void StartScan()
	{
		...
		{
			DrxAutoCriticalSection lock(m_lock); // lock probably not needed but let's assume multiple threads can call GetLoadedFiles()
		...
		{
			DrxFatalError("Error spawning \"MaterialFilesScanning\" thread.");
		}
	}
...
public:
	CDrxSignal<void(const CFileUtil::FileArray&)> filesAddedSignal;
	CDrxSignal<void()>                            fileCacheClearedSignal;
...
bool QTexturePreview::IsPower2(i32 n)
{
	return (((n - 1) & n) == 0);
}
...
{
	// Save the filename
	string relPath = PathUtil::AbsolutePathToDrxPakPath(fileName.toStdString().c_str());
	if (relPath.empty())
	...
	if (!CFileUtil::FileExists(CString(PathUtil::GamePathToDrxPakPath(relPath))) ||
	    !CImageUtil::LoadImage(relPath, texture, &bQualityLoss))
	{
		...
		{
			DrxLog("Error loading layer texture (%s)...", (tukk)relPath);
			bError = true;
		}
	}
	...
	for (i32 y = 0; y < texture.GetHeight(); y++)
	{
		for (i32 x = 0; x < texture.GetWidth(); x++)
		{
			u32 val = texture.ValueAt(x, y);
...
void QMaterialPreview::PreviewItem(const QString& path)
{
	...
	if (pManager)
	{
		...
		i32 count = pNewMaterial->GetSubMaterialCount();
...
void CMaterialToolBar::OnFilter()
{
	...
	i32 cmd = menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RETURNCMD, point.x, point.y, this);
	switch (cmd)
	{
...
//////////////////////////////////////////////////////////////////////////
void CMaterialBrowserCtrl::OnSize(UINT nType, i32 cx, i32 cy)
{
	...
	for (i32 i = 0; i < num; i++)
	{
		...
		else if (!pRecord->materialName.IsEmpty())
		{
			...
			if (cryfile.Open(filename, "rb"))
			{
				if (!cryfile.IsInPak())
				{
					u32 scFileAttributes = CFileUtil::GetAttributes(filename);
		...
		if (m_bShowOnlyCheckedOut && isVisible)
		{
			if (isMaterial)
			{
				for (i32 i = 0; i < pRecord->GetChilds()->GetCount(); i++)
...
//////////////////////////////////////////////////////////////////////////
CMaterialBrowserRecord* CMaterialBrowserCtrl::InsertItemToTree(IDataBaseItem* pItem, i32 nSubMtlSlot, CMaterialBrowserRecord* pParentRecord)
{
...
void CMaterialBrowserCtrl::ReloadTreeSubMtls(CMaterialBrowserRecord* pRecord, bool bForceCtrlUpdate)
{
	...
	{
		for (i32 i = pRecord->GetChildCount() - 1; i >= 0; --i)
		{
	...
	if (pRecord)
	{
		...
		{
			for (i32 i = 0; i < pMtl->GetSubMaterialCount(); i++)
			{
...
void CMaterialBrowserCtrl::TickRefreshMaterials()
{
	...
	for (i32 i = 0, num = pRows->GetCount(); i < num; i++)
	{
			...
			if (pTrgParentItem == pSrcParentItem)
			{
				...
				if (pTrgItem->nSubMtlSlot >= 0 && pSrcItem->nSubMtlSlot >= 0)
				{
					if (pTrgParentItem && pTrgParentItem->pMaterial != NULL && pTrgParentItem->pMaterial->IsMultiSubMaterial())
					{
						...
						i32 nSrcSlot = pSrcItem->nSubMtlSlot;
						i32 nTrgSlot = pTrgItem->nSubMtlSlot;
...
void CMaterialBrowserCtrl::OnRenameItem()
{
	...
	else
	{
		...
		{
			Warning("Material with name %s already exist", (tukk)itemName);
			return;
		}
...
void CMaterialBrowserCtrl::OnSetSubMtlCount(CMaterialBrowserRecord* pRecord)
{
	...
	i32 num = pMtl->GetSubMaterialCount();
	...
	case ESCM_IMPORT:
		if (pMtl)
		{
			...
			if (dlg.DoModal() == IDOK)
			{
				...
				i32 nTextures = pMtl->GetTextureFilenames(filenames);
				for (i32 i = 0; i < nTextures; ++i)
	...
	case ESCM_CHECKOUT:
		{
			if (pMtl && (pMtl->GetFileAttributes() & SCC_FILE_ATTRIBUTE_BYANOTHER))
			{
				...
				if (!GetIEditorImpl()->GetSourceControl()->GetOtherUser(pMtl->GetFilename(), username, 64))
					drx_strcpy(username, "another user");
				CString str;
...
//////////////////////////////////////////////////////////////////////////
void CMaterialBrowserCtrl::SetSubMaterial(CMaterialBrowserRecord* pRecord, i32 nSlot, CMaterial* pSubMaterial, bool bSelectSubMtl)
{
...
{
	i32 nSlot = (INT_PTR)pMtlItem->pUserData;
	if (nSlot < 0)
	{
		return;
	}
...
void CMaterialBrowserCtrl::OnSaveToFile(bool bMulti)
{
	...
	if (pCurrentMaterial)
	{
		...
		filename = PathUtil::AbsolutePathToDrxPakPath(filename.GetString());
		...
		i32 flags = pCurrentMaterial->GetFlags();
		if (bMulti)
	...
	if (m_pMaterialImageListCtrl)
	{
		...
		if (pMtl)
		{
			...
			if (m_pLastActiveMultiMaterial != pMultiMtl || pMultiMtl == 0)
			{
				// Normal material.
				m_pMaterialImageListCtrl->DeleteAllItems();
				m_pMaterialImageListCtrl->AddMaterial(pMtl, (uk )(intptr_t)pRecord->nSubMtlSlot);
			}
			else
			{
				m_pMaterialImageListCtrl->SetMaterial(0, pMtl, (uk )(intptr_t)pRecord->nSubMtlSlot);
			}
			if (m_pLastActiveMultiMaterial != pMultiMtl && pMultiMtl != 0)
			{
				for (i32 i = 0; i < pMultiMtl->GetSubMaterialCount(); i++)
				{
					if (pMultiMtl->GetSubMaterial(i))
						m_pMaterialImageListCtrl->AddMaterial(pMultiMtl->GetSubMaterial(i), (uk )(intptr_t)i);
				}
...
void CMaterialBrowserCtrl::ShowContextMenu(CMaterialBrowserRecord* pRecord, CPoint point)
{
	...
	{
		i32 numMaterialsSelected = 0;
		for (i32 i = 0; i < m_markedRecords.size(); ++i)
		{
	...
	else
	{
		if (pRecord->nSubMtlSlot >= 0)
		{
			...
			{
				u32 nFileAttr = pParentItemInfo->pMaterial->GetFileAttributes();
		...
		if (pMtl)
		{
			...
			u32 nFileAttr = pMtl->GetFileAttributes();
	...
	i32 cmd = menu.TrackPopupMenu(TPM_RETURNCMD | TPM_LEFTALIGN | TPM_LEFTBUTTON, point.x, point.y, this);
	switch (cmd)
	{
	case MENU_UNDEFINED:
		return;                    // do nothing
...
{
	u32 materialNameCrc32 = MaterialNameToCrc32(materialName);
	u32 parentNameCrc32 = MaterialNameToCrc32(parentMaterialName);
...
//////////////////////////////////////////////////////////////////////////
u32 CMaterialBrowserCtrl::MaterialNameToCrc32(tukk str)
{
...
CMaterialBrowserRecord* CMaterialBrowserCtrl::GetSelectedRecord()
{
	...
	i32 nCount = m_tree.GetSelectedRows()->GetCount();
...
void CMaterialBrowserTreeCtrl::OnSelectionChanged()
{
	...
	i32 nCount = GetSelectedRows()->GetCount();
	for (i32 i = 0; i < nCount; i++)
	{
...
//////////////////////////////////////////////////////////////////////////
CTreeItemRecord* CMaterialBrowserTreeCtrl::CreateGroupRecord(tukk name, i32 nGroupIcon)
{
...
void CMaterialBrowserTreeCtrl::SelectItem(CMaterialBrowserRecord* pRecord, const TMaterialBrowserRecords* pMarkedRecords)
{
	...
	if (pRecord || pMarkedRecords)
	{
		...
		for (i32 i = 0, nNumRows = pRows->GetCount(); i < nNumRows; i++)
		{
			CXTPReportRow* pRow = pRows->GetAt(i);
			CMaterialBrowserRecord* pCurrentRecord = RowToRecord(pRow);
...
void CMaterialBrowserTreeCtrl::UpdateItemState(CMaterialBrowserRecord* pRecord)
{
	...
	i32 nIconMtl = ITEM_IMAGE_MATERIAL;
...
//////////////////////////////////////////////////////////////////////////
i32 _cdecl CMaterialBrowserTreeCtrl::RowCompareFunc(const CXTPReportRow** pRow1, const CXTPReportRow** pRow2)
{
...
bool CMaterialBrowserTreeCtrl::OnFilterTestSubmaterials(CTreeItemRecord* pRecord, bool bAddOperation)
{
	...
	i32 nSubMaterialCount = pMaterial->GetSubMaterialCount();
	for (i32 i = 0; i < nSubMaterialCount; ++i)
	{
...
void CMaterialBrowserRecord::CreateItems()
{
	...
	if (nSubMtlSlot >= 0)
	{
		i32 numSubMaterials = 0;
		if (pMaterial && pMaterial->GetParent())
			numSubMaterials = pMaterial->GetParent()->GetSubMaterialCount();
		tukk format = numSubMaterials > 9 ? "[%02d] %s" : "[%d] %s";
		itemText.Format(format, nSubMtlSlot + 1, (tukk)materialName);
	}
	else
	{
...
{
	for (i32 i = GetChildCount() - 1; i >= 0; --i)
	{
