...
class CMaterialEditorClass : public IViewPaneClass
{
	...
	virtual tukk    ClassName()       override { return MATERIAL_EDITOR_NAME; };
	virtual tukk    Category()        override { return "Editor"; };
	virtual tukk    GetMenuPath()     override { return ""; }
	//////////////////////////////////////////////////////////////////////////
	virtual CRuntimeClass* GetRuntimeClass() override { return RUNTIME_CLASS(CMaterialDialog); };
	virtual tukk    GetPaneTitle()    override { return _T(MATERIAL_EDITOR_NAME); };
...
struct STextureVars
{
	...
	//CSmartVariable<i32> amount;
	CSmartVariable<bool>    is_tile[2];

	CSmartVariableEnum<i32> etcgentype;
	CSmartVariableEnum<i32> etcmrotatetype;
	CSmartVariableEnum<i32> etcmumovetype;
	CSmartVariableEnum<i32> etcmvmovetype;
	CSmartVariableEnum<i32> etextype;
	CSmartVariableEnum<i32> filter;
...
struct SVertexWaveFormUI
{
	...
	CSmartVariableEnum<i32> waveFormType;
...
{
	DRX_ASSERT(!strcmp(pVar->GetName(), "TexType"));
...
public:
	...
	CVarEnumList<i32>*     enumTexType;
	CVarEnumList<i32>*     enumTexGenType;
	CVarEnumList<i32>*     enumTexModRotateType;
	CVarEnumList<i32>*     enumTexModUMoveType;
	CVarEnumList<i32>*     enumTexModVMoveType;
	CVarEnumList<i32>*     enumTexFilterType;

	CVarEnumList<i32>*     enumVertexMod;
	CVarEnumList<i32>*     enumWaveType;
	...
	void SetToMaterial(CMaterial* mtl, i32 propagationFlags = MTL_PROPAGATE_ALL);

	void SetShaderResources(const SInputShaderResources& srTextures, const SInputShaderResources& srTpl, bool bSetTextures = true);
	void GetShaderResources(SInputShaderResources& sr, i32 propagationFlags);

	void SetVertexDeform(const SInputShaderResources& sr);
	void GetVertexDeform(SInputShaderResources& sr, i32 propagationFlags);

	void SetDetailDecalInfo(const SInputShaderResources& sr);
	void GetDetailDecalInfo(SInputShaderResources& sr, i32 propagationFlags);
	...
	CVarBlock* CreateVars()
	{
		...
		// Init enums.
		//////////////////////////////////////////////////////////////////////////
		enumTexType = new CVarEnumList<i32>;
		...
		enumTexModUMoveType = new CVarEnumList<i32>;
		...
		enumTexModVMoveType = new CVarEnumList<i32>;
		...
		enumTexFilterType = new CVarEnumList<i32>;
		...
		// Vertex Mods.
		//////////////////////////////////////////////////////////////////////////
		enumVertexMod = new CVarEnumList<i32>;
		...
		enumWaveType = new CVarEnumList<i32>;
		...
		for (i32 nLayer = 0; nLayer < MTL_LAYER_MAX_SLOTS; ++nLayer)
		{
...
private:
	//////////////////////////////////////////////////////////////////////////
	void InitTextureVars(i32 id, const string& name)
	{
	...
	void   SetTextureResources(const SInputShaderResources& sr, const SInputShaderResources& srTpl, i32 texid, bool bSetTextures);
	void   GetTextureResources(SInputShaderResources& sr, i32 texid, i32 propagationFlags);
...
//////////////////////////////////////////////////////////////////////////
void CMaterialUI::GetShaderResources(SInputShaderResources& sr, i32 propagationFlags)
{
...
inline float RoundDegree(float val)
{
	...
	//return v*0.01f;
	return (float)((i32)(val * 100 + 0.5f)) * 0.01f;
}

//////////////////////////////////////////////////////////////////////////
void CMaterialUI::SetTextureResources(const SInputShaderResources& sr, const SInputShaderResources& srTpl, i32 tex, bool bSetTextures)
{
	/*
	   // Enable/Disable texture map, depending on the mask.
	   i32 flags = textureVars[tex].GetFlags();
	...
	textures[tex].filter = (i32)sr.m_Textures[tex].m_Filter;
...
//////////////////////////////////////////////////////////////////////////
static tukk fpGetExtension(tukk in)
{
...
static bool IsFlashUIFile(tukk pFlashFileName)
{
	if (pFlashFileName)
	{
		tukk pExt = fpGetExtension(pFlashFileName);
...
void CMaterialUI::GetTextureResources(SInputShaderResources& sr, i32 tex, i32 propagationFlags)
{
	...
	sr.m_Textures[tex].m_Name = (tukk)texFilename;
	...
	sr.m_Textures[tex].m_Filter = (i32)textures[tex].filter;
...
{
	vertexMod.type = (i32)sr.m_DeformInfo.m_eType;
...
//////////////////////////////////////////////////////////////////////////
void CMaterialUI::GetVertexDeform(SInputShaderResources& sr, i32 propagationFlags)
{
	if ((propagationFlags & MTL_PROPAGATE_VERTEX_DEF) == 0)
		return;

	sr.m_DeformInfo.m_eType = (EDeformType)((i32)vertexMod.type);
	...
	sr.m_DeformInfo.m_WaveX.m_eWFType = (EWaveForm)((i32)vertexMod.wave[0].waveFormType);
	...
	sr.m_DeformInfo.m_WaveY.m_eWFType = (EWaveForm)((i32)vertexMod.wave[1].waveFormType);
	...
	sr.m_DeformInfo.m_WaveZ.m_eWFType = (EWaveForm)((i32)vertexMod.wave[2].waveFormType);
	...
	sr.m_DeformInfo.m_WaveW.m_eWFType = (EWaveForm)((i32)vertexMod.wave[3].waveFormType);
...
//////////////////////////////////////////////////////////////////////////
void CMaterialUI::GetDetailDecalInfo(SInputShaderResources& sr, i32 propagationFlags)
{
	if ((propagationFlags & MTL_PROPAGATE_TEXTURES) == 0)
		return;

	const float fRatioToUint8 = 255.0f / 100.0f;
	sr.m_DetailDecalInfo.nBlending = (u8)int_round(detailDecal.fBlending * fRatioToUint8);
	sr.m_DetailDecalInfo.nSSAOAmount = (u8)int_round(detailDecal.fSSAOAmount * fRatioToUint8);
	...
	sr.m_DetailDecalInfo.nThreshold[0] = (u8) int_round(detailDecal.fThreshold[0] * fRatioToUint8);
	sr.m_DetailDecalInfo.nDeformation[0] = (u8) int_round(detailDecal.fDeformation[0] * fRatioToUint8);
	...
	sr.m_DetailDecalInfo.nThreshold[1] = (u8) int_round(detailDecal.fThreshold[1] * fRatioToUint8);
	sr.m_DetailDecalInfo.nDeformation[1] = (u8) int_round(detailDecal.fDeformation[1] * fRatioToUint8);
...
void CMaterialUI::PropagateToLinkedMaterial(CMaterial* mtl, CVarBlockPtr pShaderParams)
{
	...
	if (parentMtl)
	{
		for (i32 i = 0; i < parentMtl->GetSubMaterialCount(); ++i)
		{
	...
	bPropagateShaderParams = (i32)bPropagateShaderParams & - (i32)bPropagateMaterialSettings;
	bPropagateShaderGenParams = (i32)bPropagateShaderGenParams & - (i32)bPropagateMaterialSettings;

	propFlags |= MTL_PROPAGATE_MATERIAL_SETTINGS & - (i32)bPropagateMaterialSettings;
	propFlags |= MTL_PROPAGATE_OPACITY & - (i32)bPropagateOpactity;
	propFlags |= MTL_PROPAGATE_LIGHTING & - (i32)bPropagateLighting;
	propFlags |= MTL_PROPAGATE_ADVANCED & - (i32)bPropagateAdvanced;
	propFlags |= MTL_PROPAGATE_TEXTURES & - (i32)bPropagateTexture;
	propFlags |= MTL_PROPAGATE_SHADER_PARAMS & - (i32)bPropagateShaderParams;
	propFlags |= MTL_PROPAGATE_SHADER_GEN & - (i32)bPropagateShaderGenParams;
	propFlags |= MTL_PROPAGATE_VERTEX_DEF & - (i32)bPropagateVertexDef;
	propFlags |= MTL_PROPAGATE_LAYER_PRESETS & - (i32)bPropagateLayerPresets;
...
void CMaterialUI::SetFromMaterial(CMaterial* mtlIn, bool bSetTemplate)
{
	...
	u32k nDefaultFlagsEFTT = (1 << EFTT_DETAIL_OVERLAY) | (1 << EFTT_DECAL_OVERLAY) | (1 << EFTT_CUSTOM) | (1 << EFTT_CUSTOM_SECONDARY);
	...
	for (i32 l(0); l < MTL_LAYER_MAX_SLOTS; ++l)
	{
...
void CMaterialUI::SetToMaterial(CMaterial* mtl, i32 propagationFlags)
{
		...
		if (m_publicVars && pPublicVars)
		{
			...
			if (m_excludedPublicVars.pMaterial)
			{
				...
				else
				{
					...
					for (i32 j = pPublicVars->GetNumVariables() - 1; j >= 0; --j)
					{
						IVariable* pVar = pPublicVars->GetVariable(j);
						bool isVarExist = false;
						for (i32 i = m_publicVars->GetNumVariables() - 1; i >= 0; --i)
						{
						...
						if (!isVarExist) // var exist in new pPublicVars block, but not in previous (m_publicVars)
						{
							...
							for (i32 i = m_excludedPublicVars.vars.GetNumVariables() - 1; i >= 0; --i)
							{
		...
		for (IDataBaseItem* pItem = pEnum->GetFirst(); pItem != NULL; pItem = pEnum->GetNext())
		{
			...
			if (pLoadedMaterial)
			{
				if (pLoadedMaterial->GetMatTemplate() == mtl->GetName())
				{
					...
					i32 flags = pLoadedMaterial->GetFlags();
...
bool CMaterialDialog::SetItemName(CBaseLibraryItem* item, const CString& groupName, const CString& itemName)
{
	...
	if (pOtherItem && pOtherItem != item)
	{
		...
		Warning("Duplicate Item Name %s", (tukk)fullName);
		return false;
	}
	else
	{
...
{
	i32 sel = m_listTypeCtrl.GetCurSel();
	if (sel == 0)
		m_wndMtlBrowser.ReloadItems(CMaterialBrowserCtrl::VIEW_ALL);
	else if (sel == 1)
