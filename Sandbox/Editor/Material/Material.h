...
struct SANDBOX_API SMaterialLayerResources
{
	...
	void SetFromUI(tukk shaderName, bool bNoDraw, bool bFadeOut);
...
public:
	//////////////////////////////////////////////////////////////////////////
	CMaterial(const string& name, i32 nFlags = 0);
	~CMaterial();
	...
	void					  RenameSubMaterial(i32 subMtlSlot, const string& newName);
	//! Renames a submaterial and correctly handles undo actions
	...
	i32    GetTextureFilenames(std::vector<string>& outFilenames) const;
	i32    GetAnyTextureFilenames(std::vector<string>& outFilenames) const;

	//! Returns number of dependencies and file paths (game path) for asset saving
	i32	   GetTextureDependencies(std::vector<SAssetDependencyInfo>& outFilenames) const;

	void   UpdateFileAttributes();
	u32 GetFileAttributes();
	//////////////////////////////////////////////////////////////////////////

	//! Sets one or more material flags from EMaterialFlags enum.
	void SetFlags(i32 flags)        { m_mtlFlags = flags; };
	//! Query this material flags.
	virtual i32  GetFlags() const override           { return m_mtlFlags; }
	...
	i32 FindSubMaterial(CMaterial* pMaterial);
	...
	CDrxSignal<void(SubMaterialChange)> signalSubMaterialsChanged;
	...
	void           LinkToMaterial(tukk name) { LinkToMaterial(string(name)); } // for CString conversion
	...
	void SetHighlightFlags(i32 highlightFlags);
	void UpdateHighlighting();
	void RecordUndo(tukk sText, bool bForceUpdate = false);

	i32  GetPropagationFlags() const          { return m_propagationFlags; }
	void SetPropagationFlags(i32k flags) { m_propagationFlags = flags; }
...
private:
	...
	// during ui editing
	i32 m_propagationFlags;

	//! Material Used in level.
	i32  m_bDummyMaterial          : 1; // Dummy material, name specified but material file not found.
	i32  m_bFromEngine             : 1; // Material from engine.
	i32  m_bIgnoreNotifyChange     : 1; // Do not send notifications about changes.
	i32  m_bRegetPublicParams      : 1;
	i32  m_bKeepPublicParamsValues : 1;
