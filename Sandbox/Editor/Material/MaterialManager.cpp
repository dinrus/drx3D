...
static tukk MATERIALS_LIBS_PATH = "Materials/";
static u32 s_highlightUpdateCounter = 0;
...
{
public:
	void Start(CMaterial* pMaterial, i32 modeFlag);
	void Stop(CMaterial* pMaterial, i32 modeFlag);
	void GetHighlightColor(ColorF* color, float* intensity, i32 flags);
...
void CMaterialHighlighter::Start(CMaterial* pMaterial, i32 modeFlag)
{
...
void CMaterialHighlighter::Stop(CMaterial* pMaterial, i32 modeFlag)
{
	if (pMaterial)
...
{
	u32 counter = s_highlightUpdateCounter;
...
void CMaterialHighlighter::GetHighlightColor(ColorF* color, float* intensity, i32 flags)
{
...
//////////////////////////////////////////////////////////////////////////
CMaterial* CMaterialManager::CreateMaterial(const string& sMaterialName, XmlNodeRef& node, i32 nMtlFlags, u64 nLoadingFlags)
{
...
//////////////////////////////////////////////////////////////////////////
CMaterial* CMaterialManager::CreateMaterial(tukk sMaterialName, XmlNodeRef& node, i32 nMtlFlags, u64 nLoadingFlags)
{
...
void CMaterialManager::Export(XmlNodeRef& node)
{
	...
	for (i32 i = 0; i < GetLibraryCount(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CMaterialManager::ExportLib(CMaterialLibrary* pLib, XmlNodeRef& libNode)
{
	i32 num = 0;
	// Export library.
	libNode->setAttr("Name", pLib->GetName());
	libNode->setAttr("File", pLib->GetFilename());
	libNode->setAttr("SandboxVersion", (tukk)GetIEditorImpl()->GetFileVersion().ToFullString());
	// Serialize prototypes.
	for (i32 j = 0; j < pLib->GetItemCount(); j++)
	{
...
void CMaterialManager::ReportDuplicateItem(CBaseLibraryItem* pItem, CBaseLibraryItem* pOldItem)
{
	...
	DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING,
	           ...
	           (tukk)pItem->GetName(), (tukk)pOldItem->GetName());
}
...
i32 CMaterialManager::GetShaderCount()
{
	...
	if (!m_bShadersEnumerated)
	{
		...
		if (root)
		{
			for (i32 i = 0; i < root->getChildCount(); ++i)
			{
				...
				tukk pTagName = ChildNode->getTag();
		...
		{
			DRX_ASSERT(shaderName.length() > 0);
...
{
	virtual bool nativeEventFilter(const QByteArray& eventType, uk pMessage, long* pResult) override
	{
...
CMaterial* CMaterialManager::LoadMaterial(const string& sMaterialName, bool bMakeIfNotFound)
{
	...
	i32 nExtLen = strlen(MATERIAL_FILE_EXT);
	...
	if (filename.GetLength() - nExtLen < sMaterialNameClear.GetLength() - 1)
	{
		...
		i32 nNewLenMaterialName = filename.GetLength() - nExtLen;
	...
	else
	{
		if (bMakeIfNotFound)
		{
			...
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Material %s not found", (tukk)sMaterialNameClear);
		}
	}
	//
...
//////////////////////////////////////////////////////////////////////////
CMaterial* CMaterialManager::LoadMaterial(tukk sMaterialName, bool bMakeIfNotFound)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CMaterialManager::GetHighlightFlags(CMaterial* pMaterial) const
{
	if (pMaterial == NULL)
		return 0;
...
void CMaterialManager::AddForHighlighting(CMaterial* pMaterial)
{
	...
	i32 highlightFlags = (GetHighlightFlags(pMaterial) & m_highlightMask);
	if (highlightFlags != 0)
	{
		m_pHighlighter->Start(pMaterial, highlightFlags);
	}

	i32 count = pMaterial->GetSubMaterialCount();
...
//////////////////////////////////////////////////////////////////////////
void CMaterialManager::RemoveFromHighlighting(CMaterial* pMaterial, i32 mask)
{
	if (pMaterial == NULL)
		return;
...
//////////////////////////////////////////////////////////////////////////
IMaterial* CMaterialManager::OnLoadMaterial(tukk sMtlName, bool bForceCreation, u64 nLoadingFlags)
{
...
bool CMaterialManager::DeleteMaterial(CMaterial* pMtl)
{
	...
	for (i = 0; i < m_pLevelLibrary->GetItemCount(); i++)
	{
		...
		{
			for (i32 slot = 0; slot < pMultiMtl->GetSubMaterialCount(); slot++)
			{
...
//////////////////////////////////////////////////////////////////////////
bool CMaterialManager::SelectSaveMaterial(string& itemName, tukk defaultStartPath)
{
	string startPath;
...
//////////////////////////////////////////////////////////////////////////
CMaterial* CMaterialManager::SelectNewMaterial(i32 nMtlFlags, tukk szStartPath)
{
	string path;
	if (szStartPath)
	{
...
void CMaterialManager::Command_Duplicate()
{
	...
	if (!pSrcMtl)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "material.duplicate_current called while no materials selected");
		return;
	}

	if (GetIEditorImpl()->IsSourceControlAvailable())
	{
		u32 attrib = pSrcMtl->GetFileAttributes();
...
//////////////////////////////////////////////////////////////////////////
CMaterial* CMaterialManager::DuplicateMaterial(tukk newName, CMaterial* pOriginal)
{
	if (!newName)
	{
...
void CMaterialManager::Command_Delete()
{
	...
	if (pMtl)
	{
		...
		str.Format(_T("Delete Material %s?\r\nNote: Material file %s will also be deleted."),
		           (tukk)pMtl->GetName(), (tukk)pMtl->GetFilename());
...
void CMaterialManager::Command_CreateTerrainLayer()
{
	...
	for (i32 i = 0, n = terrainManager->GetLayerCount(); i < n; ++i)
	{
...
void CMaterialManager::Command_AssignToSelection()
{
	...
	if (pMtl)
	{
		...
		if (pMtl->IsPureChild())
		{
			i32 nSelectionCount = pSel->GetCount();
...
void CMaterialManager::PickPreviewMaterial(HWND hWndCaller)
{
	...
	if (!pMtl->IsMultiSubMaterial())
	{
		...
		if (texturesNode)
		{
			for (i32 i = 0; i < texturesNode->getChildCount(); i++)
			{
				...
				if (texNode->getAttr("File", file))
				{
					...
					drx_strcat(sFullFilenameLC, PathUtil::GamePathToDrxPakPath(file.GetString()));
...
{
	i32 debugFlags = gViewportDebugPreferences.GetDebugFlags();

	i32 mask = GetHighlightMask();
...
//////////////////////////////////////////////////////////////////////////
void CMaterialManager::SetHighlightMask(i32 highlightMask)
{
...
void CMaterialManager::GatherResources(IMaterial* pMaterial, CUsedResources& resources)
{
	...
	i32 nSubMtlCount = pMaterial->GetSubMtlCount();
	if (nSubMtlCount > 0)
	{
		for (i32 i = 0; i < nSubMtlCount; i++)
		{
...
///////////////////////////////////////////////////////////////////////////
void CMaterialManager::GetHighlightColor(ColorF* color, float* intensity, i32 flags)
{
