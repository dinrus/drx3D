...
void SMaterialLayerResources::SetFromUI(tukk shaderName, bool bNoDraw, bool bFadeOut)
{
...
//////////////////////////////////////////////////////////////////////////
CMaterial::CMaterial(const string& name, i32 nFlags)
	: m_highlightFlags(0)
{
...
CMaterial::~CMaterial()
{
	...
	{
		for (i32 i = 0; i < m_subMaterials.size(); i++)
		{
			if (m_subMaterials[i])
			{
...
void CMaterial::RenameSubMaterial(CMaterial* pSubMaterial, const string& newName)
{
	...
	protected:
		...
		virtual tukk GetDescription() { return "Rename Sub-Material"; };
	...
	DRX_ASSERT(pSubMaterial && pSubMaterial->IsPureChild() && pSubMaterial->GetParent() == this);
...
void CMaterial::RenameSubMaterial(i32 subMtlSlot, const string& newName)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CMaterial::GetTextureFilenames(std::vector<string>& outFilenames) const
{
	for (i32 i = 0; i < EFTT_MAX; ++i)
	{
		...
		{
			stl::push_back_unique(outFilenames, PathUtil::GamePathToDrxPakPath(name.GetString()).c_str());
		}
		...
		{
			stl::push_back_unique(outFilenames, PathUtil::GamePathToDrxPakPath(dccFilename.GetString(), true).c_str());
		}
	}

	if (IsMultiSubMaterial())
	{
		for (i32 i = 0; i < GetSubMaterialCount(); ++i)
		{
...
//////////////////////////////////////////////////////////////////////////
i32 CMaterial::GetAnyTextureFilenames(std::vector<string>& outFilenames) const
{
	for (i32 i = 0; i < EFTT_MAX; ++i)
	{
...
i32 CMaterial::GetTextureDependencies(std::vector<SAssetDependencyInfo>& outFilenames) const
{
	for (i32 i = 0; i < EFTT_MAX; ++i)
	{
		string name = m_shaderResources->m_Textures[i].m_Name;
...
//////////////////////////////////////////////////////////////////////////
u32 CMaterial::GetFileAttributes()
{
	if (IsDummy())
	{
...
bool CMaterial::LoadShader()
{
	...
	if (newShaderItem.m_pShader && (newShaderItem.m_pShader->GetFlags() & EF_NOTFOUND) != 0)
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Failed to load shader \"%s\" in material \"%s\"", newShaderItem.m_pShader->GetName(), m_name);
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Failed to Load Shader %s", (tukk)m_shaderName);
		return false;
	}
...
bool CMaterial::LoadMaterialLayers()
{
	...
	if (m_shaderItem.m_pShader && m_shaderItem.m_pShaderResources)
	{
		...
		u32 nMaskGenBase = m_shaderItem.m_pShader->GetGenerationMask();
		SShaderGen* pShaderGenBase = m_shaderItem.m_pShader->GetGenerationParams();

		for (u32 l(0); l < MTL_LAYER_MAX_SLOTS; ++l)
		{
			...
			if (!pCurrLayer->m_shaderName.IsEmpty())
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Failed to load material layer shader \"%s\" in material \"%s\"", pCurrLayer->m_shaderName, m_pMatInfo->GetName());
					if (!pNewShader)
						continue;
				}
				...
				{
					for (i32 nLayerBit(0); nLayerBit < pShaderGenLayer->m_BitMask.size(); ++nLayerBit)
					{
						SShaderGenBit* pLayerBit = pShaderGenLayer->m_BitMask[nLayerBit];

						for (i32 nBaseBit(0); nBaseBit < pShaderGenBase->m_BitMask.size(); ++nBaseBit)
						{
...
void CMaterial::UpdateMatInfo()
{
	if (m_pMatInfo)
	{
		...
		if (IsMultiSubMaterial())
		{
			...
			for (u32 i = 0; i < m_subMaterials.size(); i++)
			{
				if (m_subMaterials[i])
...
//////////////////////////////////////////////////////////////////////////
u32 CMaterial::GetTexmapUsageMask() const
{
	i32 mask = 0;
	if (m_shaderItem.m_pShader)
	{
...
void CMaterial::Serialize(SerializeContext& ctx)
{
	...
	if (ctx.bLoading)
	{
		...
		if (!IsMultiSubMaterial())
		{
			...
			if (!pMtlTmpl)
			{
				...
				if (!(m_mtlFlags & MTL_64BIT_SHADERGENMASK))
				{
					m_nShaderGenMask = GetIEditorImpl()->GetRenderer()->EF_GetRemapedShaderMaskGen((tukk) m_shaderName, m_nShaderGenMask);
					m_mtlFlags |= MTL_64BIT_SHADERGENMASK;
				}

				if (node->getAttr("StringGenMask", m_pszShaderGenMask))
					m_nShaderGenMask = GetIEditorImpl()->GetRenderer()->EF_GetShaderGlobalMaskGenFromString((tukk) m_shaderName, (tukk)m_pszShaderGenMask, m_nShaderGenMask);   // get common mask gen
				else
				{
					// version doens't has string gen mask yet ? Remap flags if needed
					m_nShaderGenMask = GetIEditorImpl()->GetRenderer()->EF_GetRemapedShaderMaskGen((tukk) m_shaderName, m_nShaderGenMask, ((m_mtlFlags & MTL_64BIT_SHADERGENMASK) != 0));
				}
				m_mtlFlags |= MTL_64BIT_SHADERGENMASK;
		...
		if (pMtlTmpl)
		{
			for (i32 n = 0; n < MTL_LAYER_MAX_SLOTS; n++)
			{
		...
		else
		{
			...
			if (mtlLayersNode)
			{
				i32 nChildCount = min((i32) MTL_LAYER_MAX_SLOTS, (i32)  mtlLayersNode->getChildCount());
				for (i32 l = 0; l < nChildCount; ++l)
				{
	...
	{
		i32 extFlags = MTL_64BIT_SHADERGENMASK;
		{
			const string& name = GetName();
			const size_t len = name.GetLength();
			if (len > 4 && strstri(((tukk)name) + (len - 4), "_con"))
		...
		if (!IsMultiSubMaterial())
		{
			...
			m_pszShaderGenMask = GetIEditorImpl()->GetRenderer()->EF_GetStringFromShaderGlobalMaskGen((tukk)m_shaderName, m_nShaderGenMask);
...
/*
   ...
   else
   {
    ...
    for (i32 i = 0; i < m_shaderParams.size(); i++)
    {
      ...
      {
      case eType_BYTE:
        paramNode->setAttr( "Value",(i32)pParam->m_Value.m_Byte );
        paramNode->setAttr( "Type",(i32)pParam->m_Value.m_Byte );
        break;
      case eType_SHORT:
        paramNode->setAttr( "Value",(i32)pParam->m_Value.m_Short );
        break;
      case eType_INT:
        paramNode->setAttr( "Value",(i32)pParam->m_Value.m_Int );
        break;
      case eType_FLOAT:
...
void CMaterial::SetFromMatInfo(IMaterial* pMatInfo)
{
	...
	// Material link names
	if (tukk szLinkName = pMatInfo->GetMaterialLinkName())
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CMaterial::GetSubMaterialCount() const
{
	return m_subMaterials.size();
}

//////////////////////////////////////////////////////////////////////////
void CMaterial::SetSubMaterialCount(i32 nSubMtlsCount)
{
	//Remove parenting link from severed submaterials
	i32k currentCount = m_subMaterials.size();
	...
	if (nSubMtlsCount < currentCount)
	{
		for (i32 i = nSubMtlsCount; i < currentCount; i++)
		{
			if (m_subMaterials[i])
			{
				m_subMaterials[i]->m_pParent = NULL;
...
//////////////////////////////////////////////////////////////////////////
CMaterial* CMaterial::GetSubMaterial(i32 index) const
{
	i32k nSubMats = m_subMaterials.size();
...
//////////////////////////////////////////////////////////////////////////
void CMaterial::SetSubMaterial(i32 nSlot, CMaterial* mtl)
{
...
void CMaterial::ResetSubMaterial(i32 nSlot, bool bOnlyIfEmpty)
{
...
void CMaterial::RemoveSubMaterial(i32 nSlot)
{
...
CVarBlock* CMaterial::UpdateTextureNames(CSmartVariableArray textureVars[EFTT_MAX])
{
	...
	IShader* pTemplShader = m_shaderItem.m_pShader;
	i32 nTech = max(0, m_shaderItem.m_nTechnique);
...
i32 CMaterial::FindSubMaterial(CMaterial* pMaterial)
{
	DRX_ASSERT(pMaterial && pMaterial->GetParent() == this);
	i32 index = -1;
...
void CMaterial::Validate()
{
	if (IsDummy())
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Material %s file not found", (tukk)GetName());
	}
	// Reload shader.
	LoadShader();
...
void CMaterial::GatherUsedResources(CUsedResources& resources)
{
	...
	for (i32 texid = 0; texid < EFTT_MAX; texid++)
	{
...
bool CMaterial::Save(bool bSkipReadOnly)
{
	...
	//filename = gEnv->pDrxPak->AdjustFileName( filename,path,0 );
...
void CMaterial::NotifyChanged()
{
	...
	if (!CanModify() && !IsModified() && CUndo::IsRecording())
	{
		...
		Warning("Modifying read only material %s\r\nChanges will not be saved!", (tukk)GetName());
	}
...
{
public:
	CUndoMaterial(CMaterial* pMaterial, tukk undoDescription, bool bForceUpdate)
	{
		assert(pMaterial);
		...
		if (m_bIsSubMaterial)
		{
			...
			{
				bool bFound = false;
				i32k subMaterialCount = pParentMaterial->GetSubMaterialCount();
				for (i32 i = 0; i < subMaterialCount; ++i)
				{
...
protected:
	...
	virtual tukk GetDescription() { return m_undoDescription; };
...
private:
	CMaterial* GetMaterial()
	{
		...
		{
			bool bFound = false;
			i32k subMaterialCount = pMaterial->GetSubMaterialCount();
			for (i32 i = 0; i < subMaterialCount; ++i)
			{
...
//////////////////////////////////////////////////////////////////////////
void CMaterial::RecordUndo(tukk sText, bool bForceUpdate)
{
...
//////////////////////////////////////////////////////////////////////////
void CMaterial::SetHighlightFlags(i32 highlightFlags)
{
