...
class SANDBOX_API CDrxEditDoc : public _i_reference_target_t
{
public:
	CDrxEditDoc();
	virtual ~CDrxEditDoc();
	...
	void               InitEmptyLevel(i32 resolution = 0, float unitSize = 0, bool bUseTerrain = false);
	...
	void               SetLastLoadedLevelName(tukk lastLoadedFileName);
	...
	i32                GetMissionCount() const     { return m_missions.size(); };
	//! Get Mission by index.
	CMission*          GetMission(i32 index) const { return m_missions[index]; };
...
protected:
	...
	string      GetDrxIndexPath(const LPCTSTR levelFilePath);
