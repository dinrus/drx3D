...
public:
	...
	static CDrxEditApp*          GetInstance();
	...
	ECreateLevelResult           CreateLevel(const string& levelName, i32 resolution, float unitSize, bool bUseTerrain);
	...
	CDrxEditDoc*                 LoadLevel(tukk lpszFileName);
...
private:
	...
	void TagLocation(i32 index);
	void GotoTagLocation(i32 index);
	void LoadTagLocations();
	bool UserExportToGame(bool bNoMsgBox = true);
	bool DoExportToGame(u32 flags, const SGameExporterSettings& exportSettings, bool showMsgBox = false);
