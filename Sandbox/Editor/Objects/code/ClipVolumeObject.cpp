...
i32 CClipVolumeObject::s_nEditClipVolumeRollUpID = 0;
i32 CClipVolumeObject::s_nGlobalFileClipVolumeID = 0;
...
bool CClipVolumeObject::CreateGameObject()
{
	...
	if (m_pEntity)
	{
		...
		spawnParams.pUserData = (uk )gameFileName.GetString();
...
void CClipVolumeObject::Display(DisplayContext& dc)
{
	if (IStatObj* pStatObj = GetIStatObj())
	{
		...
		dd.tm = GetWorldTM();
		dd.bExtrude = false;
		dd.color = ColorB((u32)solidColor);
		dd.lineColor = ColorB((u32)lineColor);

		const float fAlpha = (IsSelected() || mv_filled) ? 0.3f : 0;
		dd.color.a = u8(fAlpha * 255);
		dd.lineColor.a = 255;

		if (IsSelected())
		{
			dd.color.a = u8(0.15f * 255);
			dd.lineColor.a = u8(0.15f * 255);
			;

			dc.DepthTestOff();
			pStatObj->DebugDraw(dd);

			dd.color.a = u8(0.25f * 255);
			dd.lineColor.a = u8(1.0f * 255);
			;
...
void CClipVolumeObject::UpdateCollisionData()
{
	...
	if (IStatObj* pStatObj = GetIStatObj())
	{
		if (IRenderMesh* pRenderMesh = pStatObj->GetRenderMesh())
		{
			...
			{
				i32 posStride;
				u8k* pPositions = (u8*)pRenderMesh->GetPosPtr(posStride, FSL_READ);
...
bool CClipVolumeObject::HitTest(HitContext& hc)
{
	...
	if (Intersect::Ray_AABB(raySrc, rayDir, bboxLS, vHitPos))
	{
		...
		else if (hc.camera)
		{
			...
			for (i32 e = 0; e < m_MeshEdges.size(); ++e)
			{
...
void CClipVolumeObject::BeginEditParams(IEditor* ie, i32 flags)
{
