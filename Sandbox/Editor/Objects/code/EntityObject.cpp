...
#include <DrxAISystem/IAIObject.h>
#include <DrxAISystem/IAIActor.h>
...
#include <Drx3DEngine/IGeomCache.h>
...
#include <DrxSchematyc/IObjectProperties.h>
#include <DrxSchematyc/Utils/ClassProperties.h>
...
tukk CEntityObject::s_LensFlarePropertyName("flare_Flare");
tukk CEntityObject::s_LensFlareMaterialName("%ENGINE%/EngineAssets/Materials/lens_optics");
...
private:
	...
	virtual tukk GetDescription() { return "Attachment Changed"; }

	DrxGUID                        m_attachedEntityGUID;
...
public:
	...
	{
		const DrxGUID& componentInstanceGUID = AddComponentInternal(pEntity);
...
private:
	const DrxGUID& AddComponentInternal(IEntity* pEntity)
	{
		EntityComponentFlags flags = m_pClassDescription->GetComponentFlags();
		flags.Add(EEntityComponentFlags::UserAdded);
		IEntityComponent::SInitParams initParams(pEntity, DrxGUID::Create(), "", m_pClassDescription, flags, nullptr, nullptr);
		...
		static DrxGUID invalidGUID = DrxGUID::Null();
		return invalidGUID;
	}

	virtual tukk GetDescription() { return "Added Entity Component"; }
...
public:
	...
	virtual void SetUserData(tukk key, uk userData) override
	{
...
protected:
	...
	virtual tukk GetDescription() { return "Entity Link"; }
	virtual tukk GetObjectName()  { return ""; }

	virtual void        Undo(bool bUndo)
	{
		for (i32 i = 0, iLinkSize(m_Links.size()); i < iLinkSize; ++i)
		{
...
{
public:
	CUndoCreateFlowGraph(CEntityObject* entity, tukk groupName)
...
protected:
	...
	virtual tukk GetDescription() override { return "Create Flow Graph"; }
...
i32 CEntityObject::m_rollupId = 0;
...
bool CEntityObject::Init(CBaseObject* pPrev, const string& file)
{
	...
	{
		i32 startIdx = file.ReverseFind('/') + 1;
		i32 endIdx = file.ReverseFind('.') - startIdx;
		if (endIdx <= 0)
...
{
	enum class EComponentDictionaryColumn : i32
	{
		Name = 0,
		Count
	};
	...
	public:
		CComponentDictionaryEntry(tukk szName, const Schematyc::IEnvComponent& component, QString tooltip, tukk szIcon, bool bEnabled, const CAbstractDictionaryEntry* pParentEntry = nullptr)
			: m_name(szName)
		...
		{
			if (columnIndex == (u32)EComponentDictionaryColumn::Name)
			{
				return m_name;
			}
			return QVariant();
		...
		virtual const QIcon*                    GetColumnIcon(i32 columnIndex) const override { return &m_icon; }
	...
	public:
		CComponentDictionaryCategoryEntry(tukk szName, CComponentDictionaryCategoryEntry* pParent = nullptr)
			: m_name(szName)
			, m_pParentEntry(pParent) {}
		virtual ~CComponentDictionaryCategoryEntry() {}

		// CAbstractDictionaryEntry
		virtual u32   GetType() const override { return CAbstractDictionaryEntry::Type_Folder; }
		virtual QVariant GetColumnValue(i32 columnIndex) const override
		{
			if (columnIndex == (u32)EComponentDictionaryColumn::Name)
			{
		...
		virtual i32                           GetNumChildEntries() const override       { return m_entries.size(); }
		virtual const CAbstractDictionaryEntry* GetChildEntry(i32 index) const override { return m_entries[index].get(); }
		...
		CComponentDictionaryCategoryEntry& AddCategoryEntry(tukk szName)
		{
	...
	public:
		...
		virtual i32                           GetNumEntries() const override       { return m_entries.size(); }
		virtual const CAbstractDictionaryEntry* GetEntry(i32 index) const override { return (m_entries.size() > index) ? m_entries[index].get() : nullptr; }

		virtual i32                           GetNumColumns() const override       { return (u32)EComponentDictionaryColumn::Count; };
		virtual QString                         GetColumnName(i32 index) const override
		{
			if (index == (u32)EComponentDictionaryColumn::Name)
				return QString("Name");
			return QString();
		}

		virtual i32 GetDefaultFilterColumn() const override { return (u32)EComponentDictionaryColumn::Name; }
		// ~DrxGraphEditor::CAbstractDictionary
	...
	for (i32 i = 0, n = pSelectionGroup->GetCount(); i < n; ++i)
	{
		...
		DRX_ASSERT(pSelectedObjectClass == RUNTIME_CLASS(CEntityObject) || pSelectedObjectClass->IsDerivedFrom(RUNTIME_CLASS(CEntityObject)));
	...
	for (const std::pair<tukk , std::vector<const Schematyc::IEnvComponent*>>& componentCategoryPair : componentsMap)
	{
		...
		for (const Schematyc::IEnvComponent* pComponent : componentCategoryPair.second)
		{
			...
			for (const CEntityComponentClassDesc* pComponentClassDesc : existingComponentClassDescs)
			{
				...
				{
					tukk szIncompatibleComponentName = pComponentClassDesc->GetLabel();
			...
			tukk szName = pComponent->GetDesc().GetLabel();
			if (*szName == '\0')
			{
...
void CreateComponentWidget(CEntityObject* pEntityObject, DrxGUID componentTypeGUID, size_t typeInstanceIndex, Serialization::IArchive& archive)
{
...
void CEntityObject::CreateComponentWidgets(CInspectorWidgetCreator& creator)
{
	...
	{
		QPushButton* pAddComponentButton = new QPushButton(DrxIcon("icons:General/Plus.ico"), "Add Component");
	...
	std::unordered_map<DrxGUID, i32> componentIndexMap;
	...
	for (IEntityComponent* pComponent : components)
	{
		...
		tukk szComponentLabel = pComponent->GetName();
		...
		DrxGUID componentClassGUID = pComponent->GetClassDesc().GetGUID();
		if (componentClassGUID.IsNull())
		{
			...
			if (IDrxFactory* pFactory = pComponent->GetFactory())
			{
		...
		DrxGUID componentInstanceGUID = pComponent->GetGUID();
...
void CEntityObject::GetLocalBounds(AABB& box)
{
	...
	if (m_pEntity != nullptr)
	{
		for (i32 i = 0, n = m_pEntity->GetSlotCount(); i < n; ++i)
		{
...
bool CEntityObject::HitTestCharacter(ICharacterInstance* pCharacter, HitContext& hc, SRayHitInfo& hitInfo, bool& bHavePhysics)
{
	...
	{
		for (i32 iAttachment = 0, numAttachments = pAttachmentManager->GetAttachmentCount(); iAttachment < numAttachments; ++iAttachment)
		{
	    ...
	    && pCharacterPhysicalEntity->GetStatus(&pe_status_nparts()) > 0)
	{
		...
		ray_hit hit;
		i32 numHits = gEnv->pPhysicalWorld->RayTraceEntity(pCharacterPhysicalEntity, hc.raySrc, hc.rayDir * 10000.0f, &hit);
		if (numHits > 0)
		{
			hc.dist = hit.dist;
			hc.object = this;
...
bool CEntityObject::HitTestEntity(HitContext& hc, bool& bHavePhysics)
{
	    ...
	    && pPhysicalEntity->GetType() != PE_SOFT)
	{
		...
		ray_hit hit;
		i32 numHits = gEnv->pPhysicalWorld->RayTraceEntity(pPhysicalEntity, hc.raySrc, hc.rayDir * 10000.0f, &hit);
		if (numHits > 0)
		{
			hc.dist = hit.dist;
			hc.object = this;
...
bool CEntityObject::HitTest(HitContext& hc)
{
	if (!hc.b2DViewport)
	{
		...
		if (m_visualObject && !gViewportPreferences.showIcons && !gViewportPreferences.showSizeBasedIcons)
		{
			...
			i32 col = (m_visualObject->GetPhysGeom() && m_visualObject->GetPhysGeom()->pGeom)
...
bool CEntityObject::SetClass(const string& entityClass, bool bForceReload, XmlNodeRef xmlEntityNode)
{
	...
	{
		m_bLoadFailed = true;
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Entity creation failed, class %s was not registered with the engine!", m_entityClass.c_str());
...
void CEntityObject::ResetEditorClassInfo()
{
	...
	i32 iconId = 0;
	if (gEnv->pDrxPak->IsFileExist(classIcon))
	{
...
bool CEntityObject::SetEntityScript(std::shared_ptr<CEntityScript> pEntityScript, bool bForceReload, XmlNodeRef xmlEntityNode)
{
	...
	if (!m_pEntityScript->IsValid())
	{
		...
		{
			m_bLoadFailed = true;
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Failed to load entity script for Entity %s(Script: %s) %s",
			           (tukk)GetName(),
			           (tukk)m_entityClass,
			           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
	...
	for (i32 i = 0, num = m_pEntityScript->GetEmptyLinkCount(); i < num; i++)
	{
		const string& linkName = m_pEntityScript->GetEmptyLink(i);
		i32 j = 0;
		i32 numlinks = (i32)m_links.size();
		...
		if (j >= numlinks)
		{
			AddEntityLink(linkName, DrxGUID::Null());
		}
	}
...
IVariable* CEntityObject::FindVariableInSubBlock(CVarBlockPtr& properties, IVariable* pSubBlockVar, tukk pVarName)
{
...
void CEntityObject::SpawnEntity()
{
	...
	if (IsLegacyObject())
	{
		...
		params.nFlagsExtended = (params.nFlagsExtended & ~ENTITY_FLAG_EXTENDED_GI_MODE_BIT_MASK) | ((((i32)mv_giMode) << ENTITY_FLAG_EXTENDED_GI_MODE_BIT_OFFSET) & ENTITY_FLAG_EXTENDED_GI_MODE_BIT_MASK);
	}
	...
	if (m_pEntity)
	{
		...
		if (CBaseObject* pLinkedToObject = GetLinkedTo())
		{
			if (pLinkedToObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
			{
				...
				DRX_ASSERT(params.pParent != nullptr);
			}
		}
		...
		if (!pEntitySystem->InitEntity(m_pEntity, params))
		{
			...
			m_bLoadFailed = true;
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Failed to initialize Entity %s(Script: %s) %s",
			           (tukk)GetName(),
			           (tukk)m_entityClass,
			           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
	...
	{
		m_bLoadFailed = true;
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Entity %s Failed to Spawn (Script: %s) %s",
		           (tukk)GetName(),
		           (tukk)m_entityClass,
		           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
	}
...
{
	using namespace Private_EntityObject;
	DRX_ASSERT_MESSAGE(!GetIEditorImpl()->GetObjectManager()->IsLoading(), "Entities should not be spawned and unloaded during loading");
...
void CEntityObject::SetName(const string& name)
{
	...
	if (m_pEntity)
	{
		m_pEntity->SetName((tukk)GetName());
	}
...
void CEntityObject::SetSelected(bool bSelect)
{
	...
	if (m_pEntity)
	{
		//m_pEntity->SetFlags()
		{
			...
			if (pRenderNode)
			{
				i32 flags = pRenderNode->GetRndFlags();
				if (bSelect)
				{
					flags |= ERF_SELECTED;
				}
...
template<> struct IVariableType<i32>
{
...
void CEntityObject::Serialize(CObjectArchive& ar)
{
	...
	if (ar.bLoading)
	{
		...
		Vec3 angles;
		// Backward compatibility, with FarDrx levels.
		...
		if (eventTargets)
		{
			for (i32 i = 0; i < eventTargets->getChildCount(); i++)
			{
				XmlNodeRef eventTarget = eventTargets->getChild(i);
				CEntityEventTarget et;
				et.target = 0;
				DrxGUID targetId = DrxGUID::Null();
	...
	else
	{
		...
		if (!m_eventTargets.empty())
		{
			...
			for (i32 i = 0; i < m_eventTargets.size(); i++)
			{
...
void CEntityObject::CheckSpecConfig()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "AI entity %s ->> spec dependent", (tukk)GetName());
	}
}
...
XmlNodeRef CEntityObject::Export(const string& levelPath, XmlNodeRef& xmlExportNode)
{
	...
	if (IsLegacyObject())
	{
		if (mv_ratioLOD != 100)
		{
			objNode->setAttr("LodRatio", (i32)mv_ratioLOD);
		}

		if (mv_ratioViewDist != 100)
		{
			objNode->setAttr("ViewDistRatio", (i32)mv_ratioViewDist);
		}
	...
	if (GetMinSpec() != 0)
	{
		objNode->setAttr("MinSpec", (u32)GetMinSpec());
	}

	u32 nMtlLayersMask = GetMaterialLayersMask();
	if (nMtlLayersMask != 0)
	{
...
void CEntityObject::DrawTextureIcon(DisplayContext& dc, const Vec3& pos, float alpha, bool bDisplaySelectionHelper, float distanceSquared)
{
	...
	i32 mainIconSize = OBJECT_TEXTURE_ICON_SIZE;
	i32 componentIconSize = i32(OBJECT_TEXTURE_ICON_SIZE * 0.75f);

	constexpr i32 iconDistX = 2;
	constexpr i32 iconDistY = 5;
	...
	if (m_pEntity != nullptr)
	{
		...
		constexpr i32 maxColumnCount = 5;

		Vec3 iconPos = GetTextureIconDrawPos();

		bDrewComponentIcon = m_componentIconTextureIds.size() > 0;

		for (i32 i = 0, n = m_componentIconTextureIds.size(); i < n; ++i)
		{
			i32 rowIndex = i / maxColumnCount;
			i32 columnIndex = i - rowIndex * maxColumnCount;

			i32 lastRowColumnCount = n % maxColumnCount;
			i32 numColumnItems = i < n - lastRowColumnCount ? maxColumnCount : lastRowColumnCount;
...
void CEntityObject::RenewTextureIconsBuffer()
{
	...
	{
		tukk szIcon = pComponent->GetClassDesc().GetIcon();

		if (szIcon != nullptr && szIcon[0] != '\0')
		{
			DrxIcon icon(szIcon);
			i32 iconId = GetIEditor()->GetIconManager()->GetIconTexture(szIcon, icon);
...
IVariable* CEntityObject::GetLightVariable(tukk name0) const
{
	if (m_pLuaProperties2)
	{
		...
		if (pLightProperties)
		{
			for (i32 i = 0; i < pLightProperties->GetNumVariables(); ++i)
			{
...
string CEntityObject::GetLightAnimation() const
{
	...
	if (pStyleGroup)
	{
		for (i32 i = 0; i < pStyleGroup->GetNumVariables(); ++i)
		{
...
void CEntityObject::InvalidateTM(i32 nWhyFlags)
{
...
void CEntityObject::OnLink(CBaseObject* pParent)
{
	...
	if (pLinkedTo && pLinkedTo->IsKindOf(RUNTIME_CLASS(CEntityObject)))
	{
		...
		{
			XFormGameEntity();
			i32k flags = ((m_attachmentType == eAT_GeomCacheNode) ? IEntity::ATTACHMENT_GEOMCACHENODE : 0)
			                  | ((m_attachmentType == eAT_CharacterBone) ? IEntity::ATTACHMENT_CHARACTERBONE : 0);
...
void CEntityObject::BindIEntityChilds()
{
	...
	i32 numChilds = GetChildCount();
	for (i32 i = 0; i < numChilds; i++)
	{
		...
		if (pChild && pChild->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			...
			if (ientChild)
			{
				...
				i32k flags = ((pChildEntity->m_attachmentType == eAT_GeomCacheNode) ? IEntity::ATTACHMENT_GEOMCACHENODE : 0)
...
void CEntityObject::PostClone(CBaseObject* pFromObject, CObjectCloneContext& ctx)
{
	...
	{
		i32 numTargets = pFromEntity->m_eventTargets.size();
		for (i32 i = 0; i < numTargets; i++)
		{
	...
	{
		i32 numTargets = pFromEntity->m_links.size();
		for (i32 i = 0; i < numTargets; i++)
		{
			...
			{
				AddEntityLink(et.name.c_str(), DrxGUID::Null());
			}
		}
	}
...
void CEntityObject::ResolveEventTarget(CBaseObject* object, u32 index)
{
	// Find target id.
	...
	if (object)
	{
		if (!object->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ASSERT, "Attempting to link Object %s which is not an EntityObject to entity %s", object->GetName(), GetName());
		}
...
void CEntityObject::LoadLink(XmlNodeRef xmlNode, CObjectArchive* pArchive)
{
	...
	for (i32 i = 0; i < xmlNode->getChildCount(); i++)
	{
		...
		if (linkNode->getAttr("TargetId", targetId))
		{
			...
			DrxGUID newTargetId = pArchive ? pArchive->ResolveID(targetId) : targetId;
...
void CEntityObject::SaveLink(XmlNodeRef xmlNode)
{
	...
	for (i32 i = 0, num = m_links.size(); i < num; i++)
	{
		if (m_links[i].targetId == DrxGUID::Null())
			continue;
...
void CEntityObject::OnEventTargetEvent(CBaseObject* target, i32 event)
{
	// When event target is deleted.
	if (event == OBJECT_ON_DELETE)
	{
		// Find this target in events list and remove.
		i32 numTargets = m_eventTargets.size();
		for (i32 i = 0; i < numTargets; i++)
		{
...
i32 CEntityObject::AddEventTarget(CBaseObject* target, const string& event, const string& sourceEvent, bool bUpdateScript)
{
...
void CEntityObject::RemoveEventTarget(i32 index, bool bUpdateScript)
{
...
i32 CEntityObject::AddEntityLink(const string& name, DrxGUID targetEntityId)
{
	CEntityObject* target = 0;
	if (targetEntityId != DrxGUID::Null())
	{
		...
		if (pObject && pObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ASSERT, "Attempting to link Object %s with itself", m_pEntity->GetName());
				return -1;
			}
		}
		else
		{
			if (pObject)
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ASSERT, "Attempting to link Object %s which is not an EntityObject to entity %s", pObject->GetName(), GetName());
			}
			return -1;
		}
	}
...
bool CEntityObject::EntityLinkExists(const string& name, DrxGUID targetEntityId)
{
	for (i32 i = 0, num = m_links.size(); i < num; ++i)
	{
...
void CEntityObject::RemoveEntityLink(i32 index)
{
...
void CEntityObject::RenameEntityLink(i32 index, const string& newName)
{
	if (index >= 0 && index < m_links.size())
	{
...
void CEntityObject::OnEntityFlagsChange(IVariable* var)
{
	if (m_pEntity)
	{
		i32 flags = m_pEntity->GetFlags();
		i32 flagsEx = m_pEntity->GetFlagsExtended();
...
void CEntityObject::SetMaterialLayersMask(u32 nLayersMask)
{
	__super::SetMaterialLayersMask(nLayersMask);
	UpdateMaterialInfo();
}

void CEntityObject::SetMinSpec(u32 nMinSpec, bool bSetChildren)
{
...
void CEntityObject::Validate()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Entity %s Failed to Spawn (Script: %s) %s", (tukk)GetName(), (tukk)m_entityClass,
		           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
		return;
	}
	...
	// Check Entity.
	i32 numObj = m_pEntity->GetSlotCount();
	for (slot = 0; slot < numObj; slot++)
	{
		...
		{
			// File Not found.
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Geometry File in Slot %d for Entity %s not found %s", slot, (tukk)GetName(),
			           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
		}
	}
}
...
string CEntityObject::GetAssetPath() const
{
	...
	else if (m_pEntity)
	{
		...
		{
			if (tukk szFileName = pObject->GetScriptFile())
			{
				return szFileName;
			}
		}
...
void CEntityObject::OnMenuScriptEvent(i32 eventIndex)
{
...
void CEntityObject::OnMenuConvertToPrefab()
{
	...
	if (pLibrary == NULL)
	{
		...
		DrxWarning(VALIDATOR_MODULE_ENTITYSYSTEM, VALIDATOR_ERROR, sError);
		return;
	}
	...
	i32 nIdx = 0;
	i32 nLen = sLibraryName.GetLength();
	i32 nLen2 = strFullName.GetLength();
	...
	tukk szItemName = strFullName.GetString();
...
void CEntityObject::OnContextMenu(CPopupMenuItem* pMenu)
{
	...
	if (!flowgraphs.empty())
	{
		...
		for (iter = flowgraphs.begin(), end = flowgraphs.end(); iter != end; ++iter)
		{
			...
			if (*iter == pEntityFG)
			{
				...
				fgMenu.AddCommand(name, CommandString("flowgraph.open_view_and_select", (*iter)->GetName(), (tukk)GetName()));
	...
	if (pScript && pScript->GetEventCount() > 0)
	{
		...
		i32 eventCount = pScript->GetEventCount();
...
SRenderLight* CEntityObject::GetLightProperty() const
{
	...
	if (pLightEntities == NULL)
		return NULL;
	for (i32 i = 0, iLightSize(pLightEntities->Count()); i < iLightSize; ++i)
	{
...
void CEntityObject::ResetCallbacks()
{
	...
	if (m_pLuaProperties)
	{
		...
		if (pColorGroup)
		{
			i32k nChildCount = pColorGroup->GetNumVariables();
			for (i32 i = 0; i < nChildCount; ++i)
			{
...
IStatObj* CEntityObject::GetIStatObj()
{
	...
	for (i32 i = 0, iSlotCount(m_pEntity->GetSlotCount()); i < iSlotCount; ++i)
	{
...
template<typename T>
T CEntityObject::GetEntityProperty(tukk pName, T defaultvalue) const
{
...
template<typename T>
void CEntityObject::SetEntityProperty(tukk pName, T value)
{
...
bool CEntityObject::GetEntityPropertyBool(tukk name) const
{
	return GetEntityProperty<bool>(name, false);
}

i32 CEntityObject::GetEntityPropertyInteger(tukk name) const
{
	return GetEntityProperty<i32>(name, 0);
}

float CEntityObject::GetEntityPropertyFloat(tukk name) const
{
...
string CEntityObject::GetMouseOverStatisticsText() const
{
	if (m_pEntity)
	{
		...
		u32k slotCount = m_pEntity->GetSlotCount();
		for (u32 slot = 0; slot < slotCount; ++slot)
		{
			...
			if (pCharacter)
			{
				...
				{
					i32k count = pAttachmentManager->GetAttachmentCount();
					for (i32 i = 0; i < count; ++i)
					{
						if (IAttachment* pAttachment = pAttachmentManager->GetInterfaceByIndex(i))
						{
							...
							if (pAttachmentSkin)
							{
								...
								if (pSkin)
								{
									u32k numLods = pSkin->GetNumLODs();
									tukk szFilename = PathUtil::GetFile(pSkin->GetModelFilePath());

									statsText += "\n  " + string(szFilename);
									for (i32 j = 0; j < numLods; ++j)
									{
										IRenderMesh* pRenderMesh = pSkin->GetIRenderMesh(j);
										if (pRenderMesh)
										{
											i32k vertCount = pRenderMesh->GetVerticesCount();
											statsText += ", LOD" + FormatWithThousandsSeperator(j) + ": " + FormatWithThousandsSeperator(vertCount) + " Vertices";
										}
									}

									u32k vertexFrameCount = pSkin->GetVertexFrames()->GetCount();
...
void CEntityObject::OnBnClickedRemoveFlowGraph()
{
	if (GetFlowGraph())
	{
		...
		string str;
		str.Format("Remove Flow Graph for Entity %s?", (tukk)GetName());
...
string CEntityObject::GetEntityPropertyString(tukk name) const
{
	return GetEntityProperty<string>(name, "");
}

void CEntityObject::SetEntityPropertyBool(tukk name, bool value)
{
	SetEntityProperty<bool>(name, value);
}

void CEntityObject::SetEntityPropertyInteger(tukk name, i32 value)
{
	SetEntityProperty<i32>(name, value);
}

void CEntityObject::SetEntityPropertyFloat(tukk name, float value)
{
	SetEntityProperty<float>(name, value);
}

void CEntityObject::SetEntityPropertyString(tukk name, const string& value)
{
	SetEntityProperty<string>(name, value);
}

SPyWrappedProperty CEntityObject::PyGetEntityProperty(tukk pName) const
{
	IVariable* pVariable = NULL;
	if (m_pLuaProperties2)
	{
	...
	else if (pVariable->GetType() == IVariable::VECTOR)
	{
		...
		if (pVariable->GetDataType() == IVariable::DT_COLOR)
		{
			...
			COLORREF col = CMFCUtils::ColorLinearToGamma(ColorF(
			                                               ...
			                                               tempVec3[1],
			                                               tempVec3[2]));
			value.property.colorValue.r = (i32)GetRValue(col);
			value.property.colorValue.g = (i32)GetGValue(col);
			value.property.colorValue.b = (i32)GetBValue(col);
		}
		else
		{
...
void CEntityObject::PySetEntityProperty(tukk pName, const SPyWrappedProperty& value)
{
	IVariable* pVariable = NULL;
	if (m_pLuaProperties2)
	{
...
SPyWrappedProperty PyGetEntityProperty(tukk pObjName, tukk pPropName)
{
	CBaseObject* pObject;
	if (GetIEditorImpl()->GetObjectManager()->FindObject(pObjName))
		pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pObjName);
	else if (GetIEditorImpl()->GetObjectManager()->FindObject(DrxGUID::FromString(pObjName)))
		pObject = GetIEditorImpl()->GetObjectManager()->FindObject(DrxGUID::FromString(pObjName));
	else
	{
...
void PySetEntityProperty(tukk entityName, tukk propName, SPyWrappedProperty value)
{
...
SPyWrappedProperty PyGetEntityParam(tukk pObjName, tukk pVarPath)
{
...
void PySetEntityParam(tukk pObjectName, tukk pVarPath, SPyWrappedProperty value)
{
	string varPath = pVarPath;
	std::deque<string> splittedPath;
	i32 currentPos = 0;
...
static void PyOpenEntityArchetype(tukk archetype)
{
...
REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE(PySetEntityParam, general, set_entity_param,
                                          ...
                                          "general.set_entity_param(str entityName, str paramName, [ bool || i32 || float || str || (float, float, float) ] paramValue)");
...
REGISTER_ONLY_PYTHON_COMMAND_WITH_EXAMPLE(PySetEntityProperty, general, set_entity_property,
                                          ...
                                          "general.set_entity_property(str entityName, str propertyName, [ bool || i32 || float || str || (float, float, float) ] propertyValue)");
