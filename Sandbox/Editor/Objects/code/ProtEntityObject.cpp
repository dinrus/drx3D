...
bool CProtEntityObject::Init(CBaseObject* prev, const string& file)
{
	...
	{
		SetPrototype(DrxGUID::FromString(file));
...
//////////////////////////////////////////////////////////////////////////
void CProtEntityObject::SetPrototype(DrxGUID guid, bool bForceReload)
{
	...
	if (!prototype)
	{
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Cannot find Entity Archetype: %s for Entity %s %s", guid.ToString(), (tukk)GetName(),
		           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName().GetString()));
		return;
	}
...
SPreviewDesc CProtEntityObjectClassDesc::GetPreviewDesc(tukk id) const
{
	...
	const auto guid = DrxGUID::FromString(id);
