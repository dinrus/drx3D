...
void CConstraintEntity::Display(CObjectRenderHelper& objRenderHelper)
{
	...
	i32k nPreAllocatedListSize(1024);                            //Magic number, probably big enough for the list.
	...
	i32 objtypes = ent_static | ent_rigid | ent_sleeping_rigid | ent_sort_by_mass | ent_allocate_list;
	i32 nEnts = pWorld->GetEntitiesInBox(center - radVec, center + radVec, pEnts, objtypes, nPreAllocatedListSize);
	...
	if (maxLimit > 0 && minLimit < maxLimit)
	{
		...
		for (i32 i = 0; i < divs; i++, angle += delta)
		{
...
{
	for (i32 i = 0; i < pObj->GetSubObjectCount(); i++)
	{
			...
			case GEOM_BOX:
				{
					...
					for (i32 j = 0, mask0 = 2, mask1, bit = 0; j < 6; j++, mask0 = mask1, bit = incm3(bit))
					{
						mask1 = mask0 ^ 1 << bit;
...
void CJointGenEntity::Display(CObjectRenderHelper& objRenderHelper)
{
	...
	if (props->GetValue("object_BreakTemplate", str))
	{
		...
		i32 ncont = (i32)gEnv->pPhysicalWorld->PrimitiveWorldIntersection(pp);
		for (i32 i = 0; i < ncont; i++)
		{
			...
			for (i32 j = 0; j < max(1, pObj->GetSubObjectCount()); j++)
			{
