...
#include <DrxNetwork/IRemoteCommand.h>
...
void FindSelectableObjectsInRectRecursive(CObjectManager* manager, CViewport* view, CBaseObject* obj, HitContext& hc, std::vector<CBaseObject*>& selectableObjects)
{
	if (obj->IsKindOf(RUNTIME_CLASS(CGroup)))
	{
		...
		if (pGroup->IsOpen())
		{
			for (i32 j = 0; j < pGroup->GetChildCount(); ++j)
			{
...
public:
	...
	tukk    ClassName()       { return type; };
	tukk    Category()        { return category; };
	CRuntimeClass* GetRuntimeClass() { return superType->GetRuntimeClass(); };
	tukk    GetTextureIcon()  { return superType->GetTextureIcon(); };
	tukk    GetFileSpec()
	{
		if (!fileSpec.IsEmpty())
			return fileSpec;
	...
	virtual tukk GetDataFilesFilterString() override { return dataFilter; }
...
protected:
	virtual tukk GetDescription() override { return "New BaseObject"; };
	virtual tukk GetObjectName()  override { return m_object->GetName(); };
...
protected:
	virtual tukk GetDescription() override { return "Delete BaseObject"; };
	virtual tukk GetObjectName() override  { return m_object->GetName(); };
...
private:
	CBaseObjectPtr       m_object;
	std::vector<DrxGUID> m_linkedObjectGuids;
	XmlNodeRef           m_undo;
	TGUIDRemap           m_remaping;
	bool                 m_bSelected;
	bool                 m_isPrefab;
...
protected:
	...
	virtual tukk GetDescription() override { return "Select Objects"; };
...
private:
	void Select(bool bSelect)
	{
		...
		for (i32 i = m_objectsGuids.size() - 1; i >= 0; --i)
		{
	...
	std::vector<DrxGUID> m_objectsGuids;
	bool                 m_bUndoSelect;
	bool                 m_bRedoSelect;
};
...
DrxGUID CRandomUniqueGuidProvider::GetFrom(const DrxGUID&) const
{
	DrxGUID newGuid;
	do
	{
		newGuid = DrxGUID::Create();
	}
	while (GetIEditorImpl()->GetObjectManager()->FindObject(newGuid));
	return newGuid;
}

DrxGUID IGuidProvider::GetFor(CBaseObject* pObject) const
{
...
CBaseObject* CObjectManager::NewObject(CObjectClassDesc* cls, CBaseObject* prev, const string& file)
{
	...
	DRX_ASSERT(rtClass->IsDerivedFrom(RUNTIME_CLASS(CBaseObject)));
	if (prev)
	{
		// Both current and previous object must be of same type.
		DRX_ASSERT(cls == prev->GetClassDesc());
	}
...
CBaseObject* CObjectManager::NewObject(CObjectArchive& ar, CBaseObject* pUndoObject, bool bLoadInCurrentLayer)
{
	...
	string typeName;
	DrxGUID id = DrxGUID::Null();
	DrxGUID idInPrefab = DrxGUID::Null();
	...
	if (pUndoObject)
	{
		...
		if (objNode->getAttr("Layer", layerName))
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Could not find layer %s", (tukk)layerName);
				return nullptr;
			}
		}
	}
	else
	{
		...
		if (!cls)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "RuntimeClass %s not registered", (tukk)typeName);
			return nullptr;
		}
		...
		else
		{
			if (objNode->getAttr("Layer", layerName))
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Could not find layer %s for object %s", (tukk)layerName, (tukk)objName);
					return nullptr;
				}
			}
			else
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Object %s is missing Layer information", (tukk)objName);
				return nullptr;
			}
		}
		...
		if (obj && !CGuidCollisionResolver(pObject, ar).Resolve())
		{
			...
			error.Format(_T("[Error] Object %s in layer %s is a duplicate of object %s in layer %s. %s has been removed. %s"),
			             (tukk)pObject->GetName(), (tukk)pObject->GetLayer()->GetName(), (tukk)obj->GetName(),
			             (tukk)obj->GetLayer()->GetName(), pObject->GetName(),
			             DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", obj->GetName()));

			CLogFile::WriteLine(error);

			if (!GetIEditorImpl()->IsInTestMode() && !GetIEditorImpl()->IsInLevelLoadTestMode())
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, (tukk)error);
			}
...
CBaseObject* CObjectManager::NewObject(const string& typeName, CBaseObject* prev, const string& file)
{
	...
	if (!cls)
	{
		GetIEditorImpl()->GetSystem()->GetILog()->Log("Warning: RuntimeClass %s (as well as %s) not registered", (tukk)typeName, (tukk)fullName);
		return 0;
	}
...
void CObjectManager::DeleteObjects(std::vector<CBaseObject*>& objects)
{
	...
	for (auto pObject : objects)
	{
		...
		if (CUndo::IsRecording())
		{
			...
			for (i32 i = 0; i < pObject->GetChildCount(); i++)
			{
...
//////////////////////////////////////////////////////////////////////////
CBaseObject* CObjectManager::FindObject(const DrxGUID& guid) const
{
...
//////////////////////////////////////////////////////////////////////////
CBaseObject* CObjectManager::FindObject(tukk objectName, CRuntimeClass* pRuntimeClass) const
{
...
bool CObjectManager::AddObject(CBaseObject* obj)
{
	...
	if (p)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "New Object %s has Duplicate GUID %s, New Object Ignored %s",
		           (tukk)obj->GetName(), obj->GetId().ToString(),
		           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", obj->GetName()));
...
//////////////////////////////////////////////////////////////////////////
void CObjectManager::ChangeObjectId(const DrxGUID& oldGuid, const DrxGUID& newGuid)
{
...
void CObjectManager::ShowDuplicationMsgWarning(CBaseObject* obj, const string& newName, bool bShowMsgBox) const
{
	...
	if (pExisting)
	{
		...
		// DrxWarning uses printf internally
		sRenameWarning.Replace("%", "%%");
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, sRenameWarning);
	}
}
...
bool CObjectManager::ChangeObjectName(CBaseObject* pObject, const string& newName, bool bGenerateUnique /* = false*/)
{
	...
	if (newName != pObject->GetName())
	{
		...
		if (IsDuplicateObjectName(name))
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Name %s already in use", newName);
				return false;
			}
		}
...
//////////////////////////////////////////////////////////////////////////
i32 CObjectManager::GetObjectCount() const
{
...
{
	DRX_PROFILE_FUNCTION(PROFILE_EDITOR);
...
void CObjectManager::LinkSelectionTo(CBaseObject* pLinkTo, tukk szTargetName)
{
	if (!pLinkTo)
		return;
...
void CObjectManager::Link(CBaseObject* pObject, CBaseObject* pLinkTo, tukk szTargetName)
{
	if (!pLinkTo || !pObject)
		return;
...
void CObjectManager::Link(const std::vector<CBaseObject*> objects, CBaseObject* pLinkTo, tukk szTargetName)
{
	if (!pLinkTo)
		return;
...
#if defined(USE_GEOM_CACHES)
void CObjectManager::LinkToGeomCacheNode(const std::vector<CBaseObject*> objects, CGeomCacheEntity* pLinkTo, tukk target)
{
	for (CBaseObject* pObject : objects)
	{
		// If the current object is not an entity, then fall back to linking to object
...
void CObjectManager::GetObjects(DynArray<CBaseObject*>& objects, const CObjectLayer* layer) const
{
	...
	objects.clear();
	for (i32 i = 0, iCount(objectArray.size()); i < iCount; ++i)
...
void CObjectManager::GetCameras(std::vector<CEntityObject*>& objects)
{
	...
	for (Objects::iterator it = m_objects.begin(); it != m_objects.end(); ++it)
	{
		...
		if (object->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			...
			{
				if (Drx::DefaultComponents::CCameraComponent* pCameraComponent = pEntity->GetComponent<Drx::DefaultComponents::CCameraComponent>())
				{
					objects.push_back(pEntityObject);
				}
			}
...
void CObjectManager::ClearSelection()
{
	...
	for (i32 i = 0; i < m_currSelection.GetCount(); ++i)
	{
...
{
	DRX_PROFILE_FUNCTION(PROFILE_EDITOR);
	DRX_ASSERT_MESSAGE(!bDisplay || passInfo, "a valid passinfo is required when objects need to be displayed.");
	...
	{
		i32 numVis = m_visibleObjects.size();
		for (i32 i = 0; i < numVis; i++)
		{
	...
	else
	{
		...
		if (pSelection && pSelection->GetCount() > 1)
		{
			...
			for (i32 i = 0, iCount(pSelection->GetCount()); i < iCount; ++i)
			{
...
//////////////////////////////////////////////////////////////////////////
i32 CObjectManager::MoveObjects(const AABB& box, const Vec3& offset, i32 nRot, bool bIsCopy)
{
	AABB objBounds;
		...
		if (pObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			...
			if (pEntityObj)
			{
				...
				if (pIEntity)
				{
					...
					{
						string msg("");
						msg.Format(_T("AI object %s cannot be deleted when AI/Physics mode is activated."), (tukk)pObject->GetName());
...
bool CObjectManager::HitTest(HitContext& hitInfo)
{
	...
	tukk name = nullptr;
	i32 numVis = pDispayedViewObjects->GetObjectCount();
	for (i32 i = 0; i < numVis; i++)
	{
...
void CObjectManager::FindObjectsInRect(CViewport* view, const CRect& rect, std::vector<DrxGUID>& guids)
{
...
//////////////////////////////////////////////////////////////////////////
u16 FindPossibleObjectNameNumber(std::set<u16>& numberSet)
{
	i32k LIMIT = 65535;
	size_t nSetSize = numberSet.size();
	for (u16 i = 1; i < LIMIT; ++i)
	{
		u16 candidateNumber = (i + nSetSize) % LIMIT;
...
void CObjectManager::RegisterObjectName(const string& name)
{
	...
	{
		std::set<u16> numberSet;
		numberSet.insert(num);
		m_nameNumbersMap[typeName] = numberSet;
	}
	else
	{
		std::set<u16>& numberSet = iNameNumber->second;
		numberSet.insert(num);
	}
}
...
void CObjectManager::UpdateRegisterObjectName(const string& name)
{
	// Remove all numbers from the end of typename.
	string typeName = name;
	i32 nameLen = typeName.GetLength();
	i32 len = nameLen;
...
string CObjectManager::GenUniqObjectName(const string& theTypeName)
{
	...
	if (tukk subName = ::strstr(theTypeName, "::"))
	{
	...
	string str;
	str.Format("%s-%d", (tukk)typeName, lastNumber);
...
void CObjectManager::GetClassCategories(std::vector<string>& categories)
{
	...
	std::set<string> cset;
	for (i32 i = 0; i < classes.size(); i++)
	{
		tukk category = classes[i]->Category();
...
void CObjectManager::GetClassCategoryToolClassNamePairs(std::vector<std::pair<string, string>>& categoryToolClassNamePairs)
{
	...
	{
		tukk category = classes[i]->Category();
		tukk toolClassName = ((CObjectClassDesc*)classes[i])->GetToolClassName();
...
//////////////////////////////////////////////////////////////////////////
void CObjectManager::Serialize(XmlNodeRef& xmlNode, bool bLoading, i32 flags)
{
	if (!xmlNode)
		return;
	...
	for (Objects::iterator it = m_objects.begin(); it != m_objects.end(); ++it)
	{
		    ...
		    (!pObject->CheckFlags(OBJFLAG_SHARED) && !onlyShared))
		{
			if (pObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
			{
				...
				auto upperBoundIt = std::upper_bound(sortedObjects.begin(), sortedObjects.end(), sortOrder, [](i32k sortOrder, const SOrderedObject& other) -> bool { return sortOrder < other.sortOrder; });
...
//////////////////////////////////////////////////////////////////////////
void CObjectManager::RegisterTypeConverter(CRuntimeClass* pSource, tukk szTargetName, std::function<void(CBaseObject* pObject)> conversionFunc)
{
	m_typeConverters.emplace_back(SConverter { pSource, szTargetName, conversionFunc });
}

//////////////////////////////////////////////////////////////////////////
void CObjectManager::IterateTypeConverters(CRuntimeClass* pSource, std::function<void(tukk szTargetName, std::function<void(CBaseObject* pObject)> conversionFunc)> callback)
{
...
//////////////////////////////////////////////////////////////////////////
void CObjectManager::StartObjectsLoading(i32 numObjects)
{
	if (m_pLoadProgress)
		return;
...
IStatObj* CObjectManager::GetGeometryFromObject(CBaseObject* pObject)
{
	...
	if (pObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
	{
		...
		if (pEntityObj->GetIEntity())
		{
			...
			for (i32 i = 0; pGameEntity != NULL && i < pGameEntity->GetSlotCount(); i++)
			{
...
CBaseObject* CObjectManager::FindPhysicalObjectOwner(IPhysicalEntity* pPhysicalEntity)
{
	...
	i32 itype = pPhysicalEntity->GetiForeignData();
	switch (itype)
	{
...
void CObjectManager::FindAndRenameProperty2(tukk property2Name, const string& oldValue, const string& newValue)
{
	CBaseObjectsArray objects;
	GetObjects(objects);
...
void CObjectManager::FindAndRenameProperty2If(tukk property2Name, const string& oldValue, const string& newValue, tukk otherProperty2Name, const string& otherValue)
{
	CBaseObjectsArray objects;
	GetObjects(objects);
	...
	for (Objects::iterator it = m_objects.begin(); it != m_objects.end(); ++it)
	{
		...
		else if (obj->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			...
			if (pEntityObj && pEntityObj->GetStatObj(0) && pEntityObj->GetStatObj(0)->IsDefaultObject())
			{
				...
				if (pVars)
				{
					for (i32 i = 0; i < pVars->GetNumVariables(); i++)
					{
						pModelVar = pVars->GetVariable(i);
						if (pModelVar && pModelVar->GetDataType() == IVariable::DT_FILE)
						{
							pModelVar->Get(geometryFile);
							tukk ext = PathUtil::GetExt(geometryFile);
							if (stricmp(ext, DRX_GEOMETRY_FILE_EXT) == 0 || stricmp(ext, DRX_SKEL_FILE_EXT) == 0 || stricmp(ext, DRX_CHARACTER_DEFINITION_FILE_EXT) == 0 || stricmp(ext, DRX_ANIM_GEOMETRY_FILE_EXT) == 0)
							{
								if (!gEnv->pDrxPak->IsFileExist(geometryFile))
								{
									pEntity = pEntityObj;
									break;
								}
		...
		{
			string mes;
			mes.Format(_T("Geometry file for object \"%s\" is missing/removed. \r\nFile: %s\r\nAttempt to locate this file?"), (tukk)obj->GetName(), (tukk)geometryFile);
...
void CObjectManager::ResolveMissingMaterials()
{
	...
	for (Objects::iterator it = m_objects.begin(); it != m_objects.end(); ++it)
	{
		...
		{
			string mes;
			mes.Format(_T("Material for object \"%s\" is missing/removed. \r\nFile: %s\r\nAttempt to locate this file?"), (tukk)obj->GetName(), (tukk)oldFilename);
...
void CObjectManager::SetSelectionMask(i32 mask) const
{
...
void CObjectManager::SaveEntitiesInternalState(struct IDataWriteStream& writer) const
{
	...
	for (u32 i = 0; i < exportedXml.size(); ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
void CObjectManager::HitTestObjectAgainstRect(CBaseObject* pObj, CViewport* view, HitContext hc, std::vector<DrxGUID>& guids)
{
	if (!pObj->IsSelectable())
		return;
	...
	if (pObj->IsKindOf(RUNTIME_CLASS(CGroup)))
	{
		...
		if (pGroup->IsOpen())
		{
			for (i32 i = 0; i < pGroup->GetChildCount(); ++i)
			{
...
namespace
{
std::vector<STxt> PyGetAllObjects(const string& className, const string& layerName)
{
	...
	i32 count = pObjMgr->GetObjectCount();
...
std::vector<STxt> PyGetAllLayers()
{
...
std::vector<STxt> PyGetNamesOfSelectedObjects()
{
	const CSelectionGroup* pSel = GetIEditorImpl()->GetSelection();
	std::vector<STxt> result;
	i32k selectionCount = pSel->GetCount();
	result.reserve(selectionCount);
	for (i32 i = 0; i < selectionCount; i++)
	{
...
void PySelectObject(tukk objName)
{
...
void PySelectAndGoToObject(tukk objName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(objName);
...
void PyUnselectObjects(const std::vector<STxt>& names)
{
	...
	for (i32 i = 0; i < pBaseObjects.size(); i++)
	{
		GetIEditorImpl()->GetObjectManager()->UnselectObject(pBaseObjects[i]);
	}
}

void PySelectObjects(const std::vector<STxt>& names)
{
...
bool PyIsObjectHidden(tukk objName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(objName);
	if (!pObject)
	{
...
void PyHideAllObjects()
{
	...
	for (i32 i = 0; i < baseObjects.size(); i++)
	{
...
void PyHideObject(tukk objName)
{
...
void PyUnhideObject(tukk objName)
{
...
void PyFreezeObject(tukk objName)
{
...
void PyUnfreezeObject(tukk objName)
{
...
void PyDeleteObject(tukk objName)
{
...
string PyGetEntityGeometryFile(tukk objName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(objName);
	if (pObject == NULL)
		return "";
...
void PySetEntityGeometryFile(tukk objName, tukk filePath)
{
...
string PyGetDefaultMaterial(tukk objName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(objName);
	if (pObject == NULL)
		return "";
...
string PyGetCustomMaterial(tukk objName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(objName);
	if (pObject == NULL)
		return "";
...
string PyGetAssignedMaterial(tukk pName)
{
...
void PySetCustomMaterial(tukk objName, tukk matName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(objName);
	if (pObject == NULL)
		return;
...
std::vector<STxt> PyGetFlowGraphsUsingThis(tukk objName)
{
	std::vector<STxt> list;
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(objName);
	if (pObject == NULL)
		return list;
...
boost::python::tuple PyGetObjectPosition(tukk pName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
boost::python::tuple PyGetWorldObjectPosition(tukk pName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
void PySetObjectPosition(tukk pName, float fValueX, float fValueY, float fValueZ)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
boost::python::tuple PyGetObjectRotation(tukk pName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
void PySetObjectRotation(tukk pName, float fValueX, float fValueY, float fValueZ)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
boost::python::tuple PyGetObjectScale(tukk pName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
void PySetObjectScale(tukk pName, float fValueX, float fValueY, float fValueZ)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
std::vector<STxt> PyGetObjectLayer(const std::vector<STxt>& names)
{
	std::vector<STxt> result;
	std::set<STxt> tempSet;
	CBaseObjectsArray objectArray;
	GetIEditorImpl()->GetObjectManager()->GetObjects(objectArray);

	for (i32 i = 0; i < objectArray.size(); i++)
	{
		for (i32 j = 0; j < names.size(); j++)
		{
			if ((objectArray.at(i))->GetName() == names[j].c_str())
			{
				tempSet.insert(STxt((objectArray.at(i))->GetLayer()->GetName()));
			}
		}
	}
	std::set<STxt>::iterator it;
...
void PySetObjectLayer(const std::vector<STxt>& names, tukk pLayerName)
{
...
std::vector<STxt> PyGetAllObjectsOnLayer(tukk pLayerName)
{
	CObjectLayer* pLayer = GetIEditorImpl()->GetObjectManager()->GetLayersManager()->FindLayerByName(pLayerName);
	if (!pLayer)
	{
	...
	std::vector<STxt> vectorObjects;

	for (i32 i = 0; i < objectArray.size(); i++)
	{
		if ((objectArray.at(i))->GetLayer()->GetName() == string(pLayerName))
		{
			vectorObjects.push_back(static_cast<STxt>((objectArray.at(i))->GetName()));
		}
	}
...
tukk PyGetObjectParent(tukk pName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
std::vector<STxt> PyGetObjectChildren(tukk pName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
	...
	{
		result.push_back(static_cast<STxt>(it->get()->GetName()));
	}
	return result;
}

void PyGenerateCubemap(tukk pObjectName)
{
...
string PyGetObjectType(tukk pName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
i32 PyGetObjectLodsCount(tukk pName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pName);
	if (!pObject)
	{
...
void PyRenameObject(tukk pOldName, tukk pNewName)
{
...
void PyAttachObject(tukk pParent, tukk pChild, tukk pAttachmentType, tukk pAttachmentTarget)
{
...
void PyDetachObject(tukk pObjectName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pObjectName);
	if (!pObject)
	{
...
void PyAddEntityLink(tukk pObjectName, tukk pTargetName, tukk pName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(pObjectName);
	if (!pObject)
	{
