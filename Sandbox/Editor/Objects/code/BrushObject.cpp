...
CBrushObject::CBrushObject()
{
	...
	CVarEnumList<i32>* pHideModeList = new CVarEnumList<i32>;
...
bool CBrushObject::CreateGameObject()
{
	if (!m_pRenderNode)
	{
		u32 ForceID = GetObjectManager()->ForceID();
		...
		{
			// keep high part of DrxGUID (stays compatible with old GUID.Data1)
...
void CBrushObject::ApplyStatObjProperties()
{
	if (m_pGeometry)
	{
		tukk const szProperties = m_pGeometry->GetIStatObj()->GetProperties();

		ParsePropertyString(szProperties, "\n\r", [this](tukk pKey, size_t keyLength, tukk pValue, size_t valueLength)
		{
...
void CBrushObject::Display(CObjectRenderHelper& objRenderHelper)
{
	...
	if (!m_pGeometry)
	{
		static i32 nagCount = 0;
		if (nagCount < 100)
		{
			const string& geomName = mv_geometryFile;
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Brush '%s' (%s) does not have geometry!", (tukk)GetName(), (tukk)geomName);
			nagCount++;
		}
	}
...
bool CBrushObject::HitTest(HitContext& hc)
{
	...
	if (Intersect::Ray_AABB(raySrc, rayDir, m_bbox, pnt))
	{
		...
		if (physics)
		{
			...
			ray_hit hit;
			i32 col = GetIEditorImpl()->GetSystem()->GetIPhysicalWorld()->RayTraceEntity(physics, origin, dir, &hit, 0, geom_collides);
			if (col > 0)
			{
				// World space distance.
				hc.dist = hit.dist;
...
//////////////////////////////////////////////////////////////////////////
i32 CBrushObject::HitTestAxis(HitContext& hc)
{
	//@HACK Temporary hack.
	return 0;
}
...
void CBrushObject::InvalidateTM(i32 nWhyFlags)
{
...
//////////////////////////////////////////////////////////////////////////
static bool IsBrushFilename(tukk filename)
{
	...
	tukk const ext = DrxGetExt(filename);

	if ((stricmp(ext, DRX_GEOMETRY_FILE_EXT) == 0) ||
	    (stricmp(ext, DRX_ANIM_GEOMETRY_FILE_EXT) == 0))
	{
		return true;
	}
...
//////////////////////////////////////////////////////////////////////////
void CBrushObject::CreateBrushFromMesh(tukk meshFilename)
{
	if (m_pGeometry)
	{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "File %s cannot be used for brush geometry (unsupported file extension)", meshFilename);
		m_pGeometry = 0;
	}
	else
	{
...
//////////////////////////////////////////////////////////////////////////
void CBrushObject::SetMinSpec(u32 nSpec, bool bSetChildren)
{
...
//////////////////////////////////////////////////////////////////////////
void CBrushObject::SetMaterialLayersMask(u32 nLayersMask)
{
...
void CBrushObject::Validate()
{
	...
	if (!m_pGeometry)
	{
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "No Geometry %s for Brush %s %s", (tukk)file, (tukk)GetName(),
	...
	{
		string file = mv_geometryFile;
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Geometry file %s for Brush %s Failed to Load %s", (tukk)file, (tukk)GetName(),
		           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
	}
}
...
//////////////////////////////////////////////////////////////////////////
bool CBrushObject::StartSubObjSelection(i32 elemType)
{
	bool bStarted = false;
	if (m_pGeometry)
	{
		if (m_pGeometry->GetUserCount() > 1)
		{
			...
			str.Format("Geometry File %s is used by multiple objects.\r\nDo you want to make an unique copy of this geometry?",
			           (tukk)m_pGeometry->GetFilename());
			i32 res = MessageBox(AfxGetMainWnd()->GetSafeHwnd(), str, "Warning", MB_ICONQUESTION | MB_YESNOCANCEL);
			if (res == IDCANCEL)
				return false;
			if (res == IDYES)
			{
				...
				string filename = levelPath + "Objects\\" + GetName() + "." + DRX_GEOMETRY_FILE_EXT;
...
void CBrushObject::GetVerticesInWorld(std::vector<Vec3>& vertices) const
{
	...
	{
		for (i32 v = 0; v < meshDesc.m_nVertCount; ++v)
		{
