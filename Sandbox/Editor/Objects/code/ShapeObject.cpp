...
#include <DrxInput/IHardwareMouse.h>
...
public:
	...
	virtual void   SetUserData(tukk key, uk userData);
	...
	void           OnManipulatorBeginDrag(IDisplayViewport*, ITransformManipulator*, const Vec2i&, i32 flags);
	void           OnManipulatorDrag(IDisplayViewport*, ITransformManipulator*, const Vec2i&, const Vec3&, i32);
	...
	void           OnShapePropertyChange(CBaseObject*, i32);
...
//////////////////////////////////////////////////////////////////////////
void CEditShapeTool::SetUserData(tukk key, uk userData)
{
	m_shape = (CShapeObject*)userData;
	DRX_ASSERT(m_shape != nullptr);
...
bool CEditShapeTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
	...
	else if (nChar == Qt::Key_Delete)
	{
		if (m_shape)
		{
			i32 sel = m_shape->GetSelectedPoint();
...
void CEditShapeTool::OnManipulatorBeginDrag(IDisplayViewport* view, ITransformManipulator*, const Vec2i&, i32 flags)
{
...
void CEditShapeTool::OnManipulatorDrag(IDisplayViewport*, ITransformManipulator*, const Vec2i&, const Vec3& offset, i32 flags)
{
	if (GetIEditorImpl()->GetEditMode() == eEditModeMove)
	{
...
void CEditShapeTool::OnShapePropertyChange(CBaseObject*, i32 event)
{
...
//////////////////////////////////////////////////////////////////////////
bool CEditShapeTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
	if (!m_shape)
		return false;
	...
	if (event == eMouseLDown || event == eMouseMove || event == eMouseLDblClick || event == eMouseLUp)
	{
		...
		if ((flags & MK_CONTROL) && !m_modifying)
		{
			...
			if (p1 >= 0 && p2 >= 0 && dist < fShapeCloseDistance + view->GetSelectionTolerance())
			{
				...
				if (event == eMouseLDown)
				{
					...
					i32 index = m_shape->InsertPoint(p2, intPnt, true);
		...
		i32 index = m_shape->GetNearestPoint(raySrc, rayDir, dist);
...
//////////////////////////////////////////////////////////////////////////
void CMergeShapesTool::SetUserData(tukk key, uk userData)
{
	/*
	   // Modify shape undo.
	   if (!CUndo::IsRecording())
...
//////////////////////////////////////////////////////////////////////////
bool CMergeShapesTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
	if (nChar == Qt::Key_Escape)
		GetIEditorImpl()->SetEditTool(0);
	return true;
...
//////////////////////////////////////////////////////////////////////////
bool CMergeShapesTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
	//return true;
...
//////////////////////////////////////////////////////////////////////////
void CSplitShapeTool::SetUserData(tukk key, uk userData)
{
	m_curIndex = -1;
	m_shape = (CShapeObject*)userData;
	DRX_ASSERT(m_shape != 0);
...
//////////////////////////////////////////////////////////////////////////
bool CSplitShapeTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
	if (nChar == Qt::Key_Escape)
		GetIEditorImpl()->SetEditTool(0);
	return true;
...
//////////////////////////////////////////////////////////////////////////
bool CSplitShapeTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
	if (!m_shape)
		return false;
...
protected:
	...
	virtual tukk GetDescription() { return "Shape Target"; };
...
void CShapeObject::SaveShapeTargets(XmlNodeRef xmlNode)
{
	...
	for (i32 i = 0, num = m_entities.GetCount(); i < num; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
void CShapeObject::InvalidateTM(i32 nWhyFlags)
{
...
bool CShapeObject::ConvertFromObject(CBaseObject* pObject)
{
	...
	if (pObject->IsKindOf(RUNTIME_CLASS(CShapeObject)))
	{
		...
		ClearPoints();
		for (i32 i = 0; i < pShapeObject->GetPointCount(); ++i)
		{
...
//////////////////////////////////////////////////////////////////////////
i32 CShapeObject::MouseCreateCallback(IDisplayViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
...
void CShapeObject::Serialize(CObjectArchive& ar)
{
	...
	if (ar.bLoading)
	{
		...
		if (points)
		{
			for (i32 i = 0; i < points->getChildCount(); i++)
			{
		...
		if (entities)
		{
			for (i32 i = 0; i < entities->getChildCount(); i++)
			{
...
//////////////////////////////////////////////////////////////////////////
i32 CShapeObject::InsertPoint(i32 index, const Vec3& point, bool const bModifying)
{
	if (GetPointCount() >= GetMaxPoints())
	...
	if (nCount > 1) // At least 1 point should be set already
	{
		...
		for (size_t nIdx = 0; nIdx < nCount; ++nIdx)
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "The point is too close to another point!");
				return nIdx;
			}
		}
	}
...
void CShapeObject::SplitAtPoint(i32 index, bool bSnap, Vec3 point)
{
...
//////////////////////////////////////////////////////////////////////////
void CShapeObject::SetMergeIndex(i32 index)
{
	m_mergeIndex = index;
}
...
void CShapeObject::Merge(CShapeObject* shape)
{
	...
	i32 const index = m_mergeIndex + 1;
...
//////////////////////////////////////////////////////////////////////////
void CShapeObject::RemovePoint(i32 index)
{
...
{
	DRX_ASSERT(pBaseObject != nullptr);
	m_bAreaModified = true;
...
{
	DRX_ASSERT(index >= 0 && index < m_entities.GetCount());
...
//////////////////////////////////////////////////////////////////////////
void CShapeObject::OnObjectEvent(CBaseObject* const pBaseObject, i32 const event)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CShapeObject::GetAreaId()
{
	return mv_areaId;
}

//////////////////////////////////////////////////////////////////////////
void CShapeObject::SelectPoint(i32 index)
{
	m_selectedPoint = index;
}

//////////////////////////////////////////////////////////////////////////
void CShapeObject::SetPoint(i32 index, const Vec3& pos)
{
	Vec3 p = pos;
	if (m_bForce2D)
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CShapeObject::GetNearestPoint(const Vec3& raySrc, const Vec3& rayDir, float& distance)
{
	i32 index = -1;
	float minDist = FLT_MAX;
...
//////////////////////////////////////////////////////////////////////////
void CShapeObject::GetNearestEdge(const Vec3& pos, i32& p1, i32& p2, float& distance, Vec3& intersectPoint)
{
	p1 = -1;
	p2 = -1;
	float minDist = FLT_MAX;
...
//////////////////////////////////////////////////////////////////////////
void CShapeObject::GetNearestEdge(const Vec3& raySrc, const Vec3& rayDir, i32& p1, i32& p2, float& distance, Vec3& intersectPoint)
{
	p1 = -1;
	p2 = -1;
	float minDist = FLT_MAX;
	...
	i32 totPoints = m_points.size();

	for (i32 i = 0; i < totPoints; i++)
	{
		i32 j = (i + 1) % totPoints;
...
CAIPathObject::CAIPathObject()
{
	...
	m_navType.AddEnumItem("Unset", (i32)IAISystem::NAV_UNSET);
	m_navType.AddEnumItem("Triangular", (i32)IAISystem::NAV_TRIANGULAR);
	m_navType.AddEnumItem("Waypoint Human", (i32)IAISystem::NAV_WAYPOINT_HUMAN);
	m_navType.AddEnumItem("Waypoint 3D Surface", (i32)IAISystem::NAV_WAYPOINT_3DSURFACE);
	m_navType.AddEnumItem("Flight", (i32)IAISystem::NAV_FLIGHT);
	m_navType.AddEnumItem("Volume", (i32)IAISystem::NAV_VOLUME);
	m_navType.AddEnumItem("Road", (i32)IAISystem::NAV_ROAD);
	m_navType.AddEnumItem("SmartObject", (i32)IAISystem::NAV_SMARTOBJECT);
	m_navType.AddEnumItem("Free 2D", (i32)IAISystem::NAV_FREE_2D);

	m_navType = (i32)IAISystem::NAV_UNSET;
	m_anchorType = "";
...
void CAIPathObject::UpdateGameArea()
{
	...
	if (ai)
	{
		...
		if (!worldPts.empty())
		{
			...
			i32 type = pAIMgr->AnchorActionToId(anchorType);
...
void CAIPathObject::DrawSphere(DisplayContext& dc, const Vec3& p0, float r, i32 n)
{
...
void CAIPathObject::DrawCylinder(DisplayContext& dc, const Vec3& p0, const Vec3& p1, float rad, i32 n)
{
	Vec3 dir(p1 - p0);
...
void CAIPathObject::Display(CObjectRenderHelper& objRenderHelper)
{
	...
	if (m_bValidatePath && IsSelected())
	{
		dc.DepthWriteOff();
		i32 num = m_points.size();
		if (!m_bRoad && m_navType == (i32)IAISystem::NAV_VOLUME && num >= 3)
		{
...
i32 CAIPathObject::InsertPoint(i32 index, const Vec3& point, bool const bModifying)
{
	i32 result = __super::InsertPoint(index, point, bModifying);
...
void CAIPathObject::SetPoint(i32 index, const Vec3& pos)
{
...
void CAIPathObject::RemovePoint(i32 index)
{
	...
	if (ai)
	{
		...
		if (!worldPts.empty())
		{
			...
			SNavigationShapeParams params(m_lastGameArea, AREATYPE_GENERIC, false, mv_closed, &worldPts[0], worldPts.size(),
			                              mv_height, 0, type, (EAILightLevel)(i32)m_lightLevel);
...
void CGameShapeObject::InitVariables()
{
	...
	if (pGameVolumesEdit)
	{
		...
		{
			tukk className = pGameVolumesEdit->GetVolumeClass(i);
			m_shapeClasses.AddEnumItem(className, (i32)i);
...
template<typename T>
bool CGameShapeObject::GetParameterValue(T& outValue, tukk functionName) const
{
...
void CGameShapeObject::SetMinSpec(u32 nSpec, bool bSetChildren)
{
...
void CGameShapeObject::SetMaterialLayersMask(u32 nLayersMask)
{
...
i32 CGameShapeObject::GetMaxPoints() const
{
	return m_maxPoints;
}

i32 CGameShapeObject::GetMinPoints() const
{
	return m_minPoints;
}
...
void CGameShapeObject::SetCustomMinAndMaxPoints()
{
	...
	if (pScript)
	{
		tukk functionNameMinPoints = "ShapeMinPoints";
		HSCRIPTFUNCTION scriptFunctionMinPoints(NULL);

		i32 minPoints = __super::GetMinPoints();
		...
		tukk functionNameMaxPoints = "ShapeMaxPoints";
		HSCRIPTFUNCTION scriptFunctionMaxPoints(NULL);

		i32 maxPoints = __super::GetMaxPoints();
...
void CGameShapeLedgeObject::SetPoint(i32 index, const Vec3& pos)
{
	__super::SetPoint(index, pos);
...
void CNavigationAreaObject::CreateInspectorWidgets(CInspectorWidgetCreator& creator)
{
	...
	creator.AddPropertyTree<CNavigationAreaObject>("Navigation Area", [](CNavigationAreaObject* pObject, Serialization::IArchive& ar, bool bMultiEdit)
	{
		...
		for (i32 i = 0, n = pObject->mv_agentTypes.GetNumVariables(); i < n; ++i)
		{
...
void CNavigationAreaObject::PostLoad(CObjectArchive& ar)
{
	if (IAISystem* aiSystem = GetIEditorImpl()->GetSystem()->GetAISystem())
	{
		if (INavigationSystem* aiNavigation = aiSystem->GetNavigationSystem())
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Unable to register Navigation Area with name '%s'", GetName().GetString());
			}
		}
	}
...
void CNavigationAreaObject::InitVariables()
{
	...
	if (agentTypeCount)
	{
		...
		for (size_t i = 0; i < agentTypeCount; ++i)
		{
			...
			tukk name = manager->GetNavigationAgentTypeName(i);
...
void CNavigationAreaObject::RelinkWithMesh(const ERelinkWithMeshesMode relinkMode)
{
	...
	DRX_ASSERT(pAINavigation);
	if (!pAINavigation)
		return;

	tukk navigationAreaName = GetName().GetString();
	const NavigationVolumeID volumeId = pAINavigation->GetAreaId(navigationAreaName);
	DRX_ASSERT(volumeId == m_volume);
...
//////////////////////////////////////////////////////////////////////////
void CNavigationAreaObject::RegisterNavigationArea(INavigationSystem& navigationSystem, tukk szAreaName)
{
	DRX_ASSERT(m_bRegistered == false);
	DRX_ASSERT(m_volume == NavigationVolumeID());
	...
	if (m_bRegistered)
	{
		...
		if (IAISystem* pAISystem = GetIEditorImpl()->GetSystem()->GetAISystem())
		{
			if (INavigationSystem* pNavigationSystem = pAISystem->GetNavigationSystem())
			{
				...
				//DRX_ASSERT(NavigationVolumeID() == pNavigationSystem->GetAreaId(szName));
...
//////////////////////////////////////////////////////////////////////////
void CNavigationAreaObject::RegisterNavigationListener(INavigationSystem& navigationSystem, tukk szAreaName)
{
...
//////////////////////////////////////////////////////////////////////////
void CNavigationAreaObject::RemovePoint(i32 index)
{
	__super::RemovePoint(index);
...
//////////////////////////////////////////////////////////////////////////
i32 CNavigationAreaObject::InsertPoint(i32 index, const Vec3& point, bool const bModifying)
{
	i32 result = __super::InsertPoint(index, point, bModifying);
...
//////////////////////////////////////////////////////////////////////////
void CNavigationAreaObject::SetPoint(i32 index, const Vec3& pos)
{
	__super::SetPoint(index, pos);
...
namespace
{
static void PyInsertPoint(tukk objName, i32 idx, float xPos, float yPos, float zPos)
{
	CBaseObject* pObject;

	if (GetIEditorImpl()->GetObjectManager()->FindObject(objName))
		pObject = GetIEditorImpl()->GetObjectManager()->FindObject(objName);
	else if (GetIEditorImpl()->GetObjectManager()->FindObject(DrxGUID::FromString(objName)))
		pObject = GetIEditorImpl()->GetObjectManager()->FindObject(DrxGUID::FromString(objName));
	else
