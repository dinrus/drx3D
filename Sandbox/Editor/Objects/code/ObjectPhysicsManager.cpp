...
void PyPhysicsResetState(tukk objectName)
{
...
void PyPhysicsGetState(tukk objectName)
{
	CBaseObject* pObject = GetIEditorImpl()->GetObjectManager()->FindObject(objectName);
	if (pObject)
...
IGeometry* PhysicalizeIdxMesh(IStatObj* pSrcObj, bool bFlip = false)
{
	...
	i32 i, nTris = pMesh->GetIndexCount() / 3;
	if (bFlip)
		for (i = 0; i < nTris; i++)
			std::swap(pMesh->m_pIndices[i * 3 + 1], pMesh->m_pIndices[i * 3 + 2]);
	i32* pIdx = new i32[nTris];
	for (i = 0; i < nTris; i++)
		pIdx[i] = i;
	tuk pMats = new char[nTris];
	memset(pMats, 0, nTris * sizeof(pMats[0]));
	for (i = 0; i < pMesh->m_subsets.size(); i++)
		for (i32 idx0 = pMesh->m_subsets[i].nFirstIndexId / 3, idx = idx0, nidx = pMesh->m_subsets[i].nNumIndices / 3; idx < idx0 + nidx; pMats[idx++] = pMesh->m_subsets[i].nMatID)
			;
...
IGeometry* GetTriMesh(IGeometry* pGeom, bool bClone, bool bFlip, i32 defaultMatId, IGeometry* pGeomFallback)
{
...
template<class T> inline void remap(T& dst, i32* pMap)
{
	T negmask = dst >> 31;
	dst = pMap[dst & ~negmask] | dst & negmask;
}

void RemapStatObjMatids(IStatObj* pStatObj, i32* pMap)
{
	CMesh* pMesh = pStatObj->GetIndexedMesh(true)->GetMesh();
	for (i32 i = 0; i < pMesh->m_subsets.size(); remap(pMesh->m_subsets[i++].nMatID, pMap))
		;
	...
	if (pPhysGeom->pGeom->GetType() == GEOM_TRIMESH)
	{
		...
		for (i32 j = 0; j < md->nTris && md->pMats; remap(md->pMats[j++], pMap))
			;
	}
}
...
void AlignCutTemplate(const IStatObj* pSrcObj, const IStatObj* pObjTpl, i32 align, const Vec3& tplOffs, float tplScale, Matrix33& R, Vec3& offset, float& scale)
{
	AABB boxTpl = pObjTpl->GetAABB(), boxSrc = pSrcObj->GetAABB();
	Vec3 szSrc = boxSrc.GetSize(), szTpl = boxTpl.GetSize();
	i32 imaxTpl = idxmax3(szTpl), iminTpl = idxmin3(szTpl), imaxSrc = idxmax3(szSrc), iminSrc = idxmin3(szSrc) != imaxSrc ? idxmin3(szSrc) : 2 - imaxSrc;
	if (align == -1)
	{
	...
	{
		if (align > 23)
			align = drx_random(0, 23);
		R.SetZero();
		i32 xnew = align >> 3, flipy = align >> 2 & 1, ynew = decm3(xnew) * flipy + incm3(xnew) * (1 - flipy), sgx = align >> 1 & 1, sgy = align & 1;
		R(0, xnew) = 1 - sgx * 2;
		R(1, ynew) = 1 - sgy * 2;
...
void CObjectPhysicsManager::Command_GenerateJoints()
{
	...
	const CSelectionGroup* pSelection = GetIEditorImpl()->GetSelection();
	for (i32 iobj = 0, nHits; iobj < pSelection->GetCount(); iobj++)
		if (pSelection->GetObject(iobj)->GetType() == OBJTYPE_ENTITY || pSelection->GetObject(iobj)->GetType() == OBJTYPE_BRUSH)
		{
			...
			for (; islot >= 0; islot--)
				if (pTrgObj->GetSubObject(islot)->nType == STATIC_SUB_OBJECT_MESH)
				{
					...
					for (i32 i = nHits = 0; i < epq.nCount; i++)
					{
						epq.pEntities[i]->GetScriptTable()->GetValue("Properties", props);
					...
					if (tplObj.length())
						if (IStatObj* pObjTpl = gEnv->p3DEngine->LoadStatObj(tplObj, 0, 0, false))
							if (VerifySingleProxy(pObjTpl))
							{
								...
								i32 nInside = 0, nOutside = 0, nCut = 0, nFailed = 0, nSmall = 0, nTries;
								...
								i32 nMats = max(1, max(pMtl->GetSubMtlCount(), pSrcMtl->GetSubMtlCount())), * mtlMap = new i32[nMats];
								for (i32 i = 0; i < nMats; mtlMap[i] = i, i++)
									;
								...
								i32 surfTypes[MAX_SUB_MATERIALS];
								memset(surfTypes, 0, sizeof(surfTypes));
								i32 nsurfTypes = pSrcMtl->FillSurfaceTypeIds(surfTypes);

								for (i32 i = pCutObj->GetSubObjectCount() - 1; i >= 0; i--)
								{
								...
								if (pCutObj->GetSubObjectCount() > 0)
								{
									...
									float kmass = 0;
									char strmass[32];
									if (tukk pmass = strstr(pSubObj0->properties, "mass"))
									...
									for (i32 j = 0; j < pCutObj->GetSubObjectCount(); j++)
										if (pCutObj->GetSubObject(j)->nType == STATIC_SUB_OBJECT_MESH)
										{
											...
											if (timeoutMaxSize && sqr(slot0.pStatObj->GetPhysGeom()->V) * cube(slot.tm.GetColumn0().len2()) < sqr(timeoutMaxSize))
											{
												if (timeout >= 0.0f)
												{
													drx_sprintf(strmass, "\ntimeout=%.1f", max(0.0f, timeout + drx_random(-timeoutRand, timeoutRand)));
													slot.properties += strmass;
												}
			...
			for (i32 i = pTrgObj->GetSubObjectCount() - 1; i >= 0; i--)
			...
			if (nHits)
			{
				...
				static tuk g_txtBuf = 0;
				static i32 g_txtBufSize = 0;
				i32 sz = pent->GetStateSnapshotTxt(0, 0, -1);
				if (sz > g_txtBufSize)
				{
					delete[] g_txtBuf;
				...
				string filename;
				if (CFileUtil::SelectSaveFile("CGF Files (*.cgf)|*.cgf", DRX_GEOMETRY_FILE_EXT, PathUtil::Make(PathUtil::GetGameFolder(), "objects").c_str(), filename))
					if (!(filename = PathUtil::AbsolutePathToDrxPakPath(filename.GetString())).IsEmpty())
					{
...
void CObjectPhysicsManager::UpdateSimulatingObjects()
{
	{
		CUndo undo("Simulate");
		for (i32 i = 0; i < m_simObjects.size(); )
		{
...
//////////////////////////////////////////////////////////////////////////
i32 CObjectPhysicsManager::RegisterCollisionClass(const SCollisionClass& collclass)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CObjectPhysicsManager::GetCollisionClassId(const SCollisionClass& collclass)
{
	TCollisionClassVector::iterator it = std::find(m_collisionClasses.begin(), m_collisionClasses.end(), collclass);
	if (it == m_collisionClasses.end())
		return 0;
...
void CObjectPhysicsManager::SerializeCollisionClasses(CXmlArchive& xmlAr)
{
	if (!xmlAr.bLoading)
	{
		...
		i32 count = m_collisionClasses.size();
