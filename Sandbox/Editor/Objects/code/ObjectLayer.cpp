...
protected:
	virtual tukk GetDescription() { return "Set Layer State"; }
...
protected:
	virtual tukk GetDescription() { return "Change Layer Name"; }
...
private:
	void Reparent(DrxGUID childGUID, DrxGUID newParentGUID)
	{
		CObjectLayerManager* pLayerManager = GetIEditorImpl()->GetObjectManager()->GetLayersManager();
...
CObjectLayer::CObjectLayer(tukk szName, EObjectLayerType type)
	: m_guid(DrxGUID::Create())
	, m_name(szName)
	, m_hidden(false)
	, m_frozen(false)
...
void CObjectLayer::SerializeBase(XmlNodeRef& node, bool isLoading)
{
	...
	else // save
	{
		...
		node->setAttr("Type", static_cast<i32>(m_layerType));
	}
}
...
void CObjectLayer::Serialize(XmlNodeRef& node, bool isLoading)
{
	...
	else
	{
		...
		DrxGUID parentGUID = m_parentGUID;
		if (parentGUID != DrxGUID::Null())
...
uint CObjectLayer::GetObjectCount() const
{
	...
	return i32(objects.size());
}

CObjectLayer* CObjectLayer::Create(tukk szName, EObjectLayerType type)
{
...
//////////////////////////////////////////////////////////////////////////
CObjectLayer* CObjectLayer::GetChild(i32 index) const
{
