...
//////////////////////////////////////////////////////////////////////////
void CRopeObject::InvalidateTM(i32 nWhyFlags)
{
...
void CRopeObject::SetMaterial(IEditorMaterial* mtl)
{
	...
	if (pRopeNode != nullptr)
	{
		...
		u8 nMaterialLayersFlags = GetMaterialLayersMask();
	...
	if (m_points.size() > 1)
	{
		...
		if (pRopeNode && m_endLinksDisplayUpdateCounter > 10 && gEnv->pPhysicalWorld->GetPhysVars()->lastTimeStep == 0.0f)
		{
			...
			for (i32 i = 0; i < 2; i++)
			{
				...
				{
					pe_status_pos ppos;
					i32 nPointIndex = (i == 0) ? 0 : GetPointCount() - 1;
...
static void SerializeBitflag(Serialization::IArchive& ar, i32& flag, i32 field, tukk name, tukk label)
{
	...
	if (ar.bLoading)
	{
		...
		if (xmlNodeRope)
		{
			...
			if (xmlNodeAudio)
			{
				...
				std::underlying_type<DrxAudio::EOcclusionType>::type tempOcclusionType;
				xmlNodeAudio->getAttr("OcclusionType", tempOcclusionType);
				m_ropeAudioData.occlusionType = static_cast<DrxAudio::EOcclusionType>(tempOcclusionType);
...
void CRopeObject::UpdateAudioData()
{
	...
	if (pIRopeRenderNode != nullptr)
	{
		...
		audioParams.startTrigger = DrxAudio::StringToId(m_ropeAudioData.startTriggerName.c_str());
		audioParams.stopTrigger = DrxAudio::StringToId(m_ropeAudioData.stopTriggerName.c_str());
		audioParams.angleParameter = DrxAudio::StringToId(m_ropeAudioData.angleParameterName.c_str());
