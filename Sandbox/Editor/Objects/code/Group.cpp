...
private:
	...
	void RestoreTransformsWithNewParent()
	{
		...
		for (i32 i = 1; i < m_transformDelegates.size(); ++i)
		{
...
private:
	...
	virtual i32         GetSize()        { return sizeof(CUndoBatchAttachBaseObject); }
	virtual tukk GetDescription() { return "Attachment Changed"; }

	std::vector<DrxGUID> m_objectsGuids;
	std::vector<DrxGUID> m_oldLayersGuids;
	DrxGUID              m_parentGuid;
...
bool CGroup::HitTestMembers(HitContext& hcOrg)
{
	...
	i32 numMembers = static_cast<i32>(m_members.size());
...
void CGroup::CreateInspectorWidgets(CInspectorWidgetCreator& creator)
{
	...
	if (!GetIEditorImpl()->IsCPrefabObject(this))
	{
		creator.AddPropertyTree<CGroup>("Group", [](CGroup* pObject, Serialization::IArchive& ar, bool bMultiEdit)
		{
			...
			{
			  ar(Serialization::ActionButton(std::bind(&CDrxEditApp::OnGroupUngroup, CDrxEditApp::GetInstance())), "ungroup", "^Ungroup");
			  if (pObject->IsOpen())
			  {
			    ar(Serialization::ActionButton(std::bind(&CDrxEditApp::OnGroupClose, CDrxEditApp::GetInstance())), "close", "^Close");
			  }
			  else
			  {
			    ar(Serialization::ActionButton(std::bind(&CDrxEditApp::OnGroupOpen, CDrxEditApp::GetInstance())), "open", "^Open");
			  }
			  ar.closeBlock();
			}
		});
...
void CGroup::SerializeMembers(CObjectArchive& ar)
{
	...
	if (ar.bLoading)
	{
		if (!ar.bUndo)
		{
			i32 num = static_cast<i32>(m_members.size());
			for (i32 i = 0; i < num; ++i)
			{
			...
			if (childsRoot)
			{
				...
				i32 numObjects = childsRoot->getChildCount();
...
i32 CGroup::SelectObjects(const AABB& box, bool bUnselect)
{
	i32 numSel = 0;
...
void CGroup::SetMaterial(IEditorMaterial* pMtl)
{
	if (pMtl)
	{
		for (i32 i = 0, iNumberSize(m_members.size()); i < iNumberSize; ++i)
		{
...
{
	for (i32 i = 0, totObjects = GetChildCount(); i < totObjects; ++i)
	{
