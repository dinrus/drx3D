...
//////////////////////////////////////////////////////////////////////////
void CRoadObject::InvalidateTM(i32 nWhyFlags)
{
...
//////////////////////////////////////////////////////////////////////////
float CRoadObject::GetLocalWidth(i32 index, float t)
{
	float kof = t;
	i32 i = index;
...
void CRoadObject::SetRoadSectors()
{
	...
	i32 sizeOld = m_sectors.size();
	i32 sizeNew = 0;

	i32 points_size = m_points.size();

	float fSegLen = 0.0f;

	for (i32 i = 0; i < points_size - 1; ++i)
	{
		i32 kn = GetRoadSectorCount(i);
...
//////////////////////////////////////////////////////////////////////////
i32 CRoadObject::GetRoadSectorCount(i32 index)
{
	i32 kn = i32((GetBezierSegmentLength(index) + 0.5f) / GetStepSize());
	if (kn == 0)
		return 1;
	return kn;
}
...
void CRoadObject::UpdateSectors()
{
	...
	i32 MAX_TRAPEZOIDS_IN_CHUNK = 16;
	i32 nChunksNum = m_sectors.size() / MAX_TRAPEZOIDS_IN_CHUNK + 1;

	CRoadSector& sectorFirstGlobal = m_sectors[0];
	CRoadSector& sectorLastGlobal = m_sectors[m_sectors.size() - 1];

	for (i32 nChunkId = 0; nChunkId < nChunksNum; ++nChunkId)
	{
		i32 nStartSecId = nChunkId * MAX_TRAPEZOIDS_IN_CHUNK;
		i32 nSectorsNum = min(MAX_TRAPEZOIDS_IN_CHUNK, (i32)m_sectors.size() - nStartSecId);
...
void CRoadObject::AlignHeightMap()
{
	...
	i32k nPoints = GetPointCount();

	for (i32 i = 0; i < nPoints - 1; ++i)
	{
		float fminx = 0, fminy = 0, fmaxx = 0, fmaxy = 0;
		bool bIsInitminmax = false;
		i32 kn = i32 (0.5f + (GetBezierSegmentLength(i) + 1) * 4);
		...
		heightmap->RecordUndo(i32(fminy / unitSize) - 1, i32(fminx / unitSize) - 1, i32(fmaxy / unitSize) + unitSize - i32(fminy / unitSize) + 1, i32(fmaxx / unitSize) + unitSize - i32(fminx / unitSize) + 1);

		for (i32 ty = i32(fminx / unitSize); ty <= i32(fmaxx / unitSize) + unitSize; ++ty)
			for (i32 tx = i32(fminy / unitSize); tx <= i32(fmaxy / unitSize) + unitSize; ++tx)
			{
				i32 x = ty * unitSize;
				i32 y = tx * unitSize;
				...
				if (findk != -1)
				{
					...
					{
						//i32 tx = pos_directed_rounding(y / unitSize);
						//i32 ty = pos_directed_rounding(x / unitSize);
...
void CRoadObject::EraseVegetation()
{
	if (CVegetationMap* vegetationMap = GetIEditorImpl()->GetVegetationMap())
	{
		...
		for (i32 i = 0; i < nPoints - 1; ++i)
		{
			i32k nSegment = GetBezierSegmentLength(i);
			i32k nSector = GetRoadSectorCount(i);

			for (i32 k = 0; k < nSector; ++k)
			{
				...
				i32k segmentStart = k * nSegment / nSector;
				i32k segmentEnd = (k + 1) * nSegment / nSector;
				...
				for (std::vector<CVegetationInstance*>::iterator it = instances.begin(); it != instances.end(); ++it)
				{
					...
					{
						fFinalEraseWidth -= float(rand() % i32(mv_eraseVegWidthVar * 10.f)) * 0.1f;
					}
					if (CVegetationObject* obj = (*it)->object)
					{
						fFinalEraseWidth += obj->GetObjectSize() * 0.5f;
					}
					for (i32 l = segmentStart; l < segmentEnd; ++l)
					{
...
//////////////////////////////////////////////////////////////////////////
void CRoadObject::SetMinSpec(u32 nSpec, bool bSetChildren)
{
...
//////////////////////////////////////////////////////////////////////////
void CRoadObject::SetMaterialLayersMask(u32 nLayersMask)
{
...
//////////////////////////////////////////////////////////////////////////
void CRoadObject::SetLayerId(u16 nLayerId)
{
	for (size_t i = 0; i < m_sectors.size(); ++i)
	{
		if (m_sectors[i].m_pRoadSector)
