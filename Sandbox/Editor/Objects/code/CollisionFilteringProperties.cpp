...
CCollisionFilteringProperties::CCollisionFilteringProperties()
{
	if (m_bNamesInitialised == false)
	{
		if (auto* pEditorGame = GetIEditorImpl()->GetGameEngine()->GetIEditorGame())
		{
			...
			for (i32 i = 0; i < k_maxCollisionClasses; i++)
			{
				tukk name = physicsSettings ? physicsSettings->GetCollisionClassName(i) : "";
...
i32 CCollisionFilteringProperties::GetCollisionClassExportId()
{
