...
struct CScriptMethodsDump : public IScriptTableDumpSink
{
	...
	virtual void OnElementFound(i32 nIdx, ScriptVarType type) {}
...
public:
	...
	//////////////////////////////////////////////////////////////////////////
	inline bool IsPropertyTypeMatch(tukk type, tukk name, i32 nameLen, bool exactName)
	{
		if (exactName)
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IVariable* CreateVar(tukk name, IVariable::EType defaultType, tukk & displayName)
	{
		displayName = name;
		...
		{
			// Try to detect type.
			for (i32 i = 0; i < DRX_ARRAY_COUNT(s_scriptParamTypes); i++)
			{
	...
	void Dump(IScriptTable* pObject, string sTablePath)
	{
		...
		{
			tukk sName = m_elements[i].name;
			ScriptVarType type = m_elements[i].type;

			tukk sDisplayName = sName;
...
//////////////////////////////////////////////////////////////////////////
i32 CEntityScript::GetEventCount()
{
...
//////////////////////////////////////////////////////////////////////////
tukk CEntityScript::GetEvent(i32 i)
{
	if (IEntityEventHandler* pEventHandler = m_pClass->GetEventHandler())
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CEntityScript::GetEmptyLinkCount()
{
	return (i32)m_emptyLinks.size();
}

//////////////////////////////////////////////////////////////////////////
const string& CEntityScript::GetEmptyLink(i32 i)
{
...
bool CEntityScript::Load()
{
	...
	if (strlen(m_pClass->GetScriptFile()) > 0)
	{
		if (m_pClass->LoadScript(false))
		{
			...
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Entity class %s failed to load, (script: %s could not be loaded)", m_pClass->GetName(), m_pClass->GetScriptFile());
	}
	else
	{
	...
	if (pEntity->GetValue("Editor", pEditorTable))
	{
		...
		if (pEditorTable->GetValue("Links", pLinksTable))
		{
			...
			while (pLinksTable->MoveNext(iter))
			{
				...
				{
					tukk sLinkName = iter.value.GetString();
...
{
	string line;
	line.Format("%s:%s", (tukk)GetName(), (tukk)method);

	// Search this line in script file.
	i32 lineNum = FindLineNum(line);
	if (lineNum >= 0)
	{
...
void CEntityScript::AddMethod(const string& method)
{
	...
	if (f)
	{
		fprintf(f, "\n");
		fprintf(f, "-------------------------------------------------------\n");
		fprintf(f, "function %s:%s()\n", (tukk)m_pClass->GetName(), (tukk)method);
		fprintf(f, "end\n");
		fclose(f);
	}
}
...
//////////////////////////////////////////////////////////////////////////
i32 CEntityScript::FindLineNum(const string& line)
{
	...
	i32 size = ftell(fileHandle);
	fseek(fileHandle, 0, SEEK_SET);

	tuk text = new char[size + 16];
	fread(text, size, 1, fileHandle);
	text[size] = 0;

	tuk token = strtok(text, "\n");
	while (token)
	{
...
//////////////////////////////////////////////////////////////////////////
void CEntityScript::CopyPropertiesToScriptInternal(IEntity* pEntity, CVarBlock* pVarBlock, bool bCallUpdate, tukk tableKey)
{
	if (!IsValid())
	{
		return;
	...
	for (i32 i = 0; i < pVarBlock->GetNumVariables(); i++)
	{
...
void CEntityScript::VarToScriptTable(IVariable* pVariable, IScriptTable* pScriptTable)
{
	...
	{
		i32 type = pScriptTable->GetValueType(pVariable->GetName());
		if (type != svtObject)
		{
			return;
		}
		...
		{
			for (i32 i = 0; i < pVariable->GetNumVariables(); i++)
			{
	...
	tukk name = pVariable->GetName();
	i32 type = pScriptTable->GetValueType(name);

	if (type == svtString)
	{
		string value;
		pVariable->Get(value);
		pScriptTable->SetValue(name, (tukk)value);
	}
...
//////////////////////////////////////////////////////////////////////////
void CEntityScript::CopyPropertiesFromScriptInternal(IEntity* pEntity, CVarBlock* pVarBlock, tukk tableKey)
{
	if (!IsValid())
	{
		return;
...
void CEntityScript::RunMethod(IEntity* pEntity, const string& method)
{
	...
	Script::CallMethod(scriptTable, (tukk)method);
}
...
void CEntityScript::SetEventsTable(CEntityObject* pEntity)
{
	...
	for (i32 i = 0; i < pEntity->GetEventTargetCount(); i++)
	{
	...
	for (std::set<string>::iterator it = sourceEvents.begin(); it != sourceEvents.end(); it++)
	{
		...
		for (i32 i = 0; i < pEntity->GetEventTargetCount(); i++)
		{
			...
			if (stricmp(et.sourceEvent, sourceEvent) == 0)
			{
				...
				pTrgEvent->SetAt(2, (tukk)et.event);
			}
		}
	}
}
...
void CEntityScriptRegistry::OnEditorNotifyEvent(EEditorNotifyEvent event)
{
	...
	{
		m_needsScriptReload = false;
		CDrxEditApp::GetInstance()->OnReloadEntityScripts();
	}
}

std::shared_ptr<CEntityScript> CEntityScriptRegistry::Find(tukk szName) const
{
...
std::shared_ptr<CEntityScript> CEntityScriptRegistry::Insert(IEntityClass* pClass, tukk szAlias)
{
	if (strlen(szAlias) == 0)
	{
