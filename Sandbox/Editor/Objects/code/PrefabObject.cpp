...
public:
	CScopedPrefabEventsDelay()
	{
		...
		DRX_ASSERT(pPrefabEvents != nullptr);
...
bool CPrefabChildGuidProvider::IsValidChildGUid(const DrxGUID& id, CPrefabObject* pPrefabObject)
{
	const auto& prefabGuid = pPrefabObject->GetId();
	return (prefabGuid.hipart ^ prefabGuid.lopart) == id.hipart;
}

DrxGUID CPrefabChildGuidProvider::GetFrom(const DrxGUID& loadedGuid) const
{
	const auto& prefabGuid = m_pPrefabObject->GetId();
	return DrxGUID(prefabGuid.hipart ^ prefabGuid.lopart, loadedGuid.hipart);
}

//////////////////////////////////////////////////////////////////////////
CUndoChangePivot::CUndoChangePivot(CBaseObject* pObj, tukk undoDescription)
{
...
//////////////////////////////////////////////////////////////////////////
tukk CUndoChangePivot::GetObjectName()
{
...
bool CPrefabObject::Init(CBaseObject* prev, const string& file)
{
	...
	if (!file.IsEmpty())
	{
		SetPrefabGuid(DrxGUID::FromString(file));
	}
	return res;
}
...
void CPrefabObject::ConvertToProceduralObject()
{
	...
	if (!pObject)
	{
		...
		DrxWarning(VALIDATOR_MODULE_ENTITYSYSTEM, VALIDATOR_ERROR, "Conversion Failure.");
		return;
	}
...
void CPrefabObject::RecursivelyDisplayObject(CBaseObject* obj, CObjectRenderHelper& objRenderHelper)
{
	...
	//////////////////////////////////////////////////////////////////////////
	i32 numObjects = obj->GetChildCount();
	for (i32 i = 0; i < numObjects; i++)
		RecursivelyDisplayObject(obj->GetChild(i), objRenderHelper);

	i32 numLinkedObjects = obj->GetLinkedObjectCount();
	for (i32 i = 0; i < numLinkedObjects; ++i)
...
{
	for (i32 i = 0; i < obj->GetChildCount(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CPrefabObject::SetPrefab(DrxGUID guid, bool bForceReload)
{
	...
	else
	{
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Cannot find Prefab %s with GUID: %s for Object %s %s",
		           (tukk)m_prefabName, guid.ToString(), (tukk)GetName(),
...
void CPrefabObject::SetPrefab(CPrefabItem* pPrefab, bool bForceReload)
{
	...
	DRX_ASSERT(pPrefabManager != nullptr);
	...
	if (!objectsXml)
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Prefab %s does not contain objects %s", (tukk)m_prefabName,
		           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", GetName()));
		return;
	}
	...
	//force using this ID, incremental. Keep high part of DrxGUID (stays compatible with old GUID.Data1)
...
{
	i32 numObjects = ar.GetLoadedObjectsCount();
...
{
	if (object->GetIdInPrefab() == DrxGUID::Null())
...
void CPrefabObject::SerializeMembers(Serialization::IArchive& ar)
{
	...
	if (ar.isInput())
	{
		...
		if (changed)
		{
			...
			std::unordered_set<DrxGUID> childGuids;
...
void CPrefabObject::CloneAll(std::vector<CBaseObject*>& extractedObjects)
{
	...
	CScopedSuspendUndo suspendUndo;
	for (i32 i = 0, numObjects = clonedObjectArchive.GetLoadedObjectsCount(); i < numObjects; ++i)
	{
		CBaseObject* pClonedObject = clonedObjectArchive.GetLoadedObject(i);

		// Add to selection
		pClonedObject->SetIdInPrefab(DrxGUID::Null());
...
void CPrefabObject::CloneSelected(CSelectionGroup* pSelectedGroup, std::vector<CBaseObject*>& clonedObjects)
{
	...
	std::map<DrxGUID, XmlNodeRef> objects;
	for (i32 i = 0, count = pSelectedGroup->GetCount(); i < count; ++i)
	{
		...
		DrxGUID cloneObjectID = DrxGUID::Null();
	...
	for (i32 i = 0, count = objectsNode->getChildCount(); i < count; ++i)
	{
		XmlNodeRef object = objectsNode->getChild(i);
		DrxGUID objectID = DrxGUID::Null();
...
void CPrefabObject::AddMember(CBaseObject* pObj, bool bKeepPos /*=true */)
{
	...
	DrxGUID newGuid = CPrefabChildGuidProvider(this).GetFor(pObj);
...
void CPrefabObject::SyncPrefab(const SObjectChangedContext& context)
{
	...
	if (context.m_operation == eOCOT_Delete)
	{
		...
		if (pObj && pObj->IsKindOf(RUNTIME_CLASS(CGroup)) && !pObj->IsKindOf(RUNTIME_CLASS(CPrefabObject)))
		{
			...
			for (i32 i = 0, count = pObj->GetChildCount(); i < count; ++i)
			{
				children.push_back(pObj->GetChild(i));
			}
			for (i32 i = 0, count = children.size(); i < count; ++i)
			{
...
//////////////////////////////////////////////////////////////////////////
void CPrefabObject::SetWorldTM(const Matrix34& tm, i32 flags /* = 0 */)
{
...
//////////////////////////////////////////////////////////////////////////
void CPrefabObject::SetWorldPos(const Vec3& pos, i32 flags /* = 0 */)
{
	if (m_bChangePivotPoint)
		SetPivot(pos);
	else
...
//////////////////////////////////////////////////////////////////////////
void CPrefabObject::SetMaterialLayersMask(u32 nLayersMask)
{
	for (i32 i = 0; i < GetChildCount(); i++)
	{
		if (GetChild(i)->CheckFlags(OBJFLAG_PREFAB))
...
void CPrefabObject::SetName(const string& name)
{
	...
	{
		CPrefabManager* pPrefabManager = GetIEditor()->GetPrefabManager();
		DRX_ASSERT(pPrefabManager != NULL);
		CPrefabEvents* pPrefabEvents = pPrefabManager->GetPrefabEvents();
		DRX_ASSERT(pPrefabEvents != NULL);
...
bool CPrefabObject::HitTestMembers(HitContext& hcOrg)
{
	...
	i32 numberOfChildren = allChildrenObj.size();
	for (i32 i = 0; i < numberOfChildren; ++i)
	{
...
void CPrefabObject::UpdatePivot(const Vec3& newWorldPivotPos)
{
	...
	for (i32 i = 0, iCount(objects.size()); i < iCount; ++i)
	{
...
namespace
{
boost::python::tuple PyGetPrefabOfChild(tukk pObjName)
{
	...
	else if (GetIEditor()->GetObjectManager()->FindObject(DrxGUID::FromString(pObjName)))
		pObject = GetIEditor()->GetObjectManager()->FindObject(DrxGUID::FromString(pObjName));
	else
	{
...
static void PyNewPrefabFromSelection(tukk libraryName, tukk groupName, tukk itemName)
{
...
static void PyDeletePrefabItem(tukk itemName, tukk libraryName)
{
	IDataBaseLibrary* pLibrary = GetIEditor()->GetPrefabManager()->FindLibrary(libraryName);
	if (pLibrary == NULL)
		IDataBaseLibrary* pLibrary = GetIEditor()->GetPrefabManager()->LoadLibrary(libraryName);
...
static std::vector<STxt> PyGetPrefabLibrarys()
{
	CPrefabManager* pPrefabManager = GetIEditor()->GetPrefabManager();
	if (!pPrefabManager)
	{
	...
	std::vector<STxt> results;
	i32 intLibCount = pPrefabManager->GetLibraryCount();
	for (i32 i = 0; i < pPrefabManager->GetLibraryCount(); i++)
	{
		IDataBaseLibrary* pPrefabLibrary = pPrefabManager->GetLibrary(i);
		results.push_back(static_cast<STxt>(pPrefabLibrary->GetName()));
	}
	return results;
}

static std::vector<STxt> PyGetPrefabGroups(tukk pLibraryName)
{
	std::vector<STxt> results;
	...
	for (i32 i = 0; i < pPrefabLibrary->GetItemCount(); i++)
	{
		CPrefabItem* pPrefabItem = static_cast<CPrefabItem*>(pPrefabLibrary->GetItem(i));
		stl::push_back_unique(results, static_cast<STxt>(pPrefabItem->GetGroupName()));
	}
	return results;
}

static std::vector<STxt> PyGetPrefabItems(tukk pLibraryName, tukk pGroupName)
{
	std::vector<STxt> results;
	...
	for (i32 i = 0; i < pPrefabLibrary->GetItemCount(); i++)
	{
		...
		{
			stl::push_back_unique(results, static_cast<STxt>(pPrefabItem->GetShortName()));
		}
	}
	return results;
}

boost::python::tuple PyGetPrefabChildWorldPos(tukk pObjName, tukk pChildName)
{
	CBaseObject* pObject;
	if (GetIEditor()->GetObjectManager()->FindObject(pObjName))
		pObject = GetIEditor()->GetObjectManager()->FindObject(pObjName);
	...
	for (i32 i = 0, iChildCount(pObject->GetChildCount()); i < iChildCount; ++i)
	{
...
static bool PyHasPrefabLibrary(tukk pLibraryName)
{
	CPrefabManager* pPrefabManager = GetIEditor()->GetPrefabManager();
	if (!pPrefabManager)
	{
...
static bool PyHasPrefabGroup(tukk pLibraryName, tukk pGroupName)
{
	CPrefabManager* pPrefabManager = GetIEditor()->GetPrefabManager();
	if (!pPrefabManager)
	{
...
static bool PyHasPrefabItem(tukk pLibraryName, tukk pItemName)
{
	CPrefabManager* pPrefabManager = GetIEditor()->GetPrefabManager();
	if (!pPrefabManager)
	{
