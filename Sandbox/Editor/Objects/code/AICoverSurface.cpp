...
void CAICoverSurface::ValidateGenerated()
{
	...
	if (!m_surfaceID ||	!gEnv->pAISystem->GetCoverSystem()->GetSurfaceInfo(m_surfaceID, &surfaceInfo) || (surfaceInfo.sampleCount < 2))
	{
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "AI Cover Surface '%s' at (%.2f, %.2f, %.2f) is empty! %s", (tukk)GetName(), pos.x, pos.y, pos.z,
...
{
	u32 sampleCount = m_sampler->GetSampleCount();
	if (sampleCount)
	{
		ICoverSystem::SurfaceInfo surfaceInfo;
		...
		for (u32 i = 0; i < surfaceInfo.sampleCount; ++i)
		{
...
void CAICoverSurface::InvalidateTM(i32 whyFlags)
{
