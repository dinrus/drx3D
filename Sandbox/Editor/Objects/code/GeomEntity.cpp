...
void CGeomEntity::SetGeometryFile(const string& filename)
{
	...
	if (stricmp(szExt, DRX_SKEL_FILE_EXT) == 0 || stricmp(szExt, DRX_CHARACTER_DEFINITION_FILE_EXT) == 0 || stricmp(szExt, DRX_ANIM_GEOMETRY_FILE_EXT) == 0)
	{
...
bool CGeomEntity::ConvertFromObject(CBaseObject* object)
{
	...
	if (object->IsKindOf(RUNTIME_CLASS(CBrushObject)))
	{
		...
		i32 rndFlags = pBrushObject->GetRenderFlags();
