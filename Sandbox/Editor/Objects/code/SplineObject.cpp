...
public:
	...
	void           OnManipulatorDrag(IDisplayViewport* pView, ITransformManipulator* pManipulator, const Vec2i& point0, const Vec3& value, i32 flags);
	void           OnManipulatorBegin(IDisplayViewport* view, ITransformManipulator* pManipulator, const Vec2i& point, i32 flags);
	...
	void           OnSplineEvent(CBaseObject* pObj, i32 evt);
...
//////////////////////////////////////////////////////////////////////////
void CEditSplineObjectTool::SetUserData(tukk key, uk userData)
{
...
void CEditSplineObjectTool::GetManipulatorPosition(Vec3& position)
{
	...
	i32 index = m_pSpline->GetSelectedPoint();
...
//////////////////////////////////////////////////////////////////////////
bool CEditSplineObjectTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
...
void CEditSplineObjectTool::OnSplineEvent(CBaseObject* pObj, i32 evt)
{
...
//////////////////////////////////////////////////////////////////////////
void CEditSplineObjectTool::SelectPoint(i32 index)
{
	if (index < 0)
...
//////////////////////////////////////////////////////////////////////////
void CEditSplineObjectTool::OnManipulatorDrag(IDisplayViewport* pView, ITransformManipulator* pManipulator, const Vec2i& point0, const Vec3& value, i32 flags)
{
...
void CEditSplineObjectTool::OnManipulatorBegin(IDisplayViewport* view, ITransformManipulator* pManipulator, const Vec2i& point, i32 flags)
{
	i32 editMode = GetIEditorImpl()->GetEditMode();
...
//////////////////////////////////////////////////////////////////////////
bool CEditSplineObjectTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
	if (!m_pSpline)
		return false;
	...
	if (event == eMouseLDown || event == eMouseMove || event == eMouseLDblClick || event == eMouseLUp)
	{
		...
		if ((flags & MK_CONTROL) && !m_modifying)
		{
			...
			if (p1 >= 0 && p2 >= 0 && dist < fSplineCloseDistance + view->GetSelectionTolerance())
			{
				...
				if (event == eMouseLDown)
				{
					...
					i32 index = m_pSpline->InsertPoint(p2, intPnt);
					SelectPoint(index);
		...
		i32 index = m_pSpline->GetNearestPoint(raySrc, rayDir, dist);
...
//////////////////////////////////////////////////////////////////////////
void CSplitSplineObjectTool::SetUserData(tukk key, uk userData)
{
	m_curPoint = -1;
	m_pSpline = (CSplineObject*)userData;
	assert(m_pSpline != 0);
...
//////////////////////////////////////////////////////////////////////////
bool CSplitSplineObjectTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
	if (nChar == Qt::Key_Escape)
	{
		GetIEditorImpl()->SetEditTool(0);
...
//////////////////////////////////////////////////////////////////////////
bool CSplitSplineObjectTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
	if (!m_pSpline)
		return false;
...
//////////////////////////////////////////////////////////////////////////
void CMergeSplineObjectsTool::SetUserData(tukk key, uk userData)
{
	m_pSpline = (CSplineObject*)userData;
	assert(m_pSpline != 0);
...
//////////////////////////////////////////////////////////////////////////
bool CMergeSplineObjectsTool::OnKeyDown(CViewport* view, u32 nChar, u32 nRepCnt, u32 nFlags)
{
	if (nChar == Qt::Key_Escape)
	{
		GetIEditorImpl()->SetEditTool(0);
...
//////////////////////////////////////////////////////////////////////////
bool CMergeSplineObjectsTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
	//return true;
	if (event == eMouseLDown || event == eMouseMove)
	{
		...
		i32 cnt = pSpline->GetPointCount();
		if (cnt < 2)
			return false;

		i32 p = pSpline->GetNearestPoint(raySrc, rayDir, dist);
		if (p != 0 && p != cnt - 1)
			return false;
...
//////////////////////////////////////////////////////////////////////////
void CSplineObject::SelectPoint(i32 index)
{
...
//////////////////////////////////////////////////////////////////////////
void CSplineObject::SetPoint(i32 index, const Vec3& pos)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CSplineObject::InsertPoint(i32 index, const Vec3& point)
{
	i32 newindex = GetPointCount();
...
//////////////////////////////////////////////////////////////////////////
void CSplineObject::RemovePoint(i32 index)
{
...
//////////////////////////////////////////////////////////////////////////
Vec3 CSplineObject::GetBezierPos(i32 index, float t) const
{
	float invt = 1.0f - t;
...
//////////////////////////////////////////////////////////////////////////
Vec3 CSplineObject::GetBezierTangent(i32 index, float t) const
{
	float invt = 1.0f - t;
...
//////////////////////////////////////////////////////////////////////////
float CSplineObject::GetBezierSegmentLength(i32 index, float t) const
{
	i32k kSteps = 32;
...
{
	float fRet = 0.f;
	for (i32 i = 0, numSegments = GetPointCount() - 1; i < numSegments; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
float CSplineObject::GetPosByDistance(float distance, i32& outIndex) const
{
	float lenPos = 0.0f;
	i32 index = 0;
	float segmentLength = 0.0f;
	for (i32 numSegments = GetPointCount() - 1; index < numSegments; ++index)
	{
...
//////////////////////////////////////////////////////////////////////////
Vec3 CSplineObject::GetBezierNormal(i32 index, float t) const
{
	float kof = 0.0f;
	i32 i = index;
...
//////////////////////////////////////////////////////////////////////////
Vec3 CSplineObject::GetLocalBezierNormal(i32 index, float t) const
{
	float kof = t;
	i32 i = index;
...
//////////////////////////////////////////////////////////////////////////
void CSplineObject::BezierCorrection(i32 index)
{
...
//////////////////////////////////////////////////////////////////////////
void CSplineObject::BezierAnglesCorrection(i32 index)
{
	i32 maxindex = GetPointCount() - 1;
...
//////////////////////////////////////////////////////////////////////////
void CSplineObject::Split(i32 index, const Vec3& point)
{
	// do not change the order of things here
	i32 in0 = InsertPoint(index, point);

	CSplineObject* pNewShape = (CSplineObject*) GetObjectManager()->CloneObject(this);

	for (i32 i = in0 + 1, cnt = GetPointCount(); i < cnt; ++i)
	{
		RemovePoint(in0 + 1);
	}
	if (pNewShape)
...
void CSplineObject::Merge(CSplineObject* pSpline)
{
	...
	for (i32 i = 0; i < pSpline->GetPointCount(); ++i)
	{
...
void CSplineObject::SerializeProperties(Serialization::IArchive& ar, bool bMultiEdit)
{
	...
	else
	{
		...
		{
			i32 selpoint = m_selectedPoint + 1;
...
void CSplineObject::CalcBBox()
{
	...
	{
		for (i32 i = 0, n = GetPointCount(); i < n; ++i)
		{
...
//////////////////////////////////////////////////////////////////////////
i32 CSplineObject::GetNearestPoint(const Vec3& raySrc, const Vec3& rayDir, float& distance)
{
	i32 index = -1;
...
bool CSplineObject::HitTest(HitContext& hc)
{
	...
	i32 maxpoint = GetPointCount();
	if (maxpoint-- >= GetMinPoints())
	{
		for (i32 i = 0; i < maxpoint; ++i)
		{
			i32 kn = 6;
			for (i32 k = 0; k < kn; ++k)
			{
...
bool CSplineObject::HitTestRect(HitContext& hc)
{
	...
	if (maxpoint-- >= GetMinPoints())
	{
		for (i32 i = 0, j = 1; i < maxpoint; ++i, ++j)
		{
			Vec3 p0 = wtm.TransformPoint(m_points[i].pos);
...
//////////////////////////////////////////////////////////////////////////
void CSplineObject::GetNearestEdge(const Vec3& raySrc, const Vec3& rayDir, i32& p1, i32& p2, float& distance, Vec3& intersectPoint)
{
	p1 = -1;
	p2 = -1;
	const float kBigRayLen = 100000.0f;
...
void CSplineObject::Serialize(CObjectArchive& ar)
{
	...
	if (ar.bLoading)
	{
		...
		if (points)
		{
			for (i32 i = 0, n = points->getChildCount(); i < n; ++i)
			{
...
//////////////////////////////////////////////////////////////////////////
i32 CSplineObject::MouseCreateCallback(IDisplayViewport* pView, EMouseEvent event, CPoint& point, i32 flags)
{
