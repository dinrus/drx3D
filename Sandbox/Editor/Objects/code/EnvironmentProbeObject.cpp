...
#include "../Drx3DEngine/StatObj.h"
#include "../Drx3DEngine/Material.h"
#include "../Drx3DEngine/MatMan.h"
...
{
public:
	CubeMapGenerationSchedule(const string& filepath, const string& relFilepath, DrxGUID cubeguid, i32 cubemapres)
	...
	virtual void OnCompilationFinished(tukk source, tukk target, ERcExitCode eReturnCode) override
	{
		...
		{
			DrxInterlockedIncrement(&m_texdone);
...
protected:
	DrxGUID      m_cubemapguid;
	string      m_filepath;
	string      m_relFilepath;
	volatile i32 m_texdone;
...
{
	CVarEnumList<i32>* enumList = new CVarEnumList<i32>;
...
IMaterial* CEnvironementProbeObject::CreateMaterial()
{
	...
	if (pMatSrc)
	{
		IMaterial* pMatDst = pMatMan->CreateMaterial((tukk)matName, pMatSrc->GetFlags() | MTL_FLAG_NON_REMOVABLE);
		if (pMatDst)
		{
...
i32 CEnvironementProbeObject::AddEntityLink(const string& name, DrxGUID targetEntityId)
{
	i32 ret = CEntityObject::AddEntityLink(name, targetEntityId);
	if (ret != -1)
	{
		UpdateLinks();
	}
