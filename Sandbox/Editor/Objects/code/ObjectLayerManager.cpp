...
protected:
	virtual tukk GetDescription() { return m_bCreate ? "Create Layer" : "Delete Layer"; };
...
protected:
	virtual tukk GetDescription() { return "Select Layer"; };
...
CObjectLayerManager::CObjectLayerManager(CObjectManager* pObjectManager) :
	...
	m_visibleSetLayer(DrxGUID::Null())
{
...
CObjectLayer* CObjectLayerManager::CreateLayer(EObjectLayerType layerType/*= eObjectLayerType_Layer*/, CObjectLayer* pParent/*= nullptr*/)
{
	...
	if (layerType == eObjectLayerType_Layer)
		...
		DRX_ASSERT_MESSAGE(0, "Please define a default name for this layer type");

	string finalName = name;

	i32 count = 0;
	while (FindAnyLayerByName(finalName.c_str()))
		finalName = name + "_" + DrxStringUtils::toString(++count);

	return CreateLayer(finalName.c_str(), layerType, pParent);
}

CObjectLayer* CObjectLayerManager::CreateLayer(tukk szName, EObjectLayerType layerType/*= eObjectLayerType_Layer*/, CObjectLayer* pParent/*= nullptr*/)
{
	using namespace Private_ObjectLayerManager;
	DRX_ASSERT((!pParent || pParent->GetLayerType() == eObjectLayerType_Folder) && layerType != eObjectLayerType_Size);
...
void CObjectLayerManager::AddLayer(CObjectLayer* pLayer, bool bNotify /*= true*/, bool loading /* = false */)
{
	...
	if (!m_bOverwriteDuplicates)
	{
		...
		if (pPrevLayer)
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "%s layer already exists. Layer names must be unique", (tukk)pLayer->GetName());
			return;
		}
		...
		if (m_layersMap.find(pLayer->GetGUID()) != m_layersMap.end())
		{
			...
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Duplicate Layer GUID,Layer <%s> collides with layer: %s",
			           (tukk)pPrevLayer->GetName(), (tukk)pLayer->GetName());
			return;
		}
	}
...
void CObjectLayerManager::DeleteLayer(CObjectLayer* pLayer, bool bNotify /*= true*/)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Cannot delete layer %s", pLayer->GetName());
		return;
	}
...
bool CObjectLayerManager::CanRename(const CObjectLayer* pLayer, tukk szNewName) const
{
...
void CObjectLayerManager::SetLayerName(CObjectLayer* pLayer, tukk szNewName, bool IsUpdateDepends)
{
...
CObjectLayer* CObjectLayerManager::FindLayer(DrxGUID guid) const
{
...
void CObjectLayerManager::Serialize(CObjectArchive& ar)
{
	...
	if (ar.bLoading)
	{
		...
		if (layers)
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Trying to load layers with unsupported format version %d", nVers);
				return;
			}

			DrxGUID currentLayerGUID;
			...
			if (!GetCurrentLayer())
			{
				...
				if (!GetCurrentLayer())
				{
					DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Sandbox needs an active layer to work correctly");
				}
			}
		}
...
CObjectLayer* CObjectLayerManager::ImportLayer(CObjectArchive& ar, bool bNotify /*= true*/)
{
	...
	if (layerObjects)
	{
		i32 numObjects = layerObjects->getChildCount();
...
CObjectLayer* CObjectLayerManager::ImportLayerFromFile(tukk filename, bool bNotify /*= true*/, CObjectArchive* globalArchive /* = nullptr */)
{
	if (CFileUtil::FileExists(filename))
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Failed to import layer file '%s'", filename);
		}
	}
	else
	{
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Layer file '%s' not found", filename);
	}
...
bool CObjectLayerManager::InitLayerSwitches(bool isOnlyClear)
{
	...
	u16 curLayerId = 0;
	bool bRet = false;
	std::map<string, u16> layerNameMap;
	for (LayersMap::const_iterator it = m_layersMap.begin(); it != m_layersMap.end(); ++it)
	{
		CObjectLayer* pLayer = it->second;
		std::map<string, u16>::const_iterator it2 = layerNameMap.find(pLayer->GetName());
...
void CObjectLayerManager::FreezeROnly()
{
	for (LayersMap::const_iterator it = m_layersMap.begin(); it != m_layersMap.end(); ++it)
	{
		...
		{
			string file = pLayer->GetLayerFilepath();
			u32 attr = CFileUtil::GetAttributes(file);
...
bool CObjectLayerManager::ReloadLayer(CObjectLayer* pLayer)
{
	if (pLayer)
	{
		...
		if (!CFileUtil::FileExists(path))
		{
			...
			DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, "Layer file cannot be found - maybe layer was renamed?");
			return false;
		}
	...
	if (pMtl)
	{
		...
		if (shaderItem.m_pShaderResources)
		{
			for (i32 texSlot = 0; texSlot < EFTT_MAX; ++texSlot)
			{
				...
				if (pTex)
				{
					tukk pExt = PathUtil::GetExt(pTex->m_Name.c_str());
...
XmlNodeRef CObjectLayerManager::GenerateDynTexSrcLayerInfo() const
{
	...
	for (size_t i = 0; i < objects.size(); ++i)
	{
		...
		if (pObj)
		{
			...
			if (pMat)
			{
				tukk pMtlName = pMat->GetName();
				assert(pMtlName);

				bool foundDynTexSrc = false;

				i32k numSubMtls = pMat->GetSubMaterialCount();
				if (!numSubMtls)
				{
				...
				{
					for (i32 subMtlIdx = 0; subMtlIdx < numSubMtls; ++subMtlIdx)
					{
				...
				if (foundDynTexSrc)
				{
					...
					tukk pLayerName = pLayer ? pLayer->GetName() : 0;
					tukk pObjName = pObj->GetName();
					tukk pMtlName = pMat->GetName();
					...
					if (itMtl != mtlSharingMap.end())
					{
						...
						if (!sharedObjList.crossLayerSharingDetected)
						{
							...
							tukk pLayerNameCmp = pLayerCmp ? pLayerCmp->GetName() : 0;
	...
	for (MtlSharingMap::const_iterator it = mtlSharingMap.begin(); it != mtlSharingMap.end(); ++it)
	{
		...
		else
		{
			...
			for (size_t i = 0; i < list.size(); ++i)
			{
				...
				DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_WARNING, "Cross layer sharing of Flash material \"%s\" detected for \"%s\". %s"
				                                                       "Associated dynamic texture source(s) will not partake in layer activation!", pMaterialName, pObjName,
				           DrxLinkService::CDrxLinkUriFactory::GetUriV("Editor", "general.select_and_go_to_object %s", pObjName));
			}
		}
	}
	...
	for (IgnoredMtlSet::const_iterator it = ignoredMtlSet.begin(); it != ignoredMtlSet.end(); ++it)
	{
		...
		DrxWarning(VALIDATOR_MODULE_EDITOR, VALIDATOR_COMMENT, "Ignored Flash material \"%s\" for layer activation.", pMtlName);
	}
