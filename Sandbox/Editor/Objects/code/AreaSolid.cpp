...
i32 CAreaSolid::s_nGlobalFileAreaSolidID = 0;

IMPLEMENT_DYNCREATE(CAreaSolid, CEntityObject)

i32 CAreaSolid::m_nRollupId = 0;
CPickEntitiesPanel* CAreaSolid::m_pPanel = NULL;

i32 CAreaSolid::m_nActivateEditAreaSolidRollUpID = 0;
CAreaSolidPanel* CAreaSolid::m_pActivateEditAreaSolidPanel = NULL;

class CAreaSolidSizer : public IDrxSizer
{
public:
	CAreaSolidSizer()
	{
	...
	bool AddObject(ukk pIdentifier, size_t nSizeBytes, i32 nCount)
	{
		m_size += nSizeBytes;
		m_count += nCount;
		return true;
	...
	void Push(tukk szComponentName)                {}
	void PushSubcomponent(tukk szSubcomponentName) {}
	void Pop()                                            {}
	void Reset()
	{
		m_count = 0;
...
//////////////////////////////////////////////////////////////////////////
void CAreaSolid::BeginEditParams(IEditor* ie, i32 flags)
{
...
//////////////////////////////////////////////////////////////////////////
void CAreaSolid::SetAreaId(i32 nAreaId)
{
	m_areaId = nAreaId;
	UpdateGameArea();
}
...
void CAreaSolid::UpdateGameArea()
{
	...
	pArea->ClearEntities();
	for (i32 i = 0; i < GetEntityCount(); i++)
	{
	...
	for (i32 i = 0, iRegionSize(regionList.size()); i < iRegionSize; ++i)
	{
		...
		for (i32 k = 0, iSize(innerRegions.size()); k < iSize; ++k)
		{
...
{
	for (i32 a = 0, iFaceSize(faceList.size()); a < iFaceSize; ++a)
	{
		std::vector<Vec3> convex;
		for (i32 b = 0, iIndexSize(faceList[a]->m_IndexList.size()); b < iIndexSize; ++b)
...
void CAreaSolid::InvalidateTM(i32 nWhyFlags)
{
