...
public:
	...
	CBaseObject* GetObject(i32 index) const;
	//! Get object from a GUID
	CBaseObject* GetObjectByGuid(DrxGUID guid) const;
	//! Get object from a GUID in a prefab
	CBaseObject* GetObjectByGuidInPrefab(DrxGUID guid) const;
	...
	i32          GetFilteredCount() const       { return m_filtered.size(); }
	CBaseObject* GetFilteredObject(i32 i) const { return m_filtered[i]; }
	...
	void Move(const Vec3& offset, i32 moveFlags, i32 referenceCoordSys, const CPoint& point = CPoint(-1, -1), bool bFromInitPos = false) const;
	//! Move objects in selection to specific position.
	void MoveTo(const Vec3& pos, EMoveSelectionFlag moveFlag, i32 referenceCoordSys, const CPoint& point = CPoint(-1, -1)) const;
	//! Rotate objects in selection by given quaternion.
	void Rotate(const Quat& qRot, i32 referenceCoordSys) const;
	//! Rotate objects in selection by given angle.
	void Rotate(const Ang3& angles, i32 referenceCoordSys) const;
	//! Rotate objects in selection by given rotation matrix.
	void Rotate(const Matrix34& matRot, i32 referenceCoordSys) const;
	//! Transforms objects
	void Transform(const Vec3& offset, EMoveSelectionFlag moveFlag, const Ang3& angles, const Vec3& scale, i32 referenceCoordSys) const;
	...
	void Scale(const Vec3& scale, i32 referenceCoordSys) const;
	void SetScale(const Vec3& scale, i32 referenceCoordSys) const;
