...
{
public:
	CDrxSignal<void(const CLayerChangeEvent&)> signalChangeEvent;
	...
	CObjectLayer* CreateLayer(tukk szName, EObjectLayerType layerType = eObjectLayerType_Layer, CObjectLayer* pParent = nullptr);
	...
	bool CanRename(const CObjectLayer* pLayer, tukk szNewName) const;
	//! Sets layer's name. Also adds path to set to be removed on save
	void SetLayerName(CObjectLayer* pLayer, tukk szNewName, bool IsUpdateDepends = false);
	...
	CObjectLayer* FindLayer(DrxGUID guid) const;
	...
	CObjectLayer* ImportLayerFromFile(tukk filename, bool bNotify = true, CObjectArchive* globalArchive = nullptr);
...
private:
	...
	typedef std::map<DrxGUID, TSmartPtr<CObjectLayer>> LayersMap;
