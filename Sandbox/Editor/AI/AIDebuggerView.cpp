...
#include <math.h>
#include <stdlib.h>
#include <DrxAISystem/IAIAction.h>
...
BOOL CAIDebuggerView::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	if (!m_hWnd)
	{
		...
		smallFont.lfHeight = (i32)(smallFont.lfHeight * 0.85f);
...
BOOL CAIDebuggerView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	if (m_timelineRect.PtInRect(pt) || m_rulerRect.PtInRect(pt))
	{
		...
		i32 x = pt.x - m_timelineRect.left;
...
void CAIDebuggerView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{
		i32 dx = point.x - m_dragStartPt.x;
		i32 dy = point.y - m_dragStartPt.y;
		...
		if (m_dragType == DRAG_PAN)
		{
			...
			if (!m_items.empty())
			{
				m_itemsOffset = (i32)m_dragStartVal2 + dy;
				i32 maxItemsOffset = max(0, m_listFullHeight - m_listRect.Height());
				if (m_itemsOffset > 0)
					m_itemsOffset = 0;
		...
		{
			m_listWidth = (i32)m_dragStartVal + dx;
			if (m_listWidth < 10) m_listWidth = 10;
			UpdateViewRects();
		}
		else if (m_dragType == DRAG_DETAILS_WIDTH)
		{
			m_detailsWidth = (i32)m_dragStartVal - dx;
		...
		else if (m_dragType == DRAG_ITEMS)
		{
			if (!m_items.empty())
			{
				m_itemsOffset = (i32)m_dragStartVal + dy;
				i32 maxItemsOffset = max(0, m_listFullHeight - m_listRect.Height());
				if (m_itemsOffset > 0)
					m_itemsOffset = 0;
				if (m_itemsOffset < -maxItemsOffset)
...
void CAIDebuggerView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	i32 rulerHit = HitTestRuler(point);
	i32 cursorHit = HitTestTimeCursor(point);
	i32 separatorHit = HitTestSeparators(point);
...
{
	tukk curLabel = GetLabelAtPoint(point);
	...
	{
		drx_sprintf(posLabel, "%.2f, %.2f, %.2f", pos.x, pos.y, pos.z);
		copyPos += posLabel;
		gotoPos += posLabel;
		hasPos = true;
	}
	...
	i32 cmd = menu.TrackPopupMenu(TPM_RETURNCMD | TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_NONOTIFY, screenPoint.x, screenPoint.y, this);
...
//////////////////////////////////////////////////////////////////////////
i32 CAIDebuggerView::HitTestRuler(CPoint pt)
{
	if (!m_rulerRect.PtInRect(pt))
		return RULERHIT_NONE;

	bool hasRuler(false);
	i32 rulerStartX, rulerEndX;
...
//////////////////////////////////////////////////////////////////////////
i32 CAIDebuggerView::HitTestTimeCursor(CPoint pt)
{
	if (!m_timelineRect.PtInRect(pt))
		return CURSORHIT_NONE;

	i32 cursorX = m_timelineRect.left + TimeToClient(m_timeCursor);
...
//////////////////////////////////////////////////////////////////////////
i32 CAIDebuggerView::HitTestSeparators(CPoint pt)
{
...
//////////////////////////////////////////////////////////////////////////
tukk CAIDebuggerView::GetLabelAtPoint(CPoint point)
{
	...
	for (TItemList::iterator it = m_items.begin(); it != m_items.end(); ++it)
	{
		...
		if (n >= 0 && n < (i32)m_streams.size() && m_streams[n].track == TRACK_TEXT)
		{
			...
			float curTime;
			pStream->Seek(t);
			return (tuk)(pStream->GetCurrent(curTime));
		}
	}
...
//////////////////////////////////////////////////////////////////////////
bool CAIDebuggerView::FindNext(tukk what, float start, float& t)
{
...
//////////////////////////////////////////////////////////////////////////
bool CAIDebuggerView::FindInStream(tukk what, float start, IAIDebugStream* pStream, float& t)
{
	float curTime;
	pStream->Seek(start);
	tuk pString = (tuk)(pStream->GetCurrent(curTime));

	while (pStream->GetCurrentIdx() < pStream->GetSize())
	{
		float nextTime;
		tuk pNextString = (tuk)(pStream->GetNext(nextTime));
...
//////////////////////////////////////////////////////////////////////////
bool CAIDebuggerView::IsStreamEmpty(const SItem& sItem, i32 streamType) const
{
	IAISystem* pAISystem(GetIEditor()->GetAI()->GetAISystem());
	assert(pAISystem);
...
{
	i32 rectHeight = m_listRect.Height();
	i32 maxItemsOffset = m_listFullHeight;
	SCROLLINFO si;
...
//////////////////////////////////////////////////////////////////////////
void CAIDebuggerView::OnSize(UINT nType, i32 cx, i32 cy)
{
	...
	i32 rectHeight = m_listRect.Height();
	i32 maxItemsOffset = max(0, m_listFullHeight - rectHeight);
	if (m_itemsOffset < -maxItemsOffset)
		m_itemsOffset = -maxItemsOffset;
...
//////////////////////////////////////////////////////////////////////////
inline i32 Blend(i32 a, i32 b, i32 c)
{
	return a + ((b - a) * c) / 255;
}

//////////////////////////////////////////////////////////////////////////
inline COLORREF Blend(COLORREF a, COLORREF b, i32 c)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CAIDebuggerView::TimeToClient(float t)
{
	return (i32)((t - m_timeStart) / m_timeScale);
}

//////////////////////////////////////////////////////////////////////////
float CAIDebuggerView::ClientToTime(i32 x)
{
...
CAIDebuggerView::SItem* CAIDebuggerView::ClientToItem(i32 clientY)
{
	i32k yListOffset = m_listRect.top + m_itemsOffset;
	...
	for (TItemList::iterator iter = m_items.begin(); iter != endIter; ++iter)
	{
		...
		i32k minY = yListOffset + item.y;
		i32k maxY = minY + item.h;
...
void CAIDebuggerView::DrawRuler(CDC* dc)
{
	...
	i32 ntick = (i32)ceilf(m_timelineRect.Width() / (float)TIME_TICK_SPACING);
	i32 w = ntick * TIME_TICK_SPACING;
	...
	i32 nfrac = max(-floorf(log10f(d)), 0.0f);
	i32 nsub = (i32)(d / powf(10.0f, floorf(log10f(d))));
	if (nsub < 2)
		nsub *= 10;
	// # of fractional digits to show
	char format[6];
	drx_sprintf(format, "%%.%df", nfrac); // simplest axis labels
	...
	i32 texth = textMetric.tmHeight;
	i32 y = m_rulerRect.bottom - 2 - texth;
	...
	i32 steps = (graphMax - graphMin) / d + 1;

	float t = graphMin;
	for (i32 i = 0; i < steps; i++, t = graphMin + i * d)
	{
		label.Format(format, t);
		i32 x = m_timelineRect.left + TimeToClient(t);
		...
		for (i32 i = 1; i < nsub; i++)
		{
			float t2 = t + ((float)i / (float)nsub) * d;
			i32 x2 = m_timelineRect.left + TimeToClient(t2);
...

	{
		...
		i32 cursorX = m_rulerRect.left + TimeToClient(m_timeCursor);
...
//////////////////////////////////////////////////////////////////////////
void CAIDebuggerView::DrawStreamDetail(CDC* dc, IAIDebugStream* pStream, i32 iy, i32 ih, i32 texth, COLORREF color)
{
	float curTime;
...
//////////////////////////////////////////////////////////////////////////
void CAIDebuggerView::DrawStreamFloatDetail(CDC* dc, IAIDebugStream* pStream, i32 iy, i32 ih, i32 texth, COLORREF color)
{
	float curTime;
	pStream->Seek(m_timeCursor);
...
//////////////////////////////////////////////////////////////////////////
void CAIDebuggerView::DrawStream(CDC* dc, IAIDebugStream* pStream, i32 iy, i32 ih, i32 texth, COLORREF color, i32 i, i32 width)
{
	...
	i32 streamStartX = CLAMP(TimeToClient(pStream->GetStartTime()), 0, width);
	i32 streamEndX = CLAMP(TimeToClient(pStream->GetEndTime()), 0, width);
...
//////////////////////////////////////////////////////////////////////////
void CAIDebuggerView::DrawStreamFloat(CDC* dc, IAIDebugStream* pStream, i32 iy, i32 ih, i32 texth, COLORREF color, i32 i, i32 width, float vmin, float vmax)
{
...
void CAIDebuggerView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	//	i32 max = GetScrollLimit(SB_VERT);

	i32 rectHeight = m_listRect.Height();
	i32 maxItemsOffset = max(0, m_listFullHeight - rectHeight);
	i32 pageSize = rectHeight;
	i32 iPosition = -m_itemsOffset;
