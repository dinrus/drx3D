...
// WritePolygonArea
//====================================================================
void CNavigation::WritePolygonArea(CDrxFile& file, const string& name, const ListPositions& pts)
{
	unsigned nameLen = name.length();
	if (nameLen >= maxForbiddenNameLen)
		nameLen = maxForbiddenNameLen - 1;
	file.Write(&nameLen, sizeof(nameLen));
	file.Write((tuk) name.c_str(), nameLen * sizeof(char));
	...
	for (it = pts.begin(); it != pts.end(); ++it)
	{
		...
		const Vec3& pt = *it;
		file.Write((uk ) &pt, sizeof(pt));
	}
}


void CNavigation::WriteAreasIntoFile(tukk fileName)
{
	CDrxFile file;
	if (false != file.Open(fileName, "wb"))
	{
		i32 fileVersion = BAI_AREA_FILE_VERSION_WRITE;
		...
		// Write designer paths
		{
			...
			for (it = m_mapDesignerPaths.begin(); it != m_mapDesignerPaths.end(); ++it)
			{
				...
				i32 navType = it->second.navType;
				file.Write(&navType, sizeof(navType));
				file.Write(&it->second.type, sizeof(i32));
		...
		// Write generic shapes
		{
			...
			for (it = m_mapGenericShapes.begin(); it != m_mapGenericShapes.end(); ++it)
			{
				...
				i32 lightLevel = (i32)it->second.lightLevel;
...
void CNavigation::ExportData(
	tukk navFileName, tukk areasFileName, tukk roadsFileName,
	tukk vertsFileName, tukk volumeFileName, tukk flightFileName)
{
	if (areasFileName)
	{
