...
private:
	...
	void  GetBox(Vec3& min, Vec3& max, i32 i, i32 j);
	AABB     m_aabb;
	unsigned m_nx, m_ny;
	...
	typedef std::vector<u8> TContainer;
...
struct SShape
{
	SShape() :
		...
		temporary(false), aabb(AABB::RESET), enabled(true), shapeMask(0), lightLevel(AILL_NONE), closed(false)
	{
	}

	explicit SShape(const ListPositions& shape_, const AABB& aabb_, IAISystem::ENavigationType navType = IAISystem::NAV_UNSET,
	                i32 type = 0, bool closed_ = false, float height_ = 0.0f, EAILightLevel lightLevel_ = AILL_NONE, bool temp = false) :
		shape(shape_), aabb(aabb_), navType(IAISystem::NAV_UNSET), type(0), devalueTime(0), height(height_),
		temporary(temp), enabled(true), shapeMask(0), lightLevel(lightLevel_), closed(closed_)
	{
	}

	explicit SShape(const ListPositions& shape_, bool allowMask = false, IAISystem::ENavigationType navType_ = IAISystem::NAV_UNSET,
	                i32 type_ = 0, bool closed_ = false, float height_ = 0.0f, EAILightLevel lightLevel_ = AILL_NONE, bool temp = false);
	...
	i32 GetIntersectionDistances(const Vec3& start, const Vec3& end, float intersectDistArray[], i32 maxIntersections, bool testHeight = false, bool testTopBottom = false) const
	{
	...
	i32                        type;        /// AIobject/anchor type associated with an AIshape
...
inline SShape::SShape(const ListPositions& shape_, bool allowMask, IAISystem::ENavigationType navType_,
                      i32 type_, bool closed_, float height_, EAILightLevel lightLevel_, bool temp) :
	shape(shape_), navType(navType_), type(type_), devalueTime(0), height(height_),
	temporary(temp), enabled(true), shapeMask(0), lightLevel(lightLevel_), closed(closed_)
{
	RecalcAABB();
	if (allowMask)
...
// GetBox
//===================================================================
inline void CShapeMask::GetBox(Vec3& min, Vec3& max, i32 i, i32 j)
{
...
inline size_t CShapeMask::MemStats() const
{
	...
	size += m_container.capacity() * sizeof(u8);
	return size;
}
...
inline void CShapeMask::Build(const struct SShape* pShape, float granularity)
{
	...
	for (unsigned j = 0; j != m_ny; ++j)
	{
		for (unsigned i = 0; i != m_nx; ++i)
		{
			...
			u8 value = GetType(pShape, min, max) << offset;
			u8 mask = 3 << offset;
...
inline CShapeMask::EType CShapeMask::GetType(const Vec3 pt) const
{
	if (!useForbiddenMask)
		return TYPE_EDGE;
	i32 i = (i32) ((pt.x - m_aabb.min.x) / m_dx);
	if (i < 0 || i >= m_nx)
		return TYPE_OUT;
	i32 j = (i32) ((pt.x - m_aabb.min.y) / m_dx);
	if (j < 0 || j >= m_ny)
		return TYPE_OUT;

	unsigned index = (i >> 2) + m_bytesPerRow * j;
	assert(index < m_container.size());
	u8k& byte = m_container[index];
...
struct SPerceptionModifierShape : SShape
{
