...
#include <DrxAISystem/NavigationSystem/INavigationUpdatesManager.h>
...
public:
	...
	i32           id;          // id used in the rules to reference this template
...
struct SAINavigationPreferences : public SPreferencePage
{
	...
	virtual bool Serialize(yasli::Archive& ar) override
	{
		...
		for (i32 agentIndex = 0; agentIndex < agentTypeCount; ++agentIndex)
		{
			const NavigationAgentTypeID currentAgentID = pNavigationSystem->GetAgentTypeID(agentIndex);
			tukk szCurrentAgentName = pNavigationSystem->GetAgentTypeName(currentAgentID);
	...
	ADD_PREFERENCE_PAGE_PROPERTY(i32, navigationDebugAgentType, setNavigationDebugAgentType)
...
public:
	...
	virtual bool                IsReadyToGameExport(u32& adjustedExportFlags) const override;
	...
	tukk GetSmartObjectTemplateName(i32 index) const {}
	...
	string                            GetPathTypeName(i32 index);
	...
	tukk                       GetNavigationAgentTypeName(size_t i) const;
	...
	void                              RegenerateNavigationByTypeName(tukk szType);
...
private:
	...
	void               OnNavigationMeshChanged(NavigationAgentTypeID agentTypeId, NavigationMeshID navigationMeshId, u32 i);
	...
	static tukk GetNavigationWorldMonitorStateName(const ENavigationWorldMonitorState state);
	...
	typedef std::map<string, i32> AnchorActions;
	...
	i32                          m_MNMRegenerationPausedCount;
