...
i32k kMinQueueSizeToShowProgressNotification = 100;
	...
	for (i32 i = 0; i < (i32)objects.size(); i++)
	{
		...
		if (pObject && pObject->IsKindOf(RUNTIME_CLASS(CEntityObject)))
		{
			...
			if (pEntityObject->GetProperties2())
			{
				...
				{
					i32 nModelVariations = 0;
					p_nModelVariations->Get(nModelVariations);
					if (nModelVariations > 0)
					{
						i32 nVariation = 1 + (i32)(drx_random(0.0f, 1.0f) * nModelVariations);
						IVariable* p_nVariation = pEntityObject->GetProperties2()->FindVariable("nVariation", true);
						if (p_nVariation)
						{
...
void CAIManager::Update(u32 updateFlags)
{
	if (!m_pAISystem)
		return;
	...
	case eNotify_OnBeginGameMode:
		{
			...
			DRX_ASSERT_MESSAGE(pVar, "The cvar ai_MNMAllowDynamicRegenInEditor is not defined.");
			if (pVar)
			{
...
void CAIManager::LoadActionGraphs()
{
	...
	while (pAction = m_pAISystem->GetAIActionManager()->GetAIAction(i++))
	{
		...
		if (m_pFlowGraph)
		{
			// [3/24/2011 evgeny] Reconnect Sandbox CHyperFlowGraph to DrxAction CFlowGraph
...
//////////////////////////////////////////////////////////////////////////
i32 CAIManager::AnchorActionToId(tukk sAction) const
{
...
void CAIManager::GetSmartObjectStates(std::vector<string>& values) const
{
	...
	tukk sStateName;
	for (i32 i = 0; sStateName = m_pAISystem->GetSmartObjectManager()->GetSmartObjectStateName(i); ++i)
...
void CAIManager::GetSmartObjectActions(std::vector<string>& values) const
{
	...
	for (i32 i = 0; IAIAction* pAIAction = pAIActionManager->GetAIAction(i); ++i)
	{
		tukk szActionName = pAIAction->GetName();
		if (szActionName)
		{
...
//////////////////////////////////////////////////////////////////////////
void CAIManager::AddSmartObjectState(tukk sState)
{
	if (!m_pAISystem)
		return;
...
//////////////////////////////////////////////////////////////////////////
string CAIManager::GetPathTypeName(i32 index)
{
	DRX_ASSERT(index >= 0 && index < m_mapPFProperties.size());
...
void CAIManager::RegenerateNavigationByTypeName(tukk szType)
{
	if (!m_pAISystem)
		return;
...
bool CAIManager::IsReadyToGameExport(u32& adjustedExportFlags) const
{
...
tukk CAIManager::GetNavigationAgentTypeName(size_t i) const
{
...
void CAIManager::OnVisualizeNavigationAccessibilityChanged()
{
	...
	DRX_ASSERT_MESSAGE(pVar, "The cvar ai_MNMDebugAccessibility is not defined.");
	if (pVar)
	{
...
void CAIManager::OnNavigationMeshChanged(NavigationAgentTypeID, NavigationMeshID, u32)
{
...
void CAIManager::StartNavigationWorldMonitor()
{
	...
	default:
		DRX_ASSERT_MESSAGE(false, "Unknown state, should never happen");
	}
	static_assert(static_cast<i32>(ENavigationWorldMonitorState::StatesCount) == 5, "Invalid state count!");
}
...
/*static */ tukk CAIManager::GetNavigationWorldMonitorStateName(const ENavigationWorldMonitorState state)
{
	switch (state)
	{
	case ENavigationWorldMonitorState::Stopped:
...
{
	DrxLog("CAIManager: navigation world monitor state: %s -> %s",
...
{
	u32 queueSize = pNavigationSystem->GetWorkingQueueSize();
	if (queueSize > 0)
	{
		...
		if (m_queueInitSize > kMinQueueSizeToShowProgressNotification)
		{
			...
			if (m_pUpdateProgressNotification)
			{
				...
				QString progressString = "Updating MNM: " + QString::number(i32(progressValue * 100)) + "%";
				if (paused)
				{
