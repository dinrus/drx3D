...
BOOL CGenerationParam::OnInitDialog()
{
	...
	ctrlSlider.SetRange(11, 100, TRUE); // Has to be divided by 10
	ctrlSlider.Detach();
...
void CGenerationParam::UpdateStaticNum()
{
	...
	// Frequency
	drx_sprintf(szFloatNum, "%f", m_sldFrequency / 10.0f);
	SetDlgItemText(IDC_FREQUENCY_NUM, szFloatNum);

	// Frequency step
	drx_sprintf(szFloatNum, "%f", m_sldFrequencyStep / 10.0f);
	SetDlgItemText(IDC_FREQSTEP_NUM, szFloatNum);

	// Fade
	drx_sprintf(szFloatNum, "%f", m_sldFade / 10.0f);
	...
	// Sharpness
	drx_sprintf(szFloatNum, "%f", m_sldSharpness / 1000.0f);
...
void CGenerationParam::FillParam(SNoiseParams* pParam)
{
	...
	DrxLog("Retrieving parameters (fFade: %f, fFreq: %f, fFreqStep: %f) from dialog...",
...
void CGenerationParam::LoadParam(SNoiseParams* pParam)
{
	...
	m_sldFade = (i32) (pParam->fFade * 10.0f);
	m_sldFrequency = (i32) (pParam->fFrequency * 10.0f);
	m_sldFrequencyStep = (i32) (pParam->fFrequencyStep * 10.0f);
	m_sldCover = pParam->iCover;
	m_sldPasses = pParam->iPasses;
	m_sldRandomBase = pParam->iRandom;
	m_sldSharpness = (i32) (pParam->iSharpness * 1000.0f);
	m_sldBlur = pParam->iSmoothness;

	DrxLog("Loading parameters (fFade: %f, fFreq: %f, fFreqStep: %f) to dialog...",
	                     pParam->fFade, pParam->fFrequency, pParam->fFrequencyStep);
}
