...
__forceinline float CNoise::Spline(float x, float* knot)
{
	...
	span = i32(x); // Slow cast is already fast because of the overloaded C float/i32 conversion
	if (span >= nknots - 3)
		span = nknots - 3;
	x -= span;
	knot += span;
...
void CNoise::FracSynthPass(CDynamicArray2D* hBuf, float freq, float zscale, i32 xres, i32 zres, BOOL bLoop)
{
