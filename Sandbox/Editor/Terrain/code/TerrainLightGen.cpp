...
//////////////////////////////////////////////////////////////////////////
void CTerrainLightGen::Init(i32k resolution, const bool bFullInit)
{
	i32 i;
	m_heightmap = GetIEditorImpl()->GetHeightmap();
	assert(m_heightmap);
...
float CTerrainLightGen::GetSunAmount(const float* inpHeightmapData, i32 iniX, i32 iniY,
                                     float infInvHeightScale, const Vec3& vSunShadowVector, const float infShadowBlur) const
{
	...
	i32k iFixPointBits = 8;                                // wide range .. more precision
	i32k iFixPointBase = 1 << iFixPointBits;               //
	...
	i32 iDirX = (i32)(vSunShadowVector.x * fForwardStepSize * iFixPointBase);
	i32 iDirY = (i32)(vSunShadowVector.y * fForwardStepSize * iFixPointBase);

	float fLen = 0.0f;

	i32 iX = iniX * iFixPointBase + iFixPointBase / 2, iY = iniY * iFixPointBase + iFixPointBase / 2;
	...
	for (; fZBottom < m_terrainMaxZ; )
	{
		...
		i32 iXBound = (iX >> iFixPointBits);                      // shift right iFixPointBits bits = /iFixPointBase
		i32 iYBound = (iY >> iFixPointBits);                      // shift right iFixPointBits bits = /iFixPointBase
...
float CTerrainLightGen::GetSkyAccessibilityFast(i32k iniX, i32k iniY) const
{
	//	assert(m_resolution);
	...
	i32 iXmul256 = 256 / invScaleX;
	i32 iYmul256 = 256 / invScaleY;

	return CImageUtil::GetBilinearFilteredAt(iniX * iXmul256, iniY * iYmul256, m_SkyAccessiblity) * (1.0f / 255.0f);
}

float CTerrainLightGen::GetSunAccessibilityFast(i32k iniX, i32k iniY) const
{
	assert(m_resolution);
	...
	i32 invScaleX = m_resolution / m_SunAccessiblity.GetWidth();
	i32 invScaleY = m_resolution / m_SunAccessiblity.GetHeight();
...
////////////////////////////////////////////////////////////////////////
bool CTerrainLightGen::GenerateSectorTexture(CPoint sector, const CRect& rect, i32 flags, CImageEx& surfaceTexture)
{
	if (m_bNotValid)
		return false;
	...
	i32 sectorFlags = GetCLightGenSectorFlag(sector);
...
//////////////////////////////////////////////////////////////////////////
void CTerrainLightGen::Log(tukk format, ...)
{
	if (!m_bLog)
		return;
...
//////////////////////////////////////////////////////////////////////////
inline Vec3 CalcVertexNormal(i32k x, i32k y, const float* pHeightmapData, i32k resolution, const float fHeightScale)
{
...
//////////////////////////////////////////////////////////////////////////
inline Vec3 CalcBilinearVertexNormal(const float fX, const float fY, const float* pHeightmapData, i32k resolution, const float fHeightScale)
{
	i32 x = (i32)fX;
	i32 y = (i32)fY;
...
inline u32 ColorLuminosityInt(u32 r, u32 g, u32 b)
{
	u32 mx, mn;
	mx = MAX(r, g);
	mx = MAX(mx, b);
...
//////////////////////////////////////////////////////////////////////////
bool CTerrainLightGen::GenerateLightmap(CPoint sector, LightingSettings* pSettings, CImageEx& lightmap, i32 genFlags)
{
	assert(m_resolution);
	...
	i32 iSurfaceStep = rc.Width() / lightmap.GetWidth();

	float fSunCol[3], fSkyCol[3];

	if (bBakeLighting || lightAlgo != eDynamicSun)
	{
		/*		Vec3 vSun = GetIEditorImpl()->Get3DEngine()->GetSunColor();
		    Vec3 vSky = GetIEditorImpl()->Get3DEngine()->GetSkyColor();
	...
	u32 iWidth = m_resolution;
	u32 iHeight = m_resolution;
	...
	if (!bUseFastLighting)
	{
		...
		{
			DrxLog("RefreshAccessibility Failed.");
			return false;
		}
	}
	...
	i32 brightness, brightness_shadowmap;

	u32* pLightmap = lightmap.GetData();
	...
	u32 dwWidth = lightmap.GetWidth();
	u32 dwHeight = lightmap.GetHeight();
	...
	//	if(lightAlgo==ePrecise || lightAlgo==eDynamicSun)
	{
		...
		{
			i32 j = rc.top + y * iSurfaceStep;

			for (i32 x = 0; x < dwWidth; x++)
			{
				i32 i = rc.left + x * iSurfaceStep;
				...
				u32* pLMPixel = &pLightmap[x + y * dwWidth];     // u32 RGB

				// Clamp color to 255.
				u32 lr = min(i32(fr * GetRValue(*pLMPixel)), 255); // 0..255
				u32 lg = min(i32(fg * GetGValue(*pLMPixel)), 255); // 0..255
				u32 lb = min(i32(fb * GetBValue(*pLMPixel)), 255); // 0..255
				...
				*pLMPixel = RGB(lr, lg, lb);
				...
				if (bPaintBrightness)
				{
					brightness = min(MAX_BRIGHTNESS, i32(MAX_BRIGHTNESS * fBrightness));
					brightness_shadowmap = min(MAX_BRIGHTNESS, i32(MAX_BRIGHTNESS * fBrightnessShadowmap));
...
void CTerrainLightGen::GenerateShadowmap(CPoint sector, CByteImage& shadowmap, float shadowAmmount, const Vec3& sunVector)
{
	...
	i32 numSectors = si.numSectors;

	i32 sectorTexSize = shadowmap.GetWidth();
	i32 sectorTexSize2 = sectorTexSize * 2;
	...
	u8* sectorImage2 = (u8*)mem.GetBuffer();

	Vec3 wp = GetIEditorImpl()->GetHeightmap()->GetTerrainGrid()->SectorToWorld(sector);
	//	GetIEditorImpl()->Get3DEngine()->MakeLightMap( wp.x+0.1f,wp.y+0.1f,sectorImage2,sectorTexSize2 );
	//GetIEditorImpl()->Get3DEngine()->MakeTerrainLightMap( wp.x,wp.y,si.sectorSize,sectorImage2,sectorTexSize2 );
	memset(sectorImage2, 255, sectorTexSize2 * sectorTexSize2 * 3);

	// Scale sector image down and store into the shadow map.
	i32 pos;
	u32 color;
	for (i32 j = 0; j < sectorTexSize; j++)
	{
		i32 sx1 = sectorTexSize - j - 1;
		for (i32 i = 0; i < sectorTexSize; i++)
		{
...
void CTerrainLightGen::CalcTerrainMaxZ()
{
	...
	m_terrainMaxZ = 0;
	i32 hmapSize = m_hmap.GetSize() / sizeof(float);
	float* pData = m_hmap.GetData();
	for (i32 i = 0; i < hmapSize; i++)
	{
...
{
	i32 sectorFlags = GetSectorFlags(sector);
...
//////////////////////////////////////////////////////////////////////////
i32 CTerrainLightGen::GetSectorFlags(CPoint sector)
{
...
//////////////////////////////////////////////////////////////////////////
i32& CTerrainLightGen::GetCLightGenSectorFlag(CPoint sector)
{
	assert(sector.x >= 0 && sector.x < m_numSectors && sector.y >= 0 && sector.y < m_numSectors);
	i32 index = sector.x + sector.y * m_numSectors;
	return m_sectorGrid[index];
...
//////////////////////////////////////////////////////////////////////////
void CTerrainLightGen::SetSectorFlags(CPoint sector, i32 flags)
{
	assert(sector.x >= 0 && sector.x < m_numSectors && sector.y >= 0 && sector.y < m_numSectors);
...
//////////////////////////////////////////////////////////////////////////
void CTerrainLightGen::GetSubImageStretched(const float fSrcLeft, const float fSrcTop, const float fSrcRight, const float fSrcBottom, CImageEx& rOutImage, i32k genFlags)
{
	...
	/*
	   // m.m. test
	   {
	    ...
	    for(i32 y=0;y<m_SunAccessiblity.GetHeight();++y)
	      for(i32 x=0;x<m_SunAccessiblity.GetWidth();++x)
	    ...
	    for(i32 y=0;y<m_SkyAccessiblity.GetHeight();++y)
	      for(i32 x=0;x<m_SkyAccessiblity.GetWidth();++x)
	...
	float* pHeightmapData = GetIEditorImpl()->GetHeightmap()->GetData();
	assert(m_resolution == GetIEditorImpl()->GetHeightmap()->GetWidth());
	...
	for (u32 dwDestY = 0; dwDestY < dwDestHeight; ++dwDestY)
	{
		...
		for (u32 dwDestX = 0; dwDestX < dwDestWidth; ++dwDestX)
		{
			...
			u32 lg = pos_directed_rounding(fSkyWeight * 255.0f);          // 0..255
	...
	/*
	   ...
	   if(bStatObjShadows)
	   {
	    i32 dwDestWidth2 = dwDestWidth*2;
	   ...
	   //		GetIEditorImpl()->Get3DEngine()->MakeTerrainLightMap( wx,wy,wsize,sectorImage2,dwDestWidth2 );
	    memset(sectorImage2, 255, dwDestWidth2*dwDestWidth2*3);

	    u32 dwShadowAmount = (u32)(256.0f*pSettings->iShadowIntensity/100.0f);		// 0..256
	   ...
	   //				color = (u32) (sectorImage2[pos]);
	        ...
	        u32 dwABGR = rOutImage.ValueAt(dwDestX,dwDestY);

	        // store it
	        u32 outcol = dwABGR;
	   //				u32 outcol = RGB(GetRValue(dwABGR),GetGValue(dwABGR), (((u32)color*(u32)GetBValue(dwABGR)) >>8) );
	        // put shadow map in r channel.
	        outcol = (outcol&(~0xFF)) | ((u8)(color));
	...
	if (bPaintBrightness)
	{
		...
		for (u32 dwDestY = 0; dwDestY < dwDestHeight; ++dwDestY)
		{
			...
			for (u32 dwDestX = 0; dwDestX < dwDestWidth; ++dwDestX)
			{
				...
				u32 col = rOutImage.ValueAt(dwDestX, dwDestY);
				...
				/*				float fAmbient = 0.1f;
				        ...
				        fBrightness = fAmbient*(1.0f-fSunVisibility) + fSunVisibility*fBrightness;
				        u32 la = pos_directed_rounding(fBrightness*255.0f);
				        if(la>255)
				          la=255;
				 */
				...
				u32 la = (i32)(fBrightness * 255.0f);
				if (la > 255)
					la = 255;

				u32 brightness = la;
				u32 shadowmap = GetRValue(col);
				u32 brightness_shadowmap = (shadowmap * brightness) >> 8;
...
//////////////////////////////////////////////////////////////////////////
bool CTerrainLightGen::RefreshAccessibility(const LightingSettings* inpLSettings, i32 genFlags)
{
	assert(m_resolution);
	...
	else
	{
		...
		if (!bUpdateSkyAccessiblity && !m_IndirLightingDataRetrieved)
		{
			...
			u8* dst = m_SkyAccessiblity.GetData();
			...
			for (DWORD i = 0; i < m_TerrainAccMapRes; ++i)
				for (DWORD j = 0; j < m_TerrainAccMapRes; ++j)
				{
					dst[j * m_TerrainAccMapRes + i] =
					  min((u8)(calc.GetSampleAt((i >> calcUnitShift), (j >> calcUnitShift)) * (255.0f / (256 * 256 - 1))),
	...
	if (bUpdateSunAccessiblity)
	{
		...
		u8* dst = m_SunAccessiblity.GetData();
		// copy intermediate to result
		for (DWORD i = 0; i < w; i++)
			for (DWORD j = 0; j < h; j++)
				dst[j * w + i] = (u8) min(((calc.GetSampleAt(i, j) + 0x80) >> 8), 255);
	}
