...
#include "DrxEditDoc.h"           // will be removed
#include <drx3D/CoreX/Memory/DrxSizer.h>   // IDrxSizer
...
u32 CLayer::GetCurrentLayerId()
{
	return m_dwLayerId;
}

u32 CLayer::GetOrRequestLayerId()
{
	if (m_dwLayerId == e_undefined)    // not set yet
	{
		bool bFree[e_undefined];

		for (u32 id = 0; id < e_undefined; id++)
			bFree[id] = true;
		...
		for (u32 id = 0; id < e_undefined; id++)
			...
			{
				m_dwLayerId = id;
				DrxLog("GetOrRequestLayerId() '%s' m_dwLayerId=%d", GetLayerName(), m_dwLayerId);
...
void CLayer::Serialize(CXmlArchive& xmlAr)
{
	...
	if (xmlAr.bLoading)
	{
...

		{
			...
			i32 iSurfaceType = GetIEditorImpl()->GetTerrainManager()->FindSurfaceType(sSurfaceType);
			if (iSurfaceType >= 0)
			{
		...
		if (!layer->getAttr("LayerId", m_dwLayerId))
		{
			...
			char str[256];
			drx_sprintf(str, "LayerId missing - old level format - generate value from detail layer %d", m_dwLayerId);
		...
		if (xmlAr.pNamedData && xmlAr.pNamedData->GetDataBlock(string("Layer_") + m_strLayerName, pData, nSize))
		{
			...
			{
				Warning("Failed to load texture for layer %s", (tukk)m_strLayerName);
			}
		}
		else if (xmlAr.pNamedData)
		{
			...
			{
				GetISystem()->Warning(VALIDATOR_MODULE_EDITOR, VALIDATOR_ERROR, VALIDATOR_FLAG_FILE, (tukk)m_strLayerTexPath, "Failed to load texture for layer %s", (tukk)m_strLayerName);
			}
		}

		if (!m_bAutoGen)
		{
			i32 maskWidth = 0, maskHeight = 0;
			...
			if (xmlAr.pNamedData)
			{
				...
				else
				{
					...
					{
						CByteImage mask;
						mask.Attach((u8*)pData, maskWidth, maskHeight);
	...
	else
	{
		...
		i32 layerTexureSize = m_cTextureDimensions.cx * m_cTextureDimensions.cy * sizeof(DWORD);
...
void CLayer::FillWithColor(COLORREF col, i32 width, i32 height)
{
...
{
	DrxLog("Loading layer texture (%s)...", (tukk)strFileName);
	...
	if (!CFileUtil::FileExists(m_strLayerTexPath) || !CImageUtil::LoadImage(m_strLayerTexPath, m_texture, &bQualityLoss))
	{
		...
		{
			DrxLog("Error loading layer texture (%s)...", (tukk)m_strLayerTexPath);
			bError = true;
		}
	}
	...
	for (i32 y = 0; y < m_texture.GetHeight(); y++)
	{
		for (i32 x = 0; x < m_texture.GetWidth(); x++)
		{
			u32 val = m_texture.ValueAt(x, y);
...
bool CLayer::LoadMask(const string& strFileName)
{
	...
	////////////////////////////////////////////////////////////////////////
	DrxLog("Loading mask texture (%s)...", (tukk)strFileName);

	m_maskFile = PathUtil::AbsolutePathToDrxPakPath(strFileName.GetString());
	if (m_maskFile.IsEmpty())
	...
	{
		DrxLog("Error loading layer mask (%s)...", (tukk)m_maskFile);
		return false;
	}
	...
	u8* dest = mask.GetData();
	u32* src = maskRGBA.GetData();
	i32 size = maskRGBA.GetWidth() * maskRGBA.GetHeight();
...
//////////////////////////////////////////////////////////////////////////
void CLayer::GenerateLayerMask(CByteImage& mask, i32 width, i32 height)
{
...
void CLayer::GenerateWaterLayer16(float* pHeightmapPixels, UINT iHeightmapWidth, UINT iHeightmapHeight, float waterLevel)
{
	...
	u8* pLayerMask = m_layerMask.GetData();
...
void CLayer::ExportMask(const string& strFileName)
{
	...
	if (m_bAutoGen)
	{
		i32 nativeResolution = GetNativeMaskResolution();
...
void CLayer::PrecacheTexture()
{
	...
	{
		Error("Error loading layer texture (%s)...", (tukk)m_strLayerTexPath);
		m_cTextureDimensions = CSize(4, 4);
		m_texture.Allocate(m_cTextureDimensions.cx, m_cTextureDimensions.cy);
		m_texture.Fill(0xff);
		return;
...
void CLayer::PrecacheMask()
{
	...
	if (!m_layerMask.IsValid())
	{
		...
		if (!m_layerMask.IsValid())
		{
			Warning("Layer %s compressed mask have invalid resolution %d.", (tukk)m_strLayerName, m_maskResolution);
		...
		if (m_bCompressedMaskValid)
		{
			...
			if (!bUncompressOk)
			{
				Warning("Layer %s compressed mask have invalid resolution.", (tukk)m_strLayerName);
			}
		}
		else
		{
...
//////////////////////////////////////////////////////////////////////////
bool CLayer::UpdateMaskForSector(CPoint sector, const CRect& sectorRect, i32k resolution, const CPoint vTexOffset,
                                 const CFloatImage& hmap, CByteImage& mask)
{
	...
	u8& sectorFlags = GetSector(sector);
...
//////////////////////////////////////////////////////////////////////////
void CLayer::AutogenLayerMask(const CRect& rc, i32k resolution, const CPoint vHTexOffset, const CFloatImage& hmap, CByteImage& mask)
{
	CRect rect = rc;
	...
	rect.left = max((i32)rect.left, 1);
	rect.top = max((i32)rect.top, 1);
	rect.right = min((i32)rect.right, resolution - 1);
	rect.bottom = min((i32)rect.bottom, resolution - 1);

	// clip within heightmap - 1 pixel
	rect.left = max((i32)rect.left, 1 + (i32)vHTexOffset.x);
	rect.top = max((i32)rect.top, 1 + (i32)vHTexOffset.y);
	rect.right = min((i32)rect.right, hmap.GetWidth() - 1 + (i32)vHTexOffset.x);
	rect.bottom = min((i32)rect.bottom, hmap.GetHeight() - 1 + (i32)vHTexOffset.y);

	// Set the loop pointers
	u8* pLayerMask = mask.GetData();
	...
	u32 x, y;
	u32 dwMaskWidth = resolution;
	i32 iHeightmapWidth = hmap.GetWidth();
	float hVal = 0.0f;
	...
	for (y = rect.top; y < rect.bottom; y++)
	{
		...
		{
			u32 hpos = (x - vHTexOffset.x) + (y - vHTexOffset.y) * iHeightmapWidth;
			u32 lpos = x + y * dwMaskWidth;
	...
	/*	if (m_bSmooth)
	   {
	    ...
	    for (y = rect.top; y < rect.bottom; y++)
	    {
	      u32 lpos = y*dwMaskWidth + rect.left;
...
//////////////////////////////////////////////////////////////////////////
u8& CLayer::GetSector(CPoint sector)
{
	i32 p = sector.x + sector.y * m_numSectors;
	assert(p >= 0 && p < m_maskGrid.size());
	return m_maskGrid[p];
}

//////////////////////////////////////////////////////////////////////////
void CLayer::SetLayerId(u32k dwLayerId)
{
	assert(dwLayerId < e_undefined);

	m_dwLayerId = dwLayerId < e_undefined ? dwLayerId : e_undefined;

	//	DrxLog("SetLayerId() '%s' m_dwLayerId=%d",GetLayerName(),m_dwLayerId);
}
...
//! Import layer block.
void CLayer::ImportBlock(CXmlArchive& xmlAr, const CPoint& offset, i32 nRot)
{
	// ignore autogenerated layers.
	//if (m_bAutoGen)
	//return;
	...
	if (xmlAr.pNamedData->GetDataBlock(string("LayerMask_") + m_strLayerName, pData, nSize))
	{
		...
		subImage.Attach((u8*)pData, subRc.Width(), subRc.Height());
...
//////////////////////////////////////////////////////////////////////////
i32 CLayer::GetNativeMaskResolution() const
{
	CHeightmap* pHeightmap = GetIEditorImpl()->GetHeightmap();
	SSectorInfo si;
	pHeightmap->GetSectorsInfo(si);
...
void CLayer::GetMemoryUsage(IDrxSizer* pSizer)
{
	pSizer->Add(*this);

	pSizer->Add((tuk)m_compressedMask.GetBuffer(), m_compressedMask.GetSize());
...
void CLayer::AssignMaterial(const string& materialName)
{
	...
	bool bFound = false;
	CDrxEditDoc* doc = GetIEditorImpl()->GetDocument();
	for (i32 i = 0; i < GetIEditorImpl()->GetTerrainManager()->GetSurfaceTypeCount(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
i32 CLayer::GetEngineSurfaceTypeId() const
{
	if (m_pSurfaceType)
