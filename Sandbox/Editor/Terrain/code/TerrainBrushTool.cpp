	...
	, m_bPickHeight(false)
{
	...
	tukk toolName = GetRuntimeClass()->m_lpszClassName;
...
bool CBrushTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
...
bool CBrushTool::OnKeyDown(CViewport* view, u32 key, u32 nRepCnt, u32 nFlags)
{
	switch (key)
	{
	case Qt::Key_Escape:
...
void CBrushTool::Paint()
{
	...
	i32 tx = pos_directed_rounding(s_pointerPos.y / unitSize);
	i32 ty = pos_directed_rounding(s_pointerPos.x / unitSize);

	float fInsideRadius = (pBrush->radiusInside / unitSize);
	i32 tsize = (pBrush->radius / unitSize);
	if (tsize == 0)
		tsize = 1;
	i32 tsize2 = tsize * 2;
	i32 x1 = tx - tsize;
...
void CFlattenTool::PaintTerrain(CHeightmap* pHeightmap, i32 tx, i32 ty, i32 tsize, float fInsideRadius, float brushSpeed)
{
...
void CSmoothTool::PaintTerrain(CHeightmap* pHeightmap, i32 tx, i32 ty, i32 tsize, float fInsideRadius, float brushSpeed)
{
	pHeightmap->SmoothSpot(tx, ty, tsize, getBrush().height, getBrush().hardness * brushSpeed);
}

bool CSmoothTool::OnKeyUp(CViewport* view, u32 key, u32 nRepCnt, u32 nFlags)
{
	switch (key)
	{
	case Qt::Key_Shift:
...
bool CRiseLowerTool::OnKeyUp(CViewport* view, u32 key, u32 nRepCnt, u32 nFlags)
{
	switch (key)
	{
	case Qt::Key_Control:
...
bool CRiseLowerTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
...
bool CRiseLowerTool::OnKeyDown(CViewport* view, u32 key, u32 nRepCnt, u32 nFlags)
{
	switch (key)
	{
	case Qt::Key_Control:
...
void CRiseLowerTool::PaintTerrain(CHeightmap* pHeightmap, i32 tx, i32 ty, i32 tsize, float fInsideRadius, float brushSpeed)
{
...
bool CHolesTool::MouseCallback(CViewport* view, EMouseEvent event, CPoint& point, i32 flags)
{
...
void CMakeHolesTool::PaintTerrain(CHeightmap* pHeightmap, i32 tx, i32 ty, i32 tsize, float fInsideRadius, float brushSpeed)
{
	Vec2i min;
	Vec2i max;
	CalculateHoleArea(s_pointerPos, getBrush().radius, min, max);
...
void CFillHolesTool::PaintTerrain(CHeightmap* pHeightmap, i32 tx, i32 ty, i32 tsize, float fInsideRadius, float brushSpeed)
{
	Vec2i min;
	Vec2i max;
	CalculateHoleArea(s_pointerPos, getBrush().radius, min, max);
