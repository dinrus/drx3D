...
i32k s_attributeCount = sizeof(s_LayerAttributes) / sizeof(CItemModelAttribute);
...
Qt::ItemFlags QLayerModel::flags(const QModelIndex& index) const
{
	...
	if (index.column() == (i32)eLayerInfo_Name)
...
i32 QLayerModel::rowCount(const QModelIndex& parent) const
{
...
i32 QLayerModel::columnCount(const QModelIndex& parent) const
{
	if (!parent.isValid())
...
QVariant QLayerModel::data(const QModelIndex& index, i32 role) const
{
	i32 row = index.row();
	i32 col = index.column();
	i32 count = min(m_previews.size(), m_pTerrainManager->GetLayerCount());
...
bool QLayerModel::setData(const QModelIndex& index, const QVariant& value, i32 role)
{
	i32 row = index.row();
	i32 col = index.column();
...
QVariant QLayerModel::headerData(i32 section, Qt::Orientation orientation, i32 role) const
{
	if (orientation == Qt::Horizontal && Private_TerrainLayerListView::s_attributeCount > section)
	{
		...
		case Attributes::s_attributeMenuPathRole:
			if (section != (i32)eLayerInfo_Name)
				return "";
		}
	}
...
void QTerrainLayerView::selectRow(i32 row)
{
...
void QTerrainLayerView::selectionChanged(const QItemSelection& selected, const QItemSelection& deselected)
{
	...
	{
		i32 selectedIndex = selected.front().top();
...
void QTerrainLayerView::SelectedLayerChanged(CLayer* pLayer)
{
	...
	if (m_pTerrainManager)
	{
		i32 layerCount = m_pTerrainManager->GetLayerCount();
