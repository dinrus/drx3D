...
public:
	...
	void           SetSurfaceTypeID(i32 nID) { m_nSurfaceTypeID = nID; };
	i32            GetSurfaceTypeID() const  { return m_nSurfaceTypeID; };
	...
	i32            GetDetailObjectCount() const         { return m_detailObjects.size(); };
	void           RemoveDetailObject(i32 i)            { m_detailObjects.erase(m_detailObjects.begin() + i); };
	const string& GetDetailObject(i32 i) const         { return m_detailObjects[i]; };

	void           AddLayerReference()                  { m_nLayerReferences++; };
	void           RemoveLayerReference()               { m_nLayerReferences--; };
	i32            GetLayerReferenceCount() const       { return m_nLayerReferences; };

	//! Set detail texture projection axis.
	void SetProjAxis(i32 axis) { m_projAxis = axis; }

	//! Get detail texture projection axis.
	i32           GetProjAxis() const { return m_projAxis; }
