...
public:
	...
	// Layer Mask
	//////////////////////////////////////////////////////////////////////////
	u8 GetLayerMaskPoint(u32 x, u32 y) { return m_layerMask.ValueAt(x, y); }
	//	void SetLayerMaskPoint( u32 x,u32 y,u8 c ) { m_layerMask.ValueAt(x,y) = c; }
	...
	bool UpdateMaskForSector(CPoint sector, const CRect& sectorRect, i32k resolution, const CPoint vTexOffset,
	...
	//////////////////////////////////////////////////////////////////////////
	i32 GetMaskResolution() const { return m_maskResolution; }

	// Texture
	i32     GetTextureWidth()      { return m_cTextureDimensions.cx; }
	i32     GetTextureHeight()     { return m_cTextureDimensions.cy; }
	...
	void    FillWithColor(COLORREF col, i32 width, i32 height);
	...
	u32& GetTexturePixel(i32 x, i32 y) { return m_texture.ValueAt(x, y); }

	// represents the editor feature
	// Load a BMP texture into the layer mask.
	bool LoadMask(const string& strFileName);
	void GenerateLayerMask(CByteImage& mask, i32 width, i32 height);
	...
	i32           GetEngineSurfaceTypeId() const;
	...
	//! Import layer block.
	void ImportBlock(CXmlArchive& ar, const CPoint& offset, i32 nRot = 0);
	...
	u32 GetOrRequestLayerId();

	//! might return 0xffffffff
	u32 GetCurrentLayerId();

	// CHeightmap::m_LayerIdBitmap
	void      SetLayerId(u32k dwLayerId);

	CBitmap&  GetTexturePreviewBitmap();
	CImageEx& GetTexturePreviewImage();
	void      Serialize(Serialization::IArchive& ar);

	CDrxSignal<void(CLayer*)> signalPropertiesChanged;
...
protected: // -----------------------------------------------------------------------
	...
	u8& GetSector(CPoint sector);

	// Autogenerate layer mask.
	// Arguments:
	//   vHTexOffset - offset within hmap
	void AutogenLayerMask(const CRect& rect, i32k resolution, const CPoint vHTexOffset, const CFloatImage& hmap, CByteImage& mask);

	// Get native resolution for layer mask (For not autogen levels).
	i32 GetNativeMaskResolution() const;
...
private:
	...
	i32                      m_iSurfaceTypeId; // -1 if not assigned yet,
	u32                   m_dwLayerId;      // used in CHeightmap::m_LayerIdBitmap, usually in the range 0..0xff, 0xffffffff means not set
	...
	// Layer Sectors.
	//////////////////////////////////////////////////////////////////////////
	std::vector<u8> m_maskGrid;  // elements can be 0 or SECTOR_MASK_VALID
	i32                        m_numSectors;
};
