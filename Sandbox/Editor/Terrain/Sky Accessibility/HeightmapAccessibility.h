...
public:
	...
	CHemisphereSink_Solid(u32k indwAngleSteps, u32k, u32k)
	{
	...
	//!
	//!
	void InitSample(SampleType& inoutValue, u32k, u32k) const
	{
...
public:
	...
	//! constructor
	CHemisphereSink_Slice(u32k indwAngleSteps, u32k indwWidth, u32k indwHeight)
...
public:
	...
	CHeightmapAccessibility(u32k indwWidth, u32k indwHeight, u32k indwAngleSteps,
	                        const float infAngleStart = 0.0f, const float infAngleEnd = (float)(gf_PI2)) : m_Sink(indwAngleSteps, indwWidth, indwHeight), m_SamplesInitialized(false)
	{
		...
		u32k cSingleAllocationSize = 4 * 1024 * 1024;//4 MB pieces
		...
		{
			for (i32 i = 0; i < m_AllocParts - 1; ++i)
	...
	void InitSamples()
	{
		if (m_SamplesInitialized)
			return;
		for (u32 x = 0; x < m_dwResultWidth; x++)
			for (u32 y = 0; y < m_dwResultHeight; y++)
	...
	bool Calc(const float* inpHeightmap, const float fScale, tukk cpHeadLine = "Calculating Sky Accessibility")
	{
		assert(inpHeightmap);
		...
		if (m_dwResultWidth >= 128)
			bProgress = true;

		CWaitProgress progress(cpHeadLine, false);
		if (bProgress)
			progress.Start();

		// the horizon is subdivided in wedges
		for (u32 dwWedge = 0; dwWedge < m_dwAngleSteps; dwWedge++)
		{
			if (bProgress)
			{
		...
		{
			for (i32 i = 0; i < m_dwResultWidth; ++i)
				for (i32 j = 0; j < m_dwResultHeight; ++j)
	...
	const TSampleType GetSampleAt(u32k indwX, u32k indwY) const
	{
		if (m_AllocParts == 1)
			return m_ResultBuffer[0][indwX + (indwY << m_XShift)];
		else
		{
			u32k cIndex = (indwY * m_dwResultWidth + indwX);
			u32k cBaseIndex = cIndex / m_PartMapping;
			u32k cOffsetIndex = cIndex - cBaseIndex * m_PartMapping;
	...
	TSampleType& GetSampleAt(u32k indwX, u32k indwY)
	{
		if (m_AllocParts == 1)
			return m_ResultBuffer[0][indwX + (indwY << m_XShift)];
		else
...
private:
	...
	u32 m_dwAngleSteps;          //!< should be >4, more m_dwAngleSteps results in better quality and less speed
	...
	u32  m_dwResultWidth;      //!< value is power of two
	u32  m_dwResultHeight;     //!< value is power of two
	u32  m_XShift;             //!< shift value corresponding to width
	u32  m_YShift;             //!< shift value corresponding to height

	u32  m_AllocParts;         //!< allocation parts, y is divided by that
	u32  m_PartMapping;        //!< mapping to allocation part
	...
	void CalcHeightmapAccessWedge(const float* inpHeightmap,
	                              ...
	                              i32k iLineDirX, i32k iLineDirY,
	                              i32k iLineStepX, i32k iLineStepY,
	                              const float infStep, u32k indwLineCount, u32k indwLineLength)
	{
		assert(inpHeightmap);
		...
		u32k dwXMask = m_dwResultWidth - 1;
		u32k dwYMask = m_dwResultHeight - 1;

		CHorizonTracker HorizonTracker;
		// many lines fill the whole block
		for (u32 dwLine = 0; dwLine < indwLineCount; dwLine++)
		{
			...
			// one line
			if (!m_bTiling)
				for (u32 dwLinePos = 0; dwLinePos < indwLineLength; dwLinePos++, fLen += fLenStep)
				{
			...
			if (m_bTiling)
				for (u32 dwLinePos = 0; dwLinePos < indwLineLength * 2; dwLinePos++, fLen += fLenStep)
				{
					assert(fFilterValue >= 0 && fFilterValue <= 1.0f);
