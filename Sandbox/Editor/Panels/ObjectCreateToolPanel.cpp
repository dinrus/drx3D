	...
	, m_stacked(new QStackedLayout())
{
	...
	for (i32 i = 0; i < categoryNames.size(); i++)
	{
...
void CObjectCreateToolPanel::OnObjectTypeSelected(tukk type)
{
	...
	bool found = false;
	for (i32 i = 0; i < m_typeToStackIndex.size(); i++)
	{
...
void CObjectCreateToolPanel::OnObjectSubTypeSelected(tukk type, QWidget* parent)
{
...
QWidget* CObjectCreateToolPanel::CreateWidgetContainer(QWidget* pChild, tukk type)
{
	...
	pButton->setIcon(DrxIcon("icons:General/Arrow_Left.ico"));
	...
	std::function<void(i32)> idxChangedFn = [this, pComboBox](i32 idx)
	{
...
QWidget* CObjectCreateToolPanel::CreateWidgetOrStartCreate(tukk type)
{
	QWidget* ret = nullptr;
		...
		if (clsDesc)
		{
			...
			if (clsDesc->IsCreatedByListEnumeration())
			{
				if (filter.empty())
				{
					...
					clsDesc->m_libraryRemoved.Connect([pView](tukk libName) { pView->RemoveEntry(libName, nullptr); });
					clsDesc->m_databaseCleared.Connect(pView, &QObjectTreeWidget::Clear);

					clsDesc->EnumerateObjects(pView);

					pView->signalOnDoubleClickFile.Connect([=](tukk selectedFile) { StartCreation(clsDesc, selectedFile); }, (intptr_t)this);
				...
				else
				{
					...
					browser->signalOnDoubleClickFile.Connect([=](tukk selectedFile) { StartCreation(clsDesc, selectedFile); }, (intptr_t)this);
...
void CObjectCreateToolPanel::StartCreation(CObjectClassDesc* cls, tukk file)
{
...
void CCreateObjectButtons::AddButton(tukk szType, const std::function<void()>& onClicked)
{
