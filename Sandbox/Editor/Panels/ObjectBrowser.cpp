...
CObjectBrowser::CObjectBrowser(tukk filters)
	: m_treeView(nullptr)
...
void CObjectBrowser::CorrectSyntax(const QStringList& filters, QStringList& extensions, QStringList& directories)
{
	...
	{
		i32k dotPosition = filter.lastIndexOf(".");
		if (dotPosition != -1)
		{
		...
		i32k slashPosition = filter.lastIndexOf("/");
		if (slashPosition != -1)
		{
...
void CObjectBrowser::Init(const QStringList& extensions, const QStringList& directories)
{
	...
	{
		for (i32 i = indices.count() - 1; i >= 0; --i)
		{
	...
	for (i32 i = CFileTreeModel::eColumn_Name + 1; i < CFileTreeModel::eColumn_Count; ++i)
	{
	...
	if (canShowPreview)
	{
		...
		m_showPreviewButton->setIcon(DrxIcon("icons:General/Visibility_False.ico"));
		...
		{
			UpdatePreviewWidget();
			m_showPreviewButton->setIcon(m_showPreviewButton->isChecked() ? DrxIcon("icons:General/Visibility_True.ico") : DrxIcon("icons:General/Visibility_False.ico"));
