...
const bool CTerrainGIGen::PrepareHeightmap(u32k cWidth, CFloatImage& rFloatImage, float& rHeightScale) const
{
...
void CTerrainGIGen::GetMaterialColor(IMaterial* pMat, std::vector<ColorF>& rSubMats, ColorF& rLastCol, const bool cFillVec) const
{
	...
	if (pMat)
	{
		u32k cSubMatCount = pMat->GetSubMtlCount();
		if (cSubMatCount > 0)
		{
			ColorF matCol(0.f, 0.f, 0.f, 0.f);
			u32 validMatCount = 0;
			for (i32 i = 0; i < cSubMatCount; ++i)
			{
...
)
{
	...
	u32k cWidth = m_pHeightMap->GetWidth();
	u32k cWidthUnits = m_UnitSize * cWidth;
	u32k cObjectMul = std::max(m_Config.vegResMultiplier, m_Config.brushResMultiplier);
	u32k cHeightmapHighResWidth = cWidthUnits / m_Config.baseGranularity * cObjectMul;
	i32 imageToTerrainShift = 0;
	...
	u32k cMatCount = crMatColours.size();
	assert(cMatCount > 0);
	...
	if (!pStatObj)
	{
		...
		u8* meshPositions = pReplacementMesh->GetPosPtr(posStride, FSL_READ);

		for (i32 chunkId = 0; chunkId < chunkList->size(); ++chunkId)
		{
	...
	{
		i32k cSubObjCnt = pStatObj ? pStatObj->GetSubObjectCount() : 0;
		i32 curObjIndex = 0;
		IStatObj* pParentStatObj = pStatObj;//follow only one indirection
		u32 matIndex = 0;//index into crMatIndices
		...
		while (pStatObj)
		{
			...
			if (pIMesh)
			{
				...
				assert(pMesh);
				u32k cSubMatCount = crMatIndices[matIndex].size();
				for (i32 i = 0; i < (i32)pMesh->m_subsets.size(); i++)
				{
					const SMeshSubset& crSubset = pMesh->m_subsets[i];
					assert(matIndex < crMatIndices.size());
					SMatChunk matChunk;
					matChunk.pIndices = pMesh->m_pIndices;
					matChunk.pPositions = (u8*)pMesh->m_pPositions;
					...
					matChunk.matIndex = crMatIndices[matIndex][std::min(crSubset.nMatID, (i32)(cSubMatCount - 1))];
			...
			else
			{
				...
				if (pRenderMesh)
				{
					...
					u8* meshPositions = pRenderMesh->GetPosPtr(posStride, FSL_READ);

					u32k cVertexCount = pRenderMesh->GetVerticesCount();
					...
					for (i32 chunkId = 0; chunkId < chunkList->size(); ++chunkId)
					{
						...
						if (matChunk.num <= 0 || pChunk->m_nMatID >= cSubMatCount)
							continue;
						matChunk.matIndex = crMatIndices[matIndex][MIN(pChunk->m_nMatID, (i32)(cSubMatCount - 1))];
						assert(matChunk.matIndex >= 0 && matChunk.matIndex < cMatCount);
						matChunk.startIndex = pChunk->nFirstIndexId;
						matChunk.posStride = posStride;
						chunkVec[matChunk.matIndex].push_back(matChunk);
	...
	bool onGround = false;
	rBBoxVec.reserve(std::max((u32)1, objectsAdded));
	...
	u32 addedMatCntr = 0;//counts all rasterized mats
	...
	for (std::vector<std::vector<SMatChunk>>::const_iterator chunkIter = chunkVec.begin(); chunkIter != cChunkEnd; ++chunkIter)
	{
		...
		for (std::vector<SMatChunk>::const_iterator iter = crChunks.begin(); iter != cSubsetEnd; ++iter)
		{
			...
			u8* pPositions = crSubset.pPositions;
			assert(pIndices && pPositions);

			const Matrix34& crLocalTM = crSubset.localTM;

			for (i32 i = crSubset.startIndex; i < crSubset.startIndex + crSubset.num; i += 3)
			{
...
const float CTerrainGIGen::GetDistToGround(const float* cpHeightMap, const AABB& crBBox, const float cWorldHMapScale, u32k cHeigthMapRes) const
{
	...
	for (i32 i = 0; i < 3; ++i)
	{
		...
		u32k cX = std::max(0, std::min((i32)(cWorldHMapScale * cPos[i].x), (i32)(cHeigthMapRes - 1)));
		u32k cY = std::max(0, std::min((i32)(cWorldHMapScale * cPos[i].y), (i32)(cHeigthMapRes - 1)));
		//WORLD_HMAP_COORDINATE_EXCHANGE
...
)
{
	...
	if (pObj->IncludeForGI() && pObj->ApplyIndirLighting())
	{
		...
		if (pRenderNode->GetEntityVisArea() == NULL /* && !(pRenderNode->GetRndFlags() & ERF_HIDDEN)*/)
		{
			...
			std::vector<std::vector<i32>> matVec; //vector containing indices into materials, for each sub object one
...
struct IIndirectLighting
{
	...
	virtual void GetTerrainVoxelInfo(SVoxelInfo* ppVoxels, u32& rCount) = 0;

	// Description:
	//		 retrieves existing terrain accessibility data
	virtual void RetrieveTerrainAcc(u8* pDst, u32k cWidth, u32k cHeight) = 0;
};
...
)
{
	...
	for (i32 i = 0; i < nodeCount; ++i)
	{
		...
		std::vector<std::vector<i32>> matVec; //vector containing indices into materials -> not used here
...
)
{
	...
	u32k cSubMatCount = matCols.size();
	rMats.resize(cSubMatCount);
	for (u32 i = 0; i < cSubMatCount; ++i)
	{
		u32k cSubSubCount = matCols[i].size();
		rMats[i].resize(cSubSubCount);
		for (i32 j = 0; j < cSubSubCount; ++j)
		{
			//quantize into rMaterials
			const ColorF& crCol = matCols[i][j];
			u32k cCurMaxIndex = rMaterials.size();
			u32 colIndex;
...
)
{
	...
	std::set<uk> addedObjects;//will track the already added objects to never include an object twice (prefabs contain objects refered to elsewhere too)
	...
	u32k cObjectNum = vegVec.size() + objects.size();
	...
	for (i32 i = 0; i < cObjectsSize; ++i)
	{
		if (objects[i]->GetType() == OBJTYPE_BRUSH || objects[i]->GetType() == OBJTYPE_PREFAB)
		{
			...
			if (objects[i]->GetType() == OBJTYPE_PREFAB)
			{
				...
				for (i32 o = 0; o < pGroup->GetChildCount(); ++o)
				{
...
void CTerrainGIGen::BuildSectorTable(u32k cWidthUnits, const CTerrainObjectMan& crTerrainObjMan)
{
	//now mark each sector as used if at least one neighbor sector is used by the terrain object manager
	std::vector<u8> sectorTableCopied(m_SectorTable);
	memset(&m_SectorTable[0], scSectorUnused, m_SectorRowNum * m_SectorRowNum);
	for (i32 i = 0; i < m_SectorRowNum; ++i)
		for (i32 j = 0; j < m_SectorRowNum; ++j)
		{
...
)
{
	...
	u32k cObjectMul = std::max(m_Config.vegResMultiplier, m_Config.brushResMultiplier);
	u32k cHeightmapHighResWidth = cWidthUnits / m_Config.baseGranularity * cObjectMul;
	u32k cWorldToHMapMul = cWidthUnits / cHeightmapHighResWidth;
	u32 worldToHMapMulShift = 0;
	...
	u32k cConvertFactor = m_Config.baseGranularity / cResMultiplier;
	u32 convertFactorShift = 0;
	...
	u32 obstructionIndex = 0xFFFFFFFF;//dummy
	float obstructionAmount = 1.f;//dummy
	u32k cCalcWidth = rHMapCalc.m_Sink.GetGridResolution();

	//sample refinement constants
	const float cInvUnitSize = 1.f / m_UnitSize;

	i32 baseGranularityShift = 0;//replace / m_Config.baseGranularity by >> baseGranularityShift
	while ((1 << baseGranularityShift) != m_Config.baseGranularity)
		++baseGranularityShift;

	i32k cMaxGranularity = m_Config.baseGranularity / cMaxMultiplier;
	i32 maxGranularityShift = 0;//replace / m_Config.baseGranularity by >> baseGranularityShift
	while ((1 << maxGranularityShift) != cMaxGranularity)
		++maxGranularityShift;

	i32k cMaxInnerGranularity = (cMaxGranularity >> 1) + 1;
	...
	static i32k scEnlargementUnits = 1;//refinement units (determined by size)
	...
	for (TObjectVec::iterator iter = rObjectVec.begin(); iter != cObjectEnd; ++iter)
	{
		...
		if ((u16)rObject.pos.x >= (u16)cWidthUnits || (u16)rObject.pos.y >= (u16)cWidthUnits)
		...
		i32k cEnlargementUnits = (rObject.zExt > 8.f) ? (scEnlargementUnits + 1) : scEnlargementUnits;//with 8 m height one unit more
		...
		u32 cGridPosX = ((u32)cCenterPos.x >> convertFactorShift) << convertFactorShift;
		u32 cGridPosY = ((u32)cCenterPos.y >> convertFactorShift) << convertFactorShift;
		...
		//WORLD_HMAP_COORDINATE_EXCHANGE
		u32k cGridIndexI = cGridPosY >> worldToHMapMulShift;
		u32k cGridIndexJ = cGridPosX >> worldToHMapMulShift;
		...
		if (cObjectType != eOT_Voxel)
		{
			...
			const float cMiddleHeight = CHemisphereSink_SH::scSampleHeightOffset +
			                            GetFilteredHeight
			                            (
			  (u16)(crMax.y + crMin.y) * 0.5f * cInvUnitSize,
			  (u16)(crMax.x + crMin.x) * 0.5f * cInvUnitSize,
			  cHeightmapHighResWidth,
			  m_TerrainMapRes,
			  cpHeightMap,
			  imageToTerrainShift
			                            );
			u32k cBoxCnt = crTerrainObj.bboxCol.size();
			bool firstTime = true;
			for (i32 b = 0; b < cBoxCnt; ++b)
			{
		...
		i32 cStartX = (i32)std::max(0.f, crMin.x);
		i32 cTempStartX = (cStartX >> convertFactorShift) << convertFactorShift;
		if (cTempStartX == cStartX)//starts at grid pos, will be dropped by inner object test
			cStartX = std::max((i32)0, cTempStartX - (cEnlargementUnits << convertFactorShift));
		else
			cStartX = std::max((i32)0, cTempStartX - ((cEnlargementUnits - 1) << convertFactorShift));
		i32 cStartY = (i32)std::max(0.f, crMin.y);
		i32 cTempStartY = (cStartY >> convertFactorShift) << convertFactorShift;
		if (cTempStartY == cStartY)//starts at grid pos, will be dropped by inner object test
			cStartY = std::max((i32)0, cTempStartY - (cEnlargementUnits << convertFactorShift));
		else
			cStartY = std::max((i32)0, cTempStartY - ((cEnlargementUnits - 1) << convertFactorShift));
		i32k cEndX = std::max((i32)0, std::min((i32)(cWidthUnits - 1), (((i32)crMax.x >> convertFactorShift) << convertFactorShift) + (cEnlargementUnits << convertFactorShift)));
		i32k cEndY = std::max((i32)0, std::min((i32)(cWidthUnits - 1), (((i32)crMax.y >> convertFactorShift) << convertFactorShift) + (cEnlargementUnits << convertFactorShift)));
		...
		i32k cMaxOuterRing = cMaxGranularity * (cEnlargementUnits / 2);

		//check if it is worth to have a sample each unit
		const bool c1UnitSample = (cObjectType != eOT_Voxel) && cNonVegPresent;

		for (i32 x = cStartX; x <= cEndX; ++x)
		{
			for (i32 y = cStartY; y <= cEndY; ++y)
			{
				const TVec2F cPos(x, y);
				//WORLD_HMAP_COORDINATE_EXCHANGE
				u32k cHighResIndexI = (y >> worldToHMapMulShift);
				u32k cHighResIndexJ = (x >> worldToHMapMulShift);
				...
				const float cHeight = CHemisphereSink_SH::scSampleHeightOffset +
				                      GetFilteredHeight
				                      (
				  (u16)((float)y * cInvUnitSize),
				  (u16)((float)x * cInvUnitSize),
				  cHeightmapHighResWidth,
				  m_TerrainMapRes,
				  cpHeightMap,
				  imageToTerrainShift
				    ...
				    x >= minInner.x - cMaxInnerGranularity && y >= minInner.y - cMaxInnerGranularity)
				{
					...
					u32 obstructionObjectIndex = 0xFFFFFFFF;
...
void CTerrainGIGen::SetDataForAdditionalSamples(const THeightmapAccess& crHMapCalc)
{
	...
	u32k cHeightmapHighResWidth = cWidthUnits / m_Config.baseGranularity * cObjectMul;
	u32k cWorldToHMapMul = cWidthUnits / cHeightmapHighResWidth;
	u32 curCount = m_GridPoints.size();
...
{
	u32k cWorldToGridCoordsMul = cWidthUnits / cHighResWidth;
	u32 worldToGridCoordsMulShift = 0;
...
)
{
	...
	assert(cGridWidth < (2 << (sizeof(i16) * 8 - 2)));
	u32k cHeightmapMul = cHighResWidth / cGridWidth;
	u32 dummy;
	...
	bool xStage = false;
	for (u16 x = 0; x < cGridWidth; ++x)
	{
		optimizeable = xStage;
		xStage = !xStage;
		for (u16 y = 0; y < cGridWidth; ++y)
		{
			TEditorGridPoint gridPoint;
			...
			if (cpHeightMap[y * cHighResWidth * cHeightmapMul + x * cHeightmapMul] <= cWaterLevel)
				continue;
			u8 initialFlag = optimizeable ? (scValid | scOptimizeable) : scValid;
...
)
{
	...
	u32k cMarkStep = cMaxMultiplier / 2;
	...
	if (cMaxMultiplier > 2)
	{
		for (i32 i = 0; i < cRes; ++i)
			for (i32 j = 0; j < cRes; ++j)
			{
...
)
{
	...
	u8k cAngleThresholdQuant = (u8)(cAngleThreshold * 256.f / (gf_PI / 2));//quantize 0..PI/2 into 256 values
	//generate some directions for lookup, slidely looking upward and 90 degree separated in xy
	static u32k scDirectionCnt = 16;
...
void CTerrainGIGen::BlurTerrainAccMap(u8* pAccMap, u32k cWidth, u8k cMinValue)
{
	//just weight center pixel
	u8* pTempMap = m_pTempBlurArea;
	...
	for (i32 x = 0; x < cWidth; ++x)
		for (i32 y = 0; y < cWidth; ++y)
		{
			u8k cCurVal = pTempMap[y * cWidth + x];
			...
			pAccMap[y * cWidth + x] = max(cMinValue, (u8)(newCol + 0.5f));
		}
}
...
const float CTerrainGIGen::GetHeightFromWorldPosFullRes(const Vec3& crWorldPos) const
{
	...
	u32k cX = (u32)cXPosFloat;
	u32k cY = (u32)cYPosFloat;
	u32k cX1 = std::min(cWidth - 1, cX + 1);
	u32k cY1 = std::min(cWidth - 1, cY + 1);
...
{
	u32k cX = cXPos >> cRGBToXYResShift;
	u32k cY = cYPos >> cRGBToXYResShift;
	u32k cX1 = std::min(cRGBImageRes - 1, cX + 1);
	u32k cY1 = std::min(cRGBImageRes - 1, cY + 1);
...
)
{
	...
	u32k cBrightness = (u32)(255.f * upperHSAveVis + 0.5f);
	assert(cBrightness < 256);
	pHeightMapAccMap[cY * cWidth + cX] = (u8)cBrightness;
}
...
)
{
	...
	u32 imageToTerrainShift = 0;
	while ((cRGBImageRes << imageToTerrainShift) != cTerrainOcclRes)
		++imageToTerrainShift;
	assert((cRGBImageRes << imageToTerrainShift) == cTerrainOcclRes);

	u32k cFullRes = cTerrainOcclRes << 1 /*super sampling*/;
	const uint64 cTerrainMapEntries = cTerrainOcclRes * cTerrainOcclRes;
	uint64 curUpdated = 0;
	//sample in double resolution if super sampling is active
	for (u32 tempX = 0; tempX < cTerrainOcclRes; ++tempX)
	{
		for (u32 tempY = 0; tempY < cTerrainOcclRes; ++tempY)
		{
			progress.Step((u32)(100.f * ((float)curUpdated / (float)cTerrainMapEntries)));
			++curUpdated;
			u8k cTerrainMarkVal = m_TerrainMapMarks[(tempY >> hmapUnitShift) * m_TerrainMapRes + (tempX >> hmapUnitShift)];
			...
			static u32k cXOffsetsSS[4] = { 0, 1, 0, 1 };
			static u32k cYOffsetsSS[4] = { 0, 0, 1, 1 };
			...
			if (cApplySS)
			{
				...
				for (i32 ss = 0; ss < 4; ++ss)//super sampling loop
				{
					u32 x = tempX * 2 + cXOffsetsSS[ss];
					u32 y = tempY * 2 + cYOffsetsSS[ss];
					...
					u32k cLeafCount = m_QuadTree.GetInterpolationNodes(nodes, cGridPos, 32.05f * 32.05f);
	...
	if (cApplySS)
	{
		for (i32 i = 0; i < cTerrainOcclRes; ++i)
			for (i32 j = 0; j < cTerrainOcclRes; ++j)
			{
				...
				val *= 0.25f;
				u32k cBrightness = (u32)(val + 0.5f);
				m_pSmallHeightMapAccMap[j * cTerrainOcclRes + i] = (u8)std::min((u32)255, cBrightness);
			}
	...
	u32* pBitmap = new u32[cTerrainOcclRes * cTerrainOcclRes];
	assert(pBitmap);
	memset(pBitmap, 0, cTerrainOcclRes * cTerrainOcclRes * sizeof(u32));
	for (i32 i = 0; i < cTerrainOcclRes; ++i)
		for (i32 j = 0; j < cTerrainOcclRes; ++j)
		{
			u32k cCol = pHeightMapAccMap[j * cTerrainOcclRes + i];
...
{
	i32k cX = cHPosX;
	i32k cXm1 = std::max(0, cX - 1);
	i32k cXp1 = std::min(cHighResWidth - 1, (u32)(cX + 1));

	i32k cY = cHPosY;
	i32k cYm1 = std::max(0, cY - 1);
	i32k cYp1 = std::min(cHighResWidth - 1, (u32)(cY + 1));
...
)
{
	...
	u32k cObstrMapRes = crTerrainObjMan.GetObstructionMapRes();
	u32k cGridToObstrMapMultiplier = cObstrMapRes / crCalc.m_Sink.GetGridResolution();
	assert(cObstrMapRes > crCalc.m_Sink.GetGridResolution());
	u32k cObstrMapToWorldMultiplier = cFullWidth / cObstrMapRes;
	assert(cGridToObstrMapMultiplier * crCalc.m_Sink.GetGridResolution() == cObstrMapRes);
	//first calculate the next non obstructed u16 position for each grid direction
	...
	const std::pair<u16, u16> cDirOffsets[4] = { std::make_pair(-1, 0), std::make_pair(1, 0), std::make_pair(0, -1), std::make_pair(0, 1) };
	std::pair<u16, u16> nextNonObstructedPos[4];
	for (i32 i = 0; i < 4; ++i)
	{
	...
	i32 shortestIndex = 0;
	u32 shortestDistance = std::numeric_limits<u32>::max();
	for (i32 i = 0; i < 4; ++i)
	{
		u32k cDist = abs(nextNonObstructedPos[i].first - hmapX) + abs(nextNonObstructedPos[i].second - hmapY);
...
void CTerrainGIGen::OnBeforeProcessing(u32k cFullWidth, CTerrainObjectMan& rTerrainObjMan, u32k cHighResWidth, THeightmapAccess& rCalc)
{
	//determine number of elements we need
	u32 num = rCalc.m_Sink.GetAddSampleCount();
	...
	for (i32 i = 0; i < cHighResWidth; ++i)
		for (i32 j = 0; j < cHighResWidth; ++j)
		{
			SSHSample& rSample = rCalc.GetSampleAt(i, j);
			//do only ray cast if it is within a certain distance of objects
			u8k cTerrainMarkVal = m_TerrainMapMarks[(j << hmapUnitShift) * m_TerrainMapRes + (i << hmapUnitShift)];
			if (cTerrainMarkVal == 0)
	...
	u32k cSingleCount = std::min((u32)num, (u32)(cSingleAllocationSize / sizeof(SSHSampleOnDemand)));
#if defined(_DEBUG)
	...
	for (i32 i = 0; i < parts; ++i)
	{
		for (i32 j = 0; j < std::min(cSingleCount, remainingCount); ++j)
		{
...
)
{
	...
	u32 cObjectMul = std::max(m_Config.vegResMultiplier, m_Config.brushResMultiplier);
	u32 cHeightmapHighResWidth = cWidthUnits / m_Config.baseGranularity * cObjectMul;
	...
	u32 cGridWidth = cWidthUnits / m_Config.baseGranularity;//width of targeted resolution (pixel size for sh accessibility)

	u32 cHighResWidth = cGridWidth * cObjectMul;
	...
	for (i32 i = 0; i < m_TerrainMapRes * m_TerrainMapRes; ++i)
		m_TerrainMapMarks[i] = 0;//initially set to no scale
	static u32k scTerrainMapUpdateRadius = 12;//update radius around objects radius for ray casting updates
	...
	u8* pHeightMapAccMap = NULL;
	if (cApplySS)
	{
		pHeightMapAccMap = new u8[(cWidthUnits * cWidthUnits) << 2 /*super sampling*/];
		m_pSmallHeightMapAccMap = new u8[cWidthUnits * cWidthUnits];//down sampled pointer
	...
	else
	{
		...
		pHeightMapAccMap = new u8[cWidthUnits * cWidthUnits];
		memset(pHeightMapAccMap, 255, cWidthUnits * cWidthUnits);//set to full vis by default
	}
	m_pTempBlurArea = new u8[cWidthUnits * cWidthUnits];//temporary blur area
	...
	AddCoarseGridPoints(cWaterLevel, cGridWidth, cHighResWidth, pHeightMap, terrainObjMan);
	{
		...
		THeightmapAccess calc(cHighResWidth, cHighResWidth, m_Config.wedges);
		{
			...
			u32k cThreadCount = calc.m_Sink.GetThreadCount();
			string headLine;
			if (cThreadCount > 1)
			{
