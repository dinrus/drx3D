...
template<class TLeafContent, u32 TMaxCellElems = 1, class TPosType = float, class TIndexType = u16, bool TUseRadius = false>
class CQuadTree;
...
//!<			TIndexType: type of indices within this tree, should be the less sized type it can get (preferably u16)
...
{
public:
	template<class TLC, u32 TMCE, class TPT, class TIT, bool TUR>
	...
	typedef void (*         SerializeContentFunction)(const TLeafContent& crContent, u8* pBuffer);
	...
	typedef const bool (*   DeSerializeContentFunction)(TLeafContent& rContent, u8k* cpBuffer);
...
private:
	typedef u16                                               TRadiusType; //!< typedef for 2 byte radius type
	...
	public:
		...
		void                                        Serialize(SerializeContentFunction pSerializeFnct, const size_t cContentSerializationSize, u8*& pStreamPos) const; //!< serializes the leaf and moves the current stream pointer
	...
	protected:                         //we need this to be private since it is using a flag together with an index, direct access will result in wrong results
		...
		u8             pad[sizeof(u32) - ((sizeof(TVec2) + sizeof(TLeafContentQuad) + sizeof(TIndexTypeQuad)) % sizeof(u32))];

		const TRadiusType GetRadius() const;    //!< retrieves radius, needed for common leaf usage

		template<class TLeafContentF, u32 TMaxCellElemsF, class TPosTypeF, class TIndexTypeF, bool TUseRadiusF>
		friend class CQuadTree;
	};
	...
	struct SHeader
	{
		u32     headerSize;          //!< size of header in bytes
		u32     fullStreamSize;      //!< size of entire stream including header in bytes
		...
		u16     maxSubDivLevels;     //!< max subdivision levels
...
public:
	...
	{
		u32 leafArrayAccesses;   //!< number of m_Leaves accesses
		u32 nodeArrayAccesses;   //!< number of m_Nodes accesses
	...
	static u32k scInitalSliceGranularity = 16;          //!< initial slice granularity (4 per quadrant)
	...
	{
		u8 sliceIndex;       //!< slice it belongs to
		u8 nextSliceIndex;   //!< closest next slice index it belongs to
	};
	...
	(
	  ...
	  u16k cMaxSubDivLevels = 9,
	  u32k cReservedNodeMem = 0,
	  u32k cReservedLeafMem = 0
	);
	//!< default constructor if to be initialized later
	CQuadTree(u32k cReservedNodeMem = 0, u32k cReservedLeafMem = 0);
	//!< destructor
	~CQuadTree();
	...
	template<u32 TNewMaxCellElems, class TNewIndexType>
	...
	void SerializeTree(std::vector<u8>& rStream, SerializeContentFunction pSerializeFnct, SerializeContentSizeFunction pSizeFnct, const bool cOptimizeBefore = true);
	...
	const bool ConstructTreeFromStream(u8k * cpStream, u32k cStreamSizeToDecode, DeSerializeContentFunction, SerializeContentSizeFunction, u8 * &newStreamPos);
	//!< returns max number of subdivision levels
	u16k GetMaxSubdivLevels() const;
	...
	void                      Draw(tukk cpFileName = "QuadTree_Debug.tga");
#if defined(_DEBUG)
...
private:
	...
	u16                  m_MaxSubDivLevels;   //!< max number of subdivision levels, level 0 is just the root node, set in constructor
	...
	static u32k     scLookupCount = 4 * scInitalSliceGranularity;
	std::pair<u8, u8> m_SliceLookupTable[scLookupCount + 1][2 /*sign of dirx*/];//+1 for safety and inaccuracy(no need to check then)
	...
	//also enable for u32 be used as index type since constants have this default type
	template<typename U>
	struct SClosestContentsChecker<i32, U>
	{
	...
	typedef std::vector<std::pair<SQuadNodeData, u16>> TNodeDataVec;       //!< typedef for node data retrieval with radius
	...
	const bool       Subdivide(const float cQuadSideLength, const TVec2F& crCenterPos, SQuadNode& rQuadNodeToSubdiv, u32k cLevels);
	...
	void                   SetMaxSubdivLevels(u16k cLevels);
	...
	u8k GetSliceIndex(const float cDirX, const float cDirY, const float cDistSq, u8& rNextIndex) const;
