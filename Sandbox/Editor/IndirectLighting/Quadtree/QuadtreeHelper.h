...
public:
	...
	u32k size() const;
	T&         operator[](u32k cIndex);
	const T&   operator[](u32k cIndex) const;
	const bool empty() const;

private:
	static u32k scMaxArrayCount = 4096;

	u8               m_Mem[sizeof(T) * scMaxArrayCount];
	u32              m_Count;
};
...
const bool SaveTGA32(tukk cpFilename, u32k* cpBuffer, u32k cSize);
...
struct SChildIndices
{
	...
	u32       count;                       //!< current number of stored indices (always reset and started from index 0)
...
private:
	...
	u32                                              m_ElemCount;     //!< elem count to retrieve, template arg for FillVector takes care of radius too
