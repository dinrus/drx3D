...
inline u32k LerpColour(const float cVal, const Vec3& crFrom, const Vec3& crTo)
{
	const Vec3 cCol = crTo * cVal + crFrom * (1.0 - cVal);
	return ((u32)cCol.z) | ((u32)cCol.y << 8) | ((u32)cCol.x << 16);
}

//!< adds alpha to a colour
inline u32k AddAlpha(u32k cCol, const float cAlpha)
{
	assert(cAlpha >= 0.f && cAlpha <= 1.f);
	return (cCol | ((u32)(cAlpha * 255.f) << 24));
}
}
...
template<class T>
inline u32k CTravVec<T >::size() const
{
	return m_Count;
}

template<class T>
inline T& CTravVec<T >::operator[](u32k cIndex)
{
...
template<class T>
inline const T& CTravVec<T >::operator[](u32k cIndex) const
{
	assert(cIndex < m_Count);
	return ((T*)m_Mem)[cIndex];
}
...
inline const bool SaveTGA32(const char* cpFilename, u32k* cpBuffer, u32k cSize)
{
	...
	*(reinterpret_cast<unsigned short*>(&header[14])) = cSize;
	...
	fwrite(cpBuffer, 1, cSize * cSize * sizeof(u32), pFile);
	fclose(pFile);
	return true;
}
...
(
  ...
  const bool   /*no effect*/
	...
	: m_CompareValue(std::numeric_limits<float>::max()), m_ElemCount((u32)cExpectedElements),
...
template<bool TUseRadius, u32 TMaxCellElems, class TIndexType>
