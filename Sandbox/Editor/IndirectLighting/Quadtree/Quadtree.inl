...
inline const TIndexType CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::GetLeafCapacity() const
{
	...
	m_DebugStats.Reset();
#endif
	return (TIndexType)min((uint64)MaxElems(), (uint64)(TMaxCellElems * (u32)pow(4.f, (int)m_MaxSubDivLevels)));
}
...
inline const TIndexType CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::MaxElems() const
{
	...
	m_DebugStats.Reset();
#endif
	u32k cMaxMemoryAvail = 64 * 1024 * 1024;
...
template<class TLeafContent, u32 TMaxCellElems, class TPosType, class TIndexType, bool TUseRadius>
void CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::SetMaxSubdivLevels(u16k cLevels)
{
...
template<class TLeafContent, u32 TMaxCellElems, class TPosType, class TIndexType, bool TUseRadius>
inline u16k CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::GetMaxSubdivLevels() const
{
#if defined(_DEBUG)
...
(
  ...
  u16k cMaxSubDivLevels,
  u32k cReservedNodeMem,
  u32k cReservedLeafMem
)
	: m_SideLength(cSideExtension), m_Center(TVec2F(crCenterPosition.x, crCenterPosition.y)), m_UniqueLeaveCount(0)
{
	...
	u32k cReservedNodeCount = cReservedNodeMem / sizeof(SQuadNode);
	if (cReservedNodeCount > 0)
		m_Nodes.reserve(cReservedNodeCount);
	u32k cReservedLeafCount = cReservedLeafMem / sizeof(TLeaf);
...
CQuadTree(u32k cReservedNodeMem, u32k cReservedLeafMem)
{
	assert(TMaxCellElems > 0);                       //do not instantiate with bad template param (0 cell leaf count)
	assert(sizeof(SQuadNode) == sizeof(TIndexType)); //if this fails, a pragma pack needs to be added to the SLeaf structure to avoid memory overheads
#if defined(_DEBUG)
...
void CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::BuildSliceLookupTable()
{
	...
	for (int x = 0; x < 2; ++x)//sign
	{
		...
		for (int y = 0; y < scLookupCount; ++y)//y
		{
			...
			{
				m_SliceLookupTable[y][x].first = ((u8)cAngleQuantIndex + 1) & (scInitalSliceGranularity - 1);
				m_SliceLookupTable[y][x].second = ((u8)cAngleQuantIndex) & (scInitalSliceGranularity - 1);
			}
			else
			{
				m_SliceLookupTable[y][x].first = ((u8)cAngleQuantIndex) & (scInitalSliceGranularity - 1);
				m_SliceLookupTable[y][x].second = ((u8)cAngleQuantIndex + 1) & (scInitalSliceGranularity - 1);
			}
			curY += cStep;
		}
	}
...
Subdivide(const float cQuadSideLength, const TVec2F& crCenterPos, SQuadNode& rQuadNodeToSubdiv, u32k cLevels)
{
#if defined(_DEBUG)
	assert(cLevels > 0);
...
{
	u32k cCount = crTraversedNodes.size();
...
) const
{
	...
	u32k cCount = traversedNodes.size();
	for (int i = cCount - 1; i >= 0; --i)
	{
		//traverse all nodes from bottom to the top, invoke with ::Type cast to be able to call for u32 as index type too
...
template<class TLeafContent, u32 TMaxCellElems, class TPosType, class TIndexType, bool TUseRadius>
void CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::
SerializeTree(std::vector<u8>& rStream, SerializeContentFunction pSerializeFnct, SerializeContentSizeFunction pSizeFnct, const bool cOptimizeBefore)
{
#if defined(_DEBUG)
	assert(m_IsInitialized == true);
	m_DebugStats.Reset();
	...
	header.fullStreamSize = (u32)cStreamSize;
	header.minSideLength = m_MinSideLength;
	rStream.resize(cStreamSize);
	u8* pCurStreamPos = &rStream[0];  //vector is not gonna be reallocated, pointer usage is safe here
	//first copy header
...
template<class TLeafContent, u32 TMaxCellElems, class TPosType, class TIndexType, bool TUseRadius>
const bool CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::
ConstructTreeFromStream(u8k* cpStream, u32k cStreamSizeToDecode, DeSerializeContentFunction pDeSerializeFnct, SerializeContentSizeFunction pSizeFnct, u8*& newStreamPos)
{
#if defined(_DEBUG)
	m_DebugStats.Reset();
#endif
	...
	u8* pCurStreamPos = const_cast<u8*>(&cpStream[cHeaderSize]);  //point to first byte behind header
...
void CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::Draw(const char* cpFileName)
{
	...
	u16k cMaxLevel = min(m_MaxSubDivLevels, (u16)11);
	u32k cSize = (u32)pow(2.f, (int)cMaxLevel);
	u32* pBitmap = new u32[cSize * cSize];
	assert(pBitmap);
	memset(pBitmap, 0, cSize * cSize * sizeof(u32));
	std::vector<u32> RGBColours(cMaxLevel + 1);
	//calc RGB colour
	u32k cMaxYellowLevel = cMaxLevel / 2;
...
CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::IsConsistent()
{
	...
	u32k cLeafCount = (u32)m_Leaves.size();
...
template<class TLeafContent, u32 TMaxCellElems, class TPosType, class TIndexType, bool TUseRadius>
inline u8k CQuadTree<TLeafContent, TMaxCellElems, TPosType, TIndexType, TUseRadius >::
GetSliceIndex(const float cDirX, const float cDirY, const float cDistSq, u8& rNextIndex) const
{
	...
	const std::pair<u8, u8>& crSlicesIndices = m_SliceLookupTable[(u32)(scLookupScale * (cDirY / cS + 1.f))][(cDirX >= 0.f) ? 0 : 1];
...
GetInterpolationNodes(TInitialInterpolRetrievalType pLeaves, const TVec2F& crPos, const float cMaxDistSq) const
{
	...
	if (leafIndex != TLeaf::INVALID_LEAF_INDEX)
	{
		for (;; )
		{
			...
			{
				u8 nextSliceIndex;
				u8k cSliceIndex = GetSliceIndex(cDirX, cDirY, cDistSq, nextSliceIndex);
...
) const
{
	...
	u32k cMaxIndexDiff = scInitalSliceGranularity >> 2;//max distance between 2 indices to get at least one sample per quadrant
	bool wasForcedIndex = false;  //if true, last sample was inserted due to too large index (cMaxIndexDiff)
	for (u32 i = 0; i < scInitalSliceGranularity; ++i)
	{
