...
inline const Vec3 Vec3FromColour(u32k cCol)
{
...
struct SSHSample
{
	...
	u32 linkID;//link into object sample vector
	u16 colCount;//number of calls to aveCol, max is wedges * wedges, 0xFFFF means set vis to 0

	void   SetSampleLink(u32k cIndex = 0xFFFFFFFF)
	{
...
private:
	u8              flags;//flags of ray casting
	static u8k scObstructed = 0x1;
	static u8k scNoRayCasting = 0x2;
	static u8k scToBeIgnored = 0x4;
	static u8k scHasLinkedObject = 0x8;
	static u8k scMapped = 0x10;
	static u8k scAlreadyDisplaced = 0x40;
	static u8k scOffseted = 0x80;
};
...
public:
	...
	struct SMPData : public IThread
	{
		...
		u32                 colCount;    //number of added colours
	...
	void Init(CTerrainObjectMan* cpTerrainObjMan, float* pHeightMap, const float cWaterLevel, const float cHeightScale, u32k cWorldSpaceMul);

	void InitSample(SampleType& rInoutValue, u32k cX, u32k cY) const;
	...
	//WORLD_HMAP_COORDINATE_EXCHANGE
	void WorldToGridPos(u16& rX, u16& rY, const float cX, const float cY) const
	{
		rX = cY / (float)m_WorldSpaceMul;
		rY = cX / (float)m_WorldSpaceMul;
	}

	u32k GetGridResolution() const
	{
		return m_Width;
	}

	u32k GetWorldSampleDist() const
	{
		return m_WorldSpaceMul;
	}

	u32k GetAngleStepCount() const
	{
		return m_AngleSteps;
	}

	u32k GetSampleCount() const
	{
		return m_SampleCount;
	}

	u32k GetThreadCount() const
	{
#if defined(DO_MP)
		return m_CoreCount;
#else
...
protected: // ---------------------------------------------------------------------------------
...
#if defined(DO_MP)
	u32            m_CoreCount;    //number of multi processing cores
	...
	u32             m_WorldSpaceMul;//granularity of grid points, m_Width * m_WorldSpaceMul yields to world space terrain extension

	u32             m_AngleSteps; //number of wedge angle steps
	u32             m_Width;//size of grid texture, every pixel represents one grid point
	u32             m_SampleCount;//sample count for upper hemisphere
