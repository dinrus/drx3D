...
u32k cMiddleValInt = (float)(CBBoxRaster::scRasterRes >> 1);
...
)
{
	...
	{
		u8k* cpPositions = iter->pPositions;
		const vtx_idx* cpIndices = iter->pIndices;
		u32k cPosStride = iter->posStride;
		u32k cStartIndex = iter->startIndex;
		u32k cNum = iter->num;
		const Matrix34& crLocalTM = iter->localTM;

		//rotate object into non translated world space
		for (i32 i = cStartIndex; i < cStartIndex + cNum; i += 3)
		{
			...
			u32 negativeCounts[3] = { 0, 0, 0 };//tracks number of vertices on the side < scRasterRes/2
			for (i32 i = 0; i < 3; ++i)
			{
				//enforce range
...
) const
{
	...
	if (RayAABBIntersect(origin, crWSDir, Vec3(0, 0, 0), Vec3(cMaxExt, cMaxExt, cMaxExt), in, out))
	{
		...
		if (in < 0.f)
		{
			...
			u32k cX = std::min(scRasterRes - 1, (((origin.x - floorf(origin.x)) >= 0.5f) ? (u32)origin.x + 1 : (u32)origin.x));
			u32k cY = std::min(scRasterRes - 1, (((origin.y - floorf(origin.y)) >= 0.5f) ? (u32)origin.y + 1 : (u32)origin.y));
		...
		for (float curRayLen = rayLen * 0.98f; curRayLen > 0.0001f; curRayLen -= cStepWidth)
		{
			...
			u32k cX = std::min(scRasterRes - 1, (((cRayPos.x - floorf(cRayPos.x)) >= 0.5f) ? (u32)cRayPos.x + 1 : (u32)cRayPos.x));
			u32k cY = std::min(scRasterRes - 1, (((cRayPos.y - floorf(cRayPos.y)) >= 0.5f) ? (u32)cRayPos.y + 1 : (u32)cRayPos.y));
			u32k cZ = std::min(scRasterRes - 1, (((cRayPos.z - floorf(cRayPos.z)) >= 0.5f) ? (u32)cRayPos.z + 1 : (u32)cRayPos.z));
			...
			if (GetEntry(m_pRasterXY, cX, cY))
			{
				u32k cZYRes = GetEntryExact(m_pRasterZY, cZ, cY);
				if ((cZYRes & (cXNegative ? 0x1 : 0x2)) == 0)
					continue;//wrong side
				u32k cZXRes = GetEntryExact(m_pRasterZX, cZ, cX);
...
const bool CBBoxRaster::RetrieveXYBBox(AABB& rXYBox, const float cHeight, const bool cUseDefault) const
{
	...
	i32k cMiddleHeight = cUseDefault ? 0 : (i32)(cHeight * m_OSScale.z) + (scRasterRes >> 1);
	i32k cIntHeight = std::max(0, std::min((i32)(scRasterRes - 1), cMiddleHeight));
	i32 pX0 = -1, pX1 = -1;
	i32 curLine = cIntHeight;
	while (pX0 == -1 && curLine < scRasterRes)
	{
		for (i32 i = 0; i < scRasterRes; ++i)
		{
...
void CBBoxRaster::SaveTGA(tukk cpName) const
{
	string name(cpName);
	...
	static u32 pixels[scRasterRes * scRasterRes];
	memset(pixels, 0, scRasterRes * scRasterRes * sizeof(u32));
	//this could be way faster but its debug stuff anyway
	for (i32 i = 0; i < scRasterRes; ++i)
		for (i32 j = 0; j < scRasterRes; ++j)
