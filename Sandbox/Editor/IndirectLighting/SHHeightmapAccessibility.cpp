...
CHemisphereSink_SH::CHemisphereSink_SH(const DWORD indwAngleSteps, const DWORD indwWidth, const DWORD indwHeight)
	: m_SHRotMatrix(9), m_pTerrainObjMan(NULL), m_AngleSteps((u32)indwAngleSteps),
	m_SHSamples((size_t)indwAngleSteps), m_WaterLevel(0.f), m_Width((u32)indwWidth)
{
	...
	for (size_t i = 0; i < (size_t)m_AngleSteps; ++i)
	{
		...
		if (m_DoMP)
		{
			u32k cWedgeCountMP = cStepSampleCount / (2 + m_CoreCount) + 1;//only upper hs
			rWedgeSHVecMP.resize(m_CoreCount);
			for (i32 mp = 0; mp < m_CoreCount; ++mp)
				rWedgeSHVecMP[mp].reserve(cWedgeCountMP);
		}
#endif

		for (i32 s = 0; s < cStepSampleCount; ++s)
		{
...
)
{
	...
	if (m_DoMP)
		for (i32 i = 0; i < m_CoreCount; ++i)
		{
			...
			{
				DrxFatalError("Error spawning \"SHLighting__%u\" thread.", i);
			}
		}
#endif
}

void CHemisphereSink_SH::InitSample(SampleType& rInoutValue, u32k cX, u32k cY) const
{
	...
	rInoutValue.posX = (u16)cPos.x;
	rInoutValue.posY = (u16)cPos.y;
	u32 obstructionIndex = 0xFFFFFFFF;
	//WORLD_HMAP_COORDINATE_EXCHANGE
	float obstructionAmount = 1.f;
	u8k cObstruction = m_pTerrainObjMan->IsObstructed(cX, cY, m_Width, obstructionIndex, obstructionAmount, false);//obstruction map is in heightmap space
...
void CHemisphereSink_SH::AddWedgeArea(const float cCurWedgeAngle, const float cInfSlope, SampleType& rInoutValue, const float cSlopeDist)
{
	...
	u32k cAzimutIndex = (u32)(cCurWedgeAngle * (float)m_AngleSteps / (float)(gf_PI2));
...
void CHemisphereSink_SH::OnCalcEnd(SampleType& rInoutValue)
{
	...
	if (rInoutValue.HasObjectLink())
	{
		...
		const NQT::SVec2<u16> cConvPos = (NQT::SVec2<u16> )(NQT::SVec2<float>(cXPos, cYPos));
	...
	if (!rInoutValue.IsToBeIgnored())
	{
		...
		float obstructionAmount = 1.f;
		//WORLD_HMAP_COORDINATE_EXCHANGE
		u8k cObstruction = m_pTerrainObjMan->IsObstructed
		                           (
		  (u16)rInoutValue.posY,
		  (u16)rInoutValue.posX,
