...
const DrxIconColorMap CCollapsibleFrameHeader::s_colorMap = []
{
	...
	, m_bCollapsed(false)
{
	if (icon.size() > 0)
	{
		...
		m_pIconLabel->setPixmap(DrxIcon(icon).pixmap(16, 16, QIcon::Normal, QIcon::On));
	}
	else
	{
	...
	{
		m_collapsedIcon = DrxIcon(collapsedPixmap, s_colorMap);
	}
	...
	{
		m_expandedIcon = DrxIcon(expandedPixmap, s_colorMap);
	}
	SetupCollapseButton();
	SetupMainLayout();
