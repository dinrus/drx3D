...
i32 QRollupBar::addWidget(QWidget * widget)
{
	return attachNewWidget(widget);
}

i32 QRollupBar::attachNewWidget(QWidget * widget, i32 index /*= -1*/)
{
...
void QRollupBar::insertWidget(QWidget * widget, i32 index)
{
...
{
	i32 index = indexOf(widget);
	removeAt(index);
}

void QRollupBar::removeAt(i32 index)
{
...
QCollapsibleFrame* QRollupBar::rollupAt(i32 index) const
{
...
i32 QRollupBar::rollupIndexAt(QPoint pos) const
{
	for (i32 i = 0; i < m_subFrames.length(); i++)
	{
		QCollapsibleFrame* var = m_subFrames[i];
		QRollupBar* current = const_cast<QRollupBar*>(this);
		i32 framePos = var->mapTo(current, QPoint(0, 0)).y() + var->height();
		if (pos.y() < framePos)
		{
			return i;
		}
...
i32 QRollupBar::indexOf(QWidget* widget) const
{
	for (i32 i = 0; i < m_subFrames.length(); i++)
	{
...
QWidget* QRollupBar::getDragHandleAt(i32 index)
{
...
QWidget* QRollupBar::widget(i32 index) const
{
	if (index >= m_subFrames.count())
		return nullptr;
...
bool QRollupBar::eventFilter(QObject * o, QEvent * e)
{
		...
		if (me->buttons() & Qt::LeftButton)
		{
			if (m_canReorder)
			{
				...
				if (m_DragInProgress)
				{
					...
					i32 currentTarget = rollupIndexAt(m_pScrollBox->mapFromGlobal(me->globalPos()));
					if (currentTarget > -1)
					{
	...
	else if (e->type() == QEvent::MouseButtonRelease)
	{
		if (m_DragInProgress)
		{
			...
			i32 targetPos = rollupIndexAt(m_pScrollBox->mapFromGlobal(me->globalPos()));
			if (targetPos == -1)
