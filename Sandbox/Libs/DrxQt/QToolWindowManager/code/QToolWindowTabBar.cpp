...
#endif
{
	...
	m_tabSelectionButton->setIcon(DrxIcon("icons:General/Pointer_Down_Expanded.ico"));
...
void QToolWindowTabBar::onSelectionMenuClicked()
{
	...
	i32 activeAction = currentIndex();

	for (i32 i = 0; i < acts.size(); i++)
	{
	...
	{
		QList<QAction*> acts = m_tabSelectionMenu->actions();
		acts[activeAction]->setIcon(DrxIcon("icons:General/Tick.ico"));
	}
	connect(signalMapper, SIGNAL(mapped(i32)), this, SLOT(setCurrentIndex(i32)));
}
