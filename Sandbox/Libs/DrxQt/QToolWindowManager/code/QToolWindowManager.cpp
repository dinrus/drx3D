...
void QSizePreservingSplitter::childEvent(QChildEvent *c)
{
	...
	{
		QList<i32> l = sizes();
		i32 i = indexOf(static_cast<QWidget*>(c->child()));
		if (i != -1 && l.size() > 1)
		{
			i32 s = l[i] + handleWidth();
			if (i == 0)
			{
				l[1] = l[1]+s;
			}
...
void QToolWindowManager::addToolWindow(QWidget* toolWindow, IToolWindowArea* area, QToolWindowAreaReference::eType reference /*= Combine*/, i32 index /*= -1*/, QRect geometry /*= QRect()*/)
{
...
void QToolWindowManager::addToolWindows(const QList<QWidget*>& toolWindows, IToolWindowArea* area, QToolWindowAreaReference::eType reference /*= Combine*/, i32 index /*= -1*/, QRect geometry /*= QRect()*/)
{
...
void QToolWindowManager::moveToolWindow(QWidget* toolWindow, IToolWindowArea* area, QToolWindowAreaReference::eType reference /*= Combine*/, i32 index /*= -1*/, QRect geometry /*= QRect()*/)
{
...
void QToolWindowManager::moveToolWindows(const QList<QWidget*>& toolWindows, IToolWindowArea* area, QToolWindowAreaReference::eType reference /*= Combine*/, i32 index /*= -1*/, QRect geometry /*= QRect()*/)
{
...
QWidget* QToolWindowManager::splitArea(QWidget* area, QToolWindowAreaReference reference, QWidget* insertWidget)
{
	...
	QList<i32> sizes;
	i32 baseSize = (splitter->orientation() == Qt::Vertical ? splitter->height() : splitter->width());
		...
		if (handlerWidget)
		{
			if (!m_dragHandler->isHandlerWidget(handlerWidget))
			{
				...
				if (area && m_lastArea != area)
				{
					...
					m_lastArea = area;
					i32 delayTime = m_config.value(QTWM_RAISE_DELAY, 500).toInt();
					m_raiseTimer->stop();
					if (delayTime > 0)
					{
		...
		foreach(IToolWindowArea* area, m_areas)
		{
			...
			while (s && s->parentWidget())
			{
				...
				{					
					if (sp_s)
					{
						i32 index = sp_s->indexOf(s);
						QList<i32> sizes = sp_s->sizes();
				...
				{
					i32 index = sp_s->indexOf(s);
					QList<i32> newSizes = sp_s->sizes();
					QList<i32> oldSizes = s->sizes();
					i32 newSum = newSizes[index];
					i32 oldSum = 0;
					foreach(i32 i, oldSizes)
					{
						oldSum += i;
					}
					for(i32 i = oldSizes.count() - 1; i >= 0; i--)
					{
...
void QToolWindowManager::restoreState(const QVariant &data)
{
	...
	i32 stateFormat = dataMap["toolWindowManagerStateFormat"].toInt();
...
IToolWindowWrapper* QToolWindowManager::restoreWrapperState(const QVariantMap &data, i32 stateFormat, IToolWindowWrapper* wrapper)
{
...
QVariantMap QToolWindowManager::saveSplitterState(QSplitter *splitter) 
{
	...
	QVariantList items;
	for(i32 i = 0; i < splitter->count(); i++)
	{
...
QSplitter *QToolWindowManager::restoreSplitterState(const QVariantMap &data, i32 stateFormat)
{
...
void QToolWindowManager::resizeSplitter(QWidget* widget, QList<i32> sizes)
{
	...
	i32 scaleFactor = s->orientation() == Qt::Horizontal ? s->width() : s->height();
...
void QToolWindowManager::SwapAreaType(IToolWindowArea* oldArea, QTWMWrapperAreaType areaType)
{
	...
	if (parentSplitter)
	{
		i32 targetIndex = parentSplitter->indexOf(oldArea->getWidget());
	...
	if (m_lastArea == oldArea)
		m_lastArea = newArea;
	i32 oldAreaIndex = m_areas.indexOf(oldArea);
