	...
	m_areaDragCanStart(false)
{
	...
	connect(tabBar(), SIGNAL(tabCloseRequested(i32)), this, SLOT(closeTab(i32)));
}
...
void QToolWindowArea::addToolWindow(QWidget* toolWindow, i32 index /*= -1 */)
{
	addToolWindows(QList<QWidget*>() << toolWindow, index);
}

void QToolWindowArea::addToolWindows(const QList<QWidget*>& toolWindows, i32 index /*= -1 */)
{
	i32 newIndex = index;
...
void QToolWindowArea::closeTab(i32 index)
{
...
void QToolWindowArea::restoreState(const QVariantMap &data, i32 stateFormat) 
{
...
void QToolWindowArea::mouseReleaseEvent(QMouseEvent * e)
{
	if (m_manager->config().value(QTWM_SUPPORT_SIMPLE_TOOLS, false).toBool())
	{
		...
		{
			i32 p = tabBar()->rect().height();

			i32 tabIndex = tabBar()->tabAt(e->pos());
...
i32 QToolWindowArea::indexOf(QWidget* w) const
{
...
void QToolWindowArea::showContextMenu(const QPoint &point)
{
	...
	i32 tabIndex = tabBar()->tabAt(point);
	if (tabIndex >= 0)
	{
#if QT_VERSION <= 0x050000
