...
#if QT_VERSION >= 0x050000
bool QCustomTitleBar::nativeEvent(const QByteArray &eventType, uk message, long *result)
{
#if defined(WIN32) || defined(WIN64)
	...
	, m_contents(nullptr)
{
	...
	m_dwm = (uk )LoadLibraryEx(_T("dwmapi.dll"), NULL, LOAD_LIBRARY_SEARCH_SYSTEM32);
	if (m_dwm)
	{
		dwmExtendFrameIntoClientArea = (uk )GetProcAddress((HMODULE)m_dwm, "DwmExtendFrameIntoClientArea");
	}
	else
	{
...
#if QT_VERSION >= 0x050000
bool QCustomWindowFrame::nativeEvent(const QByteArray &eventType, uk message, long *result)
{
	if (!m_titleBar)
		return false;
...
bool QCustomWindowFrame::winEvent(MSG *msg, long *result)
{
	...
	case WM_GETMINMAXINFO:
	{
		...
		i32 miWidth = mi.rcWork.right - mi.rcWork.left;
		i32 miHeight = mi.rcWork.bottom - mi.rcWork.top;
		i32 mi2Width = mi2.rcWork.right - mi2.rcWork.left;
		i32 mi2Height = mi2.rcWork.bottom - mi2.rcWork.top;
...
void QCustomWindowFrame::updateWindowFlags()
{
	...
	const u64 currentStyle = GetWindowLong((HWND)winId(), GWL_STYLE);
