...
void CSubstanceManager::DrxSubstanceCallbacks::outputComputed(
...
void CSubstanceManager::DrxSubstanceCallbacks::outputComputed(SubstanceAir::UInt runUid, const SubstanceAir::GraphInstance* graphInstance, SubstanceAir::OutputInstanceBase* outputInstance)
{
...
{
	u32k crc = CCrc32::ComputeLowercase(archiveName);
...
{
	u32k crc = CCrc32::ComputeLowercase(preset->GetSubstanceArchive());
	PackageMap::iterator search = m_loadedPackages.find(crc);
	if (search != m_loadedPackages.end()) {
		i32& refcount = m_refCount[crc];
		refcount--;
...
CSubstanceManager::CSubstanceManager()
{
	...
	m_pCallbacks = new CSubstanceManager::DrxSubstanceCallbacks();
