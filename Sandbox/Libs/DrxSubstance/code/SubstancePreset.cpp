...
namespace Serialization
{
	...
	inline StdFunctionActionButton LambdaActionButton(const StdFunctionActionButtonCalback& callback, tukk icon = "")
	{
...
void CSubstancePreset::SSerializer::SInputCategory::Serialize(Serialization::IArchive& ar)
{
	for each (SubstanceAir::InputInstanceBase* var in inputs)
	{
		...
		if (desc.mIdentifier == "$randomseed")
		{
			...
			i32 value = inst->getValue();
			if (ar.openBlock("randomseed", "<Random Seed"))
			{
				
				ar(value, "randomSeed", "^!");
				ar(Serialization::LambdaActionButton([inst, &ar]() {
					inst->setVal(gRandomGenerator.GetRandom(std::numeric_limits<i32>::min(), std::numeric_limits<i32>::max()));
				}), "randomize", "^Randomize");
		...
		else if (desc.mIdentifier == "$outputsize")
		{
			...
			if (ar.isEdit())
			{
				...
				for (i32 i = 2; i < 14; i++)
				{
					stringList.push_back(std::to_string(i32(pow(2, i))).c_str());
				}
				Serialization::StringList stringListY(stringList);
				Serialization::StringListValue stringListValue(stringList, std::to_string(i32(pow(2, sValue.x))).c_str());
				Serialization::StringListValue stringListValueY(stringListY, std::to_string(i32(pow(2, sValue.y))).c_str());
				...
				if (ar.isInput())
				{
					...
					i32 val = (i32)log2(std::stoi(strValue.c_str()));
					SubstanceAir::string strValueY(stringListValueY.c_str());
					i32 valY = (i32)log2(std::stoi(strValueY.c_str()));
					if (!preset->m_uniformResolution)
					{
				...
				if (ar.isInput())
				{
					inst->setVal(preset->m_resolutionBackup);
				}
			...
			case Substance_IType_Float2:
			{
				...
				if (ar.isInput())
				{
					inst->setVal(SubstanceAir::Vec2Float(value.x, value.y));
				}
			}	
				break;
			case Substance_IType_Float3:
			{
				...
				if (tDesc.mGuiWidget == SubstanceAir::Input_Color)
				{
					...
					if (ar.isInput())
					{
						inst->setVal(SubstanceAir::Vec3Float(color.r, color.g, color.b));
					}
				}
				else {
					...
					if (ar.isInput())
					{
						inst->setVal(SubstanceAir::Vec3Float(value.x, value.y, value.z));
					}
				}
			...
			case Substance_IType_Float4:
			{
				...
				if (tDesc.mGuiWidget == SubstanceAir::Input_Color)
				{
					...
					if (ar.isInput())
					{
						inst->setVal(SubstanceAir::Vec4Float(color.r, color.g, color.b, color.a));
					}
				}
				else {
					...
					if (ar.isInput())
					{
						inst->setVal(SubstanceAir::Vec4Float(value.x, value.y, value.z, value.w));
					}
				}
				...
				if (tDesc.mGuiWidget == SubstanceAir::Input_Combobox)
				{
					...
					if (ar.isInput())
					{
						...
						auto it = std::find_if(tDesc.mEnumValues.begin(), tDesc.mEnumValues.end(),
							[&sValue](const std::pair<i32, SubstanceAir::string>& p)
						{ return p.second == sValue; });
						if (it != tDesc.mEnumValues.end())
						{
							inst->setVal(it->first);
						}
			...
			case Substance_IType_Integer2:
			{
				...
				if (ar.isInput())
				{
					inst->setVal(SubstanceAir::Vec2Int(value.x, value.y));
				}
			}
			break;
			case Substance_IType_Integer3:
			{
				...
				if (ar.isInput())
				{
					inst->setVal(SubstanceAir::Vec3Int(value.x, value.y, value.z));
				}
			}
				break;
			case Substance_IType_Integer4:
			{
				...
				if (ar.isInput())
				{
					inst->setVal(SubstanceAir::Vec4Int(value.x, value.y, value.z, value.w));
				}
			}
				break;
			default:
...
void CSubstancePreset::SetGraphResolution(i32k& x, i32k& y)
{
	...
	if (pInput)
	{
		pInput->setVal(SubstanceAir::Vec2Int(x, y));
	}
...
void CSubstancePreset::FillFormatForRender(const SSubstanceRenderData& renderData, const SubstanceAir::Vec2Int& baseResolution, SubstanceAir::OutputFormat& newFormat)
{
	...
	newFormat.forceWidth = renderData.output.resolution == 0 ? SubstanceAir::OutputFormat::UseDefault : (i32)pow(2, baseResolution.x - renderData.output.resolution);
	newFormat.forceHeight = renderData.output.resolution == 0 ? SubstanceAir::OutputFormat::UseDefault : (i32)pow(2, baseResolution.y - renderData.output.resolution);
	...
	for (size_t i = 0; i <= (renderData.skipAlpha ? 2 : 3); i++)
	{
		...
		SubstanceAir::UInt channel = renderData.output.channels[i].channel == -1 ? i : max((i32)renderData.output.channels[i].channel, 0);
		i32 sourceId = renderData.output.channels[i].sourceId;
		if (sourceId > -1)
		{
