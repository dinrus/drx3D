...
enum EnumSharedMemoryServerStatus
{
	...
	CMD_DRX3D_LOADING_COMPLETED,
	CMD_DRX3D_LOADING_FAILED,
	CMD_DRX3D_SAVING_COMPLETED,
	CMD_DRX3D_SAVING_FAILED,
	CMD_MJCF_LOADING_COMPLETED,
	CMD_MJCF_LOADING_FAILED,
	CMD_REQUEST_INTERNAL_DATA_COMPLETED,
	CMD_REQUEST_INTERNAL_DATA_FAILED,
	CMD_DRX3D_DATA_STREAM_RECEIVED_COMPLETED,
	CMD_DRX3D_DATA_STREAM_RECEIVED_FAILED,
...
{
	i32 m_parentBodyIndex;
	i32 m_parentJointIndex;
	i32 m_childBodyIndex;
	i32 m_childJointIndex;
	double m_parentFrame[7];
	double m_childFrame[7];
	double m_jointAxis[3];
	i32 m_jointType;
	double m_maxAppliedForce;
	i32 m_userConstraintUniqueId;
	double m_gearRatio;
	i32 m_gearAuxLink;
...
struct b3BodyInfo
{
	...
	char m_bodyName[1024];  // for RigidBody, it does not have a base, but can still have a body name from urdf
};
...
struct b3CameraImageData
{
	i32 m_pixelWidth;
	i32 m_pixelHeight;
	u8k* m_rgbColorData;  //3*m_pixelWidth*m_pixelHeight bytes
	const float* m_depthValues;           //m_pixelWidth*m_pixelHeight floats
	i32k* m_segmentationMaskValues;  //m_pixelWidth*m_pixelHeight ints
};
...
enum eMeshDataFlags
{
	D3_MESH_DATA_SIMULATION_MESH = 1,
	D3_MESH_DATA_SIMULATION_INDICES = 2,
	D3_MESH_DATA_GRAPHICS_INDICES = 4,
	D3_MESH_DATA_SIMULATION_MESH_VELOCITY = 8,
};

enum eMeshDataEnum
{
	D3_MESH_DATA_COLLISIONSHAPEINDEX=1,
	D3_MESH_DATA_FLAGS=2,
};
...
{
	i32 m_controllerId;  //valid for VR_CONTROLLER_MOVE_EVENT and VR_CONTROLLER_BUTTON_EVENT
	i32 m_deviceType;
	i32 m_numMoveEvents;
	i32 m_numButtonEvents;
	...
	i32 m_buttons[MAX_VR_BUTTONS];                  //valid if VR_CONTROLLER_BUTTON_EVENT, see b3VRButtonInfo
};

struct b3VREventsData
{
	i32 m_numControllerEvents;
...
{
	i32 m_keyCode;   //ascii
	i32 m_keyState;  // see b3VRButtonInfo
};
...
struct b3RayHitInfo
{
	...
	i32 m_hitObjectUniqueId;
	i32 m_hitObjectLinkIndex;
...
struct b3VisualShapeData
{
	i32 m_objectUniqueId;
	i32 m_linkIndex;
	i32 m_visualGeometryType;  //box primitive, sphere primitive, triangle mesh
...
struct b3CollisionShapeData
{
	i32 m_objectUniqueId;
	i32 m_linkIndex;
	i32 m_collisionGeometryType;      //GEOM_BOX, GEOM_SPHERE etc
...
#define D3_MAX_PLUGIN_ARG_SIZE 128
#define D3_MAX_PLUGIN_ARG_TEXT_LEN 1024

struct b3PluginArguments
{
	char m_text[D3_MAX_PLUGIN_ARG_TEXT_LEN];
	i32 m_numInts;
	i32 m_ints[D3_MAX_PLUGIN_ARG_SIZE];
	i32 m_numFloats;
	double m_floats[D3_MAX_PLUGIN_ARG_SIZE];
};
...
{
	eVISUAL_SHAPE_DOUBLE_SIDED = 4,//see D3_INSTANCE_DOUBLE_SIDED
};

//limits for vertices/indices in Pydrx3d::createCollisionShape
//Make sure the data fits in SHARED_MEMORY_MAX_STREAM_CHUNK_SIZE
//(numVertices*sizeof(double)*3 + numIndices*sizeof(i32)) < SHARED_MEMORY_MAX_STREAM_CHUNK_SIZE


#ifdef __APPLE__
#define D3_MAX_NUM_VERTICES 8192
#define D3_MAX_NUM_INDICES 32768
#else
#define D3_MAX_NUM_VERTICES 131072
#define D3_MAX_NUM_INDICES 524288
#endif
