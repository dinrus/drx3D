...
bool MuJoCoPhysicsServerCommandProcessor::connect()
{
	...
	i32 result = mj_activate("mjkey.txt");
	if (result == 1)
	{
...
bool MuJoCoPhysicsServerCommandProcessor::processCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
...
bool MuJoCoPhysicsServerCommandProcessor::processRequestInternalDataCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::processSyncBodyInfoCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::processSyncUserDataCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::processLoadMJCFCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool hasStatus = true;
	...
	tukk fileName = mjcfArgs.m_mjcfFileName;
	...
	if (strlen(fileName) > 0)
	{
		...
		bool fileFound = (ResourcePath::findResourcePath(fileName, relativeFileName, 1024) > 0);
		if (!fileFound)
		{
		...
		else
		{
...

			{
				...
				{
					i32 maxBodies = d3Min(MAX_SDF_BODIES, mnew->nbody);
...
bool MuJoCoPhysicsServerCommandProcessor::processRequestBodyInfoCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool hasStatus = true;
	...
	{
		tukk name = m_data->m_mujocoModel->names + m_data->m_mujocoModel->name_bodyadr[sdfInfoArgs.m_bodyUniqueId];
...
bool MuJoCoPhysicsServerCommandProcessor::processForwardDynamicsCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool hasStatus = true;
	...
	if (m_data->m_mujocoModel)
	{
		...
		Scalar deltaTimeScaled = m_data->m_physicsDeltaTime;

		if (m_data->m_numSimulationSubSteps > 0)
		{
			for (i32 i = 0; i < m_data->m_numSimulationSubSteps; i++)
			{
...
bool MuJoCoPhysicsServerCommandProcessor::processSendPhysicsParametersCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::processRequestActualStateCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool hasStatus = true;
	...
	if (m_data->m_mujocoModel)
	{
		...
		if (bodyUniqueId >= 0 && bodyUniqueId < m_data->m_mujocoModel->nbody)
		{
			...
			{
				i32 rootLink = 0;  //todo check
				i32 type = (m_data->m_mujocoModel->jnt_type + m_data->m_mujocoModel->body_jntnum[bodyUniqueId])[rootLink];
				//assume mjJNT_FREE?
				i32 qposAdr = (m_data->m_mujocoModel->jnt_qposadr + m_data->m_mujocoModel->body_jntnum[bodyUniqueId])[rootLink];
			...
			i32 numLinks = m_data->m_mujocoModel->body_jntnum[bodyUniqueId];
			for (i32 l = 0; l < numLinks; l++)
			{
				i32 type = (m_data->m_mujocoModel->jnt_type + m_data->m_mujocoModel->body_jntnum[bodyUniqueId])[l];
				//i32 type=(m_data->m_mujocoModel->jnt_type+m_data->m_mujocoModel->body_jntnum[bodyUniqueId])[l];
...
bool MuJoCoPhysicsServerCommandProcessor::processResetSimulationCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool hasStatus = true;
...
bool MuJoCoPhysicsServerCommandProcessor::receiveStatus(struct SharedMemoryStatus& serverStatusOut, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	return false;
}
