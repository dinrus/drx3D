...
void PhysicsDirect::debugDraw(i32 debugDrawMode)
{
...
bool PhysicsDirect::processMeshData(const struct SharedMemoryCommand& orgCommand)
{
	...
	do
	{
		...
		if (hasStatus)
		{
			Assert(m_data->m_serverStatus.m_type == CMD_REQUEST_MESH_DATA_COMPLETED);

			if (m_data->m_verboseOutput)
			{
				drx3DPrintf("Mesh data OK\n");
			}

			const b3SendMeshDataArgs& args = serverCmd.m_sendMeshDataArgs;
			i32 numTotalPixels = args.m_startingVertex +
								 args.m_numVerticesCopied + args.m_numVerticesRemaining;

			Vec3* verticesReceived =
				(Vec3*)&m_data->m_bulletStreamDataServerToClient[0];

			
			m_data->m_cachedVertexPositions.resize(args.m_startingVertex +
													args.m_numVerticesCopied);

			for (i32 i = 0; i < args.m_numVerticesCopied; i++)
			{
...
void PhysicsDirect::processBodyJointInfo(i32 bodyUniqueId, const SharedMemoryStatus& serverCmd)
{
...
void PhysicsDirect::postProcessStatus(const struct SharedMemoryStatus& serverCmd)
{
	switch (serverCmd.m_type)
	{
		...
		case CMD_SDF_LOADING_COMPLETED:
		{
			...
			AlignedObjectArray<i32> bodyIdArray;
			AlignedObjectArray<i32> constraintIdArray;
			...
			{
				clearCachedBodies();
				i32k* bodyIds = (i32*)m_data->m_bulletStreamDataServerToClient;
				i32k* constaintIds = bodyIds + numBodies;

				for (i32 i = 0; i < numConstraints; i++)
				{
					i32 constraintUid = constaintIds[i];
					constraintIdArray.push_back(constraintUid);
				}
				for (i32 i = 0; i < numBodies; i++)
				{
					i32 bodyUid = bodyIds[i];
			...
			else
			{
				...
				for (i32 i = 0; i < numBodies; i++)
				{
					i32 bodyUid = serverCmd.m_sdfLoadedArgs.m_bodyUniqueIds[i];
					bodyIdArray.push_back(bodyUid);
				}
			...
			for (i32 i = 0; i < numConstraints; i++)
			{
				i32 constraintUid = constraintIdArray[i];
			...
			for (i32 i = 0; i < numBodies; i++)
			{
				i32 bodyUniqueId = bodyIdArray[i];
	  ...
	  case CMD_REQUEST_MESH_DATA_COMPLETED:
		...
		{
                       i32 bodyUniqueId = serverCmd.m_loadSoftBodyResultArguments.m_objectUniqueId;
                       BodyJointInfoCache2* bodyJoints = new BodyJointInfoCache2;
                       m_data->m_bodyJointMap.insert(bodyUniqueId, bodyJoints);
                       bodyJoints->m_bodyName = serverCmd.m_dataStreamArguments.m_bodyName;
		...
		case CMD_SYNC_USER_DATA_COMPLETED:
		{
			...
			i32* identifiers = new i32[numIdentifiers];
			memcpy(identifiers, &m_data->m_bulletStreamDataServerToClient[0], numIdentifiers * sizeof(i32));
...
bool PhysicsDirect::processCustomCommand(const struct SharedMemoryCommand& orgCommand)
{
	...
	do
	{
		...
		if (hasStatus)
		{
			...
			if (m_data->m_verboseOutput)
			{
				drx3DPrintf("Success receiving %d return data\n",
					serverCmd.m_numDataStreamBytes);
			}

			
			Assert(m_data->m_serverStatus.m_type == CMD_CUSTOM_COMMAND_COMPLETED);
...
i32 PhysicsDirect::getNumBodies() const
{
	return m_data->m_bodyJointMap.size();
}

void PhysicsDirect::removeCachedBody(i32 bodyUniqueId)
{
...
i32 PhysicsDirect::getNumUserConstraints() const
{
	return m_data->m_userConstraintInfoMap.size();
}

i32 PhysicsDirect::getUserConstraintInfo(i32 constraintUniqueId, struct b3UserConstraint& info) const
{
...
i32 PhysicsDirect::getUserConstraintId(i32 serialIndex) const
{
...
i32 PhysicsDirect::getBodyUniqueId(i32 serialIndex) const
{
...
bool PhysicsDirect::getBodyInfo(i32 bodyUniqueId, struct b3BodyInfo& info) const
{
	BodyJointInfoCache2** bodyJointsPtr = m_data->m_bodyJointMap[bodyUniqueId];
	if (bodyJointsPtr && *bodyJointsPtr)
	{
...
i32 PhysicsDirect::getNumJoints(i32 bodyUniqueId) const
{
	BodyJointInfoCache2** bodyJointsPtr = m_data->m_bodyJointMap[bodyUniqueId];
	if (bodyJointsPtr && *bodyJointsPtr)
	{
...
i32 PhysicsDirect::getNumDofs(i32 bodyUniqueId) const
{
        BodyJointInfoCache2** bodyJointsPtr = m_data->m_bodyJointMap[bodyUniqueId];
        if (bodyJointsPtr && *bodyJointsPtr)
        {
...
bool PhysicsDirect::getJointInfo(i32 bodyIndex, i32 jointIndex, struct b3JointInfo& info) const
{
...
void PhysicsDirect::setSharedMemoryKey(i32 key)
{
}

void PhysicsDirect::uploadBulletFileToSharedMemory(tukk data, i32 len)
{
...
void PhysicsDirect::uploadRaysToSharedMemory(struct SharedMemoryCommand& command, const double* rayFromWorldArray, const double* rayToWorldArray, i32 numRays)
{
...
i32 PhysicsDirect::getNumDebugLines() const
{
...
void PhysicsDirect::getCachedMassMatrix(i32 dofCountCheck, double* massMatrix)
{
...
bool PhysicsDirect::getCachedUserData(i32 userDataId, struct b3UserDataValue& valueOut) const
{
...
i32 PhysicsDirect::getCachedUserDataId(i32 bodyUniqueId, i32 linkIndex, i32 visualShapeIndex, tukk key) const
{
...
i32 PhysicsDirect::getNumUserData(i32 bodyUniqueId) const
{
	BodyJointInfoCache2** bodyJointsPtr = m_data->m_bodyJointMap[bodyUniqueId];
	if (!bodyJointsPtr || !(*bodyJointsPtr))
	{
...
void PhysicsDirect::getUserDataInfo(i32 bodyUniqueId, i32 userDataIndex, tukk* keyOut, i32* userDataIdOut, i32* linkIndexOut, i32* visualShapeIndexOut) const
{
	BodyJointInfoCache2** bodyJointsPtr = m_data->m_bodyJointMap[bodyUniqueId];
...
void PhysicsDirect::pushProfileTiming(tukk timingName)
{
