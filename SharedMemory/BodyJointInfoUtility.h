...
*/
inline tuk strDup(tukk const str)
{
#ifdef _WIN32
	return _strdup(str);
#else
...
void addJointInfoFromMultiBodyData(const T* mb, U* bodyJoints, bool verboseOutput)
{
	...
	if (mb->m_baseName)
	{
		if (verboseOutput)
		{
			drx3DPrintf("mb->m_baseName = %s\n", mb->m_baseName);
		}
	}
	i32 qOffset = 7;
	i32 uOffset = 6;

	for (i32 link = 0; link < mb->m_numLinks; link++)
	{
		{
			b3JointInfo info;
			...
			if (mb->m_links[link].m_linkName)
			{
				if (verboseOutput)
				{
					drx3DPrintf("mb->m_links[%d].m_linkName = %s\n", link,
			...
			if (mb->m_links[link].m_jointName)
			{
				if (verboseOutput)
				{
					drx3DPrintf("mb->m_links[%d].m_jointName = %s\n", link,
