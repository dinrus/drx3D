...
public:
	...
	tukk         GetRawName() const            { return m_rawName.c_str(); }
	u16              GetSize() const               { return m_size; }
	u16              GetAlignment() const          { return m_alignment; }
...
private:
	...
	{
		static_assert(sizeof(TYPE) <= UINT16_MAX, "Size of type exceeds limit of u16.");
		static_assert(AlignOf<TYPE>::Value <= UINT16_MAX, "Alignment requirement of type exceeds limit of u16.");
	}
...
template<typename TYPE>
constexpr inline Drx::Type::CTypeId TypeIdOf()
{
	return Drx::Type::CTypeId::Create<TYPE>();
}

template<typename TYPE>
constexpr inline Drx::Type::CTypeId::ValueType TypeIdValue()
{
	typedef typename Drx::Type::PureType<TYPE>::Type T;
	//return Drx::Type::Utils::SCompileTime_TypeInfo<T>::GetCrc32();
	return CCrc32::Compute_CompileTime(Drx::Type::Utils::SCompileTime_TypeInfo<T>::GetName().c_str(), Drx::Type::Utils::SCompileTime_TypeInfo<T>::GetName().length());
}

template<typename TYPE>
static const Drx::Type::CTypeDesc& TypeDescOf()
{
	static Drx::Type::CTypeDesc typeDesc = Drx::Type::CTypeDesc::Create<TYPE>();
	return typeDesc;
}

}

using DrxTypeId = Drx::Type::CTypeId;
using DrxTypeDesc = Drx::Type::CTypeDesc;
