...
template<typename MemberFunction> struct DrxMemFunTraits
{
	static constexpr bool isMemberFunction = false;
};

template<typename Ret, typename Class, typename... Args>
struct DrxMemFunTraits<Ret(Class::*)(Args...)>
{
...
template<typename Ret, typename Class, typename... Args>
struct DrxMemFunTraits<Ret(Class::*)(Args...) const>
{
	typedef Ret(Signature)(Args...);
	static constexpr bool isMemberFunction = true;
};
...
struct DrxFunctionTraits
{
	typedef decltype(&Function::operator()) OperatorCallType;
	typedef typename DrxMemFunTraits<OperatorCallType>::Signature Signature;
};

template<typename Ret, typename... Args>
struct DrxFunctionTraits<Ret(Args...)>
{
	typedef Ret(Signature)(Args...);
};
...
//Intended breach of DrxENGINE coding standard to match c++ standard for casting, i.e. static_cast/dynamic_cast etc
template<typename Sig1, typename Sig2>
inline std::function<Sig1> function_cast(const std::function<Sig2>& f)
{
	return DrxFunctionPrivate::FunctionWrapper<Sig1>::Cast(f);
}

template<typename Sig1, typename Function, typename std::enable_if<IsCallable<Function>::value, i32>::type* = 0>
inline std::function<Sig1> function_cast(const Function& f)
{
	typedef typename DrxFunctionTraits<Function>::Signature Sig2;
	return DrxFunctionPrivate::FunctionWrapper<Sig1>::Cast(std::function<Sig2>(f));
}
...
inline typename DrxFunctionPrivate::MemberFunctionWrapper<Object, Function>::ReturnType WrapMemberFunction(Object* obj, Function function)
{
	return DrxFunctionPrivate::MemberFunctionWrapper<Object, Function>::Wrap(obj, function);
}
