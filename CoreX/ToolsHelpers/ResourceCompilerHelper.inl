...
namespace
{
	...
	static bool CallProcessHelper(const wchar_t* szStartingDirectory, const wchar_t* szCommandLine, bool bShowWindow, LineStreamBuffer *pListener, int& exitCode, void* pEnvironment, CRcCancellationToken* pCancellationToken)
	{
		...
		SettingsUprHelpers::CFixedString<wchar_t, MAX_PATH * 3> wRemoteCmdLine;
...
//////////////////////////////////////////////////////////////////////////
// Modified version of of FileUtil::DirectoryExists() from Code/Tools/DrxCommonTools/FileUtil.h
//
...
{
	SettingsUprHelpers::CFixedString<wchar_t, MAX_PATH * 2> dir;
...
{
	SettingsUprHelpers::CFixedString<wchar_t, MAX_PATH * 4 + 150> tmp;
...
//////////////////////////////////////////////////////////////////////////
static void DrxFindEngineToolsFolder(unsigned int nEngineToolsPathSize, wchar_t* szEngineToolsPath)
{
	const size_t len = ::GetModuleFileNameW(DrxGetCurrentModule(), szEngineToolsPath, nEngineToolsPathSize);
	...
	SettingsUprHelpers::CFixedString<wchar_t, MAX_PATH * 2> filename;
...
{
	bool dirFound = true;
	CSettingsUprTools smTools = CSettingsUprTools();
	SettingsUprHelpers::CFixedString<wchar_t, MAX_PATH * 3> rcIniPath;
	{
		wchar_t pathBuffer[512];
		switch (rcExePath)
		{
		case eRcExePath_registry:
			smTools.GetRootPathUtf16(true, SettingsUprHelpers::CWCharBuffer(pathBuffer, sizeof(pathBuffer)));
			if (!pathBuffer[0])
			{
				dirFound = false;
			}
		...
		case eRcExePath_editor:
			DrxFindEngineToolsFolder(DRX_ARRAY_COUNT(pathBuffer), pathBuffer);
			if (!pathBuffer[0])
			{
				dirFound = false;
			}
	...
	SettingsUprHelpers::GetAsciiFilename(rcIniPath.c_str(), SettingsUprHelpers::CCharBuffer(buffer, sizeof(buffer)));
	...
	const wchar_t* szWorkingDirectory)
{
	...
	SettingsUprHelpers::CFixedString<wchar_t, MAX_PATH * 3> rcDirectory;
	{
		wchar_t pathBuffer[512];
		switch (rcExePath)
		{
	...
	smTools.GetEngineSettingsUpr()->GetValueByRef("RC_Parameters", SettingsUprHelpers::CWCharBuffer(szRegSettingsBuffer, sizeof(szRegSettingsBuffer)));
	...
	{
		wchar_t buffer[20];
		smTools.GetEngineSettingsUpr()->GetValueByRef("ShowWindow", SettingsUprHelpers::CWCharBuffer(buffer, sizeof(buffer)));
...
ERcExitCode CResourceCompilerHelper::InvokeResourceCompiler(const char* szSrcFilePath, const char* szDstFilePath, const bool bUserDialog, const bool bRefresh)
{
	...
	GetCurrentDirectoryW(DRX_ARRAY_COUNT(szProjectDir), szProjectDir);

	SettingsUprHelpers::CFixedString<wchar_t, 512> wRemoteCmdLine;
	SettingsUprHelpers::CFixedString<wchar_t, 512> wDir;
	CSettingsUprTools smTools = CSettingsUprTools();

	wchar_t szToolsDir[512];
	DrxFindEngineToolsFolder(DRX_ARRAY_COUNT(szToolsDir), szToolsDir);
	if (!szToolsDir[0])
	{
	...
	RemovePath(szDstFilePath, szDstFilename, DRX_ARRAY_COUNT(szDstFilename));
	RemoveFilename(szDstFilePath, szDstPath, DRX_ARRAY_COUNT(szDstPath));
	...
	char tmp1[512];
	char tmp2[512];
	SettingsUprHelpers::CCharBuffer dst1(tmp1, 512);
	SettingsUprHelpers::ConvertUtf16ToUtf8(wDir.c_str(), dst1);
	SettingsUprHelpers::CCharBuffer dst2(tmp2, 512);
	SettingsUprHelpers::ConvertUtf16ToUtf8(wRemoteCmdLine.c_str(), dst2);
...
void CResourceCompilerHelper::ReplaceExtension(const char* path, const char* new_ext, char* buffer, size_t bufferSizeInBytes)
{
	...
	drx_strcpy(buffer, bufferSizeInBytes, p.c_str());
}
...
void CResourceCompilerHelper::GetOutputFilename(const char* szFilePath, char* buffer, size_t bufferSizeInBytes)
{
	...
	drx_strcpy(buffer, bufferSizeInBytes, szFilePath);
}
