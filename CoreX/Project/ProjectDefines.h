...
// Scaleform base configuration
	...
	#undef DRX_FEATURE_SCALEFORM_HELPER
#endif
#if DRX_PLATFORM_MOBILE || DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID || DRX_PLATFORM_APPLE
...
//! 0=off, 1=on
//! \see http://wiki/bin/view/DinrusX/TerrainTexCompression
...
//! Since read accesses tend to be used in flow-control logic, constants allow for
//! better optimization by the compiler.
#define LOG_CONST_CVAR_ACCESS 0

#if DRX_PLATFORM_WINDOWS || LOG_CONST_CVAR_ACCESS
...
//! Enable/disable map load slicing functionality from the build.
	...
	#else
extern void SliceAndSleep(tukk pFunc, i32 line);
...
#if !defined(_RELEASE)
	#define DRX_UNIT_TESTING

// configure the unit testing framework, if we have exceptions or not
	#if !(DRX_PLATFORM_APPLE || DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID || DRX_PLATFORM_ORBIS)
		#define DRX_UNIT_TESTING_USE_EXCEPTIONS
	#endif
...
#if (DRX_PLATFORM_WINDOWS || DRX_PLATFORM_ORBIS || DRX_PLATFORM_DURANGO) && !defined(RESOURCE_COMPILER)
...
// Define here if you want to enable any external profiling tools
		...
		#endif
		#if defined(USE_BROFILER) && DRX_PLATFORM_WINDOWS
...
#if DRX_PLATFORM_WINDOWS && (!defined(_RELEASE) || defined(RESOURCE_COMPILER))
	#define DRX_ENABLE_RC_HELPER 1
#endif
...
// Specific features flags
		...
		#if !DRX_PLATFORM_ORBIS   // Causes memory wastage in RenderMesh.cpp
...
#if defined(RELEASE) && DRX_PLATFORM_WINDOWS && defined(DEDICATED_SERVER) && 0
...
//# define DRX_PROFILE_MARKERS_USE_GPA
//# define DRX_PROFILE_MARKERS_USE_NVTOOLSEXT
#endif
...
#ifdef RELEASE
// Forces the .drxproject file to be read from a .pak file instead of directly from disk.
#define DRX_FORCE_DRXPROJECT_IN_PAK
#endif
...
//! This enables checking of CRCs on archived files when they are loaded fully and synchronously in DrxPak.
