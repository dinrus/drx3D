...
// DrxStringT<T>, (::string, ::wstring):            yes    yes    implied by type (also Stack and Fixed variants)
// std::basic_string<T>, STxt, std::wstring: yes    yes    implied by type
...
// u32 (single UCS code-point):                  yes    no     not present
...
#ifndef assert
// Some tools use CRT's assert, most engine and game modules use DrxAssert.h (via platform.h maybe).
...
template<typename T, size_t S> class DrxStackStringT;
template<size_t S> class DrxFixedStringT;
template<size_t S> class DrxFixedWStringT;
template<typename T> class DrxStringLocalT;
template<typename T> class DrxStringT;
class QChar;
class QString;
namespace Unicode
{
...
template<typename T, bool InferEncoding>
struct SBindObject<DrxStringT<T>, InferEncoding>
{
	typedef typename add_const<T>::type CharType;
	static const bool  isValid = SValidChar<T, InferEncoding, true>::value;
	static const EBind value = isValid ? eBind_Data : eBind_Impossible;
...
template<typename T, bool InferEncoding>
struct SBindObject<DrxStringLocalT<T>, InferEncoding>
{
	typedef typename add_const<T>::type CharType;
	static const bool  isValid = SValidChar<T, InferEncoding, true>::value;
	static const EBind value = isValid ? eBind_Data : eBind_Impossible;
...
struct SBindObject<DrxStackStringT<T, S>, InferEncoding>
{
	typedef typename add_const<T>::type CharType;
	static const bool  isValid = SValidChar<T, InferEncoding, true>::value;
	static const EBind value = isValid ? eBind_Data : eBind_Impossible;
...
struct SBindObject<DrxFixedStringT<S>, InferEncoding>
{
...
template<size_t S, bool InferEncoding>
struct SBindObject<DrxFixedWStringT<S>, InferEncoding>
{
...
template<typename T, bool InferEncoding>
struct SBindOutput<DrxStringT<T>, InferEncoding>
{
	typedef T CharType;
	static const bool  isValid = SValidChar<T, InferEncoding, false>::value;
	static const EBind value = isValid ? eBind_Data : eBind_Impossible;
...
template<typename T, bool InferEncoding>
struct SBindOutput<DrxStringLocalT<T>, InferEncoding>
{
	typedef T CharType;
	static const bool  isValid = SValidChar<T, InferEncoding, false>::value;
	static const EBind value = isValid ? eBind_Data : eBind_Impossible;
...
template<typename T, size_t S, bool InferEncoding>
struct SBindOutput<DrxStackStringT<T, S>, InferEncoding>
{
	typedef T CharType;
	static const bool  isValid = SValidChar<T, InferEncoding, false>::value;
	static const EBind value = isValid ? eBind_Data : eBind_Impossible;
...
{
	typedef typename conditional<Input, u16k, u16>::type type;
...
struct SCharacterTrait<T, sizeof(char)>
{
	...
	{
		return ::strlen(SafeCast<tukk >(nts));
	}
	static size_t StrNLen(const T* ptr, size_t len)        //!< Narrow CRT strnlen.
	{
		return ::strnlen(SafeCast<tukk >(ptr), len);
	}
};
template<typename T>
...
inline void Feed(const SPackedIterators<InputIteratorType>& its, Sink& out, integral_constant<EBind, eBind_Iterators> )
{
	...
	for (InputIteratorType it = its.begin; it != its.end; ++it)
	{
		...
		u32k item = static_cast<u32>(bound);
		out(item);
	}
}
...
inline void Feed(const InputStringType& in, Sink& out, integral_constant<EBind, eBind_Buffer> )
{
	...
	for (size_t i = 0; i < length; ++i, ++ptr)
	{
		...
		u32k item = static_cast<u32>(unbound);
		out(item);
	}
}
...
template<typename InputCharType>
inline ukk EncodedPointer(const SPackedIterators<const InputCharType*>& its, integral_constant<EBind, eBind_Iterators> )
{
	return its.begin;
}
...
template<typename InputStringType>
inline ukk EncodedPointer(const InputStringType& in, integral_constant<EBind, eBind_Data> )
{
	return in.data();
}
...
template<typename InputStringType>
inline ukk EncodedPointer(const InputStringType& in, integral_constant<EBind, eBind_Literal> )
{
	static_assert(is_array<InputStringType>::value && extent<InputStringType>::value > 0, "'T' must be an array with at least one element!");
...
template<typename InputStringType>
inline ukk EncodedPointer(const InputStringType& in, integral_constant<EBind, eBind_Buffer> )
{
	static_assert(is_array<InputStringType>::value && extent<InputStringType>::value > 0, "'T' must be an array with at least one element!");
	return in;     // We can just let the array type decay to a pointer.
}
...
template<typename InputStringType>
inline ukk EncodedPointer(const InputStringType& in, integral_constant<EBind, eBind_NullTerminated> )
{
	static_assert(is_pointer<InputStringType>::value, "'T' must be a pointer type!");
	return in;     // Implied
}
...
template<typename InputCharType>
inline ukk EncodedPointer(const InputCharType& in, integral_constant<EBind, eBind_CodePoint> )
{
	static_assert(is_arithmetic<InputCharType>::value, "'T' must be an arithmetic type!");
...
struct SWriteSink<T, Append, eBind_Iterators>
{
	...
	void operator()(ukk , size_t);             //!< Not implemented.
	void HintSequence(u32 length) {}               //!< Don't care about sequences.
...
struct SWriteSink<T, Append, eBind_Data>
{
	...
	void operator()(ukk src, size_t length)
	{
