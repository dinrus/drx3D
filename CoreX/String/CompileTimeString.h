...
// Compile time string can be used in cases where the compiler requires a fixed memory location for tukk at compile time.
// e.g. when passing tukk as a template parameter which then uses it in constexpr: E.g. class Foo : public Bar<"MyString"> { .... }; template <tukk szStr> class Bar { constexpr GetStr() { return szStr; }
