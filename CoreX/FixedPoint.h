...
template<>
struct overflow_type<i32>
{
	typedef int64 type;
};

template<>
struct overflow_type<u32>
{
	typedef uint64 type;
};
...
{
	typedef unsigned short type;
	enum { bit_size = sizeof(type) * 8, };
};

template<>
struct unsigned_overflow_type<u8>
{
	typedef unsigned short type;
	enum { bit_size = sizeof(type) * 8, };
};
...
template<>
struct unsigned_overflow_type<i32>
{
	typedef uint64 type;
	enum { bit_size = sizeof(type) * 8, };
};
...
template<>
struct unsigned_overflow_type<u32>
{
	typedef uint64 type;
	enum { bit_size = sizeof(type) * 8, };
};
...
#define DEFAULT_ROUNDING_POLICY DrxFixedPoint::eRM_Truncate
const static DrxFixedPoint::SelectorRoundingMode<DEFAULT_ROUNDING_POLICY> kDefaultRoundingPolicy;
...
struct fixed_t
{
	...
	typedef typename DrxFixedPoint::overflow_type<value_type>::type          overflow_type;
	typedef typename DrxFixedPoint::unsigned_overflow_type<value_type>::type unsigned_overflow_type;
	...
	enum { half_unit = integer_scale >> 1, };
	enum { fractional_bitmask = integer_scale - 1, };
	...
	inline fixed_t(i32k& value)
		: v(value << fractional_bitcount)
	{
	}

	inline fixed_t(u32k& value)
		: v(value << fractional_bitcount)
	{
	}
	...
	inline fixed_t& operator/=(const fixed_t& other)
	{
		DRX_ASSERT_MESSAGE(other.v != 0, "ERROR: Divide by ZERO!");
	...
	inline i32 as_int() const
	{
		return v >> fractional_bitcount;
	}

	inline u32 as_uint() const
	{
		return v >> fractional_bitcount;
	}
	...
	inline static fixed_t fraction(i32 num, i32 denom)
	{
		fixed_t v;
	...
	inline static i32 as_int(const overflow_type& x)
	{
		return x >> fractional_bitcount;
	}

	inline static u32 as_uint(const overflow_type& x)
	{
		return x >> fractional_bitcount;
	}
	...
	inline static fixed_t sqrtf(const unsigned_overflow_type& x)
	{
		...
		unsigned_overflow_type count = (DrxFixedPoint::unsigned_overflow_type<value_type>::bit_size >> 1) + (fractional_bitcount >> 1) - ((fractional_bitcount + 1) & 1);

		do
		{
			remHi = (remHi << 2) | (remLo >> (DrxFixedPoint::unsigned_overflow_type<value_type>::bit_size - 2));
			remLo <<= 2;
			root <<= 1;
...
protected:
	...
	inline value_type rounded_multiplication(const value_type& a, const value_type& b, const DrxFixedPoint::SelectorRoundingMode<DrxFixedPoint::eRM_None>&)
	{
		...
		CheckIfMultiplicationIsCorrect(a, b, expectedResult, DrxFixedPoint::eRM_None);
#endif

		return product;
	}

	inline value_type rounded_multiplication(const value_type& a, const value_type& b, const DrxFixedPoint::SelectorRoundingMode<DrxFixedPoint::eRM_Nearest>&)
	{
		...
		CheckIfMultiplicationIsCorrect(a, b, expectedResult, DrxFixedPoint::eRM_Nearest);
#endif

		return static_cast<value_type>(product);
	}

	inline value_type rounded_multiplication(const value_type& a, const value_type& b, const DrxFixedPoint::SelectorRoundingMode<DrxFixedPoint::eRM_Truncate>&)
	{
		overflow_type product = overflow_type(a) * b;
		...
		fixed_t expectedResult;
		expectedResult.set(static_cast<value_type>(product));
		CheckIfMultiplicationIsCorrect(a, b, expectedResult, DrxFixedPoint::eRM_Truncate);
#endif
	...
	inline value_type general_rounded_division(const value_type& a, const value_type& b, const DrxFixedPoint::ERoundingModes roundingMode)
	{
		DRX_ASSERT_MESSAGE(b != 0, "ERROR: Divide by ZERO!");
	...
	inline value_type rounded_division(const value_type& a, const value_type& b, const DrxFixedPoint::SelectorRoundingMode<DrxFixedPoint::eRM_None>&)
	{
		return general_rounded_division(a, b, DrxFixedPoint::eRM_None);
	}

	inline value_type rounded_division(const value_type& a, const value_type& b, const DrxFixedPoint::SelectorRoundingMode<DrxFixedPoint::eRM_Truncate>&)
	{
		return general_rounded_division(a, b, DrxFixedPoint::eRM_Truncate);
	}

	inline value_type rounded_division(const value_type& a, const value_type& b, const DrxFixedPoint::SelectorRoundingMode<DrxFixedPoint::eRM_Nearest>&)
	{
		DRX_ASSERT_MESSAGE(b != 0, "ERROR: Divide by ZERO!");
		...
		i32k nRoundingShift = 1;
		i32k nShiftBits = fractional_bitcount + nRoundingShift;
		...
		fixed_t expectedResult;
		expectedResult.set(static_cast<value_type>(quotient));
		CheckIfDivisionIsCorrect(a, b, expectedResult, DrxFixedPoint::eRM_Nearest);
#endif
	...
	void CheckIfMultiplicationIsCorrect(const value_type& a, const value_type& b, const fixed_t& expectedResult, const DrxFixedPoint::ERoundingModes roundingMode)
	{
		...
		if (roundingMode == DrxFixedPoint::eRM_Nearest)
		{
			product2 += half_unit;
		}
		...
		DRX_ASSERT_TRACE(expectedResult == product_from_alt, ("ERROR: %.6f / %.6f should equal %.6f, but instead it is %.6f",
		                                                      a / (double)integer_scale, b / (double)integer_scale,
		                                                      product_from_alt.as_double(), expectedResult.as_double()));
	}

	void CheckIfDivisionIsCorrect(const value_type& dividend, const value_type& divisor, const fixed_t& expectedResult, const DrxFixedPoint::ERoundingModes roundingMode)
	{
		...
		DRX_ASSERT_TRACE(expectedResult == quotient_from_alt,
...
inline fixed_t<BaseType, IntegerBitCount> floor(const fixed_t<BaseType, IntegerBitCount>& x)
{
	...
	r.set(x.get() & ~(fixed_t<BaseType, IntegerBitCount>::fractional_bitmask));
...
inline fixed_t<BaseType, IntegerBitCount> ceil(const fixed_t<BaseType, IntegerBitCount>& x)
{
	fixed_t<BaseType, IntegerBitCount> r;
	r.set(x.get() & ~(fixed_t<BaseType, IntegerBitCount>::fractional_bitmask));
	if (x.get() & (fixed_t<BaseType, IntegerBitCount>::fractional_bitmask))
...
{
	typedef fixed_t<BaseType, IntegerBitCount>                    type;
	typedef typename DrxFixedPoint::overflow_type<BaseType>::type overflow_type;
