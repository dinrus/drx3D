...
#ifndef _DRX_ARRAY_H_
#define _DRX_ARRAY_H_
...
template<class T, class I = i32, class STORE = NArray::Storage>
...
template<class T>
ILINE Array<T> ArrayT(T* elems, i32 count)
{
...
template<class T, i32 nSIZE, class I = i32>
...
struct StaticDynStorage
{
	...
	protected:
		...
		DRX_ALIGN(alignof(T)) char m_aData[nSIZE * sizeof(T)];       // Storage for elems, deferred construction
	};
};
} // NArray
...
struct AllocArray
{
	...
	AllocArray(uk data_ = 0, size_t size_ = 0)
		: data(data_), size(size_) {}

	void adjust_front(i32 n)
	{
		if (data)
			(tuk&)data += n;
		if (size)
...
template<class A, class Prefix, i32 nSizeAlign = 1>
struct AllocPrefix : A
{
	AllocArray alloc(AllocArray a, size_t nSize, size_t nAlign = 1, bool bSlack = false)
	{
		...
		i32 nPrefixSize = check_cast<i32>(Align(sizeof(Prefix), nAlign));
...
//! Standard DrxModule memory allocation, using aligned versions.
struct ModuleAlloc
{
	static AllocArray alloc(AllocArray a, size_t nSize, size_t nAlign = 1, bool bSlack = false)
	{
		if (nSize)
		{
			if (nSize != a.size)
			{
				...
				a.data = DrxModuleMemalign(nSize, nAlign);
				a.size = nSize;
			}
		}
		else if (a.data)
...
template<i32 nSIZE, class A = NAlloc::StandardAlloc>
...
template<class T, class I = i32, class A = NAlloc::StandardAlloc>
using FastDynArray = DynArray<T, I, NArray::FastDynStorage<A>>;

template<class T, class I = i32, class A = NAlloc::StandardAlloc>
using SmallDynArray = DynArray<T, I, NArray::SmallDynStorage<A>>;

template<class T, i32 nSIZE, class I = i32, class A = NAlloc::StandardAlloc>
