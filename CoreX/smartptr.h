...
public:
	...
	{
		DRX_ASSERT_MESSAGE(!p, "Assign_NoAddRef should only be used on a default-constructed, not-yet-assigned smart_ptr instance");
		p = ptr;
	}
...
template<typename TDerived, typename Counter = i32> class _reference_target_no_vtable
{
public:
...
//! Default implementation is i32 counter - for better alignment.
typedef _reference_target<i32> _reference_target_t;

//! Reference target for smart pointer with configurable destruct function.
//! Implements AddRef() and Release() strategy using reference counter of the specified type.
template<typename T, typename Counter = i32> class _cfg_reference_target
{
public:
	using DeleteFncPtr = void(*)(uk );
...
public:
	...
	i32 UseCount() const
	{
		DrxFatalError("Do not use this function as it is not thread-safe.The return value of UseCount() may be wrong on return already.");
		return -1;
	}
...
private:
	mutable  i32 m_cnt; // Private: Discourage inheriting classes to observe m_nRefCounter and act on its value which is not thread safe.
};
...
typedef _i_reference_target<i32>             _i_reference_target_t;
typedef _i_multithread_reference_target<i32> _i_multithread_reference_target_t;
