...
i32 find_index(const C& container, const V& value)
{
	auto it = std::find(container.begin(), container.end(), value);
	return it == container.end() ? -1 : i32(it - container.begin());
}

template<typename C, typename P>
i32 find_index_if(const C& container, const P& pred)
{
...
//! Convert arbitrary class to tukk .
template<class Type>
inline tukk constchar_cast(const Type& type)
{
	return type;
}
...
template<>
inline tukk constchar_cast(const STxt& type)
{
	return type.c_str();
}

//! Case sensetive less key for any type convertable to tukk .
template<class Type>
struct less_strcmp
{
...
//! Case insensetive less key for any type convertable to tukk .
template<class Type>
struct less_stricmp
{
	bool operator()(const Type& left, const Type& right) const
...
// Unordered_map usage:
// typedef std::unordered_map<string, i32, stl::hash_stricmp<string>, stl::hash_stricmp<string> > StringToIntHash;

//! Simple u32 hash that has the avalanche property (a change in one bit affects all others).
class hash_uint32
{
public:
	ILINE size_t operator()(u32 a) const
	{
		a = (a + 0x7ed55d16) + (a << 12);
		a = (a ^ 0xc761c23c) ^ (a >> 19);
		a = (a + 0x165667b1) + (a << 5);
...
struct SAllocatorConstruct
{
	...
	{
		::new(const_cast<uk>(static_cast<ukk>(ptr)))T(std::forward<Args>(args) ...);
	}
#else
	...
	{
		::new(static_cast<uk>(ptr))T(std::forward<Arg1>(arg1));
	}
	...
	{
		::new(static_cast<uk>(ptr))T(std::forward<Arg1>(arg1), std::forward<Arg2>(arg2));
	}
	...
	{
		::new(static_cast<uk>(ptr))T(std::forward<Arg1>(arg1), std::forward<Arg2>(arg2), std::forward<Arg3>(arg3));
	}
