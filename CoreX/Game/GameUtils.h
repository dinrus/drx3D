...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Implements some utilities.
...
inline time_t stringToTime(tukk str)
{
	if (str)
	{
		struct tm timePtr;
		// convert the time to from a string to a time struct
		i32 n = sscanf(str, "%i-%02d-%02dT%02d:%02d:%02d",
...
inline tukk GetPrettyAsyncTime()
{
	...
	seconds -= minutes * 60;
	minutes -= hours * 60;
	static DrxFixedStringT<16> sTimeString;
	sTimeString.Format("%02d:%02d:%02d.%03d", (i32)hours, (i32)minutes, (i32)seconds, (i32)milliseconds);
...
template<class T, class U>
T GetKeyByValue(tukk value, TKeyValuePair<T, U> list[], size_t list_size)
{
...
template<class T, class U>
U GetValueByKey(tukk key, TKeyValuePair<T, U> list[], size_t list_size)
{
	return GetValueByKey(CONST_TEMP_STRING(key), list, list_size);
}

//first element of array is default value
#define KEY_BY_VALUE(v, g) GetKeyByValue(v, g, DRX_ARRAY_COUNT(g))
#define VALUE_BY_KEY(v, g) GetValueByKey(v, g, DRX_ARRAY_COUNT(g))
...
template<class T, class U>
bool HasValue(tukk value, TKeyValuePair<T, U> list[], size_t list_size)
{
	return HasValue(CONST_TEMP_STRING(value), list, list_size);
}

template<class T, class U>
bool HasKey(tukk key, TKeyValuePair<T, U> list[], size_t list_size)
{
	return HasKey(CONST_TEMP_STRING(key), list, list_size);
}

#define HAS_VALUE(v, g) HasValue(v, g, DRX_ARRAY_COUNT(g))
#define HAS_KEY(v, g)   HasKey(v, g, DRX_ARRAY_COUNT(g))
...
struct SChangeTrack
{
	...
	inline bool CompareAndSet(string& old_value, tukk new_value)
	{
		if (old_value != new_value)
		{
			old_value = new_value;
...
struct SStringPool
{
	inline tukk AddString(tukk str)
	{
