...
struct IGameToken
{
	...
	virtual void SetValueFromString(tukk sValue) = 0;

	//! Get the token value as a string
	virtual tukk GetValueAsString() const = 0;
...
struct IGameTokenSystem
{
	...
	//! Create a new token.
	virtual IGameToken* SetOrCreateToken(tukk sTokenName, const TFlowInputData& defaultValue) = 0;

	//! Deletes existing game token.
	virtual void DeleteToken(IGameToken* pToken) = 0;

	//! Find a game token by name.
	virtual IGameToken*   FindToken(tukk sTokenName) = 0;

	virtual IGameTokenIt* GetGameTokenIterator() = 0;

	//! Rename existing game token.
	virtual void RenameToken(IGameToken* pToken, tukk sNewTokenName) = 0;

	virtual void RegisterListener(tukk sGameToken, IGameTokenEventListener* pListener, bool bForceCreate = false, bool bImmediateCallback = false) = 0;
	virtual void UnregisterListener(tukk sGameToken, IGameTokenEventListener* pListener) = 0;
	...
	virtual void LoadLibs(tukk sFileSpec) = 0;

	//! Remove all tokens beginning with prefix (e.g. Library).
	virtual void RemoveLibrary(tukk sPrefix) = 0;

	//! Level to level serialization.
	virtual void SerializeSaveLevelToLevel(tukk* ppGameTokensList, u32 numTokensToSave) = 0;
	...
	template<typename T>
	bool GetTokenValueAs(tukk sGameToken, T& value)
	{
	...
	virtual void AddTokenToDebugList(tukk pToken) = 0;
	virtual void RemoveTokenFromDebugList(tukk pToken) = 0;
#endif
};
