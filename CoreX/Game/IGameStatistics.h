...
//! Can be used to save arbitrary data structures as statistics parameters while postponing XML construction until serialization stage.
...
struct SStatAnyValue
{
	...
	SStatAnyValue(const ScriptAnyValue& value)
	{
		...
		case EScriptAnyType::String:
			...
			break;
		default:
			DRX_ASSERT_MESSAGE(false, "Invalid type for stat value");
			type = eSAT_NONE;
		}
	}
...
struct SNodeLocator
{
	...
	u32 locatorType;    //!< Type of identification.
	u32 locatorValue;   //!< Identifier.
	u32 timeStamp;      //!< Used to identify nodes over the whole lifetime.
	...
	SNodeLocator(size_t _elemID, size_t _scopeID, u32 locType, u32 locValue = 0)
		: elemID(_elemID)
		, scopeID(_scopeID)
		, locatorType(locType)
...
struct SGameStatDesc
{
	...
	SGameStatDesc(size_t _statID, tukk _scriptName, tukk _serializeName)
...
struct SGameScopeDesc : public SGameStatDesc
{
	...
	SGameScopeDesc(size_t _statID, tukk _scriptName, tukk _serializeName, tukk _trackerName)
...
struct SGameElementDesc : public SGameScopeDesc
{
	...
	SGameElementDesc() {}
	SGameElementDesc(size_t _statID, tukk _scriptName, u32 _locatorID, tukk _locatorName,
	                 tukk _serializeName, tukk _trackerName)
...
struct IStatsSerializer
{
	...
	virtual void VisitNode(const SNodeLocator& locator, tukk serializeName, IStatsContainer& container, EStatNodeState state) = 0;
	virtual void LeaveNode(const SNodeLocator& locator, tukk serializeName, IStatsContainer& container, EStatNodeState state) = 0;
	// </interfuscator:shuffle>
};
...
struct IGameStatistics
{
	...
	//! \return false on name or id collision.
	...
	virtual size_t               RegisterGameEvent(tukk scriptName, tukk serializeName) = 0;

	virtual size_t               GetEventCount() const = 0;

	virtual size_t               GetEventID(tukk scriptName) const = 0;

	virtual size_t               GetEventIDBySerializeName(tukk serializeName) const = 0;
	...
	virtual size_t               RegisterGameState(tukk scriptName, tukk serializeName) = 0;

	virtual size_t               GetStateCount() const = 0;

	virtual size_t               GetStateID(tukk scriptName) const = 0;
	...
	//! \return Scope ID of the scope on the specified depth (0 - top of the stack).
	virtual size_t                GetScopeID(size_t depth = 0) const = 0;

	virtual size_t                GetScopeCount() const = 0;

	virtual size_t                GetScopeID(tukk scriptName) const = 0;
	...
	virtual XmlNodeRef CreateStatXMLNode(tukk tag = "root") = 0;

	//! Wraps XML node to serializable object.
	virtual IXMLSerializable* WrapXMLNode(const XmlNodeRef& node) = 0;

	//! This method (and similar ones in callback) are used to convert parameter data passed from script to another (possibly class inherited from IXMLSerializable) type to simplify the callback logic.
	virtual void PreprocessScriptedEventParameter(size_t eventID, SStatAnyValue& value) = 0;
	virtual void PreprocessScriptedStateParameter(size_t stateID, SStatAnyValue& value) = 0;
