...
#define DRX_INCLUDE_WINDOWS_VIA_MFC_OR_ATL_INCLUDES

// For identical behavior as DrxWindows.h
...
#if defined(DRX_USE_MFC) || defined(DRX_USE_XT)
...
// Request MFC assert macro's to be remapped to DRX_ASSERT
		#define DRX_REMAP_MFC_ASSERT
	#endif
#endif

#if defined(DRX_USE_ATL)
...
// Undefine function-names used in DinrusX interfaces.
// This does the same as DrxWindows.h.
...
// It's possible that neither DRX_MFC, DRX_ATL or DRX_XT is set, in which case we would miss that.
#include <drx3D/CoreX/Platform/DrxWindows.h>
#undef _ATL_CSTRING_EXPLICIT_CONSTRUCTORS
#undef DRX_SUPPRESS_DRXENGINE_WINDOWS_FUNCTION_RENAMING
...
// We should restore DRX_ASSERT behavior.
#include <drx3D/CoreX/Assert/DrxAssert.h>

// Replacement for MFC's ASSERT, ASSERT_KINDOF and ASSERT_VALID macro's
#if defined(DRX_REMAP_MFC_ASSERT)
	#undef ASSERT
	#undef ASSERT_VALID
	#undef ASSERT_KINDOF
	#define ASSERT(x)           DRX_ASSERT_MESSAGE(x, "MFC ASSERT")
	#define ASSERT_KINDOF(t, p) DRX_ASSERT_MESSAGE(p && p->IsKindOf(RUNTIME_CLASS(t)), "MFC ASSERT_KINDOF")
	#ifdef _DEBUG
		#define ASSERT_VALID(x)   DRX_ASSERT_MESSAGE(x && (x->AssertValid(), true), "MFC ASSERT_VALID")
	#else
		#define ASSERT_VALID(x)   DRX_ASSERT_MESSAGE(x, "MFC ASSERT_VALID")
	#endif
	#undef DRX_REMAP_MFC_ASSERT
#endif
