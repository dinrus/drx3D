...
// -------------------------------------------------------------------------
//  Имя файла:   WinBase.cpp
//  Version:     v1.00
//  Created:     10/3/2004 by Michael Glueck
//  Компиляторы:       drux (DinrusPro's Universal Compiler),   GCC
//  Описание: Linux/Mac port support for Win32API calls
// -------------------------------------------------------------------------
//  History:
//
////////////////////////////////////////////////////////////////////////////

#if !defined(DRX_PLATFORM_IMPL_H_FILE)
...
#include <errno.h>
	...
	#include <drx3D/DrxSystem/SystemUtilsApple.h>
#endif
...
#define INCLUDED_FROM_DRX_WINBASE_IMPL
	...
	#include <drx3D/CoreX/Platform/WinBase_sce.inl>
#elif DRX_PLATFORM_POSIX
	#include <drx3D/CoreX/Platform/WinBase_posix.inl>
#endif
...
const char* const ConvertFileName(char* const pBuf, const size_t bufSize, const char* const cpName)
{
	if (cpName[0] != '/')
	{
		...
		drx_strcpy(pBuf, bufSize, gEnv->pSystem->GetRootFolder());
		drx_strcat(pBuf, bufSize, cpName);
		return pBuf;
	}
...
const char* const ConvertFileName(char* const pBuf, const size_t bufSize, const char* const cpName)
{
	...
	drx_strcpy(pBuf, bufSize, cpName);
...
void _splitpath(const char* inpath, char* drv, char* dir, char* fname, char* ext)
{
	...
	typedef DrxStackStringT<char, 256> path_stack_string;
...
DWORD GetFileAttributes(LPCSTR lpFileName)
{
	...
	if (success == -1)
	{
		...
		drx_strcpy(adjustedFilename, lpFileName);
...
void __finddata64_t::CopyFoundData(const char* rMatchedFileName)
{
	...
	drx_strcpy(name, rMatchedFileName);
	...
	*pDst = 0;
	...
	if (!isDir)
	{
		DrxFileSystem::FS_STAT(filename, fsStat, fsErr);
		if (fsErr) return;
	}
	(void)fsErr;
...
int FixDirnameCase(char* path, int index = 0)
{
	...
	*slash = 0;
	...
	DrxFileSystem::FS_OPENDIR(path, dir, fsErr);
	if (!fsErr)
	{
		pathOk = true;
		DrxFileSystem::FS_CLOSEDIR_NOERR(dir);
		dir = FS_DIR_NULL;
	}
	...
	if (!pathOk)
	{
		...
		// Scan parent.
		DrxFileSystem::FS_OPENDIR(parent, dir, fsErr);
		if (fsErr)
		{
		...
		while (true)
		{
			DrxFileSystem::FS_READDIR(dir, dirent, direntSize, fsErr);
			if (fsErr)
			{
				DrxFileSystem::FS_CLOSEDIR_NOERR(dir);
				if (parentSlash) *parentSlash = '/';
		...
		DrxFileSystem::FS_CLOSEDIR(dir, fsErr);
		if (parentSlash) *parentSlash = '/';
		if (fsErr)
		{
			*slash = '/';
...
intptr_t _findfirst64(const char* pFileName, __finddata64_t* pFindData)
{
	...
	drx_strcpy(pFindData->m_ToMatch, pattern);

	// Close old directory descriptor.
	if (pFindData->m_Dir != FS_DIR_NULL)
	{
		DrxFileSystem::FS_CLOSEDIR(pFindData->m_Dir, fsErr);
...

	{
		...
		DrxFileSystem::FS_OPENDIR(dirname, pFindData->m_Dir, fsErr);

		if (fsErr)
			return -1;
		drx_strcpy(pFindData->m_DirectoryName, dirname);
		FS_DIRENT_TYPE dirent;
		uint64_t direntSize = 0;
		while (true)
		{
			DrxFileSystem::FS_READDIR(pFindData->m_Dir, dirent, direntSize, fsErr);
			if (fsErr)
			{
				DrxFileSystem::FS_CLOSEDIR_NOERR(pFindData->m_Dir);
				pFindData->m_Dir = FS_DIR_NULL;
				return -1;
			}
			if (direntSize == 0)
			...
			drx_strcpy(d_name, dirent.d_name);
...
__finddata64_t::~__finddata64_t()
{
	...
	{
		DrxFileSystem::FS_CLOSEDIR_NOERR(m_Dir);
		m_Dir = FS_DIR_NULL;
	}
}
...
const bool ConvertFilenameNoCase(char* szFilepathToAdj)
{
	...
	#if !DRX_PLATFORM_LINUX && !DRX_PLATFORM_ANDROID && !DRX_PLATFORM_APPLE // fix the parent path anyhow.
...
#else
		DrxFileSystem::FS_OPEN(adjustedFilename, flags, fd, 0, fserr);
#endif
	}
...
/*
   ...
   {
   DRX_ASSERT_MESSAGE(0, "SetFileTime not implemented yet");
   return FALSE;
   }
 */
...
const uint64 GetFileModifTime(FILE* hFile)
{
	FS_ERRNO_TYPE fsErr = 0;
	FS_STAT_TYPE st;
	DrxFileSystem::FS_FSTAT(fileno(hFile), st, fsErr);
...
DWORD GetFileSize(HANDLE hFile, DWORD* lpFileSizeHigh)
{
	...
	DrxFileSystem::FS_FSTAT(fd, st, err);
	if (err != 0)
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "CancelIo not implemented yet");
	return TRUE;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "GetOverlappedResult not implemented yet");
	return 0;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "ReadFile not implemented yet");
	abort();
	return TRUE;
}
...
{
	DRX_ASSERT_MESSAGE(0, "ReadFileEx not implemented yet");
	return TRUE;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "SetFilePointer not implemented yet");
	return 0;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "CreateEvent not implemented yet");
	return 0;
}
...
{
	//TODO: implement
	//	DRX_ASSERT_MESSAGE(0, "SleepEx not implemented yet");
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "WaitForSingleObjectEx not implemented yet");
	return 0;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "WaitForSingleObject not implemented yet");
	return 0;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "SetEvent not implemented yet");
	return TRUE;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "ResetEvent not implemented yet");
	return TRUE;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "CreateMutex not implemented yet");
	return 0;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "ReleaseMutex not implemented yet");
	return TRUE;
}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "CreateThread not implemented yet");
	return 0;
}
...
BOOL SetCurrentDirectory(LPCSTR lpPathName)
{
	...
	while (*p && *p != '/')
		++p;
	drx_strcpy(dir, fopenwrapper_basedir);
	...
	drx_strcpy(fopenwrapper_basedir, fopenwrapper_basedir_maxsize, dir);
	return TRUE;
}
#endif //0
...
BOOL DeleteFile(LPCSTR lpFileName)
{
	...
	return (0 == err);
#else
	DRX_ASSERT_MESSAGE(0, "DeleteFile not implemented yet");
	return TRUE;
#endif
}
...
BOOL MoveFile(LPCSTR lpExistingFileName, LPCSTR lpNewFileName)
{
	...
	return (0 == err);
#else
	DRX_ASSERT_MESSAGE(0, "MoveFile not implemented yet");
#endif
	return TRUE;
}
...
#else
	DRX_ASSERT_MESSAGE(0, "CopyFile not implemented yet");
	return FALSE;
#endif
}
...
BOOL GetComputerName(LPSTR lpBuffer, LPDWORD lpnSize)
{
	...
	if (-1 == err)
	{
		DrxLog("GetComputerName falied [%d]\n", errno);
		return FALSE;
	}
	return TRUE;
}
...
{
	DRX_ASSERT_MESSAGE(0, "RemoveDirectory not implemented yet");
	return TRUE;
}
...
bool DrxCreateDirectory(const char* lpPathName)
{
	...
	if (stat(lpPathName, &st_info) == 0)
	{
		...
		{
			DrxLog("%s failed: '%s' already exists and is not a directory", __FUNC__, lpPathName);
			return false;
		}
		return true;
	}

	const int res = DrxFileSystem::FS_MKDIR(lpPathName, S_IRWXU);

	if (res != 0)
	{
		DrxLog("%s: mkdir('%s') failed", __FUNC__, lpPathName);
		return false;
	}
...
int DrxGetWritableDirectory(unsigned int nBufferLength, char* lpBuffer)
{
	...
	const char* path = DrxGetUserStoragePath();
...
void DrxSetCurrentWorkingDirectory(const char* szWorkingDirectory)
{
	...
	{
		DrxFatalError("Unexpected error while trying to set current working directory to: %s", szWorkingDirectory);
	}
}
...
void DrxGetExecutableFolder(unsigned int pathSize, char* szPath)
{
	...
	if (nLen <= 0)
	{
		DrxFatalError("Unexpected error encountered trying to get executable path. readlink(\"/proc/self/exe\") failed.");
	}
...
{
	//TODO: implement
	DRX_ASSERT_MESSAGE(0, "DrxGetAsyncKeyState not implemented yet");
	return 0;
}
...
u32 DrxGetFileAttributes(const char* lpFileName)
{
	...
	FS_ERRNO_TYPE fsErr = 0;
	int fd = -1;
	DrxFileSystem::FS_OPEN(buffer, FS_O_RDONLY, fd, 0, fsErr);
	if (!fsErr)
	{
		DrxFileSystem::FS_CLOSE_NOERR(fd);
...
{
	//TODO: implement
	printf("DrxSetFileAttributes not properly implemented yet\n");
	return false;
}
...
private:
	...
	{
		u32 nCachePos = GetCachePos(pHandle);
	...
	struct SCacheEntry
	{
		...
		u32      m_lru;            //lru counter last set op
		...
		u32                        m_nSeekPos;           // store seek position, in case files needs to be reopened
	...
	u32       GetCachePos(FILE* pHandle) const      { return (u32)((TRUNCATE_PTR)pHandle) & ~nFILEHANDLESALT; }
	FILE*        GetFileHandle(u32 nCachePos) const { return (FILE*)(EXPAND_PTR)((u32)nCachePos | nFILEHANDLESALT); }

	FILE*        GetRealFileHandle(FILE* pHandle);

	inline void  CloseFile(u32 nCachePos);
	inline FILE* OpenFile(u32 nCachePos, const char* sName, const char* op, FileIoWrapper::FileAccessType type);
	inline void  ReOpenFile(u32 nCachePos);

	void         ActivateEntry(u32 nCachePos);
	void         FreeUpFD(u32 nCachePosKeep);
	...
	u32             m_nFileCount;        // number of open files
	FDMap              m_FdToFileMap;       // map to find FILE* from fd used by cellfsio
	DrxCriticalSection m_Lock;              // Lock to protected the cache against parallel access
	DrxCriticalSection m_mapLock;           // Lock to protect the FILE*->fd map from parallel access
...
{
	AUTO_LOCK_T(DrxCriticalSection, m_Lock);
	...
	for (u32 nCachePos = 0; nCachePos < nFILEENTRIES; ++nCachePos)
	{
		SCacheEntry& rEntry = m_Cache[nCachePos];
	...
	u32 nOpenSlot = FindOpenSlot();
	return OpenFile(nOpenSlot, name, mode, type);
}

u32 CFileHandleCache::FindOpenSlot()
{
	u32 curLowestCachedSlot = 0xFFFFFFFF;    //invalid slot id
	u32 curLowestCachedSlotLRU = 0xFFFFFFFF;
	u32 curLowestClosedSlot = 0xFFFFFFFF;    // invalid slot id
	u32 curLowestClosedSlotLRU = 0xFFFFFFFF;
...
int CFileHandleCache::FClose(FILE* pHandle)
{
	...
	case SCacheEntry::Open:
		...
		{
			DrxFileSystem::fflush(rEntry.m_pHandle);
		}
		else
		{
...
size_t CFileHandleCache::FRead(void* ptr, size_t size, size_t count, FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset errno
	return pRealHandle ? DrxFileSystem::fread(ptr, size, count, pRealHandle) : 0;
}
...
size_t CFileHandleCache::FWrite(const void* ptr, size_t size, size_t count, FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset errno
	return pRealHandle ? DrxFileSystem::fwrite(ptr, size, count, pRealHandle) : 0;
}
...
size_t CFileHandleCache::FTell(FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset errno
	return pRealHandle ? DrxFileSystem::ftell(pRealHandle) : (size_t)(-1L);
}
...
size_t CFileHandleCache::FSeek(FILE* pHandle, size_t offset, size_t origin)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset errno
	return pRealHandle ? DrxFileSystem::fseek(pRealHandle, offset, origin) : (size_t)(-1L);
}
...
int CFileHandleCache::VFprintf(FILE* pHandle, const char* format, va_list arg)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset errno
	return pRealHandle ? DrxFileSystem::vfprintf(pRealHandle, format, arg) : -1;
...
int CFileHandleCache::VFscanf(FILE* pHandle, const char* format, va_list arg)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset ernno
	return pRealHandle ? DrxFileSystem::vfscanf(pRealHandle, format, arg) : -1;
}
...
int CFileHandleCache::Fgetc(FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset ernno
	return pRealHandle ? DrxFileSystem::fgetc(pRealHandle, format, arg) : -1;
}
...
char* CFileHandleCache::FGets(char* str, int num, FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset ernno
	pRealHandle ? DrxFileSystem::fgets(str, num, pRealHandle) : NULL;
}
...
int CFileHandleCache::Feof(FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset ernno
	return pRealHandle ? DrxFileSystem::feof(pRealHandle) : 0;
}
...
int CFileHandleCache::Ferror(FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset ernno
	return pRealHandle ? DrxFileSystem::ferror(pRealHandle) : 0;
}
...
int CFileHandleCache::Getc(FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset ernno
	return pRealHandle ? DrxFileSystem::getc(pRealHandle) : EOF;
}
...
int CFileHandleCache::Ungetc(int character, FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset ernno
	return pRealHandle ? DrxFileSystem::ungetc(character, pRealHandle) : EOF;
}
...
int CFileHandleCache::Fputs(const char* str, FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset ernno
	return pRealHandle ? DrxFileSystem::fputs(str, pRealHandle) : EOF;
}
...
int CFileHandleCache::Fflush(FILE* pHandle)
{
	FILE* pRealHandle = GetRealFileHandle(pHandle);
	errno = 0; // reset ernno
	return pRealHandle ? DrxFileSystem::fflush(pRealHandle) : EOF;
}
...
{
	AUTO_LOCK_T(DrxCriticalSection, m_mapLock);
...
void CFileHandleCache::ActivateEntry(u32 nCachePos)
{
	...
	case SCacheEntry::Cached:
		DrxFileSystem::fseek(rEntry.m_pHandle, 0, SEEK_SET);
...
void CFileHandleCache::FreeUpFD(u32 nCachePosKeep)
{

	u32 curLowestCachedSlot = 0xFFFFFFFF;    //invalid slot id
	u32 curLowestCachedSlotLRU = 0xFFFFFFFF;
	u32 curLowestOpenSlot = 0xFFFFFFFF;    // invalid slot id
	u32 curLowestOpenSlotLRU = 0xFFFFFFFF;
...
void CFileHandleCache::CloseFile(u32 nCachePos)
{
	SCacheEntry& rEntry = m_Cache[nCachePos];
	...
	{
	case SCacheEntry::Cached:
		DrxFileSystem::fclose(rEntry.m_pHandle);
		...
		break;
	case SCacheEntry::Open:
		DrxFileSystem::ftell(rEntry.m_pHandle);
		DrxFileSystem::fclose(rEntry.m_pHandle);
		rEntry.m_pHandle = NULL;
...
FILE* CFileHandleCache::OpenFile(u32 nCachePos, const char* sName, const char* op, FileIoWrapper::FileAccessType type)
{
	SCacheEntry& rEntry = m_Cache[nCachePos];
	...
	rEntry.m_pHandle = DrxFileSystem::fopen(sName, op);
	...
	rEntry.m_state = SCacheEntry::Open;
	drx_strcpy(rEntry.m_filename, sName);
	...
	reopenOp.replace("w", "r+");
	drx_strcpy(rEntry.m_mode, reopenOp.c_str());
...
void CFileHandleCache::ReOpenFile(u32 nCachePos)
{
	SCacheEntry& rEntry = m_Cache[nCachePos];
	DrxStackStringT<char, sizeof(rEntry.m_mode)> mode = rEntry.m_mode;
	rEntry.m_pHandle = DrxFileSystem::fopen(rEntry.m_filename, mode.c_str());
	if (errno == EMFILE)
	{
		gEnv->pLog->LogError("[FileHandleCache] Opened more than 31 files on gamedata");
		DumpCacheContent();
	...
	DrxFileSystem::fseek(rEntry.m_pHandle, rEntry.m_nSeekPos, SEEK_SET)
	rEntry.m_state = SCacheEntry::Open;
	rEntry.m_nSeekPos = 0;
	rEntry.m_bDiscIsEjected = false;
	m_nFileCount += 1;
...
int WrappedStat(const char* pFilename, struct stat* pStat)
{
	...
	const char* pTrueFilename = pFilename;
		#endif
	return DrxFileSystem::FS_STAT(pTrueFilename, pStat)
}

int WrappedFStat(int files, struct stat* pStat)
{
	return DrxFileSystem::FS_STAT(files, pStat);
}
...
#if (DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID || DRX_PLATFORM_APPLE) && !defined(OutputDebugString)
