	...
	: INumberArray<vector4_t<F>, 3>
{
	...
	Matrix34H& Invert(const Matrix34H& m)
	{
		...
		F det = drxmath::hsum(row0 * minor0);
		F rdet = F(1) / det;
	...
	// Data access
	ILINE const Vec4& operator[](i32 i) const            { DRX_MATH_ASSERT((uint)i < 3); return (&x)[i]; }
	ILINE Vec4& operator[](i32 i)                        { DRX_MATH_ASSERT((uint)i < 3); return (&x)[i]; }

	ILINE F           operator()(i32 i, i32 j) const     { return (*this)[i][j]; }
	ILINE F&          operator()(i32 i, i32 j)           { return (*this)[i][j]; }

	F*                GetData()                          { return &m00; }
	const F*          GetData() const                    { return &m00; }

	ILINE void        SetRow(i32 i, const Vec3& v)       { (Vec3&)(*this)[i] = v; }
	ILINE void        SetRow4(i32 i, const Vec4& v)      { (*this)[i] = v; }

	ILINE const Vec3& GetRow(i32 i) const                { return (const Vec3&)(*this)[i]; }
	ILINE const Vec4& GetRow4(i32 i) const               { return (*this)[i]; }

	ILINE static Vec4  w()                               { return convert<V>(0,0,0,1); }

	ILINE void        SetColumn(i32 i, const Vec3& v)    { x[i] = v.x; y[i] = v.y; z[i] = v.z; }
	ILINE Vec3        GetColumn(i32 i) const             { return Vec3(x[i], y[i], z[i]); }
	...
	// Operations
	ILINE i32 IsOrthonormal(F threshold = 0.001) const   { return mat().IsOrthonormal(threshold); }
	ILINE i32 IsOrthonormalRH(F threshold = 0.001) const { return mat().IsOrthonormalRH(threshold); }
