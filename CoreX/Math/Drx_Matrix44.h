	...
	: INumberArray<F, 16>
{
	...
	{
		DRX_MATH_ASSERT(mm0.IsValid());
		DRX_MATH_ASSERT(mm1.IsValid());
		Matrix44_tpl<F> r;
	...
	ILINE F                  operator()(u32 i, u32 j) const    { DRX_MATH_ASSERT((i < 4) && (j < 4)); F* p_data = (F*)(&m00); return p_data[i * 4 + j]; }
	ILINE F&                 operator()(u32 i, u32 j)          { DRX_MATH_ASSERT((i < 4) && (j < 4)); F* p_data = (F*)(&m00); return p_data[i * 4 + j]; }

	ILINE void               SetRow(i32 i, const Vec3_tpl<F>& v)     { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); p[0 + 4 * i] = v.x; p[1 + 4 * i] = v.y; p[2 + 4 * i] = v.z; }
	ILINE void               SetRow4(i32 i, const Vec4_tpl<F>& v)    { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); p[0 + 4 * i] = v.x; p[1 + 4 * i] = v.y; p[2 + 4 * i] = v.z; p[3 + 4 * i] = v.w; }
	ILINE const Vec3_tpl<F>& GetRow(i32 i) const                     { DRX_MATH_ASSERT(i < 4); return *(const Vec3_tpl<F>*)(&m00 + 4 * i); }
	ILINE const Vec4_tpl<F>& GetRow4(i32 i) const                    { DRX_MATH_ASSERT(i < 4); return *(const Vec4_tpl<F>*)(&m00 + 4 * i); }

	ILINE void               SetColumn(i32 i, const Vec3_tpl<F>& v)  { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); p[i + 4 * 0] = v.x; p[i + 4 * 1] = v.y; p[i + 4 * 2] = v.z; }
	ILINE void               SetColumn4(i32 i, const Vec4_tpl<F>& v) { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); p[i + 4 * 0] = v.x; p[i + 4 * 1] = v.y; p[i + 4 * 2] = v.z; p[i + 4 * 3] = v.w; }
	ILINE Vec3_tpl<F>        GetColumn(i32 i) const                  { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); return Vec3_tpl<F>(p[i + 4 * 0], p[i + 4 * 1], p[i + 4 * 2]); }
	ILINE Vec4_tpl<F>        GetColumn4(i32 i) const                 { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); return Vec4_tpl<F>(p[i + 4 * 0], p[i + 4 * 1], p[i + 4 * 2], p[i + 4 * 3]); }
...
typedef DRX_ALIGN (16) Matrix44_tpl<f32> Matrix44A;
