...
template<typename F> struct Vec4H
{
	...
	explicit ILINE Vec4H(F s)                       { v = convert<V>(s); DRX_MATH_ASSERT(IsValid()); }
	ILINE Vec4H(F x_, F y_, F z_, F w_)             { v = convert<V>(x_, y_, z_, w_); DRX_MATH_ASSERT(IsValid()); }
	ILINE Vec4H& operator()(F x_, F y_, F z_, F w_) { v = convert<V>(x_, y_, z_, w_); DRX_MATH_ASSERT(IsValid()); return *this; }
	...
	// Operators
	ILINE F&   operator[](i32 index)            { DRX_MATH_ASSERT(index >= 0 && index <= 3); return ((F*)this)[index]; }
	ILINE F    operator[](i32 index) const      { DRX_MATH_ASSERT(index >= 0 && index <= 3); return ((F*)this)[index]; }
	...
	ILINE F      operator|(Vec4H b) const       { return drxmath::hsum(v * b.v); }
	...
	ILINE F    GetLengthSquared() const    { return drxmath::hsum(v * v); };
	ILINE F    GetLength() const           { return drxmath::sqrt(GetLengthSquared()); }
	ILINE F    GetLengthFast() const       { return drxmath::sqrt_fast(GetLengthSquared()); }
	ILINE F    GetInvLength() const        { return drxmath::rsqrt(GetLengthSquared()); }
	ILINE F    GetInvLengthFast() const    { return drxmath::rsqrt_fast(GetLengthSquared()); }
	ILINE Vec4H GetNormalized() const      { return v * drxmath::rsqrt(drxmath::hsumv(v * v));	}
	ILINE Vec4H GetNormalizedFast() const  { return v * drxmath::rsqrt_fast(drxmath::hsumv(v * v));	}

	ILINE void Normalize()                 { v *= drxmath::rsqrt(drxmath::hsumv(v * v)); }
	ILINE void NormalizeFast()             { v *= drxmath::rsqrt_fast(drxmath::hsumv(v * v)); }

	ILINE Vec4H ProjectionOn(Vec4H b) const
	{
		return b.v * drxmath::hsumv(v * b.v) * drxmath::rcp(drxmath::hsumv(b.v * b.v));
	}
	...
	ILINE Vec4H(const Vec4& v_)                               { v = convert<V>(&v_.x); DRX_MATH_ASSERT(IsValid()); }
	ILINE Vec4H(const Vec3& v_, F w_ = 0)                     { v = convert<V>(v_.x, v_.y, v_.z, w_); DRX_MATH_ASSERT(IsValid()); }
	...
	ILINE void SetLerp(Vec4H a, Vec4H b, F t)                 { *this = Lerp(a, b, t); DRX_MATH_ASSERT(IsValid()); }
...
#ifndef DRX_COMPILER_GCC // Currently has a problem with Vec3 in anonymous struct

#define DRX_HARDWARE_PLANE4
...
template<typename F> struct PlaneH
{
	...
	// Conversions
	ILINE PlaneH(V v_) { v = v_; DRX_MATH_ASSERT(IsValid()); }
	ILINE operator V() const { return v; }
	...
	ILINE                PlaneH(const Plane& pl)                                     { v = convert<V>(&pl.n.x); DRX_MATH_ASSERT(IsValid()); }
