...
template<typename V> using scalar_t  = typename SIMD_traits<V>::scalar_t;
	...
	#define DRX_TYPE_SIMD4

// Support for SSE 32x4 types

	#if DRX_COMPILER_CLANG || DRX_COMPILER_GCC
...
typedef i32  i32v4 __attribute__((__vector_size__(16)));
typedef u32 u32v4 __attribute__((__vector_size__(16)));

//! Define results of built-in comparison operators.
typedef decltype (f32v4() == f32v4()) mask32v4;

	#elif DRX_COMPILER_MSVC
...
template<> struct SIMD_traits<i32>
{
	using scalar_t  = i32;
...
template<> struct SIMD_traits<u32>
{
	using scalar_t  = u32;
...
template<> ILINE f32v4  convert<f32v4>(i32 v)           { return _mm_set1_ps(f32(v)); }
...
template<> ILINE i32v4  convert<i32v4>(i32 v)           { return _mm_set1_epi32(v); }
template<> ILINE i32v4  convert<i32v4>(f32v4 v)         { check_range(v, INT_MIN, INT_MAX); return _mm_cvttps_epi32(v); }
template<> ILINE i32v4  convert<i32v4>(i32k* p)  { return _mm_loadu_si128((const __m128i*)p); }

template<> ILINE i32  convert<i32>(i32v4 v)         { return _mm_cvtsi128_si32(v); }
...
template<> ILINE u32v4  convert<u32v4>(i32 v)           { DRX_MATH_ASSERT(v >= 0); return _mm_set1_epi32(v); }
template<> ILINE u32v4  convert<u32v4>(f32v4 v)         { check_range(v, 0, INT_MAX); return _mm_cvttps_epi32(v); }
template<> ILINE i32v4  convert<i32v4>(u32k* p) { return _mm_loadu_si128((const __m128i*)p); }

template<> ILINE u32 convert<u32>(u32v4 v)        { return _mm_cvtsi128_si32(v); }

//! convert<V>(s, 0): Load scalar into vector[0] and zero remaining elements
template<typename V, typename S> V convert(S v, uk );

template<> ILINE f32v4             convert<f32v4>(f32 v, uk )  { return _mm_set_ss(v); }
template<> ILINE i32v4             convert<i32v4>(i32 v, uk )  { return _mm_cvtsi32_si128(v); }
template<> ILINE u32v4             convert<u32v4>(uint v, uk ) { return _mm_cvtsi32_si128(v); }
...
template<> ILINE f32v4             convert<f32v4>(i32 s0, i32 s1, i32 s2, i32 s3)     { return _mm_setr_ps(f32(s0), f32(s1), f32(s2), f32(s3)); }
template<> ILINE i32v4             convert<i32v4>(i32 s0, i32 s1, i32 s2, i32 s3)     { return _mm_setr_epi32(s0, s1, s2, s3); }
template<> ILINE u32v4             convert<u32v4>(uint s0, uint s1, uint s2, uint s3) { return _mm_setr_epi32(s0, s1, s2, s3); }
template<> ILINE u32v4             convert<u32v4>(i32 s0, i32 s1, i32 s2, i32 s3)     { return _mm_setr_epi32(s0, s1, s2, s3); }
...
template<i32 x, i32 y, i32 z, i32 w> ILINE f32v4 shuffle(f32v4 in) { return _mm_shuffle_ps(in, in, _MM_SHUFFLE(w, z, y, x)); }
template<i32 x, i32 y, i32 z, i32 w> ILINE i32v4 shuffle(i32v4 in) { return _mm_shuffle_epi32(in, _MM_SHUFFLE(w, z, y, x)); }
template<i32 x, i32 y, i32 z, i32 w> ILINE u32v4 shuffle(u32v4 in) { return _mm_shuffle_epi32(in, _MM_SHUFFLE(w, z, y, x)); }
...
template<i32 i, typename V> ILINE V broadcast(V in) { return shuffle<i, i, i, i>(in); }

// Mix components from 2 vectors
template<i32 x, i32 y, i32 z, i32 w> ILINE f32v4 mix0011(f32v4 a, f32v4 b) { return _mm_shuffle_ps(a, b, _MM_SHUFFLE(w, z, y, x)); }
template<i32 x, i32 y, i32 z, i32 w> f32v4 mix0101(f32v4 a, f32v4 b);  //!< Default configurations unimplemented
...
template<i32 i = 0> ILINE f32 get_element(f32v4 v)
{
	if (i == 0)
...
template<i32 i = 0> ILINE i32 get_element(i32v4 v)
{
	if (i == 0)
...
template<i32 i = 0> ILINE u32 get_element(u32v4 v)
{
...
COMMPOUND_OPERATORS(i32v4, *, i32v4)
{
	...
	return _mm_mullo_epi32(a, b);
		#else
	i32kv4 m02 = _mm_mul_epu32(a, b);
	i32kv4 m13 = _mm_mul_epu32(shuffle<1, 1, 3, 3>(a), shuffle<1, 1, 3, 3>(b));
...
template<class T, class V> ILINE void check_range(V v, T lo, T hi)
{
	DRX_MATH_ASSERT(All(v >= convert<V>(lo)));
	DRX_MATH_ASSERT(All(v <= convert<V>(hi)));
}
...
// Note: for SSE2, trunc, floor, and ceil currently only work for float values in the i32 range
...
template<> ILINE bool IsValid(i32kv4& v) { return true; }
