...
#if 0
template<class S, class value_type, i32k nMAX_ENTRIES>
...
template<class value_type, i32k nMAX_ENTRIES>
...
public:
	ILINE static void fast_interpolate(float t, value_type& val, UnitFloat8* m_table)
	{
		...
		u32 ti = u32(t * scale * (nMAX_ENTRIES - 1.0f));
		u32 idx = ti >> SHIFT_AMOUNT;
		u32 frac = ti - (idx << SHIFT_AMOUNT);
		u32 vali = (u32)m_table[idx + 1].GetStore() * frac;
		frac = (1 << SHIFT_AMOUNT) - frac;
		vali += (u32)m_table[idx].GetStore() * frac;
...
public:
	...
	void GetMemoryUsage(IDrxSizer* pSizer, bool bSelf = false) const
	{
...
protected:
	...
	static i32k DIM = sizeof(value_type) / sizeof(float);
	...
	struct array
	{
		...
		ILINE S& operator[](i32 i)
		{
			assert(i >= 0 && i < DIM);
			return elems[i];
		}
		ILINE const S& operator[](i32 i) const
		{
			assert(i >= 0 && i < DIM);
			return elems[i];
		}
	...
	struct Spline
	{
...
#ifdef _DEBUG
			if (nKeys)
				((tuk)this)[alloc_size()] = 77;
#endif
		}

		static size_t alloc_size(i32 nKeys)
		{
			assert(nKeys > 0);
		...
		u8& key_flags(i32 n) const
		{
			// Flags are stored beyond last element
			assert(n < nKeys);
			return *((u8*) &aElems[nKeys-1].sc + n);
		}

		key_type key(i32 n) const
		{
			key_type key;
			if (n < nKeys)
			{
...
public:
	...
	ILINE i32 num_keys() const
	{
		return m_pSpline ? m_pSpline->nKeys : 0;
	}

	ILINE key_type key(i32 n) const
	{
		assert(n < num_keys());
	...
	void from_source(source_spline& source)
	{
		...
		if (nKeys)
		{
			...
			for (i32 i = 0; i < nKeys; i++)
			{
				...
				static const float fSlopeEquivalence = 1.f / 60.f * drxmath::sqrt(float(DIM));
