	...
	: INumberVector<F, 2, Vec2_tpl<F>>
{
	...
	ILINE Vec2_tpl(F vx, F vy) { x = vx; y = vy; DRX_MATH_ASSERT(IsValid()); }
	...
	{
		quotient_tpl<F> res;
		i32 quad = -(signnz(x * x - y * y) - 1 >> 1); // hope the optimizer will remove complement shifts and adds
		if (quad) { res.x = -y; res.y = x; }
		else { res.x = x; res.y = y; }
		i32 sgny = signnz(res.y);
	...
	{
		DRX_MATH_ASSERT((fabs_tpl(1 - (p | p))) < 0.005); //check if unit-vector
		DRX_MATH_ASSERT((fabs_tpl(1 - (q | q))) < 0.005); //check if unit-vector
