	...
	: INumberVector<F, 4, Quat_tpl<F>>
{
	...
	{
		DRX_MATH_ASSERT(m.IsOrthonormalRH(0.1f));
		SetIdentity();
	...
	{
		DRX_MATH_ASSERT(v0.IsUnit(0.01f));
		DRX_MATH_ASSERT(v1.IsUnit(0.01f));
	...
	{
		DRX_MATH_ASSERT(vdir.IsUnit(0.01f));
	...
	{
		Quat_tpl<F> q = tq;
		DRX_MATH_ASSERT((fabs_tpl(1 - (p | p))) < 0.001); //check if unit-quaternion
		DRX_MATH_ASSERT((fabs_tpl(1 - (q | q))) < 0.001); //check if unit-quaternion
		F cosine = (p | q);
		if (cosine < 0) q = -q;
	...
	{
		DRX_MATH_ASSERT(tp.IsValid());
		DRX_MATH_ASSERT(tq.IsValid());
		DRX_MATH_ASSERT(tp.IsUnit());
		DRX_MATH_ASSERT(tq.IsUnit());
	...
	{
		DRX_MATH_ASSERT((fabs_tpl(1 - (p | p))) < 0.001);   //check if unit-quaternion
		DRX_MATH_ASSERT((fabs_tpl(1 - (tq | tq))) < 0.001); //check if unit-quaternion
		Quat_tpl<F> q = tq;
		if ((p | q) < 0) { q = -q; }
...
typedef Quat_tpl<f32>  DrxQuat;
typedef Quat_tpl<f32>  quaternionf;
typedef Quat_tpl<real> quaternion;

// aligned versions
#ifndef MAX_API_NUM
typedef DRX_ALIGN (16) Quat QuatA;
typedef DRX_ALIGN (32) Quatd QuatrA;
#endif
...
template<typename F> struct QuatT_tpl
{
	...
	{
		DRX_MATH_ASSERT(p.q.IsValid());
		DRX_MATH_ASSERT(tq.q.IsValid());
		Quat_tpl<F> d = tq.q;
		if ((p.q | d) < 0) { d = -d; }
...
//! aligned versions
typedef DRX_ALIGN (32) QuatT QuatTA;       //!< wastes 4byte per quatT
typedef DRX_ALIGN (16) QuatTd QuatTrA;
...
// aligned versions
typedef DRX_ALIGN (16) QuatTS QuatTSA;
typedef DRX_ALIGN (64) QuatTSd QuatTSrA;
...
// Aligned versions.
typedef DRX_ALIGN (16) QuatTNS QuatTNSA;
typedef DRX_ALIGN (64) QuatTNSr QuatTNSrA;
typedef DRX_ALIGN (64) QuatTNS_f64 QuatTNS_f64A;
...
#else
typedef DualQuat_tpl<f32>  DrxDualQuat;
#endif
