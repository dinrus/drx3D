  ...
  SPipQTangents* pQTangents, const uint qtangentStride)
{
	...
	for (uint i = 0; i < count; ++i)
	{
		...
		pTangent = (const Vec4sf*)(((u8k*)pTangent) + tangentStride);
		pBitangent = (const Vec4sf*)(((u8k*)pBitangent) + bitangentStride);
		pQTangents = (SPipQTangents*)(((u8*)pQTangents) + qtangentStride);
	}
	return true;
}
  ...
  SPipQTangents* pQTangents, const uint qtangentStride)
{
	...
	for (uint i = 0; i < count; ++i)
	{
		...
		pTangents = (const SPipTangents*)(((u8k*)pTangents) + tangentStride);
		pQTangents = (SPipQTangents*)(((u8*)pQTangents) + qtangentStride);
	}
	return true;
}
  ...
  SMeshQTangents* pQTangents, const uint qtangentStride)
{
	...
	for (uint i = 0; i < count; ++i)
	{
		...
		*pQTangents = SMeshQTangents(qtangent);
		...
		pTangents = (const SMeshTangents*)(((u8k*)pTangents) + tangentStride);
		pQTangents = (SMeshQTangents*)(((u8*)pQTangents) + qtangentStride);
	}
	return true;
}
