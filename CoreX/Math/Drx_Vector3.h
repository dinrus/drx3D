	...
	: INumberVector<F, 3, Vec3_tpl<F>>
{
	...
	//!   Vec3 v1(1,2,3);
	//!   v2.Set(1,2,3);
	ILINE Vec3_tpl(F vx, F vy, F vz) : x(vx), y(vy), z(vz){ DRX_MATH_ASSERT(IsValid()); }
	ILINE void         operator()(F vx, F vy, F vz)                  { x = vx; y = vy; z = vz; DRX_MATH_ASSERT(IsValid()); }
	ILINE Vec3_tpl<F>& Set(const F xval, const F yval, const F zval) { x = xval; y = yval; z = zval; DRX_MATH_ASSERT(IsValid()); return *this; }

	ILINE Vec3_tpl<F>(const Vec2_tpl<F> &v, F vz = 0) { x = v.x; y = v.y; z = vz;  DRX_MATH_ASSERT(IsValid()); }
	template<class F2> ILINE Vec3_tpl<F>(const Vec2_tpl<F2> &v, F2 vz = 0) { x = F(v.x); y = F(v.y); z = F(vz);  DRX_MATH_ASSERT(IsValid()); }
	...
	ILINE Vec3_tpl GetPermutated(i32 new_z) const { return Vec3_tpl(*(&x + inc_mod3[new_z]), *(&x + dec_mod3[new_z]), *(&x + new_z)); }
	...
	{
		return b * ((*this | b) * drxmath::rcp_safe(b | b));
	}
	...
	{
		DRX_MATH_ASSERT(p.IsUnit(0.005f));
		DRX_MATH_ASSERT(q.IsUnit(0.005f));
	...
	//F* fptr = vec;
	DRX_DEPRECATED("Use begin() instead") operator F*() { return (F*)this; }
	template<class T> DRX_DEPRECATED("Use Vec3_tpl(const Vec3_tpl&) instead") explicit Vec3_tpl(const T* src) { x = src[0]; y = src[1]; z = src[2]; }
...
//specialization of the generic version from Drx_Math with adjusted epsilon
	...
	: INumberVector<F, 3, Ang3_tpl<F>>
{
	...
	ILINE Ang3_tpl(F vx, F vy, F vz) : x(vx), y(vy), z(vz) { DRX_MATH_ASSERT(IsValid()); }

	ILINE Ang3_tpl(const Vec3_tpl<F> &v) : x(v.x), y(v.y), z(v.z) { DRX_MATH_ASSERT(IsValid()); }
	ILINE operator const Vec3_tpl<F>&() const { return reinterpret_cast<const Vec3_tpl<F>&>(*this); }

	ILINE void  operator()(F vx, F vy, F vz) { x = vx; y = vy; z = vz; DRX_MATH_ASSERT(IsValid()); }
	ILINE Ang3_tpl<F>& Set(F vx, F vy, F vz) { x = vx; y = vy; z = vz; DRX_MATH_ASSERT(IsValid()); return *this; }
	...
	ILINE void RangePI()
	{
		x = drxmath::wrap(x, -gf_PI, +gf_PI);
		y = drxmath::wrap(y, -gf_PI, +gf_PI);
		z = drxmath::wrap(z, -gf_PI, +gf_PI);
	}

	//! Convert unit quaternion to angle (xyz).
	explicit Ang3_tpl(const Quat_tpl<F>& q)
	{
		DRX_MATH_ASSERT(q.IsValid());
	...
	{
		DRX_MATH_ASSERT(Matrix33_tpl<F>(m).IsOrthonormalRH(0.001f));
		y = asin_tpl(-m.m20);
		if (fabs_tpl(fabs_tpl(y) - (F)((F)g_PI * (F)0.5)) < (F)0.01)
		{
			x = F(0);
...
template<typename F> ILINE i32 idxmax3(const Vec3_tpl<F>& vec)
{
	i32 imax = isneg(vec.x - vec.y);
	imax |= isneg(vec[imax] - vec.z) << 1;
	return imax & (2 | (imax >> 1 ^ 1));
}
template<typename F> ILINE i32 idxmin3(const Vec3_tpl<F>& vec)
{
	i32 imin = isneg(vec.y - vec.x);
