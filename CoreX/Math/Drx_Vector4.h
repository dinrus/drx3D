	...
	: INumberVector<F, 4, Vec4_tpl<F>>
{
	...
	ILINE Vec4_tpl(F vx, F vy, F vz, F vw) { x = vx; y = vy; z = vz; w = vw; DRX_MATH_ASSERT(IsValid()); };
	ILINE Vec4_tpl(const Vec3_tpl<F>& v, F vw) { x = v.x; y = v.y; z = v.z; w = vw; DRX_MATH_ASSERT(IsValid()); };

	ILINE operator const Vec3_tpl<F> &() const { return reinterpret_cast<const Vec3_tpl<F>&>(*this); }

	ILINE void operator()(F vx, F vy, F vz, F vw)     { x = vx; y = vy; z = vz; w = vw; DRX_MATH_ASSERT(IsValid()); };
	ILINE void operator()(const Vec3_tpl<F>& v, F vw) { x = v.x; y = v.y; z = v.z; vw = vw; DRX_MATH_ASSERT(IsValid()); };
...
typedef Vec4H<i32>  Vec4i;

#else

typedef Vec4_tpl<f32>  Vec4;
typedef Vec4_tpl<i32>  Vec4i;

#endif

#ifdef DRX_HARDWARE_PLANE4
