	...
	: INumberArray<vector4_t<F>, 4>
{
	...
	// Access elements
	ILINE const Vec4& operator[](i32 i) const   { DRX_MATH_ASSERT((uint)i < 4); return (&x)[i]; }
	ILINE Vec4& operator[](i32 i)               { DRX_MATH_ASSERT((uint)i < 4); return (&x)[i]; }

	ILINE F    operator()(i32 i, i32 j) const   { return (&m00)[i * 4 + j]; }
	ILINE F&   operator()(i32 i, i32 j)         { return (&m00)[i * 4 + j]; }

	ILINE F*         GetData()                  { return &m00; }
	ILINE const F*   GetData() const            { return &m00; }

	ILINE Vec4 GetRow4(i32 i) const             { return (*this)[i]; }
	ILINE void SetRow4(i32 i, const Vec4& v)    { (*this)[i] = v; }

	ILINE Vec4 GetColumn4(i32 i) const          { return Vec4(x[i], y[i], z[i], w[i]); }
	ILINE void SetColumn4(i32 i, const Vec4& v) { x[i] = v.x; y[i] = v.y; z[i] = v.z; w[i] = v.w; }

	ILINE Vec3 GetRow(i32 i) const              { return (*this)[i]; }
	ILINE void SetRow(i32 i, const Vec3& v)     { (*this)(i, 0) = v.x; (*this)(i, 1) = v.y; (*this)(i, 2) = v.z; }
