...
struct TCBSplineKey : public SplineKey<T>
{
	...
	static i32 tangent_passes()
	{
		return 2;
	}
	void compute_tangents(const TCBSplineKey* prev, const TCBSplineKey* next, i32 pass, i32 num_keys)
	{
		if (pass == 0)
		{
...
private:
	...
	void compFirstDeriv(const TCBSplineKey* next, i32 num_keys);
	void compLastDeriv(const TCBSplineKey* prev, i32 num_keys);
};
...
template<class T>
void TCBSplineKey<T >::compFirstDeriv(const TCBSplineKey* next, i32 num_keys)
{
...
template<class T>
void TCBSplineKey<T >::compLastDeriv(const TCBSplineKey* prev, i32 num_keys)
{
	if (this->flags.inTangentType != ETangentType::Custom)
	{
		if (prev)
...
struct TCBAngAxisKey : public TCBSplineKey<Quat>
{
	...
	void compute_tangents(const TCBAngAxisKey* prev, const TCBAngAxisKey* next, i32 pass, i32 num_keys)
	{
		Quat qp, qm;
		float fp, fn;
