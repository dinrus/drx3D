...
//
//	File:AABBSV.h
//	Описание: shadow volume AABB functionality for overlap testings
//
//	История:
...
inline void NAABB_SV::AABB_ReceiverShadowVolume(const Vec3& PointLight, const AABB& Occluder, Shadowvolume& sv)
{
	...
	u32 p0 = BoxSides[(front << 3) + 0];
	u32 p1 = BoxSides[(front << 3) + 1];
	u32 p2 = BoxSides[(front << 3) + 2];
	u32 p3 = BoxSides[(front << 3) + 3];
	u32 p4 = BoxSides[(front << 3) + 4];
	u32 p5 = BoxSides[(front << 3) + 5];
...
{
	u32 pa = sv.sideamount + sv.nplanes;
	...
	for (u32 x = 0; x < pa; x++)
	{
		...
		u32k bitX = ux.uintVal >> 31;
		u32k bitY = uy.uintVal >> 31;
		u32k bitZ = uz.uintVal >> 31;
...
inline char NAABB_SV::Is_AABB_In_ShadowVolume_hierarchical(const Shadowvolume& sv, const AABB& Receiver)
{
	...
	f32 dot1, dot2;
	u32 notOverlap = 0x80000000; // Will be reset to 0 if there's at least one overlapping.
