...
protected:
	...
	inline void construct_(uk buffer, const container_type& value) const;
	inline void destruct_(uk buffer) const;
...
void simple_hash_lookup<KeyTy, ValueTy, KeyHash, IndexTy >::reset(size_t _capacity, size_t _bucketCount)
{
	...
	if ((max_count != _capacity) || (bucket_count != _bucketCount))
	{
		...
		if (max_count && bucket_count)
		{
			...
			buckets = (index_type*)(((u8*)values) + sizeof(container_type) * max_count);
			overflow = buckets + bucket_count;
		}
	}

	if (buckets)
		memset(buckets, (u8)invalid_value_index, sizeof(index_type) * (bucket_count + max_count));
}
...
template<typename KeyTy, typename ValueTy, typename KeyHash, typename IndexTy>
void simple_hash_lookup<KeyTy, ValueTy, KeyHash, IndexTy >::construct_(uk buffer, const container_type& value) const
{
	new(buffer) container_type(value);
}

template<typename KeyTy, typename ValueTy, typename KeyHash, typename IndexTy>
void simple_hash_lookup<KeyTy, ValueTy, KeyHash, IndexTy >::destruct_(uk buffer) const
{
