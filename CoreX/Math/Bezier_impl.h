...
bool Serialize(Serialization::IArchive& ar, std::vector<SBezierKey>& value, tukk name, tukk label)
{
	...
	else
	{
		...
		if (ar.isOutput())
		{
			...
			for (size_t i = 0; i < vecSize; ++i)
			{
				...
				keystr.Format("%d:%g:%g:%g:%g:%g:%d:%d:%d,",
				              ...
				              i32(cp.m_inTangentType), i32(cp.m_outTangentType), i32(cp.m_bBreakTangents));
		...
		if (ar.isInput())
		{
			...
			while (!key.empty())
			{
				...
				Vec2 keyInTan;
				Vec2 keyOutTan;
				i32 keyInTanType, keyOutTanType, keyBreakTan;

				i32 res = sscanf(key, "%d:%g:%g:%g:%g:%g:%d:%d:%d",
				                 &keyTime, &keyValue,
