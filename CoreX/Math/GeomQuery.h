...
public:
	...
	float PartExtent(i32 nPart) const
	{
		return nPart == 0 ?
	...
	void ReserveParts(i32 nCount)
	{
		m_afCumExtents.reserve(nCount);
	}

	//! Find element in sorted array <= index (normalized 0 to 1).
	i32 GetPart(float fIndex) const
	{
		i32 last = m_afCumExtents.size() - 1;
		if (last <= 0)
			return last;
	...
	i32 RandomPart(CRndGen& seed) const
	{
		return GetPart(seed.GenerateFloat());
	}

	void RandomParts(Array<i32> aParts, CRndGen& seed) const
	{
		for (i32& part : aParts)
	...
	Array<i32> RandomPartsAlias(Array<PosNorm> aPoints, CRndGen& seed) const;
...
{
	i32 n = seed.GetRandom(0U, array.size() - 1);
	return array[n];
}
...
inline i32 TriMeshPartCount(EGeomForm eForm, i32 nIndices)
{
	switch (eForm)
	{
	default:
...
inline i32 TriIndices(i32 aIndices[3], i32 nPart, EGeomForm eForm)
{
	switch (eForm)
	{
	default:
...
protected:
	...
	template<typename S>
	bool Snip(i32 u, i32 v, i32 w, i32 n, const S* V) const
	{
		float Ax = (*this)[V[u]].x;
		float Ay = (*this)[V[u]].y;
