...
//
//	File:Drx_Camera.h
//	Описание: Общая имплементация класса Camera
//
//	История:
...
enum cull
{
	CULL_EXCLUSION,     //!< Весь объект вне фрустума.
	CULL_OVERLAP,       //!< Объект и фрустум накладываются.
	CULL_INCLUSION      //!< Весь объект внутри фрустума (области видимости).
};

//! Реализует основные операции типа вычисления view-matrix и frustum-culling
//! с простыми геометрическими примитивами (Point, Sphere, AABB, OBB).
//! Все вычисления базируются на координатной системе DRXENGINE.
//! У нас используется "праворукая" координатная система, где положительные точки оси X
//! находятся справа, положительные точки оси Y - прочь от наблюдателя, а положительные
//! точки оси Z - вверх. Наша координатная система представлена рисунком ниже.
//! <PRE>
//! ось z
//!  ^
//!  |
...
//!  +---------------->   ось x
//! </PRE>
//! Такая же система используется в 3D-Studio-MAX. Нет ничего небычного в том, что 3D-API типа D3D9 или
//! OpenGL используют другие координатные системы. На данный момент в D3D9 используется координатка,
//! в которой ось X направлена вправо, ось Y - вниз, а ось Z- прочь от наблюдателя.
//! Чтобы преобразовать из системы DinrusX в D3D9, надо выполнить вращение по часовой стрелке
//! pi/2 вокруг оси X. Это преобразование происходит в рендерере (renderer).
//! 6 DOFs (степеней свободы) сохранены в одной единственной матрице 3x4 ("m_Matrix"). 3
//! ориентационных DOFs сохраняются в части 3x3, а 3 позиционных DOFs хранятся в векторе трансляции.
//! Можно использовать функции-члены "GetMatrix()" или "SetMatrix(Matrix34(orientation,positon))"
//! для доступа к ним, и изменения этих 6-ти степеней свободы (DOFs).
//! В Drx_Math.h есть вспомогательная функция для создания ориентации:
//! Эта функция строит ориентационную матрицу 3x3, используя view-direction и radiant
//! для вращения вокруг оси Y.
//!   Matrix33 orientation=Matrix33::CreateOrientation( Vec3(0,1,0), 0 );
//! Эта функция строит ориентационную матрицу 3x3, используя углы Yaw-Pitch-Roll.
...
public:
	...
	ILINE void            SetMatrix(const Matrix34& mat)         { DRX_MATH_ASSERT(mat.IsOrthonormal()); m_Matrix = mat; UpdateFrustum(); };
	ILINE void            SetMatrixNoUpdate(const Matrix34& mat) { DRX_MATH_ASSERT(mat.IsOrthonormal()); m_Matrix = mat; };
	...
	ILINE void            CalcScreenBounds(i32* vOut, const AABB* pAABB, i32 x, i32 y, i32 nWidth, i32 nHeight) const;
	ILINE void            CalcScreenBounds(i32* vOut, const AABB* pAABB, i32 nWidth, i32 nHeight) const;
	...
	void               SetFrustum(i32 nWidth, i32 nHeight, f32 FOV = DEFAULT_FOV, f32 nearplane = DEFAULT_NEAR, f32 farpane = DEFAULT_FAR, f32 fPixelAspectRatio = 1.0f);
	ILINE void         SetAsymmetry(float l, float r, float b, float t) { m_asymL = l; m_asymR = r; m_asymB = b; m_asymT = t; UpdateFrustum(); }

	ILINE i32          GetViewSurfaceX() const                          { return m_Width; }
	ILINE i32          GetViewSurfaceZ() const                          { return m_Height; }
	...
	ILINE const Vec3&  GetNPVertex(i32 nId) const; //!< get near-plane vertices
	ILINE const Vec3&  GetFPVertex(i32 nId) const; //!< get far-plane vertices
	ILINE const Vec3&  GetPPVertex(i32 nId) const; //!< get projection-plane vertices

	ILINE const Plane* GetFrustumPlane(i32 numplane)  const { return &m_fp[numplane]; }
	...
	u8 IsSphereVisible_FH(const Sphere& s) const;   //!< This is going to be the exact version of sphere-culling.

	// AABB-frustum test
	//! Fast
	bool  IsAABBVisible_F(const AABB& aabb) const;
	u8 IsAABBVisible_FH(const AABB& aabb, bool* pAllInside) const;
	u8 IsAABBVisible_FH(const AABB& aabb) const;

	// Exact
	bool  IsAABBVisible_E(const AABB& aabb) const;
	u8 IsAABBVisible_EH(const AABB& aabb, bool* pAllInside) const;
	u8 IsAABBVisible_EH(const AABB& aabb) const;
	...
	u8 IsOBBVisible_FH(const Vec3& wpos, const OBB& obb) const;
	bool  IsOBBVisible_E(const Vec3& wpos, const OBB& obb, f32 uscale) const;
	u8 IsOBBVisible_EH(const Vec3& wpos, const OBB& obb, f32 uscale) const;

	void GetFrustumVertices(Vec3* pVerts) const;
	void GetFrustumVerticesCam(Vec3* pVerts) const;

	void SetJustActivated(const bool justActivated) { m_JustActivated = (i32)justActivated; }
...
private:
	NO_INLINE_WEAK bool AdditionalCheck(const AABB& aabb) const
	{
	...
	NO_INLINE_WEAK bool AdditionalCheck(const Vec3& wpos, const OBB& obb, f32 uscale) const
	{
	...
	i32      m_Width;                //!< Surface width-resolution.
	i32      m_Height;               //!< Surface height-resolution.
	...
	u32 m_idx1[FRUSTUM_PLANES], m_idy1[FRUSTUM_PLANES], m_idz1[FRUSTUM_PLANES];
	u32 m_idx2[FRUSTUM_PLANES], m_idy2[FRUSTUM_PLANES], m_idz2[FRUSTUM_PLANES];
...
public:
	...
	void SetPPVertex(i32 nId, const Vec3& vVert)
	{
		switch (nId)
		{
		case 0:
	...
	inline void SetFrustumPlane(i32 i, const Plane& plane)
	{
		m_fp[i] = plane;
	...
	i32                m_JustActivated; //!< Camera activated in this frame, used for disabling motion blur effect at camera changes in movies.
...
{
	DRX_MATH_ASSERT(m.IsOrthonormal());
...
{
	DRX_MATH_ASSERT((fabs_tpl(1 - (vdir | vdir))) < 0.001);   //!< Check if unit vector.
...
{
	DRX_MATH_ASSERT(ypr.IsInRangePI());   //!< all angles need to be in range between -pi and +pi
	f32 sz, cz;
...
inline bool CCamera::Project(const Vec3& p, Vec3& result, Vec2i topLeft, Vec2i widthHeight) const
{
	...
	i32 pViewport[4] = { 0, 0, GetViewSurfaceX(), GetViewSurfaceZ() };
...
{
	i32 viewport[4] = { 0, 0, GetViewSurfaceX(), GetViewSurfaceZ() };
...
inline void CCamera::CalcScreenBounds(i32* vOut, const AABB* pAABB, i32 x, i32 y, i32 nWidth, i32 nHeight) const
{
	...
	vOut[0] = (i32)max(pViewport[0], min(pViewport[2], x0));
	vOut[1] = (i32)max(pViewport[1], min(pViewport[3], y0));
	vOut[2] = (i32)max(pViewport[0], min(pViewport[2], x1));
	vOut[3] = (i32)max(pViewport[1], min(pViewport[3], y1));
}

inline void CCamera::CalcScreenBounds(i32* vOut, const AABB* pAABB, i32 nWidth, i32 nHeight) const
{
	return CalcScreenBounds(vOut, pAABB, 0, 0, nWidth, nHeight);
}

inline void CCamera::SetFrustum(i32 nWidth, i32 nHeight, f32 FOV, f32 nearplane, f32 farpane, f32 fPixelAspectRatio)
{
	DRX_MATH_ASSERT(nearplane > 0.001f);               // Check if near-plane is valid.
	DRX_MATH_ASSERT(farpane > 0.1f);                   // Check if far-plane is valid.
	DRX_MATH_ASSERT(farpane > nearplane);              // Check if far-plane bigger then near-plane.
	DRX_MATH_ASSERT(FOV >= 0.0000001f && FOV < gf_PI); // Check if specified FOV is valid.
	...
	DRX_MATH_ASSERT(fabs(acos_tpl(Vec3d(0, m_edge_plt.y, m_edge_plt.z).GetNormalized().y) * 2 - m_fov) < 0.001);
...
inline void CCamera::UpdateFrustum()
{
	...
	u32 rh = m_Matrix.IsOrthonormalRH();
	if (rh == 0)
	{
	...
	for (i32 i = 0; i < FRUSTUM_PLANES; i++)
	{
...
ILINE const Vec3& CCamera::GetNPVertex(i32 nId) const
{
	switch (nId)
	{
	case 0:
...
ILINE const Vec3& CCamera::GetFPVertex(i32 nId) const
{
	switch (nId)
	{
	case 0:
...
ILINE const Vec3& CCamera::GetPPVertex(i32 nId) const
{
	switch (nId)
	{
	case 0:
...
inline u8 CCamera::IsSphereVisible_FH(const Sphere& s) const
{
...
//! Example: i32 InOut=camera.IsAABBVisible_FH(aabb);
...
inline u8 CCamera::IsAABBVisible_FH(const AABB& aabb, bool* pAllInside) const
{
	DRX_MATH_ASSERT(pAllInside && *pAllInside == false);
...
//! \retval CULL_OVERLAP    AABB intersects the borders of the frustum, further checks necessary.
//! \retval CULL_INCLUSION  AABB is complete inside the frustum, no further checks necessary.
inline u8 CCamera::IsAABBVisible_FH(const AABB& aabb) const
{
	if (IsAABBVisible_F(aabb) == CULL_EXCLUSION) return CULL_EXCLUSION;
	const f32* p = &aabb.min.x;
	u32 x, y, z;
...
//! Example: i32 InOut=camera.IsAABBVisible_E(aabb);
//! \retval CULL_EXCLUSION   AABB outside of frustum (very fast rejection-test).
//! \retval CULL_OVERLAP     AABB either intersects the borders of the frustum or is totally inside.
inline bool CCamera::IsAABBVisible_E(const AABB& aabb) const
{
	u8 o = IsAABBVisible_FH(aabb);
...
//! Example: i32 InOut=camera.IsAABBVisible_EH(aabb);
//! \retval CULL_EXCLUSION  AABB outside of frustum (very fast rejection-test).
//! \retval CULL_OVERLAP    AABB intersects the borders of the frustum, further checks necessary.
//! \retval CULL_INCLUSION  AABB is complete inside the frustum, no further checks necessary.
inline u8 CCamera::IsAABBVisible_EH(const AABB& aabb, bool* pAllInside) const
{
	u8 o = IsAABBVisible_FH(aabb, pAllInside);
	if (o == CULL_EXCLUSION) return CULL_EXCLUSION;
...
//! \retval CULL_OVERLAP    AABB intersects the borders of the frustum, further checks necessary.
//! \retval CULL_INCLUSION  AABB is complete inside the frustum, no further checks necessary.
inline u8 CCamera::IsAABBVisible_EH(const AABB& aabb) const
{
	u8 o = IsAABBVisible_FH(aabb);
	if (o == CULL_EXCLUSION) return CULL_EXCLUSION;
	if (o == CULL_INCLUSION) return CULL_INCLUSION;
...
inline bool CCamera::IsAABBVisible_EHM(const AABB& aabb, bool* pAllInside) const
{
	...
	for (i32 i = 0; i < m_pMultiCamera->Count(); i++)
	{
		bool bAllIn = false;
...
{
	DRX_MATH_ASSERT(obb.m33.IsOrthonormalRH(0.001f));
...
//! Not yet implemented.
ILINE u8 CCamera::IsOBBVisible_FH(const Vec3& wpos, const OBB& obb) const
{
...
//! Example: i32 InOut=camera.IsOBBVisible_E(OBB);
//! \retval CULL_EXCLUSION  OBB outside of frustum (very fast rejection-test).
...
//! Example: i32 InOut=camera.IsOBBVisible_EH(obb);
//! \retval CULL_EXCLUSION  OBB outside of frustum (very fast rejection-test)
...
inline u8 CCamera::IsOBBVisible_EH(const Vec3& wpos, const OBB& obb, f32 uscale = 1.0f) const
{
	DRX_MATH_ASSERT(obb.m33.IsOrthonormalRH(0.001f));
	//transform the obb-center into world-space
	Vec3 p = obb.m33 * obb.c * uscale + wpos;
