...
typedef u32 DrxLobbyTaskID;
const DrxLobbyTaskID DrxLobbyInvalidTaskID = 0xffffffff;

enum EDrxLobbyService
{
	eCLS_LAN,
	eCLS_Online,
	eCLS_NumServices
...
enum EDrxLobbyError
{
	...
	eCLE_TooManyParameters          = 35, //!< Too many parameters were passed to the task. e.g an array of SDrxLobbyUserData has more items than the task expected.
	eCLE_NotEnoughParameters        = 36, //!< Not enough parameters were passed to the task. e.g an array of SDrxLobbyUserData has less items than the task expected.
	eCLE_DuplicateParameters        = 37, //!< Duplicate parameters were passed to the task. e.g an array of SDrxLobbyUserData has the same item more than once.
...
#include "DrxLobbyEventCallback.h"    // Separated out for readability

struct SConfigurationParams;
typedef void (* DrxLobbyConfigurationCallback)(EDrxLobbyService service, SConfigurationParams* requestedParams, u32 paramCount);
typedef void (* DrxLobbyCallback)(EDrxLobbyService service, EDrxLobbyError error, uk arg);

//! Flags for use with IDrxLobby::GetLobbyServiceFlag.
enum EDrxLobbyServiceFlag
{
...
struct SDrxLobbyParameters
{
	u16 m_listenPort;        //!< Listen port the lobby service will use for connections.
	u16 m_connectPort;       //!< Connect port the lobby service will use for connections.
};

struct IDrxLobby : public Drx::IEnginePlugin
{
public:
	DRXINTERFACE_DECLARE_GUID(IDrxLobby, "{3ED8EF88-5332-4BDF-A5CB-5A3AD5016279"_drx_guid);
	...
	//! \param features  Which features of the service to initialise (e.g.  EDrxLobbyServiceFeatures(eCLSO_All & (~eCLSO_Reward))).
	...
	virtual EDrxLobbyError Initialise(EDrxLobbyService service, EDrxLobbyServiceFeatures features, DrxLobbyConfigurationCallback cfgCb, DrxLobbyCallback cb, uk cbArg) = 0;

	//! Shut down a lobby service.
	//! \param Service		The service to be shut down.
	//! \param Features		Which features of the service to terminate (e.g.  EDrxLobbyServiceFeatures(eCLSO_All & (~eCLSO_Reward))).
	//! \param Cb					Callback function that is called when function completes.
	//! \param CbArg			Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError Terminate(EDrxLobbyService service, EDrxLobbyServiceFeatures features, DrxLobbyCallback cb, uk cbArg) = 0;
	...
	virtual EDrxLobbyError ProcessEvents() = 0;

	//! Set the current lobby service.
	//! \param Service The service to use.
	//! \return The service that was in use before the call.
	virtual EDrxLobbyService SetLobbyService(EDrxLobbyService service) = 0;

	//! Get the current lobby service type.
	//! \return The EDrxLobbyService of the current lobby service.
	virtual EDrxLobbyService GetLobbyServiceType() = 0;

	//! Get the current lobby service.
	//! \return Pointer to an IDrxLobbyService for the current lobby service.
	virtual IDrxLobbyService* GetLobbyService() = 0;

	//! Get the current matchmaking service.
	//! \return Pointer to an IDrxMatchMaking associated with the current lobby service.
	virtual IDrxMatchMaking* GetMatchMaking() = 0;

	//! Get the DinrusXNetwork-specific interface of MatchMaking.
	virtual IDrxMatchMakingPrivate* GetMatchMakingPrivate() = 0;

	//! Get a base MatchMaking interface for console-issued matchmaking commands.
	virtual IDrxMatchMakingConsoleCommands* GetMatchMakingConsoleCommands() = 0;

	//! Get the current voice service.
	//! \return Pointer to an IDrxVoice associated with the current lobby service.
	virtual IDrxVoice* GetVoice() = 0;

	//! Get the current stats service.
	//! \return Pointer to an IDrxStats associated with the current lobby service.
	virtual IDrxStats* GetStats() = 0;

	//! Get the current lobby ui service.
	//! \return Pointer to an IDrxLobbyUI associated with the current lobby service.
	virtual IDrxLobbyUI* GetLobbyUI() = 0;

	//! Get the current friends service.
	//! \return Pointer to an IDrxFriends associated with the current lobby service.
	virtual IDrxFriends* GetFriends() = 0;

	//! Get the current friends management service.
	//! \return Pointer to an IDrxFriendsManagement associated with the current lobby service.
	virtual IDrxFriendsManagement* GetFriendsManagement() = 0;

	//! Get the current reward service (achievements/trophies).
	//! \return Pointer to an IDrxReward associated with the current lobby service.
	virtual IDrxReward* GetReward() = 0;

	//! Get the current online storage service (title/user storage on PSN and LIVE).
	//! \return Pointer to an IDrxOnlineStorage associated with the current lobby service.
	virtual IDrxOnlineStorage* GetOnlineStorage() = 0;

	//! Get the requested lobby service.
	//! \param Service		The service to wanted.
	//! \return	Pointer to an IDrxLobbyService for the lobby service requested.
	virtual IDrxLobbyService* GetLobbyService(EDrxLobbyService service) = 0;
	...
	virtual void RegisterEventInterest(EDrxLobbySystemEvent eventOfInterest, DrxLobbyEventCallback cb, uk pUserArg) = 0;
	...
	virtual void UnregisterEventInterest(EDrxLobbySystemEvent eventOfInterest, DrxLobbyEventCallback cb, uk pUserArg) = 0;

	//! Retrieve various information about the current lobby service.
	//! \return SDrxLobbyParameters - reference to a structure that contains some essential information about the lobby.
	virtual const SDrxLobbyParameters& GetLobbyParameters() const = 0;
	...
	virtual bool GetLobbyServiceFlag(EDrxLobbyService service, EDrxLobbyServiceFlag flag) = 0;
	...
	virtual void SetUserPacketEnd(u32 end) = 0;
	...
	virtual void SetDinrusXLoadHint(tukk levelName, tukk gameRules) = 0;

	//! \return Level name previously set by SetDinrusXLoadHint or "" if never set.
	virtual tukk GetDinrusXLevelNameHint() = 0;

	//! \return Game rules name previously set by SetDinrusXLoadHint or "" if never set.
	virtual tukk GetDinrusXRulesNameHint() = 0;

	/////////////////////////////////////////////////////////////////////////////
	// Host Migration
	virtual void AddHostMigrationEventListener(IHostMigrationEventListener* pListener, tukk pWho, u32 priority) = 0;
