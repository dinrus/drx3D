...
typedef u8  JointIdType;
#else
typedef u16 JointIdType;
#endif
...
struct SShaderParam
{
	...
	static bool SetParam(tukk name, DynArrayRef<SShaderParam>* Params, const UParamVal& pr)
	{
		u32 i;
		for (i = 0; i < (u32)Params->size(); i++)
		{
	...
	static bool GetValue(tukk szName, DynArrayRef<SShaderParam>* Params, float* v, i32 nID);

	static bool GetValue(u8 eSemantic, DynArrayRef<SShaderParam>* Params, float* v, i32 nID);
...
struct IShaderPublicParams : public _i_multithread_reference_target_t
{
	...
	virtual void SetParamCount(i32 nParam) = 0;
	...
	virtual SShaderParam&       GetParam(i32 nIndex) = 0;
	virtual const SShaderParam& GetParam(i32 nIndex) const = 0;

	//! Retrieves shader public parameter at specified index of the collection.
	virtual SShaderParam*       GetParamByName(tukk pszName) = 0;
	virtual const SShaderParam* GetParamByName(tukk pszName) const = 0;

	virtual SShaderParam*       GetParamBySemantic(u8 eParamSemantic) = 0;
	virtual const SShaderParam* GetParamBySemantic(u8 eParamSemantic) const = 0;

	//! Sets a shader parameter (and if doesn't exists, add it to the parameters list).
	virtual void SetParam(tukk pszName, UParamVal& pParam, EParamType nType = eType_FLOAT, u8 eSemantic = 0) = 0;

	//! Assigns shader public parameter at specified index of the collection.
	virtual void SetParam(i32 nIndex, const SShaderParam& param) = 0;
	...
	virtual void RemoveParamByName(tukk pszName) = 0;
	virtual void RemoveParamBySemantic(u8 eParamSemantic) = 0;
	...
	virtual u8 GetSemanticByName(tukk pszName) = 0;
...
public:
	...
	inline static i32 Size()
	{
		i32 nSize = sizeof(CInputLightMaterial);
		return nSize;
	}
};
...
{
	bool SetMember(tukk szParamName, float fValue)
	{
...
struct STexSamplerRT
{
	...
	u8              m_eTexType; //!< ETEX_Type e.g. eTT_2D or eTT_Cube.
	int8               m_nSamplerSlot;
	int8               m_nTextureSlot;
	bool               m_bGlobal;
	...
	u32 GetTexFlags() const { return m_nTexFlags; }
	bool   Update();
	void   PostLoad();
...
struct SEfResTextureExt
{
	...
	{
		i32 nSize = sizeof(SEfResTextureExt);
		if (m_pTexModifier)
...
struct SEfResTexture
{
	...
	void        Update(i32 nTSlot, u32& nMDMask);
	void        UpdateWithModifier(i32 nTSlot, u32& nMDMask);
	...
	inline float GetTiling(i32 n) const
	{
		if (!m_Ext.m_pTexModifier)
			return 1.0f;
		return m_Ext.m_pTexModifier->m_Tiling[n];
	}
	inline float GetOffset(i32 n) const
	{
		if (!m_Ext.m_pTexModifier)
			return 0;
	...
	i32 Size() const
	{
		i32 nSize = sizeof(SEfResTexture) - sizeof(STexSamplerRT) - sizeof(SEfResTextureExt);
		nSize += m_Name.size();
...
struct SBaseShaderResources : public _i_multithread_reference_target_t
{
	...
	i32 Size() const
	{
		i32 nSize = sizeof(SBaseShaderResources) + m_ShaderParams.size() * sizeof(SShaderParam);
		return nSize;
	}
...
struct IRenderShaderResources
{
	...
	virtual i32                        GetResFlags() = 0;
	virtual void                       SetMtlLayerNoDrawFlags(u8 nFlags) = 0;
	virtual u8                      GetMtlLayerNoDrawFlags() const = 0;
	virtual SSkyInfo*                  GetSkyInfo() = 0;
	virtual CCamera*                   GetCamera() = 0;
	virtual void                       SetCamera(CCamera* pCam) = 0;
	virtual void                       SetMaterialName(tukk szName) = 0;
	virtual SEfResTexture*             GetTexture(i32 nSlot) const = 0;
...
struct SInputShaderResources : public SBaseShaderResources
{
	...
	i32 Size() const
	{
		i32 nSize = SBaseShaderResources::Size() - sizeof(SEfResTexture) * EFTT_MAX;
...
struct SShaderGenBit
{
	...
	u32              m_Flags;
	u32              m_dwToken;
	std::vector<u32> m_PrecacheNames;
	std::vector<string> m_DependSets;
	std::vector<string> m_DependResets;
	u32              m_nDependencySet;
	u32              m_nDependencyReset;
...
struct SShaderProfile
{
	...
	{
		m_iShaderProfileQuality = (i32)rValue;
	}

	// ----------------------------------------------------------------

	i32    m_iShaderProfileQuality;  //!< EShaderQuality e.g. eSQ_Medium, use Get/Set functions if possible.
	u32 m_nShaderProfileFlags;    //!< SPF_...
};
...
public:
	...
	virtual i32                        GetFlags2() const = 0;
	virtual void                       SetFlags2(i32 Flags) = 0;
	virtual void                       ClearFlags2(i32 Flags) = 0;
	virtual bool                       Reload(i32 nFlags, tukk szShaderName) = 0;
	virtual TArray<CRenderElement*>*     GetREs(i32 nTech) = 0;
	virtual DynArrayRef<SShaderParam>& GetPublicParams() = 0;
	virtual void                       CopyPublicParamsTo(SInputShaderResources& copyToResource) = 0;
	virtual i32                        GetTexId() = 0;
	virtual ITexture*                  GetBaseTexture(i32* nPass, i32* nTU) = 0;
	virtual u32               GetUsedTextureTypes(void) = 0;
	virtual SShaderTexSlots*           GetUsedTextureSlots(i32 nTechnique) = 0;
	virtual ECull                      GetCull(void) = 0;
	virtual i32                        Size(i32 Flags) = 0;
	virtual uint64                     GetGenerationMask() = 0;
	virtual SShaderGen*                GetGenerationParams() = 0;
	virtual i32                        GetTechniqueID(i32 nTechnique, i32 nRegisteredTechnique) = 0;
	virtual InputLayoutHandle          GetVertexFormat(void) = 0;

	virtual EShaderType                GetShaderType() = 0;
	virtual u32                     GetVertexModificator() = 0;

	virtual void                       GetMemoryUsage(IDrxSizer* pSizer) const = 0;
	// </interfuscator:shuffle>

	static u32 GetTextureSlot(EEfResTextures textureType) { return (u32)textureType; }
};
...
struct SShaderItem
{
	...
	SShaderItem(IShader* pSH, IRenderShaderResources* pRS, i32 nTechnique)
...
struct CRenderChunk
{
	...
	u16   m_nMatFlags; //!< Material flags from originally assigned material @see EMaterialFlags.
	u16   m_nMatID;    //!< Material Sub-object id.
...
#include <drx3D/CoreX/Renderer/IRenderMesh.h>  // <> required for Interfuscator
...
protected:
	ILightAnimWrapper(tukk name) : m_name(name), m_pNode(0) {}
...
ILINE float GetRadiationRadius(float fIntensity, float fThreshold = ILLUMINANCE_THRESHOLD)
{
	...
	return drxmath::sqrt_fast(fIntensity / fThreshold);
}
...
struct SRenderLight
{
	...
	float GetAttenuation(float fDist) const
	{
		if (m_Flags & DLF_DIRECTIONAL)
			return 1.0f;
		return drxmath::rcp(sqr(max(fDist, +m_fAttenuationBulbSize)));
	}
	...
	float GetAttenuation(const Vec3& vPos) const
	{
		...
		return drxmath::rcp(max(fDistSq, sqr(+m_fAttenuationBulbSize)));
	}

	ITexture* GetDiffuseCubemap() const
	{
		assert(m_pDiffuseCubemap != (uk )(uintptr_t)0xfeeefeee);
		return m_pDiffuseCubemap;
	}

	ITexture* GetSpecularCubemap() const
	{
		assert(m_pSpecularCubemap != (uk )(uintptr_t)0xfeeefeee);
...
public:
	...
	i16  m_Id = 0;
	u8  m_nStencilRef[2] = {};
	u32 m_n3DEngineUpdateFrameID = 0;
	u32 m_nEntityId = (u32)-1;
	u32 m_Flags = 0;                   //!< Light flags (DLF_etc).
	...
	i16  m_sX = 0;                      //!< Scissor parameters (2d extent).
	i16  m_sY = 0;
	i16  m_sWidth = 0;
	i16  m_sHeight = 0;
	...
	u8     m_nAttenFalloffMax = 255;
	u8     m_nSortPriority = 0;
	...
	u16               m_nShadowMinResolution = 0;
	u16               m_nShadowUpdateRatio = (1 << DL_SHADOW_UPDATE_SHIFT);
	u8                m_ShadowMaskIndex = 0;
	...
	tukk          m_sName = "";  //!< Optional name of the light source.
	...
	u8                m_nLightStyle = 0;
	u8                m_nLightPhase = 0;
	u8                m_LensOpticsFrustumAngle = 255;  //!< from 0 to 255, The range will be adjusted from 0 to 360 when used.

	IClipVolume*         m_pClipVolumes[2] = {};
};

typedef std::list<SRenderLight> RenderLightsList;
typedef i16 RenderLightIndex;
...
struct SDeferredDecal
{
	...
	u32     nFlags;
	u8      nSortOrder; //!< User defined sort order.
};
...
{
	Vec3     m_center;
	u32   m_viewerInsideVolume : 1;
	u32   m_affectsThisAreaOnly : 1;
	u32   m_stencilRef : 8;
	u32   m_volumeType : 1;
	u32   m_reserved : 21;
	AABB     m_localAABB;
...
{
	static i32k MaxWaterRipplesInScene = 128;
	static i32k WaveSimulationGridSize = 50;
