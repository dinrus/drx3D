...
//! Интерфейс отображения дополнительной геометрии.
//! В основном используется для отладки, редакторских задач.
//! Предоставляет функции для отображения 2-мерной геометрии, а также текста.
struct IRenderAuxGeom
{
	// <interfuscator:shuffle>
	virtual ~IRenderAuxGeom(){}

	//! Устанавливает флаги отображения.
	//! \return Ранее установленные флаги отображения.
	virtual SAuxGeomRenderFlags SetRenderFlags(const SAuxGeomRenderFlags& renderFlags) = 0;

	//! Получает флаги отображения.
	virtual SAuxGeomRenderFlags GetRenderFlags() = 0;

	//! Присваивает текущую камеру отображения отобразителю Aux,
	// используемому для корректного проектирования 3-мерных элементов.
	virtual const CCamera&      GetCamera() const = 0;

	//! Установить текущий контекст показа для последующего вспомогательного отображения.
	virtual void SetCurrentDisplayContext(const SDisplayContextKey& displayContextKey) = 0;

	// Функции отображения 2D/3D.

	//! Нарисовать точку.
	//! \param v    Вектор, сохраняющий позицию точки.
	//! \param col  Цвет, используемый при рисовании.
	//! \param size Размер нарисованной точки.
	virtual void DrawPoint(const Vec3& v, const ColorB& col, u8 size = 1) = 0;

	//! Нарисовать n точек.
	//! \param v         Указатель на список вектора, хранящего позиции точек.
	//! \param numPoints Число точек от начала памяти области, заданной v.
	//! \param col       Цвет, используемый при рисовании.
	//! \param size	     Размер нарисованных точек.
	//! ##@{
	virtual void DrawPoints(const Vec3* v, u32 numPoints, const ColorB& col, u8 size = 1) = 0;
	virtual void DrawPoints(const Vec3* v, u32 numPoints, const ColorB* col, u8 size = 1) = 0;
	//! ##@}

	//! Нарисовать линию.
	//! \param v0         Начальная вершина линии.
	//! \param colV0      Цвет первой вершины.
	//! \param v1         Конечная вершина линии.
	//! \param colV1      Цвет второй вершины.
	...
	//! Нарисовать n линий.
	//! \param v          Список вершин, принадлежащих рисуемой линии.
	//! \param numPoints  Число точек, обнаруживаемых от начала области памяти, определённой v.
	//! \param col        Цвет вершин.
	//! \param thickness  Толщина линии.
	//! ##@{
	virtual void DrawLines(const Vec3* v, u32 numPoints, const ColorB& col, float thickness = 1.0f) = 0;
	virtual void DrawLines(const Vec3* v, u32 numPoints, const ColorB* col, float thickness = 1.0f) = 0;
	//! ##@}

	//! Нарисовать n линий.
	//! \param v            Список вершин, принадлежащих линиям, которые требуется нарисовать.
	//! \param packedColor  Список цветов. принадлежащих каждой рисуемой вершине. Packed into one u32 via ARGB8888. Is of same length as v (i.e., numPoints).
	//! \param numPoints    Число точек, обнаруживаемых от начала области памяти, определённой v.
	//! \param thickness    Толщина линии.
	//! \param alphaFlag    Enable/disable transparency.
	//! ##@{
	virtual void DrawLines(const Vec3* v, u32k* packedColorARGB8888, u32 numPoints, float thickness = 1.0f, bool alphaFlag = true) = 0;
	//! ##@}
	...
	//! \param thickness  Толщина линии.
	//! ##@{
	virtual void DrawLines(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB& col, float thickness = 1.0f) = 0;
	virtual void DrawLines(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB* col, float thickness = 1.0f) = 0;
	//! ##@}
	...
	//! \param thickness  Толщина линии.
	//! ##@{
	virtual void DrawLineStrip(const Vec3* v, u32 numPoints, const ColorB* col, float thickness = 1.0f) = 0;
	//! ##@}
	...
	//! \param thickness  Толщина линии.
	//! ##@{
	virtual void DrawPolyline(const Vec3* v, u32 numPoints, bool closed, const ColorB& col, float thickness = 1.0f) = 0;
	virtual void DrawPolyline(const Vec3* v, u32 numPoints, bool closed, const ColorB* col, float thickness = 1.0f) = 0;
	//! ##@}
	...
	//! \param col			Цвет вершин.
	//! ##@{
	virtual void DrawTriangles(const Vec3* v, u32 numPoints, const ColorB& col) = 0;
	virtual void DrawTriangles(const Vec3* v, u32 numPoints, const ColorB* col) = 0;
	//! ##@}
	...
	//! \param col			Цвет вершин.
	//! ##@{
	virtual void DrawTriangles(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB& col) = 0;
	virtual void DrawTriangles(const Vec3* v, u32 numPoints, const vtx_idx* ind, u32 numIndices, const ColorB* col) = 0;
	//! ##@}
	...
	//! ##@{
	virtual void DrawBuffer(const SAuxVertex* inVertices, u32 numVertices, bool textured) = 0;
	//! ##@}
	...
	//! \param textured		Whether the triangles are textured or not.
	//! ##@{
	virtual SAuxVertex*         BeginDrawBuffer(u32 maxVertices, bool textured) = 0;
	//! ##@}
	...
	//! ##@{
	virtual void EndDrawBuffer(u32 numVertices) = 0;
	//! ##@}
	...
	virtual void DrawAABBs(const AABB* aabbs, u32 aabbCount, bool bSolid, const ColorB& col, const EBoundingBoxDrawStyle& bbDrawStyle) = 0;
	...
	//! Нарисовать сферу.
	//! \param pos			Центр сферы.
	//! \param radius		Радиус сферы.
	//! \param col			Цвет сферы.
	...
	//! Нарисовать конус.
	//! \param pos		Центр основания конуса.
	...
	//! Draw Text.
	//! ##@{
	virtual void RenderTextQueued(Vec3 pos, const SDrawTextInfo& ti, tukk text) = 0;
	...
	void RenderText(Vec3 pos, const SDrawTextInfo& ti, tukk format, va_list args)
	{
		if( format )
		{
			char str[512];

			drx_vsprintf(str, format, args);

			RenderTextQueued(pos, ti, str);
		}
	}

	void Draw2dLabel(float x, float y, float font_size, const ColorF& fColor, bool bCenter, tukk format, va_list args)
	{
		SDrawTextInfo ti;
	...
	void Draw2dLabel(float x, float y, float font_size, const ColorF& fColor, bool bCenter, tukk label_text, ...) PRINTF_PARAMS(7, 8)
	{
		va_list args;
	...
	void Draw2dLabel(float x, float y, float font_size, const float pfColor[4], bool bCenter, tukk label_text, ...) PRINTF_PARAMS(7, 8)
	{
		va_list args;
		va_start(args, label_text);
	...
	virtual i32 PushMatrix(const Matrix34& mat) = 0;
	...
	virtual i32 SetTexture(i32 idTexture) { return -1; }
	...
	virtual void SetMatrixIndex(i32 matID) = 0;
...
public:
	...
	static void DrawText(Vec3 pos, const SDrawTextInfo& ti, tukk text)
	{
		IRenderAuxGeom::GetAux()->RenderTextQueued(pos, ti, text);
	}

	static void DrawText(Vec3 pos, const ASize& size, const AColor& color, i32 flags, tukk text)
	{
	...
	static void DrawText(Vec3 pos, const ASize& size, const AColor& color, i32 flags, tukk format, va_list args)
	{
		if( format )
		{
			char str[512];
	...
	static void DrawTextF(Vec3 pos, const ASize& size, const AColor& color, i32 flags, tukk label_text, ...) PRINTF_PARAMS(5, 6)
	{
		va_list args;
		va_start(args, label_text);
	...
	static void DrawLabel(Vec3 pos, float font_size, tukk text)
	{
		DrawText(pos, font_size, AColor::white(), eDrawText_FixedSize, text);
	}

	static void DrawLabelF(Vec3 pos, float font_size, tukk label_text, ...) PRINTF_PARAMS(3, 4)
	{
		va_list args;
		va_start(args, label_text);
	...
	static void DrawLabelEx(Vec3 pos, float font_size, const AColor& color, bool bFixedSize, bool bCenter, tukk text)
	{
		DrawText(pos, font_size, color, ((bFixedSize) ? eDrawText_FixedSize : 0) | ((bCenter) ? eDrawText_Center : 0), text);
	}

	static void DrawLabelExF(Vec3 pos, float font_size, const AColor& color, bool bFixedSize, bool bCenter, tukk label_text, ...) PRINTF_PARAMS(6, 7)
	{
		va_list args;
		va_start(args, label_text);
	...
	static void Draw2dLabelEx(float x, float y, float font_size, const AColor& color, EDrawTextFlags flags, tukk label_text, ...) PRINTF_PARAMS(6, 7)
	{
		va_list args;
		va_start(args, label_text);
	...
	static void Draw2dLabel(float x, float y, float font_size, const AColor& color, bool bCenter, tukk label_text, ...) PRINTF_PARAMS(6, 7)
	{
		va_list args;
		va_start(args, label_text);
	...
	static void Draw2dText(float posX, float posY, const AColor& color, tukk pStr)
	{
		DrawText(Vec3(posX, posY, 1.f), 1, color, eDrawText_LegacyBehavior, pStr);
	}

	static void PrintToScreen(float x, float y, float size, tukk buf)
	{
	...
	static void WriteXY(i32 x, i32 y, float xscale, float yscale, float r, float g, float b, float a, tukk format, ...)
	{
		va_list args;
	...
	static void TextToScreenColor(i32 x, i32 y, float r, float g, float b, float a, tukk text)
	{
		WriteXY((i32)(8 * x), (i32)(6 * y), 1, 1, r, g, b, a, "%s", text);
	}

	static void TextToScreen(float x, float y, tukk text)
	{
		TextToScreenColor((i32)(8 * x), (i32)(6 * y), 1, 1, 1, 1, text);
	}

	static void TextToScreenF(float x, float y, tukk format, ...)
	{
		char buffer[512];
		va_list args;
		va_start(args, format);
		...
		TextToScreenColor((i32)(8 * x), (i32)(6 * y), 1, 1, 1, 1, buffer);
	}
};

//! Don't change the xxxShift values blindly as they affect the rendering output.
//! For example, 2D primitives have to be rendered after 3D primitives,
//! alpha blended geometry must be rendered after opaque ones, etc.
//! This also applies to the individual flags in EAuxGeomPublicRenderflags_*!.
//! \note Bits 0 - 22 are currently reserved for prim type and
//! per draw call render parameters (point size, etc.).
...
struct SAuxGeomRenderFlags
{
	u32 m_renderFlags = e_Def3DPublicRenderflags;

	SAuxGeomRenderFlags();
	SAuxGeomRenderFlags(const SAuxGeomRenderFlags& rhs);
	SAuxGeomRenderFlags(u32 renderFlags);
	SAuxGeomRenderFlags& operator=(const SAuxGeomRenderFlags& rhs);
	SAuxGeomRenderFlags& operator=(u32 rhs);
...
inline
SAuxGeomRenderFlags::SAuxGeomRenderFlags(u32 renderFlags)
...
inline bool
SAuxGeomRenderFlags::operator==(u32 rhs) const
{
...
{
	i32 mode2D3D((i32)(m_renderFlags & (u32)e_Mode2D3DMask));
	switch (mode2D3D)
	{
	case e_Mode2D:
		return(e_Mode2D);
...
{
	u32 alphaBlendMode(m_renderFlags & e_AlphaBlendingMask);
...
{
	u32 drawInFrontMode(m_renderFlags & e_DrawInFrontMask);
...
{
	u32 fillMode(m_renderFlags & e_FillModeMask);
	switch (fillMode)
	{
...
{
	u32 cullMode(m_renderFlags & e_CullModeMask);
	switch (cullMode)
	{
	case e_CullModeNone:
		{
...
{
	u32 depthWriteFlag(m_renderFlags & e_DepthWriteMask);
...
{
	u32 depthTestFlag(m_renderFlags & e_DepthTestMask);
...
public:
	...
	static void Draw2dImage(float xpos, float ypos, float w, float h, i32 texture_id, float s0 = 0, float t0 = 0, float s1 = 1, float t1 = 1, float angle = 0,
	...
	static void Draw2dImage(float xpos, float ypos, float w, float h, i32 texture_id, float s0, float t0, float s1,float t1, float angle,const ColorF &col)
	{
		if (gEnv->pRenderer)
		{
			SRender2DImageDescription img;
	...
	static void DrawImage(float xpos, float ypos, float w, float h, i32 texture_id, float s0, float t0, float s1, float t1, float r, float g, float b, float a, bool filtered = true)
	{
		if (gEnv->pRenderer)
		{
			SRender2DImageDescription img;
