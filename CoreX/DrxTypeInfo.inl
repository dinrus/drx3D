...
//
//	File: DrxTypeInfo.cpp
//	Описание: Implementation	of TypeInfo classes and functions.
//
//	History:
//
//////////////////////////////////////////////////////////////////////
...
char* _i64toa(int64 value, char* string, i32 radix)
{
	if (10 == radix)
...
char* ultoa(u32 value, char* string, i32 radix)
{
	if (10 == radix)
...
string ToString(unsigned long const& val)
{
  return (string) Upp::AsString(val);}
#endif
...
float  NumToFromString(float val, int digits, bool floating, char buffer[], int buf_size)
{
	...
	if (floating)
	{
		if (val >= powf(10.f, float(digits)))
			drx_sprintf(buffer, buf_size, "%.0f", val);
		else
			drx_sprintf(buffer, buf_size, "%.*g", digits, val);
	}
	else
		drx_sprintf(buffer, buf_size, "%.*f", digits, float(val));
...
struct STypeInfoTest
{
	STypeInfoTest()
	{
		...
		TestType(i16(-0x1234));
		TestType(u16(0x8765));
		TestType(i32(-0x12345678));
		TestType(u32(0x87654321));
...
void CTypeInfo::SwapEndian(void* pData, size_t nCount, bool bWriting) const
{
	...
	case 1:
		break;
	case 2:
		::SwapEndianBase((u16*)pData, nCount);
		break;
	case 4:
		::SwapEndianBase((u32*)pData, nCount);
		break;
	case 8:
	...
	if (Vars.size() > 0)
	{
		...
		for (int i = 0; i < Vars.size(); i++)
		{
			...
			if (var.bBitfield)
			{
				...
				if (bitoffset == 0)
				{
					var.Offset = check_cast<u32>(size);
...
typedef DrxStackStringT<char, 256> CTempStr;
...
void CStructInfo::SwapEndian(void* data, size_t nCount, bool bWriting) const
{
	...
	if (EndianDesc.length() == 1 && !HasBitfields && EndianDescSize(EndianDesc) == Size)
	{
		...
		case 0x40:    // Swap 2 bytes
			::SwapEndianBase((u16*)data, nElems);
			break;
		case 0x80:    // Swap 4 bytes
			::SwapEndianBase((u32*)data, nElems);
			break;
	...
	for (; nCount-- > 0; data = (char*)data + Size)
	{
		...
		for (cstr desc = EndianDesc; *desc; desc++)
		{
			...
			case 0:       // Skip bytes
				...
				break;
			case 0x40:    // Swap 2 bytes
				::SwapEndianBase((u16*)step, nElems);
				step = (u16*)step + nElems;
				break;
			case 0x80:    // Swap 4 bytes
				::SwapEndianBase((u32*)step, nElems);
				step = (u32*)step + nElems;
				break;
			case 0xC0:    // Swap 8 bytes
...
void CStructInfo::GetMemoryUsage(IDrxSizer* pSizer, void const* data) const
{
	for (int i = 0; i < Vars.size(); i++)
