...
//
//	File:Drx_Math.h
//	Описание: Misc mathematical functions
//
//	История:
...
//
#ifndef DRX_MEMORYACCESS_H
#define DRX_MEMORYACCESS_H
...
#if DRX_PLATFORM_DURANGO // no prefetching on durango, as it costs more than it gains
	#define PrefetchLine(ptr, off) (void)(0)
	#define ResetLine128(ptr, off) (void)(0)
	#define FlushLine128(ptr, off) (void)(0)
#else
	#define PrefetchLine(ptr, off) drxPrefetchT0SSE((uk )((u8*)(ptr) + off))
	#define ResetLine128(ptr, off) (void)(0)
	#define FlushLine128(ptr, off) (void)(0)
#endif
...
#define MC_CPU_TO_CPU 0x40
	...
	#if DRX_PLATFORM_X86 || DRX_PLATFORM_X64
		#include <xmmintrin.h>
	#endif

	#define _MM_PREFETCH(MemPtr, Hint)              _mm_prefetch((MemPtr), (Hint));
	#define _MM_PREFETCH_LOOP(nCount, MemPtr, Hint) { for (i32 p = 0; p < nCount; p += 64) { _mm_prefetch((tukk)(MemPtr) + p, Hint); } \
	  }
#else
...
ILINE void drxPrefetchT0SSE(ukk src)
{
#if DRX_PLATFORM_WINDOWS && DRX_PLATFORM_32BIT && !DRX_COMPILER_CLANG
	_asm
	{
		mov esi, src
...
#else
	_MM_PREFETCH((tuk)src, _MM_HINT_T0);
#endif
}
...
#if DRX_PLATFORM_X86 && !(DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID) && !DRX_PLATFORM_APPLE
// *INDENT-OFF* - do not attempt to format inline assembly!
//! Inline assembly syntax for use with Visual C++.
inline void drxMemcpy(uk Dst, ukk Src, i32 Count )
{
#if DRX_PLATFORM_SSE2
  {
	  __asm
...
inline void drxMemcpy(uk Dst, ukk Src, i32 Count, i32 nFlags)
{
	drxMemcpy(Dst, Src, Count);
}
...
i32k PREFNTA_BLOCK = 0x4000;

ILINE void drxMemcpy(uk Dst, ukk Src, i32 n)
{
	tuk dst = (tuk)Dst;
	tuk src = (tuk)Src;
...
ILINE void drxMemcpy(uk Dst, ukk Src, i32 n, i32 nFlags)
{
	tuk dst = (tuk)Dst;
	tuk src = (tuk)Src;
	while (n > PREFNTA_BLOCK)
...
ILINE void DrxPrefetch(ukk const cpSrc) {}
#else
//! Implement something usual to bring one memory location into L1 data cache.
ILINE void DrxPrefetch(ukk const cpSrc)
{
	drxPrefetchT0SSE(cpSrc);
}
#endif

#define DrxPrefetchInl DrxPrefetch
