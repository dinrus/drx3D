...
#include <drx3D/CoreX/Thread/Synchronization.h>
...
public:
	...
	static uk SysAlloc(size_t nSize) { return VirtualAlloc(0, nSize, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE); }
	static void  SysDealloc(uk ptr)  { VirtualFree(ptr, 0, MEM_DECOMMIT); }
#else
	static uk SysAlloc(size_t nSize) { return malloc(nSize); }
	static void  SysDealloc(uk ptr)  { free(ptr); }
#endif
};
...
/*---------------------------------------------------------------------------
   HeapAllocator
   Пул памяти, который может размещать произвольное количество памяти произвольного размера
   и алиновки.Кучу можно освобождать всю разом. Индивидуальная деаллокация блоков
   отсуствует.

   Usable as a base class to implement more general-purpose allocators that
   track, free, and reuse individual memory blocks.

   Дополнительно этот класс может поддерживать многопоточность, используя второй шаблонный
   параметр. Дефолтно он multithread-safe.
   Смотрите Synchronization.h.

   Детали аллокации: Поддерживает линкованный список страниц.
...
private:
	...
	struct PageNode
	{
		...
		tuk pStart() const      { return (tuk)this + nStart(); }
		tuk pEndUsed() const    { return (tuk)this + nEndUsed; }
		tuk pEndAlloc() const   { return (tuk)this + nEndAlloc; }
		tuk pEndUsable() const  { return (tuk)pGapStack(); }
		TAlign* pGapStack() const { return (TAlign*)pEndAlloc() - nGaps; }

		uk Allocate(size_t nSize, size_t nAlign, bool bStackAlloc)
		{
			// Align current mem.
			tuk pNew = Align(pEndUsed(), nAlign);
			...
			nEndUsed = (pNew + nSize) - (tuk)this;
			nUsed += nSize;
			Validate();
			return pNew;
		}

		bool CanReallocate(uk ptr, size_t nOldSize, size_t nNewSize)
		{
		...
		bool Reallocate(uk ptr, size_t nOldSize, size_t nNewSize)
		{
		...
		void Deallocate(uk ptr, size_t nOldSize)
		{
		...
		bool CheckPtr(uk ptr, size_t nSize) const
		{
			return (tuk)ptr >= pStart() && (tuk)ptr + nSize <= pEndUsed();
		}
	};
...
public:
	...
	uk Allocate(const Lock& lock, size_t nSize, size_t nAlign = 1)
	{
		for (;; )
		{
			...
			if (_pPageList)
			{
				if (uk ptr = _pPageList->Allocate(nSize, nAlign, StackAlloc()))
				{
				...
				if (!StackAlloc() && _pPageList->pNext && _pPageList->pNext->GetMemoryFree(nAlign) > _pPageList->GetMemoryFree(nAlign))
				{
					...
					if (uk ptr = _pPageList->Allocate(nSize, nAlign, false))
					{
						_TotalMem.nUsed += nSize;
						return ptr;
					}
			...
			if (!pPageNode)
			{
				...
				uk pAlloc = A::SysAlloc(nAllocSize);
	...
	bool Reallocate(const Lock& lock, uk ptr, size_t nOldSize, size_t nNewSize, size_t nAlign = 1)
	{
	...
	bool Deallocate(const Lock& lock, uk ptr, size_t nSize, size_t nAlign = 1)
	{
	...
	{
		uk pMemory = Allocate(Lock(*this), sizeof(T), nAlign ? nAlign : alignof(T));
	...
	{
		uk pMemory = Allocate(Lock(*this), sizeof(T) * nCount, nAlign ? nAlign : alignof(T));
	...
	template<typename T, typename I = i32, uint nALIGN = alignof(T)>
	...
	bool CheckPtr(const Lock&, uk ptr, size_t size) const
	{
		assert(ptr || !size);
		if (!ptr)
			return true;
...
#if bMEM_HEAP_CHECK
		static i32 nCount = 0, nInterval = 0;
		if (nCount++ >= nInterval)
		{
			nInterval++;
			nCount = 0;
