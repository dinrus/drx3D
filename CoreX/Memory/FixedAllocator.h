...
struct FixedAllocator
{
	...
	void   free_memory();
private:
	template<typename BlockIndexType = u32>
	struct Chunk
	{
		...
		inline bool contains(uk p, block_index_type maxObjCount) const
		{
	...
	MemoryChunk* find_chunk_containing(uk p);
...
template<size_t ObjSize, size_t ObjAlign, size_t ChunkObjCount>
uk FixedAllocator<ObjSize, ObjAlign, ChunkObjCount >::alloc()
{
...
template<size_t ObjSize, size_t ObjAlign, size_t ChunkObjCount>
void FixedAllocator<ObjSize, ObjAlign, ChunkObjCount >::dealloc(uk p)
{
	assert(allocChunk_);
...
FixedAllocator<ObjSize, ObjAlign, ChunkObjCount>::find_chunk_containing(uk p)
{
	if (!chunks_.empty())
	{
