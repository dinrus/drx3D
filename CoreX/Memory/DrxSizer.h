...
//
//	File:DrxSizer.h
//  Declaration and definition of the DrxSizer class, which is used to
...
#ifndef _DRX_COMMON_DRX_SIZER_INTERFACE_HDR_
#define _DRX_COMMON_DRX_SIZER_INTERFACE_HDR_
...
#include <drx3D/Eng3D/DrxPodArray.h>
...
#include <drx3D/Sys/IResourceCollector.h>       // <> required for Interfuscator. IResourceCollector

//! Flags applicable to the IDrxSizer (retrieved via getFlags() method).
enum IDrxSizerFlagsEnum
{
...
inline size_t size_of_list(const List& c)
{
	if (!c.empty())
	{
		return c.size() * sizeof(typename List::value_type) + c.size() * sizeof(uk ) * 2; // sizeof stored type + 2 pointers prev,next
	}
	return 0;
}
...
//!   use the SIZER_COMPONENT_NAME macro or DrxSizerComponentNameHelper class:.
//!
//! void X::getSize (IDrxSizer* pSizer).
//! {.
...
public:
	...
	friend class DrxSizerComponentNameHelper;
	...
	virtual bool AddObject(ukk pIdentifier, size_t nSizeBytes, i32 nCount = 1) = 0;
	...
	//! temp dummy function while checking in the DrxSizer changes, will be removed soon.
	template<typename Type>
	...
	void AddObject(const DrxStringT<wchar_t>& rString) { this->AddObject(rString.c_str(), rString.capacity()); }
	void AddObject(const DrxFixedStringT<32>&)         {}
	...
	template<typename T, i32 AlignSize>
	...
	void AddObject(const TArray<T>& rVector)
	{
		...
		for (i32 i = 0, end = rVector.size(); i < end; ++i)
		{
	...
	{
		// dummy struct to get correct element size
		struct Dummy { uk a; uk b; uk c; uk d; K k; T t; };
	...
	{
		// dummy struct to get correct element size
		struct Dummy { uk a; uk b; uk c; uk d; T t; };
...
protected:
	...
	virtual void Push(tukk szComponentName) = 0;

	//! Pushes the name that is the name of the previous component . (dot) this name.
	virtual void PushSubcomponent(tukk szSubcomponentName) = 0;
	virtual void Pop() = 0;
...
class DrxSizerComponentNameHelper
{
public:
	//! Pushes the component name on top of the name stack of the given sizer.
	DrxSizerComponentNameHelper(IDrxSizer* pSizer, tukk szComponentName, bool bSubcomponent) :
		m_pSizer(pSizer)
	{
		if (bSubcomponent)
...
#define SIZER_COMPONENT_NAME(pSizerPointer, szComponentName)    PREFAST_SUPPRESS_WARNING(6246) DrxSizerComponentNameHelper __sizerHelper(pSizerPointer, szComponentName, false)
#define SIZER_SUBCOMPONENT_NAME(pSizerPointer, szComponentName) PREFAST_SUPPRESS_WARNING(6246) DrxSizerComponentNameHelper __sizerHelper(pSizerPointer, szComponentName, true)

#endif //_DRX_COMMON_DRX_SIZER_INTERFACE_HDR_
