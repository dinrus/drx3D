...
#ifndef __CDRXPOOLINSPECTOR__
#define __CDRXPOOLINSPECTOR__
...
class CInspector : public TAllocator
{
	...
	void WriteOut(tukk pFileName, u32 Stack, tukk pFormat, ...) const
	{
		/*
		                              if(!pFileName)
		                              {
		                              ...
		                              if(File)
		                              {
		                                ...
		                                char Buffer[1024];
		                                for(u32 a=0;a<Stack;a++)
		                                  Buffer[a]=' ';
		                                va_list args;
		                                va_start(args,pFormat);
		                                drx_vsprintf(Buffer+Stack,sizeof(Buffer)-Stack,pFormat,args);
...
public:
	...
	bool LogFileName(tukk pFileName)
	{
	...
	void SaveStats(tukk pFileName) const
	{
	...
	ILINE T Allocate(size_t Size, size_t Align = 1)
	{
		...
		WriteOut(0, 0, "[A|%d|%d|%d]", (i32)pData, Size, Align);
		if (!pData)
		{
			m_FailAllocCount++;
	...
	ILINE bool Free(T pData, bool ForceBoundsCheck = false)
	{
		...
		WriteOut(0, 0, "[F|%d|%d|%d]", (i32)pData, (i32)ForceBoundsCheck, (i32)Ret);
		m_FailFreeCount += !Ret;
		return Ret;
	}
	//template<class T>
	...
	//														WriteOut(0,0,"[F|%d|%d|%d]",(i32)pData,(i32)-1,(i32)Ret);
	//														m_FailFreeCount+=!Ret;
	//														return Ret;
	//													}
	...
	ILINE bool Resize(T** pData, size_t Size, size_t Alignment)
	{
		...
		WriteOut(0, 0, "[R|%d|%d|%d]", (i32)*pData, (i32)-1, (i32)Ret);
