...
#ifndef __CDRXPOOLCONTAINER__
#define __CDRXPOOLCONTAINER__
...
public:
	ILINE CPool()
	{
		...
		{
			u8* pData = &CMemoryStatic<TElementCount* sizeof(TElement)>::Data()[a * sizeof(TElement)];
	...
	ILINE u8* Allocate(size_t Size, size_t Align = 1)
	{
		CPoolNode* pNode = m_List.PopFirst();
		return reinterpret_cast<u8*>(pNode);
	}
	...
	ILINE TElement& operator[](u32 Idx)
	{
		u8* pData = &CMemoryStatic<TElementCount* sizeof(TElement)>::Data()[Idx * sizeof(TElement)];
		return *reinterpret_cast<TElement*>(pData);
	}

	ILINE const TElement& operator[](u32 Idx) const
	{
		u8k* pData = &CMemoryStatic<TElementCount* sizeof(TElement)>::Data()[Idx * sizeof(TElement)];
...
public:
	...
	ILINE void InitMem(const size_t S = 0, u8* pData = 0)
	{
	...
	template<class T>
	ILINE T Resolve(uk rItem)  const
	{
	...
	bool InBounds(ukk pData, const bool Check)  const
	{
		return !Check || (
	...
	ILINE u8* Handle(CListItemInPlace* pItem) const
	{
		return pItem->Data();
	}
	template<class T>
...
protected:
	...
	ILINE void Stack(CListItemReference* pItem)
	{
		...
		u8* pData = pItem->Data(pNext->Align());
...
public:
	...
	ILINE CListItemReference* Split(CListItemReference* pItem, size_t Size, size_t Align)
	{
		...
		if (!(Offset & (Align - 1)))                          //perfectly aligned?
		{
			...
			m_Items.AddBefore(pItemPrev, pItem);
			pItemPrev->Data(pItem->Data());
			pItem->Data(reinterpret_cast<u8*>(Offset));
	...
	ILINE bool   Beat()          { return false; }           //!< Dummy beat in case no defragmentator is wrapping.

	ILINE size_t MemFree() const { return TMemory::MemSize() - m_Allocated; }

	ILINE size_t MemSize() const { return TMemory::MemSize(); }

	ILINE u32 Handle(CListItemReference* pItem) const
	{
		return static_cast<u32>(pItem - &m_NodePool[0]);
	}
	ILINE CListItemReference* Item(u32 ID)
	{
		return &m_NodePool[ID];
	}
	ILINE const CListItemReference* Item(u32 ID) const
	{
		return &m_NodePool[ID];
	}
