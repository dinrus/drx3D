...
#ifndef __CDRXPOOLSTLWRAPPER__
#define __CDRXPOOLSTLWRAPPER__
...
public:
	...
	pointer allocate(size_type n = 1, const_pointer hint = 0)
	{
		...
		u8* pData = pContainer->TCont::template Allocate<u8*>(n * sizeof(T), sizeof(T));
		return pContainer->TCont::template Resolve<pointer>(pData);
		//	return Container()?Container()->Allocate<uk>(n*sizeof(T),sizeof(T)):0
	}
