...
//
//---------------------------------------------------------------------------
#ifndef __CDRXPOOLLIST__
#define __CDRXPOOLLIST__
...
public:
	...
	ILINE void   InUse(u32 A) { m_Flags |= ELIF_INUSE; m_Align = A; }
	...
	ILINE u32 Align() const   { return m_Align; }
};

class CListItemInPlace : public CListItemFlagged<CListItemInPlace>
{
public:
	ILINE void Init(u8* pData, size_t Size, CListItemInPlace* pPrev, CListItemInPlace* pNext)
	{
		Prev(pPrev);
		Next(pNext);
	...
	ILINE u8*       Data()       { return reinterpret_cast<u8*>(this) + sizeof(CListItemInPlace); }
	ILINE u8k* Data() const { return reinterpret_cast<u8k*>(this) + sizeof(CListItemInPlace); }
	ILINE size_t       MemSize() const
	{
		u8k* pNext = reinterpret_cast<u8k*>(Next());
		u8k* pThis = reinterpret_cast<u8k*>(this);
...
class CListItemReference : public CListItemFlagged<CListItemReference>
{
	...
	//	size_t							m_Size;
public:
	ILINE void Init(u8* pData, size_t Size, CListItemReference* pPrev, CListItemReference* pNext)
	{
		Data(pData);
		Prev(pPrev);
		Next(pNext);
	...
	ILINE void   Data(u8* pData) { m_pData = pData; }
	ILINE u8* Data(size_t Align)
	{
		Align--;
