...
#ifndef __CDRXPOOLALLOCATOR__
#define __CDRXPOOLALLOCATOR__
...
public:
	...
	ILINE bool Reallocate(T* pData, size_t Size, size_t Alignment)
	{
		...
		if (!pNewData)
			return false;
		memcpy(TAllocator::template Resolve<u8*>(pNewData),
		       TAllocator::template Resolve<u8*>(*pData), min(TAllocator::Item(*pData)->MemSize(), Size));
