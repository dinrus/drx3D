...
{
	return reinterpret_cast<size_t>(AlignPointer<RealType>(reinterpret_cast<tuk>(address)));
}
...
struct SUninitialized
{
	...
	{
		new(static_cast<uk>(&storage))T();
	}

	void CopyConstruct(const T& value)
	{
		new(static_cast<uk>(&storage))T(value);
	}

	void MoveConstruct(T&& value)
	{
		new(static_cast<uk>(&storage))T(std::move(value));
	}
