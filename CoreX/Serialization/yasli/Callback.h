...
{
	virtual bool serializeValue(Archive& ar, tukk name, tukk value) = 0;
	virtual CallbackInterface* clone() = 0;
	virtual void release() = 0;
	virtual TypeID type() const = 0;

	typedef std::function<void(uk , const TypeID&)> ApplyFunction;
...
struct CallbackSimple : CallbackInterface
{
	...
	bool serializeValue(Archive& ar, tukk name, tukk label) { return ar(*value, name, label); }
	TypeID type() const{ return TypeID::get<T>(); }

	void call(const ApplyFunction& applyFunction)
	{
		T newValue;
		applyFunction((uk )&newValue, TypeID::get<T>());
...
struct CallbackWithDecorator: CallbackInterface
{
	...
	void release() { delete this;	}
	bool serializeValue(Archive& ar, tukk name, tukk label) { return ar(decorator(*value), name, label);	}
	...
	void call(const ApplyFunction& applyFunction)
	{
		...
		applyFunction((uk )&dec, TypeID::get<Decorator>());
		if (oldValue != newValue)
		{
			callback(newValue);
			oldValue = newValue;
...
template<class T>
bool YASLI_SERIALIZE_OVERRIDE(Archive& ar, CallbackSimple<T>& callback, tukk name, tukk label)
{
	if (ar.isEdit())
...
bool YASLI_SERIALIZE_OVERRIDE(Archive& ar, CallbackWithDecorator<T, Decorator>& callback, tukk name, tukk label)
{
	if (ar.isEdit())
		return ar(static_cast<CallbackInterface&>(callback), name, label);
	else
