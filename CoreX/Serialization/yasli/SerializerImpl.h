...
inline bool Serializer::operator()(Archive& ar, tukk name, tukk label) const{
...
inline void PointerInterface::YASLI_SERIALIZE_METHOD(Archive& ar) const
{
	...
	tukk const typePropertyName = noEmptyNames ? "type" : "";
	tukk const dataPropertyName = noEmptyNames ? "data" : "";

	TypeID baseTypeID = baseType();
	tukk oldRegisteredName = registeredTypeName();
