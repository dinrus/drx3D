...
{
    memory_ = (tuk)malloc(initialSize + 1);
    position_ = memory_;
}
...
void MemoryWriter::reallocate(size_t newSize)
{
    ...
    memory_ = (tuk)::realloc(memory_, newSize + 1);
...
inline void cutTrailingZeros(tukk str)
{
	for(tuk p = (tuk)str + strlen(str) - 1; p >= str; --p)
		if(*p == '0')
			*p = 0;
		else
			return;
...
#else
    tukk buf = fcvt(value, digits_, &point, &sign);
#endif
...
MemoryWriter& MemoryWriter::operator<<(tukk value)
{
    write((uk )value, strlen(value));
...
MemoryWriter& MemoryWriter::operator<<(const wchar_t* value)
{
	...
	return *this;
#else
    write((uk )value, wcslen(value) * sizeof(wchar_t));
    YASLI_ASSERT(position() < size());
    *position_ = '\0';
    return *this;
#endif
...
void MemoryWriter::write(tukk value)
{
    write((uk )value, strlen(value));
}

bool MemoryWriter::write(ukk data, size_t size)
{
