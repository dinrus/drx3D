...
public:
    ...
    i32 refCount() const{ return refCounter_; }

    void acquire(){ ++refCounter_; }
    i32 release(){ return --refCounter_; }
private:
    i32 refCounter_;
};
...
template<class T>
bool YASLI_SERIALIZE_OVERRIDE(Archive& ar, SharedPtr<T>& ptr, tukk name, tukk label);

template<class T>
bool YASLI_SERIALIZE_OVERRIDE(Archive& ar, PolyPtr<T>& ptr, tukk name, tukk label);

template<class T>
bool YASLI_SERIALIZE_OVERRIDE(Archive& ar, AsObjectWrapper<SharedPtr<T> >& ptr, tukk name, tukk label);
