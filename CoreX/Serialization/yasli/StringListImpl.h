...
// ---------------------------------------------------------------------------
YASLI_INLINE void splitStringList(StringList* result, tukk str, char delimeter)
{
    result->clear();
...
YASLI_INLINE bool YASLI_SERIALIZE_OVERRIDE(Archive& ar, StringList& value, tukk name, tukk label)
{
	return ar(static_cast<StringListBase&>(value), name, label);
}

YASLI_INLINE bool YASLI_SERIALIZE_OVERRIDE(Archive& ar, StringListValue& value, tukk name, tukk label)
{
    if(ar.isEdit()){
			...
			if(!ar(str, name, label))
				return false;
			if(ar.isInput()){
				i32 index = value.stringList().find(str.c_str());
				if(index >= 0){
					value = index;
				}
			}
...
YASLI_INLINE bool YASLI_SERIALIZE_OVERRIDE(Archive& ar, StringListStaticValue& value, tukk name, tukk label)
{
    if(ar.isEdit()) {
			Serializer ser = Serializer::forEdit(value);
			return ar(ser, name, label);
