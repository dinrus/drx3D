...
public:
	...
	uk pointer() const{ return reinterpret_cast<uk>(container_); }
	...
	void move(i32 index)
	{
 	...
 	template<typename T, typename std::enable_if<std::is_copy_assignable<T>::value, i32>::type = 0>
	void moveImpl(i32 index)
	{
	...
	uk elementPointer() const{ return &*it_; }
  size_t elementSize() const { return sizeof(typename Container::value_type); }

	bool operator()(Archive& ar, tukk name, tukk label){
	...
	void serializeNewElement(Archive& ar, tukk name = "", tukk label = 0) const{
...
public:
	...
	tukk registeredTypeName() const{
		if(ptr_)
			return ClassFactory<T>::the().getRegisteredTypeName(ptr_.get());
		else
			return "";
	}
	void create(tukk typeName) const{
	...
	uk get() const{
		return reinterpret_cast<uk>(ptr_.get());
	}
	ukk handle() const { return &ptr_; }
...
inline bool YASLI_SERIALIZE_OVERRIDE(yasli::Archive& ar, yasli::string& value, tukk name, tukk label)
{
...
inline bool YASLI_SERIALIZE_OVERRIDE(yasli::Archive& ar, yasli::wstring& value, tukk name, tukk label)
{
...
{
	tukk get() const { return pair_.first.c_str(); }
	void set(tukk key) { pair_.first.assign(key); }
	ukk handle() const { return &pair_; }
	yasli::TypeID type() const { return yasli::TypeID::get<string>(); }
	bool serializeValue(yasli::Archive& ar, tukk name, tukk label)
	{
...
#if YASLI_CXX11
template<class T>
bool YASLI_SERIALIZE_OVERRIDE(yasli::Archive& ar, std::shared_ptr<T>& ptr, tukk name, tukk label)
{
...
bool YASLI_SERIALIZE_OVERRIDE(yasli::Archive& ar, std::unique_ptr<T, Del>& ptr, tukk name, tukk label)
{
