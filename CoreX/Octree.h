 ...
 * This should be a good starting point for your own implementation :
   ...
   void Split(SVec3Container* (&octantContainers)[OctreeUtils::eOctant_Count], const AABB (&octantAABBs)[OctreeUtils::eOctant_Count],const Vec3& reference, uk pUserData);
   bool ShouldSplit(Vec3& outSplitReference, const size_t nodeDepth, const AABB& domain, uk pUserData) const;

   TPoints content;
   };

   void SVec3Container::Split(SVec3Container* (&octantContainers)[OctreeUtils::eOctant_Count], const AABB (&octantsBoxes)[OctreeUtils::eOctant_Count], const Vec3& reference, uk pUserData)
   {
   (void)octantsBoxes;
   (void)pUserData;
   for (TPoints::const_iterator it = content.begin(), itEnd = content.end(); it != itEnd; ++it)
   {
    ...
    i32k octant = ((diff.x >= 0.0f) ? 1 : 0) | ((diff.y >= 0.0f) ? 2 : 0) | ((diff.z >= 0.0f) ? 4 : 0);
    octantContainers[octant]->content.push_back(*it);
   }
   }

   bool SVec3Container::ShouldSplit(Vec3& outSplitReference, const size_t nodeDepth, const AABB& domain, uk pUserData) const
   {
   (void)nodeDepth;
   (void)pUserData;
...
void COctree<TPrimitiveContainer, TUserData>::Node::DestroyChildren()
{
	if (pChildren)
	{
		for (i32 i = 0; i < OctreeUtils::eOctant_Count; ++i)
		{
			pChildren[i].~Node();
		}
		free(pChildren);
...
{
	DRX_ASSERT_MESSAGE(IsLeaf(), "Cannot split a non-leaf node from the octree.");
...
{
	DRX_ASSERT_MESSAGE(m_root.IsLeaf(), "Octree has already been split into leaves. PushInitialPrimitive() shouldn't be called anymore on it");
	if (m_root.IsLeaf())
	{
...
void COctree<TPrimitiveContainer, TUserData >::EvaluateSplitting(const SLeafInfo leaf)
{
	...
	if (leaf.pNode->ShouldSplit(splittingReference, leaf.depth, m_pUserData))
	{
		...
		i32 childrenDepth = leaf.depth + 1;
		for (i32 i = 0; i < OctreeUtils::eOctant_Count; ++i)
		{
