...
struct SpawnParams
{
	...
	i32                      nSeed           = -1;                               //!< Initial seed. Default is -1 which means random seed.
	...
	DrxAudio::EOcclusionType occlusionType   = DrxAudio::EOcclusionType::Ignore; //!< Audio obstruction/occlusion calculation type.
...
struct IParticleAttributes
{
	...
	typedef DrxVariant<bool, i32, float, ColorB> TVariant;
	...
	i32          GetAsInteger(TAttributeId id, i32 defaultValue = 0) const              { return GetValueAs(id, defaultValue); }
	...
	void         SetAsInteger(TAttributeId id, i32 value)  { SetValue(id, value); }
...
struct IParticleEffect : public _i_reference_target_t
{
	...
	virtual IParticleEffect* GetChild(i32 index) const = 0;
	...
	virtual void InsertChild(i32 slot, IParticleEffect* pEffect) = 0;
	...
	virtual i32 FindChild(IParticleEffect* pEffect) const = 0;
...
struct IParticleEmitter : public IRenderNode, public CMultiThreadRefCount
{
	...
	virtual void SetEntity(IEntity* pEntity, i32 nSlot) = 0;
	...
	virtual u32 GetAttachedEntityId() = 0;

	//! Get the Entity Slot that this particle emitter is attached to.
	virtual i32 GetAttachedEntitySlot() = 0;
...
struct IParticleUpr
{
	...
	virtual const ParticleParams& GetDefaultParams(i32 nVersion = 0) const = 0;
	...
	virtual void AddVertexIndexPoolUsageEntry(u32 nVertexMemory, u32 nIndexMemory, tukk pContainerName) = 0;
...
public:
	...
	~CParticleLightProfileSection()
	{
		...
		IF (pPartMan != NULL, 1)
		{
			pPartMan->GetLightProfileCounts().AddFrameTicks(DrxGetTicks() - m_nTicks);
		}
	}
private:
	uint64 m_nTicks;
...
public:
	...
	~CParticleLightProfileSectionSyncTime()
	{
		...
		IF (pPartMan != NULL, 1)
		{
			pPartMan->GetLightProfileCounts().AddFrameSyncTicks(DrxGetTicks() - m_nTicks);
		}
	}
private:
	uint64 m_nTicks;
