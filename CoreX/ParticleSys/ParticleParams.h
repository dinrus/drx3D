...
#include <drx3D/CoreX/Math/FinalizingSpline.h>
...
struct ETrinary : ETrinaryNames
{
	...
	ETrinary(ETrinaryNames::E e = Both)
		: ETrinaryNames(e) {}
	ETrinary(bool b)
		: ETrinaryNames(ETrinaryNames::E(1 << u8(b))) {}

	//! Test against ETrinary, or directly against bool.
	u8 operator*(ETrinary t) const
	{
		return +*this & + t;
	}
	u8 operator*(bool b) const
	{
		return +*this & (1 << u8(b));
	}
};
...
{
public:
	//! Initialize with an i32.
	explicit inline CChaosKey(u32 uSeed)
		: m_Key(uSeed) {}

	explicit inline CChaosKey(float fSeed)
		: m_Key((u32)(fSeed * float(0xFFFFFFFF))) {}

	explicit inline CChaosKey(void const* ptr)
		: m_Key(u32(size_t(ptr))) {}
	...
	inline u32 operator*(u32 nRange) const
	{
		return m_Key % nRange;
	}
	inline u16 operator*(u16 nRange) const
	{
		return m_Key % nRange;
	}
...
private:
	...
	static inline u32 Jumble(u32 key)
	{
		key += ~rot_left(key, 15);
	...
	static inline u32 rot_left(u32 u, i32 n)
	{
		return (u << n) | (u >> (32 - n));
	}
	static inline u32 rot_right(u32 u, i32 n)
	{
		return (u >> n) | (u << (32 - n));
	}
};
...
typedef TSmall<bool, u8, 0, 1>     TSmallBoolTrue;
typedef TSmall<uint, u8, 1>        PosInt8;
...
{
	return drx_random(0, chance.GetMaxStore() - 1) < chance.GetStore();
}
...
{
	static CTypeInfo Info("ISplineInterpolator*", sizeof(uk ), alignof(uk ));
	return Info;
}
...
struct ParticleParams
{
	...
	CDrxName sTexture;                              //!< Texture asset for sprite.
	CDrxName sMaterial;                             //!< Material (overrides texture).

	struct STextureTiling
	{
		PosInt8 nTilesX, nTilesY;                   //!< Number of tiles texture is split into.
		u8   nFirstTile = 0;                     //!< First (or only) tile to use.
	...
	CDrxName sGeometry;                             //!< Geometry for 3D particles.
	...
	// <Group=Audio>
	CDrxName           sStartTrigger;               //!< Audio start trigger to execute.
	CDrxName           sStopTrigger;                //!< Audio stop trigger to execute.
	...
	{
		u8 nTailSteps = 0;                       //!< <SoftMax=16> Number of tail segments.
		AUTO_STRUCT_INFO;
	...
	u8      nMaxCollisionEvents = 0;             //!< Max # collision events per particle (0 = no limit).
	...
	TFixed<u8, MAX_HEATSCALE> fHeatScale;                  //!< Multiplier to thermal vision.
	...
	TRangedType<u8, 0, 2>     nSortQuality;                //!< Sort new particles as accurately as possible into list, by main camera distance.
