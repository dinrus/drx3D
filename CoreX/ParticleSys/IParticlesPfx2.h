...
struct IParticleFeature : public _i_reference_target_t
{
	...
	virtual tukk                   GetConnectorName(uint connectorId) const = 0;
	virtual void                          ConnectTo(tukk pOtherName) = 0;
	virtual void                          DisconnectFrom(tukk pOtherName) = 0;
	virtual uint                          GetNumResources() const = 0;
	virtual tukk                   GetResourceName(uint resourceId) const = 0;
};
...
struct IParticleSystem : public IDrxUnknown
{
	DRXINTERFACE_DECLARE_GUID(IParticleSystem, "cc3aa21d-44a2-4ded-9aa1-daded21d570a"_drx_guid);
...
static std::shared_ptr<IParticleSystem> GetIParticleSystem()
{
	...
	static bool created = DrxCreateClassInstanceForInterface(drxiidof<IParticleSystem>(), pParticleSystem);
