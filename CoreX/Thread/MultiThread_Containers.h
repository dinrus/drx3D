...
// -------------------------------------------------------------------------
//  Имя файла:   MultiThread_Containers.h
//  Version:     v1.00
...
public:
	...
	DrxCriticalSection& get_lock() const { return m_cs; }

	bool                empty() const    { AutoLock lock(m_cs); return v.empty(); }
	i32                 size() const     { AutoLock lock(m_cs); return v.size(); }
...
public:
	...
	bool     empty() const                { AutoLock lock(m_cs); return v.empty(); }
	i32      size() const                 { AutoLock lock(m_cs); return v.size(); }
	void     resize(i32 sz)               { AutoLock lock(m_cs); v.resize(sz); }
	void     reserve(i32 sz)              { AutoLock lock(m_cs); v.reserve(sz); }
...
public:
	...
	DrxCriticalSection& get_lock()                           { return m_cs; }
...
private:
	typedef typename std::allocator<T*> Alloc_rebind;
...
//! Realized with only  variables and memory barriers.
...
template<typename T>
class DRX_ALIGN(128) SingleProducerSingleConsumerQueue: public DrxMT::detail::SingleProducerSingleConsumerQueueBase
{
public:
...
private:
	...
	DRX_ALIGN(16)  u32 m_nProducerIndex;
	DRX_ALIGN(64)  u32 m_nComsumerIndex;
};
...
{
	DrxModuleMemalignFree(m_arrBuffer);
	m_nBufferSize = 0;
}
...
inline void SingleProducerSingleConsumerQueue<T >::Init(size_t nSize)
{
	...
	m_arrBuffer = alias_cast<T*>(DrxModuleMemalign(nSize * sizeof(T), 16));
	m_nBufferSize = nSize;
}
...
inline void SingleProducerSingleConsumerQueue<T >::Push(const T& rObj)
{
	assert(m_arrBuffer != NULL);
	assert(m_nBufferSize != 0);
	SingleProducerSingleConsumerQueueBase::Push((uk )&rObj, m_nProducerIndex, m_nComsumerIndex, m_nBufferSize, m_arrBuffer, sizeof(T));
}
...
template<typename T>
class DRX_ALIGN(128) N_ProducerSingleConsumerQueue: public DrxMT::detail::N_ProducerSingleConsumerQueueBase
{
public:
...
private:
	...
	u32 m_nBufferSize;
	 ...
	 u32 m_nProducerIndex;
	 u32 m_nComsumerIndex;
	 u32 m_nRunning;
	 u32 m_nProducerCount;
};
...
{
	DrxModuleMemalignFree(m_arrBuffer);
	DrxModuleMemalignFree((uk )m_arrStates);
	m_nBufferSize = 0;
}
...
inline void N_ProducerSingleConsumerQueue<T >::Init(size_t nSize)
{
	...
	m_arrBuffer = alias_cast<T*>(DrxModuleMemalign(nSize * sizeof(T), 16));
	m_arrStates = alias_cast<u32*>(DrxModuleMemalign(nSize * sizeof(u32), 16));
	memset((uk )m_arrStates, 0, sizeof(u32) * nSize);
	m_nBufferSize = nSize;
}
...
inline void N_ProducerSingleConsumerQueue<T >::AddProducer()
{
	...
	if (m_nRunning == 0)
		__debugbreak();
#endif
	DrxInterlockedIncrement(( i32*)&m_nProducerCount);
}
...
inline void N_ProducerSingleConsumerQueue<T >::RemoveProducer()
{
	...
	if (m_nRunning == 0)
		__debugbreak();
#endif
	if (DrxInterlockedDecrement(( i32*)&m_nProducerCount) == 0)
		m_nRunning = 0;
}
...
inline void N_ProducerSingleConsumerQueue<T >::Push(const T& rObj)
{
	...
	assert(m_arrStates != NULL);
	assert(m_nBufferSize != 0);
	DrxMT::detail::N_ProducerSingleConsumerQueueBase::Push((uk )&rObj, m_nProducerIndex, m_nComsumerIndex, m_nRunning, m_arrBuffer, m_nBufferSize, sizeof(T), m_arrStates);
}
...
inline bool N_ProducerSingleConsumerQueue<T >::Pop(T* pResult)
{
	...
	assert(m_arrStates != NULL);
	assert(m_nBufferSize != 0);
	return DrxMT::detail::N_ProducerSingleConsumerQueueBase::Pop(pResult, m_nProducerIndex, m_nComsumerIndex, m_nRunning, m_arrBuffer, m_nBufferSize, sizeof(T), m_arrStates);
}
};

namespace stl
{
template<typename T> void free_container(DrxMT::vector<T>& v)
{
	v.free_memory();
}
template<typename T> void free_container(DrxMT::queue<T>& v)
{
	v.free_memory();
}
}
