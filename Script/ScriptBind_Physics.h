...
// -------------------------------------------------------------------------
//  Имя файла:   ScriptBind_Physics.h
//  Version:     v1.00
...
public:
	...
	i32 SimulateExplosion(IFunctionHandler* pH, SmartScriptTable explosionParams);
	...
	i32 RegisterExplosionShape(IFunctionHandler* pH, tukk sGeometryFile, float fSize, i32 nIdMaterial, float fProbability,
	                           tukk sSplintersFile, float fSplintersOffset, tukk sSplintersCloudEffect);

	//! <code>Physics.RegisterExplosionCrack( sGeometryFile, i32 nMaterialId )</code>
	...
	i32 RegisterExplosionCrack(IFunctionHandler* pH, tukk sGeometryFile, i32 nIdMaterial);
	...
	i32 RayTraceCheck(IFunctionHandler* pH, Vec3 src, Vec3 dst, ScriptHandle skipEntityId1, ScriptHandle skipEntityId2);
	...
	i32 SamplePhysEnvironment(IFunctionHandler* pH);
