...
*/
static i32 iscleared (global_State *g, const GCObject *o) {
...
*/
GCObject *luaC_newobjdt (lua_State *L, i32 tt, size_t sz, size_t offset) {
  global_State *g = G(L);
...
GCObject *luaC_newobj (lua_State *L, i32 tt, size_t sz) {
...
*/
static i32 remarkupvals (global_State *g) {
  lua_State *thread;
  lua_State **p = &g->twups;
  i32 work = 0;  /* estimate of how much work was done here */
  while ((thread = *p) != NULL) {
    work++;
...
** by 'genlink'.
*/
static i32 traverseephemeron (global_State *g, Table *h, i32 inv) {
  i32 marked = 0;  /* true if an object is marked in this traversal */
  i32 hasclears = 0;  /* true if table has white keys */
  i32 hasww = 0;  /* true if table has entry "white-key -> white-value" */
  u32 i;
  u32 asize = luaH_realasize(h);
  u32 nsize = sizenode(h);
  /* traverse array part */
  for (i = 0; i < asize; i++) {
...
static i32 traverseudata (global_State *g, Udata *u) {
  i32 i;
  markobjectN(g, u->metatable);  /* mark its metatable */
...
*/
static i32 traverseproto (global_State *g, Proto *f) {
  i32 i;
...
static i32 traverseCclosure (global_State *g, CClosure *cl) {
  i32 i;
...
*/
static i32 traverseLclosure (global_State *g, LClosure *cl) {
  i32 i;
...
*/
static i32 traversethread (global_State *g, lua_State *th) {
  UpVal *uv;
  StkId o = th->stack.p;
...
*/
static GCObject **sweeplist (lua_State *L, GCObject **p, i32 countin,
                             i32 *countout) {
  global_State *g = G(L);
  i32 ow = otherwhite(g);
  i32 i;
  i32 white = luaC_white(g);  /* current white */
  for (i = 0; *p != NULL && i < countin; i++) {
    GCObject *curr = *p;
    i32 marked = curr->marked;
...
static void dothecall (lua_State *L, uk ud) {
  UNUSED(ud);
...
*/
static i32 runafewfinalizers (lua_State *L, i32 n) {
  global_State *g = G(L);
  i32 i;
...
*/
static void separatetobefnz (global_State *g, i32 all) {
  GCObject *curr;
  GCObject **p = &g->finobj;
...
static void setpause (global_State *g) {
  ...
  i32 pause = getgcparam(g->gcpause);
    ...
    if (iswhite(curr)) {  /* is 'curr' dead? */
      ...
      *p = curr->next;  /* remove 'curr' from list */
      ...
      if (getage(curr) == G_NEW) {  /* new objects go back to white */
        i32 marked = curr->marked & ~maskgcbits;  /* erase GC bits */
...
*/
void luaC_changemode (lua_State *L, i32 newmode) {
  global_State *g = G(L);
  if (newmode != g->gckind) {
...
static i32 sweepstep (lua_State *L, global_State *g,
                      i32 nextstate, GCObject **nextlist) {
  if (g->sweepgc) {
    l_mem olddebt = g->GCdebt;
    i32 count;
...
** by 'statemask'
*/
void luaC_runtilstate (lua_State *L, i32 statesmask) {
  global_State *g = G(L);
...
static void incstep (lua_State *L, global_State *g) {
  i32 stepmul = (getgcparam(g->gcstepmul) | 1);  /* avoid division by 0 */
...
*/
void luaC_fullgc (lua_State *L, i32 isemergency) {
  global_State *g = G(L);
  lua_assert(!g->gcemergency);
