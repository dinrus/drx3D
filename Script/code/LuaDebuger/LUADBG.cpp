...
#include <drx3D/Script/StdAfx.h>
	...
	#include <drx3D/Script/LUADBG.h>
	#include <drx3D/Script/_TinyRegistry.h>
	#include <drx3D/Script/_TinyFileEnum.h>
	#include <drx3D/Script/_TinyBrowseFolder.h>
	#include <drx3D/Script/AboutWnd.h>
	#include <drx3D/Script/GoToWnd.h>
	#include <drx3D/Script/GoToFuncWnd.h>
	...
	#include <direct.h>
		...
		#include <drx3D/Script/Shlwapi.h>
...
LRESULT CSourceEdit::OnPaint(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	i32 iFirstLine = m_wEditWindow.GetFirstVisibleLine() + 1;
	TEXTMETRIC tm;
	m_wEditWindow.GetTextMetrics(&tm);

	i32k iFontY = tm.tmHeight;
	i32k iFontHalf = iFontY / 2;
...
LRESULT CSourceEdit::OnRButtonDown(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	i32k xPos = _TINY_GET_X_LPARAM(lParam);
	i32k yPos = _TINY_GET_Y_LPARAM(lParam);

	if (xPos > 0 && xPos < BORDER_SIZE)
	{
		_TinyRect rect;
		GetClientRect(&rect);
		i32k h = rect.bottom - rect.top;
		...
		if (yPos > 0 && yPos < h)
		{
			...
			i32k fontY = tm.tmHeight;
			i32k firstLine = m_wEditWindow.GetFirstVisibleLine() + 1;
			i32k clickedLine = (yPos / fontY) + firstLine;
...
CLUADbg::CLUADbg(CScriptSystem* pScriptSystem)
{
	...
	_Tiny_InitApp((HINSTANCE)DrxGetCurrentModule(), (HINSTANCE) gDLLHandle, NULL, NULL, IDI_SMALL);
...
inline void MyPathAddBackslash(tuk szPath)
{
	if (szPath[0] == 0)
		return;
...
LRESULT CLUADbg::OnCreate(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	tukk szFolder = m_pIPak->GetAlias("SCRIPTS");
	if (szFolder)
		sScriptsFolder = szFolder + string("/");
	else
		sScriptsFolder = "SCRIPTS/";
	m_wFilesTree.ScanFiles((tuk)sScriptsFolder.c_str());

	string sUIFolder = "LIBS/UI/UIACTIONS/";
	m_wFilesTree.ScanFiles((tuk)sUIFolder.c_str());
	string sItemsFolder = "ITEMS/";
	m_wFilesTree.ScanFiles((tuk)sItemsFolder.c_str());
...
void CLUADbg::StrSuperCat(tuk* dest, tukk source, UINT& iFormattedLength, UINT& iDestBufPos, size_t n)
{
	i32 length = (n == -1 ? strlen(source) : n);
	while (iDestBufPos + length >= iFormattedLength)
	{
		...
		// Reallocate buffer
		iFormattedLength *= 2;
		tuk temp = new char[iFormattedLength];
		strcpy(temp, *dest);
		delete[] (*dest);
		*dest = temp;
	}
...
bool CLUADbg::LoadFile(tukk pszFile, bool bForceReload)
{
	...
	// Now it could easily be refactored again to just use DrxString, which concatenates efficiently

	FILE* hFile = NULL;
	tuk pszScript = NULL, * pszFormattedScript = NULL;
	...
	// set filename in window title
	{
		...
		drx_sprintf(str, "Lua Debugger [%s]", pszFile);
		SetWindowText(str);
	}
	...
	while (pszScript[iSrcBufPos] != '\0')
	{
		...
		bIsKeyWord = false;
		for (iCurKWrd = 0; iCurKWrd < DRX_ARRAY_COUNT(szKeywords); iCurKWrd++)
		{
...
{
	tukk source = m_wScriptWindow.GetSourceFile();
	i32 line = m_wScriptWindow.GetCurLine();
...
bool CLUADbg::Reshape(i32 w, i32 h)
{
	/*
	   i32 nWatchHeight=(((float)h)*WATCH_HEIGHT_MULTIPLIER);
	   i32 nFilesWidth=(((float)h)*FILES_WIDTH_MULTIPLIER);
...
LRESULT CLUADbg::UpdateCheckboxes(HWND hWnd)
{
	...
	i32 success = CheckMenuRadioItem(GetMenu(m_hWnd), ID_DEBUG_DISABLE, ID_DEBUG_ENABLE, modeItem, MF_BYCOMMAND);
	if (!success)
		_TINY_CHECK_LAST_ERROR;
	return 1;
}
...
//////////////////////////////////////////////////////////////////////////
void CLUADbg::SetStatusBarText(tukk pszText)
{
...
//////////////////////////////////////////////////////////////////////////
string CLUADbg::GetLineFromFile(tukk sFilename, i32 nLine)
{
	CDrxFile file;
...
void CLUADbg::GetStackAndLocals()
{
	...
	///////////
	{
		...
		if (!callstack.empty())
		{
			for (i32 i = callstack.size() - 1; i >= 0; i--)
			{
		...
		if (m_bCppCallstackEnabled)
		{
			...
			if (nFuncCount > 0)
			{
				...
				iItem = m_wCallstack.InsertItem(iItem + 1, "", 0);
				i32 nIndex = 1;
				for (i32 i = 8; i < nFuncCount; i++) // Skip some irrelevant functions
				{
					tukk s = funcs[i];
					// Skip all Lua functions (written in C)
					if (strstr(s, ".c:") == 0)
					{
						i32 thisItem = m_wCallstack.InsertItem(iItem + nIndex, funcs[i], 0);
...
void CLUADbg::ShowLocalsForFrame(i32 frame)
{
...
HTREEITEM CLUADbg::AddVariableToTree(tukk sName, ScriptVarType type, HTREEITEM hParent)
{
	char szBuf[2048];
	char szType[32];
	...
	{
	case svtNull:
		drx_strcpy(szType, "[nil]");
		break;
	case svtString:
		drx_strcpy(szType, "[string]");
		break;
	case svtNumber:
		drx_strcpy(szType, "[numeric]");
		break;
	case svtFunction:
		drx_strcpy(szType, "[function]");
		break;
	case svtObject:
		drx_strcpy(szType, "[table]");
		break;
	case svtUserData:
	case svtPointer:
		drx_strcpy(szType, "[user data]");
		break;
	case svtBool:
		drx_strcpy(szType, "[bool]");
		break;
	default:
		drx_strcpy(szType, "[unknown]");
		break;
	}
	...
	if (type == svtString || type == svtNumber || type == svtBool || type == svtPointer || type == svtUserData)
	{
		...
		if (sName[0] == '[')
		{
			...
			else if (type == svtBool)
			{
				...
				if (bValue)
					drx_strcpy(temp, "true");
				else
					drx_strcpy(temp, "false");
			}
			else if (type == svtPointer)
			{
				ScriptHandle ptr(0);
				bRetrieved = m_pIVariable->GetAt(iIdx, ptr);
				drx_sprintf(temp, "0x%016p", ptr.ptr);
			}
		}
		else
		{
		...
		{
			if (type == svtString)
				drx_sprintf(szBuf, "%s %s = \"%s\"", szType, sName, szContent);
			else
				drx_sprintf(szBuf, "%s %s = %s", szType, sName, szContent);
		}
		else
			drx_sprintf(szBuf, "%s %s = (Unknown)", szType, sName);
	}
	else
		drx_sprintf(szBuf, "%s %s", szType, sName);
...
void CLUADbg::OnElementFound(tukk sName, ScriptVarType type)
{
...
void CLUADbg::OnElementFound(i32 nIdx, ScriptVarType type)
{
	char szBuf[32];
	drx_sprintf(szBuf, "[%i]", nIdx);
...
//////////////////////////////////////////////////////////////////////////
bool CLUADbg::InvokeDebugger(tukk pszSourceFile, i32 iLine, tukk pszReason)
{
	...
	if (pszSourceFile && pszSourceFile[0] == '@')
	{
		...
		if (!strcmp(pszReason, "StepOver") || !strcmp(pszReason, "StepInt"))
		{
			...
			i32 lastVisibleLine = m_wScriptWindow.GetLastVisibleLine();
			static i32 previousLine = 0;
	...
	{
		tukk file = m_wScriptWindow.GetSourceFile();
		if (*file == '@')
	...
	return (i32) msg.wParam == 0;

	return true;
}

//////////////////////////////////////////////////////////////////////////
bool CLUADbg::HaveBreakPointAt(tukk sSourceFile, i32 nLine)
{
...
bool CLUADbg::IsTracepoint(tukk sSourceFile, i32 nLine)
{
	i32 nCount = m_breakPoints.size();
	for (i32 i = 0; i < nCount; i++)
	{
...
void CLUADbg::ToggleTracepoint(tukk sSourceFile, i32 nLine)
{
	i32 nCount = m_breakPoints.size();
	for (i32 i = 0; i < nCount; i++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CLUADbg::AddBreakPoint(tukk sSourceFile, i32 nLine, bool bTracepoint, bool bSave)
{
...
//////////////////////////////////////////////////////////////////////////
void CLUADbg::RemoveBreakPoint(tukk sSourceFile, i32 nLine)
{
	i32 nCount = m_breakPoints.size();
	for (i32 i = 0; i < nCount; i++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CLUADbg::SaveBreakpoints(tukk filename)
{
...
//////////////////////////////////////////////////////////////////////////
void CLUADbg::LoadBreakpoints(tukk filename)
{
	...
	if (file)
	{
		...
		while (!feof(file))
		{
			i32k result = fscanf(file, "%d:%d:%2047s", &nLine, &tracepoint, str);
			if (result == 3)
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_SCRIPTSYSTEM, VALIDATOR_WARNING, "Script: %s for breakpoint at line %d doesn't exist. Skipped adding breakpoint.", str, nLine);
				}
			}
			else if (result != EOF)
			{
				DrxWarning(VALIDATOR_MODULE_SCRIPTSYSTEM, VALIDATOR_WARNING, "Malformed breakpoint information at line %d. Skipping line. Information read: %d:%s", fileLine, nLine, str);
		...
		default:
			{
				...
				{
					DrxLogAlways("$2Lua Tracepoint: %s - Line %d: %s", ar->short_src, ar->currentline, GetLineFromFile(ar->short_src, ar->currentline).c_str());
				}
			}
		}
	}
