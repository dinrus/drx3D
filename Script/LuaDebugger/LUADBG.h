...
public:
	...
	{
		tuk** pppText = reinterpret_cast<tuk**>(dwCookie);
		tuk* ppText = *pppText;
	...
	friend DWORD CALLBACK EditStreamCallbackOut(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG* pcb)
	{
		...
		str->append((tukk)pbBuff, cb);
		*pcb = cb;
		return 0;
	};

	void SetSourceFile(tukk pszFileName)
	{
		m_strSourceFile = pszFileName;
	};

	tukk GetSourceFile() { return m_strSourceFile.c_str(); };
	...
	i32 GetFirstVisibleLine()
	{
		return m_wEditWindow.GetFirstVisibleLine();
	}

	i32 GetLastVisibleLine()
	{
...
public:
	...
	void   Break();
	bool   InvokeDebugger(tukk sSourceFile, i32 nLine, tukk pszReason);
	bool   LoadFile(tukk pszFile, bool bForceReload = false);
	void   PlaceLineMarker(UINT iLine);
	void   SetStatusBarText(tukk pszText);
	...
	void         AddBreakPoint(tukk sSourceFile, i32 nLine, bool bTracepoint = false, bool bSave = true);
	bool         IsTracepoint(tukk sSourceFile, i32 nLine);
	void         ToggleTracepoint(tukk sSourceFile, i32 nLine);
	...
	void              SaveBreakpoints(tukk filename);
	void              LoadBreakpoints(tukk filename);
	...
	void   OnElementFound(tukk sName, ScriptVarType type);
	void   OnElementFound(i32 nIdx, ScriptVarType type);
...
protected:
	...
	LRESULT OnNotify(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		if (n->code == NM_DBLCLK || n->code == NM_RETURN)
		{
			...
			{
				tukk pszFileName = m_wFilesTree.GetCurItemFileName();
				if (!pszFileName)
					return 1;
			...
			{
				char temp[512];
				i32 sel = m_wCallstack.GetSelection();
				if (sel != -1)
				{
					m_wCallstack.GetItemText(sel, 1, temp, sizeof(temp));
					i32 linenum = atoi(temp);
					if (linenum != -1)
					{
	...
	HTREEITEM AddVariableToTree(tukk sName, ScriptVarType type, HTREEITEM hParent = NULL);
	void      DumpTable(HTREEITEM hParent, IScriptTable* pITable, UINT& iRecursionLevel);
	void      SetFocusToEditControl();
	void      ShowLocalsForFrame(i32 frame);
	...
	void StrSuperCat(tuk* dest, tukk source, UINT& iFormattedLength, UINT& iDestBufPos, size_t n = -1);
