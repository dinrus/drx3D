...
public:
	...
	static i32 CALLBACK SortItems(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
	{
		CFileTree* parent = static_cast<CFileTree*>((CFileTree*)lParamSort);
		i32 diff = (parent->m_isDir[lParam2] ? 1 : 0) - (parent->m_isDir[lParam1] ? 1 : 0);
		if (diff)
			return diff;
		else
	...
	void ScanFiles(tuk pszRootPath)
	{
...
protected:
	...
	i32 EnumerateScripts(HTREEITEM hRoot, tuk pszEnumPath)
	{
		i32 num_added = 0;
		_finddata_t c_file;
		...
		if ((hFile = gEnv->pDrxPak->FindFirst((string(pszEnumPath) + "/*.*").c_str(), &c_file)) == -1L)
			return 0;
		...
		while (gEnv->pDrxPak->FindNext(hFile, &c_file) == 0)
		{
			...
			drx_sprintf(szFullFilePath, "%s%s", pszEnumPath, c_file.name);
			...
			if (c_file.attrib & _A_SUBDIR)
			{
				...
				drx_sprintf(szPath, "%s%s/", pszEnumPath, c_file.name);
