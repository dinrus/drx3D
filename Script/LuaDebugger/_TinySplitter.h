...
public:
	...
	bool Reshape(i32 w, i32 h)
	{
		if (m_pPan0 == NULL || m_pPan1 == NULL)
			return false;
		i32k iHlfSplitterWdh = SPLITTER_WIDTH / 2;
		SetWindowPos(0, 0, w, h, SWP_NOZORDER | SWP_NOMOVE);

		if (m_bVertical)
		{
			i32 iEndUpper = ReverseYAxis(m_iSplitterPos) - iHlfSplitterWdh;
		...
		{
			i32 iEndLeft = m_iSplitterPos - iHlfSplitterWdh;
...
protected:
	...
	LRESULT OnMouseMove(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		if (m_bDragging)
		{
			i32 xPos = _TINY_GET_X_LPARAM(lParam);
			i32 yPos = _TINY_GET_Y_LPARAM(lParam);
