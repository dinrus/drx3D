...
bool CMouse::Init()
{
	...
	if (FAILED(hr))
	{
		DrxFatalError("CMouse::Init SetProperty DIPROP_AXISMODE failed.");
		return false;
	}
		...
		while (nElements > 0)
		{
			...
			if (nElements > 0)
			{
				for (i32 i = 0; i < (i32)nElements; i++)
				{
					...
					{
					case DIMOFS_X:
						m_deltas.x += float((i32)Events[i].dwData);
						break;
					case DIMOFS_Y:
						m_deltas.y += float((i32)Events[i].dwData);
						break;
					case DIMOFS_Z:
						m_mouseWheel += float((i32)Events[i].dwData);
						break;
					}
				}
			}
