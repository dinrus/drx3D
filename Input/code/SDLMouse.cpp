...
void CSDLMouse::Update(bool focus)
{
	...
	for (i32 i = 0; i < nEvents; ++i)
	{
		...
		if (type == SDL_MOUSEBUTTONDOWN || type == SDL_MOUSEBUTTONUP)
		{
			...
			pSymbol = DevSpecIdToSymbol((u32)MOUSE_SYM(buttonEvent->button));
			if (pSymbol == NULL)
				continue;
		...
		else if (type == SDL_MOUSEMOTION)
		{
			...
			//newX = (i32)rintf(motionEvent->x * 800.f / m_pRenderer->GetWidth());
			//newY = (i32)rintf(motionEvent->y * 600.f / m_pRenderer->GetHeight());
	...
	/*if (!m_bGrabInput)
	   ...
	   assert(pSymbol);
	   pSymbol->state = eIS_Changed;
	        pSymbol->value = (i32)newX - (i32)m_posX;
	   CLinuxInputDevice::PostEvent(pSymbol);
	   pSymbol = DevSpecIdToSymbol(MOUSE_AXIS_Y);
	   assert(pSymbol);
	   pSymbol->state = eIS_Changed;
	        pSymbol->value = (i32)newY - (i32)m_posY;
	   CLinuxInputDevice::PostEvent(pSymbol);
	   pSymbol = DevSpecIdToSymbol(MOUSE_AXIS_Z);
	   assert(pSymbol);
	   pSymbol->state = eIS_Changed;
...
void CSDLMouse::GrabInput()
{
	...
	if (SDL_SetRelativeMouseMode(SDL_TRUE) != 0)
		DrxLogAlways("SDL: Could not set relative mouse mode: %s", SDL_GetError());
...
void CSDLMouse::UngrabInput()
{
	...
	if (SDL_SetRelativeMouseMode(SDL_FALSE) != 0)
		DrxLogAlways("SDL: Could not unset relative mouse mode: %s", SDL_GetError());
