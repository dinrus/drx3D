...
CInputDevice::CInputDevice(IInput& input, tukk deviceName)
	: m_input(input)
...
tukk CInputDevice::GetKeyName(const SInputEvent& event) const
{
	return GetKeyName(event.keyId);
}

tukk CInputDevice::GetKeyName(const EKeyId keyId) const
{
	...
	DRX_ASSERT(pInputSymbol != NULL);

	return pInputSymbol->name.c_str();
}

u32 CInputDevice::GetInputCharUnicode(const SInputEvent& event)
{
	return '\0';
}

tukk CInputDevice::GetOSKeyName(const SInputEvent& event)
{
	return "";
}
...
SInputSymbol* CInputDevice::DevSpecIdToSymbol(u32 devSpecId) const
{
...
u32 CInputDevice::NameToId(const TKeyName& name) const
{
...
SInputSymbol* CInputDevice::MapSymbol(u32 deviceSpecificId, EKeyId keyId, const TKeyName& name, SInputSymbol::EType type, u32 user)
{
...
const SInputSymbol* CInputDevice::GetSymbolByName(tukk name) const
{
...
// ------------------------------------------------------------------------
CInputDevice::CDebugPressedButtons::SData::SData(const SInputSymbol* pSymbol_, u32 frame_)
{
	frame = 0;
	if (pSymbol_)
	{
