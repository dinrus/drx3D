...
#include <drx3D/Input/InputCVars.h>
#include <drx3D/Input/EyeTrackerInput.h>
...
{
	DRX_ASSERT(pListenerA != NULL && pListenerB != NULL);
...
{
	DRX_PROFILE_FUNCTION(PROFILE_INPUT);
...
void CBaseInput::SetExclusiveMode(EInputDeviceType deviceType, bool exclusive, uk pUser)
{
	...
	if (deviceType < (i32)m_inputDevices.size())
	{
...
tukk CBaseInput::GetKeyName(EKeyId keyId) const
{
	...
	for (; iter != iterEnd; ++iter)
	{
		...
		DRX_ASSERT(pDevice != NULL);
		tukk szKeyName = pDevice->GetKeyName(keyId);
		if (szKeyName)
		{
			return szKeyName;
		}
...
u32 CBaseInput::GetInputCharUnicode(const SInputEvent& event)
{
	for (TInputDevices::iterator i = m_inputDevices.begin(); i != m_inputDevices.end(); ++i)
	{
		...
		if (event.deviceType == eIDT_Unknown || pInputDevice->GetDeviceType() == event.deviceType)
		{
			u32 inputChar = pInputDevice->GetInputCharUnicode(event);
			if (inputChar)
			{
				return inputChar;
			}
...
tukk CBaseInput::GetOSKeyName(const SInputEvent& event)
{
	for (TInputDevices::iterator i = m_inputDevices.begin(); i != m_inputDevices.end(); ++i)
	{
		tukk ret = (*i)->GetOSKeyName(event);
		if (ret && *ret)
			return ret;
	}
	return "";
...
//////////////////////////////////////////////////////////////////////////
SInputSymbol* CBaseInput::LookupSymbol(EInputDeviceType deviceType, i32 deviceIndex, EKeyId keyId)
{
	for (TInputDevices::iterator i = m_inputDevices.begin(); i != m_inputDevices.end(); ++i)
	{
...
const SInputSymbol* CBaseInput::GetSymbolByName(tukk name) const
{
	TInputDevices::const_iterator iter = m_inputDevices.begin();
	TInputDevices::const_iterator iterEnd = m_inputDevices.end();
	for (; iter != iterEnd; ++iter)
...
void CBaseInput::ClearAnalogKeyState()
{
	...
	for (TInputDevices::iterator i = m_inputDevices.begin(); i != m_inputDevices.end(); ++i)
	{
		...
		i32 clearedSize = clearedSymbols.size();

		for (i32 j = 0; j < clearedSize; j++)
		{
...
{
	DRX_PROFILE_FUNCTION(PROFILE_INPUT);
	//DrxAutoCriticalSection postInputLock(m_postInputEventMutex);
...
bool CBaseInput::SendEventToListeners(const SInputEvent& event)
{
	...
	{
		bool ret = m_pExclusiveListener->OnInputEvent(e);
		if (ret)
			return false;
	}
	...
	bool bInputBlocked = ShouldBlockInputEventPosting(e.keyId, e.deviceType, e.deviceIndex);
	if (!bInputBlocked)
	{
...
void CBaseInput::RemoveDeviceHoldSymbols(EInputDeviceType deviceType, u8 deviceIndex)
{
	if (!m_holdSymbols.empty())
	{
		...
		for (TInputSymbols::iterator it = begin; it != m_holdSymbols.end(); )
		{
			...
			if (symbol->deviceType == deviceType && symbol->deviceIndex == deviceIndex)
			{
				...
				if (g_pInputCVars->i_debug)
				{
					gEnv->pLog->Log("InputDebug: RemoveDeviceHoldSymbols Device %d %u, Symbol %s %d", deviceType, deviceIndex, releaseEvent.keyName.c_str(), releaseEvent.keyId);
				}
...
bool CBaseInput::ShouldBlockInputEventPosting(const EKeyId keyId, const EInputDeviceType deviceType, u8k deviceIndex) const
{
#if DRX_PLATFORM_DURANGO
	return false;
#else
...
void CBaseInput::ForceFeedbackSetDeviceIndex(i32 index)
{
...
void CBaseInput::OnSystemEvent(ESystemEvent event, UINT_PTR wparam, UINT_PTR lparam)
{
	...
	{
		DrxLogAlways("BaseInput - Shutting down all force feedback");
...
i32 CBaseInput::GetNumBlockingInputs() const
{
	return (i32)m_inputBlockData.size();
}
...
bool CBaseInput::AddTouchEventListener(ITouchEventListener* pListener, tukk name)
{
...
IInputDevice* CBaseInput::GetDevice(u16 id, EInputDeviceType deviceType)
{
