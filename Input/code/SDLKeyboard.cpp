...
void CSDLKeyboard::Update(bool focus)
{
	...
	for (i32 i = 0; i < nEvents; ++i)
	{
	...
	#endif
			pSymbol = DevSpecIdToSymbol((u32)keySym);
			if (pSymbol == NULL)
				continue;
		...
		else if (type == SDL_TEXTINPUT)
		{
			...
			uint32_t codepoint;
			for (Unicode::CIterator<tukk , false> it(txtEvent->text); codepoint = *it; ++it)
			{
...
u8 CSDLKeyboard::Event2ASCII(const SInputEvent& event)
{
...
i32 CSDLKeyboard::ConvertModifiers(unsigned keyEventModifiers)
{
	i32 modifiers = 0;
