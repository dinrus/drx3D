...
i32 CKeyboard::s_disableWinKeys = 0;
...
//////////////////////////////////////////////////////////////////////////
u32 CKeyboard::Event2Unicode(const SInputEvent& event)
{
	u32 dik = NameToId(event.keyName);
...
void CKeyboard::RebuildScanCodeTable(HKL layout)
{
	...
	if (GetKeyboardLayoutNameW(layoutName) == TRUE)
	{
		...
		DrxLog("Input locale changed to %s", layoutNameUtf8.c_str());
	}
	else
	{
		DrxLog("Input locale changed to [unknown %p]", layout);
	}
	...
	const auto translateScanCode = [&](UINT scanCode) -> u32
	{
...
//////////////////////////////////////////////////////////////////////////
void CKeyboard::ProcessKey(u32 devSpecId, bool pressed)
{
	...
	i32 modifiers = GetDXInput().GetModifiers();
	if (pressed)
	{
	...
	else
	{
		...
		/*
		   ...
		   tmp[0] = 0;
		   MultiByteToWideChar(CP_ACP, 0, (tuk)&szAsciiInput, 1, tmp, 2);
	...
	while (GetDirectInputDevice())
	{
		...
		if (SUCCEEDED(hr))
		{
			if (g_pInputCVars->i_bufferedkeys)
			{
				...
				for (u32 k = 0; k < dwItems; k++)
				{
					i32 key = rgdod[k].dwOfs;
...
u32 CKeyboard::GetInputCharUnicode(const SInputEvent& event)
{
...
tukk CKeyboard::GetOSKeyName(const SInputEvent& event)
{
	if (IS_KEYBOARD_KEY(event.keyId) && GetDirectInputDevice())
	{
		u32 iDIK = NameToId(event.keyName);
