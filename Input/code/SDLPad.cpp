...
// can create 1:1 mapping to input symbol
	...
	#define SDL_GAMEPAD_DPAD(X) (SDL_GAMEPAD_SYM_DPAD + (X))
...
CSDLPad::CSDLPad(CLinuxInput& input, i32 device) :
...
bool CSDLPad::SetForceFeedback(IFFParams params)
{
	...
	if (m_supportsFeedback && m_curHapticEffect < 0)
	{
		...
		effect.leftright.large_magnitude = (u16)(clamp_tpl(params.strengthB, 0.0f, 1.0f) * 65536.0f);
		effect.leftright.small_magnitude = (u16)(clamp_tpl(params.strengthA, 0.0f, 1.0f) * 65536.0f);
...
i32 CSDLPad::GetInstanceId() const
{
...
void CSDLPad::HandleAxisEvent(const SDL_JoyAxisEvent& axisEvt)
{
	...
	//DrxLogAlways("CSDLPad - AXIS requested(%d) %d on Gamepad [%d]", axisEvt.axis, axisEvt.value, m_deviceNo);
	i32k id = SDL_GAMEPAD_AXIS(axisEvt.axis);
...
void CSDLPad::HandleHatEvent(const SDL_JoyHatEvent& hatEvt)
{
	...
	SInputSymbol* pSymbol = NULL;
	SInputEvent inputEvt;
	//DrxLogAlways( "HAT %x", hatEvt.value);
...
void CSDLPad::HandleButtonEvent(const SDL_JoyButtonEvent& btEvt)
{
	...
	//DrxLogAlways("BTEVENT EVT: bt:%d %s",btEvt.button, (btEvt.state & SDL_PRESSED) ? "PRESSED" : "RELEASED" );
	...
	if (pSymbol)
	{
		...
		//DrxLogAlways("BTEVENT SYM: name: %s state:%d=>%d", pSymbol->name.c_str(), i32(pSymbol->state), i32(currState));
		bool isPost = false;
		switch (btEvt.button)
		{
...
void CSDLPad::HandleConnectionState(const bool connected)
{
	if (m_connected != connected)
	{
		...
		event.deviceIndex = (u8) m_deviceNo;
...
CSDLPadUpr::CSDLPadUpr(CLinuxInput& input) :
	m_rLinuxInput(input),
	m_gamePads()
{
...
bool CSDLPadUpr::Init()
{
	#if defined(SDL_HINT_ACCELEROMETER_AS_JOYSTICK)
	// On platforms that have accelerometer, don't return it as a joystick
	i32k zero = 0;
	if (!SDL_SetHintWithPriority(SDL_HINT_ACCELEROMETER_AS_JOYSTICK, (tukk)&zero, SDL_HINT_OVERRIDE))
	{
		gEnv->pLog->LogError("CSDLPadUpr - SDL_SetHint SDL_HINT_ACCELEROMETER_AS_JOYSTICK failed: %s", SDL_GetError());
	}
	#endif
	// setup devices
	i32 nGamePads = SDL_NumJoysticks();
	for (i32 i = 0; i < nGamePads; ++i)
	{
		if (!AddGamePad(i))
		{
			return false;
...
void CSDLPadUpr::Update(bool bFocus)
{
	SDL_Event eventList[SDL_GAMEPAD_MAX_PEEP];

	i32 nEvents = SDL_PeepEvents(eventList, SDL_GAMEPAD_MAX_PEEP, SDL_GETEVENT, SDL_JOYAXISMOTION, SDL_JOYDEVICEREMOVED);
	unsigned type = 0;

	if (nEvents == -1)
	{
		gEnv->pLog->LogError("CSDLPadUpr - SDL_GetEvent error: %s", SDL_GetError());
		return;
	}
	for (i32 i = 0; i < nEvents; ++i)
	{
		...
		if (type == SDL_JOYAXISMOTION)
		{
			...
			//DRX_ASSERT(axisEvt.axis < 4);
			i32 id = axisEvt.which;
			if (id < 0)
			{
				gEnv->pLog->LogError("CSLDPadUpr - Received event from invalid Gamepad [%d]", axisEvt.which);
				continue;
			}
		...
		else if (type == SDL_JOYHATMOTION)
		{
			...
			if (id < 0)
			{
				gEnv->pLog->LogError("CSLDPadUpr - Received event from invalid Gamepad [%d]", hatEvt.which);
				continue;
			}
			CSDLPad* pad = FindPadByInstanceId(id);
			if (pad != NULL)
		...
		else if (type == SDL_JOYBUTTONDOWN || type == SDL_JOYBUTTONUP)
		{
			...
			if (id < 0)
			{
				gEnv->pLog->LogError("CSLDPadUpr - Received event from invalid Gamepad [%d]", btEvt.which);
				continue;
			}
			CSDLPad* pad = FindPadByInstanceId(id);
			if (pad != NULL)
		...
		else if (type == SDL_JOYBALLMOTION)
		{
			...
			gEnv->pLog->LogError("CSLDPadUpr - Gamepad [%d] triggered a ball motion event which is not supported", ballEvt.which);
		}
		else if (type == SDL_JOYDEVICEREMOVED)
		{
			...
			{
				gEnv->pLog->LogError("CSLDPadUpr - Failed to remove Gamepad [%d]", devEvt.which);
			}
		}
		else if (type == SDL_JOYDEVICEADDED)
		{
			...
			{
				gEnv->pLog->LogError("CSLDPadUpr - Failed to Add Gamepad [%d]", devEvt.which);
			}
		}
		else
		{
			gEnv->pLog->LogError("CSDLPadUpr - Unknown event processed: %x", type);
			break;
		}
	}
...
float CSDLPad::DeadZoneFilter(i32 input)
{
	float retval = 0.0f;
...
bool CSDLPadUpr::AddGamePad(i32 deviceIndex)
{
	...
	else
	{
		...
		if (!pPad->Init())
		{
			gEnv->pLog->LogError("CSDLPadUpr - Failed to init Gamepad [%d]", deviceIndex);
			delete pPad;
			return false;
		}
...
bool CSDLPadUpr::RemovGamePad(i32 instanceId)
{
...
CSDLPad* CSDLPadUpr::FindPadByInstanceId(i32 instanceId)
{
	...
	gEnv->pLog->LogError("CSLDPadUpr - Unknown device instance [%d]", instanceId);
	return NULL;
}

CSDLPad* CSDLPadUpr::FindPadByDeviceIndex(i32 deviceIndex)
{
