...
#include <drx3D/Input/DXInputDevice.h>
...
CDXInputDevice::CDXInputDevice(CDXInput& input, tukk deviceName, const GUID& guid) : CInputDevice(input, deviceName),
...
bool CDXInputDevice::CreateDirectInputDevice(LPCDIDATAFORMAT dataFormat, DWORD coopLevel, DWORD bufSize)
{
	...
	/*
	   ...
	   format->dwDataSize = 0;
	   for (u32 i = 0; i < customFormat.size(); ++i)
	   {
...
/*
   ...
   for (i = customFormat.begin(); i!= customFormat.end(); ++i)
   {
    ...
    {
      drx_sprintf(tempBuff, "axis_%d", nAxis);
      //symbol.type = IT_Axis;
   ++nAxis;
    }
    else if ((*i).dwType & DIDFT_BUTTON)
    {
      drx_sprintf(tempBuff, "button_%d", nButton);
      //symbol.type = IT_Button;
   ++nButton;
    }
    else if ((*i).dwType & DIDFT_POV)
    {
      drx_sprintf(tempBuff, "pov_%d", nPov);
      //symbol.type = IT_Pov;
   ++nPov;
    }
