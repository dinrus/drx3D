...
#ifdef DRXINPUT_EXPORTS
	#define DRXINPUT_API DLL_EXPORT
#else
	#define DRXINPUT_API DLL_IMPORT
#endif
...
static i32k EFF_INVALID_DEVICE_INDEX = -1;
...
#define USE_DRX_NAME_FOR_KEY_NAMES 0

#if USE_DRX_NAME_FOR_KEY_NAMES

typedef CDrxName TKeyName;
...
struct TKeyName
{
	...
	TKeyName() { key = ""; }
	TKeyName(tukk _key) { key = _key; };
	operator tukk () const { return key; };
	bool        operator<(const TKeyName& n) const             { return stricmp(key, n.key) < 0; }
	bool        operator>(const TKeyName& n) const             { return stricmp(key, n.key) > 0; }
	bool        operator==(const TKeyName& n) const            { return stricmp(key, n.key) == 0; }
	bool        operator!=(const TKeyName& n) const            { return stricmp(key, n.key) != 0; }
	bool        operator<(tukk str) const               { return stricmp(key, str) < 0; }
	bool        operator>(tukk str) const               { return stricmp(key, str) > 0; }
	bool        operator==(tukk str) const              { return stricmp(key, str) == 0; }
	bool        operator!=(tukk str) const              { return stricmp(key, str) != 0; }
	tukk c_str() const                                  { return key; }
};
inline bool   operator==(tukk str, const TKeyName& n) { return n == str; }
inline bool   operator!=(tukk str, const TKeyName& n) { return n != str; }
inline bool   operator<(tukk str, const TKeyName& n)  { return n < str; }
inline bool   operator>(tukk str, const TKeyName& n)  { return n > str; }

#endif //USE_DRX_NAME_FOR_KEY_NAMES
...
struct SInputEvent
{
	...
	i32              modifiers;        //!< Key modifiers enabled at the time of this event.
	float            value;            //!< Value associated with the event.
	SInputSymbol*    pSymbol;          //!< Input symbol the event originated from.
	u8            deviceIndex;      //!< Local index of this particular controller type.
	u8            deviceUniqueID;   //!< Process wide unique controller ID.
...
struct SUnicodeEvent
{
	...
	SUnicodeEvent(u32 inputChar = 0) : inputChar(inputChar) {}
	void GetMemoryUsage(IDrxSizer* pSizer) const {}
};
...
struct SFFTriggerOutputData
{
	...
	u16 leftEnv, rightEnv;
	u32 flags;

	SFFTriggerOutputData()  { Init(Initial::ZeroIt); }
	SFFTriggerOutputData(Initial::Value v)  { Init(v); }
	SFFTriggerOutputData(bool leftTouchToActivate, bool rightTouchToActivate, float lTrigger, float rTrigger, float lStrength, float rStrength, u16 lTriggerEnv, u16 rTriggerEnv) :
...
struct SInputSymbol
{
	...
	SInputSymbol(u32 devSpecId_, EKeyId keyId_, const TKeyName& name_, EType type_, u32 user_ = 0)
	...
	void AssignTo(SInputEvent& event, i32 modifiers = 0)
	{
		event.pSymbol = this;
	...
	u32k     devSpecId;       //!< Device internal id of this symbol (we will use it to look it up).
	EInputState      state;           //!< Current state.
	const EType      type;            //!< Type of this symbol.
	float            value;           //!< Current value.
	u32           user;            //!< Type dependent value (toggle-mask for toggles).
...
enum KIN_IDENTITY_ENROLLMENT : u32
{
...
typedef bool (* KinIdentifyCallback)(const SKinIdentityMessage& message, uk pData);

typedef uk   KIN_SPEECH_HANDLE;
...
{
	u64         ulAudioTimeOffset;
	u64         ulAudioSizeTime;
	u64         ulAudioStreamOffset;
	u64         ulAudioSizeBytes;
	u64         ulRetainedStreamOffset;
	u64         ulRetainedSizeBytes;
...
struct IKinectInput
{
	...
	virtual void RegisterInputListener(IKinectInputListener* pInputListener, tukk name) = 0;
	virtual void UnregisterInputListener(IKinectInputListener* pInputListener) = 0;
#else
	virtual void RegisterKinectAudioListener(IKinectInputAudioListener* pInputListener, tukk name) = 0;
	...
	virtual tukk GetUserStatusMessage() = 0;
	//! Adds an Arc rail - The most common used way for menu navigation
	//! \return true if the rail is successfully added
	virtual bool RegisterArcRail(i32 gripId, i32 railId, const Vec2& vScreenPos, const Vec3& vDir, float fLenght, float fDeadzoneLength, float fToleranceConeAngle) = 0;

	virtual void UnregisterArcRail(i32 gripId) = 0;
	...
	//! \return true if the rail is successfully added
	virtual bool RegisterHoverTimeRail(i32 gripId, i32 railId, const Vec2& vScreenPos, float fHoverTime, float fTimeTillCommit, SKinGripShape* pGripShape = NULL) = 0;

	//! Unregister a specific grip.
	//! You can only remove the full grip, not one rail at a time
	virtual void UnregisterHoverTimeRail(i32 gripId) = 0;
	...
	virtual bool GetSkeletonRawData(u32 iUser, SKinSkeletonRawData& skeletonRawData) const = 0;

	//! Get default skeleton data.
	virtual bool GetSkeletonDefaultData(u32 iUser, SKinSkeletonDefaultData& skeletonDefaultData) const = 0;

	//! Get the skeleton that is closest to the camera.
	virtual u32 GetClosestTrackedSkeleton() const = 0;
	...
	virtual bool IdentityIdentify(DWORD dwTrackingId, KinIdentifyCallback callbackFunc, uk pData) = 0;
	...
	virtual bool KinSpeechSetEventInterest(u64 ulEvents) = 0;
	...
	virtual void UpdateSkeletonAlignment(u32 skeletonIndex) {}
	// </interfuscator:shuffle>
};
...
struct IKinectInputListener
{
	...
	virtual void OnKinectClosestSkeletonChanged(u32 skeletonIndex) = 0;
	virtual void OnKinectSkeletonMoved(u32 skeletonIndex, float distanceMoved) = 0;
	// </interfuscator:shuffle>
};
...
struct IKinectInputAudioListener
{
	...
	virtual void OnVoiceCommand(tukk voiceCommand) = 0;
	virtual void OnGrammarChange(tukk grammarName, eGrammaChange change) = 0;
};
...
struct IInputDevice
{
	...
	virtual void                SetUniqueId(u8 const uniqueId) = 0;
	virtual tukk         GetKeyName(const SInputEvent& event) const = 0;
	virtual tukk         GetKeyName(const EKeyId keyId) const = 0;
	virtual u32              GetInputCharUnicode(const SInputEvent& event) = 0;
	virtual tukk         GetOSKeyName(const SInputEvent& event) = 0;
	virtual SInputSymbol*       LookupSymbol(EKeyId id) const = 0;
	virtual const SInputSymbol* GetSymbolByName(tukk name) const = 0;
...
struct IInputEngineModule : public Drx::IDefaultModule
{
	DRXINTERFACE_DECLARE_GUID(IInputEngineModule, "2c8744b1-6794-4fe0-8003-8fc3ade9076a"_drx_guid);
};
...
struct IInput
{
	...
	virtual bool AddTouchEventListener(ITouchEventListener* pListener, tukk name) = 0;
	...
	virtual void ProcessKey(u32 key, bool pressed, wchar_t unicode, bool repeat) = 0;
	...
	virtual void ForceFeedbackSetDeviceIndex(i32 index) = 0;
	...
	virtual void SetExclusiveMode(EInputDeviceType deviceType, bool exclusive, uk hwnd = 0) = 0;
	...
	virtual tukk GetKeyName(EKeyId keyId) const = 0;
	...
	virtual SInputSymbol* LookupSymbol(EInputDeviceType deviceType, i32 deviceIndex, EKeyId keyId) = 0;
	...
	virtual IInputDevice* GetDevice(u16 id, EInputDeviceType deviceType) = 0;

	//! Returns EInputPlatformFlags.
	virtual u32 GetPlatformFlags() const = 0;
	...
	virtual i32 GetNumBlockingInputs() const = 0;
	...
	virtual bool ShouldBlockInputEventPosting(const EKeyId keyId, const EInputDeviceType deviceType, u8k deviceIndex) const = 0;

	//! Show or hide the mouse cursor
	//! \return Internal display count. Cursor is visible if the display count is greater or equal to zero.
	virtual i32 ShowCursor(const bool bShow) = 0;
...
typedef IInput  (* DRX_PTRCREATEINPUTFNC(ISystem * pSystem, uk hwnd));

DRXINPUT_API IInput* CreateInput(ISystem* pSystem, uk hwnd);
