...
ERequestStatus CImpl::Init(u32 const objectPoolSize, u32 const eventPoolSize)
{
	...
	char const* szAssetDirectory = gEnv->pSystem->GetIProjectUpr()->GetCurrentAssetDirectoryRelative();

#if defined(INCLUDE_PORTAUDIO_IMPL_PRODUCTION_CODE)
	if (strlen(szAssetDirectory) == 0)
	{
		Drx::Audio::Log(ELogType::Error, "<Audio - PortAudio>: No asset folder set!");
	...
	if (err != paNoError)
	{
		Drx::Audio::Log(ELogType::Error, "Failed to initialize PortAudio: %s", Pa_GetErrorText(err));
	}
...
ERequestStatus CImpl::ShutDown()
{
	...
	if (err != paNoError)
	{
		Drx::Audio::Log(ELogType::Error, "Failed to shut down PortAudio: %s", Pa_GetErrorText(err));
	}
...
ERequestStatus CImpl::RegisterInMemoryFile(SFileInfo* const pFileInfo)
{
	...
	if (pFileInfo != nullptr)
	{
		...
		{
			Drx::Audio::Log(ELogType::Error, "Invalid AudioFileEntryData passed to the PortAudio implementation of RegisterInMemoryFile");
		}
	}
...
ERequestStatus CImpl::UnregisterInMemoryFile(SFileInfo* const pFileInfo)
{
	...
	if (pFileInfo != nullptr)
	{
		...
		{
			Drx::Audio::Log(ELogType::Error, "Invalid AudioFileEntryData passed to the PortAudio implementation of UnregisterInMemoryFile");
		}
	}
...
ITrigger const* CImpl::ConstructTrigger(XmlNodeRef const pRootNode)
{
	...
	if (_stricmp(szTag, s_szEventTag) == 0)
	{
		...
		if (!path.empty())
		{
			...
			if (pSndFile != nullptr)
			{
				DrxFixedStringT<16> const eventTypeString(pRootNode->getAttr(s_szTypeAttribute));
				...
				i32 const subFormat = sfInfo.format & SF_FORMAT_SUBMASK;
				...
				i32 const failure = sf_close(pSndFile);

				if (failure)
				{
					Drx::Audio::Log(ELogType::Error, "Failed to close SNDFILE during CImpl::NewAudioTrigger");
				}
			}
		}
	}
	...
	{
		Drx::Audio::Log(ELogType::Warning, "Unknown PortAudio tag: %s", szTag);
	}
