...
ERequestStatus CImpl::Init(u32 const objectPoolSize, u32 const eventPoolSize)
{
	...
	DrxFixedStringT<MaxInfoStringLength> systemVersion;
	systemVersion.Format("%08x", version);
	DrxFixedStringT<MaxInfoStringLength> headerVersion;
	...
	uk pExtraDriverData = nullptr;
	i32 initFlags = FMOD_INIT_NORMAL | FMOD_INIT_VOL0_BECOMES_VIRTUAL;
	i32 studioInitFlags = FMOD_STUDIO_INIT_NORMAL;
...
ERequestStatus CImpl::RegisterInMemoryFile(SFileInfo* const pFileInfo)
{
	...
	if (pFileInfo != nullptr)
	{
		...
		if (pFileData != nullptr)
		{
			...
			{
				DrxFatalError("<Audio>: allocation not %d byte aligned!", 32);
			}
#endif      // INCLUDE_FMOD_IMPL_PRODUCTION_CODE

			FMOD_RESULT fmodResult = m_pSystem->loadBankMemory(static_cast<tuk>(pFileInfo->pFileData), static_cast<i32>(pFileInfo->size), FMOD_STUDIO_LOAD_MEMORY_POINT, FMOD_STUDIO_LOAD_BANK_NORMAL, &pFileData->pBank);
			ASSERT_FMOD_OK;

			DrxFixedStringT<MaxFilePathLength> streamsBankPath = pFileInfo->szFilePath;
			PathUtil::RemoveExtension(streamsBankPath);
			streamsBankPath += ".streams.bank";

			size_t const streamsBankFileSize = gEnv->pDrxPak->FGetSize(streamsBankPath.c_str());
		...
		{
			Drx::Audio::Log(ELogType::Error, "Invalid FileData passed to the Fmod implementation of RegisterInMemoryFile");
		}
	}
...
ERequestStatus CImpl::UnregisterInMemoryFile(SFileInfo* const pFileInfo)
{
	...
	if (pFileInfo != nullptr)
	{
		...
		{
			Drx::Audio::Log(ELogType::Error, "Invalid FileData passed to the Fmod implementation of UnregisterInMemoryFile");
		}
	}
...
IObject* CImpl::ConstructGlobalObject()
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, "Trying to construct an already registered audio object.");
	}
...
void CImpl::DestructObject(IObject const* const pIObject)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, "Trying to delete a non-existing audio object.");
	}
...
{
	DRX_ASSERT(pIStandaloneFile != nullptr);
...
ITrigger const* CImpl::ConstructTrigger(XmlNodeRef const pRootNode)
{
	...
	if (_stricmp(szTag, s_szEventTag) == 0)
	{
		...
		{
			Drx::Audio::Log(ELogType::Warning, "Unknown Fmod event: %s", path.c_str());
		}
	}
	else if (_stricmp(szTag, s_szSnapshotTag) == 0)
	{
		...
		{
			Drx::Audio::Log(ELogType::Warning, "Unknown Fmod snapshot: %s", path.c_str());
		}
	}
	else
	{
		Drx::Audio::Log(ELogType::Warning, "Unknown Fmod tag: %s", szTag);
	}
...
IParameter const* CImpl::ConstructParameter(XmlNodeRef const pRootNode)
{
	...
	else if (_stricmp(szTag, s_szVcaTag) == 0)
	{
		...
		{
			Drx::Audio::Log(ELogType::Warning, "Unknown Fmod VCA: %s", fullName.c_str());
		}
	}
	else
	{
...
IEnvironment const* CImpl::ConstructEnvironment(XmlNodeRef const pRootNode)
{
	...
	if (_stricmp(szTag, s_szBusTag) == 0)
	{
		...
		{
			Drx::Audio::Log(ELogType::Warning, "Unknown Fmod bus: %s", path.c_str());
		}
	}
...
//////////////////////////////////////////////////////////////////////////
void CImpl::CreateVersionString(DrxFixedStringT<MaxInfoStringLength>& stringOut) const
{
...
//////////////////////////////////////////////////////////////////////////
FMOD_RESULT F_CALLBACK FmodFileOpenCallback(tukk szName, u32* pFileSize, uk * pHandle, uk pUserData)
{
	FMOD_RESULT fmodResult = FMOD_ERR_FILE_NOTFOUND;

	char const* szFileName = (char const*)pUserData;
	FILE* const pFile = gEnv->pDrxPak->FOpen(szFileName, "rbx", IDrxPak::FOPEN_HINT_DIRECT_OPERATION);

	if (pFile != nullptr)
	{
		gEnv->pDrxPak->FSeek(pFile, 0, SEEK_END);
		auto const fileSize = static_cast<size_t>(gEnv->pDrxPak->FTell(pFile));
		gEnv->pDrxPak->FSeek(pFile, 0, SEEK_SET);
		...
		{
			*pFileSize = static_cast<u32>(fileSize);
			*pHandle = pFile;
			fmodResult = FMOD_OK;
		}
	}
...
//////////////////////////////////////////////////////////////////////////
FMOD_RESULT F_CALLBACK FmodFileCloseCallback(uk pHandle, uk pUserData)
{
	FMOD_RESULT fmodResult = FMOD_ERR_FILE_NOTFOUND;
	FILE* const pFile = static_cast<FILE*>(pHandle);

	if (gEnv->pDrxPak->FClose(pFile) == 0)
	{
		fmodResult = FMOD_OK;
	}
...
//////////////////////////////////////////////////////////////////////////
FMOD_RESULT F_CALLBACK FmodFileReadCallback(uk pHandle, uk pBuffer, u32 sizeBytes, u32* pBytesRead, uk pUserData)
{
	FMOD_RESULT fmodResult = FMOD_ERR_FILE_NOTFOUND;
	FILE* const pFile = static_cast<FILE*>(pHandle);
	size_t const bytesRead = gEnv->pDrxPak->FReadRaw(pBuffer, 1, static_cast<size_t>(sizeBytes), pFile);
...
//////////////////////////////////////////////////////////////////////////
FMOD_RESULT F_CALLBACK FmodFileSeekCallback(uk pHandle, u32 pos, uk pUserData)
{
	FMOD_RESULT fmodResult = FMOD_ERR_FILE_COULDNOTSEEK;
	FILE* const pFile = static_cast<FILE*>(pHandle);

	if (gEnv->pDrxPak->FSeek(pFile, static_cast<long>(pos), 0) == 0)
	{
		fmodResult = FMOD_OK;
	}
...
FMOD_RESULT CImpl::LoadBankCustom(char const* const szFileName, FMOD::Studio::Bank** ppBank)
{
	...
	bankInfo.userdata = reinterpret_cast<uk>(const_cast<tuk>(szFileName));
...
bool CImpl::LoadMasterBanks()
{
	...
	DrxFixedStringT<MaxFilePathLength + MaxFileNameLength> search(m_regularSoundBankFolder + "/*.bank");
	_finddata_t fd;
	intptr_t const handle = gEnv->pDrxPak->FindFirst(search.c_str(), &fd);
	...
	{
		size_t const masterBankFileSize = gEnv->pDrxPak->FGetSize(m_masterBankPath.c_str());
		DRX_ASSERT(masterBankFileSize > 0);
		size_t const masterBankStringsFileSize = gEnv->pDrxPak->FGetSize(m_masterStringsBankPath.c_str());
		DRX_ASSERT(masterBankStringsFileSize > 0);
		...
		if (masterBankFileSize > 0 && masterBankStringsFileSize > 0)
		{
			...
			if (m_pMasterBank != nullptr)
			{
				...
				if (numBuses > 0)
				{
					...
					i32 numRetrievedBuses = 0;
					fmodResult = m_pMasterBank->getBusList(pBuses, numBuses, &numRetrievedBuses);
					ASSERT_FMOD_OK;
					DRX_ASSERT(numBuses == numRetrievedBuses);

					for (i32 i = 0; i < numRetrievedBuses; ++i)
					{
			...
			size_t const masterBankAssetsFileSize = gEnv->pDrxPak->FGetSize(m_masterAssetsBankPath.c_str());
			...
			size_t const masterBankStreamsFileSize = gEnv->pDrxPak->FGetSize(m_masterStreamsBankPath.c_str());
	...
	else
	{
		...
		Drx::Audio::Log(ELogType::Error, "Fmod failed to load master banks");
		return true;
	}
