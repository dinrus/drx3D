...
//////////////////////////////////////////////////////////////////////////
FMOD_RESULT F_CALLBACK EventCallback(FMOD_STUDIO_EVENT_CALLBACK_TYPE type, FMOD_STUDIO_EVENTINSTANCE* event, uk parameters)
{
	...
	if (pEventInstance != nullptr)
	{
		...
		FMOD_RESULT const fmodResult = pEventInstance->getUserData(reinterpret_cast<uk *>(&pEvent));
		ASSERT_FMOD_OK;
...
void CObjectBase::RemoveEvent(CEvent* const pEvent)
{
	if (!stl::find_and_erase(m_events, pEvent))
	{
		...
		{
			Drx::Audio::Log(ELogType::Error, "Tried to remove an event from an audio object that does not own that event");
		}
	}
}
...
void CObjectBase::RemoveFile(CStandaloneFileBase const* const pFile)
{
	if (!stl::find_and_erase(m_files, pFile))
	{
		...
		{
			Drx::Audio::Log(ELogType::Error, "Tried to remove an audio file from an audio object that is not playing that file");
		}
	}
}
...
bool CObjectBase::SetEvent(CEvent* const pEvent)
{
	...
	if (pEvent->PrepareForOcclusion())
	{
		...
		DRX_ASSERT_MESSAGE(pEventInstance != nullptr, "Event instance doesn't exist.");
		CTrigger const* const pTrigger = pEvent->GetTrigger();
		DRX_ASSERT_MESSAGE(pTrigger != nullptr, "Trigger doesn't exist.");
		...
		if (g_triggerToParameterIndexes.find(pTrigger) != g_triggerToParameterIndexes.end())
		{
			...
			{
				u32 const parameterId = parameterPair.first->GetId();
			...
			for (auto const& switchPair : m_switches)
			{
				...
				u32 const parameterId = pSwitchState->GetId();
...
ERequestStatus CObjectBase::ExecuteTrigger(ITrigger const* const pITrigger, IEvent* const pIEvent)
{
		...
		case EEventType::Start:
			{
				...
				if (pEventDescription != nullptr)
				{
					DRX_ASSERT(pEvent->GetInstance() == nullptr);
					...
					DRX_ASSERT(pEvent->GetId() == InvalidCRC32);
					pEvent->SetId(pTrigger->GetId());
					pEvent->SetObject(this);

					DRX_ASSERT_MESSAGE(std::find(m_pendingEvents.begin(), m_pendingEvents.end(), pEvent) == m_pendingEvents.end(), "Event was already in the pending list");
		...
		case EEventType::Resume:
			{
				...
				if (pEventDescription != nullptr)
				{
					...
					i32 const capacity = 32;
					i32 count = 0;

#if defined(INCLUDE_FMOD_IMPL_PRODUCTION_CODE)
					fmodResult = pEventDescription->getInstanceCount(&count);
					ASSERT_FMOD_OK;
					DRX_ASSERT_MESSAGE(count < capacity, "Instance count (%d) is higher or equal than array capacity (%d).", count, capacity);
	...
	{
		Drx::Audio::Log(ELogType::Error, "Invalid trigger or event pointers passed to the Fmod implementation of ExecuteTrigger.");
	}
...
ERequestStatus CObjectBase::PlayFile(IStandaloneFile* const pIStandaloneFile)
{
	...
	if (pStandaloneFile != nullptr)
	{
		...
		DRX_ASSERT_MESSAGE(std::find(m_pendingFiles.begin(), m_pendingFiles.end(), pStandaloneFile) == m_pendingFiles.end(), "standalone file was already in the pending standalone files list");
		m_pendingFiles.push_back(pStandaloneFile);

		return ERequestStatus::Success;
	}

	Drx::Audio::Log(ELogType::Error, "Invalid standalone file pointer passed to the Fmod implementation of PlayFile.");
...
ERequestStatus CObjectBase::StopFile(IStandaloneFile* const pIStandaloneFile)
{
	...
	{
		Drx::Audio::Log(ELogType::Error, "Invalid standalone file pointer passed to the Fmod implementation of StopFile.");
	}
...
//////////////////////////////////////////////////////////////////////////
void CObjectBase::StopEvent(u32 const id)
{
	FMOD_RESULT fmodResult = FMOD_ERR_UNINITIALIZED;
...
ERequestStatus CGlobalObject::SetEnvironment(IEnvironment const* const pIEnvironment, float const amount)
{
	...
	{
		Drx::Audio::Log(ELogType::Error, "Invalid Environment pointer passed to the Fmod implementation of SetEnvironment");
	}
...
ERequestStatus CGlobalObject::SetParameter(IParameter const* const pIParameter, float const value)
{
	...
	{
		Drx::Audio::Log(ELogType::Error, "Invalid parameter pointer passed to the Fmod implementation of SetParameter");
	}
...
ERequestStatus CGlobalObject::SetSwitchState(ISwitchState const* const pISwitchState)
{
	...
	{
		Drx::Audio::Log(ELogType::Error, "Invalid switch pointer passed to the Fmod implementation of SetSwitchState");
	}
...
{
	Drx::Audio::Log(ELogType::Error, "Trying to set occlusion and obstruction values on the global audio object!");
	return ERequestStatus::Failure;
}
...
ERequestStatus CObject::SetEnvironment(IEnvironment const* const pIEnvironment, float const amount)
{
	...
	{
		Drx::Audio::Log(ELogType::Error, "Invalid IEnvironment pointer passed to the Fmod implementation of SetEnvironment");
...
ERequestStatus CObject::SetParameter(IParameter const* const pIParameter, float const value)
{
	...
	if (pParameter != nullptr)
	{
		...
		{
			u32 const parameterId = pParameter->GetId();
			FMOD_RESULT fmodResult = FMOD_ERR_UNINITIALIZED;
	...
	{
		Drx::Audio::Log(ELogType::Error, "Invalid AudioObjectData or ParameterData passed to the Fmod implementation of SetParameter");
		result = ERequestStatus::Failure;
	}
