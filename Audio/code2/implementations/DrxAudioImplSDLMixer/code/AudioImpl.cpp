...
///////////////////////////////////////////////////////////////////////////
void OnStandaloneFileFinished(CATLStandaloneFile& standaloneFile, tukk szFile)
{
...
ERequestStatus CImpl::RegisterInMemoryFile(SFileInfo* const pFileInfo)
{
	...
	if (pFileInfo != nullptr)
	{
		...
		{
			Drx::Audio::Log(ELogType::Error, "Invalid AudioFileEntryData passed to the SDL Mixer implementation of RegisterInMemoryFile");
		}
	}
...
ERequestStatus CImpl::UnregisterInMemoryFile(SFileInfo* const pFileInfo)
{
	...
	if (pFileInfo != nullptr)
	{
		...
		{
			Drx::Audio::Log(ELogType::Error, "Invalid AudioFileEntryData passed to the SDL Mixer implementation of UnregisterInMemoryFile");
		}
	}
	return result;
}
...
ERequestStatus CImpl::ConstructFile(XmlNodeRef const pRootNode, SFileInfo* const pFileInfo)
{
	...
	if ((_stricmp(pRootNode->getTag(), s_szFileTag) == 0) && (pFileInfo != nullptr))
	{
		...
		DrxFixedStringT<MaxFilePathLength> fullFilePath;
		if (szPath)
		{
			fullFilePath = szPath;
			fullFilePath += "/";
...
{
	static DrxFixedStringT<MaxFilePathLength> s_path;
...
ITrigger const* CImpl::ConstructTrigger(XmlNodeRef const pRootNode)
{
	...
	if (_stricmp(pRootNode->getTag(), s_szEventTag) == 0)
	{
		...
		if (type == EEventType::Start)
		{
			...
			if (isAttenuationEnabled)
			{
				...
				{
					Drx::Audio::Log(ELogType::Warning, "Min distance (%f) was greater than max distance (%f) of %s", minDistance, maxDistance, szFileName);
					std::swap(minDistance, maxDistance);
				}
			}

			// Translate decibel to normalized value.
			static i32k maxVolume = 128;
			float volume = 0.0f;
			pRootNode->getAttr(s_szVolumeAttribute, volume);
			auto const normalizedVolume = static_cast<i32>(pow_tpl(10.0f, volume / 20.0f) * maxVolume);
			...
			auto const fadeInTimeMs = static_cast<i32>(fadeInTimeSec * 1000.0f);

			float fadeOutTimeSec = s_defaultFadeOutTime;
			pRootNode->getAttr(s_szFadeOutTimeAttribute, fadeOutTimeSec);
			auto const fadeOutTimeMs = static_cast<i32>(fadeOutTimeSec * 1000.0f);
	...
	{
		Drx::Audio::Log(ELogType::Warning, "Unknown SDL Mixer tag: %s", pRootNode->getTag());
	}
...
ISwitchState const* CImpl::ConstructSwitchState(XmlNodeRef const pRootNode)
{
	...
	if (_stricmp(pRootNode->getTag(), s_szEventTag) == 0)
	{
		...
		value = drxmath::clamp(value, 0.0f, 1.0f);
...
void CImpl::DestructStandaloneFile(IStandaloneFile const* const pIStandaloneFile)
{
	...
	DRX_ASSERT_MESSAGE(pStandaloneEvent->m_channels.size() == 0, "Events always have to be stopped/finished before they get deleted");
#endif  // INCLUDE_SDLMIXER_IMPL_PRODUCTION_CODE
