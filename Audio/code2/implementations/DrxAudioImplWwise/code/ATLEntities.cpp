...
ERequestStatus CObject::Set3DAttributes(SObject3DAttributes const& attributes)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, "Wwise - SetPosition failed with AKRESULT: %d", wwiseResult);
...
ERequestStatus CObject::SetEnvironment(IEnvironment const* const pIEnvironment, float const amount)
{
	...
	if (pEnvironment != nullptr)
	{
		...
		default:
			{
				DRX_ASSERT(false); //Unknown AudioEnvironmentImplementation type
			}
		}
	}
	else
	{
		Drx::Audio::Log(ELogType::Error, "Wwise - Invalid EnvironmentData passed to the Wwise implementation of SetEnvironment");
	}
...
ERequestStatus CObject::SetParameter(IParameter const* const pIParameter, float const value)
{
	...
	{
		Drx::Audio::Log(ELogType::Error, "Wwise - Invalid RtpcData passed to the Wwise implementation of SetParameter");
	}
...
ERequestStatus CObject::SetSwitchState(ISwitchState const* const pISwitchState)
{
	...
	if (pSwitchState != nullptr)
	{
		...
		default:
			{
				Drx::Audio::Log(ELogType::Warning, "Wwise - Unknown ESwitchType: %" PRISIZE_T, pSwitchState->type);
	...
	{
		Drx::Audio::Log(ELogType::Error, "Wwise - Invalid SwitchState passed to the Wwise implementation of SetSwitchState");
	}
...
ERequestStatus CObject::SetObstructionOcclusion(float const obstruction, float const occlusion)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, "Wwise - invalid listener Id during SetObjectObstructionAndOcclusion!");
	}
...
ERequestStatus CObject::ExecuteTrigger(ITrigger const* const pITrigger, IEvent* const pIEvent)
{
	...
	if ((pTrigger != nullptr) && (pEvent != nullptr))
	{
		...
		else
		{
			...
			Drx::Audio::Log(ELogType::Warning, "Failed to Post Wwise event %" PRISIZE_T, pEvent->m_id);
		}
	}
	else
	{
		Drx::Audio::Log(ELogType::Error, "Invalid AudioObjectData, ATLTriggerData or EventData passed to the Wwise implementation of ExecuteTrigger.");
	}
...
ERequestStatus CObject::SetName(char const* const szName)
{
	...
	DRX_ASSERT(wwiseResult == AK_Success);
...
ERequestStatus CObject::PostEnvironmentAmounts()
{
	...
	if (numEnvironments > 0)
	{
		...
		{
			Drx::Audio::Log(ELogType::Warning, "Wwise - SetGameObjectAuxSendValues failed on object %" PRISIZE_T " with AKRESULT: %d", m_id, wwiseResult);
		}
...
ERequestStatus CListener::Set3DAttributes(SObject3DAttributes const& attributes)
{
	...
	if (!IS_WWISE_OK(wwiseResult))
	{
		Drx::Audio::Log(ELogType::Warning, "Wwise - CListener::Set3DAttributes failed with AKRESULT: %d", wwiseResult);
