...
#include <AK/SoundEngine/Common/AkMemoryMgr.h>       // Memory Upr
...
namespace AK
{
uk AllocHook(size_t in_size)
{
	return DrxModuleMalloc(in_size);
}

void FreeHook(uk in_ptr)
{
	DrxModuleFree(in_ptr);
}

uk VirtualAllocHook(uk in_pMemAddress, size_t in_size, DWORD in_dwAllocationType, DWORD in_dwProtect)
{
	return AllocHook(in_size);
}

void VirtualFreeHook(uk in_pMemAddress, size_t in_size, DWORD in_dwFreeType)
{
	FreeHook(in_pMemAddress);
}

#if DRX_PLATFORM_DURANGO
uk APUAllocHook(size_t in_size, u32 in_alignment)
{
	uk pAlloc = nullptr;

	#if defined(PROVIDE_WWISE_IMPL_SECONDARY_POOL)
	size_t const nSecondSize = DrxAudio::Impl::Wwise::g_audioImplMemoryPoolSecondary.MemSize();

	if (nSecondSize > 0)
	{
		size_t const nAllocHandle = DrxAudio::Impl::Wwise::g_audioImplMemoryPoolSecondary.Allocate<size_t>(in_size, in_alignment);

		if (nAllocHandle > 0)
		{
			pAlloc = DrxAudio::Impl::Wwise::g_audioImplMemoryPoolSecondary.Resolve<uk>(nAllocHandle);
			DrxAudio::Impl::Wwise::g_audioImplMemoryPoolSecondary.Item(nAllocHandle)->Lock();
		}
	}
	#endif  // PROVIDE_AUDIO_IMPL_SECONDARY_POOL

	DRX_ASSERT(pAlloc != nullptr);
	return pAlloc;
}

void APUFreeHook(uk in_pMemAddress)
{
	#if defined(PROVIDE_WWISE_IMPL_SECONDARY_POOL)
	size_t const nAllocHandle = DrxAudio::Impl::Wwise::g_audioImplMemoryPoolSecondary.AddressToHandle(in_pMemAddress);
	DrxAudio::Impl::Wwise::g_audioImplMemoryPoolSecondary.Free(nAllocHandle);
	#else
	DrxFatalError("%s", "<Audio>: Called APUFreeHook without secondary pool");
	#endif  // PROVIDE_AUDIO_IMPL_SECONDARY_POOL
}
  ...
  )
{
	...
	Drx::Audio::Log(
	  ((in_eErrorLevel& AK::Monitor::ErrorLevel_Error) != 0) ? DrxAudio::ELogType::Error : DrxAudio::ELogType::Comment,
...
public:
	...
	void Init(CImpl* const pImpl)
	{
		...
		if (!gEnv->pThreadUpr->SpawnThread(this, "AuxWwiseAudioThread"))
		{
			DrxFatalError(R"(Error spawning "AuxWwiseAudioThread" thread.)");
		}
	}
...
void CImpl::Update()
{
	if (AK::SoundEngine::IsInitialized())
	{
		...
		static i32 enableOutputCapture = 0;
...
///////////////////////////////////////////////////////////////////////////
ERequestStatus CImpl::Init(u32 const objectPoolSize, u32 const eventPoolSize)
{
	...
	{
		Drx::Audio::Log(ELogType::Error, "AK::MemoryMgr::Init() returned AKRESULT %d", wwiseResult);
		ShutDown();
	...
	{
		Drx::Audio::Log(ELogType::Error, "AK::MemoryMgr::CreatePool() PrepareEventMemoryPool failed!\n");
		ShutDown();
	...
	if (wwiseResult != AK_Success)
	{
		Drx::Audio::Log(ELogType::Error, "AK::MemoryMgr::SetPoolName() could not set name of event prepare memory pool!\n");
		ShutDown();
	...
	streamSettings.uMemorySize = g_cvars.m_streamUprMemoryPoolSize << 10; // 64 KiB is the default value!
	if (AK::StreamMgr::Create(streamSettings) == nullptr)
	{
		Drx::Audio::Log(ELogType::Error, "AK::StreamMgr::Create() failed!\n");
		ShutDown();
	...
	const SThreadConfig* pBankManger = pThreadConfigMngr->GetThreadConfig("Wwise_BankUpr");
...
#if defined(DRX_PLATFORM_POSIX)
		if (!(pDeviceThread->priority > 0 && pDeviceThread->priority <= 99))
			DrxWarning(VALIDATOR_MODULE_AUDIO, VALIDATOR_WARNING, "Thread 'Wwise_Device' requested priority %d when allowed range is 1 to 99. Applying default value of %d.", pDeviceThread->priority, deviceSettings.threadProperties.nPriority);
		else
#endif
	...
	if (wwiseResult != AK_Success)
	{
		Drx::Audio::Log(ELogType::Error, "m_fileIOHandler.Init() returned AKRESULT %d", wwiseResult);
		ShutDown();

		return ERequestStatus::Failure;
	}

	DrxFixedStringT<AK_MAX_PATH> temp = AUDIO_SYSTEM_DATA_ROOT "/";
	...
	g_cvars.m_panningRule = drxmath::clamp(g_cvars.m_panningRule, 0, 1);
	initSettings.settingsMainOutput.ePanningRule = static_cast<AkPanningRule>(g_cvars.m_panningRule);

	initSettings.bUseLEngineThread = g_cvars.m_enableEventUprThread > 0;
	initSettings.bUseSoundBankMgrThread = g_cvars.m_enableSoundBankUprThread > 0;

	// We need this additional thread during bank unloading if the user decided to run Wwise without the EventUpr thread.
	if (g_cvars.m_enableEventUprThread == 0)
	{
	...
	// Bank Upr thread settings
	if (pBankManger->paramActivityFlag & SThreadConfig::eThreadParamFlag_Affinity)
		platformInitSettings.threadBankUpr.dwAffinityMask = pBankManger->affinityFlag;
	...
	if (pBankManger->paramActivityFlag & SThreadConfig::eThreadParamFlag_Priority)
		...
		if (!(pBankManger->priority > 0 && pBankManger->priority <= 99))
			DrxWarning(VALIDATOR_MODULE_AUDIO, VALIDATOR_WARNING, "Thread 'Wwise_BankUpr' requested priority %d when allowed range is 1 to 99. Applying default value of %d.", pBankManger->priority, platformInitSettings.threadBankUpr.nPriority);
		else
#endif
		platformInitSettings.threadBankUpr.nPriority = pBankManger->priority;

	if (pBankManger->paramActivityFlag & SThreadConfig::eThreadParamFlag_StackSize)
		platformInitSettings.threadBankUpr.uStackSize = pBankManger->stackSizeBytes;
	...
	if (pLEngineThread->paramActivityFlag & SThreadConfig::eThreadParamFlag_Priority)
		...
		if (!(pLEngineThread->priority > 0 && pLEngineThread->priority <= 99))
			DrxWarning(VALIDATOR_MODULE_AUDIO, VALIDATOR_WARNING, "Thread 'Wwise_LEngine' requested priority %d when allowed range is 1 to 99. Applying default value of %d.", pLEngineThread->priority, platformInitSettings.threadLEngine.nPriority);
		else
#endif
	...
	if (pMonitorThread->paramActivityFlag & SThreadConfig::eThreadParamFlag_Priority)
		...
		if (!(pMonitorThread->priority > 0 && pMonitorThread->priority <= 99))
			DrxWarning(VALIDATOR_MODULE_AUDIO, VALIDATOR_WARNING, "Thread 'Wwise_Monitor' requested priority %d when allowed range is 1 to 99. Applying default value of %d.", pMonitorThread->priority, platformInitSettings.threadMonitor.nPriority);
		else
#endif
	...
	if (wwiseResult != AK_Success)
	{
		Drx::Audio::Log(ELogType::Error, "AK::SoundEngine::Init() returned AKRESULT %d", wwiseResult);
		ShutDown();
	...
	if (wwiseResult != AK_Success)
	{
		Drx::Audio::Log(ELogType::Error, "AK::MusicEngine::Init() returned AKRESULT %d", wwiseResult);
		ShutDown();
	...
	if (g_cvars.m_enableCommSystem == 1)
	{
		...
		if (wwiseResult != AK_Success)
		{
			Drx::Audio::Log(ELogType::Error, "AK::Comm::Init() returned AKRESULT %d. Communication between the Wwise authoring application and the game will not be possible\n", wwiseResult);
			m_bCommSystemInitialized = false;
		}
		...
		{
			AK::Comm::Term();
			Drx::Audio::Log(ELogType::Error, "AK::Monitor::SetLocalOutput() returned AKRESULT %d", wwiseResult);
			m_bCommSystemInitialized = false;
		}
	}
#endif  // !WWISE_FOR_RELEASE
...
#if defined(WWISE_USE_OCULUS)
	m_pOculusSpatializerLibrary = DrxLoadLibrary(OCULUS_SPATIALIZER_DLL);
	...
	if (m_pOculusSpatializerLibrary != nullptr)
	{
		...
		AkGetSoundEngineCallbacksType AkGetSoundEngineCallbacks =
		  (AkGetSoundEngineCallbacksType)DrxGetProcAddress(m_pOculusSpatializerLibrary, "AkGetSoundEngineCallbacks");
		...
		if (AkGetSoundEngineCallbacks != nullptr)
		{
			...
			if (AkGetSoundEngineCallbacks(AKEFFECTID_OCULUS, AKEFFECTID_OCULUS_SPATIALIZER, CreateOculusFX, CreateOculusFXParams))
			{
				...
				if (wwiseResult != AK_Success)
				{
					Drx::Audio::Log(ELogType::Error, "Failed to register OculusSpatializer plugin.");
				}
			}
			else
			{
				Drx::Audio::Log(ELogType::Error, "Failed call to AkGetSoundEngineCallbacks in " OCULUS_SPATIALIZER_DLL);
			}
			...
			if (AkGetSoundEngineCallbacks(AKEFFECTID_OCULUS, AKEFFECTID_OCULUS_SPATIALIZER_ATTACHMENT, CreateOculusFX, CreateOculusFXParams))
			{
				...
				if (wwiseResult != AK_Success)
				{
					Drx::Audio::Log(ELogType::Error, "Failed to register OculusSpatializer attachment.");
				}
			}
			else
			{
		...
		{
			Drx::Audio::Log(ELogType::Error, "Failed to load functions AkGetSoundEngineCallbacks in " OCULUS_SPATIALIZER_DLL);
		}
	}
	else
	{
		Drx::Audio::Log(ELogType::Error, "Failed to load " OCULUS_SPATIALIZER_DLL);
	}
#endif  // WWISE_USE_OCULUS
	...
	//if (wwiseResult != AK_Success)
	//{
	//	Drx::Audio::Log(eALT_WARNING, "AK::SoundEngine::RegisterGlobalCallback() returned AKRESULT %d", wwiseResult);
	//}
	...
	if (wwiseResult != AK_Success)
	{
		...
		Drx::Audio::Log(ELogType::Error, "Wwise failed to load Init.bnk, returned the AKRESULT: %d", wwiseResult);
		m_initBankId = AK_INVALID_BANK_ID;
	}
...
ERequestStatus CImpl::ShutDown()
{
	...
	if (m_bCommSystemInitialized)
	{
		...
		if (wwiseResult != AK_Success)
		{
			Drx::Audio::Log(ELogType::Warning, "AK::Monitor::SetLocalOutput() returned AKRESULT %d", wwiseResult);
		}
	...
	if (AK::SoundEngine::IsInitialized())
	{
		...
		if (wwiseResult != AK_Success)
		{
			Drx::Audio::Log(ELogType::Error, "Failed to clear banks\n");
		}
		...
		//if (wwiseResult != AK_Success)
		//{
		//	Drx::Audio::Log(eALT_WARNING, "AK::SoundEngine::UnregisterGlobalCallback() returned AKRESULT %d", wwiseResult);
		//}
	...
	// that lives in the Stream Upr, and unregisters itself as the File Location Resolver.
	...
	if (m_pOculusSpatializerLibrary != nullptr)
	{
		DrxFreeLibrary(m_pOculusSpatializerLibrary);
		m_pOculusSpatializerLibrary = nullptr;
	}
#endif  // WWISE_USE_OCULUS
	...
	if (pFileInfo != nullptr)
	{
		...
		if (pFileData != nullptr)
		{
			...
			else
			{
				...
				Drx::Audio::Log(ELogType::Error, "Failed to load file %s\n", pFileInfo->szFileName);
			}
		}
		else
		{
			Drx::Audio::Log(ELogType::Error, "Invalid AudioFileEntryData passed to the Wwise implementation of RegisterInMemoryFile");
		}
	}
...
ERequestStatus CImpl::UnregisterInMemoryFile(SFileInfo* const pFileInfo)
{
	...
	if (pFileInfo != nullptr)
	{
		...
		if (pFileData != nullptr)
		{
			...
			// If the EventUpr thread has been disabled the synchronous UnloadBank version will get stuck.
			if (g_cvars.m_enableEventUprThread > 0)
			{
			...
			{
				Drx::Audio::Log(ELogType::Error, "Wwise Failed to unregister in memory file %s\n", pFileInfo->szFileName);
			}
		}
		else
		{
			Drx::Audio::Log(ELogType::Error, "Invalid AudioFileEntryData passed to the Wwise implementation of UnregisterInMemoryFile");
		}
	}
...
IObject* CImpl::ConstructGlobalObject()
{
	...
	if (!IS_WWISE_OK(wwiseResult))
	{
		Drx::Audio::Log(ELogType::Warning, "Wwise ConstructGlobalObject failed with AKRESULT: %d", wwiseResult);
	}
#else
...
IObject* CImpl::ConstructObject(char const* const szName /*= nullptr*/)
{
	...
	if (!IS_WWISE_OK(wwiseResult))
	{
		Drx::Audio::Log(ELogType::Warning, "Wwise ConstructObject failed with AKRESULT: %d", wwiseResult);
	}
#else
	AK::SoundEngine::RegisterGameObj(m_gameObjectId);
#endif  // INCLUDE_WWISE_IMPL_PRODUCTION_CODE
...
void CImpl::DestructObject(IObject const* const pIObject)
{
	...
	if (!IS_WWISE_OK(wwiseResult))
	{
		Drx::Audio::Log(ELogType::Warning, "Wwise DestructObject failed with AKRESULT: %d", wwiseResult);
	}
...
IListener* CImpl::ConstructListener(char const* const szName /*= nullptr*/)
{
	...
	if (IS_WWISE_OK(wwiseResult))
	{
		...
		{
			Drx::Audio::Log(ELogType::Warning, "SetDefaultListeners failed with AKRESULT: %d", wwiseResult);
		}
	}
	else
	{
		Drx::Audio::Log(ELogType::Warning, "RegisterGameObj in ConstructListener failed with AKRESULT: %d", wwiseResult);
	}
#else
	AK::SoundEngine::RegisterGameObj(m_gameObjectId);
...
void CImpl::DestructListener(IListener* const pIListener)
{
	...
	if (!IS_WWISE_OK(wwiseResult))
	{
		Drx::Audio::Log(ELogType::Warning, "Wwise DestructListener failed with AKRESULT: %d", wwiseResult);
	}
...
{
	DRX_ASSERT(m_mapInputDevices.find(deviceUniqueID) == m_mapInputDevices.end()); // Mustn't exist yet!
	...
	else
	{
		...
		Drx::Audio::Log(ELogType::Error, "AK::SoundEngine::AddOutput failed! (%u : %u)", deviceUniqueID, deviceID);
	}
}
...
void CImpl::GamepadDisconnected(DeviceId const deviceUniqueID)
{
	...
	if (Iter != m_mapInputDevices.end())
	{
		if (Iter->second.akIsActiveOutputDevice)
		{
			...
			if (!IS_WWISE_OK(wwiseResult))
			{
				Drx::Audio::Log(ELogType::Error, "AK::SoundEngine::RemoveOutput failed in GamepadDisconnected! (%u : %u)", deviceUniqueID, deviceID);
			}
		}
...
ITrigger const* CImpl::ConstructTrigger(XmlNodeRef const pRootNode)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, "Unknown Wwise tag: %s", pRootNode->getTag());
	}
...
ISwitchState const* CImpl::ConstructSwitchState(XmlNodeRef const pRootNode)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, "Unknown Wwise tag: %s", szTag);
	}
...
IEnvironment const* CImpl::ConstructEnvironment(XmlNodeRef const pRootNode)
{
	...
	if (_stricmp(szTag, s_szAuxBusTag) == 0)
	{
		...
		{
			DRX_ASSERT(false); // Unknown AuxBus
		}
	}
	else if (_stricmp(szTag, s_szParameterTag) == 0)
	{
		...
		{
			DRX_ASSERT(false); // Unknown parameter
		}
	}
	else
	{
...
void CImpl::ParseRtpcImpl(XmlNodeRef const pNode, AkRtpcID& rtpcId, float& multiplier, float& shift)
{
	if (_stricmp(pNode->getTag(), s_szParameterTag) == 0)
	{
		...
		else
		{
			...
			Drx::Audio::Log(ELogType::Warning, "Invalid Wwise parameter name %s", szName);
		}
	}
	else
	{
		// Unknown Wwise RTPC tag!
		Drx::Audio::Log(ELogType::Warning, "Unknown Wwise tag %s", pNode->getTag());
	}
}
...
void CImpl::OnRefresh()
{
	...
	if (m_initBankId != AK_INVALID_BANK_ID)
	{
		...
		if (wwiseResult != AK_Success)
		{
			Drx::Audio::Log(ELogType::Error, "Wwise failed to unload Init.bnk, returned the AKRESULT: %d", wwiseResult);
		}
	}

	DrxFixedStringT<MaxFilePathLength> const temp("Init.bnk");
	AkOSChar const* szTemp = nullptr;
	CONVERT_CHAR_TO_OSCHAR(temp.c_str(), szTemp);
...
void CImpl::SetLanguage(char const* const szLanguage)
{
	if (szLanguage != nullptr)
	{
		...
		DrxFixedStringT<MaxFilePathLength> temp(m_localizedSoundBankFolder);
		temp += "/";
...
//////////////////////////////////////////////////////////////////////////
void CImpl::SetPanningRule(i32 const panningRule)
{
