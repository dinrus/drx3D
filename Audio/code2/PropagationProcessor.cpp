...
#if defined(INCLUDE_AUDIO_PRODUCTION_CODE)
u32 constexpr g_numIndices = 6;
vtx_idx constexpr g_auxIndices[g_numIndices] = { 2, 1, 0, 2, 3, 1 };
u32 constexpr g_numPoints = 4;
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
...
///////////////////////////////////////////////////////////////////////////
i32 CPropagationProcessor::OnObstructionTest(EventPhys const* pEvent)
{
...
#if defined(INCLUDE_AUDIO_PRODUCTION_CODE)
	m_listenerOcclusionPlaneColor.set(drx_random<u8>(0, 255), drx_random<u8>(0, 255), drx_random<u8>(0, 255), u8(64));
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
}
...
void CPropagationProcessor::SetOcclusionType(EOcclusionType const occlusionType, Vec3 const& listenerPosition)
{
	...
	if (CanRunObstructionOcclusion())
	{
		...
		static i32 const physicsFlags = ent_water | ent_static | ent_sleeping_rigid | ent_rigid | ent_terrain;
		...
		{
			ISurfaceTypeUpr* const pSurfaceTypeUpr = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr();
			DRX_ASSERT(rayInfo.numHits <= s_maxRayHits);
			...
			for (size_t i = 0; i < rayInfo.numHits; ++i)
			{
				...
				if (distance > 0.0f)
				{
					ISurfaceType* const pMat = pSurfaceTypeUpr->GetSurfaceType(rayInfo.hits[i].surface_idx);
...
//////////////////////////////////////////////////////////////////////////
bool DrxAudio::CPropagationProcessor::CanRunObstructionOcclusion() const
{
	return
	  m_occlusionType != EOcclusionType::None &&
...
{
	DRX_ASSERT((0 <= pAudioRayInfo->samplePosIndex) && (pAudioRayInfo->samplePosIndex < g_numRaySamplePositionsHigh));
	...
	{
		ISurfaceTypeUpr* const pSurfaceTypeUpr = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr();
		DRX_ASSERT(pAudioRayInfo->numHits <= s_maxRayHits);
		bool const accumulate = g_cvars.m_accumulateOcclusion > 0;
		...
		for (std::size_t i = 0; i < pAudioRayInfo->numHits; ++i)
		{
			...
			if (distance > 0.0f)
			{
				ISurfaceType* const pMat = pSurfaceTypeUpr->GetSurfaceType(pAudioRayInfo->hits[i].surface_idx);
	...
	{
		DrxFatalError("Negative ref or ray count on audio object");
	}
#endif
...
void CPropagationProcessor::ProcessObstructionOcclusion()
{
	m_occlusion = 0.0f;
	m_obstruction = 0.0f;
	DRX_ASSERT_MESSAGE(m_currentListenerDistance > 0.0f, "Distance to Listener is 0!");
  ...
  bool const bSynch)
{
	...
	// We use "rwi_max_piercing" to allow audio rays to always pierce surfaces regardless of the "pierceability" attribute.
	// Note: The very first entry of rayInfo.hits (solid slot) is always empty.
	i32 const numHits = gEnv->pPhysicalWorld->RayWorldIntersection(
	  origin,
		...
		case EOcclusionType::Adaptive:
			{
				...
				case EOcclusionType::High:
					...
					break;
				default:
					DRX_ASSERT_MESSAGE(false, "Calculated Adaptive Occlusion Type invalid");
					break;
				}
			}
			break;
...
void CPropagationProcessor::DrawDebugInfo(IRenderAuxGeom& auxGeom, EObjectFlags const objectFlags, Vec3 const& listenerPosition) const
{
	if ((g_cvars.m_drawAudioDebug & EAudioDebugDrawFilter::ShowOcclusionRays) != 0)
	{
		...
		{
			size_t const numConcurrentRays = GetNumConcurrentRays();
			DRX_ASSERT(numConcurrentRays > 0);
