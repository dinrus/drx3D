...
void CCVars::RegisterVariables()
{
	...
	m_fileCacheUprSize = 384 << 10;      // 384 MiB on PC
	...
	m_fileCacheUprSize = 384 << 10;      // 384 MiB on XboxOne
	m_audioObjectPoolSize = 256;
	m_audioEventPoolSize = 256;
	m_audioStandaloneFilePoolSize = 1;
	m_occlusionMaxSyncDistance = 10.0f;
	...
	m_fileCacheUprSize = 384 << 10;      // 384 MiB on PS4
	m_audioObjectPoolSize = 256;
	m_audioEventPoolSize = 256;
	m_audioStandaloneFilePoolSize = 1;
	m_occlusionMaxSyncDistance = 10.0f;
	...
	m_fileCacheUprSize = 384 << 10;      // 384 MiB on Mac
	m_audioObjectPoolSize = 256;
	m_audioEventPoolSize = 256;
	m_audioStandaloneFilePoolSize = 1;
	m_occlusionMaxSyncDistance = 10.0f;
	...
	m_fileCacheUprSize = 384 << 10;      // 384 MiB on Linux
	m_audioObjectPoolSize = 256;
	m_audioEventPoolSize = 256;
	m_audioStandaloneFilePoolSize = 1;
	m_occlusionMaxSyncDistance = 10.0f;
	...
	m_fileCacheUprSize = 384 << 10;      // 384 MiB on iOS
	m_audioObjectPoolSize = 256;
	m_audioEventPoolSize = 256;
	m_audioStandaloneFilePoolSize = 1;
	m_occlusionMaxSyncDistance = 10.0f;
	...
	m_fileCacheUprSize = 72 << 10;      // 72 MiB
	m_audioObjectPoolSize = 256;
	m_audioEventPoolSize = 256;
	m_audioStandaloneFilePoolSize = 1;
	m_occlusionMaxSyncDistance = 10.0f;
	...
	REGISTER_CVAR2("s_FileCacheUprSize", &m_fileCacheUprSize, m_fileCacheUprSize, VF_REQUIRE_APP_RESTART,
	               "Sets the size in KiB the AFCM will allocate on the heap.\n"
	               "Usage: s_FileCacheUprSize [0/...]\n"
	...
	REGISTER_CVAR2("s_DrawAudioDebug", &m_drawAudioDebug, 0, VF_CHEAT | VF_CHEAT_NOCHECK | VF_BITFIELD,
	               ...
	               "x: Show FileCache Upr debug info.\n"
	               );

	REGISTER_CVAR2("s_FileCacheUprDebugFilter", &m_fileCacheUprDebugFilter, 0, VF_CHEAT | VF_CHEAT_NOCHECK | VF_BITFIELD,
	               "Allows for filtered display of the different AFCM entries such as Globals, Level Specifics, Game Hints and so on.\n"
	               "Usage: s_FileCacheUprDebugFilter [0ab...] (flags can be combined)\n"
	               "Default: 0 (all)\n"
	               "a: Globals\n"
	               "b: Level Specifics\n"
	               "c: Game Hints\n");
...
void CCVars::UnregisterVariables()
{
	...
	if (pConsole != nullptr)
	{
		...
		pConsole->UnregisterVariable("s_FileCacheUprSize");
		...
		pConsole->UnregisterVariable("s_FileCacheUprDebugFilter");
...
{
	i32 const numArgs = pCmdArgs->GetArgCount();

	if ((numArgs == 2) || (numArgs == 3))
	{
		ControlId const triggerId = DrxAudio::StringToId(pCmdArgs->GetArg(1));
		gEnv->pAudioSystem->ExecuteTrigger(triggerId);
	}
	else
	{
		Drx::Audio::Log(ELogType::Error, "Usage: s_ExecuteTrigger [TriggerName]");
	}
}
...
void CCVars::CmdStopTrigger(IConsoleCmdArgs* pCmdArgs)
{
	...
	{
		Drx::Audio::Log(ELogType::Error, "Usage: s_StopTrigger [TriggerName]");
	}
}
...
void CCVars::CmdSetParameter(IConsoleCmdArgs* pCmdArgs)
{
	...
	{
		ControlId const parameterId = DrxAudio::StringToId(pCmdArgs->GetArg(1));
		double const value = atof(pCmdArgs->GetArg(2));
		gEnv->pAudioSystem->SetParameter(parameterId, static_cast<float>(value));
	}
	else
	{
		Drx::Audio::Log(ELogType::Error, "Usage: s_SetParameter [ParameterName] [ParameterValue]");
	}
}
...
void CCVars::CmdSetSwitchState(IConsoleCmdArgs* pCmdArgs)
{
	...
	if ((numArgs == 3) || (numArgs == 4))
	{
		ControlId const switchId = DrxAudio::StringToId(pCmdArgs->GetArg(1));
		SwitchStateId const switchStateId = DrxAudio::StringToId(pCmdArgs->GetArg(2));
		gEnv->pAudioSystem->SetSwitchState(switchId, switchStateId);
	}
	else
	{
		Drx::Audio::Log(ELogType::Error, "Usage: s_SetSwitchState [SwitchName] [SwitchStateName]");
	}
}
} //endns DrxAudio
