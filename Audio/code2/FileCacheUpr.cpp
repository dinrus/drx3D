...
#include <drx3D/Audio/FileCacheUpr.h>
...
{
//////////////////////////////////////////////////////////////////////////
CFileCacheUpr::CFileCacheUpr(AudioPreloadRequestLookup& preloadRequests)
...
//////////////////////////////////////////////////////////////////////////
void CFileCacheUpr::Init(Impl::IImpl* const pIImpl)
{
	m_pIImpl = pIImpl;
	AllocateHeap(static_cast<size_t>(g_cvars.m_fileCacheUprSize), "AudioFileCacheUpr");
}

//////////////////////////////////////////////////////////////////////////
void CFileCacheUpr::Release()
{
	DRX_ASSERT(m_audioFileEntries.empty());
	m_pIImpl = nullptr;
}

//////////////////////////////////////////////////////////////////////////
void CFileCacheUpr::AllocateHeap(size_t const size, char const* const szUsage)
{
	if (size > 0)
	{
#if DRX_PLATFORM_DURANGO
		m_pMemoryHeap = gEnv->pSystem->GetIMemoryUpr()->CreateCustomMemoryHeapInstance(IMemoryUpr::eapAPU);
#else
		m_pMemoryHeap = gEnv->pSystem->GetIMemoryUpr()->CreateCustomMemoryHeapInstance(IMemoryUpr::eapCustomAlignment);
...
//////////////////////////////////////////////////////////////////////////
FileEntryId CFileCacheUpr::TryAddFileCacheEntry(XmlNodeRef const pFileNode, EDataScope const dataScope, bool const bAutoLoad)
{
	FileEntryId fileEntryId = DrxAudio::InvalidFileEntryId;
	Impl::SFileInfo fileInfo;

	if (m_pIImpl->ConstructFile(pFileNode, &fileInfo) == ERequestStatus::Success)
	{
		DrxFixedStringT<MaxFilePathLength> fullPath(m_pIImpl->GetFileLocation(&fileInfo));
		fullPath += "/";
		...
		if (pFileEntry != nullptr)
		{
			...
			if (pExisitingFileEntry == nullptr)
			{
				...
				size_t const fileSize = gEnv->pDrxPak->FGetSize(pFileEntry->m_path.c_str());
				...
				{
					Drx::Audio::Log(ELogType::Warning, "Couldn't find audio file %s for pre-loading.", pFileEntry->m_path.c_str());
				}
			...
			else
			{
				if ((pExisitingFileEntry->m_flags & EFileFlags::UseCounted) > 0 && bAutoLoad)
				{
					...
					Drx::Audio::Log(ELogType::Always, R"(Upgraded file entry from "manual loading" to "auto loading": %s)", pExisitingFileEntry->m_path.c_str());
				}
...
//////////////////////////////////////////////////////////////////////////
bool CFileCacheUpr::TryRemoveFileCacheEntry(FileEntryId const audioFileEntryId, EDataScope const dataScope)
{
	bool bSuccess = false;
	...
	if (iter != m_audioFileEntries.end())
	{
		...
		else if ((dataScope == EDataScope::LevelSpecific) && ((pAudioFileEntry->m_flags & EFileFlags::NeedsResetToManualLoading) > 0))
		{
			...
			Drx::Audio::Log(ELogType::Always, R"(Downgraded file entry from "auto loading" to "manual loading": %s)", pAudioFileEntry->m_path.c_str());
		}
	}
...
//////////////////////////////////////////////////////////////////////////
void CFileCacheUpr::UpdateLocalizedFileCacheEntries()
{
...
//////////////////////////////////////////////////////////////////////////
ERequestStatus CFileCacheUpr::TryLoadRequest(PreloadRequestId const audioPreloadRequestId, bool const bLoadSynchronously, bool const bAutoLoadOnly)
{
...
//////////////////////////////////////////////////////////////////////////
ERequestStatus CFileCacheUpr::TryUnloadRequest(PreloadRequestId const audioPreloadRequestId)
{
	bool bFullSuccess = false;
	bool bFullFailure = true;
	CATLPreloadRequest* const pPreloadRequest = stl::find_in_map(m_preloadRequests, audioPreloadRequestId, nullptr);
...
//////////////////////////////////////////////////////////////////////////
ERequestStatus CFileCacheUpr::UnloadDataByScope(EDataScope const dataScope)
{
...
//////////////////////////////////////////////////////////////////////////
bool CFileCacheUpr::UncacheFileCacheEntryInternal(CATLAudioFileEntry* const pAudioFileEntry, bool const bNow, bool const bIgnoreUsedCount /* = false */)
{
	bool bSuccess = false;
	...
	if (pAudioFileEntry->m_useCount < 1 || bIgnoreUsedCount)
	{
		...
		{
			Drx::Audio::Log(ELogType::Always, "Trying to remove a loading file cache entry %s", pAudioFileEntry->m_path.c_str());
		...
		else if ((pAudioFileEntry->m_flags & EFileFlags::MemAllocFail) > 0)
		{
			...
			Drx::Audio::Log(ELogType::Always, "Resetting a memalloc-failed file cache entry %s", pAudioFileEntry->m_path.c_str());
...
//////////////////////////////////////////////////////////////////////////
void CFileCacheUpr::StreamAsyncOnComplete(IReadStream* pStream, u32 nError)
{
	// We "user abort" quite frequently so this is not something we want to assert on.
	DRX_ASSERT(nError == 0 || nError == ERROR_USER_ABORT);
...
#if defined(INCLUDE_AUDIO_PRODUCTION_CODE)
//////////////////////////////////////////////////////////////////////////
void CFileCacheUpr::DrawDebugInfo(IRenderAuxGeom& auxGeom, float const posX, float posY)
{
	if ((g_cvars.m_drawAudioDebug & EAudioDebugDrawFilter::ShowFileCacheUprInfo) > 0)
	{
		...
		DrxFixedStringT<MaxMiscStringLength> tempString;
		float time = 0.0f;
		float ratio = 0.0f;
		...
		auxGeom.Draw2dLabel(posX, posY, 1.5f, orange, false, "FileCacheUpr (%d of %d KiB) [Entries: %d]", static_cast<i32>(m_currentByteTotal >> 10), static_cast<i32>(m_maxByteTotal >> 10), static_cast<i32>(m_audioFileEntries.size()));
		posY += 16.0f;
		...
		if (!m_audioFileEntries.empty())
		{
			...
			bool const bDrawAll = (g_cvars.m_fileCacheUprDebugFilter == EAudioFileCacheUprDebugFilter::All);
			bool const bDrawUseCounted = ((g_cvars.m_fileCacheUprDebugFilter & EAudioFileCacheUprDebugFilter::UseCounted) > 0);
			bool const bDrawGlobals = (g_cvars.m_fileCacheUprDebugFilter & EAudioFileCacheUprDebugFilter::Globals) > 0;
			bool const bDrawLevelSpecifics = (g_cvars.m_fileCacheUprDebugFilter & EAudioFileCacheUprDebugFilter::LevelSpecifics) > 0;

			std::vector<DrxFixedStringT<MaxFilePathLength>> soundBanksSorted; // Create list to sort soundbanks alphabetically.
			...
			for (auto const& soundBankName : soundBanksSorted)
			{
				for (auto const& audioFileEntryPair : m_audioFileEntries)
				{
					if (audioFileEntryPair.second->m_path == soundBankName)
					{
						...
						DrxFixedStringT<MaxControlNameLength> lowerCaseSoundBankFileName(szSoundBankFileName);
						lowerCaseSoundBankFileName.MakeLower();

						if ((lowerCaseSearchString.empty() || (lowerCaseSearchString == "0")) || (lowerCaseSoundBankFileName.find(lowerCaseSearchString) != DrxFixedStringT<MaxControlNameLength>::npos))
						{
							bool bDraw = false;
...
//////////////////////////////////////////////////////////////////////////
bool CFileCacheUpr::DoesRequestFitInternal(size_t const requestSize)
{
	// Make sure these unsigned values don't flip around.
	DRX_ASSERT(m_currentByteTotal <= m_maxByteTotal);
	bool bSuccess = false;
...
//////////////////////////////////////////////////////////////////////////
bool CFileCacheUpr::FinishStreamInternal(IReadStreamPtr const pStream, i32 unsigned const error)
{
	bool bSuccess = false;

	FileEntryId const audioFileEntryId = static_cast<FileEntryId>(pStream->GetUserData());
	CATLAudioFileEntry* const pAudioFileEntry = stl::find_in_map(m_audioFileEntries, audioFileEntryId, nullptr);
	DRX_ASSERT(pAudioFileEntry != nullptr);
	...
	if (pAudioFileEntry != nullptr && (pAudioFileEntry->m_flags & EFileFlags::Loading) > 0)
	{
		...
		else if (error == ERROR_USER_ABORT)
		{
			...
			Drx::Audio::Log(ELogType::Always, "AFCM: user aborted stream for file %s (error: %u)", pAudioFileEntry->m_path.c_str(), error);
		}
		else
		{
			UncacheFileCacheEntryInternal(pAudioFileEntry, true, true);
			Drx::Audio::Log(ELogType::Error, "AFCM: failed to stream in file %s (error: %u)", pAudioFileEntry->m_path.c_str(), error);
		}
	}
...
//////////////////////////////////////////////////////////////////////////
bool CFileCacheUpr::AllocateMemoryBlockInternal(CATLAudioFileEntry* const __restrict pAudioFileEntry)
{
	// Must not have valid memory yet.
	DRX_ASSERT(pAudioFileEntry->m_pMemoryBlock == nullptr);
...
//////////////////////////////////////////////////////////////////////////
void CFileCacheUpr::UncacheFile(CATLAudioFileEntry* const pAudioFileEntry)
{
	...
	DRX_ASSERT(pAudioFileEntry->m_useCount == 0);
...
//////////////////////////////////////////////////////////////////////////
void CFileCacheUpr::TryToUncacheFiles()
{
	for (auto const& audioFileEntryPair : m_audioFileEntries)
	{
		CATLAudioFileEntry* const pAudioFileEntry = audioFileEntryPair.second;
...
///////////////////////////////////////////////////////////////////////////
void CFileCacheUpr::UpdateLocalizedFileEntryData(CATLAudioFileEntry* const pAudioFileEntry)
{
	...
	DrxFixedStringT<MaxFileNameLength> fileName(PathUtil::GetFile(pAudioFileEntry->m_path.c_str()));
	fileEntryInfo.pImplData = pAudioFileEntry->m_pImplData;
	...
	pAudioFileEntry->m_size = gEnv->pDrxPak->FGetSize(pAudioFileEntry->m_path.c_str());
	DRX_ASSERT(pAudioFileEntry->m_size > 0);
}

///////////////////////////////////////////////////////////////////////////
bool CFileCacheUpr::TryCacheFileCacheEntryInternal(
  ...
  size_t const useCount /*= 0*/)
{
	    ...
	    (pAudioFileEntry->m_flags & (EFileFlags::Cached | EFileFlags::Loading)) == 0)
	{
		if (DoesRequestFitInternal(pAudioFileEntry->m_size) && AllocateMemoryBlockInternal(pAudioFileEntry))
		{
			...
			streamReadParams.nSize = static_cast<i32 unsigned>(pAudioFileEntry->m_size);
		...
		else
		{
			...
			DRX_ASSERT(pAudioFileEntry->m_pMemoryBlock == nullptr || pAudioFileEntry->m_pMemoryBlock->GetData() == nullptr);
			...
			Drx::Audio::Log(ELogType::Error, R"(AFCM: could not cache "%s" as we are out of memory!)", pAudioFileEntry->m_path.c_str());
		}
	}
	else if ((pAudioFileEntry->m_flags & (EFileFlags::Cached | EFileFlags::Loading)) > 0)
	{
		...
		if ((pAudioFileEntry->m_flags & EFileFlags::Loading) > 0)
		{
			Drx::Audio::Log(ELogType::Warning, R"(AFCM: could not cache "%s" as it's already loading!)", pAudioFileEntry->m_path.c_str());
		}
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
	...
	{
		// The user should be made aware of it.
		Drx::Audio::Log(ELogType::Error, R"(AFCM: could not cache "%s" as it was not found at the target location!)", pAudioFileEntry->m_path.c_str());
	}
