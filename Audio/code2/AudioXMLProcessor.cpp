...
{
	DrxFixedStringT<MaxFilePathLength> sRootFolderPath(szFolderPath);
	...
	if (m_pIImpl != nullptr)
	{
		...
		DrxFixedStringT<MaxFilePathLength + MaxFileNameLength> sSearch(sRootFolderPath + "/*.xml");
		_finddata_t fd;
		intptr_t handle = gEnv->pDrxPak->FindFirst(sSearch.c_str(), &fd);

		if (handle != -1)
		{
			DrxFixedStringT<MaxFilePathLength + MaxFileNameLength> sFileName;
	...
	Drx::Audio::Log(ELogType::Comment, R"(Parsed controls data in "%s" for data scope "%s" in %.3f ms!)", szFolderPath, szDataScope, duration);
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
}
...
{
	i32 const rootChildCount = pRootNode->getChildCount();

	for (i32 i = 0; i < rootChildCount; ++i)
	{
		...
		if (pChildNode != nullptr)
		{
			...
			{
				Drx::Audio::Log(ELogType::Warning, "Unknown AudioSystemData node: %s", szChildNodeTag);
				DRX_ASSERT(false);
			}
		}
	}
...
void CAudioXMLProcessor::ParseDefaultControlsFile(XmlNodeRef const pRootNode)
{
	...
	for (i32 i = 0; i < rootChildCount; ++i)
	{
		...
		if (pChildNode != nullptr)
		{
			...
			{
				Drx::Audio::Log(ELogType::Warning, "Unknown AudioSystemData node: %s", childNodeTag);
				DRX_ASSERT(false);
			}
		}
	}
...
{
	DrxFixedStringT<MaxFilePathLength> rootFolderPath(szFolderPath);
	...
	if (m_pIImpl != nullptr)
	{
		...
		DrxFixedStringT<MaxFilePathLength + MaxFileNameLength> search(rootFolderPath + "/*.xml");
		_finddata_t fd;
		intptr_t const handle = gEnv->pDrxPak->FindFirst(search.c_str(), &fd);

		if (handle != -1)
		{
			DrxFixedStringT<MaxFilePathLength + MaxFileNameLength> fileName;
			...
			do
			{
				...
				if (pRootNode != nullptr)
				{
					if (_stricmp(pRootNode->getTag(), s_szRootNodeTag) == 0)
					{
						...
						i32 const numChildren = pRootNode->getChildCount();
						...
						for (i32 i = 0; i < numChildren; ++i)
						{
							...
							if (pChildNode != nullptr)
							{
								...
								if (_stricmp(szChildNodeTag, s_szPreloadsNodeTag) == 0)
								{
									...
									{
										DrxFixedStringT<MaxFilePathLength> const folderName(rootFolderPath.substr(lastSlashIndex + 1, rootFolderPath.size()));
	...
	float const duration = (gEnv->pTimer->GetAsyncTime() - startTime).GetMilliSeconds();
	Drx::Audio::Log(ELogType::Comment, R"(Parsed preloads data in "%s" for data scope "%s" in %.3f ms!)", szFolderPath, szDataScope, duration);
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
}
...
void CAudioXMLProcessor::ParsePreloads(XmlNodeRef const pPreloadDataRoot, EDataScope const dataScope, char const* const szFolderName, uint const version)
{
	...
	i32 const numPreloadRequests = pPreloadDataRoot->getChildCount();

	for (i32 i = 0; i < numPreloadRequests; ++i)
	{
		...
		if (pPreloadRequestNode && _stricmp(pPreloadRequestNode->getTag(), s_szPreloadRequestTag) == 0)
		{
			...
			if (preloadRequestId != InvalidPreloadRequestId)
			{
				...
				i32 const platformCount = pPreloadRequestNode->getChildCount();

				for (i32 j = 0; j < platformCount; ++j)
				{
				...
				if (pFileListParentNode != nullptr)
				{
					...
					i32 const fileCount = pFileListParentNode->getChildCount();

					CATLPreloadRequest::FileEntryIds cFileEntryIDs;
					cFileEntryIDs.reserve(fileCount);

					for (i32 k = 0; k < fileCount; ++k)
					{
						...
						{
							Drx::Audio::Log(ELogType::Warning, R"(Preload request "%s" could not create file entry from tag "%s"!)", szPreloadRequestName, pFileListParentNode->getChild(k)->getTag());
						}
					}
					...
					if (pPreloadRequest == nullptr)
					{
						...
						{
							DrxFatalError("<Audio>: Failed to allocate CATLPreloadRequest");
						}
					}
					else
					{
			...
			{
				Drx::Audio::Log(ELogType::Error, R"(Preload request "%s" already exists! Skipping this entry!)", szPreloadRequestName);
			}
		}
	}
}
...
{
	i32 const numEnvironments = pAudioEnvironmentRoot->getChildCount();

	for (i32 i = 0; i < numEnvironments; ++i)
	{
		...
		if ((pEnvironmentNode != nullptr) && (_stricmp(pEnvironmentNode->getTag(), s_szEnvironmentTag) == 0))
		{
			...
			if ((environmentId != InvalidControlId) && (stl::find_in_map(m_environments, environmentId, nullptr) == nullptr))
			{
				...
				i32 const numConnections = pEnvironmentNode->getChildCount();
			...
			{
				Drx::Audio::Log(ELogType::Error, R"(Environment "%s" already exists!)", szEnvironmentName);
			}
		}
	}
}
...
{
	i32 const numTriggers = pXMLTriggerRoot->getChildCount();

	for (i32 i = 0; i < numTriggers; ++i)
	{
		...
		if (pTriggerNode && _stricmp(pTriggerNode->getTag(), s_szTriggerTag) == 0)
		{
			...
			{
				i32 const numConnections = pTriggerNode->getChildCount();
				...
				for (i32 m = 0; m < numConnections; ++m)
				{
			...
			{
				Drx::Audio::Log(ELogType::Error, R"(Trigger "%s" already exists!)", szTriggerName);
			}
		}
	}
}
	...
	for (i32 i = 0; i < numTriggers; ++i)
	{
		...
		if (pTriggerNode && _stricmp(pTriggerNode->getTag(), s_szTriggerTag) == 0)
		{
			...
			if (numConnections == 0)
			{
				...
				{
					DRX_ASSERT_MESSAGE(false, R"(Default trigger "%s" does not exist.)", szTriggerName);
				}
			}
			else
			{
			...
			{
				DRX_ASSERT_MESSAGE(false, R"(Trigger "%s" could not get constructed.)", szTriggerName);
			}
		}
	}
}
...
{
	i32 const numSwitches = pXMLSwitchRoot->getChildCount();

	for (i32 i = 0; i < numSwitches; ++i)
	{
		...
		if (pSwitchNode && _stricmp(pSwitchNode->getTag(), s_szSwitchTag) == 0)
		{
			...
			if ((switchId != InvalidControlId) && (stl::find_in_map(m_switches, switchId, nullptr) == nullptr))
			{
				...
				i32 const numStates = pSwitchNode->getChildCount();

				for (i32 j = 0; j < numStates; ++j)
				{
					...
					if (pSwitchStateNode && _stricmp(pSwitchStateNode->getTag(), s_szStateTag) == 0)
					{
						...
						{
							i32 const numConnections = pSwitchStateNode->getChildCount();
							CATLSwitchState::ImplPtrVec connections;
							connections.reserve(numConnections);

							for (i32 k = 0; k < numConnections; ++k)
							{
			...
			{
				Drx::Audio::Log(ELogType::Error, R"(Switch "%s" already exists!)", szSwitchName);
			}
		}
	}
}
...
{
	i32 const numParameters = pXMLParameterRoot->getChildCount();
	...
	for (i32 i = 0; i < numParameters; ++i)
	{
		...
		if (pParameterNode && _stricmp(pParameterNode->getTag(), s_szParameterTag) == 0)
		{
			...
			{
				i32 const numConnections = pParameterNode->getChildCount();
			...
			{
				Drx::Audio::Log(ELogType::Error, R"(Parameter "%s" already exists!)", szParameterName);
			}
		}
	}
}
	...
	for (i32 i = 0; i < numParameters; ++i)
	{
		...
		if (pParameterNode && _stricmp(pParameterNode->getTag(), s_szParameterTag) == 0)
		{
			...
			if (numConnections == 0)
			{
				...
				{
					DRX_ASSERT_MESSAGE(false, R"(Default Parameter "%s" does not exist.)", szParameterName);
				}
			}
			else
			{
			...
			{
				Drx::Audio::Log(ELogType::Error, R"(Parameter "%s" could not get constructed!)", szParameterName);
			}
		}
	}
}
