...
namespace DrxAudio
{
TriggerInstanceId DrxAudio::CATLAudioObject::s_triggerInstanceIdCounter = 1;
CSystem* DrxAudio::CATLAudioObject::s_pAudioSystem = nullptr;
CAudioEventUpr* DrxAudio::CATLAudioObject::s_pEventUpr = nullptr;
CAudioStandaloneFileUpr* DrxAudio::CATLAudioObject::s_pStandaloneFileUpr = nullptr;
  ...
  ERequestFlags const flags)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, "Reported a started instance %u that couldn't be found on an object", audioTriggerInstanceId);
	}
}
...
void CATLAudioObject::ReportStartedEvent(CATLEvent* const pEvent)
{
	...
	if (iter != m_triggerStates.end())
	{
		...
		case EEventState::PlayingDelayed:
			{
				DRX_ASSERT(audioTriggerInstanceState.numLoadingEvents > 0);
...
void CATLAudioObject::ReportFinishedEvent(CATLEvent* const pEvent, bool const bSuccess)
{
	...
	if (FindPlace(m_triggerStates, pEvent->m_audioTriggerInstanceId, iter))
	{
		...
		case EEventState::Playing:
			{
				SAudioTriggerInstanceState& audioTriggerInstanceState = iter->second;
				DRX_ASSERT(audioTriggerInstanceState.numPlayingEvents > 0);
	...
	else
	{
		...
		{
#if defined(INCLUDE_AUDIO_PRODUCTION_CODE)
			Drx::Audio::Log(ELogType::Warning, "Reported finished event on an inactive trigger %s", pEvent->m_pTrigger->m_name.c_str());
#else
			Drx::Audio::Log(ELogType::Warning, "Reported finished event on an inactive trigger %u", pEvent->m_pTrigger->GetId());
#endif  // INCLUDE_AUDIO_PRODUCTION_CODE
		}
		else
		{
			Drx::Audio::Log(ELogType::Warning, "Reported finished event on a trigger that does not exist anymore");
		}
	}
}
  ...
  ERequestFlags const flags)
{
	...
	{
		CATLEvent* const pEvent = s_pEventUpr->ConstructAudioEvent();
		...
		{
			s_pEventUpr->ReleaseEvent(pEvent);
	...
	if (result != ERequestStatus::Success)
	{
		...
		Drx::Audio::Log(ELogType::Warning, R"(Trigger "%s" failed on AudioObject "%s")", pTrigger->m_name.c_str(), m_name.c_str());
	}
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
...
ERequestStatus CATLAudioObject::HandleSetSwitchState(CATLSwitch const* const pSwitch, CATLSwitchState const* const pState)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, R"(Failed to set the ATLSwitch "%s" to ATLSwitchState "%s" on AudioObject "%s")", pSwitch->m_name.c_str(), pState->m_name.c_str(), m_name.c_str());
	}
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
...
ERequestStatus CATLAudioObject::HandleSetParameter(CParameter const* const pParameter, float const value)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, R"(Failed to set the Audio Parameter "%s" to %f on Audio Object "%s")", pParameter->m_name.c_str(), value, m_name.c_str());
	}
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
...
ERequestStatus CATLAudioObject::HandleSetEnvironment(CATLAudioEnvironment const* const pEnvironment, float const amount)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, R"(Failed to set the ATLAudioEnvironment "%s" to %f on AudioObject "%s")", pEnvironment->m_name.c_str(), amount, m_name.c_str());
	}
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
...
ERequestStatus CATLAudioObject::LoadTriggerAsync(CATLTrigger const* const pTrigger, bool const bLoad)
{
	...
	if (result != ERequestStatus::Success)
	{
		...
		Drx::Audio::Log(ELogType::Warning, R"(LoadTriggerAsync failed on AudioObject "%s")", m_name.c_str());
	}
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
...
ERequestStatus CATLAudioObject::HandleResetEnvironments(AudioEnvironmentLookup const& environmentsLookup)
{
	...
	{
		Drx::Audio::Log(ELogType::Warning, R"(Failed to Reset AudioEnvironments on AudioObject "%s")", m_name.c_str());
	}
#endif // INCLUDE_AUDIO_PRODUCTION_CODE
...
{
	SAudioTriggerInstanceState& audioTriggerInstanceState = iter->second;
	SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportFinishedTriggerInstance> requestData(audioTriggerInstanceState.audioTriggerId);
...
{
	DRX_ASSERT(calcType != EOcclusionType::None);
...
//////////////////////////////////////////////////////////////////////////
ERequestStatus CATLAudioObject::HandlePlayFile(CATLStandaloneFile* const pFile, uk const pOwner, uk const pUserData, uk const pUserDataOwner)
{
	...
	{
#if defined(INCLUDE_AUDIO_PRODUCTION_CODE)
		Drx::Audio::Log(ELogType::Warning, R"(PlayFile failed with "%s" on AudioObject "%s")", pFile->m_hashedFilename.GetText().c_str(), m_name.c_str());
#endif //INCLUDE_AUDIO_PRODUCTION_CODE

		s_pStandaloneFileUpr->ReleaseStandaloneFile(pFile);
	}
	...
	while (iter != iterEnd)
	{
		...
		if (pStandaloneFile != nullptr && pStandaloneFile->m_hashedFilename == hashedFilename)
		{
			...
			if (pStandaloneFile->m_state != EAudioStandaloneFileState::Playing)
			{
				...
				Drx::Audio::Log(ELogType::Warning, R"(Request to stop a standalone audio file that is not playing! State: "%s")", szState);
			}
#endif  //INCLUDE_AUDIO_PRODUCTION_CODE
			...
			if (tempStatus != ERequestStatus::Pending)
			{
				...
				s_pStandaloneFileUpr->ReleaseStandaloneFile(pStandaloneFile);
...
i32 const CATLAudioObject::CStateDebugDrawData::s_maxToMinUpdates = 100;
	...
	if ((screenPos.z >= 0.0f) && (screenPos.z <= 1.0f))
	{
		...
		if ((g_cvars.m_debugDistance <= 0.0f) || ((g_cvars.m_debugDistance > 0.0f) && (distance <= g_cvars.m_debugDistance)))
		{
			...
			if (bShowLabel || bFilterAllObjectInfo)
			{
				...
				bObjectNameMatchesFilter = (lowerCaseObjectName.find(lowerCaseSearchString) != DrxFixedStringT<MaxControlNameLength>::npos);
			}

			// Check if any trigger matches text filter.
			bool bTriggerMatchesFilter = false;
			std::vector<DrxFixedStringT<MaxMiscStringLength>> triggerInfo;
			...
			if ((bShowTriggers && !m_triggerStates.empty()) || bFilterAllObjectInfo)
			{
				...
				for (auto const& triggerCountsPair : triggerCounts)
				{
					...
					if (pTrigger != nullptr)
					{
						...
						if (lowerCaseTriggerName.find(lowerCaseSearchString) != DrxFixedStringT<MaxControlNameLength>::npos)
						{
							bTriggerMatchesFilter = true;
						}

						DrxFixedStringT<MaxMiscStringLength> debugText;
			...
			std::vector<DrxFixedStringT<MaxMiscStringLength>> standaloneFileInfo;
			...
			if ((bShowStandaloneFiles && !m_activeStandaloneFiles.empty()) || bFilterAllObjectInfo)
			{
				...
				for (auto const& numInstancesPair : numStandaloneFiles)
				{
					...
					if (lowerCaseStandaloneFileName.find(lowerCaseSearchString) != DrxFixedStringT<MaxControlNameLength>::npos)
					{
					...
					if (pSwitch != nullptr)
					{
						...
						if (pSwitchState != nullptr)
						{
							if (!pSwitch->m_name.empty() && !pSwitchState->m_name.empty())
							{
								...
								DrxFixedStringT<MaxControlNameLength> lowerCaseSwitchName(szSwitchName);
								lowerCaseSwitchName.MakeLower();
								char const* const szStateName = pSwitchState->m_name.c_str();
								DrxFixedStringT<MaxControlNameLength> lowerCaseStateName(szStateName);
								lowerCaseStateName.MakeLower();

								if ((lowerCaseSwitchName.find(lowerCaseSearchString) != DrxFixedStringT<MaxControlNameLength>::npos) ||
								    (lowerCaseStateName.find(lowerCaseSearchString) != DrxFixedStringT<MaxControlNameLength>::npos))
								{
			...
			if ((bShowParameters && !m_parameters.empty()) || bFilterAllObjectInfo)
			{
				for (auto const& parameterPair : m_parameters)
				{
					...
					if (pParameter != nullptr)
					{
						...
						DrxFixedStringT<MaxControlNameLength> lowerCaseParameterName(szParameterName);
						lowerCaseParameterName.MakeLower();

						if (lowerCaseParameterName.find(lowerCaseSearchString) != DrxFixedStringT<MaxControlNameLength>::npos)
						{
			...
			if ((bShowEnvironments && !m_environments.empty()) || bFilterAllObjectInfo)
			{
				for (auto const& environmentPair : m_environments)
				{
					...
					if (pEnvironment != nullptr)
					{
						...
						DrxFixedStringT<MaxControlNameLength> lowerCaseEnvironmentName(szEnvironmentName);
						lowerCaseEnvironmentName.MakeLower();

						if (lowerCaseEnvironmentName.find(lowerCaseSearchString) != DrxFixedStringT<MaxControlNameLength>::npos)
						{
  ...
  bool const bSet3DAttributes)
{
	...
	for (auto const& parameterPair : audioParameters)
	{
		...
		if (pParameter != nullptr)
		{
			...
			if (result != ERequestStatus::Success)
			{
				Drx::Audio::Log(ELogType::Warning, R"(Parameter "%s" failed during audio middleware switch on AudioObject "%s")", pParameter->m_name.c_str(), m_name.c_str());
			}
		}
	}
	...
	for (auto const& switchPair : audioSwitches)
	{
		...
		if (pSwitch != nullptr)
		{
			...
			if (pState != nullptr)
			{
				...
				if (result != ERequestStatus::Success)
				{
					Drx::Audio::Log(ELogType::Warning, R"(SwitchStateImpl "%s" : "%s" failed during audio middleware switch on AudioObject "%s")", pSwitch->m_name.c_str(), pState->m_name.c_str(), m_name.c_str());
				}
			}
		}
	}
	...
	for (auto const& environmentPair : audioEnvironments)
	{
		...
		if (pEnvironment != nullptr)
		{
			...
			if (result != ERequestStatus::Success)
			{
				Drx::Audio::Log(ELogType::Warning, R"(Environment "%s" failed during audio middleware switch on AudioObject "%s")", pEnvironment->m_name.c_str(), m_name.c_str());
			}
		}
	}
		...
		if (pTrigger != nullptr)
		{
			if (!pTrigger->m_implPtrs.empty())
			{
				for (auto const pTriggerImpl : pTrigger->m_implPtrs)
				{
					...
					{
						Drx::Audio::Log(ELogType::Warning, R"(TriggerImpl "%s" failed during audio middleware switch on AudioObject "%s")", pTrigger->m_name.c_str(), m_name.c_str());
						s_pEventUpr->ReleaseEvent(pEvent);
					}
				}
			}
			else
			{
				...
				Drx::Audio::Log(ELogType::Warning, R"(No trigger connections found during audio middleware switch for "%s" on "%s")", pTrigger->m_name.c_str(), m_name.c_str());
			}
		}
		else
		{
	...
	for (auto const& standaloneFilePair : activeStandaloneFiles)
	{
		...
		{
			DRX_ASSERT(pStandaloneFile->m_state == EAudioStandaloneFileState::Playing);
			...
			{
				Drx::Audio::Log(ELogType::Warning, R"(PlayFile failed with "%s" on AudioObject "%s")", pStandaloneFile->m_hashedFilename.GetText().c_str(), m_name.c_str());
			}
		}
		else
		{
			Drx::Audio::Log(ELogType::Error, "Retrigger active standalone audio files failed on instance: %u and file: %u as m_audioStandaloneFileMgr.LookupID() returned nullptr!", standaloneFilePair.first, standaloneFilePair.second);
		}
	}
}
...
//////////////////////////////////////////////////////////////////////////
void CATLAudioObject::StopTrigger(ControlId const triggerId /* = DrxAudio::InvalidControlId */, SRequestUserData const& userData /* = SAudioRequestUserData::GetEmptyObject() */)
{
