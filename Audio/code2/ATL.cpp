...
#include <drx3D/Audio/ATL.h>
#include <drx3D/Audio/AudioImpl.h>
...
inline ERequestResult ConvertToRequestResult(ERequestStatus const eAudioRequestStatus)
{
	...
	default:
		{
			Drx::Audio::Log(ELogType::Error, "Invalid AudioRequestStatus '%u'. Cannot be converted to an AudioRequestResult. ", eAudioRequestStatus);
			DRX_ASSERT(false);
			result = ERequestResult::Failure;
			break;
		}
	...
	, m_xmlProcessor(m_triggers, m_parameters, m_switches, m_environments, m_preloadRequests, m_fileCacheMgr, m_internalControls)
{
	if (g_cvars.m_audioObjectPoolSize < 1)
	{
		...
		Drx::Audio::Log(ELogType::Warning, R"(Audio Object pool size should be greater than zero. Forcing the cvar "s_AudioObjectPoolSize" to 1!)");
	}
	...
	if (g_cvars.m_audioEventPoolSize < 1)
	{
		...
		Drx::Audio::Log(ELogType::Warning, R"(Audio Event pool size should be greater than zero. Forcing the cvar "s_AudioEventPoolSize" to 1!)");
	}
	...
	if (g_cvars.m_audioStandaloneFilePoolSize < 1)
	{
		...
		Drx::Audio::Log(ELogType::Warning, R"(Audio Standalone File pool size should be greater than zero. Forcing the cvar "s_AudioStandaloneFilePoolSize" to 1!)");
	}
...
CAudioTranslationLayer::~CAudioTranslationLayer()
{
	...
	DRX_ASSERT_MESSAGE(!m_pGlobalAudioObject, "Global audio object should have been destroyed in the audio thread, before the ATL is destructed.");
}
...
bool CAudioTranslationLayer::Initialize(CSystem* const pAudioSystem)
{
	...
	CATLAudioObject::s_pEventUpr = &m_audioEventMgr;
	CATLAudioObject::s_pAudioSystem = pAudioSystem;
	CATLAudioObject::s_pStandaloneFileUpr = &m_audioStandaloneFileMgr;
...
void CAudioTranslationLayer::ProcessRequest(CAudioRequest& request)
{
	...
	if (request.GetData())
	{
		...
		case EAudioRequestType::AudioCallbackUprRequest:
			{
				result = ProcessAudioCallbackUprRequest(request);

				break;
			}
		case EAudioRequestType::AudioUprRequest:
			{
				result = ProcessAudioUprRequest(request);

				break;
			}
		default:
			{
				Drx::Audio::Log(ELogType::Error, "Unknown audio request type: %d", static_cast<i32>(request.GetData()->type));
				DRX_ASSERT(false);
...
void CAudioTranslationLayer::NotifyListener(CAudioRequest const& request)
{
	...
	case EAudioRequestType::AudioUprRequest:
		{
			SAudioUprRequestDataBase const* const pRequestDataBase = static_cast<SAudioUprRequestDataBase const* const>(request.GetData());

			switch (pRequestDataBase->type)
			{
			case EAudioUprRequestType::SetAudioImpl:
	...
	case EAudioRequestType::AudioCallbackUprRequest:
		{
			SAudioCallbackUprRequestDataBase const* const pRequestDataBase = static_cast<SAudioCallbackUprRequestDataBase const* const>(request.GetData());

			switch (pRequestDataBase->type)
			{
			case EAudioCallbackUprRequestType::ReportFinishedTriggerInstance:
				{
					SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportFinishedTriggerInstance> const* const pRequestData = static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportFinishedTriggerInstance> const* const>(pRequestDataBase);
					audioControlID = pRequestData->audioTriggerId;
					audioSystemEvent = ESystemEvents::TriggerFinished;

					break;
				}
			case EAudioCallbackUprRequestType::ReportStartedEvent:
				{
					SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStartedEvent> const* const pRequestData = static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStartedEvent> const* const>(pRequestDataBase);
					pAudioEvent = &pRequestData->audioEvent;

					break;
				}
			case EAudioCallbackUprRequestType::ReportStartedFile:
				{
					SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStartedFile> const* const pRequestData = static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStartedFile> const* const>(pRequestDataBase);
					pStandaloneFile = &pRequestData->audioStandaloneFile;
					audioSystemEvent = ESystemEvents::FileStarted;

					break;
				}
			case EAudioCallbackUprRequestType::ReportStoppedFile:
				{
					SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStoppedFile> const* const pRequestData = static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStoppedFile> const* const>(pRequestDataBase);
					pStandaloneFile = &pRequestData->audioStandaloneFile;
	...
	default:
		{
			DrxFatalError("Unknown request type during CAudioTranslationLayer::NotifyListener!");
...
///////////////////////////////////////////////////////////////////////////
ERequestStatus CAudioTranslationLayer::ProcessAudioUprRequest(CAudioRequest const& request)
{
	ERequestStatus result = ERequestStatus::Failure;
	SAudioUprRequestDataBase const* const pRequestDataBase = static_cast<SAudioUprRequestDataBase const*>(request.GetData());

	switch (pRequestDataBase->type)
	{
	case EAudioUprRequestType::AddRequestListener:
		{
			SAudioUprRequestData<EAudioUprRequestType::AddRequestListener> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::AddRequestListener> const*>(request.GetData());
			result = m_audioEventListenerMgr.AddRequestListener(pRequestData);

			break;
		}
	case EAudioUprRequestType::RemoveRequestListener:
		{
			SAudioUprRequestData<EAudioUprRequestType::RemoveRequestListener> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::RemoveRequestListener> const*>(request.GetData());
	...
	case EAudioUprRequestType::SetAudioImpl:
		{
			SAudioUprRequestData<EAudioUprRequestType::SetAudioImpl> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::SetAudioImpl> const*>(request.GetData());
			result = SetImpl(pRequestData->pIImpl);

			break;
		}
	case EAudioUprRequestType::RefreshAudioSystem:
		{
			SAudioUprRequestData<EAudioUprRequestType::RefreshAudioSystem> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::RefreshAudioSystem> const*>(request.GetData());
			result = RefreshAudioSystem(pRequestData->levelName);

			break;
		}
	case EAudioUprRequestType::StopAllSounds:
		{
			result = m_pIImpl->StopAllSounds();

			break;
		}
	case EAudioUprRequestType::ParseControlsData:
		{
			SAudioUprRequestData<EAudioUprRequestType::ParseControlsData> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::ParseControlsData> const*>(request.GetData());
			result = ParseControlsData(pRequestData->folderPath.c_str(), pRequestData->dataScope);

			break;
		}
	case EAudioUprRequestType::ParsePreloadsData:
		{
			SAudioUprRequestData<EAudioUprRequestType::ParsePreloadsData> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::ParsePreloadsData> const*>(request.GetData());
			result = ParsePreloadsData(pRequestData->folderPath.c_str(), pRequestData->dataScope);

			break;
		}
	case EAudioUprRequestType::ClearControlsData:
		{
			SAudioUprRequestData<EAudioUprRequestType::ClearControlsData> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::ClearControlsData> const*>(request.GetData());
			result = ClearControlsData(pRequestData->dataScope);

			break;
		}
	case EAudioUprRequestType::ClearPreloadsData:
		{
			SAudioUprRequestData<EAudioUprRequestType::ClearPreloadsData> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::ClearPreloadsData> const*>(request.GetData());
			result = ClearPreloadsData(pRequestData->dataScope);

			break;
		}
	case EAudioUprRequestType::PreloadSingleRequest:
		{
			SAudioUprRequestData<EAudioUprRequestType::PreloadSingleRequest> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::PreloadSingleRequest> const*>(request.GetData());
	...
	case EAudioUprRequestType::UnloadSingleRequest:
		{
			SAudioUprRequestData<EAudioUprRequestType::UnloadSingleRequest> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::UnloadSingleRequest> const*>(request.GetData());
			result = m_fileCacheMgr.TryUnloadRequest(pRequestData->audioPreloadRequestId);

			break;
		}
	case EAudioUprRequestType::UnloadAFCMDataByScope:
		{
			SAudioUprRequestData<EAudioUprRequestType::UnloadAFCMDataByScope> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::UnloadAFCMDataByScope> const*>(request.GetData());
			result = m_fileCacheMgr.UnloadDataByScope(pRequestData->dataScope);

			break;
		}
	case EAudioUprRequestType::ReleaseAudioImpl:
		{
			ReleaseImpl();
	...
	case EAudioUprRequestType::ChangeLanguage:
		{
			SetImplLanguage();
	...
	case EAudioUprRequestType::RetriggerAudioControls:
		{
#if defined(INCLUDE_AUDIO_PRODUCTION_CODE)
	...
	case EAudioUprRequestType::ReleasePendingRays:
		{
	...
	case EAudioUprRequestType::ReloadControlsData:
		{
#if defined(INCLUDE_AUDIO_PRODUCTION_CODE)
			SAudioUprRequestData<EAudioUprRequestType::ReloadControlsData> const* const pRequestData = static_cast<SAudioUprRequestData<EAudioUprRequestType::ReloadControlsData> const*>(request.GetData());
	...
	case EAudioUprRequestType::DrawDebugInfo:
		{
#if defined(INCLUDE_AUDIO_PRODUCTION_CODE)
	...
	case EAudioUprRequestType::GetAudioFileData:
		{
			SAudioUprRequestData<EAudioUprRequestType::GetAudioFileData> const* const pRequestData =
			  static_cast<SAudioUprRequestData<EAudioUprRequestType::GetAudioFileData> const* const>(request.GetData());
			m_pIImpl->GetFileData(pRequestData->name.c_str(), pRequestData->fileData);
			break;
		}
	case EAudioUprRequestType::GetImplInfo:
		{
			SAudioUprRequestData<EAudioUprRequestType::GetImplInfo> const* const pRequestData =
			  static_cast<SAudioUprRequestData<EAudioUprRequestType::GetImplInfo> const* const>(request.GetData());
			m_pIImpl->GetInfo(pRequestData->implInfo);
			break;
		}
	case EAudioUprRequestType::None:
		{
			result = ERequestStatus::Success;
	...
	default:
		{
			Drx::Audio::Log(ELogType::Warning, "ATL received an unknown AudioUpr request: %u", pRequestDataBase->type);
...
///////////////////////////////////////////////////////////////////////////
ERequestStatus CAudioTranslationLayer::ProcessAudioCallbackUprRequest(CAudioRequest& request)
{
	ERequestStatus result = ERequestStatus::Failure;
	SAudioCallbackUprRequestDataBase const* const pRequestDataBase =
	  static_cast<SAudioCallbackUprRequestDataBase const* const>(request.GetData());

	switch (pRequestDataBase->type)
	{
	case EAudioCallbackUprRequestType::ReportStartedEvent:
		{
			SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStartedEvent> const* const pRequestData =
			  static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStartedEvent> const* const>(request.GetData());
	...
	case EAudioCallbackUprRequestType::ReportFinishedEvent:
		{
			SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportFinishedEvent> const* const pRequestData =
			  static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportFinishedEvent> const* const>(request.GetData());
	...
	case EAudioCallbackUprRequestType::ReportVirtualizedEvent:
		{
			SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportVirtualizedEvent> const* const pRequestData =
			  static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportVirtualizedEvent> const* const>(request.GetData());
	...
	case EAudioCallbackUprRequestType::ReportPhysicalizedEvent:
		{
			SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportPhysicalizedEvent> const* const pRequestData =
			  static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportPhysicalizedEvent> const* const>(request.GetData());
	...
	case EAudioCallbackUprRequestType::ReportStartedFile:
		{
			SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStartedFile> const* const pRequestData =
			  static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStartedFile> const* const>(request.GetData());
	...
	case EAudioCallbackUprRequestType::ReportStoppedFile:
		{
			SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStoppedFile> const* const pRequestData =
			  static_cast<SAudioCallbackUprRequestData<EAudioCallbackUprRequestType::ReportStoppedFile> const* const>(request.GetData());
	...
	case EAudioCallbackUprRequestType::ReportFinishedTriggerInstance:
	case EAudioCallbackUprRequestType::None:
		{
			result = ERequestStatus::Success;
	...
	default:
		{
			result = ERequestStatus::FailureInvalidRequest;
			Drx::Audio::Log(ELogType::Warning, "ATL received an unknown AudioCallbackUpr request: %u", pRequestDataBase->type);
...
ERequestStatus CAudioTranslationLayer::ProcessAudioObjectRequest(CAudioRequest const& request)
{
	...
	case EAudioObjectRequestType::SetTransformation:
		{
			...
			{
				Drx::Audio::Log(ELogType::Warning, "ATL received a request to set a position on a global object");
			}
	...
	case EAudioObjectRequestType::SetEnvironment:
		{
			...
			{
				Drx::Audio::Log(ELogType::Warning, "ATL received a request to set an environment on a global object");
			}
	...
	case EAudioObjectRequestType::RegisterObject:
		{
			...
			DRX_ASSERT(result == ERequestStatus::Success);
	...
	case EAudioObjectRequestType::ReleaseObject:
		{
			...
			{
				Drx::Audio::Log(ELogType::Warning, "ATL received a request to release the GlobalAudioObject");
			}
	...
	default:
		{
			Drx::Audio::Log(ELogType::Warning, "ATL received an unknown AudioObject request type: %u", pBaseRequestData->type);
			result = ERequestStatus::FailureInvalidRequest;
			break;
		}
	}
...
ERequestStatus CAudioTranslationLayer::ProcessAudioListenerRequest(SAudioRequestData const* const pPassedRequestData)
{
	...
	case EAudioListenerRequestType::SetTransformation:
		{
			...
			DRX_ASSERT(pRequestData->pListener != nullptr);
	...
	default:
		result = ERequestStatus::FailureInvalidRequest;
		Drx::Audio::Log(ELogType::Warning, "ATL received an unknown AudioListener request type: %u", pPassedRequestData->type);
		break;
	}
...
ERequestStatus CAudioTranslationLayer::SetImpl(Impl::IImpl* const pIImpl)
{
	...
	if (m_pIImpl == nullptr)
	{
		Drx::Audio::Log(ELogType::Warning, "nullptr passed to SetImpl, will run with the null implementation");

		auto const pImpl = new Impl::Null::CImpl();
		DRX_ASSERT(pImpl != nullptr);
	...
	if (result != ERequestStatus::Success)
	{
		...
		Drx::Audio::Log(ELogType::Error, "Failed to set the AudioImpl %s. Will run with the null implementation.", m_implInfo.name.c_str());
	...
	DRX_ASSERT(m_pGlobalAudioObject->GetImplDataPtr() == nullptr);
...
{
	Drx::Audio::Log(ELogType::Warning, "Beginning to refresh the AudioSystem!");
	...
	{
		DrxFixedStringT<MaxFilePathLength> levelPath = m_configPath;
		...
		if (result != ERequestStatus::Success)
		{
			Drx::Audio::Log(ELogType::Warning, R"(No preload request found for level - "%s"!)", szLevelName);
		}
	}

	Drx::Audio::Log(ELogType::Warning, "Done refreshing the AudioSystem!");
...
{
	IAreaUpr* const pIAreaUpr = gEnv->pEntitySystem->GetAreaUpr();
	size_t numAreas = 0;
	static size_t const s_maxAreas = 10;
	static SAudioAreaInfo s_areaInfos[s_maxAreas];

	if (pIAreaUpr->QueryAudioAreas(pObject->GetTransformation().GetPosition(), s_areaInfos, s_maxAreas, numAreas))
	{
	...
	if (pAuxGeom)
	{
		...
		if (!((g_cvars.m_drawAudioDebug & EAudioDebugDrawFilter::HideMemoryInfo) != 0))
		{
			...
			pAuxGeom->Draw2dLabel(posX, posY, textSize, s_colorWhite, false,
			                      ...
			                      static_cast<u32>((memInfo.allocated - memInfo.freed) / 1024));
			...
			if (m_pIImpl != nullptr)
			{
				...
				pAuxGeom->Draw2dLabel(posX, posY, textSize, s_colorWhite, false, "[Impl] Total Memory Used: %uKiB | Secondary Memory: %.2f / %.2f MiB | NumAllocs: %d",
				                      static_cast<u32>(memoryInfo.totalMemory / 1024),
				                      (memoryInfo.secondaryPoolUsedSize / 1024) / 1024.0f,
				                      (memoryInfo.secondaryPoolSize / 1024) / 1024.0f,
				                      static_cast<i32>(memoryInfo.secondaryPoolAllocations));
		...
		if ((g_cvars.m_drawAudioDebug & EAudioDebugDrawFilter::ShowFileCacheUprInfo) != 0)
		{
			debugDraw += "File Cache Upr, ";
		}
