...
const u64 D3_SINT8 = 0x1;
const u64 D3_SINT16 = 0x2;
const u64 D3_SINT24 = 0x4;
const u64 D3_SINT32 = 0x8;
const u64 D3_FLOAT32 = 0x10;
const u64 D3_FLOAT64 = 0x20;
...
double b3ReadWavFile::interpolate(double frame, u32 channel) const
{
	i32 iIndex = (i32)frame;                        // integer part of index
...
double b3ReadWavFile::tick(u32 channel, b3WavTicker *ticker)
{
	if (ticker->finished_) return 0.0;

	if (ticker->time_ < 0.0 || ticker->time_ > (double)(this->m_numFrames - 1.0))
	{
		for (i32 i = 0; i < ticker->lastFrame_.size(); i++) ticker->lastFrame_[i] = 0.0;
	...
	if (interpolate_)
	{
		for (i32 i = 0; i < ticker->lastFrame_.size(); i++)
...
bool b3ReadWavFile::getWavInfo(tukk fileName)
{
	...
	{
		if (temp == 32)
			dataType_ = D3_FLOAT32;
		else if (temp == 64)
			dataType_ = D3_FLOAT64;
	}
	if (dataType_ == 0)
	{
...
bool b3ReadWavFile::read(u64 startFrame, bool doNormalize)
{
	...
	u64 nFrames = this->m_numFrames;  //m_frames.frames();
	if (nFrames == 0)
	{
	...
	u64 offset = startFrame * channels_;

	// Read samples into StkFrames data m_frames.
	if (dataType_ == D3_SINT16)
	{
		i16* buf = (i16*)&m_frames[0];
	...
	else if (dataType_ == D3_SINT32)
	{
		i32* buf = (i32*)&m_frames[0];
	...
	else if (dataType_ == D3_SINT8 && wavFile_)
	{  // 8-bit WAV data is unsigned!
		u8 *buf = (u8*)&m_frames[0];
