...
DRX_CREATE_ENUM_FLAG_OPERATORS(ETriggerStatus);
...
public:
	...
	ERequestStatus   HandleExecuteTrigger(CATLTrigger const* const pTrigger, uk const pOwner = nullptr, uk const pUserData = nullptr, uk const pUserDataOwner = nullptr, ERequestFlags const flags = ERequestFlags::None);
	...
	ERequestStatus   HandlePlayFile(CATLStandaloneFile* const pFile, uk const pOwner = nullptr, uk const pUserData = nullptr, uk const pUserDataOwner = nullptr);
	...
	void                           ReportStartedTriggerInstance(TriggerInstanceId const audioTriggerInstanceId, uk const pOwnerOverride, uk const pUserData, uk const pUserDataOwner, ERequestFlags const flags);
	void                           ReportStartedEvent(CATLEvent* const pEvent);
	void                           ReportFinishedEvent(CATLEvent* const pEvent, bool const bSuccess);
	void                           ReportStartedStandaloneFile(CATLStandaloneFile* const pStandaloneFile, uk const pOwner, uk const pUserData, uk const pUserDataOwner);
	...
	void IncrementSyncCallbackCounter() { DrxInterlockedIncrement(&m_numPendingSyncCallbacks); }
	void DecrementSyncCallbackCounter() { DRX_ASSERT(m_numPendingSyncCallbacks >= 1); DrxInterlockedDecrement(&m_numPendingSyncCallbacks); }

	static CSystem*                     s_pAudioSystem;
	static CAudioEventUpr*          s_pEventUpr;
	static CAudioStandaloneFileUpr* s_pStandaloneFileUpr;
...
public:
	...
	DrxFixedStringT<MaxObjectNameLength> m_name;
...
private:
	...
	private:
		...
		static i32 const   s_maxToMinUpdates;
	};
