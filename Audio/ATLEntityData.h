...
/**
 * Реализация можкт использовать этот интерфейс для определения класса для хранения
 * специфичных для неё данных, требуемых для идентифицирования и использования
 * соответствующего объекта DrxAudio::IListener (напр., a middleware-specific unique ID)
 */
struct IListener
{
	/** @cond */
	 ...
	 * @param attributes - a struct containing the audio listener's transformation
	 *  and velocity
	 * @return ERequestStatus::Success if the AudioListener's position
	 *  has been successfully set, ERequestStatus::Failure otherwise
	 */
 ...
 * the DinrusXAudioSys and an implementation
 * Note: This struct cannot define a constructor, it needs to be a POD!
 */
struct SFileInfo
{
	uk       pFileData;            // pointer to the memory location of the file's contents
