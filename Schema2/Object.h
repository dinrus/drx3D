...
#include <drx3D/Schema2/IUpdateScheduler.h>
...
namespace sxema2
{
	...
	public:
		...
		virtual IAnyConstPtr GetProperty(u32 propertyIdx) const override;
	...
	private:
		...
		struct SPreviewProperties : public IObjectPreviewProperties
		{
			...
			{
				SComponentPreviewProperties(tukk _componentName, const IComponentPreviewPropertiesPtr& _pData);
		...
		typedef TemplateUtils::CScopedConnectionArray<TSignalObserver>	TSignalObserverConnectionArray; // #SchematycTODO : Replace with CScopedConnectionUpr!
		...
		{
		public:
			typedef i32 StateNetIdx;
			enum : StateNetIdx { INVALID_NET_IDX = -1 };
		private:
			typedef std::vector<StateNetIdx> TStateIndexVector;
			// TODO pavloi 2017.11.03: pedantically, second value should be size_t instead of i32, but this way whole element is 16 bytes
			typedef VectorMap<std::pair<size_t, StateNetIdx>, i32> TReverseStateIndexMap;
		...
		void NetworkSerialize(TSerialize serialize, i32 aspects, u8 profile, i32 flags);
