...
#include <drx3D/Schema2/IScriptFile.h> // #SchematycTODO : Remove once all graphs are script extensions!!!
...
namespace sxema2
{
	...
	{
		inline SScriptGraphParams(const SGUID& _scopeGUID, tukk _szName, EScriptGraphType _type, const SGUID& _contextGUID)
	...
	typedef TemplateUtils::CDelegate<void (tukk , tukk , EScriptGraphPortFlags, const CAggregateTypeId&)> ScriptGraphNodeOptionalOutputEnumerator;
	...
	struct IScriptGraphNode // #SchematycTODO : Clean up this interface!!!
	{
		...
		virtual tukk GetComment() const { return nullptr; }
		virtual void SetName(tukk) = 0; // #SchematycTODO : Remove!!!
		...
		virtual u32 FindInput(tukk szName) const = 0;
		...
		virtual u32 FindOutput(tukk szName) const = 0;
		...
		virtual size_t AddOptionalOutput(tukk szName, EScriptGraphPortFlags flags, const CAggregateTypeId& typeId) = 0;
	...
	struct IScriptGraph // #SchematycTODO : Clean up this interface!!!
	{
		...
		virtual tukk GetAvailableNodeName(size_t availableNodeIdx) const = 0;
		...
		virtual bool CanAddLink(const SGUID& srcNodeGUID, tukk szSrcOutputName, const SGUID& dstNodeGUID, tukk szDstInputName) const = 0;
		virtual IScriptGraphLink* AddLink(const SGUID& srcNodeGUID, tukk szSrcOutputName, const SGUID& dstNodeGUID, tukk szDstInputName) = 0;
		virtual void RemoveLink(size_t iLink) = 0;
		virtual void RemoveLinks(const SGUID& nodeGUID) = 0;
		virtual size_t GetLinkCount() const = 0;
		virtual size_t FindLink(const SGUID& srcNodeGUID, tukk srcOutputName, const SGUID& dstNodeGUID, tukk szDstInputName) const = 0;	// #SchematycTODO : Remove!?!
	...
	struct IScriptGraphExtension : public IScriptExtension, public IScriptGraph
	{
		...
		virtual void VisitInputLinks(const ScriptGraphLinkVisitor& visitor, const SGUID& dstNodeGUID, tukk szDstInputName) = 0;
		virtual void VisitInputLinks(const ScriptGraphLinkConstVisitor& visitor, const SGUID& dstNodeGUID, tukk szDstInputName) const = 0;
		virtual void VisitOutputLinks(const ScriptGraphLinkVisitor& visitor, const SGUID& srcNodeGUID, tukk szSrcOutputName) = 0;
		virtual void VisitOutputLinks(const ScriptGraphLinkConstVisitor& visitor, const SGUID& srcNodeGUID, tukk szSrcOutputName) const = 0;
