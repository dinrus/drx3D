...
namespace sxema2
{
	...
	private:
		...
		i32               m_gridSize; // The grid size. Its a square so m_gridSize x m_gridSize
		i32               m_gridSizeMinusOne; // Cached constantly needed value
		i32               m_updateHalfDistance; // The update distance around the relevant cells in cells (e.g. 1, 2... cells)

#if DEBUG_RELEVANCE_GRID
		i32               m_debugUpdateFrame;
#endif
	};
	...
	private:
		...
		struct SSlot
		{
			...
			SSlot(CUpdateScope* _pScope, u32 _firstBucketIdx, UpdateFrequency _frequency, UpdatePriority _priority);
