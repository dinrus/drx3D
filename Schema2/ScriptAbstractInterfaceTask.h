...
namespace sxema2
{
	class CScriptAbstractInterfaceTask : public CScriptElementBase<IScriptAbstractInterfaceTask>
	{
	public:

		// #SchematycTODO : Create two separate constructors: one default (before loading) and one for when element is created in editor.
		CScriptAbstractInterfaceTask(IScriptFile& file, const SGUID& guid = SGUID(), const SGUID& scopeGUID = SGUID(), tukk szName = nullptr);
	...
	DECLARE_SHARED_POINTERS(CScriptAbstractInterfaceTask)
}
