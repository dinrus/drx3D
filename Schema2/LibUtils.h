...
namespace sxema2
{
	namespace NodeUtils
	{
		inline bool CanGoToNodeType(sxema2::EScriptGraphNodeType nodeType)
		{
			return nodeType == sxema2::EScriptGraphNodeType::Function || nodeType == sxema2::EScriptGraphNodeType::Condition || nodeType == sxema2::EScriptGraphNodeType::Get || nodeType == sxema2::EScriptGraphNodeType::Set ||
				nodeType == sxema2::EScriptGraphNodeType::SendSignal || nodeType == sxema2::EScriptGraphNodeType::ProcessSignal || nodeType == sxema2::EScriptGraphNodeType::BroadcastSignal;
		}
	}
	...
	namespace LibUtils
	{
		struct SReferencesLogInfo
		{
			struct SRefInfo
			{
				...
				SRefInfo(const SLogMetaItemGUID& gotoItem, const SLogMetaChildGUID gobackItem, tukk szPath)
					: gotoItem(gotoItem)
		...
		{
			SXEMA2_COMMENT(sxema2::LOG_STREAM_DEFAULT, references.header.c_str());
			for (const SReferencesLogInfo::SRefInfo& ref : references.references)
			{
				SXEMA2_METAINFO_COMMENT(sxema2::LOG_STREAM_DEFAULT, CLogMessageMetaInfo(EDrxLinkCommand::Goto, ref.gotoItem, ref.gobackItem), ref.pathLink.c_str());
			}
			SXEMA2_COMMENT(sxema2::LOG_STREAM_DEFAULT, references.footer.c_str());
		}

		inline void FindReferences(SGUID refGuid, SGUID refGoBack, tukk szItemName, const sxema2::IScriptFile* pFile, SReferencesLogInfo& result)
		{
		...
		inline void FindAndLogReferences(SGUID refGuid, SGUID refGoBack, tukk szItemName, const sxema2::IScriptFile* pFile, bool searchInAllFiles = false)
		{
		...
		inline SGUID FindStateMachineByName(const ILibClass& libClass, tukk szName)
		{
			DRX_ASSERT(szName);
			if(szName)
			{
