...
#include <drx3D/Schema2/SharedArray.h>
#include <drx3D/Schema2/ToString.h>
...
namespace sxema2
{
	...
	public:
		...
		{
			return sxema2::ToString(m_value, output);
		}
	...
	template <typename TYPE, u32 SIZE> class CAnyArrayExtension<TYPE[SIZE]> : public IAnyArrayExtension
	{
	public:
		...
		virtual IAny* CreateElement(uk pPlacement) const override
		{
		...
		virtual IAny* CloneElement(u32 idx, uk pPlacement) const override
		{
			return idx < SIZE ? new (pPlacement) CAny<TYPE>(m_value[idx]) : nullptr;
		}

		virtual IAnyPtr CloneElement(u32 idx) const override
		{
	...
	template <typename TYPE, u32 SIZE> class CAny<TYPE[SIZE]> : public IAny
	{
	public:
		...
		virtual CTypeInfo GetTypeInfo() const override
		{
			return sxema2::GetTypeInfo<ArrayType>();
		}
		...
		virtual bool ToString(const CharArrayView& output) const override
		{
			return sxema2::ToString(m_arrayExtension, output);
		}
	...
	public:
		...
		virtual CTypeInfo GetTypeInfo() const override
		{
			return sxema2::GetTypeInfo<void>();
		}
	...
	template <typename TYPE, u32 SIZE> CAny<TYPE[SIZE]> MakeAny()
	{
		return CAny<TYPE[SIZE]>(DefaultInit<TYPE[SIZE]>());
	}

	template <typename TYPE, u32 SIZE> CAny<TYPE[SIZE]> MakeAny(const TYPE (&value)[SIZE])
	{
	...
	template <typename TYPE, u32 SIZE> std::shared_ptr<CAny<TYPE[SIZE]> > MakeAnyShared()
	{
		return IAnyPtr(std::make_shared<CAny<TYPE[SIZE]> >(DefaultInit<TYPE[SIZE]>()));
	}

	template <typename TYPE, u32 SIZE> std::shared_ptr<CAny<TYPE[SIZE]> > MakeAnyShared(const TYPE (&value)[SIZE])
	{
		return IAnyPtr(std::make_shared<CAny<TYPE[SIZE]> >(value));
	}

	template <typename TYPE> inline bool Serialize(Serialization::IArchive& archive, CAny<TYPE>& value, tukk szName, tukk szLabel)
	{
