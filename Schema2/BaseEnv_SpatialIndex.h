...
namespace SchematycBaseEnv
{
	...
	enum class ESpatialVolumeTags : u32 // Hunt specific! How can we register custom tags from game dll?
	{
		None  = 0,
	...
	WRAP_TYPE(u32, SpatialVolumeId, ~0);
	...
	typedef std::vector<i32>                                           SpatialVolumeTagValues;
	...
	private:
		...
		struct SSettings : public sxema2::IEnvSettings
		{
		...
		typedef std::vector<u32>             VolumeIdxs;
	...
	private:
		...
		SpatialVolumeId GenerateVolumeId(u32 volumeIdx);
		u32 VolumeIdxFromId(const SpatialVolumeId& id) const;
		bool CompareVolumeIds(const SpatialVolumeId& lhs, const SpatialVolumeId& rhs) const;
		void BroadPhaseQuery(u32 volumeIdx, ESpatialVolumeTags tags, VolumeIdxs& results) const;
	...
	private:
		...
		static u32k s_volumeIdxMask;
		static u32k s_volumeSaltShift;
		static u32k s_volumeSaltMask;
	...
	//Matrix34 CreateVolumeTM(const Matrix34& worldTM, ICharacterInstance* pCharacter = nullptr, i32 boneId = -1);
}
