...
#include <drx3D/Schema2/IEnvFunctionDescriptor.h>
...
namespace sxema2
{
	namespace EnvFunctionDescriptorUtils
	{
		...
		typedef SEnvFunctionResult (*StubPtr)(const SBinding&, const SEnvFunctionContext&, uk , const EnvFunctionInputs&, const EnvFunctionOutputs&);
		...
		struct SBinding
		{
			...
			inline void BindInput(u32 paramIdx, u32 id, tukk szName, tukk szDescription, const IAnyPtr& pValue) // #SchematycTODO : Should this function be global?
			{
				// #SchematycTODO : Make sure id is unique and name is valid!!!

				SXEMA2_SYSTEM_ASSERT_FATAL(paramIdx < paramCount);
				SXEMA2_SYSTEM_ASSERT_FATAL(szName && szDescription);

				SParamBinding& param = params[paramIdx];
				SXEMA2_SYSTEM_ASSERT_FATAL((param.flags & EParamFlags::AvailableInput) != 0);
			...
			inline void BindOutput(u32 paramIdx, u32 id, tukk szName, tukk szDescription) // #SchematycTODO : Should this function be global?
			{
				// #SchematycTODO : Make sure id is unique and name is valid!!!
				...
				SParamBinding& param = params[paramIdx];
				SXEMA2_SYSTEM_ASSERT_FATAL((param.flags & EParamFlags::AvailableOutput) != 0);
			...
			u32            inputCount;
			u32            inputs[s_maxEnvFunctionParams];
			u32            outputCount;
			u32            outputs[s_maxEnvFunctionParams];
		};

		template <typename TYPE> inline const typename SParamTraits<TYPE>::UnqualifiedType& InputParam(const SBinding& binding, u32 paramIdx, const EnvFunctionInputs& inputs)
		{
		...
		template <typename TYPE> inline void OutputParam(const SBinding& binding, u32 paramIdx, const EnvFunctionOutputs& outputs, const TYPE& value)
		{
			const SParamBinding& param = binding.params[paramIdx];
		...
		struct SBinder<SEnvFunctionResult (*)(const SEnvFunctionContext& context)>
		{
			...
			static SEnvFunctionResult Stub(const SBinding& binding, const SEnvFunctionContext& context, uk pObject, const EnvFunctionInputs& inputs, const EnvFunctionOutputs& outputs)
			{
	...
	public:
		...
		virtual SEnvFunctionResult Execute(const SEnvFunctionContext& context, uk pObject, const EnvFunctionInputs& inputs, const EnvFunctionOutputs& outputs) const override
		{
		...
		inline void SetNamespace(tukk szNamespace)
		{
			m_namespace = szNamespace;
		}

		virtual void SetFileName(tukk szFileName, tukk szProjectDir)
		{
			StringUtils::MakeProjectRelativeFileName(szFileName, szProjectDir, m_fileName);
		}

		inline void SetAuthor(tukk szAuthor)
		{
			m_author = szAuthor;
		}
		...
		inline u32 GetParamCount() const
		{
			return m_binding.paramCount;
		}

		inline void BindInput(u32 paramIdx, u32 id, tukk szName, tukk szDescription)
		{
		...
		inline void BindOutput(u32 paramIdx, u32 id, tukk szName, tukk szDescription)
		{
