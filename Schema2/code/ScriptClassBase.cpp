...
namespace sxema2
{
	...
	namespace ScriptClassBaseUtils
	{
		...
		inline const IScriptGroup* FindGroup(IScriptFile& file, const SGUID& scopeGUID, tukk szName)
		{
	...
	bool CScriptClassBase::SetName(tukk szName)
	{
		return false;
	}

	tukk CScriptClassBase::GetName() const
	{
		return m_name.c_str();
	}
			...
			if(baseProperty.second->GetOverridePolicy() != EOverridePolicy::Finalize)
			{
				...
				else
				{
					...
					while(baseScopeGUID != m_refGUID)
					{
						...
						SXEMA2_SYSTEM_ASSERT(pBaseGroup);
						if(pBaseGroup)
						{
					...
					{
						tukk         szGroupName = pBaseGroup->GetName();
