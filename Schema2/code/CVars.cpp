...
namespace sxema2
{
	...
	i32    CVars::sc_MaxRecursionDepth        = 100;
	...
	i32    CVars::sc_RelevanceGridDebugStatic = 0;
	i32    CVars::sc_UseNewGraphPipeline      = 0;

	static tukk g_szDisplayCriticalErrorsDescription = "Display/hide sxema critical error message alerts\n"
																													  "\t0 = hide\n"
																													  "\t1 = display\n";

	void CVars::Register()
	{
		sc_FileFormat = REGISTER_STRING("sc_FileFormat", "xml", VF_READONLY, "Format of sxema files");
		sc_RootFolder = REGISTER_STRING("sc_RootFolder", "libs/schematyc", VF_READONLY, "Root folder for sxema files");
		...
		sc_LogToFile = REGISTER_INT("sc_LogToFile", 0, VF_NULL, "Configure sxema log file output\n\t0 = disabled\n\t1 = write to file\n\tforward to standard log");
		...
		sc_FunctionTimeLimit = REGISTER_CVAR(sc_FunctionTimeLimit, sc_FunctionTimeLimit, VF_NULL, "Display critical error if sxema function takes more than x(s) to process")->GetFVal();
