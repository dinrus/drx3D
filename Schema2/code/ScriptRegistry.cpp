...
namespace sxema2
{
	...
	CNewScriptFile::CNewScriptFile(const SGUID& guid, tukk szName)
		: m_guid(guid)
		, m_name(szName)
	...
	tukk CNewScriptFile::GetName() const
	{
		return m_name;
	}
	...
	u32 CNewScriptFile::GetElementCount() const
	{
	...
	CScriptRegistry::CScriptRegistry()
	{
		...
		REGISTER_COMMAND("sc_SaveAllScriptFiles", ScriptRegistryUtils::SaveAllScriptFilesCommand, VF_NULL, "Save all sxema script file regardless of whether they have been modified");
	}

	IScriptFile* CScriptRegistry::LoadFile(tukk szFileName)
	{
		SXEMA2_SYSTEM_ASSERT(szFileName);
		if(szFileName)
		{
			...
			u32k         crc32 = CCrc32::Compute(fileNameFormatted.c_str());
			FilesByCRC::iterator itFile = m_filesByCRC.find(crc32);
			if(itFile != m_filesByCRC.end())
			{
				return itFile->second.get();
			}
			if(gEnv->pDrxPak->IsFileExist(fileNameFormatted.c_str()))
			{
				const bool     bOnDisk = gEnv->pDrxPak->IsFileExist(fileNameFormatted.c_str(), IDrxPak::eFileLocation_OnDisk);
	...
	IScriptFile* CScriptRegistry::CreateFile(tukk szFileName, EScriptFileFlags flags)
	{
	...
	IScriptFile* CScriptRegistry::GetFile(tukk szFileName)
	{
		SXEMA2_SYSTEM_ASSERT(szFileName);
		if(szFileName)
		{
	...
	void CScriptRegistry::VisitFiles(const ScriptFileVisitor& visitor, tukk szFilePath)
	{
		...
		if(visitor)
		{
			for(FilesByCRC::iterator itFile = m_filesByCRC.begin(), itEndFile = m_filesByCRC.end(); itFile != itEndFile; ++ itFile)
			{
				...
				if(!szFilePath || DrxStringUtils::stristr(file.GetFileName(), szFilePath))
				{
	...
	void CScriptRegistry::VisitFiles(const ScriptFileConstVisitor& visitor, tukk szFilePath) const
	{
		SXEMA2_SYSTEM_ASSERT(visitor);
		if(visitor)
		{
	...
	bool SortScriptInputElementsByDependency(ScriptInputElementPtrs& elements)
	{
		...
		{
			SXEMA2_SYSTEM_CRITICAL_ERROR("%d recursive dependencies detected!", recursiveDependencyCount);
			return false;
		}
	...
	bool CScriptRegistry::Load()
	{
		...
		if(gEnv->pSchematyc2->IsExperimentalFeatureEnabled("QtEditor"))
		{
			...
			auto loadJsonFile = [this, &blocks] (tukk szFileName, unsigned attributes)
			{
			...
			auto loadXmlFile = [this, &blocks] (tukk szFileName, unsigned attributes)
			{
				SScriptInputBlock           block;
				CScriptInputBlockSerializer blockSerializer(block);
			...
			tukk szScriptFolder = gEnv->pSchematyc2->GetScriptsFolder();
	...
	void CScriptRegistry::Save(bool bAlwaysSave)
	{
		...
		if(gEnv->pSchematyc2->IsExperimentalFeatureEnabled("QtEditor"))
		{
			tukk szPath = gEnv->pSchematyc2->GetScriptsFolder();
	...
	IScriptModule* CScriptRegistry::AddModule(tukk szName, IScriptElement* pScope)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor() && szName);
	...
	IScriptEnumeration* CScriptRegistry::AddEnumeration(tukk szName, IScriptElement* pScope)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor() && szName);
		if(gEnv->IsEditor() && szName)
		{
	...
	IScriptClass* CScriptRegistry::AddClass(tukk szName, const SGUID& foundationGUID, IScriptElement* pScope)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor() && szName);
		if(gEnv->IsEditor() && szName)
		{
	...
	CScriptFilePtr CScriptRegistry::CreateFile(tukk szFileName, const SGUID& guid, EScriptFileFlags flags)
	{
		...
		if(szFileName)
		{
			...
			string fileNameLowerCase = fileNameFormatted;
			fileNameLowerCase.MakeLower();
			u32k crc32 = CCrc32::Compute(fileNameLowerCase.c_str());
	...
	void CScriptRegistry::EnumFile(tukk szFileName, unsigned attributes)
	{
		LOADING_TIME_PROFILE_SECTION;
		const bool bOnDisk = (attributes & _A_IN_DRXPAK) == 0;
	...
	void CScriptRegistry::RegisterFileByGuid(const CScriptFilePtr& pFile)
	{
		...
		if(guid)
		{
			...
			{
				SXEMA2_SYSTEM_CRITICAL_ERROR("Script files have duplicate guids: %s, %s", pFile->GetFileName(), itOtherFile->second->GetFileName());
			}
		}
		else
		{
			SXEMA2_SYSTEM_CRITICAL_ERROR("Script file has no guid: %s", pFile->GetFileName());
		}
	}

	INewScriptFile* CScriptRegistry::CreateNewFile(tukk szFileName, const SGUID& guid)
	{
	...
	INewScriptFile* CScriptRegistry::CreateNewFile(tukk szFileName)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	void CScriptRegistry::SaveElementRecursive(IScriptElement& element, tukk szPath)
	{
		...
		tukk szElementName = element.GetName();
		if(ScriptRegistryUtils::IsFileElement(element.GetElementType()))
		{
			stack_string fullPath = gEnv->pDrxPak->GetGameFolder();
			fullPath.append("/");
			fullPath.append(szPath);
			gEnv->pDrxPak->MakeDir(fullPath.c_str());
