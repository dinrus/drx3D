...
namespace sxema2
{
	...
	{
		SXEMA2_SYSTEM_ASSERT_FATAL(pFunctionDescriptor);
		const SGUID guid = pFunctionDescriptor->GetGUID();
		SXEMA2_SYSTEM_ASSERT_FATAL(!GetFunction(guid));
	...
	//////////////////////////////////////////////////////////////////////////
	IFoundationPtr CEnvRegistry::CreateFoundation(const SGUID& guid, tukk name)
	{
	...
	{
		DRX_ASSERT(pComponentFactory != NULL);
	...
	{
		DRX_ASSERT(pActionFactory != NULL);
	...
	//////////////////////////////////////////////////////////////////////////
	bool CEnvRegistry::RegisterSettings(tukk name, const IEnvSettingsPtr& pSettings)
	{
		return m_settings.insert(TSettingsMap::value_type(name, pSettings)).second;
	}

	//////////////////////////////////////////////////////////////////////////
	IEnvSettingsPtr CEnvRegistry::GetSettings(tukk name) const
	{
	...
	void CEnvRegistry::LoadAllSettings()
	{
		...
		tukk szSettingsFolder = gEnv->pSchematyc2->GetSettingsFolder();
		tukk szSettingsExtension = gEnv->pSchematyc2->GetSettingsExtension();
		for(TSettingsMap::const_iterator iSettings = m_settings.begin(), iEndSettings = m_settings.end(); iSettings != iEndSettings; ++ iSettings)
		{
			tukk  szName = iSettings->first.c_str();
	...
	{
		tukk szSttingsFolder = gEnv->pSchematyc2->GetSettingsFolder();
		tukk szSettingsExtension = gEnv->pSchematyc2->GetSettingsExtension();
		for(TSettingsMap::const_iterator iSettings = m_settings.begin(), iEndSettings = m_settings.end(); iSettings != iEndSettings; ++ iSettings)
		{
			tukk szName = iSettings->first.c_str();
		...
		for(TComponentFactoryMap::const_iterator iComponentFactory = m_componentFactories.begin(), iEndComponentFactory = m_componentFactories.end(); iComponentFactory != iEndComponentFactory; ++ iComponentFactory)
		{
			...
			for(size_t iComponentDependency = 0, componentDependencyCount = componentFactory.GetDependencyCount(); iComponentDependency < componentDependencyCount; ++ iComponentDependency)
			{
				...
				if(pDependencyComponentFactory)
				{
					...
					{
						DrxFatalError("%s : Non-singleton %s component detected as dependency of %s!", __FUNCTION__, pDependencyComponentFactory->GetName(), componentFactory.GetName());
					}
					if(EnvComponentUtils::IsDependency(*pDependencyComponentFactory, componentGUID))
					{
						DrxFatalError("%s : Circular dependency detected between %s and %s components!", __FUNCTION__, componentFactory.GetName(), pDependencyComponentFactory->GetName());
					}
				}
				else
				{
					char	stringBuffer[StringUtils::s_guidStringBufferSize];
					DrxFatalError("%s : Unable to resolve dependency %s for %s component!", __FUNCTION__, StringUtils::SysGUIDToString(dependencyComponentGUID.sysGUID, stringBuffer), componentFactory.GetName());
				}
			}
		}
	}
