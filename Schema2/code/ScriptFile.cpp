...
#include <drx3D/Schema2/ScriptFilePatcher.h>
#include <drx3D/Schema2/ScriptSerializationUtils.h>
...
namespace sxema2
{
	namespace DocUtils
	{
		...
		struct SDocGraphHeader
		{
			...
			inline SDocGraphHeader(const SGUID& _guid, const SGUID& _scopeGUID, tukk _szName, EScriptGraphType _type, const SGUID& _contextGUID)
				: guid(_guid)
	...
	//////////////////////////////////////////////////////////////////////////
	CScriptFile::CScriptFile(tukk szFileName, const SGUID& guid, EScriptFileFlags flags)
	...
	//////////////////////////////////////////////////////////////////////////
	tukk CScriptFile::GetFileName() const
	{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptInclude* CScriptFile::AddInclude(const SGUID& scopeGUID, tukk szFileName, const SGUID& refGUID)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptGroup* CScriptFile::AddGroup(const SGUID& scopeGUID, tukk szName)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptEnumeration* CScriptFile::AddEnumeration(const SGUID& scopeGUID, tukk szName)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptStructure* CScriptFile::AddStructure(const SGUID& scopeGUID, tukk szName)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptSignal* CScriptFile::AddSignal(const SGUID& scopeGUID, tukk szName)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptAbstractInterface* CScriptFile::AddAbstractInterface(const SGUID& scopeGUID, tukk szName)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptAbstractInterfaceFunction* CScriptFile::AddAbstractInterfaceFunction(const SGUID& scopeGUID, tukk szName)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptAbstractInterfaceTask* CScriptFile::AddAbstractInterfaceTask(const SGUID& scopeGUID, tukk szName)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptClass* CScriptFile::AddClass(const SGUID& scopeGUID, tukk szName, const SGUID& foundationGUID)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptStateMachine* CScriptFile::AddStateMachine(const SGUID& scopeGUID, tukk szName, EScriptStateMachineLifetime lifetime, const SGUID& contextGUID, const SGUID& partnerGUID)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptState* CScriptFile::AddState(const SGUID& scopeGUID, tukk szName, const SGUID& partnerGUID)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptVariable* CScriptFile::AddVariable(const SGUID& scopeGUID, tukk szName, const CAggregateTypeId& typeId)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptProperty* CScriptFile::AddProperty(const SGUID& scopeGUID, tukk szName, const SGUID& refGUID, const CAggregateTypeId& typeId)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptContainer* CScriptFile::AddContainer(const SGUID& scopeGUID, tukk szName, const SGUID& typeGUID)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptTimer* CScriptFile::AddTimer(const SGUID& scopeGUID, tukk szName)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptComponentInstance* CScriptFile::AddComponentInstance(const SGUID& scopeGUID, tukk szName, const SGUID& componentGUID, EScriptComponentInstanceFlags flags)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	IScriptActionInstance* CScriptFile::AddActionInstance(const SGUID& scopeGUID, tukk szName, const SGUID& actionGUID, const SGUID& componentInstanceGUID)
	{
		SXEMA2_SYSTEM_ASSERT(gEnv->IsEditor());
		if(gEnv->IsEditor())
		{
	...
	//////////////////////////////////////////////////////////////////////////
	bool CScriptFile::IsElementNameUnique(const SGUID& scopeGUID, tukk szName) const
	{
	...
	void CScriptFile::PasteElementsFromXml(const SGUID& scopeGUID, const XmlNodeRef& xml)
	{
		if(gEnv->IsEditor())
		{
			if(xml->isTag("schematyc"))
			{
				...
				u32k currentVersion = g_docPatcher.GetCurrentVersion();
	...
	void CScriptFile::Load()
	{
		...
		if(!m_fileName.empty())
		{
			SXEMA2_SYSTEM_COMMENT("Loading script file : %s [%s]", m_fileName.c_str(), (m_flags & EScriptFileFlags::OnDisk) != 0 ? "Disk" : "Pak");
			...
			if(xml)
			{
				...
				if(xml && xml->isTag("schematyc"))
				{
					...
					{
						SXEMA2_SYSTEM_WARNING("Script file was previously saved in a newer build");
					}
			...
			{
				SXEMA2_SYSTEM_CRITICAL_ERROR("Failed to parse script file, see log for details : %s", m_fileName.c_str());
			}

			SXEMA2_SYSTEM_COMMENT("Loading complete");
		}
	}
	...
	void CScriptFile::Save()
	{
		...
		{
			SXEMA2_SYSTEM_COMMENT("Saving Script File : %s", m_fileName.c_str());
			m_flags &= ~EScriptFileFlags::Modified;
		}
		else
		{
			SXEMA2_SYSTEM_ERROR("Failed To Save Script File : %s", m_fileName.c_str());
		}
	}
	...
	bool CScriptFile::GetClipboardInfo(const XmlNodeRef& xml, SScriptElementClipboardInfo& clipboardInfo) const
	{
		if(xml)
		{
			tukk              szElementTypeName = xml->getTag();
	...
	void CScriptFile::AttachElement(IScriptElement& element, const SGUID& scopeGUID)
	{
		if(scopeGUID)
		{
			...
			{
				SXEMA2_SYSTEM_WARNING("Failed to find parent element!");
			}
		}
		else
		{
	...
	//////////////////////////////////////////////////////////////////////////
	XmlNodeRef CScriptFile::SaveElements(EScriptElementType type, tukk szTag)
	{
		...
		tukk szElementTypeName = GetElementTypeName(type);
	...
	//////////////////////////////////////////////////////////////////////////
	tukk CScriptFile::GetElementTypeName(EScriptElementType elementType) const
	{
		for(size_t elementTypeIdx = 0; elementTypeIdx < DRX_ARRAY_COUNT(DocUtils::ELEMENT_TYPES); ++ elementTypeIdx)
		{
	...
	//////////////////////////////////////////////////////////////////////////
	EScriptElementType CScriptFile::GetElementType(tukk szElementTypeName) const
	{
		for(size_t elementTypeIdx = 0; elementTypeIdx < DRX_ARRAY_COUNT(DocUtils::ELEMENT_TYPES); ++ elementTypeIdx)
		{
...
SERIALIZATION_ENUM_BEGIN_NESTED(sxema2, EScriptGraphNodeType, "sxema Script Graph Node Type")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Unknown, "unknown", "Unknown")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Begin, "begin", "Begin")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::BeginConstructor, "begin_constructor", "Begin Constructor")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::BeginDestructor, "begin_destructor", "Begin Destructor")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::BeginSignalReceiver, "begin_signal_receiver", "Begin Signal Receiver")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Return, "return", "Return")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Sequence, "sequence", "Sequence")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Branch, "branch", "Branch")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Switch, "switch", "Switch")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::MakeEnumeration, "make_enumeration", "Make Enumeration")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ForLoop, "for_loop", "For Loop")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ProcessSignal, "process_signal", "Process Signal")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::SendSignal, "send_signal", "Send Signal")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::BroadcastSignal, "broadcast_signal", "Broadcast Signal")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Function, "call_function", "Function") 
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Condition, "call condition", "Condition")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::AbstractInterfaceFunction, "call_abstract_interface_function", "Abstract Interface Function")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::GetObject, "get_object", "Get Object")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Set, "set", "Set")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Get, "get", "Get")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ContainerAdd, "container_add", "Container Add")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ContainerRemoveByIndex, "container_remove_by_index", "Container Remove By Index")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ContainerRemoveByValue, "container_remove_by_value", "Container Remove By Value")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ContainerClear, "container_clear", "Container Clear")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ContainerSize, "container_size", "Container Size")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ContainerGet, "container_get", "Container Get")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ContainerSet, "container_set", "Container Set")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ContainerFindByValue, "container_find_by_value", "Container Find By Value")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::StartTimer, "start_timer", "Start Timer")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::StopTimer, "stop_timer", "Stop Timer")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::ResetTimer, "reset_timer", "Reset Timer")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::State, "state", "State")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::BeginState, "begin_state", "Begin State")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::EndState, "end_state", "End State")
	SERIALIZATION_ENUM(sxema2::EScriptGraphNodeType::Comment, "comment", "Comment")
