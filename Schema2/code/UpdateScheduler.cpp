...
namespace sxema2
{
	namespace
	{
		...
		static u32k s_invalidBucketIdx       = ~0;
	}
	...
	void CRelevanceGrid::Construct()
	{
		...
		m_gridSize = i32(terrainSize / CVars::sc_RelevanceGridCellSize);
	...
	void CRelevanceGrid::Update(CUpdateRelevanceContext* pRelevanceContext)
	{
		...
		{
			m_updateHalfDistance = i32(pRelevanceContext->GetDefaultGridDistanceUpdate() / sxema2::CVars::sc_RelevanceGridCellSize);
		}
		...
		for (size_t i = 0; i < relevantEntitiesCount; ++i)
		{
			...
			if (result == eCellChangedResult_Changed)
			{
				...
				DrxLogAlways("CRelevanceGrid::Update relevant cell change for ent %u old coord %u:%u(%u) new coord %u:%u(%u), local %d",
					relevantCell.id,
			...
			{
#if DEBUG_RELEVANCE_GRID_EXTRA_LOG
				DrxLogAlways("CRelevanceGrid::Update relevant cell new for ent %u new coord %u:%u(%u), local %d",
					relevantCell.id,
					cellX, cellY, cellIdx,
	...
	void CRelevanceGrid::ConnectCell(ushort idx, bool bIsLocal)
	{
		...
		{
#if DEBUG_RELEVANCE_GRID_EXTRA_LOG
			DrxLogAlways("CRelevanceGrid::ConnectCell (%u) local %u", idx, bIsLocal);
#endif // DEBUG_RELEVANCE_GRID_EXTRA_LOG
	...
	void CRelevanceGrid::DisconnectCell(ushort idx, bool bIsLocal)
	{
		...
		{
#if DEBUG_RELEVANCE_GRID_EXTRA_LOG
			DrxLogAlways("CRelevanceGrid::DisconnectCell (%u) local %u", idx, bIsLocal);
#endif // DEBUG_RELEVANCE_GRID_EXTRA_LOG
	...
	{
		x = (ushort)clamp_tpl(i32(worldPos.x / CVars::sc_RelevanceGridCellSize), 0, m_gridSizeMinusOne);
		y = (ushort)clamp_tpl(i32(worldPos.y / CVars::sc_RelevanceGridCellSize), 0, m_gridSizeMinusOne);
	...
	void CRelevanceGrid::DebugDrawStatic(CUpdateRelevanceContext* pRelevanceContext)
	{
		...
		auto buildCellAabb = [cellSize, halfCellSize](i32 cellX, i32 cellY)
		{
			Vec3 center = Vec3((float)cellX, (float)cellY, 0.0f) * cellSize + Vec3(halfCellSize, halfCellSize, 0.0f);

			const float z = drxmath::floor(gEnv->p3DEngine->GetTerrainZ(center.x, center.y));
		...
		auto printCallback = [](const SUpdateCallback& callback, string& textBuf)
		{
			if (const IEntity* pEnt = callback.pScope->GetEntity())
			{
				...
				tukk szFormat = callback.pScope->IsBound() ? "\n%u:%s - %u" : "\n$4%u:%s - %u$o";
				textBuf.AppendFormat(szFormat, entId, szName, (u32)callback.priority);
			}
		};
	...
	bool CUpdateScheduler::Update(UpdatePriority beginPriority, UpdatePriority endPriority, CUpdateRelevanceContext* pRelevanceContext)
	{
		...
		if(m_bInFrame)
		{
			...
			const int64 startTimeTicks = DrxGetTicks();
			...
			const int64 endTimeTicks = DrxGetTicks();
	...
	bool CUpdateScheduler::ConnectInternal(CUpdateScope& scope, const UpdateCallback& callback, UpdateFrequency frequency /*= EUpdateFrequency::EveryFrame*/, UpdatePriority priority /*= EUpdatePriority::Default*/, const UpdateFilter& filter /*= UpdateFilter()*/)
	{
		...
		if ((frequency >= EUpdateFrequency::EveryFrame) && (frequency < EUpdateFrequency::Count))
		{
			...
			m_slots.push_back(SSlot(&scope, static_cast<u32>(firstBucketIdx), frequency, priority));
			for (CBucket* pBucket = m_buckets + firstBucketIdx, *pEndBucket = m_buckets + s_bucketCount; pBucket < pEndBucket; pBucket += bucketStride)
			{
				//DrxLogAlways("Entity name %s: Pos:%f, %f, %f", filter.)
	...
	////////////////////////////////////////////////////////////////
	CUpdateScheduler::SSlot::SSlot(CUpdateScope* _pScope, u32 _firstBucketIdx, UpdateFrequency _frequency, UpdatePriority _priority)
		: pScope(_pScope)
