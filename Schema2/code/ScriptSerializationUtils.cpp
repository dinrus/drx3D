...
#include <drx3D/Schema2/Deprecated/DocLogicGraph.h>
#include <drx3D/Schema2/Deprecated/DocTransitionGraph.h>
...
namespace sxema2
{
	namespace DocSerializationUtils
	{
		...
		struct SGraphHeader
		{
			...
			inline SGraphHeader(const SGUID& _guid, const SGUID& _scopeGUID, tukk _szName, EScriptGraphType _type, const SGUID& _contextGUID)
				: guid(_guid)
		...
		bool Serialize(Serialization::IArchive& archive, CInputBlock::SElement& value, tukk szName, tukk szLabel)
		{
			if(archive.isInput())
			{
		...
		IDocGraphPtr CDocGraphFactory::CreateGraph(IScriptFile& file, const SGUID& guid, const SGUID& scopeGUID, tukk szName, EScriptGraphType type, const SGUID& contextGUID)
		{
			switch(type)
			{
		...
		IScriptElementPtr CScriptElementFactory::CreateElement(Serialization::IArchive& archive, tukk szName)
		{
		...
		void CInputBlock::SElement::Serialize(Serialization::IArchive& archive)
		{
			if(ptr)
			{
				SXEMA2_SYSTEM_ASSERT(serializationPass <= ESerializationPass::PostLoad)
		...
		void CInputBlock::VerifyElementDependencies(const Elements &elements)
		{
			...
			SXEMA2_SYSTEM_ASSERT(!errorCount);
		}
	}
}
