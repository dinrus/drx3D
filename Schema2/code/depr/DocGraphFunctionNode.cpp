...
#include <drx3D/Schema2/Deprecated/DocGraphNodes/DocGraphFunctionNode.h>
...
SERIALIZATION_ENUM_BEGIN_NESTED2(sxema2, CDocGraphFunctionNode, EFunctionType, "sxema Function Type")
	SERIALIZATION_ENUM(sxema2::CDocGraphFunctionNode::EFunctionType::EnvGlobal, "EnvGlobal", "Environment Global")
	SERIALIZATION_ENUM(sxema2::CDocGraphFunctionNode::EFunctionType::EnvComponent, "EnvComponent", "Environment Component")
	SERIALIZATION_ENUM(sxema2::CDocGraphFunctionNode::EFunctionType::EnvAction, "EnvAction", "Environment Action")
	SERIALIZATION_ENUM(sxema2::CDocGraphFunctionNode::EFunctionType::Script, "Script", "Script")
...
namespace sxema2
{
	...
	//////////////////////////////////////////////////////////////////////////
	size_t CDocGraphFunctionNode::AddOptionalOutput(tukk szName, EScriptGraphPortFlags flags, const CAggregateTypeId& typeId)
	{
		return INVALID_INDEX;
	}
		...
		case EFunctionType::EnvGlobal:
			{
				...
				if(pFunction)
				{
					...
					for(size_t functionInputIdx = 0; functionInputIdx < functionInputCount; ++ functionInputIdx)
					{
						...
						DRX_ASSERT(pFunctionInputValue);
					...
					for(size_t functionOutputIdx = 0; functionOutputIdx < functionOutputCount; ++ functionOutputIdx)
					{
						...
						DRX_ASSERT(pFunctionOutputValue);
	...
	void CDocGraphFunctionNode::Serialize(Serialization::IArchive& archive)
	{
		...
		{
			tukk szInputName = CDocGraphNodeBase::GetInputName(inputIdx);
	...
	SGUID CDocGraphFunctionNode::ResolveContextGUID() const
	{
		...
		if(refGUID)
		{
			...
			if(IComponentMemberFunctionConstPtr pFunction = gEnv->pSchematyc2->GetEnvRegistry().GetComponentMemberFunction(refGUID))
			{
				...
				SXEMA2_SYSTEM_ASSERT(componentInstanceGUID);
			...
			else if(IActionMemberFunctionConstPtr pFunction = gEnv->pSchematyc2->GetEnvRegistry().GetActionMemberFunction(refGUID))
			{
				...
				SXEMA2_SYSTEM_ASSERT(actionInstanceGUID);
	...
	void CDocGraphFunctionNode::EditContext(Serialization::IArchive& archive)
	{
		...
		{
			index = static_cast<i32>(itContextGUID - contextGUIDs.begin());
		}
		...
		case EFunctionType::EnvComponent:
			{
				...
				if(pFunction)
				{
					...
					SXEMA2_SYSTEM_ASSERT(componentInstanceIdx != INVALID_INDEX);
					if(componentInstanceIdx != INVALID_INDEX)
					{
						const CAny<u32> componentIdx = MakeAny(static_cast<u32>(componentInstanceIdx));
		...
		case EFunctionType::EnvAction:
			{
				...
				if(pFunction)
				{
					...
					SXEMA2_SYSTEM_ASSERT(actionInstanceIdx != INVALID_INDEX);
					if(actionInstanceIdx != INVALID_INDEX)
					{
						const CAny<u32> actionIdx = MakeAny(static_cast<u32>(actionInstanceIdx));
