...
namespace sxema2
{
	...
	{
		if(const IEnvTypeDesc* pTypeDesc = gEnv->pSchematyc2->GetEnvRegistry().GetTypeDesc(GetEnvTypeId<i32>()))
		{
	...
	//////////////////////////////////////////////////////////////////////////
	size_t CDocGraphSwitchNode::AddOptionalOutput(tukk szName, EScriptGraphPortFlags flags, const CAggregateTypeId& typeId)
	{
		return INVALID_INDEX;
	}
	...
	void CDocGraphSwitchNode::Refresh(const SScriptRefreshParams& params)
	{
		...
		for(SCase& _case : m_cases)
		{
			...
			if(!outputName[0])
			{
				drx_sprintf(outputName, "INVALID_CASE[%d]", ++ invalidCaseCount);
			}
	...
	void CDocGraphSwitchNode::Serialize(Serialization::IArchive& archive)
	{
		...
		if(archive.isEdit())
		{
			...
			if(const IEnvTypeDesc* pTypeDesc = gEnv->pSchematyc2->GetEnvRegistry().GetTypeDesc(GetEnvTypeId<u32>()))
			{
				typeIds.push_back(pTypeDesc->GetTypeInfo().GetTypeId());
				typeNames.push_back(pTypeDesc->GetName());
			}
			...
			else if(archive.isOutput())
			{
				...
				{
					typeIdx = static_cast<i32>(itTypeId - typeIds.begin());
				}
				else
				{
					typeIds.push_back(m_typeId);
					typeNames.push_back("");
					typeIdx = static_cast<i32>(typeIds.size() - 1);
				}
	...
	void CDocGraphSwitchNode::SCase::Serialize(Serialization::IArchive& archive)
	{
		...
		SXEMA2_SYSTEM_ASSERT(pSwitchNode);
		if(pSwitchNode)
		{
			if(!pValue)
			{
	...
	void CDocGraphSwitchNode::CompileInputs(IDocGraphNodeCompiler& compiler) const
	{
		...
		if(m_pValue)
		{
			...
			for(size_t caseIdx = 0, caseCount = m_cases.size(); caseIdx < caseCount; ++ caseIdx)
			{
				...
				{
					SXEMA2_COMPILER_ERROR("Missing case value!");
					compiler.Push(*m_pValue);
				}
