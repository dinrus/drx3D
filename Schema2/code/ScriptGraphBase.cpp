...
#include <drx3D/Schema2/ScriptGraphCommentNode.h>    // #SchematycTODO : Where should script graph node creators be registered?
#include <drx3D/Schema2/ScriptGraphBeginNode.h>    // #SchematycTODO : Where should script graph node creators be registered?
#include <drx3D/Schema2/ScriptGraphFunctionNode.h> // #SchematycTODO : Where should script graph node creators be registered?
#include <drx3D/Schema2/ScriptGraphGetNode.h>      // #SchematycTODO : Where should script graph node creators be registered?
#include <drx3D/Schema2/ScriptGraphNodeFactory.h>  // #SchematycTODO : Where should the script graph node factory live?
#include <drx3D/Schema2/ScriptGraphSwitchNode.h>   // #SchematycTODO : Where should script graph node creators be registered?
...
{
	inline bool Serialize(Serialization::IArchive& archive, shared_ptr<sxema2::IScriptGraphNode>& value, tukk szName, tukk szLabel)
	{
		if(!value && archive.isInput())
		{
			sxema2::SGUID typeGUID;
			archive(typeGUID, "typeGUID");
			value = sxema2::g_scriptGraphNodeFactory.CreateNode(typeGUID);
		}
		if(value)
		{
			if(archive.isOutput() && !archive.isEdit())
			{
				sxema2::SGUID typeGUID = value->GetTypeGUID();
				archive(typeGUID, "typeGUID");
			}
	...
	/*bool Serialize(Serialization::IArchive& archive, shared_ptr<sxema2::CScriptGraphLink>& value, tukk szName, tukk szLabel)
	{
		if(!value)
		{
...
namespace sxema2
{
	...
	tukk CScriptGraphBase::GetInputName(size_t inputIdx) const
	{
		return "";
	}
	...
	tukk CScriptGraphBase::GetOutputName(size_t outputIdx) const
	{
		return "";
	}
	...
	tukk CScriptGraphBase::GetAvailableNodeName(size_t availableNodeIdx) const
	{
		return "";
	}
	...
	bool CScriptGraphBase::CanAddLink(const SGUID& srcNodeGUID, tukk szScOutputName, const SGUID& dstNodeGUID, tukk szDstInputName) const
	{
		const IScriptGraphNode* pSrcNode = GetNode(srcNodeGUID);
		const IScriptGraphNode* pDstNode = GetNode(dstNodeGUID);
		SXEMA2_SYSTEM_ASSERT(pSrcNode && pDstNode);
		if(pSrcNode && pDstNode)
		{
			...
			SXEMA2_SYSTEM_ASSERT((srcOutputIdx != INVALID_INDEX) && (dstInputIdx != INVALID_INDEX));
	...
	IScriptGraphLink* CScriptGraphBase::AddLink(const SGUID& srcNodeGUID, tukk szSrcOutputName, const SGUID& dstNodeGUID, tukk szDstInputName)
	{
	...
	void CScriptGraphBase::RemoveLink(size_t linkIdx)
	{
		...
		SXEMA2_SYSTEM_ASSERT(linkIdx < linkCount);
	...
	size_t CScriptGraphBase::FindLink(const SGUID& srcGUID, tukk szSrcOutputName, const SGUID& dstNodeGUID, tukk szDstInputName) const
	{
		SXEMA2_SYSTEM_ASSERT(szSrcOutputName && szDstInputName);
	...
	u32 CScriptGraphBase::GetNodeCount() const
	{
		return m_nodes.size();
	}
	...
	void CScriptGraphBase::VisitInputLinks(const ScriptGraphLinkVisitor& visitor, const SGUID& dstNodeGUID, tukk szDstInputName)
	{
		SXEMA2_SYSTEM_ASSERT(visitor && szDstInputName);
	...
	void CScriptGraphBase::VisitInputLinks(const ScriptGraphLinkConstVisitor& visitor, const SGUID& dstNodeGUID, tukk szDstInputName) const
	{
		SXEMA2_SYSTEM_ASSERT(visitor && szDstInputName);
		if(visitor && szDstInputName)
		{
	...
	void CScriptGraphBase::VisitOutputLinks(const ScriptGraphLinkVisitor& visitor, const SGUID& srcNodeGUID, tukk szSrcOutputName)
	{
		SXEMA2_SYSTEM_ASSERT(visitor && szSrcOutputName);
	...
	void CScriptGraphBase::VisitOutputLinks(const ScriptGraphLinkConstVisitor& visitor, const SGUID& srcNodeGUID, tukk szSrcOutputName) const
	{
		SXEMA2_SYSTEM_ASSERT(visitor && szSrcOutputName);
		if(visitor && szSrcOutputName)
		{
	...
	bool CScriptGraphBase::GetLinkSrc(const IScriptGraphLink& link, IScriptGraphNode*& pNode, u32& outputIdx)
	{
	...
	bool CScriptGraphBase::GetLinkSrc(const IScriptGraphLink& link, const IScriptGraphNode*& pNode, u32& outputIdx) const
	{
		pNode = GetNode(link.GetSrcNodeGUID());
		if(pNode)
		{
	...
	bool CScriptGraphBase::GetLinkDst(const IScriptGraphLink& link, IScriptGraphNode*& pNode, u32& inputIdx)
	{
	...
	bool CScriptGraphBase::GetLinkDst(const IScriptGraphLink& link, const IScriptGraphNode*& pNode, u32& inputIdx) const
	{
		pNode = GetNode(link.GetDstNodeGUID());
		if(pNode)
		{
