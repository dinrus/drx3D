...
SERIALIZATION_ENUM_BEGIN_NESTED2(sxema2, CScriptGraphFunctionNode, EFunctionType, "sxema Script Graph Node Function Type")
	SERIALIZATION_ENUM(sxema2::CScriptGraphFunctionNode::EFunctionType::ComponentPropertiesMember, "ComponentPropertiesMember", "Component Properties Member")
...
namespace sxema2
{
	namespace ScriptGraphFunctionNodeUtils
	{
		...
		typedef std::map<u32, SFunctionInputPtr> FunctionInputsById;

		inline bool Serialize(Serialization::IArchive& archive, SFunctionInputPtr& value, tukk szName, tukk szLabel)
		{
			if(value.pValue)
			{
		...
		{
			inline SEnvFunctionRuntimeParams(u32 _componentIdx, const IEnvFunctionDescriptor* _pEnvFunctionDescriptor = nullptr)
	...
	CScriptGraphFunctionNode::SFunctionInput::SFunctionInput(u32 _id, const IAnyPtr& _pValue)
		: id(_id)
		, pValue(_pValue)
	{}
			...
			case EFunctionType::ComponentPropertiesMember:
				{
					...
					if(pComponentInstance && pEnvFunctionDescriptor)
					{
						...
						for(u32 inputIdx = 0, inputCount = pEnvFunctionDescriptor->GetInputCount(); inputIdx < inputCount; ++ inputIdx)
						{
						...
						for(u32 outputIdx = 0, outputCount = pEnvFunctionDescriptor->GetOutputCount(); outputIdx < outputCount; ++ outputIdx)
						{
			...
			case EFunctionType::ComponentPropertiesMember:
				{
					...
					if(pEnvFunctionDescriptor)
					{
						u32k inputCount = pEnvFunctionDescriptor->GetInputCount();
	...
	{
		for(u32 inputIdx = 0, inputCount = m_functionInputs.size(); inputIdx < inputCount; ++ inputIdx)
		{
			tukk    szInputName = CScriptGraphNodeBase::GetInputName(EInputIdx::FirstParam + inputIdx);
			const IAnyPtr& pInputValue = m_functionInputs[inputIdx].pValue;
	...
	SRuntimeResult CScriptGraphFunctionNode::Execute(IObject* pObject, const SRuntimeActivationParams& activationParams, CRuntimeNodeData& data)
	{
		...
		for(u32 inputIdx = 0, inputCount = data.GetInputCount() - EInputIdx::FirstParam; inputIdx < inputCount; ++ inputIdx)
		{
			inputs[inputIdx] = data.GetInput(EInputIdx::FirstParam + inputIdx);
		}

		EnvFunctionOutputs outputs = { nullptr };
		for(u32 outputIdx = 0, outputCount = data.GetOutputCount() - EOutputIdx::FirstParam; outputIdx < outputCount; ++ outputIdx)
		{
