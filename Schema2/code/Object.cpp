...
#ifdef _RELEASE
#define SXEMA2_OBJECT_VM_DEBUG 0
#else
#define SXEMA2_OBJECT_VM_DEBUG 1
#endif
...
namespace sxema2
{
	...
	namespace
	{
		...
		inline void DrxLogVariants(const TVariantConstArray& variants)
		{
	...
	bool CObject::RunTask(const SGUID& stateMachineGUID, CStack& stack, const TaskCallbackConnection& callbackConnection)
	{
		if((m_simulationMode == ESimulationMode::Game) && HaveNetworkAuthority())
		{
			...
			DRX_ASSERT(iStateMachine != INVALID_INDEX);
	...
	void CObject::Preview(const SRendParams& params, const SRenderingPassInfo& passInfo, const IObjectPreviewProperties& previewProperties) const
	{
		...
		for(u32 componentIdx = 0, componentCount = m_componentInstances.size(); componentIdx < componentCount; ++ componentIdx)
		{
	...
	{
		u32k variableCount = m_pLibClass->GetVariableCount();
		for(u32 variableIdx = 0; variableIdx < variableCount; ++ variableIdx)
		{
	...
	void CObject::CreateAndInitComponents(const IObjectNetworkSpawnParamsPtr& pNetworkSpawnParams)
	{
		...
		u32k  componentInstanceCount = m_pLibClass->GetComponentInstanceCount();
		...
		for(u32 componentInstanceIdx = 0; componentInstanceIdx < componentInstanceCount; ++ componentInstanceIdx)
		{
			...
			if(pComponentFactory)
			{
				...
				SXEMA2_SYSTEM_ASSERT(pComponent);
				if(pComponent)
				{
					...
					u32k componentPropertyFunctionIdx = libComponentInstance.GetPropertyFunctionIdx();
					...
					u32k     parentIdx = libComponentInstance.GetParentIdx();
	...
	{
		IEnvRegistry& envRegistry = gEnv->pSchematyc2->GetEnvRegistry();
		u32k  actionInstanceCount = m_pLibClass->GetActionInstanceCount();
		m_actionInstances.reserve(actionInstanceCount);
		for(u32 actionInstanceIdx = 0; actionInstanceIdx < actionInstanceCount; ++ actionInstanceIdx)
		{
			...
			SXEMA2_SYSTEM_ASSERT(pActionFactory);
			if(pActionFactory)
			{
				IActionPtr pAction = pActionFactory->CreateAction();
				SXEMA2_SYSTEM_ASSERT(pAction);
				if(pAction)
				{
	...
	{
		for(u32 componentInstanceIdx = 0, componentInstanceCount = m_pLibClass->GetComponentInstanceCount(); componentInstanceIdx < componentInstanceCount; ++ componentInstanceIdx)
		{
			const ILibComponentInstance& libComponentInstance = *m_pLibClass->GetComponentInstance(componentInstanceIdx);
	...
	{
		DRX_ASSERT(iVariable < m_pLibClass->GetVariableCount());
		if(iVariable < m_pLibClass->GetVariableCount())
		{
			...
			DRX_ASSERT(pVariableValue != NULL);
			if(pVariableValue != NULL)
			{
				DRX_ASSERT(value.GetTypeInfo().GetTypeId() == pVariableValue->GetTypeInfo().GetTypeId());
	...
	//////////////////////////////////////////////////////////////////////////
	IAnyConstPtr CObject::GetProperty(u32 propertyIdx) const
	{
		...
		SXEMA2_SYSTEM_ASSERT(propertyIdx < m_pLibClass->GetVariableCount());
	...
	//////////////////////////////////////////////////////////////////////////
	i32 CObject::GetNetworkAspect() const
	{
	...
	//////////////////////////////////////////////////////////////////////////
	CObject::SPreviewProperties::SComponentPreviewProperties::SComponentPreviewProperties(tukk _componentName, const IComponentPreviewPropertiesPtr& _pData)
	...
	{
		u32k componentCount = componentInstances.size();
	...
	void CObject::SPreviewProperties::Serialize(Serialization::IArchive& archive)
	{
		for(SComponentPreviewProperties& _componentPreviewProperties : componentPreviewProperties)
		{
			...
			{
				tukk szComponentName = _componentPreviewProperties.componentName.c_str();
	...
	void CObject::ChangeState(size_t iStateMachine, size_t iNewState, const SEvent& event, const TVariantConstArray& inputs)
	{
		...
		i32k	currentDepth = static_cast<i32>(currentStateStack.size());
		i32k	newDepth = static_cast<i32>(newStateStack.size());
		i32				firstCommonAncestor = -1;
		for(i32 depth = /*std::*/min(currentDepth, newDepth); (firstCommonAncestor + 1) < depth; ++ firstCommonAncestor)
		{
		...
		for(i32 pos = currentDepth - 1; pos > firstCommonAncestor; -- pos)
		{
			ExitState(iStateMachine, currentStateStack[pos], event, inputs);
		}
		stateMachine.iCurrentState = iNewState;
		for(i32 pos = firstCommonAncestor + 1; pos < newDepth; ++ pos)
		{
	...
	void CObject::EnterState(size_t iStateMachine, size_t iState, const TVariantConstArray& inputs)
	{
		...
		if(bHaveNetworkAuthority)
		{
			...
			if(const ILibState* pPartnerLibState = m_pLibClass->GetState(iPartnerState))
			{
				...
				SXEMA2_SYSTEM_ASSERT_FATAL_MESSAGE(iPartnerStateMachine != INVALID_INDEX,
	...
	void CObject::InitStateMachineVariables(size_t iStateMachine, CStack& stack)
	{
		const ILibStateMachine&	libStateMachine = *m_pLibClass->GetStateMachine(iStateMachine);
		TVariantConstArray			libVariants = m_pLibClass->GetVariants();
		for(i32 iVariable = libStateMachine.GetVariableCount() - 1; iVariable >= 0; -- iVariable)
		{
			const ILibVariable&	libVariable = *m_pLibClass->GetVariable(libStateMachine.GetVariable(iVariable));
			for(i32 iFirstVariant = libVariable.GetVariantPos(), iVariant = iFirstVariant + libVariable.GetVariantCount() - 1; iVariant >= iFirstVariant; -- iVariant)
			{
				bool	useDefaultValue = true;
				if((libVariable.GetFlags() & ELibVariableFlags::StateMachineProperty) != 0)
				{
					SXEMA2_SYSTEM_ASSERT(!stack.Empty());
					if(!stack.Empty())
					{
						const CVariant	variant = stack.Top();
						SXEMA2_SYSTEM_ASSERT(variant.GetTypeId() == m_variants[iVariant].GetTypeId());
	...
	{
		DRX_PROFILE_FUNCTION_ARG(PROFILE_GAME, m_pLibClass->GetName());
		const ILibFunction*	pFunction = m_pLibClass->GetFunction(functionId);
		DRX_ASSERT(pFunction);
		if(pFunction)
		{
			LOADING_TIME_PROFILE_SECTION_NAMED_ARGS("ProcessFunction", pFunction->GetName());
			static u32 s_recursionDepth = 0;
			if(s_recursionDepth >= CVars::sc_MaxRecursionDepth)
			{
				SXEMA2_SYSTEM_METAINFO_CRITICAL_ERROR(
					SXEMA2_LOG_METAINFO(EDrxLinkCommand::Show, SLogMetaItemGUID(pFunction->GetGUID())), 
			...
			const int64	startTicks = DrxGetTicks();
			...
			DRX_ASSERT((inputs.size() >= inputCount) && (outputs.size() >= outputCount));
					...
					case SVMOp::COPY:
						{
							...
							{
								SXEMA2_SYSTEM_CRITICAL_ERROR("Invalid copy operation: class = %s, function = %s, pos = %d", m_pLibClass->GetName(), pFunction->GetName(), pos);
							}
#endif
					...
					case SVMOp::CONTAINER_SIZE:
						{
							...
							stack.Push(CVariant(static_cast<i32>(m_containers[pContainerSizeOp->iContainer].Size() / pContainerSizeOp->divisor)));
							pos += pOp->size;
							break;
						}
					...
					case SVMOp::CONTAINER_FIND_BY_VALUE:
						{
							...
							stack.Push(CVariant(i32(resultIndex)));
					...
					case SVMOp::SEND_SIGNAL:
						{
							...
							if(pSignal != NULL)
							{
								...
								if(stackSize >= (signalInputSize + 1))
								{
									...
									{
										gEnv->pSchematyc2->GetObjectUpr().SendSignal(objectId, pSendSignalOp->guid, signalInputs);
									}
								}
							}
							pos += pOp->size;
					...
					case SVMOp::BROADCAST_SIGNAL:
						{
							...
							if(pSignal != NULL)
							{
								const size_t	stackSize = stack.GetSize();
								const size_t	signalInputSize = pSignal->GetVariantInputs().size();
								DRX_ASSERT(stackSize >= signalInputSize);
								if(stackSize >= signalInputSize)
								{
									TVariantConstArray	signalInputs(signalInputSize ? &stack[stackSize - signalInputSize] : NULL, signalInputSize);
									gEnv->pSchematyc2->GetObjectUpr().BroadcastSignal(pBroadcastSignalOp->guid, signalInputs);
								}
							}
							pos += pOp->size;
							break;
					...
					case SVMOp::CALL_GLOBAL_FUNCTION:
						{
							...
							DRX_ASSERT(stackSize >= functionInputSize);
					...
					case SVMOp::CALL_ENV_ABSTRACT_INTERFACE_FUNCTION:
						{
							...
							DRX_ASSERT(pAbstractInterfaceFunction);
							if(pAbstractInterfaceFunction)
							{
								...
								DRX_ASSERT(prevStackSize >= (variantInputSize + 1));
								if(prevStackSize >= (variantInputSize + 1))
								{
									...
									if(IObject* pObject = objectId == m_objectId ? this : gEnv->pSchematyc2->GetObjectUpr().GetObjectById(objectId))
									{
					...
					case SVMOp::CALL_COMPONENT_MEMBER_FUNCTION:
						{
							...
							if(stackSize > functionInputSize)
							{
								u32k	iComponentInstance = stack[stackSize - functionInputSize - 1].AsUInt32();
								DRX_ASSERT(iComponentInstance < m_componentInstances.size());
					...
					case SVMOp::CALL_ACTION_MEMBER_FUNCTION:
						{
							...
							if(stackSize > functionInputSize)
							{
								u32k	iActionInstance = stack[stackSize - functionInputSize - 1].AsUInt32();
								DRX_ASSERT(iActionInstance < m_actionInstances.size());	// #SchematycTODO : Double check action in an env action!!!
			...
			if(CVars::sc_FunctionTimeLimit)
			{
				...
				{
					SXEMA2_SYSTEM_ERROR("Function took more than %f(s) to process: class = %s, function = %s, time = %f(s)", CVars::sc_FunctionTimeLimit, m_pLibClass->GetName(), pFunction->GetName(), time);
				}
			}
			-- s_recursionDepth;
		}
	...
	//////////////////////////////////////////////////////////////////////////
	void CObject::NetworkSerialize(TSerialize serialize, i32 aspects, u8 profile, i32 flags)
	{
		if((m_flags & EObjectFlags::NetworkReplicateActions) != 0)
		{
	...
	void CObject::CStateNetIdxMapper::Init(const ILibClass* pLibClass, const TActionInstanceVector& actionInstances)
	{
		...
		for (size_t iState = 0; iState < stateCount; ++iState)
		{
			...
			DRX_ASSERT(iStateMachine < stateMachineCount);

			const StateNetIdx idx = stateMachineCounters[iStateMachine]++;
			stateIndexMapping[iState] = idx;

			reverseStateIndexMapping.push_back(std::make_pair(std::make_pair(iStateMachine, idx), static_cast<i32>(iState)));
			...
			if (stateMachineRelevant[iStateMachine] == false)
			{
				...
				for (size_t stateActionIdx = 0; stateActionIdx < actionsCount; ++stateActionIdx)
				{
					...
					DRX_ASSERT(iAction < actionInstances.size());
	...
	//////////////////////////////////////////////////////////////////////////
	i32 CObject::CStateNetIdxMapper::GetStateCount(size_t iStateMachine) const
	{
		return m_stateMachineCounts[iStateMachine];
	}

	//////////////////////////////////////////////////////////////////////////
	i32 CObject::CStateNetIdxMapper::Encode(size_t iState) const
	{
		if (iState == INVALID_INDEX)
		{
		...
		DRX_ASSERT(iState < m_stateIndexMapping.size());
	...
	size_t CObject::CStateNetIdxMapper::Decode(size_t iStateMachine, StateNetIdx netStateIdx) const
	{
		...
		DRX_ASSERT(iStateMachine < m_stateMachineCounts.size());
