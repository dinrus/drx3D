...
#define SXEMA2_PREVIEW_DOC_PATCHES 0
...
namespace sxema2
{
	...
	namespace DocPatcherUtils
	{
		inline XmlNodeRef CloneXml(XmlNodeRef xml)
		{
			...
			for(i32 childIdx = 0, childCount = xml->getChildCount(); childIdx < childCount; ++ childIdx)
			{
				xmlClone->addChild(CloneXml(xml->getChild(childIdx)));
			}
			for(i32 attributeIdx = 0, attributeCount = xml->getNumAttributes(); attributeIdx < attributeCount; ++ attributeIdx)
			{
				tukk szKey = "";
				tukk szValue = "";
		...
		inline void CacheXmlChildren(XmlNodeRef xml, XmlNodeRefVector& xmlChildren)
		{
			...
			for(i32 childIdx = 0; childIdx < childCount; ++ childIdx)
			{
	...
	//////////////////////////////////////////////////////////////////////////
	u32 CDocPatcher::GetCurrentVersion() const
	{
		return m_patches.empty() ? 100 : m_patches.back().toVersion;
	}

	//////////////////////////////////////////////////////////////////////////
	XmlNodeRef CDocPatcher::PatchDoc(XmlNodeRef xml, tukk szFileName, bool bFromPak)
	{
		u32 version = 0;
		xml->getAttr("version", version);
		u32k currentVersion = GetCurrentVersion();
		if(version < currentVersion)
		{
			...
			for(SPatch& patch : m_patches)
			{
				if(version == patch.fromVersion)
				{
					...
					{
						SXEMA2_SYSTEM_WARNING("Patch successful: %s (v%d->%d)", szFileName, version, patch.toVersion);
						version = patch.toVersion;
						xml->setAttr("version", version);
#if SXEMA2_PREVIEW_DOC_PATCHES
					...
					{
						SXEMA2_SYSTEM_CRITICAL_ERROR("Patch failed: %s (v%d->%d)", szFileName, version, patch.toVersion);
						return XmlNodeRef();
					}
				}
			}
		...
		{
			SXEMA2_SYSTEM_CRITICAL_ERROR("Unsupported document version: %s (v%d)", szFileName, version);
			return XmlNodeRef();
		}
		return xml;
	}
	...
	//////////////////////////////////////////////////////////////////////////
	CDocPatcher::SPatch::SPatch(u32 _fromVersion, u32 _toVersion, const PatchCallback& _callback)
	...
	//////////////////////////////////////////////////////////////////////////
	bool CDocPatcher::Patch101(XmlNodeRef xml, tukk szFileName) const
	{
		...
		if(xmlVariables)
		{
			for(i32 variableIdx = 0, variableCount = xmlVariables->getChildCount(); variableIdx < variableCount; ++ variableIdx)
			{
		...
		if(xmlGraphs)
		{
			for(i32 graphIdx = 0, graphCount = xmlGraphs->getChildCount(); graphIdx < graphCount; ++ graphIdx)
			{
				...
				if(xmlGraph->isTag("graph"))
				{
					...
					if(xmlGraphImpl)
					{
						...
						if(xmlGraphDetail)
						{
							for(i32 detailElementIdx = 0, detailElementCount = xmlGraphDetail->getChildCount(); detailElementIdx < detailElementCount; ++ detailElementIdx)
							{
					...
					if(xmlGraphDetail)
					{
						...
						if(xmlGraphNodes)
						{
							for(i32 graphNodeIdx = 0, graphNodeCount = xmlGraphNodes->getChildCount(); graphNodeIdx < graphNodeCount; ++ graphNodeIdx)
							{
						...
						if(xmlGraphLinks)
						{
							for(i32 graphLinkIdx = 0, graphLinkCount = xmlGraphLinks->getChildCount(); graphLinkIdx < graphLinkCount; ++ graphLinkIdx)
							{
	...
	//////////////////////////////////////////////////////////////////////////
	bool CDocPatcher::Patch102(XmlNodeRef xml, tukk szFileName) const
	{
		// Patch variables.
		XmlNodeRef xmlVariables = xml->findChild("variables");
		if(xmlVariables)
		...
		{
			for(i32 abstractInterfaceImplementationIdx = 0, abstractInterfaceImplementationCount = xmlAbstractInterfaceImplementations->getChildCount(); abstractInterfaceImplementationIdx < abstractInterfaceImplementationCount; ++ abstractInterfaceImplementationIdx)
			{
	...
	//////////////////////////////////////////////////////////////////////////
	bool CDocPatcher::Patch103(XmlNodeRef xml, tukk szFileName) const
	{
		// Patch timers.
		XmlNodeRef xmlTimers = xml->findChild("timers");
		if(xmlTimers)
		{
			for(i32 timerIdx = 0, timerCount = xmlTimers->getChildCount(); timerIdx < timerCount; ++ timerIdx)
			{
				...
				if(xmlTimer->isTag("timer"))
				{
					...
					if(xmlTimerUnits)
					{
						...
						if(xmlTimerDelay)
						{
							tukk szTimerUnits = xmlTimerUnits->getAttr("value");
					...
					if(xmlTimerFlags)
					{
						for(i32 timerFlagIdx = 0, timerFlagCount = xmlTimerFlags->getChildCount(); timerFlagIdx < timerFlagCount; ++ timerFlagIdx)
						{
	...
	//////////////////////////////////////////////////////////////////////////
	bool CDocPatcher::Patch104(XmlNodeRef xml, tukk szFileName) const
	{
