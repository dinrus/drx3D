...
{
	u32k CSpatialIndex::s_volumeIdxMask   = 0x0000ffff;
	u32k CSpatialIndex::s_volumeSaltShift = 16;
	u32k CSpatialIndex::s_volumeSaltMask  = 0xffff0000;
	...
	bool Serialize(Serialization::IArchive& archive, SSpatialVolumeTagName& value, tukk szName, tukk szLabel)
	{
		...
		{
			i32k pos = tagNames.find(value.value.c_str());
	...
	void CSpatialIndex::SSettings::RefreshTags()
	{
		...
		for(i32 tagValue = static_cast<i32>(ESpatialVolumeTags::Begin); tagValue != static_cast<i32>(ESpatialVolumeTags::End); tagValue <<= 1)
		{
			i32k tagIdx = tagsEnumDescription.indexByValue(tagValue);
			if(tagIdx != -1)
			{
	...
	SpatialVolumeId CSpatialIndex::CreateVolume(const SSpatialVolumeParams& params, const SpatialIndexEventCallback& eventCallback)
	{
		...
		{
			u32k volumeCount = m_volumes.size();
	...
	void CSpatialIndex::DestroyVolume(const SpatialVolumeId& id)
	{
		...
		{
			u32k volumeCount = m_volumes.size();
			u32k volumeIdx = VolumeIdxFromId(id);
			DRX_ASSERT(volumeIdx < volumeCount);
			if(volumeIdx < volumeCount)
			{
				SVolume& volume = m_volumes[volumeIdx];
				DRX_ASSERT(volume.id == id);
				if(volume.id == id)
				{
		...
		if(id != SpatialVolumeId::s_invalid)
		{
			...
			if(volumeIdx < volumeCount)
			{
				...
				if(volume.id == id)
				{
					...
					for(u32 otherVolumeIdx : m_broadPhaseResults)
					{
	...
	{
		i32k tagIdx = m_pSettings->tagNames.find(tagName.value.c_str());
	...
	SpatialVolumeId CSpatialIndex::GenerateVolumeId(u32 volumeIdx)
	{
		u32k          salt = m_salt ++;
		const SpatialVolumeId volumeId((volumeIdx & s_volumeIdxMask) | ((salt << s_volumeSaltShift) & s_volumeSaltMask));
		DRX_ASSERT(VolumeIdxFromId(volumeId) == volumeIdx);
		return volumeId;
	}

	u32 CSpatialIndex::VolumeIdxFromId(const SpatialVolumeId& id) const
	{
	...
	void CSpatialIndex::BroadPhaseQuery(u32 volumeIdx, ESpatialVolumeTags tags, VolumeIdxs& results) const
	{
		const SBroadPhaseVolume	broadPhaseVolume = m_broadPhaseVolumes[volumeIdx];
		for(u32 otherVolumeIdx = 0, volumeCount = m_volumes.size(); otherVolumeIdx < volumeCount; ++ otherVolumeIdx)
		{
	...
	void CSpatialIndex::UpdateMonitoredVolumes()
	{
		...
		for(SVolume& volume : m_volumes)
		{
			...
			for(u32 cacheVolumeIdx = 0, cacheVolumeCount = volume.monitorCache.size(); cacheVolumeIdx < cacheVolumeCount; )
			{
			...
			for(u32 newVolumeIdx = 0, newVolumeCount = m_monitorResults.size(); newVolumeIdx < newVolumeCount; ++ newVolumeIdx)
			{
