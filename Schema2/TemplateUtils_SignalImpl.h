...
namespace TemplateUtils
{
	...
	public:
		...
		inline ~CSignal()
		{
			for(typename SlotVector::iterator itSlot = m_slots.begin(), itEndSlot = m_slots.end(); itSlot != itEndSlot; ++ itSlot)
			{
				...
				if(pScope)
				{
					DRX_ASSERT((pScope->m_pSignal == this) || !pScope->m_pSignal);
		...
		{
			DRX_ASSERT((scope.m_pSignal == this) || !scope.m_pSignal);
			if((scope.m_pSignal == this) || !scope.m_pSignal)
			{
				Scope* pScope = reinterpret_cast<Scope*>(&reinterpret_cast<uk &>(scope)); // Temporary workaround for PS4 compilation error.
		...
		{
			DRX_ASSERT(scope.m_pSignal == this);
		...
		{
			DRX_ASSERT(m_recursionDepth < SIGNAL_MAX_RECURSION_DEPTH);
		...
		inline i16 GetRecursionDepth() const
		{
	...
	private:
		...
		{
			inline SFindSlotsByDelegate(const Delegate& _delegate, u32& _count)
				: delegate(_delegate)
				, count(_count)
			{}
		...
		{
			inline SFindSlotsByScope(const Scope* _pScope, u32& _count)
				: pScope(_pScope)
				, count(_count)
			{}
			...
			if(m_bDirty && !m_recursionDepth)
			{
				if(m_emptySlotCount)
				{
					...
					else
					{
						...
						DRX_ASSERT(itEraseSlot != itEndSlot);
		...
		u32     m_emptySlotCount;
		i16      m_recursionDepth;
		bool       m_bDirty;
		SlotVector m_slots;
	};
	...
	private:
		...
		{
			inline SFindSlotsByKeyAndDelegate(const Key& _key, const KeyCompare& _keyCompare, const Delegate& _delegate, u32& _count)
				: key(_key)
				, keyCompare(_keyCompare)
				, delegate(_delegate)
				, count(_count)
