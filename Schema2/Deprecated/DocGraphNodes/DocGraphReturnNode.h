...
namespace sxema2
{
	class CDocGraphReturnNode : public CDocGraphNodeBase
	{
	public:

		CDocGraphReturnNode(IScriptFile& file, IDocGraph& graph, const SGUID& guid = SGUID(), const SGUID& contextGUID = SGUID(), const SGUID& refGUID = SGUID(), Vec2 pos = Vec2(ZERO));
	...
	private:
		...
		typedef std::vector<IAnyPtr> OutputValues;

		void Validate(Serialization::IArchive& archive);
		void Return(IDocGraphNodeCompiler& compiler) const;
