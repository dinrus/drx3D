...
namespace sxema2
{
	enum class EDocGraphSendSignalNodeTarget
	{
		Self = 0,
		Object,
		Broadcast
	...
	class CDocGraphSendSignalNode : public CDocGraphNodeBase
	{
	public:

		CDocGraphSendSignalNode(IScriptFile& file, IDocGraph& graph, const SGUID& guid = SGUID(), const SGUID& contextGUID = SGUID(), const SGUID& refGUID = SGUID(), Vec2 pos = Vec2(ZERO));
	...
	private:
		...
		size_t GetFirstParamInput() const;
		void CompileInputs(IDocGraphNodeCompiler& compiler) const;

		EDocGraphSendSignalNodeTarget m_target;
		StringVector                  m_signalInputNames;
		IAnyPtrVector                 m_signalInputValues;
	};
}
