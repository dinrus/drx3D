...
#define SXEMA2_MAKE_ACTION_MEMBER_FUNCTION_SHARED(function, guid, actionGUID) sxema2::SMakeActionMemberFunctionShared<TYPE_OF(&function)>()(&function, guid, actionGUID, TemplateUtils::GetFunctionName<TYPE_OF(&function), &function>(), __FILE__, "Code")
...
namespace sxema2
{
	...
	namespace ActionMemberFunctionUtils
	{
		...
		template <> struct GetStorageType<tukk >
		{
			typedef tukk Result;
		};
		...
		template <typename TYPE, u32 SIZE> struct GetDefaultValue<TYPE[SIZE]>
		{
		...
		template<> inline SParam MakeParam<tukk >(EParamFlags flags)
		{
			return SParam(MakeAnyShared(CPoolString()), flags | GetParamFlags<tukk >());
		}
		...
		public:
			...
			inline CBase(const SGUID& guid, const SGUID& actionGUID, tukk szDeclaration, tukk szFileName, tukk szProjectDir)
				: m_guid(guid)
			...
			virtual bool BindInput(u32 paramIdx, tukk szName, tukk szDescription) override
			{
				SXEMA2_SYSTEM_ASSERT_FATAL(szName && szDescription);
			...
			virtual IAnyConstPtr GetInputValue(u32 inputIdx) const override
			{
			...
			virtual bool BindOutput(u32 paramIdx, tukk szName, tukk szDescription) override
			{
				SXEMA2_SYSTEM_ASSERT_FATAL(szName && szDescription);
			...
			virtual IAnyConstPtr GetOutputValue(u32 outputIdx) const override
			{
		...
		protected:
			...
			virtual bool BindInput_Protected(u32 paramIdx, tukk szName, tukk szDescription, const IAny& value) override
			{
				if(CanBindInput(paramIdx))
				{
					SParam& param = m_params[paramIdx];
					SXEMA2_SYSTEM_ASSERT_FATAL(value.GetTypeInfo().GetTypeId() == param.pValue->GetTypeInfo().GetTypeId());
			...
			template <typename TYPE> inline void LoadParam(CVariantArrayInputArchive& archive, u32 paramIdx, TYPE& value) const
			{
			...
			inline void LoadParam(CVariantArrayInputArchive& archive, u32 paramIdx, tukk & value) const
			{
				if((m_params[paramIdx].flags & EParamFlags::IsInput) != 0)
				{
					archive(value);
			...
			template <typename TYPE> inline void StoreParam(CVariantArrayOutputArchive& archive, u32 paramIdx, TYPE& value) const
			{
			...
			template <typename TYPE> inline void StoreParam(CVariantArrayOutputArchive& archive, u32 paramIdx, const TYPE& value) const
			{
				if((m_params[paramIdx].flags & EParamFlags::IsOutput) != 0)
				{
			...
			void StoreParam(CVariantArrayOutputArchive& archive, u32 paramIdx, tukk & value) const
			{
				if((m_params[paramIdx].flags & EParamFlags::IsOutput) != 0)
				{
					archive(value);
		...
		private:
			...
			inline bool CanBindInput(u32 paramIdx) const
			{
				SXEMA2_SYSTEM_ASSERT_FATAL(paramIdx < m_params.size());
				if(paramIdx < m_params.size())
				{
					...
					SXEMA2_SYSTEM_ASSERT_FATAL(!bIsReturn && !bIsBound);
			...
			inline bool CanBindOutput(u32 paramIdx) const
			{
				SXEMA2_SYSTEM_ASSERT_FATAL(paramIdx < m_params.size());
				if(paramIdx < m_params.size())
				{
					...
					const bool bIsBound = (m_params[paramIdx].flags & (EParamFlags::IsInput | EParamFlags::IsOutput)) != 0;
					SXEMA2_SYSTEM_ASSERT_FATAL((bIsReturn || (bIsReferenceOrPointer && !bIsConstTarget)) && !bIsBound);
	...
	public:
		...
		inline CActionMemberFunction(FunctionPtr pFunction, const SGUID& guid, const SGUID& actionGUID, tukk szDeclaration, tukk szFileName, tukk szProjectDir)
	...
	template <typename ACTION> struct SMakeActionMemberFunctionShared<void (ACTION::*)()>
	{
		...
		inline IActionMemberFunctionPtr operator () (FunctionPtr pFunction, const SGUID& guid, const SGUID& actionGUID, tukk szDeclaration, tukk szFileName, tukk szProjectDir) const
		{
