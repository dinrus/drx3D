...
#define SXEMA2_MAKE_GLOBAL_FUNCTION_SHARED(function, guid) sxema2::SMakeGlobalFunctionShared<TYPE_OF(&function)>()(&function, guid, TemplateUtils::GetFunctionName<TYPE_OF(&function), &function>(), __FILE__, "Code")
...
namespace sxema2
{
	...
	namespace GlobalFunction
	{
		...
		template<> inline SParam MakeParam<tukk >(ParamFlags::EValue flags)
		{
		...
		public:
			...
			inline CBase(const SGUID& guid, tukk declaration, tukk fileName, tukk projectDir)
				: m_guid(guid)
			...
			virtual bool BindInput(size_t iParam, tukk name, tukk szDescription) override
			{
				SXEMA2_SYSTEM_ASSERT_FATAL((name != NULL) && (szDescription != NULL));
			...
			virtual bool BindOutput(size_t iParam, tukk name, tukk szDescription) override
			{
				SXEMA2_SYSTEM_ASSERT_FATAL((name != NULL) && (szDescription != NULL));
		...
		protected:
			...
			virtual bool BindInput_Protected(size_t iParam, tukk name, tukk szDescription, const IAny& value) override
			{
				if(CanBindInput(iParam) == true)
				{
					SParam&	param = m_params[iParam];
			...
			inline void LoadParam(CVariantArrayInputArchive& archive, size_t iParam, tukk & value) const
			{
				if((m_params[iParam].flags & ParamFlags::IS_INPUT) != 0)
				{
					archive(value);
			...
			void StoreParam(CVariantArrayOutputArchive& archive, size_t iParam, tukk & value) const
			{
				if((m_params[iParam].flags & ParamFlags::IS_OUTPUT) != 0)
				{
					archive(value);
		...
		private:
			...
			{
				SXEMA2_SYSTEM_ASSERT_FATAL(iParam < m_params.size());
				if(iParam < m_params.size())
				{
					...
					SXEMA2_SYSTEM_ASSERT_FATAL(!isReturn && !isBound);
					if(!isReturn && !isBound)
					{
						return true;
					}
			...
			inline bool CanBindOutput(size_t iParam) const
			{
				...
				if(iParam < m_params.size())
				{
					...
					const bool	isBound = (m_params[iParam].flags & (ParamFlags::IS_INPUT | ParamFlags::IS_OUTPUT)) != 0;
					SXEMA2_SYSTEM_ASSERT_FATAL((isReturn || (isReferenceOrPointer && !isConstTarget)) && !isBound);
	...
	public:
		...
		inline CGlobalFunction(TFunctionPtr functionPtr, const SGUID& guid, tukk declaration, tukk fileName, tukk projectDir)
	...
	template <> struct SMakeGlobalFunctionShared<void (*)()>
	{
		...
		inline IGlobalFunctionPtr operator () (TFunctionPtr functionPtr, const SGUID& guid, tukk declaration, tukk fileName, tukk projectDir) const
		{
