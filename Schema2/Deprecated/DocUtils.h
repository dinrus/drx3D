...
namespace sxema2
{
	...
	namespace DocUtils
	{
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////
		inline u32 Count(const IScriptFile& file, const SGUID& scopeGUID, EScriptElementType elementType, EVisitFlags visitFlags)
		{
			u32 count = 0;
		...
		{
			inline SGraphNodeInputLinkNameFilter(const SGUID& _dstNodeGUID, tukk _szDstInputName)
				: dstNodeGUID(_dstNodeGUID)
		...
		//////////////////////////////////////////////////////////////////////////
		inline void CollectGraphNodeInputLinks(IScriptGraph& graph, const SGUID& dstNodeGUID, tukk szDstInputName, TScriptGraphLinkVector& results)
		{
			typedef CElementCollector<IScriptGraph, IScriptGraphLink, SGraphNodeInputLinkNameFilter> Collector;

			LOADING_TIME_PROFILE_SECTION;

			SXEMA2_SYSTEM_ASSERT(szDstInputName);
			if(szDstInputName)
			{
		...
		//////////////////////////////////////////////////////////////////////////
		inline void CollectGraphNodeInputLinks(const IScriptGraph& graph, const SGUID& dstNodeGUID, tukk szDstInputName, TScriptGraphLinkConstVector& results)
		{
		...
		{
			inline SGraphNodeOutputLinkNameFilter(const SGUID& _srcNodeGUID, tukk _szSrcOutputName)
			...
			tukk szSrcOutputName;
		};

		//////////////////////////////////////////////////////////////////////////
		inline void CollectGraphNodeOutputLinks(IScriptGraph& graph, const SGUID& srcNodeGUID, tukk szSrcOutputName, TScriptGraphLinkVector& results)
		{
			typedef CElementCollector<IScriptGraph, IScriptGraphLink, SGraphNodeOutputLinkNameFilter> Collector;

			LOADING_TIME_PROFILE_SECTION;

			SXEMA2_SYSTEM_ASSERT(szSrcOutputName);
			if(szSrcOutputName)
			{
		...
		//////////////////////////////////////////////////////////////////////////
		inline void CollectGraphNodeOutputLinks(const IScriptGraph& graph, const SGUID& srcNodeGUID, tukk szSrcOutputName, TScriptGraphLinkConstVector& results)
		{
		...
		inline void GetFullElementName(const IScriptFile& file, const IScriptElement& element, stack_string& output, EScriptElementType ownerElementType = EScriptElementType::None)
		{
			...
			{
				tukk szName = pScopeElement->GetName();
				if(szName[0])
				{
					if(!output.empty())
					{
		...
		//////////////////////////////////////////////////////////////////////////
		inline bool IsElementAvailableInScope(const IScriptFile& file, const SGUID& scopeGUID, tukk szNamespace)
		{
			const IScriptClass* pClass = FindOwnerClass(file, scopeGUID);
			if(pClass)
			{
		...
		//////////////////////////////////////////////////////////////////////////
		inline bool IsElementAvailableInScope(const IScriptFile& file, const SGUID& scopeGUID, EEnvElementType ownerType, const SGUID& ownerGUID, tukk szNamespace)
		{
			switch(ownerType)
			{
		...
		inline bool IsSignalAvailableInScope(const IScriptFile& file, const SGUID& scopeGUID, const SGUID& ownerGUID, tukk szNamespace)
		{
			if(ownerGUID)
			{
		...
		//////////////////////////////////////////////////////////////////////////
		inline size_t FindGraphInput(const IDocGraph& graph, tukk name)
		{
			SXEMA2_SYSTEM_ASSERT(name);
			if(name)
			{
		...
		//////////////////////////////////////////////////////////////////////////
		inline size_t FindGraphOutput(const IDocGraph& graph, tukk name)
		{
			SXEMA2_SYSTEM_ASSERT(name);
			if(name)
			{
		...
		//////////////////////////////////////////////////////////////////////////
		inline size_t FindGraphNodeInput(const IScriptGraphNode& graphNode, tukk name)
		{
			SXEMA2_SYSTEM_ASSERT(name);
			if(name)
			{
		...
		//////////////////////////////////////////////////////////////////////////
		inline size_t FindGraphNodeOutput(const IScriptGraphNode& graphNode, tukk name)
		{
			SXEMA2_SYSTEM_ASSERT(name);
			if(name)
			{
		...
		inline void UnrollGraphSequenceRecursive_Deprecated(const IScriptGraph& graph, const SGUID& nodeGUID, GUIDVector& results)
		{
			...
			if(pGraphNode)
			{
				if(std::find(results.begin(), results.end(), nodeGUID) == results.end())
				{
					...
					{
						tukk 	inputName = pGraphNode->GetInputName(iInput);
					...
					for(size_t iGraphNodeOutput = 0, graphNodeOutputCount = pGraphNode->GetOutputCount(); iGraphNodeOutput < graphNodeOutputCount; ++ iGraphNodeOutput)
					{
						...
						{
							tukk 	outputName = pGraphNode->GetOutputName(iGraphNodeOutput);
							for(size_t iLink = 0, linkCount = graph.GetLinkCount(); iLink < linkCount; ++ iLink)
							{
								const IScriptGraphLink* pGraphLink = graph.GetLink(iLink);
