...
{
	DRX_ASSERT(id1 >= MNM::Constants::eStaticIsland_FirstValidIslandID);
	DRX_ASSERT(id2 >= MNM::Constants::eStaticIsland_FirstValidIslandID);
...
void CIslands::ComputeStaticIslandsAndConnections(CNavMesh& navMesh, const NavigationMeshID meshID, const OffMeshNavigationUpr& offMeshNavigationUpr, MNM::IslandConnections& islandConnections)
{
	...
	ResolvePendingConnectionRequests(navMesh, connectionRequests, meshID, &offMeshNavigationUpr, islandConnections);
}
...
void CIslands::ResetConnectedIslandsIDs(CNavMesh& navMesh)
{
	for (CNavMesh::TileMap::iterator tileIt = navMesh.m_tileMap.begin(); tileIt != navMesh.m_tileMap.end(); ++tileIt)
	{
		...
		for (u16 i = 0; i < tile.triangleCount; ++i)
		{
	...
	std::function<bool(const STile& prevTile, const Tile::STriangle& prevTriangle, const STile& nextTile, Tile::STriangle& nextTriangle)> executeFunc)
{
	...
	while (!trianglesToVisit.empty())
	{
		...
		for (u16 l = 0; l < currentTriangle.linkCount; ++l)
		{
...
void CIslands::ComputeStaticIslands(CNavMesh& navMesh, SConnectionRequests& connectionRequests)
{
	...
	for (CNavMesh::TileMap::iterator tileIt = navMesh.m_tileMap.begin(); tileIt != navMesh.m_tileMap.end(); ++tileIt)
	{
		...
		for (u16 triangleIndex = 0; triangleIndex < tile.triangleCount; ++triangleIndex)
		{
			...
			for (size_t index = 0; index < totalTrianglesToVisit; ++index)
			{
				...
				const TileID currentTileId = ComputeTileID(currentTriangleID);
				DRX_ASSERT_MESSAGE(currentTileId > 0, "ComputeStaticIslands is trying to access a triangle ID associated with an invalid tile id.");
...
void CIslands::UpdateIslandsForTriangles(CNavMesh& navMesh, const NavigationMeshID meshID, const MNM::TriangleID* pTrianglesArray, const size_t trianglesCount, MNM::IslandConnections& islandConnections)
{
	...
	for (size_t i = 0; i < trianglesCount; ++i)
	{
		...
		DRX_ASSERT(island.trianglesCount >= 0);
	...
	for (size_t i = 0; i < trianglesCount; ++i)
	{
		...
		for (u16 l = 0; l < sourceTriangle.linkCount; ++l)
		{
			...
			[this, &sourceIsland, &connectionRequests](const STile& prevTile, const Tile::STriangle& currentTriangle, const StaticIslandID prevValue, const STile& nextTile, Tile::STriangle& nextTriangle, StaticIslandID& nextPrevValue)
		{
			...
			{
				DRX_ASSERT(nextIslandId != currentTriangle.islandID);
				if (nextIslandId != MNM::Constants::eStaticIsland_InvalidIslandID)
				{
	...
	for (const auto& islandSplitSources : splitUpdateSources)
	{
		...
		for (const MNM::TriangleID sourceTriangleID : islandSplitSources.second)
		{
			...
			if (wantCreateNewIsland)
			{
				...
				DRX_ASSERT(sourceIsland.trianglesCount > 0);
			}
			else
			{
...
{
	DRX_ASSERT(m_islands.size() != std::numeric_limits<StaticIslandID>::max());
	...
	if (m_islandsFreeIndices.size())
	{
		...
		DRX_ASSERT(freeIdx < m_islands.size());
		DRX_ASSERT(m_islands[freeIdx].trianglesCount == 0);
	...
	DRX_ASSERT_MESSAGE(GetIslandID(id) == id, "CIslands::GetNewIsland: Too many islands created!");
...
void CIslands::ResolvePendingConnectionRequests(CNavMesh& navMesh, SConnectionRequests& connectionRequests, const NavigationMeshID meshID, const OffMeshNavigationUpr* pOffMeshNavigationUpr,
	MNM::IslandConnections& islandConnections)
{
	if (pOffMeshNavigationUpr)
	{
		const OffMeshNavigation& offMeshNavigation = pOffMeshNavigationUpr->GetOffMeshNavigationForMesh(meshID);
		...
		while (!connectionRequests.offmeshConnections.empty())
		{
			...
			while (const WayTriangleData nextTri = links.GetNextTriangle())
			{
				...
				{
					const OffMeshLink* pLink = pOffMeshNavigationUpr->GetOffMeshLink(nextTri.offMeshLinkID);
					DRX_ASSERT(pLink);
	...
	for (const auto& keyValue : connectionRequests.areaConnection)
	{
		...
		i32k connectionsChange = keyValue.second;
...
void CIslands::SConnectionRequests::AddOffmeshConnection(const StaticIslandID startingIslandId, const TriangleID startingTriangleId, u16k offMeshLinkIndex)
{
