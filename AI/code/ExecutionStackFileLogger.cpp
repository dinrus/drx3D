...
ExecutionStackFileLogger::ExecutionStackFileLogger(const EntityId entityId)
{
	...
	m_logFilePath[0] = '\0';
	if (gEnv->pDrxPak->AdjustFileName(filePath.c_str(), m_logFilePath, IDrxPak::FLAGS_FOR_WRITING))
	{
	...
	if (m_openState == NotYetAttemptedToOpenForWriteAccess)
	{
		...
		if (m_openState == OpenForWriteAccessSucceeded)
		{
			...
			if (time(&ltime) != (time_t)-1)
			{
				...
				{
					strftime(startTime, DRX_ARRAY_COUNT(startTime), "%Y-%m-%d %H:%M:%S", pTm);
				}
			}
...
void ExecutionStackFileLogger::LogNodeRecursively(const DebugNode& debugNode, const UpdateContext& updateContext, const BehaviorTreeInstance& instance, i32k indentLevel)
{
	const Node* pNode = static_cast<const Node*>(debugNode.node);
	u32k lineNum = pNode->GetXmlLine();
	tukk nodeType = pNode->GetCreator()->GetTypeName();
