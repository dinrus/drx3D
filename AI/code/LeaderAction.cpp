...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   LeaderAction.cpp

   Описание: Class Implementation for CLeaderAction (and inherited)
...
u32k CLeaderAction_Search::m_CCoverUnitProperties = UPR_COMBAT_GROUND;
...
i32 CLeaderAction::GetNumLiveUnits()  const
{
	...
	, m_timeRunning(0.0f)
{
	...
	{
		i32 numFiringUnits = m_pLeader->GetAIGroup()->GetUnitCount(m_CCoverUnitProperties);
...
void CLeaderAction_Search::PopulateSearchSpotList(Vec3& initPos)
{
	...
	{
		for (AutoAIObjectIter it(gAIEnv.pAIObjectUpr->GetFirstAIObject(OBJFILTER_TYPE, m_iSearchSpotAIObjectType)); it->GetObject(); it->Next())
		{
...
void CLeaderAction_Attack_SwitchPositions::UpdatePointList(CAIObject* pTarget)
{
	...
	if (!m_vUpdatePointTarget.IsEquivalent(pTarget->GetPos()))
	{
		...
		gAIEnv.pSmartObjectUpr->TriggerEvent("CheckTargetNear", pTargetEntity, pDummyEntity, &queryEvents);

		CAIObject* pFormationOwner = m_pLeader->GetFormationOwner().GetAIObject();
		i32 size = m_PointProperties.size();
		...
		if (pSenderEntity && (pUnit = (CAIObject*)(pSenderEntity->GetAI())))
		{
			...
			if (pAIActor)
			{
				...
				if (itUnit != GetUnitsList().end())
				{
					...
					if (signal.pEData)
					{
						m_ActorForbiddenSpotUpr.AddSpot(GetWeakRef(pAIActor), signal.pEData->point);
						GetBaseSearchPosition(*itUnit, pTarget, signal.pEData->iValue, signal.pEData->fValue);
					}
				}
			}
	...
	else if (signal.Compare(gAIEnv.SignalCRCs.m_nOnClearSpotList))
	{
		...
		if (pSenderEntity && (pUnit = (CAIObject*)(pSenderEntity->GetAI())))
		{
			...
			if (pActor)
			{
				...
				m_ActorForbiddenSpotUpr.RemoveAllSpots(GetWeakRef(pActor));
			}
		}
		return true;
	}
...
void CLeaderAction_Attack_SwitchPositions::AssignNewShootSpot(CAIObject* pUnit, i32 index)
{
	CAIObject* pFormationOwner = m_pLeader->GetFormationOwner().GetAIObject();
	CFormation* pFormation = pFormationOwner ? pFormationOwner->m_pFormation : NULL;
	i32 fsize = pFormation ? pFormation->GetSize() : 0;
...
void CLeaderAction_Attack_SwitchPositions::UpdateSpecialActions()
{
	...
	i32 size = m_pLeader->GetAIGroup()->GetGroupCount(IAISystem::GROUP_ENABLED);
...
CLeaderAction::eActionUpdateResult CLeaderAction_Attack_SwitchPositions::Update()
{
	...
	if (!m_bPointsAssigned)
	{
		...
		for (TUnitList::iterator unitItr = GetUnitsList().begin(); unitItr != itEnd; ++unitItr)
		{
			...
			CPipeUser* pPiper = pUnit->CastToCPipeUser();
			i32 iPointIndex = bAssignPoint ? GetFormationPointWithTarget(curUnit) : -1;
			if (pPiper)
			{
				CAIObject* pTarget = (CAIObject*)pPiper->GetAttentionTarget();
				if (!pTarget)
		...
		for (TUnitList::iterator unitItr = GetUnitsList().begin(); unitItr != itEnd; ++unitItr)
		{
			...
			if (pUnit->IsEnabled() && pPuppet)
			{
				...
				if (!curUnit.IsSpecial() && (curUnit.IsMoving() || curUnit.IsFollowing() && (!pTarget || !pTarget->IsAgent())))
				{
					...
					i32 iPointIndex = GetFormationPointWithTarget(curUnit);
...
i32 CLeaderAction_Attack_SwitchPositions::GetFormationPointWithTarget(CUnitImg& unit)
{
	...
	i32 myCurrentPointIndex = unit.m_FormationPointIndex;
	if (pBeacon)
	{
		...
		if (pFormation)
		{
			...
			i32 formationSize = pFormation->GetSize();
			...
			i32 size = (m_pLiveTarget ? formationSize : m_PointProperties.size());
			...
			for (i32 i = 1; i < size; i++)//exclude owner's point
			{
				Vec3 pos;
				if (i < formationSize)
				{
				...
				if (!m_ActorForbiddenSpotUpr.IsForbiddenSpot(unit.m_refUnit, pos))
				{
...
//
//----------------------------------------------------------------------------------------------------
void CLeaderAction_Attack_SwitchPositions::GetBaseSearchPosition(CUnitImg& unit, CAIObject* pTarget, i32 method, float distance)
{
	Vec3 unitpos = unit.m_refUnit.GetAIObject()->GetPos();
	if (!pTarget)
	{
...
void CLeaderAction_Attack_SwitchPositions::Serialize(TSerialize ser)
{
	...
	{
		i32 size = m_SpecialActions.size();
		ser.Value("size", size);
	...
	m_ActorForbiddenSpotUpr.Serialize(ser);
}
