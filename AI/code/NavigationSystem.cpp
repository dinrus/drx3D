...
struct SAgentTypeListAutoComplete : public IConsoleArgumentAutoComplete
{
	virtual i32 GetCount() const
	{
		...
		return static_cast<i32>(gAIEnv.pNavigationSystem->GetAgentTypeCount());
	}
...
void GenerateTileJob(MNM::CTileGenerator::Params params,  u16* state, MNM::STile* tile, MNM::CTileGenerator::SMetaData* metaData, u32* hashValue)
{
...
u32 NameHash(tukk name)
{
	...
	for (u32 i = 0; i < len; ++i)
	{
		hash += (u8)(DrxStringUtils::toLowerAscii(name[i]));
		hash += (hash << 10);
		hash ^= (hash >> 6);
	}
...
bool ShouldBeConsideredByVoxelizer(IPhysicalEntity& physicalEntity, u32& flags)
{
...
NavigationSystem::NavigationSystem(tukk configName)
	...
	, m_offMeshNavigationUpr(256)
	, m_islandConnectionsUpr()
	, m_volumes(512)
	, m_markupVolumes(512)
	, m_markupsData(512)
	...
	, m_tileGeneratorExtensionsContainer()
{
	...
	m_worldMonitor = WorldMonitor(functor(m_updatesUpr, &CMNMUpdatesUpr::EntityChanged));
...
NavigationAgentTypeID NavigationSystem::CreateAgentType(tukk name, const CreateAgentTypeParams& params)
{
	assert(name);
...
NavigationAgentTypeID NavigationSystem::GetAgentTypeID(tukk name) const
{
	assert(name);
	...
	for (; it != end; ++it)
	{
		...
		if (!agentType.name.compareNoCase(name))
			return NavigationAgentTypeID((u32)(std::distance(m_agentTypes.begin(), it) + 1));
	}
...
tukk NavigationSystem::GetAgentTypeName(NavigationAgentTypeID agentTypeID) const
{
...
NavigationMeshID NavigationSystem::CreateMesh(tukk name, NavigationAgentTypeID agentTypeID,
...
#endif
{
	...
	if (agentTypeID && (agentTypeID <= m_agentTypes.size()))
	{
		...
		m_offMeshNavigationUpr.OnNavigationMeshCreated(id);
...
NavigationMeshID NavigationSystem::CreateMeshForVolumeAndUpdate(tukk name, NavigationAgentTypeID agentTypeID, const CreateMeshParams& params, const NavigationVolumeID volumeID)
{
	if (volumeID && m_volumes.validate(volumeID))
	{
		...
		m_updatesUpr.RequestMeshUpdate(meshID, volume.aabb);
...
void NavigationSystem::DestroyMesh(NavigationMeshID meshID)
{
	if (meshID && m_meshes.validate(meshID))
	{
		...
		AILogComment("NavigationSystem::DestroyMesh meshID = %u '%s'", (u32)meshID, mesh.name.c_str());
		...
		{
			if (m_results[m_runningTasks[t]].meshID == meshID)
				gEnv->pJobUpr->WaitForJob(m_results[m_runningTasks[t]].jobState);
		}
		...
		m_updatesUpr.OnMeshDestroyed(meshID);

		m_meshes.erase(meshID);

		m_offMeshNavigationUpr.OnNavigationMeshDestroyed(meshID);
...
#endif
{
	if (meshID && m_meshes.validate(meshID))
	{
		...
		{
			AIWarning("NavigationSystem::SetMeshBoundaryVolume: setting invalid volumeID %u for a mesh %u '%s'", (u32)volumeID, (u32)meshID, mesh.name.c_str());
			volumeID = NavigationVolumeID();
		}

		AILogComment("NavigationSystem::SetMeshBoundaryVolume: set volumeID %u for a mesh %u '%s'", (u32)volumeID, (u32)meshID, mesh.name.c_str());
...
void NavigationSystem::DestroyVolume(NavigationVolumeID volumeID)
{
	if (volumeID && m_volumes.validate(volumeID))
	{
		...
		if (gEnv->IsEditor())
		{
			m_volumesUpr.InvalidateID(volumeID);
		}
			...
			for (; it != end; ++it)
			{
				...
				for (; mit != mend; ++mit)
				{
					...
					if (mesh.boundary == volumeID)
					{
						...
						m_updatesUpr.RequestMeshDifferenceUpdate(meshID, volume, newVolume);
					}

					if (std::find(mesh.exclusions.begin(), mesh.exclusions.end(), volumeID) != mesh.exclusions.end())
					{
						m_updatesUpr.RequestMeshUpdate(meshID, volume.aabb);
						m_updatesUpr.RequestMeshUpdate(meshID, newVolume.aabb);
						++mesh.version;
					}
				}
			}
		...
		for (size_t i = 0; i < agentTypeIDCount; ++i)
		{
			if (agentTypeIDs[i] && (agentTypeIDs[i] <= m_agentTypes.size()))
			{
				...
				for (; mit != mend; ++mit)
				{
					...
					{
						AILogComment("NavigationSystem::SetExclusionVolume: volumeID %u for a mesh %u '%s'", (u32)volumeID, (u32)meshID, mesh.name.c_str());
	...
	const NavigationVolumeID volumeID, const MNM::SMarkupVolumeParams& params)
{
	...
	for (AgentType& agentType : m_agentTypes)
	{
		...
		for (const AgentType::MeshInfo& meshInfo : agentType.meshes)
		{
			...
			{
				m_updatesUpr.RequestMeshUpdate(meshID, volume.aabb, false, true);
				++mesh.version;
			}
		}
	}
		...
		for (size_t i = 0; i < agentsCount; ++i)
		{
			...
			for (const AgentType::MeshInfo& meshInfo : agentType.meshes)
			{
				...
				if (newVolume.Overlaps(m_volumes[mesh.boundary].aabb))
				{
					...
					m_updatesUpr.RequestMeshUpdate(meshID, newVolume.aabb, false, true);
				}
			}
		}
	}
...
{	
	...
	MNM::IslandConnections& islandConnections = m_islandConnectionsUpr.GetIslandConnections();
 ...
 */
NavigationMeshID NavigationSystem::GetMeshID(tukk name, NavigationAgentTypeID agentTypeID) const
{
	assert(name && agentTypeID && (agentTypeID <= m_agentTypes.size()));

	if (agentTypeID && (agentTypeID <= m_agentTypes.size()))
	{
		u32 nameHash = NameHash(name);
...
tukk NavigationSystem::GetMeshName(NavigationMeshID meshID) const
{
	if (meshID && m_meshes.validate(meshID))
	{
...
void NavigationSystem::SetMeshName(NavigationMeshID meshID, tukk name)
{
	if (meshID && m_meshes.validate(meshID))
	{
		NavigationMesh& mesh = m_meshes[meshID];
...
void NavigationSystem::UpdateInternalNavigationSystemData(const bool blocking)
{
	...
	DRX_ASSERT_MESSAGE(m_pEditorBackgroundUpdate->IsRunning() == false, "Background update for editor is still running while the application has the focus!!");
	...
	static i32 lastUpdateFrameID = 0;

	i32k frameID = gEnv->nMainFrameID;
...
{
	m_offMeshNavigationUpr.ProcessQueuedRequests();
}
...
u32 NavigationSystem::GetWorkingQueueSize() const 
{ 
	return (u32)m_updatesUpr.GetRequestQueueSize();
}
...
void NavigationSystem::UpdateMeshes(const float frameTime, const bool blocking, const bool multiThreaded, const bool bBackground)
{
	...
	m_debugDraw.UpdateWorkingProgress(frameTime, m_updatesUpr.GetRequestQueueSize());

	if (!m_updatesUpr.HasUpdateRequests() && m_runningTasks.empty())
	{
		if (m_state != Idle)
		{
	...
	while (true)
	{
		...
		{
			DRX_PROFILE_REGION(PROFILE_AI, "Navigation System::UpdateMeshes() - Running Task Processing");
			...
			for (; it != end; )
			{
				...
				{
					DRX_PROFILE_REGION_WAITING(PROFILE_AI, "Navigation System::UpdateMeshes() - Running Task Processing - WaitForJob");
					gEnv->GetJobUpr()->WaitForJob(result.jobState);
				}
				...
				m_free = static_cast<u16>(resultSlot);
		...
		if (m_updatesUpr.HasUpdateRequests())
		{
			m_state = Working;

			DRX_PROFILE_REGION(PROFILE_AI, "Navigation System::UpdateMeshes() - Job Spawning");
			...
			while (m_updatesUpr.HasUpdateRequests() && (m_runningTasks.size() < MaxRunningTaskCount))
			{
				const CMNMUpdatesUpr::TileUpdateRequest& task = m_updatesUpr.GetFrontRequest();

				if (task.IsAborted())
				{
					m_updatesUpr.PopFrontRequest();
					continue;
				}
				...
				DRX_ASSERT_MESSAGE(m_free < m_results.size(), "Index out of array bounds!");
				TileTaskResult& result = m_results[m_free];
				m_free = result.next;

				if (!SpawnJob(result, task.meshID, paramsGrid, task.x, task.y, task.z, multiThreaded, task.CheckFlag(CMNMUpdatesUpr::TileUpdateRequest::EFlag::MarkupUpdate)))
				{
					result.state = TileTaskResult::Running;
					result.next = m_free;
			...
			if (blocking && m_updatesUpr.HasUpdateRequests() && multiThreaded)
			{
				while (m_updatesUpr.HasUpdateRequests())
				{
					const CMNMUpdatesUpr::TileUpdateRequest& task = m_updatesUpr.GetFrontRequest();
					...
					if (!SpawnJob(result, task.meshID, paramsGrid, task.x, task.y, task.z, false, task.CheckFlag(CMNMUpdatesUpr::TileUpdateRequest::EFlag::MarkupUpdate)))
						break;
		...
		if (blocking && (m_updatesUpr.HasUpdateRequests() || !m_runningTasks.empty()))
			continue;
...
void NavigationSystem::SetupGenerator(
	...
	u16 x, u16 y, u16 z, MNM::CTileGenerator::Params& params,
	const VolumeDefCopy& defCopy, bool bMarkupUpdate)
{
	...
	params.exclusionCount = static_cast<u16>(defCopy.exclusions.size());
	...
	DRX_ASSERT(defCopy.markups.size() == defCopy.markupIds.size());
	params.markups = defCopy.markups.data();
	params.markupIds = defCopy.markupIds.data();
	params.markupsCount = static_cast<u16>(defCopy.markups.size());
...
bool NavigationSystem::SpawnJob(TileTaskResult& result, NavigationMeshID meshID, const MNM::CNavMesh::SGridParams& paramsGrid,
                                u16 x, u16 y, u16 z, bool bMt, bool bMarkupUpdate)
{
	result.x = x;
	result.y = y;
	result.z = z;
	...
	else
	{
		...
		{
			AIWarning("NavigationSystem::SpawnJob: Detected non-valid mesh boundary volume (%u) for mesh %u '%s', skipping", (u32)mesh.boundary, (u32)meshID, mesh.name.c_str());
			return false;
		}
		...
		for (; eit != eend; ++eit)
		{
			...
			{
				DrxLogAlways("NavigationSystem::SpawnJob(): Detected non-valid exclusion volume (%d) for mesh '%s', skipping", (u32)exclusionVolumeID, mesh.name.c_str());
			}
		}
		...
		{
			DRX_ASSERT(m_markupVolumes.validate(markupVolumeId));
	...
	result.volumeCopy = static_cast<u16>(index);
...
{
	DRX_ASSERT(m_markupsData.capacity() == m_markupVolumes.capacity());
	...
	for (NavigationVolumeID markupID : def.markupIds)
	{
		if (m_markupsData.validate(markupID))
		{
			...
			DRX_ASSERT(m_markupVolumes.validate(markupID));
	...
	for (const MNM::CTileGenerator::SMetaData::SMarkupTriangles& markupTriangles : result.metaData.markupTriangles)
	{
		...
		for (u16 triangleIdx : markupTriangles.trianglesIdx)
		{
...
void NavigationSystem::CommitTile(TileTaskResult& result)
{
	...
	case TileTaskResult::Completed:
		{
			DRX_PROFILE_REGION(PROFILE_AI, "Navigation System::CommitTile() - Running Task Processing - ConnectToNetwork");
			...
			m_offMeshNavigationUpr.RefreshConnections(result.meshID, tileID);
	...
	case TileTaskResult::Failed:
		{
			DRX_PROFILE_REGION(PROFILE_AI, "Navigation System::CommitTile() - Running Task Processing - ClearTile");
...
{
	AIWarning("NavigationSystem::QueueMeshUpdate() is deprecated! INavigationUpdatesUpr::RequestMeshUpdate should be used instead");
	m_updatesUpr.RequestMeshUpdate(meshID, aabb);
	return 0;
}
	...
	for (; it != end; ++it)
	{
		...
		for (; itMesh != endMesh; ++itMesh)
		{
			if (itMesh->id && m_meshes.validate(itMesh->id))
			{
				...
				MNM::IslandConnections& islandConnections = m_islandConnectionsUpr.GetIslandConnections();
				NavigationMesh& mesh = m_meshes[meshID];
				mesh.navMesh.GetIslands().ComputeStaticIslandsAndConnections(mesh.navMesh, meshID, m_offMeshNavigationUpr, islandConnections);
			}
		}
	}
}
	...
	if (m_meshes.validate(meshID))
	{
		...
		if (mesh.navMesh.GetTriangle(startingTriangleID, startingTriangle))
		{
			if (mesh.navMesh.GetTriangle(endingTriangleID, endingTriangle))
			{
				...
				for (u16 l = 0; l < startingTriangle.linkCount; ++l)
				{
					...
					if (link.side == MNM::Tile::SLink::OffMesh)
					{
						...
						MNM::OffMeshNavigation& offMeshNavigation = m_offMeshNavigationUpr.GetOffMeshNavigationForMesh(meshID);
						...
						while (MNM::WayTriangleData nextTri = linksResult.GetNextTriangle())
						{
							...
							{
								const MNM::OffMeshLink* pLink = m_offMeshNavigationUpr.GetOffMeshLink(nextTri.offMeshLinkID);
								assert(pLink);
...
void NavigationSystem::RemoveAllIslandConnectionsForObject(const NavigationMeshID& meshID, u32k objectId)
{
	MNM::IslandConnections& islandConnections = m_islandConnectionsUpr.GetIslandConnections();
                                                               ...
                                                               const MNM::TriangleID endingTriangleID)
{
	...
	// from each other, but currently OffMeshNavigationUpr first unlinks triangles and only then unlinks islands.
  ...
  const MNM::OffMeshLinkID& linkID)
{
		...
		if (m_meshes.validate(meshID))
		{
			...
			if (mesh.navMesh.GetTriangle(startingTriangleID, startingTriangle))
			{
				if (mesh.navMesh.GetTriangle(endingTriangleID, endingTriangle))
				{
					...
					for (u16 l = 0; l < startingTriangle.linkCount && !bLinkIsFound; ++l)
					{
						const MNM::Tile::SLink& link = tile.GetLinks()[startingTriangle.firstLink + l];
						if (link.side == MNM::Tile::SLink::OffMesh)
						{
							const MNM::GlobalIslandID endingIslandID(meshID, endingTriangle.islandID);
							const MNM::OffMeshNavigation& offMeshNavigation = m_offMeshNavigationUpr.GetOffMeshNavigationForMesh(meshID);
							...
							while (MNM::WayTriangleData nextTri = linksResult.GetNextTriangle())
							{
								if (nextTri.triangleID == endingTriangleID)
								{
									...
									{
										if (const MNM::OffMeshLink* pLink = m_offMeshNavigationUpr.GetOffMeshLink(nextTri.offMeshLinkID))
										{
											bLinkIsFound = true;
											break;
										}
	...
	// TODO pavloi 2016.02.05: whole piece is suboptimal - this function is called from m_offMeshNavigationUpr already, where
	...
	if (m_meshes.validate(meshID))
	{
		...
		if (mesh.navMesh.GetTriangle(startingTriangleID, startingTriangle))
		{
			if (mesh.navMesh.GetTriangle(endingTriangleID, endingTriangle))
			{
				...
				const MNM::OffMeshLink* pLink = m_offMeshNavigationUpr.GetOffMeshLink(linkID);
				if (pLink)
				{
...
void NavigationSystem::ComputeAccessibility(const Vec3& debugLocation, NavigationAgentTypeID agentTypeId /*= NavigationAgentTypeID(0)*/)
{
	...
	if (meshId)
	{
		...
		const MNM::CNavMesh::SGridParams& paramsGrid = mesh.navMesh.GetGridParams();
		const MNM::OffMeshNavigation& offMeshNavigation = GetOffMeshNavigationUpr()->GetOffMeshNavigationForMesh(meshId);
		...
		u16k agentHeightUnits = GetAgentHeightInVoxelUnits(agentTypeId);
		...
		u16k minZOffsetMultiplier(2);
		u16k zOffsetMultiplier = min(minZOffsetMultiplier, (u16)agentTypeProperties.settings.agent.height);
...
void NavigationSystem::ClearAllAccessibility(u8 resetValue)
{
	AgentTypes::const_iterator it = m_agentTypes.begin();
	AgentTypes::const_iterator end = m_agentTypes.end();
...
bool NavigationSystem::AgentTypeSupportSmartObjectUserClass(NavigationAgentTypeID agentTypeID, tukk smartObjectUserClass) const
{
	if (agentTypeID && agentTypeID <= m_agentTypes.size())
	{
		const AgentType& agentType = m_agentTypes[agentTypeID - 1];
...
u16 NavigationSystem::GetAgentRadiusInVoxelUnits(NavigationAgentTypeID agentTypeID) const
{
	if (agentTypeID && agentTypeID <= m_agentTypes.size())
	{
...
u16 NavigationSystem::GetAgentHeightInVoxelUnits(NavigationAgentTypeID agentTypeID) const
{
	if (agentTypeID && agentTypeID <= m_agentTypes.size())
	{
...
void NavigationSystem::ClearAndNotify()
{
	...
	m_offMeshNavigationUpr.Enable();
	gAIEnv.pSmartObjectUpr->SoftReset();
}
...
bool NavigationSystem::ReloadConfig()
{
	...
	// 3) continuous nav mesh update can be taken away from editor's CAIUpr to navigation system.
	...
	if (XmlNodeRef childNode = rootNode->findChild("AreaTypes"))
	{
		...
		for (size_t at = 0; at < areaTypeCount; ++at)
		{
			...
			for (size_t childIdx = 0; childIdx < (size_t)areaTypeNode->getChildCount(); ++childIdx)
			{
				...
				{
					tukk szFlagName = agentTypeChildNode->getContent();
	...
	if (XmlNodeRef childNode = rootNode->findChild("AgentTypes"))
	{
		...
		for (size_t at = 0; at < agentTypeCount; ++at)
		{
			...
			for (size_t attr = 0; attr < (size_t)agentTypeNode->getNumAttributes(); ++attr)
			{
				...
				{
					float x, y, z;
					i32 c = sscanf(attrValue, "%g,%g,%g", &x, &y, &z);
...
void NavigationSystem::SetupTasks()
{
	...
	m_maxRunningTaskCount = (gEnv->pJobUpr->GetNumWorkerThreads() * 3 / 4) * gAIEnv.CVars.NavGenThreadJobs;
	m_results.resize(m_maxRunningTaskCount);

	for (u16 i = 0; i < m_results.size(); ++i)
...
bool NavigationSystem::RegisterArea(tukk shapeName, NavigationVolumeID& outVolumeId)
{
	return m_volumesUpr.RegisterArea(shapeName, outVolumeId);
}

void NavigationSystem::UnRegisterArea(tukk shapeName)
{
	m_volumesUpr.UnRegisterArea(shapeName);
}

NavigationVolumeID NavigationSystem::GetAreaId(tukk shapeName) const
{
	return m_volumesUpr.GetAreaID(shapeName);
}

void NavigationSystem::SetAreaId(tukk shapeName, NavigationVolumeID id)
{
	m_volumesUpr.SetAreaID(shapeName, id);
}

void NavigationSystem::UpdateAreaNameForId(const NavigationVolumeID id, tukk newShapeName)
{
	m_volumesUpr.UpdateNameForAreaID(id, newShapeName);
}

void NavigationSystem::RemoveLoadedMeshesWithoutRegisteredAreas()
{
	std::vector<NavigationVolumeID> volumesToRemove;
	m_volumesUpr.GetLoadedUnregisteredVolumes(volumesToRemove);
				...
				for (const AgentType::MeshInfo& meshInfo : agentType.meshes)
				{
					...
					if (mesh.boundary == volumeId)
					{
						...
						AILogComment("Removing NavMesh '%s' (meshId = %u, agent = %s) because it uses loaded unregistered navigation area id = %u.",
						             mesh.name.c_str(), (u32)meshID, agentType.name.c_str(), (u32)volumeId);
					}
				}
			}
		}
	...
	m_volumesUpr.ClearLoadedAreas();
}

bool NavigationSystem::RegisterEntityMarkups(const IEntity& owningEntity, tukk* shapeNamesArray, const size_t count, NavigationVolumeID* pOutIdsArray)
{
	return m_volumesUpr.RegisterEntityMarkups(*this, owningEntity, shapeNamesArray, count, pOutIdsArray);
}

void NavigationSystem::UnregisterEntityMarkups(const IEntity& owningEntity)
{
	m_volumesUpr.UnregisterEntityMarkups(*this, owningEntity);
}
...
bool NavigationSystem::IsPointReachableFromPosition(const NavigationAgentTypeID agentID, const IEntity* pEntityToTestOffGridLinks, const Vec3& startLocation, const Vec3& endLocation, const INavMeshQueryFilter* pFilter) const
{
	...
	return m_islandConnectionsUpr.AreIslandsConnected(pEntityToTestOffGridLinks, startingIslandID, endingIslandID, pFilter);
}
...
MNM::TriangleID NavigationSystem::GetTriangleIDWhereLocationIsAtForMesh(const NavigationAgentTypeID agentID, const Vec3& location, const INavMeshQueryFilter* pFilter)
{
	...
	if (meshId)
	{
		...
		const Vec3& voxelSize = mesh.navMesh.GetGridParams().voxelSize;
		u16k agentHeightUnits = GetAgentHeightInVoxelUnits(agentID);
...
size_t NavigationSystem::GetTriangleInfo(const NavigationMeshID meshID, const AABB& aabb, Vec3* centerLocations, u32* islandids, size_t max_count, const INavMeshQueryFilter* pFilter, float minIslandArea) const
{
	if (m_meshes.validate(meshID))
	{
		const MNM::vector3_t min(MNM::real_t(aabb.min.x), MNM::real_t(aabb.min.y), MNM::real_t(aabb.min.z));
...
// Helper function to read various navigationId types from file without creating intermediate u32.
template<typename TId>
static void ReadNavigationIdType(CDrxFile& file, TId& outId)
{
	static_assert(sizeof(TId) == sizeof(u32), "Navigation ID underlying type have changed");
	u32 id;
	file.ReadType(&id);
	outId = TId(id);
}
...
template<typename TId>
static void WriteNavigationIdType(CDrxFile& file, const TId& id)
{
	static_assert(sizeof(TId) == sizeof(u32), "Navigation ID underlying type have changed");
	u32k uid = id;
	file.WriteType<u32>(&uid);
}

bool NavigationSystem::ReadFromFile(tukk fileName, bool bAfterExporting)
{
	...
	if (false != file.Open(fileName, "rb"))
	{
		...
		u16 nFileVersion = eBAINavigationFileVersion::CURRENT;
		...
		if (fileVersionCompatible)
		{
			// Loading boundary volumes, their ID's and names
			{
				...
				u32 usedVolumesCount;
				file.ReadType(&usedVolumesCount);

				for (u32 idx = 0; idx < usedVolumesCount; ++idx)
				{
					// Read volume data
					NavigationVolumeID volumeId;
					float volumeHeight;
					u32 verticesCount;
					u32 volumeAreaNameSize;
					...
					for (u32 vtxIdx = 0; vtxIdx < verticesCount; ++vtxIdx)
					{
					...
					if (m_volumes.validate(volumeId))
					{
						AIWarning("NavigationSystem::ReadFromFile: Navigation Volume with volumeId=%u (name '%s') is already registered", (u32)volumeId, volumeAreaName.c_str());
						continue;
					}

					const NavigationVolumeID createdVolumeId = CreateVolume(&volumeVerticesBuffer.front(), verticesCount, volumeHeight, volumeId);
					DRX_ASSERT(volumeId == createdVolumeId);

					m_volumesUpr.RegisterAreaFromLoadedData(volumeAreaName.c_str(), volumeId);
				}
			}

			{
				...
				for (u32 idx = 0; idx < markupsCount; ++idx)
				{
					...
					DRX_ASSERT_MESSAGE(markupId != NavigationVolumeID(), "Markup volume with invalid id loaded!");
					DRX_ASSERT_MESSAGE(!m_markupVolumes.validate(markupId), "Markup volume with the same id was already loaded!");
			...
			for (u32 i = 0; i < agentsCount; ++i)
			{
...

				{
					...
					for (u32 mIdx = 0; mIdx < markupsCount; ++mIdx)
					{
						NavigationVolumeID markupId;
						ReadNavigationIdType(file, markupId);
				...
				for (u32 meshCounter = 0; meshCounter < meshesCount; ++meshCounter)
				{
					// Reading mesh id
#ifdef SW_NAVMESH_USE_GUID
					...
					meshNameLength = std::min(meshNameLength, (u32)MAX_NAME_LENGTH - 1);
					...
					{
						if (m_volumesUpr.GetLoadedAreaID(meshName) != boundaryID)
						{
							AIWarning("The NavMesh '%s' (agent = '%s', meshId = %u, boundaryVolumeId = %u) and the loaded corresponding Navigation Area have different IDs. Data might be corrupted.",
							          meshName, agentName, meshIDuint32, (u32)boundaryID);
						}

						const NavigationVolumeID existingAreaId = m_volumesUpr.GetAreaID(meshName);
						if (existingAreaId != boundaryID)
						{
							if (!m_volumesUpr.IsAreaPresent(meshName))
							{
								if (!m_volumesUpr.IsLoadedAreaPresent(meshName))
								{
					...
					u32 exclusionShapesCount = 0;
					file.ReadType(&exclusionShapesCount);
					AgentType::ExclusionVolumes exclusions;
#ifdef SW_NAVMESH_USE_GUID
					for (u32 exclusionsCounter = 0; exclusionsCounter < exclusionShapesCount; ++exclusionsCounter)
					{
			...
			DRX_ASSERT(m_markupsData.capacity() == m_markupVolumes.capacity());
			DRX_ASSERT(dataCount <= m_markupsData.capacity());
			...
			for (u32 idx = 0; idx < dataCount; ++idx)
			{
				...
				u32 meshTrianglesCount;
				file.ReadType(&meshTrianglesCount);

				for (u32 meshTriIdx = 0; meshTriIdx < meshTrianglesCount; ++meshTriIdx)
				{
					...
					for (u32 triIdx = 0; triIdx < trianglesCount; ++triIdx)
					{
						u32 x, y, z;
						u16 triIndex;
						file.ReadType(&x);
			...
			m_volumesUpr.LoadData(file, nFileVersion);
	...
	m_volumesUpr.ValidateAndSanitizeLoadedAreas(*this);
	...
	m_offMeshNavigationUpr.OnNavigationLoadedComplete();
...
u16 FilterOffMeshLinksForTile(const MNM::STile& tile, MNM::Tile::STriangle* pTrianglesBuffer, u16 trianglesBufferSize, MNM::Tile::SLink* pLinksBuffer, u16 linksBufferSize)
{
	...
	if (newLinkCount > 0)
	{
		for (u16 i = 0; i < trianglesCount; ++i)
		{
...
void NavigationSystem::GatherNavigationVolumesToSave(std::vector<NavigationVolumeID>& usedVolumes) const
{
	...
	for (const AgentType& agentType : m_agentTypes)
	{
		for (const AgentType::MeshInfo& meshInfo : agentType.meshes)
		{
			DRX_ASSERT(m_meshes.validate(meshInfo.id));
			const NavigationMesh& mesh = m_meshes.get(meshInfo.id);

			DRX_ASSERT(mesh.boundary);
			DRX_ASSERT(m_volumes.validate(mesh.boundary));
		...
		{
			DRX_ASSERT(volumeId);
			DRX_ASSERT(m_volumes.validate(volumeId));
			if (volumeId && m_volumes.validate(volumeId))
			{
...
bool NavigationSystem::SaveToFile(tukk fileName) const PREFAST_SUPPRESS_WARNING(6262)
{
#if NAVIGATION_SYSTEM_PC_ONLY
	...
	{
		i32k maxTriangles = 1024;
		i32k maxLinks = maxTriangles * 6;
	...
	#ifdef SW_NAVMESH_USE_GUID
		u32 useGUID = BAI_NAVIGATION_GUID_FLAG;
		...
		// Saving boundary volumes, their ID's and names
		{
			...
			u32k usedVolumesCount = static_cast<u32>(usedVolumes.size());
			...
			for (u32 idx = 0; idx < usedVolumesCount; ++idx)
			{
				...
				u32k verticesCount = volume.GetBoundaryVertices().size();

				volumeAreaName.clear();
				m_volumesUpr.GetAreaName(volumeId, *&volumeAreaName);
				u32k volumeAreaNameSize = static_cast<u32>(volumeAreaName.size());
		...
		// Saving markup boundary areas and data
		{
			...
			u32k markupsCount = static_cast<u32>(m_markupVolumes.size());
			u32k markupsCapacity = static_cast<u32>(m_markupVolumes.capacity());
			file.WriteType(&markupsCount);
			file.WriteType(&markupsCapacity);

			for (u32 idx = 0; idx < m_markupVolumes.capacity(); ++idx)
			{
				...
				u32k verticesCount = markupVolume.GetBoundaryVertices().size();
		...
		u32 agentsCount = static_cast<u32>(GetAgentTypeCount());
		...
		for (; typeIt != typeEnd; ++typeIt)
		{
			...
			u32 nameLength = agentType.name.length();
			nameLength = std::min(nameLength, (u32)MAX_NAME_LENGTH - 1);
			...
			AgentType::Meshes::const_iterator mend = agentType.meshes.end();
			{
				...
				u32 markupsCount = agentType.markups.size();
				file.WriteType(&markupsCount);
			...
			u32 meshesCount = agentType.meshes.size();
			file.Write(&meshesCount, sizeof(meshesCount));

			for (; mit != mend; ++mit)
			{
				u32k meshIDuint32 = mit->id;
				...
				if (!m_volumes.validate(mesh.boundary) || m_volumesUpr.GetAreaID(mesh.name.c_str()) != mesh.boundary)
				{
					DrxMessageBox("Sandbox detected a possible data corruption during the save of the navigation mesh."
	...
	#ifdef SW_NAVMESH_USE_GUID
				u32 exclusionShapesCount = mesh.exclusionsGUID.size();
				file.Write(&exclusionShapesCount, sizeof(exclusionShapesCount));
				for (u32 exclusionCounter = 0; exclusionCounter < exclusionShapesCount; ++exclusionCounter)
				{
	...
	#else
				{
					...
					u32 exclusionShapesCount = validExlusionVolumes.size();
					file.Write(&exclusionShapesCount, sizeof(exclusionShapesCount));
					for (u32 exclusionCounter = 0; exclusionCounter < exclusionShapesCount; ++exclusionCounter)
					{
...

				{
					...
					u32 markupsCount = mesh.markups.size();
					file.WriteType(&markupsCount);
				...
				// Saving tiles count
				u32 tileCount = navMesh.GetTileCount();
				...
				u16 xmin = (u16)(floor_tpl(bmin.x / (float)paramsGrid.tileSize.x));
				u16 xmax = (u16)(floor_tpl(bmax.x / (float)paramsGrid.tileSize.x));

				u16 ymin = (u16)(floor_tpl(bmin.y / (float)paramsGrid.tileSize.y));
				u16 ymax = (u16)(floor_tpl(bmax.y / (float)paramsGrid.tileSize.y));

				u16 zmin = (u16)(floor_tpl(bmin.z / (float)paramsGrid.tileSize.z));
				u16 zmax = (u16)(floor_tpl(bmax.z / (float)paramsGrid.tileSize.z));

				for (u16 x = xmin; x < xmax + 1; ++x)
				{
					for (u16 y = ymin; y < ymax + 1; ++y)
					{
						for (u16 z = zmin; z < zmax + 1; ++z)
						{
							...
							u32k tileHashValue = tile.GetHashValue();
							...
							u16k saveLinkCount = FilterOffMeshLinksForTile(tile, triangleBuffer, maxTriangles, linkBuffer, maxLinks);
							...
							u16k verticesCount = tile.GetVerticesCount();
							...
							u16k nodesCount = tile.GetBVNodesCount();
							...
							static_assert(sizeof(u16) == sizeof(tile.GetLinksCount()), "Invalid type size!");
							static_assert(sizeof(u16) == sizeof(tile.GetTrianglesCount()), "Invalid type size!");
							static_assert(sizeof(u16) == sizeof(tile.GetVerticesCount()), "Invalid type size!");
							static_assert(sizeof(u16) == sizeof(tile.GetBVNodesCount()), "Invalid type size!");
							static_assert(sizeof(u16) == sizeof(trianglesCount), "Invalid type size!");
							static_assert(sizeof(u16) == sizeof(verticesCount), "Invalid type size!");
							static_assert(sizeof(u16) == sizeof(saveLinkCount), "Invalid type size!");
							static_assert(sizeof(u16) == sizeof(nodesCount), "Invalid type size!");
							...
							static_assert(sizeof(u32) == sizeof(tile.GetHashValue()), "Invalid type size!");
						}
					}
		...
		u32k dataCount = static_cast<u32>(m_markupsData.size());
		file.WriteType(&dataCount);

		for (u32 idx = 0; idx < m_markupsData.capacity(); ++idx)
		{
			...
			u32k meshTrianglesCount = markupData.meshTriangles.size();
			...
			for (const MNM::SMarkupVolumeData::MeshTriangles& meshTriangles : markupData.meshTriangles)
			{
				...
				u32k trianglesCount = meshTriangles.triangleIds.size();
				...
				for (MNM::TriangleID triId : meshTriangles.triangleIds)
				{
					...
					u16k triIndex = MNM::ComputeTriangleIndex(triId);

					const MNM::vector3_t tileCoords = navMesh.GetTileContainerCoordinates(tileId);
					u32k x = tileCoords.x.as_uint();
					u32k y = tileCoords.y.as_uint();
					u32k z = tileCoords.z.as_uint();
		...
		m_volumesUpr.SaveData(file, nFileVersion);
...
void NavigationSystem::GetMemoryStatistics(IDrxSizer* pSizer)
{
	...
	for (u16 i = 0; i < m_meshes.capacity(); ++i)
	{
		if (!m_meshes.index_free(i))
		{
			...
			const OffMeshNavigationUpr::ProfileMemoryStats linksMemStats = m_offMeshNavigationUpr.GetMemoryStats(pSizer, meshID);
...
TileGeneratorExtensionID NavigationSystem::RegisterTileGeneratorExtension(MNM::TileGenerator::IExtension& extension)
{
	...
	{
		DRX_ASSERT_MESSAGE(newId != TileGeneratorExtensionID(), "TileGeneratorExtensionID counter is exausted");
...
void NavigationSystemDebugDraw::DebugDraw(NavigationSystem& navigationSystem)
{
	...
	if (validDebugAgent)
	{
		...
		i32k frameID = gEnv->pRenderer->GetFrameID();
		...
		{
			navigationSystem.m_offMeshNavigationUpr.UpdateEditorDebugHelpers();
		}
	}
...
MNM::TileID NavigationSystemDebugDraw::DebugDrawTileGeneration(NavigationSystem& navigationSystem, const DebugDrawSettings& settings)
{
	...
	if (forceGeneration)
	{
		...
		navigationSystem.SetupGenerator(settings.meshID, paramsGrid, static_cast<u16>(selectedX), static_cast<u16>(selectedY), static_cast<u16>(selectedZ), params, def, false);
...
void NavigationSystemDebugDraw::DebugDrawPathFinder(NavigationSystem& navigationSystem, const DebugDrawSettings& settings)
{
	...
	const OffMeshNavigationUpr* offMeshNavigationUpr = navigationSystem.GetOffMeshNavigationUpr();
	assert(offMeshNavigationUpr);
	const MNM::OffMeshNavigation& offMeshNavigation = offMeshNavigationUpr->GetOffMeshNavigationForMesh(meshID);
	...
	if (triStart && triEnd)
	{
		...
		MNM::CNavMesh::WayQueryRequest inputParams(
			...
			offMeshNavigation, *offMeshNavigationUpr, 
		...
		if (bPathFound)
		{
			...
			if (CMNMPathfinder::ConstructPathFromFoundWay(result, navMesh, offMeshNavigationUpr, fixedPointStartLoc.GetVec3(), fixedPointEndLoc.GetVec3(), *&outputPath))
			{
...
static bool FindObjectToTestIslandConnectivity(tukk szName, Vec3& outPos, IEntity** ppOutEntityToTestOffGridLinks)
{
...
void NavigationSystemDebugDraw::DebugDrawIslandConnection(NavigationSystem& navigationSystem, const DebugDrawSettings& settings)
{
	...
	tukk szStartName = "MNMIslandStart";
	tukk szStartNameAnyLink = "MNMIslandStartAnyLink";
	tukk szEndName = "MNMIslandEnd";
...
{
	DefaultTriangleColorSelector(NavigationSystem& navigationSystem, tukk szFlagToDraw) 
...
void NavigationSystemDebugDraw::DebugDrawNavigationSystemState(NavigationSystem& navigationSystem)
{
	...
	if (gAIEnv.CVars.DebugDrawNavigation)
	{
		...
		static_cast<CMNMUpdatesUpr*>(navigationSystem.GetUpdateUpr())->DebugDraw();
	}

	// Draw annotations legend
	auto DrawLabel = [&dc](float posX, float posY, const Vec2& size, const ColorF& color, float fontSize, tukk szLabel)
	{
...
void NavigationSystemDebugDraw::DebugDrawMemoryStats(NavigationSystem& navigationSystem)
{
	if (gAIEnv.CVars.MNMProfileMemory)
	{
		...
		for (u16 i = 0; i < navigationSystem.m_meshes.capacity(); ++i)
		{
			if (!navigationSystem.m_meshes.index_free(i))
			{
				...
				const MNM::OffMeshNavigation& offMeshNavigation = navigationSystem.GetOffMeshNavigationUpr()->GetOffMeshNavigationForMesh(meshID);
				const AgentType& agentType = navigationSystem.m_agentTypes[mesh.agentTypeID - 1];

				const NavigationMesh::ProfileMemoryStats meshMemStats = mesh.GetMemoryStats(pSizer);
				const MNM::OffMeshNavigation::ProfileMemoryStats offMeshMemStats = offMeshNavigation.GetMemoryStats(pSizer);
				const OffMeshNavigationUpr::ProfileMemoryStats linkMemStats = navigationSystem.GetOffMeshNavigationUpr()->GetMemoryStats(pSizer, meshID);
...
const INavMeshQueryFilter* NavigationSystemDebugDraw::GetDebugQueryFilter(tukk szName) const
{
...
void NavigationSystemDebugDraw::NavigationSystemWorkingProgress::Draw()
{
	...
	const ColorB backGroundColor(0, 255, 0, CLAMP((i32)(0.35f * m_timeUpdating * 255.0f), 0, 255));
	const ColorB progressColor(0, 255, 0, CLAMP((i32)(0.8f * m_timeUpdating * 255.0f), 0, 255));
...
NavigationMesh::ProfileMemoryStats NavigationMesh::GetMemoryStats(IDrxSizer* pSizer) const
{
...
void NavigationSystemBackgroundUpdate::Thread::ThreadEntry()
{
	while (!m_requestedStop)
	{
		if (m_navigationSystem.GetState() == INavigationSystem::Working)
		{
			...
			u32k sleepTime = max(10u, min(0u, 33u - (u32)lastUpdateTime.GetMilliSeconds()));
...
bool NavigationSystemBackgroundUpdate::Start()
{
	if (m_pBackgroundThread == NULL)
	{
		...
		if (!gEnv->pThreadUpr->SpawnThread(m_pBackgroundThread, "NavigationSystemBackgroundUpdate"))
		{
			DRX_ASSERT_MESSAGE(false, "Error spawning \"NavigationSystemBackgroundUpdate\" thread.");
...
bool NavigationSystemBackgroundUpdate::Stop()
{
	if (m_pBackgroundThread != NULL)
	{
		...
		gEnv->pThreadUpr->JoinThread(m_pBackgroundThread, eJM_Join);
...
void NavigationSystemBackgroundUpdate::OnSystemEvent(ESystemEvent event, UINT_PTR wparam, UINT_PTR lparam)
{
	...
	else if (event == ESYSTEM_EVENT_CHANGE_FOCUS)
	{
		...
		if (startBackGroundUpdate)
		{
			if (Start())
			{
				DrxLog("NavMesh generation background thread started");
			}
		}
		else
		{
