...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   AILightUpr.cpp
   $Id$
   Описание: Keeps track of the logical light level in the game world and
...
// Reset
//===================================================================
void CAILightUpr::Reset()
{
	CCCPOINT(CAILightUpr_Reset);
...
// DynOmniLightEvent
//===================================================================
void CAILightUpr::DynOmniLightEvent(const Vec3& pos, float radius, EAILightEventType type, CAIActor* pShooter, float time)
{
	CCCPOINT(CAILightUpr_DynOmniLightEvent);

	// (MATT) Are NULL shooters allowed? {2009/02/15}
	if (!pShooter)
	{
		assert(false);
		gEnv->pScriptSystem->RaiseError("[Not scripts!] CAILightUpr_DynOmniLightEvent - NULL pShooter");
	}
...
// DynSpotLightEvent
//===================================================================
void CAILightUpr::DynSpotLightEvent(const Vec3& pos, const Vec3& dir, float radius, float fov, EAILightEventType type, CAIActor* pShooter, float time)
{
	if (fov <= 0.0f)
		return;

	CCCPOINT(CAILightUpr_DynSpotLightEvent);
	...
	for (unsigned i = 0, ni = m_dynLights.size(); i < ni; ++i)
	{
		...
		{
			CCCPOINT(CAILightUpr_DynSpotLightEvent_A);
	...
	if (type == AILE_LASER || type == AILE_FLASH_LIGHT)
	{
		...
		{
			CCCPOINT(CAILightUpr_DynSpotLightEvent_B);
			...
			static i32 nameCounter = 0;
			char szName[256];
			drx_sprintf(szName, "Light of %s %d", pShooter->GetName(), nameCounter++);
			gAIEnv.pAIObjectUpr->CreateDummyObject(refAttrib, szName);
	...
	{
		CCCPOINT(CAILightUpr_DynSpotLightEvent_C);
	}
...
// Update
//===================================================================
void CAILightUpr::Update(bool forceUpdate)
{
	CCCPOINT(CAILightUpr_Update);

	CTimeValue t = GetAISystem()->GetFrameStartTime();
	int64 dt = (t - m_lastUpdateTime).GetMilliSecondsAsInt64();
	i32 nDynLightsAtStart = m_dynLights.size();     // Just for debugging - the vector can shrink
	...
	//	if (dt > 0.25f || forceUpdate)
	{
		...
		for (unsigned i = 0; i < m_dynLights.size(); )
		{
			...
			{
				assert(false);
				CCCPOINT(CAILightUpr_Update_NilShooter);
			}

			bool bValidShooter = light.refShooter.IsValid();
			if (!bValidShooter)
			{ CCCPOINT(CAILightUpr_Update_NonValidShooter); }
...
// DebugDraw
//===================================================================
void CAILightUpr::DebugDraw()
{
	i32 mode = gAIEnv.CVars.DebugDrawLightLevel;
	if (mode == 0)
		return;
	...
	dc->Draw2dLabel(20.f, height - 100, 2.0f, white, false, "Ambient: %s", g_szLightLevels[(i32)m_ambientLight]);
	...
	if (pPlayer)
	{
		...
		if (pPlayer->IsAffectedByLight())
		{
			...
			case AILL_SUPERDARK:
				...
				break;
			default:
				DRX_ASSERT_MESSAGE(false, "CAILightUpr::DebugDraw Unhandled light level");
				break;
			}
		}
		else
	...
	if (mode == 2)
	{
		...
		if (pPlayer)
		{
			...
			dc->Draw2dLabel(20.f, height - 35.f, 1.5f, white, false, "Player: %s", g_szLightLevels[(i32)playerLightLevel]);
	...
	for (unsigned i = 0, ni = m_lights.size(); i < ni; ++i)
	{
		...
		ColorB c = lightCols[(i32)light.level];
	...
	for (unsigned i = 0, ni = m_lights.size(); i < ni; ++i)
	{
		...
		dc->Draw3dLabel(light.pos, 1.1f, "%s", g_szLightLevels[(i32)light.level]);
	}
	...
	for (unsigned i = 0, ni = m_dynLights.size(); i < ni; ++i)
	{
		...
		dc->Draw3dLabel(light.pos, 1.1f, "DYN %s\n%s", g_szLightLevels[(i32)light.level], g_szLightType[(i32)light.type]);
	...
	for (ShapeMap::const_iterator di = GetAISystem()->GetGenericShapes().begin(), dend = GetAISystem()->GetGenericShapes().end(); di != dend; ++di)
	{
		...
		if (sa.height < 0.0001f)
			continue;
		DebugDrawArea(sa.shape, sa.aabb.min.z, sa.aabb.min.z + sa.height, lightCols[(i32)sa.lightLevel]);
		Vec3 pos(sa.shape.front());
		pos.z = sa.aabb.min.z + sa.height / 2;
		dc1->Draw3dLabel(pos, 1.1f, "%s\n%s", di->first.c_str(), g_szLightLevels[(i32)sa.lightLevel]);
	}
}
...
// DebugDrawArea
//===================================================================
void CAILightUpr::DebugDrawArea(const ListPositions& poly, float zmin, float zmax, ColorB color)
{
	CDebugDrawContext dc;
...
// GetLightLevelAt
//===================================================================
EAILightLevel CAILightUpr::GetLightLevelAt(const Vec3& pos, const CAIActor* pAgent, bool* outUsingCombatLight)
{
	CCCPOINT(CAILightUpr_GetLightLevelAt);
...
// UpdateLights
//===================================================================
void CAILightUpr::UpdateLights()
{
	CCCPOINT(CAILightUpr_UpdateLights);
	...
	// Get light anchors
	for (u32 i = 0, n = sizeof types / sizeof types[0]; i < n; ++i)
	{
		const short type = types[i];
		for (AIObjectOwners::iterator ai = gAIEnv.pAIObjectUpr->m_Objects.find(type), end = gAIEnv.pAIObjectUpr->m_Objects.end(); ai != end && ai->first == type; ++ai)
		{
...
// UpdateTimeOfDay
//===================================================================
void CAILightUpr::UpdateTimeOfDay()
{
...
// save/load
//===================================================================
void CAILightUpr::SAIDynLightSource::Serialize(TSerialize ser)
{
	ser.Value("pos", pos);
	ser.Value("dir", dir);
...
// save/load
//===================================================================
void CAILightUpr::Serialize(TSerialize ser)
{
	ser.BeginGroup("AILightUpr");
	i32 dynLightCount(m_dynLights.size());
	...
	{
		drx_sprintf(AILightGrpName, "AIDynLightSource-%u", i);
