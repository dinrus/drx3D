...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   AIDbgRecorder.cpp
   Описание: Simple text AI debugging event recorder
...
//
//----------------------------------------------------------------------------------------------
void CAIDbgRecorder::Record(const IAIObject* pTarget, IAIRecordable::e_AIDbgEvent event, tukk pString) const
{
	if (event == IAIRecordable::E_RESET)
	{
		LogString("\n\n--------------------------------------------------------------------------------------------\n");
	...
	i32 frame = GetAISystem()->GetAITickCount();
...
void CAIDbgRecorder::InitFile() const
{
	...
	fputs("DinrusXAI text debugging log\n", pFile);
...
//----------------------------------------------------------------------------------------------
void CAIDbgRecorder::LogString(tukk pString) const
{
...
//
//----------------------------------------------------------------------------------------------
void CAIDbgRecorder::LogStringSecondary(tukk pString) const
{
	if (m_sFile.empty())
		InitFileSecondary();
