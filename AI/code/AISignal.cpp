...
void AISignalExtraData::SetObjectName(tukk objectName)
{
	if (sObjectName)
	{
		delete[] sObjectName;
...
void AISignalExtraData::Serialize(TSerialize ser)
{
	...
	// (MATT) This change in #149616 will probably break save compatibility for Drxsis - however, we may find a way to do without these strings anyway.
...
void AISIGNAL::Serialize(TSerialize ser)
{
	...
	if (ser.IsReading())
	{
		drx_strcpy(strText, textString.c_str());
