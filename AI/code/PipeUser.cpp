...
#include <drx3D/AI/MovementBlock_InstallPipeUserInCover.h>
#include <drx3D/AI/MovementBlock_SetupPipeUserCoverInformation.h>
#include <drx3D/AI/MovementBlock_UninstallPipeUserFromCover.h>
...
void CPipeUser::Event(u16 eType, SAIEVENT* pAIEvent)
{
	...
	case AIEVENT_CLEAR:
		{
			...
			ClearActiveGoals();
			m_bLooseAttention = false;
			gAIEnv.pFormationUpr->FreeFormationPoint(GetWeakRef(this));
...
void CPipeUser::CreateRefPoint()
{
	...
	gAIEnv.pAIObjectUpr->CreateDummyObject(m_refRefPoint, string(GetName()) + "_RefPoint", STP_REFPOINT);
...
void CPipeUser::CreateLookAtTarget()
{
	...
	gAIEnv.pAIObjectUpr->CreateDummyObject(m_refLookAtTarget, string(GetName()) + "_LookAtTarget", STP_LOOKAT);
...
void CPipeUser::SetName(tukk pName)
{
	...
	if (pRefPoint)
	{
		drx_sprintf(name, "%s_RefPoint", pName);
	...
	if (pLookAtTarget)
	{
		drx_sprintf(name, "%s_LookAtTarget", pName);
...
{
	i32 branchType = Goal.params.nValue & (~NOT);
	...
	case IF_RANDOM:
		{
			if (drx_random(0.0f, 1.0f) <= Goal.params.fValue)
				return true;
		}
		break;
	case IF_NO_PATH:
	...
	case IF_SEES_TARGET:
		{
			if (Goal.params.fValueAux >= 0.0f)
			{
				if (pAttentionTarget)
				{
					...
					if (GetProxy() && GetProxy()->QueryBodyInfo(SAIBodyInfoQuery((EStance)(i32)Goal.params.fValueAux, 0.0f, 0.0f, false), bi))
					{
...
void CPipeUser::Update(EUpdateType type)
{
	...
	if (m_eNavSOMethod == nSOmSignalAnimation || m_eNavSOMethod == nSOmActionAnimation)
	{
		...
		if (m_State.curActorTargetPhase == eATP_StartedAndFinished || m_State.curActorTargetPhase == eATP_Finished)
		{
			...
			gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(GetEntity(), m_currentNavSOStates.sAnimationDoneUserStates);
			if (pObjectEntity)
				gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(pObjectEntity, m_currentNavSOStates.sAnimationDoneObjectStates);
	...
	if (!CastToCPuppet())
	{
		...
		else
		{
			...
			m_lightLevel = GetAISystem()->GetLightUpr()->GetLightLevelAt(GetPos(), this, &m_usingCombatLight);
		}
...
void CPipeUser::SetCoverState(const ICoverUser::StateFlags& state)
{
	...
	{
		DRX_ASSERT(currentState.Check(ICoverUser::EStateFlags::MovingToCover));
	...
	{
		DRX_ASSERT(!currentState.Check(ICoverUser::EStateFlags::InCover));
...
{
	DRX_ASSERT(m_pCoverUser == pCoverUser);
	if (!m_pCoverUser)
		return;
...
inline bool HasPointInRangeSq(const Vec3* points, u32 pointCount, const Vec3& pos, float rangeSq)
{
  ...
  void CPipeUser::FillCoverEyes(DynArray<Vec3>& eyesContainer)
{
	...
	u32 targetCount = pPuppet->GetBestTargets(targets, MaxTargets);
...
bool CPipeUser::ProcessRandomGoal(QGoal& Goal, bool& blocking)
{
	...
	blocking = true;
	if (Goal.params.fValue > drx_random(0, 99)) // 0 - never jump // 100 - always jump
...
void CPipeUser::SetNavSOFailureStates()
{
	if (m_eNavSOMethod != nSOmNone)
	{
		if (!m_currentNavSOStates.IsEmpty())
		{
			gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(GetEntity(), m_currentNavSOStates.sAnimationFailUserStates);
			IEntity* pObjectEntity = gEnv->pEntitySystem->GetEntity(m_currentNavSOStates.objectEntId);
			if (pObjectEntity)
				gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(pObjectEntity, m_currentNavSOStates.sAnimationFailObjectStates);
			m_currentNavSOStates.Clear();
		}

		if (!m_pendingNavSOStates.IsEmpty())
		{
			gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(GetEntity(), m_pendingNavSOStates.sAnimationFailUserStates);
			IEntity* pObjectEntity = gEnv->pEntitySystem->GetEntity(m_pendingNavSOStates.objectEntId);
			if (pObjectEntity)
				gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(pObjectEntity, m_pendingNavSOStates.sAnimationFailObjectStates);
			m_pendingNavSOStates.Clear();
		}
	}
...
void CPipeUser::ClearPath(tukk dbgString)
{
	SetNavSOFailureStates();
	...
	if (!m_vActiveGoals.empty())
	{
		for (size_t i = 0; i < m_vActiveGoals.size(); i++)
		{
			...
			/*
			   ...
			   i32 val = gAIEnv.CVars.ProfileGoals;
			...
			/*if (val)
			   ...
			   tukk goalName = Goal.op == eGO_LAST ? Goal.sPipeName.c_str() : m_pCurrentGoalPipe->GetGoalOpName(Goal.op);
...
// GetGoalPipe
//====================================================================
CGoalPipe* CPipeUser::GetGoalPipe(tukk name)
{
	CGoalPipe* pPipe = (CGoalPipe*) gAIEnv.pPipeUpr->OpenGoalPipe(name);
...
// IsUsingPipe
//====================================================================
bool CPipeUser::IsUsingPipe(tukk name) const
{
...
bool CPipeUser::IsUsingPipe(i32 goalPipeId) const
{
	const CGoalPipe* pExecutingPipe = m_pCurrentGoalPipe;
...
// RemoveActiveGoal
//====================================================================
void CPipeUser::RemoveActiveGoal(i32 nOrder)
{
	if (m_vActiveGoals.empty())
		return;
	i32 size = (i32)m_vActiveGoals.size();
...
bool CPipeUser::AbortActionPipe(i32 goalPipeId)
{
...
bool CPipeUser::SelectPipe(i32 mode, tukk name, CWeakRef<CAIObject> refArgument, i32 goalPipeId, bool resetAlways, const GoalParams* node)
{
	if (m_pipeExecuting)
	{
	...
	if (IEntity* pEntity = GetEntity())
		gAIEnv.pAIActionUpr->AbortAIAction(pEntity);
	...
	CGoalPipe* pPipe = gAIEnv.pPipeUpr->IsGoalPipe(name);
	if (pPipe)
	{
	...
	else
	{
		...
		pPipe = gAIEnv.pPipeUpr->IsGoalPipe("_first_");
		AIAssert(pPipe);
...
bool CPipeUser::RemoveSubPipe(i32 goalPipeId, bool keepInserted)
{
	if (!goalPipeId)
		return false;
...
bool CPipeUser::CancelSubPipe(i32 goalPipeId)
{
	if (!m_pCurrentGoalPipe)
	{
		if (goalPipeId != 0)
...
IGoalPipe* CPipeUser::InsertSubPipe(i32 mode, tukk name, CWeakRef<CAIObject> refArgument, i32 goalPipeId, const GoalParams* node)
{
	if (!m_pCurrentGoalPipe)
		return NULL;
	...
	if (goalPipeId != 0)
	{
		VectorSet<i32>::iterator itFind = m_notAllowedSubpipes.find(goalPipeId);
...
i32 CPipeUser::GetGoalPipeId() const
{
...
void CPipeUser::ClearMovementContext(u32 movementContext)
{
	m_movementContext &= ~movementContext;
}

void CPipeUser::SetMovementContext(u32 movementContext)
{
	m_movementContext |= movementContext;
}
...
CAIObject* CPipeUser::GetSpecialAIObject(tukk objName, float range)
{
	...
	{
		pObject = gAIEnv.pAIObjectUpr->GetAIObjectByName(objName);
	}
	return pObject;
}
...
// GetTargetScore
//====================================================================
static i32 GetTargetScore(CAIObject* pAI, CAIObject* pTarget)
{
...
Vec3 CPipeUser::GetProbableTargetPosition()
{
	...
	else
	{
		...
		i32 nearestAnyScore = 0;

		i32 combatClass = m_Parameters.m_CombatClass;

		i32 groupId = GetGroupId();
		...
		for (; gi != gend; ++gi)
		{
			...
			if (pPuppet)
			{
				...
				if (!foundLive && !pAttentionTarget && pPuppet->GetAttentionTarget())
				{
					...
					i32 score = GetTargetScore(this, pPuppet);
...
CAIObject* CPipeUser::GetOrCreateSpecialAIObject(ESpecialAIObjects type)
{
	...
	default:
		{
			char buf[64];
			drx_sprintf(buf, "_*Special%02d", (i32)type);
			name += buf;
			break;
		}
	}

	gAIEnv.pAIObjectUpr->CreateDummyObject(refSpecialObj, name, subType);
	return refSpecialObj.GetAIObject();
}
...
void CPipeUser::Serialize(TSerialize ser)
{
	...
	for (unsigned i = 0; i < COUNT_AISPECIAL; ++i)
	{
		drx_sprintf(specialName, "m_pSpecialObjects%d", i);
	...
	if (ser.IsReading())
	{
		...
		{
			pPipe = gAIEnv.pPipeUpr->IsGoalPipe(goalPipeName);
		}
	...
	{
		ResetCurrentPipe(true);
		DRX_ASSERT(m_pCurrentGoalPipe == NULL);
		m_notAllowedSubpipes.clear();
	...
	{
		i32 count = m_recentUnreachableHideObjects.size();
		...
		for (i32 i = 0; i < count; i++)
		{
			...
			drx_sprintf(name, "time_%d", i);
			ser.Value(name, time);
			drx_sprintf(name, "point_%d", i);
...
bool CPipeUser::SetCharacter(tukk character, tukk behaviour)
{
...
void CPipeUser::NotifyListeners(i32 goalPipeId, EGoalPipeEvent event)
{
	if (!goalPipeId)
		return;
	...
	// Notify listeners
	{
		...
		{
			std::pair<IGoalPipeListener*, tukk >& second = it->second;
...
void CPipeUser::RegisterGoalPipeListener(IGoalPipeListener* pListener, i32 goalPipeId, tukk debugClassName)
{
	...
	pListener->_vector_registered_pipes.push_back(std::pair<IPipeUser*, i32>(this, goalPipeId));
}

void CPipeUser::UnRegisterGoalPipeListener(IGoalPipeListener* pListener, i32 goalPipeId)
{
	...
	IGoalPipeListener::VectorRegisteredPipes::iterator itFind = std::find(
	  ...
	  std::pair<IPipeUser*, i32>(this, goalPipeId));
...
// SetPathToFollow
//====================================================================
void CPipeUser::SetPathToFollow(tukk pathName)
{
...
void CPipeUser::SetPointListToFollow(const std::vector<Vec3>& pointList, IAISystem::ENavigationType navType, bool bSpline)
{
	...
	i32 howmanyPoints = pointList.size();
	...
	for (i32 i = 0; i < howmanyPoints - 3; ++itX, ++itY, ++itZ, i++)
	{
...
void CPipeUser::SetFireMode(EFireMode mode)
{
	...
	m_spreadFireTime = drx_random(0.0f, 10.0f);
}
...
void CPipeUser::SetRefPointPos(const Vec3& pos)
{
	...
	if (GetAISystem()->IsRecording(this, IAIRecordable::E_REFPOINTPOS))
	{
		...
		drx_sprintf(buffer, "<%.0f %.0f %.0f>", pos.x, pos.y, pos.z);
...
//
//---------------------------------------------------------------------------------------------------------------------------
i32 CPipeUser::SetLooseAttentionTarget(CWeakRef<CAIObject> refObject, i32 id)
{
...
//
//---------------------------------------------------------------------------------------------------------------------------
i32 CPipeUser::CountGroupedActiveGoals()
{
	i32 count(0);
...
//
//----------------------------------------------------------------------------------------------
void CPipeUser::SetRefShapeName(tukk shapeName)
{
...
//
//----------------------------------------------------------------------------------------------
tukk CPipeUser::GetRefShapeName() const
{
...
void CPipeUser::HandlePathDecision(MNMPathRequestResult& result)
{
	...
	if (result.HasPathBeenFound())
	{
			...
			if (gAIEnv.CVars.DebugPathFinding)
			{
				if (m_State.actorTargetReq.animation.empty())
				{
					AILogAlways("CPipeUser::HandlePathDecision %s got new path during actor target request (phase: %d, vehicle: %s, seat: %d). Please investigate, this case should not happen.",
					            GetName(), (i32)m_State.curActorTargetPhase, m_State.actorTargetReq.vehicleName.c_str(), m_State.actorTargetReq.vehicleSeat);
				}
				else
				{
					AILogAlways("CPipeUser::HandlePathDecision %s got new path during actor target request (phase: %d, anim: %s). Please investigate, this case should not happen.",
					            GetName(), (i32)m_State.curActorTargetPhase, m_State.actorTargetReq.animation.c_str());
				}
			}
			SetNavSOFailureStates();
		}
		...
		i32 oldVersion = m_Path.GetVersion();
		m_Path = pNavPath;
...
void CPipeUser::HandleVisualStimulus(SAIEVENT* pAIEvent)
{
	...
	if (gAIEnv.pTargetTrackUpr->IsEnabled())
	{
		// Check if in range (using perception scale)
		if (eFOV_Outside != IsPointInFOV(pAIEvent->vPosition, fVisualPerceptionScale))
		{
			gAIEnv.pTargetTrackUpr->HandleStimulusFromAIEvent(GetAIObjectID(), pAIEvent, TargetTrackHelpers::eEST_Visual);
...
void CPipeUser::HandleSoundEvent(SAIEVENT* pAIEvent)
{
	...
	if (gAIEnv.pTargetTrackUpr->IsEnabled())
	{
		...
		{
			gAIEnv.pTargetTrackUpr->HandleStimulusFromAIEvent(GetAIObjectID(), pAIEvent, TargetTrackHelpers::eEST_Sound);
...
void CPipeUser::UpdateLookTarget(CAIObject* pTarget)
{
	...
	if (bLookAtTarget && pTarget)
	{
		...
		u16 nTargetType = pTarget->GetType();
		if ((distToTarget < 1.f) ||
	...
	else if (isMoving && (gAIEnv.configuration.eCompatibilityMode != ECCM_DRXSIS2))
	{
...
void CPipeUser::OnAIHandlerSentSignal(tukk szText, u32 crcCode)
{
	assert(crcCode != 0);
