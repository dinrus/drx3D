...
void GenericPlanner::OnNavigationMeshChanged(NavigationAgentTypeID navigationAgentTypeID, NavigationMeshID meshID, u32 tileID)
{
	QueueNavigationChange(navigationAgentTypeID, meshID, tileID, SMeshTileChange::EChangeType::AfterGeneration);
}

void GenericPlanner::OnNavigationAnnotationChanged(NavigationAgentTypeID navigationAgentTypeID, NavigationMeshID meshID, u32 tileID)
{
	QueueNavigationChange(navigationAgentTypeID, meshID, tileID, SMeshTileChange::EChangeType::Annotation);
}

void GenericPlanner::QueueNavigationChange(NavigationAgentTypeID navigationAgentTypeID, NavigationMeshID meshID, u32 tileID, const SMeshTileChange::ChangeFlags& changeFlag)
{
  ...
  void GenericPlanner::GetStatus(MovementRequestStatus& status) const
{
	...
	if (m_plan.GetBlockCount() > 0)
	{
		...
		for (u32 i = 0, n = m_plan.GetBlockCount(); i < n; ++i)
		{
			tukk blockName = m_plan.GetBlock(i)->GetName();
	...
	else
	{
		...
		while (curr != end)
		{
			...
			if (isCustomObject || isSmartObject || isLastNode)
			{
				...
				const float endDistance = blockAfterThisUsesSomeFormOfExactPositioning ? 2.5f : 0.0f;   // The value 2.5 meters was used prior to Drxsis 3
