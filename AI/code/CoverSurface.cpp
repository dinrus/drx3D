...
u32 CoverSurface::GetSampleCount() const
{
...
u32 CoverSurface::GetSegmentCount() const
{
	return m_segments.size();
}

CoverSurface::Segment& CoverSurface::GetSegment(u16 segmentIdx)
{
	return m_segments[segmentIdx];
}

const CoverSurface::Segment& CoverSurface::GetSegment(u16 segmentIdx) const
{
	return m_segments[segmentIdx];
}
...
u32k CoverSurface::GetFlags() const
{
	return m_flags;
}

void CoverSurface::ReserveSamples(u32 sampleCount)
{
...
{
	u32 sampleCount = m_samples.size();
	...
	if (m_flags & ICoverSystem::SurfaceInfo::Looped)
	{
		...
		Segment segment(CoverUp.Cross(rightToLeft).normalized(), rightToLeft.GetLength(), static_cast<u16>(m_samples.size() - 1), 0, 0);
...
void CoverSurface::GenerateLocations()
{
	...
	u32 segmentCount = m_segments.size();
	...
	for (u32 i = 0; i < segmentCount; ++i)
	{
		...
		if (isLast)
		{
			...
			float distanceSq = (GetLocation(static_cast<u16>(m_locations.size() - 1)) - right.position).len2();
...
u32 CoverSurface::GetLocationCount() const
{
	return m_locations.size();
}

Vec3 CoverSurface::GetLocation(u16 location, float offset, float* height, Vec3* normal) const
{
...
bool CoverSurface::GetCoverOcclusionAt(const Vec3& eye, const Vec3& center, float radius, float* heightSq) const
{
	...
	if ((result & (1 << Right)) != 0)
	{
		u32 lowest = (heightSqArr[Left] < heightSqArr[Right]) ?
                                         ...
                                         CoverInterval* interval) const
{
	...
	u32k pathPointCount = pathPoints.size();
	...
	for (u32 i = 0; i < (pathPointCount - 1); ++i)
	{
...
{
	u32k coverSampleCount = m_samples.size();
	...
	for (u32 i = 1; i < coverSampleCount - 1; ++i)
	{
...
{
	u32 coverSampleCount = m_samples.size();
	u32 segmentCount = m_segments.size();
	...
	{
		u32 locationCount = m_locations.size();
