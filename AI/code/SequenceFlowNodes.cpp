...
CFlowNode_AISequenceStart::~CFlowNode_AISequenceStart()
{
	...
	GetAISystem()->GetSequenceUpr()->UnregisterSequence(GetAssignedSequenceId());
}
...
void CFlowNode_AISequenceStart::Serialize(SActivationInfo* pActInfo, TSerialize ser)
{
	if (ser.IsWriting())
	{
		...
		{
			sequenceIsActive = GetAISystem()->GetSequenceUpr()->IsSequenceActive(assignedSequenceId);
		}
...
void CFlowNode_AISequenceStart::InitializeSequence(SActivationInfo* pActInfo)
{
	if (!pActInfo->pEntity)
	{
		DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_ERROR, "FG node CFlowNode_AISequenceStart: did you forget to assign an entity to this node?");
		return;
	}
	...
	if (ISequenceUpr* sequenceUpr = GetAISystem()->GetSequenceUpr())
	{
		...
		if (sequenceUpr->RegisterSequence(associatedEntityId, pActInfo->myID, sequenceProperties, pActInfo->pGraph))
		{
			sequenceUpr->StartSequence(GetAssignedSequenceId());
		}
	}
}
	...
	case eFE_Activate:
		{
			if (IsPortActive(pActInfo, InputPort_End))
			{
				...
				if (!assignedSequenceId)
				{
					DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_WARNING, "AISequence:End node doesn't have any sequence assigned.");
					return;
				}

				GetAISystem()->GetSequenceUpr()->CancelSequence(assignedSequenceId);
				GetAISystem()->GetSequenceUpr()->UnregisterSequence(assignedSequenceId);
	...
	case eFE_Activate:
		{
			if (IsPortActive(pActInfo, InputPort_Set))
			{
				...
				{
					GetAISystem()->GetSequenceUpr()->SetBookmark(assignedSequenceId, pActInfo->myID);
...
void CFlowNode_AISequenceActionMove::GetConfiguration(SFlowNodeConfig& config)
{
	...
	{
		InputPortConfig_Void("Start"),
		InputPortConfig<i32>("Speed",                 0,                                                                                            0, 0, _UICONFIG("enum_int:Walk=0,Run=1,Sprint=2")),
		InputPortConfig<i32>("Stance",                0,                                                                                            0, 0, _UICONFIG("enum_int:Relaxed=0,Alert=1,Combat=2,Crouch=3")),
...
void CFlowNode_AISequenceActionMove::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
		...
		if (m_stopRadiusSqr > 0.0f)
		{
			...
			if (CAIActor* pActor = CastToCAIActorSafe(pActInfo->pEntity->GetAI()))
			{
				if (pActor->GetPos().GetSquaredDistance(m_destPosition) <= m_stopRadiusSqr)
				{
					...
					GetAISystem()->GetSequenceUpr()->ActionCompleted(GetAssignedSequenceId());
	...
	case eFE_Activate:
	{
		...
		if (IsPortActive(pActInfo, InputPort_Start))
		{
			GetAISystem()->GetSequenceUpr()->RequestActionStart(assignedSequenceId, m_actInfo.myID);
			pActInfo->pGraph->SetRegularlyUpdated(pActInfo->myID, true);
		}
		break;
	}
...
{
	DRX_ASSERT(m_movementRequestID == result.requestID);
...
void CFlowNode_AISequenceActionMoveAlongPath::GetTeleportEndPositionAndDirection(tukk pathName, Vec3& position, Vec3& direction)
{
...
void CFlowNode_AISequenceActionAnimation::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_Activate:
	{
		...
		if (m_running && IsPortActive(pActInfo, InputPort_Stop))
		{
			...
			GetAISystem()->GetSequenceUpr()->ActionCompleted(assignedSequenceId);
			ActivateOutput(&m_actInfo, OutputPort_Done, true);
		}
		break;
	}
...
void CFlowNode_AISequenceActionWait::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_Activate:
	{
		...
		if (const SequenceId assignedSequenceId = GetAssignedSequenceId())
		{
			if (IsPortActive(pActInfo, InputPort_Start))
			{
				GetAISystem()->GetSequenceUpr()->RequestActionStart(assignedSequenceId, pActInfo->myID);
				pActInfo->pGraph->SetRegularlyUpdated(pActInfo->myID, true);
			}
		}
		break;
...
void CFlowNode_AISequenceActionWait::HandleSequenceEvent(SequenceEvent sequenceEvent)
{
	...
	case AIActionSequence::StartAction:
		m_waitTimeMs = i32(1000.0f * GetPortFloat(&m_actInfo, InputPort_Time));
			...
			if (m_actInfo.pEntity)
			{
				...
				if (pPipeUser)
				{
					...
					if (pRefPoint)
					{
						...
						m_startTime = GetAISystem()->GetFrameStartTime();
						m_fireTimeMS = i32(1000.0f * GetPortFloat(&m_actInfo, InputPort_Duration));
...
void CFlowNode_AISequenceJoinFormation::SendSignal(IAIActor* pIAIActor, tukk signalName, IEntity* pSender)
{
...
void CFlowNode_AISequenceAction_Stance::GetConfiguration(SFlowNodeConfig& config)
{
	...
	{
		InputPortConfig_Void("Start"),
		InputPortConfig<i32>("Stance",0,  0, 0, _UICONFIG("enum_int:Relaxed=3,Alerted=6,Combat=0,Crouch=1")),
		{ 0 }
	};
	...
	case StartAction:
		{
			...
			DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_ERROR, "CFlowNode_AISequenceAction_Stance - Failed to set the stance.");
		}
		break;
	}
}
