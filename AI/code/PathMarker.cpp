...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   PathMarker.cpp

   Описание: CPathMarker class implementation
...
CPathMarker::CPathMarker(float fMaxDistanceNeeded, float fStep)
{
	if (fStep <= .0f)
		...
		m_fStep = fStep;
	m_iSize = (i32)ceilf(fMaxDistanceNeeded / m_fStep) + 1;
...
Vec3 CPathMarker::GetPointAtDistanceFromNewestPoint(float fDesiredDistanceFromNewestPoint) const
{
	...
	for (i32 i = 0; i < m_iUsed - 1; i++)
	{
		i32 indexSegEnd = m_iCurrentPoint - i;
		i32 indexSegStart = m_iCurrentPoint - i - 1;
...
Vec3 CPathMarker::GetDirectionAtDistanceFromNewestPoint(float fDistanceFromNewestPoint) const
{
	...
	{
		i32 indexSegEnd = m_iCurrentPoint - i;
		i32 indexSegStart = m_iCurrentPoint - 1 - i;
                                             ...
                                             CSteeringDebugInfo* debugInfo)
{
	...
	i32 closestIndex = iPoint; // default to last point
	i32 closestPtsBack = 0;
	i32 ptsBack = 0;
	for (i32 i = 0; i < m_iUsed; i++)
	{
	...
	i32 nextIndex = (closestIndex + 1) % m_iSize;
	...
	iPoint = closestIndex;
	for (i32 i = 0; i < closestPtsBack; ++i)
	{
...
float CPathMarker::GetDistanceToPoint(Vec3& vTargetPoint, Vec3& vMyPoint)
{
	...
	i32 iClosestPoint = m_iCurrentPoint;
	for (i32 i = 0; i < m_iUsed; i++)
	{
...
void CPathMarker::DebugDraw()
{
	...
	i32 prev = m_iCurrentPoint;
	i32 cur = m_iCurrentPoint - 1;
