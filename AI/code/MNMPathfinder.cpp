...
void MNM::PathfinderUtils::QueuedRequest::SetupAttentionTargetDanger()
{
	...
	DRX_ASSERT(requesterEntityId != INVALID_ENTITYID);
...
void MNM::PathfinderUtils::QueuedRequest::SetupExplosiveDangers()
{
	...
	AutoAIObjectIter grenadesIterator(gAIEnv.pAIObjectUpr->GetFirstAIObject(OBJFILTER_TYPE, AIOBJECT_GRENADE));
...
void MNM::PathfinderUtils::QueuedRequest::SetupGroupMatesAvoidance()
{
	...
	i32k requesterGroupId = requesterAIObject->GetGroupId();
	AutoAIObjectIter groupMemberIterator(gEnv->pAISystem->GetAIObjectUpr()->GetFirstAIObject(OBJFILTER_GROUP, requesterGroupId));
...
MNM::QueuedPathID CMNMPathfinder::RequestPathTo(const EntityId requesterEntityId, const MNMPathRequest& request)
{
	...
	//////////////////////////////////////////////////////////////////////////
	// Validate Agent Type
	tukk actorName = pEntity->GetName();
...
void CMNMPathfinder::SetupNewValidPathRequests()
{
	...
	for (size_t i = 0; (i < freeAvailableSlotsToProcessNewRequests) && !m_requestedPathsQueue.empty(); ++i)
	{
		...
		{
			DRX_ASSERT_MESSAGE(0, "Trying to setup new requests while not having available slots in the pool.");
		}
...
{
	DRX_ASSERT_MESSAGE(processingContext.status == MNM::PathfinderUtils::ProcessingContext::InProgress, "PathfinderProcessingJob is spawned even if the process is not 'InProgress'.");

	DRX_ASSERT_MESSAGE(!processingContext.jobState.IsRunning(), "The job is still running and we are spawning a new one.");
...
{
	gEnv->GetJobUpr()->WaitForJob(processingContext.jobState);
}
...
bool CMNMPathfinder::SetupForNextPathRequest(MNM::QueuedPathID requestID, const MNM::PathfinderUtils::QueuedRequest& request, MNM::PathfinderUtils::ProcessingContext& processingContext)
{
	...
	u16k zOffsetMultiplier = min(u16(2), (u16)agentTypeProperties.settings.agent.height);
...
void CMNMPathfinder::ProcessPathRequest(MNM::PathfinderUtils::ProcessingContext& processingContext)
{
	...
	const MNM::CNavMesh& navMesh = mesh.navMesh;
	const MNM::CNavMesh::SGridParams& gridParams = navMesh.GetGridParams();
	const OffMeshNavigationUpr* offMeshNavigationUpr = gAIEnv.pNavigationSystem->GetOffMeshNavigationUpr();
	assert(offMeshNavigationUpr);
	const MNM::OffMeshNavigation& meshOffMeshNav = offMeshNavigationUpr->GetOffMeshNavigationForMesh(processingRequest.meshID);
	...
	MNM::CNavMesh::WayQueryRequest inputParams(
		...
		processingRequest.data.requestParams.endLocation - gridParams.origin, meshOffMeshNav, *offMeshNavigationUpr,
...
bool CMNMPathfinder::ConstructPathFromFoundWay(
  ...
  const OffMeshNavigationUpr* pOffMeshNavigationUpr,
  ...
  CPathHolder<PathPointDescriptor>& outputPath)
{
	...
	for (size_t i = 0; i < waySize; ++i)
	{
		...
		if (pWayData[i].offMeshLinkID)
		{
			...
			const MNM::OffMeshLink* pOffMeshLink = pOffMeshNavigationUpr->GetOffMeshLink(pWayData[i].offMeshLinkID);
			...
			DRX_ASSERT_MESSAGE(i > 0, "Path contains offmesh link without exit waypoint");
			if (i > 0)
			{
...
void CMNMPathfinder::ConstructPathIfWayWasFound(MNM::PathfinderUtils::ProcessingContext& processingContext)
{
	...
	DRX_PROFILE_FUNCTION_ARG(PROFILE_AI, DrxStringUtils::toString(static_cast<unsigned>(processingContext.queryResult.GetWaySize())));
...
{
	DRX_ASSERT_MESSAGE(processingContext.status == MNM::PathfinderUtils::ProcessingContext::FindWayCompleted, "PathConstructionJob spawned even if the status is not 'FindWayCompleted'");
...
void CMNMPathfinder::DebugStatistics(MNM::PathfinderUtils::ProcessingContext& processingContext, const float textY)
{
	...
	text.Format(
	  ...
	  (u32)processingContext.processingRequest.data.requestParams.agentTypeID,
