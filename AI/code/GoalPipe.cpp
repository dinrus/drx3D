...
#include <drx3D/AI/GoalOp_G02.h>
#include <drx3D/AI/GoalOp_G04.h>    //TODO move these out of AISystem
#include <drx3D/AI/GoalOp.h>
#include <drx3D/AI/GoalOpStick.h>
#include <drx3D/AI/GoalOpTrace.h>
#include <drx3D/AI/MoveOp.h>
#include <drx3D/AI/ShootOp.h>
#include <drx3D/AI/TeleportOp.h>
...
QGoal QGoal::Clone()
{
	...
	if (pGoalOp)
	{
		...
		case eGO_ACQUIRETARGET:
			...
			break;
		case eGO_ADJUSTAIM:
			goal.pGoalOp = new COPDrxsis2AdjustAim(static_cast<const COPDrxsis2AdjustAim&>(*pGoalOp));
			break;
		case eGO_PEEK:
			goal.pGoalOp = new COPDrxsis2Peek(static_cast<const COPDrxsis2Peek&>(*pGoalOp));
			break;
		case eGO_ANIMATION:
		...
		case eGO_APPROACH:
			...
			break;
		case eGO_BRANCH:
			goal.pGoalOp = (gAIEnv.configuration.eCompatibilityMode == ECCM_DRXSIS2)
		...
		case eGO_FOLLOWPATH:
			...
			break;
		case eGO_HIDE:
			goal.pGoalOp = new COPDrxsis2Hide(static_cast<const COPDrxsis2Hide&>(*pGoalOp));
			break;
		case eGO_IGNOREALL:
		...
		case eGO_TELEPORT:
			...
			break;
		case eGO_STICKPATH:
			goal.pGoalOp = new COPDrxsis2StickPath(static_cast<const COPDrxsis2StickPath&>(*pGoalOp));
			break;
		case eGO_STRAFE:
		...
		case eGO_WAITSIGNAL:
			...
			break;
		case eGO_HOVER:
			goal.pGoalOp = new COPDrxsis2Hover(static_cast<const COPDrxsis2Hover&>(*pGoalOp));
			break;
		case eGO_CHASETARGET:
			goal.pGoalOp = new COPDrxsis2ChaseTarget(static_cast<const COPDrxsis2ChaseTarget&>(*pGoalOp));
			break;
		case eGO_FIREWEAPONS:
			goal.pGoalOp = new COPDrxsis2FlightFireWeapons(static_cast<COPDrxsis2FlightFireWeapons&>(*pGoalOp));
			break;
		case eGO_ACQUIREPOSITION:
			goal.pGoalOp = new COPAcquirePosition(static_cast<COPAcquirePosition&>(*pGoalOp));
			break;
		case eGO_FLY:
			goal.pGoalOp = new COPDrxsis2Fly(static_cast<const COPDrxsis2Fly&>(*pGoalOp));
			break;
#if 0
...
CGoalPipe::CGoalPipe(tukk sName, bool bDynamic)
	: m_sName(sName),
	m_bDynamic(bDynamic),
	m_nPosition(0),
	m_pSubPipe(0),
...
{
	PREFAST_SUPPRESS_WARNING(6326) static_assert(DRX_ARRAY_COUNT(g_GoalOpNames) == eGO_LAST + 1, "Invalid array size!");
}
	...
	if (!m_Labels.empty())
	{
		...
		for (VectorOGoals::iterator gi = m_qGoalPipe.begin(); gi != m_qGoalPipe.end(); ++gi)
		{
			if (!gi->params.str.empty() && (gi->op == eGO_BRANCH || gi->op == eGO_RANDOM))
			{
				...
				if (it != m_Labels.end())
				{
					i32 absolute = it->second;
					i32 current = (i32)(gi - m_qGoalPipe.begin());
					if (current < absolute)
...
CGoalOp* CGoalPipe::CreateGoalOp(EGoalOperations op, const XmlNodeRef& goalOpNode)
{
	...
	case eGO_ADJUSTAIM:
		return new COPDrxsis2AdjustAim(goalOpNode);
	case eGO_PEEK:
		return new COPDrxsis2Peek(goalOpNode);
	case eGO_ANIMATION:
	...
	case eGO_BRANCH:
		return (gAIEnv.configuration.eCompatibilityMode == ECCM_DRXSIS2)
		       ? static_cast<CGoalOp*>(new COPDummy)
		       : static_cast<CGoalOp*>(new COPDeValue);
	case eGO_CLEAR:
		return new COPClear;
	...
	case eGO_HIDE:
		return new COPDrxsis2Hide(goalOpNode);
	case eGO_IGNOREALL:
	...
	case eGO_STICKPATH:
		return new COPDrxsis2StickPath(goalOpNode);
	case eGO_STRAFE:
	...
	case eGO_HOVER:
		return new COPDrxsis2Hover(goalOpNode);
	case eGO_CHASETARGET:
		return new COPDrxsis2ChaseTarget(goalOpNode);
	case eGO_FIREWEAPONS:
		return new COPDrxsis2FlightFireWeapons(goalOpNode);
	case eGO_FLY:
		return new COPDrxsis2Fly(goalOpNode);
	case eGO_ACQUIREPOSITION:
	...
	default:
		AIError("Unrecognized goalop code: %d.", (i32)(op));
		assert(!!!"Unrecognized goalop code.");
		return 0;
	}
}

void CGoalPipe::SetDebugName(tukk name)
{
	m_sDebugName = name;
}
...
void CGoalPipe::ParseParam(tukk param, const GoalParams& value)
{
	for (VectorOGoals::iterator it = m_qGoalPipe.begin(); m_qGoalPipe.end() != it; ++it)
	{
...
void CGoalPipe::PushLabel(tukk label)
{
...
EGoalOperations CGoalPipe::GetGoalOpEnum(tukk szName)
{
...
tukk CGoalPipe::GetGoalOpName(EGoalOperations op)
{
...
void CGoalPipe::PushGoal(EGoalOperations op, bool bBlocking, EGroupType eGrouping, GoalParameters& params)
{
	...
	case eGO_SIGNAL:
		{
			newgoal.pGoalOp = new COPSignal(params.nValueAux, params.str, static_cast<ESignalFilter>(params.nValue), (i32)params.fValueAux);
		}
		break;
	case eGO_SCRIPT:
		{
	...
	case eGO_DEVALUE:
		{
			newgoal.pGoalOp = new COPDeValue((i32)params.fValue, params.bValue);
		}
		break;
#if 0
	// deprecated and won't compile at all...
	...
	case eGO_CLEAR:
		{
			...
			{
				// Executing COPDeValue will remove the attention target. We don't want this for Drxsis 2.
...
//
//-------------------------------------------------------------------------------------------------------
void CGoalPipe::PushPipe(tukk szName, bool bBlocking, EGroupType eGrouping, GoalParameters& params)
{
	if (eGrouping == IGoalPipe::eGT_GROUPED)
		++m_nCurrentBlockCounter;
	else
...
EPopGoalResult CGoalPipe::PopGoal(QGoal& theGoal, CPipeUser* pOperand)
{
	...
	if (m_nPosition < m_qGoalPipe.size())
	{
		...
		{
			if (CGoalPipe* pPipe = gAIEnv.pPipeUpr->IsGoalPipe(current.sPipeName))
			{
		...
		else
		{
			...
			{
				// MERGE :  (MATT) We should hammer this down in Drxsis and make this a full error {2008/01/07:12:38:43}
...
bool CGoalPipe::Jump(i32 position)
{
	// if we are processing a subpipe
	if (m_pSubPipe)
	{
...
bool CGoalPipe::Jump(tukk label)
{
	// if we are processing a subpipe
	if (m_pSubPipe)
	{
...
//
//------------------------------------------------------------------------------------------------------
bool CGoalPipe::RemoveSubpipe(CPipeUser* pPipeUser, i32& goalPipeId, bool keepInserted, bool keepHigherPriority)
{
	if (m_bHighPriority)
...
void CGoalPipe::Serialize(TSerialize ser, VectorOGoals& activeGoals)
{
	...
	else
	{
		...
		{
			drx_sprintf(groupName, "ActiveGoal-%d", i);
			ser.BeginGroup(groupName);
			ser.Value("goalIdx", goalIdx);
	...
	if (ser.BeginOptionalGroup("SubPipe", bIsInSubpipe))
	{
		if (ser.IsReading())
		{
			...
			SetSubpipe(gAIEnv.pPipeUpr->IsGoalPipe(subPipeName));
		}
		else
		{
...
void CGoalPipe::SerializeDynamic(TSerialize ser)
{
	...
	u32 count(m_qGoalPipe.size());
	char buffer[16];
	ser.Value("count", count);
	for (u32 curIdx(0); curIdx < count; ++curIdx)
	{
		drx_sprintf(buffer, "goal_%d", curIdx);
		ser.BeginGroup(buffer);
		{
			...
			if (ser.IsReading())
			{
				if (gl.op != eGO_LAST)
				{
					u32 prevNumGoalOps = GetNumGoalOps();
