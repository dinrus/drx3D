...
#include <drx3D/AI/MissLocationSensor.h>
...
void CAIPlayer::CollectExposedCover()
{
	...
	if (m_coverExposedTime > 0.0f)
	{
		if (m_exposedCoverState.asyncState == AsyncReady)
		{
			...
			i32k flags = rwi_colltype_any | (geom_colltype_obstruct << rwi_colltype_bit) | (AI_VISION_RAY_CAST_PIERCEABILITY & rwi_pierceability_mask);
...
u32 CAIPlayer::GetObservableTypeMask() const
{
...
void CAIPlayer::GetPhysicalSkipEntities(PhysSkipList& skipList) const
{
	...
	// Skip exposed covers
	for (u32 i = 0, ni = m_exposedCoverObjects.size(); i < ni; ++i)
	{
...
void CAIPlayer::NotifyPlayerActionToTheLookingAgents(tukk eventName)
{
	CAISystem* pAISystem = GetAISystem();
	ActorLookUp& lookUp = *gAIEnv.pActorLookUp;
	size_t activeCount = lookUp.GetActiveCount();
