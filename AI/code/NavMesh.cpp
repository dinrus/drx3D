...
bool CNavMesh::WayQueryRequest::CanUseOffMeshLink(const OffMeshLinkID linkID, float* costMultiplier) const
{
	if (m_requesterEntityId)
	{
		...
		{
			if (const OffMeshLink* pOffMeshLink = m_offMeshNavigationUpr.GetOffMeshLink(linkID))
			{
...
bool CNavMesh::WayQueryRequest::IsPointValidForAgent(const Vec3& pos, u32 flags) const
{
...
{
	DRX_ASSERT(maxTrianglesCount > 0);
	...
	for (size_t i = 0; i < tile.triangleCount; ++i)
	{
		...
		{
			const TriangleID triangleID = ComputeTriangleID(tileID, static_cast<u16>(i));
		...
		for (uint x = minX; x <= maxX; ++x)
		{
			for (uint z = minZ; z <= maxZ; ++z)
			{
				if (const TileID tileID = GetTileID(x, y, z))
				{
					...
					DRX_ASSERT(maxTrianglesCountLeft >= trianglesInTileCount);
...
size_t CNavMesh::GetTrianglesBordersNoFilter(const TriangleID* triangleIDs, const size_t triangleCount, Vec3* pBorders, const size_t maxBorderCount) const
{
	...
	for (size_t i = 0; i < triangleCount; ++i)
	{
		...
		u16 triangleIdx = ComputeTriangleIndex(triangleIDs[i]);
		if (!tileID)
			continue;
...
const AreaAnnotation* CNavMesh::GetTriangleAnnotation(TriangleID triangleID) const
{
	...
	{
		const TileContainer& container = m_tiles[tileID - 1];
		u16k triIndex = ComputeTriangleIndex(triangleID);
		DRX_ASSERT(triIndex < container.tile.triangleCount);
...
void CNavMesh::SetTrianglesAnnotation(const MNM::TriangleID* pTrianglesArray, const size_t trianglesCount, const MNM::AreaAnnotation areaAnnotation, std::vector<TileID>& affectedTiles)
{
	for (size_t i = 0; i < trianglesCount; ++i)
	{
		...
		{
			u16k triangleIndex = MNM::ComputeTriangleIndex(triangleId);
...
void CNavMesh::PredictNextTriangleEntryPosition(const TriangleID bestNodeTriangleID,
                                                const vector3_t& bestNodePosition, const TriangleID nextTriangleID, u32k edgeIndex
		...
		else
		{
			...
			while (workingSet.aStarOpenList.CanDoStep())
			{
				...
				if (tileID)
				{
					...
					u16k triangleIdx = ComputeTriangleIndex(bestNodeID.triangleID);
					...
					for (size_t l = 0; l < triangle.linkCount; ++l)
					{
						...
						else if (link.side == Tile::SLink::OffMesh)
						{
							...
							while (nextTri = links.GetNextTriangle())
							{
								...
								{
									workingSet.nextLinkedTriangles.push_back(nextTri);
									nextTri.incidentEdge = (u32)MNM::Constants::InvalidEdgeIndex;
								}
							}
							continue;
						}
...
void CNavMesh::PullString(const vector3_t& from, const TriangleID fromTriID, const vector3_t& to, const TriangleID toTriID, vector3_t& middlePoint) const
{
	if (const TileID fromTileID = ComputeTileID(fromTriID))
	{
		...
		u16 fromTriangleIdx = ComputeTriangleIndex(fromTriID);
		const Tile::STriangle& fromTriangle = startTile.triangles[fromTriangleIdx];

		u16 vi0 = 0, vi1 = 0;
		for (i32 l = 0; l < fromTriangle.linkCount; ++l)
		{
...
void CNavMesh::AddOffMeshLinkToTile(const TileID tileID, const TriangleID triangleID, u16k offMeshIndex)
{
	STile& tile = GetTile(tileID);

	m_profiler.FreeMemory(LinkMemory, tile.linkCount * sizeof(Tile::SLink));
	m_profiler.AddStat(LinkCount, -(i32)tile.linkCount);
...
void CNavMesh::UpdateOffMeshLinkForTile(const TileID tileID, const TriangleID triangleID, u16k offMeshIndex)
{
	STile& tile = GetTile(tileID);
...
struct RaycastNode
{
	RaycastNode()
		...
		, incidentEdge((u16) MNM::Constants::InvalidEdgeIndex)
	{}

	RaycastNode(const TriangleID _triangleID, const real_t _percentageOfTotalDistance, u16k _incidentEdge)
	...
	RaycastRequestBase& raycastRequest) const
{
	...
	RaycastNode currentNode(fromTriangleID, real_t(.0f), (u16)MNM::Constants::InvalidEdgeIndex);
	...
	while (currentNode.IsValid())
	{
		...
		if (tileID != MNM::Constants::InvalidTileID)
		{
			...
			for (u16 edgeIndex = 0; edgeIndex < 3; ++edgeIndex)
			{
				...
				if (IntersectSegmentSegment(vector2_t(from), vector2_t(to), vector2_t(a), vector2_t(b), s, t))
				{
					...
					for (size_t linkIndex = 0; linkIndex < triangle.linkCount; ++linkIndex)
					{
						...
						else if (side != Tile::SLink::OffMesh)
						{
							...
							u16k currentTriangleIndex = ComputeTriangleIndex(currentNode.triangleID);
							u16k currentOppositeSide = static_cast<u16>(OppositeSide(side));
							...
							for (size_t reciprocalLinkIndex = 0; reciprocalLinkIndex < opposite.linkCount; ++reciprocalLinkIndex)
							{
								...
								if ((reciprocal.triangle == currentTriangleIndex) && (reciprocal.side == currentOppositeSide))
								{
									...
									u16k i0 = reciprocal.edge;
									u16k i1 = inc_mod3[reciprocal.edge];
...
CNavMesh::ERayCastResult CNavMesh::RayCast_v3(const vector3_t& fromPos, TriangleID fromTriangleID, const vector3_t& toPos, const TFilter& filter, RaycastRequestBase& raycastRequest) const
{
	...
	while (currentTriangleID != MNM::Constants::InvalidTriangleID)
	{
		...
		DRX_ASSERT(currentTileID != MNM::Constants::InvalidTileID);
		...
		u16k currentTriangleIndex = ComputeTriangleIndex(currentTriangleID);
		...
		if (intersectionEdgeIndex == u16(MNM::Constants::InvalidEdgeIndex))
		{
			if (bEndingInside)
			{
...
template<typename TFilter>
MNM::TriangleID CNavMesh::StepOverEdgeToNeighbourTriangle(const vector3_t& rayStart, const vector3_t& rayEnd, const TileID currentTileID, const TriangleID currentTriangleID, u16k edgeIndex, const TFilter& filter) const
{
	const TileContainer& currentContainer = m_tiles[currentTileID - 1];
	const STile& currentTile = currentContainer.tile;
				...
				if (IntersectSegmentSegment(vector2_t(from), vector2_t(to), vector2_t(a), vector2_t(b), s, t))
				{
					...
					for (size_t l = 0; l < triangle.linkCount; ++l)
					{
						...
						else if (side != Tile::SLink::OffMesh)
						{
							...
							u16k currentTriangleIndex = ComputeTriangleIndex(currentID);
							u16k currentOppositeSide = static_cast<u16>(OppositeSide(side));
							...
							for (size_t rl = 0; rl < opposite.linkCount; ++rl)
							{
								...
								if ((reciprocal.triangle == currentTriangleIndex) && (reciprocal.side == currentOppositeSide))
								{
									...
									u16k i0 = reciprocal.edge;
									u16k i1 = next_mod3(reciprocal.edge);
...
{
	i32k ox = NavMesh::GetNeighbourTileOffset(side)[0];
	i32k oy = NavMesh::GetNeighbourTileOffset(side)[1];
...
TileID CNavMesh::SetTile(size_t x, size_t y, size_t z, STile& tile)
{
	...
	else
	{
		...
		m_profiler.AddStat(VertexCount, -(i32)oldTile.vertexCount);
		m_profiler.AddStat(TriangleCount, -(i32)oldTile.triangleCount);
		m_profiler.AddStat(BVTreeNodeCount, -(i32)oldTile.nodeCount);
		m_profiler.AddStat(LinkCount, -(i32)oldTile.linkCount);
...
void CNavMesh::ClearTile(TileID tileID, bool clearNetwork)
{
...

	{
		...
		m_profiler.AddStat(VertexCount, -(i32)container.tile.vertexCount);
		m_profiler.AddStat(TriangleCount, -(i32)container.tile.triangleCount);
		m_profiler.AddStat(BVTreeNodeCount, -(i32)container.tile.nodeCount);
		m_profiler.AddStat(LinkCount, -(i32)container.tile.linkCount);
                                  ...
                                  Edge* edges, u16* adjacency)
{
...

	{
		...
		u16 edgeLookUp[MaxLookUp];
		assert(MaxLookUp > vertexCount + triangleCount * 3);

		std::fill(&edgeLookUp[0], &edgeLookUp[0] + vertexCount, static_cast<u16>(Unused));
		...
		for (u16 i = 0; i < triangleCount; ++i)
		{
			...
			{
				u16 i1 = triangle.vertex[v];
				u16 i2 = triangle.vertex[next_mod3(v)];

				if (i1 < i2)
				{
					u16 edgeIdx = edgeCount++;
...
{
	i32k nx = tileGridCoord.x.as_int();
	i32k ny = tileGridCoord.y.as_int();
	i32k nz = tileGridCoord.z.as_int();
...
{
	DRX_ASSERT(pCandidateTriangles);
	DRX_ASSERT(candidateTrianglesCount > 0);
...
void CNavMesh::ComputeAdjacency(size_t x, size_t y, size_t z, const real_t& toleranceSq, STile& tile)
{
	...
	u16 adjacency[MaxTriangleCount * 3];
	...
	tile.CopyLinks(links, static_cast<u16>(linkCount));
...
bool CNavMesh::CalculateMidEdge(const TriangleID triangleID1, const TriangleID triangleID2, Vec3& result) const
{
	...
	if (const TileID tileID = ComputeTileID(triangleID1))
	{
		const TileContainer& container = m_tiles[tileID - 1];
		const STile& tile = container.tile;
		u16k triangleIdx = ComputeTriangleIndex(triangleID1);
		const Tile::STriangle& triangle = tile.triangles[triangleIdx];
...
bool CNavMesh::FindNextIntersectingTriangleEdge(const vector3_t& rayStartPos3D, const vector3_t& rayEndPos3D, const vector2_t pTriangleVertices[3],
	real_t& rayIntersectionParam, u16& intersectingEdgeIndex) const
{
	/*       
	          _Rs
	        \ |
	...
	*/
	...
	intersectingEdgeIndex = u16(MNM::Constants::InvalidEdgeIndex);
...
void CNavMesh::ResetAccessibility(u8 accessible)
{
...
void CNavMesh::ComputeAccessibility(const AccessibilityRequest& inputRequest)
{
	...
	{
		u16k startTriangleIdx = ComputeTriangleIndex(inputRequest.fromTriangle);
		...
		while (!openNodes.empty())
		{
			...
			{
				u16k triangleIdx = ComputeTriangleIndex(currentNode.id);
				...
				for (size_t t = 0; t < nextTriangles.size(); ++t)
				{
					...
					if (const TileID nextTileID = ComputeTileID(nextTriangleID))
					{
						...
						u16k nextTriangleIndex = ComputeTriangleIndex(nextTriangleID);
