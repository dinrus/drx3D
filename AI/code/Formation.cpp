...
void CFormation::AddPoint(FormationNode& desc)
{
	...
	char name[100];
	i32 i = m_FormationPoints.size();

	{
		drx_sprintf(name, "FORMATION_%d", i);
		CStrongRef<CAIObject> refFormationDummy;
		gAIEnv.pAIObjectUpr->CreateDummyObject(refFormationDummy, name);
	...
	{
		drx_sprintf(name, "FORMATION_DUMMY_TARGET_%d", i);
		CStrongRef<CAIObject> refNewDummyTarget;
		gAIEnv.pAIObjectUpr->CreateDummyObject(refNewDummyTarget, name);
...
bool CFormation::GetPointOffset(i32 i, Vec3& ret)
{
	if (i >= GetSize())
		return false;
...
i32 CFormation::GetPointIndex(CWeakRef<const CAIObject> refRequester) const
{
	i32 pointIndex = 0;
...
//-----------------------------------------------------------------------------------------------------------------
//
Vec3 CFormation::GetPointSmooth(const Vec3& headPos, float followDist, float sideDist, float heightValue, i32 smoothValue, Vec3& smoothPosAlongCurve, CAIObject* pUser)
{
	...
	for (i32 cnt = 0; cnt < smoothValue; ++cnt)
	{
	...
	if (!m_bUpdate || fMovement < m_fUpdateThreshold && !m_bInitMovement && m_fRepositionSpeedWrefTarget == 0)
	{
		...
		if (m_fSightRotationRange > 0)
		{
			...
			for (TFormationPoints::iterator itrPoint(m_FormationPoints.begin()); itrPoint != itFormEnd; ++itrPoint)
			{
				...
				i32 randomIntervalMs = m_minUpdateSightTimeMs + drx_random(0U, (u32)intervalRange);
				i32 pointUpdateTimeMs = (i32)(currentTime - frmPoint.m_fLastUpdateSightTime).GetMilliSecondsAsInt64();
				...
				if (pointUpdateTimeMs > randomIntervalMs)
				{
					...
					if (pDummyTarget)
					{
						...
						else
						{
							...
							float angle = drx_random(-0.5f, 0.5f) * m_fSightRotationRange;
...
CAIObject* CFormation::GetNewFormationPoint(CWeakRef<CAIActor> refRequester, i32 index)
{
	CAIObject* pPoint = NULL;

	i32 size = m_FormationPoints.size();
	if (index >= size)
		return NULL;
...
i32 CFormation::GetClosestPointIndex(CWeakRef<CAIActor> refRequester, bool bReserve, i32 maxSize, i32 iClass, bool bCLosestToOwner)
{
	CCCPOINT(CFormation_GetClosestPointIndex);

	i32 size = (maxSize == 0 ? m_FormationPoints.size() : maxSize);
	i32 index = -1;
...
CAIObject* CFormation::GetFormationDummyTarget(CWeakRef<const CAIObject> refRequester) const
{
	...
	for (u32 i = 0; i < m_FormationPoints.size(); i++)
...
void CFormation::SetDummyTargetPosition(const Vec3& vPosition, CAIObject* pDummyTarget, const Vec3& vSight)
{
	...
	if (pDummyTarget)
	{
		...
		float fi = m_fSIGHT_WIDTH * drx_random(-0.5f, 0.5f);
...
void CFormation::Draw()
{
	...
	for (u32 i = 0; i < m_FormationPoints.size(); i++)
	{
		...
		ColorB colorSight(0, 255, 179, u8(255 * alfaSud));
...
void CFormation::FreeFormationPoint(i32 i)
{
...
//----------------------------------------------------------------------
float CFormation::GetDistanceToOwner(i32 i) const
{
...
//----------------------------------------------------------------------
i32 CFormation::CountFreePoints() const
{
	i32 iSize = GetSize();
	i32 iFree = 0;
...
//----------------------------------------------------------------------
CAIObject* CFormation::GetClosestPoint(CAIActor* pRequester, bool bReserve, i32 iClass)
{
	i32 index = GetClosestPointIndex(GetWeakRef(pRequester), bReserve, 0, iClass);
	...
	if (desc.m_sName != m_szDescriptor)
	{
		...
		for (i32 iteration = 0; iteration < 2; iteration++)
		{
			...
			for (++itrPoint; itrPoint != m_FormationPoints.end(); ++itrPoint)
			{
				...
				i32 iClass = frmPoint.m_Class;

				bool bFound = false;
				for (i32 iTestType = 0; iTestType < 5 && !bFound; iTestType++)
				{
					i32 j = 1;
					vi = desc.m_Nodes.begin();
					for (++vi; !bFound && vi != desc.m_Nodes.end(); ++vi)
					{
						i32 iNewClass = (*vi).eClass;
						if (!vDescChanged[j] &&
...
void CFormation::SetUpdateSight(float angleRange, float minTime /*=0*/, float maxTime /*=0*/)
{
	...
	m_minUpdateSightTimeMs = (i32)(minTime * 1000.0f);
	m_maxUpdateSightTimeMs = (i32)(maxTime * 1000.0f);
}
