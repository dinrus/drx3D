...
STRUCT_BITFIELD_INFO(linkCount, TYPE_INFO(u16), 4)
STRUCT_BITFIELD_INFO(firstLink, TYPE_INFO(u16), 12)
STRUCT_VAR_INFO(islandID, TYPE_INFO(StaticIslandID))
STRUCT_INFO_END(MNM::Tile::STriangle)

STRUCT_INFO_BEGIN(MNM::Tile::SBVNode)
STRUCT_BITFIELD_INFO(leaf, TYPE_INFO(u16), 1)
STRUCT_BITFIELD_INFO(offset, TYPE_INFO(u16), 15)
STRUCT_VAR_INFO(aabb, TYPE_INFO(AABB))
STRUCT_INFO_END(MNM::Tile::SBVNode)

STRUCT_INFO_BEGIN(MNM::Tile::SLink)
STRUCT_BITFIELD_INFO(side, TYPE_INFO(u32), 4)
STRUCT_BITFIELD_INFO(edge, TYPE_INFO(u32), 2)
STRUCT_BITFIELD_INFO(triangle, TYPE_INFO(u32), 10)
...
void STile::SetTriangles(std::unique_ptr<Tile::STriangle[]>&& pTriangles_, u16 count)
{
	triangles = pTriangles_.release();
	triangleCount = count;
}

void STile::SetVertices(std::unique_ptr<Tile::Vertex[]>&& pVertices_, u16 count)
{
	vertices = pVertices_.release();
	vertexCount = count;
}

void STile::SetNodes(std::unique_ptr<Tile::SBVNode[]>&& pNodes_, u16 count)
{
	nodes = pNodes_.release();
	nodeCount = count;
}

void STile::SetLinks(std::unique_ptr<Tile::SLink[]>&& pLinks_, u16 count)
{
	links = pLinks_.release();
	linkCount = count;
}

void STile::SetHashValue(u32 value)
{
	hashValue = value;
}
...
void STile::CopyTriangles(const Tile::STriangle* _triangles, u16 count)
{
#if MNM_USE_EXPORT_INFORMATION
...
void STile::CopyVertices(const Tile::Vertex* _vertices, u16 count)
{
...
void STile::CopyNodes(const Tile::SBVNode* _nodes, u16 count)
{
	CopyTileData(_nodes, count, nodes, nodeCount);
}

void STile::CopyLinks(const Tile::SLink* _links, u16 count)
{
	CopyTileData(_links, count, links, linkCount);
}

void STile::AddOffMeshLink(const TriangleID triangleID, u16k offMeshIndex)
{
	TILE_VALIDATE();

	u16 triangleIdx = ComputeTriangleIndex(triangleID);
	...
	if (triangleIdx < triangleCount)
	{
		...
		DRX_ASSERT_MESSAGE(!hasOffMeshLink, "Not adding offmesh link, already exists");
		...
		if (!hasOffMeshLink)
		{
			//Add off-mesh link for triangle
			{
				...
				i32k countDiff = linkCount - triangle.firstLink;
				if (countDiff > 0)
				{
					assert(links);
			...
			for (u16 tIdx = (triangleIdx + 1); tIdx < triangleCount; ++tIdx)
			{
...
void STile::UpdateOffMeshLink(const TriangleID triangleID, u16k offMeshIndex)
{
	TILE_VALIDATE();

	u16 triangleIndex = ComputeTriangleIndex(triangleID);
	assert(triangleIndex < triangleCount);
	if (triangleIndex < triangleCount)
	{
		//First link is always off-mesh if exists
		u16k linkIdx = triangles[triangleIndex].firstLink;
...
void STile::RemoveOffMeshLink(const TriangleID triangleID)
{
	...
	u16 linkToRemoveIdx = 0xFFFF;
	u16 boundTriangleIdx = ComputeTriangleIndex(triangleID);
	if (boundTriangleIdx < triangleCount)
	{
		u16k firstLink = triangles[boundTriangleIdx].firstLink;
	...
	// Try enabling DEBUG_MNM_DATA_CONSISTENCY_ENABLED if you get this
	DRX_ASSERT_MESSAGE(linkToRemoveIdx != 0xFFFF, "Trying to remove off mesh link that doesn't exist");
	...
	if (linkToRemoveIdx != 0xFFFF)
	{
		...
		i32k diffCount = linkCount - (linkToRemoveIdx + 1);
		if (diffCount > 0)
		{
		...
		for (u16 tIdx = (boundTriangleIdx + 1); tIdx < triangleCount; ++tIdx)
		{
			triangles[tIdx].firstLink--;
		}
	}
...
ColorF CalculateColorFromMultipleItems(u32 itemIdx, u32 itemsCount)
{
	if (itemsCount == 0)
		return Col_White;
...
void STile::Draw(size_t drawFlags, vector3_t origin, TileID tileID, const std::vector<float>& islandAreas, const ITriangleColorSelector& colorSelector) const
{
	...
	if (drawFlags & DrawTriangles)
	{
		...
		for (size_t i = 0; i < triangleCount; ++i)
		{
			...
			{
				triangleColor = CalculateColorFromMultipleItems(triangle.islandID, static_cast<u32>(islandAreas.size()));
			}
			...
			if ((drawFlags & DrawTrianglesId) || drawIslandData)
			{
				...
				if ((drawFlags & DrawTrianglesId) && drawIslandData)
				{
					text.Format("id: %d, area: %f, islandID: %u", ComputeTriangleID(tileID, static_cast<u16>(i)), static_cast<u16>(islandAreas[triangle.islandID - 1]), triangle.islandID);
				}
				else if (drawFlags & DrawTrianglesId)
				{
					text.Format("id: %d", ComputeTriangleID(tileID, static_cast<u16>(i)));
				}
				else
				{
...
void STile::ValidateTriangleLinks()
{
	...
	for (u16 i = 0; i < triangleCount; ++i)
	{
		...
		DRX_ASSERT_MESSAGE(triangle.firstLink <= linkCount || linkCount == 0, "Out of range link");

		DRX_ASSERT_MESSAGE(nextLink == triangle.firstLink, "Links are not contiguous");

		nextLink += triangle.linkCount;

		for (u16 l = 0; l < triangle.linkCount; ++l)
		{
			u16 linkIdx = triangle.firstLink + l;

			DRX_ASSERT_MESSAGE(links[linkIdx].side != Tile::SLink::OffMesh || l == 0, "Off mesh links should always be first");
		}
	}

	DRX_ASSERT(nextLink == linkCount);
}
...
void STile::ValidateTriangles() const
{
	...
	for (size_t triangleIdx = 0; triangleIdx < triangleCount; ++triangleIdx)
	{
		...
		for (i32 i = 0; i < 3; ++i)
		{
			...
			{
				DRX_ASSERT_MESSAGE(tri.vertex[i] < verticesMaxIndex, "MNM traingle invalid vertex index");
			}
		}
	}
}
...
void STile::InitConnectivity(u16 oldTriangleCount, u16 newTriangleCount)
{
	if (ConsiderExportInformation())
	{
		...
		if (oldTriangleCount != newTriangleCount)
		{
			...
			if (newTriangleCount)
			{
				connectivity.trianglesAccessible = new u8[newTriangleCount];
			}
		}

		if (newTriangleCount)
		{
			memset(connectivity.trianglesAccessible, 1, sizeof(u8) * newTriangleCount);
		}
		connectivity.triangleCount = newTriangleCount;
	}
}

void STile::ResetConnectivity(u8 accessible)
{
	if (ConsiderExportInformation())
	{
		...
		{
			memset(connectivity.trianglesAccessible, accessible, sizeof(u8) * connectivity.triangleCount);
		}
	}
}
