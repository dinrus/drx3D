...
//
//----------------------------------------------------------------------------------------------
CRecorderUnit::CRecorderUnit(CAIRecorder* pRecorder, CTimeValue startTime, CWeakRef<CAIObject> refUnit, u32 lifeIndex)
	: m_startTime(startTime)
	, m_pRecorder(pRecorder)
	, m_id(lifeIndex, refUnit.GetObjectID())
{
	DRX_ASSERT(refUnit.IsSet());
	DRX_ASSERT(m_pRecorder);
...
void CRecorderUnit::StreamBase::Seek(float whereTo)
{
	...
	{
		i32k iMaxIndex = max((i32)m_Stream.size() - 1, 0);
...
//
//----------------------------------------------------------------------------------------------
i32 CRecorderUnit::StreamBase::GetCurrentIdx()
{
	return m_CurIdx;
}
...
//
//----------------------------------------------------------------------------------------------
i32 CRecorderUnit::StreamBase::GetSize()
{
	return (i32)(m_Stream.size());
}
...
//
//----------------------------------------------------------------------------------------------
uk CRecorderUnit::StreamStr::GetCurrent(float& startingFrom)
{
	if (m_CurIdx < 0 || m_CurIdx >= (i32)m_Stream.size())
		return NULL;
	startingFrom = m_Stream[m_CurIdx]->m_StartTime;
	return (uk )(static_cast<StreamUnit*>(m_Stream[m_CurIdx])->m_String.c_str());
}
...
{
	sOut = (tuk)GetCurrent(startingFrom);
	return (!sOut.empty());
}

//
//----------------------------------------------------------------------------------------------
uk CRecorderUnit::StreamStr::GetNext(float& startingFrom)
{
	if (++m_CurIdx >= (i32)m_Stream.size())
	{
		m_CurIdx = (i32)m_Stream.size();
		return NULL;
	}
	startingFrom = m_Stream[m_CurIdx]->m_StartTime;
	return (uk )(static_cast<StreamUnit*>(m_Stream[m_CurIdx])->m_String.c_str());
...
bool CRecorderUnit::StreamStr::LoadStringIndex(FILE* pFile)
{
	...
	for (u32 uCount = 0; uCount < m_uIndexGen; ++uCount)
	{
		u32 uIndex = INVALID_INDEX;
...
bool CRecorderUnit::StreamStr::SaveStringIndex(FILE* pFile)
{
	...
	while (itIter != itIterEnd)
	{
		...
		u32k& uIndex(itIter->second);
		i32k strLen = (str ? str.size() : 0);
...
//
//----------------------------------------------------------------------------------------------
u32 CRecorderUnit::StreamStr::GetOrMakeStringIndex(tukk szString)
{
	u32 uResult = INVALID_INDEX;
...
//
//----------------------------------------------------------------------------------------------
bool CRecorderUnit::StreamStr::GetStringFromIndex(u32 uIndex, string& sOut) const
{
	DRX_ASSERT(m_bUseIndex);
...
//
//----------------------------------------------------------------------------------------------
bool CRecorderUnit::StreamStr::WriteValue(float t, tukk str, FILE* pFile)
{
	...
	if (m_bUseIndex)
	{
		u32 uIndex = GetOrMakeStringIndex(str);
		if (uIndex > INVALID_INDEX)
		{
			if (!fwrite(&uIndex, sizeof(u32), 1, pFile)) return false;
		}
	}
	else if (str)
	{
...
bool CRecorderUnit::StreamStr::LoadValue(float& t, string& name, FILE* pFile)
{
	...
	{
		u32 uIndex = INVALID_INDEX;
		if (!fread(&uIndex, sizeof(u32), 1, pFile)) return false;
...
//
//----------------------------------------------------------------------------------------------
uk CRecorderUnit::StreamVec3::GetCurrent(float& startingFrom)
{
	if (m_CurIdx < 0 || m_CurIdx >= (i32)m_Stream.size())
		return NULL;
	startingFrom = m_Stream[m_CurIdx]->m_StartTime;
	return (uk )(&static_cast<StreamUnit*>(m_Stream[m_CurIdx])->m_Pos);
}
...
bool CRecorderUnit::StreamVec3::GetCurrentString(string& sOut, float& startingFrom)
{
	...
	{
		const Vec3& v(*pVec);
		sOut = DrxStringUtils::toString(v);
	}
...
//
//----------------------------------------------------------------------------------------------
uk CRecorderUnit::StreamVec3::GetNext(float& startingFrom)
{
	if (++m_CurIdx >= (i32)m_Stream.size())
	{
...
//
//----------------------------------------------------------------------------------------------
uk CRecorderUnit::StreamFloat::GetCurrent(float& startingFrom)
{
	if (m_CurIdx < 0 || m_CurIdx >= (i32)m_Stream.size())
		return NULL;
	startingFrom = m_Stream[m_CurIdx]->m_StartTime;
	return (uk )(&static_cast<StreamUnit*>(m_Stream[m_CurIdx])->m_Val);
}
...
bool CRecorderUnit::StreamFloat::GetCurrentString(string& sOut, float& startingFrom)
{
	...
	if (pF)
	{
		sOut = DrxStringUtils::toString(*pF);
	}
...
//
//----------------------------------------------------------------------------------------------
uk CRecorderUnit::StreamFloat::GetNext(float& startingFrom)
{
	if (++m_CurIdx >= (i32)m_Stream.size())
	{
		m_CurIdx = (i32)m_Stream.size();
...
{
	{
		...
		i32 numStreams(stringIndexMap.size());
		...
		{
			StreamBase* pStream(strItr->second);
			i32k intType((strItr->first)); // The kind of stream
...
void CAIRecorder::GetCompleteFilename(char const* szFilename, bool bAppendFileCount, string& sOut) const
{
	...
	DRX_ASSERT(szFilename && szFilename[0]);
	...
	gEnv->pDrxPak->MakeDir(AIRECORDER_FOLDER);
...
//
//----------------------------------------------------------------------------------------------
void CAIRecorder::Start(EAIRecorderMode mode, tukk filename)
{
	if (IsRunning()) return;
	...
	case eAIRM_Disk:
		{
			...
			i32 newBufferSize = gAIEnv.CVars.DebugRecordBuffer;
...
//
//----------------------------------------------------------------------------------------------
void CAIRecorder::Stop(tukk filename)
{
	if (!IsRunning()) return;
...
//
//----------------------------------------------------------------------------------------------
bool CAIRecorder::Load(tukk filename)
{
...
bool CAIRecorder::Read(FILE* pFile)
{
	...
	// For each record
	for (i32 i = 0; i < fileHeader.unitsNumber; i++)
	{
		UnitHeader header;
		...
		gAIEnv.pAIObjectUpr->CreateDummyObject(refDummy, sDummyName);
		if (!refDummy.IsNil())
		{
	...
	do
	{
		...
		if (!bEndOfFile)
		{
			...
			{
				DRX_ASSERT_MESSAGE(false, "Recorder has streamded data. This code needs to be readded.");

				//i32 liveID = idMap[header.unitID];
				//if (!liveID)
				//{
				//	// New unannounced ID
				//	m_pLog->LogWarning("[AI Recorder] Unknown ID %d found in stream", header.unitID);
				//	// Generate a random ID - unlikely to collide.
				//	liveID = abs( (i32)drx_rand()%100000 ) + 10000;
...
//
//----------------------------------------------------------------------------------------------
bool CAIRecorder::Save(tukk filename)
{
	if (IsRunning()) return false; // To avoid undefined behaviour
...
bool CAIRecorder::Write(FILE* pFile)
{
	...
	for (TUnits::iterator unitIter = m_Units.begin(); unitIter != m_Units.end(); ++unitIter)
	{
		...
		UnitHeader header;
		header.nameLength = (i32)name.size();
...
CRecorderUnit* CAIRecorder::AddUnit(CWeakRef<CAIObject> refObject, bool force)
{
	...
	if ((IsRunning() || force) && refObject.IsSet())
	{
		...
		u32k lifeIndex = ++m_unitLifeCounter;
