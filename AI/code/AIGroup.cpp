...
template<class T>
bool SerializeWeakRefVector(TSerialize ser, tukk containerName, std::vector<CWeakRef<T>>& container)
{
				...
				if (pSS->GetGlobalValue("AICharacter", pGlobalCharacterTable))
				{
					...
					if (pGlobalCharacterTable->GetValue(sCharacter, pMyCharacterTable))
					{
						...
						if (pMyCharacterTable->GetValue("Class", soldierClass))
						{
							...
							UnitImg.SetClass(static_cast<i32>(soldierClass));
							return;
						}
					}
				}
...
// SetUnitProperties
//====================================================================
void CAIGroup::SetUnitProperties(const IAIObject* obj, u32 properties)
{
...
// GetAveragePosition
//====================================================================
Vec3 CAIGroup::GetAveragePosition(IAIGroup::eAvPositionMode mode, u32 unitClassMask) const
{
...
// GetUnitCount
//====================================================================
i32 CAIGroup::GetUnitCount(u32 unitPropMask) const
{
...
// GetTargetCount
//====================================================================
i32 CAIGroup::GetTargetCount(bool bHostileOnly, bool bLiveOnly) const
{
...
// GetForemostUnit
//====================================================================
CAIObject* CAIGroup::GetForemostUnit(const Vec3& direction, u32 unitProp) const
{
...
void CAIGroup::UpdateReinforcementLogic()
{
	...
	if (!m_reinforcementSpotsAcquired)
	{
		...
		for (AutoAIObjectIter it(gAIEnv.pAIObjectUpr->GetFirstAIObject(OBJFILTER_TYPE, AIANCHOR_REINFORCEMENT_SPOT)); it->GetObject(); it->Next())
		{
			...
			if (pAI->GetGroupId() == m_groupID && pAI->GetEntity())
			{
				...
				if (pTable)
				{
					...
					if (pTable->GetValue("Properties", props))
					{
						...
						i32 whenAllAlerted = 0;
						i32 whenInCombat = 0;
						i32 groupBodyCount = 0;
		...
		for (it = pAISystem->m_mapGroups.lower_bound(m_groupID); it != itEnd && it->first == m_groupID; ++it)
		{
			...
			if (pObject->IsEnabled() && pObject->GetType() != AIOBJECT_LEADER)
			{
				if (IEntity* pEntity = pObject->GetEntity())
					gAIEnv.pSmartObjectUpr->AddSmartObjectState(pEntity, "GroupHalved");
			}
		}
	}
}
...
// GetGroupCount
//====================================================================
i32 CAIGroup::GetGroupCount(i32 flags)
{
...
void CAIGroup::Serialize(TSerialize ser)
{
	...
	char Name[256];
	drx_sprintf(Name, "AIGroup-%d", m_groupID);
		...
		for (unsigned i = 0, n = m_reinforcementSpots.size(); i < n; ++i)
		{
			if (m_reinforcementSpots[i].pAI->IsEnabled())
			{
				...
				if (m_reinforcementState == REINF_NONE)
				{
					...
					{
						drx_sprintf(msg, "ALERTED: Alerted >= Enabled | %d >= %d", alertedCount, enabledCount);
						text += msg;
					...
					{
						if (enabledCount > 1)
							drx_sprintf(msg, "COMBAT: InCombat > 0 && LiveTarget > 1 | %d > 0 && %d > 1", combatCount, liveTargetCount);
						else
							drx_sprintf(msg, "COMBAT: InCombat > 0 && LiveTarget > 0 | %d > 0 && %d > 0", combatCount, liveTargetCount);
						text += msg;
				...
				if (m_reinforcementState == REINF_NONE /*|| m_reinforcementState == REINF_ALERTED_COMBAT*/)
				{
					...
					{
						drx_sprintf(msg, "SIZE: Bodies < Count | %d < %d",
			...
			i32 c = m_reinforcementSpots[i].type % 3;
			if (c == 0)
...
bool CAIGroup::EnterGroupScope(const CAIActor* pMember, const GroupScopeID scopeID, u32k allowedConcurrentUsers)
{
	TUnitList::iterator theUnit = std::find(m_Units.begin(), m_Units.end(), pMember);
...
u32 CAIGroup::GetAmountOfActorsInScope(const GroupScopeID scopeID) const
{
...
GroupScopeID CAIGroup::GetGroupScopeId(tukk scopeName)
{
