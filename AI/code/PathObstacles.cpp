...
static std::vector<i32> s_addedObstacleIndices;
...
u32 SPathObstacleShape2D::GetHash() const
{
	u32 hash = 0;
...
u32 SPathObstacleCircle2D::GetHash() const
{
	return HashFromVec3(center, 0.0f, 1.0f) + HashFromFloat(radius, 0.0f, 1.0f);
}

u32 CPathObstacle::GetHash() const
{
	u32 hash = HashFromFloat(float (m_type), 0.0f, 1.0f);
...
/*
   static u32 GetHash (const TPathObstacles & obstacles)
   {
   u32 hash = 0;
...
static bool ObstacleDrawingIsOnForActor(const CAIActor* pAIActor)
{
	...
	{
		tukk pathName = gAIEnv.CVars.DrawPathAdjustment;
...
i32 CPathObstacles::s_obstacleCacheSize = 32;
...
struct SCachedObstacle* CPathObstacles::GetNewCachedObstacle()
{
	if (s_obstacleCacheSize == 0)
		return 0;
	if ((i32)s_cachedObstacles.size() < s_obstacleCacheSize)
                                               ...
                                               float extraRadius, float terrainZ, const bool debug) const
{
	...
	static i32 cacheHits = 0;
	static i32 cacheMisses = 0;

	static i32 numPerOutput = 100;
...

	{
		...
		i32 nParts = entity->GetStatus(&statusNParts);
	...
	if (cachedObstacle)
	{
		...
		if (debug)
		{
			u32 origDebugBoxesSize = m_debugPathAdjustmentBoxes.size();
		...
		if (considerObject)
		{
			...
			if (altTop >= criticalTopAlt && altBase <= criticalBaseAlt)
			{
				...
				if (distToPath >= 0.0f && distToPath <= pathObstaclesInfo.maxPathDeviation)
				{
					...
					i32k actorType = pathObstaclesInfo.pAIActor ? pathObstaclesInfo.pAIActor->GetType() : AIOBJECT_ACTOR;
...
bool CPathObstacles::IsObstaclePushable(const CAIActor* pAIActor, IPhysicalEntity* pEntity, SPathObstaclesInfo& pathObstaclesInfo, float& outCullShapeScale) const
{
	...
	if (bIsPushable)
	{
		...
		else
		{
			...
			outCullShapeScale = (fPushableMassRange > FLT_EPSILON ? (fMass - fPushableMassMin) * __fres(fPushableMassRange) : 1.0f);
			bIsPushable = false;
		}
	}
                                      ...
                                      const CNavPath* pNavPath, const CAIActor* pAIActor)
{
	...
	i32k actorType = pAIActor ? pAIActor->GetType() : AIOBJECT_ACTOR;
	...
	{
		AutoAIObjectIter pVehicleIter(gAIEnv.pAIObjectUpr->GetFirstAIObject(OBJFILTER_TYPE, AIOBJECT_VEHICLE));
	...
	// Check dynamic physical entities
	{
		...
		u32k nEntityCount = GetEntitiesFromAABB(pEntities, obstacleAABB, AICE_DYNAMIC);
		for (u32 nEntity = 0; nEntity < nEntityCount; ++nEntity)
		{
