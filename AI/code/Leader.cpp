...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   Leader.cpp

   Описание: Class Implementation for CLeader
  ...
  void CLeader::Update(EUpdateType type)
{
	...
	if (m_pCurrentAction && (eUpdateResult = m_pCurrentAction->Update()) != CLeaderAction::ACTION_RUNNING)
	{
		...
		u32 unitProperties = m_pCurrentAction->GetUnitProperties();
		AbortExecution();
...
void CLeader::ProcessSignal(AISIGNAL& signal)
{
	...
	if (signal.Compare(gAIEnv.SignalCRCs.m_nAIORD_ATTACK))
	{
		...
		params.id = data.nID;
		params.pAIObject = gAIEnv.pAIObjectUpr->GetAIObjectByName(data.GetObjectName());
...
//
//----------------------------------------------------------------------------------------------------
void CLeader::AbortExecution(i32 priority)
{
...
//
//----------------------------------------------------------------------------------------------------
void CLeader::ClearAllPlannings(u32 unitProp, i32 priority)
{
	for (TUnitList::iterator unitItr = m_pGroup->GetUnits().begin(); unitItr != m_pGroup->GetUnits().end(); ++unitItr)
	{
...
//
//----------------------------------------------------------------------------------------------------
void CLeader::Search(const Vec3& targetPos, float searchDistance, u32 unitProperties, i32 searchAIObjectType)
{
...
//---------------------------------------------------------------------------------------
bool CLeader::LeaderCreateFormation(tukk szFormationDescr, const Vec3& vTargetPos, bool bIncludeLeader, u32 unitProp, CAIObject* pOwner)
{
	// (MATT) Note that this code seems to imply that pOwner must be an Actor - can the argument change? {2009/02/13}

	ReleaseFormation();
	i32 unitCount = m_pGroup->GetUnitCount(unitProp);
	if (unitCount == 0)
		return false;
...
CWeakRef<CAIObject> CLeader::GetNewFormationPoint(CWeakRef<CAIObject> refRequester, i32 iPointType)
{
	...
	if (pFormationOwner && pFormationOwner->m_pFormation)
	{
		if (iPointType <= 0)
		{
			i32 iFPIndex = GetFormationPointIndex(pRequester);
			if (iFPIndex >= 0)
...
i32 CLeader::GetFormationPointIndex(const CAIObject* pAIObject)  const
{
...
//-----------------------------------------------------------------------------------------------------------
i32 CLeader::AssignFormationPoints(bool bIncludeLeader, u32 unitProp)
{
	...
	i32 iFreePoints = pFormation->CountFreePoints();
	i32 iFormationSize = pFormation->GetSize();
	i32 iTeamSize = m_pGroup->GetUnitCount(unitProp); //m_pGroup->GetUnits().size();
	...
	for (i32 i = 1; i < iFormationSize; i++)
		AssignedUnit.push_back(itUnitEnd);

	CWeakRef<CAIObject> refAILeader = GetAssociation();
	i32 iAssignedCount = 0;

	for (i32 k = 0; k < 3; k++) // this index determines the type of test
	{
		for (i32 i = 1; i < iFormationSize; i++)
		{
	...
	for (itUnit = m_pGroup->GetUnits().begin(); itUnit != itUnitEnd; ++itUnit)
	{
		...
		//		float fMinDist = 987654321.f;
		for (i32 i = 1; i < itUnit->m_FormationPointIndex; i++)
		{
			CAIObject* pPoint = pFormation->GetFormationPoint(i);
			if ((unit.GetProperties() & unitProp) && unit.m_refUnit != m_refFormationOwner
			    && (unit.GetClass() & pFormation->GetPointClass(i)) && !pFormation->GetPointOwner(i))
			{
				...
				i32 currentIndex = itUnit->m_FormationPointIndex;
...
bool CLeader::AssignFormationPoint(CUnitImg& unit, i32 teamSize, bool bAnyClass, bool bClosestToOwner)
{
	CAIObject* const pFormationOwner = m_refFormationOwner.GetAIObject();
	...
	if (pFormationOwner && (pFormation = pFormationOwner->m_pFormation))
	{
		...
		{
			i32 myClass = unit.m_SoldierClass;
			if (myClass)
			{
...
void CLeader::AssignFormationPointIndex(CUnitImg& unit, i32 index)
{
...
//
//--------------------------------------------------------------------------------------------------------------
CLeaderAction* CLeader::CreateAction(const LeaderActionParams* params, tukk signalText)
{
...
void CLeader::AssignTargetToUnits(u32 unitProp, i32 maxUnitsPerTarget)
{

	CAIGroup::TSetAIObjects::iterator itTarget = m_pGroup->GetTargets().begin();
	std::vector<i32> AssignedUnits;

	i32 indexTarget = 0;
	const CAIObject* pTarget;
	i32 numUnits = m_pGroup->GetUnitCount(unitProp);
	i32 numtargets = m_pGroup->GetTargets().size();
	...
	i32 unitsPerTarget = numUnits / numtargets;
...
i32 CLeader::GetActiveUnitPlanCount(u32 unitProp) const
{
	i32 active = 0;
	for (TUnitList::const_iterator it = m_pGroup->GetUnits().begin(); it != m_pGroup->GetUnits().end(); ++it)
	{
