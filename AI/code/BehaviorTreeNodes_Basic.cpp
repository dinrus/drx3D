...
namespace BehaviorTree
{
	...
	public:
		...
		virtual void OnInitialize(const UpdateContext& context) override
		{
			...
			if (!coverId)
			{
				ErrorReporter(*this, context).LogError("Invalid input Cover id (= %u).", static_cast<u32>(coverId));
				runtimeData.pendingStatus = Failure;
				return;
			}
			...
			DRX_ASSERT(!runtimeData.movementRequestID);
	...
	{
		IBehaviorTreeUpr& pUpr = *gEnv->pAISystem->GetIBehaviorTreeUpr();

		REGISTER_BEHAVIOR_TREE_NODE_WITH_NAME(pUpr, Basic_MoveToPosition, "Basic_MoveToPosition");
		REGISTER_BEHAVIOR_TREE_NODE_WITH_NAME(pUpr, Basic_MoveToCover, "Basic_MoveToCover");
	}
