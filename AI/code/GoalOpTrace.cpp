...
bool COPTrace::ExecuteTrace(CPipeUser* pPipeUser, bool bFullUpdate)
{
	...
	if (m_prevFrameStartTime.GetValue() < 0ll)
	{
		...
		if (gAIEnv.pSmartObjectUpr->SmartObjectEvent("OnMove", pEntity, pNullEntity) != 0)
			return false; // Trace not finished
	}
...
void COPTrace::ExecuteManeuver(CPipeUser* pPipeUser, const Vec3& steerDir)
{
	...
	i32k maneuverTimeMinLimitMs = 300;
	i32k maneuverTimeMaxLimitMs = 5000;
	i32k manTimeMs = (i32)(m_Maneuver != eMV_None ? (now - m_ManeuverTime).GetMilliSecondsAsInt64() : 0);
  ...
  bool COPTrace::ExecutePathFollower(CPipeUser* pPipeUser, bool fullUpdate, IPathFollower* pPathFollower)
{
	...
	if (targetReachable)
	{
		...
		i32 num = min<i32>(predictedStates.size(), SAIPredictedCharacterStates::maxStates);
...
bool COPTrace::Execute2D(CPipeUser* const pPipeUser, bool fullUpdate)
{
	...
	if (stillTracingPath && distToEnd > .1f)
	{
		...
		const bool doManeuver = (gAIEnv.configuration.eCompatibilityMode != ECCM_DRXSIS2);
		if (doManeuver)
		...
		if (slopeModCoeff > 0)
		{
			...
			i32 valid = pPhysics->GetStatus(&status);
			if (valid)
			{
				if (!status.bFlying)
				{
...
bool COPTrace::ExecuteLanding(CPipeUser* pPipeUser, const Vec3& pathEnd)
{
	...
	stat.pHistory = 0;
	i32 collisions = pPipeUser->GetPhysics()->GetStatus(&stat);
	if (collisions > 0)
		return true;
	else
		return false;
	...
	case eATP_None:
		{
			...
			if (smartObject)
			{
				MNM::OffMeshLink* pOffMeshLink = gAIEnv.pNavigationSystem->GetOffMeshNavigationUpr()->GetOffMeshLink(pSmartObjectMNMData->offMeshLinkID);
				...
				if (pSOLink)
				{
					...
					else
					{
						...
						if (closestActor && (closestActor == pPipeUser->CastToCAIActor()))
						{
							...
							if (pSmartObjectUpr->PrepareNavigateSmartObject(pPipeUser, pSmartObject, pSOLink->m_pSmartObjectClass, pSOLink->m_pFromHelper, pSOLink->m_pToHelper) && pPipeUser->m_eNavSOMethod != nSOmNone)
							{
