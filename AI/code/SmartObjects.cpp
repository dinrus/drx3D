...
CSmartObjectUpr::SmartObjects CSmartObjectUpr::g_AllSmartObjects;
std::map<EntityId, CSmartObject*> CSmartObjectUpr::g_smartObjectEntityMap;
...
bool GetPelvisJointForRagdollizedActor(IEntity* pEntity, QuatT& pelvisJoint)
{
	...
	if (isArticulatedEntity && pEntity->HasAI())
	{
		...
		if (pCharInstance)
		{
			...
			i32 boneId = rIDefaultSkeleton.GetJointIDByName("Bip01 Pelvis");
			if (boneId >= 0)
			{
...
CSmartObject::CSmartObject(EntityId entityId)
	...
	, m_bHidden(false)
{
	m_fRandom = drx_random(0.0f, 0.5f);
...
{
	CSmartObjectUpr::g_AllSmartObjects.erase(this);
	UnregisterFromAllClasses();

	CSmartObjectUpr::RemoveSmartObjectFromEntity(m_entityId, this);
}
...
void CSmartObject::Register()
{
	...
	if (entityId > 0)
	{
		AIAssert(CSmartObjectUpr::g_AllSmartObjects.find(this) == CSmartObjectUpr::g_AllSmartObjects.end());
		CSmartObjectUpr::g_AllSmartObjects.insert(this);
		CSmartObjectUpr::BindSmartObjectToEntity(entityId, this);
	}
}
...
void CSmartObject::ApplyUserSize()
{
	...
	for (u32 i = 0; i < m_vClasses.size(); ++i)
	{
...
void CSmartObjectUpr::SerializePointer(TSerialize ser, tukk name, CSmartObject*& pSmartObject)
{
	if (ser.IsWriting())
	{
...
void CSmartObjectUpr::SerializePointer(TSerialize ser, tukk name, CCondition*& pRule)
{
	if (ser.IsWriting())
	{
		if (pRule)
	...
	else
	{
		...
		MapConditions& rules = gAIEnv.pSmartObjectUpr->m_Conditions;
...
void CSmartObject::Use(CSmartObject* pObject, CCondition* pCondition, i32 eventId /*=0*/, bool bForceHighPriority /*=false*/) const
{
	CAIActionUpr* pActionUpr = gAIEnv.pAIActionUpr;
	tukk sSignal = NULL;
	...
	if (!pCondition->sAction.empty() && pCondition->eActionType != eAT_None)
	{
		if (pCondition->eActionType == eAT_AISignal)
			...
			pAction = pActionUpr->GetAIAction(pCondition);
		...
		if (!pAction && !sSignal)
		{
			...
			if (!pCondition->userPreActionStates.empty())
				gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(GetEntity(), pCondition->userPreActionStates.AsUndoString());
			if (this != pObject)
				if (!pCondition->objectPreActionStates.empty())
					gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(pObject->GetEntity(), pCondition->objectPreActionStates.AsUndoString());
			return;
		}
	}

	i32 maxAlertness = pCondition->iMaxAlertness;
	...
	if (isAIAgent)
	{
		...
		if (sSignal)
		{
			...
			if (!pCondition->userPostActionStates.empty())   // check is next state non-empty
				gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(GetEntity(), pCondition->userPostActionStates.AsString());
			if (this != pObject)
				if (!pCondition->objectPostActionStates.empty())   // check is next state non-empty
					gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(pObjectEntity, pCondition->objectPostActionStates.AsString());
		...
		else
		{
			...
			if (eventId || (pAction && !pAction->GetFlowGraph()))
			{
				...
				pActionUpr->ExecuteAIAction(pAction, GetEntity(), pObject->GetEntity(), maxAlertness, eventId,
	...
	if (!pAIObject)
	{
		...
		{
			gAIEnv.pAIActionUpr->ExecuteAIAction(pAction, GetEntity(), pObject->GetEntity(), 100, eventId,
			                                         pCondition->userPostActionStates.AsString(), pCondition->objectPostActionStates.AsString(),
			                                         pCondition->userPreActionStates.AsUndoString(), pCondition->objectPreActionStates.AsUndoString());
		}
	}
...
CSmartObjectClass::CSmartObjectClass(tukk className)
	: m_sName(className)
...
CSmartObjectClass* CSmartObjectClass::find(tukk sClassName)
{
...
i32 CSmartObjectClass::FindHelperLinks(
  const SmartObjectHelper* from, const SmartObjectHelper* to, const CSmartObjectClass* pClass, float radius,
  CSmartObjectClass::HelperLink** pvHelperLinks, i32 iCount /*=1*/)
{
	i32 numFound = 0;
	i32 i = m_vHelperLinks.size();
...
i32 CSmartObjectClass::FindHelperLinks(
  ...
  CSmartObjectClass::HelperLink** pvHelperLinks, i32 iCount /*=1*/, const CSmartObject::SetStates* pObjectStatesToExcludeFromMatches /* = NULL*/)
{
	i32 numFound = 0;
	i32 i = m_vHelperLinks.size();
	while (i-- && numFound < iCount)
...
/////////////////////////////////////////
// CSmartObjectUpr class implementation
/////////////////////////////////////////

typedef std::list<string> ListTokens;

i32 Tokenize(tuk& str, i32& length)
{
	tuk token = str;
	i32 tokenLength = length;
...
void CSmartObjectUpr::String2Classes(const string& sClass, CSmartObjectClasses& vClasses)
{
	...
	AIAssert(i32(sClass.size()) < sizeof(classString) - 1);
	i32 stringLength = min(i32(sClass.size()), i32(sizeof(classString)) - 1);
	memcpy(classString, sClass.c_str(), stringLength);
	classString[stringLength] = 0;

	tuk token = classString;
	i32 tokenLength = 0;
...
void CSmartObjectUpr::String2States(tukk listStates, CSmartObject::DoubleVectorStates& states)
{
	...
	i32 inputStringLength = i32(strlen(listStates));
	i32 stringLength = min(inputStringLength, i32(sizeof(statesString)) - 1);
	...
	i32 hyphenPosition = -1;
	for (i32 i = 0; statesString[i]; ++i)
	{
		if (statesString[i] == '-')
		{
			hyphenPosition = i;
	...
	tuk positiveStateList = statesString;
	tuk negativeStateList = 0;
	if (hyphenPosition >= 0)
	{
	...
	tuk stateStrings[2] = { positiveStateList, negativeStateList };

	for (i32 stateVectorIndex = 0; stateVectorIndex < 2; ++stateVectorIndex)
	{
		tuk token = stateStrings[stateVectorIndex];
		i32 tokenLength = 0;
...
void CSmartObjectUpr::String2StatePattern(tukk sPattern, CSmartObject::CStatePattern& pattern)
{
	pattern.clear();
	string item;
...
CSmartObjectUpr::CSmartObjectUpr()
...
void CSmartObjectUpr::Serialize(TSerialize ser)
{
	if (ser.IsReading())
	{
		m_vDebugUse.clear();
	...
	if (ser.BeginOptionalGroup("SmartObjects", true))
	{
		if (ser.IsWriting())
		{
			i32 count = g_AllSmartObjects.size();
...
void CSmartObjectUpr::AddSmartObjectClass(CSmartObjectClass* soClass)
{
...
bool CSmartObjectUpr::ParseClassesFromProperties(const IEntity* pEntity, CSmartObjectClasses& vClasses)
{
	vClasses.clear();
	...
	{
		// ignore AISpawners
		tukk className = pEntity->GetClass()->GetName();
...
CSmartObjectClass* CSmartObjectUpr::GetSmartObjectClass(tukk className)
{
	AIAssert(className);
...
void CSmartObjectUpr::AddSmartObjectCondition(const SmartObjectCondition& conditionData)
{
	...
	if (!pUserClass)
	{
		AIWarning("WARNING: Smart Object class '%s' not found while trying to do CSmartObjectUpr::AddSmartObjectCondition",
	...
	else if (pCondition->bEnabled && pCondition->iRuleType == 0)
	{
		...
		for (i32 i = 0; i <= CLAMP(condition.iMaxAlertness, 0, 2); ++i)
			pUserClass->m_vActiveUpdateRules[i].push_back(pCondition);
	}
}

void CSmartObjectUpr::ClearConditions()
{
	Reset();
...
void CSmartObjectUpr::SoftReset()
{
	CClassTemplateData::m_pHelperMtl = NULL;
...
void CSmartObjectUpr::RecalculateUserSize()
{
...
float CSmartObjectUpr::CalculateDelayTime(CSmartObject* pUser, const Vec3& posUser,
...
i32 CSmartObjectUpr::TriggerEvent(tukk sEventName, IEntity*& pUser, IEntity*& pObject, QueryEventMap* pQueryEvents /*= NULL*/, const Vec3* pExtraPoint /*= NULL*/, bool bHighPriority /*= false*/)
{
	if (!IsInitialized())
		return 0;
	...
	for (itRules = pEvent->m_Conditions.begin(); itRules != itRulesEnd; ++itRules)
	{
		...
		for (itByPos = mapByPos.begin(); itByPos != itByPosEnd; ++itByPos)
		{
			...
			i32 alertness = pAIActor ? (pAIActor->IsEnabled() ? pAIActor->GetProxy()->GetAlertnessState() : 1000) : 0;
			...
			u8 factionID = IFactionMap::InvalidFactionID;
			i32 groupId = -1;
			...
			for (itObjectByPos = mapObjectByPos.lower_bound(bbMin.x); itObjectByPos != itObjectByPosEnd; ++itObjectByPos)
			{
				...
				if (pAIObjectObject)
				{
					...
					{
						// check same species
						u8 objectFactionID = pAIObjectObject->GetFactionID();
	...
	i32 id = GetAISystem()->AllocGoalPipeId();
...
i32 CSmartObjectUpr::TriggerEventUserObject(tukk sEventName, CSmartObject* pUser, CSmartObject* pObject, QueryEventMap* pQueryEvents, const Vec3* pExtraPoint, bool bHighPriority /*=false*/)
{
	float minDelay = FLT_MAX;
	CCondition* pMinRule = NULL;
	CEvent* pEvent = String2Event(sEventName);
...
i32 CSmartObjectUpr::TriggerEventUser(tukk sEventName, CSmartObject* pUser, QueryEventMap* pQueryEvents, IEntity** ppObjectEntity, const Vec3* pExtraPoint, bool bHighPriority /*=false*/)
{
	*ppObjectEntity = NULL;
...
i32 CSmartObjectUpr::TriggerEventObject(tukk sEventName, CSmartObject* pObject, QueryEventMap* pQueryEvents, IEntity** ppUserEntity, const Vec3* pExtraPoint, bool bHighPriority /*=false*/)
{
...
struct SOUpdateStats
{
	...
	i32  totalRules;
	i32  ignoredNotNeededRules;
	i32  ignoredAttTargetRules;
	i32  ignoredStatesNotMatchingRules;

	i32  appliedUserOnlyRules;
	i32  appliedUserObjectRules;

	i32  ignoredNotInRangeObjects;
	i32  ignoredAttTargetNotInRangeObjects;
	i32  ignoredStateDoesntMatchObjects;
...
void CSmartObjectUpr::Update()
{
	DRX_PROFILE_FUNCTION(PROFILE_AI)

	currentUpdateStats.reset();

	static i32 totalNumPairsToUpdate = 0;
	static i32 maxNumPairsUpdatedPerFrame = 100;
	i32 pairsUpdatedThisFrame = 0;
	       ...
	       CSmartObjectClass::g_itAllUserClasses != CSmartObjectClass::g_AllUserClasses.end())
	{
		...
		while (pairsUpdatedThisFrame < maxNumPairsUpdatedPerFrame)
		{
			...
			i32 numPairsProcessed = Process(pSmartObject, pClass);
...
i32 CSmartObjectUpr::Process(CSmartObject* pSmartObjectUser, CSmartObjectClass* pClass)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	IAIObject* pAttTarget = pAIActor ? pAIActor->GetAttentionTarget() : NULL;
	i32 alertness = pAIActor ? CLAMP(pAIActor->GetProxy()->GetAlertnessState(), 0, 2) : 0;
	...
	for (itConditions = activeRules.begin(); itConditions != itConditionsEnd; ++itConditions)
	{
		...
		for (; itByPos != mapObjectsByPos.end() && itByPos->first <= bbMax.x; ++itByPos)
		{
			...
			u8 objectFactionID = IFactionMap::InvalidFactionID;
	...
	i32 i = events.size(), use = -1, lookat = -1;
	while (i--)
	{
...
void CSmartObjectUpr::UseSmartObject(CSmartObject* pSmartObjectUser, CSmartObject* pSmartObject, CCondition* pCondition, i32 eventId /*=0*/, bool bForceHighPriority /*=false*/)
{
	AIAssert(pSmartObject);
	...
	pSmartObjectUser->m_fRandom = drx_random(0.0f, 0.5f);
	pSmartObject->m_fRandom = drx_random(0.0f, 0.5f);
...
bool CSmartObjectUpr::PrepareUseNavigationSmartObject(SAIActorTargetRequest& req, SNavSOStates& states, CSmartObject* pSmartObjectUser, CSmartObject* pSmartObject,
                                                          const CCondition* pCondition, SmartObjectHelper* pFromHelper, SmartObjectHelper* pToHelper /*, const Vec3 *pDestination*/)
{
	...
	if (pCondition->pObjectClass->m_pTemplateData && pToHelper->templateHelperIndex >= 0 &&
	    pToHelper->templateHelperIndex < (i32)pCondition->pObjectClass->m_pTemplateData->helpers.size())
	{
...
CSmartObject* CSmartObjectUpr::GetSmartObject(EntityId id)
{
	AIAssert(id);
...
void CSmartObjectUpr::BindSmartObjectToEntity(EntityId id, CSmartObject* pSO)
{
	AIAssert(pSO && id);
	if (!pSO || id == 0)
		return;
...
bool CSmartObjectUpr::RemoveSmartObjectFromEntity(EntityId id, CSmartObject* pSO /*= NULL*/)
{
	AIAssert(gEnv->pSystem->GetIEntitySystem()->GetEntity(id));
...
bool CSmartObjectUpr::CheckSmartObjectStates(const IEntity* pEntity, const CSmartObject::CStatePattern& pattern) const
{
...
bool CSmartObjectUpr::CheckSmartObjectStates(const IEntity* pEntity, tukk patternStates) const
{
	if (!IsInitialized())
		return false;
	AIAssert(pEntity);
...
void CSmartObjectUpr::ModifySmartObjectStates(IEntity* pEntity, const CSmartObject::DoubleVectorStates& states)
{
	CSmartObject* pSmartObject = GetSmartObject(pEntity->GetId());
...
void CSmartObjectUpr::ModifySmartObjectStates(CSmartObject* pSmartObject, const CSmartObject::DoubleVectorStates& states) const
{
...
void CSmartObjectUpr::ModifySmartObjectStates(IEntity* pEntity, tukk listStates)
{
	...
	CSmartObjectUpr::String2States(listStates, states);
	ModifySmartObjectStates(pEntity, states);
}
void CSmartObjectUpr::SetSmartObjectState(IEntity* pEntity, CSmartObject::CState state) const
{
	CSmartObject* pSmartObject = GetSmartObject(pEntity->GetId());
	if (pSmartObject)
...
void CSmartObjectUpr::SetSmartObjectState(CSmartObject* pSmartObject, CSmartObject::CState state) const
{
	pSmartObject->m_States.clear();
	pSmartObject->m_States.insert(state);
}

void CSmartObjectUpr::SetSmartObjectState(IEntity* pEntity, tukk sStateName)
{
	if (!GetAISystem()->IsEnabled())
		return;
...
void CSmartObjectUpr::AddSmartObjectState(IEntity* pEntity, CSmartObject::CState state) const
{
	CSmartObject* pSmartObject = GetSmartObject(pEntity->GetId());
	if (pSmartObject)
...
void CSmartObjectUpr::AddSmartObjectState(CSmartObject* pSmartObject, CSmartObject::CState state) const
{
...
void CSmartObjectUpr::AddSmartObjectState(IEntity* pEntity, tukk sState)
{
...
void CSmartObjectUpr::RemoveSmartObjectState(IEntity* pEntity, CSmartObject::CState state) const
{
	CSmartObject* pSmartObject = GetSmartObject(pEntity->GetId());
	if (pSmartObject)
...
void CSmartObjectUpr::RemoveSmartObjectState(CSmartObject* pSmartObject, CSmartObject::CState state) const
{
...
void CSmartObjectUpr::RemoveSmartObjectState(IEntity* pEntity, tukk sState)
{
	if (IsInitialized() && sState && *sState)
	{
		CSmartObject::CState state(sState);
...
void CSmartObjectUpr::DebugDraw()
{
...
void CSmartObjectUpr::RescanSOClasses(IEntity* pEntity)
{
	CSmartObject* pSmartObject = GetSmartObject(pEntity->GetId());
	CSmartObjectClasses vClasses;
	if (!ParseClassesFromProperties(pEntity, vClasses))
...
///////////////////////////////////////////////
void CSmartObjectUpr::OnSpawn(IEntity* pEntity, SEntitySpawnParams& params)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
void CSmartObjectUpr::RemoveEntity(IEntity* pEntity)
{
	DoRemove(pEntity);
}
...
// Implementation of IEntitySystemSink methods
///////////////////////////////////////////////
bool CSmartObjectUpr::OnRemove(IEntity* pEntity)
{
	DoRemove(pEntity);
	return true;
}

void CSmartObjectUpr::DoRemove(IEntity* pEntity, bool bDeleteSmartObject)
{
	// remove all actions in which this entity is participating
	gAIEnv.pAIActionUpr->OnEntityRemove(pEntity);
...
// Implementation of IEntitySystemSink methods
///////////////////////////////////////////////
void CSmartObjectUpr::OnReused(IEntity* pEntity, SEntitySpawnParams& params)
{
	assert(pEntity);
...
// Implementation of IEntitySystemSink methods
///////////////////////////////////////////////
void CSmartObjectUpr::OnEntityEvent(IEntity* pEntity, const SEntityEvent& event)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
				...
				if (newX == oldX)
				{
					if (gEnv->IsEditing() && (pSmartObject->GetAI() == NULL))
					{
						...
						for (it = classes.begin(); it != itEnd; ++it)
						{
							...
							gAIEnv.pNavigationSystem->GetOffMeshNavigationUpr()->RegisterSmartObject(pSmartObject, pSmartObjectClass);
						}
					}
				}
				else
...
void CSmartObjectUpr::RebuildNavigation()
{
...
bool CSmartObjectUpr::RegisterInNavigation(CSmartObject* pSmartObject, CSmartObjectClass* pClass)
{
	AIAssert(pSmartObject);
	if (!pSmartObject)
		return false;

	gAIEnv.pNavigationSystem->GetOffMeshNavigationUpr()->RegisterSmartObject(pSmartObject, pClass);
	return true;
}

void CSmartObjectUpr::UnregisterFromNavigation(CSmartObject* pSmartObject) const
{
	gAIEnv.pNavigationSystem->GetOffMeshNavigationUpr()->UnregisterSmartObjectForAllClasses(pSmartObject);
}

void CSmartObjectUpr::GetTemplateIStatObj(IEntity* pEntity, std::vector<IStatObj*>& statObjects)
{
	statObjects.clear();
...
bool CSmartObjectUpr::ValidateTemplate(IEntity* pEntity, bool bStaticOnly, IEntity* pUserEntity /*=NULL*/, i32 fromTemplateHelperIdx /*=-1*/, i32 toTemplateHelperIdx /*=-1*/)
{
	if (!IsInitialized())
		return false;
	CSmartObject* pSmartObject = GetSmartObject(pEntity->GetId());
...
bool CSmartObjectUpr::ValidateTemplate(CSmartObject* pSmartObject, bool bStaticOnly, IEntity* pUserEntity /*=NULL*/, i32 fromTemplateHelperIdx /*=-1*/, i32 toTemplateHelperIdx /*=-1*/)
{
...
void CSmartObjectUpr::DrawTemplate(IEntity* pEntity)
{
	CSmartObject* pSmartObject = GetSmartObject(pEntity->GetId());
	if (pSmartObject)
...
void CSmartObjectUpr::DrawTemplate(CSmartObject* pSmartObject, bool bStaticOnly)
{
	if (!IsInitialized())
		return;
	...
	const bool validMNMLink = checkAgainstMNM ?
	                          gAIEnv.pNavigationSystem->GetOffMeshNavigationUpr()->IsObjectLinkedWithNavigationMesh(pSmartObject->GetEntityId()) :
	                          false;
	...
	i32k colorMNMIndex = validMNMLink ? 6 : 2;
...
void CSmartObjectUpr::LoadSOClassTemplates()
{
...
void CSmartObjectUpr::RemoveSmartObject(IEntity* pEntity)
{
	if (!IsInitialized())
		return;
...
tukk CSmartObjectUpr::GetSmartObjectStateName(i32 id) const
{
	AIAssert(IsInitialized());
	return CSmartObject::CState::GetStateName(id);
}

void CSmartObjectUpr::RegisterSmartObjectState(tukk sStateName)
{
	if (*sStateName)
	{
...
void CSmartObjectUpr::DrawSOClassTemplate(IEntity* pEntity)
{
	//TODO get rid of forwarding
	DrawTemplate(pEntity);
}

bool CSmartObjectUpr::ValidateSOClassTemplate(IEntity* pEntity)
{
	CSmartObject* pSmartObject = GetSmartObject(pEntity->GetId());
	if (pSmartObject)
...
u32 CSmartObjectUpr::GetSOClassTemplateIStatObj(IEntity* pEntity, IStatObjPtr* ppStatObjectPtrs, u32& numAllocStatObjects)
{
	if (!IsInitialized())
		return 0;
	...
	if (!statObjects.empty())
	{
		...
		{
			DRX_ASSERT_MESSAGE(ppStatObjectPtrs, "You must allocate the ppStatObjectPtrs in order to populate it!");
			DRX_ASSERT_MESSAGE(numAllocStatObjects, "You passed 0 as the number of allocated objects in the array you allocated!");

			numToCopy = min(numAllocStatObjects, (u32)statObjects.size());
			if (numToCopy)
			{
...
void CSmartObjectUpr::ReloadSmartObjectRules()
{
...
i32 CSmartObjectUpr::SmartObjectEvent(tukk sEventName, IEntity*& pUser, IEntity*& pObject, const Vec3* pExtraPoint /*= NULL*/, bool bHighPriority /*= false */)
{
...
SmartObjectHelper* CSmartObjectUpr::GetSmartObjectHelper(tukk className, tukk helperName) const
{
	AIAssert(IsInitialized());
	CSmartObjectClass* pClass = CSmartObjectClass::find(className);
	if (pClass)
...
bool CSmartObjectUpr::LoadSmartObjectsLibrary()
{
	m_initialized = true;
	LoadSOClassTemplates();

	char szPath[512];
	drx_sprintf(szPath, "%s", SMART_OBJECTS_XML);
...
void CSmartObjectUpr::DebugDrawValidateSmartObjectArea() const
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
bool CSmartObjectUpr::GetSmartObjectLinkCostFactorForMNM(const OffMeshLink_SmartObject* pSmartObjectLink, const IEntity* pRequesterEntity, float* fCostMultiplier) const
{
	...
	if (pRequesterPipeUser && pSmartObjectLink && IsInitialized())
	{
		...
		if (pObject->GetValidationResult() != eSOV_InvalidStatic)
		{
			if (m_bannedNavSmartObjects.find(pObject) == m_bannedNavSmartObjects.end())
			{
				...
				{
					DRX_PROFILE_REGION(PROFILE_AI, "CSmartObjectUpr::GetSmartObjectLinkCostFactorForMNM::CostMultiplierCalculation");
					if (fCostMultiplier)
					{
...
i32 CSmartObjectUpr::GetNavigationalSmartObjectActionTypeForMNM(CPipeUser* pPipeUser, CSmartObject* pSmartObject, CSmartObjectClass* pSmartObjectClass, SmartObjectHelper* pFromHelper, SmartObjectHelper* pToHelper)
{
	CSmartObject* pUser = gAIEnv.pSmartObjectUpr->GetSmartObject(pPipeUser->GetEntityID());
	if (!pUser)
		return nSOmNone;
...
void CSmartObjectUpr::DebugDrawBannedNavsos()
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
bool CSmartObjectUpr::PrepareNavigateSmartObject(CPipeUser* pPipeUser, CSmartObject* pObject, CSmartObjectClass* pObjectClass, SmartObjectHelper* pFromHelper, SmartObjectHelper* pToHelper)
{
	...
	u32 numVariations = FindSmartObjectLinks(pPipeUser, pObject, pObjectClass, pFromHelper, pToHelper, vHelperLinks, 8);
	...
	std::swap(vHelperLinks[0], vHelperLinks[drx_random(0U, numVariations - 1)]);
	...
	if (gAIEnv.pSmartObjectUpr->PrepareUseNavigationSmartObject(pPipeUser->m_State.actorTargetReq, pPipeUser->m_pendingNavSOStates, pUser, pObject, vHelperLinks[0]->condition, pFromHelper, pToHelper))
	{
...
bool CSmartObjectUpr::IsSmartObjectBusy(const CSmartObject* pSmartObject) const
{
	const CSmartObject::SetStates& states = pSmartObject->GetStates();
	return (states.find(m_StateBusy) != states.end());
}

void CSmartObjectUpr::MapPathTypeToSoUser(const string& soUser, const string& pathType)
{
	m_MappingSOUserPathType[soUser] = pathType;
}

bool CSmartObjectUpr::ValidateSmartObjectArea(const SSOTemplateArea& templateArea, const SSOUser& user, EAICollisionEntities collEntities, Vec3& groundPos)
{
...
i32 CSmartObjectUpr::FindSmartObjectLinks(const CPipeUser* pPipeUser, CSmartObject* pObject, CSmartObjectClass* pObjectClass, SmartObjectHelper* pFromHelper, SmartObjectHelper* pToHelper, CSmartObjectClass::HelperLink** pvHelperLinks, i32 iCount /*= 1*/, const IEntity* pAdditionalEntity /*= NULL */, const CSmartObject::SetStates* pStatesExcludedFromMatchingTheObjectState /*= NULL*/) const
{
	AIAssert(IsInitialized());
		...
		if (pEntity)
		{
			...
			if (pUser)
			{
				...
				for (itClasses = vClasses.begin(); iCount && (itClasses != itClassesEnd); ++itClasses)
				{
					...
					{
						i32 count = pObjectClass->FindHelperLinks(pFromHelper,
						                                          pToHelper,
						                                          pClass,
						                                          1.0f,
						                                          userSetStates,
...
const AgentPathfindingProperties* CSmartObjectUpr::GetPFPropertiesOfSoUser(const string& soUser)
{
...
void CSmartObjectUpr::UpdateBannedSOs(float frameDeltaTime)
{
	// Update banned SOs
...
void CSmartObjectUpr::ResetBannedSOs()
{
	m_bannedNavSmartObjects.clear();
}

bool CSmartObjectUpr::IsInitialized() const
{
	return m_initialized;
}

float CSmartObjectUpr::CalculateDot(CCondition* condition, const Vec3& dir, CSmartObject* user) const
{
...
Vec3 CSmartObject::MeasureUserSize() const
{
	...
	for (i32 i = (STANCE_NULL + 1); i < (STANCE_LAST - 1); ++i)
	{
		SAIBodyInfo bi;
...
void CQueryEvent::Serialize(TSerialize ser)
{
	...
	CSmartObjectUpr::SerializePointer(ser, "pUser", pUser);
	CSmartObjectUpr::SerializePointer(ser, "pObject", pObject);
	CSmartObjectUpr::SerializePointer(ser, "pRule", pRule);
