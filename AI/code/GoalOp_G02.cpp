...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   GoalOp_G02.cpp
   Описание: Game 02 goalops
...
IGoalOp* CGoalOpFactoryG02::GetGoalOp(tukk sGoalOpName, IFunctionHandler* pH, i32 nFirstParam, GoalParameters& params) const
{
...
// COPSeekCover
//====================================================================
COPSeekCover::COPSeekCover(bool uncover, float radius, float minRadius, i32 iterations, bool useLastOpAsBackup, bool towardsLastOpResult) :
	m_uncover(uncover),
	m_radius(radius),
...
// CalcStrafeCoverTree
//====================================================================
void COPSeekCover::CalcStrafeCoverTree(SAIStrafeCoverPos& pos, i32 i, i32 j,
                                       ...
                                       const std::vector<Vec3FloatPair>& avoidPos, const std::vector<Vec3FloatPair>& softCoverPos)
{
	...
	{
		float u = Ease((float)(i + 1) / (float)m_iterations) * radius * drx_random(0.9f, 1.0f);
		float v = ((float)(b + k) - ((n - 1) / 2.0f));
		float a = v / (float)n * gf_PI * drx_random(0.75f, 0.80f);
		float x = cosf(a) * u;
		float y = sinf(a) * u;
...
EGoalOpResult COPSeekCover::Execute(CPipeUser* pOperand)
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_AI, "SeekCover/CalculatePathTree");
...
void COPSeekCover::DrawStrafeCoverTree(SAIStrafeCoverPos& pos, const Vec3& target) const
{
	...
	for (unsigned i = 0, ni = pos.child.size(); i < ni; ++i)
	{
		u8 a = pos.child[i].valid ? 255 : 120;
