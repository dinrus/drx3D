...
EAIFireState CFireCommandInstant::Update(IAIObject* pITarget, bool canFire, EFireMode fireMode, const AIWeaponDescriptor& descriptor, Vec3& outShootTargetPos)
{
	...
	if (m_drawFire.GetState() != DrawFireEffect::Finished)
	{
		...
		const bool canFakeHit = drx_random(0.0f, 1.0f) <= FakeHitChance;
	...
	i32k burstBulletCountMin = descriptor.burstBulletCountMin;
	i32k burstBulletCountMax = descriptor.burstBulletCountMax;
	...
	if (singleFireMode)
	{
		...
		if (m_weaponTriggerTime <= 0.0f)
		{
			...
			m_weaponTriggerTime += singleFireTriggerTime * drx_random(0.75f, 1.25f);
		}
	}
	else
	{
	    ...
	    fireMode == FIREMODE_BURST_WHILE_MOVING || fireMode == FIREMODE_BURST_SNIPE)
	{
		...
		m_curBurstBulletCount = (i32)(burstBulletCountMin + (burstBulletCountMax - burstBulletCountMin) * a * m_weaponBurstTimeScale);
		...
		if (m_weaponBurstState == BURST_NONE)
		{
			// Init
			m_weaponBurstTime = 0.0f;
			m_weaponBurstTimeScale = drx_random(1, 6) / 6.0f;
		...
		{
			i32 shotCount = m_pShooter->GetProxy()->GetAndResetShotBulletCount();
		...
		if (bGetBestLob)
		{
			...
			if (bValidAim)
			{
				...
				if (state != eAIFS_Blocking)
				{
					...
					const float fNextFireTime = drx_random(descriptor.burstPauseTimeMin, descriptor.burstPauseTimeMax);
...
EAIFireState CFireCommandLob::GetBestLob(IAIObject* pTarget, const AIWeaponDescriptor& descriptor, Vec3& outShootTargetPos)
{
	...
	if (0 == m_lastStep)
	{
		...
		const float randomPercentageForPerpendicularRadius = gAIEnv.CVars.LobThrowSimulateRandomInaccuracy ? drx_random(0.0f, 1.0f) : 0.0f;
		float perpendicularRadius = -inaccuracyRadius + randomPercentageForPerpendicularRadius * (2 * inaccuracyRadius);

		const float randomPercentageForParallelRadius = gAIEnv.CVars.LobThrowSimulateRandomInaccuracy ? drx_random(0.0f, 1.0f) : 0.0f;
                                         ...
                                         const float minimumDistanceToFriendsSq) const
{
	...
	if (eRGT_SMOKE_GRENADE != eReqGrenadeType)
	{
		...
		AutoAIObjectIter itAnchors(gAIEnv.pAIObjectUpr->GetFirstAIObject(OBJFILTER_TYPE, AIANCHOR_NOGRENADE_SPOT));
		...
		AutoAIObjectIter itFriend(gAIEnv.pAIObjectUpr->GetFirstAIObject(OBJFILTER_FACTION, m_pShooter->GetFactionID()));
...
bool CFireCommandProjectileFast::ChooseShootPoint(Vec3& outShootPoint, IAIObject* pTarget, float explRadius, float missRatio, CFireCommandProjectileFast::EAimMode aimMode)
{
	...
	else
	{
		...
		if (drx_random(0.0f, 1.0f) <= fAccuracy)
			aimMode = AIM_DIRECTHIT;
	}
	...
	else
	{
		...
		case AIM_INFRONT:
			{
				const float leftRightOffset = drx_random(-0.2f, 0.2f);
		...
		case AIM_SIDES:
			{
				const float leftRightOffset = drx_random(0.5f, 1.0f) * (drx_random(0, 1) == 0 ? 0.4f : -0.4f);
		...
		case AIM_BACK:
			{
				const float leftRightOffset = drx_random(0.5f, 1.0f) * (drx_random(0, 1) == 0 ? 0.1f : -0.1f);
				shootAtPos += -targetForward2D * shotMissOffset + shootDir * shotMissOffset * leftRightOffset;
			}
			break;

		default:
			DRX_ASSERT_MESSAGE(false, "CFireCommandProjectileFast::ChooseShootPoint Unhandled Aim Mode");
		}
	}
...
bool CFireCommandStrafing::ManageFireDirStrafing(IAIObject* pTarget, Vec3& vTargetPos, bool& fire, const AIWeaponDescriptor& descriptor)
{
	...
	i32 maxStrafing = (i32)params.m_fStrafingPitch;
	if (maxStrafing == 0)
		maxStrafing = 1;
...
EAIFireState CFireCommandFastLightMOAR::Update(IAIObject* pTarget, bool canFire, EFireMode fireMode, const AIWeaponDescriptor& descriptor, Vec3& outShootTargetPos)
{
	...
	if (canFire && !m_startFiring)
	{
		...
		m_rand.set(drx_random(-1.0f, 1.0f), drx_random(-1.0f, 1.0f));
	}
...
bool CFireCommandHurricane::SelectFireDirNormalFire(CAIObject* pTarget, float fAccuracyRamping, Vec3& outShootTargetPos)
{
	...
	bool bAccuracyMiss = drx_random(0.f, 100.f) > (shooterCurrentAccuracy * 100.f);
	...
	else
	{
		...
		if (!lowDamage)
			lowDamage = drx_random(0.f, 100.f) > (shooterCurrentAccuracy * 50.f);  // decide if want to hit legs/arms (low damage)
