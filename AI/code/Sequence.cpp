...
{
	DRX_ASSERT_MESSAGE(actionNodeId != InvalidFlowNodeId, "Sequence::RequestActionStart: clash between passed in actionNodeId and the magic value that specifies an invalid node");
...
EntityId Sequence::GetEntityIdFromNode(TFlowNodeId nodeId) const
{
	...
	if (entityId == (EntityId)0)
	{
		...
		{
			i32k entityIndex = EFLOWNODE_ENTITY_ID_GRAPH1 ? 0 : 1;
