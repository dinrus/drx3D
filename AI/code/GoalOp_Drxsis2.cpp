...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла: GoalOp_Drxsis2.cpp
   Описание: Drxsis2 goalops
...
#include <drx3D/AI/GoalOpTrace.h>
...
//
//-------------------------------------------------------------------------------------------------------------
IGoalOp* CGoalOpFactoryDrxsis2::GetGoalOp(tukk sGoalOpName, IFunctionHandler* pH, i32 firstParam, GoalParameters& params) const
{
...
//
//-------------------------------------------------------------------------------------------------------------
IGoalOp* CGoalOpFactoryDrxsis2::GetGoalOp(EGoalOperations op, GoalParameters& params) const
{
	...
	case eGO_ADJUSTAIM:
		{
			pResult = new COPDrxsis2AdjustAim((params.nValue & 0x02) != 0, (params.nValue & 0x04) != 0, params.fValue);
		}
		break;
	case eGO_PEEK:
		{
			pResult = new COPDrxsis2Peek((params.nValue & 0x02) != 0, params.fValue);
		}
		break;
	case eGO_HIDE:
		{
			pResult = new COPDrxsis2Hide(static_cast<EAIRegister>(params.nValue), params.bValue);
		}
		break;
	case eGO_COMMUNICATE:
		{
			pResult = new COPDrxsis2Communicate(CommID(params.nValue), CommChannelID(params.nValueAux), params.fValue, static_cast<EAIRegister>(i32(params.fValueAux)),
	...
	case eGO_STICKPATH:
		{
			...
			pResult = new COPDrxsis2StickPath((params.nValue & 0x02) != 0, params.vPos.y, params.vPos.x, (params.nValue & 0x01) != 0);
		}
		break;
	case eGO_HOVER:
		{
			pResult = new COPDrxsis2Hover(static_cast<EAIRegister>(params.nValue), params.bValue);
		}
		break;
	case eGO_FLY:
		{
			pResult = new COPDrxsis2Fly();
		}
		break;
	...
	case eGO_CHASETARGET:
		{
			pResult = new COPDrxsis2ChaseTarget();
		}
		break;
	case eGO_FIREWEAPONS:
		{
			pResult = new COPDrxsis2FlightFireWeapons(static_cast<EAIRegister>(params.nValue), params.fValue, params.fValueAux, params.bValue, static_cast<u32>(params.nValueAux));
		}
		break;
	default:
		return NULL;
...
//
//-------------------------------------------------------------------------------------------------------------
COPDrxsis2AdjustAim::COPDrxsis2AdjustAim(bool useLastOpAsBackup, bool allowProne, float timeout) :
	m_useLastOpAsBackup(useLastOpAsBackup),
	m_allowProne(allowProne),
	m_timeoutMs((i32)(timeout * 1000.0f)),
...
//
//-------------------------------------------------------------------------------------------------------------
COPDrxsis2AdjustAim::COPDrxsis2AdjustAim(const XmlNodeRef& node) :
...
//
//-------------------------------------------------------------------------------------------------------------
i32 COPDrxsis2AdjustAim::RandomizeTimeInterval() const
{
	return drx_random(250, 500);
}

//
//-------------------------------------------------------------------------------------------------------------
EGoalOpResult COPDrxsis2AdjustAim::Execute(CPipeUser* pPipeUser)
{
	CCCPOINT(COPDrxsis2AdjustAim_Execute);
	...
	if (!m_startTime.GetValue())
	{
		...
		if (m_timeoutRandomness > 0.0001f)
			m_runningTimeoutMs += (i32)drx_random(0.0f, m_timeoutRandomness * 1000.0f);
	...
	{
		PostureUpr::PostureID postureID = -1;
		PostureUpr::PostureInfo* posture;

		PostureUpr& postureUpr = pPuppet->GetPostureUpr();

		if (!m_queryID)
		{
			u32 checks = PostureUpr::DefaultChecks;
			if (pPuppet->GetFireMode() == FIREMODE_OFF)
				checks &= ~PostureUpr::CheckAimability;

			PostureUpr::PostureQuery query;
			query.actor = pPuppet;
			...
			query.target = target;
			query.type = PostureUpr::AimPosture;

			m_queryID = postureUpr.QueryPosture(query);
			if (!m_queryID)
			{
		...
		{
			AsyncState queryStatus = postureUpr.GetPostureQueryResult(m_queryID, &m_bestPostureID, &posture);
	...
	if (!finished)
	{
		i32 timeToNextShotMs = (i32)(pPuppet->GetTimeToNextShot() * 1000.0f);
...
bool COPDrxsis2AdjustAim::ProcessQueryResult(CPipeUser* pipeUser, AsyncState queryStatus,
                                             PostureUpr::PostureInfo* postureInfo)
{
	DRX_ASSERT(pipeUser);
	...
	if (queryStatus != AsyncFailed)
	{
		...
		if (coverAction)
		{
			tukk const coverActionName = postureInfo->agInput.c_str();
...
//
//-------------------------------------------------------------------------------------------------------------
void COPDrxsis2AdjustAim::Reset(CPipeUser* pPipeUser)
{
	...
	if (m_queryID)
	{
		pPipeUser->CastToCPuppet()->GetPostureUpr().CancelPostureQuery(m_queryID);
		m_queryID = 0;
	}
...
//
//-------------------------------------------------------------------------------------------------------------
void COPDrxsis2AdjustAim::Serialize(TSerialize ser)
{
	ser.BeginGroup("COPDrxsis2AdjustAim");
	{
...
//
//-------------------------------------------------------------------------------------------------------------
void COPDrxsis2AdjustAim::DebugDraw(CPipeUser* pPipeUser) const
{
	if (!pPipeUser)
		return;
	...
	{
		PostureUpr::PostureInfo posture;
		pPuppet->GetPostureUpr().GetPosture(m_bestPostureID, &posture);
...
//
//-------------------------------------------------------------------------------------------------------------
COPDrxsis2Peek::COPDrxsis2Peek(bool useLastOpAsBackup, float timeout) :
	m_useLastOpAsBackup(useLastOpAsBackup),
	m_timeoutMs((i32)(timeout * 1000.0f)),
	m_timeoutRandomness(0.0f),
	m_bestPostureID(-1),
...
//
//-------------------------------------------------------------------------------------------------------------
COPDrxsis2Peek::COPDrxsis2Peek(const XmlNodeRef& node) :
	m_useLastOpAsBackup(s_xml.GetBool(node, "useLastOpResultAsBackup")),
	m_timeoutRandomness(0.0f),
	m_timeoutMs(0),
	m_bestPostureID(-1),
...
//
//-------------------------------------------------------------------------------------------------------------
i32 COPDrxsis2Peek::RandomizeTimeInterval() const
{
	return drx_random(250, 500);
}
...
//
//-------------------------------------------------------------------------------------------------------------
EGoalOpResult COPDrxsis2Peek::Execute(CPipeUser* pPipeUser)
{
	CCCPOINT(COPDrxsis2Peek_Execute);
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	if (elapsedMs >= m_nextUpdateMs)
	{
		...
		if (!m_queryID)
		{
			u32 checks = PostureUpr::CheckVisibility | PostureUpr::CheckLeanability;
			...
			query.position = pPuppet->GetPhysicsPos();
			query.target = target;
			query.type = PostureUpr::PeekPosture;
...
bool COPDrxsis2Peek::ProcessQueryResult(CPipeUser* pipeUser, AsyncState queryStatus,
                                        PostureUpr::PostureInfo* postureInfo)
{
	assert(pipeUser);
...
//
//-------------------------------------------------------------------------------------------------------------
void COPDrxsis2Peek::Reset(CPipeUser* pPipeUser)
{
	pPipeUser->m_State.lean = 0.0f;
	pPipeUser->m_State.peekOver = 0.0f;
	pPipeUser->m_State.coverRequest.ClearCoverAction();
...
//
//-------------------------------------------------------------------------------------------------------------
void COPDrxsis2Peek::Serialize(TSerialize ser)
{
	ser.BeginGroup("COPDrxsis2Peek");
	{
		ser.Value("m_startTime", m_startTime);
		ser.Value("m_nextUpdateMs", m_nextUpdateMs);
		ser.Value("m_useLastOpAsBackup", m_useLastOpAsBackup);
...
//
//-------------------------------------------------------------------------------------------------------------
void COPDrxsis2Peek::DebugDraw(CPipeUser* pPipeUser) const
{
	if (!pPipeUser)
		return;
...
COPDrxsis2Hide::COPDrxsis2Hide(EAIRegister location, bool exact)
	: m_location(location)
	, m_exact(exact)
	, m_pPathfinder(0)
	, m_pTracer(0)
...
COPDrxsis2Hide::COPDrxsis2Hide(const XmlNodeRef& node) :
...
COPDrxsis2Hide::COPDrxsis2Hide(const COPDrxsis2Hide& rhs) :
...
EGoalOpResult COPDrxsis2Hide::Execute(CPipeUser* pPipeUser)
{
	CCCPOINT(COPDrxsis2Hide_Execute);

	if (m_refHideTarget.IsNil())
	{
		CCCPOINT(COPDrxsis2Hide_Execute_NoHideTarget);

		pPipeUser->ClearPath("COPDrxsis2Hide::Execute");

		if (m_location == AI_REG_REFPOINT)
		{
			CCCPOINT(COPDrxsis2Hide_Execute_RefPoint);
		...
		{
			CCCPOINT(COPDrxsis2Hide_Execute_HideObject);
	...
	if (!m_refHideTarget.IsNil())
	{
		if (!m_pPathfinder)
		{
			if (gAIEnv.CVars.DebugPathFinding)
			{
				...
				AILogAlways("COPDrxsis2Hide::Execute %s pathfinding to (%5.2f, %5.2f, %5.2f)", pPipeUser->GetName(),
		...
		if (!m_pTracer && (m_pPathfinder->Execute(pPipeUser) != eGOR_IN_PROGRESS))
		{
			if (pPipeUser->m_nPathDecision == PATHFINDER_PATHFOUND)
			{
				...
				{
					const Vec3& vPos = m_refHideTarget->GetPos();
					AILogAlways("COPDrxsis2Hide::Execute %s Creating trace to hide target (%5.2f, %5.2f, %5.2f)", pPipeUser->GetName(),
					            vPos.x, vPos.y, vPos.z);
				}
			...
			{
				CCCPOINT(COPDrxsis2Hide_Execute_Unreachable);
		...
		if (m_pTracer)
		{
			...
			{
				CCCPOINT(COPDrxsis2Hide_Execute_A);
			...
			{
				CCCPOINT(COPDrxsis2Hide_Execute_B);
...
void COPDrxsis2Hide::ExecuteDry(CPipeUser* pPipeUser)
{
	CCCPOINT(COPDrxsis2Hide_ExecuteDry);

	if (m_pTracer)
		m_pTracer->ExecuteTrace(pPipeUser, false);
}

void COPDrxsis2Hide::Reset(CPipeUser* pPipeUser)
{
	CCCPOINT(COPDrxsis2Hide_Reset);
	...
	if (pPipeUser)
	{
		...
		pPipeUser->ClearPath("COPDrxsis2Hide::Reset");
...
void COPDrxsis2Hide::Serialize(TSerialize ser)
{
	ser.BeginGroup("COPDrxsis2Hide");
	{
...
void COPDrxsis2Hide::CreateHideTarget(CPipeUser* pPipeUser, const Vec3& pos, const Vec3& dir)
{
	CCCPOINT(COPDrxsis2Hide_CreateHideTarget);

	assert(m_refHideTarget.IsNil() || !m_refHideTarget.GetAIObject());

	if (m_refHideTarget.IsNil() || !m_refHideTarget.GetAIObject())
		gAIEnv.pAIObjectUpr->CreateDummyObject(m_refHideTarget);
...
void COPDrxsis2Hide::DebugDraw(CPipeUser* pPipeUser) const
{
	if (!m_refHideTarget.IsNil())
	{
		CDebugDrawContext dc;

		dc->DrawCylinder(m_refHideTarget.GetAIObject()->GetPos() + Vec3Constants<float>::fVec3_OneZ * 0.15f * 0.5f,
		                 Vec3Constants<float>::fVec3_OneZ, pPipeUser->GetParameters().m_fPassRadius, 0.15f, Col_DarkGreen);
	}
}

void COPDrxsis2Hide::UpdateMovingToCoverAnimation(CPipeUser* pPipeUser) const
{
	...
	if (pPipeUser->IsMovingToCover() && !pPipeUser->IsInCover()) // GoalOpHide is used both when moving into cover and *in* cover..
	{
		...
		if (hideHeight < LowCoverMaxHeight) // only slides into low cover are supported
		{
			...
			COPDrxsis2Utils::TurnBodyToTargetInCover(pPipeUser, hidePos, hideNormal, pPipeUser->GetPhysicsPos());
...
COPDrxsis2Communicate::COPDrxsis2Communicate(
...
COPDrxsis2Communicate::COPDrxsis2Communicate(const XmlNodeRef& node) :
	m_expirity(0.f),
	m_target(AI_REG_NONE)
{
	tukk szName = s_xml.GetMandatoryString(node, "name");
	CCommunicationUpr* pCommunicationUpr = gAIEnv.pCommunicationUpr;
	m_commID = pCommunicationUpr->GetCommunicationID(szName);
	m_channelID = pCommunicationUpr->GetChannelID(szName);
...
EGoalOpResult COPDrxsis2Communicate::Execute(CPipeUser* pPipeUser)
{
	SCommunicationRequest request;
	request.actorID = pPipeUser->GetEntityID();
	request.commID = m_commID;
	request.channelID = m_channelID;
	request.configID = gAIEnv.pCommunicationUpr->GetConfigID(pPipeUser->GetProxy()->GetCommunicationConfigName());
	...
	gAIEnv.pCommunicationUpr->PlayCommunication(request);

	return eGOR_DONE;
}

//////////////////////////////////////////////////////////////////////////
COPDrxsis2StickPath::COPDrxsis2StickPath(bool bFinishInRange, float fMaxStickDist, float fMinStickDist, bool bCanReverse)
...
//////////////////////////////////////////////////////////////////////////
COPDrxsis2StickPath::COPDrxsis2StickPath(const XmlNodeRef& node)
	: m_State(eState_FIRST)
	, m_fPathLength(0.0f)
...
//////////////////////////////////////////////////////////////////////////
COPDrxsis2StickPath::COPDrxsis2StickPath(const COPDrxsis2StickPath& rhs)
	: m_State(rhs.m_State)
...
//////////////////////////////////////////////////////////////////////////
COPDrxsis2StickPath::~COPDrxsis2StickPath()
{
	m_refNavTarget.Release();
}

//////////////////////////////////////////////////////////////////////////
void COPDrxsis2StickPath::Reset(CPipeUser* pPipeUser)
{
...
//////////////////////////////////////////////////////////////////////////
void COPDrxsis2StickPath::Serialize(TSerialize ser)
{
	ser.BeginGroup("COPDrxsis2StickPath");
	{
...
//////////////////////////////////////////////////////////////////////////
void COPDrxsis2StickPath::ProjectTargetOntoPath(Vec3& outPos, float& outNearestDistance, float& outDistanceAlongPath) const
{
...
//////////////////////////////////////////////////////////////////////////
COPDrxsis2StickPath::ETargetRange COPDrxsis2StickPath::GetTargetRange(const Vec3& vMyPos) const
{
	assert(m_refTarget.IsValid());
...
//////////////////////////////////////////////////////////////////////////
COPDrxsis2StickPath::ETargetRange COPDrxsis2StickPath::GetTargetRange(const Vec3& vMyPos, const Vec3& vTargetPos) const
{
	ETargetRange result = eTR_TooFar;
...
//////////////////////////////////////////////////////////////////////////
Vec3 COPDrxsis2StickPath::GetProjectedPos(CPuppet* pPuppet) const
{
	assert(pPuppet);
...
//////////////////////////////////////////////////////////////////////////
EGoalOpResult COPDrxsis2StickPath::Execute(CPipeUser* pPipeUser)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//////////////////////////////////////////////////////////////////////////
void COPDrxsis2StickPath::ExecuteDry(CPipeUser* pPipeUser)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//////////////////////////////////////////////////////////////////////////
EGoalOpResult COPDrxsis2StickPath::ExecuteCurrentState(CPipeUser* pPipeUser, bool bDryUpdate)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	if (pPuppet)
	{
		...
		default:
			DRX_ASSERT_MESSAGE(false, "COPDrxsis2StickPath::Execute Unhandled state");
			break;
		}
	}
...
//////////////////////////////////////////////////////////////////////////
bool COPDrxsis2StickPath::ExecuteState_Prepare(CPuppet* pPuppet, bool bDryUpdate)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	if (!bDryUpdate)
	{
		tukk szPathName = pPuppet->GetPathToFollow();
		...
		else if (m_refNavTarget.IsNil())
		{
			...
			gAIEnv.pAIObjectUpr->CreateDummyObject(m_refNavTarget, name, CAIObject::STP_REFPOINT);
...
//////////////////////////////////////////////////////////////////////////
bool COPDrxsis2StickPath::ExecuteState_Navigate(CPuppet* pPuppet, bool bDryUpdate)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//////////////////////////////////////////////////////////////////////////
bool COPDrxsis2StickPath::ExecuteState_Wait(CPuppet* pPuppet, bool bDryUpdate)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
COPAcquirePosition::COPAcquirePosition() : m_target(AI_REG_NONE), /*m_Graph(0),*/ m_State(C2AP_INIT), m_SubState(C2APCS_GATHERSPANS)
{

}

COPAcquirePosition::COPAcquirePosition(const XmlNodeRef& node) : m_target(AI_REG_NONE), /*m_Graph(0),*/ m_State(C2AP_INIT), m_SubState(C2APCS_GATHERSPANS)
{
	s_xml.GetRegister(node, "register", m_target);
...
//COPDrxsis2Fly::SolverAllocator COPDrxsis2Fly::m_Solvers;

COPDrxsis2Fly::COPDrxsis2Fly()
	: m_State(C2F_INVALID)
	, m_target(AI_REG_NONE)
	//, m_Graph(0)
...
COPDrxsis2Fly::COPDrxsis2Fly(const XmlNodeRef& node)
	: m_State(C2F_INVALID)
	, m_target(AI_REG_NONE)
	//, m_Graph(0)
	, m_lookAheadDist(10.0f)
...
COPDrxsis2Fly::COPDrxsis2Fly(const COPDrxsis2Fly& rhs)
{
	m_State = rhs.m_State;
...
void COPDrxsis2Fly::ExecuteDry(CPipeUser* pPipeUser)
{
	Execute(pPipeUser);
}

void COPDrxsis2Fly::Reset(CPipeUser* pPipeUser)
{
	m_State = C2F_INVALID;
	//m_Graph = 0;
	m_Length = 0xffffffff;
...
void COPDrxsis2Fly::Serialize(TSerialize ser)
{
	ser.BeginGroup("COPDrxsis2Fly");
	{
		ser.Value("m_state", *(alias_cast<i32*>(&m_State)));
		ser.Value("m_target", *(alias_cast<i32*>(&m_target)));
...
COPDrxsis2Fly::TargetResult COPDrxsis2Fly::GetTarget(CPipeUser* pPipeUser, Vec3& target) const
{
...
void COPDrxsis2Fly::ParseParam(tukk param, const GoalParams& value)
{
	if (!stricmp("desiredSpeed", param))
	{
		value.GetValue(m_desiredSpeed);
		m_currentSpeed = m_desiredSpeed;
	}
	else if (!stricmp("concurrent", param))
	{
		tukk str;
		value.GetValue(str);

		CGoalPipe* pPipe = gAIEnv.pPipeUpr->IsGoalPipe(str);
	...
	else if (!stricmp("continuous", param))
	{
		value.GetValue(m_Circular);
	}
	else if (!stricmp("minDistance", param))
	{
		value.GetValue(m_minDistance);
	}
	else if (!stricmp("maxDistance", param))
	{
		value.GetValue(m_maxDistance);
	}
	else if (!stricmp("targetEntity", param))
	{
		value.GetValue(m_TargetEntity);
	}
	else if (!stricmp("impulse", param))
	{
...
EGoalOpResult COPDrxsis2Fly::CalculateTarget(CPipeUser* pPipeUser)
{
	...
	case C2F_SET_PATH:
		{
			tukk pathToFollow = pPipeUser->GetPathToFollow();
...
bool COPDrxsis2Fly::HandleThreat(CPipeUser* pPipeUser, const Vec3& moveDir)
{
	bool ret = false;
	/*
...
void COPDrxsis2Fly::SendEvent(CPipeUser* pPipeUser, tukk eventName)
{
...
float COPDrxsis2Fly::CheckTargetEntity(const CPipeUser* pPipeUser, const Vec3& lookAheadPos)
{
...
EGoalOpResult COPDrxsis2Fly::Execute(CPipeUser* pPipeUser)
{
	EGoalOpResult result = eGOR_FAILED;
	...
	case C2F_INVALID:
		{
			DRX_PROFILE_REGION(PROFILE_AI, "COPDrxsis2Fly: SETUP ASTAR" );
	...
	case C2F_PATHFINDING:
		{
			DRX_PROFILE_REGION(PROFILE_AI, "COPDrxsis2Fly: ASTAR UPDATE";
...
COPDrxsis2ChaseTarget::COPDrxsis2ChaseTarget()
{
}

  COPDrxsis2ChaseTarget::COPDrxsis2ChaseTarget(const XmlNodeRef& node)
	: m_State(C2F_INVALID)
	, m_target(AI_REG_NONE)
	, m_lookAheadDist(10.0f)
	, m_desiredSpeed(0.0f)
...
COPDrxsis2ChaseTarget::COPDrxsis2ChaseTarget(const COPDrxsis2ChaseTarget& rhs)
{
	m_State = rhs.m_State;
	m_target = rhs.m_target;
	m_destination = rhs.m_destination;
...
void COPDrxsis2ChaseTarget::ExecuteDry(CPipeUser* pPipeUser)
{
	Execute(pPipeUser);
}

void COPDrxsis2ChaseTarget::Reset(CPipeUser* pPipeUser)
{
	m_State = C2F_INVALID;
	m_desiredSpeed = 50.0f;
	m_distanceMin = 0.0f;
...
COPDrxsis2ChaseTarget::TargetResult COPDrxsis2ChaseTarget::GetTarget(CPipeUser* pPipeUser, Vec3& target, IAIObject** targetObject) const
{
	TargetResult ret = C2F_NO_TARGET;
...
void COPDrxsis2ChaseTarget::ParseParam(tukk param, const GoalParams& value)
{
	if (!stricmp("desiredSpeed", param))
	{
		value.GetValue(m_desiredSpeed);
	}
	else if (!stricmp("distanceMin", param))
	{
		value.GetValue(m_distanceMin);
	}
	else if (!stricmp("distanceMax", param))
	{
...
size_t COPDrxsis2ChaseTarget::GatherCandidateLocations(float currentLocationAlong, const ListPositions& path, float pathLength,
                                                       bool closed, float spacing, const Vec3& target, Candidates& candidates)
{
	...
	const float distanceOffset = 0.0f; //drx_random(-30.0f, 30.0f);
...
EGoalOpResult COPDrxsis2ChaseTarget::Chase(CPipeUser* pPipeUser)
{
	...
	if (gAIEnv.pNavigation->GetDesignerPath(pPipeUser->GetPathToFollow(), path))
	{
		if (!path.shape.empty())
		{
			...
			if (cloaked)
			{
				...
				{
					size_t off = drx_random((size_t)0, m_candidates.size() - 1);
					...
					if (gAIEnv.pNavigation->GetDesignerPath(pPipeUser->GetPathToFollow(), path))
					{
						if (!path.shape.empty())
						{
							...
							m_PathOut.clear();
							CalculateShortestPathTo(path.shape, path.closed, startIndex, segmentFraction,
...
void COPDrxsis2ChaseTarget::TestLocation(const CandidateLocation& location, CPipeUser* pipeUser, const Vec3& target)
{
	...
	m_visCheckRayID[0] = gAIEnv.pRayCaster->Queue(RayCastRequest::HighPriority,
	                                              RayCastRequest(pointLeft, (targetLeft - pointLeft) * 0.985f, COVER_OBJECT_TYPES,
	                                                             AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOLID_COVER | AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOFT_COVER,
	                                                             &skipList.at(0), skipList.size()), functor(*this, &COPDrxsis2ChaseTarget::VisCheckRayComplete));

	m_visCheckRayID[1] = gAIEnv.pRayCaster->Queue(RayCastRequest::HighPriority,
	                                              RayCastRequest(pointRight, (targetRight - pointRight) * 0.985f, COVER_OBJECT_TYPES,
	                                                             AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOLID_COVER | AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOFT_COVER,
	                                                             &skipList.at(0), skipList.size()), functor(*this, &COPDrxsis2ChaseTarget::VisCheckRayComplete));
...
void COPDrxsis2ChaseTarget::VisCheckRayComplete(const QueuedRayID& rayID, const RayCastResult& result)
{
...
float COPDrxsis2ChaseTarget::CalculateShortestPathTo(const ListPositions& path, bool closed, size_t startIndex,
...
EGoalOpResult COPDrxsis2ChaseTarget::Execute(CPipeUser* pPipeUser)
{
...
COPDrxsis2FlightFireWeapons::COPDrxsis2FlightFireWeapons() : m_State(eFP_START), m_NextState(eFP_STOP), m_InitCoolDown(0.0f), m_WaitTime(-1.0f), m_minTime(0.0f), m_maxTime(0.0f), m_PausedTime(0.0f), m_PauseOverrideTime(0.0f), m_target(AI_REG_NONE), m_targetId(0), m_Rotation(0.0f), m_OnlyLookAt(false), m_LastTarget(ZERO), m_FirePrimary(false), m_SecondaryWeapons(0)
{

};

COPDrxsis2FlightFireWeapons::COPDrxsis2FlightFireWeapons(const XmlNodeRef& node) : m_State(eFP_START), m_NextState(eFP_STOP), m_InitCoolDown(0.0f), m_WaitTime(-1.0f), m_PausedTime(0.0f), m_PauseOverrideTime(0.0f), m_FirePrimary(false), m_SecondaryWeapons(0), m_minTime(0.0f), m_maxTime(0.0f), m_target(AI_REG_NONE), m_targetId(0), m_Rotation(0.0f), m_OnlyLookAt(false), m_LastTarget(ZERO)
{
...
COPDrxsis2FlightFireWeapons::COPDrxsis2FlightFireWeapons(EAIRegister reg, float minTime, float maxTime, bool primary, u32 secondary) : m_State(eFP_START), m_NextState(eFP_STOP), m_InitCoolDown(0.0f), m_WaitTime(-1.0f), m_PausedTime(0.0f), m_PauseOverrideTime(0.0f), m_Rotation(0.0f), m_target(reg), m_minTime(minTime), m_maxTime(maxTime), m_FirePrimary(primary), m_SecondaryWeapons(secondary), m_OnlyLookAt(false), m_LastTarget(ZERO)
{}
...
void COPDrxsis2FlightFireWeapons::Reset(CPipeUser* pPipeUser)
{
	m_State = eFP_START;
...
void COPDrxsis2FlightFireWeapons::Serialize(TSerialize ser)
{
	ser.BeginGroup("COPDrxsis2FlightFireWeapons");
	{
		ser.Value("m_State", *(alias_cast<i32*>(&m_State)));
		ser.Value("m_NextState", *(alias_cast<i32*>(&m_NextState)));
...
void COPDrxsis2FlightFireWeapons::ParseParam(tukk param, const GoalParams& value)
{
	if (!stricmp("target", param))
	{
		m_InitCoolDown = 0.0f;
	...
	else if (!stricmp("useSecondary", param))
	{
		u32 tmp;
	...
	else if (!stricmp("fireDuration", param))
	{
		value.GetValue(m_minTime);
		m_maxTime = m_minTime;
	}
	else if (!stricmp("onlyLookAt", param))
	{
		value.GetValue(m_OnlyLookAt);
	}
	else if (!stricmp("firePause", param))
	{
		bool paused;
...
void COPDrxsis2FlightFireWeapons::ExecuteDry(CPipeUser* pPipeUser)
{
	Execute(pPipeUser);
}

bool COPDrxsis2FlightFireWeapons::CalculateHitPos(CPipeUser* pPipeUser, CAIObject* targetObject, Vec3& target)
{
	bool ret = false;
...
bool COPDrxsis2FlightFireWeapons::CalculateHitPosPlayer(CPipeUser* pPipeUser, CAIPlayer* targetPlayer, Vec3& target)
{
	bool ret = false;
	...
	if (targetPlayer)
	{
		...
		if (cloaked)
		{
			...
			{
				target = targetPlayer->GetPos() + drx_random(0.5f, 1.0f) * Vec3(3.0f, 3.0f, 0.0f);
				m_LastTarget = target;
			}
			else
			{
				target = m_LastTarget + drx_random(0.25f, 0.5f) * Vec3(2.0f, 2.0f, 0.0f);
			}
...
bool COPDrxsis2FlightFireWeapons::GetTarget(CPipeUser* pPipeUser, Vec3& target)
{
	bool ret = false;
...
void COPDrxsis2FlightFireWeapons::ExecuteShoot(CPipeUser* pPipeUser, const Vec3& targetPos)
{
...
EGoalOpResult COPDrxsis2FlightFireWeapons::Execute(CPipeUser* pPipeUser)
{
	...
	case eFP_WAIT_INIT:
		...
		{
			m_WaitTime = gEnv->pTimer->GetCurrTime() + drx_random(m_minTime, m_maxTime);
			m_State = eFP_WAIT;
		}
		else
		{
	...
	case eFP_PAUSED:
		{
			...
			{
				if (drx_random(0.0f, 1.0f) > 0.9f)
				{
					m_State = eFP_PAUSED_OVERRIDE;
					m_PauseOverrideTime = drx_random(2.0f, 4.0f);
				}
			}
...
COPDrxsis2Hover::COPDrxsis2Hover() : m_State(CHS_INVALID), m_target(AI_REG_NONE), m_Continuous(false), m_NextState(CHS_INVALID)
{

}

COPDrxsis2Hover::COPDrxsis2Hover(EAIRegister reg, bool continuous) : m_State(CHS_INVALID), m_target(reg), m_Continuous(continuous), m_NextState(CHS_INVALID)
{}

COPDrxsis2Hover::COPDrxsis2Hover(const XmlNodeRef& node) : m_State(CHS_INVALID), m_target(AI_REG_NONE), m_Continuous(false), m_NextState(CHS_INVALID)
{
	s_xml.GetRegister(node, "register", m_target);
	m_Continuous = s_xml.GetBool(node, "continuousUpdate");
}

COPDrxsis2Hover::~COPDrxsis2Hover()
{

}

void COPDrxsis2Hover::Reset(CPipeUser* pPipeUser)
{
	m_State = CHS_INVALID;
	m_NextState = CHS_INVALID;
}

void COPDrxsis2Hover::ExecuteDry(CPipeUser* pPipeUser)
{
	Execute(pPipeUser);
}

bool COPDrxsis2Hover::GetTarget(CPipeUser* pPipeUser, Vec3& target)
{
	bool ret = false;
...
EGoalOpResult COPDrxsis2Hover::Execute(CPipeUser* pPipeUser)
{
	switch (m_State)
	{
	case CHS_INVALID:
	...
	case CHS_HOVERING:
		if (pPipeUser)
		{
			...
			float speed = static_cast<float>(__fsel(diffZ, -0.05f - diffZ * 0.2f, 0.05f - diffZ * 0.2f));
