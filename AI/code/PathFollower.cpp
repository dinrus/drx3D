...
static Vec3 GetPathPointAhead(const TPathPoints& pathPts, TPathPoints::const_iterator it, float dist)
{
	...
	else
	{
		...
		i32 n = (i32)std::distance(pathPts.begin(), it);
		TPathPoints::iterator itRev = pathPtsRev.begin();
		std::advance(itRev, (i32)(pathPtsRev.size() - (n + 1)));
...
void CPathFollower::ProcessPath()
{
	...
	if (m_params.endDistance > 0.0f)
	{
		...
		for (i32 iPt = ((i32) m_pathControlPoints.size()) - 2; iPt >= 0; --iPt)
		{
			i32 iNext = iPt + 1;
	...
	for (i32 iPt = ((i32) m_pathControlPoints.size()) - 1; iPt >= 0; --iPt)
	{
...
void CPathFollower::StartFollowing(const Vec3& curPos, const Vec3& curVel)
{
	...
	i32 nPts = m_pathControlPoints.size();
	for (i32 i = 0; i < nPts - 1; ++i)
	{
		float t;
...
//===================================================================
void CPathFollower::GetNewLookAheadPos(Vec3& newLAPos, i32& newLASegIndex, const Vec3& curLAPos, i32 curLASegIndex,
...
void CPathFollower::UseLookAheadPos(Vec3& velocity, bool& reachedEnd, const Vec3 LAPos, const Vec3 curPos, const Vec3 curVel,
                                    float dt, i32 curLASegmentIndex, float& lastOutputSpeed) const
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	if (curLASegmentIndex >= (i32)m_pathControlPoints.size() - 2)
	{
...
u32 CPathFollower::GetIndex(const Vec3& pos) const
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
  ...
  bool CPathFollower::Update(PathFollowResult& result, const Vec3& curPos, const Vec3& curVel, float dt)
{
	...
	{
		result.predictedStates->resize(0);
		i32 nDesiredPredictions = (i32)(result.desiredPredictionTime / result.predictionDeltaTime + 0.5f);

		// Danny todo having a very small value here improves the smoothness of the prediction, but
		// it's expensive too.
		static float idealDt = 0.05f;
		i32 stepsPerPrediction = (i32) (result.predictionDeltaTime / idealDt);
		...
		i32 predCurLASegmentIndex = m_curLASegmentIndex;
		Vec3 predCurPos = curPos;
		Vec3 predCurVel = curVel;
		float lastOutputSpeed = m_lastOutputSpeed;

		for (i32 iPrediction = 0; iPrediction != nDesiredPredictions; ++iPrediction)
		{
			for (i32 iStep = 0; iStep < stepsPerPrediction; ++iStep)
			{
				Vec3 newLAPos2;
				i32 newLASegIndex2;
...
// GetDistToEnd
//===================================================================
float CPathFollower::GetDistToEnd(const Vec3* pCurPos, i32 curLASegmentIndex, Vec3 curLAPos) const
{
	...
	float dist = 0.0f;
	i32 nPts = m_pathControlPoints.size();
	for (i32 i = curLASegmentIndex + 1; i < nPts - 2; ++i)
...
{
	u32 nPts = m_pathControlPoints.size();
	...
	for (u32 i = m_CurIndex + 1; i < nPts; ++i)
	{
...
// GetDistToCustomNav
//===================================================================
float CPathFollower::GetDistToCustomNav(const TPathControlPoints& controlPoints, u32 curLASegmentIndex, const Vec3& curLAPos) const
{
	i32 nPts = controlPoints.size();

	float dist = std::numeric_limits<float>::max();

	if (curLASegmentIndex + 1 < (u32)nPts)
	{
		    ...
		    controlPoints[curLASegmentIndex + 1].navType != IAISystem::NAV_CUSTOM_NAVIGATION)
		{
			...
			for (i32 i = curLASegmentIndex + 1; i < nPts; ++i)
			{
				dist += DistancePointPoint(lastPos, m_pathControlPoints[i].pos);
				if (i + 1 < nPts)
				{
...
// GetPathPointAhead
//===================================================================
Vec3 CPathFollower::GetPathPointAhead(float dist, float& actualDist, i32 curLASegmentIndex, Vec3 curLAPos) const
{
	Vec3 pt = curLAPos;
	actualDist = 0.0f;
	i32k nPts1 = m_pathControlPoints.size() - 1;
...
void CPathFollower::Advance(float distance)
{
	...
	while (distLeft > 0.0f && m_curLASegmentIndex < (i32) m_pathControlPoints.size() - 1)
	{
...
{
	DRX_ASSERT(false); // not implemented
	return false;
}
