...
void CEntityAICoverUserComponent::SCoverCompromisedSignal::ReflectType(sxema::CTypeDesc<SCoverCompromisedSignal>& desc)
{
	desc.SetGUID("DB200CEA-B4F7-4F0F-9699-9DC1864B7B0B"_drx_guid);
...
void CEntityAICoverUserComponent::SCoverEnteredSignal::ReflectType(sxema::CTypeDesc<SCoverEnteredSignal>& desc)
{
	desc.SetGUID("203EDECB-215C-4DDE-BEE1-B87468602626"_drx_guid);
...
void CEntityAICoverUserComponent::SCoverLeftSignal::ReflectType(sxema::CTypeDesc<SCoverLeftSignal>& desc)
{
	desc.SetGUID("0D6713E2-09C2-42AD-8961-B4FFCA7C6F9F"_drx_guid);
...
void CEntityAICoverUserComponent::SMoveToCoverFailedSignal::ReflectType(sxema::CTypeDesc<SMoveToCoverFailedSignal>& desc)
{
	desc.SetGUID("117C8F12-F137-4991-A169-566321635AEE"_drx_guid);
...
void CEntityAICoverUserComponent::SRefreshEyesSignal::ReflectType(sxema::CTypeDesc<SRefreshEyesSignal>& desc)
{
	desc.SetGUID("C9AFCB5B-EE30-4BED-9A98-1AC51F8ADC47"_drx_guid);
...
void CEntityAICoverUserComponent::ReflectType(sxema::CTypeDesc<CEntityAICoverUserComponent>& desc)
{
...
void CEntityAICoverUserComponent::Register(sxema::IEnvRegistrar& registrar)
{
	sxema::CEnvRegistrationScope scope = registrar.Scope(IEntity::GetEntityScopeGUID());
	{
		sxema::CEnvRegistrationScope componentScope = scope.Register(SXEMA_MAKE_ENV_COMPONENT(CEntityAICoverUserComponent));

		// Functions
		{
			auto pFunction = SXEMA_MAKE_ENV_FUNCTION(&CEntityAICoverUserComponent::GetRandomCoverId, "07C39B63-5D8F-4C7B-8730-4BA189E20461"_drx_guid, "GetRandomCover");
		...
		{
			auto pFunction = SXEMA_MAKE_ENV_FUNCTION(&CEntityAICoverUserComponent::MoveToCover, "04B97008-7145-43F8-9099-947D376770B1"_drx_guid, "MoveToCover");
			pFunction->SetDescription("Starts move to cover movement request.");
			pFunction->BindInput(1, 'cid', "CoverId", "Cover Id");
			componentScope.Register(pFunction);
		}
		{
			auto pFunction = SXEMA_MAKE_ENV_FUNCTION(&CEntityAICoverUserComponent::IsInCover, "53E71E95-48DB-4A77-A31D-409E65624D0B"_drx_guid, "IsInCover");
		...
		{
			auto pFunction = SXEMA_MAKE_ENV_FUNCTION(&CEntityAICoverUserComponent::IsMovingToCover, "5D5CDA5F-EB09-4C47-A563-CF983129EB06"_drx_guid, "IsMovingToCover");
		...
		{
			auto pFunction = SXEMA_MAKE_ENV_FUNCTION(&CEntityAICoverUserComponent::IsCoverCompromised, "AA5E2D2B-9208-4AF9-91BD-B4820F9A4F94"_drx_guid, "IsCoverCompromised");
		...
		{
			auto pFunction = SXEMA_MAKE_ENV_FUNCTION(&CEntityAICoverUserComponent::GetCurrentCoverPosition, "D53E7DD9-F0C9-454A-AE95-AEA78B3ADA92"_drx_guid, "GetCurrentCoverPosition");
		...
		{
			auto pFunction = SXEMA_MAKE_ENV_FUNCTION(&CEntityAICoverUserComponent::GetCurrentCoverNormal, "293BB9B6-5EDD-4A01-A53B-388521776FB1"_drx_guid, "GetCurrentCoverNormal");
		...
		{
			auto pFunction = SXEMA_MAKE_ENV_FUNCTION(&CEntityAICoverUserComponent::AddCoverEye, "E512537F-A637-4210-96AC-DED9FE506DB0"_drx_guid, "AddCoverEye");
		...
		// Signals
		{
			componentScope.Register(SXEMA_MAKE_ENV_SIGNAL(SCoverCompromisedSignal));
			componentScope.Register(SXEMA_MAKE_ENV_SIGNAL(SCoverEnteredSignal));
			componentScope.Register(SXEMA_MAKE_ENV_SIGNAL(SCoverLeftSignal));
			componentScope.Register(SXEMA_MAKE_ENV_SIGNAL(SMoveToCoverFailedSignal));
			componentScope.Register(SXEMA_MAKE_ENV_SIGNAL(SRefreshEyesSignal));
		}
	}
}
...
void CEntityAICoverUserComponent::Start()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_WARNING, "Entity '%s' wasn't registered to Movement system before trying to add Cover action ability.", GetEntity()->GetName());
	}
}
...
{
	if (sxema::IObject* pSchematycObject = GetEntity()->GetSchematycObject())
	{
...
void CEntityAICoverUserComponent::SetState(ICoverUser::StateFlags state)
{
	...
	{
		DRX_ASSERT(m_pCoverUser->GetState().Check(ICoverUser::EStateFlags::MovingToCover));
	...
	{
		DRX_ASSERT(!m_pCoverUser->GetState().Check(ICoverUser::EStateFlags::InCover));
	}
...
CoverID CEntityAICoverUserComponent::GetRandomCoverId(float radius) const
{
	...
	return validCovers.size() ? validCovers[drx_random(size_t(0), validCovers.size() - 1)] : CoverID();
}
