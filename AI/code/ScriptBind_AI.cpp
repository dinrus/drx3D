...
/********************************************************************
   ...
   Version:     v1.00
   Описание: Core scriptbinds for the AI system
...
bool GetSoundPerceptionDescriptorHelper(IPuppet* pPuppet, i32 iSoundType, SSoundPerceptionDescriptor& sDescriptor)
{
	if (pPuppet)
	{
...
i32 ParsePostureInfo(CPuppet* pPuppet, const SmartScriptTable& posture, const PostureUpr::PostureInfo& base, i32 parentID, IScriptSystem* pSS)
{
	PostureUpr::PostureInfo postureInfo = base;
	postureInfo.parentID = parentID;
	postureInfo.enabled = true;

	i32 type = postureInfo.type;
	...
	postureInfo.type = (PostureUpr::PostureType)type;

	tukk name = "UnnamedPosture";
	...
	i32 id = pPuppet->GetPostureUpr().AddPosture(postureInfo);
...
void InitLookUp()
{
	...
	for (i32 childN = 0; childN < nodeTable->getChildCount(); ++childN)
	{
		...
		for (i32 childrenCntr(0), cellIndex(1); childrenCntr < nodeRow->getChildCount(); ++childrenCntr, ++cellIndex)
		{
			...
			{
				tukk pStrIdx = nodeCell->getAttr("ss:Index");
			...
			if (cellIndex == 1)
			{
				tukk item(nodeCellData->getContent());
				if (item)
				{
					float theValue(.0f);
...
#define SCRIPT_REG_CLASSNAME &CScriptBind_AI::
	...
	gEnv->pScriptSystem->SetGlobalValue("POSTURE_NULL", PostureUpr::InvalidPosture);
	gEnv->pScriptSystem->SetGlobalValue("POSTURE_PEEK", PostureUpr::PeekPosture);
	gEnv->pScriptSystem->SetGlobalValue("POSTURE_AIM", PostureUpr::AimPosture);
	gEnv->pScriptSystem->SetGlobalValue("POSTURE_HIDE", PostureUpr::HidePosture);
	...
	SCRIPT_REG_GLOBAL(AIOBJECT_HELICOPTERDRXSIS2);
...
//====================================================================
EntityId CScriptBind_AI::GetEntityIdFromParam(IFunctionHandler* pH, i32 i)
{
	assert(pH);
	assert(i > 0);
	ScriptHandle hdl(0);
...
//====================================================================
IEntity* CScriptBind_AI::GetEntityFromParam(IFunctionHandler* pH, i32 i)
{
...
// Warning
//====================================================================
i32 CScriptBind_AI::Warning(IFunctionHandler* pH)
{
...
// Error
//====================================================================
i32 CScriptBind_AI::Error(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	tukk sParam = NULL;
	pH->GetParam(1, sParam);
...
// LogProgress
//====================================================================
i32 CScriptBind_AI::LogProgress(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	tukk sParam = NULL;
	pH->GetParam(1, sParam);
...
// LogEvent
//====================================================================
i32 CScriptBind_AI::LogEvent(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	tukk sParam = NULL;
	pH->GetParam(1, sParam);
...
// LogComment
//====================================================================
i32 CScriptBind_AI::LogComment(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	tukk sParam = NULL;
	pH->GetParam(1, sParam);
...
// RecComment
//====================================================================
i32 CScriptBind_AI::RecComment(IFunctionHandler* pH)
{
...
/*
   //
   //-----------------------------------------------------------------------------------------------------------
   i32 CScriptBind_AI::AddSmartObjectCondition(IFunctionHandler *pH)
   {
   SCRIPT_CHECK_PARAMETERS( 1 );
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::ExecuteAction(IFunctionHandler* pH)
{
	tukk sActionName = NULL;
	...
	gAIEnv.pAIActionUpr->ExecuteAIAction(sActionName, pUser, pObject, maxAlertness, goalPipeId);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::AbortAction(IFunctionHandler* pH)
{
	IEntity* pUser = NULL;
	...
	gAIEnv.pAIActionUpr->AbortAIAction(pUser, goalPipeId);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetSmartObjectState(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	tukk stateName;
	if (pEntity && gAIEnv.pSmartObjectUpr)
	{
		...
		gAIEnv.pSmartObjectUpr->SetSmartObjectState(pEntity, stateName);
	}
	return pH->EndFunction();
}
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::ModifySmartObjectStates(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (pEntity && gAIEnv.pSmartObjectUpr)
	{
		...
		gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(pEntity, listStates);
	}
	return pH->EndFunction();
}
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SmartObjectEvent(IFunctionHandler* pH)
{
	...
	if (gAIEnv.pSmartObjectUpr)
	{
		...
		id = gAIEnv.pSmartObjectUpr->SmartObjectEvent(sActionName, pUser, pObject, bUseRefPoint ? &vRefPoint : NULL);
	}
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetLastUsedSmartObject(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	if (!pEntity)
	{
...
//
//-----------------------------------------------------------------------------------------------------------
bool CScriptBind_AI::ParseTables(i32 firstTable, bool parseMovementAbility, IFunctionHandler* pH, AIObjectParams& params, bool& autoDisable)
{
	...
	if (gAIEnv.configuration.eCompatibilityMode == ECCM_GAME04 ||
	    ...
	    gAIEnv.configuration.eCompatibilityMode == ECCM_DRXSIS2)
		autoDisable = false;
	...
	if (pMovementTable)
	{
		...
		if (pMovementTable->GetValue("AIMovementSpeeds", pSpeedTable))
		{
			...
			for (i32 s = 0; s < AgentMovementSpeeds::AMS_NUM_VALUES; ++s)
			{
				switch (s)
				{
				...
				{
					for (i32 u = 0; u < AgentMovementSpeeds::AMU_NUM_VALUES /*-1*/; ++u)
					{
						switch (u)
						{
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::ResetParameters(IFunctionHandler* pH)
{
...
i32 CScriptBind_AI::CreateGoalPipe(IFunctionHandler* pH)
{
	//	CHECK_PARAMETERS(1);
	...
	/*IGoalPipe *pPipe =*/ gAIEnv.pPipeUpr->CreateGoalPipe(name, CPipeUpr::SilentlyReplaceDuplicate);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::BeginGoalPipe(IFunctionHandler* pH)
{
	...
	m_IsGroupOpen = false;
	m_pCurrentGoalPipe = gAIEnv.pPipeUpr->CreateGoalPipe(name, CPipeUpr::SilentlyReplaceDuplicate);
	if (!m_pCurrentGoalPipe)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::EndGoalPipe(IFunctionHandler* pH)
{
	if (!m_pCurrentGoalPipe)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::BeginGroup(IFunctionHandler* pH)
{
	if (m_IsGroupOpen)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::EndGroup(IFunctionHandler* pH)
{
	if (!m_IsGroupOpen)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::PushLabel(IFunctionHandler* pH)
{
	tukk pipename;
	tukk labelname;
	...
	else
	{
		...
		if (!(pPipe = gAIEnv.pPipeUpr->OpenGoalPipe(pipename)))
			return pH->EndFunction();
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::PushGoal(IFunctionHandler* pH)
{
	...
	else
	{
		...
		pPipe = gAIEnv.pPipeUpr->OpenGoalPipe(pipename);
		cnt = 3;
	}
	...
	case eGO_PATHFIND:
		{
			if (pH->GetParamCount() > cnt)
			{
				pH->GetParam(cnt + 1, temp);
				params.str = temp;
				params.pTarget = params.str.empty() ? 0 : gAIEnv.pAIObjectUpr->GetAIObjectByName(0, temp);
			}
			pPipe->PushGoal(op, blocking, grouped, params);
		}
		break;
	...
	default:
		{
			if (pH->GetParamCount() > cnt)
			{
				...
				pH->GetParam(cnt + 1, params.fValue);
				if (!gAIEnv.pPipeUpr->OpenGoalPipe(goalname))
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::IsGoalPipe(IFunctionHandler* pH)
{
	tukk pipename;
	...
	if (gAIEnv.pPipeUpr->OpenGoalPipe(pipename))
		return pH->EndFunction(true);
	return pH->EndFunction(false);
}

bool CScriptBind_AI::GetSignalExtraData(IFunctionHandler* pH, i32 iParam, IAISignalExtraData*& pEData)
{
	...
	for (i32 i = iParam; i <= iParam + 3 && i <= pH->GetParamCount(); i++)
	{
		//i32 iType = pH->GetParamType(i);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::Signal(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 4)
	{
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::NotifyGroup(IFunctionHandler* pH, i32 groupID, ScriptHandle sender, tukk notification)
{
	return pH->EndFunction(ScriptHandle(gAIEnv.pGroupUpr->NotifyGroup(groupID, (EntityId)sender.n, notification)));
}

//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::FreeSignal(IFunctionHandler* pH)
{
	//CHECK_PARAMETERS(4);
	float fRadius;
	i32 nSignalID;
...
//////////////////////////////////////////////////////////////////////
i32 CScriptBind_AI::SetIgnorant(IFunctionHandler* pH)
{
...
i32 CScriptBind_AI::BreakEvent(IFunctionHandler* pH, ScriptHandle entityID, Vec3 pos, float radius)
{
...
i32 CScriptBind_AI::AddCoverEntity(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (gAIEnv.pCoverSystem)
...
i32 CScriptBind_AI::RemoveCoverEntity(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (gAIEnv.pCoverSystem)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetAssesmentMultiplier(IFunctionHandler* pH)
{
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetGroupCount(IFunctionHandler* pH)
{
	i32 groupId = -1;
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetGroupMember(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 2)
		return pH->EndFunction();
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetGroupTarget(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 1)
	{
...
i32 CScriptBind_AI::GetGroupTargetType(IFunctionHandler* pH, i32 groupID)
{
	const Group& group = gAIEnv.pGroupUpr->GetGroup(groupID);
	return pH->EndFunction(group.GetTargetType());
}

i32 CScriptBind_AI::GetGroupTargetThreat(IFunctionHandler* pH, i32 groupID)
{
	const Group& group = gAIEnv.pGroupUpr->GetGroup(groupID);
	return pH->EndFunction(group.GetTargetThreat());
}

i32 CScriptBind_AI::GetGroupTargetEntity(IFunctionHandler* pH, i32 groupID)
{
	const Group& group = gAIEnv.pGroupUpr->GetGroup(groupID);
...
i32 CScriptBind_AI::GetGroupScriptTable(IFunctionHandler* pH, i32 groupID)
{
	const Group& group = gAIEnv.pGroupUpr->GetGroup(groupID);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetGroupTargetCount(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 1)
	{
		AIWarningID("<CScriptBind_AI> ", "GetGroupTarget: Too few parameters.");
	...
	if (pEntity)
	{
		...
		if (pAIActor)
		{
			...
			if (pGroup)
			{
				...
				if (pH->GetParamCount() > 2)
					pH->GetParam(3, bLiveOnly);
				i32 n = pGroup->GetTargetCount(bHostileOnly, bLiveOnly);
...
/*
   static void	DrawDebugBox(const AABB& aabb, u8 r, u8 g, u8 b, float t)
   {
   Vec3	verts[8];
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::FindObjectOfType(IFunctionHandler* pH)
{
	i32 type;
	Vec3 searchPos;
	Vec3 returnPos;
	...
	if (nFlags != 0)
	{
		...
		if (nonOccupied)
		{
			...
			AutoAIObjectIter it(gAIEnv.pAIObjectUpr->GetFirstAIObjectInRange(OBJFILTER_TYPE, AIOBJECT_ACTOR, searchPos, fRadius + 1.0f, false));
		...
		AutoAIObjectIter it(gAIEnv.pAIObjectUpr->GetFirstAIObjectInRange(OBJFILTER_TYPE, type, searchPos, fRadius, false));
		for (; it->GetObject(); it->Next())
		{
			IAIObject* obj = it->GetObject();
			if (!obj->IsEnabled()) continue;
...
i32 CScriptBind_AI::SoundEvent(IFunctionHandler* pH)
{
	i32 type = 0;
	float radius = 0;
	Vec3 pos(0, 0, 0);
...
i32 CScriptBind_AI::VisualEvent(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
...
i32 CScriptBind_AI::GetSoundPerceptionDescriptor(IFunctionHandler* pH)
{
...
i32 CScriptBind_AI::SetSoundPerceptionDescriptor(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
...
i32 CScriptBind_AI::GetAnchor(IFunctionHandler* pH)
{
	CCCPOINT(CScriptBind_AI_GetAnchor);

	i32 nAnchor; // Anchor type
	float fRadiusMin = 0;
	float fRadiusMax = 0;
	float fCone = -1.0f;
...
i32 CScriptBind_AI::GetFactionOf(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity((EntityId)entityID.n))
	{
		if (IAIObject* entityObject = entity->GetAI())
		{
			if (tukk factionName = gAIEnv.pFactionMap->GetFactionName(entityObject->GetFactionID()))
...
i32 CScriptBind_AI::SetFactionOf(IFunctionHandler* pH, ScriptHandle entityID, tukk factionName)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity((EntityId)entityID.n))
	{
		if (IAIObject* entityObject = entity->GetAI())
		{
			u8 factionID = gAIEnv.pFactionMap->GetFactionID(factionName);
...
static u8 GetFactionIDFrom(IFunctionHandler* pH, u32 paramID)
{
...
i32 CScriptBind_AI::GetReactionOf(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);

	u8 factionOne = GetFactionIDFrom(pH, 1);
	u8 factionTwo = GetFactionIDFrom(pH, 2);

	return pH->EndFunction(gAIEnv.pFactionMap->GetReaction(factionOne, factionTwo));
}

i32 CScriptBind_AI::SetReactionOf(IFunctionHandler* pH, tukk factionOne, tukk factionTwo, i32 reaction)
{
	return pH->EndFunction();
}
...
i32 CScriptBind_AI::GetTypeOf(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::GetSubTypeOf(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	...
	if (pEntity)
	{
		...
		if (pObject)
		{
			i32 subType = pObject->GetSubType();
			i32 result = 0;
			...
			if (subType == IAIObject::STP_HELIDRXSIS2)
				result = AIOBJECT_HELICOPTERDRXSIS2;
...
i32 CScriptBind_AI::GetGroupOf(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::Hostile(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 1)
		return pH->EndFunction(false);
	bool bHostile = false;
	...
	if (pEntity)
	{
		...
		if (pObject && pH->GetParamCount() > 1)
		{
			...
			else if (pH->GetParamType(2) == svtString)
			{
				tukk sName;
				pH->GetParam(2, sName);
				pAI2 = gAIEnv.pAIObjectUpr->GetAIObjectByName(0, sName);
			}
...
i32 CScriptBind_AI::SetRefPointPosition(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	if (!pEntity)
		return pH->EndFunction();
	...
	if (paramType2 == svtString)
	{
		...
		{
			if (IAIObject* pFormationPoint = gAIEnv.pFormationUpr->GetFormationPoint(pAIObject))
			{
		...
		if (IAIObject* pAIObject2 = gAIEnv.pAIObjectUpr->GetAIObjectByName(0, szTargetName))
		{
			pPipeUser->SetRefPointPos(pAIObject2->GetPos());
			return pH->EndFunction(1);
		}
...
i32 CScriptBind_AI::SetRefPointDirection(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	//retrieve the entity
	Vec3 vPos(0, 0, 0);
...
i32 CScriptBind_AI::SetRefPointRadius(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	//retrieve the entity
	float radius(0);
...
i32 CScriptBind_AI::GetRefPointPosition(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	if (pEntity)
...
i32 CScriptBind_AI::GetRefPointDirection(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	if (pEntity)
...
i32 CScriptBind_AI::GetFormationPointPosition(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	...
	if (pEntity && pEntity->GetAI())
	{
		...
		IAIObject* pFormationPoint = gAIEnv.pFormationUpr->GetFormationPoint(pObject);
		if (pFormationPoint)
		{
...
// SetRefShapeName
//====================================================================
i32 CScriptBind_AI::SetRefShapeName(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	tukk shapeName = 0;
...
// GetRefShapeName
//====================================================================
i32 CScriptBind_AI::GetRefShapeName(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	if (!pEntity)
		return pH->EndFunction();
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetVehicleStickTarget(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
#ifdef USE_DEPRECATED_AI_CHARACTER_SYSTEM
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetCharacter(IFunctionHandler* pH)
{
	//SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	tukk sCharacterName = 0;
	tukk sBehaviourName = 0;
...
//-------------------------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetForcedNavigation(IFunctionHandler* pH)
{
...
//-------------------------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetAdjustPath(IFunctionHandler* pH)
{
...
//
//-------------------------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetHeliAdvancePoint(IFunctionHandler* pH)
{
...
//-------------------------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetFlyingVehicleFlockingPos(IFunctionHandler* pH)
{
	...
	i32 nCount = gAIEnv.pWorld->GetEntitiesInBox(vMyPos - bboxsize, vMyPos + bboxsize, pEntityList, ent_living | ent_rigid | ent_sleeping_rigid);
...
//-------------------------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::CheckVehicleColision(IFunctionHandler* pH)
{
	Vec3 vHitPos(ZERO);
	...
	i32 geomFlagsAny = geom_colltype0;
...
//-------------------------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::IntersectsForbidden(IFunctionHandler* pH)
{
...
//-------------------------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::IsPointInFlightRegion(IFunctionHandler* pH)
{
...
//-------------------------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::IsPointInWaterRegion(IFunctionHandler* pH)
{
...
//-------------------------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetEnclosingSpace(IFunctionHandler* pH)
{
	...
	i32 checkType = CHECKTYPE_MIN_DISTANCE;
...
i32 CScriptBind_AI::GetAIObjectPosition(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	tukk objName = "";
	if (pH->GetParamType(1) == svtPointer)
	...
	else if (pH->GetParamType(1) == svtString)
	{
		...
		IAIObject* pAIObjectTarget = (IAIObject*) gAIEnv.pAIObjectUpr->GetAIObjectByName(0, objName);
		if (pAIObjectTarget)
		{
...
i32 CScriptBind_AI::GetBeaconPosition(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	...
	else if (pH->GetParamType(1) == svtString)
	{
		tukk objName = "";
		pH->GetParam(1, objName);
		pAI = gAIEnv.pAIObjectUpr->GetAIObjectByName(0, objName);
	}
...
i32 CScriptBind_AI::SetBeaconPosition(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	Vec3 vPos;
	if (!pH->GetParam(2, vPos))
...
i32 CScriptBind_AI::GetNearestEntitiesOfType(IFunctionHandler* pH)
{
	...
	else if (pH->GetParamType(1) == svtString)
	{
		...
		pAIObject = (IAIObject*) gAIEnv.pAIObjectUpr->GetAIObjectByName(0, sObjName);
		if (!pAIObject)
			return pH->EndFunction(0);
		vPos = pAIObject->GetPos();
	}
	...
	for (AutoAIObjectIter it(gAIEnv.pAIObjectUpr->GetFirstAIObject(OBJFILTER_TYPE, iObjectType)); it->GetObject(); it->Next())
	{
		...
		i32 objectGroupId = pObjectIt->GetGroupId();
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::Event(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	tukk szEventText;
	i32 event;
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetAttentionTargetOf(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	if (!pEntity)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetAttentionTargetAIType(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	if (!pEntity)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetAttentionTargetType(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::GetAttentionTargetEntity(IFunctionHandler* pH, ScriptHandle entityID)
{
...
i32 CScriptBind_AI::GetAttentionTargetPosition(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::GetAttentionTargetDistance(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::GetAttentionTargetDirection(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::GetAttentionTargetViewDirection(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::GetAttentionTargetThreat(IFunctionHandler* pH, ScriptHandle entityID)
{
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetPotentialTargetCountFromFaction(IFunctionHandler* pH, ScriptHandle entityID, tukk factionName)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity((EntityId)entityID.n))
	{
		if (IAIObject* entityObject = entity->GetAI())
		{
			...
			{
				i32 count = gAIEnv.pTargetTrackUpr->GetPotentialTargetCountFromFaction(entityObject->GetAIObjectID(), factionName);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetPotentialTargetCount(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity((EntityId)entityID.n))
	{
		if (IAIObject* entityObject = entity->GetAI())
		{
			i32 count = gAIEnv.pTargetTrackUpr->GetPotentialTargetCount(entityObject->GetAIObjectID());
			return pH->EndFunction(count);
		}
	}
...
i32 CScriptBind_AI::CreateFormation(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 1)
		return pH->EndFunction();
	...
	if (pH->GetParam(1, name))
	{
		...
		gAIEnv.pFormationUpr->CreateFormationDescriptor(name);
		if (pH->GetParamCount() > 1)
			pH->GetParam(2, nodeDescr.eClass);
		// create the owner's node at 0,0,0
		gAIEnv.pFormationUpr->AddFormationPoint(name, nodeDescr);
	}
	return pH->EndFunction();
}

//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::AddFormationPointFixed(IFunctionHandler* pH)
{
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::AddFormationPoint(IFunctionHandler* pH)
{
	FormationNode nodeDescr;
	float fSight;
	tukk name;
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetFormationPointClass(IFunctionHandler* pH)
{
	FormationNode nodeDescr;
	i32 pos;
	tukk name;
	...
	return pH->EndFunction(gAIEnv.pFormationUpr->GetFormationPointClass(name, pos));

}

//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetLeader(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
...
i32 CScriptBind_AI::UpTargetPriority(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
	if (!pEntity || !pEntity->GetAI())
...
i32 CScriptBind_AI::DropTarget(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity || !pEntity->GetAI())
...
i32 CScriptBind_AI::ClearPotentialTargets(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::SetTempTargetPriority(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::AddAggressiveTarget(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::UpdateTempTarget(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::ClearTempTarget(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::SetExtraPriority(IFunctionHandler* pH)
{
...
i32 CScriptBind_AI::GetExtraPriority(IFunctionHandler* pH)
{
...
i32 CScriptBind_AI::RegisterTargetTrack(IFunctionHandler* pH)
{
	...
	if (pEntity)
	{
		...
		if (pAI)
		{
			...
			bResult = gAIEnv.pTargetTrackUpr->RegisterAgent(aiObjectId, szConfig, nTargetLimit);
			if (bResult && pH->GetParamCount() > 3)
			{
				float fClassThreat = 0.0f;
				pH->GetParam(4, fClassThreat);

				gAIEnv.pTargetTrackUpr->SetTargetClassThreat(aiObjectId, fClassThreat);
			}
...
i32 CScriptBind_AI::UnregisterTargetTrack(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	...
	if (pEntity)
	{
		...
		if (pAI)
		{
			bResult = gAIEnv.pTargetTrackUpr->UnregisterAgent(pAI->GetAIObjectID());
		}
	}
...
i32 CScriptBind_AI::SetTargetTrackClassThreat(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	...
	if (pEntity)
	{
		...
		if (pAI)
		{
			...
			bResult = gAIEnv.pTargetTrackUpr->SetTargetClassThreat(pAI->GetAIObjectID(), fClassThreat);
		}
	}
...
i32 CScriptBind_AI::TriggerCurrentTargetTrackPulse(IFunctionHandler* pH, ScriptHandle entityID, tukk stimulusName,
                                                   tukk pulseName)
{
	bool bResult = false;
	...
	if (pEntity)
	{
		if (IAIObject* pAI = pEntity->GetAI())
		{
			if (CPuppet* pPuppet = pAI->CastToCPuppet())
			{
				...
				if (pTargetObject && pTargetObject->IsAgent())
					bResult = gAIEnv.pTargetTrackUpr->TriggerPulse(pAI->GetAIObjectID(), pTargetObject->GetAIObjectID(),
...
i32 CScriptBind_AI::CreateStimulusEvent(IFunctionHandler* pH, ScriptHandle ownerId, ScriptHandle targetId,
                                        tukk stimulusName, SmartScriptTable pData)
{
	...
	if (pOwnerEntity)
	{
		...
		pH->EndFunction(gAIEnv.pTargetTrackUpr->HandleStimulusEventForAgent(aiOwnerId, aiTargetId, stimulusName, eventInfo));
	}

	return pH->EndFunction(false);
}

i32 CScriptBind_AI::CreateStimulusEventInRange(IFunctionHandler* pH, ScriptHandle targetId, tukk stimulusName, SmartScriptTable dataScriptTable)
{
	...
	pH->EndFunction(gAIEnv.pTargetTrackUpr->HandleStimulusEventInRange(targetAIObjectId, stimulusName, eventInfo, radius));

	return pH->EndFunction();
}

i32 CScriptBind_AI::SetUnitProperties(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::GetUnitCount(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::SetFactionThreatMultiplier(IFunctionHandler* pH)
{
 ...
 */
i32 CScriptBind_AI::SetLeader(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::GetStance(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	IAIActor* pAIActor;
...
i32 CScriptBind_AI::SetStance(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	IAIActor* pAIActor;
...
i32 CScriptBind_AI::SetMovementContext(IFunctionHandler* pH, ScriptHandle entityId, i32 context)
{
	GET_ENTITY(1);
	CPipeUser* pPipeUser;
...
i32 CScriptBind_AI::ClearMovementContext(IFunctionHandler* pH, ScriptHandle entityId, i32 context)
{
	GET_ENTITY(1);
	CPipeUser* pPipeUser;
	if (!pEntity || !(pPipeUser = CastToCPipeUserSafe(pEntity->GetAI())))
...
i32 CScriptBind_AI::AssignPFPropertiesToPathType(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(19);

	tukk sPathType = "AIPATH_DEFAULT";

	if (pH->GetParamType(1) == svtNumber)
	{
		i32 nPathType = AIPATH_DEFAULT;
...
i32 CScriptBind_AI::AssignPathTypeToSOUser(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);

	tukk soUser;
	tukk pathType = "AIPATH_DEFAULT";
	...
	if (gAIEnv.pSmartObjectUpr)
	{
		gAIEnv.pSmartObjectUpr->MapPathTypeToSoUser(soUser, pathType);
	}

	return pH->EndFunction();
};

i32 CScriptBind_AI::SetPFProperties(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::GetTargetType(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::GetTargetSubType(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
	...
	if (pAIActor)
	{
		...
		if (pTarget)
		{
			i32 subType = pTarget->GetSubType();
			i32 result = 0;
			if (subType == IAIObject::STP_CAR)
				result = AIOBJECT_CAR;
			if (subType == IAIObject::STP_BOAT)
...
i32 CScriptBind_AI::ChangeFormation(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 2)
		return pH->EndFunction();
	GET_ENTITY(1);
	...
	bool bSuccessful = gAIEnv.pFormationUpr->ChangeFormation(pAIObject /*->GetParameters().m_nGroup*/, szDescriptor, fScale);

	return pH->EndFunction(bSuccessful);

}

i32 CScriptBind_AI::ScaleFormation(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 1)
		return pH->EndFunction();
	GET_ENTITY(1);
	...
	bool bSuccessful = gAIEnv.pFormationUpr->ScaleFormation(pAIObject, fScale);

	return pH->EndFunction(bSuccessful);

}

i32 CScriptBind_AI::SetFormationUpdate(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
	...
	bool bSuccessful = gAIEnv.pFormationUpr->SetFormationUpdate(pAIObject, bUpdate);

	return pH->EndFunction(bSuccessful);

}

i32 CScriptBind_AI::SetPostures(IFunctionHandler* pH, ScriptHandle entityId, SmartScriptTable postures)
{
	GET_ENTITY(1);
	...
	pPuppet->GetPostureUpr().ResetPostures();

	i32 i = 0;
	SmartScriptTable posture;

	while (postures->GetAt(++i, posture))
		ParsePostureInfo(pPuppet, posture, PostureUpr::PostureInfo(), -1, m_pSS);

	return pH->EndFunction();
}

i32 CScriptBind_AI::SetPosturePriority(IFunctionHandler* pH, ScriptHandle entityId, tukk postureName, float priority)
{
	GET_ENTITY(1);
	...
	i32 postureID = pPuppet->GetPostureUpr().GetPostureID(postureName);
	if (postureID > -1)
		pPuppet->GetPostureUpr().SetPosturePriority(postureID, priority);

	return pH->EndFunction();
}

i32 CScriptBind_AI::GetPosturePriority(IFunctionHandler* pH, ScriptHandle entityId, tukk postureName)
{
	GET_ENTITY(1);
	...
	PostureUpr::PostureInfo posture;
	if (pPuppet->GetPostureUpr().GetPostureByName(postureName, &posture))
...
//////////////////////////////////////////////////////////////////////////
i32 CScriptBind_AI::ChangeParameter(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
	if (!pEntity)
	...
	else if (pAI)
	{
		if (pH->GetParamType(3) == svtNumber)
		{
			...
			{
			case AIPARAM_GROUPID:
				pAI->SetGroupId((i32)fValue);
				break;
			case AIPARAM_FACTION:
				pAI->SetFactionID((i32)(fValue + 0.5f));
				break;
			default:
				break;
			}
...
tukk CScriptBind_AI::GetPathTypeName(EAIPathType pathType)
{
	switch (pathType)
	{
	default:
...
i32 CScriptBind_AI::GetParameter(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//////////////////////////////////////////////////////////////////////////
i32 CScriptBind_AI::ChangeMovementAbility(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
	if (!pEntity)
...
i32 CScriptBind_AI::GetForwardDir(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetNavigationType(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 1)
	{
	...
	else if (pH->GetParamType(1) == svtNumber)
	{
		i32 groupId = -1;
		pH->GetParam(1, groupId);
		u32 unitProp = UPR_ALL;
		if (pH->GetParamCount() > 1 && pH->GetParamType(2) == svtNumber)
			pH->GetParam(2, unitProp);

		typedef std::map<i32, i32> TmapIntInt;
		TmapIntInt NavTypeCount;

		for (i32 i = 1; i <= IAISystem::NAV_MAX_VALUE; i <<= 1)
			NavTypeCount.insert(std::make_pair(i, 0));

		i32 numAgents = GetAISystem()->GetGroupCount(groupId, IAISystem::GROUP_ENABLED);
		for (i32 i = 0; i < numAgents; i++)
		{
			IAIObject* pAIObject = GetAISystem()->GetGroupMember(groupId, i, IAISystem::GROUP_ENABLED);
			IAIActor* pAIActor = CastToIAIActorSafe(pAIObject);
			if (pAIActor)
			{
				i32 navType = gAIEnv.pNavigation->CheckNavigationType(pAIObject->GetPos(), nBuilding, pAIActor->GetMovementAbility().pathfindingProperties.navCapMask);
				if (NavTypeCount.find(navType) != NavTypeCount.end())
					NavTypeCount[navType] = NavTypeCount[navType] + 1;
			}
		}
		i32 maxNavigationTypeCount = 0;
		i32 navTypeSelected = IAISystem::NAV_UNSET;
		for (TmapIntInt::iterator it = NavTypeCount.begin(); it != NavTypeCount.end(); ++it)
		{
			i32 navCount = it->second;
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetDirectAnchorPos(IFunctionHandler* pH)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//-----------------------------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::InvalidateHidespot(IFunctionHandler* pH)
{
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::EvalHidespot(IFunctionHandler* pH)
{
	// Evaluation:
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::EvalPeek(IFunctionHandler* pH)
{
	// Evaluation:
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::AddPersonallyHostile(IFunctionHandler* pH, ScriptHandle entityID, ScriptHandle hostileID)
{
	CAIActor* actor = 0;
...
i32 CScriptBind_AI::RemovePersonallyHostile(IFunctionHandler* pH, ScriptHandle entityID, ScriptHandle hostileID)
{
	CAIActor* actor = 0;
...
i32 CScriptBind_AI::ResetPersonallyHostiles(IFunctionHandler* pH, ScriptHandle entityID, ScriptHandle hostileID)
{
	CAIActor* actor = 0;
...
i32 CScriptBind_AI::IsPersonallyHostile(IFunctionHandler* pH, ScriptHandle entityID, ScriptHandle hostileID)
{
	CAIActor* actor = 0;
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::NotifyReinfDone(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	i32 done = 0;
...
i32 CScriptBind_AI::BehaviorEvent(IFunctionHandler* pH)
{
	GET_ENTITY(1);
...
i32 CScriptBind_AI::GetGroupAveragePosition(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
	u32 properties;
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetFireMode(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (pEntity)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetMemoryFireType(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (pEntity)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetMemoryFireType(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (pEntity)
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::ThrowGrenade(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetRefPointToGrenadeAvoidTarget(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 3)
	{
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::IsAgentInTargetFOV(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::AgentLookAtPos(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::ResetAgentLookAtPos(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::IsAgentInAgentFOV(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::CreateGroupFormation(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetFormationPosition(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetFormationLookingPoint(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetFormationAngleThreshold(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetFormationPosition(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
// vec3 AI.GetFormationPosition(entityId)
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetFormationLookingPoint(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
//
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::AddCombatClass(IFunctionHandler* pH)
{
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::RayWorldIntersectionWrapper(Vec3 org, Vec3 dir, i32 objtypes, u32 flags, ray_hit* hits, i32 nMaxHits,
                                                IPhysicalEntity** pSkipEnts, i32 nSkipEnts, uk pForeignData, i32 iForeignData)
{
	i32 rwiResult(0);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetRefPointAtDefensePos(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
	Vec3 vDefendSpot;
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetFormationUpdateSight(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 2)
	{
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetPathToFollow(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	tukk pathName = 0;
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetPathAttributeToFollow(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetTotalLengthOfPath(IFunctionHandler* pH)
{
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetNearestPointOnPath(IFunctionHandler* pH)
{
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetPathSegNoOnPath(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetPointOnPathBySegNo(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetPathLoop(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::SetPointListToFollow(IFunctionHandler* pH)
{
	...
	i32 navTypeScript = static_cast<i32>(IAISystem::NAV_FLIGHT);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetNearestPathOfTypeInRange(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	...
	tukk pathName = gAIEnv.pNavigation->GetNearestPathOfTypeInRange(pAI, pos, range, type, devalue, useStartNode);

	return pH->EndFunction(pathName);
}

//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetAlertness(IFunctionHandler* pH)
{
	GET_ENTITY(1);
...
//-----------------------------------------------------------------------------------------------------------
i32 CScriptBind_AI::GetGroupAlertness(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	...
	i32 maxAlertness = 0;
	i32 groupID = pAI->GetGroupId();
	i32 n = GetAISystem()->GetGroupCount(groupID, IAISystem::GROUP_ENABLED);
	for (i32 i = 0; i < n; i++)
	{
...
//====================================================================
i32 CScriptBind_AI::RegisterDamageRegion(IFunctionHandler* pH)
{
	//	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//====================================================================
i32 CScriptBind_AI::GetDistanceAlongPath(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 2)
	{
...
//====================================================================
i32 CScriptBind_AI::IgnoreCurrentHideObject(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 1)
	{
...
//====================================================================
i32 CScriptBind_AI::GetCurrentHideAnchor(IFunctionHandler* pH)
{
	if (pH->GetParamCount() < 1)
	{
	...
	{
		tukk szName = pPipeUser->m_CurrentHideObject.GetAnchorName();
		if (szName && szName[0])
		{
...
//====================================================================
i32 CScriptBind_AI::FindStandbySpotInShape(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	IAIObject* pAI = pEntity->GetAI();
	if (!pAI)
...
//====================================================================
i32 CScriptBind_AI::FindStandbySpotInSphere(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	IAIObject* pAI = pEntity->GetAI();
	if (!pAI)
	...
	IAIObject* bestObj = 0;
	float bestDist = FLT_MAX;
	AutoAIObjectIter it(gAIEnv.pAIObjectUpr->GetFirstAIObjectInRange(OBJFILTER_TYPE, type, spherePos, sphereRad, false));
	for (; it->GetObject(); it->Next())
	{
		IAIObject* obj = it->GetObject();
...
//====================================================================
i32 CScriptBind_AI::GetProbableTargetPosition(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	if (!pEntity)
		return pH->EndFunction();
...
// GetObjectRadius
//====================================================================
i32 CScriptBind_AI::GetObjectRadius(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	if (IAIObject* aiObject = pEntity ? pEntity->GetAI() : 0)
...
//====================================================================
i32 CScriptBind_AI::GetEnclosingGenericShapeOfType(IFunctionHandler* pH)
{
	Vec3 pos(0, 0, 0);
	i32 type = 0;
	i32 checkHeight = 0;
	...
	tukk name = GetAISystem()->GetEnclosingGenericShapeOfType(pos, type, checkHeight != 0);
	if (name)
...
//====================================================================
i32 CScriptBind_AI::IsPointInsideGenericShape(IFunctionHandler* pH)
{
	Vec3 pos(0, 0, 0);
	tukk shapeName = 0;
	i32 checkHeight = 0;
...
//====================================================================
i32 CScriptBind_AI::DistanceToGenericShape(IFunctionHandler* pH)
{
	Vec3 pos(0, 0, 0);
	tukk shapeName = 0;
	i32 checkHeight = 0;
...
//====================================================================
i32 CScriptBind_AI::ConstrainPointInsideGenericShape(IFunctionHandler* pH)
{
	Vec3 pos(0, 0, 0);
	tukk shapeName = 0;
	i32 checkHeight = 0;
...
//====================================================================
i32 CScriptBind_AI::CreateTempGenericShapeBox(IFunctionHandler* pH)
{
	Vec3 c(0, 0, 0);
	float r = 0;
	float height = 0;
	...
	tukk name = GetAISystem()->CreateTemporaryGenericShape(points, 4, height, type);
	if (name)
		return pH->EndFunction(name);
	return pH->EndFunction();
}
...
//====================================================================
i32 CScriptBind_AI::SetTerritoryShapeName(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	//retrieve the entity
	if (pEntity)
...
//====================================================================
i32 CScriptBind_AI::NotifySurpriseEntityAction(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	float radius = 0;
	i32 note = 0;
	...
	// Send a signal to all entities that see the specified entity, that that entity have just done something miraculous.
	AutoAIObjectIter it(gAIEnv.pAIObjectUpr->GetFirstAIObjectInRange(OBJFILTER_TYPE, AIOBJECT_ACTOR, pEntity->GetWorldPos(), radius, false));
	for (; it->GetObject(); it->Next())
	{
...
//====================================================================
i32 CScriptBind_AI::DebugReportHitDamage(IFunctionHandler* pH)
{
...
// CanMelee
//====================================================================
i32 CScriptBind_AI::CanMelee(IFunctionHandler* pH)
{
	GET_ENTITY(1);
...
// IsMoving
//====================================================================
i32 CScriptBind_AI::IsMoving(IFunctionHandler* pH)
{
	GET_ENTITY(1);
	if (!pEntity)
		return pH->EndFunction(0);
	...
	case 2:
		...
		break;
	default:
		run = (i32)AISPEED_SPRINT;
		break;
	}

	i32 idx = MovementUrgencyToIndex(pActor->GetState().fMovementUrgency);
...
// IsEnabled
//====================================================================
i32 CScriptBind_AI::IsEnabled(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	IAIObject* pAIObject = NULL;
	...
	else if (pH->GetParamType(1) == svtString)
	{
		tukk objName = "";
		pH->GetParam(1, objName);
		pAIObject = (IAIObject*)gAIEnv.pAIObjectUpr->GetAIObjectByName(0, objName);
	}
...
// EnableCoverFire
//====================================================================
i32 CScriptBind_AI::EnableCoverFire(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
// EnableFire
//====================================================================
i32 CScriptBind_AI::EnableFire(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
// EnableFire
//====================================================================
i32 CScriptBind_AI::IsFireEnabled(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
// CanFireInStance
//====================================================================
i32 CScriptBind_AI::CanFireInStance(IFunctionHandler* pH)
{
...
//====================================================================
i32 CScriptBind_AI::SetUseSecondaryVehicleWeapon(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS_MIN(2);
...
//
//====================================================================
i32 CScriptBind_AI::Animation(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
	if (!pEntity)
...
//
i32 CScriptBind_AI::SetAnimationTag(
  IFunctionHandler* pH, ScriptHandle entityID, tukk tagName)
{
	...
	u32 tagCrc = CCrc32::ComputeLowercase(tagName);
...
//	Returns:	A default result code (in Lua: void).
//
i32 CScriptBind_AI::ClearAnimationTag(
  IFunctionHandler* pH, ScriptHandle entityID, tukk tagName)
{
	IEntity* pEntity = gEnv->pEntitySystem->GetEntity((EntityId)entityID.n);
	if (pEntity == NULL)
...
i32 CScriptBind_AI::ProcessBalancedDamage(IFunctionHandler* pH)
{
...
//
//====================================================================
i32 CScriptBind_AI::SetRefpointToAnchor(IFunctionHandler* pH)
{
	...
	i32 findMethod = AIANCHOR_NEAREST;
	...
	AutoAIObjectIter it(gAIEnv.pAIObjectUpr->GetFirstAIObjectInRange(OBJFILTER_TYPE, findType, searchPos, rangeMax, false));
	for (; it->GetObject(); it->Next())
	{
		IAIObject* obj = it->GetObject();
		if (!obj->IsEnabled()) continue;
		...
		{
			ray_hit hit;
			i32 hitsCount = RayWorldIntersectionWrapper(pos, targetPos - pos, ent_static | ent_terrain | ent_sleeping_rigid | ent_rigid,
...
//
//====================================================================
i32 CScriptBind_AI::SetRefpointToPunchableObject(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
...
//
//====================================================================
i32 CScriptBind_AI::MeleePunchableObject(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	...
	if (Distance::Point_PointSq(pEntity->GetWorldPos(), origPos) < 0.5f)
	{
		if (pEntity && pEntity->GetPhysics())
		{
			...
			Vec3 pt = standPos;
			pt.x += drx_random(-0.3f, 0.3f);
			pt.y += drx_random(-0.3f, 0.3f);
			pt.z += 0.6f;
...
//
//====================================================================
i32 CScriptBind_AI::IsPunchableObjectValid(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
...
bool CScriptBind_AI::GetGroupSpatialProperties(IAIObject* pAI, float& offset, Vec3& avgGroupPos, Vec3& targetPos, Vec3& dirToTarget, Vec3& normToTarget)
{
	...
	AutoAIObjectIter it(gAIEnv.pAIObjectUpr->GetFirstAIObject(OBJFILTER_GROUP, groupId));
	for (; it->GetObject(); it->Next())
	{
...
//
//====================================================================
i32 CScriptBind_AI::GetDirLabelToPoint(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
	if (!pEntity)
...
//
//====================================================================
i32 CScriptBind_AI::GetPredictedPosAlongPath(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
	if (!pEntity)
...
// GetBiasedDirection
//====================================================================
i32 CScriptBind_AI::GetBiasedDirection(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
...
// SetAttentiontarget
//====================================================================
i32 CScriptBind_AI::SetAttentiontarget(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::RegisterInterestingEntity(IFunctionHandler* pH, ScriptHandle entityId, float radius, float baseInterest, tukk aiAction, Vec3 offset, float pause, i32 shared)
{
	IEntity* pEntity = gEnv->pEntitySystem->GetEntity((EntityId)entityId.n);
	if (!pEntity)
		return pH->EndFunction();

	ICentralInterestUpr* pCIM = gEnv->pAISystem->GetCentralInterestUpr();
	pCIM->ChangeInterestingEntityProperties(pEntity, radius, baseInterest, aiAction, offset, pause, shared);

	return pH->EndFunction();
}

i32 CScriptBind_AI::UnregisterInterestingEntity(IFunctionHandler* pH, ScriptHandle entityId)
{
	IEntity* pEntity = gEnv->pEntitySystem->GetEntity((EntityId)entityId.n);
	if (!pEntity)
		return pH->EndFunction();
...
i32 CScriptBind_AI::RegisterInterestedActor(IFunctionHandler* pH, ScriptHandle entityId, float fInterestFilter, float fAngleInDegrees)
{
	IEntity* pEntity = gEnv->pEntitySystem->GetEntity((EntityId)entityId.n);
	if (!pEntity)
		return pH->EndFunction();
...
i32 CScriptBind_AI::UnregisterInterestedActor(IFunctionHandler* pH, ScriptHandle entityId)
{
	IEntity* pEntity = gEnv->pEntitySystem->GetEntity((EntityId)entityId.n);
	if (!pEntity)
		return pH->EndFunction();
...
// IsTakingCover
//====================================================================
i32 CScriptBind_AI::IsTakingCover(IFunctionHandler* pH)
{
	GET_ENTITY(1);
...
// IsMovingToCover
//====================================================================
i32 CScriptBind_AI::IsMovingToCover(IFunctionHandler* pH)
{
	GET_ENTITY(1);
...
// IsMovingInCover
//====================================================================
i32 CScriptBind_AI::IsMovingInCover(IFunctionHandler* pH)
{
	GET_ENTITY(1);
...
// IsCoverCompromised
//====================================================================
i32 CScriptBind_AI::IsCoverCompromised(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::SetCoverCompromised(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::IsInCover(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
i32 CScriptBind_AI::SetInCover(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
// Описание: Plays communication on the AI agent.
// Arguments:
...
i32 CScriptBind_AI::PlayCommunication(IFunctionHandler* pH, ScriptHandle entityId, tukk commName,
                                      tukk channelName, float contextExpiry)
{
	...
	CCommunicationUpr& commUpr = *gAIEnv.pCommunicationUpr;

	SCommunicationRequest request;

	request.actorID = static_cast<EntityId>(entityId.n);
	request.channelID = commUpr.GetChannelID(channelName);
	request.commID = commUpr.GetCommunicationID(commName);
	request.configID = commUpr.GetConfigID(proxy->GetCommunicationConfigName());
	...
	ScriptHandle playID(commUpr.PlayCommunication(request));
...
// Описание: Stops given communication.
// Arguments:
//	playID - The id of the communication to stop.
i32 CScriptBind_AI::StopCommunication(IFunctionHandler* pH, ScriptHandle playID)
{
	gAIEnv.pCommunicationUpr->StopCommunication(CommPlayID(static_cast<u32>(playID.n)));
	return pH->EndFunction();
}
...
// IsOutOfAmmo
//====================================================================
i32 CScriptBind_AI::IsOutOfAmmo(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
// IsOutOfAmmo
//====================================================================
i32 CScriptBind_AI::IsLowOnAmmo(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
//====================================================================
i32 CScriptBind_AI::EnableWeaponAccessory(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(3);
	GET_ENTITY(1);
...
// ResetAgentState
//====================================================================
i32 CScriptBind_AI::ResetAgentState(IFunctionHandler* pH, ScriptHandle entityId, tukk stateLabel)
{
...
//====================================================================
i32 CScriptBind_AI::RegisterTacticalPointQuery(IFunctionHandler* pH)
{
	...
	i32 result = CreateQueryFromTacticalSpec(specTable);
	return pH->EndFunction(result);
}
...
// AutoDisable
//====================================================================
i32 CScriptBind_AI::AutoDisable(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
// CheckMeleeDamage
//====================================================================
i32 CScriptBind_AI::CheckMeleeDamage(IFunctionHandler* pH)
{
...
// GetTacticalPoints
//====================================================================
i32 CScriptBind_AI::GetTacticalPoints(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS_MIN(3);
	GET_ENTITY(1);
	...
	if (queryID)
	{
		...
		i32 nOptionUsed = pTPS->SyncQuery(queryID, context, point);
...
// Return:
//
i32 CScriptBind_AI::DestroyAllTPSQueries(IFunctionHandler* pH)
{
...
i32 CScriptBind_AI::CreateQueryFromTacticalSpec(SmartScriptTable specTable)
{
	ITacticalPointSystem* pTPS = gAIEnv.pTacticalPointSystem;
	i32 option = 0;
	...
	i32 queryID = pTPS->CreateQueryID(queryName);
...
// GetObjectBlackBoard
//====================================================================
i32 CScriptBind_AI::GetObjectBlackBoard(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
...
//====================================================================
i32 CScriptBind_AI::GetBehaviorBlackBoard(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
...
i32 CScriptBind_AI::SetBehaviorVariable(IFunctionHandler* pH, ScriptHandle entityId, tukk variableName, bool value)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity((EntityId)entityId.n))
	{
...
i32 CScriptBind_AI::GetBehaviorVariable(IFunctionHandler* pH, ScriptHandle entityId, tukk variableName)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity((EntityId)entityId.n))
	{
		if (CAIActor* actor = CastToCAIActorSafe(entity->GetAI()))
...
// GoTo
//====================================================================
i32 CScriptBind_AI::GoTo(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
// SetSpeed
//====================================================================
i32 CScriptBind_AI::SetSpeed(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(2);
	GET_ENTITY(1);
...
// SetEntitySpeedRange
//====================================================================
i32 CScriptBind_AI::SetEntitySpeedRange(IFunctionHandler* pH)
{
	bool ret = false;
	...
	if (pEntity)
	{
		...
		{
			u32 stancestart = 0;
			u32 stanceend = AgentMovementSpeeds::AMS_NUM_VALUES;

			i32 count = pH->GetParamCount();

			//now get the desired urgency
			u32 urgency = AgentMovementSpeeds::AMU_NUM_VALUES;
			...
			if (AgentMovementSpeeds::AMS_NUM_VALUES > stancestart)
			{
				if (CAIActor* pAIActor = aiObject->CastToCAIActor())
				{
					...
					for (u32 stance = stancestart; stanceend > stance; ++stance)
					{
...
{
public:
	CRecursiveScriptLoader(tukk ext, tukk tag, tukk folderName, SmartScriptTable tbl)
	{
		...
		if (handle > -1)
		{
			...
			do
			{
...

				{
					...
					string line(reinterpret_cast<tukk >(p), reinterpret_cast<tukk >(nl));
...
i32 CScriptBind_AI::SetAlarmed(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
	GET_ENTITY(1);
...
struct DirectoryExplorer
{
	...
	u32 operator()(tukk rootFolderName, tukk extension, Action& action)
	{
...
{
	BehaviorExplorer(tukk _createFuncName)
	...
	struct BehaviorInfo
	{
		BehaviorInfo(tukk base, tukk file)
			: baseName(base)
			, fileName(file)
			, loaded(false)
		{
	...
	void operator()(tukk fileName)
	{
		...
		tukk discoveredBehaviorName = "__DiscoveredBehavior";
...
{
	bool CheckCyclicDeps(BehaviorExplorer::Behaviors& behaviors, tukk behaviorName,
	                     BehaviorExplorer::BehaviorInfo& behaviorInfo)
	{
		tukk baseName = behaviorInfo.baseName.c_str();
		tukk baseFileName = behaviorInfo.fileName.c_str();
	...
	bool operator()(BehaviorExplorer::Behaviors& behaviors, tukk behaviorName,
	                BehaviorExplorer::BehaviorInfo& behaviorInfo)
	{
...
i32 CScriptBind_AI::LoadBehaviors(IFunctionHandler* pH, tukk folderName, tukk extension)
{
	...
	for (; it != end; ++it)
	{
		...
		{
			DRX_ASSERT_TRACE(false, ("Couldn't load behavior %s", it->first.c_str()));
			++numFailed;
		}
	}
...
#ifdef USE_DEPRECATED_AI_CHARACTER_SYSTEM
i32 CScriptBind_AI::LoadCharacters(IFunctionHandler* pH, tukk folderName, SmartScriptTable tbl)
{
...
i32 CScriptBind_AI::IsLowHealthPauseActive(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity((EntityId)entityID.n))
	{
		if (IAIObject* aiObject = entity->GetAI())
...
i32 CScriptBind_AI::GetPreviousBehaviorName(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityID.n)))
	{
		if (IAIObject* pAI = pEntity->GetAI())
...
i32 CScriptBind_AI::GetPeakThreatLevel(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityID.n)))
	{
		if (IAIObject* pAI = pEntity->GetAI())
...
i32 CScriptBind_AI::GetPeakThreatType(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityID.n)))
	{
		if (IAIObject* pAI = pEntity->GetAI())
...
i32 CScriptBind_AI::GetPreviousPeakThreatLevel(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityID.n)))
	{
		if (IAIObject* pAI = pEntity->GetAI())
...
i32 CScriptBind_AI::GetPreviousPeakThreatType(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityID.n)))
	{
		if (IAIObject* pAI = pEntity->GetAI())
...
i32 CScriptBind_AI::SetContinuousMotion(IFunctionHandler* pH, ScriptHandle entityID, bool continuousMotion)
{
...
i32 CScriptBind_AI::LoadGoalPipes(IFunctionHandler* pH)
{
	SCRIPT_CHECK_PARAMETERS(1);
...
i32 CScriptBind_AI::CheckForFriendlyAgentsAroundPoint(IFunctionHandler* pH, ScriptHandle entityID, Vec3 point, float radius)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityID.n)))
	{
		if (IAIObject* ai = entity->GetAI())
		{
			...
			AutoAIObjectIter itFriendly(gAIEnv.pAIObjectUpr->GetFirstAIObject(OBJFILTER_FACTION, ai->GetFactionID()));
...
i32 CScriptBind_AI::EnableUpdateLookTarget(IFunctionHandler* pH, ScriptHandle entityID, bool bEnable)
{
	if (IEntity* pEntity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityID.n)))
	{
		if (IAIObject* pAIObject = pEntity->GetAI())
...
i32 CScriptBind_AI::UpdateGlobalPerceptionScale(IFunctionHandler* pH, float visualScale, float audioScale)
{
...
i32 CScriptBind_AI::QueueBubbleMessage(IFunctionHandler* pH, ScriptHandle entityID, tukk message)
{
...
i32 CScriptBind_AI::SequenceBehaviorReady(IFunctionHandler* pH, ScriptHandle entityId)
{
	GetAISystem()->GetSequenceUpr()->SequenceBehaviorReady((EntityId)entityId.n);
	return pH->EndFunction();
}

i32 CScriptBind_AI::SequenceInterruptibleBehaviorLeft(IFunctionHandler* pH, ScriptHandle entityId)
{
	GetAISystem()->GetSequenceUpr()->SequenceInterruptibleBehaviorLeft((EntityId)entityId.n);
	return pH->EndFunction();
}

i32 CScriptBind_AI::SequenceNonInterruptibleBehaviorLeft(IFunctionHandler* pH, ScriptHandle entityId)
{
	GetAISystem()->GetSequenceUpr()->SequenceNonInterruptibleBehaviorLeft((EntityId)entityId.n);
	return pH->EndFunction();
}

i32 CScriptBind_AI::SetCollisionAvoidanceRadiusIncrement(IFunctionHandler* pH, ScriptHandle entityId, float radius)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityId.n)))
	{
		if (IAIObject* aiObject = entity->GetAI())
...
i32 CScriptBind_AI::RequestToStopMovement(IFunctionHandler* pH, ScriptHandle entityID)
{
...
i32 CScriptBind_AI::GetDistanceToClosestGroupMember(IFunctionHandler* pH, ScriptHandle entityIdHandle)
{
	...
	if (IEntity* myEntity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityIdHandle.n)))
	{
		if (IAIObject* me = myEntity->GetAI())
		{
			...
			{
				i32k groupId = me->GetGroupId();
				i32k groupCount = aiSystem->GetGroupCount(groupId, IAISystem::GROUP_ENABLED);
...
i32 CScriptBind_AI::IsAimReady(IFunctionHandler* pH, ScriptHandle entityIdHandle)
{
	if (IEntity* myEntity = gEnv->pEntitySystem->GetEntity(static_cast<EntityId>(entityIdHandle.n)))
	{
		if (IAIObject* me = myEntity->GetAI())
...
i32 CScriptBind_AI::AllowLowerBodyToTurn(IFunctionHandler* pH, ScriptHandle entityID, bool bAllowLowerBodyToTurn)
{
...
i32 CScriptBind_AI::GetGroupScopeUserCount(IFunctionHandler* pH, ScriptHandle entityIdHandle, tukk groupScopeName)
{
...
i32 CScriptBind_AI::StartModularBehaviorTree(IFunctionHandler* pH, ScriptHandle entityIdHandle, tukk treeName)
{
	gAIEnv.pBehaviorTreeUpr->StartModularBehaviorTree(static_cast<EntityId>(entityIdHandle.n), treeName);
	return pH->EndFunction();
}

i32 CScriptBind_AI::StopModularBehaviorTree(IFunctionHandler* pH, ScriptHandle entityIdHandle)
{
	gAIEnv.pBehaviorTreeUpr->StopModularBehaviorTree(static_cast<EntityId>(entityIdHandle.n));
	return pH->EndFunction();
}

i32 CScriptBind_AI::SetLastOpResult(IFunctionHandler* pH, ScriptHandle entityIdHandle, ScriptHandle targetEntityIdHandle)
{
	EntityId entityId = static_cast<EntityId>(entityIdHandle.n);
	IEntity* entity = gEnv->pEntitySystem->GetEntity(entityId);
	IF_UNLIKELY (!entity)
