...
#include <drx3D/AI/AIFlyingVehicle.h>
#include <drx3D/AI/AIPlayer.h>
#include <drx3D/AI/AIObjectIterators.h>//TODO get rid of this file totally!
#include <drx3D/AI/ObjectContainer.h>
#include <drx3D/AI/TargetTrackUpr.h>
#include <drx3D/AI/AIEntityComponent.h>
...
CAIObject* SAIObjectCreationHelper::RecreateObject(uk pAlloc /*=NULL*/)
{
	...
	assert(gAIEnv.pAIObjectUpr->GetAIObject(objectId) == NULL);
...
void CAIObjectUpr::Init()
{
}

void CAIObjectUpr::Reset(bool includingPooled /*=true*/)
{
	m_mapDummyObjects.clear();
	m_Objects.clear();
}

IAIObject* CAIObjectUpr::CreateAIObject(const AIObjectParams& params)
{
	CCCPOINT(CreateAIObject);
	DRX_ASSERT(params.type != 0);
	...
	{
	case AIOBJECT_DUMMY:
		DRX_ASSERT_MESSAGE(false, "Creating dummy object through the AI object manager (use CAISystem::CreateDummyObject instead)");
		return 0;
	case AIOBJECT_ACTOR:
		type = AIOBJECT_ACTOR;
		pObject = new CPuppet;
	...
	case AIOBJECT_LEADER:
		{
			i32 iGroup = params.association ? static_cast<CPuppet*>(params.association)->GetGroupId() : -1;
	...
	case AIOBJECT_HELICOPTERDRXSIS2:
		...
		pObject->SetSubType(CAIObject::STP_HELIDRXSIS2);
		break;
	default:
...
void CAIObjectUpr::CreateDummyObject(CCountedRef<CAIObject>& ref, tukk name, CAIObject::ESubType type, tAIObjectID requiredID /*= INVALID_AIOBJECTID*/)
{
...
void CAIObjectUpr::CreateDummyObject(CStrongRef<CAIObject>& ref, tukk name /*=""*/, CAIObject::ESubType type /*= CAIObject::STP_NONE*/, tAIObjectID requiredID /*= INVALID_AIOBJECTID*/)
{
	...
	AILogComment("CAIObjectUpr::CreateDummyObject %s (%p)", pObject->GetName(), pObject);
}

void CAIObjectUpr::RemoveObject(const tAIObjectID objectID)
{
	EntityId entityId = 0;
...
void CAIObjectUpr::RemoveObjectByEntityId(const EntityId entityId)
{
	AIObjectOwners::iterator it = m_Objects.begin();
	AIObjectOwners::iterator itEnd = m_Objects.end();
	for (; it != itEnd; ++it)
...
IAIObject* CAIObjectUpr::GetAIObject(tAIObjectID aiObjectID)
{
	return gAIEnv.pObjectContainer->GetAIObject(aiObjectID);
}

IAIObject* CAIObjectUpr::GetAIObjectByName(unsigned short type, tukk pName) const
{
...
CAIObject* CAIObjectUpr::GetAIObjectByName(tukk pName) const
{
...
IAIObjectIter* CAIObjectUpr::GetFirstAIObject(EGetFirstFilter filter, short n)
{
...
IAIObjectIter* CAIObjectUpr::GetFirstAIObjectInRange(EGetFirstFilter filter, short n, const Vec3& pos, float rad, bool check2D)
{
	if (filter == OBJFILTER_GROUP)
	{
...
void CAIObjectUpr::OnObjectRemoved(CAIObject* pObject)
{
...
//
//-----------------------------------------------------------------------------------------------------------
void CAIObjectUpr::RemoveObjectFromAllOfType(i32 nType, CAIObject* pRemovedObject)
{
