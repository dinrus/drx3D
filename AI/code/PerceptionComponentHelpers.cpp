...
bool SLocation::Validate(IEntity* pEntity, tukk szOwnerName) const
{
	DRX_ASSERT(szOwnerName);
	...
	if (type == EType::Bone)
	{
		...
		if (!pCharacter)
		{
			DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_WARNING, "Cannot retrieve bone '%s' for '%s' (%s) because character instance is not available!", boneName.c_str(), pEntity->GetName(), szOwnerName);
			return false;
		}

		i32k boneId = pCharacter->GetIDefaultSkeleton().GetJointIDByName(boneName);
		if (boneId < 0)
		{
			DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_WARNING, "Cannot retrieve bone '%s' for '%s' (%s) because it could not be found on the default skeleton!", boneName.c_str(), pEntity->GetName(), szOwnerName);
			return false;
		}
		...
		if (!pSkeletonPose)
		{
			DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_WARNING, "Cannot retrieve bone '%s' for '%s' (%s) because there is no skeleton pose for the character!", boneName.c_str(), pEntity->GetName(), szOwnerName);
			return false;
		}
	}
	return true;
