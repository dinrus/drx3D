...
#include <drx3D/AI/FlyHelpers_Tactical.h>
#include <drx3D/AI/FlyHelpers_PathFollower.h>
...
ILINE void SendEvent(CPipeUser* pPipeUser, tukk eventName)
{
	DRX_ASSERT(pPipeUser);
	DRX_ASSERT(eventName);
...
{
bool GetClampedXmlAttribute(const XmlNodeRef& xml, tukk const attributeName, float& valueInOut, const float minValue, const float maxValue)
{
	DRX_ASSERT(minValue <= maxValue);
...
void RegisterBehaviorTreeNodesHelicopter()
{
	...
	BehaviorTree::IBehaviorTreeUpr& manager = *gAIEnv.pBehaviorTreeUpr;
