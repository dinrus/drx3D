...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   CAISystemPhys.cpp
   $Id$
   Описание:	should contaioin all the methods of CAISystem which have to deal with Physics
...
float CAISystem::GetWaterOcclusionValue(const Vec3& targetPos) const
{
	...
	if (gAIEnv.CVars.WaterOcclusionEnable > 0)
	{
		...
		if (inWaterDepth > FLT_EPSILON)
		{
			...
			i32 areaCount = GetPhysicalEntitiesInBox(targetPos, targetPos, areas, ent_areas);
...
{
	i32 newFlags = AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOLID_COVER | AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOFT_COVER;
