...
i32 WorldMonitor::StateChangeHandler(const EventPhys* pPhysEvent)
{
...

	{
		...
		if (consider)
		{
			...
			{
				i32 entityId = gEnv->pPhysicalWorld->GetPhysicalEntityId(event->pEntity);
...
i32 WorldMonitor::EntityRemovedHandler(const EventPhys* pPhysEvent)
{
	WorldMonitor* pthis = gAIEnv.pNavigationSystem->GetWorldMonitor();
...
i32 WorldMonitor::EntityRemovedHandlerAsync(const EventPhys* pPhysEvent)
{
	WorldMonitor* pthis = gAIEnv.pNavigationSystem->GetWorldMonitor();
