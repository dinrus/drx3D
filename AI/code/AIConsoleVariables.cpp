...
void AIConsoleVars::Init()
{
	...
	REGISTER_CVAR2("ai_CommunicationUprHeighThresholdForTargetPosition", &CommunicationUprHeightThresholdForTargetPosition, 5.0f, VF_CHEAT | VF_CHEAT_NOCHECK,
	...
	DefineConstIntCVarName("ai_Recorder_Buffer", DebugRecordBuffer, 1024, VF_CHEAT | VF_CHEAT_NOCHECK,
...
void AIConsoleVars::CommTest(IConsoleCmdArgs* args)
{
	...
	i32 variationNumber = 0;

	if (args->GetArgCount() > 3)
		variationNumber = atoi(args->GetArg(3));

	gAIEnv.pCommunicationUpr->GetTestUpr().StartFor(actor->GetEntityID(), commName, variationNumber);
}
...
void AIConsoleVars::CommTestStop(IConsoleCmdArgs* args)
{
	...
	gAIEnv.pCommunicationUpr->GetTestUpr().Stop(actor->GetEntityID());
}

void AIConsoleVars::CommWriteStats(IConsoleCmdArgs* args)
{
	gAIEnv.pCommunicationUpr->WriteStatistics();
}

void AIConsoleVars::CommResetStats(IConsoleCmdArgs* args)
{
	gAIEnv.pCommunicationUpr->ResetStatistics();
}
...
void AIConsoleVars::DebugMNMAgentType(IConsoleCmdArgs* args)
{
	...
	tukk agentTypeName = args->GetArg(1);
...
struct PotentialDebugAgent
{
	Vec3        entityPosition;
	EntityId    entityId;
	tukk name;     // Pointer directly to the entity name
...
{
	// Add ai actors
	AutoAIObjectIter it(gEnv->pAISystem->GetAIObjectUpr()->GetFirstAIObject(OBJFILTER_TYPE, AIOBJECT_ACTOR));
	...
	BehaviorTree::BehaviorTreeUpr* behaviorTreeUpr = gAIEnv.pBehaviorTreeUpr;
	const size_t count = behaviorTreeUpr->GetTreeInstanceCount();
	for (size_t index = 0; index < count; ++index)
	{
		const EntityId entityId = behaviorTreeUpr->GetTreeInstanceEntityIdByIndex(index);
