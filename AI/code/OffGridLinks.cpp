...
// This needs to be static because OffMeshNavigationUpr expects all link ids to be
...
void MNM::OffMeshNavigation::TileLinks::CopyLinks(TriangleLink* links, u16 linkCount)
{
...
void MNM::OffMeshNavigation::AddLink(NavigationMesh& navigationMesh, const TriangleID startTriangleID, const TriangleID endTriangleID, OffMeshLinkID& linkID)
{
	...
	DRX_ASSERT_TRACE(tileLinks.triangleLinkCount + 1 < kMaxTileLinks, ("Maximum number of offmesh links in tile reached! (tileID = %u, links count = %d)", tileID, tileLinks.triangleLinkCount));

	//////////////////////////////////////////////////////////////////////////
	// Find if we have entries for this triangle, if not insert at the end
	u16 triangleIdx = tileLinks.triangleLinkCount;  // Default to end
	{
		for (u16 idx = 0; idx < tileLinks.triangleLinkCount; ++idx)
		{
	...
	DRX_ASSERT_TRACE(linkID <= s_linkIDGenerator, ("A link ID should not be larger than the latest generated (linkID = %u, lastGenerated = %u)", linkID, s_linkIDGenerator));
	...
	i32k diffCount = tileLinks.triangleLinkCount - triangleIdx;
	if (diffCount > 0)
	{
	...
	if (triangleIdx < (tileLinks.triangleLinkCount - 1))
	{
		...
		for (u16 idx = triangleIdx + 1; idx < tileLinks.triangleLinkCount; ++idx)
		{
...
void MNM::OffMeshNavigation::RemoveLink(NavigationMesh& navigationMesh, const TriangleID boundTriangleID, const OffMeshLinkID linkID)
{
	...
	if (tileLinksIt != m_tilesLinks.end())
	{
		...
		u16 copyCount = 0;
		for (u16 triIdx = 0; triIdx < tileLinks.triangleLinkCount; ++triIdx)
		{
...
MNM::OffMeshNavigation::QueryLinksResult MNM::OffMeshNavigation::GetLinksForTriangle(const TriangleID triangleID, u16k index) const
{
	...
	if (tileCit != m_tilesLinks.end())
	{
		...
		if (index < tileLinks.triangleLinkCount)
		{
			...
			for (u16 triIdx = index + 1; triIdx < tileLinks.triangleLinkCount; ++triIdx)
			{
...
MNM::OffMeshNavigation::ProfileMemoryStats MNM::OffMeshNavigation::GetMemoryStats(IDrxSizer* pSizer) const
{
