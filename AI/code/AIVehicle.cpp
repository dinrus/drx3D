...
void CAIVehicle::Update(EUpdateType type)
{
	...
	if (!m_bDryUpdate)
	{
		DRX_PROFILE_REGION(PROFILE_AI, "AI system vehicle full update");
...
Vec3 CAIVehicle::GetError(const Vec3& vTargetPos, const Vec3& vFirePos, const float fAccuracy)
{
	...
	if (fAccuracy < 1.0f)
	{
		...
		float rangeRotation = fAccuracyScale * drx_random(-50.0f, 50.0f);
		float zofs = dist * sinf(DEG2RAD(0.0f));

		float rangeLength;
		if (drx_random(0.0f, 0.99f) < fAccuracy)
			rangeLength = fAccuracyScale * drx_random(0.0f, 27.0f);
		else
			rangeLength = fAccuracyScale * drx_random(15.0f, 25.0f);
...
bool CAIVehicle::CheckExplosion(const Vec3& vTargetPos, const Vec3& vFirePos, const Vec3& vActuallFireDir, const float fDamageRadius)
{
	...
	if (gAIEnv.pWorld->RayWorldIntersection(
	      vFirePos, vDirVector, COVER_OBJECT_TYPES,
	...
	if (GetSubType() != CAIObject::STP_HELI)
	{
		...
		{
			if (gAIEnv.pWorld->RayWorldIntersection(
			      vFirePos, vActualDir, COVER_OBJECT_TYPES,
			      AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOLID_COVER,
			      &hit, 1, &skipList[0], skipList.size()))
			{
...
void CAIVehicle::OnDriverChanged(bool bEntered)
{
	...
	/*if (!bEntered)
	   {
	   gAIEnv.pAIObjectUpr->RemoveObjectFromAllOfType(AIOBJECT_ACTOR,this);
	   gAIEnv.pAIObjectUpr->RemoveObjectFromAllOfType(AIOBJECT_VEHICLE,this);
	   gAIEnv.pAIObjectUpr->RemoveObjectFromAllOfType(AIOBJECT_ATTRIBUTE,this);
	   }*/
}
...
bool CAIVehicle::GetEnemyTarget(i32 objectType, Vec3& hitPosition, float fDamageRadius2, CAIObject** pTarget)
{
	*pTarget = NULL;

	AIObjectOwners::iterator ai = gAIEnv.pAIObjectUpr->m_Objects.find(objectType);
	AIObjectOwners::iterator aiend = gAIEnv.pAIObjectUpr->m_Objects.end();
