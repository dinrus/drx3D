...
public:
	...
	struct RuntimeData
	{
		u32 runningChildren;     // Each bit is a child
		u32 successCount;
		u32 failureCount;
	...
	virtual void OnTerminate(const UpdateContext& context) override
	{
		...
		u32k runningChildren = runtimeData.runningChildren;
		for (size_t childIndex = 0, n = m_children.size(); childIndex < n; ++childIndex)
		{
	...
	virtual Status Update(const UpdateContext& context) override
	{
		...
		u32& runningChildren = runtimeData.runningChildren;
		u32& successCount = runtimeData.successCount;
		u32& failureCount = runtimeData.failureCount;
		...
		for (size_t childIndex = 0, n = m_children.size(); childIndex < n; ++childIndex)
		{
			...
			if (childIsRunning)
			{
				...
				else
				{
					...
					runningChildren = runningChildren & ~u32(1u << childIndex);
				}
			}
		}
...
public:
	struct RuntimeData
	{
		u8 amountOfTimesChildSucceeded;
...
private:
	u8 m_desiredRepeatCount;     // 0 means infinite
};
...
private:
	u8 m_maxAttemptCount;     // 0 means infinite
};
...
public:
	...
	virtual LoadResult LoadFromXml(const XmlNodeRef& xml, const LoadContext& context) override
	{
		...
		i32k defaultChildIndex = childCount - 1;
...
struct State
{
	...
	LoadResult LoadFromXml(const XmlNodeRef& stateXml, const LoadContext& context)
	{
		...
		if (transitionsXml)
		{
			for (i32 i = 0; i < transitionsXml->getChildCount(); ++i)
			{
...
public:
	...
	virtual LoadResult LoadFromXml(const XmlNodeRef& xml, const LoadContext& context) override
	{
		...
		for (i32 i = 0; i < xml->getChildCount(); ++i)
		{
			State state;
...
private:
	...
	LoadResult LinkAllTransitions()
	{
		for (States::iterator stateIt = m_states.begin(), end = m_states.end(); stateIt != end; ++stateIt)
		{
			...
			for (Transitions::iterator transitionIt = state.transitions.begin(), end = state.transitions.end(); transitionIt != end; ++transitionIt)
			{
				...
				{
#ifdef STORE_INFORMATION_FOR_STATE_MACHINE_NODE
				// не работает	gEnv->pLog->LogError("Cannot transition to unknown state '%s' at line %d.", transition.destinationStateName, transition.xmlLine);
#endif
					return LoadFailure;
				}
	...
	StateIndex GetIndexOfState(u32 stateNameLowerCaseCRC32)
	{
...
protected:
	virtual void OnInitialize(const UpdateContext& context) override
	{
		gAIEnv.pBehaviorTreeUpr->HandleEvent(context.entityId, m_eventToSend);
	}
...
protected:
	virtual void OnInitialize(const UpdateContext& context) override
	{
		...
		const float randomValue = drx_random(0.0f, 1.0f);
...
public:
	...
	virtual LoadResult LoadFromXml(const XmlNodeRef& xml, const LoadContext& context) override
	{
		tukk timestampName = xml->getAttr("since");
		if (!timestampName)
		{
...
protected:
	...
	{
#ifdef STORE_LOG_MESSAGE
// не работает		stack_string textBuffer;
...
public:
	...
	{
		virtual bool RunBehavior(EntityId entityId, tukk behaviorName, XmlNodeRef behaviorXmlNode) override
		{
			...
			behaviorTreeInstance = gAIEnv.pBehaviorTreeUpr->CreateBehaviorTreeInstanceFromXml(behaviorName, behaviorXmlNode);
			if (behaviorTreeInstance.get() != NULL)
			{
#ifdef USING_BEHAVIOR_TREE_NODE_CUSTOM_DEBUG_TEXT
	...
	{
		RuntimeData& runtimeData = GetRuntimeData<RuntimeData>(context);
		gAIEnv.pGraftUpr->GraftNodeReady(context.entityId, &runtimeData);
	}
	...
	virtual Status Update(const UpdateContext& context) override
	{
		...
		{
			gAIEnv.pGraftUpr->GraftBehaviorComplete(context.entityId);
	...
	virtual void OnTerminate(const UpdateContext& context) override
	{
		...
		gAIEnv.pGraftUpr->GraftNodeTerminated(context.entityId);
	}
...
void RegisterBehaviorTreeNodes_Core()
{
	...
	tukk COLOR_FAIL = "ff0000";
	tukk COLOR_DEBUG = "000000";
	tukk COLOR_TIME = "ffffff";
