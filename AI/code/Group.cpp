...
u32 Group::GetMemberCount() const
{
...
void Group::Update(float updateTime)
{
	...
	for (; mit != mend; ++mit)
	{
		...
		if (IAIObject* target = actor->GetAttentionTarget())
		{
			...
			u32 targetThreat = actor->GetAttentionTargetThreat();
			u32 targetType = actor->GetAttentionTargetType();
			u32 targetObjType = targetObject->GetType();
			...
			u32 infoCount = s_targetInfos.size();
	...
	if (!s_targetInfos.empty())
	{
		...
		if (m_target.IsNil() || !m_target.GetAIObject())
			gAIEnv.pAIObjectUpr->CreateDummyObject(m_target, "");
		...
		if (currentID != info.targetID)
		{
			...
			{
				char targetName[256];
				drx_sprintf(targetName, "Group Target [%s]", groupTarget->GetName());
	...
	if (m_targetType != m_prevTargetType)
	{
		...
		{
		case AITARGET_NONE:
			gAIEnv.pGroupUpr->NotifyGroup(m_groupID, 0, "OnGroupTargetNone");
			break;
		case AITARGET_SOUND:
			gAIEnv.pGroupUpr->NotifyGroup(m_groupID, 0, "OnGroupTargetSound");
			break;
		case AITARGET_MEMORY:
			gAIEnv.pGroupUpr->NotifyGroup(m_groupID, 0, "OnGroupTargetMemory");
			break;
		case AITARGET_VISUAL:
			gAIEnv.pGroupUpr->NotifyGroup(m_groupID, 0, "OnGroupTargetVisual");
			break;
		default:
			break;
		}
...
void Group::Notify(const NotificationID& notificationID, tAIObjectID recipientID, tukk name)
{
