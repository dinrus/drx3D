...
VisionID CVisionMap::CreateVisionID(tukk name)
{
	++m_visionIdCounter;
...
void CVisionMap::ObserverChanged(const ObserverID& observerID, const ObserverParams& newObserverParams, u32 hint)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	if (hint & eChangedSkipList)
	{
		...
		u32 skipListSize = std::min<u32>(newObserverParams.skipListSize, ObserverParams::MaxSkipListSize);
		...
		for (i32 i = 0; i < currentObserverParams.skipListSize; ++i)
...
void CVisionMap::ObservableChanged(const ObservableID& observableID, const ObservableParams& newObservableParams, u32 hint)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	if (hint & eChangedPosition)
	{
		...
		{
			DRX_PROFILE_REGION(PROFILE_AI, "CVisionMap::ObservableChanged_UpdateHashGrid");
			...
			for (i32 i = 1; i < currentObservableParams.observablePositionsCount; ++i)
			{
	...
	if (hint & eChangedSkipList)
	{
		DRX_PROFILE_REGION(PROFILE_AI, "CVisionMap::ObservableChanged_UpdateSkipList");

		assert(newObservableParams.skipListSize <= ObserverParams::MaxSkipListSize);

		u32 skipListSize = std::min<u32>(newObservableParams.skipListSize, ObserverParams::MaxSkipListSize);
		...
		for (i32 i = 0; i < currentObservableParams.skipListSize; ++i)
	...
	{
		DRX_PROFILE_REGION(PROFILE_AI, "CVisionMap::ObservableChanged_VisibilityChanged");
...
bool CVisionMap::IsInFoV(const ObserverInfo& observerInfo, const ObservableInfo& observableInfo) const
{
	...
	for (i32 i = 0; i < observableInfo.observableParams.observablePositionsCount; i++)
	{
...
RayCastRequest::Priority CVisionMap::GetRayCastRequestPriority(const ObserverParams& observerParams, const ObservableParams& observableParams)
{
	...
	u32 fromTypeMask = observerParams.typeMask;
	u32 toTypeMask = observableParams.typeMask;
	u8 fromFaction = observerParams.faction;
	u8 toFaction = observableParams.faction;
	...
	for (std::vector<PriorityMapEntry>::iterator it = m_priorityMap.begin(), itEnd = m_priorityMap.end(); it != itEnd; ++it)
	{
		    ...
		    (priorityMapEntry.toFactionMask & (1 << toFaction)))
		{
			...
			default:
				DRX_ASSERT("bad priority specified in vision map priority table");
				break;
			}
			break;
		}
...
void CVisionMap::UpdatePVS(ObserverInfo& observerInfo)
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_AI, "UpdatePVS_Step1");
	...
	{
		DRX_PROFILE_REGION(PROFILE_AI, "UpdatePVS_Step2");

		if (observerInfo.observerParams.sightRange > 0.0f)
		{
			// the sight range is defined query the obsevableGrid
			m_queryObservables.clear();
			u32 observableCount = m_observablesGrid.query_sphere_distance(observerInfo.observerParams.eyePosition, observerInfo.observerParams.sightRange, m_queryObservables);
...
bool CVisionMap::RayCastSubmit(const QueuedRayID& queuedRayID, RayCastRequest& rayCastRequest)
{
	...
	rayCastRequest.skipListCount = static_cast<u8>(skipListCount);
	...
	latencyInfo->bufferIndex = (latencyInfo->bufferIndex + 1) % DRX_ARRAY_COUNT(latencyInfo->buffer);
...
void CVisionMap::RayCastComplete(const QueuedRayID& queuedRayID, const RayCastResult& rayCastResult)
{
	...
	const bool requiresStatisticalAnalysis = (observableInfo.observableParams.mode == EObservableMode::Statistical) &&
		IsInStatisticalAnalysisSightRange(observerInfo, observableInfo);
	static_assert(static_cast<i32>(EObservableMode::Count) == 2, "Enum changed!");
...
void CVisionMap::AcquireSkipList(IPhysicalEntity** skipList, u32 skipListSize)
{
	for (u32 i = 0; i < skipListSize; ++i)
		skipList[i]->AddRef();
}

void CVisionMap::ReleaseSkipList(IPhysicalEntity** skipList, u32 skipListSize)
{
	for (u32 i = 0; i < skipListSize; ++i)
		skipList[i]->Release();
}
...
void CVisionMap::UpdateObservers()
{
	...
	i32 numberOfPVSUpdatesLeft = gAIEnv.CVars.VisionMapNumberOfPVSUpdatesPerFrame;
	...
	i32 numberOfVisibilityUpdatesLeft = gAIEnv.CVars.VisionMapNumberOfVisibilityUpdatesPerFrame;
...
void CVisionMap::DebugDrawVisionMapStats()
{
	...
	DisplayInfo displayInfoArray[DRX_ARRAY_COUNT(m_latencyInfo)];
	float cutOffTime = m_debugTimer - 10.0f;

	for (i32 i = 0; i < DRX_ARRAY_COUNT(m_latencyInfo); i++)
	{
		...
		for (i32 j = 0; j < latencyInfo->usedBufferSize; j++)
		{
	...
	i32 numObservers = m_observers.size();
	i32 numObservables = m_observables.size();
...
{
	{
		CAIObject* debugObserverAIObject = gAIEnv.pAIObjectUpr->GetAIObjectByName("VisionMapObserver");
	...
	{
		CAIObject* debugObservableAIObject = gAIEnv.pAIObjectUpr->GetAIObjectByName("VisionMapObservable");
