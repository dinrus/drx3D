...
void Voxelizer::Start(const AABB& volume, const Vec3& voxelSize)
{
	...
	m_voxelSpaceSize((i32)((spaceAbs.x * voxelConv.x) + 0.5f), (i32)((spaceAbs.y * voxelConv.y) + 0.5f), (i32)((spaceAbs.z * voxelConv.z) + 0.5f));
...
{
	return Vec3i((i32)vector.x, (i32)vector.y, (i32)vector.z);
}
...
void Voxelizer::RasterizeTriangle(const Vec3 v0World, const Vec3 v1World, const Vec3 v2World)
{
	...
	const Vec3i minVoxelIndex(Maximize<i32>(GetVec3iFromVec3(voxelMin), Vec3i(0)));
	const Vec3i maxVoxelIndex(Minimize<i32>(GetVec3iFromVec3(voxelMax), voxelSpaceSize - Vec3i(1)));
	...
	{
		for (i32 y = minVoxelIndex.y; y <= maxVoxelIndex.y; ++y)
		{
			...
			for (i32 x = minVoxelIndex.x; x <= maxVoxelIndex.x; ++x)
			{
				...
				for (i32 z = minVoxelIndex.z; z <= maxVoxelIndex.z; ++z)
				{
...
size_t WorldVoxelizer::ProcessGeometry(u32 hashValueSeed /* = 0 */, u32 hashTest /* = 0 */, u32* hashValue /* = 0 */, NavigationMeshEntityCallback pEntityCallback /* = NULL */)
{
	size_t triCount = 0;
	...
	i32 entityCount = gEnv->pPhysicalWorld->GetEntitiesInBox(m_volumeAABB.min, m_volumeAABB.max, entityList,
	...
	for (i32 i = 0; i < entityCount; ++i)
	{
		...
		while (entity->GetStatus(&sp))
		{
			if (sp.pGeomProxy && (sp.flagsOR & geom_colltype_player))
			{
				...
				{
					AABB aabb;
					u32 terrainHash = ComputeTerrainHashAndAABB(sp.pGeomProxy, aabb);
...
void WorldVoxelizer::VoxelizeGeometry(const Vec3* vertices, u32k* indices, size_t triCount, const Matrix34& worldTM)
{
	if (HasNoRotOrScale(Matrix33(worldTM)))
	{
		Vec3 offset = worldTM.GetTranslation();
...
u32 WorldVoxelizer::ComputeTerrainHashAndAABB(IGeometry* geometry, AABB& aabb)
{
	aabb.Reset();
	...
	i32k minX = max(0, (i32)((m_volumeAABB.min.x - phf->origin.x) * phf->stepr.x));
	i32k minY = max(0, (i32)((m_volumeAABB.min.y - phf->origin.y) * phf->stepr.y));
	i32k maxX = min((i32)((m_volumeAABB.max.x - phf->origin.x) * phf->stepr.x), phf->size.x);
	i32k maxY = min((i32)((m_volumeAABB.max.y - phf->origin.y) * phf->stepr.y), phf->size.y);
...
size_t WorldVoxelizer::VoxelizeTerrain(IGeometry* geometry, const Matrix34& worldTM)
{
	...
	i32k minX = max(0, (i32)((m_volumeAABB.min.x - phf->origin.x) * phf->stepr.x));
	i32k minY = max(0, (i32)((m_volumeAABB.min.y - phf->origin.y) * phf->stepr.y));
	i32k maxX = min((i32)((m_volumeAABB.max.x - phf->origin.x) * phf->stepr.x), (phf->size.x - 1));
	i32k maxY = min((i32)((m_volumeAABB.max.y - phf->origin.y) * phf->stepr.y), (phf->size.y - 1));
		...
		for (i32 y = minY; y <= maxY; ++y)
		{
			for (i32 x = minX; x <= maxX; ++x)
			{
				if (phf->fpGetSurfTypeCallback(x, y) != phf->typehole)
				{
					...
					DRX_ASSERT(faceCount < maxFacesCount);
