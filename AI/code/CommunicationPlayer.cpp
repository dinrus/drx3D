  ...
  IAICommunicationHandler::ECommunicationHandlerEvent event, EntityId actorID)
{
	...
	if (gAIEnv.CVars.DebugDrawCommunication == 5 && prevFlags != finishedFlags)
	{
		...
		{
			DrxLogAlways("CommunicationPlayer::PlayState: Still waiting for %x to finish commID[%u]", ~finishedFlags & FinishedAll, commID.id);
		}
		else
		{
			DrxLogAlways("CommunicationPlayer::PlayState: All finished! commID[%u]", commID.id);
		}
	}
}
...
void CommunicationPlayer::Reset()
{
	...
	for (; it != end; ++it)
	{
		...
		if (playState.soundInfo.playSoundControlId != DrxAudio::InvalidControlId)
			commHandler->StopSound(playState.soundInfo);

		if (playState.voiceInfo.playSoundControlId != DrxAudio::InvalidControlId)
...
bool CommunicationPlayer::Play(const CommPlayID& playID, const SCommunicationRequest& request,
                               const SCommunication& comm, u32 variationIdx, ICommunicationFinishedListener* listener,
                               uk param)
{
	...
	for (; it != end; )
	{
		...
		if (!UpdatePlaying(playId, playState, updateTime))
		{
			...
			if (gAIEnv.CVars.DebugDrawCommunication == 5)
				DrxLogAlways("CommunicationPlayer removed finished: %s[%u] as playID[%u] with listener[%p]", gAIEnv.pCommunicationUpr->GetCommunicationName(erased->second.commID), erased->second.commID.id, erased->first.id, erased->second.listener);
...
void CommunicationPlayer::Stop(const CommPlayID& playID)
{
	...
	if (it != m_playing.end())
	{
		...
		if (gAIEnv.CVars.DebugDrawCommunication == 5)
		{
			DrxLogAlways("CommunicationPlayer finished playing: %s[%u] as playID[%u]", gAIEnv.pCommunicationUpr->GetCommunicationName(playState.commID), playState.commID.id, it->first.id);
		}
