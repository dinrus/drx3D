...
void MNM::IslandConnections::SetTwoWayConnectionBetweenIslands(const MNM::GlobalIslandID islandId1, const MNM::AreaAnnotation islandAnnotation1, const MNM::GlobalIslandID islandId2, const MNM::AreaAnnotation islandAnnotation2, i32k connectionsChange)
{
	...
	if (findIt != firstIslandLinks.end())
	{
		...
		DRX_ASSERT(link.connectionsCount >= 0);
	...
	{
		DRX_ASSERT(connectionsChange > 0);
...
{
	IsLinkAssociatedWithObjectPredicate(u32k objectId)
		: m_objectId(objectId)
	{}
...
void MNM::IslandConnections::RemoveAllIslandConnectionsForObject(const NavigationMeshID& meshID, u32k objectId)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
bool MNM::IslandConnections::CanNavigateBetweenIslandsInternal(const IEntity* pEntityToTestOffGridLinks, const MNM::GlobalIslandID fromIsland, const MNM::GlobalIslandID toIsland, const TFilter& filter, TIslandsWay& way) const
{
	...
	while (!openList.IsEmpty())
	{
		...
		for (const Link& link : links)
		{
			...
			{
				const OffMeshLink* offmeshLink = offMeshNavigationUpr->GetOffMeshLink(link.offMeshLinkID);
...
void MNM::IslandConnections::DebugDraw() const
{
	...
	dc->Draw2dLabel(10.0f, 5.0f, 1.6f, Col_White, false, "Amount of global islands registered into the system %d", (i32)totalGlobalIslandsRegisterd);
