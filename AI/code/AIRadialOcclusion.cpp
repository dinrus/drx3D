...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   AIRadialOcclusion.cpp
   $Id$
   Описание:
...
void CAIRadialOcclusion::RasterizeCircle(const Vec3& pos, float r)
{
	...
	i32 ia0 = (i32)ceilf(a0 / gf_PI2 * SAMPLE_COUNT);
	i32 ia1 = (i32)floorf(a1 / gf_PI2 * SAMPLE_COUNT);

	if (ia1 >= ia0)
	{
		for (i32 i = ia0; i < ia1; ++i)
		{
			i32 x = i & SAMPLE_MASK;
			if (d < m_buffer[x])
				m_buffer[x] = d;
...
void CAIRadialOcclusion::RasterizeEdge(const Vec3& start, const Vec3& end, bool cull)
{
	...
	i32 ia0 = (i32)floorf(a0 / gf_PI2 * SAMPLE_COUNT);
	i32 ia1 = (i32)floorf(a1 / gf_PI2 * SAMPLE_COUNT);
...
// OnRwiResult
//====================================================================
i32 CAIRadialOcclusionRaycast::OnRwiResult(const EventPhys* pEvent)
{
	...
	if (pendingRays < raysPerUpdate / 4)
	{
		for (unsigned i = 0; i < raysPerUpdate; ++i)
		{
			...
			gEnv->pPhysicalWorld->RayWorldIntersection(m_center + up, dir,
			                                           ...
			                                           0, 1, 0, 0, (uk )pQuery, PHYS_FOREIGN_ID_RADIAL_OCCLUSION);
#else
			ray_hit hit;
			pSample->dist = m_depthRange * 2.0f;
			pSample->height = m_center.z + up.z + dir.z;
