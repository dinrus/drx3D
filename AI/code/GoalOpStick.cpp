...
//
//----------------------------------------------------------------------------------------------------------
COPStick::COPStick(float fStickDistance, float fEndAccuracy, float fDuration, i32 nFlags, i32 nFlagsAux, ETraceEndMode eTraceEndMode) :
...
void COPStick::Serialize(TSerialize ser)
{
	...
	ser.BeginGroup("COPStick");
	{
		...
		if (ser.IsReading())
		{
			for (u8 i = 0, size = m_stickTargetSafePoints.Size(); i < size; ++i)
			{
...
void COPStick::DebugDraw(CPipeUser* pPipeUser) const
{
	...
	for (TStickTargetSafePoints::SConstIterator it = m_stickTargetSafePoints.Begin(); it != itEnd; ++it, ++iPt)
	{
		...
		ColorB color;
		if (safePoint.safe)
			color.set(0, u8(255 * frac), u8(255 * (1.0f - frac)), 255);
		else
...
void COPStick::UpdateStickTargetSafePoints(CPipeUser* pPipeUser)
{
	...
	i32 maxNumPoints = 1 + (i32)(m_pathLengthForTeleport / m_safePointInterval);
...
EGoalOpResult COPStick::Execute(CPipeUser* pPipeUser)
{
	...
	//////////////////////////////////////////////////////////////////////////
	{
		DRX_PROFILE_REGION(PROFILE_AI, "OPStick::Execute(): Final Path Processing!")

		i32 nPathDecision = pPipeUser->m_nPathDecision;
...
bool COPStick::GetStickAndSightTargets_CreatePathfindAndTraceGoalOps(CPipeUser* pPipeUser, EGoalOpResult* peGoalOpResult)
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_AI, "OPStick::Execute(): Create Pathfinder/Tracer!");
...
{
	DRX_ASSERT_MESSAGE(m_pTraceDirective, "m_pTraceDirective should really be set here (set by the calling code)");
	if (!m_pTraceDirective)
	{
	...
	DRX_PROFILE_REGION(PROFILE_AI, "OPStick::Trace(): Tracer Execute!");
...
bool COPStick::HandleHijack(CPipeUser* pPipeUser, const Vec3& vStickTargetPos, float fPathDistanceLeft, EGoalOpResult eTraceResult)
{
	    ...
	    (m_bPathFound && (fPathDistanceLeft < HIJACK_DISTANCE)))
	{
		...
		DRX_ASSERT(m_fHijackDistance > 0.f);
		// slow down as we approach the target
		pipeUserState.fDesiredSpeed = fNormalSpeed * fRemainingDistance / m_fHijackDistance;
		pipeUserState.vMoveDir = vToStickTarget.GetNormalizedSafe(Vec3Constants<float>::fVec3_Zero);
	}
...
EGoalOpResult COPStick::HandlePathDecision(CPipeUser* pPipeUser, i32 nPathDecision, bool b2D)
{
