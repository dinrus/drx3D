...
/********************************************************************
   ...
   Имя файла:   Shape.cpp
   $Id$
   Описание: Polygon shapes used for different purposes in the AI system and various shape containers.
...
void CAIShape::BuildBins()
{
	...
	for (unsigned i = 0, ni = m_points.size(); i < ni; ++i)
	{
		...
		if (p0->y != p1->y)
		{
			...
			if (ba < 0) ba = 0;
			if (bb >= (i32)bins) bb = (i32)bins - 1;

			//			AIAssert(ba >= 0 && ba < (i32)bins);
			//			AIAssert(bb >= 0 && bb < (i32)bins);
...
bool CAIShape::IsPointInside(const Vec3& pt) const
{
	...
	// what bin are we in?
	i32 b = (i32)((pt.y - m_aabb.min.y) * m_binSort->invYdelta);

	// Outside the bin range, outside the shape.
	if (b < 0 || b >= (i32)m_binSort->bins.size())
		return false;
