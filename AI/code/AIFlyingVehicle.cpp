					...
					if (visible)
					{
						if (targetID)
						{
							if (IEntity* entity = gEnv->pEntitySystem->GetEntity(observableParams.entityId))
							{
								...
								{
									u16 aiObjectType = aiObject->GetAIType();
...
namespace
{
void AISendSignal(CAIFlyingVehicle* const pFlyingVehicle, tukk const signalName)
{
	DRX_ASSERT(pFlyingVehicle);
	DRX_ASSERT(signalName);

	u32k signalNameCrc = CCrc32::Compute(signalName);
...
void CAIFlyingVehicle::SetSignal(i32 nSignalID, tukk szText, IEntity* pSender, IAISignalExtraData* pData, u32 crcCode)
{
	static u32k s_combatTargetEnabledCrc = CCrc32::Compute("CombatTargetEnabled");
	static u32k s_combatTargetDisabledCrc = CCrc32::Compute("CombatTargetDisabled");
	static u32k s_firingAllowedCrc = CCrc32::Compute("FiringAllowed");
	static u32k s_firingNotAllowedCrc = CCrc32::Compute("FiringNotAllowed");
