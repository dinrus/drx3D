...
static void RenderText(IRenderAuxGeom* const pRenderAuxGeom, const Vec3& pos, SDrawTextInfo& ti, tukk szFormat, ...)
{
	va_list args;
...
{
	DistanceColorPicker(u16k* _distances)
		: distances(_distances)
	{}

	ColorB operator()(const CompactSpanGrid& grid, size_t i) const
	{
		u8 distance = (u8)min<u16>(distances[i] * 4, 255);
...
{
	SPaintColorPicker(u16k* _paints)
		: paints(_paints)
	{
	}
...
{
	void DrawColorLegendLine(const Vec2& pos, const Vec2& size, const float fontSize, const ColorB& color, tukk szLabel)
	{
...
{
	static void DrawLegend(const std::vector<u16>& paints)
	{
		const float fontSize = 1.8f;
		const float dy = 15.0f;
		const float x = 10.0f;
		...
		u16 maxPaintValue = CTileGenerator::Paint::NoPaint;
		if (!paints.empty())
		{
		...
		if (legend)
		{
			...
			for (u16 paintVal = CTileGenerator::Paint::OkPaintStart, idx = 1; paintVal < maxPaintValue; ++paintVal, ++idx)
			{
				legend.DrawColorLegendLine(Vec2(x, (y += dy)), Vec2(dy, dy), fontSize, colorer(paintVal), stack_string().Format("paint #%u", (u32)idx).c_str());
			}
		}
	}
};
...
{
public:
	CDistanceInfoPrinter(u16k* pDistances_)
	...
	void operator()(const CompactSpanGrid::Span& span, const SSpanCoord& spanCoord, const Vec3& topCenter, IRenderAuxGeom* const pRenderAuxGeom) const
	{
		if (IsOnScreen(topCenter))
		{
			...
			RenderText(pRenderAuxGeom, topCenter, ti, "%u", u32(pDistances[spanCoord.spanAbsIdx]));
		}
	}
private:
	u16k* pDistances;
		...
		void operator()(IRenderAuxGeom& renderAuxGeom, const CTileGenerator::ContourVertex& vtx, const size_t vtxIdx, const Vec3& vtxPosWorld) const
		{
			...
			if (vtx.debugInfoIndex < infos.size())
			{
				...
				if (IsOnScreen(vtxPosWorld))
				{
					...
					tukk const classNames[3] = { "UW", "NB", "WB" };
...
void CTileGenerator::DrawTracers(const Vec3& origin) const
{
	if (IRenderAuxGeom* pRender = gEnv->pRenderer->GetIRenderAuxGeom())
	{
		...
		pRender->SetRenderFlags(flags);
		{
			...
			i32k numPaths = m_tracerPaths.size();
			for (i32 i = 0; i < numPaths; i++)
			{
				const TracerPath& path = m_tracerPaths[i];
				i32k numTracers = path.steps.size();
				for (i32 j = 0; j < numTracers; j++)
				{
...
void CTileGenerator::DrawSimplifiedContours(const Vec3& origin) const
{
	...
	for (size_t i = 0; i < m_polygons.size(); ++i)
	{
		...
		for (size_t s = 0; s < contour.size(); ++s)
		{
			...
			IRenderAuxText::DrawLabelF(v1, 1.8f, "%d", (i32)s);
