...
//====================================================================
bool IntersectSweptSphere(Vec3* hitPos, float& hitDist, const Lineseg& lineseg, float radius, EAICollisionEntities aiCollisionEntities, IPhysicalEntity** pSkipEnts, i32 nSkipEnts, i32 geomFlagsAny)
{
...
void ConvexHull2DGraham(std::vector<Vec3>& ptsOut, const std::vector<Vec3>& ptsIn)
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_AI, "SORT Graham");
...
void ConvexHull2DAndrew(std::vector<Vec3>& ptsOut, const std::vector<Vec3>& ptsIn)
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_AI, "SORT Andrew");
	...
	while (++i <= maxmin)
	{
		...
		while ((i32)ptsOut.size() > 1) // there are at least 2 points on the stack
		{
	...
	i32 bot = (i32)ptsOut.size() - 1; // the bottom point of the upper hull stack
	i = maxmin;
	while (--i >= minmax)
	{
		...
		while ((i32)ptsOut.size() > bot + 1)    // at least 2 points on the upper stack
		{
			// test if P[i] is left of the line at the stack top
			if (IsLeftAndrew(ptsOut[ptsOut.size() - 2], ptsOut.back(), P[i]) > 0)
				break;         // P[i] is a new hull vertex
