...
bool COptionCriteria::AddToParameters(tukk sSpec, float fValue)
{
	bool bRet = true;
	...
	else
	{
		...
		case eTPQP_ObjectsType:
			m_params.iObjectsType = (i32)fValue;
			break;
		case eTPQP_Density:
...
bool COptionCriteria::AddToParameters(tukk sSpec, bool bValue)
{
	bool bRet = true;
...
bool COptionCriteria::AddToParameters(tukk sSpec, tukk sValue)
{
	bool bRet = true;
...
bool COptionCriteria::AddToGeneration(tukk sSpec, float fValue)
{
	i32 validFlags = (eTPQ_FLAG_GENERATOR | eTPQ_FLAG_GENERATOR_O);
...
bool COptionCriteria::AddToGeneration(tukk sSpec, ETPSRelativeValueSource nSource)
{
...
bool COptionCriteria::AddToConditions(tukk sSpec, bool bValue)
{
	// Taking a boolean value so must be a Boolean Property or a Test (query returning a bool)
	i32 validFlags = eTPQ_FLAG_PROP_BOOL | eTPQ_FLAG_TEST;
	CCriterion criterion = GetCriterion(sSpec);
...
bool COptionCriteria::AddToConditions(tukk sSpec, float fValue)
{
	// Taking a float value so must be a Real Property or a Measure (query returning a float)
	i32 validFlags = eTPQ_FLAG_PROP_REAL | eTPQ_FLAG_MEASURE;
	CCriterion criterion = GetCriterion(sSpec);
...
bool COptionCriteria::AddToWeights(tukk sSpec, float fWeight)
{
	...
	i32 validFlags = eTPQ_FLAG_PROP_REAL | eTPQ_FLAG_MEASURE | eTPQ_FLAG_PROP_BOOL | eTPQ_FLAG_TEST;
	CCriterion criterion = GetCriterion(sSpec);
...
CCriterion COptionCriteria::GetCriterion(tukk sSpec)
{
...
CTacticalPointQuery::CTacticalPointQuery(tukk psName) : m_sName(psName)
{
...
const COptionCriteria* CTacticalPointQuery::GetOption(i32 i) const
{
	if (i < 0 || i >= (i32)m_vOptions.size())
		return NULL;
	return &(m_vOptions[i]);
}

COptionCriteria* CTacticalPointQuery::GetOption(i32 i)
{
	if (i < 0 || i >= (i32)m_vOptions.size())
		return NULL;
	return &(m_vOptions[i]);
