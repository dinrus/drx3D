...
u32 CoverSampler::GetSampleCount() const
{
...
u32 CoverSampler::GetSurfaceFlags() const
{
	return m_flags;
}
...
bool CoverSampler::IntersectRay(const Vec3& center, const Vec3& dir, u32 collisionFlags, IntersectionResult* result) const
{
	++m_state.rwiCount;
...
CoverSampler::SurfaceType CoverSampler::GetSurfaceType(IPhysicalEntity* physicalEntity, i32 ipart) const
{
	pe_params_part pp;
	pp.ipart = ipart;
                                 ...
                                 Vec3* averageNormal, SurfaceType* surfaceType)
{
	...
	u32 sampleCount = (u32)(0.5f + maxHeight / heightInterval);
                               ...
                               Vec3* floor, Vec3* normal)
{
	...
	i32 collisionEntities = CoverCollisionEntities;
	if (collisionEntities & ent_static)
		collisionEntities |= ent_rigid | ent_sleeping_rigid; // support 0-mass rigid entities too
...
{
	u32 coverSampleCount = m_tempSamples.size();
	...
	u32 tempSampleCount = m_tempSamples.size();
	u32 iterationCount = m_state.looped ? tempSampleCount : (tempSampleCount - 3);
