...
{
	u32 count = node.GetChildCount();//node->getChildCount();
	for (u32 loop = 0; loop < count; ++loop)
	{
...
void CGoalOpParallel::SetConcurrentPipe(CGoalPipe* goalPipe)
{
	...
	else
	{
		...
		{
			m_OpInfos.resize(0);
			u32 count = m_ConcurrentPipe->GetNumGoalOps();
...
bool CGoalOpXMLReader::GetBool(const XmlNodeRef& node, tukk szAttrName, bool bDefaultValue)
{
	bool bValue = false;
...
bool CGoalOpXMLReader::GetMandatoryBool(const XmlNodeRef& node, tukk szAttrName)
{
	bool bValue;
	return m_dictBools.Get(node, szAttrName, bValue, true) ? bValue : false;
}

tukk CGoalOpXMLReader::GetMandatoryString(const XmlNodeRef& node, tukk szAttrName)
{
	tukk sz;
...
EGoalOpResult COPApproach::Execute(CPipeUser* pPipeUser)
{
	...
	i32 debugPathfinding = gAIEnv.CVars.DebugPathFinding;
...
// COPFollowPath
//====================================================================
COPFollowPath::COPFollowPath(bool pathFindToStart, bool reverse, bool startNearest, i32 loops, float fEndAccuracy, bool bUsePointList, bool bControlSpeed, float desiredSpeedOnPath) :
		...
		else // no pathfind directive
		{
			...
			if (m_pathFindToStart)
			{
				...
				if (m_refPathStartPoint.IsNil() || !m_refPathStartPoint->GetAIObjectID())
					gAIEnv.pAIObjectUpr->CreateDummyObject(m_refPathStartPoint, string(GetNameSafe(pPipeUser)) + "_PathStartPoint", CAIObject::STP_REFPOINT);
...
// COPBackoff
//===================================================================
COPBackoff::COPBackoff(float distance, float duration, i32 filter, float minDistance)
{
...
void COPBackoff::ResetMoveDirections()
{
	...
	i32 mask = (AI_MOVE_FORWARD | AI_MOVE_BACKWARD | AI_MOVE_LEFT | AI_MOVE_RIGHT | AI_MOVE_TOWARDS_GROUP | AI_MOVE_BACKLEFT | AI_MOVE_BACKRIGHT);
	mask &= m_iDirection;
	...
	if (m_bRandomOrder)
	{
		i32 size = m_MoveDirections.size();
		for (i32 i = 0; i < size; ++i)
		{
			i32 other = drx_random(0, size - 1);
			i32 temp = m_MoveDirections[i];
...
EGoalOpResult COPBackoff::Execute(CPipeUser* pPipeUser)
{
	...
	if (!m_pPathfindDirective)
	{
		...
		i32 currentDir = m_iCurrentDirectionIndex >= (i32) m_MoveDirections.size() ? 0 : m_MoveDirections[m_iCurrentDirectionIndex];
		if (m_iCurrentDirectionIndex < (i32)m_MoveDirections.size())
		...
		case AI_MOVE_TOWARDS_GROUP:
			{
				...
				vMoveDir.zero();
				i32 groupId = pPipeUser->GetGroupId();
		...
		if (m_refBackoffPoint.IsNil())
		{
			gAIEnv.pAIObjectUpr->CreateDummyObject(m_refBackoffPoint, string(GetNameSafe(pPipeUser)) + "_BackoffPoint");
...
EGoalOpResult COPTimeout::Execute(CPipeUser* pPipeUser)
{
	...
	{
		m_actualIntervalMs = (i32)(1000.0f * (m_fIntervalMin + ((m_fIntervalMax - m_fIntervalMin) * drx_random(0.0f, 1.0f))));
		m_startTime = time;
	}
...
EGoalOpResult COPFireCmd::Execute(CPipeUser* pPipeUser)
{
	...
	if (m_startTime.GetMilliSecondsAsInt64() <= 0)
	{
		...
		if (m_fIntervalMin < 0.0f)
			...
			m_actualIntervalMs = (i32)(drx_random(m_fIntervalMin, m_fIntervalMax) * 1000.0f);
		else
			m_actualIntervalMs = (i32)(m_fIntervalMin * 1000.0f);
	}
...
// todo luc fixme
COPLookAt::COPLookAt(float fStartAngle, float fEndAngle, i32 mode, bool bBodyTurn, bool bUseLastOp)
...
EGoalOpResult COPLookAt::Execute(CPipeUser* pPipeUser)
{
	...
	if (!m_bInitialized)
	{
		...
		else  // if (!m_fStartAngle && !m_fEndAngle)
		{
			...
			float fActualAngle = drx_random(m_fStartAngle, m_fEndAngle);
...
void COPLookAround::SetRandomLookDir()
{
	...
	m_lookAngle = drx_random(-1.0f, 1.0f) * range;
	m_lookZOffset = drx_random(-3.0f, 0.3f);  // Look more down then up
}
...
EGoalOpResult COPLookAround::Execute(CPipeUser* pPipeUser)
{
	...
	{
		i32 id = pPipeUser->GetLooseAttentionId();
...
COPPathFind::COPPathFind(const XmlNodeRef& node) :
	...
	m_refTarget(m_sTargetName.empty() ? NILREF : GetWeakRef(gAIEnv.pAIObjectUpr->GetAIObjectByName(m_sTargetName.c_str()))),
...
COPLocate::COPLocate(tukk szName, u16 nObjectType, float fRange) :
	m_sName(szName), m_nObjectType(nObjectType), m_fRange(fRange), m_bWarnIfFailed(true)
{
}
...
COPDeValue::COPDeValue(i32 nPuppetsAlso, bool bClearDevalued) :
			...
			case eTPSQS_Success:
				{
					...
					gAIEnv.pAIObjectUpr->CreateDummyObject(m_refHideTarget, string(GetNameSafe(pPipeUser)) + "_HideTarget");
...
bool COPTacticalPos::IsBadHiding(CPipeUser* pPipeUser)
{
	...
	rayresult = gAIEnv.pWorld->RayWorldIntersection(
	  target_pos, ai_pos - target_pos, COVER_OBJECT_TYPES,
	...
	//	ai_pos.z-=1.f;
	rayresult = gAIEnv.pWorld->RayWorldIntersection(
	  ai_pos, target_pos - ai_pos, COVER_OBJECT_TYPES,
	  AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOLID_COVER | AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOFT_COVER,
	  &hit, 1, &skipList[0], skipCount);
	if (!rayresult)
	{
		...
		ai_pos.z -= 1.f;
		rayresult = gAIEnv.pWorld->RayWorldIntersection(
		  ai_pos, target_pos - ai_pos,
		  COVER_OBJECT_TYPES, AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOLID_COVER | AI_VISION_RAY_CAST_FLAG_BLOCKED_BY_SOFT_COVER,
		  &hit, 1, &skipList[0], skipCount);
		//		if (rayresult)
		{
...
COPTacticalPos::COPTacticalPos(i32 tacQueryID, EAIRegister nReg) :
	m_nReg(nReg),
	m_pPathFindDirective(0),
	m_pTraceDirective(0),
	m_iOptionUsed(-1),
	...
	m_bLookAtHide(false)
{
	...
	tukk szQueryName = node->getAttr("name");
...
void COPTacticalPos::SendStateSignal(CPipeUser* pPipeUser, i32 nState)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	i32 queryID = m_queryInstance.GetQueryID();
...
COPLook::COPLook(i32 lookMode, bool bBodyTurn, EAIRegister nReg)
...
EGoalOpResult COPLook::Execute(CPipeUser* pPipeUser)
{
	...
	//DrxLogAlways( "look IK status [%f]", pPuppet->GetLookIKStatus() );
	...
	if (m_pTraceDirective && m_pPosDummy)
	{
		...
		else
		{
...

			{
				...
				//// TODO/HACK! [Mikko] This prevent the path to regenerated every frame in some special cases in Drxsis Core level
	...
	if (opPos.GetSquaredDistance(destination) < 2500.f)
	{
		...
		if (pRegion)
		{
			...
			if (pRegion->CheckPassability(from, to, pPipeUser->GetParameters().m_fPassRadius, navBlocker, pPipeUser->GetMovementAbility().pathfindingProperties.navCapMask))
			{
				...
				pPipeUser->m_Path.SetParams(SNavPathParams(from, to, Vec3Constants<float>::fVec3_Zero, Vec3Constants<float>::fVec3_Zero, -1, false, endDistance, true));
	...
	m_intervalMs(0)
{
	...
	m_intervalMs = (i32)(fInterval * 1000.0f);
...
//
//----------------------------------------------------------------------------------------------------------
COPWaitSignal::COPWaitSignal(tukk sSignal, float fInterval /*= 0*/)
{
	m_edMode = edNone;
	m_sSignal = sSignal;
	m_intervalMs = (i32)(fInterval * 1000.0f);
...
COPWaitSignal::COPWaitSignal(tukk sSignal, tukk sObjectName, float fInterval /*= 0*/)
{
	m_edMode = edString;
...
COPWaitSignal::COPWaitSignal(tukk sSignal, i32 iValue, float fInterval /*= 0*/)
{
	m_edMode = edInt;
	m_iValue = iValue;
...
COPWaitSignal::COPWaitSignal(tukk sSignal, EntityId nID, float fInterval /*= 0*/)
{
	m_edMode = edId;
	m_nID = nID;
...
bool COPWaitSignal::NotifySignalReceived(CAIObject* pPipeUser, tukk szText, IAISignalExtraData* pData)
{
	...
	if (!m_bSignalReceived && szText && m_sSignal == szText)
	{
		...
		else if (pData)
		{
			...
			{
				tukk objectName = pData->GetObjectName();
...
// COPAnimAction
//----------------------------------------------------------------------------------------------------------
COPAnimation::COPAnimation(EAnimationMode mode, tukk value, const bool isUrgent)
	: m_eMode(mode)
	, m_sValue(value)
...
// COPAnimTarget
//----------------------------------------------------------------------------------------------------------
COPAnimTarget::COPAnimTarget(bool signal, tukk animName, float startWidth, float dirTolerance, float startArcAngle, const Vec3& approachPos, bool autoAlignment)
	: m_bSignal(signal)
...
//
//-------------------------------------------------------------------------------------------------------------
COPWait::COPWait(i32 waitType, i32 blockCount) :
...
COPWait::COPWait(const XmlNodeRef& node) :
	...
	m_BlockCount(0)
{
	tukk szWaitType = node->getAttr("for");
...
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	i32 currentBlockCount = pPipeUser->CountGroupedActiveGoals();
	bool done = false;
	switch (m_WaitType)
	{
	case WAIT_ALL:
...
EGoalOpResult COPDodge::Execute(CPipeUser* pPipeUser)
{
	...
	if (!m_pTraceDirective)
	{
		DRX_PROFILE_REGION(PROFILE_AI, "Dodge/CalculatePathTree");
		...
		for (i32 i = 0; i < iters; ++i)
		{
			...
			float x = cosf(a);
			float y = sinf(a);
			float d = m_distance * drx_random(0.9f, 1.1f);
...
COPCommunication::COPCommunication(tukk commName, tukk channelName, tukk ordering, float expirity, float minSilence, bool ignoreSound, bool ignoreAnim)
	:
...
{
	m_commID = gAIEnv.pCommunicationUpr->GetCommunicationID(commName);
	if (!gAIEnv.pCommunicationUpr->GetCommunicationName(m_commID))
	{
		AIWarning("Unknown communication name '%s'", commName);
		m_commID = CommID(0);
	}

	m_channelID = gAIEnv.pCommunicationUpr->GetChannelID(channelName);
	if (!m_channelID)
	{
...
void COPCommunication::Reset(CPipeUser* pPipeUser)
{
	...
	if (m_playID)
	{
		gAIEnv.pCommunicationUpr->RemoveInstanceListener(m_playID);
	}
}

void COPCommunication::OnCommunicationEvent(ICommunicationUpr::ECommunicationEvent event, EntityId actorID,
                                            const CommPlayID& playID)
{
	switch (event)
	{
	case ICommunicationUpr::CommunicationCancelled:
	case ICommunicationUpr::CommunicationFinished:
	case ICommunicationUpr::CommunicationExpired:
		{
			if (m_playID == playID)
			{
				m_commFinished = true;
...
EGoalOpResult COPCommunication::Execute(CPipeUser* pPipeUser)
{
	...
	if (!m_bInitialized)
	{
		...
		m_playID = commUpr.PlayCommunication(request);
...
void COPCommunication::Serialize(TSerialize ser)
{
	...
	i32 ordering = SCommunicationRequest::Unordered;

	if (ser.IsWriting())
	{
		ordering = (i32) m_ordering;
	}
