...
tukk CActorCollisionAvoidance::GetName() const
{
...
CAIActor::~CAIActor()
{
	...
	gAIEnv.pGroupUpr->RemoveGroupMember(GetGroupId(), GetAIObjectID());
...
void CAIActor::SetBehaviorVariable(tukk variableName, bool value)
{
	{
		Variables::Collection* variableCollection = GetAISystem()->GetIBehaviorTreeUpr()->GetBehaviorVariableCollection_Deprecated(GetEntityID());
		const Variables::Declarations* variableDeclarations = GetAISystem()->GetIBehaviorTreeUpr()->GetBehaviorVariableDeclarations_Deprecated(GetEntityID());
...
bool CAIActor::GetBehaviorVariable(tukk variableName) const
{
	Variables::Collection* variableCollection = GetAISystem()->GetIBehaviorTreeUpr()->GetBehaviorVariableCollection_Deprecated(GetEntityID());
	const Variables::Declarations* variableDeclarations = GetAISystem()->GetIBehaviorTreeUpr()->GetBehaviorVariableDeclarations_Deprecated(GetEntityID());
	if (!variableCollection || !variableDeclarations)
...
void CAIActor::Reset(EObjectResetType type)
{
	...
	tukk navigationTypeName = m_proxy->GetNavigationTypeName();
...
void CAIActor::Update(IAIObject::EUpdateType type)
{
	...
	if (!CastToCPipeUser())
	{
		if (type == EUpdateType::Full)
		{
			m_lightLevel = pAISystem->GetLightUpr()->GetLightLevelAt(GetPos(), this, &m_usingCombatLight);
		}
		...
		if (pAttTarget && pAttTarget->IsEnabled())
		{
			...
			else
			{
				...
				case AITARGET_NONE:
				case AITARGET_ENEMY:
				case AITARGET_FRIENDLY:
				case AITARGET_BEACON:
				case AITARGET_GRENADE:
			    case AITARGET_RPG:
				case AITARGET_LAST:
					break;
...
void CAIActor::UpdateDamageParts(DamagePartVector& parts)
{
	...
	static ISurfaceTypeUpr* pSurfaceMan = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr();

	pe_status_nparts statusNParts;
	i32 nParts = phys->GetStatus(&statusNParts);

	if ((i32)parts.size() != nParts)
	{
		parts.resize(nParts);
...
void CAIActor::OnAIHandlerSentSignal(tukk signalText, u32 crc)
{
	IF_UNLIKELY (crc == 0)
	{
	...
	if (IsRunningBehaviorTree())
	{
		...
		GetAISystem()->GetIBehaviorTreeUpr()->HandleEvent(GetEntityID(), event);
	}
}
...
//
//------------------------------------------------------------------------------------------------------------------------
void CAIActor::SetSignal(i32 nSignalID, tukk szText, IEntity* pSender, IAISignalExtraData* pData, u32 crcCode)
{
	CCCPOINT(SetSignal);
	...
	if (nSignalID != AISIGNAL_NOTIFY_ONLY)
	{
		...
		if (nSignalID != AISIGNAL_ALLOW_DUPLICATES)
		{
			...
			for (ai = m_State.vSignals.begin(); ai != m_State.vSignals.end(); ++ai)
			{
				...
				if (stricmp((*ai).strText, szText) && (*ai).Compare(crcCode))
				{
					AIWarning("Please report to alexey@drxtek.de! Hash values are identical, but strings are different! %s - %s ", (*ai).strText, szText);
				}
			}
		}
#endif // _DEBUG
	...
	drx_strcpy(signal.strText, szText);
...
bool CAIActor::IsHostile(const IAIObject* pOtherAI, bool bUsingAIIgnorePlayer) const
{
	...
	if (pOtherAI)
	{
		...
		u8 myFaction = GetFactionID();
		u8 otherFaction = other->GetFactionID();
...
//
//------------------------------------------------------------------------------------------------------------------------
void CAIActor::SetGroupId(i32 id)
{
	if (id != GetGroupId())
	{
		gAIEnv.pGroupUpr->RemoveGroupMember(GetGroupId(), GetAIObjectID());
		if (id > 0)
			gAIEnv.pGroupUpr->AddGroupMember(id, GetAIObjectID());
...
void CAIActor::SetFactionID(u8 factionID)
{
...
void CAIActor::BehaviorChanged(tukk current, tukk previous)
{
	BehaviorListeners::iterator it = m_behaviorListeners.begin();
	BehaviorListeners::iterator end = m_behaviorListeners.end();
...
void CAIActor::Serialize(TSerialize ser)
{
	...
	u32 lastFullUpdateStance = m_lastFullUpdateStance;
	...
	u32 navigationTypeId = m_navigationTypeID;
...
u32 CAIActor::GetFactionVisionMask(u8 factionID) const
{
	u32 mask = 0;
	u32 factionCount = gAIEnv.pFactionMap->GetFactionCount();
	...
	for (; it != end; ++it)
	{
		if (const CAIObject* aiObject = static_cast<const CAIObject*>(GetAISystem()->GetAIObjectUpr()->GetAIObject(*it)))
...
void CAIActor::SerializeMovementAbility(TSerialize ser)
{
	...
	for (i32 i = 0; i < AgentMovementSpeeds::AMU_NUM_VALUES; i++)
		for (i32 j = 0; j < AgentMovementSpeeds::AMS_NUM_VALUES; j++)
		{
...
void CAIActor::GetPhysicalSkipEntities(PhysSkipList& skipList) const
{
	...
	/*if (IEntity* entity = GetEntity())
	   {
	   for (i32 i = 0, ni = entity->GetChildCount(); i < ni; ++i)
	   {
	...
	DRX_ASSERT_MESSAGE(skipList.size() <= 5, "Too many physical skipped entities determined. See SRwiRequest definition.");
}
...
tukk CAIActor::GetPathAgentName() const
{
	return GetName();
}
...
void CAIActor::SetPathToFollow(tukk pathName)
{
...
void CAIActor::ReactionChanged(u8 factionID, IFactionMap::ReactionType reaction)
{
	if (m_observer)
	{
		ObserverParams params;
...
u32 CAIActor::GetObserverTypeMask() const
{
	return General | AliveAgent | DeadAgent | Player;
}

u32 CAIActor::GetObservableTypeMask() const
{
...
void CAIActor::HandleBulletRain(SAIEVENT* pAIEvent)
{
	...
	if (gAIEnv.pTargetTrackUpr->IsEnabled())
		gAIEnv.pTargetTrackUpr->HandleStimulusFromAIEvent(GetAIObjectID(), pAIEvent, TargetTrackHelpers::eEST_BulletRain);
}
...
void CAIActor::GetMovementSpeedRange(float fUrgency, bool bSlowForStrafe, float& normalSpeed, float& minSpeed, float& maxSpeed) const
{
	...
	if (m_movementAbility.directionalScaleRefSpeedMin > 0.0f)
	{
		...
		float turnSlowDownFactor = (gAIEnv.configuration.eCompatibilityMode == ECCM_DRXSIS2) ? 0.2f : 0.4f;
...
void CAIActor::SetTerritoryShapeName(tukk szName)
{
	assert(szName);
...
tukk CAIActor::GetTerritoryShapeName() const
{
...
void CAIActor::CoordinationEntered(tukk signalName)
{
...
void CAIActor::CoordinationExited(tukk signalName)
{
...
void CAIActor::StartBehaviorTree(tukk behaviorName)
{
	if (GetAISystem()->GetIBehaviorTreeUpr()->StartModularBehaviorTree(GetEntityID(), behaviorName))
...
void CAIActor::StopBehaviorTree()
{
	...
	{
		GetAISystem()->GetIBehaviorTreeUpr()->StopModularBehaviorTree(GetEntityID());
		m_runningBehaviorTree = false;
	}
}
