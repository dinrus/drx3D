...
{
	static void ReflectType(sxema::CTypeDesc<SObservableVisibilityChangeSignal>& desc)
	{
		desc.SetGUID("c0989593-1022-4438-bedf-1039714f21e2"_drx_guid);
		desc.SetLabel("VisibilityChange");

		desc.AddMember(&SObservableVisibilityChangeSignal::observerEntityId, 'oeid', "ObserverEntity", "ObserverEntity", nullptr, sxema::ExplicitEntityId::Invalid);
		desc.AddMember(&SObservableVisibilityChangeSignal::bVisibile, 'vis', "Visible", "Visible", nullptr, false);
	}

	sxema::ExplicitEntityId observerEntityId;
	bool bVisibile;
};
...
void CEntityAIObservableComponent::ReflectType(sxema::CTypeDesc<CEntityAIObservableComponent>& desc)
{
	desc.AddBase<IEntityObservableComponent>();
	desc.SetGUID("5A32746A-9CEA-4877-B9E4-5C4E06EEE90C"_drx_guid);
...
void CEntityAIObservableComponent::Register(sxema::IEnvRegistrar& registrar)
{
	sxema::CEnvRegistrationScope scope = registrar.Scope(IEntity::GetEntityScopeGUID());
	{
		sxema::CEnvRegistrationScope componentScope = scope.Register(SXEMA_MAKE_ENV_COMPONENT(CEntityAIObservableComponent));

		// Signals
		{
			componentScope.Register(SXEMA_MAKE_ENV_SIGNAL(SObservableVisibilityChangeSignal));
		}
	}
}
...
{
	DRX_ASSERT(IsRegistered() && observableId == m_observableId);

	if (GetEntity()->GetSchematycObject())
	{
		GetEntity()->GetSchematycObject()->ProcessSignal(SObservableVisibilityChangeSignal{ sxema::ExplicitEntityId(observerParams.entityId), visible }, GetGUID());
	}
}
...
void CEntityAIObservableComponent::UpdateChange(u32k changeHintFlags)
{
	if (IsRegistered())
	{
...
void CEntityAIObservableComponent::SyncWithEntity()
{
	...
	if (locationsCount > 0)
	{
		...
		m_params.observablePositionsCount = u8(locationsCount);
	}
	else
	{
...
//////////////////////////////////////////////////////////////////////////
void CEntityAIObservableComponent::SetTypeMask(u32k typeMask)
{
...
void CEntityAIObservableComponent::AddObservableLocationOffsetFromBone(const Vec3& offsetFromBone, tukk szBoneName)
{
	Perception::ComponentHelpers::SLocation location;
...
void CEntityAIObservableComponent::SetObservableLocationOffsetFromBone(const size_t index, const Vec3& offsetFromBone, tukk szBoneName)
{
	if (index < m_observableLocations.locations.size())
	{
		Perception::ComponentHelpers::SLocation& location = m_observableLocations.locations[index];		
