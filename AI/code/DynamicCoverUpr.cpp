...
#include <drx3D/AI/DynamicCoverUpr.h>
#include <drx3D/AI/CoverSystem.h>

const bool DynamicCoverDeferred = false;

void DynamicCoverUpr::OnEntityEvent(IEntity* entity, const SEntityEvent& event)
{
...
DynamicCoverUpr::DynamicCoverUpr()
	: m_segmentsGrid(20.0f, 20.0f, 20.0f, segment_position(m_segments))
{
}

void DynamicCoverUpr::AddValidationSegment(const ValidationSegment& segment)
{
	size_t index;
...
void DynamicCoverUpr::RemoveSurfaceValidationSegments(const CoverSurfaceID& surfaceID)
{
	...
	for (; it != end; ++it)
	{
		...
		if (segment.surfaceID == surfaceID)
		{
			...
			m_freeSegments.push_back(static_cast<u16>(index));
...
void DynamicCoverUpr::AddEntity(EntityId entityID)
{
...
void DynamicCoverUpr::RemoveEntity(EntityId entityID)
{
...
void DynamicCoverUpr::RemoveEntity(EntityId entityID, EntityCoverState& state)
{
...
void DynamicCoverUpr::Reset()
{
	m_validationQueue.clear();
}

void DynamicCoverUpr::Clear()
{
...
void DynamicCoverUpr::ClearValidationSegments()
{
	m_segments.clear();
...
void DynamicCoverUpr::Update(float updateTime)
{
	...
	for (; it != end; ++it)
	{
		...
		if (state.state == EntityCoverState::Moving)
		{
			if ((now - state.lastMovement).GetMilliSecondsAsInt64() >= 150)
			{
				...
				m_entityCoverSampler.Queue(it->first, functor(*this, &DynamicCoverUpr::EntityCoverSampled));
			}
		}
	}
...
void DynamicCoverUpr::EntityCoverSampled(EntityId entityID, EntityCoverSampler::ESide side,
...
void DynamicCoverUpr::RemoveEntityCoverSurfaces(EntityCoverState& state)
{
...
void DynamicCoverUpr::BreakEvent(const Vec3& position, float radius)
{
	m_dirtyBuf.clear();
...
void DynamicCoverUpr::MoveEvent(EntityId entityID, const Matrix34& worldTM)
{
	EntityCover::iterator it = m_entityCover.find(entityID);
...
void DynamicCoverUpr::QueueValidation(i32 index)
{
...
void DynamicCoverUpr::ValidateOne()
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	if (DynamicCoverDeferred)
	{
		while (centerZ < maxZ)
		{
			...
			gAIEnv.pIntersectionTester->Queue(IntersectionTestRequest::HighestPriority,
			                                  ...
			                                  functor(*this, &DynamicCoverUpr::IntersectionTestComplete));
	...
	else
	{
		...
		u32 collisionEntities = CoverCollisionEntities;
...
void DynamicCoverUpr::IntersectionTestComplete(const QueuedIntersectionID& intID, const IntersectionTestResult& result)
{
