...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   MissLocationSensor.h
   $Id$
   Описание:
...
void CMissLocationSensor::Collect(i32 types)
{
		...
		if (entity)
		{
			...
			if (entity->GetStatus(&spos))
			{
				...
				if (entity->GetStatus(&srope) && entity->GetParams(&prope))
				{
					...
					{
						u32 pointCount = std::max(srope.nVtx, (srope.nSegments + 1)); // use the version with the most detail
...
bool CMissLocationSensor::GetLocation(CAIObject* target, const Vec3& shootPos, const Vec3& shootDir, float maxAngle, Vec3& pos)
{
	...
	u32 maxConsidered = std::min<u32>(MaxConsiderCount, m_locations.size());
	...
	for (; maxConsidered && (it != end); ++it)
	{
		...
		if (IPhysicalEntity* ownerPhysics = m_owner->GetPhysics())
		{
			...
			if (ownerPhysics->GetStatus(&ppos))
			{
				...
				u32 locationCount = m_goodies.size();
				for (u32 i = 0; (i < locationCount) && (i < MaxRandomPool); ++i)
				{
	...
	const MissLocation& location = m_goodies[drx_random((size_t)0, goodiesCount - 1)];
...
void CMissLocationSensor::AddDestroyableClass(tukk className)
{
