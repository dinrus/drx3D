...
i32 CObjectContainer::m_snObjectsRegistered = 0;
i32 CObjectContainer::m_snObjectsDeregistered = 0;

tukk GetNameFromType(EAIClass type)
{
	switch (type)
	{
	case eAIC_AIVehicle:
...
void CObjectContainer::Reset()
{
	...
	i32k numRegistered = GetNumRegistered();
	if (numRegistered != 0)
	{
		DumpRegistered();
	}
	DRX_ASSERT_MESSAGE(numRegistered == 0, "Something has leaked AI objects, and we're about to create dangling pointers! Check the log for details");
...
bool CObjectContainer::RegisterObject(CAIObject* pObject, CStrongRef<CAIObject>& ref, tAIObjectID inId /*=INVALID_AIOBJECTID*/)
{
	...
	if (pObject)
	{
		u16 capacity = static_cast<u16>(m_objects.capacity());
...
bool CObjectContainer::DeregisterObjectUntyped(CAbstractUntypedRef* ref)
{
	...
	DRX_ASSERT_TRACE(validID, ("Multiple AI objects with id %i, dangling pointers or corruption imminent", id));

	if (validID)
	{
		DRX_ASSERT_MESSAGE(!stl::find(m_DeregisteredBuffer, id), "Double deregistering object!");
		m_DeregisteredBuffer.push_back(id);
		ref->Assign(INVALID_AIOBJECTID);

#ifdef _DEBUG
		const CAIObject* object = m_objects[id];
		tukk const name = object ? object->GetName() : "<NULL OBJECT>";
	...
	{
		i32 prevIndex = m_objects.get_index_for_id(id);
		CAIObject* pObject = m_objects.get_index(prevIndex);
		DRX_ASSERT_TRACE(false, ("Previous object was %s (%d)", pObject ? pObject->GetName() : "<NULL OBJECT>", pObject ? pObject->GetAIObjectID() : 0));
...
i32 CObjectContainer::ReleaseDeregisteredObjects(bool checkForLeaks)
{
		...
		for (; itO != itOEnd; ++itO)
		{
			...
			if (validHandle)
			{
				...
				if (object)
				{
					...
					gAIEnv.pAIObjectUpr->OnObjectRemoved(object);
	...
	if (checkForLeaks)
	{
		...
		DRX_ASSERT_MESSAGE(count == totalObjects, "Something has leaked AI objects! Check the log for details");
	}
#endif
...
void CObjectContainer::DumpRegistered()
{
	...
	for (u16 i = 0; i < static_cast<u16>(m_objects.capacity()); ++i)
	{
...
void CObjectContainer::Serialize(TSerialize ser)
{
	...
	u32 totalObjects = (u32)GetNumRegistered();
	u32 capacity = m_objects.capacity();
	...
	for (u32 i = 0, totalSerialised = 0; totalSerialised < totalObjects && i < capacity; ++i)
	{
		CAIObject* object = 0;
...
void CObjectContainer::SerializeObjectIDs(TSerialize ser)
{
	...
	else
	{
		...
		ser.BeginGroup("existingObjects");
		u32 objectCount = 0;
		u32 totalObjects = (u32)m_objects.size();
		u32 capacity = (u32)m_objects.capacity();
		for (u32 i = 0; i < capacity && objectCount < totalObjects; ++i)
		{
			if (!m_objects.index_free(i))
			{
				CAIObject* object = m_objects.get_index(i);
...
void CObjectContainer::PostSerialize()
{
	...
	{
		u16 i16 = static_cast<u16>(i);
		if (!m_objects.index_free(i16))
		{
			CAIObject* object = m_objects.get_index(i16);
			if (object)
			{
			...
			else
			{
				...
				assert(stl::find(m_reservedIDs, m_objects.get_index_id(i16)));
			}
		}
	}
}
