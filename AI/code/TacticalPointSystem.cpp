...
#include <drx3D/AI/TacticalPointQuery.h>
	...
	if (postureQueryID)
	{
		if (queryInstance.queryContext.pAIActor)
		{
			if (CPuppet* puppet = static_cast<CAIActor*>(queryInstance.queryContext.pAIActor)->CastToCPuppet())
				puppet->GetPostureUpr().CancelPostureQuery(postureQueryID);
		}
	}
...
{
	DRX_ASSERT_MESSAGE(m_LanguageExtenderDummyObjects.empty(), "A language extender hasn't unregistered its dummy objects");
...
bool CTacticalPointSystem::ExtendQueryLanguage(tukk szName, ETacticalPointQueryType eType, ETacticalPointQueryCost eCost)
{
	...
	// Check for overflow
	i32k iNewQueryId = m_GameQueryIdMap[eType] + 1;
	u32k uTestField = (eType == eTPQT_GENERATOR_O ? eTPQ_MASK_QUERYSPACE_GEN_O : eTPQ_MASK_QUERYSPACE); // Test the correct set of low-order nibbles for overflow
...
//----------------------------------------------------------------------------------------------//
bool CTacticalPointSystem::ApplyCost(u32 uQueryId, ETacticalPointQueryCost eCost)
{
...
//----------------------------------------------------------------------------------------------//
i32 CTacticalPointSystem::GetCost(u32 uQueryId) const
{
	std::map<TTacticalPointQuery, i32>::const_iterator it = m_mIDToCost.find(uQueryId);
...
//----------------------------------------------------------------------------------------------//
IAIObject* CTacticalPointSystem::CreateExtenderDummyObject(tukk szDummyName)
{
	m_LanguageExtenderDummyObjects.push_back(TDummyRef());
	TDummyRef& newDummyRef = m_LanguageExtenderDummyObjects.back();

	gAIEnv.pAIObjectUpr->CreateDummyObject(newDummyRef, szDummyName);
...
void CTacticalPointSystem::ReleaseExtenderDummyObject(tAIObjectID id)
{
	...
	DRX_ASSERT_MESSAGE(false, "Trying to release a dummy object that isn't registered for this language extender");
}

//----------------------------------------------------------------------------------------------//

i32 CTacticalPointSystem::SyncQuery(TPSQueryID queryID, const QueryContext& context, CTacticalPoint& point)
{
	...
	i32 nOptionUsed = SyncQueryShortlist(queryID, context, results, 1);
...
i32 CTacticalPointSystem::SyncQueryShortlist
  (TPSQueryID queryID, const QueryContext& context, TTacticalPoints& vPoints, i32 n)
{
	...
	bool bOk = SetupQueryEvaluation(instance, evaluation);
	...
	if (elapsed.GetMilliSecondsAsInt64() >= MAX_SYNC_TIME_MS)
	{
		AIWarningID("<TacticalPointSystem> ", "Query '%s' performed by '%s' took too long! (%dms >= %dms)",
		            pQuery->GetName(), pAIActor ? pAIActor->GetName() : "<null>", (u32)elapsed.GetMilliSeconds(), (u32)(MAX_SYNC_TIME_MS));
	}
#endif
	...
	case SQueryEvaluation::eCompleted:
		...
		if (evaluation.iCurrentQueryOption >= 0)
		{
			...
			i32 nPoints = i32(std::distance(it, end));
...
bool CTacticalPointSystem::SetupQueryEvaluation(const SQueryInstance& instance, SQueryEvaluation& evaluation) const
{
	...
	i32 nOption = evaluation.iCurrentQueryOption;
...
bool CTacticalPointSystem::SetupHeapEvaluation(const std::vector<CCriterion>& vConditions, const std::vector<CCriterion>& vWeights, const QueryContext& context, const std::vector<CTacticalPoint>& vPoints, i32 n, SQueryEvaluation& eval) const
{
...
bool CTacticalPointSystem::ContinueHeapEvaluation(SQueryEvaluation& eval, CTimeValue timeLimit) const
{
	...
	i32 iImplicitExpCondsSize = 1;  // We're adding an extra implicit expensive condition
	i32 iExpCondsSize = (i32)eval.vExpConds.size();
	i32 iDefCondsSize = (i32)eval.vDefConds.size();
	i32 iExpWeightsSize = (i32)eval.vExpWeights.size();
	...
	do
	{
		...
		i32 iQueryIndex = topPoint.m_iQueryIndex++;
                                            ...
                                            CAIObject* pObject, const Vec3& vObjectPos, CAIObject* pObjectAux, const Vec3& vObjectAuxPos, TTacticalPoints& accumulator) const
{
	...
	case eTPQ_GO_Hidespots:
		...
		{
			DRX_PROFILE_REGION(PROFILE_AI, "TPS Generate Cover Locations");

			m_cover.resize(0);
			gAIEnv.pCoverSystem->GetCover(objPos, fSearchDist, m_cover);
			u32 coverCount = m_cover.size();
			...
			if (eyes.size())
			{
				DRX_PROFILE_REGION(PROFILE_AI, "TPS Generate Cover Locations [GetOcclusion]");
	...
	case eTPQ_G_Grid:
		{
			...
			i32 nSteps = (i32)(fSearchDist * 2.f / fDensity);
			float fGridHalfWidth = fDensity * nSteps / 2;
			I3DEngine* pEngine = gEnv->p3DEngine;

			float fi = 0.0f;
			for (i32 i = 0; i <= nSteps; ++i, fi += 1.0f)
			{
				float fk = 0.0f;
				for (i32 k = 0; k <= nSteps; ++k, fk += 1.0f)
				{
	...
	case eTPQ_G_PureGrid:
		{
			...
			i32 gridPointsCount = (i32)(fSearchDist * 2.f / density);
			float gridHalfWidth = density * gridPointsCount / 2;

			float x = 0.0f;
			for (i32 i = 0; i <= gridPointsCount; ++i, x += 1.0f)
			{
				float y = 0.0f;
				for (i32 j = 0; j <= gridPointsCount; ++j, y += 1.0f)
				{
				...
				for (size_t actorIndex = 0; actorIndex < activeCount; ++actorIndex)
				{
					...
					if (nObjectID != lookUp.GetEntityID(actorIndex))
					{
						...
						if ((pEntity != pOtherEntity) && objPos.GetSquaredDistance(pOtherEntity->GetPos()) < fSearchDistSqr)
							accumulator.push_back(CTacticalPoint(pOtherEntity->GetId(), pOtherEntity->GetPos()));
					}
				}
			}
		}
	...
	case eTPQ_G_Objects:
		{
			...
			AIObjectOwners::const_iterator itEnd = gAIEnv.pAIObjectUpr->m_Objects.end();

			if (objectType > 0)
				itObject = gAIEnv.pAIObjectUpr->m_Objects.lower_bound(objectType);
			else
				itObject = gAIEnv.pAIObjectUpr->m_Objects.begin();
	...
	case eTPQ_T_CanShootTwoRayTest:
		{
			...
			else if (!eval.canShootRayID && !eval.canShootSecondRayID)
			{
				...
				i32k nOption = eval.iCurrentQueryOption;
				const COptionCriteria* pOption = pQuery->GetOption(nOption);
				if (pOption)
				{
	...
	case eTPQ_T_HasShootingPosture:
		{
			if (context.pAIActor)
			{
				...
				if (CPuppet* pPuppet = pAIActor->CastToCPuppet())
				{
					...
					{
						PostureUpr::PostureQuery postureQuery;
						postureQuery.actor = pAIActor;
						postureQuery.position = point.GetPos();
						postureQuery.target = vObjectPos;
						postureQuery.checks = PostureUpr::CheckVisibility | PostureUpr::CheckAimability;
						postureQuery.allowLean = true;
						postureQuery.allowProne = false;
						postureQuery.distancePercent = 1.0f;
						postureQuery.type = PostureUpr::AimPosture;

						if (point.GetType() == ITacticalPoint::eTPT_CoverID)
							postureQuery.coverID = point.GetCoverID();

						eval.postureQueryID = pPuppet->GetPostureUpr().QueryPosture(postureQuery);
					}
					else
					{
						PostureUpr::PostureID postureID;
						PostureUpr::PostureInfo* postureInfo;

						AsyncState state = pPuppet->GetPostureUpr().GetPostureQueryResult(eval.postureQueryID, &postureID,
						                                                                      &postureInfo);
	...
	case eTPQ_PR_CoverDensity:
		{
			...
			/*
			   ...
			   CGraphLinkUpr& linkUpr = gAIEnv.pGraph->GetLinkUpr();
			   set<i32> obstacleIdxSet;
			   ...
			   const GraphNode* pPathfinderCurrent = m_pathfinderCurrent->graphNode;//m_request.m_pGraph->GetNodeUpr().GetNode(m_pathfinderCurrent);
			   for (unsigned linkId = pPathfinderCurrent->firstLinkIndex ; linkId ; linkId = m_request.m_pGraph->GetLinkUpr().GetNextLink(linkId))
			   {
			   unsigned nextNodeIndex = m_request.m_pGraph->GetLinkUpr().GetNextNode(linkId);
			   const GraphNode* nextNode = m_request.m_pGraph->GetNodeUpr().GetNode(nextNodeIndex);


			   // Run through links to all the neighboring nodes of this node
			   unsigned nLinks = pNode->GetLinkCount(linkUpr);
			   ...
			   float fDensity = 0.0f;
			   CAISystem *pAISystem = GetAISystem();
			   for (set<i32>::const_iterator itO = obstacleIdxSet.begin(); itO != obstacleIdxSet.end(); itO++)
			   {
	...
	case eTPQ_PR_FriendlyDistance:
		{
			...
			AutoAIObjectIter iter(
			  gAIEnv.pAIObjectUpr->GetFirstAIObjectInRange(OBJFILTER_TYPE, 0, context.actorPos, fRange, true));
...
bool CTacticalPointSystem::Parse(tukk sSpec, TTacticalPointQuery& _query, TTacticalPointQuery& _limits,
...
TTacticalPointQuery CTacticalPointSystem::Translate(tukk sWord) const
{
	...
	{
		tukk sReportWord = (sWord ? sWord : "NULL");
		if (sReportWord[0] == '\0')
...
ETacticalPointQueryParameter CTacticalPointSystem::TranslateParam(tukk sWord) const
{
	sBuffer.assign(sWord);
...
ETPSRelativeValueSource CTacticalPointSystem::TranslateRelativeValueSource(tukk sWord) const
{
	sBuffer.assign(sWord);
...
tukk CTacticalPointSystem::Translate(TTacticalPointQuery etpToken) const
{
	std::map<TTacticalPointQuery, string>::const_iterator itM;
...
TPSQueryID CTacticalPointSystem::CreateQueryID(tukk psName)
{
...
tukk CTacticalPointSystem::GetQueryName(TPSQueryID nID)
{
...
TPSQueryID CTacticalPointSystem::GetQueryID(tukk psName)
{
...
void CTacticalPointSystem::Update(const float fBudgetSeconds)
{
	...
	do
	{
		...
		if (evaluation.eEvalState == SQueryEvaluation::eCompleted)
		{
			...
			if (bDebugging)
			{
				...
				tukk sName = pAIActor ? pAIActor->GetName() : "NoActor";
	...
	i32 nPreviousAsyncInFrame = (i32) gAIEnv.pStatsUpr->GetStat(eStat_AsyncTPSQueries);   // If this is our only call to Update, this will always be 0
	gAIEnv.pStatsUpr->SetStat(eStat_AsyncTPSQueries, (float) (nPreviousAsyncInFrame + nQueriesProcessed));
...
void CTacticalPointSystem::CallbackQuery(SQueryEvaluation& evaluation)
{
	...
	i32 nOptionUsed = evaluation.iCurrentQueryOption;
	...
	if (nOptionUsed >= 0)
	{
		...
		for (i32 i = 0; i < nPoints; i++, ++it)
		{
			...
			if (b.GetType() == ITacticalPoint::eTPT_EntityPos)
			{
				a.entityId = b.GetEntityId();
				a.flags |= eTPDF_EntityId;
			}
...
bool CTacticalPointSystem::AddToParameters(TPSQueryID queryID, tukk sSpec, float fValue, i32 option)
{
	...
	if (!bOk) AIWarningID("<TacticalPointSystem> ", "AddToParameters(float) failed");
	return bOk;
}

//----------------------------------------------------------------------------------------------//

bool CTacticalPointSystem::AddToParameters(TPSQueryID queryID, tukk sSpec, bool bValue, i32 option)
{
	COptionCriteria* pOption = GetQueryOptionPointer(queryID, option);
	if (!pOption) return false;

	bool bOk = pOption->AddToParameters(sSpec, bValue);
	if (!bOk) AIWarningID("<TacticalPointSystem> ", "AddToParameters(bool) failed");
	return bOk;
}

//----------------------------------------------------------------------------------------------//

bool CTacticalPointSystem::AddToParameters(TPSQueryID queryID, tukk sSpec, tukk sValue, i32 option)
{
	COptionCriteria* pOption = GetQueryOptionPointer(queryID, option);
	if (!pOption) return false;

	bool bOk = pOption->AddToParameters(sSpec, sValue);
	if (!bOk) AIWarningID("<TacticalPointSystem> ", "AddToParameters(string) failed");
	return bOk;
}

//----------------------------------------------------------------------------------------------//

bool CTacticalPointSystem::AddToGeneration(TPSQueryID queryID, tukk sSpec, float fValue, i32 option)
{
	COptionCriteria* pOption = GetQueryOptionPointer(queryID, option);
	if (!pOption) return false;

	bool bOk = pOption->AddToGeneration(sSpec, fValue);
	if (!bOk) AIWarningID("<TacticalPointSystem> ", "AddToGeneration(float) failed");
	return bOk;
}

//----------------------------------------------------------------------------------------------//

bool CTacticalPointSystem::AddToGeneration(TPSQueryID queryID, tukk sSpec, tukk sValue, i32 option)
{
	COptionCriteria* pOption = GetQueryOptionPointer(queryID, option);
	if (!pOption) return false;

	bool bOk = pOption->AddToGeneration(sSpec, TranslateRelativeValueSource(sValue));
	if (!bOk) AIWarningID("<TacticalPointSystem> ", "AddToGeneration(string) failed");
	return bOk;
}

//----------------------------------------------------------------------------------------------//

bool CTacticalPointSystem::AddToConditions(TPSQueryID queryID, tukk sSpec, float fValue, i32 option)
{
	COptionCriteria* pOption = GetQueryOptionPointer(queryID, option);
	if (!pOption) return false;

	bool bOk = pOption->AddToConditions(sSpec, fValue);
	if (!bOk) AIWarningID("<TacticalPointSystem> ", "AddToConditions(float) failed");
	return bOk;
}

//----------------------------------------------------------------------------------------------//

bool CTacticalPointSystem::AddToConditions(TPSQueryID queryID, tukk sSpec, bool bValue, i32 option)
{
	COptionCriteria* pOption = GetQueryOptionPointer(queryID, option);
	if (!pOption) return false;

	bool bOk = pOption->AddToConditions(sSpec, bValue);
	if (!bOk) AIWarningID("<TacticalPointSystem> ", "AddToConditions(bool) failed");
	return bOk;
}

//----------------------------------------------------------------------------------------------//

bool CTacticalPointSystem::AddToWeights(TPSQueryID queryID, tukk sSpec, float fValue, i32 option)
{
	COptionCriteria* pOption = GetQueryOptionPointer(queryID, option);
	if (!pOption) return false;

	bool bOk = pOption->AddToWeights(sSpec, fValue);
	if (!bOk) AIWarningID("<TacticalPointSystem> ", "AddToWeights failed");
	return bOk;

}

//----------------------------------------------------------------------------------------------//
i32 CTacticalPointSystem::TestConditions(TPSQueryID queryID, const QueryContext& context, Vec3& point, bool& bValid) const
{
...
TPSQueryTicket CTacticalPointSystem::AsyncQuery(TPSQueryID queryID, const QueryContext& m_context, i32 flags, i32 nPoints, ITacticalPointResultsReceiver* pReciever)
{
	// Advance ticket - even queries with errors get a ticket, which should aid debugging
	m_nQueryInstanceTicket.Advance();
...
COptionCriteria* CTacticalPointSystem::GetQueryOptionPointer(TPSQueryID nID, i32 option)
{
	if (option < 0 || option > 32) return NULL; // Reject silly numbers of fallback options
...
void CTacticalPointSystem::TPSDescriptionWarning(const CTacticalPointQuery& query, const QueryContext& context, const COptionCriteria* pOption, tukk sMessage) const
{
...
void CTacticalPointSystem::DebugDraw() const
{
	...
	{
		tukk sTarget = gAIEnv.CVars.StatsTarget;
		if (CAIObject* pTargetObject = gAIEnv.pAIObjectUpr->GetAIObjectByName(sTarget))
	...
	while (entryIter != m_lstQueryEvaluationsForDebug.end())
	{
		...
		i32 iDrawMode = CVars.TacticalPointsDebugDrawMode;
		i32 iFadeMode = CVars.TacticalPointsDebugFadeMode;
...
bool CTacticalPointGenerateResult::AddEntity(IEntity* pEntity)
{
	...
	if (pEntity)
	{
		m_Points.push_back(CTacticalPoint(pEntity->GetId(), pEntity->GetWorldPos()));
		bResult = true;
	}
	return bResult;
}
...
bool CTacticalPointGenerateResult::AddEntityPoint(IEntity* pEntity, const Vec3& point)
{
	...
	if (pEntity)
	{
		m_Points.push_back(CTacticalPoint(pEntity->GetId(), point));
		bResult = true;
	}
	return bResult;
}
