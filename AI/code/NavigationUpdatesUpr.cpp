...
//------------------------------------------------------------------------
CMNMUpdatesUpr::CMNMUpdatesUpr(NavigationSystem* pNavSystem)
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::UpdatePostponedChanges()
{
	...
	if (m_pospondedUpdateRequests.size())
	{
		if (queueAndState.status == EUpdateRequestStatus::RequestInQueue)
		{
			...
			for (size_t i = 0; i < m_pospondedUpdateRequests.size(); ++i)
			{
				...
				DRX_ASSERT(request.state == TileUpdateRequest::EState::Postponed);
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::OnMeshDestroyed(NavigationMeshID meshID)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//------------------------------------------------------------------------
const CMNMUpdatesUpr::TileUpdateRequest& CMNMUpdatesUpr::GetFrontRequest() const
{
	return m_updateRequests[m_activeUpdateRequestsQueue.front()];
}

//------------------------------------------------------------------------
void CMNMUpdatesUpr::PopFrontRequest()
{ 
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::RemoveMeshUpdatesFromVector(TileUpdatesVector& tileUpdatesVector, NavigationMeshID meshID)
{
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::EntityChanged(i32 physicalEntityId, const AABB& aabb)
{
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::WorldChanged(const AABB& aabb)
{
#if NAVIGATION_SYSTEM_PC_ONLY
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::RequestGlobalUpdate()
{
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::RequestGlobalUpdateForAgentType(NavigationAgentTypeID agentTypeID)
{
...
//------------------------------------------------------------------------
CMNMUpdatesUpr::EUpdateRequestStatus CMNMUpdatesUpr::RequestMeshUpdate(NavigationMeshID meshID, const AABB& aabb, bool bImmediateUpdate /*= true*/, bool bMarkupUpdate /*= false*/)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	requestParams.flags |= bMarkupUpdate ? u16(TileUpdateRequest::EFlag::MarkupUpdate) : 0;
...
//------------------------------------------------------------------------
CMNMUpdatesUpr::EUpdateRequestStatus CMNMUpdatesUpr::RequestMeshDifferenceUpdate(
...
//------------------------------------------------------------------------
CMNMUpdatesUpr::SRequestParams CMNMUpdatesUpr::GetRequestParams(bool bIsExplicit)
{
	SRequestParams params;
	
	static_assert(sizeof(params.flags) == sizeof(TileUpdateRequest::EFlag), "Invalid type size!");
	params.flags = bIsExplicit ? u16(TileUpdateRequest::EFlag::Explicit) : 0;
...
//------------------------------------------------------------------------
CMNMUpdatesUpr::SRequestParams CMNMUpdatesUpr::GetRequestParamsAfterStabilization(bool bIsExplicit)
{
	SRequestParams params;
	static_assert(sizeof(params.flags) == sizeof(TileUpdateRequest::EFlag), "Invalid type size!");
	params.flags = bIsExplicit ? u16(TileUpdateRequest::EFlag::Explicit) : 0;
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::RequestQueueWorldUpdate(const SRequestParams& queueAndState, const AABB& aabb)
{
...
//------------------------------------------------------------------------
CMNMUpdatesUpr::MeshUpdateBoundaries CMNMUpdatesUpr::ComputeMeshUpdateBoundaries(const NavigationMesh& mesh, const AABB& aabb)
{
	...
	boundaries.minX = (u16)(floor_tpl(bmin.x / (float)paramsGrid.tileSize.x));
	boundaries.maxX = (u16)(floor_tpl(bmax.x / (float)paramsGrid.tileSize.x));

	boundaries.minY = (u16)(floor_tpl(bmin.y / (float)paramsGrid.tileSize.y));
	boundaries.maxY = (u16)(floor_tpl(bmax.y / (float)paramsGrid.tileSize.y));

	boundaries.minZ = (u16)(floor_tpl(bmin.z / (float)paramsGrid.tileSize.z));
	boundaries.maxZ = (u16)(floor_tpl(bmax.z / (float)paramsGrid.tileSize.z));
	return boundaries;
}


//------------------------------------------------------------------------
CMNMUpdatesUpr::MeshUpdateBoundaries CMNMUpdatesUpr::ComputeMeshUpdateDifferenceBoundaries(const NavigationMesh& mesh, const AABB& oldAABB, const AABB& newAABB)
{
	const MNM::CNavMesh& navMesh = mesh.navMesh;
...
//------------------------------------------------------------------------
bool CMNMUpdatesUpr::RequestQueueMeshUpdate(const SRequestParams& requestParams, NavigationMeshID meshID, const AABB& aabb)
{
	DRX_ASSERT(meshID != 0);
...
//------------------------------------------------------------------------
bool CMNMUpdatesUpr::RequestQueueMeshDifferenceUpdate(
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::SwitchUpdateRequestState(size_t requestId, TileUpdateRequest::EState newState)
{
	DRX_ASSERT(newState != TileUpdateRequest::EState::Free);
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::SheduleTileUpdateRequests(const SRequestParams& requestParams, NavigationMeshID meshID, const MeshUpdateBoundaries& bounds)
{
	...
	// Add new tiles
	for (u16 y = bounds.minY; y <= bounds.maxY; ++y)
	{
		for (u16 x = bounds.minX; x <= bounds.maxX; ++x)
		{
			for (u16 z = bounds.minZ; z <= bounds.maxZ; ++z)
			{
...
//------------------------------------------------------------------------
bool CMNMUpdatesUpr::HasBufferedRegenerationRequests() const
{
	return m_ignoredUpdateRequests.size() > 0;
}

//------------------------------------------------------------------------
void CMNMUpdatesUpr::ApplyBufferedRegenerationRequests()
{
	const size_t activeSize = m_activeUpdateRequestsQueue.size();
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::ClearBufferedRegenerationRequests()
{
	m_ignoredUpdateRequests.clear();
}

//------------------------------------------------------------------------
void CMNMUpdatesUpr::DisableRegenerationRequestsAndBuffer()
{
...
void CMNMUpdatesUpr::EnableRegenerationRequestsExecution(bool updateChangedVolumes)
{
	m_bIsRegenerationRequestExecutionEnabled = true;
	...
	if (updateChangedVolumes)
	{
		for (auto& meshWithAabb : m_pendingOldestAabbsOfChangedMeshes)
		{
			...
			DRX_ASSERT(m_pNavigationSystem->m_volumes.validate(mesh.boundary));
...
//------------------------------------------------------------------------
void CMNMUpdatesUpr::DebugDraw()
{
	DRX_ASSERT(m_updateRequests.GetRequestCount() == (m_activeUpdateRequestsQueue.size() + m_pospondedUpdateRequests.size() + m_ignoredUpdateRequests.size()));
...
//////////////////////////////////////////////////////////////////////////
CMNMUpdatesUpr::TileUpdateRequestArray::TileUpdateRequestArray()
...
CMNMUpdatesUpr::TileUpdateRequestArray::TileUpdateRequestArray(const TileUpdateRequestArray& rhs)
	: m_updateRequests(nullptr)
	, m_count(0)
	, m_capacity(0)
{
	// if this assert() triggers, then we must completely re-think the use-cases in which copy-construction can occur
	DRX_ASSERT(rhs.m_updateRequests == nullptr);
}
...
void CMNMUpdatesUpr::TileUpdateRequestArray::Grow(size_t amount)
{
...
void CMNMUpdatesUpr::TileUpdateRequestArray::Init(size_t initialCount)
{
	DRX_ASSERT(m_capacity == 0);
	DRX_ASSERT(m_updateRequests == nullptr);
	Grow(initialCount);
}

void CMNMUpdatesUpr::TileUpdateRequestArray::Clear()
{
	delete[] m_updateRequests;
	m_updateRequests = nullptr;
...
const size_t CMNMUpdatesUpr::TileUpdateRequestArray::AllocateTileUpdateRequest()
{
	DRX_ASSERT(m_count <= m_capacity);
...
void CMNMUpdatesUpr::TileUpdateRequestArray::FreeTileUpdateRequest(const size_t requestIdx)
{
	DRX_ASSERT(requestIdx >= 0);
	DRX_ASSERT(requestIdx < m_capacity);
...
const size_t CMNMUpdatesUpr::TileUpdateRequestArray::GetRequestCount() const
{
	return m_count;
}

CMNMUpdatesUpr::TileUpdateRequest& CMNMUpdatesUpr::TileUpdateRequestArray::operator[](size_t requestIdx)
{
	DRX_ASSERT(requestIdx < m_capacity);
	return m_updateRequests[requestIdx];
}

const CMNMUpdatesUpr::TileUpdateRequest& CMNMUpdatesUpr::TileUpdateRequestArray::operator[](size_t requestIdx) const
{
	DRX_ASSERT(requestIdx < m_capacity);
...
void CMNMUpdatesUpr::TileUpdateRequestArray::Swap(TileUpdateRequestArray& other)
{
