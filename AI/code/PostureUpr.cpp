...
void PostureUpr::CancelRays(QueuedQuery& query)
{
...
void PostureUpr::AddDefaultPostures(PostureType type)
{
	switch (type)
	{
	case HidePosture:
...
PostureUpr::PostureID PostureUpr::AddPosture(const PostureInfo& posture)
{
...
void PostureUpr::SetPosture(PostureID postureID, const PostureInfo& posture)
{
	assert(postureID >= 0 && postureID < (PostureID)m_postureInfos.size());
	m_postureInfos[postureID] = posture;
}

bool PostureUpr::GetPosture(PostureID postureID, PostureInfo* posture) const
{
	if ((postureID < 0) || (postureID >= (i32)m_postureInfos.size()))
		return false;
...
PostureUpr::PostureID PostureUpr::GetPostureID(tukk postureName) const
{
	for (u32 i = 0; i < m_postureInfos.size(); ++i)
	{
...
bool PostureUpr::GetPostureByName(tukk postureName, PostureInfo* posture) const
{
	for (u32 i = 0; i < m_postureInfos.size(); ++i)
	{
...
void PostureUpr::SetPosturePriority(PostureID postureID, float priority)
{
	if ((postureID < 0) || (postureID >= (i32)m_postureInfos.size()))
		return;
...
float PostureUpr::GetPosturePriority(PostureID postureID) const
{
	if ((postureID < 0) || (postureID >= (i32)m_postureInfos.size()))
		return 0.0f;
...
PostureUpr::PostureQueryID PostureUpr::QueryPosture(const PostureQuery& postureQuery)
{
	...
	for (size_t i = 0; i < m_postureInfos.size(); ++i)
	{
		...
		{
			new(&runningPostures[runningPostureCount++])RunningPosture(static_cast<i16>(i));
		}
	}
	...
	u32 skipListSize = skipList.size();
	...
	// Update the postures.
	for (u32 i = 0; i < runningPostureCount; ++i)
	{
		...
		u8 positiveResultCount = 0;
		u8 awaitingResultCount = 0;
		...
		if (postureQuery.checks & CheckVisibility)
		{
			...
			visibilityRayID = gAIEnv.pRayCaster->Queue(RayCastRequest::HighPriority,
			                                           ...
			                                           functor(*this, &PostureUpr::RayComplete));
			++awaitingResultCount;
		}
		else
			++positiveResultCount;
		...
		if (postureQuery.checks & CheckLeanability)
		{
			...
			if (info.parentID != -1)
			{
				for (u32 p = 0; p < runningPostureCount; ++p)
				{
...
void PostureUpr::CancelPostureQuery(PostureQueryID queryID)
{
	QueuedPostureQueries::iterator it = m_queue.begin();
	QueuedPostureQueries::iterator end = m_queue.end();
...
AsyncState PostureUpr::GetPostureQueryResult(PostureQueryID queryID, PostureID* postureID, PostureInfo** postureInfo)
{
	QueuedPostureQueries::iterator it = m_queue.begin();
	QueuedPostureQueries::iterator end = m_queue.end();
...
void PostureUpr::RayComplete(const QueuedRayID& rayID, const RayCastResult& result)
{
	QueuedPostureQueries::iterator it = m_queue.begin();
	QueuedPostureQueries::iterator end = m_queue.end();
