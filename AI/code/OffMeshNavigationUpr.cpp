...
OffMeshNavigationUpr::OffMeshNavigationUpr(i32k offMeshMapSize)
...
const MNM::OffMeshNavigation& OffMeshNavigationUpr::GetOffMeshNavigationForMesh(const NavigationMeshID& meshID) const
{
...
MNM::OffMeshNavigation& OffMeshNavigationUpr::GetOffMeshNavigationForMesh(const NavigationMeshID& meshID)
{
	if (meshID && m_offMeshMap.validate(meshID))
	{
		return m_offMeshMap[meshID];
...
void OffMeshNavigationUpr::QueueCustomLinkAddition(const MNM::LinkAdditionRequest& request)
{
#if DEBUG_MNM_LOG_OFFMESH_LINK_OPERATIONS
	AILogCommentID("<MNM:OffMeshLink>", "OffMeshNavigationUpr::QueueCustomLinkAddition for entity %x", request.requestOwner);
#endif
	m_operations.push_back(request);
}

void OffMeshNavigationUpr::QueueCustomLinkRemoval(const MNM::LinkRemovalRequest& request)
{
#if DEBUG_MNM_LOG_OFFMESH_LINK_OPERATIONS
	AILogCommentID("<MNM:OffMeshLink>", "OffMeshNavigationUpr::QueueCustomLinkRemoval linkId %u for entity %x", request.linkId, request.requestOwner);
#endif
	m_operations.push_back(request);
}

bool OffMeshNavigationUpr::IsLinkRemovalRequested(const MNM::OffMeshLinkID& linkID) const
{
...
bool OffMeshNavigationUpr::AddCustomLink(const NavigationMeshID& meshID, MNM::OffMeshLinkPtr& pLinkData, MNM::OffMeshLinkID& linkID, MNM::TriangleID* pOutStartTriangleID, MNM::TriangleID* pOutEndTriangleID, const bool bCloneLinkData)
{
	// Grab the navigation mesh
	NavigationMesh& mesh = gAIEnv.pNavigationSystem->GetMesh(meshID);

	DRX_ASSERT(pLinkData.get() != nullptr);
	...
	DRX_ASSERT_TRACE(m_offMeshMap.validate(meshID), ("Trying to add offmesh link to invalid mesh! (meshID = %u)", (u32)meshID));
	...
	DRX_ASSERT_TRACE(linkID != MNM::Constants::eOffMeshLinks_InvalidOffMeshLinkID, ("Adding new offmesh link failed"));
...
void OffMeshNavigationUpr::RemoveCustomLink(const MNM::OffMeshLinkID& linkID)
{
...
void OffMeshNavigationUpr::ProcessQueuedRequests()
{
...
void OffMeshNavigationUpr::NotifyAllListenerAboutLinkDeletion(const MNM::OffMeshLinkID& linkID)
{
...
MNM::OffMeshLink* OffMeshNavigationUpr::GetOffMeshLink(const MNM::OffMeshLinkID& linkID)
{
	TLinkInfoMap::iterator linkIt = m_links.find(linkID);
	if (linkIt != m_links.end())
	{
...
const MNM::OffMeshLink* OffMeshNavigationUpr::GetOffMeshLink(const MNM::OffMeshLinkID& linkID) const
{
...
void OffMeshNavigationUpr::RegisterListener(IOffMeshNavigationListener* pListener, tukk listenerName)
{
	m_listeners.Add(pListener, listenerName);
}

void OffMeshNavigationUpr::UnregisterListener(IOffMeshNavigationListener* pListener)
{
	m_listeners.Remove(pListener);
}

void OffMeshNavigationUpr::RemoveAllQueuedAdditionRequestForEntity(const EntityId requestOwner)
{
...
public:
	...
	struct SAgentTypeInfo
	{
		...
		bool                        IsUserClassCompatible(tukk userClass)
		{
	...
	private:
		...
		ILINE void AddUserClass(tukk userClass) { stl::push_back_unique(userClasses, userClass); }
	...
	void GenerateCompatibleTypes()
	{
		...
		for (size_t agentIdx = 0; agentIdx < agentTypeCount; ++agentIdx)
		{
			...
			for (CSmartObjectClass::THelperLinks::const_iterator linkIt = itLinksBegin; linkIt != itLinksEnd; ++linkIt)
			{
				...
				tukk userClassName = (helperLink.condition && helperLink.condition->pUserClass) ? helperLink.condition->pUserClass->GetName().c_str() : NULL;
...
void OffMeshNavigationUpr::RegisterSmartObject(CSmartObject* pSmartObject, CSmartObjectClass* pSmartObjectClass)
{
	...
	while (registerSOHelper.GetAgentInfo(agentTypeInfo))
	{
		...
		if (targetMeshID)
		{
			...
			u32 smartObjectClassNameCrC = CCrc32::ComputeLowercase(pSmartObjectClass->GetName().c_str());
			...
			for (CSmartObjectClass::THelperLinks::const_iterator linkIt = itLinksBegin; linkIt != itLinksEnd; ++linkIt)
			{
				...
				if ((fromIndex != -1) && (toIndex != -1))
				{
					...
					if (!alreadyRegistered)
					{
						...
						request.SetCallback(functor(linkIDList, &OffMeshNavigationUpr::OffMeshLinkIDList::OnLinkAdditionRequestForSmartObjectServiced));
...
void OffMeshNavigationUpr::UnregisterSmartObjectForAllClasses(CSmartObject* pSmartObject)
{
...
void OffMeshNavigationUpr::UnregisterSmartObject(CSmartObject* pSmartObject, const string& smartObjectClassName)
{
	assert(pSmartObject);
	...
	{
		u32 smartObjectClassNameCrC = CCrc32::ComputeLowercase(smartObjectClassName.c_str());
...
void OffMeshNavigationUpr::RefreshConnections(const NavigationMeshID meshID, const MNM::TileID tileID)
{
	if (m_offMeshMap.validate(meshID))
	{
		...
		for (iter = m_links.begin(); iter != m_links.end(); )
		{
			...
			if (!IsLinkRemovalRequested(linkID))
			{
				...
				if (linkIDList)
				{
					...
					request.SetCallback(functor(*linkIDList, &OffMeshNavigationUpr::OffMeshLinkIDList::OnLinkRepairRequestForSmartObjectServiced));
				}
				QueueCustomLinkAddition(request);
			}
		...
		{
			CSmartObject* pSmartObject = CSmartObjectUpr::GetSmartObject(*objectIt);
			assert(pSmartObject);
			if (pSmartObject)
			{
				CSmartObjectClasses& classes = pSmartObject->GetClasses();
...
OffMeshNavigationUpr::OffMeshLinkIDList* OffMeshNavigationUpr::GetClassInfoFromLinkInfo(const SLinkInfo& linkInfo)
{
	...
	DRX_ASSERT_MESSAGE(linkInfo.offMeshLink->GetLinkType() != MNM::OffMeshLink::eLinkType_SmartObject, "Offmesh Link for entity id %u doesn't have SmartObject assigned!", entityId);
	return nullptr;
}

void OffMeshNavigationUpr::Clear()
{
	m_offMeshMap.clear();
...
void OffMeshNavigationUpr::Enable()
{
	m_objectRegistrationEnabled = true;
}

void OffMeshNavigationUpr::OnNavigationMeshCreated(const NavigationMeshID& meshID)
{
...
void OffMeshNavigationUpr::OnNavigationMeshDestroyed(const NavigationMeshID& meshID)
{
	assert(m_offMeshMap.validate(meshID));
	...
	for (TRegisteredObjects::iterator objectIt = m_registeredObjects.begin(); objectIt != m_registeredObjects.end(); ++objectIt)
	{
		for (TSOClassInfo::iterator classInfoIt = objectIt->second.begin(); classInfoIt != objectIt->second.end(); ++classInfoIt)
		{
			...
			for (i32 i = 0; i < removedLinkIDs.size(); ++i)
			{
...
void OffMeshNavigationUpr::OnNavigationLoadedComplete()
{
	//////////////////////////////////////////////////////////////////////////
...
bool OffMeshNavigationUpr::ObjectRegistered(const EntityId objectId, const string& smartObjectClassName) const
{
	u32 smartObjectClassNameCrC = CCrc32::ComputeLowercase(smartObjectClassName.c_str());
...
bool OffMeshNavigationUpr::IsObjectLinkedWithNavigationMesh(const EntityId objectId) const
{
...
void OffMeshNavigationUpr::UpdateEditorDebugHelpers()
{
	if (!gEnv->IsEditing())
		return;
	...
	const ColorB color(255, 0, 0, (u8)(alpha * 255));
	...
	for (; objectIt != endIt; ++objectIt)
	{
		...
		{
			u32 numSOFound = 0;
			gAIEnv.pSmartObjectUpr->GetSOClassTemplateIStatObj(pObjectEntity, NULL, numSOFound);

			ISmartObjectUpr::IStatObjPtr* ppStatObjects = NULL;
			if (numSOFound)
			{
				ppStatObjects = new ISmartObjectUpr::IStatObjPtr[numSOFound];
				if (gAIEnv.pSmartObjectUpr->GetSOClassTemplateIStatObj(pObjectEntity, ppStatObjects, numSOFound) > 0)
				{
...
OffMeshNavigationUpr::ProfileMemoryStats OffMeshNavigationUpr::GetMemoryStats(IDrxSizer* pSizer, const NavigationMeshID meshID) const
{
