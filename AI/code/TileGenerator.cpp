...
static i32k NeighbourOffset_TileGenerator[8][2] =
{
	{ 0,  1  },
	{ -1, 0  },
	{ 1,  0  },
...
static u8k s_PinchCornerTable[3][3][3] =
{
	// *INDENT-OFF* - disable uncrustify's indenting, as I want to preserve specific comment formatting.
	{// UW
	 //   UW         , NB  , WB
...
static u8k s_PaintPinchCornerTable[3][3][3] =
{
	//*INDENT-OFF* - disable uncrustify's indenting, as I want to preserve specific comment formatting.
	{// D1
	 //  D1, SP, D2
...
bool CTileGenerator::Generate(const Params& params, STile& tile, SMetaData& tileMetaData, u32* tileHash)
{
	...
	else
	{
		...
		for (size_t mIdx = 0; mIdx < m_params.markupsCount; ++mIdx)
		{
			...
			hash.Add(static_cast<u32>(volume.bStoreTriangles));
	...
	if (m_params.flags & Params::BuildBVTree)
		tile.CopyNodes(&m_bvtree.front(), static_cast<u16>(m_bvtree.size()));
...
size_t CTileGenerator::VoxelizeVolume(const AABB& volume, u32 hashValueSeed, u32* hashValue)
{
	...
	for (size_t y = 0; y < filterParams.gridHeight; ++y)
	{
		...
		for (size_t x = 0; x < filterParams.gridWidth; ++x)
		{
			if (const CompactSpanGrid::Cell cell = m_spanGrid[x + ymult])
			{
				...
				u32k boundaryFlag(boundaryCell ? TileBoundary : 0);
...
/*static*/ bool CTileGenerator::FilterWalkable_CheckNeighbours(const SSpanCoord& spanCoord, const SSpanClearance& spanClearance, const SFilterWalkableParams& filterParams, SNonWalkableNeighbourReason* pOutReason)
{
	...
	for (size_t n = 0; n < axisNeighbourCount; ++n)
	{
		...
		for (size_t ns = 0; ns < ncount; ++ns)
		{
			...
			const size_t dTop = (size_t)abs((i32)(neighbourClearance.top - spanClearance.top));
			    ...
			    && min(spanClearance.nextBottom, neighbourClearance.nextBottom) >= max(spanClearance.top, neighbourClearance.top) + filterParams.heightVoxelCount)
			{
				...
				sdpTopFirst = sdpTopLast = i32(neighbourClearance.top) - i32(spanClearance.top);
		...
		if (dpTopFirst > 0)
		{
			...
			for (; pc <= filterParams.inclineTestCount; ++pc)
			{
				...
				for (size_t ps = 0; ps < pcount; ++ps)
				{
					...
					const size_t dpTop = (size_t)abs((i32)(probeClearance.top - ptopLast));
					    ...
					    && min(pnextBottomLast, probeClearance.nextBottom) >= max(ptopLast, probeClearance.top) + filterParams.heightVoxelCount)
					{
						...
						const size_t pTopOffs = (size_t)abs(i32(probeClearance.top) - i32(spanClearance.top));
						pTopOffsMax = max(pTopOffsMax, pTopOffs);

						sdpTopLast = (i32)(probeClearance.top - ptopLast);
...
void CTileGenerator::FilterWalkable_CheckBoundaries(const AABB& aabb, const size_t gridWidth, const size_t gridHeight)
{
	...
	for (size_t e = 0; e < m_params.exclusionCount; ++e)
	{
		...
		if (exclusion.Overlaps(aabb))
		{
			...
			u16 xmin = (u16)(std::max(0.0f, emin.x) * convX);
			u16 xmax = (u16)(std::min(bmax.x, emax.x) * convX);

			u16 ymin = (u16)(std::max(0.0f, emin.y) * convY);
			u16 ymax = (u16)(std::min(bmax.y, emax.y) * convY);

			u16 zmin = (u16)(std::max(0.0f, emin.z) * convZ);
			u16 zmax = (u16)(std::min(bmax.z, emax.z) * convZ);
	...
	if (m_params.boundary)
	{
		...
		for (u16 y = 0; y <= gridHeight; ++y)
		{
			const size_t ymult = y * gridWidth;

			for (u16 x = 0; x <= gridWidth; ++x)
			{
...
void CTileGenerator::ComputeDistanceTransform()
{
	...
	// I see no dependency on this particular value and it should be something like u16::max instead.
		...
		for (size_t x = 0; x < gridWidth; ++x)
		{
			if (const CompactSpanGrid::Cell cell = m_spanGrid.GetCell(x, y))
			{
				...
				for (size_t s = 0; s < count; ++s)
				{
					...
					u16k current = m_distances[index + s];

					u16 sweepValues[DistanceTransform::SweepPixelCount];
					...
					u16 minimum = sweepValues[0];

					for (size_t p = 1; p < DistanceTransform::SweepPixelCount; ++p)
					{
						minimum = std::min<u16>(minimum, sweepValues[p]);
					}
				...
				if (const CompactSpanGrid::Cell cell = m_spanGrid.GetCell(x, y))
				{
					...
					for (size_t s = 0; s < count; ++s)
					{
						...
						if (orig > threshold)
						{
							...
							u16k c = static_cast<u16>((accum + 5) / 9);          // round
							m_labels[index + s] = c;
						}
						else
...
struct StreamElement
{
	StreamElement(size_t _x, size_t _y, size_t _span)
		: x(static_cast<u16>(_x))
		, y(static_cast<u16>(_y))
		, span(_span)
	{}
...
size_t RunStream(const CompactSpanGrid& spanGrid, size_t climbableVoxelCount, LinearStream& unexplored,
                 const StreamElement& first, LinearStream& stream, u16k* data, size_t erosion, u16* labels)
{
	stream.clear();
	...
	while (!unexplored.empty())
	{
		...
		while (bfirst)
		{
			for (size_t n = 0; n < 4; ++n)
			{
				...
				{
					u16 label = labels[nindex];

					if (!IsBorderLabel(label))
					{
						u16 ndata = data[nindex];
						if (ndata > minimum)
							minimum = ndata;
					}
				}
...
void CTileGenerator::PaintBorder(u16* data, size_t borderH, size_t borderV)
{
			...
			for (size_t x = 0; x < width; ++x)
			{
				if (const CompactSpanGrid::Cell cell = m_spanGrid.GetCell(x, y))
				{
					...
					for (size_t s = 0; s < count; ++s)
					{
						...
						u16k top = span.bottom + span.height;
...
real_t DistVertexToLineSq(i32 x, i32 y, i32 z, i32 ax, i32 ay, i32 az, i32 bx, i32 by, i32 bz)
{
...
real_t DistVertexToLineSq(i32 x, i32 y, i32 ax, i32 ay, i32 bx, i32 by)
{
...
bool CTileGenerator::GatherSurroundingInfo(const Vec2i& vertex, const Vec2i& direction, u16k top,
                                           u16k climbableVoxelCount, size_t& height, SurroundingSpanInfo& left, SurroundingSpanInfo& front,
...
void CTileGenerator::DetermineContourVertex(const Vec2i& vertex, const Vec2i& direction, u16k top,
                                            u16k climbableVoxelCount, ContourVertex& contourVertex, const bool bInternalContour, SContourVertexDebugInfo* pDebugInfo) const
{
	...
	IF_UNLIKELY (pDebugInfo)
	{
		...
		pDebugInfo->walkableBit = static_cast<u8>(walkableBit);
		pDebugInfo->borderBitH = static_cast<u8>(borderBitH);
		pDebugInfo->borderBitV = static_cast<u8>(borderBitV);
	...
	u8k pinchTableMask = bInternalContour ? PinchCornerTable::eInt : PinchCornerTable::eExt;
	...
	contourVertex.x = static_cast<u16>(cx);
	contourVertex.y = static_cast<u16>(cy);
	contourVertex.z = static_cast<u16>(cz);
	contourVertex.flags = static_cast<u16>(flags);
}
...
i32 CTileGenerator::LabelTracerPath(const CTileGenerator::TracerPath& path, size_t climbableVoxelCount, Region& region, Contour& contour, u16k internalLabel, u16k internalLabelFlags, u16k externalLabel, const bool bIsHole)
{
	i32k numSteps = path.steps.size();
	contour.reserve(numSteps);

	for (i32 i = numSteps - 1, j = 0; j < numSteps; i = j++)
	{
		...
		DetermineContourVertex(Vec2i(curr.pos), curr.GetDir(), curr.pos.z, static_cast<u16>(climbableVoxelCount), *&vertex, bInternalContour, pDebugInfo);
...
inline bool IsWalkable(u16 label, u16 distance, size_t erosion)
{
...
inline bool IsWalkable(const Vec2i& voxel, size_t& index, u16& label, size_t top, size_t climbableVoxelCount,
                       size_t erosion, const CompactSpanGrid& spanGrid, u16k* distances, u16k* labels)
{
...
u16 CTileGenerator::GetPaintVal(const AABB& aabb, size_t x, size_t y, size_t z, size_t index, size_t borderH, size_t borderV, size_t erosion)
{
	// TODO pavloi 2016.03.15: there is already a function bool IsWalkable(u16 label, u16 distance, size_t erosion)
...
void CTileGenerator::CreatePaintPalette()
{
	...
	for (MarkupData& markupData : m_markups)
	{
		...
		else
		{
			...
			u16 pId = 0;
			u16 count = u16(m_paintPalette.size());
			for (; pId < count; ++pId)
			{
...
void CTileGenerator::PaintMarkups(const AABB& tileAabb)
{
	...
	i32k borderH = (i32)BorderSizeH(m_params);
...
void CTileGenerator::PaintMarkupDirect(const MarkupData& markupData, const AABB& tileAabb, const Vec2i& canvasSize, const Vec2i& borderSize, const Vec3& voxelSize, const Vec3& voxelSizeInv)
{
	...
	i32k yStart = max(borderSize.y, i32(tranformedAABB.min.y * voxelSizeInv.y));
	i32k yEnd = min(paintEnd.y, i32(tranformedAABB.max.y * voxelSizeInv.y));

	i32k zStart = i32(tranformedAABB.min.z * voxelSizeInv.z);
	i32k zEnd = i32(tranformedAABB.max.z * voxelSizeInv.z);

	std::vector<i32> xPositions;
	...
	for (i32 y = yStart; y < yEnd; ++y)
	{
		...
		for (size_t i = 0, j = count - 1; i < count; j = i++)
		{
			...
			if (p1.y < yLine && p0.y >= yLine || p0.y < yLine && p1.y >= yLine)
			{
				...
				xPositions.push_back(i32(xPos * voxelSizeInv.x));
			}
		}
		...
		for (i = 0; i < xCount; i += 2)
		{
			...
			for (i32 x = xPositions[i]; x <= xPositions[i + 1]; ++x)
			{
				...
				for (size_t s = 0; s < cell.count; ++s)
				{
					...
					const CompactSpanGrid::Span& span = m_spanGrid.GetSpan(index);
					u32k spanTop = span.bottom + span.height;
...
public:
	...
	i32 m_agentRadius;
	u16 m_paintIdx;
	std::vector<u16> m_paint;
	std::vector<u16> m_distances;

	MarkupCanvas(const Vec2i& rasterMin, const Vec2i& rasterMax, i32 agentRadius, u16 paintIdx)
		...
		, m_paintIdx(paintIdx)
	{
		...
		std::fill(m_distances.begin(), m_distances.end(), 0xffff - u16(DistanceTransform::KDiagonal));
	}
	...
	void Rasterize(const std::vector<Vec3>& vertices, const Vec3& voxelSize, const Vec3& voxelSizeInv)
	{
		...
		for (i32 y = m_paintMin.y; y < m_paintMax.y; ++y)
		{
			xPositions.clear();
			...
			for (size_t i = 0, j = count - 1; i < count; j = i++)
			{
				...
				{
					const float xPos = p0.x + (p1.x - p0.x) * (yLine - p0.y) / (p1.y - p0.y);
					xPositions.push_back(i32(xPos * voxelSizeInv.x) - m_paintMin.x);
				}
			}
	...
	void Erode()
	{
		for (size_t y = 0; y < m_size.y; ++y)
		{
			for (size_t x = 0; x < m_size.x; ++x)
			{
				...
				u16 bestValue = m_distances[index];
				...
				for (size_t p = 0; p < DistanceTransform::SweepPixelCount; ++p)
				{
					...
					if (nx < m_size.x && ny < m_size.y)
					{
						...
						u16k nvalue = m_distances[nindex] + DistanceTransform::downSweep[p].k;
						if (nvalue < bestValue)
						{
							bestValue = nvalue;
						}
		...
		for (size_t y = m_size.y - 1; y < m_size.y; --y)
		{
			for (size_t x = m_size.x - 1; x < m_size.x; --x)
			{
				...
				for (size_t p = 0; p < DistanceTransform::SweepPixelCount; ++p)
				{
					...
					{
						const size_t nindex = (ny * m_size.x) + nx;
						u16k nvalue = m_distances[nindex] + DistanceTransform::upSweep[p].k;
						if (nvalue < bestValue)
						{
							bestValue = nvalue;
						}
		...
		u16k erosion = (m_agentRadius + 1) << 1;
		for (size_t y = 0; y < m_size.y; ++y)
		{
			for (size_t x = 0; x < m_size.x; ++x)
			{
...
void CTileGenerator::PaintMarkupExpanded(const MarkupData& markupData, const AABB& tileAabb, const Vec2i& canvasSize, const Vec2i& borderSize, const Vec3& voxelSize, const Vec3& voxelSizeInv)
{
	...
	const Vec2i rasterMin(max(0, i32(tranformedAABB.min.x * voxelSizeInv.x)), max(0, i32(tranformedAABB.min.y * voxelSizeInv.y)));
	const Vec2i rasterMax(min(canvasSize.x, i32(tranformedAABB.max.x * voxelSizeInv.x)), min(canvasSize.y, i32(tranformedAABB.max.y * voxelSizeInv.y)));
	...
	for (size_t y = 0; y < markupCanvas.m_size.y; ++y)
	{
		for (size_t x = 0; x < markupCanvas.m_size.x; ++x)
		{
			...
			u16k paint = markupCanvas.m_paint[markupIndex];
			if(paint == NoPaint)
				continue;
...
size_t CTileGenerator::ExtractContours(const AABB& aabb)
{
	...
	for (size_t y = borderH; y < gridHeight - borderH; ++y)
	{
		for (size_t x = borderH; x < gridWidth - borderH; ++x)
		{
			...
			{
				const size_t index = cell.index + s;
				u16k label = m_labels[index];
				u16k labelsafe = label & NoLabel;
				...
				u16k prevLabelSafe = (prev.label & NoLabel);
					...
					else
					{
						...
						if (path.turns > 0)
						{
							...
							if ((prev.label & ExternalContour) == 0 && (label & InternalContour) == 0)
							{
								...
								{
									DRX_ASSERT(prevLabelSafe < m_regions.size());
				...
				else if (prevwalkable)
				{
					if ((prev.label & ExternalContour) == 0 && (label & InternalContour) == 0)
					{
						...
						{
							//DrxLogAlways("Ending unknown section.");
						}
					}
				}
			}
...
size_t CTileGenerator::InsertUniqueVertex(VertexIndexLookUp& lookUp, size_t x, size_t y, size_t z)
{
	...
	u32k vertexID = ((u32)(z & zmask) << 20) | ((u32)(x & xmask) << 10) | (y & ymask);

	bool inserted;
	u16& index = *lookUp.insert(vertexID, 0, &inserted);
...
bool CTileGenerator::SimplifyContour(const Contour& contour, const real_t& tolerance2DSq, const real_t& tolerance3DSq, PolygonContour& poly)
{
	...
	u16 simplified[MaxSimplifiedCount];
	...
	if (simplifiedCount && (simplified[simplifiedCount - 1] != (contourSize - 1)))
	{
		...
		{
			simplified[simplifiedCount++] = static_cast<u16>(contourSize - 1);
		}
	}

	if (!simplifiedCount)
	{
		Vec3i min(std::numeric_limits<i32>::max());
		size_t minVertex = 0;

		Vec3i max(std::numeric_limits<i32>::min());
		size_t maxVertex = 0;
		...
		simplified[simplifiedCount++] = static_cast<u16>(minVertex);
		simplified[simplifiedCount++] = static_cast<u16>(maxVertex);
	}
	...
	for (size_t s0 = 0; s0 < simplifiedCount; )
	{
		...
		if (index != ~0ul)
		{
			...
			simplified[s0 + 1] = static_cast<u16>(index);
			++simplifiedCount;
	...
	{
		u16k as = simplified[a];
		u16k bs = simplified[b];
		u16k cs = simplified[c];
		...
		for (size_t h = 0; h < region.holes.size(); ++h)
		{
			...
			else
			{
				...
				for (size_t hvi = 0; hvi < holeSize; ++hvi)
				{
					...
					i32k vertDist = int_ceil(sqrt_tpl((float)((hv - hole.center).GetLength2())));
...
{
	u16k i0 = static_cast<u16>(vertex ? vertex - 1 : vertexCount - 1);
	u16k i1 = static_cast<u16>(vertex);
	u16k i2 = static_cast<u16>((vertex + 1) % vertexCount);

	const VertexTy& v0 = vertices[i0];
	const VertexTy& v1 = vertices[i1];
	const VertexTy& v2 = vertices[i2];

	i32 area = ((i32)v1.x - (i32)v0.x) * ((i32)v2.y - (i32)v0.y) - ((i32)v1.y - (i32)v0.y) * ((i32)v2.x - (i32)v0.x);
	return area > 0;
}

template<typename VertexTy>
inline bool IsReflex(size_t vertex, const VertexTy* vertices, u16* indices, size_t indexCount)
{
	u16k i0 = static_cast<u16>(vertex ? vertex - 1 : indexCount - 1);
	u16k i1 = static_cast<u16>(vertex);
	u16k i2 = static_cast<u16>((vertex + 1) % indexCount);
...
bool IsEar(size_t vertex, const VertexTy* vertices, size_t vertexCount, size_t agentHeight)
{
	...
	// Calc edges
	i32k x01 = ((i32)v0.x - (i32)v1.x);
	i32k y01 = ((i32)v0.y - (i32)v1.y);
	i32k x12 = ((i32)v1.x - (i32)v2.x);
	i32k y12 = ((i32)v1.y - (i32)v2.y);
	i32k x20 = ((i32)v2.x - (i32)v0.x);
	i32k y20 = ((i32)v2.y - (i32)v0.y);
	...
	for (size_t k = 0; k < vertexCount; ++k)
	{
		...
		bool e0 = ((((i32)p.x - (i32)v0.x) * y01 - ((i32)p.y - (i32)v0.y) * x01) < 0);
		bool e1 = ((((i32)p.x - (i32)v1.x) * y12 - ((i32)p.y - (i32)v1.y) * x12) < 0);
		bool e2 = ((((i32)p.x - (i32)v2.x) * y20 - ((i32)p.y - (i32)v2.y) * x20) < 0);
...
template<typename VertexTy>
bool IsEar(size_t vertex, const VertexTy* vertices, size_t vertexCount, u16* indices, size_t indexCount,
           size_t agentHeight)
{
	u16k i0 = static_cast<u16>(vertex ? vertex - 1 : indexCount - 1);
	u16k i1 = static_cast<u16>(vertex);
                                   ...
                                   const size_t borderV, VertexIndexLookUp& lookUp)
{
	...
	while (true)
	{
		...
		for (size_t i = 0; i < vertexCount; ++i)
		{
			...
			u16 i2 = indices[(i + 1) % vertexCount];
			...
			i32k a = (x13 * x23 + y13 * y23);
			i32k b = (y13 * x23 - x13 * y23);
...

		{
			...
			u16 v0i = static_cast<u16>(InsertUniqueVertex(lookUp, v0.x - borderH, v0.y - borderH, v0.z - borderV));
			u16 v2i = static_cast<u16>(InsertUniqueVertex(lookUp, v1.x - borderH, v1.y - borderH, v1.z - borderV));
			u16 v1i = static_cast<u16>(InsertUniqueVertex(lookUp, v2.x - borderH, v2.y - borderH, v2.z - borderV));
			...
			u16k updi0 = static_cast<u16>(bestIdx ? (bestIdx - 1) : (vertexCount - 1));
			u16k updi2 = static_cast<u16>(bestIdx % vertexCount);
...
bool Intersects(const Vec2i& a0, const Vec2i& a1, const Vec2i& b0, const Vec2i& b1)
{
	...
	i32k det = da.x * db.y - da.y * db.x;
	i32k signAdjustment = det >= 0 ? 1 : -1;
	i32k minAllowedValue = 0;
	i32k maxAllowedValue = 1 * det * signAdjustment;
	if (det != 0)
	{
		i32 s = (e.x * db.y - e.y * db.x) * signAdjustment;

		if ((s < minAllowedValue) || (s > maxAllowedValue))
			return false;

		i32 t = (e.x * da.y - e.y * da.x) * signAdjustment;
...
void CTileGenerator::MergeHole(PolygonContour& contour, const size_t contourVertex, const PolygonContour& hole, const size_t holeVertex, i32k distSqr) const
{
		...
		while (!holes.empty())
		{
			...
			for (size_t ci0 = contourNum - 2, ci1 = contourNum - 1, ci2 = 0; ci2 < contourNum; ci0 = ci1, ci1 = ci2++)
			{
				...
				for (size_t hi = 0; hi < numHoles; hi++)
				{
					...
					i32k holeDistSqr = (hole.center - cv1_2D).GetLength2();
					if (bestDist == kMaxDist || holeDistSqr < sqr(hole.rad + bestDist))
					{
						...
						for (size_t vi0 = vertNum - 2, vi1 = vertNum - 1, vi2 = 0; vi2 < vertNum; vi0 = vi1, vi1 = vi2++)
						{
							...
							i32k holeVertDistSqr = diff.GetLengthSquared();
			...
			if (bValid)
			{
				...
				i32k heightMin = min(holeVerts[bestHoleIdx].z, contour[bestContourIdx].z) - agentHeight;
				i32k heightRange = (max(holeVerts[bestHoleIdx].z, contour[bestContourIdx].z) - heightMin) + agentHeight;
				...
				for (size_t i = contourNum - 1, j = 0; j < contourNum; i = j++)
				{
					...
					i32k z0 = contour[i].z - heightMin;
					i32k z1 = contour[j].z - heightMin;
				...
				if (bValid)
				{
					...
					for (size_t i = holeNum - 1, j = 0; j < holeNum; i = j++)
					{
						...
						i32k z0 = holeVerts[i].z - heightMin;
						i32k z1 = holeVerts[j].z - heightMin;
						if ((z0 > heightRange && z1 > heightRange) || (z0 < 0 && z1 < 0))
							continue;
...
struct BVTriangle
{
	BVTriangle(u16 _triangleID = 0, u16 _binID = 0)
              ...
              Tile::SBVNode* nodes, size_t& nodeCount)
{
	...
	const real_t BinConv = (distCentroid > 0) ? (real_t(static_cast<u32>(BinCount)) - BinEpsilon) / distCentroid : 0;
	...
	for (size_t i = firstTri; i < firstTri + triCount; ++i)
	{
		...
		tri.binID = static_cast<u16>(binID);
	...
	real_t costLowest = leftArea * static_cast<u32>(leftTriCount);
	...
	for (size_t i = 0; i < BinCount - 2; ++i)
	{
		...
		const real_t cost = rightArea * static_cast<u32>(rightTriCount) + bin.leftArea * static_cast<u32>(bin.leftTriCount);
	...
	else
	{
		...
		size_t splitTri = std::distance(triangles + firstTri,
		                                std::lower_bound(triangles + firstTri, triangles + firstTri + triCount, BVTriangle(0, static_cast<u16>(split + 1))));
...
void CTileGenerator::BuildBVTree()
{
	...
	for (size_t i = 0; i < triangleCount; ++i)
	{
		...
		bvTri.triangleID = static_cast<u16>(i);
...
void CTileGenerator::CGeneratedMesh::CopyIntoTile(STile& tile) const
{
	...
	{
		DRX_ASSERT_MESSAGE(verticesCount == size_t(verticesMaxIndex), "MNM vertex count overflow");
	}
#endif // DEBUG_MNM_DATA_CONSISTENCY_ENABLED

	tile.CopyTriangles(&m_triangles.front(), static_cast<u16>(min<size_t>(k_maxTriangleCount, m_triangles.size())));
	tile.CopyVertices(&m_vertices.front(), static_cast<u16>(m_vertices.size()));
...
{
	static_assert(std::is_same<u16, typename Tile::Vertex::value_type::value_type>::value, "Tile::Vertex base type is unexpected");
...
void CTileGenerator::CGeneratedMesh::ResetHashSeed(u32 hashSeed)
{
	m_hashComputer = HashComputer(hashSeed);
}

u32 CTileGenerator::CGeneratedMesh::CompleteAndGetHashValue()
{
...
{
	DRX_ASSERT(indexStart >= 0 && indexStart < m_triangles.size());
	DRX_ASSERT(indexEnd >= 0 && indexEnd < m_triangles.size());
	...
	{
		u16 markupIdx = (u16)paintData.markupIdx;
		...
		for (size_t i = indexStart; i <= indexEnd; ++i)
		{
			it->trianglesIdx.push_back(u16(i));
		}
	}
}
