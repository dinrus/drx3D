...
#include <drx3D/AI/AIActor.h> // Big one, but needed for timestamp collection
#include <drx3D/AI/Puppet.h>  // Big one, but needed for the posture manager
...
#include <drx3D/AI/PipeUser.h>                                // Big one, but needed for GetProxy and SetBehavior
#include <drx3D/AI/TacticalPointSystem.h> // Big one, but needed for InitQueryContextFromActor
...
{
u32 TreeLocalGoalPipeCounter = 0;
...
public:
	...
	virtual LoadResult LoadFromXml(const XmlNodeRef& goalPipeXml, const LoadContext& context) override
	{
		...
		reader.ParseGoalPipe(m_goalPipeName, goalPipeXml, CPipeUpr::SilentlyReplaceDuplicate);
...
public:
	struct RuntimeData : public IActorBehaviorListener
	{
		...
		// Overriding IActorBehaviorListener
		virtual void BehaviorChanged(IAIObject* actor, tukk current, tukk previous) override
		{
		}
	};
...
public:
	...
	virtual void OnInitialize(const UpdateContext& context) override
	{
		...
		runtimeData.effectiveStopDistanceSq = square(m_stopWithinDistance + drx_random(0.0f, m_stopDistanceVariation));
...
public:
	...
	virtual LoadResult LoadFromXml(const XmlNodeRef& xml, const LoadContext& context) override
	{
		...
		tukk queryName = xml->getAttr("name");
...
class Communicate : public Action
{
	typedef Action BaseClass;
public:
	struct RuntimeData : public ICommunicationUpr::ICommInstanceListener
	{
		...
		void UnregisterListener()
		{
			if (this->playID)
			{
				gEnv->pAISystem->GetCommunicationUpr()->RemoveInstanceListener(this->playID);
				this->playID = CommPlayID(0);
			}
		}

		void OnCommunicationEvent(
		  ICommunicationUpr::ECommunicationEvent event,
	...
	virtual LoadResult LoadFromXml(const XmlNodeRef& xml, const LoadContext& context) override
	{
		...
		ICommunicationUpr* communicationUpr = gEnv->pAISystem->GetCommunicationUpr();

		tukk commName;
		if (xml->getAttr("name", &commName))
		{
			m_commID = communicationUpr->GetCommunicationID(commName);
			if (!communicationUpr->GetCommunicationName(m_commID))
			{
		...
		{
			m_channelID = communicationUpr->GetChannelID(channelName);
			if (!m_channelID)
			{
...
protected:
	virtual void OnInitialize(const UpdateContext& context) override
	{
		...
		ICommunicationUpr& commUpr = *gEnv->pAISystem->GetCommunicationUpr();
		...
		runtimeData.playID = commUpr.PlayCommunication(request);
...
protected:
	...
	virtual Status Update(const UpdateContext& context) override
	{
		...
		{
			const Group& group = gAIEnv.pGroupUpr->GetGroup(pipeUser.GetGroupId());
			...
			for (; it != end; ++it)
			{
				...
				{
					IAIObject* member = GetAISystem()->GetAIObjectUpr()->GetAIObject(memberID);
					if (member)
					{
...
protected:
	virtual void OnInitialize(const UpdateContext& context) override
	{
		...
		gAIEnv.pAIObjectUpr->CreateDummyObject(runtimeData.fireTarget);
	...
	virtual Status Update(const UpdateContext& context) override
	{
		...
		IF_UNLIKELY (fireTargetAIObject == NULL)
		{
			...
			DRX_ASSERT_MESSAGE(false, "Behavior Tree node Aim suddenly lost its fire target!!");
			return Success;
		}
...
private:
	...
	void UpdateAimingPosition(RuntimeData& runtimeData)
	{
		...
		const float rotationAngle = drx_random(-m_maxAngleRange, m_maxAngleRange);
		Vec3 zAxis(0, 0, 1);
...
private:
	...
	Vec3 ApplyRandomTurnOnNormalXYPlane(const Vec3& initialNormal) const
	{
		...
		float randomAngle = drx_random(m_randomMinAngle, m_randomMaxAngle);
		if (drx_random(0.0f, 1.0f) < m_randomTurnRightChance)
		{
...
protected:
	...
	{
		CPipeUser& pipeUser = *GetPipeUser(context);
		gAIEnv.pTargetTrackUpr->ResetAgent(pipeUser.GetAIObjectID());
...
private:
	void ModifySmartObjectStates(const string& statesToModify, EntityId entityId)
	{
		if (gAIEnv.pSmartObjectUpr)
		{
			...
			{
				gAIEnv.pSmartObjectUpr->ModifySmartObjectStates(pEntity, statesToModify.c_str());
			}
		}
	}
...
public:
	struct RuntimeData
	{
		...
		PostureUpr::PostureID      bestPostureID;
...
protected:
	...
	virtual void OnTerminate(const UpdateContext& context) override
	{
		if (CPipeUser* pipeUser = GetPipeUser(context))
		{
			...
			{
				pipeUser->CastToCPuppet()->GetPostureUpr().CancelPostureQuery(runtimeData.postureQueryID);
	...
	virtual Status Update(const UpdateContext& context) override
	{
		...
		if (runtimeData.postureQueryID == 0)
		{
			...
			query.checks = PostureUpr::DefaultChecks;
			...
			PostureUpr& postureUpr = puppet->GetPostureUpr();
			runtimeData.postureQueryID = postureUpr.QueryPosture(query);
		}
		else
		{
			// Check status
			...
			AsyncState status = postureUpr.GetPostureQueryResult(runtimeData.postureQueryID, &runtimeData.bestPostureID, &postureInfo);
...
private:
	void SetupFireTargetForLocalPosition(CPipeUser& pipeUser, RuntimeData& runtimeData)
	{
		...
		gAIEnv.pAIObjectUpr->CreateDummyObject(runtimeData.dummyTarget);
...
protected:
	virtual Status Update(const UpdateContext& context) override
	{
		if (CPipeUser* pipeUser = GetPipeUser(context))
		{
			gAIEnv.pTargetTrackUpr->PullDownThreatLevel(pipeUser->GetAIObjectID(), AITHREAT_SUSPECT);
		}
...
{
	assert(gAIEnv.pBehaviorTreeUpr);

	IBehaviorTreeUpr& manager = *gAIEnv.pBehaviorTreeUpr;
