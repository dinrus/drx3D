...
void CAIObject::SetPos(const Vec3& pos, const Vec3& dirForw)
{
	...
	if (!GetProxy())
	{
		Vec3 forward = dirForw.GetNormalizedSafe(Vec3Constants<float>::fVec3_Zero);
		m_vMoveDir = forward;
		m_vBodyDir = forward;
		m_vEntityDir = forward;
		m_vView = forward;
	}
...
Vec3 CAIObject::GetPhysicsPos() const
{
	...
	if (pEntity)
	{
		if ((m_expectedPhysicsPosFrameId == gEnv->nMainFrameID))
		{
...
void CAIObject::SetExpectedPhysicsPos(const Vec3& pos)
{
	...
	m_expectedPhysicsPosFrameId = gEnv->nMainFrameID;
...
tukk CAIObject::GetName() const
{
	return m_name.c_str();
}

void CAIObject::SetName(tukk pName)
{
	m_name = pName;
...
void CAIObject::Serialize(TSerialize ser)
{
	...
	i32 formationIndex = CFormation::INVALID_FORMATION_ID;
	...
	{
		m_pFormation = gAIEnv.pFormationUpr->GetFormation(formationIndex);
	}
...
//
//------------------------------------------------------------------------------------------------------------------------
bool CAIObject::CreateFormation(u32k nCrc32ForFormationName, Vec3 vTargetPos /* = ZERO */)
{
	string sFormationName(gAIEnv.pFormationUpr->GetFormationNameFromCRC32(nCrc32ForFormationName));
	return CreateFormation(sFormationName.c_str(), vTargetPos);
}

bool CAIObject::CreateFormation(tukk szName, Vec3 vTargetPos)
{
	if (m_pFormation)
	{
		gAIEnv.pFormationUpr->ReleaseFormation(GetWeakRef(this), true);
	}
	...
	m_pFormation = gAIEnv.pFormationUpr->CreateFormation(GetWeakRef(this), szName, vTargetPos);
...
//
//------------------------------------------------------------------------------------------------------------------------
tukk CAIObject::GetEventName(unsigned short eType) const
{
	switch (eType)
	{
...
// SetGroupId
//====================================================================
void CAIObject::SetGroupId(i32 id)
{
	m_groupId = id;
}
...
// GetGroupId
//====================================================================
i32 CAIObject::GetGroupId() const
{
	return m_groupId;
}

u8 CAIObject::GetFactionID() const
{
	return m_factionID;
}

void CAIObject::SetFactionID(u8 factionID)
{
...
const Vec3 CAIObject::GetPosInNavigationMesh(u32k agentTypeID) const
{
...
u32 CAIObject::GetObservableTypeMask() const
{
	return General;
}
...
void CAIObject::UpdateObservableSkipList() const
{
	if (m_observable)
	{
		...
		ObservableParams observableParams;
		observableParams.skipListSize = std::min<u32>(skipList.size(), ObservableParams::MaxSkipListSize);
		for (size_t i = 0; i < static_cast<size_t>(observableParams.skipListSize); ++i)
			observableParams.skipList[i] = skipList[i];
...
void SAIPredictedCharacterStates::Serialize(TSerialize ser)
{
	ser.BeginGroup("SAIPredictedCharacterStates");
	{
		...
		for (i32 i(0); i < maxStates; ++i, ++counter)
		{
			drx_sprintf(stateGroupName, "State_%d", counter);
...
void SOBJECTSTATE::Serialize(TSerialize ser)
{
	ser.BeginGroup("SOBJECTSTATE");
	{
		...
		{
			i32 signalsCount = vSignals.size();
