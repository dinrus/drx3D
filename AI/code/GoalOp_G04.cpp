...
IGoalOp* CGoalOpFactoryG04::GetGoalOp(tukk sGoalOpName, IFunctionHandler* pH, i32 nFirstParam, GoalParameters& params) const
{
	...
	if (m_eApproachMode == EOPG4AS_Evaluate)
	{
		...
		if (m_refHideTarget.IsNil())
		{
			...
			if (m_eApproachMode == EOPG4AS_GoToHidepoint)
			{
				...
				i32 queryID = pTPS->GetQueryID("G4ApproachSuperior");
				// TODO(marcio): fix
			...
			else
			{
				...
				gAIEnv.pAIObjectUpr->CreateDummyObject(m_refHideTarget, string(pOperand->GetName()) + "_HideTarget");
