...
{
	DRX_ASSERT(coverID || m_state.IsEmpty());
...
{
	DRX_PROFILE_REGION(PROFILE_AI, "CoverUser::Update");

	UpdateBlacklisted(timeDelta);

	if (!m_state.IsEmpty())
	{
		DRX_ASSERT(m_coverID);
		if (m_coverID)
		{
			UpdateCoverEyes();
...
bool CoverUser::IsInCover(const Vec3& pos, float radius, const Vec3* eyes, u32 eyeCount) const
{
