...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   PuppetPhys.cpp
   $Id$
   Описание:	should contaioin all the methods of CPuppet which have to deal with Physics
...
struct SDamageLUTSorter
{
	...
	bool operator()(i32 lhs, i32 rhs) const { return m_values[lhs] < m_values[rhs]; }
...
{
	return Vec3(
	  v.x + drx_random(-amount.x, amount.x),
	  v.y + drx_random(-amount.y, amount.y),
	  v.z + drx_random(-amount.z, amount.z));
}
                               ...
                               float clampAngle, Vec3* posOut)
{
	...
	else
	{
		...
		if (m_lastMissShotsCount == shotsCount && STP_HELIDRXSIS2 != GetSubType())
			out = m_targetLastMissPoint;
		else
		{
			bool found = false;

			if (targetObject && gAIEnv.CVars.EnableCoolMisses && drx_random(0.0f, 1.0f) < gAIEnv.CVars.CoolMissesProbability)
			{
                                                        ...
                                                        Vec3* posOut)
{
	...
	if (m_targetSilhouette.valid)
	{
		...
		if (Overlap::Point_Polygon2D(silhouettePoint, m_targetSilhouette.points))
		{
			...
			float angle = (1.5f * atan2_tpl(bias.y, bias.x)) + drx_random(-1.0f, 1.0f) * spread * 0.5f;
...
bool CPuppet::CalculateHitPointOnTarget(CAIObject* targetObject, const Vec3& target, float clampAngle, Vec3* posOut)
{
	...
	else
	{
		...
		size_t targetPart = s_weights[drx_random((size_t)0, considerCount - 1)].second;
...
Vec3 CPuppet::ChooseMissPoint_Deprecated(const Vec3& vTargetPos) const
{
	...
	i32 trysLimit(5); // how many times try to get good point to shoot
	...
	while (--trysLimit >= 0)
	{
		...
		spreadHoriz = drx_random(0.9f, 1.2f) * (drx_random(0, 99) < 50 ? -1.0f : 1.0f);
		//miss randomly on up/dowm - only in 3D
		//		spreadVert = drx_random(1.2f, 1.5f) * (drx_random(0, 99)<50 ? -1.0f : 1.0f);
		//miss randomly on up/dowm
		spreadVert = drx_random(0.0f, 1.5f) * (drx_random(0, 99) < 50 ? -1.0f : .35f);
...
bool CPuppet::CheckAndGetFireTarget_Deprecated(IAIObject* pTarget, bool lowDamage, Vec3& vTargetPos, Vec3& vTargetDir) const
{
	...
	if (pDamageParts)
	{
		...
		std::vector<i32> partLut;

		// Check if the parts have multipliers set up.
		float accMult = 0.0f;
		i32 n = (i32)parts.size();
		for (i32 i = 0; i < n; ++i)
		...
		if (accMult > 0.001f)
		{
			...
			for (i32 i = 0; i < n; ++i)
			{
				weights[i] = parts[i].damageMult * drx_random(1.0f, 1.01f);
		...
		else
		{
			...
			for (i32 i = 0; i < n; ++i)
			{
				...
				weights[i] = -parts[i].volume * drx_random(1.0f, 1.01f);
				partLut.push_back(i);
			}
		}
		...
		for (std::vector<i32>::iterator it = partLut.begin(); it != partLut.end(); ++it)
		{
