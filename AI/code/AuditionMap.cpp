...
SListenerInstanceId CAuditionMap::CreateListenerInstanceId(tukk szName)
{
	++m_listenerInstanceIdGenerator;
	while (m_listenerInstanceIdGenerator == SListenerInstanceId::s_invalidId)
		++m_listenerInstanceIdGenerator;

	u32k generatedId = m_listenerInstanceIdGenerator;
...
void CAuditionMap::UnregisterListner(EntityId entityId)
{
	...
	m_rayCastUpr.CancelAllPendingStimuliDirectedAtListener(entityId);
	m_listeners.erase(entityId);
}

void CAuditionMap::ListenerChanged(EntityId entityId, const SListenerParams& newListenerParams, u32k changesMask)
{
	Listeners::iterator it = m_listeners.find(entityId);
	DRX_ASSERT(it != m_listeners.end());
...
SListenerInstanceId CAuditionMap::RegisterGlobalListener(const SGlobalListenerParams& globalListenerParams, tukk name)
{
...
{
	DRX_ASSERT(listenerInstanceId != SListenerInstanceId());
...
/*
	...
	DRX_ASSERT(listenerParams.onSoundHeardCallback);
...
void CAuditionMap::DeliverStimulusToListener(const SSoundStimulusParams& soundParams, const EntityId listenerEntityId, const size_t receivingEarIndex)
{
	...
	DRX_ASSERT(false);   // Trying to deliver stimulus to listener that (no longer) exists?
}
...
void CAuditionMap::OnSoundEvent(const SSoundStimulusParams& soundParams, tukk szDebugText)
{
	/*
	#if (AUDITION_MAP_GENERIC_DEBUGGING != 0)
	{
	...
	static_assert((i32)EStimulusObstructionHandling::Count == 2, "Unexpected enum count");
}
...
{
	const float soundLinearFallOffFactor = AuditionHelpers::CAuditionMapRayCastUpr::ComputeSoundLinearFallOffFactor(soundParams.radius);
	AuditionHelpers::SPendingStimulusParamsHelper pendingStimulusParamsHelper(&m_rayCastUpr, soundParams);
	...
	for (Listeners::const_iterator listenerIter = m_listeners.begin(); listenerIter != listenerEndIter; ++listenerIter)
	{
		...
		if (ShouldListenerAcknowledgeSound(listener, soundParams))
		{
			...
			{
				m_rayCastUpr.QueueRaysBetweenStimulusAndListener(soundParams.position, pendingStimulusParamsHelper.GetStimulusIndex(), soundLinearFallOffFactor, listenerEntityId, listener.params);
			}
		}
	}
}
...
void CAuditionMap::DebugDrawGlobalListenersInfo() const
{
	...
	else
	{
		     ...
		     ++globalListenersIter)
		{
			...
			text.Format("name = '%s'   instanceID = %u", listenerDescText.c_str(), (u32)listenerInstanceId);
...
void CAuditionMap::DebugDrawText2D(IRenderer* pRenderer, const Vec2& screenSpacePos, const float fontSize, const ColorF color, tukk pText)
{
	...
	i32k flags = eDrawText_2D | eDrawText_800x600 | eDrawText_FixedSize;
...
void CAuditionMap::DebugDrawTextLineFeed(Vec2* pTopLeftScreenPos, tukk szText, const ColorF textColor / * = Col_White * /)
{
