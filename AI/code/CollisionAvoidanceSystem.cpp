...
CCollisionAvoidanceSystem::AgentID CCollisionAvoidanceSystem::CreateAgent(NavigationAgentTypeID navigationTypeID, const INavMeshQueryFilter* pQueryFilter, tukk szName)
{
		...
		if (agentIndex != nearbyAgentIndex)
		{
			...
			if (nearby && sameFloor && !ignore)
			{
				...
				//if (isVisible)
				{
					...
					nearbyAgents.push_back(SNearbyAgent(distanceSq, static_cast<u16>(nearbyAgentIndex),
