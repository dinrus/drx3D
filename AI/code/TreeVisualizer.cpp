...
void TreeVisualizer::DrawNode(const DebugNode& node, u32k depth)
{
	// Construct a nice readable debug text for this node
	stack_string str;

	// Line
	u32k xmlLine = static_cast<const Node*>(node.node)->GetXmlLine();
	if (xmlLine > 0)
	...
	// Node type
	tukk nodeType = static_cast<const Node*>(node.node)->GetCreator()->GetTypeName();
	str += nodeType;
...
void TreeVisualizer::DrawLine(tukk label, const ColorB& color)
{
...
void TreeVisualizer::DrawTimestampCollection(const TimestampCollection& timestampCollection)
{
	...
	for (; it != end; ++it)
	{
		...
		if (it->IsValid())
		{
		//не работает	s.Format("%s [%.2f]", it->id.timestampName, (timeNow - it->time).GetSeconds());
...
{
	u32k xmlLine = static_cast<const Node*>(debugNode.node)->GetXmlLine();

	stack_string tempOutput;
	tukk szNodeType = static_cast<const Node*>(debugNode.node)->GetCreator()->GetTypeName();
