...
//////////////////////////////////////////////////////////////////////////
CTargetTrackGroup::CTargetTrackGroup(TargetTrackHelpers::ITargetTrackPoolProxy* pTrackPoolProxy, tAIObjectID aiObjectId, u32 uConfigHash, i32 nTargetLimit)
	...
	, m_bEnabled(true)
{
	...
	{
		gAIEnv.pAIObjectUpr->CreateDummyObject(m_dummyPotentialTarget.refDummyRepresentation, "Target Track Dummy");
		if (bAIObjectValid)
			InitDummy();
	}
	...
	m_pDebugHistoryUpr = gEnv->pGameFramework->CreateDebugHistoryUpr();
	assert(m_pDebugHistoryUpr);
...
void CTargetTrackGroup::Serialize_Write(TSerialize ser)
{
	...
	i32 iTrackCount = m_TargetTracks.size();
...
void CTargetTrackGroup::Serialize_Read(TSerialize ser)
{
	...
	for (i32 iTrack = 0; iTrack < iTrackCount; ++iTrack)
	{
	...
	else
	{
		...
		if (FindFreeGraphSlot(uDebugGraphIndex))
		{
			...
			if (pTarget) // might be null in the case of bookmarked entities
			{
				...
				IDebugHistory* pDebugHistory = m_pDebugHistoryUpr->CreateHistory(pTarget->GetName());
				if (pDebugHistory)
				{
...
//////////////////////////////////////////////////////////////////////////
bool CTargetTrackGroup::HandleStimulusEvent(const TargetTrackHelpers::STargetTrackStimulusEvent& stimulusEvent, u32 uStimulusNameHash)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
	...
	return HandleStimulusEvent_Target(stimulusEvent, uStimulusNameHash, pTrack);
}

bool CTargetTrackGroup::TriggerPulse(tAIObjectID targetID, u32 uStimulusNameHash, u32 uPulseNameHash)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//////////////////////////////////////////////////////////////////////////
bool CTargetTrackGroup::TriggerPulse(u32 uStimulusNameHash, u32 uPulseNameHash)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//////////////////////////////////////////////////////////////////////////
bool CTargetTrackGroup::HandleStimulusEvent_All(const TargetTrackHelpers::STargetTrackStimulusEvent& stimulusEvent, u32 uStimulusNameHash)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
//////////////////////////////////////////////////////////////////////////
bool CTargetTrackGroup::HandleStimulusEvent_Target(const TargetTrackHelpers::STargetTrackStimulusEvent& stimulusEvent, u32 uStimulusNameHash, CTargetTrack* pTrack)
{
	DRX_PROFILE_FUNCTION(PROFILE_AI);
...
bool CTargetTrackGroup::GetDesiredTarget(TargetTrackHelpers::EDesiredTargetMethod eMethod, CWeakRef<CAIObject>& outTarget, SAIPotentialTarget*& pOutTargetInfo)
{
	...
	u32 count = GetBestTrack(eMethod, &pBestTrack, 1);
...
//////////////////////////////////////////////////////////////////////////
u32 CTargetTrackGroup::GetBestTrack(TargetTrackHelpers::EDesiredTargetMethod eMethod,
                                       CTargetTrack** tracks, u32 maxCount)
{
	UpdateSortedTracks();
	...
	default:
		DRX_ASSERT_MESSAGE(false, "CTargetTrackGroup::GetBestTrack Unhandled desired target method");
		break;
	}
...
bool CTargetTrackGroup::TestTrackAgainstFilters(CTargetTrack* pTrack, TargetTrackHelpers::EDesiredTargetMethod eMethod) const
{
	...
	CTargetTrackUpr* pUpr = gAIEnv.pTargetTrackUpr;
	assert(pUpr);

	u32k uFilterBitmask = eMethod & TargetTrackHelpers::eDTM_FILTER_MASK;
	...
	{
		/*i32k iTargetLimit = pUpr->GetTargetLimit(aiTargetId);
		   bResult &= (iTargetLimit <= 0 || pUpr->GetDesiredTargetCount(aiTargetId, m_aiObjectId) < iTargetLimit);*/
	}

	if (uFilterBitmask & TargetTrackHelpers::eDTM_Filter_LimitPotential)
	{
		/*i32k iTargetLimit = pUpr->GetTargetLimit(aiTargetId);
		   bResult &= (iTargetLimit <= 0 || pUpr->GetPotentialTargetCount(aiTargetId, m_aiObjectId) < iTargetLimit);*/
	}
...
void CTargetTrackGroup::UpdateTargetRepresentation(const CTargetTrack* pBestTrack, CWeakRef<CAIObject>& outTarget, SAIPotentialTarget*& pOutTargetInfo)
{
	...
	default:
		DRX_ASSERT_MESSAGE(0, "CTargetTrackGroup::UpdateDummyTargetRep Unhandled AI target type");
		break;
	}
...
#ifdef TARGET_TRACK_DEBUG
//////////////////////////////////////////////////////////////////////////
bool CTargetTrackGroup::FindFreeGraphSlot(u32& outIndex) const
{
	outIndex = UINT_MAX;

	for (u32 i = 0; i < DEBUG_GRAPH_OCCUPIED_SIZE; ++i)
	{
...
void CTargetTrackGroup::DebugDrawTracks(TargetTrackHelpers::ITargetTrackConfigProxy* pConfigProxy, bool bLastDraw)
{
	...
	for (; itTrack != itTrackEnd; ++itTrack)
	{
		...
		i32k iIndex = (i32)std::distance(m_SortedTracks.begin(), std::find(m_SortedTracks.begin(), m_SortedTracks.end(), pTrack));
		...
		if (bUpdateGraph)
		{
			...
			{
				u32k uDebugGraphIndex = pTrack->GetDebugGraphIndex();
				u32k uGraphX = uDebugGraphIndex % 3;
				u32k uGraphY = uDebugGraphIndex / 4;

				// Debug graph update
				IDebugHistory* pDebugHistory = m_pDebugHistoryUpr->GetHistory(pTarget->GetName());
				if (pDebugHistory)
				{
...
//////////////////////////////////////////////////////////////////////////
void CTargetTrackGroup::DebugDrawTargets(i32 nMode, i32 nTargetedCount, bool bExtraInfo)
{
	assert(nMode > 0);
	...
	for (; itTrack != itTrackEnd; ++itTrack)
	{
		...
		default:
			DRX_ASSERT_MESSAGE(0, "CTargetTrackGroup::DebugDrawTargets Unhandled target type");
			break;
		}
		if (!bFirst)
		{
			drawColor.r = u8((float)drawColor.r * fProbableRatio);
			drawColor.g = u8((float)drawColor.g * fProbableRatio);
			drawColor.b = u8((float)drawColor.b * fProbableRatio);
			drawColor.a = u8((float)drawColor.a * fProbableRatio);
		}
		...
		if (bExtraInfo)
		{
			...
			dc->Draw3dLabel(vTargetPos + (vPos - vTargetPos) * 0.5f, 1.25f, "%s", sExtraInfoText.c_str());
		}
