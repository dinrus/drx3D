...
class CPathFollower : public IPathFollower
{
	...
	void GetNewLookAheadPos(Vec3& newLAPos, i32& newLASegIndex, const Vec3& curLAPos, i32 curLASegIndex,
	...
	Vec3 GetPathPointAhead(float dist, float& actualDist, i32 curLASegmentIndex, Vec3 curLAPos) const;

	/// As public version but you pass the current state in
	float GetDistToEnd(const Vec3* pCurPos, i32 curLASegmentIndex, Vec3 curLAPos) const;
	...
	u32       GetIndex(const Vec3& pos) const;
...
public:
	...
	virtual float GetDistToCustomNav(const TPathControlPoints& controlPoints, u32 curLASegmentIndex, const Vec3& curLAPos) const;
