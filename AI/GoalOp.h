...
#include <drx3D/Sys/XMLAttrReader.h>
...
enum EOPBranchType
{
	IF_ACTIVE_GOALS = 0,                // Default as it was in DinrusX1 - jumps if there are active (not finished) goal operations.
	IF_ACTIVE_GOALS_HIDE,               // Default as it was in DinrusX1 - jumps if there are active goals or hide spot wasn't found.
...
ILINE tukk GetNameSafe(CAIObject* pObject)
{
	return pObject ? pObject->GetName() : "<Null Object>";
}

inline i32 MovementUrgencyToIndex(float urgency)
{
	i32 sign = urgency < 0.0f ? -1 : 1;
...
{
	CXMLAttrReader<u16>  m_dictAIObjectType;
...
public:
	...
	template<typename T>
	T Get(const XmlNodeRef& node, tukk szAttrName, T defaultValue)
	{
		T value;
	...
	template<typename T>
	bool GetMandatory(const XmlNodeRef& node, tukk szAttrName, T& value)
	{
		if (node->getAttr(szAttrName, value))
		{
			return true;
	...
	bool GetAIObjectType(const XmlNodeRef& node, tukk szAttrName, u16& nValue, bool bMandatory = false)
	{ return m_dictAIObjectType.Get(node, szAttrName, nValue, bMandatory); }
	bool GetAnimationMode(const XmlNodeRef& node, tukk szAttrName, EAnimationMode& eValue, bool bMandatory = false)
	{ return m_dictAnimationMode.Get(node, szAttrName, eValue, bMandatory); }
	bool GetCoverLocation(const XmlNodeRef& node, tukk szAttrName, ECoverUsageLocation& eValue, bool bMandatory = false)
	{ return m_dictCoverLocation.Get(node, szAttrName, eValue, bMandatory); }
	bool GetFireMode(const XmlNodeRef& node, tukk szAttrName, EFireMode& eValue, bool bMandatory = false)
	{ return m_dictFireMode.Get(node, szAttrName, eValue, bMandatory); }
	bool GetLook(const XmlNodeRef& node, tukk szAttrName, ELookMotivation& eValue, bool bMandatory = false)
	{ return m_dictLook.Get(node, szAttrName, eValue, bMandatory); }
	bool GetRegister(const XmlNodeRef& node, tukk szAttrName, EAIRegister& eValue, bool bMandatory = false)
	{ return m_dictRegister.Get(node, szAttrName, eValue, bMandatory); }
	bool GetSignalFilter(const XmlNodeRef& node, tukk szAttrName, ESignalFilter& eValue, bool bMandatory = false)
	{ return m_dictSignalFilter.Get(node, szAttrName, eValue, bMandatory); }
	bool GetStance(const XmlNodeRef& node, tukk szAttrName, EStance& eValue, bool bMandatory = false)
	{ return m_dictStance.Get(node, szAttrName, eValue, bMandatory); }
	bool GetUrgency(const XmlNodeRef& node, tukk szAttrName, float& fValue, bool bMandatory = false)
	{ return m_dictUrgency.Get(node, szAttrName, fValue, bMandatory); }

	bool        GetBool(const XmlNodeRef& node, tukk szAttrName, bool bDefaultValue = false);
	bool        GetMandatoryBool(const XmlNodeRef& node, tukk szAttrName);

	tukk GetMandatoryString(const XmlNodeRef& node, tukk szAttrName);
...
struct IGoalOp
{
	...
	virtual void ParseParam(tukk param, const GoalParams& node) = 0;
...
public:
	...
	virtual void          ParseParam(tukk param, const GoalParams& node) {} //const XmlNodeRef &value) {};

protected:
	static CGoalOpXMLReader s_xml;
};

class CGoalOpParallel : public CGoalOp
{
	...
	struct OpInfo
	{
		...
		OpInfo() : done(false), blocking(false)
		{}
	};
...
public:
	...
	CGoalOpParallel() : m_NextConcurrentPipe(0), m_ConcurrentPipe(0)
	{}
...
public:
	...
	COPAcqTarget(const XmlNodeRef& node) : m_sTargetName(node->getAttr("name")) {}
...
public:
	COPApproach(float fEndDistance, float fEndAccuracy = 1.f, float fDuration = 0.0f,
	            ...
	            bool bForceReturnPartialPath = false, bool bStopOnAnimationStart = false, tukk szNoPathSignalText = 0);
...
public:
	...
	COPFollowPath(bool pathFindToStart, bool reverse, bool startNearest, i32 loops, float fEndAccuracy, bool bUsePointList, bool bControlSpeed, float desiredSpeedOnPath);
...
public:
	...
	COPBackoff(float distance, float duration = 0.f, i32 filter = 0, float minDistance = 0.f);
...
private:
	...
	i32                   m_iDirection;
	i32                   m_iCurrentDirectionIndex;
	std::vector<i32>      m_MoveDirections;
...
class COPLookAround : public CGoalOp
{
	...
	{
		m_scanTimeOutMs = (i32)((m_fScanIntervalRange + drx_random(0.2f, 1.0f)) * 1000.0f);
	}

	ILINE void RandomizeTimeout()
	{
		m_timeOutMs = (i32)(drx_random(m_fIntervalMin, m_fIntervalMax) * 1000.0f);
	}
...
class COPPathFind : public CGoalOp
{
	...
	i32   m_nForceTargetBuildingID; // if >= 0 forces the path destination to be within a particular building
...
public:
	...
	void                  SetForceTargetBuildingId(i32 nForceTargetBuildingID) { m_nForceTargetBuildingID = nForceTargetBuildingID; }
...
public:
	COPLocate(tukk szName, u16 nObjectType = 0, float fRange = 0.f);
...
public:
	COPIgnoreAll(bool bIgnoreAll) : m_bIgnoreAll(bIgnoreAll) {}
	COPIgnoreAll(const XmlNodeRef& node) : m_bIgnoreAll(s_xml.GetBool(node, "id", true)) {}
...
public:
	COPSignal(i32 nSignalID, const string& sSignal, ESignalFilter cFilter, i32 iDataValue) :
...
public:
	COPDeValue(i32 nPuppetsAlso = 0, bool bClearDevalued = false);
...
public:
	COPHide(float distance, float minDistance, i32 method, bool bExact, bool bLookatLastOp)
	{
...
class COPTacticalPos : public CGoalOp
{
	...
	i32                         m_iOptionUsed;               // Stored so that later signals (say, on reaching point) can give the option number
	CStrongRef<CAIObject>       m_refHideTarget;
	COPPathFind*                m_pPathFindDirective;
	COPTrace*                   m_pTraceDirective;
	bool                        m_bLookAtHide;
...
public:
	COPTacticalPos(i32 tacQueryID, EAIRegister nReg);
...
protected:
	...
	void SendStateSignal(CPipeUser* pPipeUser, i32 nState);
};
...
class COPLook : public CGoalOp
{
	...
	EAIRegister m_nReg;
	i32         m_nLookID; // keep track of this look command
	bool        m_bInitialised;
	float       m_fLookTime, m_fTimeLeft;

public:
	COPLook(i32 lookMode, bool bBodyTurn, EAIRegister nReg);
...
public:
	COPForm(tukk name) : m_sName(name) {}
...
public:
	COPLookAt(float fStartAngle, float fEndAngle, i32 mode = 0, bool bBodyTurn = true, bool bUseLastOp = false);
...
public:
	...
	COPWaitSignal(tukk sSignal, float fInterval = 0);
	COPWaitSignal(tukk sSignal, tukk sObjectName, float fInterval = 0);
	COPWaitSignal(tukk sSignal, i32 iValue, float fInterval = 0);
	COPWaitSignal(tukk sSignal, EntityId nID, float fInterval = 0);

	virtual EGoalOpResult Execute(CPipeUser* pPipeUser);
	virtual void          Reset(CPipeUser* pPipeUser);
	virtual void          Serialize(TSerialize ser);

	bool                  NotifySignalReceived(CAIObject* pPipeUser, tukk szText, IAISignalExtraData* pData);
};
...
public:
	COPAnimation(EAnimationMode mode, tukk value, const bool isUrgent);
...
public:
	COPAnimTarget(bool signal, tukk animName, float startWidth, float dirTolerance, float startArcAngle, const Vec3& approachPos, bool autoAlignment);
...
class COPWait : public CGoalOp
{
	...
	i32         m_BlockCount; //	number of non-blocking goals in the block

public:
	COPWait(i32 waitType, i32 blockCount);
...
//
////////////////////////////////////////////////////////
class COPCommunication : public CGoalOp, ICommunicationUpr::ICommInstanceListener
{
	CommID                           m_commID;
...
public:
	COPCommunication(tukk commName, tukk channelName, tukk ordering = "unordered", float expirity = 0.0f, float minSilence = -1.0f, bool ignoreSound = false, bool ignoreAnim = false);
	...
	//ICommunicationUpr::ICommInstanceListener
	virtual void OnCommunicationEvent(ICommunicationUpr::ECommunicationEvent event, EntityId actorID,
