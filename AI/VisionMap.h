...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Implements a visibility determination map for use with AI.
...
class CVisionMap : public IVisionMap
{
public:
	CVisionMap();
	virtual ~CVisionMap();
	...
	virtual VisionID                CreateVisionID(tukk name);
	...
	virtual void                    ObserverChanged(const ObserverID& observerID, const ObserverParams& observerParams, u32 hint);
	virtual void                    ObservableChanged(const ObservableID& observableID, const ObservableParams& newObservableParams, u32 hint);
...
private:
	...
	typedef std::unordered_map<ObservableID, ObservableInfo, stl::hash_uint32>             Observables;
	...
	void AcquireSkipList(IPhysicalEntity** skipList, u32 skipListSize);
	void ReleaseSkipList(IPhysicalEntity** skipList, u32 skipListSize);
...
#if VISIONMAP_DEBUG
	float      m_debugTimer;
	u32     m_numberOfPVSUpdatesThisFrame;
	u32     m_numberOfVisibilityUpdatesThisFrame;
	u32     m_numberOfRayCastsSubmittedThisFrame;
	...
	i32         m_pendingRayCounts[RayCastRequest::TotalNumberOfPriorities];
#endif

	u32 m_visionIdCounter;
};
