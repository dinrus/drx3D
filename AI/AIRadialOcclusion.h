 ...
 *********************************************************************/
#ifndef _AIRADIALOCCLUSION_H_
#define _AIRADIALOCCLUSION_H_
...
{
public:
	CAIRadialOcclusion() : m_center(0, 0, 0)
	{
	...
	inline bool IsVisible(const Vec3& pos) const
	{
		...
		i32 ia = ((i32)floor(a / gf_PI2 * SAMPLE_COUNT)) & SAMPLE_MASK;
		return v.GetLengthSquared2D() < sqr(m_buffer[ia]);
	}

	// Returns sample index from a position
	inline i32 GetIndex(const Vec3& pos) const
	{
		Vec3 v = pos - m_center;
		float a = atan2f(v.y, v.x);
		return ((i32)floor(a / gf_PI2 * SAMPLE_COUNT)) & SAMPLE_MASK;
	}
...
public:
	...
	struct SSpan
	{
		SSpan(float smin, float smax) : smin(smin), smax(smax) {}
		float smin, smax;
	};
...
private:
	...
	{
		SSampleRayQuery() : id(0), i(0), pRaycaster(0) {}
		unsigned                   id;
		unsigned                   i;
	...
	static i32 m_activeCount;

	static SSampleRayQuery* GetQuery(i32 idx, CAIRadialOcclusionRaycast* pCaller)
	{
	...
	static i32  OnRwiResult(const EventPhys* pEvent);
};
