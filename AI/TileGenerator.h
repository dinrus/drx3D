...
#include <drx3D/AI/MNMProfiler.h>
#include <drx3D/AI/CompactSpanGrid.h>
#include <drx3D/AI/Tile.h>
#include <drx3D/AI/BoundingVolume.h>
#include <drx3D/AI/MarkupVolume.h>
#include <drx3D/AI/HashComputer.h>

#include <drx3D/AI/MNMTileGenerator.h>

#include <drx3D/CoreX/Math/SimpleHashLookUp.h>
...
struct STileGeneratorExtensionsContainer
{
	...
	mutable DrxReadModifyLock     extensionsLock;
};
...
public:
	...
	struct SAgentSettings
	{
		...
		u32 radius          : 8; //!< Agent radius in voxels count
		u32 height          : 8; //!< Agent height in voxels count
		u32 climbableHeight : 8; //!< Maximum step height that the agent can still walk through in voxels count
		u32 maxWaterDepth   : 8; //!< Maximum walkable water depth in voxels count
	...
	struct SMetaData
	{
		...
		{
			SMarkupTriangles(u16 markupIdx) : markupIdx(markupIdx) {}
			
			u16 markupIdx;                 //Index of markup volume in markups array
			std::vector<u16> trianglesIdx; //Indices of triangles in generated tile
		};
	...
	bool Generate(const Params& params, STile& tile, SMetaData& tileMetaData, u32* hashValue);
	...
	typedef std::vector<u16> SpanExtraInfo;
...
protected:
	...
	struct ContourVertex
	{
		...
		ContourVertex(u16 _x, u16 _y, u16 _z)
			: x(_x)
			, y(_y)
			, z(_z)
			, flags(0)
	...
	struct PolygonVertex
	{
		...
		PolygonVertex(u16 _x, u16 _y, u16 _z)
			: x(_x)
			, y(_y)
			, z(_z)
			, flags(0)
	...
	struct MarkupData
	{
		...
		i32 markupIdx; // Index of the markup volume in the generator params markups
		i32 paintIdx; // Index of the paint in the palette
	};
	...
	public:
		...
		void                 ResetHashSeed(u32 hashSeed);
		u32               CompleteAndGetHashValue();
	...
	size_t VoxelizeVolume(const AABB& volume, u32 hashValueSeed = 0, u32* hashValue = 0);
	...
	void        PaintBorder(u16* data, size_t borderH, size_t borderV);
	...
	u16 GetPaintVal(const AABB& aabb, size_t x, size_t y, size_t z, size_t index, size_t borderH, size_t borderV, size_t erosion);
	...
	i32    LabelTracerPath(const CTileGenerator::TracerPath& path, size_t climbableVoxelCount, Region& region, Contour& contour, u16k internalLabel, u16k internalLabelFlags, u16k externalLabel, const bool bIsHole);
	...
	typedef simple_hash_lookup<u32, u16> VertexIndexLookUp;
	void       MergeHole(PolygonContour& contour, const size_t contourVertex, const PolygonContour& hole, const size_t holeVertex, i32k distSqr) const;
	...
	{
		SurroundingSpanInfo(u16 _label, size_t _index, size_t _flags = 0)
			: flags(_flags)
			, label(_label)
			, index(_index)
		{}
	...
	bool GatherSurroundingInfo(const Vec2i& vertex, const Vec2i& direction, u16k top,
	...
	void        DetermineContourVertex(const Vec2i& vertex, const Vec2i& direction, u16 top,
	                                   u16 climbableVoxelCount, ContourVertex& contourVertex, const bool bInternalContour, SContourVertexDebugInfo* pDebugInfo) const;
	...
	{
		SNonWalkableReason(tukk szReason_ = nullptr)
			: szReason(szReason_)
			, bIsNeighbourReason(false)
			, neighbourReason()
		{}

		SNonWalkableReason(tukk szReason_, const SNonWalkableNeighbourReason& neighbourReason_)
			: szReason(szReason_)
	...
	inline void DebugAddNonWalkableReason(const SSpanCoord& spanCoord, tukk szReason)
	{
#if DEBUG_MNM_GATHER_NONWALKABLE_REASONS
	...
	struct SContourVertexDebugInfo
	{
		...
		DrxFixedStringT<64>     unremovableReason;
		Tracer                  tracer;
		i32                     tracerIndex;
	...
	void DebugAddContourVertexUnremovableReason(SContourVertexDebugInfo* pDebugInfo, tukk szUnremovableReason) const
	{
#if DEBUG_MNM_GATHER_EXTRA_CONTOUR_VERTEX_INFO
