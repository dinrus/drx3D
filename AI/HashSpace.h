...
public:
	...
	CHashSpace(const Vec3& cellSize, i32 buckets);
	CHashSpace(const Vec3& cellSize, i32 buckets, const Traits& traits);
	~CHashSpace();
	...
	void     GetAABBFromIJK(i32 i, i32 j, i32 k, AABB& aabb) const;
	/// return the individual i, j, k for 3D array lookup
	void     GetIJKFromPosition(const Vec3& pos, i32& i, i32& j, i32& k) const;
	/// returns bucket index from given i,j,k
	unsigned GetHashBucketIndex(i32 i, i32 j, i32 k) const;

	/// Returns true if successful, false otherwise
	bool ReadFromFile(CDrxFile& file);

	/// Populates a given vector with information about bucket usage
	bool RecordBucketUsage(const T& object, std::vector<u32>& cellCounts);

	/// Reserves space in the buckets according to the supplied vector
	bool ReserveSpaceInBuckets(const std::vector<u32>& cellCounts);

	/// Returns the memory usage in bytes
	size_t          MemStats() const;

	inline unsigned GetLocationMask(i32 i, i32 j, i32 k) const
	{
...
//====================================================================
template<typename T, typename TraitsT>
inline CHashSpace<T, TraitsT>::CHashSpace(const Vec3& cellSize, i32 buckets, const Traits& traits) :
	Traits(traits),
	m_cellSize(cellSize),
...
template<typename T, typename TraitsT>
inline CHashSpace<T, TraitsT>::CHashSpace(const Vec3& cellSize, i32 buckets) :
	m_cellSize(cellSize),
	m_cellScale(1.0f / cellSize.x, 1.0f / cellSize.y, 1.0f / cellSize.z),
	m_buckets(buckets),
	m_totalNumObjects(0)
...
//====================================================================
template<typename T, typename TraitsT>
inline void CHashSpace<T, TraitsT >::GetIJKFromPosition(const Vec3& pos, i32& i, i32& j, i32& k) const
{
	i = (i32)floorf(pos.x * m_cellScale.x);
	j = (i32)floorf(pos.y * m_cellScale.y);
	k = (i32)floorf(pos.z * m_cellScale.z);
}
...
//====================================================================
template<typename T, typename TraitsT>
inline unsigned CHashSpace<T, TraitsT >::GetHashBucketIndex(i32 i, i32 j, i32 k) const
{
	i32k h1 = 0x8da6b343;
	i32k h2 = 0xd8163841;
	i32k h3 = 0xcb1ab31f;
	i32 n = h1 * i + h2 * j + h3 * k;
	...
	/*	i32k shift = 5;
	   i32k mask = (1 << shift)-1;
	   i32 n = (i & mask) | ((j&mask) << shift) | ((k&mask) << (shift*2));
	   return (unsigned)(n & (m_buckets-1));*/
}
...
//====================================================================
template<typename T, typename TraitsT>
inline void CHashSpace<T, TraitsT >::GetAABBFromIJK(i32 i, i32 j, i32 k, AABB& aabb) const
{
...
//====================================================================
template<typename T, typename TraitsT>
bool CHashSpace<T, TraitsT >::ReadFromFile(CDrxFile& file)
{
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_Navigation, 0, "HashSpace");
...
//====================================================================
template<typename T, typename TraitsT>
bool CHashSpace<T, TraitsT >::RecordBucketUsage(const T& object, std::vector<u32>& cellCounts)
{
...
//====================================================================
template<typename T, typename TraitsT>
bool CHashSpace<T, TraitsT >::ReserveSpaceInBuckets(const std::vector<u32>& cellCounts)
{
	if (m_cells.size() == cellCounts.size())
	{
