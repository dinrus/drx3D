...
typedef u32 tAIObjectID;   //TODO make a handle or something instead, description needs to be accessible from many places
...
protected:
	union
	{
		struct
		{
			u32 _fastcast_CAIActor         : 1;
			u32 _fastcast_CAIAttribute     : 1; // (MATT) Unused {2009/03/31}
			u32 _fastcast_CAIPlayer        : 1;
			u32 _fastcast_CLeader          : 1;
			u32 _fastcast_CPipeUser        : 1;
			u32 _fastcast_CPuppet          : 1;
			u32 _fastcast_CAIVehicle       : 1;
			u32 _fastcast_CAIFlyingVehicle : 1;
		};
		i32 _fastcast_any;
	};
public:
	...
	virtual const Vec3  GetPosInNavigationMesh(u32k agentTypeID) const = 0;
	...
	//! \note EntityId / other AI objects may not be valid at this point. Use PostSerialize if needed.
	...
	//! \return position weapon fires from.
	virtual const Vec3&  GetFirePos() const = 0;

	virtual IBlackBoard* GetBlackBoard() { return NULL; }

	virtual void         SetGroupId(i32 id) = 0;
	virtual i32          GetGroupId() const = 0;
	...
	virtual void         SetFactionID(u8 factionID) = 0;

	//! \return true if the AIObject is AIActor (but not CLeader).
	...
	////////////////////////////////////////////////////////////////////////////////////////
	// Formations
	virtual bool        CreateFormation(u32k nCrc32ForFormationName, Vec3 vTargetPos = ZERO) = 0;
	virtual bool        CreateFormation(tukk szName, Vec3 vTargetPos = ZERO) = 0;
