...
struct GoalParams
{
	...
	GoalParams(i32 value) : m_Name(0) { SetValue(value); }
	GoalParams(float value) : m_Name(0) { SetValue(value); }
	GoalParams(tukk value) : m_Name(0) { SetValue(value); }
	...
	void              SetValue(i32 number)               { m_Data.number = number; m_Data.d = eD_NUMBER; }
	bool              GetValue(i32& number) const        { number = m_Data.number; return m_Data.d == eD_NUMBER; }

	void              SetValue(u32 number)            { m_Data.number = number; m_Data.d = eD_NUMBER; }
	bool              GetValue(u32& number) const     { number = m_Data.number; return m_Data.d == eD_NUMBER; }
	...
	void              SetValue(tukk str)          { m_Data.str = str; m_Data.d = eD_STRING; }
	bool              GetValue(tukk & str) const   { str = m_Data.str; return m_Data.d == eD_STRING; }
	...
	void              SetName(tukk name)          { m_Name = name; }
	...
	GoalParams&       GetChild(u32 index)             { assert(static_cast<DynArray<GoalParams>::size_type>(index) < m_Childs.size()); return m_Childs[index]; }
	const GoalParams& GetChild(u32 index) const       { assert(static_cast<DynArray<GoalParams>::size_type>(index) < m_Childs.size()); return m_Childs[index]; }
...
struct IGoalPipe
{
	...
	//! \note This new PushGoal shouldn't need all these parameters but will do for a while.
	...
	virtual void PushLabel(tukk label) = 0;
	virtual void PushPipe(tukk szName, bool bBlocking, EGroupType eGrouping, GoalParameters& params) = 0;
	virtual void SetDebugName(tukk name) = 0;
