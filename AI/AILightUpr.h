...
private:
	...
	struct SAIDynLightSource
	{
		SAIDynLightSource(const Vec3& pos, const Vec3& dir, float radius, float fov, EAILightLevel level, EAILightEventType type, CWeakRef<CAIActor> _refShooter, CCountedRef<CAIObject> _refAttrib, float t) :
			pos(pos), dir(dir), radius(radius), fov(fov), level(level), type(type), refShooter(_refShooter), refAttrib(_refAttrib), t(0), tmax((i32)(t * 1000.0f)) {}
		SAIDynLightSource() :
			pos(ZERO), dir(Vec3Constants<float>::fVec3_OneY), radius(0), fov(0), level(AILL_NONE), type(AILE_GENERIC), t(0), tmax((i32)(t * 1000.0f)) {}
