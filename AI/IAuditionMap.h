...
{
	static i32k kAuditionMapMaxEarsPerListener = 4;
	...
	protected:
		...
		SListenerInstanceId(u32k id) : m_id(id) {}
...
struct IAuditionMap
{
	...
	virtual void ListenerChanged(EntityId entityId, const Perception::SListenerParams& newListenerParams, u32k changesMask) = 0;

	virtual Perception::SListenerInstanceId RegisterGlobalListener(const Perception::SGlobalListenerParams& globalListenerParams, tukk name) = 0;
	virtual void UnregisterGlobalListener(const Perception::SListenerInstanceId& listenerInstanceId) = 0;

	virtual void OnSoundEvent(const Perception::SSoundStimulusParams& soundParams, tukk szDebugText) = 0;
