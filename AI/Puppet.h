...
public:
	...
	inline PostureUpr&       GetPostureUpr()   { return m_postureUpr; };
	...
	void SetDelayedStance(i32 stance);
	...
	virtual bool CreateFormation(tukk szName, Vec3 vTargetPos);
	...
	// Описание: returns the distance of a point along the puppet's path
	// Arguments:
	// Vec3& pos: point
	...
	u32        GetBestTargets(tAIObjectID* targets, u32 maxCount) const;
	...
	void RequestThrowGrenade(ERequestedGrenadeType eGrenadeType, i32 iRegType);

	// Returns the current alertness level of the puppet.
	inline i32                       GetAlertness() const { return m_Alertness; }
	...
	// [AlexMcC|26.04.10]: These won't work with CTargetTrackUpr
...
protected:
	...
	struct STargetSelectionInfo
	{
		...
		STargetSelectionInfo() : targetThreat(AITHREAT_NONE), targetType(AITARGET_NONE), pTargetInfo(),
			bCurrentTargetErased(false), bIsGroupTarget(false) {}
	};
	...
	CPersonalInterestUpr* GetPersonalInterestUpr();
	...
	i32                  m_lastChaseUrgencyDist;
	i32                  m_lastChaseUrgencySpeed;
	...
	i32                   m_delayedStance;
	i32                   m_delayedStanceMovementCounter;
...
public:
	...
	public:
		...
		i32  GetQueries() const { return m_queries; }
		i32  GetHits() const    { return m_hits; }
