...
#include <drx3D/CoreX/Containers/VariableCollection.h>
#include <drx3D/AI/TimestampCollection.h>
...
public:
	...
	Event(u32 nameCRC32, ukk pData = 0)
...
#ifdef STORE_EVENT_NAME
	Event(u32 nameCRC32, tukk name, ukk pData = 0)
		: m_nameCRC32(nameCRC32)
		, m_name(name)
		, m_pData(pData)
	{
...
private:
	virtual uk       GetDataVoidPointer() = 0;
	virtual ukk GetDataVoidPointer() const = 0;
};
...
private:
	...
	virtual uk       GetDataVoidPointer() override       { return &m_value; }
	virtual ukk GetDataVoidPointer() const override { return &m_value; }
...
struct BlackboardVariableId
{
	...
	BlackboardVariableId(tukk _name)
	{
	...
	static inline u32 GetIdValueFromName(tukk name)
	{
...
protected:
	...
	void AddMetaExtension(IMetaExtensionPtr pExtension)
	{
		...
		{
			DrxFatalError("Behavior tree meta extension already registered!");
		}
...
struct BehaviorTreeTemplate
{
	...
	DrxFixedStringT<64> mbtFilename;
#endif
};
...
#endif
	virtual void                           LoadFromDiskIntoCache(tukk behaviorTreeName) = 0;
	virtual bool StartModularBehaviorTree(const EntityId entityId, tukk treeName) = 0;
	virtual bool StartModularBehaviorTreeFromXml(const EntityId entityId, tukk treeName, XmlNodeRef treeXml) = 0;
...
struct INodeCreator
{
	...
	virtual uk       AllocateRuntimeData(const RuntimeDataID runtimeDataID) = 0;
	virtual uk       GetRuntimeData(const RuntimeDataID runtimeDataID) const = 0;
...
struct INodeFactory
{
	...
	virtual INodePtr CreateNodeOfType(tukk typeName) = 0;
	...
	virtual size_t   GetSizeOfImmutableDataForAllAllocatedNodes() const = 0;
	virtual size_t   GetSizeOfRuntimeDataForAllAllocatedNodes() const = 0;
	virtual uk    AllocateRuntimeDataMemory(const size_t size) = 0;
	virtual void     FreeRuntimeDataMemory(uk pointer) = 0;
	virtual uk    AllocateNodeMemory(const size_t size) = 0;
	virtual void     FreeNodeMemory(uk pointer) = 0;
};
...
template<typename NodeType> class NodeCreator : public INodeCreator
{
	...
	typedef VectorMap<RuntimeDataID, uk> RuntimeDataCollection;

	friend class NodeDeleter<NodeType, ThisNodeCreatorType>;

public:
	NodeCreator(tukk typeName)
		: m_typeName(typeName)
		, m_nodeCount(0)
	{
	}
	...
	virtual INodePtr Create() override
	{
		...
		uk const pointer = m_nodeFactory->AllocateNodeMemory(sizeof(NodeType));

		if (pointer == NULL)
		{
			DrxFatalError("Could not allocate the memory needed for a '%s' node.", GetTypeName());
		}
	...
	virtual uk AllocateRuntimeData(const RuntimeDataID runtimeDataID) override
	{
		DRX_PROFILE_FUNCTION(PROFILE_AI);
		uk pointer = m_nodeFactory->AllocateRuntimeDataMemory(sizeof(RuntimeDataType));
		RuntimeDataType* runtimeData = new(pointer) RuntimeDataType;
		assert(runtimeData != NULL);
		m_runtimeDataCollection.insert(std::make_pair(runtimeDataID, reinterpret_cast<uk>(runtimeData)));
		return runtimeData;
	}

	virtual uk GetRuntimeData(const RuntimeDataID runtimeDataID) const override
	{
		DRX_PROFILE_FUNCTION(PROFILE_AI);
