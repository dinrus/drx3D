...
private:
	...
	AgentID               CreateAgent(NavigationAgentTypeID navigationTypeID, const INavMeshQueryFilter* pFilter, tukk szName);
	...
	struct SNearbyAgent
	{
		...
		SNearbyAgent(float _distanceSq, u16 _agentID, u16 _flags = 0)
	...
	struct SNearbyObstacle
	{
		...
		SNearbyObstacle(float _distanceSq, u16 _agentID, u16 _flags = 0)
			: distanceSq(_distanceSq)
			, obstacleID(_agentID)
			, flags(_flags)
		{
	...
	typedef DrxFixedArray<ICollisionAvoidanceAgent*, kMaxAvoidingAgents> CurrentAvoidingAgents;
