...
public:
	...
	const CTemplateHelper* GetHelper(tukk helperName) const
	{
	...
	{
		if (!m_pHelperMtl)
			m_pHelperMtl = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial("%EDITOR%/Objects/Helper");
		return m_pHelperMtl;
	};
};
...
public:
	...
	class CState
	{
		typedef std::map<string, i32>    MapSmartObjectStateIds;
		typedef std::vector<tukk > MapSmartObjectStates;

		static MapSmartObjectStateIds g_mapStateIds;
		static MapSmartObjectStates   g_mapStates;
		static SetStates              g_defaultStates;

		void Assign(tukk state)
		{
			if (*state)
			{
				std::pair<MapSmartObjectStateIds::iterator, bool> pr = g_mapStateIds.insert(std::make_pair(state, static_cast<i32>(g_mapStates.size())));
	...
	public:
		...
		i32                asInt() const                         { return m_StateId; }
		tukk        c_str() const                         { return (m_StateId >= 0 && m_StateId < (i32)g_mapStates.size()) ? g_mapStates[m_StateId] : ""; }

		static i32         GetNumStates()                        { return g_mapStates.size(); }
		static tukk GetStateName(i32 i)                   { return i >= 0 && i < (i32)g_mapStates.size() ? g_mapStates[i] : NULL; }
	};
...
public:
	...
	void Serialize(TSerialize ser);
	void SerializePointer(TSerialize ser, tukk name, CSmartObject*& pSmartObject);
	...
	void             Use(CSmartObject* pObject, CCondition* pCondition, i32 eventId = 0, bool bForceHighPriority = false) const;
...
public:
	...
	CSmartObjectClass(tukk className);
	~CSmartObjectClass();

	static CSmartObjectClass* find(tukk sClassName);
	...
	{
		u32 size = m_allSmartObjectInstances.size();
	...
	SmartObjectHelper* GetHelper(tukk name) const
	{
	...
	i32  FindHelperLinks(const SmartObjectHelper* from, const SmartObjectHelper* to, const CSmartObjectClass* pClass, float radius,
...
///////////////////////////////////////////////
// CSmartObjectUpr receives notifications from entity system about entities being spawned and deleted.
...
///////////////////////////////////////////////
class CSmartObjectUpr final
	: public IEntitySystemSink
	, public IEntityEventListener
	, public ISmartObjectUpr
{
private:
...
public:
	...
	virtual tukk        GetSmartObjectStateName(i32 id) const;

	virtual void               RegisterSmartObjectState(tukk sStateName);
	virtual void               AddSmartObjectState(IEntity* pEntity, tukk sState);
	virtual void               RemoveSmartObjectState(IEntity* pEntity, tukk sState);
	virtual bool               CheckSmartObjectStates(const IEntity* pEntity, tukk patternStates) const;
	virtual void               SetSmartObjectState(IEntity* pEntity, tukk sStateName);

	virtual void               ModifySmartObjectStates(IEntity* pEntity, tukk listStates);

	virtual void               DrawSOClassTemplate(IEntity* pEntity);
	virtual bool               ValidateSOClassTemplate(IEntity* pEntity);

	virtual u32             GetSOClassTemplateIStatObj(IEntity* pEntity, IStatObjPtr* ppStatObjectPtrs, u32& numAllocStatObjects);

	virtual void               ReloadSmartObjectRules();

	virtual i32                SmartObjectEvent(tukk sEventName, IEntity*& pUser, IEntity*& pObject, const Vec3* pExtraPoint = NULL, bool bHighPriority = false);

	virtual SmartObjectHelper* GetSmartObjectHelper(tukk className, tukk helperName) const;
	//ISmartObjectUpr/////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////
	...
	i32 TriggerEvent(tukk sEventName, IEntity*& pUser, IEntity*& pObject, QueryEventMap* pQueryEvents = NULL, const Vec3* pExtraPoint = NULL, bool bHighPriority = false);
	...
	i32                  GetNavigationalSmartObjectActionTypeForMNM(CPipeUser* pPipeUser, CSmartObject* pSmartObject, CSmartObjectClass* pSmartObjectClass, SmartObjectHelper* pFromHelper, SmartObjectHelper* pToHelper);
	...
	void                 UseSmartObject(CSmartObject* pSmartObjectUser, CSmartObject* pObject, CCondition* pCondition, i32 eventId = 0, bool bForceHighPriority = false);
...
private:
	...
	i32                               FindSmartObjectLinks(const CPipeUser* pPipeUser, CSmartObject* pObject, CSmartObjectClass* pObjectClass,
	                                                       SmartObjectHelper* pFromHelper, SmartObjectHelper* pToHelper, CSmartObjectClass::HelperLink** pvHelperLinks,
	                                                       i32 iCount = 1, const IEntity* pAdditionalEntity = NULL, const CSmartObject::SetStates* pStatesExcludedFromMatchingTheObjectState = NULL) const;
	...
	static void        String2States(tukk listStates, CSmartObject::DoubleVectorStates& states);
	static void        String2StatePattern(tukk sPattern, CSmartObject::CStatePattern& pattern);

	static void        SerializePointer(TSerialize ser, tukk name, CSmartObject*& pSmartObject);
	static void        SerializePointer(TSerialize ser, tukk name, CCondition*& pRule);

	CSmartObjectClass* GetSmartObjectClass(tukk className);
	...
	i32                Process(CSmartObject* pSmartObjectUser, CSmartObjectClass* pClass);
	...
	bool ValidateTemplate(IEntity* pEntity, bool bStaticOnly, IEntity* pUserEntity = NULL, i32 fromTemplateHelperIdx = -1, i32 toTemplateHelperIdx = -1);
	bool ValidateTemplate(CSmartObject* pSmartObject, bool bStaticOnly, IEntity* pUserEntity = NULL, i32 fromTemplateHelperIdx = -1, i32 toTemplateHelperIdx = -1);
...
private:
	...
	i32  TriggerEventUserObject(tukk sEventName, CSmartObject* pUser, CSmartObject* pObject, QueryEventMap* pQueryEvents, const Vec3* pExtraPoint, bool bHighPriority);
	i32  TriggerEventUser(tukk sEventName, CSmartObject* pUser, QueryEventMap* pQueryEvents, IEntity** ppObjectEntity, const Vec3* pExtraPoint, bool bHighPriority);
	i32  TriggerEventObject(tukk sEventName, CSmartObject* pObject, QueryEventMap* pQueryEvents, IEntity** ppUserEntity, const Vec3* pExtraPoint, bool bHighPriority);
};
...
tukk CSmartObjectBase::GetName() const
{
...
struct OffMeshLink_SmartObject : public MNM::OffMeshLink
{
	...
	{
		return gAIEnv.pSmartObjectUpr->GetSmartObjectLinkCostFactorForMNM(this, pRequester, costMultiplier);
	}
