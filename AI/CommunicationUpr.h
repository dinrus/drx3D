...
#ifndef __CommunicationUpr_h__
#define __CommunicationUpr_h__
...
/**
   ...
   The CommunicationUpr acts as the heart of the communication system.
   ...
   The CommunicationUpr is a global singleton managing:
 ...
 * A set of communication channels controlled by CommunicationChannelUpr:
 ...
 * A CommunicationPlayer instance...
   ...
   The CommunicationPlayer is owned by the CommunicationUpr singleton and manages:
 ...
 * Informs CommunicationUpr via listener of finished communications.
...
public:
	CCommunicationUpr(tukk configurationFileName = NULL);

	// ICommunicationUpr
	virtual u32        GetConfigCount() const override;
	virtual tukk   GetConfigName(u32 index) const override;
	virtual CommConfigID  GetConfigIDByIndex(u32 index) const override;

	virtual CommChannelID GetChannelID(tukk name) const override;
	virtual CommConfigID  GetConfigID(tukk name) const override;
	virtual CommID        GetCommunicationID(tukk name) const override;
	virtual tukk   GetCommunicationName(const CommID& communicationID) const override;
	...
	virtual void          RegisterListener(ICommGlobalListener* eventListener, tukk name = NULL) override { m_globalListeners.Add(eventListener, name); }
	...
	virtual u32        GetConfigCommunicationCount(const CommConfigID& configID) const override;
	virtual tukk   GetConfigCommunicationNameByIndex(const CommConfigID& configID, u32 index) const override;
	...
	virtual void SetVariableValue(tukk variableName, const bool newVariableValue) override;
	virtual void GetVariablesNames(tukk* variableNames, const size_t maxSize, size_t& actualSize) const override;
	//~ICommunicationUpr

	size_t GetPriority(const CommPlayID& playID) const;

	void   Reload();
	void   LoadConfigurationAndScanRootFolder(tukk rootFolder);
	...
	virtual void OnCommunicationFinished(const CommPlayID& playID, u8 stateFlags) override;
	//~ICommunicationFinishedListener
	...
	CommunicationTestUpr&   GetTestUpr();

private:
	void ScanFolder(tukk folderName);

	bool LoadCommunicationSettingsXML(tukk fileName);
	...
	void LoadVariables(const XmlNodeRef& rootNode, tukk fileName);
	void LoadGlobalConfiguration(const XmlNodeRef& rootNode);

	bool ChooseVariation(const SCommunication& comm, const SCommunication::EVariationChoiceMethod method, u32k index, u32& outputIndex) const;
	bool CanVariationBePlayed(const SCommunication& comm, u32k variationIndex) const;
	void UpdateHistory(SCommunication& comm, u32 variationPlayed);
	...
	CommunicationChannelUpr m_channels;
	...
	mutable std::vector<u8> m_randomPool;

	CommunicationTestUpr   m_testUpr;
	...
	static tukk m_kTargetIsAbove_VariableName;
	static tukk m_kTargetIsBelow_VariableName;

	WWiseConfiguration m_wiseConfiguration;
};

#endif // __CommunicationUpr_h__
