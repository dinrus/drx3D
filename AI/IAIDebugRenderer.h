...
/********************************************************************
   ...
   - 2009-2-11   : Moved to DrxCommon by Evgeny Adamenkov
...
struct IAIDebugRenderer
{
	...
	//! Draw agent circles.
	virtual void DrawCircles(const Vec3& vPos, float fMinRadius, float fMaxRadius, i32 numRings, const ColorF& insideColor, const ColorF& outsideColor) = 0;
	...
	virtual void Draw2dLabel(i32 nCol, i32 nRow, tukk szText, const ColorB& color) = 0;

	virtual void Draw2dLabel(float fX, float fY, float fFontSize, const ColorB& color, bool bCenter, tukk text, ...) PRINTF_PARAMS(7, 8) = 0;

	//! Draw label in the game world (rather than on the screen).
	virtual void Draw3dLabel(Vec3 vPos, float fFontSize, tukk text, ...) PRINTF_PARAMS(4, 5) = 0;
	virtual void Draw3dLabelEx(Vec3 vPos, float fFontSize, const ColorB& color, bool bFixedSize, bool bCenter, bool bDepthTest, bool bFramed, tukk text, ...) PRINTF_PARAMS(9, 10) = 0;

	//! Draw label
	virtual void DrawLabel(Vec3 pos, SDrawTextInfo& ti, tukk text) = 0;

	//! Draw a 2D image on the screen (HUD etc.).
	virtual void Draw2dImage(float fX, float fY, float fWidth, float fHeight, i32 nTextureID, float fS0 = 0, float fT0 = 0, float fS1 = 1, float fT1 = 1, float fAngle = 0, float fR = 1, float fG = 1, float fB = 1, float fA = 1, float fZ = 1) = 0;
	...
	virtual void DrawPolyline(const Vec3* va, u32 nPoints, bool bClosed, const ColorB& color, float fThickness = 1.0f) = 0;
	virtual void DrawPolyline(const Vec3* va, u32 nPoints, bool bClosed, const ColorB* colorArray, float fThickness = 1.0f) = 0;
	...
	virtual void DrawRangePolygon(const Vec3 polygon[], i32 nVertices, float fWidth, const ColorB& colorFill, const ColorB& colorOutline, bool bDrawOutline) = 0;
	...
	virtual void DrawTriangles(const Vec3* va, u32 numPoints, const ColorB& color) = 0;
	...
	virtual ITexture* LoadTexture(tukk sNameOfTexture, u32 nFlags) = 0;
	...
	//! Draw text on screen.
	virtual void TextToScreen(float fX, float fY, tukk format, ...) PRINTF_PARAMS(4, 5) = 0;

	//! Draw text on screen in color.
	virtual void TextToScreenColor(i32 nX, i32 nY, float fRed, float fGreen, float fBlue, float fAlpha, tukk format, ...) PRINTF_PARAMS(8, 9) = 0;
	...
	virtual u32 PopState() = 0;
	virtual u32 PushState() = 0;
	// </interfuscator:shuffle>
};
