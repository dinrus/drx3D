...
template<class TYPE, i32 SORT_AFTER = 32> class CItemPool
{
public:
	...
	size_type Allocate()
	{
		...
		DRX_ASSERT(index >= 0 && index < m_itemsArray.size());
...
private:
	...
	i32                    m_releasesBeforeSortingCounter = SORT_AFTER;
};

template<class TYPE, i32 SORT_AFTER = 32> class CItemPoolRefCount : public CItemPool<TYPE, SORT_AFTER>
{
public:
	...
	void Release(size_type index)
	{
		DRX_ASSERT(base::operator[](index).GetRefCount() == 0);
		base::Release(index);
	}
};
