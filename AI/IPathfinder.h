...
struct IAIPathAgent
{
	...
	virtual tukk                 GetPathAgentName() const = 0;
	...
	virtual bool                 IsPointValidForAgent(const Vec3& pos, u32 flags) const = 0;
	// </interfuscator:shuffle>
};
...
struct SNavPathParams
{
	SNavPathParams(const Vec3& start = Vec3Constants<float>::fVec3_Zero, const Vec3& end = Vec3Constants<float>::fVec3_Zero,
	               ...
	               i32 nForceBuildingID = -1, bool allowDangerousDestination = false, float endDistance = 0.0f,
...
public:
	...
	virtual void                                       Clear(tukk contextName) = 0;
	...
	virtual void      Dump(tukk name) const = 0;
...
protected:
	...
	{
#if !defined(_RELEASE)
		DRX_ASSERT_MESSAGE(BeingConstructedViaMakeSharedFunction(), "IMNMCustomPathCostComputer: you should have used MakeShared() to instantiate your class (since it injects the built-in custom deleter to guarantee safe cross-DLL deletion).");
...
struct MNMPathRequest
{
	...
	MNMPathRequest(const Vec3& start, const Vec3& end, const Vec3& _endDirection, i32 _forceTargetBuildingId,
