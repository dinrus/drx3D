...
#include <drx3D/Phys/AgePriorityQueue.h>
...
struct ProcessingContext
{
	...
	void Reset()
	{
		...
		status = Invalid;
		jobState = JobUpr::SJobState();
	}

	tukk GetStatusAsString() const
	{
		switch (status)
		{
		case Invalid:
...
public:
	...
	{
		DRX_ASSERT_MESSAGE(processingContextId >= 0 && processingContextId < m_pool.size(), "ProcessingContextsPool::GetContextAtPosition Trying to access an invalid element of the pool.");
	...
	void CancelPathRequest(MNM::QueuedPathID requestId)
	{
		...
		if (element != end)
		{
			gEnv->GetJobUpr()->WaitForJob(element->jobState);
	...
	void Reset()
	{
		i32k poolSize = gAIEnv.CVars.MNMPathfinderConcurrentRequests;
...
typedef DrxMT::vector<PathfindingFailedEvent>    PathfinderFailedEventQueue;
typedef DrxMT::vector<PathfindingCompletedEvent> PathfinderCompletedEventQueue;
}
}
