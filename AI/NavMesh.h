...
struct DangerAreaT : public DangerArea
{
	...
	DangerAreaT(const Vec3& _location, const float _effectRange, u8k _cost)
		: location(_location)
...
private:
	...
	u32k                        cost;          // Absolute cost associated with the Danger represented by the DangerAreaT
	const DangerWeightCalculation<WeightType> weightCalculator;
};

enum { max_danger_amount = 5, };

typedef DrxFixedArray<MNM::DangerAreaConstPtr, max_danger_amount> DangerousAreasList;
...
public:
	...
	struct WayQueryRequest
	{
		WayQueryRequest(EntityId _requesterEntityId, TriangleID _from, const vector3_t& _fromLocation, TriangleID _to,
		                const vector3_t& _toLocation, const OffMeshNavigation& _offMeshNavigation, const OffMeshNavigationUpr& _offMeshNavigationUpr,
			...
			, m_offMeshNavigationUpr(_offMeshNavigationUpr)
		...
		bool                            IsPointValidForAgent(const Vec3& pos, u32 flags) const;
	...
	private:
		...
		const OffMeshNavigationUpr& m_offMeshNavigationUpr;
	...
	void                      AddOffMeshLinkToTile(const TileID tileID, const TriangleID triangleID, u16k offMeshIndex);
	void                      UpdateOffMeshLinkForTile(const TileID tileID, const TriangleID triangleID, u16k offMeshIndex);
	...
	void ResetAccessibility(u8 accessible);
...
private:
	...
	void    PredictNextTriangleEntryPosition(const TriangleID bestNodeTriangleID, const vector3_t& startPosition, const TriangleID nextTriangleID, u32k vertexIndex, const vector3_t& finalLocation, vector3_t& outPosition) const;
	...
	bool FindNextIntersectingTriangleEdge(const vector3_t& rayStartPos, const vector3_t& rayEndPos, const vector2_t pVertices[3], real_t& rayIntersectionParam, u16& intersectingEdgeIndex) const;
	...
	template<typename TFilter>
	TriangleID StepOverEdgeToNeighbourTriangle(const vector3_t& rayStart, const vector3_t& rayEnd, const TileID currentTileID, const TriangleID currentTriangleID, u16k edgeIndex, const TFilter& filter) const;
...
protected:
	...
	inline Tile::STriangle& GetTriangleUnsafe(const TileID tileID, u16k triangleIdx) const
	{
		const TileContainer& container = m_tiles[tileID - 1];

		DRX_ASSERT(triangleIdx < container.tile.triangleCount);
	...
	typedef std::map<u32, u32> TileMap;
	TileMap             m_tileMap;
