...
/********************************************************************
   -------------------------------------------------------------------------
   Имя файла:   LeaderAction.h

   Описание: Header for CLeaderAction, CUnitImg, classes
...
public:
	...
	i32                         GetNumLiveUnits() const;
	inline u32               GetUnitProperties() const { return m_unitProperties; }
	...
	inline i32                  GetPriority() const                   { return m_Priority; };
...
private:
	...
	enum  { m_timeLimit = 20 };
	static const float  m_CSearchDistance;
	static u32k m_CCoverUnitProperties;
	i32                 m_iCoverUnitsLeft;
	Vec3                m_vEnemyPos;
	float               m_fSearchDistance;
	//SetObstacleRefs		m_Passed;
	TPointMap m_HideSpots;
	bool      m_bInitialized;
	i32       m_iSearchSpotAIObjectType;
...
protected:
	...
	public:
		...
		virtual void Serialize(TSerialize ser)
		{
			...
			{
				drx_sprintf(name, "ActorSpot_%d", i);
				ser.BeginGroup(name);
				{
					if (ser.IsWriting())
					{
	...
	void         GetBaseSearchPosition(CUnitImg& unit, CAIObject* pTarget, i32 method, float distance = 0);
...
protected:
	...
	CActorSpotListUpr m_ActorForbiddenSpotUpr;
};
