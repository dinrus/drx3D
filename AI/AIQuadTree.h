...
public:
	...
	/// incrementally...).
	void BuildQuadTree(i32 maxElementsPerCell, float minCellSize);
	...
	/// Dumps our contents
	void Dump(tukk debugName) const;
...
private:
	...
	struct CQuadTreeCell
	{
		...
		std::vector<i32> m_elementIndices;
	...
	struct CCellSorter
	{
		CCellSorter(const Vec3* dirPtr, const std::vector<CQuadTreeCell>* cellsPtr) : m_dirPtr(dirPtr), m_cellsPtr(cellsPtr) {}
	...
	void DumpCell(const CQuadTreeCell& cell, i32 indentLevel) const;
...
#include <drx3D/AI/AIQuadTree.inl>
