...
#include <drx3D/AI/AuditionMapItemPool.h>
...
class CAuditionMapRayCastUpr
{
	friend class SPendingStimulusParamsHelper;

public:
	CAuditionMapRayCastUpr(CAuditionMap& owner);
	~CAuditionMapRayCastUpr();
...
private:
	...
	bool                                    DetermineSoundObstruction(ISurfaceTypeUpr* pSurfaceTypeUpr, const SRayCastRequestInfo& rayCastRequestInfo, const ray_hit& rayHitInfo, float& resultSoundObstruction) const;
	inline static bool                      DeterminePhysicsSurfaceSoundObstruction(ISurfaceTypeUpr* pSurfaceTypeUpr, const short surfaceIndex, float& resultSoundObstruction);
...
{
public:
	SPendingStimulusParamsHelper(CAuditionMapRayCastUpr* pRaycastUpr, const SSoundStimulusParams& soundParams)
		: pRaycastUpr(pRaycastUpr)
	{
		stimulusParamsIndex = pRaycastUpr->PreparePendingStimulusParams(soundParams);
	}
	~SPendingStimulusParamsHelper()
	{
		pRaycastUpr->ReleasePendingStimulusParamsIfNotUsed(stimulusParamsIndex);
	}

	PendingStimulusParamsIndex GetStimulusIndex() const { return stimulusParamsIndex; }

private:
	CAuditionMapRayCastUpr* pRaycastUpr;
	PendingStimulusParamsIndex  stimulusParamsIndex;
};
