...
inline void BidirectionalMap<T0, T1 >::Erase(const T1& f)
{
	...
	assert(bwd_it != mBwd.end());
	const DataRecord* to_be_deleted = reinterpret_cast<const DataRecord*>((tuk)*bwd_it - offsetof(DataRecord, second));
...
BidirectionalMap<T0, T1 >::Find(const T1& f) const
{
	...
	if (it == mBwd.end())
		return mFwd.end();
	const DataRecord* value = reinterpret_cast<const std::pair<T0, T1>*>((tuk)*it - offsetof(DataRecord, second));
...
template<typename T0, typename T1>
inline i32 BidirectionalMap<T0, T1 >::Size() const
{
