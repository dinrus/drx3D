...
public:
	...
	/// incrementally...).
	void BuildOctree(i32 maxTrianglesPerCell, float minCellSize, const AABB& bbox);
	...
	bool IntersectLineseg(CIntersectionData* data, Lineseg lineseg, bool twoSided, i32 startingCell = 0) const;
	...
	i32 FindCellContainingAABB(const AABB& aabb) const;
...
private:
	...
	struct COctreeCell
	{
		...
		i32 m_childCellIndices[NUM_CHILDREN];

		/// indices of the triangles (if leaf)
		std::vector<i32> m_triangleIndices;
	...
	struct CCellSorter
	{
		...
		bool operator()(i32 cell1Index, i32 cell2Index) const
		{
	...
	mutable std::vector<i32> m_cellsToTest;

	/// cell indices cached from CacheCellsOverlappingAABB
	std::vector<i32> m_cachedCells;
	...
	mutable std::vector<i32> m_cachedCells2;
