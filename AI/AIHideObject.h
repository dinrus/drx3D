...
public:
	...
	i32          GetCoverUsage() const                       { return m_useCover; }
	void         SetCoverUsage(i32 type)                     { m_useCover = type; }

	// Returns unique ID which is updated each time the Set method succeeds. 0 is invalid ID.
	u32 GetCoverId() const { return m_id; }

	// Returns the name of the anchor being used
	tukk GetAnchorName() const;
