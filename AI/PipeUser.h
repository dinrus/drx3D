...
#include <drx3D/AI/AIHideObject.h>
#include <drx3D/AI/ObjectContainer.h>
#include <drx3D/AI/CoverUser.h>
#include <drx3D/AI/MNMPathfinder.h>
#include <drx3D/AI/IMovementSystem.h>
#include <drx3D/AI/PipeUserMovementActorAdapter.h>
...
public:
	...
	virtual void             Event(u16 eType, SAIEVENT* pAIEvent) override;
	...
	virtual void             SetName(tukk pName) override;
	void                     GetStateFromActiveGoals(SOBJECTSTATE& state);
	CGoalPipe*               GetGoalPipe(tukk name);
	void                     RemoveActiveGoal(i32 nOrder);
	...
	virtual void                 RequestPathTo(const Vec3& pos, const Vec3& dir, bool allowDangerousDestination, i32 forceTargetBuildingId = -1, float endTol = std::numeric_limits<float>::max(),
	...
	/// inherited
	virtual void                 SetPathToFollow(tukk pathName) override;
	virtual void                 SetPathAttributeToFollow(bool bSpline) override;

	tukk                  GetPathToFollow() const { return m_pathToFollowName.c_str(); }
	...
	tukk                  GetActiveGoalPipeName() const         { const CGoalPipe* pipe = GetActiveGoalPipe(); return pipe ? pipe->GetName() : "No Active GoalPipe"; }
	void                         ResetCurrentPipe(bool resetAlways) override;
	i32                          GetGoalPipeId() const override;
	...
	void               ClearMovementContext(u32 movementContext);
	void               SetMovementContext(u32 movementContext);
	u32k GetMovementContext() const                                                   { return m_movementContext; }
	...
	// by someone else so the requester can't clear the loose att target	i32 SetLooseAttentionTarget(CAIObject* pObject=NULL,i32 id=0);
	i32          SetLooseAttentionTarget(CWeakRef<CAIObject> refObject, i32 id = -1);
	...
	inline i32 SetLooseAttentionTarget(const Vec3& pos)
	{
	...
	inline i32 GetLooseAttentionId()
	{
		return m_looseAttentionId;
	}
	...
	void                  RegisterAttack(tukk name);
	void                  RegisterRetreat(tukk name);
	void                  RegisterWander(tukk name);
	void                  RegisterIdle(tukk name);
	bool                  SelectPipe(i32 id, tukk name, CWeakRef<CAIObject> refArgument, i32 goalPipeId = 0, bool resetAlways = false, const GoalParams* node = 0) override; //const XmlNodeRef *node = 0);
	IGoalPipe*            InsertSubPipe(i32 id, tukk name, CWeakRef<CAIObject> refArgument, i32 goalPipeId = 0, const GoalParams* node = 0) override;                        //const XmlNodeRef *node = 0);
	bool                  CancelSubPipe(i32 goalPipeId) override;
	bool                  RemoveSubPipe(i32 goalPipeId, bool keepInserted = false) override;
	bool                  IsUsingPipe(tukk name) const override;
	bool                  IsUsingPipe(i32 goalPipeId) const override;
	bool                  AbortActionPipe(i32 goalPipeId);
#ifdef USE_DEPRECATED_AI_CHARACTER_SYSTEM
	bool                  SetCharacter(tukk character, tukk behaviour = NULL);
#endif
	...
	virtual void        SetRefShapeName(tukk shapeName) override;
	virtual tukk GetRefShapeName() const override;
	...
	virtual void                 ClearPath(tukk dbgString) override;
	...
	virtual CAIObject*             GetSpecialAIObject(tukk objName, float range = 0.0f) override;
	...
	virtual void              OnAIHandlerSentSignal(tukk szText, u32 crcCode) override;
...
#ifdef _DEBUG
	i32 m_DEBUGmovementReason;
#endif

	virtual void DebugDrawGoals();

	// goal pipe notifications - used by action graph nodes
	typedef std::multimap<i32, std::pair<IGoalPipeListener*, tukk >> TMapGoalPipeListeners;
	TMapGoalPipeListeners m_mapGoalPipeListeners;
	void         NotifyListeners(i32 goalPipeId, EGoalPipeEvent event);
	void         NotifyListeners(CGoalPipe* pPipe, EGoalPipeEvent event, bool includeSubPipes = false);
	virtual void RegisterGoalPipeListener(IGoalPipeListener* pListener, i32 goalPipeId, tukk debugClassName) override;
	virtual void UnRegisterGoalPipeListener(IGoalPipeListener* pListener, i32 goalPipeId) override;

	i32          CountGroupedActiveGoals(); //
...
protected:
	...
	VectorSet<i32>             m_notAllowedSubpipes; // sub-pipes that we tried to remove or cancel even before inserting them
	bool                       m_bFirstUpdate;
...
private:
	...
	struct DelayedPipeSelection
	{
		...
		DelayedPipeSelection(i32 _mode, tukk _name, CWeakRef<CAIObject> _refArgument,
		                     i32 _goalPipeId, bool _resetAlways /*, const GoalParams* _params*/)
			: mode(_mode)
			, name(_name)
