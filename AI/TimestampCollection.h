...
public:
	...
	void HandleEvent(u32 eventNameCRC32)
	{
	...
	void LoadFromXml(const XmlNodeRef& xml)
	{
		for (i32 i = 0; i < xml->getChildCount(); ++i)
		{
			...
			tukk setOnEvent = child->getAttr("setOnEvent");
			tukk exclusiveTo = child->getAttr("exclusiveTo");
