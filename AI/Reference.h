...
// -------------------------------------------------------------------------
//  Имя файла:   Reference.h
//  Created:     11/02/2008 by Matthew
//  Описание: References to AI objects
// -------------------------------------------------------------------------
//  История:
//
//  Notes:
 ...
 * StrongRef -    Wraps an ID with templated type and unique_ptr semantics. Uniquely owns the object, controlling its validity. Within AI.
...
/**
 * Typed strong reference for defining ownership, with unique_ptr semantics.
...
public:
	...
	void       Serialize(TSerialize ser, tukk sName = NULL);
...
protected:
	// Allow just CObjectUpr to create in this way and we trust it to have ensured the type is appropriate
...
template<class T>
void SerialisationHack(TSerialize ser, tukk sName, T** pObj);
...
protected:
	...
	struct SRefCounter
	{
		...
		i32           m_nRefs;        // Since this isn't in the object itself, it needn't be mutable
	};
