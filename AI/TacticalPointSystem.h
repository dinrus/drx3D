...
/********************************************************************
   ---------------------------------------------------------------------
   Имя файла:   TacticalPointSystem.h
   $Id$
   $DateTime$
   Описание: System for Tactical Point queries
...
public:
	...
	virtual TPSQueryID  CreateQueryID(tukk psName) override;
	...
	virtual tukk GetQueryName(TPSQueryID queryID) override;
	// Get the query ID of a query by name
	virtual TPSQueryID  GetQueryID(tukk psName) override;
	// Returns a pointer to indexed option of a given query
	virtual tukk GetOptionLabel(TPSQueryID queryID, i32 option) override;

	// Build up a query
	// The "option" parameter allows you to build up fallback options
	virtual bool AddToParameters(TPSQueryID queryID, tukk sSpec, float fValue, i32 option = 0) override;
	virtual bool AddToParameters(TPSQueryID queryID, tukk sSpec, bool bValue, i32 option = 0) override;
	virtual bool AddToParameters(TPSQueryID queryID, tukk sSpec, tukk sValue, i32 option = 0) override;
	virtual bool AddToGeneration(TPSQueryID queryID, tukk sSpec, float fValue, i32 option = 0) override;
	virtual bool AddToGeneration(TPSQueryID queryID, tukk sSpec, tukk sValue, i32 option = 0) override;
	virtual bool AddToConditions(TPSQueryID queryID, tukk sSpec, float fValue, i32 option = 0) override;
	virtual bool AddToConditions(TPSQueryID queryID, tukk sSpec, bool bValue, i32 option = 0) override;
	virtual bool AddToWeights(TPSQueryID queryID, tukk sSpec, float fValue, i32 option = 0) override;

	// Start a new asynchronous query. Returns the id "ticket" for this query instance.
	// Types needed to avoid confusion?
	virtual TPSQueryTicket AsyncQuery(TPSQueryID queryID, const QueryContext& m_context, i32 flags, i32 nPoints, ITacticalPointResultsReceiver* pReciever) override;
	...
	i32 TestConditions(TPSQueryID queryID, const QueryContext& context, Vec3& point, bool& bValid) const;

	// Make a synchronous query for one point
	// Returns: query option used, or -1 if there was an error or no points found
	// Has no side-effects
	i32 SyncQuery(TPSQueryID queryID, const QueryContext& context, CTacticalPoint& point);

	// Synchronous query for up to n best points in order of decreasing fitness
	// Returns: query option used, or -1 if there was an error or no points found
	// No side-effects
	i32 SyncQueryShortlist(TPSQueryID queryID, const QueryContext& context, TTacticalPoints& points, i32 n);
	...
	virtual bool ExtendQueryLanguage(tukk szName, ETacticalPointQueryType eType, ETacticalPointQueryCost eCost) override;
	...
	virtual IAIObject* CreateExtenderDummyObject(tukk szDummyName) override;
...
private:
	...
	struct SPointEvaluation
	{
		...
		i32            m_iQueryIndex;                          // The index of the next criterion to be evaluated on this point
	...
	struct SQueryEvaluation
	{
		...
		i32 iCurrentQueryOption;                               // Whether we are currently evaluating the first option, the first fallback, the next, etc.
		...
		PostureUpr::PostureQueryID                      postureQueryID;
		...
		i32                                                 nFoundBestN;   // Number of best points found so far
		...
		void operator=(const SQueryEvaluation& that)
		{
			...
			{
				DrxFatalError("Can't make a copy of a deferredExtenderCancelFunc - it will lead to a double call - this message should never happen.");
			}
		}
	...
	bool ApplyCost(u32 uQueryId, ETacticalPointQueryCost eCost);
	i32  GetCost(u32 uQueryId) const;
	...
	// -----------Moves structure to {eHeapEvaluation, eCompletedOption, eCompleted}
	bool SetupHeapEvaluation(const std::vector<CCriterion>& vConditions, const std::vector<CCriterion>& vWeights, const QueryContext& context, const std::vector<CTacticalPoint>& vPoints, i32 n, SQueryEvaluation& evaluation) const;
	...
	bool        Parse(tukk sSpec, TTacticalPointQuery& query, TTacticalPointQuery& limits,
	...
	TTacticalPointQuery Translate(tukk sWord) const;
	// Convert a single TPS token into a single word - or NULL if unrecognised
	tukk         Translate(TTacticalPointQuery etpToken) const;

	// Convert a single word into a single TPS parameter token - or eTPQP_None if unrecognised
	ETacticalPointQueryParameter TranslateParam(tukk sWord) const;
	ETPSRelativeValueSource      TranslateRelativeValueSource(tukk sWord) const;

	// bit of duplicaiton here...?
	const CTacticalPointQuery* GetQuery(TPSQueryID nQueryID) const;
	COptionCriteria*           GetQueryOptionPointer(TPSQueryID queryID, i32 options);

	// Generate a warning message, giving as many standard details of the context as possible
	void TPSDescriptionWarning(const CTacticalPointQuery& query, const QueryContext& context, const COptionCriteria* pOption, tukk sMessage = NULL) const;
	...
	std::map<TTacticalPointQuery, i32>             m_mIDToCost; // Map of strings to query costs
	i32 m_CostMap[eTPQC_COUNT];
	i32 m_GameQueryIdMap[eTPQT_COUNT];
	...
	struct CVarsDef
	{
		i32   DebugTacticalPoints;
		i32   DebugTacticalPointsBlocked;
		i32   TacticalPointsDebugDrawMode;
		i32   TacticalPointsDebugFadeMode;
		float TacticalPointsDebugScaling;
		float TacticalPointsDebugTime;
		i32   TacticalPointsWarnings;
	};
