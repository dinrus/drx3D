...
#include <drx3D/AI/IMNM.h>
#include <drx3D/AI/MNMNavMesh.h>
#include <drx3D/AI/NavigationIdTypes.h>
#include <functional>

struct IOffMeshNavigationUpr;
struct INavigationUpdatesUpr;
...
typedef Functor3<NavigationAgentTypeID, NavigationMeshID, u32> NavigationMeshChangeCallback;
typedef Functor2wRet<IPhysicalEntity&, u32&, bool>             NavigationMeshEntityCallback;
typedef u32                                                    NavigationAgentTypesMask;
...
struct INavigationSystem
{
	...
	{
		CreateAgentTypeParams(const Vec3& _voxelSize = Vec3(0.1f), u16 _radiusVoxelCount = 4,
		                      u16 _climbableVoxelCount = 4, u16 _heightVoxelCount = 18,
		                      u16 _maxWaterDepthVoxelCount = 6)
		...
		u16 radiusVoxelCount;
		u16 climbableVoxelCount;
		u16 heightVoxelCount;
		u16 maxWaterDepthVoxelCount;
	};

	struct CreateMeshParams
	{
		CreateMeshParams(const Vec3& _origin = Vec3(ZERO), const Vec3i& _tileSize = Vec3i(8), u32k _tileCount = 1024)
			: origin(_origin)
			, tileSize(_tileSize)
	...
	virtual NavigationAgentTypeID           CreateAgentType(tukk name, const CreateAgentTypeParams& params) = 0;
	virtual NavigationAgentTypeID           GetAgentTypeID(tukk name) const = 0;
	virtual NavigationAgentTypeID           GetAgentTypeID(size_t index) const = 0;
	virtual tukk                     GetAgentTypeName(NavigationAgentTypeID agentTypeID) const = 0;
...
#ifdef SW_NAVMESH_USE_GUID
	virtual NavigationMeshID CreateMesh(tukk name, NavigationAgentTypeID agentTypeID, const CreateMeshParams& params, NavigationMeshGUID guid) = 0;
	virtual NavigationMeshID CreateMesh(tukk name, NavigationAgentTypeID agentTypeID, const CreateMeshParams& params, NavigationMeshID requestedID, NavigationMeshGUID guid) = 0;
#else
	virtual NavigationMeshID CreateMesh(tukk name, NavigationAgentTypeID agentTypeID, const CreateMeshParams& params) = 0;
	virtual NavigationMeshID CreateMesh(tukk name, NavigationAgentTypeID agentTypeID, const CreateMeshParams& params, NavigationMeshID requestedID) = 0;
#endif
	virtual NavigationMeshID CreateMeshForVolumeAndUpdate(tukk name, NavigationAgentTypeID agentTypeID, const CreateMeshParams& params, const NavigationVolumeID volumeID) = 0;
	...
	virtual NavigationMeshID GetMeshID(tukk name, NavigationAgentTypeID agentTypeID) const = 0;
	virtual tukk  GetMeshName(NavigationMeshID meshID) const = 0;
	virtual void         SetMeshName(NavigationMeshID meshID, tukk name) = 0;
	...
	virtual u32       GetWorkingQueueSize() const = 0;
	...
	virtual INavigationUpdatesUpr* GetUpdateUpr() = 0;
	...
	virtual bool                       ReadFromFile(tukk fileName, bool bAfterExporting) = 0;
	virtual bool                       SaveToFile(tukk fileName) const = 0;

	virtual void                       RegisterListener(INavigationSystemListener* pListener, tukk name = NULL) = 0;
	virtual void                       UnRegisterListener(INavigationSystemListener* pListener) = 0;

	virtual void                       RegisterUser(INavigationSystemUser* pExtension, tukk name = NULL) = 0;
	...
	virtual bool               RegisterArea(tukk shapeName, NavigationVolumeID& outVolumeId) = 0;
	virtual void               UnRegisterArea(tukk shapeName) = 0;
	virtual NavigationVolumeID GetAreaId(tukk shapeName) const = 0;
	virtual void               SetAreaId(tukk shapeName, NavigationVolumeID id) = 0;
	virtual void               UpdateAreaNameForId(const NavigationVolumeID id, tukk newShapeName) = 0;
	...
	virtual bool               RegisterEntityMarkups(const IEntity& owningEntity, tukk* shapeNamesArray, const size_t count, NavigationVolumeID* pOutIdsArray) = 0;
	...
	virtual void               GetTileBoundsForMesh(NavigationMeshID meshID, u32 tileID, AABB& bounds) const = 0;
	...
	virtual const IOffMeshNavigationUpr& GetIOffMeshNavigationUpr() const = 0;
	virtual IOffMeshNavigationUpr&       GetIOffMeshNavigationUpr() = 0;
	...
	virtual size_t GetTriangleInfo(const NavigationMeshID meshID, const AABB& aabb, Vec3* centerLocations, u32* islandids, size_t max_count, const INavMeshQueryFilter* pFilter, float minIslandArea = 0.f) const = 0;
