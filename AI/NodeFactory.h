...
public:
	...
	virtual INodePtr CreateNodeOfType(tukk typeName) override
	{
		INodePtr node;
	...
	virtual uk AllocateRuntimeDataMemory(const size_t size) override
	{
		return s_bucketAllocator.allocate(size);
	}

	virtual void FreeRuntimeDataMemory(uk pointer) override
	{
	...
	virtual uk AllocateNodeMemory(const size_t size) override
	{
		return new char[size];
	}

	//! This will be called while unloading a level or exiting game in the editor.
	virtual void FreeNodeMemory(uk pointer) override
	{
		delete[] (tuk)pointer;
	}
...
#else
	typedef node_alloc<eDrxDefaultMalloc, true, 2*1024*1024>                                                                       BehaviorTreeBucketAllocator;
#endif
