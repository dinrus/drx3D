...
typedef u32 TBubbleRequestOptionFlags;
...
struct SAIBubbleRequest
{
	...
	SAIBubbleRequest(const EntityId _entityID, tukk _message, TBubbleRequestOptionFlags _flags, float _duration = 0.0f, const ERequestType _requestType = eRT_ErrorMessage)
		: entityID(_entityID)
		, message(_message)
		, flags(_flags)
		, duration(_duration)
...
private:
	...
	EntityId                                   entityID;
	DrxFixedStringT<MAX_BUBBLE_MESSAGE_LENGHT> message;
...
struct SBubbleRequestId
{
	...
	explicit SBubbleRequestId(u32k _id) : id(_id) {}
...
struct IAIBubblesSystem
{
	 ...
	 */
	virtual SBubbleRequestId QueueMessage(tukk messageName, const SAIBubbleRequest& request) = 0;
	...
	*/
	virtual void SetNameFilter(tukk szMessageNameFilter) = 0;
};
