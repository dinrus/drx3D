...
#include <drx3D/AI/AgentParams.h>
...
#include <limits>
#include <drx3D/AI/ICommunicationUpr.h> // <> required for Interfuscator
#include <drx3D/AI/IAIMannequin.h>
...
#define AIOBJECT_HELICOPTERDRXSIS2 41
...
inline tukk GetStanceName(i32 stance)
{
	switch (stance)
	{
	case STANCE_NULL:
...
//! LookStyle essentially define the transition style when look target is set.
...
enum EFireMode
{
	...
	FIREMODE_BURST_WHILE_MOVING = 9, //!< (To be renamed.
...
enum EUnitProperties
{
	...
	UPR_ALL           = -1,      //!< The unit can take part in all actions.
};
...
//! making a pathfind request, it is suggested to set up bunch of constant sets
...
struct AgentPathfindingProperties
{
	...
	//! The type of nodes this agent can traverse.
	...
	float  zScale;
	u32 customNavCapsMask;

	float  radius;
	float  height;
	float  maxSlope;

	// Unique id of this instance among others. If there were an array
	// of these structs this could have been an index into that array.
	i32  id;
	bool avoidObstacles;
};
...
struct AgentMovementSpeeds
{
	...
	{
		for (i32 s = 0; s < AMS_NUM_VALUES; ++s)
		{
	...
	void SetRanges(i32 stance, i32 urgency, float sdef, float smin, float smax)
	{
	...
	void CopyRanges(i32 stance, i32 toUrgency, i32 fromUrgency)
	{
		assert(stance >= 0 && stance < AMS_NUM_VALUES);
	...
	const SSpeedRange& GetRange(i32 stance, i32 urgency) const
	{
		assert(stance >= 0 && stance < AMS_NUM_VALUES);
		assert(urgency >= 0 && urgency < AMU_NUM_VALUES);
	...
	//! Non-const version needed for serialisation.
	SSpeedRange& GetRange(i32 stance, i32 urgency)
	{
		assert(stance >= 0 && stance < AMS_NUM_VALUES);
		assert(urgency >= 0 && urgency < AMU_NUM_VALUES);
		return speedRanges[stance][urgency];
...
struct AgentMovementAbility
{
	...
	//! \note If disabled, other agents don't avoid it either!.
	...
	//! itself and other agents/obstacles.
...
struct AIObjectParams
{
	...
	AIObjectParams(u16 _type, const AgentParameters& sParamStruct, const AgentMovementAbility& moveAbility)
	...
	AIObjectParams(u16 _type, IAIObject* _association = 0, EntityId _entityID = 0)
		: type(_type)
...
struct AIWeaponDescriptor
{
	...
	float  fRangeMin;
	bool   bSignalOnShoot;
	i32    burstBulletCountMin;   //!< The burst bullet count is scaled between min and max based on several factors (distance, visibility).
	i32    burstBulletCountMax;   //!< Each burst is followed by a pause.
...
struct IFireCommandHandler
{
	...
	virtual i32 GetShotCount() const = 0;
...
typedef u32 TAnimationGraphQueryID;
...
struct IPipeUser
{
	...
	// <interfuscator:shuffle>
	virtual ~IPipeUser() {}
	virtual bool       SelectPipe(i32 id, tukk name, IAIObject* pArgument = 0, i32 goalPipeId = 0, bool resetAlways = false, const GoalParams* node = 0) = 0;
	virtual IGoalPipe* InsertSubPipe(i32 id, tukk name, IAIObject* pArgument = 0, i32 goalPipeId = 0, const GoalParams* node = 0) = 0;
	virtual bool       CancelSubPipe(i32 goalPipeId) = 0;
	virtual bool       RemoveSubPipe(i32 goalPipeId, bool keepInserted = false) = 0;
	virtual bool       IsUsingPipe(tukk name) const = 0;
	virtual bool       IsUsingPipe(i32 goalPipeId) const = 0;
	...
	virtual IAIObject* GetSpecialAIObject(tukk objName, float range = 0.0f) = 0;
	...
	virtual void        RegisterGoalPipeListener(IGoalPipeListener* pListener, i32 goalPipeId, tukk debugClassName) = 0;
	virtual void        UnRegisterGoalPipeListener(IGoalPipeListener* pListener, i32 goalPipeId) = 0;
	virtual i32         GetGoalPipeId() const = 0;
	...
	virtual void        SetRefShapeName(tukk shapeName) = 0;
	virtual tukk GetRefShapeName() const = 0;
	...
	virtual void        ClearPath(tukk dbgString) = 0;
...
struct IGoalPipeListener
{
	virtual void OnGoalPipeEvent(IPipeUser* pPipeUser, EGoalPipeEvent event, i32 goalPipeId, bool& unregisterListenerAfterEvent) = 0;

private:
	friend class CPipeUser;

	typedef DynArray<std::pair<IPipeUser*, i32>> VectorRegisteredPipes;
...
struct ISmartObjectUpr
{
	...
	// <interfuscator:shuffle>
	virtual ~ISmartObjectUpr(){}
	virtual void        RemoveSmartObject(IEntity* pEntity) = 0;

	virtual tukk GetSmartObjectStateName(i32 id) const = 0;

	virtual void        RegisterSmartObjectState(tukk sStateName) = 0;
	virtual void        AddSmartObjectState(IEntity* pEntity, tukk sState) = 0;
	virtual void        RemoveSmartObjectState(IEntity* pEntity, tukk sState) = 0;
	virtual bool        CheckSmartObjectStates(const IEntity* pEntity, tukk patternStates) const = 0;
	virtual void        SetSmartObjectState(IEntity* pEntity, tukk sStateName) = 0;
	virtual void        ModifySmartObjectStates(IEntity* pEntity, tukk listStates) = 0;
	...
	virtual u32             GetSOClassTemplateIStatObj(IEntity* pEntity, IStatObjPtr* ppStatObjects, u32& numAllocStatObjects) = 0; // TODO make this const at some point if possible.

	virtual void               ReloadSmartObjectRules() = 0;

	virtual i32                SmartObjectEvent(tukk sEventName, IEntity*& pUser, IEntity*& pObject, const Vec3* pExtraPoint = NULL, bool bHighPriority = false) = 0;

	virtual SmartObjectHelper* GetSmartObjectHelper(tukk className, tukk helperName) const = 0;
	// </interfuscator:shuffle>
};

struct IAIAction;

struct IAIActionUpr : public IGoalPipeListener
{
	// <interfuscator:shuffle>
	//! \return An existing AI Action from the library or NULL if not found.
	virtual IAIAction* GetAIAction(tukk sName) = 0;

	//! \return An existing AI Action by its index in the library or NULL index is out of range.
	...
	virtual void AbortAIAction(IEntity* pEntity, i32 goalPipeId = 0) = 0;

	//! Finishes specific AI Action (specified by goalPipeId) for the pEntity as a user.
	virtual void FinishAIAction(IEntity* pEntity, i32 goalPipeId) = 0;
	// </interfuscator:shuffle>
};
...
struct IPuppet
{
	...
	//! \return true and sends signal "OnFriendInWay" if there is a friendly agent in the line of fire.
	...
	//! \return true if puppet has a perceived location for this target.
...
struct IAISignalExtraData
{
	...
	virtual tukk GetObjectName() const = 0;
	virtual void        SetObjectName(tukk objectName) = 0;
...
struct AISIGNAL
{
	...
	static i32k    SIGNAL_NAME_LENGTH = 50;
	...
	inline bool Compare(u32 crc) const
	{
...
struct SAICoverRequest
{
	...
	void SetCoverAction(tukk const name, const float postureLean)
	{
	...
	{
		DRX_ASSERT(position.IsValid());
		DRX_ASSERT(forwardDirection.IsValid());
		DRX_ASSERT(forwardDirection.IsUnit());
...
struct SOBJECTSTATE
{
	...
	i32                   weaponAccessories; //!< Weapon accessories to enable.
	i32                   bodystate;
	float                 lean;
	float                 peekOver;

	//! The requested position to shoot at. This value must be passed directly to weapon system.
	...
	//! The requested position to aim at.
	//! This value is slightly different from the vShootTarget, and is used for
	//! visual representation to where to shoot at. Notes: can be (0,0,0) if not begin requested!
	Vec3 vAimTargetPos;
	...
	//! Temporary flag to avoid the body turn while doing specific operation.
	...
	//! Flags that give context to movement, which can be used by animation selection system.
...
{
	SCommunicationSound()
		: playSoundControlId(DrxAudio::InvalidControlId)
		, stopSoundControlId(DrxAudio::InvalidControlId)
	{
	}

	SCommunicationSound(const DrxAudio::ControlId _playSoundControlId, const DrxAudio::ControlId _stopSoundControlId)
		: playSoundControlId(_playSoundControlId)
		, stopSoundControlId(_stopSoundControlId)
	{
	}

	DrxAudio::ControlId playSoundControlId;
	DrxAudio::ControlId stopSoundControlId;
};
...
struct IAICommunicationHandler
{
	...
	virtual SCommunicationSound PlaySound(CommPlayID playID, tukk name, IEventListener* listener = 0) = 0;
	virtual void                StopSound(const SCommunicationSound& soundToStop) = 0;

	virtual SCommunicationSound PlayVoice(CommPlayID playID, tukk name, IEventListener* listener = 0) = 0;
	virtual void                StopVoice(const SCommunicationSound& soundToStop) = 0;

	virtual void                SendDialogueRequest(CommPlayID playID, tukk name, IEventListener* listener = 0) = 0;

	virtual void                PlayAnimation(CommPlayID playID, tukk name, EAnimationMethod method, IEventListener* listener = 0) = 0;
	virtual void                StopAnimation(CommPlayID playID, tukk name, EAnimationMethod method) = 0;

	virtual bool                IsInAGState(tukk name) = 0;
	virtual void                ResetAnimationState() = 0;

	virtual bool                IsPlayingAnimation() const = 0;
	virtual bool                IsPlayingSound() const = 0;

	virtual void                OnSoundTriggerFinishedToPlay(DrxAudio::ControlId const triggerId) = 0;
	// </interfuscator:shuffle>
};
