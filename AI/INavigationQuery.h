...
struct SNavMeshQueryFilterDefaultBase : public INavMeshQueryFilter
{
	...
	{
		DRX_ASSERT(triangle.areaAnnotation.GetType() < MaxAreasCount);
	...
	{
		DRX_ASSERT(currentTriangle.areaAnnotation.GetType() < MaxAreasCount);
	...
	{
		DRX_ASSERT(areaIdx >= 0 && areaIdx < MaxAreasCount);
		costs[areaIdx] = cost;
	}
