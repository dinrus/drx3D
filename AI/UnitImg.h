...
/********************************************************************
   ...
   Описание: Header for CUnitImg class
...
public:
	...
	void        TaskExecuted();
	void        ExecuteTask();
	void        ClearPlanning(i32 priority = 0);
	...
	inline void         SetProperties(u32 prop) { m_properties = prop; };
	inline u32 GetProperties() const            { return m_properties; };
	const CUnitAction*  GetCurrentAction() const         { return m_pCurrentAction; };
	inline i32          GetClass() const                 { return m_SoldierClass; };
	inline void         SetClass(i32 soldierclass)       { m_SoldierClass = soldierclass; };
	...
	Vec3               m_TagPoint;
	u16             m_flags;
	i32                m_Group;   // used to split units in different groups
	i32                m_FormationPointIndex;
	i32                m_SoldierClass;
	float              m_fDistance;
	float              m_fDistance2;
	CTimeValue         m_lastReinforcementTime;

	u32             m_properties; // binary mask to set properties (UPR_*)
