...
template<class B>
void CPolymorphicQueue<B >::GetMemoryStatistics(IDrxSizer* pSizer, bool countingThis /*= false*/)
{
...
template<class B>
uk CPolymorphicQueue<B >::Grab(size_t sz, uk retaddr)
{
	POLY_HEADER;
	if (sz & (WORD_SIZE - 1))
		sz += WORD_SIZE;
...
#if CHECKING_POLYMORPHIC_QUEUE
	m_lastAdder = (uk ) m_flushBlocks[m_flushBlock]->data[m_flushCursor++];
#endif
	uk out = &(m_flushBlocks[m_flushBlock]->data[m_flushCursor]);
	m_flushCursor += sz;
	POLY_FOOTER;
	return (B*)out;
}
