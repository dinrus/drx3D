...
static u32k LINKEDELEM_TERMINATOR = 0x3fffffff; // top two bits are free to pack history counts
...
struct SContextViewObjectEx
{
	...
	{
		for (i32 i = 0; i < eHSS_NUM_SLOTS; i++) // just to avoid an assert below...
	...
	void                Reset()
	{
		...
		for (NetworkAspectID i = 0; i < NumAspects; i++)
		{
			...
			ackedAspectVersions[i] = ~u32(0);
		}
		for (i32 i = 0; i < eHSS_NUM_SLOTS; i++)
		{
