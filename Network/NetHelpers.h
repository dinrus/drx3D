...
#include <drx3D/Network/INetwork.h> // <> required for Interfuscator
...
protected:
	...
	static SNetMessageDef* Helper_AddMessage(SNetMessageDef::HandlerType handler,
	                                         ENetReliabilityType reliability, tukk description, u32 parallelFlags, u32 nUser = 0xBA5E1E55)
	{
		...
		if (ms_statics.m_nMessages == MAX_STATIC_MESSAGES)
		{
			...
			DrxFatalError("Maximum static messages exceeded");
			((void (*)())NULL)();
		}
	...
	static SNetMessageDef* Helper_AddAspectMessage(
		SNetMessageDef::HandlerType handler, tukk szDescription, 
		i32k aspectNum, u32 parallelFlags = 0)
	{
		return Helper_AddMessage(handler, eNRT_UnreliableUnordered,
			// Leak is intentional, but constant.
			(new string(szDescription + DrxStringUtils::toString(aspectNum)))->c_str(),
			parallelFlags);
	}
...
//    (TSerialize pSer, u32 nCurSeq, u32 nOldSeq)
...
#define NET_DECLARE_IMMEDIATE_MESSAGE(name)                                                       \
  private:                                                                                        \
    static TNetMessageCallbackResult Trampoline ## name(u32,                                   \
                                                        INetMessageSink*, TSerialize,             \
                                                        u32, u32, u32, EntityId*, INetChannel*); \
    bool Handle ## name(TSerialize, u32, u32, u32, EntityId*, INetChannel*);                     \
  public:                                                                                         \
...
#define NET_IMPLEMENT_IMMEDIATE_MESSAGE(cls, name, reliability, parallelFlags)                                                          \
  ...
  inline bool cls::Handle ## name(TSerialize ser,                                                                                       \
                                  u32 nCurSeq, u32 nOldSeq, u32 timeFraction32, EntityId * pEntityId, INetChannel * pNetChannel)
...
#define NET_IMPLEMENT_ATSYNC_MESSAGE(cls, name, reliability, parallelFlags)                                           \
  ...
  inline bool cls::Handle ## name(TSerialize ser,                                                                     \
                                  u32 nCurSeq, u32 nOldSeq, u32 timeFraction32, EntityId entityId, INetChannel * pChannel)
...
public:
	...
	tukk                 GetName() { return m_name.c_str(); }
...
template<class T>
void AddSetValue(IContextEstablisher* pEst, EContextViewState state, T* pWhere, T what, tukk name)
{
...
template<class T>
void AddStoreValueThenChangeTo(IContextEstablisher* pEst, EContextViewState state, T init, T what, tukk name)
{
...
public:
	...
	EContextEstablishTaskResult OnStep(SContextEstablishState&)
	{
		...
		{
			DrxLog("Waiting for value %s", m_name.c_str());
			m_lastNotification = now;
		}
...
template<class T>
void AddWaitValue(IContextEstablisher* pEst, EContextViewState state, T* pWhere, T what, tukk name, CTimeValue notificationInterval)
{
...
template<class T>
void AddAddValue(IContextEstablisher* pEst, EContextViewState state, T* pWhere, T what, tukk name)
{
...
template<class T>
void AddCallMemberFunction(IContextEstablisher* pEst, EContextViewState state, T* pObj, void (T::* memberFunc)(), tukk name)
{
