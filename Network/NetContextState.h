...
#include <drx3D/Network/INetContextState.h>
...
public:
	CNetContextState(CNetContext*, i32 token, CNetContextState* pPrev);
	~CNetContextState();
	...
	public:
		...
		CNetObjectBindLock(CNetContextState* pContext, SNetObjectID objId, tukk msg) : m_pContext(pContext), m_objId(objId), m_msg(msg)
		{
			assert((bool)m_objId);
	...
	bool LockObject(SNetObjectID id, i32 /*EContextObjectLock*/ lock)
	{
		NET_ASSERT(GetContextObject(id).main);
		u8 old = (u8)(m_vObjects[id.id].vLocks[lock]++);
		NET_ASSERT(m_vObjects[id.id].vLocks[lock]);
		return old == 0;
	}
	bool UnlockObject(SNetObjectID id, i32 /*EContextObjectLock*/ lock)
	{
		NET_ASSERT(GetContextObject(id).main);
		NET_ASSERT(m_vObjects[id.id].vLocks[lock]);
		return --m_vObjects[id.id].vLocks[lock] == 0;
	}
	bool IsLocked(SNetObjectID id, i32 /*EContextObjectLock*/ lock)
	{
		NET_ASSERT(GetContextObject(id).main);
		return m_vObjects[id.id].vLocks[lock] != 0;
	}
	CNetObjectBindLock LockObject(SNetObjectID, tukk why);
	...
	bool                   DoSetAspectProfile(EntityId id, NetworkAspectType aspectBit, u8 profile, bool checkOwnership, bool informedGame, bool unlockUpdate);
	...
	INetContextListenerPtr FindListener(tukk name);
	...
	bool                   UnbindObject(SNetObjectID id, u32 flags);
	...
	CMementoMemoryUpr& GetStateMMM() { return *m_pMMM; }
	...
	void          LogRMI(tukk function, ISerializable* pParams);
	void          LogCppRMI(EntityId id, IRMICppLogger* pLogger);
	void          DeclareAspect(tukk name, NetworkAspectType aspectBit, u8 aspectFlags);
	...
	EntityId      AddReservedUnboundEntityMapEntry(u16 partialNetID, EntityId eID);
	EntityId      RemoveReservedUnboundEntityMapEntry(u16 partialNetID);
	EntityId      GetReservedUnboundEntityMapEntry(u16 partialNetID);
#endif
	void          RemoveRMIListener(IRMIListener* pListener);
	bool          RemoteContextHasAuthority(INetChannel* pChannel, EntityId id);
	void          SetAspectProfile(EntityId id, NetworkAspectType aspectBit, u8 profile);
	u8         GetAspectProfile(EntityId id, NetworkAspectType aspectBit);
	void          SetParentObject(EntityId objId, EntityId parentId);
	void          LogBreak(const SNetBreakDescription& breakage);
	bool          SetSchedulingParams(EntityId objId, u32 normal, u32 owned);
	void          PulseObject(EntityId objId, u32 pulseType);
	void          EnableBackgroundPassthrough(bool enable);
	i32           RegisterPredictedSpawn(INetChannel* pChannel, EntityId id);
	void          RegisterValidatedPredictedSpawn(INetChannel* pChannel, i32 predictionHandle, EntityId id);
	...
	void GC_SetAspectProfile(NetworkAspectType, u8, SNetObjectID, CNetObjectBindLock);
	void GC_EndContext();
	void GC_SetParentObject(EntityId objId, EntityId parentId);
	void GC_PulseObject(EntityId objId, u32 pulseType);
	...
	void NC_SetAspectProfile(EntityId userID, NetworkAspectType aspectBit, u8 profile, bool lockedUpdate);
...
private:
...
#if RESERVE_UNBOUND_ENTITIES
	typedef VectorMap<u16, EntityId>   TReservedUnboundEntityMap;
#endif
	...
	void              LogChangedProfile(SNetObjectID id, NetworkAspectType aspect, u8 profile);

	int64 m_netAspectSendTime;
	i32   m_lastPacketSendRate;
	...
	std::priority_queue<u16, std::vector<u16>, std::greater<u16>> m_freeLowBitObjects;
	std::priority_queue<u16, std::vector<u16>, std::greater<u16>> m_freeMediumBitObjects;
	std::priority_queue<u16, std::vector<u16>, std::greater<u16>> m_freeHighBitObjects;
	...
	void CacheAspectProfileChange(SNetObjectID id, NetworkAspectType aspect, u8 profile);
	bool GetAspectProfileFromCache(SNetObjectID id, NetworkAspectType aspect, u8& profile);
	...
	void AddToFreeObjects(u16 id);
	void ClearFreeObjects();

#if USE_SYSTEM_ALLOCATOR
	typedef std::unordered_map<EntityId, SNetObjectID, stl::hash_uint32>                                                                                      TNetIDMap;
#else
	typedef std::unordered_map<EntityId, SNetObjectID, stl::hash_uint32, std::equal_to<u32>, STLMementoAllocator<std::pair<const EntityId, SNetObjectID>>> TNetIDMap;
#endif
	...
	bool UnboundObject(SNetObjectID nID, tukk reason);
	void BoundObject(SNetObjectID nID, tukk reason)
	{
