...
struct NetMessage
{
	...
	ILINE NetMessage(tukk description, float sizeInBytes, float queuingLatency, tukk additionalText) :
...
struct FrequentNetMessage
{
	...
	{
		if (m_message.m_sizeInBytes > (u32(1) << 31))
		{
...
struct CompareFrequentNetMessage
{
	...
	CompareFrequentNetMessage(i32 key) { this->key = key; }
...
public:
	...
	void AddMessage(tukk description, float sizeInBytes = 0, float queuingLatency = -1, tukk additionalText = NULL);
...
private:
	...
	void   DrawLabel(float startPos, float col, i32 row, float* color, float glow, tukk szText, float fScale) const;
