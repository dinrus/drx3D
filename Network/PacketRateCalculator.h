...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  manages our packet rate and bandwidth usage locally for one
               channel
   -------------------------------------------------------------------------
   История:
...
public:
	...
	void AddSample(CTimeValue x, CTimeValue y)
	{
		if (m_count)
		{
			...
			{
				NetWarning("CTimerRegression::AddSample() diffInTimes %" PRIi64 " (diff local %llu, diff remote %llu) - time diverged too much; clearing samples", diffInTimes, diffX, diffY);
				m_count = 0;
			}
		}
...
public:
	...
	static u32k MaxBandwidth = 100 * 1024 * 1024 / 8;
	static u32k MinBandwidth = 9600 / 8;

	CPacketRateCalculator();
	void       SentPacket(CTimeValue nTime, u32 nSeq, u16 nSize);
	void       GotPacket(CTimeValue nTime, u16 nSize);
	void       AckedPacket(CTimeValue nTime, u32 nSeq, bool bAck);
	...
	u16     GetMaxPacketSize(CTimeValue nTime) { return m_pmtuDiscovery.GetMaxPacketSize(nTime); }
	u16     GetIdealPacketSize(i32 age, bool idle, u16 maxPacketSize);
	CTimeValue GetNextPacketTime(i32 age, bool idle);
...
private:
	...
	SPacketSizerResult NextPacket(i32 age, bool idle);
	...
	CCyclicStatsBuffer<u32, 128>       m_bandwidthUsedAmount[2];
	...
	//std::map<u32, CTimeValue> m_latencyLab;
	MiniQueue<CTimeValue, 127> m_latencyLab; // NOTE: the max value of N must NOT be bigger than 127 which is half the u8 space (-128 ~ 127)!!!
	u32                     m_latencyLabHighestSequence;
};
