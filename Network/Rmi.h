...
public:
	bool Sync() override
	{
		...
		DRX_ASSERT_MESSAGE(pEntity && pUser && ok, "RMI remote at sync: failed with"
			" entity id %d. pEntity:%x, pUser:%x (guid:%x), ok:%d", m_id, pEntity,
			pUser, drxiidof<User>().hipart, ok);
		return ok;
	}
...
public:
	...
	static CeRMIBody* Create(const INetEntity::SRmiHandler& ermi,
		...
		IRMIListener* pListener, i32 userId, EntityId dependentId)
	{
...
private:
	...
	CeRMIBody(const INetEntity::SRmiHandler& ermi, const INetEntity::SRmiIndex rmi_idx,
		EntityId id, const T& params, IRMIListener* pListener, i32 userId, EntityId dependentId)
...
//! \par Example
//! \include DinrusXEntitySys/Examples/ComponentRemoteMethodInvocation.cpp
template <class User, class Param, DrxRmi::RmiCallback<User, Param> fn>
struct SRmi<DrxRmi::RmiCallback<User, Param>, fn>
{
...
protected:
	static void Invoke(User *this_user, Param &&p, i32 where, i32 channel = -1, const EntityId dependentId = 0)
	{
		INetEntity::SRmiHandler *handler = nullptr;
		INetEntity::SRmiIndex idx = this_user->GetEntity()->GetNetEntity()->RmiByDecoder(
			Decoder, &handler);
		gEnv->pGameFramework->DoInvokeRMI(DrxRmi::CeRMIBody<Param>::Create(
...
public:
	...
	static inline void InvokeOnClient(User *this_user, Param&& p, i32 targetClientChannelId, const EntityId dependentId = 0)
	{
		DRX_ASSERT(gEnv->bServer);
