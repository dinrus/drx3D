...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  Records a game session and serializes it to a file.
   -------------------------------------------------------------------------
   История:
   - ??/2005 : Created by Craig Tiller
   - 04/2006 : Taken over by Jan M�ller
*************************************************************************/
...
public:
	CDemoRecordListener(CNetContext* pContext, tukk filename);
	~CDemoRecordListener();
...
private:
	...
	void DoSetAspectProfile(SNetObjectID id, NetworkAspectType aspects, u8 profile);
...
L_continue:;
	...
	public:
		...
		void Value(tukk name, const T_Value& value)
		{
		...
		void Value(tukk name, const CTimeValue& value)
		{
			m_pOut->Put(name, value.GetSeconds());
		}
		void Value(tukk name, const ScriptAnyValue& value)
		{
		}
		void Value(tukk name, const XmlNodeRef& value)
		{
		}
		template<class T_Value, class T_Policy>
		ILINE void Value(tukk szName, const T_Value& value, const T_Policy& policy)
		{
			Value(szName, value);
		}
		void Value(tukk szName, EntityId value, u32 policy)
		{
			//assert(policy == 'eid');
	...
	public:
		...
		virtual void            Disconnect(EDisconnectionCause cause, tukk fmt, ...) { m_connected = false; }
		...
		virtual DrxSessionHandle        GetSession() const                                              { return DrxSessionInvalidHandle; }
		...
		virtual bool                    GetRemoteNetAddress(u32& uip, u16& port, bool firstLocal) { return false; }
		virtual bool                    SendTo(u8k* pData, size_t nSize, const TNetAddress& to) { return false; }
		virtual tukk             GetName()                                                       { return "DemoRecorder"; }
		virtual tukk             GetNickname()                                                   { return 0; }
		...
		virtual i32                     GetContextViewStateDebugCode() const                            { return 0; }
