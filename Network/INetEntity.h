...
#define _DRXNETWORK_CONCAT(x, y) x ## y
#define DRXNETWORK_CONCAT(x, y)  _DRXNETWORK_CONCAT(x, y)
#define ASPECT_TYPE DRXNETWORK_CONCAT(uint, UNSAFE_NUM_ASPECTS)
...
{
	virtual ~IGameObjectProfileUpr() {}

	//! Override to receive profile change events.
	virtual bool  SetAspectProfile(EEntityAspects aspect, u8 profile) = 0;
	//! Override to change the default (spawned) profile for an aspect.
	virtual u8 GetDefaultProfile(EEntityAspects aspect) = 0;
};
...
struct INetEntity
{
	...
	virtual bool   SetAspectProfile(EEntityAspects aspect, u8 profile, bool fromNetwork = false) = 0;
	virtual u8  GetAspectProfile(EEntityAspects aspect) = 0;

	virtual bool   CaptureProfileUpr(IGameObjectProfileUpr* pPM) = 0;
	virtual void   ReleaseProfileUpr(IGameObjectProfileUpr* pPM) = 0;
	virtual bool   HasProfileUpr() = 0;
	virtual void   ClearProfileUpr() = 0;
	...
	virtual bool NetSerializeEntity(TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags) = 0;
