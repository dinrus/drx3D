...
#ifndef __NEWMESSAGEQUEUE_H__
#define __NEWMESSAGEQUEUE_H__
...
struct IMessageOutput
{
	...
	#if ENABLE_PACKET_PREDICTION
	virtual CMessageMapper*     GetOutputMapper() = 0;
	#endif
...
public:
	...
	void                           DebugDrawAccountingGroups(const SSchedulingParams& params);
	void                           DebugDrawSchedulerSends(const SSchedulingParams& params);
...
private:
	...
	struct SAccountingGroup
	{
		...
		SAccountingGroup()
		{
			...
			expirationPeriod = CNetCVars::Get().SchedulerSendExpirationPeriod;
		}
		...
		float GetBandwidthUsed(CTimeValue now)
		{
		...
		bool BandwidthExceeded(CTimeValue now, bool randomEarlyDetection)
		{
		...
		void PushSend(SSentEnt& sent)
		{
			...
			totBandwidthUsed += sent.bandwidth;
			bandwidthThisFrame += sent.bandwidth;
			sends.Push(sent);
		}
	};
	...
	{
		SMsgEntOrderingInfo() : latencyClass(eLC_DontBother), schedulingOrder(0.0f) {}
	...
	{
		SMessageHistory(void) {};
		SMessageHistory(u16 size, u16 index) : m_size(size), m_index(index) {}

		u16 m_size;
		u16 m_index;
	};
	static std::vector<SMessageHistory> s_messageHistory;

	std::vector<SSendableHandle>        m_pendingRemovals;

	static u32k                 MAX_ACCOUNTING_GROUPS = 64;
	u32                              m_nAccountingGroups;
