...
#include <drx3D/Network/IVoicePacketListener.h>
...
static i32k MAX_BREAK_STREAMS = 6; // must match with compression policy 'brId'
...
public:
	...
	CContextViewObjectLock LockObject(SNetObjectID id, tukk why);
	...
	void SetAspectProfile(SNetObjectID objectID, NetworkAspectType aspect, u8 profile);
	...
	CMementoMemoryUpr& GetMMM() { return *m_pMMM; }
...
protected:
	...
	void SetMMM(CMementoMemoryUprPtr pMMM) { m_pMMM = pMMM; }
	...
	virtual void OnViewStateDisconnect(tukk message);
	// ~CContextViewStateUpr

	static tukk GetStateName(EContextViewState state);
	static tukk GetWaitStateName(EContextViewState state);
	...
	bool         PartialAspect(NetworkAspectID aspectIdx, TSerialize ser, u32, u32, u32);

private:
	virtual bool        ShouldInitContext() = 0;
	virtual tukk DebugString() = 0;
...
protected:
	...
	public:
		...
		virtual void UpdateState(u32 nFromSeq, ENetSendableStateUpdate state)
		{
	...
	bool            UpdateAspect(NetworkAspectID i, TSerialize pSerialize, u32 nCurSeq, u32 nOldSeq, u32 timeFraction32);
	SReceiveContext CreateReceiveContext(TSerialize ser, NetworkAspectID index, u32 nCurSeq, u32 nOldSeq, u32 timeFraction32, bool* ok);
	...
	void ChangedObject(SNetObjectID id, u32 flags, NetworkAspectType dirtyAspects);
...
private:
	...
	bool GetDependentMessageIDForObject(SNetObjectID id, i32& handle);
...
#if ENABLE_DEBUG_KIT
	std::set<u16> m_updatedObjectsThisFrame;
...
typedef u8 aspect_num;
template<template<i32> class Func, aspect_num... values>
...
template<template<i32> class Func, aspect_num k, aspect_num... values>
...
template<i32 AspectNum, class T, bool (T::*memf)(NetworkAspectID, TSerialize, u32, u32, u32)>
TNetMessageCallbackResult TrampolineAspect(u32, INetMessageSink *instance,
	TSerialize serialize, u32 curSeq, u32 oldSeq, u32 timeFraction32, EntityId *, INetChannel *)
{
