...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  implements a communications channel between two computers
   -------------------------------------------------------------------------
   История:
...
{
public:
	CNetChannel(const TNetAddress& ipRemote, const CExponentialKeyExchange::KEY_TYPE& key, u32 remoteSocketCaps, DrxSessionHandle session);
	~CNetChannel();
	...
	virtual void               Disconnect(EDisconnectionCause cause, tukk fmt, ...);
	virtual void               SendMsg(INetMessage*);
	virtual bool               AddSendable(INetSendablePtr pSendable, i32 numAfterHandle, const SSendableHandle* afterHandle, SSendableHandle* handle);
	virtual bool               SubstituteSendable(INetSendablePtr pSendable, i32 numAfterHandle, const SSendableHandle* afterHandle, SSendableHandle* handle);
	virtual bool               RemoveSendable(SSendableHandle handle);
	virtual const SStatistics& GetStatistics();
	virtual void               SetPassword(tukk password);
	virtual DrxSessionHandle   GetSession() const;
	...
	virtual bool          GetRemoteNetAddress(u32& uip, u16& port, bool firstLocal = true);
	virtual tukk   GetName();
	virtual tukk   GetNickname();
	virtual void          SetNickname(tukk name);
	...
	virtual i32                     GetContextViewStateDebugCode() const;
	...
	virtual i32  GetProfileId() const;
	...
	virtual void LogRMIBenchmark(ERMIBenchmarkAction action, const SRMIBenchmarkParams& params, void (* pCallback)(ERMIBenchmarkLogPoint point0, ERMIBenchmarkLogPoint point1, int64 delay, uk pUserData), uk pUserData);
#endif
	...
	void       GetBandwidthStatistics(u32 channelIndex, SBandwidthStats* const pStats);

	string     GetCDKeyHash() const;

	ILINE bool NetAddSendable(INetSendable* pMsg, i32 numAfterHandle, const SSendableHandle* afterHandle, SSendableHandle* handle)
	{
		ASSERT_GLOBAL_LOCK;
	...
	ILINE bool NetSubstituteSendable(INetSendable* pMsg, i32 numAfterHandle, const SSendableHandle* afterHandle, SSendableHandle* handle)
	{
		ASSERT_GLOBAL_LOCK;
	...
	void            ChangeSubscription(ICTPEndpointListener* pListener, u32 eventMask) { m_ctpEndpoint.ChangeSubscription(pListener, eventMask); }
	...
	virtual bool         IsDead();
	virtual void         ProcessPacket(bool forceWaitForSync, u8k* pData, u32 nLength);
	...
	void Send(u8k* pData, size_t nLength)
	{
		ASSERT_GLOBAL_LOCK;
		if (m_pNub)
		{
	...
	// like CNetNub::SendTo
	void SendWithNubTo(u8k* pData, size_t nLength, const TNetAddress& to)
	{
		if (m_pNub)
		{
	...
	u32                   GetMostRecentAckedSeq() const        { return m_ctpEndpoint.GetMostRecentAckedSeq(); }
	u32                   GetMostRecentSentSeq() const         { return m_ctpEndpoint.GetMostRecentSentSeq(); }
	bool                     IsServer();
	...
	CMementoMemoryUprPtr GetChannelMMM()   { return m_pMMM; }
	...
	{
		u8 buf[LOAD_NETWORK_PKTSIZE];
		void          SerializeWith(TSerialize ser)
		{
			for (i32 i = 0; i < LOAD_NETWORK_PKTSIZE; i++)
			{
				ser.Value("b", buf[i]);
			}
		}
	...
	//bool GetRemoteNetAddress(u32 &ip, u16 &port);
	...
	void SetProfileId(i32 id);
	void SetCDKeyHash(tukk hash);
...
private:
	...
	{
		return (m_remoteSocketCaps & eSIOMC_SupportsBackoff) != 0 && (CNetwork::Get()->GetExternalSocketIOUpr().caps & eSIOMC_SupportsBackoff) != 0;
	}
	...
	struct SRMIBenchmarkRecord
	{
		...
		void       (* pCallback)(ERMIBenchmarkLogPoint point0, ERMIBenchmarkLogPoint point1, int64 delay, uk pUserData);
		uk      pUserData;
		EntityId   entity;
		u32     eventBits;
	...
	i32                m_nStrongCaptureRMIs;
	u32               m_remoteSocketCaps;
	i32                m_profileId;
	i32                m_pendingStuffBeforeDead;
