...
// until they are sent (in case of outgoing traffic) or until they are polled by ReceiveMessage().
...
public:
	...
	ILINE ServiceNetworkAddress(const ServiceNetworkAddress& other, u16 newPort = 0)
	...
	ILINE ServiceNetworkAddress(u8 ip0, u8 ip1, u8 ip2, u8 ip3, u16 port)
	{
		m_address.m_ip0 = ip0;
		m_address.m_ip1 = ip1;
		m_address.m_ip2 = ip2;
...
public:
	...
	virtual uk GetPointer() = 0;

	//! Get pointer to the message data.
	virtual ukk GetPointer() const = 0;

	//! Create reader interface for reading message data, returned object is not reference-counted but it will hold a reference to the message.
...
public:
	static u32k kDefaultFlushTime = 10000; //!< ms.
	...
	//! \return false only if connection has been damaged beyond repair.
	virtual bool IsAlive() const = 0;

	//! Get number of messages sent by this connection so far.
	virtual u32 GetMessageSendCount() const = 0;

	//! Get number of messages received by this connection so far.
	virtual u32 GetMessageReceivedCount() const = 0;
	...
	virtual void FlushAndClose(u32k timeoutMs = kDefaultFlushTime) = 0;
...
public:
	...
	virtual IServiceNetworkMessage* AllocMessageBuffer(u32k size) = 0;
	...
	virtual struct IDataReadStream* CreateMessageReader(ukk pData, u32k dataSize) = 0;

	//! Translate host address (string:port) to network address.
	virtual ServiceNetworkAddress GetHostAddress(const string& addressString, u16 optionalPort = 0) const = 0;

	//! Create network listener on given local port, listening and accepting connections is done on network thread.
	virtual IServiceNetworkListener* CreateListener(u16 localPort) = 0;
