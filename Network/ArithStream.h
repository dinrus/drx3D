...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  simple arithmetic-style encoding and decoding
...
protected:
	void AddEncToCRC(uint64 tot, u32 low, u32 sym)
	{
		m_crc.Add64(tot);
		m_crc.Add32(low);
		m_crc.Add32(sym);
...
{
protected:
	ILINE void AddEncToCRC(uint64 tot, u32 low, u32 sym) {}
	ILINE void ResetCRC()                                      {}
};
#endif
...
protected:
	...
	static i32k               ShiftBits = 55;
	static i32k               ExtraBits = 7;
...
public:
	...
	CCommOutputStream(u8* vOutput, size_t nMaxOutputSize, u8 bonus = 0);
	// output to a dynamically allocated buffer
	CCommOutputStream(IStreamAllocator*, size_t nInitialSize, u8 bonus = 0);
	~CCommOutputStream()
	{
	...
	void EncodeShift(i32 bits, ProbabilityType low, ProbabilityType sym);

	// finish encoding;
	// returns the size of the stream
	size_t       Flush();
	// get a pointer to the encoded data
	u8k* GetPtr() const { return m_vOutput; }
	...
	static inline float EstimateArithSizeInBitsShift(i32 bits, ProbabilityType sym)
	{
	...
	inline void WriteBits(u32 nValue, i32 nBits)
	{
		EncodeShift(nBits, nValue, 1);
	}

	inline void WriteBitsLarge(uint64 nValue, i32 nBits)
	{
		if (nBits > 32)
		{
			nBits -= 32;
	...
	inline void WriteInt(u32 nValue, u32 nMax)
	{
		Encode(InternalStateType(nMax) + 1, nValue, 1);
	}

	inline void PutZeros(i32 n)
	{
		for (i32 i = 0; i < n; i++)
			PutByte(0);
	}
...
private:
...
#if ENABLE_ACCURATE_BANDWIDTH_PROFILING
	ILINE void UpdateSizeBits(i32 n)
	{
		m_accurateSize += n;
	}
	...
	NO_INLINE void Grow()
	{
		if (!m_pSA)
		{
			DrxFatalError("No allocator and overflowed output");
			return; // for static analysis tools to understand this
		}
		m_nAllocSize *= 2;
		m_vOutput = (u8*)m_pSA->Realloc(m_vOutput, m_nAllocSize);
	}
...
inline CCommOutputStream::CCommOutputStream(u8* vOutput, size_t nMaxOutputSize, u8 bonus)
{
#if ENABLE_DEBUG_KIT
	m_bLog = false;
#endif
...
inline CCommOutputStream::CCommOutputStream(IStreamAllocator* pSA, size_t nInitialSize, u8 bonus)
{
#if ENABLE_DEBUG_KIT
	m_bLog = false;
#endif
	m_vOutput = (u8*)pSA->Alloc(nInitialSize);
	m_nAllocSize = nInitialSize;
	m_pSA = pSA;
	Reset(bonus);
}

inline void CCommOutputStream::Reset(u8 bonus)
{
	m_low = 0;
	m_range = TopValue;
	m_buffer = bonus;
...
inline void CCommOutputStream::EncodeShift(i32 bits, ProbabilityType low, ProbabilityType sym)
{
#if ENABLE_DEBUG_KIT
	if (m_bLog)
...
public:
	CCommInputStream(u8k* input, size_t length);
	...
	ProbabilityType DecodeShift(i32 bits)
	{
		Renormalize();
		m_help = m_range >> bits;
		NET_ASSERT(m_help);
	...
	void UpdateShift(i32 bits, ProbabilityType low, ProbabilityType sym)
	{
	...
	u8  GetBonus() const { return m_bonus; }
	...
	inline u32 ReadBits(i32 nBits)
	{
		u32 val = DecodeShift(nBits);
		UpdateShift(nBits, (ProbabilityType)val, (ProbabilityType)1);
		return val;
	}

	inline uint64 ReadBitsLarge(i32 nBits)
	{
		uint64 nValue = 0;
		if (nBits > 32)
		{
		...
		u32 nFinal = DecodeShift(nBits);
		UpdateShift(nBits, (ProbabilityType)nFinal, (ProbabilityType)1);
		nValue |= nFinal;
		return nValue;
	}

	inline u32 ReadInt(u32 nMax)
	{
		u32 val = Decode(InternalStateType(nMax) + 1);
...
inline CCommInputStream::CCommInputStream(u8k* input, size_t length)
{
#if ENABLE_DEBUG_KIT
	m_bLog = false;
#endif
