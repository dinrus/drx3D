...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  реализует мультиплексирование и соединение/разъединение
               каналов
   -------------------------------------------------------------------------
   История:
...
static i32k VERSION_SIZE = 6;

void TraceUnrecognizedPacket(tukk inszTxt, u8k* cBuf, size_t nCount, const TNetAddress& ip)
{
#ifdef _DEBUG
	OutputDebugString("\n");
	...
	::OutputDebugString("-------------------------------\n");
	drx_sprintf(sTemp, "INVALID PACKET FROM [%s]\n", RESOLVER.ToString(ip).c_str());
	::OutputDebugString(sTemp);
	for (size_t n = 0; n < nCount; n++)
	{
		drx_sprintf(sTemp, "%02X ", cBuf[n]);
		::OutputDebugString(sTemp);
		if (n && (n % 16) == 0)
			::OutputDebugString("\n");
	}
...
template<class Map>
typename Map::iterator LookupAddress(const TNetAddress& addr, Map& m, u32 flags)
{
	...
	if (flags & eCLF_HandleBrokenWindowsICMP)
	{
		...
		{
			i32 count = 0;
			typename Map::iterator itOut = m.end();
...
{
public:
	bool IsIPBanned(u32 ip)                              { return false; }
	void OnPunkDetected(tukk ip, EPunkType punkType) {}
};
...
public:
	...
	NegVector::iterator FindNeg(i32 cookie)
	{
	...
	NegVector::const_iterator FindNeg(i32 cookie) const
	{
	...
	void OnCompleted(i32 cookie, bool success, DRXSOCKADDR_IN* addr)
	{
	...
	EState GetState(i32 cookie) const
	{
	...
	void SetState(i32 cookie, EState state)
	{
		NegVector::iterator it = FindNeg(cookie);
		if (it != m_negotiations.end())
			it->m_state = state;
	}
	bool GetResult(i32 cookie) const
	{
		NegVector::const_iterator it = FindNeg(cookie);
		if (it == m_negotiations.end())
			return false;
	...
	void GetAddr(i32 cookie, DRXSOCKADDR_IN& a) const
	{
		NegVector::const_iterator it = FindNeg(cookie);
		if (it != m_negotiations.end())
			a = it->m_addr;
...
#if TIMER_DEBUG
	char buffer[128];
	drx_sprintf(buffer, "CNetNub::CNetNub() m_statsTimer [%p]", this);
#else
	tuk buffer = NULL;
#endif // TIMER_DEBUG
...
i32 SMessageIDCountCmp(ukk p1, ukk p2)
{
	u32 c1 = g_debugPacketDataSize.pMessageIDData[((const SMessageIDCount*)p1)->id].data[((const SMessageIDCount*)p1)->table].count;
	u32 c2 = g_debugPacketDataSize.pMessageIDData[((const SMessageIDCount*)p2)->id].data[((const SMessageIDCount*)p1)->table].count;
...
CNetNub::~CNetNub()
{
	...
	if (m_pSocketMain)
	{
		...
		ISocketIOUpr* pSocketIOUpr = &CNetwork::Get()->GetInternalSocketIOUpr();
		if (m_pLobby && m_pLobby->GetLobbyServiceType() == eCLS_Online)
		{
			pSocketIOUpr = &CNetwork::Get()->GetExternalSocketIOUpr();
		}
		pSocketIOUpr->FreeDatagramSocket(m_pSocketMain);
		m_pSocketMain = NULL;
	}
	...
	{
		u32 unaccountedData = g_debugPacketDataSize.totalDataSize -
		...
		u32 origTotalDataSize = g_debugPacketDataSize.totalDataSize + (g_debugPacketDataSize.origMessageIDSize - g_debugPacketDataSize.dataSize[eDPDST_MessageID]) + (g_debugPacketDataSize.origNetIDSize - g_debugPacketDataSize.dataSize[eDPDST_NetID]);
	...
	if ((g_debugPacketDataSize.numMessageIDs > 0) && (g_debugPacketDataSize.totalNumMessages[eMIDT_Global]))
	{
		...
		for (u32 t = 0; t < eMIDT_NumTables; t++)
		{
			if (g_debugPacketDataSize.totalNumMessages[t] > 0)
			{
				for (u32 i = 0; i < g_debugPacketDataSize.numMessageIDs; i++)
				{
...
bool CNetNub::ConnectTo(tukk address, tukk connectionString)
{
	SCOPED_GLOBAL_LOCK;
	CNubConnectingLock conlk(this);
	NetLogAlways("connection requested to: %s", address);
	IDrxLobby* pLobby = gEnv->pLobby;
	tukk nat_prefix = "<nat>";
	size_t nat_prefixLen = strlen(nat_prefix);
	tukk sessionPrefix = "<session>";
	DrxSessionHandle session = DrxSessionInvalidHandle;
	...
	IDrxMatchMakingPrivate* pMMPrivate = pLobby ? pLobby->GetMatchMakingPrivate() : nullptr;
	if (session != DrxSessionInvalidHandle && pMMPrivate)
	{
		TNetAddressVec tmp;
...
void CNetNub::GN_Lazy_ContinueConnectTo(CNameRequestPtr pReq, DrxSessionHandle session, string connectionString, CNubConnectingLock conlk)
{
	SCOPED_GLOBAL_LOCK;
...
void CNetNub::GN_Loop_ContinueConnectTo(CNameRequestPtr pReq, DrxSessionHandle session, string connectionString, CNubConnectingLock conlk)
{
...
void CNetNub::LogChannelPerformanceMetrics(u32 id) const
{
...
void CNetNub::SetChannelPerformanceMetrics(u32 id, INetChannel::SPerformanceMetrics& metrics)
{
	bool foundChannel = false;
...
void CNetNub::DoConnectTo(const TNetAddressVec& ips, DrxSessionHandle session, string connectionString, CNubConnectingLock conlk)
{
	// cannot connect to an already connected nub
	bool isAlreadyConnected = false;
	{
		DrxAutoLock<DrxCriticalSection> lock(m_lockChannels);
...
void CNetNub::DisconnectPlayer(EDisconnectionCause cause, EntityId plr_id, tukk reason)
{
	for (TChannelMap::iterator i = m_channels.begin(); i != m_channels.end(); ++i)
	{
		CNetChannel* pCh = i->second->GetNetChannel();
...
void CNetNub::TimerCallback(NetTimerId, uk p, CTimeValue)
{
...
#if TIMER_DEBUG
		char buffer[128];
		drx_sprintf(buffer, "CNetNub::TimerCallback() m_statsTimer [%p]", pThis);
#else
		tuk buffer = NULL;
#endif // TIMER_DEBUG
...
bool CNetNub::SendPendingConnect(SPendingConnection& pc)
{
	...
	if (pc.kes == eKES_SetupInitiated)
	{
		if (g_time - pc.kesStart >= KESTIMEOUT)
		{
			...
			IDrxMatchMakingPrivate* pMMPrivate = m_pLobby ? m_pLobby->GetMatchMakingPrivate() : nullptr;
			if (pMMPrivate)
			{
				DrxLog("CNetNub::SendPendingConnect calling SessionDisconnectRemoteConnection");
		...
		v[5] = htonl(CNetwork::Get()->GetExternalSocketIOUpr().caps);
		...
		PREFAST_SUPPRESS_WARNING(6255) u8 * pBuffer = (u8*) alloca(1 + (VERSION_SIZE)*sizeof(u32) + 2 * sizeof(u32) + sizeof(DrxSessionHandle) + cslen);

		u32 cur_size = 0;
		pBuffer[cur_size] = Frame_IDToHeader[eH_ConnectionSetup];
		cur_size++;
		memcpy(pBuffer + cur_size, v, VERSION_SIZE * sizeof(u32));
		cur_size += VERSION_SIZE * sizeof(u32);
		memcpy(pBuffer + cur_size, &profile, sizeof(u32));
		cur_size += sizeof(u32);
		memcpy(pBuffer + cur_size, &tokenId, sizeof(u32));
		cur_size += sizeof(u32);
		DrxSessionHandle session = htonl(pc.session);
		memcpy(pBuffer + cur_size, &session, sizeof(DrxSessionHandle));
		cur_size += sizeof(DrxSessionHandle);
...
bool CNetNub::Init(CNetwork* pNetwork)
{
	...
	m_pSocketMain = pSocketIOUpr->CreateDatagramSocket(m_addr, flags);
	if (!m_pSocketMain)
		return false;
...
{
public:
	CVerifyBuffer(u8k* pBuffer, unsigned nLength)
	{
		m_nLength = nLength;
		m_vBuffer = new u8[nLength];
...
bool CNetNub::SendTo(u8k* pData, size_t nSize, const TNetAddress& to)
{
...
void CNetNub::OnPacket(const TNetAddress& from, u8k* pData, u32 nLength)
{
	NET_ASSERT(nLength);
	...
	if (!processed)
	{
		TraceUnrecognizedPacket("ProcessPacketFrom", (u8*)pData, nLength, from);
	}
}

void CNetNub::ProcessSimplePacket(const TNetAddress& from, u8k* pData, u32 nLength)
{
	if (nLength != 1)
		return;

	u8 nType = Frame_HeaderToID[pData[0]];
...
bool CNetNub::ProcessTransportPacketFrom(const TNetAddress& from, u8k* pData, u32 nLength)
{
	//	NET_ASSERT(  == CTPFrameHeader );

	//	u8 type = GetFrameType(((u8*)MMM_PACKETDATA.PinHdl(hdl))[0]);
...
void CNetNub::ProcessDisconnect(const TNetAddress& from, u8k* pData, u32 nSize)
{
	CNetChannel* pNetChannel = GetChannelForIP(from, 0);
	u8 cause = eDC_Unknown;
	string reason;
	...
	else
	{
		...
		{
			string backlog((tuk)pData + 2, (tuk)pData + nSize);
...
void CNetNub::ProcessAlreadyConnecting(const TNetAddress& from, u8k* pData, u32 nSize)
{
...
void CNetNub::DisconnectChannel(EDisconnectionCause cause, const TNetAddress* pFrom, CNetChannel* pChannel, tukk reason)
{
	if (!pFrom)
	{
		NET_ASSERT(pChannel);
	...
	if (pFrom)
	{
		AddDisconnectEntry(*pFrom, pChannel ? pChannel->GetSession() : DrxSessionInvalidHandle, cause, reason);
	}
}

void CNetNub::DisconnectChannel(EDisconnectionCause cause, DrxSessionHandle session, tukk pReason)
{
	SCOPED_GLOBAL_LOCK;
	...
	if (pMMPrivate)
	{
		u32 id = pMMPrivate->GetChannelIDFromGameSessionHandle(session);
...
INetChannel* CNetNub::GetChannelFromSessionHandle(DrxSessionHandle session)
{
	SCOPED_GLOBAL_LOCK;
...
void CNetNub::NetDump(ENetDumpType type, INetDumpLogger& logger)
{
	if ((type == eNDT_ClientConnectionState) || (type == eNDT_ServerConnectionState))
	{
		...
		{
			sockaddrStr.Format("%s:%u", DrxSock::inet_ntoa(addr.sin_addr), addr.sin_port);
		}
		else
		{
			sockaddrStr = "unknown";
...
#if defined(_DEBUG)
	u8 buffer[2 + sizeof(dis.info) + sizeof(dis.backlog)]; // frame + cause + reason + backlog
#else
	u8 buffer[2 + sizeof(dis.info)]; // frame + cause + reason
#endif
...
bool CNetNub::ProcessPingQuery(const TNetAddress& from, u8k* pData, u32 nSize)
{
	//NET_ASSERT( GetFrameType(((u8*)MMM_PACKETDATA.PinHdl(hdl))[0]) == QueryFrameHeader );
...
void CNetNub::ProcessLanQuery(const TNetAddress& from)
{
	...
	m_pSocketMain->Send((u8k*)str.c_str(), str.length(), from);
}

void CNetNub::LobbySafeDisconnect(CNetChannel* pChannel, EDisconnectionCause cause, tukk reason)
{
	IDrxMatchMakingPrivate* pMMPrivate = m_pLobby ? m_pLobby->GetMatchMakingPrivate() : nullptr;
	if (pMMPrivate)
	{
...
#endif
	u32 socketCaps = htonl(CNetwork::Get()->GetExternalSocketIOUpr().caps);
	memcpy(pktbuf + 1 + 3 * KS, &socketCaps, sizeof(u32));
...
void CNetNub::ProcessKeyExchange0(const TNetAddress& from, u8k* pData, u32 nSize)
{
...
L_continue:
		...
		if (nSize != 1 + 3 * CExponentialKeyExchange::KEY_SIZE + sizeof(u32))
		{
...
#endif
		memcpy(&iter->socketCaps, pData, sizeof(u32));
		pData += sizeof(u32);
...
void CNetNub::ProcessKeyExchange1(const TNetAddress& from, u8k* pData, u32 nSize)
{
	if (GetChannelForIP(from, 0) != NULL)
		return;
	...
	if (iterCon == m_connectingMap.end())
	{
		AddDisconnectEntry(from, DrxSessionInvalidHandle, eDC_CantConnect, "KeyExchange1 with no connection");
...
void CNetNub::ProcessSetup(const TNetAddress& from, u8k* pData, u32 nSize)
{
#if defined(PURE_CLIENT)
	if ((gEnv->bMultiplayer) && (gEnv->bServer == gEnv->IsClient()))
	{
	...
	if (nSize < 1 + VERSION_SIZE * sizeof(u32) + 2 * sizeof(u32) + sizeof(DrxSessionHandle))
	{
		AddDisconnectEntry(from, DrxSessionInvalidHandle, eDC_ProtocolError, "Too short connection packet");
		return;
	}

	SFileVersion ver = gEnv->pSystem->GetProductVersion();
	u32 v[VERSION_SIZE];
	memcpy(v, pData + 1, VERSION_SIZE * sizeof(u32));

	u32 profile_data = 0;
	memcpy(&profile_data, (tuk)pData + 1 + VERSION_SIZE * sizeof(u32), sizeof(u32));
	u32 profile = ntohl(profile_data);

	memcpy(&profile_data, (tuk)pData + 1 + VERSION_SIZE * sizeof(u32) + sizeof(u32), sizeof(u32));
	u32 tokenId = ntohl(profile_data);

	DrxSessionHandle session;
	memcpy(&session, (tuk)pData + 1 + VERSION_SIZE * sizeof(u32) + 2 * sizeof(u32), sizeof(DrxSessionHandle));
	...
	string connectionString((tuk)pData + 1 + VERSION_SIZE * sizeof(u32) + 2 * sizeof(u32) + sizeof(DrxSessionHandle), (tuk)pData + nSize);
	...
	if (gEnv->bMultiplayer && m_pLobby && m_pLobby->GetMatchMaking())
	{
		...
		// m_disconnectMap isn't really needed when DrxMatchMaking is in use as DrxMatchMaking handles connects and disconnects
		// but we still need to keep it in case DrxMatchMaking isn't in use.
		// What we can say is if DrxMatchMaking knows of this address then it doesn't want to be disconnected because of m_disconnectMap so make sure it is not stored.
		m_disconnectMap.erase(from);

		DRX_ASSERT_MESSAGE(session != DrxSessionInvalidHandle, "Incoming connection with no session");

		if (session == DrxSessionInvalidHandle || !m_pLobby->GetMatchMakingPrivate()->IsNetAddressInLobbyConnection(from))
		{
			if (session != DrxSessionInvalidHandle)
			{
				DrxLogAlways("Canary : Duplicate Channel ID - Work around");
			}
...
CNetChannel* CNetNub::GetChannelForIP(const TNetAddress& ip, u32 flags)
{
	ASSERT_GLOBAL_LOCK;
...
void CNetNub::GetMemoryStatistics(IDrxSizer* pSizer)
{
...
void CNetNub::CreateChannel(const TNetAddress& ip, const CExponentialKeyExchange::KEY_TYPE& key, const string& connectionString, u32 remoteSocketCaps, u32 proifle, DrxSessionHandle session, CNubConnectingLock conlk)
{
...
#if NEW_BANDWIDTH_MANAGEMENT
u32 CNetNub::GetTotalBandwidthShares() const
{
	u32 totalShares = 0;
...
void CNetNub::AddDisconnectEntry(const TNetAddress& ip, DrxSessionHandle session, EDisconnectionCause cause, tukk reason)
{
	// If we are using CDrxMatchMaking then it is responsible for the connections so don't send nub disconnects just inform matchmaking.
	if (m_pLobby && m_pLobby->GetMatchMaking())
	{
		DrxLog("CNetNub::AddDisconnectEntry calling SessionDisconnectRemoteConnection reason %s", reason ? reason : "no reason");
