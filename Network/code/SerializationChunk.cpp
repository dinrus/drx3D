...
namespace
{
i32k NUM_BITS_FOR_OPS_SIZE = 16;
i16k INVALID_SKIP_FALSE = 0x7FFF;
...
{
	u32 len = s.GetTyped<u32>();
	tukk pBuffer = (tukk) s.Get(len);
...
struct GenImpl
{
	static void DecodeFromStream(tukk name, CNetInputSerializeImpl& input, CByteOutputStream& output, ICompressionPolicyPtr pPolicy)
	{
		T temp;
		input.ValueImpl(name, temp, pPolicy);
		CopyToBuffer(temp, output);
	}
	static void EncodeToStream(tukk name, CByteInputStream& input, CNetOutputSerializeImpl& output, ICompressionPolicyPtr pPolicy)
	{
		T temp;
...
typedef void (* DecodeFromStreamFunction)(tukk name, CNetInputSerializeImpl& input, CByteOutputStream& output, ICompressionPolicyPtr pPolicy);
typedef void (* EncodeToStreamFunction)(tukk name, CByteInputStream& input, CNetOutputSerializeImpl& output, ICompressionPolicyPtr pPolicy);
...
public:
	...
	template<class T>
	void Value(tukk name, T& value, u32 policy)
	{
		...
		NET_ASSERT(m_pChunk->m_overrideCompression || (CNetwork::Get()->GetCompressionUpr().GetCompressionPolicy(policy) == op.pPolicy));
	...
	void Value(tukk name, SSerializeString& value, u32 policy)
	{
		NET_ASSERT(m_op < m_pChunk->m_ops.size());
		SOp op = m_pChunk->m_ops[m_op];
#if TRACK_ENCODING
	...
	bool BeginOptionalGroup(tukk szName, bool cond)
	{
		SOp op = m_pChunk->m_ops[m_op];
#if TRACK_ENCODING
...
#endif
		u16k skipFalse = m_pChunk->m_ops[m_op].skipFalse;
...
public:
	...
	bool BeginOptionalGroup(tukk szName, bool cond)
	{
		...
		m_resolveConditions.push_back(static_cast<u16>(m_pChunk->m_ops.size()));
	...
	void EndGroup()
	{
		...
		if (m_inOptionalGroup.back())
		{
			...
			op.skipFalse = static_cast<i16>(m_pChunk->m_ops.size() - m_resolveConditions.back());
...
private:
	void AddValue(tukk name, EOps type, u32 policy)
	{
		SOp op;
		op.type = type;
		...
		op.pPolicy = CNetwork::Get()->GetCompressionUpr().GetCompressionPolicy(policy);
#if TRACK_ENCODING
		op.name = name;
#endif
#if CRC8_ASPECT_FORMAT
...
void CSerializationChunk::DecodeFromStream(CNetInputSerializeImpl& input, CByteOutputStream& output, ChunkID chunkID, u8 nProfile) const
{
	...
	output.PutTyped<u8>() = nProfile;

#if CRC8_ASPECT_FORMAT
	u8 recCrc = input.GetInput().ReadBits(8);
	NET_ASSERT(recCrc == m_crc);
	if (recCrc != m_crc)
		input.Failed();

	static_assert((NUM_BITS_FOR_OPS_SIZE % 8) == 0, "Invalid bit count, has to be a multiple of 8!");
	static_assert((NUM_BITS_FOR_OPS_SIZE / 8) <= sizeof(u16), "Invalid type size!");
	u16 recLen = input.GetInput().ReadBits(NUM_BITS_FOR_OPS_SIZE);
	...
	for (TOps::const_iterator pOp = m_ops.begin(); pOp != m_ops.end(); ++pOp)
	{
		...
		{
		default:
			assert(pOp->type < DRX_ARRAY_COUNT(DecodeFromStreamImpl));
		...
		case eO_OptionalGroup:
			{
				...
				//				else
				//					i32 k = 0; // make this break-pointable
			}
			break;
		}
	}
	...
	if (!input.Ok())
	{
		NetWarning("Readback failed: magic number is %.2x/%.2x:%.2x/%.2x", recCrc, recLen, m_crc, (u32)m_ops.size());
	}
#endif
}

void CSerializationChunk::EncodeToStream(CByteInputStream& input, CNetOutputSerializeImpl& output, ChunkID chunkID, u8 nProfile) const
{
	// when should we call EndGroup()
	// TODO: this probably isn't necessary under the current implementation... maybe it could be removed and save a few cycles on serialization
	MiniQueue<TOps::const_iterator, 128> endGroupOps;
	...
	u8 savedProfile = input.GetTyped<u8>();
	...
	for (TOps::const_iterator pOp = m_ops.begin(); pOp != m_ops.end(); ++pOp)
	{
		...
		{
		default:
			assert(pOp->type < DRX_ARRAY_COUNT(EncodeToStreamImpl));
		...
		case eO_OptionalGroup:
			{
				...
				if (pOp->skipFalse)
				{
					...
					NET_ASSERT(((i32)(pOp - m_ops.begin())) + pOp->skipFalse < (i32)m_ops.size());
					if (!cond)
						pOp += pOp->skipFalse;
					else
						endGroupOps.Push(pOp + pOp->skipFalse); // remember when to call EndGroup()
...
ESynchObjectResult CSerializationChunk::UpdateGame(IGameContext* pCtx, CByteInputStream& input, EntityId id, NetworkAspectType nAspect, ChunkID chunkID, u8 nProfile, bool& wasPartialUpdate) const
{
	ChunkID savedChunkID = input.GetTyped<ChunkID>();
	u8 savedProfile = input.GetTyped<u8>();
	...
	if (savedChunkID != chunkID || savedProfile != nProfile)
	{
		if (CNetCVars::Get().LogLevel > 0)
			NetWarning("Chunk/profile mismatch: chunk is %d and was %d; profile is %d and was %d; skipping update", i32(chunkID), i32(savedChunkID), i32(nProfile), i32(savedProfile));
		return eSOR_Skip;
	}
...
{
	SSerializationLogger(IEntity* pEntity, NetworkAspectType aspectType, u8 profile)
		: m_pEntity(pEntity)
	{
		if (m_pEntity)
		{
...
bool CSerializationChunk::FetchFromGame(IGameContext* pCtx, CByteOutputStream& output, EntityId id, NetworkAspectType nAspect, ChunkID chunkID, u8 nProfile) const
{
	output.PutTyped<ChunkID>() = chunkID;
	output.PutTyped<u8>() = nProfile;
...
bool CSerializationChunk::Init(IGameContext* pCtx, EntityId id, NetworkAspectType nAspect, u8 nProfile, NetworkAspectType noCompression)
{
...
#if ENABLE_DEBUG_KIT
void CSerializationChunk::Dump(EntityId eid, u32 id)
{
	NetLog("----------------------------------------------");
	...
	#if CRC8_ASPECT_FORMAT
	NetLog("-------- SERIALIZATION CHUNK %.8x magicnumber=%.2x/%.2x --------", id, i32(m_crc), m_ops.size());
	#else
