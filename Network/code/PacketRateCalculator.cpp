...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  calculates how long we should wait between packets, and how
...
float CPacketRateCalculator::GetBandwidthUsage(CTimeValue nTime, CPacketRateCalculator::eIncomingOutgoing direction)
{
	...
	u32 total = m_bandwidthUsedAmount[direction].GetTotal();
...
void CPacketRateCalculator::SentPacket(CTimeValue nTime, u32 nSeq, u16 nSize)
{
...
void CPacketRateCalculator::GotPacket(CTimeValue nTime, u16 nSize)
{
	m_bandwidthUsedTime[eIO_Incoming].AddSample(nTime);
	m_bandwidthUsedAmount[eIO_Incoming].AddSample(nSize + UDP_HEADER_SIZE);
}

void CPacketRateCalculator::AckedPacket(CTimeValue nTime, u32 nSeq, bool bAck)
{
	if (!bAck)
	{
		m_hadAnyLoss = true;
	...
	{
		u32 sz = m_latencyLab.Size() - 1;
		u32 lowestSeq = m_latencyLabHighestSequence - sz;                 // two's complement - u32
		i32 i = nSeq - lowestSeq, i1 = m_latencyLabHighestSequence - nSeq; // two's complement - u32 => i32
...
u16 CPacketRateCalculator::GetIdealPacketSize(i32 age, bool idle, u16 maxPacketSize)
{
	float sz = NextPacket(age, idle).ps;
	return u16(Clamp(sz, 0, maxPacketSize) + 0.5f);
}

CTimeValue CPacketRateCalculator::GetNextPacketTime(i32 age, bool idle)
{
...
static float SlowPacketRateForAge(float des, float low, i32 age)
{
	if (des < 1)
		return des;
...
float CPacketRateCalculator::GetTcpFriendlyBitRate()
{
	...
	else
	{
		...
		if (0.0f == lossEventRate && !m_hadAnyLoss)
		{
			...
			rate = m_lastSlowStartRate = m_tfrcHist.Empty() ? averageSegmentSize : (1.0f + (i32)allowInc) * m_tfrcHist.GetLast();
			m_allowSlowStartIncrease = false;
		}
		else
		{
...
CPacketRateCalculator::SPacketSizerResult CPacketRateCalculator::NextPacket(i32 age, bool idle)
{
