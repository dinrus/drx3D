...
#if TIMER_DEBUG
NetTimerId CNetTimer::AddTimer(CTimeValue when, NetTimerCallback callback, uk pUserData, tuk pFile, size_t line, tuk pName)
#else
NetTimerId CNetTimer::AddTimer(CTimeValue when, NetTimerCallback callback, uk pUserData)
#endif // TIMER_DEBUG
{
	ASSERT_GLOBAL_LOCK;
	...
	i32 slotsInFuture = (i32)((when - m_lastExec).GetSeconds() * TIMER_HERTZ);
	...
	{
		i32 slot = (m_curSlot + slotsInFuture) % TIMER_SLOTS;
...
uk CNetTimer::CancelTimer(NetTimerId& id)
{
	ASSERT_GLOBAL_LOCK;
	if (id <= 0 || id >= (i32)m_callbacks.size())
	{
		id = 0;
		return NULL;
	}
...
CTimeValue CNetTimer::Update()
{
	...
	if (elapsed > 60)
		...
		advanceSlots = (i32)(elapsed * TIMER_HERTZ);
#if TIMER_DEBUG
	i32 oldAdvanceSlots = advanceSlots;
#endif
...
#endif
	i32 endSlot = (m_curSlot + advanceSlots) % TIMER_SLOTS;
	for (; m_curSlot != endSlot; m_curSlot = (m_curSlot + 1) % TIMER_SLOTS)
	{
		...
		i32 id = m_timerCallbacks[m_curSlot];
		while (id != -1)
		{
	...
	{
		i32 id = m_slowCallbacks.begin()->second;
		m_execCallbacks.push_back(id);
	...
	for (TGlobalIntVector::iterator iter = m_execCallbacks.begin(); iter != m_execCallbacks.end(); ++iter)
	{
		...
		if (m_callbacks[id].cancelled)
			continue;
		m_callbacks[id].cancelled = true;
		uk pUserData = m_callbacks[id].pUserData;
...
#if TIMER_DEBUG
NetTimerId CAccurateNetTimer::AddTimer(CTimeValue when, NetTimerCallback callback, uk pUserData, tuk pFile, size_t line, tuk pName)
#else
NetTimerId CAccurateNetTimer::AddTimer(CTimeValue when, NetTimerCallback callback, uk pUserData)
#endif // TIMER_DEBUG
{
	ASSERT_GLOBAL_LOCK;
...
uk CAccurateNetTimer::CancelTimer(NetTimerId& id)
{
	ASSERT_GLOBAL_LOCK;
	uk pUser = NULL;
...
CTimeValue CAccurateNetTimer::Update()
{
	...
	static u32 maxCallbacks = 0;
	u32 activeCallbacks = 0;
#endif // defined(LOG_MAXIMUM_CALLBACKS)

	for (u32 index = 0; index < m_callbacks.size(); ++index)
	{
		if (m_callbacks[index].inUse)
		{
			...
			if (scheduled <= g_time)
			{
				uk pUserData = m_callbacks[index].pUserData;
