...
tukk GetNameForCompareFrequentNetMessageType(i32 id)
{
	switch (id)
	{
	case eCFNMT_TotSize:
...
void CNetworkInspector::AddMessage(tukk description, float sizeInBytes, float queuingLatency, tukk additionalText)
{
...
string CNetworkInspector::CreateOutputText(NetMessage& msg)
{
	...
	i32 descLength = dedicated ? 20 : 35;
	i32 targetLength = 30;

	string output;
	if (!dedicated)
		output.append("Message: ");
	i32 offset = msg.m_description.length() - descLength;
...
void CNetworkInspector::DumpToScreen()
{
	...
	if (pTMC)
	{
		tukk name = GetNameForCompareFrequentNetMessageType(CNetCVars::Get().NetInspector);
		if (!name)
			name = "none";
		char buf[128];
		drx_sprintf(buf, "Sort order: %s", name);
...
void CNetworkInspector::DrawLabel(float startPos, float col, i32 row, float* color, float glow, tukk szText, float fScale) const
{
	const float ColumnSize = COL_SIZE;
	const float RowSize = ROW_SIZE;

	char msg[256];
	drx_strcpy(msg, szText);
...
void CNetworkInspector::RefreshShowcase()
{
	...
	{
		u32 seconds = (u32)(m_lastDumpTime.GetSeconds() - (*frqIT).m_lastTime.GetSeconds());
		if (!seconds)
			continue;
		if (seconds > 5)
		{
