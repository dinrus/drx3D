...
{
public:
	CMementoInputStream(u32 id) : CByteInputStream((u8k*)MMM().PinHdl(id), MMM().GetHdlSize(id))
	{
	}
};
...
{
	// any data yet?
	return ctx.ctxObj.xtra->vAspectData[ctx.index] != CMementoMemoryUpr::InvalidHdl;
}
...
bool CMementoHistory::NeedToSync(const SSyncContext& ctx, CSyncContext* pSyncContext, const CRegularlySyncedItem& item)
{
	...
	u32 ackedVersion = ctx.pViewObjEx->ackedAspectVersions[ctx.index];
	u32 currentVersion = ctx.ctxObj.xtra->vAspectDataVersion[ctx.index];
	if (ackedVersion != currentVersion || ackedVersion == ~u32(0) || currentVersion == ~u32(0))
		return true;
	else if (ackedVersion != ~u32(0) && currentVersion != ~u32(0) && ackedVersion == currentVersion)
		return false;
	...
	in.GetTyped<u8>(); //skip first byte!
	u32 version = in.GetTyped<u32>();
	if (version != currentVersion)
		return true;

	NET_ASSERT((ctx.ctxObj.main->vAspectProfiles[ctx.index] & ~PROFILE_MASK) == 0);

	u8k* pBasisBuffer = (u8k*)MMM().PinHdl(pElem->data);
	u8k bonusByte = pBasisBuffer[0];
	u8 storedProfile = bonusByte & PROFILE_MASK;
...
bool CMementoHistory::SendCurrentValue(const SSendContext& ctx, CSyncContext* pSyncContext, u32& newValue)
{
...
#if ENABLE_DEBUG_KIT
	u8 bonusByte = POLLUTION_BIT;
	if (sp.pElem)
	{
		CByteInputStream curState((u8k*)MMM().PinHdl(sp.pElem->data), MMM().GetHdlSize(sp.pElem->data));
		bonusByte = curState.GetTyped<u8>();
	}
...
bool CMementoHistory::ReadCurrentValue(const SReceiveContext& ctx, bool commit)
{
	...
	u32 newHandle = CMementoMemoryUpr::InvalidHdl;
...
bool CMementoHistory::Serialize(const SSerializeParams& sp)
{
	...
	u32 usingBasisId = ~u32(0);
#endif

	TSerialize ser = sp.ser;
	u8 newProfile = sp.profile;
	...
	if (sp.pElem)
	{
		curState = CByteInputStream((u8k*)MMM().PinHdl(sp.pElem->data), MMM().GetHdlSize(sp.pElem->data));
		u8 bonusByte = curState.GetTyped<u8>();
	...
	newState.PutTyped<u8>() = newProfile + sp.isOwner * OWNER_BIT;
	CNetSerialize* pSer = GetNetSerializeImpl(sp.ser);
	u32 mementoAge = sp.ctx.currentSeq - sp.ctx.basisSeq;

	// skip over first 32 bit word of curstate if it's there
	if (curState.GetSize())
		curState.GetTyped<u32>();

	// store the version number for comparison later
	newState.PutTyped<u32>() = sp.ctx.ctxObj.xtra->vAspectDataVersion[sp.ctx.index];
...
void CMementoHistory::Destroy(u32 data)
{
	MMM().FreeHdl(data);
}
	...
	case eHE_Pollute:
		{
			...
			if (ctxObj.main)
			{
				...
				{
					*(u8*)MMM().PinHdl(pElem->data) |= POLLUTION_BIT;
					viter.Next();
				}
			}
		}
...
void CMementoReadSyncedItems::Destroy(u32 data)
{
	MMM().FreeHdl(data);
}
