...
bool CQuantizer::Load(XmlNodeRef node, const string& filename, const string& child, EFloatQuantizationMethod method, i32 bits)
{
	if (!node)
		return false;
	...
	if (XmlNodeRef params = node->findChild(child))
	{
		...
		tukk qm = params->getAttr("quantization");
		if (qm[0])
		{
...
u32 CQuantizer::Quantize(float nValue) const
{
	u32 quantizedValue = 0;
...
float CQuantizer::Dequantize(u32 nValue) const
{
