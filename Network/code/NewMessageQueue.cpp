...
static i32 m_messageQueueCount = 0;
...
public:
	...
	ILINE i32 BasicCompare(const SMsgEntOrderingInfo& a, const SMsgEntOrderingInfo& b)
	{
...
public:
	...
	ILINE bool operator()(u32 a, u32 b)
	{
...
ILINE i32 CMessageQueue::SMsgEntOrderingInfo::LatencyBucket() const
{
	return i32(latencyClass) * 2 + (latencyClass == eLC_DontCare ? bandwidthExceeded : 0);
}

bool CMessageQueue::CConfig::Read(XmlNodeRef n)
{
	for (i32 i = 0; i < n->getChildCount(); i++)
	{
		...
		if (0 == strcmp("Group", child->getTag()))
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_ERROR,
...
void CMessageQueue::CConfig::GetMemoryStatistics(IDrxSizer* pSizer)
{
...
CMessageQueue::CConfig* CMessageQueue::LoadConfig(tukk name)
{
...
static ILINE float ToPower(float x, i32 n)
{
	for (i32 i = 1; i < n; i++)
		x *= x;
	return x;
...
public:
	CIncrementalSorter(CMessageQueue* pQ) : m_liveList(pQ->m_liveList), m_pSlots(&pQ->m_slots)
	{
		...
		for (i32 i = 0; i < 2 * eLC_NUM_CLASSES; i++)
		{
			m_latencyBuckets[i] = u32(-1);
		}
		...
		{
			i32 bkt = (*m_pSlots)[pSlot->sortOrderingSlot].ordering.LatencyBucket();
	...
	u32 Next()
	{
		...
		while (m_curLiveListElem == m_liveList.size())
		{
			...
			while (m_curOrderingBucket == m_orderingBuckets.end())
			{
				...
				if (m_latencyBuckets[m_curLatencyBucket] == u32(-1))
					continue;

				m_orderingBuckets.clear();

				u32 cur = m_latencyBuckets[m_curLatencyBucket];
				while (cur + 1)
				{
					NET_ASSERT(cur < m_pSlots->size());
					u32 bkt = u32(10000.0f * (*m_pSlots)[(*m_pSlots)[cur].sortOrderingSlot].ordering.schedulingOrder + 0.5f);
					TBucketMap::iterator it = m_orderingBuckets.lower_bound(bkt);
					if (it == m_orderingBuckets.end() || it->first != bkt)
						it = m_orderingBuckets.insert(it, std::make_pair(bkt, u32(-1)));
					u32& bktHeadRef = it->second;
			...
			u32 cur = m_curOrderingBucket->second;
			while (cur + 1)
			{
				NET_ASSERT(cur < m_pSlots->size());
...
private:
	...
	typedef std::map<u32, u32, std::greater<u32>>                                                 TBucketMap;
	#else
	typedef std::map<u32, u32, std::greater<u32>, STLMementoAllocator<std::pair<u32k, u32>>> TBucketMap;
	#endif

	i32                    m_curLatencyBucket;
	TBucketMap::iterator   m_curOrderingBucket;
	i32                    m_curLiveListElem;

	static i32k       NUM_ORDERINGS = 256;

	u32                 m_latencyBuckets[2 * eLC_NUM_CLASSES];
	...
	, m_nAccountingGroups(0)
{
	...
	for (i32 i = 0; i < MAX_ACCOUNTING_GROUPS; i++)
		m_accountingGroups[i].id = ~u32(0);
	if (pConfig)
	{
		SetConfig(pConfig, CNetwork::Get()->GetMessageQueueConfigVersion());
	}

	for (i32 i = 0; i < eMSS_NUM_LIVE_SLOT_TYPES; i++)
	{
...
void CMessageQueue::GetMemoryStatistics(IDrxSizer* pSizer, bool countingThis /* = false */)
{
...
CMessageQueue::SMsgSlot* CMessageQueue::AllocSlot(SSendableHandle& hdl, EMsgSlotState initState)
{
	...
	bool found = false;
	for (u32 i = 0; i < m_freeSlotNumElems.size(); i++)
	{
	...
	{
		u32 rootId = m_rootSlots[initState].id;
	...
	{
		DrxFatalError("Invalid initial state for message slot %d", initState);
		return 0;
	}
...
void CMessageQueue::AddDependencyToSlot(SSendableHandle hdl, SMsgSlot* pSlot)
{
	...
	{
		for (u32 depNodeId = pParentSlot->msg.firstDepNodeId; depNodeId != InvalidDepNodeId; depNodeId = m_depNodes[depNodeId].next)
...
{
	for (u32 depNodeId = pSlot->msg.firstDepNodeId; depNodeId != InvalidDepNodeId; )
	{
		u32 next = m_depNodes[depNodeId].next;
...
EMessageQueueAddSendableResult CMessageQueue::AddSendable(INetSendable* pMsg, i32 nAfterHandle, const SSendableHandle* afterHandles, SSendableHandle* pHandle, bool subs)
{
	VerifyBlocking();
...
{
	for (u32 i = 0; i < m_nAccountingGroups; i++)
	{
...
void CMessageQueue::PrepareLiveList()
{
	...
	u32 activeRoot = m_rootSlots[eMSS_Active].id;
...
void CMessageQueue::CalculatePerFrameData(const SSchedulingParams& params)
{
	...
	while (SMsgSlot* pEnt = iter.Next())
	{
		...
		if (posInfo.obj)
		{
			u32 id = posInfo.obj.id;
			if (m_objectHeads.size() <= id)
				m_objectHeads.resize(id + 1, ~u32(0));
			u32& head = m_objectHeads[id];
			u32 cur = ent.sortOrderingSlot;
			u32& next = m_slotState[cur].nextObj;
			next = head;
			head = cur;
		}
		...
		for (u32 depNodeId = ent.msg.firstDepNodeId; depNodeId != InvalidDepNodeId; depNodeId = m_depNodes[depNodeId].next)
		{
...
void CMessageQueue::PatchObjectGroupings()
{
	...
	u32k* pStart = &*m_objectHeads.begin();
	u32k* pEnd = pStart + m_objectHeads.size();
	while (pStart != pEnd)
	{
		if (*pStart != ~u32(0))
		{
...
void CMessageQueue::PatchOrderedPriorities()
{
	...
	u32 activeRoot = m_rootSlots[eMSS_Active].id;
	u32 curTop = m_slotState[activeRoot].nextTop;
	...
	while (!m_recurseCache.empty())
	{
		...
		for (u32 depNodeId = pChildEnt->msg.firstDepNodeId; depNodeId != InvalidDepNodeId; depNodeId = m_depNodes[depNodeId].next)
		{
...
bool CMessageQueue::AreMessagesToWrite(const SSchedulingParams& params)
{
	...
	while (true)
	{
		...
		for (u32 depNodeId = pEntSend->msg.firstDepNodeId; depNodeId != InvalidDepNodeId; depNodeId = m_depNodes[depNodeId].next)
		{
			SSendableHandle parentHdl = m_depNodes[depNodeId].hdl;
...
u32 g_DBASizePriorToUpdate = 0;
DrxStringLocal g_DBAMainProfileBuffer;
DrxStringLocal g_DBASmallProfileBuffer;
DrxStringLocal g_DBALargeProfileBuffer;
...
struct SNetMessageProfileLogger
{
	...
	void OnMessage(CNetChannel* pNC, u32 group, tukk message, float size)
	{
		OnMessage(msg_profile, pNC, message, size);
		OnMessage(grp_profile, pNC, KeyToString(group).c_str(), size);
	}

	static void OnMessage(TProfile& profile, CNetChannel* pNC, tukk message, float size)
	{
	...
	static FILE* OpenLogFile(tukk name, i32* firstTime)
	{
		FILE* fout = NULL;

		if (*firstTime)
		{
			fout = gEnv->pDrxPak->FOpen(name, "w+");
			tukk header = "id\tprefix\tchannel\tmessage\tcount\tmisize\tmaxsize\ttotalsize\tavgsize\n";
			fprintf(fout, header);
			* firstTime = 0;
		}
		else
		{
			fout = gEnv->pDrxPak->FOpen(name, "a+");
		}
		return fout;
	}
	...
	void DumpSocketStatistics(FILE* fout, tukk prefix)
	{
		// sck line slightly different - dumpcount,prefix,bandwidth,packets

		i32 packetChange = g_socketBandwidth.bandwidthStats.m_total.m_totalPacketsSent - lastPacketsSent;
		u32 bandwidthChange = (u32)(g_socketBandwidth.bandwidthStats.m_total.m_totalBandwidthSent - lastBandwidthSent);
	...
	void DumpProfile(const TProfile& profile, FILE* fout, tukk prefix)
	{
		uint total = 0;
...
void CMessageQueue::WriteMessages(IMessageOutput* pOut, const SSchedulingParams& params)
{
	...
	u32 newReliableSeq = m_reliableSeq;
	...
	while (true)
	{
		...
		if (!writtenHeader)
		{
			...
			IF_UNLIKELY (DBAEnabled)
			{
				u32 sizeHeader = (u32)pStm->GetBitSize();
		...
		u32 sizeBefore = pStm->GetApproximateSize();
	#if DEEP_BANDWIDTH_ANALYSIS
		...
		if (doWrite)
		{
			//DRX_PROFILE_REGION(PROFILE_NETWORK, "CMessageQueue::WriteMessages.EncodeMessage");
		...
		u32 sizeAfter = pStm->GetApproximateSize();
	#if DEEP_BANDWIDTH_ANALYSIS
		IF_UNLIKELY (DBAEnabled)
		{
	...
	bool logMessageQueueUnsent = (isLocal == false) && (CNetCVars::Get().net_logMessageQueue > 1) && ((idxEntSend != u32(-1)) || areDelayedMessages);
	...
	tukk channelName = (m_pParent != NULL) ? m_pParent->GetName() : "<unknown>";
	if (logMessages)
	{
		if (label != NULL)
		{
	...
	while (idxEntSend != u32(-1))
	{
		...
		if (logMessages)
		{
			...
			u32 calcSlot = pMsg - &*m_slots.begin();
...
void CMessageQueue::FinishFrame(const SSchedulingParams* pParams)
{
	...
	static std::vector<std::pair<u32, EMsgSlotState>> changeStates;
	...
	while (SMsgSlot* pSlot = iter.Next())
	{
		if (pSlot->liveness == eL_Rotten || pSlot->liveness == eL_Discarded)
		{
			...
			changeStates.push_back(std::make_pair(u32(pSlot - &*m_slots.begin()), eMSS_Dead));
		}
		else if (pSlot->liveness == eL_Sent)
		{
			changeStates.push_back(std::make_pair(u32(pSlot - &*m_slots.begin()), eMSS_Waiting));
		}
	}
	iter = CActiveElemIterator(this, eMSS_JustQueued);
	while (SMsgSlot* pSlot = iter.Next())
	{
		changeStates.push_back(std::make_pair(u32(pSlot - &*m_slots.begin()), eMSS_Active));
	}
...
void CMessageQueue::SetConfig(CConfig* pConfig, i32 version)
{
	m_nAccountingGroups = 0;
	for (std::map<u32, SAccountingGroupPolicy>::iterator iter = pConfig->m_policy.begin(); iter != pConfig->m_policy.end(); ++iter)
	{
		NET_ASSERT(iter->first != ~u32(0));
	...
	if (m_nAccountingGroups > 0)
	{
		...
		for (u32 index = 0; index < m_nAccountingGroups; ++index)
		{
...
ILINE CMessageQueue::SAccountingGroup* CMessageQueue::GetAccountingGroup(u32 grpId)
{
	u32 count = m_nAccountingGroups;
...
void CMessageQueue::RegularCleanup(const SSchedulingParams& params)
{
	...
	i32 curVersion = CNetwork::Get()->GetMessageQueueConfigVersion();
...
void CMessageQueue::FlushObjectHeads()
{
	if (!m_objectHeads.empty())
	{
		std::fill(m_objectHeads.begin(), m_objectHeads.end(), ~u32(0));
	}
}
...
void CMessageQueue::AckMessages(SSendableHandle* pMsgs, size_t nMsgs, u32 nSeq, bool ack, bool clear)
{
	VerifyBlocking();
...
{
	std::vector<u32> mySlots;
	for (i32 type = 0; type < eMSS_NUM_LIVE_SLOT_TYPES; type++)
	{
...
static i32 GetIndentForMessage(i32 depth)
{
	/*
	   NET_ASSERT(depth <= 0);
	   depth = -depth;
	   return i32(0.5f+100*expf(-0.10536f*depth));
	 */
	return 0;
}
...
void CMessageQueue::DrawLabel(float x, float y, float* clr, tukk msg, ...)
{
	va_list args;
	va_start(args, msg);
	char buffer[4096];
	drx_vsprintf(buffer, msg, args);
	...
	if (t)
		t->PutText((i32)(x / (drawScale * 4.0f)) - 2, (i32)(y / (drawScale * 10.0f)) - 1, buffer);

	va_end(args);
}

tukk CMessageQueue::DebugMessageForHandle(SSendableHandle hdl)
{
...
void CMessageQueue::SetState(u32 slot, EMsgSlotState state)
{
	...
	if (slotState.state >= eMSS_NUM_LIVE_SLOT_TYPES)
		DrxFatalError("Cannot change state from the special states");
	else if (state >= eMSS_NUM_LIVE_SLOT_TYPES)
		DrxFatalError("Cannot change state to the special states");
	if (slotState.state != state)
	{
		m_slotState[slotState.prev].next = slotState.next;
		m_slotState[slotState.next].prev = slotState.prev;

		u32 rootId = m_rootSlots[state].id;
		slotState.next = rootId;
		slotState.prev = m_slotState[rootId].prev;
...
void CMessageQueue::GetBandwidthStatistics(u32 channelIndex, SBandwidthStats* pStats)
{
	...
	while (accountingGroupIndex < m_nAccountingGroups)
	{
		...
		stats.m_inUse = true;
		drx_strcpy(stats.m_name, KeyToString(accountingGroup.id).c_str());
		...
		stats.m_priority = static_cast<u32>(accountingGroup.policy.priority);
...
void CMessageQueue::DebugDrawAccountingGroups(const SSchedulingParams& params)
{
	...
	for (u32 i = 0; i < m_nAccountingGroups; i++)
	{
		...
		bool flash = ((static_cast<i32>(params.now.GetSeconds() * 4.0f)) & 0x01);
	...
	while (true)
	{
		...
		i32 indent = GetIndentForMessage(pMsg->depth);
		...
		DrxFixedStringT<512> msglbl;
		DrxFixedStringT<512> tmpstr;
		...
		{
			msglbl += " (";
			for (u32 depNodeId = pMsg->msg.firstDepNodeId; depNodeId != InvalidDepNodeId; depNodeId = m_depNodes[depNodeId].next)
...
void CMessageQueue::DebugDrawSchedulerSends(const SSchedulingParams& params)
{
	...
	for (u32 groupIndex = 0; groupIndex < m_nAccountingGroups; ++groupIndex)
	{
