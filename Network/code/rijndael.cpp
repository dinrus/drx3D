...
// Author : Szymon Stefanek (stefanek<at>tin.it)
//
...
i32 Rijndael::init(Mode mode, Direction dir, const UINT8* key, KeyLength keyLen, UINT8* initVector)
{
	// Not initialized yet
	m_state = Invalid;
...
i32 Rijndael::blockEncrypt(const UINT8* input, i32 inputLen, UINT8* outBuffer)
{
	i32 i, k, numBlocks;
...
i32 Rijndael::padEncrypt(const UINT8* input, i32 inputOctets, UINT8* outBuffer)
{
	i32 i, numBlocks, padLen;
	UINT8 block[16], * iv;
...
i32 Rijndael::blockDecrypt(const UINT8* input, i32 inputLen, UINT8* outBuffer)
{
	i32 i, k, numBlocks;
	UINT8 block[16], iv[4][4];
...
i32 Rijndael::padDecrypt(const UINT8* input, i32 inputOctets, UINT8* outBuffer)
{
	i32 i, numBlocks, padLen;
	UINT8 block[16];
	UINT32 iv[4];
...
void Rijndael::keySched(UINT8 key[_MAX_KEY_COLUMNS][4])
{
	...
	for (j = 0; (j < (i32)uKeyColumns) && (r <= (i32)m_uRounds); )
	{
	...
	assert(uKeyColumns > 1);
	PREFAST_ASSUME(uKeyColumns > 1);
	while (r <= m_uRounds)
	{
