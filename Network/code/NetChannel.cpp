...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  реализует коммуникации между двумя машинами, с
...
#include  <drx3D/Network/FixedSizeArena.h>
...
CNetChannel::CNetChannel(const TNetAddress& ipRemote, const CExponentialKeyExchange::KEY_TYPE& key, u32 remoteSocketCaps, DrxSessionHandle session) :
	m_privateKey(key),
	m_ip(ipRemote),
	...
	m_statsTimer(0),
	m_pMMM(new CMementoMemoryUpr(string().Format("NetChannel[%s]", ((CNetwork*)gEnv->pNetwork)->GetResolver()->ToNumericString(ipRemote).c_str()))),
	m_pGameChannel(NULL),
	m_pContextView(NULL),
	m_pNub(NULL),
#if ENABLE_DEBUG_KIT
...
{
	SCOPED_GLOBAL_LOCK;
	DRX_ASSERT_MESSAGE(!m_pContextView, "Error: setting the channel type twice.");
	DRX_ASSERT_MESSAGE(pNetContext, "Must set a context to make a client.");
...
void CNetChannel::SetServer(INetContext* pNetContext)
{
	SCOPED_GLOBAL_LOCK;
	DRX_ASSERT_MESSAGE(!m_pContextView, "Error: setting the channel type twice.");
	DRX_ASSERT_MESSAGE(pNetContext, "Must set a context to make a server.");
...
bool CNetChannel::GetRemoteNetAddress(u32& uip, u16& port, bool firstLocal)
{
	TNetAddress ip = m_ip;
	if (IsLocal())
	{
...
void CNetChannel::GetBandwidthStatistics(u32 channelIndex, SBandwidthStats* const pStats)
{
	SNetChannelStats& stats = pStats->m_channel[channelIndex];
	stats.m_inUse = true;

	tukk nickName = GetNickname();
	if (nickName == NULL)
	{
		nickName = "<no name>";
	}
	...
	DrxFixedStringT<STATS_MAX_NAME_SIZE> name;
	name.Format("%s [%c]", nickName, type);
	drx_strcpy(stats.m_name, name.c_str());

	m_ctpEndpoint.GetBandwidthStatistics(channelIndex, pStats);
}

tukk CNetChannel::GetName()
{
...
void CNetChannel::Disconnect(EDisconnectionCause cause, tukk fmt, ...)
{
	SCOPED_GLOBAL_LOCK;
...
bool CNetChannel::AddSendable(INetSendablePtr pSendable, i32 numAfterHandle, const SSendableHandle* afterHandle, SSendableHandle* handle)
{
	SCOPED_GLOBAL_LOCK;
	return NetAddSendable(pSendable, numAfterHandle, afterHandle, handle);
}

bool CNetChannel::SubstituteSendable(INetSendablePtr pSendable, i32 numAfterHandle, const SSendableHandle* afterHandle, SSendableHandle* handle)
{
	SCOPED_GLOBAL_LOCK;
...
void CNetChannel::SetPassword(tukk password)
{
	SCOPED_GLOBAL_LOCK;
	if (m_pContextView)
...
static CFixedSizeArena<sizeof(uk )*8 + sizeof(CTimeValue)*2, 64> m_pingpongarena;
...
public:
	...
	virtual tukk GetDescription()                                             { return "Ping"; }
...
public:
	...
	virtual void        UpdateState(u32 nFromSeq, ENetSendableStateUpdate update) {}
	virtual tukk GetDescription()                                             { return "Pong"; }
	virtual void        GetPositionInfo(SMessagePositionInfo& pos)                   {}
...
void CNetChannel::TimerCallback(NetTimerId id, uk pUser, CTimeValue time)
{
...
#if TIMER_DEBUG
			char buffer[128];
			drx_sprintf(buffer, "CNetChannel::TimerCallback() m_statsTimer [%s][%p]", pThis->GetNickname(), pThis);
#else
			tuk buffer = NULL;
#endif // TIMER_DEBUG
...
CTimeValue CNetChannel::GetInactivityTimeout(bool backingOff)
{
	...
	inactivityTimeout += 60.0f * CNetwork::Get()->GetExternalSocketIOUpr().HasPendingData();
	inactivityTimeout += 60.0f * (i32)backingOff;
...
void CNetChannel::Update(bool finalUpdate, CTimeValue nTime)
{
	...
	if (gEnv->pLobby == NULL)
	{
		// If DrxLobby is available, it will deal with timeouts and disconnects
		...
		if (CNetCVars::Get().BackoffTimeout && backingOff && backoffTime.GetSeconds() > CNetCVars::Get().BackoffTimeout)
		{
			...
			char buf[256];
			drx_sprintf(buf, "Zombie kicked; remote machine was backing off for %.1f seconds", idleTime);
		...
		if (m_nKeepAliveTimer != CTimeValue(0.0f) && !IsLocal())
		{
			...
			{
				char buf[256];
				drx_sprintf(buf, "Timeout occurred; no packet for %.1f seconds", idleTime.GetSeconds());
				Disconnect(eDC_Timeout, buf);
			}
		}
	}
...
void CNetChannel::UpdateTimer(CTimeValue time, bool forceUpdate)
{
	...
	#if TIMER_DEBUG
			char buffer[128];
			drx_sprintf(buffer, "CNetChannel::UpdateTimer() m_timer [%s][%p]", GetNickname(), this);
	#else
			tuk buffer = NULL;
	#endif // TIMER_DEBUG
			TIMER.CancelTimer(m_timer);
...
#if TIMER_DEBUG
	char buffer[128];
	drx_sprintf(buffer, "CNetChannel::Init() m_statsTimer [%s][%p]", GetNickname(), this);
#else
	tuk buffer = NULL;
#endif // TIMER_DEBUG
...
void CNetChannel::SyncWithGame(ENetworkGameSync type)
{
	...
	if (pSchedDebug)
	{
		tukk v = pSchedDebug->GetString();
		if (v[0] == '0' && v[1] == '\0')
			;
		else if (DrxStringUtils::stristr(GetName(), v))
...
void CNetChannel::ProcessPacket(bool forceWaitForSync, u8k* pData, u32 nLength)
{
	MMM_REGION(m_pMMM);
	...
	{
		for (i32 i = 0; i < LOAD_NETWORK_COUNT; i++)
		{
			AddLoadMsg();
		}
	}
...
void CNetChannel::GetMemoryStatistics(IDrxSizer* pSizer, bool countingThis)
{
...
i32 CNetChannel::GetContextViewStateDebugCode() const
{
	if (m_pContextView)
...
void CNetChannel::GC_AddPingReadout(float elapsedRemote, float sinceSent, float ping)
{
	...
	if (!m_pDebugHistory)
	{
		m_pDebugHistory = gEnv->pGameFramework->CreateDebugHistoryUpr();
	}
...
i32 CNetChannel::GetProfileId() const
{
	return m_profileId;
}
...
void CNetChannel::SetProfileId(i32 id)
{
	m_profileId = id;
	;
}

void CNetChannel::SetNickname(tukk name)
{
	m_nickname = name;
}
...
void CNetChannel::SetCDKeyHash(tukk hash)
{
	m_CDKeyHash = hash;
}

DrxSessionHandle CNetChannel::GetSession() const
{
	return m_session;
}
...
void CNetChannel::LogRMIBenchmark(ERMIBenchmarkAction action, const SRMIBenchmarkParams& params, void (* pCallback)(ERMIBenchmarkLogPoint point0, ERMIBenchmarkLogPoint point1, int64 delay, uk pUserData), uk pUserData)
{
	SRMIBenchmarkRecord& record = m_RMIBenchmarkRecords[params.seq];
	u32 point = (params.message << 2) | action;
