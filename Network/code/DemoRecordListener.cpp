...
bool CDemoRecordListener::CDemoRecorderChannel::AddSendable(INetSendablePtr pSendable, i32 numAfterHandle, const SSendableHandle* afterHandle, SSendableHandle* handle)
{
...
static std::unique_ptr<CSimpleOutputStream> CreateOutput(tukk filename)
{
	...
	drx_sprintf(buf, "_Build%04d_", ver.v[0]);
	path += buf;
...
CDemoRecordListener::CDemoRecordListener(CNetContext* pContext, tukk filename) :
...
void CDemoRecordListener::DoUpdateObject(const SContextObjectRef& obj, NetworkAspectType aspects)
{
	...
	for (NetworkAspectID i = 0; i < NumAspects; i++)
	{
		...
		if (aspect & aspects)
		{
			u8 profile = obj.main->vAspectProfiles[i];
...
void CDemoRecordListener::DoSetAspectProfile(SNetObjectID id, NetworkAspectType aspects, u8 profile)
{
	SCOPED_GLOBAL_LOCK;
