...
#include <drx3D/Plugins/BigDigits/bigd.h>
#include <drx3D/Plugins/BigDigits/bigdigits.h>
#include <drx3D/Plugins/BigDigits/bigdRand.h>
// big digits defines uint32_t as u64, which breaks compilation
#undef uint32_t
...
i32 CExponentialKeyExchange::RandFunc(u8* bytes, size_t nbytes, u8k* seed, size_t slen)
{
	NET_ASSERT(slen % sizeof(u32) == 0);
	CMTRand_int32 r((u32k*)seed, slen / sizeof(u32));
	while (nbytes >= sizeof(u32))
	{
		*(u32*)bytes = r.GenerateUint32();
		bytes += sizeof(u32);
		nbytes -= sizeof(u32);
	}
	if (nbytes)
	{
		u32 last = r.GenerateUint32();
		u8* p = (u8*)&last;
		while (nbytes--)
			*bytes++ = *p++;
	}
...
regenerate:         // LH : Should be safe to simply regenerate a new key
	...
	CAutoBIGD a;
	bdRandomSeeded(a.Get(), KEY_SIZE_IN_BITS, (u8*)&seed, sizeof(seed), RandFunc);
	...
	CAutoBIGD p;
	seed += 13;
	bdGeneratePrime(p.Get(), KEY_SIZE_IN_BITS, 5, (u8*)&seed, sizeof(seed), RandFunc);
	...
	CAutoBIGD q;
	seed += 13;
	bdGeneratePrime(q.Get(), KEY_SIZE_IN_BITS, 5, (u8*)&seed, sizeof(seed), RandFunc);
...
regenerate:     // LH : Should be safe to simply regenerate a new key
	...
	// generate private key
	CAutoBIGD b;
	bdRandomSeeded(b.Get(), KEY_SIZE_IN_BITS, (u8*)&seed, sizeof(seed), RandFunc);
