...
u32 CInternetSimulatorSocket::sm_nProfiles = 0;
...
void CInternetSimulatorSocket::SimulatorUpdate(NetTimerId tid, uk p, CTimeValue t)
{
...
ESocketError CInternetSimulatorSocket::Send(u8k* pBuffer, size_t nLength, const TNetAddress& to)
{
	...
	g_socketBandwidth.simLastPacketLag = (u32)(fExtraTime * 1000.0f);
...
ESocketError CInternetSimulatorSocket::SendVoice(u8k* pBuffer, size_t nLength, const TNetAddress& to)
{
	if (stl::get_if<TLocalNetAddress>(&to))
	{
...
DRXSOCKET CInternetSimulatorSocket::GetSysSocket()
{
...
{
	DrxLog("[LagProfiles] %u lag profiles available.", sm_nProfiles);
	...
	case PROFILE_PERFECT:
		{
			...
			DrxLog("[LagProfiles] Using Perfect profile settings.");
		}
		break;
	case PROFILE_FATAL:
		{
			...
			DrxLog("[LagProfiles] Using Fatal profile settings.");
		}
		break;
	case PROFILE_RANDOM:
		{
			profile = (EProfile)(s_InternetSimRandomGen.GenerateUint32() % sm_nProfiles);
			DrxLog("[LagProfiles] Picking Random profile %d of %u.", profile, sm_nProfiles);
			// deliberate fall through to default case
		}
	default:
		{
			if ((profile > PROFILE_PERFECT) && (profile < (i32)sm_nProfiles))
			{
				...
				DrxLog("[LagProfiles] Using Profile %d settings. (%.2f%% chance of packet loss, %.2f-%.2f sec variable lag)", profile, fLoss, pEntry->fLagMin, pEntry->fLagMax);
			}
			else
			{
				DrxLogAlways("[LagProfiles] Invalid profile index.");
			}
		}
		break;
	}
...
void CInternetSimulatorSocket::LoadXMLProfiles(tukk pFileName)
{
	if (sm_pProfiles && (sm_pProfiles != sm_DefaultProfiles))
	{
		SAFE_DELETE_ARRAY(sm_pProfiles);
	...
	if (pFileName[0] != 0)
	{
		...
		if (rootNode)
		{
			...
			if (nProfileCount > 0)
			{
				...
				DrxLog("[LagProfiles] \"%s\" file parsed. %u lag profiles loaded.", pFileName, sm_nProfiles);
				return;
			}
		}
	}

	DrxLog("[LagProfiles] Could not load \"%s\". %u default profiles activated.", pFileName, sm_nProfiles);
}
