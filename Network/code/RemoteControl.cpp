...
void CRemoteControlServer::Start(u16 serverPort, const string& password, IRemoteControlServerListener* pListener)
{
...
void CRemoteControlServer::SendResult(u32 commandId, const string& result)
{
...
void CRemoteControlServerInternal::GetBindAddress(TNetAddress& addr, u16 port)
{
...
void CRemoteControlServerInternal::Start(u16 serverPort, string password)
{
...
void CRemoteControlServerInternal::AuthenticationTimeoutTimer(NetTimerId id, uk p, CTimeValue)
{
...
void CRemoteControlServerInternal::SendResult(u32 commandId, string result)
{
	...
	m_pSocketSession->Send((u8*)&msg_result, sizeof(msg_result));
	m_pSocketSession->Send((u8k*)result.data(), result.size());
}

void CRemoteControlServerInternal::OnConnectionAccepted(IStreamSocketPtr pStreamSocket, uk pUserData)
{
	if (m_pSocketSession)
	{
		SRCONServerMsgInSession insession;
		pStreamSocket->Send((u8*)&insession, sizeof(insession));
		pStreamSocket->Close();
	}
	else
	{
		...
		for (size_t i = 0; i < 4; ++i)
			*(u32*)(m_challenge.challenge + i * sizeof(u32)) = r.GenerateUint32();
		m_pSocketSession->Send((u8*)&m_challenge, sizeof(m_challenge));
...
void CRemoteControlServerInternal::OnConnectionClosed(bool graceful, uk pUserData)
{
	if (m_pSocketSession)
	{
		m_pSocketSession->Close();
...
void CRemoteControlServerInternal::OnIncomingData(u8k* pData, size_t nSize, uk pUserData)
{
	u8k* pBuffer = pData;
	...
	while (nLength > 0)
	{
		...
		if (m_amountRemaining == 0)
		{
			...
			else if (m_receivingState == eRS_ReceivingBody)
			{
				...
				case RCONCLIENTMSGTYPE_MD5DIGEST:
					{
						std::vector<u8> temp(sizeof(m_challenge.challenge) + m_password.size());
						u8* p = &temp[0];
						size_t n = temp.size();
						...
						else
						{
							...
							m_pSocketSession->Send((u8*)&authorized, sizeof(authorized));
				...
				case RCONCLIENTMSGTYPE_RCONCOMMAND:
					{
						u8* p = &m_receivingBody[0];
						size_t n = m_receivingBody.size();
						u32 commandId = *(u32*)p;
						p += sizeof(commandId);
						n -= sizeof(commandId);
						string command((tuk)p, n);
...
void CRemoteControlClient::Connect(const string& serverAddr, u16 serverPort, const string& password, IRemoteControlClientListener* pListener)
{
	m_pListener = pListener;
...
u32 CRemoteControlClient::SendCommand(const string& command)
{
	u32 commandId = rgen.GenerateUint32();
	while (commandId == 0)
...
void CRemoteControlClientInternal::Connect(string serverAddr, u16 serverPort, string password)
{
...
void CRemoteControlClientInternal::SendCommand(u32 commandId, string command)
{
	if (!m_pSocketSession || m_sessionState != eSS_Authorized)
		return;
	SRCONClientMsgRConCommand msg_command;
	msg_command.commandId = commandId;
	drx_strcpy((tuk)msg_command.command, sizeof(msg_command.command), command.c_str());
	m_pSocketSession->Send((u8*)&msg_command, sizeof(msg_command));
	m_pendingCommands[commandId] = command;
}

void CRemoteControlClientInternal::OnConnectCompleted(bool succeeded, uk pUserData)
{
	if (succeeded)
	{
...
void CRemoteControlClientInternal::OnConnectionClosed(bool graceful, uk pUserData)
{
	TO_GAME(&CRemoteControlClientInternal::Disconnect, this);
	TO_GAME(&IRemoteControlClientListener::OnSessionStatus, m_pClient->m_pListener,
...
void CRemoteControlClientInternal::OnIncomingData(u8k* pData, size_t nSize, uk pUserData)
{
	u8k* pBuffer = pData;
	size_t nLength = nSize;
				...
				case RCONSERVERMSGTYPE_CHALLENGE:
					{
						...
						u8* challenge = &m_receivingBody[0];
						size_t chalength = m_receivingBody.size();
						std::vector<u8> temp(chalength + m_password.size());
						u8* p = &temp[0];
						size_t n = temp.size();
						...
						m_pSocketSession->Send((u8*)&msg_digest, sizeof(msg_digest));
				...
				case RCONSERVERMSGTYPE_RCONRESULT:
					{
						...
						{
							m_resultCommandId = *(u32*)p;
							p += sizeof(u32);
							m_amountRemaining = *(u32*)p;
							m_receivingBody.resize(m_amountRemaining);
							m_bufferIndicator = 0;
						...
						string result((tuk)p, n);
						std::map<u32, string>::iterator itor = m_pendingCommands.find(m_resultCommandId);
