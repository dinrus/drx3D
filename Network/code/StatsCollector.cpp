...
#include <drx3D/Sys/ITextModeConsole.h>
	...
	#include  <drx3D/Network/DebugKit/DebugKit.h>
#endif

#if STATS_COLLECTOR

static i32k STATS_FILE_SIZE_BYTES = 1024 * 1024;
static i32k STATS_FILE_AMOUNT = 100;
...
CStatsCollector::CStatsCollector(tukk filename)
	#if STATS_COLLECTOR_DEBUG_KIT
	: m_debugKit(false)
	#endif
{
...
void CStatsCollector::WriteToLogFile(tukk format, ...)
{
	if (!m_file)
		return;
...
void CStatsCollector::BeginPacket(const TNetAddress& to, CTimeValue nTime, u32 id)
{
...
void CStatsCollector::AddOverheadBits(tukk describe, float bits)
{
...
void CStatsCollector::Message(tukk message, float bits)
{
	#if 0//STATS_COLLECTOR_DEBUG_KIT
	if (m_debugKit)
	{
...
void CStatsCollector::BeginGroup(tukk name)
{
	#if 0//STATS_COLLECTOR_DEBUG_KIT
	if (m_debugKit)
	{
...
void CStatsCollector::AddData(tukk name, float bits)
{
	#if STATS_COLLECTOR_DEBUG_KIT
	if (m_debugKit)
	{
...
void CStatsCollector::EndPacket(u32 nSize)
{
	#if STATS_COLLECTOR_DEBUG_KIT
	if (m_debugKit)
	{
...
void CStatsCollector::LostPacket(u32 id)
{
	#if 0//STATS_COLLECTOR_DEBUG_KIT
	if (m_debugKit)
	{
...
void CStatsCollector::DrawLine(i32 line, tukk fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	char buf[512];
	drx_vsprintf(buf, fmt, args);
	va_end(args);
