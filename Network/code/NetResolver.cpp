...
public:
	...
	{
		char buf[128];
		drx_sprintf(buf, "%s:%u", LOCAL_CONNECTION_STRING, addr);
		m_result = buf;
	}
	...
	void Visit(SSockAddrStorageAddr addr)
	{
		...
		default:
			DrxFatalError("CResolverToNumericStringVisitor: SSockAddrStorageAddr Unknown address family %u", addr.addr.ss_family);
		}
	}
#endif
	...
	{
		u32 a = (addr.addr >> 24) & 0xff;
		u32 b = (addr.addr >> 16) & 0xff;
		u32 c = (addr.addr >> 8) & 0xff;
		u32 d = (addr.addr) & 0xff;
...
{
public:
	CResolverToNumericStringVisitor_LocalBuffer(tuk result) : m_result(result) {}
	...
	void Visit(SSockAddrStorageAddr addr)
	{
		...
		default:
			DrxFatalError("CResolverToNumericStringVisitor_LocalBuffer: SSockAddrStorageAddr Unknown address family %u", addr.addr.ss_family);
		}
	}
#endif
...
public:
	...
	{
		char buf[128];
		drx_sprintf(buf, "%s:%d", LOCAL_CONNECTION_STRING, addr);
		m_result = buf;
		m_ok = true;
	}
	...
	void Visit(SIPv4Addr addr)
	{
		...
		m_ok = VisitIP((DRXSOCKADDR*)&saddr, sizeof(saddr), false);
	}
...
private:
	...
	bool VisitIP(const DRXSOCKADDR* pAddr, size_t len, bool bracket)
	{
		...
		i32 rv = getnameinfo(pAddr, len, host, NI_MAXHOST, serv, NI_MAXSERV, NI_DGRAM | NI_NUMERICSERV);
		if (0 != rv)
		{
		...
		{
			drx_sprintf(addr, bracket ? "[%s]:%s" : "%s:%s", host, serv);
			m_result = addr;
			return true;
		}
	#endif
...
bool CNetAddressResolver::SlowLookupName(TAddressString str, TNetAddressVec& addrs)
{
	...
	else
	{
		...
		else
		{
			...
			i32 resInfo = getaddrinfo(str.c_str(), port.empty() ? NULL : port.c_str(), &hints, &res);
			if (resInfo == 0)
			{
				SIPv4Addr addr;
				inaddr.s_addr = ((DRXSOCKADDR_IN*)(res->ai_addr))->sin_addr.s_addr;
				addr.addr = ntohl(inaddr.s_addr);
				addr.port = portNum;
				addrs.push_back(TNetAddress(addr));
				freeaddrinfo(res);
	...
	#else
			DRXHOSTENT* host = gethostbyname(str.c_str());

			if (host != NULL)
			{
				SIPv4Addr addr;
				inaddr.s_addr = *(u64*) host->h_addr_list[0];
				addr.addr = ntohl(inaddr.s_addr);
				addr.port = portNum;
				addrs.push_back(TNetAddress(addr));
			}
		...
		hints.ai_socktype = SOCK_DGRAM;
		i32 err = getaddrinfo(str.c_str(), port.empty() ? NULL : port.c_str(), &hints, &result);
		if (!err)
		{
			for (struct addrinfo* p = result; p; p = p->ai_next)
			{
				...
				case AF_INET:
					{
						DRXSOCKADDR_IN* pAddr = (DRXSOCKADDR_IN*) p->ai_addr;
						SIPv4Addr addr;
		...
		{
			SIPv4Addr addr;
			DRXINADDR_T addr32 = DrxSock::GetAddrForHostOrIP(str.c_str(), 0);
...
public:
	...
	void Visit(SIPv4Addr addr)
	{
		u32 ip = addr.addr;   // in host byte order, should NOT be in net byte order, since the masks below are all in host byte order
		m_private =
...
bool ConvertAddr(const TNetAddress& in, DRXSOCKADDR_IN* pOut)
{
	NET_ASSERT(pOut);
...
TNetAddress ConvertAddr(const DRXSOCKADDR* pSockAddr, i32 addrLength)
{
	...
	case AF_INET:
		{
			DRXSOCKADDR_IN* pSA = (DRXSOCKADDR_IN*)pSockAddr;
			SIPv4Addr addr;
	...
	default:
		DrxLog("Unknown address length: %d", addrLength);
		break;
	}
...
{
public:
	CConvertAddrVisitor(DRXSOCKADDR* pOut, i32* pOutLen) : m_pOut(pOut), m_pOutLen(pOutLen), m_result(false) {}
	...
	void Visit(SIPv4Addr addr)
	{
		if (*m_pOutLen < sizeof(DRXSOCKADDR_IN))
			return;
		DRXSOCKADDR_IN* pOut = (DRXSOCKADDR_IN*)m_pOut;
		memset(pOut, 0, sizeof(*pOut));
		pOut->sin_family = AF_INET;
		S_ADDR_IP4(*pOut) = htonl(addr.addr);
		pOut->sin_port = htons(addr.port);
		*m_pOutLen = sizeof(DRXSOCKADDR_IN);
		m_result = true;
	}
...
{
	if (!gEnv->pThreadUpr->SpawnThread(this, "NetAddressSolver"))
	{
		DrxFatalError("Error spawning \"NetAddressSolver\" thread.");
	}
}
...
{
	DrxAutoLock<TLock> lkP(m_parentLock);
...
CNameRequest::CNameRequest(const string& name, TNameRequestLock& parentLock, DrxConditionVariable& parentCond) :
...
bool CNameRequest::TimedWait(float seconds)
{
	...
	if (m_state == eNRR_Pending)
		done = m_parentCond.TimedWait(m_parentLock, u32(1000 * seconds));
	return done;
}
...
CToStringRequest::CToStringRequest(const TNetAddress& name, TNameRequestLock& parentLock, DrxConditionVariable& parentCond) :
	m_addr(name),
	m_state(eNRR_Pending),
	m_parentLock(parentLock),
	m_parentCond(parentCond)
