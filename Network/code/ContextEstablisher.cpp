...
bool CContextEstablisher::StepTo(SContextEstablishState& state)
{
	...
	while (m_offset < (i32)m_tasks.size() && m_tasks[m_offset].state <= state.contextState)
	{
#ifndef _RELEASE
...
void CContextEstablisher::Fail(EDisconnectionCause cause, const string& reason)
{
	...
	if (!gEnv->bMultiplayer)
		DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "!Failed to establish context! %s", reason.c_str());
...
void CContextEstablisher::OutputTiming(void)
{
	DrxComment("-- OutputTiming");
	{
		...
		for (size_t i = 0; i < m_tasks.size(); i++)
		{
			...
			DrxComment("%d: %s [%d @ %.2f] started at %." PRId64, m_tasks[i].state, m_tasks[i].pTask->GetName(), m_tasks[i].numRuns, tm, beg.GetValue());
		}
	}
}
#endif
