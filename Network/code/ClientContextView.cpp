...
void CClientContextView::SendEstablishedMessage()
{
	...
	public:
		...
		EMessageSendResult WritePayload(TSerialize, u32, u32)
		{
			return eMSR_SentOk;
		}
...
tukk CClientContextView::ValidateMessage(const SNetMessageDef* pMsg, bool bNetworkMsg)
{
...
bool CClientContextView::DoBeginBind(TSerialize ser, u32 flags)
{
...
NET_IMPLEMENT_ATSYNC_MESSAGE(CClientContextView, PerformSimpleBreak, eNRT_ReliableUnordered, eMPF_BlocksStateChange)
{
	if (CNetContextState* pState = ContextState())
	{
		if (IGameContext* pGameContext = pState->GetGameContext())
		{
			...
			uk userData = pGameContext->ReceiveSimpleBreakage(ser);
	...
	{
		DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "PerformSimpleBreak FAILED! GameContext was NULL!");
	}
	return true;
}

void CClientContextView::GC_PerformSimpleBreak(uk userData, INetBreakageSimplePlaybackPtr pBreakSimplePlayback)
{
	if (CNetContextState* pState = ContextState())
	{
		if (IGameContext* pGameContext = pState->GetGameContext())
...
NET_IMPLEMENT_IMMEDIATE_MESSAGE(CClientContextView, BeginSyncFiles, eNRT_ReliableUnordered, 0)
{
	...
	if (!m_fileSyncPhase)
		return false;
	DrxLog("BEGIN FILE SYNC: %d", m_fileSyncPhase);
	return true;
}
...
NET_IMPLEMENT_IMMEDIATE_MESSAGE(CClientContextView, AddFileData, eNRT_ReliableUnordered, 0)
{
	...
	if (length > CSyncedFileSet::MAX_SEND_CHUNK_SIZE)
		return false;
	u8 buf[CSyncedFileSet::MAX_SEND_CHUNK_SIZE];
	for (i32 i = 0; i < length; i++)
		ser.Value("data", buf[i], 'u8');
...
bool CClientContextView::SetAspectProfileMessage(NetworkAspectID aspectIdx, TSerialize ser, u32, u32, u32)
{
	ASSERT_GLOBAL_LOCK;
