...
static TNetAddress addr(tukk s)
{
	TNetAddressVec temp;
...
void CDistributedLogger::OnWriteToConsole(tukk sText, bool bNewLine)
{
}

void CDistributedLogger::OnWriteToFile(tukk sText, bool bNewLine)
{
	if (m_inFlush)
		return;
...
void CDistributedLogger::Update_BetweenPoll(CTimeValue now)
{
	...
	if (now - m_lastPoll > pollInterval)
	{
		...
		tuk pkt = "?";
		m_pSocket->Send((u8*)pkt, strlen(pkt), addr("255.255.255.255:61705"));
		m_inPoll = true;
		m_lastPoll = now;
	}
}
...
void CDistributedLogger::Flush()
{
	...
	m_inFlush = true;
	for (std::map<TNetAddress, bool>::iterator iter = m_listeners.begin(); iter != m_listeners.end(); ++iter)
		m_pSocket->Send((u8*)m_buffer, m_bufferPos, iter->first);
	InitBuffer();
	m_inFlush = false;
}
