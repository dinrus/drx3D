...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  implementation of DinrusXNetwork IArithModel
   -------------------------------------------------------------------------
   История:
...
i32 CCheckForNullEntityEncoding::m_check = 0;
...
CArithModel::CArithModel(const CArithModel& mdl)
	...
	, m_pNetContext(nullptr)
{
	for (i32 i = 0; i < NUM_TIME_STREAMS; i++)
...
void CArithModel::GetMemoryStatistics(IDrxSizer* pSizer, bool countingThis)
{
...
void CArithModel::STimeAdaption::STimeDelta::Update(u16 encodedDelta, bool hit)
{
	//	NetLog( "in - rate:%.4x error:%.4x encodedDelta:%.4x [%s]",
	//		rate, error, encodedDelta, hit? "hit" : "miss" );

	u32 newRate = (7 * rate + encodedDelta) / 8;
	...
	i32 e = abs((i32)rate - (i32)encodedDelta);
	rate = newRate;
...
int64 CArithModel::WriteGetTimeDelta(ETimeStream time, const CTimeValue& value, STimeAdaption::STimeDelta** pOutDeltaInfo)
{
	...
	if (adapt.timeFraction32 != 0)
		dt = (m_timeFraction32 - adapt.timeFraction32) * 1000 / (i32)REPLICATION_TIME_PRECISION;
...
CTimeValue CArithModel::ReadTime(CCommInputStream& stm, ETimeStream time)
{
	...
	int64 millis;
	u16 encodedDelta; // so it can be read later, in a debugger
	if (isLarge)
	{
...
{
	u32 sym = m_entityIDAlphabet.ReadSymbol(stm);
	SNetObjectID netID;
	switch (sym)
	{
	case eSEI_NullId:
...
void CArithModel::SetTimeFraction(u32 timeFraction32)
{
	m_timeFraction32 = timeFraction32;
}

u32 CArithModel::GetTimeFraction() const
{
