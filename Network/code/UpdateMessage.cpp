...
CUpdateMessage::CUpdateMessage(const SUpdateMessageConfig& cfg, u32 syncFlags)
...
NetworkAspectType CUpdateMessage::ComputeSendAspectMask(INetSender* pSender, SHistorySyncContext& hsc, SContextViewObject* pViewObj, SContextViewObjectEx* pViewObjEx, bool& willSend)
{
	...
	u8k forceSendData = (m_syncFlags & eSCF_AssumeEnabled) != 0;
...
EMessageSendResult CUpdateMessage::SendMain(SHistorySyncContext& hsc, INetSender* pSender)
{
	...
	//DrxLogAlways("%cM: %s -> %s [%08x %08x %08x %08x]", CheckHook()? 'S':'U', m_netID.GetText(), m_pView->Parent()->GetName(), m_maybeSendHistories[0], m_maybeSendHistories[1], m_maybeSendHistories[2], m_maybeSendHistories[3]);
	for (i32 history = 0; res == eMSR_SentOk && history < eH_NUM_HISTORIES; history++)
	{
		CHistory* pHistory = m_pView->GetHistory((EHistory) history);
	...
	if (res == eMSR_SentOk && m_pView->Context()->IsMultiplayer() && pViewObjEx)
	{
		...
		{
			i32& cnt = pViewObjEx->partialUpdatesRemaining[aspectIdx];
			cnt -= (cnt != 0);
...
void CUpdateMessage::UpdateState(u32 nFromSeq, ENetSendableStateUpdate update)
{
	if (m_pView->IsDead())
		return;
	...
	if (isBound && m_pView->Context()->IsMultiplayer() && update != eNSSU_Rejected)
	{
		...
		if (ctxObj.xtra)
		{
			...
			if (update == eNSSU_Ack)
			{
				...
				while (iter.Next(aspectIdx))
				{
					u32 sent = m_sentAspectVersions[aspectIdx];
					if (pViewObjEx)
					{
						u32& acked = pViewObjEx->ackedAspectVersions[aspectIdx];
						if (sent != ~u32(0) && (sent > acked || acked == ~u32(0)))
							acked = sent;
					}
				}
			}
...
bool CUpdateMessage::AnyAttachments(CContextView* pView, SNetObjectID id)
{
	...
	SAttachmentIndex idxLast = { id, ~u32(0) };
...
void CUpdateMessage::UpdateAttachmentsState(u32 nFromSeq, ENetSendableStateUpdate update)
{
...
tukk CUpdateMessage::GetDescription()
{
	if (CNetContextState* pContextState = m_pView->ContextState())
	{
...
void CRegularUpdateMessage::UpdateState(u32 nFromSeq, ENetSendableStateUpdate state)
{
	CUpdateMessage::UpdateState(nFromSeq, state);
}

CRegularUpdateMessage* CRegularUpdateMessage::Create(const SUpdateMessageConfig& cfg, u32 flags)
{
	ASSERT_GLOBAL_LOCK;
