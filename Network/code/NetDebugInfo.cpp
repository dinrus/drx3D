...
#include <drx3D/Network/StdAfx.h>
	...
	#include  <drx3D/Network/NetDebugInternetSimulator.h>
	#include  <drx3D/Network/NetDebugTrafficBandwidth.h>
	#include  <drx3D/Network/NetDebugProfileViewer.h>
	#include  <drx3D/Network/NetDebugServerInfo.h>
	#include  <drx3D/Network/NetDebugChannelViewer.h>
...
CNetDebugInfoSection::CNetDebugInfoSection(u32 page, float xpos, float ypos)
	: m_xpos(xpos),
	m_ypos(ypos),
	m_fontSize(1.5F),
	m_lineHeight(16.0F),
...
void CNetDebugInfoSection::DrawHeader(tukk sectionHeaderStr)
{
	DrawLine(0, s_yellow, sectionHeaderStr);
}

void CNetDebugInfoSection::DrawLine(u32 line, const float* pColor, tukk pFormat, ...)
{
	va_list args;
...
void CNetDebugInfoSection::VDrawLine(u32 line, const float* pColor, tukk pFormat, va_list args)
{
...
void CNetDebugInfoSection::Draw3dLine(const Vec3 pos, u32 nLine, const float* pColor, tukk pFormat, ...)
{
	va_list args;
	va_start(args, pFormat);
...
void CNetDebugObjectsInfo::Tick(const SNetworkProfilingStats* const pProfilingStats) PREFAST_SUPPRESS_WARNING(6262)
{
	...
	m_sendTraffic.Add((u32)deltaStats.m_totalBandwidthSent);
	m_sendPackets.Add((u32)deltaStats.m_totalPacketsSent);
	m_recvTraffic.Add((u32)deltaStats.m_totalBandwidthRecvd);
	m_recvPackets.Add((u32)deltaStats.m_totalPacketsRecvd);
}
...
void CNetDebugObjectsInfo::DrawObjects(u32 line, tukk pLabel, u32 count, u32 minVal, float average, u32 maxVal)
{
