...
#include  <drx3D/Network/Config.h>
	...
	#include  <drx3D/Network/DemoDefinitions.h>
...
bool DecodeString(tukk buf, SNetObjectID& value)
{
	i32 id, salt;
...
bool DecodeString(tukk buf, SSerializeString& value)
{
	value = buf;
	return true;
}

bool DecodeString(tukk buf, EntityId& value)
{
	value = 0;
	return 1 == sscanf(buf, "%x", &value);
}

bool DecodeString(tukk buf, bool& value)
{
	value = false;
	if (0 == strcmp(buf, "0"))
		value = false;
...
bool DecodeString(tukk buf, float& value)
{
	value = 0.0f;
	return 1 == sscanf(buf, "%f", &value);
}

bool DecodeString(tukk buf, i32& value)
{
	value = 0;
	...
	#if defined(_MSC_VER)
bool DecodeString(tukk buf, int64& value)
{
	value = 0;
	return 1 == sscanf(buf, "%I64d", &value);
}

bool DecodeString(tukk buf, uint64& value)
{
	value = 0;
...
bool DecodeString(tukk buf, CTimeValue& value)
{
	value = 0.0f;
	float temp;
...
bool DecodeString(tukk buf, ScriptAnyValue& value)
{
	return false;
}

	#define KINDA_INT_TYPE(type)                       \
	  bool DecodeString(tukk buf, type & value) \
	  {                                                \
	    i32 temp = 0;                                  \
...
bool DecodeString(tukk buf, Ang3& value)
{
	value = Ang3(0, 0, 0);
	return 3 == sscanf(buf, "%f,%f,%f", &value.x, &value.y, &value.z);
}

bool DecodeString(tukk buf, Vec3& value)
{
	value = Vec3(0, 0, 0);
	return 3 == sscanf(buf, "%f,%f,%f", &value.x, &value.y, &value.z);
}

bool DecodeString(tukk buf, Vec2& value)
{
	value = Vec2(0, 0);
	return 3 == sscanf(buf, "%f,%f", &value.x, &value.y);
}

bool DecodeString(tukk buf, Quat& value)
{
	value = Quat(1, 0, 0, 0);
...
public:
	...
	void ValueImpl(tukk name, T_Value& value)
	{
		DRX_PROFILE_FUNCTION(PROFILE_NETWORK);
	...
	void ValueImpl(tukk name, XmlNodeRef& value)
	{
	}
	template<class T_Value>
	...
	ILINE void Value(tukk szName, EntityId& value, u32 policy)   //, const NSerPolicy::AC_EntityId& policy )
	{
		//assert(policy == 'eid');
		ValueImpl(szName, value);
		if (policy == 'eid')
	...
	bool BeginOptionalGroup(tukk szName, bool bCond)
	{
		Value(szName, bCond);
		return bCond;
	}
...
CDemoPlaybackListener::CDemoPlaybackListener(CNetContext* pContext, tukk filename, CNetChannel* pClientChannel, CNetChannel* pServerChannel)
{
	m_pContext = pContext;
...
void CDemoPlaybackListener::DoUpdate()
{
	...
	while (true)
	{
		...
		{
			DRX_PROFILE_REGION(PROFILE_NETWORK, "DoUpdate:Select");
		...
		{
			DRX_PROFILE_REGION(PROFILE_NETWORK, "DoUpdate:Dispatch");
...
template<typename T>
bool CDemoPlaybackListener::DecodeValue(tukk name, T& value, bool peek)
{
