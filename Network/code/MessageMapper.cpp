...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  maps messages to their definitions
   -------------------------------------------------------------------------
   История:
                           ...
                           size_t nProtocols)
{
	...
	if (m_messages.empty())
		return;
	i32k nMessageIDs = GetNumberOfMsgIds();
	u32 i = 0;
	m_IdMap.clear();
	m_DefMap.resize(0);
...
u32 CMessageMapper::GetMsgId(const SNetMessageDef* def) const
{
	NET_ASSERT(def);
#if 0
	...
	DrxFatalError("Message def %s (%p) does not belong to this protocol", def->description, def);
#else
...
const SNetMessageDef* CMessageMapper::GetDispatchInfo(u32 id, size_t* pnWhichProtocol)
{
	...
	DrxFatalError("No such message id %d", id);
#else
...
u32 CMessageMapper::GetNumberOfMsgIds() const
{
	return m_messages.empty() ?
	       1 :
	       (u32)(m_messages.back().nStart + m_messages.back().pLast - m_messages.back().pFirst);
}

bool CMessageMapper::LookupMessage(tukk name, SNetMessageDef const** pDef, size_t* pnSubProtocol)
{
