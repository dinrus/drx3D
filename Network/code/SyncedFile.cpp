...
static u32k BAD_UINT32 = 0xdeadbeef;
...
bool CSyncedFile::FillData(u8* pBuffer, u32 bufLen)
{
	ASSERT_GLOBAL_LOCK;
	if (!EnterState(eS_Loaded))
		return false;
...
bool CSyncedFile::PutData(u8k* pData, u32 length)
{
	ASSERT_GLOBAL_LOCK;
	...
	{
		DrxLog("Server tried to send a too-large file (>%d bytes)", MAX_FILE_SIZE);
		return false;
	}
	...
	if (m_length + length > m_capacity)
	{
		...
		if (m_length + length > m_capacity)
			return false;
		u8* pNewData = (u8*) malloc(m_capacity);
		if (!pNewData)
			return false;
