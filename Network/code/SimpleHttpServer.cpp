...
#pragma warning(pop)
	...
	m_pWsAllocHeap = DrxGetIMemoryUpr()->CreateGeneralExpandingMemoryHeap(HEAP_MAX_SIZE, HEAP_RESERVE_SIZE, "HttpServer");
#endif
}
...
void CSimpleHttpServer::Start(u16 port, const string& password, IHttpServerListener* pListener)
{
...
void CSimpleHttpServer::SendResponse(i32 connectionID, EStatusCode statusCode, EContentType contentType, const string& content, bool closeConnection)
{
	FROM_GAME(&CSimpleHttpServerInternal::SendResponse, &m_internal, connectionID, statusCode, contentType, content, closeConnection);
}

void CSimpleHttpServer::SendWebpage(i32 connectionID, const string& webpage)
{
...
void CSimpleHttpServer::SendWebsocketData(i32 connectionID, IHttpWebsocketProtocol::SMessageData& data, bool closeConnection)
{
	...
	tuk pNewBufferWithExtraSpace = (tuk)AllocateHeapBuffer(data.m_bufferSize + WEBSOCKET_EXTRA_HEADER_SPACE);
	...
	{
		DrxLogAlways("CSimpleHttpServer::SendWebsocketData failed to allocate a temporary work buffer. Aborting send.");
	}
}

uk CSimpleHttpServer::AllocateHeapBuffer(u32 bufferSize)
{
	SCOPED_GLOBAL_LOCK;
	...
	{
		return (uk ) new int8[bufferSize];
	}
}

void CSimpleHttpServer::FreeHeapBuffer(uk pBuffer)
{
	SCOPED_GLOBAL_LOCK;

	if (m_pWsAllocHeap)
	{
		m_pWsAllocHeap->Free((uk )pBuffer);
	}
	else
	{
		delete[] (int8*)pBuffer;
...
CSimpleHttpServerInternal::HttpConnection::HttpConnection(i32 ID)
{
	m_ID = ID;
...
CSimpleHttpServerInternal::HttpConnection::WsReadChunk::WsReadChunk(u8k* pData, size_t nSize)
{
	m_readOffset = 0;
	m_bufferSize = 0;
	m_pBuffer = (u8*)CSimpleHttpServer::GetSingleton().AllocateHeapBuffer(nSize);
	if (m_pBuffer)
	{
...
bool CSimpleHttpServerInternal::HttpConnection::AddWsReadChunk(u8k* pData, size_t nSize)
{
	m_WsReadChunks.push_back(WsReadChunk(pData, nSize));
	return true;
}

bool CSimpleHttpServerInternal::HttpConnection::CollateWsReadChunks(tuk pBuffer, uint64 bufferSize)
{
...
void CSimpleHttpServerInternal::GetBindAddress(TNetAddress& addr, u16 port) const
{
...
void CSimpleHttpServerInternal::Start(u16 port, string password)
{
...
CSimpleHttpServerInternal::HttpConnection* CSimpleHttpServerInternal::GetConnection(i32 connectionID)
{
	for (size_t i = 0; i < m_connections.size(); i++)
	{
...
void CSimpleHttpServerInternal::CloseHttpConnection(i32 connectionID, bool bGraceful)
{
	for (size_t i = 0; i < m_connections.size(); i++)
	{
...
void CSimpleHttpServerInternal::IgnorePending(i32 connectionID)
{
...
void CSimpleHttpServerInternal::SendResponse(i32 connectionID, ISimpleHttpServer::EStatusCode statusCode, ISimpleHttpServer::EContentType contentType, string content, bool closeConnection)
{
	HttpConnection* conn = GetConnection(connectionID);
	...
	conn->m_pSocketSession->Send((u8k*)response.data(), response.size());
...
void CSimpleHttpServerInternal::SendWebpage(i32 connectionID, string webpage)
{
	HttpConnection* conn = GetConnection(connectionID);
	...
	drx_sprintf(contentLength, "%u", (unsigned)webpage.size());
...
#if defined(HTTP_WEBSOCKETS)
void CSimpleHttpServerInternal::SendToWebsocket(i32 connectionID, IHttpWebsocketProtocol::EMessageType type, tuk pBuffer, u32 bufferSize, bool closeConnection)
{
	...
	if (conn && conn->m_pSocketSession)
	{
		if (conn->m_receivingState == eRS_Websocket)
		{
			u64 headerSize = bufferSize < 126 ? 2 : (bufferSize < 65536) ? 4 : 10;
			...
			default:
				{
					...
					DrxLogAlways("CSimpleHttpServerInternal::SendWebsocketData: Unknown websocket data type - aborting send.");
			...
			case 10:
				{
					...
					SwapEndian(sz, eBigEndian);
					memcpy(pHeader + 1, (tuk)&sz, 8);
				}
				break;
			}

			conn->m_pSocketSession->Send((u8k*)pHeader, bufferSize + headerSize);
		}
		else
		{
			DrxLogAlways("CSimpleHttpServerInternal::SendWebsocketData: Trying to send Websocket data on a non-websocket. Aborting send.");
		}
	}
	...
	if (closeConnection)
	{
		...
		if (conn)
			conn->m_pSocketSession->Send((u8k*)&closeHeader, sizeof(WsDataHeader));

		CloseHttpConnection(connectionID, true);
	}
}

void CSimpleHttpServerInternal::OnWebsocketUpgrade(i32 connectionID, IHttpWebsocketProtocol* pProtocol)
{
	if (pProtocol)
	{
...
void CSimpleHttpServerInternal::OnWebsocketReceive(i32 connectionID, IHttpWebsocketProtocol* pProtocol, IHttpWebsocketProtocol::SMessageData data)
{
	SCOPED_GLOBAL_LOCK;
...
void CSimpleHttpServerInternal::OnWebsocketClose(i32 connectionID, IHttpWebsocketProtocol* pProtocol, bool bGraceful)
{
	if (pProtocol)
	{
...
void CSimpleHttpServerInternal::OnConnectionAccepted(IStreamSocketPtr pStreamSocket, uk pUserData)
{
	...
	if (m_connections.size() >= MAX_HTTP_CONNECTIONS)
	{
		...
		pStreamSocket->Send((u8k*)response.data(), response.size());
		pStreamSocket->Close();
	}
	else
	{
		...
		conn->m_pSocketSession->SetListener(this, (uk )(EXPAND_PTR)conn->m_ID);  // in this specific application, the same listener is used for both sockets
...
void CSimpleHttpServerInternal::OnConnectionClosed(bool graceful, uk pUserData)
{
	CloseHttpConnection((i32)(TRUNCATE_PTR)pUserData, false);
}
...
bool CSimpleHttpServerInternal::ParseHeaderLine(i32 connectionID, string& line)
{
	HttpConnection* conn = GetConnection(connectionID);
		...
		if ("get" == token)
		{
			...
			if (!conn->m_uri.empty())
			{
				...
				if ("http/1.0" == token || "http/1.1" == token || "" == token)
				{
					...
					//	m_pSocketSession->Send((u8*)response.data(), response.size());
...
void CSimpleHttpServerInternal::ProcessIncomingWebSocketData(i32 connectionID, u8k* pData, size_t nSize)
{
		...
		case eWSCS_RecvHeader:
			{
				...
				if (pConnection->CollateWsReadChunks((tuk)&pConnection->m_WsRecvCurrentHeader, sizeof(WsDataHeader)))
				{
					switch (pConnection->m_WsRecvCurrentHeader.opcode)
					{
					case eWSOC_Text:
						{
							//DrxLogAlways("CSimpleHttpServerInternal::ProcessIncomingWebSocketData: OPCODE Text, len=%d, connection %d", pConnection->m_WsRecvCurrentHeader.len, connectionID);
					...
					case eWSOC_Binary:
						{
							//DrxLogAlways("CSimpleHttpServerInternal::ProcessIncomingWebSocketData: OPCODE Binary, len=%d, connection %d", pConnection->m_WsRecvCurrentHeader.len, connectionID);
					...
					case eWSOC_Continuation:
						{
							//DrxLogAlways("CSimpleHttpServerInternal::ProcessIncomingWebSocketData: OPCODE Continuation, len=%d, connection %d", pConnection->m_WsRecvCurrentHeader.len, connectionID);
							pConnection->m_WsConnectState = eWSCS_RecvExtendedSize;
						}
						break;
					case eWSOC_ConnectionClose:
						{
							...
							//DrxLogAlways("CSimpleHttpServerInternal::ProcessIncomingWebSocketData: OPCODE: ConnectionClose, connection %d", connectionID);
							CloseHttpConnection(connectionID, true);
							bDone = true;
						}
						break;
					case eWSOC_Ping:
						{
							//DrxLogAlways("CSimpleHttpServerInternal::ProcessIncomingWebSocketData: OPCODE: Ping, TODO, connection %d", connectionID);
						}
						break;
					case eWSOC_Pong:
						{
							//DrxLogAlways("CSimpleHttpServerInternal::ProcessIncomingWebSocketData: OPCODE: Pong. TODO, connection %d", connectionID);
						}
						break;
					default:
						{
							// invalid opcode. Send a ConnectionClose back to client.
							//DrxLogAlways("CSimpleHttpServerInternal::ProcessIncomingWebSocketData: INVALID OPCODE 0x%02X, connection %d", pConnection->m_WsRecvCurrentHeader.opcode, connectionID);
							//DrxLogAlways("Header: fin=%d, mask=%d, len=%d", pConnection->m_WsRecvCurrentHeader.fin, pConnection->m_WsRecvCurrentHeader.mask, pConnection->m_WsRecvCurrentHeader.len);
		...
		case eWSCS_RecvExtendedSize:
			{
				...
				else if (pConnection->m_WsRecvCurrentHeader.len == 126)
				{
					// collate 2 bytes
					unsigned short len;
					if (pConnection->CollateWsReadChunks((tuk)&len, 2))
					{
				...
				else if (pConnection->m_WsRecvCurrentHeader.len == 127)
				{
					// collate 8 bytes
					uint64 len64;
					if (pConnection->CollateWsReadChunks((tuk)&len64, 8))
					{
		...
		case eWSCS_RecvMask:
			{
				...
				{
					if (pConnection->CollateWsReadChunks((tuk)pConnection->m_WsRecvCurrentMask, 4))
					{
		...
		case eWSCS_RecvPayload:
			{
				...
				tuk pWorkBuffer = &pConnection->m_WsRecvCombinedMessageBuffer[oldSize];
				if (pWorkBuffer)
				{
					if (pConnection->CollateWsReadChunks(pWorkBuffer, pConnection->m_WsRecvCurrentPayloadSize))
					{
						...
						if (pConnection->m_WsRecvCurrentHeader.fin)
						{
							...
							if (pConnection->m_websocketProtocol)
							{
								...
								data.m_pBuffer = (tuk)m_pServer->AllocateHeapBuffer(data.m_bufferSize + 1);
		...
		case eWSCS_SendGracefulClose:
			{
				...
				pConnection->m_pSocketSession->Send((u8k*)&closeHeader, sizeof(WsDataHeader));
...
void CSimpleHttpServerInternal::OnIncomingData(u8k* pData, size_t nSize, uk pUserData)
{
	HttpConnection* conn = GetConnection((i32)(TRUNCATE_PTR)pUserData);
	...
	while (nLength > 0)
	{
		...
		else if (conn->m_receivingState == eRS_ReceivingContent)
		{
			...
			conn->m_content.append((tuk)pBuffer, toAppend);
			pBuffer += toAppend;
			nLength -= toAppend;
...
void CSimpleHttpServerInternal::BuildNormalResponse(i32 connectionID, string& response, ISimpleHttpServer::EStatusCode statusCode, ISimpleHttpServer::EContentType contentType, const string& content)
{
	response = "";
...
void CSimpleHttpServerInternal::UnauthorizedResponse(i32 connectionID, string& response)
{
	HttpConnection* conn = GetConnection(connectionID);
	...
	{
		CWhirlpoolHash hash((u8*)timestamp, strlen(timestamp));
...
void CSimpleHttpServerInternal::BadRequestResponse(i32 connectionID)
{
	HttpConnection* conn = GetConnection(connectionID);
	...
	conn->m_pSocketSession->Send((u8*)response.data(), response.size());

	CloseHttpConnection(connectionID, true);
}

#if defined(HTTP_WEBSOCKETS)
bool CSimpleHttpServerInternal::WebsocketUpgradeResponse(i32 connectionID)
{
	HttpConnection* conn = GetConnection(connectionID);
	...
	DrxSHA1::sha1Calc(wskey.c_str(), message_digest);
	...
	tuk pBase64 = (tuk)m_pServer->AllocateHeapBuffer(lenBase64 + 1);
	Base64::encode_base64(pBase64, (tukk const)message_digest, sha_len * 4, true);
...
void CSimpleHttpServerInternal::NotImplementedResponse(i32 connectionID)
{
	HttpConnection* conn = GetConnection(connectionID);
...
bool CSimpleHttpServerInternal::ParseDigestAuthorization(i32 connectionID, string& line)
{
	// TODO:
	return false;
}
