...
CNetDebugProfileViewer::CNetDebugProfileViewer(u32 type)
...
void CNetDebugProfileViewer::DrawProfileEntry(const SNetProfileStackEntry* entry, i32 depth, i32* line)
{
	...
	case CNetDebugInfo::eP_ProfileViewLast5Seconds:
		{
			...
			i32 extraSpace = PROFILE_ENTRY_STRING_DISPLAY_LENGTH - stringBuffer.length();
			for (i32 j = 0; j < extraSpace; j++)
			{
				stringBuffer.append(" ");
			}
...
void CNetDebugProfileViewer::DrawTree(const SNetProfileStackEntry* root, i32 depth, i32* line)
{
	...
	case CNetDebugInfo::eP_ProfileViewLast5Seconds:
		{
			...
			DrxFixedStringT<1024> stringBufferTemp;
			for (i32 i = 0; i < PROFILE_ENTRY_STRING_DISPLAY_LENGTH; i++)
			{
