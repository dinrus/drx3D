...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  Server implementation of a context view
   -------------------------------------------------------------------------
   История:
...
#include  <drx3D/Network/PerformBreakage.h>
...
private:
	tukk GetBasicDescription() const
	{
		return "Bind";
	}
...
{
public:
	CDeclareBrokenProductMessage(SNetObjectID id, i32 brk, CServerContextView* pView)
...
{
public:
	CPerformBreak(CServerContextView* pView, i32 id, const SNetIntBreakDescription* pDesc) : INetSendable(eMPF_BlocksStateChange, eNRT_ReliableUnordered), m_id(id), m_pView(pView), m_pDesc(pDesc)
	{
	}
...
{
public:
	CBeginBreakStream(i32 id) : INetSendable(eMPF_BlocksStateChange, eNRT_ReliableUnordered), m_id(id)
	{
	}
...
CServerContextView::CServerContextView(CNetChannel* pChannel, CNetContext* pContext)
{
	...
	for (i32 i = 0; i < MAX_BREAK_STREAMS; i++)
	{
...
void CServerContextView::GC_BindObject( SNetObjectID netID, CNetObjectBindLock lk, CChangeStateLock cslk, bool levelInit)
{
	...
	if (!IsLocal())
	{
		...
		u32 grp = m_objects[netID.id].authority? obj.xtra->scheduler_owned : obj.xtra->scheduler_normal;

		static u32 initGroup = 0;
		if (levelInit)
		{
			if (initGroup == 0)
...
void CServerContextView::InitSessionIDs()
{
	...
	public:
		...
		EMessageSendResult WritePayload(TSerialize ser, u32, u32)
		{
			m_sessionID.SerializeWith(ser);
			return eMSR_SentOk;
		}

		void UpdateState(u32, ENetSendableStateUpdate)
		{
		}
...
bool CServerContextView::EnterState(EContextViewState state)
{
	...
	case eCVS_EstablishContext:
		...
		if (!IsLocal())
		{
			if (tukk failureReason = PbAuthClient(
			      const_cast<tuk>(RESOLVER.ToNumericString(Parent()->GetIP()).c_str()),
			      i32(m_clientHasPunkBuster != 0),
			      const_cast<tuk>(Parent()->GetCDKeyHash().c_str())))
			{
...
tukk CServerContextView::ValidateMessage(const SNetMessageDef* pMsg, bool bNetworkMsg)
{
...
void CServerContextView::GetMemoryStatistics(IDrxSizer* pSizer)
{
...
void CServerContextView::GotBreakage(const SNetIntBreakDescription* pDesc)
{
	...
	for (i32 i = minSegId.x; i <= maxSegId.x; i++)
		for (i32 j = minSegId.y; j <= maxSegId.y; j++)
			for (i32 k = minSegId.z; k <= maxSegId.z; k++)
	...
	{
		for (i32 i = 0; i < pDesc->spawnedObjects.size(); i++)
		{
	...
	for (i32 i = 1; i < MAX_BREAK_STREAMS; i++)
