...
public:
	...
	#if USE_ARITHSTREAM
	ILINE bool ReadValue(CCommInputStream& in, Vec3& value, CArithModel* pModel, u32 age) const
	{
	...
	ILINE bool WriteValue(CCommOutputStream& out, const Vec3& value, CArithModel* pModel, u32 age) const
	{
	...
	#else
	ILINE bool ReadValue(CNetInputSerializeImpl* in, Vec3& value, u32 age) const
	{
		Vec3i quantized;
	...
	ILINE bool WriteValue(CNetOutputSerializeImpl* out, const Vec3& value, u32 age) const
	{
		SEncodeResult er = m_predictor.Encode(value, age);
...
private:
	...
	mutable i32                m_hadMemento;
};

class CDirectionalPredictor
{
public:
	ILINE bool Load(XmlNodeRef node, i32 bits)
	{
	...
	ILINE bool ReadMemento(CByteInputStream& in) const
	{
		m_nValues = in.GetTyped<u8>();
		NET_ASSERT(/*m_nValues >= 0 && */ m_nValues <= 2);
		for (i32 i = 0; i < 3; i++)
			for (i32 j = 0; j < 3; j++)
				m_quantized[i][j] = in.GetTyped<i32>();
		return true;
	}
	ILINE bool WriteMemento(CByteOutputStream& out) const
	{
		NET_ASSERT(/*m_nValues >= 0 && */ m_nValues <= 2);
		out.PutTyped<u8>() = m_nValues;
		for (i32 i = 0; i < 3; i++)
			for (i32 j = 0; j < 3; j++)
				out.PutTyped<i32>() = m_quantized[i][j];
		return true;
	}
	ILINE void NoMemento() const
	{
	...
	ILINE SEncodeResult Encode(const Vec3& value, i32 age) const
	{
	...
	ILINE bool Decode(Vec3i encoded, i32 age, Vec3& out) const
	{
		if (m_nValues != 2)
			return false;
		Vec3i pred = GetPrediction();
	...
	ILINE Vec3i ClampVec(const Vec3i& v) const
	{
		Vec3i out;
		for (i32 i = 0; i < 3; i++)
			out[i] = CLAMP(v[i], 0, (i32)m_quantizer.GetMaxQuantizedValue());
		return out;
	}
...
private:
	...
	ILINE Vec3i EncodeErr(const Vec3i& err) const
	{
		...
		Vec3i err2;
		err2.x = (i32)(((int64(dir.x) * err.x + int64(dir.y) * err.y) << INTEGER_ERROR_ENCODE_EXTRA_BITS) / approx_length);
		err2.y = (i32)(((-int64(dir.y) * err.x + int64(dir.x) * err.y) << INTEGER_ERROR_ENCODE_EXTRA_BITS) / approx_length);
		err2.z = err.z;
		return err2;
	#else
	...
	#ifdef INTEGER_ERROR_ENCODE_MATH
		Vec3i dir = m_quantized[2];
		i32 approx_length = abs(dir.x) > abs(dir.y) ? abs(dir.x) : abs(dir.y);

		Vec3i err2;
		err2.x = (i32)((int64(dir.x) * err.x - int64(dir.y) * err.y) / approx_length);
		err2.y = (i32)((int64(dir.y) * err.x + int64(dir.x) * err.y) / approx_length);
		err2.z = err.z;
		return err2;
	#else
		Vec3d dir = m_quantized[2];
...
{
	/*
	   CNetwork::Get()->GetCompressionUpr().Reset(true);
	   CDefaultStreamAllocator alc;
	   CCommOutputStream stm(&alc, 1024);
	   ICompressionPolicyPtr pol = CNetwork::Get()->GetCompressionUpr().GetCompressionPolicy('wrld');
	   pol->NoMemento( false );
	   ...
	   {
	    Vec3 r;
	    float ang = drx_random(0.0f, gf_PI*2);
	    float rad = expf(drx_random(0.0f, 15.0836251125665f)) * 0.0000719793139544965f - 0.0000719793139544965f;
	    r.x = cosf(ang)*rad + 1000.0f;
	    r.y = sinf(ang)*rad + 1000.0f;
	    r.z = 100.0f + drx_random(0.0f, 0.2f);
	    saved.push_back(r);
