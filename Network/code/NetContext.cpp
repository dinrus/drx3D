...
#include  <drx3D/Network/DemoRecordListener.h>
#include  <drx3D/Network/DemoPlaybackListener.h>
#include  <drx3D/Network/RMILogger.h>
...
static i32 SessionIDCounter = 10101;
#endif
...
CNetContext::CNetContext(IGameContext* pGameContext, u32 flags) :
...
#endif
{
	...
	CNetwork::Get()->GetCompressionUpr().Reset(m_multiplayer, false);
...
void CNetContext::SetAspectChannelMask(NetworkAspectType aspectBit, ChannelMaskType mask)
{
	...
	{
		DrxFatalError("Cannot set mask on multiple aspects at the same time");
	}
...
CNetContext::~CNetContext()
{
	...
	CNetwork::Get()->GetCompressionUpr().Reset(false, true);
	--g_objcnt.netContext;
...
void CNetContext::ActivateDemoRecorder(tukk filename)
{
	SCOPED_GLOBAL_LOCK;
...
void CNetContext::ActivateDemoPlayback(tukk filename, INetChannel* pClient, INetChannel* pServer)
{
	SCOPED_GLOBAL_LOCK;
#if INCLUDE_DEMO_RECORDING
...
void CNetContext::BackgroundPassthrough(NetTimerId, uk p, CTimeValue)
{
...
{
	SCOPED_GLOBAL_LOCK;
	// THIS IS A TEMPORARY HACK TO MAKE THE GAME PLAY NICELY, ASK peter@drxtek WHY IT'S STILL HERE
	enable = false;
...
void CNetContext::DeclareAspect(tukk name, NetworkAspectType aspectBit, u8 aspectFlags)
{
	SCOPED_GLOBAL_LOCK;
	...
	if (CountBits(aspectBit) != 1)
	{
		DrxFatalError("Can only declare one aspect at a time");
	}
	aspectIdx = BitIndex(aspectBit);
	if (m_declaredAspects & aspectBit)
	{
		DrxFatalError("Net aspect %u declared twice", aspectIdx);
	}
...
void CNetContext::EstablishedContext(i32 establishToken)
{
	SCOPED_GLOBAL_LOCK;
...
void CNetContext::LogRMI(tukk function, ISerializable* pParams)
{
...
void CNetContext::SetAspectProfile(EntityId id, NetworkAspectType aspectBit, u8 profile)
{
	m_pState->SetAspectProfile(id, aspectBit, profile);
}

u8 CNetContext::GetAspectProfile(EntityId id, NetworkAspectType aspectBit)
{
...
EntityId CNetContext::RemoveReservedUnboundEntityMapEntry(u16 partialNetID)
{
...
bool CNetContext::SetSchedulingParams(EntityId objId, u32 normal, u32 owned)
{
	return m_pState->SetSchedulingParams(objId, normal, owned);
}

void CNetContext::PulseObject(EntityId objId, u32 pulseType)
{
...
i32 CNetContext::RegisterPredictedSpawn(INetChannel* pChannel, EntityId id)
{
	return m_pState->RegisterPredictedSpawn(pChannel, id);
}

void CNetContext::RegisterValidatedPredictedSpawn(INetChannel* pChannel, i32 predictionHandle, EntityId id)
{
...
void CNetContext::GetMemoryStatistics(IDrxSizer* pSizer)
{
...
void CNetContext::RegisterServerControlledFile(tukk filename)
{
	SCOPED_GLOBAL_LOCK;
...
static IDrxPak* FallbackGetIDrxPak()
{
	return gEnv->pDrxPak;
}

IDrxPak* CNetContext::GetServerControlledIDrxPak()
{
	SCOPED_GLOBAL_LOCK;

	if (!ServerFileSyncEnabled())
		return FallbackGetIDrxPak();
...
static XmlNodeRef FallbackXmlLoad(tukk filename, tukk msg)
{
	if (msg)
		DrxLog("Server synced XML load of '%s' failed, using on-disk version: %s", filename, msg);

	return gEnv->pSystem->LoadXmlFromFile(filename);
}

XmlNodeRef CNetContext::GetServerControlledXml(tukk filename)
{
	if (!ServerFileSyncEnabled())
	...
	return gEnv->pSystem->LoadXmlFromBuffer((tukk)lk.GetData(), lk.GetLength());
}
