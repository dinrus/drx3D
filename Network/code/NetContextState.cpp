...
#include <drx3D/CoreX/Game/IGameFramework.h>       // LOCAL_PLAYER_ENTITY_ID
...
static u8 GetDefaultProfileForAspect(EntityId id, EEntityAspects aspectID)
{
	IEntity* pEntity = gEnv->pEntitySystem->GetEntity(id);
	if (!pEntity)
	{
		DRX_ASSERT_MESSAGE(gEnv->IsEditor(), "Trying to get default profile for aspect %d on unknown entity %d", aspectID, id);
		return ~u8(0);
	}
...
static void SetEntityAspectProfile(EntityId id, NetworkAspectType aspectBit, u8 profile)
{
	DRX_ASSERT(0 == (aspectBit & (aspectBit - 1)));
...
CNetContextState::CNetContextState(CNetContext* pContext, i32 token, CNetContextState* pPrev) : m_pMMM(new CMementoMemoryUpr(string().Format("NetContextState[%d]", token)))
{
	MMM_REGION(m_pMMM);
	...
	if (pPrev && pPrev->m_vObjects.size())
	{
		...
		for (size_t i = 0; i < m_vObjects.size(); i++)
		{
			...
			AddToFreeObjects(static_cast<u16>(i));
		}
	}
...
void CNetContextState::SendBindEventsTo(INetContextListenerPtr pListener, bool alsoAuth)
{
	...
	for (TContextObjects::iterator iter = m_vObjects.begin(); iter != m_vObjects.end(); ++iter)
	{
		if (iter->bAllocated)
		{
			u16 idx = (u16)(iter - m_vObjects.begin());
...
void CNetContextState::SendSetAspectProfileEventsTo(INetContextListenerPtr pListener)
{
	...
	for (size_t it = 0; it < m_vObjects.size(); ++it)
	{
		...
		{
			event.id = SNetObjectID(static_cast<u16>(it), m_vObjects[it].salt);
...
void CNetContextState::FetchAndPropogateChangesFromGame(bool allowFetch)
{
	...
	i32 psr = CNetCVars::Get().PacketSendRate;
	...
	for (u32 i = 0; i < m_subscriptions.size(); /* intentionally left blank */)
	{
...
public:
	...
	bool Next()
	{
		while (m_pNextChange != m_pEndChange)
		{
			...
			if (GetRemoteAspectDataHandle() == CMementoMemoryUpr::InvalidHdl)
				continue;
	...
	{
		const_cast<u32&>(m_obj.xtra->vAspectDataVersion[GetAspectIndex()])++;
	}
	...
	bool ShouldTakeUpdate(TMemHdl tgtHdl, TMemHdl newHdl)
	{
		...
		if (tgtHdl != CMementoMemoryUpr::InvalidHdl && MMM().GetHdlSize(tgtHdl) == newSize)
		{
			i32 ofs = 0;
			if (IsTimestamped())
				ofs += sizeof(CTimeValue);
			u8k* pOld = (u8k*)MMM().PinHdl(tgtHdl);
			u8k* pNew = (u8k*)MMM().PinHdl(newHdl);
			i32 cmpSz = newSize - ofs;
	...
	{
		char buf[256];
		drx_sprintf(buf, "Failed to commit data from network to game! obj(eid:%.8x name:%s netid:%s aspect:%s)", GetUserID(), m_obj.main->GetName(), m_pCurChange->obj.GetText(), m_state.m_pContext->GetAspectName(GetAspectIndex()));
	...
	bool SendToGameAndUpdateFreeHandleWith(CByteInputStream& in, CAutoFreeHandle& freeHdl, CCompressionUpr& cman)
	{
		bool wasPartialUpdate = false; // in-out
		EntityId userId = GetUserID();
		NetworkAspectType aspectBit = GetAspectBit();
		u8 aspectProfile = GetAspectProfile();
...
{
public:
	static i32k DODGYHACK_PHYSICS_ASPECT_INDEX = 3;
...
void CNetContextState::PropogateChangesToGame()
{
	...
	while (ctx.Next())
	{
		...
		u8k* remData = (u8k*) MMM().PinHdl(remHdl);
...
void CNetContextState::SendBindAspectsTo(INetContextListenerPtr pListener)
{
	...
	if (m_multiplayer)
	{
		for (size_t netID = 0; netID < m_vObjects.size(); netID++)
		{
			...
			event.id = SNetObjectID(static_cast<u16>(netID), m_vObjects[netID].salt);
...
void CNetContextState::CacheAspectProfileChange(SNetObjectID id, NetworkAspectType aspect, u8 profile)
{
...
bool CNetContextState::GetAspectProfileFromCache(SNetObjectID id, NetworkAspectType aspect, u8& profile)
{
	for (TAspectProfileCache::iterator it = m_aspectProfileCache.begin(), eit = m_aspectProfileCache.end(); it != eit; ++it)
	{
		if (it->id == id && it->aspect == aspect)
...
void CNetContextState::SetAspectProfile(EntityId id, NetworkAspectType aspectBit, u8 profile)
{
	if (!m_pContext)
		return;
...
void CNetContextState::NC_SetAspectProfile(EntityId userID, NetworkAspectType aspectBit, u8 profile, bool lockedUpdate)
{
	ASSERT_GLOBAL_LOCK;
	DoSetAspectProfile(userID, aspectBit, profile, true, true, lockedUpdate);
}

u8 CNetContextState::GetAspectProfile(EntityId userID, NetworkAspectType aspectBit)
{
	SCOPED_GLOBAL_LOCK;
	...
	{
		DrxFatalError("Can only set one aspect profile at a time");
		return 255;
	}
...
void CNetContextState::LogChangedProfile(SNetObjectID id, NetworkAspectType aspect, u8 profile)
{
	bool found = false;
	for (TChangedProfiles::iterator it = m_changedProfiles.begin(); it != m_changedProfiles.end(); ++it)
	{
...
bool CNetContextState::DoSetAspectProfile(EntityId userID, NetworkAspectType aspectBit, u8 profile, bool checkOwnership, bool informedGame, bool unlockUpdate)
{
	MMM_REGION(m_pMMM);
	...
	if (m_multiplayer)
	{
		if (obj.xtra->vAspectData[aspectIdx] != CMementoMemoryUpr::InvalidHdl)
		{
			MMM().FreeHdl(obj.xtra->vAspectData[aspectIdx]);
			const_cast<SContextObjectEx*>(obj.xtra)->vAspectData[aspectIdx] = CMementoMemoryUpr::InvalidHdl;
		}
...
bool CNetContextState::SetSchedulingParams(EntityId objId, u32 normal, u32 owned)
{
	SCOPED_GLOBAL_LOCK;
	if (!m_multiplayer)
		return true;
...
void CNetContextState::LogRMI(tukk function, ISerializable* pParams)
{
	SCOPED_GLOBAL_LOCK;
	// check fast path
	...
	else
	{
		...
		if (m_multiplayer)
		{
			...
			for (NetworkAspectID i = 0; i < NumAspects; i++)
			{
				...
				objx.vAspectData[i] = CMementoMemoryUpr::InvalidHdl;
				objx.vRemoteAspectData[i] = CMementoMemoryUpr::InvalidHdl;

				// probe object for which serialization chunks it will use for various profiles
				for (size_t c = 0; c < MaxProfilesPerAspect; c++)
				{
					objx.vAspectProfileChunks[i][c] = CNetwork::Get()->GetCompressionUpr().GameContextGetChunkID(m_pGameContext, userId, nAspect, static_cast<u8>(c), skipCompression);
				}
				nAspect <<= 1;
			}
...
bool CNetContextState::UnbindObject(SNetObjectID netID, u32 flags)
{
	ASSERT_GLOBAL_LOCK;
	if (!GetContextObject(netID).main)
	{
...
bool CNetContextState::UnboundObject(SNetObjectID netID, tukk reason)
{
	MMM_REGION(m_pMMM);
...
void CNetContextState::PulseObject(EntityId objId, u32 pulseType)
{
	FROM_GAME(&CNetContextState::GC_PulseObject, this, objId, pulseType);
}

void CNetContextState::GC_PulseObject(EntityId objId, u32 pulseType)
{
	ASSERT_GLOBAL_LOCK;
	if (!m_multiplayer)
		return;
                                      ...
                                      bool bOwned, ESpawnType spawnType, INetContextListenerPtr pController)
{
	...
	if (!netID)
	{
		if (m_multiplayer)
		{
			...
			bool lowBitID = false;
			i32 numObjects = m_vObjects.size();
			tukk pName = "<no name>";
			...
			if (userID)
			{
				...
				if (pEntity)
				{
					...
					if (DrxStringUtils::stristr(pClass->GetName(), "player"))
					{
						lowBitID = true;
					}
				}
			...
			if (!netID)
			{
				if (m_freeHighBitObjects.empty())
				{
					...
					else
					{
						...
						DrxFatalError("AllocateObject: Failed to allocate a netid for %s. See Game.log for details.", pName);
						return false;
					}
				}
				else
		...
		else
		{
			if (m_freeLowBitObjects.empty())
			{
				...
				{
					DrxFatalError("WAY too many synchronized objects");
				}

				netID.id = static_cast<u16>(m_vObjects.size());
				m_vObjects.push_back(SContextObject());
			}
			else
			{
	...
	for (i32 i = 0; i < eCOL_NUM_LOCKS; i++)
	{
		obj.vLocks[i] = 0;
	}
			...
			for (NetworkAspectID i = 0; i < NumAspects; i++)
			{
				...
				if (nAspect & aspectBits)
				{
					...
					else
					{
						...
						objx.vAspectProfileChunks[i][0] = CNetwork::Get()->GetCompressionUpr().GameContextGetChunkID(m_pGameContext, userID, nAspect, 0, skipCompression);
...
#if RESERVE_UNBOUND_ENTITIES
EntityId CNetContextState::AddReservedUnboundEntityMapEntry(u16 partialNetID, EntityId eID)
{
	...
	#endif
				m_ReservedUnboundEntityMap.insert(std::pair<u16, EntityId>(partialNetID, eID));
			}
			return eID;
		}
		else
...
EntityId CNetContextState::RemoveReservedUnboundEntityMapEntry(u16 partialNetID)
{
	if (partialNetID != SNetObjectID::InvalidId)
	{
...
EntityId CNetContextState::GetReservedUnboundEntityMapEntry(u16 partialNetID)
{
	if (partialNetID != SNetObjectID::InvalidId)
	{
...
void CNetContextState::TurnAspectsOn(SNetObjectID netID, NetworkAspectType aspects)
{
	...
	while (itAspects.Next(i))
	{
		...
		{
			bool allEmpty = true;
			for (i32 a = 0; allEmpty && a < MaxProfilesPerAspect; a++)
				allEmpty &= CNetwork::Get()->GetCompressionUpr().IsChunkEmpty(objx.vAspectProfileChunks[i][a]);
			if (allEmpty)
			{
				aspects &= ~(1 << i);
				continue;
...
void CNetContextState::GetAllObjects(std::vector<SNetObjectID>& objs)
{
	...
	for (TContextObjects::iterator iter = m_vObjects.begin(); iter != m_vObjects.end(); ++iter)
	{
		if (iter->bAllocated && iter->userID)
			objs.push_back(SNetObjectID((u16)(iter - m_vObjects.begin()), iter->salt));
	}
}

INetContextListenerPtr CNetContextState::FindListener(tukk name)
{
	ASSERT_GLOBAL_LOCK;
	if (name[0] == 0)
		return 0;
...
void CNetContextState::LogBreak(const SNetBreakDescription& breakage)
{
	...
	{
		for (i32 i = 0; i < breakage.nEntities; i++)
		{
			if (gEnv->bServer)
			{
...
void CNetContextState::GC_BoundObject(const EntityId eid)
{
	...
	DRX_ASSERT_MESSAGE(pEntity, "[net] notification of binding non existant entity %.8x received", eid);
...
void CNetContextState::GC_SendPostSpawnEntities(CContextViewPtr pView)
{
	...
	for (TContextObjects::iterator iter = m_vObjects.begin(); iter != m_vObjects.end(); ++iter)
	{
		if (iter->bAllocated && iter->userID)
		{
			...
			{
				SNetObjectID id((u16)(iter - m_vObjects.begin()), iter->salt);
#if ENABLE_DEBUG_KIT
...
void CNetContextState::GC_SetAspectProfile(NetworkAspectType aspect, u8 profile, SNetObjectID netID, CNetObjectBindLock lock)
{
	ASSERT_GLOBAL_LOCK;
	ENSURE_REALTIME;
	ASSERT_PRIMARY_THREAD;
...
i32 CNetContextState::RegisterPredictedSpawn(INetChannel* pChannel, EntityId id)
{
	SCOPED_GLOBAL_LOCK;
...
void CNetContextState::RegisterValidatedPredictedSpawn(INetChannel* pChannel, i32 predictionHandle, EntityId id)
{
	SCOPED_GLOBAL_LOCK;
...
CNetObjectBindLock CNetContextState::LockObject(SNetObjectID id, tukk why)
{
...
void CNetContextState::GetMemoryStatistics(IDrxSizer* pSizer)
{
		...
		for (u32 nLoop = 0; nLoop < 2; nLoop++)
		{
			...
			for (TNetIDMap::const_iterator iterNetIDs = m_pNetIDs->begin(); iterNetIDs != m_pNetIDs->end(); ++iterNetIDs)
			{
				...
				if (pEntity)
				{
					...
					tukk name = pEntity ? pEntity->GetName() : "<<no name>>";
			...
			for (std::map<string, i32>::iterator classIter = classesMap.begin(); classIter != classesMap.end(); ++classIter)
			{
...
tukk SContextObject::GetName() const
{
	ASSERT_GLOBAL_LOCK;
	if (userID)
	{
...
tukk SContextObject::GetClassName() const
{
	ASSERT_GLOBAL_LOCK;
	if (userID)
	{
...
void CNetContextState::DrawDebugScreens()
{
	...
	#if CHECKING_BIND_REFCOUNTS
				for (std::map<string, i32>::const_iterator it = iter->debug_whosLocking.begin(); it != iter->debug_whosLocking.end(); ++it)
					if (it->second)
	...
	if (CVARS.MemInfo & eDMM_Context)
	{
		...
		{
			if (iter->bAllocated && iter->userID)
				cm[gEnv->pEntitySystem->GetEntity(iter->userID)->GetClass()].push_back(SNetObjectID((u16)(iter - m_vObjects.begin()), iter->salt));
		}
		...
		muAll.required += sizeof(u16) * m_freeLowBitObjects.size();                      //-- m_freeLowBitObjects vector approximate
		muAll.required += sizeof(u16) * m_freeMediumBitObjects.size();                   //-- m_freeMediumBitObjects vector approximate
		muAll.required += sizeof(u16) * m_freeHighBitObjects.size();                     //-- m_freeHighBitObjects vector approximate
		...
		for (ClassMap::const_iterator cmiter = cm.begin(); cmiter != cm.end(); ++cmiter)
		{
			...
			for (TNetObjectIDs::const_iterator noiter = cmiter->second.begin(); noiter != cmiter->second.end(); ++noiter)
			{
				...
				for (i32 j = 0; j < NumAspects; j++)
				{
					if (objx.vAspectData[j] != CMementoMemoryUpr::InvalidHdl)
					{
						muClassObjectsAspectData.required += MMM().GetHdlSize(objx.vAspectData[j]);
					}
					if (objx.vRemoteAspectData[j] != CMementoMemoryUpr::InvalidHdl)
					{
	...
	if (CNetCVars::Get().netDebugEntityInfo)
	{
		...
		tukk pClassName = CNetCVars::Get().netDebugEntityInfoClassName->GetString();
			...
			if (pRenderAuxGeom)
			{
				for (u32 pass = 0; pass < 2; pass++)
				{
					...
					while (IEntity* pEntity = it->Next())
					{
						...
						if ((pClassName[0] == 0) || DrxStringUtils::stristr(pEntity->GetClass()->GetName(), pClassName))
						{
...
void CNetContextState::AddToFreeObjects(u16 id)
{
	CNetCVars& netCVars = CNetCVars::Get();
