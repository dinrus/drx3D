...
//  Описание: non-sequential receive protocol
//
//	История:
...
/*
   static tukk  ReliabilityMessage( INetMessage * pMsg )
   {
 ...
 */
//static u32k TimestampBits = 64;
static i32k LogMaxMessagesPerPacket = 16;
static i32k MaxMessagesPerPacket = 1 << LogMaxMessagesPerPacket;

static i32k SequenceNumberBits = 8;
static u32k SequenceNumberMask = (1u << SequenceNumberBits) - 1u;
static u32k SequenceNumberDiameter = 1u << SequenceNumberBits;
static u32k SequenceNumberRadius = 1u << (SequenceNumberBits - 1);

static const CTimeValue IncomingTimeoutLength = 0.033f;

#if ENABLE_DEBUG_KIT || ENABLE_CORRUPT_PACKET_DUMP
static void DumpBytes(u8k* p, size_t len)
{
	char l[256];
...
CCTPEndpoint::SBigEndpointState::SBigEndpointState(size_t acks, CMessageMapper& msgMapper, Rijndael::Direction dir, u8k* key, u8* initVec)
#elif ENCRYPTION_STREAMCIPHER
CCTPEndpoint::SBigEndpointState::SBigEndpointState(size_t acks, CMessageMapper & msgMapper, u8k * key, i32 keyLen)
#else
...
#endif
{
	...
	for (u32 i = 0; i < UPDATEOBJECT_NUM_HIGH_BIT_MESSAGE_IDS; i++)
	{
		char idName[64];

		drx_sprintf(idName, "CClientContextView:UpdateAspect%d", i);
		...
		drx_sprintf(idName, "CServerContextView:UpdateAspect%d", i);
...
#if USE_ARITHSTREAM
ILINE void CCTPEndpoint::SBigEndpointState::WriteMsgIDData(CCommOutputStream& stm, u32 id)
{
...
ILINE u32 CCTPEndpoint::SBigEndpointState::ReadMsgIDData(CCommInputStream& stm)
{
	u32 msg = m_MsgAlphabet.ReadSymbol(stm);
	#if DOUBLE_WRITE_MESSAGE_HEADERS
	u32 msg2 = m_MsgAlphabet.ReadSymbol(stm);
	NET_ASSERT(msg == msg2);
	#endif
...
#else
ILINE void CCTPEndpoint::SBigEndpointState::WriteMsgIDData(CNetOutputSerializeImpl& stm, u32 id, i32 numBits)
{
...
ILINE u32 CCTPEndpoint::SBigEndpointState::ReadMsgIDData(CNetInputSerializeImpl& stm, i32 numBits)
{
	u32 msg = stm.ReadBits(numBits);
	#if DOUBLE_WRITE_MESSAGE_HEADERS
	u32 msg2 = stm.ReadBits(numBits);
	NET_ASSERT(msg == msg2);
	#endif
...
ILINE void CCTPEndpoint::SBigEndpointState::SwitchTable(u32 id)
{
	for (u32 i = 0; i < NUM_MESSAGEID_TABLE_CHANGE_DATAS; i++)
	{
...
//void CCTPEndpoint::SBigEndpointState::GetMemoryStatistics(IDrxSizer *pSizer)
//{
...
//void CCTPEndpoint::CBigEndpointStateUpr::GetMemoryStatistics(IDrxSizer *pSizer)
//{
...
// Reset
ILINE void CCTPEndpoint::CState::Reset(CBigEndpointStateUpr* pBigMgr)
{
	m_nAcks = 0;
	m_nAckedPackets = 0;
	m_nPackets = 0;
...
ILINE void CCTPEndpoint::COutputState::Reset(CBigEndpointStateUpr* pBigMgr)
{
...
ILINE void CCTPEndpoint::CInputState::Reset(CBigEndpointStateUpr* pBigMgr)
{
	CState::Reset(pBigMgr);
	m_nValidSeq = 0;
}
...
// GetMemoryStatistics
//void CCTPEndpoint::CState::GetMemoryStatistics(IDrxSizer *pSizer)
//{
...
//void CCTPEndpoint::COutputState::GetMemoryStatistics(IDrxSizer *pSizer)
//{
...
// Clone
ILINE void CCTPEndpoint::CState::Clone(CBigEndpointStateUpr* pBigMgr,
                                       const CState& s)
{
	m_nAcks = s.m_nAcks;
...
void CCTPEndpoint::COutputState::Clone(CBigEndpointStateUpr* pBigMgr,
...
void CCTPEndpoint::CInputState::Clone(CBigEndpointStateUpr* pBigMgr,
                                      const CCTPEndpoint::CInputState& s, u32 nSeq)
{
	CCTPEndpoint::CState::Clone(pBigMgr, s);
...
// acks & nacks
ILINE u32 CCTPEndpoint::CState::AddAck(bool bAck)
{
	if (bAck)
		m_nAcks++;
	m_nAcksThisPacket++;
...
#if USE_ARITHSTREAM
bool CCTPEndpoint::CState::ReadAck(CCommInputStream& stm, bool& bAck, u32& nSeq, bool& recvAckNeeded)
#else
bool CCTPEndpoint::CState::ReadAck(CNetInputSerializeImpl& stm, bool& bAck, u32& nSeq, bool& recvAckNeeded)
#endif
{
#if DEBUG_STREAM_INTEGRITY
	u8 stmBits = stm.ReadBits(8);
...
#else
	u8 symbol = stm.ReadBits(ACK_TYPE_NUM_BITS);
#endif
...
// message id's
#if USE_ARITHSTREAM
ILINE bool CCTPEndpoint::CState::WriteMsgId(CCommOutputStream& stm, u32 id, CStatsCollector* pStats, tukk name)
#else
ILINE bool CCTPEndpoint::CState::WriteMsgId(CNetOutputSerializeImpl& stm, u32 id, CStatsCollector* pStats, tukk name)
#endif
{
	if (!m_pBigState)
		return false;
...
#if USE_ARITHSTREAM
ILINE u32 CCTPEndpoint::CState::ReadMsgId(CCommInputStream& stm)
#else
ILINE u32 CCTPEndpoint::CState::ReadMsgId(CNetInputSerializeImpl& stm)
#endif
{
#if DEBUG_STREAM_INTEGRITY
	u32 check = stm.ReadBits(8);
	NET_ASSERT(check == 73);
#endif

#if USE_ARITHSTREAM
	u32 msg = m_pBigState->ReadMsgIDData(stm);
#else
	u32 msg;
...
CCTPEndpoint::CCTPEndpoint(CMementoMemoryUprPtr pMMM) :
	...
	m_freeSentElem(CCTPEndpoint::InvalidSentElem)
{
	...
	drx_sprintf(filename, "tfrc_endpoint_%08x.log", (u32)this);
...
void CCTPEndpoint::Init(CNetChannel* pParent)
{
	...
	{
		char buf[30];
		drx_sprintf(buf, "output.%d.log", ++N);
		m_log_output = fxopen(buf, "wt");
		drx_sprintf(buf, "input.%d.log", N);
...
void CCTPEndpoint::MarkNotUserSink(INetMessageSink* pSink)
{
	for (size_t i = 0; i < m_MessageSinks.size(); i++)
		if (m_MessageSinks[i].pSink == pSink)
			m_MessageSinks[i].bDinrusXNetwork = true;
}

void CCTPEndpoint::GetMemoryStatistics(IDrxSizer* pSizer, bool countingThis)
{
	...
	pSizer->AddObject(&m_timedOutPackets, m_timedOutPackets.size() * sizeof(u32));
...
bool CCTPEndpoint::CallAddSubstitute(INetSendable* pMsg, i32 numAfterHandle, const SSendableHandle* afterHandle, SSendableHandle* handle, bool subs)
{
...
CTimeValue CCTPEndpoint::GetNextUpdateTime(CTimeValue nTime)
{
	...
	i32 age = m_nOutputSeq - m_nInputAck;
...
void CCTPEndpoint::SendPacketsIfNecessary(CTimeValue nTime, bool isDisconnecting, bool bAllowUserSend, bool bForce, bool bFlush)
{
	...
	do
	{
		...
		u16 maxOutputSize = m_PacketRateCalculator.GetMaxPacketSize();
#else
		u16 maxOutputSize = m_PacketRateCalculator.GetMaxPacketSize(nTime);
		...
		if (bSend)
		{
			...
			if (bResend)
			{
				...
				m_PacketRateCalculator.SentPacket(nTime, m_nOutputSeq, static_cast<u16>(outSize));
				m_nRepeated++;
				m_nSent++;
			}
			else
...
#if !NEW_BANDWIDTH_MANAGEMENT
				u16 capOutputSize = maxOutputSize;
...
{
	ASSERT_GLOBAL_LOCK;
	DRX_PROFILE_REGION(PROFILE_NETWORK, "CCTPEndpoint:Update");
...
bool CCTPEndpoint::AckPacket(CTimeValue nTime, u32 nSeq, bool bOk)
{
	...
	{
		char msg[512];
		drx_sprintf(msg, "Received an ack for a packet newer than what we've sent (received %u, at %u); disconnecting", nSeq, m_nOutputSeq);
...
{
public:
	CSequenceNumberParser(u8k* normBytes, u32 inputSeq, bool inSync, size_t pktLen)
	{
		ok = false;
		reorderings = 0;
		...
		u32 nBasisSeqTag = Frame_HeaderToID[normBytes[0]];
		...
		u32 nCurrentSeqTag = UnseqBytes[normBytes[1]];
		...
		//DEBUG:
		u32 nProperSeq = *(u32*)(normBytes + pktLen + 2);
	...
	bool IsCompatibleWithBasis(const CInputState& basis, u32 lastBasisSeq) const
	{
...
private:
	bool CheckValidResult(u32 inputSeq)
	{
		// check our location
...
static u8 QuickHashBytes(u8k* pSrc, u32 len)
{
	u8 hash = 0;
	for (u32 i = 0; i < len; i++)
		hash = 5 * hash + pSrc[i];
...
{
public:
	CParsePacketContext(u8k* normBytes, u32 pktLen, CInputState& state, CNetInputSerializeImpl& stmImpl, bool inSync, const CSequenceNumberParser& seq, u32 timeFraction32)
		: m_bCompleted(false)
		, m_numMessages(0)
		, m_state(state)
		, m_stmImpl(stmImpl)
	...
	ILINE u32 GetPktLen() const
	{
		return m_pktLen;
	}
	ILINE u8k* GetPktBytes() const
	{
		return m_normBytes;
	}
	...
	ILINE u32 GetCurrentSeq() const
	{
		return m_seq.nCurrent;
	}

	ILINE u32 GetBasisSeq() const
	{
		return m_seq.nBasis;
	}

	ILINE u32 GetTimeFraction32() const
	{
...
void CCTPEndpoint::QueueForLater(CTimeValue nTime, CAutoFreeHandle& hdl, u32 nCurrentSeq)
{
...
static bool CRCCheck(CNetInputSerializeImpl& stm)
	#endif
{
	u8 crcStream = stm.GetCRC();
	u8 crcWritten = stm.ReadBits(8);
...
void CCTPEndpoint::DoPacketDump()
{
	if (m_corruptPacketDumpData.processingPacket && !m_corruptPacketDumpData.doingDump)
	{
		...
		i32 startPacketReadDebugOutput = CNetCVars::Get().packetReadDebugOutput;
		i32 startLogLevel = CNetCVars::Get().LogLevel;
	#if LOG_INCOMING_MESSAGES
		i32 startLogNetMessages = CNetCVars::Get().LogNetMessages;
	#endif
		...
		NetLog("Packet Data");
		DumpBytes((u8*)MMM().PinHdl(hdl.Peek()), MMM().GetHdlSize(hdl.Peek()));
...
void CCTPEndpoint::ProcessPacket(CTimeValue nTime, CAutoFreeHandle& hdl, bool bQueueing, bool inSync)
{
	...
	ASSERT_GLOBAL_LOCK;
	DRX_PROFILE_REGION(PROFILE_NETWORK, "CCTPEndpoint:ProcessPacket");
	...
	u32k pktSize = MMM().GetHdlSize(hdl.Peek());
	...
	u8* normBytes = (u8*)MMM().PinHdl(hdl.Peek());
...
#endif
	{
		for (u32 i = m_nLastBasisSeq; i < seq.nBasis; i++)
	...
	u8 hash = QuickHashBytes(normBytes, pktLen);
	...
	u32 sendTime = stmImpl.GetInput().ReadBits(32);
...
void CCTPEndpoint::ProcessPacket_EndOfStream(CParsePacketContext& ppctx)
{
	...
	u8 signingKey = ppctx.GetCommStream().ReadBits(8);
	u8 pktSignedByte = ppctx.GetPktBytes()[ppctx.GetPktLen()];
...
void CCTPEndpoint::ProcessPacket_NormalMessage(u32 msg, CParsePacketContext& ppctx)
{
	ENSURE_REALTIME;
...
public:
	ILINE CMessageSender(CCTPEndpoint* pEndpoint, COutputState& State,
	                     size_t nSize, i32& nMessages, CStatsCollector* pStats,
	                     INetChannel* pChannel, CTimeValue now, u32 timeFraction32) :
	...
	EMessageSendResult HeaderBody(uk p)
	{
		return m_pEndpoint->m_pParent->WriteHeader(this);
	}

	EMessageSendResult FooterBody(uk p)
	{
		return m_pEndpoint->m_pParent->WriteFooter(this);
	}

	EMessageSendResult MessageBody(uk p)
	{
	...
	u32 GetStreamSize()
	{
		...
		return (u32)GetStream()->GetBitSize();
#else
		return GetStream()->GetApproximateSize();
#endif
	}

	ILINE EWriteMessageResult DoWrite(EMessageSendResult (CMessageSender::* sendBody)(uk ), uk pUser)
	{
...
private:
	...
	i32  m_nMessagesInBlock;

	bool m_bHadUrgentData;
	bool m_bIsCorrupt;
};

u32 CCTPEndpoint::SendPacket(CTimeValue nTime, const SSendPacketParams& params)
{
	ASSERT_GLOBAL_LOCK;
	MMM_REGION(m_pMMM);

	DRX_PROFILE_REGION(PROFILE_NETWORK, "CCTPEndpoint:SendPacket");
	...
	u8 nBasisSeqTag = m_nOutputSeq - m_nInputAck - 1;
...
#if ENABLE_DEBUG_KIT
	SDebugPacket debugPacket(-(i32)m_pParent->GetLoggingChannelID(), true, m_nOutputSeq);
#endif
	...
	u8 signingKey = (drx_random_uint32() >> 6) & 0xff;
	...
	u32 timeToSend = (u32)(gEnv->pTimer->GetReplicationTime() * REPLICATION_TIME_PRECISION);
	...
	nSent = u32(m_outputStreamImpl.GetOutput().Flush());

	if (m_bWritingPacketNeedsInSyncProcessing)
	{
		u8& hdr = m_assemblyBuffer[0];
	...
	// sign
	i32 nBig = nSent + 1 + 2; // one byte for the header that the range encoder needs, 2 bytes for signing
	...
	u8* normBytes = m_assemblyBuffer;
	u8* endBytes = normBytes + nBig - 2;
	endBytes[0] = signingKey;
	u8 hash = QuickHashBytes(normBytes, nBig - 2);
...
#if DEBUG_SEQUENCE_NUMBERS
	*(u32*)(normBytes + nBig) = m_nOutputSeq;
#endif
...
bool CCTPEndpoint::LookupMessage(tukk name, SNetMessageDef const** ppDef, INetMessageSink** ppSink)
{
	bool result;
	size_t nProto;
...
void CCTPEndpoint::ChangeSubscription(ICTPEndpointListener* pListener, u32 eventMask)
{
	for (std::vector<SListener>::iterator iter = m_listeners.begin(); iter != m_listeners.end(); ++iter)
	{
...
tukk CCTPEndpoint::GetName(void) const
{
	return (m_pParent != NULL) ? m_pParent->GetName() : "<unknown>";
}

void CCTPEndpoint::GetBandwidthStatistics(u32 channelIndex, SBandwidthStats* pStats)
{
	...
	stats.m_ping = static_cast<u32>(m_PacketRateCalculator.GetPing(false) * 1000.0f);
	stats.m_pingSmoothed = static_cast<u32>(m_PacketRateCalculator.GetPing(true) * 1000.0f);
	...
	stats.m_desiredPacketRate = static_cast<u32>(m_PacketRateCalculator.GetPacketRate(IsIdle(), g_time));
#endif // NEW_BANDWIDTH_MANAGEMENT
...
void CCTPEndpoint::ChannelStatsDraw()
{
	...
	i32 frame = gEnv->pRenderer->GetFrameID();
	if (frame != yframe)
	{
		yframe = frame;
		y = drawScale * 30.f;
...
#if !NEW_BANDWIDTH_MANAGEMENT
	u16 maxsz = m_PacketRateCalculator.GetMaxPacketSize(g_time);
#else
	u16 maxsz = m_PacketRateCalculator.GetMaxPacketSize();
#endif // !NEW_BANDWIDTH_MANAGEMENT
	y += (drawScale * 10.0f);
	m_queue.DrawLabel(drawScale * 10.f, y, cyan, m_pParent->GetName());

	CTimeValue backoffTime;
	typedef DrxFixedStringT<64> TBOStr;
	...
	y += (drawScale * 10.0f);
	m_queue.DrawLabel(0.0f, y, white, "%4d", static_cast<u32>(m_PacketRateCalculator.GetPing(false) / 1000.0f));
	m_queue.DrawLabel(drawScale * 40.0f, y, white, "[%4d]", static_cast<u32>(m_PacketRateCalculator.GetPing(true) / 1000.0f));
...
void CCTPEndpoint::SentElem(u32& head, const SSendableHandle& hdl)
{
	ASSERT_GLOBAL_LOCK;
	u32 cur;
...
void CCTPEndpoint::AckMessages(u32& head, u32 nSeq, bool ack, bool clear)
{
	static const size_t MESSAGE_BUFFER_SIZE = 4096;
	static DrxFixedArray<SSendableHandle, MESSAGE_BUFFER_SIZE / sizeof(SSendableHandle)> temp;
	u32 cur = head;
...
tukk CCTPEndpoint::GetCurrentProcessingMessageDescription()
{
