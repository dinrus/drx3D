...
bool MakeSocketNonBlocking(DRXSOCKET sock)
{
	...
	u64 nTrue = 1;
	if (ioctlsocket(sock, FIONBIO, &nTrue) == DRX_SOCKET_ERROR)
		return false;
#elif DRX_PLATFORM_ORBIS
	i32 nonblocking = 1;
	if (DrxSock::setsockopt(sock, SOL_SOCKET, SO_NBIO, (tukk)&nonblocking, sizeof(nonblocking)) == -1)
		return false;
#else
...
template<class T>
static bool SetSockOpt(DRXSOCKET s, i32 level, i32 optname, const T& value)
{
	return 0 == DrxSock::setsockopt(s, level, optname, (tukk)&value, sizeof(T));
}
...
bool CUDPDatagramSocket::Init(SIPv4Addr addr, u32 flags)
{
	ASSERT_GLOBAL_LOCK;
	...
	if (m_pUDPFragBuffer)
		delete[] m_pUDPFragBuffer;
	m_pUDPFragBuffer = new u8[FRAG_MAX_MTU_SIZE];
	if (m_pFragmentedPackets)
		delete[] m_pFragmentedPackets;
	m_pFragmentedPackets = new SFragmentedPacket[FRAG_NUM_PACKET_BUFFERS];
	for (u32 a = 0; a < FRAG_NUM_PACKET_BUFFERS; a++)
	{
	...
	if (flags & eSF_Online)
	{
		m_pSockIO = &CNetwork::Get()->GetExternalSocketIOUpr();
	}
	...
	if (initResult)
	{
#if 0 && DRX_PLATFORM_WINDOWS && !CHECK_ENCODING
...
{
	return m_socket == DRX_INVALID_SOCKET;
}

DRXSOCKET CUDPDatagramSocket::GetSysSocket()
{
	return m_socket;
}

bool CUDPDatagramSocket::Init(i32 af, u32 flags, uk pSockAddr, size_t sizeSockAddr)
{
...
#endif // (USE_LOBBYIDADDR)
	{
	...
	#endif
	#if USE_PSN
		if ((flags & eSF_Online) && (static_cast<DRXSOCKADDR_IN*>(pSockAddr)->sin_port == htons(DRX_PSN_PORT)))
		{
			m_protocol = IPROTO_UDPP2P_SAFE;
			dgram = DRX_ORBIS_SOCKET_DGRAM;
		}
	#endif
#endif
...
#if USE_PSN
		if (dgram == DRX_ORBIS_SOCKET_DGRAM)
		{
	#if DRX_PLATFORM_ORBIS
			SetSockOpt(m_socket, SOL_SOCKET, DRX_ORBIS_SOCKET_USECRYPTO, TRUE);
	#endif
		}
#endif
		...
		SFlag allflagsudp[] =
		{
			...
			//{ SOL_SOCKET, SO_RCVBUF, eSF_BigBuffer, ((CNetwork::Get()->GetSocketIOUpr().caps & eSIOMC_NoBuffering)==0) * 1024*1024, 4096, eF_Ignore },
			{ SOL_SOCKET, SO_RCVBUF,            eSF_BigBuffer,        SOCKET_BUFFER_SIZE, SOCKET_BUFFER_SIZE, eF_Ignore },
			//{ SOL_SOCKET, SO_SNDBUF, eSF_BigBuffer, ((CNetwork::Get()->GetSocketIOUpr().caps & eSIOMC_NoBuffering)==0) * 1024*1024, 4096, eF_Ignore },
			{ SOL_SOCKET, SO_SNDBUF,            eSF_BigBuffer,        SOCKET_BUFFER_SIZE, SOCKET_BUFFER_SIZE, eF_Ignore },
		...
		case IPROTO_UDPP2P_SAFE:
			...
			numflags = DRX_ARRAY_COUNT(allflagsp2pudp);
			break;
#endif // #if USE_PSN

		case IPPROTO_UDP:
			allflags = allflagsudp;
			numflags = DRX_ARRAY_COUNT(allflagsudp);
			break;

#if USE_LIVE
		case IPPROTO_VDP:
			allflags = allflagsvdp;
			numflags = DRX_ARRAY_COUNT(allflagsvdp);
			break;
#endif
		}
		...
		{
			// Change DrxSock::bind information to be udp2p2 compatible - duplicated to avoid trashing address passed in
			memset(&inP2PSock, 0, sizeof(inP2PSock));
			inP2PSock.sin_family = AF_INET;
			inP2PSock.sin_port = htons(DRX_PSN_PORT);
...
#endif
		i32 bindret = DrxSock::bind(m_socket, static_cast<DRXSOCKADDR*>(pSockAddr), sizeSockAddr);
		if (bindret)
			return InitWinError();
	}
#if (USE_LOBBYIDADDR)
...
{
	i32 error = DrxSock::GetLastSocketError();
	LogWinError(error);
}

void CUDPDatagramSocket::LogWinError(i32 error)
{
	// ugly
	tukk msg = ((CNetwork*)(gEnv->pNetwork))->EnumerateError(MAKE_NRESULT(NET_FAIL, NET_FACILITY_SOCKET, error));
...
void CUDPDatagramSocket::GetSocketAddresses(TNetAddressVec& addrs)
{
...
#endif // (USE_LOBBYIDADDR)
	{
		...
		if (0 == DrxSock::getsockname(m_socket, (DRXSOCKADDR*)addrBuf, (DRXSOCKLEN_T*)&addrLen))
		{
			TNetAddress addr = ConvertAddr((DRXSOCKADDR*)addrBuf, addrLen);
			bool valid = true;
			...
			else if (SSockAddrStorageAddr* pSockAddrStorage = stl::get_if<SSockAddrStorageAddr>(&addr))
			{
				...
				default:
					DrxFatalError("CUDPDatagramSocket::GetSocketAddresses: Unknown address family %u", pSockAddrStorage->addr.ss_family);
					valid = false;
					break;
				}
			}
...
#endif // DRX_PLATFORM_WINDOWS || DRX_PLATFORM_DURANGO || DRX_PLATFORM_ORBIS
		...
		USockAddr sockAddr;
		DRXSOCKLEN_T sockAddrSize = sizeof(sockAddr);

		if (0 != DrxSock::getsockname(m_socket, (DRXSOCKADDR*)&sockAddr, &sockAddrSize))
		{
			InitWinError();
			return;
		}
		if (sockAddrSize == sizeof(DRXSOCKADDR_IN))
		{
		...
		{
	#ifdef _DEBUG
			DrxFatalError("Unhandled DRXSOCKADDR type");
	#endif
			return;
		}

		char hostnameBuffer[NI_MAXHOST];
		if (!DrxSock::gethostname(hostnameBuffer, sizeof(hostnameBuffer)))
		...
		{
			DRXHOSTENT* hp = DrxSock::gethostbyname(iter->c_str());
			if (hp)
			{
				...
				case AF_INET:
					{
						...
						{
							addr.addr = ntohl(*(u32*)hp->h_addr_list[i]);
...
ESocketError CUDPDatagramSocket::Send(u8k* pBuffer, size_t nLength, const TNetAddress& to)
{
	...
	if (nLength > FRAG_MAX_MTU_SIZE)
	{
		if (nLength > ((FRAG_MAX_MTU_SIZE - fho_FragHeaderSize) * FRAG_MAX_FRAGMENTS))
		{
			...
			NetQuickLog(true, 10.f, "[Fragmentation]: Packet too big for fragmentation buffer, Packet Size = %d", static_cast<i32>(nLength));
...
ESocketError CUDPDatagramSocket::SendVoice(u8k* pBuffer, size_t nLength, const TNetAddress& to)
{
#if ENABLE_DEBUG_KIT
	CAutoCorruptAndRestore acr(pBuffer, nLength, CVARS.RandomPacketCorruption == 1);
#endif
...
void CUDPDatagramSocket::OnRecvFromComplete(const TNetAddress& from, u8k* pData, u32 len)
{
	u32k thisRecvBytes = len + UDP_HEADER_SIZE;
	u32k thisRecvBits = thisRecvBytes << 3;
...
void CUDPDatagramSocket::ClearFragmentationEntry(u32 index, TFragPacketId id, const TNetAddress& from)
{
...
void CUDPDatagramSocket::SendFragmented(u8k* pBuffer, size_t nLength, const TNetAddress& to)
{
	...
	while (nLength)
	{
		i32 blkSize = std::min(nLength, (size_t) FRAG_MAX_MTU_SIZE - fho_FragHeaderSize);
...
u8k* CUDPDatagramSocket::ReceiveFragmented(const TNetAddress& from, u8k* pData, u32& len)
{
	u8 nType = Frame_HeaderToID[pData[fho_HeaderID]];
	if (nType == eH_Fragmentation)
	{
		...
		i32 bufferIndex = FindBufferIndex(from, fragmentedPacketID);
		...
		u8 fragmentIndex = ((*pFragInfo) >> FRAG_SEQ_BIT_SIZE);
		...
		u8 rSeq = fragmentIndex;
		u32 seqStart = rSeq * (FRAG_MAX_MTU_SIZE - fho_FragHeaderSize);
		u32 seqLength = len - fho_FragHeaderSize;
...
void CUDPDatagramSocket::DiscardStaleEntries()
{
	...
	i32 sources_occupied[FRAG_NUM_PACKET_BUFFERS]; // number of packets per source

	uint sources_oldest_packets_seq[FRAG_NUM_PACKET_BUFFERS];  // smallest packet id per source
	i32 sources_oldest_packets_index[FRAG_NUM_PACKET_BUFFERS]; // smallest packet id per source

	// collect unique sources
	i32 last_source = 0;
	for (i32 i = 0; i != FRAG_NUM_PACKET_BUFFERS; ++i)
	{
...
u8 CUDPDatagramSocket::FindBufferIndex(const TNetAddress& from, TFragPacketId id)
{
	DiscardStaleEntries();

	i32 match = FRAG_NUM_PACKET_BUFFERS, unused = FRAG_NUM_PACKET_BUFFERS;
...
void CUDPDatagramSocket::SocketSend(u8k* pBuffer, size_t nLength, const TNetAddress& to)
{
...
#if NET_MINI_PROFILE || NET_PROFILE_ENABLE
		CSocketIOUpr* pSockIO = (CSocketIOUpr*)m_pSockIO;
