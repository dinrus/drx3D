...
CLanQueryListener::~CLanQueryListener()
{
	...
	if (m_pSocket)
	{
		...
		CNetwork::Get()->GetExternalSocketIOUpr().FreeDatagramSocket(m_pSocket);
	}
	m_bDead = true;
}
...
{
	m_pSocket = CNetwork::Get()->GetExternalSocketIOUpr().CreateDatagramSocket(TNetAddress(SIPv4Addr()), eSF_BroadcastSend);
	if (m_pSocket)
...
void CLanQueryListener::GetMemoryStatistics(IDrxSizer* pSizer)
{
...
void CLanQueryListener::OnPacket(const TNetAddress& addr, u8k* pBuffer, u32 nLength)
{
	if (nLength >= 1)
	{
		switch (pBuffer[0])
...
{
	DrxLogAlways("LanQueryListener received ESocketError code %i.", (i32)error);
}

void CLanQueryListener::TimerCallback(NetTimerId id, uk pUser, CTimeValue time)
{
...
void CLanQueryListener::ProcessPongFrom(u8k* buffer, size_t bufferLength, const TNetAddress& addr)
{
	DRX_PROFILE_FUNCTION(PROFILE_NETWORK);
...
#else
	if (1 != _snscanf((tukk)buffer + PONG_LENGTH, bufferLength - PONG_LENGTH, "%I64d", &serNumber))
		return;
#endif
	...
	u32 ping = i32((now - when.GetValue()) * 0.01f);
...
void CLanQueryListener::ProcessResultFrom(u8k* buffer, size_t bufferLength, const TNetAddress& addr)
{
	DRX_PROFILE_FUNCTION(PROFILE_NETWORK);
	...
	string temp((tukk)buffer, (tukk)(buffer + bufferLength));
...
void CLanQueryListener::SendPingTo(tukk addr)
{
	SCOPED_GLOBAL_LOCK;
...
void CLanQueryListener::GQ_SendPingTo(CNameRequestPtr pReq)
{
	...
	drx_sprintf(buffer, "%s %" PRIi64, "PING",
#if defined(__GNUC__)
	            (long long)serNumber
#else
	            serNumber
	...
	m_pSocket->Send((u8k*)buffer, strlen(buffer), addrVec[0]);
...
void CLanQueryListener::NQ_AddPong(string address, u32 ping)
{
