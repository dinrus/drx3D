...
CNetCVars::CNetCVars()
{
	...
	static i32k DEFAULT_CHEAT_PROTECTION = 3;
#else
	#if ENABLE_DEBUG_KIT
	static i32k DEFAULT_CHEAT_PROTECTION = 3;
	#else
	static i32k DEFAULT_CHEAT_PROTECTION = 0;//0 for GameK01 for now
	#endif
	...
	REGISTER_CVAR2_DEV_ONLY("net_enable_watchdog_timer", &enableWatchdogTimer, 0, VF_NULL, "Enable watchdog timer. Not needed if DrxLobby is being used.");
...
void CNetCVars::Stall(IConsoleCmdArgs* pArgs)
{
	...
	else
	{
		...
		SCOPED_GLOBAL_LOCK;
		DrxSleep((DWORD)(-1000.0f * stallTime));
	}
}
#endif
...
{
	CInternetSimulatorSocket::LoadXMLProfiles((tukk)val->GetString());
}
...
void CNetCVars::SetChannelPerformanceMetrics(IConsoleCmdArgs* pArguments)
{
	...
	if (pNetNub)
	{
		...
		u32 id = 0;
		u32 argument = 1;
		u32 argc = pArguments->GetArgCount();
