...
public:
	...
	bool ReadValue(CCommInputStream& in, Ang3& value, CArithModel* pModel, u32 age) const
	{
		for (i32 i = 0; i < 3; i++)
		{
			u32 q = in.ReadBits(m_floats[i].GetNumBits());
	...
	bool WriteValue(CCommOutputStream& out, Ang3 value, CArithModel* pModel, u32 age) const
	{
		for (i32 i = 0; i < 3; i++)
		{
			u32 q = m_floats[i].Quantize(value[i]);
	...
	bool WriteValue(CNetOutputSerializeImpl* out, Ang3 value, u32 age) const
	{
