...
static u32k CONTEXT_EVENTS = eNOE_UnbindObject | eNOE_Reset | eNOE_SyncWithGame_End | eNOE_SyncWithGame_Start | eNOE_SendVoicePackets | eNOE_ChangeContext;

static i32k UPDATE_RATE = 10;
...
{
	return CVoiceUpr::CreateEncoder(CVARS.pVoiceCodec->GetString());
}

static IVoiceDecoder* CreateDecoder()
{
	return CVoiceUpr::CreateDecoder(CVARS.pVoiceCodec->GetString());
}
...
public:
	...
	~CVoiceGroup()
	{
		if (m_pVoiceContext)
		{
			DrxAutoLock<CVoiceContext::TVoiceMutex> lock(m_pVoiceContext->m_Mutex);
...
i32 CVoiceContext::m_voiceDebug = 0;
...
{
	DrxAutoLock<TVoiceMutex> lock(m_Mutex);
	if (!witness)
		return;
...
CVoiceContext::~CVoiceContext()
{
	DrxAutoLock<TVoiceMutex> lock(m_Mutex);
	Die();
	for (u32 i = 0; i < m_voiceGroups.size(); i++)
	{
...
void CVoiceContext::OnObjectEvent(CNetContextState* pState, SNetObjectEvent* pEvent)
{
	...
	case eNOE_SyncWithGame_End:
		{
			for (u32 i = 0; i < m_newDataReaders.size(); )
			{
			...
			for (TEncSessions::iterator it = m_encSessions.begin(); it != m_encSessions.end(); ++it)
			{
				...
				if (obj.main && obj.main->userID)
				{
					...
					u32 sz = std::min(pReader->GetSampleCount(), 1024U);
					i16* pSamples = pReader->GetSamples();
					...
					ed.m_pSession->AddVoiceFragment((i32)sz, pSamples, &sink);
			...
			if (m_voiceDebug > 0)
			{
				...
				{
					IRenderAuxText::Draw2dLabel(100, y, 1, clr, false, "VG%d %d players", (i32)i, (i32)m_voiceGroups[i]->m_objects.size());
					y += 10.0f;
					//          if(m_voiceDebug > 1)
					//          {
...
bool CVoiceContext::GetDataFor(EntityId id, u32 numSamples, i16* pData)
{
	if (!CVARS.EnableVoiceChat)
		return false;
	...
	i->second.m_pSession->GetSamples(i32(numSamples), pData);
...
void CVoiceContext::TimerCallback(NetTimerId, uk obj, CTimeValue time)
{
			...
			for (TObjects::iterator iterFrom = m_allObjects.begin(); iterFrom != m_allObjects.end(); ++iterFrom)
			{
				...
				if (!std::binary_search(m_mutes.begin(), m_mutes.end(), p))
				{
					...
					if (iterProx != m_proximity.end() && *iterProx == p)
						...
						for (u32 i = 0; op != eRO_Send && i < m_voiceGroups.size(); i++)
						{
							// check voice groups
