...
static i32 s_netProfileLogging = 0;
static i32 s_netProfileBudgetLogging = 0;
static bool s_budgetExceeded = false;
static i32 s_netProfileWorstNumChannels = 0;
static i32 s_netProfileShowSocketMeasurements = 0;
static i32 s_netProfileEnable = 0;
static i32 s_netProfileUntouchedDelay = 1;
static i32 s_netProfileNumRemoteClients = 0;
static bool s_netProfileCountReadBits = false;

CNetProfileStackEntryList::CNetProfileStackEntryList(u32 numItems)
{
...
void netProfileInitEntry(SNetProfileStackEntry* parent, SNetProfileStackEntry* child, tukk name, float budget, bool rmi)
{
...
void netProfileRemoveUntouchedNodes(SNetProfileStackEntry* parent)
{
	...
	while (current)
	{
		...
		if (current->m_untouchedCounter > (u32)s_netProfileUntouchedDelay)
		{
			if (previous == NULL)
			{
				//-- start of child list.
...
bool netProfileGetChildFromCurrent(tukk name, SNetProfileStackEntry** ioEntry, bool rmi)
{
...
void netProfileRegisterBeginCall(tukk name, SNetProfileStackEntry** ioEntry, float budget, bool rmi)
{
...
void calculateEntryStats(SNetProfileStackEntry* entry, u32 parentWriteBits, i32 numChannels)
{
	...
	u32 serialisedBits = entry->m_bits - entry->m_rmibits;
...
void netProfileEndProfile()
{
	...
	float bitsTokbits = floatTotalBits * one1024th;
	u32 serialisedBits = s_netProfileRoot->m_bits - s_netProfileRoot->m_rmibits;
	float floatSerialisedBits = float(serialisedBits);
...
FILE* netProfileOpenLogFile(tukk name, i32* firstTime)
{
	FILE* fout = NULL;
...
static void netProfilePrintEntry(FILE* file, const SNetProfileStackEntry* entry, i32 depth)
{
	DrxFixedStringT<1024> stringBufferTemp;
	DrxFixedStringT<1024> stringBuffer;
	...
	i32 doTabs = NUM_TABS - (stringBuffer.length() / TAB_SIZE);
	for (i32 j = 0; j < doTabs; j++)
	{
		stringBuffer.append("\t");
	}
...
void netProfileFormatSocketView(FILE* file)
{
	...
	i32 doTabs = NUM_TABS - (strlen("socket") / TAB_SIZE);
	for (i32 i = 0; i < doTabs; i++)
	{
		tabs[i] = '\t';
	}
...
void netProfileFormatHierarchyView(FILE* file, SNetProfileStackEntry* root, i32 depth)
{
	SNetProfileStackEntry* entry = NULL;
...
void netProfileWriteLogFiles()
{
	...
	static i32 firstTimeBudgetLogging = 1;
	...
	tukk header[9] = { "LAST", "AVERAGE", "Calls", "Seqbits/RMIbits", "Payload(Kbits)", "OnWire(Kbits)", "Worst(Kbits)", "Heir%", "Self%" };
	char tabs[NUM_TABS + 1] = { 0 };
	FILE* fout = NULL;

	for (i32 i = 0; i < NUM_TABS; i++)
	{
		tabs[i] = '\t';
	}
...
bool netProfileSocketMeasurementTick()
{
	...
	if ((when - g_socketBandwidth.last) > UPDATE_PERIOD)
	{
		...
		g_socketBandwidth.bandwidthStats.m_10secAvg.m_totalPacketsSent = (i32)g_socketBandwidth.numPacketsSent.GetAverage();
		...
		g_socketBandwidth.bandwidthStats.m_10secAvg.m_lobbyPacketsSent = (i32)g_socketBandwidth.numLobbyPacketsSent.GetAverage();
		...
		g_socketBandwidth.bandwidthStats.m_10secAvg.m_seqPacketsSent = (i32)g_socketBandwidth.numSeqPacketsSent.GetAverage();
