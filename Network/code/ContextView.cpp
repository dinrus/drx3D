...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание:  context views replicate state between contexts
   -------------------------------------------------------------------------
   История:
...
#include  <drx3D/Network/MementoHistory.h>
...
#include  <drx3D/Network/VoiceContext.h>
#include  <drx3D/Network/ServiceUpr.h>
#include  <drx3D/Network/IContextViewExtension.h>

#include  <drx3D/Network/UpdateMessage.h>
...
public:
	u32 CalcFlags(u32 flags, bool blockStateChange)
	{
		if (blockStateChange)
...
public:
	u32 CalcFlags(IRMIMessageBody* pBody, bool blockStateChange)
	{
		u32 flags = pBody->pMessageDef->parallelFlags;
		if (blockStateChange)
			flags |= eMPF_BlocksStateChange;
		else
			flags &= ~eMPF_BlocksStateChange;
...
#if STATS_COLLECTOR
		tukk grp = m_pBody->pMessageDef->description;
		while (tukk grp_2 = strchr(grp, ':'))
			grp = grp_2 + 1;
#endif
	...
	void UpdateState(u32 nFromSeq, ENetSendableStateUpdate bAck)
	{
...
tukk CContextView::GetStateName(EContextViewState state)
{
...
tukk CContextView::GetWaitStateName(EContextViewState state)
{
	static char buffer[eCVS_NUM_STATES][64];
	if (!buffer[state][0])
		drx_sprintf(buffer[state], "WaitFor_%s", GetStateName(state));
	return buffer[state];
}
	...
	m_allowVoice(true)
{
	...
	for (i32 i = 0; i < eH_NUM_HISTORIES; i++)
		m_history[i] = 0;
	...
	if (pState == ContextState())
	{
		...
		case eNOE_SyncWithGame_Start:
			...
			if (CNetCVars::Get().DebugConnectionState)
				CContextViewStateUpr::DumpLockers(Parent()->GetName());
#endif
			break;
...
{
public:
	CCET_UnlockStateChanges(CContextViewPtr pView, tukk name) : m_pView(pView), m_name(name)
	{
...
{
public:
	CCET_SafetySleep(CContextViewPtr pView, u32 length) : m_pView(pView), m_start(0), m_length(length) {}
...
void CContextView::GC_GetEstablishmentOrder()
{
	...
	{
		for (i32 i = eCVS_Begin; i <= eCVS_InGame; i++)
...
void CContextView::ExitState(EContextViewState state)
{
	...
	{
		state = EContextViewState(i32(state) + 1);
	}
	SetLocalState(state);
}
...
void CContextView::OnViewStateDisconnect(tukk message)
{
	if (Parent())
...
void CContextView::CompleteInitialization()
{
	...
	if (!Parent()->IsFakeChannel())
	{
		...
		CNetwork::Get()->GetServiceUpr()->CreateExtension(IsServer(), &adder);
	}
}
...
void CContextView::ClearAllState()
{
	...
	if (Context() && Context()->IsMultiplayer())
	{
		...
		{
			SNetObjectID id(static_cast<u16>(i), m_objects[i].salt);
			if (IsObjectBound(id))
				ClearHistory(id);
		}
	}
...
void CContextView::UpdateSchedulerState(SNetObjectID id)
{
	...
	if (pVwObj->msgHandle)
	{
		...
		if (pSendable)
		{
			u32 prioCls = pVwObj->authority ? obj.xtra->scheduler_owned : obj.xtra->scheduler_normal;
...
NetworkAspectType CContextView::GetSentAspects(SNetObjectID id, bool assumeEnabled, EGetSentAspectsAuthority assumeAuthority)
{
	...
	if ((assumeEnabled && IsObjectBound(id)) || IsObjectEnabled(id))
	{
		...
		u8 authMask = 4 | m_objects[id.id].authority;
...
void CContextView::ClearHistory(SNetObjectID netID)
{
	if (Context()->IsMultiplayer())
	{
		...
		for (i32 history = 0; history < eH_NUM_HISTORIES; history++)
		{
...
void CContextView::GetMemoryStatistics(IDrxSizer* pSizer)
{
...
void CContextView::SetAspectProfile(SNetObjectID objectID, NetworkAspectType aspect, u8 profile)
{
}
...
void CContextView::ChangedObject(SNetObjectID id, u32 flags, NetworkAspectType dirtyAspects)
{
...
bool CContextView::UpdateAspect(NetworkAspectID i, TSerialize ser, u32 nCurSeq, u32 nOldSeq, u32 timeFraction32)
{
	if (IsLocal())
	{
	...
	{
		std::set<u16>::iterator itCur = m_updatedObjectsThisFrame.find(CurrentObjectID().id);
	...
	{
		char msg[256];
		drx_sprintf(msg, "Failed creating sync context for aspect %s on object %s", Context()->GetAspectName(i), ctx.objId.GetText());
	...
	if (!ok)
	{
		...
		//
		char msg[256];
		drx_sprintf(msg, "Failed reading current value for aspect %s on object %s", Context()->GetAspectName(i), ctx.objId.GetText());
		Parent()->Disconnect(eDC_ContextCorruption, msg);
		return false;
	}
	return ok;
...
SReceiveContext CContextView::CreateReceiveContext(TSerialize ser, NetworkAspectID index, u32 nCurSeq, u32 nOldSeq, u32 timeFraction32, bool* ok)
{
...
CContextViewObjectLock CContextView::LockObject(SNetObjectID id, tukk why)
{
...
void CContextView::NotifyPartialUpdate(SNetObjectID id, NetworkAspectID aspectIdx)
{
	if (!IsObjectEnabled(id) || !Context()->IsMultiplayer() || ContextState() != Context()->GetCurrentState())
		return;
	// technically not correct; should be >= 0; but drxsis doesn't use aspect 0
...
bool CContextView::PartialAspect(NetworkAspectID aspectIdx, TSerialize ser, u32, u32, u32)
{
	SNetObjectID id;
