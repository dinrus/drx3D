...
void CPerformBreakSimpleBase::SerialiseNetIds(TSerialize ser)
{
	...
	for (i32 i = 0; i < m_numEntities; i++)
	{
		ser.Value("id", m_entityNetIds[i], 'eid');
		DrxLogAlways("brk: # serialise netId: %s", m_entityNetIds[i].GetText());
	}
}
...
void CPerformBreakSimpleServer::Init(CServerContextView* pView, const SNetIntBreakDescription* pDesc, SSendableHandle msgHandle, SNetObjectID dependentNetId)
{
	...
	i32k n = pDesc->spawnedObjects.size();
	for (i32 i = 0; i < n; i++)
	{
...
void CPerformBreakSimpleServer::UpdateState(u32 nFromSeq, ENetSendableStateUpdate state)
{
...
void CPerformBreakSimpleClient::BindSpawnedEntity(EntityId id, i32 spawnIdx)
{
