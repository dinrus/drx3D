...
#if USE_ARITHSTREAM
void CAdaptiveFloat::WriteValue(CCommOutputStream& stm, float value, u32 mementoAge, u32 timeFraction32) const
{
	//quantize value
	...
	u32 left, right;
	left = right = 0;
	i32 predicted = m_predictor.Predict(left, right, m_nHeight, m_nQuantizedMinDifference, m_quantizer.GetMaxQuantizedValue(), m_nQuantizedMaxDifference, mementoAge, timeFraction32);
...
bool CAdaptiveFloat::ReadValue(CCommInputStream& stm, float& value, u32 mementoAge, u32 timeFraction32) const
{
	u32 left, right;
	left = right = 0;
	i32 prediction = m_predictor.Predict(left, right, m_nHeight, m_nQuantizedMinDifference, m_quantizer.GetMaxQuantizedValue(), m_nQuantizedMaxDifference, mementoAge, timeFraction32);
	u32 quantized;
	...
	if (m_haveMemento)
	{
		...
		{
			DrxLogAlways("[CAdaptiveFloat::ReadValue] read error!\n");
...
#else
void CAdaptiveFloat::WriteValue(CNetOutputSerializeImpl* stm, float value, u32 mementoAge, u32 timeFraction32) const
{
	u32 quantized = m_quantizer.Quantize(value);
...
bool CAdaptiveFloat::ReadValue(CNetInputSerializeImpl* stm, float& value, u32 mementoAge, u32 timeFraction32) const
{
...
i32 CAdaptiveFloat::GetBitCount()
{
