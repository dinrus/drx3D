...
struct SNetObjectEvent
{
	...
	union
	{
		struct
		{
			...
			u8 profile; // only for eNOE_SetAspectProfile
		};
		// eNOE_SetAuthority
		bool authority;
...
struct IRMILoggerValue
{
	...
	virtual void WriteStream(CSimpleOutputStream&, tukk ) = 0;
};

struct IRMILogger
{
	virtual ~IRMILogger(){}
	virtual void BeginFunction(tukk name) = 0;
	virtual void OnProperty(tukk name, IRMILoggerValue* pSer) = 0;
...
static i32k NumAspects = NUM_ASPECTS;
...
struct SContextObject
{
	...
	u16                vLocks[eCOL_NUM_LOCKS]; //min
#if CHECKING_BIND_REFCOUNTS
	std::map<string, i32> debug_whosLocking;
#endif
	// our parent object
	...
	u8                  vAspectProfiles[NumAspects]; //xtra
	// what is the default profile for each aspect
	u8                  vAspectDefaultProfiles[NumAspects]; //xtra
	u16                 salt;

	tukk GetName() const;

	tukk GetClassName() const;

	tukk GetLockString() const
	{
...
struct SContextObjectEx
{
	SContextObjectEx()
	{
		...
		for (size_t i = 0; i < NumAspects; ++i)
		{
			...
			vAspectData[i] = CMementoMemoryUpr::InvalidHdl;
			vRemoteAspectData[i] = CMementoMemoryUpr::InvalidHdl;
		}
	}
	...
	// scheduling profiles
	u32                 scheduler_normal; //xtra
	u32                 scheduler_owned;  //xtra
	...
	u32  vAspectDataVersion[NumAspects]; //xtra
};
