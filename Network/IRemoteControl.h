...
struct IRemoteControlServerListener
{
	...
	virtual void OnClientCommand(u32 commandId, string command) = 0;
...
struct IRemoteControlServer
{
	...
	virtual void Start(u16 serverPort, const string& password, IRemoteControlServerListener* pListener) = 0;

	//! Stops the RCON_server.
	virtual void Stop() = 0;

	//! Sends command result back to the command client.
	virtual void SendResult(u32 commandId, const string& result) = 0;
	// </interfuscator:shuffle>
};
...
struct IRemoteControlClientListener
{
	...
	virtual void OnCommandResult(u32 commandId, string command, string result) = 0;
...
struct IRemoteControlClient
{
	...
	virtual void Connect(const string& serverAddr, u16 serverPort, const string& password, IRemoteControlClientListener* pListener) = 0;

	//! Disconnects from an RCON_server.
	virtual void Disconnect() = 0;

	//! Sends RCON commands to the connected RCON_server; returns a unique command ID.
	virtual u32 SendCommand(const string& command) = 0;
	// </interfuscator:shuffle>
};
