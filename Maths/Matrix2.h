 ...
 */
class DRX3D_EXPORT Matrix2 {
public:
	/**
	 * Конструктор класса Matrix2. The matrix is initialised to the identity.
	 * @param diagonal The value set to the diagonals.
	 */
	Matrix2(float diagonal = 1.0f);

	/**
	 * Конструктор класса Matrix2.
...
namespace std {
template<>
struct hash<drx3d::Matrix2> {
	size_t operator()(const drx3d::Matrix2 &matrix) const noexcept {
		size_t seed = 0;
