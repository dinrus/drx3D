#ifndef DRX3D_HASH_MAP_H
#define DRX3D_HASH_MAP_H
...
struct HashString
{
		...
		: m_string1(name)
	{
		...
		for (i32 i = 0; m_string1.c_str()[i]; i++)
		{
...
i32k DRX3D_HASH_NULL = 0xffffffff;
...
public:
	...
	SIMD_FORCE_INLINE u32 getHash() const
	{
		...
		u32 key = VOID_IS_8 ? m_hashValues[0] + m_hashValues[1] : m_hashValues[0];
		// Thomas Wang's hash
		key += ~(key << 15);
		key ^= (key >> 10);
		key += (key << 3);
...
///The HashMap template class implements a generic and lightweight hashmap.
///A basic sample of how to use HashMap is located in Demos\BasicDemo\main.cpp
...
protected:
	...
	AlignedObjectArray<Value> m_valueArray;
	AlignedObjectArray<Key> m_keyArray;
	...
	void growTables(const Key& /*key*/)
	{
		...
		if (m_hashTable.size() < newCapacity)
		{
			...
			{
				m_hashTable[i] = DRX3D_HASH_NULL;
			}
			for (i = 0; i < newCapacity; ++i)
			{
				m_next[i] = DRX3D_HASH_NULL;
			}
...
public:
	...
	void remove(const Key& key)
	{
		...
		Assert(index != DRX3D_HASH_NULL);

		i32 previous = DRX3D_HASH_NULL;
	...
	i32 findIndex(const Key& key) const
	{
		...
		{
			return DRX3D_HASH_NULL;
		}

		i32 index = m_hashTable[hash];
		while ((index != DRX3D_HASH_NULL) && key.equals(m_keyArray[index]) == false)
		{
			index = m_next[index];
		}
		return index;
