...
template<typename T>
constexpr auto Vector4<T>::d3Min() const {
	return std::min({x, y, z, w});
}

template<typename T>
constexpr auto Vector4<T>::d3Max() const {
...
template<typename T>
template<typename K>
constexpr auto Vector4<T>::d3Min(const Vector4<K> &other) {
...
template<typename T>
template<typename K>
constexpr auto Vector4<T>::d3Max(const Vector4<K> &other) {
	using THighestP = decltype(x + other.x);
...
namespace std {
template<typename T>
struct hash<drx3d::Vector4<T>> {
	size_t operator()(const drx3d::Vector4<T> &vector) const noexcept {
		size_t seed = 0;
		drx3d::Maths::HashCombine(seed, vector.x);
		drx3d::Maths::HashCombine(seed, vector.y);
		drx3d::Maths::HashCombine(seed, vector.z);
		drx3d::Maths::HashCombine(seed, vector.w);
		return seed;
	}
};
}
