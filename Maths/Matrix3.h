 ...
 */
class DRX3D_EXPORT Matrix3 {
public:
	/**
	 * Конструктор класса Matrix3. The matrix is initialised to the identity.
	 * @param diagonal The value set to the diagonals.
	 */
	Matrix3(float diagonal = 1.0f);

	/**
	 * Конструктор класса Matrix3.
...
namespace std {
template<>
struct hash<drx3d::Matrix3> {
	size_t operator()(const drx3d::Matrix3 &matrix) const noexcept {
		size_t seed = 0;
