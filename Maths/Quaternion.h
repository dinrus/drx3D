 ...
 */
class DRX3D_EXPORT Quaternion {
public:
	/**
	 * Конструктор класса Quaternion.
	 */
	Quaternion() = default;
...
namespace std {
template<>
struct hash<drx3d::Quaternion> {
	size_t operator()(const drx3d::Quaternion &quaternion) const noexcept {
		size_t seed = 0;
		drx3d::Maths::HashCombine(seed, quaternion.x);
		drx3d::Maths::HashCombine(seed, quaternion.y);
		drx3d::Maths::HashCombine(seed, quaternion.z);
		drx3d::Maths::HashCombine(seed, quaternion.w);
		return seed;
	}
};
}
