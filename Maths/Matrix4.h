 ...
 */
class DRX3D_EXPORT Matrix4 {
public:
	/**
	 * Конструктор класса Matrix4. The matrix is initialised to the identity.
	 * @param diagonal The value set to the diagonals.
	 */
	Matrix4(float diagonal = 1.0f);

	/**
	 * Конструктор класса Matrix4.
...
namespace std {
template<>
struct hash<drx3d::Matrix4> {
	size_t operator()(const drx3d::Matrix4 &matrix) const noexcept {
		size_t seed = 0;
		drx3d::Maths::HashCombine(seed, matrix[0]);
		drx3d::Maths::HashCombine(seed, matrix[1]);
		drx3d::Maths::HashCombine(seed, matrix[2]);
		drx3d::Maths::HashCombine(seed, matrix[3]);
		return seed;
	}
};
}
