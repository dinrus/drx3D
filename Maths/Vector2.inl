...
template<typename T>
constexpr auto Vector2<T>::d3Min() const {
	return std::min({x, y});
}

template<typename T>
constexpr auto Vector2<T>::d3Max() const {
	return std::max({x, y});
}
...
template<typename T>
template<typename K>
constexpr auto Vector2<T>::d3Min(const Vector2<K> &other) {
...
template<typename T>
template<typename K>
constexpr auto Vector2<T>::d3Max(const Vector2<K> &other) {
	using THighestP = decltype(x + other.x);
...
namespace std {
template<typename T>
struct hash<drx3d::Vector2<T>> {
	size_t operator()(const drx3d::Vector2<T> &vector) const noexcept {
		size_t seed = 0;
