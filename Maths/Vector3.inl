...
template<typename T>
constexpr auto Vector3<T>::d3Min() const {
	return std::min({x, y, z});
}

template<typename T>
constexpr auto Vector3<T>::d3Max() const {
...
template<typename T>
template<typename K>
constexpr auto Vector3<T>::d3Min(const Vector3<K> &other) {
...
template<typename T>
template<typename K>
constexpr auto Vector3<T>::d3Max(const Vector3<K> &other) {
	using THighestP = decltype(x + other.x);
...
namespace std {
template<typename T>
struct hash<drx3d::Vector3<T>> {
	size_t operator()(const drx3d::Vector3<T> &vector) const noexcept {
		size_t seed = 0;
