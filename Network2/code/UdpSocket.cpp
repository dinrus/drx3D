...
Socket::Status UdpSocket::Send(ukk data, std::size_t size, const IpAddress &remoteAddress, uint16_t remotePort) {
	// Create the internal socket if it doesn't exist.
	Create();
	...
	auto sent = sendto(GetHandle(), static_cast<tukk >(data), static_cast<i32>(size), 0, reinterpret_cast<sockaddr *>(&address), sizeof(address));
...
Socket::Status UdpSocket::Receive(uk data, std::size_t size, std::size_t &received, IpAddress &remoteAddress, uint16_t &remotePort) {
	...
	auto sizeReceived = recvfrom(GetHandle(), static_cast<char *>(data), static_cast<i32>(size), 0, reinterpret_cast<sockaddr *>(&address), &addressSize);
...
Socket::Status UdpSocket::Receive(Packet &packet, IpAddress &remoteAddress, uint16_t &remotePort) {
	...
	auto status = Receive(&buffer[0], buffer.size(), received, remoteAddress, remotePort);
