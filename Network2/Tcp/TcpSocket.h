 ...
 *
 * The high-level interface uses packets (see drx3d::Packet), which are easier to use and
 * provide more safety regarding the data that is exchanged.
 * You can look at the drx3d::Packet class to get more details about how they work.
 *
 ...
 */
class DRX3D_EXPORT PendingPacket {
public:
 ...
 */
class DRX3D_EXPORT TcpSocket : public Socket {
...
public:
	 ...
	 * use the send(ukk , std::size_t, std::size_t&) overload instead.
	 * This function will fail if the socket is not connected.
	 * @param data Pointer to the sequence of bytes to send.
	 * @param size Number of bytes to send.
	 * @return Status code.
	 */
	Status Send(ukk data, std::size_t size);
	 ...
	 * @return Status code.
	 */
	Status Send(ukk data, std::size_t size, std::size_t &sent);
	 ...
	 * @return Status code.
	 */
	Status Receive(uk data, std::size_t size, std::size_t &received);
