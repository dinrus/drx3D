...
struct IEntityScriptFileHandler
{
	...
	virtual void        GetMemoryUsage(IDrxSizer* pSizer) const { /*REPLACE LATER*/ }
};
...
struct IEntityEventHandler
{
	...
	virtual bool GetEventInfo(i32 index, SEventInfo& info) const = 0;

	//! Send the specified event to the entity.
	//! \param entity The entity to send the event to.
	//! \param eventName Name of the event to send.
	virtual void SendEvent(IEntity* entity, tukk eventName) = 0;
};
...
struct IEntityClass
{
	...
	//! }
	typedef IEntityComponent* (* UserProxyCreateFunc)(IEntity* pEntity, SEntitySpawnParams& params, uk pUserData);
	...
	virtual bool FindEventInfo(tukk sEvent, SEventInfo& event) = 0;
...
struct IEntityClassRegistry
{
	struct SEntityClassDesc
	{
		...
		//! GUID for the sxema runtime class
		DrxGUID schematycRuntimeClassGuid;
	...
	virtual IEntityClass* FindClass(tukk sClassName) const = 0;

	//! Retrieves pointer to the IEntityClass interface by GUID.
	//! \return Pointer to the IEntityClass interface, or nullptr if class not found.
	virtual IEntityClass* FindClassByGUID(const DrxGUID& guid) const = 0;
	...
	virtual void LoadClasses(tukk szFilename, bool bOnlyNewClasses = false) = 0;
	...
	virtual bool UnregisterStdClass(const DrxGUID& classGUID) = 0;
	...
	//! \return Return a pointer to the next IEntityClass interface, or nullptr if is the end
	virtual i32 GetClassCount() const = 0;
	// </interfuscator:shuffle>
};
