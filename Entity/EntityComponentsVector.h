...
{
	i32                               proxyType;            //!< Proxy id associated with this component (Only when component correspond to know proxy, -1 overwise)
	i32                               eventPriority;        //!< Event dispatch priority
	u16                            creationOrder;        //!< Determines when an order was constructed, i.e. 0 = first component, 1 = second. Not necessarily sequential.
	EntityEventMask                   registeredEventsMask; //!< Bitmask of the EEntityEvent values
	DrxInterfaceID                    typeId;               //!< Interface IDD for the registered component.
	...
	i32 GetProxyType() const { return proxyType; }
	i32 GetEventPriority() const { return eventPriority; }
	u16 GetCreationOrder() const { return creationOrder; }
...
struct SMinimalEntityComponentRecord
{
	...
	// Check whether the component has been removed
	bool IsValid() const { return pComponent != nullptr; }
	i32 GetProxyType() const { return pComponent != nullptr ? pComponent->GetProxyType() : ENTITY_PROXY_LAST; }
	i32 GetEventPriority() const { return pComponent != nullptr ? pComponent->GetEventPriority() : 0; }
	u16 GetCreationOrder() const { return GetEventPriority(); }
...
enum class EComponentIterationResult : u8
{
	Continue,
	Break
};
...
private:
	...
	u16                     m_activeScopes : 14;     //!< Counts current iteration scopes (cleanup cannot occur unless this is 0).
	u16                     m_cleanupRequired : 1;   //!< Indicates NULL elements in listener.
	u16                     m_sortingValid : 1;      //!< Indicates that sorting order of the elements maybe not be valid.
...
public:
	...
	{
		DRX_ASSERT_MESSAGE(m_activeScopes == 0, "Entity components vector was destroyed while scopes were active, most likely on another thread!");
	...
	{
		DRX_ASSERT_MESSAGE(gEnv->mMainThreadId == DrxGetCurrentThreadId(), "Components vector can only be added to by the main thread!");
		DRX_ASSERT(m_sortingValid || m_activeScopes > 0);
	...
	{
		DRX_ASSERT_MESSAGE(gEnv->mMainThreadId == DrxGetCurrentThreadId(), "Existing component can only be queried by the main thread, or use FindComponent!");
	...
	{
		DRX_ASSERT_MESSAGE(gEnv->mMainThreadId == DrxGetCurrentThreadId(), "Existing component can only be re-sorted by the main thread!");
		DRX_ASSERT_MESSAGE(m_activeScopes == 0, "Re-sorting an existing component record while iteration is in progress is not supported!");
		DRX_ASSERT(!m_cleanupRequired);
		DRX_ASSERT(m_sortingValid);
	...
	{
		DRX_ASSERT_MESSAGE(gEnv->mMainThreadId == DrxGetCurrentThreadId(), "Existing component can only be removed by the main thread!");
	...
	void Clear()
	{
		DRX_ASSERT_MESSAGE(gEnv->mMainThreadId == DrxGetCurrentThreadId(), "Components vector can only be cleared by the main thread!");
		DRX_ASSERT(!m_cleanupRequired);
		DRX_ASSERT(m_activeScopes == 0);
	...
	{
		DRX_ASSERT_MESSAGE(gEnv->mMainThreadId == DrxGetCurrentThreadId(), "Components vector capacity can only be changed by the main thread!");
	...
	{
		DRX_ASSERT_MESSAGE(m_activeScopes > 0 || (m_sortingValid && !m_cleanupRequired), "Non-recursive call to ForEach should not require cleanup or sorting!");
		...
		if (m_activeScopes == 0)
		{
			DRX_ASSERT(gEnv->mMainThreadId == DrxGetCurrentThreadId() || (!m_cleanupRequired && m_sortingValid));
	...
	{
		DrxOptionalAutoLock<DrxCriticalSection> lock(m_lock, gEnv->mMainThreadId != DrxGetCurrentThreadId());
