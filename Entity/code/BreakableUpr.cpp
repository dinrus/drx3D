...
#include <drx3D/Entity/BreakablePlane.h>
...
//////////////////////////////////////////////////////////////////////////
float ExtractFloatKeyFromString(tukk key, tukk props)
{
	if (tukk ptr = strstr(props, key))
	{
		ptr += strlen(key);
		if (*ptr++ == '=')
...
//////////////////////////////////////////////////////////////////////////
string ExtractStringKeyFromString(tukk key, const string& props)
{
	tukk ptr = strstr(props, key);
	if (ptr)
	{
		ptr += strlen(key);
		while (*ptr && *ptr != '\n' && !isalnum((u8)*ptr))
...
//////////////////////////////////////////////////////////////////////////
void AssociateStatObjWithPhysics(IStatObj* pStatObj, IPhysicalEntity* pent, i32 partid)
{
	pe_params_part pp;
	pp.partid = partid;
	if (pStatObj && !pStatObj->GetPhysGeom() && pent && pent->GetParams(&pp) && pp.pPhysGeom && pp.pPhysGeom->pGeom)
	{
		gEnv->pPhysicalWorld->GetGeomUpr()->AddRefGeometry(pp.pPhysGeom);
...
ISurfaceType* GetSurfaceType(IRenderMesh* pMesh, IMaterial* pMtl)
{
	...
	i32 numChunks = meshChunks.size();
	for (i32 nChunk = 0; nChunk < numChunks; nChunk++)
	{
...
ISurfaceType* GetSurfaceType(phys_geometry* pGeom)
{
	if (pGeom && pGeom->surface_idx < pGeom->nMats)
	{
		...
		i32 nMat = pGeom->pMatMapping[pGeom->surface_idx];
		ISurfaceType* pSrfType = pSurfaceTypeUpr->GetSurfaceType(nMat);
		if (pSrfType && pSrfType->GetId() != 0) // If not default surface type
			return pSrfType;
	}
	return NULL;
...
ISurfaceType* GetSurfaceType(GeomRef const& geo)
{
	...
	|| (pSurface = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr()->GetSurfaceType(0));
	return pSurface;
}

void CBreakableUpr::AddBreakEventListener(IBreakEventListener* pListener)
{
	assert(m_pBreakEventListener == NULL);
	m_pBreakEventListener = pListener;
}

void CBreakableUpr::RemoveBreakEventListener(IBreakEventListener* pListener)
{
	assert(m_pBreakEventListener == pListener);
	m_pBreakEventListener = NULL;
}

void CBreakableUpr::EntityDrawSlot(CEntity* pEntity, i32 slot, i32 flags)
{
...
IParticleEffect* GetSurfaceEffect(ISurfaceType* pSurface, tukk sType, SpawnParams& params)
{
	...
	tukk sEffect = pBreakParticles->particle_effect.c_str();

	if (sEffect && sEffect[0])
	{
		if (IParticleEffect* pEffect = gEnv->pParticleUpr->FindEffect(sEffect, string("Surface Effect ") + pSurface->GetName()))
		{
...
//////////////////////////////////////////////////////////////////////////
ISurfaceType* CBreakableUpr::GetFirstSurfaceType(IStatObj* pStatObj)
{
	return GetSurfaceType(pStatObj);
}

//////////////////////////////////////////////////////////////////////////
ISurfaceType* CBreakableUpr::GetFirstSurfaceType(ICharacterInstance* pCharacter)
{
	return GetSurfaceType(pCharacter);
}

//////////////////////////////////////////////////////////////////////////
bool CBreakableUpr::CanShatterRenderMesh(IRenderMesh* pRenderMesh, IMaterial* pMtl)
{
...
//////////////////////////////////////////////////////////////////////////
bool CBreakableUpr::CanShatter(IStatObj* pStatObj)
{
	if (!pStatObj)
		return false;
...
//////////////////////////////////////////////////////////////////////////
bool CBreakableUpr::CanShatterEntity(IEntity* pEntity, i32 nSlot)
{
	if (nSlot < 0)
		nSlot = 0;
...
void GetPhysicsStatus(IPhysicalEntity* pPhysEnt, float& fMass, float& fVolume, float& fDensity, Vec3& vCM, Vec3& vLinVel, Vec3& vAngVel)
{
	...
	i32 nparts = pPhysEnt->GetStatus(&statusTmp);
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::BreakIntoPieces(GeomRef& geoOrig, const Matrix34& mxSrcTM,
                                        ...
                                        BreakageParams const& Breakage, i32 nMatLayers)
{
	...
	struct MFXExec
	{
		MFXExec(GeomRef& geoOrig, const Matrix34& mxSrcTM, const BreakageParams& Breakage, i32k& nMatLayers) :
			geoOrig(geoOrig),
			mxSrcTM(mxSrcTM),
			Breakage(Breakage),
		...
		~MFXExec()
		{
			...
			if (Breakage.bMaterialEffects)
			{
				...
				tukk sType = SURFACE_BREAKAGE_TYPE("destroy");
	...
	i32 nSubObjs = pPiecesObj->GetSubObjectCount();
	...
	if (geoOrig.m_pPhysEnt)
	{
		...
		pe_status_nparts statusTmp;
		i32 nparts = geoOrig.m_pPhysEnt->GetStatus(&statusTmp);
		for (i32 p = 0; p < nparts; p++)
		{
			pe_params_part pp;
			pp.ipart = p;
	...
	tukk sSourceGeometryProperties = "";
	if (pRemovedObj)
		sSourceGeometryProperties = pRemovedObj->GetProperties();

	//////////////////////////////////////////////////////////////////////////
	for (i32 i = 0; i < max(nSubObjs, 1); i++)
	{
		IStatObj* pSubObj;
		...
		if (pPhysGeom)
		{
			...
			if (geoOrig.m_pChar)
			{
				...
				i32 boneId = geoOrig.m_pChar->GetIDefaultSkeleton().GetJointIDByName(bone.c_str());
				if (boneId >= 0)
			...
			i32 nCount = (i32)ExtractFloatKeyFromString("count", sProperties);
			if (nCount <= 0)
				nCount = (i32)ExtractFloatKeyFromString("generic", sProperties);
			...
			for (i32 n = 0; n < max(nCount, 1); n++)
			{
				...
				if (nCount)
				{
					...
					{
						Ang3 angRot;
						angRot.x = strchr(sRotAxes, 'x') ? DEG2RAD(drx_random(-180.0f, 180.0f)) : 0.f;
						angRot.y = strchr(sRotAxes, 'y') ? DEG2RAD(drx_random(-180.0f, 180.0f)) : 0.f;
						angRot.z = strchr(sRotAxes, 'z') ? DEG2RAD(drx_random(-180.0f, 180.0f)) : 0.f;
						mxPiece = mxPiece * Matrix33::CreateRotationXYZ(angRot);
					}

					if (fSizeVar != 0.f)
					{
						float fScale = 1.f + drx_random(-fSizeVar, fSizeVar);
						if (fScale <= 0.01f)
							fScale = 0.01f;
				...
				if (!bEntity)
				{
					if (!pEmitter)
					{
						...
						pEmitter = gEnv->pParticleUpr->CreateEmitter(mxSrcTM, params);
						if (!pEmitter)
							continue;
						pEmitter->SetMaterialLayers(nMatLayers);
					}
				...
				if (pPhysEnt)
				{
					...
					Vec3 vObj = vPartCM + Vec3(drx_random(-fHalfRadius, fHalfRadius), drx_random(-fHalfRadius, fHalfRadius), drx_random(-fHalfRadius, fHalfRadius));
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::BreakIntoPieces(IEntity* pEntity, i32 nOrigSlot, i32 nPiecesSlot, BreakageParams const& Breakage)
{
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::CreateSurfaceEffect(IStatObj* pStatObj, const Matrix34& tm, tukk sType)
{
	GeomRef geo;
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::AttachSurfaceEffect(IEntity* pEntity, i32 nSlot, tukk sType, SpawnParams const& paramsIn)
{
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::CreateObjectAsParticles(IStatObj* pStatObj, IPhysicalEntity* pPhysEnt, IBreakableUpr::SCreateParams& createParams)
{
	if (!pPhysEnt)
		return;
	...
	IParticleEmitter* pEmitter = gEnv->pParticleUpr->CreateEmitter(tm, params);
	if (!pEmitter)
		return;
...
//////////////////////////////////////////////////////////////////////////
IEntity* CBreakableUpr::CreateObjectAsEntity(IStatObj* pStatObj, IPhysicalEntity* pPhysEnt, IPhysicalEntity* pSrcPhysEnt, IBreakableUpr::SCreateParams& createParams, bool bCreateSubstProxy)
{
	...
	if (createParams.pSrcStaticRenderNode && bCreateSubstProxy)/* && gEnv->IsEditor())
	                                                            ...
	                                                            */
	{
		IEntitySubstitutionComponent* pSubstProxy = drxcomponent_cast<IEntitySubstitutionComponent*>(pEntity->CreateProxy(ENTITY_PROXY_SUBSTITUTION));
	...
	if (bDefault)
	{
		...
		IBreakableUpr::SBrokenObjRec rec;
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::ReplayRemoveSubPartsEvent(const EventPhysRemoveEntityParts* pRemoveEvent)
{
	HandlePhysicsRemoveSubPartsEvent(pRemoveEvent);
}

//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::CreateObjectCommon(IStatObj* pStatObj, IPhysicalEntity* pPhysEnt, IBreakableUpr::SCreateParams& createParams)
{
	if (createParams.pSrcStaticRenderNode)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CBreakableUpr::CheckForPieces(IStatObj* pSrcStatObj, IStatObj::SSubObject* pSubObj, const Matrix34& worldTM, i32 nMatLayers,
                                       IPhysicalEntity* pPhysEnt)
{
	tukk sProperties = pSubObj->pStatObj->GetProperties();
...
IStatObj::SSubObject* CBreakableUpr::CheckSubObjBreak(IStatObj* pStatObj, IStatObj::SSubObject* pSubObj, const EventPhysCreateEntityPart* epcep)
{
	if (pSubObj->nBreakerJoints)
	{
		...
		i32 nDestroyedJoints = pSubObj->nBreakerJoints;
...
void CBreakableUpr::GetBrokenObjectIndicesForCloning(i32* pPartRemovalIndices, i32& iNumPartRemovalIndices,
                                                         i32* pOutIndiciesForCloning, i32& iNumEntitiesForCloning,
                                                         const EventPhysRemoveEntityParts* BreakEvents)
{
	i32 iNumIndices = iNumPartRemovalIndices;
	i32 iLocalNumEntitiesToClone = 0;

	//Get pointer to part removal event array from break replicator
	IBreakableUpr* pBreakableMgr = g_pIEntitySystem->GetBreakableUpr();

	i32 iNumBrokenObjects = 0;
	const IBreakableUpr::SBrokenObjRec* pPartBrokenObjects = pBreakableMgr->GetPartBrokenObjects(iNumBrokenObjects);

	//Remove duplicates
	BreakLogAlways("PR:   Getting indicies into the BrokenObject list for cloning, %d present", iNumIndices);

	for (i32 i = 0; i < iNumIndices; i++)
	{
	...
	//DrxLogAlways("PR:   All render nodes from break events during kill cam:");
	//  for(i32 k=iNumIndices-1; k>=0; k--)
	//  {
	//    DrxLogAlways("PR:     Idx: %3d > RenderNode: 0x%p", k, BreakEvents[pPartRemovalIndices[k]].pForeignData);
	//  }

	//DrxLogAlways("PR: Carrying out clones");
	...
	for (i32 k = iNumIndices - 1; k >= 0; k--)
	{
		...
		if (BreakEvents[pPartRemovalIndices[k]].iForeignData == PHYS_FOREIGN_ID_ENTITY)
		{
			...
			for (i32 j = iNumBrokenObjects - 1; j >= 0; j--)
			{
				const IBreakableUpr::SBrokenObjRec& brokenObjRec = pPartBrokenObjects[j];
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::ClonePartRemovedEntitiesByIndex(i32* pBrokenObjIndicies, i32 iNumBrokenObjectIndices,
                                                        ...
                                                        SRenderNodeCloneLookup& nodeLookup)
{
	...
	i32 iNumClonedEntitiesLocal = 0;

	i32 iNumBrokenObjects;
	const IBreakableUpr::SBrokenObjRec* pPartBrokenObjects = pBreakableMgr->GetPartBrokenObjects(iNumBrokenObjects);

	for (i32 iBrokenObjectIndex = iNumBrokenObjectIndices - 1; iBrokenObjectIndex >= 0; iBrokenObjectIndex--)
	{
		...
		const IBreakableUpr::SBrokenObjRec& brokenObjRec = pPartBrokenObjects[pBrokenObjIndicies[iBrokenObjectIndex]];
		...
		for (i32 l = 0; l < iNumRecordingEntities; l++)
		{
...
void CBreakableUpr::HideBrokenObjectsByIndex(i32k* pBrokenObjectIndices, i32k iNumBrokenObjectIndices)
{
	for (i32 k = iNumBrokenObjectIndices - 1; k >= 0; k--)
	{
...
void CBreakableUpr::UnhidePartRemovedObjectsByIndex(i32k* pPartRemovalIndices, i32k iNumPartRemovalIndices, const EventPhysRemoveEntityParts* pBreakEvents)
{
	IBreakableUpr* pBreakableMgr = g_pIEntitySystem->GetBreakableUpr();
	BreakLogAlways("PR: Unhiding part removed objects");

	i32 iBrokenObjectCount = 0;
	const IBreakableUpr::SBrokenObjRec* pPartBrokenObjects = pBreakableMgr->GetPartBrokenObjects(iBrokenObjectCount);

	for (i32 k = iNumPartRemovalIndices - 1; k >= 0; k--)
	{
		i32 iEventIndex = pPartRemovalIndices[k];
		...
		else
		{
			...
			for (i32 j = iBrokenObjectCount - 1; j >= 0; j--)
			{
				const IBreakableUpr::SBrokenObjRec& brokenObjRec = pPartBrokenObjects[j];
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::ApplySinglePartRemovalFromEventIndex(i32 iPartRemovalEventIndex, const SRenderNodeCloneLookup& renderNodeLookup, const EventPhysRemoveEntityParts* pBreakEvents)
{
	ApplyPartBreakToClonedObjectFromEvent(renderNodeLookup, pBreakEvents[iPartRemovalEventIndex]);
}

//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::ApplyPartRemovalsUntilIndexToObjectList(i32 iFirstIndex, const SRenderNodeCloneLookup& renderNodeLookup, const EventPhysRemoveEntityParts* pBreakEvents)
{
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::ApplyPartBreakToClonedObjectFromEvent(const SRenderNodeCloneLookup& renderNodeLookup, const EventPhysRemoveEntityParts& OriginalEvent)
{
	...
	{
		if (OriginalEvent.pForeignData == static_cast<uk>(renderNodeLookup.pOriginalNodes[a]))
		{
			iNodeIndex = a;
			break;
		}
	...
	if (iNodeIndex >= 0)
	{
		...
		tempEvent.pForeignData = static_cast<uk>(pEntity);
		...
		i32 iSubObjectCount = pStatObj->GetSubObjectCount();
		...
		if (!(pStatObj->GetFlags() & STATIC_OBJECT_GENERATED))
		{
			...
			for (i32 i = iSubObjectCount - 1; i >= 1; i--)
			{
...
{
	DRX_ENTITY_COMPONENT_INTERFACE_AND_CLASS_GUID(CTimeoutKillComponent, "CTimeoutKillComponent", "b55e276d-1ca5-aa14-87fb-ca961e102dff"_drx_guid)
...
public:
	...
	void         SetTimeout(i32 timeoutMillis)
	{
...
private:
	i32 m_timeoutMillis = 0;
};

void SetEntityLifetime(CEntity* pEntity, tukk props, bool visible)
{
	float timeout = -1, timeoutInvis = -1;
	if (tukk ptr = strstr(props, "timeout"))
		timeout = (float)atof(ptr + 8);
	if (tukk ptr = strstr(props, "invis"))
	...
	if (timeout > 0 || timeoutInvis >= 0)
	{
		...
		pTimeout->SetTimeout(static_cast<i32>(1000 * std::max(timeout, timeoutInvis)));
	}
}

//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::HandlePhysicsCreateEntityPartEvent(const EventPhysCreateEntityPart* pCreateEvent)
{
	i32 iForeignData = pCreateEvent->pEntity->GetiForeignData();
	uk pForeignData = pCreateEvent->pEntity->GetForeignData(iForeignData);
	...
	IBreakableUpr::SCreateParams createParams;
	i32 partidSrc = pCreateEvent->partidSrc;
	...
	{
		//DrxLogAlways( "* CreateEvent Entity" );
	...
	{
		//DrxLogAlways( "* CreateEvent Static" );
	...
	if (pNewStatObj)
	{
		...
		else
		{
			...
			i32 lastDrawnFrame = pSrcRenderNode ? pSrcRenderNode->GetDrawFrame() : 0;
	...
	if (pCutShape && !is_unused(pCreateEvent->cutPtLoc[1]) && pNewStatObj)
	{
		...
		tukk sType = SURFACE_BREAKAGE_TYPE("breakage");
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::HandlePhysicsRevealSubPartEvent(const EventPhysRevealEntityPart* pRevealEvent)
{
	i32 iForeignData = 0;
	uk pForeignData = NULL;
	i32 id = pRevealEvent->partId;
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::HandlePhysicsRemoveSubPartsEvent(const EventPhysRemoveEntityParts* pRemoveEvent)
{
	i32 iForeignData = 0;
	uk pForeignData = NULL;
	...
	i32 j = 0, idOffs = pRemoveEvent->idOffs;
	for (i32 i = 0; i < sizeof(pRemoveEvent->partIds) / sizeof(pRemoveEvent->partIds[0]); i++)
	...
	if (iForeignData == PHYS_FOREIGN_ID_ENTITY)
	{
		//DrxLogAlways( "* RemoveEvent Entity" );
		pEntity = static_cast<CEntity*>(pForeignData);
	...
	{
		//DrxLogAlways( "* RemoveEvent Entity Static" );
	...
	if (pStatObj && pStatObj->GetFlags() & STATIC_OBJECT_COMPOUND)
	{
		...
		for (i32 i = sizeof(pRemoveEvent->partIds) * 8 - 1; i >= 0; i--)
		{
			if (pRemoveEvent->partIds[i >> 5] & 1 << (i & 31))
			{
				// This sub-object must be removed.
				u32 nSubObjIndex = i + idOffs;
				nSubObjHideMask |= (hidemask1 << nSubObjIndex);
			}
		}
		i32 nVisibleSubObj = 0, iVisibleSubObj = -1;
		hidemaskOneBit nBitmask = hidemask1;
		for (i32 i = 0, num = pStatObj->GetSubObjectCount(); i < num && nVisibleSubObj < 2; i++)
		{
			i32 vis = pStatObj->GetSubObject(i)->pStatObj && !(nSubObjHideMask & nBitmask);
			nVisibleSubObj += vis;
			...
			{
				{
					...
					{
						SetEntityLifetime(pEntity, pStatObj->GetSubObject(iVisibleSubObj)->pStatObj->GetProperties(), (u32)pEntity->GetRenderNode()->GetDrawFrame() + 10 > gEnv->nMainFrameID);
					}
				}
			}
		}
		/*
		    i32 nMeshes,iMesh;
		    for(i32 i=0,nMeshes=0;i<pStatObj->GetSubObjectCount();i++)
		    {
		      IStatObj::SSubObject *pSubObj = pStatObj->GetSubObject(i);
...
//////////////////////////////////////////////////////////////////////////
i32 CBreakableUpr::HandlePhysics_UpdateMeshEvent(const EventPhysUpdateMesh* pUpdateEvent)
{

	CEntity* pCEntity = 0;
	Matrix34A mtx;
	i32 iForeignData = pUpdateEvent->pEntity->GetiForeignData();
	uk pForeignData = pUpdateEvent->pEntity->GetForeignData(iForeignData);
	...
	if (pmu && pSrcStatObj && GetSurfaceType(pSrcStatObj))
	{
		...
		if (pEffect)
		{
			...
			i32 nTris = 0;
			for (i32 i = 0; i < pmu->nNewTri; i++)
				if (pmu->pNewTri[i].iop)
				{
					Vec3 vtx[3];
					for (i32 j = 0; j < 3; j++)
					{
						i32 ivtx = pmu->pNewTri[i].iVtx[j];
						if (ivtx < 0)
	...
	if (pCEntity)
	{
		...
		if (ICharacterInstance* pChar = pIEntityRender->GetCharacter(0))
			if (IAttachmentUpr* pAttMan = pChar->GetIAttachmentUpr())
				for (i32 i = pAttMan->GetAttachmentCount() - 1; i >= 0 && !bChar; i--)
		...
		if (m_pBreakEventListener && pSrcStatObj != pDeformedStatObj && pUpdateEvent->iReason != EventPhysUpdateMesh::ReasonDeform)
		{
			...
			IBreakableUpr::SBrokenObjRec brokenObj;
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::FakePhysicsEvent(EventPhys* pEvent)
{
	switch (pEvent->idval)
	{
...
//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::FreezeRenderNode(IRenderNode* pRenderNode, bool bEnable)
{

}

//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::ResetBrokenObjects()
{
...
//////////////////////////////////////////////////////////////////////////
EProcessImpactResult CBreakableUpr::ProcessPlaneImpact(const SProcessImpactIn& in, SProcessImpactOut& out)
{
	return (EProcessImpactResult)CBreakablePlane::ProcessImpact(in, out);
}

//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::ExtractPlaneMeshIsland(const SExtractMeshIslandIn& in, SExtractMeshIslandOut& out)
{
	CBreakablePlane::ExtractMeshIsland(in, out);
}

//////////////////////////////////////////////////////////////////////////
void CBreakableUpr::GetPlaneMemoryStatistics(uk pPlaneRaw, IDrxSizer* pSizer)
{
	if (!pPlaneRaw)
		return;
...
//////////////////////////////////////////////////////////////////////////
bool CBreakableUpr::IsGeometryBreakable(IPhysicalEntity* pEntity, IStatObj* pStatObj, IMaterial* pMaterial)
{
	IIndexedMesh* pIndexedMesh = pStatObj->GetIndexedMesh(true);
	if (!pIndexedMesh)
		return true;

	i32 subsetCount = pIndexedMesh->GetSubSetCount();
	for (i32 i = 0; i < subsetCount; ++i)
	{
	...
	i32 subobjectCount = pStatObj->GetSubObjectCount();
	for (i32 i = 0; i < subobjectCount; ++i)
	{
