...
static i32k MAX_AABB_DIFFERENCES = 4 * 4 * 4;
static void AABBDifference(const AABB& a, const AABB& b, AABB* differences, i32* ndifferences)
{
	*ndifferences = 0;
...
void CProximityTriggerSystem::MoveTrigger(SProximityElement* pTrigger, const AABB& aabb, bool invalidateCachedAABB)
{
	...
	if (invalidateCachedAABB)
	{
		...
		u32 num = (u32)pTrigger->inside.size();
		for (u32 i = 0; i < num; ++i)
		{
		...
		if (!m_triggers.empty())
		{
			...
			for (u32 i = 0; i < num; ++i)
			{
				...
				if (pTrigger->bActivated)
				{
					...
					for (i32 j = 0; j < (i32)pTrigger->inside.size(); )
					{
					...
					/*
					   #define LOG_AABB(idx,xx) DrxLogAlways("diff[%d]: (%.1f,%.1f,%.1f)->(%.1f,%.1f,%.1f)", idx, xx.min.x, xx.min.y, xx.min.z, xx.max.x, xx.max.y, xx.max.z)
					          if (ndifferences)
					          {
...
//////////////////////////////////////////////////////////////////////////
inline bool IsIntersectInAxis(const AABB& a, const AABB& b, u32 axis)
{
...
void CProximityTriggerSystem::ProcessOverlap(SProximityElement* pEntity, SProximityElement* pTrigger)
{
	...
	//DrxLogAlways( "[Trigger] Overlap %s and %s",GET_ENTITY_NAME(pEntity->id),GET_ENTITY_NAME(pTrigger->id) );
}
...
void CProximityTriggerSystem::Update()
{
	...
	if (!m_entities.empty())
	{
		...
		if (!m_minPosList0.empty() && !m_minPosList1.empty())
		{
			...
			u32k* Sorted0 = &m_Sorted0[0];
			u32k* Sorted1 = &m_Sorted1[0];
			u32k* const LastSorted0 = &Sorted0[m_minPosList0.size()];
			u32k* const LastSorted1 = &Sorted1[m_minPosList1.size()];
			u32k* RunningAddress0 = Sorted0;
			u32k* RunningAddress1 = Sorted1;
			...
			while (RunningAddress1 < LastSorted1 && Sorted0 < LastSorted0)
			{
				...
				u32k* RunningAddress2_1 = RunningAddress1;
			...
			while (RunningAddress0 < LastSorted0 && Sorted1 < LastSorted1)
			{
				...
				u32k* RunningAddress2_0 = RunningAddress0;
...
void CProximityTriggerSystem::SendEvent(EEntityEvent eventId, EntityId triggerId, EntityId entityId)
{
	...
	if (pTriggerEntity)
	{
		...
		//DrxLogAlways( "[Trigger] Enter %s into %s",GET_ENTITY_NAME( entityId ),pTriggerEntity->GetName() );
		//	else
		// if (eventId = ENTITY_EVENT_LEAVEAREA)
		//DrxLogAlways( "[Trigger] Leave %s from %s",GET_ENTITY_NAME( entityId ),pTriggerEntity->GetName() );
		//	else
		//		assert(false)
		//}
		event.event = eventId;
...
{
	for (i32 i = 0; i < (i32)m_triggersToRemove.size(); i++)
	{
		SProximityElement* pTrigger = m_triggersToRemove[i];
		PREFAST_ASSUME(pTrigger);
		for (i32 j = 0; j < (i32)pTrigger->inside.size(); j++)
		{
			SProximityElement* pEntity = pTrigger->inside[j];
...
{
	i32 num = (i32)pEntity->inside.size();
	for (i32 i = 0; i < num; ++i)
	{
...
void CProximityTriggerSystem::CheckIfLeavingTrigger(SProximityElement* pEntity)
{
	...
	for (i32 i = 0; i < (i32)pEntity->inside.size(); )
	{
		SProximityElement* pTrigger = pEntity->inside[i];
		if (!pTrigger->aabb.IsIntersectBox(pEntity->aabb))
		{
...
void CProximityTriggerSystem::Reset()
{
	...
	for (u32 i = 0; i < (u32)m_entitiesToRemove.size(); i++)
	{
		delete m_entitiesToRemove[i];
	}

	for (u32 i = 0; i < (u32)m_triggersToRemove.size(); i++)
	{
...
void CProximityTriggerSystem::GetMemoryUsage(IDrxSizer* pSizer) const
{
