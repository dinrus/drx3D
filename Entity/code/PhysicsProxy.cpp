...
i32k PHYS_ENTITY_SUSPEND = 1;
i32k PHYS_ENTITY_RESTORE = 2;
...
namespace
{
static i32 ValidateSetParams(const EventPhys* pEvent)
{
	...
	if ((event->iSimClass[1] == SC_SLEEPING_RIGID || event->iSimClass[1] == SC_ACTIVE_RIGID) && event->pEntity->GetType() == PE_ARTICULATED)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_ENTITYSYSTEM, VALIDATOR_ERROR_DBGBRK, "articulated entity simulated as rigid while still attached to a host");
		}
	}
	return 1;
}
...
i32 CEntityPhysics::GetPhysAttachId()
{
	return GetEntity()->m_hierarchy.attachId;
}

i32 CEntityPhysics::GetPartId0(i32 nSlot)
{
	i32 id = nSlot;
	i32 nLevels, nBits;
...
i32 AttachPartId(i32 idPart, i32* attachIdRemap)
{
	i32 nLevels, nBits;
...
i32 DetachPartId(i32 idPart, i32* attachIdRemap)
{
	i32 nLevels, nBits;
	EntityPhysicsUtils::ParsePartId(idPart, nLevels, nBits);
	i32 idAttach = attachIdRemap[idPart >> nBits & EntityPhysicsUtils::PARTID_MAX_ATTACHMENTS - 1];
	if (!(idPart & 1 << 30 && idAttach >= 0))
		return -1;
	return idAttach & 1 << 30 ^ 1 << 30 | (nLevels - 1) << 28 | idAttach << nBits | idPart & (1 << nBits) - 1;
}

inline i32 AllocAttachId(attachMask& usedMask)
{
	attachMask mask1 = (usedMask ^ usedMask + 1) + 1;
	i32 id = !!mask1 ? ilog2(mask1) - 1 : EntityPhysicsUtils::PARTID_MAX_ATTACHMENTS - 1;
	usedMask |= attachMask1 << id;
	return id;
}

void CEntityPhysics::RemapChildAttachIds(CEntity* pent, attachMask& idmaskSrc, attachMask& idmaskDst, i32* idmap)
{
	if (pent->m_hierarchy.pParent)
		for (uint i = 0; i < pent->m_hierarchy.children.size(); i++)
		{
			i32 idOld = pent->m_hierarchy.children[i]->m_hierarchy.attachId;
			if (idOld < 0)
				continue;
			i32 idNew = idmap[idOld] = AllocAttachId(idmaskDst);
...
//////////////////////////////////////////////////////////////////////////
void CEntityPhysics::UpdateParamsFromRenderMaterial(i32 nSlot, IPhysicalEntity* pPhysEntity)
{
	...
	if (pMtl)
	{
		...
		i32 numIds = pMtl->FillSurfaceTypeIds(surfaceTypesId);
...
void CEntityPhysics::ProcessEvent(const SEntityEvent& event)
{
	...
	case ENTITY_EVENT_ATTACH:
		{
			...
			if (pChild && pChild->GetParentBindingType() != CEntity::EBindingType::eBT_LocalSim)
			{
				    ...
				    (i = pAdamProxy->m_pPhysicalEntity->GetType()) <= PE_WHEELEDVEHICLE && i != PE_STATIC)
				{
					...
					i32 idmap[EntityPhysicsUtils::PARTID_MAX_ATTACHMENTS + 1];
	...
	case ENTITY_EVENT_COLLISION:
		if (event.nParam[1] == 1)
		{
			...
			i32 islot = EntityPhysicsUtils::GetSlotIdx(pColl->partid[1]);
			pe_params_bbox pbb;
			Vec3 sz;
			float r, strength;
			Matrix34 mtx;
					...
					if (!is_unused(pepjb->pNewEntity[iop]) && (pent = static_cast<CEntity*>(pepjb->pEntity[iop]->GetForeignData(PHYS_FOREIGN_ID_ENTITY))))
					{
						...
						if (j >= 0)
						{
							...
							else
							{
								...
								if (pSubObj)
								{
									IParticleEffect* pEffect = gEnv->pParticleUpr->FindEffect(pSubObj->properties);
									if (pEffect)
...
void CEntityPhysics::OnTimer(i32 id)
{
	...
	{
		for (i32 slot = 0; slot < GetEntity()->GetSlotCount(); slot++)
		{
			IRenderMesh* pRM;
...
void CEntityPhysics::Serialize(TSerialize ser)
{
	if (ser.BeginOptionalGroup("PhysicsProxy", (ser.GetSerializationTarget() == eST_Network ? !GetEntity()->HasInternalFlag(CEntity::EInternalFlag::PhysicsDisableNetworkSerialization) : NeedNetworkSerialize())))
	{
		...
		{
			DRX_ASSERT_MESSAGE(ser.GetSerializationTarget() != eST_Network,
		...
		if (ser.GetSerializationTarget() != eST_Network) // no foliage over network for now.
		{
			...
			{
				for (i32 i = 0; pSlot->GetCharacter()->GetISkeletonPose()->GetCharacterPhysics(i); i++)
				{
...
void CEntityPhysics::SerializeTyped(TSerialize ser, i32 type, i32 flags)
{
...
void CEntityPhysics::EnablePhysics(bool bEnable)
{
	...
	{
		for (i32 i = 0, n = GetEntity()->GetSlotCount(); i < n; ++i)
		{
...
void CEntityPhysics::Physicalize(SEntityPhysicalizeParams& params)
{
	...
	if (m_pPhysicalEntity)
	{
		...
		if (params.type == PE_ARTICULATED && previousPhysType == PE_LIVING)
		{
			...
			v = sd.v;
			i32 hideOnly = params.fStiffnessScale > 0;
			if (hideOnly)
			{
				pe_params_foreign_data pfd;
...
bool CEntityPhysics::TriggerEventIfStateChanged(IPhysicalEntity* pPhysEntity, const pe_status_pos* pPrevStatus) const
{
	...
	i32k currSimClass = psp.iSimClass;
...
//////////////////////////////////////////////////////////////////////////
void CEntityPhysics::AssignPhysicalEntity(IPhysicalEntity* pPhysEntity, i32 nSlot)
{
	assert(pPhysEntity);
...
//////////////////////////////////////////////////////////////////////////
i32 CEntityPhysics::AddSlotGeometry(i32 nSlot, SEntityPhysicalizeParams& params, i32 bNoSubslots)
{
...
//////////////////////////////////////////////////////////////////////////
void CEntityPhysics::RemoveSlotGeometry(i32 nSlot)
{
	CEntitySlot* pSlot = GetEntity()->GetSlot(nSlot);
...
//////////////////////////////////////////////////////////////////////////
void CEntityPhysics::UpdateSlotGeometry(i32 nSlot, IStatObj* pStatObjNew, float mass, i32 bNoSubslots)
{
...
//////////////////////////////////////////////////////////////////////////
phys_geometry* CEntityPhysics::GetSlotGeometry(i32 nSlot)
{
	CEntitySlot* pSlot = GetEntity()->GetSlot(nSlot);
	if (!pSlot || !pSlot->GetStatObj())
		return 0;
...
//////////////////////////////////////////////////////////////////////////
void CEntityPhysics::DestroyPhysicalEntity(bool bDestroyCharacters, i32 iMode)
{
	...
	for (i32 i = 0; i < GetEntity()->GetSlotCount(); i++)
...
//////////////////////////////////////////////////////////////////////////
void CEntityPhysics::ReattachSoftEntityVtx(IPhysicalEntity* pAttachToEntity, i32 nAttachToPart)
{
	for (i32 slot = 0; slot < GetEntity()->GetSlotCount(); slot++)
	{
...
void CEntityPhysics::AttachSoftVtx(IRenderMesh* pRM, IPhysicalEntity* pAttachToEntity, i32 nAttachToPart)
{
	...
	if ((pColors.data = (ColorB*)pRM->GetColorPtr(pColors.iStride, FSL_READ)))
	{
		...
		aap.nPoints = 0;
		i32k verticesCount = pRM->GetVerticesCount();
		aap.piVtx = new i32[verticesCount];
		u32 dummy = 0, mask = 0, * pVtxMap = pRM->GetPhysVertexMap();
		i32 i;
		if (!pVtxMap)
	...
	case SEntityPhysicalizeParams::AreaDefinition::AREA_SPHERE:
		{
			...
			IGeometry* pGeom = PhysicalWorld()->GetGeomUpr()->CreatePrimitive(primitives::sphere::type, &geomSphere);
	...
	case SEntityPhysicalizeParams::AreaDefinition::AREA_BOX:
		{
			...
			geomBox.bOriented = 0;
			IGeometry* pGeom = PhysicalWorld()->GetGeomUpr()->CreatePrimitive(primitives::box::type, &geomBox);
			pPhysicalEntity = PhysicalWorld()->AddArea(pGeom, affineParts.pos, affineParts.rot, fEntityScale);
		}
		break;
	...
	case SEntityPhysicalizeParams::AreaDefinition::AREA_CYLINDER:
		{
			...
			/*
			   DrxLogAlways( "CYLINDER: center:(%f %f %f) axis:(%f %f %f) radius:%f height:%f",
			 ...
			 */
			IGeometry* pGeom = PhysicalWorld()->GetGeomUpr()->CreatePrimitive(primitives::cylinder::type, &geomCylinder);
			pPhysicalEntity = PhysicalWorld()->AddArea(pGeom, affineParts.pos, affineParts.rot, fEntityScale);
		}
		break;
	...
	case SEntityPhysicalizeParams::AreaDefinition::AREA_SPLINE:
		{
			...
			for (i32 i = 0; i < pAreaDef->nNumPoints; i++)
...
bool CEntityPhysics::PhysicalizeCharacter(SEntityPhysicalizeParams& params)
{
	...
	i32 partid0 = GetPartId0(params.nSlot);
	...
	if (!m_pPhysicalEntity || params.type == PE_LIVING)
	{
		...
		if (pAttman)
			for (i32 i = pAttman->GetAttachmentCount() - 1; i >= 0; i--)
	...
	else
	{
		...
		m_pPhysicalEntity->SetParams(&pp);
		for (i32 iAux = 0; pCharacter->GetISkeletonPose()->GetCharacterPhysics(iAux); iAux++)
			pCharacter->GetISkeletonPose()->GetCharacterPhysics(iAux)->SetParams(&pfd);
	}
	return true;
}
...
bool CEntityPhysics::ConvertCharacterToRagdoll(SEntityPhysicalizeParams& params, const Vec3& velInitial)
{
	...
	if (pPhysEntity)
	{
		...
		//i32 i, nAux; = pCharacter->CreateAuxilaryPhysics( pPhysEntity, GetEntity()->GetSlotWorldTM(params.nSlot) );
		pe_params_foreign_data pfd;
		pfd.iForeignFlagsOR = PFF_UNIMPORTANT;
		if (pPhysEntity)
		...
		for (i32 nAux = 0; pCharacter->GetISkeletonPose()->GetCharacterPhysics(nAux); nAux++)
		{
			...
			for (i32 i = pAttman->GetAttachmentCount() - 1; i >= 0; i--)
			{
				...
				if (pAttachmentObject && pAttman->GetInterfaceByIndex(i)->GetType() == CA_BONE)
				{
					...
					if (pAttachedCharacter)
						for (i32 nAux = 0; pAttachedCharacter->GetISkeletonPose()->GetCharacterPhysics(nAux); nAux++)
						{
...
//////////////////////////////////////////////////////////////////////////
bool CEntityPhysics::PhysicalizeFoliage(i32 iSlot)
{
	if (GetEntity()->GetEntityRender())
...
void CEntityPhysics::DephysicalizeFoliage(i32 iSlot)
{
	if (IFoliage* pFoliage = GetFoliage(iSlot))
		pFoliage->Release();
}

IFoliage* CEntityPhysics::GetFoliage(i32 iSlot)
{
	if (GetEntity()->GetEntityRender())
		if (IRenderNode* pRN = GetEntity()->GetRenderNode(iSlot))
...
//////////////////////////////////////////////////////////////////////////
void CEntityPhysics::AddImpulse(i32 ipart, const Vec3& pos, const Vec3& impulse, bool bPos, float fAuxScale, float fPushScale /* = 1.0f */)
{
	ENTITY_PROFILER
...
void CEntityPhysics::OnPhysicsPostStep(EventPhysPostStep* pEvent)
{
	...
	if (physType != PE_RIGID) // In Rigid body sub-parts are not controlled by physics.
	{
		if (!GetEntity()->HasInternalFlag(CEntity::EInternalFlag::PhysicsSyncCharacter) && (physType == PE_ARTICULATED || physType == PE_WHEELEDVEHICLE))
		{
			...
			for (i32 slot = 0; slot < GetEntity()->GetSlotCount(); slot++)
			{
				i32 nSlotFlags = GetEntity()->GetSlotFlags(slot);
...
//////////////////////////////////////////////////////////////////////////
void CEntityPhysics::CreateRenderGeometry(i32 nSlot, IGeometry* pFromGeom, bop_meshupdate* pLastUpdate)
{
...
void CEntityPhysics::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
{
	const ptrdiff_t offsetFromEntity = Drx::Memory::GetMemberOffset(&CEntity::m_physics);
