...
DRXREGISTER_CLASS(CEntityComponentSubstitution);
...
void CEntityComponentSubstitution::Done()
{
	...
	if (m_pSubstitute)
	{
		//gEnv->pLog->Log("DRXSIS-3502: CSubstitutionProxy::Done: Ptr=%d", (i32)m_pSubstitute);
		//gEnv->pLog->Log("DRXSIS-3502: CSubstitutionProxy::Done: %s", m_pSubstitute->GetName());
		//gEnv->pLog->Log("DRXSIS-3502: CSubstitutionProxy::Done: Pos=(%.2f,%.2f,%.2f)", m_pSubstitute->GetPos().x, m_pSubstitute->GetPos().y, m_pSubstitute->GetPos().z);
...
void CEntityComponentSubstitution::SetSubstitute(IRenderNode* pSubstitute)
{
	...
	//gEnv->pLog->Log("DRXSIS-3502: CSubstitutionProxy::SetSubstitute: Ptr=%d", (i32)m_pSubstitute);
	//gEnv->pLog->Log("DRXSIS-3502: CSubstitutionProxy::SetSubstitute: %s", m_pSubstitute->GetName());
	//gEnv->pLog->Log("DRXSIS-3502: CSubstitutionProxy::SetSubstitute: Pos=(%.2f,%.2f,%.2f)", m_pSubstitute->GetPos().x, m_pSubstitute->GetPos().y, m_pSubstitute->GetPos().z);
}
...
void CEntityComponentSubstitution::GameSerialize(TSerialize ser)
{
	...
	if (ser.IsReading())
	{
		if (!m_pSubstitute)
		{
			...
			m_pSubstitute = 0;
			i32 i = gEnv->pPhysicalWorld->GetEntitiesInBox(center - Vec3(0.05f), center + Vec3(0.05f), pents, ent_static);
			...
			{
				//gEnv->pLog->Log("DRXSIS-3502: CSubstitutionProxy::Serialize: Ptr=%d", (i32)m_pSubstitute);
				//gEnv->pLog->Log("DRXSIS-3502: CSubstitutionProxy::Serialize: %s", m_pSubstitute->GetName());
				//gEnv->pLog->Log("DRXSIS-3502: CSubstitutionProxy::Serialize: Pos=(%.2f,%.2f,%.2f)", m_pSubstitute->GetPos().x, m_pSubstitute->GetPos().y, m_pSubstitute->GetPos().z);
