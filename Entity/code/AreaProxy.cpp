...
DRXREGISTER_CLASS(CEntityComponentArea);
...
{
	m_pArea = static_cast<CAreaUpr*>(g_pIEntitySystem->GetAreaUpr())->CreateArea();
		...
		case ENTITY_AREA_TYPE_SHAPE:
			{
				...
				DRX_ASSERT_MESSAGE(!s_tmpWorldPoints.empty(), "An area shape without points cannot be moved, Verify that it is properly initialized!");
...
void CEntityComponentArea::ProcessEvent(const SEntityEvent& event)
{
	...
	case ENTITY_EVENT_SCRIPT_EVENT:
		{
			tukk pName = (tukk)event.nParam[0];
...
void CEntityComponentArea::LegacySerializeXML(XmlNodeRef& entityNode, XmlNodeRef& componentNode, bool bLoading)
{
	...
	if (bLoading)
	{
		...
		if (pointsNode)
		{
			...
			i32 const numPoints = pointsNode->getChildCount();
			DRX_ASSERT(s_tmpWorldPoints.empty());
		...
		else if (areaNode->getAttr("VolumeRadius", m_fRadius))
		{
			...
			if (pointsNode)
			{
				for (i32 i = 0; i < pointsNode->getChildCount(); i++)
				{
					XmlNodeRef pntNode = pointsNode->getChild(i);
					Vec3 pt;
		...
		else if (areaNode->getAttr("AreaSolidFileName", &token))
		{
			...
			{
				i32 numberOfClosedPolygon = 0;
				i32 numberOfOpenPolygon = 0;
		...
		else
		{
			...
			if (pNodeSoundData)
			{
				DRX_ASSERT(m_abObstructSound.size() == 0);

				for (i32 i = 0; i < pNodeSoundData->getChildCount(); ++i)
				{
				...
				DRX_ASSERT(m_abObstructSound.size() == 6);
			}
		}
		...
		if (entitiesNode)
		{
			for (i32 i = 0; i < entitiesNode->getChildCount(); i++)
			{
	...
	else
	{
		...
		if (type == ENTITY_AREA_TYPE_SHAPE)
		{
			...
			if ((numLocalPoints + 2) != m_abObstructSound.size())
			{
				...
				DRX_ASSERT_MESSAGE(false, temp.c_str());
		...
		else if (type == ENTITY_AREA_TYPE_BOX)
		{
			...
			{
				DRX_ASSERT_MESSAGE(m_abObstructSound.size() == 6, "AreaBox is missing obstruct sound data. Its initialization could have failed!");
			}
		}
		else if (type == ENTITY_AREA_TYPE_GRAVITYVOLUME)
		{
			...
			for (u32 i = 0; i < m_bezierPoints.size(); i++)
			{
		...
		if (!entIDs.empty())
		{
			...
			for (u32 i = 0; i < entIDs.size(); i++)
			{
				i32 entityId = entIDs[i];
...
//////////////////////////////////////////////////////////////////////////
void CEntityComponentArea::ReadPolygonsForAreaSolid(CDrxFile& file, i32 numberOfPolygons, bool bObstruction)
{
	static i32k numberOfStaticVertices(200);
	Vec3 pStaticVertices[numberOfStaticVertices];

	for (i32 i = 0; i < numberOfPolygons; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
void CEntityComponentArea::AddConvexHullToSolid(const Vec3* verticesOfConvexHull, bool bObstruction, i32 numberOfVertices)
{
...
//////////////////////////////////////////////////////////////////////////
void CEntityComponentArea::SetGravityVolume(const Vec3* pPoints, i32 nNumPoints, float fRadius, float fGravity, bool bDontDisableInvisible, float fFalloff, float fDamping)
{
	m_bIsEnableInternal = true;
	m_fRadius = fRadius;
	m_fGravity = fGravity;
...
{
	DRX_ASSERT(index < m_abObstructSound.size());
