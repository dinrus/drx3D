...
void CEntitySlot::UpdateRenderNode(bool bForceRecreateNode)
{
	...
	if (m_pRenderNode)
	{
		...
		{
			u32 entityFlags = m_pEntity->GetFlags();
...
//////////////////////////////////////////////////////////////////////////
void CEntitySlot::OnXForm(i32 nWhyFlags)
{
	ComputeWorldTransform();
...
bool CEntitySlot::IsRendered() const
{
	...
	else if (m_pRenderNode)
	{
		...
		if (std::abs((i32)m_pRenderNode->GetDrawFrame() - (i32)gEnv->nMainFrameID) < 3)
		{
			return true;
		}
	}
...
/*
   //////////////////////////////////////////////////////////////////////////
   void CEntityObject::Render(SRendParams& rParams, i32 nRndFlags, CRenderProxy* pIEntityRender, const SRenderingPassInfo& passInfo)
   {
     ...
     i32 nOldObjectFlags = rParams.dwFObjFlags;
   ...
   /*
     ...
     {
      //DRX_FIXME(03,12,2009,"Animation & Rendering of entities needs to be re-written to avoid derivation of local offset due to float inaccuracy - Richard Semmens");
     ...
     u32k renderProxyFlags = pIEntityRender->GetFlags();
...
//////////////////////////////////////////////////////////////////////////
void CEntitySlot::SetFlags(u32 nFlags)
{
	bool bChange = m_flags != nFlags;
	u32 prevFlags = m_flags;
	m_flags = nFlags;
...
bool CEntitySlot::GetBoundsChanged() const
{
	if (GetCharacter())
	{
		i32 characterFlags = GetCharacter()->GetFlags();
...
//////////////////////////////////////////////////////////////////////////
void CEntitySlot::SetAsLight(const SRenderLight& lightData, u16 layerId)
{
	if (m_pRenderNode && m_pRenderNode->GetRenderNodeType() != eERType_Light)
	{
		ReleaseObjects();
