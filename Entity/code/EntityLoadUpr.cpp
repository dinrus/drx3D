...
/*************************************************************************
   ...
   $Id$
   $DateTime$
   Описание: Handles management for loading of entities
...
//////////////////////////////////////////////////////////////////////////
CEntityLoadUpr::~CEntityLoadUpr()
{
	stl::free_container(m_queuedFlowgraphs);
}

//////////////////////////////////////////////////////////////////////////
void CEntityLoadUpr::Reset()
{
	m_entityPool.clear();
}

//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::LoadEntities(XmlNodeRef& entitiesNode, bool bIsLoadingLevelFile, const Vec3& segmentOffset)
{
	bool bResult = false;
...
//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::ReserveEntityIds(XmlNodeRef& entitiesNode)
{
	assert(entitiesNode);
	...
	i32k iChildCount = (entitiesNode ? entitiesNode->getChildCount() : 0);
...
//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::CanParseEntity(XmlNodeRef& entityNode)
{
	assert(entityNode);
	...
	if (entityNode->getAttr("MinSpec", nMinSpec) && nMinSpec > 0)
	{
		...
		i32 obj_quality = (e_obj_quality ? e_obj_quality->GetIVal() : 0);
	...
	if (bResult)
	{
		tukk pLayerName = entityNode->getAttr("Layer");
...
//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::ParseEntities(XmlNodeRef& entitiesNode, bool bIsLoadingLevelFile, const Vec3& segmentOffset)
{
	...
	i32k entityCount = entitiesNode->getChildCount();

	DrxLog("Parsing %d entities...", entityCount);
		...
		if (entityNode && entityNode->isTag("Entity") && CanParseEntity(entityNode))
		{
			...
			if (!gEnv->IsEditorGameMode())
			{
				...
				if (!ExtractEntityLoadParams(entityNode, loadParameterStorage.back(), segmentOffset, true))
				{
					...
					EntityWarning("CEntityLoadUpr::ParseEntities : Failed when parsing entity \'%s\'", sName.empty() ? "Unknown" : sName.c_str());
				}
				else
				{
...
bool CEntityLoadUpr::ExtractCommonEntityLoadParams(XmlNodeRef& entityNode, SEntityLoadParams& outLoadParams, const Vec3& segmentOffset, bool bWarningMsg, tukk szEntityClass, tukk szEntityName) const
{
	assert(entityNode);
	...
	if (pClass)
	{
		...
		i32 castShadowMinSpec = CONFIG_LOW_SPEC;
		...
		tukk szArchetypeName = entityNode->getAttr("Archetype");
		...
		if (XmlNodeRef componentsNode = entityNode->findChild("Components"))
		{
			for (i32 i = 0, n = componentsNode->getChildCount(); i < n; ++i)
			{
				...
				if (!componentNode->haveAttr("typeId"))
				{
					...
					DRX_ASSERT(!typeGUID.IsNull());

					if (const sxema::IEnvComponent* pEnvComponent = gEnv->pSchematyc->GetEnvRegistry().GetComponent(typeGUID))
					{
...
//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::ExtractArcheTypeLoadParams(XmlNodeRef& entityNode, SEntityLoadParams& outLoadParams, const Vec3& segmentOffset, bool bWarningMsg) const
{
	...
	tukk szEntityName = entityNode->getAttr("Name");
	return ExtractCommonEntityLoadParams(entityNode, outLoadParams, segmentOffset, bWarningMsg, entityClass.c_str(), szEntityName);
}

//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::ExtractArcheTypeLoadParams(XmlNodeRef& entityNode, SEntitySpawnParams& spawnParams) const
{
...
//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::ExtractEntityLoadParams(XmlNodeRef& entityNode, SEntityLoadParams& outLoadParams, const Vec3& segmentOffset, bool bWarningMsg) const
{
	tukk szEntityClass = entityNode->getAttr("EntityClass");
	tukk szEntityName = entityNode->getAttr("Name");
	return ExtractCommonEntityLoadParams(entityNode, outLoadParams, segmentOffset, bWarningMsg, szEntityClass, szEntityName);
}

//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::ExtractEntityLoadParams(XmlNodeRef& entityNode, SEntitySpawnParams& spawnParams) const
{
	SEntityLoadParams loadParams;
...
//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::CreateEntity(XmlNodeRef& entityNode, SEntitySpawnParams& pParams, EntityId& outUsingId)
{
	SEntityLoadParams loadParams;
...
//////////////////////////////////////////////////////////////////////////
bool CEntityLoadUpr::CreateEntity(CEntity* pPreallocatedEntity, SEntityLoadParams& loadParams, EntityId& outUsingId, bool bIsLoadingLevellFile)
{
	...
	DrxGUID entityGuid = spawnParams.guid;
	if (entityNode)
	{
		// Only runtime prefabs should have GUID id's
		tukk entityGuidStr = entityNode->getAttr("Id");
		if (entityGuidStr[0] != '\0')
		{
			entityGuid = DrxGUID::FromString(entityGuidStr);
		}
	}
	...
	if (bResult && pSpawnedEntity)
	{
		...
		{
			// ... we load it...
			IMaterial* pMtl = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(szMtlName);
			if (pMtl)
			{
				// ... and set it...
		...
		tukk szAttachmentType = entityNode->getAttr("AttachmentType");
		...
		if (entityNode->getAttr("ParentGuid", parentGuid))
		{
			...
			DRX_ASSERT_MESSAGE(spawnParams.pParent != nullptr, "Parent must have been spawned before the child!");
		}
		else
		{
			// Legacy maps loading.
		...
		if (entityNode)
		{
			...
			tukk sGeom = entityNode->getAttr("Geometry");
			if (sGeom[0] != 0)
			{
				// check if character.
				tukk ext = PathUtil::GetExt(sGeom);
			...
			if (linksNode)
			{
				i32k iChildCount = linksNode->getChildCount();
				for (i32 i = 0; i < iChildCount; ++i)
				{
	...
	if (!bResult)
	{
		EntityWarning("[CEntityLoadUpr::CreateEntity] Entity Load Failed: %s (%s)", spawnParams.sName, spawnParams.pClass->GetName());
	}
...
//////////////////////////////////////////////////////////////////////////
void CEntityLoadUpr::PrepareBatchCreation(i32 nSize)
{
...
//////////////////////////////////////////////////////////////////////////
void CEntityLoadUpr::AddQueuedFlowgraph(CEntity* pEntity, XmlNodeRef& pNode)
{
	SQueuedFlowGraph f;
	f.pEntity = pEntity;
	f.pNode = pNode;
...
//////////////////////////////////////////////////////////////////////////
void CEntityLoadUpr::AddQueuedEntityLink(CEntity* pEntity, XmlNodeRef& pNode)
{
	SQueuedFlowGraph f;
	f.pEntity = pEntity;
	f.pNode = pNode;
...
//////////////////////////////////////////////////////////////////////////
void CEntityLoadUpr::OnBatchCreationCompleted()
{
	// Load flowgraphs
	...
	for (; itQueuedEntityLink != itQueuedEntityLinkEnd; ++itQueuedEntityLink)
	{
		...
		if (f.pEntity)
		{
			...
			tukk sLinkName = f.pNode->getAttr("Name");
