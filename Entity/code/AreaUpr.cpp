	...
	: m_bAreasDirty(true)
{
	...
	{
		pSystemEventDispatcher->RegisterListener(this, "CAreaUpr");
	}
}
...
//////////////////////////////////////////////////////////////////////////
CArea* CAreaUpr::CreateArea()
{
	DrxAutoCriticalSectionNoRecursive lock(m_accessAreas);
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::Unregister(CArea const* const pArea)
{
	DrxAutoCriticalSectionNoRecursive lock(m_accessAreas);
...
//////////////////////////////////////////////////////////////////////////
IArea const* const CAreaUpr::GetArea(size_t const nAreaIndex) const
{
#if defined(DEBUG_AREAMANAGER)
	if (nAreaIndex >= m_areas.size())
	{
		DrxFatalError("<AreaUpr>: GetArea index out of bounds (Count: %d Index: %d)!", static_cast<i32>(m_areas.size()), static_cast<i32>(nAreaIndex));
	}
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::DrawLinkedAreas(EntityId linkedId) const
{
...
//////////////////////////////////////////////////////////////////////////
bool CAreaUpr::GetLinkedAreas(EntityId linkedId, EntityId* pOutArray, i32& outAndMaxResults) const
{
	i32 nMaxResults = outAndMaxResults;
	i32 nArrayIndex = 0;
...
//////////////////////////////////////////////////////////////////////////
size_t CAreaUpr::GetLinkedAreas(EntityId linkedId, i32 areaId, std::vector<CArea*>& areas) const
{
	size_t const nCount = m_areas.size();
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::MarkEntityForUpdate(EntityId const entityId)
{
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::TriggerAudioListenerUpdate(IArea const* const _pArea)
{
...
void CAreaUpr::Update()
{
	...
	if (!m_mapEntitiesToUpdate.empty())
	{
		...
		{
			DRX_ASSERT(entityIdPair.second > 0);
			--(entityIdPair.second);
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::UpdateEntity(Vec3 const& position, CEntity* const pIEntity)
{
	EntityId const entityId = pIEntity->GetId();
	...
	if (pAreaCache != nullptr && !position.IsEquivalent(pAreaCache->lastUpdatePos, posDelta))
	{
		...
		DRX_ASSERT(m_areasAtPos[Threads::Main].empty());
		...
		for (SAreaCacheEntry& areaCacheEntry : pAreaCache->entries)
		{
			...
			if (!bIsHidden && (pArea->GetState() & Drx::AreaUpr::EAreaState::Hidden) != 0)
			{
				...
				areaCacheEntry.bNear = false;
				areaCacheEntry.bInside = false;
				pArea->RemoveState(Drx::AreaUpr::EAreaState::Hidden);
			}
		...
		if (pIEntityAudioComponent != nullptr)
		{
			...
			{
				pIEntityAudioComponent->SetEnvironmentAmount(areaEnvironment.audioEnvironmentId, areaEnvironment.amount, DrxAudio::InvalidAuxObjectId);
			}
		}
...
//////////////////////////////////////////////////////////////////////////
bool CAreaUpr::QueryAudioAreas(Vec3 const& pos, SAudioAreaInfo* const pResults, size_t const numMaxResults, size_t& outNumResults)
{
	DrxAutoCriticalSectionNoRecursive lock(m_accessAreas);
	...
	if (pResults != nullptr && numMaxResults > 0)
	{
		...
		u32 numAreas = 0;
		DRX_ASSERT(m_areasAtPos[Threads::Audio].empty());
		...
		if (m_areaGrid.GetAreas(position, m_areasAtPos[Threads::Audio]))
		{
			...
			DRX_ASSERT(outNumResults == areaEnvironments.size());
  ...
  AreaEnvironments& areaEnvironments)
{
	...
	if (bIsPointWithin)
	{
		...
		u32 const nEntityFlagsExtended = pIEntity->GetFlagsExtended();
	...
	else
	{
		...
		if (isNear)
		{
			...
			if (!bExclusiveUpdate)
			{
				...
				DRX_ASSERT(areaCacheEntry.bNear && !areaCacheEntry.bInside); // We must be near but not inside yet!
...
//////////////////////////////////////////////////////////////////////////
bool CAreaUpr::ProceedExclusiveUpdateByHigherArea(
  SAreasCache* const pAreaCache,
  ...
  AreaEnvironments& areaEnvironments)
{
	...
	bool bResult = false;
	i32 const currentGroupId = pArea->GetGroup();
	...
	if (currentGroupId > 0)
	{
		...
		i32 const minPriority = pArea->GetPriority();
		...
		for (SAreaCacheEntry const& areaCacheEntry : pAreaCache->entries)
		{
			...
			if (currentGroupId == pHigherPrioArea->GetGroup())
			{
				...
				if (minPriority < pHigherPrioArea->GetPriority() && (pHigherPrioArea->GetState() & Drx::AreaUpr::EAreaState::Hidden) == 0)
				{
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::ExitArea(EntityId const entityId, CArea const* const pArea)
{
	if (pArea != nullptr)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CAreaUpr::GetEnvFadeValueInner(
  SAreasCache const& areaCache,
...
{
	bool bSuccess = false;
	i32 const currentGroupId = areaCacheEntry.pArea->GetGroup();

	if (currentGroupId > 0)
	{
		// Find areas of higher priority that belong to the same group as the passed pArea.
		i32 const minPriority = areaCacheEntry.pArea->GetPriority();
...
//////////////////////////////////////////////////////////////////////////
bool CAreaUpr::RetrieveEnvironmentAmount(
	...
	if (attachedEntities > 0)
	{
		for (size_t i = 0; i < attachedEntities; ++i)
		{
			...
			if (pIEntity != nullptr)
			{
				...
				if (pIEntityAudioComponent != nullptr)
				{
					DrxAudio::EnvironmentId const audioEnvironmentId = pIEntityAudioComponent->GetEnvironmentId();

					if (audioEnvironmentId != DrxAudio::InvalidEnvironmentId)
					{
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::ExitAllAreas(EntityId const entityId)
{
	SAreasCache* const pAreaCache = GetAreaCache(entityId);
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::DrawAreas(ISystem const* const pSystem)
{
#if defined(INCLUDE_ENTITYSYSTEM_PRODUCTION_CODE)
	...
	i32 const drawDebugValue = CVar::pDrawAreaDebug->GetIVal();
	...
	{
		float posY = 10.0f;
		IRenderAuxText::Draw2dLabel(10.0f, posY, 1.5f, color, false, "<AreaUpr>");
		posY += 20.0f;
		IRenderAuxText::Draw2dLabel(30.0f, posY, 1.3f, color, false, "Entities: %d | Areas in Grid: %d", static_cast<i32>(m_mapAreaCache.size()), static_cast<i32>(m_areaGrid.GetNumAreas()));
		posY += 20.0f;
		...
		for (auto const& areaCachePair : m_mapAreaCache)
		{
			...
			if (pIEntity != nullptr)
			{
				...
				if (drawDebugValue == 1 || drawDebugValue == 2 || entityId == static_cast<EntityId>(drawDebugValue))
				{
					...
					IRenderAuxText::Draw2dLabel(30.0f, posY, 1.3f, color, false, "Entity: %d (%s) Pos: (%.2f, %.2f, %.2f) Areas in AreaCache: %d", entityId, pIEntity->GetName(), pos.x, pos.y, pos.z, static_cast<i32>(areaCache.entries.size()));
					posY += 12.0f;
					...
					for (SAreaCacheEntry const& areaCacheEntry : areaCache.entries)
					{
						...
						DrxFixedStringT<16> areaTypeName("unknown");
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::DrawGrid()
{
	m_areaGrid.Draw();
}

//////////////////////////////////////////////////////////////////////////
size_t CAreaUpr::MemStat()
{
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::SetAreasDirty()
{
	m_bAreasDirty = true;
}

//////////////////////////////////////////////////////////////////////////
void CAreaUpr::SetAreaDirty(IArea* pArea)
{
	if (!m_bAreasDirty)
	{
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::UpdateDirtyAreas()
{
	if (m_bAreasDirty)
	{
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::AddEventListener(IAreaUprEventListener* pListener)
{
	stl::push_back_unique(m_EventListeners, pListener);
}

//////////////////////////////////////////////////////////////////////////
void CAreaUpr::RemoveEventListener(IAreaUprEventListener* pListener)
{
	stl::find_and_erase(m_EventListeners, pListener);
}

//////////////////////////////////////////////////////////////////////////
void CAreaUpr::OnEvent(EEntityEvent event, EntityId TriggerEntityID, IArea* pArea)
{
	for (auto const pListener : m_EventListeners)
	{
		pListener->OnAreaUprEvent(event, TriggerEntityID, pArea);
	}
}

//////////////////////////////////////////////////////////////////////////
i32 CAreaUpr::GetNumberOfPlayersNearOrInArea(CArea const* const pArea)
{
	// Find the area index
...
//////////////////////////////////////////////////////////////////////////
size_t CAreaUpr::GetOverlappingAreas(const AABB& bb, PodArray<IArea*>& list) const
{
...
//////////////////////////////////////////////////////////////////////////
bool CAreaUpr::SIsNotInGrid::operator()(SAreaCacheEntry const& cacheEntry) const
{
	bool bResult = false;
...
bool CAreaUpr::SRemoveIfNoAreasLeft::operator()(std::pair<K, V>& cacheEntry) const
{
	bool bResult = false;
...
//////////////////////////////////////////////////////////////////////////
void CAreaUpr::OnSystemEvent(ESystemEvent event, UINT_PTR wparam, UINT_PTR lparam)
{
	switch (event)
	{
	case ESYSTEM_EVENT_LEVEL_LOAD_END:
		{
			LOADING_TIME_PROFILE_SECTION_NAMED("CAreaUpr::OnSystemEvent ESYSTEM_EVENT_LEVEL_LOAD_END");
			for (auto const pArea : m_areas)
			{
