...
DRXREGISTER_CLASS(CEntityComponentRope);
...
void CEntityComponentRope::Initialize()
{
	...
	i32 nSlot = GetOrMakeEntitySlotId();
...
void CEntityComponentRope::GameSerialize(TSerialize ser)
{
	...
	/*
	   ...
	   if (pSlot && pSlot->pCharacter && !pSlot->pCharacter->GetISkeleton()->GetCharacterPhysics())
	    for(i32 i=0;pSlot->pCharacter->GetISkeleton()->GetCharacterPhysics(i);i++)
	      if (ser.IsReading())
	...
	if (bLoading)
	{
		...
		if (ropeNode)
		{
			...
			if (pointsNode)
			{
				...
				for (i32 i = 0, num = pointsNode->getChildCount(); i < num; i++)
				{
			...
			if (xmlNodeAudio)
			{
				...
				audioParams.startTrigger = DrxAudio::StringToId(szTemp);

				xmlNodeAudio->getAttr("StopTrigger", &szTemp);
				audioParams.stopTrigger = DrxAudio::StringToId(szTemp);

				xmlNodeAudio->getAttr("AngleParameter", &szTemp);
				audioParams.angleParameter = DrxAudio::StringToId(szTemp);

				std::underlying_type<DrxAudio::EOcclusionType>::type tempOcclusionType;
				xmlNodeAudio->getAttr("OcclusionType", tempOcclusionType);
				audioParams.occlusionType = static_cast<DrxAudio::EOcclusionType>(tempOcclusionType);
