...
CEntityLayer::CEntityLayer(tukk name, u16 id, bool havePhysics, i32 specs, bool defaultLoaded, TGarbageHeaps& garbageHeaps)
	: m_name(name)
	, m_id(id)
	, m_isEnabled(true)
...
void CEntityLayer::GetMemoryUsage(IDrxSizer* pSizer, i32* pOutNumEntities)
{
	if (pOutNumEntities)
		*pOutNumEntities = 0;
	for (TEntityProps::iterator it = m_entities.begin(); it != m_entities.end(); ++it)
