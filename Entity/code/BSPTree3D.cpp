...
void CBSPTree3D::BSPTreeNode::GetMemoryUsage(class IDrxSizer* pSizer) const
{
...
void CBSPTree3D::BuildTree(const IBSPTree3D::FaceList& faceList, NodeStorage& treeNodes) const
{
	...
	while (!taskQueue.empty())
	{
		...
		if (!curTask.m_FaceList.empty())
		{
			...
			for (i32 i = 1, faceSize(curTask.m_FaceList.size()); i < faceSize; ++i)
			{
...
AreaUtil::ESplitResult CBSPTree3D::SplitFaceByPlane(const AreaUtil::CPlaneBase& plane, const IBSPTree3D::CFace& inFace, IBSPTree3D::CFace& outPosFace, IBSPTree3D::CFace& outNegFace)
{
	...
	for (i32 i = 0, iPtSize(inFace.size()); i < iPtSize; ++i)
	{
	...
	if (bPosPt && bNegPt)
	{
		i32 nIndexFromPlusToMinus = -1;
		i32 nIndexFromMinusToPlus = -1;
		Vec3 vFromPlusToMinus;
		Vec3 vFromMinusToPlus;

		for (i32 i = 0, iPtSize(signInfoList.size()); i < iPtSize; )
		{
			i32 next_i = i;
		...
		for (i32 i = 0, iSize(signInfoList.size()); i < iSize; ++i)
		{
			i32 nIndex = (i + nIndexFromMinusToPlus) % iSize;
		...
		for (i32 i = 0, iSize(signInfoList.size()); i < iSize; ++i)
		{
			i32 nIndex = (i + nIndexFromPlusToMinus) % iSize;
...
void CBSPTree3D::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
size_t CBSPTree3D::WriteToBuffer(uk pBuffer) const
{
	if (u32 nNodes = m_BSPTree.size())
	{
		const size_t nBufferSize = sizeof(nNodes) + nNodes * sizeof(BSPTreeNode);

		if (pBuffer)
		{
			u32 nNodeCount = SwapEndianValue(nNodes);
			memcpy(pBuffer, &nNodeCount, sizeof(nNodeCount));

			BSPTreeNode* pDst = reinterpret_cast<BSPTreeNode*>((u8*)pBuffer + sizeof(nNodeCount));
...
void CBSPTree3D::ReadFromBuffer(ukk pBuffer)
{
	if (pBuffer)
	{
		u32 nNodes = 0;
		...
		if (nNodes > 0)
		{
			BSPTreeNode* pSrc = reinterpret_cast<BSPTreeNode*>((u8*)pBuffer + sizeof(nNodes));
