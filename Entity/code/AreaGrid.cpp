...
static constexpr i32 GridCellSize = 4;
...
//////////////////////////////////////////////////////////////////////////
bool CAreaGrid::GetAreas(u32 const x, u32 const y, TAreaPointers& outAreas)
{
	DRX_ASSERT(x < m_numCellsPerAxis && y < m_numCellsPerAxis);

	u32 const* const pBitsLHS = m_bitFieldX.data() + (m_bitFieldSizeU32 * x);
	u32 const* const pBitsRHS = m_bitFieldY.data() + (m_bitFieldSizeU32 * y);

	for (u32 i = 0, offset = 0; i < m_bitFieldSizeU32; ++i, offset += 32)
	{
		u32 currentBitField = pBitsLHS[i] & pBitsRHS[i];

		for (u32 j = 0; currentBitField != 0; ++j, currentBitField >>= 1)
		{
	...
	if (m_pAreas != nullptr)
	{
		...
		for (; Iter != IterEnd; ++Iter)
		{
			if ((*Iter) == pArea)
			{
				...
				DRX_ASSERT(outIndex < m_pAreas->size());
				bSuccess = true;
...
//////////////////////////////////////////////////////////////////////////
void CAreaGrid::AddAreaBit(const Vec2i& start, const Vec2i& end, u32 areaIndex)
{
	DRX_ASSERT(start.x >= 0 && start.y >= 0 && end.x < (i32)m_numCellsPerAxis && end.y < (i32)m_numCellsPerAxis);
	DRX_ASSERT(start.x <= end.x && start.y <= end.y);

	u32* pBits;
	u32k bucketBit = 1 << (areaIndex & 31);
	u32k bucketIndex = areaIndex >> 5;

	// -- X ---
	pBits = m_bitFieldX.data() + (m_bitFieldSizeU32 * start.x) + bucketIndex;
	for (i32 i = start.x; i <= end.x; i++, pBits += m_bitFieldSizeU32)
		*pBits |= bucketBit;

	// -- Y ---
	pBits = m_bitFieldY.data() + (m_bitFieldSizeU32 * start.y) + bucketIndex;
	for (i32 i = start.y; i <= end.y; i++, pBits += m_bitFieldSizeU32)
		*pBits |= bucketBit;
...
//////////////////////////////////////////////////////////////////////////
void CAreaGrid::RemoveAreaBit(u32 areaIndex)
{
...
bool CAreaGrid::ResetArea(CArea* pArea)
{
	...
	if (m_pAreas != nullptr)
	{
		...
		TAreaPointers::const_iterator iter(m_pAreas->begin());
		TAreaPointers::const_iterator const iterEnd(m_pAreas->end());
		u32 const numAreas = std::min<u32>(m_maxNumAreas, static_cast<u32>(m_pAreas->size()));
...
//////////////////////////////////////////////////////////////////////////
void CAreaGrid::AddArea(CArea* pArea, u32 areaIndex)
{
	...
	Vec2i start((i32)((vBBCentre.x - vBBExtent.x) * GridCellSizeR), (i32)((vBBCentre.y - vBBExtent.y) * GridCellSizeR));
	Vec2i end((i32)((vBBCentre.x + vBBExtent.x) * GridCellSizeR), (i32)((vBBCentre.y + vBBExtent.y) * GridCellSizeR));

	if ((end.x | end.y) < 0 || start.x >= (i32)m_numCellsPerAxis || start.y > (i32)m_numCellsPerAxis)
		return;

	start.x = std::max<i32>(start.x, 0);
	start.y = std::max<i32>(start.y, 0);
	end.x = std::min<i32>(end.x, (i32)m_numCellsPerAxis - 1);
	end.y = std::min<i32>(end.y, (i32)m_numCellsPerAxis - 1);
...
void CAreaGrid::Compile(TAreaPointers const& areas)
{
	...
	i32 const terrainSize = gEnv->p3DEngine->GetTerrainSize();
	u32 const numCellsPerAxis = static_cast<u32>(terrainSize / GridCellSize);
	...
	u32 const oldNumCellsPerAxis = m_numCellsPerAxis;
	u32 const numAreas = static_cast<u32>(areas.size());
	u32 const bitFieldSizeU32 = ((numAreas + 31) >> 5);
...
bool CAreaGrid::GetAreas(Vec3 const& position, TAreaPointers& outAreas)
{
	...
	{
		u32 const gridX = static_cast<u32>(position.x * GridCellSizeR);
		u32 const gridY = static_cast<u32>(position.y * GridCellSizeR);
...
void CAreaGrid::Draw()
{
	...
	if (m_pAreas != nullptr && !m_pAreas->empty())
	{
		...
		for (u32 gridX = 0; gridX < m_numCellsPerAxis; gridX++)
		{
			float const cellX = (static_cast<float>(gridX) + 0.5f) * GridCellSize;

			for (u32 gridY = 0; gridY < m_numCellsPerAxis; gridY++)
			{
				...
				if (CVar::pDrawAreaGridCells->GetIVal() != 0)
				{
					...
					i32k w = IRenderAuxGeom::GetAux()->GetCamera().GetViewSurfaceX();
					i32k h = IRenderAuxGeom::GetAux()->GetCamera().GetViewSurfaceZ();
...
void CAreaGrid::Debug_CheckBB(Vec2 const& vBBCentre, Vec2 const& vBBExtent, CArea const* const pArea)
{
	...
	{
		DrxLogAlways("Error: AABB extent was not found in grid\n");
	}
}
#endif // INCLUDE_ENTITYSYSTEM_PRODUCTION_CODE
