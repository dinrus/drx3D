...
static DrxCriticalSection g_updateSchedulingProfileCritSec;
...
NetworkAspectType CNetEntity::CombineAspects()
{
	...
	if (!m_bNoSyncPhysics && (m_pEntity->GetPhysicalEntity() || m_pProfileUpr))
	{
		aspects |= eEA_Physics;
	}
...
bool CNetEntity::CaptureProfileUpr(IGameObjectProfileUpr* pPM)
{
	if (m_pProfileUpr || !pPM)
		return false;
	m_pProfileUpr = pPM;
	return true;
}
void CNetEntity::ReleaseProfileUpr(IGameObjectProfileUpr* pPM)
{
	if (m_pProfileUpr != pPM)
		return;
	m_pProfileUpr = 0;
...
void CNetEntity::ClearProfileUpr()
{
	m_pProfileUpr = 0;
}
bool CNetEntity::HasProfileUpr()
{
	return m_pProfileUpr != nullptr;
}

bool CNetEntity::NetSerializeEntity(TSerialize ser, EEntityAspects aspect, u8 profile, i32 flags)
{
	...
	if (aspect == eEA_Physics && !m_pProfileUpr)
	{
...
void CNetEntity::RmiRegister(const SRmiHandler& handler)
{
	...
	DRX_ASSERT_MESSAGE(found == m_rmiHandlers.end(), "Registering a duplicate RMI message.");

	DRX_ASSERT_MESSAGE(m_rmiHandlers.size() < std::numeric_limits<decltype(SRmiIndex::value)>::max(),
...
INetEntity::SRmiIndex CNetEntity::RmiByDecoder(SRmiHandler::DecoderF decoder, SRmiHandler** handler)
{
	...
	DRX_ASSERT_MESSAGE(found != m_rmiHandlers.end(), "Sending an unregistered RMI message.");
...
void CNetEntity::SetChannelId(u16 id)
{
	m_channelId = id;
}
u8 CNetEntity::GetAspectProfile(EEntityAspects aspect)
{
	u8 profile = 255;
	if (m_isBoundToNetwork)
	{
...
bool CNetEntity::SetAspectProfile(EEntityAspects aspect, u8 profile, bool fromNetwork)
{
...
bool CNetEntity::DoSetAspectProfile(EEntityAspects aspect, u8 profile, bool fromNetwork)
{
	if (m_isBoundToNetwork)
	{
		if (fromNetwork)
		{
			if (m_pProfileUpr)
			{
				if (m_pProfileUpr->SetAspectProfile(aspect, profile))
				{
	...
	else if (m_pProfileUpr)
	{
		//DRX_ASSERT( !fromNetwork );
		if (m_pProfileUpr->SetAspectProfile(aspect, profile))
		{
			m_profiles[BitIndex(NetworkAspectType(aspect))] = profile;
			return true;
...
u8 CNetEntity::GetDefaultProfile(EEntityAspects aspect)
{
	if (m_pProfileUpr)
		return m_pProfileUpr->GetDefaultProfile(aspect);
	else
		return 0;
}
