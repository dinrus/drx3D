...
i32 CBreakablePlane::g_nPieces = 0;
float CBreakablePlane::g_maxPieceLifetime = 0;

i32 UpdateBrokenMat(IMaterial* pRenderMat, i32 idx, tukk substName)
{
	...
	if (pRenderMat)
	{
		   ...
		   else*/if ((pSubMtl = pRenderMat->GetSubMtl(idx)) && (len = strlen(name = pSubMtl->GetName())) && (len < 7 || strcmp(name + len - 7, "_broken")))
		{
			...
			if (i < 0 && *substName)
			{
				...
				if (i < 0)
				{
					if (IMaterial* pMatSubst = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(substName, false))
					{
...
bool CBreakablePlane::SetGeometry(IStatObj* pStatObj, IMaterial* pRenderMat, i32 bStatic, i32 seed)
{
	m_bStatic = bStatic;
	...
	if (phys_geometry* pPhysGeom = pStatObj->GetPhysGeom())
	{
		...
		i32 iup = idxmin3((float*)&bbox.size);
		if (min(bbox.size[inc_mod3[iup]], bbox.size[dec_mod3[iup]]) < bbox.size[iup] * 4)
		{
			DrxLog("[Breakable2d] : geometry is not thin enough, breaking denied ( %s %s)", pStatObj->GetFilePath(), pStatObj->GetGeoName());
			return false;
		}
		...
		m_nCells.set((i32)(bbox.size[inc_mod3[iup]] * 2 / m_cellSize + 0.5f), (i32)(bbox.size[dec_mod3[iup]] * 2 / m_cellSize + 0.5f));
		m_nCells.x = max(1, m_nCells.x);
		m_nCells.y = max(1, m_nCells.y);
		if (m_nCells.x * m_nCells.y > 4000)
		{
			DrxLogAlways("[Breakable2d] : breakable tessellation is too fine, breaking denied ( %s %s)", pStatObj->GetFilePath(), pStatObj->GetGeoName());
			return false;
		}
		m_pGeom = 0;
		m_pSampleRay = 0;
		...
		if (pPhysGeom->pGeom->GetType() == GEOM_TRIMESH)
		{
			...
			if (pPhysMesh->pMats)
				for (i32 i = 1; i < pPhysMesh->nTris; i++)
					if (pPhysMesh->pMats[i] != pPhysMesh->pMats[0])
					{
						DrxLogAlways("[Breakable2d] : geometry has several sub materials, breaking denied ( %s %s)", pStatObj->GetFilePath(), pStatObj->GetGeoName());
						return false;
					}
			i32 i;
			i32 j;
			...
			if (i < pPhysMesh->nTris)
			{
				...
				do
				{
					...
					for (i32 iter0 = 0; iter0 < pPhysMesh->nTris; iter0++)
					{
						j = inc_mod3[j];
						i32 i1;
			...
			if (cosa > 0.03f)
			{
				...
				aray.origin.zero();
				aray.dir = axis;
				m_pSampleRay = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::ray::type, &aray);
			}
		}
		else
		{
		...
		i32k indexCount = pMesh->GetIndexCount();
		...
		IGeomUpr* pGeoman = gEnv->pPhysicalWorld->GetGeomUpr();
...
void CBreakablePlane::FillVertexData(CMesh* pMesh, i32 ivtx, const Vec2& pos, i32 iside)
{
	Quat qnRot = IDENTITY;
	Vec2 Coord(ZERO);
	...
	if (m_pGeom)
	{
		...
		WriteLockCond lock;
		geom_contact* pcont;
		if (i32 ncont = m_pGeom->IntersectLocked(m_pSampleRay, gwd, gwd + 1, 0, pcont, lock))
		{
...
IStatObj* CBreakablePlane::CreateFlatStatObj(i32*& pIdx, Vec2* pt, Vec2* bounds, const Matrix34& mtxWorld,
                                             IParticleEffect* pEffect, bool bNoPhys, bool bUseEdgeAlpha)
{
	i32 i, j, nVtx, nCntVtx, nTris, itri, itriCnt, nCntVtxTri, bValid, clrmask;
	i32* pVtxMap, * pCntVtxMap, * pCntVtxList;
	...
	memset(pVtxMap = new i32[nVtx], -1, sizeof(pVtxMap[0]) * nVtx);
	memset(pCntVtxMap = new i32[nVtx], -1, sizeof(pVtxMap[0]) * nVtx);
	...
	i32 b2Sided = pIdx[i] == -1 || !m_bOneSided;

	i32 nOutVtx = nVtx + (nVtx + nCntVtx * 4) * b2Sided;
	...
	if (bNoPhys && *pIdx != -2)
	{
		...
		i32k vertexCount = pMesh->GetVertexCount();
	...
	if (*pIdx == -2 && m_bAxisAligned) // && m_bStatic)
	{
		...
		params.size.Set(max(1, (i32)(n.x + 0.5f)), max(1, (i32)(n.y + 0.5f)), max(1, (i32)(n.z + 0.5f)));
...
i32* CBreakablePlane::Break(const Vec3& pthit, float r, Vec2*& ptout, i32 seed, float filterAng, float ry)
{
...
void CBreakablePlane::ExtractMeshIsland(const SExtractMeshIslandIn& in, SExtractMeshIslandOut& out)
{
	...
	static  i32 g_lockExtractMeshIsland = 0;
	...
	if ((unsigned)in.itriSeed < (unsigned)pmd->nTris && pmd->pMats[in.itriSeed] >= 0)
	{
		...
		{
			i32 surfaceTypesIds[MAX_SUB_MATERIALS];
		...
		Vec3 center(ZERO);
		queue[0] = in.itriSeed;
		i32 imat = pmd->pMats[queue[0]];
		pmd->pMats[queue[0]] = -2;
		i32 nNewTris = 0;
		i32 ivtxMin = pmd->nVertices;
		i32 ivtxMax = 0;
		i32 j = 0;
		for (i32 itail = 0, ihead = 1; ihead != itail && nNewTris < pmd->nTris; nNewTris++)
		{
			i32 i = queue[itail];
			itail = itail + 1 & DRX_ARRAY_COUNT(queue) - 1;
			for (j = 0; j < 3; j++)
			{
				...
				if (in.bCreateIsle)
				{
					i32 ivtx = pIdx[pmd->pForeignIdx[i] * 3 + j];
		...
		if (in.bCreateIsle)
		{
			...
			i32 idxMin = pmd->pForeignIdx[in.itriSeed] * 3;
			i32 ichunk = meshChunks.size() - 1;

			for (; ichunk > 0 && (u32)idxMin - meshChunks[ichunk].nFirstIndexId >= meshChunks[ichunk].nNumIndices; ichunk--)
				;
			...
			float r = 0.f;
			for (i32 i = 0; i <= ivtxMax - ivtxMin; i++)
			{
		...
		else
		{
			...
			for (i32 i = 0; i < pmd->nTris; i++)
				if (pmd->pMats[i] == -2)
				{
					for (i32 ivtx = 0; ivtx < 3; ivtx++)
					{
						i32 idx = pmd->pForeignIdx[i] * 3 + ivtx;
...
i32 CBreakablePlane::ProcessImpact(const SProcessImpactIn& in, SProcessImpactOut& out)
{
	...
	i32 bStatic = 1 ^ iszero(i32(in.processFlags & ePlaneBreak_Static));
	i32 bAutoSmash = 1 ^ iszero(i32(in.processFlags & ePlaneBreak_AutoSmash));
	i32 bFractureEffect = iszero(i32(in.processFlags & ePlaneBreak_NoFractureEffect));
	...
	i32 nMaxParticles = particle_limit ? particle_limit->GetIVal() : 200;
	if (curTime > g_maxPieceLifetime)
		g_nPieces = 0;
	i32 nCurParticles = min(g_nPieces, gEnv->pPhysicalWorld->GetEntityCount(PE_PARTICLE));
	...
	if (pPhysGeom = pStatObj->GetPhysGeom())
	{
		    ...
		    )
		{
			...
			if (bAutoSmash && pIsleStatObj == NULL)
			{
				if (bFractureEffect && pBreak2DParams->full_fracture_fx[0] &&
				    (pFractureFx = gEnv->pParticleUpr->FindEffect(pBreak2DParams->full_fracture_fx)))
				{
		...
		{
			if (!in.bVerify)
				DrxLog("[Breakable2d] : geometry is not suitable for breaking, breaking denied ( %s %s)", in.pStatObj->GetFilePath(), in.pStatObj->GetGeoName());
		...
		tukk strEffectName = pBreak2DParams->particle_effect;
		IParticleEffect* pEffect = 0;
		if (bFractureEffect && strEffectName[0])
			pEffect = gEnv->pParticleUpr->FindEffect(strEffectName);
		bool bUseEdgeAlpha = pBreak2DParams->use_edge_alpha != 0;
		float ry = r * (1.0f + drx_random(0.0f, pBreak2DParams->vert_size_spread));
		...
		if (bFractureEffect && !pFractureFx && pBreak2DParams->fracture_fx[0] &&
		    (pFractureFx = gEnv->pParticleUpr->FindEffect(pBreak2DParams->fracture_fx)))
		{
			Vec3 n = in.mtx.TransformVector(bbox.Basis.GetRow(iup));
		...
		else if (!pPlane)
		{
			...
			drx_strcpy(pPlane->m_mtlSubstName, pBreak2DParams->broken_mtl);
				...
				if (pNewStatObj = pPlane->CreateFlatStatObj(pIdx, ptout, bounds, in.mtx, pEffect, !bRigidBody, bUseEdgeAlpha))
				{
					...
					if (*pIdx == -1 && lifetime > 0)
					{
						...
						if (!bRigidBody)
						{
							...
							pNewStatObj->AddRef();
							IParticleEmitter* pEmitter = gEnv->pParticleUpr->CreateEmitter(QuatTS(params.qRotation, params.vPosition), pp);
							if (pEmitter)
