			...
			if (numSegments > 0)
			{
				...
				else
				{
					...
					if (index == numSegments)
					{
						...
						if (bObstructs)
						{
							m_state |= Drx::AreaUpr::EAreaState::ObstructRoof;
						}
						else
						{
							m_state &= ~Drx::AreaUpr::EAreaState::ObstructRoof;
					...
					else if (index == numSegments + 1)
					{
						...
						if (bObstructs)
						{
							m_state |= Drx::AreaUpr::EAreaState::ObstructFloor;
						}
						else
						{
							m_state &= ~Drx::AreaUpr::EAreaState::ObstructFloor;
...
void CArea::SetAreaType(EEntityAreaType type)
{
	...
	// AreaUpr::UpdatePlayer function, that caused problems.
...
void CArea::ClearPoints()
{
	...
	m_state &= ~Drx::AreaUpr::EAreaState::Initialized;
}
...
{
	DRX_PROFILE_FUNCTION(PROFILE_ENTITY);
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) == 0)
		return -1;
...
bool CArea::CalcPointWithinNonCached(Vec3 const& point3d, bool const bIgnoreHeight) const
{
	...
	default:
		{
			DrxFatalError("Unknown area type during CArea::CalcPointWithin");
...
float CArea::CalcPointWithinDist(EntityId const nEntityID, Vec3 const& point3d, bool const bIgnoreSoundObstruction /* = true */, bool const bCacheResult /* = true */)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		...
		if (bGoAheadAndCompute)
		{
			...
			case ENTITY_AREA_TYPE_SOLID:
				{
					...
					i32 queryFlag = bIgnoreSoundObstruction ? CAreaSolid::eSegmentQueryFlag_Obstruction : CAreaSolid::eSegmentQueryFlag_All;
					Vec3 vOnHull;
			...
			case ENTITY_AREA_TYPE_SHAPE:
				{
					...
					else
					{
						...
						if (m_height > 0.0f)
						{
							...
							if ((m_state & Drx::AreaUpr::EAreaState::ObstructFloor) == 0)
							{
								fDistToFloor = point3d.z - m_origin;
							}

							if ((m_state & Drx::AreaUpr::EAreaState::ObstructRoof) == 0)
							{
			...
			default:
				{
					DrxFatalError("Unknown area type during CArea::CalcPointWithinDist");
...
float CArea::ClosestPointOnHullDistSq(EntityId const nEntityID, Vec3 const& Point3d, Vec3& OnHull3d, bool const bIgnoreSoundObstruction /* = true */, bool const bCacheResult /* = true */)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		...
		else
		{
			...
			default:
				{
					DrxFatalError("Unknown area type during CArea::ClosestPointOnHullDistSq");
...
float CArea::CalcPointNearDistSq(EntityId const nEntityID, Vec3 const& Point3d, Vec3& OnHull3d, bool const bIgnoreSoundObstruction /* = true */, bool const bCacheResult /* = true */)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		...
		else
		{
						...
						if (m_height)
						{
							...
							if (bIsIn2DShape)
							{
								...
								if ((Point3d.z < m_origin + m_height && Point3d.z > m_origin))
								{
									...
									if (((m_state & Drx::AreaUpr::EAreaState::ObstructRoof) == 0) && (fZRoofSq < fXDistSq) && (fZRoofSq < fZFloorSq))
									{
									...
									if (((m_state & Drx::AreaUpr::EAreaState::ObstructFloor) == 0) && (fZFloorSq < fXDistSq) && (fZFloorSq < fZRoofSq))
									{
			...
			default:
				{
					DrxFatalError("Unknown area type during CArea::CalcPointNearDistSq (1)");
...
float CArea::CalcPointNearDistSq(EntityId const nEntityID, Vec3 const& Point3d, bool const bIgnoreHeight /* = false */, bool const bIgnoreSoundObstruction /* = true */, bool const bCacheResult /* = true */)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		...
		else
		{
			...
			default:
				{
					DrxFatalError("Unknown area type during CArea::CalcPointNearDistSq (2)");
...
EAreaPosType CArea::CalcPosType(EntityId const nEntityID, Vec3 const& rPos, bool const bCacheResult /* = true */)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		...
		else
		{
			...
			default:
				{
					DrxFatalError("Unknown area type during CArea::CalcPosType");
...
void CArea::CalcClosestPointToObstructedShape(EntityId const nEntityID, Vec3& outClosest, float& outClosestDistSq, Vec3 const& sourcePos)
{
	...
	if (m_height)
	{
		...
		if (ePosType == AREA_POS_TYPE_2DINSIDE_ZABOVE && ((m_state & Drx::AreaUpr::EAreaState::ObstructRoof) == 0))
		{
		...
		else if (ePosType == AREA_POS_TYPE_2DINSIDE_ZBELOW && ((m_state & Drx::AreaUpr::EAreaState::ObstructFloor) == 0))
		{
...
void CArea::CalcClosestPointToSolid(Vec3 const& rv3SourcePos, bool bIgnoreSoundObstruction, float& rfClosestDistSq, Vec3* rv3ClosestPos) const
{
	...
	i32 queryFlag = bIgnoreSoundObstruction ? CAreaSolid::eSegmentQueryFlag_All : CAreaSolid::eSegmentQueryFlag_Open;
...
//////////////////////////////////////////////////////////////////////////
void CArea::AddConvexHullToSolid(const Vec3* verticesOfConvexHull, bool bObstruction, i32 numberOfVertices)
{
...
{
	if ((m_state & Drx::AreaUpr::EAreaState::EntityIdsResolved) == 0)
	{
		size_t index = 0;
		...
		m_state |= Drx::AreaUpr::EAreaState::EntityIdsResolved;
	}
}
...
void CArea::AddEntity(const EntityId entId)
{
	if (entId != INVALID_ENTITYID)
	{
		...
		{
			DrxLog("<AreaUpr> Area %u Direct Event: %s", m_entityId, "ATTACH_THIS");
		}
		...
		if (pIEntity != nullptr)
		{
			...
			{
				m_pAreaUpr->TriggerAudioListenerUpdate(this);
...
void CArea::RemoveEntity(EntityId const entId)
{
	if (entId != INVALID_ENTITYID)
	{
		...
		if (CVar::pDrawAreaDebug->GetIVal() == 2)
		{
			DrxLog("<AreaUpr> Area %u Direct Event: %s", m_entityId, "DETACH_THIS");
		}
...
void CArea::SendCachedEventsFor(EntityId const nEntityID)
{
	...
	if (((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0) && nCountCachedEvents > 0)
	{
		for (size_t i = 0; i < nCountCachedEvents; ++i)
		{
			...
			if (cachedEvent.nParam[0] == nEntityID)
			{
				if (CVar::pDrawAreaDebug->GetIVal() == 2)
				{
					...
					DrxLog("<AreaUpr> Area %u Queued Event: %s", m_entityId, sState.c_str());
				}
...
{
	m_pAreaUpr->OnEvent(newEvent.event, (EntityId)newEvent.nParam[0], this);
...
void CArea::EnterArea(EntityId const entityId)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		...
		if (CVar::pDrawAreaDebug->GetIVal() == 2)
		{
			DrxLog("<AreaUpr> Area %u Direct Event: %s", m_entityId, "ENTER");
		}
...
void CArea::LeaveArea(EntityId const entityId)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		if (CVar::pDrawAreaDebug->GetIVal() == 2)
		{
			DrxLog("<AreaUpr> Area %u Direct Event: %s", m_entityId, "LEAVE");
		}
  ...
  float const distance)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		...
		if (CVar::pDrawAreaDebug->GetIVal() == 2)
		{
			DrxLog("<AreaUpr> Area %u Direct Event: %s", m_entityId, "ENTERNEAR");
		}
...
void CArea::LeaveNearArea(EntityId const entityId)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		if (CVar::pDrawAreaDebug->GetIVal() == 2)
		{
			DrxLog("<AreaUpr> Area %u Direct Event: %s", m_entityId, "LEAVENEAR");
		}
  ...
  float const fade)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		if (CVar::pDrawAreaDebug->GetIVal() == 2)
		{
			DrxLog("<AreaUpr> Area %u Direct Event: %s", m_entityId, "MOVEINSIDE");
		}
  ...
  Vec3 const& position)
{
	...
	if ((m_state & Drx::AreaUpr::EAreaState::Initialized) != 0)
	{
		if (CVar::pDrawAreaDebug->GetIVal() == 2)
		{
			DrxLogAlways("<AreaUpr> Area %u Direct Event: %s", m_entityId, "MOVENEAR");
		}
...
//////////////////////////////////////////////////////////////////////////
void CArea::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void CArea::GetRandomPoints(Array<PosNorm> points, CRndGen seed, EGeomForm eForm) const
{
	...
	case ENTITY_AREA_TYPE_SHAPE:
	{
		...
		auto SetVertexPoint = [=](PosNorm& point, i32 n, float tv)
		{
		...
		auto SetSegmentPoint = [=](PosNorm& point, i32 n, float th, float tv)
		{
		...
		if (eForm == GeomForm_Vertices)
		{
			for (auto& point : points)
			{
				i32 part = seed.GetRandom(0, (i32)m_areaPoints.size() - 1);
		...
		else if (eForm == GeomForm_Edges)
		{
			for (auto& point : points)
			{
				i32 part = ext.RandomPart(seed);
				if (m_height > 0.0f && part < (i32)m_areaPoints.size())
				{
					// Vertical edge
