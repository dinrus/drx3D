...
#include <drx3D/Entity/SaltHandle.h>           // CSaltHandle
	...
	void Debug()
	{
		...
		for (EntityIndex i = 0; i < MaxSize; ++i)
		{
			if (m_Buffer[i].m_NextIndex == EndMarker)
				printf("%d.%d ", (i32)i, (i32)m_Buffer[i].m_Salt);
			else if (m_Buffer[i].m_NextIndex == UsedMarker)
				printf("%d.%d* ", (i32)i, (i32)m_Buffer[i].m_Salt);
			else
				printf("%d.%d->%d ", (i32)i, (i32)m_Buffer[i].m_Salt, (i32)m_Buffer[i].m_NextIndex);
		}
