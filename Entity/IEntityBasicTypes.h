...
typedef DrxGUID EntityGUID;

enum class EEntitySimulationMode : u8
{
	Idle,   // Not running.
...
enum class EEvent
{
	...
	//! \par Example
	//! \include DinrusXEntitySys/Examples/MonitorCollision.cpp
	PhysicsCollision,
	...
	//! nParam[0] stores a const DrxAudio::SRequestInfo* const
	AudioTriggerStarted,
...
{
using EEntityEvent = Drx::Entity::EEvent;
...
using EntityEventMask = Drx::Entity::EntityEventMask;
constexpr EntityEventMask ENTITY_EVENT_BIT(EEntityEvent event) { return EventToMask(event); }
}
}
}

using namespace Drx::Entity::Deprecated_With_5_5;

//! Структура SEntityEvent описывает ид сущности и параметры, которые могут быть направлены к любой сущности.
struct SEntityEvent
{
	SEntityEvent(
	  i32k n0,
...
using EntityTransformationFlagsType = u32;
