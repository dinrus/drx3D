...
struct SChildAttachParams
{
	...
	SChildAttachParams(i32k nAttachFlags)
		: m_nAttachFlags(nAttachFlags)
		, m_target(NULL)
	{}

	SChildAttachParams(i32k nAttachFlags, tukk target)
		: m_nAttachFlags(nAttachFlags)
		, m_target(target)
	{}
...
#if (eDrxModule != eDrxM_LegacyGameDLL && eDrxModule != eDrxM_ScriptSystem && eDrxModule != eDrxM_AISystem && eDrxModule != eDrxM_FlowGraph \
  && eDrxModule != eDrxM_GameFramework && eDrxModule != eDrxM_Legacy && eDrxModule != eDrxM_Editor && eDrxModule != eDrxM_EntitySystem \
  && eDrxModule != eDrxM_MonoBridge)
	#define DRX_DEPRECATED_ENTITY_ARCHETYPE DRX_DEPRECATED("(v5.4) Entity archetypes are deprecated in favour of sxema usage")
#else
	#define DRX_DEPRECATED_ENTITY_ARCHETYPE
#endif

#if (eDrxModule != eDrxM_LegacyGameDLL && eDrxModule != eDrxM_System && eDrxModule != eDrxM_ScriptSystem && eDrxModule != eDrxM_Movie \
  && eDrxModule != eDrxM_FlowGraph && eDrxModule != eDrxM_GameFramework && eDrxModule != eDrxM_Legacy && eDrxModule != eDrxM_Editor   \
  && eDrxModule != eDrxM_EntitySystem && eDrxModule != eDrxM_AISystem && eDrxModule != eDrxM_MonoBridge)
	#define DRX_DEPRECATED_ENTITY_LUA DRX_DEPRECATED("(v5.5) Lua has been deprecated in favor of C# and sxema")
#else
	#define DRX_DEPRECATED_ENTITY_LUA
#endif
...
struct SEntitySpawnParams
{
	...
	/*DRX_DEPRECATED_ENTITY_LUA*/ IScriptTable*           pPropertiesTable = nullptr;
	/*DRX_DEPRECATED_ENTITY_LUA*/ IScriptTable*           pPropertiesInstanceTable = nullptr;
	/*DRX_DEPRECATED_ENTITY_ARCHETYPE*/ IEntityArchetype* pArchetype = nullptr;
};
...
struct SEntityPhysicalizeParams
{
	...
	struct AreaDefinition
	{
		...
		i32       nNumPoints;     //!< Number of points in pPoints array.
...
{
#ifndef SWIG
	//! This is a GUID of the sxema environment scope where all entity components must be registered under to be found in UI.
	static DrxGUID GetEntityScopeGUID()
	{
		static DrxGUID guid = "be845278-0dd2-409f-b8be-97895607c256"_drx_guid;
		return guid;
	}
#endif
	...
	using AttachmentFlagsType = u32;

	static constexpr i32 CREATE_NEW_UNIQUE_TIMER_ID = -666;
	static constexpr i32 KILL_ALL_TIMER = -1;

	struct SRenderNodeParams
	{
		u8             lodRatio = 100;                  //!< LOD ratio
		u8             viewDistRatio = 100;             //!< View Distance Ratio
		...
		u32            additionalRenderNodeFlags = 0;   //!< Additional flags to add on the render node (see ERenderNodeFlags)

		// Helper methods.
		void SetRndFlags(uint64 flags, bool enable) { if (enable) additionalRenderNodeFlags |= flags; else additionalRenderNodeFlags &= ~flags; }
		void SetLodRatio(i32 nLodRatio)             { lodRatio = (u8)std::min(255, std::max(0, nLodRatio)); }
		void SetViewDistRatio(i32 nViewDistRatio)   { viewDistRatio = (u8)std::min(255, std::max(0, nViewDistRatio)); }
...
public:
	...
	DRX_DEPRECATED_ENTITY_ARCHETYPE virtual IEntityArchetype* GetArchetype() const = 0;
	...
	virtual u8 GetComponentChangeState() const = 0;
	...
	virtual void SetName(tukk sName) = 0;
	...
	virtual IEntity* GetChild(i32 nIndex) const = 0;
	...
	virtual void Serialize(TSerialize serializer, i32 nFlags = 0) = 0;
	...
	virtual i32 SetTimer(i32 nTimerId, i32 nMilliSeconds) = 0;
	...
	virtual void KillTimer(i32 nTimerId) = 0;
	...
	virtual IEntityComponent* CreateComponentByInterfaceID(const DrxInterfaceID& interfaceId, IEntityComponent::SInitParams* pInitParams = nullptr) = 0;
	...
	virtual IEntityComponent* GetComponentByTypeId(const DrxInterfaceID& interfaceID) const = 0;
	//! Return all components with the specified class ID contained in the entity.
	//! \param interfaceID Identifier for the component implementation.
	virtual void GetComponentsByTypeId(const DrxInterfaceID& interfaceID, DynArray<IEntityComponent*>& components) const = 0;

	//! Return component with the unique GUID.
	//! \param guid Identifier for the component.
	virtual IEntityComponent* GetComponentByGUID(const DrxGUID& guid) const = 0;

	//! Queries an array of entity components implementing the specified component interface
	virtual void              QueryComponentsByInterfaceID(const DrxInterfaceID& interfaceID, DynArray<IEntityComponent*>& components) const = 0;
	//! Queries an entity component implementing the specified component interface
	virtual IEntityComponent* QueryComponentByInterfaceID(const DrxInterfaceID& interfaceID) const = 0;

#if !defined(SWIG)
	// Helper to check for reflection in std::enable_if, since this can't be done with a constexpr function
	template<class ComponentType>
	struct IsComponentReflected { static constexpr bool value = sxema::IsReflectedType<ComponentType>(); };

	// Incorrectly implemented component, either doesn't reflect sxema properties or is an incorrectly configured legacy component!
	...
	{
		static const DrxGUID& GetGUID()
		{
			static const DrxGUID s_guid = sxema::GetTypeGUID<ComponentType>();
			return s_guid;
		}
		...
		static ComponentType* CreateInEntity(IEntity* pEntity, TArgs&& ... args)
		{
			...
			IEntityComponent::SInitParams initParams = IEntityComponent::SInitParams { pEntity, DrxGUID::Create(), "", &sxema::GetTypeDesc<ComponentType>(), EEntityComponentFlags::None, nullptr, nullptr };
	...
	void GetAllComponents(DynArray<ComponentType*>& components, bool bCheckHierarchy = true) const
	{
		...
		DynArray<IEntityComponent*>* pRawComponents = (DynArray<IEntityComponent*>*)(uk ) & components;
		if (bCheckHierarchy)
		{
		...
		{
			*it = static_cast<ComponentType*>((IEntityComponent*)(uk )*it);
		}
	}
	...
	virtual u32 GetComponentsCount() const = 0;
	...
	//! \par Example
	//! \include DinrusXEntitySys/Examples/PhysicalizeEntity.cpp
	...
	virtual void AssignPhysicalEntity(IPhysicalEntity* pPhysEntity, i32 nSlot = -1) = 0;
	...
	//! \see IEntity::Physicalize
	//! \par Example
	//! \include DinrusXEntitySys/Examples/PhysicalizeEntitySlot.cpp
	virtual i32              PhysicalizeSlot(i32 slot, SEntityPhysicalizeParams& params) = 0;
	virtual void             UnphysicalizeSlot(i32 slot) = 0;
	virtual void             UpdateSlotPhysics(i32 slot) = 0;
	...
	virtual void AddImpulse(i32 ipart, const Vec3& pos, const Vec3& impulse, bool bPos, float fAuxScale, float fPushScale = 1.0f) = 0;

	//! Physicalizes the foliage of StatObj in slot iSlot.
	virtual bool PhysicalizeFoliage(i32 iSlot) = 0;
	//! Dephysicalizes the foliage in slot iSlot.
	virtual void DephysicalizeFoliage(i32 iSlot) = 0;

	//! retrieve starting partid for a given slot.
	virtual i32 GetPhysicalEntityPartId0(i32 islot = 0) = 0;

	//! Enable/disable network serialization of the physics aspect.
	virtual void PhysicsNetSerializeEnable(bool enable) = 0;
	virtual void PhysicsNetSerializeTyped(TSerialize& ser, i32 type, i32 flags) = 0;
	...
	virtual bool IsSlotValid(i32 nIndex) const = 0;

	//! Allocates index for an entity slot.
	virtual i32 AllocateSlot() = 0;

	//! Frees slot at specified index.
	virtual void FreeSlot(i32 nIndex) = 0;

	//! Gets number of allocated object slots in the entity.
	virtual i32 GetSlotCount() const = 0;
	...
	virtual bool GetSlotInfo(i32 nIndex, SEntitySlotInfo& slotInfo) const = 0;

	//! \return World transformation matrix of the object slot.
	virtual Matrix34 GetSlotWorldTM(i32 nSlot) const = 0;
	...
	virtual Matrix34 GetSlotLocalTM(i32 nSlot, bool bRelativeToParent) const = 0;

	//! Sets local transformation matrix of the object slot.
	virtual void SetSlotLocalTM(i32 nSlot, const Matrix34& localTM, EntityTransformationFlagsMask transformReasons = EntityTransformationFlagsMask()) = 0;

	//! Sets camera space position of the object slot.
	virtual void SetSlotCameraSpacePos(i32 nSlot, const Vec3& cameraSpacePos) = 0;

	//! Gets camera space position of the object slot.
	virtual void GetSlotCameraSpacePos(i32 nSlot, Vec3& cameraSpacePos) const = 0;
	...
	virtual bool SetParentSlot(i32 nParentIndex, i32 nChildIndex) = 0;
	...
	//! \param pMaterial Pointer to custom material interface.
	virtual void SetSlotMaterial(i32 nSlot, IMaterial* pMaterial) = 0;
	...
	virtual IMaterial* GetSlotMaterial(i32 nSlot) const = 0;
	...
	virtual IMaterial* GetRenderMaterial(i32 nSlot = -1) const = 0;
	...
	virtual void SetSlotFlags(i32 nSlot, u32 nFlags) = 0;
	...
	virtual u32 GetSlotFlags(i32 nSlot) const = 0;

	//! Assign sub-object hide mask to slot.
	//! \param nSlot Slot to apply hide mask to.
	virtual void SetSubObjHideMask(i32 nSlot, hidemask nSubObjHideMask) = 0;

	//! Retrieve sub-object hide mask from slot.
	//! \param nSlot Slot to apply hide mask to.
	virtual hidemask GetSubObjHideMask(i32 nSlot) const = 0;

	//! \param nSlot - Index of the slot.
	//! \return true if character is to be updated, otherwise false.
	virtual bool ShouldUpdateCharacter(i32 nSlot) const = 0;
	...
	virtual ICharacterInstance* GetCharacter(i32 nSlot) = 0;
	...
	virtual i32 SetCharacter(ICharacterInstance* pCharacter, i32 nSlot, bool bUpdatePhysics = true) = 0;
	...
	virtual IStatObj* GetStatObj(i32 nSlot) = 0;
	...
	virtual IParticleEmitter* GetParticleEmitter(i32 nSlot) = 0;
	...
	virtual IGeomCacheRenderNode* GetGeomCacheRenderNode(i32 nSlot) = 0;
	...
	//! \param nSlot Index of the slot.
	virtual void MoveSlot(IEntity* targetIEnt, i32 nSlot) = 0;
	...
	virtual i32 SetStatObj(IStatObj* pStatObj, i32 nSlot, bool bUpdatePhysics, float mass = -1.0f) = 0;
	...
	//! \par Example
	//! \include DinrusXEntitySys/Examples/LoadGeometry.cpp
	virtual i32 LoadGeometry(i32 nSlot, tukk sFilename, tukk sGeomName = NULL, i32 nLoadFlags = 0) = 0;
	...
	//! \return Slot id where the object was loaded, or -1 if loading failed.
	//! \par Example
	//! \include DinrusXEntitySys/Examples/LoadCharacter.cpp
	virtual i32 LoadCharacter(i32 nSlot, tukk sFilename, i32 nLoadFlags = 0) = 0;
	...
	//! \return Slot id where the object was loaded, or -1 if loading failed.
	//! \par Example
	//! \include DinrusXEntitySys/Examples/LoadGeomCache.cpp
	virtual i32 LoadGeomCache(i32 nSlot, tukk sFilename) = 0;
#endif

	//! Assigns a 3DEngine render node to the Entity Slot.
	virtual i32 SetSlotRenderNode(i32 nSlot, IRenderNode* pRenderNode) = 0;

	//! Returns a 3DEngine render node assigned to an Entity Slot.
	virtual IRenderNode* GetSlotRenderNode(i32 nSlot) = 0;
	...
	//! \par Example
	//! \include DinrusXEntitySys/Examples/LoadParticleEmitter.cpp
	virtual i32 LoadParticleEmitter(i32 nSlot, IParticleEffect* pEffect, SpawnParams const* params = NULL, bool bPrime = false, bool bSerialize = false) = 0;

	//! Sets an existing particle emitter to the specified slot, or the next available slot.
	//! If this slot number is occupied by different kind of object it is overwritten.
	//! \return Slot id where the particle emitter was loaded, or -1 if loading failed.
	virtual i32 SetParticleEmitter(i32 nSlot, IParticleEmitter* pEmitter, bool bSerialize = false) = 0;
	...
	//! \par Example
	//! \include DinrusXEntitySys/Examples/LoadLight.cpp
	virtual i32 LoadLight(i32 nSlot, SRenderLight* pLight) = 0;
	...
	//! \par Example
	//! \include DinrusXEntitySys/Examples/LoadFogVolume.cpp
	virtual i32 LoadFogVolume(i32 nSlot, const SFogVolumeProperties& properties) = 0;
	...
	DRX_DEPRECATED_ENTITY_LUA virtual IScriptTable* GetScriptTable() const = 0;
	...
	virtual IRenderNode* GetRenderNode(i32 nSlot = -1) const = 0;
	...
	virtual IEntityLink* AddEntityLink(tukk sLinkName, EntityId entityId, EntityGUID entityGuid) = 0;
	virtual void         RenameEntityLink(IEntityLink* pLink, tukk sNewLinkName) = 0;
	...
	virtual IEntity* UnmapAttachedChild(i32& partId) = 0;
	...
	virtual bool HandleVariableChange(tukk szVarName, ukk pVarData) = 0;
	...
	virtual u32 GetEditorObjectID() const = 0;
	...
	virtual sxema::IObject* GetSchematycObject() const = 0;
	virtual void                OnSchematycObjectDestroyed() = 0;

	//////////////////////////////////////////////////////////////////////////
	// Helper methods
	//! Set the viewDistRatio on the render node.
	void  SetViewDistRatio(i32 nViewDistRatio);
	//! Sets the LodRatio on the render node.
	void  SetLodRatio(i32 nLodRatio);
...
DST drxcomponent_cast(SRC pComponent) { return static_cast<DST>(pComponent); }

// Helper methods
//! Set the viewDistRatio on the render node.
inline void IEntity::SetViewDistRatio(i32 nViewDistRatio)
{
...
//! Sets the LodRatio on the render node.
inline void IEntity::SetLodRatio(i32 nLodRatio)
{
	auto params = GetRenderNodeParams();
...
//////////////////////////////////////////////////////////////////////////
ILINE i32 IEntityComponent::GetOrMakeEntitySlotId()
{
...
inline void IEntityComponent::SetTransformMatrix(const Matrix34& transform)
{
	if (m_pTransform == nullptr)
	{
		...
		m_pTransform = std::make_shared<DrxTransform::CTransform>();
	}

	*m_pTransform = DrxTransform::CTransform(transform);
	GetEntity()->UpdateSlotForComponent(this);
}

inline void IEntityComponent::SetTransformMatrix(const DrxTransform::CTransformPtr& transform)
{
	if (m_pTransform == nullptr)
	{
		m_componentFlags.Add(EEntityComponentFlags::Transform);
...
inline i32 ParsePartId(i32 partId, i32& nLevels, i32& nSlotBits)
{
...
inline i32 AllocPartIdRange(i32 partId, i32 nSlots)
{
	i32 nLevels, nBits;
...
inline i32 GetSlotIdx(i32 partId, i32 level = 0)
{
	i32 nLevels, nBits;
	ParsePartId(partId, nLevels, nBits);
