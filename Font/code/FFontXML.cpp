...
static inline i32 GetBlendModeFromString(const string& str, bool dst)
{
	i32 blend = GS_BLSRC_ONE;
...
public:
	...
	void ScanXmlNodesRecursively(XmlNodeRef node)
	{
		...
		for (i32 i = 0, count = node->getNumAttributes(); i < count; ++i)
		{
			tukk key = "";
			tukk value = "";
		...
		for (i32 i = 0, count = node->getChildCount(); i < count; ++i)
		{
...
private:
		...
		case ELEMENT_FONT:
			{
				...
				if (!fontLoaded)
				{
					...
					{
						tukk pFontPath = m_strFontPath.c_str();
						tukk pFontName = PathUtil::GetFile(pFontPath);
	...
	void FoundAttribute(const string& name, const string& value)
	{
		...
		case ELEMENT_FONT:
			...
			{
				m_FontTexSize.x = (i32)atof(value.c_str());
			}
			else if (name == "h")
			{
				m_FontTexSize.y = (i32)atof(value.c_str());
			}
		...
		case ELEMENT_PASS_COLOR:
			if (!m_pPass) break;
			if (name == "r")       { m_pPass->m_color.r = (u8)((float)atof(value.c_str()) * 255.0f); }
			else if (name == "g")  { m_pPass->m_color.g = (u8)((float)atof(value.c_str()) * 255.0f); }
			else if (name == "b")  { m_pPass->m_color.b = (u8)((float)atof(value.c_str()) * 255.0f); }
			else if (name == "a")  { m_pPass->m_color.a = (u8)((float)atof(value.c_str()) * 255.0f); }
			break;
...
public:
	...
	i32                     m_FontSmoothMethod;
	i32                     m_FontSmoothAmount;
};

//////////////////////////////////////////////////////////////////////////
// Main loading function
bool CFFont::Load(tukk pXMLFile)
{
