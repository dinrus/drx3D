...
static i32k TabCharCount = 4;
static const size_t MsgBufferSize = 1024;
static const size_t MaxDrawVBQuads = 1024;

CFFont::CFFont(ISystem* pSystem, CDrxFont* pDrxFont, tukk pFontName)
	: m_name(pFontName)
	, m_curPath("")
	, m_pFontTexture(0)
	, m_fontBufferSize(0)
...
CFFont::~CFFont()
{
	if (m_pDrxFont)
	{
		m_pDrxFont->UnregisterFont(m_name);
		m_pDrxFont = 0;
	}
...
bool CFFont::Load(tukk pFontFilePath, u32 width, u32 height, u32 flags)
{
	if (!pFontFilePath)
		return false;
	...
	i32 iSmoothMethod = (flags & TTFFLAG_SMOOTH_MASK) >> TTFFLAG_SMOOTH_SHIFT;
	i32 iSmoothAmount = (flags & TTFFLAG_SMOOTH_AMOUNT_MASK) >> TTFFLAG_SMOOTH_AMOUNT_SHIFT;
	...
	if (!m_pFontTexture || !m_pFontTexture->CreateFromMemory(pBuffer, (i32) fileSize, width, height, iSmoothMethod, iSmoothAmount))
	{
		delete[] pBuffer;
		return false;
	}
...
void CFFont::DrawString(float x, float y, tukk pStr, const bool asciiMultiLine, const STextDrawContext& ctx)
{
...
void CFFont::DrawString(float x, float y, float z, tukk pStr, const bool asciiMultiLine, const STextDrawContext& ctx)
{
	IF(!pStr, 0) return;
...
void CFFont::RenderCallback(float x, float y, float z, tukk pStr, const bool asciiMultiLine, const STextDrawContext& ctx,IRenderAuxGeom* pAux)
{
	...
	bool isRGB = m_pDrxFont->RndPropIsRGBA();

	float halfTexelShift = m_pDrxFont->RndPropHalfTexelOffset();

	bool passZeroColorOverridden = ctx.IsColorOverridden();

	u32 alphaBlend = passZeroColorOverridden ? ctx.m_colorOverride.a : fx.m_passes[0].m_color.a;
	if (alphaBlend > 128)
	...
	for (size_t j = 0, numPasses = fx.m_passes.size(); j < numPasses; ++j)
	{
		...
		Unicode::CIterator<tukk , false> pChar(pStr);
		while (uint32_t ch = *pChar)
		{
			...
			i32 charWidth = m_pFontTexture->GetCharacterWidth(ch);
			float width = charWidth * rcpCellWidth;

			// get texture coordinates
			float texCoord[4];

			i32 charOffsetX, charOffsetY; // in font texels
			i32 charSizeX, charSizeY;     // in font texels
			...
			DWORD packedColor = 0;
			{
				...
				tempColor.a = ((u32) tempColor.a * alphaBlend) >> 8;
...
Vec2 CFFont::GetTextSize(tukk pStr, const bool asciiMultiLine, const STextDrawContext& ctx)
{
	IF (!pStr, 0)
...
Vec2 CFFont::GetTextSizeUInternal(tukk pStr, const bool asciiMultiLine, const STextDrawContext& ctx)
{
	const size_t fxSize = m_effects.size();
	...
	for (size_t i = 0, numPasses = fx.m_passes.size(); i < numPasses; ++i)
	{
		...
		while (uint32_t ch = *pChar)
		{
			...
			if (ctx.m_proportional)
			{
				i32 iCharWidth = m_pFontTexture->GetCharacterWidth(ch);
...
size_t CFFont::GetTextLength(tukk pStr, const bool asciiMultiLine) const
{
	size_t len = 0;
...
void CFFont::WrapText(string& result, float maxWidth, tukk pStr, const STextDrawContext& ctx)
{
	result = pStr;
	...
	i32 curChar = 0;
	Unicode::CIterator<tukk , false> pChar(result.c_str());
	while (uint32_t ch = *pChar)
	{
		++pChar;
		// ignore color codes
...
void CFFont::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
u32 CFFont::GetEffectId(tukk pEffectName) const
{
	if (pEffectName)
	{
...
{
	m_texID = gEnv->pRenderer->FontCreateTexture(m_pFontTexture->GetWidth(), m_pFontTexture->GetHeight(), (u8*)m_pFontTexture->GetBuffer(), eTF_A8);
	return m_texID >= 0;
}
...
CFFont::SEffect* CFFont::AddEffect(tukk pEffectName)
{
...
void CFFont::Prepare(tukk pStr, bool updateTexture)
{
	bool texUpdateNeeded = m_fontTexDirty || m_pFontTexture->PreCacheString(pStr) == 1;
	if (updateTexture && texUpdateNeeded && m_texID >= 0)
	{
		gEnv->pRenderer->FontUpdateTexture(m_texID, 0, 0, m_pFontTexture->GetWidth(), m_pFontTexture->GetHeight(), (u8*) m_pFontTexture->GetBuffer());
		m_fontTexDirty = false;
	}
	else
