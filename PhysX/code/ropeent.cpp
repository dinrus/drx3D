...
PhysXRope::PhysXRope(i32 id,pe_params* params) 
{ 
...
void PhysXRope::setGlobalPose(const QuatT& pose, i32 components)
{
...
i32 PhysXRope::SetParams(pe_params* _params, i32 bThreadSafe)
{
	if (_params->type==pe_params_rope::type_id) {
		...
		if (i<0) {
			...
			if (!m_joint)
				m_joint = PxDistanceJointCreate(*g_drxPhysX.Physics(), actors[0],T(pt[0]), actors[1],T(pt[1]));
			else {
...
i32 PhysXRope::GetStatus(pe_status* _status) const
{
	if (_status->type==pe_status_rope::type_id) {
		...
		if (!is_unused(status->pVelocities) && status->pVelocities)
			for(i32 i=0; i<=m_points.size(); status->pVelocities[i++].zero());
		return 1;
	}
	return PhysXEnt::GetStatus(_status);
}

i32 PhysXRope::GetParams(pe_params* _params) const
{
	if (_params->type==pe_params_rope::type_id) {
		pe_params_rope *params = (pe_params_rope*)_params;
...
i32 PhysXRope::Action(pe_action* _action, i32 bThreadSafe)
{
