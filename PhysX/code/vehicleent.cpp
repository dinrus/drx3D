...
i32 PhysXVehicle::AddGeometry(phys_geometry* pgeom, pe_geomparams* _params, i32 id, i32 bThreadSafe)
{
	i32 res = PhysXEnt::AddGeometry(pgeom,_params,id,bThreadSafe);
...
i32 PhysXVehicle::SetParams(pe_params* _params, i32 bThreadSafe)
{
...
i32 PhysXVehicle::Action(pe_action* _action, i32 bThreadSafe)
{
...
i32 PhysXVehicle::GetStatus(pe_status* _status) const 
{
	if (_status->type==pe_status_vehicle::type_id && m_vehicle) {
		...
		for(i32 i=status->bWheelContact=0; i<m_wheelsQuery.size(); status->bWheelContact|=(i32)m_wheelsQuery[i++].isInAir);
...
PxQueryHitType::Enum WheelRaycastPreFilter(PxFilterData fdWheel, PxFilterData fdObj, ukk constantBlock, PxU32 constantBlockSize, PxHitFlags& queryFlags)
{
...
bool PhysXVehicle::SetupPxVehicle()
{
	...
	i32 nwheels=m_wheels.size(), driveMask=0;
	...
	i32 nShapes = pRD->getNbShapes();
	PxShape **shapes = (PxShape**)alloca(nShapes*sizeof(uk ));
	...
	for(i32 i=0; i<nwheels; i++) {
		...
		i32 ipos = isneg(m_wheels[i].pivot.x) | 1-sgnnz(m_wheels[i].pivot.y);
		...
		i32 ipart=idxPart(m_wheels[i].partid), ishape;
	...
	m_vehicle->setup(g_drxPhysX.Physics(), pRD, *wsd, dsd, max(nwheels-4,0));
	wsd->free();
	...
	m_rayCaster = g_drxPhysX.Scene()->createBatchQuery(sqDesc);

	return true;
}

void PhysXVehicle::PostStep(float dt, i32 immediate)
{
