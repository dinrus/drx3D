...
PhysXCloth::PhysXCloth(i32 id, pe_params* params) : PhysXEnt(PE_SOFT,id,params) 
{
...
void PhysXCloth::Enable(bool enable)
{
	...
	if (m_cloth)
		if (enable)	{
			g_drxPhysX.Scene()->addActor(*m_cloth);
			InsertBefore(this, g_pPhysWorld->m_auxEntList[1]); 
		} else {
			g_drxPhysX.Scene()->removeActor(*m_cloth);
			if (!(m_mask & 0x18)) {
...
void PhysXCloth::setGlobalPose(const QuatT& pose, i32 components)
{
...
void PhysXCloth::PostStep(float dt, i32 immediate)
{
	if (immediate && m_cloth) {
		...
		m_parts[1].shape->userData = (uk )(INT_PTR)-1;
		...
		if (m_host)	{
			...
			i32 nsph=0, ncaps=0, *capsBuf = (i32*)alloca(m_host->m_parts.size()*sizeof(i32));
			PxCapsuleGeometry caps;
			PxSphereGeometry sph;
			for(i32 i=0;i<m_host->m_parts.size();i++)	{
			...
			//for(i32 i=m_cloth->getNbCollisionCapsules()-1; i>=0; i--)	
			//	m_cloth->removeCollisionCapsule(i);	// Px crashes
			if (!m_cloth->getNbCollisionCapsules())	for(i32 i=0; i<ncaps; i++)
		...
		if ((m_windTimer+=dt*4)>1) {
			...
			m_wind[1] = Vec3(drx_random(wmin.x,wmax.x), drx_random(wmin.y,wmax.y), drx_random(wmin.z,wmax.z));
			m_windTimer -= (i32)m_windTimer;
		}
...
i32 PhysXCloth::RefineRayHit(const Vec3& rayOrg, const Vec3& rayDir, Vec3 &ptRet, Vec3& nRet, i32 partid) const
{
	if (m_cloth) {
...
i32 PhysXCloth::AddGeometry(phys_geometry* pgeom, pe_geomparams* params, i32 id, i32 bThreadSafe)
{
	if (pgeom->pGeom->GetType()!=GEOM_TRIMESH)
		return -1;
	m_mass += max(0.0f, params->mass);
	i32 res = PhysXEnt::AddGeometry(pgeom, params, id, bThreadSafe);
	if (res>=0) {
...
void PhysXCloth::Attach(i32 *piVtx, i32 nPoints)
{
	...
	for(i32 i=0; i<nPoints; vtx[piVtx[i++]].invWeight=0);
	...
	if (!m_parts[0].geom->pForeignData) {
		...
		m_parts[0].geom->pForeignData = PxClothFabricCreate(*g_drxPhysX.Physics(), cmd, V(g_pPhysWorld->m_vars.gravity), false);
	}
	m_cloth = g_drxPhysX.Physics()->createCloth(m_actor->getGlobalPose(), *(PxClothFabric*)m_parts[0].geom->pForeignData, vtx, PxClothFlag::eSCENE_COLLISION);
...
i32 PhysXCloth::SetParams(pe_params* _params, i32 bThreadSafe)
{
...
i32 PhysXCloth::Action(pe_action* _action, i32 bThreadSafe)
{
	if (_action->type==pe_action_attach_points::type_id && m_parts.size()) {
		...
		if (!m_cloth && !m_host && action->pEntity!=WORLD_ENTITY && action->pEntity->GetType()!=PE_STATIC) {
			...
			i32 ipart = m_host->idxPart(m_idpartHost = action->partid);
			if (ipart>=0)
		...
		if (m_cloth) {
			...
			if (action->nPoints<0) {
				for(i32 i=m_cloth->getNbParticles()-1; i>=0; i--)
			...
			} else if (action->nPoints>0) for(i32 i=0;i<action->nPoints;i++)
	...
	if (_action->type==pe_action_reset::type_id && m_cloth) {
		...
		if (!vtxBuf || !vtxBuf->particles)
			return 0;
		mesh_data *md = (mesh_data*)m_parts[0].geom->pGeom->GetData();
		for(i32 i=min(md->nVertices,m_cloth->getNbParticles())-1; i>=0; i--)
...
i32 PhysXCloth::GetStatus(pe_status* _status) const
{
