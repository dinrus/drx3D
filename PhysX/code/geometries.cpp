...
#include <drx3D/PhysX/../../DinrusXPhys/geoman.h>
...
void CGeomUpr::InitGeoman()	{	m_nCracks = 0; }
void CGeomUpr::ShutDownGeoman() {}
...
{
	i32 nPolies = pMesh->getNbPolygons(), nTris=0;
	for(i32 i=0;i<nPolies; i++) {
		PxHullPolygon pg;	pMesh->getPolygonData(i,pg);
		nTris += pg.mNbVerts-2;
	}
	pdata->pIndices = new index_t[nTris*3];
	pdata->flags |= mesh_shared_idx;
	u8k *pIdx = pMesh->getIndexBuffer();
	for(i32 i=nTris=0; i<nPolies; i++) {
		PxHullPolygon pg;	pMesh->getPolygonData(i,pg);
		for(i32 j=0; j<pg.mNbVerts-2; j++,nTris++)
			((Vec3_tpl<index_t>*)pdata->pIndices)[nTris].Set(pIdx[pg.mIndexBase],pIdx[pg.mIndexBase+j+1],pIdx[pg.mIndexBase+(j+2 & j+2-(i32)pg.mNbVerts>>31)]);
	}
	pdata->nTris = nTris;
}
...
IGeometry* CGeomUpr::CreateMesh(strided_pointer<const Vec3> pVertices, strided_pointer<unsigned short> pIndices, char *pIds, i32 *pForeignIdx, i32 nTris,
	i32 flags, float approx_tolerance, SMeshBVParams *pParams)
{
	i32 nVtx;
	for(i32 i=nVtx=0; i<nTris*3; i++) nVtx = max(nVtx,pIndices[i]+1);
	...
	if (approx_tolerance > 0.9f) {
		...
		for(i32 i=0;i<nTris;i++) {
			...
			for(i32 j=0;j<3;j++) C += dotproduct_matrix(vtx(j),vtx(j),Ctmp)*area;
		}
		#undef vtx
		...
		Vec3 eigen;
		for(i32 i=0;i<3;i++) eigen[i] = fabs(boxEigen.Basis.GetRow(i)*(C*boxEigen.Basis.GetRow(i)));
		Vec3 BBox[] = { Vec3(ZERO),Vec3(ZERO) };
		for(i32 i=0;i<nTris*3;i++) {
		...
		if (flags & (mesh_approx_cylinder | mesh_approx_capsule)) {
			cylinder cyl;
			i32 i = idxmax3(min((eigen-eigen.GetPermutated(0)).abs(), (eigen-eigen.GetPermutated(1)).abs()));
			cyl.center = center;
	...
	if (flags & mesh_multicontact0 && nTris<30 || nTris<=12) {
		...
		g_drxPhysX.Cooking()->cookConvexMesh(cmd, buf, &result);
		if (pGeom->m_geom.mesh.pMeshConvex = g_drxPhysX.Physics()->createConvexMesh(PxDefaultMemoryInputData(buf.getData(), buf.getSize())))
		...
		g_drxPhysX.Cooking()->setParams(cookParams);
		if (pGeom->m_geom.mesh.pMesh = g_drxPhysX.Cooking()->createTriangleMesh(tmd, g_drxPhysX.Physics()->getPhysicsInsertionCallback()))
...
i32 PhysXGeom::Intersect(IGeometry* pCollider, geom_world_data* pdata1, geom_world_data* pdata2, intersection_params* pparams, geom_contact*& pcontacts)
{ 
	QuatT pose[2] = { QuatT(IDENTITY), QuatT(IDENTITY) };
	Diag33 scale[2] = { Diag33(1), Diag33(1) };
	for(i32 i=0;i<2;i++) if (geom_world_data *pdata = ((geom_world_data*[2]){pdata1,pdata2})[i]) {
...
void CGeomUpr::DestroyGeometry(IGeometry *pGeom) {}

//////////////////////////////////////////////////////////////////////////

void CGeomUpr::SaveGeometry(CMemStream &stm, IGeometry *pGeom) {}
IGeometry *CGeomUpr::LoadGeometry(CMemStream &stm, strided_pointer<const Vec3> pVertices, strided_pointer<unsigned short> pIndices, char *pIds) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }
IGeometry *CGeomUpr::CloneGeometry(IGeometry *pGeom) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }

ITetrLattice *CGeomUpr::CreateTetrLattice(const Vec3 *pt, i32 npt, i32k *pTets, i32 nTets) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }
i32 CGeomUpr::RegisterCrack(IGeometry *pGeom, Vec3 *pVtx, i32 idmat) { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; }
void CGeomUpr::UnregisterCrack(i32 i) {}
IGeometry *CGeomUpr::GetCrackGeom(const Vec3 *pt, i32 idmat, geom_world_data *pgwd) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }

IBreakableGrid2d *CGeomUpr::GenerateBreakableGrid(Vec2 *ptsrc, i32 npt, const Vec2i &nCells, i32 bStaticBorder, i32 seed) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }

//////////////////////////////////////////////////////////////////////////

IGeometry *CGeomUpr::CreatePrimitive(i32 type, const primitives::primitive *pprim)
{
	PhysXGeom	*pGeom = new PhysXGeom;	
	switch (pGeom->m_type = type) {
		...
		case heightfield::type: {
			...
			float hmin=0,hmax=0;
			for(i32 ix=0;ix<hf.size.x;ix++) for(i32 iy=0;iy<hf.size.y;iy++) {
			...
			for(i32 ix=0;ix<hf.size.x;ix++) for(i32 iy=0;iy<hf.size.y;iy++) {
				...
				cell.height = (i32)(hf.getheight(ix,iy)*rscale);
			...
			pGeom->m_geom.hf.pHF = g_drxPhysX.Cooking()->createHeightField(hfd, g_drxPhysX.Physics()->getPhysicsInsertionCallback());
...
phys_geometry *CGeomUpr::RegisterGeometry(IGeometry *pGeom,i32 defSurfaceIdx, i32 *pMatMapping,i32 nMats)
{
	phys_geometry *pgeom = new phys_geometry;
	memset(pgeom, 0, sizeof(phys_geometry));
	if (pMatMapping)
		memcpy(pgeom->pMatMapping = new i32[nMats], pMatMapping, nMats*sizeof(i32));
	pgeom->nMats = nMats;
	pgeom->nRefCount = 1;
	pgeom->q.SetIdentity();
...
i32 CGeomUpr::AddRefGeometry(phys_geometry *pgeom) { return DrxInterlockedIncrement(&pgeom->nRefCount); }

i32 CGeomUpr::UnregisterGeometry(phys_geometry *pgeom)
{
	i32 res = DrxInterlockedDecrement(&pgeom->nRefCount);
	if (res <= 0)	{
...
template<typename Tprim> PhysXGeom* LoadPrim(CMemStream &stm, CGeomUpr *geoman) 
{
	Tprim prim; stm.Read(prim);
	return (PhysXGeom*)geoman->CreatePrimitive(Tprim::type, &prim);
}

phys_geometry *CGeomUpr::LoadPhysGeometry(CMemStream &stm, strided_pointer<const Vec3> pVertices, strided_pointer<unsigned short> pIndices, char *pIds)
{
	i32 ver; stm.Read(ver);
	...
	switch (type) {
		...
		case GEOM_TRIMESH: {
			...
			if (bForeignIdx && !(flags & mesh_full_serialization)) {
				pIdx = new u16[nTris*3];
				for(i32 i=0; i<nTris; i++) {
					i32 idx = stm.Read<u16>();
					for(i32 j=0;j<3;j++) pIdx[i*3+j] = pIndices[idx*3+j];
				}
			}	else {
				stm.m_iPos += sizeof(u16)*nTris*bForeignIdx;
				if (!(flags & mesh_shared_vtx)) {
					pVtx = strided_pointer<Vec3>((Vec3*)(stm.m_pBuf+stm.m_iPos));
					stm.m_iPos += nvtx*sizeof(Vec3);
				}
				pIdx = (u16*)(stm.m_pBuf+stm.m_iPos);
			}
...
void CGeomUpr::SavePhysGeometry(CMemStream &stm, phys_geometry *pgeom) 
{
	...
	if (itype != GEOM_TRIMESH) {
		...
		for(i32 i=0; i<md.nVertices; stm.Write(md.pVertices[i++]));
		for(i32 i=0; i<md.nTris*3; stm.Write((u16)md.pIndices[i++]));
	}
}

void CGeomUpr::SetGeomMatMapping(phys_geometry *pgeom, i32 *pMatMapping, i32 nMats)
{
