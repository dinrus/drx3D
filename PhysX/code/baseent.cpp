...
i32 phys_geometry_refcnt::Release() 
{ 
	i32 res = DrxInterlockedDecrement(( i32*)&nRefCount); 
	if (res<=0) 
		delete this; 
	return res; 
}

PhysXEnt::PhysXEnt(pe_type type, i32 id, pe_params *params) : m_type(type), m_id(id)
{
	...
	if (type==PE_STATIC || type==PE_AREA || type==PE_SOFT)	{
		m_actor = g_drxPhysX.Physics()->createRigidStatic(T(trans));
		m_iSimClass = (type==PE_AREA)*5;
	} else {
		PxRigidDynamic *pRD = g_drxPhysX.Physics()->createRigidDynamic(T(trans));
...
i32 PhysXEnt::AddGeometry(phys_geometry* pgeom, pe_geomparams* params, i32 id, i32 bThreadSafe)
{
	...
	part newPart;
	if (id<0)
		for(i32 i=id=0;i<m_parts.size();i++) id = max(id,PartId(m_parts[i].shape)+1);
	newPart.geom = (phys_geometry_refcnt*)pgeom;
	PhysXGeom *pGeom = (PhysXGeom*)pgeom->pGeom;
	i32 *pMatMapping=pgeom->pMatMapping, nMats=pgeom->nMats;
	...
	newPart.shape->userData = (uk )(INT_PTR)id;
...
void PhysXEnt::RemoveGeometry(i32 id, i32 bThreadSafe)
{
	if (!m_actor)
		return;
	WriteLockScene lock;
...
void PhysXEnt::MarkForAdd()
{
	if (!g_pPhysWorld->m_dt) {
		...
		_InterlockedOr(( long*)&m_mask, 1u<<31);
	}
}
...
PhysXEnt* PhysXEnt::AddToScene(bool fromList)
{
	...
	Enable(!(m_mask & 1<<5));
	_InterlockedAnd(( long*)&m_mask, ~(1u<<31 | 1<<5));
	return next;
}

void PhysXEnt::DrawHelperInformation(IPhysRenderer *pRenderer, i32 iDrawHelpers)
{
	pe_params_bbox bb;
...
{ 
	if (!enable) {
		_InterlockedAnd(( long*)&m_mask, ~4);
		g_drxPhysX.Scene()->removeActor(*m_actor);
	} else {
		_InterlockedOr(( long*)&m_mask, 4);
		g_drxPhysX.Scene()->addActor(*m_actor);
		if (m_iSimClass<=3) 
...
QuatT PhysXEnt::getLocalPose(i32 idxPart) const 
{ 
...
void PhysXEnt::setLocalPose(i32 idxPart, const QuatT& pose) 
{ 
	QuatT posePx = pose;
...
void PhysXEnt::SetSimClass(i32 iSimClass)
{
...
void PhysXEnt::PostStep(float dt, i32 immediate)
{
...
i32 PhysXEnt::GetParams(pe_params* _params) const 
{ 
	if (_params->type==pe_params_foreign_data::type_id) {
		pe_params_foreign_data *params = (pe_params_foreign_data*)_params;
...
i32 PhysXEnt::GetStatus(pe_status* _status) const
{
...
template<class Taction> void ExtractConstrFrames(PhysXEnt *ent0,PhysXEnt *ent1, i32 ipart0,i32 ipart1, Taction *action, QuatT* frame)
{
	for(i32 i=0; i<2; i++)
...
i32 PhysXEnt::Action(pe_action* _action, i32 bThreadSafe)
{
	...
	if (_action->type==pe_action_impulse::type_id) {
		...
		i32 i = is_unused(action->ipart) ? 0 : action->ipart;
	...
	if (_action->type==pe_action_add_constraint::type_id) {
		...
		for(i32 i=0;i<2;i++) if (body[i])
			frame[i] = T(body[i]->getGlobalPose()).GetInverted()*frame[i];
		PxD6Joint *joint = PxD6JointCreate(*g_drxPhysX.Physics(), body[0], T(frame[0]), body[1], T(frame[1]));
		if (!joint)
			return 0;
		...
		if (is_unused(action->id))
			std::find_if(m_constraints.begin(),m_constraints.end(), [&](auto p)->bool { return (i32)(INT_PTR)p->userData!=++id; });
		else {
			id = action->id;
		...
		joint->userData = (uk )(INT_PTR)max(id,1);
...
i32 PhysXEnt::GetStateSnapshot(TSerialize ser, float time_back, i32 flags) { return 0; }
i32 PhysXEnt::SetStateFromSnapshot(TSerialize ser, i32 flags) { return 0; }
i32 PhysXEnt::SetStateFromTypedSnapshot(TSerialize ser, i32 type, i32 flags) { return 0; }
