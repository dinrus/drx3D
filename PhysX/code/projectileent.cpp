...
PhysXProjectile::PhysXProjectile(i32 id, pe_params *params)
{
	m_type = PE_PARTICLE;
	m_id = id;
	if (params) 
...
i32 PhysXProjectile::SetParams(pe_params* _params, i32 bThreadSafe)
{
...
i32 PhysXProjectile::GetParams(pe_params* _params) const
{
	if (_params->type==pe_params_particle::type_id) {
		pe_params_particle *params = (pe_params_particle*)_params;
...
i32 PhysXProjectile::GetStatus(pe_status* _status) const
{
	if (!m_actor) {
...
i32 PhysXProjectile::Action(pe_action* _action, i32 bThreadSafe)
{
...
i32 PhysXProjectile::DoStep(float dt, i32 iCaller) 
{
	...
	i32 solid = 0;
	if (i32 nhits = g_pPhysWorld->RayWorldIntersection(rp)) {
		solid = hits[0].dist>=0;
		if (!iCaller) for(i32 j=0; j<nhits; j++) {
			i32 i = j+1 & ~(nhits-1-solid-j>>31);	// report solid hit (slot 0) the last
	...
	if (solid)	{
		...
		if (m_flags & particle_single_contact || noBounce && !m_gravity.len2() || iCaller) {
			...
			PxRigidDynamic *pRD = g_drxPhysX.Physics()->createRigidDynamic(T(QuatT(Quat(IDENTITY),m_pos)));
