...
PhysXArticulation::PhysXArticulation(i32 id, pe_params* params)
{
	m_type = PE_ARTICULATED;
	m_id = id;
	m_iSimClass = 4;
	m_actor = g_drxPhysX.Physics()->createRigidDynamic(T(QuatT(IDENTITY)));
	m_actor->userData = this;
	m_artic = g_drxPhysX.Physics()->createArticulation();
	m_artic->userData = this;
	if (params) 
		SetParams(params); 
}

PxArticulationLink *PhysXArticulation::findJoint(i32 id) const
{
	i32 n = m_artic->getNbLinks();
	PxArticulationLink **links = (PxArticulationLink**)alloca(n*sizeof(uk ));
	m_artic->getLinks(links,n);
	for(i32 i=0;i<n;i++) if (JointId(links[i])==id)
		return links[i];
	return nullptr;
}
...
void PhysXArticulation::setGlobalPose(const QuatT& pose, i32 components)
{
	if (IsRagdoll()) {
...
PxRigidBody *PhysXArticulation::getRigidBody(i32 ipart) const
{
...
QuatT PhysXArticulation::getLocalPose(const PxRigidBody* link, i32 idxPart) const 
{ 
	PxShape *shapeLink = m_parts[idxPart].shape;
	for(i32 i=link->getNbShapes()-1; i>=0 && link->getShapes(&shapeLink,1,i) && PartId(shapeLink)!=PartId(m_parts[idxPart].shape); i--);
...
QuatT PhysXArticulation::getLocalPose(i32 idxPart) const 
{ 
	if (IsRagdoll())
...
void PhysXArticulation::Enable(bool enable)
{
	...
	g_drxPhysX.Scene()->removeArticulation(*m_artic);
	if (enable)	{
		_InterlockedOr(( long*)&m_mask, 4);
		if (IsRagdoll()) {
			g_drxPhysX.Scene()->addArticulation(*m_artic);
			Awake(m_iSimClass==2);
		} else
...
void PhysXArticulation::Awake(bool awake, float minTime)
{
	if (IsRagdoll() && m_mask & 4) {
		...
		if (awake && m_list[0]==this)	{
			_InterlockedOr(( long*)&m_mask, awake ? 1:2);
...
void PhysXArticulation::SetSimClass(i32 iSimClass)
{
...
i32 PhysXArticulation::AddGeometry(phys_geometry* pgeom, pe_geomparams* params, i32 id, i32 bThreadSafe)
{
	i32 idJoint = 0;
	...
	if (id>=0)
		m_parts[m_parts.size()-1].shape->userData = (uk )(INT_PTR)(idJoint<<16 | id);
	return id;
}

i32 PhysXArticulation::SetParams(pe_params* _params, i32 bThreadSafe)
{
	if (_params->type==pe_params_joint::type_id) {
		...
		if (!link) {
			i32 i0=-1,n=0; 
			for(i32 i=0;i<m_parts.size();i++) if (JointId(m_parts[i].shape)==params->op[1])
				n++, i0 += i-i0 & i0>>31;
			if (i0<0)
				return 0;
			...
			if (!(link = m_artic->createLink(linkParent=findJoint(params->op[0]), T(poseEnt*pose))))
				return 0;
			link->setName((tukk)linkParent);
			link->userData = this;
			float *densities = (float*)alloca(n*sizeof(float));
			for(i32 i=n=0; i<m_parts.size(); i++) if (JointId(m_parts[i].shape)==params->op[1]) {
		...
		if (PxArticulationJoint *joint = link->getInboundJoint()) {
			...
			i32 nlimits = 0;
			for(i32 i=0;i<3;i++) for(i32 j=0;j<2;j++) if (!is_unused(params->limits[j][i]) && !(params->flags & angle0_locked<<i))
		...
		if (!is_unused(params->pHost) || !is_unused(params->posHostPivot) || !is_unused(params->qHostPivot)) {
			if (!is_unused(params->pHost) && !params->pHost) {
				...
				if (!m_hostAttach) {
					...
					m_hostAttach = PxFixedJointCreate(*g_drxPhysX.Physics(), host,T(pose), m_actor,T(QuatT(IDENTITY)));
...
i32 PhysXArticulation::GetStatus(pe_status* _status) const
{
	if (_status->type==pe_status_joint::type_id) {
		pe_status_joint *status = (pe_status_joint*)_status;
		i32 idJoint = status->idChildBody, ipart;
	...
	i32 res = PhysXEnt::GetStatus(_status);
...
i32 PhysXArticulation::Action(pe_action* _action, i32 bThreadSafe)
{
	if (_action->type==pe_action_batch_parts_update::type_id) {
		...
		if (action->pValidator && !action->pValidator->Lock())
			return 0;
		ReapplyParts:
		for(i32 i=0,j=0,nParts; i<action->numParts; i++) {
