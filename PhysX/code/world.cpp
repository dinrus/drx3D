...
IPhysicalEntity* PhysXWorld::SetupEntityGrid(i32 axisz, Vec3 org, i32 nx, i32 ny, float stepx, float stepy, i32 log2PODscale, i32 bCyclic, IPhysicalEntity* pHost, const QuatT& posInHost) 
{
	PxBroadPhaseRegion rgn;
	rgn.bounds = PxBounds3(V(org-Vec3(0,0,100)), V(org+Vec3(nx*stepx,ny*stepy,200)));
	g_drxPhysX.Scene()->removeBroadPhaseRegion(m_idWorldRgn);
	m_idWorldRgn = g_drxPhysX.Scene()->addBroadPhaseRegion(rgn);
	return nullptr;
}

IPhysicalEntity *PhysXWorld::SetHeightfieldData(const heightfield *phf, i32 *pMatMapping, i32 nMats)
{
...
i32 PhysXWorld::GetPhysicalEntityId(IPhysicalEntity* pent) { return pent ? ((PhysXEnt*)pent)->m_id : -2; }
IPhysicalEntity* PhysXWorld::GetPhysicalEntityById(i32 id) 
{
	return id==-1 ? m_phf : (uint)id<m_entsById.size() && !((INT_PTR)m_entsById[id]&1) ? m_entsById[id] : nullptr;
}

i32 PhysXWorld::GetFreeEntId()
{
	if (m_idFree<0)
		return m_entsById.size();
	else {
		i32 id = m_idFree;
		m_idFree = (i32)(INT_PTR)m_entsById[id]>>1;
		m_entsById[id] = nullptr;
		return id;
	}
}

IPhysicalEntity* PhysXWorld::CreatePhysicalEntity(pe_type type, float lifeTime, pe_params* params, uk pForeignData, i32 iForeignData, i32 id, IPhysicalEntity*,IGeneralMemoryHeap*)
{
	i32 flagsOR = 0;
	if (id<0)	{
		id = GetFreeEntId();
		...
		if ((INT_PTR)m_entsById[id] & 1) {
			if (id==m_idFree)
				m_idFree = (i32)(INT_PTR)m_entsById[m_idFree]>>1;
			else {
				i32 id0=id-1;
				if (id0<0 || !((INT_PTR)m_entsById[id0] & 1) || (i32)(INT_PTR)m_entsById[id0]>>1!=id)
					for(id0=m_idFree; (i32)(INT_PTR)m_entsById[id0]>>1!=id; id0=(i32)(INT_PTR)m_entsById[id0]>>1);
				m_entsById[id0] = m_entsById[id];
			}
		} else if (m_entsById[id]) {
			i32 id1 = GetFreeEntId();
	...
	if (id >= m_entsById.size()) {
		...
		i32 size = m_entsById.size();
		m_entsById.resize(id+1, 0);
		if (size<id-1) {
			for(i32 i=size; i<id-1; i++) m_entsById[i]=(PhysXEnt*)(INT_PTR)(i+1<<1|1);
...
i32 PhysXWorld::DestroyPhysicalEntity(IPhysicalEntity* _pent, i32 mode, i32 bThreadSafe)
{
	...
	if (mode)	{
		_InterlockedAnd(( long*)&pent->m_mask, ~(1<<5));
		_InterlockedOr(( long*)&pent->m_mask, (mode&1)<<5);
		pent->MarkForAdd();
		return 1;
	}
...
i32 PhysXWorld::SetSurfaceParameters(i32 surface_idx, float bounciness, float friction, u32 flags) 
{
	if ((uint)surface_idx>=(uint)NSURFACETYPES)
		return 0;
	if (!m_mats[surface_idx])
		m_mats[surface_idx] = g_drxPhysX.Physics()->createMaterial(0.5f,0.3f,0.0f);
	...
	m_mats[surface_idx]->userData = (uk )(INT_PTR)(surface_idx | (flags&15)<<16);
	return 1;
}

i32 PhysXWorld::GetSurfaceParameters(i32 surface_idx, float& bounciness, float& friction, u32& flags) 
{
...
i32 CopyHitData(const PxRaycastHit &src, ray_hit &dst, bool valid=true, const Vec3& rayOrg=Vec3(0), const Vec3& rayDir=Vec3(0))
{
	if (!valid) {
		dst.dist = -1;
		return 0;
...
struct RaycastFilter : PxQueryFilterCallback {
	RaycastFilter(i32 pierceability=15, PhysXEnt** pSkipEnts=nullptr, i32 nSkipEnts=0) : pierceability(pierceability),pSkipEnts(pSkipEnts), nSkipEnts(nSkipEnts) {}
	virtual PxQueryHitType::Enum preFilter(const PxFilterData& filterData, const PxShape* shape, const PxRigidActor* actor, PxHitFlags& queryFlags) {
		...
		if (nSkipEnts && pSkipEnts[0] && pSkipEnts[0]->m_pForeignData==Ent(actor)->m_pForeignData)
			return PxQueryHitType::eNONE;
		for(i32 i=1;i<nSkipEnts;i++) if (pSkipEnts[i]==Ent(actor))
			return PxQueryHitType::eNONE;
		PxMaterial *pMat;
		shape->getMaterials(&pMat,1);
		return pierceability < (i32)(INT_PTR)pMat->userData>>16 ? PxQueryHitType::eTOUCH : PxQueryHitType::eBLOCK;
	}
	virtual PxQueryHitType::Enum postFilter(const PxFilterData& filterData, const PxQueryHit& hit) { return PxQueryHitType::eBLOCK; }
	i32 pierceability;
	i32 flagsAny=geom_colltype_ray, flagsAll=0;
	PhysXEnt **pSkipEnts;
	i32 nSkipEnts;
	i32 skipId = -10;
};

inline PxQueryFlags objtypesFilter(i32 objtypes) {
	return 
...
PxQueryHitType::Enum RaycastBatchFilter(PxFilterData queryFD, PxFilterData shapeFD, ukk constantBlock, PxU32 constantBlockSize, PxHitFlags& hitFlags)
{
...
i32 PhysXWorld::RayWorldIntersection(const SRWIParams& rp, tukk pNameTag, i32 iCaller)
{
	...
	if (rp.flags & rwi_queue) {
		...
		i32 *skipIds = (i32*)&fd.data.word1, nSkipIds=0;
		if ((rp.objtypes & (ent_static|ent_terrain))==ent_terrain)
			skipIds[nSkipIds++] = -1;
		for(i32 i=0; i<rp.nSkipEnts && nSkipIds<3; i++)
	...
	if (i32 collTypes = rp.flags >> rwi_colltype_bit)
	...
	if (!(rp.objtypes & ent_terrain))
		hitFilter.skipId = -1;
	ReadLockScene lock;
	g_drxPhysX.Scene()->raycast(V(rp.org), V(dirNorm), rp.dir*dirNorm, hitCB, hitFlags, fd, &hitFilter);
	i32 nhits = CopyHitData(hitCB.block, rp.hits[0], hitCB.hasBlock, rp.org,rp.dir);
	for(i32 i=0; i<hitCB.nbTouches; i++)
		nhits += CopyHitData(hitCB.touches[i], rp.hits[hitCB.hasBlock+i], true, rp.org,rp.dir);
	return nhits;
}

i32 PhysXWorld::TracePendingRays(i32 bDoActualTracing) 
{
	{ WriteLock lock(m_lockRWIqueue),lock1(m_lockRWIres);
	...
	{ WriteLock lock(m_lockRWIres);
		for(i32 i=0; i<m_rwiResPx.size(); i++) {
...
i32 PhysXWorld::RayTraceEntity(IPhysicalEntity* pient, Vec3 org, Vec3 dir, ray_hit* pHit, pe_params_pos* pp, u32 geomFlagsAny) 
{
	QuatT qres(IDENTITY);
	if (pp) {
	...
	pHit->dist = 1e10f;
	for(i32 i=0; i<pent->m_parts.size(); i++) {
		cache.actor = (cache.shape = pent->m_parts[i].shape)->getActor();
		if (g_drxPhysX.Scene()->raycast(V(org), V(dirNorm), dir*dirNorm, hitCB, PxHitFlags(PxHitFlag::eDEFAULT), PxQueryFilterData(PxQueryFlag::eANY_HIT), 0, &cache) && 
...
float PhysXWorld::PrimitiveWorldIntersection(const SPWIParams& pp, WriteLockCond* pLockContacts, tukk pNameTag)
{
	static geom_contact g_contacts[16];
	static  i32 g_lockContacts = 0;
	QuatT pose(IDENTITY);
	Diag33 scale(1);
	PhysXGeom geom;
	...
	if (pp.sweepDir.len2()) {
		...
		if (geom.CreateAndUse(pose,scale, [&](PxGeometry &pxGeom)	{
			if (pp.nSkipEnts>=0)
				return g_drxPhysX.Scene()->sweep(pxGeom, T(pose), V(dirNorm), dirNorm*pp.sweepDir, res, flags, fd, &filter); 
			else {
				PxQueryCache cache;
				SweepCallback res1;
				res.block.distance = 1e10f;
				fd.flags = PxQueryFlags(0);
				for(i32 i=0; i<-pp.nSkipEnts; i++) {
					PhysXEnt *pent = (PhysXEnt*)pp.pSkipEnts[i];
					cache.actor = pent->m_actor;
					for(i32 j=0; j<pent->m_parts.size(); j++) {
						cache.shape = pent->m_parts[j].shape;
						if (g_drxPhysX.Scene()->sweep(pxGeom, T(pose), V(dirNorm), dirNorm*pp.sweepDir, res1, flags, fd, 0, &cache) && res1.block.distance < res.block.distance)
		...
		if (geom.CreateAndUse(pose,scale, [&](PxGeometry &pxGeom) {
			return g_drxPhysX.Scene()->overlap(pxGeom, T(pose), res, fd, &filter); }))
		{
			for(i32 i=0; i<res.nbTouches; i++)
...
i32 PhysXWorld::CollideEntityWithPrimitive(IPhysicalEntity* _pent, i32 itype, primitives::primitive* pprim, Vec3 dir, ray_hit* phit, intersection_params* pip)
{
	SPWIParams pp;
	geom_contact *cont;
...
struct BBoxFilter : public PxQueryFilterCallback {
	BBoxFilter(i32 ithread, const Vec3& vmin, const Vec3& vmax) : ithread(ithread), bbox(V(vmin),V(vmax)) {}
	virtual PxQueryHitType::Enum preFilter(const PxFilterData& filterData, const PxShape* shape, const PxRigidActor* actor, PxHitFlags& queryFlags) {
		...
		if (!(pent->m_mask & 1<<ithread)) {
			...
			if (nEnts < 64)
				...
				if (nEnts==64)
					memcpy(&PhysXEnt::g_pPhysWorld->m_entList[0], entBuf, 64*sizeof(uk ));
...
i32 PhysXWorld::GetEntitiesInBox(Vec3 ptmin, Vec3 ptmax, IPhysicalEntity**& pList, i32 objtypes, i32 szListPrealloc) 
{
	i32 ithread;
	do {
	...
	{ ReadLockScene lock;
		g_drxPhysX.Scene()->overlap(PxBoxGeometry(V(ptmax-ptmin)*0.5f), PxTransform(V(ptmax+ptmin)*0.5f,PxQuat0), BBoxCallback(), 
	...
	if (objtypes & ent_sort_by_mass)
		std::qsort(pListSrc, filter.nEnts, sizeof(uk ), [](ukk pent0,ukk pent1)->i32 { 
	...
	for(i32 i=0; i<filter.nEnts; i++) 
		AtomicAdd(&pListSrc[i]->m_mask, -(1<<filter.ithread));
	i32 n = 0;
	for(i32 i=0; i<filter.nEnts; i++)	if (!(pListSrc[i]->m_mask & (8|16)))
		pList[n++] = pListSrc[i];
	if (objtypes & ent_addref_results)
		for(i32 i=0; i<n; pListSrc[i++]->AddRef()); 
...
void PhysXWorld::AddEventClient(i32 type, i32(*func)(const EventPhys*), i32 bLogged, float priority)
{
...
i32 PhysXWorld::RemoveEventClient(i32 type, i32(*func)(const EventPhys*), i32 bLogged) 
{ 
	i32 count = 0;
...
EventPhys* PhysXWorld::AddDeferredEvent(i32 type, EventPhys* event)
{
...
uk PhysXWorld::GetInternalImplementation(i32 type, uk object)
{
	switch (type)
	{
	case 0: return reinterpret_cast<uk>( cpx::g_drxPhysX.Scene() ); break;                 // return native physx scene
	case 1: return reinterpret_cast<uk>( static_cast<PhysXEnt*>(object)->m_actor ); break; // return native physx actor
	default: break;
	}
	return nullptr;
}
...
void PhysXWorld::onWake(PxActor** actors, PxU32 count)
{
	for(i32 i=0; i<count; i++) {
		...
		_InterlockedOr(( long*)&Ent((PxRigidActor*)actors[i])->m_mask, 1);
	}
}
...
{
	for(i32 i=0; i<count; i++)
		_InterlockedOr(( long*)&Ent((PxRigidActor*)actors[i])->m_mask, 2);
}
...
{
	i32 active = pent->m_vel.len2()>0;
	Insert(pent, m_projectilesList[active], active^1);
}


void PhysXWorld::TimeStep(float dt, i32 flags)
{
	//if (!m_dt && dt)
	//	g_cpx.Scene()->forceDynamicTreeRebuild(true,true);
	m_time += dt;
	if (m_vars.fixedTimestep>0 && dt>0)	{
		g_drxPhysX.dt() = dt = m_vars.fixedTimestep;
		m_dtSurplus = 0;
	}
	m_vars.lastTimeStep = m_dt = dt;
	g_drxPhysX.Scene()->setBounceThresholdVelocity(m_vars.minBounceSpeed);
	...
	if (flags & ent_rigid && (!m_vars.bSingleStepMode || m_vars.bDoStep)) {
		float dtFixed = g_drxPhysX.dt();
		for(i32 i=0; m_dtSurplus+dt>=dtFixed && i<m_vars.nMaxSubsteps; dt-=dtFixed,i++)	{
			g_drxPhysX.Scene()->simulate(dtFixed,0,m_scratchBuf.data(),m_scratchBuf.size());
			WriteLockScene lockScene;
			{ WriteLock lock(m_lockCollEvents); 
				g_drxPhysX.Scene()->fetchResults(true);
			}
			g_drxPhysX.Scene()->flushQueryUpdates();
			AtomicAdd(( uint*)&m_updated, 1);
	...
	if (flags & ent_deleted) { 
		...
		for(PhysXEnt *pent=m_delEntList[1],*pentNext; pent!=ListStart(m_delEntList); pent=pentNext) {
			...
			if (!(pent->m_mask & 16))	{
				pent->Enable(false);
				_InterlockedOr(( long*)&pent->m_mask, 16);
			}
	...
	if (PxPvdSceneClient *dbg = g_drxPhysX.Scene()->getScenePvdClient()) {
		const CCamera &cam = gEnv->pSystem->GetViewCamera();
		dbg->updateCamera("DrxCamera", V(cam.GetPosition()), V(cam.GetUp()), V(cam.GetPosition()+cam.GetViewdir()));
...
EventPhys *PhysXWorld::AllocEvent(i32 id)
{
...
void PhysXWorld::PumpLoggedEvents()
{
	if (m_updated) { 
		...
		{ WriteLock lock(g_lockEntList);
			...
			if (n) pents = (PhysXEnt**)alloca(n*sizeof(uk ));
		...
		for(i32 i=0; i<n; i++) {
			...
			for(i32 j=0;j<2;j++) if (pents[i]->m_mask & 1<<j && pents[i]->m_flags & pef_log_state_changes) {
			...
			_InterlockedAnd(( long*)&pents[i]->m_mask,~3);
		}
		...
		{ ReadLock lock(g_lockEntList);
			...
			if (n1>n) pents = (PhysXEnt**)alloca(n1*sizeof(uk ));
	...
	for(i32 i=0;i<nEvents;i++) if (m_collEvents[1][i].pForeignData[0] && ((PhysXEnt*)m_collEvents[1][i].pEntity[0])->m_pForeignData && m_collEvents[1][i].pForeignData[1])
		SendEvent(m_collEvents[1][i],1);

	{ WriteLock lock(m_lockRWIres);
		for(i32 i=0; i<m_rwiRes[2].size(); i++)
...
PhysXWorld::PhysXWorld(ILog* pLog) : m_debugDraw(false)
{
	...
	g_drxPhysX.Scene()->setSimulationEventCallback(this);
	m_pentStatic = new PhysXEnt(PE_STATIC,-2);
	PxBatchQueryDesc bqd(1000,0,0);
	bqd.preFilterShader = RaycastBatchFilter;
	m_batchQuery[0] = g_drxPhysX.Scene()->createBatchQuery(bqd);
	m_batchQuery[1] = g_drxPhysX.Scene()->createBatchQuery(bqd);
...
i32 PhysXWorld::CheckAreas(const Vec3& ptc, Vec3& gravity, pe_params_buoyancy* pb, i32 nMaxBuoys, i32 iMedium, const Vec3& vec, IPhysicalEntity* pent, i32 iCaller) 
{
	pb[0] = m_pbGlob;
...
IPhysicalEntity *PhysXWorld::CreatePhysicalPlaceholder(pe_type type, pe_params* params, uk pForeignData, i32 iForeignData, i32 id)
{
	return m_pentStatic;
}

void PhysXWorld::SimulateExplosion(pe_explosion* pexpl, IPhysicalEntity** pSkipEnts, i32 nSkipEnts, i32 iTypes, i32 iCaller) 
{
	pexpl->nAffectedEnts = 0;
}

void PhysXWorld::DrawPhysicsHelperInformation(IPhysRenderer *pRenderer, i32 iCaller)
{
	...
	if (!m_vars.iDrawHelpers) { m_debugDraw = false; g_drxPhysX.SetDebugVisualizationForAllSceneElements(false); return; }
	if (m_vars.iDrawHelpers && (m_debugDraw == false)) { m_debugDraw = true; g_drxPhysX.SetDebugVisualizationForAllSceneElements(true); }

	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eSCALE, m_vars.iDrawHelpers ? 1.0f : 0.0f);

	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eCONTACT_POINT, m_vars.iDrawHelpers & (1 << 0) ? 1.0f : 0.0f);
	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eCONTACT_NORMAL, m_vars.iDrawHelpers & (1 << 0) ? 1.0f : 0.0f);
	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eCONTACT_FORCE, 0.0f);

	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eCOLLISION_SHAPES, m_vars.iDrawHelpers & (1 << 1) ? 1.0f : 0.0f);
	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eBODY_LIN_VELOCITY, 0.0f);
	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eBODY_ANG_VELOCITY, 0.0f);
	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eACTOR_AXES, 0.0f);
	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eCOLLISION_AABBS, m_vars.iDrawHelpers & (1 << 2) ? 2.0f : 0.0f);
	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eJOINT_LOCAL_FRAMES, 0.0f);
	g_drxPhysX.Scene()->setVisualizationParameter(PxVisualizationParameter::eJOINT_LIMITS, 0.0f);

	float dist = 50.0f;
	if (ICVar *CVdist = gEnv->pConsole->GetCVar("p_cull_distance"))
		dist = CVdist->GetFVal();
	Vec3 campos = gEnv->pSystem->GetViewCamera().GetPosition();
	physx::PxRenderBuffer const& rb = g_drxPhysX.Scene()->getRenderBuffer();
	...
	i32 nTriangles = rb.getNbTriangles();
	if (nTriangles > 0)
	{
...
void PhysXWorld::DrawEntityHelperInformation(IPhysRenderer *pRenderer, i32 entityId, i32 iDrawHelpers)
{
