...
uk GuardCalloc(size_t count, size_t size, size_t *objectStride)
{
	if (objectStride)
		*objectStride = size;
	return (uk )btAlignedAlloc(count * size, 16);
}
void GuardFree(uk buf)
{
	btAlignedFree(buf);
}
...
static i32 GuardMarkBuffer(uk buffer, i32 flag);
...
i32 GuardMarkReadOnly(uk buf)
{
	return GuardMarkBuffer(buf, PROT_READ);
}

i32 GuardMarkReadWrite(uk buf)
{
	return GuardMarkBuffer(buf, PROT_READ | PROT_WRITE);
}

i32 GuardMarkWriteOnly(uk buf)
{
	return GuardMarkBuffer(buf, PROT_WRITE);
}

static i32 GuardMarkBuffer(uk buf, i32 flag)
{
	if (mprotect((char *)buf - kPageSize, kPageSize, PROT_READ))
	{
