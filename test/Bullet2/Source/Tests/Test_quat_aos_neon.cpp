...
public:
	...
	//
	inline Quat_ref &setElem(i32 idx, float value);
...
inline Quat_ref &Quat_ref::setElem(i32 idx, float value)
{
	*(&mX + idx) = value;
	return *this;
}

inline float Quat_ref::getElem(i32 idx) const
{
	return *(&mX + idx);
}

inline float &Quat_ref::operator[](i32 idx)
{
	return *(&mX + idx);
}

inline float Quat_ref::operator[](i32 idx) const
{
	return *(&mX + idx);
}
