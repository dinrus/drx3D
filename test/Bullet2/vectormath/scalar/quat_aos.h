...
inline Quat &Quat::setElem(i32 idx, float value)
{
	*(&mX + idx) = value;
	return *this;
}

inline float Quat::getElem(i32 idx) const
{
	return *(&mX + idx);
}

inline float &Quat::operator[](i32 idx)
{
	return *(&mX + idx);
}

inline float Quat::operator[](i32 idx) const
{
	return *(&mX + idx);
}
