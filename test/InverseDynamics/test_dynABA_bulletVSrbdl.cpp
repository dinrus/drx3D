...
i32 compareABAForwardDynamicsBulletAndRbdl(btAlignedObjectArray<btScalar> &q, btAlignedObjectArray<btScalar> &u, btAlignedObjectArray<btScalar> &joint_forces, btAlignedObjectArray<btScalar> &dot_u, btVector3 &gravity, bool verbose,
...
#define RETURN_ON_FAILURE(x)                         \
	...
	for (i32 l = 0; l < btmb->getNumLinks(); l++)
	{
	...
	for (i32 link = 0; link < btmb->getNumLinks(); link++)
	{
	...
	*acc_error = 0;
	...
	{
		for (i32 i = 0; i < btmb->getNumDofs(); i++)
		{
			if (verbose)
			{
...
TEST(DynCompareBulletAndRbdl, UrdfPantilt)
{
	...
	for (i32 loop = 0; loop < kNLoops; loop++)
	{
