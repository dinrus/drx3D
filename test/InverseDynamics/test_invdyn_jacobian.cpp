...
void calculateDotJacUError(const MultiBodyTreeCreator& creator, i32k nloops,
						   double* max_error)
{
	...
	i32k ndofs = tree1->numDoFs();
	i32k nbodies = tree1->numBodies();
	if (ndofs <= 0)
	{
		*max_error = 0;
		return;
	...
	for (i32 loop = 0; loop < nloops; loop++)
	{
...
void calculateJacobianError(const MultiBodyTreeCreator& creator, i32k nloops,
							double* max_error)
{
...
void calculateVelocityJacobianError(const MultiBodyTreeCreator& creator, i32k nloops,
									double* max_error)
{
...
TEST(InvDynJacobians, JacDotJacU)
{
...
#endif
	i32k kNumLoops = 20;
	for (i32 level = 0; level < kNumLevels; level++)
	{
		i32k nbodies = BT_ID_POW(2, level);
	...
	i32k kRandomLoops = 100;
	i32k kMaxRandomBodies = 128;
	for (i32 loop = 0; loop < kRandomLoops; loop++)
	{
...
TEST(InvDynJacobians, Jacobians)
{
	...
	i32k kRandomLoops = 20;
	i32k kMaxRandomBodies = 16;
	for (i32 loop = 0; loop < kRandomLoops; loop++)
	{
		RandomTreeCreator random(kMaxRandomBodies);
		double error;
