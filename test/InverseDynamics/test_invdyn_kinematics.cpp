...
i32k kLevel = 5;
i32k kNumBodies = BT_ID_POW(2, kLevel);
...
public:
	...
	void init(STxt name, idScalar dt)
	{
		m_name = name;
		m_dt = dt;
		m_num_updates = 0;
...
{
public:
	VecDiffFD(STxt name, i32 dim, idScalar dt) : m_name(name), m_fd(dim), m_dt(dt)
	{
		for (i32 i = 0; i < m_fd.size(); i++)
		{
			char buf[256];
	...
	void update(i32 i, ValueType& val, DiffType& true_diff) { m_fd[i].update(val, true_diff); }
...
i32 calculateDifferentiationError(const MultiBodyTreeCreator& creator, idScalar deltaT,
