...
i32 m_glutScreenWidth;
i32 m_glutScreenHeight;
...
void loadBufferData()
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 6 * sizeof(i32), indexData, GL_STATIC_DRAW);
...
void initTestTexture()
{
	...
	i32 width = 256;
	i32 height = 256;
	u8* image = (u8*)malloc(width * height);
...
tukk fileName = "../../bin/1000 stack.bullet";
void Usage()
{
	printf("\nprogram.exe [--pause_simulation=<0 or 1>] [--load_bulletfile=test.bullet] [--enable_interop=<0 or 1>] [--enable_gpusap=<0 or 1>] [--enable_convexheightfield=<0 or 1>] [--enable_static=<0 or 1>] [--x_dim=<i32>] [--y_dim=<num>] [--z_dim=<i32>] [--x_gap=<float>] [--y_gap=<float>] [--z_gap=<float>]\n");
};
...
i32 main(i32 argc, tuk argv[])
{
	...
	i32 droidRegular;  //, droidItalic, droidBold, droidJapanese, dejavu;
	GLuint texture;

	i32 fontTextureWidth = 512;
	i32 fontTextureHeight = 512;
			...
			if (0)
				for (i32 i = 0; i < 1; i++)
				{
					...
					if (once != 1)
					{
						...
						//sth_draw_text(stash, droidJapanese,16.f, dx, dy-36, (tukk) "\xE7\xA7\x81\xE3\x81\xAF\xE3\x82\xAC\xE3\x83\xA9\xE3\x82\xB9\xE3\x82\x92\xE9\xA3\x9F\xE3\x81\xB9\xE3\x82\x89\xE3\x82\x8C\xE3\x81\xBE\xE3\x81\x99\xE3\x80\x82",&dx,
						//              width,height);//はabcdefghijlkmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^&*()_-+=?/\][{}.,<>`~@#$%^", &dx);
						//				sth_draw_text(stash, droidJapanese,32.f, dx, dy, (tukk) "私はガラスを食べられます。それは私を傷つけません。",&dx);//はabcdefghijlkmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^&*()_-+=?/\][{}.,<>`~@#$%^", &dx);
