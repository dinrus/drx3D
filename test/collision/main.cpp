...
btVector3 MyBulletShapeSupportFunc(ukk shapeAptr, const btVector3& dir, bool includeMargin)
{
...
btVector3 MyBulletShapeCenterFunc(ukk shapeAptr)
{
...
inline i32 btComputeGjkEpaSphereSphereCollision(const btSphereSphereCollisionDescription& input, btDistanceInfo* distInfo, SphereSphereTestMethod method)
{
...
{
	{
		...
		i32 result = btComputeSphereSphereCollision(ssd, &distInfo);
		ASSERT_EQ(0, result);
		ASSERT_EQ(btScalar(0), distInfo.m_distance);
	}

	for (i32 rb = 1; rb < 5; rb++)
		for (i32 z = -5; z < 5; z++)
		{
			for (i32 j = 1; j < 5; j++)
			{
...
public:
	...
	virtual void processTriangle(btVector3* triangle, i32 partId, i32 triangleIndex)
	{
