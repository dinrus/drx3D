...
u8* openFile(tukk fileName, i32* sizeInBytesPtr)
{
	*sizeInBytesPtr = 0;
			...
			else
			{
				...
				if (result == UNZ_OK)
				{
					...
					else
					{
						...
						if (result == UNZ_OK)
						{
							buffer = (u8*)malloc(info.uncompressed_size);
	...
	if (!buffer)
	{
		...
		if (f)
		{
			...
			i32 actualRead = fread(buffer, sizeInBytes, 1, f);
			if (actualRead != 1)
			{
				free(buffer);
				buffer = 0;
...
void testLauncher(tukk fileName2, b3LauncherCL& launcher, cl_context ctx)
{
	i32 sizeInBytes = 0;

	u8* buf = openFile(fileName2, &sizeInBytes);
	ASSERT_FALSE(buf == NULL);
	if (buf)
	{
		i32 serializedBytes = launcher.deserializeArgs(buf, sizeInBytes, ctx);
...
TEST_F(ExecuteBullet3NarrowphaseKernels, mprKernelsCL)
{
...

	{
		...
		if (1)
		{
			tukk fileNames[] = {"mprPenetrationKernel60.bin", "mprPenetrationKernel61.bin", "mprPenetrationKernel70.bin", "mprPenetrationKernel128.bin"};
			i32 results[] = {0, 1, 46, 98};

			i32 numTests = sizeof(fileNames) / sizeof(tukk);
			for (i32 i = 0; i < numTests; i++)
			{
				...
				b3OpenCLArray<i32> totalContactsOut(this->m_clContext, this->m_clQueue);
				totalContactsOut.setFromOpenCLBuffer(data.m_clBuffer, 1);
				i32 numContacts = totalContactsOut.at(0);
