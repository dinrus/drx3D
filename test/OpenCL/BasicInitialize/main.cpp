...
void myerrorwarningprintf(tukk msg)
{
...
i32 main(i32 argc, tuk argv[])
{
	...
	for (i32 i = 0; i < numPlatforms; i++)
	{
		...
		i32 numDevices = b3OpenCLUtils::getNumDevices(g_cxMainContext);
	...
	if (g_cxMainContext)
	{
		i32 numDev = b3OpenCLUtils::getNumDevices(g_cxMainContext);

		for (i32 i = 0; i < numDev; i++)
		{
			cl_device_id device;
