...
i32 gPreferredDeviceId = -1;
i32 gPreferredPlatformId = -1;
	...
	u32 iterations)
{
	...
	i32 max_elements = num_elements;
	b3AlignedObjectArray<u32> hostData;
	...
	b3OpenCLArray<u32> gpuData(g_cxMainContext, g_cqCommandQueue);
	gpuData.copyFromHost(hostData);
	//sorter.executeHost(gpuData);
	sorter.execute(gpuData);

	b3AlignedObjectArray<u32> hostDataSorted;
 ...
 */
template <typename K>
void RandomBits(K &key, i32 entropy_reduction = 0, i32 lower_key_bits = sizeof(K) * 8)
{
	u32k NUM_UCHARS = (sizeof(K) + sizeof(u8) - 1) / sizeof(u8);
	u8 key_bits[NUM_UCHARS];

	do
	{
		for (i32 j = 0; j < NUM_UCHARS; j++)
		{
			u8 quarterword = 0xff;
			for (i32 i = 0; i <= entropy_reduction; i++)
			{
				quarterword &= (rand() >> 7);
			}
...
i32 CompareResults(T *computed, T *reference, SizeT len, bool verbose = true)
{
	printf("\n");
...
void Usage()
{
	printf("\ntest_large_problem_sorting [--device=<device index>] [--v] [--i=<num-iterations>] [--n=<num-elements>] [--key-values] [--deviceId=<i32>] [--platformId=<i32>]\n");
	printf("\n");
...
public:
	...
	template <typename T>
	void GetCmdLineArgument(tukk arg_name, T &val);
...
template <typename T>
void b3CommandLineArgs::GetCmdLineArgument(tukk arg_name, T &val)
{
	using namespace std;
	map<string, string>::iterator itr;
	if ((itr = pairs.find(arg_name)) != pairs.end())
...
template <>
void b3CommandLineArgs::GetCmdLineArgument<char *>(tukk arg_name, char *&val)
{
	using namespace std;
	map<string, string>::iterator itr;
	if ((itr = pairs.find(arg_name)) != pairs.end())
...
i32 main(i32 argc, char **argv)
{
	...
	i32 numDev = b3OpenCLUtils_getNumDevices(g_cxMainContext);
	...
	u32 num_elements = 8 * 1024 * 1024;  //4*1024*1024;//4*1024*1024;//257;//8*524288;//2048;//512;//524288;
	u32 iterations = 10;
	bool keys_only = true;
