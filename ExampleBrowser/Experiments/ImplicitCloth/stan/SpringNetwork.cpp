...
inline float3x3 dfdx_spring(const float3 &dir, float length, float rest, float k)
{
	...
	return ((I - outerprod(dir, dir)) * d3Min(1.0f, rest / length) - I) * -k;
}
inline float3x3 dfdx_damp(const float3 &dir, float length, const float3 &vel, float rest, float damping)
{
	// inner spring damping   vel is the relative velocity  of the endpoints.
	return (I - outerprod(dir, dir)) * (-damping * -(dot(dir, vel) / d3Max(length, rest)));
}
...
SpringNetwork::SpringNetwork(i32 _n) : X(_n), V(_n), F(_n), dV(_n), A(_n), dFdX(_n), dFdV(_n)
{
...
void SpringNetwork::Simulate(float dt)
{
	...
	if (!awake) return;
	CalcForces();
	i32 n = X.count;    // all our big vectors are of this size
