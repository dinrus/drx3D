...
i32 argmin(const float a[], i32 n);
i32 argmax(const float a[], i32 n);
float squared(float a);
float clamp(float a, const float minval = 0.0f, const float maxval = 1.0f);
i32 clamp(i32 a, i32k minval, i32k maxval);
...
template <class T>
T d3Max(const T &a, const T &b)
{
	return (a > b) ? a : b;
}

template <class T>
T d3Min(const T &a, const T &b)
{
	return (a < b) ? a : b;
}
...
public:
	...
	inline T &operator[](i32 i) { return ((T *)this)[i]; }
	inline const T &operator[](i32 i) const { return ((T *)this)[i]; }
};

typedef vec2<i32> int2;
typedef vec2<float> float2;

template <class T>
inline i32 operator==(const vec2<T> &a, const vec2<T> &b)
{
...
{
	return vec3<T>(d3Min(a.x, b.x), d3Min(a.y, b.y), d3Min(a.z, b.z));
}
template <class T>
inline vec3<T> VectorMax(const vec3<T> &a, const vec3<T> &b)
{
	return vec3<T>(d3Max(a.x, b.x), d3Max(a.y, b.y), d3Max(a.z, b.z));
}
i32 overlap(const float3 &bmina, const float3 &bmaxa, const float3 &bminb, const float3 &bmaxb);
...
public:
	...
	inline vec3<T> &operator[](i32 i) { return (&x)[i]; }
	inline const vec3<T> &operator[](i32 i) const { return (&x)[i]; }
	inline T &operator()(i32 r, i32 c) { return ((&x)[r])[c]; }
	inline const T &operator()(i32 r, i32 c) const { return ((&x)[r])[c]; }
};
...
public:
	...
	T &operator[](i32 i) { return ((T *)this)[i]; }
	const T &operator[](i32 i) const { return ((T *)this)[i]; }
	inline const vec3<T> &xyz() const { return *((vec3<T> *)this); }
	inline vec3<T> &xyz() { return *((vec3<T> *)this); }
};

typedef vec4<float> float4;
typedef vec4<i32> int4;
typedef vec4<u8> byte4;

template <class T>
inline i32 operator==(const vec4<T> &a, const vec4<T> &b)
{
...
public:
	...
	inline vec4<T> &operator[](i32 i)
	{
		assert(i >= 0 && i < 4);
		return (&x)[i];
	}
	inline const vec4<T> &operator[](i32 i) const
	{
		assert(i >= 0 && i < 4);
		return (&x)[i];
	}
	inline T &operator()(i32 r, i32 c)
	{
		assert(r >= 0 && r < 4 && c >= 0 && c < 4);
		return ((&x)[r])[c];
	}
	inline const T &operator()(i32 r, i32 c) const
	{
		assert(r >= 0 && r < 4 && c >= 0 && c < 4);
		return ((&x)[r])[c];
	}
...
i32 operator==(const float4x4 &a, const float4x4 &b);
...
inline Quat QuatFromAxisAngle(const float3 &_v, float angle_radians)
{
	float3 v = normalize(_v) * sinf(angle_radians / 2.0f);
	return Quat(v.x, v.y, v.z, cosf(angle_radians / 2.0f));
}
...
float3 rotate(const Quat &q, const float3 &v);
//float3		operator*( const Quat& q, const float3& v );
//float3		operator*( const float3& v, const Quat& q );

Quat slerp(const Quat &a, const Quat &b, float t);
Quat Interpolate(const Quat &q0, const Quat &q1, float t);
Quat RotationArc(float3 v0, float3 v1);  // returns quat q where q*v0*q^-1=v1
float4x4 MatrixFromQuatVec(const Quat &q, const float3 &v);

inline Quat QuatFromMat(const float3 &t, const float3 &b, const float3 &n)
{
...
public:
	...
	Quat orientation;
	Pose() {}
	Pose(const float3 &p, const Quat &q) : position(p), orientation(q) {}
...
Quat YawPitchRoll(float yaw, float pitch, float roll);
...
Plane Transform(const Plane &p, const float3 &translation, const Quat &rotation);
...
float3 PlanesIntersection(const Plane *planes, i32 planes_count, const float3 &seed = float3(0, 0, 0));

i32 Clip(const Plane &p, const float3 *verts_in, i32 count, float *verts_out);                                                            // verts_out must be preallocated with sufficient size >= count+1 or more if concave
i32 ClipPolyPoly(const float3 &normal, const float3 *clipper, i32 clipper_count, const float3 *verts_in, i32 in_count, float3 *scratch);  //scratch must be preallocated
...
i32 BoxInside(const float3 &p, const float3 &bmin, const float3 &bmax);
i32 BoxIntersect(const float3 &v0, const float3 &v1, const float3 &bmin, const float3 &bmax, float3 *impact);
...
float3 NormalOf(const float3 *vert, i32k n);
Quat VirtualTrackBall(const float3 &cop, const float3 &cor, const float3 &dir0, const float3 &dir1);
i32 Clip(const float3 &plane_normal, float plane_dist, const float3 *verts_in, i32 count, float *verts_out);                              // verts_out must be preallocated with sufficient size >= count+1 or more if concave
i32 ClipPolyPoly(const float3 &normal, const float3 *clipper, i32 clipper_count, const float3 *verts_in, i32 in_count, float3 *scratch);  //scratch must be preallocated
float3 Diagonal(const float3x3 &M);
Quat Diagonalizer(const float3x3 &A);
float3 Orth(const float3 &v);
i32 SolveQuadratic(float a, float b, float c, float *ta, float *tb);  // if true returns roots ta,tb where ta<=tb
i32 HitCheckPoly(const float3 *vert, i32k n, const float3 &v0, const float3 &v1, float3 *impact = NULL, float3 *normal = NULL);
i32 HitCheckRaySphere(const float3 &sphereposition, float radius, const float3 &_v0, const float3 &_v1, float3 *impact, float3 *normal);
i32 HitCheckRayCylinder(const float3 &p0, const float3 &p1, float radius, const float3 &_v0, const float3 &_v1, float3 *impact, float3 *normal);
i32 HitCheckSweptSphereTri(const float3 &p0, const float3 &p1, const float3 &p2, float radius, const float3 &v0, const float3 &_v1, float3 *impact, float3 *normal);
void BoxLimits(const float3 *verts, i32 verts_count, float3 &bmin_out, float3 &bmax_out);
void BoxLimits(const float4 *verts, i32 verts_count, float3 &bmin_out, float3 &bmax_out);

template <class T>
inline i32 maxdir(const T *p, i32 count, const T &dir)
{
	assert(count);
	i32 m = 0;
...
float3 CenterOfMass(const float3 *vertices, const int3 *tris, i32k count);
float3x3 Inertia(const float3 *vertices, const int3 *tris, i32k count, const float3 &com = float3(0, 0, 0));
float Volume(const float3 *vertices, const int3 *tris, i32k count);
i32 calchull(float3 *verts, i32 verts_count, int3 *&tris_out, i32 &tris_count, i32 vlimit);  // computes convex hull see hull.cpp
