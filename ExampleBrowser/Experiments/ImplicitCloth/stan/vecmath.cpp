...
float clamp(float a, const float minval, const float maxval) { return d3Min(maxval, d3Max(minval, a)); }
i32 clamp(i32 a, i32k minval, i32k maxval) { return d3Min(maxval, d3Max(minval, a)); }
...
void BoxLimits(const float3 *verts, i32 verts_count, float3 &bmin, float3 &bmax)
{
	bmin = float3(FLT_MAX, FLT_MAX, FLT_MAX);
	bmax = float3(-FLT_MAX, -FLT_MAX, -FLT_MAX);
	for (i32 i = 0; i < verts_count; i++)
	{
		bmin = VectorMin(bmin, verts[i]);
		bmax = VectorMax(bmax, verts[i]);
	}
}
void BoxLimits(const float4 *verts, i32 verts_count, float3 &bmin, float3 &bmax)
{
	bmin = float3(FLT_MAX, FLT_MAX, FLT_MAX);
	bmax = float3(-FLT_MAX, -FLT_MAX, -FLT_MAX);
	for (i32 i = 0; i < verts_count; i++)
...
Quat slerp(const Quat &_a, const Quat &b, float interp)
{
	Quat a = _a;
	if (dot(a, b) < 0.0)
	{
...
Quat Interpolate(const Quat &q0, const Quat &q1, float alpha)
{
...
Quat RotationArc(float3 v0, float3 v1)
{
	static Quat q;
...
Quat VirtualTrackBall(const float3 &cop, const float3 &cor, const float3 &dir1, const float3 &dir2)
{
	// routine taken from game programming gems.
...
i32 countpolyhit = 0;
i32 HitCheckPoly(const float3 *vert, i32k n, const float3 &v0, const float3 &v1, float3 *impact, float3 *normal)
{
	countpolyhit++;
	i32 i;
	float3 nrml(0, 0, 0);
...
i32 HitCheckRaySphere(const float3 &sphereposition, float radius, const float3 &_v0, const float3 &_v1, float3 *impact, float3 *normal)
{
	...
	float ta, tb;
	i32 doesIntersect = SolveQuadratic(a, b, c, &ta, &tb);
...
float3 PlanesIntersection(const Plane *planes, i32 planes_count, const float3 &seed)
{
	i32 i;
...
i32 Clip(const float3 &plane_normal, float plane_dist, const float3 *verts_in, i32 count_in, float3 *verts_out)
{
	// clips a polygon specified by the non-indexed vertex list verts_in.
	// verts_out must be preallocated with a size >= count+1
	assert(verts_out);
	i32 n = 0;
	i32 prev_status = (dot(plane_normal, verts_in[count_in - 1]) + plane_dist > 0);
	for (i32 i = 0; i < count_in; i++)
	{
		i32 status = (dot(plane_normal, verts_in[i]) + plane_dist > 0);
...
i32 ClipPolyPoly(const float3 &normal, const float3 *clipper, i32 clipper_count, const float3 *verts_in, i32 in_count, float3 *scratch)
{
	...
	i32 i;
	i32 bsize = clipper_count + in_count;
	i32 count = in_count;
	for (i = 0; i < clipper_count; i++)
	{
		i32 i1 = (i + 1) % clipper_count;
...
float Volume(const float3 *vertices, const int3 *tris, i32k count)
{
	...
	float volume = 0;
	for (i32 i = 0; i < count; i++)  // for each triangle
	{
...
float3x3 ShapeInertiaContrib(const float3 &cor, const float3 &position, const Quat &orientation,
...
Quat Diagonalizer(const float3x3 &A)
{
	...
	i32 maxsteps = 24;  // certainly wont need that many.
	i32 i;
	Quat q(0, 0, 0, 1);
	for (i = 0; i < maxsteps; i++)
	{
		...
		i32 k = (om.x > om.y && om.x > om.z) ? 0 : (om.y > om.z) ? 1 : 2;  // index of largest element of offdiag
		i32 k1 = (k + 1) % 3;
		i32 k2 = (k + 2) % 3;
		...
		Quat jr(0, 0, 0, 0);                                                       // jacobi rotation for this iteration.
