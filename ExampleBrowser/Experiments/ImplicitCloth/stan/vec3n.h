...
//#include <malloc.h>
//template <class T> uk  vec4<T>::operator new[](size_t n){ return _mm_malloc(n,64); }
//template <class T> void vec4<T>::operator delete[](uk a) { _mm_free(a); }
...
struct HalfConstraint
{
	...
	i32 vi;
	float s, t;
	HalfConstraint(const float3 &_n, i32 _vi, float _t) : n(_n), vi(_vi), s(0), t(_t) {}
	HalfConstraint() : vi(-1) {}
};
...
public:
	...
	float3Nx3N() : n(0) {}
	float3Nx3N(i32 _n) : n(_n)
	{
		for (i32 i = 0; i < n; i++) blocks.Add(Block((short)i, (short)i));
	}
	template <class E>
...
i32 ConjGradient(float3N &X, float3Nx3N &A, float3N &B);
i32 ConjGradientFiltered(float3N &X, const float3Nx3N &A, const float3N &B, const float3Nx3N &S, Array<HalfConstraint> &H);
i32 ConjGradientFiltered(float3N &X, const float3Nx3N &A, const float3N &B, const float3Nx3N &S);
...
{
	float3N r(a.count);
	for (i32 i = 0; i < a.count; i++) r[i] = a[i] + b[i];
	return r;
}
...
{
	float3N r(a.count);
	for (i32 i = 0; i < a.count; i++) r[i] = a[i] * s;
	return r;
}
inline float3N operator/(const float3N &a, const float &s)
{
...
{
	float3N r(a.count);
	for (i32 i = 0; i < a.count; i++) r[i] = -a[i];
	return r;
}
...
inline float3N &operator-=(float3N &A, const float3N &B)
{
	...
	for (i32 i = 0; i < A.count; i++) A[i] -= B[i];
	return A;
}
inline float3N &operator+=(float3N &A, const float3N &B)
{
...
public:
	...
	{
		for (i32 i = 0; i < v.count; i++) r[i] = -v[i];
	}
...
public:
	...
	void evalequals(float3N &r) const
	{
		for (i32 i = 0; i < v.count; i++) r[i] = v[i] * s;
	}
	void evalpluseq(float3N &r) const
	{
		for (i32 i = 0; i < v.count; i++) r[i] += v[i] * s;
...
public:
	...
	void evalequals(float3N &r) const
	{
		for (i32 i = 0; i < a.count; i++) r[i] = a[i] * s + b[i];
	}
	void evalpluseq(float3N &r) const
	{
		for (i32 i = 0; i < a.count; i++) r[i] += a[i] * s + b[i];
...
public:
	...
	void evalequals(float3N &r) const
	{
		for (i32 i = 0; i < a.count; i++) r[i] = a[i] * s + b[i] * t;
	}
	void evalpluseq(float3N &r) const
	{
		for (i32 i = 0; i < a.count; i++) r[i] += a[i] * s + b[i] * t;
...
public:
	...
	{
		for (i32 i = 0; i < r.blocks.count; i++) r.blocks[i].m = m.blocks[i].m * s;
	}
...
public:
	...
	void evalequals(float3Nx3N &r) const
	{
		for (i32 i = 0; i < r.blocks.count; i++) r.blocks[i].m = a.blocks[i].m * s + b.blocks[i].m * t;
	}
	void evalpluseq(float3Nx3N &r) const
	{
		for (i32 i = 0; i < r.blocks.count; i++) r.blocks[i].m += a.blocks[i].m * s + b.blocks[i].m * t;
