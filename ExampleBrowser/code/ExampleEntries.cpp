...
#include "../Importers/ImportObjExample.h"
...
#include <drx3D/Importers/MJCF/ImportMJCFSetup.h>
...
struct ExampleEntry
{
	...
	ExampleEntry(i32 menuLevel, tukk name)
		: m_menuLevel(menuLevel), m_name(name), m_description(0), m_createFunc(0), m_option(0)
	{
	}

	ExampleEntry(i32 menuLevel, tukk name, tukk description, CommonExampleInterface::CreateFunc* createFunc, i32 option = 0)
...
static ExampleEntry gDefaultExamples[] =
	{
		...
		ExampleEntry(1, "Базовый Пример", "Создаются твёрдые тела посредством форм боксовых коллизий. Хороший пример для знакомства с базовой инициализацией drx3D. Базовый Пример можно скомпилировать без ГИП, как консольное приложение. Нажмите W, чтобы видеть wireframe, A - AABBы, I - приостановить/презапустить симуляцию физики. Нажмите D, чтобы преключать автодеактивацию симуляции. ", BasicExampleCreateFunc),
		...
		ExampleEntry(1, "Constraints", "Show the use of the various constraints in drx3D. Press the L key to visualize the constraint limits. Press the C key to visualize the constraint frames.",
		...
		ExampleEntry(1, "TestHingeTorque", "Apply a torque in the hinge axis. This example uses a btHingeConstraint and RigidBody. The setup is similar to the multi body example TestJointTorque.",
		...
		ExampleEntry(1, "Gyroscopic", "Show the Dzhanibekov effect using various settings of the gyroscopic term. You can select the gyroscopic term computation using RigidBody::setFlags, with arguments DRX3D_ENABLE_GYROSCOPIC_FORCE_EXPLICIT (using explicit integration, which adds energy and can lead to explosions), DRX3D_ENABLE_GYROSCOPIC_FORCE_IMPLICIT_WORLD, DRX3D_ENABLE_GYROSCOPIC_FORCE_IMPLICIT_BODY. If you don't set any of these flags, there is no gyroscopic term used.", GyroscopicCreateFunc),
    ...
    ExampleEntry(1, "Kinematic Body", "Let the user set the transform, the physics engine computes the velocity for one-way contact and friction interaction.", KinematicRigidBodyExampleCreateFunc),
		...
		ExampleEntry(1, "TestPendulum", "Simulate a pendulum using btMultiBody with a constant joint torque applied. The same code is also used as a unit test comparing drx3D with the numerical solution of second-order non-linear differential equation stored in pendulum_gold.h", TestPendulumCreateFunc),
    ...
    ExampleEntry(1, "Kinematic MultiBody", "Let the user set the transform, the physics engine computes the velocity for one-way contact and friction interaction.", KinematicMultiBodyExampleCreateFunc),
		...
		ExampleEntry(1, "Inverse Dynamics URDF", "Create a btMultiBody from URDF. Create an inverse MultiBodyTree model from that. Use either decoupled PD control or computed torque control using the inverse model to track joint position targets", drx3d_inverseExampleCreateFunc, DRX3D_ID_LOAD_URDF),
		ExampleEntry(1, "Inverse Dynamics Prog", "Create a btMultiBody programatically. Create an inverse MultiBodyTree model from that. Use either decoupled PD control or computed torque control using the inverse model to track joint position targets", drx3d_inverseExampleCreateFunc, DRX3D_ID_PROGRAMMATICALLY),
		...
		ExampleEntry(1, "Spheres & Plane C-API (Bullet2)", "Collision C-API using drx3D 2.x backend", CollisionTutorialBullet2CreateFunc, TUT_SPHERE_PLANE_BULLET2),
//ExampleEntry(1, "Spheres & Plane C-API (Bullet3)", "Collision C-API using drx3D 3.x backend", CollisionTutorialBullet2CreateFunc,TUT_SPHERE_PLANE_RTB3),
		...
		ExampleEntry(1, "Ragdolls", "Benchmark the performance of the ragdoll constraints, btHingeConstraint and ConeTwistConstraint, in addition to capsule collision detection.", BenchmarkCreateFunc, 3),
		ExampleEntry(1, "Convex stack", "Benchmark the performance and stability of rigid bodies using ConvexHullShape.", BenchmarkCreateFunc, 4),
		ExampleEntry(1, "Prim vs Mesh", "Benchmark the performance and stability of rigid bodies using primitive collision shapes (SphereShape, BoxShape), resting on a triangle mesh, BvhTriangleMeshShape.", BenchmarkCreateFunc, 5),
		ExampleEntry(1, "Convex vs Mesh", "Benchmark the performance and stability of rigid bodies using convex hull collision shapes (ConvexHullShape), resting on a triangle mesh, BvhTriangleMeshShape.", BenchmarkCreateFunc, 6),
		ExampleEntry(1, "Raycast", "Benchmark the performance of the CollisionWorld::rayTest. Note that currently the rays are not rendered.", BenchmarkCreateFunc, 7),
		ExampleEntry(1, "Convex Pack", "Benchmark the performance of the convex hull primitive.", BenchmarkCreateFunc, 8),
		ExampleEntry(1, "Heightfield", "Raycast against a HeightfieldTerrainShape", HeightfieldExampleCreateFunc),
		//#endif
		...
		ExampleEntry(1, "URDF (RigidBody)", "Import a URDF file, and create rigid bodies (RigidBody) connected by constraints.", ImportURDFCreateFunc, 0),
		ExampleEntry(1, "URDF (MultiBody)", "Import a URDF file and create a single multibody (MultiBody) with tree hierarchy of links (mobilizers).",
		...
		ExampleEntry(1, "Raytest", "Cast rays using the CollisionWorld::rayTest method. The example shows how to receive the hit position and normal along the ray against the first object. Also it shows how to receive all the hits along a ray.", RaytestCreateFunc),
		ExampleEntry(1, "Raytracer", "Implement an extremely simple ray tracer using the ray trace functionality in CollisionWorld.",
					 RayTracerCreateFunc),
		...
		ExampleEntry(1, "Voronoi Fracture", "Automatically create a compound rigid body using voronoi tesselation. Individual parts are modeled as rigid bodies using a ConvexHullShape.",
					 VoronoiFractureCreateFunc),

		ExampleEntry(1, "Fracture demo", "Create a basic custom implementation to model fracturing objects, based on a CompoundShape. It explicitly propagates the collision impulses and breaks the rigid body into multiple rigid bodies. Press F to toggle fracture and glue mode.", FractureDemoCreateFunc),

		ExampleEntry(1, "Planar 2D", "Show the use of 2D collision shapes and rigid body simulation. The collision shape is wrapped into a Convex2dShape. The rigid bodies are restricted in a plane using the 'setAngularFactor' and 'setLinearFactor' API call.", Planar2DCreateFunc),
#if DRX3D_THREADSAFE
...
#endif
#endif  //
static AlignedObjectArray<ExampleEntry> gAdditionalRegisteredExamples;

struct ExampleEntriesInternalData
{
	AlignedObjectArray<ExampleEntry> m_allExamples;
};
...
#ifdef D3_USE_CLEW
#ifndef NO_OPENGL3
	i32 numDefaultEntries = sizeof(gOpenCLExamples) / sizeof(ExampleEntry);
...
void ExampleEntriesAll::initExampleEntries()
{
	...
	for (i32 i = 0; i < gAdditionalRegisteredExamples.size(); i++)
	{
		m_data->m_allExamples.push_back(gAdditionalRegisteredExamples[i]);
	}

	i32 numDefaultEntries = sizeof(gDefaultExamples) / sizeof(ExampleEntry);
	for (i32 i = 0; i < numDefaultEntries; i++)
	{
...
void ExampleEntriesAll::registerExampleEntry(i32 menuLevel, tukk name, tukk description, CommonExampleInterface::CreateFunc* createFunc, i32 option)
{
	ExampleEntry e(menuLevel, name, description, createFunc, option);
	gAdditionalRegisteredExamples.push_back(e);
}

i32 ExampleEntriesAll::getNumRegisteredExamples()
{
	return m_data->m_allExamples.size();
}

CommonExampleInterface::CreateFunc* ExampleEntriesAll::getExampleCreateFunc(i32 index)
{
	return m_data->m_allExamples[index].m_createFunc;
}

i32 ExampleEntriesAll::getExampleOption(i32 index)
{
	return m_data->m_allExamples[index].m_option;
}

tukk ExampleEntriesAll::getExampleName(i32 index)
{
	return m_data->m_allExamples[index].m_name;
}

tukk ExampleEntriesAll::getExampleDescription(i32 index)
{
