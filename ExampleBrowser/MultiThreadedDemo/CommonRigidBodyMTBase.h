...
inline tukk getSolverTypeName(SolverType t)
{
	switch (t)
	{
	...
	Assert(!"unhandled solver type in switch");
	return "???";
}
...
#define USE_MOTIONSTATE 1
	...
	RigidBody* createKinematicBody(const Transform2& startTransform, CollisionShape* shape)
	{
		Assert((!shape || shape->getShapeType() != INVALID_SHAPE_PROXYTYPE));

		RigidBody* body = new RigidBody(0.0f, NULL, shape);
		body->setWorldTransform(startTransform);
		body->setCollisionFlags(body->getCollisionFlags() | CollisionObject2::CF_KINEMATIC_OBJECT);
		body->setUserIndex(-1);
		m_dynamicsWorld->addRigidBody(body);
		return body;
	}
