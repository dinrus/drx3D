...
#include <cstdlib>
#include "../our_gl.h"
#include <drx3D/Common/b3MinMax.h>

namespace TinyRender
{
IShader::~IShader() {}

Matrix viewport(i32 x, i32 y, i32 w, i32 h)
{
	Matrix Viewport;
...
void triangleClipped(mat<4, 3, float> &clipc, mat<4, 3, float> &orgClipc, IShader &shader, TGAImage &image, float *zbuffer, i32 *segmentationMaskBuffer, const Matrix &viewPortMatrix, i32 objectAndLinkIndex)
{
	...
	for (i32 i = 0; i < 3; i++)
	{
		for (i32 j = 0; j < 2; j++)
		{
			bboxmin[j] = d3Max(0.f, d3Min(bboxmin[j], pts2[i][j]));
			bboxmax[j] = d3Min(clamp[j], d3Max(bboxmax[j], pts2[i][j]));
		}
	}
	...
	for (P.x = bboxmin.x; P.x <= bboxmax.x; P.x++)
	{
		for (P.y = bboxmin.y; P.y <= bboxmax.y; P.y++)
		{
			double frag_depth = 0;
			{
				Vec3d bc_screen = barycentric(pts2[0], pts2[1], pts2[2], Vec2f(P.x, P.y));
			...
			Vec3d bc_screen2 = barycentric(orgPts2[0], orgPts2[1], orgPts2[2], Vec2f(P.x, P.y));
...
void triangle(mat<4, 3, float> &clipc, IShader &shader, TGAImage &image, float *zbuffer, i32 *segmentationMaskBuffer, const Matrix &viewPortMatrix, i32 objectAndLinkIndex)
{
	mat<3, 4, float> pts = (viewPortMatrix * clipc).transpose();  // transposed to ease access to each of the points

	mat<3, 2, float> pts2;
	for (i32 i = 0; i < 3; i++) pts2[i] = proj<2>(pts[i] / pts[i][3]);
