...
/////////// box should swing (rotate) left-to-right forever
		{
			...
			localA.setIdentity();localA.getOrigin() = Vec3(0,0,0);
			localB.setIdentity();localB.setOrigin(Vec3(0,0.5,0));
...
/////////// the box should rotate around the y axis
		{
			...
			localA.setIdentity();localA.getOrigin() = Vec3(4,0,0);
			localB.setIdentity();
...
/////////// the target will be negated periodically
		{
			...
			localA.setIdentity();localA.getOrigin() = Vec3(7,0,0);
			localB.setIdentity();
...
////////// the chain will be pulled to the left and to the right periodically. They should strictly stick together.
		{
			...
			for(i32 i = 0; i < bodycount; ++i)
			{
				...
				if(prevBody != 0)
				{
					...
					if(i < bodycount - 1)
					{
						localA.setIdentity();localA.getOrigin() = Vec3(0,0,3);
						localB.setIdentity();
				...
				{
					localA.setIdentity();localA.getOrigin() = Vec3(bodycount,0,3);
					localB.setIdentity();
...
/////// servo motor: flip its target periodically
	...
	//Scalar pos = m_data->m_ServoMotorConstraint->getRotationalLimitMotor(2)->m_currentPosition;
	//Scalar target = m_data->m_ServoMotorConstraint->getRotationalLimitMotor(2)->m_servoTarget;
	if (servoNextFrame < 0)
	{
