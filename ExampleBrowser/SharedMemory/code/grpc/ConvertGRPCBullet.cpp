...
pybullet_grpc::PyBulletCommand* convertBulletToGRPCCommand(const struct SharedMemoryCommand& clientCmd, pybullet_grpc::PyBulletCommand& grpcCommand)
{
	...
	switch (clientCmd.m_type)
	{
		...
		case CMD_INIT_POSE:
		{
			...
			::pybullet_grpc::InitPoseCommand* pose = grpcCommand.mutable_initposecommand();
			{
				...
				i32 maxQ = 0;
				for (i32 q = 0; q < MAX_DEGREE_OF_FREEDOM; q++)
				{
	...
	if (0 == grpcCmdPtr)
	{
		...
		if (sz > 0)
		{
			grpcCommand.add_unknowncommandbinaryblob((tukk)&clientCmd, sz);
		}
	}
	Assert(grpcCmdPtr);
	return grpcCmdPtr;
...
SharedMemoryCommand* convertGRPCToBulletCommand(const PyBulletCommand& grpcCommand, SharedMemoryCommand& cmd)
{
	...
	i32 sz = grpcCommand.unknowncommandbinaryblob_size();
	if (sz)
	{
		if (sz == 1)
		{
			tukk data = grpcCommand.unknowncommandbinaryblob().Get(0).c_str();
			i32 numBytes = grpcCommand.unknowncommandbinaryblob().Get(0).size();

			Assert(sizeof(SharedMemoryCommand) == numBytes);
		...
		{
			printf("Ошибка: Ignore unexpected unknowncommandbinaryblob\n");
		}
		cmdPtr = &cmd;
	}
	...
	if (cmdPtr == 0)
	{
		switch (cmd.m_type)
		{
			...
			case CMD_INIT_POSE:
			{
				Assert(grpcCommand.has_initposecommand());
				if (grpcCommand.has_initposecommand())
				{
					cmdPtr = &cmd;
					b3SharedMemoryCommandHandle commandHandle = (b3SharedMemoryCommandHandle)cmdPtr;
					...
					i32 hasInitialQ[MAX_DEGREE_OF_FREEDOM] = {0};
					i32 hasInitialQdot[MAX_DEGREE_OF_FREEDOM] = {0};

					if (grpcCmd.initialstateq_size() == grpcCmd.hasinitialstateq_size())
					{
						i32 numInitial = d3Min(grpcCmd.initialstateq_size(), MAX_DEGREE_OF_FREEDOM);
						for (i32 i = 0; i < numInitial; i++)
						{
					...
					{
						printf("Ошибка: if (grpcCmd.initialstateq_size() != grpcCmd.hasinitialstateq_size())\n");
					}
					if (grpcCmd.initialstateqdot_size() == grpcCmd.hasinitialstateqdot_size())
					{
						i32 numInitial = d3Min(grpcCmd.initialstateqdot_size(), MAX_DEGREE_OF_FREEDOM);
						for (i32 i = 0; i < numInitial; i++)
						{
					...
					{
						printf("Ошибка: (grpcCmd.initialstateqdot_size() != grpcCmd.hasinitialstateqdot_size())\n");
					}
				}
				break;
			}
			...
			case CMD_REQUEST_ACTUAL_STATE:
			{
				Assert(grpcCommand.has_requestactualstatecommand());
				if (grpcCommand.has_requestactualstatecommand())
				{
					cmdPtr = &cmd;
					b3SharedMemoryCommandHandle commandHandle = (b3SharedMemoryCommandHandle)cmdPtr;
			...
			case CMD_SEND_DESIRED_STATE:
			{
				...
				i32 maxQ = motor->hasdesiredstateflags_size();
				for (i32 q = 0; q < maxQ; q++)
				{
			...
			case CMD_SEND_PHYSICS_SIMULATION_PARAMETERS:
			{
				...
				i32 updateFlags = grpcCommand.setphysicssimulationparameterscommand().updateflags();
			...
			case CMD_CONFIGURE_OPENGL_VISUALIZER:
			{
				Assert(grpcCommand.has_configureopenglvisualizercommand());
			...
			case CMD_LOAD_SDF:
			{
				Assert(grpcCommand.has_loadsdfcommand());
				if (grpcCommand.has_loadsdfcommand())
				{
			...
			case CMD_LOAD_MJCF:
			{
				Assert(grpcCommand.has_loadmjcfcommand());
				if (grpcCommand.has_loadmjcfcommand())
				{
			...
			{
				Assert(grpcCommand.has_changedynamicscommand());
				if (grpcCommand.has_changedynamicscommand())
				{
					cmdPtr = &cmd;
					b3SharedMemoryCommandHandle commandHandle = (b3SharedMemoryCommandHandle)cmdPtr;
					const ::pybullet_grpc::ChangeDynamicsCommand& grpcCmd = grpcCommand.changedynamicscommand();
					i32 bodyUniqueId = grpcCmd.bodyuniqueid();
					i32 linkIndex = grpcCmd.linkindex();
			...
			{
				Assert(grpcCommand.has_getdynamicscommand());
				if (grpcCommand.has_getdynamicscommand())
				{
					cmdPtr = &cmd;
					b3SharedMemoryCommandHandle commandHandle = (b3SharedMemoryCommandHandle)cmdPtr;
...
bool convertGRPCToStatus(const PyBulletStatus& grpcReply, SharedMemoryStatus& serverStatus, tuk bufferServerToClient, i32 bufferSizeInBytes)
{
	bool converted = false;

	serverStatus.m_type = grpcReply.statustype();

	i32 sz = grpcReply.binaryblob().size();
	if (sz > 0)
	{
		if (sz == 1)
		{
			tukk data = grpcReply.binaryblob().Get(0).c_str();
			i32 numBytes = grpcReply.binaryblob().Get(0).size();
	...
	switch (grpcReply.statustype())
	{
		case CMD_ACTUAL_STATE_UPDATE_COMPLETED:
		{
			...
			i32 numLinks = stat->numlinks();
			serverStatus.m_sendActualStateArgs.m_numLinks = numLinks;

			i32 numDegreeOfFreedomQ = stat->numdegreeoffreedomq();
			i32 numDegreeOfFreedomU = stat->numdegreeoffreedomu();
		...
		case CMD_REQUEST_KEYBOARD_EVENTS_DATA_COMPLETED:
		{
			...
			for (i32 i = 0; i < serverStatus.m_sendKeyboardEvents.m_numKeyboardEvents; i++)
			{
		...
		case CMD_SYNC_BODY_INFO_COMPLETED:
		{
			...
			for (i32 i = 0; i < serverStatus.m_sdfLoadedArgs.m_numBodies; i++)
			{
			...
			for (i32 i = 0; i < serverStatus.m_sdfLoadedArgs.m_numUserConstraints; i++)
			{
      ...
      i32 numBodies = stat->bodyuniqueids_size();
		...
		default:
		{
			...
			if (grpcReply.unknownstatusbinaryblob_size() > 0)
			{
				if (grpcReply.unknownstatusbinaryblob_size() == 1)
				{
					...
					tukk data = grpcReply.unknownstatusbinaryblob().Get(0).c_str();
					i32 numBytes = grpcReply.unknownstatusbinaryblob().Get(0).size();

					Assert(sizeof(SharedMemoryStatus) == numBytes);
					if (sizeof(SharedMemoryStatus) == numBytes)
					{
						memcpy(&serverStatus, data, numBytes);
					}
					//printf("slow fallback on command type %d\n", serverStatus.m_type);
					Assert(grpcReply.statustype() == serverStatus.m_type);
					converted = true;
				}
				else
				{
...
bool convertStatusToGRPC(const SharedMemoryStatus& serverStatus, tuk bufferServerToClient, i32 bufferSizeInBytes, PyBulletStatus& grpcReply)
{
	bool converted = false;
	...
	switch (serverStatus.m_type)
	{
		...
		case CMD_URDF_LOADING_COMPLETED:
		{
			...
			i32 objectUniqueId = b3GetStatusBodyIndex(statusHandle);
		...
		{
			converted = true;
			i32 bodyIndicesOut[MAX_SDF_BODIES];
			::pybullet_grpc::SdfLoadedStatus* stat = grpcReply.mutable_sdfstatus();
			b3SharedMemoryStatusHandle statusHandle = (b3SharedMemoryStatusHandle)&serverStatus;
			i32 numBodies = b3GetStatusBodyIndices(statusHandle, bodyIndicesOut, MAX_SDF_BODIES);
			if (numBodies > MAX_SDF_BODIES)
			{
				printf("SDF exceeds body capacity: %d > %d", numBodies, MAX_SDF_BODIES);
			}
	...
	*/
	...
	//printf("convertStatusToGRPC: unknown status (%d), slow fallback", serverStatus.m_type);
			...
			grpcReply.add_unknownstatusbinaryblob((tukk)&serverStatus, sz);
			converted = true;
#ifdef ALLOW_GRPC_STATUS_CONVERSION
		}
	}
