...
public:
	...
	MyGraphWindow(const MyGraphInput& input)
		: Gwen::Controls::WindowControl(input.m_data->pCanvas),
		  m_menuItems(0)
	{
		Gwen::WTxt str = drx::WTxt(input.m_name);
		SetTitle(str);
		...
		if (input.m_texName)
		{
			Gwen::WTxt texName = Gwen::WTxt(input.m_texName);
...
MyGraphWindow* setupTextureWindow(const MyGraphInput& input)
{
	...
	Gwen::WTxt str = Gwen::WTxt(input.m_name);
	menuItems->m_item = input.m_data->m_viewMenu->GetMenu()->AddWItem(str, menuItems, (Gwen::Event::Handler::Function)&MyMenuItems2::MenuItemSelect);
