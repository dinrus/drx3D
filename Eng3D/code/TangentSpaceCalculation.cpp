...
#include <drx3D/Plugins/TangentBasisComputation/mikkelsen/mikktspace.h>
...
{
	u32 dwTriCount = proxy.GetTriangleCount();

	// search for polygons that use the same indices (input data problems)
	for (u32 a = 0; a < dwTriCount; a++)
	{
		u32 dwAPos[3], dwANorm[3], dwAUV[3];

		proxy.GetTriangleIndices(a, dwAPos, dwANorm, dwAUV);

		for (u32 b = a + 1; b < dwTriCount; b++)
		{
			u32 dwBPos[3], dwBNorm[3], dwBUV[3];
...
eCalculateTangentSpaceErrorCode CTangentSpaceCalculation::CalculateTangentSpace(
  const ITriangleInputProxy& inInput,
  const bool bUseCustomNormals,
  const bool bIgnoreDegeneracies,
  string& errorMessage)
{
	...
	u32 dwTriCount = inInput.GetTriangleCount();
	...
	std::multimap<CBaseIndex, u32, CBaseIndexOrder> mBaseMap;
	...
	// distribute the uv vectors to the vertices
	{
		...
		for (u32 i = 0; i < dwTriCount; i++)
		{
			...
			for (e = 0; e < 3; e++)
			{
				...
				Indx.p[e] = AddUV2Base(mBaseMap, dwPos[e], dwNorm[e], TriBase.u * fWeight, TriBase.v * fWeight, TriBase.n.normalize(), bIgnoreDegeneracies);
			}
...
u32 CTangentSpaceCalculation::AddUV2Base(std::multimap<CBaseIndex, u32, CBaseIndexOrder>& inMap,
                                            u32k indwPosNo, u32k indwNormNo, const Vec3& inU, const Vec3& inV, const Vec3& inNormN,
                                            bool bIgnoreDegeneracies)
{
	CBaseIndex Indx;
	...
	std::multimap<CBaseIndex, u32, CBaseIndexOrder>::iterator iFind, iFindEnd;
	...
	u32 dwBaseUVIndex = 0xffffffff;                          // init with not found
	...
	if (dwBaseUVIndex == 0xffffffff)
	{
		...
		inMap.insert(std::pair<CBaseIndex, u32>(Indx, dwBaseUVIndex));
...
void CTangentSpaceCalculation::AddNormal2Base(std::multimap<CBaseIndex, u32, CBaseIndexOrder>& inMap, u32k indwPosNo, u32k indwNormNo, const Vec3& inNormal)
{
	CBaseIndex Indx;
	...
	std::multimap<CBaseIndex, u32, CBaseIndexOrder>::iterator iFind = inMap.find(Indx);
	...
	else
	{
		...
		inMap.insert(std::pair<CBaseIndex, u32>(Indx, dwBaseNIndex));
		m_baseVectors.push_back(Base);
	}
...
void CTangentSpaceCalculation::GetBase(u32k indwPos, float* outU, float* outV, float* outN)
{
...
void CTangentSpaceCalculation::GetTriangleBaseIndices(u32k indwTriNo, u32 outdwBase[3])
{
...
static i32 MikkGetNumVerticesOfFace(const SMikkTSpaceContext* pContext, i32k iFace)
{
	return 3;
}

static void MikkGetPosition(const SMikkTSpaceContext* pContext, float fvPosOut[], i32k iFace, i32k iVert)
{
	SMikkMesh* mikkMesh = (SMikkMesh*)pContext->m_pUserData;
...
static void MikkGetNormal(const SMikkTSpaceContext* pContext, float fvNormOut[], i32k iFace, i32k iVert)
{
	SMikkMesh* mikkMesh = (SMikkMesh*)pContext->m_pUserData;
	const SMikkFace& face = mikkMesh->mikkFaces[iFace];
...
static void MikkGetTexCoord(const SMikkTSpaceContext* pContext, float fvTexcOut[], i32k iFace, i32k iVert)
{
	SMikkMesh* mikkMesh = (SMikkMesh*)pContext->m_pUserData;
	const SMikkFace& face = mikkMesh->mikkFaces[iFace];
...
static void MikkSetTSpace(const SMikkTSpaceContext* pContext, const float fvTangent[], const float fvBiTangent[], const float fMagS, const float fMagT, const tbool bIsOrientationPreserving, i32k iFace, i32k iVert)
{
	SMikkMesh* mikkMesh = (SMikkMesh*)pContext->m_pUserData;
	const SMikkFace& face = mikkMesh->mikkFaces[iFace];
	i32k index = face.vertexOffset + iVert;
...
{
	u32k numFaces = proxy.GetTriangleCount();
	...
	std::map<CBase33, i32, CBase33PredicateLess> uniqueBaseVectors;
	std::map<CBase33, i32, CBase33PredicateLess>::const_iterator it;

	// remove tangent basis duplicates and add them to the mesh
	for (i32 f = 0; f < mikkMesh.mikkNumFaces; ++f)
	{
		...
		{
			i32k index = face.vertexOffset + ii;
