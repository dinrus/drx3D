...
// -------------------------------------------------------------------------
//  Имя файла:   3denginelight.cpp
//  Version:     v1.00
//  Created:     28/5/2001 by Vladimir Kajalin
//  Компиляторы:       drux (DinrusPro's Universal Compiler),   mingw-w64-clang-x86_64
//  Описание:    Управление источниками света.
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
void C3DEngine::RegisterLightSourceInSectors(SRenderLight* pDynLight, const SRenderingPassInfo& passInfo)
{
	...
	if (m_pVisAreaUpr)
		m_pVisAreaUpr->AddLightSource(pDynLight, passInfo);
}
...
void C3DEngine::FindPotentialLightSources(const SRenderingPassInfo& passInfo)
{
	...
	for (i32 i = 0; i < m_lstStaticLights.Count(); i++)
	{
		...
		i32 nRenderNodeMinSpec = (pLightEntity->m_dwRndFlags & ERF_SPEC_BITS_MASK) >> ERF_SPEC_BITS_SHIFT;
...
void C3DEngine::AddDynamicLightSource(const SRenderLight& LSource, ILightSource* pEnt, i32 nEntityLightId, float fFadeout, const SRenderingPassInfo& passInfo)
{
	...
	if (!(LSource.m_Flags & DLF_POST_3D_RENDERER))
	{
		...
		{
			i32 nMaxReqursion = (LSource.m_Flags & DLF_THIS_AREA_ONLY) ? 2 : 3;
			if (!m_pObjUpr || !m_pVisAreaUpr || !m_pVisAreaUpr->IsEntityVisAreaVisible(pEnt, nMaxReqursion, &LSource, passInfo))
			{
				if (LSource.m_Flags & DLF_SUN && m_pVisAreaUpr && m_pVisAreaUpr->m_bSunIsNeeded)
				{
...
void C3DEngine::PrepareLightSourcesForRendering_0(const SRenderingPassInfo& passInfo)
{
	...
	if (passInfo.IsRecursivePass())
	{
		...
		for (i32 i = 0; i < m_nRealLightsNum && i < m_lstDynLights.Count(); i++)
		{
	...
	else
	{
		...
		for (i32 i = 0; i < m_lstDynLights.Count(); i++)
		{
			...
			i32k nMaxLightsNum = max((i32)GetCVars()->e_DynamicLightsMaxCount, MAX_LIGHTS_NUM);
			if (i >= nMaxLightsNum)
			{
				// ignored by renderer
				...
				if (i >= nMaxLightsNum && !bWarningPrinted && (passInfo.GetMainFrameID() & 7) == 7)
				{
					...
					Warning("C3DEngine::PrepareLightSourcesForRendering: No more than %d real lsources allowed on the screen", (i32)nMaxLightsNum);
...
void C3DEngine::PrepareLightSourcesForRendering_1(const SRenderingPassInfo& passInfo)
{
	...
	else
	{
		...
		for (i32 i = m_nRealLightsNum; i < m_lstDynLights.Count(); i++)
		{
			// ignored by renderer
			m_lstDynLights[i]->m_Id = -1;
			GetRenderer()->EF_ADDDlight(m_lstDynLights[i], passInfo);
	...
	if (GetCVars()->e_DynamicLights == 2)
	{
		for (i32 i = 0; i < m_lstDynLights.Count(); i++)
		{
			...
			IRenderAuxText::DrawLabelF(pL->m_Origin, 1.3f, "id=%d, rad=%.1f, vdr=%d", pL->m_Id, pL->m_fRadius, (i32)(pL->m_pOwner ? pL->m_pOwner->m_ucViewDistRatio : 0));
		}
	}
}
...
void C3DEngine::InitShadowFrustums(const SRenderingPassInfo& passInfo)
{
	...
	if (GetCVars()->e_ShadowsDebug == 4)
	{
		for (i32 i = 0; i < m_nRealLightsNum && i < m_lstDynLights.Count(); i++)
		{
			for (i32 j = (m_nRealLightsNum - 1); j >= (i + 1); j--)
			{
...
{
	for (i32 i = 0; i < m_lstPerObjectShadows.Count(); ++i)
...
void C3DEngine::GetCustomShadowMapFrustums(ShadowMapFrustum**& arrFrustums, i32& nFrustumCount)
{
...
void C3DEngine::SetupLightScissors(SRenderLight* pLight, const SRenderingPassInfo& passInfo)
{
	...
	i32k nMaxVertsToProject = 10;
	i32 nVertsToProject = 4;
	...
	for (i32 i = nStart; i < nVertsToProject; i++)
	{
		if (GetCVars()->e_ScissorDebug)
		{
			if (GetRenderer()->GetIRenderAuxGeom() != NULL)
			{
				...
				i32 nPrevVert = (i - 1) < 0 ? nVertsToProject - 1 : (i - 1);
	...
	i32 iWidth = pCam.GetViewSurfaceX();
	i32 iHeight = pCam.GetViewSurfaceZ();
...
float C3DEngine::GetLightAmountInRange(const Vec3& pPos, float fRange, bool bAccurate)
{
	...
	CVisAreaUpr* pVisAreaMan(GetVisAreaUpr());
	if (!pVisAreaMan)
	{
		return 0.0f;
	}
	...
	for (i32 l(0); l < m_lstStaticLights.Count(); ++l)
	{
		...
		if (bAccurate && (pLight.m_Flags & DLF_CASTSHADOW_MAPS))
		{
			...
			u32k nEntQueryFlags(rwi_any_hit | rwi_stop_at_pierceable);
			i32k nObjectTypes(ent_terrain | ent_static | ent_rigid | ent_sleeping_rigid);
...
PodArray<struct ILightSource*>* C3DEngine::GetAffectingLights(const AABB& bbox, bool bAllowSun, const SRenderingPassInfo& passInfo)
{
	...
	for (i32 nListId = 0; nListId < m_lstAffectingLightsCombinations.Count(); nListId++)
	{
