...
#pragma warning(disable: 4101)
	...
	#define MMRM_FRAME_PROFILER(x) DRX_PROFILE_REGION(PROFILE_3DENGINE, x)
#else
...
struct DRX_ALIGN(16) Vec3A: public Vec3
{
	Vec3A()
	{
	}
...
struct DRX_ALIGN(16) DualQuatA: public DualQuat
{
	DualQuatA()
	{
	}
...
static inline Quat mat33_to_quat(const Matrix33& m)
{
	...
	static inline void CvtToHalf(Vec2f16& v, const Vec2& value)
	{
		...
		*reinterpret_cast<i32*>(&v) = _mm_cvtsi128_si32(result); // _mm_storeu_si32 not supported on all compilers, this should (hopefully) be MOVD m32, xmm
	}
	#else
	...
	static inline void CvtToHalf(Vec3f16& v, __m128 value)
	{
		...
		v.x = (reinterpret_cast<i32*>(&result))[0];
		v.y = (reinterpret_cast<i32*>(&result))[1];
		v.z = (reinterpret_cast<i32*>(&result))[2];
	}
...
//----------------------------------------------------------------------------------
static ILINE i32 _Lineseg_Sphere(const Lineseg& ls, const Sphere& s, Vec3& i0, Vec3& i1)
{
  ...
  const Vec3& origin, const Vec3& rotationOrigin, float zRotation)
{
	...
	i32k cullFrustum = context.flags & MMRM_CULL_FRUSTUM;
	i32k cullDistance = context.flags & MMRM_CULL_DISTANCE;
	i32k cullLod = context.flags & MMRM_CULL_LOD;
	i32k forceLod = (context.flags >> MMRM_LOD_SHIFT) & 3;
	i32k sampleReduction = context.flags >> MMRM_SAMPLE_REDUCTION_SHIFT;
	...
	for (size_t j = 0; j < context.amount; ++j)
	{
		...
		i32 nLodTmp = (i32)(distanceSq * lodRatioSq / (max(20.0f * 20.f * diameterSq, 0.001f)));
  ...
  , size_t count)
{
	...
	__m128 vweights[4];
	__m128 vw[4];
	DRX_ALIGN(16) Quat out_tangents[4];
	DRX_ALIGN(16) Quat in_tangents[4];
	#if MMRM_UNROLL_GEOMETRY_BAKING_LOOPS
	for (size_t i = 0; i < (count & ~3); i += 4)
	{
		_wq[0] = _wq[1] = _wq[2] = _wq[3] = _wq[4] = _wq[5] = _wq[6] = _wq[7] = _mm_xor_ps(_wq[0], _wq[0]);
  ...
  , size_t count)
{
	...
	#endif
	{
		//_mm_prefetch((tukk)&in[i+4], _MM_HINT_T0);
		_wq[0] = _wq[1] = _mm_xor_ps(_wq[0], _wq[0]);
		in_tangents[0] = in[i + 0].qt.GetQ();
		vweights[0] = _mm_load_ps(&in[i + 0].weights[0]);
		switch (maxSpines)
  ...
  , size_t count)
{
	...
	{
		//_mm_prefetch((tukk)&in[i+4], _MM_HINT_T0);
		//_mm_prefetch((tukk)&in[i+5], _MM_HINT_T0);
		//_mm_prefetch((tukk)&in[i+6], _MM_HINT_T0);
		//_mm_prefetch((tukk)&in[i+7], _MM_HINT_T0);
		_wq[0] = _wq[1] = _wq[2] = _wq[3] = _wq[4] = _wq[5] = _wq[6] = _wq[7] = _mm_xor_ps(_wq[0], _wq[0]);
...
static inline void UpdateIndex(vtx_idx* out, vtx_idx* in, u32 base)
{
	MMRM_PROFILE_FUNCTION();
	*out = *in + base;
	mmrm_assert(*out <= 0xffff);
}
static inline void UpdateIndices(vtx_idx* out, vtx_idx* in, u32 base, size_t count)
{
	MMRM_PROFILE_FUNCTION();
...
Vec3 SampleWind(const Vec3 pos, const Vec3 (&samples)[(MMRM_WIND_DIM)][(MMRM_WIND_DIM)][(MMRM_WIND_DIM)])
{
	...
	i32 d = MMRM_WIND_DIM, ix = static_cast<i32>(ftix), iy = static_cast<i32>(ftiy), iz = static_cast<i32>(ftiz);
...
static void MergeInstanceList(SMMRMInstanceContext& context)
{
	...
	DualQuatA * bones = spines && geom->numSpineVtx ? DrxStackAllocVector(DualQuatA, (geom->numSpineVtx + 1), DRX_PLATFORM_ALIGNMENT) : NULL;
	...
	mmrm_printf("updating %#x<==>%#x general \n", (u32)general, (u32)TLS_GET(u32, s_general_end));
	mmrm_printf("updating %#x<==>%#x tangents\n", (u32)tangents, (u32)TLS_GET(u32, s_tangents_end));
	mmrm_printf("updating %#x<==>%#x idxBuf\n", (u32)idxBuf, (u32)TLS_GET(u32, s_idx_end));
#endif
	PREFAST_SUPPRESS_WARNING(6255)
	Vec3A * npt = spines && geom->numSpineVtx ? DrxStackAllocVector(Vec3A, geom->numSpineVtx, DRX_PLATFORM_ALIGNMENT) : NULL;
	if (npt)
	{
	...
	PREFAST_SUPPRESS_WARNING(6255)
	Vec3A * opt = spines && geom->numSpineVtx ? DrxStackAllocVector(Vec3A, geom->numSpineVtx, DRX_PLATFORM_ALIGNMENT) : NULL;
	if (opt)
	{
		mmrm_printf("alloced %#x bytes\n", sizeof(Vec3A) * (geom->numSpineVtx));
	...
	PREFAST_SUPPRESS_WARNING(6255)
	Vec3A * nvel = spines && geom->numSpineVtx ? DrxStackAllocVector(Vec3A, geom->numSpineVtx, DRX_PLATFORM_ALIGNMENT) : NULL;
	if (nvel)
	{
		mmrm_printf("alloced %#x bytes\n", sizeof(Vec3A) * (geom->numSpineVtx));
	...
	PREFAST_SUPPRESS_WARNING(6255)
	SMMRMContact * contacts = geom->numSpineVtx ? DrxStackAllocVector(SMMRMContact, geom->numSpineVtx, DRX_PLATFORM_ALIGNMENT) : NULL;
	if (contacts) mmrm_printf("alloced %#x bytes\n", sizeof(SMMRMContact) * (geom->numSpineVtx));
	i32 i = 0, j = 0, l = 0, off = 0, boneIdx = 1, nLod, nspines = (i32)geom->numSpines;
	float i_f = 0.f;
	i32 base = 0, ncolliders = context.ncolliders, nprojectiles = context.nprojectiles;
	Matrix34A wmat, smat, iwmat, tmat;
	const float fExtents = c_MergedMeshesExtent;
	const aVec3 origin = context.min;
	i32 iter = 0, max_iter = context.max_iter, frame_id = update->frame_count;
	...
	for (size_t k = 0; k < context.amount; ++k)
	{
		SMMRMInstance& sample = context.samples[k];
		{
			...
			if (DinrusX3dEngBase::GetCVars()->e_MergedMeshesDebug) TraceWindSample(offs, dw, context);
#endif

			var = dw.len() * variance;
			dw += Vec3(drx_random(0.0f, var), drx_random(0.0f, var), drx_random(0.0f, var));
...
#endif
#if MMRM_USE_VECTORIZED_SSE_INSTRUCTIONS
		const vec4 vWX = _mm_load_ps((float*)((tuk)&wmat + sizeof(__m128) * 0));
		const vec4 vWY = _mm_load_ps((float*)((tuk)&wmat + sizeof(__m128) * 1));
		const vec4 vWZ = _mm_load_ps((float*)((tuk)&wmat + sizeof(__m128) * 2));
#endif
		...
		if (spines)
		{
			...
			for (j = 0, off = 0; j < nspines; ++j)
			{
				...
				for (i = 1; i < (i32)geomSpineInfo[j].nSpineVtx; ++i)
				{
#if MMRM_SIMULATION_USES_FP32
					opt[off + i] = spines[base + off + i].pt;
					nvel[off + i] = spines[base + off + i].vel;
#else
					i32 sp_x(spines[base + off + i].pt.x), sp_y(spines[base + off + i].pt.y), sp_z(spines[base + off + i].pt.z);
					i32 sv_x(spines[base + off + i].vel.x), sv_y(spines[base + off + i].vel.y), sv_z(spines[base + off + i].vel.z);
				...
				while (dtcur > MMRM_FIXED_STEP)
				{
					...
					for (i = 0, i_f = -1.f; i < (i32)geomSpineInfo[j].nSpineVtx; ++i, i_f += 1.f)
					{
#if MMRM_USE_VECTORIZED_SSE_INSTRUCTIONS
						const vec4 vopt0 = _mm_load_ps(&opt[off + i].x);
						const vec4 vopt1 = _mm_load_ps(&opt[off + i + (iszero(i) - 1)].x);
						const vec4 vopt2 = _mm_load_ps(&opt[off + i - (iszero(i - ((i32)geomSpineInfo[j].nSpineVtx - 1)) - 1)].x);
						...
						dir[1] = (opt[off + i - (iszero(i - ((i32)geomSpineInfo[j].nSpineVtx - 1)) - 1)] - opt[off + i]);
					...
					do
					{
						for (l = 0; l < (i32)geomSpineInfo[j].nSpineVtx - 1; ++l)
							contacts[l].i = -1;
						...
						// vertex was moved inside
						{
							...
							for (i = 0; i < (i32)geom->numSpineVtx; ++i)
	...
	#endif
						for (i = 0; i < (i32)geomSpineInfo[j].nSpineVtx - 1; ++i)
						{
	#if  MMRM_USE_VECTORIZED_SSE_INSTRUCTIONS
							...
							vLX = _mm_load_ps((float*)((tuk)&lmat + sizeof(__m128) * 0));
							vLY = _mm_load_ps((float*)((tuk)&lmat + sizeof(__m128) * 1));
							vLZ = _mm_load_ps((float*)((tuk)&lmat + sizeof(__m128) * 2));
						...
						for (i = (i32)geomSpineInfo[j].nSpineVtx - 2, i_f = (float)i; i > 0; --i, i_f -= 1.f)
						{
...
#endif
						for (i = 0, i_f = -1.f; i < (i32)geomSpineInfo[j].nSpineVtx - 1; ++i, i_f += 1.f)
						{
#if MMRM_USE_VECTORIZED_SSE_INSTRUCTIONS
							vnpt1 = _mm_load_ps(&npt[off + i + 1].x);
...
#else
				spines[base + off + 0].vel.y += static_cast<i16>(clamp_tpl(dtcur, 0.0f, MMRM_FIXED_STEP + (1.f / 1000.f)) * 1000.f);
	...
	#endif
					spines[base + off + i].vel.x = static_cast<i16>(sv_x);
					spines[base + off + i].vel.y = static_cast<i16>(sv_y);
					spines[base + off + i].vel.z = static_cast<i16>(sv_z);
					spines[base + off + i].pt.x = static_cast<i16>(sp_x);
					spines[base + off + i].pt.y = static_cast<i16>(sp_y);
					spines[base + off + i].pt.z = static_cast<i16>(sp_z);
#endif
				}
		...
		// Bake the geometry into the buffer
		{
			...
			for (size_t n = 0; n < geom->numChunks[nLod]; ++n)
			{
				...
				u32 siv = context.updateChunks[n].voff, & iv = context.updateChunks[n].voff;
				u32& ii = context.updateChunks[n].ioff;
...
static inline void MergeInstanceListDeform(SMMRMInstanceContext& context)
{
	...
	SMMRMProjectile* projectiles = context.projectiles;
	PREFAST_SUPPRESS_WARNING(6255)
	i32 i = 0, k = 0, off = 0, n = 0;
	i32 ncolliders = context.ncolliders, nprojectiles = context.nprojectiles;
	Vec3 ls0, ls1;
	...
	, rdt;
	i32 iter = 0, max_iter = update->group->physConfig.max_iter;
	dttot += update->dtscale;
	update->dtscale = 0.f;
	for (k = 0; k < (i32)context.amount && context.wind; ++k)
	{
		...
		while (dtcur > MMRM_FIXED_STEP)
		{
			...
			rdt = 1.f / dt;
			for (i = 0; i < (i32)deform->nvertices; ++i, i_f += i_step)
			{
			...
			do
			{
				for (i = 0; i < (i32)deform->nconstraints; ++i)
				{
				...
				for (i = 0; i < (i32)deform->nvertices; ++i)
				{
		...
		// Skip if not visible
		i32 nLod = sample.lastLod;
		if (nLod < 0)
			continue;

		// Bake the geometry into the buffer
		for (n = 0; n < (i32)geom->numChunks[nLod]; ++n)
		{
			const SMMRMChunk& chunk = geom->pChunks[nLod][n];
			u32 siv = context.updateChunks[n].voff, & iv = context.updateChunks[n].voff;
			u32& ii = context.updateChunks[n].ioff;
...
////////////////////////////////////////////////////////////////////////////////
void SMMRMGroupHeader::CullInstances(CCamera* cam, Vec3* origin, Vec3* rotationOrigin, float zRotation, i32 flags)
{
	MEMORY_SCOPE_CHECK_HEAP();
	MMRM_PROFILE_FUNCTION();
...
#if MMRM_USE_BOUNDS_CHECK
	mmrm_assert(DrxInterlockedIncrement(const_cast< i32*>(&header->debugLock)) == 1);
#endif
...
#if MMRM_USE_BOUNDS_CHECK
	mmrm_assert(DrxInterlockedDecrement(const_cast< i32*>(&header->debugLock)) == 0);
#endif
}
  ...
  , i32 flags)
{
	...
	DrxInterlockedDecrement(update->updateFlag);
...
struct SMergedMeshInstanceSorter
{
	...
	SMergedMeshInstanceSorter(u32 frameId)
		: m_frameId()
	{}
...
void CMergedMeshesUpr::SortActiveInstances_Async(const SRenderingPassInfo passInfo)
{
...
size_t CMergedMeshRenderNode::QueryDensity(const Vec3& pos, IMergedMeshesUpr::TFixedSurfacePtrTypeArray& surfaceTypes, IMergedMeshesUpr::TFixedDensityArray& density)
{
	size_t i = 0;
	...
	float ftix(floorf(px)), ftiy(floorf(py)), ftiz(floorf(pz));
	float fx = px - ftix, fy = py - ftiy, fz = pz - ftiz;
	i32 d = MMRM_DENSITY_DIM, ix = static_cast<i32>(ftix), iy = static_cast<i32>(ftiy), iz = static_cast<i32>(ftiz);
...
void CMergedMeshRenderNode::CalculateDensity()
{
	...
	if (m_density == NULL)
		m_density = (SSampleDensity*)DrxModuleMemalign(sizeof(SSampleDensity) * cube(MMRM_DENSITY_DIM), 16);
	...
	for (size_t i = 0; i < m_nGroups; ++i)
	{
		...
		StatInstGroup& instGroup = GetObjUpr()->m_lstStaticTypes[geom->srcGroupId];
		...
		for (size_t j = 0; j < numSamples; ++j)
		{
			...
			i32 iminx = (i32)(((vmin.x) * rExtents) * (MMRM_DENSITY_DIM - 1));
			i32 iminy = (i32)(((vmin.y) * rExtents) * (MMRM_DENSITY_DIM - 1));
			i32 iminz = (i32)(((vmin.z) * rExtents) * (MMRM_DENSITY_DIM - 1));

			i32 imaxx = (i32)(((vmax.x) * rExtents) * (MMRM_DENSITY_DIM - 1));
			i32 imaxy = (i32)(((vmax.y) * rExtents) * (MMRM_DENSITY_DIM - 1));
			i32 imaxz = (i32)(((vmax.z) * rExtents) * (MMRM_DENSITY_DIM - 1));
			...
			for (i32 ik = iminz; ik <= imaxz; ++ik)
				for (i32 ij = iminy; ij <= imaxy; ++ij)
					for (i32 ii = iminx; ii <= imaxx; ++ii)
					{
						u32 density = m_density[ii + ij * MMRM_DENSITY_DIM + ik * sqr(MMRM_DENSITY_DIM)].density_u8[si];
						density = min(density + 0x20u, 0xffu);
						m_density[ii + ij * MMRM_DENSITY_DIM + ik * sqr(MMRM_DENSITY_DIM)].density_u8[si] = static_cast<u8>(density);
					}
		}
	}
}

void CMergedMeshRenderNode::InitializeSamples(float fExtents, u8k* pBuffer)
{
