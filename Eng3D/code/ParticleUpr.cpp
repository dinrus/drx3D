...
IParticleUpr* CreateParticleUpr(bool bEnable)
{
	return DrxAlignedNew<CParticleUpr>(bEnable);
}

void DestroyParticleUpr(IParticleUpr* pParticleUpr)
{
	DrxAlignedDelete(static_cast<CParticleUpr*>(pParticleUpr));
}

//////////////////////////////////////////////////////////////////////////
// CParticleBatchDataUpr implementation

void CParticleBatchDataUpr::PrepareForRender(const SRenderingPassInfo& passInfo)
{
...
void CParticleBatchDataUpr::FinishParticleRenderTasks(const SRenderingPassInfo& passInfo)
{
	...
	i32 nJobStart = m_ParticlesJobStart[passInfo.ThreadID()][passInfo.GetRecursiveLevel()];
...
void CParticleBatchDataUpr::GetMemoryUsage(IDrxSizer* pSizer) const
{
	for (i32 t = 0; t < RT_COMMAND_BUF_COUNT; ++t)
		pSizer->AddObject(m_ParticlesToScene[t]);
}

void CParticleBatchDataUpr::ResetData()
{
	for (i32 t = 0; t < RT_COMMAND_BUF_COUNT; ++t)
		stl::free_container(m_ParticlesToScene[t]);

	for (i32 i = 0, end = m_UpdateParticleStates.size(); i < end; ++i)
...
void CParticleBatchDataUpr::SyncAllUpdateParticlesJobs()
{
	for (i32 i = 0; i < m_nUsedStates; ++i)
	{
		gEnv->GetJobUpr()->WaitForJob(*m_UpdateParticleStates[i]);
	}
	m_nUsedStates = 0;
}
...
//////////////////////////////////////////////////////////////////////////
CParticleUpr::CParticleUpr(bool bEnable)
{
	...
	i32& cvarCollisions = GetCVars()->e_ParticlesCollisions;
...
CParticleUpr::~CParticleUpr()
{
	if (Get3DEngine()->GetIVisAreaUpr())
	{
		Get3DEngine()->GetIVisAreaUpr()->RemoveListener(this);
	}
	Reset();
...
void CParticleUpr::CollectEffectStats(TEffectStats& mapEffectStats, size_t iSortField) const
{
...
void CParticleUpr::GetCounts(SParticleCounts& counts)
{
	counts = m_GlobalCounts;
}

void CParticleUpr::DisplayStats(Vec2& location, float lineHeight)
{
	m_pParticleSystem->DisplayStats(location, lineHeight);
}

void CParticleUpr::GetMemoryUsage(IDrxSizer* pSizer) const
{
	{
...
void CParticleUpr::PrintParticleMemory()
{
	// Get stats and mem usage for all effects.
	IDrxSizer* pSizer = GetSystem()->CreateSizer();
	...
	DrxLogAlways("Particle effects: %d loaded, %d enabled, %d active, %" PRISIZE_T " KB, %" PRISIZE_T " allocs",
	...
	DrxLogAlways("Particle heap: %d KB used, %d KB freed, %d KB unused",
	             i32(memParticle.nUsed >> 10), i32(memParticle.nPoolFree() >> 10), i32(memParticle.nNonPoolFree() >> 10));
	DrxLogAlways("Non heap: %d KB", i32(pSizer->GetTotalSize() - memParticle.nUsed) >> 10);

	pSizer->Release();
}

void CParticleUpr::Reset()
{
...
void CParticleUpr::FinishParticleRenderTasks(const SRenderingPassInfo& passInfo)
{
	m_pParticleSystem->FinishRenderTasks(passInfo);
	CParticleBatchDataUpr::FinishParticleRenderTasks(passInfo);
}

//////////////////////////////////////////////////////////////////////////
void CParticleUpr::ClearRenderResources(bool bForceClear)
{
...
//////////////////////////////////////////////////////////////////////////
void CParticleUpr::ClearDeferredReleaseResources()
{
...
//////////////////////////////////////////////////////////////////////////
void CParticleUpr::AddEventListener(IParticleEffectListener* pListener)
{
	assert(pListener);
...
void CParticleUpr::RemoveEventListener(IParticleEffectListener* pListener)
{
	assert(pListener);
...
void CParticleUpr::SetDefaultEffect(const IParticleEffect* pEffect)
{
...
const ParticleParams& CParticleUpr::GetDefaultParams(ParticleParams::EInheritance eInheritance, i32 nVersion) const
{
...
IParticleEffect* CParticleUpr::CreateEffect()
{
	if (!m_bEnabled)
		return NULL;
...
//////////////////////////////////////////////////////////////////////////
void CParticleUpr::RenameEffect(CParticleEffect* pEffect, cstr sNewName)
{
	assert(pEffect);
	pEffect->AddRef();
...
//////////////////////////////////////////////////////////////////////////
void CParticleUpr::DeleteEffect(IParticleEffect* pEffect)
{
	if (!pEffect)
		return;
...
//////////////////////////////////////////////////////////////////////////
CParticleEffect* CParticleUpr::FindLoadedEffect(cstr sEffectName)
{
	if (!m_bEnabled)
		return NULL;
	...
	// Find sub-effect
	if (tukk pDot = strchr(sEffectName, '.'))
	{
...
//////////////////////////////////////////////////////////////////////////
IParticleEffect* CParticleUpr::FindEffect(cstr sEffectName, cstr sSource, bool bLoad)
{
...
void CParticleUpr::LogEvents()
{
	...
	if (GetCVars()->e_ParticlesDebug & AlphaBits('ed'))
	{
		...
		else
		{
			...
			{
				static i32k nWidth = 50;
				i32 nSumStart = m_aEvents.size();
				float timeTotal = 0.f;

				// Add to summary entries at end.
				for (i32 e = 0; e < nSumStart; e++)
				{
				...
				if ((GetCVars()->e_ParticlesDebug & AlphaBit('e')) || timeTotal > m_timeThreshold)
				{
					...
					for (auto& pEventC : m_aEvents)
					{
						...
						for (i32 t = c; t < m_aEvents.size(); t++)
						{
							...
							cstr sThread = gEnv->pThreadUpr->GetThreadName(pEventT->nThread);
							...
							// Log event times.
							for (i32 e = t; e < m_aEvents.size(); e++)
							{
								SEventTiming* pEvent = &m_aEvents[e];
								if (!pEvent->sEvent)
									continue;
								...
								i32 start_iter = pEventC->nContainerId ? e : 0;
								i32 end_iter = pEventC->nContainerId ? e + 1 : nSumStart;
								float timeStart = m_aEvents[e].timeStart,
								      timeEnd = m_aEvents[e].timeEnd,
								      timeTotal = 0.f;
								for (i32 i = start_iter; i < end_iter; i++)
								{
									...
									i32 nEventStart = int_round((pEventI->timeStart - timeFrameStart) * nWidth / timeFrameTime);
									i32 nEventEnd = int_round((pEventI->timeEnd - timeFrameStart) * nWidth / timeFrameTime);
...
void CParticleUpr::OnFrameStart()
{
	...
	if (!m_bRegisteredListener && Get3DEngine()->GetIVisAreaUpr())
	{
		Get3DEngine()->GetIVisAreaUpr()->AddListener(this);
		m_bRegisteredListener = true;
	}
}

void CParticleUpr::Update()
{
	DRX_PROFILE_REGION(PROFILE_3DENGINE, "ParticleUpr Update");
	DRXPROFILE_SCOPE_PROFILE_MARKER("ParticleUpr Update");
	...
	if (m_bEnabled && GetCVars()->e_Particles)
	{
		...
		{
			DRX_PROFILE_REGION(PROFILE_PARTICLE, "SyncComputeVerticesJobs");
...
//////////////////////////////////////////////////////////////////////////
CParticleEmitter* CParticleUpr::CreateEmitter(const ParticleLoc& loc, const IParticleEffect* pEffect, const SpawnParams* pSpawnParams)
{
	PARTICLE_LIGHT_PROFILER();
	...
	uk pAlloc = m_Emitters.push_back_new();
	if (!pAlloc)
		return NULL;
...
IParticleEmitter* CParticleUpr::CreateEmitter(const ParticleLoc& loc, const ParticleParams& Params, const SpawnParams* pSpawnParams)
{
	PARTICLE_LIGHT_PROFILER();
	DRX_PROFILE_FUNCTION(PROFILE_PARTICLE);
...
//////////////////////////////////////////////////////////////////////////
void CParticleUpr::EraseEmitter(CParticleEmitter* pEmitter)
{
	if (pEmitter)
	{
		// Free resources.
...
void CParticleUpr::DeleteEmitter(IParticleEmitter* pEmitter)
{
	if (pEmitter)
		pEmitter->Kill();
}

void CParticleUpr::DeleteEmitters(FEmitterFilter filter)
{
	for (auto& e : m_Emitters)
	{
		if (filter(e))
...
void CParticleUpr::UpdateEmitters(IParticleEffect* pEffect)
{
...
void CParticleUpr::ReloadAllEffects()
{
...
bool CParticleUpr::CanAccessFiles(cstr sObject, cstr sSource) const
{
	if (!gEnv->pDrxPak->CheckFileAccessDisabled(0, 0))
		return true;
...
void CParticleUpr::UpdateEngineData()
{
	...
	if (GetRenderer())
	{
		...
		m_RenderFlags.SetState(i32(bParticleTesselation) - 1, FOB_ALLOW_TESSELLATION);
...
void CParticleUpr::OnVisAreaDeleted(IVisArea* pVisArea)
{
	for (auto& e : m_Emitters)
		e.OnVisAreaDeleted(pVisArea);
}

IParticleEmitter* CParticleUpr::SerializeEmitter(TSerialize ser, IParticleEmitter* pIEmitter)
{
...
//////////////////////////////////////////////////////////////////////////
bool CParticleUpr::LoadPreloadLibList(const cstr filename, const bool bLoadResources)
{
	if (!m_bEnabled)
		return false;
...
//////////////////////////////////////////////////////////////////////////
bool CParticleUpr::LoadLibrary(cstr sParticlesLibrary, cstr sParticlesLibraryFile, bool bLoadResources)
{
	if (!m_bEnabled)
		return false;
...
XmlNodeRef CParticleUpr::ReadLibrary(cstr sParticlesLibrary)
{
...
{
	for (i32 i = 0; i < nodeParent->getChildCount(); i++)
	{
...
XmlNodeRef CParticleUpr::ReadEffectNode(cstr sEffectName)
{
...
bool CParticleUpr::LoadLibrary(cstr sParticlesLibrary, XmlNodeRef& libNode, bool bLoadResources)
{
	if (!m_bEnabled)
		return false;
	...
	DRX_DEFINE_ASSET_SCOPE("ParticleLibrary", sParticlesLibrary);

	m_LoadedLibs[sParticlesLibrary] = libNode;

	// Load special defaults effects, if created (no warning otherwise).
	if (!m_pDefaultEffect && stricmp(sParticlesLibrary, "System") != 0 && gEnv->pDrxPak->IsFileExist(EFFECTS_SUBPATH "System.xml"))
		SetDefaultEffect("System.Default");

	// Load all nodes from the particle libaray.
	for (i32 i = 0, count = libNode->getChildCount(); i < count; i++)
	{
...
IParticleEffect* CParticleUpr::LoadEffect(cstr sEffectName, XmlNodeRef& effectNode, bool bLoadResources, const cstr sSource)
{
	if (!m_bEnabled)
		return NULL;
...
void CParticleUpr::ClearCachedLibraries()
{
	if (GetCVars()->e_ParticlesAllowRuntimeLoad)
		return;
...
void CParticleUpr::RenderDebugInfo()
{
...
i32 CParticleUpr::AddEventTiming(cstr sEvent, const CParticleContainer* pCont)
{
	if (!m_aEvents.size() && *sEvent != '*')
		return -1;

	SEventTiming* pEvent = m_aEvents.push_back();
	pEvent->nContainerId = (u32)pCont;
	pEvent->pEffect = pCont ? pCont->GetEffect() : 0;
	pEvent->nThread = DrxGetCurrentThreadId();
...
//////////////////////////////////////////////////////////////////////////
void CParticleUpr::Serialize(TSerialize ser)
{
...
void CParticleUpr::PostSerialize(bool bReading)
{
	if (bReading)
		ListEmitters("after load");
}

void CParticleUpr::ListEmitters(cstr sDesc, bool bForce)
{
	if (bForce || GetCVars()->e_ParticlesDebug & AlphaBit('l'))
	{
		...
		DrxLog("Emitters %s: time %.3f", sDesc, GetParticleTimer()->GetFrameStartTime().GetSeconds());
		for (const auto& e : m_Emitters)
		{
			if (e.IsActive())
				anEmitters[0]++;
				...
				anEmitters[2]++;
			DrxLog(" %s", e.GetDebugString('s').c_str());
		}
		DrxLog("Total: %d active, %d dormant, %d dead",
		       anEmitters[0], anEmitters[1], anEmitters[2]);
	}
}

void CParticleUpr::ListEffects()
{
...
void CParticleUpr::CreatePerfHUDWidget()
{
	if (!m_pWidget)
	{
		if (IDrxPerfHUD* pPerfHUD = gEnv->pSystem->GetPerfHUD())
		{
...
extern  i32 g_allocCounter;
#endif

void CParticleUpr::DumpAndResetVertexIndexPoolUsage()
{
#if defined(PARTICLE_COLLECT_VERT_IND_POOL_USAGE)
	...
	// See http://jira.drxengine.com/browse/CE-10725 for details on the bug.
	...
	if (bOutOfMemory || GetCVars()->e_ParticlesProfile == 1)
	{
		...
		m_pPartUpr->GetCounts(CurCounts);

		u32 nParticleVertexBufferSize = 0;
		u32 nParticleIndexBufferSize = 0;
		u32 nMaxParticleContainer = 0;
		...
		IRenderAuxText::Draw2dLabel(fTextSideOffset, fTopOffset, fTextSize, pColor, false, "\tAvail. %3u Used %3u Free %3u",
		                             nParticleIndexBufferSize / (uint)sizeof(u16), m_nRequieredIndexPoolMemory / (uint)sizeof(u16),
		                             m_nRequieredIndexPoolMemory >= nParticleIndexBufferSize ? 0 : (nParticleIndexBufferSize - m_nRequieredIndexPoolMemory) / (uint)sizeof(u16));
		fTopOffset += 18.0f;
		...
		IRenderAuxText::Draw2dLabel(fTextSideOffset, fTopOffset, fTextSize, pColor, false, "\tLimit    %3d", (i32)GetCVars()->e_ParticlesMaxScreenFill);
		fTopOffset += 18.0f;

		IRenderAuxText::Draw2dLabel(fTextSideOffset, fTopOffset, fTextSize, pColor, false, "\tProcessed %3d", (i32)(CurCounts.pixels.updated / fScreenPix));
		fTopOffset += 18.0f;

		IRenderAuxText::Draw2dLabel(fTextSideOffset, fTopOffset, fTextSize, pColor, false, "\tRendered  %3d", (i32)(CurCounts.pixels.rendered / fScreenPix));
		fTopOffset += 18.0f;
		fTopOffset += 18.0f;
		...
		for (i32 i = 0; i < nVertexIndexPoolUsageEntries; ++i)
		{
...
//PerfHUD
CParticleWidget::CParticleWidget(IMiniCtrl* pParentMenu, IDrxPerfHUD* pPerfHud, CParticleUpr* pPartUpr) : IDrxPerfHUDWidget(eWidget_Particles)
{
	m_pPartMgr = pPartUpr;
...
void CParticleWidget::Enable(i32 mode)
{
	...
	if (m_displayMode != newMode)
	{
		...
		default:
			DrxLogAlways("[Particle Stats] Attempting to set incorrect display mode set: %d", mode);
			break;
		}
	}
...
void CParticleWidget::Update()
{
	...
	CParticleUpr::TEffectStats mapEffectStats;
	...
	if (mapEffectStats[NULL].components.alive)
	{
		...
		for (auto& me : mapEffectStats)
		{
			...
			{
				m_pTable->AddData(1, col, "%d", (i32)counts.components.rendered);
				m_pTable->AddData(2, col, "%d", (i32)counts.components.updated);
				m_pTable->AddData(3, col, "%d", (i32)counts.components.alive);
			}
			else
			{
				m_pTable->AddData(1, col, "%d", (i32)counts.particles.rendered);
				m_pTable->AddData(2, col, "%d", (i32)counts.particles.updated);
				m_pTable->AddData(3, col, "%d", (i32)counts.particles.alive);
...
CParticleUpr::SEffectsKey::SEffectsKey(const cstr& sName)
{
	stack_string lowerName(sName);
	DrxStringUtils::toLowerInplace((tuk)lowerName.c_str());
	...
	pZLib->MD5Update(&context, (u8k*)lowerName.c_str(), lowerName.size());
	pZLib->MD5Final(&context, (u8*)c16);
}
