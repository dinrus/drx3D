...
i32 CFeatureCollision::GetRayTraceFilter() const
{
	i32k allowedCollisions = C3DEngine::GetCVars()->e_ParticlesCollisions & 3;
	i32 filter = 0;
...
static i32k kCollisionsFlags = sf_max_pierceable | (geom_colltype_ray | geom_colltype13) << rwi_colltype_bit | rwi_colltype_any | rwi_ignore_noncolliding | rwi_ignore_back_faces;
...
struct QuadPathT
{
	...
	float TimeFromDist(const Vec3& dir, float dist) const
	{
		...
		float t[2];
		for (i32 n = solve_quadratic(accNorm * 0.5f, vel0Norm, -dist, t); --n >= 0; )
		{
			if (t[n] >= 0.0f)
			{
...
bool CFeatureCollision::RayWorldIntersection(SContactPoint& contact, const Vec3& startIn, const Vec3& rayIn) const
{
	...
	if (m_objectFilter & ent_terrain_raytrace)
	{
		...
		if (DinrusX3dEngBase::GetTerrain()->RayTrace(start, start + ray, &rt, false))
		{
	...
	if (m_objectFilter & ~ent_terrain_raytrace)
	{
		...
		while (gEnv->pPhysicalWorld->RayWorldIntersection(start, ray, (m_objectFilter & ~ent_terrain_raytrace | ent_no_ondemand_activation), kCollisionsFlags, &rayHit, 1))
		{
			...
			else
			{
				...
				contact.m_pSurfaceType = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr()->GetSurfaceType(rayHit.surface_idx);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureCollision, "Motion", "Collisions", colorMotion);
...
DRX_PFX2_LEGACY_FEATURE(CFeatureGPUCollision, "Motion", "Collision");
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureGPUCollision, "GPU Particles", "Collision", colorGPU);
