...
DRXREGISTER_SINGLETON_CLASS(CParticleSystem)

CParticleSystem::CParticleSystem()
	: m_threadData(gEnv->pJobUpr->GetNumWorkerThreads() + 2) // 1 for main thread, 1 for each job thread, 1 for sum stats
{
...
void CParticleSystem::Update()
{
	...
	auto gpuMan = gEnv->pRenderer->GetGpuParticleUpr();
	gpuMan->BeginFrame();
	...
	{
		DRX_PFX2_ASSERT(data.memHeap.GetTotalMemory().nUsed == 0);  // some emitter leaked memory on mem stack
		if (&data != &sumData)
		{
	...
	for (auto& pEmitter : m_emitters)
	{
		...
		if (pEmitter->IsAlive())
		{
			...
			pEmitter->Update();
			m_jobUpr.AddUpdateEmitter(pEmitter);
		}
		else
		{
			pEmitter->Unregister();
	...
	m_jobUpr.ScheduleUpdates();
}

void CParticleSystem::SyncMainWithRender()
{
	if (ThreadMode() >= 1)
		m_jobUpr.SynchronizeUpdates();
...
{
	tukk titleLabel = "%-12s: Rendered /  Updated /    Alive /    Alloc";
...
void DisplayElementStats(Vec2& displayLocation, float lineHeight, cstr name, const TElementCounts<float>& stats, i32 prec = 0)
{
	if (!stats.IsZero())
	{
		tukk emittersLabel = " %-11s: %8.*f / %8.*f / %8.*f / %8.*f";
		gEnv->p3DEngine->DrawTextRightAligned(
			displayLocation.x, displayLocation.y,
			emittersLabel,
...
void CParticleSystem::ClearRenderResources()
{
	...
	{
		DRX_ASSERT_MESSAGE(pEmitter->Unique(), "Emitter %s not unique", pEmitter->GetCEffect()->GetShortName().c_str());
	}
...
IMaterial* CParticleSystem::GetFlareMaterial()
{
	if (!m_pFlareMaterial)
	{
		tukk flareMaterialName = "%ENGINE%/EngineAssets/Materials/lens_optics";
		m_pFlareMaterial = gEnv->p3DEngine->GetMaterialUpr()->FindMaterial(flareMaterialName);
	}
...
PParticleEffect CParticleSystem::LoadEffect(cstr effectName)
{
	...
	if (gEnv->pDrxPak->IsFileExist(effectName))
	{
...
void CParticleSystem::GetMemoryUsage(IDrxSizer* pSizer) const
{
}
