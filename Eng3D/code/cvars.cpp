...
//  Описание:    Консольные переменные, используемые в 3dengine.
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
#include <drx3D/Eng3D/IStatObj.h>  // MAX_STATOBJ_LODS_NUM
...
{
	gEnv->p3DEngine->GetTimeOfDay()->SetTime(DinrusX3dEngBase::GetCVars()->e_TimeOfDay);
}
...
void OnTimeOfDaySpeedVarChange(ICVar* pArgs)
{
	...
	advInfo.fAnimSpeed = DinrusX3dEngBase::GetCVars()->e_TimeOfDaySpeed;
...
void OnCGFStreamingChange(ICVar* pArgs)
{
	if (gEnv->IsEditor())
	{
		DinrusX3dEngBase::GetCVars()->e_StreamCgf = 0;
	}
}
...
void OnPerCharacterShadowsChange(ICVar* pArgs)
{
	...
	if (DinrusX3dEngBase::GetVisAreaUpr())
	{
		DinrusX3dEngBase::GetVisAreaUpr()->MarkAllSectorsAsUncompiled();
	}
}

void OnGsmLodsNumChange(ICVar* pArgs)
{
	if (DinrusX3dEngBase::GetRenderer())
	{
		DinrusX3dEngBase::GetRenderer()->UpdateCachedShadowsLodCount(pArgs->GetIVal());
}
}
...
void OnDynamicDistanceShadowsVarChange(ICVar* pArgs)
{
	...
	DinrusX3dEngBase::Get3DEngine()->SetRecomputeCachedShadows(ShadowMapFrustum::ShadowCacheData::eFullUpdate);
}

void OnVegetationVisibleChange(ICVar* pArgs)
{
	COctreeNode* pObjectsTree = DinrusX3dEngBase::Get3DEngine()->m_pObjectsTree;
...
void CVars::Init()
{
...
#if DRX_PLATFORM_WINDOWS && DRX_PLATFORM_64BIT // for big dense levels and for editor
	...
	i32 defaultMergedMeshesValue = 1;
	...
	REGISTER_CVAR(e_CheckOctreeObjectsBoxSize, 1, VF_NULL, "DrxWarning for crazy sized COctreeNode m_objectsBoxes");
