...
// -------------------------------------------------------------------------
//  Имя файла:   SceneTree.cpp
//  Created:     2012 by Vladimir Kajalin.
//  Описание: CPU side SVO
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
i32 CSvoEnv::ExportSvo(IDrxArchive* pArchive)
{
...
Vec3* GeneratePointsOnHalfSphere(i32 n);
...
bool CSvoEnv::Render()
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_3DENGINE, "CSvoEnv::Render_FindProbe");
		...
		for (i32 n = 0; n < arrObjects.Count(); n++)
		{
	...
	static i32 s_updateLightingPrev = GetCVars()->e_svoTI_UpdateLighting;
	...
	static i32 s_updateGeometryPrev = GetCVars()->e_svoTI_UpdateGeometry;
	...
	{
		pObjVox = DinrusX3dEngBase::GetIdentityCRenderObject((*CVoxelSegment::m_pCurrPassInfo));
		pObjVox->SetAmbientColor(DinrusX3dEngBase::Get3DEngine()->GetSkyColor(), (*CVoxelSegment::m_pCurrPassInfo));
	...
	{
		DRX_PROFILE_REGION(PROFILE_3DENGINE, "CSvoEnv::Traverse SVO");
		...
		if (arrVertsOut.Count() && GetCVars()->e_svoDVR == 10)
		{
			...
			static _smart_ptr<IMaterial> pMat = DinrusX3dEngBase::MakeSystemMaterialFromShader("Total_Illumination.RenderDvrProxiesPass_GS", &res);
			pObjVox->m_pCurrMaterial = pMat;

			DinrusX3dEngBase::GetRenderer()->EF_AddPolygonToScene(pMat->GetShaderItem(),
			                                                     arrVertsOut.Count(), &arrVertsOut[0], nullptr, pObjVox, (*CVoxelSegment::m_pCurrPassInfo), &dummyIndex, 1, false);
		}
	#endif
	}

	i32 maxLoadedNodes = DinrusX3dEngBase::GetCVars()->e_svoMaxBricksOnCPU;
	...
	{
		DRX_PROFILE_REGION(PROFILE_3DENGINE, "CSvoEnv::Render_StartStreaming");

		for (i32 treeLevel = 0; (treeLevel < SVO_STREAM_QUEUE_MAX_SIZE); treeLevel++)
		{
			for (i32 distId = 0; (distId < SVO_STREAM_QUEUE_MAX_SIZE); distId++)
			{
				if (!m_bFirst_StartStreaming)
					for (i32 i = 0; (i < m_arrForStreaming[treeLevel][distId].Count()) && (CVoxelSegment::m_streamingTasksInProgress < DinrusX3dEngBase::GetCVars()->e_svoMaxStreamRequests); i++)
					{
	...
	if (CVoxelSegment::m_arrLoadedSegments.Count() > (maxLoadedNodes - DinrusX3dEngBase::GetCVars()->e_svoMaxStreamRequests))
	{
		{
			DRX_PROFILE_REGION(PROFILE_3DENGINE, "CSvoEnv::Render_UnloadStreamable_Sort");
		...
		{
			DRX_PROFILE_REGION(PROFILE_3DENGINE, "CSvoEnv::Render_UnloadStreamable_FreeRenderData");

			i32 numNodesToDelete = 4 + DinrusX3dEngBase::GetCVars()->e_svoMaxStreamRequests;//CVoxelSegment::m_arrLoadedSegments.Count()/1000;
	...
	CVoxelSegment::m_maxBrickUpdates = max(1, bMultiUserMode ? DinrusX3dEngBase::GetCVars()->e_svoMaxBrickUpdates * 16 : (DinrusX3dEngBase::GetCVars()->e_svoMaxBrickUpdates / max(1, DinrusX3dEngBase::GetCVars()->e_svoTI_LowSpecMode / (i32)2 + 1)));
	if (!(m_streamingStartTime < 0)) // if not bSvoReady yet
		CVoxelSegment::m_maxBrickUpdates *= 100;

	//if(nUserId == 0 || !bMultiUserMode)
	{
		DRX_PROFILE_REGION(PROFILE_3DENGINE, "CSvoEnv::Render_BrickUpdate");

		CVoxelSegment::m_updatesInProgressTex = 0;
		CVoxelSegment::m_updatesInProgressBri = 0;

		for (i32 treeLevel = 0; (treeLevel < 16); treeLevel++)
		{
			for (i32 i = 0; (i < gSvoEnv->m_arrForBrickUpdate[treeLevel].Count()) && (CVoxelSegment::m_updatesInProgressBri < CVoxelSegment::m_maxBrickUpdates); i++)
			{
		...
		if (m_texNodePoolId)
		{
			DRX_PROFILE_REGION(PROFILE_3DENGINE, "CSvoEnv::Render_UpdateNodeRenderDataPtrs");
	...
	if (GetCVars()->e_svoDVR == 10)
	{
		/*
		   ...
		   static SInputShaderResources res;
		   static _smart_ptr< IMaterial > pMat = DinrusX3dEngBase::MakeSystemMaterialFromShader("Total_Illumination.BlendDvrIntoScene", &res);

		   i32 nRGBD=0, nNORM=0, nOPAC=0;
	...
	else if (GetCVars()->e_svoDVR > 1)
	{
		...
		static _smart_ptr<IMaterial> pMat = DinrusX3dEngBase::MakeSystemMaterialFromShader("Total_Illumination.SvoDebugDraw", &res);
		pObjVox->m_pCurrMaterial = pMat;
	...
	if (DinrusX3dEngBase::GetCVars()->e_svoDebug == 4 && 0)
	{
		if (!pKernelPoints)
		{
			...
			DinrusX3dEngBase::PrintMessage("Organizing subsets . . .");
			...
			for (i32 i = 0; i < 1000; i++)
			{
				...
				i32 m0 = GetWorstPointInSubSet(0, kRaysNum / 2);
				i32 m1 = GetWorstPointInSubSet(kRaysNum / 2, kRaysNum);
				std::swap(pKernelPoints[m0], pKernelPoints[m1]);
			}

			DinrusX3dEngBase::PrintMessage("Writing kernel.txt . . .");
			...
			if (f)
			{
				...
				fprintf(f, "{\n");
				for (i32 p = 0; p < kRaysNum; p++)
					fprintf(f, "  float3( %.6f , %.6f, %.6f ),\n", pKernelPoints[p].x, pKernelPoints[p].y, pKernelPoints[p].z);
				fprintf(f, "};\n");
				fclose(f);
			}

			DinrusX3dEngBase::PrintMessage("Done");
		}

		static Vec3 vPos = CVoxelSegment::m_voxCam.GetPosition();
		DinrusX3dEngBase::DrawSphere(vPos, 4.f);
		for (i32 i = 0; i < kRaysNum; i++)
			DinrusX3dEngBase::DrawSphere(vPos + pKernelPoints[i] * 4.f, .1f, (i < kRaysNum / 2) ? Col_Yellow : Col_Cyan);
	}
...
void CSvoEnv::ProcessSvoRootTeleport()
{
	...
	if (GetCVars()->e_svoDebug == 3 || GetCVars()->e_svoDebug == 6)
		DinrusX3dEngBase::Get3DEngine()->DrawBBox(m_pSvoRoot->m_nodeBox, Col_Yellow);
	...
	for (i32 axisId = 0; axisId < 3; axisId++)
	{
		float scrollDir = 0;
		float threshold = 2.f;
...
CSvoNode* CSvoNode::FindNodeByPosition(const Vec3& vPosWS, i32 treeLevelToFind, i32 treeLevelCur)
{
...
AABB CSvoNode::GetChildBBox(i32 childId)
{
	i32 x = (childId / 4);
	i32 y = (childId - x * 4) / 2;
...
bool CSvoNode::CheckReadyForRendering(i32 treeLevel, PodArray<CVoxelSegment*> arrForStreaming[SVO_STREAM_QUEUE_MAX_SIZE][SVO_STREAM_QUEUE_MAX_SIZE])
{
	bool bAllReady = true;
	...
	if (m_pSeg)
	{
		...
		if (pCloud->m_eStreamingStatus == ecss_NotLoaded)
		{
			...
			i32 distanceId = min(SVO_STREAM_QUEUE_MAX_SIZE - 1, (i32)(m_nodeBox.GetDistance(CVoxelSegment::m_voxCam.GetPosition()) / boxSize));
			i32 treeLevelId = min(SVO_STREAM_QUEUE_MAX_SIZE - 1, treeLevel);
			if (arrForStreaming[treeLevelId][distanceId].Count() < DinrusX3dEngBase::GetCVars()->e_svoMaxStreamRequests)
...
CSvoNode* CSvoNode::FindNodeByPoolAffset(i32 allocatedAtlasOffset)
{
...
void CSvoNode::RegisterMovement(const AABB& objBox)
{
	if (Overlap::AABB_AABB(objBox, m_nodeBox))
	{
		...
		if (m_pSeg &&
		    ...
		    m_pSeg->GetBoxSize() <= DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize)
		{
...
Vec3i ComputeDataCoord(i32 atlasOffset)
{
...
void CSvoNode::Render(PodArray<struct SPvsItem>* pSortedPVS, uint64 nodeKey, i32 treeLevel, PodArray<SVF_P3F_C4B_T2F>& arrVertsOut, PodArray<CVoxelSegment*> arrForStreaming[SVO_STREAM_QUEUE_MAX_SIZE][SVO_STREAM_QUEUE_MAX_SIZE])
{
	float boxSize = m_nodeBox.GetSize().x;
	...
	const float boxSizeRated = boxSize * DinrusX3dEngBase::GetCVars()->e_svoTI_VoxelizationLODRatio;
	...
	if (DinrusX3dEngBase::GetCVars()->e_svoTI_Active && m_pSeg && m_pSeg->m_eStreamingStatus == ecss_Ready && m_pSeg->m_pBlockInfo)
		if (m_pSeg->m_dwChildTrisTest || (m_pSeg->GetBoxSize() > DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize) || (DinrusX3dEngBase::GetCVars()->e_svoTI_Troposphere_Subdivide))
		{
	...
	if ((!m_ppChilds || (nodeToCamDist > boxSizeRated) || (boxSize <= DinrusX3dEngBase::GetCVars()->e_svoMinNodeSize))
	...
	{
		if (DinrusX3dEngBase::GetCVars()->e_svoDVR)
		{
			// check one child per frame
			i32 childId = GetCurrPassMainFrameID() % 8;
	...
	if (bDrawThisNode)
	{
		...
		if (m_pSeg && DinrusX3dEngBase::GetCVars()->e_svoDebug == 2)
			if (CVoxelSegment::m_voxCam.IsAABBVisible_E(m_nodeBox) && m_nodeBox.GetSize().z <= DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize)
			{
				//        if(fBoxSize <= DinrusX3dEngBase::GetCVars()->e_svoMinNodeSize)
				//          DinrusX3dEngBase::Get3DEngine()->DrawBBox(m_nodeBox);

				DinrusX3dEngBase::Get3DEngine()->DrawBBox(m_nodeBox,
				                                         ColorF(
				                                           (m_nodeBox.GetSize().z == DinrusX3dEngBase::GetCVars()->e_svoMinNodeSize),
				                                           ((m_nodeBox.GetSize().z - DinrusX3dEngBase::GetCVars()->e_svoMinNodeSize) / (DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize - DinrusX3dEngBase::GetCVars()->e_svoMinNodeSize)),
				                                           (m_nodeBox.GetSize().z == DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize),
				                                           1));
			}
	}
	else if (m_ppChilds)
...
Vec3* GeneratePointsOnHalfSphere(i32 n)
{
	...
	i32 i, j;
	i32 counter = 0, countmax = n * 1024 * 4;
	i32 minp1 = 0, minp2 = 0;
	float d = 0, mind = 0;
	static Vec3 p[1000];
	...
	for (i = 0; i < n; i++)
	{
		p[i].x = float(drx_random(-500, 500));
		p[i].y = float(drx_random(-500, 500));
		p[i].z = fabs(float(drx_random(-500, 500)));
...
CSvoEnv::CSvoEnv(const AABB& worldBox)
{
	...
	if (GetCVars()->e_svoTI_Active)
		GetCVars()->e_svoVoxelPoolResolution = min((i32)256, GetCVars()->e_svoVoxelPoolResolution);

	CVoxelSegment::m_voxTexPoolDimXY = min(GetCVars()->e_svoVoxelPoolResolution, (i32)512);
	CVoxelSegment::m_voxTexPoolDimZ = min(GetCVars()->e_svoVoxelPoolResolution, (i32)1024);
	CVoxelSegment::m_voxTexPoolDimZ = max(CVoxelSegment::m_voxTexPoolDimZ, (i32)256);
...
void CSvoNode::CheckAllocateSegment(i32 lodId)
{
	if (!m_pSeg)
	{
		i32 segmentId = CVoxelSegment::m_nextSegmentId;
...
u32 CSvoNode::SaveNode(PodArray<byte>& rS, u32& nodesCounterRec, IDrxArchive* pArchive, u32& totalSizeCounter)
{
	assert(m_pSeg);
		...
		for (i32 childId = 0; childId < 8; childId++)
		{
			if (m_ppChilds && m_ppChilds[childId])
			{
				...
				if (pArchive->FindFile(szFileName) == nullptr)
				{
					...
					u32 nodesCounter = 0;
					float startTime = DinrusX3dEngBase::GetTimer()->GetAsyncCurTime();
					...
					terrainBox.Expand(-Vec3(DinrusX3dEngBase::GetCVars()->e_svoTI_VoxelizationMapBorder + 1.f, DinrusX3dEngBase::GetCVars()->e_svoTI_VoxelizationMapBorder + 1.f, 0));
					...
					if (nodesCounter)
					{
						...
						pArchive->UpdateFile(szFileName, arrSvoData.GetElements(), arrSvoData.GetDataSize(), IDrxArchive::METHOD_STORE);
						totalSizeCounter += arrSvoData.GetDataSize();

						i32 areaGridDim = i32(DinrusX3dEngBase::Get3DEngine()->GetTerrainSize() / (SVO_ROOTLESS_PARENT_SIZE / 2));
						DinrusX3dEngBase::PrintMessage("Exported segment %s, Progress = %d %%", szFileName, i32(100.f * float(CVoxelSegment::m_exportVisitedAreasCounter) / float(areaGridDim * areaGridDim * areaGridDim)));

						DinrusX3dEngBase::PrintMessage("%d KB stored in %.1f sec (%d nodes, total size = %.1f MB)",
						                              arrSvoData.Count() / 1024, DinrusX3dEngBase::GetTimer()->GetAsyncCurTime() - startTime,
	...
	i32 boxSize = (i32)m_nodeBox.GetSize().z;
	if (boxSize == DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize)
	{
		DinrusX3dEngBase::PrintMessage("Voxelizing SVO area %04d_%04d_%04d_%04d", vKey.x, vKey.y, vKey.z, boxSize);
	}
	#endif

	u32 nNodeDataSize = rS.Count();
	...
	{
		DinrusX3dEngBase::PrintMessage("SVO nodes processed: %d K", CVoxelSegment::m_exportVisitedNodesCounter / 1000);

		if (DinrusX3dEngBase::IsEscapePressed())
		{
			if (!CVoxelSegment::m_bExportAbortRequested)
			{
				DinrusX3dEngBase::PrintMessage("Aborting SVO export process with the ESC Key");
			}
	...
	if (m_ppChilds && !CVoxelSegment::m_bExportAbortRequested)
	{
		...
		for (i32 childId = 0; childId < 8; childId++)
		{
			if (m_ppChilds && m_ppChilds[childId])
			{
				((u32*)&rS[childOffsetsPos])[childId * 2 + 0] = rS.Count();  // file offset

				i32 childNodeDataSize = m_ppChilds[childId]->SaveNode(rS, nodesCounterRec, pArchive, totalSizeCounter);

				((u32*)&rS[childOffsetsPos])[childId * 2 + 1] = childNodeDataSize;  // file data size
			}
		}

		nNodeDataSize += 8 * sizeof(u32) * 2;
	}

	#ifdef SVO_DEBUG
	if (boxSize == DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize)
	{
		DinrusX3dEngBase::PrintMessage("%d tris voxelized", m_pSeg->m_pTrisInArea ? m_pSeg->m_pTrisInArea->Count() : 0);
	}
	#endif
...
void CSvoNode::MakeNodeFilePath(tuk szFileName)
{
	Vec3_tpl<u16> vPos = (Vec3_tpl<u16> )m_nodeBox.GetCenter();

	u16 nBoxSize = (u16)m_nodeBox.GetSize().z;

	tukk pLevelPath = DinrusX3dEngBase::Get3DEngine()->GetLevelFilePath(COMPILED_SVO_FOLDER_NAME);

	sprintf(szFileName, "%s%04d_%04d_%04d_%04d.drxsvo", pLevelPath, vPos.x, vPos.y, vPos.z, nBoxSize);
}

CVoxelSegment* CSvoNode::AllocateSegment(i32 segmentId, i32 stationId, i32 lodId, EFileStreamingStatus eStreamingStatus, bool bDroppedOnDisk)
{
...
Vec3i CSvoNode::GetStatGeomCheckSumm()
{
	...
	if (nodeSize > DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize)
		return vCheckSumm;
	...
	DinrusX3dEngBase::Get3DEngine()->GetObjectsByTypeGlobal(lstObjects, eERType_Brush, &boxEx);
	DinrusX3dEngBase::Get3DEngine()->GetVisAreaUpr()->GetObjectsByType(lstObjects, eERType_Brush, &boxEx);
	...
	for (i32 i = 0; i < lstObjects.Count(); i++)
	{
		...
		if (pStatObj && pStatObj->m_eStreamingStatus == ecss_Ready)
		{
			...
			vCheckSumm.x += u16(((uint64)pNode->GetMaterial()) / 64);
		}
	}
	...
	for (i32 v = 0;; v++)
	{
		CVisArea* pVisArea = (CVisArea*)DinrusX3dEngBase::GetVisAreaUpr()->GetVisAreaById(v);
		if (!pVisArea)
			break;
		...
		for (i32 childId = 0; childId < 8; childId++)
		{
			...
			if (pChildNode && pChildNode->m_pSeg)
			{
				...
				if (pChildNode->m_pSeg->CheckUpdateBrickRenderData(true))
				{
					...
					i32 allChildsNum = 0;
					i32 readyChildsNum = 0;

					if (pChildNode->m_ppChilds)
					{
						for (i32 subChildId = 0; subChildId < 8; subChildId++)
						{
...
void CSvoNode::GetTrisInAreaStats(i32& trisCount, i32& vertCount, i32& trisBytes, i32& vertBytes, i32& maxVertPerArea, i32& matsCount)
{
...
void CSvoNode::GetVoxSegMemUsage(i32& allocated)
{
	if (m_pSeg)
	{
		allocated += m_pSeg->m_nodeTrisAllMerged.capacity() * sizeof(i32);
...
bool CPointTreeNode::GetAllPointsInTheBox(const AABB& testBox, const AABB& nodeBox, PodArray<i32>& arrIds)
{
	if (m_pPoints)
	{
		// leaf
		for (i32 s = 0; s < m_pPoints->Count(); s++)
		{
...
bool CPointTreeNode::TryInsertPoint(i32 pointId, const Vec3& vPos, const AABB& nodeBox, i32 recursionLevel)
{
	if (recursionLevel >= 7)
	{
		if (!m_pPoints)
...
AABB CPointTreeNode::GetChildBBox(i32 childId, const AABB& nodeBox)
{
	i32 x = (childId / 4);
	i32 y = (childId - x * 4) / 2;
	i32 z = (childId - x * 4 - y * 2);
...
bool CSvoEnv::GetSvoStaticTextures(I3DEngine::SSvoStaticTexInfo& svoInfo, PodArray<I3DEngine::SLightTI>* pLightsTI_S, PodArray<I3DEngine::SLightTI>* pLightsTI_D)
{
	...
	{
		if (IVisArea* pCurVisArea = GetVisAreaUpr()->GetCurVisArea())
		{
			PodArray<IVisArea*> visitedAreas;
			pCurVisArea->FindSurroundingVisArea(2, true, &visitedAreas, 32);

			i32 portalCounter = 0;
			for (i32 areaID = 0; areaID < visitedAreas.Count(); areaID++)
			{
	...
	for (i32 stage = 0; stage < 2; stage++)
	{
		...
		{
			u32 bytesToCopy = min((u32)sizeof(svoInfo.arrAnalyticalOccluders[stage]), m_analyticalOccluders[stage].GetDataSize());
...
void CSvoEnv::GetSvoBricksForUpdate(PodArray<I3DEngine::SSvoNodeInfo>& arrNodeInfo, float nodeSize, PodArray<SVF_P3F_C4B_T2F>* pVertsOut)
{
	...
	if (nodeSize == 0)
	{
		...
		static i32 e_svoTI_Troposphere_Active_Max = 0;
		...
		//		fCheckVal += i32(Get3DEngine()->GetSunDir().x*10);
		//	fCheckVal += i32(Get3DEngine()->GetSunDir().y*10);
		//fCheckVal += i32(Get3DEngine()->GetSunDir().z*10);
		...
		{
			static i32 s_prevFlagAsync = GetCVars()->e_svoTI_UpdateLighting;
		...
		{
			static i32 s_prevFlagAsync = (gSvoEnv->m_svoFreezeTime <= 0);
		...
		{
			i32 autoUpdateVoxNum = 0;

			i32 maxVox = GetCVars()->e_svoTI_Reflect_Vox_Max * (i32)30 / (i32)max(30.f, gEnv->pTimer->GetFrameRate());
			i32 maxVoxEdit = GetCVars()->e_svoTI_Reflect_Vox_MaxEdit * (i32)30 / (i32)max(30.f, gEnv->pTimer->GetFrameRate());
			...
			for (uint n = 0; n < numBlocks; n++)
			{
				...
				if (SBlockMinMax* pBlock = CVoxelSegment::m_pBlockPacker->GetBlockInfo(s_autoBlockId))
				{
					...
					if ((pSeg->GetBoxSize() >= nodeSize && pSeg->GetBoxSize() <= GetCVars()->e_svoMaxNodeSize) || !nodeSize)
						...
						{
							if (autoUpdateVoxNum && (autoUpdateVoxNum + (i32(sqrt((float)pSeg->m_solidVoxelsNum)) + GetCVars()->e_svoTI_Reflect_Vox_Max_Overhead) > maxVox))
							{
								if (bEditing)
								{
									if (pSeg->m_bStatLightsChanged)
										...
										{
											if (autoUpdateVoxNum && (autoUpdateVoxNum + (i32(sqrt((float)pSeg->m_solidVoxelsNum)) + GetCVars()->e_svoTI_Reflect_Vox_Max_Overhead) > maxVoxEdit))
											{
											}
											else
											{
												pSeg->m_pNode->m_requestSegmentUpdateFrametId = max(pSeg->m_pNode->m_requestSegmentUpdateFrametId, GetCurrPassMainFrameID());
												autoUpdateVoxNum += (i32(sqrt((float)pSeg->m_solidVoxelsNum)) + GetCVars()->e_svoTI_Reflect_Vox_Max_Overhead);
											}
										}
								}
								else
...
void CSvoEnv::DetectMovement_StatLights()
{
	...
	for (uint blockId = 0; blockId < numBlocks; blockId++)
	{
		if (SBlockMinMax* pBlock = CVoxelSegment::m_pBlockPacker->GetBlockInfo(blockId))
		{
			...
			boxEx.Expand(Vec3(border, border, border));
			DinrusX3dEngBase::Get3DEngine()->GetObjectsByTypeGlobal(lstObjects, eERType_Light, &boxEx);
			DinrusX3dEngBase::Get3DEngine()->GetVisAreaUpr()->GetObjectsByType(lstObjects, eERType_Light, &boxEx);
				...
				if (pNode->GetGIMode() == IRenderNode::eGM_StaticVoxelization)
				{
					if (pNode->GetRenderNodeType() == eERType_Light)
					{
						...
						if ((m_light.m_Flags & DLF_PROJECT) && (m_light.m_fLightFrustumAngle < 90.f) && m_light.m_pLightImage)
						{
							...
							vCheckSumm.x += i32(m_light.m_fLightFrustumAngle);
						...
						{
							//							vCheckSumm.x += i32(Get3DEngine()->GetSunDir().x*50);
							//						vCheckSumm.y += i32(Get3DEngine()->GetSunDir().y*50);
							//					vCheckSumm.z += i32(Get3DEngine()->GetSunDir().z*50);
							continue;
						}
					}
			...
			{
				vCheckSumm.x += (i32)GetCVars()->e_svoVoxNodeRatio;
				vCheckSumm.y += (i32)GetCVars()->e_svoVoxDistRatio;
			}
...
void CSvoEnv::StartupStreamingTimeTest(bool bDone)
{
	...
	{
		m_streamingStartTime = DinrusX3dEngBase::GetTimer()->GetAsyncCurTime();
	}
	else if (m_streamingStartTime > 0 && bDone && m_bStreamingDonePrev)
	{
		//		float time = DinrusX3dEngBase::GetTimer()->GetAsyncCurTime();
		...
		i32 maxBricksOnGPU = SVO_ATLAS_DIM_BRICKS_XY * SVO_ATLAS_DIM_BRICKS_XY * SVO_ATLAS_DIM_BRICKS_Z;
...
i32 CSvoEnv::GetWorstPointInSubSet(i32k start, i32k end)
{
	i32 p0 = -1;
...
{
	return CSvoUpr::GetSvoStaticTextures(svoInfo, pLightsTI_S, pLightsTI_D);
}

void C3DEngine::GetSvoBricksForUpdate(PodArray<SSvoNodeInfo>& arrNodeInfo, float nodeSize, PodArray<SVF_P3F_C4B_T2F>* pVertsOut)
{
	CSvoUpr::GetSvoBricksForUpdate(arrNodeInfo, nodeSize, pVertsOut);
}
...
i32 C3DEngine::GetSvoCompiledData(IDrxArchive* pArchive)
{
	return CSvoUpr::ExportSvo(pArchive);
}

void C3DEngine::LoadTISettings(XmlNodeRef pInputNode)
{
	tukk szXmlNodeName = "Total_Illumination_v2";

	// Total illumination
	if (GetCVars()->e_svoTI_Active >= 0)
		GetCVars()->e_svoTI_Apply = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "Active", "0"));

	GetCVars()->e_svoVoxelPoolResolution = 64;
	i32 voxelPoolResolution = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "VoxelPoolResolution", "0"));
	...
	GetCVars()->e_svoTI_VoxelizationMapBorder = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "VoxelizationMapBorder", "0"));
	...
	GetCVars()->e_svoTI_NumberOfBounces = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "NumberOfBounces", "0"));
	...
	if (gEnv->IsEditor())
	{
		GetCVars()->e_svoTI_UpdateLighting = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "UpdateLighting", "0"));
		GetCVars()->e_svoTI_UpdateGeometry = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "UpdateGeometry", "0"));
	}
	GetCVars()->e_svoTI_SkyColorMultiplier = (float)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "SkyColorMultiplier", "0"));
	GetCVars()->e_svoTI_UseLightProbes = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "UseLightProbes", "0"));
	...
	GetCVars()->e_svoTI_SpecularFromDiffuse = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "SpecularFromDiffuse", "0"));

	GetCVars()->e_svoMinNodeSize = (float)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "MinNodeSize", "0"));

	GetCVars()->e_svoTI_SkipNonGILights = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "SkipNonGILights", "0"));
	GetCVars()->e_svoTI_ForceGIForAllLights = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "ForceGIForAllLights", "0"));
	...
	GetCVars()->e_svoTI_SunRSMInject = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "SunRSMInject", "0"));
	GetCVars()->e_svoTI_SSDepthTrace = (float)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "SSDepthTrace", "0"));

	GetCVars()->e_svoTI_ShadowsFromSun = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "ShadowsFromSun", "0"));
	GetCVars()->e_svoTI_ShadowsSoftness = (float)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "ShadowsSoftness", "0"));
	GetCVars()->e_svoTI_ShadowsFromHeightmap = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "ShadowsFromHeightmap", "0"));
	GetCVars()->e_svoTI_Troposphere_Active = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "Troposphere_Active", "0"));
	...
	//GetCVars()->e_svoTI_Troposphere_Subdivide = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "Troposphere_Subdivide", "0"));
	GetCVars()->e_svoTI_Troposphere_Subdivide = GetCVars()->e_svoTI_Troposphere_Active;

	GetCVars()->e_svoTI_AnalyticalOccluders = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "AnalyticalOccluders", "0"));
	GetCVars()->e_svoTI_AnalyticalGI = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "AnalyticalGI", "0"));
	GetCVars()->e_svoTI_TraceVoxels = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "TraceVoxels", "1"));

	i32 lowSpecMode = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "LowSpecMode", "0"));
	if (lowSpecMode > -2 && gEnv->IsEditor()) // otherwise we use value from sys_spec_Light.cfg
		GetCVars()->e_svoTI_LowSpecMode = lowSpecMode;

	GetCVars()->e_svoTI_HalfresKernelPrimary = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "HalfresKernelPrimary", "0"));
	GetCVars()->e_svoTI_HalfresKernelSecondary = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "HalfresKernelSecondary", "0"));
	...
	GetCVars()->e_svoTI_IntegrationMode = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "IntegrationMode", "0"));
	#else
	...
	if (DinrusX3dEngBase::GetCVars()->e_svoStreamVoxels != 2)
		GetCVars()->e_svoStreamVoxels = (i32)atof(GetXMLAttribText(pInputNode, szXmlNodeName, "StreamVoxels", "0"));
	...
	if (CSvoNode::IsStreamingActive())
	{
		char szFileName[256];
		tukk pLevelPath = DinrusX3dEngBase::Get3DEngine()->GetLevelFilePath(COMPILED_SVO_FOLDER_NAME);
		sprintf(szFileName, "%s*.drxsvo", pLevelPath);

		_finddata_t fd;
		if (gEnv->pDrxPak->FindFirst(szFileName, &fd) < 0)
		{
...
{
	#define CVAR_CPP
	#include <drx3D/Eng3D/SceneTreeCVars.inl>
...
static i32 SLightTI_Compare(ukk v1, ukk v2)
{
...
void CSvoEnv::CollectLights()
{
	...
	{
		if (i32 objCount = gEnv->p3DEngine->GetObjectsByTypeInBox(eERType_Light, nodeBox, (IRenderNode**)0))
		{
			...
			for (i32 n = 0; n < objCount; n++)
			{
				...
				if (eVoxMode == IRenderNode::eGM_DynamicVoxelization || eVoxMode == IRenderNode::eGM_StaticVoxelization)
				{
					...
					{
						if ((pRN->GetDrawFrame(0) > 10) && (pRN->GetDrawFrame(0) >= (i32)GetCurrPassMainFrameID()))
						{
...
static i32 SAnalyticalOccluder_Compare(ukk v1, ukk v2)
{
...
void CSvoEnv::CollectAnalyticalOccluders()
{
	...
	for (i32 stageId = 0; stageId < 2; stageId++)
	...
	if (i32 objCount = gEnv->p3DEngine->GetObjectsByTypeInBox(eERType_MovableBrush, areaBox, (IRenderNode**)0))
	{
		PodArray<IRenderNode*> arrObjects(objCount, objCount);
	...
	if (i32 objCount = gEnv->p3DEngine->GetObjectsByTypeInBox(eERType_Character, areaBox, (IRenderNode**)0))
	{
		PodArray<IRenderNode*> arrObjects(objCount, objCount);
	...
	for (i32 stageId = 0; stageId < 2; stageId++)
	{
		...
		for (i32 n = 0; n < m_analyticalOccluders[stageId].Count(); n++)
		{
	...
	if (GetCVars()->e_svoTI_AnalyticalOccluders)
	{
		if (ICharacterInstance* pCharacter = (ICharacterInstance*)pRN->GetEntityCharacter(&matParent))
		{
			...
			if (ISkeletonPose* pSkeletonPose = pCharacter->GetISkeletonPose())
			{
				...
				for (i32 capsuleId = 0; capsuleId < capsuleInfos.size(); capsuleId++)
				{
					const SBoneShadowCapsule& capsuleInfo = capsuleInfos[capsuleId];

					Vec3 arrBonePositions[2];

					for (i32 jointSlot = 0; jointSlot < 2; jointSlot++)
					{
						i32 jointID = capsuleInfo.arrJoints[jointSlot];
...
{
	return (!gEnv->IsEditor() && DinrusX3dEngBase::GetCVars()->e_svoStreamVoxels) || DinrusX3dEngBase::GetCVars()->e_svoStreamVoxels == 2;
}
...
void CSvoNode::CheckAllocateChilds()
{
	...
	if (m_nodeBox.GetSize().x <= DinrusX3dEngBase::GetCVars()->e_svoMinNodeSize)
		return;

	i32k currPassMainFrameID = GetCurrPassMainFrameID();
	...
	for (i32 childId = 0; childId < 8; childId++)
	{
		...
		if (!IsStreamingActive() && DinrusX3dEngBase::GetCVars()->e_svoTI_VoxelizationPostpone && (!m_ppChilds || !m_ppChilds[childId]) && (childBox.GetSize().z == DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize))
		{
			...
			if (!CVoxelSegment::CheckCollectObjectsForVoxelization(childBox, nullptr, bThisIsAreaParent, bThisIsLowLodNode, true))
			{
				...
				if (DinrusX3dEngBase::GetCVars()->e_svoDebug == 7)
					DinrusX3dEngBase::Get3DEngine()->DrawBBox(childBox, Col_Lime);
		...
		//					(m_pCloud->GetBoxSize() > DinrusX3dEngBase::GetCVars()->e_svoMaxNodeSize) || (DinrusX3dEngBase::GetCVars()->e_rsDebugAI&2) || 1)
		{
			if (!m_ppChilds)
			{
...

			{
				...
				if (IsStreamingActive() && m_nodeBox.GetSize().x <= SVO_ROOTLESS_PARENT_SIZE)
				{
					if (m_ppChilds[childId]->m_nodeBox.GetSize().x == SVO_ROOTLESS_PARENT_SIZE / 2)
					{
						...
						if (gEnv->pDrxPak->IsFileExist(szAreaFileName))
						{
							//
							m_ppChilds[childId]->m_pSeg->m_fileStreamOffset64 = 0;

							i32k parentNodeFileStreamSize = 132;
							//assert(parentNodeFileStreamSize == (sizeof(i32) + sizeof(u32) * 2 * 8));
...
uk CSvoNode::operator new(size_t)
{
	return gSvoEnv->m_nodeAllocator.GetNewElement();
}

void CSvoNode::operator delete(uk ptr)
{
