...
// -------------------------------------------------------------------------
//  Имя файла:   Vegetation.cpp
//  Version:     v1.00
//  Created:     28/5/2001 by Vladimir Kajalin
//  Компиляторы:       drux (DinrusPro's Universal Compiler),   mingw-w64-clang-x86_64
//  Описание:   Загрузка деревьев, строений, регистрация/отрегистрация
//              сущностей для отображения.
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
float DRX_ALIGN(128) CVegetation::g_scBoxDecomprTable[256];
...
//////////////////////////////////////////////////////////////////////////
void CVegetation::CalcMatrix(Matrix34A& tm, i32* pFObjFlags)
{
...
CLodValue CVegetation::ComputeLod(i32 wantedLod, const SRenderingPassInfo& passInfo)
{
	u8 nDissolveRefA = 0;
	i32 nLodA = -1;
	i32 nLodB = -1;
	...
	if (CStatObj* pStatObj = vegetGroup.GetStatObj())
	{
		...
		if (passInfo.IsGeneralPass() && passInfo.IsZoomActive())
		{
			...
			wantedLod = CObjUpr::GetObjectLOD(this, fEntDistance);
...
void CVegetation::Render(const SRenderingPassInfo& passInfo, const CLodValue& lodValue, SSectorTextureSet* pTerrainTexInfo) const
{
	...
	if (GetObjUpr()->AddOrCreatePersistentRenderObject(pTempData, pRenderObject, &lodValue, objMat, passInfo))
	{
	...
	if (u8 nMaterialLayers = GetMaterialLayers())
	{
		u8 nFrozenLayer = (nMaterialLayers & MTL_LAYER_FROZEN) ? MTL_LAYER_FROZEN_MASK : 0;
		u8 nWetLayer = (nMaterialLayers & MTL_LAYER_WET) ? MTL_LAYER_WET_MASK : 0;
		pRenderObject->m_nMaterialLayers = (u32) (nFrozenLayer << 24) | (nWetLayer << 16);
	}
	else
	...
	pRenderObject->m_nRenderQuality = (u16)(fRenderQuality * 65535.0f);
	...
	{
		GetObjUpr()->FillTerrainTexInfo(m_pOcNode, fEntDistance, pTerrainTexInfo, GetBBox());
	}
	...
	if (pTerrainTexInfo)
		if (pRenderObject->m_ObjFlags & (FOB_BLEND_WITH_TERRAIN_COLOR))
		{
			...
			pRenderObject->m_nTextureID = -(i32)pTerrainTexInfo->nSlot0 - 1; // nTextureID is set only for proper batching, actual texture id is same for all terrain sectors
	...
	if (pFoliage && ((CStatObjFoliage*)pFoliage)->m_pVegInst == this)
	{
		...
		if (pOD)
		{
			...
			pFoliage->SetFlags(pFoliage->GetFlags() & ~IFoliage::FLAG_FROZEN | -(i32)(pRenderObject->m_nMaterialLayers & MTL_LAYER_FROZEN) & IFoliage::FLAG_FROZEN);
		}
	}
	...
	if (CObjUpr::IsAfterWater(vObjCenter, vCamPos, passInfo, Get3DEngine()->GetWaterLevel()))
	...
	if (lodValue.LodA() > pStatObj->GetMaxUsableLod() && vegetGroup.bUseSprites && GetCVars()->e_VegetationBillboards)
	{
		...
		if (pStatObj->GetBillboardMaterial())
		{
			...
			GetObjUpr()->GetBillboardRenderMesh(pRenderObject->m_pCurrMaterial)->Render(pRenderObject, passInfo);
		}
	}
	else
	{
	...
	if (GetCVars()->e_BBoxes)
		GetObjUpr()->RenderObjectDebugInfo((IRenderNode*)this, pRenderObject->m_fDistance, passInfo);
}
...
bool CVegetation::PhysicalizeFoliage(bool bPhysicalize, i32 iSource, i32 nSlot)
{
...
{
	// delete old physics
	if (m_pPhysEnt && GetSystem()->GetIPhysicalWorld()->DestroyPhysicalEntity(m_pPhysEnt, 4 * (i32)bKeepIfReferenced))
	{
		m_pPhysEnt = 0;
...
void CVegetation::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
tukk CVegetation::GetName() const
{
	return (GetStatObjGroupSize() && GetStatObj()) ?
	       GetStatObj()->GetFilePath() : "StatObjNotSet";
}
//////////////////////////////////////////////////////////////////////////
IRenderMesh* CVegetation::GetRenderMesh(i32 nLod)
{
...
void CVegetation::OnRenderNodeBecomeVisibleAsync(SRenderNodeTempData* pTempData, const SRenderingPassInfo& passInfo)
{
	...
	userData.nWantedLod = CObjUpr::GetObjectLOD(this, fEntDistance);
}
...
void CVegetation::UpdateSpriteInfo(SVegetationSpriteInfo& si, float fSpriteAmount, SSectorTextureSet* pTerrainTexInfo, const SRenderingPassInfo& passInfo) const
{
	...
	i32 dwAngle = ((i32)(fAngle * (255.0f / (2.0f * gf_PI)))) & 0xff;
...
IFoliage* CVegetation::GetFoliage(i32 nSlot)
{
...
IPhysicalEntity* CVegetation::GetBranchPhys(i32 idx, i32 nSlot)
{
	IFoliage* pFoliage = GetFoliage();
	return pFoliage && (u32)idx < (u32)((CStatObjFoliage*)pFoliage)->m_nRopes ? ((CStatObjFoliage*)pFoliage)->m_pRopes[idx] : 0;
}

void CVegetation::SetStatObjGroupIndex(i32 nVegetationGroupIndex)
{
...
void CVegetation::UpdateRndFlags()
{
	...
	IRenderNode::SetLodRatio((i32)(vegetGroup.fLodDistRatio * 100.f));
	IRenderNode::SetViewDistRatio((i32)(vegetGroup.fMaxViewDistRatio * 100.f));
}
...
IStatObj* CVegetation::GetEntityStatObj(u32 nSubPartId, Matrix34A* pMatrix, bool bReturnOnlyVisible)
{
	if (pMatrix)
	{
		if (const auto pTempData = m_pTempData.load())
