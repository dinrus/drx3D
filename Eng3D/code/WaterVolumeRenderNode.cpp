...
void CWaterVolumeRenderNode::CreateArea(uint64 volumeID, const Vec3* pVertices, u32 numVertices, const Vec2& surfUVScale, const Plane& fogPlane, bool keepSerializationParams)
{
	...
	// copy  creation params to be able to serialize water volume if needed (only in editor)
	...
	for (u32 i = 0; i < pTessVertList.size(); ++i)
	{
	...
	u32 nIterations = 4;
	for (u32 i = 0; i < nIterations; ++i)
	{
		u32 nIndices = m_waterSurfaceIndices.size();
		for (u32 t = 0; t < nIndices; t += 3)
		{
			// Get triangle, compute median edge vertex, insert to vertex list
			u16 id_a = m_waterSurfaceIndices[t + 0];
			u16 id_b = m_waterSurfaceIndices[t + 1];
			u16 id_c = m_waterSurfaceIndices[t + 2];
			...
			u16 id_d = (u16) pVertsTemp.size() - 1;
			...
			u16 id_e = (u16) pVertsTemp.size() - 1;
			...
			u16 id_f = (u16) pVertsTemp.size() - 1;
		...
		memcpy(&m_waterSurfaceIndices[0], &pIndicesTemp[0], sizeof(u16) * pIndicesTemp.size());
...
void CWaterVolumeRenderNode::CreateRiver(uint64 volumeID, const Vec3* pVertices, u32 numVertices, float uTexCoordBegin, float uTexCoordEnd, const Vec2& surfUVScale, const Plane& fogPlane, bool keepSerializationParams)
{
...
void CWaterVolumeRenderNode::SetAreaPhysicsArea(const Vec3* pVertices, u32 numVertices, bool keepSerializationParams)
{
	const bool serializeWith3DEngine = keepSerializationParams && !IsAttachedToEntity();
...
void CWaterVolumeRenderNode::SetRiverPhysicsArea(const Vec3* pVertices, u32 numVertices, bool keepSerializationParams)
{
...
IPhysicalEntity* CWaterVolumeRenderNode::SetAndCreatePhysicsArea(const Vec3* pVertices, u32 numVertices)
{
...
tukk CWaterVolumeRenderNode::GetEntityClassName() const
{
	return "WaterVolume";
}

tukk CWaterVolumeRenderNode::GetName() const
{
	return "WaterVolume";
}
...
{
	DRX_ASSERT_MESSAGE(!IsAttachedToEntity(), "FIXME: Don't currently support transforming attached water volumes");
...
void CWaterVolumeRenderNode::Render_JobEntry(SRendParams rParam, SRenderingPassInfo passInfo)
{
	...
	if (GetCVars()->e_Fog)
	{
		...
		{
			DRX_ASSERT(!(m_pVolumeRE[fillThreadID]->m_drawWaterSurface));
	...
	// submit surface
	{
		DRX_ASSERT(m_pSurfaceRE[fillThreadID]->m_drawWaterSurface);
...
void CWaterVolumeRenderNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
const SWaterVolumeSerialize* CWaterVolumeRenderNode::GetSerializationParams()
{
	...
	// before returning, copy non- serialization params
...
void CWaterVolumeRenderNode::CopyVolatilePhysicsAreaContourSerParams(const Vec3* pVertices, u32 numVertices)
{
	if (!m_pSerParams)
...
void CWaterVolumeRenderNode::CopyVolatileRiverSerParams(const Vec3* pVertices, u32 numVertices, float uTexCoordBegin, float uTexCoordEnd, const Vec2& surfUVScale)
{
	if (!m_pSerParams)
		m_pSerParams = new SWaterVolumeSerialize;
...
void CWaterVolumeRenderNode::CopyVolatileAreaSerParams(const Vec3* pVertices, u32 numVertices, const Vec2& surfUVScale)
{
	if (!m_pSerParams)
		m_pSerParams = new SWaterVolumeSerialize;
...
i32 OnWaterUpdate(const EventPhysAreaChange* pEvent)
{
...
IPhysicalEntity* CWaterVolumeRenderNode::CreatePhysicsAreaFromSettings()
{
	...
	GetPhysicalWorld()->AddEventClient(EventPhysAreaChange::id, (i32 (*)(const EventPhys*))OnWaterUpdate, 1);
...
void CWaterVolumeRenderNode::OffsetPosition(const Vec3& delta)
{
	...
	m_center += delta;
	m_WSBBox.Move(delta);
	for (i32 i = 0; i < (i32)m_waterSurfaceVertices.size(); ++i)
