...
// -------------------------------------------------------------------------
//  Имя файла:   MatMan.cpp
//  Version:     v1.00
//  Created:     28/5/2001 by Vladimir Kajalin
//  Компиляторы:       drux (DinrusPro's Universal Compiler),   mingw-w64-clang-x86_64
//  Описание:    Реализация управления материалом.
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
#if !defined(_RELEASE)
static tukk szReplaceMe = "%ENGINE%/EngineAssets/TextureMsg/ReplaceMe.tif";
static tukk szGeomNotBreakable = "%ENGINE%/EngineAssets/TextureMsg/GeomNotBreakable.tif";
#else
static tukk szReplaceMe = "%ENGINE%/EngineAssets/TextureMsg/ReplaceMeRelease.tif";
static tukk szGeomNotBreakable = "%ENGINE%/EngineAssets/TextureMsg/ReplaceMeRelease.tif";
#endif
...
CMatMan::CMatMan()
{
	...
	m_pSurfaceTypeUpr = new CSurfaceTypeUpr();
...
CMatMan::~CMatMan()
{
	delete m_pSurfaceTypeUpr;
	i32 nNotUsed = 0, nNotUsedParents = 0;
	m_pDefaultMtl = NULL;
	m_pDefaultTerrainLayersMtl = NULL;
	m_pDefaultLayersMtl = NULL;
	m_pDefaultHelperMtl = NULL;
...
//////////////////////////////////////////////////////////////////////////
tukk CMatMan::UnifyName(tukk sMtlName)
{
	static char name[260];
	i32 n = strlen(sMtlName);
	...
	for (i32 i = 0; i < n && i < sizeof(name); i++)
	{
		name[i] = DrxStringUtils::toLowerAscii(sMtlName[i]);
		if (sMtlName[i] == '.')
		{
			n = i;
			break;
...
//////////////////////////////////////////////////////////////////////////
IMaterial* CMatMan::CreateMaterial(tukk sMtlName, i32 nMtlFlags)
{
...
void CMatMan::DelayedMaterialDeletion()
{
	...
	u32 nID = (m_nDelayedDeleteID + 1) % MATERIAL_DELETION_DELAY;
...
{
	DRX_ASSERT(m_AccessLock.IsLocked());
...
//////////////////////////////////////////////////////////////////////////
void CMatMan::RenameMaterial(IMaterial* pMtl, tukk sNewName)
{
	AUTO_LOCK(m_AccessLock);

	assert(pMtl);
	tukk sName = pMtl->GetName();
	if (*sName != '\0')
	{
...
//////////////////////////////////////////////////////////////////////////
IMaterial* CMatMan::FindMaterial(tukk sMtlName) const
{
	AUTO_LOCK(m_AccessLock);

	tukk name = UnifyName(sMtlName);
...
//////////////////////////////////////////////////////////////////////////
IMaterial* CMatMan::LoadMaterial(tukk sMtlName, bool bMakeIfNotFound, bool bNonremovable, u64 nLoadingFlags)
{
	DRX_ASSERT(!m_AccessLock.IsLocked());
	...
	DRX_DEFINE_ASSET_SCOPE("Material", sMtlName);
	...
	if (!pMtl)
	{
		...
		static i32 nRecursionCounter = 0;
...
//////////////////////////////////////////////////////////////////////////
IMaterial* CMatMan::MakeMaterialFromXml(tukk sMtlName, tukk sMtlFilename, XmlNodeRef node, bool bForcePureChild, u16 sortPrio, IMaterial* pExistingMtl, u64 nLoadingFlags, IMaterial* pParentMtl)
{
	CMatMan* pMatMan = static_cast<CMatMan*>(gEnv->p3DEngine->GetMaterialUpr());

	i32 mtlFlags = 0;
	DrxFixedStringT<128> shaderName;
	...
	if (!(mtlFlags & MTL_FLAG_MULTI_SUBMTL))
	{
		...
		if (!(mtlFlags & MTL_64BIT_SHADERGENMASK))
		{
			...
			nShaderGenMask = GetRenderer() ?
			                 GetRenderer()->EF_GetRemapedShaderMaskGen((tukk) shaderName, nShaderGenMask) : 0;
		...
		if (GetRenderer())
		{
			...
			{
				tukk pszShaderGenMask = node->getAttr("StringGenMask");
				nShaderGenMask = GetRenderer()->EF_GetShaderGlobalMaskGenFromString((tukk)shaderName, pszShaderGenMask, nShaderGenMask);  // get common mask gen
			}
			else
			{
				// version doens't has string gen mask yet ? Remap flags if needed
				nShaderGenMask = GetRenderer()->EF_GetRemapedShaderMaskGen((tukk)shaderName, nShaderGenMask, ((mtlFlags & MTL_64BIT_SHADERGENMASK) != 0));
			}
		}
		mtlFlags |= MTL_64BIT_SHADERGENMASK;

		tukk surfaceType = node->getAttr("SurfaceType");
		...
		if (sr)
		{
			...
			for (EEfResTextures texId = EFTT_DIFFUSE; texId < EFTT_MAX; texId = EEfResTextures(texId + 1))
			{
				...
				tukk name = sr->m_Textures[texId].m_Name;
				if (name && (strchr(name, ':') != NULL))
				{
					DrxLog("Invalid texture '%s' found in material '%s'", name, sMtlName);
				}
			}
		}
	...
	if (pLinkName)
	{
		tukk szLinkName = pLinkName->getAttr("name");
	...
	if (pMtl && pMtl->GetShaderItem().m_pShader && pMtl->GetShaderItem().m_pShaderResources)
	{
		...
		if (pMtlLayersNode)
		{
			i32 nLayerCount = min((i32) MTL_LAYER_MAX_SLOTS, (i32) pMtlLayersNode->getChildCount());
			if (nLayerCount)
			{
				u8 nMaterialLayerFlags = 0;

				pMtl->SetLayerCount(nLayerCount);
				for (i32 l(0); l < nLayerCount; ++l)
				{
					XmlNodeRef pLayerNode = pMtlLayersNode->getChild(l);
					if (pLayerNode)
					{
						if (tukk pszShaderName = pLayerNode->getAttr("Name"))
						{
							bool bNoDraw = false;
...
//////////////////////////////////////////////////////////////////////////
bool CMatMan::LoadMaterialShader(IMaterial* pMtl, IMaterial* pParentMtl, tukk sShader, uint64 nShaderGenMask, SInputShaderResources& sr, XmlNodeRef& publicsNode, u64 nLoadingFlags)
{
...
bool CMatMan::LoadMaterialLayerSlot(u32 nSlot, IMaterial* pMtl, tukk szShaderName, SInputShaderResources& pBaseResources, XmlNodeRef& pPublicsNode, u8 nLayerFlags)
{
	...
	u32 nMaskGenBase = (u32)pShaderItemBase.m_pShader->GetGenerationMask();
...
static void shGetVector4(tukk buf, float v[4])
{
	if (!buf)
		return;
	i32 res = sscanf(buf, "%f,%f,%f,%f", &v[0], &v[1], &v[2], &v[3]);
	assert(res);
}
...
void CMatMan::ParsePublicParams(SInputShaderResources& sr, XmlNodeRef paramsNode)
{
	...
	i32 nA = paramsNode->getNumAttributes();
	if (!nA)
		return;

	for (i32 i = 0; i < nA; i++)
	{
		tukk key = NULL, * val = NULL;
		paramsNode->getAttributeByIndex(i, &key, &val);
		SShaderParam Param;
		assert(val && key);
		drx_strcpy(Param.m_Name, key);
...
//////////////////////////////////////////////////////////////////////////
ISurfaceType* CMatMan::GetSurfaceTypeByName(tukk sSurfaceTypeName, tukk sWhy)
{
	return m_pSurfaceTypeUpr->GetSurfaceTypeByName(sSurfaceTypeName, sWhy);
};

//////////////////////////////////////////////////////////////////////////
i32 CMatMan::GetSurfaceTypeIdByName(tukk sSurfaceTypeName, tukk sWhy)
{
	ISurfaceType* pSurfaceType = m_pSurfaceTypeUpr->GetSurfaceTypeByName(sSurfaceTypeName, sWhy);
	if (pSurfaceType)
...
//////////////////////////////////////////////////////////////////////////
void CMatMan::GetLoadedMaterials(IMaterial** pData, u32& nObjCount) const
{
	AUTO_LOCK(m_AccessLock);
...
//////////////////////////////////////////////////////////////////////////
void CMatMan::SetAltMaterialSuffix(tukk pSuffix)
{
	AUTO_LOCK(m_AccessLock);
...
//////////////////////////////////////////////////////////////////////////
IMaterial* CMatMan::CloneMaterial(IMaterial* pSrcMtl, i32 nSubMtl)
{
	if (pSrcMtl->GetFlags() & MTL_FLAG_MULTI_SUBMTL)
	{
		...
		i32 nSubMtls = pSrcMtl->GetSubMtlCount();
...
//////////////////////////////////////////////////////////////////////////
IMaterial* CMatMan::CloneMultiMaterial(IMaterial* pSrcMtl, tukk sSubMtlName)
{
	if (pSrcMtl->GetFlags() & MTL_FLAG_MULTI_SUBMTL)
	{
		IMaterial* pMultiMat = new CMatInfo;
...
void CMatMan::InitDefaults()
{
	...
	{
		m_pSurfaceTypeUpr->LoadSurfaceTypes();
	}

	auto CreateDefaultMaterialWithShader = [](tukk mat_name, tukk shader_name)
	{
	...
	if (!m_pDefaultMtl)
	{
		...
		DrxLogAlways("Initializing default materials...");
...
//////////////////////////////////////////////////////////////////////////
IMaterial* CMatMan::LoadCGFMaterial(tukk szMaterialName, tukk szCgfFilename, u64 nLoadingFlags)
{
	FUNCTION_PROFILER_3DENGINE;

	DrxPathString sMtlName = szMaterialName;
...
//////////////////////////////////////////////////////////////////////////
IMaterial* CMatMan::LoadMaterialFromXml(tukk sMtlName, XmlNodeRef mtlNode)
{
...
void CMatMan::PreloadLevelMaterials()
{
	...
	//bool bMtlfCacheExist = GetISystem()->GetIResourceUpr()->LoadLevelCachePak( MTL_LEVEL_CACHE_PAK,"" );
	//if (!bMtlfCacheExist)
	//return;
	...
	DrxPathString mtlName;
	DrxPathString mtlFilename;
	DrxPathString mtlCacheFilename;
	for (tukk sName = pResList->GetFirst(); sName != NULL; sName = pResList->GetNext())
	{
	...
	//GetISystem()->GetIResourceUpr()->UnloadLevelCachePak( MTL_LEVEL_CACHE_PAK );
...
void CMatMan::PreloadDecalMaterials()
{
	...
	// Wildcards load.
	DrxPathString sPath = PathUtil::Make(DrxPathString(MATERIAL_DECALS_FOLDER), DrxPathString(MATERIAL_DECALS_SEARCH_WILDCARD));
	...
	for (i32 i = 0, num = (i32)mtlFiles.size(); i < num; i++)
	{
		DrxPathString sMtlName = mtlFiles[i];
		PathUtil::RemoveExtension(sMtlName);

		if (bVerboseLogging)
		{
			DrxLog("Preloading Decal Material: %s", sMtlName.c_str());
		}
...
void CMatMan::FreeAllMaterials()
{
...
#ifndef _RELEASE
	{
		...
		if (nObjCount > 0)
		{
			...
			Warning("CObjUpr::CheckMaterialLeaks: %d materials(s) found in memory", nObjCount);
			for (u32 i = 0; i < nObjCount; i++)
			{
...
void CMatMan::GetMemoryUsage(IDrxSizer* pSizer) const
{
	...
	pSizer->AddObject(m_pSurfaceTypeUpr);
