// Разработка 2015-2018 DinrusPro / Dinrus Group. РНЦП Динрус.
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLocationOffset, "Location", "Offset", colorLocation);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLocationBox, "Location", "Box", colorLocation);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLocationSphere, "Location", "Sphere", colorLocation);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLocationCircle, "Location", "Circle", colorLocation);
...
public:
	...
	virtual void GetSpatialExtents(const CParticleComponentRuntime& runtime, TConstArray<float> scales, TVarArray<float> extents) override
	{
		...
		for (uint i = 0; i < numInstances; ++i)
		{
			...
			for (i32 dim = (i32)m_location; dim > 0; --dim)
				extent *= scales[i];
			extents[i] += extent;
		}
	}
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLocationGeometry, "Location", "Geometry", colorLocation);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLocationNoise, "Location", "Noise", colorLocation);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLocationBeam, "Location", "Beam", colorLocation);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLocationBindToCamera, "Location", "BindToCamera", colorLocation);
...
void WrapSector(Vec3& pos, const Vec3& posPrev, Vec2 scrWidth)
{
	...
	i32 n = solve_quadratic(dd, dp * 2.0f, pp - 1.0f, dists);
	while (--n >= 0)
	{
...
ILINE i32 WrapRotation(Vec3& pos, Vec3& posPrev, const Matrix33& camRot, Vec2 scrWidth)
{
...
DRX_UNIT_TEST(WrapRotationTest)
{
	SChaosKey chaosKey(0u);
	for (i32 i = 0; i < 100; ++i)
	{
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLocationOmni, "Location", "Omni", colorLocation);
