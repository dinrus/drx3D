...
i32 COctreeNode::m_nInstStreamTasksInProgress = 0;
FILE* COctreeNode::m_pFileForSyncRead = 0;
i32 COctreeNode::m_nNodesCounterAll = 0;
i32 COctreeNode::m_nNodesCounterStreamable = 0;
i32 COctreeNode::m_nInstCounterLoaded = 0;
...
COctreeNode::~COctreeNode()
{
	...
	GetObjUpr()->m_arrStreamingNodeStack.Delete(this);
...
void COctreeNode::CheckManageVegetationSprites(float fNodeDistance, i32 nMaxFrames, const SRenderingPassInfo& passInfo)
{
	u32k iMainFrameId = passInfo.GetMainFrameID();
	if ((u32)m_nManageVegetationsFrameId >= iMainFrameId - nMaxFrames && !passInfo.IsZoomInProgress())
		return;
	...
	if (GetCVars()->e_VegetationUseTerrainColor)
		GetObjUpr()->FillTerrainTexInfo(this, m_fNodeDistance, pTerrainTexInfo, m_objectsBox);
	/*
	   Vec3 vD = GetBBox().GetCenter() - vCamPos;
	   Vec3 vCenter = GetBBox().GetCenter();
	   Vec3 vD1 = vCenter + vD;
	   m_dwSpritesAngle = ((u32)(atan2_tpl(vD1.x, vD1.y)*(255.0f/(2*g_PI)))) & 0xff;
	 */
	...
	for (CVegetation* pObj = (CVegetation*)m_arrObjects[eRNListType_Vegetation].m_pFirstNode, * pNext; pObj; pObj = pNext)
	{
		...
		if (pObj->m_pSpriteInfo)
		{
			...
			if (fEntDistance2D < (fSpriteSwitchDist - fSwitchRange) && pTempData)
			{
				...
				nLodA = CLAMP(pTempData->userData.nWantedLod, (u32)pStatObj->GetMinUsableLod(), (u32)pStatObj->m_nMaxUsableLod);
...
void COctreeNode::Render_Object_Nodes(bool bNodeCompletelyInFrustum, i32 nRenderMask, const Vec3& vAmbColor, u32 passCullMask, const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	const bool bPushIntoOcclusionCuller = Get3DEngine()->IsStatObjBufferRenderTasksAllowed() && passInfo.IsGeneralPass() && JobUpr::InvokeAsJob("CheckOcclusion");
	...
	for (i32 l = 0; l < eRNListType_ListsNum; l++)
	{
	...
	if (GetCVars()->e_VegetationSpritesBatching && GetCVars()->e_VegetationSprites)
		CheckManageVegetationSprites(nodeDistance, (i32)(nodeDistance * 0.2f), passInfo);
	...
	if (HasAnyRenderableCandidates(passInfo))
	{
		...
		if (bPushIntoOcclusionCuller)
		{
			...
			GetObjUpr()->PushIntoCullQueue(SCheckOcclusionJobData::CreateOctreeJobData(this, nRenderMask, vAmbColor, passCullMask, passInfo));
		}
		else
		{
...
{
	if (GetCVars()->e_ObjectsTreeBBoxes == 3 || GetCVars()->e_ObjectsTreeBBoxes == (i32)GetNodeBox().GetSize().x)
	{
		const AABB& nodeBox = GetNodeBox();
...
void COctreeNode::GetStreamedInNodesNum(i32& nAllStreamable, i32& nReady)
{
...
void COctreeNode::UpdateStaticInstancing()
{
	...
	for (IRenderNode* pObj = m_arrObjects[eRNListType_Vegetation].m_pFirstNode, * pNext; pObj; pObj = pNext)
	{
		...
		/*
		   ...
		   i32 wantedLod = CObjUpr::GetObjectLOD(pInst, fEntDistance);

		   i32 minUsableLod = pStatObj->GetMinUsableLod();
		   i32 maxUsableLod = (i32)pStatObj->m_nMaxUsableLod;
...
bool IsSphereInsideHull(const SPlaneObject* pHullPlanes, i32 nPlanesNum, const Sphere& objSphere);

// single function for all possible culling types, used for octree nodes and for objects
u32 COctreeNode::UpdateCullMask(u32 onePassTraversalFrameId, u32 onePassTraversalShadowCascades, const IRenderNode::RenderFlagsType renderFlags, const SRenderingPassInfo& passInfo, const AABB& nodeBox, const float nodeDistance, const float nodeMaxViewDistance, const bool bTestCoverageBuffer,
                                   bool& bCompletelyInMainFrustum, OcclusionTestClient* occlusionTestClient, u32 passCullMask)
{
	...
	if (passCullMask & kPassCullMainMask)
	{
		...
		else
		{
			...
			if (occlusionTestClient && GetObjUpr()->IsBoxOccluded(nodeBox, nodeDistance, occlusionTestClient, false, eoot_OCCELL, passInfo))
			{
				passCullMask &= ~kPassCullMainMask;
			}
			// check against coverage buffer in main view
			else if (bTestCoverageBuffer && !GetObjUpr()->CheckOcclusion_TestAABB(nodeBox, nodeDistance))
			{
				passCullMask &= ~kPassCullMainMask;
			}
		}
	...
	DRX_ASSERT(!shadowPasses || shadowPasses->size() <= kMaxShadowPassesNum);
	...
	if (passCullMask & ~kPassCullMainMask)
	{
		...
		else
		{
			...
			for (u32 n = 0, passId = n + 1; n < (u32)shadowPasses->size(); n++, passId++)
			{
				...
				if (!bSkipCaster && pFr->IntersectAABB(nodeBox, &bCompletellyInShadowFrustum, shadowPass.ShadowFrustumSide()))
				{
					...
					{
						while ((n + 1) < (u32)shadowPasses->size())
						{
...
void COctreeNode::Render_LightSources(bool bNodeCompletelyInFrustum, const SRenderingPassInfo& passInfo)
{
	...
	u32 passCullMask = UpdateCullMask(m_onePassTraversalFrameId, ~0, m_renderFlags, passInfo, m_objectsBox, nodeDistance, m_fObjectsMaxViewDist, false, bNodeCompletelyInFrustum, &m_occlusionTestClient, kPassCullMainMask);
	...
	if (bNodeCompletelyInFrustum || passInfo.GetCamera().IsAABBVisible_EH(m_objectsBox, &bNodeCompletelyInFrustum))
	{
		for (IRenderNode* pObj = m_arrObjects[eRNListType_Unknown].m_pFirstNode; pObj; pObj = pObj->m_pNext)
		{
			...
			if (rnType == eERType_Light)
			{
				...
				u32 objCullMask = UpdateCullMask(pObj->m_onePassTraversalFrameId, pObj->m_onePassTraversalShadowCascades, pObj->m_dwRndFlags, passInfo, objBox, entDistance, pObj->m_fWSMaxViewDist, true, bObjectCompletelyInFrustum, nullptr, passCullMask);
				...
				if (objCullMask)
				{
					...
					GetObjUpr()->RenderObject(pObj, nullptr, Vec3(0), objBox, entDistance, rnType, passInfo, objCullMask);
				}
			}
		}
...
void COctreeNode::DeleteObjectsByFlag(i32 nRndFlag)
{
	FUNCTION_PROFILER_3DENGINE;
	for (i32 l = 0; l < eRNListType_ListsNum; l++)
		for (IRenderNode* pObj = m_arrObjects[l].m_pFirstNode, * pNext; pObj; pObj = pNext)
...
void COctreeNode::UnregisterEngineObjectsInArea(const SHotUpdateInfo* pExportInfo, PodArray<IRenderNode*>& arrUnregisteredObjects, bool bOnlyEngineObjects)
{
	...
	u32 nObjTypeMask = pExportInfo ? pExportInfo->nObjTypeMask : (u32) ~0;
...
i32 COctreeNode::PhysicalizeInBox(const AABB& bbox)
{
	...
	struct _Op
	{
		void operator()(IRenderNode* pObj, const AABB& _bbox, i32 checkActive, i32& _nCount) const
		{
...
i32 COctreeNode::DephysicalizeInBox(const AABB& bbox)
{
	if (!Overlap::AABB_AABB(m_objectsBox, bbox))
		return 0;
...
i32 COctreeNode::PhysicalizeOfType(ERNListType listType, bool bInstant)
{
...
i32 COctreeNode::DePhysicalizeOfType(ERNListType listType, bool bInstant)
{
	for (IRenderNode* pObj = m_arrObjects[listType].m_pFirstNode, * pNext; pObj; pObj = pNext)
	{
		pNext = pObj->m_pNext;
...
i32 COctreeNode::GetObjectsCount(EOcTeeNodeListType eListType)
{
	i32 nCount = 0;
...
void COctreeNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
	for (i32 l = 0; l < eRNListType_ListsNum; l++)
	{
		for (IRenderNode* pObj = m_arrObjects[l].m_pFirstNode; pObj; pObj = pObj->m_pNext)
...
void COctreeNode::ActivateObjectsLayer(u16 nLayerId, bool bActivate, bool bPhys, IGeneralMemoryHeap* pHeap, const AABB& layerBox)
{
	...
	for (IRenderNode* pObj = m_arrObjects[eRNListType_Brush].m_pFirstNode; pObj; pObj = pObj->m_pNext)
	{
		if (pObj->GetRenderNodeType() == eERType_Brush)
		{
			...
			if (pBrush->m_nLayerId == nLayerId || nLayerId == u16(~0))
			{
	...
	for (IRenderNode* pObj = m_arrObjects[eRNListType_DecalsAndRoads].m_pFirstNode; pObj; pObj = pObj->m_pNext)
	{
		...
		if (eType == eERType_Decal)
		{
			...
			if (pDecal->GetLayerId() == nLayerId || nLayerId == u16(~0))
			{
	...
	for (IRenderNode* pObj = m_arrObjects[eRNListType_Unknown].m_pFirstNode; pObj; pObj = pObj->m_pNext)
	{
		if (pObj->GetRenderNodeType() == eERType_WaterVolume)
		{
			...
			if (pWatVol->GetLayerId() == nLayerId || nLayerId == u16(~0))
			{
		...
		if (pObj->GetRenderNodeType() == eERType_DistanceCloud)
		{
			...
			if (pCloud->GetLayerId() == nLayerId || nLayerId == u16(~0))
			{
...
void COctreeNode::GetLayerMemoryUsage(u16 nLayerId, IDrxSizer* pSizer, i32* pNumBrushes, i32* pNumDecals)
{
	for (IRenderNode* pObj = m_arrObjects[eRNListType_Brush].m_pFirstNode; pObj; pObj = pObj->m_pNext)
	{
		if (pObj->GetRenderNodeType() == eERType_Brush)
...
void COctreeNode::GetObjects(PodArray<IRenderNode*>& lstObjects, const AABB* pBBox)
{
	...
	u32 nCurrentObject(eRNListType_First);
...
bool COctreeNode::GetShadowCastersTimeSliced(IRenderNode* pIgnoreNode, ShadowMapFrustum* pFrustum, const PodArray<struct SPlaneObject>* pShadowHull, i32 renderNodeExcludeFlags, i32& totalRemainingNodes, i32 nCurLevel, const SRenderingPassInfo& passInfo)
{
	...
	for (i32 i = pFrustum->pShadowCacheData->mOctreePath[nCurLevel]; i < 8; ++i, ++pFrustum->pShadowCacheData->mOctreePath[nCurLevel])
	{
...
void COctreeNode::GenerateStatObjAndMatTables(std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, std::vector<IStatInstGroup*>* pStatInstGroupTable, SHotUpdateInfo* pExportInfo)
{
	...
	for (i32 l = 0; l < eRNListType_ListsNum; l++)
		for (IRenderNode* pObj = m_arrObjects[l].m_pFirstNode; pObj; pObj = pObj->m_pNext)
		{
			...
			{
				CBrush* pBrush = (CBrush*)pObj;
				if (CObjUpr::GetItemId<IStatObj>(pStatObjTable, pBrush->GetEntityStatObj(), false) < 0)
			...
			if (eType == eERType_Brush ||
			    ...
			    eType == eERType_WaterWave)
			{
				if ((eType != eERType_Brush || ((CBrush*)pObj)->m_pMaterial) && CObjUpr::GetItemId(pMatTable, pObj->GetMaterial(), false) < 0)
			...
			if (eType == eERType_MergedMesh)
			{
				...
				for (u32 i = 0; i < pRN->NumGroups(); ++i)
				{
					i32 grpid = pRN->Group(i)->instGroupId;
...
i32 COctreeNode::Cmp_OctreeNodeSize(ukk v1, ukk v2)
{
...
i32 COctreeNode::CountChildNodes()
{
	return
	  (m_arrChilds[0] != 0) +
	  (m_arrChilds[1] != 0) +
...
void COctreeNode::ReleaseEmptyNodes()
{
	...
	i32 nInitCunt = m_arrEmptyNodes.Count();

	for (i32 i = 0; i < nInitCunt && m_arrEmptyNodes.Count(); i++)
	{
...
void COctreeNode::StreamingCheckUnload(const SRenderingPassInfo& passInfo, PodArray<SObjManPrecacheCamera>& rPreCacheCameras)
{
	...
	i32 nMaxTestsPerFrame = m_arrStreamedInNodes.Count() / 30 + 1;

	static i32 nCurNodeId = -1;

	const float fMaxViewDistance = Get3DEngine()->GetMaxViewDistance();

	for (i32 t = 0; t < nMaxTestsPerFrame && m_arrStreamedInNodes.Count(); t++)
	{
		nCurNodeId++;
		...
		bool bOldRound = CObjUpr::m_nUpdateStreamingPrioriryRoundId > (pNode->m_nUpdateStreamingPrioriryRoundId + (i32)2);
...
bool COctreeNode::UpdateStreamingPriority(PodArray<COctreeNode*>& arrRecursion, float fMinDist, float fMaxDist, bool bFullUpdate, const SObjManPrecacheCamera* pPrecacheCams, size_t nPrecacheCams, const SRenderingPassInfo& passInfo)
{
	...
	CVisArea* pRoot0 = GetVisAreaUpr() ? GetVisAreaUpr()->GetCurVisArea() : NULL;
		...
		for (IRenderNode* pObj = m_arrObjects[l].m_pFirstNode; pObj; pObj = pObj->m_pNext)
		{
...
#ifdef _DEBUG
			tukk szName = pObj->GetName();
			tukk szClassName = pObj->GetEntityClassName();
			...
			for (size_t iPrecacheCam = 0; iPrecacheCam < nPrecacheCams; ++iPrecacheCam)
			{
				...
				if (/*fMinDistSq <= fEntDistanceSq &&*/ fEntDistanceSq < fMaxDistCombSq)
				{
					...
					GetObjUpr()->UpdateRenderNodeStreamingPriority(pObj, fDist, fImportanceScale, bFullUpdate, passInfo);
				}
			}
		}
	}
...
i32 COctreeNode::Load(FILE*& f, i32& nDataSize, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, EEndian eEndian, AABB* pBox, const SLayerVisibility* pLayerVisibilityMask)
{
	return Load_T(f, nDataSize, pStatObjTable, pMatTable, eEndian, pBox, pLayerVisibilityMask);
}
i32 COctreeNode::Load(u8*& f, i32& nDataSize, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, EEndian eEndian, AABB* pBox, const SLayerVisibility* pLayerVisibilityMask)
{
	return Load_T(f, nDataSize, pStatObjTable, pMatTable, eEndian, pBox, pLayerVisibilityMask);
}

i32 FTell(FILE*& f)  { return DinrusX3dEngBase::GetPak()->FTell(f); }
i32 FTell(u8*& f) { return 0; }

template<class T>
i32 COctreeNode::Load_T(T*& f, i32& nDataSize, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, EEndian eEndian, AABB* pBox, const SLayerVisibility* pLayerVisibilityMask)
{
	if (pBox && !Overlap::AABB_AABB(GetNodeBox(), *pBox))
		return 0;
	...
	// process childs
	for (i32 nChildId = 0; nChildId < 8; nChildId++)
	{
		if (chunk.ucChildsMask & (1 << nChildId))
		{
			...
			i32 nNewNodesNum = m_arrChilds[nChildId]->Load_T(f, nDataSize, pStatObjTable, pMatTable, eEndian, pBox, pLayerVisibilityMask);
...
void COctreeNode::BuildLoadingDatas(PodArray<SOctreeLoadObjectsData>* pQueue, byte* pOrigData, byte*& pData, i32& nDataSize, EEndian eEndian)
{
	SOcTreeNodeChunk chunk;
...
bool COctreeNode::StreamLoad(u8* pData, i32 nDataSize, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, EEndian eEndian, AABB* pBox)
{
	...
	else
	{
		...
		{
			pData += data.offset;
			nDataSize -= (i32)data.offset;

			// load objects data into memory buffer, make sure buffer is aligned
			_smart_ptr<IMemoryBlock> pMemBlock = gEnv->pDrxPak->PoolAllocMemoryBlock(data.size + 8, "LoadObjectInstances");
...
i32 COctreeNode::GetData(byte*& pData, i32& nDataSize, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, std::vector<IStatInstGroup*>* pStatInstGroupTable, EEndian eEndian, SHotUpdateInfo* pExportInfo)
{
	AABB* pBox = (pExportInfo && !pExportInfo->areaBox.IsReset()) ? &pExportInfo->areaBox : NULL;
...
//////////////////////////////////////////////////////////////////////////
bool COctreeNode::CheckRenderFlagsMinSpec(u32 dwRndFlags)
{
	i32 nRenderNodeMinSpec = (dwRndFlags & ERF_SPEC_BITS_MASK) >> ERF_SPEC_BITS_SHIFT;
...
template<class T>
i32 COctreeNode::ReadObjects(T*& f, i32& nDataSize, EEndian eEndian, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, const SLayerVisibility* pLayerVisibilityMask, SOcTreeNodeChunk& chunk, ELoadObjectsMode eLoadMode)
{
	...
	{
		{
			_smart_ptr<IMemoryBlock> pMemBlock = gEnv->pDrxPak->PoolAllocMemoryBlock(chunk.nObjectsBlockSize + 8, "LoadObjectInstances");
			byte* pPtr = (byte*)pMemBlock->GetData();
...
void COctreeNode::StartStreaming(bool bFinishNow, IReadStream_AutoPtr* ppStream)
{
	...
	tukk szFileName = Get3DEngine()->GetLevelFilePath(COMPILED_HEIGHT_MAP_FILE_NAME);
	...
	else
	{
		...
		params.ePriority = estpAboveNormal;// (EStreamTaskPriority)CLAMP(i32((float)estpIdle - log2(GetNodeBox().GetSize().x / fNodeMinSize)), i32(estpUrgent), i32(estpIdle));
...
template<class T>
void COctreeNode::StreamOnCompleteReadObjects(T* f, i32 nDataSize)
{
	FUNCTION_PROFILER_3DENGINE;
...
bool COctreeNode::CheckStartStreaming(bool bFullUpdate)
{
	...
	if (GetCVars()->e_StreamInstances == 1)
		m_nUpdateStreamingPrioriryRoundId = CObjUpr::m_nUpdateStreamingPrioriryRoundId;
...
COctreeNode::COctreeNode(const AABB& box, CVisArea* pVisArea, COctreeNode* pParent)
{
	...
	if (GetCVars()->e_CheckOctreeObjectsBoxSize != 0) // pParent is checked as silly sized things are added to the root (e.g. the sun)
		if (pParent && (m_objectsBox.min.len() > CHECK_OBJECTS_BOX_WARNING_SIZE || m_objectsBox.max.len() > CHECK_OBJECTS_BOX_WARNING_SIZE))
			DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR_DBGBRK, "COctreeNode being created with a huge m_objectsBox: [%f %f %f] -> [%f %f %f]\n", m_objectsBox.min.x, m_objectsBox.min.y, m_objectsBox.min.z, m_objectsBox.max.x, m_objectsBox.max.y, m_objectsBox.max.z);
#endif
...
//////////////////////////////////////////////////////////////////////////
void COctreeNode::RenderContent(i32 nRenderMask, const Vec3& vAmbColor, u32 passCullMask, const SRenderingPassInfo& passInfo)
{
	if (Get3DEngine()->IsStatObjBufferRenderTasksAllowed() && passInfo.IsGeneralPass() && JobUpr::InvokeAsJob("CheckOcclusion"))
		GetObjUpr()->AddCullJobProducer();

	TRenderContentJob renderContentJob(nRenderMask, vAmbColor, passCullMask, passInfo);
	renderContentJob.SetClassInstance(this);
	renderContentJob.SetPriorityLevel(JobUpr::eHighPriority);
	renderContentJob.Run();
}

//////////////////////////////////////////////////////////////////////////
void COctreeNode::RenderContentJobEntry(i32 nRenderMask, Vec3 vAmbColor, u32 passCullMask, SRenderingPassInfo passInfo)
{
#if !defined(_RELEASE)
	...
	if (passCullMask & kPassCullMainMask) // if visible for main pass
	{
		...
		if (m_fPrevTerrainTexScale != (pTerrainTexInfo ? pTerrainTexInfo->fTexScale : 0))
		{
			...
			for (u32 nListId = eRNListType_First; nListId < eRNListType_ListsNum; ++nListId)
			{
...
///////////////////////////////////////////////////////////////////////////////
void COctreeNode::RenderVegetations(TDoublyLinkedList<IRenderNode>* lstObjects, u32k passCullMask, i32 nRenderMask, const bool bOcNodeCompletelyInFrustum,
                                    PodArray<SRenderLight*>* pAffectingLights, SSectorTextureSet* pTerrainTexInfo, const SRenderingPassInfo& passInfo)
{
	...
	CThreadSafeRendererContainer<SVegetationSpriteInfo>& arrSpriteInfo = GetObjUpr()->m_arrVegetationSprites[passInfo.GetRecursiveLevel()][passInfo.ThreadID()];

	const bool bOcclusionCullerInUse = Get3DEngine()->IsStatObjBufferRenderTasksAllowed() && passInfo.IsGeneralPass() && JobUpr::InvokeAsJob("CheckOcclusion");
	...
	for (CVegetation* pObj = (CVegetation*)m_arrObjects[eRNListType_Vegetation].m_pFirstNode, * pNext; pObj; pObj = pNext)
	{
		...
		u32 objCullMask = UpdateCullMask(pObj->m_onePassTraversalFrameId, pObj->m_onePassTraversalShadowCascades, pObj->m_dwRndFlags, passInfo, objBox, fEntDistance, pObj->m_fWSMaxViewDist, bCheckPerObjectOcclusion, bObjectCompletelyInFrustum, nullptr, passCullMask);
		...
		if (objCullMask)
		{
			...
			{
				GetObjUpr()->RenderVegetation(pObj, pAffectingLights, objBox, fEntDistance, pTerrainTexInfo, bCheckPerObjectOcclusion, passInfo, objCullMask);
			}
			else
			{
				GetObjUpr()->PushIntoCullOutputQueue(SCheckOcclusionOutput::CreateCommonObjectOutput(pObj, pAffectingLights, Vec3(0), objBox, fEntDistance, pTerrainTexInfo, objCullMask, passInfo));
			}
		}
	}
}
...
///////////////////////////////////////////////////////////////////////////////
void COctreeNode::RenderBrushes(TDoublyLinkedList<IRenderNode>* lstObjects, u32k passCullMask, const bool bOcNodeCompletelyInFrustum, PodArray<SRenderLight*>* pAffectingLights, SSectorTextureSet* pTerrainTexInfo, const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	for (CBrush* pObj = (CBrush*)lstObjects->m_pFirstNode, * pNext; pObj; pObj = pNext)
	{
		...
		if (objCullMask)
		{
			...
			{
				GetObjUpr()->RenderBrush(pObj, pAffectingLights, pTerrainTexInfo, objBox, fEntDistance, bCheckPerObjectOcclusion, passInfo, objCullMask);
			}
			else
			{
				GetObjUpr()->PushIntoCullOutputQueue(SCheckOcclusionOutput::CreateCommonObjectOutput(pObj, pAffectingLights, Vec3(0), objBox, fEntDistance, pTerrainTexInfo, objCullMask, passInfo));
...
void COctreeNode::RenderObjectIntoShadowViews(const SRenderingPassInfo& passInfoGeneral, float fEntDistance, IRenderNode* pObj, const AABB& objBox, u32k passCullMask)
{
	DRX_ASSERT_MESSAGE(!passInfoGeneral.IsShadowPass(), "RenderObjectIntoShadowViews expects to get a general pass as input.");
...
///////////////////////////////////////////////////////////////////////////////
void COctreeNode::RenderDecalsAndRoads(TDoublyLinkedList<IRenderNode>* lstObjects, u32k passCullMask, i32 nRenderMask, const Vec3& vAmbColor, const bool bOcNodeCompletelyInFrustum, PodArray<SRenderLight*>* pAffectingLights, const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	for (IRenderNode* pObj = lstObjects->m_pFirstNode, * pNext; pObj; pObj = pNext)
	{
		...
		// check culling of all passes
		bool bObjectCompletelyInFrustum = bOcNodeCompletelyInFrustum;
		u32 objCullMask = UpdateCullMask(pObj->m_onePassTraversalFrameId, pObj->m_onePassTraversalShadowCascades, pObj->m_dwRndFlags, passInfo, objBox, fEntDistance, pObj->m_fWSMaxViewDist, true, bObjectCompletelyInFrustum, nullptr, passCullMask & kPassCullMainMask);
		...
		if (objCullMask)
		{
			...
			{
				GetObjUpr()->RenderDecalAndRoad(pObj, pAffectingLights, vAmbColor, objBox, fEntDistance, bCheckPerObjectOcclusion, passInfo);
			}
			else
			{
				GetObjUpr()->PushIntoCullOutputQueue(SCheckOcclusionOutput::CreateDecalsAndRoadsOutput(pObj, pAffectingLights, vAmbColor, objBox, fEntDistance, bCheckPerObjectOcclusion, passInfo));
			}
		}
	}
}
...
///////////////////////////////////////////////////////////////////////////////
void COctreeNode::RenderCommonObjects(TDoublyLinkedList<IRenderNode>* lstObjects, u32k passCullMask, i32 nRenderMask, const Vec3& vAmbColor, const bool bOcNodeCompletelyInFrustum, PodArray<SRenderLight*>* pAffectingLights, SSectorTextureSet* pTerrainTexInfo, const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	for (IRenderNode* pObj = lstObjects->m_pFirstNode, * pNext; pObj; pObj = pNext)
	{
		...
		// check culling of all passes
		bool bObjectCompletelyInFrustum = bOcNodeCompletelyInFrustum;
		u32 objCullMask = UpdateCullMask(pObj->m_onePassTraversalFrameId, pObj->m_onePassTraversalShadowCascades, pObj->m_dwRndFlags, passInfo, objBox, fEntDistance, pObj->m_fWSMaxViewDist, true, bObjectCompletelyInFrustum, nullptr, passCullMask);
		...
		if (objCullMask)
		{
			...
			if (pObj->CanExecuteRenderAsJob() || !bOcclusionCullerInUse)
			{
				...
				{
					GetObjUpr()->RenderBrush((CBrush*)pObj, pAffectingLights, nullptr, objBox, fEntDistance, true, passInfo, passCullMask);
				}
				else
				{
				GetObjUpr()->RenderObject(pObj, pAffectingLights, vAmbColor, objBox, fEntDistance, rnType, passInfo, objCullMask);
			}
			}
			else
			{
				GetObjUpr()->PushIntoCullOutputQueue(SCheckOcclusionOutput::CreateCommonObjectOutput(pObj, pAffectingLights, vAmbColor, objBox, fEntDistance, pTerrainTexInfo, objCullMask, passInfo));
			}
		}
	}
}
...
void COctreeNode::InsertObject(IRenderNode* pObj, const AABB& objBox, const float fObjRadiusSqr, const Vec3& vObjCenter)
{
	...
	EERType eType = pObj->GetRenderNodeType();
	u32k renderFlags = (pObj->GetRndFlags() & (ERF_GOOD_OCCLUDER | ERF_CASTSHADOWMAPS | ERF_HAS_CASTSHADOWMAPS));
	...
	while (true)
	{
		...
		if (GetCVars()->e_CheckOctreeObjectsBoxSize != 0) // pCurrentNode->m_pParent is checked as silly sized things are added to the root (e.g. the sun)
			...
			{
				DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR_DBGBRK, "Huge object being added to a COctreeNode, name: '%s', objBox: [%f %f %f] -> [%f %f %f]\n", pObj->GetName(), objBox.min.x, objBox.min.y, objBox.min.z, objBox.max.x, objBox.max.y, objBox.max.z);
			}
#endif
...
//////////////////////////////////////////////////////////////////////////
AABB COctreeNode::GetChildBBox(i32 nChildId)
{
	i32 x = (nChildId / 4);
	i32 y = (nChildId - x * 4) / 2;
	i32 z = (nChildId - x * 4 - y * 2);
...
//////////////////////////////////////////////////////////////////////////
void COctreeNode::CompileCharacter(ICharacterInstance* pChar, u8& nInternalFlags)
{
	u32k nCompileMask = IRenderNode::REQUIRES_FORWARD_RENDERING | IRenderNode::REQUIRES_NEAREST_CUBEMAP;
	if (pChar)
	{
		...
		if (pCharMaterial)
		{
			u8 nInternalFlagsNew = pCharMaterial->IsForwardRenderingRequired() ? IRenderNode::REQUIRES_FORWARD_RENDERING : 0;
		...
		if (IAttachmentUpr* pAttMan = pChar->GetIAttachmentUpr())
		{
				...
				if (IAttachment* pAtt = pAttMan->GetInterfaceByIndex(i))
					if (IAttachmentObject* pAttObj = pAtt->GetIAttachmentObject())
					{
						...
						if (IStatObj* pStatObj = pAttObj->GetIStatObj())
						{
							...
							if (pMat)
							{
								u8 nInternalFlagsNew = pMat->IsForwardRenderingRequired() ? IRenderNode::REQUIRES_FORWARD_RENDERING : 0;
...
//////////////////////////////////////////////////////////////////////////
i16 CObjUpr::GetNearestCubeProbe(PodArray<SRenderLight*>* pAffectingLights, IVisArea* pVisArea, const AABB& objBox, bool bSpecular, Vec4* pEnvProbeMults)
{
	// Only used for alpha blended geometry - but still should be optimized further
	float fMinDistance = FLT_MAX;
	i32 nMaxPriority = -1;
	CLightEntity* pNearestLight = NULL;
	i16 nDefaultId = Get3DEngine()->GetBlackCMTexID();
	...
	{
		Get3DEngine()->GetVisAreaUpr()->GetNearestCubeProbe(fMinDistance, nMaxPriority, pNearestLight, &objBox);
	}
...
//////////////////////////////////////////////////////////////////////////
bool CObjUpr::IsAfterWater(const Vec3& vPos, const Vec3& vCamPos, const SRenderingPassInfo& passInfo, float fUserWaterLevel)
{
...
//////////////////////////////////////////////////////////////////////////
void CObjUpr::RenderObjectDebugInfo(IRenderNode* pEnt, float fEntDistance, const SRenderingPassInfo& passInfo)
{
...
//////////////////////////////////////////////////////////////////////////
void CObjUpr::FillTerrainTexInfo(IOctreeNode* pOcNode, float fEntDistance, struct SSectorTextureSet*& pTerrainTexInfo, const AABB& objBox)
{
...
//////////////////////////////////////////////////////////////////////////
void CObjUpr::RenderBrush(CBrush* pEnt, PodArray<SRenderLight*>* pAffectingLights,
...
//////////////////////////////////////////////////////////////////////////
void CObjUpr::RemoveCullJobProducer()
{
	m_CheckOcclusionOutputQueue.RemoveProducer();
}

//////////////////////////////////////////////////////////////////////////
void CObjUpr::AddCullJobProducer()
{
	m_CheckOcclusionOutputQueue.AddProducer();
}

//////////////////////////////////////////////////////////////////////////
bool CObjUpr::CheckOcclusion_TestAABB(const AABB& rAABB, float fEntDistance)
{
	return m_CullThread.TestAABB(rAABB, fEntDistance);
}

//////////////////////////////////////////////////////////////////////////
bool CObjUpr::CheckOcclusion_TestQuad(const Vec3& vCenter, const Vec3& vAxisX, const Vec3& vAxisY)
{
...
#ifndef _RELEASE
//////////////////////////////////////////////////////////////////////////
void CObjUpr::CoverageBufferDebugDraw()
{
...
//////////////////////////////////////////////////////////////////////////
bool CObjUpr::LoadOcclusionMesh(tukk pFileName)
{
	return m_CullThread.LoadLevel(pFileName);
}

//////////////////////////////////////////////////////////////////////////
void CObjUpr::PushIntoCullQueue(const SCheckOcclusionJobData& rCheckOcclusionData)
{
#if !defined(_RELEASE)
...
//////////////////////////////////////////////////////////////////////////
void CObjUpr::PopFromCullQueue(SCheckOcclusionJobData* pCheckOcclusionData)
{
	m_CheckOcclusionQueue.Pop(pCheckOcclusionData);
}

//////////////////////////////////////////////////////////////////////////
void CObjUpr::PushIntoCullOutputQueue(const SCheckOcclusionOutput& rCheckOcclusionOutput)
{
	m_CheckOcclusionOutputQueue.Push(rCheckOcclusionOutput);
}

//////////////////////////////////////////////////////////////////////////
bool CObjUpr::PopFromCullOutputQueue(SCheckOcclusionOutput* pCheckOcclusionOutput)
{
	DRX_PROFILE_REGION(PROFILE_3DENGINE, "3DEngine: WaitCullOutputQueue");

	return m_CheckOcclusionOutputQueue.Pop(pCheckOcclusionOutput);
}

///////////////////////////////////////////////////////////////////////////////
i32 CObjUpr::GetObjectLOD(const IRenderNode* pObj, float fDistance)
{
	SFrameLodInfo frameLodInfo = Get3DEngine()->GetFrameLodInfo();
	i32 resultLod = MAX_STATOBJ_LODS_NUM - 1;
	...
	if (!bLodFaceArea)
	{
		...
		resultLod = (i32)(fDistance * (fLodRatioNorm * fLodRatioNorm) / (max(frameLodInfo.fLodRatio * min(fRadius, GetFloatCVar(e_LodCompMaxSize)), 0.001f)));
	}
...
{
	i32 nFrameID = passInfo.GetFrameID();
	i32 nRecursiveLevel = passInfo.GetRecursiveLevel();
	i32 nNewLightMaskFrameID = nFrameID + nRecursiveLevel;
	...
	if (pInstStreamCheckReady)
	{
		if (!CheckStartStreaming(false))
		{
			*pInstStreamCheckReady = false;
			...
			{
				DinrusX3dEngBase::Get3DEngine()->DrawBBox(objectsBox, Col_Red);
			}
#endif
		}
	}
...
///////////////////////////////////////////////////////////////////////////////
void COctreeNode::GetNearestCubeProbe(float& fMinDistance, i32& nMaxPriority, CLightEntity*& pNearestLight, const AABB* pBBox)
{
	if (!m_bHasLights)
		return;
	...
	for (IRenderNode* pObj = m_arrObjects[eListType].m_pFirstNode; pObj; pObj = pObj->m_pNext)
	{
		if (pObj->GetRenderNodeType() == eERType_Light)
		{
			...
			if (pLightEnt->GetLayerId() != u16(~0) && (pObj->m_dwRndFlags & ERF_HIDDEN))
				continue;
...
///////////////////////////////////////////////////////////////////////////////
bool CObjUpr::IsBoxOccluded(const AABB& objBox,
                                float fDistance,
                                ...
                                const SRenderingPassInfo& passInfo)
{
	...
	if (GetCVars()->e_OcclusionLazyHideFrames)
	{
		...
		if (pOcclTestVars->nLastVisibleMainFrameID > mainFrameID - GetCVars()->e_OcclusionLazyHideFrames)
		{
			...
			i32 nId = (i32)(UINT_PTR(pOcclTestVars) / 256);
	...
	if (GetCVars()->e_OcclusionVolumes && pVisAreaUpr && pVisAreaUpr->IsOccludedByOcclVolumes(objBox, passInfo))
	{
...
void COctreeNode::SetTraversalFrameId(IRenderNode* pObj, u32 onePassTraversalFrameId, i32 shadowFrustumLod)
{
