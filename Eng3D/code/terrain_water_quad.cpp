...
// -------------------------------------------------------------------------
//  Имя файла:   terrain_water_quad.cpp
//  Version:     v1.00
//  Created:     28/8/2001 by Vladimir Kajalin
//  Компиляторы:       drux (DinrusPro's Universal Compiler),   mingw-w64-clang-x86_64
//  Описание:  Создание и отрисовка водной геометрии ландшафта
//            (сетка пространства экрана, циклические буферы).
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
std::tuple<u32, u32> GenerateOceanSurfaceVertices(
  ...
  i32 swathWidth)
{
	...
	for (i32 y(0); y < nScrGridSizeY; ++y)
	{
		vv.y = (float)y * fRcpScrGridSizeY;// + fRcpScrGridSize;

		for (i32 x(0); x < nScrGridSizeX; ++x)
		{
			// vert 1
	...
	if (bUseTessHW)
	{
		// Normal approach
		i32 nIndex = 0;
		for (i32 y(0); y < nScrGridSizeY - 1; ++y)
		{
			for (i32 x(0); x < nScrGridSizeX - 1; ++x, ++nIndex)
			{
	...
	else
	{
		...
		if (swathWidth <= 0)
		{
			...
			for (i32 y(0); y < nScrGridSizeY - 1; ++y)
			{
				for (i32 x(0); x < nScrGridSizeX; ++x, ++nIndex)
				{
					AddIndex(nIndex);
		...
		else
		{
			...
			i32 startX = 0, endX = swathWidth - 1;

			do
			{

				for (i32 y(0); y < nScrGridSizeY - 1; y += 2)
				{
					// Forward
...
u32 COcean::m_nVisiblePixelsCount = ~0;
...
COcean::~COcean()
{
	for (i32 x = 0; x < CYCLE_BUFFERS_NUM; ++x)
	{
...
void COcean::Update(const SRenderingPassInfo& passInfo)
{
	...
	i32 nFillThreadID = passInfo.ThreadID();
	u32 nBufID = passInfo.GetFrameID() % CYCLE_BUFFERS_NUM;
	...
	if (vCamPos.z < fWaterLevel)
	{
		...
		{
			for (i32 i = 0; i < pVisArea->m_lstConnections.Count(); i++)
			{
	...
	if (fabs(m_fLastFov - rCamera.GetFov()) < 0.01f && GetCVars()->e_HwOcclusionCullingWater && passInfo.IsGeneralPass())
	{
		...
		if ((!GetVisAreaUpr()->IsOceanVisible() && rCamera.IsAABBVisible_EM(boxOcean)) ||
		    (GetVisAreaUpr()->IsOceanVisible() && rCamera.IsAABBVisible_E(boxOcean)))
		{
			...
			if (!m_pREOcclusionQueries[nBufID])
			{
				...
				m_pREOcclusionQueries[nBufID]->m_pRMBox = (CRenderMesh*)GetObjUpr()->GetRenderMeshBox();
			}
...
void COcean::Create(const SRenderingPassInfo& passInfo)
{
	...
	i32 nScrGridSizeX = 20 * GetCVars()->e_WaterTessellationAmount;
	...
	i32 nScrGridSizeY = (GetCVars()->e_WaterTessellationAmountX && GetCVars()->e_WaterTessellationAmountY) ? GetCVars()->e_WaterTessellationAmountY : nScrGridSizeX;

	// Store permanently the swath width
	static i32 swathWidth = 0;
	...
	i32k currentSwathWidth = min(nScrGridSizeX, GetCVars()->e_WaterTessellationSwathWidth);
	...
	if ((m_bOceanFFT && bUsingFFT != m_bOceanFFT) || bUseTessHW != bUseWaterTessHW || swathWidth != currentSwathWidth || !m_nVertsCount || !m_nIndicesCount || nScrGridSizeX * nScrGridSizeY != m_nPrevGridDim)
	{
		...
		i32k nScreenY = pCam.GetViewSurfaceX();
		i32k nScreenX = pCam.GetViewSurfaceZ();
		...
		u32k reqVerticesCount = nScrGridSizeX * nScrGridSizeY;
		u32k reqIndicesCount  = nScrGridSizeX * nScrGridSizeY * 6;
		const bool result = m_pOceanRE->RequestVerticesBuffer(&pReqVertices, (u8**)&pReqIndices, reqVerticesCount, reqIndicesCount, sizeof(vtx_idx));
		...
		DRX_ASSERT(m_nVertsCount <= reqVerticesCount);
		DRX_ASSERT(m_nIndicesCount <= reqIndicesCount);

		m_pOceanRE->SubmitVerticesBuffer(m_nVertsCount, m_nIndicesCount, sizeof(vtx_idx), pReqVertices, (u8*)pReqIndices);
	}
}
...
void COcean::Render(const SRenderingPassInfo& passInfo)
{
	...
	i32 nBufID = (passInfo.GetFrameID() & 1);
	Vec3 vCamPos = passInfo.GetCamera().GetPosition();
	float fWaterLevel = p3DEngine->GetWaterLevel();

	i32k fillThreadID = passInfo.ThreadID();
...
void COcean::RenderBottomCap(const SRenderingPassInfo& passInfo)
{
	...
	if (!m_pBottomCapVerts.Count() || !m_pBottomCapIndices.Count() || nScrGridSize * nScrGridSize != m_pBottomCapVerts.Count())
	{
		...
		// Grid vertex generation
		for (i32 y(0); y < nScrGridSize; ++y)
		{
			vv.y = (float) y * fRcpScrGridSize + fRcpScrGridSize;
			for (i32 x(0); x < nScrGridSize; ++x)
			{
		...
		// Normal approach
		i32 nIndex = 0;
		for (i32 y(0); y < nScrGridSize - 1; ++y)
		{
			for (i32 x(0); x < nScrGridSize; ++x, ++nIndex)
			{
...
void COcean::RenderFog(const SRenderingPassInfo& passInfo)
{
	                                        ...
	                                        (isLowSpec && m_pFogIntoMatLowSpec && m_pFogOutofMatLowSpec)))
	{
		...
		if (!isFastpath || (distCamToFogPlane < p3DEngine->m_oceanWavesSize))
		{
			...
			size_t subDivSize(min(64, 1 + (i32) (planeSize / 512.0f)));
			if (isFastpath)
				subDivSize = 4;
			...
			if (m_wvVertices[fillThreadID].size() != numSubDivVerts)
			{
				...
				size_t ind(0);
				for (u32 y(0); y < subDivSize; ++y)
				{
					for (u32 x(0); x < subDivSize; ++x, ind += 6)
					{
						m_wvIndices[fillThreadID][ind + 0] = static_cast<u16>((y) * (subDivSize + 1) + (x));
						m_wvIndices[fillThreadID][ind + 1] = static_cast<u16>((y) * (subDivSize + 1) + (x + 1));
						m_wvIndices[fillThreadID][ind + 2] = static_cast<u16>((y + 1) * (subDivSize + 1) + (x + 1));

						m_wvIndices[fillThreadID][ind + 3] = static_cast<u16>((y) * (subDivSize + 1) + (x));
						m_wvIndices[fillThreadID][ind + 4] = static_cast<u16>((y + 1) * (subDivSize + 1) + (x + 1));
						m_wvIndices[fillThreadID][ind + 5] = static_cast<u16>((y + 1) * (subDivSize + 1) + (x));
					}
				}
			}
			{
				...
				size_t ind(0);
				float yd(-planeSize);
				for (u32 y(0); y <= subDivSize; ++y, yd += xyDelta)
				{
					float xd(-planeSize);
					for (u32 x(0); x <= subDivSize; ++x, xd += xyDelta, ++ind)
					{
...
float COcean::GetWave(const Vec3& pPos, i32 nFrameID)
{
	// todo: optimize...
...
u32 COcean::GetVisiblePixelsCount()
{
