...
bool SIntersectionData::Init(IRenderMesh* param_pRenderMesh, SRayHitInfo* param_pHitInfo, IMaterial* param_pMtl, bool param_bRequestDecalPlacementTest)
{
	...
	if (nInds == 0 || nVerts == 0)
		return false;
	pPos = (u8*)pRenderMesh->GetPosPtr(nPosStride, FSL_READ);
	...
	if (bAllDMeshData)
	{
		pUV = (u8*)pRenderMesh->GetUVPtr(nUVStride, FSL_READ);
		pCol = (u8*)pRenderMesh->GetColorPtr(nColStride, FSL_READ);
...
bool CRenderMeshUtils::RayIntersectionImpl(SIntersectionData* pIntersectionRMData, SRayHitInfo* phitInfo, IMaterial* pMtl, bool bAsync)
{
	...
	if (hitInfo.bUseCache && !bAsync)
	{
		...
		for (i32 i = 0; i < MAX_CACHED_HITS; i++)
		{
			if (last_hits[i].pRenderMesh == rIntersectionRMData.pRenderMesh)
			{
				...
				if (Intersect::Ray_Triangle(hitInfo.inRay, last_hits[i].tri[0], last_hits[i].tri[2], last_hits[i].tri[1], vOut))
				{
					...
					//DrxLogAlways( "TestTime :%.2f", (t1-t0).GetMilliSeconds() );
					//static i32 nCount = 0; DrxLogAlways( "Cached Hit %d",++nCount );
	...
	i32 nInds = rIntersectionRMData.nInds;

	assert(nInds != 0 && nVerts != 0);

	// get position offset and stride
	i32k nPosStride = rIntersectionRMData.nPosStride;
	u8* const pPos = rIntersectionRMData.pPos;
	...
	for (i32 nChunkId = 0; nChunkId < nChunkCount; nChunkId++)
	{
		...
		IF (pChunk->m_nMatFlags & MTL_FLAG_NODRAW || !pChunk->pRE, 0)
			continue;
		i16k nChunkMatID = pChunk->m_nMatID;
...
bool CRenderMeshUtils::RayIntersectionFastImpl(SIntersectionData& rIntersectionRMData, SRayHitInfo& hitInfo, IMaterial* pMtl, bool bAsync)
{
	...
	i32 nVerts = rIntersectionRMData.nVerts;
	i32 nInds = rIntersectionRMData.nInds;

	assert(nInds != 0 && nVerts != 0);

	// get position offset and stride
	i32 nPosStride = rIntersectionRMData.nPosStride;
	u8* pPos = rIntersectionRMData.pPos;

	i32 nUVStride = rIntersectionRMData.nUVStride;
	u8* pUV = rIntersectionRMData.pUV;

	i32 nColStride = rIntersectionRMData.nColStride;
	u8* pCol = rIntersectionRMData.pCol;
	...
	if (!inRay.direction.IsZero() && hitInfo.nHitTriID >= 0)
	{
		...
		i32 I0 = pInds[hitInfo.nHitTriID + 0];
		i32 I1 = pInds[hitInfo.nHitTriID + 1];
		i32 I2 = pInds[hitInfo.nHitTriID + 2];
	...
	if (hitInfo.nHitTriID == HIT_UNKNOWN)
	{
		...
		{
			if (const PodArray<std::pair<i32, i32>>* pTris = rIntersectionRMData.pRenderMesh->GetTrisForPosition(inRay.origin + inRay.direction * 0.5f, pMtl))
			{
				for (i32 nId = 0; nId < pTris->Count(); ++nId)
				{
					std::pair<i32, i32>& t = pTris->GetAt(nId);

					if (t.first + 2 >= nInds)
						return false;

					i32 I0 = pInds[t.first + 0];
					i32 I1 = pInds[t.first + 1];
					i32 I2 = pInds[t.first + 2];
	...
	if (bAnyHit)
	{
		...
		if (hitInfo.bGetVertColorAndTC && hitInfo.nHitTriID >= 0 && !inRay.direction.IsZero())
		{
			...
			if (GetBarycentricCoordinates(vHitPos - tv0, tv1 - tv0, tv2 - tv0, u, v, w, 16.0f))
			{
				...
				// get tangent basis
				i32 nTangsStride = rIntersectionRMData.nTangsStride;
				byte* pTangs = rIntersectionRMData.pTangs;

				Vec4 tangent[3];
				Vec4 bitangent[3];
				i32 arrId[3] = { I0, I1, I2 };
...
bool CRenderMeshUtils::ProcessBoxIntersection(Ray& inRay, SRayHitInfo& hitInfo, SIntersectionData& rIntersectionRMData, IMaterial* pMtl, vtx_idx* pInds, i32 nVerts, u8* pPos, i32 nPosStride, u8* pUV, i32 nUVStride, u8* pCol, i32 nColStride, i32 nInds, bool& bAnyHit, float& fBestDist, Vec3& vHitPos, Vec3* tri)
{
	AABB voxBox;
			...
			for (uint ii = pChunk->nFirstIndexId; ii < pChunk->nFirstIndexId + pChunk->nNumIndices; ii += 3)
			{
				...
				if (Overlap::AABB_Triangle(voxBox, tv0, tv2, tv1))
				{
					...
					if (triBox.GetRadiusSqr() > 0.00001f)
					{
						...
						ht.nOpacity = SATURATEB(i32(fOpacity * 255.f));
	...
	else if (const PodArray<std::pair<i32, i32>>* pTris = rIntersectionRMData.pRenderMesh->GetTrisForPosition(inRay.origin, pMtl))
	{
		for (i32 nId = 0; nId < pTris->Count(); ++nId)
		{
			std::pair<i32, i32>& t = pTris->GetAt(nId);
