...
// -------------------------------------------------------------------------
//  Имя файла:   terrain_sector_render.cpp
//  Version:     v1.00
//  Created:     28/5/2001 by Vladimir Kajalin
//  Компиляторы:       drux (DinrusPro's Universal Compiler),   mingw-w64-clang-x86_64
//  Описание:    Создание буфера, копирование его в память переменных, отрисовка.
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
public:
	...
	i32   Count() const       { return m_nCount; }
	Type* GetElements() const { return m_pMemory; }

	void  Add(const Type& obj);

	i32   MemorySize() const { return m_nCap * sizeof(Type); }
...
private:
	...
	i32   m_nCount;       // number of objects in the container
	i32   m_nCap;         // maximum number of objects fitting into the allocated memory
};
...
void CTerrainTempDataStorage<Type >::Add(const Type& obj)
{
	...
	{
		DrxFatalError("terrain sector scheduled update error");
		return;
	}
...
struct CStripsInfo
{
	...
	i32                              nNonBorderIndicesCount;

	inline void Clear() { idx_array.Clear(); nNonBorderIndicesCount = 0; }

	inline void AddIndex(i32 _x, i32 _y, i32 _step, i32 nSectorSize)
	{
...
public:
	...
	JobUpr::SJobState m_JobStateBuildIndices;
	JobUpr::SJobState m_JobStateBuildVertices;
	...
	// for Update Indices
	DRX_ALIGN(128) CStripsInfo m_StripsInfo;
	// for update vectices
	DRX_ALIGN(128) CTerrainTempDataStorage<SVF_P2S_N4B_C4B_T1F> m_lstTmpVertArray;
};

CTerrainUpdateDispatcher::CTerrainUpdateDispatcher()
{
	m_pHeapStorage = DrxGetIMemoryUpr()->AllocPages(TempPoolSize);
	m_pHeap = DrxGetIMemoryUpr()->CreateGeneralMemoryHeap(m_pHeapStorage, TempPoolSize, "Terrain temp pool");
}
...
CTerrainUpdateDispatcher::~CTerrainUpdateDispatcher()
{
	...
	m_pHeap = NULL;
	DrxGetIMemoryUpr()->FreePages(m_pHeapStorage, TempPoolSize);
}
...
bool CTerrainUpdateDispatcher::AddJob(CTerrainNode* pNode, bool executeAsJob, const SRenderingPassInfo& passInfo)
{
	...
	i32 nNumVerts = (meshDim + 1 + 2) * (meshDim + 1 + 2);
	i32 nNumIdx = (meshDim + 2) * (meshDim + 2) * 6;
	...
	u8* pTempData = (u8*)m_pHeap->Memalign(__alignof(CUpdateTerrainTempData), allocSize, ""); // /*NOTE: Disabled due aliasing issues in the allocator with SNC compiler */m_TempStorage.Allocate<u8*>(allocSize, __alignof(CUpdateTerrainTempData));
	if (pTempData)
	{
		...
		pTempData = (u8*)((reinterpret_cast<uintptr_t>(pTempData) + alignPad) & ~uintptr_t(alignPad));
...
void CTerrainUpdateDispatcher::SyncAllJobs(bool bForceAll, const SRenderingPassInfo& passInfo)
{
	...
	do
	{
		...
		{
			DrxLogAlways("ERROR: not all terrain sector vertex/index update requests could be scheduled");
			break;
		}
	}
...
void CTerrainUpdateDispatcher::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
{
	for (i32 s = 0; s < SRangeInfo::e_max_surface_types; s++)
	{
		for (i32 p = 0; p < 4; p++)
		{
...
void CTerrainNode::SetupTexGenParams(SSurfaceType* pSurfaceType, float* pOutParams, u8 ucProjAxis, bool bOutdoor, float fTexGenScale)
{
	assert(pSurfaceType);
...
void CTerrainNode::DrawArray(const SRenderingPassInfo& passInfo)
{
	...
	if (passInfo.IsShadowPass() || (m_nTreeLevel >= GetCVars()->e_TerrainMeshInstancingMinLod))
	{
		...
		if (!GetObjUpr()->AddOrCreatePersistentRenderObject(pTempData, pTerrainRenderObject, &lodValue, objMat, passInfo))
		{
			...
			pTerrainRenderObject->m_nTextureID = -(i32)m_nTexSet.nSlot0 - 1;
	...
	if (passInfo.RenderTerrainDetailMaterial() && !passInfo.IsShadowPass())
	{
		...
		for (i32 nP = 0; nP < 2; nP++)
			if (bDrawDetailLayersXYZ[nP])
			{
				CRenderObject* pDetailObj = nullptr;

				CLodValue detailLodValue(1 + nP, 0, 1 + nP);
				if (GetObjUpr()->AddOrCreatePersistentRenderObject(pTempData, pDetailObj, &detailLodValue, objMat, passInfo))
					continue;
				...
				pDetailObj->m_nTextureID = -(i32)m_nTexSet.nSlot0 - 1;
...
void CTerrainNode::UpdateRenderMesh(CStripsInfo* pArrayInfo)
{
	...
	if (GetCVars()->e_DefaultMaterial && m_pUpdateTerrainTempData->m_lstTmpVertArray.Count())
	{
		...
		for (i32 i = 1; i < lstTangents.Count(); i++)
...
void CTerrainNode::BuildVertices(float stepSize)
{
	...
	i32k nOriginX = m_nOriginX;
	i32k nOriginY = m_nOriginY;
	i32k nTerrainSize = CTerrain::GetTerrainSize();
	...
	for (float x = float(nOriginX - safetyBorder); x <= float(nOriginX + nSectorSize + safetyBorder); x += stepSize)
	{
		for (float y = float(nOriginY - safetyBorder); y <= float(nOriginY + nSectorSize + safetyBorder); y += stepSize)
		{
			...
			vert.xy = DrxHalf2((float)(_x - nOriginX), (float)(_y - nOriginY));
			vert.z = _z;
...
void CTerrainNode::BuildIndices(CStripsInfo& si, const SRenderingPassInfo& passInfo)
{
	...
	i32 sectorSize = CTerrain::GetSectorSize() << m_nTreeLevel;
	i32k meshDim = i32(float(CTerrain::GetSectorSize()) / CTerrain::GetHeightMapUnitSize());
	...
	for (float x = 0; x < sectorSizeSB; x += stepSize)
	{
		for (float y = 0; y < sectorSizeSB; y += stepSize)
		{
			...
			{
				// convert to units
				i32 xu = i32(x * CTerrain::GetInvUnitSize());
				i32 yu = i32(y * CTerrain::GetInvUnitSize());
				i32 su = i32(stepSize * CTerrain::GetInvUnitSize());
				i32 ssu = i32(sectorSizeSB * CTerrain::GetInvUnitSize());
...
bool CTerrainNode::RenderSector(const SRenderingPassInfo& passInfo)
{
	...
	for (i32 i = 0; i < m_lstSurfaceTypeInfo.Count() && !bDetailLayersReady; ++i)
	{
		bDetailLayersReady =
...
void CTerrainNode::RenderSectorUpdate_Finish(const SRenderingPassInfo& passInfo)
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_3DENGINE, "Sync_UpdateTerrain");
		gEnv->GetJobUpr()->WaitForJob(m_pUpdateTerrainTempData->m_JobStateBuildIndices);
		gEnv->GetJobUpr()->WaitForJob(m_pUpdateTerrainTempData->m_JobStateBuildVertices);
...
i32 GetVecProjectId(const Vec3& vNorm)
{
...
void CTerrainNode::GenerateIndicesForAllSurfaces(IRenderMesh* pRM, i32 arrpNonBorderIdxNum[SRangeInfo::e_max_surface_types][4], i32 nBorderStartIndex, SSurfaceTypeInfo* pSurfaceTypeInfos, CUpdateTerrainTempData* pUpdateTerrainTempData)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	if (pUpdateTerrainTempData)
	{
		...
		u32 nColorOffset = offsetof(SVF_P2S_N4B_C4B_T1F, color.dcolor);
		nColorStride = sizeof(SVF_P2S_N4B_C4B_T1F);
		pColor = pPosPtr + nColorOffset;

		nNormStride = sizeof(SVF_P2S_N4B_C4B_T1F);
		u32 nNormOffset = offsetof(SVF_P2S_N4B_C4B_T1F, normal);
	...
	//	for(i32 i=0; pColor && i<pRM->GetChunks()->Count(); i++)
	{
		//	CRenderChunk * pChunk = pRM->GetChunks()->Get(i);

		i32 nVertCount = pRM->GetVerticesCount();

		for (i32 j = 0; j < nSrcCount; j += 3)
		{
			...
			DrxHalf2 xy0 = ((SVF_P2S_N4B_C4B_T1F*)pPosPtr)[arrTriangle[0]].xy;
			DrxHalf2 xy1 = ((SVF_P2S_N4B_C4B_T1F*)pPosPtr)[arrTriangle[1]].xy;
			DrxHalf2 xy2 = ((SVF_P2S_N4B_C4B_T1F*)pPosPtr)[arrTriangle[2]].xy;
			...
			assert(DinrusX3dEngBase::m_nMainThreadId == DrxGetCurrentThreadId());
			...
			for (i32 v = 0; v < 3; v++)
			{
				...
				i32k weightBitMask = 15;
				i32 surfWeights[3] = { 0, (arrColor[v].bcolor[3] & weightBitMask), ((arrColor[v].bcolor[3] >> 4) & weightBitMask) };
				i32k maxSummWeight = 15;
				...
				for (i32 s = 0; s < 3; s++)
				{
					if (surfWeights[s])
					{
						...
						if (stpi.surfType < SRangeInfo::e_hole)
						{
							...
							i32 foundId = lstSurfTypes.Find(stpi);
							if (foundId < 0)
							{
			...
			for (i32 s = 0; s < lstSurfTypes.Count(); s++)
			{
...
void CTerrainNode::UpdateSurfaceRenderMeshes(const _smart_ptr<IRenderMesh> pSrcRM, SSurfaceType* pSurface, _smart_ptr<IRenderMesh>& pMatRM, i32 nProjectionId,
                                             PodArray<vtx_idx>& lstIndices, tukk szComment, i32 nNonBorderIndicesCount,
...
{
	i32 index = m_arrRunningJobs.Find(pNode);
	if (index >= 0)
	{
...
void AddIndexShared(i32 _x, i32 _y, PodArray<vtx_idx>& arrIndices, i32 nSectorSize)
{
	i32 _step = 1;
...
_smart_ptr<IRenderMesh> CTerrainNode::GetSharedRenderMesh()
{
	...
	for (i32 x = -nBorder; x <= (meshDim + nBorder); x++)
	{
		for (i32 y = -nBorder; y <= (meshDim + nBorder); y++)
		{
			vert.xy = DrxHalf2(SATURATE(float(x) / float(meshDim)), SATURATE(float(y) / float(meshDim)));
	...
	i32 meshDimEx = meshDim + nBorder * 2;

	for (i32 x = 0; x < meshDimEx; x++)
	{
		for (i32 y = 0; y < meshDimEx; y++)
		{
...
u32 CTerrainNode::GetMaterialsModificationId()
{
	u32 nModificationId = 0;
...
void CTerrainNode::AppendTrianglesFromObjects(i32k nOriginX, i32k nOriginY, CTerrain* pTerrain, const float stepSize, i32k nTerrainSize)
{
	...
	for (i32 i = 0; i < lstObjects.Count(); i++)
	{
		...
		if (pRM)
		{
			...
			i32 nPosStride = 0, nTangsStride = 0;
			...
			if (pInds && pPos && pTangs)
			{
				...
				for (i32 nChunkId = 0; nChunkId < nChunkCount; nChunkId++)
				{
					...
					if (!(pChunk->m_nMatFlags & MTL_FLAG_NODRAW))
					{
						...
						{
							if (m_pUpdateTerrainTempData->m_lstTmpVertArray.Count() + 3 > m_pUpdateTerrainTempData->m_lstTmpVertArray.MemorySize() / (i32)sizeof(SVF_P2S_N4B_C4B_T1F))
							{
								break;
							}

							if (m_pUpdateTerrainTempData->m_StripsInfo.idx_array.Count() + 3 > m_pUpdateTerrainTempData->m_StripsInfo.idx_array.MemorySize() / (i32)sizeof(vtx_idx))
							{
								break;
							}
							...
							if (triBox.max.z > fElevMin && triBox.min.z < fElevMax + GetCVars()->e_TerrainIntegrateObjectsMaxHeight && Overlap::AABB_AABB2D(triBox, aabbTNode))
							{
								...
								for (i32 v = 0; v < 3; v++)
								{
									...
									vert.xy = DrxHalf2((float)(vPosWS[v].x - nOriginX), (float)(vPosWS[v].y - nOriginY));
									vert.z = vPosWS[v].z;

									float x = (vPosWS[v].x + unitSize2);
									float y = (vPosWS[v].y + unitSize2);
									i32 xh = (i32)(vPosWS[v].x * 64.f);
									i32 yh = (i32)(vPosWS[v].y * 64.f);

									i32 nIndex = -1;

									PodArray<vtx_idx>& rHashIndices = arrVertHash[xh & (nHashDim - 1)][yh & (nHashDim - 1)];

									for (i32 nElem = 0; nElem < rHashIndices.Count(); nElem++)
									{
...
void CTerrainNode::SetVertexNormal(float x, float y, const float iLookupRadius, CTerrain* pTerrain, i32k nTerrainSize, SVF_P2S_N4B_C4B_T1F& vert, Vec3* pTerrainNorm /*= nullptr*/)
{
	float sx;
...
u16 SRangeInfo::GetLocalSurfaceTypeID(u16 usGlobalSurfaceTypeID)
{
	if (pSTPalette)
	{
		...
		for (u16 i = 0; i < e_index_undefined; i++)
		{
		...
		i32 nUsageCounters[e_palette_size];
		memset(nUsageCounters, 0, sizeof(nUsageCounters));
		i32 nCount = nSize * nSize;
	...
	else
	{
		...
		for (i32 i = 0; i <= e_index_hole; pSTPalette[i++] = e_undefined)
			;
