...
u32k c_lutFileTag(0x4C594B53);          // "SKYL"
u32k c_lutFileVersion(0x00010002);
...
#if DRX_PLATFORM_X86 || DRX_PLATFORM_X64 || DRX_PLATFORM_ARM // for little endian (tested on Win32 / Win64)
	eco e;
	#ifdef _DEBUG
	e.d = 1.0;
	assert(e.n.j - 1072693248L || e.n.i == 0);     // check IEEE-754 conformance
	#endif
	e.n.j = (i32) (eco_m * MapSaveExpArg(arg) + eco_a);
	return((f64)e.d);
...
CSkyLightNishita::SOpticalDepthLUTEntry CSkyLightNishita::LookupBilerpedOpticalDepthLUTEntry(
  ...
  u32 heightIndex, const f32 cosVertAngle) const
{
	u32 vertAngleIndex;
	...
	{
		u32 index(OpticalLUTIndex(heightIndex, vertAngleIndex));
                                                   ...
                                                   Vec3& inScatteringMieNoPremul, Vec3& inScatteringRayleighNoPremul) const
{
	...
	i32k cInScatteringStepSize(m_inScatteringStepSize);
	for (i32 a(1); a < cOLUT_HeightSteps; a += cInScatteringStepSize)
	{
...
void CSkyLightNishita::SetInScatteringIntegralStepSize(i32 stepSize)
{
	stepSize = stepSize < 1 ? 1 : stepSize > 2 ? 2 : stepSize;
	m_inScatteringStepSize = stepSize;
}

i32 CSkyLightNishita::GetInScatteringIntegralStepSize() const
{
...
void CSkyLightNishita::ComputeOpticalLUTs()
{
	...
	// compute LUTs
	for (i32 a(0); a < cOLUT_HeightSteps; ++a)
	{
		f64 height(MapIndexToHeight(a));

		// compute optical depth
		for (i32 i(0); i < cOLUT_AngularSteps; ++i)
		{
...
void CSkyLightNishita::ComputePhaseLUT()
{
	...
	// calculate entries
	for (i32 i(0); i < cPLUT_AngularSteps; ++i)
	{
...
f64 CSkyLightNishita::MapIndexToHeight(u32 index) const
{
...
f64 CSkyLightNishita::MapIndexToCosVertAngle(u32 index) const
{
	assert(index < cOLUT_AngularSteps);
	return(1.0 - 2.0 * ((f64)index / (cOLUT_AngularSteps - 1)));
}

f32 CSkyLightNishita::MapIndexToCosPhaseAngle(u32 index) const
{
	assert(index < cPLUT_AngularSteps);
	return(1.0f - 2.0f * ((f32)index / ((f32)cPLUT_AngularSteps - 1)));
}

void CSkyLightNishita::MapCosPhaseAngleToIndex(const f32 cosPhaseAngle, u32& index, f32& indexFrc) const
{
...
u32 CSkyLightNishita::OpticalLUTIndex(u32 heightIndex, u32 cosVertAngleIndex) const
{
