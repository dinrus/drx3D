...
//  Описание: Реализация методов интерфейса I3DEngine.
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
#include <drx3D/Eng3D/CloudBlockerRenderNode.h>
...
threadID DinrusX3dEngBase::m_nMainThreadId = 0;
bool DinrusX3dEngBase::m_bRenderTypeEnabled[eERType_TypesNum];
ISystem* DinrusX3dEngBase::m_pSystem = 0;
#if !defined(DEDICATED_SERVER)
IRenderer* DinrusX3dEngBase::m_pRenderer = 0;
#else
IRenderer* const DinrusX3dEngBase::m_pRenderer = 0;
#endif
ITimer* DinrusX3dEngBase::m_pTimer = 0;
ILog* DinrusX3dEngBase::m_pLog = 0;
IPhysicalWorld* DinrusX3dEngBase::m_pPhysicalWorld = 0;
CTerrain* DinrusX3dEngBase::m_pTerrain = 0;
CObjUpr* DinrusX3dEngBase::m_pObjUpr = 0;
IConsole* DinrusX3dEngBase::m_pConsole = 0;
C3DEngine* DinrusX3dEngBase::m_p3DEngine = 0;
CVars* DinrusX3dEngBase::m_pCVars = 0;
IDrxPak* DinrusX3dEngBase::m_pDrxPak = 0;
IParticleUpr* DinrusX3dEngBase::m_pPartUpr = 0;
IOpticsUpr* DinrusX3dEngBase::m_pOpticsUpr = 0;
CDecalUpr* DinrusX3dEngBase::m_pDecalUpr = 0;
CSkyLightUpr* DinrusX3dEngBase::m_pSkyLightUpr = 0;
CVisAreaUpr* DinrusX3dEngBase::m_pVisAreaUpr = 0;
CClipVolumeUpr* DinrusX3dEngBase::m_pClipVolumeUpr = 0;
CWaterWaveUpr* DinrusX3dEngBase::m_pWaterWaveUpr = 0;
CRenderMeshMerger* DinrusX3dEngBase::m_pRenderMeshMerger = 0;
CMatMan* DinrusX3dEngBase::m_pMatMan = 0;
CMergedMeshesUpr* DinrusX3dEngBase::m_pMergedMeshesUpr = 0;
CBreezeGenerator* DinrusX3dEngBase::m_pBreezeGenerator = 0;
IStreamedObjectListener* DinrusX3dEngBase::m_pStreamListener = 0;
#if defined(USE_GEOM_CACHES)
CGeomCacheUpr* DinrusX3dEngBase::m_pGeomCacheUpr = 0;
#endif

bool DinrusX3dEngBase::m_bProfilerEnabled = false;
bool DinrusX3dEngBase::m_bLevelLoadingInProgress = false;
bool DinrusX3dEngBase::m_bIsInRenderScene = false;
i32 DinrusX3dEngBase::m_mergedMeshesPoolSize = 0;

#if DRX_PLATFORM_DESKTOP
bool DinrusX3dEngBase::m_bEditor = false;
#endif
ESystemConfigSpec DinrusX3dEngBase::m_LightConfigSpec = CONFIG_VERYHIGH_SPEC;
i32 DinrusX3dEngBase::m_arrInstancesCounter[eERType_TypesNum];
IEditorHeightmap* C3DEngine::m_pEditorHeightmap = 0;

#define LAST_POTENTIALLY_VISIBLE_TIME 2

// Единственная прямая функция частицы, необходимая для 3DEngine, реализованная в этой же DLL.
extern IParticleUpr* CreateParticleUpr(bool bEnable);
extern void              DestroyParticleUpr(IParticleUpr* pParticleUpr);
...
public:
	...
	virtual void Warning(tukk format) { DinrusX3dEngBase::Warning("%s", format); }
	virtual void Error(tukk format)   { DinrusX3dEngBase::Error("%s", format); }
	virtual bool IsValidationEnabled()       { return DinrusX3dEngBase::GetCVars()->e_StatObjValidate != 0; }
};
}
...
C3DEngine::C3DEngine(ISystem* pSystem)
{
	...
	DinrusX3dEngBase::m_nMainThreadId = DrxGetCurrentThreadId();
	DinrusX3dEngBase::m_pSystem = pSystem;
#if !defined(DEDICATED_SERVER)
	DinrusX3dEngBase::m_pRenderer = gEnv->pRenderer;
#endif
	DinrusX3dEngBase::m_pTimer = gEnv->pTimer;
	DinrusX3dEngBase::m_pLog = gEnv->pLog;
	DinrusX3dEngBase::m_pPhysicalWorld = gEnv->pPhysicalWorld;
	DinrusX3dEngBase::m_pConsole = gEnv->pConsole;
	DinrusX3dEngBase::m_p3DEngine = this;
	DinrusX3dEngBase::m_pDrxPak = gEnv->pDrxPak;
	DinrusX3dEngBase::m_pCVars = 0;
	DinrusX3dEngBase::m_pRenderMeshMerger = new CRenderMeshMerger;
	DinrusX3dEngBase::m_pMergedMeshesUpr = new CMergedMeshesUpr;
	DinrusX3dEngBase::m_pMatMan = new CMatMan;
	DinrusX3dEngBase::m_pBreezeGenerator = new CBreezeGenerator;
	DinrusX3dEngBase::m_pStreamListener = NULL;

	memset(DinrusX3dEngBase::m_arrInstancesCounter, 0, sizeof(DinrusX3dEngBase::m_arrInstancesCounter));
	...
	m_pDecalUpr = 0;    //new CDecalUpr   (m_pSystem, this);
	m_pPartUpr = 0;
	m_pOpticsUpr = 0;
	m_pVisAreaUpr = 0;
	m_pClipVolumeUpr = new CClipVolumeUpr();
	m_pSkyLightUpr = DrxAlignedNew<CSkyLightUpr>();
	m_pWaterWaveUpr = 0;
	m_pWaterRippleUpr.reset(new CWaterRippleUpr());
	...
	m_pDeferredPhysicsEventUpr = new CDeferredPhysicsEventUpr();

#if defined(USE_GEOM_CACHES)
	m_pGeomCacheUpr = new CGeomCacheUpr();
#endif
...
C3DEngine::~C3DEngine()
{
	...
	SAFE_DELETE(m_pClipVolumeUpr);

	DrxAlignedDelete(m_pSkyLightUpr);
	m_pSkyLightUpr = 0;
	...
	delete m_pDeferredPhysicsEventUpr;
}

//////////////////////////////////////////////////////////////////////

void C3DEngine::RemoveEntInFoliage(i32 i, IPhysicalEntity* pent)
{
	...
	i32 j = m_arrEntsInFoliage.size() - 1;
	if (i < j)
	{
...
bool C3DEngine::Init()
{
	m_pPartUpr = CreateParticleUpr(!gEnv->IsDedicated());
	m_pSystem->SetIParticleUpr(m_pPartUpr);

	m_pOpticsUpr = new COpticsUpr;
	m_pSystem->SetIOpticsUpr(m_pOpticsUpr);

	DRX_ASSERT(m_pWaterRippleUpr);
	m_pWaterRippleUpr->Initialize();

	for (i32 i = 0; i < eERType_TypesNum; i++)
	{
	...
	DRX_ASSERT_MESSAGE(tempPoolSize != 0, "C3DEngine::Init() temp pool size is set to 0");

	{
		...
		{
			DrxFatalError("C3DEngine::Init() could not initialize temporary pool");
			return false;
		}
	}
	...
	DRX_ASSERT(frameLodInfo.fTargetSize > 0.f);
...
bool C3DEngine::IsCameraAnd3DEngineInvalid(const SRenderingPassInfo& passInfo, tukk szCaller)
{
	if (!m_pObjUpr)
	{
		return (true);
...
void C3DEngine::OnFrameStart()
{
	...
	if (m_pPartUpr)
		m_pPartUpr->OnFrameStart();

	m_nRenderWorldUSecs = 0;
	m_pDeferredPhysicsEventUpr->Update();

#if defined(USE_GEOM_CACHES)
	if (m_pGeomCacheUpr && !m_bLevelLoadingInProgress)
	{
		m_pGeomCacheUpr->StreamingUpdate();
	}
#endif

	UpdateWindAreas();

	if (m_pWaterRippleUpr)
	{
		m_pWaterRippleUpr->OnFrameStart();
	}
}

float g_oceanLevel, g_oceanStep;
float GetOceanLevelCallback(i32 ix, i32 iy)
{
...
u8 GetOceanSurfTypeCallback(i32 ix, i32 iy)
{
	return 0;
}
...
void C3DEngine::Update()
{
	...
	///	if(m_pVisAreaUpr)
	//	m_pVisAreaUpr->Preceche(m_pObjUpr);

	if (GetObjUpr())
		GetObjUpr()->ClearStatObjGarbage();
	...
	if (m_pDecalUpr)
		m_pDecalUpr->Update(GetTimer()->GetFrameTime());
	...
	if ((pArea = gEnv->pPhysicalWorld->AddGlobalArea())->GetParams(&pa))
	{
		...
		if (bOceanEnabled && (!pa.pGeom || g_oceanStep != GetFloatCVar(e_PhysOceanCell)))
		{
			...
			hf.size.set((i32)(8192 / g_oceanStep), (i32)(8192 / g_oceanStep));
			...
			hf.typehole = 255;
			hf.heightscale = 1.0f;
			pa.pGeom = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::heightfield::type, &hf);
			pArea->SetParams(&pa);
...
{
	static i32 nObjectLayersActivation = -1;
	...
	if (m_fRefreshSceneDataCVarsSumm != -1 && m_fRefreshSceneDataCVarsSumm != fNewCVarsSumm)
	{
		...
		GetObjUpr()->ReregisterEntitiesInArea(nullptr, true);
	...
	i32 nRenderTypeEnableCVarSum = (GetCVars()->e_Vegetation << 0)
...

	{
		...
		if (m_bEditor)
		{
			fNewCVarsSumm2 += float(i32(GetSkyColor().GetLength() * 10) / 10);
		}
...
void C3DEngine::ShutDown()
{
	if (GetRenderer() != GetSystem()->GetIRenderer())
		DrxFatalError("Renderer was deallocated before I3DEngine::ShutDown() call");
...
#if defined(USE_GEOM_CACHES)
	delete m_pGeomCacheUpr;
	m_pGeomCacheUpr = 0;
#endif

	DestroyParticleUpr(m_pPartUpr);
	m_pPartUpr = nullptr;
	m_pSystem->SetIParticleUpr(0);
	...
	DRX_ASSERT(m_pWaterRippleUpr);
	m_pWaterRippleUpr->Finalize();
	m_pWaterRippleUpr.reset();

	DrxAlignedDelete(m_pObjUpr);
	m_pObjUpr = 0;
	...
	{
		DrxFatalError("C3DEngine::Shutdown() could not shutdown temporary pool");
	}
}
...
void C3DEngine::ProcessStreamingLatencyTest(const CCamera& camIn, CCamera& camOut, const SRenderingPassInfo& passInfo)
{
	...
	static i32 nOpenRequestCount = 0;
	...
	else if (GetCVars()->e_SQTestBegin == 3)
	{
		...
		if (param.pTextures)
		{
			for (u32 i = 0; i < param.numTextures; i++)
			{
				ITexture* pTexture = param.pTextures[i];
				if (pTexture->GetAccessFrameId() > (i32)(passInfo.GetMainFrameID() - 4))
	...
	else if (GetCVars()->e_SQTestBegin == 7)
	{
		...
		{
			i32 nFinishedNum = 0;
			for (i32 i = 0; i < arrTestTextures.Count(); i++)
			{
			...
			if (nFinishedNum == arrTestTextures.Count())
			{
				...
				{
					tukk szTestResults = "%USER%/TestResults";
					char path[IDrxPak::g_nMaxPath] = "";
					gEnv->pDrxPak->AdjustFileName(string(string(szTestResults) + "\\" + "Streaming_Latency_Test.xml").c_str(), path, IDrxPak::FLAGS_PATH_REAL | IDrxPak::FLAGS_FOR_WRITING);
					gEnv->pDrxPak->MakeDir(szTestResults);

					if (FILE* f = ::fopen(path, "wb"))
					{
						float fAverTime = 0;
						for (i32 i = 0; i < m_arrProcessStreamingLatencyTestResults.Count(); i++)
							fAverTime += m_arrProcessStreamingLatencyTestResults[i];
						fAverTime /= m_arrProcessStreamingLatencyTestResults.Count();

						i32 nAverTexNum = 0;
						for (i32 i = 0; i < m_arrProcessStreamingLatencyTexNum.Count(); i++)
...
//////////////////////////////////////////////////////////////////////
void C3DEngine::UpdateRenderingCamera(tukk szCallerName, const SRenderingPassInfo& passInfo)
{
	CCamera newCam = passInfo.GetCamera();

#if defined(FEATURE_SVO_GI)
	if (passInfo.IsGeneralPass())
		CSvoUpr::Update(passInfo, newCam);
#endif
...

	{
		...
		tukk const pCamGoto = GetCVars()->e_CameraGoto->GetString();
		assert(pCamGoto);
		if (strlen(pCamGoto) > 1)
		{
			Ang3 aAngDeg;
			Vec3 vPos;
			i32 args = sscanf(pCamGoto, "%f %f %f %f %f %f", &vPos.x, &vPos.y, &vPos.z, &aAngDeg.x, &aAngDeg.y, &aAngDeg.z);
			if (args >= 3)
			{
				...
				{
					char buf[128];
					drx_sprintf(buf, "goto %f %f %f", vPos.x, vPos.y, vPos.z);
	...
	if (passInfo.IsGeneralPass() && IsStatObjBufferRenderTasksAllowed() && JobUpr::InvokeAsJob("CheckOcclusion"))
	{
		if (gEnv->IsEditor())
			GetObjUpr()->PrepareCullbufferAsync(passInfo.GetCamera());
		else
			assert(IsEquivalent(passInfo.GetCamera().GetViewdir(), GetObjUpr()->m_CullThread.GetViewDir())); // early set camera differs from current main camera - will cause occlusion errors
	}
	...
	for (i32 i = m_arrEntsInFoliage.size() - 1; i >= 0; i--)
	...
	for (i32 i = 0, nSize = m_deferredRenderProxyStreamingPriorityUpdates.size(); i < nSize; ++i)
	{
		...
		GetObjUpr()->UpdateRenderNodeStreamingPriority(pRenderNode, fEntDistance, 1.0f, false, passInfo);
		if (GetCVars()->e_StreamCgfDebug == 2)
			PrintMessage("C3DEngine::RegisterEntity__GetObjUpr()->UpdateRenderNodeStreamingPriority %s", pRenderNode->GetName());
	}
...
void C3DEngine::PrepareOcclusion(const CCamera& rCamera)
{
	...
	if (!bInEditor && bStatObjBufferRenderTasks && !bIsFMVPlaying && (!bCameraAtZero || bPost3dEnabled))
		GetObjUpr()->PrepareCullbufferAsync(rCamera);
}

void C3DEngine::EndOcclusion()
{
	GetObjUpr()->EndOcclusionCulling();
}
...
IStatObj* C3DEngine::LoadStatObj(tukk szFileName, tukk szGeomName, IStatObj::SSubObject** ppSubObject, bool bUseStreaming, u64 nLoadingFlags)
{
	...
	return m_pObjUpr->LoadStatObj(szFileName, szGeomName, ppSubObject, bUseStreaming, nLoadingFlags);
}

IStatObj* C3DEngine::FindStatObjectByFilename(tukk filename)
{
	if (filename == NULL)
		return NULL;
	...
	return m_pObjUpr->FindStaticObjectByFilename(filename);
}

void C3DEngine::RegisterEntity(IRenderNode* pEnt)
{
	FUNCTION_PROFILER_3DENGINE;
	u32 nFrameID = gEnv->nMainFrameID;
...
{
	AsyncOctreeUpdate(pEnt, (i32)0, true);
}

bool C3DEngine::CreateDecalInstance(const DinrusXDecalInfo& decal, CDecal* pCallerManagedDecal)
{
	if (!GetCVars()->e_Decals && !pCallerManagedDecal)
		return false;

	return m_pDecalUpr->Spawn(decal, pCallerManagedDecal);
}
...
void C3DEngine::CreateDecal(const struct DinrusXDecalInfo& decal)
{
	...
	{
		DinrusXDecalInfo decal_adjusted = decal;
		decal_adjusted.nGroupId = nGroupId;
		decal_adjusted.bDeferred = true;
		m_pDecalUpr->SpawnHierarchical(decal_adjusted, NULL);
		return;
	}
	...
	if (decal.ownerInfo.pRenderNode && decal.fSize > 0.5f && !decal.bForceSingleOwner)
	{
		...
		for (i32 i = 0; i < lstEntities.Count(); i++)
		{
			...
			DinrusXDecalInfo decalOnRenderNode = decal;
			...
			m_pDecalUpr->SpawnHierarchical(decalOnRenderNode, NULL);
		}
	}
	else
	{
		DinrusXDecalInfo decalStatic = decal;
		decalStatic.nGroupId = nGroupId;
		m_pDecalUpr->SpawnHierarchical(decalStatic, NULL);
	}
}
...
void C3DEngine::SetSkyColor(Vec3 vColor)
{
	if (m_pObjUpr)
	{
		m_pObjUpr->m_vSkyColor = vColor;
		m_pObjUpr->m_fSunSkyRel = GetSunSkyRel(m_pObjUpr->m_vSkyColor, m_pObjUpr->m_vSunColor);
	}
}
...
void C3DEngine::SetTerrainSurfaceType(i32 x, i32 y, i32 nType)
{
	assert(0);
}

void C3DEngine::SetTerrainSectorTexture(i32k nTexSectorX, i32k nTexSectorY, u32 textureId)
{
	if (m_pTerrain)
	{
...
{
	CSkyLightUpr::SSkyDomeCondition skyCond;
	m_pSkyLightUpr->GetCurSkyDomeCondition(skyCond);
...
void C3DEngine::SetSkyLightParameters(const Vec3& sunDir, const Vec3& sunIntensity, float Km, float Kr, float g, const Vec3& rgbWaveLengths, bool forceImmediateUpdate)
{
	...
	m_pSkyLightUpr->SetSkyDomeCondition(skyCond);
	if (forceImmediateUpdate && IsHDRSkyMaterial(GetSkyMaterial()))
		m_pSkyLightUpr->FullUpdate();
}
...
{
	return m_pObjUpr ? m_pObjUpr->m_vSunColor : Vec3(0, 0, 0);
}

float C3DEngine::GetSkyBrightness() const
{
	return m_pObjUpr ? m_pObjUpr->m_fSkyBrightMul : 1.0f;
}

float C3DEngine::GetSSAOAmount() const
{
	return m_pObjUpr ? m_pObjUpr->m_fSSAOAmount : 1.0f;
}

float C3DEngine::GetSSAOContrast() const
{
	return m_pObjUpr ? m_pObjUpr->m_fSSAOContrast : 1.0f;
}

float C3DEngine::GetGIAmount() const
{
	return m_pObjUpr ? m_pObjUpr->m_fGIAmount : 1.0f;
}

float C3DEngine::GetSunRel() const
{
	return m_pObjUpr->m_fSunSkyRel;
}
...
void C3DEngine::SetRainParams(const SRainParams& rainParams)
{
	if (m_pObjUpr)
	{
		m_pObjUpr->m_rainParams.bIgnoreVisareas = rainParams.bIgnoreVisareas;
		m_pObjUpr->m_rainParams.bDisableOcclusion = rainParams.bDisableOcclusion;
		m_pObjUpr->m_rainParams.qRainRotation = rainParams.qRainRotation;
		m_pObjUpr->m_rainParams.vWorldPos = rainParams.vWorldPos;
		m_pObjUpr->m_rainParams.vColor = rainParams.vColor;
		m_pObjUpr->m_rainParams.fAmount = rainParams.fAmount;
		m_pObjUpr->m_rainParams.fCurrentAmount = rainParams.fCurrentAmount;
		m_pObjUpr->m_rainParams.fRadius = rainParams.fRadius;
		m_pObjUpr->m_rainParams.fFakeGlossiness = rainParams.fFakeGlossiness;
		m_pObjUpr->m_rainParams.fFakeReflectionAmount = rainParams.fFakeReflectionAmount;
		m_pObjUpr->m_rainParams.fDiffuseDarkening = rainParams.fDiffuseDarkening;
		m_pObjUpr->m_rainParams.fRainDropsAmount = rainParams.fRainDropsAmount;
		m_pObjUpr->m_rainParams.fRainDropsSpeed = rainParams.fRainDropsSpeed;
		m_pObjUpr->m_rainParams.fRainDropsLighting = rainParams.fRainDropsLighting;
		m_pObjUpr->m_rainParams.fMistAmount = rainParams.fMistAmount;
		m_pObjUpr->m_rainParams.fMistHeight = rainParams.fMistHeight;
		m_pObjUpr->m_rainParams.fPuddlesAmount = rainParams.fPuddlesAmount;
		m_pObjUpr->m_rainParams.fPuddlesMaskAmount = rainParams.fPuddlesMaskAmount;
		m_pObjUpr->m_rainParams.fPuddlesRippleAmount = rainParams.fPuddlesRippleAmount;
		m_pObjUpr->m_rainParams.fSplashesAmount = rainParams.fSplashesAmount;

		m_pObjUpr->m_rainParams.nUpdateFrameID = gEnv->nMainFrameID;
	}
}
...
{
	bool bRet = false;
	i32k nFrmID = gEnv->nMainFrameID;
	if (m_pObjUpr)
	{
		// Copy shared rain data only
		rainParams.bIgnoreVisareas = m_pObjUpr->m_rainParams.bIgnoreVisareas;
		rainParams.bDisableOcclusion = m_pObjUpr->m_rainParams.bDisableOcclusion;
		rainParams.qRainRotation = m_pObjUpr->m_rainParams.qRainRotation;
		rainParams.vWorldPos = m_pObjUpr->m_rainParams.vWorldPos;
		rainParams.vColor = m_pObjUpr->m_rainParams.vColor;
		rainParams.fAmount = m_pObjUpr->m_rainParams.fAmount;
		rainParams.fCurrentAmount = m_pObjUpr->m_rainParams.fCurrentAmount;
		rainParams.fRadius = m_pObjUpr->m_rainParams.fRadius;
		rainParams.fFakeGlossiness = m_pObjUpr->m_rainParams.fFakeGlossiness;
		rainParams.fFakeReflectionAmount = m_pObjUpr->m_rainParams.fFakeReflectionAmount;
		rainParams.fDiffuseDarkening = m_pObjUpr->m_rainParams.fDiffuseDarkening;
		rainParams.fRainDropsAmount = m_pObjUpr->m_rainParams.fRainDropsAmount;
		rainParams.fRainDropsSpeed = m_pObjUpr->m_rainParams.fRainDropsSpeed;
		rainParams.fRainDropsLighting = m_pObjUpr->m_rainParams.fRainDropsLighting;
		rainParams.fMistAmount = m_pObjUpr->m_rainParams.fMistAmount;
		rainParams.fMistHeight = m_pObjUpr->m_rainParams.fMistHeight;
		rainParams.fPuddlesAmount = m_pObjUpr->m_rainParams.fPuddlesAmount;
		rainParams.fPuddlesMaskAmount = m_pObjUpr->m_rainParams.fPuddlesMaskAmount;
		rainParams.fPuddlesRippleAmount = m_pObjUpr->m_rainParams.fPuddlesRippleAmount;
		rainParams.fSplashesAmount = m_pObjUpr->m_rainParams.fSplashesAmount;
		...
		bRet = m_pObjUpr->m_rainParams.nUpdateFrameID == nFrmID;
	}
	return bRet;
}
...
void C3DEngine::SetSnowSurfaceParams(const Vec3& vCenter, float fRadius, float fSnowAmount, float fFrostAmount, float fSurfaceFreezing)
{
	if (m_pObjUpr)
	{
		m_pObjUpr->m_snowParams.m_vWorldPos = vCenter;
		m_pObjUpr->m_snowParams.m_fRadius = fRadius;
		m_pObjUpr->m_snowParams.m_fSnowAmount = fSnowAmount;
		m_pObjUpr->m_snowParams.m_fFrostAmount = fFrostAmount;
		m_pObjUpr->m_snowParams.m_fSurfaceFreezing = fSurfaceFreezing;
	}
}
...
bool C3DEngine::GetSnowSurfaceParams(Vec3& vCenter, float& fRadius, float& fSnowAmount, float& fFrostAmount, float& fSurfaceFreezing)
{
	...
	if (m_pObjUpr)
	{
		vCenter = m_pObjUpr->m_snowParams.m_vWorldPos;
		fRadius = m_pObjUpr->m_snowParams.m_fRadius;
		fSnowAmount = 0.f;
		fFrostAmount = 0.f;
		fSurfaceFreezing = 0.f;
		if (IsOutdoorVisible())
		{
			fSnowAmount = m_pObjUpr->m_snowParams.m_fSnowAmount;
			fFrostAmount = m_pObjUpr->m_snowParams.m_fFrostAmount;
			fSurfaceFreezing = m_pObjUpr->m_snowParams.m_fSurfaceFreezing;
		}
		bRet = true;
	}
	return bRet;
...
void C3DEngine::SetSnowFallParams(i32 nSnowFlakeCount, float fSnowFlakeSize, float fSnowFallBrightness, float fSnowFallGravityScale, float fSnowFallWindScale, float fSnowFallTurbulence, float fSnowFallTurbulenceFreq)
{
	if (m_pObjUpr)
	{
		m_pObjUpr->m_snowParams.m_nSnowFlakeCount = nSnowFlakeCount;
		m_pObjUpr->m_snowParams.m_fSnowFlakeSize = fSnowFlakeSize;
		m_pObjUpr->m_snowParams.m_fSnowFallBrightness = fSnowFallBrightness;
		m_pObjUpr->m_snowParams.m_fSnowFallGravityScale = fSnowFallGravityScale;
		m_pObjUpr->m_snowParams.m_fSnowFallWindScale = fSnowFallWindScale;
		m_pObjUpr->m_snowParams.m_fSnowFallTurbulence = fSnowFallTurbulence;
		m_pObjUpr->m_snowParams.m_fSnowFallTurbulenceFreq = fSnowFallTurbulenceFreq;
	}
}

bool C3DEngine::GetSnowFallParams(i32& nSnowFlakeCount, float& fSnowFlakeSize, float& fSnowFallBrightness, float& fSnowFallGravityScale, float& fSnowFallWindScale, float& fSnowFallTurbulence, float& fSnowFallTurbulenceFreq)
{
	bool bRet = false;
	if (m_pObjUpr)
	{
		...
		if (IsOutdoorVisible())
		{
			nSnowFlakeCount = m_pObjUpr->m_snowParams.m_nSnowFlakeCount;
			fSnowFlakeSize = m_pObjUpr->m_snowParams.m_fSnowFlakeSize;
			fSnowFallBrightness = m_pObjUpr->m_snowParams.m_fSnowFallBrightness;
			fSnowFallGravityScale = m_pObjUpr->m_snowParams.m_fSnowFallGravityScale;
			fSnowFallWindScale = m_pObjUpr->m_snowParams.m_fSnowFallWindScale;
			fSnowFallTurbulence = m_pObjUpr->m_snowParams.m_fSnowFallTurbulence;
			fSnowFallTurbulenceFreq = m_pObjUpr->m_snowParams.m_fSnowFallTurbulenceFreq;
		}
		bRet = true;
	}
	return bRet;
...
Vec3 C3DEngine::GetAmbientColorFromPosition(const Vec3& vPos, float fRadius)
{
	...
	if (m_pObjUpr)
	{
		if (CVisArea* pVisArea = (CVisArea*)m_pVisAreaUpr->GetVisAreaFromPos(vPos))
...
void C3DEngine::FreeRenderNodeState(IRenderNode* pEnt)
{
	...
	i32 nElementID = m_deferredRenderProxyStreamingPriorityUpdates.Find(pEnt);
	if (nElementID != -1)
		m_deferredRenderProxyStreamingPriorityUpdates.DeleteFastUnsorted(nElementID);

	m_pObjUpr->RemoveFromRenderAllObjectDebugInfo(pEnt);
	...
	if (m_pDecalUpr && (pEnt->m_nInternalFlags & IRenderNode::DECAL_OWNER))
		m_pDecalUpr->OnEntityDeleted(pEnt);
	...
	m_visibleNodesUpr.OnRenderNodeDeleted(pEnt);
}

tukk C3DEngine::GetLevelFilePath(tukk szFileName)
{
	drx_strcpy(m_sGetLevelFilePathTmpBuff, m_szLevelFolder);
	if (*szFileName && (*szFileName == '/' || *szFileName == '\\'))
		drx_strcat(m_sGetLevelFilePathTmpBuff, szFileName + 1);
	else
		drx_strcat(m_sGetLevelFilePathTmpBuff, szFileName);
	return m_sGetLevelFilePathTmpBuff;
}

void C3DEngine::SetTerrainBurnedOut(i32 x, i32 y, bool bBurnedOut)
{
	assert(!"not supported");
}

bool C3DEngine::IsTerrainBurnedOut(i32 x, i32 y)
{
	assert(!"not supported");
	return 0;//m_pTerrain ? m_pTerrain->IsBurnedOut(x, y) : 0;
}

i32 C3DEngine::GetTerrainSectorSize()
{
	return m_pTerrain ? m_pTerrain->GetSectorSize() : 0;
}

void C3DEngine::ActivatePortal(const Vec3& vPos, bool bActivate, tukk szEntityName)
{
	if (m_pVisAreaUpr)
		m_pVisAreaUpr->ActivatePortal(vPos, bActivate, szEntityName);
}

void C3DEngine::ActivateOcclusionAreas(IVisAreaTestCallback* pTest, bool bActivate)
{
	if (m_pVisAreaUpr)
		m_pVisAreaUpr->ActivateOcclusionAreas(pTest, bActivate);
}

bool C3DEngine::SetStatInstGroup(i32 nGroupId, const IStatInstGroup& siGroup)
{
	m_fRefreshSceneDataCVarsSumm = -100;

	m_pObjUpr->m_lstStaticTypes.resize(max(nGroupId + 1, m_pObjUpr->m_lstStaticTypes.Count()));

	if (nGroupId < 0 || nGroupId >= m_pObjUpr->m_lstStaticTypes.Count())
		return false;

	StatInstGroup& rGroup = m_pObjUpr->m_lstStaticTypes[nGroupId];

	rGroup.pStatObj = siGroup.pStatObj;

	if (rGroup.pStatObj)
		drx_strcpy(rGroup.szFileName, siGroup.pStatObj->GetFilePath());
	else
	...
	if (gEnv->IsEditor() && pMaterial != rGroup.pMaterial)
		m_pMergedMeshesUpr->ResetActiveNodes();
...
bool C3DEngine::GetStatInstGroup(i32 nGroupId, IStatInstGroup& siGroup)
{
	if (nGroupId < 0 || nGroupId >= m_pObjUpr->m_lstStaticTypes.Count())
		return false;
	...
	if (siGroup.pStatObj)
		drx_strcpy(siGroup.szFileName, rGroup.pStatObj->GetFilePath());
...
void C3DEngine::UpdateStatInstGroups()
{
	...
	PodArray<StatInstGroup>& rGroupTable = m_pObjUpr->m_lstStaticTypes;
...
void C3DEngine::GetMemoryUsage(class IDrxSizer* pSizer) const
{
...

	{
		...
		pSizer->AddObject(m_pOpticsUpr);
	}

	{
		SIZER_COMPONENT_NAME(pSizer, "SkyLightUpr");
		pSizer->AddObject(m_pSkyLightUpr);
	}

	{
		SIZER_COMPONENT_NAME(pSizer, "DecalUpr");
		pSizer->AddObject(m_pDecalUpr);
	}
	...
	{
		SIZER_COMPONENT_NAME(pSizer, "ObjUpr");
		pSizer->AddObject(m_pObjUpr);
	}
...

	{
		...
		pSizer->AddObject(m_pVisAreaUpr);
	}

	{
		SIZER_COMPONENT_NAME(pSizer, "ClipVolumes");
		pSizer->AddObject(m_pClipVolumeUpr);
	}
...

	{
		...
		pSizer->AddObject(m_pWaterRippleUpr.get());
	}
}

void C3DEngine::GetResourceMemoryUsage(IDrxSizer* pSizer, const AABB& cstAABB)
{
	//////////////////////////////////////////////////////////////////////////
...
void C3DEngine::AddWaterRipple(const Vec3& vPos, float scale, float strength)
{
	if (m_pWaterRippleUpr)
	{
		m_pWaterRippleUpr->AddWaterRipple(vPos, scale, strength);
	}
}
...
void C3DEngine::SetOceanRenderFlags(u8 nFlags)
{
	m_nOceanRenderFlags = nFlags;
}

u32 C3DEngine::GetOceanVisiblePixelsCount() const
{
	return COcean::GetVisiblePixelsCount();
}

float C3DEngine::GetBottomLevel(const Vec3& referencePos, float maxRelevantDepth, i32 objflags)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	ray_hit hit;
	i32 rayFlags = geom_colltype_player << rwi_colltype_bit | rwi_stop_at_pierceable;
...
float C3DEngine::GetBottomLevel(const Vec3& referencePos, i32 objflags)
{
	return GetBottomLevel(referencePos, 10.0f, objflags);
}

#if defined(USE_GEOM_CACHES)
IGeomCache* C3DEngine::LoadGeomCache(tukk szFileName)
{
	if (!szFileName || !szFileName[0])
	{
	...
	return m_pGeomCacheUpr->LoadGeomCache(szFileName);
}

IGeomCache* C3DEngine::FindGeomCacheByFilename(tukk szFileName)
{
	...
	return m_pGeomCacheUpr->FindGeomCacheByFilename(szFileName);
}
#endif
...
i32 ReadFromBuffer(tukk pSource, i32 nSourceSize, i32 nSourcePos, uk pDest, i32 nDestSize)
{
...
//! Code\Sandbox\Plugins\DrxDesigner\Core\ModelCompiler.cpp, function ModelCompiler::SaveMesh
IStatObj* C3DEngine::LoadDesignerObject(i32 nVersion, tukk szBinaryStream, i32 size)
{
	...
	nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nSubObjectCount, sizeof(i32));
	...
	if (nVersion == 2)
	{
		i32 nStaticObjFlags = 0;
		nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nStaticObjFlags, sizeof(i32));
	...
	for (i32 k = 0, iCount(statObjList.size()); k < iCount; ++k)
	{
		...
		nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nPositionCount, sizeof(i32));
		nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nTexCoordCount, sizeof(i32));
		if (nPositionCount <= 0 || nTexCoordCount <= 0)
			return NULL;

		if (nVersion == 0)
		{
			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nFaceCount, sizeof(i32));
			if (nFaceCount <= 0)
				return NULL;
		}
		else if (nVersion >= 1)
		{
			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nIndexCount, sizeof(i32));
			if (nIndexCount <= 0)
				return NULL;
			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nTangentCount, sizeof(i32));
		}
		nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nSubsetCount, sizeof(i32));
		IIndexedMesh* pMesh = statObjList[k]->GetIndexedMesh();
		if (!pMesh)
			return NULL;

		pMesh->FreeStreams();
		pMesh->SetVertexCount((i32)nPositionCount);
		pMesh->SetFaceCount((i32)nFaceCount);
		pMesh->SetIndexCount((i32)nIndexCount);
		pMesh->SetTexCoordCount((i32)nTexCoordCount);
		...
		else if (nVersion >= 1)
		{
			...
			if (sizeof(vtx_idx) == sizeof(u16))
			{
				nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, indices, sizeof(u16) * nIndexCount);
			}
			else
			{
				u16* indices16 = new u16[nIndexCount];
				nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, indices16, sizeof(u16) * nIndexCount);
				for (i32 i = 0; i < nIndexCount; ++i)
					indices[i] = (vtx_idx)indices16[i];
				delete[] indices16;
			}
			pMesh->SetTangentCount((i32)nTangentCount);
			if (nTangentCount > 0)
			{
		...
		// optimization for AABB loading - keep ifdef temporarily to keep  changes for after GDC16
#if 0
		else
		{
		...
		for (i32 i = 0; i < nSubsetCount; ++i)
		{
			...
			i32 nMatID = 0;
			i32 nMatFlags = 0;
			i32 nPhysicalizeType = 0;

			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nFirstIndexId, sizeof(i32));
			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nNumIndices, sizeof(i32));
			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nFirstVertId, sizeof(i32));
			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nNumVerts, sizeof(i32));
			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nMatID, sizeof(i32));
			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nMatFlags, sizeof(i32));
			nBufferPos = ReadFromBuffer(szBinaryStream, size, nBufferPos, &nPhysicalizeType, sizeof(i32));

			pMesh->SetSubsetBounds(i, subset.vCenter, subset.fRadius);
			pMesh->SetSubsetIndexVertexRanges(i, (i32)nFirstIndexId, (i32)nNumIndices, (i32)nFirstVertId, (i32)nNumVerts);
			pMesh->SetSubsetMaterialId(i, nMatID == -1 ? 0 : (i32)nMatID);
			pMesh->SetSubsetMaterialProperties(i, (i32)nMatFlags, (i32)nPhysicalizeType);
		}
...
float C3DEngine::GetWaterLevel(const Vec3* pvPos, IPhysicalEntity* pent, bool bAccurate)
{
	...
	bool bInVisarea = m_pVisAreaUpr && m_pVisAreaUpr->GetVisAreaFromPos(*pvPos) != 0;

	Vec3 gravity;
	pe_params_buoyancy pb[4];
	i32 i, nBuoys = m_pPhysicalWorld->CheckAreas(*pvPos, gravity, pb, 4, -1, ZERO, pent);
...
float C3DEngine::GetAccurateOceanHeight(const Vec3& pCurrPos) const
{
	...
	static i32 nFrameID = -1;
	i32k nEngineFrameID = gEnv->nMainFrameID;
...
{
	static i32 nFrameID = -1;
	i32 nCurrFrameID = gEnv->nMainFrameID;
...
{
	return m_pObjUpr ? m_pVisAreaUpr->CreateVisArea(visGUID) : 0;
}

void C3DEngine::DeleteVisArea(IVisArea* pVisArea)
{

	if (!m_pVisAreaUpr->IsValidVisAreaPointer((CVisArea*)pVisArea))
	{
	...
	if (m_pObjUpr)
	{
		...
		m_pVisAreaUpr->DeleteVisArea((CVisArea*)pVisArea);
...
void C3DEngine::UpdateVisArea(IVisArea* pVisArea, const Vec3* pPoints, i32 nCount, tukk szName,
                              const SVisAreaInfo& info, bool bReregisterObjects)
{
	...
	m_pVisAreaUpr->UpdateVisArea((CVisArea*)pVisArea, pPoints, nCount, szName, info);
	...
	if (bReregisterObjects)
	{
		...
		m_pObjUpr->ReregisterEntitiesInArea(&aabb);
	}
}
...
{
	MarkRNTmpDataPoolForReset();
	return m_pClipVolumeUpr->CreateClipVolume();
}

void C3DEngine::DeleteClipVolume(IClipVolume* pClipVolume)
{
	MarkRNTmpDataPoolForReset();
	m_pClipVolumeUpr->DeleteClipVolume(pClipVolume);
}

void C3DEngine::UpdateClipVolume(IClipVolume* pClipVolume, _smart_ptr<IRenderMesh> pRenderMesh, IBSPTree3D* pBspTree, const Matrix34& worldTM, u8 viewDistRatio, bool bActive, u32 flags, tukk szName)
{
	MarkRNTmpDataPoolForReset();
	m_pClipVolumeUpr->UpdateClipVolume(pClipVolume, pRenderMesh, pBspTree, worldTM, viewDistRatio, bActive, flags, szName);
}
...
void C3DEngine::DeleteRenderNode(IRenderNode* pRenderNode)
{
	...
	{
		DRX_ASSERT((pRenderNode->GetRndFlags() & ERF_PENDING_DELETE) == 0);
...
{
	m_renderNodesToDeleteID = (m_renderNodesToDeleteID + 1) % DRX_ARRAY_COUNT(m_renderNodesToDelete);
...
void C3DEngine::SetWind(const Vec3& vWind)
{
	...
	if (!m_vWindSpeed.IsZero())
	{
		...
		if (!m_pGlobalWind)
		{
			...
			geomSphere.r = 1e7f;
			IGeometry* pGeom = m_pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::sphere::type, &geomSphere);
...
bool C3DEngine::SampleWind(Vec3* pSamples, i32 nSamples, const AABB& volume, bool bIndoors) const
{
...
#if DRX_PLATFORM_SSE2 && !DRX_PLATFORM_F16C
static inline __m128i float_to_half_SSE2(__m128 f, __m128i& s)
{
	#define DECL_CONST4(name, val) static const DRX_ALIGN(16) uint name[4] = { (val), (val), (val), (val) }
...
void C3DEngine::AddForcedWindArea(const Vec3& vPos, float fAmountOfForce, float fRadius)
{
	...
	Vec3 vSpeed = drx_random(Vec3(-1, -1, -1), Vec3(1, 1, 1));
	vSpeed.Normalize();
...
void C3DEngine::RasterWindAreas(std::vector<SOptimizedOutdoorWindArea>* pWindAreas, const Vec3& vGlobalWind)
{
	...
	i32k nSize = rWindGrid.m_nWidth * rWindGrid.m_nHeight;
...
#else
	DrxHalf2* pData = &rWindGrid.m_pData[0];
	i32* pFrames = &m_pWindAreaFrames[0];
...
void C3DEngine::UpdateWindGridArea(SWindGrid& rWindGrid, const SOptimizedOutdoorWindArea& windArea, const AABB& windBox)
{
	...
	i32 nX1 = (i32)windBoxC.min.x;
	i32 nY1 = (i32)windBoxC.min.y;
	i32 nX2 = (i32)windBoxC.max.x;
	i32 nY2 = (i32)windBoxC.max.y;
	...
	i32 nXGrid = (i32)vGridStart.x;
	i32 nYGrid = (i32)vGridStart.y;
...
#else
		DrxHalf2* pData = &rWindGrid.m_pData[nYGrid * rWindGrid.m_nWidth + nXGrid];
		i32* pFrames = &m_pWindAreaFrames[nYGrid * rWindGrid.m_nWidth + nXGrid];
...
void C3DEngine::StartWindGridJob(const Vec3& vPos)
{
	...
	if (!rWindGrid.m_pData)
	{
		...
		rWindGrid.m_pData = reinterpret_cast<DrxHalf2*>(DrxModuleMemalign(rWindGrid.m_nWidth * rWindGrid.m_nHeight * sizeof(DrxHalf2), DRX_PLATFORM_ALIGNMENT));
		memset(rWindGrid.m_pData, 0, rWindGrid.m_nWidth * rWindGrid.m_nHeight * sizeof(DrxHalf2));

		if (!m_pWindField)
		{
			m_pWindField = reinterpret_cast<Vec2*>(DrxModuleMemalign(rWindGrid.m_nWidth * rWindGrid.m_nHeight * sizeof(Vec2), DRX_PLATFORM_ALIGNMENT));
			memset(m_pWindField, 0, rWindGrid.m_nWidth * rWindGrid.m_nHeight * sizeof(Vec2));

			m_pWindAreaFrames = reinterpret_cast<i32*>(DrxModuleMemalign(rWindGrid.m_nWidth * rWindGrid.m_nHeight * sizeof(i32), DRX_PLATFORM_ALIGNMENT));
			memset(m_pWindAreaFrames, 0, rWindGrid.m_nWidth * rWindGrid.m_nHeight * sizeof(i32));
		}
	}
	...
	/*auto* pWindAreas = &m_outdoorWindAreas[m_nCurrentWindAreaList];
	   ...
	   static FILE *fp = NULL;
	   if (!fp)
	    fp = gEnv->pDrxPak->FOpen("Wind.txt", "w");

	   if (fp)
	   {
	    gEnv->pDrxPak->FPrintf(fp, "\n\n%d Wind Areas:\n", pWindAreas->size());
	   }
	   ...
	   for (const auto &windArea : *pWindAreas)
	   {
	    if (fp)
	    {
	      gEnv->pDrxPak->FPrintf(fp, "[%.3f, %.3f] | [%.3f, %.3f] | [%.3f, %.3f] | [%.3f, %.3f] || [%.3f, %.3f]\n",
	   ...
	   if (fp)
	   {
	    gEnv->pDrxPak->FPrintf(fp, "{230, 270} = [%.3f, %.3f]\n", m_pWindField[270*512+230].x, m_pWindField[270*512+230].y);
	    gEnv->pDrxPak->FFlush(fp);
	   }
	   }*/

	JobUpr::SJobState* pJobState = &m_WindJobState;

	TUpdateWindJob job(vPos);
	job.RegisterJobState(pJobState);
	job.SetClassInstance(this);
	job.SetPriorityLevel(JobUpr::eLowPriority);
	job.Run();
...
void C3DEngine::FinishWindGridJob()
{
	...
	if (m_bWindJobRun)
	{
		gEnv->GetJobUpr()->WaitForJob(m_WindJobState);
...
{
	if (m_pObjUpr && m_pVisAreaUpr)
		return m_pVisAreaUpr->GetVisAreaFromPos(vPos);

	return 0;
}

bool C3DEngine::IntersectsVisAreas(const AABB& box, uk * pNodeCache)
{
	if (m_pObjUpr && m_pVisAreaUpr)
		return m_pVisAreaUpr->IntersectsVisAreas(box, pNodeCache);
	return false;
}

bool C3DEngine::ClipToVisAreas(IVisArea* pInside, Sphere& sphere, Vec3 const& vNormal, uk pNodeCache)
{
	if (pInside)
		return pInside->ClipToVisArea(true, sphere, vNormal);
	else if (m_pVisAreaUpr)
		return m_pVisAreaUpr->ClipOutsideVisAreas(sphere, vNormal, pNodeCache);
	return false;
}

bool C3DEngine::IsVisAreasConnected(IVisArea* pArea1, IVisArea* pArea2, i32 nMaxRecursion, bool bSkipDisabledPortals)
{
	if (pArea1 == pArea2)
...
{
	if (m_pObjUpr && m_pVisAreaUpr)
		return m_pVisAreaUpr->IsOutdoorAreasVisible();
...
{
	return DinrusX3dEngBase::GetMatMan()->s_materialHelpers;
}

IMaterialUpr* C3DEngine::GetMaterialUpr()
{
	return DinrusX3dEngBase::GetMatMan();
}
...
void C3DEngine::CheckPhysicalized(const Vec3& vBoxMin, const Vec3& vBoxMax)
{
	/*  if(!GetCVars()->e_stream_areas)
	   ...
	   { // indoor
	    ...
	    i32 nConections = pVisArea->GetRealConnections(arrConnections,16);
	    for(i32 i=0; i<nConections; i++)
	   ...
	   { // outdoor
	    ...
	    {
	      m_pTerrain->GetSecInfo(i32(vBoxMin.x), i32(vBoxMin.y)),
	      m_pTerrain->GetSecInfo(i32(vBoxMax.x), i32(vBoxMin.y)),
	      m_pTerrain->GetSecInfo(i32(vBoxMin.x), i32(vBoxMax.y)),
	      m_pTerrain->GetSecInfo(i32(vBoxMax.x), i32(vBoxMax.y))
	    };
...
void C3DEngine::UpdateWindAreas()
{
	...
	i32 nextWindAreaList = (m_nCurrentWindAreaList + 1) % 2;
	...
	if (m_pCVars->e_WindAreas)
	{
		...
		while (pArea = GetPhysicalWorld()->GetNextArea(pArea))
		{
			...
			windArea.x0 = i32(floorf(b0.x));
			windArea.x1 = i32(ceilf(b1.x));
			windArea.y0 = i32(floorf(b0.y));
			windArea.y1 = i32(ceilf(b1.y));
			...
			for (i32 i = 4; i >= 0; idir = idir.rot90cw() + Vec2i(-(i >> 2), i >> 2), --i)
			{
...
//////////////////////////////////////////////////////////////////////////
i32 C3DEngine::GetLoadedObjectCount()
{
	i32 nObjectsLoaded = m_pObjUpr ? m_pObjUpr->GetLoadedObjectCount() : 0;

	if (gEnv->pCharacterUpr)
	{
		ICharacterUpr::Statistics AnimStats;
		gEnv->pCharacterUpr->GetStatistics(AnimStats);
...
//////////////////////////////////////////////////////////////////////////
void C3DEngine::GetLoadedStatObjArray(IStatObj** pObjectsArray, i32& nCount)
{
	if (m_pObjUpr)
		m_pObjUpr->GetLoadedStatObjArray(pObjectsArray, nCount);
	else
		nCount = 0;
}
...
void C3DEngine::GetObjectsStreamingStatus(SObjectsStreamingStatus& outStatus)
{
	if (m_pObjUpr)
	{
		m_pObjUpr->GetObjectsStreamingStatus(outStatus);
	}
	else
	{
...
//////////////////////////////////////////////////////////////////////////
void C3DEngine::GetStreamingSubsystemData(i32 subsystem, SStremaingBandwidthData& outData)
{
	switch (subsystem)
	{
	...
	case eStreamTaskTypeGeometry:
		m_pObjUpr->GetBandwidthStats(&outData.fBandwidthRequested);
		break;
...
{
	if (m_pDecalUpr && pEntity && (pEntity->m_nInternalFlags & IRenderNode::DECAL_OWNER))
		m_pDecalUpr->OnEntityDeleted(pEntity);
}
...
{
	if (m_pDecalUpr)
		m_pDecalUpr->DeleteDecalsInRange(pAreaBox, pEntity);
}

void C3DEngine::LockCGFResources()
{
	if (m_pObjUpr)
		m_pObjUpr->m_bLockCGFResources = 1;
}
...
void C3DEngine::UnlockCGFResources()
{
	if (m_pObjUpr)
	{
		bool bNeedToFreeCGFs = m_pObjUpr->m_bLockCGFResources != 0;
		m_pObjUpr->m_bLockCGFResources = 0;
		if (bNeedToFreeCGFs)
		{
			m_pObjUpr->FreeNotUsedCGFs();
		}
	}
}
...
void C3DEngine::SerializeState(TSerialize ser)
{
	...
	ser.Value("m_eShadowMode", *(alias_cast<i32*>(&m_eShadowMode)));
	if (ser.IsReading())
		UpdateMoonDirection();
	...
	if (m_pDecalUpr)
		m_pDecalUpr->Serialize(ser);

	m_pPartUpr->Serialize(ser);
	m_pTimeOfDay->Serialize(ser);
}

void C3DEngine::PostSerialize(bool bReading)
{
	m_pPartUpr->PostSerialize(bReading);
}
...
void C3DEngine::InitMaterialDefautMappingAxis(IMaterial* pMat)
{
	...
	for (i32 c = 0; c < 3 && c < pMat->GetSubMtlCount(); c++)
	{
...
//////////////////////////////////////////////////////////////////////////
CContentCGF* C3DEngine::CreateChunkfileContent(tukk filename)
{
...
bool C3DEngine::LoadChunkFileContent(CContentCGF* pCGF, tukk filename, bool bNoWarningMode, bool bCopyChunkFile)
{
	...
	else
	{
		...
		DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "%s: Failed to load chunk file: '%s'", __FUNCTION__, cgf.GetLastError());
		delete pChunkFile;
	}
...
bool C3DEngine::LoadChunkFileContentFromMem(CContentCGF* pCGF, ukk pData, size_t nDataLen, u32 nLoadingFlags, bool bNoWarningMode, bool bCopyChunkFile)
{
	if (!pCGF)
	{
...
ChunkFile::IChunkFileWriter* C3DEngine::CreateChunkFileWriter(EChunkFileFormat eFormat, IDrxPak* pPak, tukk filename) const
{
	ChunkFile::DrxPakFileWriter* const p = new ChunkFile::DrxPakFileWriter();
...
i32 C3DEngine::GetTerrainTextureNodeSizeMeters()
{
	if (m_pTerrain)
...
i32 C3DEngine::GetTerrainTextureNodeSizePixels(i32 nLayer)
{
	if (m_pTerrain)
...
void C3DEngine::PrecacheLevel(bool bPrecacheAllVisAreas, Vec3* pPrecachePoints, i32 nPrecachePointsNum)
{
	LOADING_TIME_PROFILE_SECTION;

	if (GetVisAreaUpr())
		GetVisAreaUpr()->PrecacheLevel(bPrecacheAllVisAreas, pPrecachePoints, nPrecachePointsNum);
}
...
void C3DEngine::SetRecomputeCachedShadows(uint nUpdateStrategy)
{
	...
	if (IRenderer* const pRenderer = GetRenderer())
	{
		...
		{
			static i32 lastFrameId = 0;

			i32 newFrameId = pRenderer->GetFrameID();
...
i32 C3DEngine::GetShadowsCascadeCount(const SRenderLight* pLight) const
{
	i32 nCascadeCount = m_eShadowMode == ESM_HIGHQUALITY ? MAX_GSM_LODS_NUM : GetCVars()->e_GsmLodsNum;
...
{
	if (m_pDecalUpr)
		m_pDecalUpr->OnRenderMeshDeleted(pRenderMesh);
}
...
{
	SRenderNodeTempData* pNewTempData = m_visibleNodesUpr.AllocateTempData(passInfo.GetFrameID());
	...
	else if (GetClipVolumeUpr()->IsClipVolumeRequired(pRNode))
		GetClipVolumeUpr()->UpdateEntityClipVolume(pRNode->GetPos(), pRNode);
...
SRenderNodeTempData* C3DEngine::CheckAndCreateRenderNodeTempData(IRenderNode* pRNode, const SRenderingPassInfo& passInfo)
{
	...
	i32k currentFrame = passInfo.GetFrameID();
	...
	if (pRNode->m_manipulationFrame != currentFrame)
	{
		...
		if (pRNode->m_manipulationFrame != currentFrame)
		{
			...
			if (!pTempData)
			{
				...
				DRX_ASSERT(pTempData && "CreateRenderNodeTempData() failed!");
			}
			...
			else if (pTempData->hasValidRenderObjects)
			{
				...
				{
					if (nStatObjLastModificationId != GetObjUpr()->GetResourcesModificationChecksum(pRNode))
					{
						pTempData->FreeRenderObjects();
					}
				}
	...
	if (!m_visibleNodesUpr.SetLastSeenFrame(pTempData, passInfo))
		pTempData = nullptr;
...
void C3DEngine::CopyObjectsByType(EERType objType, const AABB* pBox, PodArray<IRenderNode*>* plstObjects, uint64 dwFlags)
{
	...
	if (GetVisAreaUpr())
		GetVisAreaUpr()->GetObjectsByType(*plstObjects, objType, pBox, 0, dwFlags);
}
...
void C3DEngine::CopyObjects(const AABB* pBox, PodArray<IRenderNode*>* plstObjects)
{
	...
	if (GetVisAreaUpr())
		GetVisAreaUpr()->GetObjects(*plstObjects, pBox);
}

u32 C3DEngine::GetObjectsByType(EERType objType, IRenderNode** pObjects)
{
...
u32 C3DEngine::GetObjectsByTypeInBox(EERType objType, const AABB& bbox, IRenderNode** pObjects, uint64 dwFlags)
{
	PodArray<IRenderNode*> lstObjects;
...
u32 C3DEngine::GetObjectsInBox(const AABB& bbox, IRenderNode** pObjects)
{
	PodArray<IRenderNode*> lstObjects;
...
u32 C3DEngine::GetObjectsByFlags(uint dwFlags, IRenderNode** pObjects /* =0 */)
{
	PodArray<IRenderNode*> lstObjects;
	...
	if (GetVisAreaUpr())
		GetVisAreaUpr()->GetObjectsByFlags(dwFlags, lstObjects);
...
i32 SImageInfo::GetMemoryUsage()
{
	i32 nSize = 0;
	if (detailInfo.pImgMips[0])
		nSize += (i32)((float)(detailInfo.nDim * detailInfo.nDim * sizeof(ColorB)) * 1.3f);
	if (baseInfo.pImgMips[0])
		nSize += (i32)((float)(baseInfo.nDim * baseInfo.nDim * sizeof(ColorB)) * 1.3f);
	return nSize;
}

byte** C3DEngine::AllocateMips(byte* pImage, i32 nDim, byte** pImageMips)
{
	...
	for (i32 nMip = 1; (nDim >> nMip) && nMip < SImageSubInfo::nMipsNum; nMip++)
	{
		i32 nDimMip = nDim >> nMip;
		...
		for (i32 x = 0; x < nDimMip; x++)
		{
			for (i32 y = 0; y < nDimMip; y++)
			{
				ColorF colSumm(0, 0, 0, 0);
				float fCount = 0;
				for (i32 _x = x * nSubSize - nSubSize / 2; _x < x * nSubSize + nSubSize + nSubSize / 2; _x++)
				{
					for (i32 _y = y * nSubSize - nSubSize / 2; _y < y * nSubSize + nSubSize + nSubSize / 2; _y++)
					{
						i32 nMask = nDim - 1;
						i32 id = (_x & nMask) * nDim + (_y & nMask);
...
void C3DEngine::SetTerrainLayerBaseTextureData(i32 nLayerId, byte*, i32, tukk pImgFileName, IMaterial* pMat, float fBr, float fTiling, i32 nDetailSurfTypeId, float fLayerTilingDetail, float fSpecularAmount, float fSortOrder, ColorF layerFilterColor, float fUseRemeshing, bool bShowSelection)
{
}

SImageInfo* C3DEngine::GetBaseTextureData(i32 nLayerId)
{
...
SImageInfo* C3DEngine::GetBaseTextureDataFromSurfType(i32 nLayerId)
{
...
{
	if (GetObjUpr())
		GetObjUpr()->RegisterForStreaming(pObj);
}

void C3DEngine::UnregisterForStreaming(IStreamable* pObj)
{
	if (GetObjUpr())
		GetObjUpr()->UnregisterForStreaming(pObj);
}

SImageSubInfo* C3DEngine::GetImageInfo(tukk pName)
{
...
SImageSubInfo* C3DEngine::RegisterImageInfo(byte** pMips, i32 nDim, tukk pName)
{
	if (m_imageInfos.find(string(pName)) != m_imageInfos.end())
		return m_imageInfos[string(pName)];
	...
	i32 nMipDim = pImgSubInfo->nDim;

	for (i32 m = 0; m < SImageSubInfo::nMipsNum && nMipDim; m++)
	{
...
tukk gVoxelEditOperationNames[eveoLast] =
{
	"None",
	"Soft Create",
	"Soft Substract",
...
tukk C3DEngine::GetVoxelEditOperationName(EVoxelEditOperation eOperation)
{
#if DRX_PLATFORM_WINDOWS || DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID || DRX_PLATFORM_APPLE
...
{
	if (m_pObjUpr)
		m_pObjUpr->ProcessObjectsStreaming_Finish();
}
...
void C3DEngine::ActivateObjectsLayer(u16 nLayerId, bool bActivate, bool bPhys, bool bObjects, bool bStaticLights, tukk pLayerName, IGeneralMemoryHeap* pHeap, bool bCheckLayerActivation /*=true*/)
{
	...
	if (bObjects)
	{
		...
		if (m_pVisAreaUpr)
			m_pVisAreaUpr->ActivateObjectsLayer(nLayerId, bActivate, bPhys, pHeap, m_arrObjectLayersActivity[nLayerId].objectsBox);
	}
...
bool C3DEngine::IsObjectsLayerHidden(u16 nLayerId, const AABB& objBox)
{
...
void C3DEngine::GetLayerMemoryUsage(u16 nLayerId, IDrxSizer* pSizer, i32* pNumBrushes, i32* pNumDecals) const
{
	if (pNumBrushes)
		*pNumBrushes = 0;
	if (pNumDecals)
...
void C3DEngine::SkipLayerLoading(u16 nLayerId, bool bClearList)
{
	if (bClearList)
...
bool C3DEngine::IsLayerSkipped(u16 nLayerId)
{
	return m_skipedLayers.find(nLayerId) != m_skipedLayers.end() ? true : false;
}

void C3DEngine::PrecacheCharacter(IRenderNode* pObj, const float fImportance, ICharacterInstance* pCharacter, IMaterial* pSlotMat, const Matrix34& matParent, const float fEntDistance, const float fScale, i32 nMaxDepth, bool bForceStreamingSystemUpdate, const SRenderingPassInfo& passInfo)
{
	if (m_pObjUpr)
	{
		...
		{
			m_pObjUpr->ProcessObjectsStreaming(passInfo);
		}
	}
}
...
void C3DEngine::PrecacheRenderNode(IRenderNode* pObj, float fEntDistanceReal)
{
	...
	if (m_pObjUpr)
	{
		...
		m_pObjUpr->UpdateRenderNodeStreamingPriority(pObj, fEntDistanceReal, 1.0f, fEntDistanceReal < GetFloatCVar(e_StreamCgfFastUpdateMaxDistance), passInfo, true);
...
{
	FUNCTION_PROFILER_3DENGINE;
	static u32 nLastIndex = 0;
	u32k nDECALS_PER_FRAME = 50;

	if (u32 nNumDecalRenderNodes = m_decalRenderNodes.size())
	{
		for (u32 i = 0, end = __min(nDECALS_PER_FRAME, nNumDecalRenderNodes); i < end; ++i, ++nLastIndex)
		{
...
void C3DEngine::SetRenderNodeMaterialAtPosition(EERType eNodeType, const Vec3& vPos, IMaterial* pMat)
{
	...
	if (GetVisAreaUpr())
		GetVisAreaUpr()->GetObjectsByType(lstObjects, eNodeType, &aabbPos);
	...
	for (i32 i = 0; i < lstObjects.Count(); i++)
	{
		PrintMessage("Game changed render node material: %s EERType:%d pos: (%d,%d,%d)",
		             ...
		             (i32)eNodeType,
		             (i32)vPos.x, (i32)vPos.y, (i32)vPos.z);
...
void C3DEngine::OverrideCameraPrecachePoint(const Vec3& vPos)
{
	if (m_pObjUpr)
	{
		m_pObjUpr->m_vStreamPreCacheCameras[0].vPosition = vPos;
		m_pObjUpr->m_bCameraPrecacheOverridden = true;
	}
}

i32 C3DEngine::AddPrecachePoint(const Vec3& vPos, const Vec3& vDir, float fTimeOut, float fImportanceFactor)
{
	if (m_pObjUpr)
	{
		if (m_pObjUpr->m_vStreamPreCachePointDefs.size() >= CObjUpr::MaxPrecachePoints)
		{
			size_t nOldestIdx = 0;
			i32 nOldestId = INT_MAX;
			for (size_t i = 1, c = m_pObjUpr->m_vStreamPreCachePointDefs.size(); i < c; ++i)
			{
				if (m_pObjUpr->m_vStreamPreCachePointDefs[i].nId < nOldestId)
				{
					nOldestIdx = i;
					nOldestId = m_pObjUpr->m_vStreamPreCachePointDefs[i].nId;
				}
			}

			assert(nOldestIdx > 0);

			DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "Precache points full - evicting oldest (%f, %f, %f)",
			           m_pObjUpr->m_vStreamPreCacheCameras[nOldestIdx].vPosition.x,
			           m_pObjUpr->m_vStreamPreCacheCameras[nOldestIdx].vPosition.y,
			           m_pObjUpr->m_vStreamPreCacheCameras[nOldestIdx].vPosition.z);

			m_pObjUpr->m_vStreamPreCachePointDefs.DeleteFastUnsorted((i32)nOldestIdx);
			m_pObjUpr->m_vStreamPreCacheCameras.DeleteFastUnsorted((i32)nOldestIdx);
		}

		SObjManPrecachePoint pp;
		pp.nId = m_pObjUpr->m_nNextPrecachePointId++;
		...
		m_pObjUpr->m_vStreamPreCachePointDefs.Add(pp);
		m_pObjUpr->m_vStreamPreCacheCameras.Add(pc);
		//m_pObjUpr->m_bCameraPrecacheOverridden = true;
...
void C3DEngine::ClearPrecachePoint(i32 id)
{
	if (m_pObjUpr)
	{
		for (size_t i = 1, c = m_pObjUpr->m_vStreamPreCachePointDefs.size(); i < c; ++i)
		{
			if (m_pObjUpr->m_vStreamPreCachePointDefs[i].nId == id)
			{
				m_pObjUpr->m_vStreamPreCachePointDefs.DeleteFastUnsorted((i32)i);
				m_pObjUpr->m_vStreamPreCacheCameras.DeleteFastUnsorted((i32)i);
				break;
			}
		}
	}
...
void C3DEngine::ClearAllPrecachePoints()
{
	if (m_pObjUpr)
	{
		m_pObjUpr->m_vStreamPreCachePointDefs.resize(1);
		m_pObjUpr->m_vStreamPreCacheCameras.resize(1);
	}
}

void C3DEngine::GetPrecacheRoundIds(i32 pRoundIds[MAX_STREAM_PREDICTION_ZONES])
{
	if (m_pObjUpr)
	{
		pRoundIds[0] = m_pObjUpr->m_nUpdateStreamingPrioriryRoundIdFast;
		pRoundIds[1] = m_pObjUpr->m_nUpdateStreamingPrioriryRoundId;
	}
}

void static DrawMeter(float scale, float& x, float& y, i32 nWidth, i32 nHeight, IRenderAuxGeom* pAuxRenderer)
{
	vtx_idx indLines[8] =
	{
		0, 1, 1, 2,
...
///////////////////////////////////////////////////////////////////////////////
i32 C3DEngine::GetZoomMode() const
{
	return m_nZoomMode;
}
...
///////////////////////////////////////////////////////////////////////////////
void C3DEngine::SetZoomMode(i32 nZoomMode)
{
...
void C3DEngine::GetCollisionClass(SCollisionClass& collclass, i32 tableIndex)
{
...
///////////////////////////////////////////////////////////////////////////////
void C3DEngine::SaveInternalState(struct IDataWriteStream& writer, const AABB& filterArea, const bool bTerrain, u32k objectMask)
{
	ITerrain* pTerrain = GetITerrain();
	IVisAreaUpr* pVisAreaUpr = GetIVisAreaUpr();
	if ((NULL != pTerrain) && (NULL != pVisAreaUpr))
	{
		const float levelBounds = 2048.0f;
		u8k terrainMask = bTerrain ? 1 : 0;
		...
		i32 objectCompiledDataSize = 0;
		i32 terrainCompiledDataSize = pTerrain->GetCompiledDataSize(&info);
...
///////////////////////////////////////////////////////////////////////////////
void C3DEngine::LoadInternalState(struct IDataReadStream& reader, u8k* pVisibleLayersMask, u16k* pLayerIdTranslation)
{
	SHotUpdateInfo info;
	...
	u32k terrainDataSize = reader.ReadUint32();
	u32k objectDataSize = reader.ReadUint32();
	...
	if (objectDataSize > 0)
	{
		IVisAreaUpr* pVisAreaUpr = gEnv->p3DEngine->GetIVisAreaUpr();
		if (NULL != pVisAreaUpr)
		{
			pVisAreaUpr->SetCompiledData(
...
u32 CDebugDrawListMgr::m_filter = I3DEngine::DLOT_ALL;
...
void CDebugDrawListMgr::AddObject(I3DEngine::SObjectInfoToAddToDebugDrawList& newObjInfo)
{
	...
	if (!ShouldFilterOutObject(newObjInfo))
	{
		...
		else
		{
			...
			if (m_assets.size() < uint(DinrusX3dEngBase::GetCVars()->e_DebugDrawListSize))
			{
				used = true;
...
void CDebugDrawListMgr::Update()
{
	...
	tukk pHeaderStr = "";
	switch (DinrusX3dEngBase::GetCVars()->e_DebugDraw)
	{
	case LM_TRI_COUNT:
	...
	for (u32 i = 0; i < m_assets.size(); ++i)
	{
		...
		if (DinrusX3dEngBase::GetCVars()->e_DebugDrawListBBoxIndex - 1 == i)
			colorLine = Col_Blue;
		const TAssetInfo& currAsset = m_assets[i];
		...
		i32 filenameSep = 7 * (max(standardNameSize, i32(currAsset.name.length())) + 2);
	...
	if (DinrusX3dEngBase::GetCVars()->e_DebugDrawListBBoxIndex > 0 && uint(DinrusX3dEngBase::GetCVars()->e_DebugDrawListBBoxIndex - 1) < m_assets.size())
	{
		const TAssetInfo& assetInfo = m_assets[DinrusX3dEngBase::GetCVars()->e_DebugDrawListBBoxIndex - 1];
		u32 numBoxesDrawn = 0;
		for (u32 i = 0; i < m_drawBoxes.size(); ++i)
		{
	...
	{
		ClearFrameData();
		m_assets.reserve(DinrusX3dEngBase::GetCVars()->e_DebugDrawListSize);
	}
	m_lock.Unlock();
}

//////////////////////////////////////////////////////////////////////////
void CDebugDrawListMgr::PrintText(float x, float y, const ColorF& fColor, tukk label_text, ...)
{
	va_list args;
...
//////////////////////////////////////////////////////////////////////////
void CDebugDrawListMgr::MemToString(u32 memVal, TMyStandardString& outStr)
{
	if (memVal < 1024 * 1024)
...
//////////////////////////////////////////////////////////////////////////
void CDebugDrawListMgr::MyStandardString_Concatenate(TMyStandardString& outStr, tukk pStr)
{
...
//////////////////////////////////////////////////////////////////////////
void CDebugDrawListMgr::MyFileNameString_Assign(TFilenameString& outStr, tukk pStr)
{
...
void CDebugDrawListMgr::DumpLog()
{
	...
	GetStrCurrFilter(filterStr);
	DrxLog("--------------------------------------------------------------------------------");
	DrxLog("                           DebugDrawList infodebug");
	DrxLog("--------------------------------------------------------------------------------");
	DrxLog(" total objects: %d    Ordered by: %s     Showing: %s", m_counter, GetStrCurrMode(), filterStr.c_str());
	DrxLog("");
	DrxLog("   tris      verts   draw Calls   texMem     meshMem    type");
	DrxLog(" -------   --------  ----------  --------   --------  ----------");
	for (u32 i = 0; i < m_assets.size(); ++i)
	{
		const TAssetInfo& currAsset = m_assets[i];
		...
		MemToString(currAsset.texMemory, texMemoryStr);
		MemToString(currAsset.meshMemory, meshMemoryStr);
		DrxLog("%8d  %8d     %5d     %s   %s  %s      %s    %s", currAsset.numTris, currAsset.numVerts, currAsset.drawCalls,
...
//////////////////////////////////////////////////////////////////////////
tukk CDebugDrawListMgr::GetStrCurrMode()
{
	tukk pModesNames[] =
	{
		"Tri count",
	...
	u32 index = DinrusX3dEngBase::GetCVars()->e_DebugDraw - LM_BASENUMBER;
	u32 numElems = sizeof(pModesNames) / sizeof *(pModesNames);
	if (index < numElems)
...
void CDebugDrawListMgr::GetStrCurrFilter(TMyStandardString& strOut)
{
	...
	u32 numElems = sizeof(pFilterNames) / sizeof *(pFilterNames);
	for (u32 i = 1, bitVal = 1; i < numElems; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
tukk CDebugDrawListMgr::GetAssetTypeName(I3DEngine::EDebugDrawListAssetTypes type)
{
	tukk pNames[] =
	{
		"",
	...
	u32 numElems = sizeof(pNames) / sizeof *(pNames);
	for (u32 i = 1, bitVal = 1; i < numElems; ++i)
	{
		if (bitVal == type)
			return pNames[i];
...
{
	return m_pObjUpr ? m_pObjUpr->m_vSkyColor : Vec3(0, 0, 0);
}
...
{
	DRX_ASSERT(passInfo.IsShadowPass());
	...
	{
		DRX_ASSERT(passInfo.ShadowCacheLod() < MAX_GSM_CACHED_LODS_NUM);
	...
	i32 wantedLod = pTempData->userData.nWantedLod;
...
///////////////////////////////////////////////////////////////////////////////
void C3DEngine::AsyncOctreeUpdate(IRenderNode* pEnt, u32 nFrameID, bool bUnRegisterOnly)
{
	FUNCTION_PROFILER_3DENGINE;

#ifdef _DEBUG   // crash test basically
	tukk szClass = pEnt->GetEntityClassName();
	...
	if (!(dwRndFlags & ERF_RENDER_ALWAYS) && !(dwRndFlags & ERF_CASTSHADOWMAPS))
		if (GetCVars()->e_ObjFastRegister && pEnt->m_pOcNode && ((COctreeNode*)pEnt->m_pOcNode)->IsRightNode(aabb, fObjRadiusSqr, pEnt->m_fWSMaxViewDist))
		{
			...
			IVisArea* pVisAreaFromPos = (!m_pVisAreaUpr || dwRndFlags & ERF_OUTDOORONLY) ? NULL : GetVisAreaUpr()->GetVisAreaFromPos(vEntCenter);
			if (pVisAreaFromPos == pVisArea)
			{
				...
				if (GetClipVolumeUpr()->IsClipVolumeRequired(pEnt))
					GetClipVolumeUpr()->UpdateEntityClipVolume(vEntCenter, pEnt);
	...
	else if (GetCVars()->e_StreamCgf && pEnt->IsAllocatedOutsideOf3DEngineDLL())
	{
		...
		if ((u32)pEnt->GetDrawFrame(0) < nFrameID - 16)
		{
	...
	//////////////////////////////////////////////////////////////////////////
	if (pEnt->m_dwRndFlags & ERF_OUTDOORONLY || !(m_pVisAreaUpr && m_pVisAreaUpr->SetEntityArea(pEnt, aabb, fObjRadiusSqr)))
	{
		if (m_pObjectsTree)
		{
