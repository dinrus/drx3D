...
i32 CDecal::Update(bool& active, const float fFrameTime)
{
...
void CDecal::Render(const float fCurTime, i32 nAfterWater, float fDistanceFading, float fDistance, const SRenderingPassInfo& passInfo)
{
	...
	case eDecalType_OS_OwnersVerticesUsed:
		{
			...
			if (m_ownerInfo.pRenderNode && m_ownerInfo.pRenderNode->GetRenderNodeType() == eERType_Vegetation)
			{
				...
				assert(pObjUpr && pVegetation && pBody);
