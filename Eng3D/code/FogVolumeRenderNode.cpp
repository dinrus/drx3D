...
void CFogVolumeRenderNode::SetTraceableArea(const AABB& traceableArea, const SRenderingPassInfo& passInfo)
{
	...
	IVisArea* pVisAreaOfCenter(GetVisAreaUpr() ? GetVisAreaUpr()->GetVisAreaFromPos(traceableAreaCenter) : NULL);
	...
	, m_cachedFogColor(1, 1, 1, 1)
{
	...
	m_windOffset.x = drx_random(0.0f, 1000.0f);
	m_windOffset.y = drx_random(0.0f, 1000.0f);
	m_windOffset.z = drx_random(0.0f, 1000.0f);
...
void CFogVolumeRenderNode::SetFogVolumeProperties(const SFogVolumeProperties& properties)
{
	...
	m_volumeType = clamp_tpl<i32>(properties.m_volumeType,
...
tukk CFogVolumeRenderNode::GetEntityClassName() const
{
	return "FogVolume";
}

tukk CFogVolumeRenderNode::GetName() const
{
	return "FogVolume";
}
...
void CFogVolumeRenderNode::Render(const SRendParams& rParam, const SRenderingPassInfo& passInfo)
{
	...
	else
	{
		...
		i32 nAfterWater = GetObjUpr()->IsAfterWater(m_pos, passInfo.GetCamera().GetPosition(), passInfo, Get3DEngine()->GetWaterLevel()) ? 1 : 0;
...
void CFogVolumeRenderNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
