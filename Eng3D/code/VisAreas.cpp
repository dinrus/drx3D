...
i32 CVisArea::s_nGetDistanceThruVisAreasCallCounter = 0;

void CVisArea::Update(const Vec3* pPoints, i32 nCount, tukk szName, const SVisAreaInfo& info)
{
	assert(m_pVisAreaColdData);

	drx_strcpy(m_pVisAreaColdData->m_sName, szName);
...
CVisArea::~CVisArea()
{
	...
	GetVisAreaUpr()->OnVisAreaDeleted(this);
}
...
void CVisArea::FindSurroundingVisAreaReqursive(i32 nMaxReqursion, bool bSkipDisabledPortals, PodArray<IVisArea*>* pVisitedAreas, i32 nMaxVisitedAreas, i32 nDeepness, PodArray<CVisArea*>* pUnavailableAreas)
{
	...
	if (nMaxReqursion > (nDeepness + 1))
		for (i32 p = 0; p < m_lstConnections.Count(); p++)
...
void CVisArea::FindSurroundingVisArea(i32 nMaxReqursion, bool bSkipDisabledPortals, PodArray<IVisArea*>* pVisitedAreas, i32 nMaxVisitedAreas, i32 nDeepness)
{
	if (pVisitedAreas)
	{
		if (pVisitedAreas->capacity() < (u32)nMaxVisitedAreas)
...
Vec3 CVisArea::GetConnectionNormal(CVisArea* pPortal)
{
	...
	for (i32 v = 0; v < m_lstShapePoints.Count(); v++)
	{
		nIntersNum = 0;
		...
		for (i32 p = 0; p < pPortal->m_lstShapePoints.Count(); p++)
		{
	...
	{
		i32 nBottomPoints = 0;
		for (i32 p = 0; p < pPortal->m_lstShapePoints.Count() && p < 4; p++)
...
void        CVisArea::PreRender(i32 nReqursionLevel,
                                ...
                                const SRenderingPassInfo& passInfo)
{
	...
	if (m_bAffectedByOutLights)
		GetVisAreaUpr()->m_bSunIsNeeded = true;

	if (m_lstCurCamerasLen == m_lstCurCamerasCap)
	{
		i32 newIdx = s_tmpCameras.size();
	    ...
	    (this != pCurPortal || !pCurPortal->IsPointInsideVisArea(CurCamera.GetPosition())))
	{
		...
		else if (!vPortNorm.IsEquivalent(Vec3(0, 0, 0), VEC_EPSILON) && vPortNorm.z == 0)
		{
			...
			i32 arrInAreaPointId[2] = { -1, -1 };
			i32 nInAreaPointCounter = 0;

			Vec3 arrOutAreaPoint[2] = { Vec3(0, 0, 0), Vec3(0, 0, 0) };
			i32 nOutAreaPointCounter = 0;

			// find 2 points of portal in this area (or in this outdoors)
			for (i32 i = 0; i < m_lstShapePoints.Count() && nInAreaPointCounter < 2; i++)
			{
			...
			for (i32 i = 0; i < m_lstShapePoints.Count() && nOutAreaPointCounter < 2; i++)
			{
				Vec3 vTestPoint = m_lstShapePoints[i] + Vec3(0, 0, m_fHeight * 0.5f);
				CVisArea* pAnotherArea = m_lstConnections[0];
				if ((pParent && (pParent->IsPointInsideVisArea(vTestPoint))) ||
		...
		if (lstPortVertsClipped.Count() > 2)
		{
			...
			for (i32 i = 0; i < lstPortVertsClipped.Count(); i++)
			{
				Vec3 vSS;
		...
		if (lstPortVertsClipped.Count() > 2 && aabb.min.z > 0.01f)
		{
			...
			// get 3d positions of portal bounds
			{
				...
				CurCamera.m_ScissorInfo.x1 = u16(CLAMP(aabb.min.x * w / 100, 0, w));
				CurCamera.m_ScissorInfo.y1 = u16(CLAMP(aabb.min.y * h / 100, 0, h));
				CurCamera.m_ScissorInfo.x2 = u16(CLAMP(aabb.max.x * w / 100, 0, w));
				CurCamera.m_ScissorInfo.y2 = u16(CLAMP(aabb.max.y * h / 100, 0, h));
			}

			if (GetCVars()->e_Portals == 4)
				for (i32 i = 0; i < lstPortVertsSS.Count(); i++)
				{
	...
	if (!m_bThisIsPortal && m_lstConnections.Count())
	{
		...
		qsort(&m_lstConnections[0], m_lstConnections.Count(),
		      sizeof(m_lstConnections[0]), CVisAreaUpr__CmpDistToPortal);
	}
...
i32 CVisArea::GetRealConnections(IVisArea** pAreas, i32 nMaxConnNum, bool bSkipDisabledPortals)
{
	i32 nOut = 0;
	for (i32 nArea = 0; nArea < m_lstConnections.Count(); nArea++)
	{
		if (nOut < nMaxConnNum)
...
i32 CVisArea::GetVisAreaConnections(IVisArea** pAreas, i32 nMaxConnNum, bool bSkipDisabledPortals)
{
	i32 nOut = 0;
	if (IsPortal())
	{
	...
	{
		for (i32 nPort = 0; nPort < m_lstConnections.Count(); nPort++)
		{
			CVisArea* pPortal = m_lstConnections[nPort];
			assert(pPortal->IsPortal());
			for (i32 nArea = 0; nArea < pPortal->m_lstConnections.Count(); nArea++)
			{
...
{
	i32 nCount = m_lstConnections.Count();
	...
	if (v1Max.x > v2Min.x && v2Max.x > v1Min.x)
		if (v1Max.y > v2Min.y && v2Max.y > v1Min.y)
			if (v1Max.z > v2Min.z && v2Max.z > v1Min.z)
			{
				...
				// horisontal portal
				{
					i32 nBottomPoints = 0, nUpPoints = 0;
					for (i32 p = 0; p < pPortal->m_lstShapePoints.Count() && p < 4; p++)
						if (IsPointInsideVisArea(pPortal->m_lstShapePoints[p]))
							nBottomPoints++;
...
/*
   void CVisArea::SetTreeId(i32 nTreeId)
   {
...
void CVisArea::DrawAreaBoundsIntoCBuffer(CCullBuffer* pCBuffer)
{
	...
	/*  if(m_lstShapePoints.Count()!=4)
	   ...
	   i32 v=0;
	   i32 i=0;
	   for(i32 p=0; p<4 && p<m_lstShapePoints.Count(); p++)
	   {
...
void CVisArea::GetMemoryUsage(IDrxSizer* pSizer)
{
	...
	// for(i32 nStatic=0; nStatic<2; nStatic++)
	//   for(i32 i=0; i<m_lstEntities[nStatic].Count(); i++)
...
void CVisArea::UpdateOcclusionFlagInTerrain()
{
	if (m_bAffectedByOutLights && !m_bThisIsPortal)
	{
		...
		if (vCenter.z < GetTerrain()->GetZApr(vCenter.x, vCenter.y))
		{
			...
			for (i32 i = 0; i < lstResult.Count(); i++)
...
void CVisArea::AddConnectedAreas(PodArray<CVisArea*>& lstAreas, i32 nMaxRecursion)
{
...
bool CVisArea::GetDistanceThruVisAreas(AABB vCurBoxIn, IVisArea* pTargetArea, const AABB& targetBox, i32 nMaxReqursion, float& fResDist)
{
...
bool CVisArea::GetDistanceThruVisAreasReq(AABB vCurBoxIn, float fCurDistIn, IVisArea* pTargetArea, const AABB& targetBox, i32 nMaxReqursion, float& fResDist, CVisArea* pPrevArea, i32 nCallID)
{
...
void CVisArea::OffsetPosition(const Vec3& delta)
{
	...
	for (i32 i = 0; i < m_lstShapePoints.Count(); i++)
	{
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool InsidePolygon(const Vec3* __restrict polygon, i32 N, const Vec3& p)
{
	i32 counter = 0;
	i32 i;
	float xinters;
...
///////////////////////////////////////////////////////////////////////////////
bool InsideSpherePolygon(Vec3* polygon, i32 N, Sphere& S)
{
	i32 i;
	Vec3 p1, p2;
...
namespace
{
	inline void HelperGetNearestCubeProbe(PodArray<CVisArea*>& arrayVisArea, float& fMinDistance, i32& nMaxPriority, CLightEntity*& pNearestLight, const AABB* pBBox)
	{
	...
	inline i32 HelperGetObjectsCount(PodArray<CVisArea*>& arrayVisArea, EOcTeeNodeListType eListType)
	{
		i32 nCount = 0;
		i32 s = arrayVisArea.Count();
		for (i32 i = 0; i < s; ++i)
	...
	inline void HelperGetStreamedInNodesNum(PodArray<CVisArea*>& arrayVisArea, i32& nAllStreamable, i32& nReady)
	{
		i32 s = arrayVisArea.Count();
		for (i32 dwI = 0; dwI < s; ++dwI)
		{
...
void CVisAreaUpr::GetNearestCubeProbe(float& fMinDistance, i32& nMaxPriority, CLightEntity*& pNearestLight, const AABB* pBBox)
{
...
///////////////////////////////////////////////////////////////////////////////
void CVisAreaUpr::GetObjectsByType(PodArray<IRenderNode*>& lstObjects, EERType objType, const AABB* pBBox, bool* pInstStreamReady, uint64 dwFlags)
{
...
///////////////////////////////////////////////////////////////////////////////
i32 CVisAreaUpr::GetObjectsCount(EOcTeeNodeListType eListType)
{
	return HelperGetObjectsCount(m_lstVisAreas, eListType) + HelperGetObjectsCount(m_lstPortals, eListType);
}

///////////////////////////////////////////////////////////////////////////////
void CVisAreaUpr::GetStreamedInNodesNum(i32& nAllStreamable, i32& nReady)
{
...
///////////////////////////////////////////////////////////////////////////////
bool CVisAreaUpr::IsOccludedByOcclVolumes(const AABB& objBox, const SRenderingPassInfo& passInfo, bool bCheckOnlyIndoorVolumes)
{
	FUNCTION_PROFILER_3DENGINE;

	PodArray<CVisArea*>& rList = bCheckOnlyIndoorVolumes ? m_lstIndoorActiveOcclVolumes : m_lstActiveOcclVolumes;

	for (i32 i = 0; i < rList.Count(); i++)
	{
		...
		if (pCamera)
		{
			...
			DRX_ALIGN(128) char bufCamera[sizeof(CCamera)];
...
///////////////////////////////////////////////////////////////////////////////
IVisArea* CVisAreaUpr::GetVisAreaFromPos(const Vec3& vPos)
{
	FUNCTION_PROFILER_3DENGINE;
...
void CVisArea::UpdateClipVolume()
{
	...
	i32k nPoints = m_lstShapePoints.size();
	i32k nVertexCount = nPoints * 2;
	i32k nIndexCount = (2 * nPoints + 2 * (nPoints - 2)) * 3; // 2*nPoints for sides, nPoints-2 for top and nPoints-2 for bottom
	...
	{
		i32 nPointIdx = IsShapeClockwise() ? nPoints - 1 - i : i;
	...
	i32 nTris = gEnv->pPhysicalWorld->GetPhysUtils()->TriangulatePoly(&triangulationPoints[0], triangulationPoints.size(), &triangleIndices[0], triangleIndices.size());
...
{
	i32 nFlags = IClipVolume::eClipVolumeIsVisArea;
...
void CVisArea::UpdatePortalBlendInfo(const SRenderingPassInfo& passInfo)
{
	if (m_bThisIsPortal && m_lstConnections.Count() > 0 && m_nStencilRef>0 && GetCVars()->e_PortalsBlend > 0 && m_fPortalBlending > 0)
	{
		...
		for (i32 p = 0; p < m_lstShapePoints.Count(); ++p)
		{
			Vec3 vTestPoint = m_lstShapePoints[p] + Vec3(0, 0, m_fHeight * 0.5f);
			i32 nVisAreaIndex = (m_lstConnections[0] && m_lstConnections[0]->IsPointInsideVisArea(vTestPoint)) ? 0 : 1;
...
bool CVisAreaUpr::SetEntityArea(IRenderNode* pEnt, const AABB& objBox, const float fObjRadiusSqr)
{
	assert(pEnt);
	...
	i32k kNumPortals = m_lstPortals.Count();
	for (i32 v = 0; v < kNumPortals; v++)
	{
	...
	{
		i32k kNumVisAreas = m_lstVisAreas.Count();
		for (i32 v = 0; v < kNumVisAreas; v++)
		{
...
bool CVisArea::ClipToVisArea(bool bInside, Sphere& sphere, Vec3 const& vNormal)
{
	...
	*/
	...
	if (bClipXY)
	{
		...
		for (i32 n = m_lstShapePoints.Count() - 1; n >= 0; n--)
		{
...
{
	i32k kNumPoints = m_lstShapePoints.Count();
	if (kNumPoints)
	{
...
///////////////////////////////////////////////////////////////////////////////
bool CVisArea::FindVisArea(IVisArea* pAnotherArea, i32 nMaxRecursion, bool bSkipDisabledPortals)
{
...
///////////////////////////////////////////////////////////////////////////////
bool CVisArea::FindVisAreaReqursive(IVisArea* pAnotherArea, i32 nMaxReqursion, bool bSkipDisabledPortals, StaticDynArray<CVisArea*, 1024>& arrVisitedParents)
{
