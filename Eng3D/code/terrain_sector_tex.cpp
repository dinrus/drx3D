...
// -------------------------------------------------------------------------
//  Имя файла:   terrain_sector_tex.cpp
//  Version:     v1.00
...
i32 CTerrainNode::CreateSectorTexturesFromBuffer(float * pSectorHeightMap)
{
	...
	//	u32 nSectorDataSize = pLayers[0].nSectorSizeBytes + pLayers[1].nSectorSizeBytes;
	...
	else
	{
		...
		i32 nDim = GetTerrain()->m_arrBaseTexInfos.m_TerrainTextureLayer[0].nSectorSizePixels;
	...
	if (!m_pParent)
	{
		...
		i32 nSizeDxtMip0 = GetRenderer()->GetTextureFormatDataSize(nDim, nDim, 1, 1, texFormat);
		i32 nSizeMip0 = GetRenderer()->GetTextureFormatDataSize(nDim, nDim, 1, 1, eTF_R8G8B8A8);
...
void CTerrainNode::StreamAsyncOnComplete(IReadStream* pStream, unsigned nError)
{
	...
	if(pLayers[0].eTexFormat != m_pTerrain->m_texCache[0].m_eTexFormat)
	{
		...
		i32 nDim = m_pTerrain->m_texCache[0].m_nDim;

		i32 nSizeDxt = GetRenderer()->GetTextureFormatDataSize(nDim, nDim, 1, 1, pLayers[0].eTexFormat);
		i32 nSizeRgb = GetRenderer()->GetTextureFormatDataSize(nDim, nDim, 1, 1, eTF_R8G8B8A8);
...
void CTerrainNode::StartSectorTexturesStreaming(bool bFinishNow)
{
	...
	i32 nFileOffset = sizeof(SCommonFileHeader) + sizeof(STerrainTextureFileHeader)
	...
	i32 nUserDataSize = m_pTerrain->m_texCache[2].m_nDim * m_pTerrain->m_texCache[2].m_nDim;
...
void CTerrainNode::CalculateTexGen(const CTerrainNode* pTextureSourceNode, float& fTexOffsetX, float& fTexOffsetY, float& fTexScale)
{
	...
	u16 nSectorSizePixels = GetTerrain()->m_arrBaseTexInfos.m_TerrainTextureLayer[0].nSectorSizePixels;
