...
namespace
{
static u32k sCurrentPresetVersion = 2;
...
void CTimeOfDayVariable::Init(tukk group, tukk displayName, tukk name, ITimeOfDay::ETimeOfDayParamID nParamId, ITimeOfDay::EVariableType type, float defVal0, float defVal1, float defVal2)
{
	m_id = nParamId;
	m_type = type;
	m_name = name;
...
size_t CTimeOfDayVariable::GetSplineKeyCount(i32 nSpline) const
{
...
bool CTimeOfDayVariable::GetSplineKeys(i32 nSpline, SBezierKey* keysArray, u32 keysArraySize) const
{
	if (const CBezierSpline* pSpline = GetSpline(nSpline))
	{
...
bool CTimeOfDayVariable::SetSplineKeys(i32 nSpline, const SBezierKey* keysArray, u32 keysArraySize)
{
...
bool CTimeOfDayVariable::UpdateSplineKeyForTime(i32 nSpline, float fTime, float newKey)
{
	if (CBezierSpline* pSpline = GetSpline(nSpline))
	{
...
CTimeOfDayVariable* CEnvironmentPreset::GetVar(tukk varName)
{
	for (size_t i = 0; i < ITimeOfDay::PARAM_TOTAL; ++i)
	{
...
bool CEnvironmentPreset::InterpolateVarInRange(ITimeOfDay::ETimeOfDayParamID id, float fMin, float fMax, u32 nCount, Vec3* resultArray) const
{
...
void CEnvironmentPreset::AddVar(tukk group, tukk displayName, tukk name, ITimeOfDay::ETimeOfDayParamID nParamId, ITimeOfDay::EVariableType type, float defVal0, float defVal1, float defVal2)
{
