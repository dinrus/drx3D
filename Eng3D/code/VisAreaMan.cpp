...
void CVisAreaUpr::DeleteAllVisAreas()
{
...
SAABBTreeNode::SAABBTreeNode(PodArray<CVisArea*>& lstAreas, AABB box, i32 nRecursion)
{
	...
	for (i32 i = 0; i < lstAreas.Count(); i++)
	{
...
SAABBTreeNode* SAABBTreeNode::GetTopNode(const AABB& box, uk * pNodeCache)
{
	AABB boxClip = box;
...
bool SAABBTreeNode::IntersectsVisAreas(const AABB& box)
{
	if (nodeBox.IsIntersectBox(box))
	{
		...
		{
			// leaf
			for (i32 i = 0; i < nodeAreas.Count(); i++)
			{
...
i32 SAABBTreeNode::ClipOutsideVisAreas(Sphere& sphere, Vec3 const& vNormal)
{
	i32 nClipped = 0;
...
void CVisAreaUpr::UpdateAABBTree()
{
	delete m_pAABBTree;
...
bool CVisAreaUpr::IsEntityVisible(IRenderNode* pEnt)
{
...
void CVisAreaUpr::SetCurAreas(const SRenderingPassInfo& passInfo)
{
	m_pCurArea = 0;
	m_pCurPortal = 0;
	...
	for (i32 v = 0; v < m_lstPortals.Count(); v++)
	...
	if (!m_pCurPortal)
	{
		//		i32 nFoundAreasNum = 0;

		// find camera area
		for (i32 nVolumeId = 0; nVolumeId < m_lstVisAreas.Count(); nVolumeId++)
		{
	...
	/*
	   ...
	   {
	    IVisArea * arrAreas[8];
	    i32 nres = m_pCurArea->GetVisAreaConnections(arrAreas, 8);
	    nres=nres;
	   }
	   DefineTrees();*/
	...
	/*	if(GetCVars()->e_Portals == 4)
	   {
	    if(m_pCurPortal)
	    {
	      ...
	      i32 nConnections = m_pCurPortal->GetVisAreaConnections(arrAreas,64);
	    ...
	    {
	      IVisArea * arrAreas[64];
	      i32 nConnections = m_pCurArea->GetVisAreaConnections(arrAreas,64);
	      PrintMessage("CurArea = %s, nRes = %d", m_pCurArea->m_sName, nConnections);
	    }
	   }*/
}

bool CVisAreaUpr::IsSkyVisible()
{
	return m_bSkyVisible;
}

bool CVisAreaUpr::IsOceanVisible()
{
	return m_bOceanVisible;
}

bool CVisAreaUpr::IsOutdoorAreasVisible()
{
	if (!m_pCurArea && !m_pCurPortal)
	{
...
/*void CVisAreaUpr::SetAreaFogVolume(CTerrain * pTerrain, CVisArea * pVisArea)
   {
   pVisArea->m_pFogVolume=0;
   for(i32 f=0; f<Get3DEngine()->GetFogVolumes().Count(); f++)
   {
    ...
    if(!Get3DEngine()->GetFogVolumes()[f].bOcean)
    {
      ...
      if(!bIntersect)
      {
        for(i32 i=0; i<pVisArea->m_lstShapePoints.Count(); i++)
...
void CVisAreaUpr::PortalsDrawDebug()
{
	UpdateConnections();
	/*
	   if(m_pCurArea)
	   {
	    for(i32 p=0; p<m_pCurArea->m_lstConnections.Count(); p++)
	    {
	   ...
	   else*/
	{
		// debug draw areas
		Vec3 oneVec(1, 1, 1);
		for (i32 v = 0; v < m_lstVisAreas.Count(); v++)
		{
		...
		for (i32 v = 0; v < m_lstPortals.Count(); v++)
		{
			...
			ColorB col(
			  (i32)clamp_tpl(fError * 255.0f, 0.0f, 255.0f),
			  (i32)clamp_tpl(fError * (pPortal->m_lstConnections.Count() < 2) * 255.0f, 0.0f, 255.0f),
			  0,
			  64);
			...
			i32 nConnections = pPortal->m_lstConnections.Count();
			if (nConnections == 1)
				col = ColorB(0, 255, 0, 64);
			else
				col = ColorB(0, 0, 255, 64);

			for (i32 i = 0; i < nConnections && i < 2; i++)
		...
		/*
		    ...
		    for(i32 v=0; v<m_lstVisAreas.Count(); v++)
		    for(i32 p=0; p<m_lstVisAreas[v]->m_lstShapePoints.Count(); p++)
		      GetRenderer()->DrawLabel(m_lstVisAreas[v]->m_lstShapePoints[p], 2,"%d", p);
		    for(i32 v=0; v<m_lstPortals.Count(); v++)
		    for(i32 p=0; p<m_lstPortals[v]->m_lstShapePoints.Count(); p++)
		      GetRenderer()->DrawLabel(m_lstPortals[v]->m_lstShapePoints[p], 2,"%d", p);*/
	}
}

void CVisAreaUpr::DrawVisibleSectors(const SRenderingPassInfo& passInfo, u32 passCullMask)
{
	FUNCTION_PROFILER_3DENGINE;

	for (i32 i = 0; i < m_lstVisibleAreas.Count(); i++)
	{
...
void CVisAreaUpr::PhysicalizeInBox(const AABB& bbox)
{
	for (i32 i = 0; i < m_lstVisAreas.Count(); i++)
	{
...
void CVisAreaUpr::DephysicalizeInBox(const AABB& bbox)
{
	for (i32 i = 0; i < m_lstVisAreas.Count(); i++)
	{
		CVisArea* pArea = m_lstVisAreas[i];
...
void CVisAreaUpr::CheckVis(const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	else
	{
		...
		else if (m_pCurArea)
		{
			...
			for (i32 ii = 0; ii < m_lstOutdoorPortalCameras.Count(); ii++) // process all exit portals
			{
				...
				for (i32 i = 0; i < m_lstActiveEntransePortals.Count(); i++) // entrance into another building is visible
		...
		else if (m_lstActiveEntransePortals.Count())
		{
			...
			for (i32 i = 0; i < m_lstActiveEntransePortals.Count(); i++)
...
void CVisAreaUpr::ActivatePortal(const Vec3& vPos, bool bActivate, tukk szEntityName)
{
	//  bool bFound = false;
...
void CVisAreaUpr::ActivateOcclusionAreas(IVisAreaTestCallback* pTest, bool bActivate)
{
	for (i32 v = 0; v < m_lstOcclAreas.Count(); v++)
	{
...
bool CVisAreaUpr::IsValidVisAreaPointer(CVisArea* pVisArea)
{
...
bool CVisAreaUpr::DeleteVisArea(CVisArea* pVisArea)
{
	bool bFound = false;
...
/*void CVisAreaUpr::LoadVisAreaShapeFromXML(XmlNodeRef pDoc)
   {
   for(i32 i=0; i<m_lstVisAreas.Count(); i++)
   {
    delete m_lstVisAreas[i];
   ...
   if (pObjectsNode)
   {
    for (i32 i = 0; i < pObjectsNode->getChildCount(); i++)
    {
      XmlNodeRef pNode = pObjectsNode->getChild(i);
      if (pNode->isTag("Object"))
      {
        tukk  pType = pNode->getAttr("Type");
   ...
   //					pNode->getAttr("DynAmbientColor", pArea->m_vDynAmbColor);
          ...
          if (pPointsNode)
          for (i32 i = 0; i < pPointsNode->getChildCount(); i++)
          {
...
void CVisAreaUpr::UpdateVisArea(CVisArea* pArea, const Vec3* pPoints, i32 nCount, tukk szName, const SVisAreaInfo& info)
{
...
void CVisAreaUpr::UpdateConnections()
{
	// Reset connectivity
	for (i32 p = 0; p < m_lstPortals.Count(); p++)
...
void CVisAreaUpr::MoveObjectsIntoList(PodArray<SRNInfo>* plstVisAreasEntities, const AABB* boxArea, bool bRemoveObjects)
{
	for (i32 p = 0; p < m_lstPortals.Count(); p++)
	{
...
bool CVisAreaUpr::IntersectsVisAreas(const AABB& box, uk * pNodeCache)
{
	FUNCTION_PROFILER_3DENGINE;
...
bool CVisAreaUpr::ClipOutsideVisAreas(Sphere& sphere, Vec3 const& vNormal, uk pNodeCache)
{
	FUNCTION_PROFILER_3DENGINE;
...
//	is still safe.
CVisArea* CVisAreaUpr::CreateVisArea(VisAreaGUID visGUID)
{
	return new CVisArea(visGUID);
}

bool CVisAreaUpr::IsEntityVisAreaVisibleReqursive(CVisArea* pVisArea, i32 nMaxReqursion, PodArray<CVisArea*>* pUnavailableAreas, const SRenderLight* pLight, const SRenderingPassInfo& passInfo)
{
	i32 nAreaId = pUnavailableAreas->Count();
	...
	if (pVisArea)
	{
		...
		{
			if (nMaxReqursion > 1)
				for (i32 n = 0; n < pVisArea->m_lstConnections.Count(); n++)
				{
					// loop other sectors
...
bool CVisAreaUpr::IsEntityVisAreaVisible(IRenderNode* pEnt, i32 nMaxReqursion, const SRenderLight* pLight, const SRenderingPassInfo& passInfo)
{
	if (!pEnt)
		return false;
	...
	/*
	   if(pEnt->GetEntityVisArea())
	   {
	    ...
	    { // check is lsource area was rendered in prev frame
	      ...
	      i32 nRndFrameId = passInfo.GetFrameID();
	      if(abs(pVisArea->m_nRndFrameId - nRndFrameId)>2)
	      {
	        ...
	        if(pEnt->GetEntityVisArea()->IsPortal())
	        {
	          ...
	          for(i32 n=0; n<pPort->m_lstConnections.Count(); n++)
	          { // loop other sectors
	        ...
	        {
	          for(i32 t=0; !bFound && t<pVisArea->m_lstConnections.Count(); t++)
	          { // loop portals
...
i32 __cdecl CVisAreaUpr__CmpDistToPortal(ukk v1, ukk v2)
{
...
void CVisAreaUpr::MakeActiveEntransePortalsList(const CCamera* pCamera, PodArray<CVisArea*>& lstActiveEntransePortals, CVisArea* pThisPortal, const SRenderingPassInfo& passInfo)
{
	lstActiveEntransePortals.Clear();
	float fZoomFactor = passInfo.GetZoomFactor();

	for (i32 nPortalId = 0; nPortalId < m_lstPortals.Count(); nPortalId++)
	{
		...
		if (pPortal->m_lstConnections.Count() == 1 && pPortal != pThisPortal && pPortal->IsActive() && !pPortal->m_bSkyOnly)
		{
			if (!pCamera || pCamera->IsAABBVisible_F(pPortal->m_boxStatics))
			{
				...
				// test occlusion
				if (GetObjUpr()->IsBoxOccluded(pPortal->m_boxStatics, pPortal->m_fDistance, &pColdData->m_occlusionTestClient, false, eoot_PORTAL, passInfo))
					continue;
	...
	if (lstActiveEntransePortals.Count())
	{
		qsort(&lstActiveEntransePortals[0], lstActiveEntransePortals.Count(),
		      sizeof(lstActiveEntransePortals[0]), CVisAreaUpr__CmpDistToPortal);
		//		m_pCurPortal = lstActiveEntransePortals[0];
	}
}

void CVisAreaUpr::MergeCameras(CCamera& cam1, const CCamera& cam2, const SRenderingPassInfo& passInfo)
{
...
void CVisAreaUpr::DrawOcclusionAreasIntoCBuffer(const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
			...
			if (pArea->m_bActive && passInfo.GetCamera().IsAABBVisible_E(pArea->m_boxArea))
			{
				...
				if (fDist < fRadius * pArea->m_fViewDistRatio * fDistRatio && pArea->m_lstShapePoints.Count() >= 2)
				{
					...
					*pArea->m_arrOcclCamera[nRecursiveLevel] = passInfo.GetCamera();
					...
					{
						DRX_ASSERT_MESSAGE(pArea->m_lstShapePoints.Count() == 2, "Occlusion area only supports planes or quads");
	...
	if (m_lstActiveOcclVolumes.Count())
	{
		...
		qsort(&m_lstActiveOcclVolumes[0], m_lstActiveOcclVolumes.Count(),
		      sizeof(m_lstActiveOcclVolumes[0]), CVisAreaUpr__CmpDistToPortal);

		// remove occluded occluders
		for (i32 i = m_lstActiveOcclVolumes.Count() - 1; i >= 0; i--)
		{
		...
		if (GetCVars()->e_Portals == 4)
		{
			...
			for (i32 i = 0; i < m_lstActiveOcclVolumes.Count(); i++)
			{
				CVisArea* pArea = m_lstActiveOcclVolumes[i];
...
/*
   void CVisAreaUpr::CompileObjects()
   {
   // areas
   for(i32 v=0; v<m_lstVisAreas.Count(); v++)
 ...
 */
void CVisAreaUpr::GetStreamingStatus(i32& nLoadedSectors, i32& nTotalSectors)
{
	nLoadedSectors = 0;//m_nLoadedSectors;
	nTotalSectors = m_lstPortals.Count() + m_lstVisAreas.Count();
}

void CVisAreaUpr::GetMemoryUsage(IDrxSizer* pSizer) const
{
	// areas
	for (i32 v = 0; v < m_lstVisAreas.Count(); v++)
...
void CVisAreaUpr::PrecacheLevel(bool bPrecacheAllVisAreas, Vec3* pPrecachePoints, i32 nPrecachePointsNum)
{
	DrxLog("Precaching the level ...");
	...
	for (i32 v = 0; v < m_lstVisAreas.Count() && bPrecacheAllVisAreas; v++)
	{
		...
		/*		bool bGeomFound = false;
		    ...
		    for(i32 s=0; s<ENTITY_LISTS_NUM; s++)
		    for(i32 e=0; e<m_lstVisAreas[v]->m_lstEntities[s].Count(); e++)
		    {
	...
	DrxLog("Precached %u visarea sectors", dwPrecacheLocations);
	...
	for (i32 p = 0; pPrecachePoints && p < nPrecachePointsNum; p++) //loop over outdoor-camera position
	{
		DrxLog("  Precaching PrecacheCamera point %d of %d", p, nPrecachePointsNum);
		for (i32 i = 0; i < 6; i++) //loop over 6 camera orientations
		{
			// setup camera
			CCamera cam = gEnv->pSystem->GetViewCamera();
			Matrix33 mat = Matrix33::CreateRotationVDir(arrCamDir[i], 0);
	...
	DrxLog("Precached %d PrecacheCameraXX points", nPrecachePointsNum);

	GetRenderer()->EnableSwapBuffers(true);

	float fPrecacheTime = GetTimer()->GetAsyncCurTime() - fPrecacheTimeStart;
	DrxLog("Level Precache finished in %.2f seconds", fPrecacheTime);
}

void CVisAreaUpr::GetObjectsAround(Vec3 vExploPos, float fExploRadius, PodArray<SRNInfo>* pEntList, bool bSkip_ERF_NO_DECALNODE_DECALS, bool bSkipDynamicObjects)
{
	...
	/*
	   ...
	   for(i32 i=0; pVisArea && i<pVisArea->m_lstEntities[STATIC_OBJECTS].Count(); i++)
	   {
...
void CVisAreaUpr::IntersectWithBox(const AABB& aabbBox, PodArray<CVisArea*>* plstResult, bool bOnlyIfVisible)
{
	for (i32 p = 0; p < m_lstPortals.Count(); p++)
	{
...
void CVisAreaUpr::AddLightSourceReqursive(SRenderLight* pLight, CVisArea* pArea, i32k nDeepness, const SRenderingPassInfo& passInfo)
{
	if (pArea->IsObjectsTreeValid())
	{
	...
	if (1 < nDeepness)
	{
		for (i32 v = 0; v < pArea->m_lstConnections.Count(); v++)
		{
...
void CVisAreaUpr::AddLightSource(SRenderLight* pLight, const SRenderingPassInfo& passInfo)
{
	...
	if (pLightArea)
	{
		i32 nPortal = pLightArea->IsPortal() ? 1 : 0;
	...
	else if (!bThisAreaInly)
	{
		...
		arrAreas.Clear();
		m_pVisAreaUpr->IntersectWithBox(lightBox, &arrAreas, true);
		for (i32 i = 0; i < arrAreas.Count(); i++)
...
i32 CVisAreaUpr::GetNumberOfVisArea() const
{
	return m_lstPortals.size() + m_lstVisAreas.size();
}

IVisArea* CVisAreaUpr::GetVisAreaById(i32 nID) const
{
	if (nID < 0)
		return NULL;

	if (nID < (i32)m_lstPortals.size())
	{
		return m_lstPortals[nID];
	}
	nID -= m_lstPortals.size();
	if (nID < (i32)m_lstVisAreas.size())
	{
...
//////////////////////////////////////////////////////////////////////////
void CVisAreaUpr::AddListener(IVisAreaCallback* pListener)
{
...
//////////////////////////////////////////////////////////////////////////
void CVisAreaUpr::RemoveListener(IVisAreaCallback* pListener)
{
	m_lstCallbacks.Delete(pListener);
}

void CVisAreaUpr::CloneRegion(const AABB& region, const Vec3& offset, float zRotation)
{
	...
	i32 numAreas = visAreas.size();
	for (i32 i = 0; i < numAreas; ++i)
	{
		...
		tukk pName = pSrcArea->m_pVisAreaColdData->m_sName;

		UpdateVisArea(pCloneArea, &points[0], numPoints, pName, info);
	}
}

void CVisAreaUpr::ClearRegion(const AABB& region)
{
	PodArray<CVisArea*> visAreas;
	IntersectWithBox(region, &visAreas, false);
	...
	for (i32 i = 0; i < numAreas; ++i)
	{
		...
		if (pVisArea->m_boxArea.min.z < region.max.z && pVisArea->m_boxArea.max.z > region.min.z)
		{
			...
			DRX_ASSERT_MESSAGE(!deletedVis || (m_visAreas.Find(pVisArea) >= 0), "Should only clear pooled vis areas, going to leak");
			DRX_ASSERT_MESSAGE(!deletedPortal || (m_portals.Find(pVisArea) >= 0), "Should only clear pooled portals, going to leak");
			DRX_ASSERT_MESSAGE(!deletedOccluder || (m_occlAreas.Find(pVisArea) >= 0), "Should only clear pooled occluders, going to leak");
...
inline void HelperActivateObjectsLayer(PodArray<CVisArea*>& arrayVisArea, u16 nLayerId, bool bActivate, bool bPhys, IGeneralMemoryHeap* pHeap, const AABB& layerBox)
{
	i32 s = arrayVisArea.Count();
	for (i32 i = 0; i < s; ++i)
	{
...
void CVisAreaUpr::MarkAllSectorsAsUncompiled()
{
	HelperMarkAllSectorsAsUncompiled(m_lstPortals);
	HelperMarkAllSectorsAsUncompiled(m_lstVisAreas);
}

void CVisAreaUpr::ActivateObjectsLayer(u16 nLayerId, bool bActivate, bool bPhys, IGeneralMemoryHeap* pHeap, const AABB& layerBox)
{
...
void CVisAreaUpr::GetObjects(PodArray<IRenderNode*>& lstObjects, const AABB* pBBox)
{
	HelperGetObjects(m_lstVisAreas, lstObjects, pBBox);
	HelperGetObjects(m_lstPortals, lstObjects, pBBox);
}

void CVisAreaUpr::GetObjectsByFlags(uint dwFlags, PodArray<IRenderNode*>& lstObjects)
{
	HelperGetObjectsByFlags(m_lstVisAreas, dwFlags, lstObjects);
	HelperGetObjectsByFlags(m_lstPortals, dwFlags, lstObjects);
}

void CVisAreaUpr::GenerateStatObjAndMatTables(std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, std::vector<IStatInstGroup*>* pStatInstGroupTable, SHotUpdateInfo* pExportInfo)
{
...
bool CVisAreaUpr::IsAABBVisibleFromPoint(AABB& box, Vec3 pos)
{
...
bool CVisAreaUpr::FindShortestPathToVisArea(CVisArea* pThisArea, CVisArea* pTargetArea, PodArray<CVisArea*>& arrVisitedAreas, i32& nRecursion, const Shadowvolume& sv)
{
	...
	if (pThisArea)
	{
		for (i32 p = 0; p < pThisArea->m_lstConnections.Count(); p++)
...
CVisArea* CVisAreaUpr::CreateTypeVisArea()
{
...
CVisArea* CVisAreaUpr::CreateTypePortal()
{
...
CVisArea* CVisAreaUpr::CreateTypeOcclArea()
{
...
CVisArea* CVisAreaUpr::FindVisAreaByGuid(VisAreaGUID guid, PodArray<CVisArea*>& lstVisAreas)
{
	if (!guid)
	{
		for (i32 i = 0; i < lstVisAreas.Count(); i++)
		{
...
void CVisAreaUpr::OffsetPosition(const Vec3& delta)
{
	for (i32 i = 0; i < m_lstVisAreas.Count(); i++)
