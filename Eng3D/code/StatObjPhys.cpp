...
//////////////////////////////////////////////////////////////////////////
void CStatObj::PhysicalizeCompiled(CNodeCGF* pNode, i32 bAppend)
{
	...
	if (pMesh)
	{
		...
		{
			for (i32 m = 0; m < mesh.GetSubSetCount(); m++)
			{
				tuk pFaceMats = &faceMaterials[0];
				SMeshSubset& subset = mesh.m_subsets[m];
				for (i32 nFaceIndex = subset.nFirstIndexId / 3, last = subset.nFirstIndexId / 3 + subset.nNumIndices / 3; nFaceIndex < last; nFaceIndex++)
				{
		...
		if (!physData.empty())
		{
			...
			if (pMesh)
			{
				...
				if (mesh.m_pPositionsF16)
				{
					...
					{
						pPhysGeom = GetPhysicalWorld()->GetGeomUpr()->LoadPhysGeometry(stm, 0, 0, 0);
					}
					else
					{
						...
						pPhysGeom = GetPhysicalWorld()->GetGeomUpr()->LoadPhysGeometry(stm, pPos, mesh.m_pIndices, pFaceMats);
						delete[] pPos;
					}
				}
				else
				{
					pPhysGeom = GetPhysicalWorld()->GetGeomUpr()->LoadPhysGeometry(stm, mesh.m_pPositions, mesh.m_pIndices, pFaceMats);
				}
			}
			else
			{
			    ...
			    (uint)(pmd->nTris - 1) > (uint)(GetCVars()->e_PhysProxyTriLimit - 1))
			{
				...
				GetPhysicalWorld()->GetGeomUpr()->UnregisterGeometry(pPhysGeom);
				m_isProxyTooBig = true;
				m_pMaterial = Get3DEngine()->GetMaterialUpr()->LoadMaterial("%ENGINE%/EngineAssets/Materials/PhysProxyTooBig");
			}
			else
...
//////////////////////////////////////////////////////////////////////////
bool CStatObj::PhysicalizeGeomType(i32 nGeomType, CMesh& mesh, float tolerance, i32 bAppend)
{
	...
	for (i32 m = 0; m < mesh.GetSubSetCount(); m++)
	{
		...
		for (i32 idx = subset.nFirstIndexId; idx < subset.nFirstIndexId + subset.nNumIndices; idx += 3)
		{
			for (i32 c = 0; c < 3; ++c)
			{
	...
	Vec3* pVertices = NULL;
	i32 nVerts = mesh.GetVertexCount();
	...
	if (nVerts > 2)
	{
		...
		i32 nMinTrisPerNode = 2;
		i32 nMaxTrisPerNode = 4;
		...
		//////////////////////////////////////////////////////////////////////////
		IGeomUpr* pGeoman = GetPhysicalWorld()->GetGeomUpr();
...
//////////////////////////////////////////////////////////////////////////
void CStatObj::AssignPhysGeom(i32 nGeomType, phys_geometry* pPhysGeom, i32 bAppend, i32 bLoading)
{
	assert(pPhysGeom);
	...
	if (m_pMaterial)
	{
		i32 surfaceTypesId[MAX_SUB_MATERIALS], flagsPhys[MAX_SUB_MATERIALS];
		memset(surfaceTypesId, 0, sizeof(surfaceTypesId));
		i32 i, j, numIds = m_pMaterial->FillSurfaceTypeIds(surfaceTypesId);

		if (nGeomType == PHYS_GEOM_TYPE_DEFAULT)
		{
			ISurfaceTypeUpr* pSurfaceTypeUpr = Get3DEngine()->GetMaterialUpr()->GetSurfaceTypeUpr();
			ISurfaceType* pMat;
			...
			for (i = 0; i < numIds; i++)
			{
				...
				if ((pMat = pSurfaceTypeUpr->GetSurfaceType(surfaceTypesId[i])) &&
				    (pMat->GetFlags() & SURFACE_TYPE_VEHICLE_ONLY_COLLISION))
				{
					flagsPhys[i] |= (i32)1 << 16;
				}
				if (flags & sf_manually_breakable)
					flagsPhys[i] |= (i32)1 << 17;
			}
		...
		GetPhysicalWorld()->GetGeomUpr()->SetGeomMatMapping(pPhysGeom, surfaceTypesId, numIds);
...
void CStatObj::SavePhysicalizeData(CNodeCGF* pNode)
{
	...
	for (i32 nGeomType = 0; nGeomType < MAX_PHYS_GEOMS_TYPES; nGeomType++)
	{
...
{
	T* ptr[2];
	T& operator[](i32 idx) { return ptr[idx >> 15][idx & ~(1 << 15)]; }
	T* operator+(i32 idx)  { return ptr[idx >> 15] + (idx & ~(1 << 15)); }
	T* operator-(i32 idx)  { return ptr[idx >> 15] - (idx & ~(1 << 15)); }
};
}

static inline i32 mapiTri(i32 itri, i32* pIdx2iTri)
{
	i32 mask = (itri - BOP_NEWIDX0) >> 31;
	return itri & mask | pIdx2iTri[itri - BOP_NEWIDX0 | mask];
}

static inline void swap(i32* pSubsets, vtx_idx* pidx, i32* pmap, i32 i1, i32 i2)
{
	i32 i = pSubsets[i1];
...
static void qsort(i32* pSubsets, vtx_idx* pidx, i32* pmap, i32 ileft, i32 iright, i32 iter = 0)
{
...
static inline i32 check_mask(u32* pMask, i32 i)
{
	return pMask[i >> 5] >> (i & 31) & 1;
}
static inline void set_mask(u32* pMask, i32 i)
{
	pMask[i >> 5] |= 1u << (i & 31);
}
static inline void clear_mask(u32* pMask, i32 i)
{
	pMask[i >> 5] &= ~(1u << (i & 31));
}

static i32 InjectVertices(CMesh* pMesh, i32 nNewVtx, SMeshBoneMapping_uint8*& pBoneMapping)
{
	assert(pMesh->m_pPositionsF16 == 0);

	i32 i, j, nVtx0 = pMesh->GetVertexCount(), nVtx, nMovedSubsets = 0;
	...
	for (i = (i32)pMesh->m_subsets.size() - 1; i >= 0; i--)
	{
...
{
	i32 i, j, j1, itri, iop, itriOrg, ivtx, ivtxNew, nTris0, nVtx0, iSubsetB = 0, nNewVtx, nNewPhysVtx, ivtxRef, bEmptyObj, ibox;
	i32* pIdx2iTri, * piTri2Idx, * pSubsets, * pSubsets0, * pVtxWeld;
	i32 bHasColors[2] = { 0, 0 };
	vtx_idx* pIdx0, * pIdx;
	float dist[3];
	Vec4 Color;
	u32* pRemovedVtxMask;
...
#define AllocVtx                                                                                \
  ...
  {                                                                                             \
    ...
    pVtxMap.ptr[1] = (i32*)realloc(pVtxMap.ptr[1], (nNewVtx + 32) * sizeof(pVtxMap.ptr[1][0])); \
  ...
  ivtx = nNewVtx++ | 1 << 15
	...
	else if (!(pStatObj[0]->GetFlags() & STATIC_OBJECT_CLONE))
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, "Error: breaking StatObj unsuitable for breaking (%s)", pStatObj[0]->GetFilePath());
			return pStatObj[0];
		}
		...
		{
			pObjClone->Release();
			DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, "Error: breaking StatObj without a mesh (%s)", pStatObj[0]->GetFilePath());
			pStatObj[0]->SetFlags(pStatObj[0]->GetFlags() | STATIC_OBJECT_CANT_BREAK);
			return pStatObj[0];
		}
		pObjClone->GetIndexedMesh(true)->GetMesh()->CopyFrom(*(pMesh[0] = pStatObj[0]->GetIndexedMesh(true)->GetMesh()));
	...
	memset(pVtxWeld = new i32[pPhysMesh->nVertices], -1, pPhysMesh->nVertices * sizeof(i32));
	memset(pRemovedVtxMask = new u32[((pPhysMesh->nVertices - 1) >> 5) + 1], 0, (((pPhysMesh->nVertices - 1) >> 5) + 1) * sizeof(i32));
	pVtxMap.ptr[1] = 0;
	pVtx.ptr[1] = 0;
	BBox.Reset();
	...
	piTri2Idx = new i32[nTris0];
	pSubsets = new i32[nTris0];
	pIdx2iTri = (new i32[nTris0 + 1]) + 1;
	for (i = 0; i < nTris0; i++)
		piTri2Idx[i] = i, pSubsets[i] = 1 << 30;
	pIdx2iTri[-1] = 0;
	for (i = 0; i < (i32)pMesh[0]->m_subsets.size(); i++)
	...
	for (pmu = pmu0; pmu; pmu = pmu->next)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, "Error: breaking StatObj unsuitable for breaking (%s)", pStatObj[1]->GetFilePath());
			pPhysGeom->SetForeignData(0, DATA_MESHUPDATE);
			pStatObj[0]->SetFlags(pStatObj[0]->GetFlags() | STATIC_OBJECT_CANT_BREAK);
			goto ExitUDS;
		}
		...
		i32 subset0 = 0, subsBmask = 0, * pSubsetsB = &subset0;

		if (pMesh[0]->GetIndexCount())
		{
			if ((j = max(1, pStatObj[1]->GetMaterial()->GetSubMtlCount())) > 1)
			{
				pSubsetsB = new i32[pMesh[1]->GetIndexCount() / 3];
				subsBmask = -1;
			}
			for (--j; j >= 0; j--)
			{
				pMaterial[1] = pStatObj[1]->GetMaterial()->GetSafeSubMtl(j);
				matname = pMaterial[1]->GetName();
				for (i = pMaterial[0]->GetSubMtlCount() - 1; i >= 0 && strcmp(pMaterial[0]->GetSafeSubMtl(i)->GetName(), matname); i--)
					;
				if (i < 0)
				{
				...
				for (i = 0; i < pMesh[1]->m_subsets.size(); i++)
					if (pMesh[1]->m_subsets[i].nMatID == j)
						for (i32 itri0 = pMesh[1]->m_subsets[i].nFirstIndexId / 3, itri = itri0, nTris = pMesh[1]->m_subsets[i].nNumIndices / 3; itri < itri0 + nTris; pSubsetsB[itri++ & subsBmask] = iSubsetB)
							;
			}
			subset0 = iSubsetB;
			pSubsets0 = new i32[nTris0];
		...
		else
		{
			...
			pSubsets0 = new i32[pMesh[1]->GetIndexCount() / 3];
			for (i = pMesh[1]->GetIndexCount() / 3 - 1; i >= 0; i--)
				pSubsets0[i] = 1 << 30;
			for (i = 0; i < (i32)pMesh[1]->m_subsets.size(); i++)
			{
		...
		{
			i32k vertexCount = pMesh[0]->GetVertexCount();
			for (i = 0; i < nNewVtx; i++)
		...
		for (i = 0; i < pmu->nNewTri; i++)
		{
			...
			for (j = 0; j < 3; j++)
			{
				...
				else
				{
					...
					if (ivtxNew >= 0 && (j1 = -pmu->pNewVtx[ivtxNew].idxTri[iop] - 1) >= 0 && pTexCoord[0][j1].IsEquivalent(Coord, 0.003f) && pTangents[0][j1].IsEquivalent(Tangent, Bitangent, NormalFlip, 0.01f))
					{
						ivtx = j1;
					}
					else
					{
						...
						i32 flip = 1 - (iop & isneg(-pmu->relScale)) * 2;

						pTexCoord[0][ivtx] = SMeshTexCoord(Coord);
						pTangents[0][ivtx] = SMeshTangents(qrot * Tangent, qrot * Bitangent, NormalFlip * (i16)flip);
						pNormals[0][ivtx] = SMeshNormal(Normal * float(flip));
			...
			pSubsets[itri] = (iop ^ 1 | bEmptyObj) ? pSubsets0[itriOrg] : pSubsetsB[itriOrg & subsBmask];
		}
		if (pSubsetsB != &subset0) delete[] pSubsetsB;
					...
					if (pMesh[1]->m_subsets[i].nMatFlags & MTL_FLAG_HIDEONBREAK)
						...
						for (ibox = 0; ibox < pmu->nMovedBoxes; ibox++)
						{
							...
							if (min(min(n.x, n.y), n.z) > 0) // the subset's center is inside an obstruct box that was attached to this mesh
							{
								if (!pUsedVtx)
								{
									pUsedVtx = new i32[pMesh[1]->GetVertexCount()];
	...
	for (i = 0; i < CMesh::LAST_STREAM; i++)
		if (meshAppend.m_streamSize[i])
		{
			uk pSrcStream = 0, * pTrgStream = 0;
			i32 nElementSize = 0;
			...
			if (pSrcStream && pTrgStream)
				memcpy((tuk)pTrgStream + (nVtx0 + nNewPhysVtx) * nElementSize, pSrcStream, meshAppend.m_streamSize[i] * nElementSize);
		}
	...
	{
		i32k indexCount = pMesh[0]->GetIndexCount();
		for (i = 0; i < indexCount; i++)
			pMesh[0]->m_pIndices[i] = (pMesh[0]->m_pIndices[i] & ~(1 << 15)) + ((nVtx0 + nNewPhysVtx) & - i32(pMesh[0]->m_pIndices[i] >> 15));
	}
		...
		pMesh[0]->m_subsets[i].nFirstIndexId = pMesh[0]->m_subsets[i].nNumIndices = 0;
	{
		...
		{
			minVtx = min(minVtx, (i32)min(min(pMesh[0]->m_pIndices[i * 3], pMesh[0]->m_pIndices[i * 3 + 1]), pMesh[0]->m_pIndices[i * 3 + 2]));
			maxVtx = max(maxVtx, (i32)max(max(pMesh[0]->m_pIndices[i * 3], pMesh[0]->m_pIndices[i * 3 + 1]), pMesh[0]->m_pIndices[i * 3 + 2]));
...
ExitUDS:
	delete[] pVtxWeld;
	if (pVtxMap.ptr[1]) free(pVtxMap.ptr[1]);
	delete[] (pIdx2iTri - 1);
	delete[] piTri2Idx;
	delete[] pSubsets;
	...
	if (pStatObj[0]->GetIndexedMesh(true))
	{
		...
		if (!pStatObj[0]->GetRenderMesh())
		{
			pStatObj[0]->SetBBoxMax(Vec3(0));
			pStatObj[0]->SetBBoxMin(Vec3(0));
		}
...
i32 CStatObj::SubobjHasDeformMorph(i32 iSubObj)
{
	i32 i;
	char nameDeformed[256];
	drx_strcpy(nameDeformed, m_subObjects[iSubObj].name);
	drx_strcat(nameDeformed, "_Destroyed");
...
static inline i32 _getBidx(i32 islot, i32* pIdxABuf, u16* pFaceToFace0A, i32* pFace0ToFaceB, vtx_idx* pIdxB)
{
	i32 idx, mask;
...
i32 CStatObj::SetDeformationMorphTarget(IStatObj* pDeformed)
{
	i32 i, j, k, j0, it, ivtx, nVtxA, nVtxAnew, nVtxA0, nIdxA, nFacesA, nFacesB;
	vtx_idx* pIdxA, * pIdxB;
	i32* pVtx2IdxA, * pIdxABuf, * pFace0ToFaceB;
	u16* pFaceToFace0A, * pFaceToFace0B, maxFace0;
	...
	memset(pVtx2IdxA = new i32[nVtxA + 1], 0, (nVtxA + 1) * sizeof(i32));
	for (i = 0; i < nIdxA; i++)
		pVtx2IdxA[pIdxA[i]]++;
	...
	memset(pFace0ToFaceB = new i32[maxFace0 + 1], -1, (maxFace0 + 1) * sizeof(i32));
	for (i = nFacesB - 1; i >= 0; i--)
...
{
	i32 i, nVtx = pMesh->GetVerticesCount();
	float r2 = r * r, rr = 1 / r;
...
static inline i32 GetEdgeByBuddy(mesh_data* pmd, i32 itri, i32 itri_buddy)
{
	i32 iedge = 0, imask;
...
static i32 __s_pvtx_map_dummy = 0;

static void SyncToRenderMesh(SSyncToRenderMeshContext* ctx,  i32* updateState)
{
	...
	if (pPhysGeom)
	{
		...
		if (pPhysGeom->GetForeignData(DATA_MESHUPDATE) || !ctx->pObj->m_hasClothTangentsData || pObjSrc != ctx->pObj && pObjSrc != ctx->pObj->GetCloneSourceObject())
		{
			...
			if (updateState)
				DrxInterlockedDecrement(updateState);
			pPhysGeom->Unlock(0);
			return;
		}
	}
	...
	i32 iVtx0 = ctx->iVtx0, nVtx = ctx->nVtx, mask = ctx->mask;
	strided_pointer<Vec3> pVtx = ctx->pVtx;
	i32* pVtxMap = (mask == ~0) ? &__s_pvtx_map_dummy : ctx->pVtxMap;
...
static void SkinVertices(SSyncToRenderMeshContext* ctx,  i32* ready)
{
	ctx->pObj->SkinVertices(ctx->pVtx, Matrix34(IDENTITY), ready);
}

DECLARE_JOB(
  "StatObj_SkinVertices"
  , TStatObj_SkinVertices
  , SkinVertices);

IStatObj* CStatObj::UpdateVertices(strided_pointer<Vec3> pVtx, strided_pointer<Vec3> pNormals, i32 iVtx0, i32 nVtx, i32* pVtxMap, float rscale)
{
	CStatObj* pObj = this;
	if (m_pRenderMesh)
	{
		...
		i32 i, j, mask = 0, dummy = 0, nVtxFull;
		if (!pVtxMap)
		...
		if (!m_hasClothTangentsData && GetPhysGeom() && GetPhysGeom()->pGeom->GetType() == GEOM_TRIMESH && m_pRenderMesh)
		{
			...
			for (i = 0; i < nVtxFull && pTangents; i++)
			{
				...
				i32 itri = ctd[i].ivtxT, iedge = ctd[i].sgnNorm, itriT = 0, iedgeT = 0, itri1, loop;
				Vec3 n, edge, edge0;
				n.zero();
		...
		if (!(GetFlags() & STATIC_OBJECT_CLONE) || (GetTetrLattice() || m_hasSkinInfo) && !pObj->m_pClonedSourceObject && pVtx)
		{
		...
		gEnv->pJobUpr->WaitForJob(m_pAsyncUpdateContext->jobState);
		...
		if (GetTetrLattice() || m_hasSkinInfo)
		{
			...
			if (pVtx)
			{
				...
				{
					SkinVertices(pVtx, Matrix34(IDENTITY), NULL);
				}
			}
			return pObj;
		}
...
namespace
{
DrxCriticalSection g_cPrepareSkinData;
}
...
void CStatObj::PrepareSkinData(const Matrix34& mtxSkelToMesh, IGeometry* pPhysSkel, float r)
{
	...
	pSphere = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::sphere::type, &sph);
	sph.r *= 3.0f;
	pSphereBig = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::sphere::type, &sph);
	assert(pPhysSkel);
...
IStatObj* CStatObj::SkinVertices(strided_pointer<Vec3> pSkelVtx, const Matrix34& mtxSkelToMesh,  i32* ready)
{
	...
	if (!m_pRenderMesh || !m_hasSkinInfo)
		return this;
	CStatObj* pObj = this;
	if (!(GetFlags() & STATIC_OBJECT_CLONE) || !pObj->m_pClonedSourceObject)
	{
		pObj = (CStatObj*)Clone(true, true, false);
		pObj->m_pRenderMesh->KeepSysMesh(true);
	}
...
i32 CStatObj::Physicalize(IPhysicalEntity* pent, pe_geomparams* pgp, i32 id, tukk szPropsOverride)
{
	i32 res = -1;
	...
	else
	{
		...
		for (i = m_arrPhysGeomInfo.GetGeomCount() - 1, nNoColl = 0, V = 0.0f; i >= 0; i--)
			if (m_arrPhysGeomInfo.GetGeomType(i) == PHYS_GEOM_TYPE_DEFAULT)
				...
				iNoColl = i, ++nNoColl;
		i32 flags0 = pgp->flags;
		ISurfaceTypeUpr* pSurfaceMan = Get3DEngine()->GetMaterialUpr()->GetSurfaceTypeUpr();
		...
		(pgp->flags &= ~geom_colltype_explosion) |= geom_colltype_explosion & ~-(i32)m_bDontOccludeExplosions;
		(pgp->flags &= ~geom_manually_breakable) |= geom_manually_breakable &  - (i32)m_bBreakableByGame;
		    ...
		    (nNoColl == 1 || nNoColl == 2 && m_arrPhysGeomInfo[PHYS_GEOM_TYPE_NO_COLLIDE] && m_arrPhysGeomInfo[PHYS_GEOM_TYPE_OBSTRUCT]))
		{
			...
			if (m_arrPhysGeomInfo.GetGeomCount() > nNoColl)
			{
				if (nNoColl && m_pParentObject && m_pParentObject != this)
				{
					...
					psj.partid[0] = id;
					psj.partid[1] = pParent->GetSubObjectCount();
			...
			if (nNoColl == 1)
			{
				...
				res = pent->AddGeometry(m_arrPhysGeomInfo[iNoColl], pgp, psj.partid[1]);
				if (bHasJoints)
					pent->SetParams(&psj);
			}
			else
			{
				...
				pent->AddGeometry(m_arrPhysGeomInfo[PHYS_GEOM_TYPE_OBSTRUCT], pgp, psj.partid[1]++);
				if (bHasJoints)
					pent->SetParams(&psj);
				pgp->flags = geom_log_interactions | geom_colltype_foliage_proxy;
				i32 flagsCollider = pgp->flagsCollider;
				pgp->flagsCollider = 0;
				pent->AddGeometry(m_arrPhysGeomInfo[PHYS_GEOM_TYPE_NO_COLLIDE], pgp, psj.partid[1]);
		...
		else
		{
			...
			for (i = 0; i < m_arrPhysGeomInfo.GetGeomCount(); i++)
				if (m_arrPhysGeomInfo.GetGeomType(i) == PHYS_GEOM_TYPE_NO_COLLIDE)
					pgp->flags = geom_colltype_ray, res = pent->AddGeometry(m_arrPhysGeomInfo[i], pgp, id);
				else if (m_arrPhysGeomInfo.GetGeomType(i) == PHYS_GEOM_TYPE_OBSTRUCT)
					pgp->flags = geom_colltype_obstruct, res = pent->AddGeometry(m_arrPhysGeomInfo[i], pgp, id);
		}
		pgp->flags = flags0;
...
#define isalpha(c) isalpha((u8)c)
#define isdigit(c) isdigit((u8)c)

i32 CStatObj::PhysicalizeSubobjects(IPhysicalEntity* pent, const Matrix34* pMtx, float mass, float density, i32 id0, strided_pointer<i32> pJointsIdMap, tukk szPropsOverride, i32 idbodyArtic)
{
	i32 i, j, i0, i1, len = 0, len1, nObj = GetSubObjectCount(), ipart[2], nparts, bHasPlayerOnlyGeoms = 0, nGeoms = 0, id, idNext = 0;
	float V[2], M, scale3, jointsz;
	tukk pval, * properties;
	...
	scale3 = pMtx ? pMtx->GetColumn0().len() * pMtx->GetColumn1().len() * pMtx->GetColumn2().len() : 1.0f;
	...
	for (i = 0; i < nObj; i++)
		if ((pSubObj = GetSubObject(i))->nType == STATIC_SUB_OBJECT_MESH && pSubObj->pStatObj && pSubObj->pStatObj->GetPhysGeom() &&
		    pSubObj->bHidden &&
		    (!strncmp(pSubObj->name, "childof_", 8) && (pSubObj1 = FindSubObject((tukk)pSubObj->name + 8)) ||
	...
	for (i = 0, V[0] = V[1] = M = 0; i < nObj; i++)
		...
		{
			float mi = -1.0f, dens = -1.0f, Vsubobj, scaleLoc3 = scale3 * pSubObj->tm.GetColumn0().len() * pSubObj->tm.GetColumn1().len() * pSubObj->tm.GetColumn2().len();
			...
			if (dens > 0)
				mi = Vsubobj * scaleLoc3 * dens; // Calc mass.

			if (mi != 0.0f)
				V[isneg(mi)] += Vsubobj * scaleLoc3;
			M += max(0.0f, mi);
			...
			{
				Vec3 center = pSubObj->localTM * (((CStatObj*)pSubObj->pStatObj)->m_AABB.GetCenter());
				...
				for (j = 0, i0 = i; j < nObj; j++)
					if (j != i && (pSubObj1 = GetSubObject(j)) && pSubObj1->pStatObj && ((CStatObj*)pSubObj1->pStatObj)->m_nRenderTrisCount > 0 &&
					    (curdist = (pSubObj1->localTM * (((CStatObj*)pSubObj1->pStatObj)->m_AABB.GetCenter()) - center).len2()) < mindist)
					{
						mindist = curdist;
						i0 = j;
					}
	...
	for (i = 0; i < nObj; i++)
		if ((pSubObj = GetSubObject(i))->nType == STATIC_SUB_OBJECT_MESH && pSubObj->pStatObj && ((CStatObj*)pSubObj->pStatObj)->m_arrPhysGeomInfo.GetGeomCount() && !pSubObj->bHidden)
		{
			...
			if (strstr(pSubObj->properties, "force_joint"))
			{
				...
				psj1.bBreakable = 0;
				psj1.pt = *partpos.pMtx3x4 * pSubObj->pStatObj->GetAABB().GetCenter();
				pent->SetParams(&psj1);
			}
		}
		else if (pSubObj->nType == STATIC_SUB_OBJECT_DUMMY && !strncmp(pSubObj->name, "$joint", 6))
		{
			properties = szPropsOverride ? szPropsOverride : (tukk)pSubObj->properties;
			...
			if (pSubObj->name[6] != ' ')
			{
				...
				for (i0 = nparts = 0; i0 < nObj && nparts < 2; i0++)
					    ...
					    strncmp(pSubObj1->name, "skeleton_", 9) && !strstr(pSubObj1->properties, "group"))
					{
						...
						if (!pJointBox)
							pJointBox = GetPhysicalWorld()->GetGeomUpr()->CreatePrimitive(primitives::box::type, &joint_bbox);
						{
				...
				if (strlen((tukk)pSubObj->name) >= 7 && !strncmp((tukk)pSubObj->name + 7, "sample", 6))
					psj.bBroken = 2;
				else if (strlen((tukk)pSubObj->name) >= 7 && !strncmp((tukk)pSubObj->name + 7, "impulse", 7))
			...
			else
			{
				for (i0 = 0; i0 < nObj && ((pSubObj1 = GetSubObject(i0))->nType != STATIC_SUB_OBJECT_MESH ||
				                           (strlen((tukk)pSubObj->name) >= 7 && (strncmp((tukk)pSubObj->name + 7, pSubObj1->name, len = strlen(pSubObj1->name)) || isalpha(pSubObj->name[7 + len])))); i0++)
					;
				for (i1 = 0; i1 < nObj && ((pSubObj1 = GetSubObject(i1))->nType != STATIC_SUB_OBJECT_MESH ||
				                           (strlen((tukk)pSubObj->name) >= 8 && (strncmp((tukk)pSubObj->name + 8 + len, pSubObj1->name, len1 = strlen(pSubObj1->name)) || isalpha(pSubObj->name[8 + len + len1])))); i1++)
					;
				if (i0 >= nObj && i1 >= nObj)
					DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, "Error: cannot resolve part names in %s (%s)", (tukk)pSubObj->name, (tukk)m_szFileName);
			}
			if (pJointsIdMap)
			...
			if ((pval = strstr(properties, "limit")) && (pval - 11 < properties - 11 || strncmp(pval - 11, "constraint_", 11)))
			{
				for (pval += 5; *pval && !isdigit(*pval); pval++)
					;
				if (pval) psj.maxTorqueBend = atof(pval);
			...
			if (pval = strstr(properties, "twist"))
			{
				for (pval += 5; *pval && !isdigit(*pval); pval++)
					;
				if (pval) psj.maxTorqueTwist = atof(pval);
			}
			...
			if (pval = strstr(properties, "push"))
			{
				for (pval += 4; *pval && !isdigit(*pval); pval++)
					;
				if (pval) psj.maxForcePush = atof(pval);
			}
			if (pval = strstr(properties, "pull"))
			{
				for (pval += 4; *pval && !isdigit(*pval); pval++)
					;
				if (pval) psj.maxForcePull = atof(pval);
			}
			if (pval = strstr(properties, "shift"))
			{
				for (pval += 5; *pval && !isdigit(*pval); pval++)
					;
				if (pval) psj.maxForceShift = atof(pval);
			}
			if (pval = strstr(properties, "damage_accum"))
			{
				for (pval += 5; *pval && !isdigit(*pval); pval++)
					;
				if (pval) psj.damageAccum = atof(pval);
			}
			if (pval = strstr(properties, "damage_accum_threshold"))
			{
				for (pval += 5; *pval && !isdigit(*pval); pval++)
					;
				if (pval) psj.damageAccumThresh = atof(pval);
			}
			...
			if ((len = 16, pval = strstr(properties, "constraint_limit")) || (len = 5, pval = strstr(properties, "C_lmt")))
			{
				for (pval += len; *pval && !isdigit(*pval); pval++)
					;
				if (pval) psj.limitConstraint.z = atof(pval);
			}
	...
	for (i = 0; i < nObj; i++)
		if (!strcmp((pSubObj = GetSubObject(i))->name, "$compiled_joints"))
			pent->SetStateFromSnapshotTxt(pSubObj->properties, strlen(pSubObj->properties));

	pe_params_part pp;
	if (bHasSkeletons)
		for (i = 0; i < nObj; i++)
			if ((pSubObj = GetSubObject(i))->nType == STATIC_SUB_OBJECT_MESH && pSubObj->pStatObj && pSubObj->pStatObj->GetPhysGeom() && !pSubObj->bHidden &&
			    !strncmp(pSubObj->name, "skeleton_", 9) && (pSubObj1 = FindSubObject((tukk)pSubObj->name + 9)))
			{
...
void CStatObj::CopyFoliageData(IStatObj* pObjDst, bool bMove, IFoliage* pSrcFoliage, i32* pVtxMap, primitives::box* pMovedBoxes, i32 nMovedBoxes)
{
	CStatObj* pDst = (CStatObj*)pObjDst;
	CMesh* pMesh = 0;
	i32 i, j, nVtx = 0, nVtxDst = 0, isubs, ibone, i1;
	...
	else
	{
		...
		for (i1 = isubs = 0, ibone = 1; i1 < (i32)m_chunkBoneIds.size() && m_chunkBoneIds[i1] != ibone; i1++)
		...
		for (i = 0; i < m_nSpines; ibone += m_pSpines[i++].nVtx - 1)
		{
			...
			{
				for (; i1 < (i32)m_chunkBoneIds.size() && m_chunkBoneIds[i1] != ibone; i1++)
					isubs += !m_chunkBoneIds[i1];
				j = 0;
...
static i32 UpdatePtTriDist(const Vec3* vtx, const Vec3& n, const Vec3& pt, float& minDist, float& minDenom)
{
	float rvtx[3] = { (vtx[0] - pt).len2(), (vtx[1] - pt).len2(), (vtx[2] - pt).len2() }, elen2[2], dist, denom;
	i32 i = idxmin3(rvtx), bInside[2];
...
static inline i32 mapsymb(i32 symb, i32* pmap, i32 n)
{
	i32 i, j;
	for (i = j = 0; j < n; j++)
...
static i32 Bin2ascii(u8k* pin, i32 sz, u8* pout)
{
	i32 a0, a1, a2, i, j, nout, chr[4];
...
static i32 Ascii2bin(u8k* pin, i32 sz, u8* pout, i32 szout)
{
	i32 a0, a1, a2, a3, i, nout;
...
static void SerializeData(TSerialize ser, tukk name, uk pdata, i32 size)
{
	/*static std::vector<i32> arr;
	   if (ser.IsReading())
	   {
	   ser.Value(name, arr);
	...
	{
		ser.Value(name, str);
		i32 n = Ascii2bin((u8k*)(tukk)str, str.length(), (u8*)pdata, size);
		assert(n == size);
		(void)n;
	}
	else
	{
		...
		i32 n = Bin2ascii((u8k*)pdata, size, (u8*)(tukk)str);
		assert(n == str.length());
		(void)n;
		ser.Value(name, str);
	}
...
i32 CStatObj::Serialize(TSerialize ser)
{
	ser.BeginGroup("StatObj");
	ser.Value("Flags", m_nFlags);
	if (GetFlags() & STATIC_OBJECT_COMPOUND)
	{
		i32 i, nParts = m_subObjects.size();
		bool bVal;
		string srcObjName;
	...
	else
	{
		...
		DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, "Error: full geometry serialization should never happen on consoles. file: '%s' Geom: '%s'", m_szFileName.c_str(), m_szGeomName.c_str());
#else
		CMesh* pMesh;
		i32 i, nVtx, nTris, nSubsets;
		string matName;
		IMaterial* pMat;
		...
		if (ser.IsReading())
		{
			...
			if (nVtx)
			{
				...
				SetMaterial(gEnv->p3DEngine->GetMaterialUpr()->FindMaterial(matName));
				ser.Value("MaterialAux", matName);
				if (m_pMaterial && (pMat = gEnv->p3DEngine->GetMaterialUpr()->FindMaterial(matName)))
				{
				...
				if (i)
				{
					...
					IGeometry* pPhysMesh = GetPhysicalWorld()->GetGeomUpr()->LoadGeometry(stm, pMesh->m_pPositions, pMesh->m_pIndices, pIds);
					pPhysMesh->AddRef();
					pPhysMesh->SetForeignData((IStatObj*)this, 0);
					m_arrPhysGeomInfo.SetPhysGeom(GetPhysicalWorld()->GetGeomUpr()->RegisterGeometry(pPhysMesh, 0, surfaceTypesId, numIds));
					delete[] pbuf;
				}
				delete[] pIds;
		...
		else
		{
			if (GetIndexedMesh(true))
			{
				...
				{
					CMemStream stm(false);
					GetPhysicalWorld()->GetGeomUpr()->SaveGeometry(stm, GetPhysGeom()->pGeom);
...
//////////////////////////////////////////////////////////////////////////
IParticleEffect* CStatObj::GetSurfaceBreakageEffect(tukk sType)
{
	if (!m_pRenderMesh)
		return 0;
	...
	for (i32 iss = 0; iss < meshChunks.size(); iss++)
	{
		if ((meshChunks[iss].m_nMatFlags & (MTL_FLAG_NOPHYSICALIZE | MTL_FLAG_NODRAW)) == MTL_FLAG_NOPHYSICALIZE ||
			...
			if (m_pMaterial && ((pSubMtl = m_pMaterial->GetSubMtl(meshChunks[iss].m_nMatID)) || (pSubMtl = m_pMaterial)))
			{
				...
				if (pParticles)
				{
					return gEnv->pParticleUpr->FindEffect(pParticles->particle_effect);
				}
			}
	}
	return 0;
...
void CStatObj::AnalyzeFoliage(IRenderMesh* pRenderMesh, CContentCGF* pCGF)
{
	...
	if (pCGF && pCGF->GetFoliageInfo()->nSpines)
	{
		...
		for (i32 i = 0; i < fi.nSpines; ++i)
		{
			const SSpineRC& srcSpine = fi.pSpines[i];

			i32k nVtx = srcSpine.nVtx;
		...
		std::vector<u16> chunkBoneIds(fi.chunkBoneIds.size());
		for (i32 i = 0; i < (i32)fi.chunkBoneIds.size(); ++i)
		{
		...
		{
			i32k nMaxBoneIds = 256;

			if ((i32)chunkBoneIds.size() <= nMaxBoneIds)
			{
				DrxLog("Vegetation object [%s] has [%" PRISIZE_T "] bone IDs", m_szFileName.c_str(), chunkBoneIds.size());
			}
			else
			{
				DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING,
				           "Vegetation object[%s] has [%" PRISIZE_T "] bone IDs - maximum supported is [%d]",
				           m_szFileName.c_str(), chunkBoneIds.size(), nMaxBoneIds);
			}
		}
		...
		// Convert per-subset bone ids to per-mesh bone ids
		{
...

			{
				...
				for (i32 chunkIndex = 0; chunkIndex < (i32)renderChunks.size(); ++chunkIndex)
				{
					...
					for (i32 i = 0; i < (i32)chunk.nNumVerts; ++i)
					{
						...
						for (i32 jj = 0; jj < 4; ++jj)
						{
							...
							pBoneMapping[vIdx].boneIds[jj] = fi.chunkBoneIds[start + localBoneId];
						}
					}
				}
			}
...
errorDetected:
			if (err)
			{
				...
				for (i32 i = 0; i < fi.nSkinnedVtx; ++i)
				{
					for (i32 jj = 0; jj < 4; ++jj)
					{
	...
	if (!pRenderMesh)
		return;
	i32 i, j, isle, iss, ivtx0, nVtx, itri, ivtx, iedge, ibran0, ibran1, ibran, nBranches, ispine, ispineDst, nGlobalBones,
	    * pIdxSorted, * pForeignIdx;
	Vec3 pt[7], edge, edge1, n, nprev, axisx, axisy;
	Vec2 tex[6];
	float t, s, mindist, k, denom, len, dist[3], e, t0, t1, denomPrev;
	u32* pUsedTris, * pUsedVtx;
	...
	{
		drx_sprintf(sname, "%s_", m_szGeomName.c_str());
		pname += strlen(sname);
		CNodeCGF* pNode;
	...
	do
	{
		...
		do
		{
			drx_sprintf(pname, "branch%d_%d", nBranches + 1, branch.npt + 1);
	...
	pIdxSorted = new i32[nMeshVtx];
	nVtx = 0;
	pForeignIdx = new i32[nMeshIdx / 3];
	pUsedTris = new u32[i = ((nMeshIdx - 1) >> 5) + 1];
	memset(pUsedTris, 0, i * sizeof(i32));
	pUsedVtx = new u32[i = ((nMeshVtx - 1) >> 5) + 1];
	memset(pUsedVtx, 0, i * sizeof(i32));

	// iterate through all triangles, track the closest for each branch point
	for (iss = 0; iss < (i32)meshChunks.size(); iss++)
		if ((meshChunks[iss].m_nMatFlags & (MTL_FLAG_NOPHYSICALIZE | MTL_FLAG_NODRAW)) == MTL_FLAG_NOPHYSICALIZE)
	...
	i32 nBones;
	for (iss = 0, nGlobalBones = 1; iss < (i32)meshChunks.size(); iss++, nGlobalBones += nBones - 1)
		if ((meshChunks[iss].m_nMatFlags & (MTL_FLAG_NOPHYSICALIZE | MTL_FLAG_NODRAW)) == MTL_FLAG_NOPHYSICALIZE)
		{
			...
			for (i = 0, j = meshChunks[iss].nFirstIndexId / 3; j*3 < meshChunks[iss].nFirstIndexId + meshChunks[iss].nNumIndices; j++, i++)
				pForeignIdx[i] = j;
			pPhysGeom = GetPhysicalWorld()->GetGeomUpr()->CreateMesh(pMeshVtx, pMeshIdx + meshChunks[iss].nFirstIndexId, 0, pForeignIdx,
...
i32 CStatObj::PhysicalizeFoliage(IPhysicalEntity* pTrunk, const Matrix34& mtxWorld, IFoliage*& pIRes, float lifeTime, i32 iSource)
{
	...
	float collDist0 = 0.03f, collDist1 = 0.07f;
	if (tukk thickness = strstr(m_szProperties, "thickness"))
		collDist0 = atof(thickness + 9), collDist1 = 0;
	pr.bTargetPoseActive = 2;
	pr.stiffnessAnim = GetFloatCVar(e_FoliageBranchesStiffness);
	if (tukk stiffness = strstr(m_szProperties, "stiffness"))
		pr.stiffnessAnim = atof(stiffness + 9);
	pRes->m_stiffness = pr.stiffnessAnim;
	pr.friction = 2.0f;
	...
	else
	{
		...
		if (tukk damping = strstr(m_szProperties, "damping"))
			pr.dampingAnim = atof(damping + 7);
	}
	pRes->m_minEnergy = sp.minEnergy = sqr(0.01f - 0.09f * j);
	pfd.iForeignData = PHYS_FOREIGN_ID_FOLIAGE;
	pfd.pForeignData = pRes;
	pto.maxTimeIdle = GetCVars()->e_FoliageBranchesTimeout;
	if (tukk timeout = strstr(m_szProperties, "timeout"))
	...
	for (i = 0; i < m_nSpines; i++)
		if (m_pSpines[i].bActive)
		{
			...
			pr.collDist = collDist0 + collDist1 * isneg(3.0f - m_pSpines[i].len);
			pfd.iForeignFlags = i;
			...
			{
				i32 ivtx = m_pSpines[i].iAttachSeg;
	...
	if (!gEnv->IsDedicated() && m_pRenderMesh && !m_pRenderMesh->GetChunksSkinned().size() && m_pRenderMesh->GetVerticesCount())
	{
		...
		for (i = 0; i < (i32)meshChunks.size(); i++)
		{
		...
		for (i = 0; i < (i32)meshChunks.size(); i++)
			if (!meshChunks[i].m_bUsesBones)
				for (j = meshChunks[i].nFirstVertId; j < (i32)meshChunks[i].nFirstVertId + (i32)meshChunks[i].nNumVerts; j++)
		...
		if (0)
		{
			...
			//--------------------------------------------------------------------------------------
			for (u32 s = 0; s < (u32)meshChunks.size(); s++)
			{
				u32k nStartIndex = meshChunks[s].nFirstVertId;
				u32k nNumVertices = meshChunks[s].nNumVerts;

				if (meshChunks[s].m_bUsesBones)
				{
					for (i = nStartIndex; i < (i32)(nStartIndex + nNumVertices); i++)
					{
						u32 v = i;
						// get bone-ids
						u16 b0 = m_pBoneMapping[v].boneIds[0];
						u16 b1 = m_pBoneMapping[v].boneIds[1];
						u16 b2 = m_pBoneMapping[v].boneIds[2];
						u16 b3 = m_pBoneMapping[v].boneIds[3];
						// get weights
						u8k w0 = m_pBoneMapping[v].weights[0];
						u8k w1 = m_pBoneMapping[v].weights[1];
						u8k w2 = m_pBoneMapping[v].weights[2];
						u8k w3 = m_pBoneMapping[v].weights[3];
						// if weight is zero set bone ID to zero as the bone has no influence anyway,
						if (w0 == 0) b0 = 0;
						if (w1 == 0) b1 = 0;
						if (w2 == 0) b2 = 0;
						if (w3 == 0) b3 = 0;
						static i32k maxBoneIdx = 255;
						...
						/*
						   if (b0 > maxBoneIdx)
						   DrxFatalError("Fatal Error: index out of range: Index: %d   maxBoneIdx: %d (contact: Anton@Dinrus.de or Ivo@Dinrus.de)",b0,maxBoneIdx );
						   if (b1 > maxBoneIdx)
						   DrxFatalError("Fatal Error: index out of range: Index: %d   maxBoneIdx: %d (contact: Anton@Dinrus.de or Ivo@Dinrus.de)",b1,maxBoneIdx );
						   if (b2 > maxBoneIdx)
						   DrxFatalError("Fatal Error: index out of range: Index: %d   maxBoneIdx: %d (contact: Anton@Dinrus.de or Ivo@Dinrus.de)",b2,maxBoneIdx );
						   if (b3 > maxBoneIdx)
						   DrxFatalError("Fatal Error: index out of range: Index: %d   maxBoneIdx: %d (contact: Anton@Dinrus.de or Ivo@Dinrus.de)",b3,maxBoneIdx );
						 */
					}
				}
			}
...
u32 CStatObjFoliage::ComputeSkinningTransformationsCount()
{
	u32 count = 1;
	u32 spineCount = u32(m_pStatObj->m_nSpines);
	for (u32 i = 0; i < spineCount; ++i)
		count += u32(m_pStatObj->m_pSpines[i].nVtx - 1);
	return count;
}

void CStatObjFoliage::ComputeSkinningTransformations(u32 nList)
{
	u32 jointCount = ComputeSkinningTransformationsCount();
	...
	for (u32 i = 0; i < spineCount; ++i)
	{
		if (!m_pRopes[i])
		{
			jointIndex += u32(m_pStatObj->m_pSpines[i].nVtx - 1);
			continue;
		}
		...
		if (sr.timeLastActive > m_pRopesActiveTime[i])
		{
			...
			m_pRopes[i]->GetStatus(&sr);
			point0 = sr.pPoints[0];
			u32 spineVertexCount = m_pStatObj->m_pSpines[i].nVtx - 1;
			for (u32 j = 0; j < spineVertexCount; ++j)
			{
				assert(jointIndex < jointCount);
				PREFAST_ASSUME(jointIndex < jointCount);
		...
		{
			u32 count = m_pStatObj->m_pSpines[i].nVtx - 1;
...
SSkinningData* CStatObjFoliage::GetSkinningData(const Matrix34& RenderMat34, const SRenderingPassInfo& passInfo)
{
	...
	i32 nNumBones = ComputeSkinningTransformationsCount();

	// get data to fill
	i32 nSkinningFrameID = passInfo.GetIRenderView()->GetSkinningPoolIndex();
	i32 nSkinningList = nSkinningFrameID % 3;
	i32 nPrevSkinningList = (nSkinningFrameID - 1) % 3;

	//i32 nCurrentFrameID = passInfo.GetFrameID();
...
void CStatObjFoliage::Update(float dt, const CCamera& rCamera)
{
	...
	i32 i, j, nContactEnts = 0, nColl;
	//i32 nContacts[4]={0,0,0,0};
	pe_status_rope sr;
	pe_params_bbox pbb;
	...
	i32 bVisible = rCamera.IsAABBVisible_E(bbox);
	i32 bEnable = bVisible & isneg(((rCamera.GetPosition() - bbox.GetCenter()).len2() - sqr(clipDist)) * clipDist - 0.0001f);
	if (!bEnable)
	{
...
void CStatObjFoliage::SetFlags(i32 flags)
{
	if (flags != m_flags)
	{
		pe_params_rope pr;
		...
		for (i32 i = 0; i < m_pStatObj->m_nSpines; i++)
			if (m_pRopes[i])
...
void CStatObjFoliage::BreakBranch(i32 idx)
{
	if (m_pRopes[idx])
	{
		i32 i, nActiveRopes;
...
i32 CStatObjFoliage::Serialize(TSerialize ser)
{
	i32 i, nRopes = m_nRopes;
	bool bVal;
