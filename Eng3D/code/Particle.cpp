...
void CParticle::Move(SParticleState& state, float fTime, STargetForces const& forces_ext) const
{
	...
	if (params.fTurbulence3DSpeed)
	{
		...
		for (i32 a = 0; a < 3; a++)
		{
			CChaosKey key(*(u32*)&state.m_Loc.t[a]);
...
{
	hit.dist = 1.f;
	u32 nCollideFlags = context.nEnvFlags & ENV_COLLIDE_ANY;
	if (nCollideFlags & ENV_COLLIDE_CACHE && !(GetCVars()->e_ParticlesCollisions & AlphaBit('p')))
	{
		...
		{
			i32& iDrawHelpers = gEnv->pPhysicalWorld->GetPhysVars()->iDrawHelpers;
			i32 iSave = iDrawHelpers;
			iDrawHelpers = 0;
			ray_hit hit2;
...
void CParticle::Init(SParticleUpdateContext const& context, float fAge, CParticleSubEmitter* pEmitter, const EmitParticleData& data)
{
	...
	if (params.Connection)
		...
		m_nTileVariant = drx_random(0U, params.TextureTiling.nVariantCount - 1);
...
void CParticle::InitPos(SParticleUpdateContext const& context, QuatTS const& loc, float fEmissionStrength)
{
	...
	if (eAttachType != GeomType_None && rSource.GetEmitGeom())
	{
		...
		rSource.GetEmitGeom().GetRandomPos(ran, drx_random_next(), eAttachType, eAttachForm, loc, bCentered);
		m_Loc.t = ran.vPos;
	...
	if ((fPhi > 0.f) && (m_Vel.vLin.len2() > FLT_EPSILON))
	{
		...
		float fTheta = drx_random(0.0f, DEG2RAD(360));
	...
	if (context.b3DRotation)
	{
		...
		Vec3 vAngles = DEG2RAD(params.vInitAngles + drx_random_componentwise(-r, r));
		m_Loc.q = m_Loc.q * Quat::CreateRotationXYZ(Ang3(vAngles));
		r = params.vRandomRotationRate;
		m_Vel.vRot = m_Loc.q * DEG2RAD(params.vRotationRate + drx_random_componentwise(-r, r));
		m_fAngle = 0.f;
	}
	else
	{
		// 2D relative rotation about Y.
		m_fAngle = DEG2RAD(params.vInitAngles.y + drx_random(-1.0f, 1.0f) * params.vRandomAngles.y);
		m_Vel.vRot.y = DEG2RAD(params.vRotationRate.y + drx_random(-1.0f, 1.0f) * params.vRandomRotationRate.y);
	}
...
Vec3 CParticle::GenerateOffset(SParticleUpdateContext const& context)
{
	...
	{
		vOffset.Set(
		  drx_random(-1.0f, 1.0f) * params.vRandomOffset.x,
		  drx_random(-1.0f, 1.0f) * params.vRandomOffset.y,
		  drx_random(-1.0f, 1.0f) * params.vRandomOffset.z);
...
void CParticle::UpdateAllocations(i32 nPrevHistorySteps)
{
	SParticleHistory* aPrevHist = m_aPosHistory;
	if (i32 nNewSteps = GetContainer().GetHistorySteps())
	{
		if ((m_aPosHistory = (SParticleHistory*) ParticleObjectAllocator().Allocate(sizeof(SParticleHistory) * nNewSteps)))
		{
			...
			for (i32 n = nPrevHistorySteps; n < nNewSteps; n++)
				m_aPosHistory[n].SetUnused();
		}
	}
	else
...
void CParticle::AddPosHistory(SParticleState const& stateNew)
{
	...
	i32 nCount = GetContainer().GetHistorySteps();
	...
	if (nStart > 0)
	{
		for (i32 n = nStart; n < nCount; n++)
			m_aPosHistory[n - nStart] = m_aPosHistory[n];
		for (i32 n = nCount - nStart; n < nCount; n++)
			m_aPosHistory[n].SetUnused();
		nCount -= nStart;
	}
...
void CParticle::Update(SParticleUpdateContext const& context, float fFrameTime, bool bNew)
{
	...
	{
		u32 nFlags = context.nEnvFlags & ENV_PHYS_AREA;
		u32 nCollideFlags = m_pCollisionInfo ? context.nEnvFlags & ENV_COLLIDE_ANY : 0;
		...
		while (fFrameTime > 0.f)
		{
			...
			if (params.bSpaceLoop)
			{
				...
				for (i32 a = 0; a < 3; a++)
				{
					...
					{
						float fCorrect = float(i32(fPosSpace));
						fCorrect += drxmath::signnz(fCorrect);
...
void CParticle::GetCollisionParams(i32 nCollSurfaceIdx, float& fElasticity, float& fDrag) const
{
	// Get phys params from material, or particle params.
	...
	i32 iSurfaceIndex = GetSurfaceIndex();
	if (iSurfaceIndex > 0)
	{
...
i32 CParticle::GetSurfaceIndex() const
{
	ResourceParticleParams const& params = GetParams();
	...
	if (params.pMaterial)
	{
		...
		if (i32 index = params.pMaterial->GetSurfaceTypeId())
			return index;
	}
	if (m_pMeshObj)
	{
