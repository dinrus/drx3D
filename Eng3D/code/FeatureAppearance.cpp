...
#include <drx3D/Eng3D/FeatureCommon.h>
#include <drx3D/CoreX/Serialization/Decorators/Resources.h>

namespace pfx2
{

MakeDataType(EPDT_Tile, u8);
...
private:
	...
	void AssignTiles(CParticleComponentRuntime& runtime)
	{
		...
		TIOStream<u8> tiles = container.IOStream(EPDT_Tile);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureAppearanceTextureTiling, "Appearance", "Texture Tiling", colorAppearance);
...
public:
	...
	virtual void AddToComponent(CParticleComponent* pComponent, SComponentParams* pParams) override
	{
		...
		{
			pParams->m_pMaterial = gEnv->p3DEngine->GetMaterialUpr()->FindMaterial(m_materialName);
			if (!pParams->m_pMaterial)
			{
				GetPSystem()->CheckFileAccess(m_materialName);
				pParams->m_pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(m_materialName);
			}
		}
	...
	virtual tukk GetResourceName(uint resourceId) const override
	{
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureAppearanceMaterial, "Appearance", "Material", colorAppearance);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureAppearanceLighting, "Appearance", "Lighting", colorAppearance);
...
public:
	...
	virtual void AddToComponent(CParticleComponent* pComponent, SComponentParams* pParams) override
	{
		pParams->m_renderStateFlags = (pParams->m_renderStateFlags & ~OS_TRANSPARENT) | (i32)m_blendMode;
		MakeGpuInterface(pComponent, gpu_pfx2::eGpuFeatureType_Dummy);
	}
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureAppearanceBlending, "Appearance", "Blending", colorAppearance);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureAppearanceSoftIntersect, "Appearance", "SoftIntersect", colorAppearance);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureAppearanceVisibility, "Appearance", "Visibility", colorAppearance);
