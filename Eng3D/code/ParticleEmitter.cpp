...
JobUpr::SJobState* CParticleBatchDataUpr::AddUpdateJob(CParticleEmitter* pEmitter)
{
	JobUpr::SJobState* pJobState = GetJobState();
...
void CParticleEmitter::AddUpdateParticlesJob()
{
	...
	m_pUpdateParticlesJobState = CParticleUpr::Instance()->AddUpdateJob(this);
}
...
void CParticleEmitter::SyncUpdateParticlesJob()
{
	if (m_pUpdateParticlesJobState)
	{
		...
		gEnv->GetJobUpr()->WaitForJob(*m_pUpdateParticlesJobState);
...
IMaterial* CParticleEmitter::GetMaterial(Vec3*) const
{
	...
	if (!m_Containers.empty())
		return reinterpret_cast<const CParticleSubEmitter*>(m_Containers.front())->GetParams().pMaterial;
	return NULL;
}
...
float CParticleEmitter::GetMaxViewDist()
{
	...
	return CParticleUpr::Instance()->GetMaxAngularDensity(gEnv->pSystem->GetViewCamera())
	       * m_fMaxParticleSize
	       * GetParticleScale()
	       * m_fViewDistRatio;
}
...
void CParticleEmitter::UpdateState()
{
	...
	if (m_bbWorld.IsReset() && nEnvFlags & ENV_PHYS_AREA)
	{
		...
		m_PhysEnviron.Update(CParticleUpr::Instance()->GetPhysEnviron(), AABB(vPos),
	...
	CParticleUpr::Instance()->GetPhysEnviron().Update(m_PhysEnviron, m_bbWorld, 
...
{
	uk pMem = m_Containers.push_back_new();
	if (pMem)
...
void CParticleEmitter::AddEffect(CParticleContainer* pParentContainer, const CParticleEffect* pEffect, bool bUpdate)
{
	...
	if (pEffect->IsActive())
	{
		...
		{
			// Add new container
			if (uk pMem = pNext ? m_Containers.insert_new(pNext) : m_Containers.push_back_new())
	...
	if (pEffect)
	{
		for (i32 i = 0, n = pEffect->GetChildCount(); i < n; i++)
		{
...
void CParticleEmitter::EmitParticle(const EmitParticleData* pData)
{
	...
	if (!m_Containers.empty())
	{
	     reinterpret_cast<CParticleEmitter*>(m_Containers.front())->EmitParticle(pData);
		UpdateResetAge();
		m_fStateChangeAge = m_fAge;
	}
}

void CParticleEmitter::SetEntity(IEntity* pEntity, i32 nSlot)
{
	...
	for (auto& c : m_Containers)
		if (CParticleSubEmitter* e = (CParticleSubEmitter*) c.GetDirectEmitter())
			e->ResetLoc();
}
...
void CParticleEmitter::Render(SRendParams const& RenParams, const SRenderingPassInfo& passInfo)
{
	...
	u32 nRenFlags = REN_SPRITE | REN_GEOMETRY;
	u32 nRenRequiredFlags = 0;
	...
	u32 nEnvFlags = m_nEnvFlags & CParticleUpr::Instance()->GetAllowedEnvironmentFlags();
	nRenFlags &= nEnvFlags;

	if (!nRenFlags)
		return;

	u32 nRenRequiredCheckFlags = nRenRequiredFlags & ~nRenFlags;
	...
	PartParams.m_fMaxAngularDensity = CParticleUpr::Instance()->GetMaxAngularDensity(passInfo.GetCamera()) * GetParticleScale() * m_fViewDistRatio;
	...
	PartParams.m_nRenObjFlags = CParticleUpr::Instance()->GetRenderFlags();

	if (!passInfo.IsAuxWindow())
	{
		CLightVolumesMgr& lightVolumeUpr = m_p3DEngine->GetLightVolumeUpr();
		PartParams.m_nDeferredLightVolumeId = lightVolumeUpr.RegisterVolume(GetPos(), m_bbWorld.GetRadius() * 0.5f, RenParams.nClipVolumeStencilRef, passInfo);
	}
	else
	{
	...
	if (passInfo.IsAuxWindow())
	{
		...
		CParticleUpr::Instance()->PrepareForRender(passInfo);
	}
	...
	for (auto& c : m_Containers)
	{
		u32 nContainerFlags = c.GetEnvironmentFlags();
	...
	if (passInfo.IsAuxWindow())
	{
		...
		CParticleUpr::Instance()->FinishParticleRenderTasks(passInfo);
		RenderDebugInfo();
	}
	...
	if (TimeNotRendered() < 0.25f)
	{
		...
		if (bSelected || (GetCVars()->e_ParticlesDebug & AlphaBit('b')))
		{
			...
			if (!bSelected)
			{
				...
				color = DrxStringUtils::CalculateHash(GetEffect()->GetName());
...
void CParticleEmitter::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void CParticleEmitter::UpdateStreamingPriority(const SUpdateStreamingPriorityContext& context)
{
	...
	for (const auto& c : m_Containers)
	{
		...
		const float normalizedDist = context.distance * drxmath::rcp_safe(params.fSize.GetMaxValue());
		...
		{
			m_pObjUpr->PrecacheStatObj(pStatObj, context.lod, Matrix34A(m_Loc), pStatObj->GetMaterial(),
