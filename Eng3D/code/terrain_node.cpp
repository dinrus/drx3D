...
i32 CTerrainNode::m_nNodesCounter = 0;

CTerrainNode* CTerrainNode::GetTexuringSourceNode(i32 nTexMML, eTexureType eTexType)
{
...
CTerrainNode* CTerrainNode::GetReadyTexSourceNode(i32 nTexMML, eTexureType eTexType)
{
...
void CTerrainNode::SetupTexturing(bool bMakeUncompressedForEditing, const SRenderingPassInfo& passInfo)
{
	...
	i32 nRL = passInfo.GetRecursiveLevel();
	...
	i32 nDefaultTexId = GetTerrain()->m_nWhiteTexId;
...
bool CTerrainNode::CheckVis(bool bAllInside, bool bAllowRenderIntoCBuffer, const SRenderingPassInfo& passInfo, u32 passCullMask)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	i32k posponeGeomErrorUpdateFrames = 5;
	...
	else
	{
		...
		{
			GetObjUpr()->PushIntoCullQueue(SCheckOcclusionJobData::CreateTerrainJobData(this, boxWS, distance, passCullMask));
		}
		else
		{
...
void CTerrainNode::Init(i32 x1, i32 y1, i32 nNodeSize, CTerrainNode* pParent, bool bBuildErrorsTable)
{
	m_pChilds = NULL;
	...
	u8 m_cNodeNewTexMML = m_cNodeNewTexMML_Min = 0;
	...
	m_nLastTimeUsed = (i32)GetCurTimeSec() + 20;
	...
	for (i32 iStackLevel = 0; iStackLevel < MAX_RECURSION_LEVELS; iStackLevel++)
	{
...
void CTerrainNode::SaveCompressedMipmapLevel(ukk data, size_t size, uk userData)
{
	gTerrainCompressedImgData.Clear();
	gTerrainCompressedImgData.AddList((byte*)data, size);
}

void CTerrainNode::EnableTextureEditingMode(u32 nEditorDiffuseTex)
{
	FUNCTION_PROFILER_3DENGINE;
...
void CTerrainNode::UpdateNodeTextureFromEditorData()
{
	...
	i32 texDim = GetTerrain()->m_arrBaseTexInfos.m_TerrainTextureLayer[0].nSectorSizePixels;
	...
	for (i32 i = 0; i < lookupTableSize; i++)
	{
		...
		colR.Clamp();
		colR.rgb2srgb();
		lookupTable[i] = SATURATEB((u32)(colR.r * 255.0f + 0.5f));
	}

	float sectorSizeM = GetBBox().GetSize().x;

	i32 smallestEditorTileSizeM = GetTerrain()->GetTerrainSize() / 16;
...
void CTerrainNode::UpdateNodeNormalMapFromHeightMap()
{
	...
	i32 nTexSize = GetTerrain()->m_arrBaseTexInfos.m_TerrainTextureLayer[1].nSectorSizePixels;
	...
	for (i32 x = 0; x < nTexSize; x++)
	{
		for (i32 y = 0; y < nTexSize; y++)
		{
			...
			u32 dwR = SATURATEB(u32(vNormal.x * 127.5f + 127.5f));
			u32 dwB = SATURATEB(u32(vNormal.y * 127.5f + 127.5f));
			u32 dwA = 0;
			u32 dwG = 0;
...
void CTerrainNode::CheckNodeGeomUnload(const SRenderingPassInfo& passInfo)
{
	...
	i32 nTime = fastftol_positive(GetCurTimeSec());
	...
	{
		// try to release vert buffer if not in use i32 time
		ReleaseHeightMapGeometry();
	}
}
...
void CTerrainNode::RenderNodeHeightmap(const SRenderingPassInfo& passInfo, u32 passCullMask)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	assert(GetVisAreaUpr()->IsOutdoorAreasVisible());
	...
	if (passCullMask & kPassCullMainMask)
	{
		for (i32 s = 0; s < DinrusX3dEngBase::GetTerrain()->m_SSurfaceType.Count(); s++)
		{
			SSurfaceType* pSurf = &DinrusX3dEngBase::GetTerrain()->m_SSurfaceType[s];
...
float CTerrainNode::GetSurfaceTypeAmount(Vec3 vPos, i32 nSurfType)
{
...
bool CTerrainNode::CheckUpdateProcObjects(const SRenderingPassInfo& passInfo)
{
	...
	for (i32 nLayer = 0; nLayer < m_lstSurfaceTypeInfo.Count(); nLayer++)
	{
		for (i32 g = 0; g < m_lstSurfaceTypeInfo[nLayer].pSurfaceType->lstnVegetationGroups.Count(); g++)
			if (m_lstSurfaceTypeInfo[nLayer].pSurfaceType->lstnVegetationGroups[g] >= 0)
			{
				i32 nGroupId = m_lstSurfaceTypeInfo[nLayer].pSurfaceType->lstnVegetationGroups[g];
				assert(nGroupId >= 0);
				if (nGroupId < 0)
					continue;
				StatInstGroup* pGroup = std::find_if(GetObjUpr()->m_lstStaticTypes.begin(), GetObjUpr()->m_lstStaticTypes.end(), [nGroupId](StatInstGroup& a) { return a.nID == nGroupId; });
				if (pGroup == GetObjUpr()->m_lstStaticTypes.end() || !pGroup->GetStatObj() || pGroup->fSize <= 0)
					continue;
				...
				for (float fX = fMinX; fX < fMaxX; fX += pGroup->fDensity)
				{
					for (float fY = fMinY; fY < fMaxY; fY += pGroup->fDensity)
					{
						...
						i32 nGroupIndex = static_cast<i32>(std::distance(GetObjUpr()->m_lstStaticTypes.begin(), pGroup));
						pEnt->SetStatObjGroupIndex(nGroupIndex);

						u32k nRnd = rndGen.GenerateUint32();
						const bool bRandomRotation = pGroup->bRandomRotation;
						i32k nRotationRange = pGroup->nRotationRangeToTerrainNormal;
						...
						else if (nRotationRange > 0)
						{
							...
							{
								const float rndDegree = (float)-nRotationRange * 0.5f + (float)(nRnd % (u32)nRotationRange);
...
CVegetation* CProcObjSector::AllocateProcObject()
{
	...
	// find pool id
	i32 nLastPoolId = m_nProcVegetNum / GetCVars()->e_ProcVegetationMaxObjectsInChunk;
	if (nLastPoolId >= m_ProcVegetChunks.Count())
	{
		...
		// init objects
		for (i32 o = 0; o < GetCVars()->e_ProcVegetationMaxObjectsInChunk; o++)
			pChunk->m_pInstances[o].Init();
	}

	// find empty slot id and return pointer to it
	i32 nNextSlotInPool = m_nProcVegetNum - nLastPoolId * GetCVars()->e_ProcVegetationMaxObjectsInChunk;
...
{
	for (i32 i = 0; i < m_ProcVegetChunks.Count(); i++)
	{
		SProcObjChunk* pChunk = m_ProcVegetChunks[i];
		for (i32 o = 0; o < pChunk->nAllocatedItems; o++)
...
void CProcObjSector::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void SProcObjChunk::GetMemoryUsage(class IDrxSizer* pSizer) const
{
	pSizer->AddObject(this, sizeof(*this));
...
/*
   i32 CTerrainNode__Cmp_SSurfaceType_UnitsNum(ukk v1, ukk v2)
   {
 ...
 */
/*
   i32 CTerrainNode__Cmp_TerrainSurfaceLayerAmount_UnitsNum(ukk v1, ukk v2)
   {
...
void CTerrainNode::UpdateDetailLayersInfo(bool bRecursive)
{
	...
	if (m_pChilds)
	{
		...
		for (i32 nChild = 0; nChild < 4; nChild++)
		{
			...
			for (i32 i = 0; i < m_pChilds[nChild].m_lstSurfaceTypeInfo.Count(); i++)
			{
		...
		for (i32 i = 0; i < lstChildsLayers.Count(); i++)
		{
			SSurfaceTypeInfo si;
	...
	else
	{
		...
		i32 arrSurfaceTypesInSector[SRangeInfo::e_max_surface_types];
		...
		for (float X = m_nOriginX; X <= m_nOriginX + CTerrain::GetSectorSize(); X += CTerrain::GetHeightMapUnitSize())
		{
			for (float Y = m_nOriginY; Y <= m_nOriginY + CTerrain::GetSectorSize(); Y += CTerrain::GetHeightMapUnitSize())
			{
				...
				for (i32 s = 0; s < SSurfaceTypeItem::kMaxSurfaceTypesNum; s++)
				{
					if (st.we[s])
					{
						DRX_ASSERT(st.ty[s] < SRangeInfo::e_max_surface_types);
...
void CTerrainNode::UnloadNodeTexture(bool bRecursive)
{
	...
	for (i32 i = 0; m_pChilds && bRecursive && i < 4; i++)
...
void CTerrainNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
	{
...
void CTerrainNode::GetResourceMemoryUsage(IDrxSizer* pSizer, const AABB& cstAABB)
{
...
/*bool CTerrainNode::CheckUpdateLightMap()
   ...
   if(nLM_Finished_Size)
   {
    ...
    m_nNodeTexSet.nTex1 = GetRenderer()->DownLoadToVideoMemory((u8*)m_LightMap.GetData(), nLM_Finished_Size, nLM_Finished_Size,
...
bool CTerrainNode::AssignTextureFileOffset(i16*& pIndices, i16& nElementsLeft)
{
...
i32 SSurfaceTypeInfo::GetIndexCount()
{
	i32 nCount = 0;
	for (i32 i = 0; i < 3; i++)
		if (arrpRM[i])
...
i32 CTerrainNode::GetSectorSizeInHeightmapUnits() const
{
	i32 nSectorSize = CTerrain::GetSectorSize() << m_nTreeLevel;
	return i32(nSectorSize * CTerrain::GetHeightMapUnitSizeInverted());
}
...
void CTerrainNode::SetTraversalFrameId(u32 onePassTraversalFrameId, i32 shadowFrustumLod)
{
