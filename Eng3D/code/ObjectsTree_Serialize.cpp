...
struct SBrushChunk : public SRenderNodeChunk
{
	...
	Matrix34f m_Matrix;
	i16     m_collisionClassIdx;
	u16    m_flags;
	i32     m_nMaterialId;
	i32     m_nMaterialLayers;
...
struct SWaterVolumeChunk : public SRenderNodeChunk
{
	...
	f32    m_volumeDepth;
	f32    m_streamSpeed;
	u32 m_numVerticesPhysAreaContour;
...
#include <drx3D/Eng3D/ObjectsTree_Serialize_info.h>
...
inline void LoadCommonData(SRenderNodeChunk* pChunk, IRenderNode* pObj, const SLayerVisibility* pLayerVisibility)
{
	...
	{
		u16k newLayerId = pLayerVisibility->pLayerIdTranslation[pChunk->m_nLayerId];
...
//////////////////////////////////////////////////////////////////////////
inline static u8 CheckLayerVisibility(u16k layerId, const SLayerVisibility* pLayerVisibility)
{
	return pLayerVisibility->pLayerVisibilityMask[layerId >> 3] & (1 << (layerId & 7));
}

i32 COctreeNode::SaveObjects_CompareRenderNodes(ukk v1, ukk v2)
{
	IRenderNode* p[2] = { *(IRenderNode**)v1, *(IRenderNode**)v2 };

	EERType t0 = p[0]->GetRenderNodeType();
	i32 f0 = p[0]->m_dwRndFlags;

	EERType t1 = p[1]->GetRenderNodeType();
	i32 f1 = p[1]->m_dwRndFlags;
...
//////////////////////////////////////////////////////////////////////////
i32 COctreeNode::SaveObjects(CMemoryBlock* pMemBlock, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, std::vector<IStatInstGroup*>* pStatInstGroupTable, EEndian eEndian, const SHotUpdateInfo* pExportInfo)
{
	...
	for (i32 i = 0; i < arrSortedObjects.Count(); i++)
	{
...
//////////////////////////////////////////////////////////////////////////
i32 COctreeNode::LoadObjects(byte* pPtr, byte* pEndPtr, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, EEndian eEndian, i32 nChunkVersion, const SLayerVisibility* pLayerVisibility, ELoadObjectsMode eLoadMode)
{
	while (pPtr < pEndPtr)
	{
		IRenderNode* pRN = 0;
...
i32 COctreeNode::GetSingleObjectFileDataSize(IRenderNode* pObj, const SHotUpdateInfo* pExportInfo)
{
	IRenderNode* pRenderNode = pObj;
	EERType eType = pRenderNode->GetRenderNodeType();
	...
	else if (eType == eERType_Road)
	{
		...
		nBlockSize += ((CRoadRenderNode*)pRenderNode)->m_dynamicData.indices.size() * sizeof(u32);
...
void COctreeNode::SaveSingleObject(byte*& pPtr, i32& nDatanSize, IRenderNode* pEnt, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, std::vector<IStatInstGroup*>* pStatInstGroupTable, EEndian eEndian, const SHotUpdateInfo* pExportInfo)
{
	...
	if (eType == eERType_Brush && !(pEnt->GetRndFlags() & ERF_PROCEDURAL))
	{
		...
		chunk.m_nMaterialId = CObjUpr::GetItemId(pMatTable, pObj->GetMaterial());

		COPY_MEMBER_SAVE(&chunk, pObj, m_collisionClassIdx);

		{
			// find StatObj Id
			i32 nItemId = CObjUpr::GetItemId<IStatObj>(pStatObjTable, pObj->GetEntityStatObj());
			if (nItemId < 0)
	...
	else if (eType == eERType_Vegetation && !(pEnt->GetRndFlags() & ERF_PROCEDURAL))
	{
...

		{
			...
			i32 nItemId = CObjUpr::GetItemId<IStatInstGroup>(pStatInstGroupTable, &group);
			if (nItemId < 0)
	...
	else if (eType == eERType_MergedMesh)
	{
		...
		if (NULL == pExportInfo)
		{
			...
			for (size_t i = 0; i < pObj->NumGroups(); ++i)
			{
				...
				i32 grpid = pObj->Group(i)->instGroupId;
	...
	else if (eType == eERType_Road)
	{
		...
		DRX_ASSERT_MESSAGE(sizeof(vtx_idx) == sizeof(u32), "Road exporting can only occur on a platform with 32-bit vertex indices");
	...
	else if (eERType_Decal == eType && !(pEnt->GetRndFlags() & ERF_PROCEDURAL))
	{
		...
		chunk.m_depth = (u8)SATURATEB(255.f - pDecalProperties->m_depth * 255);
	...
	else if (eERType_WaterVolume == eType)
	{
		...
		if (pSerData)
		{
			...
			chunk.m_materialID = CObjUpr::GetItemId(pMatTable, pSerData->m_pMaterial);
	...
	else if (eERType_DistanceCloud == eType)
	{
		...
		chunk.m_materialID = CObjUpr::GetItemId(pMatTable, pObj->GetMaterial());
	...
	else if (eERType_WaterWave == eType)
	{
		...
		chunk.m_nID = (i32)pSerData->m_nID;
		chunk.m_nMaterialID = CObjUpr::GetItemId(pMatTable, pSerData->m_pMaterial);
...
void COctreeNode::LoadSingleObject(byte*& pPtr, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, EEndian eEndian, i32 nChunkVersion, const SLayerVisibility* pLayerVisibility, ELoadObjectsMode eLoadMode, IRenderNode*& pRN)
{
	...
	if (eType == eERType_Brush)
	{
		...
		pObj->SetStatObj(CObjUpr::GetItemPtr(pStatObjTable, (i32)pChunk->m_nObjectTypeIndex));

		// set material
		pObj->SetMaterial(CObjUpr::GetItemPtr(pMatTable, pChunk->m_nMaterialId));
	...
	else if (eType == eERType_Vegetation)
	{
		...
		if (CheckSkipLoadObject(eType, pChunk->m_dwRndFlags, eLoadMode) || !CheckRenderFlagsMinSpec(GetObjUpr()->m_lstStaticTypes[pChunk->m_nObjectTypeIndex].m_dwRndFlags))
			return;

		StatInstGroup* pGroup = &GetObjUpr()->m_lstStaticTypes[pChunk->m_nObjectTypeIndex];
	...
	else if (eType == eERType_MergedMesh)
	{
		...
		CMergedMeshRenderNode* pObj = m_pMergedMeshesUpr->GetNode((pChunk->m_Extents.max + pChunk->m_Extents.min) * 0.5f);
	...
	else if (eType == eERType_Road)
	{
		...
		u32* pIndices = StepData<u32>(pPtr, pChunk->m_roadData.numIndices, eEndian);
		...
		if (sizeof(vtx_idx) != sizeof(u32))
		{
			// Strided copy, need to cast u32 from u16 since we exported from PC (u32 vertices) and load on console / mobile (u16)
			for (u32 i = 0; i < pChunk->m_roadData.numIndices; i++)
			{
		...
		if (NULL != pLayerVisibility)
		{
			DRX_ASSERT(pChunk->m_nLayerId != 0);
			if (!CheckLayerVisibility(pObj->GetLayerId(), pLayerVisibility))
			{
				pObj->SetRndFlags(ERF_HIDDEN, true);
			}
	...
	else if (eERType_Decal == eType)
	{
		...
		u8 depth = std::min<size_t>(pChunk->m_depth, 254);
		properties.m_depth = 1.f - 1.f / 255.f * depth;

		IMaterial* pMaterial(CObjUpr::GetItemPtr(pMatTable, pChunk->m_nMaterialId));
		assert(pMaterial);
		...
		{
			tukk pMatName("_can't_resolve_material_name_");
			i32 matID(pChunk->m_nMaterialId);
			if (matID >= 0 && matID < (i32)pMatTable->size())
		...
		{
			Get3DEngine()->RegisterEntity(pObj);
			GetObjUpr()->m_decalsToPrecreate.push_back(pObj);
		}
	}
	else if (eERType_WaterVolume == eType)
	{
		...
		i32k volumeTypeAndMiscBitShift = 24;

		if (CheckSkipLoadObject(eType, pChunk->m_dwRndFlags, eLoadMode) || !CheckRenderFlagsMinSpec(pChunk->m_dwRndFlags) || Get3DEngine()->IsLayerSkipped(pChunk->m_nLayerId))
		{
			i32 auxCntSrc = pChunk->m_volumeTypeAndMiscBits >> volumeTypeAndMiscBitShift;
			const float *pAuxDataSrc = StepData<float>(pPtr, auxCntSrc, eEndian);

			for (u32 j(0); j < pChunk->m_numVertices; ++j)
			{
				SWaterVolumeVertex* pVertex(StepData<SWaterVolumeVertex>(pPtr, eEndian));
			}

			for (u32 j(0); j < pChunk->m_numVerticesPhysAreaContour; ++j)
			{
				SWaterVolumeVertex* pVertex(StepData<SWaterVolumeVertex>(pPtr, eEndian));
			}
		...
		i32 auxCntSrc = pChunk->m_volumeTypeAndMiscBits >> volumeTypeAndMiscBitShift, auxCntDst;
		...
		i32k volumeType = pChunk->m_volumeTypeAndMiscBits & 0xFFFF;
		...
		// set material
		IMaterial* pMaterial(CObjUpr::GetItemPtr(pMatTable, pChunk->m_materialID));
	...
	else if (eERType_WaterWave == eType)
	{
		...
		if (CheckSkipLoadObject(eType, pChunk->m_dwRndFlags, eLoadMode) || !CheckRenderFlagsMinSpec(pChunk->m_dwRndFlags) || Get3DEngine()->IsLayerSkipped(pChunk->m_nLayerId))
		{
			for (u32 j(0); j < pChunk->m_nVertexCount; ++j)
			{
