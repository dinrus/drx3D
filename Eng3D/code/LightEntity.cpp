...
void CLightEntity::InitEntityShadowMapInfoStructure(i32 dynamicLods, i32 cachedLods)
{
...
void CLightEntity::SetLayerId(u16 nLayerId)
{
...
tukk CLightEntity::GetName(void) const
{
...
bool CLightEntity::IsLightAreasVisible()
{
	...
	// test neighbors
	IVisArea* Areas[64];
	i32 nCount = pArea->GetVisAreaConnections(Areas, 64);
...
void CLightEntity::SetMatrix(const Matrix34& mat)
{
	if (m_Matrix != mat)
	{
		...
		if (m_pShadowMapInfo != nullptr)
		{
			...
			for (i32 i = 0; i < MAX_GSM_LODS_NUM && m_pShadowMapInfo->pGSM[i]; i++)
...
void CLightEntity::UpdateGSMLightSourceShadowFrustum(const SRenderingPassInfo& passInfo)
{
	...
	i32k nMaxLodCount = min(GetCVars()->e_GsmLodsNum, MAX_GSM_LODS_NUM - 1);
	...
	i32 nDynamicLodCount = nMaxLodCount;
	i32 nCachedLodCount = 0;

	// check for shadow cache
	if (m_light.m_Flags & DLF_SUN)
	{
		i32 nFirstCachedLod = Get3DEngine()->m_nGsmCache - 1;
	...
	for (i32 nLod = nNextLod; nLod < MAX_GSM_LODS_NUM; nLod++)
	{
...
i32 CLightEntity::UpdateGSMLightSourceDynamicShadowFrustum(i32 nDynamicLodCount, i32 nDistanceLodCount, float& fDistanceFromViewNextDynamicLod, float& fGSMBoxSizeNextDynamicLod, bool bFadeLastCascade, const SRenderingPassInfo& passInfo)
{
	assert(m_pTerrain);
...
i32 CLightEntity::UpdateGSMLightSourceCachedShadowFrustum(i32 nFirstLod, i32 nLodCount, bool isHeightMapAOEnabled, float& fDistFromViewDynamicLod, float fRadiusDynamicLod, const SRenderingPassInfo& passInfo)
{
	ShadowFrustumMGPUCache* pFrustumCache = GetRenderer()->GetShadowFrustumMGPUCache();
	assert(pFrustumCache);

	i32k firstCachedFrustumIndex = nFirstLod + nLodCount;
	...
	else
	{
		...
		i32 nActiveGPUCount = GetRenderer()->GetActiveGPUCount();
...
i32 CLightEntity::UpdateGSMLightSourceNearestShadowFrustum(i32 nFrustumIndex, const SRenderingPassInfo& passInfo)
{
	DRX_ASSERT(nFrustumIndex >= 0 && nFrustumIndex < MAX_GSM_LODS_NUM);
	static ICVar* pDrawNearShadowsCVar = GetConsole()->GetCVar("r_DrawNearShadows");

	if (pDrawNearShadowsCVar && pDrawNearShadowsCVar->GetIVal() > 0 && nFrustumIndex < DRX_ARRAY_COUNT(m_pShadowMapInfo->pGSM))
	{
		if (m_light.m_Flags & DLF_SUN)
		{
...
bool CLightEntity::ProcessFrustum(i32 nLod, float fGSMBoxSize, float fDistanceFromView, PodArray<SPlaneObject>& lstCastersHull, const SRenderingPassInfo& passInfo)
{
	...
	if (bDoGSM)// && (m_light.m_Flags & DLF_SUN || m_light.m_Flags & DLF_PROJECT))
	{
		...
		u32k renderNodeFlags = pFr->m_eFrustumType == ShadowMapFrustum::e_GsmDynamicDistance ? ERF_DYNAMIC_DISTANCESHADOWS : 0xFFFFFFFF;
...
{
	float fValSnapped = fVal - fSnap * i32(fVal / fSnap);
	return fValSnapped;
}

void CLightEntity::InitShadowFrustum_SUN_Conserv(ShadowMapFrustum* pFr, i32 dwAllowedTypes, float fGSMBoxSize, float fDistance, i32 nLod, const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
...
void CLightEntity::CalculateShadowBias(ShadowMapFrustum* pFr, i32 nLod, float fGSMBoxSize) const
{
...
void GetCubemapFrustum(ShadowMapFrustum* pFr, i32 nS, CCamera& shadowFrust)
{
	...
	i32 nShadowTexSize = pFr->nTexSize;
...
void CLightEntity::CheckValidFrustums_OMNI(ShadowMapFrustum* pFr, const SRenderingPassInfo& passInfo)
{
	...
	for (i32 nS = 0; nS < OMNI_SIDES_NUM; nS++)
	{
...
bool CLightEntity::CheckFrustumsIntersect(CLightEntity* lightEnt)
{
	...
	for (i32 nS1 = 0; nS1 < nFaces1; nS1++)
	{
		for (i32 nS2 = 0; nS2 < nFaces2; nS2++)
		{
...
void CLightEntity::InitShadowFrustum_PROJECTOR(ShadowMapFrustum* pFr, i32 dwAllowedTypes, const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;

	i32k nFrameId = passInfo.GetMainFrameID();
	...
	// set texture size
	u32 nTexSize = GetCVars()->e_ShadowsMaxTexRes;
	...
	pFr->nShadowPoolUpdateRate = (u8) shadowUpdateRate;
	...
	if (m_light.m_Flags & DLF_DEFERRED_LIGHT)
	{
		...
		nTexSize = i32((fSMZ1 - fSMZ0) * ((float)GetCVars()->e_ShadowsMaxTexRes * fCoverageScaleFactor /*/(7*cCam.GetFarPlane())*/));

		u32 nPoolSize = GetCVars()->e_ShadowsPoolSize;
		u32 nMaxTexRes = GetCVars()->e_ShadowsMaxTexRes;

		u32 nMinRes, nMaxRes, nPhysicalMaxRes;
		...
		// force power of two
		nTexSize = 1 << IntegerLog2((u32)nTexSize);
	}
	...
	if ((m_light.m_Flags & DLF_PROJECT) && GetCVars()->e_ShadowsDebug == 4)
	{
		...
		auto coneBaseRadius = coneHeight * tan(DrxTransform::CAngle::FromDegrees(pFr->fFOV / 2.0f).ToRadians());
...
void CLightEntity::InitShadowFrustum_OMNI(ShadowMapFrustum* pFr, i32 dwAllowedTypes, const SRenderingPassInfo& passInfo)
{
...
i32 CLightEntity::MakeShadowCastersHull(PodArray<SPlaneObject>& lstCastersHull, const SRenderingPassInfo& passInfo)
{
	...
	for (i32 v = 0; v < 4; v++)
		arrFrustVerts[v] = vCamPos + (arrFrustVerts[v] - vCamPos).normalized() * GetObjUpr()->m_fGSMMaxDistance * 1.3f; //0.2f;
...
void CLightEntity::SetupShadowFrustumCamera_SUN(ShadowMapFrustum* pFr, i32 dwAllowedTypes, i32 nRenderNodeFlags, PodArray<SPlaneObject>& lstCastersHull, i32 nLod, const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
...
void CLightEntity::SetupShadowFrustumCamera_PROJECTOR(ShadowMapFrustum* pFr, i32 dwAllowedTypes, const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
...
void CLightEntity::SetupShadowFrustumCamera_OMNI(ShadowMapFrustum* pFr, i32 dwAllowedTypes, const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
...
ShadowMapFrustum* CLightEntity::GetShadowFrustum(i32 nId)
{
...
void CLightEntity::OnCasterDeleted(IShadowCaster* pCaster)
{
	...
	for (i32 nGsmId = 0; nGsmId < MAX_GSM_LODS_NUM; nGsmId++)
	{
...
void CLightEntity::GetMemoryUsage(IDrxSizer* pSizer) const
{
	...
	if (m_pShadowMapInfo)
	{
		...
		for (i32 n = 0; n < MAX_GSM_LODS_NUM; n++)
		{
...
void CLightEntity::Render(const SRendParams& rParams, const SRenderingPassInfo& passInfo)
{
	...
	if (m_layerId != u16(~0) && m_dwRndFlags & ERF_HIDDEN)
		return;
	...
	i32 nRenderNodeMinSpec = (m_dwRndFlags & ERF_SPEC_BITS_MASK) >> ERF_SPEC_BITS_SHIFT;
	...
	i32k nEngineFrameID = passInfo.GetFrameID();

	i32 nMaxReqursion = (m_light.m_Flags & DLF_THIS_AREA_ONLY) ? 2 : 3;
	if (!m_pObjUpr || !m_pVisAreaUpr || !m_pVisAreaUpr->IsEntityVisAreaVisible(this, nMaxReqursion, &m_light, passInfo))
	{
		if (m_light.m_Flags & DLF_SUN && m_pVisAreaUpr && m_pVisAreaUpr->m_bSunIsNeeded)
		{
			// sun may be used in indoor even if outdoor is not visible
		}
		else if (!GetEntityVisArea() && GetEntityTerrainNode() && !(m_light.m_Flags & DLF_THIS_AREA_ONLY) && m_pVisAreaUpr && m_pVisAreaUpr->m_bSunIsNeeded)
		{
	...
	if (CVisArea* pArea = (CVisArea*)GetEntityVisArea())
	{
		...
		if (!IsLightAreasVisible() && pCameraVisArea != pArea)
		{
			if (m_light.m_Flags & DLF_THIS_AREA_ONLY)
			{
				if (pArea)
				{
					i32 nRndFrameId = pArea->GetVisFrameId();
	...
	m_light.m_nStencilRef[0] = CClipVolumeUpr::AffectsEverythingStencilRef;
	m_light.m_nStencilRef[1] = CClipVolumeUpr::InactiveVolumeStencilRef;
	...
	if (m_light.m_nStencilRef[0] == CClipVolumeUpr::InactiveVolumeStencilRef && m_light.m_nStencilRef[1] == CClipVolumeUpr::InactiveVolumeStencilRef)
		return;
	...
	m_light.m_Id = i16(Get3DEngine()->GetDynamicLightSources()->Count() + passInfo.GetIRenderView()->GetLightsCount(eDLT_DeferredLight));
	...
	if (!bAdded)
	{
		...
		if (GetCVars()->e_DynamicLights && m_fWSMaxViewDist)
		{
			if (GetCVars()->e_DynamicLights == 2)
			{
				...
				IRenderAuxText::DrawLabelF(pL->m_Origin, 1.3f, "id=%d, rad=%.1f, vdr=%d", pL->m_Id, pL->m_fRadius, (i32)m_ucViewDistRatio);
			}
...
void CLightEntity::SetViewDistRatio(i32 nViewDistRatio)
{
