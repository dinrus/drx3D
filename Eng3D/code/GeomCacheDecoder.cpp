...
// ------------------------------------------------------------------------
//  Имя файла:   GeomCacheDecoder.cpp
//  Created:     23/8/2012 by Axel Gneiting
//  Описание:    Расшифровывает данные кэша геометрии.
// -------------------------------------------------------------------------
//
////////////////////////////////////////////////////////////////////////////
...
#include <drx3D/Eng3D/StdAfx.h>
	...
	#include <drx3D/Eng3D/GeomCachePredictors.h>
...
GeomCacheFile::Color FixedPointColorLerp(i32 a, i32 b, i32k lerpFactor)
{
...
__m128 _mm_dp_ps_emu(const __m128& a, const __m128& b)
{
                                  ...
                                  const GeomCacheFile::QTangent* __restrict pCeilQTangents, strided_pointer<SPipTangents> pTangents)
{
	...
	for (u32 i = 0, j = 0; i < numSIMDIterations; ++i, j += 2)
	{
	...
	#endif
	for (u32 i = scalarStart; i < numVertices; ++i)
	{
...
void DecodeAndInterpolateColorAndTexcoords(SGeomCacheRenderMeshUpdateContext& updateContext, const uint index,
                                           const SGeomCacheStaticMeshData& staticMeshData, u32 fpLerpFactor, const float lerpFactor,
                                           const GeomCacheFile::Color* __restrict pFloorReds, const GeomCacheFile::Color* __restrict pCeilReds,
                                           const GeomCacheFile::Color* __restrict pFloorGreens, const GeomCacheFile::Color* __restrict pCeilGreens,
                                           const GeomCacheFile::Color* __restrict pFloorBlues, const GeomCacheFile::Color* __restrict pCeilBlues,
                                           const GeomCacheFile::Color* __restrict pFloorAlphas, const GeomCacheFile::Color* __restrict pCeilAlphas,
                                           const GeomCacheFile::Texcoords* __restrict pFloorTexcoords, const GeomCacheFile::Texcoords* __restrict pCeilTexcoords)
{
	if (!bConstantColors && !bAnimatedColors)
	{
	...
	{
		updateContext.m_pTexcoords[index] = Vec2::CreateLerp(DecodeTexcoord(pFloorTexcoords[index]), DecodeTexcoord(pCeilTexcoords[index]), lerpFactor);
	}
}

template<uint Permutation>
void DecodeMeshVerticesBranchless(SGeomCacheRenderMeshUpdateContext& updateContext,
                                  const SGeomCacheStaticMeshData& staticMeshData, tukk pFloorFrameDataPtr,
                                  tukk pCeilFrameDataPtr, const float lerpFactor)
{
	u32k numVertices = staticMeshData.m_numVertices;
	i32k fpLerpFactor = i32(lerpFactor * 65535.0f);
	...
	const uint numPackedUInt16PerIteration = (numVerticesPerIteration * sizeof(Vec3_tpl<u16> )) / 16;
	...
	for (u32 i = 0; i < numSIMDIterations; ++i)
	{
		const uint floatOffset = i * numFloatsPerIteration;

		if (bMotionBlur && !bConstantPositions)
		{
		...
		{
			for (uint j = 0; j < numFloatsPerIteration; ++j)
			{
		...
		else if (!bConstantPositions)
		{
			...
			DRX_ALIGN(16) Vec3 positionScratch[numVerticesPerIteration];
	...
	if (bConstantPositions && bConstantTexcoords)
	{
...
u32 GetMeshDataSize(const SGeomCacheStaticMeshData& staticMeshData)
{
	u32k numVertices = staticMeshData.m_numVertices;
...
template<> TDecodeVerticesBranchlessPtr PermutationInit<0>::m_pFunction1 = &DecodeMeshVerticesBranchless<0>;
...
template<> TDecodeVerticesBranchlessPtr PermutationInit<0>::m_pFunction2 = &DecodeMeshVerticesBranchless<(kNumPermutations / 2)>;
...
void DecodeBFrame(const CGeomCache* pGeomCache, tuk pData, tuk pPrevFramesData[2], tuk pFloorIndexFrameData, tuk pCeilIndexFrameData)
{
	// Skip header
...
void FillMeshDataFromDecodedFrame(const bool bMotionBlur, SGeomCacheRenderMeshUpdateContext& updateContext,
                                  const SGeomCacheStaticMeshData& staticMeshData, tukk pFloorFrameMeshData, tukk pCeilFrameMeshData, float lerpFactor)
{
...
Vec2 DecodeTexcoord(const GeomCacheFile::Texcoords& inTexcoords)
{
	...
	return Vec2((float)inTexcoords.x * convertFromInt16Factor * GeomCacheFile::kTexcoordRange,
	            (float)inTexcoords.y * convertFromInt16Factor * GeomCacheFile::kTexcoordRange);
}
...
{
	i16 reflection = alias_cast<u32>(inQTangent.w) & 0x80000000 ? -1 : +1;
...
u32 GetDecompressBufferSize(tukk const pStartBlock, u32k numFrames)
{
	FUNCTION_PROFILER_3DENGINE;

	u32 totalUncompressedSize = 0;
	tukk pCurrentBlock = pStartBlock;

	u32k headersSize = ((sizeof(SGeomCacheFrameHeader) * numFrames) + 15) & ~15;
	...
	u32 totalSize = headersSize + totalUncompressedSize;
	if (totalSize % 16 != 0)
	{
		DrxFatalError("GetDecompressBufferSize mod 16 != 0");
	}
...
bool DecompressBlock(const GeomCacheFile::EBlockCompressionFormat compressionFormat, tuk const pDest, tukk const pSource)
{
	...
	tukk const pBlockData = reinterpret_cast<tukk const>(pSource + sizeof(GeomCacheFile::SCompressedBlockHeader));
	...
	if (compressionFormat == GeomCacheFile::eBlockCompressionFormat_None)
	{
		...
		memcpy(const_cast<tuk>(pDest), pSource + sizeof(GeomCacheFile::SCompressedBlockHeader), pBlockHeader->m_uncompressedSize);
	}
	else if (compressionFormat == GeomCacheFile::eBlockCompressionFormat_Deflate)
	{
		...
		pInflateStream->SetOutputBuffer(const_cast<tuk>(pDest), pBlockHeader->m_uncompressedSize);
	...
	else if (compressionFormat == GeomCacheFile::eBlockCompressionFormat_LZ4HC)
	{
		...
		return pDecompressor->DecompressData(pBlockData, const_cast<tuk>(pDest), pBlockHeader->m_uncompressedSize);
	}
	else
	{
		return false;
                      ...
                      tukk const pSource, const uint blockOffset, const uint numBlocks, const uint numHandleFrames)
{
	...
	tukk pCurrentSource = pSource;
	u32k headersSize = ((sizeof(SGeomCacheFrameHeader) * numHandleFrames) + 15) & ~15;
	tuk pCurrentDest = pDest + headersSize;
	...
	for (uint i = blockOffset; i < blockOffset + numBlocks; ++i)
	{
		...
		pHeader->m_offset = static_cast<u32>(pCurrentDest - pDest);
