...
void CStatObj::Refresh(i32 nFlags)
{
	if (nFlags & FRO_GEOMETRY)
	{
		...
		if (m_bCheckGarbage)
		{
			GetObjUpr()->UnregisterForGarbage(this);
		}

		i32k oldModificationId = m_nModificationId;
...
void CStatObj::LoadLowLODs(bool bUseStreaming, u64 nLoadingFlags)
{
	...
	for (i32 nLodLevel = 0; nLodLevel < MAX_STATOBJ_LODS_NUM; nLodLevel++)
		loadedLods[nLodLevel] = 0;

	for (i32 nLodLevel = 1; nLodLevel < MAX_STATOBJ_LODS_NUM; nLodLevel++)
	{
...
bool CStatObj::LoadLowLODS_Prep(bool bUseStreaming, u64 nLoadingFlags)
{
	m_bLodsLoaded = true;
	...
	tukk sFileExt = PathUtil::GetExt(m_szFileName);
	...
	if (m_nLoadedLodsNum > 1 && GetFlags() & STATIC_OBJECT_COMPOUND)
	{
		for (i32 nLodLevel = 1; nLodLevel < MAX_STATOBJ_LODS_NUM; nLodLevel++)
		{
			...
			char sLodNum[8];
			drx_strcpy(sLodFileName, m_szFileName);
			tuk sPointSeparator = strchr(sLodFileName, '.');
			if (sPointSeparator)
				*sPointSeparator = '\0'; // Terminate at the dot
			drx_strcat(sLodFileName, "_lod");
			ltoa(nLodLevel, sLodNum, 10);
			drx_strcat(sLodFileName, sLodNum);
			drx_strcat(sLodFileName, ".");
			drx_strcat(sLodFileName, sFileExt);
...
CStatObj* CStatObj::LoadLowLODS_Load(i32 nLodLevel, bool bUseStreaming, u64 nLoadingFlags, ukk pData, i32 nDataLen)
{
	tukk sFileExt = PathUtil::GetExt(m_szFileName);
	...
	pLodStatObj = stl::find_in_map(m_pObjUpr->m_nameToObjectMap, CONST_TEMP_STRING(sLodFileName), NULL);
	...
	if (pLodStatObj)
	{
		...
		LoadedObjects::iterator it = m_pObjUpr->m_lstLoadedObjects.find(pLodStatObj);
		if (it != m_pObjUpr->m_lstLoadedObjects.end())
		{
			m_pObjUpr->m_lstLoadedObjects.erase(it);
			m_pObjUpr->m_nameToObjectMap.erase(CONST_TEMP_STRING(sLodFileName));
		}
	}
...
void CStatObj::LoadLowLODS_Finalize(i32 nLoadedLods, CStatObj* loadedLods[MAX_STATOBJ_LODS_NUM])
{
	//////////////////////////////////////////////////////////////////////////
	...
	if (nLoadedLods > 1)
	{
		...
		for (i32 i = 0; i < (i32)m_subObjects.size(); i++)
		{
			...
			//			i32 nLoadedTrisCount = ((CStatObj*)pSubObject->pStatObj)->m_nLoadedTrisCount;

			for (i32 nLodLevel = 1; nLodLevel < nLoadedLods; nLodLevel++)
			{
	...
	for (i32 nLodLevel = 0; nLodLevel < MAX_STATOBJ_LODS_NUM; nLodLevel++)
	{
		...
		{
			GetObjUpr()->CheckForGarbage(loadedLods[nLodLevel]);
		}
	}
}
...
void CStatObj::CleanUnusedLods()
{
	...
	if (m_nLoadedLodsNum > 1)
	{
		i32 nMinLod = GetMinUsableLod();
		nMinLod = clamp_tpl(nMinLod, 0, (i32)m_nLoadedLodsNum - 1);
		for (i32 i = 0; i < nMinLod; i++)
		{
...
static string FindCGFSourceFilename(tukk filename)
{
...
//////////////////////////////////////////////////////////////////////////
bool CStatObj::LoadStreamRenderMeshes(tukk filename, ukk pData, i32k nDataSize, bool bLod)
{
	...
	{
	public:
		virtual void Warning(tukk format) {}
		virtual void Error(tukk format)   {}
	...
	i32 nSubObjCount = (i32)m_subObjects.size();

	bool bBreakNodeLoop = false;

	for (i32 i = 0; i < pCGF->GetNodeCount() && !bBreakNodeLoop; i++)
	{
		...
		CStatObj* pStatObj = 0;
		for (i32 s = 0; s < nSubObjCount && !pStatObj; s++)
		{
			CStatObj* pSubStatObj = (CStatObj*)m_subObjects[s].pStatObj;
			if (!pSubStatObj)
				continue;
			for (i32 nLod = 0; nLod < MAX_STATOBJ_LODS_NUM; nLod++)
			{
		...
		if (!pStatObj && m_nSubObjectMeshCount <= 1)
		{
			...
			for (i32 nLod = 0; nLod < MAX_STATOBJ_LODS_NUM && !pStatObj; nLod++)
			{
...
void CStatObj::CommitStreamRenderMeshes()
{
	...
	{
		DrxAutoCriticalSection lock(m_streamingMeshLock);
	...
	if (m_pLODs)
	{
		for (i32 nLod = 0; nLod < MAX_STATOBJ_LODS_NUM; nLod++)
		{
			...
			{
				DrxAutoCriticalSection lock(pLodObj->m_streamingMeshLock);
...
bool CStatObj::IsDeformable()
{
	...
	if (m_isDeformable)
	{
		...
		for (i32 i = 0, n = GetSubObjectCount(); i < n; ++i)
		{
...
//////////////////////////////////////////////////////////////////////////
bool CStatObj::LoadCGF(tukk filename, bool bLod, u64 nLoadingFlags, ukk pData, i32k nDataSize)
{
	FUNCTION_PROFILER_3DENGINE;

	DRX_DEFINE_ASSET_SCOPE("CGF", filename);
	...
	m_bHasStreamOnlyCGF = gEnv->pDrxPak->IsFileExist(streamPath.c_str());
...
//////////////////////////////////////////////////////////////////////////
static _smart_ptr<IMaterial> LoadCGFMaterial(CMatMan* pMatMan, tukk szMaterialName, tukk szCgfFilename, u64 nLoadingFlags)
{
	...
	if (pMaterial->IsDefault())
	{
		...
		{
			DrxLog("Loaded material \"%s\" instead of missing \"%s\" (file \"%s\")", cgfBasedMaterialName.c_str(), szMaterialName, szCgfFilename);
		}
	}

#if defined(FEATURE_SVO_GI)
	if ((DinrusX3dEngBase::GetCVars()->e_svoTI_Active >= 0) && (gEnv->IsEditor() || DinrusX3dEngBase::GetCVars()->e_svoTI_Apply))
...
bool CStatObj::LoadCGF_Int(tukk filename, bool bLod, u64 nLoadingFlags, ukk pData, i32k nDataSize)
{
	CLoaderCGF cgfLoader(util::pool_allocate, util::pool_free, GetCVars()->e_StatObjTessellationMode != 2 || bLod);
	...
	{
		size_t fileSize = gEnv->pDrxPak->FGetSize(filename, true);
	...
	if (pCGF->GetNodeCount() > 1 || m_nSubObjectMeshCount > 0)
	{
		...
		for (i32 ii = 0; ii < pCGF->GetNodeCount(); ii++)
		{
		...
		for (i32 ii = 0; ii < pCGF->GetNodeCount(); ii++)
		{
			    ...
			    (subObject.nType == STATIC_SUB_OBJECT_HELPER_MESH))
			{
				if (pNode->pSharedMesh)
				{
					...
					for (i32 k = 0, num = (i32)meshToObject.size(); k < num; k++)
					{
			    ...
			    (strnicmp(pNode->name, CGF_NODE_NAME_LOD_PREFIX, lodNamePrefixLength) == 0))
			{
				...
				if (!pExportInfo->bMergeAllNodes && m_nSubObjectMeshCount > 0 && pNode->pParent)
				{
					...
					for (i32 i = 0, num = nodes.size(); i < num; i++)
					{
				...
				i32k nLodLevel = atoi(pNode->name + lodNamePrefixLength);
				if ((nLodLevel >= 1) && (nLodLevel < MAX_STATOBJ_LODS_NUM))
				{
					...
					{
						tukk existingGeoName = pStatObjParent->m_pLODs[nLodLevel]->GetGeoName();
		...
		i32 nNumCgfNodes = (i32)nodes.size();
		if (nNumCgfNodes > 0)
		{
			...
			for (i32 i = 0; i < nNumCgfNodes; i++)
			{
				if (pNodes[i]->type != CNodeCGF::NODE_MESH)
				{
					...
					if (i < nNumMeshes)
					{
						...
						for (i32 j = i + 1; j < nNumCgfNodes; j++)
						{
			...
			if (bHaveMeshNamedMain)
			{
				...
				for (i32 i = 0, n = m_subObjects.size(); i < n; i++)
				{
	...
	if (!bLod)
	{
		for (i32 i = 0, numNodes = pCGF->GetNodeCount(); i < numNodes; i++)
		{
			CNodeCGF* pNode = pCGF->GetNode(i);
			if (pNode->bPhysicsProxy)
			{
	...
	if (!bLod)
	{
		...
		if (pPi->nRetTets)
		{
			m_pLattice = GetPhysicalWorld()->GetGeomUpr()->CreateTetrLattice(pPi->pRetVtx, pPi->nRetVtx, pPi->pRetTets, pPi->nRetTets);
		}
	}
...
{
	i32k subObjectsCount = GetSubObjectCount();
	...
	for (i32 i = 0; i < subObjectsCount; ++i)
	{
		IStatObj::SSubObject* subObject = GetSubObject(i);
...
//////////////////////////////////////////////////////////////////////////
CStatObj* CStatObj::MakeStatObjFromCgfNode(CContentCGF* pCGF, CNodeCGF* pNode, bool bLod, i32 nLoadingFlags, AABB& commonBBox)
{
...
_smart_ptr<IRenderMesh> CStatObj::MakeRenderMesh(CMesh* pMesh, bool bDoRenderMesh)
{
	...
	//////////////////////////////////////////////////////////////////////////
	for (i32 i = 0; i < pMesh->GetSubSetCount(); i++)
	{
...
static inline CNodeCGF* CreateNodeCGF(CContentCGF* pCGF, CStatObj* pStatObj, tukk name, CNodeCGF* pParent, CMaterialCGF* pMaterial, const Matrix34& localTM = Matrix34(IDENTITY), tukk properties = 0)
{
	CNodeCGF* pNode = NULL;
	...
	if (!pNode)
	{
		DrxLog("SaveToCgf: failed to allocate CNodeCGF aborting");
		return 0;
	}

	drx_sprintf(pNode->name, "%s", name);
	...
	i32k subobjCount = pStatObj ? pStatObj->GetSubObjectCount() : 0;
	std::vector<CNodeCGF*> nodes;
	nodes.resize(subobjCount);
	for (i32 subidx = 0; subidx < subobjCount; ++subidx)
		nodes[subidx] = pNode;
	for (i32 subidx = 0; subidx < subobjCount; ++subidx)
	{
...
bool CStatObj::SaveToCGF(tukk sFilename, IChunkFile** pOutChunkFile, bool bHavePhiscalProxy)
{
	...
	drx_strcpy(pCGF->GetExportInfo()->rc_version_string, "From Sandbox");
	...
	if (m_pMaterial)
		drx_strcpy(pMaterialCGF->name, m_pMaterial->GetName());
	else
	...
	if (CreateNodeCGF(pCGF, this, GetGeoName() ? GetGeoName() : "Merged", NULL, pMaterialCGF))
	{
		...
		const bool bStoreIndicesAsU16 = (sizeof(vtx_idx) == sizeof(u16));
...
//////////////////////////////////////////////////////////////////////////
inline tuk trim_whitespaces(tuk str, tuk strEnd)
{
	tuk first = str;
		...
		if (line < lineEnd)
		{
			...
			else
			{
				...
				else if (0 == strcmp(line, "no_hit_refinement"))
				{
					...
					for (i32 i = m_arrPhysGeomInfo.GetGeomCount() - 1; i >= 0; i--)
...
void CStatObj::CheckCreateBillboardMaterial()
{
	...
	tukk  arrTextureSuffixes[2] = { "_billbalb.dds", "_billbnorm.dds" };
	i32 nBillboardTexturesFound = 0;
	assert(EFTT_DIFFUSE == 0 && EFTT_NORMALS == 1);
	for (i32 nSlot = EFTT_DIFFUSE; nSlot <= EFTT_NORMALS; nSlot++)
	{
		string szTextureName = m_szFileName;
		if (szTextureName.find(".cgf") != string::npos)
		{
			szTextureName.replace(".cgf", arrTextureSuffixes[nSlot]);
			if (gEnv->pDrxPak->IsFileExist(szTextureName.c_str()))
