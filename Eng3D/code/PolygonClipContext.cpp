...
// -------------------------------------------------------------------------
//  Имя файла:   PolygonClipContext.cpp
//  Created:
//  Описание:
// -------------------------------------------------------------------------
//  История:
...
i32 CPolygonClipContext::ClipEdge(const Vec3& v1, const Vec3& v2, const Plane& ClipPlane, Vec3& vRes1, Vec3& vRes2)
{
...
void CPolygonClipContext::ClipPolygon(PodArray<Vec3>& PolygonOut, const PodArray<Vec3>& pPolygon, const Plane& ClipPlane)
{
	...
	for (i32 i = 0; i < pPolygon.Count(); i++)
	{
		Vec3 vNewVert1(0, 0, 0), vNewVert2(0, 0, 0);
		if (i32 nNewVertNum = ClipEdge(pPolygon.GetAt(i), pPolygon.GetAt((i + 1) % pPolygon.Count()), ClipPlane, vNewVert1, vNewVert2))
		{
	...
	// check result
	for (i32 i = 0; i < PolygonOut.Count(); i++)
	{
...
void CPolygonClipContext::GetMemoryUsage(IDrxSizer* pSizer) const
{
