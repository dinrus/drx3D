...
static i32k nSERIALIZE_VERSION = 28;
static i32k nMIN_SERIALIZE_VERSION = 19;
...
{
	CTypeInfo const& info = TypeInfo(&val);
	i32 nAttrs = xml.getNumAttributes();
	for (i32 a = 0; a < nAttrs; a++)
	{
		cstr sKey, sVal;
...
void AddTravel(AABB& bb, Vec3 const& vVel, SForceParams const& force, float fTime, i32 nAxes)
{
...
void AddEmitDirs(AABB& bb, Vec3 const& vRefDir, SEmitParams const& emit, SForceParams const& force, float fTime, i32 nAxes)
{
...
void TravelBB(AABB& bb, SEmitParams const& emit, SForceParams const& force, float fTime, i32 nAxes)
{
...
void ResourceParticleParams::GetMaxTravelBounds(AABB& bbResult, const QuatTS& loc, const SPhysForces& forces, const FStaticBounds& opts) const
{
	...
	for (i32 iDragIndex = (afDrag[1] != afDrag[0] ? 1 : 0); iDragIndex >= 0; iDragIndex--)
	{
		force.fDrag = afDrag[iDragIndex];
		for (i32 iGravIndex = (afGrav[1] != afGrav[0] ? 1 : 0); iGravIndex >= 0; iGravIndex--)
		{
...
//////////////////////////////////////////////////////////////////////////
i32 ResourceParticleParams::LoadResources(tukk pEffectName)
{
	...
	if (!pMaterial && (!sTexture.empty() || !sMaterial.empty()))
	{
		if (!sMaterial.empty())
		{
			pMaterial = Get3DEngine()->GetMaterialUpr()->LoadMaterial(sMaterial.c_str());
			if (!pMaterial || pMaterial == Get3DEngine()->GetMaterialUpr()->GetDefaultMaterial())
			{
				DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "Particle effect material %s not found",
				           sMaterial.c_str());
		...
		{
			u32k textureLoadFlags = FT_DONT_STREAM;
			i32k nTexId = GetRenderer()->EF_LoadTexture(sTexture.c_str(), textureLoadFlags)->GetTextureID();			
			if (nTexId <= 0)
				DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "Particle effect texture %s not found", sTexture.c_str());
			else
				nLoaded++;

			pMaterial = gEnv->p3DEngine->GetMaterialUpr()->CreateMaterial(pEffectName);
			if (gEnv->pRenderer)
			{
		...
		else if (sGeometry.empty())
		{
			...
			if (!shader.m_pShader)
				DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "Particle effect material %s has invalid shader",
				           sMaterial.c_str());
			nLoaded++;
		}
		fTexAspect = 0.0f;
	...
	{
		DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "Particle effect has no material for decal, texture = %s",
		           sTexture.c_str());
	}
	...
	if (!pStatObj && !sGeometry.empty())
	{
		...
		if (!pStatObj)
			DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "Particle effect geometry not found: %s", sGeometry.c_str());
		else
			nLoaded++;
	}
...
struct CompatibilityParticleParams
	: ParticleParams, ZeroInit<CompatibilityParticleParams>, DinrusX3dEngBase
{
	i32    nVersion;
	...
	CompatibilityParticleParams(i32 nVer, cstr sSandboxVer, const ParticleParams& paramsDefault)
...
CParticleEffect::CParticleEffect(tukk sName)
	: m_parent(0),
	m_pParticleParams(0),
	m_strName(sName)
{
...
void CParticleEffect::SetEnabled(bool bEnabled)
{
	if (bEnabled != IsEnabled())
	{
		...
		CParticleUpr::Instance()->UpdateEmitters(this);
	}
}
...
void CParticleEffect::SetName(cstr sNewName)
{
	if (!m_parent)
	{
		...
		{
			CParticleUpr::Instance()->RenameEffect(this, sNewName);
			m_strName = sNewName;
		}
	}
	else
	{
		...
		for (i32 i = m_parent->m_children.size() - 1; i >= 0; i--)
		{
...
void CParticleEffect::InstantiateParams()
{
	if (!m_pParticleParams)
		m_pParticleParams = new ResourceParticleParams(CParticleUpr::Instance()->GetDefaultParams());
}

//////////////////////////////////////////////////////////////////////////
tukk CParticleEffect::GetBaseName() const
{
	tukk sName = m_strName.c_str();
	if (m_parent)
	{
...
void CParticleEffect::SetParent(IParticleEffect* pParent)
{
	...
	if (m_parent)
	{
		i32 i = m_parent->FindChild(this);
		if (i >= 0)
			m_parent->m_children.erase(i);
		CParticleUpr::Instance()->UpdateEmitters(m_parent);
	}
	if (pParent)
	{
		if (!m_parent)
			// Was previously top-level effect.
			CParticleUpr::Instance()->DeleteEffect(this);
...
//////////////////////////////////////////////////////////////////////////
void CParticleEffect::InsertChild(i32 slot, IParticleEffect* pEffect)
{
	if (slot < 0)
		slot = 0;
...
//////////////////////////////////////////////////////////////////////////
i32 CParticleEffect::FindChild(IParticleEffect* pEffect) const
{
	i32 i = 0;
	for (auto& child : m_children)
	{
...
CParticleEffect* CParticleEffect::FindChild(tukk szChildName) const
{
	for (auto& child : m_children)
	{
...
const CParticleEffect* CParticleEffect::FindActiveEffect(i32 nVersion) const
{
	...
	if (m_pParticleParams && m_pParticleParams->IsActive())
	{
		...
		i32 nEffectVersion = atoi(GetBaseName());
	...
	for (auto& child : m_children)
	{
		...
		{
			i32 nEffectVersion = atoi(pChild->GetBaseName());
...
bool CParticleEffect::LoadResources(bool bAll, cstr sSource) const
{
	...
	if (sSource && !ResourcesLoaded(true) && !CParticleUpr::Instance()->CanAccessFiles(GetFullName(), sSource))
		return false;
	...
	{
		DRX_DEFINE_ASSET_SCOPE("Particle Effect", GetFullName());

#ifdef DRX_PFX1_BAIL_UNSUPPORTED
		if (GetParent() == 0)
	...
	if (bAll)
		for (auto& child : m_children)
			nLoaded += (i32)child.LoadResources(true);
	return nLoaded > 0;
}
...
const ParticleParams& CParticleEffect::GetDefaultParams(ParticleParams::EInheritance eInheritance, i32 nVersion) const
{
	...
	return CParticleUpr::Instance()->GetDefaultParams(eInheritance, nVersion);
}

static i32 UpdateDefaultValues(const CTypeInfo& info, uk data, ukk def_data, ukk new_data)
{
	i32 nUpdated = 0;
	if (info.HasSubVars())
	{
...
const ParticleParams& CParticleEffect::GetParticleParams() const
{
	...
	return CParticleUpr::Instance()->GetDefaultParams();
}

//////////////////////////////////////////////////////////////////////////
IParticleEmitter* CParticleEffect::Spawn(const ParticleLoc& loc, const SpawnParams* pSpawnParams)
{
	return CParticleUpr::Instance()->CreateEmitter(loc, this, pSpawnParams);
}
...
void CParticleEffect::Serialize(XmlNodeRef node, bool bLoading, bool bAll)
{
	...
	if (bLoading)
	{
		...
		i32 nVersion = nSERIALIZE_VERSION;
		...
		if (bAll)
		{
			...
			if (childsNode)
			{
				for (i32 i = 0; i < childsNode->getChildCount(); i++)
				{
	...
	else
	{
		...
		if (m_strName == "System.Default")
			gEnv->pParticleUpr->SetDefaultEffect(this);
	}
}
...
{
	if (XmlNodeRef node = CParticleUpr::Instance()->ReadEffectNode(GetFullName()))
	{
...
void CParticleEffect::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
{
	i32 nPieces = 0;
	for (i32 i = pParent->GetSubObjectCount() - 1; i >= 0; i--)
