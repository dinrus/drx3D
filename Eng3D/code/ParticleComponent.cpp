...
void SComponentParams::Serialize(Serialization::IArchive& ar)
{
	...
	bool first = true;
	buffer[0] = 0;
	u32 bytesPerParticle = 0;
	for (auto type : EParticleDataType::values())
	{
		if (pComponent->UseParticleData(type))
		{
			if (first)
				drx_sprintf(buffer, "%s", type.name());
			else
				drx_sprintf(buffer, "%s, %s", buffer, type.name());
			first = false;
	...
	drx_sprintf(buffer, "%d", bytesPerParticle);
	ar(string(buffer), "", "!Bytes per Particle:");
}

void SComponentParams::GetMaxParticleCounts(i32& total, i32& perFrame, float minFPS, float maxFPS) const
{
	...
	perFrame = i32(m_maxParticlesBurst + m_maxParticlesPerFrame) + int_ceil(m_maxParticleRate / minFPS);
}
...
{
	DRX_PFX2_ASSERT(numSwapIds == m_features.size());
...
{
	DRX_PFX2_ASSERT(size > 0);        // instance data of 0 bytes makes no sense
	SetChanged();
...
void CParticleComponent::GetMaxParticleCounts(i32& total, i32& perFrame, float minFPS, float maxFPS) const
{
 	m_Params.GetMaxParticleCounts(total, perFrame, minFPS, maxFPS);
	if (m_parent)
	{
		i32 totalParent, perFrameParent;
...
IMaterial* CParticleComponent::MakeMaterial()
{
	...
	pMaterial = gEnv->p3DEngine->GetMaterialUpr()->CreateMaterial(materialName);
	if (gEnv->pRenderer)
	{
		tukk shaderName = UsesGPU() ? "Particles.ParticlesGpu" : "Particles";
		const string& diffuseMap = m_Params.m_diffuseMap;
		u32k textureLoadFlags = 0;//FT_DONT_STREAM;
		...
		if (pTexture->GetTextureID() <= 0)
			DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_WARNING, "Particle effect texture %s not found", diffuseMap.c_str());
		...
		u32 mask = eGpuParticlesVertexShaderFlags_None;
...
void CParticleComponent::SetName(tukk name)
{
	if (!m_pEffect)
		m_name = name;
	else
