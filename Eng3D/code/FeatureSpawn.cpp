...
protected:
	...
	virtual void SpawnParticles(CParticleComponentRuntime& runtime, TDynArray<SSpawnEntry>& spawnEntries) override
	{
		...
		for (uint i = 0; i < numInstances; ++i)
		{
			...
			{
				SSpawnEntry entry = {};
				entry.m_count = u32(ceil(spawnData.m_spawned + spawned) - ceil(spawnData.m_spawned));
				if (entry.m_count)
				{
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureSpawnCount, "Spawn", "Count", colorSpawn);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureSpawnRate, "Spawn", "Rate", colorSpawn);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureSpawnDistance, "Spawn", "Distance", colorSpawn);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureSpawnDensity, "Spawn", "Density", colorSpawn);
