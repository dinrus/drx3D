...
public:
	...
	void GenerateSurface(spline::CatmullRomSpline<Vec3>* pSpline, float fRadius, bool bClosed,
	                     const Vec3& rkUpVector, i32 iMedialSamples, i32 iSliceSamples,
	...
	i32  Index(i32 iS, i32 iM) { return iS + (m_iSliceSamples + 1) * iM; };
	void ComputeSinCos();
	void ComputeVertices(Vec3* akVertex);
	void ComputeNormals();
	void ComputeTextures(const Vec2& rkTextureMin,
	                     const Vec2& rkTextureMax, Vec2* akTexture);
	void ComputeConnectivity(i32& riTQuantity, vtx_idx*& raiConnect, bool bInsideView);

	float                           m_fRadius;
	i32                             m_iMedialSamples, m_iSliceSamples;
	Vec3                            m_kUpVector;
	float*                          m_afSin;
	float*                          m_afCos;
	...
	i32                             iTQuantity; // Num triangles (use iNumIndices instead).
...
void TubeSurface::GenerateSurface(spline::CatmullRomSpline<Vec3>* pSpline, float fRadius,
                                  bool bClosed, const Vec3& rkUpVector, i32 iMedialSamples,
                                  i32 iSliceSamples, bool bWantNormals, bool bWantColors,
...
void TubeSurface::ComputeSinCos()
{
	...
	for (i32 i = 0; i < m_iSliceSamples; i++)
	{
...
void TubeSurface::ComputeVertices(Vec3* akVertex)
{
	...
	/*	float fTotalLength;
	   if ( m_bSampleByArcLength )
	    ...
	    fTotalLength = 0.0f;
	 */
	if (DinrusX3dEngBase::GetCVars()->e_Ropes == 2)
	{
		for (i32 i = 0, npoints = m_pSpline->num_keys() - 1; i < npoints; i++)
		{
	...
	for (i32 iM = 0, iV = 0; iM < m_iMedialSamples; iM++)
	{
	...
	if (m_bClosed)
	{
		for (i32 i = 0; i <= m_iSliceSamples; i++)
		{
			i32 i1 = Index(i, m_iMedialSamples);
			i32 i0 = Index(i, 0);
...
void TubeSurface::ComputeNormals()
{
	...
	else
	{
		...
		for (iS = 0; iS < m_iSliceSamples; iS++)
		{
			...
			i32 nIndex = Index(iS, m_iMedialSamples - 1);
			m_akNormals[nIndex] = kN;
			m_akTangents[nIndex] = kT;
			m_akBitangents[nIndex] = kB;
		}
                                  ...
                                  const Vec2& rkTextureMax, Vec2* akTexture)
{
	...
	i32 iMMax = (m_bClosed ? m_iMedialSamples : m_iMedialSamples - 1);
	for (i32 iM = 0, iV = 0; iM <= iMMax; iM++)
	{
		float fMRatio = ((float)iM) / ((float)iMMax);
		float fMValue = rkTextureMin.y + fMRatio * kTextureRange.y;
		for (i32 iS = 0; iS <= m_iSliceSamples; iS++)
		{
...
//----------------------------------------------------------------------------
void TubeSurface::ComputeConnectivity(i32& riTQuantity, vtx_idx*& raiConnect, bool bInsideView)
{
	if (m_bClosed)
...
/*
   ...
   {
   i32 j = m_iVertexQuantity - m_iSliceSamples - 1;
   for (i32 i = 0; i <= m_iSliceSamples; i++, j++)
...
private:
	static DrxCriticalSectionNonRecursive  g_CacheLock;
	static std::vector<CRopeSurfaceCache*> g_CachePool;
};

DrxCriticalSectionNonRecursive CRopeSurfaceCache::g_CacheLock;
std::vector<CRopeSurfaceCache*> CRopeSurfaceCache::g_CachePool;

CRopeSurfaceCache* CRopeSurfaceCache::GetSurfaceCache()
{
	DrxAutoLock<DrxCriticalSectionNonRecursive> lock(g_CacheLock);
...
//////////////////////////////////////////////////////////////////////////
i32 CRopeRenderNode::OnPhysStateChange(EventPhys const* pEvent)
{
...
//////////////////////////////////////////////////////////////////////////
tukk CRopeRenderNode::GetEntityClassName() const
{
	return "RopeEntity";
}

//////////////////////////////////////////////////////////////////////////
void CRopeRenderNode::SetName(tukk sName)
{
	m_sName = sName;
}

//////////////////////////////////////////////////////////////////////////
tukk CRopeRenderNode::GetName() const
{
	return m_sName;
}
...
bool CRopeRenderNode::RenderDebugInfo(const SRendParams& rParams, const SRenderingPassInfo& passInfo)
{
	...
	i32 nTris = m_pRenderMesh->GetIndicesCount() / 3;
	// text
...
void CRopeRenderNode::Physicalize(bool bInstant)
{
	...
	i32 nSrcPoints = (i32)m_points.size();
	i32 nSrcSegments = nSrcPoints - 1;

	m_physicsPoints.resize(m_params.nPhysSegments + 1);
	i32 nPoints = (i32)m_physicsPoints.size();
	i32 nTargetSegments = nPoints - 1;
	i32 i;
	...
	else
	{
		...
		i32 numSplinePoints = m_points.size();
		m_spline.resize(numSplinePoints);
		for (i32 pnt = 0; pnt < numSplinePoints; pnt++)
		{
		...
		i32 nLengthSamples = m_params.nNumSegments + 1;
...
void CRopeRenderNode::AnchorEndPoints(pe_params_rope& pr)
{
	...
	i32 collisionEntityTypes = ent_static | ent_sleeping_rigid | ent_rigid | ent_ignore_noncolliding;
	float d = 0;
...
//////////////////////////////////////////////////////////////////////////
void CRopeRenderNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void CRopeRenderNode::UpdateRenderMesh()
{
	...
	// Tesselate.
	//////////////////////////////////////////////////////////////////////////
	i32 numSplinePoints = m_physicsPoints.size();
	m_spline.resize(numSplinePoints);
	for (i32 pnt = 0; pnt < numSplinePoints; pnt++)
	{
		m_spline.key(pnt).flags = 0;
	...
	i32 nNewVertexCount = tubeSurf.iVQuantity;
	...
	for (i32 i = 0; i < nNewVertexCount; i++)
	{
...
//////////////////////////////////////////////////////////////////////////
void CRopeRenderNode::SetPoints(const Vec3* pPoints, i32 nCount)
{
...
//////////////////////////////////////////////////////////////////////////
i32 CRopeRenderNode::GetPointsCount() const
{
	return (i32)m_points.size();
}
...
void CRopeRenderNode::OnPhysicsPostStep()
{
	...
	else
	{
		...
		{
			DrxAudio::SCreateObjectData const objectData("RopeyMcRopeFace", m_audioParams.occlusionType);
			m_pIAudioObject = gEnv->pAudioSystem->CreateObject(objectData);
			m_pIAudioObject->SetSwitchState(DrxAudio::AbsoluteVelocityTrackingSwitchId, DrxAudio::OnStateId);
			UpdateAudio();
...
void CRopeRenderNode::UpdateAudio()
{
	...
	i32 idx = m_audioParams.segementToAttachTo - 1;
	...
	if (m_audioParams.angleParameter != DrxAudio::InvalidControlId)
	{
		float angle = 180.0f;
...
void CRopeRenderNode::DisableAudio()
{
	if (m_pIAudioObject != nullptr)
	{
		if (m_audioParams.stopTrigger != DrxAudio::InvalidControlId)
		{
...
void CRopeRenderNode::SetAudioParams(SRopeAudioParams const& audioParams)
{
	...
	if (m_pIAudioObject != nullptr)
	{
		...
		{
			if (m_audioParams.startTrigger != DrxAudio::InvalidControlId)
			{
				m_pIAudioObject->StopTrigger(m_audioParams.startTrigger);
			}
		}
