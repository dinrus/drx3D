...
tukk CBrush::GetEntityClassName() const
{
	return "Brush";
}

tukk CBrush::GetName() const
{
	if (m_pStatObj)
...
CLodValue CBrush::ComputeLod(i32 wantedLod, const SRenderingPassInfo& passInfo)
{
	i32 nLodA = -1;
	...
	if (CStatObj* pStatObj = m_pStatObj)
	{
		...
		nLodA = CLAMP(wantedLod, pStatObj->GetMinUsableLod(), (i32)pStatObj->m_nMaxUsableLod);
...
bool CBrush::PhysicalizeFoliage(bool bPhysicalize, i32 iSource, i32 nSlot)
{
	if (nSlot < 0 || !m_pStatObj->GetSubObjectCount())
	{
		bool res = false;
		for (i32 i = 0; i < m_pStatObj->GetSubObjectCount(); i++)
...
IFoliage* CBrush::GetFoliage(i32 nSlot)
{
	if (!m_pStatObj)
		return nullptr;
...
//////////////////////////////////////////////////////////////////////////
void CBrush::UpdatePhysicalMaterials(i32 bThreadSafe)
{
	if (!m_pPhysEnt)
		return;
	...
	if (m_pMaterial)
	{
		...
		i32 i, numIds = m_pMaterial->FillSurfaceTypeIds(surfaceTypesId);
...
void CBrush::Dephysicalize(bool bKeepIfReferenced)
{
	...
	// delete old physics
	if (m_pPhysEnt && 0 != GetSystem()->GetIPhysicalWorld()->DestroyPhysicalEntity(m_pPhysEnt, ((i32)bKeepIfReferenced) * 4))
		m_pPhysEnt = 0;
...
void CBrush::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void CBrush::SetLayerId(u16 nLayerId)
{
...
IRenderMesh* CBrush::GetRenderMesh(i32 nLod)
{
...
void CBrush::Render(const CLodValue& lodValue, const SRenderingPassInfo& passInfo, SSectorTextureSet* pTerrainTexInfo, PodArray<SRenderLight*>* pAffectingLights)
{
	...
	/*
	   ...
	   if (pFoliage)
	   {
	    pFoliage->SetFlags(pFoliage->GetFlags() & ~IFoliage::FLAG_FROZEN | -(i32)(rParams.nMaterialLayers&MTL_LAYER_FROZEN) & IFoliage::FLAG_FROZEN);
	...
	u8k nMaterialLayers = IRenderNode::GetMaterialLayers();
	...
	if (!pObj)
	{
		if (GetObjUpr()->AddOrCreatePersistentRenderObject(pTempData, pObj, &lodValue, transformMatrix, passInfo))
		{
	...
	{
		if (!(pObj->m_nTextureID = GetObjUpr()->CheckCachedNearestCubeProbe(this)) || !GetCVars()->e_CacheNearestCubePicking)
			pObj->m_nTextureID = GetObjUpr()->GetNearestCubeProbe(pAffectingLights, m_pOcNode->GetVisArea(), CBrush::GetBBox());
	...
	if (pTerrainTexInfo)
	{
		...
		if (bUseTerrainColor)
		{
			...
			pObj->m_nTextureID = -(i32)pTerrainTexInfo->nSlot0 - 1; // nTextureID is set only for proper batching, actual texture id is same for all terrain sectors
	...
	if (m_pFoliage)
	{
		if (SRenderObjData* pOD = pObj->GetObjData())
		{
			...
			//m_pFoliage->SetFlags(m_pFoliage->GetFlags() & ~IFoliage::FLAG_FROZEN | -(i32)(pObj->m_nMaterialLayers & MTL_LAYER_FROZEN) & IFoliage::FLAG_FROZEN);
		}
	}

	{
		...
		if (GetCVars()->e_BBoxes)
			GetObjUpr()->RenderObjectDebugInfo((IRenderNode*)this, pObj->m_fDistance, passInfo);

		if (lodValue.LodA() <= 0 && DinrusX3dEngBase::GetCVars()->e_MergedMeshes != 0 && m_pDeform && m_pDeform->HasDeformableData())
		{
			if (Get3DEngine()->IsStatObjBufferRenderTasksAllowed() && passInfo.IsGeneralPass() && JobUpr::InvokeAsJob("CheckOcclusion"))
			{
				GetObjUpr()->PushIntoCullOutputQueue(SCheckOcclusionOutput::CreateDeformableBrushOutput(this, gEnv->pRenderer->EF_DuplicateRO(pObj, passInfo), lodValue.LodA(), passInfo));
			}
			else
			{
...
///////////////////////////////////////////////////////////////////////////////
IStatObj* CBrush::GetEntityStatObj(u32 nSubPartId, Matrix34A* pMatrix, bool bReturnOnlyVisible)
{
	if (pMatrix)
		*pMatrix = m_Matrix;
