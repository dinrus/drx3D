...
void CTerrain::AddVisSector(CTerrainNode* pNode, u32 passCullMask)
{
	m_lstVisSectors.Add(STerrainVisItem(pNode, passCullMask));
}

void CTerrain::CheckVis(const SRenderingPassInfo& passInfo, u32 passCullMask)
{
	...
	if (passInfo.IsGeneralPass())
	{
		m_bOceanIsVisible = (i32)((m_fDistanceToSectorWithWater != OCEAN_IS_VERY_FAR_AWAY) || !m_checkVisSectorsCount);
...
i32 __cdecl CmpTerrainNodesImportance(ukk v1, ukk v2)
{
...
i32 __cdecl CmpTerrainNodesDistance(ukk v1, ukk v2)
{
	CTerrainNode* p1 = *(CTerrainNode**)v1;
	CTerrainNode* p2 = *(CTerrainNode**)v2;
...
void CTerrain::ActivateNodeTexture(CTerrainNode* pNode, const SRenderingPassInfo& passInfo)
{
	...
	pNode->m_nNodeTextureLastUsedSec4 = (u16)(GetCurTimeSec() / 4.f);
...
i32 CTerrain::GetNotReadyTextureNodesCount()
{
	i32 nRes = 0;
	for (i32 i = 0; i < m_lstActiveTextureNodes.Count(); i++)
...
void CTerrain::CheckNodesGeomUnload(const SRenderingPassInfo& passInfo)
{
	...
	for (i32 n = 0; n < 32; n++)
	{
		static u32 nOldSectorsX = ~0, nOldSectorsY = ~0, nTreeLevel = ~0;

		if (nTreeLevel > (u32) GetParentNode()->m_nTreeLevel)
			nTreeLevel = GetParentNode()->m_nTreeLevel;

		u32 nTableSize = CTerrain::GetSectorsTableSize() >> nTreeLevel;
		assert(nTableSize);
...
void CTerrain::UpdateNodesIncrementaly(const SRenderingPassInfo& passInfo)
{
	...
	if (m_lstActiveTextureNodes.Count() && m_texCache[0].Update())
	{
		...
		i32 nMaxNodesToLoad = 6;
		assert(m_texCache[0].m_FreeTextures.Count());
		for (i32 i = 0; i < m_lstActiveTextureNodes.Count() && nMaxNodesToLoad; i++)
	...
	if (m_lstActiveProcObjNodes.Count())
	{
		...
		for (i32 i = 0; i < m_lstActiveProcObjNodes.Count(); i++)
		...
		static i32 nMaxSectors = MAX_PROC_SECTORS_NUM;
		...
		while (1)
		{
			...
			{
				i32 nAll = 0;
				i32 nUsed = CTerrainNode::GetProcObjChunkPool()->GetUsedInstancesCount(nAll);
...
i32 CTerrain::UpdateOcean(const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
...
i32 CTerrain::RenderOcean(const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
...
void CTerrain::GetMemoryUsage(class IDrxSizer* pSizer) const
{
...
i32 CTerrain::GetTerrainNodesAmount()
{
	//	((N pow l)-1)/(N-1)
#if defined(__GNUC__)
...
void CTerrain::MarkAllSectorsAsUncompiled()
{
	...
	if (DinrusX3dEngBase::Get3DEngine()->GetObjectsTree())
	{
		DinrusX3dEngBase::Get3DEngine()->GetObjectsTree()->MarkAsUncompiled();
	}
}
...
i32 CTerrain::GetTerrainLightmapTexId(Vec4& vTexGenInfo)
{
	AABB nearWorldBox;
	float fBoxSize = 512;
...
void CTerrain::GetAtlasTexId(i32& nTex0, i32& nTex1, i32& nTex2)
{
...
void CTerrain::GetResourceMemoryUsage(IDrxSizer* pSizer, const AABB& crstAABB)
{
