...
public:
	Verify()
	{
		for (i32 i = 0; s_TexSlotSemantics[i].name; i++)
		{
...
EEfResTextures MaterialHelpers::FindTexSlot(tukk texName) const
{
	if (texName)
	{
		for (i32 i = 0; s_TexSlotSemantics[i].name; i++)
...
tukk MaterialHelpers::FindTexName(EEfResTextures texSlot) const
{
	for (i32 i = 0; s_TexSlotSemantics[i].name; i++)
	{
...
tukk MaterialHelpers::LookupTexName(EEfResTextures texSlot) const
{
	assert((texSlot >= 0) && (texSlot < EFTT_MAX));
	return s_TexSlotSemantics[texSlot].name;
}

tukk MaterialHelpers::LookupTexEnum(EEfResTextures texSlot) const
{
	assert((texSlot >= 0) && (texSlot < EFTT_MAX));
	return s_TexSlotSemantics[texSlot].ename;
}

tukk MaterialHelpers::LookupTexSuffix(EEfResTextures texSlot) const
{
	assert((texSlot >= 0) && (texSlot < EFTT_MAX));
...
//////////////////////////////////////////////////////////////////////////
bool MaterialHelpers::SetGetMaterialParamFloat(IRenderShaderResources& pShaderResources, tukk sParamName, float& v, bool bGet) const
{
...
//////////////////////////////////////////////////////////////////////////
bool MaterialHelpers::SetGetMaterialParamVec3(IRenderShaderResources& pShaderResources, tukk sParamName, Vec3& v, bool bGet) const
{
	EEfResTextures texSlot = EFTT_UNKNOWN;
...
//////////////////////////////////////////////////////////////////////////
void MaterialHelpers::SetTexturesFromXml(SInputShaderResources& pShaderResources, const XmlNodeRef& node, tukk szBaseFileName) const
{
	...
	for (i32 c = 0; c < texturesNode->getChildCount(); c++)
	{
		XmlNodeRef texNode = texturesNode->getChild(c);

		tukk const szTexmap = texNode->getAttr("Map");
		...
		tukk const szFile = texNode->getAttr("File");
...

		{
			...
			if (texNode->getAttr("TexType", texTypeName))
			{
				...
				if (texType > eTT_MaxTexType || !IsTextureTypeExposed(texType))
				{
					DrxLog("%s: Ignoring bad value '%s' for texture type.\n", __FUNCTION__, texTypeName.c_str());
				}
				else
				{
		...
		i32 filter = pShaderResources.m_Textures[texId].m_Filter;
...
void MaterialHelpers::SetXmlFromTextures(const SInputShaderResources& pShaderResources, XmlNodeRef& node, tukk szBaseFileName) const
{
	...
	for (EEfResTextures texId = EEfResTextures(0); texId < EFTT_MAX; texId = EEfResTextures(texId + 1))
	{
		if (!pShaderResources.m_Textures[texId].m_Name.empty())
		{
			...
			{
				DRX_ASSERT(IsTextureTypeExposed(pShaderResources.m_Textures[texId].m_Sampler.m_eTexType));
...
{
	i32 vertModif = pShaderResources.m_DeformInfo.m_eType;
...
void MaterialHelpers::SetShaderParamsFromXml(SInputShaderResources& pShaderResources, const XmlNodeRef& node) const
{
	...
	for (i32 i = 0; i < nA; i++)
	{
		...
		for (i32 i = 0; i < pShaderResources.m_ShaderParams.size(); i++)
		{
		...
		if (!bFound)
		{
			...
			i32 res = sscanf(val, "%f,%f,%f,%f", &Param.m_Value.m_Color[0], &Param.m_Value.m_Color[1], &Param.m_Value.m_Color[2], &Param.m_Value.m_Color[3]);
			assert(res);
...
void MaterialHelpers::SetXmlFromShaderParams(const SInputShaderResources& pShaderResources, XmlNodeRef& node) const
{
	for (i32 i = 0; i < pShaderResources.m_ShaderParams.size(); i++)
	{
		...
		{
		case eType_BYTE:
			node->setAttr(pParam->m_Name, (i32)pParam->m_Value.m_Byte);
			break;
		case eType_SHORT:
			node->setAttr(pParam->m_Name, (i32)pParam->m_Value.m_Short);
			break;
		case eType_INT:
			node->setAttr(pParam->m_Name, (i32)pParam->m_Value.m_Int);
			break;
		case eType_FLOAT:
...
tukk MaterialHelpers::GetNameFromTextureType(u8 texType) const
{
	DRX_ASSERT(texType < eTT_MaxTexType);
	switch (texType)
	{
	case eTT_1D:
		return "1D";
	...
	case eTT_Auto2D:
		return "Auto 2D-Map";
	default:
		DRX_ASSERT(0 && "unknown texture type");
		return nullptr;
	}
}

u8 MaterialHelpers::GetTextureTypeFromName(tukk szName) const
{
	DRX_ASSERT(szName);
	for (i32 i = 0; i < eTT_MaxTexType; ++i)
	{
...
bool MaterialHelpers::IsTextureTypeExposed(u8 texType) const
{
	DRX_ASSERT(texType < eTT_MaxTexType);
	switch (texType)
	{
