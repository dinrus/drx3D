...
#include <drx3D/Eng3D/EnvironmentPreset.h>
...
public:
	...
	{
		for (i32 i = 0, nkeys = num_keys(); i < nkeys; i++)
		{
			ValueType val;
...
void ReadCTimeOfDayVariableFromXmlNode(CTimeOfDayVariable* pVar, XmlNodeRef varNode)
{
	...
	if (var.GetType() == ITimeOfDay::TYPE_FLOAT)
	{
		...
		i32k nKeyCount = floatSpline.GetKeyCount();
		pSpline->Resize(nKeyCount);
		for (i32 k = 0; k < nKeyCount; ++k)
		{
			...
			i32k nKeyFlags = floatSpline.GetKeyFlags(k);
	...
	else if (var.GetType() == ITimeOfDay::TYPE_COLOR)
	{
		...
		ISplineInterpolator::ValueType fValue;
		i32k nKeyCount = colorSpline.GetKeyCount();
		...
		for (i32 k = 0; k < nKeyCount; ++k)
		{
			...
			i32k nKeyFlags = colorSpline.GetKeyFlags(k);
			ESplineKeyTangentType inTangentType = ESplineKeyTangentType((nKeyFlags & SPLINE_KEY_TANGENT_IN_MASK) >> SPLINE_KEY_TANGENT_IN_SHIFT);
			ESplineKeyTangentType outTangentType = ESplineKeyTangentType((nKeyFlags & SPLINE_KEY_TANGENT_OUT_MASK) >> SPLINE_KEY_TANGENT_OUT_SHIFT);
...
{
	i32k nChildCount = node->getChildCount();
	for (i32 i = 0; i < nChildCount; ++i)
	{
		XmlNodeRef varNode = node->getChild(i);
		tukk varName = varNode->getAttr("Name");
		if (!varName)
			continue;
...
void MigrateLegacyData(CEnvironmentPreset& preset, bool bSunIntensity)
{
	if (bSunIntensity)    // Convert sun intensity as specified up to CE 3.8.2 to illuminance
	{
		...
		i32k numKeys = varSunMultSpline->GetKeyCount();
		for (i32 key = 0; key < numKeys; ++key)
		{
...
static tukk sPresetsLibsPath = "libs/environmentpresets/";
static tukk sPresetXMLRootNodeName = "EnvironmentPreset";
...
string GetPresetXMLFilenamefromName(const string& presetName, const bool bForWriting)
{
	...
	tukk szTemp = gEnv->pDrxPak->AdjustFileName(filePath.c_str(), szAdjustedFile, bForWriting ? IDrxPak::FLAGS_FOR_WRITING : 0);
...
bool SavePresetToXML(const CEnvironmentPreset& preset, const string& presetName)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, "TimeOfDay: Failed to save preset: %s", presetName.c_str());
		return false;
	}
...
{
	const string filename = GetPresetXMLFilenamefromName(presetName, false);
	const bool bFileExist = gEnv->pDrxPak->IsFileExist(filename.c_str());
	if (!bFileExist)
	{
		DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, "TimeOfDay: Failed to load preset: %s", presetName.c_str());
...
CTimeOfDay::CTimeOfDay()
	: m_timeOfDayRtpcId(DrxAudio::InvalidControlId)
	, m_listeners(16)
{
	m_pTimer = 0;
	...
	m_timeOfDayRtpcId = DrxAudio::StringToId("time_of_day");
}
...
bool CTimeOfDay::GetPresetsInfos(SPresetInfo* resultArray, u32 arraySize) const
{
...
bool CTimeOfDay::SetCurrentPreset(tukk szPresetName)
{
...
tukk CTimeOfDay::GetCurrentPresetName() const
{
	return m_currentPresetName.c_str();
}

bool CTimeOfDay::AddNewPreset(tukk szPresetName)
{
...
bool CTimeOfDay::RemovePreset(tukk szPresetName)
{
	TPresetsSet::const_iterator it = m_presets.find(szPresetName);
	if (it != m_presets.end())
	{
...
bool CTimeOfDay::SavePreset(tukk szPresetName) const
{
...
bool CTimeOfDay::LoadPreset(tukk szFilePath)
{
...
void CTimeOfDay::ResetPreset(tukk szPresetName)
{
	TPresetsSet::iterator it = m_presets.find(szPresetName);
	if (it != m_presets.end())
	{
...
bool CTimeOfDay::ImportPreset(tukk szPresetName, tukk szFilePath)
{
	TPresetsSet::iterator it = m_presets.find(szPresetName);
	if (it == m_presets.end())
		return false;
...
bool CTimeOfDay::ExportPreset(tukk szPresetName, tukk szFilePath) const
{
	const string sPresetName(szPresetName);
	TPresetsSet::const_iterator it = m_presets.find(sPresetName);
	if (it != m_presets.end())
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, "TimeOfDay: Failed to save preset: %s", szFilePath);
			return false;
		}
		return true;
	}
...
//////////////////////////////////////////////////////////////////////////
bool CTimeOfDay::GetVariableInfo(i32 nIndex, SVariableInfo& varInfo)
{

	if (nIndex < 0 || nIndex >= (i32)ITimeOfDay::PARAM_TOTAL)
		return false;
...
//////////////////////////////////////////////////////////////////////////
void CTimeOfDay::SetVariableValue(i32 nIndex, float fValue[3])
{
	if (nIndex < 0 || nIndex >= (i32)ITimeOfDay::PARAM_TOTAL)
		return;
...
bool CTimeOfDay::InterpolateVarInRange(i32 nIndex, float fMin, float fMax, u32 nCount, Vec3* resultArray) const
{
...
uint CTimeOfDay::GetSplineKeysCount(i32 nIndex, i32 nSpline) const
{
	if (nIndex >= 0 && nIndex < ITimeOfDay::PARAM_TOTAL && m_pCurrentPreset)
	{
...
bool CTimeOfDay::GetSplineKeysForVar(i32 nIndex, i32 nSpline, SBezierKey* keysArray, u32 keysArraySize) const
{
	if (nIndex >= 0 && nIndex < ITimeOfDay::PARAM_TOTAL && m_pCurrentPreset)
	{
		CTimeOfDayVariable* pVar = m_pCurrentPreset->GetVar((ITimeOfDay::ETimeOfDayParamID)nIndex);
...
bool CTimeOfDay::SetSplineKeysForVar(i32 nIndex, i32 nSpline, const SBezierKey* keysArray, u32 keysArraySize)
{
	if (nIndex >= 0 && nIndex < ITimeOfDay::PARAM_TOTAL && m_pCurrentPreset)
	{
		CTimeOfDayVariable* pVar = m_pCurrentPreset->GetVar((ITimeOfDay::ETimeOfDayParamID)nIndex);
...
bool CTimeOfDay::UpdateSplineKeyForVar(i32 nIndex, i32 nSpline, float fTime, float newValue)
{
	if (nIndex >= 0 && nIndex < ITimeOfDay::PARAM_TOTAL && m_pCurrentPreset)
	{
		CTimeOfDayVariable* pVar = m_pCurrentPreset->GetVar((ITimeOfDay::ETimeOfDayParamID)nIndex);
...
void CTimeOfDay::LoadInternalState(struct IDataReadStream& reader)
{
	...
	{
		XmlNodeRef node = gEnv->pSystem->LoadXmlFromBuffer((tukk)&timeOfDatXML[0], timeOfDatXML.size());
		if (node)
		{
			Serialize(node, true);
			Update(true, true);
...
//////////////////////////////////////////////////////////////////////////
bool CTimeOfDay::RegisterListenerImpl(IListener* const pListener, tukk const szDbgName, const bool staticName)
{
...
void CTimeOfDay::SetTime(float fHour, bool bForceUpdate)
{
	...
	DinrusX3dEngBase::GetCVars()->e_TimeOfDay = m_fTime;

	Update(true, bForceUpdate);

	// Inform audio of this change.
	if (m_timeOfDayRtpcId != DrxAudio::InvalidControlId)
	{
...
void CTimeOfDay::Serialize(XmlNodeRef& node, bool bLoading)
{
	if (bLoading)
	{
		...
		{
			i32k nPresetsCount = presetsNode->getChildCount();
			for (i32 i = 0; i < nPresetsCount; ++i)
			{
				if (XmlNodeRef presetNode = presetsNode->getChild(i))
				{
					tukk presetName = presetNode->getAttr("Name");
					if (presetName)
					{
		...
		else
		{
			...
			{
				tukk pLevelName = gEnv->pGameFramework->GetLevelName();
...
//////////////////////////////////////////////////////////////////////////
void CTimeOfDay::NetSerialize(TSerialize ser, float lag, u32 flags)
{
...
//////////////////////////////////////////////////////////////////////////
void CTimeOfDay::NotifyOnChange(const IListener::EChangeType changeType, tukk const szPresetName)
{
