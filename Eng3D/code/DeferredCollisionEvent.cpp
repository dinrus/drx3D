...
void CDeferredPhysicsEventUpr::DispatchDeferredEvent(IDeferredPhysicsEvent* pEvent)
{
	assert(pEvent);
...
i32 CDeferredPhysicsEventUpr::HandleEvent(const EventPhys* pEvent, IDeferredPhysicsEventUpr::CreateEventFunc pCreateFunc, IDeferredPhysicsEvent::DeferredEventType type)
{
	...
	{
		i32 nResult = pDeferredEvent->Result((EventPhys*)pEvent);
...
void CDeferredPhysicsEventUpr::RegisterDeferredEvent(IDeferredPhysicsEvent* pDeferredEvent)
{
	assert(pDeferredEvent);
	m_activeDeferredEvents.push_back(pDeferredEvent);
}

void CDeferredPhysicsEventUpr::UnRegisterDeferredEvent(IDeferredPhysicsEvent* pDeferredEvent)
{
...
void CDeferredPhysicsEventUpr::ClearDeferredEvents()
{
...
void CDeferredPhysicsEventUpr::Update()
{
	std::vector<IDeferredPhysicsEvent*> tmp = m_activeDeferredEvents;
...
IDeferredPhysicsEvent* CDeferredPhysicsEventUpr::GetLastCollisionEventForEntity(IPhysicalEntity* pPhysEnt)
{
	EventPhysCollision* pLastPhysEvent;
	for (i32 i = m_activeDeferredEvents.size() - 1; i >= 0; i--)
