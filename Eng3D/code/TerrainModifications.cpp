...
#include <drx3D/Eng3D/TerrainModifications.h>        // CTerrainModifications
#include <drx3D/Eng3D/terrain.h>                      // CTerrain
#include <drx3D/Eng3D/terrain_sector.h>               // CTerrainNode
...
float CTerrainModifications::ComputeMaxDepthAt(const float fX, const float fY, u32k dwCheckExistingMods) const
{
	float fRet = 0;
...
void CTerrainModifications::MakeCrater(const STerrainMod& ref, u32k dwCheckExistingMod)
{
	...
	for (i32 s = 0; s < lstNearSecInfos.Count(); s++)
