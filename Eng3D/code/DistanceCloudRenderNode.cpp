...
tukk CDistanceCloudRenderNode::GetEntityClassName() const
{
	return "DistanceCloud";
}

tukk CDistanceCloudRenderNode::GetName() const
{
	return "DistanceCloud";
}

static inline u16 HalfFlip(u16 h)
{
	u16 mask = -i16(h >> 15) | 0x8000;
	return h ^ mask;
}
...
void CDistanceCloudRenderNode::Render(const SRendParams& rParam, const SRenderingPassInfo& passInfo)
{
	...
	if (!pRenderObject)
		return;
	pRenderObject->m_nSort = HalfFlip(DrxConvertFloatToHalf(zDist));
	...
	i32 afterWater(GetObjUpr()->IsAfterWater(m_pos, passInfo.GetCamera().GetPosition(), passInfo, Get3DEngine()->GetWaterLevel()) ? 1 : 0);
...
void CDistanceCloudRenderNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
