	...
	, m_nextPhysFrag(0)
{
	...
	for (i32 i = 0; i < GLASSCFG_MAX_NUM_PHYS_FRAGMENTS; ++i)
	{
...
{
	DRX_ASSERT((m_dwRndFlags & ERF_PENDING_DELETE) == 0);

	// Remove all remaining glass physics
	DephysicalizeGlass();

	i32k numPhysFrags = m_physFrags.size();
	SGlassPhysFragment* pPhysFrags = m_physFrags.begin();

	for (i32 i = 0; i < numPhysFrags; ++i)
	{
...
//--------------------------------------------------------------------------------------------------
void CBreakableGlassRenderNode::SetId(u16k id)
{
	m_id = id;
}//-------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------
u16 CBreakableGlassRenderNode::GetId()
{
	return m_id;
}//-------------------------------------------------------------------------------------------------
...
void CBreakableGlassRenderNode::PhysicalizeGlass()
{
	...
	IGeomUpr* pGeomMan = pPhysWorld->GetGeomUpr();
...
void CBreakableGlassRenderNode::DephysicalizeGlassFragment(SGlassPhysFragment& physFrag)
{
	...
	if (physFrag.m_pPhysEnt)
	{
		...
		u16 physFragData = physFrag.m_bufferIndex;
		physFragData |= (u16)physFrag.m_fragIndex << 8;
...
void CBreakableGlassRenderNode::Update(SBreakableGlassUpdateParams& params)
{
	if (m_pBreakableGlassRE)
	{
		...
		{
			DRX_ASSERT_MESSAGE(0, "Glass physical fragment arrays are out of sync.");
		...
		i32k numNewPhysFrags = newPhysFrags.size();
		bool fragsActive = (numNewPhysFrags > 0);

		for (i32 i = 0; i < numNewPhysFrags; ++i)
		{
...
void CBreakableGlassRenderNode::ApplyExplosionToGlass(const EventPhysCollision* pPhysEvent)
{
	if (m_pBreakableGlassRE && pPhysEvent)
	{
		...
		params.speed = GLASSCFG_MIN_DRX3D_SPEED * 2.0f;
...
void CBreakableGlassRenderNode::DestroyPhysFragment(SGlassPhysFragment* pPhysFrag)
{
	if (pPhysFrag && pPhysFrag->m_pPhysEnt)
	{
		...
		{
			DRX_ASSERT_MESSAGE(0, "Glass physicalized fragment being destroyed by invalid render node.");
...
void CBreakableGlassRenderNode::UpdateGlassState(const EventPhysCollision* pPhysEvent)
{
	if (m_pBreakableGlassRE && m_pPhysEnt)
	{
		...
		u8 lastNumImpacts = m_lastGlassState.m_numImpacts;
		u8 lastNumLooseFrags = m_lastGlassState.m_numLooseFrags;
		...
		if (physicsChanged)
		{
			...
			i32 numEntities = gEnv->pPhysicalWorld->GetEntitiesInBox(glassArea.min, glassArea.max, pEntities, ent_sleeping_rigid | ent_living | ent_ignore_noncolliding);
...
void CBreakableGlassRenderNode::SetParticleEffectColours(IParticleEffect* pEffect, const Vec4& rgba)
{
	if (pEffect)
	{
		...
		i32k numSubEffects = pEffect->GetChildCount();
		for (i32 i = 0; i < numSubEffects; ++i)
		{
...
//--------------------------------------------------------------------------------------------------
tukk CBreakableGlassRenderNode::GetName() const
{
...
//--------------------------------------------------------------------------------------------------
tukk CBreakableGlassRenderNode::GetEntityClassName() const
{
	return "BreakableGlass";
}//-------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------
void CBreakableGlassRenderNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void CBreakableGlassRenderNode::SetMaterial(IMaterial* pMaterial)
{
	...
	if (m_glassParams.pGlassMaterial)
	{
		...
		if (shaderItem.m_pShaderResources && shaderItem.m_pShader)
		{
			...
			i32k numParams = shaderParams.size();
...
void CBreakableGlassRenderNode::Render(const SRendParams& renderParams, const SRenderingPassInfo& passInfo)
{
	if (m_glassParams.pGlassMaterial && m_pBreakableGlassRE)
	{
		...
		SGlassPhysFragment* pPhysFrags = m_physFrags.begin();
		DRX_ASSERT(GLASSCFG_MAX_NUM_PHYS_FRAGMENTS == m_physFrags.max_size());
