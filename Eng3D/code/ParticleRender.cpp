...
// -------------------------------------------------------------------------
//  Имя файла:   ParticleRender.cpp
//  Version:     v1.00
//  Created:     28/5/2001 by Vladimir Kajalin
//  Evolved:     2005 by J Scott Peter
//  Описание:    Функции рендеринга для CParticle и CParticleContainer.
// -------------------------------------------------------------------------
//
////////////////////////////////////////////////////////////////////////////
...
struct SParticleVertexContext
{
	...
	i32   m_nParticlesCulled;
	i32   m_nParticlesClipped;

	// Connected-particle info.
	i32          m_nPrevSequence;
	i32          m_nSequenceParticles;
	Vec3         m_vPrevPos;
...
static void CopyToVideoMemory(uk pDst, uk pSrc, u32 nSize, u32 nTransferID)
{
...
///////////////////////////////////////////////////////////////////////////////
template<class T, uint nBUFFER_SIZE, uint nCHUNK_SIZE, i32 nMEM_TRANSFER_ID>
struct SBufferedWriter
{
	SBufferedWriter(FixedDynArray<T>& aDestMem)
		: m_pDestMem(&aDestMem)
		, m_aDestMemBegin((tuk)aDestMem.begin())
		, m_nFlushedBytes(0)
		, m_nWrittenDestBytes(0)
	{
		uint nAlignOffset = alias_cast<uint>(m_aDestMemBegin - (tuk)m_aSrcBuffer) & 127;
		m_aSrcMemBegin = m_aSrcBuffer + nAlignOffset;
		i32 nCapacity = (sizeof(m_aSrcBuffer) - nAlignOffset) / sizeof(T);
	...
	ILINE T* CheckAvailable(i32 nElems)
	{
...
private:
	...
	tuk             m_aDestMemBegin;                  // Cached pointer to start of VMEM.
	...
	tuk             m_aSrcMemBegin;                   // Aligned pointer into SrcBuffer;
	char              m_aSrcBuffer[nBUFFER_SIZE + 128]; // Raw buffer for writing.

	uint              UnflushedBytes() const
	{
		return check_cast<uint>((tuk)m_aSrcArray.end() - m_aSrcMemBegin - m_nFlushedBytes);
	}
	...
	void WrapBuffer()
	{
		...
		m_aSrcArray.set(ArrayT((T*)(m_aSrcMemBegin + nUnFlushedBytes), i32(nAvailableBytes / sizeof(T))));
		m_nFlushedBytes = 0;
	}
...
///////////////////////////////////////////////////////////////////////////////
struct DRX_ALIGN(128) SLocalRenderVertices
{
	...
	bool CheckAvailable(i32 nVertices, i32 nIndices)
	{
		return aVertices.CheckAvailable(nVertices) && aIndices.CheckAvailable(nIndices);
	}

	ILINE static void DuplicateVertices(SVF_Particle* pDest, i32 nCount, const SVF_Particle& Src)
	{
		static const uint nStride = sizeof(SVF_Particle) / sizeof(uint);
		DrxPrefetch(pDest + nCount);
	...
	ILINE void AddQuadIndices(i32 nVertAdvance = 4, i32 bStrip = 0)
	{
		u16* pIndices = aIndices.Array().append(bStrip ? 4 : 6, eNoInit);
	...
	{
		u16* pIndices = aIndices.Array().append(18, eNoInit);
	...
	void AddPolyIndices(i32 nVerts, i32 bStrip = 0)
	{
		nVerts >>= 1;
		while (--nVerts > 0)
...
private:
	...
	SBufferedWriter<u16, 4* 1024, 1* 1024, 6> aIndices;
	i32 nBaseVertexIndex;
...
void CParticleContainer::RenderDecals(const SRenderingPassInfo& passInfo)
{
	...
	decal.nSortOrder = clamp_tpl(i32(params.fSortOffset * 100.f), 0, 255);
	decal.nFlags = 0;
...
void CParticle::AddLight(const SRendParams& RenParams, const SRenderingPassInfo& passInfo) const
{
	    ...
	    && cam.IsSphereVisible_F(Sphere(m_Loc.t, dl.m_fRadius)))
	{
		...
		dl.m_nStencilRef[0] = params.LightSource.bAffectsThisAreaOnly ? RenParams.nClipVolumeStencilRef : CClipVolumeUpr::AffectsEverythingStencilRef;
		dl.m_nStencilRef[1] = CClipVolumeUpr::InactiveVolumeStencilRef;
...
float CParticle::ComputeRenderAlpha(const SParticleRenderData& RenderData, float fRelativeAge, SParticleVertexContext& Context) const
{
	...
	if (Context.m_bOcclusionCull)
	{
		...
		if (fFillPix >= fMIN_OCCLUSION_CULL_PIX)
		{
			...
			if (!GetObjUpr()->CheckOcclusion_TestQuad(Vert.xyz, Vert.xaxis, Vert.yaxis))
			{
				Context.m_nParticlesCulled++;
				return 0.f;
			}
...
//////////////////////////////////////////////////////////////////////////
void CParticle::SetVertices(SLocalRenderVertices& alloc, SParticleVertexContext& Context, u8 uAlpha) const
{
	ResourceParticleParams const& params = Context.m_Params;
	...
	if (params.TextureTiling.nAnimFramesCount > 1)
	{
		...
		BaseVert.st.z += i32(fAnimPos);
		BaseVert.st.w = float_to_ufrac8(fAnimPos - i32(fAnimPos));
	}
	...
	if (params.Connection)
	{
		...
		{
			i32 nIndex = Context.m_nPrevSequence == GetEmitterSequence() ? Context.m_nSequenceParticles : -1;
...
void CParticle::SetTailVertices(const SVF_Particle& BaseVert, SParticleRenderData RenderData, SLocalRenderVertices& alloc, SParticleVertexContext const& Context) const
{
	...
	i32 nPos = GetContainer().GetHistorySteps() - 1;
	for (; nPos >= 0; nPos--)
	{
...
i32 CParticleContainer::CullParticles(SParticleVertexContext& Context, i32& nVertices, i32& nIndices, u8 auParticleAlpha[])
{
	FUNCTION_PROFILER_CONTAINER(this);

	assert(!NeedsUpdate());

	i32 nParticlesRendered = 0;
...
void CParticleContainer::WriteVerticesToVMEM(SParticleVertexContext& Context, SRenderVertices& RenderVerts, u8k auParticleAlpha[])
PREFAST_SUPPRESS_WARNING(6262)   // Function uses '33724' bytes of stack: exceeds /analyze:stacksize'16384'.
{
	...
	u8 uAlphaCullMask = Context.m_Params.Connection ? 1 : 0;

	for (const auto& part : m_Particles)
	{
		u8 uAlpha = *auParticleAlpha++;
...
void SParticleVertexContext::Init(float fMaxContainerPixels, CParticleContainer* pContainer)
{
	...
	if (abs(m_fFillFactor) > std::numeric_limits<float>::denorm_min())
		m_fInvMinPix = sqr(CParticleUpr::Instance()->GetMaxAngularDensity(*m_CamInfo.pCamera) * emitterMain.GetViewDistRatioFloat() * params.fViewDistanceAdjust)
		/ m_fFillFactor;
	else
...
void CParticleContainer::ComputeVertices(const SCameraInfo& camInfo, CREParticle* pRE, uint64 uVertexFlags, float fMaxPixels)
{
	...
	if (!m_Particles.empty())
	{
		...
		// Culling stage
		i32 nVertices = 0, nIndices = 0;
		STACK_ARRAY(u8, auParticleAlpha, m_Particles.size());
		...
		if (CullParticles(Context, nVertices, nIndices, auParticleAlpha))
		{
			...
			if (pRenderVerts->aVertices.capacity() < nVertices || pRenderVerts->aIndices.capacity() < nIndices)
				CParticleUpr::Instance()->MarkAsOutOfMemory();
...
void CParticleUpr::AddVertexIndexPoolUsageEntry(u32 nVertexMemory, u32 nIndexMemory, tukk pContainerName)
{
...
void CParticleUpr::MarkAsOutOfMemory()
{
#if defined(PARTICLE_COLLECT_VERT_IND_POOL_USAGE)
