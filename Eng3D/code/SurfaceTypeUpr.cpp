...
public:
	...
	CMaterialSurfaceType(tukk name)
	{
		m_name = name;
		m_nId = -1;
		m_nFlags = 0;
	...
	virtual tukk                 GetType() const                            { return m_typename; };
	virtual i32                         GetFlags() const                           { return m_nFlags; };
	virtual void                        Execute(SSurfaceTypeExecuteParams& params) {};
	virtual bool                        Load(i32 nId)                              { m_nId = nId; return true; };
	virtual i32                         GetBreakability() const                    { return m_physParams.iBreakability; }
	virtual i32                         GetHitpoints() const                       { return (i32)m_physParams.hit_points; }
	...
	virtual SBreakageParticles*    GetBreakageParticles(tukk sType, bool bLookInDefault = true);
	//////////////////////////////////////////////////////////////////////////
};
...
//////////////////////////////////////////////////////////////////////////
CMaterialSurfaceType::SBreakageParticles* CMaterialSurfaceType::GetBreakageParticles(tukk sType, bool bLookInDefault)
{
	for (i32 i = 0, num = m_breakageParticles.size(); i < num; i++)
	{
	...
	{
		for (i32 i = 0, num = g_pDefaultSurfaceType->m_breakageParticles.size(); i < num; i++)
		{
...
{
	CSurfaceTypeUpr* pMgr = (CSurfaceTypeUpr*)(gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr());
	pMgr->LoadSurfaceTypes();

}

//////////////////////////////////////////////////////////////////////////
// SurfaceUpr implementation.
//////////////////////////////////////////////////////////////////////////
CSurfaceTypeUpr::CSurfaceTypeUpr()
{
...
//////////////////////////////////////////////////////////////////////////
void CSurfaceTypeUpr::RemoveAll()
{
...
void CSurfaceTypeUpr::RegisterAllDefaultTypes()
{
...
CMaterialSurfaceType* CSurfaceTypeUpr::RegisterDefaultType(tukk szName)
{
...
void CSurfaceTypeUpr::ResetSurfaceTypes()
{
	//Remove all entries
	RemoveAll();
...
//////////////////////////////////////////////////////////////////////////
ISurfaceType* CSurfaceTypeUpr::GetSurfaceTypeByName(tukk sName, tukk sWhy, bool warn)
{
...
//////////////////////////////////////////////////////////////////////////
ISurfaceType* CSurfaceTypeUpr::GetSurfaceTypeFast(i32 nSurfaceId, tukk sWhy)
{
...
//////////////////////////////////////////////////////////////////////////
ISurfaceType* CSurfaceTypeUpr::GetSurfaceType(i32 nSurfaceId, tukk sWhy)
{
	return GetSurfaceTypeFast(nSurfaceId, sWhy);
}

//////////////////////////////////////////////////////////////////////////
ISurfaceTypeEnumerator* CSurfaceTypeUpr::GetEnumerator()
{
	return new CSurfaceTypeEnumerator<NameToSurfaceMap>(&m_nameToSurface);
}

//////////////////////////////////////////////////////////////////////////
bool CSurfaceTypeUpr::RegisterSurfaceType(ISurfaceType* pSurfaceType, bool bDefault)
{
	assert(pSurfaceType);

	i32 nSurfTypeId = pSurfaceType->GetId();
...
//////////////////////////////////////////////////////////////////////////
void CSurfaceTypeUpr::UnregisterSurfaceType(ISurfaceType* pSurfaceType)
{
	assert(pSurfaceType);
...
//////////////////////////////////////////////////////////////////////////
void CSurfaceTypeUpr::LoadSurfaceTypes()
{
	...
	for (i32 i = 0, nChilds = root->getChildCount(); i < nChilds; i++)
	{
		SLICE_AND_SLEEP();
		...
		tukk sName = matNode->getAttr("name");
		NameToSurfaceMap::iterator it = m_nameToSurface.find(CONST_TEMP_STRING(sName));
		...
		if (physNode)
		{
			...
			if (collTypeStr.length())
			{
				...
				for (tukk ptr = collTypeStr.c_str(); *ptr; )
				{
					for (; *ptr && (u8)(*ptr - '0') > 9u; ptr++)
						;
					tukk ptr0 = ptr;
					i32 num = 0;
					for (; (u8)(*ptr - '0') <= 9u; (num *= 10) += *ptr++ - '0')
						;
					if (ptr > ptr0)
				...
				tukk collNames[] = { "default", "all", "player", "vehicle", "explosion", "ray", "float", "water" };
				i32k collVals[] = { geom_colltype0, geom_collides, geom_colltype_player, geom_colltype_vehicle, geom_colltype_explosion, geom_colltype_ray, geom_floats, geom_floats };
				for (i32 j = 0; j < DRX_ARRAY_COUNT(collVals); j++)
					if (tukk name = strstr(collTypeStr.c_str(), collNames[j]))
		...
		if (break2dNode)
		{
			...
			if (break2dParams.broken_mtl.length())
				gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(break2dParams.broken_mtl, false);
...

		{
			...
			for (i32 n = 0; n < matNode->getChildCount(); n++)
			{
