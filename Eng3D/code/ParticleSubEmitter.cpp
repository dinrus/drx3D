...
CParticleSubEmitter::CParticleSubEmitter(CParticleSource* pSource, CParticleContainer* pCont)
	...
	, m_pContainer(pCont)
	, m_pSource(pSource)
	, m_startAudioTriggerId(DrxAudio::InvalidControlId)
	, m_stopAudioTriggerId(DrxAudio::InvalidControlId)
	, m_audioParameterId(DrxAudio::InvalidControlId)
	, m_pIAudioObject(nullptr)
	, m_currentAudioOcclusionType(DrxAudio::EOcclusionType::Ignore)
...
void CParticleSubEmitter::Initialize(float fAge)
{
	...
	// Reseed randomness.
	m_ChaosKey = CChaosKey(drx_random_uint32());
	m_fActivateAge = fAge;
			...
			if (fCount > 0.f)
			{
				...
				if (fParticleLife > 0.f)
				{
					...
					else
					{
						...
						for (i32 nEmit = int_round(fCount); nEmit > 0; nEmit--)
						{
...
void CParticleSubEmitter::UpdateForce()
{
	...
	if (fAge >= m_fStartAge && fAge <= GetParticleStopAge())
	{
		...
		if (!m_pForce)
		{
			IGeometry* pGeom = m_pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::box::type, &geomBox);
...
void CParticleSubEmitter::UpdateAudio()
{
	...
	if (spawnParams.bEnableAudio && GetCVars()->e_ParticlesAudio > 0)
	{
		...
		{
			if (m_audioParameterId != DrxAudio::InvalidControlId && params.fSoundFXParam(VMAX, VMIN) < 1.0f)
			{
		...
		else if (m_bExecuteAudioTrigger)
		{
			...
			if (fAge >= fAge0 && fAge <= fAge1)
			{
				...
				{
					m_startAudioTriggerId = DrxAudio::StringToId(params.sStartTrigger.c_str());
				}

				if (!params.sStopTrigger.empty())
				{
					m_stopAudioTriggerId = DrxAudio::StringToId(params.sStopTrigger.c_str());
				}

				if (m_startAudioTriggerId != DrxAudio::InvalidControlId || m_stopAudioTriggerId != DrxAudio::InvalidControlId)
				{
					DrxAudio::SCreateObjectData const objectData("ParticleSubEmitter", spawnParams.occlusionType, GetEmitTM(), INVALID_ENTITYID, true);
					m_pIAudioObject = gEnv->pAudioSystem->CreateObject(objectData);
					m_currentAudioOcclusionType = spawnParams.occlusionType;

					if (!spawnParams.audioRtpc.empty())
					{
						m_audioParameterId = DrxAudio::StringToId(spawnParams.audioRtpc.c_str());
						float const value = params.fSoundFXParam(m_ChaosKey, GetStrength(0.0f, params.eSoundControlTime));
						m_pIAudioObject->SetParameter(m_audioParameterId, value);
					}

					// Execute start trigger immediately.
					if (m_startAudioTriggerId != DrxAudio::InvalidControlId)
					{
						m_pIAudioObject->ExecuteTrigger(m_startAudioTriggerId);
					}
				}
...
i32 CParticleSubEmitter::EmitParticle(SParticleUpdateContext& context, const EmitParticleData& data_in, float fAge, QuatTS* plocPreTransform)
{
	const ResourceParticleParams& params = GetParams();
	...
	if (data.pStatObj && !data.pPhysEnt && params.eGeometryPieces != params.eGeometryPieces.Whole)
	{
		...
		i32 nPieces = 0;
		for (i32 i = pParentObj->GetSubObjectCount() - 1; i >= 0; i--)
		{
			if (IStatObj::SSubObject* pSub = GetSubGeometry(pParentObj, i))
			{
				if (params.eGeometryPieces == params.eGeometryPieces.AllPieces || (nPieces++ == 0 || drx_random(0, nPieces - 1) == 0))
				{
					// Emit this piece
	...
	DRX_ALIGN(128) char buffer[sizeof(CParticle)];
...
void CParticleSubEmitter::DeactivateAudio()
{
	if (m_pIAudioObject != nullptr)
	{
		if (m_audioParameterId != DrxAudio::InvalidControlId)
		{
			ResourceParticleParams const& params = GetParams();
			float const value = params.fSoundFXParam(m_ChaosKey, GetStrength(0.0f, params.eSoundControlTime));
			m_pIAudioObject->SetParameter(m_audioParameterId, value);
		...
		if (m_stopAudioTriggerId != DrxAudio::InvalidControlId)
		{
			m_pIAudioObject->SetTransformation(GetEmitTM());
		...
		{
			DRX_ASSERT(m_startAudioTriggerId != DrxAudio::InvalidControlId);
...
ResourceParticleParams const& CParticleSubEmitter::GetParams() const
{
