...
void SPhysForces::Add(SPhysForces const& other, u32 nEnvFlags)
{
...
//////////////////////////////////////////////////////////////////////////
static u32 GetAreaFlags(IPhysicalEntity* pEntity, pe_params_area& parea, pe_params_buoyancy& pbuoy, Plane* pWaterPlane = 0)
{
	u32 flags = 0;
...
static i32 StaticOnAreaChange(const EventPhys* pEvent)
{
...
void SWorldPhysEnviron::Update()
{
	...
	for (IPhysicalEntity* pArea = 0; pArea = gEnv->pPhysicalWorld->GetNextArea(pArea); )
	{
		...
		u32 nAreaFlags = GetAreaFlags(pArea, parea, pbuoy, &plWater);
		...
		i32 nStatus = pArea->GetStatus(&sarea);
		if (!nStatus)
			continue;
...
void SPhysEnviron::Update(SPhysEnviron const& envSource, AABB const& box, bool bIndoors, u32 nFlags, bool bNonUniformAreas, ukk pObjectSkip)
{
			...
			if (area.m_nFlags & nFlags)
			{
				...
				if (!bIndoors || !area.m_bOutdoorOnly)
				{
					...
					if (area.m_bounds.IsIntersectBox(box))
					{
						...
						i32 nStatus = area.m_pArea->GetStatus(&sarea);
						if (!nStatus)
							continue;
...
void SPhysEnviron::GetForces(SPhysForces& forces, AABB const& bb, u32 nFlags, bool bAverage) const
{
	DRX_PROFILE_FUNCTION(PROFILE_PARTICLE);
...
bool SPhysEnviron::PhysicsCollision(ray_hit& hit, Vec3 const& vStart, Vec3 const& vEnd, float fRadius, u32 nEnvFlags, IPhysicalEntity* pTestEntity)
{
	DRX_PROFILE_FUNCTION(PROFILE_PARTICLE);
...
void SPhysEnviron::SArea::GetForces(SPhysForces& forces, AABB const& bb, u32 nFlags, bool bAverage) const
{
	nFlags &= m_nFlags;
