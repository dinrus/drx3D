...
i32 CObjUpr::m_nUpdateStreamingPrioriryRoundId = 1;
i32 CObjUpr::m_nUpdateStreamingPrioriryRoundIdFast = 1;
i32 CObjUpr::s_nLastStreamingMemoryUsage = 0;

struct CObjUpr_Cmp_Streamable_Priority
{
	...
	ILINE bool operator()(const SStreamAbleObject& v1, const SStreamAbleObject& v2) const
	{
		...
		i32 MemUsage0 = v1.GetStreamableContentMemoryUsage();
		i32 MemUsage1 = v2.GetStreamableContentMemoryUsage();
...
void CObjUpr::RegisterForStreaming(IStreamable* pObj)
{
...
void CObjUpr::UnregisterForStreaming(IStreamable* pObj)
{
...
void CObjUpr::UpdateObjectsStreamingPriority(bool bSyncLoad, const SRenderingPassInfo& passInfo)
{
	...
	static i32 nOld_e_StaticInstancing = GetCVars()->e_StaticInstancing;
	...
	CVisAreaUpr* pVisAreaMgr = GetVisAreaUpr();
	...
	if (bSyncLoad || (passInfo.GetFrameID() & 3) || GetFloatCVar(e_StreamCgfFastUpdateMaxDistance) == 0)
	{
		...
		{
			DRX_PROFILE_REGION(PROFILE_3DENGINE, "UpdateObjectsStreamingPriority_Init");
			...
			for (size_t ppIdx = 0, ppCount = nPrecachePoints; ppIdx != ppCount; ++ppIdx)
			{
				...
				if (CVisArea* pRoot0 = pCurArea)
				{
					...
					{
						for (i32 v = 0; v < m_tmpAreas0.Count(); v++)
						{
				...
				else if (GetVisAreaUpr())
				{
					...
					// find portals around
					m_tmpAreas0.Clear();
					GetVisAreaUpr()->MakeActiveEntransePortalsList(NULL, m_tmpAreas0, NULL, passInfo);

					// make list of areas for streaming
					m_tmpAreas1.Clear();
					for (i32 p = 0; p < m_tmpAreas0.Count(); p++)
					...
					for (i32 v = 0; v < m_tmpAreas1.Count(); v++)
					{
...

		{
			...
			{
				DRX_PROFILE_REGION(PROFILE_3DENGINE, "UpdateObjectsStreamingPriority_MarkNodes");
	...
	{
		DRX_PROFILE_REGION(PROFILE_3DENGINE, "UpdateObjectsStreamingPriority_Mark_NEAR_Nodes");

		PodArray<COctreeNode*> fastStreamingNodeStack;
		i32k nVisAreaRecursion = min(GetCVars()->e_StreamPredictionMaxVisAreaRecursion, 2);
...
static bool AreTexturesStreamed(IMaterial* pMaterial, float fMipFactor)
{
	...
	if (pMaterial)
	{
		...
		{
			for (i32 iSlot = 0; iSlot < EFTT_MAX; ++iSlot)
			{
...
void CObjUpr::CheckTextureReadyFlag()
{
	FUNCTION_PROFILER_3DENGINE;
...
void CObjUpr::ProcessObjectsStreaming(const SRenderingPassInfo& passInfo)
{
	FUNCTION_PROFILER_3DENGINE;
	...
	{
		DrxWarning(VALIDATOR_MODULE_3DENGINE, VALIDATOR_ERROR, "ProcessObjectsStreaming invoked without a following ProcessObjectsStreaming_Finish, please check your update logic");
	}
...
void CObjUpr::ProcessObjectsStreaming_Impl(bool bSyncLoad, const SRenderingPassInfo& passInfo)
{
...
void CObjUpr::ProcessObjectsStreaming_Sort(bool bSyncLoad, const SRenderingPassInfo& passInfo)
{
	i32 nNumStreamableObjects = m_arrStreamableObjects.Count();
	...
	{
		DRX_PROFILE_REGION(PROFILE_3DENGINE, "ProcessObjectsStreaming_Sort");
		...
		if (bSyncLoad)
		{
			...
			for (i32 i = 0; i < nNumStreamableObjects; i++)
			{
				...
				i32 nOffset = (i32)(GetPak()->GetFileOffsetOnMedia(fileName.c_str()) / 1024);
		...
		else
		{
			...
			i32 nRoundId = CObjUpr::m_nUpdateStreamingPrioriryRoundId - 1;
			...
			for (i32 i = 0; i < nNumStreamableObjects; i++)
			{
				...
				for (i32 nRoundIdx = 0; nRoundIdx < 2; nRoundIdx++)
				{
		...
		std::sort(&arrStreamableObjects[0], &arrStreamableObjects[nNumStreamableObjects], CObjUpr_Cmp_Streamable_Priority());

		fLastTime = fTime;
	}
}

void CObjUpr::ProcessObjectsStreaming_Release()
{
	DRX_PROFILE_REGION(PROFILE_3DENGINE, "ProcessObjectsStreaming_Release");
	i32 nMemoryUsage = 0;

	i32 nNumStreamableObjects = m_arrStreamableObjects.Count();
	SStreamAbleObject* arrStreamableObjects = nNumStreamableObjects ? &m_arrStreamableObjects[0] : NULL;

	for (i32 nObjId = 0; nObjId < nNumStreamableObjects; nObjId++)
	{
		...
		if (!bUnload && GetCVars()->e_StreamCgfDebug == 4)
			if (rObj.GetStreamAbleObject()->m_arrUpdateStreamingPrioriryRoundInfo[0].nRoundId < (CObjUpr::m_nUpdateStreamingPrioriryRoundId - 8))
				bUnload = true;
...
void CObjUpr::ProcessObjectsStreaming_InitLoad(bool bSyncLoad)
{
	DRX_PROFILE_REGION(PROFILE_3DENGINE, "ProcessObjectsStreaming_InitLoad");

	i32 nMaxInProgress = GetCVars()->e_StreamCgfMaxTasksInProgress;
	i32 nMaxToStart = GetCVars()->e_StreamCgfMaxNewTasksPerUpdate;
	i32 nMaxMemUsage = GetCVars()->e_StreamCgfPoolSize * 1024 * 1024;
	i32 nNumStreamableObjects = m_arrStreamableObjects.Count();
	SStreamAbleObject* arrStreamableObjects = nNumStreamableObjects ? &m_arrStreamableObjects[0] : NULL;

	i32 nMemoryUsage = 0;
	i32 nInProgress = 0;
	i32 nInProgressMem = 0;
	i32 nStarted = 0;
	...
	for (i32 nObjId = 0; (nObjId < nNumStreamableObjects) && ((nInProgress < nMaxInProgress && (nInProgressMem < static_cast<i32>(poolLimit) || !poolLimit) && nStarted < nMaxToStart) || bSyncLoad); nObjId++)
	{
		const SStreamAbleObject& rObj = arrStreamableObjects[nObjId];
		IStreamable* pStatObj = rObj.GetStreamAbleObject();

		i32 size = rObj.GetStreamableContentMemoryUsage();
		if (poolLimit && poolLimit <= (size_t)(nInProgressMem + size))
		{
			...
			// streamed in. This has to change after drxsis2 - for now, this will include a white list of meshes that need to be
			// loaded for drxsis2 in the prism2 level.
			if ((size_t)poolLimit <= (size_t)size)
			{
				string name;
				pStatObj->GetStreamableName(name);
				DrxLogAlways("[WARNING] object '%s' skipped because too large (%d kb (>= %" PRISIZE_T " kb limit))", name.c_str(), size / 1024, poolLimit / 1024);
				continue;
			}
...
void CObjUpr::ProcessObjectsStreaming_Finish()
{
	...
	DRX_PROFILE_REGION(PROFILE_3DENGINE, "ProcessObjectsStreaming_Finish");
	bool bSyncLoad = Get3DEngine()->IsStatObjSyncLoad();

	{
		...
		for (size_t nId = 0; nId < m_arrStreamableToLoad.size(); nId++)
		{
			...
			i32 size = pStatObj->GetStreamableContentMemoryUsage();
...
#ifdef OBJMAN_STREAM_STATS
void CObjUpr::ProcessObjectsStreaming_Stats(const SRenderingPassInfo& passInfo)
{
	...
	i32 nCurrentFrameId = passInfo.GetMainFrameID();
	...
	for (i32 nObjId = 0; nObjId < nNumStreamableObjects; nObjId++)
	{
		...
		{
			i32 framesSinceLastUse = nCurrentFrameId - pObj->GetLastDrawMainFrameId();
			...
			if (framesSinceLastUse < 2)
			{
				if (!pObj->m_nStatsInUse)
				{
					...
					IF_UNLIKELY (nBegunUse == DRX_ARRAY_COUNT(pBegunUse))
					{
			...
			else
			{
				if (pObj->m_nStatsInUse)
				{
					...
					IF_UNLIKELY (nEndUse == DRX_ARRAY_COUNT(pEndUse))
					{
...
void CObjUpr::GetObjectsStreamingStatus(I3DEngine::SObjectsStreamingStatus& outStatus)
{
	...
	for (LoadedObjects::iterator it = m_lstLoadedObjects.begin(); it != m_lstLoadedObjects.end(); ++it)
	{
		...
		for (i32 l = 0; l < MAX_STATOBJ_LODS_NUM; l++)
	...
	for (i32 nObjId = 0; nObjId < m_arrStreamableObjects.Count(); nObjId++)
	{
		...
		if (rStreamAbleObject.GetStreamAbleObject()->m_arrUpdateStreamingPrioriryRoundInfo[0].nRoundId >= (CObjUpr::m_nUpdateStreamingPrioriryRoundId - 4))
			outStatus.nMemRequired += rStreamAbleObject.GetStreamableContentMemoryUsage();
	}
}

void CObjUpr::PrecacheStatObjMaterial(IMaterial* pMaterial, const float fEntDistance, IStatObj* pStatObj, bool bFullUpdate, bool bDrawNear)
{
	//  FUNCTION_PROFILER_3DENGINE;
...
void CObjUpr::PrecacheStatObj(CStatObj* pStatObj, i32 nLod, const Matrix34A& statObjMatrix, IMaterial* pMaterial, float fImportance, float fEntDistance, bool bFullUpdate, bool bHighPriority)
{
	if (!pStatObj)
		return;
...
void CObjUpr::UpdateRenderNodeStreamingPriority(IRenderNode* pObj, float fEntDistanceReal, float fImportanceFactor, bool bFullUpdate, const SRenderingPassInfo& passInfo, bool bHighPriority)
{
	//  FUNCTION_PROFILER_3DENGINE;
	...
	i32 nLod = CObjUpr::GetObjectLOD(pObj, fEntDistanceReal);
	...
	if (const auto pTempData = pObj->m_pTempData.load())
	{
		if (GetFloatCVar(e_StreamCgfGridUpdateDistance) != 0.0f || GetFloatCVar(e_StreamPredictionAhead) != 0.0f || GetFloatCVar(e_StreamPredictionMinFarZoneDistance) != 0.0f)
		{
			...
			pTempData->userData.nWantedLod = CObjUpr::GetObjectLOD(pObj, fDistanceToCam);
		}
		else
		{
