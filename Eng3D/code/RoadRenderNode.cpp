...
STRUCT_VAR_INFO(numVertices, TYPE_INFO(u32))
STRUCT_VAR_INFO(numIndices, TYPE_INFO(u32))
STRUCT_VAR_INFO(numTangents, TYPE_INFO(u32))

STRUCT_VAR_INFO(physicsGeometryCount, TYPE_INFO(u32))

STRUCT_VAR_INFO(sourceVertexCount, TYPE_INFO(u32))
...
void CRoadRenderNode::SetVertices(const Vec3* pVertsAll, i32 nVertsNumAll,
                                  ...
                                  float fTexCoordBeginGlobal, float fTexCoordEndGlobal)
{
	...
	const float fNodeStart = static_cast<float>(static_cast<i32>(fTexCoordBegin));
...
void CRoadRenderNode::Compile() PREFAST_SUPPRESS_WARNING(6262) //function uses > 32k stack space
{
	...
	if (m_bRebuildFull)
	{
		...
		i32 nVertsNumAll = m_arrVerts.Count();
		...
		i32k nMaxVerticesToMerge = 1024 * 32; // limit memory usage
		...
		// for every trapezoid
		for (i32 nVertId = 0; nVertId <= nVertsNumAll - 4; nVertId += 2)
		{
			...
			// make indices
			i32 dx = i32((x2 - x1) / unitSize);
			i32 dy = i32((y2 - y1) / unitSize);
			...
			for (i32 x = 0; x < dx; x++)
			{
				...
				{
					i32 nIdx0 = (x * (dy + 1) + y);
					i32 nIdx1 = (x * (dy + 1) + y + (dy + 1));
					i32 nIdx2 = (x * (dy + 1) + y + 1);
					i32 nIdx3 = (x * (dy + 1) + y + 1 + (dy + 1));
			...
			// clip triangles
			i32 nOrigCount = s_tempIndices.Count();
			...
			if (m_bPhysicalize)
			{
				...
				for (i32 j = 0; j < s_tempIndices.Count(); j++)
				{
			...
			for (i32 i = 0; i < s_tempVertexPositions.Count(); i++)
			{
				SVF_P3F_C4B_T2S tmp;
				...
				tmp.color.bcolor[3] = u8(255.f * fAlpha);
			...
			// shift indices
			for (i32 i = 0; i < s_tempIndices.Count(); i++)
				s_tempIndices[i] += m_dynamicData.vertices.size();

			if (m_dynamicData.vertices.size() + s_tempVertices.Count() > nMaxVerticesToMerge)
			{
				Warning("Road object is too big, has to be split into several smaller parts (pos=%d,%d,%d)", (i32)m_serializedData.worldSpaceBBox.GetCenter().x, (i32)m_serializedData.worldSpaceBBox.GetCenter().y, (i32)m_serializedData.worldSpaceBBox.GetCenter().z);
				return;
			}
	...
	if (m_dynamicData.indices.Count() && GetRenderer())
	{
		...
		m_pRenderMesh->SetChunk((m_pMaterial != NULL) ? (IMaterial*)m_pMaterial : gEnv->p3DEngine->GetMaterialUpr()->GetDefaultMaterial(),
		...
		if (m_bPhysicalize)
		{
			...
			if (m_pMaterial && m_pPhysEnt)
			{
				...
				for (i32 i = 0, numParts = m_dynamicData.physicsGeometry.Count(); i < numParts; i++)
				{
...
void CRoadRenderNode::SetSortPriority(u8 sortPrio)
{
	m_sortPrio = sortPrio;
}
...
void CRoadRenderNode::Render(const SRendParams& RendParams, const SRenderingPassInfo& passInfo)
{
	...
	if (GetObjUpr()->AddOrCreatePersistentRenderObject(m_pTempData.load(), pObj, nullptr, objMat, passInfo))
		return;
...
bool CRoadRenderNode::ClipTriangle(PodArray<Vec3>& lstVerts, PodArray<vtx_idx>& lstInds, i32 nStartIdxId, Plane* pPlanes)
{
...
void CRoadRenderNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void CRoadRenderNode::OnTerrainChanged()
{
	...
	for (i32 i = 0, nVertsNum = m_pRenderMesh->GetVerticesCount(); i < nVertsNum; i++)
	{
...
void CRoadRenderNode::GetClipPlanes(Plane* pPlanes, i32 nPlanesNum, i32 nVertId)
{
	const Vec3* pVerts = &m_arrVerts[nVertId];
