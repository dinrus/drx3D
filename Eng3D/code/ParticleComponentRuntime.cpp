	...
	, m_chaosV(0)
{
	...
	{
		m_pGpuRuntime = gEnv->pRenderer->GetGpuParticleUpr()->CreateParticleContainer(
...
void CParticleComponentRuntime::UpdateAll()
{
	...
	u32 emitterSeed = m_pEmitter->GetCurrentSeed();
	u32 componentId = m_pComponent->GetComponentId() << 16;
...
void CParticleComponentRuntime::ReparentParticles(TConstArray<TParticleId> swapIds)
{
	...
	for (TParticleId particleId = 0; particleId != m_container.GetNumParticles(); ++particleId)
	{
		...
		{
			DRX_PFX2_ASSERT(parentId < swapIds.size());   // this particle was pointing to the wrong parentId already
...
void CParticleComponentRuntime::RemoveParticles()
{
	...
	u32k numParticles = m_container.GetNumParticles();
...
void CParticleComponentRuntime::CalculateBounds()
{
	...
	const TParticleGroupId lastParticleGroupId { lastParticleId & ~(DRX_PFX2_PARTICLESGROUP_STRIDE - 1) };
	...
	DRX_PFX2_ASSERT(m_bounds.GetRadius() < 1000000.f);
}
...
void CParticleComponentRuntime::DebugStabilityCheck()
{
	...
	if (m_container.HasNewBorns())
	{
		for (auto particleId : SpawnedRange())
		{
			...
			DRX_PFX2_ASSERT(parentIds.Load(particleId) != gInvalidId);      // recently spawn particles are not supposed to be orphan
		}
	}
	...
	{
		TParticleId parentId = parentIds.Load(particleId);
		DRX_PFX2_ASSERT(parentId < parentCount || parentId == gInvalidId);    // this particle is not pointing to the correct parent
	}

	for (auto instance : m_subInstances)
	{
		DRX_PFX2_ASSERT(instance.m_parentId < parentCount);    // this instance is not pointing to the correct parent
	}
#endif
}
