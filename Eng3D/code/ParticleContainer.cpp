...
inline void move_raw_array_elem(T* pDest, T* pSrc)
{
	T val = *pSrc;
	if (pSrc < pDest)
		memmove(pSrc, pSrc + 1, (tuk)pDest - (tuk)(pSrc + 1));
	else
		memmove(pDest + 1, pDest, (tuk)pSrc - (tuk)pDest);
	*pDest = val;
}
...
u32 CParticleContainer::GetEnvironmentFlags() const
{
...
void CParticleContainer::OnEffectChange()
{
	...
	i32 nPrevSteps = m_nHistorySteps;
...
{
	if (uk pMem = m_Emitters.push_back_new())
	{
...
CParticle* CParticleContainer::AddParticle(SParticleUpdateContext& context, const CParticle& part)
{
	...
	if (params.Connection)
	{
		...
		i32 nSequence = part.GetEmitter()->GetSequence();
		for (auto& p : m_Particles)
		{
...
void CParticleContainer::ComputeStaticBounds(AABB& bb, bool bWithSize, float fMaxLife)
{
	...
	if (GetTarget(target, (CParticleSubEmitter*) GetDirectEmitter()))
	{
...
void CParticleContainer::ComputeUpdateContext(SParticleUpdateContext& context, float fUpdateTime)
{
	...
	// Sorting support.
	context.nSortQuality = max((i32)params.nSortQuality, GetCVars()->e_ParticlesSortQuality);
...
void CParticleContainer::UpdateParticleStates(SParticleUpdateContext& context)
{
	...
	if ((context.nEnvFlags & REN_SORT) && context.nSortQuality > 0)
	{
		...
		i32 nMaxParticles = GetMaxParticleCount(context);
...
void CParticleContainer::Render(SRendParams const& RenParams, SPartRenderParams const& PRParams, const SRenderingPassInfo& passInfo)
{
	...
	u32 nRenderFlags = GetEnvironmentFlags() & PRParams.m_nRenFlags;
	...
	else if (nRenderFlags & REN_SPRITE)
	{
		...
		SAddParticlesToSceneJob& job = CParticleUpr::Instance()->GetParticlesToSceneJob(passInfo);
		...
		i32 passId = passInfo.IsShadowPass() ? 1 : 0;
		i32 passMask = BIT(passId);
...
CRenderObject* CParticleContainer::CreateRenderObject(uint64 nObjFlags, const SRenderingPassInfo& passInfo)
{
	...
	pRenderObject->m_RState = u8(nObjFlags);
	...
	IF(!!pParams->fHeatScale, 0)
	{
		...
		u32 nHeatAmount = pParams->fHeatScale.GetStore();
...
float CParticleContainer::GetEmitterLife() const
{
	...
	if (!m_pParentContainer)
	{
		...
		{
			if (!m_Emitters.empty())
				fEmitterLife = min(fEmitterLife, reinterpret_cast<const CParticleSubEmitter*>(m_Emitters.front())->GetStopAge());
			else
				fEmitterLife = 0.f;
		}
	}
...
i32 CParticleContainer::GetMaxParticleCount(const SParticleUpdateContext& context) const
{
	const ResourceParticleParams& params = GetParams();

	// Start count is existing number of particles plus deferred
	i32 nCount = m_Particles.size() + m_DeferredEmitParticles.size();
	...
	if (params.eGeometryPieces == params.eGeometryPieces.AllPieces && params.pStatObj != 0)
	{
		...
		i32 nPieces = GetSubGeometryCount(params.pStatObj);
	...
	// Account for pulsing
	i32 nEmitCount = int_ceil(fEmitCount) * m_Emitters.size();
...
void CParticleContainer::UpdateContainerLife(float fAgeAdjust)
{
	...
	if (CParticleSubEmitter* pDirectEmitter = (CParticleSubEmitter*) GetDirectEmitter())
...
void CParticleContainer::GetMemoryUsage(IDrxSizer* pSizer) const
{
