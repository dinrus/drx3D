...
void CStatObj::ShutDown()
{
	...
	for (i32 n = 0; n < m_arrPhysGeomInfo.GetGeomCount(); n++)
		if (m_arrPhysGeomInfo[n])
		{
			if (m_arrPhysGeomInfo[n]->pGeom->GetForeignData() == (uk )this)
				m_arrPhysGeomInfo[n]->pGeom->SetForeignData(0, 0);
			GetPhysicalWorld()->GetGeomUpr()->UnregisterGeometry(m_arrPhysGeomInfo[n]);
		}
	m_arrPhysGeomInfo.m_array.clear();
		...
		for (i32 i = 0; i < MAX_STATOBJ_LODS_NUM; i++)
			if (m_pLODs[i])
			{
				if (m_pLODs[i]->m_pParentObject)
					GetObjUpr()->UnregisterForStreaming(m_pLODs[i]->m_pParentObject);
				else
					GetObjUpr()->UnregisterForStreaming(m_pLODs[i]);
	...
	for (size_t i = 0; i < m_subObjects.size(); i++)
	{
		...
		if (pChildObj)
		{
			...
			GetObjUpr()->UnregisterForStreaming(pChildObj);
			pChildObj->Release();
		}
	}
	m_subObjects.clear();
	...
	if (m_pParentObject && !m_pParentObject->m_subObjects.empty())
	{
		...
		for (i32 i = 0, num = m_pParentObject->m_subObjects.size(); i < num; i++)
		{
...
//////////////////////////////////////////////////////////////////////////
i32 CStatObj::Release()
{
	// Has to be thread safe, as it can be called by a worker thread for deferred plane breaks
	i32 newRef = DrxInterlockedDecrement(&m_nUsers);
	if (newRef <= 1)
	{
		if (m_pParentObject && m_pParentObject->m_nUsers <= 0)
		{
			GetObjUpr()->CheckForGarbage(m_pParentObject);
		}
		if (newRef <= 0)
		{
			GetObjUpr()->CheckForGarbage(this);
		}
	}
	return newRef;
}
...
//////////////////////////////////////////////////////////////////////////
uk CStatObj::operator new(size_t size)
{
	CObjUpr* pObjUpr = GetObjUpr();
	assert(size == sizeof(CStatObj));
	return pObjUpr->AllocateStatObj();
}

void CStatObj::operator delete(uk pToFree)
{
	CObjUpr* pObjUpr = GetObjUpr();
	pObjUpr->FreeStatObj((CStatObj*)pToFree);
}
...
void CStatObj::CalcRadiuses()
{
	...
	{
		Error("CStatObj::CalcRadiuses: Invalid bbox, Имя файла: %s", m_szFileName.c_str());
		m_AABB.min.zero();
		m_AABB.max.zero();
	}
...
void CStatObj::MakeRenderMesh()
{
	...
	SMeshBoneMapping_u16* const pBoneMap = pMesh->m_pBoneMapping;
...
///////////////////////////////////////////////////////////////////////////////////////
Vec3 CStatObj::GetHelperPos(tukk szHelperName)
{
...
///////////////////////////////////////////////////////////////////////////////////////
const Matrix34& CStatObj::GetHelperTM(tukk szHelperName)
{
	SSubObject* pSubObj = FindSubObject(szHelperName);
...
bool CStatObj::IsSameObject(tukk szFileName, tukk szGeomName)
{
	// cmp object names
	if (szGeomName)
	{
	...
	tuk pszDest = szFileNameNorm;
	tukk pszSource = szFileName;
	while (*pszSource)
	{
		if (*pszSource == '\\')
			*pszDest++ = '/';
...
void CStatObj::GetMemoryUsage(IDrxSizer* pSizer) const
{
	{
	...
	if (m_pLODs)
		for (i32 i = 1; i < MAX_STATOBJ_LODS_NUM; i++)
		{
...
//////////////////////////////////////////////////////////////////////////
void CStatObj::SetLodObject(i32 nLod, IStatObj* pObj)
{
	...
	if (pLod && !bLodCompound)
	{
		...
		i32 nPrevLodMatIds = m_nRenderMatIds;
		i32 nPrevLodTris = m_nLoadedTrisCount;
		...
		{
			if ((i32)m_nMaxUsableLod < nLod)
				m_nMaxUsableLod = nLod;
		}
		else
		{
			i32 min_tris = GetCVars()->e_LodMinTtris;
			if (((pLod->m_nLoadedTrisCount >= min_tris || nPrevLodTris >= (3 * min_tris) / 2)
			     || pLod->m_nRenderMatIds < nPrevLodMatIds) && nLod > (i32)m_nMaxUsableLod)
			{
				m_nMaxUsableLod = nLod;
			}
		}
		...
		if (nLod + 1 > (i32)m_nLoadedLodsNum)
			m_nLoadedLodsNum = nLod + 1;

		// When assigning lod to child object.
		if (m_pParentObject)
		{
			if (nLod + 1 > (i32)m_pParentObject->m_nLoadedLodsNum)
...
//////////////////////////////////////////////////////////////////////////
IStatObj* CStatObj::GetLodObject(i32 nLodLevel, bool bReturnNearest)
{
	if (nLodLevel < 1)
	{
		return this;
	...
	if (nLodLevel < MAX_STATOBJ_LODS_NUM)
	{
		...
		if (bReturnNearest && !pLod)
		{
			...
			for (i32 lod = nLodLevel + 1; lod < MAX_STATOBJ_LODS_NUM; lod++)
			{
				if (m_pLODs[lod])
				{
					pLod = m_pLODs[lod];
...
IStatObj* CStatObj::Clone(bool bCloneGeometry, bool bCloneChildren, bool bMeshesOnly)
{
	...
	for (i32 i = 0; i < m_arrPhysGeomInfo.GetGeomCount(); i++)
	{
		...
		if (pNewObj->m_arrPhysGeomInfo[i])
			GetPhysicalWorld()->GetGeomUpr()->AddRefGeometry(pNewObj->m_arrPhysGeomInfo[i]);
	}
	...
	i32 numSubObj = (i32)m_subObjects.size();
	if (bMeshesOnly)
	{
		numSubObj = 0;
		for (size_t i = 0; i < m_subObjects.size(); i++)
...
IIndexedMesh* CStatObj::GetIndexedMesh(bool bCreateIfNone)
{
	...
	else if (m_pRenderMesh && bCreateIfNone)
	{
		...
		i32 i, j, i0 = pMesh->m_subsets[0].nFirstVertId + pMesh->m_subsets[0].nNumVerts;
...
//////////////////////////////////////////////////////////////////////////
static bool SubObjectsOfCompoundHaveLOD(const CStatObj* pStatObj, i32 nLod)
{
	i32 numSubObjects = pStatObj->GetSubObjectCount();
	for (i32 i = 0; i < numSubObjects; ++i)
	{
...
//////////////////////////////////////////////////////////////////////////
void CStatObj::MergeSubObjectsRenderMeshes(bool bFromStreaming, CStatObj* pLod0, i32 nLod)
{
	if (m_bUnmergable)
		return;
	...
	i32 nSubObjCount = (i32)pLod0->m_subObjects.size();
	...
	for (i32 s = 0; s < nSubObjCount; s++)
	{
		...
		if (pSubObj->pStatObj && pSubObj->nType == STATIC_SUB_OBJECT_MESH)
		{
			...
			if (pStatObj)
			{
				DrxAutoCriticalSection lock(pStatObj->m_streamingMeshLock);
...
bool CStatObj::IsMatIDReferencedByObj(u16 matID)
{
	//Check root obj
...
bool CStatObj::CanMergeSubObjects()
{
	...
	i32 nSubMeshes = 0;
	i32 nTotalVertexCount = 0;
	i32 nTotalTriCount = 0;
	i32 nTotalRenderChunksCount = 0;
	...
	if (m_pMaterial)
	{
		i32 nSubMtls = m_pMaterial->GetSubMtlCount();
		if (nSubMtls > 0)
		{
			for (i32 i = 0; i < nSubMtls; ++i)
			{
				...
				if (pSubMtl)
				{
					...
					if (pSFType && pSFType->GetBreakability() != 0)
					{
						...
						else if (IsMatIDReferencedByObj((u16)i))
						{
							return false;
						}
					}
...
CStatObj::SSubObject* CStatObj::FindSubObject_CGA(tukk sNodeName)
{
	u32 numSubObjects = m_subObjects.size();
...
//////////////////////////////////////////////////////////////////////////
CStatObj::SSubObject* CStatObj::FindSubObject(tukk sNodeName)
{
	u32 numSubObjects = m_subObjects.size();
	i32 len = 1; // some objects has ' ' in the beginning
...
//////////////////////////////////////////////////////////////////////////
CStatObj::SSubObject* CStatObj::FindSubObject_StrStr(tukk sNodeName)
{
	u32 numSubObjects = m_subObjects.size();
	for (u32 i = 0; i < numSubObjects; i++)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CStatObj::RemoveSubObject(i32 nIndex)
{
...
//////////////////////////////////////////////////////////////////////////
void CStatObj::SetSubObjectCount(i32 nCount)
{
	// remove sub objects.
	while ((i32)m_subObjects.size() > nCount)
	{
...
//////////////////////////////////////////////////////////////////////////
bool CStatObj::CopySubObject(i32 nToIndex, IStatObj* pFromObj, i32 nFromIndex)
{
	SSubObject* pSrcSubObj = pFromObj->GetSubObject(nFromIndex);
	if (!pSrcSubObj)
		return false;

	if (nToIndex >= (i32)m_subObjects.size())
	{
...
bool CStatObj::RayIntersection(SRayHitInfo& hitInfo, IMaterial* pCustomMtl)
{
	...
	if ((m_nFlags & STATIC_OBJECT_COMPOUND) && !m_bMerged)
	{
		...
		for (i32 i = 0, num = m_subObjects.size(); i < num; i++)
		{
			if (m_subObjects[i].pStatObj && m_subObjects[i].nType == STATIC_SUB_OBJECT_MESH && !m_subObjects[i].bHidden)
			{
				...
				i32 nFirstTriangleId = hit.pHitTris ? hit.pHitTris->Count() : 0;
				...
				if (hit.pHitTris)
				{
					for (i32 t = nFirstTriangleId; t < hit.pHitTris->Count(); t++)
					{
...
bool CStatObj::LineSegIntersection(const Lineseg& lineSeg, Vec3& hitPos, i32& surfaceTypeId)
{
	bool intersects = false;
	if (m_pRenderMesh)
	{
		m_pRenderMesh->LockForThreadAccess();
		i32 numIndices = m_pRenderMesh->GetIndicesCount();
		i32 numVertices = m_pRenderMesh->GetVerticesCount();
		if (numIndices && numVertices)
		{
			i32 posStride;
			u8* pPositions = (u8*)m_pRenderMesh->GetPosPtr(posStride, FSL_READ);
			...
			for (i32 nChunkId = 0; nChunkId < nChunkCount; nChunkId++)
			{
				...
				if (!(pChunk->m_nMatFlags & MTL_FLAG_NODRAW))
				{
					...
					for (i32 i = pChunk->nFirstIndexId; i < lastIndex; )
					{
...
//////////////////////////////////////////////////////////////////////////
i32 CStatObj::CountChildReferences()
{
	// Check if it must be released.
	i32 nChildRefs = 0;
	i32 numChilds = (i32)m_subObjects.size();
...
{
	if (i32 nLowestLod = CStatObj::GetMinUsableLod())
		return m_pLODs ? (CStatObj*)m_pLODs[CStatObj::GetMinUsableLod()] : (CStatObj*)NULL;
	return this;
}

//////////////////////////////////////////////////////////////////////////
i32 CStatObj::FindHighestLOD(i32 nBias)
{
	i32 nLowestLod = CStatObj::GetMinUsableLod();

	// if requested lod is not ready - find nearest ready one
	i32 nLod = CLAMP((i32)m_nMaxUsableLod + nBias, nLowestLod, (i32)m_nMaxUsableLod);
...
void CStatObj::GetStatisticsNonRecursive(SStatistics& si)
{
	...
	for (i32 j = 0; j < pStatObj->m_arrPhysGeomInfo.GetGeomCount(); j++)
	{
		if (pStatObj->GetPhysGeom(j))
		{
			IDrxSizer* pPhysSizer = GetISystem()->CreateSizer();
			pStatObj->GetPhysGeom(j)->pGeom->GetMemoryStatistics(pPhysSizer);
			i32 sz = pPhysSizer->GetTotalSize();
...
void CStatObj::GetStatistics(SStatistics& si)
{
	...
	else
	{
		...
		std::vector<uk> addedObjects;
		for (i32 k = 0; k < GetSubObjectCount(); k++)
		{
			if (!GetSubObject(k))
				continue;
			CStatObj* pSubObj = (CStatObj*)GetSubObject(k)->pStatObj;

			i32 nSubObjectType = GetSubObject(k)->nType;
