...
i32 CDecalRenderNode::m_nFillBigDecalIndicesCounter = 0;
...
void CDecalRenderNode::CreatePlanarDecal()
{
	...
	drx_strcpy(decalInfo.szMaterialName, m_pMaterial->GetName());
...
void CDecalRenderNode::CreateDecalOnStaticObjects()
{
	...
	else if (pVisAreaUpr && m_pOcNode && m_pOcNode->GetVisArea())
		pVisAreaUpr->GetObjectsAround(m_decalProperties.m_pos, m_decalProperties.m_radius, &decalReceivers, true);

	// delete vegetations
	for (i32 nRecId(0); nRecId < decalReceivers.Count(); ++nRecId)
	{
	...
	for (i32 nRecId(0); nRecId < decalReceivers.Count(); ++nRecId)
	{
		...
		i32 nSubObjCount = pMainStatObj->GetSubObjectCount();
		if (!GetCVars()->e_DecalsMerge && nSubObjCount)
		{
			for (i32 nSubObjId = 0; nSubObjId < nSubObjCount; nSubObjId++)
			{
...
void CDecalRenderNode::SetDecalProperties(const SDecalProperties& properties)
{
	    ...
	    (properties.m_projectionType == SDecalProperties::eProjectOnTerrainAndStaticObjects || properties.m_projectionType == SDecalProperties::eProjectOnTerrain))
	{
		...
		if (m_pMaterial->GetFlags() & MTL_FLAG_TRACEABLE_TEXTURE && m_pMaterial->GetShaderItem().m_pShaderResources)
			if (SEfResTexture* pTex = m_pMaterial->GetShaderItem().m_pShaderResources->GetTexture(EFTT_DIFFUSE))
				if (ITexture* pITex = pTex->m_Sampler.m_pITex)
					...
					{
						i32k lp = 5;
						Vec2i szKrn((i32)sz.x / lp, (i32)sz.y / lp), rszKrn((1 << 23) / szKrn.x, (1 << 23) / szKrn.y);
						Vec2 rsz(1.0f / sz.x, 1.0f / sz.y);

						auto RunningSum = [](u8* psrc, u8* pdst, i32 stride, i32 w, i32 wk, i32 rwk, i32 rwkShift)
						{
							i32 i, sum;
							u8* p0 = psrc, * p1 = psrc;
						...
						u8* buf0 = new u8[sz.x * sz.y], * buf1 = new u8[sz.x * sz.y], * buf2 = new u8[sz.x * sz.y];
						for (i32 i = sz.x * sz.y - 1; i >= 0; i--)
							buf0[i] = data[i].a;
						for (i32 iy = 0; iy < sz.y; iy++)	// low-pass along x
							RunningSum(buf0 + iy * sz.x, buf1 + iy * sz.x, 1, sz.x, szKrn.x, rszKrn.x, 23);
						for (i32 ix = 0; ix < sz.x; ix++)	// low-pass along y, set to 1 if resulting alpha>0.5
							RunningSum(buf1 + ix, buf0 + ix, sz.x, sz.y, szKrn.y, rszKrn.y, 23 + 7);
						for (i32 iy = 0; iy < sz.y; iy++)	// count neighbors (+self) along x
							RunningSum(buf0 + iy * sz.x, buf1 + iy * sz.x, 1, sz.x, 3, 1, 0);
						for (i32 ix = 0; ix < sz.x; ix++)	// ..repeat along y
							RunningSum(buf1 + ix, buf2 + ix, sz.x, sz.y, 3, 1, 0);
						i32 nVtx = 0;
						for (i32 i = sz.x * sz.y - 1; i >= 0; nVtx += buf0[i] & isneg((i32)buf2[i] - 6), i--)
							;	// count filled points with less than 5 filled neighbors
						ptitem2d* pts = new ptitem2d[nVtx];
						for (i32 iy = 0, i = 0, j = 0; iy < sz.y; iy++)
							for (i32 ix = 0; ix < sz.x; ix++, i++)
								if (buf0[i] && buf2[i] < 6)
									pts[j++].pt.set((ix + 0.5f) * 2 * rsz.x - 1, 1 - (iy + 0.5f) * 2 * rsz.y);

						edgeitem* edges = new edgeitem[nVtx * 2], * pedge;
						i32 nEdges = gEnv->pPhysicalWorld->GetPhysUtils()->qhull2d(pts, nVtx, edges, 16);
						for (pedge = edges; nEdges && !pedge->next; pedge++)
							;
						Vec3* vtx = new Vec3[nEdges * 2];
						Vec3_tpl<u16>* tri = new Vec3_tpl<u16>[(nEdges - 1) * 4];
						for (i32 i = 0; i < nEdges; i++, pedge = pedge->next)
						{
							Vec3 pt = m_Matrix.TransformVector(Vec3(pedge->pvtx->pt));
							vtx[i] = pt - gwd[0].v * thickness;
							vtx[i + nEdges] = pt + gwd[0].v * thickness;
						}
						for (i32 i = 0; i < nEdges - 2; i++)
						{
							tri[i].Set(0, i + 2, i + 1);
		...
		i32 useStatics = properties.m_projectionType == SDecalProperties::eProjectOnTerrainAndStaticObjects;
		i32 nEnts = gEnv->pPhysicalWorld->GetEntitiesInBox(center - size, center + size, pents, ent_terrain | ent_static & - useStatics, 16);
		...
		if (pGeom->GetType() == GEOM_TRIMESH)
		{
			...
			for (i32 i = 0; i*2 < md->nVertices; i++)
			{
...
tukk CDecalRenderNode::GetEntityClassName() const
{
	return "Decal";
}

tukk CDecalRenderNode::GetName() const
{
	return "Decal";
}
...
void CDecalRenderNode::Render(const SRendParams& rParam, const SRenderingPassInfo& passInfo)
{
	...
	for (size_t i(0); i < m_decals.size(); ++i)
	{
		...
		if (pDecal && 0 != pDecal->m_pMaterial)
		{
			...
			bool bAfterWater = CObjUpr::IsAfterWater(pDecal->m_vWSPos, passInfo.GetCamera().GetPosition(), passInfo, waterLevel);
...
void CDecalRenderNode::GetMemoryUsage(IDrxSizer* pSizer) const
{
...
void CDecalRenderNode::CleanUpOldDecals()
{
	if (m_nLastRenderedFrameId != 0 && // was rendered at least once
	    (i32)GetRenderer()->GetFrameID(false) > (i32)m_nLastRenderedFrameId + GetCVars()->e_DecalsMaxValidFrames)
	{
		DeleteDecals();
		m_nLastRenderedFrameId = 0;
