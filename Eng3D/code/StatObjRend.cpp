...
// -------------------------------------------------------------------------
//  Имя файла:   statobjrend.cpp
//  Version:     v1.00
//  Created:     28/5/2001 by Vladimir Kajalin
//  Компиляторы:       drux (DinrusPro's Universal Compiler),   mingw-w64-clang-x86_64
//  Описание:    Подготовка и добавление элемента отображения в отображатель.
// -------------------------------------------------------------------------
//  История:
//
////////////////////////////////////////////////////////////////////////////
...
#ifndef _RELEASE
	i32 nMaxDrawCalls = GetCVars()->e_MaxDrawCalls;
	if (nMaxDrawCalls > 0)
	{
...
void CStatObj::RenderStreamingDebugInfo(CRenderObject* pRenderObject, const SRenderingPassInfo& passInfo)
{
	...
	for (i32 nLod = 1; pStreamable->m_pLODs && nLod < MAX_STATOBJ_LODS_NUM; nLod++)
	{
	...
	if (nKB > GetCVars()->e_StreamCgfDebugMinObjSize)
	{
		...
		i32 nDiff = SATURATEB(i32(float(nKB - GetCVars()->e_StreamCgfDebugMinObjSize) / max((i32)1, GetCVars()->e_StreamCgfDebugMinObjSize) * 255));
...
{
	for (i32 i = 0; i < GetSubObjectCount(); ++i)
	{
                                ...
                                SInstancingInfo* pInstInfo, CRenderObject*& pObj, const SRenderingPassInfo& passInfo)
{
	...
	{
		u8 nFrozenLayer = (rParams.nMaterialLayers & MTL_LAYER_FROZEN) ? MTL_LAYER_FROZEN_MASK : 0;
		u8 nWetLayer = (rParams.nMaterialLayers & MTL_LAYER_WET) ? MTL_LAYER_WET_MASK : 0;
		pObj->m_nMaterialLayers = (u32) (nFrozenLayer << 24) | (nWetLayer << 16);
	}
	else
...
bool CStatObj::RenderDebugInfo(CRenderObject* pObj, const SRenderingPassInfo& passInfo)
{
	...
	i32 e_DebugDraw = GetCVars()->e_DebugDraw;
	...
	if (m_nRenderTrisCount > 0 && !bOnlyBoxes && !bFiltered)
	{
		...
		i32k nMaxUsableLod = (m_pLod0 != 0) ? m_pLod0->m_nMaxUsableLod : m_nMaxUsableLod;
		i32k nRealNumLods = (m_pLod0 != 0) ? m_pLod0->m_nLoadedLodsNum : m_nLoadedLodsNum;

		i32 nNumLods = nRealNumLods;
		...
		i32 nMats = m_pRenderMesh ? m_pRenderMesh->GetChunks().size() : 0;
		i32 nRenderMats = 0;

		if (nMats)
		{
			for (i32 i = 0; i < nMats; ++i)
			{
		...
		case 3:
			{
				...
				if (nNumLods < 2)
				{
					...
					else
					{
						...
						clr.g = 127 + (i32)(sinf(fAngle) * 120);  // flashing color
					}
				}
				else
				{
				...
				{
					i32k nLod0 = GetMinUsableLod();
					i32k maxLod = GetMaxUsableLod();
					const bool bRenderNodeValid(pObj && pObj->m_pRenderNode && ((UINT_PTR)(uk )(pObj->m_pRenderNode) > 0));
		...
		case 4:
			{
				...
				if (m_pRenderMesh)
				{
					i32 nTexMemUsage = m_pRenderMesh->GetTextureMemoryUsage(pMaterial);
		...
		case 6:
			{
				...
				IRenderAuxText::DrawLabelExF(pos, 1.3f, color, true, true, "%d,%d,%d,%d", (i32)(col.r * 255.0f), (i32)(col.g * 255.0f), (i32)(col.b * 255.0f), (i32)(col.a * 255.0f));
			}
			break;
		...
		case 16:
			{
				...
				{
					tukk shortName = PathUtil::GetFile(m_szFileName.c_str());
					...
					{
						for (i32 i = 0; i < m_pParentObject->SubObjectCount(); i++)
						{
		...
		case 19:  // Displays the triangle count of physic proxies.
			if (!bNoText)
			{
				i32 nPhysTrisCount = 0;
				for (i32 j = 0; j < MAX_PHYS_GEOMS_TYPES; ++j)
				{
					if (GetPhysGeom(j))
...
{
	i32 nSubCount = m_subObjects.size();
	if (!nSubCount)
	{
...
i32 CStatObj::ComputeLodFromScale(float fScale, float fLodRatioNormalized, float fEntDistance, bool bFoliage, bool bForPrecache)
{
	assert(!m_pLod0);
	...
	nNewLod = (i32)(fEntDistance * (fLodRatioNormalized * fLodRatioNormalized) / (max(GetCVars()->e_LodRatio * min(GetRadius() * fScale, GetFloatCVar(e_LodCompMaxSize)), 0.001f)));

	i32k nMinLod = GetMinUsableLod();
	nNewLod += nMinLod;

	i32 nMaxUsableLod = min((i32)m_nMaxUsableLod, GetCVars()->e_LodMax);
...
void CStatObj::DebugDraw(const SGeometryDebugDrawInfo& info)
{
	...
	else
	{
		...
		if (m_pLODs)
		{
			...
			for (i32 nLod = 0; nLod <= (i32)m_nMaxUsableLod; nLod++)
			{
...
void CStatObj::RenderInternal(CRenderObject* pRenderObject, hidemask nSubObjectHideMask, const CLodValue& lodValue, const SRenderingPassInfo& passInfo)
{
	...
	if ((m_nFlags & STATIC_OBJECT_COMPOUND) && !m_bMerged)
	{
		...
		if (m_nSubObjectMeshCount > 0)
		{
			...
			for (i32 i = 0, subObjectsSize = m_subObjects.size(); i < subObjectsSize; ++i, nBitIndex <<= 1)
			{
...
///////////////////////////////////////////////////////////////////////////////
void CStatObj::RenderSubObject(CRenderObject* pRenderObject, i32 nLod,
...
///////////////////////////////////////////////////////////////////////////////
void CStatObj::RenderSubObjectInternal(CRenderObject* pRenderObject, i32 nLod, const SRenderingPassInfo& passInfo)
{
	...
	nLod = CLAMP(nLod, GetMinUsableLod(), (i32)m_nMaxUsableLod);
	DRX_ASSERT(nLod >= 0 && nLod < MAX_STATOBJ_LODS_NUM);
	...
	else
	{
		...
		if (m_pLODs)
			for (; nLod <= (i32)m_nMaxUsableLod; nLod++)
			{
...
///////////////////////////////////////////////////////////////////////////////
void CStatObj::RenderObjectInternal(CRenderObject* pRenderObject, i32 nTargetLod, u8 uLodDissolveRef, bool dissolveOut, const SRenderingPassInfo& passInfo)
{
	...
	i32 nLod = CLAMP(nTargetLod, GetMinUsableLod(), (i32)m_nMaxUsableLod);
	DRX_ASSERT(nLod >= 0 && nLod < MAX_STATOBJ_LODS_NUM);
...
///////////////////////////////////////////////////////////////////////////////
i32 CStatObj::GetMaxUsableLod()
{
	auto lodMax = GetCVars()->e_LodMax;
	lodMax = CLAMP(lodMax, 0, MAX_STATOBJ_LODS_NUM - 1);

	i32 maxUsable = m_pLod0 ? max((i32)m_nMaxUsableLod, (i32)m_pLod0->m_nMaxUsableLod) : (i32)m_nMaxUsableLod;
	return min(maxUsable, lodMax);
}

///////////////////////////////////////////////////////////////////////////////
i32 CStatObj::GetMinUsableLod()
{
	auto lodMin = GetCVars()->e_LodMin;
	lodMin = CLAMP(lodMin, 0, MAX_STATOBJ_LODS_NUM - 1);

	i32 minUsable = m_pLod0 ? max((i32)m_nMinUsableLod0, (i32)m_pLod0->m_nMinUsableLod0) : (i32)m_nMinUsableLod0;
	return max(minUsable, lodMin);
}

///////////////////////////////////////////////////////////////////////////////
i32 CStatObj::FindNearestLoadedLOD(i32 nLodIn, bool bSearchUp)
{
	// make sure requested lod is loaded
	/*  if(CStatObj * pObjForStreamIn = nLodIn ? m_pLODs[nLodIn] : this)
	{
	bool bRenderNodeValid(rParams.pRenderNode && ((i32)(uk )(rParams.pRenderNode)>0) && rParams.pRenderNode->m_fWSMaxViewDist);
	...
	if (nLod >(i32)m_nMaxUsableLod)
	{
		if (bSearchUp)
		{
			nLod = min((i32)m_nMaxUsableLod, nLodIn);
...
i32 CStatObj::AddRef()
{
	return DrxInterlockedIncrement(&m_nUsers);
}
