...
IRenderNode* GetRenderNodeFromPhys(uk pForeignData, i32 iForeignData)
{
...
IStatObj* GetIStatObjFromPhysicalEntity(IPhysicalEntity* pent, i32 nPartId, Matrix34* pTM, i32& nMats, i32*& pMatMapping);

i32       CPhysCallbacks::OnFoliageTouched(const EventPhys* pEvent)
{
	...
	if (pOverlap->iForeignData[1] == PHYS_FOREIGN_ID_STATIC || pOverlap->iForeignData[1] == PHYS_FOREIGN_ID_ENTITY)
	{
		...
		i32 nWheels = 0, nParts = pOverlap->pEntity[0]->GetStatus(&snp);
...
foundbox:
		...
		else if (i < (i32)pEngine->m_arrEntsInFoliage.size())
		...
		i32 cullDist = GetCVars()->e_CullVegActivation;
		i32 iSource = 0;
...
//////////////////////////////////////////////////////////////////////////
i32 CPhysCallbacks::OnPhysStateChange(const EventPhys* pEvent)
{
...
void CDeferredCollisionEventOnPhysCollision::RayTraceVegetation()
{
	...
	i32 i, j, bHit = 0, * pMats, nSlot = pCollision->partid[1] & 1023;
	...
	if (pent->GetParams(&ppart))
	{
		if (ppart.pPhysGeom && ppart.pPhysGeom->pGeom)
		{
			...
			if (pStatObj &&
				...
				for (i32 i = pStatObjParent->GetSubObjectCount() - 1; i >= 0; i--)
...
void CDeferredCollisionEventOnPhysCollision::TestCollisionWithRenderMesh()
{
	...
	u32 pierceability0 = 0, pierceability1;
	float dummy;
	...
	if (pStatObj)
		if (i32 nMinLod = GetCVars()->e_RenderMeshCollisionLod)
...
void CDeferredCollisionEventOnPhysCollision::FinishTestCollisionWithRenderMesh()
{
	...
	//DrxLogAlways( "Do Test %f,%f",vHitPos.x,vHitPos.y );
...
i32 CPhysStreamer::CreatePhysicalEntity(uk pForeignData, i32 iForeignData, i32 iForeignFlags)
{
	if (iForeignData == PHYS_FOREIGN_ID_STATIC)
...
i32 CPhysStreamer::CreatePhysicalEntitiesInBox(const Vec3& boxMin, const Vec3& boxMax)
{
	DRX_PROFILE_FUNCTION(PROFILE_3DENGINE);
	if (((C3DEngine*)gEnv->p3DEngine)->m_pObjectsTree)
		((C3DEngine*)gEnv->p3DEngine)->m_pObjectsTree->PhysicalizeInBox(AABB(boxMin, boxMax));
	if (((C3DEngine*)gEnv->p3DEngine)->m_pVisAreaUpr)
		((C3DEngine*)gEnv->p3DEngine)->m_pVisAreaUpr->PhysicalizeInBox(AABB(boxMin, boxMax));
	return 1;
}

i32 CPhysStreamer::DestroyPhysicalEntitiesInBox(const Vec3& boxMin, const Vec3& boxMax)
{
	DRX_PROFILE_FUNCTION(PROFILE_3DENGINE);
	if (((C3DEngine*)gEnv->p3DEngine)->m_pObjectsTree)
		((C3DEngine*)gEnv->p3DEngine)->m_pObjectsTree->DephysicalizeInBox(AABB(boxMin, boxMax));
	if (((C3DEngine*)gEnv->p3DEngine)->m_pVisAreaUpr)
		((C3DEngine*)gEnv->p3DEngine)->m_pVisAreaUpr->DephysicalizeInBox(AABB(boxMin, boxMax));
	return 1;
}

//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
i32 CDeferredCollisionEventOnPhysCollision::OnCollision(const EventPhys* pEvent)
{
	...
	return pDeferredPhysicsEventUpr->HandleEvent(pEvent, &CDeferredCollisionEventOnPhysCollision::CreateCollisionEvent, IDeferredPhysicsEvent::PhysCallBack_OnCollision);
	...
	m_bNeedMeshThreadUnLock(false)
{
	...
	m_pNextEvent = 0;
	if (IDeferredPhysicsEvent* pLastEvent = gEnv->p3DEngine->GetDeferredPhysicsEventUpr()->GetLastCollisionEventForEntity(pCollisionEvent->pEntity[0]))
	{
...
CDeferredCollisionEventOnPhysCollision::~CDeferredCollisionEventOnPhysCollision()
{
	...
	IDeferredPhysicsEventUpr* pDeferredPhysicsEventUpr = gEnv->p3DEngine->GetDeferredPhysicsEventUpr();
	pDeferredPhysicsEventUpr->UnRegisterDeferredEvent(this);
}
...
void CDeferredCollisionEventOnPhysCollision::Start()
{
	...
	m_bTaskRunning = true;
	IDeferredPhysicsEventUpr* pDeferredPhysicsEventUpr = gEnv->p3DEngine->GetDeferredPhysicsEventUpr();
	pDeferredPhysicsEventUpr->DispatchDeferredEvent(this);
}
...
i32 CDeferredCollisionEventOnPhysCollision::Result(EventPhys* pOrigEvent)
{
	Sync();
	if (m_pNextEvent)
...
void CDeferredCollisionEventOnPhysCollision::MarkFinished(i32 nResult)
{
	m_nResult = nResult;
	m_bFinished = true;
}
...
void CDeferredCollisionEventOnPhysCollision::Sync()
{
	...
	gEnv->GetJobUpr()->WaitForJob(m_jobStateRayInter);
	gEnv->GetJobUpr()->WaitForJob(m_jobStateOnCollision);
...
uk CDeferredCollisionEventOnPhysCollision::operator new(size_t)
{
	return gCDeferredCollisionEventOnPhysCollisionPoolAllocator.Allocate();
}

void CDeferredCollisionEventOnPhysCollision::operator delete(uk ptr)
{
