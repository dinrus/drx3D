...
class CFeatureLightSource : public CParticleFeature, public DinrusX3dEngBase
{
public:
	...
	virtual void AddToComponent(CParticleComponent* pComponent, SComponentParams* pParams) override
	{
		...
		if (GetPSystem()->GetFlareMaterial() && !m_flare.empty())
			m_hasFlareOptics = gEnv->pOpticsUpr->Load(m_flare.c_str(), m_lensOpticsId);
	...
	virtual void RenderDeferred(const CParticleComponentRuntime& runtime, const SRenderContext& renderContext) override
	{
		...
		light.m_nStencilRef[0] = m_affectsThisAreaOnly ? renderContext.m_renderParams.nClipVolumeStencilRef : CClipVolumeUpr::AffectsEverythingStencilRef;
		light.m_nStencilRef[1] = CClipVolumeUpr::InactiveVolumeStencilRef;
		...
		if (m_hasFlareOptics)
		{
			...
			IOpticsElementBase* pOptics = gEnv->pOpticsUpr->GetOptics(m_lensOpticsId);
...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureLightSource, "Light", "Light", colorLight);
