...
#pragma warning(disable: 4800) // 'i32': forcing value to bool 'true' or 'false'
...
namespace
{
	...
	struct ColorAttr : ColorB
	{
		...
		ColorAttr(i32 in)      { r = g = b = drxmath::clamp(in, 0, 0xff); a = 0xff; }
		ColorAttr(float in)    { r = g = b = float_to_ufrac8(in); a = 0xff; }

		operator bool() const  { return r + g + b > 0; }
		operator i32() const   { return int_round(Luminance()); }
		operator float() const { return Luminance() / 255.0f; }
	};

	TAttribConverterFn s_AttributeConverterTable[numTypes][numTypes] =
	{
		{ ConvertAttrib<bool, bool>      , ConvertAttrib<bool, i32>      , ConvertAttrib<bool, float>      , ConvertAttr3<bool, ColorAttr, ColorB>   },
		{ ConvertAttrib<i32, bool>       , ConvertAttrib<i32, i32>       , ConvertAttrib<i32, float>       , ConvertAttr3<i32, ColorAttr, ColorB>    },
		{ ConvertAttrib<float, bool>     , ConvertAttrib<float, i32>     , ConvertAttrib<float, float>     , ConvertAttr3<float, ColorAttr, ColorB>  },
		{ ConvertAttr3<ColorB, ColorAttr, bool> , ConvertAttr3<ColorB, ColorAttr, i32> , ConvertAttr3<ColorB, ColorAttr, float> , ConvertAttrib<ColorB, ColorB> }
	};
...
i32 CAttributeTable::FindAttributeIdByName(const CDrxName& name) const
{
...
bool CAttributeTable::SerializeName(IArchive& ar, CDrxName& value, cstr name, cstr label) const
{
	if (ar.isEdit())
	{
		if (m_enumDesc.count())
		{
			i32 attributeId = FindAttributeIdByName(value);
			if (!m_enumDesc.serialize(ar, attributeId, name, label))
				return false;
			value = attributeId >= 0 ? m_attributes[attributeId].m_name : CDrxName();
			return true;
		}
	}
...
{
	DRX_PFX2_ASSERT(pTable != nullptr);
...
void CAttributeInstance::Serialize(IArchive& ar)
{
	...
	else
	{
		...
		for (uint attributeId = 0; attributeId < numAttributes; ++attributeId)
		{
			...
			tukk name = attribute.m_name.c_str();
...
void CAttributeInstance::TransferInto(IParticleAttributes* pReceiver) const
{
	...
	DRX_PFX2_ASSERT(pSourceTable); // this instance over-lived the table
...
auto CAttributeInstance::FindAttributeIdByName(const CDrxName& name) const->TAttributeId
{
...
{
	const CDrxName& name = GetDesc(id).m_name;
	i32k editId = stl::find_index_if(m_attributesEdit, [&name](const SAttributeEdit& edit)
	{
...
bool CAttributeInstance::SetValue(TAttributeId id, const TValue& input)
{
	...
	if (output.get_if<i32>())
	{
		output = max(+desc.m_minInt, output.get<i32>());
		output = min(+desc.m_maxInt, output.get<i32>());
	}
