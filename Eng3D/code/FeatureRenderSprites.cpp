...
DRX_PFX2_IMPLEMENT_FEATURE(CParticleFeature, CFeatureRenderSprites, "Render", "Sprites", colorRender);
...
void CFeatureRenderSprites::CullParticles(SSpritesContext& spritesContext)
{
		...
		if (culling)
		{
			...
			if (cullNear + cullFar + sumArea)
			{
				...
				if (cullNear)
				{
					...
					alpha *= drxmath::saturate((1.0f - ratio) * 2.0f);
				}
				if (cullFar)
				{
					const float ratio = min(fullSize * invMinAng, maxCamDist) * invDist;
					alpha *= drxmath::saturate((ratio - 1.0f) * 3.0f);
				}
				if (sumArea)
				{
	...
	if (spritesContext.m_particleIds.size() && !emitter.GetSpawnParams().bIgnoreVisAreas)
	{
		...
		{
			DRX_PROFILE_SECTION(PROFILE_PARTICLE, "pfx2::CullParticles:VisArea");
			DRX_PFX2_ASSERT(container.HasData(EPDT_Size));
	...
	{
		DRX_PROFILE_SECTION(PROFILE_PARTICLE, "pfx2::CullParticles:Water");
		DRX_PFX2_ASSERT(container.HasData(EPDT_Size));
...
void CFeatureRenderSprites::SortSprites(SSpritesContext& spritesContext)
{
	...
	THeapArray<u32> indices(memHeap, numSprites);
	...
	if (byAge)
	{
		DRX_PFX2_ASSERT(container.HasData(EPDT_NormalAge));
		DRX_PFX2_ASSERT(container.HasData(EPDT_LifeTime));
	...
	memcpy(particleIds.data(), indices.data(), sizeof(u32) * indices.size());
}
...
void CFeatureRenderSprites::WriteToGPUMem(const SSpritesContext& spritesContext, SRenderVertices* pRenderVertices, const TAxesSampler& axesSampler)
{
	...
	const TIStream<u8> tiles = container.IStream(EPDT_Tile);
	...
	colorSTDefault.st.z = u8(params.m_shaderData.m_firstTile);
	...
	for (uint chunk = 0, spriteIdx = 0; chunk < numChunks; ++chunk)
	{
		...
		for (uint sprite = 0; sprite < chunkSprites; ++sprite, ++spriteIdx)
		{
			...
			if (hasAnimation)
			{
				...
				colorST.st.z += i32(animPos);
				colorST.st.w = FloatToUFrac8Saturate(animPos - i32(animPos));
			}
