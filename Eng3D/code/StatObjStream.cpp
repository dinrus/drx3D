...
void CStatObj::StreamOnComplete(IReadStream* pStream, unsigned nError)
{
	...
	else
	{
		...
		//////////////////////////////////////////////////////////////////////////
		for (i32 nSID = 0; nSID < m_pObjUpr->m_lstStaticTypes.Count(); nSID++)
		{
			PodArray<StatInstGroup>& rGroupTable = m_pObjUpr->m_lstStaticTypes;
			for (i32 nGroupId = 0, nGroups = rGroupTable.Count(); nGroupId < nGroups; nGroupId++)
			{
...
void CStatObj::ReleaseStreamableContent()
{
	...
	for (i32 s = 0; s < GetSubObjectCount(); s++)
	{
...
i32 CStatObj::GetStreamableContentMemoryUsage(bool bJustForDebug)
{
	...
	if (m_arrRenderMeshesPotentialMemoryUsage[bCountLods] < 0)
	{
		...
		{
			if (m_pLODs)
				for (i32 nLod = 1; nLod < MAX_STATOBJ_LODS_NUM; nLod++)
				{
	...
	if (m_pMergedRenderMesh)
	{
		...
		m_nMergedMemoryUsage = m_pMergedRenderMesh->GetVerticesCount() * (sizeof(SPipTangents) + sizeof(SVF_P3S_C4B_T2S)) + m_pMergedRenderMesh->GetIndicesCount() * sizeof(u16);
	}
...
{
	i32 nRoundId = GetObjUpr()->m_nUpdateStreamingPrioriryRoundId;
...
bool CStatObj::UpdateStreamableComponents(float fImportance, const Matrix34A& objMatrix, bool bFullUpdate, i32 nNewLod)
{
	...
	{
		for (i32 s = 0, num = SubObjectCount(); s < num; s++)
		{
			...
			if (subObj.pStatObj && subObj.nType == STATIC_SUB_OBJECT_MESH && !subObj.bShadowProxy)
			{
				...
				for (i32 l = nNewLod; l <= (nNewLod + 1) && l < MAX_STATOBJ_LODS_NUM; l++)
				{
	...
	if (!m_szStreamingDependencyFilePath.empty())
		if (CStatObj* pNextState = (CStatObj*)GetObjUpr()->FindStaticObjectByFilename(m_szStreamingDependencyFilePath))
	...
	if (m_pParentObject && !m_pParentObject->m_szStreamingDependencyFilePath.empty())
		if (CStatObj* pNextState = (CStatObj*)GetObjUpr()->FindStaticObjectByFilename(m_pParentObject->m_szStreamingDependencyFilePath))
			pNextState->UpdateStreamableComponents(fImportance, objMatrix, bFullUpdate, nNewLod);
...
void CStatObj::DisableStreaming()
{
	for (i32 nLodLevel = 0; nLodLevel < MAX_STATOBJ_LODS_NUM; nLodLevel++)
	{
		if (CStatObj* pLodObj = (CStatObj*)GetLodObject(nLodLevel))
		{
			...
			pLodObj->UpdateStreamingPrioriryLowLevel(1.f, GetObjUpr()->m_nUpdateStreamingPrioriryRoundId, true);
			...
			GetObjUpr()->RegisterForStreaming(pParentObject);
...
bool CStatObj::CheckForStreamingDependencyLoop(tukk szFilenameDependancy) const
{
	CObjUpr* pObjUpr = GetObjUpr();
	while (szFilenameDependancy)
	{
		const CStatObj* pStatObjDependency = static_cast<const CStatObj*>(pObjUpr->FindStaticObjectByFilename(szFilenameDependancy));
