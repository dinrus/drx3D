...
#include <drx3D/CoreX/Thread/IThreadUpr.h>		// For CScopedFloatingPointException
...
private:
	...
	DRX_ALIGN(16) NVMath::vec4 m_VMaxXY;
	static DRX_ALIGN(128) float m_ZBufferMainMemory[SIZEX * SIZEY];
	u32    m_SizeX4;
	...
	DRX_ALIGN(128) tdZexel m_ZBufferSwapMerged[SIZEX * SIZEY]; // 128 byte for XMemSet128
	  ...
	  const NVMath::vec4& rV2)
	{
		...
		u32k Idx = SignMask(Shuffle<xzzz>(Shuffle<zzzz>(V0, V1), V2)) & (BitX | BitY | BitZ);
		...
		case BitX | BitY | BitZ:
			break;
//#if !DRX_PLATFORM_ORBIS && !DRX_PLATFORM_ANDROID
	//	default:
			//__assume(0);
//#endif
		}
	...
	inline bool Triangle2D(NVMath::vec4 rV0, NVMath::vec4 rV1, NVMath::vec4 rV2, u32 MinX = 0, u32 MinY = 0, u32 MaxX = 0, u32 MaxY = 0, NVMath::vec4& VMinMax = NVMath::Vec4Zero(), NVMath::vec4& V210 = NVMath::Vec4Zero())
#else
	inline bool Triangle2D(NVMath::vec4 rV0, NVMath::vec4 rV1, NVMath::vec4 rV2, u32 MinX = 0, u32 MinY = 0, u32 MaxX = 0, u32 MaxY = 0, NVMath::vec4 VMinMax = NVMath::Vec4Zero (), NVMath::vec4 V210 = NVMath::Vec4Zero ())
#endif
	{
		...
		if (PROJECT)
		{
			...
			u32k* pMM = reinterpret_cast<u32*>(&VMinMax);
			MinX = pMM[0];
			MinY = pMM[1];
			MaxX = pMM[2];
			MaxY = pMM[3];
		...
		do
		{
			...
			vec4* pDstZ = reinterpret_cast<vec4*>(&m_ZOutput[MinX + y * (u16)SIZEX]);
			y++;
			u16 x = MinX;
			do
			{
	...
	inline bool Quad2D(const NVMath::vec4& rV0, const NVMath::vec4& rV1, const NVMath::vec4& rV3, const NVMath::vec4& rV2)
	{
		...
		u16 MinX = Vec4int32(VMinMax, 0);
		u16k MinY = Vec4int32(VMinMax, 1);
		u16k MaxX = Vec4int32(VMinMax, 2);
		u16k MaxY = Vec4int32(VMinMax, 3);
		if (MinX >= MaxX || MinY >= MaxY)
		{
			return false;
		}
...
public:
	...
	~CCullRenderer()
	{
		...
		{
			DrxModuleMemalignFree(m_ZBufferSwap[i]);
		}
	...
	void Prepare()
	{
		...
		m_nNumWorker = gEnv->pJobUpr->GetNumWorkerThreads();
		m_ZBufferSwap = new tdZexel*[m_nNumWorker];
		for (u32 i = 0; i < m_nNumWorker; ++i)
		{
			m_ZBufferSwap[i] = (tdZexel*)DrxModuleMemalign(sizeof(tdZexel) * SIZEX * SIZEY, 128);
		}
	}
	...
	inline void Clear()
	{
		...
		for (u32 a = 0, S = SIZEX * SIZEY; a < S; a++)
		{
	...
	void ReprojectHWDepthBuffer(const Matrix44A& rCurrent, float nearPlane, float farPlane, float nearestMax, float Bias, i32 nStartLine, i32 nNumLines)
	{
		//#define USE_W_DEPTH
		//#define SCALE_DEPTH

		u32 nWorkerThreadID = JobUpr::GetWorkerThreadId();
	...
	void MergeReprojectHWDepthBuffer(i32 nStartLine, i32 nNumLines)
	{
		i32k sizeX = SIZEX;
		using namespace NVMath;
		...
		for (u32 i = 0; i < m_nNumWorker; ++i)
		{
			for (i32 y = nStartLine; y < nStartLine + nNumLines; y++)
			{
				for (i32 x = 0; x < sizeX; x += 4)
				{
	...
	void ReprojectHWDepthBufferAfterMerge(const Matrix44A& rCurrent, float nearPlane, float farPlane, float nearestMax, float Bias, i32 nStartLine, i32 nNumLines)
	{
		using namespace NVMath;
		i32 sizeX = SIZEX;
		i32 sizeY = SIZEY;
		...
		for (i32 y = nStartLine; y < nStartLine + nNumLines; y++)
		{
			i32 minY = max((i32)0, (i32)y - 1);
			i32 maxY = min((i32)sizeY - 1, (i32)y + 1);
			i32 maxX = min(pitchX - 1, 0 + 1);
			...
			i32 vecX = 0;
			for (i32 x = 0; x < sizeX; x += 4) //todo, fix edge cases
			{
				vec4 vDst;
	...
	inline i32 AABBInFrustum(const NVMath::vec4* pViewProj, Vec3 Min, Vec3 Max, Vec3 ViewPos)
	{
		using namespace NVMath;
	...
	inline void Rasterize(const NVMath::vec4* pViewProj, const NVMath::vec4* __restrict pTriangles, size_t TriCount)
	{
		...
		for (size_t a = 0, S = TriCount; a < S; a += VCacheCount)
		{
			...
			else
			{
...
#if !DRX_PLATFORM_LINUX && !DRX_PLATFORM_ANDROID && !DRX_PLATFORM_APPLE //to avoid DivBy0 exception on PC
					Det = Select(Det, NVMath::Vec4(-FLT_EPSILON), CmpEq(Det, Vec4Zero()));
#endif
					Det = Rcp(Det);
					Det012 = Splat<0>(Det);
				...
				{
					u32k* pMM = reinterpret_cast<u32*>(pDetTmp);
					u16k MinX = pMM[0];
					u16k MinY = pMM[1];
					u16k MaxX = pMM[2];
					u16k MaxY = pMM[3];
	...
	inline bool Rasterize(const NVMath::vec4* pViewProj, tdVertexCacheArg vertexCache,
	                            const tdIndex* __restrict pIndices, u32k ICount,
	                            u8k* __restrict pVertices, u32k VertexSize, u32k VCount)
	{
		using namespace NVMath;
		if (!VCount || !ICount)
		{
		...
		vec4 SMask = Vec4Zero();
		for (u32 a = 0, S = VCount & ~3; a < S; a += 4)
		{
		...
		for (u32 a = VCount & ~3, S = VCount; a < S; a++)
		{
	...
	void DrawDebug(IRenderer* pRenderer, i32 nStep)
	{
		...
		for (u32 y = 0; y < SIZEY; y += 1)
		{
			...
			for (u32 x = 0; x < SIZEX; x += 4)
			{
				...
				u32 ValueColor0 = (u32)(pVMemZ[x + 0]);
				u32 ValueColor1 = (u32)(pVMemZ[x + 1]);
				u32 ValueColor2 = (u32)(pVMemZ[x + 2]);
				u32 ValueColor3 = (u32)(pVMemZ[x + 3]);
				...
				DRX_ASSERT(pVertices <= vertices.data() + vertices.size());
			}
		}
	...
	inline u32 SizeX() const
	{
		return SIZEX;
	}
	inline u32 SizeY() const
	{
		return SIZEY;
	}
...
template<u32 SIZEX, u32 SIZEY>
DRX_ALIGN(128) float NAsyncCull::CCullRenderer<SIZEX, SIZEY>::m_ZBufferMainMemory[SIZEX * SIZEY];
#endif
