...
ILINE u32 SChaosKey::Rand()
{
	return m_key = Jumble(m_key);
}

ILINE u32 SChaosKey::Rand(u32 range)
{
	return Rand() % range;
}
...
{
	return float(i32(Rand())) * g_MaxI32toF;  // Cast rand as signed to generate signed float
}
...
ILINE SChaosKeyV::SChaosKeyV(u32 key)
{
...
ILINE uint32v SChaosKeyV::Rand(u32 range)
{
...
{
	// Cast rand as signed, as only signed SIMD i32-to-float conversions are available
	int32v iv = Rand();
