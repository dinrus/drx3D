// Copyright 2015-2018 DinrusPro / Dinrus Group. РНЦП Динрус. 

#pragma once

#include <drx3D/CoreX/Math/FinalizingSpline.h>
#include <drx3D/CoreX/Serialization/Enum.h>
#include <drx3D/CoreX/DynamicEnum.h>
...
{
public:
	explicit SChaosKey(u32 key) : m_key(key) {}
...
struct SChaosKeyV
{
	...
	explicit SChaosKeyV(u32 key);
	explicit SChaosKeyV(uint32v keys) : m_keys(keys) {}
	uint32v Rand();
	uint32v Rand(u32 range);
	floatv  RandUNorm();
	floatv  RandSNorm();
...
struct TDimInfo
{
	...
	static TElem Elem(const T& t, uint e) { DRX_ASSERT(e == 0); return t; }
};
...
struct TDataType: EParticleDataType
{
	...
	{
		DRX_ASSERT(this->info().domain & EDD_HasUpdate);
