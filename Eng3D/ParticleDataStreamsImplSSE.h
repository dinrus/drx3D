...
template<>
ILINE uint32v TIStream<u32>::SafeLoad(TParticleGroupId pgId) const
{
	return _mm_load_si128(reinterpret_cast<const __m128i*>(m_pStream + (pgId & m_safeMask)));
}

template<>
ILINE uint32v TIStream<u32>::SafeLoad(TParticleIdv pIdv) const
{
	if (!m_safeMask)
		return m_safeSink;
...
{
	DRX_PFX2_DEBUG_ASSERT(IsValid(value));
