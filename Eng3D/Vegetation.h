...
public:
	...
	static DRX_ALIGN(128) float g_scBoxDecomprTable[256];

	CVegetation();
	virtual ~CVegetation();
	void                 SetStatObjGroupIndex(i32 nVegetationGroupIndex) final;

	void                 CheckCreateDeformable();
	virtual bool         CanExecuteRenderAsJob() final;
	i32                  GetStatObjGroupId() const final      { return m_nObjectTypeIndex; }
	tukk          GetEntityClassName(void) const final { return "Vegetation"; }
	...
	void                 SetScale(float fScale)               { m_ucScale = (u8)SATURATEB(fScale * VEGETATION_CONV_FACTOR); }
	tukk          GetName(void) const final;
	...
	IRenderMesh*         GetRenderMesh(i32 nLod) final;
	...
	bool                 PhysicalizeFoliage(bool bPhysicalize = true, i32 iSource = 0, i32 nSlot = 0) final;
	virtual IRenderNode* Clone() const final;
	IPhysicalEntity*     GetBranchPhys(i32 idx, i32 nSlot = 0) final;
	IFoliage*            GetFoliage(i32 nSlot = 0) final;
	...
	IStatObj*            GetEntityStatObj(u32 nSubPartId = 0, Matrix34A* pMatrix = NULL, bool bReturnOnlyVisible = false) final;
	...
	void                 UpdateRndFlags();
	ILINE i32            GetStatObjGroupSize() const
	{
		return GetObjUpr()->m_lstStaticTypes.Count();
	}
	ILINE StatInstGroup& GetStatObjGroup() const
	{
		return GetObjUpr()->m_lstStaticTypes[m_nObjectTypeIndex];
	}
	ILINE CStatObj* GetStatObj() const
	{
		return GetObjUpr()->m_lstStaticTypes[m_nObjectTypeIndex].GetStatObj();
	}
	float         GetZAngle() const;
	AABB          CalcBBox();
	void          CalcMatrix(Matrix34A& tm, i32* pTransFags = NULL);
	virtual u8 GetMaterialLayers() const final
	{
		return GetObjUpr()->m_lstStaticTypes[m_nObjectTypeIndex].nMaterialLayers;
	}
