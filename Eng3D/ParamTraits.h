...
#include <drx3D/Eng3D/ParticleSpline.h>
...
private:
	ILINE friend bool Serialize(Serialization::IArchive& ar, SEnable& val, tukk name, tukk label);
	bool m_value;
};
...
public:
	...
	void Set(T value)
	{
		m_value = TTraits::To(drxmath::clamp(value, HardMin(), HardMax()));
	}
...
private:
	...
	friend bool Serialize(Serialization::IArchive& ar, TValue& val, tukk name, tukk label)
	{
		return val.Serialize(ar, name, label);
	}

	bool Serialize(Serialization::IArchive& ar, tukk name, tukk label);

	T m_value;
};

template<i32 iMin, typename Base = BaseTraits>
struct THardMin: Base
{
	template<typename T> static T HardMin() { return T(iMin); }
};

template<i32 iMin, i32 iMax, typename Base = BaseTraits>
...
template<i32 iTo, i32 iFrom, typename Base = BaseTraits>
