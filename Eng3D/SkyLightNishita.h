...
class CSkyLightNishita : public DinrusX3dEngBase
{
private:

	// size of lookup tables
	static u32k cOLUT_HeightSteps = 32;
	static u32k cOLUT_AngularSteps = 256;

	// definition of optical depth LUT for mie/rayleigh scattering
	struct DRX_ALIGN(8) SOpticalDepthLUTEntry
	{
		f32 mie;
		f32 rayleigh;
	...
	struct DRX_ALIGN(8) SPhaseLUTEntry
	{
		f32 mie;
		f32 rayleigh;
	};
	...
	static u32k cPLUT_AngularSteps = 256;
	//replace std::vector by a custom class with static storage
	class PhaseLUT
	{
		DRX_ALIGN(128) SPhaseLUTEntry m_LUT[cPLUT_AngularSteps];
		size_t m_Size;
	public:
		PhaseLUT() : m_Size(0) {}
...
public:
	...
	void  SetInScatteringIntegralStepSize(i32 stepSize);
	i32 GetInScatteringIntegralStepSize() const;
...
private:
	// mapping helpers
	f64  MapIndexToHeight(u32 index) const;
	f64  MapIndexToCosVertAngle(u32 index) const;
	f32  MapIndexToCosPhaseAngle(u32 index) const;

	void MapCosPhaseAngleToIndex(const f32 cosPhaseAngle, u32& index, f32& indexFrc) const;

	// optical lookup table access helpers
	u32 OpticalLUTIndex(u32 heightIndex, u32 cosVertAngleIndex) const;
...
private:
	...
	i32 m_inScatteringStepSize;

	friend class CSkyLightUpr;
};
