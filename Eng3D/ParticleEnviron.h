...
#include <drx3D/CoreX/ParticleSys/ParticleParams.h> // just for ETrinary
...
struct SPhysForces
{
	...
	void Add(SPhysForces const& other, u32 nEnvFlags);
};
...
struct SPhysEnviron : DinrusX3dEngBase
{
	// PhysArea caching.
	SPhysForces m_UniformForces;
	ETrinary    m_tUnderWater;
	u32      m_nNonUniformFlags;         // EParticleEnviron flags of non-uniform areas found.
	u32      m_nNonCachedFlags;          // Env flags of areas requiring physics system access.
	...
	struct SAreaSpec
	{
		...
		SAreaSpec() {}
		SAreaSpec(u32 flags, AABB const& bb) : m_nFlags(flags), m_bounds(bb) {}
	...
	struct SArea: SAreaSpec
	{
		_smart_ptr<IPhysicalEntity>
		              m_pArea;
		 i32* m_pLock;                  // Copy of lock for area.
		...
		u8    m_nGeomShape;                  // GEOM_BOX, GEOM_SPHERE, ...
		...
		void  GetForces(SPhysForces& forces, AABB const& bb, u32 nFlags, bool bAverage) const;
	...
	void Update(SPhysEnviron const& envSource, AABB const& box, bool bIndoors, u32 nFlags, bool bNonUniformAreas = true, ukk pObjectSkip = 0);
	...
	void LockAreas(u32 nFlags, i32 iLock) const
	{
			...
			for (auto& area : m_NonUniformAreas)
			{
				if (area.m_nFlags & nFlags)
					if (!area.m_bCacheForce)
						DrxInterlockedAdd(area.m_pLock, iLock);
			}
		}
	}

	void ForNonumiformAreas(AABB const& bb, u32 nFlags, std::function<void(const SArea&)> func) const
	{
	...
	void GetForces(SPhysForces& forces, Vec3 const& vPos, u32 nFlags) const
	{
	...
	// Phys collision
	static bool PhysicsCollision(ray_hit& hit, Vec3 const& vStart, Vec3 const& vEnd, float fRadius, u32 nEnvFlags, IPhysicalEntity* pThisEntity = 0);
...
struct SWorldPhysEnviron: SPhysEnviron
{
	...
	bool Update(SPhysEnviron& env, AABB const& box, bool bIndoors, u32 nFlags, bool bNonUniformAreas = true, ukk pObjectSkip = 0) const
	{
...
// Vis area management.
//
struct SVisEnviron : DinrusX3dEngBase
{
	SVisEnviron()
	{
		Clear();
