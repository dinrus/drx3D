...
struct OcclusionTestClient
{
	...
	u32 nLastVisibleMainFrameID, nLastOccludedMainFrameID;
	u32 nLastShadowCastMainFrameID, nLastNoShadowCastMainFrameID;

#ifdef SUPP_HMAP_OCCL
	Vec3 vLastVisPoint;
	i32  nTerrainOccLastFrame;
#endif
};
...
struct SRenderNodeTempData
{
	struct SUserData
	{
		i32                 lastSeenFrame[MAX_RECURSION_LEVELS];             // must be first, see IRenderNode::SetDrawFrame()
		i32                 lastSeenShadowFrame;                             // When was last rendered to shadow
		...
		u32              nCubeMapId                  : 16;
		u32              nCubeMapIdCacheClearCounter : 16;
		u32              nWantedLod                  : 8;
		u32              bTerrainColorWasUsed        : 1;
		u32              bClipVolumeAssigned         : 1;
		IRenderNode*        pOwnerNode;
		u32              nStatObjLastModificationId;
	};

public:
	SUserData           userData;

	DrxRWLock           arrPermanentObjectLock[MAX_STATOBJ_LODS_NUM];

	std::atomic<u32> hasValidRenderObjects;
	std::atomic<u32> invalidRenderObjects;

public:
	SRenderNodeTempData()  { ZeroStruct(userData); hasValidRenderObjects = invalidRenderObjects = false; }
	~SRenderNodeTempData() { Free(); };

	CRenderObject* GetRenderObject(i32 nLod); /* thread-safe */
	void           Free();
...
struct IShadowCaster
{
	...
	virtual bool                       HasOcclusionmap(i32 nLod, IRenderNode* pLightOwner)           { return false; }
	virtual CLodValue                  ComputeLod(i32 wantedLod, const SRenderingPassInfo& passInfo) { return CLodValue(wantedLod); }
	...
	u8                              m_shadowCacheLastRendered[MAX_GSM_CACHED_LODS_NUM];
	u8                              m_shadowCacheLod[MAX_GSM_CACHED_LODS_NUM];
 ...
 *
 * To visualize objects in a world DRXENGINE defines the concept of render nodes and render elements. Render nodes represent general objects in the 3D engine. Among other things they are used to build a hierarchy for visibility culling, allow physics interactions (optional) and rendering.
...
public:
	...
	virtual void         ReleaseNode(bool bImmediate = false) { DRX_ASSERT((m_dwRndFlags & ERF_PENDING_DELETE) == 0); delete this; }
	...
	virtual IStatObj* GetEntityStatObj(u32 nSubPartId = 0, Matrix34A* pMatrix = NULL, bool bReturnOnlyVisible = false);
	...
	virtual struct IGeomCacheRenderNode* GetGeomCacheRenderNode(u32 nSlot, Matrix34A* pMatrix = NULL, bool bReturnOnlyVisible = false) { return NULL; }
#endif

	//! \return IRenderMesh of the object.
	virtual struct IRenderMesh* GetRenderMesh(i32 nLod) { return 0; };

	//! Allows to adjust default lod distance settings.
	//! If fLodRatio is 100 - default lod distance is used.
	virtual void SetLodRatio(i32 nLodRatio) { m_ucLodRatio = min(255, max(0, nLodRatio)); }

	//! Get material layers mask.
	virtual u8 GetMaterialLayers() const { return m_nMaterialLayers; }
	...
	virtual bool PhysicalizeFoliage(bool bPhysicalize = true, i32 iSource = 0, i32 nSlot = 0) { return false; }

	//! Get physical entity (rope) for a given branch (if foliage is physicalized).
	virtual IPhysicalEntity* GetBranchPhys(i32 idx, i32 nSlot = 0) { return 0; }

	//! \return Physicalized foliage, or NULL if it isn't physicalized.
	virtual struct IFoliage* GetFoliage(i32 nSlot = 0) { return 0; }

	//! Make sure I3DEngine::FreeRenderNodeState(this) is called in destructor of derived class.
	virtual ~IRenderNode() { DRX_ASSERT(!m_pTempData.load()); };
	...
	virtual void       SetCollisionClassIndex(i32 tableIndex)          {}

	virtual void       SetStatObjGroupIndex(i32 nVegetationGroupIndex) {}
	virtual i32        GetStatObjGroupId() const                       { return -1; }
	virtual void       SetLayerId(u16 nLayerId)                     {}
	virtual u16     GetLayerId()                                    { return 0; }
	...
	virtual u8 GetSortPriority()                        { return 0; }
	...
	ILINE void SetDrawFrame(i32 nFrameID, i32 nRecursionLevel)
	{
		...
		i32* pDrawFrames = pTempData->userData.lastSeenFrame;
		pDrawFrames[nRecursionLevel] = nFrameID;
	}

	ILINE i32 GetDrawFrame(i32 nRecursionLevel = 0) const
	{
		// If we can get a pointer atomically it must be valid [until the end of the frame] and we can access it
		const auto pTempData = m_pTempData.load();
	...
	ILINE i32 GetViewDistRatio() const { return (m_ucViewDistRatio == 255) ? 1000l : m_ucViewDistRatio; }

	//! Retrieves the view distance settings without any value interpretation.
	ILINE i32 GetViewDistRatioVal() const { return m_ucViewDistRatio; }
	...
	ILINE i32 GetLodRatio() const { return m_ucLodRatio; }
	...
	//! \return Lod distance ratio.
	ILINE i32 GetShadowLodBias() const { return m_cShadowLodBias; }
	...
	ILINE void               SetMaterialLayers(u8 nMtlLayers) { m_nMaterialLayers = nMtlLayers; }

	ILINE i32                GetMinSpec() const                  { return (m_dwRndFlags & ERF_SPEC_BITS_MASK) >> ERF_SPEC_BITS_SHIFT; };
	...
	virtual void SetEditorObjectId(u32 nEditorObjectId)
	{
	...
	{
		u32 flags = (bSelected * (1)) | (bHighlighted * (1 << 1));
...
public:
	...
	DrxRWLock                         m_manipulationLock;	
	i32                               m_manipulationFrame = -1;

	//! Hud silhouette parameter, default is black with alpha zero
	u32 m_nHUDSilhouettesParam;
	...
	//! The high 24 bits store the actual ID of the object. This need not be the same as DrxGUID,
	//! though the DrxGUID could be used to generate it
	u32 m_nEditorSelectionID;

	//! Used to request visiting of the node during one-pass traversal
	u32 m_onePassTraversalFrameId = 0;
	u32 m_onePassTraversalShadowCascades = 0;
};

inline void IRenderNode::SetViewDistRatio(i32 nViewDistRatio)
{
...
///////////////////////////////////////////////////////////////////////////////
inline IStatObj* IRenderNode::GetEntityStatObj(u32 nSubPartId, Matrix34A* pMatrix, bool bReturnOnlyVisible)
{
	return 0;
}
...
i32k FAR_TEX_COUNT = 16;             //!< Number of sprites per object.
i32k FAR_TEX_ANGLE = (360 / FAR_TEX_COUNT);
i32k FAR_TEX_HAL_ANGLE = (256 / FAR_TEX_COUNT) / 2;
...
struct ILightSource : public IRenderNode
{
	...
	virtual struct ShadowMapFrustum* GetShadowFrustum(i32 nId = 0) = 0;
...
{
	// <interfuscator:shuffle>
	virtual void SetVertices(const Vec3* pVerts, i32 nVertsNum, float fTexCoordBegin, float fTexCoordEnd, float fTexCoordBeginGlobal, float fTexCoordEndGlobal) = 0;
	virtual void SetSortPriority(u8 sortPrio) = 0;
	virtual void SetIgnoreTerrainHoles(bool bVal) = 0;
	virtual void SetPhysicalize(bool bVal) = 0;
	virtual void GetClipPlanes(Plane* pPlanes, i32 nPlanesNum, i32 nVertId = 0) = 0;
...
struct IVoxelObject : public IRenderNode
{
	...
	virtual void                 SetCompiledData(uk pData, i32 nSize, u8 ucChildId, EEndian eEndian) = 0;
	virtual void                 SetObjectName(tukk pName) = 0;
...
struct SFogVolumeProperties
{
	...
	i32    m_volumeType = IFogVolumeRenderNode::eFogVolumeType_Box;
...
struct IWaterVolumeRenderNode : public IRenderNode
{
	...
	virtual void             CreateArea(uint64 volumeID, const Vec3* pVertices, u32 numVertices, const Vec2& surfUVScale, const Plane& fogPlane, bool keepSerializationParams = false) = 0;
	virtual void             CreateRiver(uint64 volumeID, const Vec3* pVertices, u32 numVertices, float uTexCoordBegin, float uTexCoordEnd, const Vec2& surfUVScale, const Plane& fogPlane, bool keepSerializationParams = false) = 0;

	virtual void             SetAreaPhysicsArea(const Vec3* pVertices, u32 numVertices, bool keepSerializationParams = false) = 0;
	virtual void             SetRiverPhysicsArea(const Vec3* pVertices, u32 numVertices, bool keepSerializationParams = false) = 0;

	virtual IPhysicalEntity* SetAndCreatePhysicsArea(const Vec3* pVertices, u32 numVertices) = 0;
	// </interfuscator:shuffle>
};
...
{
	// <interfuscator:shuffle>
	virtual void                    Create(uint64 nID, const Vec3* pVertices, u32 nVertexCount, const Vec2& pUVScale, const Matrix34& pWorldTM) = 0;
...
struct IRopeRenderNode : public IRenderNode
{
	...
	struct SRopeParams
	{
		i32   nFlags = eRope_CheckCollisinos; //!< ERopeParamFlags.
	...
	struct SRopeAudioParams
	{
		...
		DrxAudio::ControlId      startTrigger = DrxAudio::InvalidControlId;
		DrxAudio::ControlId      stopTrigger = DrxAudio::InvalidControlId;
		DrxAudio::ControlId      angleParameter = DrxAudio::InvalidControlId;
...
{
	virtual bool LoadGeomCache(tukk sGeomCacheFileName) = 0;
	...
	virtual void SetStandIn(tukk pFilePath, tukk pMaterial) = 0;
	virtual void SetFirstFrameStandIn(tukk pFilePath, tukk pMaterial) = 0;
	virtual void SetLastFrameStandIn(tukk pFilePath, tukk pMaterial) = 0;
	...
	virtual tukk GetNodeName(const uint nodeIndex) const = 0;
	virtual u32      GetNodeNameHash(const uint nodeIndex) const = 0;
