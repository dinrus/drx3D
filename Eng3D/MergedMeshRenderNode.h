...
// simulation / skinning
#define MMRM_USE_VECTORIZED_SSE_INSTRUCTIONS DRX_PLATFORM_SSE2
...
#if MMRM_DEBUG_PRINT
	#define mmrm_printf(...) DrxLog(__VA_ARGS__)
#endif
#ifndef mmrm_printf
	  ...
	  , public DinrusX3dEngBase
{
	...
	inline tukk ToString(State state) const
	{
		switch (state)
		{
		case INITIALIZED:
	...
	u32 m_needsStaticMeshUpdate : 1;

	// Does the mesh need a dynamic mesh update
	u32 m_needsDynamicMeshUpdate : 1;

	// Is the mesh needs a static mesh update
	u32 m_needsPostRenderStatic : 1;

	// Is the mesh needs a static mesh update
	u32 m_needsPostRenderDynamic : 1;
	...
	JobUpr::SJobState m_spineInitializationState;
	...
	size_t QueryDensity(const Vec3& pos, IMergedMeshesUpr::TFixedSurfacePtrTypeArray& surfaceTypes, IMergedMeshesUpr::TFixedDensityArray& density);
...
public:
	...
	bool        AddGroup(u32 statInstGroupId, u32 numInstances);
	...
	u32 VisibleInstances(u32 frameId) const;

	// Returns the number of spine instances
	u32 SpineCount() const;

	// Returns the number of deformed instances
	u32 DeformCount() const;
	...
	i32  IsActive() const             { return m_nActive; }
	void SetActive(i32 active)        { m_nActive = active; }

	i32  IsInVisibleSet() const       { return m_nVisible; }
	void SetInVisibleSet(i32 visible) { m_nVisible = visible; }
	...
	bool Compile(byte* pData, i32& nSize, string* pName, std::vector<struct IStatInstGroup*>* pVegGroupTable, const Vec3& segmentOffset = Vec3(0, 0, 0));
	...
	void InitializeSamples(float fExtents, u8k* pBuffer);
	...
	//////////////////////////////////////////////////////////////////////
	tukk             GetName() const             { return "Runtime MergedMesh";  }
	tukk             GetEntityClassName() const  { return "Runtime MergedMesh"; };
	...
	ILINE StatInstGroup&    GetStatObjGroup(i32k index) const
	{
		return GetObjUpr()->m_lstStaticTypes[index];
	}
	ILINE CStatObj* GetStatObj(i32k index) const
	{
		return GetObjUpr()->m_lstStaticTypes[index].GetStatObj();
	}
};
...
public:
	...
	// Inherited from IRenderNode
	//////////////////////////////////////////////////////////////////////
	tukk             GetName() const                                                                     { return "Runtime MergedMesh Proxy Instance";  }
	tukk             GetEntityClassName() const                                                          { return "Runtime MergedMesh Proxy Instance"; };
	...
	void                    GetMemoryUsage(IDrxSizer* pSizer) const                                             { __debugbreak(); }
};
...
class CMergedMeshesUpr
	: public DinrusX3dEngBase
	  , public IMergedMeshesUpr
{
	friend class CMergedMeshRenderNode;
	friend class CDeformableNode;
	friend inline void QueryProjectiles(SMMRMProjectile*&, i32&, const AABB&);
	...
	static i32 OnPhysPostStep(const EventPhys*);
...
public:
	...
	const IMergedMeshesUpr::SInstanceSector& GetInstanceSector(size_t index) const { return m_InstanceSectors[index]; }
	size_t                                       GetInstanceSectorCount() const        { return m_InstanceSectors.size(); }
	void                                         ClearInstanceSectors();

	// Compile cluster areas
	bool CompileAreas(DynArray<SMeshAreaCluster>& clusters, i32 flags);
