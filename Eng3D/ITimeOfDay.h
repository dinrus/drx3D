...
struct ITimeOfDay
{
	...
	{
		tukk          name;        //!< Variable name.
		tukk          displayName; //!< Variable user readable name.
		tukk          group;       //!< Group name.
		i32                  nParamId;
		EVariableType        type;
	...
	struct IListener
	{
		...
		virtual ~IListener() { }
		virtual void OnChange(const EChangeType changeType, tukk const szPresetName) = 0;
	};

	// <interfuscator:shuffle>
	virtual ~ITimeOfDay(){}

	virtual i32  GetPresetCount() const = 0;
	virtual bool GetPresetsInfos(SPresetInfo* resultArray, u32 arraySize) const = 0;
	virtual bool SetCurrentPreset(tukk szPresetName) = 0;
	virtual tukk GetCurrentPresetName() const = 0;
	virtual bool AddNewPreset(tukk szPresetName) = 0;
	virtual bool RemovePreset(tukk szPresetName) = 0;
	virtual bool SavePreset(tukk szPresetName) const = 0;
	virtual bool LoadPreset(tukk szFilePath) = 0;
	virtual void ResetPreset(tukk szPresetName) = 0;

	virtual bool ImportPreset(tukk szPresetName, tukk szFilePath) = 0;
	virtual bool ExportPreset(tukk szPresetName, tukk szFilePath) const = 0;

	//! Access to variables that control time of the day appearance.
	virtual i32  GetVariableCount() = 0;
	virtual bool GetVariableInfo(i32 nIndex, SVariableInfo& varInfo) = 0;
	virtual void SetVariableValue(i32 nIndex, float fValue[3]) = 0;

	//! Editor interface.
	virtual bool  InterpolateVarInRange(i32 nIndex, float fMin, float fMax, u32 nCount, Vec3* resultArray) const = 0;
	virtual uint  GetSplineKeysCount(i32 nIndex, i32 nSpline) const = 0;
	virtual bool  GetSplineKeysForVar(i32 nIndex, i32 nSpline, SBezierKey* keysArray, u32 keysArraySize) const = 0;
	virtual bool  SetSplineKeysForVar(i32 nIndex, i32 nSpline, const SBezierKey* keysArray, u32 keysArraySize) = 0;
	virtual bool  UpdateSplineKeyForVar(i32 nIndex, i32 nSpline, float fTime, float newValue) = 0;
	...
	static i32k NETSER_FORCESET = BIT(0);
	static i32k NETSER_COMPENSATELAG = BIT(1);
	static i32k NETSER_STATICPROPS = BIT(2);
	virtual void     NetSerialize(TSerialize ser, float lag, u32 flags) = 0;
	...
	bool RegisterListener(IListener* const pListener, tukk const szDbgName, const bool staticName) { return RegisterListenerImpl(pListener, szDbgName, staticName); }
	void UnRegisterListener(IListener* const pListener) { return UnRegisterListenerImpl(pListener); }

protected:
	virtual bool RegisterListenerImpl(IListener* const pListener, tukk const szDbgName, const bool staticName) = 0;
