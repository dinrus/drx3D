...
// ------------------------------------------------------------------------
//  Имя файла:   GeomCachePredictors.h
//  Created:     17/1/2013 by Axel Gneiting
//  Описание: Predictors for index frame compression
// -------------------------------------------------------------------------
////////////////////////////////////////////////////////////////////////////
...
void ParallelogramPredictor(const uint numValues, T* pIn, T* pOut, const std::vector<u16>& predictorData)
{
	...
	{
		u16k uDist = predictorData[predictorDataPos++];
		T predictedValue;
		...
		else
		{
			...
			u16k vDist = predictorData[predictorDataPos++];
			u16k wDist = predictorData[predictorDataPos++];
...
void QTangentPredictor(const uint numValues, const GeomCacheFile::QTangent* pIn, GeomCacheFile::QTangent* pOut, const std::vector<u16>& predictorData)
{
	...
	for (uint i = 0, predictorDataPos = 0; i < numValues; ++i)
	{
		...
		else
		{
			...
			predictedValue = Vec4_tpl<i32>(u) + Vec4_tpl<i32>(v);
...
template<bool kbEncode>
inline void ColorPredictor(const uint numValues, const GeomCacheFile::Color* pIn, GeomCacheFile::Color* pOut, const std::vector<u16>& predictorData)
{
...
void InterpolateDeltaEncode(const uint numValues, u8k lerpFactor, const T* pFloorFrame, const T* pCeilFrame, const T* pIn, T* pOut)
{
...
template<class I, class T>
void MotionDeltaEncode(const uint numValues, u8k acceleration, const T* const pPrevFrames[2], const T* pIn, T* pOut)
{
	for (uint i = 0; i < numValues; ++i)
	{
...
void InterpolateMotionDeltaPredictor(const GeomCacheFile::STemporalPredictorControl& controlIn, const STemporalPredictorData<T>& data, const T* pIn, T* pOut)
{
	...
	u8k& lerpFactor = controlIn.m_indexFrameLerpFactor;
	u8k& acceleration = controlIn.m_acceleration;
	u8k combineFactor = controlIn.m_combineFactor;
...
ILINE __m128i Interpolate(__m128i a, __m128i b, __m128i c, u32k factor, i32k shiftFactor)
{
...
template<> void InterpolateMotionDeltaPredictor<u32, u16, false>
  (const GeomCacheFile::STemporalPredictorControl& controlIn, const STemporalPredictorData<u16>& data, u16k* pIn, u16* pOut)
{
	...
	const uint tailSizeInBytes = (data.m_numElements - numBlocks * 8) * sizeof(u16);
...
template<> void InterpolateMotionDeltaPredictor<Vec2_tpl<u32>, Vec2_tpl<u16>, false>
  (const GeomCacheFile::STemporalPredictorControl& controlIn, const STemporalPredictorData<Vec2_tpl<u16>>& data,
  const Vec2_tpl<u16>* pIn, Vec2_tpl<u16>* pOut)
{
	STemporalPredictorData<u16> uInt16Data;

	uInt16Data.m_pFloorFrame = (u16*)data.m_pFloorFrame;
	uInt16Data.m_pCeilFrame = (u16*)data.m_pCeilFrame;
	uInt16Data.m_pPrevFrames[0] = (u16*)data.m_pPrevFrames[0];
	uInt16Data.m_pPrevFrames[1] = (u16*)data.m_pPrevFrames[1];
	uInt16Data.m_numElements = data.m_numElements * 2;

	InterpolateMotionDeltaPredictor<u32, u16, false>(controlIn, uInt16Data, (u16*)pIn, (u16*)pOut);
}

template<> void InterpolateMotionDeltaPredictor<Vec3_tpl<u32>, Vec3_tpl<u16>, false>
  (const GeomCacheFile::STemporalPredictorControl& controlIn, const STemporalPredictorData<Vec3_tpl<u16>>& data,
  const Vec3_tpl<u16>* pIn, Vec3_tpl<u16>* pOut)
{
	STemporalPredictorData<u16> uInt16Data;
...
template<> void InterpolateMotionDeltaPredictor<Vec4_tpl<u32>, Vec4_tpl<u16>, false>
  (const GeomCacheFile::STemporalPredictorControl& controlIn, const STemporalPredictorData<Vec4_tpl<u16>>& data,
  const Vec4_tpl<u16>* pIn, Vec4_tpl<u16>* pOut)
{
	STemporalPredictorData<u16> uInt16Data;
