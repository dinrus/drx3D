...
SERIALIZATION_ENUM_DECLARE(EAnimationCycle, : u8,
	Once,
	Loop,
	Mirror
)
...
struct STextureAnimation
{
	...
	TValue<u16, THardLimits<1, 256>> m_frameCount;    //!< Number of tiles (frames) of animation
...
struct SComponentParams: STimingParams
{
	...
	u32                    m_maxParticlesBurst;
	u32                    m_maxParticlesPerFrame;
	...
	i32                       m_renderStateFlags;
	u8                     m_particleObjFlags;

	void GetMaxParticleCounts(i32& total, i32& perFrame, float minFPS = 4.0f, float maxFPS = 120.0f) const;
};
