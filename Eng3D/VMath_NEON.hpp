...
namespace drxengine_vmath_neon_internal
{
// Contains pre-calculated lookup indices for Shuffle().  Indexed by ESwizzleMask
static u8kx16_t g_SwizzleMasks[] = {
	SWIZZLEMASK1
};
} //endns drxengine_vmath_neon_internal
 ...
 */
static u32k BitX = 0x00000080;
static u32k BitY = 0x00008000;
static u32k BitZ = 0x00800000;
static u32k BitW = 0x80000000;
...
ILINE vec4 Shuffle(vec4 V0, vec4 V1)
{
	...
	assert(M >= 0 && M < sizeof(drxengine_vmath_neon_internal::g_SwizzleMasks) / sizeof(drxengine_vmath_neon_internal::g_SwizzleMasks[0]));
	// Get precomputed vector containing lookup indices for swizzle
	u8kx16_t mask = drxengine_vmath_neon_internal::g_SwizzleMasks[M];
	// Split the lookup indexes into xy and zw pair
	u8kx8_t low_index = vget_low_u8(mask);
	u8kx8_t high_index = vget_high_u8(mask);
	...
	u8kx8_t low = vtbl4_u8(tbl, low_index);
	u8kx8_t high = vtbl4_u8(tbl, high_index);
	u8kx16_t ret = vcombine_u8(low, high);
	return (float32x4_t)ret;
	#endif // AVOID_COMPILER_BUG
}
...
ILINE u32 SignMask(vec4 V)
{
	...
	u8kx8_t idx_sign = { 3, 7, 11, 15 }; // MSB -> LSB = W -> X (0xWWZZYYXX)
	u8kx8_t sign_bytes = vtbl2_u8(tbl, idx_sign);
		#else
	const int32x4_t rev = vshrq_n_s32((int32x4_t)V, 24);
	const int16x4_t shorts = vmovn_s32(rev);
	u8kx8_t tbl = (uint8x8_t)shorts;
	u8kx8_t index = { 1, 3, 5, 7 };
	u8kx8_t sign_bytes = vtbl1_u8(tbl, index);
		#endif
	return vget_lane_u32((uint32x2_t)sign_bytes, 0);
	//return (V.m_Xu>>31)|((V.m_Yu>>31)<<1)|((V.m_Zu>>31)<<2)|((V.m_Wu>>31)<<3);
	#else
	u8kx16_t tbl = vcombine_u8((uint8x8_t)vget_low_f32(V), (uint8x8_t)vget_high_f32(V));
	u8kx8_t idx_sign = { 3, 7, 11, 15 }; // MSB -> LSB = W -> X (0xWWZZYYXX)
	u8kx8_t sign_bytes = vtbl1q_u8(tbl, idx_sign);
	return vget_lane_u32((uint32x2_t)sign_bytes, 0);
	#endif
}
