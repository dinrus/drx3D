...
std::shared_ptr<ParticleType> ParticleType::Create(const std::shared_ptr<Image2d> &image, uint32_t numberOfRows, const Color &colorOffset, float lifeLength,
	float stageCycles, float scale) {
	ParticleType temp(image, numberOfRows, colorOffset, lifeLength, stageCycles, scale);
	Node node;
	node << temp;
	return Create(node);
}

ParticleType::ParticleType(std::shared_ptr<Image2d> image, uint32_t numberOfRows, const Color &colorOffset, float lifeLength, float stageCycles,
	float scale) :
...
void ParticleType::Update(const std::vector<Particle> &particles) {
	...
	for (const auto &particle : particles) {
		...
		instance->colorOffset = particle.GetParticleType()->colorOffset;
...
const Node &operator>>(const Node &node, ParticleType &particleType) {
	...
	node["colorOffset"].Get(particleType.colorOffset);
...
Node &operator<<(Node &node, const ParticleType &particleType) {
	...
	node["colorOffset"].Set(particleType.colorOffset);
