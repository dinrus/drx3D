...
{
	for (i32 i = 0; i < m_tmpModels.size(); i++)
	{
...
static bool parseVector3(Vec3& vec3, const STxt& vector_str, ErrorLogger* logger, bool lastThree = false)
{
	vec3.setZero();
	Array<STxt> pieces;
	Array<double> rgba;
...
// </bullet>
static void ParseUserData(const XMLElement* element, HashMap<HashString,
	STxt>& user_data, ErrorLogger* logger) {
	// Parse any custom drx3D-specific info.
	for (const XMLElement* bullet_xml = element->FirstChildElement("bullet");
		...
		for (const XMLElement* user_data_xml = bullet_xml->FirstChildElement("user-data");
			...
			tukk key_attr = user_data_xml->Attribute("key");
			if (!key_attr) {
				logger->reportError("User data tag must have a key attribute.");
			}
			tukk text = user_data_xml->GetText();
	...
	// color
	{
		...
		if (c)
		{
			if (c->Attribute("rgba"))
			{
				...
				{
					STxt msg = material.m_name + " has no rgba";
...
bool UrdfParser::parseTransform(Transform2& tr, XMLElement* xml, ErrorLogger* logger, bool parseSDF)
{
	tr.setIdentity();
	...
	if (parseSDF)
	{
		parseVector3(vec, STxt(xml->GetText()), logger);
	}
	else
	{
		tukk xyz_str = xml->Attribute("xyz");
		if (xyz_str)
		{
			parseVector3(vec, STxt(xyz_str), logger);
		}
	}
	...
	{
		Vec3 rpy;
		if (parseVector3(rpy, STxt(xml->GetText()), logger, true))
		{
			double phi, the, psi;
			double roll = rpy[0];
			double pitch = rpy[1];
	...
	{
		tukk rpy_str = xml->Attribute("rpy");
		if (rpy_str != NULL)
		{
			Vec3 rpy;
			if (parseVector3(rpy, STxt(rpy_str), logger))
			{
				double phi, the, psi;
				double roll = rpy[0];
				double pitch = rpy[1];
...
bool UrdfParser::parseGeometry(UrdfGeometry& geom, XMLElement* g, ErrorLogger* logger)
{
	...
	//const STxt type_name = shape->ValueTStr().c_str();
	const STxt type_name = shape->Val();
	if (type_name == "sphere")
	{
		...
		else
		{
			...
			if (shape->Attribute("scale"))
			{
				if (!parseVector3(geom.m_meshScale, shape->Attribute("scale"), logger))
				{
					...
					STxt scalar_str = shape->Attribute("scale");
					double scaleFactor = urdfLexicalCast<double>(scalar_str.c_str());
					if (scaleFactor)
					{
						geom.m_meshScale.setVal(scaleFactor, scaleFactor, scaleFactor);
					}
				}
			}
		}
...
bool UrdfParser::parseCollision(UrdfCollision& collision, XMLElement* config, ErrorLogger* logger)
{
	...
	{
		tukk group_char = config->Attribute("group");
		if (group_char)
		{
			collision.m_flags |= URDF_HAS_COLLISION_GROUP;
			collision.m_collisionGroup = urdfLexicalCast<i32>(group_char);
		}
	}

	{
		tukk mask_char = config->Attribute("mask");
		if (mask_char)
		{
			collision.m_flags |= URDF_HAS_COLLISION_MASK;
			collision.m_collisionMask = urdfLexicalCast<i32>(mask_char);
		}
	}

	tukk name_char = config->Attribute("name");
	if (name_char)
		collision.m_name = name_char;

	tukk concave_char = config->Attribute("concave");
	if (concave_char)
	...
	if (mat)
	{
		if (m_parseSDF)
		{
			...
			model.m_materials.insert(matPtr->m_name.c_str(), matPtr);
			{
				...
				if (diffuse)
				{
					STxt diffuseText = diffuse->GetText();
					Vec4 rgba(1, 0, 0, 1);
			...
			}
			{
				...
				if (specular)
				{
					STxt specularText = specular->GetText();
					Vec3 rgba(1, 1, 1);
...
{
	tukk linkName = config->Attribute("name");
	if (!linkName)
	{
...
{
	tukk mu = config->Attribute("mu");
	tukk lambda = config->Attribute("lambda");
	tukk damping = config->Attribute("damping");
	if (!mu || !lambda)
	{
...
bool UrdfParser::parseDeformable(UrdfModel& model, tinyxml2::XMLElement* config, ErrorLogger* logger)
{
	...
	tukk deformableName = config->Attribute("name");
	if (!deformableName)
	{
	...
	if (spring_xml)
	{
		...
		if (spring_xml->Attribute("bending_stiffness"))
		{
			...
			if (spring_xml->Attribute("bending_stride"))
				deformable.m_springCoefficients.bending_stride = urdfLexicalCast<i32>(spring_xml->Attribute("bending_stride"));
		}
	}
	...
	STxt fn = vis_xml->Attribute("filename");
...
bool UrdfParser::parseReducedDeformable(UrdfModel& model, tinyxml2::XMLElement* config, ErrorLogger* logger)
{
	...
	tukk name = config->Attribute("name");
	if (!name)
	{
...
bool UrdfParser::parseJointLimits(UrdfJoint& joint, XMLElement* config, ErrorLogger* logger)
{
	...
	{
		tukk lower_str = config->Attribute("lower");
		if (lower_str)
		{
			joint.m_lowerLimit = urdfLexicalCast<double>(lower_str);
		}

		tukk upper_str = config->Attribute("upper");
		if (upper_str)
		{
		...
		tukk twist_str = config->Attribute("twist");
		if (twist_str)
		{
		...
		tukk effort_str = config->Attribute("effort");
		if (effort_str)
		{
		...
		tukk velocity_str = config->Attribute("velocity");
		if (velocity_str)
		{
...
bool UrdfParser::parseJointDynamics(UrdfJoint& joint, XMLElement* config, ErrorLogger* logger)
{
	...
	{
		// Get joint damping
		tukk damping_str = config->Attribute("damping");
		if (damping_str)
		{
		...
		// Get joint friction
		tukk friction_str = config->Attribute("friction");
		if (friction_str)
		{
...
bool UrdfParser::parseJoint(UrdfJoint& joint, XMLElement* config, ErrorLogger* logger)
{
	...
	if (parent_xml)
	{
		if (m_parseSDF)
		{
			joint.m_parentLinkName = STxt(parent_xml->GetText());
		}
		else
		{
			tukk pname = parent_xml->Attribute("link");
			if (!pname)
			{
			...
			{
				joint.m_parentLinkName = STxt(pname);
			}
		}
	}
	...
	if (child_xml)
	{
		if (m_parseSDF)
		{
			joint.m_childLinkName = STxt(child_xml->GetText());
		}
		else
		{
			tukk pname = child_xml->Attribute("link");
			if (!pname)
			{
			...
			{
				joint.m_childLinkName = STxt(pname);
			}
		}
	}

	// Get Joint type
	tukk type_char = config->Attribute("type");
	if (!type_char)
	{
	...
	if (m_parseSDF)
	{
		if (joint.m_type != URDFFloatingJoint && joint.m_type != URDFFixedJoint)
		{
			...
			if (!axis_xml)
			{
				STxt msg("urdfdom: no axis element for Joint, defaulting to (1,0,0) axis");
				msg = msg + " " + joint.m_name + "\n";
				logger->reportWarning(msg.c_str());
				joint.m_localJointAxis.setVal(1, 0, 0);
			}
			else
			{
				...
				if (xyz_xml)
				{
					if (!parseVector3(joint.m_localJointAxis, STxt(xyz_xml->GetText()), logger))
					{
	...
	else
	{
		...
		{
			// Get joint damping
			tukk damping_str = prop_xml->Attribute("damping");
			if (damping_str)
			{
				joint.m_jointDamping = urdfLexicalCast<double>(damping_str);
			}
			...
			// Get joint friction
			tukk friction_str = prop_xml->Attribute("friction");
			if (friction_str)
			{
				joint.m_jointFriction = urdfLexicalCast<double>(friction_str);
			}
...
bool UrdfParser::parseSensor(UrdfModel& model, UrdfLink& link, UrdfJoint& joint, XMLElement* config, ErrorLogger* logger)
{
	...
	tukk sensorName = config->Attribute("name");
	if (!sensorName)
	{
	...
	joint.m_name = STxt(sensorName).append("_Joint");
	joint.m_childLinkName = sensorName;
	joint.m_type = URDFFixedJoint;
	joint.m_localJointAxis.setVal(0, 0, 0);
...
static void CalculatePrincipalAxisTransform(Scalar* masses, const Transform2* transforms, Matrix3x3* inertiasIn, Transform2& principal, Vec3& inertiaOut)
{
	i32 n = 2;
	...
	for (k = 0; k < n; k++)
	{
		...
		const Transform2& t = transforms[k];
	...
	inertiaOut.setVal(tensor[0][0], tensor[1][1], tensor[2][2]);
}

bool UrdfParser::mergeFixedLinks(UrdfModel& model, UrdfLink* link, ErrorLogger* logger, bool forceFixedBase, i32 level)
{
	for (i32 l = 0; l < level; l++)
	{
		printf("\t");
	}
	printf("processing %s\n", link->m_name.c_str());
	
	for (i32 i = 0; i < link->m_childJoints.size();)
	{
		if (link->m_childJoints[i]->m_type == URDFFixedJoint)
		{
			...
			for (i32 l = 0; l < level+1; l++)
			{
				printf("\t");
			}
			//mergeChildLink
			printf("merge %s into %s!\n", childLink->m_name.c_str(), link->m_name.c_str());
			for (i32 c = 0; c < childLink->m_collisionArray.size(); c++)
			{
			...
			for (i32 c = 0; c < childLink->m_visualArray.size(); c++)
			{
			...
			if (!isStaticBase)
			{
				Scalar masses[2] = { (Scalar)link->m_inertia.m_mass, (Scalar)childLink->m_inertia.m_mass };
				Transform2 transforms[2] = { link->m_inertia.m_linkLocalFrame, childJoint->m_parentLinkToJointTransform * childLink->m_inertia.m_linkLocalFrame };
				Matrix3x3 inertiaLink(
				...
				Matrix3x3 inertiasIn[2] = { inertiaLink, inertiaChild };
				Vec3 inertiaOut;
...
bool UrdfParser::printTree(UrdfLink* link, ErrorLogger* logger, i32 level)
{
	printf("\n");
	for (i32 l = 0; l < level; l++)
	{
	...
	for (i32 i = 0; i<link->m_childJoints.size(); i++)
	{
		printTree(link->m_childLinks[i], logger, level + 1);
		Assert(link->m_childJoints[i]->m_parentLinkName == link->m_name);
	}
	return true;
}
...
bool UrdfParser::recreateModel(UrdfModel& model, UrdfLink* link, ErrorLogger* logger)
{
	...
	for (i32 i = 0; i < link->m_childJoints.size(); i++)
	{
		...
		tukk childName = link->m_childLinks[i]->m_name.c_str();
		UrdfLink* childLink = link->m_childLinks[i];
		model.m_links.insert(childName, childLink);
		tukk jointName = link->m_childLinks[i]->m_parentJoint->m_name.c_str();
...
bool UrdfParser::loadUrdf(tukk urdfText, ErrorLogger* logger, bool forceFixedBase, bool parseSensors)
{
	XMLDocument xml_doc;
	...
	// Get robot name
	tukk name = robot_xml->Attribute("name");
	if (!name)
	{
  ...
  if (deformable_xml) {
	...
	if (forceFixedBase)
	{
		for (i32 i = 0; i < m_urdf2Model.m_rootLinks.size(); i++)
		{
...
void UrdfParser::activateModel(i32 modelIndex)
{
	m_activeSdfModel = modelIndex;
}

bool UrdfParser::loadSDF(tukk sdfText, ErrorLogger* logger)
{
	XMLDocument xml_doc;
...
STxt UrdfParser::sourceFileLocation(XMLElement* e)
{
#if 0
...
#endif
	STxt str = m_urdf2Model.m_sourceFile.c_str() + STxt(":") + STxt(row);
	return str;
#endif
}
