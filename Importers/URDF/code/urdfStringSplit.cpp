...
void urdfStringSplit(AlignedObjectArray<STxt> &pieces, const STxt &vector_str, const AlignedObjectArray<STxt> &separators)
{
	...
	if (separators.size() == 1)
	{
		...
		i32 numSubStr = urdfStrArrayLen(strArray);
...
{
	i32 numSeps = strlen(seps);
	for (i32 i = 0; i < numSeps; i++)
	{
		char sep2[2] = {0, 0};
...
char **urdfStrArrayAppend(char **array, size_t nitems, tukk item,
						  size_t itemlen)
{
...
char **urdfStrSplit(tukk input, tukk sep)
{
	size_t nitems = 0;
	char **array = NULL;
	tukk start = input;
