...
void CLiveCreateCmd_ObjectAreaUpdate::Execute()
{
	...
	for (u32 i = 0; i < m_layerNames.size(); ++i)
	{
		u16k dataId = m_layerIds[i];
		if (dataId < kMaxLayers)
		{
			i32k layerId = gEnv->pEntitySystem->GetLayerId(m_layerNames[i].c_str());
			if (layerId == -1)
			{
...
void CLiveCreateCmd_EntitySetMaterial::Execute()
{
	...
	if (NULL != pEntity)
	{
		...
		{
			// load the material
			IMaterial* pMaterial = gEnv->p3DEngine->GetMaterialUpr()->FindMaterial(m_materialName.c_str());
			if (NULL == pMaterial)
			{
				pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(m_materialName.c_str());
				if (NULL == pMaterial)
				{
