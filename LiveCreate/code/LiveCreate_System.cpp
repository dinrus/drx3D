...
void CLiveCreateCmd_EnableLiveCreate::Execute()
{
	...
	// show layers
	for (u32 i = 0; i < m_visibleLayers.size(); ++i)
	{
		const string& layerName = m_visibleLayers[i];

		gEnv->pEntitySystem->ToggleLayerVisibility(layerName.c_str(), true);

		i32k layerIndex = gEnv->pEntitySystem->GetLayerId(layerName.c_str());
		if (-1 != layerIndex)
		{
			const bool bCheckLayerActivation = false;
			gEnv->p3DEngine->ActivateObjectsLayer((u16)layerIndex, true, true, true, true, "ALL_LAYERS", NULL, bCheckLayerActivation);
		}
	...
	// hide layers
	for (u32 i = 0; i < m_hiddenLayers.size(); ++i)
	{
		...
		{
			const bool bCheckLayerActivation = false;
			gEnv->p3DEngine->ActivateObjectsLayer((u16)layerIndex, false, true, true, true, "ALL_LAYERS", NULL, bCheckLayerActivation);
		}
	}
}
...
{
	XmlNodeRef node = gEnv->pSystem->LoadXmlFromBuffer((tukk)&m_data[0], m_data.size());
	if (node)
	{
...
void CLiveCreateCmd_SetArchetypesFull::Execute()
{
	...
	if (node)
	{
		...
		i32k numChildren = node->getChildCount();
...
void CLiveCreateCmd_SetParticlesFull::Execute()
{
	...
	if (node)
	{
		...
		i32k nNumberOfChildren = oParticlesLibrary->getChildCount();
		for (i32 nCurrentChild = 0; nCurrentChild < nNumberOfChildren; ++nCurrentChild)
		{
		...
		#undef LoadLibrary
	#endif
			gEnv->pParticleUpr->LoadLibrary(strLibraryName.c_str(), oLibrary, true);
		}
	}
}
...
void CLiveCreateCmd_FileSynced::Execute()
{
	...
	if (fileExt == "mtl")
	{
		...
		gEnv->p3DEngine->GetMaterialUpr()->LoadMaterialFromXml(str.c_str(), xml);
	}
	else
	{
...
void CLiveCreateCmd_SyncLayerVisibility::Execute()
{
	if (gEnv->pEntitySystem)
	{
		...
		i32k layerIndex = gEnv->pEntitySystem->GetLayerId(m_layerName.c_str());
		if (-1 != layerIndex)
		{
			const bool bCheckLayerActivation = false;
			gEnv->p3DEngine->ActivateObjectsLayer((u16)layerIndex, m_bIsVisible, true, true, true, "ALL_LAYERS", NULL, bCheckLayerActivation);
		}
